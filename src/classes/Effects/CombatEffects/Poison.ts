...
export class Poison extends CombatEffect {
    public update(character: Character) {
        if (character.perks.has(PerkType.Medicine) && Utils.rand(100) <= 14) {
            ...
            DisplayText.text("You manage to cleanse the poison from your system with your knowledge of medicine!");
        }
        else {
            character.combat.stats.loseHP(8 + Utils.rand(character.stats.maxHP() / 20), null);
            DisplayText.text("The poison continues to work on your body, wracking you with pain!");
        }
