package classes {
	...
	public class PlayerEvents extends BaseContent implements TimeAwareInterface {
		...
		public function timeChange():boolean {
			...
			if (flags[FlagEnum.SOCK_COUNTER] > 0) {
				flags[FlagEnum.SOCK_COUNTER]--;
				if (flags[FlagEnum.SOCK_COUNTER] < 0) flags[FlagEnum.SOCK_COUNTER] = 0;
				if (flags[FlagEnum.SOCK_COUNTER] > 24) flags[FlagEnum.SOCK_COUNTER] = 24;
			}
			...
			if (!player.perks.has("WellAdjusted")) {
				getGame().dynStats("lus", player.stats.lib * 0.04, "resisted", false); //Raise lust
				if (player.perks.has("Lusty")) getGame().dynStats("lus", player.stats.lib * 0.02, "resisted", false); //Double lust rise if lusty.
			}
			else { //Well adjusted perk
				getGame().dynStats("lus", player.stats.lib * 0.02); //Raise lust
				if (player.perks.has("Lusty")) getGame().dynStats("lus", player.stats.lib * 0.01, "resisted", false); //Double lust rise if lusty.
			}
			if (player.tailType == TAIL.BEE_ABDOMEN || player.tailType == TAIL.SPIDER_ADBOMEN) { //Spider and Bee Sting Recharge
			...
			if (flags[FlagEnum.FOX_BAD_END_WARNING] == 1) {
				if (player.faceType != FACE.FOX || player.tailType != TAIL.FOX || player.upperBody.head.earType != EARS.FOX || player.lowerBody != LOWER_BODY.FOX || player.skinType != SKIN.FUR) {
					flags[FlagEnum.FOX_BAD_END_WARNING] = 0;
				}
			}
			...
			if (player.flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00285] >= 50 && !player.perks.has("LuststickAdapted")) { //Luststick resistance unlock
				getGame().sophieBimbo.unlockResistance();
				if (player.statusAffects.has("Luststick")) player.statusAffects.remove("Luststick");
				needNext = true;
			}
			if (flags[FlagEnum.DICK_EGG_INCUBATION] > 0) {
				flags[FlagEnum.DICK_EGG_INCUBATION]--;
				trace("DICK BIRTH TIMER: " + flags[FlagEnum.DICK_EGG_INCUBATION]);
				if (flags[FlagEnum.DICK_EGG_INCUBATION] == 1) {
			...
			if (player.inRut) { //Rut v1 is bonus cum, v2 is bonus libido, v3 is hours till it's gone
				...
				if (player.statusAffects.get("$1").value3 <= 1 || player.totalCocks() == 0) { //Remove bonus libido from rut
					getGame().dynStats("lib", -player.statusAffects.get("Rut").value2, "resisted", false, "noBimbo", true);
			...
			//flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00330] prevents PC getting two of the same notices overnite
			else if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00330] > 0) flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00330]--;
			...
			if (player.statusAffects.has("WormPlugged") && flags[FlagEnum.PLAYER_PREGGO_WITH_WORMS] == 0) { //Update worm drippy-cooch
			...
			if (player.findStatusAffect(StatusAffects.LactationReduction) < 0) { //Lactation reduction
				if (player.biggestLactation() > 0) player.statusAffects.add(new StatusAffect("LactationReduction", 0, 0, 0, 0)));
			}
			...
			if (flags[FlagEnum.BIKINI_ARMOR_BONUS] > 0) {
				if (player.armorName == "lusty maiden's armor") {
					if (getGame().model.time.hours == 0) flags[FlagEnum.BIKINI_ARMOR_BONUS]--; //Adjust for inflation
					if (flags[FlagEnum.BIKINI_ARMOR_BONUS] < 0) flags[FlagEnum.BIKINI_ARMOR_BONUS] = 0; //Keep in bounds.
					if (flags[FlagEnum.BIKINI_ARMOR_BONUS] > 8) flags[FlagEnum.BIKINI_ARMOR_BONUS] = 8;
				}
				else flags[FlagEnum.BIKINI_ARMOR_BONUS] = 0;
			}
			
			//No better place for these since the code for the event is part of CoC.as or one of its included files
			if (flags[FlagEnum.TIME_SINCE_VALA_ATTEMPTED_RAPE_PC] > 0) flags[FlagEnum.TIME_SINCE_VALA_ATTEMPTED_RAPE_PC]--; //Vala post-rape countdown
			if (flags[FlagEnum.GATS_ANGEL_TIME_TO_FIND_KEY] > 0 && flags[FlagEnum.GATS_ANGEL_TIME_TO_FIND_KEY] < 500) flags[FlagEnum.GATS_ANGEL_TIME_TO_FIND_KEY]++;
			
			if (getGame().model.time.hours > 23) { //Once per day
				flags[FlagEnum.BROOKE_MET_TODAY] = 0;
				if (getGame().model.time.days % 2 == 0 && flags[FlagEnum.KAIJU_BAD_END_COUNTER] > 0) {
					flags[FlagEnum.KAIJU_BAD_END_COUNTER]--;
					if (flags[FlagEnum.KAIJU_BAD_END_COUNTER] < 0) flags[FlagEnum.KAIJU_BAD_END_COUNTER] = 0;
				}
				if (flags[FlagEnum.GILDED_JERKED] > 0) flags[FlagEnum.GILDED_JERKED] = 0;
				if (flags[FlagEnum.FED_SCYLLA_TODAY] == 1) flags[FlagEnum.FED_SCYLLA_TODAY] = 0;
				if (flags[FlagEnum.NOT_HELPED_ARIAN_TODAY] != 0) flags[FlagEnum.NOT_HELPED_ARIAN_TODAY] = 0;
				if (flags[FlagEnum.RUBI_PROSTITUTION] > 0) flags[FlagEnum.RUBI_PROFIT] += 2 + rand(4);
				flags[FlagEnum.BENOIT_TALKED_TODAY] = 0;
				getGame().bazaar.benoit.updateBenoitInventory();
				flags[FlagEnum.ROGAR_FUCKED_TODAY] = 0;
				if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00285] > 0) flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00285]--; //Reduce lust-stick resistance building
				if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00155] > 0) { //Dominika fellatrix countdown
					flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00155]--;
					if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00155] < 0) flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00155] = 0;
				}
				if (flags[FlagEnum.LOPPE_DENIAL_COUNTER] > 0) { //Loppe denial counter
					flags[FlagEnum.LOPPE_DENIAL_COUNTER]--;
					if (flags[FlagEnum.LOPPE_DENIAL_COUNTER] < 0) flags[FlagEnum.LOPPE_DENIAL_COUNTER] = 0;
				}
				if (flags[FlagEnum.WEEKLY_FAIRY_ORGY_COUNTDOWN] > 0) { //Countdown to next faerie orgy
					flags[FlagEnum.WEEKLY_FAIRY_ORGY_COUNTDOWN]--;
					if (flags[FlagEnum.WEEKLY_FAIRY_ORGY_COUNTDOWN] < 0) flags[FlagEnum.WEEKLY_FAIRY_ORGY_COUNTDOWN] = 0;
				}
				if (getGame().model.time.days % 7 == 0) flags[FlagEnum.WHITNEY_GEMS_PAID_THIS_WEEK] = 0; //Clear Whitney's Weekly limit
				if (flags[FlagEnum.USED_MILKER_TODAY] > 0) flags[FlagEnum.USED_MILKER_TODAY] = 0; //Clear 'has fucked milker today'
		...
		public function timeChangeLarge():boolean {
			...
			if (getGame().model.time.hours == 1 && getGame().isHolidays() && getGame().date.fullYear > flags[FlagEnum.PC_ENCOUNTERED_CHRISTMAS_ELF_BEFORE]) { //XMAS ELF
				getGame().xmasBitchEncounter(); //Set it to remember the last year encountered
				return true;
			}
			if (checkedTurkey++ == 0 && (rand(5) == 0 && (getGame().model.time.hours == 18 || getGame().model.time.hours == 19)) && (getGame().date.fullYear > flags[FlagEnum.TURKEY_FUCK_YEAR_DONE] || flags[FlagEnum.MORE_TURKEY] > 0) && getGame().isThanksgiving() && player.gender > 0) {
			...
			if (checkedDream++ == 0 && getGame().model.time.hours == 3) { //You can only have one dream each night
				...
				switch (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00218] + flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00219] + flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00220]) {
				...
				if (getGame().anemoneScene.kidAXP() >= 40 && flags[FlagEnum.HAD_KID_A_DREAM] == 0 && player.gender > 0) {
					getGame().anemoneScene.kidADreams();
					flags[FlagEnum.HAD_KID_A_DREAM] = 1;
					return true;
				}
				...
				if (player.stats.lib > 50 || player.lust > 40) { //Randomly generated dreams here
					if (getGame().dreamSelect()) return true;
				}
			}
			if (player.statusAffects.get("SlimeCraving").value1 >= 18 && player.str <= 1) { //Bad end!
