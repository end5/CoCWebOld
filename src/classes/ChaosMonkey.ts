package classes
{
	...
	{	
		...
		*/
		public let run:boolean
		private let _excludeMenuKeys:boolean;
		private let _catchOutputTextErrors:boolean;
		private let _mainClassPtr:CoC;
		private let _stage:Stage;
		private let debug:boolean;
		private let buttons:Array;

		private let exitKeyCode:number = 123;
		...
		private function initAvailableKeysList():void
		{
			...
			*/
			...
			let blockedButtons:Array = [112,     // Quicksave buttons
											113,
											114,
											115,
											116,
			...
			for (let button:string in controlMethods)
			{
		...
		CONFIG::debug 
		{
			public function catchGlobalError(event:UncaughtErrorEvent):void
			{
				...
				{
					let tempError:Error = event.error as Error;
					stackTrace = tempError.getStackTrace();
					// do something with the error
				}
				else if (event.error is ErrorEvent)
				{
					let errorEvent:ErrorEvent = event.error as ErrorEvent;
		...
		private function setupExitKey():void
		{
			...
			let stopMonkahTempFunc:Function = function(callFunc:Function):Function
			{ 
		...
		public function throwAMonkeyAtIt(e:*):void
		{
			...
			{
				let fakeEvent:KeyboardEvent = new KeyboardEvent(KeyboardEvent.KEY_DOWN);
