export default class Camp extends NPCAwareContent {
	...
	*/
	...
	private doCamp(): void { //Only called by playerMenu
		...
		*/
		...
		Flags.list[FlagEnum.BONUS_ITEM_AFTER_COMBAT_ID] = "";
		//History perk backup
		if (Flags.list[FlagEnum.HISTORY_PERK_SELECTED] == 0) {
			Flags.list[FlagEnum.HISTORY_PERK_SELECTED] = 2;
			hideMenus();
		...
		if (!marbleScene.marbleFollower()) {
			if (Flags.list[FlagEnum.MARBLE_LEFT_OVER_CORRUPTION] == 1 && player.stats.cor <= 40) {
				hideMenus();
		...
		if (marbleScene.marbleFollower()) {
			...
			if (player.stats.cor < 50 && !campCorruptJojo() && !amilyScene.amilyCorrupt() && !vapulaSlave()
				&& Flags.list[FlagEnum.MARBLE_PURIFICATION_STAGE] == 0 && Flags.list[FlagEnum.MARBLE_COUNTUP_TO_PURIFYING] >= 200
			...
			if (Flags.list[FlagEnum.MARBLE_PURIFICATION_STAGE] >= 5) {
				if (Flags.list[FlagEnum.MARBLE_WARNED_ABOUT_CORRUPTION] == 0 && player.stats.cor >= 50) {
					hideMenus();
					marblePurification.marbleWarnsPCAboutCorruption();
					return;
				}
				if (Flags.list[FlagEnum.MARBLE_WARNED_ABOUT_CORRUPTION] == 1 && Flags.list[FlagEnum.MARBLE_LEFT_OVER_CORRUPTION] == 0 && player.stats.cor >= 60) {
					hideMenus();
					marblePurification.marbleLeavesThePCOverCorruption();
					return;
				}
			}
			if (Flags.list[FlagEnum.MARBLE_RATHAZUL_COUNTER_1] == 1 && (time.hours == 6 || time.hours == 7)) {
				hideMenus();
				marblePurification.rathazulsMurbelReport();
				return;
			}
			if (Flags.list[FlagEnum.MARBLE_RATHAZUL_COUNTER_2] == 1) {
				hideMenus();
				marblePurification.claraShowsUpInCampBECAUSESHESACUNT();
				return;
			}
		}
		if (arianFollower() && Flags.list[FlagEnum.ARIAN_MORNING] == 1) {
			hideMenus();
			arianScene.wakeUpAfterArianSleep();
			return;
		}
		if (arianFollower() && Flags.list[FlagEnum.ARIAN_EGG_EVENT] >= 30) {
			hideMenus();
			arianScene.arianEggingEvent();
			return;
		}
		if (arianFollower() && Flags.list[FlagEnum.ARIAN_EGG_COUNTER] >= 24 && Flags.list[FlagEnum.ARIAN_VAGINA] > 0) {
			hideMenus();
			arianScene.arianLaysEggs();
			return;
		}
		if (Flags.list[FlagEnum.JACK_FROST_PROGRESS] > 0) {
			hideMenus();
		...
		if (kGAMECLASS.nieveHoliday() && model.time.hours == 6) {
			if (player.hasKeyItem("Nieve's Tear") >= 0 && Flags.list[FlagEnum.NIEVE_STAGE] != 5) {
				kGAMECLASS.returnOfNieve();
				hideMenus();
				return;
			}
			else if (Flags.list[FlagEnum.NIEVE_STAGE] == 0) {
				hideMenus();
				kGAMECLASS.snowLadyActive();
				return;
			}
			else if (Flags.list[FlagEnum.NIEVE_STAGE] == 4) {
				hideMenus();
		...
		if (kGAMECLASS.helScene.followerHel()) {
			if (helFollower.isHeliaBirthday() && Flags.list[FlagEnum.HEL_FOLLOWER_LEVEL] >= 2 && Flags.list[FlagEnum.HELIA_BIRTHDAY_OFFERED] == 0) {
				hideMenus();
		...
		if (Flags.list[FlagEnum.HELSPAWN_AGE] == 1 && Flags.list[FlagEnum.HELSPAWN_GROWUP_COUNTER] == 7) {
			hideMenus();
			helSpawnScene.helSpawnGraduation();
			return;
		}
		if (model.time.hours >= 10 && model.time.hours <= 18 && (model.time.days % 20 == 0 || model.time.hours == 12) && Flags.list[FlagEnum.HELSPAWN_DADDY] == 2 && helSpawnScene.helspawnFollower()) {
			hideMenus();
			helSpawnScene.maiVisitsHerKids();
			return;
		}
		if (model.time.hours == 6 && Flags.list[FlagEnum.HELSPAWN_DADDY] == 1 && model.time.days % 30 == 0 && Flags.list[FlagEnum.SPIDER_BRO_GIFT] == 0 && helSpawnScene.helspawnFollower()) {
			hideMenus();
			helSpawnScene.spiderBrosGift();
			return;
		}
		if (model.time.hours >= 10 && model.time.hours <= 18 && (model.time.days % 15 == 0 || model.time.hours == 12) && helSpawnScene.helspawnFollower() && Flags.list[FlagEnum.HAKON_AND_KIRI_VISIT] == 0) {
			hideMenus();
			helSpawnScene.hakonAndKiriComeVisit();
			return;
		}
		if (Flags.list[FlagEnum.HELSPAWN_AGE] == 2 && Flags.list[FlagEnum.HELSPAWN_DISCOVER_BOOZE] == 0 && (rand(10) == 0 || Flags.list[FlagEnum.HELSPAWN_GROWUP_COUNTER] == 6)) {
			hideMenus();
			helSpawnScene.helspawnDiscoversBooze();
			return;
		}
		if (Flags.list[FlagEnum.HELSPAWN_AGE] == 2 && Flags.list[FlagEnum.HELSPAWN_WEAPON] == 0 && Flags.list[FlagEnum.HELSPAWN_GROWUP_COUNTER] == 3 && model.time.hours >= 10 && model.time.hours <= 18) {
			hideMenus();
			helSpawnScene.helSpawnChoosesAFightingStyle();
			return;
		}
		if (Flags.list[FlagEnum.HELSPAWN_AGE] == 2 && (model.time.hours == 6 || model.time.hours == 7) && Flags.list[FlagEnum.HELSPAWN_GROWUP_COUNTER] == 7 && Flags.list[FlagEnum.HELSPAWN_FUCK_INTERRUPTUS] == 1) {
			helSpawnScene.helspawnAllGrownUp();
			return;
		}
		if ((sophieFollower() || bimboSophie()) && Flags.list[FlagEnum.SOPHIE_DAUGHTER_MATURITY_COUNTER] == 1) {
			Flags.list[FlagEnum.SOPHIE_DAUGHTER_MATURITY_COUNTER] = 0;
			sophieBimbo.sophieKidMaturation();
			hideMenus();
			return;
		}
		//Bimbo Sophie Move In Request!
		if (bimboSophie() && Flags.list[FlagEnum.SOPHIE_BROACHED_SLEEP_WITH] == 0 && sophieScene.pregnancy.event >= 2) {
			hideMenus();
			sophieBimbo.sophieMoveInAttempt();
			return;
		}
		if (!kGAMECLASS.nieveHoliday() && model.time.hours == 6 && Flags.list[FlagEnum.NIEVE_STAGE] > 0) {
			kGAMECLASS.nieveIsOver();
			return;
		}
		//Amily followup!
		if (Flags.list[FlagEnum.PC_PENDING_PREGGERS] == 1) {
			kGAMECLASS.amilyScene.postBirthingEndChoices();
			Flags.list[FlagEnum.PC_PENDING_PREGGERS] = 2;
			return;
		}
		if (timeQ > 0) {
			if (!campQ) {
		...
		if (Flags.list[FlagEnum.FUCK_FLOWER_KILLED] == 0 && Flags.list[FlagEnum.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE] > 0) {
			if (Flags.list[FlagEnum.FUCK_FLOWER_LEVEL] == 0 && Flags.list[FlagEnum.FUCK_FLOWER_GROWTH_COUNTER] >= 8) {
				holliScene.getASprout();
				hideMenus();
				return;
			}
			if (Flags.list[FlagEnum.FUCK_FLOWER_LEVEL] == 1 && Flags.list[FlagEnum.FUCK_FLOWER_GROWTH_COUNTER] >= 7) {
				holliScene.fuckPlantGrowsToLevel2();
				hideMenus();
				return;
			}
			if (Flags.list[FlagEnum.FUCK_FLOWER_LEVEL] == 2 && Flags.list[FlagEnum.FUCK_FLOWER_GROWTH_COUNTER] >= 25) {
				holliScene.flowerGrowsToP3();
				hideMenus();
				return;
			}
			//Level 4 growth
			if (Flags.list[FlagEnum.FUCK_FLOWER_LEVEL] == 3 && Flags.list[FlagEnum.FUCK_FLOWER_GROWTH_COUNTER] >= 40) {
		...
		//Jojo treeflips!
		if (Flags.list[FlagEnum.FUCK_FLOWER_LEVEL] >= 4 && Flags.list[FlagEnum.FUCK_FLOWER_KILLED] == 0 && player.statusAffects.has("PureCampJojo")) {
			holliScene.JojoTransformAndRollOut();
			hideMenus();
			return;
		}
		//Amily flips out
		if (amilyScene.amilyFollower() && !amilyScene.amilyCorrupt() && Flags.list[FlagEnum.FUCK_FLOWER_LEVEL] >= 4 && Flags.list[FlagEnum.FUCK_FLOWER_KILLED] == 0) {
			holliScene.amilyHatesTreeFucking();
			hideMenus();
			return;
		}
		if (Flags.list[FlagEnum.FUCK_FLOWER_KILLED] == 1 && Flags.list[FlagEnum.AMILY_TREE_FLIPOUT] == 1 && !amilyScene.amilyFollower() && Flags.list[FlagEnum.AMILY_VISITING_URTA] == 0) {
			holliScene.amilyComesBack();
			Flags.list[FlagEnum.AMILY_TREE_FLIPOUT] = 2;
			hideMenus();
			return;
		}
		...
		//Izzys tits asplode
		if (isabellaFollower() && Flags.list[FlagEnum.ISABELLA_MILKED_YET] >= 10 && player.hasKeyItem("Breast Milker - Installed At Whitney's Farm") >= 0) {
			isabellaFollowerScene.milktasticLacticLactation();
			hideMenus();
			return;
		}
		//Marble meets follower izzy when moving in
		if (Flags.list[FlagEnum.ISABELLA_MURBLE_BLEH] == 1 && isabellaFollower() && player.statusAffects.has("CampMarble")) {
		...
		if (player.pregnancyIncubation <= 280 && player.pregnancyType == PregnancyType.COTTON &&
			Flags.list[FlagEnum.COTTON_KNOCKED_UP_PC_AND_TALK_HAPPENED] == 0 && (model.time.hours == 6 || model.time.hours == 7)) {
			kGAMECLASS.telAdre.cotton.goTellCottonShesAMomDad();
			hideMenus();
			return;
		}
		//Bimbo Sophie finds ovi elixer in chest!
		if (bimboSophie() && hasItemInStorage(consumables.OVIELIX) && rand(5) == 0 && Flags.list[FlagEnum.TIMES_SOPHIE_HAS_DRUNK_OVI_ELIXIR] == 0 && player.gender > 0) {
			sophieBimbo.sophieEggApocalypse();
			hideMenus();
			return;
		}
		//Amily + Urta freakout!
		if (!kGAMECLASS.urtaQuest.urtaBusy() && Flags.list[FlagEnum.AMILY_VISITING_URTA] == 0 && rand(10) == 0 && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] >= 0 && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00147] == 0 && Flags.list[FlagEnum.AMILY_NEED_TO_FREAK_ABOUT_URTA] == 1 && amilyScene.amilyFollower() && Flags.list[FlagEnum.AMILY_FOLLOWER] == 1 && !amilyScene.pregnancy.isPregnant) {
			finter.amilyUrtaReaction();
			hideMenus();
			return;
		}
		//Find jojo's note!
		if (Flags.list[FlagEnum.JOJO_FIXED_STATUS] == 1 && Flags.list[FlagEnum.AMILY_BLOCK_COUNTDOWN_BECAUSE_CORRUPTED_JOJO] == 0) {
			finter.findJojosNote();
			hideMenus();
			return;
		}
		//Rathazul freaks out about jojo
		if (Flags.list[FlagEnum.RATHAZUL_CORRUPT_JOJO_FREAKOUT] == 0 && rand(5) == 0 && player.statusAffects.has("CampRathazul") && campCorruptJojo()) {
			finter.rathazulFreaksOverJojo();
			hideMenus();
			return;
		}
		//Izma/Marble freakout - marble moves in
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00237] == 1) {
			izmaScene.newMarbleMeetsIzma();
			hideMenus();
			return;
		}
		//Izma/Amily freakout - Amily moves in
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00236] == 1) {
			izmaScene.newAmilyMeetsIzma();
			hideMenus();
			return;
		}
		//Amily/Marble Freakout
		if (Flags.list[FlagEnum.AMILY_NOT_FREAKED_OUT] == 0 && player.statusAffects.has("CampMarble") && Flags.list[FlagEnum.AMILY_FOLLOWER] == 1 && amilyScene.amilyFollower() && marbleScene.marbleAtCamp()) {
		...
		if (Flags.list[FlagEnum.HEL_FOLLOWER_LEVEL] == 2 && kGAMECLASS.helScene.followerHel() && Flags.list[FlagEnum.HEL_INTROS_LEVEL] == 0) {
		...
		if (Flags.list[FlagEnum.HEL_INTROS_LEVEL] > 9000 && kGAMECLASS.helScene.followerHel() && isabellaFollower() && Flags.list[FlagEnum.HEL_ISABELLA_THREESOME_ENABLED] == 0) {
			helFollower.angryHelAndIzzyCampHelHereFirst();
			hideMenus();
			return;
		}
		//Reset.
		Flags.list[FlagEnum.CAME_WORMS_AFTER_COMBAT] = 0;
		campQ = false;
		//Build explore menus
		...
		Flags.list[FlagEnum.PC_CURRENTLY_LUSTSTICK_AFFECTED] = 0;
		...
		//Build main menu
		let exploreEvent: Function = Game.exploration.doExplore;
		let masturbate: Function = (player.stats.lust > 30 ? Game.masturbation.masturbateMenu : null);
		MainScreen.clearText();
		...
		if (Flags.list[FlagEnum.CLARA_IMPRISONED] > 0) {
			marblePurification.claraCampAddition();
		}
		//Nursery
		if (Flags.list[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] == 100 && player.statusAffects.has("CampMarble")) {
			MainScreen.text("  Marble has built a fairly secure nursery amongst the rocks to house your ", false);
			if (Flags.list[FlagEnum.MARBLE_KIDS] == 0) MainScreen.text("future children", false);
			else {
				MainScreen.text(num2Text(Flags.list[FlagEnum.MARBLE_KIDS]) + " child", false);
				if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) MainScreen.text("ren", false);
			}
		...
		if (Flags.list[FlagEnum.SOPHIE_ADULT_KID_COUNT] > 0) {
			...
			else if (Flags.list[FlagEnum.SOPHIE_ADULT_KID_COUNT] <= 4) {
			...
			else if (Flags.list[FlagEnum.SOPHIE_ADULT_KID_COUNT] <= 10) {
			...
			else if (Flags.list[FlagEnum.SOPHIE_ADULT_KID_COUNT] <= 20) {
			...
			else if (Flags.list[FlagEnum.SOPHIE_ADULT_KID_COUNT] <= 50) {
		...
		if (Flags.list[FlagEnum.EMBER_CURRENTLY_FREAKING_ABOUT_MINOCUM] == 1) {
		...
		//BIMBO SOPHAH
		if (bimboSophie() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) sophieBimbo.sophieCampLines();
		if (player.statusAffects.has("CampMarble")) {
			...
			//Marble is out!
			if (Flags.list[FlagEnum.MARBLE_PURIFICATION_STAGE] == 4) MainScreen.text("Marble isn’t here right now; she’s still off to see her family.");
			//requires at least 1 kid, time is just before sunset, this scene always happens at this time if the PC has at least one kid.
			else if (Flags.list[FlagEnum.MARBLE_KIDS] >= 1 && (model.time.hours == 19 || model.time.hours == 20)) {
				MainScreen.text("Marble herself is currently in the nursery, putting your ");
				if (Flags.list[FlagEnum.MARBLE_KIDS] == 1) MainScreen.text("child");
			...
			//after nightfall, scene always displays at this time unless PC is wormed
			else if (model.time.hours >= 21 && !player.statusAffects.has("Infested")) {
				...
				if (Flags.list[FlagEnum.MARBLE_LUST] > 30) MainScreen.text("  She seems to be feeling antsy.");
			}
			else if (Flags.list[FlagEnum.MARBLE_KIDS] > 0 && model.time.hours < 19 && model.time.hours > 7) {
				//requires at least 6 kids, and no other parental characters in camp
				if (rand(2) == 0 && Flags.list[FlagEnum.MARBLE_KIDS] > 5) MainScreen.text("Marble is currently tending to your kids, but she looks a bit stressed out right now.  It looks like " + num2Text(Flags.list[FlagEnum.MARBLE_KIDS]) + " might just be too many for her to handle on her own...");
				//requires at least 4 kids
				else if (rand(3) == 0 && Flags.list[FlagEnum.MARBLE_KIDS] > 3) MainScreen.text("Marble herself is in the camp right now, telling a story about her travels around the world to her kids as they gather around her.  The children are completely enthralled by her words.  You can't help but smile.");
				//Requires 2 boys
				else if (rand(3) == 0 && Flags.list[FlagEnum.MARBLE_BOYS] > 1) {
				...
				else if (rand(3) == 0 && Flags.list[FlagEnum.MARBLE_KIDS] - Flags.list[FlagEnum.MARBLE_BOYS] > 1) MainScreen.text("Marble herself is involved in a play fight with two of your kids brandishing small sticks.  It seems that the <i>mommy monster</i> is terrorising the camp and needs to be stopped by the <i>Mighty Moo and her sidekick Bovine Lass</i>.");
				else if (rand(3) == 0 && Flags.list[FlagEnum.MARBLE_KIDS] > 1) MainScreen.text("Marble herself is out right now; she's taken her kids to go visit Whitney.  You're sure though that she'll be back within the hour, so you could just wait if you needed her.");
				else {
					//requires at least 1 kid
					if (rand(2) == 0) {
						MainScreen.text("Marble herself is nursing ");
						if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) MainScreen.text("one of your cow-girl children");
						...
						if (Flags.list[FlagEnum.MARBLE_KIDS] > 0) MainScreen.text("s");
		...
		if (player.statusAffects.has("CampRathazul")) {
			if (Flags.list[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] <= 1) {
				...
				if (Flags.list[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] == 1) MainScreen.text("  Some kind of spider-silk-based equipment is hanging from a nearby rack.  <b>He's finished with the task you gave him!</b>", false);
				MainScreen.text("\n\n", false);
			}
		...
		//MOUSEBITCH
		if (amilyScene.amilyFollower() && Flags.list[FlagEnum.AMILY_FOLLOWER] == 1) {
			if (Flags.list[FlagEnum.FUCK_FLOWER_LEVEL] >= 4) MainScreen.text("Amily has relocated her grass bedding to the opposite side of the camp from the strange tree; every now and then, she gives it a suspicious glance, as if deciding whether to move even further.");
		...
		//Corrupt mousebitch!
		else if (amilyScene.amilyFollower() && Flags.list[FlagEnum.AMILY_FOLLOWER] == 2 && Flags.list[FlagEnum.FOLLOWER_AT_FARM_AMILY] == 0) {
		...
		else if (Flags.list[FlagEnum.AMILY_VISITING_URTA] == 1 || Flags.list[FlagEnum.AMILY_VISITING_URTA] == 2) {
		...
		if (campCorruptJojo() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_JOJO] == 0) MainScreen.text("From time to time you can hear movement from around your camp, and you routinely find thick puddles of mouse semen.  You are sure Jojo is here if you ever need to sate yourself.\n\n", false);
		//Pure Jojo
		...
		//Izma
		if (izmaFollower() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_IZMA] == 0) {
		...
		if (kGAMECLASS.desert.antsScene.phyllaWaifu()) {
			...
			if (Flags.list[FlagEnum.ANT_KIDS] > 0) MainScreen.text(" one of your many children exit the anthill to unload some dirt before continuing back down into the colony.  It makes you feel good knowing your offspring are so productive.");
		...
		if (player.stats.lust >= 100) {
			...
			else if (Flags.list[FlagEnum.UNABLE_TO_MASTURBATE_BECAUSE_CENTAUR] > 0 && player.lowerBody.isTaur()) {
	...
	public followersCount(): number {
		...
		if (Flags.list[FlagEnum.VALARIA_AT_CAMP] == 1) counter++;
		...
		if (sophieFollower() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) counter++;
	...
	public slavesCount(): number {
		let counter: number = 0;
		if (latexGooFollower() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_LATEXY] == 0) counter++;
		if (vapulaSlave() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 0) counter++;
		if (campCorruptJojo() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_JOJO] == 0) counter++;
		if (amilyScene.amilyFollower() && amilyScene.amilyCorrupt() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_AMILY] == 0) counter++;
		//Bimbo sophie
		if (bimboSophie() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) counter++;
		if (ceraphIsFollower()) counter++;
		if (milkSlave() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] == 0) counter++;
		return counter;
	}
	...
	public loversCount(): number {
		...
		//Izma!
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00238] == 1 && Flags.list[FlagEnum.FOLLOWER_AT_FARM_IZMA] == 0) counter++;
		if (isabellaFollower() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_ISABELLA] == 0) counter++;
		if (player.statusAffects.has("CampMarble") && Flags.list[FlagEnum.FOLLOWER_AT_FARM_MARBLE] == 0) counter++;
		if (amilyScene.amilyFollower() && !amilyScene.amilyCorrupt()) counter++;
		if (followerKiha()) counter++;
		if (Flags.list[FlagEnum.NIEVE_STAGE] == 5) counter++;
		if (Flags.list[FlagEnum.ANT_WAIFU] > 0) counter++;
		return counter;
	}
	...
	public campLoversMenu(): void {
		...
		//Isabella
		if (isabellaFollower() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_ISABELLA] == 0) {
			...
			if (model.time.hours >= 21 || model.time.hours <= 5) MainScreen.text("Isabella is sound asleep in her bunk and quietly snoring.", false);
				...
				if (amilyScene.amilyFollower() && Flags.list[FlagEnum.AMILY_FOLLOWER] == 1 && Flags.list[FlagEnum.AMILY_BLOCK_COUNTDOWN_BECAUSE_CORRUPTED_JOJO] == 0)
					izzyCreeps[izzyCreeps.length] = 2;
				if (amilyScene.amilyFollower() && Flags.list[FlagEnum.AMILY_FOLLOWER] == 2 && Flags.list[FlagEnum.AMILY_BLOCK_COUNTDOWN_BECAUSE_CORRUPTED_JOJO] == 0 && Flags.list[FlagEnum.FOLLOWER_AT_FARM_AMILY] == 0)
				...
				if (Flags.list[FlagEnum.ISABELLA_MILKED_YET] == -1) MainScreen.text("Isabella has just returned from a late visit to Whitney's farm, bearing a few filled bottles and a small pouch of gems.", false);
		...
		//MARBLE
		if (player.statusAffects.has("CampMarble") && Flags.list[FlagEnum.FOLLOWER_AT_FARM_MARBLE] == 0) {
			temp = rand(5);
			MainScreen.text("A second bedroll rests next to yours; a large two handed hammer sometimes rests against it, depending on whether or not its owner needs it at the time.  ", false);
			//Normal Murbles
			if (Flags.list[FlagEnum.MARBLE_PURIFICATION_STAGE] != 4) {
				//(Choose one of these at random to display each hour)
		...
		if (Flags.list[FlagEnum.ANT_WAIFU] > 0) MainScreen.addButton(8, "Phylla", Game.desert.antsScene.introductionToPhyllaFollower);
	...
	public campSlavesMenu(): void {
		...
		//Bimbo Sophie
		if (bimboSophie() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) {
			sophieBimbo.sophieCampLines();
			sophieEvent = sophieBimbo.approachBimboSophieInCamp;
		}
		if (latexGooFollower() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_LATEXY] == 0) {
			MainScreen.text(Flags.list[FlagEnum.GOO_NAME] + " lurks in a secluded section of rocks, only venturing out when called for or when she needs to gather water from the stream.\n\n");
			goo = latexGirl.approachLatexy;
		}
		if (ceraphIsFollower()) ceraph = ceraphFollowerScene.ceraphFollowerEncounter;
		//JOJO
		//If Jojo is corrupted, add him to the masturbate menu.
		if (campCorruptJojo() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_JOJO] == 0) {
		...
		if (milkSlave() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] == 0) {
		...
		//choices("Amily",amilyEvent,"Ceraph",ceraph,"Jojo",jojoEvent,"Sophie",sophieEvent,"Vapula",vapula,"",0,"",0,"",0,Flags.list[FlagEnum.GOO_NAME],goo,"Back",1);	
		menu();
		if (amilyEvent != null) MainScreen.addButton(0, "Amily", amilyEvent);
		...
		if (milkSlave() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] == 0) MainScreen.addButton(7, Flags.list[FlagEnum.MILK_NAME], milkWaifu.milkyMenu);
		if (goo != null) MainScreen.addButton(8, Flags.list[FlagEnum.GOO_NAME], goo);
		MainScreen.addButton(9, "Back", playerMenu);
	}
	...
	public campFollowers(): void {
		...
		if (player.statusAffects.has("CampRathazul")) {
			...
			if (Flags.list[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] <= 1) {
				MainScreen.text("Tucked into a shaded corner of the rocks is a bevy of alchemical devices and equipment.  The alchemist Rathazul looks to be hard at work with his chemicals, working on who knows what.", false);
				if (Flags.list[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] == 1) MainScreen.text("  Some kind of spider-silk-based equipment is hanging from a nearby rack.  He's finished with the task you gave him!", false);
				MainScreen.text("\n\n", false);
			}
			else MainScreen.text("Tucked into a shaded corner of the rocks is a bevy of alchemical devices and equipment.  The alchemist Rathazul looks to be hard at work on the silken equipment you've commissioned him to craft.\n\n", false);
		}
		if (sophieFollower() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) {
			if (rand(5) == 0) MainScreen.text("Sophie is sitting by herself, applying yet another layer of glittering lip gloss to her full lips.\n\n");
			...
			else if (rand(2) == 0 || Flags.list[FlagEnum.SOPHIE_ADULT_KID_COUNT] == 0) {
				if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) MainScreen.text("Your platinum-blonde harpy, Sophie, is currently reading a book - a marked change from her bimbo-era behavior.  Occasionally, though, she glances up from the page and gives you a lusty look.  Some things never change....\n\n");
				...
				if (Flags.list[FlagEnum.SOPHIE_ADULT_KID_COUNT] < 5) {
				...
				if (Flags.list[FlagEnum.SOPHIE_ADULT_KID_COUNT] == 1) MainScreen.text("her");
		...
		if (Flags.list[FlagEnum.VALARIA_AT_CAMP] == 1) valeria2 = valeria.valeriaFollower;
		MainScreen.addButton(0, "Ember", ember);
		if (helspawnFollower()) MainScreen.addButton(1, Flags.list[FlagEnum.HELSPAWN_NAME], helSpawnScene.helspawnsMainMenu);
	...
	public doSleep(clrScreen: boolean = true): void {
		...
		if (timeQ == 0) {
			...
			/******************************************************************/
			//HEL SLEEPIES!
			if (helFollower.helAffection() >= 70 && Flags.list[FlagEnum.HEL_REDUCED_ENCOUNTER_RATE] == 0 && Flags.list[FlagEnum.HEL_FOLLOWER_LEVEL] == 0) {
				Game.heliaDiscovery();
				sleepRecovery(false);
				return;
			}
			...
			if (player.lowerBody.cockSpot.hasCock() && followerShouldra() && player.statusAffects.get("Exgartuan").value1 == 1) {
				if (Flags.list[FlagEnum.SHOULDRA_EXGARTUDRAMA] == 0) {
					shouldraFollower.shouldraAndExgartumonFightGottaCatchEmAll();
					sleepRecovery(false);
					return;
				}
				else if (Flags.list[FlagEnum.SHOULDRA_EXGARTUDRAMA] == 3) {
					shouldraFollower.exgartuMonAndShouldraShowdown();
					sleepRecovery(false);
					return;
				}
			}
			if (player.lowerBody.cockSpot.hasCock() && followerShouldra() && Flags.list[FlagEnum.SHOULDRA_EXGARTUDRAMA] == -0.5) {
			...
			/******************************************************************/
			//Marble Sleepies
			if (marbleScene.marbleAtCamp() && player.statusAffects.has("CampMarble") && Flags.list[FlagEnum.SLEEP_WITH] == "Marble" && Flags.list[FlagEnum.FOLLOWER_AT_FARM_MARBLE] == 0) {
				if (marbleScene.marbleNightSleepFlavor()) {
					sleepRecovery(false);
					return;
				}
			}
			else if (Flags.list[FlagEnum.SLEEP_WITH] == "Arian" && arianScene.arianFollower()) {
				arianScene.sleepWithArian();
				return;
			}
			else if (Flags.list[FlagEnum.SLEEP_WITH] == "Sophie" && (bimboSophie() || sophieFollower()) && Flags.list[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) {
				...
				if (Flags.list[FlagEnum.SLEEP_WITH] == "Helia" && kGAMECLASS.helScene.followerHel()) {
	...
	private dungeonFound(): boolean { //Returns true as soon as any known dungeon is found
		if (Flags.list[FlagEnum.DISCOVERED_DUNGEON_2_ZETAZ] > 0) return true;
		if (player.statusAffects.has("FoundFactory")) return true;
		if (Flags.list[FlagEnum.DISCOVERED_WITCH_DUNGEON] > 0) return true;
		if (Flags.list[FlagEnum.D3_DISCOVERED] > 0) return true;
		return false;
	}
	...
	private farmFound(): boolean { //Returns true as soon as any known dungeon is found
		if (player.statusAffects.has("MetWhitney") && player.statusAffects.get("MetWhitney").value1 > 1) {
			if (Flags.list[FlagEnum.FARM_DISABLED] == 0) return true;
			if (player.stats.cor >= 70 && player.level >= 12 && Game.farm.farmCorruption.corruptFollowers() >= 2 && Flags.list[FlagEnum.FARM_CORRUPTION_DISABLED] == 0) return true;
		}
		if (Flags.list[FlagEnum.FARM_CORRUPTION_STARTED]) return true;
		return false;
	}

	private placesKnown(): boolean { //Returns true as soon as any known place is found
		if (Flags.list[FlagEnum.BAZAAR_ENTERED] > 0) return true;
		if (player.statusAffects.has("BoatDiscovery")) return true;
		if (Flags.list[FlagEnum.FOUND_CATHEDRAL] == 1) return true;
		if (dungeonFound()) return true;
		if (farmFound()) return true;
		if (Flags.list[FlagEnum.OWCA_UNLOCKED] == 1) return true;
		if (player.statusAffects.has("HairdresserMeeting")) return true;
		if (player.statusAffects.get("TelAdre").value1 >= 1) return true;
		if (Flags.list[FlagEnum.AMILY_VILLAGE_ACCESSIBLE] > 0) return true;
		if (Flags.list[FlagEnum.MET_MINERVA] >= 4) return true;
		return false;
	}

	//Places menu
	private places(): void { //Displays a menu for all known places
		if (Flags.list[FlagEnum.PLACES_PAGE] != 0) {
			placesPage2();
			return;
		}
		menu();
		if (Flags.list[FlagEnum.BAZAAR_ENTERED] > 0) MainScreen.addButton(0, "Bazaar", Game.bazaar.enterTheBazaar);
		if (player.statusAffects.has("BoatDiscovery")) MainScreen.addButton(1, "Boat", Game.boat.boatExplore);
		if (Flags.list[FlagEnum.FOUND_CATHEDRAL] == 1) {
			if (Flags.list[FlagEnum.GAR_NAME] == 0)
				MainScreen.addButton(2, "Cathedral", Game.gargoyle.gargoylesTheShowNowOnWBNetwork);
			else MainScreen.addButton(2, "Cathedral", Game.gargoyle.returnToCathedral);
		}
		if (dungeonFound()) MainScreen.addButton(3, "Dungeons", dungeons);
		MainScreen.addButton(4, "Next", placesPage2);
		if (farmFound()) MainScreen.addButton(5, "Farm", Game.farm.farmExploreEncounter);
		if (Flags.list[FlagEnum.OWCA_UNLOCKED] == 1) MainScreen.addButton(6, "Owca", Game.owca.gangbangVillageStuff);
		if (player.statusAffects.has("HairdresserMeeting")) MainScreen.addButton(7, "Salon", Game.mountain.salon.salonGreeting);
		if (player.statusAffects.get("TelAdre").value1 >= 1) MainScreen.addButton(8, "Tel'Adre", Game.telAdre.telAdreMenu);
		MainScreen.addButton(9, "Back", playerMenu);
	}

	private placesPage2(): void {
		menu();
		Flags.list[FlagEnum.PLACES_PAGE] = 1;
		//turn on ruins
		if (Flags.list[FlagEnum.AMILY_VILLAGE_ACCESSIBLE] > 0) MainScreen.addButton(0, "TownRuins", amilyScene.exploreVillageRuin);
		if (Flags.list[FlagEnum.MET_MINERVA] >= 4) MainScreen.addButton(1, "Oasis Tower", Game.highMountains.minervaScene.encounterMinerva);
		MainScreen.addButton(4, "Previous", placesToPage1);
		MainScreen.addButton(9, "Back", playerMenu);
	}

	private placesToPage1(): void {
		Flags.list[FlagEnum.PLACES_PAGE] = 0;
		places();
	}
	...
	private dungeons(): void {
		menu();
		//Turn on dungeons
		if (Flags.list[FlagEnum.DISCOVERED_DUNGEON_2_ZETAZ] > 0) MainScreen.addButton(0, "Deep Cave", kGAMECLASS.enterZetazsLair);
		if (player.statusAffects.has("FoundFactory")) MainScreen.addButton(1, "Factory", kGAMECLASS.enterFactory);
		if (Flags.list[FlagEnum.DISCOVERED_WITCH_DUNGEON] > 0) MainScreen.addButton(2, "Desert Cave", kGAMECLASS.enterBoobsDungeon);
		if (Flags.list[FlagEnum.D3_DISCOVERED] > 0) MainScreen.addButton(3, "Stronghold", kGAMECLASS.d3.enterD3);
	...
	private exgartuanCampUpdate(): void {
		...
		if (player.statusAffects.has("Exgartuan")) {
			...
			//Tit removal
				...
				MainScreen.text("", true);
				MainScreen.text("<b>Black milk dribbles from your " + BreastDescriptor.describeNipple(0) + ".  It immediately dissipates into the air, leaving you feeling alone.  It looks like you became too small for Exgartuan!\n</b>", false);
				player.statusAffects.remove("Exgartuan");
			}
		}
		doNext(playerMenu);
	...
	private fixHistory():void {
		...
		Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00418] = 2;
		menu();
		doNext(10036);
	}
	*/
