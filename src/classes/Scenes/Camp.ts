...
*/
		...
		public returnToCamp(timeUsed: number):void {
			...
			else Render.text(Num2Text(timeUsed) + " hours pass...\n");
...
//		fixHistory();
	...
	Render.text(images.showImage("camping"), false);
	//Isabella upgrades camp level!


	if(isabellaFollower()) {
		Render.text("Your campsite got a lot more comfortable once Isabella moved in.  Carpets cover up much of the barren ground, simple awnings tied to the rocks provide shade, and hand-made wooden furniture provides comfortable places to sit and sleep.", false);
		if(model.time.days >= 20) Render.text("  You've even managed to carve some artwork into the rocks around the camp's perimeter.", false);
	}
	//Live in-ness
	else {
		if(model.time.days < 10) Render.text("Your campsite is fairly simple at the moment.  Your tent and bedroll are set in front of the rocks that lead to the portal.  You have a small fire pit as well.", false);
		else if(model.time.days < 20) Render.text("Your campsite is starting to get a very 'lived-in' look.  The fire-pit is well defined with some rocks you've arranged around it, and your bedroll and tent have been set up in the area most sheltered by rocks.", false);
		else Render.text("Your new home is as comfy as a camp site can be.  The fire-pit and tent are both set up perfectly, and in good repair, and you've even managed to carve some artwork into the rocks around the camp's perimeter.", false);
	}
	...
	if(flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] == 100 && player.statusAffects.has("CampMarble")) {
		Render.text("  Marble has built a fairly secure nursery amongst the rocks to house your ",false);
		if(flags[FlagEnum.MARBLE_KIDS] == 0) Render.text("future children", false);
		else {
			Render.text(num2Text(flags[FlagEnum.MARBLE_KIDS]) + " child", false);
			if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text("ren", false);
		}
		Render.text(".", false);
	}
	//HARPY ROOKERY
	if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] > 0) {
		...
		if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] == 1) {
			Render.text("  There's a smallish harpy nest that your daughter has built up with rocks piled high near the fringes of your camp.  It's kind of pathetic, but she seems proud of her accomplishment.");
		}
		//Medium (2-3 mature daughters)
		else if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] <= 3) {
			Render.text("  There's a growing pile of stones built up at the fringes of your camp.  It's big enough to be considered a small hill by this point, dotted with a couple small harpy nests just barely big enough for two.");
		}
		//Big (4 mature daughters)
		else if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] <= 4) {
			Render.text("  The harpy rookery at the edge of camp has gotten pretty big.  It's taller than most of the standing stones that surround the portal, and there's more nests than harpies at this point.  Every now and then you see the four of them managing a boulder they dragged in from somewhere to add to it.");
		}
		//Large (5-10 mature daughters)
		else if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] <= 10) {
			Render.text("  The rookery has gotten quite large.  It stands nearly two stories tall at this point, dotted with nests and hollowed out places in the center.  It's surrounded by the many feathers the assembled harpies leave behind.");
		}
		//Giant (11-20 mature daughters)
		else if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] <= 20) {
			Render.text("  A towering harpy rookery has risen up at the fringes of your camp, filled with all of your harpy brood.  It's at least three stories tall at this point, and it has actually begun to resemble a secure structure.  These harpies are always rebuilding and adding onto it.");
		}
		//Massive (31-50 mature daughters)
		else if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] <= 50) {
			Render.text("  A massive harpy rookery towers over the edges of your camp.  It's almost entirely built out of stones that are fit seamlessly into each other, with many ledges and overhangs for nests.  There's a constant hum of activity over there day or night.");
		}
		//Immense (51+ Mature daughters)
		else {
			Render.text("  An immense harpy rookery dominates the edge of your camp, towering over the rest of it.  Innumerable harpies flit around it, always working on it, assisted from below by the few sisters unlucky enough to be flightless.");
		}
	}
	//Traps
	if(player.statusAffects.has("DefenseCanopy")) {
		Render.text("  A thorny tree has sprouted near the center of the camp, growing a protective canopy of spiky vines around the portal and your camp.", false);
	}
	else Render.text("  You have a number of traps surrounding your makeshift home, but they are fairly simple and may not do much to deter a demon.", false);
	Render.text("  The portal shimmers in the background as it always does, looking menacing and reminding you of why you came.\n\n", false);

	//Ember's anti-minotaur crusade!
	if(flags[FlagEnum.EMBER_CURRENTLY_FREAKING_ABOUT_MINOCUM] == 1) {
		//Modified Camp Description
		Render.text("Since Ember began " + emberMF("his","her") + " 'crusade' against the minotaur population, skulls have begun to pile up on either side of the entrance to " + emberScene.emberMF("his","her") + " den.  There're quite a lot of them.\n\n");
	}
	//Dat tree!
	if(flags[FlagEnum.FUCK_FLOWER_LEVEL] >= 4 && flags[FlagEnum.FUCK_FLOWER_KILLED] == 0) {
		Render.text("On the outer edges, half-hidden behind a rock, is a large, very healthy tree.  It grew fairly fast, but seems to be fully developed now.  Holli, Marae's corrupt spawn, lives within.\n\n");
	}
	if(flags[FlagEnum.CLARA_IMPRISONED] > 0)
	{
		//claraCampAddition();
	...
	if(player.statusAffects.has("CampMarble")) {
		temp = rand(5);
		Render.text("A second bedroll rests next to yours; a large two-handed hammer sometimes rests against it, depending on whether or not its owner needs it at the time.  ", false);
		//Marble is out!
		if(flags[FlagEnum.MARBLE_PURIFICATION_STAGE] == 4) Render.text("Marble isn’t here right now; she’s still off to see her family.");
		//requires at least 1 kid, time is just before sunset, this scene always happens at this time if the PC has at least one kid.
			...
			Render.text("Marble herself is currently in the nursery, putting your ");
			if(flags[FlagEnum.MARBLE_KIDS] == 1) Render.text("child");
			else Render.text("children");
			Render.text(" to bed.");
		}
		//at 6-7 in the morning, scene always displays at this time
		else if(model.time.hours == 6 || model.time.hours == 7) Render.text("Marble is off in an open area to the side of your camp right now.  She is practicing with her large hammer, going through her daily training.");
		//after nightfall, scene always displays at this time unless PC is wormed
			...
			Render.text("Marble is hanging around her bedroll waiting for you to come to bed.  However, sometimes she lies down for a bit, and sometimes she paces next to it.");
			if(flags[FlagEnum.MARBLE_LUST] > 30) Render.text("  She seems to be feeling antsy.");
		}
			...
			if(rand(2) == 0 && flags[FlagEnum.MARBLE_KIDS] > 5) Render.text("Marble is currently tending to your kids, but she looks a bit stressed out right now.  It looks like " + num2Text(flags[FlagEnum.MARBLE_KIDS]) + " might just be too many for her to handle on her own...");
			//requires at least 4 kids
			else if(rand(3) == 0 && flags[FlagEnum.MARBLE_KIDS] > 3) Render.text("Marble herself is in the camp right now, telling a story about her travels around the world to her kids as they gather around her.  The children are completely enthralled by her words.  You can't help but smile.");
			//Requires 2 boys
			else if(rand(3) == 0 && flags[FlagEnum.MARBLE_BOYS] > 1)
			{
				Render.text("Marble herself is currently refereeing a wrestling match between two of your sons.  It seems like it's a contest to see which one of them gets to go for a ride between her breasts in a game of <i>Bull Blasters</i>, while the loser has to sit on her shoulders.");
			}
			//requires at least 2 kids
			else if(rand(3) == 0 && flags[FlagEnum.MARBLE_KIDS] - flags[FlagEnum.MARBLE_BOYS] > 1) Render.text("Marble herself is involved in a play fight with two of your kids brandishing small sticks.  It seems that the <i>mommy monster</i> is terrorising the camp and needs to be stopped by the <i>Mighty Moo and her sidekick Bovine Lass</i>.");
			else if(rand(3) == 0 && flags[FlagEnum.MARBLE_KIDS] > 1) Render.text("Marble herself is out right now; she's taken her kids to go visit Whitney.  You're sure though that she'll be back within the hour, so you could just wait if you needed her.");
			else {
				//requires at least 1 kid
				if(rand(2) == 0) 
				{
					Render.text("Marble herself is nursing ");
					if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text("one of your cow-girl children");
					else Render.text("your cow-girl child");
					Render.text(" with a content look on her face.");
				}
				else 
				{
					Render.text("Marble herself is watching your kid");
					if(flags[FlagEnum.MARBLE_KIDS] > 0) Render.text("s");
					Render.text(" playing around the camp right now.");
				}
			}
		}
		//(Choose one of these at random to display each hour)
		else if(temp == 0) Render.text("Marble herself has gone off to Whitney's farm to get milked right now.  You're sure she'd be back in moments if you needed her.", false);
		else if(temp == 1) Render.text("Marble herself has gone off to Whitney's farm to do some chores right now.  You're sure she'd be back in moments if you needed her.", false);
		else if(temp == 2) Render.text("Marble herself isn't at the camp right now; she is probably off getting supplies, though she'll be back soon enough.  You're sure she'd be back in moments if you needed her.", false);
		else if(temp == 3) {
			Render.text("Marble herself is resting on her bedroll right now.", false);
			
		}
		else if(temp == 4) {
			Render.text("Marble herself is wandering around the camp right now.", false);
		}
		Render.text("\n\n", false);	
	}
	//RATHAZUL
	...
	if(player.statusAffects.has("CampRathazul")) {
		if(flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] <= 1) {
			Render.text("Tucked into a shaded corner of the rocks is a bevy of alchemical devices and equipment.  The alchemist Rathazul looks to be hard at work with his chemicals, working on who knows what.", false);
			if(flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] == 1) Render.text("  Some kind of spider-silk-based equipment is hanging from a nearby rack.  <b>He's finished with the task you gave him!</b>", false);
			Render.text("\n\n", false);
		}
		else Render.text("Tucked into a shaded corner of the rocks is a bevy of alchemical devices and equipment.  The alchemist Rathazul looks to be hard at work on the silken equipment you've commissioned him to craft.\n\n", false);
	}
	//MOUSEBITCH
	if(amilyScene.amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 1) {
		if(flags[FlagEnum.FUCK_FLOWER_LEVEL] >= 4) Render.text("Amily has relocated her grass bedding to the opposite side of the camp from the strange tree; every now and then, she gives it a suspicious glance, as if deciding whether to move even further.");
		else Render.text("A surprisingly tidy nest of soft grasses and sweet-smelling herbs has been built close to your bedroll. A much-patched blanket draped neatly over the top is further proof that Amily sleeps here. She changes the bedding every few days, to ensure it stays as nice as possible.\n\n", false);
	}
	//Corrupt mousebitch!
	else if(amilyScene.amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2 && flags[FlagEnum.FOLLOWER_AT_FARM_AMILY] == 0) {
		Render.text("Sometimes you hear a faint moan from not too far away. No doubt the result of your slutty toy mouse playing with herself.\n\n", false);
	}
	//Amily out freaking Urta?
	else if(flags[FlagEnum.AMILY_VISITING_URTA] == 1 || flags[FlagEnum.AMILY_VISITING_URTA] == 2) {
		Render.text("Amily's bed of grass and herbs lies empty, the mouse-woman still absent from her sojourn to meet your other lover.\n\n", false);
	}
	//JOJO
	//If Jojo is corrupted, add him to the masturbate menu.
	if(campCorruptJojo() && flags[FlagEnum.FOLLOWER_AT_FARM_JOJO] == 0) Render.text("From time to time you can hear movement from around your camp, and you routinely find thick puddles of mouse semen.  You are sure Jojo is here if you ever need to sate yourself.\n\n", false);
	//Pure Jojo
	if(player.statusAffects.has("PureCampJojo")) Render.text("There is a small bedroll for Jojo near your own, though the mouse is probably hanging around the camp's perimeter.\n\n", false);
	//Izma
	if(izmaFollower() && flags[FlagEnum.FOLLOWER_AT_FARM_IZMA] == 0) {
		Render.text("Neatly laid near the base of your own is a worn bedroll belonging to Izma, your tigershark lover.  It's a snug fit for her toned body, though it has some noticeable cuts and tears in the fabric.  Close to her bed is her old trunk, almost as if she wants to have it at arms length if anyone tries to rob her in her sleep.  ", false);
		temp = rand(3);
		//Text 1} I
		if(temp == 0) Render.text("Izma's lazily sitting on the trunk beside her bedroll, reading one of the many books from inside it.  She smiles happily when your eyes linger on her, and you know full well she's only half-interested in it.", false);
		//Text 2
		else if(temp == 1) Render.text("You notice Izma isn't around right now.  She's probably gone off to the nearby stream to get some water.  Never mind, she comes around from behind a rock, still dripping wet.", false);
		//Text 3 
		else Render.text("Izma is lying on her back near her bedroll.  You wonder at first just why she isn't using her bed, but as you look closer you notice all the water pooled beneath her and the few droplets running down her arm, evidence that she's just returned from the stream.", false);
		Render.text("\n\n", false);
	}
	//►[Added Campsite Description]
	if(kGAMECLASS.desert.antsScene.phyllaWaifu()) {
		Render.text("You see Phylla's anthill in the distance.  Every now and then you see");
		//If PC has children w/ Phylla:
		if(flags[FlagEnum.ANT_KIDS] > 0) Render.text(" one of your many children exit the anthill to unload some dirt before continuing back down into the colony.  It makes you feel good knowing your offspring are so productive.");
		else Render.text(" Phylla appear out of the anthill to unload some dirt.  She looks over to your campsite and gives you an excited wave before heading back into the colony.  It makes you feel good to know she's so close.");
		Render.text("\n\n");
	}
	//Clear bee-status
	if(player.statusAffects.has("ParalyzeVenom")) {
		...
		Render.text("<b>You feel quicker and stronger as the paralyzation venom in your veins wears off.</b>\n\n", false);
	}
	//The uber horny
	if(player.lust >= 100) {
		if(player.statusAffects.has("Dysfunction")) {
			Render.text("<b>You are debilitatingly aroused, but your sexual organs are so numbed the only way to get off would be to find something tight to fuck or get fucked...</b>\n\n", false);
		}
		else if(flags[FlagEnum.UNABLE_TO_MASTURBATE_BECAUSE_CENTAUR] > 0 && player.isTaur()) {
			Render.text("<b>You are delibitatingly aroused, but your sex organs are so difficult to reach that masturbation isn't at the forefront of your mind.</b>\n\n", false);
		}
		else {
			Render.text("<b>You are debilitatingly aroused, and can think of doing nothing other than masturbating.</b>\n\n", false);
			exploreEvent = null;
			placesEvent = null;
	...
	//Night
	if(model.time.hours < 6 || model.time.hours > 20) {
		Render.text("It is dark out, made worse by the lack of stars in the sky.  A blood-red moon hangs in the sky, seeming to watch you, but providing little light.  It's far too dark to leave camp.\n", false);
		restName = "Sleep";
		restEvent = doSleep;
		exploreEvent = null;
		placesEvent = null;
	...
	//Day Time!
		...
		Render.text("It's light outside, a good time to explore and forage for supplies with which to fortify your camp.\n", false);
...
public campLoversMenu():void {
	...
	if(kGAMECLASS.helScene.followerHel()) {
		if(flags[FlagEnum.HEL_FOLLOWER_LEVEL] == 2) {
			...
			//(6-7) 
			if(model.time.hours <= 7) Render.text("Hel is currently sitting at the edge of camp, surrounded by her scraps of armor, sword, and a few half-empty bottles of vodka.  By the way she's grunting and growling, it looks like she's getting ready to flip her shit and go running off into the plains in her berserker state.\n\n");
			//(8a-5p) 
			else if(model.time.hours <= 17) Render.text("Hel's out of camp at the moment, adventuring on the plains.  You're sure she'd be on hand in moments if you needed her, though.\n\n");
			//5-7) 
			else if(model.time.hours <= 19) Render.text("Hel's out visiting her family in Tel'Adre right now, though you're sure she's only moments away if you need her.\n\n");
			//(7+)
			else Render.text("Hel is fussing around her hammock, checking her gear and sharpening her collection of blades.  Each time you glance her way, though, the salamander puts a little extra sway in her hips and her tail wags happily.\n\n");
		}
		else if(flags[FlagEnum.HEL_FOLLOWER_LEVEL] == 1) {
			if(flags[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] == 1) {
				Render.text("Hel has returned to camp, though for now she looks a bit bored.  Perhaps she is waiting on something.\n\n");
			}
			else {
				Render.text("<b>You see the salamander Helia pacing around camp, anxiously awaiting your departure to the harpy roost. Seeing you looking her way, she perks up, obviously ready to get underway.</b>\n\n");
			}
		}
	...
	if(followerKiha()) {
		//(6-7) 
		if(model.time.hours < 7) Render.text("Kiha is sitting near the fire, her axe laying across her knees as she polishes it.[pg]");
		else if(model.time.hours < 19) Render.text("Kiha's out right now, likely patrolling for demons to exterminate.  You're sure a loud call could get her attention.\n\n");
		else Render.text("Kiha is utterly decimating a set of practice dummies she's set up out on the edge of camp.  All of them have crudely drawn horns. Most of them are on fire.\n\n");
	...
	if(isabellaFollower() && flags[FlagEnum.FOLLOWER_AT_FARM_ISABELLA] == 0) {
		...
		if(model.time.hours >= 21 || model.time.hours <= 5) Render.text("Isabella is sound asleep in her bunk and quietly snoring.", false);
		else if(model.time.hours == 6) Render.text("Isabella is busy eating some kind of grain-based snack for breakfast.  The curly-haired cow-girl gives you a smile when she sees you look her way.", false);
		else if(model.time.hours == 7) Render.text("Isabella, the red-headed cow-girl, is busy with a needle and thread, fixing up some of her clothes.", false);
		else if(model.time.hours == 8) Render.text("Isabella is busy cleaning up the camp, but when she notices you looking her way, she stretches up and arches her back, pressing eight bullet-hard nipples into the sheer silk top she prefers to wear.", false);
		else if(model.time.hours == 9) Render.text("Isabella is out near the fringes of your campsite.  She has her massive shield in one hand and appears to be keeping a sharp eye out for intruders or demons.  When she sees you looking her way, she gives you a wave.", false);
		else if(model.time.hours == 10) Render.text("The cow-girl warrioress, Isabella, is sitting down on a chair and counting out gems from a strange pouch.  She must have defeated someone or something recently.", false);
		else if(model.time.hours == 11) Render.text("Isabella is sipping from a bottle labelled 'Lactaid' in a shaded corner.  When she sees you looking she blushes, though dark spots appear on her top and in her skirt's middle.", false);
		else if(model.time.hours == 12) Render.text("Isabella is cooking a slab of meat over the fire.  From the smell that's wafting this way, you think it's beef.  Idly, you wonder if she realizes just how much like her chosen food animal she has become.", false);
			...
			if(izzyCreeps[choice] == 0) Render.text("is sitting down with Rathazul, chatting amiably about the weather.", false);
			else if(izzyCreeps[choice] == 1) Render.text("is sitting down with Jojo, smiling knowingly as the mouse struggles to keep his eyes on her face.", false);
			else if(izzyCreeps[choice] == 2) Render.text("is talking with Amily, sharing stories of the fights she's been in and the enemies she's faced down.  Amily seems interested but unimpressed.", false);
			else if(izzyCreeps[choice] == 3) Render.text("is sitting down chatting with Amily, but the corrupt mousette is just staring at Isabella's boobs and masturbating.  The cow-girl is pretending not to notice.", false);
			else if(izzyCreeps[choice] == 4) Render.text("is sitting down with Izma and recounting some stories, somewhat nervously.  Izma keeps flashing her teeth in a predatory smile.", false);
			else Render.text("is sitting down and thumbing through a book.", false);
		}
		else if(model.time.hours == 14) Render.text("Isabella is working a grindstone and sharpening her tools.  She even hones the bottom edge of her shield into a razor-sharp cutting edge.  The cow-girl is sweating heavily, but it only makes the diaphanous silk of her top cling more alluringly to her weighty chest.", false);
		else if(model.time.hours == 15) Render.text("The warrior-woman, Isabella is busy constructing dummies of wood and straw, then destroying them with vicious blows from her shield.  Most of the time she finishes by decapitating them with the sharp, bottom edge of her weapon.  She flashes a smile your way when she sees you.", false);
		else if(model.time.hours == 16) Render.text("Isabella is sitting down with a knife, the blade flashing in the sun as wood shavings fall to the ground.  Her hands move with mechanical, practiced rhythm as she carves a few hunks of shapeless old wood into tools or art.", false);
		else if(model.time.hours == 17) Render.text("Isabella is sitting against one of the large rocks near the outskirts of your camp, staring across the wasteland while idly munching on what you assume to be a leg of lamb.  She seems lost in thought, though that doesn't stop her from throwing a wink and a goofy food-filled grin toward you.", false);
		else if(model.time.hours == 18) Render.text("The dark-skinned cow-girl, Isabella, is sprawled out on a carpet and stretching.  She seems surprisingly flexible for someone with hooves and oddly-jointed lower legs.", false);
		else if(model.time.hours == 19) {
			//[(Izzy Milked Yet flag = -1)
			if(flags[FlagEnum.ISABELLA_MILKED_YET] == -1) Render.text("Isabella has just returned from a late visit to Whitney's farm, bearing a few filled bottles and a small pouch of gems.", false);
			else Render.text("Isabella was hidden behind a rock when you started looking for her, but as soon as you spot her in the darkness, she jumps, a guilty look flashing across her features.  She turns around and adjusts her top before looking back your way, her dusky skin even darker from a blush.  The cow-girl gives you a smile and walks back to her part of camp.  A patch of white decorates the ground where she was standing - is that milk?  Whatever it is, it's gone almost as fast as you see it, devoured by the parched, wasteland earth.", false);
		}
		else if(model.time.hours == 20) Render.text("Your favorite chocolate-colored cowgirl, Isabella, is moving about, gathering all of her scattered belongings and replacing them in her personal chest.  She yawns more than once, indicating her readiness to hit the hay, but her occasional glance your way lets you know she wouldn't mind some company before bed.", false);
		else Render.text("Isabella looks incredibly bored right now.", false);
		Render.text("\n\n", false);		
	}
	//Izma
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00238] == 1 && flags[FlagEnum.FOLLOWER_AT_FARM_IZMA] == 0) {
	...
	if(player.statusAffects.has("CampMarble") && flags[FlagEnum.FOLLOWER_AT_FARM_MARBLE] == 0) {
		...
		{
			//(Choose one of these at random to display each hour)
			if(temp == 0) Render.text("Marble herself has gone off to Whitney's farm to get milked right now.", false);
			if(temp == 1) Render.text("Marble herself has gone off to Whitney's farm to do some chores right now.", false);
			if(temp == 2) Render.text("Marble herself isn't at the camp right now; she is probably off getting supplies, though she'll be back soon enough.", false);
			if(temp == 3) {
				Render.text("Marble herself is resting on her bedroll right now.", false);
			}
			if(temp == 4) {
			...
			if(temp < 3) Render.text("  You're sure she'd be back in moments if you needed her.", false);
			marbleEvent = marbleScene.interactWithMarbleAtCamp;
		}
		//Out getting family
		else Render.text("Marble is out in the wilderness right now, searching for a relative.");
		Render.text("\n\n", false);
	}
	//AMILY
	if(amilyScene.amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 1 && flags[FlagEnum.AMILY_BLOCK_COUNTDOWN_BECAUSE_CORRUPTED_JOJO] == 0) {
		Render.text("Amily is currently strolling around your camp, ", false);
		temp = rand(6);
		if(temp == 0) {
			Render.text("dripping water and stark naked from a bath in the stream", false);
			if(player.statusAffects.has("CampRathazul")) Render.text(".  Rathazul glances over and immediately gets a nosebleed", false);
		}
		else if(temp == 1) Render.text("slouching in the shade of some particularly prominent rocks, whittling twigs to create darts for her blowpipe", false);
		else if(temp == 2) Render.text("dipping freshly-made darts into a jar of something that looks poisonous", false);
		else if(temp == 3) Render.text("eating some of your supplies", false);
		else if(temp == 4) Render.text("and she flops down on her nest to have a rest", false);
		else Render.text("peeling the last strips of flesh off of an imp's skull and putting it on a particularly flat, sun-lit rock to bleach as a trophy", false);
		Render.text(".\n\n", false);
	...
	if(arianScene.arianFollower()) Render.text("Arian's tent is here, if you'd like to go inside.\n\n");
	...
	if (flags[FlagEnum.ANT_WAIFU] > 0) addButton(8,"Phylla", getGame().desert.antsScene.introductionToPhyllaFollower);
...
public campSlavesMenu():void {
	...
	if(latexGooFollower() && flags[FlagEnum.FOLLOWER_AT_FARM_LATEXY] == 0) {
		Render.text(flags[FlagEnum.GOO_NAME] + " lurks in a secluded section of rocks, only venturing out when called for or when she needs to gather water from the stream.\n\n");
	...
	if(campCorruptJojo() && flags[FlagEnum.FOLLOWER_AT_FARM_JOJO] == 0) {
		Render.text("From time to time you can hear movement from around your camp, and you routinely find thick puddles of mouse semen.  You are sure Jojo is here if you ever need to sate yourself.\n\n", false);
	...
	if(milkSlave() && flags[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] == 0) {
		Render.text("Your well-endowed, dark-skinned milk-girl is here.  She flicks hopeful eyes towards you whenever she thinks she has your attention.\n\n");
	}
	...
	if (milkSlave() && flags[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] == 0) addButton(7,flags[FlagEnum.MILK_NAME],milkWaifu.milkyMenu);
	if (goo != null) addButton(8,flags[FlagEnum.GOO_NAME],goo);
	addButton(9, "Back", playerMenu);
}
...
public campFollowers():void {
	...
	//Pure Jojo
	if(player.statusAffects.has("PureCampJojo")) {
		Render.text("There is a small bedroll for Jojo near your own, though the mouse is probably hanging around the camp's perimeter.\n\n", false);
	...
	if(player.statusAffects.has("CampRathazul")) {
		...
		if(flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] <= 1) {
			Render.text("Tucked into a shaded corner of the rocks is a bevy of alchemical devices and equipment.  The alchemist Rathazul looks to be hard at work with his chemicals, working on who knows what.", false);
			if(flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] == 1) Render.text("  Some kind of spider-silk-based equipment is hanging from a nearby rack.  He's finished with the task you gave him!", false);
			Render.text("\n\n", false);
		}
		else Render.text("Tucked into a shaded corner of the rocks is a bevy of alchemical devices and equipment.  The alchemist Rathazul looks to be hard at work on the silken equipment you've commissioned him to craft.\n\n", false);
	}
	if(sophieFollower() && flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) {
		if(rand(5) == 0) Render.text("Sophie is sitting by herself, applying yet another layer of glittering lip gloss to her full lips.\n\n");
		else if(rand(4) == 0) Render.text("Sophie is sitting in her nest, idly brushing out her feathers.  Occasionally, she looks up from her work to give you a sultry wink and a come-hither gaze.\n\n");
		else if(rand(3) == 0) Render.text("Sophie is fussing around in her nest, straightening bits of straw and grass, trying to make it more comfortable.  After a few minutes, she flops down in the middle and reclines, apparently satisfied for the moment.\n\n");
		else if(rand(2) == 0 || flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] == 0) {
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) Render.text("Your platinum-blonde harpy, Sophie, is currently reading a book - a marked change from her bimbo-era behavior.  Occasionally, though, she glances up from the page and gives you a lusty look.  Some things never change....\n\n");
			else Render.text("Your pink harpy, Sophie, is currently reading a book.  She seems utterly absorbed in it, though you question how she obtained it.  Occasionally, though, she'll glance up from the pages to shoot you a lusty look.\n\n");
		}
		else {
			Render.text("Sophie is sitting in her nest, ");
			if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] < 5) {
				Render.text("across from your daughter");
				if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] > 1) Render.text("s");
			}
			else Render.text("surrounded by your daughters");
			Render.text(", apparently trying to teach ");
			if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] == 1) Render.text("her");
			else Render.text("them");
			Render.text(" about hunting and gathering techniques.  Considering their unusual upbringing, it can't be as easy for them...\n\n");
		}
	...
	if (helspawnFollower()) addButton(1, flags[FlagEnum.HELSPAWN_NAME], helSpawnScene.helspawnsMainMenu);
...
private rest():void {
	...
	if(timeQ == 0) {
		...
		if(timeQ != 1) Render.text("You continue to rest for " + num2Text(timeQ) + " more hours.\n", true);
		else Render.text("You continue to rest for another hour.\n", true);
	}
	goNext(timeQ,true);
}
...
private doWait():void {
	...
	Render.text("", true);
	if(timeQ == 0) {
		Render.text("You wait four hours...\n", false);
		timeQ = 4;
		//Marble withdrawl
		if(player.statusAffects.has("MarbleWithdrawl")) {
		...
		if(timeQ != 1) Render.text("You continue to wait for " + num2Text(timeQ) + " more hours.\n", false);
		else Render.text("You continue to wait for another hour.\n", false);
	}
	goNext(timeQ,true);
}
...
public doSleep(clrScreen:boolean = true):void {
	...
	if(timeQ == 0) {
		...
		if(marbleScene.marbleAtCamp() && player.statusAffects.has("CampMarble") && flags[FlagEnum.SLEEP_WITH] == "Marble" && flags[FlagEnum.FOLLOWER_AT_FARM_MARBLE] == 0) {
			...
			//(1) 
			if(rand(4) == 0) {
				Render.text("You curl up next to Sophie, planning to sleep for " + num2Text(timeQ) + " hour");
				if(timeQ > 1 ) Render.text("s");
				Render.text(".  She wraps her feathery arms around you and nestles her chin into your shoulder.  Her heavy breasts cushion flat against your back as she gives you a rather chaste peck on the cheek and drifts off towards dreamland...");
			}
			//(2) 
			else if(rand(3) == 0) {
				Render.text("While you're getting ready for bed, you see that Sophie has already beaten you there.  She's sprawled out on her back with her arms outstretched, making little beckoning motions towards the valley of her cleavage.  You snuggle in against her, her pillowy breasts supporting your head and her familiar heartbeat drumming you to sleep for " + num2Text(timeQ) + " hour");
				if(timeQ > 1) Render.text("s");
				Render.text(".");
			}
			//(3)
			else if(rand(2) == 0) {
				Render.text("As you lay down to sleep for " + num2Text(timeQ) + " hour");
				if(timeQ > 1) Render.text("s");
				Render.text(", you find the harpy-girl, Sophie, snuggling herself under her blankets with you.  She slips in between your arms and guides your hands to her enormous breasts, her backside already snug against your loins.  She whispers, \"<i>Something to think about for next morning...  Sweet dreams.</i>\" as she settles in for the night.");
			}
			//(4)
			else {
				Render.text("Sophie climbs under the sheets with you when you go to sleep, planning on resting for " + num2Text(timeQ) + " hour");
				if(timeQ > 1) Render.text("s");
				Render.text(".  She sleeps next to you, just barely touching you.  You rub her shoulder affectionately before the two of you nod off.");
			}
			Render.text("\n");
		}
		else {
			if(flags[FlagEnum.SLEEP_WITH] == "Helia" && kGAMECLASS.helScene.followerHel()) {
				Render.text("You curl up next to Helia, planning to sleep for " + num2Text(timeQ) + " ");
			}
			//Normal sleep message
			else Render.text("You curl up, planning to sleep for " + num2Text(timeQ) + " ", false);
			if(timeQ == 1) Render.text("hour.\n", false);
			else Render.text("hours.\n", false);
		}
		sleepRecovery(true);
	}
	else {
		if(timeQ != 1) Render.text("You lie down to resume sleeping for the remaining " + num2Text(timeQ) + " hours.\n", true);
		else Render.text("You lie down to resume sleeping for the remaining hour.\n", true);
	}
	goNext(timeQ, true);
}
...
private places():void { //Displays a menu for all known places
	...
	menu();
	if (flags[FlagEnum.BAZAAR_ENTERED] > 0) addButton(0, "Bazaar", getGame().bazaar.enterTheBazaar);
	if (player.statusAffects.has("BoatDiscovery")) addButton(1, "Boat", getGame().boat.boatExplore);
	...
	if (flags[FlagEnum.OWCA_UNLOCKED] == 1) addButton(6, "Owca", getGame().owca.gangbangVillageStuff);
	if (player.statusAffects.has("HairdresserMeeting")) addButton(7, "Salon", getGame().mountain.salon.salonGreeting);
	if (player.statusAffects.get("TelAdre").value1 >= 1) addButton(8, "Tel'Adre", getGame().telAdre.telAdreMenu);
	addButton(9, "Back", playerMenu);
}
...
private placesPage2():void {
	...
	//turn on ruins
	if (flags[FlagEnum.AMILY_VILLAGE_ACCESSIBLE] > 0) addButton(0, "TownRuins", amilyScene.exploreVillageRuin);
	if (flags[FlagEnum.MET_MINERVA] >= 4) addButton(1, "Oasis Tower", getGame().highMountains.minervaScene.encounterMinerva);
...
private dungeons():void {
	...
	if (player.statusAffects.has("FoundFactory")) addButton(1, "Factory", kGAMECLASS.enterFactory);
	if (flags[FlagEnum.DISCOVERED_WITCH_DUNGEON] > 0) addButton(2, "Desert Cave", kGAMECLASS.enterBoobsDungeon);
	if (flags[FlagEnum.D3_DISCOVERED] > 0) addButton(3, "Stronghold", kGAMECLASS.d3.enterD3);
...
private exgartuanCampUpdate():void {
	...
	if(player.statusAffects.has("Exgartuan"))
	{
		...
		{
			Render.text("", true);
			Render.text("<b>You suddenly feel the urge to urinate, and stop over by some bushes.  It takes wayyyy longer than normal, and once you've finished, you realize you're alone with yourself for the first time in a long time.", false);
			if(player.lowerBody.cockSpot.hasCock()) Render.text("  Perhaps you got too small for Exgartuan to handle?</b>\n", false);
			else Render.text("  It looks like the demon didn't want to stick around without your manhood.</b>\n", false);
		...
		{
			Render.text("", true);
			Render.text("<b>Black milk dribbles from your " + nippleDescript(0) + ".  It immediately dissipates into the air, leaving you feeling alone.  It looks like you became too small for Exgartuan!\n</b>", false);
			player.statusAffects.remove("Exgartuan");
		}		
	}
	doNext(playerMenu);
