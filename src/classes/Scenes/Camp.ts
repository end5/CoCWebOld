...
//		fixHistory();
	...
	if(flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] == 100 && player.statusAffects.has("CampMarble")) {
		...
		if(flags[FlagEnum.MARBLE_KIDS] == 0) outputText("future children", false);
		else {
			outputText(num2Text(flags[FlagEnum.MARBLE_KIDS]) + " child", false);
			if(flags[FlagEnum.MARBLE_KIDS] > 1) outputText("ren", false);
		}
		outputText(".", false);
	}
	//HARPY ROOKERY
	...
	if(player.statusAffects.has("CampMarble")) {
		...
		//Marble is out!
		if(flags[FlagEnum.MARBLE_PURIFICATION_STAGE] == 4) outputText("Marble isn’t here right now; she’s still off to see her family.");
		//requires at least 1 kid, time is just before sunset, this scene always happens at this time if the PC has at least one kid.
			...
			if(flags[FlagEnum.MARBLE_KIDS] == 1) outputText("child");
		...
		//after nightfall, scene always displays at this time unless PC is wormed
			...
			if(flags[FlagEnum.MARBLE_LUST] > 30) outputText("  She seems to be feeling antsy.");
		}
			...
			if(rand(2) == 0 && flags[FlagEnum.MARBLE_KIDS] > 5) outputText("Marble is currently tending to your kids, but she looks a bit stressed out right now.  It looks like " + num2Text(flags[FlagEnum.MARBLE_KIDS]) + " might just be too many for her to handle on her own...");
			//requires at least 4 kids
			else if(rand(3) == 0 && flags[FlagEnum.MARBLE_KIDS] > 3) outputText("Marble herself is in the camp right now, telling a story about her travels around the world to her kids as they gather around her.  The children are completely enthralled by her words.  You can't help but smile.");
			//Requires 2 boys
			...
			//requires at least 2 kids
			else if(rand(3) == 0 && flags[FlagEnum.MARBLE_KIDS] - flags[FlagEnum.MARBLE_BOYS] > 1) outputText("Marble herself is involved in a play fight with two of your kids brandishing small sticks.  It seems that the <i>mommy monster</i> is terrorising the camp and needs to be stopped by the <i>Mighty Moo and her sidekick Bovine Lass</i>.");
			else if(rand(3) == 0 && flags[FlagEnum.MARBLE_KIDS] > 1) outputText("Marble herself is out right now; she's taken her kids to go visit Whitney.  You're sure though that she'll be back within the hour, so you could just wait if you needed her.");
			else {
				//requires at least 1 kid
				if(rand(2) == 0) 
				{
					outputText("Marble herself is nursing ");
					if(flags[FlagEnum.MARBLE_KIDS] > 1) outputText("one of your cow-girl children");
				...
				else 
				{
					...
					if(flags[FlagEnum.MARBLE_KIDS] > 0) outputText("s");
	...
	if(player.statusAffects.has("CampRathazul")) {
		if(flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] <= 1) {
			...
			if(flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] == 1) outputText("  Some kind of spider-silk-based equipment is hanging from a nearby rack.  <b>He's finished with the task you gave him!</b>", false);
			outputText("\n\n", false);
		}
	...
	if(amilyScene.amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 1) {
		if(flags[FlagEnum.FUCK_FLOWER_LEVEL] >= 4) outputText("Amily has relocated her grass bedding to the opposite side of the camp from the strange tree; every now and then, she gives it a suspicious glance, as if deciding whether to move even further.");
	...
	if(campCorruptJojo() && flags[FlagEnum.FOLLOWER_AT_FARM_JOJO] == 0) outputText("From time to time you can hear movement from around your camp, and you routinely find thick puddles of mouse semen.  You are sure Jojo is here if you ever need to sate yourself.\n\n", false);
	//Pure Jojo
	if(player.statusAffects.has("PureCampJojo")) outputText("There is a small bedroll for Jojo near your own, though the mouse is probably hanging around the camp's perimeter.\n\n", false);
	//Izma
	...
	if(kGAMECLASS.desert.antsScene.phyllaWaifu()) {
		...
		if(flags[FlagEnum.ANT_KIDS] > 0) outputText(" one of your many children exit the anthill to unload some dirt before continuing back down into the colony.  It makes you feel good knowing your offspring are so productive.");
...
public function campLoversMenu():void {
	...
	if(isabellaFollower() && flags[FlagEnum.FOLLOWER_AT_FARM_ISABELLA] == 0) {
		...
		if(model.time.hours >= 21 || model.time.hours <= 5) outputText("Isabella is sound asleep in her bunk and quietly snoring.", false);
			...
			if(flags[FlagEnum.ISABELLA_MILKED_YET] == -1) outputText("Isabella has just returned from a late visit to Whitney's farm, bearing a few filled bottles and a small pouch of gems.", false);
	...
	if(amilyScene.amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 1 && flags[FlagEnum.AMILY_BLOCK_COUNTDOWN_BECAUSE_CORRUPTED_JOJO] == 0) {
		...
		if(temp == 0) {
			...
			if(player.statusAffects.has("CampRathazul")) outputText(".  Rathazul glances over and immediately gets a nosebleed", false);
		}
...
public function campSlavesMenu():void {
	...
	if(latexGooFollower() && flags[FlagEnum.FOLLOWER_AT_FARM_LATEXY] == 0) {
		outputText(flags[FlagEnum.GOO_NAME] + " lurks in a secluded section of rocks, only venturing out when called for or when she needs to gather water from the stream.\n\n");
...
public function campFollowers():void {
	...
	if(player.statusAffects.has("CampRathazul")) {
		...
		if(flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] <= 1) {
			outputText("Tucked into a shaded corner of the rocks is a bevy of alchemical devices and equipment.  The alchemist Rathazul looks to be hard at work with his chemicals, working on who knows what.", false);
			if(flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] == 1) outputText("  Some kind of spider-silk-based equipment is hanging from a nearby rack.  He's finished with the task you gave him!", false);
			outputText("\n\n", false);
		}
		else outputText("Tucked into a shaded corner of the rocks is a bevy of alchemical devices and equipment.  The alchemist Rathazul looks to be hard at work on the silken equipment you've commissioned him to craft.\n\n", false);
	}
	if(sophieFollower() && flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) {
		if(rand(5) == 0) outputText("Sophie is sitting by herself, applying yet another layer of glittering lip gloss to her full lips.\n\n");
			...
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) outputText("Your platinum-blonde harpy, Sophie, is currently reading a book - a marked change from her bimbo-era behavior.  Occasionally, though, she glances up from the page and gives you a lusty look.  Some things never change....\n\n");
			...
			if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] == 1) outputText("her");
...
private function exgartuanCampUpdate():void {
	...
	if(player.statusAffects.has("Exgartuan"))
	{
		...
		if(player.statusAffects.get("Exgartuan").value1 == 1 && (player.cockArea(0) < 100 || player.lowerBody.cockSpot.count() == 0))
		{
			...
			if(player.lowerBody.cockSpot.hasCock()) outputText("  Perhaps you got too small for Exgartuan to handle?</b>\n", false);
