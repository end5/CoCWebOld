...
//[LOSS SEXAHNZ]
		public function gobboAssassinBeatYaUp():void {
			...
			if(player.cockThatFits(monster.vaginalCapacity()) >= 0 && player.lowerBody.cockSpot.hasCock() && (!player.lowerBody.vaginaSpot.hasVagina() || rand(2) == 0)) 
			{
				...
				if(player.lowerBody.cockSpot.count() == 1) outputText("that thing");
				...
				if(player.lowerBody.balls > 0) outputText("[sack], giving it a gentle caress before vanishing.");
				else if(player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript() + ", the tips giving your soaked mound only the barest ghost of a touch.");
				...
				if (player.lowerBody.cockSpot.hasSheath()) outputText(" the sheath");
				...
				if(player.lowerBody.balls > 0) outputText("  A hand squeezes your [sack] encouragingly");
			...
			//iii. Lust Loss - Female
				...
				if(player.upperBody.chest.hasFuckableNipples()) outputText("piston in and out of your [nipple], masturbating it just as hard as the real thing.");
