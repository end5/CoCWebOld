...
package classes.Scenes.Monsters
{
	...
	public class GoblinAssassinScene extends BaseContent
	{
		...
		public goblinAssassinEncounter():void {
			...
			{
				Render.text("A needle whizzes through the air and lands next to your foot. The fine craftsmanship along the side of the glass tube only brings you to one conclusion - it's owner has access to precision machinery and manufacturing capabilities uncommon in this land.");
			...
			//Repeat Intro - Cell Chambers
				...
				Render.text("A familiar sight catches your attention as a shadowy blur lands in front of you - another goblin assassin appears to be lurking around these parts. You ready your [weapon] as she straps on a belt loaded with various types of needles and assumes a combat stance.");
				//[Initiate combat encounter – goblin assassin]
			}
...
//[LOSS SEXAHNZ]
		public gobboAssassinBeatYaUp():void {
			...
			if(player.cockThatFits(monster.vaginalCapacity()) >= 0 && player.lowerBody.cockSpot.hasCock() && (!player.lowerBody.vaginaSpot.hasVagina() || rand(2) == 0)) 
			{
				...
				if(player.lowerBody.cockSpot.count() == 1) Render.text("that thing");
				else Render.text("those things");
				Render.text(" look mighty tasty.  You don’t mind if I sample just a little bit of your seed do you?</i>\"  The sapphire-maned beauty kneels down next to you and lets her tiny, delicate digits dance across the underside of [oneCock], drawing a lurid moan from your lips as she wonders out loud, “I don’t hear a no, " + player.mf("stud","sexy") + ".  That’s good... I’m just going to take a little bit of cum... just a few squirts.  Surely a ");
				if(player.tallness >= 60) Render.text("big, ");
				Render.text(player.mf("strapping young man","sexy breeder") + " will have enough jizz to pay for " + player.mf("his","her") + " trespass?</i>\"");
				Render.text("  Those delightful, dancing fingers pirouette down to your ");
				if(player.lowerBody.balls > 0) Render.text("[sack], giving it a gentle caress before vanishing.");
				else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(vaginaDescript() + ", the tips giving your soaked mound only the barest ghost of a touch.");
				else Render.text("taint, the tips giving your sensitive skin only the barest ghost of a touch.");
				...
				if(player.femininity < 40) Render.text("almost ");
				Render.text("girlish moan.  The need thrumming through [eachCock] is telling you - no, demanding - that you do what she says and fuck her.  Like a beast in rut, you groan.  You look the short-but-stacked aggressor in her big, liquid blue eyes and make a decision entirely rationalized by your aching, drug-fueled needs.  Still pumping your shaft like a man possessed, you whimper, \"<i>Yes... I... I came to have sex with you.</i>\"");
				Render.text("\n\nVeridian skin caresses you just above");
				if (player.lowerBody.cockSpot.hasSheath()) Render.text(" the sheath");
				else Render.text("your base");
				...
				Render.text(" wild with need.  Thankfully, as soon as you submit to her puffy mound and give it a lick, a pair of soft, wrinkled feet wrap around your " + cockDescript(x) + ", drawing out a thick flood of your pre-cum.");
				...
				Render.text("\n\n\"<i>");
				if(silly()) Render.text("Stop right there, criminal scum.");
				else Render.text("Hold your horses.");
				Render.text("</i>\"  The emerald assassin glances over her shoulder at you and urges, \"<i>Come on now.  Fill me up.  Let out all that spunk I’ve been making build up ");
				if(player.lowerBody.balls > 0) Render.text("in your balls");
				else Render.text("inside you");
				Render.text(".  Pay your ‘fine’, and I might let you go.</i>\"");
				if(player.lowerBody.balls > 0) Render.text("  A hand squeezes your [sack] encouragingly");
				else Render.text("  Hands squeeze your [butt] encouragingly");
				Render.text(" as the goblin impales herself to the hilt with a sloppy squelch.  You orgasm, cumming hard inside her.  Your back arches, your body clenches, and rope after rope of seed sprays inside the goblin’s baby-hungry womb.  Her pussy seems to drink it down, wicking the jizz away from you just as fast as you can put it out.");
				if(player.cumQ() >= 750) {
					Render.text("  Even her near-supernatural fertility can’t keep up with your fantastic virility, and soon the goblin is sporting a small bump on her belly.");
					if(player.cumQ() >= 2000) Render.text("  It swells to a rounded, pregnant dome by the time you finish.");
				}
				Render.text("  All of your passion spends itself in the glorious climax, leaving you feeling like an empty, wasted husk.");
			...
			else if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > monster.vaginalCapacity() && player.lowerBody.cockSpot.hasCock() && (!player.lowerBody.vaginaSpot.hasVagina() || rand(2) == 0))
			{
				...
				Render.text("You tear through your [armor] in a rush, anything to release [eachCock] from its hellish confinement.  You’re so aroused, so horny.  Any passing thoughts of modesty are immediately discarded and forgotten, washed away under the wave of your all-consuming lust as your fingers slide home around [oneCock] and begin to play with the turgid tool.  You immediately buck from the sensation of incredible, pent-up need, nearly cumming on the spot.  Looking up, you meet the emerald-skinned woman’s eyes pleadingly.");
				...
				if(player.biggestCockLength() > player.tallness/2) Render.text("  Your incredibly long phallus blocks your view of the sultry goblin, and you never see the blow coming.");
				Render.text("  A none-too-tender swat connects with your ");
				if(player.lowerBody.balls > 0) Render.text("[balls]");
				else Render.text("cock");
				Render.text(" as the goblin grunts, \"<i>Stop squirming!</i>\"");
				...
				if(player.biggestCockLength() < player.tallness/2) Render.text("sitting on your face, the " + player.cockHead(x) + " of your maleness jutting against her drooling lips.");
				else Render.text("sitting on the ground ‘above’ you, your " + player.cockHead(x) + " reaching all the way up to spear her moist box, judging by what you’re feeling anyway.");
				...
				if(player.cumQ() >= 1000) Render.text("You spurt huge dollops of pre-cum into her waiting twat thanks to your prodigious virility, though it is only a hint of things to come.  ");
				else if(player.cumQ() >= 250) Render.text("You drool a steady flow of pre-cum into her waiting twat, though it’s only a hint of things to come.  ");
				Render.text("  \"<i>It won’t be long now.  Yeah, I’m an assassin, but I take care of myself too.  I keep these puppies nice and moisturized, just in case I need to use them on a freak-cock like yours.</i>\"");

				Render.text("\n\nYou try to grunt in protest, but it’s muffled by your ");

				if(player.biggestCockLength() >= player.tallness/2) Render.text("pulsating, iron-hard cock");
				else Render.text("captor’s pillowy asscheeks");
				Render.text(".  The goblin pulls hard with her feet, and her sloppy, stretched cunt just barely slurps in your entire " + player.cockHead(x) + ".  You shiver in delight from the tight, vise-like grip constricting your maleness, and nearly erupt from sensation when the little green minx manages to pull another inch inside herself.  She trembles and shudders, her feet suddenly going wild along your length.  The goblin’s cushy soles smear along your " + cockDescript(x) + " with the wild abandon of one lost to pleasure, her juices squirting out from her slit as she climaxes.  The onslaught of touches is too much for your straining member, and with a tight, hot clench, you feel your imminent release has arrived.  A fat bulge appears at the base of your shaft, coupled with an explosion of warm pleasure from within you.  It travels from your base all the way towards the tip, before disappearing into the goblin with a cunt-stretching squirt.  She moans out loud, clearly enjoying the sensation of being stuffed with cream, and the two of you sigh together while your dick finishes pumping ");
				if(player.cumQ() <= 10) Render.text("dribbles");
				else if(player.cumQ() <= 100) Render.text("squirts");
				else if(player.cumQ() <= 250) Render.text("thick wads");
				else if(player.cumQ() <= 1750) Render.text("waves of gooey spooge");
				else Render.text("rivers of cum");
				Render.text(" into the goblin’s box.");
				if(player.cumQ() >= 2500) Render.text(" Her belly rounds up nicely under your voluminous attentions.");
			...
			//iii. Lust Loss - Female
				...
				if(player.upperBody.chest.hasFuckableNipples()) Render.text("piston in and out of your [nipple], masturbating it just as hard as the real thing.");
				else Render.text("squeeze around a taut nipple, twisting and pulling on it unthinkingly.");
				Render.text("  Your masturbatory reverie is interrupted sudden impacts against both your wrists.");
