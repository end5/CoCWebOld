	...
	public class AbstractSuccubus extends Monster
	{
		protected whipAttack():void
		{
			if (statusAffects.has("WhipReady")) {
				...
				//Whip yo tits!
				if (temp == 1) {
					if (player.upperBody.chest.count() > 0 && player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) MainScreen.text(player.allBreastsDescript() + " (9)", false);
				...
				if (temp == 2) {
					...
					if (player.gender == 1) {
						MainScreen.text("groin, dealing painful damage to your " + player.multiCockDescriptLight() + ", doubling you over in agony (" + int((player.tou * 2 + 50) / 4) + ")", false);
						game.dynStats("lus", -15);
						player.takeDamage(int((player.maxHP()) / 4));
					}
					if (player.gender == 2) {
						MainScreen.text("groin, making your " + vaginaDescript(0) + " sting with pain (-10)", false);
						player.takeDamage(10);
						game.dynStats("lus", -8);
					}
					if (player.gender == 3) {
						MainScreen.text("groin, dealing painful damage to your " + player.multiCockDescriptLight() + " and " + player.vaginaDescript(0) + ", doubling you over in agony (" + int((player.tou * 2 + 50) / 3) + ")", false);
		...
		protected kissAttack():void
		{
			...
			if (player.stats.spe > spe && rand(4) == 0 || (player.perks.has("Evade") && rand(4) == 0) || (player.perks.has("Misdirection") && rand(4) == 0 && player.armorName == "red, high-society bodysuit")) {
				...
				if (player.lust >= 70) MainScreen.text("  Maybe you shouldn't have bothered to move, it might have been fun.", false);
			}
			//get hit
			else {
		...
		protected seduceAttack():void
		{
			...
			if (temp == 1 && breastRows[0].breastRating >= 2) {
				...
				if (breastRows.length > 1) {
					...
					if (lust >= 50) MainScreen.text(", your " + nippleDescript(0) + "s painfully visible.", false);
