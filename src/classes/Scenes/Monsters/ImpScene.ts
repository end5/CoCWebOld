...
package classes.Scenes.Monsters
{
	...
	public class ImpScene extends BaseContent
	{
		...
		private function rapeImpWithDick():void {
			...
			if(player.lowerBody.cockSpot.count() == 1) {
				...
				if(player.lowerBody != LOWER_BODY.CENTAUR) {
					...
					if(player.upperBody.chest.count() > 0 && player.upperBody.chest.list[0].breastRating > 2) outputText("smashing him against your " + breastDescript(0) + " while you jerk hard on your " + cockDescript(x) + ", bringing it to a full, throbbing erection.", false);
				...
				if(player.lowerBody.cockSpot.list[0].cockType == CockType.CAT) outputText("  The tiny creature's claws dig into your sides at the feeling of soft, hooked barbs stroking his sensitive insides.", false);
				if(player.lowerBody.cockSpot.list[0].cockLength >= 7 && player.lowerBody.cockSpot.list[0].cockLength <= 12) outputText("  Each thrust obviously distorts the imp's abdomen.  It amazes you that it doesn't seem to be hurting him.", false);
				if(player.lowerBody.cockSpot.list[0].cockLength > 12) outputText("  Each plunge into the imp's tight asshole seems to distort its entire body, bulging obscenely from its belly and chest.  Amazingly he doesn't seem to mind, his efforts focused solely on his sorely throbbing demon-dick.", false);
				outputText("\n\nThe tight confines of the imp's ass prove too much for you, and you feel your orgasm build.", false);
				if(player.lowerBody.balls == 0 && player.lowerBody.vaginaSpot.count() > 0) outputText("  The cum seems to boil out from inside you as your " + vaginaDescript(0) + " soaks itself.  With delicious slowness you fire rope after rope of cum deep into the imp's rectum.", false);
				if(player.lowerBody.balls == 0 && player.lowerBody.vaginaSpot.count() == 0) outputText("  The cum seems to boil out from inside you, flowing up your " + cockDescript(x) + ".  With delicious slowness, you fire rope after rope of cum deep into the imp's rectum.", false);
			...
			if(player.lowerBody.cockSpot.count() >= 2) {
				...
				if(player.lowerBody != LOWER_BODY.CENTAUR) {
					...
					if(player.upperBody.chest.count() > 0 && player.upperBody.chest.list[0].breastRating > 2) outputText("smashing him against your " + breastDescript(0) + " while you jerk hard on one of your " + cockDescript(x) + "s, bringing it to a full, throbbing erection.", false);
				...
				if(player.lowerBody.cockSpot.list[0].cockLength >= 7 && player.lowerBody.cockSpot.list[0].cockLength <= 12) outputText("  Each thrust obviously distorts the imp's abdomen.  It amazes you that it doesn't seem to be hurting him.", false);
				if(player.lowerBody.cockSpot.list[0].cockLength > 12 && player.lowerBody.cockSpot.list[0].cockLength <= 18) outputText("  Each plunge into the imp's tight asshole seems to distort its entire body, bulging obscenely from its belly and chest.  Amazingly he doesn't seem to mind, his efforts focused solely on his sorely throbbing demon-dick.", false);
				outputText("\n\nThe tight confines of the imp's ass prove too much for you, and you feel your orgasm build.", false);
				if(player.lowerBody.balls > 0) outputText("The cum seems to boil in your balls, sending heat spreading through your " + cockDescript(x) + " as your muscles clench reflexively, propelling hot spurts of jism deep into the imp's rectum.  Your other equipment pulses and dripples steady streams of its own cum.", false);
				if(player.lowerBody.balls == 0 && player.lowerBody.vaginaSpot.count() > 0) outputText("The cum seems to boil out from inside you as your " + vaginaDescript(0) + " soaks itself.  With delicious slowness you fire rope after rope of cum deep into the imp's rectum.  Your other equipment drizzles small streams of jizz in sympathy.", false);
				if(player.lowerBody.balls == 0 && player.lowerBody.vaginaSpot.count() == 0) outputText("The cum seems to boil out from inside you, flowing up your " + cockDescript(x) + ".  With delicious slowness, you fire rope after rope of cum deep into the imp's rectum.  Your other equipment drizzles small streams of jizz in sympathy.", false);
				if(player.cumQ() >= 14 && player.cumQ() <= 30) outputText("  Your orgasm drags on and on, until your slick jism is dripping out around your " + cockDescript(x) + ".", false);
				if(player.cumQ() > 30 && player.cumQ() <= 100) outputText("  Your orgasm seems to last forever, jizz dripping out of the imp's asshole around your " + cockDescript(x) + " as you plunder him relentlessly.", false);
				if(player.cumQ() > 100) outputText("  Your orgasm only seems to grow more and more intense as it goes on, each spurt more powerful and copious than the last.  The imp begins to look slightly pregnant as you fill him, and tiny jets of cum squirt out around your " + cockDescript(x) + " with each thrust.", false);
				outputText("\n\nSatisfied at last, you pull him off just as he reaches his own orgasm, splattering his hot demon-cum all over the ground.   You drop the imp hard and he passes out, dripping mixed fluids that seem to be absorbed by the dry earth as fast as they leak out.", false);
		...
		private function sprocketImp():void {
			...
			//Variable cocktext
			if(player.lowerBody.cockSpot.list[0].cockType == CockType.HUMAN || player.lowerBody.cockSpot.list[0].cockType == CockType.DEMON || player.lowerBody.cockSpot.list[0].cockType.Index > 4) outputText("your " + cockDescript(0) + ", rubbing the tip of his prick against your own, ", false);
			else if(player.hasKnot(0)) outputText("your " + cockDescript(0) + ", rubbing the tip of his prick against your point, ", false);
			else if(player.lowerBody.cockSpot.list[0].cockType == CockType.HORSE) outputText("your " + cockDescript(0) + ", rubbing the tip of his prick against your flared head, ", false);
			else if(player.lowerBody.cockSpot.list[0].cockType == CockType.TENTACLE) outputText("your huge green dick, rubbing the tip of his prick against your purplish cock-head, ", false);
			...
			if(player.lowerBody.cockSpot.list[0].cockType == CockType.HUMAN || player.lowerBody.cockSpot.list[0].cockType == CockType.DEMON || player.lowerBody.cockSpot.list[0].cockType.Index > 4) outputText("shoves his shaft deeply into the slit in the head of your dick.  ", false);
			...
			else if(player.lowerBody.cockSpot.list[0].cockType == CockType.HORSE) outputText("seats his dick in the flared head of your prick, and then pushes farther. His shaft plunges into yours, filling your cock more than any cum load ever could.  ", false);
			else if(player.lowerBody.cockSpot.list[0].cockType == CockType.TENTACLE) outputText("shoves his dick deeply into the slit in the head of your vine-like cock.  ", false);
			//New PG
			outputText("\n\n", false);
			...
			if(player.upperBody.chest.count() > 0 && player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText(", fucking you so hard that your " + player.allBreastsDescript() + " bounce with each thrust.  ", false);
			...
			if(player.lowerBody.cockSpot.count() == 2) outputText("your other cock finding pleasure in rubbing against his body ", false);
			if(player.lowerBody.cockSpot.count() > 2) outputText("your other cocks finding pleasure in rubbing against his body ", false);
			//Cum
			...
			//Extra cum-texts
			if(player.lowerBody.cockSpot.count() == 2) outputText("Your other cock cums at the same time, liberally splattering your spunk up his back.  ", false);
			if(player.lowerBody.cockSpot.count() > 2) outputText("The rest of your " + multiCockDescriptLight() + " twitch and release their seed at the same time, creating a shower of spunk that rains down on both you and the imp, coating both of your bodies.  ", false);
			...
			//Vagoooz
			if(player.lowerBody.vaginaSpot.count() > 0) outputText("Your pussy quivers, contracting furiously as your orgasm hits you - like it's trying to milk a phantom dick dry.  ", false);
			//new PG
		...
		private function centaurGirlOnImps():void {
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) {
				...
				//[Has breasts: 
				if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText("  Your hands reflexively grab your " + chestDesc() + " and mash them about.", false);
				outputText("\n\n", false);
				...
				if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) {
					...
					if(player.upperBody.chest.hasFuckableNipples()) outputText("  His fingers slide inside your " + nippleDescript(0) + "s and start spreading and squishing them. Your femcum leaks out over his hands and soon your front is slick and shiny.", false);
					//All other nipples: 
		...
		private function centaurOnImpStart():void {
			...
			//<< Cor <50 >> 
			if(player.stats.cor < 50) outputText("  You lick your lips slightly as you begin to approach the small figure.", false);
			...
			if(player.lowerBody.cockSpot.count() > 1 && player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockLength >= 24) {
				...
				//<< 1 or 2 cocks, Cor 50+>>
					...
					//<<Cor 80+>> 
						...
						if(player.upperBody.chest.hasFuckableNipples()) outputText("Shoving your fingers deep into your " + nippleDescript(0) + "s", false);
				...
				//<< 3+ cocks, Cor 80+>>
					...
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 0) {
						...
						//<<nips have pussies>> 
						if(player.upperBody.chest.hasFuckableNipples()) outputText(" and fingering your " + nippleDescript(0) + "s", false);
		...
		private function centaurOnImpFemale(vape:boolean = false):void {
			...
			//<<Gaping>>
				...
				//<<Cor 50+>>
				if(player.stats.cor >= 50) outputText("You take comfort in knowing that at least there is a cock inside of you, and that soon it will be filling you with its seed.  Perhaps it might even impregnate you!  ", false);
				...
				//<<Cor <80>>
				if(player.stats.cor < 80) outputText("Tears pour out of your eyes and you are sure you must be bleeding slightly, ", false);
				//<<Cor <50>>
				if(player.stats.cor <50) outputText("and you hang on to the tree, afraid of the pain from even the slightest movement.  ", false);
				//<<Cor 50+>>
				else outputText("and you hang on to the tree, grunting like a rutting animal as you delight in the intense pain.  ", false);
				//<<Cor 80+>>
				if(player.stats.cor >= 80) outputText("You howl out in pain and pleasure, bucking and hoping to intensify the sensation, hurling enticements and insults at the imp like a slut.  ", false);
				//<<Cor 50+, Breasts>>
				if(player.stats.cor >= 50 && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) {
					...
					//<<w/ nip-pussies>>
					if(player.upperBody.chest.hasFuckableNipples()) outputText(" and shoving your fingers into your " + nippleDescript(0) + ".  ", false);
					else outputText(".  ", false);
					//<</Breasts>>
				}
				...
				if(player.analCapacity() >= 35) {
					...
					if(player.ass.analLooseness < 4 && player.stats.cor < 80) outputText("Your howl of pain leaves your throat raw.  ", false);
				...
				if(player.stats.cor < 80) outputText("promise to yourself that you will not do that again.", false);
		...
		*/
		...
		private function centaurOnImpResults(iNum:number):void {
			...
			if(iNum == 1) {
				...
				if(player.cumQ() >= 250) {
					//<<no knot>>
					if(player.lowerBody.cockSpot.list[x].cockType != CockType.DOG) outputText("Soon the amount is overflowing from the abused " + eAssholeDescript() + ", dripping between you with no sign of stopping as you continue thrusting yourself into the imp.  ", false);
					//<<knot>>
					...
					//<<I1_1>>
					//<<2 cocks>>
					if(player.lowerBody.cockSpot.count() == 2) outputText("Your other cock drenches the imp's back with its own secretions that immediately start dripping down its sides.  ", false);
					//<<3+ cocks>>
					if(player.lowerBody.cockSpot.count() > 2) outputText("Your other cocks release their cum all over the creature's back and sides, leaving it a glazed mess.  ", false);
					//<</I1_1>>
		...
		public function impGangabangaEXPLOSIONS():void {
			...
			if(player.lowerBody == LOWER_BODY.CENTAUR) {
				if(rand(2) == 0 && (player.lowerBody.cockSpot.count() == 0 || player.gender == 3)) {
					...
					if(player.findStatusAffect(StatusAffects.ImpGangBang) < 0) {
						...
						if(player.lowerBody.cockSpot.count() > 0) outputText(cockDescript(0), false);
					...
					//(If the player has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText("Having your arms tied behind your back forces your chest out, making your " + player.allBreastsDescript() + " stand out. They bounce as you struggle.  ", false);
					...
					//(Low Corruption)
					if(player.stats.cor < 50) outputText("Your face flushes with humiliation. Your imp rider twists your " + hairDescript() + " hard and you whimper in pain. Imps rub their cocks along your " + hipDescript() + " while others stroke themselves and jeer at your helplessness.  ", false);
					//(High Corruption)
					...
					if(player.lowerBody.balls > 0) outputText("The tongue slides over your " + sackDescript() + ", coating it with warm drool.  ", false);
					//(Low Corruption)
					if(player.stats.cor < 50) outputText("You shake your hips, trying to escape the demonic tongue. The imp grips your " + hipDescript() + " and pulls his face further into your cunt, sliding his nimble tongue over your lips. You grit your teeth, trying to ignore the warmth spreading from your " + vaginaDescript(0) + ".", false);
					//(High Corruption)
					...
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) {
						...
						//(Low Corruption)
						if(player.stats.cor < 50) outputText("You can't escape the tongues lapping and pulling at your " + nippleDescript(0) + ", matching the one in your cunt. You shake your head to deny the pleasure, but your breathing comes faster and faster as lust invades your body.", false);
						//(High Corruption)
					...
					outputText("Dimly through your haze of lust and pain you see a large imp step forward from the mob. Four feet tall and broader and stronger looking than any imp you've seen before, with a face as much bull as imp, this new imp has mottled grey skin, broad purple demon wings, two curving bull-horns on his head, and a " + Appearance.cockNoun(CockType.HORSE) + " big enough to choke a minotaur. The mushroom-like head of it bobs just below his mouth, and his snake-tongue darts out to flick a bit of pre-cum off the head and onto your face. You shudder as the hot fluid stings the sensitive skin of your lips. His " +  monster.ballsDescriptLight() + " are each the size of your fist and slick with sweat. He slaps his sweaty cock-head against your cheek, nearly scalding you with the heat.  ", false);
					//(Low corruption)
					if(player.stats.cor < 50) outputText("You yelp and twist your head to escape the heat.  ", false);
					//(End low corruption) 
					...
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText("The big imp grabs one of your painfully distended breasts in each hand, mauling and bouncing the flesh as if weighing them. You gasp in pain as your " + player.allBreastsDescript() + " swell further at his touch. ", false);
					...
					//(Low corruption)
					if(player.stats.cor < 50) outputText("It's either swallow or have that demon-tongue forced all the way down your throat. Against your will you gulp back the glob.", false);
					//(High Corruption)
					...
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText("The other imps reclaim your aching breasts, sucking your " + nippleDescript(0) + " and mauling your " + player.allBreastsDescript() + " so hard their fingers disappear into your swelling flesh. ", false);
					outputText("The imp rubs his hands over your sides and flanks, his " + Appearance.cockNoun(CockType.HORSE) + " bobbing as he walks. The other imps watch their master as he moves around you. Only the imp sucking your " + vaginaDescript(0) + " doesn't notice, his tongue thrusting deeply into your folds. The big imp grabs him by the neck and easily tosses him aside, his tongue dragging through your cunt as he's pulled away from you. The master imp takes position behind you and grabs his " + Appearance.cockNoun(CockType.HORSE) + ", bringing the mushroom-head of it down to your pussy. You shake, knowing what's coming next. The other imps watch and stroke themselves as their master readies his hips to push into you.\n\n", false);
					//(Low corruption)
					if(player.stats.cor < 50) outputText("You scream for help", false);
					//(High corruption) 
					...
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText("The imps working your breasts suck harder, kneading your tit-flesh as though trying to milk you. ", false);
					...
					//(Low corruption)
					if(player.stats.cor < 50) outputText(" in helpless terror as you feel the bulge work up from the base of your groin towards your stomach. You let out a shuddering moan of pain as inch after inch of monstrous " + Appearance.cockNoun(CockType.HORSE) + " stretches your belly", false);
					//(High corruption)
					else outputText(", panting in lust as the monstrous " + Appearance.cockNoun(CockType.HORSE) + " pushes your flesh aside to make room for itself", false);
					outputText(". ", false);
					...
					//(Low corruption)
					if(player.stats.cor < 50) outputText("whimper, spasming as the hot shaft presses against new areas", false);
					//(High corruption)
					...
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("your " + player.allBreastsDescript() + " roll on your chest as you squirm", false);
					//(If the character doesn't have breasts)
					else outputText("your eyes roll back as you squirm", false);
					outputText(".\n\n", false);
					
					outputText("Finally the big imp pulls back his " + Appearance.cockNoun(CockType.HORSE) + ", each ridge pulling on your pussy flesh as he slides out. You yelp and buck as the mushroom-head catches on your folds. ", false);
					//(If the character has a cock)
					if(player.lowerBody.cockSpot.count() > 0) outputText("Your " + multiCockDescriptLight() + " bounces as the bulge passes over it.  ", false);
					...
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("The slow fucking shakes your breasts, and the imps sucking at your nipples cling tightly to your monstrously swollen " + player.allBreastsDescript() + ". Your " + biggestBreastSizeDescript() + " have grown three cup sizes since the imps pumped their venom into you. An ache starts deep in the base of your tits and works its way to your sore " + nippleDescript(0) + ". Your already bloated nipples swell as the imps suckle and you gasp as the first rush of milk spills into their mouths. Your rider reaches around and starts to milk your udders, moving his hands between your " + player.allBreastsDescript() + " and forcing out more milk for his gangmates.\n\n", false);
					...
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText("Imps gulp milk from your bloated " + biggestBreastSizeDescript() + ". As one imp drinks his fill and staggers away with a sloshing belly, another steps up to pump your milk-spewing udders.  ", false);
					//(If the character has a dick)
					if(player.lowerBody.cockSpot.count() > 0) outputText("Your " + multiCockDescriptLight() + " swell painfully as the rough fucking pumps blood into your groin.  ", false);
					...
					//(Low Corruption)
					if(player.stats.cor < 50) outputText("You're being raped by a demon, milked like a cow, and you're about to cum hard. This corrupted land has left its mark on you.", false);
					//(High corruption)
					...
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) outputText("Your udders shake back and forth under your chest in time to the rough fucking. You arch your back to press your " + nippleDescript(0) + " into eager mouths, moaning as your rider milks your distended " + player.allBreastsDescript() + ". ", false);
					//(Low Corruption).
					if(player.stats.cor < 50) outputText("Some part of you can still feel shame, and you whine and clench your teeth as the urge to <i>moo</i> rises in you.", false);
					//(High corruption)
					...
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText(", and bouncing " + player.allBreastsDescript(), false);
					...
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText("milk spurts from your " + nippleDescript(0) + " and ", false);
					...
					//(Low corruption)
					if(player.stats.cor < 50) outputText("Your last coherent thought is to find a way to better hide your camp, so this never happens again.", false);
					//(High corruption)
				...
				//Scene number 2 - male possible.
					...
					if(player.statusAffects.has("ImpGangBang")) {
						...
						if(player.stats.cor < 50) outputText("You can't tell if this is the same " + monster.short + " as last time or not. You're not racist, but all imps look alike to you. " + monster.capitalA  + " surges forward, grabbing at your legs and arms and running their hands over your body. You struggle, but there are just too many to fight. The result is the same as last time...\n\n", false);
					...
					//(Low Corruption)
					if(player.stats.cor < 50) outputText("swing your " + player.weaponName + " wildly, determined not to let them take you", false);
					//(High Corruption)
					...
					//(If the player has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) outputText("Your unbound " + biggestBreastSizeDescript() + " bounce out over the weathered log. ", false);
					...
					//(Low Corruption)
					if(player.stats.cor < 50) outputText("Your face flushes with humiliation as they turn their attentions on each other, each working their hands and tongue over the other's dick. How dare these demons use you as a bed to sate their lusts?!", false);
					//(High Corruption)
					...
					//(If the player has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText("An imp mounts the log and slaps his " + monster.cockDescriptShort(0) + " between your " + player.allBreastsDescript() + ", squeezing them tight over his cock as he rubs back and forth. He mauls your breasts cruelly, squeezing his fingers deep into your soft flesh.  ", false);
					//(If the player has a SINGLE cock)
					if(player.lowerBody.cockSpot.count() == 1) outputText("An imp ducks under your body and grabs your " + cockDescript(0) + ". His nimble tongue flicks over your cock-head while he pricks the shaft with his tiny claws.  ", false);
					//(If the player has a MULTI cock)
					if(player.lowerBody.cockSpot.count() > 1) outputText("Two imps duck under your body and seize your " + multiCockDescriptLight() + ", licking the tips with their inhumanly flexible tongues while they stroke the shafts.  ", false);
					//(Low Corruption)
					if(player.stats.cor < 50) outputText("You fight to free your hind legs and buck the imps off your back, while sweaty hands slide over your crotch. You whine through clenched teeth as sharp claws jab at your sensitive flesh.\n\n", false);
					//(High Corruption)
					...
					//(If the player has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText("The imp fucking your " + biggestBreastSizeDescript() + " handles your soft flesh roughly, pressing and pulling your tits into a fuck-canal for his demon cock. Other imps slap your " + player.allBreastsDescript() + " and laugh as you cry out.  ", false);
					//(Low Corruption)
					if(player.stats.cor < 50) outputText("You whimper as your mistreated flesh stings with dozens of pin-prick scratches and bites, and the " + monster.short + " slaps your chest and flanks. The abuse falls on you from all sides, leaving you with no escape. The imp on your shoulders pries your jaws open, and you gag on his " + monster.ballsDescriptLight()  + ".", false);
					//(High Corruption)
					...
					//(If the player has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText("one of your sore " + biggestBreastSizeDescript() + " in his calloused hand, brutally pressing his fingers into your flesh", false);
					...
					outputText("The master imp walks back to your hips, lightly dragging his sharp claws over your flanks. He kicks another imp out of the way and takes position behind your " + hipDescript()  + ". He pulls his monstrously long " + Appearance.cockNoun(CockType.DOG) + " down and rubs the tip over your ", false);
					...
					outputText(".  ", false);
					//(If the player has a cock)
					if(player.lowerBody.cockSpot.count() > 0) outputText("Pre-cum drips from the broad tip of it, dripping down to the base of your " + multiCockDescriptLight() + ".  ", false);
					outputText("The big imp's hot pre-cum stings your flesh. The imps licking your crotch lap up the hot fluid, cooling you with their saliva. The big imp sneers as you whimper, and presses the head of his " + Appearance.cockNoun(CockType.DOG) + " against your ", false);
					//(If the player has a vagina)
					if(player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript(0), false);
					//(If the player doesn't have a vagina)
					else outputText(assholeDescript(), false);
					outputText(". ", false);
					//(Low Corruption)
					if(player.stats.cor < 50) outputText("You try to pull away from the hot cock-head rubbing against your hole, but the " + monster.short + " holds you tight.", false);
					//(High Corruption)
					else outputText("The scent of musk steaming off the" + Appearance.cockNoun(CockType.DOG) + " drives you wild, and you push back to try and capture the cock-tip.", false);
					outputText("\n\n", false);
					
					outputText("The pointed tip of the master imp's " + Appearance.cockNoun(CockType.DOG) + " plunges into your hole, splitting your ", false);
					//(If the player has a vagina)
					if(player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript(0), false);
					//(If the player doesn't have a vagina)
					else outputText(assholeDescript(), false);
					...
					outputText(".", false);
					//(If the player has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText("  The imp fucking your aching " + biggestBreastSizeDescript() + " paints your tits with a massive load of cum. He falls off the log and another imp jumps up to take his place.", false);
					outputText("\n\n", false);
					
					outputText("The big imp fucks you roughly, clenching your " + hipDescript()  + " in his clawed hands as he hammers his " + Appearance.cockNoun(CockType.DOG) + " into you. The head of his mutated shaft pounds ", false);
					//(If the player has a vagina)
					if(player.lowerBody.vaginaSpot.hasVagina()) outputText("the entrance of your womb", false);
					//(If the player doesn't have a vagina)
					...
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText("The rough fucking shakes your cum-stained breasts, and the imp fucking your " + player.allBreastsDescript() + " clings tightly to your red and swollen tit flesh. Your " + biggestBreastSizeDescript() + " burn with agony as the " + monster.short + " slaps your tits like drums.  ", false);
					//(Low Corruption)
					if(player.stats.cor < 50) outputText("You're being raped again by demons, impaled on cocks like a roast pig on a spit, and you can feel your lust rising. This corrupted land has left its mark on you.", false);
					//(High corruption)
					else outputText("This corrupted land has left its mark on you. You could never have taken a cock this big before you arrived here.", false);
					outputText("\n\n", false);
		
					//(Low Corruption)
					if(player.stats.cor < 50) outputText("You gurgle helplessly as the cock raping your throat pours thick wads of", false);
					//(High Corruption)
					...
					//(If the player has a cock)
					if(player.lowerBody.cockSpot.count() > 0) outputText("  Your " + multiCockDescriptLight() + " shoots cum across the ground and into the waiting mouths of the imps licking your crotch.", false);
					outputText("\n\n", false);
					
					outputText("Another imp-cock spasms in your throat as its owner rams deep into you. He floods your already swollen stomach with inhuman amounts of cum. Again you feel yourself about to black out as the demon pumps jism into you. He pulls out and again you orgasm as you wheeze for air. Another imp forces his cock down your throat as you moan and gasp. Your body shakes in pleasure on the big imp's " + Appearance.cockNoun(CockType.DOG) + ".  Tightening his grip on your " + hipDescript() + " the master imp howls and slams his shaft into your ", false);
					//(If the player has a vagina)
					if(player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript(0), false);
					//(If the player doesn't have a vagina)
					else outputText(assholeDescript(), false);
					...
					//(Low Corruption)
					if(player.stats.cor < 50) outputText("You howl around the imp-cock stretching your throat. The bloated knot opens your hole far beyond anything you've endured before. Your violent thrashing throws the imps off your back and you buck uselessly, thrashing as the swollen " + Appearance.cockNoun(CockType.DOG) + " plunges deeper into you.", false);
					//(High corruption)
					else outputText("The master imp's bloated knot stretches your entrance and plunges into your hole with a loud <i>pop</i>. Another orgasm hits you as the " + Appearance.cockNoun(CockType.DOG) + " rams even deeper into you. You howl around the imp-cock stretching your throat, bucking as your orgasm shakes you. Your violent thrashing throws the imps off your back and slams your hips against the big imp, pushing him further into your hole.", false);
					outputText("  The big imp howls again as he cums, each wave of steaming demon-cum stretching his knot and shaft even more. His cum-pumping " + Appearance.cockNoun(CockType.DOG) + " is bottomed out deep in your ", false);
					//(If the player has a vagina)
					...
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText("The imp riding your " + biggestBreastSizeDescript() + " cums, his load lost in the flood of jism dripping off your abused fuck-udders. ", false);
					outputText("Your master isn't done with you yet, churning his " + Appearance.cockNoun(CockType.DOG) + " knot in your ", false);
					//(If the player has a vagina)
					if(player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript(0), false);
					//(If the player doesn't have a vagina)
					else outputText(assholeDescript(), false);
					...
					//(If the player has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) outputText("your bruised and throbbing tits pressed against the rough wood, ", false);
					...
					//(Low Corruption)
					if(player.stats.cor < 50) outputText("How long can you last in this corrupted land, when your body can be so horribly twisted by the sick pleasures of its denizens?", false);
					//(High corruption)
			...
			//NOT CENTAUR
				...
				if(rand(2) == 0 && (player.lowerBody.cockSpot.count() == 0 || player.gender == 3)) {
					...
					//(If the player has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) outputText("Imps surround your chest, slapping their " + monster.cockDescriptShort(0) + "s on your " + player.allBreastsDescript() + " and rubbing their slippery pre-cum into your " + nippleDescript(0) + ".  ", false);
					...
					//(If the player has a cock)
					if(player.lowerBody.cockSpot.count() > 0) outputText("An imp rubs his hand across his cock-head, smearing it with his pre-cum. He rubs his hand over your " + multiCockDescriptLight() + ", making your cock-skin tingle as his fluid soaks into you.", false);
					outputText("\n\n", false);
					...
					//(Low corruption)
					if(player.stats.cor < 50) outputText("You renew your struggles, trying to break free of your captors. They only laugh and bear down harder on you.  ", false);
					//(High corruption)
					...
					outputText("Four feet tall and broader and healthier looking than any imp you've seen before, with a face as much bull as imp, this new imp has mottled grey skin, broad purple demon wings, two curving bull-horns on his head, and a " + Appearance.cockNoun(CockType.HORSE) + " big enough to choke a minotaur. The mushroom-like head of it bobs just below his mouth, and his snake-tongue darts out to flick a bit of pre-cum off the head and onto your groin. You shudder as the hot fluid stings the sensitive skin of your " + vaginaDescript(0), false);
					//(If the player has a dick)
					if(player.lowerBody.cockSpot.count() > 0) outputText(" and " + multiCockDescriptLight(), false);
					...
					//(Low corruption)
					if(player.stats.cor < 33) outputText("You yelp and buck your hips to escape the heat.  ", false);
					...
					if(player.stats.cor < 50) outputText("You scream for help", false);
					//(High corruption)
					if(player.stats.cor >= 50) outputText("You moan with lust", false);
					...
					//(Low corruption)
					if(player.stats.cor < 50) outputText("in helpless terror as the bulge inches up from the base of your groin towards your stomach. You let out a shuddering moan of pain as inch after inch of monstrous " + Appearance.cockNoun(CockType.HORSE) + " stretches your belly", false);
					//(High corruption)
					else outputText("panting in lust as the monstrous " + Appearance.cockNoun(CockType.HORSE) + " pushes your flesh aside to make room for itself", false);
					outputText(". ", false);
					//(This is a good place for the virginity-loss message, if needed)
					...
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) outputText("your " + player.allBreastsDescript() + " roll on your chest as you squirm", false);
					//(If the character doesn't have breasts)
					else outputText("your eyes roll back as you squirm", false);
					outputText(".  ", false);
					//(If the character has a cock)
					if(player.lowerBody.cockSpot.count() > 0) outputText("Your " + multiCockDescriptLight() + " slaps against your distended belly as you shake.", false);
					outputText("\n\n", false);
					outputText("Finally the big imp pulls back his " + Appearance.cockNoun(CockType.HORSE) + ", each ridge pulling on your pussy flesh as he slides out. An imp reaches out and slaps the bulge as it withdraws, making you yelp and buck.  ", false);
					//(If the character has a cock)
					if(player.lowerBody.cockSpot.count() > 0) outputText("Your " + multiCockDescriptLight() + " bounces as the bulge passes under it.  ", false);
					outputText("You moan as the mushroom-head reaches the entrance of your " + vaginaDescript(0) + ", your stretched pussy-flesh slowly returning to normal. The master imp pushes forward again, reclaiming your pussy for his monstrous cock. ", false);
					//(Low corruption)
					if(player.stats.cor < 50) outputText("You try to pull your hips back, fighting to break free as the bulge of his cock-head works its way high up into your belly. You're held down by too many imps. You can only writhe around the hot shaft stretching out your " + vaginaDescript(0) + ". Your head is held steady by two imps, you can't even look away as their master rapes you. The big imp grunts as his cock-head pops past your cervix, and you moan and shake in pain.", false);
					//(High corruption)
					else outputText("You moan in ecstasy as the hot " + Appearance.cockNoun(CockType.HORSE) + " pushes deep into your " + vaginaDescript(0) + ", turning every inch of your pussy into a pleasure-sheath for the big imp. You know you're nothing but a fuck-toy for this corrupt creature, just a wet pussy for him to fill with cum, and the thought almost makes you orgasm as he forces his huge cock-head past your cervix.", false);
					outputText("Finally the corrupt cock bottoms out against your womb. The imp pulls back again, and starts to fuck you slowly.\n\n", false);
					...
					//(Low corruption)
					if(player.stats.cor < 50) outputText("The pain as this huge cock stretches you is overwhelming, but every thrust rubs more corrupted pre-cum into your pussy walls. You start to pant as the imp rapes you, using your body for his own pleasure. Your nipples swell as the heat of his pre-cum soaks through your body. The huge shaft forces your " + clitDescript() + " out, and the steaming fluids splashing on it make it tingle almost painfully. Your whimpers and moans of pain start to take on a different tone, and the master imp starts to fuck you faster.", false);
					//(High corruption)
					else outputText("Pain and pleasure blend into one as the huge " + Appearance.cockNoun(CockType.HORSE) + " stretches you, rubbing pre-cum into steaming pussy. You moan as the big imp fucks you, turning you into a mindless fuck-puppet. Your " + clitDescript() + " swells painfully as hot juices splash over it. Your " + nippleDescript(0) + " tingle almost painfully as the heat of his pre-cum spreads through your body.", false);
					outputText("\n\n", false);
					...
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) outputText("Your " + player.allBreastsDescript() + " bounce and jiggle back and forth as the master imp roughly fucks you.  ", false);
					//(If the character has a dick)
					...
					//(If the character has a dick)
					if(player.lowerBody.cockSpot.count() > 0) outputText("He draws his tongue back and wraps it around your " + cockDescript(0) + ", sliding its length along your shaft and flicking his tongue over your cock-head.", false);
					...
					outputText("  You gasp in time to the big imp's thrusts, whimpering when his cock or tongue hit a sensitive point.  ", false);
					//(Low Corruption)
					if(player.stats.cor < 50) outputText("You're being raped by a demon, forced to take an inhuman cock, and you're about to cum hard. This corrupted land has left its mark on you.", false);
					//(High corruption)
					else outputText("This corrupted land has left its mark on you. You could never have taken a cock this big before you arrived here.", false);
					outputText("  You moan as you rise towards your orgasm.\n\n", false);
					...
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) outputText(" and " + player.allBreastsDescript(), false);
					...
					//(If the character has a cock) 
					if(player.lowerBody.cockSpot.count() > 0) outputText(", and cum-dripping " + cockDescript(0), false);
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) outputText(", and bouncing " + player.allBreastsDescript(), false);
					...
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3 && player.biggestLactation() > 1) outputText("milk spurts from your " + nippleDescript(0) + " and ", false);
					...
					if(player.statusAffects.has("ImpGangBang")) {
						//(Subsequent encounters - Low Corruption)
						if(player.stats.cor < 50) outputText("You can't tell if this is the same " + monster.short + " as last time or not - all imps look alike to you.  The " + monster.capitalA  + " surges forward, grabbing at your " + player.legs() + " and arms and running their hands over your body. You struggle, but there are just too many to fight. The result is the same as last time...\n\n", false);
						//(Subsequent encounters - High Corruption)
						else outputText("It's about time they showed up. It's not like there's a lot to do in these rocks, and you were getting bored. You grab an imp dick in either hand and spread your legs as other imps grope your thighs...\n\n", false);
					}
					outputText("The " + monster.capitalA + " swarms over you, dragging you to the ground as ", false);
					//(Low Corruption)
					if(player.stats.cor < 50) outputText("you punch and kick wildly, determined not to let them take you", false);
					//(High Corruption)
					...
					//(If the player has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) outputText("An imp mounts your chest and slaps his " + monster.cockDescriptShort(0) + " between your " + player.allBreastsDescript() + ", squeezing them tight over his cock as he rubs back and forth.  ", false);
					//(If the player has a SINGLE cock)
					if(player.lowerBody.cockSpot.count() == 1) outputText("Your " + cockDescript(0) + " is seized by an imp, who licks the tip with his inhumanly nimble tongue while he strokes the shaft.  ", false);
					//(If the player has a MULTI cock)
					if(player.lowerBody.cockSpot.count() > 1) outputText("Two imps seize your " + multiCockDescriptLight() + ", licking the tips with their inhumanly nimble tongues while they stroke the shafts.  ", false);
					//(Low Corruption)
					if(player.stats.cor < 50) outputText("You fight to free your arms and shake the imp off your chest while tiny hands slide over your face. They tug at your lips and try to pry your jaws open", false);
					//High Corruption)
					...
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) {
						...
						//(Low Corruption)
						if(player.stats.cor < 50) outputText("You can't escape the tongue lapping and pulling at your " + nippleDescript(0) + ". You shake your head to deny the pleasure, but your breathing comes faster and faster as lust invades your body.", false);
						//(High Corruption)
						...
						if(player.upperBody.chest.hasFuckableNipples()) outputText("With the imp's taint seeping into your " + nippleDescript(0) + ", each one's cunt-like shape begins swelling. The fuckable orifices engorge into larger and fatter looking labia, becoming fuller cunts each with an engorged clitoral nub the size of a golf ball. Their color deepens as the skin of your nipple-cunts becomes tighter and smoother.  The imp giggles and continues nibbling the newly swollen sensitive flesh, injecting further doses of venom.", false);
						outputText("\n\n", false); 
						//Grow tits!
						player.growTits(2, player.upperBody.chest.count(), false, 1);
					...
					outputText(", leering down at your trapped body. Four feet tall and broader and stronger than any imp in the pack, with a face as much dog as imp, this new imp has grey fur, broad black demon wings, two long demon-horns on his head, and a " + Appearance.cockNoun(CockType.DOG) + " big enough to choke a minotaur. Pre-cum drips from the broad tip of it, dripping down onto your ", false);
					//(If the player has a cock)
					if(player.lowerBody.cockSpot.count() > 0) outputText(multiCockDescriptLight(), false);
					//(If the player doesn't have a cock)
					else outputText(vaginaDescript(0), false);
					outputText(".  ", false);
					outputText("The heat stings your flesh. The imps licking your groin lap up the hot fluid, cooling you with their saliva. The big imp sneers as you whimper, and drags the head of his " + Appearance.cockNoun(CockType.DOG) + " down to your ", false);
					//(If the player has a vagina)
					if(player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript(0), false);
					//(If the player doesn't have a vagina)
					else outputText(assholeDescript(), false);
					...
					outputText(". ", false);
					//(Low Corruption)
					if(player.stats.cor < 50) outputText("You screech in agony as the big imp forces his mutated wolf-cock into your hole, brutally shoving thick inch after inch of painfully hot " + Appearance.cockNoun(CockType.DOG) + " deeper into you than anything should ever go.  ", false);
					//(High Corruption)
					else outputText("The master imp's painfully hot " + Appearance.cockNoun(CockType.DOG) + " stretches your hole wider than it ever should be, and you moan in perverse ecstasy.  ", false); 
					...
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) outputText("The big imp reaches past your tit-rider and grabs one of your painfully distended breasts in each hand, mauling and bouncing the flesh as if weighing them. You gasp in pain as your " + player.allBreastsDescript() + " swell further at his touch.  ", false);
					...
					outputText(". ", false);
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText("The rough fucking shakes your breasts, and the imp sucking your nipples clings tightly to your monstrously swollen " + player.allBreastsDescript() + ". Your " + biggestBreastSizeDescript() + " have grown three cup sizes since the imp pumped his venom into you.  ", false);
					...
					outputText("\n\n", false);
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) outputText("An ache starts deep in the base of your tits and works its way to your sore " + nippleDescript(0) + ". Your already bloated nipples swell as your rider suckles and you gasp as the first rush of milk spills into his mouth. Your rider milks your udders, moving his hands between your " + player.allBreastsDescript() + " and forcing out more milk than he could ever drink. Other imps lick the milk from the shiny skin of your swollen breasts.\n\n", false);
					...
					outputText(" spasms on the steaming pole that impales it. Another imp shoves his cock in your mouth as you scream, throat convulsing around his cock-head.", false);
					//(If the player has a cock)
					if(player.lowerBody.cockSpot.count() > 0) outputText("  Your " + multiCockDescriptLight() + " shoots cum across your belly and into the waiting mouths of the imps licking your crotch.", false);
					outputText("\n\n", false);
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) {
						...
						if(player.upperBody.chest.hasFuckableNipples()) {
							...
							//[IF breastSize <= DD]
							if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 5) outputText("You feel the bulbous head of his cock squeeze further and deeper until it pushes up against your ribs.", false);
					...
					//(If the character has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) outputText("The imp riding your " + biggestBreastSizeDescript() + " finally cums, painting your distended fuck-udders with his massive load.  ", false);
					outputText("Your master isn't done with you yet, churning his " + Appearance.cockNoun(CockType.DOG) + " knot in your ", false);
					//(If the player has a vagina)
					if(player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript(0), false);
					//(If the player doesn't have a vagina)
					...
					//(If the player has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) outputText("and your milk-filled udders are still swollen with imp-venom, ", false);
					outputText("quivering with every orgasmic twitch that passes through you. The skin of your distended belly ", false);
					//(If the player has breasts)
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) outputText("and massive tits ", false);
					...
					//(Low Corruption)
					if(player.stats.cor < 50) outputText("How long can you last in this corrupted land, when your body can be so horribly twisted for the sick pleasures of its denizens?\n\n", false);
					//(High corruption)
					else outputText("Why bother with your silly quest, when you've only scratched the surface of the pleasures this land offers you?\n\n", false);
					player.orgasm();
					dynStats("lib", 2, "cor", 3);
			...
			if(player.lust >= 100) {
				...
				if(player.gender == 2 || player.gender == 3) {
					...
					if(player.pregnancyIncubation > 0 && player.pregnancyIncubation <= 216) {
						...
						if(player.inHeat) outputText("You find yourself hoping you're pregnant as you swiftly lose consciousness.", false);
							...
							if(player.stats.cor > 75) outputText("With an appreciative moan, you bury your fingers in its slimy warmth, hoping you are pregnant with some fiendish offspring, and lose consciousness.", false);
				...
				if(player.gender == 0 || player.gender == 1) {
					...
					if (player.gender == 1 && rand(2) == 0) {
						...
						if(player.lowerBody == LOWER_BODY.CENTAUR) outputText("hocks and knees, your " + cockDescript(0) + " hurting from the massive blood pressure caused by your unbridled lust. He approaches you and stops about two feet in front of you, watching with delight your helpless state", false);
						...
						if(player.lowerBody == LOWER_BODY.CENTAUR) outputText("Dropping down to the ground, your " + cockDescript(0) + " trembles against your body to the rhythm of the imp's thrusts, leaving your underbelly smeared with its own pre-cum.\n\n", false);
						...
						if(player.lowerBody.ballSize >= 5) outputText("Your huge " + ballsDescriptLight() + " swing heavily against you as well, responding to the force of the imp's thrusts, slapping your own ass and driving your " + cockDescript(0) + " even stiffer with lust, the pre-cum pulsing out of your cock in time with the slapping.\n\n", false);
						...
						if(player.lowerBody != LOWER_BODY.CENTAUR) outputText("as you are disrobed", false);
		...
		private function noogaisNippleRape():void {
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) outputText("dripping wet pussy", false);
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 4) outputText("bottoming out halfway on his immense dick.", false);
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 4) outputText("you feel his swollen cock bottom out, your petite breast unable to swallow any more of his throbbing maleness", false);
			//((over D) 
			...
			//[(if male)
			if(player.lowerBody.cockSpot.hasCock()) outputText(" pumping your " + cockDescript(0), false);
			//(if female)
			else if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" fingering your hungry baby tunnel", false);
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				...
				if(player.lowerBody.vaginaSpot.hasVagina()) outputText(", while ", false);
			}
			if(player.lowerBody.vaginaSpot.hasVagina()) {
			...
			//(if corruption > 60)
			if(player.stats.cor > 60) outputText("You thrust your digits into your " + nippleDescript(0) + " once more, scooping out as much imp jizz as you can reach.  You happily drink up the thick goo, savoring the cloying taste before quickly getting dressed and leaving the imp to slumber.", false);
		...
		private function getBlownByAnImpLord():void {
			...
			if(player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockThickness < 7) {
				...
				if(player.lowerBody.balls > 0) outputText("  His hands move down towards your [balls].");
				outputText("\n\n");
				//if(balls > 0)
				if(player.lowerBody.balls > 0) {
		...
		private function loseToImpLord():void {
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				...
				if(player.lowerBody.balls > 0) outputText("Your [balls] clench up against your body, desperate to finally expel their contents.  ");
		...
		private function putBeeEggsInAnImpYouMonster():void {
			...
			//[If cock: 
			else if(player.lowerBody.cockSpot.hasCock()) outputText("the rise of his swollen belly soon presses against [oneCock] and the rhythm of your thrusts strokes his shiny red stomach against your sensitive organ.");
			else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("the imp’s tiny, clawed feet scrabble against you as he flails in pleasure.  By mistake, one slips between the lips of your pussy, small toes wriggling against your inner walls, and you instinctively push down against the small limb, fucking yourself with his foot.");
