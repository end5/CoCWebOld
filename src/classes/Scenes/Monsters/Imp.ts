package classes.Scenes.Monsters
{
	...
	public class Imp extends Monster
	{
		...
		protected lustMagicAttack():void {
			...
			if (player.lust < 30) MainScreen.text("You feel strangely warm.  ");
			if (player.lust >= 30 && player.lust < 60) MainScreen.text("Blood rushes to your groin as a surge of arousal hits you, making your knees weak.  ");
			if (player.lust >= 60) MainScreen.text("Images of yourself fellating and fucking the imp assault your mind, unnaturally arousing you.  ");
			if (player.lowerBody.cockSpot.count() > 0) {
				if (player.lust >= 60)
					MainScreen.text("You feel your " + player.multiCockDescriptLight() + " dribble pre-cum.");
				else if (player.lust >= 30 && player.lowerBody.cockSpot.count() == 1)
					MainScreen.text("Your " + player.cockDescript(0) + " hardens, distracting you further.");
				else if (player.lust >= 30 && player.lowerBody.cockSpot.count() > 1)
					MainScreen.text("Your " + player.multiCockDescriptLight() + " harden uncomfortably.");
