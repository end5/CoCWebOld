export default class ImpLord extends Imp {
	//Special Attack 1
	protected impFire(): void {
		MainScreen.text("The imp mutters something to himself. Before you have time to react the demonic creature's hand is filled with a bright red fire that he hurls at you.  The flames lick at your body leaving a painful burn on you torso, as well as an arousing heat in your groin.");
		//[-HP // +Lust(minor)]
	...
	protected impLordHeavyEncounter(): void {
		let damage: number = int((str + weaponAttack + 20) - rand(player.stats.tou) - player.armorDef);
		MainScreen.text("The demonic creature slashes a clawed hand towards your stomach,");
		if (combatMiss() || combatEvade() || combatFlexibility() || combatMisdirect()) MainScreen.text(" but you handily avoid it.");
		else if (damage <= 0) MainScreen.text(" but the attack proves ineffectual.");
		else {
			MainScreen.text("leaving a large gash. The attack leaves you slightly stunned, but you recover. ");
	...
	protected impLordLustAttack(): void {
		MainScreen.text("Lowering his loincloth the imp reveals his inhumanly thick shaft.  He smirks and licks his lips as he gives his cock a squeeze, milking a few beads of clear pre from the tip.  You shake your head and try to ignore your growing need.");
		//[+Lust]
	...
	protected impLordLustAttack2(): void {
		MainScreen.text("Reaching into his satchel the devilish creature pulls out a leather riding crop.  He quickly rushes forward, but somehow manages to get behind you.  Before you can react the imp lashes out, striking your [butt] twice with the riding crop.  The strikes leave a slight burning feeling, as well as a strange sense of arousal.");
