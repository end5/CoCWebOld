		...
		public function goblinRapesPlayer():void
		{
			...
			if (player.perks.has("BimboBrains") || player.perks.has("FutaFaculties")) {
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) {
					...
					//--[If nipplecunts]
					if (player.upperBody.chest.hasFuckableNipples()) outputText(", fingering your wet nipplecunts and pinching the now-puffy areolas tightly, sending uncontrollable waves of pleasure from your chest to your groin.", false);
					//-[If not]
			...
			if (player.lowerBody.cockSpot.count() > 0) {
				...
				if (player.lowerBody.cockSpot.list[0].cockLength <= 7) outputText("as your bulge begins to press between her soft ass-cheeks", false);
				else if (player.lowerBody.cockSpot.list[0].cockLength <= 14) outputText("as your bulge grows upwards between her ass-cheeks and lays against the small of her back", false);
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				...
				if (player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.NORMAL) outputText("The lips of your sex engorge, becoming almost as puffy as the goblin's.  ", false);
				else if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) outputText("Feminine lubricant soaks into the back of your " + player.armorName + ".  ", false);
			...
			if (player.upperBody.chest.hasFuckableNipples()) outputText("slipping her thumbs inside the tender cunts and pinching against them tightly", false);
			...
			if (player.totalCocks() > 0 && (!player.lowerBody.vaginaSpot.hasVagina() || rand(2) == 0)) {
				...
				if (player.cockArea(0) > monster.vaginalCapacity()) {
					...
					if (player.stats.cor > 50) outputText("prolong your pleasure", false);
					...
					if (player.lowerBody.vaginaSpot.hasVagina()) outputText("caressing your " + clitDescript(), false);
				...
				//[DICK FITS]
					...
					if (player.lowerBody.cockSpot.list[0].cockType == CockType.HORSE) outputText("flare", false);
					...
					if (player.stats.cor < 50) outputText("You shake your head, trying to get away from the sweet-smelling drug, but she tilts it up, pouring most of it into your throat.", false);
					...
					if (player.lowerBody.balls > 0) outputText("cup your " + ballsDescriptLight() + " in one hand, squeezing gently as the drug begins to affect them", false);
			...
			//[FEMALEZ]
				...
				if (player.stats.cor < 33) outputText("You shake your head violently, clearly indicating 'No', but when you open your mouth to vocalize your complaint, your lips are swiftly plugged with bulging sex-toy.", false);
				else if (player.stats.cor < 66) outputText("You open your mouth to stammer out your concerns, but she plugs the hole with the floppy artificial dong, turning your speech into surprised muffles.", false);
				...
				if (player.lowerBody.vaginaSpot.list[0].clitLength >= 7) outputText("Of course, the sheer size of your clit makes it difficult for the goblin to handle in the normal way - it keeps slipping between up her breasts.  The tiny green tart's eyes light up with a devilish idea.  She pulls out a vial of pink slime and dumps it over her breasts, pushing them around your clit and smothering them in slippery flesh.   You cry out in delight, overwhelmed by the feelings radiating from your over-sized pleasure-buzzer as it is mercilessly worked by the tiny woman.\n\n", false);
				...
				if (player.vaginas[0].vaginalLooseness <= VAGINA_LOOSENESS.GAPING) outputText("gape apart momentarily", false);
...
//[WIN RAEPZ]
		...
		private function gobboButtSecks():void
		{
			...
			if (player.lowerBody.balls > 0) outputText(sackDescript(), false);
...
//[FEMSAUCE]
		private function gobboGetsRapedFem():void
		{
			...
			//Goblin victory rape, female naga:
				...
				//[Has fangs:
				if (player.faceType == FACE.SNAKE_FANGS) outputText("You bare your fangs at her and the curiosity disappears. She turns to run, but your tail is faster than she is.", false);
				//[No fangs: 
				...
				if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS.WET) outputText("grows puffy and moist", false);
				else if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) outputText("drips with feminine moisture", false);
				else if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLAVERING) outputText("slowly begins to soak your thighs", false);
				...
				if (player.vaginas[0].vaginalWetness > VAGINA_WETNESS.DROOLING) outputText("splattering her with your fluids", false);
				else if (player.vaginas[0].vaginalWetness > VAGINA_WETNESS.SLICK) outputText("coating her face with the proof of your pleasure", false);
				else if (player.vaginas[0].vaginalWetness > VAGINA_WETNESS.WET) outputText("leaving her sticky with your nether-juices", false);
				...
				if (player.stats.cor > 50) outputText("smirk", false);
				else outputText("sigh", false);
				outputText(" and saunter off, feeling ", false);
				if (player.stats.cor < 50) outputText("a bit guilty about overdoing it.", false);
...
//Corrupt too big fuck
		private function rapeAGoblinCorruptTooBig():void
		{
			...
			if (player.lowerBody.cockSpot.count() == 1) outputText(" is", false);
			...
			if (player.lowerBody.cockSpot.count() > 1) outputText("one of your " + multiCockDescriptLight() + " comes to rest on top of her and the fact that it's ", false);
			else outputText("your " + cockDescript(x) + " comes to rest on top of her and the fact that it's ", false);
			if (player.lowerBody.cockSpot.list[0].cockLength < 20) outputText("about as long as her entire torso", false);
			...
			if (player.lowerBody.cockSpot.list[0].cockLength < 20) outputText("can feel the massive bulge in her midsection", false);
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				...
				if (player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.SLICK) outputText("juices ", false);
				else if (player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.DROOLING) outputText("floods ", false);
				else outputText("explodes ", false);
				if (player.lowerBody.cockSpot.count() > 1) outputText("and the rest of your " + multiCockDescriptLight() + " drench her, ", false);
			}
			else if (player.lowerBody.cockSpot.count() > 1) {
				outputText("while your other " + Appearance.cockNoun(CockType.HUMAN), false);
				if (player.lowerBody.cockSpot.count() > 2) outputText(" drench her, ", false);
				else outputText(" drenches her, ", false);
			}
			outputText("the bitch is too tight! Like a natural cock-ring! ", false);
			if (player.lowerBody.balls >= 2) outputText("Your " + ballsDescriptLight() + " are trying but, h", false);
			...
			if (player.lowerBody.cockSpot.count() > 2) outputText("Your remaining " + multiCockDescriptLight() + " blast geysers into the air, coating you both in your spunk. ", false);
			if (player.lowerBody.cockSpot.count() == 2) outputText("Your remaining penis blasts geysers into the air, coating you both with spunk. ", false);
...
//(TOO BIG – pin the bitch to the ground with your cock, coat it in her potions, and make her lick it clean, then blow your load in her mouth, possible cum inflation.)
		private function manRapesGoblinTooBig():void
		{
			...
			if (player.stats.cor < 50) outputText("shrug and guess you may as well try since she's ready", false);
...
//[DUDEGASM]
		private function gobboGetsRapedMaleFits():void
		{
			...
			if (player.cockArea(x) > monster.vaginalCapacity() * .8) {
				...
				if (player.lowerBody.cockSpot.list[0].cockType == CockType.DEMON) outputText("rubbing painfully against your demonic nubs", false);
				else if (player.hasKnot(0)) outputText("stretching painfully around your knot", false);
				else if (player.lowerBody.cockSpot.list[0].cockType == CockType.HORSE || player.lowerBody.cockSpot.list[0].cockType.Index > 3) outputText("feeling painfully tight around you", false);
				...
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText(player.allBreastsDescript(), false);
				...
				if (player.lowerBody.balls > 0) outputText("Your " + ballsDescriptLight() + " tighten, releasing the seed of your orgasm.  ", false);
				...
				if (player.stats.cor > 50) outputText("Chuckling", false);
			...
			//(FITS – Get ridden)
				...
				if (player.lowerBody.cockSpot.list[0].cockType == CockType.DEMON) outputText("rubbing perfectly against your demonic nubs", false);
				else if (player.hasKnot(0)) outputText("stretching tightly around your knot", false);
				else if (player.lowerBody.cockSpot.list[0].cockType == CockType.HORSE || player.lowerBody.cockSpot.list[0].cockType.Index > 3) outputText("feeling absolutely perfect around you", false);
...
//Spider goblin condom
		private function goblinCondomed():void
		{
			...
			if (player.lowerBody.balls > 0) outputText("your throbbing sack", false);
...
//REQUIRES: AT LEAST ONE DICK AND A COPY OF ATLAS SHRUGGED - MUST NOT BE MONSTROUSLY HUGE
		private function gatsGoblinBoners():void
		{
			...
			if (player.lowerBody.cockSpot.count() == 1) outputText("your dick inside of the panting whore, pushing her forwards violently as you enter her tight snatch", false);
			else if (player.lowerBody.cockSpot.count() == 2) outputText("both of your dicks inside of the panting whore, pushing her forwards violently as you enter her tight snatch and asshole", false);
			else {
				...
				if (player.lowerBody.cockSpot.count() >= 4) outputText("s", false);
		...
		private function laySomeDriderEggsInGobboTwat():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("harden");
			else if (player.lowerBody.vaginaSpot.hasVagina()) outputText("moisten");
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText("and y");
			}
			else if (player.lowerBody.vaginaSpot.hasVagina()) outputText("  Y");
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("our [vagina] clenching in time with hers as your copious fluids drench your spider half.");
