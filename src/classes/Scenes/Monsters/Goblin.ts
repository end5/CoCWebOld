package classes.Scenes.Monsters
{
	...
	public class Goblin extends Monster
	{
		protected function goblinDrugAttack():void {
			...
			if(color != "blue") {
				if(short == "Tamani's daughters") Render.text("Tamani uncorks a glass bottle full of " + color + " fluid and swings her arm, flinging a wave of fluid at you.", false);
				else Render.text(capitalA + short + " uncorks a glass bottle full of " + color + " fluid and swings her arm, flinging a wave of fluid at you.", false);
			}
			//Drink blue pots
			else {
				if(short == "Tamani's daughters") {
					Render.text("Tamani pulls out a blue vial and uncaps it, then douses the mob with the contents.", false);
					if(HPRatio() < 1) {
						Render.text("  Though less effective than ingesting it, the potion looks to have helped the goblins recover from their wounds!\n", false);
						addHP(80);
					}
		...
		protected function goblinTeaseAttack():void {
			...
			if(det == 2) Render.text(capitalA + short + " bends over, putting on a show and jiggling her heart-shaped ass at you.  She looks over her shoulder and sucks on her finger, batting her eyelashes.", false);
			game.dynStats("lus", rand(player.stats.lib/10)+8);
			Render.text("  The display distracts you long enough to prevent you from taking advantage of her awkward pose, leaving you more than a little flushed.\n\n", false);
			combatRoundOver();
		}
		...
		{
			if (player.gender == 0) {
				Render.text("You collapse in front of the goblin, too wounded to fight.  She giggles and takes out a tube of lipstick smearing it whorishly on your face.  You pass into unconsciousness immediately.  It must have been drugged.", false);
