	...
	public class Sheila extends Monster
	{
		...
		private sheilaFrogPunch():void {
			...
			//Miss:
				...
				Render.text("Sheila bounces up to you and crouches low, curling up her body like a watchspring.  The girl uncoils with fist raised, but you lean away from the uppercut, catching a faceful of her breasts instead!  Sheila squeals and pushes away from you");
				//[(libido>40)
				if(player.stats.lib > 40) {
					Render.text(", though not before you have a chance to stick your tongue in her cleavage!");
				}
				else Render.text(".");
				Render.text("  Blushing pinkly, she crosses her arms over her chest as she resumes her distance.");
				//(+med-small lib-based lust damage to PC)
				kGAMECLASS.dynStats("lus", 10+rand(player.stats.sens/10));
			}
			//Hit:
			else {
				Render.text("Sheila bounces up to you and crouches low, curling up her body like a watchspring.  The girl uncoils just as quickly, launching herself at your face with a fist raised in front of her.  She lands a staggering crack on your jaw which knocks your head back and blurs your vision!");
				...
				if(rand(2) == 0 && !player.perks.has("Resolute")) {
					...
					Render.text("  <b>You are stunned!</b>");
				}
		...
		private suspiciousGlint():void {
			...
			//Miss:
				...
				Render.text("Sheila's eyes glint suspiciously as she proclaims her affection for you and begs you to look into them, but you keep your head down and focus on her feet.  You can feel her stare boring holes into you, but eventually she abandons the attempt.");
			}
			//Hit:
			else {
				Render.text("Sheila's eyes glint suspiciously, and you feel your mind slowing down and your body heating up as you meet her lascivious gaze.  Too late you look away, but the damage is done; her fantasies of ");
				if(!player.lowerBody.cockSpot.hasCock()) Render.text("burying her drooling tail inside you until it squirts");
				else Render.text("riding your dick to the hilt");
				Render.text(" run rampant inside your head and crowd out everything else.  \"<i>Did you see it, [name]?  My love for you?</i>\" Sheila asks, smiling.  God, did you ever!  You can hardly focus on anything!");
		...
		private tittyMonsterAttack():void {
			...
			if(game.sheilaScene.sheilaCorruption() < 20) Render.text("  But with nothing there for her to work with, it's a lot like being teased by a dressmaker's mannequin.");
			//(else if sheila corruption < 150; 'hit')
			else if(game.sheilaScene.sheilaCorruption() < 150) {
				Render.text("  As her hands run over the soft-looking mammaries, kneading and squeezing them, teasing the nipples relentlessly until she lets out a cute little moan, you feel the blood rush to your face.  \"<i>Enjoying this, are you?</i>\" she calls sweetly.  \"<i>Why don't you stop being contrary and come play with them too?</i>\"");
				//med lib-based lust damage if 20 < sheila corruption < 150
				game.dynStats("lus", 25+player.stats.lib/10);
			}
			//(else; 'miss')
			else {
				Render.text("  She has trouble even budging tits so comically mismatched to her slender frame; her hands just sink into the voluminous flesh when she tries to squeeze them together, but the demon doesn't manage to move mountains.  It's like watching someone try to push half-inflated swimming equipment around.  You actually laugh a little as she gives up, rubbing her lower back with a gripe.");
			}
			combatRoundOver();
		}
		...
		private splashAttackLookOutShellEvolveIntoGyrados():void {
			...
			if(!combatMiss() && !combatEvade() && !combatMisdirect() && !combatFlexibility()) {
				Render.text("It lands on target, and you're forced to close your eyes lest it get in them!");
				player.statusAffects.add(new StatusAffect("Blind",1,0,0,0)));
				player.statusAffects.add(new StatusAffect("SheilaOil",0,0,0,0)));
			}
			else {
				Render.text("You easily lean away from the path of her tainted fluids, and she sighs.  \"<i>You're no fun, mate.</i>\"");
			}
			combatRoundOver();
		}
		//4: Sit 'n Pout
		...
		private lickEmAndStickEm():void {
			...
			else Render.text("you take Sheila from behind by plunging her spaded tail into your [vagina] as she lies face-down on the ground with her ass pushed in the air");
			Render.text(".");
			//results, no new pg
			//[(int check passed)
			//Miss:
