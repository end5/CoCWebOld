package classes.Scenes.NPCs
{
	...
	public class Sheila extends Monster
	{
		...
		private sheilaFrogPunch():void {
			...
			//Miss:
			else if(combatMiss() || combatFlexibility() || combatEvade() || combatMisdirect() || statusAffects.has("Blind")) {
				MainScreen.text("Sheila bounces up to you and crouches low, curling up her body like a watchspring.  The girl uncoils with fist raised, but you lean away from the uppercut, catching a faceful of her breasts instead!  Sheila squeals and pushes away from you");
				//[(libido>40)
				if(player.stats.lib > 40) {
					MainScreen.text(", though not before you have a chance to stick your tongue in her cleavage!");
				}
				else MainScreen.text(".");
				MainScreen.text("  Blushing pinkly, she crosses her arms over her chest as she resumes her distance.");
				//(+med-small lib-based lust damage to PC)
				kGAMECLASS.dynStats("lus", 10+rand(player.stats.sens/10));
			}
			//Hit:
			else {
				MainScreen.text("Sheila bounces up to you and crouches low, curling up her body like a watchspring.  The girl uncoils just as quickly, launching herself at your face with a fist raised in front of her.  She lands a staggering crack on your jaw which knocks your head back and blurs your vision!");
				...
				if(rand(2) == 0 && !player.perks.has("Resolute")) {
					player.statusAffects.add(new StatusAffect("Stunned",1,0,0,0)));
					MainScreen.text("  <b>You are stunned!</b>");
				}
		...
		private suspiciousGlint():void {
			if(statusAffects.has("Blind") && rand(2) == 0) {
				MainScreen.text("Sheila's blind eyes glint suspiciously as she focuses her power, trying to send her fantasy to anything caught in their stare.  It seems to work - the rock next to you vibrates a little.");
			}
			//Miss:
			else if(player.stats.int / 15 + rand(20) + 1 > 16) {
				MainScreen.text("Sheila's eyes glint suspiciously as she proclaims her affection for you and begs you to look into them, but you keep your head down and focus on her feet.  You can feel her stare boring holes into you, but eventually she abandons the attempt.");
			}
			//Hit:
			else {
				MainScreen.text("Sheila's eyes glint suspiciously, and you feel your mind slowing down and your body heating up as you meet her lascivious gaze.  Too late you look away, but the damage is done; her fantasies of ");
				if(!player.lowerBody.cockSpot.hasCock()) MainScreen.text("burying her drooling tail inside you until it squirts");
				else MainScreen.text("riding your dick to the hilt");
				MainScreen.text(" run rampant inside your head and crowd out everything else.  \"<i>Did you see it, [name]?  My love for you?</i>\" Sheila asks, smiling.  God, did you ever!  You can hardly focus on anything!");
				...
				if(findStatusAffect(StatusAffects.TwuWuv) < 0) {
					statusAffects.add(new StatusAffect("TwuWuv",0,0,0,0)));
		...
		private tittyMonsterAttack():void {
			MainScreen.text("Sheila giggles and strokes her " + game.sheilaScene.sheilaCup() + " breasts, trying to entice you.");
			//results, no new pg
			//[(sheila corruption < 20; 'miss')
			if(game.sheilaScene.sheilaCorruption() < 20) MainScreen.text("  But with nothing there for her to work with, it's a lot like being teased by a dressmaker's mannequin.");
			//(else if sheila corruption < 150; 'hit')
			else if(game.sheilaScene.sheilaCorruption() < 150) {
				MainScreen.text("  As her hands run over the soft-looking mammaries, kneading and squeezing them, teasing the nipples relentlessly until she lets out a cute little moan, you feel the blood rush to your face.  \"<i>Enjoying this, are you?</i>\" she calls sweetly.  \"<i>Why don't you stop being contrary and come play with them too?</i>\"");
				//med lib-based lust damage if 20 < sheila corruption < 150
				game.dynStats("lus", 25+player.stats.lib/10);
			}
			//(else; 'miss')
			else {
				MainScreen.text("  She has trouble even budging tits so comically mismatched to her slender frame; her hands just sink into the voluminous flesh when she tries to squeeze them together, but the demon doesn't manage to move mountains.  It's like watching someone try to push half-inflated swimming equipment around.  You actually laugh a little as she gives up, rubbing her lower back with a gripe.");
			}
			combatRoundOver();
		}
		...
		private splashAttackLookOutShellEvolveIntoGyrados():void {
			MainScreen.text("Sheila waits patiently, staring at you and stroking her dark, spaded tail with its opposite.  A line of the always-oozing oil falls from the slit, pooling in the smooth brown coil; she unwinds it rapidly, flinging the liquid at your face playfully.  ");
			//results, no new PG
			//Hit:
			if(!combatMiss() && !combatEvade() && !combatMisdirect() && !combatFlexibility()) {
				MainScreen.text("It lands on target, and you're forced to close your eyes lest it get in them!");
				player.statusAffects.add(new StatusAffect("Blind",1,0,0,0)));
				player.statusAffects.add(new StatusAffect("SheilaOil",0,0,0,0)));
			}
			else {
				MainScreen.text("You easily lean away from the path of her tainted fluids, and she sighs.  \"<i>You're no fun, mate.</i>\"");
			}
			combatRoundOver();
		}
		//4: Sit 'n Pout
		...
		private lickEmAndStickEm():void {
			MainScreen.text("Sheila's voice gets closer, becoming disarmingly apologetic as you scrub furiously at your face in darkness.  \"<i>Oh, my.  I didn't mean to get that in your eyes... let me help clean you up, love.</i>\"  Your face is gently gripped between her hands and pulled down, then the demon begins passing her tongue over you affectionately, wiping the fluid away with long, ticklish licks as you wait for the other shoe to fall.");
			MainScreen.text("\"<i>All better,</i>\" Sheila announces.  With her thumb, she gingerly pushes one eyelid up before you can pull away, proving her claim - and causing you to look right into her own glowing, purple iris.  A fantasy invades your mind, one where ");
			if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("[oneCock] fucks Sheila to the base while her tail snakes around and penetrates your [vagOrAss]");
			else MainScreen.text("you take Sheila from behind by plunging her spaded tail into your [vagina] as she lies face-down on the ground with her ass pushed in the air");
			MainScreen.text(".");
			//results, no new pg
			//[(int check passed)
			//Miss:
			if(player.stats.int / 15 + rand(20) + 1 > 16) {
		...
		private pressurePointsAttack():void {
			MainScreen.text("For a moment, all goes quiet, save for a soft rustle.\n\n");
			//results, no new pg
			//[(sheila corruption < 100; hit, 'light damage')]
			if(game.sheilaScene.sheilaCorruption() < 100) {
				MainScreen.text("The silence is broken with a giggle as the demon catches you in an embrace, pressing her " + game.sheilaScene.sheilaCup() + " breasts into you.  You shiver as she drags the perky nipples over your " + player.skinFurScales() + ", but push her away.");
				game.dynStats("lus", 15+player.stats.sens/20 + player.stats.lib/20);
			}
				...
				MainScreen.text("You're a bit unnerved, but soon realize that you can tell where Sheila is by listening for the telltale sounds of her colossal breasts scraping the ground as she draws closer to you.  With this in mind, you continue to face your opponent and back away as you wipe your eyes.");
		...
		private demonSheilaAI():void {
			...
			if(statusAffects.get("Counter").value1 >= 5) {
				sitAndPout();
				return;
			}
			let choices:Array = [];
		...
		{
			if(game.flags[FlagEnum.SHEILA_DEMON] == 1) game.sheilaScene.beatUpDemonSheila();
			else game.sheilaScene.sheilaGotWhomped();
		}

		public won(hpVictory:boolean, pcCameWorms:boolean):void
		{
			if(game.flags[FlagEnum.SHEILA_DEMON] == 1) game.sheilaScene.loseToSheila();
		...
		public Sheila()
		{
			...
			// this.plural = false;
			this.createVagina(game.flags[FlagEnum.SHEILA_XP] <= 3 && !sheilaDemon, VAGINA_WETNESS.SLICK, VAGINA_LOOSENESS.NORMAL);
			this.statusAffects.add(new StatusAffect("BonusVCapacity", 30, 0, 0, 0)));
			this.createBreastRow(game.flags[FlagEnum.SHEILA_CORRUPTION]/10);
			...
			this.level = 14;
			this.gems = rand(5) + 5;
			if (game.flags[FlagEnum.SHEILA_DEMON] == 0){
