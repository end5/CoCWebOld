...
// TIMES_EMBER_LUSTY_FUCKED: number = 824;
		...
		public function timeChange():boolean
		{
			...
			if (followerEmber() && player.findStatusAffect(StatusAffects.EmberNapping) < 0) {
				...
				if (flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] == 1 && !player.perks.has("MinotaurCumAddict") && flags[FlagEnum.EMBER_CURRENTLY_FREAKING_ABOUT_MINOCUM] == 0) {
					minotaurJizzFreakout();
					needNext = true;
				}
				//Ember is freaking out about addiction, but PC no longer addicted!
				else if (flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] == 0 && flags[FlagEnum.EMBER_CURRENTLY_FREAKING_ABOUT_MINOCUM] == 1) {
				...
				if (player.lust >= 100 && player.gender > 0) {
					flags[FlagEnum.EMBER_LUST_BITCHING_COUNTER]++;
					if (flags[FlagEnum.EMBER_LUST_BITCHING_COUNTER] >= 10) {
						emberBitchesAtYouAboutLustiness();
						needNext = true;
					}
				}
				//Reset lust counter if not max lust'ed
				else flags[FlagEnum.EMBER_LUST_BITCHING_COUNTER] = 0;
			}
			if (model.time.hours > 23) {
				if (!player.isPregnant()) flags[FlagEnum.EMBER_BITCHES_ABOUT_PREGNANT_PC] = 0;
				flags[FlagEnum.DRANK_EMBER_BLOOD_TODAY] = 0;
			}
			return needNext;
		}
		...
		public function timeChangeLarge():boolean {
			if (player.findStatusAffect(StatusAffects.EmberNapping) < 0 && followerEmber() && player.findStatusAffect(StatusAffects.EmberFuckCooldown) < 0) {
				...
				else if (player.lowerBody.cockSpot.hasCock() && player.inRut && !pregnancy.isPregnant && rand(10) == 0 && flags[FlagEnum.EMBER_GENDER] >= 2) {
					emberRapesYourHeatness();
					return true;
				}
			}
		...
		public function emberAffection(changes:number = 0):number
		{
			...
			else if (flags[FlagEnum.EMBER_AFFECTION] < 0) flags[FlagEnum.EMBER_AFFECTION] = 0;
		...
		private function emberCorruption(changes:number = 0):number
		{
			...
			else if (flags[FlagEnum.EMBER_COR] < 0) flags[FlagEnum.EMBER_COR] = 0;
			return flags[FlagEnum.EMBER_COR];
		}

		public followerEmber():boolean
		{
			return flags[FlagEnum.EMBER_HATCHED] > 0;

		}

		public emberMF(man:string, woman:string):string
		{
			if (flags[FlagEnum.EMBER_GENDER] == 1) return man;
			else return woman;
		}
...
//Approaching Ember (Z)
		public function emberCampMenu():void
		{
			...
			//Low Affection:
			if (emberAffection() <= 25) Render.text("Ember sighs as you approach, and doesn't even look you in the eye before speaking.  \"<i>What do you want?</i>\"");
			//Moderate Affection:
			else if (emberAffection() <= 75) Render.text("Ember fidgets as " + emberMF("his", "her") + " tail starts to sway from side to side, " + emberMF("he", "she") + " looks at you and asks, \"<i>What is it?</i>\"");
			else Render.text("Ember's eyes light up as you close in on " + emberMF("him", "her") + ", and " + emberMF("he", "she") + " smiles nervously.  \"<i>Y-Yes?</i>\"");
			//OPTIONS HERE
			//[APPEARANCE]
			...
			if (flags[FlagEnum.EMBER_OVIPOSITION] > 0 && flags[FlagEnum.EMBER_GENDER] >= 2 && !pregnancy.isPregnant) egg = emberIsAnEggFactory;
			if (flags[FlagEnum.EMBER_MILK] > 0) milk = getMilkFromEmber;
...
//Approach for sex - initial output when selecting [Sex] menu (Z)
		private function emberSexMenu(output:boolean = true):void
		{
			if (output) {
				...
				Render.text("  ");
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
					Render.text(emberMF("His", "Her") + " cock ");
					if (flags[FlagEnum.EMBER_INTERNAL_DICK] == 1 || flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text("is poking out of " + emberMF("his", "her") + " slit");
					else Render.text("is starting to swell with blood");
					Render.text(".  ");
				}
				if (flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("You can see tiny rivulets of moisture starting to run down Ember's inner thighs as they rub together, barely hiding her precious treasure from your hungry eyes.  ");
				Render.text("Well, " + emberMF("he", "she") + " is a sexy beast; you ask what naturally comes to mind.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
				...
				if (player.lust >= 33 && player.lowerBody.vaginaSpot.hasVagina()) {
					getPenetrated = getPenetratedByEmberLastSexSceneWoooo;
				}
			}
			//scenes that require Ember to have a cunt
			if (flags[FlagEnum.EMBER_GENDER] >= 2) {
				eatOut = slurpDraggieCunnies;
				if (player.lowerBody.cockSpot.hasCock() && player.lust >= 33) penetrateHer = penetrateEmbrah;
			}
			if (player.lowerBody.vaginaSpot.hasVagina() && player.lust >= 33) getEatenOut = getEatenOutByEmbra;
			...
			if (emberAffection() >= 95 && player.lowerBody.cockSpot.hasCock() && player.cockThatFits(emberVaginalCapacity()) >= 0 && (player.hasItem(consumables.L_DRAFT) || player.stats.lib >= 50 || player.minLust() >= 40))
...
//Triggers randomly on exploration in Swamp
		public function findEmbersEgg():void
		{
			...
			if (flags[FlagEnum.TIMES_FOUND_EMBERS_EGG] == 0) {
				Render.text("You spot a cave entrance partially hidden behind mossy vegetation and decide to investigate.");
				...
				Render.text("You spot a familiar cave partially hidden behind the mossy vegetation and decide to confirm your suspicion.");
			...
			flags[FlagEnum.TIMES_FOUND_EMBERS_EGG]++;
...
//[=Take=] (Z)
		private function takeEmbersEggHomeInADoggieBag():void
		{
			...
			if (player.tallness < 84) Render.text("  Lifting it isn't as much of a problem as you thought; it's surprisingly light.  It is, however, very big and very awkward to carry.");
			else Render.text("  Between the egg's surprising lightness, and your own size and wide arms, you can easily carry the egg.");
			...
			flags[FlagEnum.TOOK_EMBER_EGG] = 1;
			flags[FlagEnum.EMBER_COR] = 50;
			doNext(camp.returnToCampUseOneHour);
		}
...
//Modified Camp Description (Z)
		public function emberCampDesc():void
		{
			...
			//NOT AN EGG! HAHA!
				...
				if (choice == 0) Render.text("Ember is lying in front of " + emberMF("his", "her") + " excavated den, watching the camp and resting.  Every once in a while " + emberMF("his", "her") + " eyes dart in your direction, before " + emberMF("he", "she") + " quickly looks away.\n\n");
				else if (choice == 1) Render.text("Ember doesn't seem to be around " + emberMF("his", "her") + " excavated den... it doesn't take long before Ember lands in front of it and then takes off again, apparently occupied with darting around in the sky for fun.\n\n");
				else Render.text("Ember appears to be fighting to stay awake; sometimes " + emberMF("he", "she") + " falls into a doze, but quickly snaps back to consciousness.\n\n");
			}
		}
...
//General Egg Interaction (Z)
		public function emberEggInteraction():void
		{
			...
			if (flags[FlagEnum.EMBER_COR] <= 25) {
				Render.text("  As you observe the egg, it glows with soft, metered pulses and you're overcome with a sense of calm and peace.  Watching them, you feel serene, as if untouched by this world's taint.");
			}
			//(else If the egg Corruption level is 26-74, aka \"<i>Tainted</i>\")
			else if (flags[FlagEnum.EMBER_COR] <= 75) {
				Render.text("  As you observe the egg, it glows with bright, gaudy pulses and you're overcome with a sense of arrogance and strength.  You feel somehow as if you could do anything, as if you were superior to everything, and no one could dare say otherwise.");
			}
			//(If the egg Corruption level is 75-100, aka \"<i>Corrupt</i>\")
			else {
				Render.text("  As you observe the egg you realize what you had taken for its pulses are actually just its normal color; the egg is actually 'glowing' with a black light!  As you stare, mesmerized, you begin to consider the pleasing contrast that would result if you covered it in your ");
				if (player.gender == 0) Render.text("cum, if you had any... ");
				else {
					if (player.lowerBody.cockSpot.hasCock()) Render.text("white ");
					else Render.text("glistening girl-");
					Render.text("cum... ");
				}
				Render.text(" You stop yourself and shake your head.  Where did that thought come from?");
				dynStats("lus", 10 + player.stats.cor / 10);
			}
			//(If player has lust >= 33)
			if (player.lust >= 33 && (flags[FlagEnum.EMBER_EGG_FLUID_COUNT] < 5 || flags[FlagEnum.EMBER_JACKED_ON] == 0)) {
				...
				if (flags[FlagEnum.EMBER_EGG_FLUID_COUNT] == 4) {
			...
			if (flags[FlagEnum.EMBER_EGG_FLUID_COUNT] >= 5 && flags[FlagEnum.EMBER_JACKED_ON] > 0 && flags[FlagEnum.EMBER_GENDER] > 0) {
...
//[= No =]
		private function dontEggFap():void
		{
			...
			if (player.hasItem(consumables.LACTAID)) lactaid = useLactaidOnEmber;
			let hatch:Function =null;
			if (flags[FlagEnum.EMBER_EGG_FLUID_COUNT] >= 5 && flags[FlagEnum.EMBER_JACKED_ON] > 0 && flags[FlagEnum.EMBER_GENDER] > 0) hatch = hatchZeMuzzles;

			Render.text("  The egg's rhythm suddenly changes; as if it were excited by something - something that you have brought near it.");
...
//Incubus Draft/Purified Incubus Draft (Z)
		private function useIncubusDraftOnEmber(purified:boolean = false):void
		{
			...
			if (purified) {
				...
				flags[FlagEnum.EMBER_COR] -= 10;
				if (flags[FlagEnum.EMBER_COR] < 0) flags[FlagEnum.EMBER_COR] = 0;
			}
			else {
			...
			Render.text("Uncorking the vial, you drizzle the slimy off-white fluid onto the pointed cone of the egg.  It oozes slowly across the surface, then seeps through the shell, leaving not a drop of moisture.");
			if (flags[FlagEnum.EMBER_GENDER] == 3 || flags[FlagEnum.EMBER_GENDER] == 0) {
				Render.text("  The egg's shell slowly changes to a soft, pastel blue.");
				flags[FlagEnum.EMBER_GENDER] = 1;
			}
			else if (flags[FlagEnum.EMBER_GENDER] == 2) {
				Render.text("   The egg's shell slowly changes to a lavender hue.");
				flags[FlagEnum.EMBER_GENDER] = 3;
			}
			flags[FlagEnum.EMBER_EGG_FLUID_COUNT]++;
			doNext(playerMenu);
		}
...
//Succubi Milk/Purified Succubi Milk (Z)
		private function useSuccubiMilkOnEmber(purified:boolean = false):void
		{
			...
			Render.text("Popping the cap off of the milk bottle, you pour the contents onto the egg - the porous shell soaks up the milk as fast as you dump it, spilling not a drop.");
			//(If Unsexed or Herm:
			if (flags[FlagEnum.EMBER_GENDER] == 3 || flags[FlagEnum.EMBER_GENDER] == 0) {
				Render.text("  The egg's shell slowly changes to a muted pink color.");
				flags[FlagEnum.EMBER_GENDER] = 2;
			}
			//If Male:
			else if (flags[FlagEnum.EMBER_GENDER] == 1) {
				Render.text("  The egg's shell slowly changes to a lavender hue.");
				flags[FlagEnum.EMBER_GENDER] = 3;
			}
			flags[FlagEnum.EMBER_EGG_FLUID_COUNT]++;
...
//Lactaid (Z)
		private function useLactaidOnEmber():void
		{
			...
			//max uses 1
			Render.text("Feeling a little bemused, you pour the creamy fluid onto the egg.  It is absorbed through the shell, and a spiderwork of creamy yellow vein-like markings suddenly forms on the shell's surface.");
			flags[FlagEnum.EMBER_MILK] = 1;
			flags[FlagEnum.EMBER_EGG_FLUID_COUNT]++;
			doNext(playerMenu);
		}
...
//Hair Extension Serum (Z)
		private function hairExtensionSerum():void
		{
			...
			//max uses 2
			Render.text("Wondering at your motivations, you pour the goblin gunk onto the egg.  Most rolls harmlessly off of the shell, leaving you annoyed at the waste... until you see ");
			if (flags[FlagEnum.EMBER_HAIR] == 0) {
				Render.text("a narrow tiger-stripe pattern suddenly develop");
				flags[FlagEnum.EMBER_HAIR] = 1;
			}
			else {
				Render.text("the tiger-stripes multiply");
				flags[FlagEnum.EMBER_HAIR] = 2;
			}
			Render.text(" on the egg.");
			flags[FlagEnum.EMBER_EGG_FLUID_COUNT]++;
			doNext(playerMenu);
		}
...
//Your Blood (Z)
		private function giveEmberBludSausages():void
		{
			clearOutput();
			//max uses 2
			Render.text("Examining your hand and the egg's reaction to it, you wonder if this is what the book meant by \"<i>sharing your essence</i>\".  Could be worth trying.  Wincing in pain as you bite the skin on your thumb, you smear the bloody digit along the surface of the egg, marking its exterior in crimson.  Shortly thereafter the blood is absorbed, leaving only a stain.  You wait expectantly for something else to happen");
			//[(0 prior),
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) {
				Render.text(" but the egg just glows its excitement, as if it wanted still more.");
				flags[FlagEnum.EMBER_INTERNAL_DICK] = 1;
			}
			else {
				Render.text(", but nothing does.  How disappointing.");
				flags[FlagEnum.EMBER_INTERNAL_DICK] = 0;
			}
			player.takeDamage(1);
			flags[FlagEnum.EMBER_ROUNDFACE] = 1;
...
//Genderless Version (Z)
		private function masturbateOntoAnEgg():void
		{
			clearOutput();
			if (player.gender == 0) {
				Render.text("The light pulses decrease in speed as you disrobe and expose your bare crotch, leaving you disappointed after summoning your perversity to bring you this far.  You feel as if you've let it down somehow...  This is confusing!  You decide to go away and deal with this fickle egg another time.");
				doNext(camp.returnToCampUseOneHour);
				return;
			}
			...
			//Male Version (Z)
			if (player.gender == 1) {
				Render.text("The light of the egg pulses rapidly, throwing strange illumination and shadow over your form as you hastily peel off your [armor], too far gone to recognize the absurdity.  Your heart is racing so fast with excitement, lust, anticipation... it actually matches the tempo of the pulses from the egg, when you care to notice.");
				Render.text("\n\nGrabbing your [cock] in your hands, you stand in front of the egg, ");
				if (player.lowerBody.cockSpot.count() <= 2) Render.text("pumping vigorously.");
				else Render.text("wrangling all your shafts together into one awkward bouquet of male organs and furiously stroking and squeezing them as best you can manage");
				Render.text(".  The egg's pulsations lure you on, coaxing you to squeeze, pull, thrust, and massage " + sMultiCockDesc() + " as best you can.  Harder and faster you go, feeling the churning ache from deep inside you.  Finally, with a cry of release, you unleash a ");
				if (player.cumQ() < 100) Render.text("trickle");
				else if (player.cumQ() <= 500) Render.text("stream");
				else if (player.cumQ() <= 1000) Render.text("gout");
				else Render.text("wave");
				Render.text(" of cum onto the egg.");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 0) {
			...
			//Female Version (Z)
				...
				if (player.wetness() <= 3) Render.text("few drops");
				else if (player.wetness() < 5) Render.text("squirt");
				else Render.text("torrent");
				Render.text(" of girlcum jumps from your pussy onto the egg.");
				...
				if (player.lowerBody.cockSpot.count() == 1) Render.text("your [cock]");
				else Render.text("as many of your cocks as you can manage to grab without falling off");
				Render.text(".  Finally, relentlessly, inexorably, you cum, spraying your semen into the air to spatter back onto the egg, mixing with the girlish juices from your netherlips to soak into the egg's surface, leaving it slick with your mixed sexual fluids.");
			...
			if (flags[FlagEnum.EMBER_EGG_FLUID_COUNT] < 5) {
			...
			flags[FlagEnum.EMBER_JACKED_ON] = 1;
...
//HATCH DAT BITCH
		private function hatchZeMuzzles():void
		{
			...
			if (flags[FlagEnum.EMBER_GENDER] == 0) Render.text("white ");
			else if (flags[FlagEnum.EMBER_GENDER] == 1) Render.text("blue ");
			else if (flags[FlagEnum.EMBER_GENDER] == 2) Render.text("pink ");
			else Render.text("purple ");
			Render.text("dust.");
			//FURRAH
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) {
				//Male Anthro (Z)
				if (flags[FlagEnum.EMBER_GENDER] == 1) {
					...
					Render.text(" the monster's crotch; there, a deceptively small slit in the flesh suddenly disgorges a 16-inch penis unlike anything you've ever seen before, bearing a rounded, elongated head and a series of ridges that give it an almost segmented look.  A pair of apple-sized balls drop into place under it.  He is most definitely male");
					if (flags[FlagEnum.EMBER_MILK] > 0) Render.text(", drooling nipples notwithstanding");
					Render.text(".");
				}
				//Female Anthro (Z)
				else if (flags[FlagEnum.EMBER_GENDER] == 2) {
					...
					Render.text(" the monster's crotch; there, you see that the fine scales actually separate to reveal a slick-looking pussy.  She's clearly a female, with no noteworthy 'additions' that you can see.");
				}
				//Herm Anthro (Z)
				else {
					Render.text("\n\nIt's huge, standing 7 feet tall at the very least.  Its build is lean and slender, with powerful arms and legs that end in reptilian claws, complete with splay-toed feet capped by menacing talons.");
					...
					Render.text("the monster's crotch; there, you see the scales part in two places.  The lower opening is unmistakably a pussy; but from the slit just above it suddenly distends a 16-inch penis unlike anything you've ever seen before, bearing a rounded, elongated head and a series of ridges that give it an almost segmented look.  Beneath it, a pair of apple-sized balls fall into place heavily, leaving you no doubt that she is a hermaphrodite.");
				}
			}
			//Boring version
			else {
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1) {
					...
					if (flags[FlagEnum.EMBER_MILK] > 0 || flags[FlagEnum.EMBER_HAIR] > 0) Render.text(", complete with ");
					if (flags[FlagEnum.EMBER_MILK] > 0) {
						Render.text("huge breasts, easily F-cups");
						if (flags[FlagEnum.EMBER_HAIR] > 0) Render.text(" and ");
					}
					if (flags[FlagEnum.EMBER_HAIR] > 0) Render.text("long, feminine locks of hair");
					Render.text(", but his maleness is undeniable.  Especially when you spot ");
					if (flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) Render.text("the slit in his pelvis that disgorges a foot and a half-long inhuman penis");
					else Render.text("a foot and a half long human penis that sways between his legs");
					Render.text(", completed by apple-size nuts held inside a fleshy sack.");
				}
				...
				//Herm Monstergirl (Z)
					...
					if (flags[FlagEnum.EMBER_INTERNAL_DICK] == 0) Render.text("huge, human prick");
					else Render.text("huge inhuman cock hanging from some kind of internal sheath");
					Render.text(" and apple-size nuts held inside a fleshy sack slung under - the ensemble hangs nearly level with her knees.  She... or he? is obviously a hermaphrodite.");
				}
			}
			//Aftermath (Z)
...
//Aftermath (Z)
		private function meetEmberAftermath():void
		{
			...
			flags[FlagEnum.EMBER_HATCHED] = 1;
...
//Appearance (shows Ember's appearance, always available)
		private function embersAppearance():void
		{
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) {
				Render.text("Ember is a 7' 3\" tall humanoid dragon, with supple, long limbs and a build that is toned and athletic, with powerful underlying muscles.  " + emberMF("He", "She") + " looks strong and imposing, but ");
				if (flags[FlagEnum.EMBER_GENDER] == 1) Render.text(" not overly muscled.");
				else Render.text(" feminine.");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1) {
					...
					if (flags[FlagEnum.EMBER_MILK] > 0) Render.text(" dribbling breasts,");
					Render.text(" snout, wings, and horns, gives him the silhouette of a prince from your village's stories: dashing and handsome.");
				}
				//(Female/Herm)
				else {
				...
				if (flags[FlagEnum.EMBER_HAIR] == 1) {
					if (flags[FlagEnum.EMBER_GENDER] == 1) Render.text("  Short ");
					else Render.text("  Shoulder-length ");
					Render.text("steel-gray hair sprouts from " + emberMF("his", "her") + " head.  You'd think that a dragon with hair would look weird, but it actually compliments Ember's looks very well.");
				}
				//(if Ember has a level 2 mane)
				else if (flags[FlagEnum.EMBER_HAIR] == 2) {
					Render.text("  Tracing " + emberMF("his", "her") + " spine, a mane of hair grows; starting at the base of " + emberMF("his", "her") + " neck and continuing down " + emberMF("his", "her") + " tail, ending on the tip of " + emberMF("his", "her") + " tail in a small tuft.  It is the same color as the hair on " + emberMF("his", "her") + " head, but shorter and denser; it grows in a thick vertical strip, maybe two inches wide.  It reminds you vaguely of a horse's mane.");
				}
				...
				Render.text("  The ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text("little ");
				Render.text("exposed flesh of Ember's body is a light shade of pink; but flushes when " + emberMF("he", "she") + "'s aroused, drawing your eyes towards " + emberMF("his", "her") + " most sexual parts.");

				switch (pregnancy.event) {
					case 2: Render.text("  At the moment there's a slight pudgyness to her golden belly.");
							break;
					case 3: Render.text("  Her golden belly is a little larger and firmer than usual.");
							break;
					case 4:
					case 5: Render.text("  Her golden belly has grown quite a bit.  Ember often rests her hand on it, especially when she sees you looking at it.");
							break;
					case 6: Render.text("  Her large pregnant belly has forced Ember to change her posture.  It looks like she has a big golden bullseye painted on her stomach.  Ember looks tired but happy.");
							break;
					case 7: Render.text("  Her swollen belly is as large as that of any pregnant woman you can remember from Ingnam and you " + (flags[FlagEnum.EMBER_OVIPOSITION] > 0 ? "can easily make out the egg's outline under her golden skin" : "occasionally see movement as the baby shifts in Ember's womb") + ".");
							break;
					case 8:
					case 9: Render.text("  Ember's golden belly is stretched taut by the large " + (flags[FlagEnum.EMBER_OVIPOSITION] > 0 ? "egg" : "baby") + " in her womb.  You're sure she'll give birth very soon, there just isn't room for the " + (flags[FlagEnum.EMBER_OVIPOSITION] > 0 ? "egg" : "baby") + " to grow any larger.");
							break;
					default:
				}
				...
				if (flags[FlagEnum.EMBER_MILK] > 0 || flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("F-cup soft, pillowy breasts");
				else Render.text("flat manly pecs");
				Render.text(" covered in fine scales excepting " + emberMF("his", "her") + " areolas; 0.5 inch nipples protrude from the center of the ");
				if (flags[FlagEnum.EMBER_MILK] > 0 || flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("generous mounds");
				else Render.text("masculine pectorals");
				Render.text(".");
			...
			//Dragon-girl Appearance (By Radar) (Z)
				...
				Render.text("Ember is a 7' 3</i>\" tall ");
				if (flags[FlagEnum.EMBER_GENDER] == 1) Render.text("male");
				else if (flags[FlagEnum.EMBER_GENDER] == 2) Render.text("female");
				else Render.text("hermaphrodite");
				Render.text(" dragon-" + emberMF("boy", "girl") + ", with slender limbs and a thin frame; " + emberMF("he", "she") + " refuses to wear any kind of clothing and exposes " + emberMF("him", "her") + "self naturally to the world.  " + emberMF("He", "She") + " sports a rather human looking face, but with several key differences.  Where a normal set of human eyes would be located, instead a pair of orange, reptilian eyes stares back at you, filled with immeasurable pride and ferocity.");
				...
				//Short:
				if (flags[FlagEnum.EMBER_HAIR] < 1) Render.text("is quite short, giving " + emberMF("him", "her") + " that definitive " + emberMF("masculine", "tomboy") + " look.");
				else Render.text("drops down to " + emberMF("his", "her") + " shoulders, giving " + emberMF("him", "her") + " the look of the " + emberMF("handsome", "beautiful") + " warriors from your village legends.");

				Render.text("\n\n" + emberMF("His", "Her") + " chest is also human in appearance and houses a pair of ");
				if (flags[FlagEnum.EMBER_MILK] > 0 || flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("F-cup breasts that support 0.5 inch nipples and hang heavily; you idly wonder if " + emberMF("he", "she") + "'ll develop lower back problems as she spends more time in Mareth");
				else Render.text("manly pectorals with 0.5 inch nipples");
				Render.text(".  Just below " + emberMF("his", "her") + " collarbone, in the middle of " + emberMF("his", "her") + " chest, you see what looks like a small, golden, heart-shaped scale; adorning the chest like a birthmark of some sort.");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
					if (flags[FlagEnum.EMBER_INTERNAL_DICK] == 0) Render.text("  " + emberMF("He", "She") + " sports a flaccid penis and a pair of apple-sized balls that sit dangerously exposed to the elements, let alone to naked blades or a heavy blow.  Yet " + emberMF("he", "she") + " doesn't seem concerned about that in the least, almost daring anyone to focus on them.  While " + emberMF("he", "she") + " isn't aroused right now, Ember's penis can reach a length of approximately 16 inches, and it looks to be about 2 inches thick.");

					else Render.text("  " + emberMF("He", "She") + " sports what looks like a protective slit of some sort, protecting " + emberMF("his", "her") + " dick from the elements as well as stray blows.  You can't see it right now; but you remember it to be about 16 inches long and 2 inches thick.");
				}
				//Ember has a pussy:
				if (flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("  The inviting lips of a human-looking pussy purse between her legs; some moisture seems to have gathered on her labia, giving it a slick look that just begs for attention.");
...
//Talk
		private function talkToEmber():void
		{
			...
			if (player.isPregnant()) { //Extra check might protect against inappropriate Ember complaints
				if (flags[FlagEnum.EMBER_OVI_BITCHED_YET] == 0 && player.pregnancyType == PregnancyType.OVIELIXIR_EGGS) {
					emberBitchesAboutPCBeingFullOfEggs();
					doNext(camp.returnToCampUseOneHour);
					return;
				}
				if (player.pregnancyIncubation < 200 && player.pregnancyType != PregnancyType.EMBER && flags[FlagEnum.EMBER_BITCHES_ABOUT_PREGNANT_PC] == 0) {
					manEmberBitchesAboutPCPregnancy();
					doNext(camp.returnToCampUseOneHour);
					return;
				}
				if (player.pregnancyType == PregnancyType.EMBER && player.pregnancyType < 300 && flags[FlagEnum.EMBER_TALKS_TO_PC_ABOUT_PC_MOTHERING_DRAGONS] == 0) {
					emberTalksToPCAboutPCDragoNPregnancy();
					doNext(camp.returnToCampUseOneHour);
					return;
				}
			}
			if (flags[FlagEnum.EMBER_PREGNANT_TALK] == 0 && pregnancy.event > 1) {
				emberIsPregnantFirstTimeTalkScene();
				doNext(camp.returnToCampUseOneHour);
			}
			clearOutput();
			Render.text("What will you talk about?");
...
//Talk about Dragons (Z)
		private function talkToEmberAboutDragonzzz():void
		{
			...
			if (choice == 0) {
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) Render.text(", a blush creeping onto " + emberMF("his", "her") + " cheeks");
				Render.text(".");
...
//Exploration (Z)
		private function discussExplorationWithEmber():void
		{
			...
			Render.text("You ask Ember for news of anything interesting " + emberMF("he", "she") + " has seen during " + emberMF("his", "her") + " aerial explorations of Mareth.");
			...
			if (choice == 0) {
				...
				if (subChoice == 0) Render.text("imps. Only a few imps, and they seemed very nervous. I'm guessing that whatever naturally lives there is something that they really don't want to get involved with.");
				else if (subChoice == 1) Render.text("two goblins in the foothills, arguing with each other. One was saying that the other shouldn't go up into the hills, as apparently the minotaurs that live there are too much for an untrained girl like her to take without being split open. The second goblin just laughed at her, called her a 'wimp' and told her that she's going to get herself 'some juicy bull-cock and tasty mino-spooge'. Ugh, disgusting little creatures.");
				else if (subChoice == 2) Render.text("a pair of muscle-bound bull-men beating on each other with their bare fists. They spent over an hour smashing each other into a bloody pulp, and then the winner promptly started fucking the loser up the ass.  I had seen more than enough by that point, so I left.");
				else if (subChoice == 3) Render.text("this... creature... that looked kind of like a human woman, but with a big dick where her clit should be.  She was walking around stark naked, 'cept for a bunch of piercings, and leading this bull-man along like a pet by a chain attached to a ring anchored into his cockhead.");
				else if (subChoice == 4) Render.text("a couple of goblins sharpening scissors on some rocks outside of a cave with a door on it. Weird. Wonder what they could be doing in there?");
				else if (player.findStatusAffect(StatusAffects.WormsOff) < 0 && rand(2) == 0) Render.text("a horrible swarm of slimy white worms, clumped together into a mockery of a human form and squelching along.  It managed to latch onto this two-headed dog-creature and... ugh!  The worms started forcing their way into both of its cocks!  I've never seen anything so disgusting!");
				else if (subChoice == 5) Render.text("this two-headed dog-morph loping around; it spotted an imp, dropped to all fours, then gave chase. Managed to catch the ugly little demon, whereupon it ass-raped it, then ate it.");
			}
			else if (choice == 1) {
				subChoice = rand(5);
				...
				if (subChoice == 0) Render.text("a whole tribe of imps, just lounging around in a glade, jerking themselves off or squabbling over food.  Nasty little things, but easily dispatched.");
				else if (subChoice == 1) Render.text("a  goblin with a huge pregnant belly, laughing to herself and swilling down that ale they brew, slopping it all over herself.  Little hedonists.");
				else if (subChoice == 2) Render.text("this strange bee-woman creature... she made this, this music that started messing with my head.  I spat a tongue of flames at her and she flew away in fright, luckily.");
				//(If player has impregnated Tamani)
				else if (subChoice == 3 && flags[FlagEnum.TAMANI_NUMBER_OF_DAUGHTERS] > 0) {
					Render.text("that green-skinned baby-making whore, Tamani.  She was letting some of her daughters suckle from her and grinning ear to ear as she named the 'prize catch' she got to father them, exhorting them to hunt him down.");
					if (flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("  You should have more pride than to let some brainless cunt like that have her way with you!");
				}
				//(If player has not yet impregnated Tamani)
				else if (subChoice == 3 && !kGAMECLASS.forest.tamaniScene.pregnancy.isPregnant) Render.text("one goblin being teased by a bunch of pregnant goblins for not being pregnant yet.  She just spat back that she wanted a 'better catch' to be her baby-maker than a mere imp and wandered off.");
				//(If Jojo isn't in the camp & not corrupt)
				else if (rand(2) == 0 && monk <= 1 && player.findStatusAffect(StatusAffects.PureCampJojo) < 0) Render.text("this mouse-morph monk, sitting in a glade and meditating. A goblin tried to proposition him; he just gave her a lecture and sent her running away in tears.  When an imp tried to attack him, he crushed its skull with a staff he had.  Not bad moves for such a weedy little thing...");
				else Render.text("one glade I touched down in to catch myself a nice brace of plump coneys, when all of a sudden this... this thing made out of flailing vines and fruit attacks me.  It went up in a puff of smoke once I torched it, of course.");
			}
			else if (choice == 2) {
				subChoice = rand(2);
				...
				if (subChoice == 0) {
					Render.text("a pair of shark-women - well, one was a woman, the other had breasts but also had a cock");
					if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text(" like me");
					Render.text(". They were on the beach, fucking each other's brains out.");
				}
				else {
					//(If Whitney's Farm is not yet in the Places menu)
					if (rand(3) == 0 && player.statusAffects.get("MetWhitney").value1 <= 1) Render.text(" a big farm on the shoreline.  I saw some sort of cow-woman tending to fields of peppers, and a dog-woman herding cattle. They looked pretty fat and tasty, too... What?  I meant the cattle, stupid!  And I wouldn't have taken them - it's no fun snatching up livestock.  I prefer to chase down a deer or a wild auroch or something like that.");
					//(if Fetish Cult is encounterable)
					else if (rand(3) == 0 && player.statusAffects.has("FetishOn")) Render.text("a pair of humans, arguing about sex.  They both wanted it, but the guy said he wanted to do the 'naughty schoolboy and female teacher' routine, while the girl wanted to do the 'noblewoman and love-servant' routine.  Weird; their clothes kept changing back and forth as they argued.");
					//(If Beautiful Sword has not been acquired)
					else if (rand(3) == 0 && player.findStatusAffect(StatusAffects.TookBlessedSword) < 0 && player.findStatusAffect(StatusAffects.BSwordBroken) < 0) Render.text("a sword jammed into a tree.  Weird; what kind of idiot would stick a weapon there like that?  And what kind of weakling wouldn't be able to take it out?");
					else if (rand(3) == 0 && player.findStatusAffect(StatusAffects.CampRathazul) < 0) Render.text("a smelly rat-man moping around while some weird equipment bubbled and boiled.  I think maybe he was an alchemist.");
					else Render.text("a great blob of green goo, sliding along and minding its own business.  I could swear it looked up at me once, and grew a penis... that can't be right, though.");
				}
			}
			else if (choice == 3) {
				subChoice = rand(4);
				...
				if (subChoice == 0) Render.text("a woman with four big breasts, squeezing milk out of her tits and into the sand.  I didn't know breasts could hold that much milk!");
				else if (subChoice == 1) Render.text("a whole tribe of demons, lounging around an oasis.  Would have been too much bother to kick the crap out of them, so I left them alone - well, alright, I did buzz them to make them scatter like scared sheep for fun.");
				//(if player hasn't solved Marcus & Lucia's argument)
				else if (rand(2) == 0 && player.findStatusAffect(StatusAffects.WandererDemon) < 0 && player.findStatusAffect(StatusAffects.WandererHuman) < 0) Render.text("a human with balls so big he had to carry them in a wheelbarrow, trundling through the wasteland with a succubus.  They were arguing about whether or not he should become an incubus.");
				else {
					Render.text("this strange creature, like a woman with a snake's tail for legs, slithering through the sand.  ");
					if (player.isNaga()) Render.text("She looked a lot like you.");
					else Render.text("I've never seen anything like her before.");
				}
			}
			Render.text("</i>\"");
			doNext(camp.returnToCampUseOneHour);
...
//Talk about Yourself (Z)
		private function talkToEmberAboutYourself():void
		{
			clearOutput();
			let points:number = 0;
			Render.text("You ask Ember what " + emberMF("he", "she") + " thinks about you.");
			//(Low Affection)
			...
			//(If PC Str < 50)
			if (player.str < 50) Render.text("  \"<i>You could use more training; you look pretty weak...</i>\" Ember says, critically.");
			//(If PC Str < 90)
			else if (player.str < 90) {
				Render.text("  \"<i>You're well on your way to being as strong as a dragon, but there's still room for improvement.</i>\" Ember says, pleased.");
				//(+1 point)
				points++;
			}
			//(If PC Str >= 90)
			else {
				Render.text("  \"<i>With muscles like these, you could easily lift anything you wanted... even me...</i>\" Ember trails off dreamily, almost purring.  \"<i>N-Not that I'd want you to carry me in your arms or anything...</i>\"");
				//(+2 points)
				points += 2;
			}
			Render.text("\n\nNext, Ember pokes at your chest and your ribs.  ");
			//(If PC Tou < 50)
			if (player.tou < 50) Render.text("\"<i>I don't know how you could hope to survive out there.  You look like the wind could blow you away.</i>\"");
			//(If PC Tou < 90)
			else if (player.tou < 90) {
				Render.text("\"<i>Not bad; some more training and you could be as tough as me!</i>\" Ember says, pleased.");
				//(+1 point)
				points++;
			}
			//(If PC Tou >= 90)
			else {
				Render.text("\"<i>Your body's as tough as rock, you should show off more frequently...  N-Not that I'd want you to go naked!  You should always wear something to protect yourself,</i>\" " + emberMF("he", "she") + " quickly adds.");
				//(+1 Affection)
				emberAffection(1);
				//(+2 points)
				points += 2;
			...
			//(If PC Int < 50)
			if (player.stats.int < 50) Render.text("\"<i>You're still very naive... anyone or anything could trick you at any time...</i>\"  Ember frowns.");
			//(If PC Int < 90)
			else if (player.stats.int < 90) {
				Render.text("\"<i>I see wisdom in your eyes, but you could always use more lessons.</i>\"");
				//(+1 point)
				points++;
			}
			//(If PC Int >= 90)
			else {
				Render.text("Ember's eyes widen.  \"<i>I see great wisdom in your eyes...</i>\"  Ember's face grows more absent by the minute as " + emberMF("he", "she") + " looks at you in deep thought, until the trance breaks.  " + emberMF("He", "She") + " blows at your face, brushing a claw on your forehead.  \"<i>Y-You had something in your face... I wasn't staring into your steely eyes.</i>\"");
				//(+2 points)
				points += 2;
			}
			...
			//(If PC Spd < 50)
			if (player.stats.spe < 50) Render.text("You try to block the pebble, but you're not quick enough.  It hits your belly, not hard enough to hurt.  \"<i>You need to work on your reactions; anything could surprise you out there and you'd be helpless.</i>\"");
			//(If PC Spd < 90)
			else if (player.stats.spe < 90) {
				Render.text("You successfully deflect the pebble.  \"<i>Not bad!  Next time try to catch it,</i>\"  Ember says, pleased.");
				//(+1 point)
				points++;
			}
			//(If PC Spd >= 90)
			else {
				Render.text("You easily catch the pebble and throw it back at Ember, surprising " + emberMF("him", "her") + ".  \"<i>With reflexes like these, you could even...</i>\" Ember mumbles.  \"<i>D-Don't even think about it!</i>\" " + emberMF("he", "she") + " snaps.");
				//(+2 points
				points += 2;
			}
			...
			if (player.nagaScore() >= 3 || player.lizardScore() >= 3 || player.dragonScore() >= 3) {
				Render.text("  Ember's eyes linger on your form.  After a moment of awkward silence, you clear your throat.  " + emberMF("he", "she") + " blinks and says hurriedly.  \"<i>Sorry... I was just admiring you-</i>\" Realizing what " + emberMF("he", "she") + " was about to say,and quickly blurts out.  \"<i>I mean the weather! Yes, nice day today isn't it?</i>\"  You're not convinced, but let it slide. Ember recomposes and clears " + emberMF("his", "her") + " throat before saying.");
				//(+1 Affection)
				points++;
			}
			Render.text("\n\n\"<i>All right, here's what I think about you.</i>\"  ");
			if (points < 2) {
				Render.text("\"<i>This is not good at all; it's a miracle you even managed to survive in this world thus far.  With the things I've seen roaming the land...</i>\" Ember trails off, concerned.  \"<i>You should stay in the camp; I'll help you train.</i>\"  You don't much like the assessment.");
			}
			//(else if points < 6)
			else if (points < 6) {
				Render.text("\"<i>You're doing fine, but make sure you don't slack off, and keep training.</i>\"  You thank Ember for sharing " + emberMF("his", "her") + " thoughts.");
			}
			//(else)
			else {
				Render.text("\"<i>You're quite a catch... if you strolled down the street you'd have dragons fawning all over you...</i>\"  Realizing what " + emberMF("he", "she") + " just said, Ember coughs furiously.  \"<i>I-I mean... lesser dragons might fawn all over you.  You don't meet my standards!</i>\"");
				emberAffection(5);
			}
			doNext(camp.returnToCampUseOneHour);
		}
...
//To be implimented once preggers is set up.
		private function manEmberBitchesAboutPCPregnancy():void
		{
			...
			//(Low Affection)
			if (emberAffection() <= 25) Render.text("The two of you talk about nothing in particular.  It's light, airy and pointless.  When you finish up, though, you realize something odd; Ember was doing " + emberMF("his", "her") + " best all throughout the conversation to avoid looking at your pregnant belly - almost as if " + emberMF("he", "she") + " were upset by it?");
			//(Moderate Affection)
			else if (emberAffection() < 75) Render.text("Ember stares coldly at your gravid midriff as icebreaker after icebreaker from you falls flat.  You ask what's wrong.  \"<i>Nothing.  Nothing is wrong,</i>\" Ember states, flatly.");
			//(High Affection)
			else Render.text("The expression Ember gives you is very cold.  \"<i>What is the meaning of this!?</i>\" " + emberMF("he", "she") + " says, motioning towards your bloated belly.  \"<i>You're supposed to help <b>me</b> breed more dragons; not slut around with trash!</i>\"  Ember blows out an indignant puff of smoke and walks away.");
		}
...
//To be implimented once preggers is set up.
		private function emberTalksToPCAboutPCDragoNPregnancy():void
		{
			...
			Render.text("You notice Ember's eyes are fixated on your swollen belly, and cautiously ask what " + emberMF("he", "she") + "'s looking at.");
...
//Occurs once during Ember's first pregnancy.
		private function emberIsPregnantFirstTimeTalkScene():void
		{
			...
			Render.text("You can't help but stare at Ember's swollen belly; it's still so hard to take in that you have actually fathered a child with a creature of legend.  Especially given that there are times when it's hard to be entirely certain that Ember genuinely likes you.");
			Render.text("\n\nEmber catches you staring");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) Render.text(" and blushes");
			Render.text(", one of her hands rubbing her belly lovingly and tenderly.  \"<i>W-what is it?  Why are you looking at me like that?  Something wrong?</i>\"");

			Render.text("\n\nYou merely give her a faint smile and tell her that she's beautiful when pregnant.");
			if (player.stats.cor >= 66) Render.text("  You almost choke on those cheesy words; still, she'll never put out for you if you don't butter her up.");
			...
			if (player.stats.cor >= 66) Render.text("  Is she seriously buying this nonsense?  Your smile would be miles wide if you let your lip curl even a bit, so you keep a straight face with due dilligence.");
			...
			if (player.stats.cor < 30) Render.text("  Given her attitude and nature, you know how lucky you are to have someone like her want to be your mate.");
...
//Yup, you guessed it, only once.
		private function emberBitchesAboutPCBeingFullOfEggs():void
		{
			...
			flags[FlagEnum.EMBER_OVI_BITCHED_YET] = 1;
		}
...
//counter triggered when PC starts an hour in camp at 100 Lust, output when reaching 10 counters
		public function emberBitchesAtYouAboutLustiness():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				Render.text("your " + num2Text(player.lowerBody.cockSpot.count()) + " stiff as iron");
				if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and ");
			}
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("your " + vaginaDescript() + " slick and wet with moisture, ready to fuck");
			Render.text(".  You want sex so bad it almost hurts...");
			...
			Render.text("\n");
			flags[FlagEnum.EMBER_LUST_BITCHING_COUNTER] = 0;
		}
...
//This should reduce the chance of meeting minotaurs.
		public function minotaurJizzFreakout():void
		{
			...
			if (followerKiha()) Render.text("\n\nKiha saunters up and smirks.  \"<i>I thought I had a temper.</i>\"");
			flags[FlagEnum.EMBER_CURRENTLY_FREAKING_ABOUT_MINOCUM] = 1;
			Render.text("\n");
		}
...
//Normal note for PC getting over mino cum addiction plays first
		public function emberGetOverFreakingOutAboutMinoJizz():void
		{
			...
			//(High Affection)
				...
				//(Male/HermEmber:
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text(", especially for sucking cock,");
				Render.text(" you should come to me instead; I'll help you.</i>\"  Then as if realising what " + emberMF("he", "she") + "'s just implied, Ember looks away; and hurriedly adds, \"<i>But not the way you're thinking...</i>\"");
...
//PCs that are dragony enough might be bestowed with Tainted Ember's signature breath weapon.
		private function bloodForTheBloodGod():void
		{
			...
			if (flags[FlagEnum.DRANK_EMBER_BLOOD_TODAY] == 1 && !debug) {
			...
			//(Medium Affection)
				...
				if (player.stats.cor < 50) Render.text(", expressing your gratitude; gently, you kiss " + emberMF("his", "her") + " fingers, then the cut, letting its cool, iron-tinted tang roll across your lips and tongue.  You carefully lick, trying to avoid causing pain as you drink that which " + emberMF("he", "she") + " has so generously offered you.");
				else Render.text(", quickly drawing it to your mouth and forming a seal around the cut with your lips, anxious to let not a drop escape.");
			...
			flags[FlagEnum.DRANK_EMBER_BLOOD_TODAY] = 1;
...
//[=Stop=]
		private function noMoDagronBlud():void
		{
			...
			if (emberAffection() < 75) {
				Render.text("You decide to stop for now and pull away.  Ember licks " + emberMF("his", "her") + " own wound " + emberMF("him", "her") + "self and you thank " + emberMF("him", "her") + " for sharing.");
				Render.text("\n\n\"<i>D-Don't mention it...</i>\"");
			}
			else {
				Render.text("You decide to stop for now and pull away.  Ember licks " + emberMF("his", "her") + " lips, draws " + emberMF("his", "her") + " tongue back into " + emberMF("his", "her") + " mouth and purrs with delight.  When " + emberMF("he", "she") + " realizes what " + emberMF("he", "she") + "'s doing, though, " + emberMF("he", "she") + " sobers.  \"<i>D-Don't get any strange ideas...</i>\"");
		...
		private function drinkDeeplyOfDagronBlud():void
		{
			clearOutput();
			if (emberAffection() < 75) {
				Render.text("You decide to continue drinking Ember's blood; intent on acquiring all the power it can bring out from within you.");
...
//TF messages (Z)
		private function emberTFs():void
		{
			...
			if (changes < changeLimit && player.dragonCocks() < player.totalCocks() && rand(3) == 0) {
				...
				while (temp > 0) {
					temp--;
					if (player.lowerBody.cockSpot.list[temp].cockType != CockType.DRAGON) choices[choices.length] = temp;
				}
				...
				if (player.stats.sens >= 50) Render.text("however, it's not until you press on your new, sensitive knot that you manage to blow your load and enjoy the last few spasms of pleasure as it finally finishes changing.");
				else Render.text("but you sternly rein in your hands and tuck them into your armpits as the arousing changes run their course.");
				Render.text("  <b>You now have a dragon penis.</b>");
				//lose lust if sens>=50, gain lust if else
				dynStats("sen", 10, "lus", 10);
				changes++;
				//Apply the TF
				player.lowerBody.cockSpot.list[select].cockType = CockType.DRAGON;
				player.lowerBody.cockSpot.list[select].knotMultiplier = 1.3;
			}
			//Gain Dragon Head
			if (changes < changeLimit && rand(3) == 0 && player.faceType != FACE.DRAGON && flags[FlagEnum.EMBER_ROUNDFACE] == 0) {
			...
			if (player.hornType != HORNS.DRACONIC_X4_12_INCH_LONG && changes < changeLimit && rand(5) == 0) {
				//No dragon horns yet.
				if (player.hornType != HORNS.DRACONIC_X2 && player.hornType != HORNS.DRACONIC_X4_12_INCH_LONG) {
					...
					if (player.horns > 0) {
						...
						if (player.hornType == HORNS.DEMON && player.horns > 4) {
							...
							if (player.horns > 13) {
								Render.text("  The change seems to have shrunken the horns, they're about a foot long now.", false);
								player.horns = 12;
							}
				...
				//ALREADY DRAGON
					...
					if (player.hornType == HORNS.DRACONIC_X2) {
						if (player.horns < 12) {
							...
							if (player.horns >= 12) Render.text("  <b>Your horns settle down quickly, as if they're reached their full size.</b>", false);
							changes++;
						}
						//maxxed out, new row
						else {
			...
			//Gain Dragon Ears
			if (changes < changeLimit && rand(3) == 0 && player.upperBody.head.earType != EARS.DRAGON) {
				player.upperBody.head.earType = EARS.DRAGON;
			...
			//Gain Dragon Tongue
			if (changes < changeLimit && rand(3) == 0 && player.tongueType != TONUGE.DRACONIC) {
			...
			//Gain Dragon Scales
			if (player.skinType != SKIN.SCALES && changes < changeLimit && rand(3) == 0) {
			...
			//Gain Dragon Legs
			if (player.lowerBody != LOWER_BODY.DRAGON && changes < changeLimit && rand(3) == 0) {
				//(if drider)
				if (player.lowerBody == LOWER_BODY.DRIDER_LOWER_BODY)
				...
				//(If hoofed legs)
				else if (player.lowerBody == LOWER_BODY.HOOFED) {
			 ...
			 */
			//Grow Dragon Wings
			if (player.upperBody.wingType != WING.DRACONIC_LARGE && changes < changeLimit && rand(3) == 0) {
				if (player.upperBody.wingType == WING.NONE) {
					...
					player.upperBody.wingType = WING.DRACONIC_SMALL;
					player.upperBody.wingDesc = "small, draconic";
				}
				//(If Small Dragon Wings Present)
				else if (player.upperBody.wingType == WING.DRACONIC_SMALL) {
			...
			if (player.dragonScore() >= 4 && changes < changeLimit && !player.perks.has("Dragonfire")) {
				...
				if (emberAffection() >= 75) Render.text("\n\nEmber immediately dives back in to soothe your battered throat and mouth with another kiss.");
				changes++;
			}
			if (player.dragonScore() >= 4 && rand(3) == 0 && player.gender > 0) {
				Render.text("\n\nA sudden swell of lust races through your ");
				if (player.lowerBody.cockSpot.hasCock()) {
					Render.text(cockDescript(0));
					if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and ");
				}
				if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(vaginaDescript());
				Render.text(", making you wish Ember hadn't run you off.  All you can think about now is fucking " + emberMF("his", "her") + "; ");
				if (player.lowerBody.cockSpot.hasCock() && flags[FlagEnum.EMBER_GENDER] >= 2) {
					Render.text("filling her womb with your seed and fertilizing her eggs");
					if (player.lowerBody.vaginaSpot.hasVagina() && flags[FlagEnum.EMBER_GENDER] == 3) Render.text(" even while ");
				}
				if (player.lowerBody.vaginaSpot.hasVagina() && (flags[FlagEnum.EMBER_GENDER] == 3 || flags[FlagEnum.EMBER_GENDER] == 1)) Render.text("taking that hard, spurting cock inside your own " + vaginaDescript(0));
				Render.text("... too late, you realize that <b>Ember's blood has sent your draconic body into ");
				if (player.lowerBody.cockSpot.hasCock() && flags[FlagEnum.EMBER_GENDER] >= 2) {
					Render.text("rut");
...
//Player always gets the egg.
		private function emberIsAnEggFactory():void
		{
			...
			if (emberAffection() <= 25) {
				...
				if (player.stats.cor >= 40) Render.text("or sell it for quick cash ");
				Render.text("anyway, but you don't tell her that.");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
					Render.text(" and her ");
					if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) Render.text("dragon-dick jutting from its slit,");
					else Render.text("veiny, bulbous penis fully");
					Render.text(" erect");
				}
				Render.text(".  One of her hands gently teases her clit, as another traces her outer labia; she bites her lips, trying to stifle her moans of pleasure, but it's useless... every touch brings a sigh.");
				...
				//[(if Ember has a dick)
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text(" and her throbbing cock is soft enough to hide");
				Render.text(" before she presents the egg to you.");
			...
			//(Medium Affection)
				...
				Render.text("\n\nEmber gasps");
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) Render.text(" and her cheeks grow red with embarrassment");
				Render.text(".  \"<i>T-This...</i>\"  She can't quite bring herself to finish the sentence and settles for just looking at the ground.");
			...
			//(High Affection)
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text(", and her cock points you accusingly");
				Render.text(".");
				...
				if (player.tongueType > TONUGE.HUMAN) Render.text(" and out... and out...");
				Render.text(" and then lean forward to give her a great, wet, sloppy lick, straight up the center of her pussy");
				//(E.Herm:
				if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text(" not stopping until you have slurped your way to the very tip of her cock");
				Render.text(", savoring the unmistakable taste of her intimate juices.");
				...
				if (player.tongueType > TONUGE.HUMAN) Render.text("which is quite far indeed, ");
				Render.text("caressing and stroking and playing all the tricks you can possibly think of to orally pleasure your draconic lover.  From the amount of juices beginning to seep onto your lapping tongue");
				if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text(" and dribbling down her painfully stiff dick");
				Render.text(", you think you're doing rather well.");
				...
				if (player.tongueType > TONUGE.HUMAN) Render.text("  Right at this time, you feel something round and smooth on the tip of your tongue, gently spreading Ember's walls.  Realizing that this can only be her egg, you start trying to worm your long, sinuous tongue between it and her innermost walls, hoping to coax it out of her.");
...
//[Fob Off]
		private function dontWatchEmberLayEgg():void
		{
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text(".. it even drips with some kind of off-white fluid.");
			Render.text("  \"<i>H-here's your egg.  Use it while it's fresh, okay?</i>\"  Her eyes glaze over a bit at the suggestion, and she giggles.  ");
...
//[Watch]
		private function watchMediumAffectionEmberEggLay():void
		{
			...
			//(if Ember has a cock:
			if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text(", cock poking straight out");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) Render.text(" of its hiding place");
			Render.text(".  \"<i>Okay... but you're only allowed to watch!  If you do anything weird, I swear I'll smack you!</i>\"");
			Render.text("\n\nYou promise her that you won't do anything");
			if (player.stats.lib >= 50) Render.text(" she doesn't ask you to, beaming a salacious grin");
			Render.text(".");
			...
			if (player.gender > 0) {
				Render.text("; ");
				if (player.lowerBody.cockSpot.hasCock()) {
					Render.text("the bulge inside your [armor]");
					if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and ");
				}
				if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("wetness gathering in your own pussy");
				Render.text(" more than indicate");
				if (!player.lowerBody.cockSpot.hasCock() || !player.lowerBody.vaginaSpot.hasVagina()) Render.text("s");
				Render.text(" your desire to break your promise.");
			}
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text(" and caressing her throbbing cock");
			Render.text(".");
			Render.text("\n\nWith a deep moan, Ember shakes and orgasms.");
			if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text("  Ropes of jism spew out of her cock, arching into the air to hit the ground nearby.");
			Render.text("  The egg plugs her wet pleasure hole, preventing any liquid from escaping, until finally with a wet, squelching pop,it flies out of her pussy and into your waiting hands; releasing a flood of juices.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text(" and her cock hanging out");
			Render.text(", it's hard to see her as the same angrily defensive ice queen from before.");
...
//Get Milk
		private function getMilkFromEmber():void
		{
			...
			Render.text("You think for a few moments, then find your gaze drawn to Ember's round, firm");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text(", scaly");
			Render.text(" breasts, " + emberMF("his", "her") + " perky nipples bare as always and enticing you.  With a repressed smile, you ask if " + emberMF("he", "she") + "'ll let you suckle " + emberMF("his", "her") + " milk.");
			//Summary:
			...
			if (emberAffection() <= 25) {
				...
				//noEmberkids:
				if (flags[FlagEnum.EMBER_EGGS] == 0 && emberChildren() == 0) Render.text(emberMF("he", "she") + " has no offspring to feed them with");
				//(1+ EmberEggs:
				else if (flags[FlagEnum.EMBER_EGGS] > 0) Render.text(emberMF("his", "her") + " children haven't hatched yet");
				else Render.text(emberMF("his", "her") + " children don't drink nearly enough to properly empty " + emberMF("him", "her") + "), so is " + emberMF("he", "she") + " just going to suffer with " + emberMF("his", "her") + " breasts so full and aching?");
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text("scales");
				else Render.text("skin");
				Render.text(".  " + emberMF("He", "She") + " lets out a cute gasp of pleasure, eyes darting furiously around to look at anything but you. You beam a smile and " + emberMF("he", "she") + " turns her head away in embarrassment.  \"<i>T-They're not getting any emptier like this, y'know!?</i>\"");
				...
				if (flags[FlagEnum.EMBER_GENDER] != 2) {
					...
					if (!player.isTaur()) Render.text("belly");
					else Render.text("back");
					Render.text(" and look down at Ember's engorging shaft.  Smiling around Ember's nipple, you start to gently stroke and squeeze the partially-erect draconic dick " + emberMF("he", "she") + "'s presenting.  If " + emberMF("he", "she") + "'s enjoying this so much, well, why not make it all the more fun for " + emberMF("him", "her") + "?");
				}
				//(else)
				else {
				...
				if (player.statusAffects.has("DragonBreathCooldown")) {
					...
					Render.text("  Your throat feels soothed as the scratching and soreness die down; you feel like you could shout to the mountaintops!");
				}
				//(no new PG, PC has dragon-morph status and is opposite Ember's sex:
				if (rand(2) == 0 && player.dragonScore() >= 4 && player.gender > 0 && (player.gender != flags[FlagEnum.EMBER_GENDER] || (player.gender == 3 && flags[FlagEnum.EMBER_GENDER] == 3))) {
					Render.text("  Though, a sudden swell of lust races through your ");
					if (player.lowerBody.cockSpot.hasCock()) {
						Render.text(cockDescript(0));
						if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and ");
					}
					...
					if (player.lowerBody.vaginaSpot.hasVagina() && (flags[FlagEnum.EMBER_GENDER] == 3 || flags[FlagEnum.EMBER_GENDER] == 1)) Render.text("taking that hard, spurting cock inside your own " + vaginaDescript(0));
					Render.text("... too late, you realize that <b>Ember's milk has sent your draconic body into ");
					if (player.lowerBody.cockSpot.hasCock() && flags[FlagEnum.EMBER_GENDER] >= 2) {
						Render.text("rut");
			...
			//(High Affection)
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
					Render.text("the faintest hint of " + emberMF("his", "her") + " cock ");
					if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) Render.text("emerging");
					else Render.text("hardening");
				}
				else Render.text("small beads of moisture gathering on her honeypot");
				Render.text(".  \"<i>Ok... I suppose I could use some help draining myself, but no funny moves!</i>\"");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
					Render.text(emberMF("his", "her") + " erecting dragon-prick, gently stroking its");
					if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) Render.text(" strangely ridged, almost scaly");
					Render.text(" surface");
				}
				else Render.text("her gently-dripping cunt, sliding in between the lips to stroke the wet interior");
				Render.text(".");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text("stroke " + emberMF("his", "her") + " shaft");
				else Render.text("probe gently into her slick depths");
				Render.text(".  Ember squirms and moans, humming softly, while you suckle and tease, delighting as much in the pleasure you are giving as in the delicious milk cascading down your thirsty throat.");
				...
				Render.text("  You tell your dragon that you're finished.  You're up and turning to leave when a looping tail around your waist stops you.");
...
//Winning gets you affection.
		...
		public function beatEmberSpar():void
		{
			clearOutput();
			if (emberAffection() <= 25) {
				Render.text("Ember lies on " + emberMF("his", "her") + " back, while you stand over the defeated dragon triumphantly.  You extend a helping hand, intent on pulling " + emberMF("him", "her") + " up, but " + emberMF("he", "she") + " bats it away, flinching in shame at " + emberMF("his", "her") + " defeat.");
			...
			//Medium affection
				...
				Render.text("Ember lies on " + emberMF("his", "her") + " back, while you stand over the defeated dragon triumphantly.  You extend a helping hand, intent on helping " + emberMF("him", "her") + " up; " + emberMF("he", "she") + " takes it.");
			...
			//High affection
				...
				Render.text("Ember lies on " + emberMF("his", "her") + " back, while you stand over the defeated dragon triumphantly.  You extend a helping hand, intent on helping " + emberMF("him", "her") + " up; " + emberMF("he", "she") + " gladly accepts your help.");
		...
		public function loseToEmberSpar():void
		{
			...
			//Low affection
			if (emberAffection() <= 25) {
				Render.text("Panting, you drop your fighting stance and sit on the floor, lifting a hand and saying you've had enough.");
...
//[Catch Anal] - a dragon coq up the date (Z)
		private function catchAnal():void
		{
			...
			Render.text("You contemplate Ember's somewhat dominant streak in bed and ask if " + emberMF("he", "she") + " feels in the mood to ride your ass.");
			//(Low affection)
			if (emberAffection() <= 25) {
			...
			if (emberAffection() < 75) {
				...
				if (flags[FlagEnum.EMBER_INTERNAL_DICK] == 1 || flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text(" of its hiding place");
				Render.text(", growing slowly but steadily.");
			...
			if (player.lowerBody.balls > 0) {
				Render.text(" testicles swaying lightly as you arch your backside up at " + emberMF("him", "her"));
				if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" while... ");
				else Render.text(".");
			}
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" your " + clitDescript() + " drips with every passing moment.");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) Render.text("segmented");
			else Render.text("smooth");
			Render.text(" length of " + emberMF("his", "her") + " prick between your " + buttDescript() + " cheeks, letting you feel the refreshing coolness of " + emberMF("his", "her") + " pre dripping from the tip.  \"<i>This... isn't going to work,</i>\" " + emberMF("he", "she") + " growls.");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) Render.text("mouth");
			else Render.text("maw");
			Render.text(" and sliding " + emberMF("his", "her") + " cock meaningfully against your pucker.  \"<i>You sure you want this?  Last chance to back out.</i>\"  As if you're going to turn away now, so close to filling that hunger for a nice, hot cock in your ass.  Rearing your rump up, you show " + emberMF("him", "her") + " that you're ready if " + emberMF("he", "she") + " is.");
			...
			if (player.ass.analLooseness == 0 || player.analCapacity() < 25) Render.text(", leaving both of you wincing as the sensitive head of " + emberMF("his", "her") + " cock struggles to make room for its insertion.  \"<i>I can't believe you really want me to put it here... you're so tight it almost hurts,</i>\" " + emberMF("he", "she") + " whimpers.");
			//(PC has a loose asshole:)
			if (player.analCapacity() < 50) {
				Render.text(", causing you to gasp as " + emberMF("his", "her") + " ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) Render.text("draconic ");
				Render.text("dick slips inside of you; stretching you and making you wince a bit in pain as " + emberMF("his", "her") + " spit rubs off along the interior of your anus.  \"<i>It feels pretty snug... good thing I prepared for this,</i>\" " + emberMF("he", "she") + " remarks with a gasp of pleasure.");
			}
			//(PC has a gaping asshole:)
			else {
				Render.text("; the looseness of your " + assholeDescript() + " gives " + emberMF("him", "her") + " no resistance whatsoever as " + emberMF("he", "she") + " glides in without trouble.  \"<i>Huh... that was easier than I expected,</i>\" " + emberMF("he", "she") + " remarks in surprise.  \"<i>What have you been getting yourself into?</i>\"");
			}
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) Render.text("draconic ");
			Render.text("prick in and out, reaching for your deepest recesses to rub on the right spots.");
			...
			//(PC has prick:
			if (player.lowerBody.cockSpot.hasCock()) Render.text("  You can feel the dragon's member rubbing against your prostate, making " + sMultiCockDesc() + " jump to painful erectness, bubble pre-cum, and drool it down the shaft to pool on the ground.");
			//(PC has cunt:
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				Render.text("  The stimulus is starting to make you wet as well, your [vagina] drooling feminine lubricant ");
				if (player.lowerBody.balls > 0) Render.text("over your [balls]");
				Render.text(" to puddle underneath you");
				if (player.lowerBody.cockSpot.hasCock()) Render.text(", mingling with your pre to form an ever-growing pool of mixed sexual fluids.");
			}
			...
			//(if PC has balls:
			if (player.lowerBody.balls > 0) Render.text("  " + emberMF("His", "Her") + " own balls swing into yours, and every time they connect a shrill wave of pleasure flows through you.");
			...
			if (pregnancy.event > 3) {
				if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) Render.text("swollen ");
				else Render.text("egg-laden ");
			}
			Render.text("belly, and start to rise and sink, pistoning your ass up and down. You have total control over the sex, and you intend to use that.");
			...
			if (player.gender > 0) Render.text(", ");
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("your cunt gushing feminine lubricant to glaze Ember's belly");
			if (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.cockSpot.hasCock()) Render.text(" and ");
			if (player.lowerBody.cockSpot.hasCock()) {
				Render.text("your cock ");
				if (player.cumQ() < 250) Render.text("spurting gouts of cum onto Ember's upper torso");
				else if (player.cumQ() < 1000) Render.text("raining spunk across the supine dragon");
				else Render.text("cascading your man-cream across your blissed-out lover until " + emberMF("he", "she") + " is completely glazed in a sperm coating");
			}
			Render.text(".");

			Render.text("\n\nEmber hugs you close, enjoying the afterglow");
			if (player.gender > 0) Render.text(" and rubbing the results of your own orgasm against you");
			Render.text(".  You sink readily into the dragon's embrace, rubbing against " + emberMF("him", "her") + ", feeling sleepy and blissful after a good, hard fuck.  Sadly, this moment of respite doesn't last long; you feel a certain discomfort as Ember's body heats up.  Looking up, you see " + emberMF("his", "her") + " eyes are wide open.  Before you can ask " + emberMF("him", "her") + " what's wrong, " + emberMF("he", "she") + " knocks you off with a start.");
...
//[Blow Ember] - your shipment of dragon dildoes has arrived
		private function suckEmberCock():void
		{
			...
			Render.text("You stare at Ember's crotch, imagining the taste of draconic cum on your tongue.  The thought makes you lick your lips in eagerness, and you ask if Ember would enjoy having you suck on " + emberMF("his", "her") + " cock.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1) {
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) Render.text("his single genital slit");
				else Render.text("his genitals");
			}
			else {
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) Render.text("her twin genital slits");
				else Render.text("her genitals");
			}
			Render.text(", " + emberMF("his", "her") + " ");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) Render.text("draconic ");
			Render.text("prick already hard and throbbing.  The dragon taps a clawed finger idly on the stump, awaiting your attention.");
			...
			if (player.lowerBody.cockSpot.hasCock()) Render.text("  [EachCock] hardens, spearing aimlessly into the ground in your arousal.");
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Your [vagina] starts to seep with feminine juices, your [clit] hardening in anticipation as your excitement dribbles down onto the thirsty ground.");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) {
				Render.text(" that is even now springing forth from " + emberMF("his", "her") + " ");
				if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text("first ");
				Render.text("genital slit");
			}
			Render.text(".  Up and up it rises, until all 16 inches of ");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) Render.text("dragon ");
			Render.text("dick are proudly lancing towards your face, pre-cum gently bubbling from the tip and flowing down its length as you massage and lubricate it for ease of swallowing.  You cast a coy look at the dragon, who is shivering from your ministrations, but stoically refuses to show how much you are affecting " + emberMF("him", "her") + ".  Not willing to play any more if " + emberMF("he", "she") + " isn't willing to cooperate, you open your mouth and start to swallow " + emberMF("his", "her") + " cock.");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text("scaly");
			else Render.text("soft-skinned");
			Render.text(" lower belly");
			if (pregnancy.event > 3) Render.text(", though the gravid orb resists you");
			Render.text(", and discover a surge of pride in you that you got all sixteen inches of it down.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3 && (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0)) {
				Render.text("  Struck by perverse inspiration, you manage to wriggle your tongue under the base of " + emberMF("his", "her") + " cock and thrust it into " + emberMF("his", "her") + " genital slit.  You guide it as deep into the strangely pussy-like orifice as you can, tickling and caressing.");
				//(DemonTongue:
				if (player.tongueType > TONUGE.HUMAN) Render.text("  Your inhuman length slithers deeper and deeper inside, and you realize you can feel two rounded objects; " + emberMF("his", "her") + " balls! You're actually touching the testicles that are normally locked away inside " + emberMF("his", "her") + " body, except when " + emberMF("he", "she") + " reaches " + emberMF("his", "her") + " most aroused states...");
			}
			...
			if (player.stats.cor < 80) Render.text("amused");
			else Render.text("increasingly leery of " + emberMF("his", "her") + " batty behavior");
			Render.text(", then pick yourself up to head back to the camp.");
...
//Get Blown - put your dick in the knife drawer, it'll be fun! (Z, with reservation)
		private function stickDickInKnifeDrawer():void
		{
			...
			Render.text("Trying to appear confident and a little aloof, you suggest that maybe Ember would be willing to put " + emberMF("his", "her") + " kinky tongue to work on your cock.");
			...
			if (!player.isNaga()) Render.text("seat yourself on a conveniently lichen-covered boulder.");
			else Render.text("coil up on your serpentine tail) and start peeling off the clothes on your lower half, exposing [eachCock] to the air and awaiting Ember's response.");
			...
			if (player.lowerBody.cockSpot.count() > 1) {
				Render.text("  " + emberMF("He", "She") + " selects the biggest cock and gives it a gentle stroke,");
			}
			else Render.text("  " + emberMF("He", "She") + " takes your [cock " + y + "] in hand and begins gently stroking it,");
			Render.text(" sniffing your musk with obvious delight.");

			//(if Ember chose a dragon cock)
			if (player.lowerBody.cockSpot.list[x].cockType == CockType.DRAGON) Render.text("\n\n\"<i>Such a magnificent, beautiful cock... it feels powerful... and familiar.</i>\"");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text(", the scales on " + emberMF("his", "her") + " palms producing an indescribable feeling as they slide across your sensitive flesh,");
			Render.text(" and unthinkingly blurt out your understanding.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text("  " + emberMF("His", "Her") + " shaft is already hard as rock, and sometimes you can feel it poke your " + player.feet() + ", smearing small beads of Ember's barely contained pleasure on your " + player.skinFurScales() + ".");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("  Looking behind her, you see a small pool of her feminine juices has formed; pheromones fly through the air and hit you like a wave, spurring you on, making you feed Ember more of your delicious pre.");
			...
			if (flags[FlagEnum.EMBER_HAIR] == 0 && flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text("scaly pate");
			else Render.text("hair");
			Render.text(", telling " + emberMF("him", "her") + " that " + emberMF("he", "she") + " certainly has your attention and you're looking forward to finding out what " + emberMF("he", "she") + " has in mind.");

			Render.text("\n\nEmber flicks " + emberMF("his", "her") + " tongue against your ");
			if (player.lowerBody.balls > 0) Render.text("balls");
			else if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(vaginaDescript());
			else Render.text("[cock " + y + "]");
			Render.text(" and begins licking you from base to tip in slow, almost torturous, strokes.  \"<i>Mmm.  So tasty...</i>\"  Sparks of pleasure surge up your body from the dragon's ministrations, and you find yourself biting back words because you're not sure if you want " + emberMF("him", "her") + " to go faster or to keep at it like this.");
			...
			if (player.cumQ() < 250) {
				...
				if (!player.isTaur()) Render.text("a finger");
				else Render.text("the thumb of her wing");
				Render.text(" into your ass to tickle your prostate, drawing a few last drops out of you.");
			}
			//(Medium Cum Amount)
			...
			if (player.stats.cor < 75) Render.text(" and chuckle; you know " + emberMF("he", "she") + " loves you, really.");
			else Render.text(", folding your arms; " + emberMF("his", "her") + " dementia is getting worse...");
...
//Pitch Anal - horses are terrible people, so no centaurs unless rewritten (Z)
		private function stickItInEmbersButt():void
		{
			...
			Render.text("Your eyes are drawn to Ember's butt like iron filings to a magnet.  Haunted by the temptation to see " + emberMF("him", "her") + " bent over and offering " + emberMF("his", "her") + " ass to your hungry touch, you ask if Ember would be willing to be the catcher in a bout of anal sex.");
			...
			//(High Affection)
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
					Render.text(emberMF("his", "her") + " stiffening cock");
					if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text(" and ");
				}
				if (flags[FlagEnum.EMBER_GENDER] == 2) Render.text("her ");
				if (flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("dripping pussy");
				Render.text(" make it obvious that the idea is at least partially arousing.");
			}
			//(first time and corr < 40)
			if (flags[FlagEnum.TIMES_BUTTFUCKED_EMBER] == 0) Render.text("\n\nYou ask if " + emberMF("he", "she") + "'s really so certain that it won't fit; you're eager to give it a try if " + emberMF("he", "she") + " is, but you won't push if " + emberMF("he", "she") + "'s really that scared of the idea...");
			...
			flags[FlagEnum.TIMES_BUTTFUCKED_EMBER]++;
			...
			Render.text("to the dragon.  Once you're fully undressed, you turn to look at Ember, to see " + emberMF("him", "her") + " openly masturbating; " + emberMF("he", "she") + " pants as " + emberMF("he", "she") + " strokes " + emberMF("his", "her") + " ");
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text("cock ");
			if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text("and ");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("pussy ");
			Render.text("with a hand, while " + emberMF("he", "she") + " suckles on the fingers of the other.");
			...
			//First Time
			if (flags[FlagEnum.TIMES_BUTTFUCKED_EMBER] == 1) {
			...
			Render.text("\n\nYou ");
			if (player.stats.cor < 50) Render.text("promise the dragon you understand, and insist you wouldn't dream of doing otherwise; you want this to be good for " + emberMF("him", "her") + ", not to cause " + emberMF("him", "her") + " pain.");
			else Render.text("nod.");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text("scaly ");
			Render.text("cheeks and rubbing the base of " + emberMF("his", "her") + " long, flexible tail to try and convey just how wild " + emberMF("his", "her") + " ass is driving you.");

			Render.text("\n\n\"<i>Ow!  Okay, okay!  Just stop and give me a moment to adjust at least,</i>\" Ember replies");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text(", panting with lust");
			else Render.text(", a blush adorning " + emberMF("his", "her") + " face");
			Render.text(".  You force yourself to remain still, giving the horny dragon a chance to recover from your " + cockDescript(x) + " impaling " + emberMF("his", "her") + " back passage, savoring the feeling of " + emberMF("his", "her") + " cool muscles squeezing and rippling around your shaft.");

			Render.text("\n\nAfter a few minutes, Ember finally starts to move, stroking you with " + emberMF("his", "her") + " inner muscles.  ");
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text("Droplets of pre slide along " + emberMF("his", "her") + " own shaft and down " + emberMF("his", "her") + " balls to gather at your belly.  ");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("Her slick pussy drips rhythmically, wetting your lower body with slick dragon juice.  ");
			Render.text(emberMF("His", "Her") + " lusty moans each time " + emberMF("he", "she") + " comes down on your " + cockDescript(x) + " send a thrill up your spine, and the deep purring emanating from " + emberMF("his", "her") + " chest is audible enough to give you an idea of how much Ember is enjoying this, despite " + emberMF("his", "her") + " initial reluctance.");
			...
			else Render.text("the base of your spine");
			Render.text(", and you groan to Ember that you're going to cum, soon.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text(", jet upon jet of spunk arcing over " + emberMF("his", "her") + " head to paint the ground below");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("; a veritable gush of juices spews forth from her forgotten love-hole to splash against your lower body and over herself");
			Render.text(".  The force of " + emberMF("his", "her") + " orgasm makes the dragon clench " + emberMF("his", "her") + " ass, sending waves of renewed pleasure through you.");
			...
			if (player.str < 70) {
				Render.text("desperately try to grab any of the flying pieces, but Ember keeps you pinned with " + emberMF("his", "her") + " tail");
				Render.text(".");
			...
			//(else str >=80)
				...
				Render.text("grab " + emberMF("his", "her") + " tail, pulling the dragon off " + emberMF("his", "her") + " feet and throwing " + emberMF("him", "her") + " to the ground as you roll over... and causing " + emberMF("him", "her") + " to drop them everywhere as " + emberMF("he", "she") + " falls.");
				...
				//New
				Render.text("\"<i>What do you think you're doing!?</i>\" Ember yells at you, obviously mad at your little stunt, one hand reaching behind to prevent any more cum from spilling.");
...
//Eat Ember Out - b-baka! (Z)
		private function slurpDraggieCunnies():void
		{
			...
			Render.text("You contemplate the possibilities, and then offer to get down on your knees before the dragon and pleasure her orally.  You would love to have a taste of some dragon juice.");
			...
			if (emberAffection() <= 25) {
				...
				if (!silly()) Render.text("with me");
				else Render.text("on my affection meter");
				Render.text(" just because you're offering to do this,</i>\" the dragon hastily adds.");
			}
			//(Medium Affection)
			else if (emberAffection() < 75) {
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text(" and slowly hardening shaft");
				Render.text(".");
			}
			//(High Affection)
			else {
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text(" and shaft");
				Render.text(".  \"<i>Ok... I suppose I would like that,</i>\" Ember replies, trying to calm herself down, lest she seem too excited by the idea.");
			}
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text("  With Ember's cock jutting out above your head, you can't resist sliding your tongue along the underside of her shaft, just for an extra teasing motion.");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) Render.text("blushing all the while and ");
			Render.text("panting from the stimulation.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text("  You think droplets of pre are starting to slide down her girl-cock and drip onto your head... but that's part and parcel of loving a herm, so you pay it no attention.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text(", heedless of the steady rain of pre dribbling into your " + hairDescript() + ".  You're confident she'll crack soon...");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text("  Her throbbing shaft, not intending to be left out of the fun, records the event by marking the tree bark with white dragon spooge.");
			...
			if (silly()) Render.text("  My meter's not that full!");
			Render.text("</i>\"");
			...
			if (player.lowerBody.cockSpot.hasCock()) Render.text("stiffened shaft");
			if (player.gender == 3) Render.text(" and ");
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("moist cunt");
			if (player.gender == 0) Render.text("empty crotch");
			Render.text(" in sympathy, you head back to camp.");
...
//Get Eaten Out - actually halfway likeable
		private function getEatenOutByEmbra():void
		{
			...
			Render.text("You think about Ember's long tongue and the many advantages it must have, when you suddenly get an idea of what you'd like to do. You ask Ember if " + emberMF("he", "she") + "'d be willing to put that tongue of " + emberMF("his", "hers") + " to use and eat you out.");
			//(Low Affection)
			if (emberAffection() <= 25) {
			...
			if (!player.isNaga()) Render.text(" with your [legs] spread wide");
			Render.text(", displaying your [vagina].  You tell Ember that " + emberMF("he", "she") + "'ll need to make the next move from here, smiling as you do so.");
			...
			if (player.lowerBody.cockSpot.hasCock()) Render.text(", as well as the musk emanating from your drooling cock");
			Render.text(".  Ember's eyes close as " + emberMF("he", "she") + " savors your scent, committing it to memory and licking " + emberMF("his", "her") + " lips in preparation for the task ahead.");
			...
			if (player.lowerBody.cockSpot.hasCock()) Render.text(", and pre-cum beginning to bubble out of " + sMultiCockDesc());
			Render.text(".  You wriggle in delight, praising Ember's skill with " + emberMF("his", "her") + " tongue and begging " + emberMF("him", "her") + " to keep going.");

			Render.text("\n\nEmber presses on, nudging your pleasure button with " + emberMF("his", "her") + " nose and wrapping your [vagina] with " + emberMF("his", "her") + " ");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text("maw");
			else Render.text("mouth");
			Render.text(", kissing your contracting box as if it were a long-lost lover, pushing " + emberMF("his", "her") + " tongue ever deeper inside you, as if intent on reaching your womb.");
			...
			if (player.wetness() >= 4) Render.text("spattering");
			else Render.text("drenching");
			Render.text(" Ember's face with your feminine honey");
			if (player.lowerBody.cockSpot.hasCock()) Render.text(" even as " + sMultiCockDesc() + " belches cum all over the preoccupied dragon");
			Render.text(".");
			...
			if (player.wetness() >= 4) Render.text("  However, your squirting spatters make this an impossible task.  Ember doesn't seem to care, even as " + emberMF("his", "her") + " face is completely covered in femcum.");
...
//Penetrate Her - seems not to accommodate centaurs, more's the pity (Z)
		private function penetrateEmbrah():void
		{
			...
			Render.text("Your eyes are drawn to the dragon's crotch, and you ask if she has any feminine itches she'd like scratched.");
			...
			//(High Affection)
				...
				if (flags[FlagEnum.EMBER_GENDER] == 3) {
					if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) Render.text(" and her cock peek out of its protective slit");
					else Render.text(" and her cock begin hardening");
				}
				Render.text(" in anticipation.  \"<i>Yeah... maybe... why do you ask?</i>\"");
			}
			//(First Time)
			if (flags[FlagEnum.EMBER_PUSSY_FUCK_COUNT] == 0) {
				...
				if (player.stats.cor > 75) Render.text("  As if bitches like her could qualify as maidens in any but the most literal sense.");
			...
			Render.text("\n\nYou ");
			if (player.stats.cor < 50) Render.text("hasten to undress yourself for the impatient dragoness, quickly pulling");
			else Render.text("languidly denude yourself, making sure to exaggerate every movement as you pull");
			Render.text(" off your [armor] until you are standing naked before her, letting her see what you have to offer in terms of phallic delights.");
			...
			if (player.cockThatFits(emberVaginalCapacity()) == -1) {
				...
				if (player.lowerBody.cockSpot.count() == 1) Render.text("It doesn't ");
				else Render.text("None of them ");
				Render.text("fit!</i>\"");
				...
				if (player.lowerBody.cockSpot.count() == 1) Render.text("that... that... monster inside me,");
				else Render.text("any of those... those... monsters inside me,");
				Render.text(" you'll tear me apart!</i>\"  Ember finishes by exhaling a puff of smoke in frustration... clearly she wants this as much as you.");

				Render.text("\"<i>Find some way to shrink ");
				if (player.lowerBody.cockSpot.count() == 1) Render.text("that");
				else Render.text("those");
				Render.text(" and then come back!</i>\"  Ember turns on her heels and walks away, moodier than usual.");
				doNext(camp.returnToCampUseOneHour);
				return;
			}
			...
			if (player.lowerBody.cockSpot.list[x].cockType == CockType.DRAGON) {
			...
			Render.text("smile and, with a flamboyant grace, ");
			Render.text("gently lay yourself down, spreading your limbs and making it clear you're ready for whatever she has in mind.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
				Render.text("; her ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) Render.text("draconic");
				else Render.text("humanoid");
				Render.text(" prick slaps wetly against your belly with each thrust, smearing pre on you as the engorged tool bounces back and forth");
			}
			Render.text(".");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text("; her own erect cock spurts a small jet of pre");
			Render.text(".");
			...
			if (player.cumQ() < 250) Render.text("dribble");
			else if (player.cumQ() < 1000) Render.text("flow");
			else Render.text("gush");
			Render.text(" from her used cunt and you sit on the ground, while Ember remains limp on the ground, panting. Still suffering from the afterglow of your recent orgasm, you dimly realize that Ember herself doesn't seem to have cum yet; the only fluids dripping from her pussy are yours");
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text(", and her cock, while engorged and swollen, hasn't poured forth yet");
			Render.text(".");
			...
			if (player.stats.cor >= 66) {
				...
				emberAffection(-5);
				doNext(camp.returnToCampUseOneHour);
				flags[FlagEnum.EMBER_PUSSY_FUCK_COUNT]++;
				return;
			}
			//else
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text("; her cock jumps and unloads on the ground below, marking this as the spot of your lovemaking");
			Render.text(".  Her pussy grabs at you like a vice, constricting you with such pressure that you have trouble moving, even though her cunt is well-lubed from your mixed fluids.");
			...
			//(High Cum Amount)
				...
				if (player.lowerBody.balls == 0) Render.text("prick is ");
				else Render.text("[balls] are ");
				Render.text("finished, Ember doesn't only look more than a few months pregnant; she looks absolutely huge, like her water could break at any moment!");
			}
...
//Get Penetrated - also horse-proof, sorry folks! (Z)
		private function getPenetratedByEmberLastSexSceneWoooo():void
		{
			...
			Render.text("Your eyes are drawn to the 16 inches of cool, throbbing ");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) Render.text("dragon");
			else Render.text("human");
			Render.text(" dick between Ember's legs, and you lick your lips hungrily.  Your " + vaginaDescript() + " throbs eagerly as you tell Ember you have certain itches you need to scratch... is Ember enough of a " + emberMF("man", "herm") + " to scratch them for you?");
			...
			//(High Affection)
				...
				Render.text("Ember's cock jumps ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] > 0 && flags[FlagEnum.EMBER_INTERNAL_DICK] == 0) Render.text("to attention");
				else Render.text("out of its protective slit");
				Render.text(" at the mere mention of what you expect " + emberMF("him", "her") + " to do.  \"<i>I'm more than enough to handle two... no... three of you!  And if you want proof all you have to do is ask!</i>\" Ember boasts proudly.");
			}
			...
			if (player.tongueType == TONUGE.HUMAN) Render.text("yours ");
			else Render.text("your own inhumanly sinuous muscle ");
			Render.text("and slithering almost into your throat.  " + emberMF("He", "She") + " kisses you madly, even as " + emberMF("he", "she") + " sinks to " + emberMF("his", "her") + " knees and gently lays you out on the ground, clearly ready to start the sexing.");
			if (player.lowerBody.cockSpot.hasCock()) Render.text("  As if " + emberMF("his", "her") + " hands caressing your cock weren't evidence of that.");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) Render.text("draconic ");
			else Render.text("human-like ");
			Render.text("cock begins making its way past your labia.");

			//(if Ember has a dragon cock)
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) {
				...
				Render.text("Finally you arrive at the base of " + emberMF("his", "her") + " cock where a bulbous knot sits; thankfully it hasn't inflated yet, but still, you can't help but groan as you are stretched even more by its entrance.");
			}
			//else
			else {
				...
				Render.text("  It's not as if it isn't still warm... but its lower temperature feels relieving and exciting all the same.");
			}
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				Render.text("  Your cock explodes, belching cum to ");
				if (player.cumQ() < 250) Render.text("spatter");
				else if (player.cumQ() < 1000) Render.text("drench");
				else if (player.cumQ() < 2000) Render.text("completely cover");
				else Render.text("utterly soak");
				Render.text(" your draconic partner.");
			}
			...
			if (player.perks.has("HistoryWhore") || player.perks.has("HistorySlut")) Render.text("  Using the skills you've honed, you make the muscles in your vagina ripple and wrinkle, teasing the cock caught inside you in a way few women can.");
			Render.text("  Mmm... you can feel your own second orgasm coming in hot.  Maybe after another 4 or 5, you'll let the dragon go...");
			...
			player.orgasm();
			dynStats("sen", -2);
			if (player.stats.lib > 50) dynStats("lib", -3);
			player.slimeFeed();
...
//Part II
		private function getPenetratedByEmberLastSexSceneWooooPartII():void
		{
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) Render.text("protective slit");
			else Render.text("limp cock and balls");
			Render.text(".  \"<i>You used me like a fucktoy!</i>\"");
			...
			if (emberAffection() < 75) Render.text(emberMF("his", "her") + " cock was unlike anything else in this world?");
			else Render.text(emberMF("he", "she") + " could handle two or even three of you at the same time?");
			Render.text("  You expected more resistance if that was the case.");
...
//Ember must not be pregnant if you're going to breed her and the PC must not be pregnant to be bred.
		...
		public function emberRapesYourHeatness():void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina() && player.inHeat && (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3))
				Render.text("your [vagina] aching to be filled and your womb put to its proper purpose");
			else Render.text("[eachCock] burning to be seeding ripe, ready wombs");
			Render.text(".");

			Render.text("\n\nEmber responds by ");
			if (player.lowerBody.vaginaSpot.hasVagina() && player.inHeat && (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3))
				Render.text("humping against you, smearing your thigh with dragon pre.");
			else Render.text("tightly pressing her drooling, puffy netherlips on your thighs.");
			Render.text("  \"<i>I need you, [name].  I need you so badly... can you see how badly I need you?</i>\" Ember asks, panting in barely contained lust.  \"<i>I want to fuck you so badly... Let's make a baby now!</i>\"");
			dynStats("lus", 10 + player.stats.lib / 10);
			Render.text("\n\nWhat do you say?");
			//[Accept] [Deny]
...
//[=Deny=]
		private function fuckOffEmberIWantANap():void
		{
			...
			Render.text(" so yearn");
			if (player.gender == 1 || player.gender == 2) Render.text("s");
			Render.text(" to take Ember up on " + emberMF("his", "her") + " offer! ...But you are better than this; you are not some mindless animal, a slave to your lusts.  You fight down the arousal gnawing at you and tell Ember you don't want to fuck " + emberMF("him", "her") + ".");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text("protruding prick");
			if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text(" and ");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("creaming wet vagina");
			Render.text(".  No doubt " + emberMF("he", "she") + "'s off to take care of " + emberMF("his", "her") + " needs by " + emberMF("him", "her") + "self.  You doubt " + emberMF("he", "she") + "'s going to be happy once " + emberMF("his", "her") + " head is clear... still, " + emberMF("he", "she") + "'ll get over it.");
			emberAffection(-10);
...
//[=Accept=]
		private function timeToPuffTheMagicDragon():void
		{
			...
			if (player.pregnancyIncubation == 0 && !pregnancy.isPregnant && flags[FlagEnum.EMBER_GENDER] == 3 && player.gender == 3) {
			...
			//Female Breeding Scene:
			//PC not pregnant, Ember has dick, PC is in heat.
			else if (player.pregnancyIncubation == 0 && (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) && player.inHeat) {
...
//Ember never fails to impregnate the PC or be impregnated - unless the player is on contraceptives.
		private function getKnockedUpByEmbrahBroBaby():void
		{
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 && flags[FlagEnum.EMBER_MILK] == 0) Render.text("his flat, muscular chest");
			else Render.text(emberMF("his", "her") + " soft, squeezable boobs");
			Render.text(" and savoring the kiss.  Emboldened, you poke your tongue into " + emberMF("his", "her") + " mouth, to see how " + emberMF("he", "she") + " will react.");
			...
			if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) Render.text("little dragons");
			else Render.text("dragon eggs");
			Render.text(", my beautiful.</i>\"  Ember resumes " + emberMF("his", "her") + " kissing; " + emberMF("his", "her") + " own hands roaming all over your body, as if mapping you out through touch, looking for the best places to touch and tease.");
			...
			if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) Render.text("dragonlings");
			else Render.text("dragon eggs");
			Render.text(".  You want to breed!");
			...
			Render.text("  You groan throatily, your need burning through you, and start trying to grind yourself against Ember's shaft.  " + emberMF("He", "She") + " pins you down, though, making it impossible for you to actually do anything more than rub " + emberMF("his", "her") + " glans against your needy netherlips.");
			...
			//(if Ember has a vagina)
			if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text("\n\nYour bedroll is growing slick underneath you as lubricant drools from the herm dragon's neglected cunt, her tail thrashing around madly before sliding into the wet orifice to help goad her on in her goal to breed you.  ");
			else Render.text("\n\n");
			Render.text("You growl throatily, feeling that same need burning inside you. You clutch and claw against the dragon, fingers grasping deeply into " + emberMF("his", "her") + " scaly limbs as you drag, scrape, and thrust yourself against " + emberMF("him", "her"));
			if (player.lowerBody.cockSpot.hasCock()) Render.text(", your neglected [cock] drooling pre-cum that is being smeared all across your belly and " + emberMF("his", "hers") + " with every move you make");
			Render.text(".  Gods, you can feel " + emberMF("him", "her") + " filling you...");
			...
			Render.text("Your contracting walls milk the dragon's sensitive cock, trying to coax even more seed out of your panting dragon lover.  \"<i>I-it's time,</i>\" Ember mutters, crashing into your wet bedroll and thrusting deep inside you.");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) Render.text("blushing ");
			Render.text("face.  \"<i>But if you must know, I do love you.</i>\"   With a final sigh Ember sets down on your bedroll for a quick nap.");
			...
			if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) Render.text("baby dragon");
			else Render.text("dragon egg");
			Render.text(".  Yawning, you curl up to the dragon for a quick nap of your own.");
			player.orgasm();
			dynStats("sen", -2);
...
//Breeding Ember
		private function breedEmberPregnantAsIfThereWasAnyOtherKindOfBreeding():void
		{
			...
			if (x < 0) x = player.smallestCockIndex();
			Render.text("Ember catches you and rolls you around, pinning you to the ground under her.  She smiles at you seductively and reaches down to stroke your " + cockDescript(x) + ".");
			if (player.lowerBody.balls > 0) Render.text("  She then reaches down lower to rub at your cum filled orbs.");
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("  The tip of her tail gently teases your slick [vagina] with tiny strokes.");
			Render.text("  You shudder and instinctively thrust towards her, letting her feel your needy cock, full and aching to father young.");
			...
			if (emberHasCock()) Render.text(", her own cock springing up and throbbing with the electric contact.");
			else Render.text(".");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text("scale-covered ");
			Render.text("orbs gently squish ");
			if (flags[FlagEnum.EMBER_MILK] > 0) Render.text("and leak milk ");
			Render.text("under your grasp.");
			...
			if (flags[FlagEnum.EMBER_MILK] > 0) Render.text("the slowly trickling rivulets of milk along with ");
			Render.text("Ember's pleased moans are enough to make you push away such thoughts, you're glad she has those!");

			Render.text("\n\nYou hungrily kiss her ");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text("draconic ");
			else Render.text("oh-so-human ");
			Render.text("face, nipping lightly at her lip, rolling your tongue over hers and savoring her inhuman tastes; Ember answers in kind with a kiss of her own, purring and moaning into your mouth.  Her vibrating breasts massage your hands as you knead, lift, and then roll them about.");

			//(if Ember has a dick)
			if (emberHasCock()) {
				Render.text("\n\nYou feel something poke your [chest] and look down to see Ember's painfully erect ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) Render.text("dragon ");
				Render.text("prick.  With a mischievous curl of your lips, you pull Ember deeper into your kiss and grab her cock, stroking it vigorously.");
			}
			...
			if (emberHasCock()) Render.text("her cock spurts a few jets of cum onto your [chest]; ");
			Render.text("her eyes roll to the back of her head.  With a muffled roar she finally caves, a veritable wave of dragon juices crashing into your lower body and pooling on your bedroll, filling the whole tent with the delicious smell of sex and pheromones.");

			//(If Ember has a dick)
			if (emberHasCock()) Render.text("  Ember's cock wets the side of the tent with continuous jets of cum, only adding to the already overpowering smell of sex as she empties herself on top of you.");
			...
			if (emberHasCock()) Render.text("her spent cock throb; ");
			Render.text("her eyes shut tight in pleasure as she reaches her second orgasm.  You muffle her groan of pleasure with one of your own.  A second wave of dragon fluids joins the first in wetting your lower body, along with your bedroll.");
			if (emberHasCock()) Render.text("  Little gobs of cum spill from her cock to slide down her shaft and gather on your belly button.");
			Render.text("  Her tail coils around you, helping her stay in position as she melts on top of you like putty.");
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("your neglected cunt splashing fluid down your [legs] and ");
			Render.text("your " + cockDescript(x) + " gushing into the dragon's ready womb.");
			...
			//(Low Cum Amount)
			if (player.cumQ() < 250) Render.text("  Though you usually don't cum that much, Ember's contracting walls literally suck the cum out of you; forcing you to give up more than you usually would... something you're quite glad to do!");
			//(Medium Cum Amount)
			else if (player.cumQ() < 1000) Render.text("  Your generous helping of cum works its way towards her awaiting womb.  In fact, due to the delicious massage your shaft is receiving from Ember's contracting, almost sucking vaginal walls, you find yourself pumping out even more cum that you would usually have, quickly filling her to the brim and beyond.  It's not until Ember's belly is slightly distended that you stop.");
			//(High Cum Amount)
			else Render.text("  You dump a huge load of jism into Ember's awaiting belly, but her contracting walls seem to milk you for ever more!  Ember is filled to the brim, her belly already slightly distended, but you still continue to fill her with your seed.  Ember's tightly sealed pussy lips means the cum has nowhere to go, so instead it gathers in her womb, distending her belly until she looks at least a few months pregnant; and by the look of pleasure on Ember's panting face, you don't think she minds it.");

			Render.text("\n\nWith a gasp and a sigh of utter relief, you slump into Ember's arms, collapsing onto her ");
			if (emberHasCock()) Render.text("cum-slick ");
			Render.text("belly and laying there to regain your strength, ");
			if (emberHasCock()) Render.text("dismissive of the spent cock lying sandwiched between you as you start ");
			Render.text("feeling the heat in her gut that marks the beginning of a child.");
...
//Bred/Breeding Aftermath
		private function emberBreedingAfterMathWatchOutForRadioactiveFallout(emberPregged:boolean = true):void
		{
			...
			Render.text("\n\nEmber's cheeks ");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text(" are probably blushing under " + emberMF("his", "her") + " scales.");
			else Render.text("blush.");
			Render.text("  \"<i>You're welcome, but this is the only time I'll clean up after you!  I'm not your personal maid!</i>\"");
			...
			if (!emberPregged) Render.text("touch your belly");
			else Render.text("point at Ember's belly");
			Render.text(", and you ask the dragon if " + emberMF("he", "she") + " thinks it \"<i>took</i>\".");

			Render.text("\n\nEmber puffs " + emberMF("his", "her") + " chest and proudly boasts, \"<i>Of course it did!  We're both very virile!  And after all the cum you ");
			if (emberPregged) Render.text("pumped into me...");
			else Render.text("had me pump into you...");
			Render.text("</i>\" Ember trails off, and you can see " + emberMF("his", "her") + " hand move to " + emberMF("his", "her") + " crotch to caress " + emberMF("his", "her") + " ");
			if (emberPregged) Render.text("slit");
			else Render.text("cock");
			Render.text(", as if remembering the intense sex session you two just had.");
			...
			Render.text("\n\nEmber's ");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) Render.text("blush");
			else Render.text("discomfort");
			Render.text(" deepens, and " + emberMF("he", "she") + " frowns at you.  \"<i>A-are you questioning my virility!?  Well, if you want, I can go again, right now!</i>\"");
		...
		private function emberPregUpdate():boolean
		{
			...
			switch (pregnancy.eventTriggered()) {
				case 1: //
						...
						return true;
				case 2: if (flags[FlagEnum.EMBER_OVIPOSITION] > 0) {
				...
				case 4: Render.text("\nEmber's belly has grown quite a bit.  Anyone can tell she's pregnant with a single glance.  ");
						if (flags[FlagEnum.EMBER_OVIPOSITION] > 0) {
							Render.text("Ember notices you looking.  \"<i>W-what? Never seen a pregnant woman before?</i>\" she asks indignantly, although she can't hide her smile as you continue to look.\n");
						}
						else {
							Render.text("Ember catches you looking" + (flags[FlagEnum.EMBER_ROUNDFACE] == 1 ? " and blushes" : "") + ".  \"<i>W-what is it?</i>\"  You reply merely that she carries a baby bump very well; she looks good pregnant.  \"<i>Oh, uh...  Thanks I guess?</i>\" she replies, looking away and flicking her tongue out nervously.\n");
						}
						return true;
				case 5: if (flags[FlagEnum.EMBER_OVIPOSITION] > 0) {
				...
				case 6: if (flags[FlagEnum.EMBER_OVIPOSITION] > 0) {
						...
						return true;
				case 7: if (flags[FlagEnum.EMBER_OVIPOSITION] > 0) {
		...
		public function emberGivesBirth():void
		{
			...
			if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) {
				...
				Render.text(" as her womb works to expel its long-time occupant.  You place a hand on either side of the distended orb and begin to massage it, trying to help soothe the strained, aching muscles.  Your eyes go from her stomach to her crotch; her vagina is opened wide as the infant inside her begins making its way into the world.");
				if (emberHasCock()) {
					if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 && flags[FlagEnum.EMBER_INTERNAL_DICK] == 0) Render.text("  Her human-like cock dangles heavily in front of her pussy, partially erect from the muscular spasms.");
					else Render.text("  Her draconic cock has been pushed from its internal sheathe by the pressure, but it's too painful for her to be erect.");
				}
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text("n anthro dragon");
				else Render.text(" hybrid of human and dragon");
				Render.text(".  It's a beautiful strong, healthy little ");
				let roll: number = rand(100);
				if (roll < 40) Render.text("boy");
				else if (roll < 80) Render.text("girl");
				else Render.text("herm");
				Render.text(".");

				Render.text("\n\nYou cradle ");
				if (roll < 40) Render.text("him");
				else Render.text("her");
				Render.text(" in your arms, uncaring of the stains as the fluid drenching its little body soaks into your [armor], gently soothing ");
				if (roll < 40) Render.text("him");
				else Render.text("her");
				Render.text(".  Then, seeing Ember is recovering from her recent exertions, you grin proudly at her and gently hold your ");
				if (roll < 40) Render.text("son");
				else Render.text("daughter");
				Render.text(" out for her to take.");
				...
				if (emberChildren() > 1) Render.text("  Your other draconic offspring quietly peek into the lair, having vacated the place when Ember went into labor.  They smile at the sight of their new sibling, gently slipping in to curl up beside the mother and new child.");
				Render.text("  With a contented expression on your face, you leave Ember and your ");
				if (emberChildren() <= 1) Render.text("new child");
				else Render.text("newly expanded draconic brood");
				Render.text(" to get some rest, leaving the den.\n");
				if (roll < 40) flags[FlagEnum.EMBER_CHILDREN_MALES]++;
				else if (roll < 80) flags[FlagEnum.EMBER_CHILDREN_FEMALES]++;
				else flags[FlagEnum.EMBER_CHILDREN_HERMS]++;
			}
			//Ember Lays Egg
			else {
				...
				Render.text(" as her womb works to expel its long-time occupant.  You place a hand on either side of the distended orb and begin to massage it, trying to help soothe the strained, aching muscles.  Your eyes go from her stomach to her crotch; her vagina is opened wide as the globular egg inside her begins dropping its way into the world.");
				if (emberHasCock()) {
					if (flags[FlagEnum.EMBER_ROUNDFACE] > 0 && flags[FlagEnum.EMBER_INTERNAL_DICK] == 0) Render.text("  Her human cock dangles heavily in front of her pussy, partially erect from the muscular spasms.");
					else Render.text("  Her draconic cock has been pushed from its internal sheathe by the pressure, but it's too painful for her to be erect.");
				}
				Render.text("\n\nMind racing, you bend your head in close and stick out your tongue, sliding it back in and out of your mouth to get it nice and wet before delivering a long, sloppy lick to her inner walls.  You can taste her, and the strange salty-blood taste of amniotic fluid mingles with her natural lubricants.  It's an unusual taste but not unbearable, and you begin to lick with greater fervor and purpose.  Your intention is to try and drown the pain of her contractions with pleasurable stimulus.  It's a crazy idea, but it makes sense in a place like Mareth.");
				...
				if (flags[FlagEnum.EMBER_EGGS] > 0) {
					Render.text(", along with the ");
					if (flags[FlagEnum.EMBER_EGGS] == 1) Render.text("other");
					else Render.text(num2Text(flags[FlagEnum.EMBER_EGGS]) + " others");
				}
				Render.text(", before telling her that she's beautiful too, and your daughters will certainly hatch and grow up to be just as gorgeous as she.");
		...
		public function giveBirthToEmberKids():void
		{
			...
			if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) {
				...
				if (player.lowerBody.balls > 0) Render.text("slowly pushing your [balls]");
				else if (player.lowerBody.cockSpot.hasCock()) Render.text("slowly pushing your " + multiCockDescriptLight());
				if (player.lowerBody.balls > 0 || player.lowerBody.cockSpot.hasCock()) Render.text(" out of " + emberMF("his", "her") + " way, ");
				Render.text("then " + emberMF("he", "she") + " blows softly on your contracting love-hole, slowly extending " + emberMF("his", "her") + " tongue to penetrate you.");
				...
				if (player.perks.has("Masochist")) Render.text("deliciously ");
				Render.text("on the boundary between pleasure and pain.  You aren't sure how much more you can take.");
				...
				if (roll < 40) Render.text("He");
				else Render.text("She");
				Render.text(" was getting hungry and I didn't know what to do,</i>\" Ember explains.");
				if (flags[FlagEnum.EMBER_MILK] > 0) {
					Render.text("  \"<i>I tried to feed ");
					if (roll < 40) Render.text("him");
					else Render.text("her");
					Render.text(" myself, but ");
					if (roll < 40) Render.text("he");
					else Render.text("she");
					Render.text(" wanted yours...</i>\"");
				}
				...
				Render.text(" face and starts to cry softly.  You gently help ");
				if (roll < 40) Render.text("him");
				else Render.text("her");
				Render.text(" up onto your shoulder and gently pat ");
				if (roll < 40) Render.text("him");
				else Render.text("her");
				Render.text(" on the back between ");
				if (roll < 40) Render.text("his");
				else Render.text("her");
				Render.text(" little wings, eliciting a burp that clearly leaves ");
				if (roll < 40) Render.text("him");
				else Render.text("her");
				Render.text(" feeling better.  ");
				if (roll < 40) Render.text("He");
				else Render.text("She");
				Render.text(" coos, giggles and nuzzles into your neck, clearly happy to be here in the real world at last.");
			...
			//PC Lays Egg
				...
				Render.text("\n\nEmber ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) Render.text("blushes and ");
				Render.text("withdraws, licking " + emberMF("his", "her") + " lips of a stray droplet of milk.  \"<i>Sorry, it's just that you looked so full, and all that milk would've been wasted...  So, I thought I could help myself, not that I've been wanting to drink your milk or anything like that.</i>\"");
				...
				if (flags[FlagEnum.EMBER_EGGS] > 0) Render.text("s hatch");
				else Render.text(" hatches");
				Render.text(", anyway.  The dragon ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) Render.text("blushes and then ");
				Render.text("scurries away, even as you pull yourself upright and get ready to go about your business.\n");
				flags[FlagEnum.EMBER_EGGS]++;
			}
...
//Note: This scene is meant for Tainted Ember after you've been through the lost dragon city dungeon. While we do not have the dungeon and post-quest Ember, this scene may be accessed from regular Ember's pool of scenes if her affection is High.
		...
		private function highAffectionEmberLustFuck():void
		{
			...
			let x: number = player.cockThatFits(emberVaginalCapacity());
			if (x < 0) x = player.smallestCockIndex();
			Render.text("You strip your [armor] and watch Ember as " + emberMF("he", "she") + " appraises your naked body.  You can see " + emberMF("his", "her"));
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
				Render.text(" dragon cock ");
				if (emberInternalDick()) Render.text("poking out of " + emberMF("his", "her") + " protective slit");
				else Render.text("growing erect");
			}
			if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text(" and");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) Render.text(" her pussy beginning to drip in excitement");
			Render.text(".");

			Render.text("\n\n\"<i>Mmm... now that's a view and a half; just look at you... what a gorgeous " + player.mf("man", "girl") + ", with such ");
			if (player.lowerBody.cockSpot.count() == 1) Render.text("a wonderful " + cockDescript(0));
			else Render.text(" wonderful cocks");
			Render.text("... and you're all mine, here and now,</i>\" " + emberMF("he", "she") + " croons appreciatively, giving you a lewd wink and flick of " + emberMF("his", "her") + " tongue.  \"<i>Still, while I'm enjoying the view, don't keep a " + emberMF("guy", "girl") + " in suspense; what do you have planned?</i>\" " + emberMF("he", "she") + " asks, tail flicking from side to side in an amused manner.");
			...
			if (flags[FlagEnum.TIMES_EMBER_LUSTY_FUCKED] == 0) {
				...
				Render.text("\"<i>All of them,</i>\" you state.");
				...
				Render.text("\n\nYou nod");
				if (player.stats.lib < 50 && player.minLust() < 40) {
					Render.text(", grabbing a vial of Lust Draft from your pouches");
					player.consumeItem(consumables.L_DRAFT);
				}
				Render.text(".");
			}
			flags[FlagEnum.TIMES_EMBER_LUSTY_FUCKED]++;
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1) {
				...
				Render.text("\n\nYour ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text("she-dragon");
				else Render.text("dragon-girl");
				Render.text(" moans throatily and gives you a rapturous look.  \"<i>I love you, [name]... gods, I love you so much, and I'm so happy I can finally say that to your face,</i>\" she tells you.");
			...
			//else if Ember has a pussy and the PC more than one cock:
				...
				Render.text(" of those?  I-I know it's not exactly part of the pose and all, but...</i>\"");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text(", not to mention the stiffness of her dick");
				Render.text(", making it obvious she's ready.  \"<i>Oh, my mate... Please, fill me now!  Quit teasing me and just shove your cocks up your dragon's ready holes, jam them in as far as they can fit!</i>\" she pleads, but unable to meet your eyes in her embarrassment at her dirty talk.");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text(", cock dribbling precum onto her belly");
				Render.text(".");

				Render.text("\n\nThe feeling of having ");
				if (player.lowerBody.cockSpot.count() == 2) Render.text("both ");
				else Render.text("two of ");
				Render.text("your cocks enveloped in slick tightness and warmth is almost enough to make you fill her up with your seed right then and there, but somehow you manage to hold on.");

				Render.text("\n\nYour ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text("she-dragon");
				else Render.text("dragon-girl");
				Render.text(" moans throatily and gives you a rapturous look.  \"<i>I love you, [name]... gods, I love you so much, and I'm so happy I can finally say that to your face,</i>\" she tells you.  Then she starts looking less in love and more in lust.  \"<i>But I also just love these cocks; these two wonderful breeding rods - come on, mate!  Breed your horny dragon!</i>\" she commands lustfully.");
			}
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2 && player.lowerBody.cockSpot.count() > 1) Render.text(" and ass");
			Render.text(" rippling and squeezing your intruding member");
			if (flags[FlagEnum.EMBER_GENDER] >= 2 && player.lowerBody.cockSpot.count() > 1) Render.text("s");
			Render.text(".");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("vagina");
			else Render.text("ass");
			Render.text(" of " + emberMF("his", "hers") + " the pounding it deserves.");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2) {
				Render.text(" slick pussy");
				if (player.lowerBody.cockSpot.count() > 1) Render.text(" and tight ass");
			}
			else Render.text(" tight ass");
			Render.text(" and blow your load.");

			Render.text("\n\nEmber howls exultantly as your steaming load gushes inside of " + emberMF("him", "her") + ".  " + emberMF("His", "Her") + " ");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) {
				Render.text("cunt shudders, splashing femcum all over your intruding shaft");
				if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text(", and her ");
				else Render.text(", ");
			}
			if (flags[FlagEnum.EMBER_GENDER] == 3 || flags[FlagEnum.EMBER_GENDER] == 1) {
				Render.text("cock erupts, spraying draconic cum up into the air to rain back down upon " + emberMF("his", "her") + " body, spattering " + emberMF("him", "her") + " in " + emberMF("his", "her") + " own juices and ");
			}
			Render.text("leaving " + emberMF("him", "her") + " gasping for breath as the climax ebbs away.");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2 && player.lowerBody.cockSpot.count() > 1) Render.text("s");
			Render.text(", letting a satisfying stream of white mat the leaves below.  You smile at Ember, shaft");
			if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
			Render.text(" still flagging above " + emberMF("him", "her") + ".");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1) Render.text("ass");
			else if (player.lowerBody.cockSpot.count() == 1) Render.text("pussy");
			else Render.text("holes");
			Render.text(" with a squelch.  It feels so good... taking " + emberMF("him", "her") + " one time after the other.");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 1 && player.lowerBody.cockSpot.count() > 1) Render.text("s");
			Render.text(".  The dragon lets out an exultant cry as " + emberMF("his", "her") + " cum-slimed ");
			if (flags[FlagEnum.EMBER_GENDER] == 1) Render.text("ass squeezes");
			else if (player.lowerBody.cockSpot.count() == 1) Render.text("pussy squeezes");
			else Render.text("holes squeeze");
			Render.text(" you, milking your ");
			if (flags[FlagEnum.EMBER_GENDER] == 1) Render.text("shaft as his cock spurts dragon-seed onto the leaves below him, filling the air with the scent of spunk and matting them into a steaming morass.");
			//{twin shafts} as {her cunt spasms wetly, drenching your lap with fresh femcum} {and/or} {[her] cock spurts dragon-seed onto the leaves below {her}, filling the air with the scent of spunk and matting them into a steaming morass}
				...
				if (player.lowerBody.cockSpot.count() == 1) Render.text("shaft as her cunt spasms wetly, drenching your lap with fresh femcum");
				else Render.text("twin shafts as her cunt spasms wetly, drenching your lap with fresh femcum");
				if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text(" and her cock spurts dragon seed onto the leaves below her, filling the air with the scent of spunk and matting them into a steaming morass");
				Render.text(".");
			}
			Render.text("  Groaning and moaning like a whore, the dragon's wings beat, sending  cool air wafting over your overheating bodies before " + emberMF("he", "she") + " slumps onto the ground, barely able to hold " + emberMF("him", "her") + "self upright.  \"<i>...So good...</i>\"  Ember pants.  \"<i>I... uh... are you sure you want... round three?</i>\" " + emberMF("he", "she") + " asks, sounding a little tired as " + emberMF("he", "she") + " does, obviously not sure if you'll manage it.");
			...
			//Else:
				...
				if (flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("her cunt starts to drip with moisture");
				if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text(" and ");
				if (flags[FlagEnum.EMBER_GENDER] == 3 || flags[FlagEnum.EMBER_GENDER] == 1) Render.text(emberMF("his", "her") + " cock begins to grow erect again");
				Render.text(", amazement giving way to arousal.  " + emberMF("He", "She") + " licks " + emberMF("his", "her") + " lips with " + emberMF("his", "her") + " inhuman tongue, clearly eager to begin again.");
			}

			Render.text("\n\nEmber crawls over towards you, gripping the base of your shaft");
			if (player.lowerBody.cockSpot.count() > 1 && flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("s");
			Render.text(" tenderly yet firmly, stroking you slowly.  \"<i>I can't believe how hot I get when I see you sporting ");
			if (player.lowerBody.cockSpot.count() == 1) Render.text("this");
			else Render.text("these");
			Render.text(".  It's just so... intoxicating... your scent, the way you look at me, everything really.</i>\"  You pat Ember's head");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 1 || flags[FlagEnum.EMBER_HAIR] > 0) {
				Render.text(", ruffling " + emberMF("his ", "her "));
				if (flags[FlagEnum.EMBER_HAIR] >= 2) Render.text("mane");
				else Render.text("hair");
			}
			Render.text(".  " + emberMF("He", "She") + " leans against your hand, hugging your midriff and letting your shaft");
			if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
			Render.text(" brush against " + emberMF("his", "her") + " cheek.");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 0) Render.text("mingled ");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1) Render.text("depths");
			else Render.text("nethers");
			if (flags[FlagEnum.EMBER_GENDER] >= 2 && player.lowerBody.cockSpot.count() > 1) Render.text(" and depths");
			Render.text(".  \"<i>Ahn.  D-deeper...</i>\"  You hook your arm under " + emberMF("his", "her") + " knee and pull " + emberMF("his", "her") + " leg up, nearly throwing the dragon off balance.  \"<i>Ah!  D-do you like it when I let you take charge?  Ugh- oh!  Well, I think maybe I kind of like letting you be in charge, too...</i>\"  You barely pay attention to " + emberMF("his", "her") + " teasing remarks, instead focusing on exploring " + emberMF("his", "her") + " cummy ");
			if (flags[FlagEnum.EMBER_GENDER] == 1) Render.text("ass");
			else if (player.lowerBody.cockSpot.count() == 1) Render.text("pussy");
			else Render.text("love-holes");
			Render.text(".  \"<i>Uhn... yes...  take me again.</i>\"  " + emberMF("He", "She") + " lets " + emberMF("his", "her") + " tongue loll out as " + emberMF("he", "she") + " pants in pleasure.");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2 && player.lowerBody.cockSpot.count() > 1) Render.text("both of your dicks");
			else Render.text("your dick");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) Render.text(", moisture drenching the cock buried in her cunt as it slobbers greedily across your burning flesh.");
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text("  " + emberMF("His", "Her") + " cock throbs and pulsates, ripples of arousal giving way to cumvein-bulging jets that shoot from " + emberMF("his", "her") + " prick to splatter wetly against the wall.");
			Render.text("  Wobbling unsteadily, the dragon becomes increasingly dependent on you to hold " + emberMF("him", "her") + " upright - a climax is coming, and it looks like it's going to be big...");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
				Render.text(emberMF("his", "her") + " cock fountaining seed across the wall, practically whitewashing it in steaming hot dragon-spunk");
			}
			if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text(" and ");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("her cunt gushing female ejaculate, almost soaking you with the cascade of feminine fluids and puddling wetly against your [feet]");
			Render.text(".");
			...
			Render.text(", feeling the excess slide out around your cock");
			if (flags[FlagEnum.EMBER_GENDER] >= 2 && player.lowerBody.cockSpot.count() > 1) Render.text("s");
			Render.text(".  \"<i>Ahh... more seed from my lover...</i>\" " + emberMF("he", "she") + " whispers, nearly passing out from the pleasure.  You feel yourself grow dizzy with pleasure and tumble onto the soft grass covering the den's floor, bringing Ember along.  Your dick");
			if (flags[FlagEnum.EMBER_GENDER] >= 2 && player.lowerBody.cockSpot.count() > 1) Render.text("s");
			Render.text(" slide");
			if (player.lowerBody.cockSpot.count() == 1) Render.text("s");
			Render.text(" out of " + emberMF("his", "her") + " with a wet slurp, allowing your deposit to leak its way under your prone forms.");
...
//Fatigue stays gained, whereupon it's lost if PC stays and rests? (Sure!)
		private function noStayingForCuddlesPostLustFuck():void
		{
			...
			if (player.stats.cor < 50) Render.text("thank her for being so understanding");
			else Render.text("grunt an acknowledgement");
			Render.text(" and then gather your things before heading off to wash yourself down.");
			doNext(camp.returnToCampUseOneHour);
		}
...
//[=Yes=]
		private function stayWithEmberAfterLustFuck():void
		{
			...
			let x: number = player.cockThatFits(emberVaginalCapacity());
			if (x < 0) x = player.smallestCockIndex();
			Render.text("With a smile, you tell " + emberMF("him", "her") + " that you'd be happy to.  \"<i>Great, come here...</i>\" " + emberMF("he", "she") + " croons, scooting over towards you.  You open your arms and allow the dragon to snuggle up against you, folding your arms comfortably under " + emberMF("his", "her") + " wings.");
			...
			//else if Ember is female:
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) Render.text("-girl");
				Render.text(" states, idly rubbing your cocks together.  One of your hands slips down between the two of you, sliding over the ridged surface of the herm dragon's cock before pressing itself against her well-used quim; sure enough, it's already drooling eagerly at the prospect of being filled again.");
...
//Frotting:
		private function frottingWithEmber(clear:boolean = true):void
		{
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) Render.text(emberMF("-boy", "-girl"));
			Render.text("'s overly long tongue sticks out as pleasure overrides " + emberMF("his", "her") + " senses.  You groan as you slide your cock against Ember's, the thought occurring to you that this is going to be pretty messy when the inevitable comes, but you are too overwhelmed to actually say so.");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("  Her moist dragon pussy is not helping the mess in the least.  You can smell her femcum pooling along with her pre.");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2) Render.text("  Her other hand frigs her pussy as she comes down from her orgasm, gushing female juices and leaking some of the cum you've pumped into her earlier.");

			Render.text("\n\nThe dragon");
			if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) Render.text(emberMF("-boy", "-girl"));
			Render.text(" uses what strength " + emberMF("he", "she") + " still has to crawl over to you and slide back between your arms.  \"<i>Sorry, just had to get that last bit out of my system.</i>\"  " + emberMF("He", "She") + " yawns lowdly.  \"<i>[name]?  How about a quick nap?</i>\"  " + emberMF("He", "She") + " asks not even bothering to hear your reply before exhaustion gets " + emberMF("his", "her") + " and " + emberMF("he", "she") + " falls asleep, snoring lightly.  You smile at " + emberMF("him", "her") + " and stroke " + emberMF("his", "her"));
			if (flags[FlagEnum.EMBER_HAIR] == 0 && flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text(" head");
			else if (flags[FlagEnum.EMBER_HAIR] == 2) Render.text(" mane");
			else Render.text(" hair");
			Render.text(" before allowing yourself to also fall asleep.");
			player.orgasm();
			//doNext(14);
			menu();
...
//Penetrate:
		private function penetrateWithEmber(clear:boolean = true):void
		{
			...
			let x: number = player.cockThatFits(emberVaginalCapacity());
			if (x < 0) x = player.smallestCockIndex();
			Render.text("\"<i>Go on.</i>\"  She moves her arms around you and into a hug.  \"<i>Enter me.</i>\"  With no further prelude needed, you slide yourself into the damp interior of her cunt, the organ eagerly accepting you back for the fourth time.");
			...
			if (flags[FlagEnum.EMBER_MILK] > 0) Render.text(", milk seeping across your fingers,");
			Render.text(" and promise you will, sliding slowly in until you have hilted yourself yet again.");

			Render.text("\n\nThe dragon");
			if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) Render.text("-girl");
			Render.text(" moans at your ministrations");
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
				Render.text(", her draconic prick pressing tightly against you as it leaks a steady stream of pre, matting ");
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) Render.text("your [chest], ");
				Render.text("your belly");
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) Render.text(",");
				Render.text(" and Ember's own.");
			}
			Render.text("  \"<i>Do you like playing with my body?</i>\"  Ember asks, gazing at you through half-lidded eyes.  You give her a playful smirk and run your fingers expertly over her nipples, making her moan and mewl at the stimulation and saying more than mere words could on the matter.");
			...
			if (player.tailType > TAIL.NONE) Render.text("the base of your tail, tugging lightly on it and stroking it for a moment, then her hands move on to ");
			Render.text("your [butt], grabbing the ");
			...
			if (player.upperBody.wingType > WING.NONE) Render.text("stopping momentarily to stroke along your " + player.upperBody.wingDesc + " wings before continuing up and ");
			Render.text("stopping at the back of your head.");

			Render.text("\n\nSlowly she guides you towards one of her erect nipples.  \"<i>");
			if (flags[FlagEnum.EMBER_MILK] > 0) Render.text("Drink my milk, I've been saving it up specially for you,");
			else Render.text("I may not have any milk in my breasts right now, but maybe if you keep suckling I'll be able to make you some,");
			Render.text("</i>\" Ember teases you.  You smile at her and accept the nipple, rolling it between your lips and exerting gentle, teasing pressure with your teeth.");
			if (flags[FlagEnum.EMBER_MILK] > 0) Render.text("  You savor the resultant gush of sweet dragon's milk as it squirts obediently down your throat.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) Render.text("  A small dollop of pre escapes Ember's dragon-dick at the pleasure of the movement.");
			Render.text("  \"<i>And don't forget about my other breast either.</i>\"  She takes your hand in her own and guide it to her other mound, helping you knead it.");
			...
			if (flags[FlagEnum.EMBER_MILK] > 0) Render.text("allowing milk to spill into your mouth and gulp down sweet load after load, ");
			Render.text("listening to her moan softly in pleasure from your actions, feeling her netherlips rippling around your " + cockDescript(x) + ".  Your nursing is suddenly interrupted when you're pulled off Ember's breasts and into a deep kiss.  The dragon-girl shoves her tongue inside your mouth, exploring it in detail");
			if (flags[FlagEnum.EMBER_MILK] > 0) Render.text(", not even caring that she's tasting her own milk as you finish gulping it down");
			Render.text(".  You kiss her back as eagerly as you can until lack of air forces you to break the kiss and catch your breath.");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) Render.text("-girl");
			Render.text(" roaring towards the ceiling, then turning to look at you as her pussy constricts and milks you for all you're worth.  Already spent from your past three orgasms, you just sit back and enjoy yourself being milked by her strong vaginal muscles one last time.");
			...
			Render.text(".");
			if (flags[FlagEnum.EMBER_GENDER] == 3) Render.text("  Her draconic cock throbs all the way through your orgasm, shooting blanks a few times before spurting a couple ropes of pre onto her belly.");
...
//Frotting and Penetrate connect here.
		private function emberJizzbangbangEnding():void
		{
			...
			Render.text(", letting you see the whiteness on " + emberMF("his", "her") + " tongue, before tipping " + emberMF("his", "her") + " head back and gulping it down, licking " + emberMF("his", "her") + " lips and moaning as if " + emberMF("he", "she") + " was tasting a fine wine.  \"<i>Thanks for the snack, my mate.</i>\" " + emberMF("He", "She") + " croons at you, a teasing expression on " + emberMF("his", "her") + " face.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) Render.text("anthro");
				else Render.text("monster-" + emberMF("boy", "girl"));
				Render.text(" to come clean.  \"<i>Cut me some slack... I'm not made of stone, you know.  I was just giving my mate a blowjob, you can't seriously expect me not to react to that...</i>\"  " + emberMF("He", "She") + " crosses " + emberMF("his", "her") + " arms and blows a puff of smoke as " + emberMF("he", "she") + " looks away, obviously flustered.");
