		...
		private function emberSexMenu(output:boolean = true):void
		{
			if (output) {
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
					...
					if (flags[FlagEnum.EMBER_INTERNAL_DICK] == 1 || flags[FlagEnum.EMBER_ROUNDFACE] == 0) outputText("is poking out of " + emberMF("his", "her") + " slit");
					else outputText("is starting to swell with blood");
					outputText(".  ");
				}
				if (flags[FlagEnum.EMBER_GENDER] >= 2) outputText("You can see tiny rivulets of moisture starting to run down Ember's inner thighs as they rub together, barely hiding her precious treasure from your hungry eyes.  ");
		...
		public function emberEggInteraction():void
		{
			...
			//(If the egg Corruption level is 75-100, aka \"<i>Corrupt</i>\")
				...
				if (player.gender == 0) outputText("cum, if you had any... ");
					...
					if (player.lowerBody.cockSpot.hasCock()) outputText("white ");
...
//Genderless Version (Z)
		private function masturbateOntoAnEgg():void
		{
			...
			if (player.gender == 1) {
				...
				if (player.lowerBody.cockSpot.count() <= 2) outputText("pumping vigorously.");
			...
			//Female Version (Z)
				...
				if (player.lowerBody.cockSpot.count() == 1) outputText("your [cock]");
...
//HATCH DAT BITCH
		private function hatchZeMuzzles():void
		{
			...
			if (flags[FlagEnum.EMBER_GENDER] == 0) outputText("white ");
			else if (flags[FlagEnum.EMBER_GENDER] == 1) outputText("blue ");
			else if (flags[FlagEnum.EMBER_GENDER] == 2) outputText("pink ");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) {
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1) {
					...
					if (flags[FlagEnum.EMBER_MILK] > 0) outputText(", drooling nipples notwithstanding");
					outputText(".");
				}
				//Female Anthro (Z)
				else if (flags[FlagEnum.EMBER_GENDER] == 2) {
			...
			//Boring version
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1) {
					...
					if (flags[FlagEnum.EMBER_MILK] > 0 || flags[FlagEnum.EMBER_HAIR] > 0) outputText(", complete with ");
					if (flags[FlagEnum.EMBER_MILK] > 0) {
						outputText("huge breasts, easily F-cups");
						if (flags[FlagEnum.EMBER_HAIR] > 0) outputText(" and ");
					}
					if (flags[FlagEnum.EMBER_HAIR] > 0) outputText("long, feminine locks of hair");
					outputText(", but his maleness is undeniable.  Especially when you spot ");
					if (flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) outputText("the slit in his pelvis that disgorges a foot and a half-long inhuman penis");
				...
				//Herm Monstergirl (Z)
					...
					if (flags[FlagEnum.EMBER_INTERNAL_DICK] == 0) outputText("huge, human prick");
...
//Appearance (shows Ember's appearance, always available)
		private function embersAppearance():void
		{
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) {
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1) outputText(" not overly muscled.");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1) {
					...
					if (flags[FlagEnum.EMBER_MILK] > 0) outputText(" dribbling breasts,");
				...
				if (flags[FlagEnum.EMBER_HAIR] == 1) {
					if (flags[FlagEnum.EMBER_GENDER] == 1) outputText("  Short ");
				...
				outputText("  The ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) outputText("little ");
				...
				switch (pregnancy.event) {
					...
					case 6: outputText("  Her large pregnant belly has forced Ember to change her posture.  It looks like she has a big golden bullseye painted on her stomach.  Ember looks tired but happy.");
							break;
					case 7: outputText("  Her swollen belly is as large as that of any pregnant woman you can remember from Ingnam and you " + (flags[FlagEnum.EMBER_OVIPOSITION] > 0 ? "can easily make out the egg's outline under her golden skin" : "occasionally see movement as the baby shifts in Ember's womb") + ".");
							break;
					case 8:
					case 9: outputText("  Ember's golden belly is stretched taut by the large " + (flags[FlagEnum.EMBER_OVIPOSITION] > 0 ? "egg" : "baby") + " in her womb.  You're sure she'll give birth very soon, there just isn't room for the " + (flags[FlagEnum.EMBER_OVIPOSITION] > 0 ? "egg" : "baby") + " to grow any larger.");
							break;
					default:
				}
				...
				if (flags[FlagEnum.EMBER_MILK] > 0 || flags[FlagEnum.EMBER_GENDER] >= 2) outputText("F-cup soft, pillowy breasts");
				...
				if (flags[FlagEnum.EMBER_MILK] > 0 || flags[FlagEnum.EMBER_GENDER] >= 2) outputText("generous mounds");
			...
			//Dragon-girl Appearance (By Radar) (Z)
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1) outputText("male");
				else if (flags[FlagEnum.EMBER_GENDER] == 2) outputText("female");
				...
				//Short:
				if (flags[FlagEnum.EMBER_HAIR] < 1) outputText("is quite short, giving " + emberMF("him", "her") + " that definitive " + emberMF("masculine", "tomboy") + " look.");
				...
				if (flags[FlagEnum.EMBER_MILK] > 0 || flags[FlagEnum.EMBER_GENDER] >= 2) outputText("F-cup breasts that support 0.5 inch nipples and hang heavily; you idly wonder if " + emberMF("he", "she") + "'ll develop lower back problems as she spends more time in Mareth");
				...
				//Ember has a pussy:
				if (flags[FlagEnum.EMBER_GENDER] >= 2) outputText("  The inviting lips of a human-looking pussy purse between her legs; some moisture seems to have gathered on her labia, giving it a slick look that just begs for attention.");
...
//Talk about Dragons (Z)
		private function talkToEmberAboutDragonzzz():void
		{
			...
			if (choice == 0) {
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) outputText(", a blush creeping onto " + emberMF("his", "her") + " cheeks");
				outputText(".");
		...
		private function discussExplorationWithEmber():void
		{
			...
			if (choice == 0) {
				...
				//(If player has impregnated Tamani)
					...
					if (flags[FlagEnum.EMBER_GENDER] >= 2) outputText("  You should have more pride than to let some brainless cunt like that have her way with you!");
				}
				...
				if (subChoice == 0) {
					...
					if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(" like me");
					outputText(". They were on the beach, fucking each other's brains out.");
				}
				else {
					//(If Whitney's Farm is not yet in the Places menu)
					if (rand(3) == 0 && player.statusAffects.get("MetWhitney").value1 <= 1) outputText(" a big farm on the shoreline.  I saw some sort of cow-woman tending to fields of peppers, and a dog-woman herding cattle. They looked pretty fat and tasty, too... What?  I meant the cattle, stupid!  And I wouldn't have taken them - it's no fun snatching up livestock.  I prefer to chase down a deer or a wild auroch or something like that.");
					//(if Fetish Cult is encounterable)
					else if (rand(3) == 0 && player.statusAffects.has("FetishOn")) outputText("a pair of humans, arguing about sex.  They both wanted it, but the guy said he wanted to do the 'naughty schoolboy and female teacher' routine, while the girl wanted to do the 'noblewoman and love-servant' routine.  Weird; their clothes kept changing back and forth as they argued.");
...
//Talk about Yourself (Z)
		private function talkToEmberAboutYourself():void
		{
			...
			//(If PC Int < 50)
			if (player.stats.int < 50) outputText("\"<i>You're still very naive... anyone or anything could trick you at any time...</i>\"  Ember frowns.");
			//(If PC Int < 90)
			...
			//(If PC Spd < 50)
			if (player.stats.spe < 50) outputText("You try to block the pebble, but you're not quick enough.  It hits your belly, not hard enough to hurt.  \"<i>You need to work on your reactions; anything could surprise you out there and you'd be helpless.</i>\"");
			//(If PC Spd < 90)
...
//To be implimented once preggers is set up.
		private function emberTalksToPCAboutPCDragoNPregnancy():void
		{
			...
			outputText("\n\nYou ");
			if (player.stats.lib < 40) outputText("start at this revelation, then ");
...
//Occurs once during Ember's first pregnancy.
		private function emberIsPregnantFirstTimeTalkScene():void
		{
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) outputText(" and blushes");
			...
			if (player.stats.cor >= 66) outputText("  You almost choke on those cheesy words; still, she'll never put out for you if you don't butter her up.");
			...
			if (player.stats.cor >= 66) outputText("  Is she seriously buying this nonsense?  Your smile would be miles wide if you let your lip curl even a bit, so you keep a straight face with due dilligence.");
			...
			if (player.stats.cor < 30) outputText("  Given her attitude and nature, you know how lucky you are to have someone like her want to be your mate.");
...
//counter triggered when PC starts an hour in camp at 100 Lust, output when reaching 10 counters
		public function emberBitchesAtYouAboutLustiness():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				outputText("your " + num2Text(player.lowerBody.cockSpot.count()) + " stiff as iron");
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" and ");
			}
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("your " + vaginaDescript() + " slick and wet with moisture, ready to fuck");
...
//Normal note for PC getting over mino cum addiction plays first
		public function emberGetOverFreakingOutAboutMinoJizz():void
		{
			...
			//(High Affection)
				...
				//(Male/HermEmber:
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText(", especially for sucking cock,");
...
//PCs that are dragony enough might be bestowed with Tainted Ember's signature breath weapon.
		private function bloodForTheBloodGod():void
		{
			...
			//(Medium Affection)
				...
				if (player.stats.cor < 50) outputText(", expressing your gratitude; gently, you kiss " + emberMF("his", "her") + " fingers, then the cut, letting its cool, iron-tinted tang roll across your lips and tongue.  You carefully lick, trying to avoid causing pain as you drink that which " + emberMF("he", "she") + " has so generously offered you.");
...
//TF messages (Z)
		private function emberTFs():void
		{
			...
			if (changes < changeLimit && player.dragonCocks() < player.totalCocks() && rand(3) == 0) {
				...
				if (player.stats.sens >= 50) outputText("however, it's not until you press on your new, sensitive knot that you manage to blow your load and enjoy the last few spasms of pleasure as it finally finishes changing.");
			...
			if (player.dragonScore() >= 4 && rand(3) == 0 && player.gender > 0) {
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript());
				...
				if (player.lowerBody.cockSpot.hasCock() && flags[FlagEnum.EMBER_GENDER] >= 2) {
					...
					if (player.lowerBody.vaginaSpot.hasVagina() && flags[FlagEnum.EMBER_GENDER] == 3) outputText(" even while ");
				}
				if (player.lowerBody.vaginaSpot.hasVagina() && (flags[FlagEnum.EMBER_GENDER] == 3 || flags[FlagEnum.EMBER_GENDER] == 1)) outputText("taking that hard, spurting cock inside your own " + vaginaDescript(0));
...
//Player always gets the egg.
		private function emberIsAnEggFactory():void
		{
			...
			if (emberAffection() <= 25) {
				...
				if (player.stats.cor >= 40) outputText("or sell it for quick cash ");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
					outputText(" and her ");
					if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) outputText("dragon-dick jutting from its slit,");
				...
				//[(if Ember has a dick)
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText(" and her throbbing cock is soft enough to hide");
			...
			//(Medium Affection)
				...
				outputText("\n\nEmber gasps");
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) outputText(" and her cheeks grow red with embarrassment");
			...
			//(High Affection)
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText(", and her cock points you accusingly");
				outputText(".");
				...
				if (player.tongueType > TONUGE.HUMAN) outputText(" and out... and out...");
				outputText(" and then lean forward to give her a great, wet, sloppy lick, straight up the center of her pussy");
				//(E.Herm:
				if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(" not stopping until you have slurped your way to the very tip of her cock");
				...
				if (player.tongueType > TONUGE.HUMAN) outputText("which is quite far indeed, ");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(" and dribbling down her painfully stiff dick");
				...
				if (player.tongueType > TONUGE.HUMAN) outputText("  Right at this time, you feel something round and smooth on the tip of your tongue, gently spreading Ember's walls.  Realizing that this can only be her egg, you start trying to worm your long, sinuous tongue between it and her innermost walls, hoping to coax it out of her.");
...
//[Fob Off]
		private function dontWatchEmberLayEgg():void
		{
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(".. it even drips with some kind of off-white fluid.");
...
//[Watch]
		private function watchMediumAffectionEmberEggLay():void
		{
			...
			//(if Ember has a cock:
			if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(", cock poking straight out");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) outputText(" of its hiding place");
			...
			if (player.stats.lib >= 50) outputText(" she doesn't ask you to, beaming a salacious grin");
			outputText(".");
			...
			if (player.gender > 0) {
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText("wetness gathering in your own pussy");
				outputText(" more than indicate");
				if (!player.lowerBody.cockSpot.hasCock() || !player.lowerBody.vaginaSpot.hasVagina()) outputText("s");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(" and caressing her throbbing cock");
			outputText(".");
			outputText("\n\nWith a deep moan, Ember shakes and orgasms.");
			if (flags[FlagEnum.EMBER_GENDER] == 3) outputText("  Ropes of jism spew out of her cock, arching into the air to hit the ground nearby.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(" and her cock hanging out");
...
//Get Milk
		private function getMilkFromEmber():void
		{
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) outputText(", scaly");
			...
			if (emberAffection() <= 25) {
				...
				//noEmberkids:
				if (flags[FlagEnum.EMBER_EGGS] == 0 && emberChildren() == 0) outputText(emberMF("he", "she") + " has no offspring to feed them with");
				//(1+ EmberEggs:
				else if (flags[FlagEnum.EMBER_EGGS] > 0) outputText(emberMF("his", "her") + " children haven't hatched yet");
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) outputText("scales");
			...
			//(Medium Affection)
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) outputText("flesh");
			...
			//(High Affection)
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
					...
					if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) outputText("emerging");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
					...
					if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) outputText(" strangely ridged, almost scaly");
					outputText(" surface");
				}
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText("stroke " + emberMF("his", "her") + " shaft");
...
//[Catch Anal] - a dragon coq up the date (Z)
		private function catchAnal():void
		{
			...
			if (emberAffection() < 75) {
				...
				if (flags[FlagEnum.EMBER_INTERNAL_DICK] == 1 || flags[FlagEnum.EMBER_ROUNDFACE] == 0) outputText(" of its hiding place");
			...
			if (player.lowerBody.balls > 0) {
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" while... ");
				else outputText(".");
			}
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" your " + clitDescript() + " drips with every passing moment.");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) outputText("segmented");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) outputText("mouth");
			else outputText("maw");
			...
			if (player.analCapacity() < 50) {
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) outputText("draconic ");
			...
			//(PC has prick:
			if (player.lowerBody.cockSpot.hasCock()) outputText("  You can feel the dragon's member rubbing against your prostate, making " + sMultiCockDesc() + " jump to painful erectness, bubble pre-cum, and drool it down the shaft to pool on the ground.");
			//(PC has cunt:
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				outputText("  The stimulus is starting to make you wet as well, your [vagina] drooling feminine lubricant ");
				if (player.lowerBody.balls > 0) outputText("over your [balls]");
				outputText(" to puddle underneath you");
				if (player.lowerBody.cockSpot.hasCock()) outputText(", mingling with your pre to form an ever-growing pool of mixed sexual fluids.");
			}
			...
			//(if PC has balls:
			if (player.lowerBody.balls > 0) outputText("  " + emberMF("His", "Her") + " own balls swing into yours, and every time they connect a shrill wave of pleasure flows through you.");
			...
			if (pregnancy.event > 3) {
				if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) outputText("swollen ");
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("your cunt gushing feminine lubricant to glaze Ember's belly");
			if (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.cockSpot.hasCock()) outputText(" and ");
			if (player.lowerBody.cockSpot.hasCock()) {
...
//[Blow Ember] - your shipment of dragon dildoes has arrived
		private function suckEmberCock():void
		{
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1) {
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) outputText("his single genital slit");
				else outputText("his genitals");
			}
			else {
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) outputText("her twin genital slits");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("  [EachCock] hardens, spearing aimlessly into the ground in your arousal.");
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your [vagina] starts to seep with feminine juices, your [clit] hardening in anticipation as your excitement dribbles down onto the thirsty ground.");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) {
				...
				if (flags[FlagEnum.EMBER_GENDER] == 3) outputText("first ");
				outputText("genital slit");
			}
			outputText(".  Up and up it rises, until all 16 inches of ");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) outputText("dragon ");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(", your tongue wriggling around in " + emberMF("his", "her") + " slit");
			...
			if (player.stats.cor < 80) outputText("amused");
...
//Get Blown - put your dick in the knife drawer, it'll be fun! (Z, with reservation)
		private function stickDickInKnifeDrawer():void
		{
			...
			if (player.lowerBody.cockSpot.list[x].cockType == CockType.DRAGON) outputText("\n\n\"<i>Such a magnificent, beautiful cock... it feels powerful... and familiar.</i>\"");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) outputText(", the scales on " + emberMF("his", "her") + " palms producing an indescribable feeling as they slide across your sensitive flesh,");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText("  " + emberMF("His", "Her") + " shaft is already hard as rock, and sometimes you can feel it poke your " + player.feet() + ", smearing small beads of Ember's barely contained pleasure on your " + player.skinFurScales() + ".");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) outputText("  Looking behind her, you see a small pool of her feminine juices has formed; pheromones fly through the air and hit you like a wave, spurring you on, making you feed Ember more of your delicious pre.");
			...
			if (flags[FlagEnum.EMBER_HAIR] == 0 && flags[FlagEnum.EMBER_ROUNDFACE] == 0) outputText("scaly pate");
			...
			else if (player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript());
			...
			if (player.stats.cor < 75) outputText(" and chuckle; you know " + emberMF("he", "she") + " loves you, really.");
...
//Pitch Anal - horses are terrible people, so no centaurs unless rewritten (Z)
		private function stickItInEmbersButt():void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("and [vagina] ");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText("cock ");
			if (flags[FlagEnum.EMBER_GENDER] == 3) outputText("and ");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) outputText("pussy ");
			...
			outputText("\n\nYou ");
			if (player.stats.cor < 50) outputText("promise the dragon you understand, and insist you wouldn't dream of doing otherwise; you want this to be good for " + emberMF("him", "her") + ", not to cause " + emberMF("him", "her") + " pain.");
			else outputText("nod.");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) outputText(", panting with lust");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText("Droplets of pre slide along " + emberMF("his", "her") + " own shaft and down " + emberMF("his", "her") + " balls to gather at your belly.  ");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) outputText("Her slick pussy drips rhythmically, wetting your lower body with slick dragon juice.  ");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText(", jet upon jet of spunk arcing over " + emberMF("his", "her") + " head to paint the ground below");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) outputText("; a veritable gush of juices spews forth from her forgotten love-hole to splash against your lower body and over herself");
...
//Eat Ember Out - b-baka! (Z)
		private function slurpDraggieCunnies():void
		{
			...
			//(Medium Affection)
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText(" and slowly hardening shaft");
				outputText(".");
			}
			//(High Affection)
			else {
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText(" and shaft");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText("  With Ember's cock jutting out above your head, you can't resist sliding your tongue along the underside of her shaft, just for an extra teasing motion.");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) outputText("blushing all the while and ");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText("  You think droplets of pre are starting to slide down her girl-cock and drip onto your head... but that's part and parcel of loving a herm, so you pay it no attention.");
			...
			if (player.tongueType > TONUGE.HUMAN) outputText("inhuman ");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText(", heedless of the steady rain of pre dribbling into your " + hairDescript() + ".  You're confident she'll crack soon...");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText("  Her throbbing shaft, not intending to be left out of the fun, records the event by marking the tree bark with white dragon spooge.");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("stiffened shaft");
			if (player.gender == 3) outputText(" and ");
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("moist cunt");
...
//Get Eaten Out - actually halfway likeable
		private function getEatenOutByEmbra():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText(", as well as the musk emanating from your drooling cock");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText(", and pre-cum beginning to bubble out of " + sMultiCockDesc());
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) outputText("maw");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText(" even as " + sMultiCockDesc() + " belches cum all over the preoccupied dragon");
			outputText(".");
		...
		private function penetrateEmbrah():void
		{
			...
			//(High Affection)
				...
				if (flags[FlagEnum.EMBER_GENDER] == 3) {
					if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) outputText(" and her cock peek out of its protective slit");
			...
			if (flags[FlagEnum.EMBER_PUSSY_FUCK_COUNT] == 0) {
				...
				if (player.stats.cor > 75) outputText("  As if bitches like her could qualify as maidens in any but the most literal sense.");
				...
				if (player.stats.cor <= 75) outputText("cute... but very troublesome");
			...
			outputText("\n\nYou ");
			if (player.stats.cor < 50) outputText("hasten to undress yourself for the impatient dragoness, quickly pulling");
			...
			if (player.cockThatFits(emberVaginalCapacity()) == -1) {
				...
				if (player.lowerBody.cockSpot.count() == 1) outputText("It doesn't ");
				...
				if (player.lowerBody.cockSpot.count() == 1) outputText("that... that... monster inside me,");
				...
				if (player.lowerBody.cockSpot.count() == 1) outputText("that");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
				outputText("; her ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) outputText("draconic");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText("; her own erect cock spurts a small jet of pre");
			outputText(".");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText(", and her cock, while engorged and swollen, hasn't poured forth yet");
			outputText(".");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) outputText("; her cock jumps and unloads on the ground below, marking this as the spot of your lovemaking");
			...
			//(High Cum Amount)
				...
				if (player.lowerBody.balls == 0) outputText("prick is ");
...
//Get Penetrated - also horse-proof, sorry folks! (Z)
		private function getPenetratedByEmberLastSexSceneWoooo():void
		{
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) outputText("dragon");
			...
			//(High Affection)
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] > 0 && flags[FlagEnum.EMBER_INTERNAL_DICK] == 0) outputText("to attention");
			...
			if (player.tongueType == TONUGE.HUMAN) outputText("yours ");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("  As if " + emberMF("his", "her") + " hands caressing your cock weren't evidence of that.");
			...
			if (player.perks.has("HistoryWhore") || player.perks.has("HistorySlut")) outputText("  Using the skills you've honed, you make the muscles in your vagina ripple and wrinkle, teasing the cock caught inside you in a way few women can.");
...
//Part II
		private function getPenetratedByEmberLastSexSceneWooooPartII():void
		{
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) outputText("protective slit");
...
//[=Deny=]
		private function fuckOffEmberIWantANap():void
		{
			...
			if (player.gender == 3) outputText(" and ");
			if (player.lowerBody.cockSpot.hasCock()) outputText("[eachCock]");
			outputText(" so yearn");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText("protruding prick");
			if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(" and ");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) outputText("creaming wet vagina");
...
//Ember never fails to impregnate the PC or be impregnated - unless the player is on contraceptives.
		private function getKnockedUpByEmbrahBroBaby():void
		{
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 && flags[FlagEnum.EMBER_MILK] == 0) outputText("his flat, muscular chest");
			...
			if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) outputText("little dragons");
			...
			if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) outputText("dragonlings");
			else outputText("dragon eggs");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText(", your neglected [cock] drooling pre-cum that is being smeared all across your belly and " + emberMF("his", "hers") + " with every move you make");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) outputText("blushing ");
			...
			if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) outputText("baby dragon");
...
//Breeding Ember
		private function breedEmberPregnantAsIfThereWasAnyOtherKindOfBreeding():void
		{
			...
			if (player.lowerBody.balls > 0) outputText("  She then reaches down lower to rub at your cum filled orbs.");
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("  The tip of her tail gently teases your slick [vagina] with tiny strokes.");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) outputText("scale-covered ");
			outputText("orbs gently squish ");
			if (flags[FlagEnum.EMBER_MILK] > 0) outputText("and leak milk ");
			...
			if (flags[FlagEnum.EMBER_MILK] > 0) outputText("the slowly trickling rivulets of milk along with ");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) outputText("draconic ");
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("your neglected cunt splashing fluid down your [legs] and ");
...
//Bred/Breeding Aftermath
		private function emberBreedingAfterMathWatchOutForRadioactiveFallout(emberPregged:boolean = true):void
		{
			...
			outputText("\n\nEmber's cheeks ");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) outputText(" are probably blushing under " + emberMF("his", "her") + " scales.");
			...
			outputText("\n\nEmber's ");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) outputText("blush");
			...
			outputText("\n\nEmber ");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) outputText("blushes");
			...
			switch (pregnancy.eventTriggered()) {
				...
				case 4: outputText("\nEmber's belly has grown quite a bit.  Anyone can tell she's pregnant with a single glance.  ");
						if (flags[FlagEnum.EMBER_OVIPOSITION] > 0) {
							...
							outputText("Ember catches you looking" + (flags[FlagEnum.EMBER_ROUNDFACE] == 1 ? " and blushes" : "") + ".  \"<i>W-what is it?</i>\"  You reply merely that she carries a baby bump very well; she looks good pregnant.  \"<i>Oh, uh...  Thanks I guess?</i>\" she replies, looking away and flicking her tongue out nervously.\n");
						}
						return true;
		...
		public function emberGivesBirth():void
		{
			...
			if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) {
				...
				if (emberHasCock()) {
					if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 && flags[FlagEnum.EMBER_INTERNAL_DICK] == 0) outputText("  Her human-like cock dangles heavily in front of her pussy, partially erect from the muscular spasms.");
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) outputText("head crowning");
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) outputText("n anthro dragon");
			...
			//Ember Lays Egg
				...
				if (emberHasCock()) {
					if (flags[FlagEnum.EMBER_ROUNDFACE] > 0 && flags[FlagEnum.EMBER_INTERNAL_DICK] == 0) outputText("  Her human cock dangles heavily in front of her pussy, partially erect from the muscular spasms.");
					else outputText("  Her draconic cock has been pushed from its internal sheathe by the pressure, but it's too painful for her to be erect.");
				}
				outputText("\n\nMind racing, you bend your head in close and stick out your tongue, sliding it back in and out of your mouth to get it nice and wet before delivering a long, sloppy lick to her inner walls.  You can taste her, and the strange salty-blood taste of amniotic fluid mingles with her natural lubricants.  It's an unusual taste but not unbearable, and you begin to lick with greater fervor and purpose.  Your intention is to try and drown the pain of her contractions with pleasurable stimulus.  It's a crazy idea, but it makes sense in a place like Mareth.");
				...
				if (flags[FlagEnum.EMBER_EGGS] > 0) {
					...
					if (flags[FlagEnum.EMBER_EGGS] == 1) outputText("other");
					else outputText(num2Text(flags[FlagEnum.EMBER_EGGS]) + " others");
				}
		...
		public function giveBirthToEmberKids():void
		{
			...
			if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) {
				...
				if (player.lowerBody.balls > 0) outputText("slowly pushing your [balls]");
				else if (player.lowerBody.cockSpot.hasCock()) outputText("slowly pushing your " + multiCockDescriptLight());
				if (player.lowerBody.balls > 0 || player.lowerBody.cockSpot.hasCock()) outputText(" out of " + emberMF("his", "her") + " way, ");
				...
				if (player.perks.has("Masochist")) outputText("deliciously ");
			...
			//PC Lays Egg
				...
				outputText("\n\nEmber ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) outputText("blushes and ");
				...
				if (flags[FlagEnum.EMBER_EGGS] > 0) outputText("s hatch");
				else outputText(" hatches");
				outputText(", anyway.  The dragon ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) outputText("blushes and then ");
...
//Note: This scene is meant for Tainted Ember after you've been through the lost dragon city dungeon. While we do not have the dungeon and post-quest Ember, this scene may be accessed from regular Ember's pool of scenes if her affection is High.
		...
		private function highAffectionEmberLustFuck():void
		{
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(" and");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) outputText(" her pussy beginning to drip in excitement");
			outputText(".");

			outputText("\n\n\"<i>Mmm... now that's a view and a half; just look at you... what a gorgeous " + player.mf("man", "girl") + ", with such ");
			if (player.lowerBody.cockSpot.count() == 1) outputText("a wonderful " + cockDescript(0));
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText("stiffness of " + emberMF("his", "her") + " cock");
			if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(" and ");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) outputText("the slickness of her pussy");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1) {
				...
				outputText("\n\nYour ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) outputText("she-dragon");
			...
			//else if Ember has a pussy and the PC more than one cock:
				...
				if (player.lowerBody.cockSpot.count() == 2) outputText("both");
				else outputText("two");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(", not to mention the stiffness of her dick");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(", cock dribbling precum onto her belly");
				outputText(".");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2 && player.lowerBody.cockSpot.count() > 1) outputText(" and ass");
			outputText(" rippling and squeezing your intruding member");
			if (flags[FlagEnum.EMBER_GENDER] >= 2 && player.lowerBody.cockSpot.count() > 1) outputText("s");
			outputText(".");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2) outputText("vagina");
			else outputText("ass");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2) {
				...
				if (player.lowerBody.cockSpot.count() > 1) outputText(" and tight ass");
			}
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2) {
				...
				if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(", and her ");
				else outputText(", ");
			}
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1) outputText("ass");
			else if (player.lowerBody.cockSpot.count() == 1) outputText("pussy");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 1 && player.lowerBody.cockSpot.count() > 1) outputText("s");
			outputText(".  The dragon lets out an exultant cry as " + emberMF("his", "her") + " cum-slimed ");
			if (flags[FlagEnum.EMBER_GENDER] == 1) outputText("ass squeezes");
			else if (player.lowerBody.cockSpot.count() == 1) outputText("pussy squeezes");
			else outputText("holes squeeze");
			outputText(" you, milking your ");
			if (flags[FlagEnum.EMBER_GENDER] == 1) outputText("shaft as his cock spurts dragon-seed onto the leaves below him, filling the air with the scent of spunk and matting them into a steaming morass.");
			//{twin shafts} as {her cunt spasms wetly, drenching your lap with fresh femcum} {and/or} {[her] cock spurts dragon-seed onto the leaves below {her}, filling the air with the scent of spunk and matting them into a steaming morass}
				...
				if (player.lowerBody.cockSpot.count() == 1) outputText("shaft as her cunt spasms wetly, drenching your lap with fresh femcum");
				else outputText("twin shafts as her cunt spasms wetly, drenching your lap with fresh femcum");
				if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(" and her cock spurts dragon seed onto the leaves below her, filling the air with the scent of spunk and matting them into a steaming morass");
				outputText(".");
			}
			...
			if (player.stats.lib >= 50 || player.minLust() >= 40) {
				...
				if (flags[FlagEnum.EMBER_GENDER] >= 2) outputText("her cunt starting to drip");
				if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(" and ");
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText(emberMF("his", "her") + " cock growing hard again");
			...
			//Else:
				...
				if (flags[FlagEnum.EMBER_GENDER] >= 2) outputText("her cunt starts to drip with moisture");
				if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(" and ");
				if (flags[FlagEnum.EMBER_GENDER] == 3 || flags[FlagEnum.EMBER_GENDER] == 1) outputText(emberMF("his", "her") + " cock begins to grow erect again");
			...
			if (player.lowerBody.cockSpot.count() > 1 && flags[FlagEnum.EMBER_GENDER] >= 2) outputText("s");
			outputText(" tenderly yet firmly, stroking you slowly.  \"<i>I can't believe how hot I get when I see you sporting ");
			if (player.lowerBody.cockSpot.count() == 1) outputText("this");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 1 || flags[FlagEnum.EMBER_HAIR] > 0) {
				...
				if (flags[FlagEnum.EMBER_HAIR] >= 2) outputText("mane");
				else outputText("hair");
			}
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 0) outputText("mingled ");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1) outputText("depths");
			else outputText("nethers");
			if (flags[FlagEnum.EMBER_GENDER] >= 2 && player.lowerBody.cockSpot.count() > 1) outputText(" and depths");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2 && player.lowerBody.cockSpot.count() > 1) outputText("both of your dicks");
			else outputText("your dick");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) outputText(", moisture drenching the cock buried in her cunt as it slobbers greedily across your burning flesh.");
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText("  " + emberMF("His", "Her") + " cock throbs and pulsates, ripples of arousal giving way to cumvein-bulging jets that shoot from " + emberMF("his", "her") + " prick to splatter wetly against the wall.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) outputText(" and ");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) outputText("her cunt gushing female ejaculate, almost soaking you with the cascade of feminine fluids and puddling wetly against your [feet]");
			outputText(".");
...
//Fatigue stays gained, whereupon it's lost if PC stays and rests? (Sure!)
		private function noStayingForCuddlesPostLustFuck():void
		{
			...
			if (player.stats.cor < 50) outputText("thank her for being so understanding");
...
//[=Yes=]
		private function stayWithEmberAfterLustFuck():void
		{
			...
			//else if Ember is female:
				...
				if (flags[FlagEnum.EMBER_MILK] > 0) outputText("milk-filled ");
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) outputText("-girl");
...
//Frotting:
		private function frottingWithEmber(clear:boolean = true):void
		{
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) outputText(emberMF("-boy", "-girl"));
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2) outputText("  Her moist dragon pussy is not helping the mess in the least.  You can smell her femcum pooling along with her pre.");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2) outputText("  Her other hand frigs her pussy as she comes down from her orgasm, gushing female juices and leaking some of the cum you've pumped into her earlier.");
			...
			if (flags[FlagEnum.EMBER_HAIR] == 0 && flags[FlagEnum.EMBER_ROUNDFACE] == 0) outputText(" head");
			else if (flags[FlagEnum.EMBER_HAIR] == 2) outputText(" mane");
			else outputText(" hair");
...
//Penetrate:
		private function penetrateWithEmber(clear:boolean = true):void
		{
			...
			if (flags[FlagEnum.EMBER_MILK] > 0) outputText(", milk seeping across your fingers,");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
				...
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText("your [chest], ");
				outputText("your belly");
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText(",");
			...
			if (player.upperBody.wingType > WING.NONE) outputText("stopping momentarily to stroke along your " + player.upperBody.wingDesc + " wings before continuing up and ");
			outputText("stopping at the back of your head.");

			outputText("\n\nSlowly she guides you towards one of her erect nipples.  \"<i>");
			if (flags[FlagEnum.EMBER_MILK] > 0) outputText("Drink my milk, I've been saving it up specially for you,");
			...
			if (flags[FlagEnum.EMBER_MILK] > 0) outputText("  You savor the resultant gush of sweet dragon's milk as it squirts obediently down your throat.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) outputText("  A small dollop of pre escapes Ember's dragon-dick at the pleasure of the movement.");
			...
			if (flags[FlagEnum.EMBER_MILK] > 0) outputText("allowing milk to spill into your mouth and gulp down sweet load after load, ");
			...
			if (flags[FlagEnum.EMBER_MILK] > 0) outputText(", not even caring that she's tasting her own milk as you finish gulping it down");
			...
			outputText(".");
			if (flags[FlagEnum.EMBER_GENDER] == 3) outputText("  Her draconic cock throbs all the way through your orgasm, shooting blanks a few times before spurting a couple ropes of pre onto her belly.");
...
//Frotting and Penetrate connect here.
		private function emberJizzbangbangEnding():void
		{
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) outputText("anthro");
