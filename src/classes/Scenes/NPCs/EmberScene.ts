		...
		private emberSexMenu(output:boolean = true):void
		{
			if (output) {
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
					...
					if (flags[FlagEnum.EMBER_INTERNAL_DICK] == 1 || flags[FlagEnum.EMBER_ROUNDFACE] == 0) MainScreen.text("is poking out of " + emberMF("his", "her") + " slit");
					else MainScreen.text("is starting to swell with blood");
					MainScreen.text(".  ");
				}
				if (flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("You can see tiny rivulets of moisture starting to run down Ember's inner thighs as they rub together, barely hiding her precious treasure from your hungry eyes.  ");
...
//[=Destroy it=] (Z)
		...
		{
			MainScreen.clearText();
			MainScreen.text("Raising your [weapon], you rain down blow after blow upon the egg.  The shell is freakishly tough, taking a lot of punishment before it shatters apart to spill a wave of egg white onto your " + player.feet() + "; a great pulpy mass of weirdly bluish-red yolk remains in the broken shell.");
...
//Genderless Version (Z)
		private masturbateOntoAnEgg():void
		{
			...
			if (player.gender == 1) {
				...
				MainScreen.text(".  The egg's pulsations lure you on, coaxing you to squeeze, pull, thrust, and massage " + sMultiCockDesc() + " as best you can.  Harder and faster you go, feeling the churning ache from deep inside you.  Finally, with a cry of release, you unleash a ");
			...
			//Female Version (Z)
				...
				if (player.wetness() <= 3) MainScreen.text("few drops");
				else if (player.wetness() < 5) MainScreen.text("squirt");
...
//HATCH DAT BITCH
		private hatchZeMuzzles():void
		{
			...
			if (flags[FlagEnum.EMBER_GENDER] == 0) MainScreen.text("white ");
			else if (flags[FlagEnum.EMBER_GENDER] == 1) MainScreen.text("blue ");
			else if (flags[FlagEnum.EMBER_GENDER] == 2) MainScreen.text("pink ");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) {
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1) {
					...
					if (flags[FlagEnum.EMBER_MILK] > 0) MainScreen.text(", drooling nipples notwithstanding");
					MainScreen.text(".");
				}
				//Female Anthro (Z)
				else if (flags[FlagEnum.EMBER_GENDER] == 2) {
			...
			//Boring version
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1) {
					...
					if (flags[FlagEnum.EMBER_MILK] > 0 || flags[FlagEnum.EMBER_HAIR] > 0) MainScreen.text(", complete with ");
					if (flags[FlagEnum.EMBER_MILK] > 0) {
						MainScreen.text("huge breasts, easily F-cups");
						if (flags[FlagEnum.EMBER_HAIR] > 0) MainScreen.text(" and ");
					}
					if (flags[FlagEnum.EMBER_HAIR] > 0) MainScreen.text("long, feminine locks of hair");
					MainScreen.text(", but his maleness is undeniable.  Especially when you spot ");
					if (flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) MainScreen.text("the slit in his pelvis that disgorges a foot and a half-long inhuman penis");
				...
				//Herm Monstergirl (Z)
					...
					if (flags[FlagEnum.EMBER_INTERNAL_DICK] == 0) MainScreen.text("huge, human prick");
...
//Appearance (shows Ember's appearance, always available)
		private embersAppearance():void
		{
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) {
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1) MainScreen.text(" not overly muscled.");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1) {
					...
					if (flags[FlagEnum.EMBER_MILK] > 0) MainScreen.text(" dribbling breasts,");
				...
				if (flags[FlagEnum.EMBER_HAIR] == 1) {
					if (flags[FlagEnum.EMBER_GENDER] == 1) MainScreen.text("  Short ");
				...
				MainScreen.text("  The ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) MainScreen.text("little ");
				...
				switch (pregnancy.event) {
					...
					case 6: MainScreen.text("  Her large pregnant belly has forced Ember to change her posture.  It looks like she has a big golden bullseye painted on her stomach.  Ember looks tired but happy.");
							break;
					case 7: MainScreen.text("  Her swollen belly is as large as that of any pregnant woman you can remember from Ingnam and you " + (flags[FlagEnum.EMBER_OVIPOSITION] > 0 ? "can easily make out the egg's outline under her golden skin" : "occasionally see movement as the baby shifts in Ember's womb") + ".");
							break;
					case 8:
					case 9: MainScreen.text("  Ember's golden belly is stretched taut by the large " + (flags[FlagEnum.EMBER_OVIPOSITION] > 0 ? "egg" : "baby") + " in her womb.  You're sure she'll give birth very soon, there just isn't room for the " + (flags[FlagEnum.EMBER_OVIPOSITION] > 0 ? "egg" : "baby") + " to grow any larger.");
							break;
					default:
				}
				...
				if (flags[FlagEnum.EMBER_MILK] > 0 || flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("F-cup soft, pillowy breasts");
				...
				if (flags[FlagEnum.EMBER_MILK] > 0 || flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("generous mounds");
			...
			//Dragon-girl Appearance (By Radar) (Z)
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1) MainScreen.text("male");
				else if (flags[FlagEnum.EMBER_GENDER] == 2) MainScreen.text("female");
				...
				//Short:
				if (flags[FlagEnum.EMBER_HAIR] < 1) MainScreen.text("is quite short, giving " + emberMF("him", "her") + " that definitive " + emberMF("masculine", "tomboy") + " look.");
				...
				if (flags[FlagEnum.EMBER_MILK] > 0 || flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("F-cup breasts that support 0.5 inch nipples and hang heavily; you idly wonder if " + emberMF("he", "she") + "'ll develop lower back problems as she spends more time in Mareth");
				...
				//Ember has a pussy:
				if (flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("  The inviting lips of a human-looking pussy purse between her legs; some moisture seems to have gathered on her labia, giving it a slick look that just begs for attention.");
...
//Talk about Dragons (Z)
		private talkToEmberAboutDragonzzz():void
		{
			...
			if (choice == 0) {
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) MainScreen.text(", a blush creeping onto " + emberMF("his", "her") + " cheeks");
				MainScreen.text(".");
		...
		private discussExplorationWithEmber():void
		{
			...
			if (choice == 0) {
				...
				//(If player has impregnated Tamani)
					...
					if (flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("  You should have more pride than to let some brainless cunt like that have her way with you!");
				}
				...
				else if (rand(2) == 0 && monk <= 1 && player.findStatusAffect(StatusAffects.PureCampJojo) < 0) MainScreen.text("this mouse-morph monk, sitting in a glade and meditating. A goblin tried to proposition him; he just gave her a lecture and sent her running away in tears.  When an imp tried to attack him, he crushed its skull with a staff he had.  Not bad moves for such a weedy little thing...");
				...
				if (subChoice == 0) {
					...
					if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(" like me");
					...
					else if (rand(3) == 0 && player.findStatusAffect(StatusAffects.TookBlessedSword) < 0 && player.findStatusAffect(StatusAffects.BSwordBroken) < 0) MainScreen.text("a sword jammed into a tree.  Weird; what kind of idiot would stick a weapon there like that?  And what kind of weakling wouldn't be able to take it out?");
					else if (rand(3) == 0 && player.findStatusAffect(StatusAffects.CampRathazul) < 0) MainScreen.text("a smelly rat-man moping around while some weird equipment bubbled and boiled.  I think maybe he was an alchemist.");
...
//Talk about Yourself (Z)
		private talkToEmberAboutYourself():void
		{
			...
			//(If PC Str < 50)
			if (player.str < 50) MainScreen.text("  \"<i>You could use more training; you look pretty weak...</i>\" Ember says, critically.");
			//(If PC Str < 90)
			else if (player.str < 90) {
			...
			//(If PC Tou < 50)
			if (player.tou < 50) MainScreen.text("\"<i>I don't know how you could hope to survive out there.  You look like the wind could blow you away.</i>\"");
			//(If PC Tou < 90)
...
//Occurs once during Ember's first pregnancy.
		private emberIsPregnantFirstTimeTalkScene():void
		{
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) MainScreen.text(" and blushes");
...
//counter triggered when PC starts an hour in camp at 100 Lust, output when reaching 10 counters
		public emberBitchesAtYouAboutLustiness():void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("your " + vaginaDescript() + " slick and wet with moisture, ready to fuck");
...
//Normal note for PC getting over mino cum addiction plays first
		public emberGetOverFreakingOutAboutMinoJizz():void
		{
			...
			//(High Affection)
				...
				//(Male/HermEmber:
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(", especially for sucking cock,");
...
//TF messages (Z)
		private emberTFs():void
		{
			...
			if (player.dragonScore() >= 4 && rand(3) == 0 && player.gender > 0) {
				...
				if (player.lowerBody.cockSpot.hasCock()) {
					MainScreen.text(cockDescript(0));
					if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" and ");
				}
				...
				if (player.lowerBody.cockSpot.hasCock() && flags[FlagEnum.EMBER_GENDER] >= 2) {
					...
					if (player.lowerBody.vaginaSpot.hasVagina() && flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(" even while ");
				}
				if (player.lowerBody.vaginaSpot.hasVagina() && (flags[FlagEnum.EMBER_GENDER] == 3 || flags[FlagEnum.EMBER_GENDER] == 1)) MainScreen.text("taking that hard, spurting cock inside your own " + vaginaDescript(0));
		...
		private emberIsAnEggFactory():void
		{
			...
			if (emberAffection() <= 25) {
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
					...
					if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) MainScreen.text("dragon-dick jutting from its slit,");
				...
				//[(if Ember has a dick)
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(" and her throbbing cock is soft enough to hide");
			...
			//(Medium Affection)
				...
				MainScreen.text("\n\nEmber gasps");
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) MainScreen.text(" and her cheeks grow red with embarrassment");
			...
			//(High Affection)
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(", and her cock points you accusingly");
				MainScreen.text(".");
				...
				if (player.tongueType > TONUGE.HUMAN) MainScreen.text(" and out... and out...");
				MainScreen.text(" and then lean forward to give her a great, wet, sloppy lick, straight up the center of her pussy");
				//(E.Herm:
				if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(" not stopping until you have slurped your way to the very tip of her cock");
				...
				if (player.tongueType > TONUGE.HUMAN) MainScreen.text("which is quite far indeed, ");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(" and dribbling down her painfully stiff dick");
				...
				if (player.tongueType > TONUGE.HUMAN) MainScreen.text("  Right at this time, you feel something round and smooth on the tip of your tongue, gently spreading Ember's walls.  Realizing that this can only be her egg, you start trying to worm your long, sinuous tongue between it and her innermost walls, hoping to coax it out of her.");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("  Her cock throbs and pumps out long streams of cum to paint the ground around you two; marking it as your special place.");
...
//[Fob Off]
		private dontWatchEmberLayEgg():void
		{
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(".. it even drips with some kind of off-white fluid.");
...
//[Watch]
		private watchMediumAffectionEmberEggLay():void
		{
			...
			//(if Ember has a cock:
			if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(", cock poking straight out");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) MainScreen.text(" of its hiding place");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(" and caressing her throbbing cock");
			MainScreen.text(".");
			MainScreen.text("\n\nWith a deep moan, Ember shakes and orgasms.");
			if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("  Ropes of jism spew out of her cock, arching into the air to hit the ground nearby.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(" and her cock hanging out");
...
//Get Milk
		private getMilkFromEmber():void
		{
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) MainScreen.text(", scaly");
			...
			if (emberAffection() <= 25) {
				...
				//noEmberkids:
				if (flags[FlagEnum.EMBER_EGGS] == 0 && emberChildren() == 0) MainScreen.text(emberMF("he", "she") + " has no offspring to feed them with");
				//(1+ EmberEggs:
				else if (flags[FlagEnum.EMBER_EGGS] > 0) MainScreen.text(emberMF("his", "her") + " children haven't hatched yet");
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) MainScreen.text("scales");
				...
				if (flags[FlagEnum.EMBER_GENDER] != 2) {
					...
					if (!player.isTaur()) MainScreen.text("belly");
			...
			//(Medium Affection)
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) MainScreen.text("flesh");
			...
			//(High Affection)
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
					...
					if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) MainScreen.text("emerging");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
					...
					if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) MainScreen.text(" strangely ridged, almost scaly");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("stroke " + emberMF("his", "her") + " shaft");
...
//[Catch Anal] - a dragon coq up the date (Z)
		private catchAnal():void
		{
			...
			if (emberAffection() < 75) {
				...
				if (flags[FlagEnum.EMBER_INTERNAL_DICK] == 1 || flags[FlagEnum.EMBER_ROUNDFACE] == 0) MainScreen.text(" of its hiding place");
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" your " + clitDescript() + " drips with every passing moment.");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) MainScreen.text("segmented");
			else MainScreen.text("smooth");
			MainScreen.text(" length of " + emberMF("his", "her") + " prick between your " + buttDescript() + " cheeks, letting you feel the refreshing coolness of " + emberMF("his", "her") + " pre dripping from the tip.  \"<i>This... isn't going to work,</i>\" " + emberMF("he", "she") + " growls.");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) MainScreen.text("mouth");
			...
			if (player.ass.analLooseness == 0 || player.analCapacity() < 25) MainScreen.text(", leaving both of you wincing as the sensitive head of " + emberMF("his", "her") + " cock struggles to make room for its insertion.  \"<i>I can't believe you really want me to put it here... you're so tight it almost hurts,</i>\" " + emberMF("he", "she") + " whimpers.");
			//(PC has a loose asshole:)
			if (player.analCapacity() < 50) {
				MainScreen.text(", causing you to gasp as " + emberMF("his", "her") + " ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) MainScreen.text("draconic ");
			...
			//(PC has a gaping asshole:)
				...
				MainScreen.text("; the looseness of your " + assholeDescript() + " gives " + emberMF("him", "her") + " no resistance whatsoever as " + emberMF("he", "she") + " glides in without trouble.  \"<i>Huh... that was easier than I expected,</i>\" " + emberMF("he", "she") + " remarks in surprise.  \"<i>What have you been getting yourself into?</i>\"");
			}
			...
			//(PC has prick:
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  You can feel the dragon's member rubbing against your prostate, making " + sMultiCockDesc() + " jump to painful erectness, bubble pre-cum, and drool it down the shaft to pool on the ground.");
			//(PC has cunt:
			...
			if (pregnancy.event > 3) {
				if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) MainScreen.text("swollen ");
...
//[Blow Ember] - your shipment of dragon dildoes has arrived
		private suckEmberCock():void
		{
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1) {
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) MainScreen.text("his single genital slit");
				else MainScreen.text("his genitals");
			}
			else {
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) MainScreen.text("her twin genital slits");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) {
				...
				if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("first ");
				MainScreen.text("genital slit");
			}
			MainScreen.text(".  Up and up it rises, until all 16 inches of ");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) MainScreen.text("dragon ");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(", your tongue wriggling around in " + emberMF("his", "her") + " slit");
...
//Get Blown - put your dick in the knife drawer, it'll be fun! (Z, with reservation)
		private stickDickInKnifeDrawer():void
		{
			...
			if (!player.isNaga()) MainScreen.text("seat yourself on a conveniently lichen-covered boulder.");
			...
			if (player.lowerBody.cockSpot.list[x].cockType == CockType.DRAGON) MainScreen.text("\n\n\"<i>Such a magnificent, beautiful cock... it feels powerful... and familiar.</i>\"");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) MainScreen.text(", the scales on " + emberMF("his", "her") + " palms producing an indescribable feeling as they slide across your sensitive flesh,");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("  " + emberMF("His", "Her") + " shaft is already hard as rock, and sometimes you can feel it poke your " + player.feet() + ", smearing small beads of Ember's barely contained pleasure on your " + player.skinFurScales() + ".");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("  Looking behind her, you see a small pool of her feminine juices has formed; pheromones fly through the air and hit you like a wave, spurring you on, making you feed Ember more of your delicious pre.");
			...
			if (flags[FlagEnum.EMBER_HAIR] == 0 && flags[FlagEnum.EMBER_ROUNDFACE] == 0) MainScreen.text("scaly pate");
			...
			if (player.cumQ() < 250) {
				...
				if (!player.isTaur()) MainScreen.text("a finger");
...
//Pitch Anal - horses are terrible people, so no centaurs unless rewritten (Z)
		private stickItInEmbersButt():void
		{
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("cock ");
			if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("and ");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("pussy ");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) MainScreen.text(", panting with lust");
			else MainScreen.text(", a blush adorning " + emberMF("his", "her") + " face");
			MainScreen.text(".  You force yourself to remain still, giving the horny dragon a chance to recover from your " + cockDescript(x) + " impaling " + emberMF("his", "her") + " back passage, savoring the feeling of " + emberMF("his", "her") + " cool muscles squeezing and rippling around your shaft.");

			MainScreen.text("\n\nAfter a few minutes, Ember finally starts to move, stroking you with " + emberMF("his", "her") + " inner muscles.  ");
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("Droplets of pre slide along " + emberMF("his", "her") + " own shaft and down " + emberMF("his", "her") + " balls to gather at your belly.  ");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("Her slick pussy drips rhythmically, wetting your lower body with slick dragon juice.  ");
			MainScreen.text(emberMF("His", "Her") + " lusty moans each time " + emberMF("he", "she") + " comes down on your " + cockDescript(x) + " send a thrill up your spine, and the deep purring emanating from " + emberMF("his", "her") + " chest is audible enough to give you an idea of how much Ember is enjoying this, despite " + emberMF("his", "her") + " initial reluctance.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(", jet upon jet of spunk arcing over " + emberMF("his", "her") + " head to paint the ground below");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("; a veritable gush of juices spews forth from her forgotten love-hole to splash against your lower body and over herself");
...
//Eat Ember Out - b-baka! (Z)
		private slurpDraggieCunnies():void
		{
			...
			//(Medium Affection)
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(" and slowly hardening shaft");
				MainScreen.text(".");
			}
			//(High Affection)
			else {
				...
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(" and shaft");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("  With Ember's cock jutting out above your head, you can't resist sliding your tongue along the underside of her shaft, just for an extra teasing motion.");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) MainScreen.text("blushing all the while and ");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("  You think droplets of pre are starting to slide down her girl-cock and drip onto your head... but that's part and parcel of loving a herm, so you pay it no attention.");
			...
			if (player.tongueType > TONUGE.HUMAN) MainScreen.text("inhuman ");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(", heedless of the steady rain of pre dribbling into your " + hairDescript() + ".  You're confident she'll crack soon...");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("  Her throbbing shaft, not intending to be left out of the fun, records the event by marking the tree bark with white dragon spooge.");
...
//Get Eaten Out - actually halfway likeable
		private getEatenOutByEmbra():void
		{
			...
			if (!player.isNaga()) MainScreen.text(" with your [legs] spread wide");
			...
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(", and pre-cum beginning to bubble out of " + sMultiCockDesc());
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) MainScreen.text("maw");
			...
			if (player.wetness() >= 4) MainScreen.text("spattering");
			else MainScreen.text("drenching");
			MainScreen.text(" Ember's face with your feminine honey");
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" even as " + sMultiCockDesc() + " belches cum all over the preoccupied dragon");
			MainScreen.text(".");
			...
			if (player.wetness() >= 4) MainScreen.text("  However, your squirting spatters make this an impossible task.  Ember doesn't seem to care, even as " + emberMF("his", "her") + " face is completely covered in femcum.");
		...
		private penetrateEmbrah():void
		{
			...
			//(High Affection)
				...
				if (flags[FlagEnum.EMBER_GENDER] == 3) {
					if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) MainScreen.text(" and her cock peek out of its protective slit");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
				MainScreen.text("; her ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) MainScreen.text("draconic");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("; her own erect cock spurts a small jet of pre");
			MainScreen.text(".");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(", and her cock, while engorged and swollen, hasn't poured forth yet");
			MainScreen.text(".");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("; her cock jumps and unloads on the ground below, marking this as the spot of your lovemaking");
...
//Get Penetrated - also horse-proof, sorry folks! (Z)
		private getPenetratedByEmberLastSexSceneWoooo():void
		{
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] > 0) MainScreen.text("dragon");
			else MainScreen.text("human");
			MainScreen.text(" dick between Ember's legs, and you lick your lips hungrily.  Your " + vaginaDescript() + " throbs eagerly as you tell Ember you have certain itches you need to scratch... is Ember enough of a " + emberMF("man", "herm") + " to scratch them for you?");
			...
			//(High Affection)
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] > 0 && flags[FlagEnum.EMBER_INTERNAL_DICK] == 0) MainScreen.text("to attention");
			...
			if (player.tongueType == TONUGE.HUMAN) MainScreen.text("yours ");
...
//Part II
		private getPenetratedByEmberLastSexSceneWooooPartII():void
		{
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 || flags[FlagEnum.EMBER_INTERNAL_DICK] == 1) MainScreen.text("protective slit");
...
//[=Deny=]
		private fuckOffEmberIWantANap():void
		{
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("protruding prick");
			if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(" and ");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("creaming wet vagina");
...
//Ember never fails to impregnate the PC or be impregnated - unless the player is on contraceptives.
		private getKnockedUpByEmbrahBroBaby():void
		{
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 && flags[FlagEnum.EMBER_MILK] == 0) MainScreen.text("his flat, muscular chest");
			...
			if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) MainScreen.text("little dragons");
			...
			if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) MainScreen.text("dragonlings");
			else MainScreen.text("dragon eggs");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) MainScreen.text("blushing ");
			...
			if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) MainScreen.text("baby dragon");
...
//Breeding Ember
		private breedEmberPregnantAsIfThereWasAnyOtherKindOfBreeding():void
		{
			...
			if (x < 0) x = player.smallestCockIndex();
			MainScreen.text("Ember catches you and rolls you around, pinning you to the ground under her.  She smiles at you seductively and reaches down to stroke your " + cockDescript(x) + ".");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) MainScreen.text("scale-covered ");
			MainScreen.text("orbs gently squish ");
			if (flags[FlagEnum.EMBER_MILK] > 0) MainScreen.text("and leak milk ");
			...
			if (flags[FlagEnum.EMBER_MILK] > 0) MainScreen.text("the slowly trickling rivulets of milk along with ");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) MainScreen.text("draconic ");
			...
			MainScreen.text("your " + cockDescript(x) + " gushing into the dragon's ready womb.");
...
//Bred/Breeding Aftermath
		private emberBreedingAfterMathWatchOutForRadioactiveFallout(emberPregged:boolean = true):void
		{
			...
			MainScreen.text("\n\nEmber's cheeks ");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) MainScreen.text(" are probably blushing under " + emberMF("his", "her") + " scales.");
			...
			MainScreen.text("\n\nEmber's ");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) MainScreen.text("blush");
			...
			MainScreen.text("\n\nEmber ");
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 1) MainScreen.text("blushes");
			...
			switch (pregnancy.eventTriggered()) {
				...
				case 4: MainScreen.text("\nEmber's belly has grown quite a bit.  Anyone can tell she's pregnant with a single glance.  ");
						if (flags[FlagEnum.EMBER_OVIPOSITION] > 0) {
							...
							MainScreen.text("Ember catches you looking" + (flags[FlagEnum.EMBER_ROUNDFACE] == 1 ? " and blushes" : "") + ".  \"<i>W-what is it?</i>\"  You reply merely that she carries a baby bump very well; she looks good pregnant.  \"<i>Oh, uh...  Thanks I guess?</i>\" she replies, looking away and flicking her tongue out nervously.\n");
						}
						return true;
		...
		public emberGivesBirth():void
		{
			...
			if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) {
				...
				if (emberHasCock()) {
					if (flags[FlagEnum.EMBER_ROUNDFACE] == 0 && flags[FlagEnum.EMBER_INTERNAL_DICK] == 0) MainScreen.text("  Her human-like cock dangles heavily in front of her pussy, partially erect from the muscular spasms.");
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) MainScreen.text("head crowning");
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) MainScreen.text("n anthro dragon");
			...
			//Ember Lays Egg
				...
				if (emberHasCock()) {
					if (flags[FlagEnum.EMBER_ROUNDFACE] > 0 && flags[FlagEnum.EMBER_INTERNAL_DICK] == 0) MainScreen.text("  Her human cock dangles heavily in front of her pussy, partially erect from the muscular spasms.");
					else MainScreen.text("  Her draconic cock has been pushed from its internal sheathe by the pressure, but it's too painful for her to be erect.");
				}
				MainScreen.text("\n\nMind racing, you bend your head in close and stick out your tongue, sliding it back in and out of your mouth to get it nice and wet before delivering a long, sloppy lick to her inner walls.  You can taste her, and the strange salty-blood taste of amniotic fluid mingles with her natural lubricants.  It's an unusual taste but not unbearable, and you begin to lick with greater fervor and purpose.  Your intention is to try and drown the pain of her contractions with pleasurable stimulus.  It's a crazy idea, but it makes sense in a place like Mareth.");
				...
				if (flags[FlagEnum.EMBER_EGGS] > 0) {
					...
					if (flags[FlagEnum.EMBER_EGGS] == 1) MainScreen.text("other");
					else MainScreen.text(num2Text(flags[FlagEnum.EMBER_EGGS]) + " others");
				}
		...
		public giveBirthToEmberKids():void
		{
			...
			if (flags[FlagEnum.EMBER_OVIPOSITION] == 0) {
				...
				else if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("slowly pushing your " + multiCockDescriptLight());
			...
			//PC Lays Egg
				...
				MainScreen.text("\n\nEmber ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) MainScreen.text("blushes and ");
				...
				if (flags[FlagEnum.EMBER_EGGS] > 0) MainScreen.text("s hatch");
				else MainScreen.text(" hatches");
				MainScreen.text(", anyway.  The dragon ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) MainScreen.text("blushes and then ");
...
//Note: This scene is meant for Tainted Ember after you've been through the lost dragon city dungeon. While we do not have the dungeon and post-quest Ember, this scene may be accessed from regular Ember's pool of scenes if her affection is High.
		...
		private highAffectionEmberLustFuck():void
		{
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(" and");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text(" her pussy beginning to drip in excitement");
			MainScreen.text(".");

			MainScreen.text("\n\n\"<i>Mmm... now that's a view and a half; just look at you... what a gorgeous " + player.mf("man", "girl") + ", with such ");
			if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("a wonderful " + cockDescript(0));
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("stiffness of " + emberMF("his", "her") + " cock");
			if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(" and ");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("the slickness of her pussy");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1) {
				...
				MainScreen.text("\n\nYour ");
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) MainScreen.text("she-dragon");
			...
			//else if Ember has a pussy and the PC more than one cock:
				...
				if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(", not to mention the stiffness of her dick");
				...
				if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(", cock dribbling precum onto her belly");
				MainScreen.text(".");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2 && player.lowerBody.cockSpot.count() > 1) MainScreen.text(" and ass");
			MainScreen.text(" rippling and squeezing your intruding member");
			if (flags[FlagEnum.EMBER_GENDER] >= 2 && player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
			MainScreen.text(".");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("vagina");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2) {
				...
				if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(", and her ");
				else MainScreen.text(", ");
			}
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1) MainScreen.text("ass");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 1 && player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
			MainScreen.text(".  The dragon lets out an exultant cry as " + emberMF("his", "her") + " cum-slimed ");
			if (flags[FlagEnum.EMBER_GENDER] == 1) MainScreen.text("ass squeezes");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1) MainScreen.text("shaft as his cock spurts dragon-seed onto the leaves below him, filling the air with the scent of spunk and matting them into a steaming morass.");
			//{twin shafts} as {her cunt spasms wetly, drenching your lap with fresh femcum} {and/or} {[her] cock spurts dragon-seed onto the leaves below {her}, filling the air with the scent of spunk and matting them into a steaming morass}
				...
				if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(" and her cock spurts dragon seed onto the leaves below her, filling the air with the scent of spunk and matting them into a steaming morass");
				MainScreen.text(".");
			}
			...
			if (player.stats.lib >= 50 || player.minLust() >= 40) {
				...
				if (flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("her cunt starting to drip");
				if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(" and ");
				if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(emberMF("his", "her") + " cock growing hard again");
			...
			//Else:
				...
				if (flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("her cunt starts to drip with moisture");
				if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(" and ");
				if (flags[FlagEnum.EMBER_GENDER] == 3 || flags[FlagEnum.EMBER_GENDER] == 1) MainScreen.text(emberMF("his", "her") + " cock begins to grow erect again");
			...
			if (player.lowerBody.cockSpot.count() > 1 && flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("s");
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] == 1 || flags[FlagEnum.EMBER_HAIR] > 0) {
				...
				if (flags[FlagEnum.EMBER_HAIR] >= 2) MainScreen.text("mane");
				else MainScreen.text("hair");
			}
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 0) MainScreen.text("mingled ");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1) MainScreen.text("depths");
			else MainScreen.text("nethers");
			if (flags[FlagEnum.EMBER_GENDER] >= 2 && player.lowerBody.cockSpot.count() > 1) MainScreen.text(" and depths");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2 && player.lowerBody.cockSpot.count() > 1) MainScreen.text("both of your dicks");
			else MainScreen.text("your dick");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text(", moisture drenching the cock buried in her cunt as it slobbers greedily across your burning flesh.");
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("  " + emberMF("His", "Her") + " cock throbs and pulsates, ripples of arousal giving way to cumvein-bulging jets that shoot from " + emberMF("his", "her") + " prick to splatter wetly against the wall.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text(" and ");
			if (flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("her cunt gushing female ejaculate, almost soaking you with the cascade of feminine fluids and puddling wetly against your [feet]");
			MainScreen.text(".");
...
//[=Yes=]
		private stayWithEmberAfterLustFuck():void
		{
			...
			//else if Ember is female:
				...
				if (flags[FlagEnum.EMBER_MILK] > 0) MainScreen.text("milk-filled ");
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) MainScreen.text("-girl");
...
//Frotting:
		private frottingWithEmber(clear:boolean = true):void
		{
			...
			if (flags[FlagEnum.EMBER_ROUNDFACE] > 0) MainScreen.text(emberMF("-boy", "-girl"));
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("  Her moist dragon pussy is not helping the mess in the least.  You can smell her femcum pooling along with her pre.");
			...
			if (flags[FlagEnum.EMBER_GENDER] >= 2) MainScreen.text("  Her other hand frigs her pussy as she comes down from her orgasm, gushing female juices and leaking some of the cum you've pumped into her earlier.");
			...
			if (flags[FlagEnum.EMBER_HAIR] == 0 && flags[FlagEnum.EMBER_ROUNDFACE] == 0) MainScreen.text(" head");
			else if (flags[FlagEnum.EMBER_HAIR] == 2) MainScreen.text(" mane");
			else MainScreen.text(" hair");
...
//Penetrate:
		private penetrateWithEmber(clear:boolean = true):void
		{
			...
			if (flags[FlagEnum.EMBER_MILK] > 0) MainScreen.text(", milk seeping across your fingers,");
			...
			if (player.tailType > TAIL.NONE) MainScreen.text("the base of your tail, tugging lightly on it and stroking it for a moment, then her hands move on to ");
			...
			if (player.upperBody.wingType > WING.NONE) MainScreen.text("stopping momentarily to stroke along your " + player.upperBody.wingDesc + " wings before continuing up and ");
			MainScreen.text("stopping at the back of your head.");

			MainScreen.text("\n\nSlowly she guides you towards one of her erect nipples.  \"<i>");
			if (flags[FlagEnum.EMBER_MILK] > 0) MainScreen.text("Drink my milk, I've been saving it up specially for you,");
			...
			if (flags[FlagEnum.EMBER_MILK] > 0) MainScreen.text("  You savor the resultant gush of sweet dragon's milk as it squirts obediently down your throat.");
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("  A small dollop of pre escapes Ember's dragon-dick at the pleasure of the movement.");
			...
			if (flags[FlagEnum.EMBER_MILK] > 0) MainScreen.text("allowing milk to spill into your mouth and gulp down sweet load after load, ");
			...
			if (flags[FlagEnum.EMBER_MILK] > 0) MainScreen.text(", not even caring that she's tasting her own milk as you finish gulping it down");
			...
			MainScreen.text(".");
			if (flags[FlagEnum.EMBER_GENDER] == 3) MainScreen.text("  Her draconic cock throbs all the way through your orgasm, shooting blanks a few times before spurting a couple ropes of pre onto her belly.");
...
//Frotting and Penetrate connect here.
		private emberJizzbangbangEnding():void
		{
			...
			if (flags[FlagEnum.EMBER_GENDER] == 1 || flags[FlagEnum.EMBER_GENDER] == 3) {
				...
				if (flags[FlagEnum.EMBER_ROUNDFACE] == 0) MainScreen.text("anthro");
