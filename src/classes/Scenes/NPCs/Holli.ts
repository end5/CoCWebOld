...
export default class Holli extends Monster {
	/*Fight -Z
	...
	public escapeFailWithHolli(): void {
		...
		MainScreen.text("You ");
		if (player.canFly()) MainScreen.text("beat your wings and ");
		MainScreen.text("try to escape, but " + short + " wraps one of her writhing roots around your [leg], slamming you to the ground and tying you up with several more!  \"<i>And just where do you think you're going, my little meat?</i>\" she hisses.  Her bark splits open, exposing her body, and a green shaft snakes out of her crotch, sprouting thorns and blooming into a rose at the tip.  She holds the drooling blossom over your [face] as she forces your mouth open with her roots!");
	...
	public lightHolliOnFireMagically(): void {
		if (statusAffects.has("JojoIsAssisting")) {
			if (findStatusAffect(StatusAffects.HolliBurning) < 0) {
				MainScreen.text("The magical fire effectively ignites a wide swath of Jojo's tinder, and the demon howls in rage.  Seeing this, Jojo drops the burning torch he carries and turns back toward the forest to fetch more tinder.\n\n");
				statusAffects.add(new StatusAffect("HolliBurning", 0, 0, 0, 0)));
			}
		}
	}
	...
	private fuckinJamanjiFlowerDarts(): void {
		MainScreen.text("A blossom opens up on a high branch of the tree, revealing an evil-looking eye surrounded by vicious spines.  With a gesture, " + short + " fires several at you!");

		//Blinded - no hit penalty
		if (statusAffects.has("Blind")) MainScreen.text("  Though the demon herself is blinded, the fresh eye on the flower seems more than capable of aiming for her!");
		if (combatMiss() || combatEvade() || combatFlexibility() || combatMisdirect()) {
			MainScreen.text("  Nimbly, you step aside and let the darts whistle by.");
		}
		//Hit
		else {
			MainScreen.text("  The darts find flesh, and you feel yourself slowing down drastically; all you want to do as the plant woman's poison takes you is fuck and sleep.  \"<i>Just give up,</i>\" Holli coos.  \"<i>Think how good it would be to fall into my arms and ");
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("come inside me");
			else MainScreen.text("have me inside you");
			MainScreen.text(", forever...</i>\"");
	...
	private holliConstrictAttack(): void {
		MainScreen.text("A forest of thick roots bursts from the ground and several lash toward your [legs], trying to ensnare you!");
		//Blinded - hit penalty, but not 100%
		if (statusAffects.has("Blind") && rand(6) == 0) {
			MainScreen.text("  Luckily, the demon's blindness makes it fairly easy to dodge the grasping roots, though there are a few close scrapes.");
		}
		//Miss
		if (combatMiss() || combatEvade() || combatFlexibility() || combatMisdirect()) {
			MainScreen.text("  It's a narrow thing, but you manage to avoid the roots - one of them almost grabs you, but you duck aside and let it find only its neighbor.");
		}
		//Hit
		else {
			MainScreen.text("  She latches onto you with a painful smack and several more root tentacles join the first; as she pulls you close, her bark opens and a long, phallic stalk extends from her crotch, wrapped in thorns and flowering with a rose!  It caresses your face, then dangles the blossom above your mouth, dripping her sap.  Several of the roots pry your jaws apart, forcing you to drink the tainted fluids from her pseudo-cock!  \"<i>What do you think of my little sap rose?</i>\"");
	...
	public struggleOutOfHolli(): void {
		...
		if (player.upperBody.head.face.tongueType > TongueType.HUMAN) {
			MainScreen.text("You can't move an arm nor a [leg] to bat the flower away... but she's literally holding your mouth open.  Your long tongue rolls out, gripping and ripping out several of the petals on the end of her stalk!  Holli screams and her roots slacken, allowing you to batter your way out of them.");
			player.statusAffects.remove("HolliConstrict");
		}
		//else if normal str-based success
		else if (player.stats.str / 10 + rand(20) + 1 + player.statusAffects.get("HolliConstrict").value1 > 30) {
			MainScreen.text("You manage to force the roots open when the distracted Holli begins to stroke her plant-shaft, pulling out of the bindings just as a drop of sap oozes out and falls where you were standing.  You're free!");
			//sap rose pls go
			player.statusAffects.remove("HolliConstrict");
		}
		//Struggle Fail/Wait
		else {
			MainScreen.text("You try to escape the entangling roots, but cannot break their grip!  ");
	...
	override protected performCombatAction(): void {
		...
		else if (rand(4) == 0 && !player.statusAffects.has("HolliConstrict")) holliConstrictAttack();
	...
	public teased(lustDelta: number): void {
		if (statusAffects.has("HolliBurning")) {
			MainScreen.text("Holli doesn't even seem to notice, so concerned is she with defeating you before the mounting bonfire causes her any more pain.");
			lustDelta = 0;
		}
