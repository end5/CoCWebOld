							...
							if (model.time.hours % 9 == 0) { //Only once every 9 hours or so.
								if (rand(3) == 0) { //lactation messing with!
									...
									if (rand(2) == 0)
										...
										if (player.stats.cor < 50) outputText(", angry at yourself for falling prey to the demon's directions");
										outputText(".");
...
private function drinkFountainEndowment():void {
	...
	if(rand(4) == 0 && player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength < 6) outputText("gets as big as a cock", false);
...
public function exgartuanMasturbation():void {
	...
	if(player.statusAffects.get("Exgartuan").value1 == 1) {
		spriteSelect(15);
		//Early prep
		if(player.stats.cor < 15) outputText("You sheepishly find some rocks to hide in, where you remove your clothes.  Exgartuan loudly grumbles, \"<i>Quit fucking around and hiding.  I WANT someone to walk in on this!</i>\"\n\nDisgusting...\n\n", false);
		if(player.stats.cor >= 15 && player.stats.cor < 30) outputText("You make sure you are alone and strip naked.  Exgartuan mutters, \"<i>Why did you wait until you were alone?  Wouldn't it be fun to have a succubus wander in on us?</i>\"\n\n", false);
		if(player.stats.cor >= 30 && player.stats.cor < 60) outputText("You happily remove your " + player.armorName + ", eager to pleasure yourself.  Your possessed " + cockDescript(0) + " throbbs happily.\n\n", false);
		if(player.stats.cor >= 60 && player.stats.cor < 80) outputText("You strip naked in an exaggerated fashion, hoping someone might be watching.  The demon-possessed member at your groin pulses happily, flooding you with lust in reward for your attitude.\n\n", false);
		if(player.stats.cor >= 80) outputText("You strip naked, fondling your naughty bits as you do so and casting seductive looks around, hoping someone or something is nearby to fuck you.  Your possessed member wiggles happily, flooding your body with feelings of lust and desire in reward.  Maybe it can call in a demonic companion for you?\n\n", false);
		...
		//Medium corruption characters
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) outputText("shove the bloated demon between your " + allBreastsDescript() + ", surrounding it in a home of jiggling flesh.  The wet demon-pre soaks into the skin of your tits, lubricating the passage as you mash them around, vigorously tit-fucking the source of your unholy desires.", false);
		...
		//high corruption characters
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) outputText("your hands smashing your " + allBreastsDescript() + " around the " +cockDescript(0) + " and vigorously titfucking it.", false);
	...
	//TITURBATION
		...
		if(player.skinType == SKIN.PLAIN || player.skinType >= SKIN.UNDEFINED) outputText("flesh", false);
		else if(player.skinType == SKIN.FUR) outputText("furry-covered flesh", false);
		else if(player.skinType == SKIN.SCALES) outputText("soft scaley flesh", false);
		...
		if(player.upperBody.chest.BreastRatingLargest[0].nippleLength < 2) outputText("pinch together and pull, and it's too much for you.", false);
...
public function exgartuanBored():void {
	...
	if(player.statusAffects.get("Exgartuan").value1 == 1 && player.cockArea(0) >= 100) {
		...
		if(select == 0) {
			...
			if(player.lowerBody.balls == 0) outputText("inside?", false);
			...
			if(player.lowerBody.balls > 0) outputText("teasing your ballsack", false);
			//[if vagina, no balls]
			else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("poking your labia", false);
			...
			if(player.lowerBody.balls > 0) outputText("boiling in your " + ballsDescriptLight() + ".", false);
		...
		//(+Lust in combat) for girls
			...
			switch(select) {
				case 0:
					...
					if(player.stats.cor < 50) outputText("  You glance down at the sensitive bulge in your " + player.armorName + " and sigh.  Damned demons.", false);
					break;
				case 1:
				...
				case 2:
					...
					if(player.stats.cor < 50) outputText("  That's terrible!  It's all that demon's fault!", false);
					break;
				case 3:
					...
					if(player.stats.cor < 50) outputText("  You snap back to reality and curse yourself for falling to the demon's desires.", false);
					break;
				case 4:
...
public function exgartuanArmorShift():void {
	...
	if(player.stats.cor < 33) outputText("  You cringe and blush bright crimson, raging against the demon inside you and wishing he would stop tormenting you!", false);
	else if(player.stats.cor < 66) outputText("  You cringe a bit at the exhibitionist outfit you're forced to wear, but spend a little time admiring just how well the changes show off your package... Maybe Exgartuan isn't all bad?", false);
...
private function exgartuanSleepSurprise():void {
	...
	if(player.stats.cor <= 20 && player.perks.has("BulgeArmor")) {
		...
		if(flags[FlagEnum.TIMES_AUTOFELLATIOED_EXGARTUAN] == 0) {
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText("wrap your tits around me and squeeze", false);
		...
		if(player.stats.cor < 33) {
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText(" and bouncing your " + allBreastsDescript() + " around it", false);
			...
			if(flags[FlagEnum.TIMES_AUTOFELLATIOED_EXGARTUAN] < 5) {
				...
				if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("you squeeze your tits around your " + cockDescript(0) + " with your biceps", false);
			...
			//(MED+COR, DONE A LOT:
				...
				if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) {
					...
					if(player.upperBody.chest.hasFuckableNipples() && player.biggestLactation() > 1) outputText("  Milk and lubricants ", false);
					else if(player.upperBody.chest.hasFuckableNipples()) outputText("  Lubricants ", false);
		...
		if(player.gender == 3) {
			...
			if(player.lowerBody.balls == 0) {
				...
				if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.WET) outputText("puffy", false);
				else if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) outputText("glistening", false);
				...
				if(player.skinType == SKIN.SCALES) outputText("scales", false);
				...
				if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) outputText(" or stroke your " + clitDescript(), false);
		...
		if(player.stats.cor < 33) outputText("The desire shocks you with the sheer... wrongness of it, but you cannot deny the pleasure of drilling your own slobbering mouth.", false);
		else if(player.stats.cor < 66) outputText("The desire sinks its hooks into the lust-addled flesh of your brain and drags you deeper into lust.  You can't deny the pleasure of drilling your own slobbering mouth.", false);
		...
		if(monk >= 5 && player.findStatusAffect(StatusAffects.NoJojo) < 0 && flags[FlagEnum.JOJO_DEAD_OR_GONE] == 0) outputText(", and judging by Jojo's high pitched whines, he certainly is,", false);
		...
		if(player.lowerBody.balls > 0) outputText(" and your " + ballsDescriptLight() + " pull tight against your groin from the effort", false);
		outputText(".", false);
		temp = player.cumQ();
		if(temp > 500) {
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("over your chest", false);
...
private function exgartuanBulgeTortureII():void {
	...
	if(player.lowerBody.balls > 0 || player.lowerBody.vaginaSpot.hasVagina()) outputText(" along with anything else unlucky enough to reside within", false);
	outputText(".\n\n", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2 && player.bRows() == 1) outputText("  Your quest begins squarely on your pair of " + chestDesc() + ", your mitts feverishly groping and tugging away in the hope of finding any sanctuary from your encroaching lust.  Unable to extract any meaningful solace from your magnificent mounds, your sights set on your " + nippleDescript(0) + "s.", false);
	//[if ≥2 rows of breasts]
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("  The " + allBreastsDescript() + " lined down your chest should serve a proper conquest.  Your palms set off to work at once, manhandling as much titflesh as they can satisfy at once.  They dart from one row to the next, inside to out, anything to relieve your libido.  You don't appear to be settling down any, however, so you draw your focus on your top row of " + nippleDescript(0) + "s.", false);
	...
	//[if nipplecunts]
	if(player.upperBody.chest.hasFuckableNipples()) outputText("  You waste little time in pushing four fingers into each gaping teat, anxious for any sort of pleasure.  You tickle and tease, push and pull, anything you can think to release you from this torment.", false);
...
private function exgartuanBulgeTortureIII():void {
	...
	//[if cocks ==2]
	if(player.lowerBody.cockSpot.count() == 2) outputText("  Your " + cockDescript(1) + " remains flaccid beside this bronze adonis, as if sapped of all its life.", false);
	//[if cocks ≥3]
	else if(player.lowerBody.cockSpot.count() > 2) outputText("  Your extra cocks remain flaccid beside this bronze adonis, as if sapped of all their life.", false);
	...
	if(flags[FlagEnum.EXGARTUAN_TIGHTPANTS_MASTURBATE_COUNT] == 0) outputText("  No words are exchanged; no thoughts dare to cross your mind. The only sound is that of your heart, its rhythm acting as prelude for the action you desperately covet.  Mixed emotions of guilt, fright, awe, and satisfaction claw at you.", false);
	//[if occurrence ≥2]
	else if(flags[FlagEnum.EXGARTUAN_TIGHTPANTS_MASTURBATE_COUNT] < 6) outputText("  \"<i>Your will cannot even begin to compare to my might, my helpless little champion. You really shouldn't continue acting as if you're above temptation; I'd hate to see what would happen.</i>\"", false);
	//[if occurrence ≥6]
	else if(flags[FlagEnum.EXGARTUAN_TIGHTPANTS_MASTURBATE_COUNT] >= 6 && flags[FlagEnum.EXGARTUAN_TIGHTPANTS_MASTURBATE_COUNT] < 10) outputText("  \"<i>You know, I never thought I would enjoy even the mere thought of celibacy-not even for a moment.  But watching such an <b>honest</b> champion break down to a quivering pile of helplessness is worth it when the attention I receive afterward is so... thorough.</i>\"", false);
	//[if occurrence ≥10]
	else if(flags[FlagEnum.EXGARTUAN_TIGHTPANTS_MASTURBATE_COUNT] >= 10 && flags[FlagEnum.EXGARTUAN_TIGHTPANTS_MASTURBATE_COUNT] < 12)outputText("\"<i>I can keep this up much longer than you can, " + player.armorName + ".  As much as I enjoy watching you struggle to maintain some form of morality and moderation, I would much rather be exploring the world and its many orifices.  Some day you'll leave that fantasy realm you keep running to and join me in the real world.</i>\"", false);
	//[if occurrence ≥12]
	...
	//[if breasts present]
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText("  Your " + chestDesc() + " work just as hard, smothering Exgartuan in all of their jiggly glory.", false);
	...
	//[if scrotum present]
	if(player.lowerBody.balls > 0) outputText("  Your " + ballsDescriptLight() + " churn with mighty force, roiling in anticipation.", false);
	outputText("\n\n", false);
...
private function boobGartuanSURPRISE():void {
	...
	if(flags[FlagEnum.BOOBGARTUAN_SURPRISE_COUNT] == 0) {
		...
		outputText("\"<i>", false);
		if(player.stats.cor < 33) outputText("Not long enough, Nemus, I'm afraid!", false);
		else if(player.stats.cor < 66) outputText("Too long, honestly.  You've grown slow over the years, Nemus,", false);
		...
		if(player.stats.cor < 33) outputText("I always wondered where you kept this.", false);
		else if(player.stats.cor < 66) outputText("This is one delivery that you'll just have to return to sender, I'm afraid.", false);
...
private function boobgartuanSurprise2():void {
	...
	if(flags[FlagEnum.BOOBGARTUAN_SURPRISE_COUNT] == 0) {
		...
		outputText("\"<i>", false);
		if(player.stats.cor < 33) outputText("Hey, Teensy, remember me?</i>\"", false);
		//[if corruption ≥30 & <70]
		else if(player.stats.cor < 66) outputText("I see you haven't gotten any prettier since last time we met, Teensy.</i>\"", false);
		//[if corruption ≥70]
		...
		//[if corruption <30]
		if(player.stats.cor < 33) outputText("Your " + chestDesc() + " brush against the imp's enormous scrotum, causing him to eventually tumble over from the sensation, blocking the impeding entourage of imps behind you.", false);
		//[if corruption ≥30 & <70]
		else if(player.stats.cor < 66) outputText("As you coast under the beast, you take the opportunity to slap around his crown jewels, playing them as if they were a novelty pair of bongos.  The sensation causes him to tumble over, blocking the impeding entourage of imps behind you.", false);
		...
		//[if corruption <30]
		if(player.stats.cor < 33) outputText("wave goodbye to Nemus", false);
		//[if corruption ≥30 & <70]
		else if(player.stats.cor < 66) outputText("blow a kiss to Nemus", false);
		//[if corruption ≥70]
...
private function boobgartuanSurprise3():void {
	...
	//[if occurrence ≥1]
		...
		//[if occurrence ≥2]
		if(flags[FlagEnum.BOOBGARTUAN_SURPRISE_COUNT] >= 2) outputText("s", false);
	...
	//if occurrence ≥4
	else if(flags[FlagEnum.BOOBGARTUAN_SURPRISE_COUNT] < 8) outputText("</i>\"ARE YOU DENSE, CHAMPION?</i>\" Exgartuan shouts, your mammoth melons bouncing furiously on her every word, \"<i>I will knock you SENSELESS until I clear some space in that vapid cesspool between your ears!</i>\" You want to try and calm her down, but your " + chestDesc() + " promptly launch your face square into the dirt.  \"<i>Don't go on thinking for another SECOND that you can ignore ME!</i>\"", false);
	//[if occurrence ≥8]
	...
	if(player.stats.cor < 50) outputText("  Exgartuan's tormenting has gone on long enough tonight.", false);
	else outputText("  You love foreplay just as much as any other creature, but this is getting ridiculous.", false);
	outputText("\n\n", false);
	
	//[if occurrence ==0]
	if(flags[FlagEnum.BOOBGARTUAN_SURPRISE_COUNT] == 0) outputText("\"<i>Don't think you can overpower me like those puny imps that infest your dreams, champion.</i>\"", false);
	//[if occurrence ≥1]
	else if(flags[FlagEnum.BOOBGARTUAN_SURPRISE_COUNT] < 4)outputText("\"<i>We haven't even started the curriculum yet, champion.</i>\"", false);
	//[occurrence ≥4]
	else if(flags[FlagEnum.BOOBGARTUAN_SURPRISE_COUNT] < 8)outputText("\"<i>HOW can you forget to caress these cans!?  Tease these teats!?</i>\"", false);
	//[if occurrence ≥8]
	...
	//[if occurrence ==0]
	if(flags[FlagEnum.BOOBGARTUAN_SURPRISE_COUNT] == 0) outputText("\"<i>Well, I suppose you've had enough, champion,</i>\" Exgartuan yields, building lust betraying her usual taunting behavior, \"<i>Now why don't we move on to what we both want, hm?</i>\"", false);
	//[if occurrence ≥1]
	else if(flags[FlagEnum.BOOBGARTUAN_SURPRISE_COUNT] < 4) outputText("\"<i>Ready for your final, eh, " + player.short + "?</i>\" Exgartuan teases, a faint lustful waver in her voice contrasting against her dominating demeanor, \"<i>Let's make this one to remember.</i>\"", false);
	//[if occurrence ≥4]
	else if(flags[FlagEnum.BOOBGARTUAN_SURPRISE_COUNT] < 8) outputText("\"<i>Do you think you'll remember now!</i>\" Exgartuan shouts exasperatedly, your breasts pushing against your grasp one final time, \"<i>Now stop wasting my time and get to the only thing you're halfway decent at.</i>\"", false);
	//[if occurrence ≥8]
	...
	//[if corruption <50]
	if(player.stats.cor < 50) outputText("It is becoming increasingly clear to you that she evidently just cannot seem to sit idly by when you actually do decide to pay her heed.", false);
	//[else]
	...
	if(player.gender > 0) {
		...
		//[if cock >0]
		if(player.lowerBody.cockSpot.hasCock() && !player.lowerBody.vaginaSpot.hasVagina()) outputText("pre-cum", false);
		//[if vagina >0]
		if(player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock()) outputText("juice", false);
		//[if cock >0, vagina >0]
		if(player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) outputText("an aromatic cocktail of pre and juice", false);
	...
	//[if corruption <50]
	if(player.stats.cor < 50) outputText("  At this point you are too far gone to resist.", false);
	//[else]
	...
	//[if corruption <50]
	if(player.stats.cor < 50) outputText("  Brought on artificially or not, you cannot deny it; you would probably be cursing the wretched demoness if you were not already preoccupied contending with your mounting passion and desire.", false);
	...
	//[if occurrence ==0]
	if(flags[FlagEnum.BOOBGARTUAN_SURPRISE_COUNT] == 0) outputText("Perhaps instead of dreaming of being fucked by imps and raiding tombs, you'll remember to take care of these luscious love muffins.", false);
	//[if occurrence ≥1]
	else if(flags[FlagEnum.BOOBGARTUAN_SURPRISE_COUNT] < 4) outputText("I guess you're ready to graduate.  Just don't forget what you've learned here today.", false);
	//[if occurrence ≥4]
	else if(flags[FlagEnum.BOOBGARTUAN_SURPRISE_COUNT] < 8) outputText("Never again forget that these tantalizing tits are the most important things in your life, champion.  Or I'll make sure that you remember again.", false);
	//[if occurrence ≥8]
...
public function exgartuanNagaStoleMyMasturbation():void {
	...
	outputText("", true);
	//[if corruption <15]
	if(player.stats.cor < 15) outputText("You sheepishly find some rocks to hide in, where you remove your clothes.  \"Keeping me all to yourself, slut? Hide as much as you want, it'll never keep me down,\" Exgartuan gloats.\n\n", false);
	
	outputText("Irritating... ", false);
	//[if corruption ≥15 & <30]
	if(player.stats.cor <= 25) outputText("You make sure you are alone and strip naked.  Exgartuan chides, \"Yes, champion, wallow around in the shadows.  Keep pretending you can continue to hide my brilliance from the world.\"", false);
	//[if corruption ≥30 & <60]
	else if(player.stats.cor <= 50) outputText("You happily remove your " + player.armorName + ", eager to pleasure yourself.  Your possessed cock pulses happily as you remove the last article.", false);
	//[if corruption ≥60 & <80]
	else if(player.stats.cor <= 75) outputText("You strip naked in an exaggerated fashion, hoping someone might be watching.  Exgartuan begins flooding you with lust in response to your willing inclination.", false);
	//[if corruption ≥80]
	...
	if(player.stats.cor < 33) {
		...
		//[if cocks == 2]
		if(player.lowerBody.cockSpot.count() == 2) outputText(", your remaining tool hiding away inside you, uninvited.", false);
		//[if cocks ≥ 3]
		else if(player.lowerBody.cockSpot.count() >= 3) outputText(", your remaining poles hiding away inside you, uninvited.", false);
		else outputText(".", false);
	...
	if(player.stats.cor > 33 && player.stats.cor < 66) {
		...
		//[if cocks == 2]
		if(player.lowerBody.cockSpot.count() == 2) outputText(", your remaining tool still hiding away inside you, uninvited.", false);
		//[if cocks ≥ 3]
		else if(player.lowerBody.cockSpot.count() > 2) outputText(", your remaining poles still hiding away inside you, uninvited.", false);
	...
	//[else]
		...
		//[if cocks == 2]
		if(player.lowerBody.cockSpot.count() == 2) outputText("  Speaking of resisting, it appears your second tool has been forced to stay in the shed.", false);
		//[if cocks ≥ 3]
		else if(player.lowerBody.cockSpot.count() >= 3) outputText("  Speaking of resisting, it appears your remaining tools have been forced to stay in the shed.", false);
	...
	//[If int > 9]
	if(player.stats.int > 9) outputText("You aren't stupid; you can hazard more than just a guess as to what's about to happen.", false);
	//[else]
	...
	if(player.lowerBody.cockSpot.list[0].cockLength >= player.tallness * .5) outputText("  There's not quite enough of you to fully envelop the horny monument, however.  Its head pokes out of the top of the scaley volcano, continuing to spew its not-quite-so-molten lava.", false);
	//[else]
	...
	//[if corruption <33]
	if(player.stats.cor < 33) outputText("\"<i>You can't measure up to treatment of this magnitude, champion,\" Exgarutan insults you, his voice trembling as the slick massage persists, \"But don't think that I'm giving you permission to slack off.  A slut like you can go on dreaming; one day maybe you'll have the perseverance to get close.\"", false);
	//[if corruption ≥33 & <66]
	else if(player.stats.cor < 66) outputText("Your corrupt cock begins to speak up, the demon's unrelenting motions refusing to ease off, \"You're loving every bit of this champion.  There isn't a hint of uncertainty lingering in your senses.  If only you were this accepting all the time.\"", false);
	//[else]
