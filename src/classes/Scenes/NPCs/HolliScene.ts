	...
	if(flags[FlagEnum.FOUGHT_HOLLI] > 0) {
		...
		if(output) {
			...
			if(flags[FlagEnum.HOLLI_SUBMISSIVE] == 1) {
				outputText("\n\n\"<i>Welcome back, [name].  ");
				if(flags[FlagEnum.TIMES_FUCKED_FLOWER] > 0 && rand(3) == 0) outputText("Did you want to fertilize me again?  My flower is ready and waiting...");
				else if(flags[FlagEnum.TIMES_RIDDEN_FLOWER] > 0 && rand(3) == 0) outputText("Did you want to use my tentacles again?  They've been restless without you...");
				...
				outputText("\n\n\"<i>Welcome back, [name].  ");
				if(flags[FlagEnum.TIMES_FUCKED_FLOWER] > 0 && rand(3) == 0) outputText("Did you want to fertilize me again?  Gardening is such exhausting work, but so rewarding...");
				else if(flags[FlagEnum.TIMES_RIDDEN_FLOWER] > 0 && rand(2) == 0) outputText("Did you come back to play again?  My tentacles are so restless.");
			...
			if(flags[FlagEnum.HOLLI_FRUIT] > 0) {
				...
				//Optional Addition: 
				if(flags[FlagEnum.HOLLI_FRUIT_EXPLAINED] > 0) outputText("  A very small, strange part of you actually feels proud to have made something like that with the tree-woman.");
			}
		}
...
public function getASprout():void {
	...
	if(flags[FlagEnum.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE] == 2) outputText("  She even promised to plant a tree in your camp...");
...
private function fuckFuckingFuckFlowerP2():void {
	...
	if(player.stats.cor < 33) outputText(" and you timidly look around, hoping you haven't attracted an audience.");
	else if(player.stats.cor < 66) outputText(" and you worriedly glance around, hoping yet terrified that someone will walk in on it.");
	...
	if(player.lowerBody.cockSpot.hasSheath()) outputText(" inside your violated sheath");
	...
	if(player.lowerBody.balls > 0) outputText("Your [balls] gurgle, a surprisingly happy sound, and cling tight to your loins.  ");
...
private function rideDatFuckingFukkFlowerP2():void {
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength > 4) outputText("  Your heavy buzzer is so big that the flexible plant-cock has plenty of room to curl around it, and if you could see it, your clit would look almost like a candy-cane (if one set of stripes was jerking off the cane beneath).");
	if(player.lowerBody.cockSpot.count() > 0) outputText("  [EachCock] soon has its own partner twisting around it, a pussy-slicked vine to jerk and twine about its girth.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText("ir");
	...
	if(flags[FlagEnum.PC_FETISH] == 0 && player.stats.cor + player.stats.lib < 150) outputText("embarrassment");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  [EachCock] explodes in release, pumping heavy flows of seed into the petals, even as y");
	else outputText("Y");
...
public function flowerGrowsToP3():void {
	...
	if(flags[FlagEnum.TIMES_FUCKED_FLOWER] + flags[FlagEnum.TIMES_RIDDEN_FLOWER] > 0) outputText("  The fucking probably only helped it to grow faster.");
...
private function fuckTheFlower():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Wetness builds in your [vagina] as well, but you're focusing on your maleness just now.  Your neglected twat will have to wait its turn.");
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() > 2) outputText("es rub");
	...
	if(player.lowerBody.balls > 0) outputText(" with your gurgling balls slapping against the flower's edge");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText(", as your multiple endowments allow you to drizzle the purple bloom's exterior with alabaster cream");
	outputText(".");
...
private function torchP3Tree():void {
	...
	//[(nerdfire)
	if(player.statusAffects.has("KnowsWhitefire")) outputText("Closing your eyes to focus, you gather your energies, and unleash your white, supernatural flames upon the thing.");
	//(bromancefire)
	else if(player.perks.has("FireLord")) outputText("Sucking in your breath, you inflate your chest and stir the embers of the jaguar demon's gift.  Dredging up an enormous pine-green fireball, you exhale and launch it at the tree.");
	...
	if(player.statusAffects.has("KnowsWhitefire")) outputText("magical will");
...
public function treePhaseFourGo():void {
	...
	if(flags[FlagEnum.TIMES_FUCKED_FLOWER] + flags[FlagEnum.TIMES_RIDDEN_FLOWER] > 0) outputText("Did you enjoy feeding me?  You should have seen the expression on your face when you were cumming in me.  Your eyes were rolling back and your mouth was just hanging open.  Do you know what it made me want to do?");
...
public function JojoTransformAndRollOut():void {
	...
	if(player.stats.cor < 25) outputText("Not willing to lie to your friend, you also admit that she seems to have changed after your actions in the factory - that, though you were trying to render the tanks permanently inoperable, the spillage you created pushed the cthonic spirit over the edge.");
...
private function slapAmilysWhoreFace():void {
	...
	if(flags[FlagEnum.AMILY_NOT_FURRY] == 0) outputText("nose");
	...
	if(flags[FlagEnum.AMILY_NOT_FURRY] == 0) outputText("mouse-");
...
private function fuckHolliInZeFlowerPuss():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		if(player.lowerBody.cockSpot.count() == 2) outputText("  Your other penis splays");
	...
	if(player.lowerBody.balls > 0) outputText("cradle your [balls], gently massaging your sack");
	else {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("s, massaging the bundle of cocks with uncommon ferver");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("your own");
...
private function haveAMapleSyrupSnack():void {
	...
	if(flags[FlagEnum.HOLLI_SUBMISSIVE] == 0) outputText("knowingly, Holli asks, \"<i>Come for a little pick-me-up?  Well, go on; drink deeply of my nectar, </i>champion.\"  The last word is inundated with malicious, mocking tones.");
...
private function eatHolliFruit():void {
	...
	if(flags[FlagEnum.HOLLI_FRUIT] == 1) outputText("the fruit");
...
private function askHolliToWatch():void {
	...
	if(flags[FlagEnum.HOLLI_DEFENSE_ON] == 0) {
		...
		if(flags[FlagEnum.THREATENED_HOLLI] > 0) outputText("<b>  You could always just slap the cunt.</b>");
...
private function askBrokenHolliToGuard():void {
	...
	if(flags[FlagEnum.HOLLI_DEFENSE_ON] == 1) outputText("keeping watch");
...
private function holliGetsDickDommed():void {
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 80) outputText(", jaw stretching obscenely");
	outputText(".");
	...
	if(player.lowerBody.balls > 0) outputText("[sack] feel taut and ready to burst");
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 80) outputText("  It bends slightly as the immense girth is forced inside her, aiming down, deep inside the trunk.");
...
private function fullOnTentacleTasticGangBangForHolli():void {
	...
	if(player.lowerBody.balls > 0) outputText("  Your [balls] draw up tight to your groin and squeeze, rhythmically discharging their heavy loads.");
...
private function vaginalDomHollisTentacruels():void {
	...
	if(!player.upperBody.chest.hasFuckableNipples() && player.lactationQ() < 50) outputText(" and pay special attention to your [nipples], tugging them in time with every sluggish thrust in your slit.");
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength >= 3.5) outputText(" for a moment before wrapping around and jerking on it");
	outputText(".");
...
internal function enjoyYourBadEndBIYAAAATCH():void {
	...
	if(player.gender == 1 || (rand(2) == 0 && player.lowerBody.cockSpot.hasCock())) {
		...
		if(player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockThickness > 6) outputText(", stretching her jaw wide open in a way that makes you wince,");
		...
		if(player.lowerBody.balls > 0) outputText(", and your [balls] draw close to your body to unload");
		...
		if(player.lowerBody.balls > 0) outputText("migrating the other direction into your [balls], entangling and infiltrating them, filling them with the same warmth, then pressing out against your sack.  ");
	...
	//branch of vagina-having
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText(" and a glob of precum from [eachCock]");
		outputText(".");
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("s");
		else outputText("chest");
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("  [OneCock] drools precum down its length and onto the flower, triggering another spasm and another gush from your prick.  Holli laughs as you continue to self-stimulate with your uncontrollable fluids.");
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText("wrapping around and under your breasts to keep them permanently jutting out in a lurid display");
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText("your breasts cupped and pressed together by vines, ");
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText(" and [eachCock]");
		...
		if(player.skinType == SKIN.FUR || player.skinType == SKIN.SCALES) outputText(" under your " + player.skinFurScales());
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("folded atop your chest");
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText(", your breasts exposed from the sides,");
...
private function holliPlaysWithPenisesBadEnd():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("masculine shape, it bears");
	...
	if (player.lowerBody.cockSpot.count() > 1) outputText("spraying the girl's stomach and tits with waste seed from " + player.mf("his", "her") + " unused manhood and ");
...
private function girlsGetANiceManToBadEnd():void {
	...
	//[(if tits)
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText("[chest] cupped by vines and held out toward the young man, ");
	else outputText("chest crisscrossed with a mesh of fine vines, ");
	outputText("its legs elevated and spread apart to display a ");
	if(player.lowerBody.balls > 0 && player.lowerBody.ballSize > 10 && player.gender == 3) outputText(multiCockDescriptLight() + " with [balls] swinging below.");
	//(if H and ballsize < 10)
	else if(player.lowerBody.balls > 0 && player.lowerBody.ballSize <= 10 && player.gender == 3) outputText(multiCockDescriptLight() + " and [balls] that hang slightly over a [vagina].");
	//(if H and noballs)
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("[chest]");
	...
	if(player.lowerBody.ballSize > 10 && player.lowerBody.balls > 0) outputText("lifting the hefty testicles to reveal a wet, eager pussy.");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		outputText("; ");
		if(player.lowerBody.balls > 0) outputText("her balls draw closer to her body and ");
		outputText("her trembling dick");
		if(player.lowerBody.cockSpot.count() == 1) outputText(" throbs");
	...
	if(player.lowerBody.cockSpot.count() == 1) outputText("cock");
	else if(player.lowerBody.cockSpot.count() > 1) outputText("cocks");
	else outputText("chest");
...
private function holliAndGenderlessSittingInATree():void {
	...
	if(player.skinType != SKIN.FUR && player.skinType != SKIN.SCALES) outputText(player.skinTone + "-colored");
	...
	if(player.skinType ==SKIN.SCALES ) outputText("a quiver of arousal shakes its scales");
	else if(player.skinType == SKIN.FUR) outputText("its fur begins to prick up with arousal");
