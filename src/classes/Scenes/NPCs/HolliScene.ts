...
public treeMenu(output:boolean = true):void {
	...
	if(flags[FlagEnum.FOUGHT_HOLLI] > 0) {
	...
	else if(flags[FlagEnum.FUCK_FLOWER_LEVEL] == 1) {
	...
	else if(flags[FlagEnum.FUCK_FLOWER_LEVEL] == 2) {
		...
		if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(100) >= 0 && player.lust >= 33) fuck = fuckFuckingFuckFlowerP2;
		
		if(player.lowerBody.vaginaSpot.hasVagina() && player.lust >= 33) ride = rideDatFuckingFukkFlowerP2;
		simpleChoices("Fuck It", fuck, "Ride Stamen", ride, "", null, "Destroy It", destroyDatFuckingPlantAtP2, "Back", inventory.inventoryMenu);
	}
	else if(flags[FlagEnum.FUCK_FLOWER_LEVEL] == 3) {
		...
		if(player.lowerBody.cockSpot.hasCock() && player.lust >= 33) fuck = fuckTheFlower;
		if(player.lowerBody.vaginaSpot.hasVagina() && player.lust >= 33) ride = rideTheWalrusP3;
		...
		if(output) {
			...
			if(flags[FlagEnum.HOLLI_SUBMISSIVE] == 1) {
				MainScreen.text("\n\n\"<i>Welcome back, [name].  ");
				if(flags[FlagEnum.TIMES_FUCKED_FLOWER] > 0 && rand(3) == 0) MainScreen.text("Did you want to fertilize me again?  My flower is ready and waiting...");
				else if(flags[FlagEnum.TIMES_RIDDEN_FLOWER] > 0 && rand(3) == 0) MainScreen.text("Did you want to use my tentacles again?  They've been restless without you...");
				...
				MainScreen.text("\n\n\"<i>Welcome back, [name].  ");
				if(flags[FlagEnum.TIMES_FUCKED_FLOWER] > 0 && rand(3) == 0) MainScreen.text("Did you want to fertilize me again?  Gardening is such exhausting work, but so rewarding...");
				else if(flags[FlagEnum.TIMES_RIDDEN_FLOWER] > 0 && rand(2) == 0) MainScreen.text("Did you come back to play again?  My tentacles are so restless.");
			...
			if(flags[FlagEnum.HOLLI_FRUIT] > 0) {
				if(flags[FlagEnum.HOLLI_FRUIT] == 1) MainScreen.text("\n\nA large, purple fruit hangs from Holli's branches, twisting softly in the breeze.  It looks succulent and ripe.");
				else MainScreen.text("\n\n" + Num2Text(flags[FlagEnum.HOLLI_FRUIT]) + " large, purple fruits hang from Holli's branches, twisting softly in the breeze.  They look succulent and ripe.");
				//Optional Addition: 
				if(flags[FlagEnum.HOLLI_FRUIT_EXPLAINED] > 0) MainScreen.text("  A very small, strange part of you actually feels proud to have made something like that with the tree-woman.");
			}
		}
		if(flags[FlagEnum.HOLLI_SUBMISSIVE] == 0) {
			let eat:Function = null;
			if(flags[FlagEnum.HOLLI_FRUIT] > 0) eat = eatHolliFruit;
			if(player.lowerBody.cockSpot.hasCock() && player.lust >= 33) fuck = fuckHolliInZeFlowerPuss;
			if(player.lowerBody.vaginaSpot.hasVagina() && player.lust >= 33) ride = level4RideHollisTentacruels;
			
			let guardT:string = "";
			if(flags[FlagEnum.HOLLI_DEFENSE_ON] == 1) guardT = "Stop Guarding";
			...
			menu();
			if(player.lowerBody.cockSpot.hasCock() && player.lust >= 33) MainScreen.addButton(0, "Fuck Holli", holliGetsDickDommed);
			if(player.tentacleCocks() >= 10 && player.lust >= 33) MainScreen.addButton(1, "TentacleBone", fullOnTentacleTasticGangBangForHolli);
			if(player.lowerBody.vaginaSpot.hasVagina() && player.lust >= 33) MainScreen.addButton(2, "Ride Holli", vaginalDomHollisTentacruels);
			MainScreen.addButton(3, "Drink Sap", haveAMapleSyrupSnack);
			if(flags[FlagEnum.HOLLI_FRUIT] > 0) MainScreen.addButton(4, "Eat A Fruit", eatHolliFruit);
			MainScreen.addButton(5, "Guarding", askBrokenHolliToGuard);
			MainScreen.addButton(9, "Leave", inventory.inventoryMenu);
		}
		
		if (flags[FlagEnum.FOLLOWER_AT_FARM_HOLLI] == 0 && flags[FlagEnum.FARM_CORRUPTION_STARTED] == 1) MainScreen.addButton(6, "Farm Help", helpWithFarm);
	}
}
...
private helpWithFarm():void
{
	...
	if (flags[FlagEnum.HOLLI_FUCKED_TODAY] == 0)
	{
...
private doFarmFuckery():void
{
	...
	flags[FlagEnum.FOLLOWER_AT_FARM_HOLLI] = 1;
...
private fertilizeHolli(cock:boolean = true):void {
	...
	if(odds >= rand(101)) {
		flags[FlagEnum.HOLLI_FRUIT] += 1 + rand(Math.floor(odds/10));
		trace("Holli pregged!");
	}
}
...
public getASprout():void {
	...
	if(flags[FlagEnum.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE] == 2) MainScreen.text("  She even promised to plant a tree in your camp...");
	...
	flags[FlagEnum.FUCK_FLOWER_LEVEL] = 1;
	flags[FlagEnum.FUCK_FLOWER_GROWTH_COUNTER] = 0;
	//[Yes] [No]
...
private destroyDatFukkinTree():void {
	...
	//(-5 corruption)
	dynStats("cor", -5);
	flags[FlagEnum.FUCK_FLOWER_KILLED] = 1;
	doNext(camp.returnToCampUseOneHour);
}
//[No] (edited)
...
public fuckPlantGrowsToLevel2():void {
	MainScreen.clearText();
	flags[FlagEnum.FUCK_FLOWER_LEVEL] = 2;
	flags[FlagEnum.FUCK_FLOWER_GROWTH_COUNTER] = 0;
...
private fuckFuckingFuckFlowerP2():void {
	...
	MainScreen.text(".  With so much blood trapped in your " + cockDescript(x) + ", it's bigger and harder than ever, twitching valiantly against its restraints with every beat of your heart.");
	...
	player.orgasm();
	dynStats("cor", 1);
	flags[FlagEnum.TIMES_FUCKED_FLOWER] = 1;
	if(flags[FlagEnum.FUCK_FLOWER_GROWTH_COUNTER] < 1000) flags[FlagEnum.FUCK_FLOWER_GROWTH_COUNTER] += 4;
	fatigue(5);
	doNext(camp.returnToCampUseOneHour);
}
...
private rideDatFuckingFukkFlowerP2():void {
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength > 4) MainScreen.text("  Your heavy buzzer is so big that the flexible plant-cock has plenty of room to curl around it, and if you could see it, your clit would look almost like a candy-cane (if one set of stripes was jerking off the cane beneath).");
	...
	if(player.bRows() > 1) MainScreen.text("lowest row of tits, cupping them before groping to your top row of breasts");
	...
	if(flags[FlagEnum.PC_FETISH] == 0 && player.stats.cor + player.stats.lib < 150) MainScreen.text("embarrassment");
	...
	dynStats("cor", 1);
	fatigue(5);
	flags[FlagEnum.TIMES_RIDDEN_FLOWER] = 1;
	if(flags[FlagEnum.FUCK_FLOWER_GROWTH_COUNTER] < 1000) flags[FlagEnum.FUCK_FLOWER_GROWTH_COUNTER] += 4;
	doNext(camp.returnToCampUseOneHour);
}
...
public flowerGrowsToP3():void {
	...
	if(flags[FlagEnum.TIMES_FUCKED_FLOWER] + flags[FlagEnum.TIMES_RIDDEN_FLOWER] > 0) MainScreen.text("  The fucking probably only helped it to grow faster.");
	...
	flags[FlagEnum.FUCK_FLOWER_LEVEL] = 3;
	flags[FlagEnum.FUCK_FLOWER_GROWTH_COUNTER] = 0;
	MainScreen.text(".\n\n<b>What do you do?</b>");
	let fuck:Function = null;
	let ride:Function = null;
...
private fuckTheFlower():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.wetness() < 4) MainScreen.text("leaks");
	...
	dynStats("cor", 1);
	fatigue(5);
	if(flags[FlagEnum.FUCK_FLOWER_GROWTH_COUNTER] < 1000) flags[FlagEnum.FUCK_FLOWER_GROWTH_COUNTER] += 5;
	flags[FlagEnum.TIMES_FUCKED_FLOWER]++;
	flags[FlagEnum.HOLLI_FUCKED_TODAY] = 1;
	doNext(camp.returnToCampUseOneHour);
}
...
private rideTheWalrusP3():void {
	...
	if(flags[FlagEnum.FUCK_FLOWER_GROWTH_COUNTER] < 1000) flags[FlagEnum.FUCK_FLOWER_GROWTH_COUNTER] += 5;
	flags[FlagEnum.TIMES_RIDDEN_FLOWER]++;;
	flags[FlagEnum.HOLLI_FUCKED_TODAY] = 1;
	doNext(camp.returnToCampUseOneHour);
}
...
public treePhaseFourGo():void {
	...
	if(flags[FlagEnum.TIMES_FUCKED_FLOWER] + flags[FlagEnum.TIMES_RIDDEN_FLOWER] > 0) MainScreen.text("Did you enjoy feeding me?  You should have seen the expression on your face when you were cumming in me.  Your eyes were rolling back and your mouth was just hanging open.  Do you know what it made me want to do?");
...
private stayQuietWhileAmilyBitchesAboutFuckingArborday():void {
	...
	flags[FlagEnum.AMILY_TREE_FLIPOUT] = 1;
...
private slapAmilysWhoreFace():void {
	...
	if(flags[FlagEnum.AMILY_NOT_FURRY] == 0) MainScreen.text("nose");
	...
	if(flags[FlagEnum.AMILY_NOT_FURRY] == 0) MainScreen.text("mouse-");
...
private haveAMapleSyrupSnack():void {
	...
	if(flags[FlagEnum.HOLLI_SUBMISSIVE] == 0) MainScreen.text("knowingly, Holli asks, \"<i>Come for a little pick-me-up?  Well, go on; drink deeply of my nectar, </i>champion.\"  The last word is inundated with malicious, mocking tones.");
...
private eatHolliFruit():void {
	...
	if(flags[FlagEnum.HOLLI_FRUIT] == 1) MainScreen.text("the fruit");
	else MainScreen.text("one of the fruits");
	MainScreen.text(".  ");
	if(flags[FlagEnum.HOLLI_FRUIT_EXPLAINED] == 0) {
		...
		flags[FlagEnum.HOLLI_FRUIT_EXPLAINED]++;
	}
	...
	//TF CHANCES
	if(rand(2) == 0 && player.lowerBody.cockSpot.count() > player.tentacleCocks()) {
		let choices:Array = [];
		temp = 0;
		while(temp < player.lowerBody.cockSpot.count()) {
			if(player.lowerBody.cockSpot.list[temp].cockType != CockType.TENTACLE) choices[choices.length] = temp;
			temp++;
		}
		...
		if(player.tentacleCocks() > 0) MainScreen.text("nother");
		MainScreen.text(" tentacle-cock!</b>");
		player.lowerBody.cockSpot.list[temp].cockType = CockType.TENTACLE;
		player.lowerBody.cockSpot.list[temp].knotMultiplier = 1.3;
		dynStats("sen", 3, "lus", 10);
	}
	flags[FlagEnum.HOLLI_FRUIT]--;
	dynStats("cor", 1);
...
private askHolliToWatch():void {
	MainScreen.clearText();
	if(flags[FlagEnum.HOLLI_DEFENSE_ON] == 0) {
		...
		if(flags[FlagEnum.THREATENED_HOLLI] > 0) MainScreen.text("<b>  You could always just slap the cunt.</b>");
		...
		if(flags[FlagEnum.THREATENED_HOLLI] == 0) MainScreen.addButton(1,"Threaten",threatenHolli);
	...
	//Stop Guarding (edited)(C)
		...
		flags[FlagEnum.HOLLI_DEFENSE_ON] = 0;
		doNext(treeMenu);
	}
}
...
private begHolli4Watches():void {
	...
	flags[FlagEnum.HOLLI_DEFENSE_ON] = 1;
	doNext(treeMenu);
}
...
private domUpSomeHolli():void {
	...
	//[Next] - Holli's Menu
	flags[FlagEnum.HOLLI_SUBMISSIVE] = 1;
	treeMenu(false);
}
...
private askBrokenHolliToGuard():void {
	...
	if(flags[FlagEnum.HOLLI_DEFENSE_ON] == 1) MainScreen.text("keeping watch");
	else MainScreen.text("sleeping, instead");
	MainScreen.text(".</i>\"");
	
	//[Guard] [Don't Guard] [Back]
	menu();
	if(flags[FlagEnum.HOLLI_DEFENSE_ON] == 1) MainScreen.addButton(1,"Don't Guard",toggleBrokenHolliGuard);
...
private holliGetsDickDommed():void {
	MainScreen.clearText();
	MainScreen.text("Tossing your [armor] aside, you expose your " + multiCockDescriptLight() + ".  Holli's eyes fixate on [oneCock], her attention rapturously contained by a single phallus.  You grip your maleness in hand and bounce it up and down into your other palm.  The solid slaps of genital impacts echo off the surrounding rocks, each sending a pleasant tingle of pleasure through the root of your manhood.  Before long, you're completely erect and throbbingly hard, bouncing unbidden with each and every thrum of your heart.");
...
private vaginalDomHollisTentacruels():void {
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength >= 3.5) MainScreen.text(" for a moment before wrapping around and jerking on it");
	MainScreen.text(".");
	...
	if(player.wetness() < 4) MainScreen.text("onto the ground");
...
private threatenHolli():void {
	...
	if((player.gender > 0 && player.statusAffects.has("JojoNightWatch") && player.statusAffects.has("PureCampJojo")) || flags[FlagEnum.ANEMONE_WATCH] > 0) {
	...
	flags[FlagEnum.THREATENED_HOLLI] = 1;
	doNext(camp.returnToCampUseOneHour);
}
...
internal function enjoyYourBadEndBIYAAAATCH():void {
	...
	//branch of vagina-having
		...
		if(player.isTaur()) MainScreen.text(" and turning you around");
		...
		MainScreen.text("\n\nSomething ");
		if(!player.isGoo() && !player.isNaga()) MainScreen.text("slips between your thighs and ");
		...
		if(!player.isNaga() && !player.isGoo()) MainScreen.text("your thighs spread wide open, ");
		...
		if(player.ass.analWetness > 0) MainScreen.text("Betraying your depravity, the slick crack opens wide and your greedy, wet asshole practically welcomes the demon's entire hand, sucking it up and making you shiver with repressed delight.  \"<i>Oh, mother!  It seems as though this is not a new lesson to you, is it?  My little whore, will you beg and moan as I fill you?</i>\"");
		...
		//[(iff buttslut)
		if(player.ass.analWetness > 0) MainScreen.text("Try as you might, you can't see far enough down your back to determine what the demon has done.  The question answers itself, though, when she presses a shaft almost as thick as her leg into your [butt], nosing it into your wet, receptive asshole.  Despite yourself, you shiver in anticipation at being filled so completely.  ");
		...
		if(player.ass.analWetness > 0) MainScreen.text("mingling with your own lubrication and ]easing her thrusts.  Holli's tempo picks up quickly, and soon you can feel her twitch inside you as she nears her climax.");
		...
		//[(buttslut or loose)
		if(player.ass.analWetness > 0 || player.analCapacity() > 30) MainScreen.text("spilling a wave of her deposited fluids down your [legs].  ");
		...
		if(player.skinType == SKIN.FUR || player.skinType == SKIN.SCALES) MainScreen.text(" under your " + player.skinFurScales());
...
private holliPlaysWithPenisesBadEnd():void {
	...
	MainScreen.text(multiCockDescript() + ", standing erect and hard as iron; the brown-haired girl blushes and looks away.");
...
private girlsGetANiceManToBadEnd():void {
	...
	if(player.lowerBody.balls > 0 && player.lowerBody.ballSize > 10 && player.gender == 3) MainScreen.text(multiCockDescriptLight() + " with [balls] swinging below.");
	//(if H and ballsize < 10)
	else if(player.lowerBody.balls > 0 && player.lowerBody.ballSize <= 10 && player.gender == 3) MainScreen.text(multiCockDescriptLight() + " and [balls] that hang slightly over a [vagina].");
	//(if H and noballs)
	else if(player.gender == 3) MainScreen.text(multiCockDescriptLight() + " and a [vagina].");
	...
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("With a nervous glance downward, he slides one hand to her [cock smallest].  \"<i>It's not gay if it's a chick's,</i>\" he mumbles to himself, and then begins to stroke haltingly, playing his finger along the urethra and drawing a squirt of precum from his shivering partner.  ");
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength < 6) MainScreen.text("he smears it over the little button and then begins to trace up and down the underside with a finger.  ");
	...
	//(vagina and no cock)
		...
		MainScreen.text("\n\nA ");
		if(player.wetness() < 3) MainScreen.text("gush");
		else if(player.wetness() < 4) MainScreen.text("rush");
		else MainScreen.text("torrent");
...
private holliAndGenderlessSittingInATree():void {
	...
	if(player.skinType != SKIN.FUR && player.skinType != SKIN.SCALES) MainScreen.text(player.skinTone + "-colored");
	...
	if(player.skinType ==SKIN.SCALES ) MainScreen.text("a quiver of arousal shakes its scales");
	else if(player.skinType == SKIN.FUR) MainScreen.text("its fur begins to prick up with arousal");
