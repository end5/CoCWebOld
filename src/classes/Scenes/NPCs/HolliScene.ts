...
public function treeMenu(output:boolean = true):void {
	...
	if(flags[FlagEnum.FOUGHT_HOLLI] > 0) {
		...
		Render.text("The demon tree is still here, and the face peeking from it still stares daggers at you even from a distance.  It looks like forgiveness won't be forthcoming.");
		...
		if(output) {
			...
			if(flags[FlagEnum.HOLLI_SUBMISSIVE] == 1) {
				Render.text("\n\n\"<i>Welcome back, [name].  ");
				if(flags[FlagEnum.TIMES_FUCKED_FLOWER] > 0 && rand(3) == 0) Render.text("Did you want to fertilize me again?  My flower is ready and waiting...");
				else if(flags[FlagEnum.TIMES_RIDDEN_FLOWER] > 0 && rand(3) == 0) Render.text("Did you want to use my tentacles again?  They've been restless without you...");
				else if(rand(2) == 0) Render.text("Did you come for a drink?  I've built up lots of nectar for you.  Surely, a sip would be refreshing for you...");
				else Render.text("How may I serve you?  Come, give me a command so that I can fulfill it for you.");
				Render.text("</i>\"");
				
			}
			else {
				Render.text("\n\n\"<i>Welcome back, [name].  ");
				if(flags[FlagEnum.TIMES_FUCKED_FLOWER] > 0 && rand(3) == 0) Render.text("Did you want to fertilize me again?  Gardening is such exhausting work, but so rewarding...");
				else if(flags[FlagEnum.TIMES_RIDDEN_FLOWER] > 0 && rand(2) == 0) Render.text("Did you come back to play again?  My tentacles are so restless.");
				else if(rand(2) == 0) Render.text("Did you come for a drink?  I've built up lots of nectar for you.  Surely a hungry adventurer like yourself needs a sip, right?");
				else Render.text("Do you want to play?  Come on, taste Mother Marae's finest creation.");
				Render.text("</i>\"");
			}
			//Extra line on description:  
			if(flags[FlagEnum.HOLLI_FRUIT] > 0) {
				if(flags[FlagEnum.HOLLI_FRUIT] == 1) Render.text("\n\nA large, purple fruit hangs from Holli's branches, twisting softly in the breeze.  It looks succulent and ripe.");
				...
				//Optional Addition: 
				if(flags[FlagEnum.HOLLI_FRUIT_EXPLAINED] > 0) Render.text("  A very small, strange part of you actually feels proud to have made something like that with the tree-woman.");
			}
		}
		if(flags[FlagEnum.HOLLI_SUBMISSIVE] == 0) {
			...
			menu();
			if(player.lowerBody.cockSpot.hasCock() && player.lust >= 33) addButton(0, "Fuck Holli", holliGetsDickDommed);
			if(player.tentacleCocks() >= 10 && player.lust >= 33) addButton(1, "TentacleBone", fullOnTentacleTasticGangBangForHolli);
			if(player.lowerBody.vaginaSpot.hasVagina() && player.lust >= 33) addButton(2, "Ride Holli", vaginalDomHollisTentacruels);
			addButton(3, "Drink Sap", haveAMapleSyrupSnack);
			if(flags[FlagEnum.HOLLI_FRUIT] > 0) addButton(4, "Eat A Fruit", eatHolliFruit);
			addButton(5, "Guarding", askBrokenHolliToGuard);
			addButton(9, "Leave", inventory.inventoryMenu);
		}
		
		if (flags[FlagEnum.FOLLOWER_AT_FARM_HOLLI] == 0 && flags[FlagEnum.FARM_CORRUPTION_STARTED] == 1) addButton(6, "Farm Help", helpWithFarm);
	}
}
...
private function helpWithFarm():void
{
	...
	Render.text("You ask if a plant goddess might not be able to help the farm you recently acquired.");
...
private function noPlzDontFuckWithFarm():void
{
	...
	Render.text("“<i>Such a fickle mortal,</i>” she sighs. “<i>But I am a kind goddess. Ask me for it anytime, assuming you’ve... performed the rites.</i>”");
...
public function getASprout():void {
	...
	if(flags[FlagEnum.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE] == 2) Render.text("  She even promised to plant a tree in your camp...");
	Render.text("  You look closer and see purple veins on the undersides of the leaves, throbbing with what can only be the tainted liquids that suffuse much of this planet.  Fully grown, it might spread its corruption to your camp.\n\n<b>Do you destroy it?</b>");
...
private function letZeFuckingSproutLive():void {
	clearOutput();
	Render.text("Looking down at the sapling, you stay your wrath.  It may be corrupt, but it hasn't done anything to harm you just yet.  You give it a little pat on the uppermost leaves and leave it be.  It's not like it's going anywhere.");
...
public function fuckPlantGrowsToLevel2():void {
	...
	if(player.gender == 0) Render.text("  Sucks to be it!  You don't waste time with stupid shit, like genitals.");
	Render.text("  Is this Marae's idea of a gift?");
	if(player.gender > 0) Render.text("  Well, you could give it a whirl... just being around it seems to slowly turn you on.");
	Render.text("  Of course, destroying it would be the safest option.");
...
private function fuckFuckingFuckFlowerP2():void {
	...
	Render.text(" and form a sticky puddle.  Now moaning, you grab the stem and start to pump the sheath up and down, using the plant like the cock-toy it was meant to be.  Lewd, wet squishes ring out through the air,");
	if(player.stats.cor < 33) Render.text(" and you timidly look around, hoping you haven't attracted an audience.");
	else if(player.stats.cor < 66) Render.text(" and you worriedly glance around, hoping yet terrified that someone will walk in on it.");
	else Render.text(" and you glance around, hoping someone will walk in on the sexy show you're making.");
	...
	if(player.lowerBody.cockSpot.hasSheath()) Render.text(" inside your violated sheath");
	Render.text(".  With so much blood trapped in your " + cockDescript(x) + ", it's bigger and harder than ever, twitching valiantly against its restraints with every beat of your heart.");
	...
	if(player.lowerBody.balls > 0) Render.text("Your [balls] gurgle, a surprisingly happy sound, and cling tight to your loins.  ");
	Render.text("Inside you, you can feel the liquid heat of your spurting release exploding out, melting everything before it with liquid, orgasmic bliss.  A low moan bursts from your lips when the sounding tentacle pulls out of your sloppy cock-slot, and the rushing tide of bubbling cum follows it, bursting into the sweet, suckling nectar-pot.");
	if(player.cumQ() > 500) Render.text("  You pour forth your prodigious essence without pause or fail, and the plant immediately begins to drink it down, the stem bulging with rounded balls of sperm that it carries underground.");
	if(player.cumQ() > 1500) Render.text("  Soon, the meager stem cannot swallow it all.  Spunk froths and squirts out around the base of your tool for a moment, and then a veritable tide of the stuff gushes out to soak  you.");
...
private function rideDatFuckingFukkFlowerP2():void {
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength > 4) Render.text("  Your heavy buzzer is so big that the flexible plant-cock has plenty of room to curl around it, and if you could see it, your clit would look almost like a candy-cane (if one set of stripes was jerking off the cane beneath).");
	if(player.lowerBody.cockSpot.count() > 0) Render.text("  [EachCock] soon has its own partner twisting around it, a pussy-slicked vine to jerk and twine about its girth.");
	Render.text("\n\nWith nothing to occupy your roving hands, you let them slide up your " + player.skinFurScales() + " to your ");
	if(player.bRows() > 1) Render.text("lowest row of tits, cupping them before groping to your top row of breasts");
	else Render.text("[chest]");
	Render.text(", caressing the");
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) Render.text("ir");
	Render.text(" swell as you search for the lust-tightened skin of your [nipples].  They seem almost as sensitive as little clits, and as soon as you find them, you immediately set to circling them slowly, occasionally tugging and twisting when a particularly powerful sensation works through your pleasure-wracked body.");
	if(player.lactationQ() > 50) {
		Render.text("  One after the other, they begin to ");
		if(player.lactationQ() < 150) Render.text("trickle");
		else if(player.lactationQ() < 500) Render.text("leak");
		else Render.text("spray torrents of");
		Render.text(" milk.");
	}
	Render.text("  You moan in unabashed delight, your mouth hanging open in a mindless, blissful 'o' in between vocalizations.  If your friends back home could see you now, writhing in the dirt, face open in simple, animal pleasure, and perhaps worst of all, fucking a plant, you'd die of ");
	if(flags[FlagEnum.PC_FETISH] == 0 && player.stats.cor + player.stats.lib < 150) Render.text("embarrassment");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  [EachCock] explodes in release, pumping heavy flows of seed into the petals, even as y");
	else Render.text("Y");
	Render.text("our quivering pussy, already sopping wet, releases the last of your juices in a soaked ");
	if(player.cumQ() < 50) Render.text("drizzle");
	else if(player.cumQ() < 250) Render.text("stream");
	else if(player.cumQ() < 750) Render.text("spray");
	else Render.text("torrent");
	Render.text(".  The tentacles inside you curl in on themselves and slowly pull back from the well-creamed quim, scraping out as much fluid as possible from your oversensitive honey-pot.  The binding on your [clit] releases, which of course sets off a whole new wave of rippling pleasure-contractions in your cunt.");
...
public function flowerGrowsToP3():void {
	clearOutput();
	Render.text("Again, you hear odd noises coming from a corner of your camp - the corner where you let that tainted shrubbery grow.");
	if(flags[FlagEnum.TIMES_FUCKED_FLOWER] + flags[FlagEnum.TIMES_RIDDEN_FLOWER] > 0) Render.text("  The fucking probably only helped it to grow faster.");
	Render.text("  The distinct sound of wood bowing and creaking under new weight catches your ears, and well before you can get to the plant, you can see a leafy canopy stretching towards the sky.  <b>It's grown into a small tree, with bark, leaves, and everything!</b>  Warily, you round one of the ancient standing stones to get a better look.");
	...
	let burnIt:Function = null;
	if(player.perks.has("Dragonfire") || player.perks.has("FireLord") || player.perks.has("Hellfire") || player.statusAffects.has("KnowsWhitefire")) {
		Render.text(", though you suppose you could burn it down with magical fire if you set your mind to it");
		burnIt = torchP3Tree;
	}
...
private function fuckTheFlower():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Wetness builds in your [vagina] as well, but you're focusing on your maleness just now.  Your neglected twat will have to wait its turn.");
	Render.text("  A feverish sweat breaks out on your brow, though whether from your own arousal, the plant's heat, or some chemical in its juices, you cannot tell.");
	...
	if(player.cockArea(x) >= 150) Render.text(", sliding down the trunk as easily if the entire inside was a giant pussy just waiting to get fucked");
	Render.text(".");
	if(player.lowerBody.cockSpot.count() > 1) {
		Render.text("  Your other penis");
		if(player.lowerBody.cockSpot.count() > 2) Render.text("es rub");
		else Render.text(" rubs");
		Render.text(" sensually across the moist outer blossom, drooling their clear pre-cum over the petals.");
	}
	Render.text("  Schliiick.  You slide out.  Slorp-pop.  You slide in, this time to the ");
	if(player.lowerBody.cockSpot.hasSheath()) Render.text("sheath");
	else Render.text("base");
	if(player.lowerBody.balls > 0) Render.text(" with your gurgling balls slapping against the flower's edge");
	Render.text(", accompanied by a small squirt of sweet lubricants.");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text(", as your multiple endowments allow you to drizzle the purple bloom's exterior with alabaster cream");
	Render.text(".");
	if(player.cumQ() > 500) Render.text("  Pearly ropes big enough to fill a flask lurch out from your cum-slit, propelled by your quivering orgasmic muscles deep into the trunk, and you swear you can hear the pitch of your sexual pumping rising in tone, changed by the flooded vessel.");
	if(player.cumQ() > 1500) Render.text("  A wave of your thick gunk rolls out from under your pumping cock - you've filled the tree beyond even its capacity, and the thick puddle forming beneath your [feet] is all the proof you need of your virility.");
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("  Your hungry cunt, long denied in this exchange, trembles as it ");
		if(player.wetness() < 4) Render.text("leaks");
		else Render.text("sprays");
		Render.text(" your own female fluids in sympathy.");
	}
	Render.text("  After what feels like forever, the spurting seed no longer flows from your long-abused dick, and as your member softens, the tentacles swing you away, setting you down softly.");
...
private function drinkThePlantGirlsSap():void {
	...
	if(silly()) Render.text(", tree hugger that you are");
	Render.text(".  The thick 'milk' quickly fills your body with energy, though it runs out nearly as soon as it started.");
...
private function rideTheWalrusP3():void {
	...
	Render.text("You eye the tentacles hiding amongst the leafy branches above, writhing obscenely around each other as they snake through the foilage.  You shed your [armor] into a worthless pile and bare your body, cupping your [chest] enticingly as you whistle up at the tree.  Your calls seem to fall on deaf ears; the whirling vines stay put up in their perch.  They're ignoring your delicious, feminine flesh!  Irritated, you stomp up to the trunk's flower and plunge your fist inside it.  Fresh fluid gushes out over your armor, releasing a spray of sweet, musky lubricants.  You blindly feel around inside the pliant plant-pussy with your fingers, giggling to yourself whenever the soft cavity clenches tightly to your arm.  Whatever this is, it can feel pleasure.");
...
private function torchP3Tree():void {
	...
	Render.text("This has gone on long enough!  This thing cannot continue to grow in your camp any longer, and you have just the means to remove it: fire.  ");
	//[(nerdfire)
	if(player.statusAffects.has("KnowsWhitefire")) Render.text("Closing your eyes to focus, you gather your energies, and unleash your white, supernatural flames upon the thing.");
	//(bromancefire)
	else if(player.perks.has("FireLord")) Render.text("Sucking in your breath, you inflate your chest and stir the embers of the jaguar demon's gift.  Dredging up an enormous pine-green fireball, you exhale and launch it at the tree.");
	else Render.text("Sucking in your breath, you inflate your chest and stir the embers of your fiery gift.  Dredging up an enormous fireball, you exhale and launch it at the tree.");
	Render.text("  It goes up by the torch, though the fire starts to fade sooner than you would expect.  You huff, but repeat the action, layering a new blaze atop the previous one; you put all your concentration into this new conflagration, stoking the inferno with every ounce of your ");
	if(player.statusAffects.has("KnowsWhitefire")) Render.text("magical will");
	else Render.text("breath");
	Render.text(".");
...
public function treePhaseFourGo():void {
	...
	if(flags[FlagEnum.TIMES_FUCKED_FLOWER] + flags[FlagEnum.TIMES_RIDDEN_FLOWER] > 0) Render.text("Did you enjoy feeding me?  You should have seen the expression on your face when you were cumming in me.  Your eyes were rolling back and your mouth was just hanging open.  Do you know what it made me want to do?");
	else Render.text("Did you enjoy watching me grow?  You should have fed me.  It was so much harder to grow without proper nutrients.  Do you know what I thought about during all the time I spent waiting?  Do you know what it made me want to do?");
	Render.text("</i>\"");
...
public function JojoTransformAndRollOut():void {
	...
	Render.text("\"<i>[name], come here please.</i>\"");
	...
	if(player.stats.cor < 25) Render.text("Not willing to lie to your friend, you also admit that she seems to have changed after your actions in the factory - that, though you were trying to render the tanks permanently inoperable, the spillage you created pushed the cthonic spirit over the edge.");
	else Render.text("Though, you do leave out the precise, needless details of what caused the change.");
	Render.text("  The monk's eyes widen in surprise and surmise.");

	Render.text("\n\n\"<i>Unbelievable...</i>\" he blurts.  \"<i>");
	if(flags[FlagEnum.TIMES_FUCKED_FLOWER] + flags[FlagEnum.TIMES_RIDDEN_FLOWER] == 0) Render.text("To think that I must now consider Marae herself my enemy.  Forgive me, but this place is not safe for us anymore if it has attracted the attention of such a creature.  For now, I will return to the forest.  I know that you believe your mission keeps you here, but I urge you to stay away from that thing and watch the portal from an adequate distance.");
	//(totally deflowered that flower)
	else Render.text("And even knowing that this could be the case, you still chose to fornicate with it?  [name]... I cannot say that I am not disappointed.  This place is not safe for me anymore.  For now, I will return to the forest.");
	Render.text("</i>\"");
	
	Render.text("Jojo looks at you sadly; any argument you try to make is met with a resigned shake of his head.  \"<i>If the creature truly is the demonic offspring of a corrupted earth spirit, doubtless it will be powerful and the wrath of its parent severe.");
	if(player.level < 20) Render.text("  Nonetheless, if you find yourself in a position where you must confront this demon or perish, call upon me.  I will render what aid I can, though I prefer it not be in vain.");
	else Render.text("  Though, a mortal of your stature and resources may be able to destroy her and escape the reprisal.  I would counsel against direct confrontation until you are sure of victory, but if you must fight, then call upon me and you will have what aid I can give.");
	Render.text("</i>\"  So saying, he presses a small talisman into your hand.");
	
	Render.text("\"<i>Until then, you will find me in the forest if you need my assistance.</i>\"  Jojo makes a polite bow and departs.");
	Render.text("(<b>Jojo is no longer a companion!</b>)");
...
public function amilyHatesTreeFucking():void {
	clearOutput();
	Render.text("Your mousey cohabitant tramps up to you, irritation plain on her expression.");
...
private function stayQuietWhileAmilyBitchesAboutFuckingArborday():void {
	clearOutput();
	Render.text("Amily berates you for some while longer, then eyeballs you.  Just as you begin to speak, she cuts you off again.");
...
private function slapAmilysWhoreFace():void {
	clearOutput();
	Render.text("You slap Amily across the ");
	if(flags[FlagEnum.AMILY_NOT_FURRY] == 0) Render.text("nose");
	...
	if(flags[FlagEnum.AMILY_NOT_FURRY] == 0) Render.text("mouse-");
	Render.text("girl says, eyes wet with tears, \"<i>but I don't.  Goodbye.</i>\"");
...
private function fuckHolliInZeFlowerPuss():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		if(player.lowerBody.cockSpot.count() == 2) Render.text("  Your other penis splays");
		else Render.text("The rest of your penises splay");
		Render.text(" to the side, smearing furrows of the stuff around as they slide over the butter-smooth skin.");
	}
	Render.text(" The demon dryad's hand reaches below to ");
	if(player.lowerBody.balls > 0) Render.text("cradle your [balls], gently massaging your sack");
	else {
		Render.text("gently caress your ");
		if(player.lowerBody.cockSpot.hasSheath()) Render.text("sheath");
		else Render.text("loins");
	}
	Render.text(" encouragingly.  You respond by edging your [hips] upward, more effectively presenting your shaft to be pleasured.");
	
	Render.text("\n\nSighing happily, the immobile beauty ");
	if(player.cockArea(x) < 100) Render.text("leans down and kisses the tip, opening her glossy green puckers to slurp it down, sucking just hard enough to make your " + player.cockHead(x) + " tingle with erotic warmth.");
	else Render.text("kisses up and down your huge, towering shaft, slobbering her glossy green puckers all over your urethral bulge, making it even more of a wet mess.");
	Render.text("  You grab hold of the edges of her tits and squeeze, smushing them tight around yourself and sliding them up and down.  The plant-woman's twinkling golden eyes meet your own while she pleasures your " + cockDescript(x) + ", and her hands come to rest over top of your own, assisting you with the quick tit-fuck.");
	...
	Render.text(", slapping wetly against you.  Soon, the slick texture of her soft melons has you throbbing eagerly and dribbling hot trails of pre-cum.  You're going to blow any second now, and the hot bloom of pleasure within you is spreading further and further.  Those gold and black eyes look up imploringly while the pale green cheeks hollow hungrily, and you release ecstatically, arching your back as you pound the heavy tits down your length.");
	
	Render.text("\n\nA geyser of spunk rockets out into ");
	if(player.cockArea(x) >= 100) Render.text("the air, where it hangs for a moment before splattering down atop the plant-girl's leaf-colored hair and face, forming a gossamer shroud.");
	else Render.text("her mouth, which works noisily to swallow the hot load even as you inject it into her.  After a few swallows, she leans back and lets you shoot it up into the air, so that it can fall down over her face and hair, into a gossamer shroud.");
	//highish cum:  
	if(player.cumQ() >= 500) Render.text("  You keep blasting heavy ropes of seed until her body and trunk are painted with goo, and then you cum some more, smattering spunk until she looks more like a gooey waterfall than a fey creature.");
	if(player.cumQ() >= 1500) Render.text("  Before you know it, there's a lake around the two of you and a small river running downhill away from camp.");
	Render.text("  A mischievous grin breaks out under the goopy facial, followed by an unnaturally long tongue that pulls gobs of the stuff into the dryad's hungry mouth.  Once she can see again, the girl gives her tits a squeeze and affectionately kisses your cock, practically worshipping it with sloppy-sweet licks.");
	...
	if(player.tallness < 60) Render.text("tits");
	else Render.text("arms");
	Render.text(".  [EachCock], instead of slamming into the bark painfully, slots into something warm and wet, pulsating and pliant.  A clingy, tight hold that ripples gently around your member");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s, massaging the bundle of cocks with uncommon ferver");
	else Render.text(", massing your tool with uncommon fervor");
	Render.text(".  Your grunt of irritation vanishes as soon as your tender sex is forced into the heavenly hole, and with the soft breasts pressing into ");
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) Render.text("your own");
	else Render.text("your [chest]");
	Render.text(", you give up and kiss the dryad full on the lips, biting the lower one roughly in irritation.");
...
private function haveAMapleSyrupSnack():void {
	clearOutput();
	Render.text("You approach Holli with a hungry smile plastered across your [face], licking your chops as you eye the arboreal slut with unrestrained desire.  Of course, your gaze fixes on her heavy chest, with its distended, dripping nipples.  They leak sweet sap as you stare; the demon seems eager to feel your mouth on her slick, almond-colored buds.  Looking at you ");
	if(flags[FlagEnum.HOLLI_SUBMISSIVE] == 0) Render.text("knowingly, Holli asks, \"<i>Come for a little pick-me-up?  Well, go on; drink deeply of my nectar, </i>champion.\"  The last word is inundated with malicious, mocking tones.");
	else Render.text("eagerly, Holli asks, \"<i>Care for a little pick-me-up?  They're getting very engorged, and I'm sure it would be a refreshing snack for you.</i>\"");
...
private function level4RideHollisTentacruels():void {
	...
	if(silly()) Render.text("  She proceeds to sing about Jack Sparrow's awesome adventures for a solid ten minutes, then goes on as if nothing had happened.");
	...
	if(player.lactationQ() == 0) Render.text("droplets of milk dripping from your nipples.  When did you start lactating?");
	else Render.text("steady streams of milk erupting from your nipples.  When did you start lactating this hard?");
	...
	if(!player.isBiped()) Render.text("you'd be walking bowlegged for a week, if you had legs.");
	else Render.text("you'll be walking bowlegged for a week!");
...
private function eatHolliFruit():void {
	clearOutput();
	Render.text("You reach up into Holli's branches and pluck ");
	if(flags[FlagEnum.HOLLI_FRUIT] == 1) Render.text("the fruit");
	else Render.text("one of the fruits");
	Render.text(".  ");
	if(flags[FlagEnum.HOLLI_FRUIT_EXPLAINED] == 0) {
		Render.text("Holli smiles over at you and asks, \"<i>Enjoying the fruits of our union?</i>\"");
	...
	Render.text("Biting into it, sweet juices seem to explode from the flesh, dribbling down your chin.  It tastes like a dessert and you chow down, happily munching away.  In no time flat, you're down to just a core.  You toss it and wipe your [face] clean, then burp.  Damn, that was good!");
	//TF CHANCES
	if(rand(2) == 0 && player.lowerBody.cockSpot.count() > player.tentacleCocks()) {
		...
		if(player.tentacleCocks() > 0) Render.text("nother");
		Render.text(" tentacle-cock!</b>");
...
private function askHolliToWatch():void {
	...
	if(flags[FlagEnum.HOLLI_DEFENSE_ON] == 0) {
		...
		if(flags[FlagEnum.THREATENED_HOLLI] > 0) Render.text("<b>  You could always just slap the cunt.</b>");
		...
		if(flags[FlagEnum.THREATENED_HOLLI] == 0) addButton(1,"Threaten",threatenHolli);
	...
	//Stop Guarding (edited)(C)
		...
		Render.text("You tell Holli you'd like her to stop watching at night.  She laughs, \"<i>You just want a chance to come back and beg some more, don't you?  I thought you were supposed to be tough shit, not a simpering little submissive.</i>\"");
...
private function begHolli4Watches():void {
	clearOutput();
	Render.text("You swallow your pride and drop down prostrate on the ground.  Sighing, you grudgingly say, \"<i>Please, Holli, guard my camp at night.  I don't want to be raped by monsters.</i>\"");
...
private function domUpSomeHolli():void {
	clearOutput();
	flags[FlagEnum.HOLLI_FUCKED_TODAY] = 1;
	Render.text("You swagger up to the tree and put your hand on the bark beside Holli.  Glancing over, she quips, \"<i>Is that supposed to be intimidating?</i>\"  The dryad smirks ingratiatingly.  \"<i>I'm the offshoot of a demon goddess.  I don't think you thought this through.</i>\"");
...
private function askBrokenHolliToGuard():void {
	clearOutput();
	Render.text("Holli bows her head when you mention her guarding camp.  She asks, \"<i>Shall I keep watch for foes to drain or let them into camp for you to have the pleasure of dealing with, [name]?  <b>I've been ");
	if(flags[FlagEnum.HOLLI_DEFENSE_ON] == 1) Render.text("keeping watch");
	else Render.text("sleeping, instead");
	Render.text(".</i>\"");
	
	//[Guard] [Don't Guard] [Back]
	menu();
	if(flags[FlagEnum.HOLLI_DEFENSE_ON] == 1) addButton(1,"Don't Guard",toggleBrokenHolliGuard);
...
private function toggleBrokenHolliGuard():void {
	clearOutput();
	if(flags[FlagEnum.HOLLI_DEFENSE_ON] == 0) {
		Render.text("\"<i>All right, I'll get to have some fun then,</i>\" Holli quips while rubbing her palms together in anticipation.");
		Render.text("\n\nYou suppose you can sleep better with your pet demoness taking on the lesser whelps of this land.");
		flags[FlagEnum.HOLLI_DEFENSE_ON] = 1;
	}
	//Guard Off
	else {
		Render.text("\"<i>All right, they're yours to smite or play with,</i>\" Holli says with a wink.  You give her a glare, to which she responds with a shrug.  \"<i>What?  I know how it is!  You mortals can't fight worth a damn when you're sleeping.</i>\"");
...
private function holliGetsDickDommed():void {
	clearOutput();
	Render.text("Tossing your [armor] aside, you expose your " + multiCockDescriptLight() + ".  Holli's eyes fixate on [oneCock], her attention rapturously contained by a single phallus.  You grip your maleness in hand and bounce it up and down into your other palm.  The solid slaps of genital impacts echo off the surrounding rocks, each sending a pleasant tingle of pleasure through the root of your manhood.  Before long, you're completely erect and throbbingly hard, bouncing unbidden with each and every thrum of your heart.");
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 80) Render.text(", jaw stretching obscenely");
	Render.text(".");
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() <= 80) {
		Render.text(", swallowed to the ");
		if(player.lowerBody.balls > 0) Render.text("[balls]");
		else Render.text("[sheath]");
		Render.text(".");
	}
	else Render.text(", swallowed as deep as Holli's distended neck can handle.");
	Render.text("  Gurgling, the demonic dryad flutters her lengthy eyelashes and looks up at your eyes, twinkling with happiness at her servitude.  You wiggle your crotch around, and it twists your [cock biggest] around her mouth and throat.  The tainted tree's tongue massages the underside of your length, embracing you with subtle moisture and frictionless pressure.");
	Render.text("\n\nBubbling sexual pleasure begins to build up inside you, making your ");
	if(player.lowerBody.balls > 0) Render.text("[sack] feel taut and ready to burst");
	else Render.text("body feel strung tight like an over-tuned guitar");
	Render.text(".  Holli pulls back, inciting a snarl of displeasure to vibrate from you, but as soon as she's withdrawn, her hands clasp onto the slick shaft and begin to pump it with vigorous strokes.  Her mouth settles back into place around your [cockHead biggest], circling it with her tongue while she bobs up and down on it, snaring your most sensitive place with overwhelming sensations.");
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 80) Render.text("  It bends slightly as the immense girth is forced inside her, aiming down, deep inside the trunk.");
	Render.text("  Holli flops into your [chest], twitching weakly and breathing heavily.  \"<i>Fffuck, so good!</i>\" she pants, nuzzling her cheek into your collarbone while her hands play across your shoulders.  You ruthlessly mount her flowery mound with fast motions.  Juices squirt out around your member at the apex of every stroke, raining down over the surrounding ground.  You grab Holli's deep green hair and pull her back.  Her face is wrecked by bliss, her mouth and eyes moving in uncoordinated twitches.");
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		Render.text("  Excess jism spurts onto Holli's jiggling udders from below, spotting the viridian mounds with white.");
		if(player.cumQ() >= 500) Render.text("  The constant sprays turn them white in little time, dripping from the peaks of her nipples along with her sticky sap.");
	}
	if(player.cumQ() > 1000) Render.text("  A trickle of sperm washes out of the demoness's entrance, somehow escaping the heavy suction.");
	if(player.cumQ() > 2000) Render.text("  A moment later, that trickle turns into a river.");
	if(player.cumQ() > 3000) Render.text("  You swear that her trunk must be absolutely stuffed with spunk.");
	if(player.cumQ() > 5000) Render.text("  A spooge-spout sprays out as the pressure goes through the roof, confirming your suspicions in the messiest way possible.");
	Render.text("  Holli's tentacles cum with her, spraying her seed through her canopy in aimless ropes.  Most of it sticks to her branches and leaves, but small amounts of it trickle down to spatter the ground.  One of them lands on the back of your hand.  You frown and push it up against the tree-woman's mouth.  She licks you clean without question, still dazed from her orgasm.");
...
private function fullOnTentacleTasticGangBangForHolli():void {
	clearOutput();
	flags[FlagEnum.HOLLI_FUCKED_TODAY] = 1;
	Render.text("You remove your [armor] and expose your writhing mass of wriggling tendrils to the tree-tart.  She places a hand over her mouth and exhales, \"<i>Oh my, Mother Marae sure knows how to pick them, doesn't she?</i>\"");
	...
	if(silly()) Render.text(", inflatable-arm, flailing tube-cocks");
	Render.text(".  Below the orgy of arboreal affection, Holli's slick petals have spread wide.  You can actually see her interior twitching voraciously.  It opens and closes with pulsing hunger, vibrantly engorged.  As you watch, the fourth of your members lances forward of its own volition, spearing straight for that welcoming hole with no guidance from you.  It hits like a missile.  Two feet of that cock is gone in a moment, swallowed deep in the cunt-flower's bottomless depths.  A gush of girl-cum spurts out around the intruding member, and you can feel the muffled moan vibrate through the mouth-bound cock as she gives in to the four-way fucking.");
	...
	if(player.lowerBody.balls > 0) Render.text("  Your [balls] draw up tight to your groin and squeeze, rhythmically discharging their heavy loads.");
...
private function vaginalDomHollisTentacruels():void {
	...
	if(!player.upperBody.chest.hasFuckableNipples() && player.lactationQ() < 50) Render.text(" and pay special attention to your [nipples], tugging them in time with every sluggish thrust in your slit.");
	else if(player.lactationQ() >= 50) {
		Render.text(" and sigh at the ");
		if(player.lactationQ() < 150) Render.text("squirts");
		else if(player.lactationQ() < 800) Render.text("sprays");
		else if(player.lactationQ() < 2000) Render.text("outpouring");
		else Render.text("thick fountains");
		Render.text(" of milk that escape with every tug, timed to match the sluggish thrusts at your slit.");
	}
	else Render.text(" and sink your fingers into your [nipples], fucking them to the sluggish tempo of the thrusting tentacle.");
	Render.text("  A loud groan escapes your mouth, and you rock your [hips] against the undulating tendril.  Your body's eagerness for more pleasure is unparalleled, but you control yourself as efficiently as you control the dryad.");
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength >= 3.5) Render.text(" for a moment before wrapping around and jerking on it");
	Render.text(".");
...
private function threatenHolli():void {
	clearOutput();
	Render.text("You snarl and threaten to burn the barked bitch to the ground if she won't make herself useful after you ask politely.");
	...
	if(player.level < 30) Render.text("Mighty as you are, Marae is likely still beyond your power.  Perhaps you should just ignore this demon for the time being.");
	else Render.text("Marae is likely beyond your power yet, but... you feel like she might not pressure you too hard if you can make a suitably credible threat.");
...
private function cutHerDown():void {
	clearOutput();
	Render.text("\"<i>Are you serious?  You really want to be broken that badly, my little toy?</i>\"  The demon folds her arms over her hefty chest, her earlier, suggestive posture completely absent now.  \"<i>You know my bark is as strong as any armor, right?  That Marae herself - the land itself - bolsters me?  Take that back right now, and I'll let you return to my good graces... if you put your face in the dirt and plead for the duty to pleasure me whenever I want.</i>\"");
...
internal function defeatHolli():void {
	...
	if(monster.findStatusAffect(StatusAffects.JojoIsAssisting) < 0) {
		Render.text("Weary, the succubus godling folds into her tree, unwilling to allow you the sight of her defeat.  With her energy suppressed, you're free to ");
		if(player.weaponName != "large axe") Render.text("fetch an axe and ");
		Render.text("hack at her trunk, laboriously peeling bark and making wedge-shaped cuts in it as you try to fell the abomination.  It takes nearly eight hours, but eventually the gnarled tree topples.  No sooner does it fall than it shrivels and turns to ash.");
		temp = 7;
		while(temp > 0) {
			temp--;
			model.time.hours++;
	...
	//Win with Jojo -Z
		...
		Render.text("The fire-blackened mouse cheers at you as the succubus groans and retreats completely into her bark.  \"<i>Well done, [name]!  Help me carry tinder and we can burn this abomination to the ground!</i>\"");
...
private function recruitJojoToCamp():void {
	clearOutput();
	Render.text("Jojo nods eagerly at you when you invite him to share your camp.  \"<i>Excellent idea.  If you'll excuse me, I wish to attempt to purify the spot where the creature grew, and then I will fetch my things.</i>\"");
...
private function dontRecruitJojoToCamp():void {
	clearOutput();
	Render.text("You hold your tongue, allowing Jojo to make his way back to the forest.");
	Render.text("\n\n(Key Item Gained: Holli's Ashes)");
	player.createKeyItem("Holli's Ashes",0,0,0,0);
	cleanupAfterCombat();
}
...
internal function enjoyYourBadEndBIYAAAATCH():void {
	...
	if(player.gender == 1 || (rand(2) == 0 && player.lowerBody.cockSpot.hasCock())) {
		...
		if(player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockThickness > 6) Render.text(", stretching her jaw wide open in a way that makes you wince,");
		Render.text(" engulfs you.  Holli's tongue plays over your slit and she looks up at you for just a moment to be sure you're watching.");
		...
		if(player.biggestCockLength() > 24) Render.text("her neck ripples and extends grotesquely, and her head begins to migrate down all " + num2Text(Math.round(player.biggestCockLength())) + " of your prick with alarming speed, pushing the stem inside along with it.  ");
		else Render.text("she sinks her head down to the base of your prick in one quick thrust, bringing the stem with it.  ");
		Render.text("The sensation of having your entire [cock biggest] penetrated and sucked into her hot, wet, clinging throat pushes you back toward the edge and over, and it literally becomes a race for Holli to reach bottom before your nerves fire.  Her wet lips impact your crotch with a lewd splatter, but her tongue keeps going deep into your body, stopping only once it reaches your prostate.  A small bulge fires down the thin stem, forcing through your slit and into the shaft to lodge in your gland.  Holli's mouth and tongue withdraw once it is in place, and she grabs your cock in her hand, masturbating you vigorously.");
		
		Render.text("\n\nIt's too much; your climax comes in a flood");
		if(player.lowerBody.balls > 0) Render.text(", and your [balls] draw close to your body to unload");
		Render.text(".  Your eyes roll as you flex and flex, but nothing comes out.  As you conclude your dry orgasm, you begin to feel a pressure on your prostate.");
		...
		if(player.lowerBody.balls > 0) Render.text("migrating the other direction into your [balls], entangling and infiltrating them, filling them with the same warmth, then pressing out against your sack.  ");
		else Render.text("exploring the depths of your body and curling back against the surface of your " + player.skin() + ".  ");
		Render.text("Your stomach turns as the sprouts push their way through, crawling from your crotch down the outside of your [legs] to root in the dry earth.  The shoots thicken into fibrous roots, forming a trunk that supports you and pushes you closer to Holli and lower to the ground, until your writhing, tentacle-tipped prick is lined up with her pussy flower and within her easy reach.");
	...
	//branch of vagina-having
		...
		if(player.isTaur()) Render.text(" and turning you around");
		Render.text(", the demon buries her face in your [vagina], penetrating it with her wet, flexible tongue.");
		...
		if(player.lowerBody.cockSpot.hasCock()) Render.text("  [OneCock] drools precum down its length and onto the flower, triggering another spasm and another gush from your prick.  Holli laughs as you continue to self-stimulate with your uncontrollable fluids.");
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) Render.text("wrapping around and under your breasts to keep them permanently jutting out in a lurid display");
		else Render.text("tracing a lattice of slender, criss-crossing ivy over your chest that hangs just far enough over your [nipples] that a stiff breeze will drag it into them");
		Render.text(", then continuing over your back and up your neck.  The tendrils thicken and multiply, occluding your forearms and lower [legs] in a casing of young bark and leaf buds, and Holli's own release you as yours take over the duty of holding you in place to receive whatever pleasure she forces upon you.  You're left in a salacious pose when the growth subsides, suspended several feet off the ground with your arms held over your head, your back arched in a memorial of your incredible orgasm, ");
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) Render.text("your breasts cupped and pressed together by vines, ");
		if(!player.isNaga() && !player.isGoo()) Render.text("your thighs spread wide open, ");
		Render.text("and your nectar-drooling flower-sex exposed with your [clit]");
		if(player.lowerBody.cockSpot.hasCock()) Render.text(" and [eachCock]");
		Render.text(" bobbing in a state of permanent semi-arousal.");
		...
		if(player.ass.analWetness > 0) Render.text("Betraying your depravity, the slick crack opens wide and your greedy, wet asshole practically welcomes the demon's entire hand, sucking it up and making you shiver with repressed delight.  \"<i>Oh, mother!  It seems as though this is not a new lesson to you, is it?  My little whore, will you beg and moan as I fill you?</i>\"");
		else if(player.analCapacity() < 30) Render.text("With some difficulty, she forces it into your pucker.  \"<i>I think we'll just use this as our inroads, instead.  It may be slow going, but the journey is half the fun.</i>\"");
		else Render.text("Despite your clenching, your loose hole proves no impediment to the demon's exploration.  \"<i>Ah, how roomy you are!  I think I'll move right in.</i>\"");
		...
		//[(iff buttslut)
		if(player.ass.analWetness > 0) Render.text("Try as you might, you can't see far enough down your back to determine what the demon has done.  The question answers itself, though, when she presses a shaft almost as thick as her leg into your [butt], nosing it into your wet, receptive asshole.  Despite yourself, you shiver in anticipation at being filled so completely.  ");
		else if(player.analCapacity() < 30) Render.text("Your jaw hits your collarbone as Holli angles the florid head of a long, narrow penis into your field of view.  Judging by how far it comes up her chest, it's almost a foot and half long, and it's drooling sticky sap all over your back.  With a twisted smile, the demon pushes it along your spine until it reaches your [butt], then shoves it toward the hole.  ");
		else Render.text("The demon presses up to your back, brushing her nipples against your skin and shoving her new growth between your thighs.  Looking down, you can see a fat, green shaft sticking from your crotch, glistening and dripping with sticky slime.  With another moan, Holli pulls it back from between your legs, then pushes apart your asscheeks and plunges it into your [asshole].  ");
		Render.text("The sap oozing from her member coats your passage, ");
		if(player.ass.analWetness > 0) Render.text("mingling with your own lubrication and ]easing her thrusts.  Holli's tempo picks up quickly, and soon you can feel her twitch inside you as she nears her climax.");
		...
		//[(buttslut or loose)
		if(player.ass.analWetness > 0 || player.analCapacity() > 30) Render.text("spilling a wave of her deposited fluids down your [legs].  ");
		else Render.text("and your once-tight pucker reflexively squeezes shut as best it can, trapping all but a few trickles of Holli's deposit inside you.  ");
		Render.text("The freshly-grown shaft between Holli's legs begins to shrivel, withering and browning like a cut flower.  With an expression of mild disdain, she pulls it from her body and casts it aside, where it crumbles to pieces.");
		
		Render.text("\n\nThe fluids in your gut begin to mingle, sending a disquieting pinprick sensation through the skin");
		if(player.skinType == SKIN.FUR || player.skinType == SKIN.SCALES) Render.text(" under your " + player.skinFurScales());
		Render.text(".  Panic intensifies as the sensation quickly focuses on your crotch, and when the tendrils holding you push you away from the demon's tree, your curiosity gets the better of you.  You look over your shoulder again, trying to see what's happening.  As you strain to watch, you feel hundreds of tiny, sharp pinches and small vines begin to creep from your body, just below your ass.  The vines merge and thread into thick trunks as they get closer to the ground, pushing deep into the dirt and mooring you to the spot.  Arousal builds in you as they join the web of roots linking the demon Marae and Holli, as does a swelling at your crotch.  Another cluster of thin vines push free from your groin, growing upward in a lattice and lashing your arms into place, ");
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) Render.text("folded atop your chest");
		else Render.text("crooked at your sides with your hands resting atop your [chest]");
		Render.text(".  They continue past your face, reaching over your head to form a canopy similar to Holli's own.  Struggle as you may, when they're done growing you're left unable to budge and locked in a rather artful pose, head turned over your shoulder with your [ass] thrust out");
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) Render.text(", your breasts exposed from the sides,");
		Render.text(" and your [legs] concealed below your buttcheeks by a thick tree trunk.");
...
private function holliPlaysWithPenisesBadEnd():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) Render.text("masculine shape, it bears");
	else Render.text("feminine shape, it nonetheless bears");
	Render.text(multiCockDescript() + ", standing erect and hard as iron; the brown-haired girl blushes and looks away.");
	...
	if(player.smallestCockArea() > 36) Render.text("  As the dryad speaks, the silent figure's [cock smallest] shudders and begins to shrink, dwindling to a size that would fit " + player.mf("him","her") + " prospective lover.");
	...
	if (player.lowerBody.cockSpot.count() > 1) Render.text("spraying the girl's stomach and tits with waste seed from " + player.mf("his", "her") + " unused manhood and ");
	Render.text(" filling the Champion's pussy with so much semen it cascades from her stretched hole as Holli continues to pump.");
	...
	Render.text(" drooling aftershocks onto her breasts.");
...
private function girlsGetANiceManToBadEnd():void {
	...
	//[(if tits)
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) Render.text("[chest] cupped by vines and held out toward the young man, ");
	else Render.text("chest crisscrossed with a mesh of fine vines, ");
	Render.text("its legs elevated and spread apart to display a ");
	if(player.lowerBody.balls > 0 && player.lowerBody.ballSize > 10 && player.gender == 3) Render.text(multiCockDescriptLight() + " with [balls] swinging below.");
	//(if H and ballsize < 10)
	else if(player.lowerBody.balls > 0 && player.lowerBody.ballSize <= 10 && player.gender == 3) Render.text(multiCockDescriptLight() + " and [balls] that hang slightly over a [vagina].");
	//(if H and noballs)
	else if(player.gender == 3) Render.text(multiCockDescriptLight() + " and a [vagina].");
	else Render.text("a wet, drooling cunt that practically begs to be filled.");
	Render.text("  Though its eyes are covered, the figure's mouth works constantly, biting and licking its lips as if beset by otherworldly amounts of lust.  Holli beckons the youth toward this tree.");
	...
	if(player.lowerBody.ballSize > 10 && player.lowerBody.balls > 0) Render.text("lifting the hefty testicles to reveal a wet, eager pussy.");
	else Render.text("spreading the labia and exposing the eager, clenching hole inside.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) {
		Render.text("reaches up and grabs two handfuls of breast, passing his thumb over a nipple and watching the ");
		if(player.gender == 2) Render.text("woman");
		else Render.text("hermaphrodite");
		Render.text("'s mouth open and close in bliss.  ");
	}
	else Render.text("leans down and kisses the [woman/hermaphrodite]'s smooth chest, causing her to arch her back in pleasure.  ");
	if(player.lowerBody.cockSpot.hasCock()) Render.text("With a nervous glance downward, he slides one hand to her [cock smallest].  \"<i>It's not gay if it's a chick's,</i>\" he mumbles to himself, and then begins to stroke haltingly, playing his finger along the urethra and drawing a squirt of precum from his shivering partner.  ");
	else {
		Render.text("His hand slides to the [clit] above her wet fuck-hole; collecting some of the frothing lube from their wet sex, ");
		if(player.lowerBody.vaginaSpot.list[0].clitLength < 6) Render.text("he smears it over the little button and then begins to trace up and down the underside with a finger.  ");
		else Render.text("he coats the swollen organ with it and begins to jerk it off valiantly, like a cock.  ");
		Render.text("\"<i>Does that feel good?</i>\" he asks, upping the pace of his thrusts.  ");
	}
	Render.text("As if in reply, the ");
	if(player.gender == 2) Render.text("woman");
	else Render.text("hermaphrodite");
	Render.text(" in the tree arches her back and her [vagina] clamps down on his tool");
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("; ");
		if(player.lowerBody.balls > 0) Render.text("her balls draw closer to her body and ");
		Render.text("her trembling dick");
		if(player.lowerBody.cockSpot.count() == 1) Render.text(" throbs");
		else Render.text("s throb");
		Render.text(" as she nears climax");
	}
	Render.text(".");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" toward her face; ");
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() < 20) {
			Render.text("semen spurts and dribbles down her [chest], pooling in the combined laps and mingling with the sticky lube ringing her vagina.  ");
			if(player.cumQ() >= 500) Render.text("So much so that by the time she finishes her climax, both her partner's legs and her trunk are covered in a sheen of milky white seed.  ");
		}
		else if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() < 80) {
			Render.text("her mouth is now full of cockhead, and she gulps as eagerly as she spurts, drooling lines of white from the corners of her lips.  ");
			//[(big skeet)]
			if(player.cumQ() >= 500) Render.text("The waves of seed don't stop coming, pouring into her and swelling her belly out until its round bulge pushes the cock away from her mouth and the remaining squirts fall on her [chest].  ");
		}
		//else cock biggest > 80)
		else {
			Render.text("her body is easily dwarfed by her male sex's massive size, and globs of sperm shoot into her canopy to splatter among her leaves and branches.  ");
			if(player.cumQ() >= 1000) Render.text("The overproductive hermaphrodite's body pushes out so much seed that the entire canopy soon turns white and drips constantly as though it were in a tropical rainforest.  ");
		}
	}
	//(vagina and no cock)
	else {
		Render.text("\n\nA ");
		if(player.wetness() < 3) Render.text("gush");
		else if(player.wetness() < 4) Render.text("rush");
		else Render.text("torrent");
		Render.text(" of female orgasm pours from her pussy, and her mouth opens wide in pleasure.  The tree branches surrounding the young man seem to close in, pushing him toward her.  Getting the message, he leans in and kisses his lover; her tongue probes his mouth with abandon as her body shakes out its fit and her [vagina] wrings his prick.  ");
	}
	Render.text("Pushed past his limit by her lewd display, the young man groans, shudders, and begins to unload his pent-up cargo.  Falling on top of his partner's slick ");
	if(player.lowerBody.cockSpot.count() == 1) Render.text("cock");
	else if(player.lowerBody.cockSpot.count() > 1) Render.text("cocks");
	else Render.text("chest");
	Render.text(", he goes limp and slack-jawed as his dick squirts, filling her vagina.");
	...
	Render.text("'s stomach.  \"<i>In fact, she seems to have enjoyed it so much that she would have me bestow a blessing on you.</i>\"");
...
private function holliAndGenderlessSittingInATree():void {
	...
	if(player.skinType != SKIN.FUR && player.skinType != SKIN.SCALES) Render.text(player.skinTone + "-colored");
	else Render.text("covered in " + player.skinFurScales());
	Render.text(" and wrapping its body around the tree in a way that makes determining its sex impossible.  No... it's not wrapping around the tree, but embedded in it up to the hips!");
	...
	if(player.skinType ==SKIN.SCALES ) Render.text("a quiver of arousal shakes its scales");
	else if(player.skinType == SKIN.FUR) Render.text("its fur begins to prick up with arousal");
	else Render.text("a blush of lust colors its " + player.skinFurScales());
	Render.text("...");
...
public function amilyComesBack():void {
	clearOutput();
	Render.text("Amily arrives with her belongings over her shoulder and a smile on her face.  \"<i>I knew you'd do the right thing, [name].  I'll get my nest set back up.</i>\"\n\n(<b>Amily has moved back in!  She can be found in the lovers tab.</b>)");
	//flags[FlagEnum.AMILY_TREE_FLIPOUT] = 1;
