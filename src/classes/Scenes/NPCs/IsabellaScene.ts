...
public function isabellaGreeting():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00257] == 0) {
		...
		if(player.statusAffects.has("Marble")) outputText("She's like Marble - she has a human face with horns and cow-like ears.", false);
	...
	//Camp Meeting – Was welcome tall, but not short yet!
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			...
			if(player.lowerBody.cockSpot.list[player.shortestCockIndex()].cockLength < 9) {
				...
				if(player.lowerBody.cockSpot.list[player.shortestCockIndex()].cockLength < 20) outputText("large", false);
	...
	//Camp Meeting – Welcomed Short but Not Tall
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			...
			if(player.lowerBody.cockSpot.list[player.shortestCockIndex()].cockLength >= 9) outputText("before sighing wistfully.", false);
			else {
	...
	//Camp Meeting – Standard Repeat
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			...
			if(player.lowerBody.cockSpot.list[player.shortestCockIndex()].cockLength >= 9) outputText(", sighing wistfully.", false);
			else {
...
public function nomOnMommaIzzysTits():void {
	...
	if(player.tallness > 78 || player.isTaur()) {
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			...
			if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 40) outputText("hard, barely constraining " + sMultiCockDesc() + ".", false);
			else outputText("visibly from " + sMultiCockDesc() + ".", false);
			outputText("  ", false);
		}
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			if(player.lowerBody.cockSpot.hasCock()) outputText("Even y", false);
		...
		if(player.horns > 0 && player.hornType > HORNS.NONE) outputText("your horns", false);
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("  " + SMultiCockDesc() + " twitches and drips from the intensity of the kiss, but you had completely forgotten about your sexual needs while you were drinking.", false);
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your " + vaginaDescript(0) + " aches with need and desire from the intensity of the kiss, but you had completely forgotten it during the feeding.", false);
		...
		if(player.upperBody.wingType > WING.NONE && player.upperBody.wingType != WING.SHARK_FIN) outputText(" and stroke your wings", false);
		...
		if(player.lowerBody.cockSpot.hasCock() && x >= 0) {
			...
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00261] == 0) outputText("  S-she fucked you in your sleep?  That explains how good your dreams were.  On one hand you feel a little violated, but on the other you have to wonder how long this woman has held her camp against the demons with nothing to please her.", false);
	...
	//Voluntary Nursing (Small Characters)
		...
		if(player.lowerBody.cockSpot.hasCock() && x >= 0) {
			...
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00261] == 0) {
				...
				if(player.lowerBody.balls > 0) outputText("balls feel", false);
	...
	if(rand(2) == 0 && player.lowerBody.vaginaSpot.hasVagina()) outputText(player.modTone(0,4), false);
	doNext(camp.returnToCampUseOneHour);
}
...
public function izzyGivesSmallWangsFreeOral():void {
	...
	if(player.stats.cor < 33) outputText("blush hard and tell Isabella that she can lick if she wants to.", false);
	else if(player.stats.cor < 66) outputText("blush and tell Isabella that she can definitely give you a lick.", false);
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 50) {
			...
			if(player.lowerBody.cockSpot.count() > 2) outputText("s", false);
	...
	if(player.stats.cor < 50) outputText("before blushing, ashamed by your wanton behavior but not really wanting it to end.", false);
	...
	if(player.stats.sens < 50) {
		...
		if(player.lowerBody.balls > 0) outputText("She cups your " + ballsDescriptLight() + " and begins caressing the twitching orbs, giving them gentle squeezes each time the pleasure forces them to involuntarily contract towards your groin.", false);
			...
			if(player.lowerBody.vaginaSpot.list[0].clitLength < 3) outputText("gentle squeezes", false);
	...
	//(High sensitivity fail!) 
		...
		if(player.lowerBody.cockSpot.hasSheath()) outputText("around your sheath", false);
		else if(player.lowerBody.balls > 0) outputText("above your balls", false);
		...
		if(player.lowerBody.balls > 0) outputText("her hand stroking and gently squeezing at your " + ballsDescriptLight() + " as if she could milk a bigger load from them that way.", false);
		else if(player.lowerBody.cockSpot.count() > 1) outputText("her hand stroking and squeezing your neglected " + cockDescript(1) + " almost as an afterthought.", false);
...
public function volunteerToSlurpCowCunt():void {
	...
	if(player.stats.cor < 33) outputText("Hesitantly, you start to lean closer and closer until you're mere inches from the cow-girl's slick box.  The air practically fogs with her anticipation.  A strong, impatient hand grabs you by the back of the head and pushes you forward, burying your nose and lips into her wet, squelching lips.", false);
	else if(player.stats.cor < 66) outputText("Eagerly, you start to lean further and further forward until you're less than an inch from the cow-girl's slick folds, and the air fogs with lusty anticipation.  Before you can dive in, her hand grips the back of your head and FORCES you inside her.  Her greedy lips swallow your nose and lips with one wet squelch.", false);
	...
	if(player.stats.cor < 33) outputText("You blush at her words and hope she's just talking dirty.", false);
	else if(player.stats.cor < 66) outputText("You blush at her words and wonder what it would be like if she wasn't talking dirty.", false);
...
public function IsabellaWinsAndSpanks():void {
	...
	if(player.tallness <= 78) {
		...
		if(player.perks.has("Masochist")) outputText(" and pleasure", false);
		...
		if(player.perks.has("Masochist")) {
			...
			if(player.lowerBody.cockSpot.hasCock()) outputText("squirt pre-cum ", false);
			else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("slick your thighs ", false);
		...
		if(player.perks.has("Masochist")) outputText("excitedly, wondering just how many more smacks you'll get to take", false);
		...
		if(player.perks.has("Masochist")) outputText(" while you swoon and moan, wiggling your rump", false);
		...
		if(!player.perks.has("Masochist")) outputText("draws a scream from your lips, pushing you past the bounds of consciousness.  Isabella hefts your limp form like a wet noodle and grasses sway behind you as you're carried off.", false);
		else {
	...
	//[Lose And Get Spanked – Tall]
		...
		if(!player.perks.has("Masochist")) outputText("piteously", false);
		...
		outputText("\"<i>Bad <SMACK> " + player.mf("boy","girl") + "! <CRACK> Learn <SLAP> your <SWAT> lesson!</i>\"  Her sentence is punctuated with one bone-jarring blow that", false);
		if(player.perks.has("Masochist")) outputText(" draws a scream from your lips, pushing you past the bounds of consciousness.  Isabella hefts your limp form heavily, dragging you through the dirt as your eyes close.", false);
		else {
			outputText(" pushes you past your limit.", false);
			if(player.lowerBody.cockSpot.hasCock()) {
...
public function isabellaRapesYouWithHerAss():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		{
			outputText("  The bovine bitch reaches down to swat at some of your " + Appearance.cockNoun(CockType.HUMAN) + "s, pushing them out of the way.  \"<i>So gross.  Why ");
	...
	if(player.lowerBody.balls > 0) outputText(" or the tightening of your " + sackDescript() + " as it pulls your "+ ballsDescriptLight(), false);
	...
	if(player.cockThatFits(38) >= 0) {
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText(", smushing against your other dick", false);
		if(player.lowerBody.cockSpot.count() > 2) outputText("s", false);
		outputText(".\n\n", false);
		...
		if(player.lowerBody.balls > 0) outputText("Involuntary muscles contract, tugging your " + sackDescript() + " up against your crotch while your " + ballsDescriptLight() + " visibly contract, loading your body full of liquid lust that's about to explode.", false);
	...
	//[TOOBIG]
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			if(player.lowerBody.cockSpot.count()) outputText("Even though they're being ignored, each time her cheeks crush against your other cocks, they squeeze out a few spurts of pre-cum.", false);
		...
		if(player.lowerBody.balls > 0) outputText("Your " + sackDescript() + " pulls tight against your body, each of your " + ballsDescriptLight() + " quivering and pushing its load through you.", false);
		...
		if(player.lowerBody.balls == 0) outputText("crotch", false);
...
public function IsabellaPostSpankFeedSex():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("on your chest", false);
	...
	if(player.tongueType == TONUGE.HUMAN) outputText("smaller", false);
...
public function victoryLactation69():void {
	...
	if(player.upperBody.chest.count() > 1) outputText("top row of ", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("squeeze " + sMultiCockDesc() + " gently, feeling the heavy, turgid mass pulsating with each beat of your heart.", false);
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("rub your fingers over the puffy lips of your pussy, feeling your vulva spread to give the juice-lubed digits access.", false);
	...
	if(player.upperBody.chest.count() > 1) outputText(", while your other " + breastDescript(1) + " plasters her crimson hair into her scalp", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Of course, you're pumping at your own quim nearly as hard, but the messy cow-girl's efforts still manage to drown you out.", false);
	else if(player.lowerBody.cockSpot.hasCock()) outputText("  Of course, you're pumping at " + sMultiCockDesc() + " just as hard, but the sounds of your fapping are easily drowned out by the enthusiastic cow-girl.", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(multiCockDescriptLight(), false);
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript(), false);
	...
	//(Cuntnips) 
	if(player.upperBody.chest.hasFuckableNipples()) outputText("  Isabella thrusts her tongue at your " + nippleDescript(0) + ", burrowing it deep into the unusual passage to get at the last of your liquid bounty and inadvertently intensifying your pleasure.  Your arm gives out and you drop on top of her, burying her in tit while hers serve as your pillows.", false);
	//(Cock & Puss)
	if(player.gender == 3) {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.balls > 0) outputText(", your " + ballsDescriptLight() + " pulling tight against your groin", false);
...
public function PCVictoryOnIsabellaButtsex():void {
	...
	if(player.lust < 70) {
		...
		if(player.lowerBody.cockSpot.count() == 1) outputText(".  It seems", false);
...
public function victoryAgainstIzzzzzySixtyNine():void {
	...
	if(player.gender == 2) {
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength >= 5) outputText("struggling with the cock-like appendage", false);
		else if(player.lowerBody.vaginaSpot.list[0].clitLength >= 2) outputText("suckling the large button with slow, measured inhalations", false);
	...
	//(WANGS - fits)
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			...
			if(player.lowerBody.cockSpot.count() > 2) outputText("s as they smear her cheeks, focusing on this one, perfectly-matched specimen.", false);
	...
	//(WANGS – no fit)
		...
		else if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText(", getting stuck at the medial ring", false);
		else if(player.lowerBody.cockSpot.list[x].cockType == CockType.DEMON) outputText(", slowed by all the nubs", false);
		else if(player.lowerBody.cockSpot.list[x].cockType == CockType.CAT) outputText(", slowed by all the spines", false);
		else if(player.lowerBody.cockSpot.list[x].cockType == CockType.TENTACLE) outputText(", stopping under the mushroom-like head", false);
		else if(player.lowerBody.cockSpot.list[x].cockType == CockType.ANEMONE) outputText(", giggling from the 'stings' your anemone-like prick gave her", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("twitching as your member disentangles itself from her tongue.", false);
...
public function tooBigVictoryTittyFuckingFuntimesWithMilk():void {
	...
	outputText("\"<i>", false);
	if(player.stats.cor < 33) outputText("I'm sorry, but you're too hot to resist, and it's better I do this than try to force it in a hole that could never accept it,", false);
	...
	if(player.stats.cor < 33) outputText("You take care to remove it without damaging the garment, as if respect for property will make up for slaking your lusts on her unwilling body.", false);
	else if(player.stats.cor < 66) outputText("You remove the garment without much care for its condition, not damaging it too much as you wrench it free.", false);
	...
	if(player.lowerBody.cockSpot.list[x].cockLength < 50) outputText("burying the top few inches of your length into her mouth", false);
	...
	if(player.lowerBody.cockSpot.list[x].cockLength < 50) outputText("to her mouth", false);
	...
	if(player.lowerBody.balls > 0) outputText("  Your balls pull tight to your loins, accompanied by a burst of warmth", false);
	...
	if(player.lowerBody.cockSpot.list[x].cockLength < 50) outputText("leans forward to swallow more of your cock just before", false);
...
public function tinyVictoryTittyFuckingFuntimesWithMilk():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText(", singling the smallest one out for some reason", false);
	...
	if(player.stats.cor < 33) outputText("bringing a shameful blush to your cheeks", false);
	else if(player.stats.cor < 66) outputText("bringing a tiny blush to your cheeks", false);
