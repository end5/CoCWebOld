...
public function meetSophie():void {
	...
	if(player.stats.cor < 33) outputText("In the interest of politeness, ", false);
	else if(player.stats.cor < 66) outputText("Barely remembering your manners, ", false);
...
private function sophieMeetingChoseSex():void {
	...
	if(player.totalCocks() == 0) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" and insists, \"<i>Well, keep looking; if I wanted a girl I'd be busy with one of my nieces right now.</i>\"", false);
...
private function cramANippleInIt():void {
	...
	if(player.upperBody.chest.count() > 1) outputText("top-", false);
	...
	if(player.biggestLactation() < 2) {
		...
		if(player.upperBody.chest.countNipples() > 2) {
			...
			if(player.upperBody.chest.countNipples() < 5) outputText("before finishing the last one.", false);
	...
	//(Medium PG)
		...
		if(player.totalBreasts() > 2) {
			if(player.upperBody.chest.countNipples() > 2) outputText("She drinks from each of the nipples that adorn your " + allBreastsDescript() + " until her belly is full and gurgling.", false);
			else outputText("She drinks from each breast in turn until her belly is full and gurgling.", false);
		}
		else if(player.upperBody.chest.countNipples() > 2) outputText("She drinks from each nipple in turn until her belly is full and gurgling.", false);
		outputText("\n\n", false);
	}
	//(High PG) 
	else {
		...
		if(player.upperBody.chest.countNipples()) outputText("other ", false);
		else outputText("next ", false);
		outputText("one and begins the cycle anew.", false);
		if(player.upperBody.chest.countNipples() >= 4) outputText("  With all your nipples, this goes on for quite some time.", false);
		outputText("\n\n", false);
	}
	...
	if(player.stats.cor > 60) outputText("  You give her wide ass a playful slap and start climbing down before she can retaliate.", false);
	dynStats("lus", -50);
...
private function consensualHotSophieDickings():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("her breasts rubbing against your own", false);
	else {
	...
	if(player.stats.cor < 75) outputText("making you feel terrible and shamed.  Your entire body flushes with embarrassment", false);
	...
	if(player.lowerBody.balls == 0) outputText("prostate", false);
	...
	else if(flags[FlagEnum.FUCKED_SOPHIE_COUNTER] <= 2) outputText("You really like it when I milk you don't you?  That must be why you keep coming back for more.", false);
...
private function postSophieSexSnuggle():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("all of ", false);
	...
	if(player.stats.cor > 50) outputText("nearly give her a good-bye kiss, but catch yourself at the last moment.  She quips, \"<i>Too bad, it was nice.</i>\"\n\n", false);
...
private function consensualSophieSexNoFit():void {
	...
	if(player.tallness <= 48) outputText("her breasts pressed into your face", false);
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("own", false);
	...
	if(player.stats.cor < 50) outputText("Her teases and taunts sting far more than they should, making you feel terrible and shamed.  Your entire body flushes with embarrassment ", false);
	...
	if(player.lowerBody.balls > 0) outputText("your " + ballsDescriptLight() + ", careful not to snag them with her talons", false);
	...
	if(player.lowerBody.balls > 0) outputText(" and your balls churn", false);
...
private function maleVictorySophieRapeHUGE():void {
	...
	if(player.lowerBody.cockSpot.list[x].cockLength < 36) outputText(" each time it pushes into them.", false);
	...
	if(player.lowerBody.cockSpot.list[x].cockLength >= 40) {
		...
		if(player.lowerBody.cockSpot.list[x].cockLength < 50) outputText("before you pull back.", false);
		else outputText("before she lets it back out before another push onward.", false);
	}
	if(player.lowerBody.cockSpot.list[x].cockLength >= 50) outputText("  Her tongue darts out and licks the underside when it slips by her head.", false);
	...
	if(player.lowerBody.cockSpot.list[x].cockLength < 36) outputText("tits", false);
	else if(player.lowerBody.cockSpot.list[x].cockLength < 50) outputText("face", false);
	else if(player.lowerBody.cockSpot.list[x].cockLength < 100) outputText("nest", false);
	...
	if(player.cumQ() >= 500) {
		...
		if(player.lowerBody.cockSpot.list[x].cockLength >= 50) outputText("'s nest", false);
		outputText(".", false);
	...
	if(player.lowerBody.cockSpot.list[x].cockLength < 50) outputText("covered the underside of your " + cockDescript(x) + " in kisses during your orgasm, but the tingling oral drugs only made it feel better.", false);
...
private function sophieVictoryPussyGrind():void {
	...
	//(small clit)
	if(player.lowerBody.vaginaSpot.list[0].clitLength < 2) outputText("Your " + clitDescript() + " puffs up inside your folds, turning into a hard little bump of pleasure.  Moaning happily, you start to drag your " + vaginaDescript(0) + " back and forth against Sophie's wet gash, shivering each time you bump into her own rapidly engorging clitoris.  ", false);
	//(large clit) 
	else if(player.lowerBody.vaginaSpot.list[0].clitLength < 5) outputText("Your " + clitDescript() + " puffs up, protruding from your folds like a miniature cock.  Moaning happily, you start to pump your " + vaginaDescript(0) + " back and forth across Sophie's wet gash, shivering as your " + clitDescript() + " continually slips in and out of it.  The harpy's own nub quickly grows hard, and you feel it bumping against you pleasantly while your hips gyrate against her.  ", false);
	//(cock-clit)
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("You pinch a " + nippleDescript(0), false);
	...
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLAVERING) outputText("Juices squirt from the joined pussies", false);
	else if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLICK) outputText("Juices soak the nesting material", false);
	else if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.WET) outputText("Juice trickles from the paired pussies", false);
	...
	if(flags[FlagEnum.TIMES_FUCKED_SOPHIE_LESBIAN] == 0) outputText("Cutey, you sure know your way around a pussy, but I just don't like to do this kind of thing.  Don't expect me to be some lesbo bird-slut who'll lick your slit at the drop of a hat.", false);
	else if(flags[FlagEnum.TIMES_FUCKED_SOPHIE_LESBIAN] < 6) outputText("Cutey, you get me off hard every time, but I'd still rather not mess with another girl.  I much prefer getting a younger man in my nest and letting him ravage me.", false);
...
private function fuckDatClit():void {
	...
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) outputText("Juice dribbles down your " + player.legs() + " as your body enjoys the treatment.  ", false);
...
private function tinyDickSupremeSophieLoss():void {
	...
	if(flags[FlagEnum.AMILY_MET] > 0 && player.stats.cor > 50) outputText("You imagine Amily's voice nattering away about how terrible the corruption is, but it doesn't help!  Y", false);
...
private function normalLossRapuuuuSophie():void {
	...
	if(player.lowerBody.balls > 0) outputText("The moisture trickles over your " + ballsDescriptLight() + " and they shift and churn, building up a thick load of cum for your harpy lover.  ", false);
