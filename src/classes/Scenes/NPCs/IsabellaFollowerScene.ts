...
internal function isabellaMoovesInGreeting():void {
	...
	if(flags[FlagEnum.ISABELLA_TIMES_OFFERED_FOLLOWER] == 0) {
		...
		if(player.stats.cor < 33) outputText("smile, and admit that you do enjoy the time you spend with her.", false);
		else if(player.stats.cor < 66) outputText("grin and joke that she probably missed you more.", false);
...
private function isabellasAccentCoaching():void {
	...
	if(result < 12) {
		...
		//(FAIL3) 
			...
			if(player.lowerBody.cockSpot.hasCock()) outputText(", and you leave the lesson with certain stiffness in your loins.", false);
			else if(player.lowerBody.vaginaSpot.hasVagina()) outputText(", and you leave the lesson with wet panties.", false);
...
public function isabellaMorningWakeupCall():void {
	...
	if(flags[FlagEnum.ISABELLA_MORNING_FELLATIO_COUNT] == 1) outputText("Isabella is fellating you!?", false);
	...
	if(player.lowerBody.balls > 0) outputText(ballsDescriptLight() + " against her chin, the cum-bloated orbs bouncing and shuddering with your " + cockDescript(x) + " as release nears.", false);
	else {
	...
	if(player.lowerBody.balls > 0) outputText("  Your balls clench tightly as they release your seed", false);
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() > 2) outputText("the rest of your " + multiCockDescriptLight(), false);
...
private function toggleIsabellasMorningWoodChopping():void {
	...
	if(flags[FlagEnum.ISABELLA_BLOWJOBS_DISABLED] == 0) {
		...
		outputText("You ", false);
		if(player.stats.cor < 33) outputText("blush and ", false);
		else if(player.stats.cor >= 66) outputText("smirk and ", false);
...
private function repeatGermanBratwurstInCamp():void {
	...
	if(player.stats.cor < 33) outputText("Coloring red, you sheepishly answer in the affirmative.", false);
	else if(player.stats.cor < 66) outputText("Not seeing a reason to hide it, you admit as much.", false);
	...
	if(player.stats.cor < 33) outputText("feel the weight of shame settling over you", false);
	else if(player.stats.cor < 66) outputText("feel mildly ashamed", false);
	...
	if(player.lowerBody.cockSpot.count() == 2) outputText("The other penis is shoved to the side, forgotten about as Isabella selects the lucky fuck-stick.  ", false);
	else if(player.lowerBody.cockSpot.count() > 2) outputText("The other penises are shoved aside, forgotten about as Isabella selects the lucky fuck-stick.  ", false);
	...
	//(Big dicks) 
		...
		if(player.lowerBody.cockSpot.list[x].cockLength > 48) outputText("to as high as she can reach on the towering shaft's length", false);
		...
		if(player.lowerBody.balls > 0) outputText("squeeze your balls", false);
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("rub your soaked vulva and " + clitDescript(), false);
...
private function tentacleBoneFollowerIzzy():void {
	...
	if(player.canFly() && player.str >= 90) {
		outputText("\"<i>Time for a wild ride, sweetheart.</i>\"  You flap your " + player.upperBody.wingDesc + " wings, mobilizing all your muscles in order to lift yourself off.  The weight you are burdened with is absurdly heavy, and you groan and pant in your attempts at taking off; eventually, you manage to part from the possessive surface little by little, grunting in triumph as you see your redoubled effort paying off; soon you're way up in the sky and Isabella's dickflesh prison is wobbling back and forth as you drag her under you. If it weren't for your colossal tentacle strength, the poor cow-girl would probably crash to the earth in a matter of seconds.  You keep ascending until Isabella realizes how high you both are and shrieks.  \"<i>", false);
	...
	if(player.lowerBody.balls > 0) outputText("sack ", false);
...
private function izzyMilkingMeinMilkersMya():void {
	spriteSelect(31);
	outputText("", true);
	if(player.stats.cor < 50) outputText("Concerned", false);
	...
	if(player.stats.cor < 33) outputText("unbearable, but you quickly", false);
...
private function izzyMilkingMeinMilkersMya2():void {
	...
	outputText("You ", false);
	if(player.stats.cor > 50) outputText("briefly consider forbidding her outright in order to see that exquisite look of pained pleasure on her face again, but you don't think she'd comply.", false);
...
private function AllowIzzyMilkerUse():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("(You have a hunch that you might be able to catch her using the milkers at the farm if you 'explore' there, provided she hasn't been recently milked.)\n\n");
...
private function getMilk():void {
	...
	if(flags[FlagEnum.ISABELLA_MILK_COOLDOWN] > 0) {
		...
		if(flags[FlagEnum.ISABELLA_MILKED_YET] == -1) outputText("'s out of bottled milk right now, and that you should check back later.", false);
...
private function sparring(type: number = 1):void {
	spriteSelect(31);
	outputText("", true);
...
private function getIzzyBurped():void {
	...
	if(flags[FlagEnum.ISABELLA_PROBOVA_BURP_COUNT] == 1) {
		...
		if(player.horns <= 0 || player.hornType != HORNS.COW_MINOTAUR) {
			if(player.horns > 0 && player.hornType > HORNS.NONE) outputText("  Your existing horns quiver, then reform into those of a " + player.mf("bull","cow"), false);
		...
		if(player.tailType != TAIL.COW) {
			if(player.tailType > TAIL.NONE) outputText("  The tip of your tail reshapes into a burst of fur, the entire length changing to match.", false);
		...
		if(player.lowerBody.cockSpot.hasCock() && player.longestCockLength() < 18) {
			...
			if(player.lowerBody.balls < 0) outputText("  Two testicles fall into a scrotum near the base of your enlarged meat, already swelling into baseball-sized balls o' fun.", false);
			else if(player.lowerBody.ballSize < 4) outputText("  Your testicles engorge in the transformative powers of the cow-girl's violent reaction to the Pro Bova, growing to baseball-sized in short order.", false);
			changed = true;
		}
		...
		if(player.lowerBody != LOWER_BODY.HOOFED || player.horns <= 0 || player.hornType != HORNS.COW_MINOTAUR || player.upperBody.head.earType != EARS.COW || player.tailType != TAIL.COW || (player.lowerBody.cockSpot.hasCock() && player.longestCockLength() < 18)) {
			...
			if((!player.lowerBody.cockSpot.hasCock() || rand(2) == 0) && player.lowerBody.vaginaSpot.hasVagina()) outputText("Isabella beaming with eagerness at your sloshing and swelling bosom.\n\n", false);
			else if(player.lowerBody.cockSpot.hasCock()) outputText("Isabella not able to keep a little frustration from her face as your dick grows to enviable size.\n\n", false);
		}
...
internal function receiveAllTheCowTOngues():void {
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength < 1) outputText(", your clit grinding into her nose,");
...
private function fuckIsabella():void {
	...
	//[if cock in her size: 
	if(player.lowerBody.cockSpot.list[x].cockLength < 9) outputText("pulls your small [cockFit " + y + "] out of your armor, preparing to suck you off as normal.  You grab her shoulders to stop her, more interested in... giving back, this time.");
	else {
...
private function fuckIsabellaInTheBarn():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("es");
