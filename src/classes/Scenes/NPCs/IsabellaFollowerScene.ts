...
public callForFollowerIsabella():void {
	...
	if (flags[FlagEnum.FOLLOWER_AT_FARM_ISABELLA] == 0 && flags[FlagEnum.FARM_CORRUPTION_STARTED] == 1) MainScreen.addButton(5, "Farm Work", sendToFarm);
	if (flags[FlagEnum.FOLLOWER_AT_FARM_ISABELLA] == 1) MainScreen.addButton(5, "Go Camp", backToCamp);
	
	if (flags[FlagEnum.FOLLOWER_AT_FARM_ISABELLA] == 1) MainScreen.addButton(9, "Back", kGAMECLASS.farm.farmCorruption.rootScene);
}
...
private campIzzySexMenu():void {
	...
	if(player.lust < 33) MainScreen.text("  <b>You aren't aroused enough to initiate sex.</b>", false);
...
public isabellaMorningWakeupCall():void {
	...
	if(flags[FlagEnum.ISABELLA_MORNING_FELLATIO_COUNT] == 1) MainScreen.text("Isabella is fellating you!?", false);
	else MainScreen.text("Isabella is giving you one of her traditional good-morning blowjobs.", false);
	MainScreen.text("  Her massive, malleable tongue moulds around your " + cockDescript(x) + " as she works to enhance your pleasure.  A trickle of warm, creamy milk escapes the heavy jugs resting on your " + player.legs() + ", showing you just how much she enjoys dispensing oral pleasure.  The tangled, crimson mass of hair slowly begins to bob up and down upon your loins, and Isabella's big brown eyes look up at you with radiant happiness, the corners of her mouth curling into a cock-filled smile.\n\n", false);
	
	MainScreen.text("You groan out loud at the sight of the sexy cow-girl's mischievous gaze.  She's face-fucking your " + cockDescript(x) + " without any difficulty and laying languid, lascivious licks over every part of your maleness simultaneously.  Your cock is utterly, helpless bound up by her thick oral organ.  It twitches inside her mouth-muscle's embrace, helplessly releasing rivulets of pre-cum that disappear into Isabella's throat as fast as you can produce them.  You can see her cheeks hollowing as she begins to pull the tongue-wrapped dick deep into her maw, the suction making your " + cockDescript(x) + " grow dangerously sensitive to the cow-girl's whims.\n\n", false);
	
	MainScreen.text("\"<i>Oh fuck!</i>\" you groan without meaning to, lifting your " + hipDescript() + " to press your ", false);
	if(player.lowerBody.balls > 0) MainScreen.text(ballsDescriptLight() + " against her chin, the cum-bloated orbs bouncing and shuddering with your " + cockDescript(x) + " as release nears.", false);
	else {
		MainScreen.text(cockDescript(x) + " the whole way into her mouth, the slippery, hot cow-girl's lips sealing tightly around your ", false);
		if(player.lowerBody.cockSpot.hasSheath()) MainScreen.text("sheath", false);
		else MainScreen.text("base", false);
		MainScreen.text(".", false);
	}
	MainScreen.text("  Isabella brushes her hair back to watch your reaction; her shining eyes never leave your face as she kicks her efforts into overdrive, pumping and stroking your " + cockDescript(x) + " with nothing more than her tongue's moist heat.\n\n", false);
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() > 2) MainScreen.text("the rest of your " + multiCockDescriptLight(), false);
	...
	MainScreen.text("milk-puddle around your " + player.legs() + ".  She sighs and says, \"<i>", false);
	if(flags[FlagEnum.ISABELLA_MORNING_FELLATIO_COUNT] == 1) {
		...
		if(isabellaAccent()) MainScreen.text("Yum!  You cum so hard for momma Izabella, don't you?</i>\"  The sated cow-girl gives your " + cockDescript(x) + " an affectionate squeeze before murmuring, \"<i>So yummy...", false);
		else MainScreen.text("Yum!  You cum so hard for momma Isabella, don't you?</i>\"  The sated cow-girl gives your " + cockDescript(x) + " an affectionate squeeze before murmuring, \"<i>So yummy...", false);
	}
	else {
...
private repeatGermanBratwurstInCamp():void {
	...
	MainScreen.text("You ask Isabella if she would mind helping you blow off some pressure before you go back out.  She glances down at " + sMultiCockDesc() + " and ", false);
	...
	MainScreen.text("You nod, catching a whiff of her musky pussy's aroma without meaning to.  It's not like you could avoid it, with her body mere inches in front of your " + player.face() + ", shaking so perfectly and so closely that an occasional droplet catches on your nose.  Your hands clamp around the chair with an iron grip as you fight not to rise and rape her tender, teasing asshole on the spot.  " + SMultiCockDesc() + " is trickling and dribbling, bouncing with your heartbeats as your " + player.legs() + " shiver and shake with denied lusts.  You NEED to do something... to fuck, thrust, or just fap... anything - your " + multiCockDescriptLight() + " can't take it anymore!\n\n", false);
	...
	MainScreen.text("The curvaceous cow-girl's stance shifts slightly, her teasing, shaking dance coming to a stop for just a moment as she adjusts to rest her rump against your " + cockDescript(x) + ".  ", false);
	...
	if(player.cockArea(x) <= 38) {
		if(isabellaAccent()) MainScreen.text("\"<i>Oh my, does mein behind feel so good zat your cute, delectable cock vill squirt all over me before you even get to feel ze best part?</i>\" teases Isabella as she flexes one side and then the other, forcing your " + cockDescript(x) + " to bend in response to the two dueling masses of her booty cheeks.  She keeps teasing you as she caresses your member with her hefty butt.  \"<i>You have such a naughty look on your face, drooling on yourself from just a little bit of play with Momma Isabella.  No, don't stop, I want to watch the ecstasy spread through you while I work your nice, hot cock.  My asshole wants to kiss it... Can I kiss your dick with my asshole?</i>\"", false);
		else MainScreen.text("\"<i>Oh my, does my ass-job feel so good that your cute, delectable cock is gonna squirt all over me before you even get to feel the best part?</i>\" teases Isabella as she flexes one side and then the other, forcing your " + cockDescript(x) + " to bend in response to the two dueling masses of her booty cheeks.  She keeps teasing you as she caresses your member with her hefty butt.  \"<i>You have such a naughty look on your face, drooling on yourself from just a little bit of play with Momma Isabella.  No, don't stop, I want to watch the ecstasy spread through you while I work your nice, hot cock.  My asshole wants to kiss it... Can I kiss your dick with my asshole?</i>\"", false);
		//(if libido <=15:  
		...
		MainScreen.text("  She doesn't even let you answer.  Her back-and-forth flexing abruptly halts as she switches to an up and down motion, hotdogging your " + cockDescript(x) + " in the tight vice of her spotted backside.\n\n", false);
		
		MainScreen.text("You grab hold of the chair again with both hands, struggling not to cum while Isabella's ass slides over your member.  It's so hot, wet, and sensual that you're trembling from the effort of holding still, but you manage, knowing the longer you hold back, the better the finish will be.  She slowly pumps your " + cockDescript(x) + " in her anal valley while you whine and moan, your face taking on a more lurid, sexual expression with every passing moment.  When at last you feel you can hold back no longer, you groan, \"<i>G-gonna... gonna-cum-soon!</i>\"\n\n", false);
		...
		MainScreen.text("Your " + cockDescript(x) + " writhes in bliss for what seems like forever, but before you know it, you're coming back down to Earth.  Isabella whispers, \"<i>Good " + player.mf("boy","girl") + ",</i>\" to you and stands up, ignoring the cum that drips from her dark pucker to stain " + sMultiCockDesc() + ".  She helps you up, and her hand tussles your hair affectionately.  Then she turns towards her side of camp, walking a little bow-legged as she leaves.", false);
	}
	//(Big dicks) 
	else {
		if(isabellaAccent()) MainScreen.text("\"<i>Oh my, does mein ass feel zat good on ze huge, pervy thing you call a dick?</i>\" teases Isabella as she flexes one side then the other, forcing your " + cockDescript(x) + " to bend in response to the two dueling masses of her booty cheeks.  She keeps talking, \"<i>You have such a naughty look on your face, drooling all over yourself like a dog.  Is that what having a big dick does to you?  Does it make you so horny and weak-willed that you'll waste your cum all over my ass?</i>\"  Isabella stops her back-and-forth flexing in order to begin bouncing up and down.  Her soft, flexing buns caress your " + cockDescript(x) + " from base ", false);
		else MainScreen.text("\"<i>Oh my, does my ass-job feel that good on that huge, pervy thing you call a dick?</i>\" teases Isabella as she flexes one side then the other, forcing your " + cockDescript(x) + " to bend in response to the two dueling masses of her booty cheeks.  She keeps talking, \"<i>You have such a naughty look on your face, drooling all over yourself like a dog.  Is that what having a big dick does to you?  Does it make you so horny and weak-willed that you'll waste your cum all over my ass?</i>\"  Isabella stops her back-and-forth flexing in order to begin bouncing up and down.  Her soft, flexing buns caress your " + cockDescript(x) + " from base ", false);
		if(player.lowerBody.cockSpot.list[x].cockLength > 48) MainScreen.text("to as high as she can reach on the towering shaft's length", false);
		...
		else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("rub your soaked vulva and " + clitDescript(), false);
...
private tentacleBoneFollowerIzzy():void {
	...
	MainScreen.text("You grin at Isabella as you nonchalantly remove your " + player.armorName + ", exposing your monstrous tentacle-cocks, fully erect, to her frightened face.  Crossing your arms, you stand before her, dicks squirming all over.  \"<i>", false);
	...
	if(isabellaAccent()) MainScreen.text("Without a word, you adroitly guide your squirming bunch of tentacle peckers toward her body, slowly encircling her. In no time your " + cockDescript(t1) + " is already wrapping her supple waist, quickly followed by your " + cockDescript(t2) + "; using the combined strength of your inhumanly powerful pair of dick muscles, you lift her off the ground.  \"<i>Oh... you're a kinky boy, aren't you? Well, if that's what you want...</i>\"", false);
	else MainScreen.text("Without a word, you adroitly guide your squirming bunch of tentacle peckers toward her body, slowly encircling her. In no time your " + cockDescript(t1) + " is already wrapping her supple waist, quickly followed by your " + cockDescript(t2) + "; using the combined strength of your inhumanly powerful pair of dick muscles, you lift her off the ground.  \"<i>Oh... you are a kinky boy, ja?  Vell, if zat is vhat you vant...</i>\"", false);
	...
	if(t7 >= 0) {
		...
		//[if 8th cock] - TITS 2
		if(t8 >= 0) MainScreen.text("  It is soon joined by your " + cockDescript(t8) + ", each prick taking care of a nipple, teasing the holes with shallow penetrations by her hard nubs, the pre-cum battling against the flow of milk in the opposite direction and stretching your urethra with a sensation that makes you shiver wildly.", false);
	...
	if(t9 >= 0) {
		...
		if(t10 >= 0) MainScreen.text("s are left with nothing to stuff, writhing in frustration.  You think it would be fair to put them to use: you wrap one around her wrists, binding her hands and immobilizing her to ease the penetration of the others, then run the tip across the unused parts of her body, and simply enjoy the caress of her delicate skin against your " + cockDescript(t9) + ", feeling her shiver under your fluid tentacle motion.  Your remaining " + cockDescript(t10) + " wraps around her face, squeezing her cheeks and pointing its tip toward her eyes, in order to make sure her every sense is saturated with cock.", false);
		else MainScreen.text(" is left with nothing to stuff, writhing in frustration.  You think it would be fair to put it to use: you wrap it around her wrists, binding her hands and immobilizing her to ease the penetration of the others, then run the tip across the unused parts of her body, and simply enjoy the caress of her delicate skin against your " + cockDescript(t9) + ", feeling her shiver under your fluid tentacle motion.", false);
		MainScreen.text("\n\n", false);
	}
	
	MainScreen.text("This thorough tentacle fuck has made you tired and ridiculously aroused.  Your ", false);
	if(player.canFly() && player.str >= 90) MainScreen.text("wings and your ", false);
	MainScreen.text(multiCockDescriptLight() + " did all the work; your groin feels sore from the sheer effort deployed in order to engage in a maximally effective penetration.  As you keep pumping, grinding and teasing her with your wriggling rods, you feel a pressure growing in your crotch and all the way up your urethras; you accelerate the thrusting pace even as ", false);
	if(player.canFly() && player.str >= 90) MainScreen.text("you descend, setting your " + player.feet() + " on the soil again but still holding Isabella herself in the air; ", false);
	...
	MainScreen.text("You keep holding her with " + sMultiCockDesc() + " for a moment, both of you enjoying the powerful embrace as you literally dick-hug her.  The entirety of her body is dripping with your jism but she doesn't seem to care.  Eventually, you put her down, letting her splat in the puddle of juices that formed below your junk-prison.  At last, you unfold your arms.\n\n", false);
	player.orgasm();
...
private getMilk():void {
	...
	if(flags[FlagEnum.ISABELLA_MILK_COOLDOWN] > 0) {
		...
		if(flags[FlagEnum.ISABELLA_MILKED_YET] == -1) MainScreen.text("'s out of bottled milk right now, and that you should check back later.", false);
...
private getIzzyBurped():void {
	...
	if(flags[FlagEnum.ISABELLA_PROBOVA_BURP_COUNT] == 1) {
		...
		if(player.lowerBody != LOWER_BODY.HOOFED) {
			...
			if(player.isTaur()) MainScreen.text("  Confused, you quickly inspect yourself before your gaze settles on your hooves – or should you say, your SINGLE PAIR of hooves.  Triggered by what you surmise are the Pro Bova's fumes, your centaur body has vanished, leaving you with a single pair of cow-like hooves.", false);
		...
		if(player.horns <= 0 || player.hornType != HORNS.COW_MINOTAUR) {
			if(player.horns > 0 && player.hornType > HORNS.NONE) MainScreen.text("  Your existing horns quiver, then reform into those of a " + player.mf("bull","cow"), false);
		...
		if(player.tailType != TAIL.COW) {
			if(player.tailType > TAIL.NONE) MainScreen.text("  The tip of your tail reshapes into a burst of fur, the entire length changing to match.", false);
			else MainScreen.text("  A tail creeps out of your " + player.armorName + ", perking up and growing rapidly.  A flare of fur bursts from the tip, the rest of the length changing to match.", false);
			changed = true;
		}
...
internal function receiveAllTheCowTOngues():void {
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength < 1) MainScreen.text(", your clit grinding into her nose,");
...
private fuckIsabella():void {
	...
	//[if cock in her size: 
	if(player.lowerBody.cockSpot.list[x].cockLength < 9) MainScreen.text("pulls your small [cockFit " + y + "] out of your armor, preparing to suck you off as normal.  You grab her shoulders to stop her, more interested in... giving back, this time.");
	else {
