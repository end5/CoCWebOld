...
internal function kihaSpiderEventIntro():void {
	...
	if(player.stats.cor < 66) outputText("You try to calm the dragoness down, explaining that you aren't one of Lethice's minions, that what you told her the first time around was true - that you're here to end the demon threat.  Kiha doesn't seem to be buying it, though...\n\n", false);
...
private function helpKihaAgainstSpoidahs():void {
	...
	if(player.stats.cor >= 66) outputText("beat", false);
...
public function beatSpiderMob():void {
	...
	if(player.stats.cor < 50) outputText("squeeze her shoulder and ", false);
...
private function jumpDaBitches():void {
	...
	if(player.lowerBody.cockSpot.list[0].cockLength >= 8) outputText("bends down her throat");
	else if(player.lowerBody.cockSpot.list[0].cockLength > 6) outputText("hits the back of her throat", false);
...
private function lovinHugKiha():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("[pg][EachCock] springs free, smacking into Kiha's thigh as it fully engorges.  She gives it a gentle pump before commenting, \"<i>Acceptable, I suppose.</i>\"", false);
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("[pg]Your [vag] tingles as it's exposed to the air.  The lusty dragon-maid dips a digit inside your depths before commenting, \"<i>Not bad, I guess.</i>\"", false);
...
private function lovingHugDickings():void {
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 100) outputText("How do you walk with this thing swinging around everywhere, bludgeoning into everyone you meet?", false);
	else if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 9) outputText("How do you walk with this thing sticking out all the time, or was that just for me?");
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() <= 9) outputText("little ", false);
	...
	if(player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockLength >= 36) outputText(", even though you bump her repeatedly in the nose with your moist tip");
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() == 2) outputText("your other");
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() == 2) outputText("dual ");
...
private function boneTheShitOutofKihaHolesWithHorsecock():void {
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("fat horse-cock");
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("bestial equine dong");
	else outputText("[cock " + y + "]");
	outputText(" and its slowly-filling ");
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("flared ");
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("flare's mesa");
	else outputText(player.cockHead(x));
	outputText(" as she opens wide.  ");
	if(player.lowerBody.cockSpot.hasSheath()) outputText("She pumps you just above the sheath");
	else outputText("She pumps your shaft");
	outputText(" with one hand, using slow, even strokes that tickle every sensitive area.  ");
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("Her other caresses the skin of your sheath, fingers slipping in between the folded outer skin and your dick's concealed, untouched flesh.  ");
	else if(player.lowerBody.balls > 0) outputText("Her other fondles your [balls], steadily increasing your desire for release.  ");
	...
	if(player.stats.int < 20) {
		...
		if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("bestial whinny ");
		...
		if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("animal");
		else outputText("well");
		outputText("-endowed form.");
	}
	else outputText("  A ");
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE || player.isTaur()) outputText("whinny ");
	else outputText("groan ");
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("beast-");
	outputText("prick.  The ");
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("thick flare");
	else outputText(player.cockHead(x));
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("her free hand eagerly caressing you inside your own sheath with inhuman warmth");
	else {
		if(player.lowerBody.balls > 0) outputText("her tongue working as much of your shaft as possible with inhuman warmth.");
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("big, pony-like prick.");
	else outputText("[cock " + y + "].");
	if(player.lowerBody.balls > 0) outputText("  Your [balls] smear and shudder against her chin, so tight, hot, and ready to explode.");
	outputText("[pg]Kiha ");
	if(player.lowerBody.balls > 0) outputText("squeezes your [sack]");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("fondles your [vag]");
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("flare");
	else outputText(player.cockHead(x));
	...
	if(c <= 250) outputText("She manages to guzzle it all down without too much difficulty.");
		...
		if(player.lowerBody.cockSpot.list[x].cockType.Index > 0 && player.lowerBody.cockSpot.list[x].cockType.Index < 10) outputText("animal ");
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("horse-");
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("flat ");
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("wide head of your stallion-shaft");
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("well-lubed horse-dick");
	else outputText(cockDescript(x));
	outputText(" into Kiha's fully-dilated pussy.  The dragon-cunt is nice and tight around your large");
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText(", mammalian");
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("flat-tipped horsecock");
	else outputText(cockDescript(x));
	...
	if(player.lowerBody.balls) outputText("  You slap your [balls] into in her ass hard as you thrust, smearing them all over her sweaty ass and tail.");
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("equine ");
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("flaring ");
	...
	if(player.lowerBody.cockSpot.list[x].cockType.Index > 0) outputText("animal-");
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("The unique shape of your cockhead makes such penetration difficult, but with animalistic stubbornness, you keep at it.");
	...
	if(player.lowerBody.cockSpot.hasSheath()) outputText("to your sensitive sheath");
	...
	if(player.lowerBody.balls > 0) outputText("[balls]");
	else if(player.lowerBody.cockSpot.hasSheath()) outputText("sheath");
	...
	if(flags[FlagEnum.KIHA_HORSECOCK_FUCKED] == 0) {
		...
		if(player.stats.cor <= 33) outputText(" and wince");
		outputText(", wondering just what caused you to go so lust-crazy.  ");
		if(player.stats.lib < 80 && player.minLust() < 50) outputText("Was it the lust draft?  Or");
		...
		if(player.statusAffects.get("Exgartuan").value1 == 1) outputText("  Exgartuan suggests, \"<i>She probably loved it if she had as much fun as I did.</i>\"");
...
private function kihaPlaysWithBigassCocksFemDomAhoy():void {
	...
	if(player.lowerBody.cockSpot.list[x].cockLength > 48) outputText(" for a moment before climbing higher.  It quickly blocks your view of your lover.");
	...
	if(player.lowerBody.cockSpot.list[x].cockLength > 48) outputText("bigger than");
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HUMAN) outputText(", particularly your medial ring and the sensitive edge of your flare.");
	else if(player.hasKnot(x)) outputText(", particularly your massive, bulging canid knot.");
	else if(player.lowerBody.cockSpot.list[x].cockType == CockType.DEMON) outputText(", particularly the hundreds of demonic nodules that ring your shaft.");
	else if(player.lowerBody.cockSpot.list[x].cockType == CockType.ANEMONE) outputText(", particularly the groups of aphrodisiac laced tentacles under your crown.");
	else if(player.lowerBody.cockSpot.list[x].cockType == CockType.TENTACLE) outputText(", particularly the underside of your tentacle's mushroom-like tip.");
	else if(player.lowerBody.cockSpot.list[x].cockType == CockType.CAT) outputText(", particularly the hundreds of rubbery spines covering your feline maleness.");
	else outputText(".");
	...
	if(player.lowerBody.cockSpot.list[x].cockLength > 48) outputText("pulls your cockhead down and ");
	...
	if(player.hasKnot(x)) outputText("beneath your knot ");
		...
		if(player.lowerBody.cockSpot.hasSheath()) outputText(", just above the sheath, ");
		else outputText(" ");
	}
	...
	if(player.lowerBody.balls > 0) outputText("Your balls clench and relax in time with the waves of pleasure, and y");
	else outputText("Y");
	...
	if (flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] > 0) {
		if(player.stats.cor < 50) outputText("  You hope she doesn't get jumped upby your tiger-shark children.");
...
private function kihaGirlGirlSex():void {
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength >= 10) outputText(", gasping when it grows to a ridiculous size.");
	else if(player.lowerBody.vaginaSpot.list[0].clitLength >= 3) outputText(", humming appreciably when it grows to cock-like length.");
...
private function savinTheAnalForKiha():void {
	...
	//(depending on length: 
	if(player.lowerBody.cockSpot.list[x].cockLength < 12) outputText("belly");
	...
	if(player.lowerBody.cockSpot.list[x].cockLength < 3) outputText("; you pull her cheeks apart just long enough to slip your prick between them, letting Kiha hotdog you");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(", just as yours are");
	outputText(".");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) outputText(" as she palms your " + biggestBreastSizeDescript(), false);
	...
	if(player.lowerBody.cockSpot.list[x].cockLength >= 40) outputText("bottoms out on your cock");
	else outputText("rests her cheeks on your crotch");
	outputText(".  Both of you are gasping and panting from the slow descent, Kiha ");
	if(flags[FlagEnum.TIMES_KIHA_ANALED] >= 5) outputText("growing pleased by");
...
private function fuckKihasVagInCamp():void {
	...
	if(player.skinType != SKIN.SCALES) outputText("leave long, red scratches on your back.");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("[pg]Kiha's belly is quickly glazed white with goo, and her ");
...
private function fuckKihaWithATentacle():void {
	...
	if(player.lowerBody.balls > 0) outputText("churn and quiver");
...
internal function kihaBitchesOutCorruptPCs():void {
	...
	if(!followerKiha()) {
		...
		if(flags[FlagEnum.KIHA_AFFECTION_LEVEL] == 1) outputText("friend");
		...
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("[vag]");
...
internal function pcWinsDomFight():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("pull your quickly hardening " + multiCockDescriptLight());
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("release your lathered up " + vaginaDescript(0));
	...
	if(flags[FlagEnum.KIHA_CHOKED_OUT_PC] > 0) outputText("  Not again!");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(hipDescript());
	else outputText("face");
	outputText(", the dragoness draws ");
	if(player.lowerBody.cockSpot.hasCock()) outputText("you ");
	...
	//PC's vagina is chosen/ has vagina only: 
		...
		//Kiha response one (requires cock): 
		if(player.lowerBody.cockSpot.hasCock() && rand(2) == 0) outputText("[pg]\"<i>Ha!  The Champion still has some fight in " + player.mf("him", "her") + "!  Keep me here if you can, wimp!  I WILL pin you down and make you beg to plant your seed in my womb!  Just wa-OOhhOhhh...</i>\"");
		//Kiha response one (requires vagina): 
	...
	if(select == 0 || !player.lowerBody.vaginaSpot.hasVagina()) outputText("love canal");
	else outputText("clit");
	outputText(".  \"<i>Come on, idiot!  Give me all you've got, if you have the balls!</i>\"");

	//PC is male or herm, nutless: 
	if(player.gender != 2 && player.lowerBody.balls == 0) outputText("[pg]You're not sure if that's an insult or a challenge to step it up.");
	...
	if(select == 0 || !player.lowerBody.vaginaSpot.hasVagina()) outputText(" tug along her lengths and pull her head back towards you, soliciting a pained scream from Kiha as her roots convey the agony of the act to her.  \"<i>Is that all you've GOT?! Give me a break!</i>\"");
	...
	//Kiha traps the player in a submissive 69 (universal): 
		...
		if(!player.lowerBody.cockSpot.hasCock()) outputText("lapping viciously at your cunt.");
	...
	//Random event two of three: Footjobs and missionary, oh my!
		...
		if(player.tongueType == TONUGE.SNAKE || player.tongueType == TONUGE.DEMONIC) {
			...
			//Demon: 
			if(player.tongueType == TONUGE.DEMONIC) outputText("taking to her pointed mammary like a boa to prey as you lather up each teat separately.  The hot-tempered dragon girl squirms, completely at your mercy while you have your fun."); 
			//Snake: 
