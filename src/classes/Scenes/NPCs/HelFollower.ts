export default class HelFollower extends NPCAwareContent {
	...
	private helAnalCapacity(): number {
		let anal: number = 85;
		if (Flags.list[FlagEnum.HELIA_ANAL_TRAINING] >= 1) anal += 100;
		if (Flags.list[FlagEnum.HELIA_ANAL_TRAINING] >= 2) anal += 300;
		return anal;
	}
	...
	public helAffection(diff: number = 0): number {
		if (Flags.list[FlagEnum.HEL_AFFECTION_FOLLOWER] > 70 && Flags.list[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] == 0) Flags.list[FlagEnum.HEL_AFFECTION_FOLLOWER] = 70;
		if (Flags.list[FlagEnum.HEL_AFFECTION_FOLLOWER] < 100 || (Flags.list[FlagEnum.HEL_BONUS_POINTS] == 0 && diff < 0)) {
			Flags.list[FlagEnum.HEL_AFFECTION_FOLLOWER] += diff;
			if (Flags.list[FlagEnum.HEL_AFFECTION_FOLLOWER] >= 100) Flags.list[FlagEnum.HEL_AFFECTION_FOLLOWER] = 100;
			if (Flags.list[FlagEnum.HEL_AFFECTION_FOLLOWER] < 0) Flags.list[FlagEnum.HEL_AFFECTION_FOLLOWER] = 0;
		}
		else if (followerHel()) {
			Flags.list[FlagEnum.HEL_AFFECTION_FOLLOWER] = 100;
			Flags.list[FlagEnum.HEL_BONUS_POINTS] += diff * 3;
			if (diff > 0) if (Flags.list[FlagEnum.HEL_BONUS_POINTS] > 150) Flags.list[FlagEnum.HEL_BONUS_POINTS] = 150;
			else if (diff < 0) if (Flags.list[FlagEnum.HEL_BONUS_POINTS] < 0) Flags.list[FlagEnum.HEL_BONUS_POINTS] = 0;
		}
		return Flags.list[FlagEnum.HEL_AFFECTION_FOLLOWER];
		trace("HEL AFFECTION" + Flags.list[FlagEnum.HEL_AFFECTION_FOLLOWER]);
	}
	...
	public heliaFollowerIntro(): void {
		...
		if (Flags.list[FlagEnum.HARPY_QUEEN_EXECUTED] > 0) MainScreen.text("kill");
		else MainScreen.text("bring down");
		MainScreen.text(" the Harpy Queen... and getting my sister out of that shithole.  For giving me a chance to meet my dad.  For everything.</i>\"  You start to tell her that you're happy to help, but Hel cuts you off, speaking quickly: <i>\"Kiri, Dad, and I have been trying to make it work, living out on the plains - we really have - but Dad's not in any shape to fight and Kiri's no good at it; the plains are just too dangerous for them and Dad's been having nightmares about mom, and...\"</i> she pauses to take a gasping breath, unaccustomed to talking so quickly.");
		...
		menu();
		MainScreen.addButton(0, "I Can't", iCantLetFireButtsRapeMyCampsButt);
		MainScreen.addButton(1, "Come2Camp", comeToCampHeliaIWantTailInButt);
		MainScreen.addButton(2, "JustFriends", justFriendsWithAnalTailWaifu);
	}
	...
	private iCantLetFireButtsRapeMyCampsButt(): void {
		...
		if (Flags.list[FlagEnum.HEL_FOXY_FOURSOME_WARNED] > 0 || Flags.list[FlagEnum.HEL_EDRYN_OFFER] > 0) MainScreen.text("like usual");
		MainScreen.text(", and maybe you and I can spend some quality time together.  Alright?\"</i>");

		MainScreen.text("\n\nYou nod and tell your fiery lover you'll be sure to do just that.");
		Flags.list[FlagEnum.HELIA_FOLLOWER_DISABLED] = 1;
		doNext(playerMenu);
	}
	...
	private comeToCampHeliaIWantTailInButt(): void {
		...
		//place holder
		model.time.hours++;
		MainScreen.addButton(0, "Next", afterMoveInBoningAnalFireTail);
	}
	...
	private afterMoveInBoningAnalFireTail(): void {
		MainScreen.clearText();
		MainScreen.text("<b>An hour later...</b>");
		...
		Flags.list[FlagEnum.HEL_FOLLOWER_LEVEL] = 2;
		doNext(playerMenu);
	}
	...
	private justFriendsWithAnalTailWaifu(): void {
		MainScreen.clearText();
		MainScreen.text("Awkwardly, you spend the next few minutes saying that, while you appreciate the offer and her affections, you're more interested in just being friends with Hel, as you have been for some time.");
		...
		MainScreen.text(" to give you a little peck on the cheek.  <i>\"Well, that's that, then.  I guess.  Well, see you around, [name].\"</i>");

		MainScreen.text("\n\n<i>\"Sure will.\"</i>");

		MainScreen.text("\n\nYou wave as Hel retreats back toward her own home.");
		Flags.list[FlagEnum.HELIA_FOLLOWER_DISABLED] = .5;
	...
	public helFollowersIntro(): void {
		...
		if (followerKiha() && Flags.list[FlagEnum.HEL_INTROS_LEVEL] < 1) {
			Flags.list[FlagEnum.HEL_INTROS_LEVEL] = 1;
			MainScreen.text("You pace around camp, awaiting the return of your new companion.  After several minutes, you notice Kiha sitting behind you, eyeing you with an eyebrow cocked.  Suddenly self-conscious, you stop and face the dragon-girl.");
		...
		//If Isabella 
		else if (isabellaFollower() && Flags.list[FlagEnum.HEL_INTROS_LEVEL] < 2) {
			Flags.list[FlagEnum.HEL_INTROS_LEVEL] = 2;
			//is at Camp (She and Hel are cool)
			if (Flags.list[FlagEnum.HEL_ISABELLA_THREESOME_ENABLED] > 0) {
				MainScreen.text("Showing Hel around, you eventually come to the part of your camp inhabited by the towering cowgirl Isabella.  When you arrive, you find Isabella reclining in her armchair, humming a sweet melody as she cleans her tower shield.");
			...
			//If Isabella is at Camp (And she and Hel are NOT cool)
				...
				//(corruption = pussy)
				if (player.stats.cor < 50) MainScreen.text("fall silent under their considering gazes.");
				//(corruption = high enough to call bitches out when they're hypocrites)
				else MainScreen.text("glower balefully at them, as if to challenge either to invite your opinion of their own sexual 'résumé' with an ill-considered comment.");
				...
				if (isabellaAccent()) MainScreen.text("<i>\"You are... velcome,\"</i> Isabella says before collecting some of the scattered belongings from the ground.  You continue your tour, now that the girls are...  not going to murder each other in the middle of the night, at least.");
				else MainScreen.text("<i>\"You're... welcome,\"</i> Isabella says before collecting some of the scattered belongings from the ground.  You continue your tour, now that the girls are...  not going to murder each other in the middle of the night, at least.");
				Flags.list[FlagEnum.HEL_ISABELLA_THREESOME_ENABLED] = 1;
			}
		}
		//If Rath is in Camp
		else if (Flags.list[FlagEnum.HEL_INTROS_LEVEL] < 3 && player.statusAffects.has("CampRathazul")) {
			Flags.list[FlagEnum.HEL_INTROS_LEVEL] = 3;
			MainScreen.text("You take Hel over to the small section of camp Rathazul has cordoned off for his 'laboratory,' surrounding himself with glass tubes and beakers and other, stranger instruments.  You poke through the array of equipment to find old Rath sitting in front of some experiment or another, furiously scribbling notes.  With a light cough, you alert him to your presence.");
			...
			MainScreen.text("\n\n<i>\"DON'T TOUCH THAT, DAMN YOUR EyeType!\"</i> Rath shouts, shooing her away.  <i>\"Stay out of my laboratory!\"</i>");
		...
		//If Bimbo Sophie is at Camp:
		else if (Flags.list[FlagEnum.HEL_INTROS_LEVEL] < 4 && bimboSophie() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) {
			Flags.list[FlagEnum.HEL_INTROS_LEVEL] = 4;
			MainScreen.text("As you help Hel string up her hammock between a few of the rocks inside your perimeter, you hear the tell-tale flapping of useless wings and a clattering of claws on the hard-packed dirt.  You brace for impact as your bimbo harpy prances up to you, planting a big, wet kiss on your cheek");
			...
			if (player.stats.int >= 40 || player.stats.int / 5 + rand(20) + 1 > 10) {
				MainScreen.text("Or, maybe there's a way to make this work...");
				//(Display Option: [Boot Sophie] [Boot Hel] [Work it Out])
				menu();
				MainScreen.addButton(0, "Boot Sophie", bimboSophieGetsBooted4Firebutt);
				MainScreen.addButton(1, "Boot Hel", bootHelOutForBimboSophie);
				MainScreen.addButton(2, "Work It Out", workItOutWithSophieAndFireTits);
				return;
			}
			menu();
			MainScreen.addButton(0, "Boot Sophie", bimboSophieGetsBooted4Firebutt);
			...
			Flags.list[FlagEnum.HEL_INTROS_LEVEL] = 9001;
			playerMenu();
			return;
		}
		menu();
		MainScreen.addButton(0, "Next", helFollowersIntro);
	}
	...
	private bimboSophieGetsBooted4Firebutt(): void {
		MainScreen.clearText();
		MainScreen.text("You sigh and pick Sophie up, slinging the harpy over your shoulders.  <i>\"Yaaaay~\"</i> she cheers.  </i>\"I'm going for a ride!\"</i>");
		...
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] >= 3) {
			...
			Flags.list[FlagEnum.MINO_SONS_HAVE_SOPHIE] = 1;
		}
	...
	private bootHelOutForBimboSophie(): void {
		MainScreen.clearText();
		MainScreen.text("<i>\"Nope, you get out,\"</i> you answer, scowling at the salamander.");
	...
	private workItOutWithSophieAndFireTits(): void {
		MainScreen.clearText();
		MainScreen.text("You cross your arms and tell Hel to deal with it, remarking that you won't just dump Sophie out in the wilderness - she can't take care of herself - but you still want Hel around.");
		...
		Flags.list[FlagEnum.KEEP_HELIA_AND_SOPHIE] = 1;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	public angryHelAndIzzyCampHelHereFirst(): void {
		MainScreen.clearText();
		MainScreen.text("Showing Isabella around, you eventually come to the chaotic, cluttered part of camp inhabited by Hel the salamander, who's currently sitting on her hammock sharpening her scimitar.");
		MainScreen.text("\n\nYou only have a moment to remember Hel's disdain for the cowgirl before...");
		...
		if (isabellaAccent()) MainScreen.text("V");
		else MainScreen.text("W");
		MainScreen.text("hat are YOU doing here?!\"</i>");
	...
	public heliaFollowerMenu(display: boolean = true): void {
		...
		if (Flags.list[FlagEnum.HEL_FOLLOWER_LEVEL] == 2) {
			if (Flags.list[FlagEnum.HELIA_ANAL_TRAINING_OFFERED] == 0 && display && player.lowerBody.cockSpot.biggestCocks[0].cockArea() > heliaAnalCapacity()) {
				heliaAnalTrainingPrompt();
				return;
			}
			if (display) MainScreen.text("You call your salamander lover over, and in a few moments Hel walks your way, hips and tail swaying gaily as she moves.  She wraps an arm around your shoulders, pressing her soft, warm body against yours, and grins.  <i>\"Heyya, lover mine.  You need anything?\"</i>");
			menu();
			...
			//Take a Bath
			//Appearance
			MainScreen.addButton(0, "Appearance", helSpawnScene.heliasAppearanceScreen);
			MainScreen.addButton(1, "Sex", heliaRoughSex);
			MainScreen.addButton(2, "Threesomes", heliaThreesomes);
			MainScreen.addButton(4, "Talk", heliaOptions);
			if (!kGAMECLASS.helScene.pregnancy.isPregnant) MainScreen.addButton(5, "Spar", sparWithHeliaFirebuttsAreHot);
			else MainScreen.text("\n\n<b>Helia will not spar or box while pregnant.</b>");
			if (!kGAMECLASS.helScene.pregnancy.isPregnant) MainScreen.addButton(6, "Box", boxWithInCampHel);
			if (Flags.list[FlagEnum.HEL_LOVE] == 1 || Flags.list[FlagEnum.HEL_LOVE] == -1) {
				if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(heliaCapacity()) >= 0 && player.stats.lust >= 33 &&
					!helPregnant() && Flags.list[FlagEnum.HELSPAWN_AGE] == 0) MainScreen.addButton(7, "Have A Kid", helSpawnScene.haveAKid);
			}
			MainScreen.addButton(9, "Back", camp.campLoversMenu)
		}
		else if (Flags.list[FlagEnum.HEL_FOLLOWER_LEVEL] == 1) {
			if (Flags.list[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] == 1) {
				if (display) MainScreen.text("(You've reached the culmination of Helia's current storyline.  Stay tuned for more!)");
			}
			else {
				if (display) MainScreen.text("You approach Hel as she's pacing around camp.  She's clad in her normal field attire: a simple scale bikini top and leather thong which supports her scimitar's scabbard.  Her cloak is loosely thrown over her shoulders, giving her a slight measure of protection from the mountain's harsh environs.");
	...
	private heliaOptions(): void {
		if (kGAMECLASS.helScene.pregnancy.event >= 3 && Flags.list[FlagEnum.HELIA_TALK_SEVEN] == 0) {
			helSpawnScene.heliaTalkSeven();
			return;
		}
		if (Flags.list[FlagEnum.HELSPAWN_AGE] == 1 && Flags.list[FlagEnum.HEL_TALK_EIGHT] == 0) {
			helSpawnScene.heliaTalkEight();
			return;
		}
		menu();
		MainScreen.addButton(0, "Discuss", talkToHel);
		if (model.time.hours >= 21) MainScreen.addButton(1, "Cuddle", hugASmokeyTail);
		else MainScreen.addButton(2, "Hug", hugASmokeyTail);
		if (Flags.list[FlagEnum.HELIA_ANAL_TRAINING_OFFERED] > 0 && Flags.list[FlagEnum.HELIA_ANAL_TRAINING] < 2 && player.lowerBody.cockSpot.biggestCocks[0].cockArea() > heliaAnalCapacity() && player.inventory.items.has(consumables.GOB_ALE, 1)) MainScreen.addButton(3, "Anal Train", heliaGapeSceneChoices);
		MainScreen.addButton(5, "Bathe", takeABath);
		if (Flags.list[FlagEnum.HELSPAWN_AGE] == 1) MainScreen.addButton(7, Flags.list[FlagEnum.HELSPAWN_NAME], helSpawnScene.playWithYourKid);
		if (Flags.list[FlagEnum.HEL_GUARDING] == 0) MainScreen.addButton(8, "GuardCamp", helGuardToggle);
		else MainScreen.addButton(8, "NoGuarding", helGuardToggle);
		MainScreen.addButton(9, "Back", heliaFollowerMenu);
	}
	...
	private helFollowerAppearance():void {
		...
		menu();
		MainScreen.addButton(0,"Next",heliaFollowerMenu);
	}*/
	//Hel: Spar Intro
	private sparWithHeliaFirebuttsAreHot(): void {
		MainScreen.clearText();
		MainScreen.text("Giving Hel a playful punch on the shoulder, you ask the salamander-girl if she'd be up for a little battle practice.");
		...
		startCombat(new Hel());
		monster.statusAffects.add(new StatusAffect("Sparring", 0, 0, 0, 0)));
		//No gems.
		monster.XP = 1;
		monster.gems = 0;
	}
	...
	internal function loseToSparringHeliaLikeAButtRapedChump(): void {
		MainScreen.clearText();
		//If HP loss) 
		if (player.HP < 1) MainScreen.text("You collapse on the ground, overwhelmed by pain and exhaustion caused by the berserker's onslaught.");
		else MainScreen.text("Your arousal is too great, and your mind can no longer focus on anything but a desperate need for release.  Lust robs you of your will, and with buckling knees you collapse.");
	...
	internal function PCBeatsUpSalamanderSparring(): void {
		MainScreen.clearText();
		//(If HP loss) 
		if (monster.HP < 1) MainScreen.text("Unable to withstand your onslaught, the salamander collapses to a knee, barely supporting her weight on her sword.");
		else MainScreen.text("Panting heavily, knees snaking, she collapses, heavily leaning upon her sword.");

		MainScreen.text("\n\n<i>\"Come on, [name],\"</i> Hel groans, swaying unsteadily.  ");
		if (monster.HP < 1) MainScreen.text("<i>\"Kick my ass a little harder, why don't you?\"</i>");
		else MainScreen.text("<i>\"Turn me on till I can't think straight, why don't you?\"</i>");
	...
	private talkToHel(): void {
		...
		if (Flags.list[FlagEnum.FOLLOWER_HEL_TALKS] == 0) {
			MainScreen.text("You run a hand through Hel's hair and ask the recent addition to your camp if she'd like to talk.");
			...
			//(If Toughness <50: 
			if (player.stats.tou < 50) MainScreen.text("You cough and gag, eyes misting as the pure-grain booze burns your throat worse than hot coals.  Hel laughs riotously, slapping you on the back hard enough to cause booze to snort out through your nose.");
			else MainScreen.text("You knock back the stein easily, your super-human endurance keeping you from choking on the powerful brew.  Impressed, Hel gives you a high-five and throws her head back in a laugh.");
		...
		//Hel Talk 2
		else if (Flags.list[FlagEnum.FOLLOWER_HEL_TALKS] == 1) {
			MainScreen.text("Sitting Hel down, you ask if she's got any stories she'd like to share.  After all, she's been adventuring for years - surely she's got some fun tales to tell.");
		...
		//Hel Talk 5(C)
		else if (Flags.list[FlagEnum.FOLLOWER_HEL_TALKS] == 4) {
			MainScreen.text("<i>\"Hey, [name],\"</i> Hel says, slipping out of your grasp with a come-hither wag of her finger, <i>\"wanna have a drink with me?\"</i>");
			MainScreen.text("\n\n<i>\"Sure,\"</i> you say, following Hel over to her still.");

			MainScreen.text("\n\nShe pours out two tankards of ");
			if (helSpawnScene.helPregnant()) MainScreen.text("beer ");
			else MainScreen.text("fizzy, non-alcoholic beer ");
			MainScreen.text("and, swinging onto your lap, clinks her glass against yours.  <i>\"To loving friends and an awesome new home,\"</i> she says, raising her tankard high before knocking it back.  Before you can finish yours, Hel belches loudly and violently shakes her head");
			if (!helSpawnScene.helPregnant()) MainScreen.text(", her eyes crossing a little.  Looks like she was dipping in before you got here...");
			else MainScreen.text(".");

			MainScreen.text("\n\nHel laughs ");
			if (!helSpawnScene.helPregnant()) MainScreen.text("drunkenly");
			else MainScreen.text("happily");
			if (!helSpawnScene.helPregnant()) {
				MainScreen.text(", filling her cup up again and, with a wide smile, jumping out of your lap.  <i>\"Hey, lover, check this out!\"</i> she shouts, grabbing her fiery tail in one hand and swirling a mouth-full of alcohol.  You have just enough time to take cover before Hel swings her tail around and spits a stream of pure-grain over it, resulting in a great gout of flame that streaks into the heavens.");
				//if Kiha is in camp: 
				if (kihaFollower.followerKiha()) MainScreen.text("  From across camp, you hear a certain dusky dragoness shout <i>\"HEY! That's MY trick, firebutt!\"</i>");
				...
				if (kihaFollower.followerKiha()) MainScreen.text("further pissing off Kiha, who throws a rock at Hel, and ");
				MainScreen.text("shooting up like a beacon over the wasteland.");
			}
			else {
				MainScreen.text(" and launches into a series of debauched jokes. You answer her, lewd jest for jest.");
			}
			MainScreen.text("\n\n<i>\"Hehehe.  Nice one, lover,\"</i> Hel laughs, pulling you ");
			if (player.tallness < 60) MainScreen.text("up ");
			else MainScreen.text("down ");
			MainScreen.text("into a hug, mashing your face into her warm, soft bosom.  You struggle for air, finally bursting out of her tight cleavage.  Hel chuckles, running her hand through your [hair] as her fiery tail wags gaily behind her.");
		...
		//Hel Talk 6 (Needs Isabella and Kiha at camp; at least 1 gem)(C)
		else if (Flags.list[FlagEnum.FOLLOWER_HEL_TALKS] == 5 && player.stats.gems >= 1 && isabellaFollower() && kihaFollower.followerKiha()) {
			let gems: number = 0;
			MainScreen.text("<i>\"Hey, [name],\"</i> Hel says with a sly grin.  <i>\"Me, Izzy, and spitfire were just playing a little game.  Wanna deal in?\"</i>");
			...
			if (!isabellaAccent()) MainScreen.text(", obviously only somewhat interested");
			MainScreen.text(".");
			...
			//{Make an INT check}
			if (rand(20) + 1 + player.stats.int / 5 >= 15) {
				//[Successful]
			...
			if (player.stats.gems + gems < 0) gems = player.stats.gems;
			if (gems < 0) MainScreen.text("\n\nYou lost " + gems + " gems in the game!");
			else MainScreen.text("\n\nYou gained " + gems + " gems in the game!");
			player.stats.gems += gems;
			statScreenRefresh();
		}
		else {
			Flags.list[FlagEnum.FOLLOWER_HEL_TALKS] = 1;
			talkToHel();
			return;
		}

		Flags.list[FlagEnum.FOLLOWER_HEL_TALKS]++;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private shutUpHelTalks(): void {
		MainScreen.clearText();
		MainScreen.text("Quickly you interject before Helia can continue with the story; you've heard quite enough.  <i>\"Whaaaat?  We were just getting to the good part...\"</i>  Hel whines, rolling her eyes.  <i>\"Hey, you're the one who asked!\"</i>");
	...
	private hugASmokeyTail(): void {
		...
		if (model.time.hours >= 21) {
			...
			if (player.lowerBody.tailType > TailType.NONE) MainScreen.text("s");
			MainScreen.text(" intertwined.  <i>\"Oh, this is nice,\"</i> Hel laughs, running her long, scaled fingers through your hair.  <i>\"So, you wanna stay with me tonight, lover mine?  I'd appreciate the company....\"</i>");
	...
	private heliaRoughSex(output: boolean = true): void {
		if (output) {
			MainScreen.clearText();
			MainScreen.text("<i>\"Mmm, need to blow off some steam, lover?\"</i>  Hel grins, reaching around to undo the straps of her scale bikini.  <i>\"Well, I'm all for that!\"</i>");
		}
		...
		if (player.lowerBody.cockSpot.hasCock() && player.stats.lust >= 33) {
			...
			if (player.cockThatFits(heliaCapacity()) >= 0 && buttons < 9) {
				MainScreen.addButton(buttons, "FuckVag", helScene.beatUpHelAndStealHerWalletFromHerVagina);
				buttons++;
			}
			//85 ass capacity
			if (player.cockThatFits(heliaAnalCapacity()) >= 0 && buttons < 9) {
				MainScreen.addButton(buttons, "Anal", helScene.fuckHelsAss);
				buttons++;
			}
			if (buttons < 9) {
				MainScreen.addButton(buttons, "Get Blown", helScene.helBlowsYou);
				buttons++;
			}
			if (player.cockThatFits(heliaCapacity()) >= 0 && player.cockThatFits2(heliaCapacity()) >= 0 && buttons < 9) {
				MainScreen.addButton(buttons, "DoublePen", helScene.dpHel);
				buttons++;
			}
			if (buttons < 9) {
				MainScreen.addButton(buttons, "Tail Wank", helScene.helTailWanksYourDickBecauseSheLovesYouDesuDesuHoraHora);
				buttons++;
			}
		}
		if (player.lowerBody.vaginaSpot.hasVagina() && player.stats.lust >= 33 && buttons < 9) {
			MainScreen.addButton(buttons, "GetLicked", helScene.getLickedByHel);
			buttons++;
		}
		if (player.stats.lust >= 33 && buttons < 9) {
			MainScreen.addButton(buttons, "TailPeg", helScene.helTailPegging);
			buttons++;
		}
		//Morph-based: [Possession] [Mount Her] [Hanging 69] [Coil Her Up] [Tentafuck])
		if (player.stats.lust >= 33 && player.lowerBody.isTaur()) {
			if (player.lowerBody.cockSpot.hasCock()) {
				if (player.cockThatFits(heliaCapacity()) >= 0 && buttons < 9) {
					MainScreen.addButton(buttons, "Mount Her", centaurMountsCampHel);
					buttons++;
				}
			}
			if (player.hasKeyItem("Centaur Pole") >= 0 && player.lowerBody.vaginaSpot.hasVagina() && buttons < 9) {
				MainScreen.addButton(buttons, "CentaurToy", femtaurPlusCampHel);
				buttons++;
			}
		}
		if (player.stats.lust >= 33 && player.perks.has("Incorporeality") && izmaFollower() && Flags.list[FlagEnum.IZMA_NO_COCK] == 0 && buttons < 9) {
			MainScreen.addButton(buttons, "Possess", heliaCampPossession);
			buttons++;
		}
		if (player.stats.lust >= 33 && player.lowerBody.cockSpot.countType(CockType.TENTACLE) > 6) {
			MainScreen.addButton(buttons, "Tentacles", heliaFollowerTentafuck);
			buttons++;
		}
		if (player.stats.lust >= 33 && player.lowerBody.isNaga()) {
			//"Rough" Sex -- Naga Coil (Female w/ Naga Lower Body)
			if (player.lowerBody.vaginaSpot.hasVagina() && buttons < 9) {
				MainScreen.addButton(buttons, "NagaCoilF", nagaCoilForHelCampWithGirls);
				buttons++;
			}
			//"Rough" Sex (Well, it's in the menu, if not in spirit) -- Naga Coil (Male w/ Naga Lower Body)
			//Male naga shit
			if (player.lowerBody.cockSpot.hasCock() && buttons < 9) {
				if (player.cockThatFits(heliaCapacity()) >= 0) {
					MainScreen.addButton(buttons, "NagaCoilM", inCampHelNagaLuv);
					buttons++;
				}
			}
		}
	...
	private heliaCampPossession(): void {
		MainScreen.clearText();
		MainScreen.text("With a coy grin, you let your ghostly attributes go transparent, and ask Hel if she'd mind taking an extra passenger for a little bit.");
		...
		menu();
		MainScreen.addButton(0, "Izma", possessIzma);
	...
	private inCampHelNagaLuv(): void {
		...
		MainScreen.text("\n\nUnable to move much in your serpentine embrace, Hel begins to rapidly contract her vaginal and anal muscles, squeezing and grinding against your " + CockDescriptor.describeCock(player, x) + " and tip.  Content to close your eyes and bask in the pleasures of your lover, ready to lose yourself in pleasure.  You relax, leaning back on yourself as you buck your hips into your lover's sweet hot box, flick your tail inside her ass, and play your tongue around her mouth, tasting her warm saliva and the aftertaste of beer.");

		MainScreen.text("\n\nYou cum easily, only a small grunt escaping your lips as Hel's delicious cunt brings you over, pulling your " + CockDescriptor.describeCock(player, x) + " deep into her and tightening your hold around her.  The sudden squeeze is the last hurrah as you empty your ");
		if (player.lowerBody.balls > 0) MainScreen.text("[balls]");
		else MainScreen.text("load");
		MainScreen.text(" well into her womb.  Your tail spasms as you cum, running wild inside Hel's rectum until she cums too, yelping into your open mouth as she climaxes a moment after you, girlcum squirting out of her twat until your [hips] are thoroughly soaked.");
	...
	private centaurMountsCampHel(): void {
		...
		if (player.cockArea(x) > heliaCapacity()) MainScreen.text("she can cram in, the rest of your length twitching between her legs");
		else MainScreen.text("you have to offer her");
		MainScreen.text(".  Finally, though, you manage to settle down, your breath coming ragged as Hel's vaginal muscles clamp and release, gently massaging your shaft with expert movements, always keeping you on edge until...");

		MainScreen.text("\n\nTHWACK!");

		MainScreen.text("\n\nYou start forward as a rough tail-slap lands on your flank.  You reflexively step forward, your equine muscles acting on instinct...  which only serves to rock Hel off and down again on your " + CockDescriptor.describeCock(player, x) + ", her impromptu harness moving her like a swing. You arch your back as the movement hammers Hel down on your cock like a free-swinging onahole, but the moment of pleasure lurches out of control when she gives you another hard tail-swat, and another; your insticts carry you forward until you reach a full canter. Shudders and twitches wrack your body as your own motions rock Hel on and off your [cock], slamming her back down on your shaft again and again.");
		...
		MainScreen.text("\n\nAs Hel lets out another high-pitched shriek of pleasure, you finally feel yourself hurtling over the edge of bliss. You rear yourself up as you cum, staggering on your hind legs as the first searing spurt of spooge squirts into the screaming salamander.  A huge spurt of cum jets out of your " + CockDescriptor.describeCock(player, x) + ", smearing Hel's spasming walls white with your potent seed.  As your prick fires off again and again, you slowly bring yourself to a halt, your prick still twitching inside Hel's cum-soaked box.  When you've brought yourself to a complete stop, you feel a sudden lightness as Hel flops out of her harness, quickly surrounded by a small pool of jizz leaking out of her.");
	...
	private heliaFollowerTentafuck(): void {
		MainScreen.clearText();
		MainScreen.text("With a flourish you pull your [armor] off, exposing your writhing mass of tentacle-cocks for Hel`s enjoyment.  \"<i>Wow, think you got enough of th--urf!</i>\" the salamander starts to say before you shut her up with one of your tentacle pricks, lashing forward and plugging her mouth.  Hel staggers back, flailing around before grabbing the cock you've shoved down her throat and giving it a rough tug.  Just as she nearly succeeds in pulling it out, your other wriggling members wrap around her thick thighs; you pull her in toward you, grabbing her shoulders as your pricks peel off her bikini bottom to reveal the smooth face of Helia's crotch.");
		...
		if (player.lowerBody.balls > 0) MainScreen.text("[balls]");
		else MainScreen.text("wriggling tentacles");
		MainScreen.text(" feel painfully empty and white slush pours from all her holes, painting the ground white beneath her dangling feet.");
	...
	private boxWithInCampHel(): void {
		...
		if (player.stats.int / 20 + 1 + rand(20) >= 13) {
		...
		else if (player.stats.str / 20 + 1 + rand(20) >= 13) {
		...
		//If PC fails the Check:  
			...
			player.stats.tou += 2;
			if (player.stats.lust >= 33 && player.lowerBody.cockSpot.hasCock()) {
	...
	private muddyLizardFeet(): void {
		...
		if (player.lowerBody.balls > 0) MainScreen.text("her heel coming to caress your taut sack");
		else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("her heel brushing against the nub of your [clit]");
		else MainScreen.text("her heel going so low as to brush your taint, making your [asshole] clench in surprise before she moves back up your length");
		MainScreen.text(".  You groan lustily as her toes and soft sole jerk you off, gently urging the tight knot of pleasure in your gut to fruition.  Cupping one of her hefty tits, Hel licks her lips as the first thick smears of pre leak from your cock's [cockHead biggest], smearing onto her long-clawed toes.");
		...
		if (player.lowerBody.cockSpot.biggestCocks[0].cockArea() > heliaCapacity()) MainScreen.text("you feel your [cockHead biggest] against the lips of her cervix, as fully into her as you can go");
		else MainScreen.text("your groin meets Helia's, your [cock] wholly enveloped within her");
		MainScreen.text(".  Before you can think to withdraw, Hel tightens her hold on you, whispering huskily, \"<i>Just leave it... just for a second, [name].  Oh, God,</i>\" she moans, her eyes closing as she savors the feeling of you buried so deep inside her, your slick prick twitching as her walls massage it, milking it of the precum that freely flows from your tip.");
	...
	private takeABath(): void {
		...
		if (Flags.list[FlagEnum.HEL_CAN_SWIM] == 0) {
			...
			//[Drop it] [Push it]
			menu();
			MainScreen.addButton(0, "Drop It", heliaFollowerMenu);
			MainScreen.addButton(1, "Push It", pushHelToSwim);
		}
		//{Repeat Intro (If you taught Hel to swin; else, play 1st Time)}
		else {
			MainScreen.clearText();
			MainScreen.text("You ruffle Hel's hair and ask if the fiery salamander's up for a dip.");
			...
			if (player.inventory.armor.displayName == "goo armor" && player.stats.lust >= 33 && 9999 == 0) {
	...
	private pushHelToSwim(): void {
		...
		MainScreen.addButton(0, "Teach Her", teachHelToSwim);
		MainScreen.addButton(1, "Drop It", heliaFollowerMenu);
	}

	//Teach Her
	private teachHelToSwim(): void {
		MainScreen.clearText();
		MainScreen.text("Putting a hand on Hel's shoulder, you offer to teach her how to swim.");
		...
		Flags.list[FlagEnum.HEL_CAN_SWIM] = 1;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	public heliaThreesomes(): void {
		MainScreen.clearText();
		MainScreen.text("You slip an arm around Hel's waist and ask if she's up for some... group activities?");
		...
		//[Kiha]
		menu();
		if (vapulaSlave() && player.stats.lust >= 33 && player.lowerBody.cockSpot.hasCock()) {
			if (player.cockThatFits(heliaCapacity()) >= 0) {
				MainScreen.addButton(1, "Vapula", heliaAndVapula);
			}
			else MainScreen.text("\n\nYou're too big to fuck Helia and Vapula with your cock.");
		}
		if (followerKiha() && player.stats.lust >= 33 && player.gender > 0) {
			MainScreen.addButton(0, "Kiha", campHelAndKihaThreeSome);
		}
		if ((player.inventory.armor.displayName == "goo armor" || Flags.list[FlagEnum.VALARIA_AT_CAMP] == 1) && player.stats.lust >= 33 && player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.addButton(2, "Valeria", helAndValeriaCampThreesomes);
		}
		if (player.stats.lust >= 33 && bimboSophie()) {
			MainScreen.addButton(3, "Sophie", helAndSluttyHarpy);
		}
	...
	private heliaAndVapula(): void {
		MainScreen.clearText();
		let x: number = player.cockThatFits(heliaCapacity());
		let y: number = player.cockThatFits2(heliaCapacity());
		MainScreen.text("You gab Hel and start for the edge of camp, where a certain succubus dwells....");
		...
		MainScreen.text("\n\nYou feel hands wandering across your body, Hel slowly, sensuously, pulling off your [armor].  Soon, the two of you are bare, clothes and weapons discarded all around the succubus.  You push Hel over, grinding your hard " + CockDescriptor.describeCock(player, x) + " between her legs.  As you grope the salamander girl's body, you gesture at Vapula and tell her to come closer so that you can stroke her, too.  You begin caressing each other, spending a few moments in languid pleasure until your " + CockDescriptor.describeCock(player, x) + " looks ready to explode.");
		...
		MainScreen.text("\n\nThis libertine orgy cannot last indefinitely, and even your girls are trembling convulsively, ready to reach their climax. You manage to slide your " + CockDescriptor.describeCock(player, x) + " up the reptile girl's twat a few more times before blasting her with your spooge.  You fill Hel's interior, pouring cum like a torrent into her womb");
		//[IF cum production massive: 
		if (player.cumQ() >= 750) MainScreen.text(" until her belly swells and starts spurting ropes of jism back out of her love-tunnel");
		MainScreen.text(".  Your junk palpitates and bulges as globs push their way out of your shaft before being absorbed by her needy pussy, and her skin and scales seem to literally boil with consumed lust.  At the peak of your orgasm, you stare at your girls' faces: Hel's is nothing but complete, insensate bliss, while Vapula is literally crying in ecstasy, tears of joy streaming down her cheeks.");

		MainScreen.text("\n\nSeeing how much pleasure you're giving them is quite touching, and you give each of them a long, ardent kiss, letting each girl know how the other tastes like.  At last, when your " + CockDescriptor.describeCock(player, x) + " has unloaded its last rope of jism, you pull out and break the kiss.  Hel pants and sighs, utterly drained; you see her tail popping out of Vapula's ass, and the released succubus just falls on her back, cunt exposed and mouth open in blissful exhaustion.");
		...
		if (!followerHel()) {
			MainScreen.text("  You kiss her and ask if she'll be able to find her way back to her home.");
	...
	private dudeHeliaAndKihaThreeSome(): void {
		...
		MainScreen.text("\n\nKiha gives the salamander a grave look, but soon softens, nuzzling her chin in your " + HeadDescriptor.describeHair(player) + ".  Hel sighs, but leans down to give you each a quick kiss on the head; her foot never misses a beat, though, and combined with Kiha's sensual massaging, you realize you're dangerously close to the edge again.  You grit your teeth and let loose an animalistic growl, the only forewarning the distracted lizards get before the first burst issues forth from your [cockHead], smearing the sole of Hel's foot with creamy seed.  The salamander makes a happy gasp as another squirt of cum smears her clawed toes, a particularly strong arc fountaining so high that it smears her thighs.");
	...
	private helAndValeriaCampThreesomes(): void {
		...
		if (player.inventory.armor.displayName == "goo armor") MainScreen.text("Giving Hel a little wink, you ask your current clothing if she'd like to come out and play.  The gooey parts of your armor slither out, Valeria's face materializing out of your breastplate with a grin.  \"<i>Howdy, firebutt,</i>\" the armor-goo says, giving Hel a big hug. Your salamander lover giggles, squeezing Val so hard she pops in half, only to reform beside you.  \"<i>Thanks for the invite, partner... I was getting pretty hungry, you know...</i>\"");
		//{if PC is NOT wearing Valeria:}
		else MainScreen.text("You look over to your stash and give a sharp whistle, calling Valeria over.  An amorphous blue blob soon pops up, forming into Valeria's shapely body as she strides over to give Helia a big hug.  Your salamander lover giggles, squeezing Val so hard she pops in half, only to reform beside you as you ask if she'd like to join in on your fun.  She beams, nodding quickly.  \"<i>Thanks for the invite, partner... I was getting pretty hungry, you know...</i>\"");

		menu();
		MainScreen.addButton(0, "Next", helXValeriaFemalePC);
	}

	//Hel x Valeria Female Version
	private helXValeriaFemalePC(): void {
		MainScreen.clearText();
		MainScreen.text("In the blink of an eye, Valeria slithers around Hel's body, almost completely encasing her in thick blue goo, leaving only her head and her resplendent reptilian rack bare.  Hel grins at you, putting a hand to her crotch and taking a handful of goo between her fingers.  With a bit of effort, she shapes a pair of thick blue pricks out of her groin, each shimmering in the sunlight as Hel strokes Valeria's new appendages, coaxing out a drop of slimey pre from both tips.  Valeria's face reappears on Hel's flat belly a moment after she's done, looking down to her groin with glee as she further shapes the erect pillars of slime, morphing them into a thick knotty doggy-dick on top, and a huge-flared horsecock on bottom.  You lick your lips as the dog-dick's knot inflates enticingly.");
	...
	private helGuardToggle(): void {
		...
		if (Flags.list[FlagEnum.HEL_GUARDING] == 0) {
			MainScreen.text("You ask Hel if she wouldn't mind watching the perimeter for a few hours during the night.");
			MainScreen.text("\n\n\"<i>Sure thing, [name]!  I've got your back!</i>\" she says, her tail reaching around to give you a pop on the [butt].");
			Flags.list[FlagEnum.HEL_GUARDING] = 1;
		}
		//[If Hel IS guarding Camp]
		else {
			MainScreen.text("You tell Hel she doesn't need to keep watch at night anymore.");
			MainScreen.text("\n\n\"<i>Is that a fact?  Well, I guess that gives us more time to party at night!</i>\"");
			Flags.list[FlagEnum.HEL_GUARDING] = 0;
		}
		menu();
		MainScreen.addButton(0, "Next", heliaFollowerMenu);
	}
	...
	private heliaAnalTrainingPrompt(): void {
		MainScreen.clearText();
		Flags.list[FlagEnum.HELIA_ANAL_TRAINING_OFFERED] = 1;
		MainScreen.text("“<i>Hey there, lover mine. Got a minute?</i>” Helia asks with dangerous glint in her eyes.");
		...
		if (player.tallness < 60) MainScreen.text("her bosom towering over you");
		else if (player.tallness < 72) MainScreen.text("towering over you");
		else if (player.tallness < 90) MainScreen.text("meeting you eye to eye");
		else MainScreen.text("looking up at you confidently");
		MainScreen.text(" as she boldly reaches into your [armor] to squeeze your ");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("biggest ");
	...
	private giveHeliaAnalTraining(): void {
		MainScreen.clearText();
		MainScreen.text("You hand Helia a bottle of goblin ale and question if she’s sure this won’t make her loose.");
		...
		if (player.lowerBody.balls > 0) MainScreen.text("She tilts forward to press her chin into your [sack] and begins to lather your [balls] with frothy, warm, salamander spit. One hand hefts a weighty nut as she works, caressing it as if it would somehow coax the cum out of it. ");
		MainScreen.text("Helia slides her palms along either side of your urethra, letting her fingers handle the central, sensitive bit on the underside, following in the path of slick saliva she left for them to follow. She rubs her nose into the sensitive area just below your [cockHead biggest]. You grunt as pre-cum begins to pour in response, your bloated boner flexing atop Helia’s head as you feel a dangerously powerful orgasm welling up within you.");
		...
		if (player.lowerBody.balls > 0) MainScreen.text(", hard enough to lift your nuts in their [sack]");
		MainScreen.text(", and you erupt, cumming a steamy hot load of spunk straight onto Helia’s face, head, and back.");
		if (player.cumQ() < 250) MainScreen.text(" You run out before long, but it’s still enough to make her a white painted mess. She licks the excess from her upper lip and shudders happily.");
		else if (player.cumQ() < 2000) MainScreen.text(" You spurt rope after rope onto her, actually drenching her hair with alabaster moisture. Her back and face look equally smothered with spooge when you finish. She’s happy to wipe the goop out of her eyes and shovel it into her mouth, swallowing it down with a satisfied smile.");
		else {
			MainScreen.text(" You pour ivory waves of spooge across her body, soaking her from head to toe with the fruit of your loins, liberally basting her with baby-batter. She groans as the excess washes out around her knees");
			if (player.cumQ() >= 3000) MainScreen.text(", but you keep cumming, cascading fresh waves onto her until there’s not a single part of her that isn’t stained and dripping with your salty outflow");
			MainScreen.text(". Helia licks her hands clean, wipes her eyes and devours that as well, and then gives you a dripping-wet, sensuous smile.");
		}
		...
		Flags.list[FlagEnum.HELIA_ANAL_TRAINING] = 1;
		if (player.lowerBody.cockSpot.biggestCocks[0].cockArea() < helAnalCapacity()) {
			MainScreen.text("then maybe I can handle you, my hung " + player.mf("stud", "breeder") + ".");
		}
		else {
			MainScreen.text("then maybe we can see about trying something bigger.");
		}
		MainScreen.text("</i>”");

		player.inventory.items.consumeItem(consumables.GOB_ALE, 1);
		player.orgasm();
		player.stats.sens += -1;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private heliaGapeSceneChoices(): void {
		if (Flags.list[FlagEnum.HELIA_ANAL_TRAINING] == 0) giveHeliaAnalTraining();
		else if (Flags.list[FlagEnum.HELIA_ANAL_TRAINING] == 1) heliaAnalTrainingPartTwo();
	}
	...
	private heliaAnalTrainingPartTwo(): void {
		player.inventory.items.consumeItem(consumables.GOB_ALE, 1);
		MainScreen.clearText();
		MainScreen.text("Pulling out another bottle of ale you dangle it before the slutty salamander, saying, “<i>A present for my favorite buttslut.</i>”");
		...
		//[Yes] [No]
		menu();
		MainScreen.addButton(0, "Anal Train", yesMakeHeliaAGapeSlut);
		MainScreen.addButton(1, "Stop", noGapeSlutForHelia);
	}
	//Stop the Asspocalypse
	private noGapeSlutForHelia(): void {
		MainScreen.clearText();
		MainScreen.text("You step up and kiss Helia, telling her that she doesn’t have to this for you, at least not right now.");
	...
	private yesMakeHeliaAGapeSlut(): void {
		...
		if (player.stats.str < 50) MainScreen.text("strain to ");
		MainScreen.text("heft it up, pressing the [cockHead biggest] on Helia’s thirsty lips. She sandwitches the " + num2Text(Math.round(player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockLength)) + "-thick shaft in between her tits and left arm while her right tickles the sensitive underside. Her tongue slides around your urethra, collecting every single drop of pre-cum and stimulating nerve endings you didn’t even know you had. You groan and flex powerfully, preparing to dump a thick load into the hot salamander’s mouth. She spreads her lips around your cum-slit and kisses your cock hard, sucking powerfully, drawing the bubbling tide of wide straight out of your [balls] with her hungry movements.");
		...
		if (player.cumQ() < 1000) MainScreen.text(", ignoring the way your sheer volume of spunk leaks from the corners of her mouth and nose, her belly gradually rounding with cum in addition to thick dildo");
		MainScreen.text(". The slutty salamander eventually pulls it off and ");
		if (player.cumQ() < 2000) MainScreen.text("squeezes a last few dollops onto her face, letting them stain her with your scent.");
		else MainScreen.text("allows you to hose her down with your ongoing waves of salty jism, soaking her in your sexual scent.");
		MainScreen.text(" Helia burps. “<i>Fuck me that was awesome! Whoah, don’t mind me, but I, uh... I think I’m gonna stay here for a little while, maybe just rock up and down till I come down off this buzz. She blushes and begins to finger her snatch again. Just let me... just let me get used to.</i>” She shoos you away and goes back to toying with herself, getting used to her new “flexibility”.");
	...
	public heliasBirthday(): void {
		...
		Flags.list[FlagEnum.HELIA_BIRTHDAY_OFFERED] = 1;
		menu();
		MainScreen.addButton(1, "Stay Home", stayHomeFromHeliaParty);
		MainScreen.addButton(0, "Go To Party", goWithHelia);
	}
	...
	private stayHomeFromHeliaParty(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>Aww, lame,</i>\" Hel groans, sighing.  \"<i>Fiiiiiine.  I guess I'll just go have awesome fun and party hard without you. Don't stick too hard into that mud, lover,</i>\" she chuckles, giving you a reassuringly affectionate kiss on the cheek before trucking off.");
		doNext(playerMenu);
	}
	...
	private helPartyMenu(): void {
		menu();
		//heliaDrinks
		if (Flags.list[FlagEnum.HELIA_BDAY_DRINKS] == 0) MainScreen.addButton(0, "Drinks", heliaDrinks);
		//heliaHakonAndKiri
		if (Flags.list[FlagEnum.HELIA_BDAY_HAKON_AND_KIRI] == 0) MainScreen.addButton(1, "Hakon 'n Kiri", heliaHakonAndKiri);
		//heliaPhoenixes
		if (Flags.list[FlagEnum.HELIA_BDAY_PHOENIXES] == 0) MainScreen.addButton(2, "Phoenixes", heliaPhoenixes);
		//heliaAndFoxTwins
		if (Flags.list[FlagEnum.HELIA_BDAY_FOX_TWINS] == 0) MainScreen.addButton(3, "Fox Twins", heliaAndFoxTwins);
		//leaveWithoutSex
		MainScreen.addButton(4, "Leave", leaveWithoutSex);
		//leaveWithGirls
		if (Flags.list[FlagEnum.HELIA_BDAY_FOX_TWINS] > 0) MainScreen.addButton(5, "Leave w/Girls", leaveWithGirls);
	}
	...
	private goWithHelia(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>Sounds like fun.  Let's go,</i>\" you say, offering your arm to Hel.  She grins and takes it, holding herself close as she takes the lead, steady paces quickly drawing you out into the vast deserts of Mareth beside your lover.");
		//{If Kiha, Isabella, and/or Valeria are at camp:}
		if (isabellaFollower() || followerKiha() || Flags.list[FlagEnum.VALARIA_AT_CAMP] == 1) {
			MainScreen.text("  Looking over your shoulder as you depart, though, you notice that camp looks particularly... empty. Perhaps your other camp-mate");
			if (camp.companionsCount() > 1) MainScreen.text("s are");
			else MainScreen.text(" is");
			MainScreen.text(" out, too? You shrug. You and Hel aren't the only ones with lives, you suppose.");
		}
		...
		if (player.tallness <= 72) MainScreen.text(", her head resting on your shoulder");
		MainScreen.text(".  \"<i>Hey, thanks for coming, [name],</i>\" she says quietly, squeezing your arm, \"<i>Kiri's done nothing but talk about you since the tower.  I think someone has a crush...</i>\"");
		...
		let list: Array = ["Edryn"];
		if (Flags.list[FlagEnum.VALARIA_AT_CAMP] == 1) list.push("Valeria");
		...
		MainScreen.text(formatStringArray(list) + ". You turn to Helia and cock a grin as you see the surprise etched on her face, followed by the growing smile.  \"<i>Happy birthday, Helia!</i>\" they all shout out at once, raising mugs frothing over with beer.");
		/*
			addToList("Edryn");
			if(Flags.list[FlagEnum.VALARIA_AT_CAMP] == 1) {
				addToList("Valeria");
			}
			if(followerKiha()) {
				addToList("Kiha");
			...
			MainScreen.text(outputList() + ". You turn to Helia and cock a grin as you see the surprise etched on her face, followed by the growing smile.  \"<i>Happy birthday, Helia!</i>\" they all shout out at once, raising mugs frothing over with beer.");
		*/
		...
		if (Flags.list[FlagEnum.HARPY_QUEEN_EXECUTED] > 0) MainScreen.text("who forgave us for the queen's death");
		else MainScreen.text("who wanted to be friends still, even after the Tower");
		MainScreen.text(" and... it sort of became a thing.  Hope you don't mind!</i>\"");
		...
		if (Flags.list[FlagEnum.HARPY_QUEEN_EXECUTED] > 0) MainScreen.text("finally killing that bitch of a queen");
		else MainScreen.text("setting the queen right, but leaving her alive for when it's finally time to bring the fight to Lethice");
		MainScreen.text(".  You nod at his thanks, and his hand is soon replaced by Helia's as she grabs you on her way toward the gigantic cake.  The towering white confectionary seems more than big enough to feed everyone in the group, a huge extravagance on the part of her family.  You hear a quiet whisper about ");
		if (Flags.list[FlagEnum.FREED_VALA] == 1) MainScreen.text("a certain waitress fairy having... supplied the icing herself");
		else MainScreen.text("a certain minotaur chef having supplied the icing himself");
		MainScreen.text(" as Hel is handed an oversized knife and urged to blow out the candles atop it, arranged into a perfect 23.");
		...
		if (Flags.list[FlagEnum.HEL_FOXY_FOURSOME_WARNED] == 1) MainScreen.text("a pair of lovely fox-eared herms");
		else MainScreen.text("Miko and Mai");
		MainScreen.text(" burst out, their big breasts covered in nothing but tiny pink pasties and an all-too-small thong sheathing their red pricks.  Arm in arm with tail wagging furiously, they cry out \"<i>Happy birthday, Hel!</i>\" before hopping out and slipping into the salamander's arms.  \"<i>Ladies,</i>\" Hel says, grinning like an idiot, a moment before she's pushed into a waiting chair behind her.  One of the girl's busts is quickly shoved into her face as the rest of the group laughs and starts passing out another round of drinks.");
	...
	private heliaDrinks(): void {
		MainScreen.clearText();
		//Drinks
		MainScreen.text("You saunter up to the bar, where the staff have laid out more than enough beers for the entire party.  Edryn and a handful of phoenixes are milling around, exchanging tips for swordsmanship: Edryn advocating charging in with a greatweapon swinging, as the phoenixes try and explain their shield-wall tactics.  You knock back a drink and listen in, watching with interest as the phoenix girls get progressively more and more flustered");
		//if PC is male:
		if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" just as you do");
		MainScreen.text(" in the presence of the steadily mounting pheromone cloud around the lusty centauress.  Before you've finished your drink, all the half-harpies are sporting sizable tents in their britches and are subtly sniffing around, trying to figure out what's happening to them... just before Edryn grabs the lead phoenix by the arm and leads her off into one of the back rooms, shooting the others a wink and a flicking tail over her drooling horse-sex.  The others are quick to follow.");
		Flags.list[FlagEnum.HELIA_BDAY_DRINKS] = 1;
		helPartyMenu();
	}
	...
	private heliaHakonAndKiri(): void {
		...
		MainScreen.text(".  We salamanders are a bit more open about that sort of thing, though, if you haven't noticed.  Helia's regaled us with more than a few of her 'stories,' let me tell you. Isn't that right, sweetheart?</i>\"");
		...
		Flags.list[FlagEnum.HELIA_BDAY_HAKON_AND_KIRI] = 1;
		helPartyMenu();
	}
	...
	private heliaPhoenixes(): void {
		MainScreen.clearText();
		MainScreen.text("You approach the small squad of phoenixes milling around the cake, mostly talking amongst themselves.  They nod as you approach, a bit wary of the person who recently kicked their asses inside out.  Still, they nod respectfully as you approach.");
		...
		if (Flags.list[FlagEnum.HARPY_QUEEN_EXECUTED] > 0) MainScreen.text("dead");
		else MainScreen.text("deposed");
		MainScreen.text(", he thought it would be good to try and at least... know each other.  I'm not complaining.  Things like this party... never would have seen anything like this under Queen Calais.  It's nice to see new things, meet new people.</i>\"");

		MainScreen.text("\n\nYou see a few glances going toward Edryn and the fox twins");
		if (followerKiha() && isabellaFollower()) MainScreen.text(", then to the fiery dragoness and the ultra-busty cowgirl talking in the corner");
		MainScreen.text(", and you figure you know what they mean.");
		Flags.list[FlagEnum.HELIA_BDAY_PHOENIXES] = 1;
		helPartyMenu();
	}
	...
	private heliaAndFoxTwins(): void {
		MainScreen.clearText();
		MainScreen.text("You approach the foxy strippers, still well into giving Helia a dual lap-dance, their red puppy peckers poking well out of their tiny thongs.  They grin as you approach, and Hel pulls you into her lap, and you quickly find yourself with a pair of big, soft breasts to rest your head on, courtesy of ");
		if (Flags.list[FlagEnum.HEL_FOXY_FOURSOME_WARNED] == 1) MainScreen.text("Mai");
		else MainScreen.text("one of the foxes");
		MainScreen.text(".  Hel plants a deep, serpent-tongue filled kiss on your lips, nuzzling her cheek on your [chest].");
		...
		Flags.list[FlagEnum.HELIA_BDAY_FOX_TWINS] = 1;
		helPartyMenu();
	}
	//Leave {Or: Joke's on you, no (special) birthday sex!}
	private leaveWithoutSex(): void {
		MainScreen.clearText();
		MainScreen.text("You decide you've had about enough of the party for now. You slip over to Hel and tell her you're ready to head out when she is.");
	...
	private helAndSluttyHarpy(): void {
		...
		// Choice
		menu();
		MainScreen.addButton(0, "Cock", helAndSluttyHarpyMale);
		MainScreen.addButton(1, "Pussy", helAndSluttyHarpyFemale);
		MainScreen.addButton(9, "Back", heliaThreesomes);
	}

	private helAndSluttyHarpyMale(): void {
		MainScreen.clearText();

		MainScreen.text("You take Helia’s hand and lead her through the camp in an attempt to find someone to include in your threesome. After some searching you see Sophie sitting on a rock and get a wonderful idea. You ask Hel if she would be ok with a threesome with the busty harpy. Her smile turns to a serious frown for a moment.");
		...
		if (player.biggestCockLength() >= 8) MainScreen.text(" no longer be hilted inside");
		else MainScreen.text(" slip out of");
		MainScreen.text(" her sodden cunt, and hear Hel gasp for air. Before the birdslut can comment on the matter, you spin her around and push her down onto Helia, leaving them face to face.");
		...
		if (player.lowerBody.cockSpot.count() == 1) {
			...
			MainScreen.text(" to boil over. White jissom blasts from your [cock], coating Helia and Sophie’s stomachs, as well as the underside of the pair’s tits.");
			if (player.lowerBody.vaginaSpot.count() > 0) MainScreen.text(" Your unused [vagina] cums with your cock in a show of sympathetic orgasm, coating your thighs with femcum.");
		}
		// 2-3 cocks
			...
			if (player.cumQ() < 500) MainScreen.text(" with your cum");
			else {
				MainScreen.text(" full of cum");
				if (player.cumQ() <= 1000) {
					MainScreen.text(" until it overflows");
					if (player.cumQ() >= 5000) MainScreen.text(", pooling around your knees");
				}
			}
			MainScreen.text(".");
			if (player.lowerBody.cockSpot.count() == 3) MainScreen.text(" Your unused cock splatters the two with hot, white jizz.");
			if (player.lowerBody.vaginaSpot.count() > 0) MainScreen.text(" Your unused [vagina] cums with your cocks in a show of sympathetic orgasm, coating your thighs with femcum.");
		}
		// > 4 cocks
		else {
			...
			if (Flags.list[FlagEnum.HELIA_ANAL_TRAINING] == 0) MainScreen.text(" tight");
			else if (Flags.list[FlagEnum.HELIA_ANAL_TRAINING] == 1) MainScreen.text(" well-trained");
			else if (Flags.list[FlagEnum.HELIA_ANAL_TRAINING] >= 2) MainScreen.text(" gaped");
			MainScreen.text(" asshole. She gasps at the sudden double penetration, and you reciprocate because of the furnace-like heat of both her holes. Your [cock biggest3] lines up with Sophie’s slutty pussy and drives home while your [cock smallest] does the same to her ass."); // Haha look at me being a lazy shit.
			...
			MainScreen.text(". The two girls seem to share your problem, their teeth clenched and clinging tightly to each other trying to resist. Seeing their clearly shared pleasure in the moment puts you over the edge, and you let loose the torrent of liquid lust from [eachCock]. Sophie cums as the tsunami of jizz floods her levies, the copious amount spilling back out of her holes and pooling around your knees.");
			if (player.lowerBody.cockSpot.count() > 4) MainScreen.text(" Your extra cocks spray pearly cum all over the nearby ground.");
			if (player.lowerBody.vaginaSpot.count() > 0) MainScreen.text(" Your unused [vagina] cums with your cocks in sympathetic orgasm, coating your thighs with femcum.");
		}
		// Merge
	...
	private helAndSluttyHarpyFemale(): void {
		...
		if (player.lowerBody.cockSpot.count() >= 1) {
			MainScreen.text("The blasts spatter against the underside of Sophie’s gigantic melons and slide off Hel’s sides like an overfull cream pastry.");
		}
	...
	private helAndSluttyHarpyGenderless(): void {
		...
		MainScreen.text("You walk up to Helia and ask if she would be up for fucking you with that wondrous tail of hers. She smiles slyly and wraps her arms around you. You take her response as an unspoken yes, hugging her back and pulling her off to find a better place for your loving.");
