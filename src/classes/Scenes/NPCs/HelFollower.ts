...
private helAnalCapacity():number {
	let anal: number = 85;
	if(flags[FlagEnum.HELIA_ANAL_TRAINING] >= 1) anal += 100;
	if(flags[FlagEnum.HELIA_ANAL_TRAINING] >= 2) anal += 300;
	return anal;
}
...
public helAffection(diff:number = 0):number {
	if(flags[FlagEnum.HEL_AFFECTION_FOLLOWER] > 70 && flags[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] == 0) flags[FlagEnum.HEL_AFFECTION_FOLLOWER] = 70;
	if(flags[FlagEnum.HEL_AFFECTION_FOLLOWER] < 100 || (flags[FlagEnum.HEL_BONUS_POINTS] == 0 && diff < 0)) {
		flags[FlagEnum.HEL_AFFECTION_FOLLOWER] += diff;
		if(flags[FlagEnum.HEL_AFFECTION_FOLLOWER] >= 100) flags[FlagEnum.HEL_AFFECTION_FOLLOWER] = 100;
		if(flags[FlagEnum.HEL_AFFECTION_FOLLOWER] < 0) flags[FlagEnum.HEL_AFFECTION_FOLLOWER] = 0;
	}
	else if(followerHel()) {
		flags[FlagEnum.HEL_AFFECTION_FOLLOWER] = 100;
		flags[FlagEnum.HEL_BONUS_POINTS] += diff * 3;
		if(diff > 0) if(flags[FlagEnum.HEL_BONUS_POINTS] > 150) flags[FlagEnum.HEL_BONUS_POINTS] = 150;
		else if(diff < 0) if(flags[FlagEnum.HEL_BONUS_POINTS] < 0) flags[FlagEnum.HEL_BONUS_POINTS] = 0;		
	}
	return flags[FlagEnum.HEL_AFFECTION_FOLLOWER];
	trace("HEL AFFECTION" + flags[FlagEnum.HEL_AFFECTION_FOLLOWER]);
}
...
public heliaFollowerIntro():void {
	...
	if(flags[FlagEnum.HARPY_QUEEN_EXECUTED] > 0) MainScreen.text("kill");
...
private iCantLetFireButtsRapeMyCampsButt():void {
	...
	if(flags[FlagEnum.HEL_FOXY_FOURSOME_WARNED] > 0 || flags[FlagEnum.HEL_EDRYN_OFFER] > 0) MainScreen.text("like usual");
	...
	flags[FlagEnum.HELIA_FOLLOWER_DISABLED] = 1;
	doNext(playerMenu);
}
...
private afterMoveInBoningAnalFireTail():void {
	...
	flags[FlagEnum.HEL_FOLLOWER_LEVEL] = 2;
	doNext(playerMenu);
}
...
private justFriendsWithAnalTailWaifu():void {
	...
	flags[FlagEnum.HELIA_FOLLOWER_DISABLED] = .5;
...
public helFollowersIntro():void {
	...
	if(followerKiha() && flags[FlagEnum.HEL_INTROS_LEVEL] < 1) {
		flags[FlagEnum.HEL_INTROS_LEVEL] = 1;
	...
	//If Isabella 
	else if(isabellaFollower() && flags[FlagEnum.HEL_INTROS_LEVEL] < 2) {
		flags[FlagEnum.HEL_INTROS_LEVEL] = 2;
	...
	//If Rath is in Camp
	else if(flags[FlagEnum.HEL_INTROS_LEVEL] < 3 && player.statusAffects.has("CampRathazul")) {
		flags[FlagEnum.HEL_INTROS_LEVEL] = 3;
	...
	//If Bimbo Sophie is at Camp:
	else if(flags[FlagEnum.HEL_INTROS_LEVEL] < 4 && bimboSophie() && flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) {
		flags[FlagEnum.HEL_INTROS_LEVEL] = 4;
		...
		flags[FlagEnum.HEL_INTROS_LEVEL] = 9001;
		playerMenu();
		return;
	}
	menu();
...
private bimboSophieGetsBooted4Firebutt():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] >= 3) {
		...
		flags[FlagEnum.MINO_SONS_HAVE_SOPHIE] = 1;
	}
...
private workItOutWithSophieAndFireTits():void {
	...
	flags[FlagEnum.KEEP_HELIA_AND_SOPHIE] = 1;
	doNext(camp.returnToCampUseOneHour);
}
...
public heliaFollowerMenu(display:boolean = true):void {
	...
	if(flags[FlagEnum.HEL_FOLLOWER_LEVEL] == 2) {
		if(flags[FlagEnum.HELIA_ANAL_TRAINING_OFFERED] == 0 && display && player.lowerBody.cockSpot.biggestCocks[0].cockArea() > heliaAnalCapacity()) {
		...
		if (flags[FlagEnum.HEL_LOVE] == 1 || flags[FlagEnum.HEL_LOVE] == -1) {
			if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(heliaCapacity()) >= 0 && player.lust >= 33 &&
					!helPregnant() && flags[FlagEnum.HELSPAWN_AGE] == 0) MainScreen.addButton(7,"Have A Kid",helSpawnScene.haveAKid);
		}
...
private heliaOptions():void {
	if (kGAMECLASS.helScene.pregnancy.event >= 3 && flags[FlagEnum.HELIA_TALK_SEVEN] == 0) {
		helSpawnScene.heliaTalkSeven();
		return;
	}
	if (flags[FlagEnum.HELSPAWN_AGE] == 1 && flags[FlagEnum.HEL_TALK_EIGHT] == 0) {
	...
	if(flags[FlagEnum.HELIA_ANAL_TRAINING_OFFERED] > 0 && flags[FlagEnum.HELIA_ANAL_TRAINING] < 2 && player.lowerBody.cockSpot.biggestCocks[0].cockArea() > heliaAnalCapacity() && player.hasItem(consumables.GOB_ALE,1)) MainScreen.addButton(3,"Anal Train",heliaGapeSceneChoices);
	MainScreen.addButton(5,"Bathe",takeABath);
	if(flags[FlagEnum.HELSPAWN_AGE] == 1) MainScreen.addButton(7,flags[FlagEnum.HELSPAWN_NAME],helSpawnScene.playWithYourKid);
	if(flags[FlagEnum.HEL_GUARDING] == 0) MainScreen.addButton(8,"GuardCamp",helGuardToggle);
...
private helFollowerAppearance():void {
	...
	if(flags[FlagEnum.HEL_ISABELLA_THREESOME_ENABLED] >= 1) MainScreen.text("  Hel's blue bandanna is wrapped around her brow, mostly hidden beneath her fiery hair.");
...
private talkToHel():void {
	...
	if(flags[FlagEnum.FOLLOWER_HEL_TALKS] == 0) {
		...
		//(If Toughness <50: 
		if(player.tou < 50) MainScreen.text("You cough and gag, eyes misting as the pure-grain booze burns your throat worse than hot coals.  Hel laughs riotously, slapping you on the back hard enough to cause booze to snort out through your nose.");
	...
	//Hel Talk 2
	else if(flags[FlagEnum.FOLLOWER_HEL_TALKS] == 1) {
	...
	//Hel Talk 3(C)
	else if(flags[FlagEnum.FOLLOWER_HEL_TALKS] == 2) {
	...
	//Hel Talk 4(C)
	else if(flags[FlagEnum.FOLLOWER_HEL_TALKS] == 3) {
		//Hel Talk 4(C)
	...
	//Hel Talk 5(C)
	else if(flags[FlagEnum.FOLLOWER_HEL_TALKS] == 4) {
	...
	//Hel Talk 6 (Needs Isabella and Kiha at camp; at least 1 gem)(C)
	else if(flags[FlagEnum.FOLLOWER_HEL_TALKS] == 5 && player.stats.gems >= 1 && isabellaFollower() && kihaFollower.followerKiha()) {
		let gems: number = 0;
		...
		flags[FlagEnum.FOLLOWER_HEL_TALKS] = 1;
		talkToHel();
		return;
	}

	flags[FlagEnum.FOLLOWER_HEL_TALKS]++;
	doNext(camp.returnToCampUseOneHour);
}
...
private hugASmokeyTail():void {
	...
	if(model.time.hours >= 21) {
		...
		if(player.tailType > TAIL.NONE) MainScreen.text("s");
		...
		flags[FlagEnum.SLEEP_WITH] = "Helia";
		menu();
...
private heliaRoughSex(output:boolean = true):void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina() && player.lust >= 33 && buttons < 9) {
	...
	if(player.lust >= 33 && player.perks.has("Incorporeality") && izmaFollower() && flags[FlagEnum.IZMA_NO_COCK] == 0 && buttons < 9) {
...
private boxWithInCampHel():void {
	...
	//If PC fails the Check:  
		...
		dynStats("tou", 2);
		if(player.lust >= 33 && player.lowerBody.cockSpot.hasCock()) {
...
private takeABath():void {
	//{First Time intro}
	if(flags[FlagEnum.HEL_CAN_SWIM] == 0) {
		MainScreen.clearText();
...
//needs wings or 50+ str, dick, added as an option \"<i>Carry her to camp</i>\" when you defeat Hel while she is in Fuckbuddy mode.
	...
	//[Kiha]
	menu();
	if(vapulaSlave() && player.lust >= 33 && player.lowerBody.cockSpot.hasCock()) {
		if(player.cockThatFits(heliaCapacity()) >= 0) {
	...
	if((player.armorName == "goo armor" || flags[FlagEnum.VALARIA_AT_CAMP] == 1) && player.lust >= 33 && player.lowerBody.vaginaSpot.hasVagina()) {
...
private helGuardToggle():void {
	...
	if(flags[FlagEnum.HEL_GUARDING] == 0) {
...
private heliaAnalTrainingPrompt():void {
	MainScreen.clearText();
	flags[FlagEnum.HELIA_ANAL_TRAINING_OFFERED] = 1;
...
private giveHeliaAnalTraining():void {
	...
	flags[FlagEnum.HELIA_ANAL_TRAINING] = 1;
...
private heliaGapeSceneChoices():void {
	if(flags[FlagEnum.HELIA_ANAL_TRAINING] == 0) giveHeliaAnalTraining();
	else if(flags[FlagEnum.HELIA_ANAL_TRAINING] == 1) heliaAnalTrainingPartTwo();
}
...
private yesMakeHeliaAGapeSlut():void {
	MainScreen.clearText();
	flags[FlagEnum.HELIA_ANAL_TRAINING] = 2;
	...
	if(player.str < 50) MainScreen.text("strain to ");
...
public heliasBirthday():void {
	...
	flags[FlagEnum.HELIA_BIRTHDAY_OFFERED] = 1;
	menu();
...
private helPartyMenu():void {
	menu();
	//heliaDrinks
	if(flags[FlagEnum.HELIA_BDAY_DRINKS] == 0) MainScreen.addButton(0,"Drinks",heliaDrinks);
	//heliaHakonAndKiri
	if(flags[FlagEnum.HELIA_BDAY_HAKON_AND_KIRI] == 0) MainScreen.addButton(1,"Hakon 'n Kiri",heliaHakonAndKiri);
	//heliaPhoenixes
	if(flags[FlagEnum.HELIA_BDAY_PHOENIXES] == 0) MainScreen.addButton(2,"Phoenixes",heliaPhoenixes);
	//heliaAndFoxTwins
	if(flags[FlagEnum.HELIA_BDAY_FOX_TWINS] == 0) MainScreen.addButton(3,"Fox Twins",heliaAndFoxTwins);
	//leaveWithoutSex
	MainScreen.addButton(4,"Leave",leaveWithoutSex);
	//leaveWithGirls
	if(flags[FlagEnum.HELIA_BDAY_FOX_TWINS] > 0) MainScreen.addButton(5,"Leave w/Girls",leaveWithGirls);
}
...
private goWithHelia():void {
	...
	if(isabellaFollower() || followerKiha() || flags[FlagEnum.VALARIA_AT_CAMP] == 1) {
	...
	//If PC did UrtaQuest: 
	MainScreen.text("\n\n");
	if(flags[FlagEnum.URTA_MET_HEL] == 1) MainScreen.text("From the quick look she and the salamander exchange, you get the feeling they've met before. Weird. Still, ");
	...
	if(flags[FlagEnum.VALARIA_AT_CAMP] == 1) list.push("Valeria");
...
/*
	addToList("Edryn");
	if(flags[FlagEnum.VALARIA_AT_CAMP] == 1) {
		addToList("Valeria");
	}
	if(followerKiha()) {
		addToList("Kiha");
...
*/	
	...
	if(flags[FlagEnum.HARPY_QUEEN_EXECUTED] > 0) MainScreen.text("who forgave us for the queen's death");
	...
	if(flags[FlagEnum.HARPY_QUEEN_EXECUTED] > 0) MainScreen.text("finally killing that bitch of a queen");
	...
	if(flags[FlagEnum.FREED_VALA] == 1) MainScreen.text("a certain waitress fairy having... supplied the icing herself");
	...
	if(flags[FlagEnum.HEL_FOXY_FOURSOME_WARNED] == 1) MainScreen.text("a pair of lovely fox-eared herms");
...
private heliaDrinks():void {
	...
	flags[FlagEnum.HELIA_BDAY_DRINKS] = 1;
	helPartyMenu();
}
...
private heliaHakonAndKiri():void {
	...
	flags[FlagEnum.HELIA_BDAY_HAKON_AND_KIRI] = 1;
	helPartyMenu();
}
...
private heliaPhoenixes():void {
	...
	if(flags[FlagEnum.HARPY_QUEEN_EXECUTED] > 0) MainScreen.text("dead");
	...
	flags[FlagEnum.HELIA_BDAY_PHOENIXES] = 1;
	helPartyMenu();
}
...
private heliaAndFoxTwins():void {
	...
	if(flags[FlagEnum.HEL_FOXY_FOURSOME_WARNED] == 1) MainScreen.text("Mai");
	...
	flags[FlagEnum.HELIA_BDAY_FOX_TWINS] = 1;
	helPartyMenu();
}
...
private helAndSluttyHarpyMale():void
{
	...
	else
	{
		...
		if (flags[FlagEnum.HELIA_ANAL_TRAINING] == 0) MainScreen.text(" tight");
		else if (flags[FlagEnum.HELIA_ANAL_TRAINING] == 1) MainScreen.text(" well-trained");
		else if (flags[FlagEnum.HELIA_ANAL_TRAINING] >= 2) MainScreen.text(" gaped");
