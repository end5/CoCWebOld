...
public function heliaFollowerIntro():void {
	...
	if(flags[FlagEnum.HARPY_QUEEN_EXECUTED] > 0) outputText("kill");
...
private function iCantLetFireButtsRapeMyCampsButt():void {
	...
	if(flags[FlagEnum.HEL_FOXY_FOURSOME_WARNED] > 0 || flags[FlagEnum.HEL_EDRYN_OFFER] > 0) outputText("like usual");
...
public function helFollowersIntro():void {
	...
	//If Isabella 
		...
		//If Isabella is at Camp (And she and Hel are NOT cool)
			...
			//(corruption = pussy)
			if(player.stats.cor < 50) outputText("fall silent under their considering gazes.");
...
private function helFollowerAppearance():void {
	...
	if(flags[FlagEnum.HEL_ISABELLA_THREESOME_ENABLED] >= 1) outputText("  Hel's blue bandanna is wrapped around her brow, mostly hidden beneath her fiery hair.");
...
private function centaurMountsCampHel():void {
	...
	//if no boobs: 
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 3) outputText(" but your body is too big, and you must content yourself to endure Hel's ministrations");
...
private function muddyLizardFeet():void {
	...
	if(player.lowerBody.balls > 0) outputText("her heel coming to caress your taut sack");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("her heel brushing against the nub of your [clit]");
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > heliaCapacity()) outputText("you feel your [cockHead biggest] against the lips of her cervix, as fully into her as you can go");
...
private function dudeHeliaAndKihaThreeSome():void {
	...
	//if 1 cock: 
	if(player.lowerBody.cockSpot.count() == 1) outputText("the rest of your shaft, her own tongue coiling around your tool");
...
private function heliaAnalTrainingPrompt():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("biggest ");
...
private function giveHeliaAnalTraining():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("selecting the biggest and heaviest to stroke as you near her");
	...
	if(player.lowerBody.balls > 0) outputText("She tilts forward to press her chin into your [sack] and begins to lather your [balls] with frothy, warm, salamander spit. One hand hefts a weighty nut as she works, caressing it as if it would somehow coax the cum out of it. ");
	...
	if(player.lowerBody.balls > 0) outputText(", hard enough to lift your nuts in their [sack]");
...
private function yesMakeHeliaAGapeSlut():void {
	...
	outputText("heft it up, pressing the [cockHead biggest] on Helia’s thirsty lips. She sandwitches the " + num2Text(Math.round(player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockLength)) + "-thick shaft in between her tits and left arm while her right tickles the sensitive underside. Her tongue slides around your urethra, collecting every single drop of pre-cum and stimulating nerve endings you didn’t even know you had. You groan and flex powerfully, preparing to dump a thick load into the hot salamander’s mouth. She spreads her lips around your cum-slit and kisses your cock hard, sucking powerfully, drawing the bubbling tide of wide straight out of your [balls] with her hungry movements.");
...
private function goWithHelia():void {
	...
	//If PC did UrtaQuest: 
	outputText("\n\n");
	if(flags[FlagEnum.URTA_MET_HEL] == 1) outputText("From the quick look she and the salamander exchange, you get the feeling they've met before. Weird. Still, ");
...
*/	
	...
	if(flags[FlagEnum.HARPY_QUEEN_EXECUTED] > 0) outputText("who forgave us for the queen's death");
	...
	if(flags[FlagEnum.HARPY_QUEEN_EXECUTED] > 0) outputText("finally killing that bitch of a queen");
	...
	if(flags[FlagEnum.FREED_VALA] == 1) outputText("a certain waitress fairy having... supplied the icing herself");
	...
	if(flags[FlagEnum.HEL_FOXY_FOURSOME_WARNED] == 1) outputText("a pair of lovely fox-eared herms");
...
private function heliaDrinks():void {
	...
	//if PC is male:
	if(player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) outputText(" just as you do");
...
private function heliaPhoenixes():void {
	...
	if(flags[FlagEnum.HARPY_QUEEN_EXECUTED] > 0) outputText("dead");
...
private function heliaAndFoxTwins():void {
	...
	if(flags[FlagEnum.HEL_FOXY_FOURSOME_WARNED] == 1) outputText("Mai");
...
private function helAndSluttyHarpyMale():void
{
	...
	if (player.lowerBody.cockSpot.count() == 1)
	{
		...
		if (player.lowerBody.vaginaSpot.count() > 0) outputText(" Your unused [vagina] cums with your cock in a show of sympathetic orgasm, coating your thighs with femcum.");
	}
	// 2-3 cocks
	else if (player.lowerBody.cockSpot.count() <= 3)
	{
		...
		outputText(".");
		if (player.lowerBody.cockSpot.count() == 3) outputText(" Your unused cock splatters the two with hot, white jizz.");
		if (player.lowerBody.vaginaSpot.count() > 0) outputText(" Your unused [vagina] cums with your cocks in a show of sympathetic orgasm, coating your thighs with femcum.");
	}
	// > 4 cocks
	else
	{
		...
		if (flags[FlagEnum.HELIA_ANAL_TRAINING] == 0) outputText(" tight");
		else if (flags[FlagEnum.HELIA_ANAL_TRAINING] == 1) outputText(" well-trained");
		else if (flags[FlagEnum.HELIA_ANAL_TRAINING] >= 2) outputText(" gaped");
		...
		if (player.lowerBody.cockSpot.count() > 4) outputText(" Your extra cocks spray pearly cum all over the nearby ground.");
		if (player.lowerBody.vaginaSpot.count() > 0) outputText(" Your unused [vagina] cums with your cocks in sympathetic orgasm, coating your thighs with femcum.");
	}
	// Merge
