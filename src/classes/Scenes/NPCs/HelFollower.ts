...
public heliaFollowerIntro():void {
	...
	if(flags[FlagEnum.HARPY_QUEEN_EXECUTED] > 0) Render.text("kill");
	else Render.text("bring down");
	Render.text(" the Harpy Queen... and getting my sister out of that shithole.  For giving me a chance to meet my dad.  For everything.</i>\"  You start to tell her that you're happy to help, but Hel cuts you off, speaking quickly: <i>\"Kiri, Dad, and I have been trying to make it work, living out on the plains - we really have - but Dad's not in any shape to fight and Kiri's no good at it; the plains are just too dangerous for them and Dad's been having nightmares about mom, and...\"</i> she pauses to take a gasping breath, unaccustomed to talking so quickly.");
...
private iCantLetFireButtsRapeMyCampsButt():void {
	...
	if(flags[FlagEnum.HEL_FOXY_FOURSOME_WARNED] > 0 || flags[FlagEnum.HEL_EDRYN_OFFER] > 0) Render.text("like usual");
	Render.text(", and maybe you and I can spend some quality time together.  Alright?\"</i>");
...
private justFriendsWithAnalTailWaifu():void {
	clearOutput();
	Render.text("Awkwardly, you spend the next few minutes saying that, while you appreciate the offer and her affections, you're more interested in just being friends with Hel, as you have been for some time.");
	...
	Render.text(" to give you a little peck on the cheek.  <i>\"Well, that's that, then.  I guess.  Well, see you around, [name].\"</i>");
...
public helFollowersIntro():void {
	...
	if(followerKiha() && flags[FlagEnum.HEL_INTROS_LEVEL] < 1) {
		...
		Render.text("You pace around camp, awaiting the return of your new companion.  After several minutes, you notice Kiha sitting behind you, eyeing you with an eyebrow cocked.  Suddenly self-conscious, you stop and face the dragon-girl.");
	...
	//If Isabella 
		...
		if(flags[FlagEnum.HEL_ISABELLA_THREESOME_ENABLED] > 0) {
			Render.text("Showing Hel around, you eventually come to the part of your camp inhabited by the towering cowgirl Isabella.  When you arrive, you find Isabella reclining in her armchair, humming a sweet melody as she cleans her tower shield.");
		...
		//If Isabella is at Camp (And she and Hel are NOT cool)
			...
			//(corruption = pussy)
			if(player.stats.cor < 50) Render.text("fall silent under their considering gazes.");
			//(corruption = high enough to call bitches out when they're hypocrites)
			else Render.text("glower balefully at them, as if to challenge either to invite your opinion of their own sexual 'résumé' with an ill-considered comment.");
			...
			if(isabellaAccent()) Render.text("<i>\"You are... velcome,\"</i> Isabella says before collecting some of the scattered belongings from the ground.  You continue your tour, now that the girls are...  not going to murder each other in the middle of the night, at least.");
			else Render.text("<i>\"You're... welcome,\"</i> Isabella says before collecting some of the scattered belongings from the ground.  You continue your tour, now that the girls are...  not going to murder each other in the middle of the night, at least.");
	...
	//If Bimbo Sophie is at Camp:
		...
		Render.text("As you help Hel string up her hammock between a few of the rocks inside your perimeter, you hear the tell-tale flapping of useless wings and a clattering of claws on the hard-packed dirt.  You brace for impact as your bimbo harpy prances up to you, planting a big, wet kiss on your cheek");
		...
		if(player.stats.int >= 40 || player.stats.int/5 + rand(20) + 1 > 10) {
			Render.text("Or, maybe there's a way to make this work...");
...
private bimboSophieGetsBooted4Firebutt():void {
	clearOutput();
	Render.text("You sigh and pick Sophie up, slinging the harpy over your shoulders.  <i>\"Yaaaay~\"</i> she cheers.  </i>\"I'm going for a ride!\"</i>");
...
private bootHelOutForBimboSophie():void {
	clearOutput();
	Render.text("<i>\"Nope, you get out,\"</i> you answer, scowling at the salamander.");
...
private workItOutWithSophieAndFireTits():void {
	clearOutput();
	Render.text("You cross your arms and tell Hel to deal with it, remarking that you won't just dump Sophie out in the wilderness - she can't take care of herself - but you still want Hel around.");
...
public angryHelAndIzzyCampHelHereFirst():void {
	clearOutput();
	Render.text("Showing Isabella around, you eventually come to the chaotic, cluttered part of camp inhabited by Hel the salamander, who's currently sitting on her hammock sharpening her scimitar.");
	Render.text("\n\nYou only have a moment to remember Hel's disdain for the cowgirl before...");
	...
	if(isabellaAccent()) Render.text("V");
	else Render.text("W");
	Render.text("hat are YOU doing here?!\"</i>");
...
public heliaFollowerMenu(display:boolean = true):void {
	...
	if(flags[FlagEnum.HEL_FOLLOWER_LEVEL] == 2) {
		...
		if(display) Render.text("You call your salamander lover over, and in a few moments Hel walks your way, hips and tail swaying gaily as she moves.  She wraps an arm around your shoulders, pressing her soft, warm body against yours, and grins.  <i>\"Heyya, lover mine.  You need anything?\"</i>");
		menu();
		...
		if (flags[FlagEnum.HEL_LOVE] == 1 || flags[FlagEnum.HEL_LOVE] == -1) {
			if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(heliaCapacity()) >= 0 && player.lust >= 33 &&
					!helPregnant() && flags[FlagEnum.HELSPAWN_AGE] == 0) addButton(7,"Have A Kid",helSpawnScene.haveAKid);
		}
		...
		if(flags[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] == 1) {
			if(display) Render.text("(You've reached the culmination of Helia's current storyline.  Stay tuned for more!)");
		}
		else {
			if(display) Render.text("You approach Hel as she's pacing around camp.  She's clad in her normal field attire: a simple scale bikini top and leather thong which supports her scimitar's scabbard.  Her cloak is loosely thrown over her shoulders, giving her a slight measure of protection from the mountain's harsh environs.");
...
private heliaOptions():void {
	...
	if(flags[FlagEnum.HELIA_ANAL_TRAINING_OFFERED] > 0 && flags[FlagEnum.HELIA_ANAL_TRAINING] < 2 && player.lowerBody.cockSpot.biggestCocks[0].cockArea() > heliaAnalCapacity() && player.hasItem(consumables.GOB_ALE,1)) addButton(3,"Anal Train",heliaGapeSceneChoices);
	addButton(5,"Bathe",takeABath);
	if(flags[FlagEnum.HELSPAWN_AGE] == 1) addButton(7,flags[FlagEnum.HELSPAWN_NAME],helSpawnScene.playWithYourKid);
	if(flags[FlagEnum.HEL_GUARDING] == 0) addButton(8,"GuardCamp",helGuardToggle);
...
private sparWithHeliaFirebuttsAreHot():void {
	clearOutput();
	Render.text("Giving Hel a playful punch on the shoulder, you ask the salamander-girl if she'd be up for a little battle practice.");
...
internal function loseToSparringHeliaLikeAButtRapedChump():void {
	clearOutput();
	//If HP loss) 
	if(player.HP < 1) Render.text("You collapse on the ground, overwhelmed by pain and exhaustion caused by the berserker's onslaught.");
	else Render.text("Your arousal is too great, and your mind can no longer focus on anything but a desperate need for release.  Lust robs you of your will, and with buckling knees you collapse.");
...
internal function PCBeatsUpSalamanderSparring():void {
	clearOutput();
	//(If HP loss) 
	if(monster.HP < 1) Render.text("Unable to withstand your onslaught, the salamander collapses to a knee, barely supporting her weight on her sword.");
	else Render.text("Panting heavily, knees snaking, she collapses, heavily leaning upon her sword.");
	
	Render.text("\n\n<i>\"Come on, [name],\"</i> Hel groans, swaying unsteadily.  ");
	if(monster.HP < 1) Render.text("<i>\"Kick my ass a little harder, why don't you?\"</i>");
	else Render.text("<i>\"Turn me on till I can't think straight, why don't you?\"</i>");
	Render.text("  You chuckle and offer her a hand up. The blushing Salamander takes it as you pull her into a tight embrace.  The cool wetness between her thighs a potent reminder of how much your lover seems to get off on violence.  Since she's so turned on, you could easily turn this into some rough loving.");
...
private talkToHel():void {
	...
	if(flags[FlagEnum.FOLLOWER_HEL_TALKS] == 0) {
		Render.text("You run a hand through Hel's hair and ask the recent addition to your camp if she'd like to talk.");
		...
		//(If Toughness <50: 
		if(player.tou < 50) Render.text("You cough and gag, eyes misting as the pure-grain booze burns your throat worse than hot coals.  Hel laughs riotously, slapping you on the back hard enough to cause booze to snort out through your nose.");
		else  Render.text("You knock back the stein easily, your super-human endurance keeping you from choking on the powerful brew.  Impressed, Hel gives you a high-five and throws her head back in a laugh.");
	...
	//Hel Talk 2
		...
		Render.text("Sitting Hel down, you ask if she's got any stories she'd like to share.  After all, she's been adventuring for years - surely she's got some fun tales to tell.");
	...
	//Hel Talk 5(C)
		...
		Render.text("<i>\"Hey, [name],\"</i> Hel says, slipping out of your grasp with a come-hither wag of her finger, <i>\"wanna have a drink with me?\"</i>");
		Render.text("\n\n<i>\"Sure,\"</i> you say, following Hel over to her still.");
		
		Render.text("\n\nShe pours out two tankards of ");
		if(helSpawnScene.helPregnant()) Render.text("beer ");
		else Render.text("fizzy, non-alcoholic beer ");
		Render.text("and, swinging onto your lap, clinks her glass against yours.  <i>\"To loving friends and an awesome new home,\"</i> she says, raising her tankard high before knocking it back.  Before you can finish yours, Hel belches loudly and violently shakes her head");
		if(!helSpawnScene.helPregnant()) Render.text(", her eyes crossing a little.  Looks like she was dipping in before you got here...");
		else Render.text(".");
		
		Render.text("\n\nHel laughs ");
		if(!helSpawnScene.helPregnant()) Render.text("drunkenly");
		else Render.text("happily");
		if(!helSpawnScene.helPregnant())
		{ 
			Render.text(", filling her cup up again and, with a wide smile, jumping out of your lap.  <i>\"Hey, lover, check this out!\"</i> she shouts, grabbing her fiery tail in one hand and swirling a mouth-full of alcohol.  You have just enough time to take cover before Hel swings her tail around and spits a stream of pure-grain over it, resulting in a great gout of flame that streaks into the heavens.");
			//if Kiha is in camp: 
			if(kihaFollower.followerKiha()) Render.text("  From across camp, you hear a certain dusky dragoness shout <i>\"HEY! That's MY trick, firebutt!\"</i>");
			...
			if(kihaFollower.followerKiha()) Render.text("further pissing off Kiha, who throws a rock at Hel, and ");
			Render.text("shooting up like a beacon over the wasteland.");
		}
		else {
			Render.text(" and launches into a series of debauched jokes. You answer her, lewd jest for jest.");
		}
		Render.text("\n\n<i>\"Hehehe.  Nice one, lover,\"</i> Hel laughs, pulling you ");
		if(player.tallness < 60) Render.text("up ");
		else Render.text("down ");
		Render.text("into a hug, mashing your face into her warm, soft bosom.  You struggle for air, finally bursting out of her tight cleavage.  Hel chuckles, running her hand through your [hair] as her fiery tail wags gaily behind her.");
	...
	//Hel Talk 6 (Needs Isabella and Kiha at camp; at least 1 gem)(C)
		...
		let gems: number = 0;
		Render.text("<i>\"Hey, [name],\"</i> Hel says with a sly grin.  <i>\"Me, Izzy, and spitfire were just playing a little game.  Wanna deal in?\"</i>");
		...
		if(!isabellaAccent()) Render.text(", obviously only somewhat interested");
		Render.text(".");
...
private shutUpHelTalks():void {
	clearOutput();
	Render.text("Quickly you interject before Helia can continue with the story; you've heard quite enough.  <i>\"Whaaaat?  We were just getting to the good part...\"</i>  Hel whines, rolling her eyes.  <i>\"Hey, you're the one who asked!\"</i>");
...
private hugASmokeyTail():void {
	...
	if(model.time.hours >= 21) {
		Render.text("As the sun sets over the camp, you see Helia standing over her hammock, stretching and yawning, ready to turn in for the night.  You approach her, sliding your arms around her supple waist and burying your face in her soft crimson locks, holding your lover close.  Hel giggles girlishly as you give her a long hug, nuzzling into the nape of her neck.");
		...
		if(player.tailType > TAIL.NONE) Render.text("s");
		Render.text(" intertwined.  <i>\"Oh, this is nice,\"</i> Hel laughs, running her long, scaled fingers through your hair.  <i>\"So, you wanna stay with me tonight, lover mine?  I'd appreciate the company....\"</i>");
...
private heliaRoughSex(output:boolean = true):void {
	if(output) {
		clearOutput();
		Render.text("<i>\"Mmm, need to blow off some steam, lover?\"</i>  Hel grins, reaching around to undo the straps of her scale bikini.  <i>\"Well, I'm all for that!\"</i>");
	}
...
private heliaCampPossession():void {
	clearOutput();
	Render.text("With a coy grin, you let your ghostly attributes go transparent, and ask Hel if she'd mind taking an extra passenger for a little bit.");
...
private inCampHelNagaLuv():void {
	...
	Render.text(" well into her womb.  Your tail spasms as you cum, running wild inside Hel's rectum until she cums too, yelping into your open mouth as she climaxes a moment after you, girlcum squirting out of her twat until your [hips] are thoroughly soaked.");
...
private centaurMountsCampHel():void {
	...
	//if no boobs: 
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 3) Render.text(" but your body is too big, and you must content yourself to endure Hel's ministrations");
	else Render.text(" but all you can do is take hold of your [chest] and tease and tweak your [nipples], getting what pleasure you can from your sensitive teats");
	Render.text(".");
	...
	if(player.cockArea(x) > heliaCapacity()) Render.text("she can cram in, the rest of your length twitching between her legs");
	else Render.text("you have to offer her");
	Render.text(".  Finally, though, you manage to settle down, your breath coming ragged as Hel's vaginal muscles clamp and release, gently massaging your shaft with expert movements, always keeping you on edge until...");
...
private heliaFollowerTentafuck():void {
	clearOutput();
	Render.text("With a flourish you pull your [armor] off, exposing your writhing mass of tentacle-cocks for Hel`s enjoyment.  \"<i>Wow, think you got enough of th--urf!</i>\" the salamander starts to say before you shut her up with one of your tentacle pricks, lashing forward and plugging her mouth.  Hel staggers back, flailing around before grabbing the cock you've shoved down her throat and giving it a rough tug.  Just as she nearly succeeds in pulling it out, your other wriggling members wrap around her thick thighs; you pull her in toward you, grabbing her shoulders as your pricks peel off her bikini bottom to reveal the smooth face of Helia's crotch.");
	...
	if(player.lowerBody.balls > 0) Render.text("[balls]");
	else Render.text("wriggling tentacles");
	Render.text(" feel painfully empty and white slush pours from all her holes, painting the ground white beneath her dangling feet.");
...
private muddyLizardFeet():void {
	...
	if(player.lowerBody.balls > 0) Render.text("her heel coming to caress your taut sack");
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("her heel brushing against the nub of your [clit]");
	else Render.text("her heel going so low as to brush your taint, making your [asshole] clench in surprise before she moves back up your length");
	Render.text(".  You groan lustily as her toes and soft sole jerk you off, gently urging the tight knot of pleasure in your gut to fruition.  Cupping one of her hefty tits, Hel licks her lips as the first thick smears of pre leak from your cock's [cockHead biggest], smearing onto her long-clawed toes.");
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > heliaCapacity()) Render.text("you feel your [cockHead biggest] against the lips of her cervix, as fully into her as you can go");
	else Render.text("your groin meets Helia's, your [cock] wholly enveloped within her");
	Render.text(".  Before you can think to withdraw, Hel tightens her hold on you, whispering huskily, \"<i>Just leave it... just for a second, [name].  Oh, God,</i>\" she moans, her eyes closing as she savors the feeling of you buried so deep inside her, your slick prick twitching as her walls massage it, milking it of the precum that freely flows from your tip.");
...
private takeABath():void {
	...
	//{Repeat Intro (If you taught Hel to swin; else, play 1st Time)}
		...
		clearOutput();
		Render.text("You ruffle Hel's hair and ask if the fiery salamander's up for a dip.");
...
private teachHelToSwim():void {
	clearOutput();
	Render.text("Putting a hand on Hel's shoulder, you offer to teach her how to swim.");
...
public heliaThreesomes():void {
	clearOutput();
	Render.text("You slip an arm around Hel's waist and ask if she's up for some... group activities?");
...
private heliaAndVapula():void {
	...
	Render.text("You gab Hel and start for the edge of camp, where a certain succubus dwells....");
	...
	if(player.cumQ() >= 750) Render.text(" until her belly swells and starts spurting ropes of jism back out of her love-tunnel");
	Render.text(".  Your junk palpitates and bulges as globs push their way out of your shaft before being absorbed by her needy pussy, and her skin and scales seem to literally boil with consumed lust.  At the peak of your orgasm, you stare at your girls' faces: Hel's is nothing but complete, insensate bliss, while Vapula is literally crying in ecstasy, tears of joy streaming down her cheeks.");
	...
	if(!followerHel()) {
		Render.text("  You kiss her and ask if she'll be able to find her way back to her home.");
...
private helXValeriaFemalePC():void {
	clearOutput();
	Render.text("In the blink of an eye, Valeria slithers around Hel's body, almost completely encasing her in thick blue goo, leaving only her head and her resplendent reptilian rack bare.  Hel grins at you, putting a hand to her crotch and taking a handful of goo between her fingers.  With a bit of effort, she shapes a pair of thick blue pricks out of her groin, each shimmering in the sunlight as Hel strokes Valeria's new appendages, coaxing out a drop of slimey pre from both tips.  Valeria's face reappears on Hel's flat belly a moment after she's done, looking down to her groin with glee as she further shapes the erect pillars of slime, morphing them into a thick knotty doggy-dick on top, and a huge-flared horsecock on bottom.  You lick your lips as the dog-dick's knot inflates enticingly.");
...
private helGuardToggle():void {
	...
	if(flags[FlagEnum.HEL_GUARDING] == 0) {
		Render.text("You ask Hel if she wouldn't mind watching the perimeter for a few hours during the night.");
	...
	//[If Hel IS guarding Camp]
		...
		Render.text("You tell Hel she doesn't need to keep watch at night anymore.");
...
private heliaAnalTrainingPrompt():void {
	...
	Render.text("“<i>Hey there, lover mine. Got a minute?</i>” Helia asks with dangerous glint in her eyes.");
...
private giveHeliaAnalTraining():void {
	clearOutput();
	Render.text("You hand Helia a bottle of goblin ale and question if she’s sure this won’t make her loose.");
	...
	if(player.lowerBody.balls > 0) Render.text("She tilts forward to press her chin into your [sack] and begins to lather your [balls] with frothy, warm, salamander spit. One hand hefts a weighty nut as she works, caressing it as if it would somehow coax the cum out of it. ");
	Render.text("Helia slides her palms along either side of your urethra, letting her fingers handle the central, sensitive bit on the underside, following in the path of slick saliva she left for them to follow. She rubs her nose into the sensitive area just below your [cockHead biggest]. You grunt as pre-cum begins to pour in response, your bloated boner flexing atop Helia’s head as you feel a dangerously powerful orgasm welling up within you.");
	...
	if(player.lowerBody.balls > 0) Render.text(", hard enough to lift your nuts in their [sack]");
	Render.text(", and you erupt, cumming a steamy hot load of spunk straight onto Helia’s face, head, and back.");
	if(player.cumQ() < 250) Render.text(" You run out before long, but it’s still enough to make her a white painted mess. She licks the excess from her upper lip and shudders happily.");
	else if(player.cumQ() < 2000) Render.text(" You spurt rope after rope onto her, actually drenching her hair with alabaster moisture. Her back and face look equally smothered with spooge when you finish. She’s happy to wipe the goop out of her eyes and shovel it into her mouth, swallowing it down with a satisfied smile.");
	else {
		Render.text(" You pour ivory waves of spooge across her body, soaking her from head to toe with the fruit of your loins, liberally basting her with baby-batter. She groans as the excess washes out around her knees");
		if(player.cumQ() >= 3000) Render.text(", but you keep cumming, cascading fresh waves onto her until there’s not a single part of her that isn’t stained and dripping with your salty outflow");
		Render.text(". Helia licks her hands clean, wipes her eyes and devours that as well, and then gives you a dripping-wet, sensuous smile.");
	}
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() < helAnalCapacity()) {
		Render.text("then maybe I can handle you, my hung " + player.mf("stud","breeder") + ".");
	}
	else {
		Render.text("then maybe we can see about trying something bigger.");
	}
	Render.text("</i>”");
...
private heliaAnalTrainingPartTwo():void {
	player.consumeItem(consumables.GOB_ALE,1);
	clearOutput();
	Render.text("Pulling out another bottle of ale you dangle it before the slutty salamander, saying, “<i>A present for my favorite buttslut.</i>”");
...
private noGapeSlutForHelia():void {
	clearOutput();
	Render.text("You step up and kiss Helia, telling her that she doesn’t have to this for you, at least not right now.");
...
private yesMakeHeliaAGapeSlut():void {
	...
	if(player.str < 50) Render.text("strain to ");
	Render.text("heft it up, pressing the [cockHead biggest] on Helia’s thirsty lips. She sandwitches the " + num2Text(Math.round(player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockLength)) + "-thick shaft in between her tits and left arm while her right tickles the sensitive underside. Her tongue slides around your urethra, collecting every single drop of pre-cum and stimulating nerve endings you didn’t even know you had. You groan and flex powerfully, preparing to dump a thick load into the hot salamander’s mouth. She spreads her lips around your cum-slit and kisses your cock hard, sucking powerfully, drawing the bubbling tide of wide straight out of your [balls] with her hungry movements.");
	...
	if(player.cumQ() < 1000) Render.text(", ignoring the way your sheer volume of spunk leaks from the corners of her mouth and nose, her belly gradually rounding with cum in addition to thick dildo");
	Render.text(". The slutty salamander eventually pulls it off and ");
	if(player.cumQ() < 2000) Render.text("squeezes a last few dollops onto her face, letting them stain her with your scent.");
	else Render.text("allows you to hose her down with your ongoing waves of salty jism, soaking her in your sexual scent.");
	Render.text(" Helia burps. “<i>Fuck me that was awesome! Whoah, don’t mind me, but I, uh... I think I’m gonna stay here for a little while, maybe just rock up and down till I come down off this buzz. She blushes and begins to finger her snatch again. Just let me... just let me get used to.</i>” She shoos you away and goes back to toying with herself, getting used to her new “flexibility”.");
...
private stayHomeFromHeliaParty():void {
	clearOutput();
	Render.text("\"<i>Aww, lame,</i>\" Hel groans, sighing.  \"<i>Fiiiiiine.  I guess I'll just go have awesome fun and party hard without you. Don't stick too hard into that mud, lover,</i>\" she chuckles, giving you a reassuringly affectionate kiss on the cheek before trucking off.");
	doNext(playerMenu);
}
...
private helPartyMenu():void {
	menu();
	//heliaDrinks
	if(flags[FlagEnum.HELIA_BDAY_DRINKS] == 0) addButton(0,"Drinks",heliaDrinks);
	//heliaHakonAndKiri
	if(flags[FlagEnum.HELIA_BDAY_HAKON_AND_KIRI] == 0) addButton(1,"Hakon 'n Kiri",heliaHakonAndKiri);
	//heliaPhoenixes
	if(flags[FlagEnum.HELIA_BDAY_PHOENIXES] == 0) addButton(2,"Phoenixes",heliaPhoenixes);
	//heliaAndFoxTwins
	if(flags[FlagEnum.HELIA_BDAY_FOX_TWINS] == 0) addButton(3,"Fox Twins",heliaAndFoxTwins);
	//leaveWithoutSex
	addButton(4,"Leave",leaveWithoutSex);
	//leaveWithGirls
	if(flags[FlagEnum.HELIA_BDAY_FOX_TWINS] > 0) addButton(5,"Leave w/Girls",leaveWithGirls);
}
...
private goWithHelia():void {
	clearOutput();
	Render.text("\"<i>Sounds like fun.  Let's go,</i>\" you say, offering your arm to Hel.  She grins and takes it, holding herself close as she takes the lead, steady paces quickly drawing you out into the vast deserts of Mareth beside your lover.");
	//{If Kiha, Isabella, and/or Valeria are at camp:}
	if(isabellaFollower() || followerKiha() || flags[FlagEnum.VALARIA_AT_CAMP] == 1) {
		Render.text("  Looking over your shoulder as you depart, though, you notice that camp looks particularly... empty. Perhaps your other camp-mate");
		if(camp.companionsCount() > 1) Render.text("s are");
		else Render.text(" is");
		Render.text(" out, too? You shrug. You and Hel aren't the only ones with lives, you suppose.");
	}
	...
	//if less than 7' PC: 
	if(player.tallness < 84) Render.text("sweeping you off your feet");
	else Render.text("hanging off you");
	Render.text(" as you approach the great gates.  The only one on gate duty today is Urta, the grey-coated fox curled up under a small awning beside the gate with a huge canteen resting beside you.  Her centauress partner is noticeably missing, but Urta still gives you a wave and a grin you can't quite read.");
	
	Render.text("\n\n\"<i>Hey, cutie,</i>\" she says with a wink, pushing open the door for you.");
	//If PC did UrtaQuest: 
	Render.text("\n\n");
	if(flags[FlagEnum.URTA_MET_HEL] == 1) Render.text("From the quick look she and the salamander exchange, you get the feeling they've met before. Weird. Still, ");
	Render.text("Urta gives you a small nod as Hel leads you into town, past the half-empty guard post and into the bustling main street.  The market way's particularly busy in the late afternoon, bustling with activity as merchants hawk their wares and the first of the street-walkers start to come out, standing in seductive poses in the shade of the alleys.  More than a few send catcalls your way, a few even offering discounts for couples.");
	...
	if(player.tallness <= 72) Render.text(", her head resting on your shoulder");
	Render.text(".  \"<i>Hey, thanks for coming, [name],</i>\" she says quietly, squeezing your arm, \"<i>Kiri's done nothing but talk about you since the tower.  I think someone has a crush...</i>\"");
	...
	Render.text(formatStringArray(list) + ". You turn to Helia and cock a grin as you see the surprise etched on her face, followed by the growing smile.  \"<i>Happy birthday, Helia!</i>\" they all shout out at once, raising mugs frothing over with beer.");
/*
	addToList("Edryn");
	...
	Render.text(outputList() + ". You turn to Helia and cock a grin as you see the surprise etched on her face, followed by the growing smile.  \"<i>Happy birthday, Helia!</i>\" they all shout out at once, raising mugs frothing over with beer.");
*/	
	...
	if(flags[FlagEnum.HARPY_QUEEN_EXECUTED] > 0) Render.text("who forgave us for the queen's death");
	else Render.text("who wanted to be friends still, even after the Tower");
	Render.text(" and... it sort of became a thing.  Hope you don't mind!</i>\"");
	...
	if(flags[FlagEnum.HARPY_QUEEN_EXECUTED] > 0) Render.text("finally killing that bitch of a queen");
	else Render.text("setting the queen right, but leaving her alive for when it's finally time to bring the fight to Lethice");
	Render.text(".  You nod at his thanks, and his hand is soon replaced by Helia's as she grabs you on her way toward the gigantic cake.  The towering white confectionary seems more than big enough to feed everyone in the group, a huge extravagance on the part of her family.  You hear a quiet whisper about ");
	if(flags[FlagEnum.FREED_VALA] == 1) Render.text("a certain waitress fairy having... supplied the icing herself");
	else Render.text("a certain minotaur chef having supplied the icing himself");
	Render.text(" as Hel is handed an oversized knife and urged to blow out the candles atop it, arranged into a perfect 23.");
	...
	if(flags[FlagEnum.HEL_FOXY_FOURSOME_WARNED] == 1) Render.text("a pair of lovely fox-eared herms");
	else Render.text("Miko and Mai");
	Render.text(" burst out, their big breasts covered in nothing but tiny pink pasties and an all-too-small thong sheathing their red pricks.  Arm in arm with tail wagging furiously, they cry out \"<i>Happy birthday, Hel!</i>\" before hopping out and slipping into the salamander's arms.  \"<i>Ladies,</i>\" Hel says, grinning like an idiot, a moment before she's pushed into a waiting chair behind her.  One of the girl's busts is quickly shoved into her face as the rest of the group laughs and starts passing out another round of drinks.");
...
private heliaHakonAndKiri():void {
	...
	Render.text(".  We salamanders are a bit more open about that sort of thing, though, if you haven't noticed.  Helia's regaled us with more than a few of her 'stories,' let me tell you. Isn't that right, sweetheart?</i>\"");
...
private heliaPhoenixes():void {
	clearOutput();
	Render.text("You approach the small squad of phoenixes milling around the cake, mostly talking amongst themselves.  They nod as you approach, a bit wary of the person who recently kicked their asses inside out.  Still, they nod respectfully as you approach.");
	...
	if(flags[FlagEnum.HARPY_QUEEN_EXECUTED] > 0) Render.text("dead");
	else Render.text("deposed");
	Render.text(", he thought it would be good to try and at least... know each other.  I'm not complaining.  Things like this party... never would have seen anything like this under Queen Calais.  It's nice to see new things, meet new people.</i>\"");
	
	Render.text("\n\nYou see a few glances going toward Edryn and the fox twins");
	if(followerKiha() && isabellaFollower()) Render.text(", then to the fiery dragoness and the ultra-busty cowgirl talking in the corner");
	Render.text(", and you figure you know what they mean.");
...
private heliaAndFoxTwins():void {
	clearOutput();
	Render.text("You approach the foxy strippers, still well into giving Helia a dual lap-dance, their red puppy peckers poking well out of their tiny thongs.  They grin as you approach, and Hel pulls you into her lap, and you quickly find yourself with a pair of big, soft breasts to rest your head on, courtesy of ");
	if(flags[FlagEnum.HEL_FOXY_FOURSOME_WARNED] == 1) Render.text("Mai");
	else Render.text("one of the foxes");
	Render.text(".  Hel plants a deep, serpent-tongue filled kiss on your lips, nuzzling her cheek on your [chest].");
...
private leaveWithoutSex():void {
	clearOutput();
	Render.text("You decide you've had about enough of the party for now. You slip over to Hel and tell her you're ready to head out when she is.");
...
private helAndSluttyHarpyMale():void
{
	...
	Render.text("You take Helia’s hand and lead her through the camp in an attempt to find someone to include in your threesome. After some searching you see Sophie sitting on a rock and get a wonderful idea. You ask Hel if she would be ok with a threesome with the busty harpy. Her smile turns to a serious frown for a moment.");
	...
	if (player.biggestCockLength() >= 8) Render.text(" no longer be hilted inside");
	else Render.text(" slip out of");
	Render.text(" her sodden cunt, and hear Hel gasp for air. Before the birdslut can comment on the matter, you spin her around and push her down onto Helia, leaving them face to face.");
	...
	if (player.lowerBody.cockSpot.count() == 1)
	{
		...
		Render.text(" to boil over. White jissom blasts from your [cock], coating Helia and Sophie’s stomachs, as well as the underside of the pair’s tits.");
		if (player.lowerBody.vaginaSpot.count() > 0) Render.text(" Your unused [vagina] cums with your cock in a show of sympathetic orgasm, coating your thighs with femcum.");
	}
	// 2-3 cocks
	else if (player.lowerBody.cockSpot.count() <= 3)
	{
		...
		if (player.cumQ() < 500) Render.text(" with your cum");
		else
		{
			Render.text(" full of cum");
		 	if (player.cumQ() <= 1000)
		 	{
		 		Render.text(" until it overflows");
		 		if (player.cumQ() >= 5000) Render.text(", pooling around your knees");
		 	}
		}
		Render.text(".");
		if (player.lowerBody.cockSpot.count() == 3) Render.text(" Your unused cock splatters the two with hot, white jizz.");
		if (player.lowerBody.vaginaSpot.count() > 0) Render.text(" Your unused [vagina] cums with your cocks in a show of sympathetic orgasm, coating your thighs with femcum.");
	}
	// > 4 cocks
	else
	{
		...
		if (flags[FlagEnum.HELIA_ANAL_TRAINING] == 0) Render.text(" tight");
		else if (flags[FlagEnum.HELIA_ANAL_TRAINING] == 1) Render.text(" well-trained");
		else if (flags[FlagEnum.HELIA_ANAL_TRAINING] >= 2) Render.text(" gaped");
		Render.text(" asshole. She gasps at the sudden double penetration, and you reciprocate because of the furnace-like heat of both her holes. Your [cock biggest3] lines up with Sophie’s slutty pussy and drives home while your [cock smallest] does the same to her ass."); // Haha look at me being a lazy shit.
		...
		Render.text(". The two girls seem to share your problem, their teeth clenched and clinging tightly to each other trying to resist. Seeing their clearly shared pleasure in the moment puts you over the edge, and you let loose the torrent of liquid lust from [eachCock]. Sophie cums as the tsunami of jizz floods her levies, the copious amount spilling back out of her holes and pooling around your knees.");
		if (player.lowerBody.cockSpot.count() > 4) Render.text(" Your extra cocks spray pearly cum all over the nearby ground.");
		if (player.lowerBody.vaginaSpot.count() > 0) Render.text(" Your unused [vagina] cums with your cocks in sympathetic orgasm, coating your thighs with femcum.");
	}
	// Merge
...
private helAndSluttyHarpyFemale():void
{
	...
	if (player.lowerBody.cockSpot.count() >= 1)
	{
	Render.text("The blasts spatter against the underside of Sophie’s gigantic melons and slide off Hel’s sides like an overfull cream pastry.");
	}
...
Render.text("You walk up to Helia and ask if she would be up for fucking you with that wondrous tail of hers. She smiles slyly and wraps her arms around you. You take her response as an unspoken yes, hugging her back and pulling her off to find a better place for your loving.");
