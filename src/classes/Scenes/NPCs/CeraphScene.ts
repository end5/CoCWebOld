...
//Ceraph's fluids boost PC libido and sensitivity massively either way.
		internal function maleFuckCeraphsPussy():void
		{
			...
			if (player.cockArea(0) <= monster.vaginalCapacity()) {
				...
				if (player.stats.cor < 33) outputText("Her words shame you.  Have you really sunk this low?  You wonder just how much this world has already corrupted you as you ready yourself to rape a sex-demon.", false);
				else if (player.stats.cor < 66) outputText("Her words confuse you.  You never would've done this when you first came here, but you're horny and you know demons don't really mind.", false);
				...
				if (player.stats.cor < 33) outputText("You groan, just wishing she would stop reminding you of the consequences of this little tryst.  In a hurry to leave the demon behind, you get dressed and take off for camp.\n\n", false);
				else if (player.stats.cor < 66) outputText("You groan, wondering why she just won't get over her little act.  Even though this wasn't your best idea, it isn't nearly as bad as she makes it out to be.  You get dressed and leave, eager to be rid of her.\n\n", false);
				...
				if (player.lowerBody.cockSpot.count() > 1) outputText("primary ", false);
				...
				if (player.lowerBody.cockSpot.list[0].cockLength < 18) outputText("Your " + player.cockHead() + " crests between her breasts with every stroke, bumping against her chin.  It grows larger as the pleasure of the act intensifies to orgasmic levels.\n\n", false);
				else if (player.lowerBody.cockSpot.list[0].cockLength < 24) outputText("Your " + player.cockHead() + " slides over her cheek with every stroke, leaving a trail of pre-cum on her face.  The smudges of glistening pre grow larger and larger as the pleasure of the act nears orgasmic levels.\n\n", false);
				...
				if (player.lowerBody.cockSpot.list[0].cockLength < 18) outputText("tits", false);
				else if (player.lowerBody.cockSpot.list[0].cockLength < 24) outputText("face", false);
...
//[Female] Ride 'Dat Cawk
		internal function rideCeraphsCockLikeaBAWSSexclamation11eleven():void
		{
			...
			if (player.stats.cor < 33) outputText("Her words shame you.  Have you really sunk this low?  You wonder just how much this world has already corrupted you as you ready yourself to get off on a sex-demon's tool.", false);
			else if (player.stats.cor < 66) outputText("Her words confuse you.  You never would've done this when you first came here, but you're horny and you know demons don't really mind.", false);
			else outputText("Her dishonest words anger you, and you slap her in the face.  Both of you know that she wants this almost as much as she wanted to rape you, and any pretense at disliking this act is a deceitful sham.", false);

			outputText("\n\nIt's too late for her to have any hope of swaying you anyway.  You strip off your " + player.armorName + " ", false);

			if (flags[FlagEnum.PC_FETISH] >= 1) outputText("getting an exhibitionist thrill ", false);
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("nipples", false);
...
//[OH SHIT SON MOAR SHIT GETTING PIERCEDEDEDED] – NIPPLEZ GO
		private function loseToCeraphAnGetPierced2():void
		{
			...
			if (player.upperBody.chest.hasFuckableNipples()) outputText("slut", false);
...
//[OH SHIT!  WHY DOES FEN KEEP WIRTING THAT 3RD TIME – CROTCH GO]
		private function getPiercedByCeraphLoss3():void
		{
			...
			if (player.totalCocks() > 0) {
				...
				if (player.lowerBody.cockSpot.list[0].pierced > 0) outputText(", the old piercing clattering to the ground as it slides out of your flesh, ", false);
...
//[fight]
		private function finalCeraphEncounterStartFight():void
		{
			...
			if (player.stats.cor <= 50) outputText("you'll stop her devilish scheme", false);
...
//[Agree to become Ceraph's Fuckpet Bondage Toy bitch slut]
		private function agreeToBecomeCeraphsFuckpetBondageToyBitchSlut():void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your " + vaginaDescript(0) + " tingles as you wonder how many piercings and fetishes she could fit into your labia.", false);
			else if (player.lowerBody.cockSpot.count() > 0) outputText("  Your " + cockDescript(0) + " grows rock hard as you wonder if she'll give you a Jacob's ladder full of new fetishes and thoughts.", false);
			...
			if (player.tailType == TAIL.NONE) outputText("backside", false);
...
//(EPILOGUE)
		private function agreeToBeABitchPt2():void
		{
			...
			if (player.lowerBody.cockSpot.count() > 0) outputText(" along with a magical cock-ring", false);
...
//[YES OH YES]
		...
		public function ceraphRapesYouBADDAWGYODIGGITY():void
		{
			...
			spriteSelect(7);
			outputText("", true);
			if ((!getGame().inCombat) && flags[FlagEnum.PC_FETISH] < 3) outputText("The demoness touches your forehead, and suddenly your mind is awash with fetishes!  Judging by the playful look she has, it's temporary, and you play along.  ", false);
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText(", right between your " + player.allBreastsDescript(), false);
...
//(REQ's – HUGE WANG, Exgartuan or Jojo corruption, and !centaur)
		private function hugeCorruptionForceFuckCeraph():void
		{
			...
			if (player.statusAffects.get("Exgartuan").value1 == 1) outputText("You pat your " + cockDescript(x) + " and say, \"<i>A little help please, Exgartuan?</i>\"  ", false);
			...
			if (player.lowerBody.cockSpot.list[x].cockLength < 68) outputText("  It's impossible!  You aren't long enough for that.  Her body must have shrunk itself to a more appropriate length...", false);
			outputText("\n\n", false);

			outputText("As you're squeezed and massaged by ", false);
			if (player.lowerBody.cockSpot.list[x].cockLength < 68) outputText(num2Text(int(player.lowerBody.cockSpot.list[x].cockLength / 12)), false);
			else outputText("six", false);
			outputText(" feet of love-tunnel, you struggle not to blow your load.  ", false);
			if (player.lowerBody.cockSpot.list[x].cockLength > 84) outputText("You've still got a lot more dick to get wet, and you start pushing the rest through her, using her like a plus-sized, organic onahole.  ", false);
			else {
				outputText("You sigh happily, rubbing your ", false);
				if (!player.lowerBody.cockSpot.hasSheath()) outputText("groin", false);
			...
			if (player.lowerBody.cockSpot.count() == 2) outputText(" and marvel at the cum your other cock dumped into the puddle", false);
			else if (player.lowerBody.cockSpot.count() > 2) outputText(" and marvel at the cum your other cocks dumped into the puddle", false);
			...
			if (flags[FlagEnum.PC_FETISH] > 0) outputText("I'll remove some of my magic from you if you want, and I'll even let you keep the piercing.  Would you like that?", false);
		...
		private function ceraphsNiceOffer():void
		{
			...
			if (flags[FlagEnum.PC_FETISH] > 0) {
				...
				if (flags[FlagEnum.PC_FETISH] == 1) outputText("your exhibitionism is gone!", false);
				else if (flags[FlagEnum.PC_FETISH] == 2) outputText("your bondage fetish is gone!", false);
		...
		public function friendlyNeighborhoodSpiderManCeraph():void
		{
			...
			if (player.stats.cor > 66) outputText("and without a second thought you disrobe, tossing your " + player.armorName + " to the side.  It thumps heavily in the dry mountain dust.", false);
			else if (player.stats.cor > 33) outputText("and you squirm and writhe within your " + player.armorName + ", the material itching on your skin.  Ultimately, you manage to resist.", false);
			...
			if (player.skinType <= SKIN.FUR) outputText("skin burns", false);
			else if (player.skinType == SKIN.SCALES) outputText("scales burn", false);
			...
			if (player.stats.cor > 66) outputText("I can see just how hot I'm making you, I'm so glad you gave in and got undressed for me", false);
			...
			if (player.stats.cor > 66) outputText("Are you going to try to fight me in the buff?  If you're going to do that then you can go ahead and get dressed, I wouldn't want to accidentally leave a scar in the wrong place.", false);
...
//[Run] → 
		private function runFromCeraphsNiceDeal():void
		{
			...
			if (player.stats.cor > 66) outputText("grab your equipment and ", false);
...
//[Trade] 
		private function tradeCeraphSomething():void
		{
			...
			if (player.hasItem(consumables.BIMBOLQ)) {
				...
				if (flags[FlagEnum.PC_FETISH] > 1) outputText("all the fetishes", false);
				...
				if (player.gender > 0 || player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText("  Or if you'd rather keep it, I could take one of your endowments for my own use and remove a single fetish.  It's always fun giving one of my old pets a new part to play with.", false);
			...
			//(No Bimbo Liquer) 
				...
				if (flags[FlagEnum.PC_FETISH] == 1) outputText("your fetish for you.</i>\"\n\n", false);
...
//[Trade Genitals]
		private function tradeCeraphCrotchityBits():void
		{
			...
			//(DICKS)
				...
				if (player.lowerBody.cockSpot.list[temp2].cockType == CockType.HORSE) outputText("Oooh, is that a horse-cock?  Those are quite nice.  I love the feel of a flare inside me, or even better, plunging one deep into a slut's poor, abused asshole.", false);
				else if (player.lowerBody.cockSpot.list[temp2].cockType == CockType.DOG) outputText("Oooh, is that a dog-cock?  Those are quite nice.  It's oh-so-fun to enhance the knots on those and then watch them puff up enough to distend some poor slave's gut.", false);
				else if (player.lowerBody.cockSpot.list[temp2].cockType == CockType.DEMON) outputText("Oooh, is that a demon-cock?  As you can see I'm QUITE familiar with those, but it might be fun to replace one of my girl's tongues with it.", false);
				else if (player.lowerBody.cockSpot.list[temp2].cockType == CockType.CAT) outputText("Oooh, is that a kitty-dick?  The spines on those feel so good while they rub down all your walls or catch your prostate.  Meow.", false);
				else if (player.lowerBody.cockSpot.list[temp2].cockType == CockType.LIZARD) outputText("Wow, you've got a snake-dick?  I haven't seen one of those in a while.  I love the bulbous, knot-like texture.  Maybe I'll graft it onto a chair and make one of my boys sit on it for a few days.", false);
				else if (player.lowerBody.cockSpot.list[temp2].cockType == CockType.TENTACLE) outputText("Oooh, a tentacle cock?  How lucky to find one of those on you!  I've been meaning to add another poor tentacle-slave.", false);
				else if (player.lowerBody.cockSpot.list[temp2].cockType == CockType.ANEMONE) outputText("Oooh, an anemone-cock?  Simply marvelous!  I might have to crank up the strength of the venom and use it myself!", false);
				...
				if (player.lowerBody.cockSpot.count() > 1) {
					...
					if (player.lowerBody.cockSpot.count() == 2) outputText("the other one", false);
			...
			//(Tits) 
				...
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 14) outputText("Oh wow... just wow.  How do you walk?  With melons like this you could tit-fuck three or four people at once!  Absolutely exquisite.", false);
				else if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 6) outputText("Tell me dear, are they real?  I mean, were they always this size, or did you have to get a little bit of work done?", false);
...
//*Orgy floor Drain
		public function ceraphBodyPartDreams():void
		{
			...
			if (choice == 2) {
				...
				if (subChoice == 0) {
					...
					if (!player.lowerBody.vaginaSpot.hasVagina()) outputText("You don't have a vagina any more, do you?", false);
				...
				//PUSSY CLITPUMP
					...
					if (!player.lowerBody.vaginaSpot.hasVagina()) outputText("Wait... you don't have a clit any more... Ceraph took that away.  ", false);
				...
				//PUSSY FLESHLIGHT
					...
					if (!player.lowerBody.vaginaSpot.hasVagina()) outputText("Wait... you don't have a pussy anymore.  ", false);
		...
		internal function buttRapeCeraph():void
		{
			...
			if (player.lowerBody.cockSpot.count() == 2) outputText("  The other " + cockDescript(x) + " rubs against her butt-cheeks, sliding through them with ease thanks to her copious secretions.", false);
			else if (player.lowerBody.cockSpot.count() > 2) outputText("  The rest of your male genitals slide between the demoness' pert butt-cheeks in one huge, femcum-slicked bundle.", false);
			...
			if (!player.lowerBody.cockSpot.hasSheath()) outputText("barely touching your groin", false);
			...
			if (player.lowerBody.cockSpot.list[x].cockThickness < 3) outputText("slightly-gaped", false);
			else if (player.lowerBody.cockSpot.list[x].cockThickness < 5) outputText("gaped", false);
			else outputText("gaping-wide", false);
			outputText(" asshole leaks spunk everywhere. Disgusted with her", false);
			if (player.stats.cor < 33) outputText(" and a little bit by yourself", false);
