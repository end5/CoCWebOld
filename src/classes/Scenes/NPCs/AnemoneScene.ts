 ...
 */
export default class AnemoneScene extends BaseContent implements TimeAwareInterface {
	...
	public timeChange(): boolean {
		...
		if (Flags.list[FlagEnum.ANEMONE_KID] > 0) {
			...
			if (Flags.list[FlagEnum.KID_SITTER] == 0 && Flags.list[FlagEnum.MARBLE_KIDS] >= 5 && model.time.hours > 10 && model.time.hours < 18 && rand(4) == 0) {
				kidABabysitsCows();
				needNext = true;
			}
			if (Flags.list[FlagEnum.KID_SITTER] == 1 && model.time.hours > 10 && model.time.hours < 18 && rand(4) == 0) {
	...
	public mortalAnemoneeeeee(): void {
		...
		if (Flags.list[FlagEnum.TIMES_MET_ANEMONE] == 0 || player.inventory.items.has(consumables.MINOCUM)) {
			...
			MainScreen.text("The initial surprise subsides to wooly-headedness and a feeling of mild arousal as the stingers in her tentacles find exposed flesh.  In panic of drowning you pull free of the ropy mass and backpaddle away from the girl until your " + LowerBodyDescriptor.describeFeet(player) + " reassuringly touch the shallows of the lakebed once again and you're far enough above water to be able to fight.\n\n", false);
		} else {
			Flags.list[FlagEnum.TIMES_MET_ANEMONE]++;
	...
	public defeatAnemone(): void {
		...
		if (player.stats.lust >= 33) {
			...
			if (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.vaginaSpot.get(0).clitLength >= 4) anal = anemoneButtPlugginz;
	...
	private victoryButtholeRape(): void {
		...
		MainScreen.text("You look over the anemone in front of you.  Your attention focuses on her blue shaft; those smaller tentacles should have plenty of pleasing venom in them as well.  Stripping off your " + player.inventory.armor.displayName + ", you approach her and push her backwards.  Her gills slide off her breasts and float at her sides. revealing a pair of cute nipples.  You take the opportunity to stroke the shaft of her penis and rub her vagina a bit, soaking up some venom and making your hands tingle.\n\n", false);

		MainScreen.text("Quite soon you can hardly stand your own arousal and your lover's cock is nice and hard.  Straddling the anemone, you position your " + ButtDescriptor.describeButthole(player) + " over her colorful shaft and gradually lower yourself towards it.  The florid crown slips into your hole, delivering the expected shock, and a gasp from behind you is accompanied by the anemone's hands moving to your hips.", false);
		//[butt hymen removal]
		if (!player.buttChange(monster.cockArea(0), true)) MainScreen.text("  ", false);
		MainScreen.text("Despite your anticipatory stiffening you find yourself trembling and your muscles weakening, but by a superb effort you manage to concentrate and lower yourself gently, savoring the slow crawl of the warmth up your " + ButtDescriptor.describeButthole(player) + ".  You reach the base of the anemone's short shaft soon, and pause for a minute; looking over your shoulder at the anemone, you notice her biting her lower lip impatiently.  Making a mental note of her cute expression to fuel your imagination, you turn forward and, putting your hands down for support, you begin to rise and fall on her erect penis.\n\n", false);

		MainScreen.text("With your " + ButtDescriptor.describeButthole(player) + " heated up from the aphrodisiac already, the friction is enough to warm it to fever pitch.  Over and over you impale yourself on the girl's rod, dragging trails of venom and heat up and down your rectum.", false);
		if (player.lowerBody.cockSpot.count() > 0) {
			MainScreen.text("  One hand involuntarily moves to your " + CockDescriptor.describeCock(player, 0) + " and begins stroking, smearing the copious pre-cum forced out by the prostate stimulation over " + CockDescriptor.describeMultiCockSimpleOne(player) + ".", false);
		}
		//[(if vag and nococks)
		else if (player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("  You lift one hand up to your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " and begin jilling yourself off.  This works to satisfy you for a while, but eventually you want more and grab a brace of tentacles floating in the water beside you, shoving them into your greedy pussy and smearing them around. This provokes a lusty moan from you and a giggle from your lover.", false);
		}
		MainScreen.text("  As you work your " + ButtDescriptor.describeButthole(player) + " on the tool, something happens to push your peak closer at a startling pace...\n\n", false);

		MainScreen.text("Your blue lover, restless now and uncontent to simply lie there anymore, begins to use her own hands and hips to pump in time with you, doubling the pace of the fuck.  The fervid intensity of her strokes doesn't leave any time for the venom to disperse before the next thrust arrives, making it feel as though your " + ButtDescriptor.describeButthole(player) + " is filled with one huge, hot cock that nevertheless slides in and out even as it remains motionless.  The sensation pushes you to orgasm quickly, your " + ButtDescriptor.describeButthole(player) + " clamping down on the anemone's penis", false);
		if (player.lowerBody.cockSpot.count() > 0) {
			MainScreen.text(" as " + CockDescriptor.describeMultiCockSimpleOne(player) + " twitches and ejaculates in a ", false);
			...
			MainScreen.text(" and your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " spasms around the tentacles and your fingers", false);
		}
		MainScreen.text(".  The anemone must have been right on the edge with you, because after a few more thrusts in your hungry asshole she achieves her own climax and shoots several strings of cool, thick semen into you.  You collapse backward against your partner and she idly caresses your " + BreastDescriptor.describeNipple(0) + ".  After several minutes of relaxing in the warm water, you sit up and push yourself off of the anemone's limp penis, which drags a string of semen out of your " + ButtDescriptor.describeButthole(player) + " and prompts ", false);
		//[(dix)
		if (player.lowerBody.cockSpot.count() > 0 || player.gender == 0) MainScreen.text("a ", false);
	...
	private rapeAnemoneWithDick(): void {
		...
		if (player.cockThatFits(36) >= 0) {
			...
			MainScreen.text("Rubbing yourself through your " + player.inventory.armor.displayName + ", you look over the anemone; your attention wanders down her torso to the blue slit between her legs", false);
			//[(lust victory)
			if (monster.lust > 99) MainScreen.text(", which she's currently diddling with the hand she's not using to stroke her cock", false);
			MainScreen.text(".  Unfastening your garments, you stroke " + CockDescriptor.describeMultiCockSimpleOne(player) + " to full hardness and approach her.  The anemone looks up at you, still somewhat befogged; then, as you stand over her, she leans forward and opens her mouth invitingly.\n\n", false);
			...
			MainScreen.text(" pull apart her feathery labia, showing a velvety interior.  <i>\"Ok...\"</i> she says haltingly.  You accept the invitation in a hurry, kneeling down and holding onto her hips, then guiding your " + CockDescriptor.describeCock(player, x) + " into her.\n\n", false);
			...
			if (player.lowerBody.cockSpot.count() > 1) {
				...
				if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("s", false);
				MainScreen.text(" up against the feelers lining her pussy.  Unexpectedly, they also contain the venomous nematocysts of her tentacles and in that single stroke " + CockDescriptor.describeMultiCockSimpleOne(player) + " is throbbing hard and squeezing pre-cum all over her groin.  She reaches down and plays with it", false);
			...
			//[(if balls)
			if (player.lowerBody.balls > 0) MainScreen.text("  As your thrusts get faster your " + BallsDescriptor.describeSack(player) + " starts to slap into the tickly and quite-venomous feelers fringing her pussy, getting hotter and hotter as the aphrodisiac builds in your " + BallsDescriptor.describeBalls(true, true, player) + ".  Your body answers with swelling, causing your sack to feel bigger and tighter with every slap.", false);
			//[(if noballs and clit > 6")
			else if (player.lowerBody.vaginaSpot.get(0).clitLength >= 6) MainScreen.text("  As your thrusts into the blue girl arouse you, your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " begins to swell with blood.  Pretty soon it's so erect that a particularly violent thrust mashes it into the feelers at the base of her labia, delivering a shock that almost makes you collapse.  The anemone, her reverie interrupted as you temporarily stop pumping, looks down.  Noticing the massive girl-cock sticking out of your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ", she reaches between her legs and gives the tip a flick, grinning with sadistic playfulness.  Your eyes cross at that, sending her into a spasm of giggling.  Irritated but aroused by the abuse of your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + ", you move your thumb over her own tiny blue button and begin flicking it in revenge as you renew your pumping.", false);
			MainScreen.text("\n\n", false);
			...
			MainScreen.text("Your own orgasm takes a bit longer, but the convulsing walls of her pussy do their best to help you arrive and the feelers along her labia writhe against your " + CockDescriptor.describeMultiCockShort(player) + " in the same tempo, injecting the last of their venom.  With a deep, final thrust, " + CockDescriptor.describeMultiCockSimpleOne(player) + " erupts in her pussy.  ", false);
			//[(big skeet)
			if (player.cumQ() > 500) {
			...
			MainScreen.text("After the last of your cum has been squeezed out, you pull your " + CockDescriptor.describeCock(player, 0) + " out and rinse it off in the lakewater.  You gather your gear while the anemone holds her belly and smiles placidly, staring into the sky.", false);
		...
		//Too-big male: -requires cockarea > 36
			...
			MainScreen.text("Rubbing yourself through your " + player.inventory.armor.displayName + ", you look over the anemone; your attention wanders down her torso to the blue slit between her legs", false);
			//[(lust victory)
			if (player.stats.lust > 99) MainScreen.text(", which she's currently diddling with the hand she's not using to stroke her cock", false);
			MainScreen.text(".  Unfastening your garments, you stroke " + CockDescriptor.describeMultiCockSimpleOne(player) + " to full hardness and approach her.  The anemone looks up at you, still somewhat befogged; then, as you stand ", false);
			...
			MainScreen.text("You smile at how stunned she is by you, and waggle your erect " + CockDescriptor.describeMultiCockShort(player) + " from side-to-side.  The anemone obediently watches it swing, like a hypnotist's pendulum; her mouth reflexively opens as all her conscious thought is temporarily sidetracked.  You push on the shaft with one hand and move the tip down relative to her body, watching bemused as the girl tries to keep her mouth in line with it until it goes too low for her neck and snaps her out of her daze.  She closes her mouth and looks at you quizzically.  <i>\"No?\"</i> she asks.  You answer by pushing forward slightly, bumping the head of your " + CockDescriptor.describeCock(player, 0) + " against her <i>mons pubis</i>.  The skin on her face darkens a bit as she realizes your intention... which turns out to be a blush, by the looks of the shy glance she gives you next!  <i>\"Umm.\"</i>  She gives it a moment of concerned study before her natural instincts take over and the anemone's fingers", false);
			//[(HP defeat)
			if (monster.HP < 1) MainScreen.text(" move to the lips of her vagina and", false);
			MainScreen.text(" pull apart her feathery labia, showing a velvety interior.  <i>\"Ok...\"</i> she says, clearly unsure of the wisdom of this.  You motion for her to lay back and lift her pussy as high as possible to reduce the angle, then attempt to guide your " + CockDescriptor.describeCock(player, 0) + " into her.", false);
			...
			MainScreen.text("The first few inches are slow going, as you try to stretch the blue girl's roughly human-sized pussy around your superhuman girth.  She sports a worried expression as you struggle to push the crown of your " + CockDescriptor.nounCock(CockType.HUMAN) + " in without bending your shaft.  The girl's body proves more elastic than you'd expected, though; with each shove her cunt takes more width without suffering any obvious ill effect.  Eventually you get the head of your " + CockDescriptor.describeCock(player, 0) + " into her, and you give an experimental thrust to test her depths. You slide surprisingly far into her, your huge " + CockDescriptor.nounCock(CockType.HUMAN) + " making a bump in her pelvis; the anemone has abandoned her worry and is blissfully tweaking her nipples.  Taking the anemone's legs under your arms, you begin pumping at her, savoring the sensation of having at least part of your unwieldy tool inside someone.  Your blue partner gives out cute moans as you fuck her, prompting a grin from you, but most of your attention is focused on maintaining the angle of your " + CockDescriptor.describeCock(player, 0) + ".  As you focus on the thrusts, you gradually notice the bump sliding further up her pelvis on each push... it's nearly up to her stomach now!  You quickly make up your mind to test her unusual biology, choking up on your grip of her legs and pushing hard with each thrust in.  The anemone is now tracking the progress of the bump representing your " + CockDescriptor.nounCock(CockType.HUMAN) + " as intently as you are; as your head pushes up her chest between her breasts she presses them together against the ridge, massaging them against the shaft through her skin.", false);
			//[(cocklength > 60")
			if (player.lowerBody.cockSpot.get(0).cockLength > 60) MainScreen.text("  The imagery and the stimulation inspire you to increase your efforts, and you push even harder into her until your " + CockDescriptor.describeCock(player, 0) + " slides its way into her throat, expanding her slender neck to twice the normal width.  The anemone's mouth gapes open reflexively as if to gag as the pressure forces her head back, but she doesn't stop moaning or rubbing herself against you.  This seems to be as deep as you can get; the tip of your cock is now right up against the base of her chin.", false);
			MainScreen.text("\n\n", false);

			MainScreen.text("Inordinately pleased at having gotten your " + CockDescriptor.describeCock(player, 0) + " so far in, you begin thrusting grandly, alternating huge back-and-forth pumps with hip gyrations that press you against the sides of your partner.  The anemone", false);
			//[(dick > 60")
			if (player.lowerBody.cockSpot.get(0).cockLength > 60) MainScreen.text(", despite being unable to look anywhere except over her head thanks to the giant ridge running up the front of her body,", false);
			MainScreen.text(" has clearly been enjoying the treatment; soon she begins twitching in orgasm and her pussy spasms against the base of your " + CockDescriptor.nounCock(CockType.HUMAN) + ", flicking its venomous feelers into it, as she kneads her breasts in her hands.  Her body attempts to convulse, bending your " + CockDescriptor.describeCock(player, 0) + " slightly, while her neglected dick gives a little squirt of pearly semen which lands on the raised line in the center of her body and makes a little trail as it rolls down.  The pressure of her twisting and the sensation of her pussy lapping at your shaft with aphrodisiac is enough to send you over the edge as well.  Your " + CockDescriptor.describeCock(player, 0) + " twitches as you unload into your blue partner", false);
			//[(big skeet)
			...
			MainScreen.text("Eventually both you and she are spent and limp, and you draw your " + CockDescriptor.describeCock(player, 0) + " out of her, making an extended, wet sucking noise.  As you pull up your gear and make your way up the beach, ", false);
			//[(normal/big skeet)
	...
	private rapeAnemoneWithPussy(): void {
		...
		MainScreen.text("Next, you take the time to strip off your " + player.inventory.armor.displayName + ", making sure to give a good show; the anemone entertains herself by stroking her erect prick and smearing around the pre-cum, grinning as she watches you.", false);
		//[(breastrow0>C-cup)
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) MainScreen.text("  You give special attention to the presentation of your " + BreastDescriptor.describeBreastRow(player.upperBody.chest.get(0)) + ", removing your top with tantalizing slowness, letting each breast slip out and hang between you like fruit ripe to be taken, then making sure to rub them seductively to arouse the both of you further.", false);
		//(hipsize=girly or better)
		if (player.lowerBody.hipRating > 6) MainScreen.text("  You make good use of your " + LowerBodyDescriptor.describeHips(player) + ", too, giving a little shimmy to show off your pronounced curves.", false);
		...
		//[(breast0>C)
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) MainScreen.text("making sure to let your " + player.BreastDescriptor.describeAllBreasts(player) + " rub up against hers, ", false);
		MainScreen.text("then pull apart from her and ", false);
		//[(goddamn centaur)
		if (player.lowerBody == LowerBodyType.CENTAUR) MainScreen.text("turn away, kneeling down to display your animalistic, musky pussy readily.", false);
		else MainScreen.text("recline back on your " + LowerBodyDescriptor.describeLegs(player) + ". Spreading your thighs, you reach down with two fingers and pull apart your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " welcomingly; it's the last act in your sexual performance.", false);
		MainScreen.text("\n\n", false);
		...
		//[(no centaur)
		if (player.lowerBody != LowerBodyType.CENTAUR) MainScreen.text("You take her by the upper arms and pull her on top of you as you lie back in the sun-warmed shallows.  ", false);
		MainScreen.text("Her hair drapes over you as she lines her penis up with your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ", delivering heat to your body, but this is dwarfed by the sensation of her entry as she pushes her cock in for the first time.  ", false);
		...
		if (player.lowerBody != LowerBodyType.CENTAUR) {
			...
			//[(breasts>manly)
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text(BreastDescriptor.describeBreastRow(player.upperBody.chest.get(0)) + " and ", false);
			MainScreen.text(BreastDescriptor.describeNipple(0) + "s,", false);
		}
		//(centaur)
		else {
		...
		MainScreen.text("The overwhelming sensations drive you over the edge and your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " contracts hungrily around the heat radiating from the anemone's cock.  As your orgasming pussy ", false);
		//(squirter)
		if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= VaginaWetness.DROOLING) MainScreen.text("soaks her crotch with juice and ", false);
		...
		if (player.lowerBody == LowerBodyType.CENTAUR) MainScreen.text(" on your back", false);
		//[(notits)
	...
	public loseToAnemone(): void {
		...
		//loss, neuter:
		if (player.gender == 0) {
			MainScreen.text("Shivering, you slump before the anemone and begin trying to remove your " + player.inventory.armor.displayName + ".  She claps and peals happily, splashing over to you.  Pushing your trembling hands out of the way, she unfastens your garments and pulls them free of you... and then stops.  You look the girl in the face uncomprehendingly and she answers your gaze with a look of equal confusion.  Your head turns to follow her as she looks you up and down and even makes a circle around you, inspecting every inch and chewing her lip anxiously.", false);
			//[(lactating)
			if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier > 1) MainScreen.text("  For a moment the examination stops at the dribble of milk leaking from your " + BreastDescriptor.describeNipple(0) + "s.  With one finger she collects a bit and tastes it, only to grimace and stick her tongue out.", false);
		...
		if (player.lowerBody.cockSpot.count() > 0) {
			MainScreen.text(images.showImage("anemone-male-fuck"), false);
			if (player.cockThatFits(36) >= 0) {
				MainScreen.text("Shivering, you slump before the anemone and begin trying to remove your " + player.inventory.armor.displayName + ".  She claps and peals happily, splashing over to you.  Pushing your trembling hands out of the way, she unfastens your garments and pulls them free of you, taking the opportunity to run a hand up your " + CockDescriptor.describeCock(player, x) + ".  ", false);
				...
				//[(normal)
				if (player.lowerBody != LowerBodyType.CENTAUR) MainScreen.text(" into your lap,", false);
				//(shitty taur)
				else MainScreen.text(" downwards, onto your hams,", false);
				MainScreen.text(" delivering lances of venom into your lower body.  The tingle warms your groin and more pre-cum leaks out of " + CockDescriptor.describeMultiCockSimpleOne(player) + ".  Her eyes lock onto a glistening dollop and she drops down quickly, enfolding the head of your " + CockDescriptor.describeCock(player, x) + " in her cool mouth.  Her tongue dances around the crown of your " + CockDescriptor.nounCock(CockType.HUMAN) + ", relieving it of the sticky pre.  Looking ", false);
				if (player.tallness > 48) MainScreen.text("up", false);
				else MainScreen.text("down", false);
				MainScreen.text(" at you, you can see a smile in the lines around her eyes even though her mouth is locked around your " + CockDescriptor.nounCock(CockType.HUMAN) + ".  ", false);
				//[(if dual/multi-cox)
				if (player.lowerBody.cockSpot.count() > 1) {
					//[(cock2 doesn't exist)
					if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("Your other dick rubs", false);
					//(cock2 exists)
					else MainScreen.text("The rest of your " + CockDescriptor.describeMultiCockShort(player) + " rub", false);
					MainScreen.text(" against her cheek, smearing slick wetness on her blue face.", false);
				}
				MainScreen.text("\n\n", false);

				MainScreen.text("Her hands come up from the water and push two sheaves of her long, dangling hair into your " + CockDescriptor.describeMultiCockShort(player) + ".  Wrapping these bundles of tentacles around your " + CockDescriptor.describeCock(player, x) + ", she clasps them in place with one hand and begins sliding them up and down your length.  Your " + CockDescriptor.nounCock(CockType.HUMAN) + " begins feeling hotter and hotter from the injections and the friction of her hair, secreting more pre-cum into her obliging mouth.", false);
				//[(if vag)
				if (player.lowerBody.vaginaSpot.hasVagina()) {
					...
					MainScreen.text(" into your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ", sending a tingle through your lower lips and exposing your clitoris.", false);
					//[(if clit > 5")
					if (player.lowerBody.vaginaSpot.get(0).clitLength > 5) MainScreen.text("  Having achieved this, she pulls her hand out and wraps another bundle of tentacles around your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + ", then begins jerking it off in time to her efforts on your " + CockDescriptor.nounCock(CockType.HUMAN) + ".  Your eyes roll back in your head and your mouth gapes involuntarily at the rough stimulation of your swollen chick-stick.", false);
				}
				MainScreen.text("\n\n", false);
				...
				MainScreen.text(" quickly gets to you, and the first orgasm begins to work its way up your " + CockDescriptor.describeMultiCockShort(player) + ".  Your " + CockDescriptor.describeCock(player, x) + " lets fly into the anemone girl's mouth", false);
				//[(big skeet)
				...
				if (player.lowerBody.cockSpot.count() > 1) {
					...
					//[(dick2 = no)
					if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("your cocks shoot", false);
					else MainScreen.text("the rest of your " + CockDescriptor.describeMultiCockShort(player) + " shoot", false);
				...
				MainScreen.text("After a grateful moment of rest as the anemone swallows your issue, her hands begin pumping once again.  Oh god!  Your " + CockDescriptor.nounCock(CockType.HUMAN) + " quickly returns to erectness under the renewed siege of aphrodisiac", false);
				//[(multi)
				if (player.lowerBody.cockSpot.count() > 1) {
					if (player.lowerBody.cockSpot.count() == 2) MainScreen.text(" and your other " + CockDescriptor.describeCock(player, 1) + " follows suit", false);
			...
			//loss, too-big male (cock > 5" width or > 48" length):
				...
				MainScreen.text("Shivering, you slump before the anemone and begin trying to remove your " + player.inventory.armor.displayName + ".  She claps and peals happily, splashing over to you.  Pushing your trembling hands out of the way, she unfastens your garments and begins to pull them free of you, but your " + CockDescriptor.describeMultiCockShort(player) + " flops out and bops her in the nose!  As you fumble the rest of the fastenings and finish removing your gear, the blue girl watches mesmerized at the bobbing flesh in front of her, slick pre-cum leaking from the tip", false);
				//[(big skeet)
				if (player.cumQ() > 500) MainScreen.text(" in a steady stream", false);
				MainScreen.text(".\n\n", false);

				MainScreen.text("Almost reverently, she caresses the shaft of your " + CockDescriptor.describeCock(player, 0) + ", stroking lightly up its enormous length.  She pulls it down to her eye level, inspecting the head from several angles.  Tentatively, she opens her mouth and pulls your " + CockDescriptor.nounCock(CockType.HUMAN) + " into it, trying to fit your expansive dickflesh into a hole that even to your lust-crazed mind looks much too small.  Despite her best efforts, she can't get more than the crown past her lips, though the reflexive motions of her tongue poking around and inside the opening make you shiver and push out more pre-cum.  The anemone eventually pops your " + CockDescriptor.describeCock(player, 0) + " out of her mouth and frowns in frustration.  After a few seconds, she seems to reach a decision.  Moving your shaft out of the way, she walks around behind you.  She places one hand on your ", false);
				if (player.lowerBody != LowerBodyType.CENTAUR) MainScreen.text("waist", false);
				else MainScreen.text("flank", false);
				//[(not centaur)
				if (player.lowerBody != LowerBodyType.CENTAUR) MainScreen.text(" and pushes your shoulders down with the other", false);
				MainScreen.text(".  As she draws you backwards, you're forced to put your own ", false);
				if (player.lowerBody != LowerBodyType.CENTAUR) MainScreen.text("hands ", false);
				else MainScreen.text("forelegs knee-", false);
				MainScreen.text("down in front of you to keep from falling face-first.  ", false);
				//[(if non-centaur)
				if (player.lowerBody != LowerBodyType.CENTAUR) MainScreen.text("The head of your " + CockDescriptor.describeCock(player, 0) + " dips into the lukewarm lakewater, sending a tingle down the shaft.  ", false);
				MainScreen.text("Behind you, the anemone has taken her blue, tentacled penis into her hand and is stroking it and fondling the tip, forcing her own pre-cum out and smearing it along her length.  Satisfied with its slipperiness, she edges forward until her cock is resting on your " + ButtDescriptor.describeButt(player) + ".  Drawing her hips back, she lines it up with your " + ButtDescriptor.describeButthole(player) + ", then thrusts forward while pulling back on your waist.  The wriggly feelers slip past your butthole and light up your insides with her potent venom.", false);
				player.buttChange(monster.cockArea(0), true);
				MainScreen.text("\n\n", false);

				MainScreen.text("Taking a moment to transfer her now-free hand to your other hip, the anemone girl then begins to pump her stiff pecker into your " + ButtDescriptor.describeButthole(player) + ", pausing after every few strokes to gyrate her hips a bit, massaging your prostate with her feelers and smearing venom into it.  The stimulation brings you to your limit in minutes; your dick twitches spastically", false);
				//[(if balls)
				if (player.lowerBody.balls > 0) MainScreen.text(" and your " + BallsDescriptor.describeSack(player) + " tightens up", false);
				MainScreen.text(".  This doesn't escape your blue lover's notice, and she quickly stops pumping.  Left at the edge of orgasm, you panic and look over your shoulder at her.  Judging by her grinning demeanour, she seems to be up to something diabolical.  You stare at her confusedly until you feel a new heat at the base of your " + CockDescriptor.describeMultiCockShort(player) + ". Glancing down, you see that her tentacle-hair has wrapped around " + CockDescriptor.describeMultiCockSimpleOne(player) + " and is squeezing tightly!  Pleased with the arrangement, the anemone begins pumping and rubbing your prostate again, spreading new warmth through your " + ButtDescriptor.describeButthole(player) + ".  Your delayed climax finally arrives, but the <i>de facto</i> cockring", false);
				...
				MainScreen.text("For nearly an hour the anemone continues her performance, even going so far as to caress your swollen " + CockDescriptor.describeMultiCockShort(player) + " with her unoccupied tentacles.  Several more orgasms arrive and desert you without bringing any relief from the pressure on your ", false);
				//[(if balls)
				if (player.lowerBody.balls > 0) MainScreen.text(BallsDescriptor.describeBalls(true, true, player) + " and ", false);
				MainScreen.text(CockDescriptor.describeMultiCockShort(player) + ".  Eventually you get to the point where you can't take it anymore, and when you feel the next orgasm drawing close you straighten up and begin ", false);
				//[(man)
				if (player.lowerBody != LowerBodyType.CENTAUR) MainScreen.text("clawing at your tormentor's tentacles, trying to pry them from " + CockDescriptor.describeMultiCockSimpleOne(player) + " by main force.", false);
				//(horse)
				...
				MainScreen.text("  Looking a bit irritated that you want to bring her fun to an end, the anemone nevertheless relents and releases her visegrip on your " + CockDescriptor.describeMultiCockShort(player) + ".  As the joy of seeing the way to your release cleared overtakes you, the anemone avails herself of your distraction to grab your arms and pull you toward her while pushing your " + LowerBodyDescriptor.describeLegs(player) + " out from under you.  The two of you fall backward into the shallow water as " + CockDescriptor.describeMultiCockSimpleOne(player) + " begins unloading its immense backup of semen in a high arc.  The ", false);
				//[(skeet amount)
				...
				if (player.tallness < 48 && player.lowerBody != LowerBodyType.CENTAUR) MainScreen.text("fly over your head, and turning behind you, you see the anemone trying to catch them with an open mouth and her tongue out.", false);
				else if (player.tallness < 84 && player.lowerBody != LowerBodyType.CENTAUR) MainScreen.text("catch the air and rain down on both your faces, splashing quietly where they hit water.", false);
				else {
				...
				MainScreen.text("  After several minutes of climax with you shooting more jism than you thought possible and the anemone banging out an accompaniment on your " + ButtDescriptor.describeButthole(player) + ", you finally begin to wind down.  The anemone, clearly turned on by the impressive amount of ejaculate, unloads her own blue cock into your asshole.  Her semen, lower in temperature than yours, forms a little pocket of cool inside your " + ButtDescriptor.describeButt(player) + ".  She idly swishes her tentacles in the", false);
				//[(big skeet)
				...
				if (player.tallness < 48 && player.lowerBody != LowerBodyType.CENTAUR) MainScreen.text("She looks you over, then bends down and drinks up as much of the semen floating in the water as she can find nearby.", false);
				...
				MainScreen.text("  Having fed, she grins mischievously and grabs your " + CockDescriptor.describeCock(player, 0) + ", then tows your floating body to the shoreline with it.  She rolls you onto the sand and then swims for deep water, vanishing.", false);
		...
		//loss rape, vaginal (only full females):
			...
			MainScreen.text(images.showImage("anemone-female-fuck"), false);
			MainScreen.text("Shivering, you fall to your knees before the anemone and begin trying to remove your " + player.inventory.armor.displayName + ".  She claps and peals happily, splashing over to you.  Pushing your trembling hands out of the way, she unfastens your garments and pulls them free of you, but her bright expression dims a bit when she sees only your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".", false);
			//[(lactation)
			if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier > 1) MainScreen.text("  For a moment it brightens again when she notices the dribble of milk leaking from your " + BreastDescriptor.describeNipple(0) + "s. With one finger she collects a bit and tastes it, only to grimace and stick her tongue out.", false);
			...
			//[(if breasts)
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) MainScreen.text(" " + BreastDescriptor.describeBreastRow(player.upperBody.chest.get(0)) + " and", false);
			MainScreen.text(" " + BreastDescriptor.describeNipple(0) + "s and drifts down past your navel.  It makes a stop at your vulva, tickling your most sensitive area and causing your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " to swell with proof of your pleasure.  The hand begins its return trip, delivering one upward stroke to your now-engorged button and shooting a spark up your spine.  It comes to rest on your hip, and the anemone presses you downward, slowly but insistently, until your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " comes to rest above her hard, blue shaft.  Two of her longer tentacles reach up from the water and touch themselves to your lower lips, pulling them apart and delivering jolts of aphrodisiac that make your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " clench and release convulsively.  Her hand resumes downward pressure, guiding your twitching pussy toward her erect blue shaft; its small tentacles bend upward in the manner of a flower turning to face the sun.  In a breathless moment the head and then the shaft push past the boundary of your open lips, the first intrusion sending home its own venom and tipping you over the teetering precipice of your control.  ", false);
			//[hymen removal]
			player.cuntChange(monster.cockArea(0), true);
			...
			MainScreen.text("The anemone releases your hand and transfers hers to your other hip just as the last of your willpower evaporates; you begin bucking your hips up and down her twitching blue shaft, painting the walls of your pussy with her venom like a mad virtuoso.  As the spasms in your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " ebb and flow with each new orgasm, the anemone's cool affectation changes to a mask of faltering determination, matching her attempt to hold out as long as possible while your demented pussy does its best to wring her dry.  From the looks on your faces it's unclear now who was intending to ravish whom!  Eventually the poor girl can take no more of it and her pulsing dick, swollen almost an inch more than when it went in with frenzied tentacles whipping this way and that, twitches and unleashes the first jet of her semen.  Her ejaculate is actually colder than your venom-teased cunt by a significant amount, creating a sharply-felt contrast as she shoots several more strings against the walls of your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " and the mouth of your womb.  The dichotomy couples with the satisfaction of finally getting what your pussy wanted to trigger the biggest orgasm yet and the gobsmacked anemone's jaw practically falls off her face as your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " squeezes faster than ever on her sensitive dick right after her own climax.\n\n", false);

			MainScreen.text("After several minutes of this final orgasm you fall backwards into the shallow water with a splash and pass out with a look of bliss, floating on a surface made choppy by your hectic ride.  The poor anemone takes a while longer to collect herself, then slowly pulls her limp dick out of your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " and tugs you up the beach past the tideline so you won't roll facedown in the water while you're unconscious.  She bends down and kisses you, tracing your " + BreastDescriptor.describeNipple(0) + "; too spent to hold up her hair, it drapes over your prone form as she leans and releases a last shot of her drug to ensure that your dreams will be of her.", false);
			anemonePreg();
	...
	private minoCumForAnemonieeeeez(): void {
		spriteSelect(4);
		MainScreen.text("The initial surprise subsides to wooly-headedness and a feeling of mild arousal as the stingers in her tentacles find exposed flesh.  In panic of drowning you pull free of the ropy mass and backpaddle away from the girl until your " + LowerBodyDescriptor.describeFeet(player) + " reassuringly touch the shallows of the lakebed.  As you shake your head to clear the haze, you notice a few of your items have fallen out of your pouches and are floating in the water.  The anemone has picked one in particular up and is examining it; a bottle of minotaur cum.  Her eyes light up in recognition as the fluid sloshes back and forth and she looks beseechingly at you, cradling it next to her cheek.  \"<i>Gimme?</i>\" she asks, trying to look as sweet as possible.\n\n", false);
	...
	private giveMino(): void {
		...
		if (player.stats.lust < 30) {
			...
			if (player.lowerBody.cockSpot.count() > 0) MainScreen.text(CockDescriptor.describeCock(player, 0) + " becomes a little erect", false);
			else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " aches a bit with need", false);
			else MainScreen.text(ButtDescriptor.describeButthole(player) + " begins to tingle with want", false);
	...
	private anemoneButtPlugginz(): void {
		...
		if (!player.lowerBody.isTaur()) {
			...
			else MainScreen.text("your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)));
		...
		if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(48) >= 0) {
			if (!player.lowerBody.isTaur()) MainScreen.text("you fish your [cockFit 48] out of your garments and rub it");
			else MainScreen.text("you rub your [cockFit 48]");
			MainScreen.text(" between the smooth blue curves");
		}
		else {
			if (!player.lowerBody.isTaur()) MainScreen.text("you finger your pussy");
			else MainScreen.text("you imagine the tightness of her hole");
			MainScreen.text(" until your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " starts to fill with blood and peeks from its hood");
		}
		...
		MainScreen.text("\n\n\"<i>Where's the hole?</i>\" you demand, pointing at your own " + ButtDescriptor.describeButt(player) + " for an example.");
		...
		if (!player.lowerBody.isTaur()) hotdog = hotdogTheAnemone;
	...
	private anemoneQuoteUnquoteAnal(): void {
		...
		if (!player.lowerBody.isTaur()) {
			...
			if (dick) MainScreen.text(CockDescriptor.describeCock(player, x));
			else MainScreen.text(VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)));
			MainScreen.text(" until it protrudes from the hole in your " + player.inventory.armor.displayName + ", returned to full erectness.  ");
		}
		MainScreen.text("The anemone regards you intently as you approach her and ");
		if (!player.lowerBody.isTaur()) MainScreen.text("seize her head");
		else {
		...
		if (dick) {
			...
			if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < 4) MainScreen.text("drools its lube along your length.  ");
	...
	private hotdogTheAnemone(): void {
		...
		if (dick) {
			MainScreen.text("your swollen, painfully hard prick fountains with cum, coating the anemone's back and hair in white.  The venom in your system draws out your orgasm to incredible lengths, and " + CockDescriptor.describeMultiCockSimpleOne(player) + " ejaculates over and over until you feel lightheaded and woozy.");
			...
			if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < 4) MainScreen.text("drooling its lube down your thighs.");
	...
	private anemoneGetsLayedByBeePositor(): void {
		...
		if (player.upperBody.head.antennae > AntennaeType.NONE) MainScreen.text(" antennae");
		...
		//[(sens>=50 or horse)
		if (player.stats.sens >= 50 || player.lowerBody.isTaur()) {
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				...
				if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("male part erupts");
	...
	public anemoneKidBirthPtII(): void {
		MainScreen.clearText();
		spriteSelect(71);
		MainScreen.text("You awake and look skyward to the sun for a hint at the time.  What greets you is more of an eclipse; a shape impedes your view.  As your eyes adjust to the light, it resolves into an upside-down blue-eyed, blue-skinned face wreathed with snubby, shoulder-length tentacles of purple and green hue.  The silence continues as you stare into it, until you move to push yourself off the hard ground.  At the first sign of activity, the head disappears into the water barrel with a sloshing sound.  You push yourself to your " + LowerBodyDescriptor.describeFeet(player) + " and look back toward it; the eyes, now right side-up, peek over the rim bashfully atop a set of blue fingers.");
	...
	public approachAnemoneBarrel(): void {
		...
		if (Flags.list[FlagEnum.ANEMONE_WEAPON_ID] != 0) {
			MainScreen.text("She has " + ItemType.lookupItem(Flags.list[FlagEnum.ANEMONE_WEAPON_ID]).longName + " sitting next to it.  ");
			//MainScreen.text("Kid A pokes her head out and smiles at you.  What do you need from her?");
			weaponT = "Take Weapon";
			weaponB = takeOutOfAnemone;
		}
		//(if babysitting)
		if (Flags.list[FlagEnum.KID_SITTER] > 1) MainScreen.text("Kid A is away right now, helping season some of Marble's children.  If you wanted to leave or take an item, she would discover it when she got back.  ");
		...
		if (Flags.list[FlagEnum.ANEMONE_WATCH] > 0) MainScreen.text("\n\n<b>Kid A is currently watching for enemies at night.</b>");
	...
	private tutorAnemoneKid(): void {
		...
		//[(KidXP < 33)]
		if (kidAXP() < 33) {
			MainScreen.text("holding " + ItemType.lookupItem(Flags.list[FlagEnum.ANEMONE_WEAPON_ID]).longName + " protectively across her chest.");
		}
		else MainScreen.text("taking up an attentive stance with " + ItemType.lookupItem(Flags.list[FlagEnum.ANEMONE_WEAPON_ID]).longName + " in her hands.");
		...
		if (Flags.list[FlagEnum.ANEMONE_WEAPON_ID] == weapons.PIPE.id ||
			...
			if (Flags.list[FlagEnum.ANEMONE_WEAPON_ID] == weapons.PIPE.id) MainScreen.text("pipe");
		...
		//[Whip] or [Succubi's Whip]
			...
			if (Flags.list[FlagEnum.ANEMONE_WEAPON_ID] == weapons.SUCWHIP.id) MainScreen.text("  The demonic enchantment chooses then to activate, and her color deepens as her lust peaks, as does your own.");
		...
		if (player.HP < 1) {
			...
			if (Flags.list[FlagEnum.ANEMONE_WEAPON_ID] != weapons.S_GAUNT.id &&
				Flags.list[FlagEnum.ANEMONE_WEAPON_ID] != weapons.H_GAUNT.id) MainScreen.text("the thump of something hitting the ground and ");
	...
	private kidASex(cont: boolean = true): boolean {
		...
		if (Flags.list[FlagEnum.ANEMONE_KID] == 2 && player.lowerBody.cockSpot.hasCock() && player.cockThatFits(36) >= 0) {
			x = player.cockThatFits(36);
			MainScreen.text("\n\n");
			MainScreen.text(images.showImage("anemone-kid-male-vagfuck"), false);
			MainScreen.text("You collapse onto your back, panting your arousal into the dry air.  Shyly at first but with increasing confidence as you fail to react, your daughter slips a hand into your clothes and down to your crotch.  She bites her lip and blushes as her hand reaches the neck of your " + CockDescriptor.nounCock(player.lowerBody.cockSpot.get(x).cockType) + ", then her resolve appears to crystallize as she yanks it free from your " + player.inventory.armor.displayName + ". At first, nothing happens and your erect cock just bobs through the empty air, but then you feel soft fingertips guiding it and a warm, tight squeeze at the end of your dick.  Raising your head in surprise, you see the anemone has mounted you!  She blushes and looks away at the eye contact, but her mouth opens in a sly grin as she begins to shift her hips, sliding your " + CockDescriptor.describeCock(player, x) + " deeper into herself.");
		...
		//if KidXP >= 40 and lust > 99 after tutor and PC has wang of area <60, set that cock to x variable, also set biggest cock not yet used to y variable if multicocked
			...
			if (y >= 0 || player.lowerBody.balls > 0 || player.lowerBody.vaginaSpot.hasVagina()) {
				...
				if (y >= 0) {
					...
					if (player.lowerBody.cockSpot.get(y).cockLength >= 36) MainScreen.text("towers over her head");
				...
				//[(sac)
				else if (player.lowerBody.balls > 0) MainScreen.text("  One hand cups below your " + BallsDescriptor.describeBalls(false, true, player, true) + " and lifts them up to her chin, where the other, formerly tracing gentle circles along your shaft, grabs a tentacle from her head and begins to tickle.");
				//[(vag)
				else if (player.lowerBody.vaginaSpot.hasVagina()) {
					MainScreen.text("  Her fingers part your labia and reveal your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + ", ");
					if (player.lowerBody.vaginaSpot.get(0).clitLength >= 6) MainScreen.text("working it free as well and allowing her to tease and stroke it, cock-like, with her hands and hair.");
			...
			if (player.lowerBody.balls > 0 || y >= 0) {
				...
				if (y >= 0) MainScreen.text(CockDescriptor.describeCock(player, y));
				else MainScreen.text(BallsDescriptor.describeSack(player));
		...
		//if KidXP >= 40 and lust > 99 after tutor and PC has vag and no cox
			...
			MainScreen.text("You collapse onto your back, panting your arousal into the dry air.  Shyly at first but with increasing confidence as you fail to react, your daughter slips a hand into your clothes and down to your crotch.  She stops, vexed, as her fingers find nothing but your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ", then appears to reach a decision and pulls down the bottoms of your " + player.inventory.armor.displayName + ".  Gently, she caresses the labia, eliciting a soft moan from you, then looks up nervously at the sound to check your response.  When you continue staring into the empty sky, she pulls away and you feel hands pushing your legs apart.  Even this isn't enough to stir you from your lust-induced haze, nor is the feeling of having your backside lifted by those same hands.  However, when you feel a persistent, wriggling pressure at the entrance to your inflamed pussy, you begin to return to reality.  Lifting your head to look at the anemone, you see her holding your thighs in the air as she attempts to orient herself to you, utmost concentration lining her features.  She succeeds and slides into you, then looks up to check your face again... and drops your body into her lap as she pulls her hands in front of herself defensively at the sudden scrutiny.");

			MainScreen.text("\n\nLaughable as her reaction is, the venom now coursing through your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " ensures not a giggle escapes you; your hips begin writhing in her lap, trying to find purchase on the blue girl to better pump her shaft for its seed.");
			//[(if pc is loose)
			if (player.lowerBody.vaginaSpot.get(0).vaginalLooseness >= VaginaLooseness.GAPING) MainScreen.text("  You can barely feel her little shaft in your stretched cunt, but the chemical stimulation from the tentacles stroking your insides goes a long way toward making up for that.");
			MainScreen.text("  Emboldened, she picks up your legs haltingly, then begins to work herself in and out of your depths.");

			MainScreen.text("\n\nWith all the grace of a first-timer, the girl clumsily leans down to kiss you, but falls short and can only plant a smooch on your still-clad [chest].  Still, she continues pumping enthusiastically, worry and shame evaporating from her brow as you moan lustily instead of rebuking her temerity.  Pausing to support you with one hand as she spreads your lips wider with her fingers, she exposes your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " to the air.");
			//[(clitsize<6)]
			if (player.lowerBody.vaginaSpot.get(0).clitLength < 6) MainScreen.text("  The little button gets rubbed by her probing hand even as she resumes thrusting, accelerating your imminent peaking.");
			else if (player.lowerBody.vaginaSpot.get(0).clitLength < 36) MainScreen.text("  The monstrous chick-stick bobs in the air as she pounds you harder; the breeze alone would be enough to arouse you further, but the anemone grabs it and begins jacking the nerve-laden stalk off like a dick, which sends your back into spasms.");
	...
	public kidADreams(): void {
		...
		if (player.lowerBody.cockSpot.hasCock() && (!player.lowerBody.vaginaSpot.hasVagina() || player.femininity < 50)) {
			...
			if (player.lowerBody.isTaur()) MainScreen.text("thigh");
			...
			if (!player.lowerBody.isTaur()) MainScreen.text("soft hand strokes your face and a ");
	...
	public kidAWatchesSharks(): void {
		...
		//(else KidXP < 75 and Kidweapon = White Book or Black Book)
			...
			if (Flags.list[FlagEnum.ANEMONE_WEAPON_ID] == consumables.W__BOOK.id) MainScreen.text("blinds them with a sudden flash of white light");
