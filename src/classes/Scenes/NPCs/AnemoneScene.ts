...
//anal: -requires butthole
		private function victoryButtholeRape():void
		{
			...
			if (player.lowerBody.cockSpot.count() > 0) {
				...
				//[(if vag and cox)
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" and your pussy spasms", false);
			}
			else if (player.lowerBody.vaginaSpot.hasVagina()) {
			...
			else if (player.lowerBody.vaginaSpot.hasVagina()) outputText("another ", false);
		...
		private function rapeAnemoneWithDick():void
		{
			...
			if (player.cockThatFits(36) >= 0) {
				...
				if (player.lowerBody.cockSpot.count() > 1) {
					...
					if (player.lowerBody.cockSpot.count() > 2) outputText(", and them,", false);
			...
			//Too-big male: -requires cockarea > 36
				...
				//[(dick > 60")
				if (player.lowerBody.cockSpot.list[0].cockLength > 60) outputText(", despite being unable to look anywhere except over her head thanks to the giant ridge running up the front of her body,", false);
...
//using pussy:
		private function rapeAnemoneWithPussy():void
		{
			...
			//[(breastrow0>C-cup)
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText("  You give special attention to the presentation of your " + breastDescript(0) + ", removing your top with tantalizing slowness, letting each breast slip out and hang between you like fruit ripe to be taken, then making sure to rub them seductively to arouse the both of you further.", false);
			//(hipsize=girly or better)
			if (player.lowerBody.hipRating > 6) outputText("  You make good use of your " + hipDescript() + ", too, giving a little shimmy to show off your pronounced curves.", false);
			...
			//[(breast0>C)
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText("making sure to let your " + player.allBreastsDescript() + " rub up against hers, ", false);
			outputText("then pull apart from her and ", false);
			//[(goddamn centaur)
			if (player.lowerBody == LOWER_BODY.CENTAUR) outputText("turn away, kneeling down to display your animalistic, musky pussy readily.", false);
			...
			//[(no centaur)
			if (player.lowerBody != LOWER_BODY.CENTAUR) outputText("You take her by the upper arms and pull her on top of you as you lie back in the sun-warmed shallows.  ", false);
			...
			if (player.lowerBody != LOWER_BODY.CENTAUR) {
				...
				//[(breasts>manly)
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText(breastDescript(0) + " and ", false);
			...
			//(squirter)
			if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) outputText("soaks her crotch with juice and ", false);
			...
			if (player.lowerBody == LOWER_BODY.CENTAUR) outputText(" on your back", false);
			//[(notits)
			else if (player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText(" on your chest", false);
...
//loss rapes:
		public function loseToAnemone():void
		{
			...
			if (player.gender == 0) {
				...
				//[(low cor)
				if (player.stats.cor < 33) outputText("You reach out instinctively to comfort her, but ", false);
				//(high cor)
			...
			if (player.lowerBody.cockSpot.count() > 0) {
				...
				if (player.cockThatFits(36) >= 0) {
					...
					//[(normal)
					if (player.lowerBody != LOWER_BODY.CENTAUR) outputText(" into your lap,", false);
					//(shitty taur)
					...
					outputText(" at you, you can see a smile in the lines around her eyes even though her mouth is locked around your " + Appearance.cockNoun(CockType.HUMAN) + ".  ", false);
					//[(if dual/multi-cox)
					if (player.lowerBody.cockSpot.count() > 1) {
						//[(cock2 doesn't exist)
						if (player.lowerBody.cockSpot.count() == 2) outputText("Your other dick rubs", false);
						//(cock2 exists)
					...
					outputText("After a grateful moment of rest as the anemone swallows your issue, her hands begin pumping once again.  Oh god!  Your " + Appearance.cockNoun(CockType.HUMAN) + " quickly returns to erectness under the renewed siege of aphrodisiac", false);
					//[(multi)
					if (player.lowerBody.cockSpot.count() > 1) {
						if (player.lowerBody.cockSpot.count() == 2) outputText(" and your other " + cockDescript(1) + " follows suit", false);
				...
				//loss, too-big male (cock > 5" width or > 48" length):
					...
					if (player.lowerBody != LOWER_BODY.CENTAUR) outputText("waist", false);
					else outputText("flank", false);
					//[(not centaur)
					if (player.lowerBody != LOWER_BODY.CENTAUR) outputText(" and pushes your shoulders down with the other", false);
					outputText(".  As she draws you backwards, you're forced to put your own ", false);
					if (player.lowerBody != LOWER_BODY.CENTAUR) outputText("hands ", false);
					else outputText("forelegs knee-", false);
					outputText("down in front of you to keep from falling face-first.  ", false);
					//[(if non-centaur)
					if (player.lowerBody != LOWER_BODY.CENTAUR) outputText("The head of your " + cockDescript(0) + " dips into the lukewarm lakewater, sending a tingle down the shaft.  ", false);
					...
					//[(if balls)
					if (player.lowerBody.balls > 0) outputText(" and your " + sackDescript() + " tightens up", false);
					...
					//[(man)
					if (player.lowerBody != LOWER_BODY.CENTAUR) outputText("clawing at your tormentor's tentacles, trying to pry them from " + sMultiCockDesc() + " by main force.", false);
					//(horse)
					...
					if (player.tallness < 48 && player.lowerBody != LOWER_BODY.CENTAUR) outputText("fly over your head, and turning behind you, you see the anemone trying to catch them with an open mouth and her tongue out.", false);
					else if (player.tallness < 84 && player.lowerBody != LOWER_BODY.CENTAUR) outputText("catch the air and rain down on both your faces, splashing quietly where they hit water.", false);
					else {
						outputText(" land right on your", false);
						//[(if breasts)
						if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText(" breasts and", false);
					...
					if (player.tallness < 48 && player.lowerBody != LOWER_BODY.CENTAUR) outputText("She looks you over, then bends down and drinks up as much of the semen floating in the water as she can find nearby.", false);
...
//'Give':
		private function giveMino():void
		{
			...
			if (player.lust < 30) {
				...
				if (player.lowerBody.cockSpot.count() > 0) outputText(cockDescript(0) + " becomes a little erect", false);
				else if (player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript(0) + " aches a bit with need", false);
...
//anal
		private function anemoneButtPlugginz():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(48) >= 0) outputText(" as you smear a dribble of pre-cum into the crack of her ass");
			...
			if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(48) >= 0) outputText("cock");
...
//[FUCK IT] (if cock fit 48, use cock; else use clit scenes)
		private function anemoneQuoteUnquoteAnal():void
		{
			...
			if (player.lowerBody.balls > 0) {
				...
				if (!player.lowerBody.cockSpot.hasCock()) outputText(".  As your [balls] swell with blood and seed, you can't help but groan; there'll be nowhere for the largess to go, and it will be with you until your body reabsorbs it - or you make an outlet");
			}
			...
			if (dick) {
				...
				if (player.lowerBody.balls > 0) outputText(".  Your lover, hopeful, watches and waits for your testicles to rise into your groin and unload their cargo");
				if (player.lowerBody.cockSpot.count() == 0) outputText("; she even tries to push them up herself with her hands, as if that would make your clit produce the semen inside them");
				outputText(".");
				if (player.lowerBody.cockSpot.hasCock()) outputText("  Your clit remains unproductive; your male orgasm squirts out uselessly around her head, mingling with the lake water.  The girl's expression upon seeing this is pained, and she tries but fails to get free to follow the arcing white ropes, sending further spasms of pleasure through your sensitive distaff staff.");
				...
				if (player.lowerBody.cockSpot.hasCock()) outputText(", pointing to the squiggles of your seed floating lazily in the lakewater,");
		...
		private function anemoneGetsLayedByBeePositor():void
		{
			...
			if (player.stats.sens >= 50 || player.isTaur()) {
				...
				if (player.eggs() > 40) {
					...
					if (player.lowerBody.cockSpot.hasCock() && player.cumQ() > 1000) outputText("and so much cum ");
...
//[Ovipositor] option with spiderbutts
		private function spiderOvipositAnAnemone():void
		{
			...
			//[(cock)
			if (player.lowerBody.cockSpot.hasCock()) outputText(", and your [armor] tightens as [eachCock] swells");
			else if (player.lowerBody.vaginaSpot.hasVagina()) outputText(", and the inside of your [armor] dampens with the lube your [vagina] leaves as it clenches around empty air");
			...
			if (player.stats.cor < 60) outputText(", even though her adorable and slightly surreal puppy dog eyes implore you otherwise");
...
//[Barrel] button in [Stash] menu (appears whenever Kidswag flag >= 1)
		public function approachAnemoneBarrel():void
		{
			...
			if (flags[FlagEnum.ANEMONE_WEAPON_ID] != 0) {
				outputText("She has " + ItemType.lookupItem(flags[FlagEnum.ANEMONE_WEAPON_ID]).longName + " sitting next to it.  ");
				//outputText("Kid A pokes her head out and smiles at you.  What do you need from her?");
				weaponT = "Take Weapon";
				weaponB = takeOutOfAnemone;
			}
			//(if babysitting)
			if (flags[FlagEnum.KID_SITTER] > 1) outputText("Kid A is away right now, helping season some of Marble's children.  If you wanted to leave or take an item, she would discover it when she got back.  ");
...
//[Tutor](only appears if Kid A is armed and present)
		private function tutorAnemoneKid():void
		{
			...
			//[(KidXP < 33)]
			if (kidAXP() < 33) {
				outputText("holding " + ItemType.lookupItem(flags[FlagEnum.ANEMONE_WEAPON_ID]).longName + " protectively across her chest.");
			}
			else outputText("taking up an attentive stance with " + ItemType.lookupItem(flags[FlagEnum.ANEMONE_WEAPON_ID]).longName + " in her hands.");
			...
			if (flags[FlagEnum.ANEMONE_WEAPON_ID] == weapons.PIPE.id ||
				...
				if (flags[FlagEnum.ANEMONE_WEAPON_ID] == weapons.PIPE.id) outputText("pipe");
			...
			//[Riding Crop]
				...
				if (player.stats.lib < 50) outputText("is");
			...
			//[Whip] or [Succubi's Whip]
				...
				if (flags[FlagEnum.ANEMONE_WEAPON_ID] == weapons.SUCWHIP.id) outputText("  The demonic enchantment chooses then to activate, and her color deepens as her lust peaks, as does your own.");
			...
			//[Black Book]
				...
				if (player.lowerBody.cockSpot.hasCock()) outputText("tighten");
				else if (player.lowerBody.vaginaSpot.hasVagina()) outputText("dampen");
			...
			//[Scarred Blade](NYI)
				...
				//[(if corr <=70)
				if (player.stats.cor <= 70) outputText("  The blade's edge flashes toward you as well, when you try to pick it up.  After a few frustrated attempts, it becomes clear that you'll have to abandon it for now.");
			...
			if (player.HP < 1) {
				...
				if (flags[FlagEnum.ANEMONE_WEAPON_ID] != weapons.S_GAUNT.id &&
						flags[FlagEnum.ANEMONE_WEAPON_ID] != weapons.H_GAUNT.id) outputText("the thump of something hitting the ground and ");
			...
			if (flags[FlagEnum.HAD_KID_A_DREAM] > 0 && kidAXP() >= 40 && player.lust > 99) {
				...
				//if KidXP >= 40 and lust > 99 after tutor and PC has only huge dicks of area >= 60 or hasn't got shit
					...
					if (player.lowerBody.cockSpot.hasCock()) outputText("something too enormous");
		...
		private function kidASex(cont:boolean = true):boolean
		{
			...
			//if KidXP >= 40 and lust > 99 after tutor and PC has wang of area <60, set that cock to x variable, also set biggest cock not yet used to y variable if multicocked
				...
				if (y >= 0 || player.lowerBody.balls > 0 || player.lowerBody.vaginaSpot.hasVagina()) {
					...
					else if (player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript(0));
					//[(if cock or sack)]
					if (y >= 0 || player.lowerBody.balls > 0) outputText(", pulling it free from your armor as well");
					...
					if (y >= 0) {
						...
						if (player.lowerBody.cockSpot.list[y].cockLength >= 36) outputText("towers over her head");
					...
					//[(sac)
					else if (player.lowerBody.balls > 0) outputText("  One hand cups below your " + ballsDescript() + " and lifts them up to her chin, where the other, formerly tracing gentle circles along your shaft, grabs a tentacle from her head and begins to tickle.");
					//[(vag)
					else if (player.lowerBody.vaginaSpot.hasVagina()) {
						outputText("  Her fingers part your labia and reveal your " + clitDescript() + ", ");
						if (player.lowerBody.vaginaSpot.list[0].clitLength >= 6) outputText("working it free as well and allowing her to tease and stroke it, cock-like, with her hands and hair.");
				...
				if (player.lowerBody.balls > 0) outputText("your balls tighten up");
			...
			//if KidXP >= 40 and lust > 99 after tutor and PC has vag and no cox
				...
				//[(if pc is loose)
				if (player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.GAPING) outputText("  You can barely feel her little shaft in your stretched cunt, but the chemical stimulation from the tentacles stroking your insides goes a long way toward making up for that.");
				...
				//[(clitsize<6)]
				if (player.lowerBody.vaginaSpot.list[0].clitLength < 6) outputText("  The little button gets rubbed by her probing hand even as she resumes thrusting, accelerating your imminent peaking.");
				else if (player.lowerBody.vaginaSpot.list[0].clitLength < 36) outputText("  The monstrous chick-stick bobs in the air as she pounds you harder; the breeze alone would be enough to arouse you further, but the anemone grabs it and begins jacking the nerve-laden stalk off like a dick, which sends your back into spasms.");
...
//if KidXP drops below threshold for sex due to bad training, no more dreams and no more sex
		public function kidADreams():void
		{
			...
			//if female:
				...
				if (player.stats.cor >= 66) outputText(" and, with a sigh, you grab her head and force it down to the mess, compelling her to lick it up.");
...
//repeatable, trigger possible when visiting 2+ Izma kids
		public function kidAWatchesSharks():void
		{
			...
			//(else KidXP < 75 and Kidweapon = White Book or Black Book)
				...
				if (flags[FlagEnum.ANEMONE_WEAPON_ID] == consumables.W__BOOK.id) outputText("blinds them with a sudden flash of white light");
