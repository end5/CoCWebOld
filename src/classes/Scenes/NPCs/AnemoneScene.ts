...
package classes.Scenes.NPCs
{
	...
	public class AnemoneScene extends BaseContent implements TimeAwareInterface
	{
		...
		public defeatAnemone():void
		{
			Render.text("", true);
			//Win by HP:
			if (monster.HP < 1) Render.text("The anemone's knees buckle and she collapses, planting her hands behind her with a splash.  You stand over her, victorious.\n\n", false);
			//win by lust:
			else Render.text("The anemone slumps down and begins masturbating, stroking her cock furiously.  You think you can detect something like desperation in her opaque eyes.  It doesn't look like she'll trouble you anymore.\n\n", false);
			if (player.lust >= 33) {
				Render.text("You could always have your way with her.  If you do, which parts do you use to do the deed?", false);
...
//anal: -requires butthole
		private victoryButtholeRape():void
		{
			spriteSelect(4);
			Render.text("", true);
			Render.text(images.showImage("anemone-getanal"), false);
			Render.text("You look over the anemone in front of you.  Your attention focuses on her blue shaft; those smaller tentacles should have plenty of pleasing venom in them as well.  Stripping off your " + player.armorName + ", you approach her and push her backwards.  Her gills slide off her breasts and float at her sides. revealing a pair of cute nipples.  You take the opportunity to stroke the shaft of her penis and rub her vagina a bit, soaking up some venom and making your hands tingle.\n\n", false);

			Render.text("Quite soon you can hardly stand your own arousal and your lover's cock is nice and hard.  Straddling the anemone, you position your " + assholeDescript() + " over her colorful shaft and gradually lower yourself towards it.  The florid crown slips into your hole, delivering the expected shock, and a gasp from behind you is accompanied by the anemone's hands moving to your hips.", false);
			//[butt hymen removal]
			if (!player.buttChange(monster.cockArea(0), true)) Render.text("  ", false);
			...
			Render.text("With your " + assholeDescript() + " heated up from the aphrodisiac already, the friction is enough to warm it to fever pitch.  Over and over you impale yourself on the girl's rod, dragging trails of venom and heat up and down your rectum.", false);
			if (player.lowerBody.cockSpot.count() > 0) {
				Render.text("  One hand involuntarily moves to your " + cockDescript(0) + " and begins stroking, smearing the copious pre-cum forced out by the prostate stimulation over " + sMultiCockDesc() + ".", false);
			}
			//[(if vag and nococks)
			else if (player.lowerBody.vaginaSpot.hasVagina()) {
				Render.text("  You lift one hand up to your " + vaginaDescript(0) + " and begin jilling yourself off.  This works to satisfy you for a while, but eventually you want more and grab a brace of tentacles floating in the water beside you, shoving them into your greedy pussy and smearing them around. This provokes a lusty moan from you and a giggle from your lover.", false);
			}
			Render.text("  As you work your " + assholeDescript() + " on the tool, something happens to push your peak closer at a startling pace...\n\n", false);

			Render.text("Your blue lover, restless now and uncontent to simply lie there anymore, begins to use her own hands and hips to pump in time with you, doubling the pace of the fuck.  The fervid intensity of her strokes doesn't leave any time for the venom to disperse before the next thrust arrives, making it feel as though your " + assholeDescript() + " is filled with one huge, hot cock that nevertheless slides in and out even as it remains motionless.  The sensation pushes you to orgasm quickly, your " + assholeDescript() + " clamping down on the anemone's penis", false);
			if (player.lowerBody.cockSpot.count() > 0) {
				Render.text(" as " + sMultiCockDesc() + " twitches and ejaculates in a ", false);
				if (player.cumQ() < 50) Render.text("squirt", false);
				else if (player.cumQ() < 250) Render.text("spray", false);
				else Render.text("torrent", false);
				Render.text(" of semen", false);
				//[(if vag and cox)
				if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and your pussy spasms", false);
			}
			else if (player.lowerBody.vaginaSpot.hasVagina()) {
				Render.text(" and your " + vaginaDescript(0) + " spasms around the tentacles and your fingers", false);
			}
			Render.text(".  The anemone must have been right on the edge with you, because after a few more thrusts in your hungry asshole she achieves her own climax and shoots several strings of cool, thick semen into you.  You collapse backward against your partner and she idly caresses your " + nippleDescript(0) + ".  After several minutes of relaxing in the warm water, you sit up and push yourself off of the anemone's limp penis, which drags a string of semen out of your " + assholeDescript() + " and prompts ", false);
			//[(dix)
			if (player.totalCocks() > 0 || player.gender == 0) Render.text("a ", false);
			else if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("another ", false);
			Render.text(" giggle from the blue girl.  Standing up, you gather your gear and blow her a kiss before you leave.  She darkens in color, her camouflage reflex causing her to 'blush' in discomfort at this display of affection.", false);
		...
		private rapeAnemoneWithDick():void
		{
			spriteSelect(4);
			Render.text("", true);
			Render.text(images.showImage("anemone-male-fuck"), false);
			if (player.cockThatFits(36) >= 0) {
				let x:number = player.cockThatFits(36);
				Render.text("Rubbing yourself through your " + player.armorName + ", you look over the anemone; your attention wanders down her torso to the blue slit between her legs", false);
				//[(lust victory)
				if (monster.lust > 99) Render.text(", which she's currently diddling with the hand she's not using to stroke her cock", false);
				Render.text(".  Unfastening your garments, you stroke " + sMultiCockDesc() + " to full hardness and approach her.  The anemone looks up at you, still somewhat befogged; then, as you stand over her, she leans forward and opens her mouth invitingly.\n\n", false);

				Render.text("You smile at how eager she is for you, but shake your head.  The anemone closes her mouth and looks at you quizzically.  <i>\"No?\"</i> she asks.  Only then does she follow your gaze down to her pussy.  The skin on her face darkens a bit as she realizes your intention... which turns out to be a blush, by the looks of the shy glance she gives you next!  <i>\"Umm.\"</i>  The anemone's fingers", false);
				//[(HP defeat)
				if (monster.HP < 1) Render.text(" move to the lips of her vagina and", false);
				Render.text(" pull apart her feathery labia, showing a velvety interior.  <i>\"Ok...\"</i> she says haltingly.  You accept the invitation in a hurry, kneeling down and holding onto her hips, then guiding your " + cockDescript(x) + " into her.\n\n", false);

				Render.text("After a moment of savoring the sensation, you push all of the way in, provoking a moan and a widening of the eyes from your partner.  ", false);
				//[(multicox)
				if (player.lowerBody.cockSpot.count() > 1) {
					Render.text("As you push all the way into her, your other dick", false);
					if (player.lowerBody.cockSpot.count() > 2) Render.text("s", false);
					Render.text(" rub", false);
					if (player.lowerBody.cockSpot.count() == 2) Render.text("s", false);
					Render.text(" up against the feelers lining her pussy.  Unexpectedly, they also contain the venomous nematocysts of her tentacles and in that single stroke " + sMultiCockDesc() + " is throbbing hard and squeezing pre-cum all over her groin.  She reaches down and plays with it", false);
					if (player.lowerBody.cockSpot.count() > 2) Render.text(", and them,", false);
					Render.text(" as you start pumping.  ", false);
				}
				Render.text("The fuck begins in earnest as you demonstrate all the techniques you know or can imagine; the anemone seems to be more impressed as you go on, cooing and moaning to goad you further while wrapping her hands and hair around your hips for chemical encouragement.  Her small tits bounce a little", false);
				//[(if PC boobs > A)
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) Render.text(" in time with your own", false);
				Render.text("; here and there one of the bounces brush her gills away, giving you a tantalizing view of nipple.", false);
				//[(if balls)
				if (player.lowerBody.balls > 0) Render.text("  As your thrusts get faster your " + sackDescript() + " starts to slap into the tickly and quite-venomous feelers fringing her pussy, getting hotter and hotter as the aphrodisiac builds in your " + ballsDescriptLight() + ".  Your body answers with swelling, causing your sack to feel bigger and tighter with every slap.", false);
				//[(if noballs and clit > 6")
				else if (player.lowerBody.vaginaSpot.list[0].clitLength >= 6) Render.text("  As your thrusts into the blue girl arouse you, your " + clitDescript() + " begins to swell with blood.  Pretty soon it's so erect that a particularly violent thrust mashes it into the feelers at the base of her labia, delivering a shock that almost makes you collapse.  The anemone, her reverie interrupted as you temporarily stop pumping, looks down.  Noticing the massive girl-cock sticking out of your " + vaginaDescript(0) + ", she reaches between her legs and gives the tip a flick, grinning with sadistic playfulness.  Your eyes cross at that, sending her into a spasm of giggling.  Irritated but aroused by the abuse of your " + clitDescript() + ", you move your thumb over her own tiny blue button and begin flicking it in revenge as you renew your pumping.", false);
				Render.text("\n\n", false);

				Render.text("The anemone's eyes roll back in her head as she reaches her climax first, hips shaking and penis squirting a glob of semen that drools down the side.  ", false);
				//[(Flexibility cat perk)
				if (player.perks.has("Flexibility")) Render.text("You lean down and take the anemone's cock in your own mouth as she continues orgasming, swallowing the cool, slippery jism -- it tastes not salty and fishy as you'd hoped but somewhat faintly like algae.  The anemone, recovering her wits a bit, looks at you blankly, as though she can't fathom why anyone would want to drink <i>her</i> ejaculate instead of the other way around.  Your eyes twinkle mirthfully in response as you suck and swallow the last of her jizz.  ", false);
				Render.text("Your own orgasm takes a bit longer, but the convulsing walls of her pussy do their best to help you arrive and the feelers along her labia writhe against your " + multiCockDescriptLight() + " in the same tempo, injecting the last of their venom.  With a deep, final thrust, " + sMultiCockDesc() + " erupts in her pussy.  ", false);
				//[(big skeet)
				if (player.cumQ() > 500) {
					Render.text("You continue to pour into her even after her convulsions stop, stretching her belly", false);
					//[(super skeet)
					if (player.cumQ() > 2000) Render.text(" to enormous dimensions.  <i>\"Ohh...\"</i> she moans, as her waist distends to over four times its former slender diameter", false);
					Render.text(".  She looks ", false);
					if (player.cumQ() < 2000) Render.text("thoroughly", false);
					else Render.text("monstrously", false);
					Render.text(" pregnant when you've finished, her little blue dick poking out below a swollen stomach... not a bad look, actually.  You get a little turned on at the idea.  ", false);
				}
				Render.text("After the last of your cum has been squeezed out, you pull your " + cockDescript(0) + " out and rinse it off in the lakewater.  You gather your gear while the anemone holds her belly and smiles placidly, staring into the sky.", false);
			...
			//Too-big male: -requires cockarea > 36
				...
				Render.text("Rubbing yourself through your " + player.armorName + ", you look over the anemone; your attention wanders down her torso to the blue slit between her legs", false);
				//[(lust victory)
				if (player.lust > 99) Render.text(", which she's currently diddling with the hand she's not using to stroke her cock", false);
				Render.text(".  Unfastening your garments, you stroke " + sMultiCockDesc() + " to full hardness and approach her.  The anemone looks up at you, still somewhat befogged; then, as you stand ", false);
				if (player.tallness > 48) Render.text("over", false);
				else Render.text("before", false);
				Render.text(" her, her eyes widen as she beholds the sheer amount of cock you have.\n\n", false);

				Render.text("You smile at how stunned she is by you, and waggle your erect " + multiCockDescriptLight() + " from side-to-side.  The anemone obediently watches it swing, like a hypnotist's pendulum; her mouth reflexively opens as all her conscious thought is temporarily sidetracked.  You push on the shaft with one hand and move the tip down relative to her body, watching bemused as the girl tries to keep her mouth in line with it until it goes too low for her neck and snaps her out of her daze.  She closes her mouth and looks at you quizzically.  <i>\"No?\"</i> she asks.  You answer by pushing forward slightly, bumping the head of your " + cockDescript(0) + " against her <i>mons pubis</i>.  The skin on her face darkens a bit as she realizes your intention... which turns out to be a blush, by the looks of the shy glance she gives you next!  <i>\"Umm.\"</i>  She gives it a moment of concerned study before her natural instincts take over and the anemone's fingers", false);
				//[(HP defeat)
				if (monster.HP < 1) Render.text(" move to the lips of her vagina and", false);
				Render.text(" pull apart her feathery labia, showing a velvety interior.  <i>\"Ok...\"</i> she says, clearly unsure of the wisdom of this.  You motion for her to lay back and lift her pussy as high as possible to reduce the angle, then attempt to guide your " + cockDescript(0) + " into her.", false);
				//[(PC height > 7' and non-centaur)
				if (player.tallness > 84) Render.text("  Finally, after having gone so far as to kneel down to bring yourself in line, you begin pushing your way in.", false);
				Render.text("\n\n", false);

				Render.text("The first few inches are slow going, as you try to stretch the blue girl's roughly human-sized pussy around your superhuman girth.  She sports a worried expression as you struggle to push the crown of your " + Appearance.cockNoun(CockType.HUMAN) + " in without bending your shaft.  The girl's body proves more elastic than you'd expected, though; with each shove her cunt takes more width without suffering any obvious ill effect.  Eventually you get the head of your " + cockDescript(0) + " into her, and you give an experimental thrust to test her depths. You slide surprisingly far into her, your huge " + Appearance.cockNoun(CockType.HUMAN) + " making a bump in her pelvis; the anemone has abandoned her worry and is blissfully tweaking her nipples.  Taking the anemone's legs under your arms, you begin pumping at her, savoring the sensation of having at least part of your unwieldy tool inside someone.  Your blue partner gives out cute moans as you fuck her, prompting a grin from you, but most of your attention is focused on maintaining the angle of your " + cockDescript(0) + ".  As you focus on the thrusts, you gradually notice the bump sliding further up her pelvis on each push... it's nearly up to her stomach now!  You quickly make up your mind to test her unusual biology, choking up on your grip of her legs and pushing hard with each thrust in.  The anemone is now tracking the progress of the bump representing your " + Appearance.cockNoun(CockType.HUMAN) + " as intently as you are; as your head pushes up her chest between her breasts she presses them together against the ridge, massaging them against the shaft through her skin.", false);
				//[(cocklength > 60")
				if (player.lowerBody.cockSpot.list[0].cockLength > 60) Render.text("  The imagery and the stimulation inspire you to increase your efforts, and you push even harder into her until your " + cockDescript(0) + " slides its way into her throat, expanding her slender neck to twice the normal width.  The anemone's mouth gapes open reflexively as if to gag as the pressure forces her head back, but she doesn't stop moaning or rubbing herself against you.  This seems to be as deep as you can get; the tip of your cock is now right up against the base of her chin.", false);
				Render.text("\n\n", false);

				Render.text("Inordinately pleased at having gotten your " + cockDescript(0) + " so far in, you begin thrusting grandly, alternating huge back-and-forth pumps with hip gyrations that press you against the sides of your partner.  The anemone", false);
				//[(dick > 60")
				if (player.lowerBody.cockSpot.list[0].cockLength > 60) Render.text(", despite being unable to look anywhere except over her head thanks to the giant ridge running up the front of her body,", false);
				Render.text(" has clearly been enjoying the treatment; soon she begins twitching in orgasm and her pussy spasms against the base of your " + Appearance.cockNoun(CockType.HUMAN) + ", flicking its venomous feelers into it, as she kneads her breasts in her hands.  Her body attempts to convulse, bending your " + cockDescript(0) + " slightly, while her neglected dick gives a little squirt of pearly semen which lands on the raised line in the center of her body and makes a little trail as it rolls down.  The pressure of her twisting and the sensation of her pussy lapping at your shaft with aphrodisiac is enough to send you over the edge as well.  Your " + cockDescript(0) + " twitches as you unload into your blue partner", false);
				//[(big skeet)
				if (player.cumQ() > 500) Render.text(" until it puffs out her cheeks", false);
				Render.text(".", false);
				//[(mega skeet)
				if (player.cumQ() > 1500) Render.text("  Eventually she can't hold in the sheer volume of your ejaculate, and it erupts from her mouth in a white spray.  Spurt after spurt goes into and then out of her, dribbling out of her slack mouth and down her face until her hair is covered with seed.", false);
				Render.text("  She takes the opportunity to squeeze along the length of your cock, pushing out as much semen as you have to offer as she moans in orgiastic delight at the fluid injection.", false);
				Render.text("\n\n", false);

				Render.text("Eventually both you and she are spent and limp, and you draw your " + cockDescript(0) + " out of her, making an extended, wet sucking noise.  As you pull up your gear and make your way up the beach, ", false);
				//[(normal/big skeet)
				if (player.cumQ() < 500) Render.text("her hands are still dazedly playing with the space between her breasts where your cock used to rest.", false);
				//[(mega skeet)
				else Render.text("she continues to sputter and cough up bubbles of your spunk.", false);
				//(pass 1 hour, reset lust to min or min+10 if big or greater skeet)
				player.orgasm();
				cleanupAfterCombat();
			}
...
//using pussy:
		private rapeAnemoneWithPussy():void
		{
			spriteSelect(4);
			Render.text("", true);
			Render.text(images.showImage("anemone-female-fuck"), false);
			Render.text("As you review your handiwork, the stirrings in your feminine side focus your attention on the anemone's penis.  Those smaller tentacles on it should have plenty of pleasing venom in them as well.  You make up your mind to put them to use for you.\n\n", false);

			Render.text("The anemone looks vacantly up at you as you approach.  Reaching forward, you take her cock in your hand", false);
			//[(lust victory)
			if (monster.lust > 99) Render.text(" after brushing hers aside", false);
			Render.text(" and begin to fondle the crown, with its slippery tentacles.  As expected, her venom flows into your hand, imparting a sensation of heat that slides up your arm and diffuses into a gentle warmth.  After a few rubs, you lean down and carefully take her penis into your mouth.  It tastes of the lakewater and heats your mouth as it did your hand; ", false);
			//[(HP victory)
			if (monster.HP < 1) Render.text("you can feel it harden as ", false);
			Render.text("you caress it with your tongue before pulling it out and giving it a squeeze.  The blue girl shivers as a drop of pre-cum is forced out.\n\n", false);

			Render.text("Next, you take the time to strip off your " + player.armorName + ", making sure to give a good show; the anemone entertains herself by stroking her erect prick and smearing around the pre-cum, grinning as she watches you.", false);
			//[(breastrow0>C-cup)
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) Render.text("  You give special attention to the presentation of your " + breastDescript(0) + ", removing your top with tantalizing slowness, letting each breast slip out and hang between you like fruit ripe to be taken, then making sure to rub them seductively to arouse the both of you further.", false);
			//(hipsize=girly or better)
			if (player.lowerBody.hipRating > 6) Render.text("  You make good use of your " + hipDescript() + ", too, giving a little shimmy to show off your pronounced curves.", false);
			Render.text("  By the time you're finished, the anemone's crotch glistens with fluid from both her sexes; it's probably as wet as it was when she was underwater.  You lean into the anemone and give her a deep kiss, ", false);
			//[(breast0>C)
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) Render.text("making sure to let your " + player.allBreastsDescript() + " rub up against hers, ", false);
			Render.text("then pull apart from her and ", false);
			//[(goddamn centaur)
			if (player.lowerBody == LOWER_BODY.CENTAUR) Render.text("turn away, kneeling down to display your animalistic, musky pussy readily.", false);
			else Render.text("recline back on your " + player.legs() + ". Spreading your thighs, you reach down with two fingers and pull apart your " + vaginaDescript(0) + " welcomingly; it's the last act in your sexual performance.", false);
			Render.text("\n\n", false);

			Render.text("The anemone wastes no time in assessing your intention and crawls forward onto you, returning your kiss with equal passion.  ", false);
			//[(no centaur)
			if (player.lowerBody != LOWER_BODY.CENTAUR) Render.text("You take her by the upper arms and pull her on top of you as you lie back in the sun-warmed shallows.  ", false);
			Render.text("Her hair drapes over you as she lines her penis up with your " + vaginaDescript(0) + ", delivering heat to your body, but this is dwarfed by the sensation of her entry as she pushes her cock in for the first time.  ", false);
			player.cuntChange(monster.cockArea(0), true);
			Render.text("The penetration combines with the aphrodisiac being injected straight into your hungry pussy to produce a feeling like euphoria.  Unable to focus your thoughts any longer, you allow the anemone to take the lead as she begins pumping into you, coating your labia with a mixture of her pre-cum and your own secretion.  Soon you're moaning lustily with complete disregard for anything except the pleasure between you as your lover ups the pace; ", false);
			//[(biped)
			if (player.lowerBody != LOWER_BODY.CENTAUR) {
				Render.text("as she thrusts hard and fast, her hair whips back and forth over your ", false);
				//[(breasts>manly)
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) Render.text(breastDescript(0) + " and ", false);
				Render.text(nippleDescript(0) + "s,", false);
			}
			//(centaur)
			else {
				Render.text("as she pushes deeply into your cunt, her hair flies forward past your upper body, brushing along your skin.  On one pass you grab some and use it as a pleasure tool, rubbing it vigorously on your ", false);
				//[(breasts>manly)
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) Render.text(breastDescript(0) + " and ", false);
				Render.text(nippleDescript(0) + "s,", false);
			}
			Render.text(" spreading heat along your chest to nearly match your vagina's.\n\n", false);

			Render.text("The overwhelming sensations drive you over the edge and your " + vaginaDescript(0) + " contracts hungrily around the heat radiating from the anemone's cock.  As your orgasming pussy ", false);
			//(squirter)
			if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) Render.text("soaks her crotch with juice and ", false);
			Render.text("wrings her penis, the blue shaft responds enthusiastically; she pushes deeply into you as it begins spasming and squirting its load.  Your partner's mouth hangs open as you squeeze the cum out of her; with all her muscle control taken away, her head hangs limply", false);
			if (player.lowerBody == LOWER_BODY.CENTAUR) Render.text(" on your back", false);
			//[(notits)
			else if (player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) Render.text(" on your chest", false);
			else Render.text(" between your breasts", false);
			Render.text(" as she gives up several streams of semen into your womb.  Finally, her cock empties out with a few last spurts; she came quite a lot and your womanhood feels pleasantly filled.  The two of you lie there for some time before she can recover enough to slip out of you.  When she does, a string of semen drips out of your abused pussy and mixes with the water below.\n\n", false);

			Render.text("Having scratched your itch, you give her another kiss, catching her by surprise.  She smiles shyly at you as you gather up your clothes, then slips into the water as you start to dress again.", false);
			anemonePreg();
...
//loss rapes:
		public loseToAnemone():void
		{
			...
			//loss, neuter:
			if (player.gender == 0) {
				Render.text("Shivering, you slump before the anemone and begin trying to remove your " + player.armorName + ".  She claps and peals happily, splashing over to you.  Pushing your trembling hands out of the way, she unfastens your garments and pulls them free of you... and then stops.  You look the girl in the face uncomprehendingly and she answers your gaze with a look of equal confusion.  Your head turns to follow her as she looks you up and down and even makes a circle around you, inspecting every inch and chewing her lip anxiously.", false);
				//[(lactating)
				if (player.biggestLactation() > 1) Render.text("  For a moment the examination stops at the dribble of milk leaking from your " + nippleDescript(0) + "s.  With one finger she collects a bit and tastes it, only to grimace and stick her tongue out.", false);
				Render.text("  Back at the front, the anemone motions questioningly toward your ornament-free groin with open palms.  You follow her gesture down to your spartan nethers, then look back up.  Her bottom lip is quivering and -- yes, it looks like water is beginning to well up in her eyes.\n\n", false);

				Render.text("Hurriedly you begin to compose an explanation for your anatomy, and you get as far as telling her that you have no genitalia thanks to events since your arrival before she bursts into tears.  ", false);
				//[(low cor)
				if (player.stats.cor < 33) Render.text("You reach out instinctively to comfort her, but ", false);
				//(high cor)
				else Render.text("You smirk, amused by the turnabout, until ", false);
				Render.text("the anemone lashes out with a slap that knocks the expression off your face and makes your eardrums sing.  <i>\"Dumb!\"</i> she shouts, turning sharply; her tentacles lash past you as she about-faces.  She dives down below the surface and kicks water into your face spitefully as she swims away.  You sputter and rub your jaw a bit, then stand up and walk dizzily back to camp.", false);
			...
			if (player.lowerBody.cockSpot.count() > 0) {
				...
				if (player.cockThatFits(36) >= 0) {
					...
					if (player.cumQ() < 50) Render.text("A droplet", false);
					else if (player.cumQ() < 250) Render.text("A squirt", false);
					else Render.text("A steady flow", false);
					Render.text(" of pre-cum follows up the inside in the wake of her stroke.  She touches her finger to the tip of your sensitive urethra and draws it away, stretching a string of your fluid through the air.  Putting the finger in her mouth, she savors the taste of you; the string manages to transfer to her bottom lip before she breaks it with a flick of her tongue.\n\n", false);

					Render.text("She pushes you back on your haunches and leans over your groin.  Her hair-tentacles slither forward over her shoulders and drop", false);
					//[(normal)
					if (player.lowerBody != LOWER_BODY.CENTAUR) Render.text(" into your lap,", false);
					//(shitty taur)
					else Render.text(" downwards, onto your hams,", false);
					Render.text(" delivering lances of venom into your lower body.  The tingle warms your groin and more pre-cum leaks out of " + sMultiCockDesc() + ".  Her eyes lock onto a glistening dollop and she drops down quickly, enfolding the head of your " + cockDescript(x) + " in her cool mouth.  Her tongue dances around the crown of your " + Appearance.cockNoun(CockType.HUMAN) + ", relieving it of the sticky pre.  Looking ", false);
					if (player.tallness > 48) Render.text("up", false);
					else Render.text("down", false);
					Render.text(" at you, you can see a smile in the lines around her eyes even though her mouth is locked around your " + Appearance.cockNoun(CockType.HUMAN) + ".  ", false);
					//[(if dual/multi-cox)
					if (player.lowerBody.cockSpot.count() > 1) {
						//[(cock2 doesn't exist)
						if (player.lowerBody.cockSpot.count() == 2) Render.text("Your other dick rubs", false);
						//(cock2 exists)
						else Render.text("The rest of your " + multiCockDescriptLight() + " rub", false);
						Render.text(" against her cheek, smearing slick wetness on her blue face.", false);
					}
					Render.text("\n\n", false);

					Render.text("Her hands come up from the water and push two sheaves of her long, dangling hair into your " + multiCockDescriptLight() + ".  Wrapping these bundles of tentacles around your " + cockDescript(x) + ", she clasps them in place with one hand and begins sliding them up and down your length.  Your " + Appearance.cockNoun(CockType.HUMAN) + " begins feeling hotter and hotter from the injections and the friction of her hair, secreting more pre-cum into her obliging mouth.", false);
					//[(if vag)
					if (player.lowerBody.vaginaSpot.hasVagina()) {
						Render.text("  Her other hand slips", false);
						if (player.vaginalCapacity() < 15) Render.text(" a few fingers", false);
						else if (player.vaginalCapacity() < 30) Render.text(" partway", false);
						else Render.text(" all the way", false);
						Render.text(" into your " + vaginaDescript(0) + ", sending a tingle through your lower lips and exposing your clitoris.", false);
						//[(if clit > 5")
						if (player.lowerBody.vaginaSpot.list[0].clitLength > 5) Render.text("  Having achieved this, she pulls her hand out and wraps another bundle of tentacles around your " + clitDescript() + ", then begins jerking it off in time to her efforts on your " + Appearance.cockNoun(CockType.HUMAN) + ".  Your eyes roll back in your head and your mouth gapes involuntarily at the rough stimulation of your swollen chick-stick.", false);
					}
					Render.text("\n\n", false);

					Render.text("The heat rubbing on your cock", false);
					if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and clit", false);
					Render.text(" quickly gets to you, and the first orgasm begins to work its way up your " + multiCockDescriptLight() + ".  Your " + cockDescript(x) + " lets fly into the anemone girl's mouth", false);
					//[(big skeet)
					if (player.cumQ() > 500) Render.text(", bowing out her cheeks", false);
					//[(cray-cray skeet)
					if (player.cumQ() > 2000) Render.text(" and squirting out her nose in thick ribbons", false);
					//[(multi-dix)
					if (player.totalCocks() > 1) {
						Render.text(" as ", false);
						//[(dick2 = no)
						if (player.totalCocks() == 2) Render.text("your cocks shoot", false);
						else Render.text("the rest of your " + multiCockDescriptLight() + " shoot", false);
						Render.text(" all over her face and hair, ", false);
						//[(small skeet)
						if (player.cumQ() < 500) Render.text("drawing a pattern like a musical score on her blue skin", false);
						//(big skeet)
						else if (player.cumQ() < 2000) Render.text("painting her skin white as she flinches and shuts her eyes tightly", false);
						//(cray-cray skeet)
						else Render.text("whitewashing her entire upper body and running off until a fan of milky color spreads through the water around you", false);
					}
					Render.text(".  The anemone swallows greedily as she pumps each stroke into her mouth", false);
					//[(big or > skeet)
					if (player.cumQ() >= 500) Render.text(", her taut blue belly distending as it fills", false);
					Render.text(".\n\n", false);

					Render.text("After a grateful moment of rest as the anemone swallows your issue, her hands begin pumping once again.  Oh god!  Your " + Appearance.cockNoun(CockType.HUMAN) + " quickly returns to erectness under the renewed siege of aphrodisiac", false);
					//[(multi)
					if (player.lowerBody.cockSpot.count() > 1) {
						if (player.lowerBody.cockSpot.count() == 2) Render.text(" and your other " + cockDescript(1) + " follows suit", false);
						else Render.text(" and your other pricks follow suit", false);
					}
					Render.text(".  The blue girl continues to stroke your privates with her tentacle hair, flicking your urethra with her tongue, until you've come twice more.  Nor does she display any intention of stopping there, but mercifully you black out and collapse into the water.  Half-frowning, the anemone shrugs and pushes your insensible form up onto the sandy shore.", false);
				...
				//loss, too-big male (cock > 5" width or > 48" length):
					...
					//[(big skeet)
					if (player.cumQ() > 500) Render.text(" in a steady stream", false);
					Render.text(".\n\n", false);

					Render.text("Almost reverently, she caresses the shaft of your " + cockDescript(0) + ", stroking lightly up its enormous length.  She pulls it down to her eye level, inspecting the head from several angles.  Tentatively, she opens her mouth and pulls your " + Appearance.cockNoun(CockType.HUMAN) + " into it, trying to fit your expansive dickflesh into a hole that even to your lust-crazed mind looks much too small.  Despite her best efforts, she can't get more than the crown past her lips, though the reflexive motions of her tongue poking around and inside the opening make you shiver and push out more pre-cum.  The anemone eventually pops your " + cockDescript(0) + " out of her mouth and frowns in frustration.  After a few seconds, she seems to reach a decision.  Moving your shaft out of the way, she walks around behind you.  She places one hand on your ", false);
					if (player.lowerBody != LOWER_BODY.CENTAUR) Render.text("waist", false);
					else Render.text("flank", false);
					//[(not centaur)
					if (player.lowerBody != LOWER_BODY.CENTAUR) Render.text(" and pushes your shoulders down with the other", false);
					Render.text(".  As she draws you backwards, you're forced to put your own ", false);
					if (player.lowerBody != LOWER_BODY.CENTAUR) Render.text("hands ", false);
					else Render.text("forelegs knee-", false);
					Render.text("down in front of you to keep from falling face-first.  ", false);
					//[(if non-centaur)
					if (player.lowerBody != LOWER_BODY.CENTAUR) Render.text("The head of your " + cockDescript(0) + " dips into the lukewarm lakewater, sending a tingle down the shaft.  ", false);
					Render.text("Behind you, the anemone has taken her blue, tentacled penis into her hand and is stroking it and fondling the tip, forcing her own pre-cum out and smearing it along her length.  Satisfied with its slipperiness, she edges forward until her cock is resting on your " + buttDescript() + ".  Drawing her hips back, she lines it up with your " + assholeDescript() + ", then thrusts forward while pulling back on your waist.  The wriggly feelers slip past your butthole and light up your insides with her potent venom.", false);
					player.buttChange(monster.cockArea(0), true);
					Render.text("\n\n", false);

					Render.text("Taking a moment to transfer her now-free hand to your other hip, the anemone girl then begins to pump her stiff pecker into your " + assholeDescript() + ", pausing after every few strokes to gyrate her hips a bit, massaging your prostate with her feelers and smearing venom into it.  The stimulation brings you to your limit in minutes; your dick twitches spastically", false);
					//[(if balls)
					if (player.lowerBody.balls > 0) Render.text(" and your " + sackDescript() + " tightens up", false);
					Render.text(".  This doesn't escape your blue lover's notice, and she quickly stops pumping.  Left at the edge of orgasm, you panic and look over your shoulder at her.  Judging by her grinning demeanour, she seems to be up to something diabolical.  You stare at her confusedly until you feel a new heat at the base of your " + multiCockDescriptLight() + ". Glancing down, you see that her tentacle-hair has wrapped around " + sMultiCockDesc() + " and is squeezing tightly!  Pleased with the arrangement, the anemone begins pumping and rubbing your prostate again, spreading new warmth through your " + assholeDescript() + ".  Your delayed climax finally arrives, but the <i>de facto</i> cockring", false);
					if (player.lowerBody.cockSpot.count() > 1) Render.text("s", false);
					Render.text(" prevent");
					if (player.lowerBody.cockSpot.count() == 1) Render.text("s");
					Render.text(" any semen from escaping!  The sensations swell and fade as your orgasm passes fruitlessly, your blue partner fucking away as merrily as ever.\n\n", false);

					Render.text("For nearly an hour the anemone continues her performance, even going so far as to caress your swollen " + multiCockDescriptLight() + " with her unoccupied tentacles.  Several more orgasms arrive and desert you without bringing any relief from the pressure on your ", false);
					//[(if balls)
					if (player.lowerBody.balls > 0) Render.text(ballsDescriptLight() + " and ", false);
					Render.text(multiCockDescriptLight() + ".  Eventually you get to the point where you can't take it anymore, and when you feel the next orgasm drawing close you straighten up and begin ", false);
					//[(man)
					if (player.lowerBody != LOWER_BODY.CENTAUR) Render.text("clawing at your tormentor's tentacles, trying to pry them from " + sMultiCockDesc() + " by main force.", false);
					//(horse)
					else Render.text("bucking and stamping the ground, wanting to shake the tentacles loose but unable to reach them with your hands.", false);
					Render.text("  Looking a bit irritated that you want to bring her fun to an end, the anemone nevertheless relents and releases her visegrip on your " + multiCockDescriptLight() + ".  As the joy of seeing the way to your release cleared overtakes you, the anemone avails herself of your distraction to grab your arms and pull you toward her while pushing your " + player.legs() + " out from under you.  The two of you fall backward into the shallow water as " + sMultiCockDesc() + " begins unloading its immense backup of semen in a high arc.  The ", false);
					//[(skeet amount)
					if (player.cumQ() < 500) Render.text("strings", false);
					else if (player.cumQ() < 2000) Render.text("streams", false);
					else Render.text("gouts", false);
					Render.text(" of jism ", false);
					//[(height <4' and non-horse)
					if (player.tallness < 48 && player.lowerBody != LOWER_BODY.CENTAUR) Render.text("fly over your head, and turning behind you, you see the anemone trying to catch them with an open mouth and her tongue out.", false);
					else if (player.tallness < 84 && player.lowerBody != LOWER_BODY.CENTAUR) Render.text("catch the air and rain down on both your faces, splashing quietly where they hit water.", false);
					else {
						Render.text(" land right on your", false);
						//[(if breasts)
						if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) Render.text(" breasts and", false);
						Render.text(" face.  You hear the anemone giggling as you flinch from the white shower.", false);
					}
					Render.text("  After several minutes of climax with you shooting more jism than you thought possible and the anemone banging out an accompaniment on your " + assholeDescript() + ", you finally begin to wind down.  The anemone, clearly turned on by the impressive amount of ejaculate, unloads her own blue cock into your asshole.  Her semen, lower in temperature than yours, forms a little pocket of cool inside your " + buttDescript() + ".  She idly swishes her tentacles in the", false);
					//[(big skeet)
					if (player.cumQ() >= 500) Render.text(" semen-colored", false);
					Render.text(" water around her as you push out your last load and slip into a doze.\n\n", false);

					Render.text("Pushing your inert form off of her dick, she slips out from under you and sits up beside.  ", false);
					//[(height <4' non-centaur)
					if (player.tallness < 48 && player.lowerBody != LOWER_BODY.CENTAUR) Render.text("She looks you over, then bends down and drinks up as much of the semen floating in the water as she can find nearby.", false);
					else Render.text("She leans over you and begins licking the semen off your body, not stopping until you're clean (if slightly sticky).", false);
					Render.text("  Having fed, she grins mischievously and grabs your " + cockDescript(0) + ", then tows your floating body to the shoreline with it.  She rolls you onto the sand and then swims for deep water, vanishing.", false);
			...
			//loss rape, vaginal (only full females):
				...
				Render.text(images.showImage("anemone-female-fuck"), false);
				Render.text("Shivering, you fall to your knees before the anemone and begin trying to remove your " + player.armorName + ".  She claps and peals happily, splashing over to you.  Pushing your trembling hands out of the way, she unfastens your garments and pulls them free of you, but her bright expression dims a bit when she sees only your " + vaginaDescript(0) + ".", false);
				//[(lactation)
				if (player.biggestLactation() > 1) Render.text("  For a moment it brightens again when she notices the dribble of milk leaking from your " + nippleDescript(0) + "s. With one finger she collects a bit and tastes it, only to grimace and stick her tongue out.", false);
				Render.text("  <i>\"No food...\"</i> she muses, disappointment playing smally over her face.  You look up at her, frowning sympathetically.  She thinks for a minute, staring at your crotch, and then assumes a rakish smile", false);
				if (player.tallness < 48) Render.text(", pulling you upright", false);
				Render.text(".\n\n", false);
...
//'Give':
		private giveMino():void
		{
			...
			Render.text("You nod at the girl and she smiles and responds with a very quiet \"<i>Yay.</i>\"  As you pick up the rest of your stuff, she takes the top off of the bottle and chugs it like a champ, without even stopping to breathe.  Her eyes widen a bit as the drug hits her system, then narrow into a heavy-lidded stare.  Dropping the bottle with a splash, she falls to her knees with another.  She looks at you and licks her lips as she begins playing with her nipples. Obviously, she's feelin' good.  ", false);
			//[(lust<30)
			if (player.lust < 30) {
				Render.text("Watching as her fondling devolves into outright masturbation, your own ", false);
				if (player.lowerBody.cockSpot.count() > 0) Render.text(cockDescript(0) + " becomes a little erect", false);
				else if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(vaginaDescript(0) + " aches a bit with need", false);
				else Render.text(assholeDescript() + " begins to tingle with want", false);
				Render.text(".  You shake off the feeling and head back to camp, leaving her to her fun.", false);
			}
			//(lust>30)
			else {
				...
				Render.text("As her fondling devolves into genuine masturbation you realize this would be a good opportunity to take care of your own lusts.  If you do, how will you do it?", false);
				let cockRape:Function =null;
				let vaginaRape:Function =null;
				//Normal male: -requires dick of area < 36
				if (player.lowerBody.cockSpot.count() > 0) cockRape = rapeAnemoneWithDick;
...
//anal
		private anemoneButtPlugginz():void
		{
			...
			//victory sex choice for males with cock fit 48 or females with clit >7": "her anus"
			//(change "If you do, which of your parts" to "If you do, which part" in pre-sex choice menu)
			Render.text("Imagining your climax already, you look over the anemone.  Your gaze lingers on her breasts; she sticks them out enticingly, trying to catch your interest");
			if (monster.lust > 99) Render.text(" as she plays with herself");
			Render.text(".  Nice, but not what you're looking for...  ");
			if (!player.isTaur()) {
				Render.text("Opening your [armor] a bit, you stroke ");
				if (player.lowerBody.cockSpot.hasCock()) Render.text("[oneCock]");
				else Render.text("your " + vaginaDescript(0));
				Render.text(" as y");
			}
			else Render.text("Y");
			Render.text("ou circle around behind her.  The anemone looks over her shoulder at you as you size her up.  There... that's what you wanted to see.  Tilting the girl forward with a hand on her shoulder, you lower yourself to get a better look at her backside.");
			...
			if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(48) >= 0) {
				if (!player.isTaur()) Render.text("you fish your [cockFit 48] out of your garments and rub it");
				else Render.text("you rub your [cockFit 48]");
				Render.text(" between the smooth blue curves");
			}
			else {
				if (!player.isTaur()) Render.text("you finger your pussy");
				else Render.text("you imagine the tightness of her hole");
				Render.text(" until your " + clitDescript() + " starts to fill with blood and peeks from its hood");
			}
			Render.text(", prompting a giggle from her");
			if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(48) >= 0) Render.text(" as you smear a dribble of pre-cum into the crack of her ass");
			Render.text(".  Putting her hands down, she tries to angle her rear up to bring you in line with her pussy; you give her a harder smack and force her back down, much to her confusion.  The blue profile appears over her shoulder again, this time with a worried expression.  Squeezing her ass once with open palms, you lean down and plant a kiss on it. A nervous titter issues from the blue girl as you pull open her cheeks.");
			...
			if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(48) >= 0) Render.text("cock");
			else Render.text("clit");
			Render.text(" already drooping indolently toward the water.  She turns and looks at you out of the corner of her eye, her face a rictus of suspense.");
...
//[FUCK IT] (if cock fit 48, use cock; else use clit scenes)
		private anemoneQuoteUnquoteAnal():void
		{
			...
			let x: number = 0;
			Render.text(images.showImage("anemone-bj"), false);
			if (dick) x = player.cockThatFits(48);
			Render.text("You're in the mood for anal and anal you shall have.  ");

			if (!player.isTaur()) {
				Render.text("Ever a purist, you stroke your ");
				if (dick) Render.text(cockDescript(x));
				else Render.text(clitDescript());
				Render.text(" until it protrudes from the hole in your " + player.armorName + ", returned to full erectness.  ");
			}
			Render.text("The anemone regards you intently as you approach her and ");
			if (!player.isTaur()) Render.text("seize her head");
			else {
				Render.text("mount her head");
				if (!dick) Render.text(", angling your clit forward along your stomach by pinning it against the anemone's forehead");
			}
			Render.text("; her hair delivers tingles of venom where it brushes your skin even as her own hands dawdle playfully along your " + player.skin() + ".  Without another word, you force her head into your groin as you ram your distended ");
			if (!dick) Render.text("chick-");
			Render.text("prick into her eager face.");

			Render.text("\n\nHer moist mouth welcomes the rapid advent your ");
			if (dick) Render.text("dick");
			else Render.text("clit");
			Render.text(" first with surprise, then with relish.  As you slide past her lips, your ");
			if (dick) Render.text("[cockHead " + (x + 1) + "]");
			else Render.text("tip");
			Render.text(" is embraced by the rippling walls of her throat, already trying to milk you for spunk");
			if (!dick) Render.text(" that won't come");
			Render.text(".  The tightness seems to adjust to the ");
			if (dick) Render.text("girth of your prick");
			else Render.text("girl-th of your swollen button");
			Render.text("; the anemone looks up at you with ");
			if (player.isTaur()) Render.text("unseen ");
			Render.text("eyes a-twinkle");
			if (player.lowerBody.balls > 0) {
				Render.text(", and her hair reaches forward to caress your [sack], delivering lancets of venom through the thin skin that send your arousal, and your production, into overdrive");
				if (!player.lowerBody.cockSpot.hasCock()) Render.text(".  As your [balls] swell with blood and seed, you can't help but groan; there'll be nowhere for the largess to go, and it will be with you until your body reabsorbs it - or you make an outlet");
			}
			...
			Render.text(" suddenly, stroking vigorously even as it smears a burning wave of arousal along your length.  \"<i>F-fuck,</i>\" you moan, pushed beyond your control by the coup.");
			if (dick) {
				Render.text("  Your [cockFit 48] begins to ejaculate, filling the blue girl's mouth with your seed; she sucks it down greedily, swallowing every drop.");
				if (player.cumQ() >= 1000) Render.text("  You push so much cum into her that her belly actually rounds from the volume, transforming from a sleek, flat midriff into a barely-contained ball of sloshing liquid");
				if (player.cumQ() >= 2000) Render.text("; even when her stomach skin can stretch no further, your body won't stop filling her, and the bulge expands up her esophagus, pushing out her chest and throat until she's full to the brim and rather reminiscent of a big, blue pear");
				...
				if (player.cumQ() >= 1000) Render.text("bobbing roundly in the water as she tries to make her way from the shallows, ");
				Render.text("and head back to camp.");
			}
			else {
				Render.text("  Your clit, packed with nerves, shivers and sets off your orgasm as she strokes, and you bury it into her throat as your head rolls back.  The anemone flinches, unnoticed by you, as your vagina ");
				if (player.wetness() < 4) Render.text("drools its lube along your length.  ");
				else Render.text("squirts obscenely, coating your length and her face with your female orgasm.  ");
				Render.text("Your body shivers as near-painful sensations wrack you, engulfed in the anemone's questing passage");
				if (player.lowerBody.balls > 0) Render.text(".  Your lover, hopeful, watches and waits for your testicles to rise into your groin and unload their cargo");
				if (player.lowerBody.cockSpot.count() == 0) Render.text("; she even tries to push them up herself with her hands, as if that would make your clit produce the semen inside them");
				Render.text(".");
				if (player.lowerBody.cockSpot.hasCock()) Render.text("  Your clit remains unproductive; your male orgasm squirts out uselessly around her head, mingling with the lake water.  The girl's expression upon seeing this is pained, and she tries but fails to get free to follow the arcing white ropes, sending further spasms of pleasure through your sensitive distaff staff.");
				Render.text("  Eventually your body calms enough to respond to your impulses, and you carefully draw your [clit] out of your lover's throat.");

				Render.text("\n\n\"<i>No food,</i>\" she whines, pouting.  Negligent and uncaring, you shake your head");
				if (player.lowerBody.cockSpot.hasCock()) Render.text(", pointing to the squiggles of your seed floating lazily in the lakewater,");
				Render.text(" and leave her.");
			}
...
//[Hotdog!]
		private hotdogTheAnemone():void
		{
			...
			Render.text("Well... it's the spirit of the thing that counts, right?  That blue butt still does look pretty tempting.  You force the anemone forward again and squeeze them together; she giggles and tries once more to push her vagina toward you, but you push it down again and jam your ");
			if (dick) Render.text("[cockFit 48]");
			else Render.text("[clit]");
			Render.text(" between her small, round cheeks, seizing one in each hand and forcing them to sleeve as much of your length as possible.  The girl looks back at you, her face a picture of confusion, but you do not even care.  Her cool ass feels otherworldly as you thrust through it, poking your tip out of the crack and then pulling back again; ");
			if (dick) Render.text("pre-cum");
			else Render.text("your juice");
			Render.text(" drools onto her at one end of your motion, filling your little canyon of fun with a river of hot lubrication that you smear liberally throughout.");
			Render.text("\n\n\"<i>Hey,</i>\" you call, \"<i>you could be helping.  Use your tentacles or something.</i>\"");

			Render.text("\n\nThe anemone, ");
			if (dick) Render.text("excited by the feel and sight of your pre-cum");
			else Render.text("looking almost bored");
			Render.text(", obliges and snakes several toward you; they stop just inside the range of your furthest stroke, flitting tantalizingly as if begging you to push into them.  Wary at first, you do so and they gently caress ");
			if (dick) Render.text("your [cockHead " + (x + 1) + "]");
			else Render.text("the nerve-packed end of your [clit]");
			Render.text(", responding almost autonomously to wrap around it.  The grip is not enough to keep you from pulling back out, though, and they relinquish your shaft, leaving the tip covered in a warm lattice of venom that soaks through you.  A shudder wracks you, and you eagerly push in again, sliding first into her cheeks, then down the slick passageway made by your lube, and finally into the body-heating embrace of her tentacles.  As your ");
			if (dick) Render.text("now-throbbing prick");
			else Render.text("swollen, red clit");
			Render.text(" wicks more of her venom into you, your hips begin to disobey your will, thrusting faster with each dose and pushing the anemone's face into the lakebed; elbows crooked and fingers splayed, she still can't raise her head against the vigor of your onslaught.");

			Render.text("\n\nBefore you can rub the very skin off your ");
			if (dick) Render.text("cock");
			else Render.text("clit");
			Render.text(", you come.  Your body shakes and you nearly fall atop your lover; ");
			if (dick) {
				Render.text("your swollen, painfully hard prick fountains with cum, coating the anemone's back and hair in white.  The venom in your system draws out your orgasm to incredible lengths, and " + sMultiCockDesc() + " ejaculates over and over until you feel lightheaded and woozy.");
				if (player.cumQ() >= 500) Render.text("  When you've finished, not a single place on the anemone's backside is still blue; the giddy girl is shoving your semen into her mouth with her tentacles and hands, swallowing almost as much lakewater as jizz.");
			}
			else {
				Render.text("your pussy clamps down, trying and failing to find something to squeeze and ");
				if (player.wetness() < 4) Render.text("drooling its lube down your thighs.");
				else Render.text("squirting cum behind you to rain onto your partner's calves and the surface of the lake.");
				Render.text("  The anemone sighs impatiently as you twitch atop her, waiting for you to finish.");
			}
			...
			if (dick) Render.text("dares not move from her knees, balancing your freshly-shot load on her back as she tries to push it toward her face with her tentacles.");
			else Render.text("looks indolently at you as you go.");
...
//[Ovipositor] option in rape menu
		private anemoneGetsLayedByBeePositor():void
		{
			...
			Render.text("The tentacles on this girl sing a siren song and beg you to sample their venomous, greedy caresses as you plunge your egg-layer into her... even the small, blue feelers around the ");
			if (monster.HP < 1) Render.text("reclining");
			else Render.text("vigorously masturbating");
			Render.text(" girl's pussy call to you.  Your insectile abdomen pulses as eggs shift, lining up for deposition, and your long, black ovipositor slides out, pulsing in time with your heartbeat.  The idea of having those feelers stroke your strange organ while you unload your pent-up eggs sounds so good that a drop of honey oozes out, filling the air with a sweet scent that makes your");
			if (player.antennae > ANTENNAE.NONE) Render.text(" antennae");
			else Render.text("nose");
			Render.text(" tingle.  The anemone's eyes light up as your black shaft drools, and she leans forward, catching the nectar on a finger and raising it to her lips.");
			...
			if (silly()) Render.text("duck ");
			Render.text("face,</i>\" you reply.");
			...
			if (player.stats.sens >= 50 || player.isTaur()) {
				...
				if (player.lowerBody.cockSpot.hasCock()) {
					...
					if (player.totalCocks() == 1) Render.text("male part erupts");
					else Render.text("male parts erupt");
					Render.text(" in sympathy, lancing semen over your partner.  Surprisingly, the anemone doesn't even notice, so absorbed is she in experimenting with your modified stinger.");
				}
				//[(Lev3 eggs)
				if (player.eggs() > 40) {
					Render.text("  Eventually there are so many eggs ");
					if (player.lowerBody.cockSpot.hasCock() && player.cumQ() > 1000) Render.text("and so much cum ");
					Render.text("that they begin to collect around her, clumping and layering around her midriff like white, slimy algae on a pier post.");
				}
				Render.text("  As the last rolls from you, the anemone raises your wilting ovipositor to her lips once again, eagerly drinking the final sweet secretions that fall out in long, slimy strings.");

				Render.text("\n\n\"<i>Weird,</i>\" she repeats, tugging at the shriveling organ as it tries to recede into your slit.");
				Render.text("\"<i>Ugh...</i>\" you respond, weak in the [legs].  With a jerk, you pull your abdomen away from her grasp, flinching as her skin rubs your sensitive slit, then turn to leave.  She catches your hand, looking almost concerned.");
				...
				if (player.eggs() < 20) Render.text("It doesn't take long before you finish, pushing your cargo down her passageway in a neat, orderly line.");
				else {
					Render.text("So many come that you can see them under her skin, a myriad of tiny bumps");
					if (player.eggs() >= 40) Render.text("; these same bumps multiply upward as your long-suffering abdomen pushes out line upon line of eggs, and soon you can feel them pressing back against the tip of your ovipositor.  You squeeze the girl's head in your hands, holding her against the base, and concentrate; slowly, the deposited eggs give way to their siblings, stretching her elastic stomach and chest wide");
					Render.text(".");
				}
				...
				if (silly() && player.fertilizedEggs() > 1) Render.text("  You briefly amuse yourself imagining her carrying your eggs to term and having them hatch in her mouth, so that when she talks, she shoots bees.  Nicholas Cage would be proud.");
				Render.text("  Gathering your things, you " + player.mf("laugh", "giggle") + " at her and depart.");
			}
			player.dumpEggs();
			player.orgasm();
			cleanupAfterCombat();
...
//[Ovipositor] option with spiderbutts
		private spiderOvipositAnAnemone():void
		{
			clearOutput();
			Render.text(images.showImage("anemone-egg"), false);
			Render.text("As the girl's resistance ebbs, ");
			if (player.HP < 1) Render.text("eyes unfocused with fatigue");
			else Render.text("two fingers plunging eagerly into her pussy");
			Render.text(", you advance on her.  Your abdomen bends forward and your egg-laying organ slides out, dripping slime into the water with little 'plit' noises; the girl, fascinated despite the strangeness of it, sits up and creeps forward to touch it.");
			...
			//[(cock)
			if (player.lowerBody.cockSpot.hasCock()) Render.text(", and your [armor] tightens as [eachCock] swells");
			else if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(", and the inside of your [armor] dampens with the lube your [vagina] leaves as it clenches around empty air");
			Render.text("; you have to push yourself to finish working before you can lose yourself to it.  Completing your restraints, you release her tentacles.  They bob behind her in one mass like a long, cute ponytail, and only the ends are free to wiggle.  When she realizes this, her behavior changes dramatically.");
			...
			if (player.stats.cor < 60) Render.text(", even though her adorable and slightly surreal puppy dog eyes implore you otherwise");
			Render.text(".  \"<i>Let me just finish what I need to do first.</i>\"");
			...
			if (player.gender > 0) {
				Render.text("go for your ");
				if (player.lowerBody.cockSpot.hasCock()) Render.text("cock");
				else Render.text("cunt");
				Render.text(", leaving");
			}
			else Render.text("leave");
			Render.text(" you holding the bag... of eggs, as it were.  Your steel your resolve and stride atop her, pinning the loose, wiggling end of her ponytail harmlessly against your chitinous underside and forcing her hands underneath her by lowering some of your weight onto her back.");
			...
			//[(sexed)
			if (player.gender > 0) Render.text("while unfastening your [armor] with one hand, ");
			Render.text("\"<i>there is definitely more.</i>\"  The next two eggs slip into her as you speak, sending convulsions through her body.  Her pussy spasms again and enfolds your ovipositor even more tightly; you're ready for the sensation this time, and allow it to resonate through you, forcing out your own wet orgasm.  You hold her face as you unburden yourself");
			if (player.lowerBody.cockSpot.hasCock()) {
				Render.text(" and cover her hair with yet more white strings from your twitching manhood");
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
			}
			//(egg level 3)
			if (player.eggs() >= 40) Render.text("; so many eggs pump into her that she gives a little start when her distended belly touches the lukewarm water below");
			Render.text("... finally you let her go when you're completely empty, pulling your stalk from her with a lewd sucking noise.  A little bit of green goo drools from her pussy as she slumps over on her side, and you make ready to leave her there - bloated and pregnant, with a squiggle of her semen floating in the water next to her.");
		...
		public anemoneKidBirthPtII():void
		{
			...
			if (player.tallness > 48) Render.text(" even as the face angles up to maintain eye contact");
			Render.text(".  Peering into the barrel, you can make out a little, though most of her décolletage is obscured by a pair of gangly knees.  The previously-noted head sits atop a pair of narrow, thin shoulders bearing equally wiry-looking arms.  She raises one hand and gives you a little finger wave.");
...
//[no, bonsai anemone is awesome and fuck the haters]
		private keepAnemoneKid():void
		{
			clearOutput();
			spriteSelect(71);
			Render.text("You frown as you stare into the opaque eyes.  You can't think of any way to get her out of the barrel, short of manhandling her into the wilderness where she'll flourish or expire depending on fate, and you haven't the heart for such endeavors.  Ah... she looks so happy sitting there with her head resting on her hands, too.  Well, worse things could happen - but probably not stranger.");
...
//KID A FOLLOWER STUFF
		public anemoneBarrelDescription():void {
			if (model.time.hours < 6) //(morning)
				...
				Render.text("Kid A stands next to her barrel, refilling it from one of your waterskins.  A second full skin is slung over her shoulder.  She gives you a grin.\n\n");
			else if (flags[FlagEnum.KID_SITTER] > 1)
				Render.text("Kid A is absent from her barrel right now, dragooned into babysitting again.\n\n");
			else if (model.time.hours < 16) //(midday)
				Render.text("Kid A is deep in her barrel with the lid on top, hiding from the midday sun.\n\n");
			else if (model.time.hours < 22) //(night hours)
				Render.text("Kid A is peeking out of her barrel.  Whenever you make eye contact she breaks into a smile; otherwise she just stares off into the distance, relaxing.\n\n");
			else Render.text("Kid A is here, seated demurely on the rim of her barrel and looking somewhat more purple under the red moon.  She glances slyly at you from time to time.\n\n");
		}
...
//[Barrel] button in [Stash] menu (appears whenever Kidswag flag >= 1)
		public approachAnemoneBarrel():void
		{
			...
			clearOutput();
			spriteSelect(71);
			Render.text("You walk over to the barrel.  ");
			//[(display if hourssinceKiditem >= 16)
			if (flags[FlagEnum.KID_ITEM_FIND_HOURS] >= 16) {
				Render.text("An item sits next to it, left there by the anemone as a present to you.  Or 'rent', if you choose to think of it that way.  ");
				item = getAnemoneItem;
			}
			//[(if Kid A has been given a weapon)
			if (flags[FlagEnum.ANEMONE_WEAPON_ID] != 0) {
				Render.text("She has " + ItemType.lookupItem(flags[FlagEnum.ANEMONE_WEAPON_ID]).longName + " sitting next to it.  ");
				//Render.text("Kid A pokes her head out and smiles at you.  What do you need from her?");
				weaponT = "Take Weapon";
				weaponB = takeOutOfAnemone;
			}
			//(if babysitting)
			if (flags[FlagEnum.KID_SITTER] > 1) Render.text("Kid A is away right now, helping season some of Marble's children.  If you wanted to leave or take an item, she would discover it when she got back.  ");
			else if (kidAXP() < 25) Render.text("Kid A sinks below the rim, peering nervously at you.  ");
			else if (kidAXP() < 75) Render.text("Kid A pokes her head out and smiles at you.  ");
			else Render.text("Kid A leans her head and shoulders out of the barrel and puts her elbows on the rim, staring unabashedly at you as she rests her chin on her hands.  ");
			Render.text("What do you need from her?");
			//[(if N.Watch is toggled on)
			if (flags[FlagEnum.ANEMONE_WATCH] > 0) Render.text("\n\n<b>Kid A is currently watching for enemies at night.</b>");
			...
			if (flags[FlagEnum.KID_SITTER] <= 1) {
				if (flags[FlagEnum.ANEMONE_WEAPON_ID] != 0 && player.fatigue <= 90) addButton(3, "Tutor", tutorAnemoneKid);
				else if (player.fatigue > 90) Render.text("\n\nYou're too tired to tutor Kid A.");
...
//[Item](only appears if hourssinceKiditem flag >= 16)
		private getAnemoneItem():void
		{
			...
			let choice:number;
			let itype:ItemType;
			Render.text("You reach down and pick up her present.  Today, she's left you ");
			...
			Render.text(itype.longName + ".");
			if (itype == weapons.L__AXE) Render.text("  Holy... how did she drag this thing home!?");
			Render.text("\n\n");
...
//[Take Weapon]
		private takeOutOfAnemone():void
		{
			clearOutput();
			spriteSelect(71);
			Render.text("You take the item back.  ");
			let itype:ItemType = ItemType.lookupItem(flags[FlagEnum.ANEMONE_WEAPON_ID]);
			if (flags[FlagEnum.ANEMONE_WATCH] > 0) {
				Render.text("Your anemone daughter will not be able to guard you at night without a weapon.  If you want her to guard, you'll need to give her a new weapon and tell her to watch at night again.  ");
...
//[N.Watch]
		private anemoneWatchToggle():void
		{
			...
			if (flags[FlagEnum.ANEMONE_WATCH] > 0) {
				Render.text("You tell Kid A not to worry about guarding at night.  She nods in your direction.");
				flags[FlagEnum.ANEMONE_WATCH] = 0;
			}
			else {
				if (flags[FlagEnum.ANEMONE_WEAPON_ID] == 0) {
					Render.text("You're not really going to set this featherweight against the night barehanded, are you?!  Her hair hasn't even grown out!  You'd better hand her some kind of weapon.");
				}
				else {
					Render.text("Kid A smiles and cheers, \"<i>Guard!</i>\"");
...
//[Tutor](only appears if Kid A is armed and present)
		private tutorAnemoneKid():void
		{
			...
			//(if lust > 99, output)
			if (player.lust > 99) {
				Render.text("You're way too horny to focus on any sort of weapon instruction right now, and the anemone can see it in your expression as your gaze wanders over her body; she blushes a deep blue and shrinks into her barrel with a shy glance.");
				doNext(camp.returnToCampUseOneHour);
				return;
			}
			Render.text("The anemone obediently climbs out of her barrel, ");
			//[(KidXP < 33)]
			if (kidAXP() < 33) {
				Render.text("holding " + ItemType.lookupItem(flags[FlagEnum.ANEMONE_WEAPON_ID]).longName + " protectively across her chest.");
			}
			else Render.text("taking up an attentive stance with " + ItemType.lookupItem(flags[FlagEnum.ANEMONE_WEAPON_ID]).longName + " in her hands.");

			Render.text("  You spend some time instructing Kid A in the finer points of the equipment you've provided her with, and then finish up with a practice duel.");
			...
			if (flags[FlagEnum.ANEMONE_WEAPON_ID] == weapons.PIPE.id ||
				...
				if (flags[FlagEnum.ANEMONE_WEAPON_ID] == weapons.PIPE.id) Render.text("pipe");
				else Render.text("stick");
				Render.text(" is still enough to bruise you a bit.");
			}
			//(HP - 5, KidXP + 1)
			//[Riding Crop]
				...
				if (player.stats.lib < 50) Render.text("is");
				else Render.text("isn't");
				Render.text(" enough to keep you from thinking dirty thoughts about grabbing her naughty, teasing face and mashing it into your crotch.");
			...
			//[Whip] or [Succubi's Whip]
				...
				if (flags[FlagEnum.ANEMONE_WEAPON_ID] == weapons.SUCWHIP.id) Render.text("  The demonic enchantment chooses then to activate, and her color deepens as her lust peaks, as does your own.");
				Render.text("  You feel a point digging into your groin as her prick hardens and her struggles cease; she begins to moan openly in arousal.  As she relaxes, the coils of the whip finally loosen enough for you to extricate yourself.");
			...
			//[Black Book]
				...
				if (player.lowerBody.cockSpot.hasCock()) Render.text("tighten");
				else if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("dampen");
				else Render.text("become a hindrance");
				Render.text("... though the part between her own legs is still pointed at you.");
				//(lust + 10, KidXP + 2)
				dynStats("lus", 20);
				kidAXP(4);
			}
			//[Scarred Blade](NYI)
				...
				//[(if corr <=70)
				if (player.stats.cor <= 70) Render.text("  The blade's edge flashes toward you as well, when you try to pick it up.  After a few frustrated attempts, it becomes clear that you'll have to abandon it for now.");
			...
			if (player.HP < 1) {
				...
				if (flags[FlagEnum.ANEMONE_WEAPON_ID] != weapons.S_GAUNT.id &&
						flags[FlagEnum.ANEMONE_WEAPON_ID] != weapons.H_GAUNT.id) Render.text("the thump of something hitting the ground and ");
				Render.text("the anemone gasp, and then the world slips away from you.");
			...
			if (flags[FlagEnum.HAD_KID_A_DREAM] > 0 && kidAXP() >= 40 && player.lust > 99) {
				...
				//if KidXP >= 40 and lust > 99 after tutor and PC has only huge dicks of area >= 60 or hasn't got shit
					...
					if (player.lowerBody.cockSpot.hasCock()) Render.text("something too enormous");
					else Render.text("nothing useful");
					Render.text(" to relieve your burden with, then resumes the search with a hint of desperation.  Finally and after exhaustive prodding, she withdraws her hand and chews her lip in consternation.");
		...
		private kidASex(cont:boolean = true):boolean
		{
			if (!cont) {
				clearOutput();
				Render.text("Smiling lustily at Kid A, you run your hand through her hair, inhaling deep breaths as you allow her venom to affect you more and more.  She blushes, but watches eagerly.  Soon, the tingling lust has you overwhelmed...");
			}
			let x: number;
			let y: number;
			...
			if (flags[FlagEnum.ANEMONE_KID] == 2 && player.lowerBody.cockSpot.hasCock() && player.cockThatFits(36) >= 0) {
				...
				//[(big skeet)]
				if (player.cumQ() >= 1000) Render.text("  As you pour into her, the sensation of being stuffed with semen sets off a second orgasm in the quivering girl, and her own prick drools weakly onto your stomach");
				if (player.cumQ() >= 2000) Render.text("; even a second orgasm from your daughter isn't enough to match one of yours in length.  Your magnificently productive " + Appearance.cockNoun(player.lowerBody.cockSpot.list[x].cockType) + " fills her until her belly is inflated so far that she can't contain more, and it begins to breach the seal her pussy makes around you, even as tight as it is, to squirt out in lazy globs as you deposit the last of your spunk");
				if (player.cumQ() >= 1000) Render.text(".");
				//[(multi)]
				if (player.lowerBody.cockSpot.count() > 1) {
					Render.text("  The loads from your other shaft");
					if (player.lowerBody.cockSpot.count() > 2) Render.text("s");
					Render.text(" ooze down your [legs], directed there by her weight.");
				}
			...
			//if KidXP >= 40 and lust > 99 after tutor and PC has wang of area <60, set that cock to x variable, also set biggest cock not yet used to y variable if multicocked
				...
				Render.text("\n\n");
				Render.text(images.showImage("anemone-kid-male-bj"), false);
				...
				if (y >= 0 || player.lowerBody.balls > 0 || player.lowerBody.vaginaSpot.hasVagina()) {
					...
					if (y >= 0) Render.text(cockDescript(y));
					else if (player.lowerBody.balls > 0) Render.text(sackDescript());
					else if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(vaginaDescript(0));
					//[(if cock or sack)]
					if (y >= 0 || player.lowerBody.balls > 0) Render.text(", pulling it free from your armor as well");
					Render.text(".  Her dextrous digits are emboldened as your head lolls in the dirt, staring skyward, and quickly begin to caress their find.");
					//[(cock)
					if (y >= 0) {
						Render.text("  Another hand quickly joins the first, stroking the length of your second shaft as it ");
						if (player.lowerBody.cockSpot.list[y].cockLength >= 36) Render.text("towers over her head");
						else Render.text("quivers and twitches into her venomous locks");
						Render.text(".  Every time it brushes her hair, another jolt shoots to the base of your spine and both of your cocks force out another glob of precum which is quickly slurped up.");
					}
					//[(sac)
					else if (player.lowerBody.balls > 0) Render.text("  One hand cups below your " + ballsDescript() + " and lifts them up to her chin, where the other, formerly tracing gentle circles along your shaft, grabs a tentacle from her head and begins to tickle.");
					//[(vag)
					else if (player.lowerBody.vaginaSpot.hasVagina()) {
						Render.text("  Her fingers part your labia and reveal your " + clitDescript() + ", ");
						if (player.lowerBody.vaginaSpot.list[0].clitLength >= 6) Render.text("working it free as well and allowing her to tease and stroke it, cock-like, with her hands and hair.");
						else Render.text("in order to give the sensitive button their attention.");
						Render.text("  Her other hand slips into your garments after the first, following it down to your groin and slipping into your pussy to pump back and forth gently.");
					}
				}
				Render.text("\n\nHer attentions on your groin reach a peak as ");
				if (player.lowerBody.balls > 0) Render.text("your balls tighten up");
				else Render.text("your body tenses");
				Render.text("; seeing this, she dives down onto your shaft, forcing the head past the ginger care of lips and tongue into her welcoming throat.  Her hair drapes all around your groin, touching exposed flesh as she shakes her head and undulates her throat to coax out your climax.  Your body reacts before you can, and your hips thrust up into her face as your ejaculate erupts.");
				//[(if multicock or sac)]
				if (player.lowerBody.balls > 0 || y >= 0) {
					Render.text("  As her eyes twinkle, the hands on your ");
					if (y >= 0) Render.text(cockDescript(y));
					else Render.text(sackDescript());
					Render.text(" renew their efforts, squeezing out squirts of semen with measured strokes and squeezes.");
				}
				//[(vag)
				else if (player.lowerBody.vaginaSpot.hasVagina()) {
					Render.text("  As her eyes twinkle, a hand plunges all the way into your pussy, and your lonely muscles clamp down reflexively on the invader, attempting to wring her fingers.");
				}
				Render.text("  She swallows greedily as you unload into her throat");
				if (y >= 0) Render.text(" and onto her hair");
				Render.text(", the oral contractions squeezing the head of your penis and setting your body to shivering.");
				//[(big cum)
				if (player.cumQ() >= 500) {
					Render.text("  Squirt after huge squirt disappears into the starveling's esophagus, sliding without fail down into her belly, until ");
					if (player.cumQ() < 1000) Render.text("your climax is spent.");
					//(mega skeet)
					else Render.text("she can hold no more and it begins to drool from her mouth; still she tries her best to swallow more and more, despite the limits of her body.  Obligingly, you grab her head and hold it to you, venom tingling in your fingertips, as the last half of your orgasm empties into her.  With her face mashed into your crotch and escape cut off, the semen has nowhere to go except down the throat, expanding her taut stomach and pushing her skinny butt into the air as she balloons against your legs.  Her eyes widen and look up at you as the pressure increases, and a hum travels down your prick as she attempts to protest.");
				}
				...
				if (player.cumQ() < 1000) Render.text("then gets to her feet and returns to her barrel to digest the meal.");
				else Render.text("but can barely move from the spot thanks to the incredible distension of her belly.  She manages to drag herself over to the barrel with a great shuffling of arms and legs but cannot by any means get into it, and settles for slumping beside it to occasionally sprinkle water on herself while she deflates.");
			...
			//if KidXP >= 40 and lust > 99 after tutor and PC has vag and no cox
				...
				//[(if pc is loose)
				if (player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.GAPING) Render.text("  You can barely feel her little shaft in your stretched cunt, but the chemical stimulation from the tentacles stroking your insides goes a long way toward making up for that.");
				Render.text("  Emboldened, she picks up your legs haltingly, then begins to work herself in and out of your depths.");
				...
				//[(clitsize<6)]
				if (player.lowerBody.vaginaSpot.list[0].clitLength < 6) Render.text("  The little button gets rubbed by her probing hand even as she resumes thrusting, accelerating your imminent peaking.");
				else if (player.lowerBody.vaginaSpot.list[0].clitLength < 36) Render.text("  The monstrous chick-stick bobs in the air as she pounds you harder; the breeze alone would be enough to arouse you further, but the anemone grabs it and begins jacking the nerve-laden stalk off like a dick, which sends your back into spasms.");
				else Render.text("  The incredible length of your clitoris is such that the pace of the fuck slows to a crawl as the anemone puzzles out what to do with it; finally falling back on her racial instinct, she pops it into her mouth and begins to caress the tip gently with her teeth and tongue as her short hair reaches around to tickle the shaft.  Your body flops weakly, too wracked by the sensation from your clit to allow conscious control.");
...
//Yes]
		private reallyEvictDaAnemone():void
		{
			clearOutput();
			spriteSelect(71);
			Render.text("Time to reclaim your barrel.  Gesturing to get her attention, you grab the anemone by her upper arm and lift her to her feet.  She looks at you in confusion, but you set your face and drag her along with you as you make your way to the lake.");
			...
			Render.text("\"<i>Um... hi!</i>\" your anemone returns, enthusiastically this time, as she answers the physical contact by tugging one of the newcomer's tentacles.");
...
//if KidXP drops below threshold for sex due to bad training, no more dreams and no more sex
		public kidADreams():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock() && (!player.lowerBody.vaginaSpot.hasVagina() || player.femininity < 50)) {
				Render.text(images.showImage("anemone-kid-male-masti"), false);
				...
				if (player.isTaur()) Render.text("thigh");
				else Render.text("cheek");
				Render.text(", she leans in to kiss you, softly whispering your name over and over.  \"<i>[name]... [name]...</i>\"");

				Render.text("\n\nYour eyes snap open suddenly to see the ubiquitous red moon overhead.  A ");
				if (!player.isTaur()) Render.text("soft hand strokes your face and a ");
				Render.text("slight weight rests against your crotch; as you look down, your anemone is there with her pussy pressed to the erection bulging under your gear, frozen in mid-rub and blushing to the point that you can barely make her out.");

				Render.text("\n\n\"<i>Um... hi?</i>\"  She slowly backs away into the night, leaving you to try to sleep with your awakened libido.");
			}
			//if female:
			else {
				Render.text(images.showImage("anemone-kid-female-masti"), false);
				...
				if (player.stats.cor >= 66) Render.text(" and, with a sigh, you grab her head and force it down to the mess, compelling her to lick it up.");
				else Render.text(" and neither of you says a word as she backs away slowly on her knees.");
				Render.text("  Sighing, you turn over and attempt to return to sleep despite the pervading smell of semen.");
			}
...
//repeatable, trigger possible when visiting 2+ Izma kids
		public kidAWatchesSharks():void
		{
			...
			//(else KidXP < 75 and Kidweapon = White Book or Black Book)
				...
				if (flags[FlagEnum.ANEMONE_WEAPON_ID] == consumables.W__BOOK.id) Render.text("blinds them with a sudden flash of white light");
				else Render.text("turns deep blue as the various shark-girls clap their thighs together and look away from one another in embarrassment");
				Render.text(".  Before any of them can react, she dives into the water and fills the waterskins, then hightails it away.");
			}
			//(else KidXP < 75)
			else if (kidAXP() < 75) {
...
//repeatable for now, but semi-rare
		public goblinNightAnemone():void
		{
			...
			if (player.tallness > 48) Render.text("tall");
			else Render.text("short");
			Render.text(", " + player.mf("studly", "gorgeous") + " " + player.race() + "; told us that " + player.mf("he", "she") + "'d be able to knock us all up like " + player.mf("he", "she") + " did her, and a few of the little goblin tramps outside the family have seen one in this camp.  We're going to get our babies.</i>\"  Kid A remains silent, but shakes her head uncertainly, holding her equipment closer to her chest.");
