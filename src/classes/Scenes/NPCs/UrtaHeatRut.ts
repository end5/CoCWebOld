...
internal function approachSoberUrtaHeatRutProc():void {
	clearOutput();
	urta.urtaSprite();
	Render.text("As you walk over to the table where your foxy herm-lover sits, you think you see eyes following you, your presence attracting an unusual amount of attention. ");
	if(player.inHeat && player.inRut && player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) Render.text(" Males, females and herms alike stare at you with a mixture of puzzlement, longing and hostility, as if unsure whether they want to fight with you, fuck you senseless or be fucked senseless by you.");
	else if(player.inRut && player.lowerBody.cockSpot.hasCock()) Render.text(" Males stare at you with a hint of challenge in their eyes, whilst females eye you attentively. Herms don't seem sure whether they want to fight you or fuck you.");
	//(Player in heat
	else if(player.inHeat && player.lowerBody.vaginaSpot.hasVagina()) Render.text(" Males, and individuals who look like females at first, until you spot the telltale bulge of at least one cock in garments or hanging freely between their legs, stare at you with obvious desire as you go, more than a few erections starting to form in your wake.");
	Render.text("  You ignore them all, concentrating on Urta.");

	Render.text("\n\nAs you approach, ");
	
	if(urtaLove()) Render.text(" she opens her arms to embrace you, and smiles.");
	else Render.text(" she smiles when she sees you, but then gives the air a sniff and immediately seems to gain more color to the skin under her fur.  \"<i>Uh, I, I see.</i>\"  You sit down next to her, but even though she isn't even suggesting anything yet, you already find it hard to control yourself.");
	
	if(player.inRut && player.lowerBody.cockSpot.hasCock()) Render.text("  Urta is right next to you, and she has already proven herself a good lay");
	else Render.text("  You keep reminding yourself what Urta is packing between her legs and how much precious semen it can give, and can't help but want to try that now");
	
	Render.text(".  Not to mention, with or without the monster between her legs, her body is sexy as hell.");

	Render.text("\n\n\"<i>Well, you do help me with my own releases, so I suppose that's fair....");
	if(player.inHeat && player.lowerBody.vaginaSpot.hasVagina()) Render.text("  You... smell good, so I at least hope you're not here to tease me.");
	Render.text("</i>\"  She leans closer to you, and ");
	if(player.inRut && player.lowerBody.cockSpot.hasCock()) Render.text("your eyes are quickly drawn to her full breasts as she whispers something to you. You're too hypnotized to care.");
	else Render.text("you lose yourself in your imagination about having Urta take you raw and pump you full of sperm.  There's a faint voice you hear, and you realize it's Urta's as you snap back to reality.");
	Render.text("  She coughs and repeats herself.  \"<i>Are you here because of your, um, condition? You'd prefer to blow off some steam with ");
	if(urtaLove()) Render.text("your adorable, hot vixen");
	else Render.text("someone you know and trust");
	Render.text(", right?</i>\"");
	...
	if(player.inRut && player.lowerBody.cockSpot.hasCock()) Render.text(" focus is on the hips of the fox-morph");
	else Render.text(" focus on catching a glimpse of that amazing cock, and you keep imagining what she can do with it");
	Render.text(".  Your aggressive hormonal state demands that you ");
	if(player.inRut && player.inHeat && player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) Render.text(" put a baby in her, then let her put a baby in you");
	else if(player.inRut && player.lowerBody.cockSpot.hasCock()) Render.text(" put a baby in her so she can push it out of those beautiful hips");
	else Render.text(" put that beautiful big cock into your needy [vagina]");
	//[If Urta's fertility quest wasnât done]
	if(!urtaQuestDone()) Render.text(", even if something keeps reminding you that's impossible.");
	else Render.text(".");

	Render.text("\n\n\"<i>You know, [name] I'm glad to see you as always.  ");
	if(urtaLove()) Render.text("  I'm really thankful you managed to control yourself and see me with this.  You know that if you need me, I'm always ready to help, love.</i>\"");
	else Render.text("  I'm really flattered you thought of me when in such a state!</i>\"");
	...
	if(player.inRut && player.inHeat && player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) Render.text("mixed genitals");
	else if(player.inRut && player.lowerBody.cockSpot.hasCock()) Render.text("[cock biggest]");
	else Render.text("[vagina]");
	Render.text(" against her backside.");
	...
	//Player in rut:
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Your " + multiCockDescriptLight() + " and [vagina] are filling your clothes with mixed juices");
		else Render.text("  Your [cock biggest] begins to seep precum into your clothes");
		Render.text(", so you decide to dispose of them as you force Urta against a wall.");
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and your pussy dripping and eager");
		Render.text(".");
		menu();
...
internal function approachDrunkenUrta():void {
	...
	//Player in heat:
	else Render.text("  Males, and individuals who look like females at first, until you spot the telltale bulge of at least one cock in garments or hanging freely between their legs, stare at you with obvious desire as you go, more than a few erections starting to form in your wake.");
	 Render.text("  You ignore them all, concentrating on Urta.");
...
private notInterestedInUburDrunkUrtaRuts():void {
	...
	if(player.faceType == 2) Render.text("Your nose does seem to pick up that Urta was and still is getting unusually aroused behind you.  ");
	if(player.inRut && player.lowerBody.cockSpot.hasCock()) Render.text("[EachCock] twitches in protest at you leaving a willing lay behind.  ");
	//[Medium lust increase] 

	//[If the PC has a snake tongue]
	if(player.tongueType == TONUGE.SNAKE) Render.text("You instinctively taste the air, and Urta suddenly seems utterly delicious. ");
	//Rut: 
	if(player.inRut && player.lowerBody.cockSpot.hasCock()) Render.text("Your " + multiCockDescriptLight() + " hardens in your pants as you still force yourself to leave.");
	//[Medium lust increase]
	else Render.text("Your condition does seem kind of protesting against leaving such a willing lay behind, but it isn't time to be indulging it.");
...
private drunkUrtaIntroPartDuex(chosenSex: number = 1, newl:boolean = true):void {
	...
	Render.text("Suddenly, Urta's hand dashes to ");
	//[if players has balls]
	if(player.lowerBody.balls > 0 && chosenSex == 1) Render.text("your [balls], rolling them in her hand and squeezing a bit too roughly.");
	else if(player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("your [clit], rubbing it until it's ");
		if(chosenSex == 1) Render.text("the very same state [eachCock] seems to be in so often recently, ");
		Render.text("erect and throbbing.");
	}
	else Render.text("your [hips], stroking along them towards [eachCock] in an agonizing display of teasing.");
	...
	if(chosenSex == 1) Render.text(", and [eachCock] already gets hard in your [armor].");
	else Render.text(", and you feel the inside of your [armor] rapidly slickened by your [vagina]'s excitement.");
	...
	//Player in both : 
	if(player.inRut && player.inHeat && player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) Render.text("and she proceeds to maul her breasts, pre-cum seeping copiously from her cock and her snatch drooling over her balls and onto the floor.");
	else if(chosenSex == 1) Render.text("but she doesn't seem to mind it for the moment.  Rather, the fur between her thighs appears to be rapidly soaking.");
	else Render.text("which she seems more than intent to push into you, although you do see a bit of female lubricant sliding down her balls.");
...
private keepTeasingDatHornyFawkes():void {
	...
	if(player.lowerBody.balls > 0) Render.text("lewdly cupping and fondling your [balls], ");
	Render.text("slowly beginning to stroke and caress your " + multiCockDescriptLight() + " for the benefit of those who bear vaginas.");
	...
	if(player.isGoo() || player.isNaga()) Render.text("what passes for knees on your [legs]");
	else Render.text("knees.");
	...
	if(player.stats.cor < 50) Render.text("run");
	else Render.text("walk triumphantly");
	Render.text(" from the racous orgy that is the Wet Bitch; truly an auspicious name.");
...
private soberUrtaSatingPCHeat():void {
	...
	Render.text("As she presses your back against the wall, you spread your [legs] wide, allowing Urta easy access to your awaiting womanhood");
	//[If herm]:
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text(" by shoving your " + multiCockDescriptLight());
		if(player.lowerBody.cockSpot.hasCock() && player.lowerBody.balls > 0) Render.text(" and [balls]");
		Render.text(" out of the way");
	}
	else if(player.lowerBody.balls > 0) Render.text(" by shoving your [balls] out of the way");
	Render.text(".  Slowly, you feel the wide flared head of her magnificent pillar of flesh press onto your opening, spreading your vaginal lips and making your [clit] harden in anticipation just as your [butt] shivers, awaiting the intrusion.  Slowly, Urta's giant member starts to enter you.");
	if(player.vaginalCapacity() >= 100) Render.text("  Your hungry gash eagerly welcomes her, Urta easily sliding herself home.");
	else if(player.vaginalCapacity() > 60) Render.text("  You feel it stretch you in the most pleasuring of ways, filling you with warm, throbbing flesh as she pushes it deeper and deeper.");
	else Render.text("  You wince in pain at being stretched so much, but you grow wetter and wetter as Urta slowly pushes herself inside your depths, your [vagina] slowly getting wetter and warmer, accepting the intruder.  Pleasure starts to override pain and you let out a gasp.");
	...
	//[If Very Loose] 
	if(player.vaginalCapacity() >= 60) Render.text("Urta's erection seems completely at home in your depths, completely embraced by the wet walls of your throbbing [vagina].");
	else Render.text("You moan again, feeling Urta's member stretch, harden and throb, blood running through it.  You feel complete, and your heat is ignited further by the sensation of something so wonderful and big filling you.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("[cock biggest]");
		...
		Render.text("clitoris, ");
		if(player.lowerBody.vaginaSpot.list[0].clitLength < 2.5) Render.text("rubbing the hard, stimulated sensitive spot between her fingers");
		else Render.text("running her fingers over your entire big clit before stroking it like she'd do with a cock");
	}
	Render.text(", letting you have a few moments to get used to her mammoth member and accommodate it.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text(" Cum rushes out of [eachCock] and onto the soft fur of Urta's abdomen, then starts leaking out slowly in time with her thrusts.");
	Render.text(" Your cunt clamps down tighter onto Urta as you orgasm,");
	if(player.wetness() >= 4) Render.text(" pussy juice shooting out around her member, coating Urta's fur and soaking her balls and the base of her cock.");
	else Render.text(" love juices flowing liberally out of you, giving the skin of Urta's cock a visible shine and spreading onto her balls.");
	...
	if(player.wetness() >= 4) Render.text("shooting out all over the place");
	else Render.text("running down her member and the crack of your [butt], coating the two of you in the proof of your passion");
	Render.text(".");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("your own member responding alike to Urta's, shooting out semen, ");
	Render.text("your [vagina] ");
	if(player.wetness() >= 4) Render.text("once again setting off to coat Urta in your juices.");
	else Render.text("squeezing on her, trying to milk her of her seed.");
	...
	Render.text("\n\n\"<i>What?  ");
	if(!urtaLove()) Render.text("You're still not satisfied?</i>\"");
	else Render.text("You want to keep going, darling?  I want to make you satisfied, so I'll spare all the time I can!</i>\"");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("and " + multiCockDescriptLight() + " ");
	Render.text("hardening again in response to the act.  Urta continues to slam into your backside, delivering a small smack to your [butt].");
	...
	//[Hermaphrodite]
	if(player.lowerBody.cockSpot.hasCock()) Render.text("[eachCock] shoots its seed onto the floor of the alley, helplessly");
	else Render.text("your [vagina] squeezes and tugs on her shooting member, leaking its own juices all over her member, the intensity of the orgasm shaking you right into your core");
	Render.text(".");
	...
	Render.text("\n\n\"<i>That felt amazing!  ");
	if(!urtaLove()) Render.text("If only I had you by hand every time I need to blow a load.");
	else Render.text("Your pussy is absolutely amazing, sweetheart: I hope I've helped you settle down?  Is your heat sated for the moment?");
	Render.text("</i>\"");
	...
	Render.text("\n\n\"<i>");
	//([Regular Urta]
	if(!urtaLove()) Render.text("Sure thing, I'm here to help,</i>\" she responds with a wink.");
	else Render.text("I'm here if you need me, love.  Whatever makes you happy.  You've helped me with my own problems so much already, so I owe you at least one.  Not to mention, making love to you feels absolutely amazing!</i>\"");
	...
	if(!urtaQuestDone()) {
		...
		Render.text("  She grabs your hand and helps you stand up.");
	}
...
private sateRutWithSoberUrtaButHuegDicked():void {
	...
	Render.text("Urta observes your approaching " + cockDescript(x) + " with a hungry, eager expression. Without taking her eyes off of it, she flips up her ");
	if(!urtaLove()) Render.text("skirt");
	else Render.text("dress");
	Render.text(", her huge horse-cock whipping up to slap between her breasts, pre already dribbling down the tip.  However, as you get closer, her expression suddenly falls and her cock begins to sink.  \"<i>I'm sorry, but... that's just too big!</i>\" she protests.  \"<i>You smell absolutely gorgeous, and I'm willing to try with something a little bigger than usual, but that's... that's just frigging huge - there's no way I could fit that monster!</i>\" she declares. \"<i>Just because I have a stallion cock doesn't mean I have a mare's cunt!</i>\"");
	...
	if(!urtaLove()) {
		...
		if(player.cumQ() <= 50) Render.text("spattering spooge onto Urta's face");
		else if(player.cumQ() <= 250) Render.text("splashing her face and tits with your cum");
		else Render.text("soaking the shocked fox-morph in your jism");
		Render.text("; though you still feel the urge to breed fertile females, your lust is momentarily sated.  Urta wipes ");
		if(player.cumQ() >= 500) Render.text("halfheartedly ");
		Render.text("at the cum on her faces and gives you a friendly smile.  \"<i<There, that should feel better; I know what it's like to be all pent up with no release.</i>\"  She rises to her feet, still holding your cock, and gives it a squeeze.  \"<i>In the meantime, if you want to really cut loose, I suggest you go and find something to shrink this down.</i>\"");
	...
	//Lover Urta Path/
		...
		if(player.cumQ() <= 50) Render.text("She swallows it all without complaint, licking her lips clean of any stray droplets once you remove it from her mouth.");
		else if(player.cumQ() <= 250) Render.text("Her cheeks bulge at the amount you pump into her, but she's able to swallow with an admirable amount of control, gasping for breath.");
		else Render.text("Rivulets of cum burst from between her lips, but she stubbornly sucks and swallows as much as she can, her stomach starting to bulge from the sudden influx of spooge.  When you stop, she is drooling cum and gasping for breath, but she meets your eyes with pride, even as she gives a wet belch.");
...
private sateRutWithSoberUrta():void {
	...
	clearOutput();
	urta.urtaSprite();
	Render.text("Urta observes your approaching package with a hungry, eager expression. Without taking her eyes off of it, she flips up her skirt, her huge horse-cock whipping up to slap between her breasts, pre already dribbling down the tip.");
	...
	Render.text(" she mumbles around your tongue, so caught in lust she doesn't care where you are.  Indeed, it seems your little fox-herm has something of an exhibitionist streak...");
	...
	if(urtaLove()) Render.text("  \"<i>You're the only one who has ever accepted every part of me,</i>\" she tells you, one hand brushing affectionately against your cheek.");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(", trigging a mutual explosion from your [vagina]");
	Render.text(".  The two of you gasp for breath, both obviously drained... but your hormones are boiling, and your " + cockDescript(x) + " fails to soften fully inside her.  Within moments, you are hard again; as soon as you catch your breath, you are at it again.  You lose count of how many times you do it - maybe three or four? - but finally you are so spent that even your rut is sated that you've bred Urta well and good, for now, at least.");
	...
	if(urtaLove()) Render.text("blows you a kiss with a flirty wink of her eye and then ");
	Render.text("scrubs off the worst of the cum plastered in her fur with the dry parts of her clothes.  She saunters playfully over to a garbage bin, throwing them inside.");
	if(urtaLove()) Render.text("  Her hips deliberately sway in a mesmerizing fashion, tail flicking from side to side to give you a tantalizing grip of her still-dripping snatch and the now-flaccid prick underneath.");
	Render.text("  You watch as she pries at a loose stone with her claws, removing it to reveal a small cubby, from which she withdraws a spare set of clothing.");

	//If fertility quest done:
	if(urtaQuestDone()) {
		Render.text("\n\nYou ask Urta what she's going to do; you didn't have the chance to use a condom.");
		if(flags[FlagEnum.URTA_FERTILE] == 1) Render.text("\n\n\"<i>We'll just have to hope then, won't we, [name]?</i>\"");
		...
		if(!urtaLove()) Render.text("\"<i>And you had better learn to control yourself, " + player.mf("mister","missy") + ".");
		else Render.text("\"<i>But you should learn to control yourself lover-" + player.mf("boy","girl") + ".");
		Render.text("  I'd rather do something like this in the privacy of my home than in the middle of the street.</i>\"");
...
private drunkenUrtaFucksPCInHeat():void {
	...
	if(!urtaQuestDone()) Render.text("  You know that she can't give you babies... but, honestly, at this moment, you couldn't care less. Her wonderful stallion cock is there and ready and it's all for you.");
	else Render.text("  You dimly recall that Urta could be virile, but she's right there, her huge mare-cock just dripping to plunge into your ripe [vagina] and knock you up.  You don't care anymore; you want her babies!");

	Render.text("\n\nUrta's eyes glitter with emotion and she bends down to give you another kiss before roughly grabbing her ");
	if(!urtaLove()) Render.text("skirt");
	else Render.text("dress");
	Render.text(" and hiking it up to expose her pre-spewing cock.  A steady trickle of feminine lubricant dribbles out of her gash, oozing over her apple-sized balls and dripping noisily onto the floor, but Urta couldn't care less; she's too fixated on plunging her maleness into your ready pussy to notice her feminine urges.");
	...
	if(player.lowerBody.cockSpot.hasCock()) 
	{
		Render.text(", impatiently nudging aside your " + multiCockDescriptLight());
		if(player.lowerBody.balls > 0) Render.text(" and your [balls]");
		Render.text(" to get to the feminine orifice below");
	}
	Render.text(". You cry out in joy as your [vagina] is filled, her soft fur brushing against your inner walls before she opens her mouth as wide as she can and sticks out her tongue. The long, broad, vulpine appendage slides out and up your depths, caressing the walls, lapping up your juices, ");
	if(player.lowerBody.vaginaSpot.list[0].clitLength < 2.5) Render.text("playing with your [clit]");
	else Render.text("lapping around your huge pleasure-button, slurping it into her mouth where she suckles from it");
	Render.text(", and making you thrash and scream until you think you're going to cum from the pussy-eating alone.  ");
	if(player.wetness() >= 4) Render.text("As if to emphasize this, your squelching, slurping pussy suddenly sprays fem-cum all over Urta's face, making her pull herself free with a shocked shake of her head.");
	else Render.text("As if sensing your impending orgasm, Urta removes her head from your [vagina], giving you a cocky grin and shaking her head.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("nor even your " + multiCockDescriptLight());
...
private sateRutWithDrunkUrtaWithHugeDick():void {
	...
	if(player.lowerBody.balls > 0) Render.text("against your own [balls]");
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("against your [clit]");
	Render.text("between your [hips]");
	Render.text(".");
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() < 150) Render.text("compete with minotaurs or something?");
	else Render.text("set a world record or something?");
	Render.text("</i>\"\n\n");
...
private drunkUrtaRidesARutPCsCock():void {
	...
	let x: number = player.biggestCockIndex();
	urta.urtaSprite();
	Render.text("You decide not to fight the situation; truthfully, the promises given by the sight of the hot hermaphrodite vixen climbing on top of you ignite your lusts and sing to your rut in ways you don't think you'd be able to safely ignore.");
	...
	Render.text(", and her own balls resting comfortably on your abdomen.  She grits her teeth as you feel her pussy slowly tighten around and massage your " + cockDescript(x) + ", and you groan in response.  Urta starts to bounce up and down on your member, smacking her thighs and ass into your body, not minding her own giant equine member flopping around and occasionally smacking you over your chest.  She lets her tongue flick out into the air for a moment and moans in delight, before picking up the pace and starting to pump your cock in and out of her hungry passage furiously.");
	...
	if(player.lowerBody.balls > 0) Render.text("feel your balls churning");
	else Render.text("recognize the tingling sensation from your " + cockDescript(x) + " to your spine");
	Render.text(", a telltale sign that your penis is ready to unload the potent spunk inside the herm-nympho bouncing on top of you.  You try to announce this, but she doesn't slow down.  Moments later, ");
	//[Normal cum production]
	if(player.cumQ() <= 50) Render.text("you unload your thick spunk into her pussy");
	else if(player.cumQ() <= 250) Render.text("you erupt inside her, filling her with wave after wave of warm, thick semen");
	else Render.text("you blow massive surges of spunk inside her continuously.  The fox's stomach expands, rivulets of semen running down onto the table as she continues her ride on top of you");
	Render.text(".  As you discharge, the fox's pussy starts squeezing and contracting, and you recognize the hermaphrodite's own orgasm.  Thicker white liquid suddenly drips from the tip of her cock quite copiously and occasionally shoots out into the air.");
	...
	if(player.hasKnot(x)) Render.text("You slowly feel the blood flow to your erect penis, and the knot at the base of it seems to start to expand");
	else Render.text("You don't mind staying like this for much, much longer, and the hermaphrodite on top of you seems to share that point of view");
	Render.text(".  Summoning your slightly exhausted strength, you pull her hips down deeper into your own before starting to return the hermaphrodite's shakes and bounces with thrusts of your own.  It intensifies the sensations, making your lover yelp in surprise before smiling widely and releasing low, content moans and purrs from the depths of her throat.");
	...
	if(player.hasKnot(x)) Render.text("knot surge with more blood and your erection twitch, ensuring that you're completely trapped inside her until you're empty.");
	else Render.text("tip expand as you're getting ready to blow another load.");
	Render.text("  Urta moans on top of you, increasing the speed of her movements and running her hand up and down her own member faster and with more intense strokes.");
	...
	if(player.cumQ() >= 500 && player.cumQ() <= 1500) Render.text("\n\nUrta seems positively full and stretched now, and your rut almost subsides for a moment, her stretched tummy imitating a pregnancy quite well.");
		...
		if(player.lowerBody.balls > 0) Render.text("[balls]");
		else if(player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.vaginaSpot.list[0].clitLength >= 3.5) Render.text("cock-like clit");
		else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("pussy lips");
		else Render.text("inside of your thighs");
		Render.text(" with your own thick white semen.  Then, she slides your softening erection back into herself again.");
	}
	...
	if(player.lowerBody.balls > 0) Render.text("  Your [balls] don't seem to be completely empty yet");
	else Render.text("  It doesn't appear to be the end of what your body can produce");
	Render.text(", and your vulpine lover squeals with delight as she feels you harden inside her.  She smacks your [chest] with her huge, half-hard member.");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.balls == 0) Render.text(" the inside of your thighs");
	else if(player.lowerBody.balls > 0) Render.text(" your [balls]");
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(" your [clit]");
	Render.text(" when she's pressed entirely against you.  She smacks her own bottom and you run your other hand over her body as the two of you tug on and pull her massive erection towards your face and over your [chest], thick dollops of pre-cum and leftover semen dripping from her member over your body.  Urta licks her lips as she looks down on you with ");
	if(!urtaLove()) Render.text("intense, unadultered lust");
	else Render.text("possessiveness and a hungry, but somehow warm expression");
	Render.text(".");

	Render.text("\n\nI should make you do this every day!</i>\"");
	if(urtaLove()) Render.text("  She suddenly seems a bit shy and surprised, before saying, \"<i>Make sure to remind me I said that later....</i>\"");
	...
	if(!urtaLove()) Render.text("Show me you're " + player.mf("the man","the breeder") + " here!");
	else Render.text("You don't want to make me feel like I'm abusing you, do you?  I know you want this, so pound me!</i>\"");
	...
	if(player.lowerBody.balls > 0) Render.text("[balls]");
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("[vagina] and [clit]");
	else Render.text("taint");
	Render.text(" before finally running her tongue over your [asshole], and all the way up again, swallowing your member once again as she squeezes your [butt].  You alternate before moaning, groaning, and whimpering as blood seems to leave your head and entire body, surging into your " + cockDescript(x) + " and making it hard to think or move, much less resist.");
	...
	if(urtaLove()) Render.text("  You can rest later!  I'll even give you a massage, so keep giving it to me!");
	Render.text("</i>\"");
	...
	if(player.cumQ() <= 200) Render.text("  The table already seems like it'll be stained white by Urta's efforts alone, and your own fluid doesn't help the situation.");
	else if(player.cumQ() < 1000) Render.text("  The two of you are already fucking in a small pool of your mixed semen sauce, making naughty sounds heard all over the bar, and her vagina, dripping with your fluids, only makes the pool seem bigger.");
	else if(player.cumQ() >= 2000) Render.text("  Your mixed sperm is covering the table and the floor, and woe to anyone who will have to clean this place up.  Despite the ridiculous amount of fluids around, she seems set on the idea that the two of you are barely mid-way through the act.");
	...
	if(player.cumQ() >= 1000) Render.text("  Well, maybe you can, but let's make some twins!");
	Render.text("  Keep going!</i>\"");
	...
	if(player.tallness <= 93) Render.text("making you lick, kiss and suck on her flared, wide tip");
	else Render.text("staring deep into your eyes as her erection smacks you against your [chest]");
	Render.text(".");
	if(!urtaLove()) Render.text("  She grins at your lack of resistance.");
	else Render.text("  \"<i>You're mine, right?  Right now, you're mine and mine alone.  My wonderful virile breeder, all " + player.mf("his","her") + " seed just for me.</i>\"");
	...
	if(urtaLove()) Render.text("  Just a few more loads, okay?  You don't mind, lover?  I'm practically addicted, so let me just fucking overdose!");
	Render.text("</i>\"");
	...
	if(player.stats.cor > 35 || flags[FlagEnum.PC_FETISH] > 0) Render.text("You tell her it was actually a great experience to have such passionate sex publicly.  She blushes at that suggestion.");

	//[If Urta is infertile]
	if(!urtaQuestDone()) 
	{
		Render.text("\n\n\"<i>Well, I guess it was nice, if painful afterwards, to be of use to someone who was so ready and willing to breed.");
		if(urtaLove()) Render.text("  If only I could... uh, I guess I should just enjoy what I can.");
		Render.text("</i>\"");
	}
	//[If Urta is fertile]
	else {
	...
	Render.text("\n\nShe ");
	if(!urtaLove()) Render.text("winks at you as you get ready to leave.  I hope I'll get to play with you again soon.</i>\"");
	else {
		Render.text("gives you a quick but loving kiss before getting up and starting to prepare to leave the house.  As you get ready to move, she mumbles something, and reaches, grabbing your arm, shyly.\n\n\"<i>[name], next time, umm: find me when I'm sober.  I'll get to see how it feels to be wanted by someone the way I usually crave you, okay?</i>\"");
