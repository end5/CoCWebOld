...
internal function approachSoberUrtaHeatRutProc():void {
	...
	if(player.inRut && player.lowerBody.cockSpot.hasCock()) outputText("  Urta is right next to you, and she has already proven herself a good lay");
	...
	if(player.inHeat && player.lowerBody.vaginaSpot.hasVagina()) outputText("  You... smell good, so I at least hope you're not here to tease me.");
	outputText("</i>\"  She leans closer to you, and ");
	if(player.inRut && player.lowerBody.cockSpot.hasCock()) outputText("your eyes are quickly drawn to her full breasts as she whispers something to you. You're too hypnotized to care.");
	...
	if(player.inRut && player.lowerBody.cockSpot.hasCock()) outputText(" focus is on the hips of the fox-morph");
	...
	if(player.inRut && player.inHeat && player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) outputText(" put a baby in her, then let her put a baby in you");
	else if(player.inRut && player.lowerBody.cockSpot.hasCock()) outputText(" put a baby in her so she can push it out of those beautiful hips");
	...
	if(player.inRut && player.inHeat && player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) outputText("mixed genitals");
	else if(player.inRut && player.lowerBody.cockSpot.hasCock()) outputText("[cock biggest]");
	...
	//Player in rut:
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your " + multiCockDescriptLight() + " and [vagina] are filling your clothes with mixed juices");
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" and your pussy dripping and eager");
		outputText(".");
		menu();
...
private function notInterestedInUburDrunkUrtaRuts():void {
	...
	if(player.inRut && player.lowerBody.cockSpot.hasCock()) outputText("[EachCock] twitches in protest at you leaving a willing lay behind.  ");
	//[Medium lust increase] 

	//[If the PC has a snake tongue]
	if(player.tongueType == TONUGE.SNAKE) outputText("You instinctively taste the air, and Urta suddenly seems utterly delicious. ");
	//Rut: 
	if(player.inRut && player.lowerBody.cockSpot.hasCock()) outputText("Your " + multiCockDescriptLight() + " hardens in your pants as you still force yourself to leave.");
	//[Medium lust increase]
...
private function drunkUrtaIntroPartDuex(chosenSex: number = 1, newl:boolean = true):void {
	...
	if(player.lowerBody.balls > 0 && chosenSex == 1) outputText("your [balls], rolling them in her hand and squeezing a bit too roughly.");
	...
	//Player in both : 
	if(player.inRut && player.inHeat && player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) outputText("and she proceeds to maul her breasts, pre-cum seeping copiously from her cock and her snatch drooling over her balls and onto the floor.");
...
private function keepTeasingDatHornyFawkes():void {
	...
	if(player.lowerBody.balls > 0) outputText("lewdly cupping and fondling your [balls], ");
	...
	if(player.stats.cor < 50) outputText("run");
...
private function soberUrtaSatingPCHeat():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.hasCock() && player.lowerBody.balls > 0) outputText(" and [balls]");
		outputText(" out of the way");
	}
	else if(player.lowerBody.balls > 0) outputText(" by shoving your [balls] out of the way");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("[cock biggest]");
		...
		outputText("clitoris, ");
		if(player.lowerBody.vaginaSpot.list[0].clitLength < 2.5) outputText("rubbing the hard, stimulated sensitive spot between her fingers");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" Cum rushes out of [eachCock] and onto the soft fur of Urta's abdomen, then starts leaking out slowly in time with her thrusts.");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("your own member responding alike to Urta's, shooting out semen, ");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("and " + multiCockDescriptLight() + " ");
	...
	//[Hermaphrodite]
	if(player.lowerBody.cockSpot.hasCock()) outputText("[eachCock] shoots its seed onto the floor of the alley, helplessly");
...
private function sateRutWithSoberUrta():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(", trigging a mutual explosion from your [vagina]");
...
private function drunkenUrtaFucksPCInHeat():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) 
	{
		...
		if(player.lowerBody.balls > 0) outputText(" and your [balls]");
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength < 2.5) outputText("playing with your [clit]");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("nor even your " + multiCockDescriptLight());
...
private function sateRutWithDrunkUrtaWithHugeDick():void {
	...
	if(player.lowerBody.balls > 0) outputText("against your own [balls]");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("against your [clit]");
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() < 150) outputText("compete with minotaurs or something?");
...
private function drunkUrtaRidesARutPCsCock():void {
	...
	if(player.lowerBody.balls > 0) outputText("feel your balls churning");
	...
	if(player.cumQ() >= 500 && player.cumQ() <= 1500) outputText("\n\nUrta seems positively full and stretched now, and your rut almost subsides for a moment, her stretched tummy imitating a pregnancy quite well.");
		...
		if(player.lowerBody.balls > 0) outputText("[balls]");
		else if(player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.vaginaSpot.list[0].clitLength >= 3.5) outputText("cock-like clit");
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("pussy lips");
	...
	if(player.lowerBody.balls > 0) outputText("  Your [balls] don't seem to be completely empty yet");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.balls == 0) outputText(" the inside of your thighs");
	else if(player.lowerBody.balls > 0) outputText(" your [balls]");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" your [clit]");
	...
	if(player.lowerBody.balls > 0) outputText("[balls]");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("[vagina] and [clit]");
	...
	if(player.lowerBody.balls > 0) outputText(", your [balls] frantically gathering any cum that is left in them to prepare for another surge of shots into the hermaphrodite's eager nethers");
	...
	if(player.stats.cor > 35 || flags[FlagEnum.PC_FETISH] > 0) outputText("You tell her it was actually a great experience to have such passionate sex publicly.  She blushes at that suggestion.");
