export default class UrtaHeatRut extends NPCAwareContent {
	...
	internal function approachSoberUrtaHeatRutProc(): void {
		...
		urta.urtaSprite();
		MainScreen.text("As you walk over to the table where your foxy herm-lover sits, you think you see eyes following you, your presence attracting an unusual amount of attention. ");
		if (player.inHeat && player.inRut && player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" Males, females and herms alike stare at you with a mixture of puzzlement, longing and hostility, as if unsure whether they want to fight with you, fuck you senseless or be fucked senseless by you.");
		else if (player.inRut && player.lowerBody.cockSpot.hasCock()) MainScreen.text(" Males stare at you with a hint of challenge in their eyes, whilst females eye you attentively. Herms don't seem sure whether they want to fight you or fuck you.");
		//(Player in heat
		else if (player.inHeat && player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" Males, and individuals who look like females at first, until you spot the telltale bulge of at least one cock in garments or hanging freely between their legs, stare at you with obvious desire as you go, more than a few erections starting to form in your wake.");
		MainScreen.text("  You ignore them all, concentrating on Urta.");

		MainScreen.text("\n\nAs you approach, ");

		if (urtaLove()) MainScreen.text(" she opens her arms to embrace you, and smiles.");
		else MainScreen.text(" she smiles when she sees you, but then gives the air a sniff and immediately seems to gain more color to the skin under her fur.  \"<i>Uh, I, I see.</i>\"  You sit down next to her, but even though she isn't even suggesting anything yet, you already find it hard to control yourself.");

		if (player.inRut && player.lowerBody.cockSpot.hasCock()) MainScreen.text("  Urta is right next to you, and she has already proven herself a good lay");
		else MainScreen.text("  You keep reminding yourself what Urta is packing between her legs and how much precious semen it can give, and can't help but want to try that now");

		MainScreen.text(".  Not to mention, with or without the monster between her legs, her body is sexy as hell.");

		MainScreen.text("\n\n\"<i>Well, you do help me with my own releases, so I suppose that's fair....");
		if (player.inHeat && player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  You... smell good, so I at least hope you're not here to tease me.");
		MainScreen.text("</i>\"  She leans closer to you, and ");
		if (player.inRut && player.lowerBody.cockSpot.hasCock()) MainScreen.text("your eyes are quickly drawn to her full breasts as she whispers something to you. You're too hypnotized to care.");
		else MainScreen.text("you lose yourself in your imagination about having Urta take you raw and pump you full of sperm.  There's a faint voice you hear, and you realize it's Urta's as you snap back to reality.");
		MainScreen.text("  She coughs and repeats herself.  \"<i>Are you here because of your, um, condition? You'd prefer to blow off some steam with ");
		if (urtaLove()) MainScreen.text("your adorable, hot vixen");
		else MainScreen.text("someone you know and trust");
		...
		if (player.inRut && player.lowerBody.cockSpot.hasCock()) MainScreen.text(" focus is on the hips of the fox-morph");
		else MainScreen.text(" focus on catching a glimpse of that amazing cock, and you keep imagining what she can do with it");
		MainScreen.text(".  Your aggressive hormonal state demands that you ");
		if (player.inRut && player.inHeat && player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" put a baby in her, then let her put a baby in you");
		else if (player.inRut && player.lowerBody.cockSpot.hasCock()) MainScreen.text(" put a baby in her so she can push it out of those beautiful hips");
		else MainScreen.text(" put that beautiful big cock into your needy [vagina]");
		//[If Urta's fertility quest wasnât done]
		if (!urtaQuestDone()) MainScreen.text(", even if something keeps reminding you that's impossible.");
		else MainScreen.text(".");

		MainScreen.text("\n\n\"<i>You know, [name] I'm glad to see you as always.  ");
		if (urtaLove()) MainScreen.text("  I'm really thankful you managed to control yourself and see me with this.  You know that if you need me, I'm always ready to help, love.</i>\"");
		else MainScreen.text("  I'm really flattered you thought of me when in such a state!</i>\"");
		...
		if (player.inRut && player.inHeat && player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("mixed genitals");
		else if (player.inRut && player.lowerBody.cockSpot.hasCock()) MainScreen.text("[cock biggest]");
		else MainScreen.text("[vagina]");
		MainScreen.text(" against her backside.");

		MainScreen.text("\n\n\"<i>Um, you know, I'm horny, too, but at least wait 'til we get to my place...</i>\"");

		//Player in heat and rut:
		if (player.inRut && player.inHeat && player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) {
		...
		//Player in rut:
		else if (player.inRut && player.lowerBody.cockSpot.hasCock()) {
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Your " + CockDescriptor.describeMultiCockShort(player) + " and [vagina] are filling your clothes with mixed juices");
			else MainScreen.text("  Your [cock biggest] begins to seep precum into your clothes");
			MainScreen.text(", so you decide to dispose of them as you force Urta against a wall.");
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" and your pussy dripping and eager");
			MainScreen.text(".");
			menu();
	...
	internal function approachDrunkenUrta(): void {
		...
		//Player in heat:
		else MainScreen.text("  Males, and individuals who look like females at first, until you spot the telltale bulge of at least one cock in garments or hanging freely between their legs, stare at you with obvious desire as you go, more than a few erections starting to form in your wake.");
		MainScreen.text("  You ignore them all, concentrating on Urta.");
	...
	private notInterestedInUburDrunkUrtaRuts(): void {
		MainScreen.clearText();
		urta.urtaSprite();
		MainScreen.text("You say it's her who seems to be different today, and gently push her away, before apologizing and making your leave.  ");
		if (player.upperBody.head.face.faceType == 2) MainScreen.text("Your nose does seem to pick up that Urta was and still is getting unusually aroused behind you.  ");
		if (player.inRut && player.lowerBody.cockSpot.hasCock()) MainScreen.text("[EachCock] twitches in protest at you leaving a willing lay behind.  ");
		//[Medium lust increase] 

		//[If the PC has a snake tongue]
		if (player.upperBody.head.face.tongueType == TongueType.SNAKE) MainScreen.text("You instinctively taste the air, and Urta suddenly seems utterly delicious. ");
		//Rut: 
		if (player.inRut && player.lowerBody.cockSpot.hasCock()) MainScreen.text("Your " + CockDescriptor.describeMultiCockShort(player) + " hardens in your pants as you still force yourself to leave.");
		//[Medium lust increase]
		else MainScreen.text("Your condition does seem kind of protesting against leaving such a willing lay behind, but it isn't time to be indulging it.");
	...
	private drunkUrtaIntroPartDuex(chosenSex: number = 1, newl: boolean = true): void {
		...
		MainScreen.text("Suddenly, Urta's hand dashes to ");
		//[if players has balls]
		if (player.lowerBody.balls > 0 && chosenSex == 1) MainScreen.text("your [balls], rolling them in her hand and squeezing a bit too roughly.");
		else if (player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("your [clit], rubbing it until it's ");
			if (chosenSex == 1) MainScreen.text("the very same state [eachCock] seems to be in so often recently, ");
			MainScreen.text("erect and throbbing.");
		}
		else MainScreen.text("your [hips], stroking along them towards [eachCock] in an agonizing display of teasing.");
		...
		if (chosenSex == 1) MainScreen.text(", and [eachCock] already gets hard in your [armor].");
		else MainScreen.text(", and you feel the inside of your [armor] rapidly slickened by your [vagina]'s excitement.");
		...
		//Player in both : 
		if (player.inRut && player.inHeat && player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("and she proceeds to maul her breasts, pre-cum seeping copiously from her cock and her snatch drooling over her balls and onto the floor.");
		else if (chosenSex == 1) MainScreen.text("but she doesn't seem to mind it for the moment.  Rather, the fur between her thighs appears to be rapidly soaking.");
		else MainScreen.text("which she seems more than intent to push into you, although you do see a bit of female lubricant sliding down her balls.");
		...
		if (chosenSex == 1) MainScreen.text("hard");
		else MainScreen.text("wet");
		MainScreen.text(" for me</i>\"  Moments later, as you feel your naked [butt] against the table, her eyes dart to your ");
		if (chosenSex == 1) MainScreen.text(CockDescriptor.describeMultiCockShort(player));
		else MainScreen.text("[vagina]");
		MainScreen.text(".  Her smile opens wider as Urta presents you with a sexy face that seems a mix of positively delighted and predatory.");

		//[If player is in Rut, OR is in Heat AND Rut AND chose "Cock":
		if (chosenSex == 1) MainScreen.text("\n\n\"<i>Well, I'm sure you don't mind if I take all your spunk and those few hours of fucking all for myself, instead,</i>\" she says as she brings one leg onto the table and the side of you as she sheds her dress, lubricant now drooling from the fox's female half, falling onto your [hips].");
	...
	private stopTeasingDatHornyFox(): void {
		MainScreen.clearText();
		urta.urtaSprite();
		MainScreen.text("You decide there's no point taking things too far - besides, Urta seems just about ready to pop now, especially with how she's frantically suckling at her own cockhead.");
	...
	private keepTeasingDatHornyFawkes(): void {
		MainScreen.clearText();
		urta.urtaSprite();
		MainScreen.text("You smirk; things are just starting to get fun around here, why would you stop now?  You start to amp up your routine even more, flaunting your scents and your goods for all the bar's occupants, not just the drunken fox-herm feverishly jerking herself off in front of you.  You can't recall if you've ever danced before coming to Mareth, but you don't think you do too bad as you perform an amateur strip-dance for the audience around you, slowly peeling off your [armor] and sensuously discarding them, bending over to let Urta and any other cock-bearing character present in the bar get a good look at your [vagina], ");
		if (player.lowerBody.balls > 0) MainScreen.text("lewdly cupping and fondling your [balls], ");
		MainScreen.text("slowly beginning to stroke and caress your " + CockDescriptor.describeMultiCockShort(player) + " for the benefit of those who bear vaginas.");
		...
		if (player.lowerBody.isGoo() || player.lowerBody.isNaga()) MainScreen.text("what passes for knees on your [legs]");
		...
		if (player.stats.cor < 50) MainScreen.text("run");
		else MainScreen.text("walk triumphantly");
		MainScreen.text(" from the racous orgy that is the Wet Bitch; truly an auspicious name.");
	...
	private soberUrtaSatingPCHeat(): void {
		...
		MainScreen.text("As she presses your back against the wall, you spread your [legs] wide, allowing Urta easy access to your awaiting womanhood");
		//[If herm]:
		if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text(" by shoving your " + CockDescriptor.describeMultiCockShort(player));
			if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.balls > 0) MainScreen.text(" and [balls]");
			MainScreen.text(" out of the way");
		}
		else if (player.lowerBody.balls > 0) MainScreen.text(" by shoving your [balls] out of the way");
		MainScreen.text(".  Slowly, you feel the wide flared head of her magnificent pillar of flesh press onto your opening, spreading your vaginal lips and making your [clit] harden in anticipation just as your [butt] shivers, awaiting the intrusion.  Slowly, Urta's giant member starts to enter you.");
		if (player.vaginalCapacity() >= 100) MainScreen.text("  Your hungry gash eagerly welcomes her, Urta easily sliding herself home.");
		else if (player.vaginalCapacity() > 60) MainScreen.text("  You feel it stretch you in the most pleasuring of ways, filling you with warm, throbbing flesh as she pushes it deeper and deeper.");
		else MainScreen.text("  You wince in pain at being stretched so much, but you grow wetter and wetter as Urta slowly pushes herself inside your depths, your [vagina] slowly getting wetter and warmer, accepting the intruder.  Pleasure starts to override pain and you let out a gasp.");
		...
		//[If Very Loose] 
		if (player.vaginalCapacity() >= 60) MainScreen.text("Urta's erection seems completely at home in your depths, completely embraced by the wet walls of your throbbing [vagina].");
		else MainScreen.text("You moan again, feeling Urta's member stretch, harden and throb, blood running through it.  You feel complete, and your heat is ignited further by the sensation of something so wonderful and big filling you.");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("[cock biggest]");
			...
			MainScreen.text("clitoris, ");
			if (player.lowerBody.vaginaSpot.get(0).clitLength < 2.5) MainScreen.text("rubbing the hard, stimulated sensitive spot between her fingers");
			else MainScreen.text("running her fingers over your entire big clit before stroking it like she'd do with a cock");
		}
		MainScreen.text(", letting you have a few moments to get used to her mammoth member and accommodate it.");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" Cum rushes out of [eachCock] and onto the soft fur of Urta's abdomen, then starts leaking out slowly in time with her thrusts.");
		MainScreen.text(" Your cunt clamps down tighter onto Urta as you orgasm,");
		if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 4) MainScreen.text(" pussy juice shooting out around her member, coating Urta's fur and soaking her balls and the base of her cock.");
		else MainScreen.text(" love juices flowing liberally out of you, giving the skin of Urta's cock a visible shine and spreading onto her balls.");
		...
		if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 4) MainScreen.text("shooting out all over the place");
		else MainScreen.text("running down her member and the crack of your [butt], coating the two of you in the proof of your passion");
		MainScreen.text(".");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("your own member responding alike to Urta's, shooting out semen, ");
		MainScreen.text("your [vagina] ");
		if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 4) MainScreen.text("once again setting off to coat Urta in your juices.");
		else MainScreen.text("squeezing on her, trying to milk her of her seed.");
		...
		MainScreen.text("\n\n\"<i>What?  ");
		if (!urtaLove()) MainScreen.text("You're still not satisfied?</i>\"");
		else MainScreen.text("You want to keep going, darling?  I want to make you satisfied, so I'll spare all the time I can!</i>\"");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("and " + CockDescriptor.describeMultiCockShort(player) + " ");
		MainScreen.text("hardening again in response to the act.  Urta continues to slam into your backside, delivering a small smack to your [butt].");
		...
		//[Hermaphrodite]
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("[eachCock] shoots its seed onto the floor of the alley, helplessly");
		else MainScreen.text("your [vagina] squeezes and tugs on her shooting member, leaking its own juices all over her member, the intensity of the orgasm shaking you right into your core");
		MainScreen.text(".");
		...
		MainScreen.text("\n\n\"<i>That felt amazing!  ");
		if (!urtaLove()) MainScreen.text("If only I had you by hand every time I need to blow a load.");
		else MainScreen.text("Your pussy is absolutely amazing, sweetheart: I hope I've helped you settle down?  Is your heat sated for the moment?");
		MainScreen.text("</i>\"");
		...
		MainScreen.text("\n\n\"<i>");
		//([Regular Urta]
		if (!urtaLove()) MainScreen.text("Sure thing, I'm here to help,</i>\" she responds with a wink.");
		else MainScreen.text("I'm here if you need me, love.  Whatever makes you happy.  You've helped me with my own problems so much already, so I owe you at least one.  Not to mention, making love to you feels absolutely amazing!</i>\"");
		...
		if (!urtaQuestDone()) {
			...
			MainScreen.text("  She grabs your hand and helps you stand up.");
		}
	...
	private sateRutWithSoberUrtaButHuegDicked(): void {
		...
		MainScreen.text("Urta observes your approaching " + CockDescriptor.describeCock(player, x) + " with a hungry, eager expression. Without taking her eyes off of it, she flips up her ");
		if (!urtaLove()) MainScreen.text("skirt");
		else MainScreen.text("dress");
		MainScreen.text(", her huge horse-cock whipping up to slap between her breasts, pre already dribbling down the tip.  However, as you get closer, her expression suddenly falls and her cock begins to sink.  \"<i>I'm sorry, but... that's just too big!</i>\" she protests.  \"<i>You smell absolutely gorgeous, and I'm willing to try with something a little bigger than usual, but that's... that's just frigging huge - there's no way I could fit that monster!</i>\" she declares. \"<i>Just because I have a stallion cock doesn't mean I have a mare's cunt!</i>\"");

		MainScreen.text("\n\nYou stare at your " + CockDescriptor.describeCock(player, x) + " in an uncomprehending fashion.  How are you going to find relief now?  You look up as Urta saunters over towards you in a seductive manner, erect cock jutting before her and a mischievous grin on her face.  \"<i>While I'll have to go and attend to this myself later, I can still give you a hand...</i>\" she purrs throatily.  She kneels before you, and you have to watch yourself to avoid having her twenty inches of cock slap your [leg].  Confidently, she reaches out to take hold of your " + CockDescriptor.describeCock(player, x) + ", first with one hand, and then the other.");
		...
		if (!urtaLove()) {
			...
			if (player.cumQ() <= 50) MainScreen.text("spattering spooge onto Urta's face");
			else if (player.cumQ() <= 250) MainScreen.text("splashing her face and tits with your cum");
			else MainScreen.text("soaking the shocked fox-morph in your jism");
			MainScreen.text("; though you still feel the urge to breed fertile females, your lust is momentarily sated.  Urta wipes ");
			if (player.cumQ() >= 500) MainScreen.text("halfheartedly ");
			MainScreen.text("at the cum on her faces and gives you a friendly smile.  \"<i<There, that should feel better; I know what it's like to be all pent up with no release.</i>\"  She rises to her feet, still holding your cock, and gives it a squeeze.  \"<i>In the meantime, if you want to really cut loose, I suggest you go and find something to shrink this down.</i>\"");
		...
		//Lover Urta Path/
			...
			MainScreen.text("\n\nShe pulls her head free and gasps for breath, looking at you in admiration. \"<i>By Marae... how do you manage to do this for me, lover?</i>\" she asks, stunned.  But then, gamely, she returns her attention to your " + CockDescriptor.describeCock(player, x) + ", licking, suckling and mouthing at the head.  As she does this, she continues her restless assault with her hands upon the rest of your lengthy shaft with her skilled hands.  She shuffles closer, allowing you to rest yourself between her DD-cups.  \"<i>Can you feel how fast my heart is beating through your cock?  You turn me on so very much,</i>\" she purrs.");

			MainScreen.text("\n\nIt's no surprise that eventually you can't hold back anymore and release yourself into Urta's ready mouth.  ");
			if (player.cumQ() <= 50) MainScreen.text("She swallows it all without complaint, licking her lips clean of any stray droplets once you remove it from her mouth.");
			else if (player.cumQ() <= 250) MainScreen.text("Her cheeks bulge at the amount you pump into her, but she's able to swallow with an admirable amount of control, gasping for breath.");
			else MainScreen.text("Rivulets of cum burst from between her lips, but she stubbornly sucks and swallows as much as she can, her stomach starting to bulge from the sudden influx of spooge.  When you stop, she is drooling cum and gasping for breath, but she meets your eyes with pride, even as she gives a wet belch.");
	...
	private sateRutWithSoberUrta(): void {
		...
		MainScreen.clearText();
		urta.urtaSprite();
		MainScreen.text("Urta observes your approaching package with a hungry, eager expression. Without taking her eyes off of it, she flips up her skirt, her huge horse-cock whipping up to slap between her breasts, pre already dribbling down the tip.");
		...
		MainScreen.text(" she mumbles around your tongue, so caught in lust she doesn't care where you are.  Indeed, it seems your little fox-herm has something of an exhibitionist streak...");

		MainScreen.text("\n\nWell, you don't intend to keep the lady waiting any longer then you have to, and you slide it home, allowing her testicles to drop once you are safely inside.  The globular organs bounce atop your " + CockDescriptor.describeCock(player, x) + " and rock back and forth against your belly, jiggling with every movement the two of you make, Urta's cock sandwiched firmly between your bodies and held between her breasts.");
		...
		if (urtaLove()) MainScreen.text("blows you a kiss with a flirty wink of her eye and then ");
		MainScreen.text("scrubs off the worst of the cum plastered in her fur with the dry parts of her clothes.  She saunters playfully over to a garbage bin, throwing them inside.");
		if (urtaLove()) MainScreen.text("  Her hips deliberately sway in a mesmerizing fashion, tail flicking from side to side to give you a tantalizing grip of her still-dripping snatch and the now-flaccid prick underneath.");
		MainScreen.text("  You watch as she pries at a loose stone with her claws, removing it to reveal a small cubby, from which she withdraws a spare set of clothing.");

		//If fertility quest done:
		if (urtaQuestDone()) {
			MainScreen.text("\n\nYou ask Urta what she's going to do; you didn't have the chance to use a condom.");
			if (Flags.list[FlagEnum.URTA_FERTILE] == 1) MainScreen.text("\n\n\"<i>We'll just have to hope then, won't we, [name]?</i>\"");
			...
			if (!urtaLove()) MainScreen.text("\"<i>And you had better learn to control yourself, " + player.mf("mister", "missy") + ".");
			else MainScreen.text("\"<i>But you should learn to control yourself lover-" + player.mf("boy", "girl") + ".");
			MainScreen.text("  I'd rather do something like this in the privacy of my home than in the middle of the street.</i>\"");
	...
	private drunkenUrtaFucksPCInHeat(): void {
		MainScreen.clearText();
		urta.urtaSprite();
		MainScreen.text("You simply smile at her, inhaling deeply through your nose; you can swear you can smell her herm-musk, wafting off of her as strongly as your own pheromones must be wafting off of you.  That wonderful, earthy, primordial scent, the odor of a healthy breeder ready and willing to mate... there's nothing like it.");
		//(Fertility Quest not done:
		if (!urtaQuestDone()) MainScreen.text("  You know that she can't give you babies... but, honestly, at this moment, you couldn't care less. Her wonderful stallion cock is there and ready and it's all for you.");
		else MainScreen.text("  You dimly recall that Urta could be virile, but she's right there, her huge mare-cock just dripping to plunge into your ripe [vagina] and knock you up.  You don't care anymore; you want her babies!");

		MainScreen.text("\n\nUrta's eyes glitter with emotion and she bends down to give you another kiss before roughly grabbing her ");
		if (!urtaLove()) MainScreen.text("skirt");
		else MainScreen.text("dress");
		MainScreen.text(" and hiking it up to expose her pre-spewing cock.  A steady trickle of feminine lubricant dribbles out of her gash, oozing over her apple-sized balls and dripping noisily onto the floor, but Urta couldn't care less; she's too fixated on plunging her maleness into your ready pussy to notice her feminine urges.");
		...
		if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text(", impatiently nudging aside your " + CockDescriptor.describeMultiCockShort(player));
			if (player.lowerBody.balls > 0) MainScreen.text(" and your [balls]");
			MainScreen.text(" to get to the feminine orifice below");
		}
		MainScreen.text(". You cry out in joy as your [vagina] is filled, her soft fur brushing against your inner walls before she opens her mouth as wide as she can and sticks out her tongue. The long, broad, vulpine appendage slides out and up your depths, caressing the walls, lapping up your juices, ");
		if (player.lowerBody.vaginaSpot.get(0).clitLength < 2.5) MainScreen.text("playing with your [clit]");
		else MainScreen.text("lapping around your huge pleasure-button, slurping it into her mouth where she suckles from it");
		MainScreen.text(", and making you thrash and scream until you think you're going to cum from the pussy-eating alone.  ");
		if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 4) MainScreen.text("As if to emphasize this, your squelching, slurping pussy suddenly sprays fem-cum all over Urta's face, making her pull herself free with a shocked shake of her head.");
		else MainScreen.text("As if sensing your impending orgasm, Urta removes her head from your [vagina], giving you a cocky grin and shaking her head.");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("nor even your " + CockDescriptor.describeMultiCockShort(player));
	...
	private sateRutWithDrunkUrtaWithHugeDick(): void {
		...
		if (player.lowerBody.cockSpot.biggestCocks[0].cockArea() < 150) MainScreen.text("compete with minotaurs or something?");
		else MainScreen.text("set a world record or something?");
		MainScreen.text("</i>\"\n\n");
	...
	private drunkUrtaRidesARutPCsCock(): void {
		...
		let x: number = player.biggestCockIndex();
		urta.urtaSprite();
		MainScreen.text("You decide not to fight the situation; truthfully, the promises given by the sight of the hot hermaphrodite vixen climbing on top of you ignite your lusts and sing to your rut in ways you don't think you'd be able to safely ignore.");

		MainScreen.text("\n\nUrta lifts her balls and presses your already hard cock against her black pussy lips, justifying your decision.  Slowly, you feel your tip sink into her, then the rest of your member, as she releases a soft moan.  She keeps pressing down, forcing your " + CockDescriptor.describeCock(player, x) + " inside her wet gash until you feel her butt pressed against your ");
		if (player.lowerBody.balls > 0) MainScreen.text("[balls]");
		else MainScreen.text("[hips]");
		MainScreen.text(", and her own balls resting comfortably on your abdomen.  She grits her teeth as you feel her pussy slowly tighten around and massage your " + CockDescriptor.describeCock(player, x) + ", and you groan in response.  Urta starts to bounce up and down on your member, smacking her thighs and ass into your body, not minding her own giant equine member flopping around and occasionally smacking you over your chest.  She lets her tongue flick out into the air for a moment and moans in delight, before picking up the pace and starting to pump your cock in and out of her hungry passage furiously.");
		...
		if (player.lowerBody.balls > 0) MainScreen.text("feel your balls churning");
		else MainScreen.text("recognize the tingling sensation from your " + CockDescriptor.describeCock(player, x) + " to your spine");
		MainScreen.text(", a telltale sign that your penis is ready to unload the potent spunk inside the herm-nympho bouncing on top of you.  You try to announce this, but she doesn't slow down.  Moments later, ");
		//[Normal cum production]
		if (player.cumQ() <= 50) MainScreen.text("you unload your thick spunk into her pussy");
		else if (player.cumQ() <= 250) MainScreen.text("you erupt inside her, filling her with wave after wave of warm, thick semen");
		else MainScreen.text("you blow massive surges of spunk inside her continuously.  The fox's stomach expands, rivulets of semen running down onto the table as she continues her ride on top of you");
		MainScreen.text(".  As you discharge, the fox's pussy starts squeezing and contracting, and you recognize the hermaphrodite's own orgasm.  Thicker white liquid suddenly drips from the tip of her cock quite copiously and occasionally shoots out into the air.");
		...
		if (player.hasKnot(x)) MainScreen.text("You slowly feel the blood flow to your erect penis, and the knot at the base of it seems to start to expand");
		else MainScreen.text("You don't mind staying like this for much, much longer, and the hermaphrodite on top of you seems to share that point of view");
		MainScreen.text(".  Summoning your slightly exhausted strength, you pull her hips down deeper into your own before starting to return the hermaphrodite's shakes and bounces with thrusts of your own.  It intensifies the sensations, making your lover yelp in surprise before smiling widely and releasing low, content moans and purrs from the depths of her throat.");
		...
		if (player.hasKnot(x)) MainScreen.text("knot surge with more blood and your erection twitch, ensuring that you're completely trapped inside her until you're empty.");
		else MainScreen.text("tip expand as you're getting ready to blow another load.");
		MainScreen.text("  Urta moans on top of you, increasing the speed of her movements and running her hand up and down her own member faster and with more intense strokes.");
		...
		MainScreen.text(" Urta masturbates furiously as your body and your " + CockDescriptor.describeCock(player, x) + " finally gives in to your rut and the vixen's intense lust.  The two of you come in complete sync this time, her wet, black pussy milking you with vice-like tightness and intense, rhythmic squeezing as she fires a huge rope of jizz onto the floor next to your table before painting your chest and abdomen with lesser white strings.");

		//[If large cum production]
		if (player.cumQ() >= 500 && player.cumQ() <= 1500) MainScreen.text("\n\nUrta seems positively full and stretched now, and your rut almost subsides for a moment, her stretched tummy imitating a pregnancy quite well.");
		else if (player.cumQ() > 1500) {
			MainScreen.text("\n\nUrta is stretched and completely full, so, holding your " + CockDescriptor.describeCock(player, x) + " in her hand for a moment, she lifts herself off your cock, presses on her abdomen and allows some of your spunk to drip out of her, coating the table, your thighs and your ");
			if (player.lowerBody.balls > 0) MainScreen.text("[balls]");
			else if (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.vaginaSpot.get(0).clitLength >= 3.5) MainScreen.text("cock-like clit");
			else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("pussy lips");
			else MainScreen.text("inside of your thighs");
			MainScreen.text(" with your own thick white semen.  Then, she slides your softening erection back into herself again.");
		}

		MainScreen.text("\n\n\"<i>Good... so good!  Just a few more rounds, and we'll be done - don't go fucking soft on me!</i>\"");

		MainScreen.text("\n\nYour " + CockDescriptor.describeCock(player, x) + " feels sensitive and tender after the two past orgasms, but the horny nympho just forces it deep into her snatch, the sensitive head and shaft all the way down to your base again squeezed tighter as she starts to bounce up and down, trying to breathe life into your erection.  She kisses you furiously and passionately, exploring your mouth with her tongue as she keeps the stimulation on your groin, cooing, groaning and panting as if trying to breathe her own lust into you.");

		MainScreen.text("\n\n\"<i>Come on, come on, whatever it takes!  Squeeze those boobies, smack that ass, just get fucking hard!</i>\"");

		MainScreen.text("\n\nFollowing her plea, you give her breasts a rather rough squeeze, and you feel your " + CockDescriptor.describeCock(player, x) + " miraculously harden inside her once again.");
		if (player.lowerBody.balls > 0) MainScreen.text("  Your [balls] don't seem to be completely empty yet");
		else MainScreen.text("  It doesn't appear to be the end of what your body can produce");
		MainScreen.text(", and your vulpine lover squeals with delight as she feels you harden inside her.  She smacks your [chest] with her huge, half-hard member.");

		MainScreen.text("\n\n\"<i>Care to give me a hand with that?  There's quite a lot of cream to squeeze out of this one, too.</i>\"");

		MainScreen.text("\n\nYour tired and lust-hazed mind sees no reason not to, and you grab her, so Urta's massive mare member starts to be pleasured by her own hand as well as yours.  Squealing in delight, the horny vixen returns to what seems to occupy her mind completely: bouncing fervently on top of you, with your " + CockDescriptor.describeCock(player, x) + " trapped in her wet snatch.");
		...
		if (!player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.balls == 0) MainScreen.text(" the inside of your thighs");
		else if (player.lowerBody.balls > 0) MainScreen.text(" your [balls]");
		else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" your [clit]");
		MainScreen.text(" when she's pressed entirely against you.  She smacks her own bottom and you run your other hand over her body as the two of you tug on and pull her massive erection towards your face and over your [chest], thick dollops of pre-cum and leftover semen dripping from her member over your body.  Urta licks her lips as she looks down on you with ");
		if (!urtaLove()) MainScreen.text("intense, unadultered lust");
		else MainScreen.text("possessiveness and a hungry, but somehow warm expression");
		MainScreen.text(".");

		MainScreen.text("\n\nI should make you do this every day!</i>\"");
		if (urtaLove()) MainScreen.text("  She suddenly seems a bit shy and surprised, before saying, \"<i>Make sure to remind me I said that later....</i>\"");
		...
		if (!urtaLove()) MainScreen.text("Show me you're " + player.mf("the man", "the breeder") + " here!");
		else MainScreen.text("You don't want to make me feel like I'm abusing you, do you?  I know you want this, so pound me!</i>\"");


		MainScreen.text("\n\nAfter a while, it becomes a torture to resist the stimulation and you try to bring your own orgasm faster by responding to Urta's movement, stretching your [hips] and pushing your " + CockDescriptor.describeCock(player, x) + " deeper into her pussy.  Although the two of you have trouble catching a common rhythm, she appears way more vitalized and enthusiastic about this than you are.");

		MainScreen.text("\n\nAfter a long time of slightly too rough of a ride, your " + CockDescriptor.describeCock(player, x) + " twitches inside the horny vixen again, and she almost howls in her own climax before yours is set off. Another shot of cum coats your [chest] as her hungry love-hole milks your member, forcing you to ejaculate for the third time, delivering even more semen into her insides.");
		if (player.cumQ() >= 500) MainScreen.text("  If the two of you were thinking rationally, you'd know quite well it could be quite dangerous to her innards to continue, but Urta seems intent on continuing this ordeal, or at least, that's how you feel when looking at her expression of bliss and ecstasy, eyes rolling to the back of her head and tongue dancing around in the air.");

		MainScreen.text("\n\nYou try to announce that you're done and that's about it, but the horny nymphomaniac that seems intent on keeping your rutting " + CockDescriptor.describeCock(player, x) + " all to herself feels differently.  She allows your member to slide out of her for a moment, but only to let out some of the spunk that's filling her and to stimulate you in a different way.  Urta moves down your body, stroking your [chest] and your [nipples] before lowering her face and arms to your [hips].  She licks your sensitive head for a while before delivering a couple of noisy, strong sucks to your " + CockDescriptor.describeCock(player, x) + ".  Afterwards, she licks your ");
		if (player.lowerBody.balls > 0) MainScreen.text("[balls]");
		else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("[vagina] and [clit]");
		else MainScreen.text("taint");
		MainScreen.text(" before finally running her tongue over your [asshole], and all the way up again, swallowing your member once again as she squeezes your [butt].  You alternate before moaning, groaning, and whimpering as blood seems to leave your head and entire body, surging into your " + CockDescriptor.describeCock(player, x) + " and making it hard to think or move, much less resist.");
		...
		if (urtaLove()) MainScreen.text("  You can rest later!  I'll even give you a massage, so keep giving it to me!");
		MainScreen.text("</i>\"");
		...
		if (player.cumQ() <= 200) MainScreen.text("  The table already seems like it'll be stained white by Urta's efforts alone, and your own fluid doesn't help the situation.");
		else if (player.cumQ() < 1000) MainScreen.text("  The two of you are already fucking in a small pool of your mixed semen sauce, making naughty sounds heard all over the bar, and her vagina, dripping with your fluids, only makes the pool seem bigger.");
		else if (player.cumQ() >= 2000) MainScreen.text("  Your mixed sperm is covering the table and the floor, and woe to anyone who will have to clean this place up.  Despite the ridiculous amount of fluids around, she seems set on the idea that the two of you are barely mid-way through the act.");
		...
		if (player.cumQ() >= 1000) MainScreen.text("  Well, maybe you can, but let's make some twins!");
		MainScreen.text("  Keep going!</i>\"");
		...
		if (player.tallness <= 93) MainScreen.text("making you lick, kiss and suck on her flared, wide tip");
		else MainScreen.text("staring deep into your eyes as her erection smacks you against your [chest]");
		MainScreen.text(".");
		if (!urtaLove()) MainScreen.text("  She grins at your lack of resistance.");
		else MainScreen.text("  \"<i>You're mine, right?  Right now, you're mine and mine alone.  My wonderful virile breeder, all " + player.mf("his", "her") + " seed just for me.</i>\"");
		...
		if (player.lowerBody.balls > 0) MainScreen.text(", your [balls] frantically gathering any cum that is left in them to prepare for another surge of shots into the hermaphrodite's eager nethers");
		MainScreen.text(".  You seem to slowly lose the sensations in most of your body.  The awareness of your surroundings slips away, as all your thoughts and feelings focus in your groin, struggling to keep up with the mad pace of breeding that Urta has kept so far.  It's hard to keep conscious as you feel another surge of ecstasy pass all over your body and focus on your " + CockDescriptor.describeCock(player, x) + ", shooting off yet again inside her coaxing hole.  You hear the sound of your name being called and the milking, squeezing sensations that you barely manage to associate with her ravenous, insatiable pussy.");
		...
		if (player.stats.cor > 35 || Flags.list[FlagEnum.PC_FETISH] > 0) MainScreen.text("You tell her it was actually a great experience to have such passionate sex publicly.  She blushes at that suggestion.");

		//[If Urta is infertile]
		if (!urtaQuestDone()) {
			MainScreen.text("\n\n\"<i>Well, I guess it was nice, if painful afterwards, to be of use to someone who was so ready and willing to breed.");
			if (urtaLove()) MainScreen.text("  If only I could... uh, I guess I should just enjoy what I can.");
			MainScreen.text("</i>\"");
		}
		//[If Urta is fertile]
		else {
		...
		MainScreen.text("\n\nShe ");
		if (!urtaLove()) MainScreen.text("winks at you as you get ready to leave.  I hope I'll get to play with you again soon.</i>\"");
		else {
			MainScreen.text("gives you a quick but loving kiss before getting up and starting to prepare to leave the house.  As you get ready to move, she mumbles something, and reaches, grabbing your arm, shyly.\n\n\"<i>[name], next time, umm: find me when I'm sober.  I'll get to see how it feels to be wanted by someone the way I usually crave you, okay?</i>\"");
		...
		//[Both end here]
		MainScreen.text("\n\nSatisfied even if your body hurts all over and your " + CockDescriptor.describeCock(player, x) + " seems empty, dry, burning, and almost ruptured.  You leave the house with a small smile on your face, and a strong stench of sexual fluids following you back to the camp.");
