...
internal function approachSoberUrtaHeatRutProc():void {
	...
	//Player in rut:
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Your " + multiCockDescriptLight() + " and [vagina] are filling your clothes with mixed juices");
...
private notInterestedInUburDrunkUrtaRuts():void {
	...
	if(player.faceType == 2) MainScreen.text("Your nose does seem to pick up that Urta was and still is getting unusually aroused behind you.  ");
	...
	if(player.tongueType == TONUGE.SNAKE) MainScreen.text("You instinctively taste the air, and Urta suddenly seems utterly delicious. ");
	//Rut: 
	if(player.inRut && player.lowerBody.cockSpot.hasCock()) MainScreen.text("Your " + multiCockDescriptLight() + " hardens in your pants as you still force yourself to leave.");
	//[Medium lust increase]
...
private drunkUrtaIntroPartDuex(chosenSex: number = 1, newl:boolean = true):void {
	...
	if(chosenSex == 1) MainScreen.text(multiCockDescriptLight());
	else MainScreen.text("[vagina]");
...
private keepTeasingDatHornyFawkes():void {
	...
	MainScreen.text("slowly beginning to stroke and caress your " + multiCockDescriptLight() + " for the benefit of those who bear vaginas.");
	...
	if(player.isGoo() || player.isNaga()) MainScreen.text("what passes for knees on your [legs]");
...
private soberUrtaSatingPCHeat():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		MainScreen.text(" by shoving your " + multiCockDescriptLight());
	...
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("[cock biggest]");
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength < 2.5) MainScreen.text("rubbing the hard, stimulated sensitive spot between her fingers");
	...
	if(player.wetness() >= 4) MainScreen.text(" pussy juice shooting out around her member, coating Urta's fur and soaking her balls and the base of her cock.");
	...
	if(player.wetness() >= 4) MainScreen.text("shooting out all over the place");
	...
	if(player.wetness() >= 4) MainScreen.text("once again setting off to coat Urta in your juices.");
	...
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("and " + multiCockDescriptLight() + " ");
	...
	dynStats("lust=",0);
	dynStats("lust=",0);
	flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] += 6;
...
private sateRutWithSoberUrtaButHuegDicked():void {
	...
	MainScreen.text("Urta observes your approaching " + cockDescript(x) + " with a hungry, eager expression. Without taking her eyes off of it, she flips up her ");
...
private sateRutWithSoberUrta():void {
	...
	if(urtaQuestDone()) {
		...
		if(flags[FlagEnum.URTA_FERTILE] == 1) MainScreen.text("\n\n\"<i>We'll just have to hope then, won't we, [name]?</i>\"");
	...
	flags[FlagEnum.TIMES_RUT_FUCKED_URTAS_CUNT]++;
	dynStats("lib+",1,"sen-",2,"lust=",0);
	flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] += 6;
	doNext(camp.returnToCampUseOneHour);
}
...
private drunkenUrtaFucksPCInHeat():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) 
	{
		MainScreen.text(", impatiently nudging aside your " + multiCockDescriptLight());
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength < 2.5) MainScreen.text("playing with your [clit]");
	...
	if(player.wetness() >= 4) MainScreen.text("As if to emphasize this, your squelching, slurping pussy suddenly sprays fem-cum all over Urta's face, making her pull herself free with a shocked shake of her head.");
	...
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("nor even your " + multiCockDescriptLight());
	...
	dynStats("lust=",0);
	dynStats("lust=",0);
	flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] += 24;
	flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY]++
	urtaLove(1);
...
private drunkUrtaRidesARutPCsCock():void {
	...
	MainScreen.text(", and her own balls resting comfortably on your abdomen.  She grits her teeth as you feel her pussy slowly tighten around and massage your " + cockDescript(x) + ", and you groan in response.  Urta starts to bounce up and down on your member, smacking her thighs and ass into your body, not minding her own giant equine member flopping around and occasionally smacking you over your chest.  She lets her tongue flick out into the air for a moment and moans in delight, before picking up the pace and starting to pump your cock in and out of her hungry passage furiously.");
	...
	else MainScreen.text("recognize the tingling sensation from your " + cockDescript(x) + " to your spine");
	...
	if(player.cumQ() >= 500 && player.cumQ() <= 1500) MainScreen.text("\n\nUrta seems positively full and stretched now, and your rut almost subsides for a moment, her stretched tummy imitating a pregnancy quite well.");
		...
		if(player.lowerBody.balls > 0) MainScreen.text("[balls]");
		else if(player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.vaginaSpot.list[0].clitLength >= 3.5) MainScreen.text("cock-like clit");
	...
	if(player.stats.cor > 35 || flags[FlagEnum.PC_FETISH] > 0) MainScreen.text("You tell her it was actually a great experience to have such passionate sex publicly.  She blushes at that suggestion.");
