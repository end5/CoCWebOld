...
*/
		...
		private function get pregnancy():PregnancyType { return kGAMECLASS.urta.pregnancy; }

//Urta Pregnancy Stages
//Urta takes 2 days to progress from one pregnancy stage to the next
//In Bar Menu
public function urtaKids(): number {
	return flags[FlagEnum.URTA_KIDS_MALES] + flags[FlagEnum.URTA_KIDS_FEMALES] + flags[FlagEnum.URTA_KIDS_HERMS];
}
...
private function urtaSexMenu():void {
	...
	if(!urta.urtaDrunk()) {
		...
		if(flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) {
			...
			if(flags[FlagEnum.URTA_CUM_NO_CUM_DAYS] >= 5) addButton(1,"Suck Off",urta.slurpFawkesCocksForFunAndInflation);
		...
		//FRIEND
			...
			menu();
			if(flags[FlagEnum.URTA_CUM_NO_CUM_DAYS] >= 5) addButton(0,"Hidden BJ",urta.slurpFawkesCocksForFunAndInflation);
...
internal function urtaPregAppearance():void {
	switch (pregnancy.event) {
		case 1:
		case 2: Render.text("Urta is sitting at her usual table, looking more clear-eyed and sober than usual.  She looks extremely happy, tail wagging gently behind her, and she gives you a thrilled look when she sees you, smiling blissfully and patting her belly in a meaningful gesture.");
				break;
		case 3:
		case 4:
		case 5:
		case 6:
		case 7: Render.text("Urta has moved from her usual table to a more private stall, apparently for the comfort of its padded seats.  Her bulging belly makes it obvious she's pregnant - and the wide grin that seems fixed to her face makes it obvious she's ecstatic about it.");
				break;
		case 8:
		case 9: Render.text("Urta's pregnancy has caused her to outgrow the stall; she's moved herself to an entirely new position in a quiet, out of the way part of the bar.  Looking at the sheer size of her, she's doubtlessly going to pop any day now.");
		default:
	}
}
...
internal function urtaPreggoApproached():void {
	...
	menu();
	switch (pregnancy.event) {
		case 1: Render.text("As you approach and take your usual seat, Urta chugs down a glass of something orange-colored and fizzy, which definitely doesn't smell like her usual taste in drinks.  She sees you looking at it and smiles, shrugging casually.  \"<i>Well, I can't drink alcohol while pregnant, can I, [name]?</i>\"  She then steals a kiss from you; she tastes sweet and a little tart - must be some sort of fruit-juice...");
				//Horny:
				if (flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] == 0) {
					Render.text("  Her cock audibly clonks against the table's underside, but she just grins at the sound, fully confident in who she is now.  \"<i>Wanna come back to my place, lover" + player.mf("-boy","") + "?  They say sex is good for the baby - and really good for the mommy...</i>\"  She croons, teasingly rubbing your [legs] with her foot.");
					urtaSexMenu();
				}
				else {
					Render.text("  The lack of cock-on-wood sounds or motion on Urta's part makes it clear she's not feeling amorous right this moment.  Breaking the kiss, she licks her glossy lips clean and looks at you with a good-natured smirk.  \"<i>Feeling up to talking for a little while?</i>\"");
		...
		case 7: 
				...
				break;
		case 8:
				Render.text("Urta's finally outgrown even her booth, her belly now looking like she swallowed a prize-winning watermelon whole.  She's taken to sitting in a shapeless, very well-stuffed couch in an out-of-the-way nook of the bar.  Her hands stroke her gut and her erect dick at more or less the same time, precum dripping noisily into a bucket positioned under her erection.  She sees you coming and her cock bounces eagerly, belching another spurt of precum into the bucket from a surge in arousal.");
...
private function IAintGotTimeForUrtaBirfs():void {
	clearOutput();
	Render.text("The centaur looks perplexed at your refusal.  \"<i>I - what?  Really?</i>\"");
	...
	flags[FlagEnum.URTA_TIMES_BIRTHED]++;
	flags[FlagEnum.URTA_LATESTBORN_GENDER] = rand(3) + 1;
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 1) flags[FlagEnum.URTA_KIDS_MALES]++;
	else if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 2) flags[FlagEnum.URTA_KIDS_FEMALES]++;
	else flags[FlagEnum.URTA_KIDS_HERMS]++;
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 3 || flags[FlagEnum.URTA_LATESTBORN_GENDER] == 1) {
		if(rand(2) == 0) flags[FlagEnum.URTA_LATESTBORN_COCKTYPE] = 2;
		else flags[FlagEnum.URTA_LATESTBORN_COCKTYPE] = 1;
	}
	//If firstborn, set as current.
	if(urtaKids() == 1) {
		flags[FlagEnum.URTA_FIRSTBORN_GENDER] = flags[FlagEnum.URTA_LATESTBORN_GENDER];
		flags[FlagEnum.URTA_FIRSTBORN_COCKTYPE] = flags[FlagEnum.URTA_LATESTBORN_COCKTYPE];
	}
	flags[FlagEnum.NEED_URTA_LETTER] = 1;
	doNext(camp.returnToCampUseOneHour);
}
...
public function getUrtaLetter():void {
	...
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 1) Render.text("son");
	else if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 2) Render.text("daughter");
	else Render.text("hermaphrodite daughter");
	Render.text(", though.\n");
	flags[FlagEnum.NEED_URTA_LETTER] = 0;
}
...
private function goVisitUrtaBirfs(withUrta:boolean = false):void {
	...
	flags[FlagEnum.URTA_PREGNANT_DELIVERY_SCENE] = 0;
	flags[FlagEnum.NEW_BABY_ANNOUNCED] = 0;
	if(!withUrta) Render.text("The trip to the clinic where Urta is giving birth to your child is a blur; all that matters is getting there as fast as possible.  The centaur abandons you at the hospital doors, and you make your own way inside, approaching the counter inside.");
	
	//[1st time:
	if(flags[FlagEnum.URTA_TIMES_BIRTHED] == 0) {
	...
	if(gender == 2) Render.text("girl!");
	else if(gender == 1) Render.text("boy!");
	else Render.text("boy...?  Well... isn't that unusual, looks like you actually gave birth to a healthy herm!");
	Render.text("</i>\"");
	...
	if(gender == 1 || gender == 3) {
		if(rand(2) == 0) {
			Render.text("\n\n\"<i>Well, now... ");
			if(gender == 3) Render.text("s");
			Render.text("he's a perfectly normal little fox,</i>\" the doctor notes, and you can catch a quick glimpse of a somewhat bigger than normal vulpine sheathe on your ");
			if(gender == 1) Render.text("son");
			else Render.text("daughter");
			Render.text("'s groin from where you are.");
			flags[FlagEnum.URTA_LATESTBORN_COCKTYPE] = 2;
		}
		//[Horse Dick:
		else {
			Render.text("\n\n\"<i>...My, my; looks like ");
			if(gender == 3) Render.text("s");
			Render.text("he takes after ");
			if(gender == 3) Render.text("her");
			else Render.text("his");
			Render.text(" mother,</i>\" the doctor comments.  Snatching a quick glance reveals the leathery sheath of an equine phallus swaying between the newborn's legs.  Although infantile, it already promises that ");
			if(gender == 1) Render.text("he");
			else Render.text("she");
			Render.text(" is going to be quite a big ");
			if(gender == 1) Render.text("boy when he grows up.");
			else Render.text("girl when she grows up.");
			flags[FlagEnum.URTA_LATESTBORN_COCKTYPE] = 1;
		}
	}
	...
	if(gender >= 2) Render.text("her");
	else Render.text("him");
	Render.text(" to the breast, watching with joyous awe as the little fox-morph roots for her black nipple and then latches on.  Tears begin to roll down her cheeks at the sight.  \"<i>Our ");
	if(gender >= 2) Render.text("daughter");
	else Render.text("son");
	Render.text("... look, [name], ");
	//[she's]
	if(gender >= 2) Render.text("she's");
	else Render.text("he's");
	Render.text(" really here; our beautiful, beautiful little baby...</i>\" She sobs, ");
	if(urtaKids() == 0) Render.text("overwhelmed by the fact she's a mother at long last");
	else Render.text("still as delighted as ever to have a new addition to your growing brood");
	Render.text(".");
	...
	Render.text(" mother's breast.  \"<i>Looks like my work here is done for now... I'll be back later to give a quick check-up on the baby, but from what I can see you have nothing to worry about.  Excuse me then.</i>\"  The doctor gets up and leaves the room, letting you have some privacy with the fox-herm.");
	...
	//set all appropriate birth variables here.
	flags[FlagEnum.URTA_TIMES_BIRTHED]++;
	flags[FlagEnum.URTA_LATESTBORN_GENDER] = gender;
	if(gender == 1) flags[FlagEnum.URTA_KIDS_MALES]++;
	else if(gender == 2) flags[FlagEnum.URTA_KIDS_FEMALES]++;
	else flags[FlagEnum.URTA_KIDS_HERMS]++;
	//If firstborn, set as current.
	if(urtaKids() == 1) {
		flags[FlagEnum.URTA_FIRSTBORN_GENDER] = flags[FlagEnum.URTA_LATESTBORN_GENDER];
...
private function goAfterUrtaBirf():void {
	clearOutput();
	Render.text("You apologize, but there are pressing matters you need to attend to today... but you'll make it up to her somehow, you promise.  Urta looks disappointed, but sighs sadly and nods her head.  \"<i>Of course.  You're still a busy adventurer and all that.  Well, don't worry; I'll make sure this little one gets home and settled in safely.</i>\"");
	model.time.hours++;
	doNext(playerMenu);
}
...
private function stayAfterUrtaBirf():void {
	...
	Render.text("Of course, if that's what she wants, you'd be happy to oblige.  The vixen smiles at you thankfully.  \"<i>You won't regret it, [name]; I have an eventful day planned for us,</i>\" she finishes with a mischievous grin and a giggle of delight.");
...
private function wakeUpWithUrtaAfterStaying():void {
	...
	//PC Wakes with Urta
	Render.text("As the morning sun shines on the blinds, you open your eyes.  Then you remember the events of the last day.  You spent the whole day with Urta, not having sex, just walking together and buying stuff for your newborn ");
	if(flags[FlagEnum.URTA_KIDS_MALES] == 1) Render.text("son");
	else Render.text("daughter");
	Render.text(", then you went home and spent the better part of the evening making out with Urta which, of course, led to hot vixen sex in her bedroom.  After that... you probably fell asleep on her bed, and considering the current state of affairs, she didn't mind it one bit.");
	...
	Render.text("\n\nYou hold the shaft ");
	if(player.stats.cor < 50) Render.text("gently,");
	else Render.text("tightly,");
	Render.text(" earning a ");
	if(player.stats.cor < 50) Render.text("whimpering moan");
	else Render.text("gasping shudder");
	Render.text(" from the vixen, then begin stroking her and pushing her to the side.  The fox grumbles and shudders, rolling over to the side.  \"<i>Mmm... [name], while that's nice, I don't really wanna have to do laundry right after getting up,</i>\"  she mumbles in a sleepy voice.");
	...
	//1st time: 
	if(flags[FlagEnum.URTA_TIMES_BIRTHED] == 1) Render.text("You look at her questioningly; she's breast-feeding, so she shouldn't be drinking alcohol.  \"<i>Oh, don't worry; even I don't drink booze in the morning.  It's a little pick-me-up of my own invention; totally non-alcoholic.  It's mostly fruit juice with a few spices thrown in.</i>\"  Urta laughs.  ");
	Render.text("  Your only reply is to reach behind her balls and gently insert a wandering finger into her pussy.");
	...
	if(urtaKids() == 1) Render.text("a child");
	else Render.text("children");
	Render.text(" to look after now; I can't be leaving broken glass around, now can I?</i>\"  She puts her hands behind her head and stretches, perhaps not so coincidentally thrusting out her naked breasts.  \"<i>Mmm... now I feel awake.  Come on, lover; you're clearly in the mood, and last night was wonderful... ready for round two, hmm?</i>\"  She croons, wriggling herself over to sit right next to you and leaning onto your shoulder.");
	...
	if(urtaKids() == 1) Render.text("new ");
	else Render.text("newest ");
	Render.text("little monster,</i>\" she notes, already walking off to the nursery, indifferent to any need for clothes.  After all, it's not like the baby will mind that she's not wearing pants while feeding ");
	if(urtaKids() == 1) Render.text("him");
	else Render.text("her");
	Render.text(".");
	if(urtaKids() == 2) Render.text("  The other kid is staying at Edryn's, so there's no risk of you two needing to answer... complicated questions.");
	if(urtaKids() > 2) Render.text("  The other kids are staying at Edryn's, so there's no risk of you two needing to answer... complicated questions.");
...
private function drinkSomeUrtaPostBirthTitMilk():void {
	clearOutput();
	Render.text("You decide to ask if Urta wouldn't mind you getting breakfast before going.  Your hand wanders to caress her unoccupied breast.  She moans appreciatively and then giggles.  \"<i>Naughty [name]... but, I'm okay with that.  Just let me finish up with this little one first; I can't juggle the two of you at the same time.</i>\"");
	...
	Render.text(" together... but that's a poor substitute if you don't ever come back.  Please, be careful with yourself, [name],</i>\" she tells you solemnly.  You kiss her goodbye, giving her shaft a teasing grope then step out of the room.");
...
public function urtaPregooUpdates():boolean {
	...
	//3:
		...
		//flags[FlagEnum.EVENT_PARSER_ESCAPE] = 1;
		return true;
	}
	//4: 
		...
		if (pregnancy.type != PregnancyType.PLAYER) Render.text("sounding somewhat jealous that it's you carrying and not her.");
		else Render.text("sounding a little smug as she caresses her own gravid tummy.");
		Render.text("  You shake your head, but considering how far you are into the pregnancy, it shouldn't take long... if Urta keeps visiting, you're pretty sure it'll be kicking the next time she comes around.");
	...
	//5
		...
		if (urtaKids() == 0) Render.text(" Oh, [name], we really are going to be parents...");
		Render.text("</i>\" She babbles happily, tail wagging so enthusiastically you wonder if it's going to fall off.");
...
private function noHelpForUrtaPreggoPC():void {
	clearOutput();
	Render.text("You caress her cheek before saying a flat, \"<i>No</i>\".  Urta actually gives you puppy dogs eyes at that, then lets out a wounded-sounding huff.  \"<i>All right, [name]; I guess I better get back to work, then.  Now you take care of yourself; you've got a little one on board, now.</i>\"  She chastises you.");
	doNext(camp.returnToCampUseOneHour);
}
...
private function getUrtaHelpPreggoPC():void {
	clearOutput();
	Render.text("Good, you hope this means she'll be having plenty of time, because you sure could use a fair share of vixen right now.  \"<i>As long as you want me, I'll be here.</i>\"  Urta laughs already starting to fiddle with the straps of her armor.  \"<i>Do you have any preferences for what we do?</i>\"");
...
private function getUrtaSexWhenPreggoz():void {
	clearOutput();
	Render.text("Well... you could use a break from minding your passenger, so how does she plan to help you take your mind off your current predicament?  You ask with a knowing smile.");
...
private function getAnUrtaMassageWhenPreggo():void {
	clearOutput();
	Render.text("You tell her that a backrub sounds fine, but you'd rather not lie down.  It could hurt the baby.  \"<i>Right, sorry, what was I thinking?  Well, I think I can be flexible...</i>\"  Urta grins, cracking her fingers. \"<i>Now, let's just find somewhere comfortable to put you first...</i>\"");
...
private function declineUrtaLevelSixPreggoHelps():void {
	clearOutput();
	Render.text("You thank Urta for the offer, but you'll have to decline.  You still have business to take care of, so you'll just rest for a while longer then be on your way.  Besides, it wouldn't look good for her to be absent for too long, she has a city to take care of.");
...
private function acceptUrtaLevelSixPreggoHelps():void {
	clearOutput();
	Render.text("You're feeling really tired right now, and besides, spending a whole day with Urta would be nice, so you gladly accept her offer.");
...
public function PCGivesBirf():void {
	...
	if(gender == 1) {
		...
		Render.text(" sheath between his legs that looks like it's going to be bigger than normal when he grows up.");
		flags[FlagEnum.URTA_KIDS_MALES]++;
		flags[FlagEnum.URTA_LATESTBORN_GENDER] = 1;
	}
	//Girl:
	else if(gender == 2) {
		...
		flags[FlagEnum.URTA_KIDS_FEMALES]++;
		flags[FlagEnum.URTA_LATESTBORN_GENDER] = 2;
	}
	//Herm:
	else {
		...
		Render.text(" sheath between her legs that looks like it's going to be bigger than normal when she grows up.");
		flags[FlagEnum.URTA_KIDS_HERMS]++;
		flags[FlagEnum.URTA_LATESTBORN_GENDER] = 3;
	...
	Render.text(" out here.  I know you wouldn't let anything bad happen to ");
	if(gender >= 2) Render.text("her");
	else Render.text("him");
	Render.text(", but, well, you can't really take care of ");
	if(gender >= 2) Render.text("her");
	else Render.text("him");
	Render.text(" properly in this wilderness, now can you?</i>\"");
	
	Render.text("\n\nYou are eventually forced to admit that it would probably be better for the baby if ");
	if(gender >= 2) Render.text("she");
	else Render.text("he");
	Render.text(" was brought up in Tel'adre.  Urta smiles thankfully and gives both of you one particularly eager cuddle.  \"<i>Thank you for understanding, [name].  ");
	//[She]
	if(gender >= 2) Render.text("She");
	else Render.text("He");
	Render.text(" will be brought up safely, I promise.</i>\"  With that, she carefully bundles up the sleeping child and quietly vanishes into the wilderness, leaving you to take yourself off to bed for a proper rest.\n");
	
	//SET ALL PREGGOGOGOGOGOGO
	flags[FlagEnum.URTA_TIMES_PC_BIRTHED]++;
	//If firstborn, set as current.
	if(urtaKids() == 1) {
		flags[FlagEnum.URTA_FIRSTBORN_GENDER] = flags[FlagEnum.URTA_LATESTBORN_GENDER];
		flags[FlagEnum.URTA_FIRSTBORN_COCKTYPE] = flags[FlagEnum.URTA_LATESTBORN_COCKTYPE];
...
public function urtaSpecialDeliveries():void {
	...
	Render.text("As you enter through the gates, Edryn clops up to you, a small basket swinging from the crook of one arm");
	if (kGAMECLASS.telAdre.edryn.pregnancy.isPregnant) Render.text(" and her pregnant belly jiggling from the motions");
	Render.text(", a faint smile on her face.  \"<i>Hey there, [name]; can you do me a favor, please?  Urta's been getting deliveries to her place, but I got an unexpected call, so I'm too busy to take her groceries to her today - you're her " + player.mf("boyfriend","girlfriend") + "; can you please take this for me?</i>\"  The centauress asks.");
	...
	if (kGAMECLASS.telAdre.edryn.pregnancy.isPregnant) Render.text("as quickly as a pregnant mare can, anyway,");
	Render.text(" leaving you to head on to Urta's house.");
	...
	if (pregnancy.event >= 7) Render.text("heavily ");
	Render.text("pregnant vixen is snoring softly, laying on her side under the covers, forming a sizable bump in the soft, fluffy quilt that practically envelops her, almost invisible amidst the big plump cushions that serve to keep her comfortable as she sleeps. ");
	...
	flags[FlagEnum.URTA_PREGNANT_DELIVERY_SCENE] = 1;
	//(Return to TA menu.)
	menu();
...
internal function talkToUrtaAboutHerHouse():void {
	clearOutput();
	Render.text("You've been wondering why Urta keeps her old apartment if she has a nicer and bigger house now.  It would be much better for her to put the lease money towards something more productive than keeping that apartment...");
	if(player.stats.cor < 50) Render.text(" despite it having some emotional value to you.");
	...
	if(urtaKids() > 1) Render.text("or two ");
	Render.text("pipes up asking what we're doing.</i>\"  She shakes her head with amused smirk.  \"<i>No, I'd rather keep the old place for our little love nest; we can have our fun there, then come home and be all respectable around the kids.</i>\"");
...
private function yesUrtaHouseSex():void {
	clearOutput();
	Render.text("You smile seductively at her and raise a brow.");
	...
	if(player.stats.lib > 50) Render.text("  Sometimes you almost give up on waiting to get to her apartment and instead strip and do her right then and there... but it wouldn't bode well for the Watch captain to get a fine for public indecency.");
...
private function noUrtaHouseSex():void {
	clearOutput();
	Render.text("You smile at her and apologize, you're just not in the mood right now.");
...
private function talkAboutLiana():void {
	clearOutput();
	Render.text("You tell Urta that you need to talk with her about something.");
	Render.text("\n\n\"<i>Yes, [name]?</i>\"  The politely puzzled prick-vixen responds.");
	
	//{Had sex with Lianna:
	if(flags[FlagEnum.LIANNA_HAVESTED_LADIES] > 0 || flags[FlagEnum.LIANNA_HAVESTED_MALES] > 0) {
	...
	if(urtaKids() > 1) Render.text("ren have");
	else Render.text(" has");
	Render.text(" a special place in her heart.</i>\"");
...
private function preggoUrtaSmexOrSomething():void {
	clearOutput();
	Render.text("You let Urta know that you're for helping her vent some steam, if that's what she wants.");
	
	Render.text("\n\nUrta grins lecherously, precum ");
	switch (pregnancy.event) {
		case 7: Render.text("dripping wetly onto the floor");
				break;
		case 8:
		case 9: Render.text("slopping into the filling bucket");
				break;
		default: Render.text("staining her dress");
	}
	Render.text(".  \"<i>Really?  Good, because I could use a little release... let's get back to my place, shall we?</i>\"  She starts to pull herself upright, prompting you to step in and lend her a hand.  She smiles and kisses you thankfully, letting you feel the swell of her belly and dick pressed against you as she does.  \"<i>So...lead on, lover</i>,\" she croons.");
...
private function massagePregnantUrtasBelly():void {
	...
	if(flags[FlagEnum.TIMES_MASSAGED_URTA_BELLY] == 0) Render.text("  She looks at you and her newly cream-adorned belly contemplatively.  \"<i>I never knew my sweet tooth would bring me to this...</i>\"");
	...
	if(player.tongueType == TONUGE.SNAKE) Render.text("forked ");
	...
	Render.text("tongue hang and begin licking the cream off her breasts");
	if(player.tongueType > TONUGE.HUMAN) Render.text(", a task that is made far easier due to your tongue's ability to stretch to a surprising length");
	Render.text(".  You make sure to pay extra attention to her cream covered nubs.");
	...
	flags[FlagEnum.TIMES_MASSAGED_URTA_BELLY]++;
	//[RubDick] [RubPussy]
	menu();
...
private function rubUrtaPussy():void {
	clearOutput();
	Render.text("You decide that by \"her,\" she could only mean her pussy.  So you stretch your arm, as if reaching for her shaft, then find your way under her furry balls to her waiting slit, gently touching and massaging her labia, teasing her clit with little pinches, forcing her wetness out of her slavering honeypot.  She lets out a surprised yelp, then laughs, \"<i>Not actually what I wanted... ooh!  But I'm not complaining...</i>\"");
...
private function urtaMassageClimax():void {
	...
	//Low Cor:
	if(player.stats.cor < 33) Render.text("  You walk up to her and carefully remove her condom, tying it up with a knot to prevent any of her seed from escaping, then dispose of it.");
	//Medium Cor:
	else if(player.stats.cor < 66) Render.text("Well, whatever, she's probably too sleepy to do anything about it either way.");
	//High Cor:
	else Render.text("You walk up to her with a smirk and remove her condom, then pour down its contents over her belly, chuckling to yourself.  Now that's a great cream topping for your lovely fox-herm.");
	Render.text("  You walk out of her house, taking care to lock the door on your way out.");
	dynStats("lus", 25);
	doNext(camp.returnToCampUseOneHour);
}
...
private function urtaBoobWorshipScene():void {
	clearOutput();
	Render.text("You gaze at Urta's breasts, heavy with their milky load, a few droplets already escaping her black nipples.  \"<i>Oh, you like these, do you?  You like a nice, big pair of foxy-boobs, huh?</i>\"  She jokes, jiggling them for your amusement, before wincing at their sensitivity.");
	
	Render.text("\n\nYou can't help but note that Urta's breasts are pretty swollen.  Has she been milking herself lately?  Doesn't look like it.");
	
	//1st Time:
	if(flags[FlagEnum.TIMES_URTA_BOOB_WORSHIPPED] == 0) {
	...
	flags[FlagEnum.TIMES_URTA_BOOB_WORSHIPPED]++;
	...
	if(player.faceType == FACE.SHARK_TEETH || player.faceType == FACE.SNAKE_FANGS || player.faceType == FACE.SPIDER_FANGS) Render.text(", always minding your sharp teeth");
	Render.text(".  She yelps loudly, but makes no move to push you off, so she's obviously enjoying it.  You alternate between biting and pulling, never repeating the same motion twice.  \"<i>I- I'm gonna...!</i>\"  She gasps out, bucking and wriggling under you.  You smile wickedly and bite her nipple one last time, pulling and sucking on it.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text(", tasting a bit of yourself,");
	Render.text(" and tell her you'll be back to check up on her later.  She gives you a sleepy smile and nods absently, already practically asleep.  You quickly find your [armor] and don them, leaving Urta to rest for a spell in her apartment.");
	doNext(camp.returnToCampUseOneHour);
}
...
private function nurseFromMommaUrtasBooBees():void {
	clearOutput();
	Render.text("You look at Urta's breasts, full of milk and dripping, and absentmindedly lick your lips. Urta follows your gaze and gives you a knowing smile.  \"<i>You know, no matter how much I milk myself, I always seem to fill up again right away... I wonder if that's part of Taoth's so-called parting gift,</i>\" she muses to herself.");
	
	//1st Time:
	if(flags[FlagEnum.TIMES_NURSED_FROM_URTA] == 0) {
	...
	Render.text("tongue.  Urta lets out a sigh of pleasure, yipping softly as your tongue tantalizes her over-sensitive black nipples.  Noticing that she seemed to enjoy the licking you keep doing so, slathering her soft mounds in a thin layer of saliva, alternating between breasts.");
	if(player.tongueType > TONUGE.HUMAN) Render.text("  Every few licks you coil your tongue around one of her nipples and gently pull at it, milking a thin strand of white goodness straight on your tongue, much to the fox-herm's delight.");
	...
	flags[FlagEnum.TIMES_NURSED_FROM_URTA]++;
	//[Stroke][Ignore]
	menu();
...
private function titWorshipAndStroke():void {
	clearOutput();
	Render.text("You smile mischievously around Urta's breast and eagerly grab her shaft with a hand, careful not to hurt her, but roughly enough for her to feel what you just did.  Urta lets out a surprised groan.  \"<i>Lover?  What are you...?</i>\"  She moans again as your fingers shift on her distended dick.");
...
private function ignoreUrtaBonerWhenWorship():void {
	clearOutput();
	Render.text("You have other things to worry about right now, but you're glad she seems to be enjoying your ministrations.");
	finishTitWorshipWivUrta();
}
...
private function milkPregnantUrta():void {
	clearOutput();
	Render.text("You tell her that you feel like playing with her toys.  Urta smiles and nods.  \"<i>Sure, though I prefer a more... personal touch... myself.  They're in that cupboard over there,</i>\" she finishes, pointing it out.");
	...
	if(player.lowerBody.cockSpot.hasCock() && !player.isTaur() && player.cockThatFits(urta.urtaCapacity())) {
...
private function goHomeHorsecock():void {
	clearOutput();
	Render.text("You nod at Urta and gather your clothes, redressing in an instant.  Before going you ask if Urta needs anything.  She smiles and shakes her head, a soft smile on her face.  You bid her a good rest and leave her apartment.");
	doNext(camp.returnToCampUseOneHour);
}
...
private function stayForHorseDickings():void {
	clearOutput();
	Render.text("You yawn yourself, and suggest that maybe Urta would like some company.  You're kinda sleepy yourself.  She smirks and reclines on the bed, \"<i>Help yourself, lover - there's plenty of me to cuddle,</i>\" she giggles softly as she pats her swollen belly.");
...
private function urtaPregCravings():void {
	clearOutput();
	Render.text("You decide to ask Urta if she has any particular cravings right now.  The pregnant fox visibly thinks about, then surreptitiously eyes your crotch.");
	//noDick:
	if(!player.lowerBody.cockSpot.hasCock()) {
	...
	if(player.lactationQ() >= 200) Render.text(", which automatically causes milk to trickle forth");
	Render.text(".  \"<i>Nope, not it either...");
	if(player.lactationQ() >= 200) Render.text(" sweet, though.");
	...
	Render.text(" to the tip.  You can't resist moaning as her vulpine tongue laps at your sensitive [cock biggest].");
	...
	if(player.lowerBody.balls > 0) Render.text(" with your [balls]");
	Render.text(".  You smile apologetically down at her and ask her if she's found out what she's been craving yet.  \"<i>What do you think?</i>\"  She mumbles sarcastically, a playful twinkle in her eye, then starts noisily slurping and sucking, tongue frenziedly lapping at your intruding dick and her eyes rolling shut in obvious enjoyment as she gulps down the precum she's eagerly coaxing from you.");
	
	Render.text("\n\nYou moan and try your best not to just grab onto her head and face-fuck her.  ");
	if(player.lowerBody.cockSpot.count() > 2) Render.text("The rest of your " + multiCockDescriptLight() + " are as erect as your [cock biggest] dribbling pre along their lengths as they slap Urta during her frenzied bobs.  ");
	Render.text("You tell Urta that you're getting close; if she keeps up with this you won't last much longer.");
	...
	if(player.lowerBody.balls > 0 || player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text(", a hand playing expertly with ");
		if(player.lowerBody.balls > 0) {
			Render.text("your balls");
			if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and another with ");
		}
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("your pussy");
	}
	Render.text(".  She suddenly pops free, licking her lips.  \"<i>Come on, lover.  I thought you offered to help me with my craving?  And yet you won't give me any of that salty load of yours?  So hungry... gimme!</i>\"  She pleads, looking up at you with hopeful eyes even as she continues to stroke your shaft.");
	menu();
...
private function getBlownByPregnantUrta():void {
	clearOutput();
	Render.text("If she wants it so bad, she can have it, you think to yourself, bucking against her hand as you finally feel ");
	if(player.lowerBody.balls > 0) Render.text("your balls churn and ");
	Render.text("cum travelling down your urethra, expanding your shaft with the force of their passage, and finally jetting out of your [cockHead biggest] to arch gracefully through the air and splatter against Urta's face.");
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		Render.text("  Even if it did, you wouldn't be able to do anything about your other cock");
		if(player.lowerBody.cockSpot.count() > 2) Render.text("s as they join");
		else Render.text(" as it joins");
		Render.text(" in on the onslaught.");
	}
	...
	//(Low Cum Amount)
	if(player.cumQ() < 250) {
		Render.text("  Soon, all too soon, you're spent.  Urta is left with strands of cum covering her naked body, only a few having made it inside her open maw.  You pant as you slump down, satisfied with the outcome.");
...
private function holdOnYouAintGettingYerCumYetYouNaughtyFox():void {
	clearOutput();
	Render.text("You're not going to give in so easily.  If she wants it she'd best work hard for it.  You smile at her mischievously, even as you hump against her grip.");
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() > 2) Render.text("s bulging ominously as they too prepare to deposit their loads on the willing fox-herm.");
		else Render.text(" bulging ominously as it too prepares to deposit its load on the willing fox-herm.");
	}
	Render.text("  Urta eagerly gulps and slurps, doing her best to swallow every drop.");
	
	//(Low Cum Amount)
	if(player.cumQ() < 250) {
		Render.text("\n\nYou feed her all you can, ");
		if(player.lowerBody.cockSpot.count() > 1) {
			Render.text("even splashing some cum over her face as your other cock");
			if(player.lowerBody.cockSpot.count() == 2) Render.text(" convulses");
			else Render.text("s convulse");
			Render.text(" in orgasm, ");
		}
		Render.text("but ultimately you reach your limit and can do nothing but slump as she continues sucking on your [cock biggest] like a straw, hungry for your male milk.");
	...
	//(Medium Cum Amount)
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			Render.text("Your other cock");
			if(player.lowerBody.cockSpot.count() == 2) Render.text(" contributes");
			else Render.text("s contribute");
			Render.text(" by making a complete mess of her as ");
			if(player.lowerBody.cockSpot.count() > 2) Render.text("they randomly spatter");
			else Render.text("it randomly spatters");
			Render.text(" her with cum, not that she seems to mind.");
		}
		Render.text("  You lay down and relax, letting the cock-hungry fox-herm suck you dry as you continue pumping your male milk.  By the time she's done her belly looks a little larger, and you know that some of that bloating is not entirely due to the baby.  You smile happily at the vixen hungrily slurping on your [cock biggest] despite there being nothing more for her to drink.");
	...
	//(High Cum Amount)
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			Render.text("  Your other cock");
			if(player.lowerBody.cockSpot.count() > 2) Render.text("s shoot their loads");
			else Render.text(" shoots its load");
			Render.text(" erratically, some of it splattering against Urta's face, but most of it winds up making a complete mess of the room.");
		}
		...
		if(player.hasKnot(player.biggestCockIndex())) Render.text(", just above your knot,");
		Render.text(" with both hands, trying to pinch it shut long enough to inhale a couple of deep breaths, then goes back to trying to drink every last drop; she must really have a hankering for what you're packing.");
	...
	else Render.text("yourself");
	Render.text(" drained of your cum.");
	...
	if(player.cumQ() >= 500) Render.text(", audibly gurgling");
	Render.text(" belly, clearly feeling tired even though she wasn't the one being pleasured this time.");
...
private function urtaRaepsJoo():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("Every time her swollen tip rubs against your prostate you feel droplets of pre escape your dilating cum-hole.  ");
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("Each slap of her balls against your [vagina] sends electric waves of pleasure that expand throughout your body.  ");
	Render.text("You moan audibly, making it pretty clear you're getting off from this kind of treatment.  \"<i>See?  I knew you'd enjoy this too, now that I'm not so tense...</i>\"  She draws out the last word as she makes a particularly deep thrust inside of you.  \"<i>Mmm...  I can feel your sexy ass squeezing my dick; you're enjoying this, really, aren't you?</i>\"");
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		if(player.lowerBody.cockSpot.count() > 1) Render.text("  [EachCock] tenses in unison");
		else Render.text("  [EachCock] throbs, tensing in preparation");
		//(Low Cum Amount)
		if(player.cumQ() < 250) Render.text(" and you unload on the floor, spilling rope after rope of cum, forming a small puddle where you stand.  An undeniable testament to your enjoyment of the kinkiness of the act, however small.");
		//(Medium Cum Amount)
		else if(player.cumQ() < 1000) Render.text(" and you deliver your payload right there on the floor.  Jet after jet splash against it, and you have no choice but to continue moaning and spilling your cum as Urta remains firmly lodged in your depths, no longer able to move now that you're squeezing her shaft so deliciously tight.  By the time you're done, the puddle you've formed is big enough that you can see your own face on the reflection, enraptured in pleasure and enjoyment of Urta's equine meatstick.");
		//(High Cum Amount)
		else {
			Render.text(" and you let your prodigious load escape the confines of your ");
			if(player.lowerBody.balls > 0) Render.text("churning balls");
			else Render.text("being");
			Render.text(", splashing against the floor with such force that you form veritable streams of enjoyment in the rapidly expanding lake of perversion.  The pleasure of the kinky act, aided by your earlier drink, ensures you feel nothing but ultimate happiness for this brief moment.  By the time you've spilled the last drop, the lake has reached Urta's knees, and you pity the poor soul in charge of cleaning the back rooms.");
		}
	}
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("\n\nYour pussy grips at nothing, ");
		if(player.wetness() >= 4) Render.text("squirting");
		else Render.text("leaking");
		Render.text(" juices that paint Urta's balls, which are resting flush against your [vagina].  The spilled juice slides down Urta's balls");
		if(player.wetness() >= 4) Render.text(" to join your previous discharge on");
		else Render.text(" and down towards");
		Render.text(" the floor, where it gathers in a ");
		Render.text("puddle of its own.");
	}
	//Genderless:
	if(player.gender == 0) {
		...
		Render.text(" anal muscles milk Urta's shaft much like a pussy would.");
		if(player.ass.analWetness > 0) Render.text(" It even secretes its own wetness to slide lovingly down the fox-herm's balls and drip on the floor, an undeniable proof of your enjoyment.");
	}
	...
	if(puddle > 0) {
		if(puddle == 1) Render.text("small");
		else if(puddle == 2) Render.text("reasonable");
		else Render.text("enormous");
		Render.text(" puddle of cum you created");
	}
	else Render.text("floor");
	Render.text(" and close your eyes, letting sleep take you over even as the fox behind you drools on your back and continues to pound your ass raw.");
...
private function declineARimJob():void {
	clearOutput();
	Render.text("You tell her that you appreciate the offer, but you're going to sit this one out.  Though she tries to hide it, she looks relieved at your refusal; she probably isn't really comfortable with that level of kinkiness.  Instead, she eagerly captures your lips with hers, murmuring into your mouth as she devours your kiss, tongue sliding between your lips to caress your own tongue.");
	//Both choices converge here.
...
private function acceptUrtaRimJobbies():void {
	clearOutput();
	Render.text("Well... she is responsible for your state, and her broad vulpine tongue must feel amazing on your sensitive ass... so you nod in acceptance of her offer.  \"<i>All right then... you do know you're the only person I'd ever do this for, right?</i>\" Urta comments.  \"<i>Now, roll around and present yourself to me; you can't expect a pregnant lady to do a lot of bending, now can you?</i>\"  She smirks.");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("  Especially not if your " + multiCockDescriptLight() + " have anything to say about it, throbbing at the mere feeling of Urta's wet tongue.");
	else if(player.lowerBody.cockSpot.count() == 1) Render.text("  Especially not if your [cock] has anything to say about it, throbbing at the mere feeling of Urta's wet tongue.");
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Especially not if your [pussy] has any say in the matter, winking at Urta, hoping for a licking of its own.");
	Render.text("  She slowly begins deepening the pseudo-kiss, pressing more and more of her wet tongue against you until she is taking slow, languid licks up and down the lengths of the chasm, slathering your burning skin with her cooling goo.");
...
internal function visitKidsFirstTime():void {
	clearOutput();
	Render.text("You ask Urta if she can take you to see your child");
	if(urtaKids() > 1) Render.text("ren");
	Render.text(".");
	
	Render.text("\n\n\"<i>You want to see ");
	if(urtaKids() == 1) Render.text("our child");
	else Render.text("our babies");
	Render.text("?</i>\"  Urta asks, tail wagging excitedly at the thought.  \"<i>Why, of course, [name] - " + urtaKidsText("he'll","she'll","they'll") + " be so happy to see " + urtaKidsText("his","her","their") + " " + player.mf("father","other-mother") + "!  Come on, let's get going!</i>\"  She insists, immediately getting up from her seat");
	if(flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] == 0) Render.text(", erection already diminishing as she forgets about her horniness in her parental pride");
	Render.text(".  You waste no time in following her; indeed, she seems liable to leave you behind in her excitement to go and see her offspring.");
	...
	if(urtaKids() == 1) Render.text("child");
	else Render.text("children");
	Render.text(".  One door leads out to the back courtyard, while another looks like it closes off stairs leading down to a basement.");
...
private function pickKidSceneHere():void {
	...
	//ANNOUNCE BEHBIES
	//Once per pregnancy?
	if ((pregnancy.type == PregnancyType.PLAYER || (player.pregnancyIncubation < 300 && player.pregnancyType == PregnancyType.URTA)) && flags[FlagEnum.NEW_BABY_ANNOUNCED] == 0) {
		newBabyComing();
	}
...
private function newBabyComing():void {
	clearOutput();
	Render.text("You announce to your ");
	if(urtaKids() == 1) Render.text("child");
	else Render.text(num2Text(urtaKids()) + " kids");
	Render.text(" that you and Urta have something important to tell them.");
	
	//If UrtaKids <8:
	if(urtaKids() < 8) {
		Render.text("\n\nAs ");
		if(urtaKids() == 1) {
			if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) Render.text("she");
			else Render.text("he");
			Render.text(" approaches");
		}
		else Render.text("they gather");
		Render.text(" and curiously look");
		if(urtaKids() == 1) Render.text("s");
		Render.text(" at you, Urta gives ");
		if(urtaKids() > 1) Render.text("them");
		else if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) Render.text("her");
		else Render.text("him");
		Render.text(" a loving smile.");
		...
		else if (pregnancy.type == PregnancyType.PLAYER || (player.pregnancyType == PregnancyType.URTA))
		{
			...
			if (pregnancy.type == PregnancyType.PLAYER) Render.text("her");
			else Render.text("your");
			Render.text(" belly.");
		}
		Render.text("\n\nYour child");
		if(urtaKids() > 1) Render.text("ren are");
		else Render.text(" is");
		Render.text(" silent for a moment, digesting this information.  Then ");
		if(urtaKids() == 1) Render.text("a small fluffy bundle is");
		else Render.text("several small fluffy bundles are");
		Render.text(" doing ");
		if(urtaKids() == 1) Render.text("its");
		else Render.text("their");
		Render.text(" best to hug you both; ");
		if(urtaKids() > 1) Render.text("they");
		else if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) Render.text("she");
		else Render.text("he");
		if(urtaKids() == 1) Render.text(" is");
		else Render.text(" are");
		Render.text(" clearly delighted at the prospect.  You and Urta smile at each other over ");
		if(urtaKids() > 1) Render.text("their");
		else if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) Render.text("her");
		else Render.text("his");
		Render.text(" head");
		if(urtaKids() > 1) Render.text("s");
		Render.text(", then return the embrace, pleased that ");
		if(urtaKids() > 1) Render.text("they");
		else if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) Render.text("she");
		else Render.text("he");
		if(urtaKids() == 1) Render.text(" is");
		else Render.text(" are");
		Render.text(" excited at the prospect.");
	}
	//If UrtaKids =>8:
	else {
	...
	flags[FlagEnum.NEW_BABY_ANNOUNCED] = 1;
	doNext(camp.returnToCampUseOneHour);
}
...
private function hugAttackScene():void {
	clearOutput();
	Render.text("Before you can exchange any further pleasantries the sound of running padded feet fills the house.  Lianna simply looks at the approaching foxling");
	if(urtaKids() > 1) Render.text("s");
	...
	if(urtaKids() > 1) Render.text("es do");
	else Render.text(" does");
	Render.text(" eventually clamor" + urtaKidsText("s","s","") + " off of you, but not before taking your hands in " + urtaKidsText("his","her","their") + " own.  ");
	if(urtaKids() > 1) Render.text("They");
	else if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) Render.text("She");
	else Render.text("He");
	Render.text(" insistently pull");
	if(urtaKids() == 1) Render.text("s");
	Render.text(" you towards the backyard, while your lover chuckles at your predicament.");
...
private function noPlayTimeForKids():void {
	clearOutput();
	Render.text("You shake your head, telling your family that you're sorry but you have to go.  \"<i>Really?  All right... but, you should at least hang around long enough to take some on the go.</i>\"  Urta insists.  You tell her you'll be fine, but you'll definitely stop by to eat with them sometime.  Urta gives a melodramatic sigh and rolls her eyes, then nods and smiles.  \"<i>All right... just make sure you do.</i>\"  She tells you, then gives you a quick hug and a kiss.  \"<i>We worry about you out there; come home safe to us.</i>\"  She says, then lets you go.");
...
private function stayToPlay():void {
	clearOutput();
	Render.text("You nod your head in acceptance.  Urta smiles in delight.  \"<i>Take a seat, please, make yourself comfortable - I'll go and get it,</i>\" she says happily, tail wagging as she saunters off to the kitchen.");
	
	//1st time:
	if(flags[FlagEnum.URTA_LUNCH_PLAY] == 0) {
		flags[FlagEnum.URTA_LUNCH_PLAY]++;
...
private function bathtime():void {
	clearOutput();
	Render.text("You tell Lianna you just wanted to check in on your ");
	if(urtaKids() > 1) Render.text("kids");
	else if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) Render.text("daughter");
	else Render.text("son");
	Render.text(".  \"<i>Sure, but they need a bath, maybe you'd like to help me bathe them?</i>\"");
	//[Yes] [No]
	menu();
...
private function noBathTiemPlease():void {
	clearOutput();
	Render.text("\"<i>I see, in that case excuse me,</i>\"  She says with a blank expression.  Looks like you don't have any business here for now, so you decide to leave.");
	//(Back to TA/Camp)
	menu();
	addButton(0,"Next",telAdre.telAdreMenu);
}
private function urtaKidsText(male:string = "",female:string = "", plural:string = ""):string {
	if(urtaKids() == 1) {
		if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1) return male;
		else return female;
	}
	else return plural;
}
...
private function giveTheKidsABath():void {
	...
	if(urtaKids() == 1) {
		if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1) Render.text("your son");
		else Render.text("your daughter");
	}
	else Render.text("the kids");
	Render.text(".</i>\"  You contemplate what to do, then remove your outer layer of clothes, carefully placing them on a shelf in case they get splashed.  Now you just need to wait for Lianna to return.");
	
	Render.text("\n\nLianna enters the bathroom followed by your ");
	if(urtaKids() == 1) {
		if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1) Render.text("son");
		else Render.text("daughter");
	}
	else if(urtaKids() < 8) Render.text("kids");
	else Render.text("army of kids");
	Render.text(".  \"<i>All right, ");
	if(urtaKids() > 1) Render.text("everyone ");
	Render.text("strip up and hop in.</i>\"  ");
	if(urtaKids() == 1) {
		if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1) Render.text("He");
		else Render.text("She");
	}
	else Render.text("They");
	Render.text(" mutter" + urtaKidsText("s","s","") + " rebelliously, but then perk" + urtaKidsText("s","s","") + " up at the sight of you.  Immediately " + urtaKidsText("he","she","they") + " start" + urtaKidsText("s","s","") + " pulling off " + urtaKidsText("his","her","their") + " clothes and then, naked as the day " + urtaKidsText("he was","she was","they were") + " born, swarm" + urtaKidsText("s","s","") + " you, hugging you tightly.  You dole out comforting pats and playful strokes, then instruct " + urtaKidsText("him","her","them") + " to get into the tub.  Mournful eyes look at you, but, seeing you won't be swayed, " + urtaKidsText("he leaps","she leaps","they leap") + " with a splash into the bathtub.  Once in the water, " + urtaKidsText("he","she","they") + " start" + urtaKidsText("s","s","") + " to enjoy " + urtaKidsText("himself","herself","themselves") + ", merrily kicking and splashing about in what is, for " + urtaKidsText("him","her","them") + ", a decent-sized pool.  This gives you an opportunity to note the sex of your little rugrat" + urtaKidsText("","","s") + ".");

	//If any UrtaKids=Male:
	if(flags[FlagEnum.URTA_KIDS_MALES] > 0) {
		Render.text("\n\nYou have " + num2Text(flags[FlagEnum.URTA_KIDS_MALES]) + " vulpine son");
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) Render.text("s");
		Render.text(".");
		if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1) {
			Render.text("  Your firstborn is ");
			if(flags[FlagEnum.URTA_KIDS_MALES] == 1) Render.text("the only one,");
			else Render.text("among them,");
			Render.text(" sporting a");
			if(flags[FlagEnum.URTA_FIRSTBORN_COCKTYPE] == 2) Render.text(" vulpine penis");
			else Render.text("n equine phallus, similar to what Urta has");
			Render.text(".");
		}
		if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 1 && urtaKids() > 1) {
			Render.text("  Your newest is ");
			if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 1) Render.text("also ");
			Render.text("a boy, with a");
			if(flags[FlagEnum.URTA_LATESTBORN_COCKTYPE] == 2) Render.text(" vulpine penis");
			else Render.text("n equine phallus, just like his mom's");
			Render.text(".");
		}
		Render.text("  You have a feeling ");
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) Render.text("they're");
		else Render.text("he's");
		Render.text(" going to be quite developed when ");
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) Render.text("they hit");
		else Render.text("he hits");
		Render.text(" puberty, if not as big as Urta.  Urta is quite proud of ");
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) Render.text("them");
		else Render.text("him");
		Render.text(", and often boasts that ");
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) Render.text("they");
		else Render.text("he");
		Render.text(" will grow up to be ");
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) Render.text("strong fighters like she is.");
		else Render.text("a strong fighter like she is.");
	}
	//If any UrtaKids=Herm:
	if(flags[FlagEnum.URTA_KIDS_HERMS] > 0) {
		Render.text("\n\nYou have " + num2Text(flags[FlagEnum.URTA_KIDS_HERMS]) + " vulpine, hermaphroditic daughter");
		if(flags[FlagEnum.URTA_KIDS_HERMS] > 1) Render.text("s");
		Render.text(".");
		if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 3) {
			Render.text("  Your firstborn is a hermaphrodite, sporting a");
			if(flags[FlagEnum.URTA_FIRSTBORN_COCKTYPE] == 2) Render.text(" vulpine penis");
			else Render.text("n equine phallus, a peculiarity she inherited from her mother");
			Render.text(".");
		}
		if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 3 && urtaKids() > 1) {
			Render.text("  Your latest addition is ");
			if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 3) Render.text("also ");
			Render.text("a herm, with a");
			if(flags[FlagEnum.URTA_LATESTBORN_COCKTYPE] == 2) Render.text(" vulpine penis");
			else Render.text("n equine phallus.  No doubt such a characteristic was passed from her mother");
			Render.text(".");
		}
		Render.text("  You have a feeling ");
		if(flags[FlagEnum.URTA_KIDS_HERMS] > 1) Render.text("they're");
		else Render.text("she's");
		Render.text(" going to be quite developed when ");
		if(flags[FlagEnum.URTA_KIDS_HERMS] > 1) Render.text("they hit");
		else Render.text("she hits");
		Render.text(" puberty, if maybe not quite as big as Urta.  Urta particularly dotes on ");
		if(flags[FlagEnum.URTA_KIDS_HERMS] > 1) Render.text("them");
		else Render.text("her");
		Render.text(", maybe because she remembers how much her parents hated her for being a herm.");
	}
	//If any UrtaKids=Female:
	if(flags[FlagEnum.URTA_KIDS_FEMALES] > 0) {
		Render.text("\n\nYou have " + num2Text(flags[FlagEnum.URTA_KIDS_FEMALES]) + " vulpine daughter");
		if(flags[FlagEnum.URTA_KIDS_FEMALES] > 1) Render.text("s");
		Render.text(".");
		if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 2) {
			Render.text("  Your firstborn is a girl.");
		}
		if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 2 && urtaKids() > 1) {
			Render.text("  Your latest addition is ");
			if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 2) Render.text("also ");
			Render.text("a girl.");
		}
		Render.text("  Urta loves to play with ");
		if(flags[FlagEnum.URTA_KIDS_FEMALES] > 1) Render.text("them");
		else Render.text("her");
		Render.text(", and swears ");
		if(flags[FlagEnum.URTA_KIDS_FEMALES] > 1) Render.text("they will");
		else Render.text("she will");
		Render.text(" be quite the heartbreaker");
		if(flags[FlagEnum.URTA_KIDS_FEMALES] > 1) Render.text("s");
		Render.text(" when ");
		if(flags[FlagEnum.URTA_KIDS_FEMALES] > 1) Render.text("they grow");
		else Render.text("she grows");
		Render.text(" up.");
	}
	...
	if(urtaKids() > 1) Render.text("nearest ");
	Render.text("furry little form of your child and gently tug them over to you.  ");
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("She");
	else Render.text("He");
	Render.text(" kicks and splashes with ");
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("her");
	else Render.text("his");
	Render.text(" little legs, but smiles up at you, clearly willing to cooperate, giving you no problem as you spurt a generous dollop of the soapy goo into ");
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("her");
	else Render.text("his");
	Render.text(" hair and start to massage it... now, just how do you shampoo somebody covered from head to toe in fur...?");
	...
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("daughter");
	else Render.text("son");
	Render.text(" flicks ");
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("her");
	else Render.text("his");
	Render.text(" tail, splashing Lianna with water.  The skunkette, however, fails to display any reaction at this.");
	
	Render.text("\n\nYou apologize for that, noting that " + urtaKidsText("he","she","they") + " got you the same way earlier.  You then flick your ");
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("daughter");
	else Render.text("son");
	Render.text("'s ear and tell ");
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("her");
	else Render.text("him");
	Render.text(" to apologize to Lianna for that.  \"<i>It's all right, [name].  " + urtaKidsText("He's just a kid","She's just a kid","They're just kids") + ", you can't expect to bathe kids and not get wet.</i>\"  You think you have a pretty clear idea of why Urta picked Lianna to care for the children.  Casually, you ask if Lianna's got any younger siblings; she seems like she's got a lot of practice at this.  She shakes her head, \"<i>This is the first time I got a job where kids are involved.  Either way, it's a lot easier than taking care of minotaurs.</i>\"  You can't help but wonder if she really did somehow take care of minotaurs at one point, or if this is just some colloquial expression.");
	
	Render.text("\n\n\"<i>Here, let me help you.</i>\"  She squeezes a handful of shampoo on her hands and begins scrubbing your ");
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("daughter");
	else Render.text("son");
	Render.text("'s chest, tickling ");
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("her");
	else Render.text("him");
	Render.text(" sides as she does so.  The little fox giggles and squirms, clearly finding the actions quite ticklish, and you have to hold on a little harder to keep ");
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("her");
	else Render.text("him");
	Render.text(" from escaping.  \"<i>Now all you have to do is rinse off.</i>\"  She grabs a small bucket nearby, scooping up some water.  \"<i>Close your eyes, dear.</i>\"  Your child does as Lianna instructs; this is evidently a familiar enough routine that ");
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("she");
	else Render.text("he");
	Render.text(" has no intention of winding up with soap in ");
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("her");
	else Render.text("his");
	Render.text(" eyes.  Lianna turns the bucket, letting the water cascade over ");
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("her");
	else Render.text("his");
	Render.text(" head.  Your little fox squeals and then shakes wildly to fling the water off of ");
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("her");
	else Render.text("his");
	Render.text(" head, kicking up a cascade of water as ");
	
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("she");
	else Render.text("he");
	Render.text(" does so");
	if(urtaKids() > 2) {
		Render.text(", to the delight of ");
		if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("her");
		else Render.text("his");
		Render.text(" siblings, who are safely watching from the other side of the bath");
	}
	Render.text("."); 
	...
	Render.text(" hair is done, shaking some of the water out of your own face.");
	...
	if(urtaKids() == 1) {
		...
		Render.text(" tail.  Get out of the tub.</i>\"  The skunkette instructs.  The young fox nods and scrabbles at the side of the bath, hauling ");
		if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("her");
		else Render.text("him");
		Render.text("self up and out so Lianna can start tending to ");
		if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("her");
		else Render.text("his");
		Render.text(" tail.  You promptly make yourself useful and start scrubbing as well...");
	}
	else {
		Render.text("\n\n\"<i>Good, now who's next?</i>\"  \"<i>Me!</i>\"  ");
		if(urtaKids() > 2) Render.text("One of your other children");
		else Render.text("Your other child");
		Render.text(" pipes up.  You let go of the ");
		if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("girl");
		else Render.text("boy");
		Render.text(" you're currently holding so ");
		if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("she");
		else Render.text("he");
		Render.text(" can swim away, allowing the new volunteer to take ");
		if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) Render.text("her");
		else Render.text("his");
		Render.text(" place.  You promptly make yourself useful and start scrubbing as well...");
	}
...
private function noBathPleaseUrtaLian():void {
	clearOutput();
	Render.text("You politely refuse her invitation.  \"<i>Okay, but can you wash mine then?</i>\" she asks.  You give the idea a moment's thought and then agree; it's a harmless offer, right?  \"<i>Thanks,</i>\" she replies, getting out of the tub and sitting on a small wooden bench nearby.  \"<i>Use the flower extract shampoo, please.</i>\"  You nod your head in understanding as you climb out after her, taking up the indicated shampoo and approaching her before asking how much you should use.  \"<i>Plenty,</i>\" is her reply.");
...
private function getABackWashFromLianna():void {
	clearOutput();
	Render.text("You tell Lianna that if she is willing, you wouldn't object to her washing your back.");
...
private function takeTheKidsForWalk():void {
	clearOutput();
	Render.text("\"<i>You'd like to see " + urtaKidsText("him","her","them") + "?  Sure, but first I need to get " + urtaKidsText("him","her","them") + " ready.</i>\"   You ask her where she plans on taking " + urtaKidsText("him","her","them") + ".");
	...
	if(urtaKids() > 1) Render.text("ren");
	Render.text(" are suitably dressed for a traipse around a desert city, you usher your kit");
	if(urtaKids() > 1) Render.text("s");
	Render.text(" out the door and into the street.  \"<i>Can " + urtaKidsText("I","I","we") + " get some ice cream?!</i>\"");
	if(urtaKids() > 1) Render.text("  \"<i>Yeah, can we " + player.mf("daddy","mommy") + "?!</i>\"");
	Render.text("  Your child");
	if(urtaKids() > 1) Render.text("ren plead");
	else Render.text(" pleads");
	Render.text(".  Well, the vendors down at the market would make for a suitably far destination, which should give Lianna plenty of time for herself, and you yourself wouldn't mind some ice cream either at that, so it's decided.  \"<i>Yay! You're the best " + player.mf("dad","mom") + " ever!</i>\" ");
	if(urtaKids() > 1) Render.text("a chorus of excited cheers sounds out.");
	else Render.text("an excited cheer sounds out.");
	Render.text("  It's hard not to want to live up to that title with such ");
	if(urtaKids() == 1) Render.text("a happy face");
	else Render.text("happy faces");
	Render.text(" staring up at you.");
	...
	if(urtaKids() > 1) Render.text("ren");
	Render.text(" pick out flavours while the shopkeeper scoops them up onto wafer cones.  Once he's done, you thank the wolf and leave the shop followed by your ");
	if(urtaKids() > 10) Render.text("army of ");
	Render.text("child");
	if(urtaKids() > 1) Render.text("ren");
	Render.text(".");
	...
	Render.text("\n\nYour child");
	if(urtaKids() > 1) Render.text("ren");
	Render.text(" don't bother waiting or savouring their ice cream and neither do you, it's damned hot walking around this desert.  There's less cause for mischief too, " + urtaKidsText("his","her","their") + " face");
	if(urtaKids() == 1) Render.text("buried in cone");
	else Render.text("s buried in their cones");
	Render.text(" along with your own, greedily licking up melting droplets.  There's just a bit of cone left as you return to Urta's house, and you quickly polish that off, upon entering.  \"<i>Oh, hey.  You're back.</i>\"  Lianna greets you in her usual inexpressive fashion.");
	...
	if(urtaKids() > 1) Render.text("es");
	Render.text(" towards the bathroom.  There is a " + urtaKidsText("shrill protest","shrill protest","chorus of shrill protests") + ", but " + urtaKidsText("he","she","they") + " clearly understand");
	if(urtaKids() == 1) Render.text("s");
	Render.text(" that Lianna is not one to be trifled with and so " + urtaKidsText("he","she","they") + " march");
	if(urtaKids() == 1) Render.text("s");
	Render.text(" off to the bathroom.  You can't help but note how good Lianna is at making kids do what she says.");
...
public function visitTheHouse():void {
	...
	if(flags[FlagEnum.URTA_INCUBATION] >= 192 && rand(2) == 0) {
		...
		if(urtaKids() > 1) Render.text("s");
		Render.text(" for a walk.  Be back soon!  - Lianna");
	...
	//1st time only:
	if(flags[FlagEnum.FIRST_TIME_AT_URTA_HOUSE] == 0) {
		flags[FlagEnum.FIRST_TIME_AT_URTA_HOUSE]++;
...
private function liannasAppearance():void {
	clearOutput();
	Render.text("Lianna is a healthy looking young anthro, a skunk-morph covered in sleek, shiny black fur, with white adorning her swishing, fluffy-looking tail.  Her hair is pure black with streaks of white, tied out of the way into a short ponytail, and her green eyes glitter behind round, silver-rimmed glasses that perch atop her nose.  She wears a conservative short-sleeved blouse, and pants that, while slightly baggy, still manage to show off her nice legs.  They're certainly tight enough that you can tell she's all girl; no hidden surprises here.  She goes without shoes, exposing her paw-like feet.  She has a very nicely rounded, generous handful of an ass, and womanly hips; up top, she is no less stacked, with what are easily E-cup breasts making her even bustier than her employer is; despite all that, she tries to tone them down with her style of dress.");
	//(Display Options)
	menu();
...
private function talkToLiana(output:boolean = true):void {
	if(output) {
		clearOutput();
		Render.text("You tell her you'd like to talk her.  She adjusts her glasses.  \"<i>Oh?  What about?</i>\"");
	}
	menu();
	addButton(0,"Her Job",talkToLianaAboutHerJob);
	addButton(1,"HerResearch",talkToLianaAboutHerResearch);
	if(flags[FlagEnum.LIANNA_HAVESTED_MALES] + flags[FlagEnum.LIANNA_HAVESTED_LADIES] > 0) addButton(2,"Her Spray",talkAboutLiannasLubeSpray);
	if(flags[FlagEnum.DISCUSSED_LUBE_SPRAY] > 0) addButton(3,"Side Effect",lubeSpraySideEffects);
	addButton(9,"Back",visitTheHouse);
}
...
private function talkToLianaAboutHerJob():void {
	clearOutput();
	Render.text("You ask her about her jobs, both for Urta and what she intends to do afterwards.");
...
private function talkToLianaAboutHerResearch():void {
	clearOutput();
	Render.text("You ask her why she's so eager to get samples from you, a human.  What sort of research is she doing that she needs human samples?");
	...
	if(player.stats.cor >= 50) Render.text("however half-heartedly it may be, ");
	Render.text("and ask if that's what pushed her to studying humans and Marethians.");
...
private function talkAboutLiannasLubeSpray():void {
	clearOutput();
	flags[FlagEnum.DISCUSSED_LUBE_SPRAY] = 1;
	Render.text("You ask why it is that she produces a lubricating liquid rather than a noxious one, like you would expect of a skunk.");
...
private function lubeSpraySideEffects():void {
	clearOutput();
	Render.text("So, if she's experimented on herself in the past, that must have gone wrong for her at least once or twice in the past, you comment.");
	...
	if(player.stats.cor < 33) Render.text("mild");
	else if(player.stats.cor < 66) Render.text("some");
	else Render.text("great");
	Render.text(" reluctance.");
...
private function sexWithLianna():void {
	clearOutput();
	Render.text("\"<i>So you want to help me with my experiments?  Great!</i>\"  She takes your hand and basically drags you into her room.  You allow her to lead you, wondering what she has in mind.");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("  Your lower garments suddenly feel much tighter.");
	...
	if(player.lowerBody.cockSpot.hasCock()) addButton(1,"HarvestManly",liannaHandjobbies);
...
private function refuseGenderlessInjection():void {
	clearOutput();
	Render.text("Lianna's face falls at your refusal, but she sighs.  \"<i>All right, I can understand; I mean, you must have your reasons.  Still, if ever you get a gender, remember that I'd be happy to use some of your seed, please.</i>\"");
...
private function acceptARandomGenderFromASkunk():void {
	clearOutput();
	Render.text("You decide that it's worthwhile to go along and indicate that you'll let Lianna experiment on you with her \"genderiser\".");
	...
	//{if Centaur: 
	if(player.isTaur()) Render.text("  Or at least hold on to something, this might sting a little and I don't need you kicking down anything in here.");
	Render.text("</i>\"");
	...
	//If Tight:  
	if(player.ass.analLooseness <= 2) Render.text("\n\n\"<i>Going to need some lube for this...</i>\" she remarks.");
	else Render.text("\n\n\"<i>Pretty loose, I think I could just stick it in, but I'll apply some lube to make this as comfortable as possible.</i>\"");
	Render.text("  You really should be more surprised, but, sadly, this is kind of predictable; you were more hoping than anything that this would be an oral delivery, or at least an injection.");
	...
	if(rand(10) <= 3) {
		...
		player.lowerBody.cockSpot.list[0].cockLength = 4;
		player.lowerBody.cockSpot.list[0].cockThickness = 0.75;
	...
	//if PC becomes Female:
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) {
			...
			if(player.upperBody.chest.BreastRatingLargest[0].nippleLength < .5) {
				Render.text("  Your nipples erect into small half-inch nubs, complementing the transformation.");
	...
	//if PC becomes Herm:
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 3) {
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) Render.text("s");
			Render.text(" swelling into a soft C-Cup.");
			...
			Render.text("  You watch the soft mounds in your chest, unable to do anything but pant.");
...
private function getEggsHarvested():void {
	clearOutput();
	Render.text("\"<i>All right, let's get this show on the road.</i>\"  She ducks out into her closet once more, and comes out carrying what looks like a dildo and a lollipop.  \"<i>So, let me explain.  It's pretty simple really.</i>\"  She takes the dildo in her hand.  \"<i>This is a special dildo I had custom-made for this experiment, It will pleasure you at the same time it lets me harvest your pussy juice and hopefully one of your ovum.  All you have to do is sit back and relax.  Any questions?</i>\"");
	...
	//1st time:
	if(flags[FlagEnum.LIANNA_HAVESTED_LADIES] == 0) {
		Render.text("confusion, wondering what she's playing at");
		flags[FlagEnum.LIANNA_HAVESTED_LADIES]++;
	}
	else Render.text("remembrance, knowing what's coming next");
	Render.text(".  She unceremoniously begins shoving it inside without even a hint of pleasure or discomfort; the dildo slides in without resistance, spreading her buttcheeks apart and penetrating her with a wet squelch.  You watch the display, not sure whether or not you should find this kinky.");
	...
	if(player.vaginalCapacity() >= 50) Render.text("  \"<i>Hmm, I might not have had to lube it up after all...</i>\"  The skunkette remarks.");
	Render.text("  She aligns the tip of the dildo with you and without saying a word she begins pushing in.  Instinctively you moan and try to spread yourself as wide as possible.");
	...
	if(player.looseness() >= 4) Render.text("; your loose pussy doesn't hurt, either");
	Render.text(".  Finally, you can feel it stop inside of you, the tip pushing against your cervix, which is what has brought it to a halt.");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text(" except for the ");
		if(player.cumQ() < 50) Render.text("small");
		else if(player.cumQ() < 250) Render.text("normal");
		else if(player.cumQ() < 1000) Render.text("sizable");
		else Render.text("huge");
		Render.text(" splatters of cum from your neglected penis");
	}
	Render.text("; the dildo has absorbed it all.  You can feel it clenching tighter, shrinking down so that it no longer fills you up so perfectly, leaving you panting in release as Lianna gently frees it from inside you.");
...
private function liannaHandjobbies():void {
	clearOutput();
	flags[FlagEnum.LIANNA_HAVESTED_MALES]++;
	Render.text("\"<i>All right, let's get this show on the road.</i>\"  She ducks out into her closet once more, and comes out carrying what looks like a condom and a lollipop.  \"<i>Here's how this works.  You put on this condom and I milk your seed out of you.  Simple, yes?</i>\"");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("  \"<i>However I think milking just one is good enough.</i>\"");
	Render.text("  She motions to the bed.");
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		Render.text(", effectively binding them at their base.  You groan hollowly at this; though this certainly will mean less mess, you're not so sure you're going to enjoy the sensation of trying to cum through the " + num2Text(player.lowerBody.cockSpot.count()-1) + " cocks that have been blocked up.  Satisfied with her work, she looks at your [cock biggest].");
	}
	...
	if(player.lowerBody.balls > 0) Render.text("occasionally stopping to caress your [balls], ");
	Render.text("while the other massages your [cockHead biggest], rolling the steadily growing bubble of pre around as she kneads it.");
