...
public function getUrtaLetter():void {
	...
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 1) outputText("son");
	else if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 2) outputText("daughter");
...
private function wakeUpWithUrtaAfterStaying():void {
	...
	if(flags[FlagEnum.URTA_KIDS_MALES] == 1) outputText("son");
	else outputText("daughter");
	...
	outputText("\n\nYou hold the shaft ");
	if(player.stats.cor < 50) outputText("gently,");
	else outputText("tightly,");
	outputText(" earning a ");
	if(player.stats.cor < 50) outputText("whimpering moan");
...
private function drinkSomeUrtaPostBirthTitMilk():void {
	...
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 1) outputText("his");
	else outputText("her");
...
public function urtaPregooUpdates():boolean {
	...
	//4: 
		...
		if (pregnancy.type != PregnancyType.PLAYER) outputText("sounding somewhat jealous that it's you carrying and not her.");
...
internal function talkToUrtaAboutHerHouse():void {
	...
	if(player.stats.cor < 50) outputText(" despite it having some emotional value to you.");
...
private function yesUrtaHouseSex():void {
	...
	if(player.stats.lib > 50) outputText("  Sometimes you almost give up on waiting to get to her apartment and instead strip and do her right then and there... but it wouldn't bode well for the Watch captain to get a fine for public indecency.");
...
private function massagePregnantUrtasBelly():void {
	...
	if(flags[FlagEnum.TIMES_MASSAGED_URTA_BELLY] == 0) outputText("  She looks at you and her newly cream-adorned belly contemplatively.  \"<i>I never knew my sweet tooth would bring me to this...</i>\"");
	...
	if(player.tongueType == TONUGE.SNAKE) outputText("forked ");
	...
	if(player.tongueType > TONUGE.HUMAN) outputText(", a task that is made far easier due to your tongue's ability to stretch to a surprising length");
...
private function urtaMassageClimax():void {
	...
	//Low Cor:
	if(player.stats.cor < 33) outputText("  You walk up to her and carefully remove her condom, tying it up with a knot to prevent any of her seed from escaping, then dispose of it.");
	//Medium Cor:
	else if(player.stats.cor < 66) outputText("Well, whatever, she's probably too sleepy to do anything about it either way.");
	//High Cor:
...
private function urtaBoobWorshipScene():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", tasting a bit of yourself,");
...
private function nurseFromMommaUrtasBooBees():void {
	...
	if(player.tongueType > TONUGE.HUMAN) outputText("  Every few licks you coil your tongue around one of her nipples and gently pull at it, milking a thin strand of white goodness straight on your tongue, much to the fox-herm's delight.");
...
private function milkPregnantUrta():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("Urta promptly stops stroking your dick, though whether it's because she wants to be helpful or because she's annoyed you've stopped is anyone's guess.  ");
...
private function urtaPregCravings():void {
	...
	if(player.lowerBody.balls > 0) outputText(" with your [balls]");
	...
	if(player.lowerBody.cockSpot.count() > 2) outputText("The rest of your " + multiCockDescriptLight() + " are as erect as your [cock biggest] dribbling pre along their lengths as they slap Urta during her frenzied bobs.  ");
	...
	if(player.lowerBody.balls > 0 || player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.lowerBody.balls > 0) {
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" and another with ");
		}
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your pussy");
	}
...
private function getBlownByPregnantUrta():void {
	...
	if(player.lowerBody.balls > 0) outputText("your balls churn and ");
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() > 2) outputText("s as they join");
...
private function holdOnYouAintGettingYerCumYetYouNaughtyFox():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() > 2) outputText("s bulging ominously as they too prepare to deposit their loads on the willing fox-herm.");
	...
	if(player.cumQ() < 250) {
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			...
			if(player.lowerBody.cockSpot.count() == 2) outputText(" convulses");
	...
	//(Medium Cum Amount)
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			outputText("Your other cock");
			if(player.lowerBody.cockSpot.count() == 2) outputText(" contributes");
			else outputText("s contribute");
			outputText(" by making a complete mess of her as ");
			if(player.lowerBody.cockSpot.count() > 2) outputText("they randomly spatter");
	...
	//(High Cum Amount)
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			outputText("  Your other cock");
			if(player.lowerBody.cockSpot.count() > 2) outputText("s shoot their loads");
...
private function urtaRaepsJoo():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("Every time her swollen tip rubs against your prostate you feel droplets of pre escape your dilating cum-hole.  ");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Each slap of her balls against your [vagina] sends electric waves of pleasure that expand throughout your body.  ");
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		if(player.lowerBody.cockSpot.count() > 1) outputText("  [EachCock] tenses in unison");
		...
		//(High Cum Amount)
			...
			if(player.lowerBody.balls > 0) outputText("churning balls");
...
private function acceptUrtaRimJobbies():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("  Especially not if your " + multiCockDescriptLight() + " have anything to say about it, throbbing at the mere feeling of Urta's wet tongue.");
	else if(player.lowerBody.cockSpot.count() == 1) outputText("  Especially not if your [cock] has anything to say about it, throbbing at the mere feeling of Urta's wet tongue.");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Especially not if your [pussy] has any say in the matter, winking at Urta, hoping for a licking of its own.");
...
internal function visitKidsFirstTime():void {
	...
	if(flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] == 0) outputText(", erection already diminishing as she forgets about her horniness in her parental pride");
...
private function newBabyComing():void {
	...
	if(urtaKids() < 8) {
		outputText("\n\nAs ");
		if(urtaKids() == 1) {
			if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) outputText("she");
			else outputText("he");
		...
		else if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) outputText("her");
		else outputText("him");
		...
		else if (pregnancy.type == PregnancyType.PLAYER || (player.pregnancyType == PregnancyType.URTA))
		{
			...
			if (pregnancy.type == PregnancyType.PLAYER) outputText("her");
		...
		else if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) outputText("she");
		else outputText("he");
...
private function bathtime():void {
	...
	else if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) outputText("daughter");
	else outputText("son");
...
private function giveTheKidsABath():void {
	...
	if(urtaKids() == 1) {
		if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1) outputText("your son");
	...
	if(urtaKids() == 1) {
		if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1) outputText("son");
		else outputText("daughter");
	}
	...
	if(urtaKids() == 1) {
		if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1) outputText("He");
		else outputText("She");
	}
	...
	if(flags[FlagEnum.URTA_KIDS_MALES] > 0) {
		...
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) outputText("s");
		outputText(".");
		if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1) {
			outputText("  Your firstborn is ");
			if(flags[FlagEnum.URTA_KIDS_MALES] == 1) outputText("the only one,");
			else outputText("among them,");
			outputText(" sporting a");
			if(flags[FlagEnum.URTA_FIRSTBORN_COCKTYPE] == 2) outputText(" vulpine penis");
		...
		if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 1 && urtaKids() > 1) {
			...
			if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 1) outputText("also ");
			outputText("a boy, with a");
			if(flags[FlagEnum.URTA_LATESTBORN_COCKTYPE] == 2) outputText(" vulpine penis");
			else outputText("n equine phallus, just like his mom's");
			outputText(".");
		}
		outputText("  You have a feeling ");
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) outputText("they're");
		else outputText("he's");
		outputText(" going to be quite developed when ");
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) outputText("they hit");
		else outputText("he hits");
		outputText(" puberty, if not as big as Urta.  Urta is quite proud of ");
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) outputText("them");
		else outputText("him");
		outputText(", and often boasts that ");
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) outputText("they");
		else outputText("he");
		outputText(" will grow up to be ");
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) outputText("strong fighters like she is.");
	...
	if(flags[FlagEnum.URTA_KIDS_HERMS] > 0) {
		...
		if(flags[FlagEnum.URTA_KIDS_HERMS] > 1) outputText("s");
		outputText(".");
		...
		if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 3 && urtaKids() > 1) {
			...
			if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 3) outputText("also ");
		...
		outputText("  You have a feeling ");
		if(flags[FlagEnum.URTA_KIDS_HERMS] > 1) outputText("they're");
		else outputText("she's");
		outputText(" going to be quite developed when ");
		if(flags[FlagEnum.URTA_KIDS_HERMS] > 1) outputText("they hit");
		else outputText("she hits");
		outputText(" puberty, if maybe not quite as big as Urta.  Urta particularly dotes on ");
		if(flags[FlagEnum.URTA_KIDS_HERMS] > 1) outputText("them");
		else outputText("her");
	...
	if(flags[FlagEnum.URTA_KIDS_FEMALES] > 0) {
		...
		if(flags[FlagEnum.URTA_KIDS_FEMALES] > 1) outputText("s");
		outputText(".");
		...
		if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 2 && urtaKids() > 1) {
			outputText("  Your latest addition is ");
			if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 2) outputText("also ");
			outputText("a girl.");
		}
		outputText("  Urta loves to play with ");
		if(flags[FlagEnum.URTA_KIDS_FEMALES] > 1) outputText("them");
		else outputText("her");
		outputText(", and swears ");
		if(flags[FlagEnum.URTA_KIDS_FEMALES] > 1) outputText("they will");
		else outputText("she will");
		outputText(" be quite the heartbreaker");
		if(flags[FlagEnum.URTA_KIDS_FEMALES] > 1) outputText("s");
		outputText(" when ");
		if(flags[FlagEnum.URTA_KIDS_FEMALES] > 1) outputText("they grow");
	...
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) outputText("She");
	else outputText("He");
	outputText(" kicks and splashes with ");
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) outputText("her");
	else outputText("his");
	...
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) outputText("daughter");
	else outputText("son");
	outputText(" flicks ");
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) outputText("her");
	else outputText("his");
	...
	if(urtaKids() == 1) {
		...
		if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) outputText("girl");
		else outputText("boy");
...
private function talkToLianaAboutHerResearch():void {
	...
	if(player.stats.cor >= 50) outputText("however half-heartedly it may be, ");
...
private function lubeSpraySideEffects():void {
	...
	if(player.stats.cor < 33) outputText("mild");
	else if(player.stats.cor < 66) outputText("some");
...
private function sexWithLianna():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("  Your lower garments suddenly feel much tighter.");
...
private function liannaHandjobbies():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("  \"<i>However I think milking just one is good enough.</i>\"");
	...
	if(player.lowerBody.balls > 0) outputText("occasionally stopping to caress your [balls], ");
