...
private urtaSexMenu():void {
	...
	if(!urta.urtaDrunk()) {
		...
		if(flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) {
			...
			if(flags[FlagEnum.URTA_CUM_NO_CUM_DAYS] >= 5) MainScreen.addButton(1,"Suck Off",urta.slurpFawkesCocksForFunAndInflation);
		...
		//FRIEND
			...
			menu();
			if(flags[FlagEnum.URTA_CUM_NO_CUM_DAYS] >= 5) MainScreen.addButton(0,"Hidden BJ",urta.slurpFawkesCocksForFunAndInflation);
...
public getUrtaLetter():void {
	...
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 1) MainScreen.text("son");
	else if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 2) MainScreen.text("daughter");
...
private wakeUpWithUrtaAfterStaying():void {
	...
	if(flags[FlagEnum.URTA_KIDS_MALES] == 1) MainScreen.text("son");
	else MainScreen.text("daughter");
...
private drinkSomeUrtaPostBirthTitMilk():void {
	...
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 1) MainScreen.text("his");
	else MainScreen.text("her");
...
private massagePregnantUrtasBelly():void {
	...
	if(flags[FlagEnum.TIMES_MASSAGED_URTA_BELLY] == 0) MainScreen.text("  She looks at you and her newly cream-adorned belly contemplatively.  \"<i>I never knew my sweet tooth would bring me to this...</i>\"");
	...
	if(player.tongueType == TONUGE.SNAKE) MainScreen.text("forked ");
	...
	if(player.tongueType > TONUGE.HUMAN) MainScreen.text(", a task that is made far easier due to your tongue's ability to stretch to a surprising length");
...
private urtaBoobWorshipScene():void {
	...
	if(player.faceType == FACE.SHARK_TEETH || player.faceType == FACE.SNAKE_FANGS || player.faceType == FACE.SPIDER_FANGS) MainScreen.text(", always minding your sharp teeth");
...
private nurseFromMommaUrtasBooBees():void {
	...
	if(player.tongueType > TONUGE.HUMAN) MainScreen.text("  Every few licks you coil your tongue around one of her nipples and gently pull at it, milking a thin strand of white goodness straight on your tongue, much to the fox-herm's delight.");
...
private urtaPregCravings():void {
	...
	if(player.lowerBody.cockSpot.count() > 2) MainScreen.text("The rest of your " + multiCockDescriptLight() + " are as erect as your [cock biggest] dribbling pre along their lengths as they slap Urta during her frenzied bobs.  ");
...
private urtaRaepsJoo():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.wetness() >= 4) MainScreen.text(" to join your previous discharge on");
	...
	if(player.gender == 0) {
		...
		if(player.ass.analWetness > 0) MainScreen.text(" It even secretes its own wetness to slide lovingly down the fox-herm's balls and drip on the floor, an undeniable proof of your enjoyment.");
	}
...
private partTwoOfUrtaBoning():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(!player.isTaur()) MainScreen.text("  So close that you can almost feel its heat against your [cockHead].");
...
private acceptUrtaRimJobbies():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("  Especially not if your " + multiCockDescriptLight() + " have anything to say about it, throbbing at the mere feeling of Urta's wet tongue.");
...
internal function visitKidsFirstTime():void {
	...
	if(flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] == 0) MainScreen.text(", erection already diminishing as she forgets about her horniness in her parental pride");
...
private newBabyComing():void {
	...
	if(urtaKids() < 8) {
		MainScreen.text("\n\nAs ");
		if(urtaKids() == 1) {
			if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) MainScreen.text("she");
			else MainScreen.text("he");
		...
		else if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) MainScreen.text("her");
		else MainScreen.text("him");
		...
		else if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) MainScreen.text("she");
		else MainScreen.text("he");
...
private bathtime():void {
	...
	else if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) MainScreen.text("daughter");
	else MainScreen.text("son");
...
private giveTheKidsABath():void {
	...
	if(urtaKids() == 1) {
		if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1) MainScreen.text("your son");
	...
	if(urtaKids() == 1) {
		if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1) MainScreen.text("son");
		else MainScreen.text("daughter");
	}
	...
	if(urtaKids() == 1) {
		if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1) MainScreen.text("He");
		else MainScreen.text("She");
	}
	...
	if(flags[FlagEnum.URTA_KIDS_MALES] > 0) {
		...
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) MainScreen.text("s");
		MainScreen.text(".");
		if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1) {
			MainScreen.text("  Your firstborn is ");
			if(flags[FlagEnum.URTA_KIDS_MALES] == 1) MainScreen.text("the only one,");
			else MainScreen.text("among them,");
			MainScreen.text(" sporting a");
			if(flags[FlagEnum.URTA_FIRSTBORN_COCKTYPE] == 2) MainScreen.text(" vulpine penis");
		...
		if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 1 && urtaKids() > 1) {
			...
			if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 1) MainScreen.text("also ");
			MainScreen.text("a boy, with a");
			if(flags[FlagEnum.URTA_LATESTBORN_COCKTYPE] == 2) MainScreen.text(" vulpine penis");
			else MainScreen.text("n equine phallus, just like his mom's");
			MainScreen.text(".");
		}
		MainScreen.text("  You have a feeling ");
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) MainScreen.text("they're");
		else MainScreen.text("he's");
		MainScreen.text(" going to be quite developed when ");
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) MainScreen.text("they hit");
		else MainScreen.text("he hits");
		MainScreen.text(" puberty, if not as big as Urta.  Urta is quite proud of ");
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) MainScreen.text("them");
		else MainScreen.text("him");
		MainScreen.text(", and often boasts that ");
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) MainScreen.text("they");
		else MainScreen.text("he");
		MainScreen.text(" will grow up to be ");
		if(flags[FlagEnum.URTA_KIDS_MALES] > 1) MainScreen.text("strong fighters like she is.");
	...
	if(flags[FlagEnum.URTA_KIDS_HERMS] > 0) {
		...
		if(flags[FlagEnum.URTA_KIDS_HERMS] > 1) MainScreen.text("s");
		MainScreen.text(".");
		...
		if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 3 && urtaKids() > 1) {
			...
			if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 3) MainScreen.text("also ");
		...
		MainScreen.text("  You have a feeling ");
		if(flags[FlagEnum.URTA_KIDS_HERMS] > 1) MainScreen.text("they're");
		else MainScreen.text("she's");
		MainScreen.text(" going to be quite developed when ");
		if(flags[FlagEnum.URTA_KIDS_HERMS] > 1) MainScreen.text("they hit");
		else MainScreen.text("she hits");
		MainScreen.text(" puberty, if maybe not quite as big as Urta.  Urta particularly dotes on ");
		if(flags[FlagEnum.URTA_KIDS_HERMS] > 1) MainScreen.text("them");
		else MainScreen.text("her");
	...
	if(flags[FlagEnum.URTA_KIDS_FEMALES] > 0) {
		...
		if(flags[FlagEnum.URTA_KIDS_FEMALES] > 1) MainScreen.text("s");
		MainScreen.text(".");
		...
		if(flags[FlagEnum.URTA_LATESTBORN_GENDER] == 2 && urtaKids() > 1) {
			MainScreen.text("  Your latest addition is ");
			if(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 2) MainScreen.text("also ");
			MainScreen.text("a girl.");
		}
		MainScreen.text("  Urta loves to play with ");
		if(flags[FlagEnum.URTA_KIDS_FEMALES] > 1) MainScreen.text("them");
		else MainScreen.text("her");
		MainScreen.text(", and swears ");
		if(flags[FlagEnum.URTA_KIDS_FEMALES] > 1) MainScreen.text("they will");
		else MainScreen.text("she will");
		MainScreen.text(" be quite the heartbreaker");
		if(flags[FlagEnum.URTA_KIDS_FEMALES] > 1) MainScreen.text("s");
		MainScreen.text(" when ");
		if(flags[FlagEnum.URTA_KIDS_FEMALES] > 1) MainScreen.text("they grow");
	...
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) MainScreen.text("She");
	else MainScreen.text("He");
	MainScreen.text(" kicks and splashes with ");
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) MainScreen.text("her");
	else MainScreen.text("his");
	...
	if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) MainScreen.text("daughter");
	else MainScreen.text("son");
	...
	if(urtaKids() == 1) {
		...
		if(flags[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) MainScreen.text("girl");
...
private talkToLiana(output:boolean = true):void {
	...
	if(flags[FlagEnum.LIANNA_HAVESTED_MALES] + flags[FlagEnum.LIANNA_HAVESTED_LADIES] > 0) MainScreen.addButton(2,"Her Spray",talkAboutLiannasLubeSpray);
	if(flags[FlagEnum.DISCUSSED_LUBE_SPRAY] > 0) MainScreen.addButton(3,"Side Effect",lubeSpraySideEffects);
	MainScreen.addButton(9,"Back",visitTheHouse);
}
...
private acceptARandomGenderFromASkunk():void {
	...
	//{if Centaur: 
	if(player.isTaur()) MainScreen.text("  Or at least hold on to something, this might sting a little and I don't need you kicking down anything in here.");
	MainScreen.text("</i>\"");
...
private getEggsHarvested():void {
	...
	if(player.looseness() >= 4) MainScreen.text("; your loose pussy doesn't hurt, either");
