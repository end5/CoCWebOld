...
internal function heliasAppearanceScreen():void {
	...
	if(flags[FlagEnum.HEL_BONUS_POINTS] < 150) outputText("bright orange glow");
	...
	//if Isabella is cool: 
	if(flags[FlagEnum.HEL_ISABELLA_THREESOME_ENABLED] >= 1) outputText("; she’s got her blue bandanna wrapped around her head, mostly obscured by her fiery hair");
	...
	if (flags[FlagEnum.HELSPAWN_NAME] != 0) {
		outputText("  A dark trio of scars run down Hel’s thighs, left by " + flags[FlagEnum.HELSPAWN_NAME] + "’s youthful claws.");
	}
	else {
		switch (kGAMECLASS.helScene.pregnancy.event) {
			...
			default: //She's not pregnant, check if she's ready for it
					if (flags[FlagEnum.HEL_BONUS_POINTS] >= 150) outputText("  Hel’s long, reptilian tail is currently burning white-hot, signaling her body’s ready for motherhood.");
		}
	}
...
private function helSpawnsAppearanceScreen():void {
	clearOutput();
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) outputText("Your ");
	else outputText("Hel’s ");
	outputText("salamander daughter, " + flags[FlagEnum.HELSPAWN_NAME] + ", stands about six-foot-six tall, with pale skin and thick, ");
	if(flags[FlagEnum.HELSPAWN_DADDY] == 1) outputText("chitin-black");
	...
	if(flags[FlagEnum.HELSPAWN_PERSONALITY] >= 50) outputText("a scale bikini, just like her mother’s");
	else outputText("a short skirt, thigh-high boots, and a sky-blue blouse, in stark contrast to her mother’s sluttier attire");
	if(flags[FlagEnum.HEL_ISABELLA_THREESOME_ENABLED] >= 1) outputText(", with a brightly-colored scarf wrapped around her neck");
	outputText(". She has a human face, with ");
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) outputText("golden");
	else if(flags[FlagEnum.HELSPAWN_DADDY] == 2) outputText("bright green");
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] == 2) outputText("puppy-dog");
...
public function heliaBonusPointsAward():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.cockSpot.hasCock()) outputText(" and your [chest] and your [vagina]");
	if(!player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) outputText(" [vagina] and [chest]");
...
private function heliaLoveFollowup():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("even another woman, it seems, ");
	...
	//If HelLove = No:
	if(flags[FlagEnum.HEL_LOVE] == -1) outputText("Even if you don't love me, you'd love our child, right?  You can fuck me full of kids, or even... even get someone else to, I don't care.  I just </i>need<i> it, [name].  More than anything else in the whole wide world.");
	else {
		outputText("I love you, [name].  I love you so much.  I want to share this with you.  ");
		if(player.lowerBody.cockSpot.hasCock()) outputText("You'll fuck me full of kids, right?  Please?  Give me your seed, [name].  Give it all to me until I'm fucking gravid with your salamander babies.");
	...
	else if(player.lowerBody.cockSpot.hasCock()) outputText("  <b>Unfortunately, you're too big to squeeze inside Helia to do the business yourself.  You might need to shrink down some.</b>");
...
internal function haveAKid():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 2) outputText("supports her, pressing down on your [chest]");
...
private function HaveAHellKidPartII():void {
	...
	if(flags[FlagEnum.HEL_LOVE] == 1) outputText(", and I'm so very, very glad you love me back.");
...
private function getAnotherDad():void {
	...
	if(!player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.statusAffects.get("TelAdre").value1 >= 1) outputText("There's Miko and Mai from the bar.  Mai's said she wanted a kid, but can't take care of one... she'd probably be willing to fuck one into me!  If that's not alright, then... lemme think.  ");
...
private function noKidsHel():void {
	...
	if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(helFollower.heliaCapacity()) >= 0) outputText("You couldn't if you wanted to anyway - you're too big to fit.  ");
...
public function helGotKnockedUp():void {
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] == 2) outputText("warm ");
	else outputText("wan ");
...
private function sureHelGimmeSpidahBoyDetails():void {
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 20) outputText(", though still nothing compared to you, lover mine");
...
private function youWantABoy():void {
	...
	if(player.lowerBody.cockSpot.hasCock() && !player.lowerBody.vaginaSpot.hasVagina()) outputText("If I were a guy, I'd want a big strong son to hang out with, too.  Take him fishing, teach him how to fight the way you do... you'll make a great dad, my love.  I'm sure you will.");
...
internal function heliaTalkSeven():void {
	...
	//if PC isn't daddy:
	if(flags[FlagEnum.HELSPAWN_DADDY] != 0) outputText(", even if he's not your get, I can hope, you know?  If he spends enough time around you, maybe he'll take more after you than me");
	outputText(".</i>\"");
	menu();
...
private function encouragePregalia():void {
	...
	if(flags[FlagEnum.HEL_LOVE] == 1) outputText("You love Hel");
...
private function helsLifestyle():void {
	...
	if(flags[FlagEnum.HEL_LOVE] == 1) outputText("and I hope that's part of why you love me");
...
public function heliaBirthtime():void {
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] == 1) outputText("your ");
	else outputText("her ");
...
private function applyHelspawnName():void {
	...
	//Easter Egg Names:
	clearOutput();
	if(flags[FlagEnum.HELSPAWN_NAME] == "Helspawn") outputText("\"<i>That's what I was gonna call her!  Except not at all.  God dammit, seriously, [name]?</i>\"\n\n");
	if(flags[FlagEnum.HELSPAWN_NAME] == "Jayne") outputText("\"<i>That sounds like a hero's name... I like it.  A name that demands respect!</i>\"\n\n");
	if(flags[FlagEnum.HELSPAWN_NAME] == "Hesper") outputText("\"<i>Ah, the Evening Star.  She'll be the star of my life, alright...</i>\"\n\n");
	if(flags[FlagEnum.HELSPAWN_NAME] == "Kiri") outputText("\"<i>Aw, that's great, lover.  Sis'll shit her pants when she hears her niece is named after her!  Gonna get confusing, though...</i>\"\n\n");
	if(flags[FlagEnum.HELSPAWN_NAME] == "Mai") outputText("\"<i>Aw, that's great, lover.  Mai'll be honored, I know it.  And it is a great name...</i>\"\n\n");
	if(flags[FlagEnum.HELSPAWN_NAME] == "Tanis") outputText("\<i>I don't... I don't know if I want her to have that name, lover.  It's got a lot of baggage, but... but it does my mother an honor.  More than I could ever give her.</i>\"\n\n");
	//Syn: 
	if(flags[FlagEnum.HELSPAWN_NAME] == "Syn") outputText("\"Awesome.  I was hoping you'd choose that one, lover mine.</i>\"\n\n");
	if(flags[FlagEnum.HELSPAWN_NAME] == "Chara") outputText("\"<i>Awesome.  You liked my ideas, after all!</i>\"\n\n");
	
	outputText("\"<i>So... " + flags[FlagEnum.HELSPAWN_NAME] + " it is.  That's a good name, my love.  A strong name.  ");
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) outputText("Our");
	else outputText("my");
...
internal function heliaTalkEight():void {
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) outputText("parental ");
...
internal function playWithYourKid():void {
	clearOutput();
	spriteSelect(68);
	outputText("Picking " + flags[FlagEnum.HELSPAWN_NAME] + " up off the ground, you spin the giggling little girl around, laughing as your ");
	if(flags[FlagEnum.HELSPAWN_DADDY] != 0) outputText("ward ");
	else outputText("daughter ");
	outputText("cries out in surprise and glee.  Soon, you're bouncing her, then down on the ground crawling around with her, playing peek-a-boo and a dozen other games as you and " + flags[FlagEnum.HELSPAWN_NAME] + " laugh and hug.  Helia herself slips down with you before you know it, holding her daughter close as she tries to communicate, teaching the ever-growing girl a few words at a time.");
...
public function helSpawnGraduation():void {
	...
	outputText("You wake up to a scream, high-pitched and terrified.  You bolt up out of bed, tangling up in your bedroll for a minute before stumbling over to Hel and... " + flags[FlagEnum.HELSPAWN_NAME] + "?");
	outputText("\n\nYou do a double take as you lay eyes on the ");
	if(flags[FlagEnum.HELSPAWN_DADDY] == 1) outputText("black");
	else outputText("red");
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] == 2) outputText("And the puppy ears, now more wolf-like, are a dead giveaway.  Utterly unique to her.  ");
...
private function encourageHelspawn():void {
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] != 1) outputText("red ");
...
public function helSpawnChoosesAFightingStyle():void {
	clearOutput();
	spriteSelect(68);
	outputText("As you make your way back to camp, you begin to hear the sounds of fighting from ahead.  Readying your [weapon], you pick up the pace until your campsite comes into view.  You relax, seeing that it's only " + flags[FlagEnum.HELSPAWN_NAME] + " and her mother.  Helia's standing near the center of camp, ");
	if(camp.companionsCount() > 3) outputText("apparently having cleared out most of your other followers for the moment, ");
	outputText("her sword held at a guard pose between herself and a training dummy. " + flags[FlagEnum.HELSPAWN_NAME] + " stands a few feet behind her mother, watching intently as Hel paces around the dummy, breathing hard.");
...
private function snipermanders():void {
	...
	//If Rath is @ camp:
	if(player.statusAffects.has("CampRathazul")) outputText("  You hear your old alchemist friend suddenly putting up a ruckus as the sounds of breaking glass echo throughout camp.  " + flags[FlagEnum.HELSPAWN_NAME] + " looks up at you nervously, but you ruffle her fiery hair and tell her to try again. Rath is probably just fine.");
	...
	else if(latexGooFollower()) outputText("  You hear a sudden yelp of pain from across camp.  " + flags[FlagEnum.GOO_NAME] + " slithers up to you with an irritated look on her gooey face, pointing an accusing finger at an arrow sticking out of her tit.  \"<i>Dammit, [name], I'm a googirl, not a pin cushion!</i>\"  You wave her off, and tell " + flags[FlagEnum.HELSPAWN_NAME] + " to try again.");
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) outputText("s");
	outputText("."); 
	doNext(camp.returnToCampUseOneHour);
}
...
private function swordAndBoardmander():void {
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) outputText("\n\nYour");
	else outputText("\n\nHer");
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] != 0) outputText("her");
...
private function dasBarbarimander():void {
	...
	if(player.stats.gems >= 200) {
		outputText("You sigh and dig out a handful of gems.  Helia beams at you, planting a quick kiss on your cheek before grabbing " + flags[FlagEnum.HELSPAWN_NAME] + " and the loot.  \"<i>Thanks for the loan, lover mine.  C'mon, sweetie, we're going for a trip to town.</i>\"");
...
public function helspawnIsASlut():void {
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) outputText("daughter ");
	else outputText("ward ");
	outputText("is doing.  Well, it doesn't take a genius to figure that out, really, but still, you have to be sure.  You slide right up to them, just out of their lines of sight, as " + flags[FlagEnum.HELSPAWN_NAME] + " turns around, letting the spider-kin unclasp her bra and grab her tits, squeezing the small mounds as " + flags[FlagEnum.HELSPAWN_NAME] + " suppresses a moan, reaching back to slip a hand into the spider's crotch, grabbing her - his dick.");
	...
	if(player.stats.cor >= 50) outputText("  And by mistake you clearly mean old enough to get her ass fucked in by a cute little spider trap... you contemplate jumping in on it, but with a sigh figure it's " + flags[FlagEnum.HELSPAWN_NAME] + "'s catch.  Let her enjoy it.");
	menu();
...
private function helSpawnStopFucking():void {
	clearOutput();
	spriteSelect(68);
	outputText("You sigh and step out of the shadows, grabbing the feminine little spider boy by the shoulder and yanking him away from " + flags[FlagEnum.HELSPAWN_NAME] + ".  \"<i>And just what the hell do you two think you're doing?</i>\" you yell, scowling ");
	if(player.tallness >= 90) outputText("up ");
	else if(player.tallness <= 72) outputText("down ");
	outputText("at your ");
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) outputText("daughter ");
	else outputText("ward ");
	outputText(". " + flags[FlagEnum.HELSPAWN_NAME] + " gives a shrill cry of surprise and tumbles away from you, trying to cover herself as best she can; the spider trap squirms in your unrelenting grasp, one set of arms covering his rapidly-wilting erection as the other flails around, trying to get you off of him.  Not happening.");
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) outputText("my dad");
	...
	//If No Hel Love:
	if(flags[FlagEnum.HEL_LOVE] != 0) outputText("  You don't want to be a whore like your mother, do you?  The girl that everyone's had a ride with.");
	//if HelLove:
	if(flags[FlagEnum.HEL_LOVE] == 1) outputText("  I'm not going to a raise a whore, " + flags[FlagEnum.HELSPAWN_NAME] + ".  Your mother's made mistakes, sure, but...</i>\" you sigh.  \"<i>");
	outputText("  You don't have to be that person, " + flags[FlagEnum.HELSPAWN_NAME] + ".  Don't just give yourself out to the first person that asks, you know?  Save it for someone you really care about.  Trust me, it'll be worth it.</i>\"");
...
public function helspawnsMainMenu():void {
	clearOutput();
	outputText("You call " + flags[FlagEnum.HELSPAWN_NAME] + " over to you and she comes running, ");
...
private function hugHelspawn():void {
	...
	if(player.tallness <= 72) {
		outputText("You reach up and pull " + flags[FlagEnum.HELSPAWN_NAME] + " down to your level, wrapping your arms tight around her slender shoulders.  Grinning from ear to ear, the towering girl squeezes you right back, her powerful muscles just about crushing your smaller body.  Her tail slips around your [hips], the warm leather binding you to your beloved ");
		if(flags[FlagEnum.HELSPAWN_DADDY] == 0) outputText("daughter ");
		else outputText("ward ");
	...
	//{If PC is 6' or taller:}
		...
		outputText("You grab " + flags[FlagEnum.HELSPAWN_NAME] + " and pull her into a tight hug.  She lets out a girlish giggle as she falls into your arms and squeezes you right back.  Her powerful arms threaten to crush your bigger body, smothering you in affection.  Her tail slips around your [leg], the warm leather binding you to your beloved ");
		if(flags[FlagEnum.HELSPAWN_DADDY] == 0) outputText("daughter ");
		else outputText("ward ");
...
private function talkToHelspawn():void {
	...
	if(flags[FlagEnum.HAD_FIRST_HELSPAWN_TALK] == 0) {
		...
		//{if Sluttymander:
		if(flags[FlagEnum.HELSPAWN_PERSONALITY] >= 50) outputText("  Even if she wants to be just like mom, she shouldn't be bringing monsters home.  Some of them may be dangerous, and she's not a warrior quite like you and Helia yet.");
		...
		if(flags[FlagEnum.HELSPAWN_DADDY] == 0) outputText("Of course, as her father, you could always say it's your duty to keep her happy...");
	...
	//{Needs Rath at camp]
		...
		outputText("\"<i>" + flags[FlagEnum.HELSPAWN_NAME] + "!</i>\" you hear a ragged old voice call, \"<i>Get back here this instant!  I'm not done with you yet!</i>\"");
	...
	//{Bath Slut w/ DD or HHH must be at camp}
		...
		outputText("\"<i>Hey, have you seen " + flags[FlagEnum.MILK_NAME] + " around anywhere, " + championRef() + "?</i>\" " + flags[FlagEnum.HELSPAWN_NAME] + " asks, looking around the camp.  \"<i>Mom asked me to milk her, but I haven't seen her around.  She wouldn't have run off, would she?</i>\"");
		if(flags[FlagEnum.MILK_SIZE] == 1) outputText("\n\nYou tell her it'd be more of a wobble, but");
	...
	//{Isabella must be at camp}
		...
		outputText("\"<i>Oh hey, " + championRef() + "!</i>\" " + flags[FlagEnum.HELSPAWN_NAME] + " says, suddenly grinning, \"<i>Mom bought me a new guitar in town.  Wanna come listen?</i>\"");
		...
		outputText(".  You smile as Isabella seats herself beside " + flags[FlagEnum.HELSPAWN_NAME] + ", joining their voices together for the final chorus that leaves them both shaking as " + flags[FlagEnum.HELSPAWN_NAME] + " strums the last, desperate notes, nearly clawing through the strings in her passion.");
		
		outputText("\"<i>");
		if(!isabellaAccent()) outputText("Sehr gut");
		else outputText("Very good");
		outputText(" little " + flags[FlagEnum.HELSPAWN_NAME] + ",</i>\" Isabella says, pulling the young 'mander into a great big hug that threatens to smother her betwixt the cowgirl's massive bosoms.  Grinning, you congratulate " + flags[FlagEnum.HELSPAWN_NAME] + " on her performance, showering your girl in praise before you depart, leaving her to Isabella's instruction.  As you walk away, you can't help but notice Helia standing a short ways off, rubbing her eyes.");
...
private function dontFuckAlex():void {
	clearOutput();
	outputText("Stroking " + flags[FlagEnum.HELSPAWN_NAME] + "'s hair, you try to tell her that there's a better way, that she doesn't have to just fuck everything she comes across.  Her mother's that way, sure, but she could do so much better, so much more with herself than giving in to constant lust.");
	//{If Sluttymander:}
...
private function sparHelspawn():void {
	clearOutput();
	outputText("You ask " + flags[FlagEnum.HELSPAWN_NAME] + " if she's up for some battle practice, and she answers with an eager nod as she grabs her weapon.");
	//If Sluttymander: 
...
internal function loseSparringToDaughter():void {
	...
	if(flags[FlagEnum.HELSPAWN_PERSONALITY] >= 50) {
		...
		outputText(", " + flags[FlagEnum.HELSPAWN_NAME] + " quickly sweeps your [legs] out from under you, dropping you right on your ass.  You collapse with a grunt, ");
		...
		outputText(", and a moment later " + flags[FlagEnum.HELSPAWN_NAME] + "'s on you, straddling you with her powerful legs, pushing you down into the dirt.");
	...
	//Else If Chastemander:
		...
		outputText(", " + flags[FlagEnum.HELSPAWN_NAME] + " grabs your arm, catching you before you can fall.  \"<i>Easy there, " + championRef() + ", I got you,</i>\" she says, pulling you into a quick hug.");
...
internal function beatUpYourDaughter():void {
	...
	if(flags[FlagEnum.HELSPAWN_PERSONALITY] >= 50 && monster.lust > 99) {
		...
		//If No Incest: 
		if(flags[FlagEnum.HELSPAWN_INCEST] == 0) outputText("  You wash your hands of the defeated slut and head back to camp, leaving her to work through her tension herself.");
		else {
			...
			if(!player.lowerBody.cockSpot.hasCock()) outputText("sex");
			else outputText("your cock");
			outputText("... and that if she's lucky, you might tend to her when you've cooled off.");
		}
	}
	else {
		outputText(flags[FlagEnum.HELSPAWN_NAME] + " stumbles back, dropping her weapon and waving her arms, \"<i>I give, I give.</i>\"");
...
public function maiVisitsHerKids():void {
	clearOutput();
	spriteSelect(68);
	outputText("As you head back into camp, you notice Hel and " + flags[FlagEnum.HELSPAWN_NAME] + " are sitting around the cook fire in the center of camp, with your foster daughter balanced precariously on the lap of her own father, Mai the fox-girl.");
...
public function spiderBrosGift():void {
	...
	outputText("As you're getting ready to head out for the day, you notice " + flags[FlagEnum.HELSPAWN_NAME] + " prancing around camp with a long, brightly-colored scarf wrapped around her neck, standing painfully in contrast with her midnight-black scales and pale flesh.");
...
public function hakonAndKiriComeVisit():void {
	...
	outputText("\n\n\"<i>Sure!  ");
	if(flags[FlagEnum.HELSPAWN_WEAPON] == "scimitar and shield") outputText(championRef() + "'s been teaching me how to fight.");
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) outputText("your old man");
	else outputText("[name]");
	outputText(", too.</i>\"");
