...
internal function heliasAppearanceScreen():void {
	clearOutput();
	spriteSelect(68);
	Render.text("Hel the salamander stands seven feet tall, with pale skin and thick, bright-red scales covering her arms and legs, though she has a normal human torso and face.  A fiery tail swishes gaily behind her, blazing with a ");
	if(flags[FlagEnum.HEL_BONUS_POINTS] < 150) Render.text("bright orange glow");
	else Render.text("bright white glow");
	Render.text(" that lets off a pleasant heat, though it never seems to burn you.  Hel is wearing her scale bikini and a leather thong, and using her scimitar as a weapon");
	//if Isabella is cool: 
	if(flags[FlagEnum.HEL_ISABELLA_THREESOME_ENABLED] >= 1) Render.text("; she’s got her blue bandanna wrapped around her head, mostly obscured by her fiery hair");
	Render.text(".  She has a human face, with bright red eyes, gentle, feminine features and a smattering of pale scales on her cheeks, like freckles.  Hel has long, bright-red hair bound in a pony-tail that hangs down her back.  She has wide-flared hips and a soft, squishy butt.  Her two reptilian legs are visibly adorned with scales and claws, ending in soft, leathery soles.");
	if (flags[FlagEnum.HELSPAWN_NAME] != 0) {
		Render.text("  A dark trio of scars run down Hel’s thighs, left by " + flags[FlagEnum.HELSPAWN_NAME] + "’s youthful claws.");
	}
	else {
		switch (kGAMECLASS.helScene.pregnancy.event) {
			case 1: //She's pregnant, but no special text yet
					break;
			case 2: Render.text("  Hel's just starting to show a little bulge of pregnancy.");
					break;
			case 3: Render.text("  Hel’s belly is starting to look fairly bloated, swelling with her child.");
					break;
			case 4: Render.text("  Hel’s belly is positively gravid, full of a little salamander child.");
					break;
			default: //She's not pregnant, check if she's ready for it
					if (flags[FlagEnum.HEL_BONUS_POINTS] >= 150) Render.text("  Hel’s long, reptilian tail is currently burning white-hot, signaling her body’s ready for motherhood.");
		}
	}
...
private helSpawnsAppearanceScreen():void {
	clearOutput();
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) Render.text("Your ");
	else Render.text("Hel’s ");
	Render.text("salamander daughter, " + flags[FlagEnum.HELSPAWN_NAME] + ", stands about six-foot-six tall, with pale skin and thick, ");
	if(flags[FlagEnum.HELSPAWN_DADDY] == 1) Render.text("chitin-black");
	else Render.text("bright-red");
	Render.text(" scales covering her arms and legs, though she has a normal human face and torso.  A long leathery tail swishes happily behind her, surrounded by a dim fiery aura that emits a pleasant heat, though thankfully it isn’t hot to the touch, seemingly incapable of harming you.  " + flags[FlagEnum.HELSPAWN_NAME] + " is wearing ");
	if(flags[FlagEnum.HELSPAWN_PERSONALITY] >= 50) Render.text("a scale bikini, just like her mother’s");
	else Render.text("a short skirt, thigh-high boots, and a sky-blue blouse, in stark contrast to her mother’s sluttier attire");
	if(flags[FlagEnum.HEL_ISABELLA_THREESOME_ENABLED] >= 1) Render.text(", with a brightly-colored scarf wrapped around her neck");
	Render.text(". She has a human face, with ");
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) Render.text("golden");
	else if(flags[FlagEnum.HELSPAWN_DADDY] == 2) Render.text("bright green");
	else Render.text("crimson");
	Render.text(" eyes, girlish features, and a smattering of pale red scales on her cheeks, like freckles.  Her short, dark-red hair cuts off just below her ");
	if(flags[FlagEnum.HELSPAWN_DADDY] == 2) Render.text("puppy-dog");
	else Render.text("reptilian");
	Render.text(" ears, and it’s adorned with a pretty pink bow.  She has surprisingly large hips, and a big, squishy butt behind her.  Her reptilian legs are covered with thick, dark scales, ending in short claws atop soft, leathery soles.");
...
public heliaBonusPointsAward():void {
	...
	if(camp.companionsCount() == 1) Render.text("You cock an eyebrow, saying that you're already alone, but she shakes her head, saying she needs somewhere private, away from prying eyes.  ");
	...
	if(player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.cockSpot.hasCock()) Render.text(" and your [chest] and your [vagina]");
	if(!player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) Render.text(" [vagina] and [chest]");
	Render.text(" and everything else about you.  But... but that doesn't mean anything.  It doesn't.  I say I love minotaur dicks, and centaurs, and those two fox pricks at the bar filling both my holes, and I love beer and fighting and ramming my tail up peoples' assholes.  But that's not real love, right?  Love is - oh, god, I'm making a mess of this.  Again. I keep doing this; it always works out so much better in my head.</i>\"");
...
private noYouDontLoveHeliaYouMonster():void {
	clearOutput();
	spriteSelect(68);
	Render.text("As much as you enjoy the sex, as much as you like Hel, you can't say you feel that way towards her.  You grasp her shoulders, telling her as gently as possible how you feel.  She nods slowly as you speak, rubbing at her eyes by the time you're finished.");
...
private yesHeliaILoveYourButtHoleReaming():void {
	clearOutput();
	spriteSelect(68);
	Render.text("Smiling, you tell her yes.  The salamander cries out with joy, jumping into your arms and kissing you, squeezing you so tightly that you feel like you're about to black out by the time she breaks the kiss, stepping back with chest heaving.");
...
private heliaLoveFollowup():void {
	...
	//If HelLove = No:
	if(flags[FlagEnum.HEL_LOVE] == -1) Render.text("Even if you don't love me, you'd love our child, right?  You can fuck me full of kids, or even... even get someone else to, I don't care.  I just </i>need<i> it, [name].  More than anything else in the whole wide world.");
	else {
		Render.text("I love you, [name].  I love you so much.  I want to share this with you.  ");
		if(player.lowerBody.cockSpot.hasCock()) Render.text("You'll fuck me full of kids, right?  Please?  Give me your seed, [name].  Give it all to me until I'm fucking gravid with your salamander babies.");
		else Render.text("Even if you can't fuck me full of babies... you could grow a dick!  There's plenty of things in this god-forsaken world that do that. O-or if you don't want to, I bet I can get one of the fox-girls, or a minotaur, or something.  You know I can find a dick somewhere, just say the word.");
	}
	Render.text("  So what do you say, [name]?  Let's have a kid!</i>\"");
	menu();
	if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(helFollower.heliaCapacity()) >= 0) addButton(0,"Have A Kid",haveAKid);
	else if(player.lowerBody.cockSpot.hasCock()) Render.text("  <b>Unfortunately, you're too big to squeeze inside Helia to do the business yourself.  You might need to shrink down some.</b>");
...
internal function haveAKid():void {
	...
	else Render.text("the base of your prick");
	Render.text(" before coiling around the shaft with serpentine dexterity.  Her tail contracts and squeezes, undulating across your " + cockDescript(0) + " until it finally massages a thick blob of pre from your " + player.cockHead() + ", dribbling out to stain the wasteland ground.");
...
private HaveAHellKidPartII():void {
	...
	if(flags[FlagEnum.HEL_LOVE] == 1) Render.text(", and I'm so very, very glad you love me back.");
	else Render.text(", even if you can't say the same.  And that's alright; it doesn't change my heart, and it never will. I love you");
	Render.text(".</i>\"");
	...
	if(player.tallness >= 90) Render.text("up ");
	else if(player.tallness <= 72) Render.text("down ");
	Render.text("at you, holding you tight for a long moment before stepping away, still holding your hands.  \"<i>This is all new to me, [name].  I never pictured myself as a mother, with a mate and a stable, safe home - or as stable and safe as anything these days - but with you by my side, there's nothing I can't take on.</i>\"");
...
private getAnotherDad():void {
	...
	if(!player.lowerBody.cockSpot.hasCock()) {
		Render.text("You tell Helia you'd love to a share a child with her, but you're not... properly equipped for the endeavor.  \"<i>That's fine!  I can... I can wait, a little.  I-if you want to go grow one, I mean.  If not, then we can find someone with a cock.  ");
		if(player.statusAffects.get("TelAdre").value1 >= 1) Render.text("There's Miko and Mai from the bar.  Mai's said she wanted a kid, but can't take care of one... she'd probably be willing to fuck one into me!  If that's not alright, then... lemme think.  ");
		Render.text("Uh, maybe not a minotaur... they always plug more minotaurs, and I don't want a bull coming out of my twat.  Uh, maybe I could track down one of the spider boys from the swamp and jump on </i>his<i> dick.  They're pretty cute, right?  Dunno how that'd affect a child, though.  Maybe he'd end up with like, extra eyes, or chitin?  Still, better than an imp or some shit.  So what do you think?  Wanna grow a dick, or leave the knocking-up to someone else?</i>\"");
		//{Options in next section}
	}
	//[Another Dad] (PC has a dick)
	else {
		Render.text("\"<i>W-wha?</i>\" Hel says, cocking an eyebrow.  \"<i>You sure, [name]?  I-if you don't want to be the dad, I guess I understand.  But why?  Am I not... no, nevermind.  I don't want to know.  Okay, uh, we can find someone with a cock.  ");
		if(player.statusAffects.get("TelAdre").value1 >= 1) Render.text("There's Miko and Mai from the bar.  Mai's said she wanted a kid, but can't take care of one... she'd probably be willing to fuck one into me!  If that's not alright, then... lemme think.  ");
		Render.text("Uh, maybe not a minotaur... they always plug more minotaurs, and I don't want a bull coming out of my twat.  Uh, maybe I could track down one of the spider boys from the swamp and jump on </i>his<i> dick.  They're pretty cute, right?  Dunno how that'd affect a child, though.  Maybe he'd end up with like, extra eyes, or chitin?  Still, better than an imp or some shit.  So what do you think?</i>\"");
	}
	menu();
	//{If Tel'Adre has been discovered: [Mai]}
	if(player.statusAffects.get("TelAdre").value1 >= 1) addButton(0,"Mai",maiWouldBeTheBestInseminator);
	//[Spiderboy]
	addButton(1,"Spiderboy",spiderboyWouldBeBestDad);
	//[I will] (If PC has a dick)
	if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(helFollower.heliaCapacity()) >= 0) addButton(2,"I Will",haveAKid);
	else if(!player.lowerBody.cockSpot.hasCock()) addButton(2,"I Will",growingDicks4Hel);
...
private maiWouldBeTheBestInseminator():void {
	clearOutput();
	spriteSelect(68);
	Render.text("You tell Hel that you think Mai would make a lovely father.  Helia nods her agreement, saying, \"<i>Yeah, I agree.  She's a beauty, and I'm sure our child will be stunning... you wouldn't mind if she visited, right?  I mean, you and I will be raising our kid - and he'll be ours for sure - but I'm sure Mai will want to at least visit her kid.</i>\"");
...
private spiderboyWouldBeBestDad():void {
	clearOutput();
	spriteSelect(68);
	Render.text("You tell Helia to go find a spider boy to jump.  She beams at you, and skips off toward the swamp calling, \"<i>Thank you, thank you thank you, [name]!</i>\" over her shoulder as she goes.  You suppose the next time you see her, Hel's probably going to be pregnant with the child you'll be helping to raise.");
...
private growingDicks4Hel():void {
	clearOutput();
	spriteSelect(68);
	Render.text("You tell Hel to give you a little while, that you'll go find something to grow a cock for her.");
...
private noKidsHel():void {
	clearOutput();
	spriteSelect(68);
	Render.text("You shake you head and say no, you don't want to have children.  Not right now, anyway.  ");
	if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(helFollower.heliaCapacity()) >= 0) Render.text("You couldn't if you wanted to anyway - you're too big to fit.  ");
	Render.text("As the words leave your lips, you can see Hel's shoulders slump, a crestfallen look spreading across her face.  \"<i>A-are you sure?  Please, [name], I really, really want a child.  For </i>us<i> to have one.</i>\"");
...
public helGotKnockedUp():void {
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] == 2) Render.text("warm ");
	else Render.text("wan ");
	Render.text("smile as you approach, scooting over to allow you to sit beside her.  She almost seems distant, though she's quick to wrap her tail around your waist, hugging you close against her.");
...
private dontTellMeAboutSpiderboy():void {
	clearOutput();
	spriteSelect(68);
	Render.text("\"<i>So, let's just say I came away from that encounter carrying enough cute spiderboy cum to fill a bucket.  If I'm not carrying his child now, then I'm hopeless.  But... you know, I think I can feel it, [name].  I shouldn't be able to, but I can.  Oh, god,</i>\" Hel says, looking pointedly away from you, staring wide-eyed into the morning sky.");
...
private dontTellMeAboutMai():void {
	clearOutput();
	spriteSelect(68);
	Render.text("\"<i>So, let's just say I came away from that encounter carrying enough foxy herm cum to fill a bucket.  If I'm not carrying her child now, then I'm hopeless.  But... you know, I think I can feel it, [name].  I shouldn't be able to, but I can.  Oh, god,</i>\" Hel says, looking pointedly away from you, staring wide-eyed into the morning sky.");
...
public bulgyCampNotice():void {
	clearOutput();
	spriteSelect(68);
	Render.text("As you're walking through camp, your eyes wander over toward Helia, sunning herself on a stone near the edge of camp.  You can just see that her belly's starting to bulge out from under her, and Hel's hands lie protectively over her full womb, absently rubbing the bulge of her stomach.");
	doNext(playerMenu);
}
...
public heliaSwollenNotice():void {
	clearOutput();
	spriteSelect(68);
	Render.text("You note that Hel's wandering aimlessly around camp, one hand over her belly as she mumbles to herself.  You could swear she was cursing her now quite swollen belly, but suddenly she gives a girlishly happy cry and waves you over.\n");
...
private youWantABoy():void {
	clearOutput();
	spriteSelect(68);
	Render.text("\"<i>Is that so? Yeah, I can see it.  ");
	if(player.lowerBody.cockSpot.hasCock() && !player.lowerBody.vaginaSpot.hasVagina()) Render.text("If I were a guy, I'd want a big strong son to hang out with, too.  Take him fishing, teach him how to fight the way you do... you'll make a great dad, my love.  I'm sure you will.");
	else Render.text("Would be nice to have a man around here, you know?  I miss hanging around the boys back home, watching 'em strut like peacocks for every passing girl.");
	Render.text(" And any son of mine is going to be a real lady killer, mark my words.  We're going to have to fight off whole hordes of goblin sluts, all looking for a piece of our handsome little boy before you know it.</i>\"");
...
private youWantAGirl():void {
	clearOutput();
	spriteSelect(68);
	Render.text("\"<i>Yeah, a girl would be pretty great.  ");
	//if PC is male:
	if(player.lowerBody.cockSpot.hasCock() && !player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("I dunno if you had any sisters growing up, [name], but let me warn you: a little girl, especially a little salamandress, is going to be a hell of a handful.  But I can just see you when she's all grown up, packing a big old sword and threatening every boy that wants a piece of her: 'Treat her right or you'll have the CHAMPION to deal with.  Rawr.'");
	}
	else Render.text("We're going to be a gaggle of tittering girls before you know it, though.  Salamanders grow up so fast, [name]...  I just hope she doesn't turn out like me, you know?  I don't know if I could stomach seeing my little girl becoming a wanton slut like her mom.  I might get jealous, after all...");
	Render.text("  Any daughter of ours is going to be a real beauty, mark my words. She'll make a succubus look like a toad before she's out of swaddling.</i>\"");
...
internal function heliaTalkSeven():void {
	...
	//if PC isn't daddy:
	if(flags[FlagEnum.HELSPAWN_DADDY] != 0) Render.text(", even if he's not your get, I can hope, you know?  If he spends enough time around you, maybe he'll take more after you than me");
	Render.text(".</i>\"");
	menu();
...
private encouragePregalia():void {
	clearOutput();
	spriteSelect(68);
	Render.text("You squeeze Hel's shoulder and tell her that she's going to be a great mother, and that her child would do well to learn from a beautiful, powerful warrior like dear old mom.  ");
	if(flags[FlagEnum.HEL_LOVE] == 1) Render.text("You love Hel");
	else Render.text("Hel's your friend");
	Render.text(", and if her child follows in her footsteps, so much the better.  Your lover manages a half-hearted laugh, blushing at your words.  Her tail tip brushes your cheek as its owner cuddles up against you, smiling.  \"<i>Thanks, lover mine.  I just hope I - we - can do right by the kid.  That's all.</i>\"");
...
private helsLifestyle():void {
	clearOutput();
	spriteSelect(68);
	Render.text("Being very serious for a moment, you tell Hel that if she's worried about her terrible choices rubbing off on her kid, maybe she ought to reconsider some of the things she's done: be less of a slut, drink less, be less of a bloodthirsty maniac in battle.  Slowly, Hel nods to the rhythm of your words.  \"<i>Yeah, maybe... maybe I should.  I don't know if I can change though, [name].  I'm not as young as I was - or at least, I don't feel it - and I've been living the way I have been for years.  I don't know if I could just give up the shit I do: the partying and the drinking, the fighting and the fucking.  It's part of who I am, ");
	if(flags[FlagEnum.HEL_LOVE] == 1) Render.text("and I hope that's part of why you love me");
	else Render.text("even though I know that's probably part of why you can't love me back");
	Render.text(".  I'll try and rein it in, but maybe... it'd be better if you were the one who raised my child, [name].  God knows I'm the least qualified person to do it.</i>\"");
...
private nameDatHelspawn():void {
	clearOutput();
	spriteSelect(68);
	Render.text("You and Helia share a laugh, looking down at ");
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) Render.text("your ");
	else Render.text("her ");
	Render.text("little girl as she suckles.  After a few happy, blissful minutes though, Hel turns to you with a question:");
...
private applyHelspawnName():void {
	...
	{
		clearOutput();
		if(mainView.nameBox.text == "") Render.text("<b>You must select a name.</b>", false);
		else if(mainView.nameBox.text == "Hellgirl") Render.text("\"<i>Hey, that's copyright infringement.  Probably.</i>\"");
		else if(mainView.nameBox.text == "Kid") Render.text("\"<i>Wow, what are you, five?</i>\"");
		else if(mainView.nameBox.text == "Phoenix") Render.text("\"<i>Oh hell no.  You're not naming my little girl after one of those... things!  Sorry Kiri, but still!</i>\"");
		else if(mainView.nameBox.text == "Savin") Render.text("\"<i>That's a boy's name, dumbass.</i>\"");
		else if(mainView.nameBox.text == "Helia") Render.text("\"<i>My favorite name!  Except it's kinda taken, love.  Don't want things to get too confusing around here, do you?</i>\"");
		else if(mainView.nameBox.text == "Mini-doofus") Render.text("\"<i>Oh yeah, Kiha'll get a laugh out of that.  You ass.</i>\"");
	...
	//Easter Egg Names:
	clearOutput();
	if(flags[FlagEnum.HELSPAWN_NAME] == "Helspawn") Render.text("\"<i>That's what I was gonna call her!  Except not at all.  God dammit, seriously, [name]?</i>\"\n\n");
	if(flags[FlagEnum.HELSPAWN_NAME] == "Jayne") Render.text("\"<i>That sounds like a hero's name... I like it.  A name that demands respect!</i>\"\n\n");
	if(flags[FlagEnum.HELSPAWN_NAME] == "Hesper") Render.text("\"<i>Ah, the Evening Star.  She'll be the star of my life, alright...</i>\"\n\n");
	if(flags[FlagEnum.HELSPAWN_NAME] == "Kiri") Render.text("\"<i>Aw, that's great, lover.  Sis'll shit her pants when she hears her niece is named after her!  Gonna get confusing, though...</i>\"\n\n");
	if(flags[FlagEnum.HELSPAWN_NAME] == "Mai") Render.text("\"<i>Aw, that's great, lover.  Mai'll be honored, I know it.  And it is a great name...</i>\"\n\n");
	if(flags[FlagEnum.HELSPAWN_NAME] == "Tanis") Render.text("\<i>I don't... I don't know if I want her to have that name, lover.  It's got a lot of baggage, but... but it does my mother an honor.  More than I could ever give her.</i>\"\n\n");
	//Syn: 
	if(flags[FlagEnum.HELSPAWN_NAME] == "Syn") Render.text("\"Awesome.  I was hoping you'd choose that one, lover mine.</i>\"\n\n");
	if(flags[FlagEnum.HELSPAWN_NAME] == "Chara") Render.text("\"<i>Awesome.  You liked my ideas, after all!</i>\"\n\n");
	
	Render.text("\"<i>So... " + flags[FlagEnum.HELSPAWN_NAME] + " it is.  That's a good name, my love.  A strong name.  ");
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) Render.text("Our");
	else Render.text("my");
	Render.text(" little girl's going to grow up big and strong with a name like that... and with you around to guide her,</i>\" Hel says, kissing your cheek.  \"<i>Why don't you give me a minute to catch my breath and get the little one situated, and let's talk.  Alright?</i>\"");
...
internal function heliaTalkEight():void {
	...
	spriteSelect(68);
	Render.text("Watching your little daughter crawl around, you and Hel sit down together arm in arm, smiling as the little salamander girl plays with bits and pieces of Hel's gear.");
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) Render.text("parental ");
	else Render.text("care giver's ");
	Render.text("pride, either.  She really is growing up faster, many times faster than a human girl.  And, by the sound of things, many times faster than a salamander ought to.  You ask Hel if that's right, and she nods gravely.");
	...
	Render.text(" little girl's going to be a big girl damn soon.  Let's enjoy it while we can, eh?</i>\"");
...
internal function playWithYourKid():void {
	clearOutput();
	spriteSelect(68);
	Render.text("Picking " + flags[FlagEnum.HELSPAWN_NAME] + " up off the ground, you spin the giggling little girl around, laughing as your ");
	if(flags[FlagEnum.HELSPAWN_DADDY] != 0) Render.text("ward ");
	else Render.text("daughter ");
	Render.text("cries out in surprise and glee.  Soon, you're bouncing her, then down on the ground crawling around with her, playing peek-a-boo and a dozen other games as you and " + flags[FlagEnum.HELSPAWN_NAME] + " laugh and hug.  Helia herself slips down with you before you know it, holding her daughter close as she tries to communicate, teaching the ever-growing girl a few words at a time.");
...
public helSpawnGraduation():void {
	...
	Render.text("You wake up to a scream, high-pitched and terrified.  You bolt up out of bed, tangling up in your bedroll for a minute before stumbling over to Hel and... " + flags[FlagEnum.HELSPAWN_NAME] + "?");
	Render.text("\n\nYou do a double take as you lay eyes on the ");
	if(flags[FlagEnum.HELSPAWN_DADDY] == 1) Render.text("black");
	else Render.text("red");
	Render.text("-scaled figure curled up next to Helia; Hel herself is looking wide-eyed down at the drowsy figure on her breast, just barely opening her eyes.  \"<i>Y-you're... you're huge!</i>\" Hel manages to gasp out, pushing herself back and away from her daughter.");
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] == 2) Render.text("And the puppy ears, now more wolf-like, are a dead giveaway.  Utterly unique to her.  ");
	Render.text("You sit down beside the obviously confused girl and stroke her hair, telling her it's all right, her mother's just startled, that's all.");
...
public helspawnDiscoversBooze():void {
	...
	Render.text("As you're moving through camp, you notice Hel's away from her little nook - probably off visiting the family, seeing as her cloak's gone.  You wouldn't have given her part of camp a second look, except for a sudden whiff of brimstone and burning brewery on the wind.  Cocking an eyebrow, you creep over to Hel's still, quiet as ");
	if(player.race() != "cat-boy" && player.race() != "cat-girl") Render.text("a cat");
	else Render.text("the cat you are");
	Render.text(", and poke your head around the iron body of the still.");
...
private scoldHelSpawn():void {
	clearOutput();
	spriteSelect(68);
	Render.text("You cross your arms over your chest and ask your little salamander exactly what she thinks she's doing.");
...
private encourageHelspawn():void {
	clearOutput();
	spriteSelect(68);
	Render.text("\"<i>Move over, you,</i>\" you say, sliding down beside your little salamander as you pop the tap open again.  \"<i>Gotta pace yourself, see,</i>\" you add, taking a long draught before closing it off again, savoring the potent taste of Hel's mighty brew.");
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] != 1) Render.text("red ");
	else Render.text("black ");
	Render.text("scales and the smell of a burning brewery.");
...
public helSpawnChoosesAFightingStyle():void {
	clearOutput();
	spriteSelect(68);
	Render.text("As you make your way back to camp, you begin to hear the sounds of fighting from ahead.  Readying your [weapon], you pick up the pace until your campsite comes into view.  You relax, seeing that it's only " + flags[FlagEnum.HELSPAWN_NAME] + " and her mother.  Helia's standing near the center of camp, ");
	if(camp.companionsCount() > 3) Render.text("apparently having cleared out most of your other followers for the moment, ");
	Render.text("her sword held at a guard pose between herself and a training dummy. " + flags[FlagEnum.HELSPAWN_NAME] + " stands a few feet behind her mother, watching intently as Hel paces around the dummy, breathing hard.");
	...
	menu();
	if(player.statusAffects.get("Kelt").value1 >= 100) addButton(2,"Bow",snipermanders);
...
private snipermanders():void {
	...
	Render.text("You tell Hel to take a break: you've got this handled. Grabbing your bow and quiver from over your shoulder, you hand them over to ");
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) Render.text("your");
	else Render.text("Hel's");
	Render.text(" eager daughter. " + flags[FlagEnum.HELSPAWN_NAME] + " takes them up with surprising reverence, holding them as gingerly as glass.  Grinning, you clasp her shoulder and tell her how to put the quiver on, and get her stance right for shooting.  It takes her a moment to get set up: her scaled, reptilian legs don't naturally stand in the shooting pose you've adopted");
	if(player.lowerBody == LOWER_BODY.LIZARD) Render.text(", even with your own lizard legs");
	Render.text(", and you find yourself having to correct her grip on the haft several times.  But, finally, you get her ready to shoot.");
	...
	//If Rath is @ camp:
	if(player.statusAffects.has("CampRathazul")) Render.text("  You hear your old alchemist friend suddenly putting up a ruckus as the sounds of breaking glass echo throughout camp.  " + flags[FlagEnum.HELSPAWN_NAME] + " looks up at you nervously, but you ruffle her fiery hair and tell her to try again. Rath is probably just fine.");
	//else if Valeria/Latexy is at camp: 
	else if(flags[FlagEnum.VALARIA_AT_CAMP] == 1) Render.text("  You hear a sudden yelp of pain from across camp. Valeria slithers up to you with an irritated look on her gooey face, pointing an accusing finger at an arrow sticking out of her tit.  \"<i>Dammit, [name], I'm a googirl, not a pin cushion!</i>\"  You wave her off, and tell " + flags[FlagEnum.HELSPAWN_NAME] + " to try again.");
	else if(latexGooFollower()) Render.text("  You hear a sudden yelp of pain from across camp.  " + flags[FlagEnum.GOO_NAME] + " slithers up to you with an irritated look on her gooey face, pointing an accusing finger at an arrow sticking out of her tit.  \"<i>Dammit, [name], I'm a googirl, not a pin cushion!</i>\"  You wave her off, and tell " + flags[FlagEnum.HELSPAWN_NAME] + " to try again.");
	...
	Render.text("daughter will be a warrior worthy of her parent");
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) Render.text("s");
	Render.text("."); 
	doNext(camp.returnToCampUseOneHour);
}
...
private swordAndBoardmander():void {
	...
	Render.text("You tell Hel to take a break: you've got this handled.  You grab some kindling and string from your supplies, and a few minutes later you've got a pair of small, vaguely sword-like practice weapons pieces together.  Tossing one to the young salamander, you tell her to come get some.");
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] != 0) Render.text("her");
	else Render.text("your");
	Render.text(" daughter on her toes.  She learns quickly, though, and soon she hardly needs reminding to keep her shield up and stance braced against Helia's overwhelming assaults.");
...
private dasBarbarimander():void {
	...
	if(player.stats.gems >= 200) {
		Render.text("You sigh and dig out a handful of gems.  Helia beams at you, planting a quick kiss on your cheek before grabbing " + flags[FlagEnum.HELSPAWN_NAME] + " and the loot.  \"<i>Thanks for the loan, lover mine.  C'mon, sweetie, we're going for a trip to town.</i>\"");
...
public helspawnIsASlut():void {
	...
	if(player.weaponName != "fists") Render.text("clutch your [weapon]");
	else Render.text("ball your fists");
	Render.text(" as the intruder passes by, ready to defend yourself and your loved ones.  The moment they're out of reach, you roll onto your [feet], looking around for signs of a massed demon attack.  Instead, you see the outline of " + flags[FlagEnum.HELSPAWN_NAME] + " in the dying embers of the fire light, leading what looks like a spider girl by the hand toward her bed.");
	
	Render.text("\n\nYou creep closer, trying to get a closer look at what your ");
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) Render.text("daughter ");
	else Render.text("ward ");
	Render.text("is doing.  Well, it doesn't take a genius to figure that out, really, but still, you have to be sure.  You slide right up to them, just out of their lines of sight, as " + flags[FlagEnum.HELSPAWN_NAME] + " turns around, letting the spider-kin unclasp her bra and grab her tits, squeezing the small mounds as " + flags[FlagEnum.HELSPAWN_NAME] + " suppresses a moan, reaching back to slip a hand into the spider's crotch, grabbing her - his dick.");
	...
	if(player.stats.cor >= 50) Render.text("  And by mistake you clearly mean old enough to get her ass fucked in by a cute little spider trap... you contemplate jumping in on it, but with a sigh figure it's " + flags[FlagEnum.HELSPAWN_NAME] + "'s catch.  Let her enjoy it.");
	menu();
...
private helSpawnStopFucking():void {
	clearOutput();
	spriteSelect(68);
	Render.text("You sigh and step out of the shadows, grabbing the feminine little spider boy by the shoulder and yanking him away from " + flags[FlagEnum.HELSPAWN_NAME] + ".  \"<i>And just what the hell do you two think you're doing?</i>\" you yell, scowling ");
	if(player.tallness >= 90) Render.text("up ");
	else if(player.tallness <= 72) Render.text("down ");
	Render.text("at your ");
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) Render.text("daughter ");
	else Render.text("ward ");
	Render.text(". " + flags[FlagEnum.HELSPAWN_NAME] + " gives a shrill cry of surprise and tumbles away from you, trying to cover herself as best she can; the spider trap squirms in your unrelenting grasp, one set of arms covering his rapidly-wilting erection as the other flails around, trying to get you off of him.  Not happening.");
	...
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) Render.text("my dad");
	else Render.text("[name]");
	Render.text(". " + championRef() + ", Alex.</i>\"");
	...
	//If No Hel Love:
	if(flags[FlagEnum.HEL_LOVE] != 0) Render.text("  You don't want to be a whore like your mother, do you?  The girl that everyone's had a ride with.");
	//if HelLove:
	if(flags[FlagEnum.HEL_LOVE] == 1) Render.text("  I'm not going to a raise a whore, " + flags[FlagEnum.HELSPAWN_NAME] + ".  Your mother's made mistakes, sure, but...</i>\" you sigh.  \"<i>");
	Render.text("  You don't have to be that person, " + flags[FlagEnum.HELSPAWN_NAME] + ".  Don't just give yourself out to the first person that asks, you know?  Save it for someone you really care about.  Trust me, it'll be worth it.</i>\"");
...
public helspawnsMainMenu():void {
	clearOutput();
	Render.text("You call " + flags[FlagEnum.HELSPAWN_NAME] + " over to you and she comes running, ");
	if(player.tallness <= 72) Render.text("grabbing you off your [feet] and clutching you in a tight hug");
	else Render.text("throwing her arms around you and hugging you tightly");
	Render.text(".  Laughing despite yourself, you ruffle her fiery hair and ask her if she’s got a minute.");
...
private hugHelspawn():void {
	...
	if(player.tallness <= 72) {
		Render.text("You reach up and pull " + flags[FlagEnum.HELSPAWN_NAME] + " down to your level, wrapping your arms tight around her slender shoulders.  Grinning from ear to ear, the towering girl squeezes you right back, her powerful muscles just about crushing your smaller body.  Her tail slips around your [hips], the warm leather binding you to your beloved ");
		if(flags[FlagEnum.HELSPAWN_DADDY] == 0) Render.text("daughter ");
		else Render.text("ward ");
		Render.text("as she nuzzles up against you.");
		
		Render.text("\n\n\"<i>Love you too, " + championRef() + ",</i>\" she laughs, planting a quick kiss on your cheek before letting you go.");
	}
	//{If PC is 6' or taller:}
	else {
		Render.text("You grab " + flags[FlagEnum.HELSPAWN_NAME] + " and pull her into a tight hug.  She lets out a girlish giggle as she falls into your arms and squeezes you right back.  Her powerful arms threaten to crush your bigger body, smothering you in affection.  Her tail slips around your [leg], the warm leather binding you to your beloved ");
		if(flags[FlagEnum.HELSPAWN_DADDY] == 0) Render.text("daughter ");
		else Render.text("ward ");
		Render.text(" as she nuzzles into your [chest].");
...
private talkToHelspawn():void {
	...
	if(flags[FlagEnum.HAD_FIRST_HELSPAWN_TALK] == 0) {
		...
		//{if Sluttymander:
		if(flags[FlagEnum.HELSPAWN_PERSONALITY] >= 50) Render.text("  Even if she wants to be just like mom, she shouldn't be bringing monsters home.  Some of them may be dangerous, and she's not a warrior quite like you and Helia yet.");
		else Render.text("  She should know better than to bring monsters back.  She's not her mother; lust doesn't have to dominate her life like that.");
		...
		if(flags[FlagEnum.HELSPAWN_DADDY] == 0) Render.text("Of course, as her father, you could always say it's your duty to keep her happy...");
		else Render.text("Of course, as her adoptive father, you could always say that you love her in a different, more intimate way.");
		Render.text("  She wouldn't need monsters with you around.");
		menu();
	...
	//{Kiha must be at camp}
		...
		Render.text("\"<i>Oh hey, " + championRef() + "!  You gotta come see what I was making!</i>\"");
	...
	//{Needs Rath at camp]
		...
		Render.text("\"<i>" + flags[FlagEnum.HELSPAWN_NAME] + "!</i>\" you hear a ragged old voice call, \"<i>Get back here this instant!  I'm not done with you yet!</i>\"");
		...
		if((amilyScene.amilyFollower() && !amilyScene.amilyCorrupt())) Render.text("Little Amily's solution was makeshift at best, and the children are beyond my reach to study.  Still.  ");
		Render.text("Imagine whole legions of children reared and raised in the time it takes a goblin to do the same. We might have a fighting chance for once, with numbers to match the demons.</i>\"");
	...
	//{Isabella must be at camp}
		...
		Render.text("\"<i>Oh hey, " + championRef() + "!</i>\" " + flags[FlagEnum.HELSPAWN_NAME] + " says, suddenly grinning, \"<i>Mom bought me a new guitar in town.  Wanna come listen?</i>\"");
		...
		if(player.race() != "human") Render.text(" and you yourself have suffered as well");
		Render.text(".  You smile as Isabella seats herself beside " + flags[FlagEnum.HELSPAWN_NAME] + ", joining their voices together for the final chorus that leaves them both shaking as " + flags[FlagEnum.HELSPAWN_NAME] + " strums the last, desperate notes, nearly clawing through the strings in her passion.");
		
		Render.text("\"<i>");
		if(!isabellaAccent()) Render.text("Sehr gut");
		else Render.text("Very good");
		Render.text(" little " + flags[FlagEnum.HELSPAWN_NAME] + ",</i>\" Isabella says, pulling the young 'mander into a great big hug that threatens to smother her betwixt the cowgirl's massive bosoms.  Grinning, you congratulate " + flags[FlagEnum.HELSPAWN_NAME] + " on her performance, showering your girl in praise before you depart, leaving her to Isabella's instruction.  As you walk away, you can't help but notice Helia standing a short ways off, rubbing her eyes.");
		...
		Render.text("Unfortunately, there doesn't seem to be anything in particular to talk about at the moment.  The two of you spend the time in companionable quiet, making smalltalk.");
		doNext(camp.returnToCampUseOneHour);
	}
}
...
private helSpawnBoyfriend():void {
	clearOutput();
	Render.text("\"<i>So tell me about this spider boy.</i>\"");
...
private dontFuckAlex():void {
	clearOutput();
	Render.text("Stroking " + flags[FlagEnum.HELSPAWN_NAME] + "'s hair, you try to tell her that there's a better way, that she doesn't have to just fuck everything she comes across.  Her mother's that way, sure, but she could do so much better, so much more with herself than giving in to constant lust.");
	//{If Sluttymander:}
...
private helSpawnSureMilkHerLater():void {
	clearOutput();
	Render.text("You chuckle, telling her to let the poor girl sleep.  There'll be plenty of milk later.");
	doNext(camp.returnToCampUseOneHour);
}
...
private helSpawnMilkHerNow():void {
	clearOutput();
	Render.text("\"<i>Oh, no.  You're not getting out of your chores that easy, kiddo.  In you go,</i>\" you say, giving her a little push toward the edge of the pool.");
...
private sparHelspawn():void {
	clearOutput();
	Render.text("You ask " + flags[FlagEnum.HELSPAWN_NAME] + " if she's up for some battle practice, and she answers with an eager nod as she grabs her weapon.");
	//If Sluttymander: 
...
internal function loseSparringToDaughter():void {
	//if Sluttymander:
	if(flags[FlagEnum.HELSPAWN_PERSONALITY] >= 50) {
		Render.text("As you stumble back, ");
		if(player.lust > 99) Render.text("succumbing to your own lusts");
		else Render.text("unable to withstand her unending hail of attacks");
		Render.text(", " + flags[FlagEnum.HELSPAWN_NAME] + " quickly sweeps your [legs] out from under you, dropping you right on your ass.  You collapse with a grunt, ");
		if(player.weaponName != "fists") Render.text("weapon tumbling out of hand");
		else Render.text("unable to even clench your fists anymore");
		Render.text(", and a moment later " + flags[FlagEnum.HELSPAWN_NAME] + "'s on you, straddling you with her powerful legs, pushing you down into the dirt.");
	...
	//Else If Chastemander:
		...
		Render.text(", " + flags[FlagEnum.HELSPAWN_NAME] + " grabs your arm, catching you before you can fall.  \"<i>Easy there, " + championRef() + ", I got you,</i>\" she says, pulling you into a quick hug.");
		...
		if(player.HP < 1) Render.text("recover from that ass kicking");
		else Render.text("dunk your head");
		Render.text(".");
	}
	cleanupAfterCombat();
}
...
internal function beatUpYourDaughter():void {
	...
	if(flags[FlagEnum.HELSPAWN_PERSONALITY] >= 50 && monster.lust > 99) {
		Render.text("\"<i>N-no more...</i>\" the slutty little salamander moans, slumping down to the ground, arms wrapping around herself.  \"<i>Fuck, you're sexy... so horny...</i>\" she groans, hands slipping down to her soaked bikini bottom.");
		...
		//If No Incest: 
		if(flags[FlagEnum.HELSPAWN_INCEST] == 0) Render.text("  You wash your hands of the defeated slut and head back to camp, leaving her to work through her tension herself.");
		else {
			Render.text("  You reach down and give your lovely, lusty daughter a pat on her expansive rear, telling her she'll always look her best with her ass in the air, begging for ");
			if(!player.lowerBody.cockSpot.hasCock()) Render.text("sex");
			else Render.text("your cock");
			Render.text("... and that if she's lucky, you might tend to her when you've cooled off.");
		}
	}
	else {
		Render.text(flags[FlagEnum.HELSPAWN_NAME] + " stumbles back, dropping her weapon and waving her arms, \"<i>I give, I give.</i>\"");
		...
		if(monster.HP < 1) Render.text(", rubbing her many bruises");
		else Render.text(", blushing brightly after your slutty display");
		Render.text(".  You chuckle and ruffle her hair, \"<i>C'mon, kiddo, let's get some food in you.</i>\"");
...
public maiVisitsHerKids():void {
	clearOutput();
	spriteSelect(68);
	Render.text("As you head back into camp, you notice Hel and " + flags[FlagEnum.HELSPAWN_NAME] + " are sitting around the cook fire in the center of camp, with your foster daughter balanced precariously on the lap of her own father, Mai the fox-girl.");
...
public spiderBrosGift():void {
	...
	Render.text("As you're getting ready to head out for the day, you notice " + flags[FlagEnum.HELSPAWN_NAME] + " prancing around camp with a long, brightly-colored scarf wrapped around her neck, standing painfully in contrast with her midnight-black scales and pale flesh.");
	...
	//if Isabella: 
	if(isabellaFollower()) Render.text(", or aunt Isabella might have knitted me a new one, I don't know");
	Render.text(".</i>\"");
...
public hakonAndKiriComeVisit():void {
	...
	Render.text("As you're returning to camp, you notice Helia running around like a chicken with her head cut off, fussing about damn near everything in her part of the camp");
	if(camp.companionsCount() > 1) Render.text(" and everyone else's too, much to their chagrin");
	Render.text(". As she's furtively polishing off her giant still, trying to get the dingy old thing to shine, you approach and clear your throat. She gives a sharp yelp and spins around, but seems to relax as she sees it's just you.");
	...
	Render.text("\n\n\"<i>Sure!  ");
	if(flags[FlagEnum.HELSPAWN_WEAPON] == "scimitar and shield") Render.text(championRef() + "'s been teaching me how to fight.");
	else Render.text("Mom's been teaching me how to fight.");
	Render.text("  I'm not as good as mom and " + championRef() + " yet, but I'd love to come along.</i>\"");
	
	Render.text("\n\n\"<i>That's the spirit.  Now we just have to convince your aunt Kiri to come along.  And ");
	if(flags[FlagEnum.HELSPAWN_DADDY] == 0) Render.text("your old man");
	else Render.text("[name]");
	Render.text(", too.</i>\"");
...
private goHuntingBitches():void {
	clearOutput();
	spriteSelect(68);
	Render.text("\"<i>Sure,</i>\" you say, quickly earning approving nods from Hakon and Helia.");
...
private noHuntingBitches():void {
	clearOutput();
	spriteSelect(68);
	Render.text("\"<i>I'll pass,</i>\" you say with a laugh, earning a shrug from the salamanders.");
