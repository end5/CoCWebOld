...
public function urtaBarDescript():boolean {
	...
	if (pregnancy.isPregnant) {
		if (pregnancy.type == PregnancyType.PLAYER) {
			...
			//Egg Level 1:
			if (flags[FlagEnum.URTA_EGGS] < 20) outputText("Urta is sitting quietly at her usual seat, sipping a mug of some non-alcoholic beer.  Though her clothes are still in place, you can just make out the bulging midriff stretching them taut, a consequence of all the eggs in her womb.");
			else if (flags[FlagEnum.URTA_EGGS] < 40) outputText("Urta is sitting rather uncomfortably at her usual seat.  Her swollen stomach is so large it keeps her from wearing her usual outfit, instead forcing her into a mini-shirt and a skirt bottom, allowing her midriff to be bare.  She occasionally runs a hand over the expanse, as if she can't believe it's actually there.");
			//Egg Level 3:
	...
	if(flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) {
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] == -1 || !urtaDrunk()) outputText("Urta is sitting at her usual table, sipping a glass of wine and wearing a form-fitting evening gown of shimmering black.  She looks up at you, happiness filling her eyes when she notices you entering the bar.", false);
		//(DRUNK)
	...
	if(flags[FlagEnum.TIMES_FUCKED_URTA] > 0) {
		...
		if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 11) outputText("Thankfully she still has her other addition trapped under her tail, for now.", false);
...
public function urtaBarApproach():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00145] == 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00143] > 0) {
		...
		else if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 12) outputText("anxiously", false);
	...
	if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11 && rand(2) == 0 ) {
		...
		if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 11) outputText("moments before you hear her horse-cock thump into the bottom of the table.  ", false);
...
private function drinkUrtasBoozePtTwo():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina() && player.totalCocks() == 0) outputText("squat down and ", false);
...
private function drinkUrtasBoozeLetHer():void {
	...
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText(biggestBreastSizeDescript(), false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText("tits", false);
	...
	if(player.lowerBody.cockSpot.count() > 0) outputText(" and " + multiCockDescript(), false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("wet", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("against your cervix as the tip presses firmly against it", false);
	else outputText("into your rectum", false);
	outputText(", unloading a thick batch of cream directly into your ", false);
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("waiting womb", false);
	...
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  You briefly wonder if she's capable of knocking you up, and what the baby would look like.", false);
	...
	if(player.totalCocks() > 0) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Your pussy dribbles and clenches, leaking Urta's whiteness as she claims it for her own.", false);
	...
	if(player.lowerBody.cockSpot.count() > 0) outputText("stick it in me", false);
...
private function dualUrtaMasturbation():void {
	...
	if(player.stats.cor < 60) outputText("You blush furiously at exposing yourself in such a way", false);
	...
	if(player.totalCocks() > 0) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript(0), false);
	}
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript(0), false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		if(player.vaginas[0].vaginalWetness > VAGINA_WETNESS.SLICK) outputText("A trickle of wetness escapes your nether-lips, starting to puddle underneath you.", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText(allBreastsDescript(), false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Delving deeply between your legs, you press forward into the source of your feminine pleasures, touching and rubbing, caressing your swollen button as you get closer and closer to orgasm.\n\n", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING) outputText(" and gushing everywhere", false);
		outputText(".  ", false);
	}
...
public function urtaFuckHer(afterBefriending:boolean = false):void {
	...
	if (player.totalCocks() == 0) {
		...
		player.slimeFeed();
		outputText("You come out of the shadows, shedding your " + player.armorName + ".  ", false);
		if (player.stats.cor < 50) outputText("You blush furiously at exposing yourself in such a way");
		...
		if(player.tailType == TAIL.HORSE || player.tailType == TAIL.DOG) outputText("swishing your tail across her taut nipples", false);
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("womb", false);
		...
		outputText("You come out of the shadows, shedding your " + player.armorName + ".  ", false);
		if (player.stats.cor < 60) outputText("You blush furiously at exposing yourself in such a way");
		else outputText("Smiling seductively; you revel in exposing yourself so suddenly");
		outputText(".  Urta looks up at you in shock, blinking tears from her eyes and stammering, \"<i>B-b-b-ut I have th-thish hideoush thing... Aren't you scared of it?</i>\"\n\n", false);
		...
		if(player.lowerBody.cockSpot.count() > 0) {
			if(player.lowerBody.cockSpot.count() > 2) outputText("a pair of your " + multiCockDescriptLight() + " against her pussy and asshole.  ", false);
			else if(player.lowerBody.cockSpot.count() == 2) outputText("your " + multiCockDescriptLight() + " against her pussy and asshole.  ", false);
			else if(player.lowerBody.cockSpot.count() == 1) outputText("your " + multiCockDescriptLight() + " against her pussy.  ", false);
		}
		//DOES IT FITZ
		if(player.cockArea(x) < urtaCapacity()) {
			...
			if(player.lowerBody.cockSpot.count() >= 2) outputText("Her backdoor was rougher going – the tightly muscled entrance held you back for a moment until Urta relaxed completely, but now the tighter hole is nearly as easy to penetrate as her pussy.  ", false);
			...
			if(player.lowerBody.cockSpot.count() == 1) outputText("hole", false);
...
internal function blowUrtaUnderTable():void {
	...
	if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11) outputText(" and pulling down her fishnet to release the monster trapped inside it.  It snaps up immediately, bobbing before your face as it fills with Urta's booze-fueled lust.", false);
	...
	if(player.stats.cor < 70) outputText("do your best to suppress your gag reflex and shove forwards, ramming her cock as deeply into your throat as you can.", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("dragging between your " + allBreastsDescript(), false);
...
internal function takeUrtaInTheButtPublically():void {
	...
	if(player.stats.cor < 60 && flags[FlagEnum.PC_FETISH] == 0) outputText("You can't believe you're doing it, but you undo the lower half of your " + player.armorName + ".  ", false);
	...
	if(flags[FlagEnum.TIMES_FUCKED_URTA] < 11) outputText("unwinds her tail from around the beast", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength < 2) outputText("peep out", false);
		else outputText("grow to its full size", false);
		outputText(".  ", false);
		if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLICK) outputText("Moisture beads on your lips, signaling just how turned on this total violation is making you.", false);
		else if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLAVERING) outputText("Moisture begins dripping onto Urta's thighs, signaling just how turned on this total violation is making you.", false);
	...
	if(player.upperBody.chest.hasFuckableNipples() && player.biggestLactation() >= 1) outputText(", leaking milky lubricants and aching to be penetrated.", false);
	else if(player.upperBody.chest.hasFuckableNipples()) outputText(", aching to be penetrated.", false);
	...
	else if(player.stats.cor < 50 && flags[FlagEnum.PC_FETISH] == 0) outputText("you find yourself whimpering, regretting the decision to climb on top of her, but aroused in spite of it.", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) {
		...
		if(player.upperBody.chest.hasFuckableNipples()) outputText("dripping lubricants", false);
...
internal function getAPublicFacialFromUrta():void {
	...
	if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 11) outputText("skirt ", false);
	...
	if(player.stats.cor < 33) outputText("and it makes you blush crimson", false);
	else if(player.stats.cor < 66) outputText("and it makes you uncomfortable but horny", false);
	...
	if(player.stats.cor < 75) outputText("doing your best to play your part in the scene.", false);
	...
	if(player.stats.cor < 50) outputText("Hesitantly, you extend your tongue and lick the underside of her flat-tip.  ", false);
	...
	if(player.stats.cor > 50) outputText("You hope the show will be arousing enough to get them off on you as well, and with renewed determination you turn back to Urta and give her balls a squeeze.", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("chest.  ", false);
	...
	if(player.stats.cor > 60) outputText(", being sure to lick your fingers clean afterwards", false);
	outputText(".\n\n", false);
	...
	if(player.stats.cor > 50) {
		...
		if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11) outputText("stuffs herself back into her fishnet", false);
...
internal function goBackToUrtasForLuvinz():void {
	...
	if(flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) {
		...
		if(flags[FlagEnum.URTA_PETPLAY_DONE] == 0 && urtaLove()) outputText("  <b>A new, shiny leather collar lies half-concealed under a cushion, as though its owner haphazardly tried to bury it in a rush.</b>");
	...
	if(player.stats.lib > 50 || player.stats.cor > 50) outputText(" and are actually turned on by the thought of walking through the city with such a 'big' lover hanging off your arm", false);
...
internal function goBackToUrtasForLuvinzII():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina() && player.totalCocks() > 0) outputText("hard and wet as you've ever been", false);
	else if(player.totalCocks() > 0) outputText("hard as you've ever been", false);
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("wet as you've ever been", false);
...
private function rideUrtaTenderFemale():void {
	...
	if(player.stats.cor < 50 || player.stats.lib < 50) outputText("sheepishly ", false);
	...
	if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.WET) outputText("growing moistness in your loins", false);
	else if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) outputText("sopping wetness in your loins", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) {
		...
		if(!player.upperBody.chest.hasFuckableNipples()) outputText("rubbing over her black-skinned buds.", false);
	...
	if(player.totalCocks() > 0) {
		if(player.lowerBody.cockSpot.count() == 1) {
			...
			if(player.lowerBody.cockSpot.list[0].cockLength > 15 && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) outputText("jiggling tits", false);
...
private function dudeFuckingUrtasCooch():void {
	...
	if(cocks > 1 && y >= 0) outputText("Your second " + Appearance.cockNoun(player.lowerBody.cockSpot.list[y].cockType) + " soon bumps against her pucker and strains against it, pushing ever-so-slowly into her tight hole as she gradually relaxes before you.  ", false);
	...
	if(player.lowerBody.balls > 0) outputText("Your balls loudly smack into her squelching-wet cunt and the soaked backside of HER balls", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("your own " + breastDescript(0), false);
...
private function tenderTakeItUpTheAssFromUrta():void {
	...
	if(cocks > 0) {
		...
		if(player.lowerBody.cockSpot.list[0].cockLength >= 15) {
			...
			if(player.lowerBody.cockSpot.list[0].cockLength >= 20 && player.lowerBody.cockSpot.list[0].cockLength <= 28) outputText(", even taking time to lick and suck on the head", false);
			outputText(".", false);
	...
	//(DICKS)
	if(cocks > 0) {
		if(player.lowerBody.cockSpot.list[0].cockLength < 15) outputText("Your orgasm erupts out onto her belly, splattering her fur with your sticky cum.  ", false);
		else if(player.lowerBody.cockSpot.list[0].cockLength < 20) outputText("Your orgasm erupts all over her tits, slicking them with your sticky white cum.  ", false);
...
private function marbleCockuBlockuUrta():void {
	...
	//if nursery
	if(flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] == 100) outputText(" and a nursery", false);
	outputText(".", false);
	//(Not wuv'ed yet)
...
private function TellUrtaDAHTRUUUUF():void {
	...
	if(flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) outputText("I love you for it", false);
	...
	if(!urtaOpenAboutEdryn()) {
		...
		if(flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) outputText("been together", false);
...
private function toughenUpUrtaAfterTellingTruuf():void {
	...
	if(flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) outputText("heartbroken", false);
...
private function lietoUrtaAboutMarble():void {
	...
	if(flags[FlagEnum.MARBLE_KIDS] == 1) outputText(" and saw the kid", false);
	if(flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] > 1) outputText(" and saw the kids", false);
...
public function blowUrtaUnderTheTableLuv():void {
	...
	if(player.stats.cor < 50) outputText("do your best to suppress your gag reflex and shove forwards, ramming her cock as deeply into your throat as you can.", false);
	...
	if(player.stats.cor < 50 && flags[FlagEnum.PC_FETISH] <= 0) outputText("wishing there was a tablecloth to hide your activities from prying eyes", false);
	...
	outputText("Thumping down on your back, you're blissfully aware of the sweet air filling your lungs.  In a moment of reflection, you realize you can taste the salty flavor of Urta's seed on your lips and mouth, and the retracting horse-cock is ", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("dragging between your " + allBreastsDescript(), false);
	else outputText("dragging across your chest", false);
...
private function rideUrtasCoochLoveyDovey():void {
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		...
		if(player.lowerBody.cockSpot.count() == 1) outputText("a latex sheath", false);
	...
	if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.WET) outputText("growing moist", false);
	else if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) outputText("growing wetter and wetter as you lubricate the tip of her condom", false);
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		if(player.lowerBody.cockSpot.count() == 1) outputText("Y", false);
...
private function rideUrtaInButtAtHomeLove():void {
	...
	if(player.upperBody.chest.hasFuckableNipples()) outputText("even slipping a finger inside their slippery depths, ", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) outputText("dampens", false);
	...
	//(EVERYBODY ELSE)
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			...
			if(player.vaginas[0].vaginalWetness > VAGINA_WETNESS.WET && player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLAVERING) outputText("  Fluids soak Urta's crotch with lust with feminine drool, though the fox barely seems to notice.", false);
			else if(player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING) outputText("  An explosion of feminine fluids spatters Urta's crotch and bed, completely soaking her groin in your need, but the distracted fox barely notices.", false);
		}
		if(player.totalCocks() > 0) {
...
private function oralFiestyUberExplosionUrta():void {
	...
	if(player.gender == 2 || (player.gender == 3 && rand(2) == 0)) {
		...
		if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) outputText("equally wet", false);
		...
		if(player.stats.cor <= 50) outputText("mouth", false);
		else outputText("throat", false);
		outputText(", clamping your " + player.legs() + " onto Urta's head as you orgasm.  ", false);
		if(player.vaginas[0].vaginalWetness > VAGINA_WETNESS.DROOLING) outputText("Fluids rush from your folds, splattering her face with sex fluid.  The fox opens wide, gulping it down like someone in a desert who has just found an oasis.", false);
		...
		if(player.stats.cor > 50) outputText("pours down your throat, bulging it into an obscene imprint of the horse-cock that's lodged inside.  Your eyes cross from pleasure and you slump down a bit, shoving your nose into Urta's sheath and allowing her to dump her sperm directly into your belly.  Gurgling noisily, your belly slowly expands outwards by about two inches, making you feel bloated and full.", false);
	...
	if(player.lowerBody.balls > 0) outputText(ballsDescript() + ".  Slithering over and around them, she bathes your sack in her spit before moving up to your " + cockDescript(x) + ".", false);
	...
	if(player.lowerBody.cockSpot.hasSheath()) outputText("sheath", false);
	...
	if(player.stats.cor < 60) outputText("in your mouth, forcing you to compulsively swallow if you don't want to drown in it.", false);
...
private function urtasCoochiNeedsFuckedLove():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("main ", false);
	...
	if(player.lowerBody.cockSpot.hasSheath()) outputText("sensitive skin of your sheath rubbing her exterior pussy-lips", false);
	...
	if(player.lowerBody.cockSpot.hasSheath()) outputText("sheath", false);
	else if(player.lowerBody.balls > 0) outputText("balls", false);
	...
	if(player.hasKnot(x)) {
		...
		else if(player.cumQ() < 1000) outputText("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your overproductive " + Appearance.cockNoun(CockType.HUMAN) + " stops spewing inside her.  ", false);
		else {
		...
		outputText("You finally feel your " + Appearance.cockNoun(CockType.HUMAN) + "'s knot shrinking and pull free with a loud and messy pop.  ", false);
	...
	//[HORSE]
		...
		if(player.lowerBody.cockSpot.list[x].cockLength >= 10) outputText("prods against Urta's cervix.", false);
		...
		else if(player.cumQ() < 1000) outputText("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your over-productive " + Appearance.cockNoun(CockType.HUMAN) + " has finished spewing your seed inside her.  ", false);
		else {
	...
	//[Everybody Else]
		...
		outputText("You finally feel your " + Appearance.cockNoun(CockType.HUMAN) + " deflating, and you pull free with a messy slurp.  ", false);
		if(player.cumQ() >= 2000) outputText("Spunk rushes out in a torrent, splattering everywhere.  ", false);
		outputText("Urta's glazed pussy-lips clench reflexively at their sudden emptiness and you hear a very satisfied sigh from the far end of the bed.\n\n", false);
	}
	//[RESUME]
...
public function scyllaAndUrtaSittingInATree():void {
	...
	else
	{
		...
		if(player.stats.cor < 40) outputText("corrupted ruffians", false);
		...
		else if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11) outputText("giving you a lewd look", false);
		...
		else if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11) outputText("Ohhh, you showed up a little late... mmmhmm yes, now lick... Oh, why didn't you tell me ho - OH YES!  RIGHT THERE! - h-how good of a cock-sucker this lewd-bodied bitch was?", false);
...
private function fuckUrtaWhileScyllaSucksSober():void {
	...
	if(player.lowerBody.balls > 0) outputText(" and [balls]");
	...
	if(player.lowerBody.balls > 0) outputText("the front of your [sack]");
	...
	if(player.lowerBody.cockSpot.hasSheath()) outputText("sheath");
	...
	if(player.lowerBody.balls > 0) outputText("r [balls]");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("  The unbound portion of your package bobs in the air before coming to rest in Urta's ass-crack.  A second later, the vulpine butt is coated with sticky white icing.");
...
private function dockWithUrtaUnderScyllasSoberSupervision():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("half's ");
	outputText("musk fills the air.");
	if(player.lowerBody.balls > 0) outputText("  Your [sack] swings free, bouncing off the back of [oneCock] when it thickens precipitously.");
	...
	if(player.lowerBody.balls > 0) outputText("  Your [sack] stretches tight, totally  filled by the burgeoning testes concealed within.");
	...
	if(player.lowerBody.balls > 0) outputText("  You shift to let your bloated ball-sack breathe a little better as well.");
	outputText("\n\nUrta has the sense to stop and step back in alarm.  ");
	if(flags[FlagEnum.URTA_SCYLLA_BIG_DICK_TIMES_DONE] > 0) outputText("Just like last time, she adopts a panicked expression and lunges for the door, but she moves slowly and a gooey drop of pre-cum hangs from her flare as she moves.");
	...
	if(player.lowerBody.balls > 0) outputText("  Your [balls] quiver a little when the rushing seed reaches them, and you swear you hear them slosh with the horse-spunk pouring into their own reservoirs.");
	...
	if(player.lowerBody.balls > 0) outputText(" while your enormous balls gurgle and begin to empty");
...
private function dockWithUrtaUnderScyllasSoberSupervision2():void {
	...
	if(player.lowerBody.balls > 0) outputText("  Your balls definitely got a little bigger...");
	doNext(camp.returnToCampUseOneHour);
}
...
private function watchTwoHotBitchesAndJerkIt():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" and exposing your " + vaginaDescript(0), false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" and touch your " + vaginaDescript(0), false);
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00143] == 1) outputText("\"<i>We need to do thish again!</i>\"", false);
...
private function makeUrtaSitOnYourLapWithScylla():void {
	...
	if(player.lowerBody == LOWER_BODY.NAGA) outputText(", snaking your tail around her legs", false);
	else if(player.lowerBody == LOWER_BODY.GOO) outputText(", enveloping her legs with your goo", false);
	...
	if((player.lowerBody.cockSpot.count() > 1 && y == -1) || player.lowerBody.cockSpot.count() > 2) outputText("You allow your remaining, unused dickflesh to flop against her back, leaking pre-cum into her fur.  ", false);
	...
	if(urtaLove() || player.stats.cor <= 40) outputText("kiss her full on the mouth", false);
	...
	if(y > -1) outputText("  Meanwhile, your other " + Appearance.cockNoun(player.lowerBody.cockSpot.list[y].cockType) + " utterly fills her womb, stuffing it so completely that the fox's belly rounds out.", false);
	if((player.lowerBody.cockSpot.count() > 1 && y == -1) || player.lowerBody.cockSpot.count() > 2) outputText("  A wave of the stuff splatters her back, glazing her from the shoulder-blades to the crack of her ass and nearly catching you in the chin. Maybe there is such a thing as too many cocks?", false);
	...
	else if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 11) outputText("a nervous smile on her face, but gives you a quick peck all the same.", false);
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00143] <= 1) outputText("pay for the door and ", false);
...
private function tellUrtaToBeADrunkenHussy():void {
	...
	if(player.stats.cor < 33) outputText("blush a little bit", false);
	else if(player.stats.cor < 66) outputText("smile nervously", false);
...
public function flirtWithUrta():void {
	...
	if (flags[FlagEnum.URTA_PC_LOVE_COUNTER] == -1) { //In friends mode
		...
		outputText("You tell her that in your world you never felt a deep urge to strip off all your clothes and " + (player.stats.cor < 75 ? "make sweet love to" : "slake your lust on") + " the nearest person.\n\n");
	}
	else {
...
private function urtaFertilityDiscussion():void {
	...
	if(flags[FlagEnum.URTA_FERTILE] == 1) outputText("  Should I stay fertile?");
	...
	if(flags[FlagEnum.URTA_PREG_EVERYBODY] == 1) outputText("  Or did you come to ask me not to be fertile and virile around anyone else?");
	else {
		outputText("  Would you mind me being able to put a bun in Edryn's oven?  Imagine all the lil foxtaurs running around!");
		if(flags[FlagEnum.EDRYN_NUMBER_OF_KIDS] > 0) outputText("  After all, you already had a turn!  Fair's fair!");
	}
...
private function urtaDiscussesEdrynRomance():void {
	...
	if(!urtaLove()) {
		if(!urtaOpenAboutEdryn()) {
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Until you, she was the only woman who didn't freak out and insist it was handjobs or nothing when she saw how big I was.  ");
		...
		//Subsequent Regular Variant:
			...
			if(player.stats.cor < 66) outputText("her manner is playful");
	...
	//Lover
		...
		if(!urtaOpenAboutEdryn()) {
			...
			if(player.statusAffects.get("Edryn").value1 >= 5) outputText("  \"<i>Though I have to admit I was still a little jealous when Edryn told me you were having sex with her too.  Still, I guess that's just another thing we have in common, yeah?  A mutual taste in sexy ladies.</i>\"  She laughs, and mockingly shapes her hands through the air as if caressing Edryn's sizable boobs.");
...
public function giveTheFoxSomeEggs():void {
	...
	if(flags[FlagEnum.PC_FETISH] > 0) outputText("as much fun as it might be, ");
	...
	if(player.perks.has("SpiderOvipositor")) outputText("green slime");
	...
	if(player.gender > 0) {
		outputText("  Your own ");
		if(player.lowerBody.cockSpot.hasCock()) outputText("[cock]");
		if(player.gender == 3) outputText(" and ");
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("[vagina]");
	...
	if(player.perks.has("BeeOvipositor")) outputText("honey");
...
public function urtaChewsOutPC(newScreen:boolean = true):void {
	...
	if(player.stats.cor > 50) outputText(", especially given this may be the closest she'll ever get to experiencing pregnancy");
	outputText("?");
	...
	if(player.perks.has("SpiderOvipositor")) outputText("spider");
...
private function pegUrtasAssMessy():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("a ");
	...
	else if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 11) outputText("softly begs, \"<i>Be gentle, please,</i>\" while spreading those fishnet-clad legs a little further apart.");
	...
	if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > 15) outputText("casually");
	else if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > 11) outputText("confidently");
	else if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > 4) outputText("a little nervously");
	...
	if(player.cockArea(x) > urtaCapacity()) outputText("The aching pressure around your " + cockDescript(x) + " doubles, and you realize she simply can't take any more.  Her anus is stretched wide, dilated to the extreme to accommodate this much of you.  There's little to do but plow her with what you can until she's fountaining white.");
		...
		if(!player.lowerBody.cockSpot.hasSheath()) outputText("groin ");
	...
	if(player.lowerBody.balls > 0) outputText(" while your [balls] slap against her fuzzy buttcheeks.");
	...
	if(player.hasKnot(x) && player.cockArea(x) < urtaCapacity()) outputText(" only makes your knot bigger and harder.");
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("  Good thing you have more.");
	}
	...
	if(flags[FlagEnum.URTA_QUEST_STATUS] < 0) outputText("not-so-fertile ");
	...
	if(player.stats.cor < 50) outputText("blushing");
...
private function UrtaGetsPeggedCleanUp():void {
	clearOutput();
	if(player.stats.cor >= 50) outputText("You start to duck out, but the disapproving glare the fox gives you makes it clear that you won't be getting any more messy fun unless you stick around to clean it up this time.  Damnit.  ");
...
private function fillMeUpPleaseUrta():void {
	...
	if(player.tongueType > TONUGE.HUMAN) outputText("prodigious ");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  Hard and erect, [eachCock] stands tall and proud, but ignored.  You have other plans.");
	...
	if(player.vaginas[0].virgin) {
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("  Drool begins to leak from [eachCock], slicking the underside with pre-cum as the pleasure affects your maleness.");
	...
	if(flags[FlagEnum.PC_FETISH] > 0) outputText("  An exhibitionist thrill runs through you at that, making you even wetter.");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  You don't even notice [eachCock] dripping on Urta's belly.");
	//HERMS:
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText(", followed in short order by the untouched portion of your " + multiCockDescriptLight());
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 4) outputText(" and your [chest] rest heavily upon your cum-stuffed form");
	outputText(".");
	...
	if(player.stats.cor < 33) outputText("blushing");
	else if(player.stats.cor < 66) outputText("grinning");
...
private function lippleBondUrtaScylla():void {
	...
	//[if (cocks > 1)
	if(player.lowerBody.cockSpot.count() > 1) outputText("  Without much warning, you suddenly feel Urta's hand move away from your own palm, the sensation of your [cock smallest] being wrapped by her fingers also assaulting you as she squeezes and tugs at your second cock affectionately.");
	//[if (cocks > 2)
	if(player.lowerBody.cockSpot.count() > 2) outputText("  The barrage of sensations is intensified when Scylla also gets a bit greedy, grabbing another tool and starting to jerk it off as well.");
	...
	//[if (cocks > 1)
	if(player.lowerBody.cockSpot.count() > 1) outputText(" either");
	outputText(" the holy woman's ravenous lipples and inner, erotic tit-flesh in a mutually ecstatic, sensations-filled manner");
	if(player.lowerBody.cockSpot.count() > 1) outputText(" or more delicately and less hungrily with a feminine hand");
	outputText(".");
	...
	//if (sensitivity < 29)
	if(player.stats.sens < 29) outputText("  You can only tell so from her expression and from Scylla moaning and gasping sharply, but it appears Urta's cock is really just about ready to shoot off cum.");
	//if (sensitivity > 29)
	...
	//[if (sensitivity > 44)
	if(player.stats.sens >= 44) outputText("  In fact, you can clearly feel the pressure inside Scylla's chest increasing as Urta's huge cock expands and twitches inside her next to your own, each such twitch sending a shiver of pleasure through both you and the nun.");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("as well as the hands of both of them working your cocks ");
...
private function nagaOnUrtaMess():void {
	...
	//[if (cocks = 0)
	if(player.lowerBody.cockSpot.count() == 0) outputText("  Urta runs her fingers over and gently pushes into your now revealed female slit. \"<i>My lover's sweet, sweet pussy. The best in the whole world.</i>\"");
	//if (cocks = 1)
	else if(player.lowerBody.cockSpot.count() == 1) outputText("  \"<i>And here is my favorite cock in the whole world... Probably including mine,</i>\" she giggles and winks at you.");
	//if (cocks = 2)
	else if(player.lowerBody.cockSpot.count() == 2) outputText("  \"<i>Two delicious cocks, rather than just one...  You're trying to tell me you're the man here, aren't you, lover?</i>\" she giggles and winks at you.");
	else if(player.lowerBody.cockSpot.count() == 3) outputText("  \"<i>You want to have a spare one just in case or something?</i>\" she giggles and winks at you.");
	//[if (cocks >= 4)]
...
private function lickOutUrtaAtHome():void {
	...
	if(player.stats.cor < 66) outputText("You're wonderful");
...
internal function urtaAndRaphaelSurprise():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" and [eachCock] experience a twinge of excitement");
	...
	if(player.stats.cor > 66) outputText("pompous, ");
...
private function urtasRuinedOrgasmsFromGooPartII():void {
	...
	if (edrynAvailable) outputText("recognize as Edryn's");
		...
		if(flags[FlagEnum.MET_OLIVIA] == 0) outputText("recognize as Olivia's");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("pussy");
...
private function wearZeCollar():void {
	...
	if(flags[FlagEnum.URTA_PETPLAY_DONE] == 0) {
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("harder");
		if(player.gender == 3) outputText(" and ");
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("wetter");
...
private function doggyStyle():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("grasping your [cock biggest]");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("[cockHead biggest]");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("Her thumb rubs against the sensitive underside of your cock");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("[cock biggest] does the same");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("She wraps her fingers around your shaft, slowly pumping up and down, taking a moment to tease your [cockHead biggest] every few strokes.");
	...
	if(player.upperBody.chest.countNipples() == 2) outputText("the ");
	else outputText("an ");
	outputText("other nipple, tweaking and teasing that one while she simultaneously ");
	if(player.lowerBody.cockSpot.hasCock()) outputText("jerks you off");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("pre-cum dribbles from your dick like a leaky faucet");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", and to top it all off, you find your [leg] inexplicably kicking in mid-air");
	outputText(".");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 5) outputText("resting on your folded arms");
	...
	if(player.lowerBody.balls > 0) outputText("own");
	else if(player.lowerBody.cockSpot.hasCock()) outputText("painfully erect member");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("dick");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("[cock] slaps against your belly roughly with every passing second, smearing thick globules of precum which drip down your belly, tickling your flesh like little balls of distilled pleasure");
	if(player.gender == 0) outputText(", while your");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" unused pussy aches to be filled or touched, its lips puffy and red, sensitive to every hard slap of your foxy lover's balls");
	outputText(".");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  It then travels along to your ");
	}
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("[vagina], which, despite being devoid of cock, clamps down anyway, gushing with femcum, drenching your Owner's balls and legs in sticky, clear fluids.");
...
private function getAPetReward():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("hard");
	if(player.gender == 3) outputText(" and ");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("wet");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("onto your chest");
	...
	if(player.upperBody.chest.countNipples() == 2) outputText("its twin");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("rapidly hardening shaft.");
	else outputText("moistening slit.");

	outputText("\n\nUrta keeps this up for a minute, lovingly stroking your ");
	if(player.lowerBody.cockSpot.hasCock()) outputText("dick");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("cunt lips");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("Your own cock remains almost painfully rock hard, bobbing awkwardly beneath you, dripping precum onto the floor and bowl.");
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		if(player.gender == 3) outputText("  Not to mention");
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Meanwhile");
	...
	if(player.tongueType == TONUGE.SNAKE) outputText("serpentine ");
	else if(player.tongueType == TONUGE.DEMONIC) outputText("demonic ");
	else if(player.tongueType == TONUGE.DRACONIC) outputText("draconic ");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("shaft ");
	if(player.gender == 3) outputText(" and ");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("slit ");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("jerking off your cock between two toes");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("The cock between your legs, meanwhile, stiffens, briefly becoming harder than you've ever felt before, before blasting the floor with jet after jet of silky white cum.");
...
private function urtaTakesPCOnWalkies():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("[eachCock] grows hard beneath you");
	if(player.gender == 3) outputText(" and ");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your [vagina] moistens, ready to be on display");
	outputText(".");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) outputText("the breasts hanging from your chest and ");
	outputText("your openly displayed rear end.  ");
	if(player.lowerBody.cockSpot.hasCock()) outputText("Pre-cum drizzles from the end of your painfully erect cock");
