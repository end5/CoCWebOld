...
//Urta variables
//flags[FlagEnum.TIMES_FUCKED_URTA] = times fucked
//flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] = urtas horsecock comfort level
	//-1 = PC pissed her off
	//0 = first timern
	//1-10 = slightly comfortable
	//11+ = flaunt it
//flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = hours turned off
	//0 = ready to rock!

//Urtas horsey bits =
	//LxW = 20x3
//Luv Lock - flags[FlagEnum.URTA_PC_LOVE_COUNTER]:
//0 = normal, 1 = love, -1 = never love, flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] -1 = hate
/*Luv rating! - flags[FlagEnum.URTA_PC_AFFECTION_COUNTER]
...
//const URTA_PETPLAY_DONE: number = 857;
		...
		public timeChange():boolean
		{
			...
			if (flags[FlagEnum.NEED_URTA_LETTER] == 1 && model.time.hours == 6) urtaPregs.getUrtaLetter(); //Urta Letters
			if (pregnancy.incubation == 0 && (pregnancy.type == PregnancyType.BEE_EGGS || PregnancyType.DRIDER_EGGS)) {
				...
				flags[FlagEnum.URTA_EGGS] = 0;
				flags[FlagEnum.URTA_FERTILE_EGGS] = 0;
			}
			if (flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] > 0) flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME]--; //Count down timer for urta's lust
			if (flags[FlagEnum.URTA_EGG_FORCE_EVENT] > 0) flags[FlagEnum.URTA_EGG_FORCE_EVENT]--; //Countdown to urta freakout
			//Urta egg freak out
			if (flags[FlagEnum.URTA_EGG_FORCE_EVENT] > 0 && model.time.hours > 6 && model.time.hours < 18 && flags[FlagEnum.URTA_EGG_FORCE_EVENT] < 12) {
			...
			if (flags[FlagEnum.URTA_ANGRY_AT_PC_COUNTDOWN] > 1) {
				flags[FlagEnum.URTA_ANGRY_AT_PC_COUNTDOWN]--;
				if (flags[FlagEnum.URTA_ANGRY_AT_PC_COUNTDOWN] < 1) flags[FlagEnum.URTA_ANGRY_AT_PC_COUNTDOWN] = 1;
			}
			if (model.time.hours > 23) {
				if (flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] == 0) flags[FlagEnum.URTA_CUM_NO_CUM_DAYS]++;
				else flags[FlagEnum.URTA_CUM_NO_CUM_DAYS] = 0;
				if (flags[FlagEnum.URTA_PC_AFFECTION_COUNTER] > 0) {
					flags[FlagEnum.URTA_PC_AFFECTION_COUNTER] -= .5;
					if (flags[FlagEnum.URTA_PC_AFFECTION_COUNTER] < 0) flags[FlagEnum.URTA_PC_AFFECTION_COUNTER] = 0;
				}
			}
			return needNext;
		}
...
public urtaCapacity():number {
	let bonus: number = flags[FlagEnum.TIMES_RUT_FUCKED_URTAS_CUNT] * 5;
	if(bonus > 40) bonus = 40;
	if(flags[FlagEnum.URTA_TENTACLE_GAPED] > 0) return 500;
	return 60 + bonus;
}

public urtaLove(love:number = 0):boolean {
	if(flags[FlagEnum.URTA_PC_LOVE_COUNTER] == -1) return false;
	if(love == 0) {
		return flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 1;
	}
	if(love > 0) {
		flags[FlagEnum.URTA_PC_AFFECTION_COUNTER] += love;
	}
	if(love < 0) {
		flags[FlagEnum.URTA_PC_AFFECTION_COUNTER] -= love;
	}
	if(flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) {
		//Queue up Amily madness
		//Only happens if freakout hasn't happened yet.
		if(love > 0 && flags[FlagEnum.AMILY_VISITING_URTA] == 0 && amilyScene.amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 1) flags[FlagEnum.AMILY_NEED_TO_FREAK_ABOUT_URTA] = 1;
		return true;
	}
	else return false;
}

public urtaAvailableForSex():boolean { return urtaFuckbuddy() && telAdre.scylla.action != Scylla.SCYLLA_ACTION_FUCKING_URTA && flags[FlagEnum.URTA_ANGRY_AT_PC_COUNTDOWN] == 0; }

public urtaFuckbuddy():boolean { //Returns true if Urta is either the player's fuckbuddy or lover
	if (kGAMECLASS.urtaQuest.urtaBusy() || flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] == -1) return false;
	if (urtaJustFriends()) return false;
	return (flags[FlagEnum.TIMES_FUCKED_URTA] > 0);
}

public urtaJustFriends():boolean { return flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] == 0 && flags[FlagEnum.URTA_PC_LOVE_COUNTER] == -1; }

public urtaAtBar():boolean { //Is Urta physically at the Wet Bitch?
	return (!kGAMECLASS.urtaQuest.urtaBusy() && flags[FlagEnum.AMILY_VISITING_URTA] != 1 && model.time.hours < 15 && flags[FlagEnum.KATHERINE_UNLOCKED] != 3);
}

public urtaDrunk():boolean {
	//Preg = no drinking!
	if (pregnancy.isPregnant) return false;
	if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] == -1) return false;
	if (model.time.hours > 12 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] < 1) return true;
	if (model.time.hours > 8 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] >= 1) return true;
	else return false;
}

private urtaOpenAboutEdryn():boolean {
	//Did it come out from the scylla threesome?
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00145] > 0) return true;
	//Did it come out from marble?
	if(flags[FlagEnum.URTA_KNOWS_PC_HAS_MARBLE_FOLLOWER] > 0) return true;
	//Did it come out from discussions?
	if(flags[FlagEnum.URTA_OPEN_ABOUT_EDRYN] > 0) return true;
	return false;
}
...
public knockUpUrtaChance():void { //Moved here from UrtaPregs since it needs to be accessed from outside Urta
	...
	//GTFO IF NOT FERTILE
	if (flags[FlagEnum.URTA_FERTILE] != 1) return;
	...
	if (chance > rand(100)) {
		...
		flags[FlagEnum.URTA_PREGNANT_DELIVERY_SCENE] = 0;
	}
}
...
public urtaBarDescript():boolean {
	...
	if(flags[FlagEnum.URTA_ANGRY_AT_PC_COUNTDOWN] == 1) {
		...
		flags[FlagEnum.URTA_ANGRY_AT_PC_COUNTDOWN] = 0;
		return true;
	}
	//Raphael betrayed reward
	if(flags[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] == -1 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00148] == 0) {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00350] == 1) {
	...
	//[URTA ASHAMED]
	if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] == -1) {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00145] == 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00143] > 0) {
	...
	if (pregnancy.isPregnant) {
		if (pregnancy.type == PregnancyType.PLAYER) {
			...
			//Egg Level 1:
			if (flags[FlagEnum.URTA_EGGS] < 20) MainScreen.text("Urta is sitting quietly at her usual seat, sipping a mug of some non-alcoholic beer.  Though her clothes are still in place, you can just make out the bulging midriff stretching them taut, a consequence of all the eggs in her womb.");
			else if (flags[FlagEnum.URTA_EGGS] < 40) MainScreen.text("Urta is sitting rather uncomfortably at her usual seat.  Her swollen stomach is so large it keeps her from wearing her usual outfit, instead forcing her into a mini-shirt and a skirt bottom, allowing her midriff to be bare.  She occasionally runs a hand over the expanse, as if she can't believe it's actually there.");
			//Egg Level 3:
	...
	if (flags[FlagEnum.URTA_TIMES_EGG_PREGGED] > 0 && !pregnancy.isPregnant && flags[FlagEnum.URTA_FLATBELLY_NOTICE] == 0) {
		...
		flags[FlagEnum.URTA_FLATBELLY_NOTICE] = 1;
		return true;
	}
	//[Love Urta Bar Appearance]
	if(flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) {
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] == -1 || !urtaDrunk()) MainScreen.text("Urta is sitting at her usual table, sipping a glass of wine and wearing a form-fitting evening gown of shimmering black.  She looks up at you, happiness filling her eyes when she notices you entering the bar.", false);
		//(DRUNK)
	...
	if(flags[FlagEnum.TIMES_FUCKED_URTA] == 0 && flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > -1) {
	...
	//[URTA FUCKBUDDIED]
	if(flags[FlagEnum.TIMES_FUCKED_URTA] > 0) {
		...
		if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 11) MainScreen.text("Thankfully she still has her other addition trapped under her tail, for now.", false);
...
public urtaBarApproach():void {
	...
	//Raphael Reward
	if(flags[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] == -1 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00148] == 0) {
		flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00148] = 1;
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00145] == 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00143] > 0) {
		flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00145] = 1;
		...
		else if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 12) MainScreen.text("anxiously", false);
	...
	//TO ZE FLIPOUT!
	if(flags[FlagEnum.URTA_EGG_FORCE_EVENT] > 0) {
		urtaChewsOutPC();
		return;
	}
	...
	//HERE WE GOEZ!
	if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 5 && ((player.inRut && player.lowerBody.cockSpot.hasCock()) || (player.inHeat && player.lowerBody.vaginaSpot.hasVagina() )))
	{
	...
	if(flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] > 0 && !urtaDrunk()) {
		MainScreen.text(images.showImage("urta-bar"), false);
		//Cockblock marble convo
		if (marbleScene.marbleAtCamp() && flags[FlagEnum.URTA_KNOWS_PC_HAS_MARBLE_FOLLOWER] == 0) {
			flags[FlagEnum.URTA_KNOWS_PC_HAS_MARBLE_FOLLOWER] = 1;
			marbleCockuBlockuUrta();
			return;
		}
		//Twu wuv talk
		if(flags[FlagEnum.URTA_PC_AFFECTION_COUNTER] >= 30 && flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 0) {
			UrtaTwuWuvOffer();
			return;
		}
	...
	if(flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 1)
	{
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] == -1 || !urtaDrunk()) {
			...
			if(flags[FlagEnum.URTA_CUM_NO_CUM_DAYS] >= 5) MainScreen.addButton(1,"Suck Off",slurpFawkesCocksForFunAndInflation);
		...
		//[Horny Urta Talk – Drunk]
			...
			if(flags[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] == -2 && RaphaelLikes() && flags[FlagEnum.URTA_X_RAPHAEL_HAPPENED] == 0) {
	...
	if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] == 0 && flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > -1) {
		MainScreen.text(images.showImage("urta-bar-drunk"), false);
	...
	if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11 && rand(2) == 0 ) {
		MainScreen.text(images.showImage("urta-bar"), false);
		...
		if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 11) MainScreen.text("moments before you hear her horse-cock thump into the bottom of the table.  ", false);
	...
	menu();
	if(flags[FlagEnum.URTA_CUM_NO_CUM_DAYS] >= 5) MainScreen.addButton(0,"Hidden BJ",slurpFawkesCocksForFunAndInflation);
...
private drinkUrtasBoozePtTwo():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina() && player.totalCocks() == 0) MainScreen.text("squat down and ", false);
	MainScreen.text("open up your " + player.armorName + ", releasing the pressure onto a patch of grass.  It takes forever to stop, but when it does you feel like a million bucks.  You turn and start to close up your kit when you realize you aren't alone.  Urta is watching you, her lips parted hungrily.  She's still peeing, but what's truly shocking is the mammoth package she's got hanging between her legs.  It's a massive horse-shaft, patterned black and pink, slung down from between a pair of apple-sized balls.  Before your eyes the equine beast begins hardening, forcing the distracted fox to empty the last of her urine against a building's wall.  She blushes furiously, but the drunken fox doesn't seem to care about her shame...\n\n", false);
...
private drinkUrtasBoozeRun(): void{
	...
	doNext(camp.returnToCampUseOneHour);
	flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] = -1;
}
private drinkUrtasBoozeLetHer():void {
	urtaSprite();
	player.slimeFeed();
	flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY]++;
	flags[FlagEnum.TIMES_FUCKED_URTA]++;
	flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 5;
	hideUpDown();
	player.orgasm();
	dynStats("sen", -2);
	//[LET HER]
	...
	else MainScreen.text(nippleDescript(0) + "s", false);
	...
	MainScreen.text(".  She pulls at your " + player.armorName + " until your crotch is completely exposed and vulnerable, revealing your " + player.assholeOrPussy(), false);
	if(player.lowerBody.cockSpot.count() > 0) MainScreen.text(" and " + multiCockDescript(), false);
	MainScreen.text(".  Urta snuggles close, brushing her nose along your thighs before plunging her tongue deep into your " + player.assholeOrPussy() + " in one powerful, drunken lick.  She slurps and licks, humming away until you're ", false);
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("wet", false);
	else if(player.totalCocks() > 0) MainScreen.text("suitably lubricated and leaking pre", false);
	...
	MainScreen.text("\"<i>Mmmm I shhoulda ushed a condom, you're gonna be a messssss!</i>\" she cries as she climbs atop her sexual peak, mounting it just like she's mounting your " + player.assholeOrPussy() + ".  You're spread wide, the equine member rammed tightly ", false);
	...
	if(player.totalCocks() > 0) {
		if(player.totalCocks() > 1) MainScreen.text("Each of y", false);
		else MainScreen.text("Y", false);
		MainScreen.text("our " + multiCockDescriptLight() + " squirts hard, unloading a batch of sticky jism under your ", false);
		...
		else MainScreen.text("Your " + assholeDescript() + " clamps down tightly, spasming in pleasure around the invader.", false);
	}
	if(player.totalCocks() > 0) MainScreen.text("  As you cum, more and more whiteness dribbles out until a puddle forms underneath you, showing Urta just how much you enjoyed the rough, drunken fuck.\n\n", false);
	else MainScreen.text("\n\n", false);
...
private dualUrtaMasturbation():void {
	...
	MainScreen.text("You come out of the shadows, shedding your " + player.armorName + ".  ", false);
	...
	if(player.totalCocks() > 0) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(vaginaDescript(0), false);
	}
	else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(vaginaDescript(0), false);
	else MainScreen.text("bare groin and " + assholeDescript(), false);
	MainScreen.text(".  ", false);
	if(player.totalCocks() > 0) {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		if(player.vaginas[0].vaginalWetness > VAGINA_WETNESS.SLICK) MainScreen.text("A trickle of wetness escapes your nether-lips, starting to puddle underneath you.", false);
		else MainScreen.text("Quickly engorging, your lower lips begin to part around your " + clitDescript() + ".", false);
		MainScreen.text("  ", false);
	}
	MainScreen.text("The heat coursing through you quickly shows itself on your ", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text(allBreastsDescript(), false);
	else MainScreen.text("chest", false);
	MainScreen.text(" as your " + nippleDescript(0) + "s become full and sensitive.\n\n", false);
	...
	if(player.totalCocks() == 1) MainScreen.text("Your hands caress your " + cockDescript(0) + " feverishly, tweaking every ounce of pleasure from the bloated rod.  ", false);
	if(player.totalCocks() > 1) MainScreen.text("Your hands caress your " + multiCockDescript() + " in turn, dancing back and forth from rod to rod as you attempt to play with every available pleasure outlet.  ", false);
	...
	if(player.gender == 0) MainScreen.text("Delving down between your legs, rubbing along your taint before you slowly penetrate your " + assholeDescript() + ", pleasuring yourself in the only way possible.\n\n", false);
	...
	if(player.totalCocks() > 0) {
		if(player.cumQ() < 250) {
			...
			MainScreen.text("your " + multiCockDescriptLight() + " empties itself.  ", false);
		}
			...
			if(player.totalCocks() > 1) MainScreen.text("each of ", false);
			MainScreen.text("your " + multiCockDescriptLight() + ", blasting into the air before splattering onto the ground between you and Urta.  It forms a massive puddle, spreading as you pump gob after gob of cum into it until the sticky spooge is soaking your " + player.legs() + " and Urta too.", false);
		}
		else {
			MainScreen.text("Jism fountains from ", false);
			if(player.totalCocks() > 1) MainScreen.text("each of ", false);
			MainScreen.text("your " + multiCockDescriptLight() + ", flooding the small alleyway with an unholy amount of cum.  In no time you've painted Urta and yourself from head to toe, and the alleyway sloshes with the stuff as your production overwhelms the ground's ability to absorb liquid.  ", false);
		}
	}
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		MainScreen.text("Your sex squishes wetly around a finger, clenching down tightly", false);
		if(player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING) MainScreen.text(" and gushing everywhere", false);
		MainScreen.text(".  ", false);
	}
...
public urtaFuckHer(afterBefriending:boolean = false):void {
	...
	if (player.totalCocks() == 0) {
		...
		if(player.tailType == TAIL.HORSE || player.tailType == TAIL.DOG) MainScreen.text("swishing your tail across her taut nipples", false);
		else MainScreen.text("making her wait", false);
		MainScreen.text(".  Eventually the blunted tip of Urta's horse-cock presses against your " + player.assholeOrPussy() + ".  Agonizingly, you force yourself to relax and lower yourself down on the mottled pink and black shaft.\n\n", false);
		...
		MainScreen.text("You smirk over your shoulder and start bouncing up and down on her, slapping her balls with your " + buttDescript() + " each time you bottom out on the thick shaft.  You can feel her ring of prepuce rubbing your inner walls, pressing on nerves normally ignored.  Urta grunts underneath you and you feel her flare growing wider inside you, stretching you further and further with every bounce atop the equine-endowed slut.  Suddenly warmth is flooding into your ", false);
		...
		if(player.lowerBody.cockSpot.count() > 0) {
			if(player.lowerBody.cockSpot.count() > 2) MainScreen.text("a pair of your " + multiCockDescriptLight() + " against her pussy and asshole.  ", false);
			else if(player.lowerBody.cockSpot.count() == 2) MainScreen.text("your " + multiCockDescriptLight() + " against her pussy and asshole.  ", false);
			else if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("your " + multiCockDescriptLight() + " against her pussy.  ", false);
		}
		//DOES IT FITZ
		if(player.cockArea(x) < urtaCapacity()) {
			...
			if(cocks > 1 && y >= 0) MainScreen.text(", " + cockDescript(x) + ", and " + cockDescript(y) + ".", false);
			else MainScreen.text(" and " + cockDescript(x) + ".", false);
			MainScreen.text("  Both her hands are locked tightly around her behemoth as it unloads onto the alley floor and walls.  The juicy tunnel clamps down on top of you, squeezing your " + cockDescript(x) + " as if Urta's life depended on it.  It's as if every ounce of her being was focusing on clamping down on the invader, massaging, squeezing, and milking it, while the rest of her body goes entirely limp.", false);
			if(cocks > 1 && y >= 0) MainScreen.text("  Your " + cockDescript(y) + " feels like it's about to explode inside her asshole when her sphincter clamps down tightly, acting like the world's tightest cock-ring.", false);
			MainScreen.text("\n\n", false);
		...
		//(TOO BIG)
			...
			if(cocks == 1) {
				...
				MainScreen.text("Urta pants out, \"<i>Ah ahhh ahhhh,</i>\" and cums, her flare ballooning out until it's twice as wide as her shaft, a wave of white jism escaping from her horse-cock.  You gather some in your hand and slather it over your own " + multiCockDescript() + ", and after a few moments of fevered masturbation, you start cumming with her.  Urta happily squirms against you, painting herself with jism and soaking her clothes with the stuff.  ", false);
			...
			if(cocks > 1) {
				...
				MainScreen.text("Urta pants out, \"<i>Ah ahhh ahhhh,</i>\" and cums, her flare ballooning out until it's twice as wide as her shaft, a wave of white jism escaping from her horse-cock.  You gather some in your hand and slather it over your own " + multiCockDescriptLight() + ", and after a few moments of fevered masturbation, you start cumming with her.  Urta happily squirms against you, painting herself with jism and soaking her clothes with the stuff.  ", false);
				if(player.cumQ() < 250) MainScreen.text("You join in, though your own contribution seems meager by comparison.", false);
				else if(player.cumQ() < 500) MainScreen.text("You join in, and manage to match her spurt for spurt, until she's lying in a thick puddle of the stuff.", false);
				else MainScreen.text("You join in, and make her orgasm look like a trickle compared to the flood of spunk you unleash onto her.  By the time the pair of you finish, she's soaked from head to toe, and lying in a nicely sized puddle of the stuff.", false);
...
internal function blowUrtaUnderTable():void {
	...
	flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY]++;
	urtaLove(1);
	flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 2 + rand(4);
	MainScreen.text("", true);
	...
	if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11) MainScreen.text(" and pulling down her fishnet to release the monster trapped inside it.  It snaps up immediately, bobbing before your face as it fills with Urta's booze-fueled lust.", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) MainScreen.text("dragging between your " + allBreastsDescript(), false);
	else MainScreen.text("dragging across your chest", false);
	MainScreen.text(" as it begins to soften.  You wipe as much of her leavings from your " + player.face() + " as you can, licking the slippery glaze from your lips.  Before you get up, you make sure to give Urta's member a firm squeeze, punishment for forcing you to take her so deep.\n\n", false);
...
internal function takeUrtaInTheButtPublically():void {
	...
	if(player.stats.cor < 30 && flags[FlagEnum.PC_FETISH] == 0) {
	...
	if(player.stats.cor < 60 && flags[FlagEnum.PC_FETISH] == 0) MainScreen.text("You can't believe you're doing it, but you undo the lower half of your " + player.armorName + ".  ", false);
	else MainScreen.text("You happily remove the lower half of your " + player.armorName + ", more than a little excited at the prospect of putting on a show for the crowd.  ", false);
	MainScreen.text("Urta casually flips up her skirt and ", false);
	if(flags[FlagEnum.TIMES_FUCKED_URTA] < 11) MainScreen.text("unwinds her tail from around the beast", false);
	...
	MainScreen.text("You carefully shift your position, climbing in front of Urta.  With care, you grab the sensitive member just below the flare and begin rubbing it over your " + buttDescript() + " and ready ass-hole, lubing yourself up and trying to relax.  The impatient fox grabs you by your " + hipDescript() + " and pulls you down, forcing herself against your pucker.  ", false);
	...
	if(player.totalCocks() > 0) {
		...
		if(player.totalCocks() > 1) MainScreen.text("each of ", false);
		MainScreen.text("your " + multiCockDescriptLight() + " from swelling to full erectness.  A bead of pre forms at ", false);
		if(player.totalCocks() > 1) MainScreen.text("each ", false);
		else MainScreen.text("the ", false);
		MainScreen.text("tip, and begins leaking down the shaft", false);
		if(player.totalCocks() > 1) MainScreen.text("s", false);
		MainScreen.text(" of your " + multiCockDescriptLight() + ", squeezed out from your prostate.  ", false);
	}
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		MainScreen.text("Heat spreads through your loins as your " + vaginaDescript(0) + " engorges with blood, allowing your " + clitDescript() + " to ", false);
		if(player.lowerBody.vaginaSpot.list[0].clitLength < 2) MainScreen.text("peep out", false);
		else MainScreen.text("grow to its full size", false);
		MainScreen.text(".  ", false);
		if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLICK) MainScreen.text("Moisture beads on your lips, signaling just how turned on this total violation is making you.", false);
		else if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLAVERING) MainScreen.text("Moisture begins dripping onto Urta's thighs, signaling just how turned on this total violation is making you.", false);
	...
	else if(player.stats.cor < 50 && flags[FlagEnum.PC_FETISH] == 0) MainScreen.text("you find yourself whimpering, regretting the decision to climb on top of her, but aroused in spite of it.", false);
	else MainScreen.text("you find yourself sliding your hands under your " + player.armorName + " and pinching your " + nippleDescript(0) + "s, putting on a show for the crowd.", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) {
		MainScreen.text("  Your " + allBreastsDescript() + " are jiggling and bouncing wildly, ", false);
		if(player.upperBody.chest.hasFuckableNipples()) MainScreen.text("dripping lubricants", false);
		else if(player.biggestLactation() > 1) MainScreen.text("spurting milk", false);
		else MainScreen.text("rubbing your tight nipples against your " + player.armorName, false);
		MainScreen.text(".", false);
	}
	if(player.lowerBody.cockSpot.count() > 0) {
		if(player.lowerBody.cockSpot.count() > 1) {
			MainScreen.text("  Steady drizzles of cum begin dripping from each of your " + multiCockDescriptLight() + " as she pounds away, rubbing hard against your prostate.", false);
		}
		else {
			MainScreen.text("  Steady drizzles of cum begin dripping from your " + multiCockDescriptLight() + " as she pounds away, rubbing hard against your prostate.", false);
		}
	}
...
internal function getAPublicFacialFromUrta():void {
	...
	flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY]++;
	urtaLove(2);
	flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 3 + rand(3);
	MainScreen.text("", true);
	...
	if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 11) MainScreen.text("skirt ", false);
	...
	if(player.stats.cor > 50) {
		...
		if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11) MainScreen.text("stuffs herself back into her fishnet", false);
...
private optionalUrtaBukkake():void {
	urtaSprite();
	player.slimeFeed();
	flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME]-=2;
	urtaLove(-1);
	MainScreen.text("", true);
	MainScreen.text(images.showImage("urta-bukkake"), false);
	MainScreen.text("You pause to tear off your " + player.armorName + " and expose the rest of your body, and you reach out into the crowd, grabbing the nearest cock and caressing it as you beg, \"<i>Come on and cum, she missed most of me!</i>\"\n\n", false);
	...
	MainScreen.text("An hour later you're totally drenched in cum.  It's dripping off you from your hair down to your " + player.feet() + ", and you love it.  You lick up the mixed spooge until you start to feel full, and a blushing barmaid comes forward to hand you a towel.  You give her a salty kiss on the lips, sending her running away before you towel off and dress yourself.", false);
	doNext(camp.returnToCampUseOneHour);
}
...
internal function goBackToUrtasForLuvinz():void {
	...
	if(flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) {
		...
		if(flags[FlagEnum.URTA_PETPLAY_DONE] == 0 && urtaLove()) MainScreen.text("  <b>A new, shiny leather collar lies half-concealed under a cushion, as though its owner haphazardly tried to bury it in a rush.</b>");
		...
		if(player.totalCocks() > 0) MainScreen.text("In her hand is another sheath, ready to gird your own equipment in a fashion similar to her own.  ", false);
...
internal function goBackToUrtasForLuvinzII():void {
	...
	if(flags[FlagEnum.PC_SEEN_URTA_SEX_TOYS] == 0) {
	...
	if(player.lowerBody.vaginaSpot.hasVagina() && player.totalCocks() > 0) MainScreen.text("hard and wet as you've ever been", false);
	else if(player.totalCocks() > 0) MainScreen.text("hard as you've ever been", false);
	...
	if(player.isGoo() && player.skinType == SKIN.GOO) MainScreen.addButton(4,"Goo (Weird)",urtaGooTesticleVoreRuinedOrgasms);
}

private rideUrtaTenderFemale():void {
	urtaSprite();
	flags[FlagEnum.PC_SEEN_URTA_SEX_TOYS]++;
	urtaLove(1.5);
	flags[FlagEnum.TIMES_FUCKED_URTA]++;
	flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY]++;
	flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 3 + rand(2);
	MainScreen.text("", true);
	...
	if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.WET) MainScreen.text("growing moistness in your loins", false);
	else if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) MainScreen.text("sopping wetness in your loins", false);
	...
	if(player.totalCocks() > 0) {
		if(player.totalCocks() == 1) MainScreen.text("  Your " + multiCockDescriptLight() + " is raging hard and trembling, but ignored for now.", false);
		else MainScreen.text("  Each of your " + multiCockDescriptLight() + " is raging hard and trembling, but ignored for now.", false);
	}
	MainScreen.text("  Climbing atop the bed with her, you swivel your " + hipDescript() + " about, positioning her at your entrance and straddling your " + player.legs() + " around her.\n\n", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) {
		...
		if(player.totalCocks() > 0) {
			if(player.lowerBody.cockSpot.list[0].cockLength > 15) {
				if(player.totalCocks() == 1) MainScreen.text("  Your " + multiCockDescriptLight() + " pushes up between the jiggling breasts, slicking them with your pre-cum as it begins to get tit-fucked by both of your trembling forms.", false);
				else MainScreen.text("  Each of your " + multiCockDescriptLight() + " pushes up between the jiggling breasts, slicking them with your pre-cum as they begin to get tit-fucked by both of your trembling forms.", false);
			}
		}
		MainScreen.text("\n\n", false);
	}

	MainScreen.text("You both cry out in orgiastic pleasure as your shaking " + hipDescript() + " force your tightly-stretched cunt over Urta's rubber-wrapped horse-prick again and again.  Your internal muscles spasm, clamping down tightly in sudden orgasm.  Urta's voice trembles in a low moan of satisfaction while her lower body twitches upwards with hard pumping motions.  Her dick trembles inside you, and you feel the latex around her girthy prick inflating with her seed and pressing against your cervix.\n\n", false);
	if(player.totalCocks() > 0) {
		if(player.lowerBody.cockSpot.count() == 1) {
			MainScreen.text("Your own " + multiCockDescriptLight() + " cums as well, blasting seed over both your ", false);
			if(player.lowerBody.cockSpot.list[0].cockLength > 15 && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) MainScreen.text("jiggling tits", false);
			else MainScreen.text("bellies", false);
			MainScreen.text(", glazing them in slippery goo.", false);
		}
		else {
			MainScreen.text("Each of your own " + multiCockDescriptLight() + " cums as well, blasting seed over both your ", false);
			if(player.lowerBody.cockSpot.list[0].cockLength > 15 && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) MainScreen.text("jiggling tits", false);
			else MainScreen.text("bellies", false);
			MainScreen.text(", glazing them in slippery goo.", false);
		}
	...
	if(player.totalCocks() > 0) {
		MainScreen.text(" and ignoring the hot, wet mess your " + multiCockDescriptLight() + " ", false);
		if(player.totalCocks() == 1) MainScreen.text("has", false);
	...
	if(flags[FlagEnum.TIMES_FUCKED_URTA] <= 10 || flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 11)
	...
	//if(flags[FlagEnum.URTA_FERTILE] == 1) player.knockUp(21,515,80);
	player.orgasm();
	dynStats("sen", -2);
	doNext(camp.returnToCampUseOneHour);
}
...
private dudeFuckingUrtasCooch():void {
	...
	urtaLove(1.5);
	flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY]--;
	if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 1) flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] = 1;
	flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 2;

	MainScreen.text("You let her know that you want her just the way she is and climb into bed behind her.  She watches you apprehensively over her shoulder as you pull out your " + multiCockDescriptLight() + " and start lining yourself up to fuck her doggie-style.  ", false);
	if(cocks > 1 && y >= 0) MainScreen.text("Your secondary " + cockDescript(y) + " wobbles directly behind her back-door and Urta's eyes widen a bit when she realizes you're going to double-penetrate her.  ", false);
	if(cocks > 2) MainScreen.text("The remainder of your " + multiCockDescriptLight() + " fan out, ready to rub themselves against her butt-cheeks once you push inside.  ", false);
	...
	MainScreen.text("Obligingly, you ease your hips forward, reeling at the heat of her cunt-lips as they welcome your " + cockDescript(0) + "'s " + player.cockHead() + " with slippery moisture.  ", false);
	if(cocks > 1 && y >= 0) MainScreen.text("Your second " + Appearance.cockNoun(player.lowerBody.cockSpot.list[y].cockType) + " soon bumps against her pucker and strains against it, pushing ever-so-slowly into her tight hole as she gradually relaxes before you.  ", false);
	...
	MainScreen.text("Urta cums first.  Her cries of pleasure are muffled as she shoves her face into a pillow, screaming in ecstasy.  The warmth around your " + cockDescript(x) + " seems to grow that much hotter as it squeezes you, wringing you tightly from root to tip.  ", false);
	if(cocks > 1 && y >= 0) MainScreen.text("The intensity and pressure in her back-door is even worse, and your " + cockDescript(y) + " is nearly in pain from her asshole's tight, pulsating grip.  ", false);
	...
	if(cocks > 1 && y >= 0) MainScreen.text("The " + cockDescript(y) + " you've crammed in her ass does not disappoint either, matching its brother squirt for squirt as it spends your seed with reckless abandon.  ", false);
	if(cocks > 2) MainScreen.text("Her butt and tail are covered with the stuff as each of your remaining " + multiCockDescriptLight() + " splatters her with spooge, covering her fur in white sludge.  ", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text("your own " + breastDescript(0), false);
...
private fuckItAndLeave():void {
	...
	flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY]--;
	if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 1) flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] = 1;
	flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 36;
	urtaLove(-5);
	MainScreen.text("", true);
...
private tenderTakeItUpTheAssFromUrta():void {
	...
	flags[FlagEnum.TIMES_FUCKED_URTA]++;
	flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY]++;
	flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 2 + rand(3);
	let cocks:number = player.totalCocks();
	...
	MainScreen.text("Her cheeks color when you give her a nod and begin lowering your " + buttDescript() + " towards her pillar of horse-flesh. She gasps quietly when the tip begins to push against your " + assholeDescript() + ".  ", false);
	if(player.analCapacity() >= 60) MainScreen.text("You sigh and begin sliding down it, your previously-stretched backdoor easily accepting the gigantic intruder as it fills you.", false);
		...
		if(player.ass.analLooseness < 5) {
			if(player.ass.analLooseness == 0) MainScreen.text("<b>  Urta has taken your anal virginity.</b>", false);
			else MainScreen.text("<b>  Your ass is stretched.</b>", false);
			player.ass.analLooseness++;
		}
	}
	//Butt Stretched used to determine how long since last enlargement
	if(player.findStatusAffect(StatusAffects.ButtStretched) < 0) player.statusAffects.add(new StatusAffect("ButtStretched",0,0,0,0)));
	...
	if(player.totalCocks() > 0) {
		...
		MainScreen.text("your " + multiCockDescriptLight() + " hardens near instantly from the forced stimulation.", false);
	}
	...
	if(cocks > 0) {
		...
		if(cocks > 1) MainScreen.text("each of ", false);
		MainScreen.text("your " + multiCockDescriptLight() + " droops onto her belly, half-hard from the pressure on your prostate, but going a little soft from the lack of attention.", false);
		if(player.lowerBody.cockSpot.list[0].cockLength >= 15) {
			MainScreen.text("  Urta winks and guides ", false);
			if(cocks > 1) MainScreen.text("one of ", false);
			MainScreen.text("your " + multiCockDescriptLight() + " between her tits", false);
			if(player.lowerBody.cockSpot.list[0].cockLength >= 20 && player.lowerBody.cockSpot.list[0].cockLength <= 28) MainScreen.text(", even taking time to lick and suck on the head", false);
			MainScreen.text(".", false);
	...
	MainScreen.text("You bounce on top of your fox-lover's equine rod, spearing your " + assholeDescript() + " on it over and over.  ", false);
	...
	//(DICKS)
	if(cocks > 0) {
		if(player.lowerBody.cockSpot.list[0].cockLength < 15) MainScreen.text("Your orgasm erupts out onto her belly, splattering her fur with your sticky cum.  ", false);
		else if(player.lowerBody.cockSpot.list[0].cockLength < 20) MainScreen.text("Your orgasm erupts all over her tits, slicking them with your sticky white cum.  ", false);
...
private marbleCockuBlockuUrta():void {
	...
	//if nursery
	if(flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] == 100) MainScreen.text(" and a nursery", false);
	MainScreen.text(".", false);
	//(Not wuv'ed yet)
	if(flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 0) {
...
private TellUrtaDAHTRUUUUF():void {
	...
	if(flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) MainScreen.text("I love you for it", false);
	...
	if(!urtaOpenAboutEdryn()) {
		...
		if(flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) MainScreen.text("been together", false);
...
private comfortUrtaAfterTellinTruuf():void {
	...
	if(((player.lowerBody == LOWER_BODY.CENTAUR && player.totalCocks() > 0) ||
		player.horseCocks() > 0 ||
		(player.totalCocks() > 0 && player.stats.cor > 50)) &&
...
private toughenUpUrtaAfterTellingTruuf():void {
	...
	if(flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) MainScreen.text("heartbroken", false);
	...
	if(flags[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) {
		...
		flags[FlagEnum.URTA_PC_AFFECTION_COUNTER] = 0;
		flags[FlagEnum.URTA_PC_LOVE_COUNTER] = -1;
	}
	else {
...
private lietoUrtaAboutMarble():void {
	...
	if(flags[FlagEnum.MARBLE_KIDS] == 1) MainScreen.text(" and saw the kid", false);
	if(flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] > 1) MainScreen.text(" and saw the kids", false);
	...
	flags[FlagEnum.URTA_PC_LOVE_COUNTER] = -1;
	flags[FlagEnum.URTA_PC_AFFECTION_COUNTER] = 0;
	flags[FlagEnum.URTA_ANGRY_AT_PC_COUNTDOWN] = 48;
	doNext(camp.returnToCampUseOneHour);
}
...
private stayFuckbuddiesAfterShootingDown():void {
	...
	flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 0;
	doNext(camp.returnToCampUseOneHour);
}
//[no]
...
public eatUrtaOutNomNomPussy():void {
	urtaSprite();
	flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 3;
	urtaLove(1);
	MainScreen.text("", true);
	...
	MainScreen.text("Finished at last, Urta's member rapidly deflates, the scent of her cum on your " + hairDescript() + " and her juice on your " + player.face() + " lingers.  You slide up into the booth next to your blissed out lover.", false);
...
private giveUrtaCumGlass():void {
	urtaSprite();
	flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 1;
	urtaLove(-.5);
	flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY]++;
	MainScreen.text("", true);
...
public blowUrtaUnderTheTableLuv():void {
	...
	if(player.stats.cor < 50 && flags[FlagEnum.PC_FETISH] <= 0) MainScreen.text("wishing there was a tablecloth to hide your activities from prying eyes", false);
	...
	MainScreen.text("Thumping down on your back, you're blissfully aware of the sweet air filling your lungs.  In a moment of reflection, you realize you can taste the salty flavor of Urta's seed on your lips and mouth, and the retracting horse-cock is ", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text("dragging between your " + allBreastsDescript(), false);
	else MainScreen.text("dragging across your chest", false);
	MainScreen.text(" as it begins to soften.  You wipe as much of her leavings from your " + player.face() + " as you can, licking the slippery glaze from your lips.  Before you get up, you make sure to give Urta's member a firm squeeze, scolding it for its forceful treatment of your mouth.\n\n", false);
...
private rideUrtasCoochLoveyDovey():void {
	urtaSprite();
	flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 2 + rand(2);
	urtaLove(1);
	MainScreen.text("", true);
	//(TOO SMALL)
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		...
		MainScreen.text(" to cover your " + multiCockDescriptLight() + " with, explaining that she doesn't want to have to mop up afterward, and you grudgingly agree.  ", false);
	}
	...
	if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.WET) MainScreen.text("growing moist", false);
	else if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) MainScreen.text("growing wetter and wetter as you lubricate the tip of her condom", false);
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		...
		MainScreen.text("our " + multiCockDescriptLight() + " spurts, unloading waves of cream into the ", false);
		if(player.lowerBody.cockSpot.count() == 1) {
	...
	//if(flags[FlagEnum.URTA_FERTILE] == 1) player.knockUp(21,515,80);
	player.orgasm();
	if(player.stats.sens > 30) dynStats("sen", -1);
}
...
private rideUrtaInButtAtHomeLove():void {
	...
	if(player.totalCocks() > 0) {
		...
		if(player.totalCocks() == 1) MainScreen.text("a condom", false);
	...
	if(player.totalCocks() > 0) {
		...
		MainScreen.text("our " + multiCockDescriptLight() + " has filled its sheath with a golfball sized globule of pre-cum, and you can feel your body clenching and squeezing, your ", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		MainScreen.text("  Neglected and unused, your " + vaginaDescript(0) + " ", false);
		if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) MainScreen.text("dampens", false);
	...
	MainScreen.text("The lovemaking increases in tempo, slowly working its way to a feverish crescendo.  You're bouncing atop the fox wildly, and your lover cums hard.  As full as you felt with her inside you, it's nothing compared to being stuffed with her cock AND cum.  Each cum-filled contraction swells her member from base to flare, allowing you to feel it stretch your insides while it travels up your " + assholeDescript() + ".  It dumps the load into the bubble of cum inside you, filling you with a creamy jizz balloon.\n\n", false);

	//(GENDERLESS ORGAZM)
	if(player.gender == 0) {
		MainScreen.text("You cry out with pleasure, feeling the closest thing you can to an orgasm.  Your muscles clench around the invading rod, milking it while the rest of your body trembles.  The strength drains from your " + player.legs() + " and arms and you twitch there, completely impaled on your lover's thick cock, happily and completely penetrated.\n\n", false);
	}
	//(EVERYBODY ELSE)
	else {
		MainScreen.text("You cry out with pleasure, feeling your body cum from the invading warmth in your " + assholeDescript() + ".  Shivering as the strength drains from your arms and " + player.legs() + ", you let the intruding and still cumming cock support your body.", false);
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("  Your " + vaginaDescript(0) + " clenches desperately, peaking without a single touch, but hungry for something to milk.", false);
			if(player.vaginas[0].vaginalWetness > VAGINA_WETNESS.WET && player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLAVERING) MainScreen.text("  Fluids soak Urta's crotch with lust with feminine drool, though the fox barely seems to notice.", false);
			else if(player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING) MainScreen.text("  An explosion of feminine fluids spatters Urta's crotch and bed, completely soaking her groin in your need, but the distracted fox barely notices.", false);
		}
		if(player.totalCocks() > 0) {
			...
			if(player.cumQ() >= 750 && player.cumQ() < 1250) MainScreen.text("  By the time you're done the balloon is the size of a basketball and looking dangerously close to bursting.", false);
				...
				if(player.cumQ() >= 1700) MainScreen.text("  You fill it fuller and fuller, until it's bigger than you thought possible.  All at once the material gives out, soaking Urta from the tits down in cum, while your " + cockDescript(0) + " continues to pump even more onto her.  So much for not making a mess!", false);
			}
		}
		MainScreen.text("\n\n", false);
	}
...
private oralFiestyUberExplosionUrta():void {
	...
	if(player.cockArea(x) >= 100) {
		MainScreen.text("Urta takes one look at your " + cockDescript(x) + " and says, \"<i>I'm sorry babe, but there's no way I could take that monster in my mouth.  How about we do something else?</i>\"\n\n", false);
		doNext(urtaHomeLuvLuvinsMenu);
		return;
	}
	...
	if(player.gender == 2 || (player.gender == 3 && rand(2) == 0)) {
		...
		MainScreen.text("The musky scent of Urta's forearm-sized horse-prick wafts into your nostrils as it wobbles just below your " + player.face() + ", visibly twitching and throbbing from all the extra blood.  She whines plaintively, nuzzling her moist nose against your ", false);
		if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) MainScreen.text("equally wet", false);
		...
		MainScreen.text("Not far from cumming yourself, you grind your " + hipDescript() + " on Urta's face, smearing her fur with your fragrant pussy-juices.  Her own moans of pleasure turn into noisy wet blubbering that vibrates through your " + vaginaDescript(0) + ", getting you off on the spot.  You squeal around the cock in your ", false);
		if(player.stats.cor <= 50) MainScreen.text("mouth", false);
		else MainScreen.text("throat", false);
		MainScreen.text(", clamping your " + player.legs() + " onto Urta's head as you orgasm.  ", false);
		if(player.vaginas[0].vaginalWetness > VAGINA_WETNESS.DROOLING) MainScreen.text("Fluids rush from your folds, splattering her face with sex fluid.  The fox opens wide, gulping it down like someone in a desert who has just found an oasis.", false);
		else MainScreen.text("Wetness slowly coats her muzzle as your canal squeezes and wriggles with involuntary pleasure-spasms.  Urta never lets up, sucking and licking your " + clitDescript() + " in a rhythm that matches your own orgasmic contractions.", false);
		if(player.totalCocks() > 0) {
			MainScreen.text("  ", false);
			if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("Each of y", false);
			else MainScreen.text("Y", false);
			MainScreen.text("our " + multiCockDescriptLight() + " drools lamely, ", false);
		...
		MainScreen.text("The fox giggles and returns to cuddling and licking you.  You rest with a happy smile plastered on your " + player.face() + " from the experience.  After all the snuggling, you both rise and get dressed.  Urta finds a new dress in the closet and with a long goodbye kiss, you go your separate ways.\n\n", false);
		doNext(camp.returnToCampUseOneHour);
		player.orgasm();
		dynStats("sen", -1);
		return;
	...
	//(DUDEZILLAZ)
	MainScreen.text("Urta sits up and practically screams, \"<i>Really,</i>\" and squeals excitedly.  She claps and lies back as you climb into the bed, stopping only to yank off her cock-sheath and toss it into a nearby waste-bin.  You swing your " + player.legs() + " over her and get into position, aligning ", false);
	if(player.lowerBody.balls > 0) {
		MainScreen.text("your " + ballsDescriptLight() + " and ", false);
	}
	if(player.totalCocks() > 1) MainScreen.text("one of your ", false);
	else MainScreen.text("your ", false);
	MainScreen.text(multiCockDescriptLight() + " with Urta's muzzle.  Her rather sizable 'addition' wobbles underneath you, blocking your vision as it wobbles back and forth under you.\n\n", false);

	MainScreen.text("Your foxy lover opens wide and slips her smooth, wide tongue all over your ", false);
	if(player.lowerBody.balls > 0) MainScreen.text(ballsDescript() + ".  Slithering over and around them, she bathes your sack in her spit before moving up to your " + cockDescript(x) + ".", false);
	else MainScreen.text(cockDescript(x) + ".", false);
	...
	MainScreen.text(" while her spittle froths around her lips.  She manages to start humming deep within her throat, vibrating your " + cockDescript(x) + ".  The obstruction in your mouth stifles your moan, but Urta hears the startled gurgle and slides away, then plunges back down, humming the entire time in between gasps for air.\n\n", false);
...
internal function urtaHomeLuvLuvinsMenu():void {
	...
	if(player.isGoo() && player.skinType == SKIN.GOO) MainScreen.addButton(6,"Goo (Weird)",urtaGooTesticleVoreRuinedOrgasms);
	if(flags[FlagEnum.URTA_PETPLAY_DONE] >= 0 && player.gender > 0) MainScreen.addButton(7,"Collar",urtaPetPlayDeletedForeverBecauseThirdProovedMeWrongAboutDice);
}
...
private urtasCoochiNeedsFuckedLove():void {
	...
	MainScreen.text("You nod eagerly and advance, dropping your " + player.armorName + " by the wayside as you jump into bed.  She reaches around her male endowments, stroking along her lips to her puffy black clit.  Urta shivers in pleasure, spreading her legs as far as she possibly can, watching you with a hungry look as she licks her lips.  A few long strands of cunny-juice leak from her waiting hole onto the bed, practically begging you to slide inside and feel her nether-lips' velvet embrace.  You sidle up to your lover, pressing your ", false);
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("main ", false);
	MainScreen.text(multiCockDescriptLight() + " at the moist opening, shivering at the warmth and desire as it wets your tip");
	...
	if(player.hasKnot(x)) {
		...
		MainScreen.text("You rock back and forth, unable to pull out yet stimulating the sensitive bulb of your knot.  Your " + cockDescript(x) + " dumps its gooey cargo directly into your lover's stopped-up sex with barely audible squishing noises.  ", false);
		if(player.cumQ() < 250) {}
		...
		else if(player.cumQ() < 1000) MainScreen.text("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your overproductive " + Appearance.cockNoun(CockType.HUMAN) + " stops spewing inside her.  ", false);
		else {
			...
			if(player.cumQ() < 3000) MainScreen.text("By the time you finish, she looks VERY pregnant and is moaning with a mixture of pleasure and discomfort.  ", false);
				...
				if(player.cumQ() > 5000) MainScreen.text("Not content with such an easy release, your " + cockDescript(x) + " unloads the last of its cream until the bed and floor are both covered under inches of your canine cum.  ", false);
			}
		}
		MainScreen.text("You finally feel your " + Appearance.cockNoun(CockType.HUMAN) + "'s knot shrinking and pull free with a loud and messy pop.  ", false);
	...
	//[HORSE]
	else if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) {
		MainScreen.text("You feel your flare filling with blood as it ", false);
		if(player.lowerBody.cockSpot.list[x].cockLength >= 10) MainScreen.text("prods against Urta's cervix.", false);
		...
		if(player.cumQ() < 250) {}
		...
		else if(player.cumQ() < 1000) MainScreen.text("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your over-productive " + Appearance.cockNoun(CockType.HUMAN) + " has finished spewing your seed inside her.  ", false);
		else {
			...
			if(player.cumQ() > 5000) MainScreen.text("Not content with such an easy release, your " + cockDescript(x) + " unloads the last of its cream until the bed and floor are covered in several inches of thick equine cum.  ", false);
		}
	...
	//[Everybody Else]
		...
		MainScreen.text("Your " + cockDescript(0) + " clenches, filling her convulsing cunt as your seed begins pumping into her womb.  Urta cries, \"<i>oooooooOOOOHHhhh YESSSS... It's so warm and wet.  Yes, lover!  Keep goo-oh-OH-innnngg!!</i>\"\n\n", false);

		MainScreen.text("You rock back and forth, sliding your shaft through her spasming muscles, reveling and panting in orgiastic bliss.  Your " + cockDescript(0) + " dumps its gooey cargo deep into your lover's canal with barely audible squishing noises.  ", false);
		if(player.cumQ() < 250) {}
		else if(player.cumQ() < 700) MainScreen.text("Her belly puffs up slightly by the time you finish, thanks to your body's copious cum production.  ", false);
		else if(player.cumQ() < 1000) MainScreen.text("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your overproductive " + Appearance.cockNoun(CockType.HUMAN) + " stops spewing inside her.  ", false);
		else {
			MainScreen.text("Her belly swells larger and larger, bloating as you blast thick torrents of seed inside her, filling her completely while leftover jism squirts out around your " + cockDescript(0) + ".  ", false);
			if(player.cumQ() < 3000) MainScreen.text("By the time you finish, she looks VERY pregnant and is moaning with a mixture of pleasure and discomfort.  ", false);
			...
			if(player.cumQ() > 5000) MainScreen.text("Not content with such an easy release, your " + cockDescript(0) + " unloads the last of its cream until the bed and floor are both covered under inches of your gooey cum.  ", false);
		}
		MainScreen.text("You finally feel your " + Appearance.cockNoun(CockType.HUMAN) + " deflating, and you pull free with a messy slurp.  ", false);
		if(player.cumQ() >= 2000) MainScreen.text("Spunk rushes out in a torrent, splattering everywhere.  ", false);
		MainScreen.text("Urta's glazed pussy-lips clench reflexively at their sudden emptiness and you hear a very satisfied sigh from the far end of the bed.\n\n", false);
	}
	//[RESUME]
	MainScreen.text("Urta calms down at last, though her member is still throbbing weakly as she peels her condom off.  The bloated cum-bubble at the tip is the size of a basketball, and she struggles to get it tied off and disposed of.  The fox tosses it into a waste-basket already full of the things and gives you a toothy smile, \"<i>I think somebody's " + Appearance.cockNoun(CockType.HUMAN) + " loves my pussy almost as much as I love you!</i>\"\n\n", false);
...
public scyllaAndUrtaSittingInATree():void {
	...
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00143]++;
	...
	if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00147] == 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00145] > 0)
	{
		...
		// Butans
		if (player.cockThatFits(urtaCapacity()) != -1 && player.lowerBody != LOWER_BODY.CENTAUR)
		{
	...
	else
	{
		...
		else if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11) MainScreen.text("giving you a lewd look", false);
		...
		else if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11) MainScreen.text("Ohhh, you showed up a little late... mmmhmm yes, now lick... Oh, why didn't you tell me ho - OH YES!  RIGHT THERE! - h-how good of a cock-sucker this lewd-bodied bitch was?", false);
...
private fuckUrtaWhileScyllaSucksSober():void {
	MainScreen.clearText();
	let x: number = player.cockThatFits(urtaCapacity());
	MainScreen.text("You peel out of your straining equipment to free your " + multiCockDescriptLight());
	...
	MainScreen.text("  Throughout it all, your " + cockDescript(x) + " continues to do its level best to fill the fox.  Fireworks of uncontrollable pleasure go off one after the other, alternatively numbing and locking your muscles as you release all of your pent-up lusts, desires, and needs into the welcoming cunt.");
	if(player.cumQ() >= 500) {
...
private dockWithUrtaUnderScyllasSoberSupervision():void {
	...
	if(!player.isBiped() && !player.isDrider()) MainScreen.text("slips along");
	...
	if(flags[FlagEnum.URTA_SCYLLA_BIG_DICK_TIMES_DONE] > 0) MainScreen.text("Just like last time, she adopts a panicked expression and lunges for the door, but she moves slowly and a gooey drop of pre-cum hangs from her flare as she moves.");
	...
	flags[FlagEnum.URTA_SCYLLA_BIG_DICK_TIMES_DONE]++;
...
private flipUrtaTheBird():void {
	...
	flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] -= 2;
	if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 0) flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] = 0;
...
private watchTwoHotBitchesAndJerkIt():void {
	...
	MainScreen.text("You open your " + player.armorName + " and sit down on a crate no more than a few feet away from the orally fixated couple, fishing out " + sMultiCockDesc(), false);
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" and exposing your " + vaginaDescript(0), false);
	...
	MainScreen.text("You stroke " + sMultiCockDesc(), false);
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" and touch your " + vaginaDescript(0), false);
	...
	MainScreen.text("You pump hard on " + sMultiCockDesc(), false);
	if(player.totalCocks() > 1) MainScreen.text(" in turn", false);
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00143] == 1) MainScreen.text("\"<i>We need to do thish again!</i>\"", false);
	...
	MainScreen.text("You smile happily and pick up your " + player.armorName + ".  It's past time you checked up on your camp.", false);
...
private makeUrtaSitOnYourLapWithScylla():void {
	...
	if(player.lowerBody == LOWER_BODY.NAGA) MainScreen.text(", snaking your tail around her legs", false);
	else if(player.lowerBody == LOWER_BODY.GOO) MainScreen.text(", enveloping her legs with your goo", false);
	else MainScreen.text(", straddling your " + player.legs() + " around her hips", false);
	MainScreen.text(".  You grab her narrow waist and pull her up, dragging Scylla's cock-locked head up with her and aligning your " + cockDescript(x) + " with her backdoor before you start to lower her back down.  ", false);
	if(y > -1) MainScreen.text("Your " + cockDescript(y) + " even manages to get in line with her drooling, black-lipped pussy.  ", false);
	...
	if(y != -1) MainScreen.text(" and " + cockDescript(y), false);
	...
	if(y > -1) MainScreen.text("  Meanwhile, your other " + Appearance.cockNoun(player.lowerBody.cockSpot.list[y].cockType) + " utterly fills her womb, stuffing it so completely that the fox's belly rounds out.", false);
	...
	MainScreen.text("You grab a rag and wipe up before donning your " + player.armorName + ".  Urta saunters over, looking like she's sobered up quite a bit.  She's got ", false);
	...
	else if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 11) MainScreen.text("a nervous smile on her face, but gives you a quick peck all the same.", false);
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00143] <= 1) MainScreen.text("pay for the door and ", false);
	...
	urtaLove(3);
	//+comfort
	flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] += 3;
...
private tellUrtaNoMoreScylla():void {
	urtaSprite();
	MainScreen.text("", true);
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00147] = 1;
...
private tellUrtaToBeADrunkenHussy():void {
	...
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] = 1;
	dynStats("lus", 5);
	doNext(camp.returnToCampUseOneHour);
}
//[Drink Less]
...
private amilyXUrtaUrtaFallout(): void {
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00350] = 2;
	urtaSprite();
	MainScreen.text("", true);
	MainScreen.text("You ", false);
	if(!player.isTaur()) MainScreen.text("pull up a chair", false);
...
public urtaDialogueMenu():void {
	...
	if (flags[FlagEnum.KATHERINE_UNLOCKED] >= 4) MainScreen.addButton(5, "Katherine", urtaDiscussessKatherine);
	if(urtaPregs.urtaKids() > 0 && player.hasKeyItem("Spare Key to Urta's House") < 0)
		...
		if(flags[FlagEnum.FIRST_TIME_AT_URTA_HOUSE] > 0) MainScreen.addButton(4,"Her House",urtaPregs.talkToUrtaAboutHerHouse);
...
private urtaDiscussesSelf():void {
	...
	else if (flags[FlagEnum.URTA_QUEST_STATUS] == .5)
	...
	else if (flags[FlagEnum.URTA_QUEST_STATUS] == 1)
...
public flirtWithUrta():void {
	MainScreen.clearText();
	if (flags[FlagEnum.URTA_PC_LOVE_COUNTER] == -1) { //In friends mode
	...
	MainScreen.text("You quietly slide your " + (player.isNaga() ? "tail" : "foot") + " across under the table and rub it against the inside of her leg.  Urta practically jumps out of her seat, then tries to pretend nothing happened.\n\n");
	MainScreen.text("You’re pretty sure most of the bar noticed, but you don’t care.  You keep rubbing and feel a thump as something large smacks against the underside of the table.  Urta drains the last of her drink and whispers, “<i>" + (flags[FlagEnum.URTA_PC_LOVE_COUNTER] == -1 ? "What about Kath?  I... I wouldn't..." :"Please - it’s so hard...") + "</i>”\n\n");
	MainScreen.text("You never find out exactly what she was going to say.  When you start to rub your " + (player.isNaga() ? "tail" : "foot") + " in little circles against the root of her huge cock Urta loses her train of thought and just leans back in her chair, trying to keep from spraying her pent up cum across the floor.\n\n");
	MainScreen.text("You ease up so she has a chance of understanding what you’re saying.  “<i>" + (flags[FlagEnum.URTA_PC_LOVE_COUNTER] == -1 ? "I'll explain everything to Kath.  You want this as much as I do, right?" : "Yes, some chicks, with their big dicks, just do it for me.") + "</i>”  You slide " + (player.isNaga() ? "the tip of your tail" : "your toes") + " a little lower and start moving it in little circles against her balls.\n\n");
	...
	flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] = 0; //urtaFuckHer will raise it to 1
	flags[FlagEnum.URTA_PC_LOVE_COUNTER] = 0; //Reset both flags to put Urta back to her 'never met you before' state, ready for her first fuck
...
private urtaFertilityDiscussion():void {
	...
	if(flags[FlagEnum.URTA_FERTILE] == 1) MainScreen.text("  Should I stay fertile?");
	...
	if(flags[FlagEnum.URTA_PREG_EVERYBODY] == 1) MainScreen.text("  Or did you come to ask me not to be fertile and virile around anyone else?");
	else {
		MainScreen.text("  Would you mind me being able to put a bun in Edryn's oven?  Imagine all the lil foxtaurs running around!");
		if(flags[FlagEnum.EDRYN_NUMBER_OF_KIDS] > 0) MainScreen.text("  After all, you already had a turn!  Fair's fair!");
	}
	MainScreen.text("</i>\"  She winks at you before blowing you a playful raspberry.\n");
	menu();
	if(flags[FlagEnum.URTA_FERTILE] == 1) {
	...
	if(flags[FlagEnum.URTA_PREG_EVERYBODY] == 1) {
...
private urtaPregOthersToggle():void {
	...
	if(flags[FlagEnum.URTA_PREG_EVERYBODY] == 1) {
		...
		flags[FlagEnum.URTA_PREG_EVERYBODY] = 0;
	}
	else {
		...
		flags[FlagEnum.URTA_PREG_EVERYBODY] = 1;
	}
	menu();
	MainScreen.addButton(0,"Back",urtaFertilityDiscussion);
}
...
private urtaDiscussesAlcholism():void {
	MainScreen.clearText();
	urtaSprite();
	if(flags[FlagEnum.DISCUSSED_URTA_ALCOHOLISM] == 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00145] == 0) {
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] == 1)
			...
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] == 0)
...
private urtaDiscussAlcoholDrinkMore():void {
	...
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] = 1;
	flags[FlagEnum.DISCUSSED_URTA_ALCOHOLISM] = 1;
	doNext(camp.returnToCampUseOneHour);
}
//[=Drink Less=]
...
private urtaDiscussAlcoholNoChange():void {
	...
	//{bonus wuv points}
	if(flags[FlagEnum.DISCUSSED_URTA_ALCOHOLISM] == 0) urtaLove(1);
	flags[FlagEnum.DISCUSSED_URTA_ALCOHOLISM] = 1;
	//{No new tags flagged}
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] = 0;
	doNext(camp.returnToCampUseOneHour);
}
...
private urtaDiscussesFamily():void {
	...
	if(!urtaLove()) {
		...
		//(Regular
		if(!urtaLove()) {
			if(flags[FlagEnum.URTA_FAMILY_TALK_ATTEMPTS] == 0)
...
private urtaDiscussesSexAndRomance():void {
	...
	if(player.stats.int < 20) dynStats("int", .5);
	if(player.stats.int < 40) dynStats("int", .5);
	if(player.stats.int < 60) dynStats("int", .5);
	urtaLove(.5);
	//[Leave] [Tease]
...
private urtaDiscussesEdrynRomance():void {
	...
	if(!urtaLove()) {
		if(!urtaOpenAboutEdryn()) {
			flags[FlagEnum.URTA_OPEN_ABOUT_EDRYN] = 1;
	...
	//Lover
		...
		if(!urtaOpenAboutEdryn()) {
			...
			if(flags[FlagEnum.EDRYN_NUMBER_OF_KIDS] > 0) {
				if(flags[FlagEnum.URTA_QUEST_STATUS] == 1) {
...
private urtaDiscussesEdrynLoveTolerate():void {
	...
	//{Choosing this option should probably disable both the Romance? discussion topic and the ability to have threesomes with Urta & Edryn - if you can't accept that she needs to fuck mare-cunt when you're not around, then you're probably not the sort of guy who's going to take kindly to the idea of doing her from behind even as she pounds Edryn.}
 	flags[FlagEnum.PC_DISLIKES_URTA_AND_EDRYN_TOGETHER] = 1;
	doNext(camp.returnToCampUseOneHour);
}
//[=Breakup=]
...
private urtaDiscussessKatherine():void {
	MainScreen.clearText();
	let affection: number = flags[FlagEnum.KATHERINE_URTA_AFFECTION];
...
private urtaKathSexDont():void {
	...
	flags[FlagEnum.KATHERINE_URTA_DATE] = Katherine.KDATE_LITTLE;
...
private urtaKathSexWhenever():void {
	...
	flags[FlagEnum.KATHERINE_URTA_DATE] = Katherine.KDATE_WHENEVER;
	doNext(urtaDialogueMenu);
}
...
private urtaKathSexEncourage():void {
	...
	flags[FlagEnum.KATHERINE_URTA_DATE] = Katherine.KDATE_LOTS;
	doNext(urtaDialogueMenu);
}
...
public giveTheFoxSomeEggs():void {
	MainScreen.clearText();
	if(flags[FlagEnum.URTA_TIMES_EGG_PREGGED] > 0) {
		repeatUrtaEgging();
		return;
	}
	...
	if(flags[FlagEnum.PC_FETISH] > 0) MainScreen.text("as much fun as it might be, ");
	...
	if(player.isTaur()) MainScreen.text("positioning your centaur, ");
	else if(player.isDrider()) MainScreen.text("drider body over the bulk of her back, ");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) wetness += player.wetness()*100;
	...
	flags[FlagEnum.URTA_EGGS] = player.eggs();
	flags[FlagEnum.URTA_FERTILE_EGGS] = player.fertilizedEggs();
	flags[FlagEnum.URTA_TIMES_EGG_PREGGED]++;
	...
	flags[FlagEnum.URTA_FLATBELLY_NOTICE] = 0;
	//First time, tag for triggering freakout!
	if(flags[FlagEnum.URTA_EGG_FORCE_EVENT] == 0) flags[FlagEnum.URTA_EGG_FORCE_EVENT] = 48;
	player.dumpEggs();
	player.orgasm();
	doNext(camp.returnToCampUseOneHour);
}
...
public urtaChewsOutPC(newScreen:boolean = true):void {
	...
	//[Unfertilised Eggs:
	if(flags[FlagEnum.URTA_FERTILE_EGGS] == 0) {
	...
	flags[FlagEnum.URTA_EGG_FORCE_EVENT] = -1;
...
private condomlessUrtaInHouseSmex():void {
	...
	if(player.gender > 0 && player.isNaga()) MainScreen.addButton(2,"TailFun",nagaOnUrtaMess);
...
private pegUrtasAssMessy():void {
	...
	MainScreen.text(cockDescript(x) + " down onto the sheets, a scant few inches from Urta's puckered asshole.  The fox ");
	if(urtaDrunk()) MainScreen.text("giggles drunkenly, \"<i>You're sho dirty!</i>\" while spreading those fishnet-clad legs further.");
	else if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 11) MainScreen.text("softly begs, \"<i>Be gentle, please,</i>\" while spreading those fishnet-clad legs a little further apart.");
	...
	if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > 15) MainScreen.text("casually");
	else if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > 11) MainScreen.text("confidently");
	else if(flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > 4) MainScreen.text("a little nervously");
	...
	if(player.cockArea(x) > urtaCapacity()) MainScreen.text("The aching pressure around your " + cockDescript(x) + " doubles, and you realize she simply can't take any more.  Her anus is stretched wide, dilated to the extreme to accommodate this much of you.  There's little to do but plow her with what you can until she's fountaining white.");
		...
		MainScreen.text("The aching pleasure around your " + cockDescript(x) + " redoubles as you hilt her, smashing your ");
	...
	if(urtaDrunk()) MainScreen.text("h");
	MainScreen.text("o goooo-ooooooh yes!</i>\"  Her ass clutches and relaxes to your tempo, hugging your " + cockDescript(x) + " tight");
	...
	if(flags[FlagEnum.URTA_QUEST_STATUS] < 0) MainScreen.text("not-so-fertile ");
...
private fillMeUpPleaseUrta():void {
	...
	if(player.tongueType > TONUGE.HUMAN) MainScreen.text("prodigious ");
	...
	if(player.wetness() > 3) MainScreen.text("  You're so utterly soaked thanks to this place that a steady stream of vaginal lubricant drizzles over the meaty tool below.");
	...
	if(player.wetness() > 3) MainScreen.text(", though it's a drop in the bucket next to your copious moisture.  Hell, even now, rivers of your girl-goo are sliding down that spasming length, wreathing it in lady-spunk");
	...
	if(flags[FlagEnum.PC_FETISH] > 0) MainScreen.text("  An exhibitionist thrill runs through you at that, making you even wetter.");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text(", followed in short order by the untouched portion of your " + multiCockDescriptLight());
	...
	//{Goo:
	if(player.isGoo() && player.vaginalCapacity() > 9000) MainScreen.text("  Successive eruptions make you bigger, but your flexible cunt soon grows so bloated that the onrushing injections mostly squirt back out, painting Urta, the bed, and your [legs] white with cum.");
	...
	flags[FlagEnum.TIMES_FUCKED_URTA]++;
	player.slimeFeed();
	if(flags[FlagEnum.URTA_FERTILE] == 1) player.knockUp(PregnancyType.URTA, PregnancyType.INCUBATION_URTA, 25);
	player.orgasm();
	dynStats("lib", .2, "sen", -3);
}
...
private nagaOnUrtaMess():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		//[if (isSquirter = true)]
		if(player.wetness() >= 4) MainScreen.text("  After a few convulsions, your femcum starts to shoot out of your pussy quite violently, adding to the mess your lover is making.");
	}
	//if (cocks > 0)
	if(player.lowerBody.cockSpot.count() > 0) {
...
private lickOutUrtaAtHome():void {
	...
	let x: number = player.cockThatFits(urtaCapacity());
	if(x < 0) {
		MainScreen.text("Urta takes one look at your " + multiCockDescriptLight() + " before paling.  \"<i>You're a little too big for me...  Is there something else we could try?</i>\"");
	...
	MainScreen.text("  You push the rest of the way into her cunt and admire the velvety texture of her walls as they close in around you, inner muscles flexing and massaging your " + cockDescript(x) + " as she adjusts to your intrusion.");
	if(player.cockArea(x) >= 200) MainScreen.text("   Stretching ludicrously, her cunt dilates with surprising ease.  The elastic lips of her pussy cling oh-so-tightly to your girth as they're spread beyond reason.  Her hips even spread with only a modest bit of pressure and resistance as her passage is distorted, the shape of your " + cockDescript(x) + " visible through her stretched belly as it pushes towards her tits.  You can even make out some of the larger veins.");
	...
	flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] += 12;
...
private raphaelAndUrtaPartTwo():void {
	...
	if(player.tailType > TAIL.NONE) MainScreen.text("lift your tails and ");
	...
	player.orgasm();
	dynStats("sen", -3);
	flags[FlagEnum.URTA_X_RAPHAEL_HAPPENED]++;
	doNext(camp.returnToCampUseOneHour);
}
...
private urtasRuinedOrgasmsFromGooPartII():void {
	...
	if (edrynAvailable) MainScreen.text("recognize as Edryn's");
		...
		if(flags[FlagEnum.MET_OLIVIA] == 0) MainScreen.text("recognize as Olivia's");
	...
	if(flags[FlagEnum.MET_OLIVIA] == 0 && !edrynAvailable) {
...
private urtaPetPlayDeletedForeverBecauseThirdProovedMeWrongAboutDice():void {
	MainScreen.clearText();
	//First Time
	if(flags[FlagEnum.URTA_PETPLAY_DONE] > 0) {
		wearZeCollar();
		return;
	}
...
private ignoreUrtasCollar():void {
	flags[FlagEnum.URTA_PETPLAY_DONE] = -1;
	MainScreen.clearText();
...
private wearZeCollar():void {
	...
	//Second Time
	else if(flags[FlagEnum.URTA_PETPLAY_DONE] == 1) {
		...
		if(player.tailType > TAIL.NONE) MainScreen.text(", tail wagging");
	...
	if(player.tailType == TAIL.NONE) {
		...
		if(player.tailType == TAIL.NONE && player.ass.analWetness > 0) MainScreen.text("  Despite your embarrassment, or perhaps because of it, you find your asshole getting wet with desire, practically salivating in anticipation of that thick plug.");
	...
	flags[FlagEnum.URTA_PETPLAY_DONE]++;
...
private doggyStyle():void {
	...
	if(player.tailType > TAIL.NONE) MainScreen.text("tail");
	...
	if(player.tailType > TAIL.NONE) MainScreen.text("asshole");
	...
	if(player.tailType == TAIL.NONE) MainScreen.text(", while she grasps the plug in your ass firmly and pulls. It comes out easily with a wet <b>pop</b>, making your ass feel suddenly empty.  You guess it won't be empty for long, however");
...
private getAPetReward():void {
	...
	if(player.tongueType == TONUGE.SNAKE) MainScreen.text("serpentine ");
	else if(player.tongueType == TONUGE.DEMONIC) MainScreen.text("demonic ");
	else if(player.tongueType == TONUGE.DRACONIC) MainScreen.text("draconic ");
	MainScreen.text("tongue along the vixen's thick cumvein, which pulses lightly.  Urta groans and runs a hand through your " + hairDescript() + ", gripping it solidly.  You can't help as you're forced further onto the warm rod.");
...
private urtaTakesPCOnWalkies():void {
	...
	if(player.tailType  > 0) MainScreen.text("tail");
	else MainScreen.text("tailplug");
	...
	dynStats("lus", 15);
	flags[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 0;
	flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] += 5;
