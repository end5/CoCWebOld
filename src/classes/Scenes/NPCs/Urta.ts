export default class Urta extends NPCAwareContent implements TimeAwareInterface {

	//Urta variables
	//Flags.list[FlagEnum.TIMES_FUCKED_URTA] = times fucked
	//Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] = urtas horsecock comfort level
	//-1 = PC pissed her off
	//0 = first timern
	//1-10 = slightly comfortable
	//11+ = flaunt it
	//Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = hours turned off
	//0 = ready to rock!

	//Urtas horsey bits =
	//LxW = 20x3
	//Luv Lock - Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER]:
	//0 = normal, 1 = love, -1 = never love, Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] -1 = hate
	/*Luv rating! - Flags.list[FlagEnum.URTA_PC_AFFECTION_COUNTER]
	...
	*/
	...
	public let drainedByKath:boolean;

	public Urta() {
		pregnancy = new PregnancyType(FlagEnum.URTA_PREGNANCY_TYPE, FlagEnum.URTA_INCUBATION, 0, 0);
		pregnancy.addPregnancyEventSet(PregnancyType.PLAYER, 330, 334, 288, 240, 192, 144, 96, 48);
	...
	public timeChange(): boolean {
		...
		drainedByKath = false; //This let will only be set true after Kath licks Urta out - it stops you from going back immediately for more Urta sex
		pregnancy.pregnancyAdvance();
		trace("\nUrta time change: Time is " + model.time.hours + ", incubation: " + pregnancy.incubation + ", event: " + pregnancy.event);
		if (Flags.list[FlagEnum.NEED_URTA_LETTER] == 1 && model.time.hours == 6) urtaPregs.getUrtaLetter(); //Urta Letters
		if (pregnancy.incubation == 0 && (pregnancy.type == PregnancyType.BEE_EGGS || PregnancyType.DRIDER_EGGS)) {
			pregnancy.knockUpForce(); //Silently clear Urta's egg pregnancy
			Flags.list[FlagEnum.URTA_EGGS] = 0;
			Flags.list[FlagEnum.URTA_FERTILE_EGGS] = 0;
		}
		if (Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] > 0) Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME]--; //Count down timer for urta's lust
		if (Flags.list[FlagEnum.URTA_EGG_FORCE_EVENT] > 0) Flags.list[FlagEnum.URTA_EGG_FORCE_EVENT]--; //Countdown to urta freakout
		//Urta egg freak out
		if (Flags.list[FlagEnum.URTA_EGG_FORCE_EVENT] > 0 && model.time.hours > 6 && model.time.hours < 18 && Flags.list[FlagEnum.URTA_EGG_FORCE_EVENT] < 12) {
		...
		if (Flags.list[FlagEnum.URTA_ANGRY_AT_PC_COUNTDOWN] > 1) {
			Flags.list[FlagEnum.URTA_ANGRY_AT_PC_COUNTDOWN]--;
			if (Flags.list[FlagEnum.URTA_ANGRY_AT_PC_COUNTDOWN] < 1) Flags.list[FlagEnum.URTA_ANGRY_AT_PC_COUNTDOWN] = 1;
		}
		if (model.time.hours > 23) {
			if (Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] == 0) Flags.list[FlagEnum.URTA_CUM_NO_CUM_DAYS]++;
			else Flags.list[FlagEnum.URTA_CUM_NO_CUM_DAYS] = 0;
			if (Flags.list[FlagEnum.URTA_PC_AFFECTION_COUNTER] > 0) {
				Flags.list[FlagEnum.URTA_PC_AFFECTION_COUNTER] -= .5;
				if (Flags.list[FlagEnum.URTA_PC_AFFECTION_COUNTER] < 0) Flags.list[FlagEnum.URTA_PC_AFFECTION_COUNTER] = 0;
			}
		}
		return needNext;
	}
	...
	public urtaCapacity(): number {
		let bonus: number = Flags.list[FlagEnum.TIMES_RUT_FUCKED_URTAS_CUNT] * 5;
		if (bonus > 40) bonus = 40;
		if (Flags.list[FlagEnum.URTA_TENTACLE_GAPED] > 0) return 500;
		return 60 + bonus;
	}

	public urtaLove(love: number = 0): boolean {
		if (Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] == -1) return false;
		if (love == 0) {
			return Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] == 1;
		}
		if (love > 0) {
			Flags.list[FlagEnum.URTA_PC_AFFECTION_COUNTER] += love;
		}
		if (love < 0) {
			Flags.list[FlagEnum.URTA_PC_AFFECTION_COUNTER] -= love;
		}
		if (Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) {
			...
			if (love > 0 && Flags.list[FlagEnum.AMILY_VISITING_URTA] == 0 && amilyScene.amilyFollower() && Flags.list[FlagEnum.AMILY_FOLLOWER] == 1) Flags.list[FlagEnum.AMILY_NEED_TO_FREAK_ABOUT_URTA] = 1;
			return true;
		}
		else return false;
	}

	public urtaAvailableForSex(): boolean { return urtaFuckbuddy() && telAdre.scylla.action != Scylla.SCYLLA_ACTION_FUCKING_URTA && Flags.list[FlagEnum.URTA_ANGRY_AT_PC_COUNTDOWN] == 0; }

	public urtaFuckbuddy(): boolean { //Returns true if Urta is either the player's fuckbuddy or lover
		if (kGAMECLASS.urtaQuest.urtaBusy() || Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] == -1) return false;
		if (urtaJustFriends()) return false;
		return (Flags.list[FlagEnum.TIMES_FUCKED_URTA] > 0);
	}

	public urtaJustFriends(): boolean { return Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] == 0 && Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] == -1; }

	public urtaAtBar(): boolean { //Is Urta physically at the Wet Bitch?
		return (!kGAMECLASS.urtaQuest.urtaBusy() && Flags.list[FlagEnum.AMILY_VISITING_URTA] != 1 && model.time.hours < 15 && Flags.list[FlagEnum.KATHERINE_UNLOCKED] != 3);
	}

	public urtaDrunk(): boolean {
		//Preg = no drinking!
		if (pregnancy.isPregnant) return false;
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] == -1) return false;
		if (model.time.hours > 12 && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] < 1) return true;
		if (model.time.hours > 8 && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] >= 1) return true;
		else return false;
	}

	private urtaOpenAboutEdryn(): boolean {
		//Did it come out from the scylla threesome?
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00145] > 0) return true;
		//Did it come out from marble?
		if (Flags.list[FlagEnum.URTA_KNOWS_PC_HAS_MARBLE_FOLLOWER] > 0) return true;
		//Did it come out from discussions?
		if (Flags.list[FlagEnum.URTA_OPEN_ABOUT_EDRYN] > 0) return true;
		return false;
	}
	...
	public knockUpUrtaChance(): void { //Moved here from UrtaPregs since it needs to be accessed from outside Urta
		...
		//GTFO IF NOT FERTILE
		if (Flags.list[FlagEnum.URTA_FERTILE] != 1) return;
		...
		//FINAL ROLL!
		if (chance > rand(100)) {
			pregnancy.knockUpForce(PregnancyType.PLAYER, 384);
	...
	public urtaBarDescript(): boolean {
		...
		//Urta PISSED
		if (Flags.list[FlagEnum.URTA_ANGRY_AT_PC_COUNTDOWN] > 1) {
			MainScreen.text("Urta is sitting at a table, swishing a bottle around and looking forlorn.  She glances up and sees you, then immediately turns away.", false);
			return false;
		}
		//	[PISSED URTA TALK]
		...
		//[Post Pissed Talk]
		if (Flags.list[FlagEnum.URTA_ANGRY_AT_PC_COUNTDOWN] == 1) {
			MainScreen.text("Urta is sitting at a table, swishing a bottle around and looking forlorn.  She looks up at you and sighs, \"<i>Look, I still don't think we'll ever be the same, but to be frank, we're great in the sack and I've been lonely.  For now, I'll put the past behind me.</i>\"", false);
			Flags.list[FlagEnum.URTA_ANGRY_AT_PC_COUNTDOWN] = 0;
			return true;
		}
		//Raphael betrayed reward
		if (Flags.list[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] == -1 && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00148] == 0) {
			MainScreen.text("Urta has an ecstatic grin plastered across her muzzle, and it only gets wider when she sees you.  Perhaps you should see what all the fuss is about?", false);
			return true;
		}
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00350] == 1) {
			MainScreen.text("Urta is sitting at her usual table, holding her head in her hands.  She doesn't even seem to notice you.", false);
			return true;
		}
		//[URTA ASHAMED]
		if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] == -1) {
			MainScreen.text("You see Urta on a bench in the corner with her legs crossed nervously.  Her tail is curling tightly around her leg, hiding the massive surprise between her legs.  In her hand is a mostly empty bottle of whiskey.  She's wobbling back and forth in her seat, and keeping her head down, refusing to meet your gaze any time you spare a glance her way.  It looks like she wants nothing to do with you.", false);
			return true;
		}
		//Post Scylla Appearance
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00145] == 0 && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00143] > 0) {
			MainScreen.text("Urta's at her normal table, nursing a glass of water with nary a bottle of alcohol around.  She's fidgety and constantly glancing your way, but she never meets your eyes or acknowledges that she saw you.  Something is eating away at her.", false);
			return true;
		}
		//PREGNANT URTA
		if (pregnancy.isPregnant) {
			if (pregnancy.type == PregnancyType.PLAYER) {
				...
				//Egg Level 1:
				if (Flags.list[FlagEnum.URTA_EGGS] < 20) MainScreen.text("Urta is sitting quietly at her usual seat, sipping a mug of some non-alcoholic beer.  Though her clothes are still in place, you can just make out the bulging midriff stretching them taut, a consequence of all the eggs in her womb.");
				else if (Flags.list[FlagEnum.URTA_EGGS] < 40) MainScreen.text("Urta is sitting rather uncomfortably at her usual seat.  Her swollen stomach is so large it keeps her from wearing her usual outfit, instead forcing her into a mini-shirt and a skirt bottom, allowing her midriff to be bare.  She occasionally runs a hand over the expanse, as if she can't believe it's actually there.");
				//Egg Level 3:
				else MainScreen.text("Urta's bloated belly forces her to sit side-saddle at her usual table, the lumpy, egg-stuffed mass ballooning out in front of her.  Forced into a midriff-baring ensemble, she frequently touches it, occasionally seeming to trace an egg through the taut skin.");
			}
			return true;
		}
		//Eggs Laid:
		...
		if (Flags.list[FlagEnum.URTA_TIMES_EGG_PREGGED] > 0 && !pregnancy.isPregnant && Flags.list[FlagEnum.URTA_FLATBELLY_NOTICE] == 0) {
			MainScreen.text("Urta is seated at her usual spot in her usual dress, eagerly chugging down mug after mug of booze; her belly is washboard-flat again, and she's clearly making up for lost time after having given up alcohol for her 'pregnancy'.");
			Flags.list[FlagEnum.URTA_FLATBELLY_NOTICE] = 1;
			return true;
		}
		//[Love Urta Bar Appearance]
		if (Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) {
			if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] == -1 || !urtaDrunk()) MainScreen.text("Urta is sitting at her usual table, sipping a glass of wine and wearing a form-fitting evening gown of shimmering black.  She looks up at you, happiness filling her eyes when she notices you entering the bar.", false);
			//(DRUNK)
			else MainScreen.text("Urta is sitting at her usual table wearing a shimmering black dress.  She's pretty clearly sloshed judging by her bleary gaze as she looks up at you.  The front of her dress visibly tents, and she waves you over with a lecherous grin.", false);
			return true;
		}
		if (Flags.list[FlagEnum.TIMES_FUCKED_URTA] == 0 && Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > -1) {
			MainScreen.text("You see Urta on a bench in the corner with her feet propped up on a stool.  Her tail is coiled tightly around her left leg, just like every other time you've seen her.  The pretty gray fox gives you a slight nod before turning her attention back to the large square bottle of whiskey in front of her.", false);
			return true;
		}
		//[URTA FUCKBUDDIED]
		if (Flags.list[FlagEnum.TIMES_FUCKED_URTA] > 0) {
			MainScreen.text("You see Urta on a bench in the corner with her feet propped up on a stool.  She has a small mug on a table, quite different from the swill she chugged when you first met.  Her clothing is... far more risqué.  The top she's chosen for the moment is sheer to the extreme, making her proud black nipples visible to anyone who glances her way, and the short skirt she's wearing is nearly revealing her cunny.  ", false);
			if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 11) MainScreen.text("Thankfully she still has her other addition trapped under her tail, for now.", false);
			else MainScreen.text("Her half-hard horse-cock is plainly visible to anyone who spares her a glance, barely contained by a pair of tightly woven fishnet stockings.  She's looking at you and wiggling a finger your way...", false);
			return true;
		}
		CoC_Settings.error("");
	...
	public urtaBarApproach(): void {
		...
		if ((player.lowerBody.cockSpot.hasCock() && player.cockThatFits(urtaCapacity()) >= 0) || player.hasKeyItem("Deluxe Dildo") >= 0) spank = spankTheShitOutOfUrtaAndMakeHerCreamHerselfFromProstateStimulationAloneLikeTheHornyDrunkenSlutSheReallyIs;
		MainScreen.clearText();
		//Raphael Reward
		if (Flags.list[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] == -1 && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00148] == 0) {
			Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00148] = 1;
			player.stats.gems += 1000;
			statScreenRefresh();
			MainScreen.text(images.showImage("urta-bar"), false);
			MainScreen.text("Urta claps as you walk up to her with an exuberant smile spread across her vulpine face.  She proudly announces, \"<i>You really turned in the Russet Rogue?!  I almost can't believe it.  Do you know how many women that rascal has fleeced out of their possessions?  The last count I made was over three dozen, and he even had the audacity to try it with me.  I'm ashamed to admit it nearly worked, but once he saw ALL of me, well he couldn't keep up the facade.  I nearly caught him that night.</i>\"\n\n", false);

			MainScreen.text("You smirk at her tale.  Knowing Raphael, seeing a horny Urta would've made him white as a ghost.  She goes on to list some of his crimes, \"<i>He's guilty of petty theft, disturbing the peace, robbery, lewd public behavior, lying under oath, defacing a public structure, destroying a public structure... I'd probably have to get the paperwork to remember the rest.  In truth his shenanigans aren't usually a big deal individually, but they've added up and garnered him a fairly sizable bounty.  He'll serve his punishment, and I hope he learns his lesson for a change.</i>\"\n\n", false);

			MainScreen.text("Urta fishes out a rather sizable pouch and tosses it onto the table, the numerous gems inside tinkling loudly as it plops down in front of you.  You untie the silken cord that holds the leather bag closed and see hundreds of sparkling gems, perhaps even a thousand.  Wow!  Raphael must have pissed off the wrong people!\n\n", false);

			MainScreen.text("The gray fox knocks back a swig from her bottle and sighs as if you've just removed one of the many weights hanging from her shoulders.  She looks back at you with a twinkle in her eyes, and you realize you've definitely improved her opinion of you.  Sadly there isn't time for more chit-chat, and you head back to camp, your gem-pouch heavy with new weight.", false);
			//(+love score!)
			urtaLove(3);
		...
		//TO ZE FLIPOUT!
		if (Flags.list[FlagEnum.URTA_EGG_FORCE_EVENT] > 0) {
			urtaChewsOutPC();
			return;
		}
		...
		//HERE WE GOEZ!
		if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 5 && ((player.inRut && player.lowerBody.cockSpot.hasCock()) || (player.inHeat && player.lowerBody.vaginaSpot.hasVagina()))) {
		...
		if (Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] > 0 && !urtaDrunk()) {
			MainScreen.text(images.showImage("urta-bar"), false);
			//Cockblock marble convo
			if (marbleScene.marbleAtCamp() && Flags.list[FlagEnum.URTA_KNOWS_PC_HAS_MARBLE_FOLLOWER] == 0) {
				Flags.list[FlagEnum.URTA_KNOWS_PC_HAS_MARBLE_FOLLOWER] = 1;
				marbleCockuBlockuUrta();
				return;
			}
			//Twu wuv talk
			if (Flags.list[FlagEnum.URTA_PC_AFFECTION_COUNTER] >= 30 && Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] == 0) {
				UrtaTwuWuvOffer();
				return;
			}
			//MainScreen.text("You approach Urta, who gives you a friendly smile and begins talking with you.  Thanks to her unusual endowments, it's easy to tell she's not quite in the mood.  The conversation is still pleasant though, and the two of you knock back a few ales while Urta recounts some of the wilder scenarios she's encountered as the captain of Tel'Adre's guard.", false);
		...
		if (Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) {
			...
			if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] == -1 || !urtaDrunk()) {
				MainScreen.text(images.showImage("urta-bar-drunk"), false);
				MainScreen.text("You walk up to Urta and you see her pivot herself to the side.  Her dress visibly tents at the sight of you, and you smirk as you realize she shifted in position to avoid slamming her hardening cock into the bottom of the table.  You cuddle close and wrap an arm around your lover, giving her a quick kiss on the lips and fondling her through her dress.  The tent immediately darkens as a rush of pre soaks into the material, and Urta half-pants, half-asks, \"<i>Oooh, lover, what do you have in mind today?</i>\"\n\n", false);
				MainScreen.text("You could go back to her place, suck her off under the table, or eat her out under the table.", false);
				if (player.canOviposit()) {
					if (Flags.list[FlagEnum.URTA_TIMES_EGG_PREGGED] == 0) MainScreen.text("\n\nYou contemplate asking Urta to help you take a load off your abdomen, but decide against it; Urta's probably too shy and pent up to let you fill her with your eggs. Maybe when she's gotten herself drunk and is more pliable, though...");
				...
				menu();
				MainScreen.addButton(0, "Her Place", goBackToUrtasForLuvinz);
				if (Flags.list[FlagEnum.URTA_CUM_NO_CUM_DAYS] >= 5) MainScreen.addButton(1, "Suck Off", slurpFawkesCocksForFunAndInflation);
				else MainScreen.addButton(1, "Suck Off", blowUrtaUnderTheTableLuv);
				MainScreen.addButton(2, "Eat Out", eatUrtaOutNomNomPussy);
				return;
			}
			//[Horny Urta Talk – Drunk]
			else {
				MainScreen.text(images.showImage("urta-bar-drunk"), false);
				MainScreen.text("You walk up to Urta and chuckle as the sloshed fox-girl latches onto you.  She squeals happily and crushes you into a tight hug.  Her hard cock rubs against you under the silken covering of her dress, slowly soaking it through with pre-cum.  She purrs drunkenly under your ear, \"<i>Mmmm, I was hoping you'd show up, lover.  Would you mind if I blew a load over your face while everyone watched?  Or maybe you could just crawl up into my lap and let me pump you full of cum?</i>\"\n\n", false);
				MainScreen.text("You could let her make a show of you sucking her off, try to ride her discreetly, or walk out and leave her disappointed.", false);
				temp = null;
				if (player.canOviposit()) {
				...
				menu();
				MainScreen.addButton(0, "Jerkoff", getAPublicFacialFromUrta);
				MainScreen.addButton(1, "Anal Ride", takeUrtaInTheButtPublically);
				MainScreen.addButton(2, "Lay Eggs", temp);
				...
				if (Flags.list[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] == -2 && RaphaelLikes() && Flags.list[FlagEnum.URTA_X_RAPHAEL_HAPPENED] == 0) {
					...
					MainScreen.addButton(8, "3SomeSurprise", urtaAndRaphaelSurprise);
				}
		...
		if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] == 0 && Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > -1) {
			MainScreen.text(images.showImage("urta-bar-drunk"), false);
			MainScreen.text("Urta smiles broadly and noisily slurps a few dribbles of whiskey from her shiny black lips, watching you approach.  She gives you a smoking-hot 'come-hither' look and slurs, \"<i>I didn't think I'd get a chance to bump into you here, cutie.  Wanna dance?</i>\"\n\n", false);
			MainScreen.text("Before you can answer, something thumps the table hard enough to shake it.  You look at Urta quizzically, and she shrugs through a blush so bright it's visible through her fur.  The lithe fox squirms in her seat, panting and gasping, \"<i>Ummm, maybe later, I just realized I have to- OH MY GOD WHAT IS THAT?</i>\"\n\n", false);
			MainScreen.text("You spend a moment looking over your shoulder, but can't for the life of you figure out what she was freaking out about.  She must be really drunk!  You turn back to scold her, but Urta has vanished!  She must have left in a hurry – she left her bottle of \"<i>John Doe</i>\" behind, and still half-full.  The bar's back-door swings closed, maybe she went that way.  What do you do?", false);
		...
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] != -1 && urtaDrunk()) {
			MainScreen.text(images.showImage("urta-bar-drunk"), false);
			MainScreen.text("The drunken fox roughly gropes you and whispers in your ear, \"<i>I NEED a good fuck right now, and it feels like someone wants a piece of Urta.  I can guess just which piece you're wanting too.  The only question is do I ", false);
			//CUT FOR NOW MainScreen.text("throw you over the table and fuck you,", false);
			MainScreen.text("let the beast loose and bounce you on my lap while we drink, ", false);
			MainScreen.text("jerk off onto your face in front of everyone, or have to remember your ass for later when you bolt for the door.  What'll it be, hun?</i>\"", false);
			temp = null;
			if (player.canOviposit()) {
				if (Flags.list[FlagEnum.URTA_TIMES_EGG_PREGGED] == 0) MainScreen.text("\n\nHmm... sounds like she wants to fuck almost as badly as you want to get these eggs out of you.  Besides, didn't Urta say to you once that she's basically barren and sterile?  You'd be doing her a favor by letting her carry your eggs, wouldn't you?  Let's see if she's drunk enough to let you fill her full...");
				temp = giveTheFoxSomeEggs;
		...
		if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11 && rand(2) == 0) {
			MainScreen.text(images.showImage("urta-bar"), false);
			MainScreen.text("You approach Urta, who gives you a naughty wink and asks, \"<i>Would you like to play with my little pony?</i>\"  One of her hands is trailing along her leg, caressing her member as it hardens under her tight fishnet stockings.\n\n(You can suck her off under the table or go back to her place to fuck.)", false);
		}
		else {
			MainScreen.text(images.showImage("urta-bar"), false);
			MainScreen.text("You approach Urta, who gives you a lascivious grin ", false);
			if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 11) MainScreen.text("moments before you hear her horse-cock thump into the bottom of the table.  ", false);
			else MainScreen.text("and pets her already-growing horse-cock through her fishnet stockings.  ", false);
			if (rand(2) == 0) {
				MainScreen.text("Urta asks, \"<i>I'm really horny, do you think you could help me get off?  I guess we could go fuck at my place, or you could always just help me out under the table.</i>\"  A blush colors her face through the gray fur as she suggests the latter option.", false);
			}
			else MainScreen.text("She asks, \"<i>Would you like to go back to my place and help me play with my 'little' friend again?  Or maybe you could climb under the table and give me some relief?</i>\"", false);
		}
		if (player.canOviposit()) {
			if (Flags.list[FlagEnum.URTA_TIMES_EGG_PREGGED] == 0) MainScreen.text("\n\nYou contemplate asking Urta to help you take a load off your abdomen, but decide against it; Urta's probably too shy and pent up to let you fill her with your eggs. Maybe when she's gotten herself drunk and is more pliable, though...");
			else MainScreen.text("\n\nYou don't think it's likely Urta would agree to host your eggs in her present state; wait until she's turned down her inhibitions before asking.");
		...
		menu();
		if (Flags.list[FlagEnum.URTA_CUM_NO_CUM_DAYS] >= 5) MainScreen.addButton(0, "Hidden BJ", slurpFawkesCocksForFunAndInflation);
		else MainScreen.addButton(0, "Hidden BJ", blowUrtaUnderTable);
		MainScreen.addButton(1, "Urta's Place", goBackToUrtasForLuvinz);
	...
	private drinkUrtasBoozePtTwo(): void {
		urtaSprite();
		MainScreen.text("", true);
		MainScreen.text("With a happy sigh you ", false);
		if (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.cockSpot.count() == 0) MainScreen.text("squat down and ", false);
		MainScreen.text("open up your " + player.inventory.armor.displayName + ", releasing the pressure onto a patch of grass.  It takes forever to stop, but when it does you feel like a million bucks.  You turn and start to close up your kit when you realize you aren't alone.  Urta is watching you, her lips parted hungrily.  She's still peeing, but what's truly shocking is the mammoth package she's got hanging between her legs.  It's a massive horse-shaft, patterned black and pink, slung down from between a pair of apple-sized balls.  Before your eyes the equine beast begins hardening, forcing the distracted fox to empty the last of her urine against a building's wall.  She blushes furiously, but the drunken fox doesn't seem to care about her shame...\n\n", false);
		MainScreen.text("She shakes her massive horse-meat more than a few times, shuddering lightly with pleasure as her tip flares a bit in response.  Urta stumbles over moaning, \"<i>This ish all your fault!  You damned... sexy... mmm... letsh fuck!</i>\"\n\n", false);
		MainScreen.text("The stumbling hermaphrodite is openly rubbing herself as she closes in on you.  What do you do?", false);
		//[RUN!] [LET HER]
	...
	private drinkUrtasBoozeRun(): void {
		urtaSprite();
		MainScreen.text("", true);
		MainScreen.text("You turn about and run, nearly smacking into the door frame in your haste to get away.  You hear a half-strangled sob behind you as the door swings closed.", false);
		doNext(camp.returnToCampUseOneHour);
		Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] = -1;
	}
	private drinkUrtasBoozeLetHer(): void {
		urtaSprite();
		player.slimeFeed();
		Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY]++;
		Flags.list[FlagEnum.TIMES_FUCKED_URTA]++;
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 5;
		hideUpDown();
		player.orgasm();
		player.stats.sens += -2;
		//[LET HER]
		MainScreen.text("", true);
		MainScreen.text(images.showImage("urta-behindbar-fuck"), false);
		MainScreen.text("You nod at Urta, feeling a bit relaxed from all the whiskey and more than a little intrigued by the beast between the slender fox's legs.  She's on you before you know it, her softly furred hand squeezing one of your ", false);
		if (player.lowerBody.balls > 0) MainScreen.text(BallsDescriptor.describeBalls(true, true, player), false);
		else if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) MainScreen.text(biggestBreastSizeDescript(), false);
		else MainScreen.text(BreastDescriptor.describeNipple(0) + "s", false);
		MainScreen.text(", as her tongue spears between your lips.  She practically mouth-rapes you as her trembling member grinds between your " + LowerBodyDescriptor.describeLegs(player) + ", smearing them with a generous helping of foxy horse-pre.  The horny drunk slurs into your ear, \"<i>Gawdsh above, are you this much of a schlut for everyone with one of these?</i>\"\n\n", false);

		MainScreen.text("With surprising strength, the inebriated fox pushes you against an old bench, flat onto your ", false);
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) MainScreen.text("tits", false);
		else MainScreen.text("belly", false);
		MainScreen.text(".  She pulls at your " + player.inventory.armor.displayName + " until your crotch is completely exposed and vulnerable, revealing your " + LowerBodyDescriptor.assholeOrPussy(player), false);
		if (player.lowerBody.cockSpot.count() > 0) MainScreen.text(" and " + CockDescriptor.describeMultiCock(player), false);
		MainScreen.text(".  Urta snuggles close, brushing her nose along your thighs before plunging her tongue deep into your " + LowerBodyDescriptor.assholeOrPussy(player) + " in one powerful, drunken lick.  She slurps and licks, humming away until you're ", false);
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("wet", false);
		else if (player.lowerBody.cockSpot.count() > 0) MainScreen.text("suitably lubricated and leaking pre", false);
		else MainScreen.text("suitably lubricated", false);
		MainScreen.text(".\n\n", false);

		MainScreen.text("With a quick shift in position, she forces herself inside, her arms supporting her jiggling chest as she leans over you, dragging her nipples up your back.  Her fat shaft forces your body to spread wide.  The fat head and ring of prepuce strokes your inner walls, squeezing grunts of pleasure from you as the drunken fox forcefully begins fucking you, rutting like a beast.  You can feel her nipples harden, digging into your back and eliciting moans from their owner as they're repeatedly smashed against you.  Urta starts licking your neck where it joins your shoulder, gently nipping at your tender flesh as her tip begins to flare wider inside you.", false);
		player.cuntChange(60, true, true, false);
		MainScreen.text("\n\n", false);

		MainScreen.text("\"<i>Mmmm I shhoulda ushed a condom, you're gonna be a messssss!</i>\" she cries as she climbs atop her sexual peak, mounting it just like she's mounting your " + LowerBodyDescriptor.assholeOrPussy(player) + ".  You're spread wide, the equine member rammed tightly ", false);
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("against your cervix as the tip presses firmly against it", false);
		else MainScreen.text("into your rectum", false);
		MainScreen.text(", unloading a thick batch of cream directly into your ", false);
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("waiting womb", false);
		else MainScreen.text("abused asshole", false);
		MainScreen.text(".", false);
		if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 250) MainScreen.text("  Whatever's growing inside you is probably bathing in the stuff now.", false);
		else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  You briefly wonder if she's capable of knocking you up, and what the baby would look like.", false);
		else MainScreen.text("  Your backside fills with warmth as her spunk seeps deep inside you.", false);
		MainScreen.text("  Another blast fills you to capacity, and you're mortified by the sensation of your body being shifted by your growing belly.\n\n", false);

		MainScreen.text("Urta sucks on the edge of your ear, still pumping away as she pours a bit more spooge into your now equine-sized cavity.  Your alcohol-dulled mind reels as your body caves in to the sticky fluid assault, cumming hard and wriggling around her shaft, moaning repeatedly as you clamp down tightly.  ", false);
		if (player.lowerBody.cockSpot.count() > 0) {
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("Each of y", false);
			else MainScreen.text("Y", false);
			MainScreen.text("our " + CockDescriptor.describeMultiCockShort(player) + " squirts hard, unloading a batch of sticky jism under your ", false);
			if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 100) MainScreen.text("pregnancy swollen ", false);
			MainScreen.text("belly, making you squish with each brutal thrust of your oversized lover's rod.", false);
		}
		else {
			if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("Your pussy dribbles and clenches, leaking Urta's whiteness as she claims it for her own.", false);
			else MainScreen.text("Your " + ButtDescriptor.describeButthole(player) + " clamps down tightly, spasming in pleasure around the invader.", false);
		}
		if (player.lowerBody.cockSpot.count() > 0) MainScreen.text("  As you cum, more and more whiteness dribbles out until a puddle forms underneath you, showing Urta just how much you enjoyed the rough, drunken fuck.\n\n", false);
		else MainScreen.text("\n\n", false);

		MainScreen.text("She pulls out, dripping everywhere.  The fox leans down and proceeds to give your still quivering hole a tongue-bath, thoroughly cleaning her fluids from the entrance and teasing you until you clamp down, locking the rest inside.  With a gentle slap on your " + ButtDescriptor.describeButt(player) + " Urta says, \"<i>Thankss for the quick fuck.  Maybe shometime you can ", false);
		if (player.lowerBody.cockSpot.count() > 0) MainScreen.text("stick it in me", false);
		else MainScreen.text("come back for an even bigger load", false);
		MainScreen.text(".</i>\"\n\n", false);
		MainScreen.text("The fox helps you to your feet and the two of you get dressed, stumbling a bit over each other and giggling happily.  Urta gives you a quick peck on the lips and whispers, \"<i>Thanksh... for everything,</i>\" before the two of you make your way back into the bar and go your separate ways.\n\n", false);
		if (player.lowerBody.vaginaSpot.hasVagina()) player.cuntChange(60, true);
	...
	private urtaSneakAwayFromMasturbate(): void {
		urtaSprite();
		MainScreen.text("", true);
		MainScreen.text("You sneak away from the depressing sight, and decide to head back to camp.", false);
	...
	private dualUrtaMasturbation(): void {
		...
		MainScreen.text("", true);
		MainScreen.text(images.showImage("urta-behindbar-masti-dual"), false);
		MainScreen.text("You come out of the shadows, shedding your " + player.inventory.armor.displayName + ".  ", false);
		if (player.stats.cor < 60) MainScreen.text("You blush furiously at exposing yourself in such a way", false);
		else MainScreen.text("Smiling seductively, you revel in exposing yourself so suddenly", false);
		MainScreen.text(".  Urta looks up at you in shock, blinking tears from her eyes and stammering, \"<i>B-b-b-ut I have th-thish hideoush thing... Aren't you scared of it?</i>\"\n\n", false);

		MainScreen.text("Gently wiping a tear from her muzzle, you comfort the distraught fox and sit down across from her, giving her a perfect view of your ", false);
		if (player.lowerBody.cockSpot.count() > 0) {
			MainScreen.text(CockDescriptor.describeMultiCockShort(player), false);
		...
		else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)), false);
		else MainScreen.text("bare groin and " + ButtDescriptor.describeButthole(player), false);
		MainScreen.text(".  ", false);
		if (player.lowerBody.cockSpot.count() > 0) {
			MainScreen.text("The situation gives you all the spark you need to rise to your full size, ", false);
			if (player.cockArea(0) < 40) MainScreen.text("even if Urta's member dwarfs you.", false);
			else if (player.cockArea(0) < 70) MainScreen.text("showing Urta you're just as big.", false);
			else MainScreen.text("proving to Urta just who has the bigger burden.", false);
			MainScreen.text("  ", false);
		}
		if (player.lowerBody.vaginaSpot.hasVagina()) {
			if (player.lowerBody.vaginaSpot.get(0).vaginalWetness > VaginaWetness.SLICK) MainScreen.text("A trickle of wetness escapes your nether-lips, starting to puddle underneath you.", false);
			else MainScreen.text("Quickly engorging, your lower lips begin to part around your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + ".", false);
			MainScreen.text("  ", false);
		}
		MainScreen.text("The heat coursing through you quickly shows itself on your ", false);
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text(BreastDescriptor.describeAllBreasts(player), false);
		else MainScreen.text("chest", false);
		MainScreen.text(" as your " + BreastDescriptor.describeNipple(0) + "s become full and sensitive.\n\n", false);

		MainScreen.text("Urta relaxes, leaning against a wall as she returns to pleasuring her mismatched member.  Her eyes lock onto your groin as you join her, and she lets out a sigh that's equal parts relieved and inebriated.  Together, the two of you pleasure your bodies, each watching the other's squirming form and blissful expression.  ", false);
		if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("Your hands caress your " + CockDescriptor.describeCock(player, 0) + " feverishly, tweaking every ounce of pleasure from the bloated rod.  ", false);
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("Your hands caress your " + CockDescriptor.describeMultiCock(player) + " in turn, dancing back and forth from rod to rod as you attempt to play with every available pleasure outlet.  ", false);
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("Delving deeply between your legs, you press forward into the source of your feminine pleasures, touching and rubbing, caressing your swollen button as you get closer and closer to orgasm.\n\n", false);
		if (player.gender == 0) MainScreen.text("Delving down between your legs, rubbing along your taint before you slowly penetrate your " + ButtDescriptor.describeButthole(player) + ", pleasuring yourself in the only way possible.\n\n", false);

		MainScreen.text("The cute fox-girl reaches her peak, crying out in a loud moan that echoes around the alley.  Her equine shaft trembles, the medial ring becoming more pronounced while the flat head flares wide mid-orgasm.  A blast of cum launches into the air, fans out overhead, and splatters down over you.  Urta looks horrified, and yanks herself back, succeeding only in depositing the next blast of spoogey goodness into her own face.  The situation would be comical if the foxy horse-goo wasn't such a great lubricant for your hands, and with that thought in your mind, you cum too.\n\n", false);

		if (player.lowerBody.cockSpot.count() > 0) {
			if (player.cumQ() < 250) {
				MainScreen.text("Jism spurts out onto the alley floor, forming a nice little puddle as ", false);
				if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("each of ", false);
				MainScreen.text("your " + CockDescriptor.describeMultiCockShort(player) + " empties itself.  ", false);
			}
			else if (player.cumQ() < 500) MainScreen.text("Jism pours out onto the alley floor as your body releases the copious amount of cum it can produce.  It puddles before you, running off the stone into the grass.  ", false);
			else if (player.cumQ() < 1000) {
				MainScreen.text("Jism erupts from ", false);
				if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("each of ", false);
				MainScreen.text("your " + CockDescriptor.describeMultiCockShort(player) + ", blasting into the air before splattering onto the ground between you and Urta.  It forms a massive puddle, spreading as you pump gob after gob of cum into it until the sticky spooge is soaking your " + LowerBodyDescriptor.describeLegs(player) + " and Urta too.", false);
			}
			else {
				MainScreen.text("Jism fountains from ", false);
				if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("each of ", false);
				MainScreen.text("your " + CockDescriptor.describeMultiCockShort(player) + ", flooding the small alleyway with an unholy amount of cum.  In no time you've painted Urta and yourself from head to toe, and the alleyway sloshes with the stuff as your production overwhelms the ground's ability to absorb liquid.  ", false);
			}
		}
		if (player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("Your sex squishes wetly around a finger, clenching down tightly", false);
			if (player.lowerBody.vaginaSpot.get(0).vaginalWetness == VaginaWetness.SLAVERING) MainScreen.text(" and gushing everywhere", false);
			MainScreen.text(".  ", false);
		}
		else MainScreen.text("Your sphincter clenches down tightly around your invading digit as you get off, and the rapt attention of your audience certainly helps.  ", false);
		MainScreen.text("Sighing happily, you give your nipple a tweak as Urta finally finishes cumming, ", false);
		if (player.cumQ() >= 1000) MainScreen.text("adding to the massive pool on the ground after ", false);
		MainScreen.text("drenching herself thoroughly.\n\n", false);

		MainScreen.text("Urta lets out a VERY relieved-sounding sigh and murmurs, \"<i>Oh godsh, thank you.  You don't know how hard it ish to hide thish beasht from EVERYONE.  You're a good friend for helping me with thish... let'sh do thish again sometime, ok?</i>\"\n\n", false);
		MainScreen.text("She staggers up to her feet and rapidly discards her sodden clothes.  With a resigned sigh, Urta pulls a loose stone out of the side of a building and removes a sheer skirt and diaphanous top from the cubby.  They slip on easily, though the new clothes look a bit more 'slutty' than her previous pair, easily displaying her hard black nipples.  Urta replaces the stone, wobbling a bit unsteadily before she ", false);
		if (player.cumQ() >= 1000) MainScreen.text("tip-toes through the spunk", false);
		else MainScreen.text("gets her balance and heads for the bar's backdoor", false);
		MainScreen.text(".  Her tail curls up around the softening horse-cock, hiding it from view as she blows you a sloppy kiss and disappears inside.\n\n", false);

		MainScreen.text("You waste no time cleaning up and stagger back inside after her, sated and happy with your new friend and her 'benefits'.", false);
		doNext(camp.returnToCampUseOneHour);
		player.orgasm();
	}
	...
	public urtaFuckHer(afterBefriending: boolean = false): void {
		...
		Flags.list[FlagEnum.TIMES_FUCKED_URTA]++;
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 5;
		let cocks: number = player.lowerBody.cockSpot.count();
		if (!afterBefriending) MainScreen.clearText();
		//(FUCK HER FEMALE/GENDERLESS)
		if (player.lowerBody.cockSpot.count() == 0) {
			MainScreen.text(images.showImage("urta-behindbar-female-fuckurta"), false);
			player.slimeFeed();
			MainScreen.text("You come out of the shadows, shedding your " + player.inventory.armor.displayName + ".  ", false);
			if (player.stats.cor < 50) MainScreen.text("You blush furiously at exposing yourself in such a way");
			else MainScreen.text("Smiling seductively; you revel in exposing yourself so suddenly");
			MainScreen.text(".  Urta looks up at you in shock, blinking tears from her eyes and stammering, \"<i>B-b-b-ut I have th-thish hideoush thing... Aren't you scared of it?</i>\"\n\n", false);

			MainScreen.text("Gently wiping a tear from her muzzle, you answer, \"<i>Not at all, but I think you'd have more luck with a nice warm hole to slide that monster into.</i>\"\n\n", false);

			MainScreen.text("Urta nods, numbed by the unexpected and forward nature of your offer.  She looks up hopefully and pries her hands away from the twitching behemoth in her loins, waiting to see how you intend to deal with it.  You approach her and turn, straddling your " + LowerBodyDescriptor.describeLegs(player) + " over her, watching her expression as you lower yourself down, ", false);
			if (player.lowerBody.tailType == TailType.HORSE || player.lowerBody.tailType == TailType.DOG) MainScreen.text("swishing your tail across her taut nipples", false);
			else MainScreen.text("making her wait", false);
			MainScreen.text(".  Eventually the blunted tip of Urta's horse-cock presses against your " + LowerBodyDescriptor.assholeOrPussy(player) + ".  Agonizingly, you force yourself to relax and lower yourself down on the mottled pink and black shaft.\n\n", false);

			MainScreen.text("The horse-cock spreads you wide, filling you past the point of fullness as you sink further and further along its length.  Urta groans happily, her hips twitching involuntarily and forcing more of the monster-sized cock inside you.", false);
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				if (player.vaginalCapacity() >= 60) {
					MainScreen.text("  You sigh and sink down, taking it until the folds of her sheath are rubbing your entrance.", false);
				}
				else {
					MainScreen.text("  You nearly cry out as you sink down, taking it until the folds of her sheath are rubbing your entrance, even though it hurts to accommodate it.", false);
				}
				player.cuntChange(60, true, true, false);
			}
			else {
			...
			MainScreen.text("  Urta babbles, \"<i>Oh godsh yesh thank you so much-ohmigod it's SO HOT inside you.</i>\"\n\n", false);

			MainScreen.text("You smirk over your shoulder and start bouncing up and down on her, slapping her balls with your " + ButtDescriptor.describeButt(player) + " each time you bottom out on the thick shaft.  You can feel her ring of prepuce rubbing your inner walls, pressing on nerves normally ignored.  Urta grunts underneath you and you feel her flare growing wider inside you, stretching you further and further with every bounce atop the equine-endowed slut.  Suddenly warmth is flooding into your ", false);
			if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("womb", false);
			else MainScreen.text("rectum", false);
			MainScreen.text(", and Urta's tongue is lolling out from her muzzle, drooling down the side of her face as she loses herself in orgasm.\n\n", false);

			MainScreen.text("The substantial flare forms a perfect plug inside you and your belly distends slightly as you're packed full of foxy-horse-cum.  Your own lusts keep your hips moving, grinding and squeezing the shaft inside you as it packs you full of Urta's seed.  Your orgasm explodes through your hips and your " + LowerBodyDescriptor.describeLegs(player) + " give out underneath you, dropping you down onto the fox-girl as the pair of you cum together.  Whiteness begins squirting out around her shaft as her prick begins softening inside you.  You sigh happily, content but too nerveless to stand for the moment.  Peeking over your shoulder and seeing Urta's satisfied expression, you guess she feels about the same.\n\n", false);

			MainScreen.text("Once the horse-dick inside you has sufficiently softened, you stand up, gasping as a rush of spooge escapes from your " + LowerBodyDescriptor.assholeOrPussy(player) + " now that the 'plug' is removed.  Urta gives you a cocky grin as she's soaked with her fluids, moments before she starts staggering back to her feet.  She wipes up with her clothes and tosses them into the garbage, then wobbles up to a loose brick in the wall and pulls it free, revealing a hidden compartment filled with sets of replacement clothes.  She removes a tight skirt and sheer top, both a bit more revealing than what she wore in the bar earlier, and hurriedly dresses herself.\n\n", false);

			MainScreen.text("Urta blows you a kiss as you put your own gear back together and flounces back to the bar, calling out, \"<i>We should do thish again shoon!</i>\"\n\n", false);

			MainScreen.text("\"<i>Yes indeed,</i>\" you muse - the sooner the better.", false);
		}
		else {
			let x: number = player.cockThatFits(urtaCapacity());
			let y: number = player.cockThatFits2(urtaCapacity());
			if (x < 0) x = 0;
			MainScreen.text(images.showImage("urta-behindbar-male-fuckurta"), false);
			MainScreen.text("You come out of the shadows, shedding your " + player.inventory.armor.displayName + ".  ", false);
			if (player.stats.cor < 60) MainScreen.text("You blush furiously at exposing yourself in such a way");
			else MainScreen.text("Smiling seductively; you revel in exposing yourself so suddenly");
			MainScreen.text(".  Urta looks up at you in shock, blinking tears from her eyes and stammering, \"<i>B-b-b-ut I have th-thish hideoush thing... Aren't you scared of it?</i>\"\n\n", false);

			MainScreen.text("Gently wiping a tear from her muzzle, you answer, \"<i>Not at all, but I think you'd have more luck with something stuffed in your feminine half.</i>\"\n\n", false);

			MainScreen.text("Urta gives you a shocked look, but nods and turns around, bending over an old bench and exposing her well-toned ass and glistening, black-lipped sex.  The invitation is all you need.  You wrap your arms around her, squeezing the downy-soft fur on her breasts and leveraging ", false);
			if (player.lowerBody.cockSpot.count() > 0) {
				if (player.lowerBody.cockSpot.count() > 2) MainScreen.text("a pair of your " + CockDescriptor.describeMultiCockShort(player) + " against her pussy and asshole.  ", false);
				else if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("your " + CockDescriptor.describeMultiCockShort(player) + " against her pussy and asshole.  ", false);
				else if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("your " + CockDescriptor.describeMultiCockShort(player) + " against her pussy.  ", false);
			}
			//DOES IT FITZ
			if (player.cockArea(x) < urtaCapacity()) {
				MainScreen.text("The wet entrance yields before you, accepting you like a velvet glove.  It grips you tightly, though the grip is meaningless with the flood of lubricant slipping over your shaft.  ", false);
				if (player.lowerBody.cockSpot.count() >= 2) MainScreen.text("Her backdoor was rougher going – the tightly muscled entrance held you back for a moment until Urta relaxed completely, but now the tighter hole is nearly as easy to penetrate as her pussy.  ", false);
				MainScreen.text("The fox is openly moaning and stroking her disproportional cock, dripping pre-cum everywhere as you do your level best to violate her ", false);
				if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("hole", false);
				else MainScreen.text("holes", false);
				MainScreen.text(".\n\n", false);

				MainScreen.text("She begins grinding against you, begging, \"<i>Oh godsh yeeeessss don't shtop fucking me!  Give me more please!  PLEASE!</i>\"\n\n", false);
				...
				if (cocks == 1) MainScreen.text("fuck", false);
				else MainScreen.text("double-penetrate", false);
				MainScreen.text(" her.\n\n", false);

				MainScreen.text("\"<i>Yesh oh yessssssSSSAHHHCUMMING,</i>\" cries Urta as her body goes nerveless underneath you, held up entirely by your hands", false);

				if (cocks > 1 && y >= 0) MainScreen.text(", " + CockDescriptor.describeCock(player, x) + ", and " + CockDescriptor.describeCock(player, y) + ".", false);
				else MainScreen.text(" and " + CockDescriptor.describeCock(player, x) + ".", false);
				MainScreen.text("  Both her hands are locked tightly around her behemoth as it unloads onto the alley floor and walls.  The juicy tunnel clamps down on top of you, squeezing your " + CockDescriptor.describeCock(player, x) + " as if Urta's life depended on it.  It's as if every ounce of her being was focusing on clamping down on the invader, massaging, squeezing, and milking it, while the rest of her body goes entirely limp.", false);
				if (cocks > 1 && y >= 0) MainScreen.text("  Your " + CockDescriptor.describeCock(player, y) + " feels like it's about to explode inside her asshole when her sphincter clamps down tightly, acting like the world's tightest cock-ring.", false);
				MainScreen.text("\n\n", false);

				MainScreen.text("You fuck her savagely, abusing her spasming hole", false);
				if (cocks > 1 && y >= 0) MainScreen.text("s", false);
				MainScreen.text(" with every thrust until your own climax bursts over you like a wave.  Urta's drooling cunt ", false);
				if (cocks > 1 && y >= 0) MainScreen.text("and tightly clenched ass seem", false);
				else MainScreen.text("seems", false);
				MainScreen.text(" to be trying to milk your body of every ounce of cum.  ", false);

				if (player.cumQ() < 250) {
					MainScreen.text("After a few passionate thrusts, you're spent, and you pull free, admiring the glaze of white that remains on her cunt-lips", false);
					if (cocks > 1 && y >= 0) MainScreen.text(" and slowly-closing pucker", false);
					MainScreen.text(".", false);
				}
				else if (player.cumQ() < 500) {
					MainScreen.text("After a few passionate thrusts, you can feel your cum packing Urta's womb ", false);
					if (cocks > 1 && y >= 0) MainScreen.text("and gut ", false);
					MainScreen.text("full of spooge, but all great things come to an end eventually.  You pull free, admiring the glaze of white that remains on her cunt-lips", false);
					if (cocks > 1 && y >= 0) MainScreen.text(" and slowly-closing pucker", false);
					MainScreen.text(".", false);
				}
				else {
					MainScreen.text("After a few passionate thrusts, Urta's womb ", false);
					if (cocks > 1 && y >= 0) MainScreen.text("and belly are ", false);
					else MainScreen.text("is ", false);
					MainScreen.text("so full of spooge that she looks positively pregnant, and you're expelled from her body by the sheer force of your orgasm.  You pause to admire your handiwork, watching whiteness drip from ", false);
					if (cocks > 1 && y >= 0) MainScreen.text("both her holes.", false);
					else MainScreen.text("her abused hole.", false);
				}
				...
				MainScreen.text("You waste no time cleaning up and you stagger back inside after her, sated and happy with your new friend and her 'benefits'.", false);
				knockUpUrtaChance();
			}
			//(TOO BIG)
			else {
				if (cocks == 1) {
					MainScreen.text("Urta's hole is just way too small to handle what you're packing, so you're forced to back off, much to her lament.  She moans, \"<i>Noooo! Fuck me!</i>\"\n\n", false);
					MainScreen.text("You pull her down into the grass and throw her on her back.  She looks confused and a little afraid, but that melts away when you spread your " + LowerBodyDescriptor.describeLegs(player) + " across her and begin grinding your " + CockDescriptor.describeMultiCockShort(player) + " against her sensitive dick.  Happy, the drunken fox begins pistoning against you, gathering her copious pre and smearing it over your " + CockDescriptor.describeMultiCock(player) + ".  The two of you grind together, groin to groin, coating each other in a slippery mess.\n\n", false);
					MainScreen.text("Urta pants out, \"<i>Ah ahhh ahhhh,</i>\" and cums, her flare ballooning out until it's twice as wide as her shaft, a wave of white jism escaping from her horse-cock.  You gather some in your hand and slather it over your own " + CockDescriptor.describeMultiCock(player) + ", and after a few moments of fevered masturbation, you start cumming with her.  Urta happily squirms against you, painting herself with jism and soaking her clothes with the stuff.  ", false);

					if (player.cumQ() < 250) MainScreen.text("You join in, though your own contribution seems meager by comparison.", false);
					else if (player.cumQ() < 500) MainScreen.text("You join in, and manage to match her spurt for spurt, until she's lying in a thick puddle of the stuff.", false);
					else MainScreen.text("You join in, and make her orgasm look like a trickle compared to the flood of spunk you unleash onto her.  By the time the pair of you finish, she's soaked from head to toe, and lying in a nicely sized puddle of the stuff.", false);
					...
					MainScreen.text("After a few moments she picks herself up and wipes herself up as much as she can with her sodden clothes.  Urta quickly gives up and removes a loose brick from the wall, revealing a hidden compartment with clothes and a few towels.  It seems she's quite prepared to make a mess in this alley.  She towels the worst of it off and gets dressed in a sheer tank-top and tight skirt before staggering back towards the bar.  Urta pauses and looks over her shoulder to blow you a sloppy kiss before she goes inside.\n\n", false);

					MainScreen.text("You waste no time cleaning up and mosey back inside after her, sated and happy with your new friend and her 'benefits'.", false);
				}
				if (cocks > 1) {
					MainScreen.text("Urta's holes are just way too small to handle what you're packing, so you're forced to back off, much to her lament.  She moans, \"<i>Noooo! Fuck me!</i>\"\n\n", false);

					MainScreen.text("You pull her down into the grass and throw her on her back.  She looks confused and a little afraid, but that melts away when you spread your " + LowerBodyDescriptor.describeLegs(player) + " across her and begin grinding each of your " + CockDescriptor.describeMultiCockShort(player) + " against her sensitive dick.  Happy, the drunken fox begins pistoning against you, gathering her copious pre and smearing it over your " + CockDescriptor.describeMultiCockShort(player) + ".  The two of you grind together, groin to groin, coating each other in a slippery mess.\n\n", false);

					MainScreen.text("Urta pants out, \"<i>Ah ahhh ahhhh,</i>\" and cums, her flare ballooning out until it's twice as wide as her shaft, a wave of white jism escaping from her horse-cock.  You gather some in your hand and slather it over your own " + CockDescriptor.describeMultiCockShort(player) + ", and after a few moments of fevered masturbation, you start cumming with her.  Urta happily squirms against you, painting herself with jism and soaking her clothes with the stuff.  ", false);
					if (player.cumQ() < 250) MainScreen.text("You join in, though your own contribution seems meager by comparison.", false);
					else if (player.cumQ() < 500) MainScreen.text("You join in, and manage to match her spurt for spurt, until she's lying in a thick puddle of the stuff.", false);
					else MainScreen.text("You join in, and make her orgasm look like a trickle compared to the flood of spunk you unleash onto her.  By the time the pair of you finish, she's soaked from head to toe, and lying in a nicely sized puddle of the stuff.", false);

					MainScreen.text("The drunken fox groans and continues spurting weakly, \"<i>Oh godsh, letsh do this again sooon.  You don't know how much I needed thish!</i>\"\n\n", false);
					MainScreen.text("After a few moments she picks herself up and wipes herself up as much as she can with her sodden clothes.  Urta quickly gives up and removes a loose brick from the wall, revealing a hidden compartment with clothes and a few towels.  It seems she's quite prepared to make a mess in this alley.  She towels the worst of it off and gets dressed in a sheer tank-top and tight skirt before staggering back towards the bar.  Urta pauses and looks over her shoulder to blow you a sloppy kiss before she goes inside.\n\n", false);
	...
	internal function blowUrtaUnderTable(): void {
		...
		Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY]++;
		urtaLove(1);
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 2 + rand(4);
		MainScreen.text("", true);
		MainScreen.text(images.showImage("urta-bar-bj"), false);
		MainScreen.text("Urta's eyes widen in shock when you suggest taking her under the table.  She stammers, \"<i>Uh, okay...</i>\" and fills a glass with straight whiskey, fueling up on liquid courage while you crawl down.  You slip down under the table, pushing her legs apart", false);
		if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11) MainScreen.text(" and pulling down her fishnet to release the monster trapped inside it.  It snaps up immediately, bobbing before your face as it fills with Urta's booze-fueled lust.", false);
		else MainScreen.text(" and pushing up her tight leather skirt, smiling at the sight of her trembling balls and juicy pussy.  Her tail unfurls around the equine member and it snaps up immediately, hanging before your face.", false);
		MainScreen.text("  It actually wiggles in time with the beating of her heart, bouncing up as it gets harder and harder, until it smacks your nose.\n\n", false);

		MainScreen.text("You hear the sound of her shot-glass slamming into the table with authority as she finishes her drink.  A bead of pre-cum bubbles out from Urta's urethra, signaling her readiness.  The horse-like penis gives off a salty scent, unique and not unpleasant.  Before you have a chance to change your mind, you lunge forward, opening your jaw as far as it will go and just barely taking her first few inches into your mouth.  The fox gasps out loud at the pleasure before she remembers to relax.  You can hear her pouring another drink, and in an effort to embarrass her, you begin sucking and licking around the edges of her flare, swallowing the surge of sweet-tasting pre-cum as it begins to fill your mouth.\n\n", false);

		MainScreen.text("Urta writhes in her seat, noisily spilling alcohol over the table.  You smile around the intruder in your mouth, gripping it with both hands and stroking it with tender caresses.  The gray fox slams down the bottle; though you can't tell if it's in irritation or due to difficulty controlling her muscles, and begins chugging down what she managed to get into her glass.  It seems Urta intends to drink until she doesn't care about any embarrassment you can force upon her.\n\n", false);

		MainScreen.text("Deciding you've got to get her off before she can drink herself into a stupor, you ", false);
		if (player.stats.cor < 70) MainScreen.text("do your best to suppress your gag reflex and shove forwards, ramming her cock as deeply into your throat as you can.", false);
		else MainScreen.text("shove forwards, ramming her massive cock so far down your throat you can feel it dribbling pre-cum into your belly.", false);

		MainScreen.text("  Urta grunts, doing her best to muffle the noise as it escapes her lips, but you can feel her twitch and thicken inside you, beginning to drip with proof of her complete and total submission to your mouth.\n\n", false);

		MainScreen.text("You can hear chuckles from the bar's other occupants as they realize what's going on, and you blush, wishing there was a tablecloth to hide your activities from prying eyes.  The distraction does little to divert you from your chosen task, and you close your eyes, tasting your lover's throbbing shaft as it fills your mouth.  With both hands stroking, you bob up and down, completely centering your world around the pulsating horse-prick as it fills your mouth.\n\n", false);

		MainScreen.text("Without warning, Urta's legs wrap around your back, squeezing with her soft fur as she vices them closed.  You're forced to take her even more deeply, until the folds of her sheath are bumping your nose and chin and filling your nostrils with her heady musk.  You can't even breathe with such a massive intruder blocking your throat, but you do your best to get her off, knowing it's the quickest way to end it.  You suck hard and squeeze her balls, gurgling noisily around the sweaty shaft as you feel Urta give in.\n\n", false);

		MainScreen.text("A wave-like motion passes through your throat obstruction, stretching you wider than ever before.  It passes, leaving behind a feeling of warm fullness.  You feel another working its way into your mouth, and realize Urta is cumming, filling your belly with direct injections of horsey fox-seed. You desperately need to breathe, but you're trapped on your lover's maleness, and with nothing else to do, you give her twitching balls a squeeze, feeling her body twitch in response as she dumps a particularly heavy batch of spunk into you.  Happy that you could at least control how hard she came, you begin blacking out, barely noticing as Urta's orgasmic contractions die off.\n\n", false);

		MainScreen.text("Thumping down on your back, you're blissfully aware of the sweet air filling your lungs.  In a moment of reflection, you realize you can taste the salty flavor of Urta's seed on your lips and mouth, and the retracting horse-cock is ", false);
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) MainScreen.text("dragging between your " + BreastDescriptor.describeAllBreasts(player), false);
		else MainScreen.text("dragging across your chest", false);
		MainScreen.text(" as it begins to soften.  You wipe as much of her leavings from your " + HeadDescriptor.describeFace(player) + " as you can, licking the slippery glaze from your lips.  Before you get up, you make sure to give Urta's member a firm squeeze, punishment for forcing you to take her so deep.\n\n", false);

		MainScreen.text("Once you've climbed out from under the table, you're VERY aware of the eyes of some of the nearby bar patrons on your back.  It seems your undercover act managed to draw more than a little attention.  Urta's cheeks burn bright-red under her gray fur, even though her eyes are a bit glassy and unfocused from the amount of alcohol she's ingested.  Sure that she'll remember the embarrassment, you give her a deep kiss, making her taste her residue on your lips.", false);
		dynStats("sen", 1, "lus", rand(10) + 5 + player.stats.lib / 10);
		doNext(camp.returnToCampUseOneHour);
	...
	internal function takeUrtaInTheButtPublically(): void {
		...
		if (player.stats.cor < 30 && Flags.list[FlagEnum.PC_FETISH] == 0) {
			MainScreen.text("No way!  You're not going to do that in front of EVERYONE.", false);
			doNext(telAdre.barTelAdre);
			return;
		}
		player.slimeFeed();
		...
		urtaLove(2);
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 5;
		MainScreen.text(images.showImage("urta-public-fuck"), false);
		if (player.stats.cor < 60 && Flags.list[FlagEnum.PC_FETISH] == 0) MainScreen.text("You can't believe you're doing it, but you undo the lower half of your " + player.inventory.armor.displayName + ".  ", false);
		else MainScreen.text("You happily remove the lower half of your " + player.inventory.armor.displayName + ", more than a little excited at the prospect of putting on a show for the crowd.  ", false);
		MainScreen.text("Urta casually flips up her skirt and ", false);
		if (Flags.list[FlagEnum.TIMES_FUCKED_URTA] < 11) MainScreen.text("unwinds her tail from around the beast", false);
		else MainScreen.text("fishes the beast out of her fishnets", false);
		MainScreen.text(".  Watching in horror, your eyes never leave the equine pillar as it grows to its full size.  Amazingly, none of the bar's patrons seem to notice the trembling horse-cock, even though it's beginning to leak with pre-cum.\n\n", false);

		MainScreen.text("You carefully shift your position, climbing in front of Urta.  With care, you grab the sensitive member just below the flare and begin rubbing it over your " + ButtDescriptor.describeButt(player) + " and ready ass-hole, lubing yourself up and trying to relax.  The impatient fox grabs you by your " + LowerBodyDescriptor.describeHips(player) + " and pulls you down, forcing herself against your pucker.  ", false);
		//[GO TOO SMALL OR FITS, NO NEW PG]
		//[FITS]
		if (player.analCapacity() >= 60) {
			MainScreen.text("It's a tight fit, even with a backdoor as gaping as yours.  You're thankful for your looseness as you slide down into her lap, and Urta signals her appreciation of your capacity with a throaty sigh, \"<i>Mmmm, you're good at this.</i>\"\n\n", false);
		}
		//[DOESN'T FIT]
		else {
			tooBig = true;
			MainScreen.text("It's far too big for you, but that doesn't stop Urta from pushing it inside you.  You can feel yourself stretched around the invader, and it hurts enough that you almost scream out in pain, but inexorably, you're drawn down until your " + ButtDescriptor.describeButt(player) + " is resting on Urta's thighs.  Her thick prick keeps you stretched uncomfortably wide, and you can feel every twitch it makes.", false);
			//(STRETCH TEXT HERE)
			if (!player.buttChange(60, true)) MainScreen.text("  ", false);
			MainScreen.text("Surprisingly, the over-endowed fox notices your discomfort and hands you a narrow bottle from the table, \"<i>Drink up, it'll help with the pain until you get used to me.</i>\"\n\n", false);

			MainScreen.text("You look at the bottle suspiciously, reading the faded yellow label, \"<i>Barkardi 151</i>\".  Wincing from the pain, you suppose it couldn't hurt and you tip back the bottle, taking a hearty swig.  It burns like hellfire, and you nearly retch up the massive swallow before you get it down.  Amazingly, the pain in your throat and the bonfire in your belly nearly drowns out what's going on in your " + ButtDescriptor.describeButthole(player) + ".  You actually start to relax, and Urta nibbles on your ear in between sips of ale.  She holds her mug up to your lips and invites you to share her drink and you do, grateful for any additional pain-killers.\n\n", false);
		}
		//[BOTH]
		if (player.lowerBody.cockSpot.count() > 0) {
			MainScreen.text("You start to grow hard from the pressure inside you, and there's nothing you can do to stop ", false);
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("each of ", false);
			MainScreen.text("your " + CockDescriptor.describeMultiCockShort(player) + " from swelling to full erectness.  A bead of pre forms at ", false);
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("each ", false);
			else MainScreen.text("the ", false);
			MainScreen.text("tip, and begins leaking down the shaft", false);
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s", false);
			MainScreen.text(" of your " + CockDescriptor.describeMultiCockShort(player) + ", squeezed out from your prostate.  ", false);
		}
		if (player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("Heat spreads through your loins as your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " engorges with blood, allowing your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " to ", false);
			if (player.lowerBody.vaginaSpot.get(0).clitLength < 2) MainScreen.text("peep out", false);
			else MainScreen.text("grow to its full size", false);
			MainScreen.text(".  ", false);
			if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < VaginaWetness.SLICK) MainScreen.text("Moisture beads on your lips, signaling just how turned on this total violation is making you.", false);
			else if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < VaginaWetness.SLAVERING) MainScreen.text("Moisture begins dripping onto Urta's thighs, signaling just how turned on this total violation is making you.", false);
			else MainScreen.text("Moisture puddles on the seat under Urta as your juices flood her thighs, revealing just how turned on this violation is making you.", false);
			MainScreen.text("  ", false);
		}
		//(NIPPLES:
		MainScreen.text("Your " + BreastDescriptor.describeNipple(0) + "s tent up under your " + player.inventory.armor.displayName, false);
		if (player.upperBody.chest.hasFuckableNipples() && player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier >= 1) MainScreen.text(", leaking milky lubricants and aching to be penetrated.", false);
		else if (player.upperBody.chest.hasFuckableNipples()) MainScreen.text(", aching to be penetrated.", false);
		else if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier >= 1.5 && player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier < 2.5) MainScreen.text(", leaking thin streams of milk that rapidly become visible to everyone around you.", false);
		else if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier >= 2.5) MainScreen.text(", spurting thick streams of milk that soak your " + player.inventory.armor.displayName + " and are visible to everyone in the bar.", false);
		else MainScreen.text(".", false);
		MainScreen.text("\n\n", false);

		MainScreen.text("Soft-furred hands grip your " + LowerBodyDescriptor.describeHips(player) + " firmly and begin to lift you up, making you gasp as her flare is dragged through your hind-end, eliciting what Urta must think are cute gasps.  You hear a 'murrrr' of contentment in your ear and Urta releases you, twitching inside you as your weight slowly carries you back down into her lap.  Once your " + ButtDescriptor.describeButt(player) + " smacks into her thighs she grabs you again, dragging you back up her massive pole until you're looking down on the tables around you.  She stops and withdraws her support, guiding you back down her shaft and moaning when you take her ring of prepuce half-way down.\n\n", false);

		MainScreen.text("The eyes of everyone in the place are locked onto you, and ", false);

		if (tooBig) MainScreen.text("you find yourself giggling drunkenly, barely feeling any pain and arching your back like a slut for the crowd.", false);
		else if (player.stats.cor < 50 && Flags.list[FlagEnum.PC_FETISH] == 0) MainScreen.text("you find yourself whimpering, regretting the decision to climb on top of her, but aroused in spite of it.", false);
		else MainScreen.text("you find yourself sliding your hands under your " + player.inventory.armor.displayName + " and pinching your " + BreastDescriptor.describeNipple(0) + "s, putting on a show for the crowd.", false);

		MainScreen.text("  Urta starts breathing hard into your ear, and you start flexing your " + LowerBodyDescriptor.describeLegs(player) + " to help her lift you up.  Grateful for the effort, the fox-girl increases the pace by helping to pull you back down each time.  By now it's starting to feel pretty good, and her pre-cum is making your " + ButtDescriptor.describeButthole(player) + " so wet and slippery that you can slide quite quickly along the column of horse-meat.\n\n", false);

		MainScreen.text("Wet squelches echo through the bar as you're slammed into Urta's lap harder and harder.", false);
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) {
			MainScreen.text("  Your " + BreastDescriptor.describeAllBreasts(player) + " are jiggling and bouncing wildly, ", false);
			if (player.upperBody.chest.hasFuckableNipples()) MainScreen.text("dripping lubricants", false);
			else if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier > 1) MainScreen.text("spurting milk", false);
			else MainScreen.text("rubbing your tight nipples against your " + player.inventory.armor.displayName, false);
			MainScreen.text(".", false);
		}
		if (player.lowerBody.cockSpot.count() > 0) {
			if (player.lowerBody.cockSpot.count() > 1) {
				MainScreen.text("  Steady drizzles of cum begin dripping from each of your " + CockDescriptor.describeMultiCockShort(player) + " as she pounds away, rubbing hard against your prostate.", false);
			}
			else {
				MainScreen.text("  Steady drizzles of cum begin dripping from your " + CockDescriptor.describeMultiCockShort(player) + " as she pounds away, rubbing hard against your prostate.", false);
			}
		}
		MainScreen.text("  You can't hold back, and start moaning quietly, trying your best to be silent and failing horribly.  For her part, your foxy lover seems equally incapable of suppressing her orgasm, and you feel her thick organ flexing inside you as it prepares to cum.\n\n", false);

		MainScreen.text("Her paws pull you down hard, crushing your " + ButtDescriptor.describeButt(player) + " against her as her sheath presses into your " + ButtDescriptor.describeButthole(player) + ".  The invading flesh ripples from the base towards the tip, culminating in a flood of warmth that makes your belly gurgle, and at that moment you cum.  Your sphincter clamps down tightly around Urta's marvelous member, unintentionally providing the fox with an even better orgasm.  The next pulse of cum feels even larger, and you look down in shock when you realize you can see the outline of her shaft on the " + player.skinDesc + " of your belly.  As she cums you can see it getting less distinct as your body becomes more and more rounded.  A few blasts later you look positively pudgy, but you can feel her softening inside you.\n\n", false);

		MainScreen.text("Urta sighs and takes a sip of her drink, giving you a quick peck on the side of your neck.  You start to rise, but the drunken hermaphrodite grabs you and holds you down, \"<i>Let's just stay like this a little longer...</i>\"\n\n", false);

		MainScreen.text("You shrug and relax, cuddling with her while her massive load is plugged inside you.  She keeps you there for the better part of an hour, kissing you, nipping at your neck, and sharing her drinks with you.  When you finally do get a chance to leave, you're stumbling slightly from the alcohol and your asshole gapes and drips whiteness behind you.  You don't notice the trail of cum until you get back to your camp, and it brings a blush to your cheeks.", false);
		player.orgasm();
		player.stats.sens += -2;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	internal function getAPublicFacialFromUrta(): void {
		...
		Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY]++;
		urtaLove(2);
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 3 + rand(3);
		MainScreen.text("", true);
		MainScreen.text(images.showImage("urta-public-facial"), false);
		MainScreen.text("Given the choices, it seems like just accepting the facial would be the best option.  Urta nods, grinning lasciviously as she pulls the table to the side, and gestures for you to get on the floor.  You kneel before her as she pulls back her ", false);
		if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 11) MainScreen.text("skirt ", false);
		else MainScreen.text("stockings ", false);
		MainScreen.text("and allows her horse-dick to flop free in front of you.  It pulses, growing bigger with each beat of the fox's heart as she watches you submit yourself to her arousal.  You feel the eyes of everyone around you as they watch the spectacle ", false);
		if (player.stats.cor < 33) MainScreen.text("and it makes you blush crimson", false);
		else if (player.stats.cor < 66) MainScreen.text("and it makes you uncomfortable but horny", false);
		else MainScreen.text("and it makes you randy as hell", false);
		MainScreen.text(".\n\n", false);

		MainScreen.text("Two dexterous fur-covered hands wrap around her mottled shaft and begin pumping it.  Almost immediately the flattened-tip begins flaring wide and produces a dollop of fragrant equine pre-cum.  Urta's balls quiver happily as she plays with herself, and as you look up at her and open wide, ", false);
		if (player.stats.cor < 75) MainScreen.text("doing your best to play your part in the scene.", false);
		else MainScreen.text("hoping to encourage her to absolutely soak you with cum.", false);
		MainScreen.text("\n\nUrta looks down with a surprisingly shy expression and asks, \"<i>Could you... lick the tip please?  For me?</i>\"\n\n", false);

		if (player.stats.cor < 50) MainScreen.text("Hesitantly, you extend your tongue and lick the underside of her flat-tip.  ", false);
		else MainScreen.text("Eagerly, you lick around the outside edge of her flared tip, sticking your tongue out as far as you can to lick the underside once you finish.  ", false);
		...
		MainScreen.text("You swallow, tasting the sweet pre-cum as it slides down your throat.  Risking a glance to the side, you realize there's a crowd gathering around you, and more than a few dog-dicks and horse-cocks are being stroked.  ", false);
		if (player.stats.cor > 50) MainScreen.text("You hope the show will be arousing enough to get them off on you as well, and with renewed determination you turn back to Urta and give her balls a squeeze.", false);
		else MainScreen.text("You shudder in shame, and hope that the crowd will disperse. It's humiliating enough waiting to have Urta soak you with cum.", false);
		MainScreen.text("  She trembles and her tip flares wide, and you know she's starting to cum.\n\n", false);

		MainScreen.text("Her urethra dilates before your eyes and time seems to slow down as the first blob of horse-semen is launched towards you.  Blinking at the last second, you feel it splatter over your " + HeadDescriptor.describeFace(player) + ".  You open your mouth wider, keeping your eyes closed but knowing you're expected to swallow some of it.  Urta's aim improves with the next blast, and it does land in your mouth.  You nearly gag from the sudden salty intrusion, and due to the volume of the seed a fair portion of it leaks out the sides of your mouth as you keep it open, expecting more.  You feel more cum land in your hair, then on your cheek, and finally on your neck and ", false);
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) MainScreen.text("chest.  ", false);
		else MainScreen.text("cleavage.  ", false);
		MainScreen.text("Urta paints you with the stuff, until you're practically oozing cum.  It does eventually stop, and you wipe the jism from your eyes", false);
		if (player.stats.cor > 60) MainScreen.text(", being sure to lick your fingers clean afterwards", false);
		MainScreen.text(".\n\n", false);

		if (player.stats.cor > 50) {
			MainScreen.text("You could probably get the crowd to cover the rest of you.  Do you?", false);
			doYesNo(optionalUrtaBukkake, declineUrtaBukkake);
			return;
		}
		else {
			MainScreen.text("You blush and try to clean yourself up.  An embarrassed barmaid forces her way through the crowd to give you a towel, and starts shooing away the assembled masturbators.  You thank her and towel off Urta's thick seed, being more than a little aroused by the scene you caused.  The fox-girl is leaning back looking VERY satisfied as she ", false);
			if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11) MainScreen.text("stuffs herself back into her fishnet", false);
			else MainScreen.text("curls her tail back around her member, concealing it from view", false);
			MainScreen.text(".", false);
			dynStats("lus", 5 + rand(5) + player.stats.lib / 10);
		}
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private optionalUrtaBukkake(): void {
		urtaSprite();
		player.slimeFeed();
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] -= 2;
		urtaLove(-1);
		MainScreen.text("", true);
		MainScreen.text(images.showImage("urta-bukkake"), false);
		MainScreen.text("You pause to tear off your " + player.inventory.armor.displayName + " and expose the rest of your body, and you reach out into the crowd, grabbing the nearest cock and caressing it as you beg, \"<i>Come on and cum, she missed most of me!</i>\"\n\n", false);
		MainScreen.text("Urta leans back with a smile, toying with her softening member as a ring of horny studs and herms encircles you, jerking themselves off furiously.  You take turns touching and caressing them, murmuring enticing words as they start cumming on you, one or two at a time.  As those that finish step away, they're replaced by newcomers, turned on by the cum-hungry slut in front of them.  You do your best to greet them with a wink, a stroke, and a slurp, and then you present your ", false);
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text(BreastDescriptor.describeAllBreasts(player), false);
		else MainScreen.text(BreastDescriptor.describeNipple(0) + "s", false);
		MainScreen.text(" to a horny wolf just in time to take his hot load on your chest.\n\n", false);
		dynStats("lus", 15 + rand(5) + player.stats.lib / 10);
		MainScreen.text("An hour later you're totally drenched in cum.  It's dripping off you from your hair down to your " + LowerBodyDescriptor.describeFeet(player) + ", and you love it.  You lick up the mixed spooge until you start to feel full, and a blushing barmaid comes forward to hand you a towel.  You give her a salty kiss on the lips, sending her running away before you towel off and dress yourself.", false);
		doNext(camp.returnToCampUseOneHour);
	}
	...
	internal function goBackToUrtasForLuvinz(): void {
		urtaSprite();
		MainScreen.text("", true);
		MainScreen.text(images.showImage("urta-home-image"), false);
		if (Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) {
			MainScreen.text("Urta gives you a warm smile and grabs you by the hand, pulling you out the Wet Bitch's backdoor and into Tel'Adre's back-alleys.  Her dress does little to conceal the bulging mass underneath or the growing wet spot that forms at the tip of her impressive tent.  The long, body-hugging garment makes it difficult for Urta to walk in her current state, and you don't make it any easier for her, constantly teasing the fox as you navigate the back-alleys of Tel'Adre.\n\n", false);

			MainScreen.text("Every chance you get, you brush a hand or a hip against her sensitive nether-bulge or lust-tightened nipples.  For someone with such a bad-ass reputation, Urta's quite easy to tease, and she makes adorable gasps and shuddering shakes every time you brush against her flare or surprise her with a tender caress.  She lets out cute whimpers as she shuffles onward, but she doesn't protest as her 'male' half drenches her dress with more and more slimy pre-cum.\n\n", false);
			MainScreen.text("The door to her apartment looms before you and you push her inside.  You do your best to stifle an involuntary snicker as you close the door and see a trail of Urta's moist arousal on the street right up to the door.  The apartment is impeccably clean, though you see a few large sex-toys half-hidden about the place.  For someone as ashamed of her desires as Urta was, she sure is terrible at hiding her kinks.");
			if (Flags.list[FlagEnum.URTA_PETPLAY_DONE] == 0 && urtaLove()) MainScreen.text("  <b>A new, shiny leather collar lies half-concealed under a cushion, as though its owner haphazardly tried to bury it in a rush.</b>");
			MainScreen.text("  The busty fox has already retreated from the front-room, the half-soaked dress on the floor the only indication she was there with you a moment ago.\n\n", false);
			MainScreen.text("You follow a trail of pre-cum on the floor and the scent of copious female arousal deeper into the apartment, around a corner, and into Urta's relatively small bedroom.  Your vulpine lover is reclined on her bed, wearing nothing but lacy stockings and a latex cock-sheath.  ", false);
			if (player.lowerBody.cockSpot.count() > 0) MainScreen.text("In her hand is another sheath, ready to gird your own equipment in a fashion similar to her own.  ", false);
			doNext(urtaHomeLuvLuvinsMenu);
			return;
		}
		MainScreen.text("Urta smiles broadly and gives you a grateful peck on the cheek.  She makes like she's going to rise, then stirs uncomfortably, her eyes flashing down towards her crotch.  The fox settles back down and asks, \"<i>Would you mind paying off my tab while I slip out of here?  You've gotten me a bit too exposed to stand around trying to pay,</i>\" as she fishes out a coin-purse.  Tossing a few gems and odd coins your way, your over-endowed lover gives you a longer kiss before pushing you away from the table.\n\n", false);
		...
		MainScreen.text("You assure her that you aren't offended", false);
		if (player.stats.lib > 50 || player.stats.cor > 50) MainScreen.text(" and are actually turned on by the thought of walking through the city with such a 'big' lover hanging off your arm", false);
		MainScreen.text(".  Urta smiles weakly and hooks her arm through yours, leading you towards her apartment, making sure to stick to the more abandoned back-roads.  Her caution doesn't stop the pair of you from giving a few free shows, and if anything, each brief encounter with a passerby seems to be making her bigger and harder, revealing a half-buried exhibitionist streak.\n\n", false);
		goBackToUrtasForLuvinzII();
	}

	internal function goBackToUrtasForLuvinzII(): void {
		MainScreen.text("The journey is blessedly brief, and within minutes you arrive at Urta's house.  She fiddles with the lock anxiously for a moment before finally getting a -CLICK- and unlocking it.  You sweep inside behind your host and look around, mildly shocked at the spartan surroundings.  Urta's apartment is tiny, cramped, and nearly devoid of possessions beyond weapons, armor, and sex-toys.", false);
		if (Flags.list[FlagEnum.PC_SEEN_URTA_SEX_TOYS] == 0) {
			MainScreen.text("  The last group is the most numerous of those three.", false);
			if (kGAMECLASS.giacomo > 0) MainScreen.text("  You recognize some of the toys as Giacomo's wares, though there are a fair number that seem to be custom made, and others that are clearly living creatures.", false);
			MainScreen.text("  Looking over the things she had to use before she met you, it dawns on you just how lonely the poor fox has been.", false);
		}
		else MainScreen.text("  While the last group used to be the most numerous, it looks like Urta has cleaned house and only has a few of the toys left.  It seems that since meeting you she hasn't had to turn to her massive device collection like she used to.  It dawns on you just what a difference you've made in Urta's life, and it brings a loving smile to your face.", false);
		MainScreen.text("\n\n", false);

		MainScreen.text("\"<i>Sexy, are you going to come get some?</i>\" Urta calls out from the bedroom in the back.  You slap your forehead, angry for getting so distracted while a hot piece of ass is on the line.  Heading deeper into the apartment after her, you turn the corner to her bedroom and are floored by the sight.  Urta is totally nude save for a pair of black fishnet stockings and some kind of shining latex cock-sheath.  She's on all fours on top of a fur-lined bed, and wiggling her hind-end at you with her tail lifted in the air.  The scent of need pours off from her sex, and you find yourself as ", false);
		if (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.cockSpot.count() > 0) MainScreen.text("hard and wet as you've ever been", false);
		else if (player.lowerBody.cockSpot.count() > 0) MainScreen.text("hard as you've ever been", false);
		else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("wet as you've ever been", false);
		else MainScreen.text("ready as you've ever been to take such a monster", false);
		MainScreen.text(".\n\n", false);

		MainScreen.text("Urta is clearly looking to take a more passive role, and she begins stroking herself as she asks, \"<i>How do you want to fuck, lover?</i>\"", false);
		//[M: VAGINAL] [F: RIDE] [GENDERLESS/M: RIDE ANAL]
		let vaginal: Function = null;
		let femaleRides: Function = null;
		if (player.lowerBody.cockSpot.count() > 0) vaginal = dudeFuckingUrtasCooch;
		if (player.lowerBody.vaginaSpot.hasVagina()) femaleRides = rideUrtaTenderFemale;
		...
		menu();
		MainScreen.addButton(0, "Fuck Vagina", vaginal);
		MainScreen.addButton(1, "Ride(Vaginal)", femaleRides);
		MainScreen.addButton(2, "Ride(Anal)", tenderTakeItUpTheAssFromUrta);
		MainScreen.addButton(3, "No Condoms", condomlessUrtaInHouseSmex);
		if (player.lowerBody.isGoo() && player.skinType == SkinType.GOO) MainScreen.addButton(4, "Goo (Weird)", urtaGooTesticleVoreRuinedOrgasms);
	}

	private rideUrtaTenderFemale(): void {
		urtaSprite();
		Flags.list[FlagEnum.PC_SEEN_URTA_SEX_TOYS]++;
		urtaLove(1.5);
		Flags.list[FlagEnum.TIMES_FUCKED_URTA]++;
		Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY]++;
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 3 + rand(2);
		MainScreen.text("", true);
		MainScreen.text(images.showImage("urta-home-female-ride"), false);
		MainScreen.text("You climb into bed and playfully roll Urta over.  Her tits and horse-cock flop and jiggle from the change in position, and you watch mesmerized for a moment until Urta giggles girlishly, drawing you out of your reverie.\n\n", false);

		MainScreen.text("\"<i>Do you like them that much?</i>\" she asks.\n\n", false);

		MainScreen.text("You nod ", false);
		if (player.stats.cor < 50 || player.stats.lib < 50) MainScreen.text("sheepishly ", false);
		else MainScreen.text("eagerly ", false);
		MainScreen.text("and grab ahold of her latex-wrapped member, giving it gentle strokes along the underside.  Urta moans loudly and twitches her hips, already eager for more, and the ", false);
		if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < VaginaWetness.WET) MainScreen.text("growing moistness in your loins", false);
		else if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < VaginaWetness.DROOLING) MainScreen.text("sopping wetness in your loins", false);
		else MainScreen.text("flood of moisture leaking from your crotch", false);
		MainScreen.text(" signals your readiness to take her.", false);
		if (player.lowerBody.cockSpot.count() > 0) {
			if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("  Your " + CockDescriptor.describeMultiCockShort(player) + " is raging hard and trembling, but ignored for now.", false);
			else MainScreen.text("  Each of your " + CockDescriptor.describeMultiCockShort(player) + " is raging hard and trembling, but ignored for now.", false);
		}
		MainScreen.text("  Climbing atop the bed with her, you swivel your " + LowerBodyDescriptor.describeHips(player) + " about, positioning her at your entrance and straddling your " + LowerBodyDescriptor.describeLegs(player) + " around her.\n\n", false);

		MainScreen.text("You can see a bubble already forming at the tip of her hardness, signaling the unbridled lust your form has brought to your lover and you lower yourself with a sigh, feeling it spread your lips apart with delicious slowness as her flared tip begins penetrating you.  The horse-cock is so big and thick that it ", false);
		if (player.vaginalCapacity() < 60) {
			MainScreen.text("hurts quite a bit as it pushes further and further inside.  ", false);
			player.cuntChange(60, true);
		}
		else {
			MainScreen.text("stretches you nice and wide, rubbing you tightly even though you're quite stretched already.  ", false);
			player.cuntChange(60, true);
		}
		MainScreen.text("You gasp and sigh happily, sinking downwards slowly.\n\n", false);

		MainScreen.text("The ring of prepuce passes past your entrance ", false);
		if (player.vaginalCapacity() < 60) MainScreen.text("and you bottom out there, unable to take in any more of her equine shaft.", false);
		else MainScreen.text("and you keep going, sliding down along the marvelous member until your cunt-lips are rubbing her sheath against your [clit].", false);
		MainScreen.text("\n\n", false);

		MainScreen.text("Urta growls and grabs you by the back of the neck, pulling your " + HeadDescriptor.describeFace(player) + " down for a sloppy kiss.  Her smooth canine-like tongue forces its way into your mouth and curls around yours as the two of you swap spit.  Your hips, spurred on by the passion, begin to rise and fall to the tempo of the kiss, and in seconds you're both gasping and moaning against each other's lips, lost in the throes of passion.  Deep inside you, you can feel the condom filling with Urta's incredibly copious pre-cum, but every time your hips drop down the pressure squeezes some of it down along her length until it escapes around the condom's edge, dripping into her sheath.\n\n", false);

		//(Big Tits:
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) {
			MainScreen.text("Your " + BreastDescriptor.describeBreastRow(player.upperBody.chest.get(0)) + " mash into the fox's own, your " + BreastDescriptor.describeNipple(0) + "s ", false);
			if (!player.upperBody.chest.hasFuckableNipples()) MainScreen.text("rubbing over her black-skinned buds.", false);
			else MainScreen.text("parting as her prominent black nipples slip inside your own.", false);
			MainScreen.text("  You squeal at the sudden pleasure, arching your back and furthering the sensation from your over-burdened chest.", false);
			if (player.lowerBody.cockSpot.count() > 0) {
				if (player.lowerBody.cockSpot.get(0).cockLength > 15) {
					if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("  Your " + CockDescriptor.describeMultiCockShort(player) + " pushes up between the jiggling breasts, slicking them with your pre-cum as it begins to get tit-fucked by both of your trembling forms.", false);
					else MainScreen.text("  Each of your " + CockDescriptor.describeMultiCockShort(player) + " pushes up between the jiggling breasts, slicking them with your pre-cum as they begin to get tit-fucked by both of your trembling forms.", false);
				}
			}
			MainScreen.text("\n\n", false);
		}

		MainScreen.text("You both cry out in orgiastic pleasure as your shaking " + LowerBodyDescriptor.describeHips(player) + " force your tightly-stretched cunt over Urta's rubber-wrapped horse-prick again and again.  Your internal muscles spasm, clamping down tightly in sudden orgasm.  Urta's voice trembles in a low moan of satisfaction while her lower body twitches upwards with hard pumping motions.  Her dick trembles inside you, and you feel the latex around her girthy prick inflating with her seed and pressing against your cervix.\n\n", false);
		if (player.lowerBody.cockSpot.count() > 0) {
			if (player.lowerBody.cockSpot.count() == 1) {
				MainScreen.text("Your own " + CockDescriptor.describeMultiCockShort(player) + " cums as well, blasting seed over both your ", false);
				if (player.lowerBody.cockSpot.get(0).cockLength > 15 && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) MainScreen.text("jiggling tits", false);
				else MainScreen.text("bellies", false);
				MainScreen.text(", glazing them in slippery goo.", false);
			}
			else {
				MainScreen.text("Each of your own " + CockDescriptor.describeMultiCockShort(player) + " cums as well, blasting seed over both your ", false);
				if (player.lowerBody.cockSpot.get(0).cockLength > 15 && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) MainScreen.text("jiggling tits", false);
				else MainScreen.text("bellies", false);
				MainScreen.text(", glazing them in slippery goo.", false);
			}
		...
		MainScreen.text("Lying down on top of her, the two of you cuddle while joined at the hips, snuggling happily", false);
		if (player.lowerBody.cockSpot.count() > 0) {
			MainScreen.text(" and ignoring the hot, wet mess your " + CockDescriptor.describeMultiCockShort(player) + " ", false);
			if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("has", false);
			else MainScreen.text("have", false);
			MainScreen.text(" left between your nude bodies", false);
		}
		MainScreen.text(".  The fox-girl gives you a long, happy kiss and holds you tightly in her arms before she lets you go and begins to pull out.  The loud -SCHLLLIIIICK- of her fat member being dragged through your nethers makes you blush, and the feeling of it robs you of any strength below the waist.  The latex stretches, the cum-bubble still trapped inside you and stretching the condom obscenely for a moment, until it jerks out with a sudden 'pop'.  The relief is palpable, but your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " hangs open, with lips parted loosely after today's incredible violation.\n\n", false);

		MainScreen.text("Urta sighs and languidly pulls off her condom, nearly getting cum everywhere before she ties it off.  It looks like it'll pop at any moment – there's probably over a liter of cum stored in it.  She hefts it and mocks like she's going to throw it at you, but stops and giggles before setting it down beside the bed.  Your foxy lover says, ", false);
		if (Flags.list[FlagEnum.TIMES_FUCKED_URTA] <= 10 || Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 11)
			MainScreen.text("\"<i>Lovemaking is so much of a hassle to clean up if I don't use one of these!  At least I don't have to use them for the same reason as most of Tel'Adre.  It turns out I'm sterile or damn close to it, so all that fun-juice is just for show.</i>\"", false);
		else MainScreen.text("</i>\"<i>You're so good to me " + player.short + ".  I wish my little pony here was fertile enough for me to put a bun in your oven, if you wanted of course.</i>\"", false);
		...
		MainScreen.text("You nod before you leave, feeling satisfied and a warm glow from something other than lust.", false);
		//if(Flags.list[FlagEnum.URTA_FERTILE] == 1) player.knockUp(21,515,80);
		player.orgasm();
		player.stats.sens += -2;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private dudeFuckingUrtasCooch(): void {
		...
		MainScreen.text("", true);
		MainScreen.text(images.showImage("urta-home-male-fuck"), false);
		let cocks: number = player.lowerBody.cockSpot.count();
		Flags.list[FlagEnum.PC_SEEN_URTA_SEX_TOYS]++;
		Flags.list[FlagEnum.TIMES_FUCKED_URTA]++;
		urtaLove(1.5);
		Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY]--;
		if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 1) Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] = 1;
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 2;

		MainScreen.text("You let her know that you want her just the way she is and climb into bed behind her.  She watches you apprehensively over her shoulder as you pull out your " + CockDescriptor.describeMultiCockShort(player) + " and start lining yourself up to fuck her doggie-style.  ", false);
		if (cocks > 1 && y >= 0) MainScreen.text("Your secondary " + CockDescriptor.describeCock(player, y) + " wobbles directly behind her back-door and Urta's eyes widen a bit when she realizes you're going to double-penetrate her.  ", false);
		if (cocks > 2) MainScreen.text("The remainder of your " + CockDescriptor.describeMultiCockShort(player) + " fan out, ready to rub themselves against her butt-cheeks once you push inside.  ", false);
		MainScreen.text("You can see your lover's condom starting to bloat at the tip as her horse-cock begins to flare within its confinement.  She trembles with anticipation before taking a deep breath and saying, \"<i>I'm ready... take me!</i>\"\n\n", false);

		MainScreen.text("Obligingly, you ease your hips forward, reeling at the heat of her cunt-lips as they welcome your " + CockDescriptor.describeCock(player, 0) + "'s " + player.cockHead() + " with slippery moisture.  ", false);
		if (cocks > 1 && y >= 0) MainScreen.text("Your second " + CockDescriptor.nounCock(player.lowerBody.cockSpot.get(y).cockType) + " soon bumps against her pucker and strains against it, pushing ever-so-slowly into her tight hole as she gradually relaxes before you.  ", false);
		MainScreen.text("As you carefully breach the opening", false);
		if (cocks > 1 && y >= 0) MainScreen.text("s", false);
		MainScreen.text(", the going gets easier and easier for you.  You gently rock your [hips] forwards, sliding deep inside her until she's taken you to the hilt.  Once you're fully embedded within her, Urta wriggles happily beneath you, and you reach around to caress her drooping member, relishing in her pleasured yelps and moans.\n\n", false);

		MainScreen.text("You rock back and forth, picking up the tempo gradually.  At first the gray fox is only gasping happily, but as you go faster and faster, her gasps turn into quiet moans, then transform into ecstatic squeals.  ", false);
		if (player.lowerBody.balls > 0) MainScreen.text("Your balls loudly smack into her squelching-wet cunt and the soaked backside of HER balls", false);
		else MainScreen.text("Your crotch slaps loudly against her squelching-wet cunt and backside", false);
		MainScreen.text(" as you fuck her hard and fast.  Urta's cock twitches in your hand, and the pre-filled bubble hanging from her cock-tip is getting close to apple-sized.  Both of you are rutting like animals and riding on the edge of an orgasm.  Then, Urta throws her head up, arches her back, and starts howling like a well ridden bitch in heat.  Now that she's moving with you, her magnificent horse-cock fully engorges inside your grasping hand, and you slide it between your grasping fingers, masturbating her even while fucking her.\n\n", false);

		MainScreen.text("Urta cums first.  Her cries of pleasure are muffled as she shoves her face into a pillow, screaming in ecstasy.  The warmth around your " + CockDescriptor.describeCock(player, x) + " seems to grow that much hotter as it squeezes you, wringing you tightly from root to tip.  ", false);
		if (cocks > 1 && y >= 0) MainScreen.text("The intensity and pressure in her back-door is even worse, and your " + CockDescriptor.describeCock(player, y) + " is nearly in pain from her asshole's tight, pulsating grip.  ", false);
		MainScreen.text("Her equine member trembles in your grasp as it releases its sticky payload into the thick, latex condom.  You pump away at her shaft, feeling the spunk inside distort the tube-like appendage until your orgasm shocks you into releasing it.\n\n", false);

		MainScreen.text("You spurt into Urta's sloppy box, creaming it until her juices begin to run in an off-white color.  ", false);
		if (cocks > 1 && y >= 0) MainScreen.text("The " + CockDescriptor.describeCock(player, y) + " you've crammed in her ass does not disappoint either, matching its brother squirt for squirt as it spends your seed with reckless abandon.  ", false);
		if (cocks > 2) MainScreen.text("Her butt and tail are covered with the stuff as each of your remaining " + CockDescriptor.describeMultiCockShort(player) + " splatters her with spooge, covering her fur in white sludge.  ", false);
		MainScreen.text("The fox's internal muscles happily continue to squeeze and milk you, prolonging your orgasm beyond its normal duration.", false);
		if (player.cumQ() >= 750) MainScreen.text("  Her body is quickly bloated thanks to your body's incredible fertility, but she seems able to take it just fine.", false);
		MainScreen.text("\n\n", false);

		MainScreen.text("The two of you collapse together in a shuddering pile of post-orgasmic bliss.  The shift in position frees your " + CockDescriptor.describeMultiCockShort(player) + " from Urta with a noisy -SHHHLLUUUCK-, but you're too wiped out to care.  The fox-girl sighs happily and rolls over, planting a kiss on your nose before she works at removing her over-filled condom.  It nearly bursts when she ties it off, but she manages to gently set it down to the floor without incident. She looks at you happily and gives you a big hug, crushing her hard nipples against ", false);
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text("your own " + BreastDescriptor.describeBreastRow(player.upperBody.chest.get(0)), false);
		else MainScreen.text("your chest", false);
		MainScreen.text(" as she says, \"<i>Thank you so much, lover.  I forget how good it can be in my... vagina.  Maybe we could do this again?</i>\"\n\n", false);

		MainScreen.text("You give her a happy nod, and the two of you cuddle a bit longer before you leave to return to camp.", false);
		player.orgasm();
		player.stats.sens += -2;
		knockUpUrtaChance();
		doNext(camp.returnToCampUseOneHour);
	...
	private fuckItAndLeave(): void {
		...
		Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY]--;
		if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 1) Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] = 1;
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 36;
		urtaLove(-5);
		MainScreen.text("", true);
		MainScreen.text("You let Urta know that you've changed your mind and will be leaving.  You go before she can object and the sound of quiet sobbing chases you out into the streets.", false);
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private tenderTakeItUpTheAssFromUrta(): void {
		...
		Flags.list[FlagEnum.TIMES_FUCKED_URTA]++;
		Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY]++;
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 2 + rand(3);
		let cocks: number = player.lowerBody.cockSpot.count();
		MainScreen.text(images.showImage("urta-home-anal"), false);
		MainScreen.text("You give Urta's backside a playful slap and ask her to roll over.  She does so with a hesitant look on her face, wondering what you're about.  When you spread her legs and work your way forward to straddle her, a beaming smile breaks across her face and she says, \"<i>You really want me inside you, don't you " + player.short + "?</i>\"\n\n", false);

		MainScreen.text("Her cheeks color when you give her a nod and begin lowering your " + ButtDescriptor.describeButt(player) + " towards her pillar of horse-flesh. She gasps quietly when the tip begins to push against your " + ButtDescriptor.describeButthole(player) + ".  ", false);
		if (player.analCapacity() >= 60) MainScreen.text("You sigh and begin sliding down it, your previously-stretched backdoor easily accepting the gigantic intruder as it fills you.", false);
		else {
			MainScreen.text("You grunt and begin sliding down it, feeling painfully stretched by the gigantic intruder but pressing on anyway, knowing full-well you'll be a bit looser by the time you take her whole length.", false);
			if (player.lowerBody.butt.analLooseness < 5) {
				if (player.lowerBody.butt.analLooseness == 0) MainScreen.text("<b>  Urta has taken your anal virginity.</b>", false);
				else MainScreen.text("<b>  Your ass is stretched.</b>", false);
				player.lowerBody.butt.analLooseness++;
			}
		}
		//Butt Stretched used to determine how long since last enlargement
		if (!player.statusAffects.has("ButtStretched")) player.statusAffects.add(new StatusAffect("ButtStretched", 0, 0, 0, 0)));
	//Reset the timer on it to 0 when restretched.
	else player.changeStatusValue(StatusAffects.ButtStretched, 1, 0);
		if (player.lowerBody.cockSpot.count() > 0) {
			MainScreen.text("  The bumps and veins keep rubbing against your prostate, and ", false);
			if (cocks > 1) MainScreen.text("each of ", false);
			MainScreen.text("your " + CockDescriptor.describeMultiCockShort(player) + " hardens near instantly from the forced stimulation.", false);
		}
		MainScreen.text("  Slowly but surely, it slips further and further inside you, until you've bottomed out and have miraculously taken her entire shaft.\n\n", false);


		//(IF DICKS)
		if (cocks > 0) {
			MainScreen.text("For a few moments you sit on her deeply penetrating thickness, and ", false);
			if (cocks > 1) MainScreen.text("each of ", false);
			MainScreen.text("your " + CockDescriptor.describeMultiCockShort(player) + " droops onto her belly, half-hard from the pressure on your prostate, but going a little soft from the lack of attention.", false);
			if (player.lowerBody.cockSpot.get(0).cockLength >= 15) {
				MainScreen.text("  Urta winks and guides ", false);
				if (cocks > 1) MainScreen.text("one of ", false);
				MainScreen.text("your " + CockDescriptor.describeMultiCockShort(player) + " between her tits", false);
				if (player.lowerBody.cockSpot.get(0).cockLength >= 20 && player.lowerBody.cockSpot.get(0).cockLength <= 28) MainScreen.text(", even taking time to lick and suck on the head", false);
				MainScreen.text(".", false);
				MainScreen.text("  She begins giving you a slow tit-fuck, trying to encourage you to pick up the pace and start riding her.  It does the trick as your " + LowerBodyDescriptor.describeHips(player) + " start rocking on their own in reaction to her efforts.  She licks harder, and you reward her by lifting yourself up and sliding back down.  Her massive girth squeezes a squirt of pre-cum from your prostate with every few strokes, splattering it on her muzzle.", false);
			}
			MainScreen.text("\n\n", false);
		}
		else {
			MainScreen.text("For a few moments you sit like that, trying to adjust to the pressure in your backside.  After a few moments you can feel your muscles relaxing, allowing Urta to rest more comfortably inside you.  Flexing your " + LowerBodyDescriptor.describeLegs(player) + ", you pull yourself up off of her until the flared tip is just barely inside you, then you plunge back down.  Urta's mouth hangs open and her tongue lolls out, and you can feel the warmth of her leaking fluid inside of you, letting you know just how good your " + ButtDescriptor.describeButthole(player) + " feels to her.\n\n", false);
		}
		MainScreen.text("You bounce on top of your fox-lover's equine rod, spearing your " + ButtDescriptor.describeButthole(player) + " on it over and over.  ", false);
		if (cocks == 0) MainScreen.text("Strange pleasure spreads from your sensitive opening, and you know that somehow you're going to cum from this.  ", false);
		else MainScreen.text("Powerful pleasure spreads through you as it bumps and rubs your prostate. You know you'll cum from this, and soon.  ", false);
		MainScreen.text("Urta cries out and grabs you by the hips, yanking you down against her sheath.  Warmth blossoms inside you as her seed begins pumping into your bowels.  You can feel yourself filling up with her seed and you cum, clamping down around her intruder as it injects more and more foxy-horse-spunk into you.\n\n", false);

		//(DICKS)
		if (cocks > 0) {
			if (player.lowerBody.cockSpot.get(0).cockLength < 15) MainScreen.text("Your orgasm erupts out onto her belly, splattering her fur with your sticky cum.  ", false);
			else if (player.lowerBody.cockSpot.get(0).cockLength < 20) MainScreen.text("Your orgasm erupts all over her tits, slicking them with your sticky white cum.  ", false);
			else MainScreen.text("Your orgasm erupts into her muzzle, filling her mouth with your sticky, white cum.  ", false);
			MainScreen.text("  Urta groans underneath you and begins caressing and squeezing you, milking you of every drop.", false);
			if (player.cumQ() >= 500) MainScreen.text("  Urta gulps down as much as she can but it begins leaking around her muzzle, and she eventually lets it pop free, giving up on swallowing it entirely.", false);
			if (player.cumQ() >= 1000 && player.cumQ() < 1500) MainScreen.text("  Your output easily soaks her, and she gathers it up and slops it over her nipples, pinching and pulling on them.", false);
			if (player.cumQ() >= 1500) MainScreen.text("  Your output soaks her and the bed with seed, leaving the two of you in a warm gooey puddle in the very center of the bed.", false);
			MainScreen.text("\n\n", false);
		}
		MainScreen.text("Urta pulls you down onto her breasts and gives you a long hug and a happy lick.\n\n", false);

		MainScreen.text("\"<i>Thank you soooo much lover, I needed that,</i>\" she says, \"<i>Now you better go get back to your business, I've got to get cleaned up!", false);
		if (player.cumQ() > 750) MainScreen.text("  You really are a messy, " + player.mf("boy", "girl") + " aren't you?", false);
		MainScreen.text("</i>\"\n\n", false);
		MainScreen.text("She untangles herself from you, the filled end of her condom loudly popping free from your backside as she disengages herself from you.  The two of you do your best to clean up and get dressed, and you give her a quick but passionate kiss before ducking out the door to check up on your camp.", false);
		player.orgasm();
		player.stats.sens += -2;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private marbleCockuBlockuUrta(): void {
		urtaSprite();
		MainScreen.text("", true);
		MainScreen.text("Urta gives you a serious look when you come up to her, \"<i>I just got a report in from some of our scouts last night.  It looks like they found your camp.  There was a cow-girl there", false);
		//if nursery
		if (Flags.list[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] == 100) MainScreen.text(" and a nursery", false);
		MainScreen.text(".", false);
		//(Not wuv'ed yet)
		if (Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] == 0) {
			MainScreen.text("  I know we aren't a couple or anything, but, is she something to you?  Are you lovers?</i>\"\n\n", false);
			MainScreen.text("Urta's cheeks color sheepishly and she says, \"<i>You don't have to answer, but whatever the truth is, you can tell me.</i>\"\n\n", false);
		}
		//(WUVED)
		else {
			MainScreen.text("  I know our paths keep us apart most of the time... and multiple lovers are pretty far from uncommon, but I need to know – do you two have something going on?</i>\"\n\n", false);
			MainScreen.text("Urta's looking at you nervously, clearly dreading your answer.  How do you respond?\n\n", false);
		}
		MainScreen.text("Do you answer honestly, refuse to answer, or lie?", false);
		//[Honestly] [Refuse] [Lie]
	...
	private TellUrtaDAHTRUUUUF(): void {
		urtaSprite();
		MainScreen.text("", true);
		MainScreen.text("You explain to Urta about how you became addicted to Marble's milk at the farm, and how despite the harshness of the untamed lands around the both of you, you formed a loving relationship.  Hearing the truth and honesty backing your words, Urta's eyes water, but the saddened fox-girl refuses to look away.  Once you've finished, she dabs away a tear and takes a swig of what you hope is ale.  She wipes the foam from her muzzle with the tear-stained rag and speaks with a husky, grief-laced voice, \"<i>I... appreciate your honesty.  Some part of me was afraid you would try to lie.  Yes, I'm a little disappointed I'm not the only one to warm your bed, but telling me the truth was the right thing to do, and ", false);
		if (Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) MainScreen.text("I love you for it", false);
		else MainScreen.text("I respect you for it", false);
		MainScreen.text(".");
		if (!urtaOpenAboutEdryn()) {
			MainScreen.text("  I suppose while we're clearing out skeletons, I should mention I've had relations with Edryn a few times since we've ", false);
			if (Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) MainScreen.text("been together", false);
			else MainScreen.text("gotten intimate", false);
			MainScreen.text(".  I'm ashamed to admit I can't really control myself that well either.</i>\"\n\n", false);
			MainScreen.text("Do you comfort her, tell her to toughen up and remind her she's great in the sack, or walk out in a huff?", false);
	...
	private comfortUrtaAfterTellinTruuf(): void {
		urtaSprite();
		MainScreen.text("", true);
		MainScreen.text("You let Urta know that you understand.  After all, in a world like this monogamy is next to impossible.  There's no shame in what she did with Edryn or your relationship with Marble.  The two of you share a heartfelt hug and resolve to continue your relationship.", false);
		//(If PC has Edryn (<3) & fits her req's)
		if (((player.lowerBody == LowerBodyType.CENTAUR && player.lowerBody.cockSpot.count() > 0) ||
			player.lowerBody.cockSpot.countType(CockType.HORSE) > 0 ||
			(player.lowerBody.cockSpot.count() > 0 && player.stats.cor > 50)) &&
			player.statusAffects.get("Edryn").value1 >= 4)
			MainScreen.text("  Urta even suggests that the two of you take turns at Edryn sometime.", false);
		//Slight love loss
		urtaLove(-4);
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private toughenUpUrtaAfterTellingTruuf(): void {
		urtaSprite();
		MainScreen.text("", true);
		MainScreen.text("You look at Urta and explain that she shouldn't take it too hard.  Both of you got into this scenario without expecting anything more from it.  You explain that though she is a good friend and GREAT in the sack, you don't see your relationship going to that level.  Urta seems ", false);
		if (Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) MainScreen.text("heartbroken", false);
		else MainScreen.text("saddened", false);
		MainScreen.text(" by the news", false);
		if (Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] == 1) {
			MainScreen.text(", and she has a defeated look in her eyes.", false);
			//(Luv set to 0 and blocked if in luv,
			Flags.list[FlagEnum.URTA_PC_AFFECTION_COUNTER] = 0;
			Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] = -1;
		}
		else {
			MainScreen.text(", but you spy a determined look in her eyes.", false);
	...
	private walkoutOnUrtaAfterTellingTruff(): void {
		urtaSprite();
		MainScreen.text("", true);
		MainScreen.text("You push away from the table and turn your back on Urta, enraged at her for cheating on you with Edryn, even though you cheated on her as well.  You ignore the hypocrisy in your actions and walk out, shattering Urta's heart in the process.  As you close the door of 'The Wet Bitch' behind you, you see Urta, sobbing into a hanky while Edryn goes over to comfort her.  It figures.", false);
	...
	private lietoUrtaAboutMarble(): void {
		urtaSprite();
		MainScreen.text("", true);
		MainScreen.text("You let Urta know that the poor cow-girl needed a place to stay from the monsters that roam the wilds, and that your camp is the safest place.  In great detail you talk about sheltering her, but never taking advantage of her, and trying your best to do what's right.  Through the whole conversation Urta dabs at tears, and you assume them to be tears of relief.  The story suitably complete and air-tight, you finish and look at Urta.\n\n", false);
		MainScreen.text("\"<i>I misjudged you, " + player.short + ".  I didn't think you'd lie to me, and you did.  The scouts talked to her", false);
		if (Flags.list[FlagEnum.MARBLE_KIDS] == 1) MainScreen.text(" and saw the kid", false);
		if (Flags.list[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] > 1) MainScreen.text(" and saw the kids", false);
		MainScreen.text("!  I know you two are an item, and I can't believe you didn't own up to it!</i>\" she cries, \"<i>You better believe I'm fucking pissed at you.  I could've accepted sharing you, but lying to me like this...I won't ever trust you again.  Just...just go – I might be able to overlook this but it hurts too much right now.</i>\"\n\n", false);
		MainScreen.text("There's nothing to do but go.  You're not sure if Urta will ever let you see her again.", false);
		//(Urta pissed for 48 hours, then make-up scene.  Love forever blocked)
		Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] = -1;
		Flags.list[FlagEnum.URTA_PC_AFFECTION_COUNTER] = 0;
		Flags.list[FlagEnum.URTA_ANGRY_AT_PC_COUNTDOWN] = 48;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private UrtaTwuWuvOffer(): void {
		urtaSprite();
		MainScreen.text("", true);
		MainScreen.text("Urta looks nervously at you, fidgeting and shifting her sizable addition under the table.  She starts talking meekly, but her voice rises in confidence and intensity as she goes, \"<i>I've been thinking " + player.short + ", we've known each other for a while now.  When we're together, I don't have to worry about someone seeing something they shouldn't, or being made fun of, because it doesn't bother you and you don't care.  And what do I have to care about if you don't care about it?</i>\"\n\n", false);
		MainScreen.text("She leans over and puts her hands on your shoulders, looking you in the eye as she continues, \"<i>I've never felt this comfortable around ANYONE before, and well, I think I love you.  Please, do you feel the same way about me?</i>\"\n\n", false);
		MainScreen.text("Given how much time you've spent with her, and the amazing sex, her declaration doesn't really surprise you.  Do you love her back?", false);
		//[Yes] [No]
	...
	private noUDontLoveUrta(): void {
		...
		Flags.list[FlagEnum.URTA_PC_AFFECTION_COUNTER] = 0;
		Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] = -1;
		MainScreen.text("You gently remove her hands from your shoulders and let Urta know that you enjoy being around her, but that you aren't in love with her.  Her eyes choke up with tears, but she nods as you explain, wiping her eyes with a napkin.  Urta cries softly and murmurs, \"<i>...I didn't want to hear that, but I understand.  Do you want to keep on as we have been though?</i>\"\n\n", false);
		//[Yes] [No]
	...
	private stayFuckbuddiesAfterShootingDown(): void {
		urtaSprite();
		MainScreen.text("Urta gives you a warm smile and a kiss on the cheek, \"<i>Good.  Come see me soon, ok?</i>\"\n\n", false);
		//(READY TO GO NEXT TIEM)
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 0;
		doNext(camp.returnToCampUseOneHour);
	}
	//[no]
	private turnDownFuckbuddiesANDLove(): void {
		urtaSprite();
		MainScreen.text("", true);
		MainScreen.text("Urta bursts into tears and pushes back from the table.  She runs out of the bar, sobbing wildly.  It looks like you've totally burned that bridge.  You may as well head back to camp...", false);
		//(BURN BRIDGES MOFO)
		Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] = -1;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	public eatUrtaOutNomNomPussy(): void {
		...
		MainScreen.text("The click of heels on wood approaches, but you ignore them and step up your efforts, hoping to embarrass Urta in front of the newcomer.  A faint voice asks, \"<i>Are you O.K. Ma'am?</i>\"\n\n", false);

		MainScreen.text("Urta's thighs clamp down harder, holding you in place as she replies, \"<i>Just ah-ah little thirsty...could you bring me ahhhnother drink?</i>\"\n\n", false);

		MainScreen.text("\"<i>Um sure thing miss, just give me a moment,</i>\" blurts the other voice as it hastily retreats into the bar's background noise.  A burst of light nearly blinds you and then a hand is wrapped around your head, yanking you forwards.  Her musky box crushes against your face, filling the air under the table with lewd squishing noises as she grinds her hips into you.  You don't even slow down, and every chance you get you swirl your tongue around the hard bud of her clit, pausing to suckle it before her forceful face-fucking mashes your lips into another part of her sex.\n\n", false);

		MainScreen.text("Sucking, licking, slurping, you judge the successfulness of your oral efforts by just how much fluid you can get to soak into your " + HeadDescriptor.describeFace(player) + ".  The bar's noises are completely indecipherable, muffled as they are by the furred thighs clamped around your head and blocking your ears.  The fragrant musk of her box fills every breath you take as you eat her out in a frenzy, tonguing her with wild desire and need as your world narrows to the gash between her legs.\n\n", false);

		MainScreen.text("Eventually, the vice-like pressure around your head releases, and you stagger back, blinking in confusion and allowing her balls to drop back into their normal place.  Her cock quivers on your " + HeadDescriptor.describeHair(player) + ", depositing a thick stream of warmth down the back of your head.  Her thighs are twitching and her balls pulsating as she cums, unloading a thick batch of herm-cream down your back.  You jerk to the side and stroke her as she unloads, reaching around to the top of the table and grabbing an empty glass.  Guiding the flared tip into the receptacle, you allow the unknowing hermaphrodite to fill the glass with her seed.\n\n", false);

		MainScreen.text("Finished at last, Urta's member rapidly deflates, the scent of her cum on your " + HeadDescriptor.describeHair(player) + " and her juice on your " + HeadDescriptor.describeFace(player) + " lingers.  You slide up into the booth next to your blissed out lover.", false);
	...
	public blowUrtaUnderTheTableLuv(): void {
		...
		MainScreen.text("Deciding you've got to get her off and make her spill her drink, you ", false);
		if (player.stats.cor < 50) MainScreen.text("do your best to suppress your gag reflex and shove forwards, ramming her cock as deeply into your throat as you can.", false);
		else MainScreen.text("shove forwards, ramming her massive cock so far down your throat that you can feel it dribbling pre-cum into your belly.", false);
		MainScreen.text("  Urta grunts, doing her best to muffle the noise as it escapes her lips, but you can feel her twitch and thicken inside you, beginning to drip with proof of her complete and total submission to your mouth.\n\n", false);

		MainScreen.text("You can hear chuckles from the bar's other occupants as they realize what's going on, and you blush, ", false);
		if (player.stats.cor < 50 && Flags.list[FlagEnum.PC_FETISH] <= 0) MainScreen.text("wishing there was a tablecloth to hide your activities from prying eyes", false);
		else MainScreen.text("wishing you had even more eyes watching your show", false);
		MainScreen.text(".  The distraction does little to divert you from your chosen task, and you close your eyes, tasting your lover's throbbing shaft as it fills your mouth.  With both hands stroking, you bob up and down, completely centering your world around the pulsating horse-prick as it fills your mouth.\n\n", false);
		...
		MainScreen.text("Thumping down on your back, you're blissfully aware of the sweet air filling your lungs.  In a moment of reflection, you realize you can taste the salty flavor of Urta's seed on your lips and mouth, and the retracting horse-cock is ", false);
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text("dragging between your " + BreastDescriptor.describeAllBreasts(player), false);
		else MainScreen.text("dragging across your chest", false);
		MainScreen.text(" as it begins to soften.  You wipe as much of her leavings from your " + HeadDescriptor.describeFace(player) + " as you can, licking the slippery glaze from your lips.  Before you get up, you make sure to give Urta's member a firm squeeze, scolding it for its forceful treatment of your mouth.\n\n", false);
		MainScreen.text("Once you've climbed out from under the table, you're VERY aware of the eyes of some of the nearby bar patron's on your back.  It seems your undercover act managed to draw more than a little attention.  Urta's cheeks burn bright-red under her gray fur, though she has a loving smile on her face.  The pair of you share a tender kiss to hoots and catcalls from the audience.  Urta smiles and gushes, \"<i>You give the BEST blowjobs... I mean, WOW, but maybe next time we should go back to my place and do something a little more... fulfilling for both of us.</i>\"", false);
		doNext(camp.returnToCampUseOneHour);
		Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY]++;
	...
	private rideUrtasCoochLoveyDovey(): void {
		urtaSprite();
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 2 + rand(2);
		urtaLove(1);
		MainScreen.text("", true);
		//(TOO SMALL)
		if (player.vaginalCapacity() < 40) {
			MainScreen.text("Urta smiles at first, but as you strip down and get ready her expression turns to one of care and worry.  She apologizes, \"<i>I'm sorry sweetheart, but I don't think you could take me like that without some serious pain.  Maybe after you've loosened up that cute little snatch with some sex-toys I could give it a go... by Marae I want this, but you aren't ready for my... my thing.</i>\"\n\n", false);
			doNext(urtaHomeLuvLuvinsMenu);
			return;
		}
		//(SHIT FITS)
		MainScreen.text(images.showImage("urta-home-female-fuck"), false);
		MainScreen.text("Urta bursts into a wide smile at your suggestion and scooches over, making room for you on the bed.  ", false);
		if (player.lowerBody.cockSpot.count() > 0) {
			MainScreen.text("She tosses you ", false);
			if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("a latex sheath", false);
			else MainScreen.text("some latex sheaths", false);
			MainScreen.text(" to cover your " + CockDescriptor.describeMultiCockShort(player) + " with, explaining that she doesn't want to have to mop up afterward, and you grudgingly agree.  ", false);
		}
		...
		MainScreen.text("Her cries are muted as you launch yourself over her, brutalizing her eager lips with a ferocious kiss.  You roughly tongue-fuck her as you adjust yourself, balancing yourself with one hand while your other guides her trembling shaft closer and closer to the entrance of your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".  You sigh happily and rub the pre-filled bulb against your opening, ", false);
		if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < VaginaWetness.WET) MainScreen.text("growing moist", false);
		else if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < VaginaWetness.DROOLING) MainScreen.text("growing wetter and wetter as you lubricate the tip of her condom", false);
		else MainScreen.text("soaking her condom-covered cock with your copious cunt-secretions", false);
		MainScreen.text(".  Urta's body goes limp under your sexual assault, totally caving in to the feelings you're stoking inside her.  The only thing she manages to keep up is the twisting of her hips as she tries to enhance the sensations radiating through her massive, flared prick.\n\n", false);

		MainScreen.text("You end the kiss by pulling back, sucking her lower lip into your mouth, and then releasing it with a pop.  Nearly crying with need, your horse-endowed lover begs, \"<i>Ah, please don't stop!</i>\"\n\n", false);

		MainScreen.text("Never one to disappoint, you press onwards and downwards, sliding deeper into the bed as you work the wide, flared tip against yourself.  It slowly stretches you wide and bows under your weight, though Urta doesn't voice a word of complaint or displeasure.  You swivel slightly to the side and it works its way inside you, one half of the tip sliding inside at a time.  The fox's mouth opens into an 'O' of pleasure, letting her tongue escape as she begins panting from the pleasure.\n\n", false);

		MainScreen.text("Sinking down slowly and relishing the feeling of your passage being spread, you allow a low moan to escape your mouth.  Urta's hands lock onto her breasts and begin alternatively squeezing her soft mounds and fondling the black skin of her nipples.  The sensation of slow penetration is exquisite, growing within you while ever thicker horse-cock spears your womb.  You can feel the beat of Urta's heart racing through the shaft, swelling it slightly in time with each heartbeat.\n\n", false);

		MainScreen.text("You bottom-out, but immediately begin rocking your hips, working your lover's twitching rod deep inside your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".  ", false);
		//(LOOSENESS MESSAGES GO HERE AS APPROPRIATE)
		player.cuntChange(60, true);
		MainScreen.text("Urta's hands grab you firmly by the " + ButtDescriptor.describeButt(player) + ", reminding you just how strong the lithe fox is.  She begins working with you to lift your body up and slam it back down against her, alternating the feelings of emptiness and fullness within you.  Each time the little balloon that's formed on her flare gets larger, squishing each time she bottoms out.  As the pair of you rut faster and faster, the limited space inside you squeezes the excess pre-cum from the sheath, causing it to drip over her balls into the sheets.\n\n", false);

		MainScreen.text("Bending over, you plant another kiss full on her lips, and your vulpine lover just can't restrain herself anymore.  She moans into your mouth, her body shivering, her balls tightening up until you feel them press against your ass.  Her hands grip you hard and pull you down with incredible force, locking you against her as you feel her member swelling inside you, submitting to your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + "'s swelteringly hot embrace.  A burst of tightness fills you, growing as Urta squirms, unloading her massive horse-pole's seed into the latex balloon she's stuffed inside you.\n\n", false);

		MainScreen.text("The filling seed balloon stretches tight inside of you, ", false);
		if (player.vaginalCapacity() < 80) MainScreen.text("until there is no more room for cum and it's squirting out around the base of her cock, soaking Urta and the bed with fragrant horse-cum.", false);
		else MainScreen.text("filling your body with a large bubble of hot horse-cum.", false);
		MainScreen.text("  Watching your lover submit to your body's pleasure, the sight pushes you over the deep edge with her, and your body begins milking the thick shaft inside you, clenching around it as you spasm happily.  ", false);
		if (player.lowerBody.cockSpot.count() > 0) {
			if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("Y", false);
			else MainScreen.text("Each of y", false);
			MainScreen.text("our " + CockDescriptor.describeMultiCockShort(player) + " spurts, unloading waves of cream into the ", false);
			if (player.lowerBody.cockSpot.count() == 1) {
				MainScreen.text("condom Urta asked you to wear around it.", false);
				if (player.cumQ() >= 250) {
					if (player.cumQ() < 500) MainScreen.text("  The balloon inflates slightly, drooping down as your maleness empties its reserves.  ", false);
					else if (player.cumQ() < 1000) MainScreen.text("  The balloon swells larger and larger as you pump it full, eventually wobbling around on Urta's belly as you finish filling it.", false);
					else {
						MainScreen.text("  The balloon swells larger and larger as you pump it full, wobbling obscenely on Urta's belly and slowly drooping down onto the sheets as the sheath reaches its maximum capacity.", false);
						if (player.cumQ() > 1500) MainScreen.text("  Your potent seed pops the condom, splattering Urta with your seed as it explosively fails.", false);
					}
				}
			}
			else {
				MainScreen.text("condoms Urta asked you to wear around them.", false);
				if (player.cumQ() >= 250) {
					if (player.cumQ() < 500) MainScreen.text("  The balloons inflate slightly, drooping down as your malenesses empty their reserves.  ", false);
					else if (player.cumQ() < 1000) MainScreen.text("  The balloons swell larger and larger as you pump them full, eventually wobbling around on Urta's belly as you finish filling them.", false);
					else {
						MainScreen.text("  The balloons swell larger and larger as you pump them full, wobbling obscenely on Urta's belly and slowly drooping down onto the sheets as the sheaths reach their maximum capacity.", false);
						if (player.cumQ() > 1500) MainScreen.text("  Your potent seed pops the condoms, splattering Urta with your seed as they explosively fail.", false);
					}
				}
			}
		}
	...
	private rideUrtaInButtAtHomeLove(): void {
		...
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 2 + rand(2);
		MainScreen.text("", true);
		MainScreen.text(images.showImage("urta-home-anal-urta"), false);
		MainScreen.text("Urta raises an eyebrow at the suggestion and blushes, though her horse-like member thickens and stiffens noticeably.  You give it a gentle slap and tease, \"<i>Don't act like this isn't what you wanted!</i>\"\n\n", false);

		MainScreen.text("She squirms and smiles nervously, \"<i>It feels too good, even though it's dirty!  Besides, your sweet ass is irresistible!</i>\"\n\n", false);

		if (player.lowerBody.cockSpot.count() > 0) {
			MainScreen.text("Even though you're going to be riding her, she insists you wear ", false);
			if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("a condom", false);
			else MainScreen.text("some condoms", false);
			MainScreen.text(" to help control the mess.  You grudgingly accept, girding your hardening shaft", false);
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s", false);
			MainScreen.text(" in tight latex.\n\n", false);
		}

		MainScreen.text("You climb on to the bed, but fur-covered hands grab hold of your " + LowerBodyDescriptor.describeHips(player) + " and yank you down until something round and wide is knocking on your backdoor.  You can feel the latex sheath bubbling up, starting to give Urta's dick a more rounded tip and she begins pressing it into your backdoor.  The unexpected attempts at insertion surprised you and have kept you too tightly-clenched for the fox to batter her way inside.  She begs, \"<i>Please let me in!  I can't wait anymore!</i>\"\n\n", false);
		...
		if (player.analCapacity() < 60) MainScreen.text("tight", false);
		else MainScreen.text("welcoming", false);
		MainScreen.text(", warm hole.  A groan escapes your mouth as you sink down, feeling fuller and fuller until her real sheath is resting just between your ass-cheeks.", false);
		player.buttChange(60, true, true, false);
		if (player.lowerBody.cockSpot.count() > 0) {
			MainScreen.text("  You can feel tiny dribbles force their way out of ", false);
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("each of ", false);
			MainScreen.text("your urethra", false);
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s", false);
			MainScreen.text(" as your prostate is compressed.", false);
		}
		MainScreen.text("\n\n", false);

		MainScreen.text("Urta recovers enough to meet your gaze and licks her lips hungrily.  Her hands release your " + LowerBodyDescriptor.describeHips(player) + " and rump, sliding up your sides and in towards your " + BreastDescriptor.describeAllBreasts(player) + ".  You shiver as she circles your " + BreastDescriptor.describeNipple(0) + ", teasing the flesh around them while managing to avoid providing them with any stimulation.  She looks up at you expectantly with a patient look, as if waiting on something from you.\n\n", false);

		MainScreen.text("You shift your hips and are rewarded by a caress over your left " + BreastDescriptor.describeNipple(0) + ", but it stops almost immediately.  Catching on, you flex your " + LowerBodyDescriptor.describeLegs(player) + " and lift yourself up until her flare is the only thing connecting you before sliding back down the pulsating shaft, squeezing your muscles around it.  Urta's hands reward you by pinching and tugging on your " + BreastDescriptor.describeNipple(0) + "s, ", false);
		if (player.upperBody.chest.hasFuckableNipples()) MainScreen.text("even slipping a finger inside their slippery depths, ", false);
		MainScreen.text("spreading pleasure through the sensitive pleasure-buds.  Eager for more, you begin bouncing atop your vulpine lover, riding her as deeply as possible in return for a treasure-trove of sensation and lust.\n\n", false);

		MainScreen.text("Locked together by desire, love, and one swollen cock, you rut with her for what seems like forever.  You lose count of how many times your lover moans and screams in pleasure, and your own gasps of surprised enjoyment are too numerous to tabulate.  Urta's fur slowly becomes matted with sweat, and the salty scent of lovemaking fills the air.", false);
		if (player.lowerBody.cockSpot.count() > 0) {
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("  Each of y", false);
			else MainScreen.text("  Y", false);
			MainScreen.text("our " + CockDescriptor.describeMultiCockShort(player) + " has filled its sheath with a golfball sized globule of pre-cum, and you can feel your body clenching and squeezing, your ", false);
			if (player.lowerBody.balls > 0) MainScreen.text("balls", false);
			else MainScreen.text("prostate", false);
			MainScreen.text(" aching to unload.", false);
		}
		if (player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("  Neglected and unused, your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " ", false);
			if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < VaginaWetness.DROOLING) MainScreen.text("dampens", false);
			else MainScreen.text("floods", false);
			MainScreen.text(" the fox underneath you with moisture, puffy and aching for the slightest touch.", false);
		}
		MainScreen.text("\n\n", false);

		MainScreen.text("The lovemaking increases in tempo, slowly working its way to a feverish crescendo.  You're bouncing atop the fox wildly, and your lover cums hard.  As full as you felt with her inside you, it's nothing compared to being stuffed with her cock AND cum.  Each cum-filled contraction swells her member from base to flare, allowing you to feel it stretch your insides while it travels up your " + ButtDescriptor.describeButthole(player) + ".  It dumps the load into the bubble of cum inside you, filling you with a creamy jizz balloon.\n\n", false);

		//(GENDERLESS ORGAZM)
		if (player.gender == 0) {
			MainScreen.text("You cry out with pleasure, feeling the closest thing you can to an orgasm.  Your muscles clench around the invading rod, milking it while the rest of your body trembles.  The strength drains from your " + LowerBodyDescriptor.describeLegs(player) + " and arms and you twitch there, completely impaled on your lover's thick cock, happily and completely penetrated.\n\n", false);
		}
		//(EVERYBODY ELSE)
		else {
			MainScreen.text("You cry out with pleasure, feeling your body cum from the invading warmth in your " + ButtDescriptor.describeButthole(player) + ".  Shivering as the strength drains from your arms and " + LowerBodyDescriptor.describeLegs(player) + ", you let the intruding and still cumming cock support your body.", false);
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				MainScreen.text("  Your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " clenches desperately, peaking without a single touch, but hungry for something to milk.", false);
				if (player.lowerBody.vaginaSpot.get(0).vaginalWetness > VaginaWetness.WET && player.lowerBody.vaginaSpot.get(0).vaginalWetness < VaginaWetness.SLAVERING) MainScreen.text("  Fluids soak Urta's crotch with lust with feminine drool, though the fox barely seems to notice.", false);
				else if (player.lowerBody.vaginaSpot.get(0).vaginalWetness == VaginaWetness.SLAVERING) MainScreen.text("  An explosion of feminine fluids spatters Urta's crotch and bed, completely soaking her groin in your need, but the distracted fox barely notices.", false);
			}
			if (player.lowerBody.cockSpot.count() > 0) {
				MainScreen.text("  The warmth spreads deep inside you, firing off a series of contractions just inside your groin.  You can feel yourself about to cum, and look down just in time to see yourself squirting against your condom, deforming it from the pressure of the orgasm and slowly inflating it further.", false);
				if (player.cumQ() >= 750 && player.cumQ() < 1250) MainScreen.text("  By the time you're done the balloon is the size of a basketball and looking dangerously close to bursting.", false);
				else if (player.cumQ() >= 1250) {
					MainScreen.text("  By the time you're done, the balloon is the size of a watermelon and wobbling dangerously, looking like it could fail at any time!", false);
					if (player.cumQ() >= 1700) MainScreen.text("  You fill it fuller and fuller, until it's bigger than you thought possible.  All at once the material gives out, soaking Urta from the tits down in cum, while your " + CockDescriptor.describeCock(player, 0) + " continues to pump even more onto her.  So much for not making a mess!", false);
				}
			}
			MainScreen.text("\n\n", false);
		}
		...
		MainScreen.text("She comes out of her stupor and murmurs, \"<i>Mmmm, that was nice,</i>\" as she stretches.  With practiced motions she slides off her condom, ties it off, and tosses it into a waste-can half filled with the things.  ", false);
		if (player.cumQ() >= 1700) MainScreen.text("Urta sighs at the mess and laughs, \"<i>I guess we got a mess anyways, huh?</i>\"", false);
		else MainScreen.text("Urta sighs contentedly and laughs, \"<i>I guess I don't have much of a mess to clean for once!</i>\"", false);
	...
	private oralFiestyUberExplosionUrta(): void {
		...
		if (player.cockArea(x) >= 100) {
			MainScreen.text("Urta takes one look at your " + CockDescriptor.describeCock(player, x) + " and says, \"<i>I'm sorry babe, but there's no way I could take that monster in my mouth.  How about we do something else?</i>\"\n\n", false);
			doNext(urtaHomeLuvLuvinsMenu);
			return;
		}
		MainScreen.text(images.showImage("urta-home-69", "vert"), false);
		if (player.gender == 2 || (player.gender == 3 && rand(2) == 0)) {
			MainScreen.text("You smile and let Urta know you want to sixty-nine her.  She raises an eyebrow and teases, \"<i>I hope you don't mind having a full belly and a sore throat!</i>\" as you disrobe and climb atop her.  You turn about, straddling your legs across her face and exposing the hungry lips of your sex.  Smirking over your shoulder, you snatch her discarded dress from the floor and tie the shimmering fabric into an impromptu cock-ring.  The gray fox shudders with excitement and you watch, enraptured, as her mottled horse-cock grows larger and larger, even flaring wide at the tip.\n\n", false);

			MainScreen.text("The musky scent of Urta's forearm-sized horse-prick wafts into your nostrils as it wobbles just below your " + HeadDescriptor.describeFace(player) + ", visibly twitching and throbbing from all the extra blood.  She whines plaintively, nuzzling her moist nose against your ", false);
			if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < VaginaWetness.DROOLING) MainScreen.text("equally wet", false);
			else MainScreen.text("sopping wet", false);
			MainScreen.text(" cunt.  Shivering from the air on your bare skin and the unexpected touches on your nethers, you open wide, extend your tongue, and give Urta's flare a long, slow lick.  You pause at her urethra, slip ever-so-slightly inside, and then pull out as you slather the flare in spit.\n\n", false);

			MainScreen.text("Urta eagerly gets to work, stabbing her tongue through your folds and into your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".  Her efforts bring her muzzle flush against you as she pauses to slurp your pussy-juices over your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + ".  Her lips feel like liquid heat as they wrap around the incredibly sensitive bud, suckling it gently in between excursions into your passage.  Urta's slender fingers grab hold of your " + ButtDescriptor.describeButt(player) + " to support her as she tongue-fucks your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " with abandon.\n\n", false);
			...
			if (player.stats.cor > 50) {
				MainScreen.text("push forwards, feeling her shaft arch as it slides into your throat.  The medial ring slips past your lips and you can feel your throat bulging as you mouth-fuck the swollen horse-cock deep into your gut.  Warmth drips into your belly as the hermaphroditic fox's member closes on its sexual peak.\n\n", false);
			}
			else MainScreen.text("grab hold of her cock and begin jerking her off into your mouth.  An immediate and copious gush of pre drips onto your tongue, showing you just how much the hermaphroditic fox loves the feeling.\n\n", false);

			MainScreen.text("Not far from cumming yourself, you grind your " + LowerBodyDescriptor.describeHips(player) + " on Urta's face, smearing her fur with your fragrant pussy-juices.  Her own moans of pleasure turn into noisy wet blubbering that vibrates through your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ", getting you off on the spot.  You squeal around the cock in your ", false);
			if (player.stats.cor <= 50) MainScreen.text("mouth", false);
			else MainScreen.text("throat", false);
			MainScreen.text(", clamping your " + LowerBodyDescriptor.describeLegs(player) + " onto Urta's head as you orgasm.  ", false);
			if (player.lowerBody.vaginaSpot.get(0).vaginalWetness > VaginaWetness.DROOLING) MainScreen.text("Fluids rush from your folds, splattering her face with sex fluid.  The fox opens wide, gulping it down like someone in a desert who has just found an oasis.", false);
			else MainScreen.text("Wetness slowly coats her muzzle as your canal squeezes and wriggles with involuntary pleasure-spasms.  Urta never lets up, sucking and licking your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " in a rhythm that matches your own orgasmic contractions.", false);
			if (player.lowerBody.cockSpot.count() > 0) {
				MainScreen.text("  ", false);
				if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("Each of y", false);
				else MainScreen.text("Y", false);
				MainScreen.text("our " + CockDescriptor.describeMultiCockShort(player) + " drools lamely, ", false);
				if (player.cumQ() < 50) MainScreen.text("sputtering spunk onto Urta's neck.", false);
				else if (player.cumQ() < 250) MainScreen.text("soaking Urta's neck and chest with your spooge.", false);
				else if (player.cumQ() < 700) MainScreen.text("showering Urta's neck and body with spooge until her breasts are painted white and the bed has a small puddle soaking into it.", false);
				else if (player.cumQ() < 1500) MainScreen.text("soaking Urta with a torrent of seed that drenches her from the neck down and totally soaks the bed.", false);
				else {
					MainScreen.text("soaking Urta with a torrent of seed that drenches her body and floods the bed, drips onto the floor, and slowly spreads into a puddle.", false);
					if (player.cumQ() > 10000) MainScreen.text("  The puddle thickens as you cum until it's nearly a foot deep.", false);
				}
			}
			MainScreen.text("\n\n", false);

			MainScreen.text("Too drunk with pleasure to register the telltale thickening of Urta's cock, you keep licking and sucking while you cum, drinking a steady stream of pre-cum.  Suddenly, your improvised cock-ring snaps and the straps fail from the massive pressure.  A torrent of jism ", false);
			if (player.stats.cor > 50) MainScreen.text("pours down your throat, bulging it into an obscene imprint of the horse-cock that's lodged inside.  Your eyes cross from pleasure and you slump down a bit, shoving your nose into Urta's sheath and allowing her to dump her sperm directly into your belly.  Gurgling noisily, your belly slowly expands outwards by about two inches, making you feel bloated and full.", false);
			else MainScreen.text("erupts into your mouth, instantly filling it with thick fox-cream.  You convulsively swallow in an attempt to keep up with it, but are quickly overwhelmed by the backed up semen-flood.  It drips from your mouth and forces open your throat, flowing directly into your belly.  Some even begins dripping out from your nose as you sway with a mixture of discomfort and pleasure atop your partner.  A few trembling squirts later you feel and look a little bloated, and your " + HeadDescriptor.describeFace(player) + " is thoroughly covered in foxy-horse-jizz.", false);
			MainScreen.text("\n\n", false);

			MainScreen.text("Both of you sputter for breath as you roll off of Urta.  She gasps and looks over at you with a smile, licking her sperm from your face and nuzzling her pussy-soaked muzzle against your cheek.  She pants, \"<i>I suppose I should be mad at you for ruining the dress, but you got me off so hard that I think I gave you a few meals worth of my... um... cum.</i>\"\n\n", false);

			MainScreen.text("The fox giggles and returns to cuddling and licking you.  You rest with a happy smile plastered on your " + HeadDescriptor.describeFace(player) + " from the experience.  After all the snuggling, you both rise and get dressed.  Urta finds a new dress in the closet and with a long goodbye kiss, you go your separate ways.\n\n", false);
			doNext(camp.returnToCampUseOneHour);
			player.orgasm();
			player.stats.sens += -1;
			return;
		...
		//(DUDEZILLAZ)
		MainScreen.text("Urta sits up and practically screams, \"<i>Really,</i>\" and squeals excitedly.  She claps and lies back as you climb into the bed, stopping only to yank off her cock-sheath and toss it into a nearby waste-bin.  You swing your " + LowerBodyDescriptor.describeLegs(player) + " over her and get into position, aligning ", false);
		if (player.lowerBody.balls > 0) {
			MainScreen.text("your " + BallsDescriptor.describeBalls(true, true, player) + " and ", false);
		}
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("one of your ", false);
		else MainScreen.text("your ", false);
		MainScreen.text(CockDescriptor.describeMultiCockShort(player) + " with Urta's muzzle.  Her rather sizable 'addition' wobbles underneath you, blocking your vision as it wobbles back and forth under you.\n\n", false);

		MainScreen.text("Your foxy lover opens wide and slips her smooth, wide tongue all over your ", false);
		if (player.lowerBody.balls > 0) MainScreen.text(BallsDescriptor.describeBalls(false, true, player, true) + ".  Slithering over and around them, she bathes your sack in her spit before moving up to your " + CockDescriptor.describeCock(player, x) + ".", false);
		else MainScreen.text(CockDescriptor.describeCock(player, x) + ".", false);
		MainScreen.text("  Her tongue curves and slurps at your shaft, swelteringly warm, wet, and tantalizingly slippery.  She tastes you from ", false);
		if (player.lowerBody.cockSpot.hasSheath()) MainScreen.text("sheath", false);
		else MainScreen.text("base", false);
		MainScreen.text(" to " + player.cockHead(x) + ", stopping to suckle the sensitive end briefly before breaking away for another long lick along the underside.  Her hands stroke along the sensitive " + player.skinDesc + " of your inner thighs, making your hips shiver and nearly pulling your " + CockDescriptor.describeCock(player, x) + " away from your lover's hungry mouth.\n\n", false);

		MainScreen.text("Whilst the fox is busy with your genitals, you begin to perform your duty, reciprocating the pleasure in turn.  You stretch your mouth wide, opening as far as you can to accommodate the mammoth beast before you.  It's difficult, but you manage to stretch wide enough to take her flare in your mouth.  It tastes vaguely salty like sweat, and in Urta's excited state it's easy to feel her flare widen and retract with each beat of her heart.  A dollop of pre-cum deposits itself on your tongue as the horny fox's nymphomaniac body reacts to you.  It tastes sweet, and you swallow as you work her deeper into your oral cavity.\n\n", false);

		MainScreen.text("Muffled, excited moans reach your ears as you work on your lover's pole.  The fox is clearly enjoying herself, and her sounds of pleasure cut off as she rams your " + CockDescriptor.describeCock(player, x) + " deep into her throat.  She gurgles noisily while her tongue slides as far down your shaft as possible, licking at your ", false);
		if (player.lowerBody.balls > 0) MainScreen.text(BallsDescriptor.describeBalls(true, true, player), false);
		else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)), false);
		else MainScreen.text("taint", false);
		MainScreen.text(" while her spittle froths around her lips.  She manages to start humming deep within her throat, vibrating your " + CockDescriptor.describeCock(player, x) + ".  The obstruction in your mouth stifles your moan, but Urta hears the startled gurgle and slides away, then plunges back down, humming the entire time in between gasps for air.\n\n", false);

		MainScreen.text("Not to be outdone, you pull back for a quick gasp of air before ramming her back inside your mouth.  You push forwards, taking her to the back of your throat", false);
		if (player.stats.cor < 60) MainScreen.text(" until your gag reflex forces you back.  Not being able to deep-throat her, you shift your weight onto your right arm and grab her just below her medial ring with your left.  You start vigorously jacking her off, running your tongue around her flare and over her sensitive tip, even darting it into her urethra to get at her sweet-tasting pre-cum.", false);
		else MainScreen.text(".  You easily suppress your gag reflex and push forwards, feeling her cock arch slightly in the back of your mouth as it curves into your throat.  You extend your tongue as far past your lips as you can, licking down the shaft as you cram her further and further into your gullet.  Though you can't taste it, you can feel her urethra sending slow pulses of pre-cum inside of you, depositing it directly into your belly as you stuff your nose into her balls and tongue the skin around her sheath.", false);
		MainScreen.text("\n\n", false);

		MainScreen.text("If someone walked in on the both of you now they'd see a breathless fox and you, each with the other's cock rammed into their face.  They'd see each of you blushing bright and trembling, on the verge of orgasm and barely holding on as the other does their best to bring about an orgasm.  Wet slurps and panicked gasps fill the air while bodies twist and squirm together on the fur-covered bed.\n\n", false);

		MainScreen.text("You cum first, a victim of Urta's talented tongue and humming technique.  You'd cry out with pleasure were you not so thoroughly stuffed, and the fox slams her face into your groin after the first blast, pushing you into her throat and noisily beginning to gulp down your spooge.  ", false);
		if (player.cumQ() >= 500) {
			MainScreen.text("She gurgles as she struggles to keep up with the flow, actually letting a bubbly white froth start to leak down the sides of her mouth.  ", false);
			if (player.cumQ() >= 1000) MainScreen.text("It begins dripping from her nostrils as your explosive load overwhelms her ability to swallow spunk.  ", false);
		}
		if (player.cumQ() >= 750 && player.cumQ() < 1500) MainScreen.text("Her belly rounds out a little from all the cum as your orgasm begins to wind down.  ", false);
		if (player.cumQ() >= 1500) MainScreen.text("Her belly rounds out a little before she pulls back and you spurt all over her, soaking her face and head with more spooge than she could ever handle.  ", false);
		MainScreen.text("Sated, you sigh, though Urta pushes back onto you and intently licks every ounce of jism from your body.\n\n", false);

		MainScreen.text("With only a few moments to recover from your own orgasm, you feel the fox stiffening, her apple-sized balls pulling up tightly against her body as she starts to orgasm.  A cock-bulging load of thick fox-cream forces your mouth wider before exploding ", false);
		if (player.stats.cor < 60) MainScreen.text("in your mouth, forcing you to compulsively swallow if you don't want to drown in it.", false);
		else MainScreen.text("down your throat, directly injecting it into your belly.", false);
		...
		MainScreen.text("Urta blows you another kiss and says, \"<i>Gotta go!  Be good, and don't do anyone I wouldn't, OK?  Love you!</i>\"", false);
		player.orgasm();
		player.stats.sens += -1;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	internal function urtaHomeLuvLuvinsMenu(): void {
		MainScreen.clearText();
		urtaSprite();
		MainScreen.text("She caresses a nipple and visibly shivers as she says, \"<i>You teased me the whole way here, and I'm COMPLETELY ready for you, any way you could want.</i>\"\n\n", false);
		MainScreen.text("She blushes and continues, \"<i>So, what'll it be, lover?  A little injection of fox-cream or something else?</i>\"", false);
		...
		menu();
		if (player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("  She could ride your vagina.");
			MainScreen.addButton(0, "Ride Vag", rideUrtasCoochLoveyDovey);
		}
		MainScreen.text("  She could fuck your ass.");
		MainScreen.addButton(1, "Ride Ass", rideUrtaInButtAtHomeLove);
		if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text("  Urta could 69 with you, let you fuck her pussy, or you could even fuck her cunt full of jizz and then lick her to a second cum.");
			MainScreen.addButton(2, "69", oralFiestyUberExplosionUrta);
			MainScreen.addButton(3, "Vag Fuck", urtasCoochiNeedsFuckedLove);
			MainScreen.addButton(5, "FuckAndLick", lickOutUrtaAtHome);
		}
		if (player.gender > 0) {
			MainScreen.text("  There's always the option to ask her to go condomless for a bit of extra fun.");
			MainScreen.addButton(4, "No Condoms", condomlessUrtaInHouseSmex);
		}
		if (player.lowerBody.isGoo() && player.skinType == SkinType.GOO) MainScreen.addButton(6, "Goo (Weird)", urtaGooTesticleVoreRuinedOrgasms);
		if (Flags.list[FlagEnum.URTA_PETPLAY_DONE] >= 0 && player.gender > 0) MainScreen.addButton(7, "Collar", urtaPetPlayDeletedForeverBecauseThirdProovedMeWrongAboutDice);
	}
	...
	private urtasCoochiNeedsFuckedLove(): void {
		...
		if (x < 0) x = 0;
		MainScreen.text("", true);
		MainScreen.text(images.showImage("urta-fills-her-condom"), false);
		MainScreen.text("Urta gives you a knowing look as she spreads her legs and lifts her bulging sack to expose the glistening black lips of her sex.  She asks with a sultry expression, \"<i>Is that what you need, lover, a slick, wet cock-sleeve to sheath yourself in?</i>\"\n\n", false);

		MainScreen.text("You nod eagerly and advance, dropping your " + player.inventory.armor.displayName + " by the wayside as you jump into bed.  She reaches around her male endowments, stroking along her lips to her puffy black clit.  Urta shivers in pleasure, spreading her legs as far as she possibly can, watching you with a hungry look as she licks her lips.  A few long strands of cunny-juice leak from her waiting hole onto the bed, practically begging you to slide inside and feel her nether-lips' velvet embrace.  You sidle up to your lover, pressing your ", false);
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("main ", false);
		MainScreen.text(CockDescriptor.describeMultiCockShort(player) + " at the moist opening, shivering at the warmth and desire as it wets your tip");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(".\n\n", false);

		if (player.cockArea(x) > urtaCapacity()) {
			MainScreen.text("Urta whines a little as you press forward and scooches backwards, \"<i>I'm sorry " + player.mf("stud", "baby") + ", you're just too big for my poor little pussy to take.  Maybe we could do something else?</i>\"\n\n", false);
			doNext(urtaHomeLuvLuvinsMenu);
			return;
		}
		MainScreen.text(images.showImage("urta-home-male-fuck"), false);
		MainScreen.text("You push forwards, slowly sliding into the moist tunnel, feeling its warmth covering every inch of your " + CockDescriptor.describeCock(player, x) + ". It caresses and squeezes you, surrounding you in perfect pleasure.  Urta stretches out languidly, her engorged horse-cock flopping down between her sizable breasts as it reaches a twitching, trembling hardness.  You bottom out, feeling the ", false);
		if (player.lowerBody.cockSpot.hasSheath()) MainScreen.text("sensitive skin of your sheath rubbing her exterior pussy-lips", false);
		else MainScreen.text("wetness of her pussy rubbing against your groin", false);
		MainScreen.text(". Urta moans, \"<i>By Marae, you make my cunt feel good, lover.  I'm getting so wet... fuuuck, I'm going to need to change my furs and sheets again.</i>\"\n\n", false);

		MainScreen.text("Unthinking, your " + LowerBodyDescriptor.describeHips(player) + " begin to rock back and forth, sawing in and out of the fox's exotic pussy.  She wriggles and moans happily, losing her grip on her weighty balls and letting them slide over the top-side of your pussy-slicked " + CockDescriptor.nounCock(CockType.HUMAN) + " with each movement.  You ignore it, increasing the tempo and watching as your lover's jiggling breasts shift and bounce around her flared penis, eliciting uncontrolled gasps of pleasure and causing pre to form a bubble at the tip of her latex cock-sheath.  Every thrust is met by a visible bulging of Urta's urethra and an increase in the size of the balloon hanging from her dick's flare.\n\n", false);

		MainScreen.text("Wet sounding 'schliiiicks' and noisy slaps fill the air as the sex takes on a violent, furious pace.  You slam yourself into the fox's hole, marveling at how nicely it accommodates and squeezes you.  The walls clench down on you from time to time, usually accompanied by a lewd moan as Urta's muscles involuntarily contract with pleasure.  She grabs her tits with both hands and squeezes them around her shaft, tit-fucking herself in perfect rhythm with your brutal thrusts.  Urta's condom starts filling faster – it's nearly the size of an apple now – and she's openly moaning and panting, on the verge of orgasm.\n\n", false);

		MainScreen.text("Your " + CockDescriptor.describeCock(player, 0) + " is on the verge as well, ready to blow after fucking the dirty fox-girl so hard.  She cums first, shrieking in pleasure and throwing back her head, her hands clamping her jiggling breasts tightly around the rippling flesh of her massive horse-member as it begins unloading.  The surface of Urta's condom distorts as she fires a massive jet of her stuff into it, nearly doubling the size of the balloon.  Her hips wriggle and her pussy contracts around you, rippling along your length as the lusty fox's girl-cum squirts over your ", false);
		if (player.lowerBody.cockSpot.hasSheath()) MainScreen.text("sheath", false);
		else if (player.lowerBody.balls > 0) MainScreen.text("balls", false);
		else MainScreen.text("crotch", false);
		MainScreen.text(".  You grab her ass and slam in a final time, reaching your own peak.\n\n", false);

		//[DAWG]
		if (player.hasKnot(x)) {
			MainScreen.text("Your knot flares wide, locking itself inside her convulsing cunt as your seed begins pumping into her womb.  Urta cries, \"<i>oooooooOOOOHHhhh YESSSS... You've plugged my cunt with your doggie dick!  It's squirting and pumping into my wooooomb...</i>\"\n\n", false);

			MainScreen.text("You rock back and forth, unable to pull out yet stimulating the sensitive bulb of your knot.  Your " + CockDescriptor.describeCock(player, x) + " dumps its gooey cargo directly into your lover's stopped-up sex with barely audible squishing noises.  ", false);
			if (player.cumQ() < 250) { }
			else if (player.cumQ() < 700) MainScreen.text("Her belly puffs up slightly by the time you finish, thanks to your body's copious cum production.  ", false);
			else if (player.cumQ() < 1000) MainScreen.text("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your overproductive " + CockDescriptor.nounCock(CockType.HUMAN) + " stops spewing inside her.  ", false);
			else {
				MainScreen.text("Her belly swells larger and larger, bloating as you blast thick torrents of seed inside her, filling her completely thanks to the obstruction of your knot.  ", false);
				if (player.cumQ() < 3000) MainScreen.text("By the time you finish, she looks VERY pregnant and is moaning with a mixture of pleasure and discomfort.  ", false);
				else {
					MainScreen.text("Your seemingly never-ending orgasm pumps her up until she looks on the verge of giving birth, and you just keep going.  The pressure builds and Urta moans in discomfort and pleasure.  Eventually the pressure is too much, and your cum begins spraying out around your knot, soaking the pair of you as you both continue to cum hard.  ", false);
					if (player.cumQ() > 5000) MainScreen.text("Not content with such an easy release, your " + CockDescriptor.describeCock(player, x) + " unloads the last of its cream until the bed and floor are both covered under inches of your canine cum.  ", false);
				}
			}
			MainScreen.text("You finally feel your " + CockDescriptor.nounCock(CockType.HUMAN) + "'s knot shrinking and pull free with a loud and messy pop.  ", false);
			if (player.cumQ() >= 2000) MainScreen.text("Spunk rushes out in a torrent, splattering everywhere.  ", false);
			MainScreen.text("Urta's glazed pussy-lips clench reflexively at their sudden emptiness and you hear a very satisfied sigh from the far end of the bed.\n\n", false);
		}
		//[HORSE]
		else if (player.lowerBody.cockSpot.get(x).cockType == CockType.HORSE) {
			MainScreen.text("You feel your flare filling with blood as it ", false);
			if (player.lowerBody.cockSpot.get(x).cockLength >= 10) MainScreen.text("prods against Urta's cervix.", false);
			else MainScreen.text("seats itself deep inside Urta's passage.", false);
			MainScreen.text("  Your crotch muscles clench in an unconscious rhythm as orgasm wracks your body.  You begin spurting hard inside her, splattering it into her cervix, even allowing it to flow into her womb.  Urta cries, \"<i>ooooOOOHHHH YESSS... is this what you feel when I cum inside you?  YesYESYES- Ohhh I can feel the flare stroking and flexing inside me...</i>\"\n\n", false);

			MainScreen.text("You rock back forth, unable to resist dragging the sensitive, flared tip and textured ring of your cock all along her pussy-walls, squirting seed with reckless abandon into her hot, cum-slicked cunt.  Wet squishing noises fill the apartment as the two of you rock the bed in twinned orgasms.  ", false);
			if (player.cumQ() < 250) { }
			else if (player.cumQ() < 700) MainScreen.text("Her belly slowly rises as more and more cum slides into her womb, filling it.  Runners of the stuff leak down her thighs as it escapes around your bestial cock.  ", false);
			else if (player.cumQ() < 1000) MainScreen.text("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your over-productive " + CockDescriptor.nounCock(CockType.HUMAN) + " has finished spewing your seed inside her.  ", false);
			else {
				MainScreen.text("Her belly swells larger and larger, bloating out as you blast thick torrents of seed inside her, stuffing her completely.  Every squirt of the stuff is met with a leaking back-blast that soaks her thighs and bed in your jism.  ", false);
				if (player.cumQ() < 3000) MainScreen.text("By the time you've finished, she looks heavily pregnant and cum soaks the bed.  ", false);
				else MainScreen.text("Your seemingly endless orgasm pumps more and more into her until she looks ready to give birth, and  you just keep going.  She moans in pleasure and discomfort as each blast of your horse-cum forces thick runners to squirt from between her thighs.  ", false);
				if (player.cumQ() > 5000) MainScreen.text("Not content with such an easy release, your " + CockDescriptor.describeCock(player, x) + " unloads the last of its cream until the bed and floor are covered in several inches of thick equine cum.  ", false);
			}
			MainScreen.text("You finally feel your flare returning to normal and pull out with a prolonged 'sluuuuurp'.  ", false);
			if (player.cumQ() >= 2000) MainScreen.text("Spunk rushes out in a torrent, splattering everywhere.  ", false);
			MainScreen.text("Urta's glazed pussy-lips clench reflexively at their sudden emptiness and you hear a very satisfied sigh from the far end of the bed.\n\n", false);
		}
		//[Everybody Else]
		else {
			MainScreen.text("Your " + CockDescriptor.describeCock(player, 0) + " clenches, filling her convulsing cunt as your seed begins pumping into her womb.  Urta cries, \"<i>oooooooOOOOHHhhh YESSSS... It's so warm and wet.  Yes, lover!  Keep goo-oh-OH-innnngg!!</i>\"\n\n", false);

			MainScreen.text("You rock back and forth, sliding your shaft through her spasming muscles, reveling and panting in orgiastic bliss.  Your " + CockDescriptor.describeCock(player, 0) + " dumps its gooey cargo deep into your lover's canal with barely audible squishing noises.  ", false);
			if (player.cumQ() < 250) { }
			else if (player.cumQ() < 700) MainScreen.text("Her belly puffs up slightly by the time you finish, thanks to your body's copious cum production.  ", false);
			else if (player.cumQ() < 1000) MainScreen.text("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your overproductive " + CockDescriptor.nounCock(CockType.HUMAN) + " stops spewing inside her.  ", false);
			else {
				MainScreen.text("Her belly swells larger and larger, bloating as you blast thick torrents of seed inside her, filling her completely while leftover jism squirts out around your " + CockDescriptor.describeCock(player, 0) + ".  ", false);
				if (player.cumQ() < 3000) MainScreen.text("By the time you finish, she looks VERY pregnant and is moaning with a mixture of pleasure and discomfort.  ", false);
				else MainScreen.text("Your seemingly never-ending orgasm pumps her up until she looks on the verge of giving birth, and you just keep going.  Jism sprays out from her dripping cunt as you release a torrent of semen, splattering you, her, and the bed.  ", false);
				if (player.cumQ() > 5000) MainScreen.text("Not content with such an easy release, your " + CockDescriptor.describeCock(player, 0) + " unloads the last of its cream until the bed and floor are both covered under inches of your gooey cum.  ", false);
			}
			MainScreen.text("You finally feel your " + CockDescriptor.nounCock(CockType.HUMAN) + " deflating, and you pull free with a messy slurp.  ", false);
			if (player.cumQ() >= 2000) MainScreen.text("Spunk rushes out in a torrent, splattering everywhere.  ", false);
			MainScreen.text("Urta's glazed pussy-lips clench reflexively at their sudden emptiness and you hear a very satisfied sigh from the far end of the bed.\n\n", false);
		}
		//[RESUME]
		MainScreen.text("Urta calms down at last, though her member is still throbbing weakly as she peels her condom off.  The bloated cum-bubble at the tip is the size of a basketball, and she struggles to get it tied off and disposed of.  The fox tosses it into a waste-basket already full of the things and gives you a toothy smile, \"<i>I think somebody's " + CockDescriptor.nounCock(CockType.HUMAN) + " loves my pussy almost as much as I love you!</i>\"\n\n", false);

		MainScreen.text("She pulls you into her arms and gives you a long, wet kiss before breaking away to clean up.  Urta blows you a kiss and says, \"<i>Thanks again, love!</i>\"\n\n", false);

		MainScreen.text("You and Urta go your separate ways after that, both stinking of sex and with a sense of deep satisfaction.", false);
		knockUpUrtaChance();
		player.orgasm();
		player.stats.sens += -1;
		doNext(camp.returnToCampUseOneHour);
	...
	public scyllaAndUrtaSittingInATree(): void {
		...
		Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00143]++;
		...
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00147] == 0 && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00145] > 0) {
			MainScreen.text("You meander towards the back-rooms and a canine waitress springs forward with a key clutched in her paw.  She blushes furiously and explains that she was tipped generously to hand it to you if you arrived.  You accept the proffered tool and smile.  It must be cheaper to pay someone to wait with a key than to fix the door each time the three of you meet up!  You snicker to yourself as you plunge the key into the lock's waiting receptacle, turn it, and push your way inside.  The scene in front of you is as arousing as it is familiar.\n\n", false);

			MainScreen.text("Urta is sitting down on a crate with her legs splayed and her torso leaning drunkenly in your direction.  Her tongue is hanging out and her eyes are lidded from alcohol as she notices you and gives another high-pitched whine, not of pain, but pleasure.  The source of her 'distress' is a black-robed figure with curly black locks of hair that hang around her head like a halo and thick, glossy red lips.  Those same lips are currently wrapped firmly around Urta's twitching, lust-swollen horse-cock just above her sheath.  You can clearly see the slight bulge of Urta's medial ring highlighted through the corrupted nun's neck as she bobs up and down, so absorbed in feeding her addiction that she failed to notice your entrance.\n\n", false);

			MainScreen.text("The fox is giving you a lewd look as she cradles Scylla's head in her hands and pushes down.  She slurs, \"<i>Oooooh YEAH!  Mmmmm, you made it " + player.short + "!  It seems like each time Scylla does... does... oooh... this I cum bigger and harder.  MMmmmm so good... wanna watch?</i>\"\n\n", false);

			MainScreen.text("Throughout the conversation she's weakly twitching her pelvis, rocking her hips against Scylla's face while her balls start to swell and churn in the nun's hands.  You close the door behind you carefully and drop the key on a crate.  It's time for some fun.\n\n", false);

			// Butans
			if (player.cockThatFits(urtaCapacity()) != -1 && player.lowerBody != LowerBodyType.CENTAUR) {
				MainScreen.addButton(0, "Lap", makeUrtaSitOnYourLapWithScylla);
				MainScreen.text("(You could make Urta sit on YOUR lap for a change, or you could jerk off and watch.");
			}
			else {
				MainScreen.text("(You could jerk off and watch.");
			}

			MainScreen.addButton(1, "Jerk", watchTwoHotBitchesAndJerkIt);
			MainScreen.addButton(2, "LippleBond", lippleBondUrtaScylla);

			MainScreen.text("  Bonding with the use of Scylla's lipples is also an option.)");

		}
		else {
			MainScreen.text("You meander by the Wet Bitch's back rooms, trying not to look like a creeper as you listen for ", false);
			if (!urtaLove()) MainScreen.text("Urta", false);
			else MainScreen.text("your love", false);
			MainScreen.text("'s voice.  The first few doors are silent and the next one sounds to have a spirited game of strip-poker going on inside it.  Judging by the grunts and occasional mewling moan of pleasure, some poor girl has already lost.  One of the barmaids gives you a warning glance and you keep moving, sure to avoid the lewd game of chance as you hunt for the stray, well-endowed fox.\n\n", false);

			MainScreen.text("At first, the following entryway is silent, though you can see light underneath.  You're just about to move on from the portal when you hear Urta's voice give out a strained grunt.  Did some ", false);
			if (player.stats.cor < 40) MainScreen.text("corrupted ruffians", false);
			else MainScreen.text("demon agents", false);
			MainScreen.text(" somehow get the drop on her?  She whines, high-pitched and plaintive, almost as if she was wounded, and you cannot wait a moment later.  You lunge to the door, twist the handle, and find it locked.  Without a second thought, you throw your weight into the flimsy wood paneling and crash through to the last thing you expected to see!\n\n", false);

			MainScreen.text("Urta's sitting down on a crate with her legs akimbo and her torso leaning drunkenly in your direction.  Her tongue is hanging out and her eyes are lidded from alcohol as she notices you and gives another high-pitched whine, not of pain, but pleasure.  The source of her 'distress' is a black-robed figure with curly black locks of hair that hang around her head like a halo and thick, glossy red lips.  Those same lips are currently wrapped firmly around Urta's twitching, lust-swollen horse-cock just above her sheath.  You can clearly see the slight bulge of Urta's medial ring highlighted through the corrupted nun's neck as she bobs up and down, so absorbed in feeding her addiction that she failed to notice you bursting through the door.\n\n", false);

			MainScreen.text("The fox is ", false);
			if (urtaLove()) MainScreen.text("weakly protesting", false);
			else if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11) MainScreen.text("giving you a lewd look", false);
			else MainScreen.text("blushing hard", false);
			MainScreen.text(" as she cradles Scylla's head in her hands and pushes down.  She slurs, \"<i>", false);
			if (urtaLove()) MainScreen.text("I'm... ahhhh... s-sorry love, but we both know she's been suuuu - oh yeah, keep licking - sucking you off before, and the hungry slut just couldn't take her eyes off my cock.  Why didn't you tell me ho - OH YES!  RIGHT THERE! - h-how good she was?", false);
			else if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11) MainScreen.text("Ohhh, you showed up a little late... mmmhmm yes, now lick... Oh, why didn't you tell me ho - OH YES!  RIGHT THERE! - h-how good of a cock-sucker this lewd-bodied bitch was?", false);
			else MainScreen.text("H-hi " + player.short + "... I didn't think I'd get to see you to- Oh yeah, like that!  Mmmm... OH!  I guess since we've been intimate before you can watch...", false);
			MainScreen.text("</i>\"\n\n", false);

			MainScreen.text("Throughout the conversation she's weakly twitching her pelvis, rocking her hips against Scylla's face while her balls start to swell and churn in the nun's hands.  Thankfully the doorway is at such an angle that the bulk of the bar can't see inside, and it helps that no one seems inclined to bother the three of you for now.  What do you do?\n\n", false);
			...
			MainScreen.text("  You could throw them the finger and leave.  Scylla also has two pairs of lips unoccupied - playing with them could lead to something interesting.  You could tell Urta you never want to see her again.)");

			MainScreen.addButton(3, "Heartbreak", heartBreakHotelInTelAdre);
			MainScreen.addButton(4, "Leave", flipUrtaTheBird);
		}
	...
	private urtaAndScyllaBoningLikeBitchesSober(): void {
		...
		if (!urtaLove()) MainScreen.text("fuck-buddy");
		else MainScreen.text("lover");
		MainScreen.text("'s lusty slip-up and skittish responses, pinching her booty for good measure while you decide just how to get involved in the potential <i>ménage à trois</i>.\n\nWhat do you do?");
		dynStats("lus", 10 + player.stats.lib / 5, "resisted", false);
		//Get worshipped requires gigantic dong
		let worship: Function = null;
		let fuck: Function = null;
		if (player.cockThatFits(urtaCapacity()) >= 0) {
			fuck = fuckUrtaWhileScyllaSucksSober;
			MainScreen.text("  You could <b>fuck the fox</b> while she gets her oral pleasure.");
		}
		if (player.lowerBody.cockSpot.biggestCocks[0].cockArea() > urtaCapacity()) {
			worship = dockWithUrtaUnderScyllasSoberSupervision;
			MainScreen.text("  You could whip out your gigantic cock and let the blowjob queen <b>worship</b> a REAL dick.");
		}
		MainScreen.text("  Whatever you do, with Scylla involved, it's sure to get out of hand.");
	...
	private fuckUrtaWhileScyllaSucksSober(): void {
		MainScreen.clearText();
		let x: number = player.cockThatFits(urtaCapacity());
		MainScreen.text("You peel out of your straining equipment to free your " + CockDescriptor.describeMultiCockShort(player));
		if (player.lowerBody.balls > 0) MainScreen.text(" and [balls]");
		MainScreen.text(", hauling out your sweaty, engorged meat.  Scylla, absorbed as she is with cum-thirst, doesn't seem to notice, though her nose twitches as the scent of your musky aroma mingles with the sex-stained air.  You gently squeeze behind the fox, folding her bushy, soft tail to the side and giving you something firm to support yourself when you let her flop back into you.  Her smooth fur feels wonderful on your [chest] and she squirms and gyrates against Scylla's sucking mouth.  Fairly dripping with moisture, the fox's black-lipped sex grinds on you, resting atop [oneCock], and your length is soon soaked with her heady, animalistic fem-spunk.");
		MainScreen.text("\n\nUrta pants, \"<i>Go on... nnn... j-just put it in already!</i>\"  You readily oblige the over-endowed beast-woman by guiding your length into the clutching vice of her nethers, shivering at the onslaught of electric bliss that rockets up your " + CockDescriptor.describeCock(player, x) + ".  You hilt inside her");
		if (player.cockArea(x) >= 40) MainScreen.text(", clearly bulging her stomach with your sizeable cock");
		MainScreen.text(", sliding up into the slick, welcoming passage as if you always belonged.  Rivulets of liquid desire trickle from the stretched lips to run down ");
		if (player.lowerBody.balls > 0) MainScreen.text("the front of your [sack]");
		else MainScreen.text("your [legs]");
		MainScreen.text(".  The gray-furred femme reaches back in an awkward hug, trying to hold you against her, speared together in the most intimate of embraces.");
		...
		if (silly()) MainScreen.text("a cock and a sharp place.");
		else MainScreen.text("your lusty bodies.");
		MainScreen.text("  The velvet glove clasping your " + CockDescriptor.describeCock(player, x) + " tightens without warning, accompanied by a full-body quake.  Urta's eyes roll back as some invisible force works through her, culminating in a muscular clench that you feel will pop your " + CockDescriptor.describeCock(player, x) + " clean off your body.  You grunt, grabbing hold of the fox's tits for stability and wince from the almost painful tightness of her snatch.  The moment seems to drag on forever, but then, the vixen slumps back into your arms, allowing you to support her sagging weight.");
		MainScreen.text("\n\nBeyond your sight, the fox-girl's soft-furred sack begins to round out, growing larger in spite of the sharp tip prodding hard at her balls. The delicate, plush nuts swell bigger, big enough to pull the skin around them tight, big enough to pull Urta harder onto your " + CockDescriptor.describeCock(player, x) + ", big enough to be painfully poked, on the verge of being pierced.  Urta's vocalizations take on a keening, banshee-like quality, but before any harm can be done, a rosy glow envelops the fox-herm's poor, brutalized sack.  Stranger still, the nun's ram-like protrusions shorten in response, actually withdrawing back into her skull, pushed back by your lover's impressive, seemingly magical testes.");
		...
		MainScreen.text("\n\nMoaning, Urta whimpers, \"<i>S-so full... I - ungh - I... I can't hold it!  C-cuuuummmiiiiinnng!</i>\"  Her hips jerk forward hard and fast enough that it nearly pulls the gushing pussy off your " + CockDescriptor.describeCock(player, x) + ", but Scylla isn't so lucky.  Her nose is squished almost flat into the fox's sweaty middle-fur, chin deep in ballsack.  She doesn't seem to mind; in fact, when drizzles of frothing fluid appear at the crinkled corners of the nun's ruby lips, her eyes roll back, exposing the whites.  Meanwhile, Urta's less-than-colt-sized dong swells, but not as a return to its former glory.  No, if anything, her musky dick is still shrinking.  The sheer size of the cum-globs pumping through her urethra are visibly stretching her horse-cock, giving the illusion of increased size.");
		...
		if (player.lowerBody.balls > 0) MainScreen.text("r [balls]");
		MainScreen.text(" give the warm snatch a dose of virile medicine.  Urta, perhaps stirred from her own orgasm by the jets of hot fluid flooding her nethers, reaches back to squeeze your butt and whimpers, \"<i>Give it to me.</i>\"");
		...
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("  The unbound portion of your package bobs in the air before coming to rest in Urta's ass-crack.  A second later, the vulpine butt is coated with sticky white icing.");
		if (player.lactationQ() > 50) MainScreen.text("  Your sensitive nipples let down, releasing their own heady flows to compliment your climactic pleasure, spraying into the gray fur relentlessly.");
		MainScreen.text("  Throughout it all, your " + CockDescriptor.describeCock(player, x) + " continues to do its level best to fill the fox.  Fireworks of uncontrollable pleasure go off one after the other, alternatively numbing and locking your muscles as you release all of your pent-up lusts, desires, and needs into the welcoming cunt.");
		if (player.cumQ() >= 500) {
			MainScreen.text("  Urta's belly bloats from the thick deposit");
			if (player.cumQ() >= 2000) MainScreen.text(", rounding more and more, a taut cum-tank that jiggles with your thick seed.");
			else if (player.cumQ() >= 1200) MainScreen.text(", rounding out into a swollen cum-bank that jiggles under the weight of your seed.");
			else if (player.cumQ() >= 750) MainScreen.text(", rounding out into a gravid bump that jiggles with liquid weight.");
			else MainScreen.text(", rounding out into a nicely-sized bump.");
			MainScreen.text("  You run your hands across it as you give a last few pumps.");
		}
		...
		if (player.tallness >= 78) MainScreen.text("head");
		else if (player.tallness >= 48) MainScreen.text("narrow middle");
		else MainScreen.text("curvy hips");
		MainScreen.text(" you watch the nun's throat bulge almost double wide with each swallow of seed.  Tremendous volleys of semen are pumping down her esophagus rapidfire, and yet the suckling woman barely shows any sign of being full.  A clear puddle has sprung up underfoot, centered below the cock-sucker's crotch, and you marvel at the evidence of Scylla's pleasure.  She must be cumming non-stop!");
		...
		if (urtaLove()) MainScreen.text("  You give your lover a wet kiss and a fondle before you go, snickering when it makes her even longer.");
		else MainScreen.text("  You give her a quick hug and depart, smiling when you realize the brief touch made her even longer.");
		knockUpUrtaChance();
		//-2 sens, -100 lust
		player.orgasm();
		player.stats.sens += -2;
	...
	private dockWithUrtaUnderScyllasSoberSupervision(): void {
		...
		MainScreen.text("Unable to contain yourself, you sigh heavily and release your [armor].  Your immense package grows bigger with each beat of your heart, the blood-swollen veins pulsing hotly as they pump your cock to its full, obscene proportions.  It slowly undulates in the air as it fills to capacity with your lusty lifeblood, and the unmistakable aroma of your masculine ");
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("half's ");
		MainScreen.text("musk fills the air.");
		if (player.lowerBody.balls > 0) MainScreen.text("  Your [sack] swings free, bouncing off the back of [oneCock] when it thickens precipitously.");
		MainScreen.text("  Sighing with relief, you gently stroke your length as you eye the two beauties meaningfully.");
		...
		if (player.lowerBody.balls > 0) MainScreen.text("  Your [balls] quiver a little when the rushing seed reaches them, and you swear you hear them slosh with the horse-spunk pouring into their own reservoirs.");
		else MainScreen.text("  Your innards gurgle slightly as the rushing seed travels through them, and you swear you can feel your prostate stretching to accommodate being suddenly stuffed with horse-spunk.");
		...
		if (player.lowerBody.balls > 0) MainScreen.text(" while your enormous balls gurgle and begin to empty");
		MainScreen.text(".  The titanic, room-filling cock lifts up off the ground by a few inches, supported by the thick bulge of cum that rushes from ");
		if (player.lowerBody.cockSpot.hasSheath()) MainScreen.text("sheath");
		else MainScreen.text("base");
		MainScreen.text(" to tip in an instant. It passes through the tight crimson seal, distending the nuns neck for a moment before it settles in her belly.  She looks almost pregnant on a mix of horse and champion-spunk for a moment, but then her belly narrows and her tits swell - just in time to take the next gush of your demonically-blessed orgasm.  Scylla appears to be in heaven, and as she swallows, the darkness leaves her eyes.  What remains is wide-eyed irresistible pleasure, and the vibrations of orgasmic screams vibrate through your swollen " + player.cockHead(x) + " as you dump wave after wave of spooge through the clinging orifice.  A shiny puddle of feminine fluid widens beneath the nun's orgasm-wracked frame, and just once, you wish you could stick your head under her robes and watch it happen.");
		...
		Flags.list[FlagEnum.URTA_SCYLLA_BIG_DICK_TIMES_DONE]++;
		//{-100 lust,+1 corruption, }
		player.orgasm();
		player.stats.cor += 1;
		if (player.cumQ() < 5000) player.cumMultiplier += 2;
		if (player.lowerBody.balls > 0) player.lowerBody.ballSize += 1;
	...
	private dockWithUrtaUnderScyllasSoberSupervision2(): void {
		...
		if (!urtaLove()) MainScreen.text("waves ");
		else MainScreen.text("kisses you ");
		MainScreen.text("goodbye, she says, \"<i>Thank you.</i>\"");
		...
		if (player.lowerBody.balls > 0) MainScreen.text("  Your balls definitely got a little bigger...");
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private flipUrtaTheBird(): void {
		urtaSprite();
		MainScreen.text("", true);
		MainScreen.text("You curl your hand into a fist and extend your middle finger at the alcoholic and cum-slut.  You've got better things to do than waste your time watching those two indulge their addictions.\n\n", false);
		//(lose urta love points.)
		urtaLove(-1);
		//(lose urta dick comfort points.)
		Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] -= 2;
		if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 0) Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] = 0;
	...
	private watchTwoHotBitchesAndJerkIt(): void {
		...
		MainScreen.text("The gray-furred hermaphrodite's balls are shrinking with each blast, contracting and pausing as her body launches each volley, but they've only lost about half their enhanced size and Scylla's getting rounder than she has any right to be.  The nun seems ready for this though, and hums in concentration.  Her belly stops growing immediately, but each of her already prodigious milkers begins to enlarge, sloshing noisily like a barrel being filled.  You nearly push yourself past the edge at the sight of the growing melons, but hold back – you don't want to blow until you see just how far this goes.\n\n", false);

		MainScreen.text("Urta's hands let go of the corrupted woman's horns and she slumps back against the wall, but the nun just wraps her arms around the blissed-out vixen and suckles harder.  You keep watching and stroking, entranced by the sight of the spooge bubbles vanishing into Scylla's neck and plumping up her tits.  Urta's eyes are rolled back and she's drooling a runnel of spittle down her tit, around one of her black nipples, and down her belly, but she keeps cumming, even though her balls are nearly back to normal.  The poor fox is utterly unable to cope with the sensations running through her drunken form, but she's clearly enjoying it.\n\n", false);

		MainScreen.text("Just as Scylla's tits start to drag against the floorboards, she pulls back, letting Urta blow a few long ropes of thick horse-cum over her face and hair.  The nun sighs happily and begins to gather it in her fingers, licking it up while the exhausted fox's cock slowly deflates, dripping a trail along the floor.  Urta recovers surprisingly quickly, blinking her eyes and pulling her tongue back into her mouth with a sigh of utter, sated contentment.  You held back for so long, but watching the nun pick pearly strands of animal cum from her hair and swallow it down like a whore is just too much.\n\n", false);

		MainScreen.text("You pump hard on " + CockDescriptor.describeMultiCockSimpleOne(player), false);
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" in turn", false);
		MainScreen.text(", shooting hot ropes of semen that drape over the satisfied girls.  After so much edging, and such a hot scene, your orgasm is far more intense than any normal masturbation session.  Your whole body trembles while it spurts and squirts, making sure both of the addicts get at least a little bit of your white goo on their bodies.  ", false);
		if (player.cumQ() >= 250) MainScreen.text("A particularly coherent blast of seed splatters over Urta's tits, glazing her gray fur and black nipples with a thick layer of dripping spunk.  ", false);
		if (player.cumQ() >= 450) MainScreen.text("Another huge gout takes Scylla straight in the face, completely soaking her with yet more salty mess that drools onto the curve of her pendulous, black-clad tits.  ", false);
		if (player.cumQ() >= 700) MainScreen.text("A few more big blasts of the stuff fire off, spotting the pair with fat, sticky blotches.  ", false);
		if (player.cumQ() >= 1000) {
			MainScreen.text("Your body is so productive that it utterly drenches both participants in the stuff.  Runnels of it drip over their curves while fat droplets pool into a", false);
			if (player.cumQ() >= 2000) MainScreen.text("n inches-deep", false);
			MainScreen.text(" puddle on the ground.  ", false);
		}
		MainScreen.text("Finally spent, you slump back and smile.\n\n", false);
		...
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00143] == 1) MainScreen.text("\"<i>We need to do thish again!</i>\"", false);
		else MainScreen.text("\"<i>You're the besht " + player.mf("man", "woman") + " a girl could ashk for.</i>\"", false);
		MainScreen.text("  Before you can answer, she gives your ass a slap and finds a water barrel to wash up in.  Scylla finishes her 'meal' and looks up at you, her alabaster skin turning completely crimson before she too runs off to clean up.\n\n", false);
		MainScreen.text("You smile happily and pick up your " + player.inventory.armor.displayName + ".  It's past time you checked up on your camp.", false);
		if (urtaLove()) MainScreen.text("  You give your lover's back a wistful grin and muse over how good her taste in women is as you leave.", false);
		player.orgasm();
		//+luv
		urtaLove(2);
		//+comfort
	...
	private makeUrtaSitOnYourLapWithScylla(): void {
		...
		MainScreen.text(images.showImage("urta-scylla-lapfuck"), false);
		MainScreen.text("You smirk at the drunken fox and inform her in no uncertain words that you'll do more than watch.  She blinks at you, her inebriated mind unable to process new thoughts as you stalk forwards and sit behind her on the crate", false);
		if (player.lowerBody == LowerBodyType.NAGA) MainScreen.text(", snaking your tail around her legs", false);
		else if (player.lowerBody == LowerBodyType.GOO) MainScreen.text(", enveloping her legs with your goo", false);
		else MainScreen.text(", straddling your " + LowerBodyDescriptor.describeLegs(player) + " around her hips", false);
		MainScreen.text(".  You grab her narrow waist and pull her up, dragging Scylla's cock-locked head up with her and aligning your " + CockDescriptor.describeCock(player, x) + " with her backdoor before you start to lower her back down.  ", false);
		if (y > -1) MainScreen.text("Your " + CockDescriptor.describeCock(player, y) + " even manages to get in line with her drooling, black-lipped pussy.  ", false);
		if ((player.lowerBody.cockSpot.count() > 1 && y == -1) || player.lowerBody.cockSpot.count() > 2) MainScreen.text("You allow your remaining, unused dickflesh to flop against her back, leaking pre-cum into her fur.  ", false);
		MainScreen.text("You aren't sure if it's the alcohol in her or her own anal talents, but she slides down without a complaint, pulling Scylla's vacuum-like mouth down with her", false);
		if (y != -1) MainScreen.text(" and squeezing you tightly with her sopping cunt", false);
		MainScreen.text(".\n\n", false);

		MainScreen.text("She grunts, \"<i>", false);
		if (x != -1 && y != -1) MainScreen.text("Oooh, and in both holes?  You naughty " + player.mf("boy", "girl") + ", you!", false);
		else if (urtaLove()) MainScreen.text("Oooh yeah, fill me love!", false);
		else if (rand(2) == 0) MainScreen.text("Oooh, you know how to treat a lady!", false);
		else MainScreen.text("MMmm, that's nice!", false);
		MainScreen.text("</i>\"  She's actually trying to watch you over her shoulder now, completely ignoring the cock-addled nun as the latter bobs on her horse-meat.  You make the most of it and ", false);
		if (urtaLove() || player.stats.cor <= 40) MainScreen.text("kiss her full on the mouth", false);
		else MainScreen.text("bite her neck", false);
		MainScreen.text(" while you grab her soft, furred asscheeks and lift her up, feeling the exquisite texture of her pucker ", false);
		if (y != -1) MainScreen.text("and twat ", false);
		MainScreen.text("massaging your manhood the whole way.  Before you go too high, you drop her, and let her weight impale her back on top of you.  Her rapidly expanding balls slap into your own and smear them with her pussy juice.\n\n", false);

		MainScreen.text("Urta's tail curls snugly around your waist, and the drunken fox takes over, lifting herself up even as Scylla's magic swells her balls past the size of grapefruits.  She openly moans when she reaches the apex, and smoothly lowers herself back down, squeezing and stroking you with her internal muscles while trying to endure the nun's incessant suckling.  The measured ", false);
		if (y != -1) MainScreen.text("double penetration", false);
		else MainScreen.text("anal fuck", false);
		MainScreen.text(" drives you wild, but being on the bottom as you are, there isn't much you can do aside from grabbing fistfuls of vixen-ass and slapping it for encouragement.  Urta gets the message and begins to bounce vigorously, mewling and moaning as her balls get large enough to reach the floor.  You lend a hand and play with her sheath, feeling the flesh underneath quake and tremble as the fox's orgasm hits.\n\n", false);

		MainScreen.text("Scylla's neck is pulled tight as Urta's urethra is stretched wide by huge bulges of cum.  You can actually feel them sliding past the fingers you've dug into her sheath.  No sooner than one squeezes past is another on its way.  You wonder how Scylla can handle such incredible volumes and the only answer you can think of is magic.  Whatever the case, her swallowing gets noisier and Urta starts bouncing even faster atop your " + CockDescriptor.describeCock(player, x), false);
		if (y != -1) MainScreen.text(" and " + CockDescriptor.describeCock(player, y), false);
		MainScreen.text(".  You pinch one of her shiny black nipples between your fingers as encouragement, and she actually howls with delight.  The herm vixen's cum-pumping cock starts spasming in your grip, the order of her previous cum blasts dissolving to turn her cock into a frenzied, constantly squirting spooge-hose.  Every spurt of jism that Scylla swallows is one less in the fox's magically-enhanced ballsack, and the unnatural, mighty orbs begin to shrink back towards their normal size with alarming rapidity.\n\n", false);

		MainScreen.text("Scylla's growing belly and tits smush around your " + LowerBodyDescriptor.describeLegs(player) + ", swelling further and further as she tries to devour all the creamy horse-spunk the fox is hosing into her.  Yet the touch of long, gentle fingers caressing your " + BallsDescriptor.describeSack(player) + " and the gradual inflation of your " + BallsDescriptor.describeBalls(true, true, player) + " lets you know that the slutty nun's discovered another source of seed.  You groan as warmth and stimulation make your orbs churn and slosh, growing not just in size but also in raw, animal NEED.  Driven to near madness by the drunken fox's delicious backside and the building, backed-up cum in your sack, you grab hold of Urta with both hands and start jackhammering her asshole", false);
		if (y != -1) MainScreen.text(" and pussy", false);
		MainScreen.text(", pounding the lithe fox as if your life depended on your orgasm.\n\n", false);
		...
		MainScreen.text("You cum, hard enough to make you cross your eyes and slam Urta's hips into you with painful force, even through the padding of her supple buttcheeks.  The first ass-filling wave of jism completely packs her backdoor, surrounding you in a sea of your own sticky moisture.", false);
		if (y > -1) MainScreen.text("  Meanwhile, your other " + CockDescriptor.nounCock(player.lowerBody.cockSpot.get(y).cockType) + " utterly fills her womb, stuffing it so completely that the fox's belly rounds out.", false);
		if ((player.lowerBody.cockSpot.count() > 1 && y == -1) || player.lowerBody.cockSpot.count() > 2) MainScreen.text("  A wave of the stuff splatters her back, glazing her from the shoulder-blades to the crack of her ass and nearly catching you in the chin. Maybe there is such a thing as too many cocks?", false);
		MainScreen.text("  Scylla's hands rub the fox's belly tenderly as she humps away at the lewd nun, using whatever magic she has to ensure you won't damage the drunken vixen as you continue to push larger and larger amounts of semen into her.  After three more blasts, she runs out of room inside her, and it starts squirting back out with each fresh stroke, soaking your waist, " + LowerBodyDescriptor.describeLegs(player) + ", and rapidly shrinking balls.  You lose track of time, knowing only the pleasure of orgasm and the feeling of your own spooge pumping into your lover until the unholy cumsplosion finally ends.\n\n", false);

		MainScreen.text("You regain your wits to the sight of a very pregnant-looking Urta and Scylla leaning on each other and trying to make their way to a water barrel to clean up.  The vixen is so full she looks like a mother in her ninth month of pregnancy with triplets, but as she starts to clean up Scylla gently presses on the fox's belly and devours the cum she squeezes out.  Urta shakes, but doesn't emit a sound beyond a few strangled half-moans of pleasure.  If the fresh horse-cum soaking the nun's habit is any indicator, Urta must have gotten off again during your orgasm.  Wild!\n\n", false);

		MainScreen.text("You grab a rag and wipe up before donning your " + player.inventory.armor.displayName + ".  Urta saunters over, looking like she's sobered up quite a bit.  She's got ", false);

		if (urtaLove()) MainScreen.text("a contended smile on her face, and gives you a long, slow kiss that practically radiates her love for you.", false);
		else if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 11) MainScreen.text("a nervous smile on her face, but gives you a quick peck all the same.", false);
		else MainScreen.text("a satisfied smile on her face, and crushes her lips against yours in a passionate kiss.", false);
		MainScreen.text("  She says, \"<i>I think I'll need to help Scylla to her room... somehow whatever she did to me must have pulled all the alcohol out of me and into my cum.  Don't worry, I'll ", false);
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00143] <= 1) MainScreen.text("pay for the door and ", false);
		MainScreen.text("make sure she's ok.</i>\"\n\n", false);

		MainScreen.text("Did her cock just twitch?  No, you scold yourself; it couldn't have, not after two orgasms like that.  Urta hooks her arm around the giggling nun's midsection and pulls the stumbling sister towards the doorway.  Scylla looks back at you and half-giggles, half-slurs, \"<i>Tee-hee, thish ish so much fun!  I love you guysh!</i>\"\n\n", false);

		MainScreen.text("You make your way out with your head low, but you do see Scylla and Urta walking up the stairs towards a truly private room.  They each look a little unsteady and neither of Scylla's hands can be seen, but you're sure they'll be ok.", false);
		player.orgasm();
		player.stats.lib += 1;
		player.stats.sens += 1;
		player.stats.cor += .5;
		...
		urtaLove(3);
		//+comfort
		Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] += 3;
	...
	private tellUrtaNoMoreScylla(): void {
		urtaSprite();
		MainScreen.text("", true);
		Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00147] = 1;
		MainScreen.text("You let Urta know that you're willing to overlook this, but that Scylla is a slippery slope that's best avoided.  She nods, a sober expression on her vulpine face.  Once you finish, she replies, \"<i>I understand, but you need to know – when 'it' gets too much for me to handle... if you aren't around to help I'm going to see Edryn.  S-she helps... if it wasn't for her I probably would've given up and done something stupid a long time ago.  What we have is more important, but unless we can get together every couple days I'll NEED to visit her.  ", false);
		if (urtaLove()) MainScreen.text("Just please, don't doubt my love for you.  ", false);
		MainScreen.text("Do I need to stop drinking too?", false);
	...
	private tellUrtaMoreScyllaIsFine(): void {
		urtaSprite();
		MainScreen.text("", true);
		MainScreen.text("You give Urta a raunchy, lewd smile and let her know that you're okay with her 'helping' Scylla, and vice-versa, so long as they don't mind you popping in.  She smiles and gives you a wink, but her expression darkens after a moment of thought.  Urta interrupts to mention, \"<i>There's something else.  If I can't find you or Scylla and I NEED to cum, I'll have to visit Edryn.  She's probably the only thing that's kept me sane, and if you aren't around to help with my needs I'll visit her.  ", false);
		if (urtaLove()) MainScreen.text("Just remember that YOU'RE the one I love, not her.  ", false);
		else MainScreen.text("Me and her, we've been partners in the guard for a long time, but the sex is just mechanical, to sate us.  You don't have to worry about her stealing me away.  ", false);
		MainScreen.text("But what about my drinking or how I get when I'm drunk?  It doesn't bother you, does it?", false);
		//[Drink More] [Drink Less] [No Change] [Leave Her]
		simpleChoices("Drink More", tellUrtaToBeADrunkenHussy, "Drink Less", tellUrtaToStopBeingALush, "No Change", tellUrtaToStayTheSame, "Leave Her", leaveUrtaAfterScyllaConfrontation, "", null);
	}
	//[Leave Her]
	...
	private tellUrtaToBeADrunkenHussy(): void {
		urtaSprite();
		MainScreen.text("", true);
		MainScreen.text("You ", false);
		if (player.stats.cor < 33) MainScreen.text("blush a little bit", false);
		else if (player.stats.cor < 66) MainScreen.text("smile nervously", false);
		else MainScreen.text("grin widely", false);
		MainScreen.text(" and explain that when she gets drunk, lets her guard down, and gets a little more aggressive... you like it.  A lot.  Her ears perk up at your words, though her expression is a little uncertain while you explain it.  By the time you finish, something warm brushes by your " + LowerBodyDescriptor.describeLeg(player) + " and gently 'thunks' the table.  Clearly she's as into the idea as you.\n\n", false);

		MainScreen.text("Urta smiles, lewdly at first, though it carries a bit of a predatory glint as she waves down a waitress and orders a full bottle of Barkardi 151.  You give her a rueful smile, a stroke under the table, and a kiss just bursting with tongue before you conclude the conversation.  Urta's going to be a lot of fun from now on...", false);
		Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] = 1;
		player.stats.lust += 5;
		doNext(camp.returnToCampUseOneHour);
	}
	//[Drink Less]
	...
	private tellUrtaToStayTheSame(): void {
		urtaSprite();
		MainScreen.text("", true);
		MainScreen.text("You smile and ask her why you would ever want her to change?  You ", false);
		if (urtaLove()) MainScreen.text("love", false);
		else MainScreen.text("like", false);
		MainScreen.text(" everything about her.  She blushes hard when you tell her that, clearly enjoying the flattery and thrilled to hear that you're okay with how she chooses to blow off steam when she's not on the job.  The two of you joke around for a little longer, but before long, it's time you were on your way.  Urta's STILL blushing as she gives you a good-bye kiss – you must have earned some points with her today!", false);
		//(+love score)
		urtaLove(5);
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private itsOkayUrtaAmilyIsHot(): void {
		MainScreen.text("", true);
		urtaSprite();
		MainScreen.text("You reach out, grab Urta's shoulders, and look at her until she raises her face to look back at you.  Before she can look away, you tell her that it's fine - you had hoped your lovers would come to love each other as you do.  Urta wipes away a tear and gives you a nervous smile when you suggest that perhaps the three of you could get together sometime.\n\n", false);
		MainScreen.text("Once you've finished, Urta leans forwards and plants a salty, tear-stained kiss on your lips.  The two of you spend some more time bonding, before you realize you should probably check up on your camp and go.", false);
		doNext(camp.returnToCampUseOneHour);
	}
	//[Heart asplosion]
	...
	public chastityBeltFun(cameFromSexMenu: boolean = true): void {
		...
		MainScreen.clearText();
		if (cameFromSexMenu) {
			MainScreen.text("Eyeing your foxy lover, you tell her to lose the condom, you have something else planned.\n\n");
			MainScreen.text("“<i>Lose it?  What do you have in mind?</i>” Urta asks.  She looks past you towards her cupboard.  “<i>Oh, no, not that belt!  ...Do you really enjoy seeing me make such a mess of myself when I finally get it off?</i>” she protests.\n\n");
		...
		MainScreen.text("Urta interrupts you with a heave of exasperation.  “<i>If you want anything more from me, dear, you’re going to have to let me out of this thing before I explode,</i>” Urta tells you with a sense of urgency.  She taps one finger on the protrusion that holds her cock, which is now oozing a constant streamer of jizz to splat wetly onto the floor, for emphasis.  “<i>Lemme out of this thing before I wrestle you to the floor and take the keys - and if I gotta do that, I swear to Marae I will ram this cock of mine right up your ass and flood you with every last drop you’ve teased out of me!</i>” she vows.\n\n");
		MainScreen.text("Okay... judging by her look you can tell that she intends to do just that if you refuse... so you tell her that’s fine.  You’ve had your fun and Urta certainly earned her freedom; considering the amount of cum leaking from her cock-prison... perhaps she should get in the shower?\n\n");
		MainScreen.text("She nods eagerly and shuffles into the shower, pulling aside the cheap curtain and motioning you to follow her.  You quickly do and holding the extrusion containing her cock, you tell her to brace herself.\n\n");
		MainScreen.text("With a quick twist of the key, you pull the belt off her as fast as you can, causing her erection to spring up and slap her on the belly as it finally erupts into a veritable geyser of cum, painting the walls, the ceiling and you and Urta both, in thick, hot, fox-cum.  Urta sighs hugely, visibly slumping with relief even as her cock continues to bulge and then geyser gush after gush of cum.  “<i>Ohhh... that feels so, so good, [Name],</i>” she tells you.  She just hangs her head and lets the waves of orgasm ripple through her, visibly bloated balls shrinking before your very eyes as they empty themselves of the pent-up frustration you’ve helped Urta accumulate.\n\n");
		MainScreen.text("Wiping your face clean of the spunk, you tell Urta that this is indeed an impressive amount of cum... but perhaps a bath is in order now?  Urta nods, then puts a finger to her lips and thinks it over.  “<i>I oughta make you lick this off of me... but I’d rather a hot shower anyway,</i>” she declares.\n\n");
		MainScreen.text("With a smile you turn the faucet and let the initially cold water rinse most of the cum off of your bodies.  You help Urta wash by gently massaging her fur, even rubbing yourself against your hot fox lover; then slowly grasping her still sensitive shaft and giving it a few good pumps.  Urta moans and thrusts her cock into your pistoning hand, managing to dredge up a few last sizable spurts of goo that splat wetly into the floor and are swiftly washed away down the drain.  She says nothing, simply looking deeply into your eyes while you help her clean herself down.\n\n");
		MainScreen.text("Once done you ask her where you can get some towels?  “<i>In a cupboard, just there,</i>” Urta tells you, pointing to a small cupboard built into the wall of the bathroom.  Nodding, you quickly go fetch the towels, handing one to her as you begin drying yourself.  Once fully dried up, you look at Urta and realize that she’s still dripping wet.  You begin to form a question, when suddenly you realize the evil stare she’s giving you... before you can scream your protest, you watch as she braces herself on the wall and begins shaking herself, flinging wet droplets of water all over your, formerly, dry self.  Looking at you with as much innocence as she can muster, which is not much by the way, Urta says, “<i>What?  This is how I get dry!</i>”  You glare back at her as she does her best to stifle a laugh.\n\n");
		MainScreen.text("You quickly dry yourself... again.  And then help Urta.  Urta struggles and protests at your efforts to dry her off, but accepts it.  Finally, she laughs and whips the towel off.  “<i>I don’t know if I hate you or love you for what you just put me through... but, all in all, I do love you, so... no hard feelings, okay?</i>”  She asks, giving you her most winning smile even as she approaches a tiny sink and takes up a comb, which she starts running through the tangles of her fur.\n\n");
		MainScreen.text("Looking Urta over, her fur is in serious need of a good combing... especially her tail... you smile at her and ask if she has a spare comb?  Urta blinks, surprised, but takes up a second comb from the sink’s side and hands it over to you.  You gently take her tail in hand and begin combing through it, doing your best to deal with tangled fur without hurting Urta.  Urta lets out a quiet yelp of surprise, then growl-purrs in pleasure at your ministrations; she has to fight to keep from messing things up by wagging her tail.  She leans against you and combs her fur, allowing you sole responsibility for handling her tail.\n\n");
		MainScreen.text("Finally done, you ask Urta to give you a little twirl.  Urta does as you ask, giving a quiet giggle as she does so.  “<i>How do I look?</i>” she asks.  As lovely as ever, you reply with a thumbs up.  Sadly... you need to get going soon... so you give your vulpine lover one last kiss, redress and start on your way back to camp.");
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private QBsTalkExpack(): void {
		...
		urtaSprite();
		//(Lover Urta)
		if (urtaLove()) MainScreen.text("When you approach Urta, she smiles to see you.  However, there is no rapid swivelling in her seat to avoid banging her horse-cock against the table; evidently she's not in the mood for some sex right now.  You sit down beside her, and she happily loops her arm around you, planting a tender kiss on your lips, before almost apologetically stating, \"<i>Hey there, lover.  Want to talk for a change?</i>\" with a twinkle of good-natured mirth in her eyes.");
		else MainScreen.text("When you approach Urta, you are almost surprised at the lack of an audible 'thump'.  Seeing your confusion, the herm fox quips, \"<i>What's with the look?</i>\" while raising an eyebrow.  When you indicate that you had expected to hear her demonstrating her excitement to see you, she blushes and looks embarrassed.  \"<i>Well... you do a pretty good job of letting my little pony blow off some steam, ya know, [name]?</i>\"");
	...
	private friendsTalkExpack(): void {
		MainScreen.clearText();
		urtaSprite();
		MainScreen.text("You approack Urta's table and she motions for you to take a seat.  She finishes writing something on one of the pieces of paperwork scattered over the table, takes a sip of her drink and leans back in her chair.  “<i>Nice to see you again " + player.short + ".  What can I do for you?</i>”");
		urtaDialogueMenu();
	}
	...
	public urtaDialogueMenu(): void {
		...
		menu();
		MainScreen.addButton(0, "Urta", urtaDiscussesSelf);
		MainScreen.addButton(1, "Edryn", urtaDiscussesEdryn);
		MainScreen.addButton(2, "The Watch", urtaDiscussesTheWatch);
		if (!urtaJustFriends()) MainScreen.addButton(3, "Alcoholism", urtaDiscussesAlcholism);
		if (Flags.list[FlagEnum.KATHERINE_UNLOCKED] >= 4) MainScreen.addButton(5, "Katherine", urtaDiscussessKatherine);
		if (urtaPregs.urtaKids() > 0 && player.hasKeyItem("Spare Key to Urta's House") < 0)
			MainScreen.addButton(4, "Visit Kids", urtaPregs.visitKidsFirstTime);
		else
			if (Flags.list[FlagEnum.FIRST_TIME_AT_URTA_HOUSE] > 0) MainScreen.addButton(4, "Her House", urtaPregs.talkToUrtaAboutHerHouse);
	...
	private urtaDiscussesSelf(): void {
		MainScreen.clearText();
		urtaSprite();
		MainScreen.text("You tell Urta you'd like to talk about her a little.");
		if (!urtaLove())
			MainScreen.text("“<i>You wanna talk about me?</i>” she asks" + (urtaJustFriends() ? "" : ", blushing softly") + ".  “<i>Well, there isn't THAT much to tell, but what do you want to know?</i>”   She seems surprisingly pleased to have the chance to talk.");
		else MainScreen.text("“<i>Well, I don't think there's that much to tell,</i>” she says with a smile.  She idly blows a bang out of her face and exhales, “<i>Well, lover, I'm an open book for you, what do you want to know?</i>”");
		...
		else if (Flags.list[FlagEnum.URTA_QUEST_STATUS] == .5)
		...
		else if (Flags.list[FlagEnum.URTA_QUEST_STATUS] == 1)
	...
	public flirtWithUrta(): void {
		MainScreen.clearText();
		if (Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] == -1) { //In friends mode
			MainScreen.text("You ask Urta if, living in Tel'Adre, it's easier to resist the powerful feelings that come from living in this world.\n\n");
			MainScreen.text("“<i>I've lived here my whole life, so I don't have anything else to compare with.  What do you mean by 'feelings'?</i>”\n\n");
			MainScreen.text("You tell her that in your world you never felt a deep urge to strip off all your clothes and " + (player.stats.cor < 75 ? "make sweet love to" : "slake your lust on") + " the nearest person.\n\n");
		}
		else {
			MainScreen.text("You tell her that you don’t have any problem with herms, in fact you find some of them quite attractive.\n\n");
			MainScreen.text("Urta gulps down a bit more of her booze and you decide to have a little fun.  ");
		}
		MainScreen.text("You quietly slide your " + (player.lowerBody.isNaga() ? "tail" : "foot") + " across under the table and rub it against the inside of her leg.  Urta practically jumps out of her seat, then tries to pretend nothing happened.\n\n");
		MainScreen.text("You’re pretty sure most of the bar noticed, but you don’t care.  You keep rubbing and feel a thump as something large smacks against the underside of the table.  Urta drains the last of her drink and whispers, “<i>" + (Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] == -1 ? "What about Kath?  I... I wouldn't..." : "Please - it’s so hard...") + "</i>”\n\n");
		MainScreen.text("You never find out exactly what she was going to say.  When you start to rub your " + (player.lowerBody.isNaga() ? "tail" : "foot") + " in little circles against the root of her huge cock Urta loses her train of thought and just leans back in her chair, trying to keep from spraying her pent up cum across the floor.\n\n");
		MainScreen.text("You ease up so she has a chance of understanding what you’re saying.  “<i>" + (Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] == -1 ? "I'll explain everything to Kath.  You want this as much as I do, right?" : "Yes, some chicks, with their big dicks, just do it for me.") + "</i>”  You slide " + (player.lowerBody.isNaga() ? "the tip of your tail" : "your toes") + " a little lower and start moving it in little circles against her balls.\n\n");
		MainScreen.text("Before Urta pops you get up from the table and tell her to meet you out back, right where things went off the rails that first time.  Urta nods eagerly and the edge of the table rises slightly.\n\n");
		MainScreen.text("You wait outside, hiding in a shadowy corner.  You got Urta so excited that it’s a good fifteen minutes before she throws open the back door of the bar.  She looks hungry, yet unsure.  You’re guessing she’s had such terrible experiences with people in the past that despite your obvious interest she still can’t believe you want her.\n\n");
		Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] = 0; //urtaFuckHer will raise it to 1
		Flags.list[FlagEnum.URTA_PC_LOVE_COUNTER] = 0; //Reset both flags to put Urta back to her 'never met you before' state, ready for her first fuck
		urtaFuckHer(true); //Call urtaFuckHer() to put you on track for much Urta loving
	}

	private urtaFertilityDiscussion(): void {
		MainScreen.clearText();
		MainScreen.text("Urta traces a finger around the corner of her drink, \"<i>So, what do you think, [name]?");
		if (Flags.list[FlagEnum.URTA_FERTILE] == 1) MainScreen.text("  Should I stay fertile?");
		else MainScreen.text("  How about I cut loose and go fertile?  Can you imagine me with a nice, round pregnant belly and a cock virile enough to get you pregnant just from looking at it?");
		if (Flags.list[FlagEnum.URTA_PREG_EVERYBODY] == 1) MainScreen.text("  Or did you come to ask me not to be fertile and virile around anyone else?");
		else {
			MainScreen.text("  Would you mind me being able to put a bun in Edryn's oven?  Imagine all the lil foxtaurs running around!");
			if (Flags.list[FlagEnum.EDRYN_NUMBER_OF_KIDS] > 0) MainScreen.text("  After all, you already had a turn!  Fair's fair!");
		}
		MainScreen.text("</i>\"  She winks at you before blowing you a playful raspberry.\n");
		menu();
		if (Flags.list[FlagEnum.URTA_FERTILE] == 1) {
			MainScreen.text("\n<b>Urta is currently fertile.</b>");
			MainScreen.addButton(0, "No Fertile", urtaFertilityToggle);
		}
		else {
			MainScreen.text("\n<b>Urta can not currently get pregnant (or inseminate anyone, let alone you).</b>");
			MainScreen.addButton(0, "Go Fertile", urtaFertilityToggle);
		}
		if (Flags.list[FlagEnum.URTA_PREG_EVERYBODY] == 1) {
			MainScreen.text("\n<b>Urta thinks you're okay with her 'sharing the love (and kids)' with any other sexual partners she might have.</b>");
			MainScreen.addButton(1, "No Preg", urtaPregOthersToggle);
		}
		else {
			MainScreen.text("\n<b>Urta will not knock up or be knocked up by anyone else unless you tell her otherwise.</b>");
			MainScreen.addButton(2, "Share Preg", urtaPregOthersToggle);
		}
		MainScreen.text("\n\n<b>(Note: There is no content for Urta to impregnate or be impregnated by other NPCs, yet.)</b>");
		MainScreen.addButton(4, "Back", urtaDialogueMenu);
	}

	private urtaFertilityToggle(): void {
		MainScreen.clearText();
		if (Flags.list[FlagEnum.URTA_FERTILE] == 1) {
			MainScreen.text("\"<i>Well, I guess that's okay.  But you better make it worth my while later, okay?</i>\" Urta says with a half-hidden sigh.");
			Flags.list[FlagEnum.URTA_FERTILE] = 0;
		}
		else {
			MainScreen.text("Urta throws her hands up and cheers, \"<i>Yesss!</i>\"  Realizing that everyone is looking over at the two of you now, she gives a sheepish smirk and glances back at you.  \"<i>I can't wait!</i>\"");
			Flags.list[FlagEnum.URTA_FERTILE] = 1;
		}
		menu();
		MainScreen.addButton(0, "Back", urtaFertilityDiscussion);
	}

	private urtaPregOthersToggle(): void {
		MainScreen.clearText();
		if (Flags.list[FlagEnum.URTA_PREG_EVERYBODY] == 1) {
			MainScreen.text("\"<i>Awww, are you sure?  Wouldn't you like ");
			if (player.lowerBody.cockSpot.hasCock())
				MainScreen.text("to help me make Edryn's belly bloat with our seed, and then play 'Guess Who Is The Father'?");
			else
				if (player.lowerBody.vaginaSpot.hasVagina())
					MainScreen.text("to have me get the both of you pregnant at the same time, so we can all enjoy it together?");
				else
					MainScreen.text("see me put this thing to work in the way that it's meant to?");
			MainScreen.text("</i>\" Urta says, giving you a playful pinch.");
			Flags.list[FlagEnum.URTA_PREG_EVERYBODY] = 0;
		}
		else {
			MainScreen.text("Urta smiles salaciously, like a cat that got the cream.  Then, without warning, she kisses you square on the lips, passionately frenching you.  After nearly a full minute of frenzied kissing, she says, \"<i>This is gonna be fun!</i>\"");
			Flags.list[FlagEnum.URTA_PREG_EVERYBODY] = 1;
		}
		menu();
		MainScreen.addButton(0, "Back", urtaFertilityDiscussion);
	}
	...
	private urtaDiscussesEdryn(): void {
		MainScreen.clearText();
		urtaSprite();
		MainScreen.text("You think for a moment, then tell her that you have questions about Edryn.");
		if (!urtaLove())
		...
		let discussRomance: Function = (urtaJustFriends() ? null : urtaDiscussesEdrynRomance);
	...
	private urtaDiscussesAlcholism(): void {
		MainScreen.clearText();
		urtaSprite();
		if (Flags.list[FlagEnum.DISCUSSED_URTA_ALCOHOLISM] == 0 && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00145] == 0) {
			MainScreen.text("You quietly inform Urta that you and she need to talk about her drinking habit.  She swallows nervously and insists, \"<i>I - I only drink to try and keep my cock under control.</i>\"");
			...
			if (player.stats.cor < 33)
				MainScreen.text("and you feel a little bad for putting her through this");
			else
				if (player.stats.cor < 66)
					MainScreen.text("and you would feel bad if this talk weren't necessary");
				else
					MainScreen.text("but you don't let up for a moment");
			MainScreen.text(".  You insist that you aren't upset with her, that you're not blaming her for anything, but you need to talk to her about this matter.");
			MainScreen.text("[pg]Urta refuses to meet your eye for several long minutes.  Nervously, she begins tapping one clawed finger on the table.  \"<i>I... okay.  The truth is that I did start drinking originally in the hopes it would make my erections subside and make me harder to arouse.  As you've seen, it doesn't work like that - truth be told, I'm kind of a horny drunk.</i>\"");
			MainScreen.text("[pg]You ask why, then, she does it - why does she keep drinking?");
			MainScreen.text("[pg]\"<i>Because it's an excuse for me to cut loose, alright?</i>\" she barks indignantly.  Now she meets your eyes, staring into them with defiance.  \"<i>I work day in and day out to keep this city from falling apart, and the whole time, I have to deal with this... this thing between my legs, undermining everything I say.  ");
			if (urtaLove())
				MainScreen.text("Lover");
			else
				MainScreen.text("Cutey");
			MainScreen.text(", you don't know what I've been through.  In here, when I'm off the clock, I can just have a bottle, hang out with my friends, and kind of... just... bask in everyone else's happiness.  It's not much, but it's better than sitting at home, bored and lonely.</i>\"");
			MainScreen.text("[pg]She finally ceases her rambling dialogue and sighs loudly, shaking her head.  \"<i>Maybe it was a bad idea, but it was my idea, you know?</i>\"  Then, she looks up at you, gently.  \"<i>Besides, I got to meet you, didn't I?  Even with all the bad, I gained you for a ");
			if (!urtaLove())
				MainScreen.text("friend.</i>\"");
			else
			...
			MainScreen.text("[pg]Urta falls silent, waiting for you to speak.  Will you tell her to start drinking less?  Will you tell her that you are fine with her drinking habits, now that she's explained her startling change in behavior and her reasons for doing so?  Will you break up with her if she's that much of an alcoholic? Or do you want her to drink even more - perhaps you find her so much more fun to be with when she's blink stinking drunk?");
			...
			MainScreen.text("You tell Urta that you want to discuss her newfound drinking habits.  The grey-furred fox-morph meets your gaze calmly.  \"<i>Really?  What more do you have in mind?</i>\" she asks.");
			if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] == 1)
				MainScreen.text("  \"<i>I can't drink any more than I already do - I'm kind of pushing the laws to drink as much as I do already.</i>\"  She burps loudly, then starts on another bottle.");
			else
				if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] == 0)
					MainScreen.text("  \"<i>You said you didn't mind me drinking as much as I do... are you going to ask me to cut down?</i>\" she asks, calmly and clearly assuming that's what you intend.");
				else
					MainScreen.text("  \"<i>I said it before, I'll say it again, I'm not going to stop drinking entirely - there's nothing wrong with a few cold ones to take the edge off.  I don't drink myself stupid any more; that should be enough for you,</i>\" she replies, defensively.");
	...
	private urtaDiscussAlcoholDrinkMore(): void {
		...
		Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00146] = 1;
		Flags.list[FlagEnum.DISCUSSED_URTA_ALCOHOLISM] = 1;
		doNext(camp.returnToCampUseOneHour);
	}
	//[=Drink Less=]
	private urtaDiscussAlcoholDrinkLess(): void {
		...
		if (pregnancy.type == PregnancyType.PLAYER) {
			MainScreen.text(", once she's not pregnant anymore, of course");
		}
		MainScreen.text(".  Her eyes glitter with steely resolve, and she leans over the table to kiss you on the lips.  The two of you wrap up the conversation, with you hoping that you've seen the last of drunken Urta.");
	...
	private urtaDiscussesFamily(): void {
		...
		urtaSprite();
		if (!urtaLove()) {
			MainScreen.text("You tell Urta that you'd like to learn about her family.  What are her parents like?  Does she have any siblings?  Is she the only hermaphrodite in the family?");
			//(Regular
			if (!urtaLove()) {
				if (Flags.list[FlagEnum.URTA_FAMILY_TALK_ATTEMPTS] == 0)
	...
	private urtaDiscussesSexAndRomance(): void {
		MainScreen.clearText();
		urtaSprite();
		MainScreen.text("You ask Urta if she'll tell you her romantic & sexual history.");
	...
	private urtaDiscussionTeaseAfterRomance(): void {
		MainScreen.clearText();
		urtaSprite();
		MainScreen.text("You thank her for telling you about her past like this but follow it up by asking her what she likes in the bedroom, doing your best to hide a mischievous grin.");
	...
	private urtaDiscussesEmployment(): void {
		MainScreen.clearText();
		urtaSprite();
		MainScreen.text("You ask how and when it was that Urta came to be part of the Tel'Adre watch.");
	...
	private urtaDiscussesPrejudice(): void {
		MainScreen.clearText();
		urtaSprite();
		MainScreen.text("Hesitantly, you ask Urta if she has ever faced prejudice for her hermaphroditic nature.");
	...
	private urtaDiscussesEdrynHistory(): void {
		...
		urtaSprite();
		//[=History Together=]
		MainScreen.text("You ask Urta what her past with Edryn is.");
	...
	private urtaDiscussesWorkingWithEdryn(): void {
		MainScreen.clearText();
		urtaSprite();
		MainScreen.text("You tell Urta that you're curious about her working relationship with Edryn.  Do they work together often?");
	...
	private urtaDiscussesEdrynRomance(): void {
		...
		if (!urtaLove()) {
			if (!urtaOpenAboutEdryn()) {
				Flags.list[FlagEnum.URTA_OPEN_ABOUT_EDRYN] = 1;
				MainScreen.text("You tell Urta that you've noticed she and Edryn seem to hang out together a lot.  Furthermore, as a centauress, you figure Edryn's probably capable of handling what Urta has hanging between her legs.  You're curious; are they a couple?");
			...
			//Subsequent Regular Variant:
				...
				MainScreen.text("You give Urta a thoughtful look, and then ask how things are with Edryn these days.");
				MainScreen.text("\"<i>Good,</i>\" the fox-morph replies freely.  \"<i>She'll never compare to you, but she is a truly wonderful fuck... I swear, there is nothing quite like feeling a mare-cunt juice itself around you...</i>\"  She smirks at you, obviously enjoying the chance to make you squirm, but ");
				if (player.stats.cor < 66) MainScreen.text("her manner is playful");
				else MainScreen.text("her expression sobers when you make no response");
				MainScreen.text(".");
				//(If player can now sleep with Edryn for free:
				if (player.statusAffects.get("Edryn").value1 >= 5) {
					MainScreen.text("  \"<i>Of course, you know that firsthand, don't you?</i>\" she grins.  \"<i>Did hearing that I was having sex with her make you curious about what it was like?");
					//(If player has fathered at least 1 child on Edryn:
					if (Flags.list[FlagEnum.EDRYN_NUMBER_OF_KIDS] > 0) MainScreen.text("  Still, I can't believe you actually knocked her up - I had to pick my jaw up off the floor when she told me that the foal kicking inside her ballooning gut was yours.  I mean, those herbs are supposed to be foolproof...</i>\" She shakes her head, half in disbelief of your sheer virility, half in awe of it.");
					else MainScreen.text("</i>\"");
					if (Flags.list[FlagEnum.URTA_FERTILE] == 0) MainScreen.text("  \"<i>Still, I hope you don't have any idle thoughts in your head about knocking me up; there's a difference between overwhelming contraceptives and sowing seed where the field's barren,</i>\" she states, patting her flat belly for emphasis.  Despite how flippant she tries to sound when she says this, you can tell that it's something of a sore subject for her.");
				}
		...
		//Lover
			...
			if (!urtaOpenAboutEdryn()) {
				...
				MainScreen.text("With a smirk, you ask Urta how things have been between her and Edryn lately.");
				...
				MainScreen.text("\"<i>Hmph!  So you should be!</i>\" Urta says, sticking her nose up in the air in an indignant huff... but she can't keep a straight face for more than a few moments.  \"<i>I still can't believe you took it so well when I told you - I really am a lucky girl to have someone like you, aren't I?</i>\" she says.");
				//(If Edryn is doing the player for free:
				if (player.statusAffects.get("Edryn").value1 >= 5) MainScreen.text("  \"<i>Though I have to admit I was still a little jealous when Edryn told me you were having sex with her too.  Still, I guess that's just another thing we have in common, yeah?  A mutual taste in sexy ladies.</i>\"  She laughs, and mockingly shapes her hands through the air as if caressing Edryn's sizable boobs.");
				//(If Edryn has had at least one baby:
				if (Flags.list[FlagEnum.EDRYN_NUMBER_OF_KIDS] > 0) {
					if (Flags.list[FlagEnum.URTA_QUEST_STATUS] == 1) {
						if (urtaPregs.urtaKids() == 0) MainScreen.text("  \"<i>...I still can't believe that you actually get her pregnant, though.  It's just not fair that Edryn's got little hooves running around, and after everything I went through, I don't have kits of my own... yet.</i>\" She gives you a slightly predatory smile along with the last word.");
						else MainScreen.text("  \"<i>...I still can't believe that you actually got her pregnant, though.  I guess I'll have to keep you busy in the sack if we're going to keep up, huh?</i>\" She gives you a slightly predatory smile along with the last word.");
					}
					else MainScreen.text("  \"<i>...I still can't believe that you actually got her pregnant, though. It's just not fair that Edryn's got little hooves running around, and you and I can't have any.</i>\"  Urta sighs.");
				}
	...
	private urtaDiscussesEdrynNonLoveREJECT(): void {
		MainScreen.clearText();
		urtaSprite();
		MainScreen.text("No matter how you look at it, you can't help but feel betrayed - Urta's been sleeping around behind your back.  If she didn't want this relationship to be monogamous, she should have told you before, and you tell her as much.");
	...
	private urtaDiscussesEdrynLoveTolerate(): void {
		...
		Flags.list[FlagEnum.PC_DISLIKES_URTA_AND_EDRYN_TOGETHER] = 1;
		doNext(camp.returnToCampUseOneHour);
	}
	//[=Breakup=]
	private urtaDiscussesEdrynLoveBreakup(): void {
		MainScreen.clearText();
		urtaSprite();
		MainScreen.text("No matter how you look at it, you can't help but feel betrayed - Urta's been sleeping around behind your back.  If she didn't want this relationship to be monogamous, she should have told you before, and you tell her so.");
	...
	private urtaDiscussesWatchMembership(): void {
		MainScreen.clearText();
		urtaSprite();
		MainScreen.text("You tell Urta that you're curious about the Watch's membership.");
	...
	private urtaDiscussesWatchRole(): void {
		MainScreen.clearText();
		urtaSprite();
		MainScreen.text("You tell her that you're curious about what it is that the Watch actually does, on a day to day basis.");
	...
	private urtaDiscussesTheWatchsReach(): void {
		MainScreen.clearText();
		urtaSprite();
		MainScreen.text("You ask Urta just how far the Watch's authority spans from the city.");
	...
	private urtaDiscussesCrimeAndPunishment(): void {
		MainScreen.clearText();
		urtaSprite();
		MainScreen.text("You explain to Urta that you're curious about the law in Tel'Adre - what is a crime here, and what are the punishments that the Watch is likely to deal to a criminal?");
	...
	internal function spankTheShitOutOfUrtaAndMakeHerCreamHerselfFromProstateStimulationAloneLikeTheHornyDrunkenSlutSheReallyIs(): void {
		MainScreen.clearText();
		MainScreen.text(images.showImage("urta-bar-male-spank"), false);
		MainScreen.text("You consider Urta's offers and, tempting as they are, decide they just aren't for you right now.");
		...
		MainScreen.text("with each swat.  \"<i>You're a rude bitch when you're drunk, Urta,</i>\" you add, giving her another swat on the ass.  She tries to stand, but you roughly push her back against the table.  You give her a fourth spank for her insolence, and can only grin when you hear her cock slam up against the table again.");
		...
		if (player.cockThatFits(urtaCapacity()) >= 0) {
			let y: number = player.cockThatFits(urtaCapacity()) + 1;
			...
			if (player.cumQ() < 250) MainScreen.text("you've spent your load");
			else if (player.cumQ() < 1000) MainScreen.text("filled her utterly");
			else MainScreen.text("streams of cum leak out of her ass, pooling at your [feet]");
			MainScreen.text(".  Urta whines, desperate to cum, but you don't let her.  Not yet.");
	...
	private urtaDiscussessKatherine(): void {
		...
		if (affection == 0) { //They’re just co-workers
			...
			MainScreen.text("“<i>Heh, you really have a thing for " + (telAdre.katherine.hasCock() ? "herms" : "girls in need") + ", don’t ya?</i>” she says with a smile.  “<i>Not that I’m complaining.  Anyway, in terms of her work she still needs to brush up on the laws of the land and a few other things, but she’s doing fine.  Plus it’s nice to have a cat in the watch.  I had thought about saving her for undercover work, but I figure it’s better for everyone to see a cat who’s a good upright citizen.  There really aren’t that many and maybe some of the ones in gangs will rethink things when they see her on patrol.</i>”");
		}
		else if (affection < 11) { //They’ve had drunken sex with you (can't get to this point in friend mode)
			MainScreen.text("“<i>Yeah Kath.  Well I can see why you like her.  Hot girl" + (telAdre.katherine.hasCock() ? " with a decent sized cock" : "") + ", gets a bit horny when she’s drunk.  What’s not to like?</i>”\n\n");
			MainScreen.text("She takes a swig of her drink and gives you a hug.  “<i>So, I just want to say I’m not unhappy having sex with you.  I mean that’s always great.  But, um, I gotta admit it’s really fun when you get the three of us together.  If you decide to do that again ... I’m in.</i>”");
		}
		else if (affection < 32) { //Willing to have sober sex
			MainScreen.text("“<i>Mmmm yeah Kath,</i>” she says dreamily, then shakes her head and laughs.  “<i>Guess that about sums things up, huh?  Yeah I like Kath, I like her quite a bit.  She’s a good watch officer and she’s a good lover in a threesome.</i>”  Urta looks a little nervous, but presses on.  “<i>So... what do you think of the two of us, when you’re not around, you know?</i>”\n\n");
			MainScreen.text("You could tell Urta not to have sex with Katherine except when you’re around, tell her it’s alright for them to have sex whenever they want or encourage them to fuck each other’s brains out.");
			simpleChoices("Don't Fuck", urtaKathSexDont, "Whenever", urtaKathSexWhenever, "Encourage", urtaKathSexEncourage, "", null, "", null);
			return;
		}
		else { //Lovers
			MainScreen.text("“<i>" + player.short + ", I can honestly say I never expected to have this much sex.  I love you, I love Katherine.  It’s a bit weird, but I’m not complaining and neither is she and I hope neither are you.  I’m not the same girl you found sitting alone in the bar.  I’m so happy now.</i>”");
		}
		doNext(camp.returnToCampUseOneHour);
	}

	private urtaKathSexDont(): void {
		MainScreen.text("You give Urta a smile tell her that while you’re happy for her you’d really prefer if she waited for you to be around before banging Katherine.  You feel a little left out.\n\n");
		MainScreen.text("Urta gives you a weak smile and says, “<i>Alright " + player.short + ", I understand.  I’ll keep it in my pants.  Course I hope you’re up for a lot of three ways.  Girl’s got to get her fix.</i>”");
		Flags.list[FlagEnum.KATHERINE_URTA_DATE] = Katherine.KDATE_LITTLE;
		doNext(urtaDialogueMenu);
	}

	private urtaKathSexWhenever(): void {
		MainScreen.text("You put your arm around Urta and tell her that you don’t mind if your favorite girls need to blow off a little steam together.  As long as they don’t wear each other out that is.\n\n");
		MainScreen.text("Urta lets out a relieved laugh and says, “<i>That’s good.  That’s good.  I’ll make sure not to wear our kitten out.</i>”");
		Flags.list[FlagEnum.KATHERINE_URTA_DATE] = Katherine.KDATE_WHENEVER;
		doNext(urtaDialogueMenu);
	}

	private urtaKathSexEncourage(): void {
		MainScreen.text("You ask Urta what she would say if you told her you flat out expect them to have sex when you’re not around.\n\n");
		MainScreen.text("“<i>Are you sure " + player.short + "?  I mean I love the idea, I’m sure Kath will love it too, but you know what my appetite’s like.</i>”\n\n");
		MainScreen.text("You give her a kiss and tell her you know very well.  Since you’re not in town all the time you expect Urta to see to Kath’s needs and Kath to hers.\n\n");
		MainScreen.text("“<i>Carte blanche?  Ok " + player.short + ", but I’m warning you - your sex kitten is going to be very well fucked whenever you see her.</i>”");
		Flags.list[FlagEnum.KATHERINE_URTA_DATE] = Katherine.KDATE_LOTS;
		doNext(urtaDialogueMenu);
	}
	...
	public giveTheFoxSomeEggs(): void {
		...
		if (Flags.list[FlagEnum.PC_FETISH] > 0) MainScreen.text("as much fun as it might be, ");
		...
		if (player.lowerBody.isTaur()) MainScreen.text("positioning your centaur, ");
		else if (player.lowerBody.isDrider()) MainScreen.text("drider body over the bulk of her back, ");
		MainScreen.text("and release your ovipositor from its usual hiding slit, already dripping with ");
		if (player.perks.has("SpiderOvipositor")) MainScreen.text("green slime");
		else MainScreen.text("honey");
		MainScreen.text(" in anticipation of laying.  The appendage droops down, curling around to slide its wet length against Urta's balls and the base of her cock.  \"<i>[name], wha...?</i>\" she slurs at you, but you just gently shush her and tell her to relax.  With a little effort, you bring it slithering back up to pry at the dampness of her netherlips...");
		...
		if (player.gender > 0) {
			...
			if (player.gender == 3) MainScreen.text(" are ");
			else MainScreen.text(" is ");
			MainScreen.text("starting to grow wet with arousal, matting her back with your sexual fluids, but you ignore them, too caught up in your need of release to bother with conventional pleasures.");
		}
		else MainScreen.text("  The entirety of your sexual world begins and ends with your ovipositor, with the sensation of the mock-cock burrowing inexorably towards Urta's womb overwhelming you with pleasure.  Having no other sexual organs to distract you allows you to fully embrace it; you thrust harder and harder, desperate to ensure you have reached the womb before you start to lay.");
		...
		if (player.perks.has("BeeOvipositor")) MainScreen.text("honey");
		else MainScreen.text("spider-goo");
		MainScreen.text(" to keep Urta moist and slick and pliable.");
		...
		if (player.lowerBody.vaginaSpot.hasVagina()) wetness += player.lowerBody.vaginaSpot.get(0).vaginalWetness * 100;
		if (player.lowerBody.cockSpot.hasCock()) wetness += player.cumQ();
		if (wetness < 300) MainScreen.text("splattering");
		else if (wetness < 500) MainScreen.text("matting");
		else if (wetness < 1000) MainScreen.text("soaking");
		else MainScreen.text("flooding");
		MainScreen.text(" the fox with your gushing ");
		if (player.gender == 1) MainScreen.text("cum");
		else if (player.gender == 2) MainScreen.text("femcum");
		else MainScreen.text("mixed sexual fluids");
		MainScreen.text(".");
		...
		if (player.gender > 0) MainScreen.text(" just as her back is from your own release");
		MainScreen.text(".");

		//[Egg Level 1:
		if (player.eggs() < 20) MainScreen.text("  Urta's stomach starts to swell as your clutch of eggs fills her once-empty womb, stretching into a noticeable bulge, though not one that she couldn't hide if she wanted.");

		//[Egg Level 2:
		else if (player.eggs() < 40) MainScreen.text("  As the eggs keep on coming, Urta's womb swells and bulges, crammed full of goo and eggs, growing larger and larger until Urta could easily pass for an expectant mother, ready to give birth any day now.");

		//[Egg Level 3:
		else MainScreen.text("  You stuff Urta with eggs until you're wondering how many either of you can hold. Her belly just keeps getting rounder and rounder even as your eggs keep coming; soon, not only does she look like a woman pregnant with multiples, her skin is visibly stretched over the eggs, giving it a distinctively lumpy look rather than the round, smooth bulge she might have if your load was smaller.");
		...
		Flags.list[FlagEnum.URTA_EGGS] = player.eggs();
		Flags.list[FlagEnum.URTA_FERTILE_EGGS] = player.fertilizedEggs();
		Flags.list[FlagEnum.URTA_TIMES_EGG_PREGGED]++;
		if (player.perks.has("BeeOvipositor"))
			pregnancy.knockUpForce(PregnancyType.BEE_EGGS, 72);
		else
			pregnancy.knockUpForce(PregnancyType.DRIDER_EGGS, 72);
		Flags.list[FlagEnum.URTA_FLATBELLY_NOTICE] = 0;
		//First time, tag for triggering freakout!
		if (Flags.list[FlagEnum.URTA_EGG_FORCE_EVENT] == 0) Flags.list[FlagEnum.URTA_EGG_FORCE_EVENT] = 48;
		player.dumpEggs();
		player.orgasm();
		doNext(camp.returnToCampUseOneHour);
	}
	...
	public urtaChewsOutPC(newScreen: boolean = true): void {
		if (newScreen) MainScreen.clearText();
		else MainScreen.text("\n");
		MainScreen.text("Urta looks up at you and snorts fiercely; you almost expect steam to come flaring out of her nostrils, and her livid eyes are doing their best to burn holes straight through you.  \"<i>There you are, [name]!  You have some nerve!</i>\"");
		...
		if (player.stats.cor > 50) MainScreen.text(", especially given this may be the closest she'll ever get to experiencing pregnancy");
		MainScreen.text("?");
		...
		//[Unfertilised Eggs:
		if (Flags.list[FlagEnum.URTA_FERTILE_EGGS] == 0) {
		...
		if (player.perks.has("SpiderOvipositor")) MainScreen.text("spider");
		else MainScreen.text("little bee");
		MainScreen.text("...</i>\"\n");
		//(Technically finished, now, but waiting Fen's okay before adding repeatable version)
		Flags.list[FlagEnum.URTA_EGG_FORCE_EVENT] = -1;
	...
	private repeatUrtaEgging(): void {
		MainScreen.clearText();
		MainScreen.text(images.showImage("urta-egg-repeat"), false);
		MainScreen.text("With a smile, you ask if Urta would be willing to let you lay a few eggs in her womb again?");
		...
		if (!urtaLove()) MainScreen.text("blatantly hanging out of her stockings and drooling pre-cum everywhere");
		else MainScreen.text("vulgarly bulging her dress and rendering it sopping wet from her arousal");
		MainScreen.text(", before sashaying in a drunkenly exaggerated fashion out towards the alleyway where you and she first began your relationship- and where you first 'impreggnated' her.");
		...
		MainScreen.text("\n\nBut it's a far different need that is driving you now, and you can't be bothered to engage in much foreplay.  You straddle the sloshed fox, ");
		if (player.lowerBody.isTaur() || player.lowerBody.isDrider()) {
			if (player.lowerBody.isTaur()) MainScreen.text("positioning your centaur");
			else MainScreen.text("drider body over the bulk of her back");
			MainScreen.text(", ");
		}
		MainScreen.text("and release your ovipositor from its usual hiding slit, already dripping with ");
		if (player.canOvipositBee()) MainScreen.text("honey");
		else MainScreen.text("green slime");
		...
		if (player.gender > 0) {
			...
			if (player.gender == 3) MainScreen.text(" and ");
			if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("[vagina]");
			MainScreen.text(" are starting to grow wet with arousal, matting her back with your sexual fluids, but you ignore them, too caught up in your need of release to bother with conventional pleasures.");
		}
		...
		MainScreen.text("\n\nEgg after egg rolls down your ovipositor, each eliciting a gurgling splurt of cum from Urta's immense dick as it gushes into her, ensuring  the ground under her is soaked with sexual fluids");
		if (wetness > 0) MainScreen.text(" just as her back is from your own release");
		MainScreen.text(".");
	...
	private condomlessUrtaInHouseSmex(): void {
		MainScreen.clearText();
		MainScreen.text("You grab hold of the pre-filled bubble atop the fox's tip and yank, earning a moan as the latex sheath slides free, stretching it to an oblong mass before freeing her flare with an audible pop.  At the same time, you draw the second, unused condom from her unresisting grip and say, \"<i>Why don't we make a little mess?</i>\"");
		//non lover:
		...
		menu();
		if (player.lowerBody.cockSpot.hasCock()) {
			if (player.cockThatFits(urtaCapacity()) >= 0) MainScreen.addButton(0, "Peg Her Ass", pegUrtasAssMessy);
			else MainScreen.text("\n\n<b>You're too big to peg her ass and make her bust a nut all over herself.</b>");
		}
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.addButton(1, "Fill Me Up", fillMeUpPleaseUrta);
		if (player.gender > 0 && player.lowerBody.isNaga()) MainScreen.addButton(2, "TailFun", nagaOnUrtaMess);
		MainScreen.addButton(4, "Back", goBackToUrtasForLuvinz);
	}
	//[Peg Her Ass]
	private pegUrtasAssMessy(): void {
		MainScreen.clearText();
		MainScreen.text(images.showImage("urta-home-pegurta"), false);
		let x: number = player.cockThatFits(urtaCapacity());
		if (x < 0) x = player.smallestCockIndex();
		MainScreen.text("Fondling the fox's apple-sized balls, you give her a hard but affectionate squeeze, rolling the heavy orbs around in your palm as you lift them high.  With her stuffed sack out of the way, Urta's oozing black-lipped gash is on full display, with engorged labia soaked in lubricants that had nowhere to go thanks to her smotheringly large scrotum.  Just below, you can see her anal star tightening and relaxing with each of the vixen's heavy breaths.  To you, it looks almost hungry, panting like an animal.");
		MainScreen.text("\n\nRemoving your [armor] one-handed, you reveal your body and your intentions, flopping ");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("a ");
		else MainScreen.text("your ");
		MainScreen.text(CockDescriptor.describeCock(player, x) + " down onto the sheets, a scant few inches from Urta's puckered asshole.  The fox ");
		if (urtaDrunk()) MainScreen.text("giggles drunkenly, \"<i>You're sho dirty!</i>\" while spreading those fishnet-clad legs further.");
		else if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] < 11) MainScreen.text("softly begs, \"<i>Be gentle, please,</i>\" while spreading those fishnet-clad legs a little further apart.");
		else MainScreen.text("eagerly coos, \"<i>Feeling a little dirty, eh, lover?  Go on baby, you can fuck my ass anytime.</i>\"  She spreads her fishnet-clad legs so wide they almost form an unbroken, horizontal line from padded toe to padded toe, tying her soft, poofy tail around a leg to keep it out of the way.");
		MainScreen.text("  Scooting forward, you edge your " + player.cockHead(x) + " up to her anus.  Her body heat washes over you as you enjoy the feel of her pucker's folded flesh pressing down on your most sensitive point, lubricated by the copious female fluid that drains from the sodden gash above.");
		...
		if (urtaDrunk()) MainScreen.text("drunken ");
		MainScreen.text("whimpers of pleasure.  However, you don't wait long, slowly pushing forward and exploring your vixen's every inner crevice with your eager rod.");
		MainScreen.text("\n\n\"<i>It's ");
		if (urtaDrunk()) MainScreen.text("sho");
		else MainScreen.text("so");
		MainScreen.text(" different than with a condom over a toy,</i>\" the vulpine lass wonders aloud in delight.  She licks her lips and ");
		if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > 15) MainScreen.text("casually");
		else if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > 11) MainScreen.text("confidently");
		else if (Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > 4) MainScreen.text("a little nervously");
		else MainScreen.text("with a shaky hand");
		MainScreen.text(" strokes a padded fingertip down the bulging length of her urethra, shivering when it bumps across the sensitive medial ring.  A few inches in, you feel a rounded bulge bump into you, and as soon as you press against it, Urta's cock lifts up and thickens, flooded with blood.  The fox trembles for a second as you glide by the obstruction, and without any warning at all, her shaft belches a thick spurt of clear pre-cum with enough force to glaze the underside of her tits.");
		MainScreen.text("\n\nSmiling at the lascivious display, you gather a handful of the balmy animal cream and circle it around one of her perky nipples, admiring how the black nub perks up into a puckered, sensitive tower.  Then, while still pushing into her, you repeat the action on the other side, with identical result.  Your " + CockDescriptor.describeCock(player, x) + " milks another heavy flow from the fox's prostate, this one tinged with just a little bit of off-white.  It puddles on her belly, already forgotten.  You're pushing forward without much worry for her comfort now, and blessedly, the fox doesn't seem to mind.  Indeed, with every lurch forward, her twat's drippings come faster and faster, matched by even larger discharges from the turgid member above.  ");
		if (player.cockArea(x) > urtaCapacity()) MainScreen.text("The aching pressure around your " + CockDescriptor.describeCock(player, x) + " doubles, and you realize she simply can't take any more.  Her anus is stretched wide, dilated to the extreme to accommodate this much of you.  There's little to do but plow her with what you can until she's fountaining white.");
		else if (player.hasKnot(x)) MainScreen.text("The pleasure seems to double when your knot butts up against her furry rump, but her asshole isn't ready for that just yet.  This bitch needs a little more loving before you knot her.");
		else {
			MainScreen.text("The aching pleasure around your " + CockDescriptor.describeCock(player, x) + " redoubles as you hilt her, smashing your ");
			if (!player.lowerBody.cockSpot.hasSheath()) MainScreen.text("groin ");
			else MainScreen.text("sheath ");
			MainScreen.text("against her taut asshole with a heavy lurch.");
		}
		MainScreen.text("\n\nGroping the vixen's tits and thumbing her nipples, you have your way with her chest, torturously teasing her while your " + CockDescriptor.describeCock(player, x) + " rests immobile in her ass.  Her pussy-slicked sack rests against you, lewdly bouncing and pulsating with need, fairly ready to explode; her cock is equally aroused.  The rigid length is so hard that it hovers over her smooth abdominals without touching them, bobbing up and down while it exudes a steady drizzle of lust.  The pre-spunk is so thick on Urta's belly that her fur is matted and glossy from her hips to the underside of her tits, and ribbons of the stuff run down her sides to lubricate the sheets below.  Her hands push yours into her chest harder, smashing her tits almost flat against her.  At the same time, her hips squirm and writhe, grinding in lewd little circles against you, swirling your dick up against every part of her sensitive tail-hole.");
		...
		if (player.cockArea(x) > urtaCapacity()) MainScreen.text("most of you ");
		else MainScreen.text("your entire length ");
		MainScreen.text("before she can react, clinging to every inch of cock-flesh as it's forced deeper into her interior.  A wave of ecstasy washes through the fox, slackening her muscles in a ripple that travels from her waist to her lips in a second.  Her annoyance morphs into slack-jawed pleasure, and her protests deteriorate into babbling vocalizations of enjoyment.  Before she can recover, you pull back and thrust again, battering her sphincter repeatedly as you begin to truly pound the fox's ass.");
		MainScreen.text("\n\nUrta moans, \"<i>Fuck me!  Fuck my ass, [name]!  Gods");
		if (urtaDrunk()) MainScreen.text("h");
		MainScreen.text(", why does it feel s");
		if (urtaDrunk()) MainScreen.text("h");
		MainScreen.text("o goooo-ooooooh yes!</i>\"  Her ass clutches and relaxes to your tempo, hugging your " + CockDescriptor.describeCock(player, x) + " tight");
		if (player.lowerBody.balls > 0) MainScreen.text(" while your [balls] slap against her fuzzy buttcheeks.");
		else MainScreen.text(" while you slap your groin against her fuzzy buttcheeks.");
		...
		if (urtaDrunk()) MainScreen.text(" and of course, alcohol");
		MainScreen.text(", kissing your " + player.cockHead(x) + " every time it peeks past her tits, relishing the taste of her own lust all over it.");
		...
		if (player.cumQ() < 250) MainScreen.text(", swallowing with a smile and a wink.");
		else {
			MainScreen.text(", but she's forced to close it when her cheeks are puffed beyond reason.");
			if (player.cumQ() > 750) MainScreen.text("  You go on to wash her face with your own white sauce.");
			if (player.cumQ() >= 1200) MainScreen.text("  Your orgasm goes on and on, coating just about everything that Urta came on with yet another layer.");
			if (player.cumQ() > 3000) {
				MainScreen.text("  She gasps in surprise when you flood the ");
				if (player.cumQ() > 10000) MainScreen.text("first few feet of the ");
				MainScreen.text("room, looking up at you in admiration through cum-coated eyelashes.");
			}
		}
		MainScreen.text("\n\nAs you finish emptying your [balls], you sag down into the fox's arms.  She plants a salty kiss on your lips ");
		if (urtaLove()) MainScreen.text("and nuzzles against your cheek, just holding you for a few moments, lover to lover.");
		else MainScreen.text("and sheepishly pulls back.");
		MainScreen.text("  Climbing out of the alabaster-painted bed is no easy task, but the two of you manage, laughing and leaning on each other's arms.  Urta leads you to a cabinet in another room, and bends over to open it.  You barely notice her pulling out mops, buckets, and cleansers.  Instead, your vision is riveted to her asshole, still gaped lewdly in the shape of your " + CockDescriptor.describeCock(player, x) + ".  When she comes up with an armload of cleaning supplies, you're ");
		if (player.stats.cor < 50) MainScreen.text("blushing");
		else MainScreen.text("smiling devilishly");
		MainScreen.text(".");
		...
		//[Next]
		menu();
		MainScreen.addButton(0, "Next", UrtaGetsPeggedCleanUp);
	}
	private UrtaGetsPeggedCleanUp(): void {
		MainScreen.clearText();
		if (player.stats.cor >= 50) MainScreen.text("You start to duck out, but the disapproving glare the fox gives you makes it clear that you won't be getting any more messy fun unless you stick around to clean it up this time.  Damnit.  ");
		MainScreen.text("With Urta's help, you slowly clean up the mess you two made, nearly clogging the plumbing a few times in the process.  New sheets, pillows, and furs are brought in, and you help her fix her bed.  The floor is mopped, the walls are scrubbed, and the shutters are opened to let in some fresh air.  Thankfully, just about everything is watertight and sealed, likely something she had done to ease situations such as this.  All told, between clean-up and sex, you spend a few hours with her.");
		MainScreen.text("\n\nTenting her apron (the only thing she's bothered to put on), Urta gives you a ");
		if (!urtaLove()) MainScreen.text("wave");
		else MainScreen.text("tender hug");
		MainScreen.text(" and sends you on your way.");
	...
	private fillMeUpPleaseUrta(): void {
		MainScreen.clearText();
		MainScreen.text(images.showImage("urta-home-female-fucked"), false);
		MainScreen.text("Lazily discarding the latex, you caress Urta's stocking-clad leg and coyly climb into bed.  She starts to reciprocate your affections, but you push her arm away, instructing, \"<i>Let me... let me have this.</i>\"");
		...
		if (player.lowerBody.vaginaSpot.get(0).vaginalWetness > 3) MainScreen.text("  You're so utterly soaked thanks to this place that a steady stream of vaginal lubricant drizzles over the meaty tool below.");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  Hard and erect, [eachCock] stands tall and proud, but ignored.  You have other plans.");
		MainScreen.text("  She's ready.  You're ready.  It's time.");
		...
		if (player.lowerBody.vaginaSpot.get(0).vaginalWetness > 3) MainScreen.text(", though it's a drop in the bucket next to your copious moisture.  Hell, even now, rivers of your girl-goo are sliding down that spasming length, wreathing it in lady-spunk");
		else MainScreen.text(", deliciously staining your slit");
		MainScreen.text(".");
		//{Virgin PCs}
		if (player.lowerBody.vaginaSpot.get(0).virgin) {
			...
			if (player.tallness > 80) MainScreen.text("[clit]");
			else if (player.tallness > 60) MainScreen.text("belly");
			else MainScreen.text("[chest]");
			MainScreen.text(" as she cranes her neck to meet you, eye to eye.  \"<i>Really?</i>\"  You nod, biting your lower lip as you continue your frenzied undulations upon her rod, working yourself up to taking her.");
			...
			if (urtaDrunk()) MainScreen.text("drunkenly ");
			MainScreen.text("asks, \"<i>Are you s");
			if (urtaDrunk()) MainScreen.text("h");
			MainScreen.text("ure?  You don't have to...</i>\"");
			...
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  Drool begins to leak from [eachCock], slicking the underside with pre-cum as the pleasure affects your maleness.");
			MainScreen.text("  Before long, you've taken Urta halfway, with her ring jutting up against your entrance.");
			//{cuntChange 30}
		...
		if (Flags.list[FlagEnum.PC_FETISH] > 0) MainScreen.text("  An exhibitionist thrill runs through you at that, making you even wetter.");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  You don't even notice [eachCock] dripping on Urta's belly.");
		//HERMS:
		if (player.lowerBody.cockSpot.hasCock()) {
			...
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(", followed in short order by the untouched portion of your " + CockDescriptor.describeMultiCockShort(player));
			MainScreen.text(".  The next is your [vagina] going wild, convulsing from your entrance towards your womb with muscular contractions that milk Urta's cock relentlessly, causing it to grow bigger and harder inside you.  Urta whimpers slightly as you jizz over her belly");
			if (player.cumQ() > 400) {
				MainScreen.text(", soaking her");
				if (player.cumQ() > 1000) {
					if (player.cumQ() < 2000) MainScreen.text(" and her bed");
					else MainScreen.text(", her bed, and damn near everything else");
				}
				MainScreen.text(" with your ejaculate");
			}
			MainScreen.text(".  There's a bloom of warmth from below as the vixen's orgasm rushes towards you, and all you can do is cum your brains out in anticipation of being filled.");
		}
		//NON-HERMS:
		else {
		...
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 4) MainScreen.text(" and your [chest] rest heavily upon your cum-stuffed form");
		MainScreen.text(".");
		if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 150) MainScreen.text("  You may have been pregnant before, but you're beyond pregnant now... with cum AND child.");
		//{Goo:
		if (player.lowerBody.isGoo() && player.vaginalCapacity() > 9000) MainScreen.text("  Successive eruptions make you bigger, but your flexible cunt soon grows so bloated that the onrushing injections mostly squirt back out, painting Urta, the bed, and your [legs] white with cum.");
		else MainScreen.text("  Successive eruptions do little to bloat you further - your body is stretched to capacity, so the onrushing injections only squirt back out through your entrance, painting Urta, the bed, and your [legs] white with cum.");
		...
		if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 150) MainScreen.text("even more ");
		MainScreen.text("pregnant for a while, at least until you can pass all that filling!  She plants a salty kiss on your lips ");
		if (urtaLove()) MainScreen.text("and nuzzles against your cheek, just holding you for a few moments, lover to lover.");
		else MainScreen.text("and sheepishly pulls back.");
		...
		if (player.stats.cor < 33) MainScreen.text("blushing");
		else if (player.stats.cor < 66) MainScreen.text("grinning");
		else MainScreen.text("openly leering");
		MainScreen.text(".");
		...
		//[Next]
		menu();
		MainScreen.addButton(0, "Next", cleanUpAfterUrtaCreamInjectsYou);
		Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] += 2;
		urtaLove(2);
		Flags.list[FlagEnum.TIMES_FUCKED_URTA]++;
		player.slimeFeed();
		if (Flags.list[FlagEnum.URTA_FERTILE] == 1) player.knockUp(PregnancyType.URTA, PregnancyType.INCUBATION_URTA, 25);
		player.orgasm();
		player.stats.lib += .2;
		player.stats.sens += -3;
	}
	...
	private lippleBondUrtaScylla(): void {
		...
		//[if (cocks > 1)
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("  Without much warning, you suddenly feel Urta's hand move away from your own palm, the sensation of your [cock smallest] being wrapped by her fingers also assaulting you as she squeezes and tugs at your second cock affectionately.");
		//[if (cocks > 2)
		if (player.lowerBody.cockSpot.count() > 2) MainScreen.text("  The barrage of sensations is intensified when Scylla also gets a bit greedy, grabbing another tool and starting to jerk it off as well.");
		...
		//[if (cocks > 1)
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" either");
		MainScreen.text(" the holy woman's ravenous lipples and inner, erotic tit-flesh in a mutually ecstatic, sensations-filled manner");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" or more delicately and less hungrily with a feminine hand");
		MainScreen.text(".");
		...
		//if (sensitivity < 29)
		if (player.stats.sens < 29) MainScreen.text("  You can only tell so from her expression and from Scylla moaning and gasping sharply, but it appears Urta's cock is really just about ready to shoot off cum.");
		//if (sensitivity > 29)
		else MainScreen.text("  Scylla's titflesh slightly shifts from what is apparently the expansion of Urta's cock flare and the twitching of her massive shaft.  You're sensitive enough to notice.");
		//[if (sensitivity > 44)
		if (player.stats.sens >= 44) MainScreen.text("  In fact, you can clearly feel the pressure inside Scylla's chest increasing as Urta's huge cock expands and twitches inside her next to your own, each such twitch sending a shiver of pleasure through both you and the nun.");
		...
		//[If Cock type = Canine]
		if (player.hasKnot(player.biggestCockIndex())) MainScreen.text("  You're not making the problem any easier, in fact, your knot is halfway stuck inside her breast, surged and filled to full size right after your orgasm.");
		...
		if (player.hasKnot(player.biggestCockIndex())) MainScreen.text(", your knot sufficiently deflated now");
		MainScreen.text(", now sucking desperately at the air, looking for the tasty dick.");
	...
	private nagaOnUrtaMess(): void {
		...
		//[if (cocks = 0)
		if (player.lowerBody.cockSpot.count() == 0) MainScreen.text("  Urta runs her fingers over and gently pushes into your now revealed female slit. \"<i>My lover's sweet, sweet pussy. The best in the whole world.</i>\"");
		//if (cocks = 1)
		else if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("  \"<i>And here is my favorite cock in the whole world... Probably including mine,</i>\" she giggles and winks at you.");
		//if (cocks = 2)
		else if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("  \"<i>Two delicious cocks, rather than just one...  You're trying to tell me you're the man here, aren't you, lover?</i>\" she giggles and winks at you.");
		else if (player.lowerBody.cockSpot.count() == 3) MainScreen.text("  \"<i>You want to have a spare one just in case or something?</i>\" she giggles and winks at you.");
		//[if (cocks >= 4)]
		else MainScreen.text("  \"<i>Interesting.  It looks like a bouquet.  A beautiful bouquet of cocks just for me,</i>\" she giggles and winks at you.");

		MainScreen.text("  Without further ado, Urta leans towards your sex to return the favor, her mouth pleasuring you just as you wrap more and more of your slightly looser coils around her member, having released her legs to allow her to move... and to allow your tail's tip to reach her needy vulpine pussy more easily from that position.");
		...
		//MainScreen.text("  However, it requires no justification that your herm lover is further on her way to an orgasm then you are.");
		...
		if (player.lowerBody.vaginaSpot.hasVagina()) {
			...
			//[if (isSquirter = true)]
			if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 4) MainScreen.text("  After a few convulsions, your femcum starts to shoot out of your pussy quite violently, adding to the mess your lover is making.");
		}
		//if (cocks > 0)
		if (player.lowerBody.cockSpot.count() > 0) {
			...
			//[if (cumHigh = true)]
			if (player.cumQ() >= 1000 && player.cumQ() < 2500) MainScreen.text("  Her eagerness does end up giving her a swollen tummy, but she's loving and hungry for you until the end, swallowing as much as she can, as quickly as she can.");
			//(if [cumExtreme = True]
			else if (player.cumQ() >= 2500) MainScreen.text("  Of course, your excessive load proves too much for the poor vixen to handle, and soon she is not only inflated, but painted in your cum as well, the mess the two of you are making definitely nothing to be scoffed at.");
		}
	...
	private lickOutUrtaAtHome(): void {
		...
		MainScreen.text("\n\nYou climb overtop of Urta and allow your " + CockDescriptor.describeCock(player, x) + " to press on into her pussy.  ");
		if (player.cockArea(x) >= urtaCapacity() / 2) MainScreen.text("It fits you like a lubricated, stretchy little glove, ensuring every movement will caress her folds.");
		else MainScreen.text("It's nice and roomy, allowing you full control over how you want to press on her inner folds.");
		MainScreen.text("  You push the rest of the way into her cunt and admire the velvety texture of her walls as they close in around you, inner muscles flexing and massaging your " + CockDescriptor.describeCock(player, x) + " as she adjusts to your intrusion.");
		if (player.cockArea(x) >= 200) MainScreen.text("   Stretching ludicrously, her cunt dilates with surprising ease.  The elastic lips of her pussy cling oh-so-tightly to your girth as they're spread beyond reason.  Her hips even spread with only a modest bit of pressure and resistance as her passage is distorted, the shape of your " + CockDescriptor.describeCock(player, x) + " visible through her stretched belly as it pushes towards her tits.  You can even make out some of the larger veins.");

		MainScreen.text("\n\nThe hermaphrodite lifts one of her ebony nipples up to her midnight-hued lips and suckles it eagerly, writhing in the delight.  You meet her hungry eyes as you gently begin to slide back and forth, reveling in the liquid-hot heat of the vulpine woman's snatch with a lusty moan, pumping your " + CockDescriptor.describeCock(player, x) + " in and out over and over.  Her secretions are running unchecked onto the bedsheets now, and neither of you care.  You grab hold of Urta's neck to pull her away from her nipple so that you can kiss her on the lips, and you flick your tongue into her mouth.  She demurely accepts the passionate tonguing before allowing her own to slide into yours, slithering to the tempo your [hips] set.");
		...
		if (player.stats.cor < 66) MainScreen.text("You're wonderful");
		else MainScreen.text("You're the most amazing fuck");
		MainScreen.text("</i>\"");
		...
		if (player.cumQ() >= 500) MainScreen.text("  Spermy juices run out of her pussy in alabaster streams thick enough to obscure the black-lips they're travelling over.");
		if (player.cumQ() >= 1500) MainScreen.text("  The fox gives a surprised grin when her belly rounds slightly, though she seems concerned at how it makes her cum-filled prize wobble.");
		if (player.cumQ() >= 3000) MainScreen.text("  Your virile pecker just won't stop, and by the time you finish ejaculating, Urta is pregnant with sperm, her belly round with its belly-button protruding.");

		MainScreen.text("\n\nSharing an intimate moment with your ");
		if (urtaDrunk()) MainScreen.text("drunken ");
		MainScreen.text("lover, you kiss her again.  She returns it demurely, but midway through, you feel something slick and hard brushing up against you.  Urta whispers, \"<i>Sorry,</i>\" when you pull away to examine the sudden change.  Her pole is hardening again, surging up to rub the full prophylactic against your [chest].  You glance back at her, and all Urta has to say for herself is, \"<i>Sorry, lover... feeling you cum inside me is wonderful, and well, I had time to recover.</i>\"  She slowly pulls the rubber off her cock, careful not to jostle it too hard.  A second later, it's tied off and set aside.  With you still ");
		if (player.hasKnot(x)) MainScreen.text("knotted");
		else MainScreen.text("resting");
		MainScreen.text(" inside her juicy cunt, she asks if she needs another one, batting her eyelashes playfully.");

		MainScreen.text("\n\nShaking your head from side to side, you pull back");
		if (player.hasKnot(x)) MainScreen.text(" in an attempt to withdraw.  Her entrance dilates slowly as your full, dog-like knot is pressed against her from inside, the pulsating, reddish flesh glistening with the mixed sexual lubricants.  You tug and tug, rocking your [hips] slowly to loosen her up, and eventually, you manage to pop out, drawing a moan of mixed anguish and pleasure from Urta's maw.");
		else MainScreen.text(" and withdraw.");
		if (player.cumQ() >= 1500) MainScreen.text("   Cum runs out of her packed womb in a wave, spreading all over the sheets down to her knees.  She shakes slightly as the cum runs out of her, rubbing her full middle protectively.");
		else MainScreen.text("  Cum drools out of her still-gaping slit in steady trickles, puddling on the sheets below.  She groans happily.  Clearly the fox is proud to have such a productive lover.");
		MainScreen.text("  You take hold of the puffy, dick-stretched pussy lips and spread them around two fingers, collecting some of the warm sex-juice on your fingertips, then, you pull the dripping, creamy treat out and smear it over Urta's mouth.  She giggles as you apply the lip gloss to her and slowly runs her tongue across the shining coat of sex.  You respond by repeating the motion, this time laying it on thicker.");


		MainScreen.text("\n\n\"<i>This is all the lipstick you'll need today, babe,</i>\" you explain as you admire the way her cum-stained lips shine.  She's marked with your ejaculate and sure to have cock on her breath for hours.  Turning back to the glittering box below, you raise her balls and lean down under them, letting them fall into place on your " + HeadDescriptor.describeHair(player) + " once you're in position.  You blow on the dripping cunny, and Urta moans lustily in response.  Then, you promise, \"<i>You're going to be satisfied after this.  Your dick and cunt are gonna cum so hard you won't be getting it up for days.</i>\"");
		...
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] += 12;
	...
	internal function slurpFawkesCocksForFunAndInflation(): void {
		MainScreen.clearText();
		MainScreen.text(images.showImage("urta-home-bj"), false);
		MainScreen.text("\"<i>Thanks, Lover! I need this. I've been so busy, I haven't had any relief in days!</i>\" Urta informs you as she lightly strokes herself to full erectness. It's a short trip, and in a few pumps, she's ready to go. You glance around the bar, making sure all possible prying eyes are occupied. Feeling satisfied with your findings, you slyly shuffle your way under the table.");
		...
		if (player.pregnancyIncubation < 100 && player.pregnancyIncubation > 0) MainScreen.text(" than it already was");
		MainScreen.text(".  You slide your way back into your seat with some difficulty, fighting the wriggling, bouncing mass that is your stomach over the now shrinking room between the seat and table.  By now, some of the patrons have realized what has happened, pointing, whispering, rubbing their stomachs, and pulling their hands away from each other to symbolize your new... asset.");
		...
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 5;
		urtaLove(2);
		if (rand(2) == 0) MainScreen.text(player.modThickness(100, 2));
		if (rand(2) == 0) MainScreen.text(player.modTone(0, 2));
		doNext(camp.returnToCampUseOneHour);
	}
	...
	internal function urtaAndRaphaelSurprise(): void {
		MainScreen.clearText();
		MainScreen.text("Letting your lips split into a mischievious grin, you lean across the table and whisper into Urta's fluffy ear.  \"<i>Why don't you meet me in one of the back rooms with the lights off in about thirty minutes?  I'll see if I can get a friend, we can make a party of it.</i>\"");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" and [eachCock] experience a twinge of excitement");
		MainScreen.text(".  Grabbing a napkin and a pen from an accommodating waitress, you jot out a hasty missive:");
		...
		menu();
		MainScreen.addButton(0, "Next", raphaelAndUrtaPartTwo);
	}

	private raphaelAndUrtaPartTwo(): void {
		MainScreen.clearText();
		MainScreen.text(images.showImage("urta-raphael-3some", "vert"), false);
		MainScreen.text("Narrow, familiar paws suddenly encircle your waist, and a puff of hot breath exhales past your ear.  \"<i>Greetings, my supple swan.</i>\"");
		...
		if (player.lowerBody.tailType > TailType.NONE) MainScreen.text("lift your tails and ");
		MainScreen.text("try to relax with her thick slab of penis so close to your backdoor, hoping she'll be gentle.  A drunken twinge of arousal knifes through your [vagina] as you imagine her forcefully raping your hind-end, and Raphael can certainly feel the convulsive fluttering of your love tunnel as you sink into your imaginings.  The gray vixen nips at your ear as she presses against your [asshole]");
		if (player.analCapacity() < 30) MainScreen.text(", slowly stretching your tight sphincter with constant pressure.  You're not really loose enough back there for this to be entirely comfortable, but the incredible fullness her length provides has a way of muting the pain of stretching.");
		else if (player.analCapacity() < 60) MainScreen.text(", slowly sliding into your sphincter.  You groan into your other lover's mouth as your anus is claimed by the thick horse-pole, and you do your best to relax around the butthole-stretching shaft.");
		else MainScreen.text(", easily slipping inside your well-practiced anus.  You groan into your other lover's mouth as the thick cock claims your ready asshole, and you're glad your butthole was so well prepared to handle having a hot meat pole injected into it.");
		player.buttChange(60, true, true, false);
		...
		player.orgasm();
		player.stats.sens += -3;
		Flags.list[FlagEnum.URTA_X_RAPHAEL_HAPPENED]++;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private urtaGooTesticleVoreRuinedOrgasms(): void {
		MainScreen.clearText();
		MainScreen.text(images.showImage("urta-goo-fuck"), false);
		MainScreen.text("You tell your vixen lover that you've got something a bit... special in mind this time.  You surge up onto the bed and gently flip Urta onto her back.  Her horsecock bobs enticingly in front of you, already as hard as ");
		if (!silly()) MainScreen.text("a steel rod");
		else MainScreen.text("Marethian algebra");
		MainScreen.text(".");
		...
		//Next
		menu();
		MainScreen.addButton(0, "Next", urtasRuinedOrgasmsFromGooPartII);
	}
	...
	private urtasRuinedOrgasmsFromGooPartII(): void {
		...
		MainScreen.text("You awaken the next morning to a panicked shout.  You bolt upright...  Or at least you try to.  Something is holding you tight, binding you in place.  You try to open your eyes, but nothing but blackness surrounds you.  Struggling, you try and wrench yourself free of your bindings.");
		...
		let edrynAvailable: boolean = !kGAMECLASS.telAdre.edryn.pregnancy.isPregnant;
		if (edrynAvailable) MainScreen.text("set of hoofsteps");
		else MainScreen.text("pair of footsteps");
		MainScreen.text(" enter whatever room you're in. \"<i>Hey, Urta, you haven't shown up for your shift today so...  Great googily moogily!  What happened to you?!</i>\" a voice you ");
		if (edrynAvailable) MainScreen.text("recognize as Edryn's");
		else {
			if (Flags.list[FlagEnum.MET_OLIVIA] == 0) MainScreen.text("recognize as Olivia's");
			else MainScreen.text("don't recognize");
		}
		MainScreen.text(" shouts.");
		...
		if (edrynAvailable) MainScreen.text("Edryn");
		else MainScreen.text("Olivia");
		MainScreen.text(", I'm going to need your help with this particular problem.</i>\"");

		MainScreen.text("\n\nThere's a moment of silence, and then, \"<i>");
		if (edrynAvailable) MainScreen.text("All right, but I'm still charging you for this one");
		else MainScreen.text("Uhm, yes Captain, whatever you need");
		MainScreen.text(".</i>\"  You hear armor falling to the ground while Urta rises from wherever she was sitting; you've been all over the house so many times it's hard to keep track of where you are.");

		MainScreen.text("\n\nUrta wastes no time sinking her huge horsecock right into ");
		if (edrynAvailable) MainScreen.text("Edryn");
		else MainScreen.text("Olivia");
		MainScreen.text("'s cunt, if that wet squelching noise is any indication.  She thrusts all the way in inside in one go, making ");
		if (edrynAvailable) MainScreen.text("Edryn moan in pleasure");
		else MainScreen.text("Olivia shout with surprise");
		MainScreen.text(".  Clearly, Urta's not messing around anymore.  She pulls out and thrusts back in, and with that thrust you sway, slapping into ");
		if (edrynAvailable) MainScreen.text("Edryn");
		else MainScreen.text("Olivia");
		MainScreen.text("'s underside.  The contact is electric.  The rough slap would have sent shivers down your spine, if you'd had one.  It makes your body quake and convulse with gusto.");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("cock");
		if (player.gender == 3) MainScreen.text(" or ");
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("pussy");
		if (player.gender == 0) MainScreen.text("asshole");
		MainScreen.text(".  Your entire, gooey body feels like one giant clitoris, so sensitive, so vulnerable to pleasure.  With every slap, your body begins to crave release even more.  The pleasure builds with every thrust and accompanying slap.  It grows to the brink of orgasm, and surges forward, with no end in sight.");
		...
		MainScreen.text(".  Your eyes search around, seeing nothing but darkness once more.  You extend your gooey self, trying to tell where you are.  Wherever it is, it's tight... and there seems to be some sort of plug holding you in.");

		MainScreen.text("\n\n<b>You're inside ");
		if (edrynAvailable) MainScreen.text("Edryn");
		else MainScreen.text("Olivia");
		MainScreen.text("'s womb!</b>");
		...
		MainScreen.text(" voices her displeasure, \"<i>What the... I look like I'm pregnant!  With septuplets!</i>\"");

		MainScreen.text("\n\nUrta lets out a nervous chuckle.  \"<i>Sorry about that.  But this should be easier to deal with.  ");
		if (edrynAvailable) MainScreen.text("All you need to do is push, right?");
		else MainScreen.text("Let's just sit you back, and then all you need to do is push.");
		MainScreen.text("</i>\"  Your new host grunts in discomfort");
		if (edrynAvailable) MainScreen.text(", but obliges, sitting back on something, perhaps a bed.  You feel yourself sloshing around oddly as she lies on her back");
		MainScreen.text(".  \"<i>Now push,</i>\" Urta says, encouragingly.");
		...
		MainScreen.text(".  The first thing out, thankfully is your head.  Bright light greets your eyes, almost stinging them.  You look around, seeing ");
		if (edrynAvailable) MainScreen.text("Edryn clutching Urta's hand");
		else MainScreen.text("Olivia lying back on the bed, clutching Urta's hand");
		MainScreen.text(", panting heavily.  You surge outwards, first your shoulders, followed by one arm, and then the next.  The rest of your body comes out like a flood with one last push from your former host, sending you splattering onto the ground beneath you.  An exasperated, but relieved, sigh comes from ");
		if (edrynAvailable) MainScreen.text("Edryn");
		else MainScreen.text("Olivia");
		MainScreen.text(", one that you share.");
		...
		MainScreen.text("\n\nUrta and ");
		if (edrynAvailable) MainScreen.text("Edryn");
		else MainScreen.text("a newly dressed Olivia");
		MainScreen.text(" collapse onto the couch, exhausted.  \"<i>Gimme ten minutes, then we'll go on patrol,</i>\"  Urta says waving a hand tiredly.  You grin and take your leave, slipping out the front door and heading back to camp.  The sun blazes high in the sky, around noon, so you've lost about a day stuck inside one person or another.");
		player.orgasm();
		player.stats.sens += 2;
		model.time.days++;
		model.time.hours = 11;
	...
	private urtaPetPlayDeletedForeverBecauseThirdProovedMeWrongAboutDice(): void {
		MainScreen.clearText();
		//First Time
		if (Flags.list[FlagEnum.URTA_PETPLAY_DONE] > 0) {
			wearZeCollar();
			return;
		}
		MainScreen.text("You hold up the leather collar you found in the previous room, asking when she picked it up.");
		...
		menu();
		MainScreen.addButton(1, "Ignore", ignoreUrtasCollar);
		MainScreen.addButton(0, "Wear", wearZeCollar);
	}

	private ignoreUrtasCollar(): void {
		Flags.list[FlagEnum.URTA_PETPLAY_DONE] = -1;
		MainScreen.clearText();
	...
	private wearZeCollar(): void {
		...
		if (Flags.list[FlagEnum.URTA_PETPLAY_DONE] == 0) {
			...
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("harder");
			if (player.gender == 3) MainScreen.text(" and ");
			if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("wetter");
			MainScreen.text(" at the touch, but you can't help but wonder about her words.  What else is there to being a good dog?");
		...
		//Second Time
		else if (Flags.list[FlagEnum.URTA_PETPLAY_DONE] == 1) {
			...
			if (player.lowerBody.tailType > TailType.NONE) MainScreen.text(", tail wagging");
			MainScreen.text(".  Urta beams at you and pats your head lovingly.");
		}
		//(Jump down to Scene Proper)
		//Repeats
		else {
			MainScreen.text("You spy the collar sitting on a nearby dresser and pick it up.  You run your fingers over the engraved words on the collar's tag that reads '<i>Property of Urta</i>'.  Slipping it on, you also strip down until you're as nude as the day you were born, and kneel in front of your Owner");
			if (player.lowerBody.tailType > TailType.NONE) MainScreen.text(", tail wagging");
			MainScreen.text(".");
		}
		//Scenes Join Up Here
		...
		if (player.lowerBody.tailType == TailType.NONE) {
			...
			if (player.lowerBody.tailType == TailType.NONE && player.lowerBody.butt.analWetness > 0) MainScreen.text("  Despite your embarrassment, or perhaps because of it, you find your asshole getting wet with desire, practically salivating in anticipation of that thick plug.");
		...
		Flags.list[FlagEnum.URTA_PETPLAY_DONE]++;
		//[Doggystyle] ["Reward"] [Walkies! (Not available first time)]
		menu();
		MainScreen.addButton(0, "Doggystyle", doggyStyle);
		MainScreen.addButton(1, "\"Reward\"", getAPetReward);
		MainScreen.addButton(2, "Walkies", urtaTakesPCOnWalkies);
	}

	//Doggystyle
	private doggyStyle(): void {
		MainScreen.clearText();
		MainScreen.text(images.showImage("urta-home-petplay-doggy"), false);
		MainScreen.text("\"<i>Hmm,</i>\" Urta wonders aloud, bringing a finger up to her mouth in mock-thoughtfulness.  \"<i>What should I do with my little pet now that " + player.mf("he", "she") + "'s all collared up?</i>\"  Your Owner walks around you slowly, assessing your body.");
		...
		if (player.lowerBody.tailType > TailType.NONE) MainScreen.text("tail");
		else MainScreen.text("tailplug");
		MainScreen.text(" wags of its own accord, happily swishing back and forth.  Urta's hand dips between your buttocks, over your ");
		if (player.lowerBody.tailType > TailType.NONE) MainScreen.text("asshole");
		else MainScreen.text("plugged asshole");
		MainScreen.text(", and between your thighs, ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("grasping your [cock biggest]");
		else MainScreen.text("her fingers slipping ever-so-slightly into the folds of your pussy");
		MainScreen.text(".");

		MainScreen.text("\n\nYou moan appreciatively as she runs her fingers along it, her fingers encircling your ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("[cockHead biggest]");
		else MainScreen.text("[clit]");
		MainScreen.text(" while her other hand keeps your upper body firmly down.  With every stroke your body burns just a bit hotter, your breath coming a bit shorter.  ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("Her thumb rubs against the sensitive underside of your cock");
		else MainScreen.text("One finger slips into you, all the way down to the knuckle, and swivels around to caress your g-spot with expert accuracy");
		MainScreen.text(", making you bite your lip in ecstasy.  You want to cry out right now, but you know you can't break character right now.  Not this early in.  The strokes come faster, and you can feel the orgasmic flames coming on; your limbs tingle and your legs are already wobbling.");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("[cock biggest] does the same");
		else MainScreen.text("[vagina] grows slicker");
		MainScreen.text(".  She really is going to tease you as long as she can, you think to yourself.");

		MainScreen.text("\n\nThe first hand drifts from your belly back towards your ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("cock");
		else MainScreen.text("pussy");
		MainScreen.text(", taking over where it left off just moments ago.  ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("She wraps her fingers around your shaft, slowly pumping up and down, taking a moment to tease your [cockHead biggest] every few strokes.");
		else MainScreen.text("She buries two fingers deep inside your cunt, followed by a third, each one twisting around to rub at your g-spot softly.");
		MainScreen.text("  The hand on your chest switches to ");
		if (player.upperBody.chest.countNipples() == 2) MainScreen.text("the ");
		else MainScreen.text("an ");
		MainScreen.text("other nipple, tweaking and teasing that one while she simultaneously ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("jerks you off");
		else MainScreen.text("rubs you from the inside out");
		MainScreen.text(".  Again comes the flame of orgasmic bliss, lapping at your tingling extremities.  Your body feels suddenly electric, so overcharged with sweet, sweet pleasure that you find it hard to think.  Low moans escape your throat, entirely unbidden, while ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("pre-cum dribbles from your dick like a leaky faucet");
		else MainScreen.text("your folds grow even more damp, constantly drizzling fluids that run down your ass crack, pooling on the floor like a lewd lake");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(", and to top it all off, you find your [leg] inexplicably kicking in mid-air");
		MainScreen.text(".");
		...
		if (player.lowerBody.tailType == TailType.NONE) MainScreen.text(", while she grasps the plug in your ass firmly and pulls. It comes out easily with a wet <b>pop</b>, making your ass feel suddenly empty.  You guess it won't be empty for long, however");
		MainScreen.text(".  Finally she rubs it against your [asshole], teasing you with every brush of skin against skin.");
		...
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating <= 5) MainScreen.text("resting on your folded arms");
		else MainScreen.text("resting on your enormous boobs");
		...
		if (player.lowerBody.balls > 0) MainScreen.text("own");
		else if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("painfully erect member");
		else MainScreen.text("slit");
		MainScreen.text(", you are overwhelmed by the full, content feeling.  Your eyes roll back in their sockets, and it's hard to focus on anything but the twenty inch dick buried in your sensitive ");
		if (!silly()) MainScreen.text("ass");
		else MainScreen.text("boypussy");
		MainScreen.text(".");
		player.buttChange(60, true, true, false);

		MainScreen.text("\n\nSoon, Urta develops a good rhythm, thrusting in and out, her balls slapping your ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("dick");
		else MainScreen.text("cunt");
		MainScreen.text(" in such a way that it tingles powerfully, fueling your impending orgasm even more.  Locked in this cadence, she pounds your ass for what seems like hours.  The air grows damp and sweat drips from the two of you, only adding to your desire.  It's not long after until your nostrils are assaulted by the salty scent of sex.  Your ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("[cock] slaps against your belly roughly with every passing second, smearing thick globules of precum which drip down your belly, tickling your flesh like little balls of distilled pleasure");
		if (player.gender == 0) MainScreen.text(", while your");
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" unused pussy aches to be filled or touched, its lips puffy and red, sensitive to every hard slap of your foxy lover's balls");
		MainScreen.text(".");
		...
		if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text("groin, sending a wave of tremors down your cock, which suddenly erupts, covering your belly and [chest] in warm gooey goodness.");
			if (player.cumQ() >= 500) MainScreen.text("  It continues to jerk on its own, unloading its massive load until you're laying in a puddle of your own semen.");
			if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  It then travels along to your ");
		}
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("[vagina], which, despite being devoid of cock, clamps down anyway, gushing with femcum, drenching your Owner's balls and legs in sticky, clear fluids.");
		...
		Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] += 2;
		menu();
		MainScreen.addButton(0, "Next", partTwoOfDoggieStyle);
	}
	...
	private getAPetReward(): void {
		MainScreen.clearText();
		MainScreen.text(images.showImage("urta-home-petplay-reward"), false);
		MainScreen.text("\"<i>Well, you've been a good little pet so far,</i>\" Urta says, patting you on the head. \"<i>I think a little... reward is in order.</i>\"  You quirk an eyebrow, wondering about that pause, but wag your ");
		if (player.lowerBody.tailType > TailType.NONE) MainScreen.text("tail");
		else MainScreen.text("tailplug");
		MainScreen.text(" enthusiastically.");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("hard");
		if (player.gender == 3) MainScreen.text(" and ");
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("wet");
		MainScreen.text(", oddly enough, thinking of what your 'reward' could be.  Urta returns several minutes later, with one hand holding some kind of metal dish... a food bowl for a dog!");
		if (silly()) MainScreen.text("  In her other hand is what looks to be a jar of peanut butter.  What use could that have?");
		...
		if (silly()) MainScreen.text(", though you wonder if perhaps that's what the peanut butter is for");
		MainScreen.text(".  Your Owner stretches a foot out towards you, running her stocking-clad paw along your side.  The little tufts of fur protruding from the fishnet tickle your skin in a sensual fashion, and you can't stop your spine from stiffening at the contact.");
		...
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) MainScreen.text("onto your chest");
		else MainScreen.text("between your breasts");
		MainScreen.text(".  She takes a moment to encircle one of your nipples, which stiffens to attention along with ");
		if (player.upperBody.chest.countNipples() == 2) MainScreen.text("its twin");
		else MainScreen.text("the others");
		MainScreen.text(", before sending the paw down further where it makes contact with your ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(CockDescriptor.describeMultiCockShort(player));
		else MainScreen.text("[vagina]");
		MainScreen.text(".  There's nothing you can do to stifle the moan of pleasure that escapes your lips as your Owner's paw travels the length of your ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("rapidly hardening shaft.");
		else MainScreen.text("moistening slit.");

		MainScreen.text("\n\nUrta keeps this up for a minute, lovingly stroking your ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("dick");
		else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("cunt lips");
		MainScreen.text(" with a paw, until you bite your lip and whine at her.  She gives you a little smirk and the foot retreats.  \"<i>All right then, pet, come and get your bone,</i>\" Urta gestures down at the half-erect cock sprouting from between her thighs");
		//silly:
		if (silly()) MainScreen.text(", while she opens the jar of peanut butter and smears a dollop across the flat head.  Is... is that supposed to be enticement?  She really is treating you like a real dog");
		MainScreen.text(".  You crawl towards her nervously, and start to reach up to grasp her cock, but again she mutters, \"<i>Ah-ah, good doggies don't use their hands.</i>\"");

		MainScreen.text("\n\nNodding, you extend your tongue, running it along the length of her hardening shaft");
		if (silly()) MainScreen.text(", licking all of the peanut butter off the tip");
		MainScreen.text(".  Urta shivers and lets out a \"<i>Mm,</i>\" while her pillar of horsecock reaches its full size, clocking in at what has to be a good twenty inches.  ");
		if (silly()) MainScreen.text("  Your owner uses her fingers to smear another large dollop of peanut butter along her own shaft, giving you an almost sadistic smirk.");
		MainScreen.text("  You smooch and lick your way down her shaft, lapping at every little vein while Urta leans back and enjoys your ministrations.  Little gasps of pleasure escape from her lips, while moans leave yours as you smooch and suckle your way down to the base of that towering horsecock.");

		MainScreen.text("\n\nYour foxy Owner's prick glistens by the time you finally take it into your mouth");
		if (silly()) MainScreen.text(", the peanut butter now long forgotten");
		MainScreen.text(", stretching your jaw wide.  ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("Your own cock remains almost painfully rock hard, bobbing awkwardly beneath you, dripping precum onto the floor and bowl.");
		if (player.lowerBody.vaginaSpot.hasVagina()) {
			if (player.gender == 3) MainScreen.text("  Not to mention");
			else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Meanwhile");
			MainScreen.text(", your pussy is as wet as it's ever been, constantly leaking juices that run down your legs, pooling on the floor.");
		}
		MainScreen.text("  You swirl your tongue around Urta's flare, paying homage to the horsecocked goddess.  Lapping at the slit for a moment, you slip your ");
		if (player.upperBody.head.face.tongueType == TongueType.SNAKE) MainScreen.text("serpentine ");
		...
		MainScreen.text("tongue along the vixen's thick cumvein, which pulses lightly.  Urta groans and runs a hand through your " + HeadDescriptor.describeHair(player) + ", gripping it solidly.  You can't help as you're forced further onto the warm rod.");
		...
		MainScreen.text("tongue, lapping at the top layer and swallowing it down.  It's still warm, and quite salty with a bit of a sweet tang to it.  You quickly dive back into your 'reward', using your tongue as a spoon to consume every last drop in the bowl.  While you're busy doing that, Urta slides off the bed and takes up a position behind your body, ass high into the air, showing off all her goods.  The fox raises a paw, smearing some of her cum along your ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("shaft ");
		if (player.gender == 3) MainScreen.text(" and ");
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("slit ");
		MainScreen.text("tantalizingly.");

		MainScreen.text("\n\nWith every dollop of cum you consume, she gives your ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("[cockHead biggest]");
		else MainScreen.text("[clit]");
		MainScreen.text(" a little twirl with a soft, padded toe.  You can't help but moan into the bowl as you go, your legs shivering and stomach twisting with desire.  You continue to lap at the bowl eagerly downing mouthful after mouthful and being rewarded with more pleasure.  Even when you've licked the bowl clean, you find yourself lowering your head further and slurping away the remnants that splattered onto the floor.  Urta seems to find this satisfactory, as she picks up the pace, ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("jerking off your cock between two toes");
		else MainScreen.text("rubbing your little love button like mad with one soft toe");
		MainScreen.text(", until you just can't handle any more.");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("The cock between your legs, meanwhile, stiffens, briefly becoming harder than you've ever felt before, before blasting the floor with jet after jet of silky white cum.");
		else MainScreen.text("Meanwhile, your vagina clamps on nothing, achingly, painfully empty.  Juices spray out onto Urta's foot and down your thighs, soaking the floor beneath you in yet more slippery femcum.");
		MainScreen.text("  Your Owner doesn't just let it end there, however, she continues her paw-assault on your crotch, stroking and rubbing until you become little more than a gibbering mess of orgasm-infused flesh laying in a puddle of your own sexy goo.");
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] += 2;
		Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] += 2;
		MainScreen.text("...");
		player.orgasm();
		menu();
		MainScreen.addButton(0, "Next", feedTheBeastPartII);
	}
	...
	private urtaTakesPCOnWalkies(): void {
		MainScreen.clearText();
		MainScreen.text(images.showImage("urta-home-petplay-walkies"), false);
		MainScreen.text("\"<i>Ah, I know just what we should do!</i>\" She claps her hands together and rummages through her dresser.  Urta throws on a simple little yellow sundress and makes her way to the door.  \"<i>Come on, pet</i>\" she says with a mischevious smirk, patting her leg and holding up a leather leash. \"<i>It's time to go for a walk!</i>\"");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("[eachCock] grows hard beneath you");
		if (player.gender == 3) MainScreen.text(" and ");
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("your [vagina] moistens, ready to be on display");
		MainScreen.text(".");
		...
		if (player.lowerBody.tailType > 0) MainScreen.text("tail");
		else MainScreen.text("tailplug");
		MainScreen.text(" a nervous little shake.  Urta smiles and opens the door wide, making you squint in the bright daylight.  You feel a tugging at your neck, and realize Urta's already set off.  You scamper off after her on all fours, not wanting to disappoint your Owner.  The two of you travel through some back alleys, thankfully free of strangers, until you come out into the main thoroughfare of the city.");
		...
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) MainScreen.text("the breasts hanging from your chest and ");
		MainScreen.text("your openly displayed rear end.  ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("Pre-cum drizzles from the end of your painfully erect cock");
		else MainScreen.text("Sweet juices flow from your cunt, drizzling down onto the ground");
		MainScreen.text(", creating a trail of horniness alongside Urta's.");
		...
		if (player.gender == 3) MainScreen.text(" and ");
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("wetter");
		MainScreen.text(".  You feel like you really are a pet, out for a walk with your Owner.  It's an odd, and slightly freeing revelation, and once you come to grips with it, your entire experience changes. You find yourself trotting ahead of Urta, happily displaying yourself to all who wish to see.");
		...
		player.stats.lust += 15;
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 0;
		Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] += 5;
		if (Flags.list[FlagEnum.PC_FETISH] > 0) {
			MainScreen.text("  <b>You're so fucking turned on after exhibiting yourself in such a way that you're dripping EVERYWHERE.  Such a good doggie.</b>");
			dynStats("lus=", 100, "resisted", false);
		}
		doNext(camp.returnToCampUseOneHour);
	}
