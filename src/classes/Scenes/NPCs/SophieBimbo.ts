...
internal function sophieSprite():void {
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00283] == 0) spriteSelect(83);
	else spriteSelect(53);
}

public bimboSophie():boolean {
	return (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00283] == 0 && flags[FlagEnum.SOPHIE_DEBIMBOED] == 0);
}

public sophieIsInSeason():boolean {
	return (flags[FlagEnum.SOPHIE_HEAT_COUNTER] > 551 && !pregnancy.isPregnant);
}

internal function sophiePregChance():void {
	if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] != 0) return;
...
private declineBimboSophie():void {
	sophieSprite();
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00283] = 1;
	MainScreen.text("", true);
...
private acceptBimboSophie():void {
	...
	else if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00238] == 1)
	{
	...
	else if (flags[FlagEnum.AMILY_FOLLOWER] > 0 && flags[FlagEnum.AMILY_BLOCK_COUNTDOWN_BECAUSE_CORRUPTED_JOJO] == 0) 
	{
		MainScreen.text("  Afterwards, she offers to lick Amily's twat, if Amily will do the same to her.  Amily ", false);
		if(flags[FlagEnum.AMILY_FOLLOWER] == 1) MainScreen.text("flushes hotly and denies the bimbo's request with a terse 'no'.", false);
	...
	else if (monk >= 5 && player.findStatusAffect(StatusAffects.NoJojo) < 0 && flags[FlagEnum.JOJO_DEAD_OR_GONE] == 0)
	{
...
public approachBimboSophieInCamp(output:boolean = true):void {
	sophieSprite();
	if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0)
	{
		if(output) {
		MainScreen.text("", true);
	...
	if(flags[FlagEnum.SOPHIE_CAMP_EGG_COUNTDOWN] > 0 && output) MainScreen.text("\n\n<b>Sophie's egg is sitting nearby.</b>");
	if(flags[FlagEnum.SOPHIE_DAUGHTER_MATURITY_COUNTER] > 0) {
		MainScreen.addButton(5,"Daughter",daughterCheckup);
	}
	if(flags[FlagEnum.SOPHIE_BROACHED_SLEEP_WITH] == 1 && flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) {
	...
	if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0 && !pregnancy.isPregnant && flags[FlagEnum.FARM_CORRUPTION_STARTED] == 1) MainScreen.addButton(1, "Farm Work", sendToFarm);
	if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] != 0) MainScreen.addButton(1, "Go Camp", backToCamp);
	
	if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] != 0 && flags[FlagEnum.FOLLOWER_PRODUCTION_SOPHIE] == 0) MainScreen.addButton(2, "Harvest Eggs", harvestEggs);
	if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] != 0 && flags[FlagEnum.FOLLOWER_PRODUCTION_SOPHIE] == 1) MainScreen.addButton(2, "Change Eggs", changeEggs);
	
	if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] != 0 && flags[FlagEnum.FOLLOWER_PRODUCTION_SOPHIE] == 1) MainScreen.addButton(3, "Stop Harvest", stopHarvest);
	
	MainScreen.addButton(7, "Appearance", sophieBimboAppearance);

	if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) MainScreen.addButton(9, "Leave", camp.campSlavesMenu);
	if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] != 0) MainScreen.addButton(9, "Back", kGAMECLASS.farm.farmCorruption.rootScene);
}
...
private sophieBimboAppearance():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) MainScreen.text("very ");
	MainScreen.text("long ");
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) MainScreen.text("platinum-blond");
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) MainScreen.text(", super-sized");
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) MainScreen.text("so immense they would drag her to the ground if she tried to fly, a sexy, lingering aftereffect of the draft you gave her.  The wobbly bimbo-tits look to be about F-cups.");
	...
	if (kGAMECLASS.farm.farmCorruption.hasTattoo("sophie"))
	{
		...
		else
		{
			...
			if (flags[FlagEnum.SOPHIE_TATTOO_COLLARBONE] != 0) MainScreen.text(flags[FlagEnum.SOPHIE_TATTOO_COLLARBONE] + "\n");
			if (flags[FlagEnum.SOPHIE_TATTOO_SHOULDERS] != 0) MainScreen.text(flags[FlagEnum.SOPHIE_TATTOO_SHOULDERS] + "\n");
			if (flags[FlagEnum.SOPHIE_TATTOO_LOWERBACK] != 0) MainScreen.text(flags[FlagEnum.SOPHIE_TATTOO_LOWERBACK] + "\n");
			if (flags[FlagEnum.SOPHIE_TATTOO_BUTT] != 0) MainScreen.text(flags[FlagEnum.SOPHIE_TATTOO_BUTT] + "\n");
		}
	}
...
private postEggSelector(selected:string):void
{
	...
	flags[FlagEnum.FOLLOWER_PRODUCTION_SOPHIE] = 1;
	if (flags[FlagEnum.FOLLOWER_PRODUCTION_SOPHIE_COLORCHOICE] != selected)
	{
		flags[FlagEnum.FOLLOWER_PRODUCTION_SOPHIE_COLORCHOICE] = selected;
		flags[FlagEnum.FARM_EGG_STORED] = 0;
...
private stopHarvest():void
{
	...
	flags[FlagEnum.FOLLOWER_PRODUCTION_SOPHIE] = 0;
...
private sendToFarm():void
{
	...
	flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] = 1;
...
private backToCamp():void
{
	...
	flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] = 0;
...
private bimboSophieSexMenu():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		//BUTT FUCKS
		if(player.cockThatFits(sophieCapacity()) >= 0 && player.lust >= 33) MainScreen.addButton(2,"Anal",buttFuckBimboSophie);
	...
	if (flags[FlagEnum.TIMES_SOPHIE_HAS_DRUNK_OVI_ELIXIR] > 0 && (player.gender > 0) && !pregnancy.isPregnant) {
	...
	if(flags[FlagEnum.TOLD_SOPHIE_TO_IZMA] == 0 && flags[FlagEnum.TIMES_SOPHIE_AND_IZMA_FUCKED] > 0 && flags[FlagEnum.IZMA_NO_COCK] == 0) {
	...
	if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] > 0 && flags[FlagEnum.SOPHIE_FAMILY_INCEST] > 0 && player.cockThatFits(sophieCapacity()) >= 0 && player.lowerBody.cockSpot.hasCock())
...
private tellSophieToVisitIzma():void {
	...
	flags[FlagEnum.TOLD_SOPHIE_TO_IZMA] = 1;
	doNext(camp.returnToCampUseOneHour);
}
...
public bimboSophieLustStickSurprise():void {
	...
	if(select == 0) {
		...
		if(!player.lowerBody.cockSpot.hasCock()) {
			...
			MainScreen.text("You shiver, " + sMultiCockDesc() + " growing hard and ready to submit to the feathery blond's love.  Perhaps you'll take care of that soon...", false);
			dynStats("lus", 15);
		}
		...
		sophieSprite();
		if(player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text("While standing there, drinking a sip of water, strong hands pull down your " + player.armorName + ".  Plump lips wrap around " + oMultiCockDesc() + " and smother it in plush, warm slipperiness.  You look down at your bimbo harpy and sigh while " + oMultiCockDesc() + " grows harder in her lips.  She pops off and asks, \"<i>Like, wanna fuck?</i>\"  You sigh and try to resist the images of yourself mounting the harpy that flood your mind.  It might be time to fuck her soon...", false);
			dynStats("lus", 15);
		}
		else {
			dynStats("lus", 10);
			MainScreen.text("While standing there, drinking a sip of water, strong hands pull down your " + player.armorName + ".  Plump lips slide over your ");
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			dynStats("lus", 15);
			MainScreen.text(SMultiCockDesc() + " seems to wiggle inside your " + player.armorName + ".  Damn, she feels really good on you!  Maybe you should screw the slutty bimbo soon?  ", false);
		}
...
internal function daughterCheckup():void {
	MainScreen.clearText();
	if(flags[FlagEnum.SOPHIE_DAUGHTER_MATURITY_COUNTER] > 200) {
	...
	else if(flags[FlagEnum.SOPHIE_DAUGHTER_MATURITY_COUNTER] > 100) {
	...
	else if(flags[FlagEnum.SOPHIE_DAUGHTER_MATURITY_COUNTER] > 0) {
...
private bimboSophieLicksRugs():void {
	...
	MainScreen.text("You sit back on a nearby rock and spread your " + player.legs(), false);
	if(player.lowerBody.balls > 0) MainScreen.text(", lifting your heavy sack out of the way", false);
	MainScreen.text(" to give Sophie a perfect view of your " + vaginaDescript() + ".  Sophie's dull, somewhat glazed eyes brighten at the sight of your box, and she leans forward intently, dipping a digit into herself as she asks, \"<i>Ooh, can I, like, lick it for you?  It looks soooo yummy!</i>\"\n\n", false);
	...
	else MainScreen.text("You confidently smile and scoot your " + hipDescript() + " forward, anticipating the feeling of your bimbo-bitch's fat, pussy-licking lips on your slit.", false);
	...
	else if(player.wetness() < 5) MainScreen.text("sloppy", false);
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength < 3) {
	...
	else if(player.lowerBody.vaginaSpot.list[0].clitLength < 7) {
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength < 3) MainScreen.text("hums on your clit and doubles the speed of her licking while you watch her finger-fucking her sloppy cunt.", false);
	...
	else if(player.wetness() >= 4) MainScreen.text("gush", false);
	else if(player.wetness() >= 3) MainScreen.text("leak", false);
	else if(player.wetness() >= 2) MainScreen.text("dribble", false);
	...
	else MainScreen.text("You groan as your groin burns with desire, filling your " + multiCockDescriptLight() + " with blood.  Images of yourself relentlessly fucking your bimbo bride pregnant over and over again fill your mind.  You shudder and blush, trying to resist the artificial needs pounding through your body, but you realize your body is ready.", false);
	MainScreen.text("\n\n", false);
...
private bimboSophieSixtyNineAfterCunnilingus():void {
	...
	if(!player.lowerBody.cockSpot.hasCock() || player.cockThatFits(sophieCapacity()) == -1) {
		...
		MainScreen.text("Sophie drops down onto her elbows, her ass-length blond plumage tickling at your " + player.legs() + " as she examines your groin.  The slutty harpy leans over your gold-smeared crotch and gives your " + clitDescript() + " a kiss, her plush lips smothering the ", false);
		if(player.lowerBody.vaginaSpot.list[0].clitLength < 1) MainScreen.text("tiny ", false);
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength >= 3) MainScreen.text(", even pausing to jack it off like a penis", false);
	...
	//SixtyNine Continuation - DUDES
		...
		if(player.cumQ() >= 500) {
			MainScreen.text("  She gulps and swallows as fast as possible, but cum starts oozing from her nose, dripping from the corners of her mouth, and spurting back out around your " + cockDescript(x) + " to spatter your groin.", false);
...
private buttFuckBimboSophie():void {
	...
	MainScreen.text("Stepping closer, you toss off your " + player.armorName + " and reply, \"<i>Yes, just like that.</i>\"\n\n", false);
	...
	MainScreen.text("You squeeze her shuddering rump hard enough to feel her flesh bulging out around your fingers, the soft cheeks nearly subsuming your hands, and then you pull her hard, dragging your " + cockDescript(x) + " the rest of the way inside her, all the way to the ", false);
	...
	if(player.lowerBody.balls > 0) MainScreen.text("over your " + ballsDescriptLight(), false);
	...
	MainScreen.text("You keep butt-fucking her hard and fast, ignoring the sprinkles of sticky, maternal harpy-juice that splatter on your " + player.legs() + " as you fuck her.  In between throaty moans and screams, she bellows out \"<i>YES!</i>\" as loudly as possible, then lapses into quiet, exhausted grunts.  Yet, her vagina does not stop its prodigious leaking; if anything it's dripping and squirting out even heavier flows of submissive girl-spooge.  The harpie's anal ring is convulsing around you, reduced to pointless trembling from the hard fucking you're giving her.  Looking back with lidded eyes, she begs, \"<i>C-cum... cum in my ass... it'll... it'll feel soo good, babe.  Please?  Like, gimme your spooge!</i>\"\n\n", false);
...
private fuckBimboSophiesTits():void {
	...
	if(player.lowerBody.cockSpot.list[x].cockThickness < 8) MainScreen.text("completely surround yourself", false);
	else MainScreen.text("surround as much of yourself as possible", false);
	MainScreen.text(" in warm, smooth skin.  The smell of your " + cockDescript(x) + " mere inches from her nose seems to startle Sophie from her trance, and she breaks into a smile so big you'll worry it'll break her face.\n\n", false);
	...
	MainScreen.text("One of the dumb blonde's hands circles under her bouncing, jiggling chest to stroke your " + cockDescript(x) + ", jacking you off with the spit, sweat, and drugged gloss as lubricants.  She jerks you hard and fast, smearing it ", false);
	if(player.lowerBody.balls > 0) MainScreen.text("all over your balls whenever she needs a break, and you feel your " + ballsDescriptLight() + " growing heavier by the moment.", false);
	...
	MainScreen.text("  " + SMultiCockDesc() + " thickens and bloats, a rush of warmth rushing up the base as your internal muscles clench tight, squeezing out a globby rope of jism.  It explodes from your cum-slit, taking Sophie full on the face.\n\n", false);
	
	MainScreen.text("You're just getting started.  Sophie smirks and opens her mouth, letting her tongue slide out to taste your jizm as you launch the next rope, so long it trails from her hair to her chin and so thick it rolls in drops from the side of her face.  She hums in delight and squeezes her breasts tight around your " + cockDescript(x) + ", the next blast backing up inside you from the pressure.  A moment of buildup later, and your seed germinates and blooms over the harpy's tits, soaking them with spunk.  Droplets of the stuff splatter onto the ground around the both of you, but you just keep coming.  In your mind you have visions of yourself knocking up harpy after harpy, the slutty bird-girls mounting your fountaining fuck-stick long enough for their bellies to bloat with seed before moving on.  Like this, you could inseminate a whole tribe...\n\n", false);
	...
	MainScreen.text("and her eyes are closed, held shut by a thick layer of your sperm.  She's been well and truly bukkaked, drenched in ejaculate, and judging by how furiously she's schlicking away at her pussy, she couldn't be happier.  As for you, " + sMultiCockDesc() + " is still erect, even though you feel like you don't have a drop left in you.  Of course, the drugs soaking into your gold-tinted cock will probably keep you hard and ready to breed for hours.\n\n", false);
	sophieScene.luststickApplication(5);
	player.orgasm();
	dynStats("sen", -1);
	doNext(camp.returnToCampUseOneHour);
...
public sophieEggApocalypse():void {
	...
	MainScreen.text("", true);
	//First time
	if(flags[FlagEnum.TIMES_SOPHIE_HAS_DRUNK_OVI_ELIXIR] == 0) {
	...
	flags[FlagEnum.TIMES_SOPHIE_HAS_DRUNK_OVI_ELIXIR]++;
...
private rideDatSophieVag(nice:boolean = false):void {
	...
	if(!nice && player.stats.cor < 50) dynStats("cor", 1);
	sophieScene.luststickApplication(5);
	doNext(camp.returnToCampUseOneHour);
}
...
private sophieImpregnationSex():void {
	...
	if(y >= 0) MainScreen.text("  A sinful tightness clenches your " + cockDescript(y) + " at the same time.  You accidently started double penetrating her!  The sloppy vaginal heat around your second member is enough to make you forget that it was even an accident.");
	...
	else MainScreen.text("running down your " + cockDescript(y) + " in miniature rivers");
...
public sophieMoveInAttempt():void {
	MainScreen.clearText();
	sophieSprite();
	flags[FlagEnum.SOPHIE_BROACHED_SLEEP_WITH] = 1;
...
private yesSophieSleepWith():void {
	...
	flags[FlagEnum.SLEEP_WITH] = "Sophie";
	doNext(camp.returnToCampUseOneHour);
}
...
private noSophieSleepWith():void {
	...
	if(flags[FlagEnum.SLEEP_WITH] == "Sophie") flags[FlagEnum.SLEEP_WITH] = "";
	doNext(camp.returnToCampUseOneHour);
}
...
public fuckYoPregnantHarpyWaifu(morning:boolean = false):void {
	...
	//REGULAR FUCK
		...
		MainScreen.clearText();
		sophieSprite();
		MainScreen.text("You go ahead and take off your [armor], letting Sophie take in the view of [eachCock].  She immediately flounces over and drops to her knees in front of you.  Her hands wraps around " + cockDescript(x) + ", ");
		if(player.lust > 50) MainScreen.text("feeling its firmness.  \"<i>Wow, you're so hard already, [name]!</i>\" she comments.");
	...
	MainScreen.text(" proving the veracity of her statements many times over.  You can't SEE the wetness, but your " + cockDescript(x) + " certainly feels it, twitching inside Sophie's silky slit when the waves of wetness roll over it.");
...
private sophiePreggoTitJobs():void {
	...
	if(player.lowerBody.cockSpot.list[x].cockLength > 30) MainScreen.text("the underside of your immense erection");
	else MainScreen.text("your " + player.cockHead(x) + "");
	MainScreen.text(" with her lips, already pursed into a plump, cock-sucking doughnut hole of pleasure.  The noise she makes when your " + cockDescript(x) + " ");
	if(player.lowerBody.cockSpot.list[x].cockLength > 30) MainScreen.text("presses against");
	...
	if(player.lowerBody.cockSpot.list[x].cockLength > 30) MainScreen.text("  Sophie pulls back long enough to gasp, \"<i>You're so long, I can't even take the tip!</i>\"");
...
private goHogWildOnSophieBewbs():void {
	...
	if(player.lowerBody.cockSpot.list[x].cockLength <= 30) MainScreen.text(player.cockHead(x) + " pushes into through her startled cock-pillows, muffling her more effectively than any gag.  She immediately sucks on it, a happy look in her eyes.  The bimbo seems to want nothing more than to lie there and polish your rod.");
	...
	if(player.lowerBody.cockSpot.list[x].cockLength <= 30) MainScreen.text(" right into Sophie's mouth");
	...
	MainScreen.text(".  Your [butt] is squished up against her belly, both of you sweating and slipping all over each other.  Bulging in her breasty embrace, your " + cockDescript(x) + " flexes, and your " + player.cockHead(x) + " thickens.");
...
private letSophieMilkYoDick():void {
	...
	if(player.lowerBody.cockSpot.list[x].cockLength <= 30) MainScreen.text(", ready to catch the cum");
	else MainScreen.text(", assisting the cum");
	MainScreen.text(" that's already bubbling out of your [balls] with searing heat.  She presses harder on each side, crushing your " + cockDescript(x) + " with her slippery tits as your urethra bulges with the heavy load. With an audible squishing noise, you unload what feels like every drop of cum your body contains in one huge lance.");
	//{Fork between mouth and bukkake}
	//Mouth
	if(player.lowerBody.cockSpot.list[x].cockLength <= 30) {
...
public sophieBirthsEgg():void {
	...
	if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] == 0) MainScreen.text("  Can you handle having another harpy living in your camp?\n");
...
public sophiesEggHatches():void {
	flags[FlagEnum.SOPHIE_CAMP_EGG_COUNTDOWN] = 0;
	sophieSprite();
	...
	if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] < 4) {	
		//First Harpy:
		...
		if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] == 0) {
			...
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) MainScreen.text(", though her lips are a little larger than you'd think was normal - maybe she inherited it from her mom?");
			else MainScreen.text(".");
			MainScreen.text("  The harpy chick is actually about as big as a four or five year old, and she looks up at Sophie curiously.");
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0)  {
				flags[FlagEnum.DAUGHTER_ONE_BIMBO] = 1;
			}
		}
		//Second daughter
		//Second Harpy:
		//-Badonkadonk to 9000.  Blond feathers that grow very long. (violet eyes)
		else if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] == 1) {
			...
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0)  {
				...
				flags[FlagEnum.DAUGHTER_TWO_BIMBO] = 1;
			}
		...
		//-Not that curvy, but has human feet and purple feathers. (violet eyes)
		else if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] == 2) {
			...
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) flags[FlagEnum.DAUGHTER_THREE_BIMBO] = 1;
		}
		//Fourth Harpy:
		//-Curvy like Sophie, but with well-developed wings and extra-soft feathers (WINGJOBS).  Pink plumage.  (blue eyes)
			...
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) MainScreen.text(" like her mother used to have before her bimbofication");
			...
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) flags[FlagEnum.DAUGHTER_FOUR_BIMBO] = 1;
		}
	...
	else
	{
		...
		if(rand(2) == 0) MainScreen.text("pink");
			...
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) MainScreen.text("blonde");
	...
	flags[FlagEnum.SOPHIE_DAUGHTER_MATURITY_COUNTER] = 336;
}
...
public sophieKidMaturation():void {
	...
	//Fifth+ daughter
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) MainScreen.text("bimbo ");
	...
	if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] > 0) {
		//If Yes Incest
		if(flags[FlagEnum.SOPHIE_FAMILY_INCEST] == 1) {
	...
	flags[FlagEnum.SOPHIE_ADULT_KID_COUNT]++;
}
//[yes incest plz]
private yesToSophieIncest():void {
	...
	dynStats("lus", 5);
	flags[FlagEnum.SOPHIE_FAMILY_INCEST] = 1;
	doNext(camp.returnToCampUseOneHour);
}
//[nono]
...
public sophieAndIzmaPlay():void {
	MainScreen.text("\n");
	flags[FlagEnum.TIMES_SOPHIE_AND_IZMA_FUCKED]++;
	...
	if(flags[FlagEnum.TIMES_SOPHIE_AND_IZMA_FUCKED] == 1) {
