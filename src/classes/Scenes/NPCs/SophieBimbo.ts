...
private function acceptBimboSophie():void {
	...
	else if (flags[FlagEnum.AMILY_FOLLOWER] > 0 && flags[FlagEnum.AMILY_BLOCK_COUNTDOWN_BECAUSE_CORRUPTED_JOJO] == 0) 
	{
		...
		if(flags[FlagEnum.AMILY_FOLLOWER] == 1) outputText("flushes hotly and denies the bimbo's request with a terse 'no'.", false);
...
public function approachBimboSophieInCamp(output:boolean = true):void {
	...
	if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0)
	{
		...
		if(sophieIsInSeason() && player.lowerBody.cockSpot.hasCock()) {
			...
			if(choice == 0) {
				...
				if(player.lowerBody.cockSpot.hasCock()) outputText(" cock-pillows slurping around your member.", false);
				else if(player.lowerBody.vaginaSpot.hasVagina()) outputText(", fuckable pussy-pillows crushing against your quim.", false);
				else outputText(", fuckable pillows locking you in their embrace.", false);
				if(player.stats.cor < 33) outputText("  You find yourself nodding and blushing heavily as you drink in the bimbo's body.", false);
				else if(player.stats.cor < 66) outputText("  You find yourself nodding and giving a lewd grin as you drink in the bimbo's body.", false);
	...
	if(flags[FlagEnum.SOPHIE_CAMP_EGG_COUNTDOWN] > 0 && output) outputText("\n\n<b>Sophie's egg is sitting nearby.</b>");
...
private function sophieBimboAppearance():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) outputText("very ");
	outputText("long ");
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) outputText("platinum-blond");
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) outputText(", super-sized");
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) outputText("so immense they would drag her to the ground if she tried to fly, a sexy, lingering aftereffect of the draft you gave her.  The wobbly bimbo-tits look to be about F-cups.");
	...
	if (kGAMECLASS.farm.farmCorruption.hasTattoo("sophie"))
	{
		...
		else
		{
			...
			if (flags[FlagEnum.SOPHIE_TATTOO_COLLARBONE] != 0) outputText(flags[FlagEnum.SOPHIE_TATTOO_COLLARBONE] + "\n");
			if (flags[FlagEnum.SOPHIE_TATTOO_SHOULDERS] != 0) outputText(flags[FlagEnum.SOPHIE_TATTOO_SHOULDERS] + "\n");
			if (flags[FlagEnum.SOPHIE_TATTOO_LOWERBACK] != 0) outputText(flags[FlagEnum.SOPHIE_TATTOO_LOWERBACK] + "\n");
			if (flags[FlagEnum.SOPHIE_TATTOO_BUTT] != 0) outputText(flags[FlagEnum.SOPHIE_TATTOO_BUTT] + "\n");
		}
	}
...
public function bimboSophieLustStickSurprise():void {
	...
	if(select == 0) {
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) outputText("vulva, sucking at your clitoral hood.  Those gold, slobbery pussy-lickers look good on your groin, and you find yourself releasing your wetness from her oral attentions.  It might be time to fuck her soon...", false);
...
private function bimboSophieLicksRugs():void {
	...
	if(player.lowerBody.balls > 0) outputText(", lifting your heavy sack out of the way", false);
	...
	if(player.stats.cor < 33) outputText("You nod nervously, blushing hotly at the thought of your bimbo-slut's fat lips kissing and lapping at your slit.", false);
	else if(player.stats.cor < 66) outputText("You nod and smile, blushing a bit in anticipation of feeling your bimbo-slut's fat, pussy-licking lips against your slit.", false);
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength < 3) outputText("hums on your clit and doubles the speed of her licking while you watch her finger-fucking her sloppy cunt.", false);
	...
	if(!player.lowerBody.cockSpot.hasCock()) outputText("  It's a good thing you don't have a penis!", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", or thick, juicy dick", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("and hard ", false);
...
private function bimboSophieSixtyNineAfterCunnilingus():void {
	...
	if(!player.lowerBody.cockSpot.hasCock() || player.cockThatFits(sophieCapacity()) == -1) {
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength < 1) outputText("tiny ", false);
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength >= 3) outputText(", even pausing to jack it off like a penis", false);
...
private function buttFuckBimboSophie():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("Those cocks look", false);
	...
	if(!player.lowerBody.cockSpot.hasSheath()) outputText("hilt", false);
	...
	if(player.lowerBody.balls > 0) outputText("over your " + ballsDescriptLight(), false);
	...
	if(player.lowerBody.balls > 0) outputText("Your balls seize up at Sophie's request, and you swear you can actually hear the fluid churning and sloshing of your cum before it burns a path of warmth through your body.", false);
...
private function fuckBimboSophiesTits():void {
	...
	if(player.lowerBody.cockSpot.list[x].cockThickness < 8) outputText("completely surround yourself", false);
	...
	if(player.lowerBody.balls > 0) outputText("all over your balls whenever she needs a break, and you feel your " + ballsDescriptLight() + " growing heavier by the moment.", false);
...
private function rideDatSophieVag(nice:boolean = false):void {
	...
	if(nice) {
		...
		if(player.lowerBody.cockSpot.count() >= 2) outputText("they grow");
		...
		if(player.lowerBody.balls > 0) outputText("your [balls] bouncing and slapping against what parts of Sophie's bubble butt they can reach, ");
	...
	if(player.lowerBody.cockSpot.count() == 1) {
		...
		//{if (cocks >= 2) and (cumHigh = true)} 
			...
			if(player.lowerBody.cockSpot.count() == 3) outputText(", along with your third length, spittling and spraying onto her belly.");
			else if(player.lowerBody.cockSpot.count() > 3) outputText(", along with the rest of your cocks, your entire mass of lengths getting your seed all over her.");
...
private function sophieImpregnationSex():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("s come");
	else outputText(" comes");
	outputText(" free of the offending garment's edge and snap");
	if(player.lowerBody.cockSpot.count() == 1) outputText("s");
	outputText(" up, proud and erect.  ");
	if(player.lowerBody.cockSpot.count() == 1) outputText("It wobbles");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("one of your lengths");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText(", the rest of your lipstick-smeared shafts slipping and sliding between the soft, lube-slicked thighs");
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() > 2) outputText("s between her thighs are");
	...
	if(player.lowerBody.balls > 0) outputText(", and your [balls] begin to rhythmically pulse in your [sack]");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText(" over your extra, thigh slapping cock");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("  Ropes of creamy spunk drizzle over the ground and the underside of Sophie's tits, but the lust-drunk harpy doesn't seem to mind any more than you do.");
...
private function giantCocksImpregnateSophie():void {
	...
	if(player.lowerBody.balls > 0) outputText("in those plump balls of yours");
...
public function sophieFenCraftedSex(morning:boolean = false):void {
	...
	if(player.lowerBody.balls > 0) outputText("  Sophie's immense backside slides over your [balls] each time she bottoms out, her tail feathers tickling at the bottom of your [sack].");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("own");
...
public function fuckYoPregnantHarpyWaifu(morning:boolean = false):void {
	...
	if(player.lowerBody.balls > 0) outputText("[balls], where they hang in thick drops before falling to the ground,");
...
private function sophiePreggoTitJobs():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() == 2) outputText("other one");
	...
	if(player.lowerBody.cockSpot.list[x].cockLength > 30) outputText("the underside of your immense erection");
	...
	if(player.lowerBody.cockSpot.list[x].cockLength > 30) outputText("presses against");
	...
	if(player.lowerBody.cockSpot.list[x].cockLength > 30) outputText("  Sophie pulls back long enough to gasp, \"<i>You're so long, I can't even take the tip!</i>\"");
...
private function goHogWildOnSophieBewbs():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() > 2) outputText("s splay");
	...
	if(player.lowerBody.cockSpot.list[x].cockLength <= 30) outputText(player.cockHead(x) + " pushes into through her startled cock-pillows, muffling her more effectively than any gag.  She immediately sucks on it, a happy look in her eyes.  The bimbo seems to want nothing more than to lie there and polish your rod.");
	...
	if(player.lowerBody.cockSpot.list[x].cockLength <= 30) outputText(" right into Sophie's mouth");
	...
	if(player.lowerBody.balls > 0) outputText(", cupping your [balls] with a hand and massaging them to aid your ejaculation");
...
private function letSophieMilkYoDick():void {
	...
	if(player.lowerBody.balls > 0) outputText("Your [balls] drag across her pregnant belly as she goes to work, but you don't mind in the slightest.  There's something erotic about having them bumping up against the fruit of your union.  ");
	...
	if(player.lowerBody.cockSpot.list[x].cockLength <= 30) outputText(", ready to catch the cum");
...
public function sophieBirthsEgg():void {
	...
	if(player.stats.cor < 33) outputText("  You sit down next to her, and figuring this must be normal for her, you hold her hand with one arm and grope at her free breast with the other.");
	else if(player.stats.cor < 66) outputText("  You sit down next to her with a sigh.  Well, at least you can grope her tit with your other hand!");
	...
	if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] == 0) outputText("  Can you handle having another harpy living in your camp?\n");
...
public function sophiesEggHatches():void {
	...
	if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] < 4) {	
		...
		if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] == 0) {
			...
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) outputText(", though her lips are a little larger than you'd think was normal - maybe she inherited it from her mom?");
			else outputText(".");
		...
		//-Badonkadonk to 9000.  Blond feathers that grow very long. (violet eyes)
			...
			if(player.stats.cor > 66) outputText(" a little bit");
		...
		//-Curvy like Sophie, but with well-developed wings and extra-soft feathers (WINGJOBS).  Pink plumage.  (blue eyes)
			...
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) outputText(" like her mother used to have before her bimbofication");
	...
	else
	{
		...
		if(rand(2) == 0) outputText("pink");
			...
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) outputText("blonde");
...
public function sophieKidMaturation():void {
	...
	//Fifth+ daughter
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) outputText("bimbo ");
	...
	if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] > 0) {
		...
		if(flags[FlagEnum.SOPHIE_FAMILY_INCEST] == 1) {
			...
			if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] > 1) outputText("s");
...
private function watchIzmaSophie():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", from me OR my Alpha");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("stroke yourself through bliss");
