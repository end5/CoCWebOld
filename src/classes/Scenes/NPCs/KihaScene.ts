...
private function kihaMilkTitHumiliation():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("not noticing the stiffening in your loins", false);
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("not noticing the scent of your growing need", false);
	...
	if(player.stats.cor < 33) outputText("How far you've fallen.", false);
	else if(player.stats.cor < 66) outputText("How confusing this all is.  Why can't things be simple like they were back home?", false);
...
private function kihaRapesMen():void {
	...
	if(player.isNaga() || player.tailType == TAIL.LIZARD) outputText("\"<i>Heh, if you had half as much dexterity as you do impudence, I'd let you do this for me.  Fat chance of that,</i>\" she remarks off-handedly.  ", false);
	...
	if(player.lowerBody.balls > 0) outputText(", resting the heel of her foot against the swell of your " + ballsDescriptLight() + " and applying just enough pressure to feel the cum churning within", false);
	...
	//[Lizard/naga tail: 
	if(player.isNaga() || player.tailType == TAIL.LIZARD) outputText(" while your tail curls around to find your own asshole.  The grip of the dragon's nethers so intense, you barely feel the pressure of your own tail, pushing in easily to soothingly stroke your overtaxed prostate", false);
	...
	if(player.lowerBody.balls > 0) outputText(", your " + ballsDescriptLight() + " practically leaping in over-burdened joy as their heavy load rushes through your flesh", false);
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		if(player.lowerBody.cockSpot.count() > 2) outputText("  Your unoccupied shafts spray ropey strands of cum all around you, jerking and straining under the weight of the dragoness.", false);
...
private function tsundereMasturbationChristmasCarol():void {
	...
	//If Multi/Herm
	if(player.lowerBody.cockSpot.count() > 1 || player.gender == 3) outputText("\"<i>W-what are you going to do, you freak of nature?  If you make me play with those, I swear... I swear I'll hit you!</i>\"", false);
	//If Cock
	else if(player.lowerBody.cockSpot.hasCock()) outputText("\"<i>G-get that away from me!  As if I'd want anything to do with your hard, juicy cock!  Idiot!</i>\"", false);
	//If Vagina
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("\"<i>What do you think you're going to do with that pathetic, dripping pussy, huh?  I swear, if you make me l-lick that thing...</i>\"", false);
	//If Genderless
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.count() == 1) outputText("a small bead of pre-cum forming from the tip and slathering her back with your juices.", false);
...
private function kihaVictoryPomfTail():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 4) outputText("She looks away and blushes as she says, \"<i>It's not like I'd find such tiny breasts attractive!</i>\"  Her mannerisms seem to indicate that she does.", false);
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 12) outputText("She sighs and says, \"<i>As if I'd be impressed by udders like those!</i>\"", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", cum-,", false);
...
private function victoryDickKiha():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("and " + vaginaDescript() + " ", false);
	...
	if(y != -1 && player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() == 2) outputText(" is", false);
		...
		if(player.isNaga() || player.tailType == TAIL.LIZARD || z >= 0) outputText(" gasping mouth,", false);
		...
		if(player.lowerBody.cockSpot.count() > 2) {
			...
			if(player.lowerBody.cockSpot.count() > 3) outputText("es", false);
			outputText(".", false);
		}
		outputText("\n\n", false);
	...
	//(SINGLE DICKING VARIANT)
		...
		if(player.stats.cor < 33) outputText(", clean up, and get dressed, leaving Kiha behind to consider the effects of her arrogance.", false);
		else if(player.stats.cor < 66) outputText(", wipe your cock off on her face, and get dressed, leaving Kiha to consider being a little more humble.", false);
...
private function kihaRapesLittleGirlsISawItOnTheNews():void {
	...
	if(player.tongueType == TONUGE.SNAKE) outputText("  You are grateful for your forked tongue, which allows you to cover more of her pussy at once.", false);
	else if(player.tongueType == TONUGE.DEMONIC) outputText("  Your inhumanly long, prehensile tongue is a blessing here; you can reach into her deepest recesses, slurp your way into every nook and cranny, and generally provide a tongue-fuck that no human ever could.", false);
	outputText("\n\n", false);
...
private function rapeKihaWithWORDS():void {
	...
	//Conversation Level 2 (40- Corruption! PUREBABIES ONLY!) (Zed)
		...
		if(silly() && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00281] > 0 && telAdre.lottie.lottieMorale() <= 33) outputText("  Is that what it feels like to be Lottie?", false);
	}
...
private function analRapuzulaKiha():void {
	...
	if(player.lowerBody.balls > 0) outputText(" and threatening to crush your " + ballsDescriptLight(), false);
	...
	if(player.stats.cor < 33) outputText("You move your hands from her two mounds and down her sides, resting just above her hips. Hoping that this will work, you begin to tickle the stubborn girl.  ", false);
	else if(player.stats.cor < 66) outputText("Since she seems to be concentrating on your hands, you frantically try to think of a solution that doesn't involve them.  An idea forms and, deciding it's now or never, you lean forward and nibble on the tip of her ear.  ", false);
	...
	outputText("The heat that now encircles your " + Appearance.cockNoun(player.lowerBody.cockSpot.list[x].cockType) + " is tremendous; her tightness, incredible.  You're nearly overwhelmed by the urge to slam your whole length home and just cum your brains out.  But you want her to enjoy this, too.  You slowly slide back out of her rectum, stopping when just the tip of the head is barely inside. You lean forward, resting your chest against the warm skin of her back, and thrust your " + cockDescript(x) + " back into the crushing heat of her asshole.\n\n", false);
	...
	if(y >= 0) {
		...
		outputText("Kiha shudders in her own climax, soaking her thighs as you finally release your seed into her ravaged ass.  Your other " + Appearance.cockNoun(player.lowerBody.cockSpot.list[y].cockType) + " follows its brother, painting her leather-colored skin with alabaster cream", false);
		if(player.lowerBody.cockSpot.count() >= 3) {
			if(player.lowerBody.cockSpot.count() == 3) outputText("; your remaining cock tarnishes the ground with your cum", false);
			else outputText("; your remaining cocks tarnish the ground with your cum", false);
		}
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("; your " + vaginaDescript() + " pulses with pleasure, soaking your thighs with girl-cum as your cocks unload", false);
		outputText(".\n\n", false);
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Your " + vaginaDescript() + " pulses with pleasure, but it's an empty orgasm, leaving you wishing you had something to fill that aching void.  Still, you cannot help but marvel at the superior feeling of cumming with a multitude of sexual organs.  ", false);
