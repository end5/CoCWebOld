export default class ShouldraScene extends NPCAwareContent implements TimeAwareInterface {
	...
	public timeChange(): boolean {
		...
		if (Flags.list[FlagEnum.SHOULDRA_MAGIC_COOLDOWN] >= 1) Flags.list[FlagEnum.SHOULDRA_MAGIC_COOLDOWN]--;
		if (shouldraFollower.followerShouldra()) {
			if (player.statusAffects.get("Exgartuan").value1 == 1 && player.lowerBody.cockSpot.hasCock() && rand(10) == 0) {
				if (Flags.list[FlagEnum.SHOULDRA_EXGARTUDRAMA] == 1) {
					shouldraFollower.exgartumonAndShouldraFightPartII();
					needNext = true;
				}
				else if (Flags.list[FlagEnum.SHOULDRA_EXGARTUDRAMA] == 2) {
					shouldraFollower.exgartumonAndShouldraFightPartIII();
					needNext = true;
				}
			}
			Flags.list[FlagEnum.SHOULDRA_SLEEP_TIMER]--;
			if (shouldraFollower.shouldersWarnings()) needNext = true;
			if (Flags.list[FlagEnum.SHOULDRA_SLEEP_TIMER] == 0 || (Flags.list[FlagEnum.SHOULDRA_SLEEP_TIMER] < 0 && Flags.list[FlagEnum.SHOULDRA_SLEEP_TIMER] % 16 == 0)) {
				shouldraFollower.shouldraWakesUpOrPokesPCsForShitsAndGigglesIdunnoHowLongCanIMakeThisFunctionNameQuestionMark();
				needNext = true;
			}
			if (Flags.list[FlagEnum.SHOULDRA_PLOT_COUNTDOWN] > 0 && Game.model.time.hours == 3) Flags.list[FlagEnum.SHOULDRA_PLOT_COUNTDOWN]--;
		}
		return needNext;
	}

	public timeChangeLarge(): boolean {
		if (shouldraFollower.followerShouldra() && Flags.list[FlagEnum.SHOULDRA_PLOT_COUNTDOWN] == 0 && Game.model.time.hours == 3) {
			Flags.list[FlagEnum.SHOULDRA_PLOT_COUNTDOWN] = -1;
			shouldraFollower.shouldraDream1();
			return true;
		}
		//Ghostgirl recruitment priority
		if (Flags.list[FlagEnum.SHOULDRA_FOLLOWER_STATE] == .5 && model.time.hours == 6) {
			Game.shouldraFollower.morningShouldraAlert();
			return true;
		}
		//Ghostgirl pissed off dreams
		if (shouldraFollower.followerShouldra() && Flags.list[FlagEnum.SHOULDRA_SLEEP_TIMER] <= -236 && model.time.hours == 3 && player.gender > 0) {
			Game.shouldraFollower.nightTimeShouldraRapesThePC();
			return true;
		}
		//Ghostgirl madness
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00365] > 0) {
			if (player.lowerBody.cockSpot.count() > 1 || player.upperBody.head.face.faceType != FaceType.HUMAN || player.lowerBody != LowerBodyType.HUMAN || player.lowerBody.tailType > TailType.NONE || player.upperBody.head.horns > 0 || player.stats.cor > 15 || player.longestCockLength() > 10 || player.tallness < 65 || player.tallness > 78 || player.lowerBody.vaginaSpot.hasVagina())
	...
	internal function shouldraGreeting(): void {
		...
		//First 3 encounters
		if (Flags.list[FlagEnum.TIMES_MET_SHOULDRA] < 3) {
			MainScreen.text("You step into the ruins of the village cautiously, zigzagging through passageways and searching for anything worth pocketing. A sound from the inside of a half-collapsed cottage stops you in your tracks. You furrow your brow, upset at allowing yourself to drop your guard in a place like this. Unluckily, the presence seems aware of you, and you hear it shuffling towards the disfigured doorway. You drop into a crouch, readying your " + player.weaponName + " and preparing for battle. You are caught off-guard when a completely normal female leg steps out of the frame. The young woman takes a few steps closer, apparently ignoring your intimidating stance for a moment as she sizes you up. ", false);
			//([if first time] 
			if (Flags.list[FlagEnum.TIMES_MET_SHOULDRA] == 0) MainScreen.text("\"<i>You're the asshole that took my racks, aren't you?  I was hoard-...er, I was keeping an eye on those!</i>\" she accuses, eyes narrowing angrily.  ", false);
			MainScreen.text("Finally, she acknowledges your readiness to fight. With a sigh and a shrug, she raises her arms and drops into an orthodox boxing stance. Once again, you heft your " + player.weaponName + ".", false);
		...
		//(after three encounters with her)
			...
			if (Flags.list[FlagEnum.TIMES_BEATEN_SHOULDRA] >= 3) {
				MainScreen.text("\"<i>Or... maybe not?  Truth be told, I think you've proven your ability to pummel me enough.  Wanna just skip to the good part?</i>\"\n\n", false);
				//([if first time] 
				if (Flags.list[FlagEnum.TIMES_BEATEN_SHOULDRA] == 3) MainScreen.text("You can't say you saw that coming.  It seems she has offered to forego the fight in lieu of sexings.  ", false);
				MainScreen.text("Would you like to accept her offer, or do you want to fight her regardless?", false);
	...
	private yankeeEchoPapa(): void {
		spriteSelect(66);
		MainScreen.text("", true);
		MainScreen.text("You graciously accept her offer, and she nods happily.  \"<i>Perfect!  Now then, let's get to it...</i>\"\n\n", false);
	...
	private novemberAlphaHotel(): void {
		spriteSelect(66);
		MainScreen.text("", true);
		MainScreen.text("You shake your head slowly, raising your " + player.weaponName + " as you regard her.  \"<i>Ah, so that's how you want it?</i>\" she titters, a smile gracing her features.  \"<i>In that case, enough talk... have at you!</i>\"\n\n", false);
		startCombat(new Shouldra());
	}
	...
	internal function defeatDannyPhantom(): void {
		...
		//defeat by damage)
		if (monster.HP < 1) {
			MainScreen.text("With an ear-splitting, ghostly wail that forces you to your knees, the ", false);
			if (Flags.list[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] >= 1) MainScreen.text("ghost ", false);
			MainScreen.text("girl falls back, overcome by her injuries. Before you can stop her, she simply becomes insubstantial, her clothes undergoing the change as well. A breeze slips through the ruins, picking the ghost up and scattering her to the winds.", false);
			cleanupAfterCombat();
		}
		//(defeat by lust)
		else {
			MainScreen.text("The girl glances down at her leggings, then does a double take, realizing as if for the first time she was completely overwhelmed by her lust. She sinks to her knees, dragging her breeches down and pressing a hand against her pubic mound.  ", false);
			if (Flags.list[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] < 1) MainScreen.text("Strangely, the action seems to have little to no effect.", false);
			else MainScreen.text("Of course, the motion has no effect on her libido, due to her ghostly state.", false);
			MainScreen.text("  Looking up at you with what can only be described as desperation, she pants, \"<i>Please, hero... I need release... let me in?</i>\"  You take a step forward, stroking your chin softly in contemplation.  ", false);
			//([if first time]
			if (Flags.list[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] < 1) {
				MainScreen.text("What does she mean, \"<i>let me in?</i>\" Do you want to find out?", false);
	...
	private noSlimingSlimer(): void {
		spriteSelect(67);
		MainScreen.text("", true);
		MainScreen.text("You shake your head angrily at the brazen girl, ", false);
		//([if first time] 
		if (Flags.list[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] == 0) MainScreen.text("not feeling up to heed the strange girl's whims.", false);
		else MainScreen.text("mystified at the thought of letting a ghost possess your body.", false);
		MainScreen.text("You whirl about and stomp away.  She rises, screaming, \"<i>Fine! I'll go and take over a fucking goblin, again!</i>\" and storms the other direction. Before you move out of earshot, you hear a sniffle from the departing ", false);
		//([if first time]
		if (Flags.list[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] == 0) MainScreen.text("girl (what was that about her 'taking over' a goblin?)", false);
		else MainScreen.text("spirit", false);
		MainScreen.text(", and a twinge of guilt runs through you.", false);
		cleanupAfterCombat();
	}
	...
	private littlePigLittlePigLetMeIn(): void {
		spriteSelect(67);
		MainScreen.text("", true);
		MainScreen.text("You decide ", false);
		if (Flags.list[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] == 0) MainScreen.text("you can afford being a little adventurous with the obviously desperate girl", false);
		else MainScreen.text("having a spiritual tag-along might be a bit of fun", false);
		MainScreen.text(", and agree to her request to possess you. She staggers to her feet and undresses, panting and shivering with unfulfillable sexual need. You reach out to her, drawing her", false);
		//[if <~7' tall]
		if (player.tallness < 84) MainScreen.text(" into your arms for a tight hug", false);
		else MainScreen.text(" up into your arms for a tight hug", false);
		MainScreen.text(". The young woman heaves a happy sigh, then begins mumbling an incantation into your shoulder. Soon enough, the girl's body turns translucent and she begins to sink into you, planting a little peck on your cheek on her way in.  ", false);
		//([if first time] 
		if (Flags.list[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] == 0) MainScreen.text("Wait... WHAT!?  Did you just let some sort of... g-... g-... g-... G-... G-... GHOOOOOST possess you?", false);
		MainScreen.text("  \"<i>D-Damn,</i>\" she sighs, her voice wavering from her lust build-up, \"<i>Gotta get used to all of this " + player.skinFurScales() + "...</i>\"  You adjust the best you can to the realization that you are sharing your body with a spirit, then turn your attentions to her. You telepathically ask her if she has anything in particular in mind for relieving her of her arousal. She pulses back a 'hmm' at you, indicating she will allow you to decide her fate.\n\n", false);

		//CHOICES HURRAH
		let lake: Function = null;
		if (player.gender > 0 && ((player.statusAffects.has("BoatDiscovery") && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00230] > 0) || Flags.list[FlagEnum.TIMES_MET_OOZE] > 0)) {
			MainScreen.text("You could take her to the lake to find someone to play with...\n\n", false);
			lake = nowOnVickiLake;
		}
		...
		Flags.list[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA]++;
	...
	private nowOnVickiLake(): void {
		...
		if (ooze != null && shark != null) {
			MainScreen.text("You recall the experiences of both the slime and the shark girl. Which encounter would you wish to seek out?", false);
	...
	private slimeGinaFlation(): void {
		...
		Flags.list[FlagEnum.SLIMEGINAED]++;
	}
	...
	private penisLossThatIsntALoss(): void {
		...
		if (Flags.list[FlagEnum.SHOULDRA_PENIS_DEFEAT_TIMES] == 0) MainScreen.text("She abruptly gasps in excitement and begins moving your hands towards your groin, though you swiftly reclaim control. \"<i>You have a penis,</i>\" she telepathically pulses to you with an anxious and excited edge to her voice. \"<i>My hosts... they rarely... they...</i>\"", false);
		else MainScreen.text("She chuckles softly as she recalls the times you've shared, and she hungrily goes for your maleness once more, though you throw her off of it.", false);
		MainScreen.text("  The communication suddenly breaks as the ghost girl's will surges against yours, all thoughts of submissiveness vanishing from her mind.  ", false);
		//[if intelligence <50] 
		if (player.stats.int < 50) MainScreen.text("It's not very hard for her to break down your fortifications, and you soon feel yourself being pushed aside by her vast mental capacity.", false);
		else if (player.stats.int < 90 || player.cockThatFits(80) == -1) MainScreen.text("You begin to gain an edge, but the ghost girl is craftier than you expected. She begins to pulse mental intrusions at you, scrambling your thoughts just long enough for her to overtake you and push you to the side.  \"<i>You are one hundred years too early to challenge me,</i>\" she chides. \"<i>My intellect has long surpassed most any living creature on this plane.</i>\"", false);
		else {
			x = player.cockThatFits(80);
			MainScreen.text("The ghost girl puts up a good fight, but you're more than prepared for her mental assault. You feel her panic well as her intrusions are overwhelmed, and though she attempts to disengage the mental link, you easily stop her. She screams mental protests at you, but your ringing ears only strengthen your psychological dominance over the poor spirit. Before long, she's reduced to a whimpering wreck. \"<i>Out,</i>\" you command, tired of the troublesome specter's meddlings. Of course, she won't even try to rebel now, resignedly accumulating her essence and beginning her exit. Within moments the ghost girl flows out of your body and resolidifies before you.\n\n", false);

			MainScreen.text("She moves to leave in a huff, but you stop her with a single telepathic demand. She's yours, you remind her, and since she's gone through the trouble of trying to get you off, she might as well finish the job. Even as her lips purse in a stubborn pout, a dominating wave of influence sweeps over her, and regardless of herself, she begins unsteadily moving towards you. Resolving to meet halfway, you stride over to her and push her to your knees. As she watches, you shed your " + player.inventory.armor.displayName + " and hoist your " + CockDescriptor.describeCock(player, x) + " up to her lips, leaving a fairly clear implication of your expectations. The ghost girl's eyes widen at the sight of your manhood, and she cringes as a new wave of arousal sweeps through her. She gives a little whimper at her inability to quell her overwhelming libido, but you simply laugh mockingly and gesture at your maleness.  \"<i>Ghost blowjob,</i>\" you mutter snidely.\n\n", false);

			MainScreen.text("You don't even give her the chance to act of her own volition, simply reaching out, taking ahold of her head, and slamming her onto your " + CockDescriptor.describeCock(player, x) + ".  She takes it surprisingly well, unsurprisingly easily, taking most of the shaft in the first thrust alone.  With not as much as even a sputter, she impatiently double-handedly grabs your " + ButtDescriptor.describeButt(player) + " and thrusts her head forward.  Before you know it, your entire " + CockDescriptor.describeCock(player, x) + " is wrapped in a fleshy tunnel of love.", false);
			if (player.lowerBody.cockSpot.count() > 1) {
				MainScreen.text("  Mindful of ", false);
				if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("your other dick", false);
				else MainScreen.text("the rest of " + CockDescriptor.describeMultiCockShort(player), false);
				MainScreen.text(", the mentally dominated girl quickly clutches ", false);
				if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("your other shaft and pumps it vigorously.", false);
				else if (player.lowerBody.cockSpot.count() == 3) MainScreen.text("your other two shafts and pumps them vigorously.", false);
				else MainScreen.text("two other shafts and pumps them vigorously.", false);
			}
			MainScreen.text("\n\n", false);

			MainScreen.text("The ghost girl goes to town on your genitals, the combination of your will and her still-rising lust fueling her movements.\n\n", false);

			MainScreen.text("Although her mouth is quite full, her voice rings clearly in your head as she starts casting a spell.  You glance down at her, surmising from her almost serene expression and heavy-rimmed eyes that she's even gotten over her inability to orgasm.  She finishes her spell with the usual flourish before once more focusing her attention to your pleasure.  ", false);
			if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("Taking her hand away from your other dick, the ", false);
			else if (player.lowerBody.cockSpot.count() >= 3) MainScreen.text("Taking her hands away from your other dicks, the ", false);
			else MainScreen.text("The ", false);
			MainScreen.text("ghost girl smiles around your phallus, and you can only guess what she has planned.  You don't have long to wait, however; a strange roiling arises ", false);
			if (player.lowerBody.balls > 0) MainScreen.text("in your " + BallsDescriptor.describeBalls(true, true, player), false);
			else MainScreen.text("near the base of your shaft", false);
			MainScreen.text(".  ", false);
			if (player.lowerBody.balls == 0) MainScreen.text("The first effects of the spell make themselves known in a big way when an overladen cumsack forms and drops between your legs.  ", false);
			MainScreen.text("The strange feeling focuses on your testes, making it a bit hard to concentrate on keeping the ghost girl subdued.  Your management of willpower does not waver until you feel your legs being pushed farther apart.  A curious glance turns into a horrified stare as you gaze upon your now ");
			if (player.lowerBody.ballSize >= 9) MainScreen.text("gargantuan");
			else MainScreen.text("basketball");
			MainScreen.text("-sized balls.\n\n", false);

			MainScreen.text("Taking advantage of your distraction, your former mind slave pushes you over and jumps on top of you.  Her impotently sopping box leaves streaks of clear fluid across your belly as she continues her work on your cock", false);
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s", false);
			MainScreen.text(" while swaying her hips to and fro.  Both of her hands get to work on rubbing your ever-growing testicles, but two hands can't cover the growing amount of space necessary for effective stimulation.  She shrugs, content with patting, tickling, and squeezing the skin she can reach.  A massive churning begins in your gargantuan cumsack, a movement that actually shakes the ground.  Both of you knows what that signifies, the ghost girl going so far as to disengage from her feverish oral to stare longingly at you.  It seems you hold the key to sating her lust or denying her the orgasm she so craves; what do you do?", false);

			simpleChoices("LetHerCum", letShouldraIn, "KeepHerOut", kickShouldraOut, "", null, "", null, "", null);
			return;
		}
		MainScreen.text("\n\n", false);
		MainScreen.text("Taking complete control of your body as if it was never actually yours, she removes your " + player.inventory.armor.displayName + " and throws them carelessly to the ground. Grabbing up your " + CockDescriptor.describeCock(player, x) + " in both hands and fondling it awkwardly, she strikes you as obviously inexperienced with the male anatomy, despite her levels of intelligence. She continues to grope your lengthening member, admitting, \"<i>Even with the rare male bodies I have possessed, I rarely got to play with their...</i>\" You feel your face flush as the ghost girl relives her awkward previous possessions. Your penis finally extends to its full length, and your hands slide up and down its length in a clumsy yet admittedly effective technique. Sweat soon begins lathering your body as you sink to your knees, confusingly overwhelmed by the pleasure of a simple self-stimulation. You've never felt this good while masturbating! You figure the otherworldly influence of your new 'room-mate' has something to do with this. As if to prove your theory, your mouth moves in the strange archaic language of the girl, and your body stiffens in both pain and pleasure as ", false);
		if (player.lowerBody.balls > 0) MainScreen.text("your testicles churn with fertility and ", false);
		MainScreen.text("your manhood swells with new-found girth. The growth forces your hands further apart until your quivering pecker resembles a very excited fire hydrant. Drool begins to run from the corners of your mouth as she bites against your lower lip, head thrown back in the ecstasy of the stimulation. Finally, you feel a tightening in your scrotum, and you—all by yourself this time, as the ghost girl is in complete thrall of your body's orgasm—move your hand down to the bottom of your shaft. Remarkably, you actually feel the semen wriggling towards release under your fingers. With a final ecstatic scream equally parts ghost and human, your back arches as a geyser of semen bursts forth, showering you not unlike a fountain. After a long while kneeling and quivering, you finally get your wits about you and raise yourself back up, noting the lack of resistance from your non-corporeal friend. You sigh in relief as you watch your now-flaccid penis diminishing to its original proportions.\n\n", false);
	...
	private ghostBJFinisher(inside: boolean): void {
		...
		MainScreen.text("Like a backed up geyser, your gigantic testicles push hot, thick strands of seed out of your belaboured " + CockDescriptor.describeCock(player, x) + " straight up in the air.  The cum splatters on both you and the ", false);
		if (inside) MainScreen.text("excitedly convulsing", false);
		else MainScreen.text("practically lust-paralyzed", false);
		MainScreen.text(" girl, both of you too tired to bother moving away from the raining semen.  Panting heavily, you eventually haul yourself up, dragging the dainty ghost girl up with you.\n\n", false);
		player.orgasm();
		Flags.list[FlagEnum.SHOULDRA_PENIS_DEFEAT_TIMES]++;
		//if ejaculation allowed
		if (inside) {
			MainScreen.text("\"<i>Thank you so much,</i>\" she breathes softly, wrapping her arms around you in a soft embrace.  \"<i>I wouldn't even be able to think straight if that much lust was built up without release.</i>\"  You smile and flick her nose, and she retorts with a sly half-grin.  \"<i>Oh, and you can have this,</i>\" she adds, swiftly tugging her pants down and reaching into her still-drooling cunt, dragging out a handful of ectoplasm.  She tugs out a small bottle from her shirt pocket and sticks the gooey stuff inside before offering it to you with a wink.  \"<i>Thanks again!</i>\"\n\n", false);
			MainScreen.text("You watch her move off into the ruins with a new spring in her step, chuckling as she goes before heading back to camp yourself.", false);
		}
		//if that bitch didn't get nothing
		else {
			MainScreen.text("She wobbles almost exaggeratedly before falling sideways, smacking her head against the ground and sending up a small splash of cum.  If she felt any pain, however, she doesn't react to it at all, staring vacantly at a dilapidated building and drooling slightly.  That outta teach her to try to pull a fast one on you twice, you figure.  As you stride past her, you tug her pants down to her knees and scoop out a good amount of ectoplasm from her vagina.  You reach into her pocket and grab a bottle (honking her boob for good measure and eliciting an aroused twitch), dumping the goo in and stuffing it in your pack.  She'll probably find a goblin eventually, you figure as you begin your journey back to the camp.", false);
		}
		if (Game.inCombat) {
			Flags.list[FlagEnum.BONUS_ITEM_AFTER_COMBAT_ID] = consumables.ECTOPLS.id;
			cleanupAfterCombat();
	...
	private dewormYourGhost(): void {
		spriteSelect(67);
		MainScreen.text("", true);
		MainScreen.text("As the ghost girl settles into your body, she swiftly recognizes the unfamiliar bulge against your " + player.inventory.armor.displayName + ", and you feel a wave of joy run though your body as she regards your dick. The skin on the back of your hand shudders, and you jump in surprise when the spirit's face appears on it. She waits patiently for you to get over the initial shock, then asks, \"<i>May I... please... I mean, you... ah...</i>\" You guess she must be slightly overwhelmed by her discovery. You softly ask her if she is referring to your maleness, and she responds with a suddenly anxious \"<i>Mmhmm!</i>\"\n\n", false);

		MainScreen.text("Chuckling to yourself, you consent to her pleadings, and she falls back into your body with a gleeful squeal. You climb out of your " + player.inventory.armor.displayName + " and put a hand on your not-so-erect maleness, taking pleasure in the way she coos as blood begins to surge into your tool. Soon enough, your penis pulses in your hand, erect and ready for stimulation. You give it a little squeeze, not even noticing when one of the worms sheltering in your crotch slips out and wriggles across the floor, but the ghost girl surely does, and she makes herself known with a shrill scream that leaves your ears ringing and your head pounding. Worried, you ask her what's the matter ", false);
		if (Flags.list[FlagEnum.SHOULDRA_WORM_SCENE_COUNTER] == 0) MainScreen.text(", and she responds by taking control of your hand and pointing down at the wriggling creature that crawled out of your junk. \"<i>Oh, that?</i>\" you scoff, waving your other hand in the air dismissively; your other, ghost-controlled appendage still quivers stubbornly as you attempt to explain. \"<i>There are tons of the little guys crawling around inside me. Wanna see?</i>\"", false);
		else MainScreen.text(", and your eyes flare with a yellow light as she takes control of your mouth. \"<i>You're still carrying those disgusting, awful, terrible, horrible things!?</i>\" she cries in disbelief, and it's obvious she doesn't understand the allure of the wriggly creatures. You try to appeal to her on a more basic level, informing her of the tactical advantages of combat-ready dickworms.", false);
		MainScreen.text("\n\n", false);
		player.stats.lust += 15;
		MainScreen.text("There's silence for a moment, then you are almost rendered unconscious as she cries, \"<i>NOOOOOOOOPE,</i>\" telepathically, surging out of your body in record time. She corporealizes and backs away from you until she bumps into one of the ruined buildings. Then, she's off, booking it down the street to get as far away from you as possible. You shrug, figuring at the very least you dealt with her arousal, albeit in an unexpected way, and begin your trek back to your camp.", false);
		if (Game.inCombat)
			cleanupAfterCombat();
	...
	private ghostGinaWinSexings(): void {
		if (player.statusAffects.get("Exgartuan").value1 == 2 && rand(2) == 0) {
			ginaBoobgartuanShouldra();
			return;
		}
		spriteSelect(67);
		MainScreen.text("", true);
		MainScreen.text("The ghost girl heaves a mildly disappointed sigh as she settles into your body fully.  \"<i>I've had... many female hosts,</i>\" she explains. You momentarily feel embarrassed by the ghost's disapproval, and you almost move to apologize before the words catch in your throat and you cross your arms defensively.  ", false);
		if (Flags.list[FlagEnum.SHOULDRA_VAGINAL_POSSESSIONS] == 0) MainScreen.text("She 'hmm's several times inside your head, but you can feel a surge of elation as she apparently reaches a solution. \"<i>Of course!</i>\" she cries excitedly.  \"<i>I actually have never done this before!</i>\"\n\n", false);
		else MainScreen.text("\"<i>Oh well, I suppose we can always just do that again,</i>\" she muses. \"<i>It worked well enough the first time.</i>\"\n\n", false);

		MainScreen.text("You are about to question exactly what she is referring to, but quickly fall silent as she gently but firmly takes control of your body, sliding out of your " + player.inventory.armor.displayName + " and idly stroking your cunt. You allow her control of your mouth, and she chants a fairly lengthy spell, her absent-minded masturbation steadily gaining fervor as she progresses. With a final, shouted word of power, both of you cry out as your clitoris suddenly pulses and swells into your hand, the increased sensitivity nearly driving you insane. Your bitch-button does not stop there, however, puffing up with each heartbeat. Your eyes roll back in your head and small primitive grunts and moans escape your lips as your clit engorges in the ghostly magic, not stopping until it reaches at least four times its original length. A breeze blows through the ruined rooftops, and you scream as the wind brushes against your elongated bitch-button.  Too sensitive... the thing is way too sensitive!\n\n", false);

		MainScreen.text("Against your will, and against all of your better instincts, the ghost-girl, as aroused as you are uncomfortable, moves both hands to your sex, gripping it and giving it a squeeze. Your entire body locks up, causing you to fall backwards onto the pavement as a devastating orgasm rocks through your system. A strangled cry is the only reaction you can give to the explosive sensations. The ghost-girl, however, not satisfied with the suddenness of the climax, begins feverishly stroking your clitoris with one hand, the other reaching into your love-tunnel and stimulating the rosy flesh inside. You arch your back and attempt to dig small holes into the road with your " + LowerBodyDescriptor.describeFeet(player) + " as shock-waves run through your system over and over, never giving you a chance to recuperate. You hear, as if from a great distance, a mix of frenzied laughter and orgasmic cries of pleasure ringing inside your head. Finally, mercifully, you fade into unconsciousness, leaving the ghost girl to her constant stimulation.\n\n", false);

		MainScreen.text("You awake some time later in a prodigious puddle of girly spunk. Your attempt to sit up is impeded by a clunk against the ground.  A clunk? You prop yourself up on your elbows and glance down to see a large metal pipe, stuck in your vagina presumably some time during your black-out. Reaching down, you extricate a full ten inches of piping from your cunt, and shake your head at the tenacity of your unworldly bodily invader. You are happy to feel a tightening in your crotch, however, as the magic finally wears off, returning your womanliness to a " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ". You also note the absence of your gigantic clitoris, and give yourself a little stroke to confirm; it's reverted to normal size as well. All's well that ends well, you suppose.\n\n", false);
		MainScreen.text("Her pleasure dealt with, the ghost girl appears head-first from your belly, flowing out and reconstituting herself into the normal, brown-eyed girl you first encountered.  Almost as an afterthought, she smiles, moving to her clothing and swiftly dressing, then gives you a little wave before wandering away, rounding a corner and disappearing from sight. You resolve to find her again- her magic was pretty fun!", false);
	...
	private hermaphroditeGhostsCumEctoplasm(): void {
		...
		let x: number = player.biggestCockIndex();
		MainScreen.text("", true);
		if (Flags.list[FlagEnum.SHOULDRA_HERMSEX_COUNT] == 0) MainScreen.text("She abruptly gasps in excitement and begins moving her hands towards your groin, though you swiftly reclaim control of the unruly appendages. \"<i>You have a penis,</i>\" she telepathically pulses to you with an anxious and excited edge to her voice. \"<i>My hosts... they rarely... they... </i>\"  ", false);
		//[if repeated] 
		if (Flags.list[FlagEnum.SHOULDRA_HERMSEX_COUNT] > 0) MainScreen.text("She chuckles softly as she recalls the times you've shared, and she hungrily goes for your maleness once more, though you mentally throw her off of it.  ", false);
		MainScreen.text("The communication suddenly breaks as the ghost-girl's will surges against yours, all thoughts of submissiveness vanishing from her mind.  ", false);
		if (player.stats.int < 50) MainScreen.text("It's not very hard for her to break down your fortifications, and you soon feel yourself being pushed aside by her vast mental capacity.", false);
		else if (player.stats.int < 90 || player.cockThatFits(80) == -1) MainScreen.text("You begin to gain an edge, but the ghost girl is craftier than you expected, and she begins to pulse mental intrusions at you, scrambling your thoughts for just long enough for her to overtake you and push you to the side.  \"<i>You are one hundred years too early to challenge me,</i>\" she chides. \"<i>My intellect has long surpassed most any living creature on this plane.</i>\"", false);
		else {
			x = player.cockThatFits(80);
			MainScreen.text("The ghost girl puts up a good fight, but you're more than prepared for her mental assault. You feel her panic well as her intrusions are overwhelmed, and though she attempts to disengage the mental link, you easily stop her. She screams mental protests at you, but your ringing ears only strengthen your psychological dominance over the poor spirit. Before long, she's reduced to a whimpering wreck. \"<i>Out,</i>\" you command, tired of the troublesome specter's meddlings. Of course, she won't even try to rebel now, resignedly accumulating her essence and beginning her exit. Within moments the ghost girl flows out of your body and resolidifies before you.\n\n", false);

			MainScreen.text("She moves to leave in a huff, but you stop her with a single telepathic demand. She's yours, you remind her, and since she's gone through the trouble of trying to get you off, she might as well finish the job. Even as her lips purse in a stubborn pout, a dominating wave of influence sweeps over her, and regardless of herself, she begins unsteadily moving towards you. Resolving to meet halfway, you stride over to her and push her to her knees.  As she watches, you shed your " + player.inventory.armor.displayName + " and hoist your " + CockDescriptor.describeCock(player, x) + " up to her lips, leaving a fairly clear implication of your expectations. The ghost girl's eyes widen at the sight of your manhood, and she cringes as a new wave of arousal sweeps through her. She gives a little whimper at her inability to quell her overwhelming libido, but you simply laugh mockingly and gesture at your maleness.  \"<i>Ghost blowjob,</i>\" you mutter snidely.\n\n", false);
			...
			if (player.lowerBody.balls == 0) MainScreen.text("The first effects of the spell make themselves known in a big way when an overladen cumsack forms and drops between your legs.  ", false);
			MainScreen.text("The strange feeling focuses on your testes, making it a bit hard to concentrate on keeping the ghost girl subdued.  Your management of willpower does not waver until you feel your legs being pushed farther apart.  A curious glance turns into a horrified stare as you gaze upon your now ");
			if (player.lowerBody.ballSize > 10) MainScreen.text("gargantuan");
			else MainScreen.text("basketball");
			MainScreen.text("-sized balls.\n\n", false);
	...
	private ourDadTaughtUsNotToBeAshamedOfOurDicks(): void {
		...
		let x: number = player.biggestCockIndex();
		//[first encounter] 
		if (Flags.list[FlagEnum.SHOULDRA_PENIS_DEFEAT_TIMES] == 0) MainScreen.text("She abruptly gasps in excitement, ", false);
		else MainScreen.text("She 'ooo's softly as she recalls your anatomy, ", false);
		MainScreen.text("and you feel your hands moving unbidden towards your crotch. You snap the sneaky extremities back to your sides, knowing exactly what she wants to do. The ghost girl soon proves herself the stronger in the contest of wills, however, and eventually, your will breaks and she takes complete control once again. She forces your body to stand, removing your " + player.inventory.armor.displayName + " and throwing them carelessly to the ground. She takes your " + CockDescriptor.describeCock(player, x) + " in both hands and fondles it awkwardly, obviously inexperienced with the male anatomy.  \"<i>Most of my hosts have been women,</i>\" she explains as she continues to grope your lengthening member.  \"<i>And even then, I rarely got to play with their...</i>\"\n\n", false);

		MainScreen.text("You feel your face flush as the ghost girl relives her awkward previous possessions. Your penis finally extends to its fullest, and your hands slide up and down its length in a clumsy yet admittedly effective technique. Sweat soon begins lathering your body as you sink to your knees, confusingly overwhelmed by the pleasure of a simple self-stimulation. You've never felt this good while masturbating! You figure the otherworldly influence of your new 'room-mate' has something to do with this. As if to prove your theory, your mouth moves in the strange archaic language of the girl, and your body stiffens in both pain and pleasure as ", false);
		///[if present] 
		if (player.lowerBody.balls > 0) MainScreen.text("your testicles churn with fertility and ", false);
	...
	private ghostGinaLosses(): void {
		spriteSelect(67);
		MainScreen.text("The ghost girl heaves a mildly disappointed sigh as she settles into your body fully.  \"<i>I've had... many female hosts,</i>\" she explains. You are almost embarrassed regarding her disapproval, and actually contemplate apologizing before remembering the source of the lament. You cross your arms (or try, as the ghost still has full control of your body) and give an annoyed huff.  ", false);
		if (Flags.list[FlagEnum.SHOULDRA_VAGINAL_POSSESSIONS] == 0) MainScreen.text("She 'hmm's several times, raising your hand and tapping your finger against your chin. She snaps your fingers as a revelation washes over her.  \"<i>Of course!</i>\" she cries excitedly.  \"<i>I actually have never done this before!</i>\"", false);
		else MainScreen.text("\"<i>Oh well, I suppose I can always just do that again,</i>\" she muses. \"<i>It worked well enough the first time.</i>\"", false);
		MainScreen.text("\n\n", false);

		MainScreen.text("You are about to question exactly what she is referring to, but quickly fall silent as she slides out of your " + player.inventory.armor.displayName + " and idly strokes your cunt. She takes control of your mouth and chants a fairly lengthy spell, her absent-minded masturbation steadily gaining fervor as she progresses. With a final, shouted word of power, both of you cry out as your clitoris suddenly pulses and swells into your hand, the increased sensitivity nearly driving you insane. Your bitch-button does not stop there, however, puffing up with each heartbeat. Your eyes roll back in your head and small primitive grunts and moans escape your lips as your clit engorges in the ghostly magic, not stopping until it reaches at least four times its original length. A breeze blows through the ruined rooftops, and you scream as the wind brushes against your elongated bitch-button. Too sensitive... the thing is way too sensitive!\n\n", false);

		MainScreen.text("Against your will, and against all of your better instincts, the ghost-girl, as aroused as you are uncomfortable, moves both hands to your sex, gripping it and giving it a squeeze. Your entire body locks up, causing you to fall backwards onto the pavement as a devastating orgasm rocks through your system. A strangled cry is the only reaction you can give to the explosive sensations. The ghost-girl, however, not satisfied with the suddenness of the climax, begins feverishly stroking your clitoris with one hand, the other reaching into your love-tunnel and stimulating the rosy flesh inside. You arch your back and attempt to dig small holes into the road with your " + LowerBodyDescriptor.describeFeet(player) + " as shock-waves run through your system over and over, never giving you a chance to recuperate. You hear, as if from a great distance, a mix of frenzied laughter and orgasmic cries of pleasure ringing inside your head. Finally, mercifully, you fade into unconsciousness, leaving the ghost girl to her constant stimulation.\n\n", false);
		MainScreen.text("You awake some time later in a prodigious puddle of girly spunk and attempt to slowly sit up, only to be impeded by a clunk against the ground.  A clunk?  You prop yourself up on your elbows and glance down to see a large metal pipe, stuck in your vagina presumably some time during your black-out. Reaching down, you extricate a full ten inches of piping from your cunt, and shake your head at the tenacity of your unworldly bodily invader. You are happy to feel a tightening in your crotch, however, as the magic finally wears off, returning your womanhood back to a " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ". You also note the absence of your gigantic clitoris, and give yourself a little stroke to confirm; it's reverted to normal size as well. All's well that ends well, you suppose.\n\n", false);
	...
	private loseToShouldraAsHerm(): void {
		spriteSelect(67);
		if (Flags.list[FlagEnum.SHOULDRA_HERMSEX_COUNT] == 0) MainScreen.text("She abruptly gasps in excitement, ", false);
		else MainScreen.text("She coos softly as she recalls your anatomy, ", false);
		MainScreen.text("and you feel your hands moving unbidden towards your crotch. You snap the sneaky extremities back to your sides, knowing exactly what she wants to do. The ghost girl soon proves herself the stronger in the contest of wills, however, and eventually, your will breaks and she takes complete control once again. She forces your body to stand, removing your " + player.inventory.armor.displayName + " and throwing them carelessly to the ground. She takes your " + CockDescriptor.describeCock(player, 0) + " in both hands and fondles it awkwardly, obviously inexperienced with the male anatomy.  \"<i>Most of my hosts have been women,</i>\" she explains as she continues to grope your lengthening member.  \"<i>And even then, I rarely got to play with their...</i>\"\n\n", false);
		...
		if (player.lowerBody.balls > 0) MainScreen.text("Stopping her double-handed stroke technique, she slips your hand under your obtrusive " + BallsDescriptor.describeBalls(true, true, player) + " and gently pushes them aside to get a better look at your previously unnoticed female sex.  ", false);
		MainScreen.text("She chuckles evilly, and you groan, mind cycling through the possible outcomes of the girl's impulses. Your lips curl unbidden in an evil smile, then begin mouthing an archaic incantation, and you wince inwardly, bracing for the worst. She finishes and goes back to the vigorous stroking, and you are left to wonder what the spell was meant to do—that is, until you feel a tingling beginning at the base of your " + CockDescriptor.describeCock(player, 0) + ", slowly climbing up the shaft until your entire length is covered in the sensation.\n\n", false);
	...
	private shouldraGiantCockLoss(): void {
		spriteSelect(67);
		let x: number = player.biggestCockIndex();
		MainScreen.text("Her attention inevitably moves to your gigantic dick, somehow still contained in your garments, and she swiftly clamors to free it from its prison. Practically tearing off your " + player.inventory.armor.displayName + ", she tilts your head back in order to fully take in your " + CockDescriptor.describeCock(player, x) + ", a small line of drool escaping the edge of your mouth. She opens your mouth as if to speak, but cannot find words, instead settling for a long and anxious groan. Her first reaction is to simply hug the titantic shaft, squeezing happily. Eventually, she seems to realize her original intent and draws back, stroking your chin with your hand while she apparently ascertains the best way to stimulate your monster. With a small 'aha!', she struggles to stand, unused to such a strange center of balance. Bringing you along for the ride, she begins wandering the streets of the city, taking control of your mouth and shouting \"<i>My oh my, how on earth will I ever blow my load with this GIANT dick?</i>\" Her efforts are rewarded when you hear a small pitter-patter of feet, distantly at first, but hastily heading toward you. A duo of goblins rush out around a corner, take a moment to confirm the reported size of your manhood, and hurry over, undressing, tripping and shoving each other in an effort to win the race to win your attention. As they reach you the excited little creatures nearly bowl you over as they start stroking and licking every inch of cockflesh they can reach. \"<i>Girls!</i>\" the ghost exclaims sternly (with your voice), crossing your arms in front of you as they turn their heads to regard you. She allows a hint of a smile to show at the corners of your mouth as she continues, \"<i>Trust me when I say there will be enough of me for both of you.</i>\"\n\n", false);

		MainScreen.text("Without another word, she reaches out and grabs one of the goblins by the shoulder and begins chanting in her strange unknown language, wiggling the fingers of your absent hand at the ", false);
		//[if first time]
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("bemused ", false);
		else MainScreen.text("excited ", false);
		MainScreen.text("girl. With a flair, she finishes the incantation, then moves to the other goblin and repeats the process. The first goblin, meanwhile, ", false);
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("searches her body for any signs of change, and when she finds none, begins pouting.", false);
		else MainScreen.text("resists the urge to smash her tits together, waiting with as much patience as she can muster.", false);
		MainScreen.text("  By this time, the ghost girl has finished with the spell, ", false);
		//[if first time] 
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("and instead of offering an explanation, she merely ", false);
		else MainScreen.text("and, to the green girl's obvious delight, ", false);
		MainScreen.text("reaches over and kneads the goblin's ", false);
		//([if first time]
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("small, firm B-cups", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) MainScreen.text("perky, sizeable C-cups", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) MainScreen.text("well-formed, jiggly D-cups", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) MainScreen.text("big and squishy E-cups", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) MainScreen.text("oversized, heaving F-cups", false);
		else MainScreen.text("gigantic and openly-bared G-cups", false);
		MainScreen.text(". The goblin ", false);
		//[if first time] 
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("instantly stops her protests, suddenly too stimulated by the massage to worry about any magical favoritism, ", false);
		else MainScreen.text("cries out in ecstasy at the still-incredible sensation, ", false);
		//[if first time]
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("although that doesn't stop the second goblin to begin voicing her own dissent", false);
		else MainScreen.text("and you can't help but notice the excited blush running across the second goblin's face", false);
		MainScreen.text(". The ghost girl pinches a quickly-stiffening nipple and pulls it out, ", false);
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("and... pulls the breast along with it?", false);
		else MainScreen.text("and pulls the breast along with it.", false);
		MainScreen.text("  You inwardly gasp ", false);
		//[if first time] 
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("as the effects of the magic are revealed", false);
		else MainScreen.text("despite seeing the magic before, always enthralled by the process", false);
		MainScreen.text("; it has turned the goblin's breasts into an almost putty-like substance", false);
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text(", evidently!", false);
		else MainScreen.text(" once again", false);
		MainScreen.text(". The ghost girl begins to speed the process up, softly laying the goblin on the ground and kneading out the green girl's tits, adding at least half a cup size with every tug. The second goblin, watching intently behind your back, ", false);
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("gasps in amazement, ", false);
		else MainScreen.text("can't restrain her urges any longer, ", false);
		MainScreen.text("swiftly reaching down to roughly grab at her own ", false);
		//[if first time] 
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("petite boobs", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) MainScreen.text("modest bosom", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) MainScreen.text("sizable chest", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) MainScreen.text("large endowments", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) MainScreen.text("huge tits", false);
		else MainScreen.text("titanic melons", false);
		MainScreen.text(", crying in excitement as her breasts also begin to billow out under her own ministrations.\n\n", false);

		MainScreen.text("The first goblin's breasts, at this time, have reached at least ", false);
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("E-cups", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) MainScreen.text("F-cups", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) MainScreen.text("G-cups", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) MainScreen.text("H-cups", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) MainScreen.text("I-cups", false);
		else MainScreen.text("J-cups", false);
		MainScreen.text(", and the ghost girl speeds up the massage ever further. Not satisfied with the progress, she raises the goblin's head to your lips and blows a steady gust of wind down the little woman's throat, ballooning her glistening mammaries out much more efficiently. This continues until the goblin nearly collapses under the weight of her new ", false);
		//[if first time] 
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("HH-cups", false);
		//[if repeated once] 
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) MainScreen.text("II-cups", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) MainScreen.text("JJ-cups", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) MainScreen.text("KK-cups", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) MainScreen.text("LL-cups", false);
		else MainScreen.text("MM-cups", false);
		MainScreen.text(", and the ghost girl turns your attention to the second goblin, whose boobs are embarrassingly uneven due to her...unpracticed skills. The ghost girl moves you over to her, mercifully evening the currently ", false);
		//([if first time] 
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("DD-cup (and C-cup, on the other side)", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) MainScreen.text("EE-cup (and D-cup, on the other side)", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) MainScreen.text("FF-cup (and E-cup, on the other side)", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) MainScreen.text("GG-cup (and F-cup, on the other side)", false);
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) MainScreen.text("HH-cup (and G-cup, on the other side)", false);
		else MainScreen.text("II-cup (and H-cup, on the other side)", false);
		MainScreen.text("breasts out, before again blowing into the goblin's mouth, inflating the goblin until she matches the first. With an excited giggle, the ghost girl swings you around to stand between the two amazed goblin girls, flinging a large glob of pre-cum into the first goblin's face, which she promptly smears over her breasts, enjoying the extra lubrication. She slowly drops to your back, presenting your cock vertically to the two increasingly eager girls, and gives a simple order: \"<i>Titfuck.</i>\"\n\n", false);

		MainScreen.text("Not needing to be asked twice, the two girls fall into place on each side of your prodigious shaft, heaving their engorged bosoms up and wrapping them around your penis. Pillowy boobflesh smooshes delightfully onto your crotch, around your " + CockDescriptor.describeCock(player, x) + ", and meeting in the middle, the goblins' nipples slipping against each other. They look to you for final confirmation, and the ghost girl nods your head; at this point, you find yourself voluntarily nodding along with her, completely entranced by this situation. With aroused grunts and moans, the goblins begin sliding their soft, slippery flesh up and down your shaft, and, on a sudden influence, they step onto your hips, heaving themselves up and using the increased height to stimulate even more of your " + CockDescriptor.describeCock(player, x) + ". You and the ghost girl both reel back by the sensations of the goblin double-paizuri, and it's not long before you feel yourself reaching the end of your sexual rope, and the ghost girl informs the goblins of this. Faster than you thought possible, given their recent additions, the second goblin zooms over to your shoulders and hoists you up to a sitting position, your gigantic cock falling right into the hands of the first girl, who promptly wraps her pair of tips around each side of your head and plunges her lips around your now-dilating urethra, sucking eagerly and squeezing dollops of breast milk out of her over-stimulated bosom. To help push you over the edge, the second goblin squishes her own pair of boobs against your back and slides up and down, leaving small milky trails across you.\n\n", false);

		MainScreen.text("With an anxious moan, you tilt your head back and tense your body as seed begins to trek across the expansive tract of land between your crotch and your cockhead, and the second goblin promptly scrambles over, leaving you to prop yourself up as you wiggle about and clench your jaw in anticipation. The goblin duo position themselves against your manhood and open wide as the first glob of semen slams against them with enough force to bulge both of their cheeks with the stuff. They quickly heft their bosoms under them, catching any errant seed in their vast canyons of cleavage. This continues, the volume of cum increasing with every pulse until both goblins might as well have dumped four gallons of white paint onto themselves. With a final, intense blast, you fall backwards, chest heaving while the aftermath of the climax hits you. While you lay there, occasional drops of semen splattering onto your " + chestDesc() + ", you manage a glance at the two to find them working together to scoop as much accumulated cum as possible into their needy fuck-holes. You zone out for around five minutes, snapping out of your daze soon after when you realize the lack of ghostly presence. You unsteadily rise to see the goblins still going at it with your semen, bellies bloated with the volume of your impressive orgasm. They stumble to their feet, making sure to move a hand under their cunts to hold in as much seed as they can while they redress—if you can even refer their get-ups as clothing. You notice their oversized breasts rapidly shrinking, deflating until they return to near-normal conditions. You manage a bit of a smile when you realize that ", false);
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] <= 4) MainScreen.text("they have kept at least a cup size of flesh from their experience", false);
		else MainScreen.text("their breasts seem to have reached their crescendo, keeping the same (admittedly incredibly stacked) measurements", false);
		MainScreen.text(", which they happily fondle as they wander away, side-by-side. Moving to your " + player.inventory.armor.displayName + " and redressing, you shake your head, wondering, not for the first time, if such a large appendage is truly beneficial to you, and wondering if this was the last you've seen of your two new goblin friends.\n\n", false);
		MainScreen.text("A strange sensation begins in your midsection, and you press a hand against the flesh curiously. You remove it swiftly as a familiar head suddenly pushes out, wiggling about until it faces you. She smiles, then draws her arms out of her torso and uses them as leverage (with a whispered spell to make her hands tangible) to remove herself from you completely. Muttering an incantation and solidifying her again-unremarkable form, she winks at you and gathers her tunic, slipping it over her frame. As you watch, she slips her thumb and forefinger into her satisfied snatch, producing a healthy helping of a spooky gooey substance. Reaching into a pocket inside her tunic, she draws out a small bottle, which she promptly squishes the ectoplasm into. She offers you the bottle, explaining, \"<i>The product of a... ghost climax, if you will. Keep it, it's good for you.</i>\"  With a happy goodbye, she wanders into an alley, grabbing her ruined leggings and discarded shoes as she goes, and disappears into the shadows. You move to leave, but find your " + LowerBodyDescriptor.describeLegs(player) + " growing heavy, fatigue washing over you in the aftermath of the possession. Before you know it, you're on your knees, and you have to struggle to keep your eyelids from drooping. You succumb to the exhaustion, slumping to the ground and soon snoring contentedly.", false);
	...
	private genderlessShouldrasLossRapes(): void {
		spriteSelect(67);
		MainScreen.text("Excited to find out what manner of host you are, the ghost-girl swiftly assumes complete control of your body and cups a hand against your crotch to feel... nothing. Confused, she quickly wriggles out of your " + player.inventory.armor.displayName + " and goes in for another grope, feeling nothing but skin. \"<i>Son of a... please don't tell me you don't have any sexual bits I can play with!</i>\" she huffs, and you chuckle sheepishly, not knowing what to tell her. She groans unhappily and falls back onto your " + ButtDescriptor.describeButt(player) + ", muttering a string of curse words under your breath. \"<i>That's it!</i>\" she fumes. \"<i>I'm going to get off, whether you want me to or not!</i>\" She resumes cursing and wiggling your hips for a bit longer, then stops suddenly, a smirk playing across your face. \"<i>Whether you want me to or not...</i>\"\n\n", false);

		MainScreen.text("She hops up and makes for the decrepit hut you first saw her in.  ", false);
		if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0) MainScreen.text("You raise an eyebrow, genuinely curious as to how she'll figure out how to relieve her desire.  ", false);
		MainScreen.text("Darting inside, the ghost girl heads unerringly towards a warped and half-rotted dresser. After searching several drawers filled with various knick-knacks and trinkets, the spirit squeaks in excitement as she produces a seemingly regular egg carton. She moves to the other side of her room and opens a curiously well-made chest, setting the carton on a nightstand as she goes.  ", false);
		if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0) MainScreen.text("Curious as to its contents, you glance inside- it's a chest of dildos, of every shape and size. Before you can ask why the hell a ghost would carry a CHEST of dildos, ", false);
		else MainScreen.text("You recall distinctly the contents of that chest, and are not surprised ", false);
		MainScreen.text("when she reaches in and rummages about, producing two eight-inch vibrators with bumpy demon-shafts. She moves back to the center of the room, launching into a short spell as she goes. She sets the dildos on the same nightstand and opens the carton, pulling out four large white eggs.\n\n", false);

		MainScreen.text("You marvel at how resourceful the ghost-girl is, then pause and consider the ramifications of the eggs. Your suspicions are confirmed as ", false);
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) MainScreen.text("two new additions bud on your chest.", false);
		else if (player.upperBody.chest.BreastRatingLargest[0].breastRating <= 8) MainScreen.text("your " + BreastDescriptor.describeAllBreasts(player) + " begin to bounce all around.", false);
		else MainScreen.text("your " + BreastDescriptor.describeAllBreasts(player) + " swells slightly to make room for a sudden increase in milk production.", false);
		MainScreen.text("  As the ghost continues darting around the room, ", false);
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating <= 8) MainScreen.text("your new EE-cup breasts settle", false);
		else MainScreen.text("the lactation continues", false);
		MainScreen.text(", and the ghost girl moves back to the table and settles down a bit. Satisfied with the changes, she wastes no time on tapping the eggs against the table, cracking them cleanly. Your arm freezes up momentarily as she tips the half-shell towards your similarly conflicted mouth, but she wins out quickly, forces your jaw open, and drops the contents inside. She swallows the tasty contents easily, then slides her hands to your chest and lifts your huge tits in anticipation. Your nipples swell up and out, lengthening and stiffening as the first three eggs are consumed. The last one, however, produces a completely different reaction on your body; a comfortable warm tingle begins, deep in the center of your oversized breasts, migrating up steadily to your nipples. With a satisfied groan, the ghost-girl circles the tingling teats with a teasing finger as the change continues. Finally, she sinks a finger into each of the suddenly yielding nipples, shivering in pleasure from the penetration. Wasting no more time, she snatches up her two dildos and shoves them into your ", false);
		//[if new]
		if (!player.upperBody.chest.hasFuckableNipples()) MainScreen.text("new ", false);
		else MainScreen.text("even larger ", false);
		MainScreen.text("nipplecunts, flicking them on as the bumpy demon-dildos penetrate your virgin tits. The ensuing vibration sends little waves up and down your boobs as they jostle about, and the ghost-girl gives a strangled yelp of pleasure. Drunk with the sensation, she staggers about, mewling and gurgling in the pleasure of having your nubs violated. Her foot gets snagged on a bedpost and she flops onto the ground, chest first, the bumpy sextoys pushing slightly further into your tits. She slams your pelvis onto the floor several times in a futile attempt to further her joy, then flops to the side, eyes finally falling upon the still-open chest of dildos. She slowly drags herself to it (mashing your bosom against the ground and inciting a few happy grunts), reaching in and grabbing up a 13-inch floppy horsecock replica. Your irises shift halfway between the yellow pallor of your possessor and your natural palette as your eyes widen in horror. She couldn't possibly...\n\n", false);

		MainScreen.text("She smacks it ", false);
		if (player.lowerBody.isNaga()) MainScreen.text("against your naga-crotch", false);
		else if (player.lowerBody.isGoo()) MainScreen.text("into your cushiony goo", false);
		else MainScreen.text("between your legs", false);
		MainScreen.text(", seemingly forgetting about your lack of genitalia, and grudgingly goes for the next penetrable orifice; your " + ButtDescriptor.describeButthole(player) + ". With blatant disregard for safety, the vibrators still buzzing happily in your increasingly warm nipples, she slams the horsecock against the ground, waits for the wiggling to somewhat cease, then lines your butt up with the flared tip. Her crazed laughter mingles with your horrified scream as she sits on it, driving it at least four inches inside in its initial penetration", false);
		//[if anus is not gaping]
		if (player.lowerBody.butt.analLooseness <= 4) MainScreen.text(" and stretching your butt obscenely", false);
		MainScreen.text(".", false);
		...
		MainScreen.text(".  With a happy sigh, you heave yourself upright, slipping back into your " + player.inventory.armor.displayName + " and moving out into the ruins once more.\n\n", false);
		MainScreen.text("A strange sensation begins in your midsection, and you press a hand against the flesh curiously. You remove it swiftly as a familiar head suddenly pushes out, wiggling about until it faces you. She smiles, then draws her arms out of her torso and uses them as leverage (with a whispered spell to make her hands tangible) to remove herself from you completely. Muttering an incantation and solidifying her again-unremarkable form, she winks at you and gathers her tunic, slipping it over her frame. As you watch, she slips her thumb and forefinger into her satisfied snatch, producing a healthy helping of a spooky gooey substance. Reaching into a pocket inside her tunic, she draws out a small bottle, which she promptly squishes the ectoplasm into. She offers you the bottle, explaining, \"<i>The product of a... ghost climax, if you will. Keep it, it's good for you.</i>\"  With a happy goodbye, she wanders into an alley, grabbing her ruined leggings and discarded shoes as she goes, and disappears into the shadows. You move to leave, but find your " + LowerBodyDescriptor.describeLegs(player) + " growing heavy, fatigue washing over you in the aftermath of the possession. Before you know it, you're on your knees, and you have to struggle to keep your eyelids from drooping. You succumb to the exhaustion, slumping to the ground and soon snoring contentedly.", false);
		Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT]++;
		player.orgasm();
		player.stats.sens += 1;
	...
	private ghostGartuanLossSceneOrSomeShit(): void {
		spriteSelect(67);
		let x: number = player.biggestCockIndex();
		MainScreen.text("Your entire body shivers in delight as the ghost girl regards your excessively large " + CockDescriptor.describeCock(player, x) + ". She places your hand over the bulge in your " + player.inventory.armor.displayName + " and titters in glee as your member twitches and hardens, just a tad. She strips eagerly, but is halted by a familiar booming voice coming from the half-erect member. \"<i>WHO THE HELL IS THIS BITCH?</i>\" a perplexed Exgartuan yells grumpily. \"<i>Living in peoples' bodies is MY THING!</i>\"\n\n", false);
		...
		MainScreen.text("\"<i>Hold up,</i>\" the demon requests, suddenly far less boisterous. \"<i>You're sayin' you're a chick, right?  And not some chick with a dick", false);
		if (player.gender == 3) MainScreen.text("like this bitch here", false);
		MainScreen.text("?</i>\"\n\n", false);
		...
		MainScreen.text("When the proverbial dust settles, you stir groggily. Blinking the sleep out of your eyes, you do a double take at the sheer amount of semen flooding the street. You feel a sensation building in your finally flaccid member, and you glance down. A pale, transparent smoke flows gently from your urethra, reforming into a very tired, moderately sore, but incredibly satisfied ghost girl.  ", false);
		if (Flags.list[FlagEnum.SHOULDRA_EXGARTUAN_SPIRIT_SEX_COUNT] == 0) MainScreen.text("\"<i>Spirit sex... I can't say I've ever tried that,</i>\" she admits. \"<i>Damn if it isn't good.</i>\"", false);
		else MainScreen.text("\"<i>Damn,</i>\" she sighs, \"<i>that is still a great ride, I'd say...</i>\"", false);
		MainScreen.text("\n\n", false);
	...
	private courtCrazyGirlsDotCom(): void {
		spriteSelect(66);
		MainScreen.text("", true);
		MainScreen.text("Her smile widens as she regards your venerable visage. \"<i>Is that so?</i>\" she says softly. \"<i>Interesting. I'll be back in a moment.</i>\" She returns to her \"<i>house,</i>\" rummaging around for some time before returning to you, hands hidden behind her. \"<i>If I am to be sought by a paladin, we must do things right,</i>\" she explains, handing you a small silk handkerchief. The white fabric slides around your fingers like a sacrosanct flow of holy water, and you reverently tuck her gift into your " + player.inventory.armor.displayName + ", laying it over your heart. \"<i>Come back in two days' time, please,</i>\" she asks softly. Your eyes meet for a long moment, and it appears as if she's struggling to say something. She instead breaks away and starts back towards her shelter. \"<i>Be safe,</i>\" she advises, and with a smile and a wave, she's gone. For moments you stand silently, then you turn and start away with long, saintly strides. There's more work to do.\n\n", false);
		Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00365] = 48;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private slimeGhostGalForHerms(clearOut: boolean = false): void {
		...
		MainScreen.text("Your naked trot towards the emerald ooze stops as worry washes over your sexual cravings.  The contemplative expression forming on your face catches the attention of your ghostly friend.  \"<i>I know exactly what you're mulling over, but don't worry,</i>\" she assures you with a condescending chuckle.  The haunted blob slowly approaches you, seamlessly transitioning from its shapeless form to a perfect representation of the apparition mid-stride.  \"<i>");
		//{event occurrence == 0}
		if (Flags.list[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 0) MainScreen.text("I've got this thing all figured out.");
		//event occurrence =1
		else if (Flags.list[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 1) MainScreen.text("I've got this thing all figured out.  For real this time.");
		//event occurrence >=2
		else if (Flags.list[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 2) MainScreen.text("I've got this thing all figured out.  For real this time.  Promise.");
		//event occurrence >=3
		else MainScreen.text("Well... don't worry TOO much.");
		MainScreen.text("</i>\"");
		...
		//event occurrence == 0
		if (Flags.list[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] <= 2) MainScreen.text("Maybe she really does have a firm grip on the reigns of this beast.");
		//event occurrence >=3
		else MainScreen.text("Regardless, you've been through this dog and pony show enough times to know that something is bound to go wrong anyway.");
		...
		//vaginalLooseness == 0
		if (player.lowerBody.vaginaSpot.get(0).vaginalLooseness == 0) MainScreen.text("tight");
		//vaginalLooseness == 1
		else if (player.lowerBody.vaginaSpot.get(0).vaginalLooseness == 1) MainScreen.text("forgiving");
		else if (player.lowerBody.vaginaSpot.get(0).vaginalLooseness == 2) MainScreen.text("loose");
		else if (player.lowerBody.vaginaSpot.get(0).vaginalLooseness == 3) MainScreen.text("gaping");
		else MainScreen.text("cavernous");
		MainScreen.text(", velvety scabbard.  Hands too busy latched onto one another's hips, ");
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) MainScreen.text("both you and the spooky slime press your tits together, your swaying thrusts only enhancing the mammary meltdown.");
		else MainScreen.text("the spooky slime presses her tits hard into your flat chest, your gyrations teasing them to no end.");
		MainScreen.text("  The fierce battle increases in tempo; the sly leering the two of you used to try and psyche the other out is giving way to slight head-lashing and gasps through clenched teeth.");
		...
		if (Flags.list[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] > 0) MainScreen.text("  Again.");
		...
		MainScreen.text("\"<i>Boo!</i>\"  your face shouts at you, triggering a panicked backwards jolt from you which then results in you slipping back down on your [butt].  The spook easily phases out of you during your tumble, standing perfectly still from your previous position as she glares down at you with the same mischievous smile from earlier.");
		MainScreen.text("\"<i>Yea, I jumped ship from that senseless slime the moment it finished filling you up to christen the S.S.  Ballsac,</i>\" she explains, her tone free of any responsibility.  \"<i>You're lucky I was around to pick up the slack for you.  ");
		if (Flags.list[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 0) MainScreen.text("You couldn't spank it even to save your life, as we've learned today.</i>\"");
		else MainScreen.text("Evidently, you haven't put in any meaningful practice since last time,</i>\" the specter chides you while making an exaggerated jerking-off motion.");
		if (Flags.list[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 1) MainScreen.text("  \"<i>I was afraid you'd have... given up the ghost.</i>\"");
		else if (Flags.list[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] > 1) MainScreen.text("  \"<i>But I'm more than willing to do it again next time with a conclusion like that!</i>\"");
		...
		Flags.list[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER]++;
	}
}
}
