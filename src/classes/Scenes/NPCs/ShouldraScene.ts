...
internal function shouldraGreeting():void {
	...
	if(flags[FlagEnum.TIMES_MET_SHOULDRA] < 3) {
		...
		//([if first time] 
		if(flags[FlagEnum.TIMES_MET_SHOULDRA] == 0) outputText("\"<i>You're the asshole that took my racks, aren't you?  I was hoard-...er, I was keeping an eye on those!</i>\" she accuses, eyes narrowing angrily.  ", false);
	...
	//(after three encounters with her)
		...
		if(flags[FlagEnum.TIMES_BEATEN_SHOULDRA] >= 3) {
			...
			//([if first time] 
			if(flags[FlagEnum.TIMES_BEATEN_SHOULDRA] == 3) outputText("You can't say you saw that coming.  It seems she has offered to forego the fight in lieu of sexings.  ", false);
...
internal function defeatDannyPhantom():void {
	...
	if(monster.HP < 1) {
		...
		if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] >= 1) outputText("ghost ", false);
	...
	//(defeat by lust)
		...
		if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] < 1) outputText("Strangely, the action seems to have little to no effect.", false);
...
private function noSlimingSlimer():void {
	...
	//([if first time] 
	if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] == 0) outputText("not feeling up to heed the strange girl's whims.", false);
	...
	//([if first time]
	if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] == 0) outputText("girl (what was that about her 'taking over' a goblin?)", false);
...
private function littlePigLittlePigLetMeIn():void {
	...
	if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] == 0) outputText("you can afford being a little adventurous with the obviously desperate girl", false);
	...
	//([if first time] 
	if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] == 0) outputText("Wait... WHAT!?  Did you just let some sort of... g-... g-... g-... G-... G-... GHOOOOOST possess you?", false);
...
private function sharkbustersVictory():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText(", " + allBreastsDescript() + " squishing and squashing against her own barely-clad bosom", false);
...
private function sharkyEctoginas():void {
	...
	//[if repeat]
	if(flags[FlagEnum.TIMED_SHARKGINAS] > 0) outputText(" again...", false);
	outputText("</i>\" she pants, brow suddenly furrowed with worry.  ", false);
	if(flags[FlagEnum.TIMED_SHARKGINAS] == 0) outputText("You begin to ask what she's talking about, but the words die in your mouth when", false);
	...
	//[if breasts present]
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("and caking your cleavage with the excess lubrication", false);
...
private function ghostIzmaPenis():void {
	...
	if(player.lowerBody.balls > 0) outputText(" and cradling your " + ballsDescriptLight() + " with almost reverent gentleness, running her coarse shark-skinned fingers across your sack as gently as she can manage", false);
	outputText(".\n\n", false);
	...
	//[if first time]
	if(flags[FlagEnum.TIMES_SHARKPENISED] == 0) outputText("whose expression reveals she's as clueless as you are.", false);
	...
	if(flags[FlagEnum.TIMES_SHARKPENISED] == 0) outputText("absently as she contemplates what could have caused the lengthening of your loins", false);
	...
	if(flags[FlagEnum.TIMES_SHARKPENISED] == 0) outputText("as she deduces the nature of the spell. \"<i>I think... oh my... my host's saliva may have expansive qualities on... sexual aspects of the body,</i>\" she explains. \"<i>So, if I were to do this...</i>\"", false);
	...
	if(player.lowerBody.balls > 0) outputText(" and softly churning sack", false);
	...
	if(player.lowerBody.balls > 0) outputText("  Your enchanted testes give an audible groan as you reach climax.", false);
...
private function ghostGooGurlzDuckfaces():void {
	...
	//([if penis present] 
	if(player.lowerBody.cockSpot.hasCock() && !player.lowerBody.vaginaSpot.hasVagina()) outputText(" presenting your " + multiCockDescriptLight() + " to the spectral slime.", false);
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" slipping a hand down to your " + clitDescript() + " eagerly.", false);
	//[if herm] 
...
private function slimeGinaFlation():void {
	...
	//([if corruption <50]
	if(player.stats.cor < 50) outputText("you beg her to stop before this gets out of hand.", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("Your frame is jostled about as the slime under you shrinks.", false);
	...
	if(player.statusAffects.has("MeetWanderer")) outputText("both of Markus's testicles, combined!", false);
	...
	else if(player.tailType == TAIL.NONE) outputText("arms and " + player.legs(), false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("see your " + nippleDescript(0) + " wiggling sporadically.", false);
...
private function slimeyPenorsAhoy():void {
	...
	//([if boobs present] 
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText(" while squishing her small boobs into your own " + allBreastsDescript(), false);
	...
	if(flags[FlagEnum.SHOULDRA_SLIME_PENOR_TIMES] > 0) outputText("You can't shake the feeling that she's used this particular spell before. What an unoriginal ghost.  ", false);
	...
	if(player.lowerBody.balls > 0) outputText("and teasing your quaking testes ", false);
	...
	if(player.lowerBody.balls > 0) outputText("  The near-vacant-minded girl rewards you with two slime-tentacles to fondle your scrotum.  You hiss in a sharp breath at the sudden stimulation, but swiftly come to welcome it.", false);
	...
	if(player.lowerBody.balls > 0) outputText("the heat spreads to your testicles, promptly puffing them up with arcane virility", false);
	...
	if(player.lowerBody.balls > 0) outputText("Your " + player.legs() + " are forced apart by your burgeoning cum-sack", false);
	...
	if(player.lowerBody.balls > 0) outputText("balls to churn", false);
	...
	if(player.lowerBody.balls > 0) outputText("waddle over to where the mewling ghost girl is twitching on the ground; your now-shrinking testicles hindering your gait.", false);
...
private function genderlessGhostBuster():void {
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0) outputText("You raise an eyebrow, genuinely curious as to how she'll figure out how to relieve her desire.  ", false);
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0) outputText("Curious as to its contents, you glance inside—it's a chest of dildos, of every shape and size. Before you can ask why the hell a ghost would carry a CHEST of dildos, ", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("two new additions bud on your chest.", false);
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 8) outputText("your " + allBreastsDescript() + " begin to bounce all around.", false);
	...
	//[if flat to E-cup]
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 8) outputText("your new EE-cup breasts settle", false);
	...
	if(!player.upperBody.chest.hasFuckableNipples()) outputText("new ", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("your tits, along with the elongated nipples, receding back into a flat chest.", false);
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 8) outputText("your boobflesh shrinking back down into its normal size, nipples included.", false);
...
private function penisLossThatIsntALoss():void {
	...
	if(flags[FlagEnum.SHOULDRA_PENIS_DEFEAT_TIMES] == 0) outputText("She abruptly gasps in excitement and begins moving your hands towards your groin, though you swiftly reclaim control. \"<i>You have a penis,</i>\" she telepathically pulses to you with an anxious and excited edge to her voice. \"<i>My hosts... they rarely... they...</i>\"", false);
	...
	//[if intelligence <50] 
	if(player.stats.int < 50) outputText("It's not very hard for her to break down your fortifications, and you soon feel yourself being pushed aside by her vast mental capacity.", false);
	else if(player.stats.int < 90 || player.cockThatFits(80) == -1) outputText("You begin to gain an edge, but the ghost girl is craftier than you expected. She begins to pulse mental intrusions at you, scrambling your thoughts just long enough for her to overtake you and push you to the side.  \"<i>You are one hundred years too early to challenge me,</i>\" she chides. \"<i>My intellect has long surpassed most any living creature on this plane.</i>\"", false);
	else {
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			...
			if(player.lowerBody.cockSpot.count() == 2) outputText("your other shaft and pumps it vigorously.", false);
			else if(player.lowerBody.cockSpot.count() == 3) outputText("your other two shafts and pumps them vigorously.", false);
		...
		if(player.lowerBody.cockSpot.count() == 2) outputText("Taking her hand away from your other dick, the ", false);
		else if(player.lowerBody.cockSpot.count() >= 3) outputText("Taking her hands away from your other dicks, the ", false);
		...
		if(player.lowerBody.balls > 0) outputText("in your " + ballsDescriptLight(), false);
		else outputText("near the base of your shaft", false);
		outputText(".  ", false);
		if(player.lowerBody.balls == 0) outputText("The first effects of the spell make themselves known in a big way when an overladen cumsack forms and drops between your legs.  ", false);
		...
		if(player.lowerBody.ballSize >= 9) outputText("gargantuan");
	...
	if(player.lowerBody.balls > 0) outputText("your testicles churn with fertility and ", false);
...
private function dewormYourGhost():void {
	...
	if(flags[FlagEnum.SHOULDRA_WORM_SCENE_COUNTER] == 0) outputText(", and she responds by taking control of your hand and pointing down at the wriggling creature that crawled out of your junk. \"<i>Oh, that?</i>\" you scoff, waving your other hand in the air dismissively; your other, ghost-controlled appendage still quivers stubbornly as you attempt to explain. \"<i>There are tons of the little guys crawling around inside me. Wanna see?</i>\"", false);
...
private function penisGartuanGhostSmexVictory():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" like this bitch here", false);
	outputText("?</i>\"\n\n", false);
	...
	if(flags[FlagEnum.SHOULDRA_EXGARTUAN_SPIRIT_SEX_COUNT] > 0) outputText("once again ", false);
	...
	if(flags[FlagEnum.SHOULDRA_EXGARTUAN_SPIRIT_SEX_COUNT] == 0) outputText("\"<i>Spirit sex... I can't say I've ever tried that,</i>\" she admits. \"<i>Damn if it isn't good.</i>\"\n\n", false);
...
private function ghostGinaWinSexings():void {
	...
	if(flags[FlagEnum.SHOULDRA_VAGINAL_POSSESSIONS] == 0) outputText("She 'hmm's several times inside your head, but you can feel a surge of elation as she apparently reaches a solution. \"<i>Of course!</i>\" she cries excitedly.  \"<i>I actually have never done this before!</i>\"\n\n", false);
...
private function hermaphroditeGhostsCumEctoplasm():void {
	...
	//[if repeated] 
	if(flags[FlagEnum.SHOULDRA_HERMSEX_COUNT] > 0) outputText("She chuckles softly as she recalls the times you've shared, and she hungrily goes for your maleness once more, though you mentally throw her off of it.  ", false);
	outputText("The communication suddenly breaks as the ghost-girl's will surges against yours, all thoughts of submissiveness vanishing from her mind.  ", false);
	if(player.stats.int < 50) outputText("It's not very hard for her to break down your fortifications, and you soon feel yourself being pushed aside by her vast mental capacity.", false);
	else if(player.stats.int < 90 || player.cockThatFits(80) == -1) outputText("You begin to gain an edge, but the ghost girl is craftier than you expected, and she begins to pulse mental intrusions at you, scrambling your thoughts for just long enough for her to overtake you and push you to the side.  \"<i>You are one hundred years too early to challenge me,</i>\" she chides. \"<i>My intellect has long surpassed most any living creature on this plane.</i>\"", false);
	else {
		x = player.cockThatFits(80);
		outputText("The ghost girl puts up a good fight, but you're more than prepared for her mental assault. You feel her panic well as her intrusions are overwhelmed, and though she attempts to disengage the mental link, you easily stop her. She screams mental protests at you, but your ringing ears only strengthen your psychological dominance over the poor spirit. Before long, she's reduced to a whimpering wreck. \"<i>Out,</i>\" you command, tired of the troublesome specter's meddlings. Of course, she won't even try to rebel now, resignedly accumulating her essence and beginning her exit. Within moments the ghost girl flows out of your body and resolidifies before you.\n\n", false);
		...
		if(player.lowerBody.balls == 0) outputText("The first effects of the spell make themselves known in a big way when an overladen cumsack forms and drops between your legs.  ", false);
		outputText("The strange feeling focuses on your testes, making it a bit hard to concentrate on keeping the ghost girl subdued.  Your management of willpower does not waver until you feel your legs being pushed farther apart.  A curious glance turns into a horrified stare as you gaze upon your now ");
		if(player.lowerBody.ballSize > 10) outputText("gargantuan");
		else outputText("basketball");
		outputText("-sized balls.\n\n", false); 
	...
	if(player.lowerBody.balls > 0) outputText("Stopping her double-handed stroke technique, she slips your hand under your obtrusive " + ballsDescriptLight() + " and gently pushes them aside to get a better look at your previously unnoticed female sex.  ", false);
...
internal function loseToShouldra():void {
	...
	//[if dick] 
	if(player.lowerBody.cockSpot.hasCock()) outputText("Each pulse pushes her a bit further onto your hardening manhood", false);
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("You can't deny the heat slowly welling up in your loins, your love-tunnel getting more lubricated by the moment", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("quivering member", false);
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("sopping gash", false);
...
private function ourDadTaughtUsNotToBeAshamedOfOurDicks():void {
	...
	let x:number = player.biggestCockIndex();
	//[first encounter] 
	if(flags[FlagEnum.SHOULDRA_PENIS_DEFEAT_TIMES] == 0) outputText("She abruptly gasps in excitement, ", false);
...
private function loseToShouldraWithWorms():void {
	spriteSelect(67);
	//[first encounter]
	if(flags[FlagEnum.SHOULDRA_WORM_SCENE_COUNTER] == 0) outputText("She abruptly gasps in excitement, ", false);
	else outputText("She 'ooo's softly as she recalls your anatomy, ", false);
...
private function ghostGinaLosses():void {
	...
	if(flags[FlagEnum.SHOULDRA_VAGINAL_POSSESSIONS] == 0) outputText("She 'hmm's several times, raising your hand and tapping your finger against your chin. She snaps your fingers as a revelation washes over her.  \"<i>Of course!</i>\" she cries excitedly.  \"<i>I actually have never done this before!</i>\"", false);
...
private function loseToShouldraAsHerm():void {
	spriteSelect(67);
	if(flags[FlagEnum.SHOULDRA_HERMSEX_COUNT] == 0) outputText("She abruptly gasps in excitement, ", false);
...
private function shouldraGiantCockLoss():void {
	...
	//[if first time]
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("bemused ", false);
	...
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("searches her body for any signs of change, and when she finds none, begins pouting.", false);
	...
	//[if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("and instead of offering an explanation, she merely ", false);
	else outputText("and, to the green girl's obvious delight, ", false);
	outputText("reaches over and kneads the goblin's ", false);
	//([if first time]
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("small, firm B-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) outputText("perky, sizeable C-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) outputText("well-formed, jiggly D-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) outputText("big and squishy E-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) outputText("oversized, heaving F-cups", false);
	else outputText("gigantic and openly-bared G-cups", false);
	outputText(". The goblin ", false);
	//[if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("instantly stops her protests, suddenly too stimulated by the massage to worry about any magical favoritism, ", false);
	else outputText("cries out in ecstasy at the still-incredible sensation, ", false);
	//[if first time]
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("although that doesn't stop the second goblin to begin voicing her own dissent", false);
	...
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("and... pulls the breast along with it?", false);
	else outputText("and pulls the breast along with it.", false);
	outputText("  You inwardly gasp ", false);
	//[if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("as the effects of the magic are revealed", false);
	...
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText(", evidently!", false);
	...
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("gasps in amazement, ", false);
	...
	//[if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("petite boobs", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) outputText("modest bosom", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) outputText("sizable chest", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) outputText("large endowments", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) outputText("huge tits", false);
	...
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("E-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) outputText("F-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) outputText("G-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) outputText("H-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) outputText("I-cups", false);
	...
	//[if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("HH-cups", false);
	//[if repeated once] 
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) outputText("II-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) outputText("JJ-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) outputText("KK-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) outputText("LL-cups", false);
	...
	//([if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("DD-cup (and C-cup, on the other side)", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) outputText("EE-cup (and D-cup, on the other side)", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) outputText("FF-cup (and E-cup, on the other side)", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) outputText("GG-cup (and F-cup, on the other side)", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) outputText("HH-cup (and G-cup, on the other side)", false);
	...
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] <= 4) outputText("they have kept at least a cup size of flesh from their experience", false);
...
private function ghostGartuanLossSceneOrSomeShit():void {
	...
	outputText("When the proverbial dust settles, you stir groggily. Blinking the sleep out of your eyes, you do a double take at the sheer amount of semen flooding the street. You feel a sensation building in your finally flaccid member, and you glance down. A pale, transparent smoke flows gently from your urethra, reforming into a very tired, moderately sore, but incredibly satisfied ghost girl.  ", false);
	if(flags[FlagEnum.SHOULDRA_EXGARTUAN_SPIRIT_SEX_COUNT] == 0) outputText("\"<i>Spirit sex... I can't say I've ever tried that,</i>\" she admits. \"<i>Damn if it isn't good.</i>\"", false);
...
private function slimeGhostGalForHerms(clearOut:boolean = false):void {
	...
	//{event occurrence == 0}
	if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 0) outputText("I've got this thing all figured out.");
	//event occurrence =1
	else if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 1) outputText("I've got this thing all figured out.  For real this time.");
	//event occurrence >=2
	else if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 2) outputText("I've got this thing all figured out.  For real this time.  Promise.");
	//event occurrence >=3
	...
	//event occurrence == 0
	if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] <= 2)outputText("Maybe she really does have a firm grip on the reigns of this beast.");
	//event occurrence >=3
	...
	//if cockLength(0) > 24
	if(player.lowerBody.cockSpot.list[0].cockLength > 24) outputText("However... it isn't exactly up to your immense scale.  ");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) outputText("both you and the spooky slime press your tits together, your swaying thrusts only enhancing the mammary meltdown.");
	...
	if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] > 0) outputText("  Again.");
	...
	if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 0) outputText("You couldn't spank it even to save your life, as we've learned today.</i>\"");
	...
	if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 1) outputText("  \"<i>I was afraid you'd have... given up the ghost.</i>\"");
	else if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] > 1) outputText("  \"<i>But I'm more than willing to do it again next time with a conclusion like that!</i>\"");
