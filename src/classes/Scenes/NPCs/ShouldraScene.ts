...
//const GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER: number = 510;
		...
		public timeChange():boolean
		{
			...
			if (flags[FlagEnum.SHOULDRA_MAGIC_COOLDOWN] >= 1) flags[FlagEnum.SHOULDRA_MAGIC_COOLDOWN]--;
			if (shouldraFollower.followerShouldra()) {
				if (player.statusAffects.get("Exgartuan").value1 == 1 && player.lowerBody.cockSpot.hasCock() && rand(10) == 0) {
					if (flags[FlagEnum.SHOULDRA_EXGARTUDRAMA] == 1) {
						shouldraFollower.exgartumonAndShouldraFightPartII();
						needNext = true;
					}
					else if (flags[FlagEnum.SHOULDRA_EXGARTUDRAMA] == 2) {
						shouldraFollower.exgartumonAndShouldraFightPartIII();
						needNext = true;
					}
				}
				flags[FlagEnum.SHOULDRA_SLEEP_TIMER]--;
				if (shouldraFollower.shouldersWarnings()) needNext = true;
				if (flags[FlagEnum.SHOULDRA_SLEEP_TIMER] == 0 || (flags[FlagEnum.SHOULDRA_SLEEP_TIMER] < 0 && flags[FlagEnum.SHOULDRA_SLEEP_TIMER] % 16 == 0)) {
					shouldraFollower.shouldraWakesUpOrPokesPCsForShitsAndGigglesIdunnoHowLongCanIMakeThisFunctionNameQuestionMark();
					needNext = true;
				}
				if (flags[FlagEnum.SHOULDRA_PLOT_COUNTDOWN] > 0 && getGame().model.time.hours == 3) flags[FlagEnum.SHOULDRA_PLOT_COUNTDOWN]--;
			}
			return needNext;
		}
	
		public timeChangeLarge():boolean {
			if (shouldraFollower.followerShouldra() && flags[FlagEnum.SHOULDRA_PLOT_COUNTDOWN] == 0 && getGame().model.time.hours == 3) {
				flags[FlagEnum.SHOULDRA_PLOT_COUNTDOWN] = -1;
				shouldraFollower.shouldraDream1();
				return true;
			}
			//Ghostgirl recruitment priority
			if (flags[FlagEnum.SHOULDRA_FOLLOWER_STATE] == .5 && model.time.hours == 6) {
				getGame().shouldraFollower.morningShouldraAlert();
				return true;
			}
			//Ghostgirl pissed off dreams
			if (shouldraFollower.followerShouldra() && flags[FlagEnum.SHOULDRA_SLEEP_TIMER] <= -236 && model.time.hours == 3 && player.gender > 0) {
				getGame().shouldraFollower.nightTimeShouldraRapesThePC();
				return true;
			}
			//Ghostgirl madness
			if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00365] > 0) {
				if (player.lowerBody.cockSpot.count() > 1 || player.faceType != FACE.HUMAN || player.lowerBody != LOWER_BODY.HUMAN || player.tailType > TAIL.NONE || player.horns > 0 || player.stats.cor > 15 || player.longestCockLength() > 10 || player.tallness < 65 || player.tallness > 78 || player.lowerBody.vaginaSpot.hasVagina())
					flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00365] = 0;
				else {
					flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00365]--;
					if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00365] == 0) {
						paladinModeFollowup();
						return true;
					}
...
internal function shouldraGreeting():void {
	spriteSelect(66);
	Render.text("", true);
	if(silly() && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00366] == 0) {
		//NO MUTANTS ALLOWED
		if(player.lowerBody.cockSpot.count() > 1 || player.faceType != FACE.HUMAN || player.lowerBody != LOWER_BODY.HUMAN || player.tailType > TAIL.NONE || player.horns > 0 || player.stats.cor > 15 || player.longestCockLength() > 10 || player.tallness < 65 || player.tallness > 78 || player.lowerBody.vaginaSpot.hasVagina()) {}
		else {
	...
	//First 3 encounters
	if(flags[FlagEnum.TIMES_MET_SHOULDRA] < 3) {
		Render.text("You step into the ruins of the village cautiously, zigzagging through passageways and searching for anything worth pocketing. A sound from the inside of a half-collapsed cottage stops you in your tracks. You furrow your brow, upset at allowing yourself to drop your guard in a place like this. Unluckily, the presence seems aware of you, and you hear it shuffling towards the disfigured doorway. You drop into a crouch, readying your " + player.weaponName + " and preparing for battle. You are caught off-guard when a completely normal female leg steps out of the frame. The young woman takes a few steps closer, apparently ignoring your intimidating stance for a moment as she sizes you up. ", false);
		//([if first time] 
		if(flags[FlagEnum.TIMES_MET_SHOULDRA] == 0) Render.text("\"<i>You're the asshole that took my racks, aren't you?  I was hoard-...er, I was keeping an eye on those!</i>\" she accuses, eyes narrowing angrily.  ", false);
		Render.text("Finally, she acknowledges your readiness to fight. With a sigh and a shrug, she raises her arms and drops into an orthodox boxing stance. Once again, you heft your " + player.weaponName + ".", false);
	...
	//(after three encounters with her)
		...
		if(flags[FlagEnum.TIMES_BEATEN_SHOULDRA] >= 3) {
			Render.text("\"<i>Or... maybe not?  Truth be told, I think you've proven your ability to pummel me enough.  Wanna just skip to the good part?</i>\"\n\n", false);
			//([if first time] 
			if(flags[FlagEnum.TIMES_BEATEN_SHOULDRA] == 3) Render.text("You can't say you saw that coming.  It seems she has offered to forego the fight in lieu of sexings.  ", false);
			Render.text("Would you like to accept her offer, or do you want to fight her regardless?", false);
...
private yankeeEchoPapa():void {
	spriteSelect(66);
	Render.text("", true);
	Render.text("You graciously accept her offer, and she nods happily.  \"<i>Perfect!  Now then, let's get to it...</i>\"\n\n", false);
...
private novemberAlphaHotel():void {
	spriteSelect(66);
	Render.text("", true);
	Render.text("You shake your head slowly, raising your " + player.weaponName + " as you regard her.  \"<i>Ah, so that's how you want it?</i>\" she titters, a smile gracing her features.  \"<i>In that case, enough talk... have at you!</i>\"\n\n", false);
	startCombat(new Shouldra());
}
...
internal function defeatDannyPhantom():void {
	...
	//defeat by damage)
	if(monster.HP < 1) {
		Render.text("With an ear-splitting, ghostly wail that forces you to your knees, the ", false);
		if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] >= 1) Render.text("ghost ", false);
		Render.text("girl falls back, overcome by her injuries. Before you can stop her, she simply becomes insubstantial, her clothes undergoing the change as well. A breeze slips through the ruins, picking the ghost up and scattering her to the winds.", false);
		cleanupAfterCombat();
	}
	//(defeat by lust)
	else {
		Render.text("The girl glances down at her leggings, then does a double take, realizing as if for the first time she was completely overwhelmed by her lust. She sinks to her knees, dragging her breeches down and pressing a hand against her pubic mound.  ", false);
		if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] < 1) Render.text("Strangely, the action seems to have little to no effect.", false);
		else Render.text("Of course, the motion has no effect on her libido, due to her ghostly state.", false);
		Render.text("  Looking up at you with what can only be described as desperation, she pants, \"<i>Please, hero... I need release... let me in?</i>\"  You take a step forward, stroking your chin softly in contemplation.  ", false);
		//([if first time]
		if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] < 1) {
			Render.text("What does she mean, \"<i>let me in?</i>\" Do you want to find out?", false);
	...
	flags[FlagEnum.TIMES_BEATEN_SHOULDRA]++;
}
//(deny entry)
private noSlimingSlimer():void {
	spriteSelect(67);
	Render.text("", true);
	Render.text("You shake your head angrily at the brazen girl, ", false);
	//([if first time] 
	if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] == 0) Render.text("not feeling up to heed the strange girl's whims.", false);
	else Render.text("mystified at the thought of letting a ghost possess your body.", false);
	Render.text("You whirl about and stomp away.  She rises, screaming, \"<i>Fine! I'll go and take over a fucking goblin, again!</i>\" and storms the other direction. Before you move out of earshot, you hear a sniffle from the departing ", false);
	//([if first time]
	if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] == 0) Render.text("girl (what was that about her 'taking over' a goblin?)", false);
	else Render.text("spirit", false);
	Render.text(", and a twinge of guilt runs through you.", false);
	cleanupAfterCombat();
}
...
private littlePigLittlePigLetMeIn():void {
	spriteSelect(67);
	Render.text("", true);
	Render.text("You decide ", false);
	if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] == 0) Render.text("you can afford being a little adventurous with the obviously desperate girl", false);
	else Render.text("having a spiritual tag-along might be a bit of fun", false);
	Render.text(", and agree to her request to possess you. She staggers to her feet and undresses, panting and shivering with unfulfillable sexual need. You reach out to her, drawing her", false);
	//[if <~7' tall]
	if(player.tallness < 84) Render.text(" into your arms for a tight hug", false);
	else Render.text(" up into your arms for a tight hug", false);
	Render.text(". The young woman heaves a happy sigh, then begins mumbling an incantation into your shoulder. Soon enough, the girl's body turns translucent and she begins to sink into you, planting a little peck on your cheek on her way in.  ", false);
	//([if first time] 
	if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] == 0) Render.text("Wait... WHAT!?  Did you just let some sort of... g-... g-... g-... G-... G-... GHOOOOOST possess you?", false);
	Render.text("  \"<i>D-Damn,</i>\" she sighs, her voice wavering from her lust build-up, \"<i>Gotta get used to all of this " + player.skinFurScales() + "...</i>\"  You adjust the best you can to the realization that you are sharing your body with a spirit, then turn your attentions to her. You telepathically ask her if she has anything in particular in mind for relieving her of her arousal. She pulses back a 'hmm' at you, indicating she will allow you to decide her fate.\n\n", false);
	
	//CHOICES HURRAH
	let lake:Function = null;
	if(player.gender > 0 && ((player.statusAffects.has("BoatDiscovery") && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00230] > 0) || flags[FlagEnum.TIMES_MET_OOZE] > 0)) {
		Render.text("You could take her to the lake to find someone to play with...\n\n", false);
		lake = nowOnVickiLake;
	}
	...
	flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA]++;
...
private penisLossThatIsntALoss():void {
	...
	if(flags[FlagEnum.SHOULDRA_PENIS_DEFEAT_TIMES] == 0) Render.text("She abruptly gasps in excitement and begins moving your hands towards your groin, though you swiftly reclaim control. \"<i>You have a penis,</i>\" she telepathically pulses to you with an anxious and excited edge to her voice. \"<i>My hosts... they rarely... they...</i>\"", false);
	else Render.text("She chuckles softly as she recalls the times you've shared, and she hungrily goes for your maleness once more, though you throw her off of it.", false);
	Render.text("  The communication suddenly breaks as the ghost girl's will surges against yours, all thoughts of submissiveness vanishing from her mind.  ", false);
	//[if intelligence <50] 
	if(player.stats.int < 50) Render.text("It's not very hard for her to break down your fortifications, and you soon feel yourself being pushed aside by her vast mental capacity.", false);
	else if(player.stats.int < 90 || player.cockThatFits(80) == -1) Render.text("You begin to gain an edge, but the ghost girl is craftier than you expected. She begins to pulse mental intrusions at you, scrambling your thoughts just long enough for her to overtake you and push you to the side.  \"<i>You are one hundred years too early to challenge me,</i>\" she chides. \"<i>My intellect has long surpassed most any living creature on this plane.</i>\"", false);
	else {
		x = player.cockThatFits(80);
		Render.text("The ghost girl puts up a good fight, but you're more than prepared for her mental assault. You feel her panic well as her intrusions are overwhelmed, and though she attempts to disengage the mental link, you easily stop her. She screams mental protests at you, but your ringing ears only strengthen your psychological dominance over the poor spirit. Before long, she's reduced to a whimpering wreck. \"<i>Out,</i>\" you command, tired of the troublesome specter's meddlings. Of course, she won't even try to rebel now, resignedly accumulating her essence and beginning her exit. Within moments the ghost girl flows out of your body and resolidifies before you.\n\n", false);
	
		Render.text("She moves to leave in a huff, but you stop her with a single telepathic demand. She's yours, you remind her, and since she's gone through the trouble of trying to get you off, she might as well finish the job. Even as her lips purse in a stubborn pout, a dominating wave of influence sweeps over her, and regardless of herself, she begins unsteadily moving towards you. Resolving to meet halfway, you stride over to her and push her to your knees. As she watches, you shed your " + player.armorName + " and hoist your " + cockDescript(x) + " up to her lips, leaving a fairly clear implication of your expectations. The ghost girl's eyes widen at the sight of your manhood, and she cringes as a new wave of arousal sweeps through her. She gives a little whimper at her inability to quell her overwhelming libido, but you simply laugh mockingly and gesture at your maleness.  \"<i>Ghost blowjob,</i>\" you mutter snidely.\n\n", false);
	...
	player.orgasm();
	flags[FlagEnum.SHOULDRA_PENIS_DEFEAT_TIMES]++;
	if (getGame().inCombat) {
		flags[FlagEnum.BONUS_ITEM_AFTER_COMBAT_ID] = consumables.ECTOPLS.id;
		cleanupAfterCombat();
	}
...
private ghostBJFinisher(inside:boolean):void {
	...
	Render.text("Like a backed up geyser, your gigantic testicles push hot, thick strands of seed out of your belaboured " + cockDescript(x) + " straight up in the air.  The cum splatters on both you and the ", false);
	if(inside) Render.text("excitedly convulsing", false);
	else Render.text("practically lust-paralyzed", false);
	Render.text(" girl, both of you too tired to bother moving away from the raining semen.  Panting heavily, you eventually haul yourself up, dragging the dainty ghost girl up with you.\n\n", false);
	player.orgasm();
	flags[FlagEnum.SHOULDRA_PENIS_DEFEAT_TIMES]++;
	//if ejaculation allowed
	if(inside) {
		Render.text("\"<i>Thank you so much,</i>\" she breathes softly, wrapping her arms around you in a soft embrace.  \"<i>I wouldn't even be able to think straight if that much lust was built up without release.</i>\"  You smile and flick her nose, and she retorts with a sly half-grin.  \"<i>Oh, and you can have this,</i>\" she adds, swiftly tugging her pants down and reaching into her still-drooling cunt, dragging out a handful of ectoplasm.  She tugs out a small bottle from her shirt pocket and sticks the gooey stuff inside before offering it to you with a wink.  \"<i>Thanks again!</i>\"\n\n", false);
		Render.text("You watch her move off into the ruins with a new spring in her step, chuckling as she goes before heading back to camp yourself.", false);
	}
	//if that bitch didn't get nothing
	else {
		Render.text("She wobbles almost exaggeratedly before falling sideways, smacking her head against the ground and sending up a small splash of cum.  If she felt any pain, however, she doesn't react to it at all, staring vacantly at a dilapidated building and drooling slightly.  That outta teach her to try to pull a fast one on you twice, you figure.  As you stride past her, you tug her pants down to her knees and scoop out a good amount of ectoplasm from her vagina.  You reach into her pocket and grab a bottle (honking her boob for good measure and eliciting an aroused twitch), dumping the goo in and stuffing it in your pack.  She'll probably find a goblin eventually, you figure as you begin your journey back to the camp.", false);
	}
	if (getGame().inCombat) {
		flags[FlagEnum.BONUS_ITEM_AFTER_COMBAT_ID] = consumables.ECTOPLS.id;
		cleanupAfterCombat();
...
private hermaphroditeGhostsCumEctoplasm():void {
	...
	let x:number = player.biggestCockIndex();
	Render.text("", true);
	if(flags[FlagEnum.SHOULDRA_HERMSEX_COUNT] == 0) Render.text("She abruptly gasps in excitement and begins moving her hands towards your groin, though you swiftly reclaim control of the unruly appendages. \"<i>You have a penis,</i>\" she telepathically pulses to you with an anxious and excited edge to her voice. \"<i>My hosts... they rarely... they... </i>\"  ", false);
	//[if repeated] 
	if(flags[FlagEnum.SHOULDRA_HERMSEX_COUNT] > 0) Render.text("She chuckles softly as she recalls the times you've shared, and she hungrily goes for your maleness once more, though you mentally throw her off of it.  ", false);
	Render.text("The communication suddenly breaks as the ghost-girl's will surges against yours, all thoughts of submissiveness vanishing from her mind.  ", false);
	if(player.stats.int < 50) Render.text("It's not very hard for her to break down your fortifications, and you soon feel yourself being pushed aside by her vast mental capacity.", false);
	else if(player.stats.int < 90 || player.cockThatFits(80) == -1) Render.text("You begin to gain an edge, but the ghost girl is craftier than you expected, and she begins to pulse mental intrusions at you, scrambling your thoughts for just long enough for her to overtake you and push you to the side.  \"<i>You are one hundred years too early to challenge me,</i>\" she chides. \"<i>My intellect has long surpassed most any living creature on this plane.</i>\"", false);
	else {
		x = player.cockThatFits(80);
		Render.text("The ghost girl puts up a good fight, but you're more than prepared for her mental assault. You feel her panic well as her intrusions are overwhelmed, and though she attempts to disengage the mental link, you easily stop her. She screams mental protests at you, but your ringing ears only strengthen your psychological dominance over the poor spirit. Before long, she's reduced to a whimpering wreck. \"<i>Out,</i>\" you command, tired of the troublesome specter's meddlings. Of course, she won't even try to rebel now, resignedly accumulating her essence and beginning her exit. Within moments the ghost girl flows out of your body and resolidifies before you.\n\n", false);
		...
		if(player.lowerBody.balls == 0) Render.text("The first effects of the spell make themselves known in a big way when an overladen cumsack forms and drops between your legs.  ", false);
		Render.text("The strange feeling focuses on your testes, making it a bit hard to concentrate on keeping the ghost girl subdued.  Your management of willpower does not waver until you feel your legs being pushed farther apart.  A curious glance turns into a horrified stare as you gaze upon your now ");
		if(player.lowerBody.ballSize > 10) Render.text("gargantuan");
		else Render.text("basketball");
		Render.text("-sized balls.\n\n", false); 
...
private ghostGinaLosses():void {
	spriteSelect(67);
	Render.text("The ghost girl heaves a mildly disappointed sigh as she settles into your body fully.  \"<i>I've had... many female hosts,</i>\" she explains. You are almost embarrassed regarding her disapproval, and actually contemplate apologizing before remembering the source of the lament. You cross your arms (or try, as the ghost still has full control of your body) and give an annoyed huff.  ", false);
	if(flags[FlagEnum.SHOULDRA_VAGINAL_POSSESSIONS] == 0) Render.text("She 'hmm's several times, raising your hand and tapping your finger against your chin. She snaps your fingers as a revelation washes over her.  \"<i>Of course!</i>\" she cries excitedly.  \"<i>I actually have never done this before!</i>\"", false);
	else Render.text("\"<i>Oh well, I suppose I can always just do that again,</i>\" she muses. \"<i>It worked well enough the first time.</i>\"", false);
	Render.text("\n\n", false);
	
	Render.text("You are about to question exactly what she is referring to, but quickly fall silent as she slides out of your " + player.armorName + " and idly strokes your cunt. She takes control of your mouth and chants a fairly lengthy spell, her absent-minded masturbation steadily gaining fervor as she progresses. With a final, shouted word of power, both of you cry out as your clitoris suddenly pulses and swells into your hand, the increased sensitivity nearly driving you insane. Your bitch-button does not stop there, however, puffing up with each heartbeat. Your eyes roll back in your head and small primitive grunts and moans escape your lips as your clit engorges in the ghostly magic, not stopping until it reaches at least four times its original length. A breeze blows through the ruined rooftops, and you scream as the wind brushes against your elongated bitch-button. Too sensitive... the thing is way too sensitive!\n\n", false);
	
	Render.text("Against your will, and against all of your better instincts, the ghost-girl, as aroused as you are uncomfortable, moves both hands to your sex, gripping it and giving it a squeeze. Your entire body locks up, causing you to fall backwards onto the pavement as a devastating orgasm rocks through your system. A strangled cry is the only reaction you can give to the explosive sensations. The ghost-girl, however, not satisfied with the suddenness of the climax, begins feverishly stroking your clitoris with one hand, the other reaching into your love-tunnel and stimulating the rosy flesh inside. You arch your back and attempt to dig small holes into the road with your " + player.feet() + " as shock-waves run through your system over and over, never giving you a chance to recuperate. You hear, as if from a great distance, a mix of frenzied laughter and orgasmic cries of pleasure ringing inside your head. Finally, mercifully, you fade into unconsciousness, leaving the ghost girl to her constant stimulation.\n\n", false);
	Render.text("You awake some time later in a prodigious puddle of girly spunk and attempt to slowly sit up, only to be impeded by a clunk against the ground.  A clunk?  You prop yourself up on your elbows and glance down to see a large metal pipe, stuck in your vagina presumably some time during your black-out. Reaching down, you extricate a full ten inches of piping from your cunt, and shake your head at the tenacity of your unworldly bodily invader. You are happy to feel a tightening in your crotch, however, as the magic finally wears off, returning your womanhood back to a " + vaginaDescript(0) + ". You also note the absence of your gigantic clitoris, and give yourself a little stroke to confirm; it's reverted to normal size as well. All's well that ends well, you suppose.\n\n", false);
...
private loseToShouldraAsHerm():void {
	spriteSelect(67);
	if(flags[FlagEnum.SHOULDRA_HERMSEX_COUNT] == 0) Render.text("She abruptly gasps in excitement, ", false);
	else Render.text("She coos softly as she recalls your anatomy, ", false);
...
private shouldraGiantCockLoss():void {
	spriteSelect(67);
	let x:number = player.biggestCockIndex();
	Render.text("Her attention inevitably moves to your gigantic dick, somehow still contained in your garments, and she swiftly clamors to free it from its prison. Practically tearing off your " + player.armorName + ", she tilts your head back in order to fully take in your " + cockDescript(x) + ", a small line of drool escaping the edge of your mouth. She opens your mouth as if to speak, but cannot find words, instead settling for a long and anxious groan. Her first reaction is to simply hug the titantic shaft, squeezing happily. Eventually, she seems to realize her original intent and draws back, stroking your chin with your hand while she apparently ascertains the best way to stimulate your monster. With a small 'aha!', she struggles to stand, unused to such a strange center of balance. Bringing you along for the ride, she begins wandering the streets of the city, taking control of your mouth and shouting \"<i>My oh my, how on earth will I ever blow my load with this GIANT dick?</i>\" Her efforts are rewarded when you hear a small pitter-patter of feet, distantly at first, but hastily heading toward you. A duo of goblins rush out around a corner, take a moment to confirm the reported size of your manhood, and hurry over, undressing, tripping and shoving each other in an effort to win the race to win your attention. As they reach you the excited little creatures nearly bowl you over as they start stroking and licking every inch of cockflesh they can reach. \"<i>Girls!</i>\" the ghost exclaims sternly (with your voice), crossing your arms in front of you as they turn their heads to regard you. She allows a hint of a smile to show at the corners of your mouth as she continues, \"<i>Trust me when I say there will be enough of me for both of you.</i>\"\n\n", false);
	
	Render.text("Without another word, she reaches out and grabs one of the goblins by the shoulder and begins chanting in her strange unknown language, wiggling the fingers of your absent hand at the ", false);
	//[if first time]
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) Render.text("bemused ", false);
	else Render.text("excited ", false);
	Render.text("girl. With a flair, she finishes the incantation, then moves to the other goblin and repeats the process. The first goblin, meanwhile, ", false);
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) Render.text("searches her body for any signs of change, and when she finds none, begins pouting.", false);
	else Render.text("resists the urge to smash her tits together, waiting with as much patience as she can muster.", false);
	Render.text("  By this time, the ghost girl has finished with the spell, ", false);
	//[if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) Render.text("and instead of offering an explanation, she merely ", false);
	else Render.text("and, to the green girl's obvious delight, ", false);
	Render.text("reaches over and kneads the goblin's ", false);
	//([if first time]
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) Render.text("small, firm B-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) Render.text("perky, sizeable C-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) Render.text("well-formed, jiggly D-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) Render.text("big and squishy E-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) Render.text("oversized, heaving F-cups", false);
	else Render.text("gigantic and openly-bared G-cups", false);
	Render.text(". The goblin ", false);
	//[if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) Render.text("instantly stops her protests, suddenly too stimulated by the massage to worry about any magical favoritism, ", false);
	else Render.text("cries out in ecstasy at the still-incredible sensation, ", false);
	//[if first time]
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) Render.text("although that doesn't stop the second goblin to begin voicing her own dissent", false);
	else Render.text("and you can't help but notice the excited blush running across the second goblin's face", false);
	Render.text(". The ghost girl pinches a quickly-stiffening nipple and pulls it out, ", false);
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) Render.text("and... pulls the breast along with it?", false);
	else Render.text("and pulls the breast along with it.", false);
	Render.text("  You inwardly gasp ", false);
	//[if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) Render.text("as the effects of the magic are revealed", false);
	else Render.text("despite seeing the magic before, always enthralled by the process", false);
	Render.text("; it has turned the goblin's breasts into an almost putty-like substance", false);
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) Render.text(", evidently!", false);
	else Render.text(" once again", false);
	Render.text(". The ghost girl begins to speed the process up, softly laying the goblin on the ground and kneading out the green girl's tits, adding at least half a cup size with every tug. The second goblin, watching intently behind your back, ", false);
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) Render.text("gasps in amazement, ", false);
	else Render.text("can't restrain her urges any longer, ", false);
	Render.text("swiftly reaching down to roughly grab at her own ", false);
	//[if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) Render.text("petite boobs", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) Render.text("modest bosom", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) Render.text("sizable chest", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) Render.text("large endowments", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) Render.text("huge tits", false);
	else Render.text("titanic melons", false);
	Render.text(", crying in excitement as her breasts also begin to billow out under her own ministrations.\n\n", false);
	
	Render.text("The first goblin's breasts, at this time, have reached at least ", false);
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) Render.text("E-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) Render.text("F-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) Render.text("G-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) Render.text("H-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) Render.text("I-cups", false);
	else Render.text("J-cups", false);
	Render.text(", and the ghost girl speeds up the massage ever further. Not satisfied with the progress, she raises the goblin's head to your lips and blows a steady gust of wind down the little woman's throat, ballooning her glistening mammaries out much more efficiently. This continues until the goblin nearly collapses under the weight of her new ", false);
	//[if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) Render.text("HH-cups", false);
	//[if repeated once] 
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) Render.text("II-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) Render.text("JJ-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) Render.text("KK-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) Render.text("LL-cups", false);
	else Render.text("MM-cups", false);
	Render.text(", and the ghost girl turns your attention to the second goblin, whose boobs are embarrassingly uneven due to her...unpracticed skills. The ghost girl moves you over to her, mercifully evening the currently ", false);
	//([if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) Render.text("DD-cup (and C-cup, on the other side)", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) Render.text("EE-cup (and D-cup, on the other side)", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) Render.text("FF-cup (and E-cup, on the other side)", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) Render.text("GG-cup (and F-cup, on the other side)", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) Render.text("HH-cup (and G-cup, on the other side)", false);
	else Render.text("II-cup (and H-cup, on the other side)", false);
	Render.text("breasts out, before again blowing into the goblin's mouth, inflating the goblin until she matches the first. With an excited giggle, the ghost girl swings you around to stand between the two amazed goblin girls, flinging a large glob of pre-cum into the first goblin's face, which she promptly smears over her breasts, enjoying the extra lubrication. She slowly drops to your back, presenting your cock vertically to the two increasingly eager girls, and gives a simple order: \"<i>Titfuck.</i>\"\n\n", false);
	
	Render.text("Not needing to be asked twice, the two girls fall into place on each side of your prodigious shaft, heaving their engorged bosoms up and wrapping them around your penis. Pillowy boobflesh smooshes delightfully onto your crotch, around your " + cockDescript(x) + ", and meeting in the middle, the goblins' nipples slipping against each other. They look to you for final confirmation, and the ghost girl nods your head; at this point, you find yourself voluntarily nodding along with her, completely entranced by this situation. With aroused grunts and moans, the goblins begin sliding their soft, slippery flesh up and down your shaft, and, on a sudden influence, they step onto your hips, heaving themselves up and using the increased height to stimulate even more of your " + cockDescript(x) + ". You and the ghost girl both reel back by the sensations of the goblin double-paizuri, and it's not long before you feel yourself reaching the end of your sexual rope, and the ghost girl informs the goblins of this. Faster than you thought possible, given their recent additions, the second goblin zooms over to your shoulders and hoists you up to a sitting position, your gigantic cock falling right into the hands of the first girl, who promptly wraps her pair of tips around each side of your head and plunges her lips around your now-dilating urethra, sucking eagerly and squeezing dollops of breast milk out of her over-stimulated bosom. To help push you over the edge, the second goblin squishes her own pair of boobs against your back and slides up and down, leaving small milky trails across you.\n\n", false);
	
	Render.text("With an anxious moan, you tilt your head back and tense your body as seed begins to trek across the expansive tract of land between your crotch and your cockhead, and the second goblin promptly scrambles over, leaving you to prop yourself up as you wiggle about and clench your jaw in anticipation. The goblin duo position themselves against your manhood and open wide as the first glob of semen slams against them with enough force to bulge both of their cheeks with the stuff. They quickly heft their bosoms under them, catching any errant seed in their vast canyons of cleavage. This continues, the volume of cum increasing with every pulse until both goblins might as well have dumped four gallons of white paint onto themselves. With a final, intense blast, you fall backwards, chest heaving while the aftermath of the climax hits you. While you lay there, occasional drops of semen splattering onto your " + chestDesc() + ", you manage a glance at the two to find them working together to scoop as much accumulated cum as possible into their needy fuck-holes. You zone out for around five minutes, snapping out of your daze soon after when you realize the lack of ghostly presence. You unsteadily rise to see the goblins still going at it with your semen, bellies bloated with the volume of your impressive orgasm. They stumble to their feet, making sure to move a hand under their cunts to hold in as much seed as they can while they redress—if you can even refer their get-ups as clothing. You notice their oversized breasts rapidly shrinking, deflating until they return to near-normal conditions. You manage a bit of a smile when you realize that ", false);
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] <= 4) Render.text("they have kept at least a cup size of flesh from their experience", false);
	else Render.text("their breasts seem to have reached their crescendo, keeping the same (admittedly incredibly stacked) measurements", false);
	Render.text(", which they happily fondle as they wander away, side-by-side. Moving to your " + player.armorName + " and redressing, you shake your head, wondering, not for the first time, if such a large appendage is truly beneficial to you, and wondering if this was the last you've seen of your two new goblin friends.\n\n", false);
	Render.text("A strange sensation begins in your midsection, and you press a hand against the flesh curiously. You remove it swiftly as a familiar head suddenly pushes out, wiggling about until it faces you. She smiles, then draws her arms out of her torso and uses them as leverage (with a whispered spell to make her hands tangible) to remove herself from you completely. Muttering an incantation and solidifying her again-unremarkable form, she winks at you and gathers her tunic, slipping it over her frame. As you watch, she slips her thumb and forefinger into her satisfied snatch, producing a healthy helping of a spooky gooey substance. Reaching into a pocket inside her tunic, she draws out a small bottle, which she promptly squishes the ectoplasm into. She offers you the bottle, explaining, \"<i>The product of a... ghost climax, if you will. Keep it, it's good for you.</i>\"  With a happy goodbye, she wanders into an alley, grabbing her ruined leggings and discarded shoes as she goes, and disappears into the shadows. You move to leave, but find your " + player.legs() + " growing heavy, fatigue washing over you in the aftermath of the possession. Before you know it, you're on your knees, and you have to struggle to keep your eyelids from drooping. You succumb to the exhaustion, slumping to the ground and soon snoring contentedly.", false);
	
	player.orgasm();
	dynStats("lib", 1, "sen", 1);
	flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT]++;
	if (getGame().inCombat) {
		flags[FlagEnum.BONUS_ITEM_AFTER_COMBAT_ID] = consumables.ECTOPLS.id;
		cleanupAfterCombat();
	}
...
private genderlessShouldrasLossRapes():void {
	spriteSelect(67);
	Render.text("Excited to find out what manner of host you are, the ghost-girl swiftly assumes complete control of your body and cups a hand against your crotch to feel... nothing. Confused, she quickly wriggles out of your " + player.armorName + " and goes in for another grope, feeling nothing but skin. \"<i>Son of a... please don't tell me you don't have any sexual bits I can play with!</i>\" she huffs, and you chuckle sheepishly, not knowing what to tell her. She groans unhappily and falls back onto your " + buttDescript() + ", muttering a string of curse words under your breath. \"<i>That's it!</i>\" she fumes. \"<i>I'm going to get off, whether you want me to or not!</i>\" She resumes cursing and wiggling your hips for a bit longer, then stops suddenly, a smirk playing across your face. \"<i>Whether you want me to or not...</i>\"\n\n", false);
	
	Render.text("She hops up and makes for the decrepit hut you first saw her in.  ", false);
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0) Render.text("You raise an eyebrow, genuinely curious as to how she'll figure out how to relieve her desire.  ", false);
	Render.text("Darting inside, the ghost girl heads unerringly towards a warped and half-rotted dresser. After searching several drawers filled with various knick-knacks and trinkets, the spirit squeaks in excitement as she produces a seemingly regular egg carton. She moves to the other side of her room and opens a curiously well-made chest, setting the carton on a nightstand as she goes.  ", false);
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0) Render.text("Curious as to its contents, you glance inside- it's a chest of dildos, of every shape and size. Before you can ask why the hell a ghost would carry a CHEST of dildos, ", false);
	else Render.text("You recall distinctly the contents of that chest, and are not surprised ", false);
	Render.text("when she reaches in and rummages about, producing two eight-inch vibrators with bumpy demon-shafts. She moves back to the center of the room, launching into a short spell as she goes. She sets the dildos on the same nightstand and opens the carton, pulling out four large white eggs.\n\n", false);
	
	Render.text("You marvel at how resourceful the ghost-girl is, then pause and consider the ramifications of the eggs. Your suspicions are confirmed as ", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) Render.text("two new additions bud on your chest.", false);
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 8) Render.text("your " + allBreastsDescript() + " begin to bounce all around.", false);
	else Render.text("your " + allBreastsDescript() + " swells slightly to make room for a sudden increase in milk production.", false);
	Render.text("  As the ghost continues darting around the room, ", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 8) Render.text("your new EE-cup breasts settle", false);
	else Render.text("the lactation continues", false);
	Render.text(", and the ghost girl moves back to the table and settles down a bit. Satisfied with the changes, she wastes no time on tapping the eggs against the table, cracking them cleanly. Your arm freezes up momentarily as she tips the half-shell towards your similarly conflicted mouth, but she wins out quickly, forces your jaw open, and drops the contents inside. She swallows the tasty contents easily, then slides her hands to your chest and lifts your huge tits in anticipation. Your nipples swell up and out, lengthening and stiffening as the first three eggs are consumed. The last one, however, produces a completely different reaction on your body; a comfortable warm tingle begins, deep in the center of your oversized breasts, migrating up steadily to your nipples. With a satisfied groan, the ghost-girl circles the tingling teats with a teasing finger as the change continues. Finally, she sinks a finger into each of the suddenly yielding nipples, shivering in pleasure from the penetration. Wasting no more time, she snatches up her two dildos and shoves them into your ", false);
	//[if new]
	if(!player.upperBody.chest.hasFuckableNipples()) Render.text("new ", false);
	else Render.text("even larger ", false);
	Render.text("nipplecunts, flicking them on as the bumpy demon-dildos penetrate your virgin tits. The ensuing vibration sends little waves up and down your boobs as they jostle about, and the ghost-girl gives a strangled yelp of pleasure. Drunk with the sensation, she staggers about, mewling and gurgling in the pleasure of having your nubs violated. Her foot gets snagged on a bedpost and she flops onto the ground, chest first, the bumpy sextoys pushing slightly further into your tits. She slams your pelvis onto the floor several times in a futile attempt to further her joy, then flops to the side, eyes finally falling upon the still-open chest of dildos. She slowly drags herself to it (mashing your bosom against the ground and inciting a few happy grunts), reaching in and grabbing up a 13-inch floppy horsecock replica. Your irises shift halfway between the yellow pallor of your possessor and your natural palette as your eyes widen in horror. She couldn't possibly...\n\n", false);
	
	Render.text("She smacks it ", false);
	if(player.isNaga()) Render.text("against your naga-crotch", false);
	else if(player.isGoo()) Render.text("into your cushiony goo", false);
	else Render.text("between your legs", false);
	Render.text(", seemingly forgetting about your lack of genitalia, and grudgingly goes for the next penetrable orifice; your " + assholeDescript() + ". With blatant disregard for safety, the vibrators still buzzing happily in your increasingly warm nipples, she slams the horsecock against the ground, waits for the wiggling to somewhat cease, then lines your butt up with the flared tip. Her crazed laughter mingles with your horrified scream as she sits on it, driving it at least four inches inside in its initial penetration", false);
	//[if anus is not gaping]
	if(player.ass.analLooseness <= 4) Render.text(" and stretching your butt obscenely", false);
	Render.text(".", false);
	...
	Render.text(".  With a happy sigh, you heave yourself upright, slipping back into your " + player.armorName + " and moving out into the ruins once more.\n\n", false);
	Render.text("A strange sensation begins in your midsection, and you press a hand against the flesh curiously. You remove it swiftly as a familiar head suddenly pushes out, wiggling about until it faces you. She smiles, then draws her arms out of her torso and uses them as leverage (with a whispered spell to make her hands tangible) to remove herself from you completely. Muttering an incantation and solidifying her again-unremarkable form, she winks at you and gathers her tunic, slipping it over her frame. As you watch, she slips her thumb and forefinger into her satisfied snatch, producing a healthy helping of a spooky gooey substance. Reaching into a pocket inside her tunic, she draws out a small bottle, which she promptly squishes the ectoplasm into. She offers you the bottle, explaining, \"<i>The product of a... ghost climax, if you will. Keep it, it's good for you.</i>\"  With a happy goodbye, she wanders into an alley, grabbing her ruined leggings and discarded shoes as she goes, and disappears into the shadows. You move to leave, but find your " + player.legs() + " growing heavy, fatigue washing over you in the aftermath of the possession. Before you know it, you're on your knees, and you have to struggle to keep your eyelids from drooping. You succumb to the exhaustion, slumping to the ground and soon snoring contentedly.", false);
	flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT]++;
	player.orgasm();
	dynStats("sen", 1);
...
private ghostGartuanLossSceneOrSomeShit():void {
	...
	Render.text("\"<i>Hold up,</i>\" the demon requests, suddenly far less boisterous. \"<i>You're sayin' you're a chick, right?  And not some chick with a dick", false);
	if(player.gender == 3) Render.text("like this bitch here", false);
	Render.text("?</i>\"\n\n", false);
	...
	Render.text("When the proverbial dust settles, you stir groggily. Blinking the sleep out of your eyes, you do a double take at the sheer amount of semen flooding the street. You feel a sensation building in your finally flaccid member, and you glance down. A pale, transparent smoke flows gently from your urethra, reforming into a very tired, moderately sore, but incredibly satisfied ghost girl.  ", false);
	if(flags[FlagEnum.SHOULDRA_EXGARTUAN_SPIRIT_SEX_COUNT] == 0) Render.text("\"<i>Spirit sex... I can't say I've ever tried that,</i>\" she admits. \"<i>Damn if it isn't good.</i>\"", false);
	else Render.text("\"<i>Damn,</i>\" she sighs, \"<i>that is still a great ride, I'd say...</i>\"", false);
	Render.text("\n\n", false);
...
private slimeGhostGalForHerms(clearOut:boolean = false):void {
	...
	Render.text("Your naked trot towards the emerald ooze stops as worry washes over your sexual cravings.  The contemplative expression forming on your face catches the attention of your ghostly friend.  \"<i>I know exactly what you're mulling over, but don't worry,</i>\" she assures you with a condescending chuckle.  The haunted blob slowly approaches you, seamlessly transitioning from its shapeless form to a perfect representation of the apparition mid-stride.  \"<i>");
	//{event occurrence == 0}
	if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 0) Render.text("I've got this thing all figured out.");
	//event occurrence =1
	else if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 1) Render.text("I've got this thing all figured out.  For real this time.");
	//event occurrence >=2
	else if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 2) Render.text("I've got this thing all figured out.  For real this time.  Promise.");
	//event occurrence >=3
	else Render.text("Well... don't worry TOO much.");
	Render.text("</i>\"");
	...
	//event occurrence == 0
	if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] <= 2)Render.text("Maybe she really does have a firm grip on the reigns of this beast.");
	//event occurrence >=3
	else Render.text("Regardless, you've been through this dog and pony show enough times to know that something is bound to go wrong anyway.");
	...
	//vaginalLooseness == 0
	if(player.looseness() == 0) Render.text("tight");
	//vaginalLooseness == 1
	else if(player.looseness() == 1) Render.text("forgiving");
	else if(player.looseness() == 2) Render.text("loose");
	else if(player.looseness() == 3) Render.text("gaping");
	else Render.text("cavernous");
	Render.text(", velvety scabbard.  Hands too busy latched onto one another's hips, ");
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) Render.text("both you and the spooky slime press your tits together, your swaying thrusts only enhancing the mammary meltdown.");
	else Render.text("the spooky slime presses her tits hard into your flat chest, your gyrations teasing them to no end.");
	Render.text("  The fierce battle increases in tempo; the sly leering the two of you used to try and psyche the other out is giving way to slight head-lashing and gasps through clenched teeth.");
	...
	if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] > 0) Render.text("  Again.");
	...
	Render.text("\"<i>Boo!</i>\"  your face shouts at you, triggering a panicked backwards jolt from you which then results in you slipping back down on your [butt].  The spook easily phases out of you during your tumble, standing perfectly still from your previous position as she glares down at you with the same mischievous smile from earlier.");
	Render.text("\"<i>Yea, I jumped ship from that senseless slime the moment it finished filling you up to christen the S.S.  Ballsac,</i>\" she explains, her tone free of any responsibility.  \"<i>You're lucky I was around to pick up the slack for you.  ");
	if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 0) Render.text("You couldn't spank it even to save your life, as we've learned today.</i>\"");
	else Render.text("Evidently, you haven't put in any meaningful practice since last time,</i>\" the specter chides you while making an exaggerated jerking-off motion.");
	if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 1) Render.text("  \"<i>I was afraid you'd have... given up the ghost.</i>\"");
	else if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] > 1) Render.text("  \"<i>But I'm more than willing to do it again next time with a conclusion like that!</i>\"");
	...
	flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER]++;
}
}
}
