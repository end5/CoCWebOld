...
internal function shouldraGreeting():void {
	...
	if(flags[FlagEnum.TIMES_MET_SHOULDRA] < 3) {
		...
		//([if first time] 
		if(flags[FlagEnum.TIMES_MET_SHOULDRA] == 0) MainScreen.text("\"<i>You're the asshole that took my racks, aren't you?  I was hoard-...er, I was keeping an eye on those!</i>\" she accuses, eyes narrowing angrily.  ", false);
	...
	//(after three encounters with her)
		...
		if(flags[FlagEnum.TIMES_BEATEN_SHOULDRA] >= 3) {
			...
			//([if first time] 
			if(flags[FlagEnum.TIMES_BEATEN_SHOULDRA] == 3) MainScreen.text("You can't say you saw that coming.  It seems she has offered to forego the fight in lieu of sexings.  ", false);
...
internal function defeatDannyPhantom():void {
	...
	if(monster.HP < 1) {
		...
		if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] >= 1) MainScreen.text("ghost ", false);
	...
	//(defeat by lust)
		...
		if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] < 1) MainScreen.text("Strangely, the action seems to have little to no effect.", false);
...
private noSlimingSlimer():void {
	...
	//([if first time] 
	if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] == 0) MainScreen.text("not feeling up to heed the strange girl's whims.", false);
	...
	//([if first time]
	if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] == 0) MainScreen.text("girl (what was that about her 'taking over' a goblin?)", false);
...
private littlePigLittlePigLetMeIn():void {
	...
	if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] == 0) MainScreen.text("you can afford being a little adventurous with the obviously desperate girl", false);
	...
	//([if first time] 
	if(flags[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] == 0) MainScreen.text("Wait... WHAT!?  Did you just let some sort of... g-... g-... g-... G-... G-... GHOOOOOST possess you?", false);
...
private sharkbustersVictory():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text(", " + allBreastsDescript() + " squishing and squashing against her own barely-clad bosom", false);
...
private sharkyEctoginas():void {
	...
	//[if repeat]
	if(flags[FlagEnum.TIMED_SHARKGINAS] > 0) MainScreen.text(" again...", false);
	MainScreen.text("</i>\" she pants, brow suddenly furrowed with worry.  ", false);
	if(flags[FlagEnum.TIMED_SHARKGINAS] == 0) MainScreen.text("You begin to ask what she's talking about, but the words die in your mouth when", false);
	...
	MainScreen.text(". After the cloud of arousal disperses from the ghost-girl's normally intelligent mind, she can't help but notice how uncomfortable you must be, squirming about with her gigantic pinky shoved up your " + vaginaDescript(0) + ".\n\n", false);
...
private ghostIzmaPenis():void {
	spriteSelect(67);
	MainScreen.text("", true);
	MainScreen.text("You swiftly disrobe and lay your " + player.armorName + " to the side, revealing your stiffening " + multiCockDescriptLight() + " in all of its glory. The ghost-girl's eyes brighten at the sight, and she even allows herself a little swoon (which looks absolutely silly, given her current tough shark-like vessel) as she regards your manhood.  ", false);
	MainScreen.text("She staggers forward, dropping to her knees in front of you", false);
	//[if testicles present] 
	if(player.lowerBody.balls > 0) MainScreen.text(" and cradling your " + ballsDescriptLight() + " with almost reverent gentleness, running her coarse shark-skinned fingers across your sack as gently as she can manage", false);
	MainScreen.text(".\n\n", false);
	...
	//[if first time]
	if(flags[FlagEnum.TIMES_SHARKPENISED] == 0) MainScreen.text("whose expression reveals she's as clueless as you are.", false);
	...
	if(flags[FlagEnum.TIMES_SHARKPENISED] == 0) MainScreen.text("absently as she contemplates what could have caused the lengthening of your loins", false);
	...
	if(flags[FlagEnum.TIMES_SHARKPENISED] == 0) MainScreen.text("as she deduces the nature of the spell. \"<i>I think... oh my... my host's saliva may have expansive qualities on... sexual aspects of the body,</i>\" she explains. \"<i>So, if I were to do this...</i>\"", false);
...
private ghostGooGurlzDuckfaces():void {
	...
	//([if penis present] 
	if(player.lowerBody.cockSpot.hasCock() && !player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" presenting your " + multiCockDescriptLight() + " to the spectral slime.", false);
	else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" slipping a hand down to your " + clitDescript() + " eagerly.", false);
	//[if herm] 
...
private slimeGinaFlation():void {
	...
	if(player.isNaga()) MainScreen.text("your tail is thrust to one side to make way for the penetration", false);
	else if(player.isGoo()) MainScreen.text("your gooey mass smooshes into the green slime, a near-mirror of jiggling substances", false);
	...
	else MainScreen.text("Your frame is jostled about as the slime under you shrinks, your goo-filled gut pushing your " + allBreastsDescript() + " up into your overburdened jaw.", false);
	MainScreen.text("\n\n", false);
	...
	if(player.isNaga()) MainScreen.text("snake-like lower body", false);
	else if(player.isGoo()) MainScreen.text("undulating, gooey mass", false);
	else if(player.tailType == TAIL.NONE) MainScreen.text("arms and " + player.legs(), false);
	else MainScreen.text("arms, " + player.legs() + ", and tail", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) MainScreen.text("see your " + nippleDescript(0) + " wiggling sporadically.", false);
	else MainScreen.text("see your " + allBreastsDescript() + " trembling, " + nippleDescript(0) + " twitching eagerly.", false);
...
private slimeyPenorsAhoy():void {
	...
	//([if boobs present] 
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) MainScreen.text(" while squishing her small boobs into your own " + allBreastsDescript(), false);
	...
	if(flags[FlagEnum.SHOULDRA_SLIME_PENOR_TIMES] > 0) MainScreen.text("You can't shake the feeling that she's used this particular spell before. What an unoriginal ghost.  ", false);
	...
	if(player.isNaga()) MainScreen.text("slapping against your tail", false);
	else if(player.isGoo()) MainScreen.text("smooshing against your goo", false);
	...
	if(player.lowerBody.balls > 0) MainScreen.text("Your " + player.legs() + " are forced apart by your burgeoning cum-sack", false);
...
private genderlessGhostBuster():void {
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0) MainScreen.text("Curious as to its contents, you glance inside—it's a chest of dildos, of every shape and size. Before you can ask why the hell a ghost would carry a CHEST of dildos, ", false);
	...
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 8) MainScreen.text("your " + allBreastsDescript() + " begin to bounce all around.", false);
	else MainScreen.text("your " + allBreastsDescript() + " swell slightly to make room for a sudden increase in milk production.", false);
	...
	if(player.ass.analLooseness < 4) MainScreen.text(" and stretching your butt obscenely", false);
	...
	MainScreen.text("  With a happy sigh, you heave yourself upright, slipping back into your " + player.armorName + " and moving out into the ruins once more.", false);
...
private penisLossThatIsntALoss():void {
	...
	if(flags[FlagEnum.SHOULDRA_PENIS_DEFEAT_TIMES] == 0) MainScreen.text("She abruptly gasps in excitement and begins moving your hands towards your groin, though you swiftly reclaim control. \"<i>You have a penis,</i>\" she telepathically pulses to you with an anxious and excited edge to her voice. \"<i>My hosts... they rarely... they...</i>\"", false);
	...
	if(player.stats.int < 50) MainScreen.text("It's not very hard for her to break down your fortifications, and you soon feel yourself being pushed aside by her vast mental capacity.", false);
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			...
			else MainScreen.text("the rest of " + multiCockDescriptLight(), false);
		...
		if(player.lowerBody.balls > 0) MainScreen.text("in your " + ballsDescriptLight(), false);
...
private ghostBJFinisher(inside:boolean):void {
	...
	MainScreen.text("Like a backed up geyser, your gigantic testicles push hot, thick strands of seed out of your belaboured " + cockDescript(x) + " straight up in the air.  The cum splatters on both you and the ", false);
...
private dewormYourGhost():void {
	...
	if(flags[FlagEnum.SHOULDRA_WORM_SCENE_COUNTER] == 0) MainScreen.text(", and she responds by taking control of your hand and pointing down at the wriggling creature that crawled out of your junk. \"<i>Oh, that?</i>\" you scoff, waving your other hand in the air dismissively; your other, ghost-controlled appendage still quivers stubbornly as you attempt to explain. \"<i>There are tons of the little guys crawling around inside me. Wanna see?</i>\"", false);
...
private penisGartuanGhostSmexVictory():void {
	...
	MainScreen.text("After a moment's more of hesitation, the ghost girl sinks down to your waist, pushing her head out of your " + hipDescript() + ".  \"<i>I apologize, sir, but our- er, sorry, your host did not mention he was already... occupied,</i>\" she admits in her soft, feminine voice.\n\n", false);
	...
	if(flags[FlagEnum.SHOULDRA_EXGARTUAN_SPIRIT_SEX_COUNT] > 0) MainScreen.text("once again ", false);
	...
	if(flags[FlagEnum.SHOULDRA_EXGARTUAN_SPIRIT_SEX_COUNT] == 0) MainScreen.text("\"<i>Spirit sex... I can't say I've ever tried that,</i>\" she admits. \"<i>Damn if it isn't good.</i>\"\n\n", false);
...
private ghostGinaWinSexings():void {
	...
	if(flags[FlagEnum.SHOULDRA_VAGINAL_POSSESSIONS] == 0) MainScreen.text("She 'hmm's several times inside your head, but you can feel a surge of elation as she apparently reaches a solution. \"<i>Of course!</i>\" she cries excitedly.  \"<i>I actually have never done this before!</i>\"\n\n", false);
...
private hermaphroditeGhostsCumEctoplasm():void {
	...
	//[if repeated] 
	if(flags[FlagEnum.SHOULDRA_HERMSEX_COUNT] > 0) MainScreen.text("She chuckles softly as she recalls the times you've shared, and she hungrily goes for your maleness once more, though you mentally throw her off of it.  ", false);
	MainScreen.text("The communication suddenly breaks as the ghost-girl's will surges against yours, all thoughts of submissiveness vanishing from her mind.  ", false);
	if(player.stats.int < 50) MainScreen.text("It's not very hard for her to break down your fortifications, and you soon feel yourself being pushed aside by her vast mental capacity.", false);
	...
	if(player.lowerBody.balls > 0) MainScreen.text("Stopping her double-handed stroke technique, she slips your hand under your obtrusive " + ballsDescriptLight() + " and gently pushes them aside to get a better look at your previously unnoticed female sex.  ", false);
...
private ourDadTaughtUsNotToBeAshamedOfOurDicks():void {
	...
	let x:number = player.biggestCockIndex();
	//[first encounter] 
	if(flags[FlagEnum.SHOULDRA_PENIS_DEFEAT_TIMES] == 0) MainScreen.text("She abruptly gasps in excitement, ", false);
...
private loseToShouldraWithWorms():void {
	spriteSelect(67);
	//[first encounter]
	if(flags[FlagEnum.SHOULDRA_WORM_SCENE_COUNTER] == 0) MainScreen.text("She abruptly gasps in excitement, ", false);
	else MainScreen.text("She 'ooo's softly as she recalls your anatomy, ", false);
...
private ghostGinaLosses():void {
	...
	if(flags[FlagEnum.SHOULDRA_VAGINAL_POSSESSIONS] == 0) MainScreen.text("She 'hmm's several times, raising your hand and tapping your finger against your chin. She snaps your fingers as a revelation washes over her.  \"<i>Of course!</i>\" she cries excitedly.  \"<i>I actually have never done this before!</i>\"", false);
...
private loseToShouldraAsHerm():void {
	spriteSelect(67);
	if(flags[FlagEnum.SHOULDRA_HERMSEX_COUNT] == 0) MainScreen.text("She abruptly gasps in excitement, ", false);
...
private shouldraGiantCockLoss():void {
	...
	//[if first time]
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("bemused ", false);
	...
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("searches her body for any signs of change, and when she finds none, begins pouting.", false);
	...
	//[if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("and instead of offering an explanation, she merely ", false);
	...
	//([if first time]
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("small, firm B-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) MainScreen.text("perky, sizeable C-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) MainScreen.text("well-formed, jiggly D-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) MainScreen.text("big and squishy E-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) MainScreen.text("oversized, heaving F-cups", false);
	else MainScreen.text("gigantic and openly-bared G-cups", false);
	MainScreen.text(". The goblin ", false);
	//[if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("instantly stops her protests, suddenly too stimulated by the massage to worry about any magical favoritism, ", false);
	else MainScreen.text("cries out in ecstasy at the still-incredible sensation, ", false);
	//[if first time]
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("although that doesn't stop the second goblin to begin voicing her own dissent", false);
	...
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("and... pulls the breast along with it?", false);
	else MainScreen.text("and pulls the breast along with it.", false);
	MainScreen.text("  You inwardly gasp ", false);
	//[if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("as the effects of the magic are revealed", false);
	...
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text(", evidently!", false);
	...
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("gasps in amazement, ", false);
	...
	//[if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("petite boobs", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) MainScreen.text("modest bosom", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) MainScreen.text("sizable chest", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) MainScreen.text("large endowments", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) MainScreen.text("huge tits", false);
	...
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("E-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) MainScreen.text("F-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) MainScreen.text("G-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) MainScreen.text("H-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) MainScreen.text("I-cups", false);
	...
	//[if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("HH-cups", false);
	//[if repeated once] 
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) MainScreen.text("II-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) MainScreen.text("JJ-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) MainScreen.text("KK-cups", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) MainScreen.text("LL-cups", false);
	...
	//([if first time] 
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) MainScreen.text("DD-cup (and C-cup, on the other side)", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) MainScreen.text("EE-cup (and D-cup, on the other side)", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) MainScreen.text("FF-cup (and E-cup, on the other side)", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) MainScreen.text("GG-cup (and F-cup, on the other side)", false);
	else if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) MainScreen.text("HH-cup (and G-cup, on the other side)", false);
	...
	if(flags[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] <= 4) MainScreen.text("they have kept at least a cup size of flesh from their experience", false);
...
private genderlessShouldrasLossRapes():void {
	...
	else MainScreen.text("your " + allBreastsDescript() + " swells slightly to make room for a sudden increase in milk production.", false);
	MainScreen.text("  As the ghost continues darting around the room, ", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 8) MainScreen.text("your new EE-cup breasts settle", false);
	...
	if(player.isNaga()) MainScreen.text("against your naga-crotch", false);
	else if(player.isGoo()) MainScreen.text("into your cushiony goo", false);
	...
	MainScreen.text(".  With a happy sigh, you heave yourself upright, slipping back into your " + player.armorName + " and moving out into the ruins once more.\n\n", false);
	MainScreen.text("A strange sensation begins in your midsection, and you press a hand against the flesh curiously. You remove it swiftly as a familiar head suddenly pushes out, wiggling about until it faces you. She smiles, then draws her arms out of her torso and uses them as leverage (with a whispered spell to make her hands tangible) to remove herself from you completely. Muttering an incantation and solidifying her again-unremarkable form, she winks at you and gathers her tunic, slipping it over her frame. As you watch, she slips her thumb and forefinger into her satisfied snatch, producing a healthy helping of a spooky gooey substance. Reaching into a pocket inside her tunic, she draws out a small bottle, which she promptly squishes the ectoplasm into. She offers you the bottle, explaining, \"<i>The product of a... ghost climax, if you will. Keep it, it's good for you.</i>\"  With a happy goodbye, she wanders into an alley, grabbing her ruined leggings and discarded shoes as she goes, and disappears into the shadows. You move to leave, but find your " + player.legs() + " growing heavy, fatigue washing over you in the aftermath of the possession. Before you know it, you're on your knees, and you have to struggle to keep your eyelids from drooping. You succumb to the exhaustion, slumping to the ground and soon snoring contentedly.", false);
	flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT]++;
	player.orgasm();
	dynStats("sen", 1);
...
private ghostGartuanLossSceneOrSomeShit():void {
	...
	MainScreen.text("When the proverbial dust settles, you stir groggily. Blinking the sleep out of your eyes, you do a double take at the sheer amount of semen flooding the street. You feel a sensation building in your finally flaccid member, and you glance down. A pale, transparent smoke flows gently from your urethra, reforming into a very tired, moderately sore, but incredibly satisfied ghost girl.  ", false);
	if(flags[FlagEnum.SHOULDRA_EXGARTUAN_SPIRIT_SEX_COUNT] == 0) MainScreen.text("\"<i>Spirit sex... I can't say I've ever tried that,</i>\" she admits. \"<i>Damn if it isn't good.</i>\"", false);
...
private slimeGhostGalForHerms(clearOut:boolean = false):void {
	...
	//{event occurrence == 0}
	if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 0) MainScreen.text("I've got this thing all figured out.");
	//event occurrence =1
	else if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 1) MainScreen.text("I've got this thing all figured out.  For real this time.");
	//event occurrence >=2
	else if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 2) MainScreen.text("I've got this thing all figured out.  For real this time.  Promise.");
	//event occurrence >=3
	...
	//event occurrence == 0
	if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] <= 2)MainScreen.text("Maybe she really does have a firm grip on the reigns of this beast.");
	//event occurrence >=3
	...
	//if cockLength(0) > 24
	if(player.lowerBody.cockSpot.list[0].cockLength > 24) MainScreen.text("However... it isn't exactly up to your immense scale.  ");
	...
	//vaginalLooseness == 0
	if(player.looseness() == 0) MainScreen.text("tight");
	//vaginalLooseness == 1
	else if(player.looseness() == 1) MainScreen.text("forgiving");
	else if(player.looseness() == 2) MainScreen.text("loose");
	else if(player.looseness() == 3) MainScreen.text("gaping");
	...
	if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] > 0) MainScreen.text("  Again.");
	...
	if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 0) MainScreen.text("You couldn't spank it even to save your life, as we've learned today.</i>\"");
	...
	if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 1) MainScreen.text("  \"<i>I was afraid you'd have... given up the ghost.</i>\"");
	else if(flags[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] > 1) MainScreen.text("  \"<i>But I'm more than willing to do it again next time with a conclusion like that!</i>\"");
