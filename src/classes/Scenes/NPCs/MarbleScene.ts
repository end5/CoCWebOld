...
private function apologizetoWalkingTitsIMEANMARBLE():void {
	...
	if(player.stats.int < 15) outputText("Eventually you find Marble stopped, looking towards you with her hands in the air saying, \"<i>You caught me!  Come here.</i>\"  She beckons you towards her chest, and you don't make her wait.");
	else if(player.stats.int < 40) outputText("Eventually you find Marble stopped and waiting for you.  She puts her hands in the air and says, \"<i>You caught me!</i>\"  It's fairly clear she's given herself up, but when she folds her hands in front of her chest and presses her breasts together, then tells you to come over, you aren't complaining.");
...
private function forceFeedMarble():void {
	...
	if(player.perks.has("Feeder")) outputText("You bet this cow-girl loves to get milked and nursed on just like you, but how often does she get to taste the creamy sweetness of milk?  Having her suck on your own leaky tits would be doing her a favor, right?  You approach the defeated cow-girl; her eyes are still locked onto you, wondering what you're going to do next.  Well, not that you can do much with this crowd watching you...\n\n");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 2) outputText("\n\nYou push her soft lips against your nipple");
...
private function marblePicksYouUpInitially():void {
	...
	if(player.upperBody.head.earType > EARS.HUMAN) outputText("gently scratches behind your ears.  ", false);
...
private function drinkMarbleMilk():void {
	...
	if(player.upperBody.head.earType > EARS.HUMAN) outputText(" and occasionally scratching behind your ears", false);
	outputText(".  ", false);
...
public function helpMarble2():void {
	...
	//[player spd <20] 
	if(player.stats.spe < 20) outputText("Even though Marble often stops to munch on a weed, she is still able to get more weeds then you do.  Despite her size, she can move surprisingly fast.  Regardless, she enjoys simply having you there while she works, and you get to enjoy the view.\n\n", false);
	//[player spd >=20, <50]
	if(player.stats.spe >= 20 && player.stats.spe < 50) outputText("You put in a good effort at cleaning out the weeds, and Marble often gives you a good look at her rear when she finds a tasty looking weed.\n\n", false);
	//[player spd >=50, <80]
	if(player.stats.spe >= 50 && player.stats.spe < 80) outputText("Moving quickly through the fields, you surprise Marble with your speed so much that she jokingly pouts that you're getting to all the tasty weeds before she has a chance to eat them.  You still end up getting a few good views of her ass.\n\n", false);
	//[player spd >=80] 
	if(player.stats.spe >= 80) outputText("Weeding the field is a breeze for you, going fast enough that you're able to bring weeds to Marble faster than she can eat them.  In the end, you do almost all the work yourself.  She does reward you with a good view for your efforts.\n\n", false);
	//(increase player spd)
	dynStats("spe", 1.5);
...
public function marbleEncounterAddictedNonWithdrawlAshamed():void {
	...
	//[affection >= 30]
	if(player.statusAffects.get("Marble").value1 >= 30) outputText("worriedly ", false);
	outputText("for a moment before it dawns on her that you aren't shaking.\n\n", false);
	//[affection >= 30] 
	if(player.statusAffects.get("Marble").value1 >= 30) outputText("\"<i>Sweetie, w", false);
...
public function postAddictionFarmExplorings():void {
	...
	if(player.perks.has("MarbleResistant")) outputText("uncomfortably ", false);
...
private function extendedMurbelFarmTalkz():void {
	...
	switch (flags[FlagEnum.MURBLE_FARM_TALK_LEVELS]) {
		...
		case 3:
			...
			if(player.statusAffects.get("Marble").value1 >= 60) outputText("  \"<i>Even though the start of the trip didn't turn out all that well, I think I found what I was looking for.</i>\"");
		...
		case 5:
			...
			if(player.perks.has("HistorySlut") || player.perks.has("HistoryWhore")) outputText("Well, you certainly gave an arousing description.  I think I can share something in return.");
			else if(player.stats.cor < 33) outputText("Sweetie, after you told me such a nice story, of course I will.");
			else if(player.stats.cor < 66) outputText("Sweetie, I can tell that you care a great deal about what happened, even if you try to hide it. Of course I'll share.");
		...
		default:
			...
			if(player.findStatusAffect(StatusAffects.CampMarble) < 0) {
				...
				if(player.statusAffects.get("$1").value3 == 2) outputText("  I really just wanted someone who loved me and loved nursing from my breast.  I had no idea what that would do to you.");
				//[if addiction quest is active, and Marble is happy with her milk]
				else if(player.statusAffects.get("$1").value3 == 1) outputText("  But I think that I've found the one who will change that.  Right sweetie?");
...
public function postAddictionFarmHelpings():void {
	...
	if(player.perks.has("MarbleResistant")) outputText("She assures you that you can't get addicted to it again if you don't drink her milk directly from her breasts.", false);
...
public function postAddictionCampMornings(extra:boolean = true):void {
	...
	if(!player.hasItem(consumables.M__MILK, 5)) {
		...
		if(player.perks.has("MarbleResistant")) outputText("She assures you that you'll be fine as long as you don't drink directly from her breasts.", false);
...
private function standardSex(newpage:boolean = true):void {
	...
	if(player.gender > 0) {
		...
		if(player.armorName == "bondage patient clothes" ||
		   ...
		   player.armorName == "maid's clothes" || player.armorName == "servant's clothes") {
			...
			if(player.totalCocks() > 0) {
				...
				if(player.lowerBody.vaginaSpot.count() > 0) outputText(" and your ", false);
			}
			//[player has a vagina]
			if(player.lowerBody.vaginaSpot.count() > 0) outputText(vaginaDescript(0), false);
		...
		//(player is not wearing fetish gear)
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("running one of your hands down them before continuing.", false);
...
public function marbleSex2Continued(genders: number):void {
	...
	if(genders == 1) {
		...
		if(player.lowerBody.cockSpot.list[0].cockLength < 8) outputText("You easily slide all the way into her depths before beginning your hungry thrusts.  ", false);
		...
		if(player.lowerBody.cockSpot.list[0].cockLength < 8 && player.hasKnot(0)) outputText(", and with a pop, your knot slips inside of her", false);
...
private function marbleMilkSex(newpage:boolean = true):void {
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("\"<i>So, which one is your favorite?</i>\"  Marble asks you while taking measure of your " + multiCockDescriptLight() + ".  Before you have a chance to answer, Marble grabs a hold of your central " + cockDescript(0) + " and says \"<i>I'll bet it's this one.</i>\"  ", false);
		...
		if(player.lowerBody.cockSpot.list[0].cockLength < 8) outputText("every bit of your " + cockDescript(0) + " is deep inside.  ", false);
	...
	//(player is female)
		...
		//[player has smaller then C breasts]
			...
			if(player.upperBody.chest.countNipples() > 2) outputText("other", false);
		...
		if(player.biggestLactation() <= 1 && !player.upperBody.chest.hasFuckableNipples()) outputText("Of course, since you aren't lactating, nothing but your " + nippleDescript(0) + " reaches her lips.  Marble doesn't seem to be put off by this and still gives you a thoroughly enjoyable experience.", false);
		outputText("\n\n", false);
		...
		if(player.upperBody.chest.countNipples() > 2) outputText("next", false);
...
public function interactWithMarbleAtCamp():void {
	...
	clearOutput();
	if(flags[FlagEnum.MARBLE_PURIFICATION_STAGE] != 2) outputText("How will you interact with Marble?");
...
private function marbleKidsPlaytime():void {
	...
	if(select == 2) {
		...
		//([corruption check, <=40] 
		if(player.stats.cor < 66) outputText("  You do make sure to skip over the items of a more explicit nature throughout.");
	...
	if(select == 4) {
		...
		if(flags[FlagEnum.MARBLE_KIDS] == 2) outputText("both");
...
private function definitelyBreakUpWithWithMarble():void {
	...
	if(flags[FlagEnum.MARBLE_KIDS] == 1) outputText(" and her child");
	else if(flags[FlagEnum.MARBLE_KIDS] > 1) outputText(" and her children");
...
private function marbleInfo():void {
	...
	{
		if(flags[FlagEnum.MARBLE_PURIFIED] == 0) outputText("So long as you don't drink milk from Marble's breasts again, you don't have to worry about getting addicted.  ", false);
...
private function marbleCampSexNew():void {
	...
	if(!player.isNaga()) {
		...
		if(player.perks.has("MarblesMilk") && player.gender > 0 && rand(2) == 0) {
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				...
				//( [Female Marble]
				if(flags[FlagEnum.MARBLE_DICK_LENGTH] == 0) outputText("  Her thighs seem to be rubbing together, and the place you're lying on feels a tiny bit wet, so you assume it isn't only you who's getting more and more excited by this.");
				...
				//[If balls] : A
				if(player.lowerBody.balls > 0) outputText("  As her hand reaches the base of your cock, she stops stroking it to take a moment and gently roll your " + ballsDescriptLight() + " with her fingers.  \"<i>Are these ready to shoot out for me soon, sweetie?</i>\""); 
				//([If pussy] : 
				if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  From the base of your cock, her hand moves even lower, gently rubbing your " + clitDescript() + ".  \"<i>This seems to also require some attention...  Maybe later, sweetie, if you do a good job nursing from me.  But right now, I want to see you shoot off,</i>\" she says, as her hand moves back to your " + multiCockDescriptLight() + ".");
		...
		//ORAL
			...
			if(player.lowerBody.cockSpot.hasCock()) outputText("a tent growing in your " + player.armorName);
			else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("a moistness growing under your " + player.armorName);
			...
			if(player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock()) outputText("probing her tongue in your " + vaginaDescript(0) + ", licking you in the most appropriate places");
			...
			if(player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock()) outputText("poking your cunt");
			...
			//[if multicock]
			if(player.lowerBody.cockSpot.count() > 1) outputText("  She alternatively sucks and jerks every cock you have, making sure there isn't a single meat-tower that isn't being taken care of.");
			if(player.lowerBody.balls > 0) outputText("  Her hands softly rub your [balls], making them churn and swell in pure arousal.");
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				...
				if(player.lowerBody.cockSpot.count() > 1) outputText(" while she strokes another");
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				...
				if(player.perks.has("MarblesMilk")) outputText("usually");
			...
			if(player.lowerBody.vaginaSpot.count() > 0) {
				...
				if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.WET) outputText("  As you recover, you see that a small amount of your girly fluids has leaked onto Marble's breasts.", false);
				else if(player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.SLICK) outputText("  As you recover, you see that Marble has been covered fairly liberally with your girly fluids.", false);
	...
	//NAGAS
		...
		if(player.stats.lib >= 50) outputText("  Quietly, you reach down and hook your fingers into the hem of her clothing, allowing her to pull it half off with her next shift and expose her pussy");
		if(flags[FlagEnum.MARBLE_DICK_LENGTH] > 0) outputText(" and " + marbleCock());
		...
		if(player.gender > 0) {
			...
			if(player.lowerBody.cockSpot.hasCock()) outputText("grabs your shaft");
			else outputText("penetrates your pussy");
			outputText(", while using her long tongue to lick the ");
			if(player.lowerBody.cockSpot.hasCock()) outputText("tip");
			...
			if(player.lowerBody.cockSpot.hasCock()) outputText("  [EachCock] unleashes a load of semen right into her mouth, which she swallows with pink-faced, wanton gusto.");
			if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your [pussy] runs and squirts as you moan, drooling your juices into the little valley where Marble's breasts press against your scales.");
		} 
		else {
...
public function marblePoopsBaybees():void {
	...
	if (pregnancy.type == PregnancyType.PLAYER) {
		...
		else
		{
			...
			if(flags[FlagEnum.MARBLE_DICK_TYPE] > 0) outputText(", but you notice that she does not have a cock of any kind. It seems that trait isn't passed on", false);
...
public function marbleNightSleepFlavor():boolean {
	...
	if(player.gender == 0) {
		...
		if(flags[FlagEnum.MARBLE_LUST] < 0) outputText("She makes a point of turning to face away from you when you climb into your bedroll.  Looks like she's still annoyed at you.");
		else if(flags[FlagEnum.MARBLE_LUST] < 20) outputText("She lies next to you contentedly when you climb into your bedroll.");
		else if(flags[FlagEnum.MARBLE_LUST] < 30) outputText("Marble gives a soft sigh when you climb into your bedroll.");
		else if(flags[FlagEnum.MARBLE_LUST] < 40) outputText("Marble fidgets around a bit when you climb into your bedroll.");
		else outputText("Marble seems rather worked up when you climb into your bedroll.");
		if(player.lust < 33 && flags[FlagEnum.MARBLE_LUST] < 20) outputText("  After a few moments of getting comfortable, you drift off to sleep.");
...
private function marbleNightSexDudes():void {
	...
	else 
	{
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("You pull your hands back up and begin to run them over your " + biggestBreastSizeDescript() + ", toying and teasing your nipples as your " + cockDescript(x) + " continues to pound away at Marble's confines.  Marble soon has her hands on the breasts you vacated, clearly enjoying teasing herself just as much as you are.  The two of you cry out in pleasure from the stimulation; you can feel your orgasm fast approaching.\n\n", false);
...
public function MarbleDigsDraftsYo():void {
	...
	if(player.statusAffects.get("Marble").value4 > 50) outputText("big ", false);
...
private function marbleAppearance():void {
	...
	if(flags[FlagEnum.MARBLE_BOVA_LEVEL] == 0) outputText("6 foot 4 ");
	else if(flags[FlagEnum.MARBLE_BOVA_LEVEL] == 1) outputText("6 foot 8 ");
	...
	if(flags[FlagEnum.MARBLE_BOVA_LEVEL] <= 1) outputText("She has a fairly normal soft human face, with light-colored skin.  ");
	...
	if(player.statusAffects.get("Marble").value4 <= 20) outputText("Two small horns grow from her forehead, similar in size and appearance to those on a young female bovine.  ", false);
	else if(player.statusAffects.get("Marble").value4 <= 50) outputText("Two medium sized horns grow from her forehead, similar in size and appearance to those on a female bovine.  ", false);
	...
	if(flags[FlagEnum.MARBLE_BOVA_LEVEL] <=1) outputText(", human until about half-way down her thigh.  The lower portion of her legs is covered in thick dark brown fur and ends in a pair of bestial hooves.\n\n",false);
	else outputText(" that are oddly jointed and end in a pair of bestial hooves.\n\n",false);

	if(player.statusAffects.get("Marble").value4 <=15) outputText("She has two pillowy breasts, each supporting a 0.4-inch lactating " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + " bra.\n\n", false);
	else if(player.statusAffects.get("Marble").value4<=25) outputText("She has two large breasts, each supporting a 0.6-inch lactating " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + ".\n\n", false);
	else if(player.statusAffects.get("Marble").value4<=35) outputText("She has two basketball-sized breasts, each supporting a 0.8-inch milk-seeping " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + " bra.\n\n", false);
	...
	if(flags[FlagEnum.MARBLE_DICK_TYPE] > 0) outputText("She has grown a " + marbleCock() + " since you brought her to camp. It is " + num2Text(int(flags[FlagEnum.MARBLE_DICK_LENGTH])) + " inches long and " + num2Text(int(flags[FlagEnum.MARBLE_DICK_THICKNESS])) + " inches thick.\n\n", false);
	
	if(player.statusAffects.get("Marble").value4<=50) outputText("She has a pussy, with a 0.5 inch clit.\n\n", false);
	else if(player.statusAffects.get("Marble").value4 <=75) outputText("She has a cunt, with a 0.6 inch clit.  You can see moisture gleaming from it.\n\n", false);
	...
	if(player.statusAffects.get("MarbleSpecials").value1 == 0) outputText("She has one virgin bum-hole, placed between her squeezable butt-cheeks where, you presume, it belongs.", false);
...
private function giveMarbleTailjobRelease():void {
	...
	if(player.stats.cor > 70) {
		...
		if(player.tongueType == TONUGE.SNAKE || player.tongueType == TONUGE.DEMONIC) outputText("  Your tongue runs along the length of the end of your tail, tasting both Marble's feminine secretions and her semen.  She gives you a smoldering gaze as you lick her juices up.  You grin at her.");
		//(Demon Tongue) 
		if(player.tongueType == TONUGE.DEMONIC) outputText("  You decide to put on a show for Marble, moving your tail as you drop out a large piece of your inhumanly long tongue, licking up her secretions sensuously while staring at her.  She quickly blushes under your gaze.");
...
private function milkMarble():void
{
	...
	if( flags[FlagEnum.MARBLE_MILKED_BEFORE] < 1 )
	{
		...
		if( flags[FlagEnum.MARBLE_BOVA_LEVEL] < 1 ) outputText( "two" );
...
private function milkMarbleCunnilingling():void
{
	...
	if( flags[FlagEnum.MARBLE_BOVA_LEVEL] < 1 ) outputText( "semi-human" );
	...
	if( flags[FlagEnum.MARBLE_DICK_TYPE] > 0 ) outputText( "With the first flick across its pink flesh, Marble grabs ahold of her "+ marbleCock() +", and tightly squeezes it, slowly pushing her hand down it." );
	...
	if( flags[FlagEnum.MARBLE_DICK_TYPE] > 0 ) outputText ("Of course, there is also a spurt of white which sprays out over her lap and your head.  " );
...
private function milkMarbleTakeHerDick():void
{
	...
	if( player.vaginalCapacity() < flags[FlagEnum.MARBLE_DICK_LENGTH] * flags[FlagEnum.MARBLE_DICK_THICKNESS] ) outputText( "With each new bit of length inserted, she waits a few moments for you to adjust to her size, letting you fill out to accommodate her.  " );
	...
	if( player.lowerBody.butt.buttRating <= 4 && !player.isGoo() ) outputText( "taut" );
	else outputText( "jiggling" );
	outputText( " flesh while her " + num2Text(Math.round(flags[FlagEnum.MARBLE_DICK_LENGTH])) + " inch length continues to push and pull into your womanhood.  Egged on by her enthusiasm, you start putting in some effort into the sex yourself by pushing your backside tight against her with each push forward and shaking it back and forth with each pull out of your interior." );
...
private function milkMarbleBarPullTail():void
{
	...
	if( flags[FlagEnum.MARBLE_DICK_TYPE] > 0 ) outputText( "  She grabs her erect member, and rubs it against your cheek a few times, spreading around some of her pre-cum." );
	...
	if( flags[FlagEnum.MARBLE_DICK_TYPE] == 0 )
	{
		...
		if(player.tongueType > TONUGE.HUMAN) outputText( "You take full advantage of your inhumanly long tongue to fill her up, running across her passage all the way to the entrance to her womb and then back to her lower lips." );
	...
	{
		outputText( "[pg]She forces your mouth open and pushes her hips forward, roughly shoving her " + num2Text(Math.round(flags[FlagEnum.MARBLE_DICK_LENGTH])) + "-inch cock into your hapless mouth and down your throat.  You choke and sputter in surprise at the sudden suffocation, but it only lasts a few seconds before she pulls back out." );
