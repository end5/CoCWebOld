...
//const MARBLE_MILKED_BEFORE: number = 1165;
		...
		public timeChange():boolean
		{
			...
			if (player.statusAffects.has("CampMarble")) {
				//Marble stuff pt I
				if (flags[FlagEnum.MARBLE_RATHAZUL_COUNTER_1] > 0) {
					flags[FlagEnum.MARBLE_RATHAZUL_COUNTER_1]--;
					//Stick it at 1 so I can trigger it off the camp screen.
					if (flags[FlagEnum.MARBLE_RATHAZUL_COUNTER_1] <= 1) flags[FlagEnum.MARBLE_RATHAZUL_COUNTER_1] = 1;
				}
				//Counting down to purity!
				if (flags[FlagEnum.MARBLE_PURIFICATION_STAGE] == 0) {
					if (flags[FlagEnum.MARBLE_COUNTUP_TO_PURIFYING] < 200) flags[FlagEnum.MARBLE_COUNTUP_TO_PURIFYING]++;
				}
				//Counter 2!
				if (flags[FlagEnum.MARBLE_RATHAZUL_COUNTER_2] > 0) {
					flags[FlagEnum.MARBLE_RATHAZUL_COUNTER_2]--;
					//Stick it at 1 so I can trigger it off the camp screen.
					if (flags[FlagEnum.MARBLE_RATHAZUL_COUNTER_2] <= 1) flags[FlagEnum.MARBLE_RATHAZUL_COUNTER_2] = 1;
				}
				//Post purification nursitude
				if (flags[FlagEnum.MARBLE_PURIFICATION_STAGE] >= 5) {
					flags[FlagEnum.MARBLE_TIME_SINCE_NURSED_IN_HOURS]++;
					if (flags[FlagEnum.MARBLE_TIME_SINCE_NURSED_IN_HOURS] > 1000) flags[FlagEnum.MARBLE_TIME_SINCE_NURSED_IN_HOURS] = 1000;
				}
				//Reset Marble corruption warning
				if (flags[FlagEnum.MARBLE_WARNED_ABOUT_CORRUPTION] == 1 && player.stats.cor < 50) flags[FlagEnum.MARBLE_WARNED_ABOUT_CORRUPTION] = 0;
				//Lactation whoopie!
				if (flags[FlagEnum.MARBLE_TIME_SINCE_NURSED_IN_HOURS] < 100) flags[FlagEnum.MARBLE_TIME_SINCE_NURSED_IN_HOURS]++;
				//Increment Marble's Lust
				if (flags[FlagEnum.MARBLE_LUST] < -100) flags[FlagEnum.MARBLE_LUST] = -100;
				if (rand(2) == 0) flags[FlagEnum.MARBLE_LUST] += 1;
				if (player.statusAffects.get("Marble").value4 > 50) flags[FlagEnum.MARBLE_LUST] += .3;
				if (player.statusAffects.get("Marble").value4 > 70) flags[FlagEnum.MARBLE_LUST] += .3;
				//If bitch-bin is in construction
				if (flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] > 0 && flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] < 100 && marbleScene.marbleAtCamp()) {
					flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION]++;
					if (flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] >= 100) {
						spriteSelect(41);
						MainScreen.text("\n<b>Marble lets you know that she's finished building a rather secure nursery for your coming offspring.</b>\n", false);
						needNext = true;
						flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] = 100;
					}
				}
			...
			if (player.findStatusAffect(StatusAffects.Infested) < 0) flags[FlagEnum.MARBLE_GROSSED_OUT_BECAUSE_WORM_INFESTATION] = 0;
			...
			if (model.time.hours > 23) {
				flags[FlagEnum.MARBLE_PLAYED_WITH_KIDS_TODAY] = 0;
			...
			if (player.statusAffects.get("$1").value3 > 0 && player.statusAffects.get("Marble").value2 < 25 && !player.perks.has("MarblesMilk") && !player.perks.has("MarbleResistant") && model.time.hours == 6) {
				...
				if (player.statusAffects.get("$1").value3 == 1) {
					...
					if (player.statusAffects.get("Marble").value1 >= 90) {
						...
						flags[FlagEnum.FOLLOWER_AT_FARM_MARBLE] = 0;
						if (kGAMECLASS.isabellaFollowerScene.isabellaFollower() && flags[FlagEnum.FOLLOWER_AT_FARM_ISABELLA] == 0) flags[FlagEnum.ISABELLA_MURBLE_BLEH] = 1;
						//if amily is there, tag it for freakout
						if (flags[FlagEnum.AMILY_FOLLOWER] > 0 && flags[FlagEnum.FOLLOWER_AT_FARM_AMILY] == 0) {
							flags[FlagEnum.MARBLE_OR_AMILY_FIRST_FOR_FREAKOUT] = 2;
						}
						else flags[FlagEnum.MARBLE_OR_AMILY_FIRST_FOR_FREAKOUT] = 1;
...
public marbleAtCamp():boolean
{
	if(marbleFollower())
	{
		if(flags[FlagEnum.MARBLE_RATHAZUL_COUNTER_2] > 0) return false;
		else return true;
	}
	return false;
}
...
{
	if(flags[FlagEnum.MARBLE_PURIFIED] > 0)
	{
		if(flags[FlagEnum.MARBLE_BREAST_SIZE] == 0) return "DD cup";
		else if(flags[FlagEnum.MARBLE_BREAST_SIZE] == 1) return"G cup";
		else if(flags[FlagEnum.MARBLE_BREAST_SIZE] == 2) return "HH cup";
		else return "J cup";
	}
	else
	{
...
private turnOffMarbleForever():void {
	...
	flags[FlagEnum.MARBLE_WARNING] = 1;
...
private leaveNonFriendsMarble4EVERRRR():void {
	...
	spriteSelect(41);
	player.statusAffects.add(new StatusAffect("NoMoreMarble",0,0,0,0)));
	flags[FlagEnum.MARBLE_WARNING] = 2;
...
private apologizetoWalkingTitsIMEANMARBLE():void {
	...
	flags[FlagEnum.MARBLE_WARNING] = 0;
...
private getOutOfDodge():void {
	...
	flags[FlagEnum.MARBLE_WARNING] = 3;
	doNext(camp.returnToCampUseOneHour);
}
...
private rapeMarbleInHerRoom():void {
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		MainScreen.text("Chuckling to yourself, you free your " + multiCockDescriptLight() + " from your " + player.armorName + ".  You spend a moment to ask the helpless cow-girl if she is ready, her only response being a whimper, before ", false);
		...
		if(player.lowerBody.cockSpot.list[0].cockThickness < 3) {
			...
			if(player.lowerBody.cockSpot.list[0].cockLength <= 8) {
				MainScreen.text("forcing your " + cockDescript(0) + " in as far as it will go.  ", false);
			} else 
			{
				MainScreen.text("forcing your " + cockDescript(0) + " in to the hilt.  ", false);
			}
			//the raping proper
		...
		//now if the player doesn't fit
			...
			MainScreen.text("attempting to push your " + cockDescript(0)  + " inside her.  Of course, the girth of your " + cockDescript(0)  + " makes this a rather difficult operation, and it becomes clear after a few moments that it just isn't going to fit.  Instead, you contend yourself by rubbing yourself between her ample ass cheeks, occasionally stroking your " + multiCockDescriptLight() + " in pride.\n\n", false);
			//is the player corrupt enough to get the fantasy?
			if(player.stats.cor>=33) 
				marbleRapeCorruptFantasy();
	...
	flags[FlagEnum.FARM_DISABLED] = 1;
...
private marblePicksYouUpInitially():void {
	...
	if(player.upperBody.head.earType > EARS.HUMAN) MainScreen.text("gently scratches behind your ears.  ", false);
...
private drinkMarbleMilk():void {
	...
	if(player.upperBody.head.earType > EARS.HUMAN) MainScreen.text(" and occasionally scratching behind your ears", false);
...
public encounterMarbleExploring2():void {
	...
	if(flags[FlagEnum.MURBLE_FARM_TALK_LEVELS] < 7) {
	...
	if(player.statusAffects.get("Marble").value1 < 60) {
		...
		if(player.stats.int < 30) dynStats("int", 4);
		else if(player.stats.int < 40) dynStats("int", 2);
		else if(player.stats.int < 60) dynStats("int", 1);
		doNext(camp.returnToCampUseOneHour);
	}
	else {
...
private AcceptMarblesSexualAdvances():void {
	...
	standardSex(true);
	if(player.findStatusAffect(StatusAffects.FuckedMarble) < 0) player.statusAffects.add(new StatusAffect("FuckedMarble",0,0,0,0)));
...
public helpMarble1():void {
	...
	//[player str <20] 
	if(player.str < 20) MainScreen.text("Unfortunately, the crates are quite heavy and you end up having to stick with small ones to keep up with Marble's pace.  She doesn't appear to mind, just enjoying having someone to talk to while she works, even if it doesn't save her many trips.\n\n", false);
	//[player str >=20, <50]
	if(player.str >= 20 && player.str < 50) MainScreen.text("You try your best, but for every crate you carry, Marble caries three. She doesn't mind though, since you'll end up saving her a quarter of the trips she would have had to make.\n\n", false);
	//[player str >=50, <80]
	if(player.str >= 50 && player.str < 80) MainScreen.text("You put your back into the job and manage to match Marble in her efforts.  She is really impressed with your strength, and together you can cut the number of trips needed in half.\n\n", false);
	//[player str >=80]
	if(player.str >= 80) MainScreen.text("Marble may be strong, but you are stronger.  She is amazed as you manage to take even more crates at a time than she can, only held back by the number you can balance.  Thanks to your efforts, the chore only takes a third of the number of trips it normally would have taken.\n\n", false);
...
private extendedMurbelFarmTalkz():void {
	//--- First Conversation ---
	switch (flags[FlagEnum.MURBLE_FARM_TALK_LEVELS]) {
		case 0:
			...
			//[if PC is a centaur]
			else if(player.isTaur()) MainScreen.text("  \"<i>I've seen a few centaurs before, but they don't seem to have regular homes.  They're nomads, wandering the plains.  Are you the same?</i>\"");
		...
		default:
			...
			if(player.str < 90) MainScreen.text("  In fact, I don't think there's anyone out there who is stronger than me.");
			...
			if(player.findStatusAffect(StatusAffects.CampMarble) < 0) MainScreen.text(", except that mission of yours that's so important to you");
			MainScreen.text(".</i>\"");
	...
	flags[FlagEnum.MURBLE_FARM_TALK_LEVELS]++;
}
	...
	if(player.gender > 0) {
		...
		if(player.armorName == "bondage patient clothes" ||
		   ...
		   player.armorName == "maid's clothes" || player.armorName == "servant's clothes") {
			MainScreen.text("You smirk at her and demonstrate just how easy it is to get at your genitals while wearing this outfit.  But you decide to not disappoint her, making a show of removing your " + player.armorName + " and pretending to have some modestly when you show off your ", false);
		...
		//(player is not wearing fetish gear)
			...
			MainScreen.text("You smile in return and begin removing your " + player.armorName + " from your body.  You start by slowly slipping off your top to reveal your " + allBreastsDescript() + ", ", false);
...
public marbleSex2Continued(genders: number):void {
	...
	if(genders == 1) {
		...
		if(player.lowerBody.cockSpot.list[0].cockLength < 8) MainScreen.text("You easily slide all the way into her depths before beginning your hungry thrusts.  ", false);
		...
		if(player.lowerBody.cockSpot.list[0].cockLength < 8 && player.hasKnot(0)) MainScreen.text(", and with a pop, your knot slips inside of her", false);
		MainScreen.text(".  Deep within her, your " + cockDescript(0) + " explodes.  Within moments, Marble feels her orgasm too, her grip intensifying immensely.  You hear her give a sigh that sounds almost like a moo and her grip relents.  The two of you lay there panting, your " + cockDescript(0) + " still inside her.\n\n", false);
	}
	//(player is female, I hope I did this right, I'm not familiar with the exact mechanics of female/female toys)
		...
		//[player has at least one dick]
		if(player.totalCocks() > 0) MainScreen.text(".  Your " + multiCockDescriptLight() + " erupts while clamped between your bodies", false);
...
private marbleMilkSex(newpage:boolean = true):void {
	...
	//[player is not wearing fetish gear]
		...
		MainScreen.text("She moves first and pulls open your " + player.armorName + " to get a view of your equipment.  ", false);
	}
	//(player is male, or a herm)
	if(player.lowerBody.cockSpot.count() > 0) {
		//[player has only human or animal dicks]
		if(player.lowerBody.cockSpot.count() == player.horseCocks() + player.dogCocks() + player.normalCocks()) {
			MainScreen.text("Marble seems pleased at the sight of your " + multiCockDescriptLight() + " and she pushes you back onto her bed.  She removes her own skirt and stops for a moment at her tail.  Giggling slightly, she uses her ribbon-tied tail to brush at your " + multiCockDescriptLight() + " before climbing on top of you and slipping her legs to your sides.\n\n", false);
		}
		//[player has at least one of a different kind of dick]
			...
			while(temp > 0) {
				temp--;
				if(player.lowerBody.cockSpot.list[temp].cockType.Index > 2) {
					MainScreen.text(cockDescript(temp), false);
					temp = -1;
				}
			}
		...
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("\"<i>So, which one is your favorite?</i>\"  Marble asks you while taking measure of your " + multiCockDescriptLight() + ".  Before you have a chance to answer, Marble grabs a hold of your central " + cockDescript(0) + " and says \"<i>I'll bet it's this one.</i>\"  ", false);
		MainScreen.text("Without much hesitation, Marble lifts herself up, and impales her moist lips upon your " + cockDescript(0) + ".  The two of you gasp as you sink into her interior.  You feel each inch slowly flow inside her until ", false);
		//[dick(0) is less than 8 inches long]
		if(player.lowerBody.cockSpot.list[0].cockLength < 8) MainScreen.text("every bit of your " + cockDescript(0) + " is deep inside.  ", false);
	...
	//(player is female)
		...
		if(player.totalBreasts() > 2) MainScreen.text(" with each of your " + allBreastsDescript(), false);
		else MainScreen.text(" with each of your " + breastDescript(0), false);
		MainScreen.text(", Marble looks up at you and sticks her tongue out again.  \"<i>Now it's time for the main course!</i>\" she whispers before lowering her head down to one of your " + nippleDescript(0) + "s and closing her mouth around it. She soon begins to suckle your nipple.\n\n", false);
		//[if player has nipplecunts]
		if(player.upperBody.chest.hasFuckableNipples()) {
			MainScreen.text("As Marble's tongue rubs against your " + nippleDescript(0) + " it slips inside, much to her surprise.  She pulls back for a moment and gives you a confused look.  You tell her it isn't a problem; it only makes you happier. Understanding, Marble quickly resumes her efforts.  Her talented tongue fucks your " + nippleDescript(0) + " in earnest.", false);
			//[player is lactating]
			if(player.biggestLactation() > 1) MainScreen.text("  The whole while, Marble continues to gulp down your milk.", false);
		}
		//[if player is lactating]
		else if(player.biggestLactation() > 1) MainScreen.text("Her suckling soon brings a stream of milk to her lips that she gulps down eagerly.  You give a contented sigh, but are soon brought out of your revelry as her sucking becomes more stimulating and intense.", false);
		//[player is neither lactating or has nipplecunts]
		if(player.biggestLactation() <= 1 && !player.upperBody.chest.hasFuckableNipples()) MainScreen.text("Of course, since you aren't lactating, nothing but your " + nippleDescript(0) + " reaches her lips.  Marble doesn't seem to be put off by this and still gives you a thoroughly enjoyable experience.", false);
		MainScreen.text("\n\n", false);
		MainScreen.text("After a while, Marble pulls back from your " + nippleDescript(0) + " and tells you, \"<i>Sweetie, I know how annoying it is to only have one of your nipples serviced, so I will be sure to finish the job.</i>\"  She dives right into your ", false);
		if(player.upperBody.chest.countNipples() > 2) MainScreen.text("next", false);
		else MainScreen.text("other", false);
		MainScreen.text(" " + nippleDescript(0) + " and starts the experience all over again.  By the end, her efforts have successfully rung an orgasm from you. After cleaning up a bit, Marble sends you off in high spirits.", false);
	}
	player.orgasm();
}
...
public marbleStatusChange(affection:number, addiction:number, isAddicted:number = -1):void {
	if(player.findStatusAffect(StatusAffects.Marble) < 0) player.statusAffects.add(new StatusAffect("Marble",0,0,0,40)));
...
public interactWithMarbleAtCamp():void {
	...
	MainScreen.clearText();
	if(flags[FlagEnum.MARBLE_PURIFICATION_STAGE] != 2) MainScreen.text("How will you interact with Marble?");
	...
	if (flags[FlagEnum.MARBLE_KIDS] > 0) playtime = marbleKidsPlaytime;

	if(flags[FlagEnum.MARBLE_PURIFICATION_STAGE] == 3 && flags[FlagEnum.MARBLE_RATHAZUL_COUNTER_1] == 0) {
		marbleGoEvent = marblePurification.murbleShouldGoRunAlongAndGetHerCuntySisterSoTheyCanBeCuntsTogether;
		goEventString = "Go";
	}
	else if(flags[FlagEnum.MARBLE_PURIFICATION_STAGE] >= 5) 
	{
...
private marbleTalkOverhaul():void {
	...
	if( (flags[FlagEnum.MARBLE_PURIFICATION_STAGE] < 5 && flags[FlagEnum.MARBLE_LUST] >= 20) || (flags[FlagEnum.MARBLE_PURIFICATION_STAGE] >= 5 && flags[FlagEnum.MARBLE_TIME_SINCE_NURSED_IN_HOURS] >= 4) )
	{
	...
	if(flags[FlagEnum.SLEEP_WITH] != "Marble") MainScreen.addButton(4,"Sleep With",marbleSleepToggle);
	else MainScreen.addButton(4,"Sleep Alone",marbleSleepToggle);
}

private marbleSleepToggle():void {
	MainScreen.clearText();
	if(flags[FlagEnum.SLEEP_WITH] != "Marble") {
		MainScreen.text("Marble says, \"<i>That sounds lovely, Sweetie.</i>\"");
		flags[FlagEnum.SLEEP_WITH] = "Marble";
	}
	else {
...
private marbleKidsPlaytime():void {
	...
	if(flags[FlagEnum.MARBLE_KIDS] > 1) choices[choices.length] = 2;
	if(flags[FlagEnum.MARBLE_KIDS] > 1) choices[choices.length] = 3;
	if(flags[FlagEnum.MARBLE_KIDS] > 1) choices[choices.length] = 4;
	if(flags[FlagEnum.MARBLE_KIDS] > 1) choices[choices.length] = 5;
	let select: number = choices[rand(choices.length)];
	if(flags[FlagEnum.KID_SITTER] == 2 && flags[FlagEnum.ANEMONE_KID] > 0) select = 6;
	//Bovine Saviour (Z)
	...
	if(select == 1) {
		if(flags[FlagEnum.MARBLE_KIDS] == 1 && flags[FlagEnum.MARBLE_BOYS] == 1)
		{
	...
	if(select == 4) {
		...
		if(flags[FlagEnum.MARBLE_KIDS] == 2) MainScreen.text("both");
	...
	if(flags[FlagEnum.MARBLE_PLAYED_WITH_KIDS_TODAY] > 0) dynStats("lus", -10);
...
private definitelyBreakUpWithWithMarble():void {
	...
	if(flags[FlagEnum.MARBLE_KIDS] == 1) MainScreen.text(" and her child");
	else if(flags[FlagEnum.MARBLE_KIDS] > 1) MainScreen.text(" and her children");
	MainScreen.text(", and leaves the camp behind.");
	doNext(playerMenu);
	flags[FlagEnum.BROKE_UP_WITH_MARBLE] = 1;
...
private talkWithMarbleAtCamp():void {
	...
	if(player.statusAffects.has("MetMarae") && flags[FlagEnum.MARBLE_CAMPTALK_LEVEL] < 1)
	{
		...
		//Level up!
		flags[FlagEnum.MARBLE_CAMPTALK_LEVEL] = 1;
	}
	//The player has discovered the factory
	else if(player.statusAffects.has("FoundFactory") && flags[FlagEnum.MARBLE_CAMPTALK_LEVEL] < 2)
	{
		...
		flags[FlagEnum.MARBLE_CAMPTALK_LEVEL] = 2;		
	}
	//The player has cleared the factory and shut it down
	else if(player.statusAffects.has("DungeonShutDown") && flags[FlagEnum.MARBLE_CAMPTALK_LEVEL] < 3)
	{
		...
		flags[FlagEnum.MARBLE_CAMPTALK_LEVEL] = 3;
	}
	//The player has met Marae after doing a shutdown of the factory and smashing the controls
	else if(player.statusAffects.has("MaraeComplete") && player.findStatusAffect(StatusAffects.FactoryOverload) < 0 && flags[FlagEnum.MARBLE_CAMPTALK_LEVEL] < 4)
	{
		...
		flags[FlagEnum.MARBLE_CAMPTALK_LEVEL] = 4;

	}
	//The player has met the corrupted Marae after blowing the storage tanks
	else if(player.statusAffects.has("MaraeComplete") && player.statusAffects.has("FactoryOverload") && flags[FlagEnum.MARBLE_CAMPTALK_LEVEL] < 4)
	{
	...
	//(story) Talk about Zetaz's unexplored lair(Z)
	else if(flags[FlagEnum.DISCOVERED_DUNGEON_2_ZETAZ] > 0 && flags[FlagEnum.MARBLE_CAMPTALK_LEVEL] < 5) {
		...
		flags[FlagEnum.MARBLE_CAMPTALK_LEVEL] = 5;
	}
	//story) Talk about Tel'Adre(Z)
	else if(player.statusAffects.get("TelAdre").value1 >= 1 && flags[FlagEnum.MARBLE_TELADRE_STORY] == 0) {
		...
		flags[FlagEnum.MARBLE_TELADRE_STORY] = 1;
	}
	//(story) Talk about defeating Zetaz(Z)
	else if(flags[FlagEnum.DEFEATED_ZETAZ] > 0 && flags[FlagEnum.MARBLE_CAMPTALK_LEVEL] < 6) {
		...
		flags[FlagEnum.MARBLE_CAMPTALK_LEVEL] = 6;
	}
	...
	else if(player.statusAffects.has("MetMarae") && player.findStatusAffect(StatusAffects.FoundFactory) < 0)
	{
	...
	else if(player.statusAffects.has("FoundFactory") && player.findStatusAffect(StatusAffects.DungeonShutDown) < 0)
	...
	else if(player.statusAffects.has("DungeonShutDown") && player.findStatusAffect(StatusAffects.MaraeComplete) < 0 && player.findStatusAffect(StatusAffects.MetCorruptMarae) < 0)
	...
	else if(flags[FlagEnum.DISCOVERED_DUNGEON_2_ZETAZ] == 0 || player.statusAffects.get("TelAdre").value1 == 0)
	...
	else if(flags[FlagEnum.DISCOVERED_DUNGEON_2_ZETAZ] == 0 && player.statusAffects.get("TelAdre").value1 >= 1)
	...
	else if(flags[FlagEnum.DISCOVERED_DUNGEON_2_ZETAZ] > 0 && flags[FlagEnum.DEFEATED_ZETAZ] == 0)
...
private marbleGathered():void {
	...
	if(flags[FlagEnum.MARBLE_PURIFICATION_STAGE] != 1 && player.statusAffects.get("MarbleHasItem").value1 <= 4)
		inventory.takeItem(consumables.VITAL_T, camp.returnToCampUseOneHour);
	//Tanned Leather clothes, armor, def: 5 (three days)
	else if (flags[FlagEnum.MARBLE_PURIFICATION_STAGE] != 1 && player.statusAffects.get("MarbleHasItem").value1 <= 7)
...
private marbleInfo():void {
	...
	{
		if(flags[FlagEnum.MARBLE_PURIFIED] == 0) MainScreen.text("So long as you don't drink milk from Marble's breasts again, you don't have to worry about getting addicted.  ", false);
...
private canGiveItem():boolean {
	...
	if (flags[FlagEnum.MARBLE_DICK_TYPE] == 0)
		if (player.hasItem(consumables.P_DRAFT, 1)) return true;
	else if (flags[FlagEnum.MARBLE_DICK_TYPE] > 0) {
		if (player.hasItem(consumables.PINKEGG, 1)) return true;
		if (player.hasItem(consumables.L_PNKEG, 1)) return true;
	}
	if (player.hasItem(consumables.PROBOVA, 1) && flags[FlagEnum.MARBLE_BOVA_LEVEL] < 2) return true;
	if (player.hasItem(consumables.P_LBOVA, 1) && flags[FlagEnum.MARBLE_PURIFICATION_STAGE] == 1) return true;
	if (flags[FlagEnum.MARBLE_PURIFICATION_STAGE] >= 5) {
...
public giveItem():void {
	...
	menu();
	if (player.hasItem(consumables.OVIELIX, 1)) MainScreen.addButton(1, "OviElixir", marbleOvulatesLikeMadDawg);
	if (flags[FlagEnum.MARBLE_DICK_TYPE] == 0)
		if (player.hasItem(consumables.P_DRAFT, 1)) MainScreen.addButton(2, "P.Incub Dr", MarbleDigsDraftsYo);
	else if (flags[FlagEnum.MARBLE_DICK_TYPE] > 0) {
		if (player.hasItem(consumables.PINKEGG, 1)) MainScreen.addButton(3, "Pink Egg", MarblepinkEgg);
		if (player.hasItem(consumables.L_PNKEG, 1)) MainScreen.addButton(4, "L.Pink Egg", MarbleLPinkEgg);
	}
	if (player.hasItem(consumables.PROBOVA, 1) && flags[FlagEnum.MARBLE_BOVA_LEVEL] < 2) MainScreen.addButton(5, "Pro Bova", giveMarbleTheProBovas4Sho);
	if (player.hasItem(consumables.P_LBOVA, 1) && flags[FlagEnum.MARBLE_PURIFICATION_STAGE] == 1) MainScreen.addButton(6, "P.LaBova", marblePurification.giveMarblePureQuestLabova);
	if (flags[FlagEnum.MARBLE_PURIFICATION_STAGE] >= 5) {
		if (player.hasItem(consumables.P_S_MLK, 1)) MainScreen.addButton(7, "P.SuccMilk", marblePurification.purifiedSuccubusMilkForPureMarbles);
		if (player.hasItem(consumables.REDUCTO, 1)) MainScreen.addButton(8, "ReductoBust", marblePurification.pureMurbleUsesReducto);
		if (player.hasItem(consumables.LACTAID, 1)) MainScreen.addButton(0, "Lactaid", marblePurification.lactaidForPureMurble);
	}
...
private marbleCampSexNew():void {
	...
	if(flags[FlagEnum.MARBLE_LUST] < 0) {
	...
	if(!player.isNaga()) {
		...
		if(player.perks.has("MarblesMilk") && player.gender > 0 && rand(2) == 0) {
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				MainScreen.text("She chuckles as her hand reaches your " + multiCockDescriptLight() + ".  \"<i>You seem quite ready and eager for something, sweetie.</i>\"");
			}
			else {
				MainScreen.text("She chuckles as her hand reaches your " + vaginaDescript(0) + ".  ");
				//[Vaginal wetness 0-1] 
				if(player.wetness() < 2) MainScreen.text("\"<i>Oh, sweetie, this just won't do!  It's like you're not looking forward to this at all!</i>\""); 
					//[Vaginal wetness 2-3] 
				else if(player.wetness() < 4) MainScreen.text("\"<i>It seems you're quite in need of something down here, sweetie...</i>\"");
				//[Vaginal wetness 4]
				else if(player.wetness() < 5) MainScreen.text("\"<i>My, my, sweetie; this is so wet... are you sure we should stimulate it more?</i>\"");
				//[Vaginal Wetness 5] 
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				...
				//( [Female Marble]
				if(flags[FlagEnum.MARBLE_DICK_LENGTH] == 0) MainScreen.text("  Her thighs seem to be rubbing together, and the place you're lying on feels a tiny bit wet, so you assume it isn't only you who's getting more and more excited by this.");
				...
				//[If balls] : A
				if(player.lowerBody.balls > 0) MainScreen.text("  As her hand reaches the base of your cock, she stops stroking it to take a moment and gently roll your " + ballsDescriptLight() + " with her fingers.  \"<i>Are these ready to shoot out for me soon, sweetie?</i>\""); 
				//([If pussy] : 
			...
			//[Female] 
				...
				if(player.wetness() >= 4) MainScreen.text("  Messy as it is, Marble never stops, wanting to make the orgasm as intense as possible.");
				//([Squirter]
				if(player.wetness() > 4) MainScreen.text("  Soon, you're shooting your femcum all over her arm, and she giggles at the intensity and visibility of your orgasm.");
		...
		//ORAL
			...
			if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("a tent growing in your " + player.armorName);
			else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("a moistness growing under your " + player.armorName);
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				...
				MainScreen.text("your " + clitDescript() + " quivers in arousal");
			}
			MainScreen.text(".\n\n");
			...
			if(player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock()) MainScreen.text("probing her tongue in your " + vaginaDescript(0) + ", licking you in the most appropriate places");
			...
			//[if cock] 
			if(player.lowerBody.cockSpot.hasCock()) {
				MainScreen.text("  Her lips tightly enclose your " + cockDescript(0));
				if(player.lowerBody.cockSpot.count() > 1) MainScreen.text(" while she strokes another");
				MainScreen.text(", acting as an organic cock-ring.  She looks up to you as you watch her lips undulating under the repeated pulsations of your spooge-pumping " + cockDescript(0) + ".  These cocksuckers are teasing you in the most infuriating way, and you savagely shove more of your prick down her throat, stretching her jaw and making her moan in delight, although her cries of pleasure are muffled by the pecker stuffing her mouth.");
			}
			//[if vagina]
			else MainScreen.text("  Her flexible, yet firm tongue really feels like a dong and it keeps provoking you with its prolonged digging. With a mighty thrust, you shove her against your " + vaginaDescript() + ", forcing the entirety of her oral muscle inside.");
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				...
				if(player.cumQ() >= 500) MainScreen.text("  Your " + cockDescript(0) + " keeps spouting more seed inside her, efficiently distending her belly");
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) {
				MainScreen.text("pressing them against your own " + biggestBreastSizeDescript() + ".  She giggles slightly as she puts one of them between her mounds and rubs them up and down around it, teasing your " + nippleDescript(0) + " with her own " + marbleNips() + ", a trickle of milk leaking out onto your locked chests.", false);
			} else {
			...
			if(player.lowerBody.cockSpot.count() > 0) {
				...
				if (player.lowerBody.cockSpot.count() > 1) {
					MainScreen.text("  Yet still unsatisfied, Marble engulfs and rub the rest of your " + multiCockDescriptLight() + " within her ample mounds.  ", false);
				}
			}
			...
			if(player.lowerBody.vaginaSpot.count() > 0) {
				...
				if (player.lowerBody.vaginaSpot.list[0].clitLength < 2) {
					MainScreen.text("  Marble runs down each of your legs once with her breasts before unexpectedly plunging two of her fingers inside your " + vaginaDescript(0) + ".", false);
				} 
				else {
					MainScreen.text("  Marble then pulls your " + clitDescript() + " between her breasts and does her best to pump and stimulate it as though it were a cock.", false);
				}
			}
			...
			if(player.lowerBody.vaginaSpot.count() > 0) {
				...
				if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.WET) MainScreen.text("  As you recover, you see that a small amount of your girly fluids has leaked onto Marble's breasts.", false);
				else if(player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.SLICK) MainScreen.text("  As you recover, you see that Marble has been covered fairly liberally with your girly fluids.", false);
				else if(player.vaginas[0].vaginalWetness <  5) MainScreen.text("  As you recover, you see that Marble is decently drenched in your girly fluids.", false);
	...
	//NAGAS
		...
		if(flags[FlagEnum.MARBLE_DICK_LENGTH] > 0) MainScreen.text(" and " + marbleCock());
		...
		if(player.gender > 0) {
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				MainScreen.text(multiCockDescriptLight());
				if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" and ");
			}
			if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(vaginaDescript());
...
private marbleCock():string {
	...
	if(int(Math.random()*3) == 0) {
		if(flags[FlagEnum.MARBLE_DICK_LENGTH] < 3) descript = "squat ";
		if(flags[FlagEnum.MARBLE_DICK_LENGTH] >= 3 && flags[FlagEnum.MARBLE_DICK_LENGTH] < 5) descript = "short ";
		if(flags[FlagEnum.MARBLE_DICK_LENGTH] >= 5 && flags[FlagEnum.MARBLE_DICK_LENGTH] < 7) descript = "average ";
		if(flags[FlagEnum.MARBLE_DICK_LENGTH] >= 7 && flags[FlagEnum.MARBLE_DICK_LENGTH] < 9) descript = "long ";
		if(flags[FlagEnum.MARBLE_DICK_LENGTH] >= 10 && flags[FlagEnum.MARBLE_DICK_LENGTH] < 13) descript = "huge ";
		if(flags[FlagEnum.MARBLE_DICK_LENGTH] >= 13 && flags[FlagEnum.MARBLE_DICK_LENGTH] < 18) descript = "massive ";
		if(flags[FlagEnum.MARBLE_DICK_LENGTH] >= 18 && flags[FlagEnum.MARBLE_DICK_LENGTH] < 30) descript = "enormous ";
		if(flags[FlagEnum.MARBLE_DICK_LENGTH] >= 30) descript = "towering ";
	}

	//Discuss girth one in 3 times.
	if(int(Math.random()*3) == 0) {
		//narrow, thin, ample, broad, distended, voluminous
		if(flags[FlagEnum.MARBLE_DICK_THICKNESS] <= .75) descript += "narrow ";
		if(flags[FlagEnum.MARBLE_DICK_THICKNESS] > 1 && flags[FlagEnum.MARBLE_DICK_THICKNESS] <= 1.4) descript += "ample ";
		if(flags[FlagEnum.MARBLE_DICK_THICKNESS] > 1.4 && flags[FlagEnum.MARBLE_DICK_THICKNESS] <= 2) descript += "broad ";
		if(flags[FlagEnum.MARBLE_DICK_THICKNESS] > 2 && flags[FlagEnum.MARBLE_DICK_THICKNESS] <= 3.5) descript += "fat ";
		if(flags[FlagEnum.MARBLE_DICK_THICKNESS] > 3.5) descript += "distended ";
	}
...
public marbleBadEndFollowup():void {
	...
	if(player.lowerBody == LOWER_BODY.CENTAUR) morph = "centaur";
	if(player.dogScore() >= 4) morph = "dog-morph";
	if(player.horseScore() >= 3) {
		if(player.lowerBody == LOWER_BODY.CENTAUR) morph = "centaur-morph";
...
//if(flags[FlagEnum.MARBLE_PREGNACY_INCUBATION] == 1)
public marblePoopsBaybees():void {
	...
	if (pregnancy.type == PregnancyType.PLAYER) {
		...
		if(flags[FlagEnum.MARBLE_PURIFIED] > 0 && rand(2) == 0)
		{
			...
			if (flags[FlagEnum.MARBLE_BOYS] ==0)
			{
			...
			flags[FlagEnum.MARBLE_BOYS]++;  //again, n is the flag for the number of male kids Marble has had
		} 
		else
		{
			...
			if(flags[FlagEnum.MARBLE_DICK_TYPE] > 0) MainScreen.text(", but you notice that she does not have a cock of any kind. It seems that trait isn't passed on", false);
...
public marbleNightSleepFlavor():boolean {
	...
	if(flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] == 0 && player.pregnancyType == PregnancyType.MARBLE && (player.pregnancyIncubation <= 128)) {
	...
	switch (pregnancy.eventTriggered()) {
		...
		case 4: MainScreen.text("<b>Marble's belly has gotten bigger; she is obviously pregnant.", false); //12 days in
				...
				if (flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] == 0 && pregnancy.type != PregnancyType.OVIELIXIR_EGGS) {
					flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION]++;
	...
	if (player.statusAffects.has("Infested")) {
		if(flags[FlagEnum.MARBLE_GROSSED_OUT_BECAUSE_WORM_INFESTATION] != 1) {
			...
			flags[FlagEnum.MARBLE_GROSSED_OUT_BECAUSE_WORM_INFESTATION] = 1;
		}
		return false;
	}
	...
	else if(flags[FlagEnum.MARBLE_LUST] < 50 || player.gender == 0) {
		MainScreen.text("As you go to lie down for the night, you find that Marble has gotten there before you.  ");
		if(flags[FlagEnum.MARBLE_LUST] < 0) MainScreen.text("She makes a point of turning to face away from you when you climb into your bedroll.  Looks like she's still annoyed at you.");
		else if(flags[FlagEnum.MARBLE_LUST] < 20) MainScreen.text("She lies next to you contentedly when you climb into your bedroll.");
		else if(flags[FlagEnum.MARBLE_LUST] < 30) MainScreen.text("Marble gives a soft sigh when you climb into your bedroll.");
		else if(flags[FlagEnum.MARBLE_LUST] < 40) MainScreen.text("Marble fidgets around a bit when you climb into your bedroll.");
		else MainScreen.text("Marble seems rather worked up when you climb into your bedroll.");
		if(player.lust < 33 && flags[FlagEnum.MARBLE_LUST] < 20) MainScreen.text("  After a few moments of getting comfortable, you drift off to sleep.");
	...
	//Replaces laying down texts when Marble is at or over 50 lust
		...
		if(player.lust < 33) {
			...
			if(player.isNaga() && flags[FlagEnum.MARBLE_DICK_LENGTH] > 0) tailJob = giveMarbleTailjobRelease;
...
private marbleCuddlin():void {
	...
	if(!pcPregWithMarblesKids() && !marblePregWithPCKids()) {
		...
		if(player.tallness <= 78) MainScreen.text("pulls your head on top of her chest.  She idly strokes your " + hairDescript() + " and says, \"<i>Sweetie, how about I be your pillow for the night?</i>\"  You happily agree, and soon you drift off to the sound of her steady breathing while you lay between her sizable breasts.", false);
...
private atNightAskMarbleForSomeSexMaybe():void {
	...
	else if(flags[FlagEnum.MARBLE_LUST] < 20) {
...
private marbleNightSexIntro(clear:boolean = true):void {
	...
	if(player.gender == 3) {
		if(player.cockArea(x) <= marbleCuntCapacity()) {
			...
			MainScreen.text("Marble says, \"<i>Well sweetie, since that " + cockDescript(x) + " of yours is too big to fit inside me, let's put that " + vaginaDescript(0) + " to use.</i>\"\n\n", false);
...
private marbleNightSexDudes():void {
	...
	flags[FlagEnum.MARBLE_LUST] = 0;
	MainScreen.text("", true);
	MainScreen.text("You gently lift yourself up and slide the tip of your " + cockDescript(x) + " into her waiting hole.  Ever so slowly, you push yourself further and further in.  Each inch gained brings gentle moans and coos from Marble, encouraging you onward.  ", false);
	//Can the PC's main cock go all the way?
	//If (cock 0 is longer than 8 inches) 
	if(player.lowerBody.cockSpot.list[x].cockLength > 8) {
		MainScreen.text("There is only one thing that will stop your advance: when you reach the end of the line.  \"<i>Keep going sweetie, there is room left.  Make sure you fill me all the way.</i>\"  With a groan, your " + cockDescript(x) + " hits the furthest part of Marble's confines and you can go no further.\n\n", false);
	}
	...
	else 
	{
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) MainScreen.text("You pull your hands back up and begin to run them over your " + biggestBreastSizeDescript() + ", toying and teasing your nipples as your " + cockDescript(x) + " continues to pound away at Marble's confines.  Marble soon has her hands on the breasts you vacated, clearly enjoying teasing herself just as much as you are.  The two of you cry out in pleasure from the stimulation; you can feel your orgasm fast approaching.\n\n", false);
	...
	if(player.cumQ() < 250) {
		MainScreen.text("Marble cries out in joy as your " + cockDescript(x) + " unleashes its load within her insides.  ", false);
	} 
	else {
	...
	if(player.totalCocks() > 1) 
	{
		...
		if(player.totalCocks() > 2) { 
			MainScreen.text("the rest of your " + multiCockDescriptLight(), false);
		} 
		else { 
...
private marbleNightSexChicks():void {
	...
	MainScreen.text("", true);
	if(flags[FlagEnum.MARBLE_DICK_TYPE] == 0) {
		MainScreen.text("You gently lift yourself up as Marble produces her double dildo and slips it inside her womanhood.  Smiling at her, you slowly lower your body and impale your " + vaginaDescript(0) + " onto the artificial rod.  ", false);
		...
		MainScreen.text("The two of you cry out in orgasm as your " + vaginaDescript(0) + " clamps down on one end of her dildo and her womanhood clamps down on the other end.  ", false);
		//Does this PC have a cock too?
		//If (PC has at least 1 cock)
		if(player.totalCocks() > 0) {
			MainScreen.text("At the same time, your " + multiCockDescriptLight() + " sprays its own load onto Marble's chest and stomach.  ", false);
		}
	...
	//PC is female, Marble is a herm
		...
		player.slimeFeed();
		MainScreen.text("You gently lift yourself up into the air and guide your " + vaginaDescript(0) + " to the tip of her waiting " + marbleCock() + ".  You slowly lower your " + vaginaDescript(0) + " onto her quivering " + marbleCock() + ", gaining inch by inch ", false);
		//is she satisfied?
		//If (player can fit all of her cock and PC is not a virgin) {
		if(player.vaginalCapacity() > flags[FlagEnum.MARBLE_DICK_LENGTH] * flags[FlagEnum.MARBLE_DICK_THICKNESS]) {
		...
		MainScreen.text("The two of you cry out in orgasm as your " + vaginaDescript(0) + " clamps down on her " + marbleCock() + " as it fills you with its seed.  ", false);
		//Does this PC have a cock too?
		if(player.totalCocks() > 0) {
			MainScreen.text("At the same time, ", false);
			if(player.totalCocks() > 1) MainScreen.text("each of ", false);
			MainScreen.text("your " + multiCockDescriptLight() + " sprays its own load onto Marble's chest and stomach.  ", false);
		}
		MainScreen.text("The two of you give one more shudder from the wonderful stimulation, before collapsing on top of one another.  ", false);
		//Pregnancy chance for PC, ¼ their fertility
		player.knockUp(PregnancyType.MARBLE, PregnancyType.INCUBATION_MARBLE, 150);
		player.cuntChange(flags[FlagEnum.MARBLE_DICK_THICKNESS] * flags[FlagEnum.MARBLE_DICK_LENGTH], true);
	}
	marbleSexFinish();
}
...
private marblePreggoChance(preggerMult:number):void {
	...
	if(rand(100) < preggerOdds && (player.perks.has("MarblesMilk") || flags[FlagEnum.MARBLE_PURIFICATION_STAGE] >= 5)) {
...
public MarbleDigsDraftsYo():void {
	...
	doNext(camp.returnToCampUseOneHour);
	flags[FlagEnum.MARBLE_DICK_TYPE] = 1;
	flags[FlagEnum.MARBLE_DICK_LENGTH] = 7;
	flags[FlagEnum.MARBLE_DICK_THICKNESS] = 2;
...
//removes her dick, no other effect.  Can only be given if Marble has a dick.
	...
	flags[FlagEnum.MARBLE_DICK_TYPE] = 0;
	flags[FlagEnum.MARBLE_DICK_LENGTH] = 0;
	flags[FlagEnum.MARBLE_DICK_THICKNESS] = 0;
	doNext(camp.returnToCampUseOneHour);
}
...
public marbleOvulatesLikeMadDawg():void {
	...
	if (flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] < 100 && pregnancy.type == PregnancyType.PLAYER) {
...
public marbleCuntCapacity():number {
	let size:number = 36;
	if(flags[FlagEnum.MARBLE_BOVA_LEVEL] >= 1) {
		size += 10;
	}
	if(flags[FlagEnum.MARBLE_BOVA_LEVEL] >= 2) size += 20;
	if(flags[FlagEnum.MARBLE_KIDS] > 0) size += 14;
	return size;
}

private giveMurbleProBova():void {
	MainScreen.clearText();
	if(flags[FlagEnum.MARBLE_BOVA_LEVEL] == 0) {
...
public giveMarbleTheProBovas4Sho():void {
	...
	if(flags[FlagEnum.MARBLE_BOVA_LEVEL] == 0) {
		...
		flags[FlagEnum.MARBLE_BOVA_LEVEL] = 1;
	...
	//end event
		...
		flags[FlagEnum.MARBLE_BOVA_LEVEL] = 2;
	}
	doNext(camp.returnToCampUseOneHour);
}

private marbleNip():string {
	if(flags[FlagEnum.MARBLE_BOVA_LEVEL] > 0) return "quad-nipple";
	else return "nipple";
}
...
private marbleAppearance():void {
	...
	if(flags[FlagEnum.MARBLE_BOVA_LEVEL] == 0) MainScreen.text("6 foot 4 ");
	else if(flags[FlagEnum.MARBLE_BOVA_LEVEL] == 1) MainScreen.text("6 foot 8 ");
	...
	if(flags[FlagEnum.MARBLE_BOVA_LEVEL] <= 1) MainScreen.text("She has a fairly normal soft human face, with light-colored skin.  ");
	...
	if(flags[FlagEnum.MARBLE_BOVA_LEVEL] <=1) MainScreen.text(", human until about half-way down her thigh.  The lower portion of her legs is covered in thick dark brown fur and ends in a pair of bestial hooves.\n\n",false);
	...
	if(flags[FlagEnum.MARBLE_DICK_TYPE] > 0) MainScreen.text("She has grown a " + marbleCock() + " since you brought her to camp. It is " + num2Text(int(flags[FlagEnum.MARBLE_DICK_LENGTH])) + " inches long and " + num2Text(int(flags[FlagEnum.MARBLE_DICK_THICKNESS])) + " inches thick.\n\n", false);
...
private giveMarbleTailjobRelease():void {
	...
	if(player.stats.cor > 70) {
		...
		if(player.tongueType == TONUGE.SNAKE || player.tongueType == TONUGE.DEMONIC) MainScreen.text("  Your tongue runs along the length of the end of your tail, tasting both Marble's feminine secretions and her semen.  She gives you a smoldering gaze as you lick her juices up.  You grin at her.");
		//(Demon Tongue) 
		if(player.tongueType == TONUGE.DEMONIC) MainScreen.text("  You decide to put on a show for Marble, moving your tail as you drop out a large piece of your inhumanly long tongue, licking up her secretions sensuously while staring at her.  She quickly blushes under your gaze.");
...
{
	MainScreen.clearText();
	if( flags[FlagEnum.MARBLE_MILKED_BEFORE] < 1 )
	{
		...
		if( flags[FlagEnum.MARBLE_BOVA_LEVEL] < 1 ) MainScreen.text( "two" );
		...
		flags[FlagEnum.MARBLE_MILKED_BEFORE] = 1;
	}
	else
	{
	...
	if( flags[FlagEnum.MARBLE_PURIFICATION_STAGE] >= 5 ) flags[FlagEnum.MARBLE_TIME_SINCE_NURSED_IN_HOURS] = 0;
	menu();
	MainScreen.addButton( 0, "Cunnilingus", milkMarbleCunnilingling );
	if( player.cockThatFits(marbleCuntCapacity()) >= 0 && (player.isBiped() || player.isNaga() || player.isGoo()) ) MainScreen.addButton( 1, "Fuck Her", milkMarbleFuckDatCowPussy );
	if( player.lowerBody.vaginaSpot.hasVagina() && flags[FlagEnum.MARBLE_DICK_TYPE] > 0 && (player.isBiped() || player.isGoo()) ) MainScreen.addButton( 2, "Take Her", milkMarbleTakeHerDick );
...
private milkMarbleCunnilingling():void
{
	...
	if( flags[FlagEnum.MARBLE_BOVA_LEVEL] < 1 ) MainScreen.text( "semi-human" );
	...
	if( flags[FlagEnum.MARBLE_DICK_TYPE] > 0 ) MainScreen.text( "With the first flick across its pink flesh, Marble grabs ahold of her "+ marbleCock() +", and tightly squeezes it, slowly pushing her hand down it." );
	...
	if( flags[FlagEnum.MARBLE_DICK_TYPE] > 0 ) MainScreen.text ("Of course, there is also a spurt of white which sprays out over her lap and your head.  " );
...
private milkMarbleFuckDatCowPussy():void
{
	...
	MainScreen.text( "[pg]With a flourish, you remove your [armor] and do a little sweep over your " + cockDescript(x) + ", asking her if everything is to her needs." );
...
private milkMarbleTakeHerDick():void
{
	...
	if( player.vaginalCapacity() < flags[FlagEnum.MARBLE_DICK_LENGTH] * flags[FlagEnum.MARBLE_DICK_THICKNESS] ) MainScreen.text( "With each new bit of length inserted, she waits a few moments for you to adjust to her size, letting you fill out to accommodate her.  " );
	...
	if( player.lowerBody.butt.buttRating <= 4 && !player.isGoo() ) MainScreen.text( "taut" );
	else MainScreen.text( "jiggling" );
	MainScreen.text( " flesh while her " + num2Text(Math.round(flags[FlagEnum.MARBLE_DICK_LENGTH])) + " inch length continues to push and pull into your womanhood.  Egged on by her enthusiasm, you start putting in some effort into the sex yourself by pushing your backside tight against her with each push forward and shaking it back and forth with each pull out of your interior." );
...
private milkMarbleBarFollowTail():void
{
	...
	if( x >= 0 )
	{
		...
		if( y >= 0 ) MainScreen.text( "  You find your " + cockDescript(y) + " slides up comfortably in between her cheeks, and squeeze them together a bit to give it a nice tight valley of soft flesh to run through." );
...
private milkMarbleBarPullTail():void
{
	...
	MainScreen.text( "[pg]You ignore her protests and smack her ass once, then roughly penetrate her with your " + cockDescript(x) + " while yelling at her to take it." );
	...
	if( flags[FlagEnum.MARBLE_DICK_TYPE] > 0 ) MainScreen.text( "  She grabs her erect member, and rubs it against your cheek a few times, spreading around some of her pre-cum." );
	...
	if( flags[FlagEnum.MARBLE_DICK_TYPE] == 0 )
	{
		...
		if(player.tongueType > TONUGE.HUMAN) MainScreen.text( "You take full advantage of your inhumanly long tongue to fill her up, running across her passage all the way to the entrance to her womb and then back to her lower lips." );
	...
	{
		MainScreen.text( "[pg]She forces your mouth open and pushes her hips forward, roughly shoving her " + num2Text(Math.round(flags[FlagEnum.MARBLE_DICK_LENGTH])) + "-inch cock into your hapless mouth and down your throat.  You choke and sputter in surprise at the sudden suffocation, but it only lasts a few seconds before she pulls back out." );
