...
//const MARBLE_MILKED_BEFORE: number = 1165;
		...
		public timeChangeLarge():boolean {
			...
			if (player.statusAffects.get("$1").value3 > 0 && player.statusAffects.get("Marble").value2 < 25 && !player.perks.has("MarblesMilk") && !player.perks.has("MarbleResistant") && model.time.hours == 6) {
				...
				Render.text("You find Marble in her room.  When you come in she looks up at you and starts.  \"<i>What happened?</i>\" she asks, \"<i>Something about you is completely different from before...</i>\"  You explain to her that you've gotten over your addiction and no longer crave her milk.\n", false);
				//(reduce corr by 5)
				dynStats("cor", -5);
				...
				//Ashamed Marble
					...
					if (player.statusAffects.get("Marble").value1 < 30) {
						...
						Render.text("The piece of paper looks like a page torn from a book.  It looks like an entry from an encyclopedia of sorts, it reads in formal script:\n", false);
						Render.text("<b><u>Codex: Lacta Bovine</u></b>\n", false);
						Render.text("Description: <i>A race of all female bovine-morphs, more commonly called cow-girls.  They appear as tall well endowed women, with numerous bovine characteristics.  Generally they have bovine horns, ears, tail, and legs.  Like all minotaurs, they are very strong and resilient, however, they are unusually sensitive compared to their relatives.</i>\n", false);
						Render.text("Skin and Fur: <i>The skin tone for these creatures is very close to being human, their fur more closely follows the common minotaur fur colors: brown, black or white with brown spots.</i>\n", false);
						Render.text("Behavior: <i>The behavior of Lacta Minotaurs varies greatly between each individual.  The only major unifying piece of behavior is their desire to give milk to almost any living creature, and their high libido, common to all corrupted creatures.</i>\n", false);
						Render.text("Special abilities: <i>A lightly corrupted creature, most of the corruption is centered on their breast milk.  It is addictive to those that drink it repeatedly, eventually making them dependent on the one from whom it was drank from.  The milk also strengthens the drinker, makes them better able to find the one who nursed them, and grants limited powers of control over them to the Lacta Minotaur that nursed them.  Finally, the breasts of Lacta Minotaurs are incredibly resilient, healing from almost any damage, even being cut off; they are able to produce milk for their entire life without fail.</i>\n", false);
			...
			if (player.statusAffects.get("$1").value3 > 0 && player.statusAffects.get("Marble").value2 >= 100 && !player.perks.has("MarblesMilk") && !player.perks.has("MarbleResistant") && model.time.hours == 6) {
				...
				Render.text("You find Marble in her room.  When you come in she looks up at you and smiles deeply.  \"<i>What happened?</i>\" she asks, \"<i>Something about you feels so wonderful and right.</i>\"  You explain to her that you've finally become entirely dependent on her milk.\n", false);
...
public encounterMarbleInitially():void {
	...
	Render.text("\"<i>My name's Marble, what's yours?</i>\" she asks you.  You introduce yourself and exchange a few pleasantries before she asks how she can help you.  You tell her that you actually came to help her, explaining that Whitney said she could use a gentle touch.  \"<i>Oh that would be nice</i>\", she says \"<i>Spending the night connected to the milking machine was a mistake, and now I need something gentle.</i>\"  How will you help her?", false);
...
private leaveNonFriendsMarble():void {
	clearOutput();
	spriteSelect(41);
	Render.text("Smiling politely and just as insincerely as Marble, you beg her pardon and excuse yourself.");
...
private leaveNonFriendsMarble4EVERRRR():void {
	...
	Render.text("Answering the cow-girl with a blank look, you shake your head and walk away, resolving to avoid Marble from now on.");
...
private apologizetoWalkingTitsIMEANMARBLE():void {
	...
	//Basic scene 
	Render.text("You give chase after the bovine woman, wandering around the many plants in search of the runaway.  Her constant giggling makes sure you know you're going in the right direction, but sometimes she likes to double back or make false trails so the game is more interesting.  ");
	//[(intelligence check; <15, 15-40, 41+) 
	if(player.stats.int < 15) Render.text("Eventually you find Marble stopped, looking towards you with her hands in the air saying, \"<i>You caught me!  Come here.</i>\"  She beckons you towards her chest, and you don't make her wait.");
	else if(player.stats.int < 40) Render.text("Eventually you find Marble stopped and waiting for you.  She puts her hands in the air and says, \"<i>You caught me!</i>\"  It's fairly clear she's given herself up, but when she folds her hands in front of her chest and presses her breasts together, then tells you to come over, you aren't complaining.");
	else Render.text("It isn't too hard to figure out that Marble isn't really trying, and you easily catch her off guard on one of her double backs.  She doesn't even notice you until you peek out from between the stalks next to her, reaching out and getting a handful of her backside.  \"<i>Clever " + player.mf("boy","girl") + "...</i>\" she says.");
...
private stayForFights():void {
	clearOutput();
	spriteSelect(41);
	Render.text("You fold your arms over your chest and scowl as Marble trudges back over the fields carrying a huge hammer.  Part of you feels terribly juvenile to be solving an argument with violence - but the other part is cheering at the opportunity to put the bossy cow in her place.");
	//go to battle
...
private getOutOfDodge():void {
	clearOutput();
	spriteSelect(41);
	Render.text("The hell you will... the truth is the truth no matter how many talking hammers show up.  Catharsis completed, you leave the farm and its cows behind.");
...
internal function marbleFightWin():void {
	...
	Render.text("", true);
	//Win by hp
	if(monster.HP < 1) Render.text("Marble falls to the ground defeated; she looks up at you helplessly, wondering what you're going to do next.  ", false);
	//win by lust
	else Render.text("Marble collapses and looks at you longingly, pulling up her skirt with a look of desperation in her eyes.  ", false);
	//after the lust+HP defeat scenes if the player wins
	Render.text("You've gathered a bit of a crowd around you now, thanks to the noise of this cow clunking around with her huge hooves and hammer.  It might not be a terribly good idea to rape Marble...  you'd have to drag her up to her room just to avoid interruption and Whitney would likely find out and be upset.  What do you do?");
...
internal function marbleFightLose():void {
	...
	Render.text("", true);
	//lose by hp
	if(player.HP < 1) Render.text("After a few too many blows to the head, you topple over to the ground.  ", false);
	//lose by lust
	else Render.text("Overcome by desire, you fall to your knees, and start masturbating furiously.  Disgusted with you, Marble hits you upside the head once more, knocking you over.  ", false);
	Render.text("She leans in close to your head and whispers \"<i>Don't ever come near me again, or I will crush your head with this hammer.</i>\"  She stands up and walks away from you as you pass out from your head injuries.  ", false);	
	cleanupAfterCombat();
}
...
private rapeMarbleInHerRoom():void {
	clearOutput();
	spriteSelect(41);
	Render.text("You aren't going to give up on this opportunity, but you don't want to have an audience either.  So you drag Marble and her hammer back to her room, and throw Marble onto her bed, grabbing and twisting her nipples, causing her to cry out in pain and pleasure.");
	//continue onto original rape
	spriteSelect(41);
	Render.text("  You suddenly grab at her breasts and squeeze them roughly, at which point she screams and ", false);
	Render.text("tries to slap you.  You easily duck under her hand and start twisting her nipples.  She squeals and begins to go limp under your painful ministrations.  You move her around and force her to kneel, pushing her face down into her bed.  Keeping one of your hands on her nipple, you pull down her skirt and expose her beautiful womanhood and asshole.\n\n", false);
	//dicked players
	if(player.lowerBody.cockSpot.count() > 0) {
		Render.text("Chuckling to yourself, you free your " + multiCockDescriptLight() + " from your " + player.armorName + ".  You spend a moment to ask the helpless cow-girl if she is ready, her only response being a whimper, before ", false);
		...
		if(player.lowerBody.cockSpot.list[0].cockThickness < 3) {
			...
			if(player.lowerBody.cockSpot.list[0].cockLength <= 8) {
				Render.text("forcing your " + cockDescript(0) + " in as far as it will go.  ", false);
			} else 
			{
				Render.text("forcing your " + cockDescript(0) + " in to the hilt.  ", false);
			}
			//the raping proper
			Render.text("With a grunt of pleasure, you start to push in and out while simultaneously manhandling her sensitive breasts.  Her pained cries and squeals only make you hornier and the experience all the more enjoyable for you.  You laugh from the pleasure you're getting at the expense of her pain.  Slapping her ass and marvelling at how it jiggles, you quicken the pace of your thrusts inside her.  Marble gasps at the increased rate, alternating between tones of pleasure and pain.\n\n", false);
			//is the player corrupt enough to get the fantasy?
			if(player.stats.cor>=33) 
				marbleRapeCorruptFantasy();
			Render.text("You taunt her one more time before feeling your body get racked by an orgasm and you blow your load inside her.  ", false);
			//set player's lust to 0
			player.orgasm();
		}
		//now if the player doesn't fit
		else {
			Render.text("attempting to push your " + cockDescript(0)  + " inside her.  Of course, the girth of your " + cockDescript(0)  + " makes this a rather difficult operation, and it becomes clear after a few moments that it just isn't going to fit.  Instead, you contend yourself by rubbing yourself between her ample ass cheeks, occasionally stroking your " + multiCockDescriptLight() + " in pride.\n\n", false);
			//is the player corrupt enough to get the fantasy?
			if(player.stats.cor>=33) 
				marbleRapeCorruptFantasy();
			Render.text("You taunt her one more time before feeling your body get racked by an orgasm and you blow your load onto her ass.  ", false);
			//set player's lust to 0
			player.orgasm();
		}
	}
	//dickless girls
		...
		if(player.stats.cor>=33)
			marbleRapeCorruptFantasy();
		Render.text("You taunt her one more time before feeling your body get racked by a satisfying orgasm from using Marble's own toy against her.  ", false);
		//set player's lust to 0
		player.orgasm();
	}
	//the genderless option
	else {
		Render.text("Your lack of genitals makes it difficult to actually rape Marble, but there are other things you can do.  With your free hand, you push one of your fingers into her womanhood, causing Marble to squeal as you start wriggling it around.  Of course, that's just the beginning, as soon there are two fingers in there, and then three.  As each one goes in, there is another gasp from Marble.  You pinch her nipples as your fourth goes in, pulling out a rather interesting gasp of both pain and pleasure.\n\n", false);
		//is the player corrupt enough to get the fantasy?
		if(player.stats.cor >= 33) 
			marbleRapeCorruptFantasy();
		Render.text("With just one more thing to do, you laugh at Marble before shoving your full fist inside her.  The act results in that familiar gasp of pain and pleasure.  Playing with her is indeed quite satisfying.  ", false);
		//Reduce player lust by 20
		dynStats("lus", -20);
	}
	//Pass several hours
	//Just before Marble hits the player with her hammer in original rape scene
	Render.text("Satisfied, you pull back from the cow-girl's quivering body, and collect her hammer from the floor, informing her that you'll be taking it as compensation for the trouble she's caused you.  After dressing, you exit the barn.");
...
private forceFeedMarble():void {
	...
	if(player.perks.has("Feeder")) Render.text("You bet this cow-girl loves to get milked and nursed on just like you, but how often does she get to taste the creamy sweetness of milk?  Having her suck on your own leaky tits would be doing her a favor, right?  You approach the defeated cow-girl; her eyes are still locked onto you, wondering what you're going to do next.  Well, not that you can do much with this crowd watching you...\n\n");
	Render.text("You never really noticed how many people live on this farm until now.  They're all probably expecting you to rape her - and not all of them are reconciled to the idea, judging by the looks you're getting.  What the hell, you might as well continue your business with these on-lookers around anyway... you're like 99% sure you won't be in trouble for feeding a cow.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 2) Render.text("\n\nYou push her soft lips against your nipple");
	else Render.text("\n\nYou lift your nipple up to her mouth and push it against her soft lips");
	Render.text(", but she keeps them tightly closed, refusing to drink your sweet milk.");
...
private resistMarbleInitially():void {
	...
	//(player chose resist)
	Render.text("", true);
	Render.text("Surprised by your resistance, she pulls back and apologizes for being presumptuous.  ", false);
...
private rapeMarble():void {
	spriteSelect(41);
	Render.text("", true);
	Render.text("You decide that rather than helping her, you are going to roughly manhandle her breasts and rape her.  You suddenly grab at her breasts and squeeze them roughly, at which point she screams and slaps you.  While you are still reeling from the blow, she uses a surprising amount of strength to force you out the door.  She slams it behind you and yells, \"<i>Don't you ever come back!</i>\" through the door. You hear her start to cry as you walk away.  Aw well, you didn't like her anyway.", true);
...
public encounterMarbleExploring():void {
	spriteSelect(41);
	Render.text("", true);
	Render.text("While wandering around the farm, you meet the cow-girl Marble heading towards the barn.  ", false);
	//[player height < 5 feet]
	if(player.tallness < 60) Render.text("Marble gives her customary greeting of hugging you to her breast before telling you that she is off to get milked at the barn.  ", false);
	//[player hight >= 5 feet]
	else Render.text("You exchange a quick greeting before Marble tells you that she is off to get milked at the barn.  ", false);
	//[affection <30]
	if(player.statusAffects.get("Marble").value1 < 30) {
...
private drinkMarbleMilk():void {
	spriteSelect(41);
	Render.text("", true);
	Render.text("Beaming, Marble leads you back to her room and sits down on the bed.  She invites you onto her lap and lets you start sucking at one of her nipples.  The moment that wonderful taste meets your tongue, you start gulping down the milk with reckless abandon. She sighs in pleasure in response.  From time to time, Marble gets you to switch nipples, all the while gently stroking your head", false);
	//[player has animal ears]
	if(player.upperBody.head.earType > EARS.HUMAN) Render.text(" and occasionally scratching behind your ears", false);
	Render.text(".  ", false);
	Render.text("Once you've had your fill, you pull back and the two of you smile at each other.  \"<i>It's really nice for you isn't it sweetie?  Nice for me too to have someone like you that can give a good suck on my itching nipples.</i>\"\n\n", false);
	...
	if(player.statusAffects.get("Marble").value2 < 50)
	Render.text("After drinking Marble's milk, a feeling of euphoria spreads through you as you leave the farm in high spirits.", false);
	applyMarblesMilk();
	doNext(camp.returnToCampUseOneHour);
}
...
public encounterMarbleExploring2():void {
	spriteSelect(41);
	Render.text("", true);
	Render.text("You decide to pay Marble a visit at her room.  ", false);
	//[player height <5]
	if(player.tallness < 60) Render.text("As you step into her room, she eagerly rushes over and hugs you to her breast. \"<i>You're as cute as ever, sweetie!</i>\"  ", false);
	Render.text("She is happy to see you and treats you to a small meal while you have a pleasant chat.  ");
	if(flags[FlagEnum.MURBLE_FARM_TALK_LEVELS] < 7) {
		extendedMurbelFarmTalkz();
		doNext(camp.returnToCampUseOneHour);
		return;
	}
	else Render.text("While you talk mostly about unimportant things, there is some discussion about the world and the dangers within.");
	//[addiction >30]
...
private turnDownMarbleSexRepeat():void {
	spriteSelect(41);
	Render.text("", true);
	Render.text("She looks disappointed at your refusal but quickly brightens up and says, \"<i>Ok sweetie, next time then.</i>\" On that note, you bid farewell to the pretty cow-girl and return to your camp.", false);
...
public helpMarble1():void {
	spriteSelect(41);
	Render.text("", true);
	Render.text("\"<i>You know, Marble is moving some produce right now. How about you go help her out?</i>\" Whitney suggests.  You agree to help the well-endowed anthropomorph and Whitney directs you to the storage shed.  You arrive to find that Marble is quite busy carrying stacks of crates into the barn.  She gives you a smile when she sees you and calls out, \"<i>Hey, sweetie!  Nice to see you.</i>\"  When you tell her you came to help her smile broadens.  \"<i>Oh, I'd love to have some help.  It'll save me some trips if you give me a hand,</i>\" she says happily before putting on a serious face and continuing, \"<i>but don't strain yourself sweetie, these are heavy. I don't want you to get hurt.</i>\"  With that, you get to work with her.\n\n", false);
	//[player str <20] 
	if(player.str < 20) Render.text("Unfortunately, the crates are quite heavy and you end up having to stick with small ones to keep up with Marble's pace.  She doesn't appear to mind, just enjoying having someone to talk to while she works, even if it doesn't save her many trips.\n\n", false);
	//[player str >=20, <50]
	if(player.str >= 20 && player.str < 50) Render.text("You try your best, but for every crate you carry, Marble caries three. She doesn't mind though, since you'll end up saving her a quarter of the trips she would have had to make.\n\n", false);
	//[player str >=50, <80]
	if(player.str >= 50 && player.str < 80) Render.text("You put your back into the job and manage to match Marble in her efforts.  She is really impressed with your strength, and together you can cut the number of trips needed in half.\n\n", false);
	//[player str >=80]
	if(player.str >= 80) Render.text("Marble may be strong, but you are stronger.  She is amazed as you manage to take even more crates at a time than she can, only held back by the number you can balance.  Thanks to your efforts, the chore only takes a third of the number of trips it normally would have taken.\n\n", false);
	Render.text("After a little while, you notice that Marble is walking with an almost mesmerizing sway in her hips as she carries the crates; it is rather hard to take your eyes off her.  ", false);
	if(afterMarbleHelp()) return;
	Render.text("When the two of you finish and you start to leave, Marble calls out to you, \"<i>Wait, let me give you something!</i>\" You turn and look back at her as she rushes up to you.  Smiling brilliantly, the cow-girl hands you a bottle full of warm milk, \"<i>My gift to you for your help, fresh from the source.</i>\" she says, patting her sizable chest.\n\n", false);
	//(increase player str)
	dynStats("str", 1);
...
public helpMarble2():void {
	spriteSelect(41);
	Render.text("You run into Whitney at the farm, and ask if there's something you could do.\n\n", true);
	Render.text("\"<i>I've got it; you can help Marble do some weeding.  She's in the field over there right now,</i>\" Whitney says, pointing to a nearby pasture.  Nodding to her, you set off to help the pretty cow-girl with her chores.  It takes you a while to find her, but you eventually find Marble bent over with her rump in the air.  Once you get closer you realize that she is munching on a weed.  \"<i>Oh!</i>\" she exclaims, noticing you.  She hurriedly straightens up and looks around a little embarrassed.  \"<i>Hi there sweetie, what are you doing here?</i>\"  You explain that Whitney suggested you could help her with the weeding.  \"<i>Oh!</i>\" she exclaims again, \"<i>I guess that would be nice, but don't stare at my bum too much while I'm eating, ok?</i>\"  You agree and set to work.\n\n", false);
	//[player spd <20] 
	if(player.stats.spe < 20) Render.text("Even though Marble often stops to munch on a weed, she is still able to get more weeds then you do.  Despite her size, she can move surprisingly fast.  Regardless, she enjoys simply having you there while she works, and you get to enjoy the view.\n\n", false);
	//[player spd >=20, <50]
	if(player.stats.spe >= 20 && player.stats.spe < 50) Render.text("You put in a good effort at cleaning out the weeds, and Marble often gives you a good look at her rear when she finds a tasty looking weed.\n\n", false);
	//[player spd >=50, <80]
	if(player.stats.spe >= 50 && player.stats.spe < 80) Render.text("Moving quickly through the fields, you surprise Marble with your speed so much that she jokingly pouts that you're getting to all the tasty weeds before she has a chance to eat them.  You still end up getting a few good views of her ass.\n\n", false);
	//[player spd >=80] 
	if(player.stats.spe >= 80) Render.text("Weeding the field is a breeze for you, going fast enough that you're able to bring weeds to Marble faster than she can eat them.  In the end, you do almost all the work yourself.  She does reward you with a good view for your efforts.\n\n", false);
	//(increase player spd)
	dynStats("spe", 1.5);
...
private afterMarbleHelp():boolean {
	...
	if(player.statusAffects.get("Marble").value2 >= 40) {
		...
		if(player.stats.int < 40 && rand(2) == 0) {
			Render.text("You find that the more time you spend being around Marble, the thirstier you grow for her milk.  Finally, as the two of you are finishing, you are unable to take it any longer and beg Marble to let you drink her milk.  After a moment, your words sink in and she blushes deeply.  \"<i>Ok sweetie, since you helped me out and all, let's go back to my room.</i>\"  You enter into her pleasant room once again.  She invites you onto her lap and lets you start sucking at one of her nipples.  The moment that wonderful taste meets your tongue, you start gulping down the milk without abandon. She sighs in pleasure in response.  From time to time, Marble gets you to switch nipples, all the while gently stroking your head and occasionally scratching behind your ears.\n\n", false);
			Render.text("Once you've had your fill, you pull back and the two of you smile at each other.  \"<i>It's really nice for you isn't it sweetie?  Nice for me too to have someone like you that can give a good suck on my sensitive nipples.</i>\"\n\n", false);
		...
		//[player succeeds the int check] 
			...
			Render.text("While you're working, you are continually plagued by the thought of drinking from Marble's breasts, but you're able to keep those thoughts at bay and continue working normally.\n\n", false);
		}
	}
	return false;
}
...
private withdrawlFarmVisit():void {
	spriteSelect(41);
	Render.text("You visit Whitney's farm once again. She quickly sees the tell-tale signs of your need and lets you know where Marble is.\n\n", false);
...
public addictedEncounterHappy(clearS:boolean = true):void {
	spriteSelect(41);
	if(clearS) Render.text("", true);
	//First visit post addiction:
	if(player.findStatusAffect(StatusAffects.MalonVisitedPostAddiction) < 0) {
		Render.text("You find Marble coming out of the barn, holding one of her bottles of milk.  When she spots you, she hurries over and hands you the bottle.  \"<i>I want to find something out. Can you drink from that bottle?</i>\" she asks.  Do you drink it?", false);
		//- player chooses yes/no
	...
	//Return visits
		...
		if(rand(2) == 0) {
			Render.text("You find Marble in her room, softly humming while reading a book on her bed.  You walk up to her, and without looking away from her book she says, \"<i>I can smell your need, sweetie. Are you ready for your drink?</i>\" She sets the book down and turns to you, her hands under her breasts as she leans forward.\n\n", false);
			//- inte check to avoid immediately drinking, if succeeded: 
			if(player.stats.int >= 40) {
				Render.text("Will you drink her milk?", false);
				//- player chooses yes/no
		...
		//Addiction event version 2:
			...
			Render.text("You find Marble in the midst of one of her chores.  She smiles at you and says that if you help her with her chores, she will give you a bottle of milk to soothe your nerves.  Do you do it for the milk, Marble, or refuse?", false);
...
private marbleChoreRefusal():void {
	spriteSelect(41);
	Render.text("", true);
	Render.text("You angrily tell her that you aren't going to work for her milk and turn away, leaving her visibly upset.  Your body seems to be upset at your refusal too, feeling painful all over.  Fortunately, you also feel a temporary reprieve from the symptoms of your withdrawal.", false);
...
public marbleEncounterAddictedNonWithdrawl():void {
	spriteSelect(41);
	Render.text("", true);
	Render.text("You decide to pay Marble a visit, as it would be nice to spend some time with her while you aren't in withdrawal.  You find her in her room reading a book.  She looks up at you surprised and says, \"<i>You don't look like you need milk right now.  What's up, sweetie?</i>\"  You tell her that you just wanted to spend some time together, and not worry about milk.  She laughs at you and says, \"<i>Sweetie, you'll always be thinking about milk; but I'm fine with pretending for a while.</i>\"  The two of you enjoy a meal together in her room.\n\n", false);
	if(player.statusAffects.get("Marble").value1 >= 80) {
		Render.text("As you eat, she looks deeply into your eyes for a moment. You think she is going to say something, but she shakes her head and avoids your questions about it for the rest of your time together.\n\n", false);
	}
	Render.text("After you finish, she thanks you for treating her to your company and asks you to come back soon.  You return to your camp, knowing you will probably be seeing her again soon for something less pleasant.", false);
	//(increase affection by 10)
	marbleStatusChange(5,0);
	doNext(camp.returnToCampUseOneHour);
}
...
private AshamedAddictionComfort():void {
	spriteSelect(41);
	Render.text("", true);
	Render.text("You walk straight up to her and wrap your arms around her.  She just stands there idly for a moment before embracing you back.  ", false);
	//[player height less than 5 feet]
	if(player.tallness < 60) Render.text("She pulls you into her chest and you feel relieved to see the Marble you know is still in there.  You feel warm drops of water fall on your head and look up to find Marble crying fresh tears, but this time with a big smile on her face.\n\n", false);
	//[player height greater than or equal to 5 feet]
	else Render.text("You hear her breath start to come in short breaths and look at her face to find Marble crying fresh tears, but this time with a big smile on her face.\n\n", false);
	Render.text("\"<i>Thank you, sweetie.</i>\" She whispers so softly that you almost don't hear it.  Unfortunately, being so close to her starts to remind you of what you so desperately need. The moment feels somewhat ruined as you unsuccessfully try to hold back your shaking.  She pulls back and looks you straight in the eye.  \"<i>Don't worry sweetie, we'll find a way to make this better together,</i>\" she says, holding you tightly in her arms.  You can tell she's putting on a brave face and you don't think she actually has any idea of what to do.  \"<i>Come back when you start to feel a need for my milk again,</i>\"' she tells you as you leave, little hiccups accompanying her words, \"<i>We will get through this.</i>\"", false);
	//(increase affection by 10)
	marbleStatusChange(10,0);
	//(delay withdrawal effect)
	doNext(camp.returnToCampUseOneHour);
...
private refuseMarblesMilkAddiction():void {
	spriteSelect(41);
	Render.text("", true);
	Render.text("You look at Marble and refuse to do as she says.  She looks at you in surprise and asks why. You tell her you can't bear to talk about her like that, and that if you have to make her feel bad to get over this need, it's not worth it.  After a moment to let your words sink in, she rushes over to you and ", false);
	if(player.tallness < 60) Render.text("hugs you to her chest, ", false);
	else Render.text("gives you a big hug, ", false);
	Render.text("all the while saying how wonderful you are.  The bottle ends up getting dumped on the ground during the embrace anyway, but neither of you care to notice until afterwards. But then, it doesn't matter anyway; you'll be fine for at least a little while. Right now, you just want to enjoy Marble's warm form wrapped around you.", false);
	//(increase affection by 5)
	marbleStatusChange(5,0);
	//(delay withdrawal for a few hours)
	if(player.statusAffects.has("BottledMilk")) {
...
private ashamedAddictionBegForIt():void {
	spriteSelect(41);
	Render.text("", true);
	Render.text("You look at her in horror at the suggestion of wasting her delicious milk in such a way. You snatch the milk bottle and hold it tightly to your chest.  You beg her not to talk about it like that and not to throw her milk away so easily.  She seems to be even more upset by your declaration and grabs hold of your hands.  Marble looks into your eyes for a moment and tells you that there is always hope to change before she runs off.  You are left with the milk bottle, but you think that you can wait until later to drink it.  It just felt right to make that bold declaration and it seems to have made you feel better, if only for now.", false);
	//(player gets a bottle of Marble's Milk)
	inventory.takeItem(consumables.M__MILK, camp.returnToCampUseOneHour);
	//(delay withdrawal for a few hours)
	if(player.statusAffects.has("BottledMilk")) {
...
public marbleEncounterAddictedNonWithdrawlAshamed():void {
	spriteSelect(41);
	Render.text("", true);
	Render.text("You decide to pay Marble a visit, as it would be nice to spend some time with her while you aren't in withdrawal.  You find her in her room and she looks at you ", false);
	//[affection >= 30]
	if(player.statusAffects.get("Marble").value1 >= 30) Render.text("worriedly ", false);
	Render.text("for a moment before it dawns on her that you aren't shaking.\n\n", false);
	//[affection >= 30] 
	if(player.statusAffects.get("Marble").value1 >= 30) Render.text("\"<i>Sweetie, w", false);
	else Render.text("\"<i>W", false);
	Render.text("hy are you here if you don't need my milk?</i>\"  You explain that you just want to enjoy her company like you used to.  She gives a genuine smile that probably hasn't been on her face for a while, and the two of you have a meal together in her room.\n\n", false);
	//[affection is 80 or more]
	if(player.statusAffects.get("Marble").value1 >= 80) {
		Render.text("While you're eating, Marble looks into your eyes deeply for a moment and you think she is going to say something, but she shakes her head and seems more reserved for the rest of your time together.\n\n", false);
	}
	Render.text("After you finish, she thanks you for thinking of her like this, even with what you are going through.  You return to your camp, knowing you will probably be seeing her again soon for something less pleasant.", false);
	//increase affection by 10)
	marbleStatusChange(10,0);
	doNext(camp.returnToCampUseOneHour);
}
...
public postAddictionFarmMornings():void {
	...
	Render.text("", true);
	Render.text("You hurry over to the farm to get your daily dose of Marble's milk.  It takes an hour of your day, but your body is satisfied.", false);
...
public postAddictionFarmExplorings():void {
	...
	if(player.perks.has("MarbleResistant")) Render.text("uncomfortably ", false);
	Render.text("aware of the smell of Marble's milk.", false);
	dynStats("int", 1);
	doNext(camp.returnToCampUseOneHour);
}
//Murble farm talks
private extendedMurbelFarmTalkz():void {
	...
	switch (flags[FlagEnum.MURBLE_FARM_TALK_LEVELS]) {
		case 0:
			Render.text("During your talk, Marble asks where you're from.");
			//[if PC is human] 
			if(player.race() == "human") {
				Render.text("  \"<i>The only other human I've ever met is that wandering trader Giacomo,</i>\" she tells you \"<i>but he doesn't really talk about himself.  Maybe you could tell me about humans?  I was wondering where they live and what kind of people they are.</i>\"");
				//[if PC is shorter then 5 feet] 
				if(player.tallness < 60) Render.text("  Her eyes light up. \"<i>Are they all as cute as you?</i>\"");
				//[if PC is taller then 6'6" feet] 
				else if(player.tallness > 78) Render.text("  \"<i>Are most of you this tall?</i>\"");
			}
			//[if PC is cow-girl/cowboi]
			else if(player.race() == "cow-morph" || player.race() == "cow-boy") {
				Render.text("  \"<i>It's so nice to see another of my kind,</i>\" she tells you, \"<i>I haven't seen any since I left home.  Where are you from?</i>\"");
			}
			//[if PC is a dogmorph] 
			else if(player.race() == "dog-morph") Render.text("  \"<i>I've seen lots of dog-morphs before, are you from Barkersvile?</i>\"");
			//[if PC is a centaur]
			else if(player.isTaur()) Render.text("  \"<i>I've seen a few centaurs before, but they don't seem to have regular homes.  They're nomads, wandering the plains.  Are you the same?</i>\"");
			//[if PC is not human, cow-girl/cowboi, dogmorph, or centaur] 
			else Render.text("  \"<i>It's very rare that we get a " + player.race() + " here.  Are you from around these parts?</i>\"");
			Render.text("\n\nYou sigh and think back for a moment before answering her.");
			//[if PC is not human anymore] 
			if(player.race() != "human") Render.text("\n\nYou start by explaining that you weren't born as what you appear to be; you were once a human.  Marble is surprised by this, but when you start to explain how you came to be what you are, she stops you.  \"<i>You don't need to tell me the power of some of the things in this world.  Mommy taught me how to find LaBova if I ever lose a part of my bovinity,</i>\" she says, winking at you.  \"<i>I don't know of anything that will give humanity though, so I can't really help you if you want to change back...</i>\"  You tell her that's fine and that you'll look on your own if you need to do so.  \"<i>Well then, where is your human home?</i>\" she asks.");
		...
		//--- Second Conversation ---
		case 1:
			Render.text("After you've been talking about inconsequential things for a few minutes, Marble asks you, \"<i>Sweetie, do you ever miss your home?  Do you ever wish you could be back with your parents?</i>\"");
			//[PC is pure] 
			if(player.stats.cor < 33) Render.text("\n\nYou sigh and wonder how to word your response for a few moments before telling her that you think about them almost everyday; that it's thoughts of home and family that keep you going.");
			else if(player.stats.cor < 66) Render.text("\n\nYou pause a moment before telling her that you used to think about them all the time, but you've since tried to push them from your mind so that you can focus on why you came to this world.");
			else Render.text("You chuckle and say that you hardly think of them with all the other fun things to think about.  Then you pause and say that it is in the moments that you do think about them that keep you from forgetting who you once were.");
		...
		case 3:
			//why she left
			Render.text("Marble looks over you a moment before saying, \"<i>You've mentioned this 'mission' a few times now; I guess that's why you left home?</i>\"  You nod.  \"<i>Fulfilling a duty, huh?  My story isn't quite the same as yours, and certainly not so noble!</i>\"  She laughs at this.");
			//[if PC is under 5 feet tall]
			if(player.tallness < 60) Render.text("  \"<i>Sorry, sorry, it's just that you're so cute!  I can't picture you as some great hero leaving behind home and family for a mission.</i>\"  She wipes a few tears from her face.");
			//[if PC is over 7 feet tall]
			else if(player.tallness > 84) Render.text("  \"<i>I'm sure a giant like you was the perfect choice for this mission.</i>\"");
			...
			if(player.statusAffects.get("Marble").value1 >= 60) Render.text("  \"<i>Even though the start of the trip didn't turn out all that well, I think I found what I was looking for.</i>\"");
		...
		case 4:
			...
			Render.text("After telling Marble about your explorations of the world thus far, Marble offers to tell you a bit more about her own journey.  \"<i>My family actually lives on the other side of the mountains to the south.</i>\"  She pauses, then chuckles.  \"<i>Or is it to the north?  You know how hard it is to describe direction over long distances, don't you?</i>\" You nod; while you've never had any problem with it back home, you can well imagine how the magic of this realm would eliminate the need for such facility.");
		...
		case 5:
			...
			Render.text("This time it's you who spends a fair bit talking about your own love life back home - or lack thereof, as it seemed to you sometimes.  After you finish, you notice that Marble is looking off to the side.  She turns back to you and thanks you for sharing.  You ask her if she doesn't mind continuing where you left off last time.  \"<i>");
			if(player.perks.has("HistorySlut") || player.perks.has("HistoryWhore")) Render.text("Well, you certainly gave an arousing description.  I think I can share something in return.");
			else if(player.stats.cor < 33) Render.text("Sweetie, after you told me such a nice story, of course I will.");
			else if(player.stats.cor < 66) Render.text("Sweetie, I can tell that you care a great deal about what happened, even if you try to hide it. Of course I'll share.");
			else Render.text("Well, you certainly gave an arousing description.  I think I can share something in return.");
			Render.text("  After my falling out with Ansgar, I noticed that I really missed nursing him more than anything else.</i>\"  She stops for a moment, shaking her head.  \"<i>That's not quite right.  It's more that I needed to keep nursing; it just felt like everything was wrong with me if I wasn't nursing something.</i>\"");
		...
		default:
			...
			if(player.findStatusAffect(StatusAffects.CampMarble) < 0) Render.text(", except that mission of yours that's so important to you");
			Render.text(".</i>\"");
			...
			if(player.findStatusAffect(StatusAffects.CampMarble) < 0) {
				Render.text("  I may not show it, but I'm actually really lonely on the inside.");
				//[if addiction quest is active, and Marble is ashamed of her milk]
				if(player.statusAffects.get("$1").value3 == 2) Render.text("  I really just wanted someone who loved me and loved nursing from my breast.  I had no idea what that would do to you.");
				//[if addiction quest is active, and Marble is happy with her milk]
				else if(player.statusAffects.get("$1").value3 == 1) Render.text("  But I think that I've found the one who will change that.  Right sweetie?");
				//[if addiction quest has not yet triggered]
				else Render.text("  Though, I'm not giving up hope yet.");
				Render.text("</i>\"");
			}
			else Render.text("  Until I met you, I was actually really lonely on the inside.</i>\"");
...
public postAddictionFarmHelpings():void {
	spriteSelect(41);
	Render.text("", true);
	Render.text("Smiling, Whitney suggests that you go help Marble out with her chores.  You readily agree and go out to meet with her.  Afterwards, Marble offers you a bottle of her milk.  ", false);
	//[if player is no longer addicted]
	if(player.perks.has("MarbleResistant")) Render.text("She assures you that you can't get addicted to it again if you don't drink her milk directly from her breasts.", false);
...
public postAddictionCampMornings(extra:boolean = true):void {
	...
	if(!player.hasItem(consumables.M__MILK, 5)) {
		...
		if(player.perks.has("MarbleResistant")) Render.text("She assures you that you'll be fine as long as you don't drink directly from her breasts.", false);
	...
	if(player.gender > 0) {
		...
		if(player.armorName == "bondage patient clothes" ||
		   ...
		   player.armorName == "maid's clothes" || player.armorName == "servant's clothes") {
			Render.text("You smirk at her and demonstrate just how easy it is to get at your genitals while wearing this outfit.  But you decide to not disappoint her, making a show of removing your " + player.armorName + " and pretending to have some modestly when you show off your ", false);
			//[player has at least one dick]
			if(player.totalCocks() > 0) {
				Render.text(multiCockDescriptLight(), false);
				//player has at least one dick and a vagina]
				if(player.lowerBody.vaginaSpot.count() > 0) Render.text(" and your ", false);
			}
			//[player has a vagina]
			if(player.lowerBody.vaginaSpot.count() > 0) Render.text(vaginaDescript(0), false);
			Render.text(".  You also make sure that there is no lingering clothing on your body for her, just as she did for you.\n\n", false);
		}
		//(player is not wearing fetish gear)
		else {
			Render.text("You smile in return and begin removing your " + player.armorName + " from your body.  You start by slowly slipping off your top to reveal your " + allBreastsDescript() + ", ", false);
			//[player's breasts are at least B cup]
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) Render.text("running one of your hands down them before continuing.", false);
			//[player has small or no breasts]
			else Render.text("running one of your hands over them before continuing on.", false);
			Render.text("  You slip open the lower half of your clothes, revealing your ", false);
			if(player.totalCocks() > 0) {
				Render.text(multiCockDescriptLight(), false);
				//player has at least one dick and a vagina]
				if(player.lowerBody.vaginaSpot.count() > 0) Render.text(" and your ", false);
			...
			//[player has a vagina]
			if(player.lowerBody.vaginaSpot.count() > 0) Render.text(vaginaDescript(0), false);
			Render.text(".  You then remove any other clothing, leaving your body bare.\n\n", false);
		}
		//(player is a herm)
		if(player.gender == 3) {
			Render.text("Marble's eyes widen as you show off your equipment.  \"<i>You have both?</i>\" she says in awe. \"<i>I'm not sure if I can pleasure both parts of you at the same time... which would you prefer to use?</i>\"\n\nWhich of your genitals do you want to have sex with?", false);
...
public marbleSex2Continued(genders: number):void {
	...
	//(player is male)
	if(genders == 1) {
		Render.text("Marble leans back and invites you to come.  With a coy grin, you slowly crawl towards her, each movement bringing the two of you closer until you are on top of her body. She puts her hands around you and lies back completely.  You carefully line up your " + cockDescript(0) + " and thrust into her warm snatch.  ", false);
		//[player is less than 8 inches in length]
		if(player.lowerBody.cockSpot.list[0].cockLength < 8) Render.text("You easily slide all the way into her depths before beginning your hungry thrusts.  ", false);
		//[player is more than 8 inches in length]
		else Render.text("She is unable to take all of you, but you're hardly discouraged as you begin your hungry thrusts.  ", false);
		Render.text("She gasps slightly and her arms clamp down on you, ", false);
		//[player is between 4 and 5 feet in height]
		if(player.tallness < 60 && player.tallness >= 48) Render.text("keeping your head tightly locked between her breasts.  ", false);
		//[player is not between 4 and 5 feet in height] 
		else Render.text("keeping your body tightly locked against her.  ", false);
		Render.text("Her tight grip does nothing to slow your thrusts, only helping to bring the both of you closer and closer to sweet release.  Finally, you push into her as far as you can", false);
		//[player has a knot and is not more than 8 inches long]
		if(player.lowerBody.cockSpot.list[0].cockLength < 8 && player.hasKnot(0)) Render.text(", and with a pop, your knot slips inside of her", false);
		Render.text(".  Deep within her, your " + cockDescript(0) + " explodes.  Within moments, Marble feels her orgasm too, her grip intensifying immensely.  You hear her give a sigh that sounds almost like a moo and her grip relents.  The two of you lay there panting, your " + cockDescript(0) + " still inside her.\n\n", false);
	}
	//(player is female, I hope I did this right, I'm not familiar with the exact mechanics of female/female toys)
	else {
		Render.text("Marble grins at you and reaches into the drawers of the mini-dresser next to her bed, pulling out a long, double-ended dildo. She slips one half into her own hole and invites you over to her.  With a coy grin, you slowly crawl towards her, each movement bringing the two of you closer until you slip your " +  vaginaDescript(0) + " onto the other end of the dildo.  Sighing, Marble lies down underneath you and begins to rock back and forth.  You bring your own ministrations to the mix in response. Soon, the two of you are panting and moaning against each other with the dildo sliding around between you.  Suddenly, she locks her arms around you tightly and ", false);
		//[player is between 4 and 5 feet in height]
		if(player.tallness < 60 && player.tallness >= 48) Render.text("squeezes your head into her breasts.  ", false);
		//[player is not between 4 and 5 feet in height]
		else Render.text("pulls you securely against her.  ", false);
		Render.text("Marble shudders with pleasure and redoubles her efforts at moving the dildo, quickly pushing you over the edge as well", false);
		//[player has at least one dick]
		if(player.totalCocks() > 0) Render.text(".  Your " + multiCockDescriptLight() + " erupts while clamped between your bodies", false);
		Render.text(".  Her grip soon relents and the two of you lay there panting, the dildo still connecting you.  You can hear Marble say under her breath \"<i>That was way better than on my own...</i>\"\n\n", false);
		//(remove vaginal virginity?  "Your hymen is torn...")
		player.cuntChange(8,true);
	}
	//(first time sex)
	if(player.findStatusAffect(StatusAffects.FuckedMarble) < 0) {
		Render.text("After a few minutes pass, Marble breaks the silence. \"<i>Sweetie, that was wonderful. You're really special to me, yah know?  Please remember that.</i>\"  You know that your relationship is special too; you won't forget Marble anytime soon.", false);
		player.statusAffects.add(new StatusAffect("FuckedMarble",0,0,0,0)));
	}
	//(repeat sex)
	else {
		Render.text("Marble sighs and gives you a big smile. \"<i>Sweetie, you're just as wonderful to be with as ever.  I'm always looking forward to our times together,</i>\" she tells you.  You would be inclined to agree with her.", false);
	}
	player.orgasm();
	//– return to trigger
}
...
private marbleMilkSex(newpage:boolean = true):void {
	...
	Render.text("Drinking her milk has filled you with an intense need, and you can see that need in Marble's eyes too.  You have no choice; you are going to have sex with her.\n\n", false);
	//[player is wearing fetish gear]
	if(player.armorName == "bondage patient clothes" ||
		   player.armorName == "crotch-revealing clothes" || player.armorName == "cute servant's clothes" ||
		   player.armorName == "maid's clothes" || player.armorName == "servant's clothes") {
		 Render.text("She moves first to get a good view of your equipment.  ", false);
	}
	//[player is not wearing fetish gear]
	else {
		Render.text("She moves first and pulls open your " + player.armorName + " to get a view of your equipment.  ", false);
	}
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		...
		if(player.lowerBody.cockSpot.count() == player.horseCocks() + player.dogCocks() + player.normalCocks()) {
			Render.text("Marble seems pleased at the sight of your " + multiCockDescriptLight() + " and she pushes you back onto her bed.  She removes her own skirt and stops for a moment at her tail.  Giggling slightly, she uses her ribbon-tied tail to brush at your " + multiCockDescriptLight() + " before climbing on top of you and slipping her legs to your sides.\n\n", false);
		}
		//[player has at least one of a different kind of dick]
		else {
			Render.text("Marble gives a long hard look before pointing at your ", false);
			temp = player.lowerBody.cockSpot.count();
			while(temp > 0) {
				temp--;
				if(player.lowerBody.cockSpot.list[temp].cockType.Index > 2) {
					Render.text(cockDescript(temp), false);
					temp = -1;
				}
			}
			Render.text(".  \"<i>Sweetie, what the heck is that?</i>\" she asks in an unsure tone. You smile at her and tell her that she should touch it.  After a moment, Marble reaches out and seems to visibly relax after feeling its texture and stiffness.  She pushes you back onto her bed before slipping free of her own garments and climbing on top of you.  She gives you a sly smile as she puts her legs on your sides.\n\n", false);
		}
		//[after either]
		//[player has more than one dick]
		if(player.lowerBody.cockSpot.count() > 1) Render.text("\"<i>So, which one is your favorite?</i>\"  Marble asks you while taking measure of your " + multiCockDescriptLight() + ".  Before you have a chance to answer, Marble grabs a hold of your central " + cockDescript(0) + " and says \"<i>I'll bet it's this one.</i>\"  ", false);
		Render.text("Without much hesitation, Marble lifts herself up, and impales her moist lips upon your " + cockDescript(0) + ".  The two of you gasp as you sink into her interior.  You feel each inch slowly flow inside her until ", false);
		//[dick(0) is less than 8 inches long]
		if(player.lowerBody.cockSpot.list[0].cockLength < 8) Render.text("every bit of your " + cockDescript(0) + " is deep inside.  ", false);
		// //[dick(0) is more than 8 inches long]
		else Render.text("Marble bottoms out at eight inches and you can go no further.  ", false);
		Render.text("With a delighted shiver, Marble starts to push herself up and down on you, her movements growing more and more frantic over time.  You try to slow her down but she seems to be beyond the capability of listening to you now, giving only louder and more frantic moans of pleasure.  Sooner than you would have preferred, you explode inside her. At that moment, Marble gives a final moan that sounds almost exactly like a moo.  She slows down, seeming to have already reached her peak.\n\n", false);
		Render.text("You can see that Marble is quite pleased and satisfied after that milking and sex combo session.  She rolls off you onto her bed and is soon asleep.  It takes you a few moments to get cleaned up, still in a slight daze after that frantic lovemaking.  As you depart, you give a final glance to Marble and see her still dozed off on her bed in a slightly lopsided position.", false);
	}
	//(player is female)
	else if(player.lowerBody.vaginaSpot.count() > 0) {
		//[player has sizable breast, C or bigger]
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) {
			Render.text("Marble runs her fingers along your " + vaginaDescript(0) + " for a moment before slipping a hand onto your " + biggestBreastSizeDescript() + ".  She smiles at you and says \"<i>I think that maybe we should play up here. You did enjoy mine, so I think I'll enjoy yours.</i>\"  She helps you slips off your garments and lies you down on her bed, quickly taking her place on top of you.  She sticks her tongue out before putting her mouth to one of your " + nippleDescript(0) + "s and giving it a series of gentle licks.  Taking a moment to lick her lips, Marble starts to rub, caress and lick each of your " + allBreastsDescript() + " in turn with her hands and face, always gently and always lovingly.  Marble seems to be quite familiar with these techniques; she has probably practiced on herself many times before.\n\n", false);
		}
		//[player has smaller then C breasts]
		else {
			Render.text("She teases your " + vaginaDescript(0) + " with her fingers for a moment before putting her hand on one of your " + biggestBreastSizeDescript() + ".  \"<i>I'm not sure what to do with teats this small, but I'll do my best. Though I feel kinda sorry for you, sweetie.</i>\"  You open your mouth to make an indignant response, but Marble puts a figure on your lips to shush you. She gives you an intense stare and tells you in no uncertain terms that since you enjoyed her breasts so much, it's only fair that she gets to experience your " + allBreastsDescript() + ".  She then sticks out her tongue at you before giving your " + nippleDescript(0) + " a lick and rub down, which she extends to the ", false);
			if(player.upperBody.chest.countNipples() > 2) Render.text("other", false);
			else Render.text("rest", false);
			Render.text(".\n\n", false);
		}
		//[after either]
		Render.text("After finishing up", false);
		//[if player has more than two breasts]
		if(player.totalBreasts() > 2) Render.text(" with each of your " + allBreastsDescript(), false);
		else Render.text(" with each of your " + breastDescript(0), false);
		Render.text(", Marble looks up at you and sticks her tongue out again.  \"<i>Now it's time for the main course!</i>\" she whispers before lowering her head down to one of your " + nippleDescript(0) + "s and closing her mouth around it. She soon begins to suckle your nipple.\n\n", false);
		//[if player has nipplecunts]
		if(player.upperBody.chest.hasFuckableNipples()) {
			Render.text("As Marble's tongue rubs against your " + nippleDescript(0) + " it slips inside, much to her surprise.  She pulls back for a moment and gives you a confused look.  You tell her it isn't a problem; it only makes you happier. Understanding, Marble quickly resumes her efforts.  Her talented tongue fucks your " + nippleDescript(0) + " in earnest.", false);
			//[player is lactating]
			if(player.biggestLactation() > 1) Render.text("  The whole while, Marble continues to gulp down your milk.", false);
		}
		//[if player is lactating]
		else if(player.biggestLactation() > 1) Render.text("Her suckling soon brings a stream of milk to her lips that she gulps down eagerly.  You give a contented sigh, but are soon brought out of your revelry as her sucking becomes more stimulating and intense.", false);
		//[player is neither lactating or has nipplecunts]
		if(player.biggestLactation() <= 1 && !player.upperBody.chest.hasFuckableNipples()) Render.text("Of course, since you aren't lactating, nothing but your " + nippleDescript(0) + " reaches her lips.  Marble doesn't seem to be put off by this and still gives you a thoroughly enjoyable experience.", false);
		Render.text("\n\n", false);
		Render.text("After a while, Marble pulls back from your " + nippleDescript(0) + " and tells you, \"<i>Sweetie, I know how annoying it is to only have one of your nipples serviced, so I will be sure to finish the job.</i>\"  She dives right into your ", false);
		if(player.upperBody.chest.countNipples() > 2) Render.text("next", false);
		else Render.text("other", false);
		Render.text(" " + nippleDescript(0) + " and starts the experience all over again.  By the end, her efforts have successfully rung an orgasm from you. After cleaning up a bit, Marble sends you off in high spirits.", false);
	}
	player.orgasm();
}
...
public marbleAddictionSex(newpage:boolean = true):void {
	...
	Render.text("You rock against each other, your hands working to remove Marble's clothes while her hands work at yours.  As your respective clothing falls to the floor, ", false);
	//[player is under 5.5 feet in height]
	if(player.tallness < 66) Render.text("Marble bends over and kisses you deeply on the lips, a kiss that you eagerly return.  She lifts you up into the air, her lips still locked on yours, and holds you tightly against her body. A full minute passes before she lowers you down so your bodies can be joined.\n\n", false);
	//[player is between 5.5 feet and 6.5 feet in height]
	else if(player.tallness < 78) Render.text("Marble lowers her head and locks her lips with yours, a show of passion that you eagerly return.  The two of you stay like that for a while, time seeming to standing still.  Then Marble breaks the kiss and looks deeply into your eyes as a sly grin spreads across her face.\n\n", false);
	//[player is over 6.5 feet in height]
	else Render.text("Marble grabs a hold of your head and pulls your mouth to hers for a passionate kiss.  You are only too eager to comply and join in.  Even without her vice grip on your head, it feels like you couldn't possibly leave her wonderful mouth. But you eventually push back and look into her eyes intensely.\n\n", false);
	//[player has at least one dick]
	if(player.totalCocks() > 0) {
		...
		if(player.tallness >= 48 && player.tallness <= 60) Render.text(", the breasts your head is clamped between,", false);
		Render.text(" and the warm folds where your " + cockDescript(0) + " is embedded.  You can't help but think about how wonderful a person Marble is, and you bet that Marble feels the same way about you.  \"<i>I love you sweetie.</i>\" Marble says to you gently.  You assure her that you feel the same, and at the moment, nothing else matters.\n\n", false);
	}
	//[player only has a vagina]
		...
		//[player height is between 4 feet and 5 feet]
		if(player.tallness >= 48 && player.tallness <= 60) Render.text(" and the breasts your head is clamped between", false);
		Render.text(".  You can't help but think about how wonderful a person Marble is, and you bet that Marble feels the same way about you.  \"<i>I love you sweetie.</i>\" Marble says to you gently.  You assure her that you feel the same, and at the moment, nothing else matters.", false);
		//(remove vaginal virginity? "Your hymen is torn...")
		player.cuntChange(8, true);
		Render.text("\n\n", false);
	}
	//[after either]
	Render.text("Long after the two of you finish your lovemaking, you still continue to hold on to one another.  Eventually Marble ", false);
	//[player height is under 5.5 feet]
	if(player.tallness < 66) Render.text("sets you down and ", false);
	Render.text("pushes you to arms length and asks you, \"<i>If it's all right with you, could we live together?</i>\"  You hesitate, before explaining that you don't think that's such a good idea. You explain that you aren't from this world, why you came here, and what you've found since doing so.  Marble looks at you for a moment before letting go of you and going over to her bed. Effortlessly, she lifts it up off the ground and turns to you.  \"<i>I can pull my weight, so don't you dare think I can't help you do something so important, champion.</i>\"  She says, setting the bed back down.  She barely is able to keep a straight face as she says \"<i>champion</i>\".  You smile and wonder how could you possibly turn down someone so wonderful?  \"<i>Well I guess that settles it, then,</i>\" she says happily, \"<i>I'll move right in.</i>\"", false);
...
public interactWithMarbleAtCamp():void {
	...
	clearOutput();
	if(flags[FlagEnum.MARBLE_PURIFICATION_STAGE] != 2) Render.text("How will you interact with Marble?");
	//Reminder to visit Rathazul
	else
	{
		Render.text("<i>\"Hello, sweetie; have you had a chance to talk to that alchemist yet? I’m so close to being pure at last, but I need his help if I’m to get over this last hurdle.\"</i>",true);
	}
...
private marbleTalkOverhaul():void {
	clearOutput();
	Render.text("What do you want to discuss with Marble?");
	menu();
	...
	{
		Render.text( "[pg]<b>Marble is fidgeting around uncomfortably, perhaps she needs to be milked?</b>" );
		addButton( 2, "Milking", milkMarble );
	}
	if(player.stats.cor<50 && player.statusAffects.get("Marble").value4<60) addButton(1,"Just Talk",talkWithMarbleAtCamp);
	addButton(9,"Back",interactWithMarbleAtCamp);
	if(flags[FlagEnum.SLEEP_WITH] != "Marble") addButton(4,"Sleep With",marbleSleepToggle);
...
private marbleSleepToggle():void {
	...
	if(flags[FlagEnum.SLEEP_WITH] != "Marble") {
		Render.text("Marble says, \"<i>That sounds lovely, Sweetie.</i>\"");
		flags[FlagEnum.SLEEP_WITH] = "Marble";
	}
	else {
		Render.text("Marble folds her arms across her bountiful bosom and sighs, \"<i>If you say so, Sweetie.</i>\"");
...
private marbleKidsPlaytime():void {
	...
	if(select == 1) {
		...
		{
			Render.text("Your daughter immediately clops up to you and jumps into your arms.  \"<i>" + player.mf("Daddy, daddy","Mommy, mommy") + "!  Play with me!</i>\"  You laugh at the girl in your arms, so young and yet full of energy.  She proceeds to rope you into a grand adventure, where you play the parts of the one who searches for a hero to save their camp, and the monster that has to be stopped, and also the monster's hostage that needs to be saved, while she is the great bovine saviour that carries out this grand quest.");
			Render.text("\n\nThe game finally ends with the bovine saviour victorious over the horde of ravenous monsters  - somewhere along the line it was decided that hostages were no fun and single combat wasn't impressive enough, and you exhausted your theatrical skills to provide a bevy of satisfyingly scary faces and growls - and the people of the land (you) rejoicing.  You're left a bit tired afterwards, but you certainly don't regret having played with your child.");
		}
	...
	if(select == 2) {
		...
		//([corruption check, <=40] 
		if(player.stats.cor < 66) Render.text("  You do make sure to skip over the items of a more explicit nature throughout.");
		Render.text("  Eventually your story comes to an end, and you bid farewell to your kids.");
	...
	//requires at least 2 kids
	if(select == 4) {
		Render.text("You notice Marble is sitting on some wood logs with your children.  Curious, you decide to check what exactly is going on.");
		...
		if(flags[FlagEnum.MARBLE_KIDS] == 2) Render.text("both");
		else Render.text("about half of your children");
		Render.text(" have at least some knack for it.");
	...
	if(select == 5) {
		Render.text("Marble looks up when you approach and gives you a weary smile.  \"<i>Hello, sweetie.</i>\"");
		...
		//[Height below 5 feet]
		if(player.tallness < 60) Render.text("grabs you and immediately lifts you up to her chest, giving you a big hug.");
		//[Height above 5 feet]
		else Render.text("embraces you strongly, pressing her warm body against yours.");
...
private breakUpWithMarble():void {
	spriteSelect(41);
	clearOutput();
	Render.text("Are you sure you want to break up with Marble?  You won't be able to get her back if you do so.");
...
private definitelyBreakUpWithWithMarble():void {
	spriteSelect(41);
	clearOutput();
	Render.text("You approach Marble and tell her that you need a bit of space and some time away from her.  She freezes at your words, and asks you to repeat yourself.  You do so and she nods, her face almost impassive.  \"<i>I guess I'll go back to the farm.  ");
	if(player.perks.has("MarblesMilk"))
		Render.text("I'll see you there when you need my milk, alright?");
	else
		Render.text("I'll see you later then.");
	Render.text("</i>\"  Marble collects her things");
	if(flags[FlagEnum.MARBLE_KIDS] == 1) Render.text(" and her child");
	else if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text(" and her children");
	Render.text(", and leaves the camp behind.");
	doNext(playerMenu);
...
private talkWithMarbleAtCamp():void {
	...
	//General thoughts:
	Render.text("You call Marble over and the two of you sit down on some rocks to chat.  After thinking a bit, you sort through everything that has happened in this strange land, trying to come up with a new story for your bovine friend.  ",true);
	...
	{
		Render.text("You tell Marble you found a demonic factory and relate everything you know about it.  \"<i>Be careful in there,</i>\" Marble tells you, \"<i>I'm certain that place will consume you if you're unprepared.</i>\"", false);
	...
	{
		Render.text("You tell Marble about what you found inside the factory.  She is horrified at what was being done to the other champions and assures you that no one should ever <i>belong</i> in a place like that. You continue and tell of the overseer and her fate. Marble reacts with surprise, ", false); 
		if(player.perks.has("OmnibusGift"))
			Render.text("and hopes that you've learned your lesson about accepting <i>gifts</i> from demons.  ", false);
		else 
			Render.text("but concludes that what you did was probably for the best.  At least you didn't fall for her trick.  ", false);
		Render.text("Finally, you tell her how you shut down the factory and what happened to the captured champions.  Marble is shocked that some stayed, but says she doesn't think there is much the two of you could do to help them if they're already addicted. \"<i>You should probably just leave them be, for now.</i>\"", false);
	...
	{
		Render.text("Marble is very happy to hear you helped Marae.  With the factory taken care of and Marae's corruption postponed for some time, the both of you will probably sleep a little easier tonight. ", false);
	...
	else if(player.statusAffects.has("MaraeComplete") && player.statusAffects.has("FactoryOverload") && flags[FlagEnum.MARBLE_CAMPTALK_LEVEL] < 4)
	{
		...
		if(!player.perks.has("MaraesGiftFertility") &&
			...
			Render.text("You finish your tale by recounting how you ran away.  She isn't really sure how to respond to your decision, but Marble does thank you for not leaving her behind and joining Marae.", false);
		}
		flags[FlagEnum.MARBLE_CAMPTALK_LEVEL] = 4;
	}
	//(story) Talk about Zetaz's unexplored lair(Z)
		...
		Render.text("You tell Marble about the cave you found deep in the forest.  For some reason, this seems to bother Marble, but when you ask her what is wrong, she claims it's just a feeling and tells you not to worry about it.  Maybe you should make sure you're ready before you go back there?");
	...
	//(story) Talk about defeating Zetaz(Z)
		...
		Render.text("You tell Marble about your run-in with an old foe in the forest cave.  She tenses up at first, but the obvious fact that you've made it out of there alright relaxes her.  Marble admits that for some reason she felt like you were in great danger, but couldn't figure out why she felt that.  \"<i>Anyway, I'm glad that you managed to take care of your old nemesis... that map sounds interesting, too.  Think it really leads to the demon lord's lair?  It is a bit overwhelming, sweetie, to think that you might actually defeat the demon lord!  Part of me wants to beg you not to go, but I know how driven you are, and I won't stop you from going to fulfill your mission.  Just promise me that you'll make absolutely sure you're ready before you follow that map, okay?</i>\"");
		flags[FlagEnum.MARBLE_CAMPTALK_LEVEL] = 6;
	}
	//If talked about nothing!
	else Render.text("You have no new stories to share with Marble, so you chat for a bit about inconsequential things.", false);
	...
	Render.text("The topic of conversation turns to your mission and you ask Marble what she thinks you should be doing next.  ", false);
	//If (player has not yet met Marae)
	if(player.findStatusAffect(StatusAffects.MetMarae) < 0) {
		Render.text("\"<i>Well sweetie, I guess you should start with getting to know the place a little better.  Why don't you look some more around the lake outside the farm?  I think that's the safest place to start.</i>\"",false);
	}
	else if(player.statusAffects.has("MetMarae") && player.findStatusAffect(StatusAffects.FoundFactory) < 0)
	{
		...
		if(player.level < 3) Render.text("\"<i>I think we should help out Marae and shut down that factory she mentioned was in the mountains, but I don't think you're ready to go into the Mountains yet. They can be brutal - get a little more practice, and make sure you've got a good weapon.</i>\"", false);
		else Render.text("\"<i> I think we should help out Marae and shut down that factory she mentioned was in the mountains. I have no idea what will happen in there, though, so make sure you're as ready as you can be before you go.</i>\"\n\n", false);
	}
	//Player has found factory but not shut it down.
		...
		Render.text("\"<i>You still haven't shut down the factory yet, have you?</i>\"  You shake your head.  \"<i>Well then go do it!</i>\"", false);		
	//(player has completed the factory but has not returned to Marae)
		...
		Render.text("\"<i>You haven't gone back to Marae yet have you?</i>\"  You shake your head.  \"<i>Well then go see her!  I'm sure she really wants to thank you.</i>\"", false);
	//If PC has not yet discovered Zetaz's lair or Tel'Adre (Z)
		...
		Render.text("\"<i>Well sweetie, maybe you should explore the world more?  From what I've heard from the centaurs who stop by the farm, I'm sure you could find something interesting in the deep forest, or maybe the desert,</i>\" she suggests.");
	//(suggestion) If PC has found Tel'Adre, but not Zetaz's lair(Z)
		...
		Render.text("\"<i>I was thinking that the deep woods probably still hide secrets from you, sweetie... a few times I've been there, I've noticed large numbers of imps carrying supplies.  Think there could be something to that?</i>\" she asks.");
	...
	//(suggestion) if PC has discovered Zetaz's lair but has not cleared the cave(Z)
		...
		Render.text("\"<i>Well, you haven't finished exploring that cave in the woods yet, have you?  Why don't you go there?</i>\" she offers.  She still seems to be a bit bothered by something, but doesn't voice it.");
	//Nothing to advise!
	else Render.text("\"<i>Well sweetie, this is it. Your mission has been to stop the demons, Lethice is their leader. Once you're ready, you need to go to their high mountain lair and take them down. Make sure you've finished everything else you want to do before you face Lethice, you won't get a second chance.</i>\"");
	//OLD else Render.text("\"<i>Sweetie, you've finished all that there is in the main story of the game so far.  You'll have to wait for Fenoxo to add more,</i>\" she says to you winking. \"<i>In the meantime, explore! There's a lot in here that isn't a part of the main story.  You could also try changing your form and see how that changes each encounter.  Just don't get caught or lose your head, ok sweetie?<i/>\"", false);
...
private gotMilk():void {
	spriteSelect(41);
	doNext(camp.returnToCampUseOneHour);
	Render.text("You ask Marble for a bottle of her milk, and she happily hands you one.  ", true);
	inventory.takeItem(consumables.M__MILK, camp.returnToCampUseOneHour);
}
...
private marbleGathered():void {
	...
	Render.text("You ask Marble about any supplies she might have found.  She smiles and hands you her latest find.\n\n", true);
...
private marbleInfo():void {
	spriteSelect(41);
	Render.text("Marble is a loyal friend and lover who has decided to help you with your quest.  She can be interacted with while she is at camp.  ",true);
	Render.text("She can share some of her thoughts and give advice on your current situation, or supply you with bottles of her milk and other useful items that she has found while scavenging.  You can also get Marble to consume some of the items you find.\n\n", false);
	//explain morning drinking sessions if the player is an addict
	if(player.perks.has("MarblesMilk"))
	{
		Render.text("She will nurse you every morning automatically to satisfy your addiction.  ",false);
		//otherwise tell the player that drinking bottles of her milk is safe
	} 
	else
	{
		if(flags[FlagEnum.MARBLE_PURIFIED] == 0) Render.text("So long as you don't drink milk from Marble's breasts again, you don't have to worry about getting addicted.  ", false);
		else Render.text("Now that she's been purified, you can drink as much of her milk as you like without fear.  ");
	}
	Render.text("Once camp improvements have been implemented, you will be able to get her to work on upgrading the camp instead of searching for supplies.", false);
	doNext(interactWithMarbleAtCamp);
}
...
public giveItem():void {
	clearOutput();
	Render.text("What item do you want to give Marble?");
	menu();
	...
	if (player.hasItem(consumables.PROBOVA, 1) && flags[FlagEnum.MARBLE_BOVA_LEVEL] < 2) addButton(5, "Pro Bova", giveMarbleTheProBovas4Sho);
	if (player.hasItem(consumables.P_LBOVA, 1) && flags[FlagEnum.MARBLE_PURIFICATION_STAGE] == 1) addButton(6, "P.LaBova", marblePurification.giveMarblePureQuestLabova);
	if (flags[FlagEnum.MARBLE_PURIFICATION_STAGE] >= 5) {
	...
	else if (player.perks.has("MarblesMilk")) addButton(0, "Lactaid", giveMarbleLactaid);
	addButton(9, "Back", interactWithMarbleAtCamp);
}
...
public giveMarbleLactaid():void {
	...
	Render.text("You show Marble the bottle of Lactaid that you found and ask her if she could drink it.  She gives you a knowing smile. \"<i>So you want a special treat?  Ok sweetie, give it here.</i>\"  She takes the bottle from you and easily drinks the liquid.  Within moments, she gasps and ", true);
	if(player.tallness<60) Render.text("lifts you up to her chest before saying breathlessly saying \"<i>Drink now!</i>\"  You waste no time obliging her.\n\n", false);
	else if(player.tallness > 72) Render.text("breathlessly says\"<i>Drink now!</i>\"  You waste no time and descend upon her breasts.\n\n", false);
	else Render.text("breathlessly says, \"<i>Drink now!</i>\"  You quickly move forward to get a drink.\n\n", false);
	Render.text("What meets your lips surprises you; it feels tastier, healthier, and just plain better than normal.  With an effort, you manage to empty her breasts, even though they seemed to have been filled with more milk than usual.  You feel even more powerful than normal and the whole world seems clearer.  However, Marble seems a little disappointed; it seems the Lactaid's effects on her were only temporary.\n\n", false);
...
private marbleCampSexNew():void {
	...
	if(!player.isNaga()) {
		...
		if(player.perks.has("MarblesMilk") && player.gender > 0 && rand(2) == 0) {
			...
			clearOutput();
			Render.text("You tell Marble you're a little bit horny and you're wondering if she'd be willing to do something special for you in private.  Hearing you whisper your idea, she giggles and strokes your [face].");
			Render.text("\n\n\"<i>That's so cute, sweetie!  Of course I could do that for you.</i>\"");
			Render.text("  She leads you by the hand to sit in a more comfortable place.");
			...
			if(player.tallness < 66) Render.text("right breast.");
			//[height above 5' 7''] 
			else Render.text("left breast.");
			//[+ if height above 8' 6'']
			if(player.tallness > 90) Render.text("  You do need to bend over in order to access her breast as well as allow her access to your groin, but while slightly uncomfortable, your plan is worth the effort.");
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				Render.text("She chuckles as her hand reaches your " + multiCockDescriptLight() + ".  \"<i>You seem quite ready and eager for something, sweetie.</i>\"");
			}
			else {
				Render.text("She chuckles as her hand reaches your " + vaginaDescript(0) + ".  ");
				//[Vaginal wetness 0-1] 
				if(player.wetness() < 2) Render.text("\"<i>Oh, sweetie, this just won't do!  It's like you're not looking forward to this at all!</i>\""); 
					//[Vaginal wetness 2-3] 
				else if(player.wetness() < 4) Render.text("\"<i>It seems you're quite in need of something down here, sweetie...</i>\"");
				//[Vaginal wetness 4]
				else if(player.wetness() < 5) Render.text("\"<i>My, my, sweetie; this is so wet... are you sure we should stimulate it more?</i>\"");
				//[Vaginal Wetness 5] 
				else Render.text("\"<i>Oh my, sweetie; it seems like you need to be toweled off...</i>\"");
			}
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				...
				//( [Female Marble]
				if(flags[FlagEnum.MARBLE_DICK_LENGTH] == 0) Render.text("  Her thighs seem to be rubbing together, and the place you're lying on feels a tiny bit wet, so you assume it isn't only you who's getting more and more excited by this.");
				else Render.text("  Her thighs are gently rubbing beneath you, and you feel something hard poking you in the back.  You smile around the nipple you have in your mouth.");
				//[If balls] : A
				if(player.lowerBody.balls > 0) Render.text("  As her hand reaches the base of your cock, she stops stroking it to take a moment and gently roll your " + ballsDescriptLight() + " with her fingers.  \"<i>Are these ready to shoot out for me soon, sweetie?</i>\""); 
				//([If pussy] : 
				if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  From the base of your cock, her hand moves even lower, gently rubbing your " + clitDescript() + ".  \"<i>This seems to also require some attention...  Maybe later, sweetie, if you do a good job nursing from me.  But right now, I want to see you shoot off,</i>\" she says, as her hand moves back to your " + multiCockDescriptLight() + ".");
				...
				if(player.cumQ() < 500) Render.text("  You cover her hand as she keeps jerking you off, squeezing out tiny little droplets once you're done ejaculating.");
				else if(player.cumQ() < 1000) Render.text("  Your load is as large as always, and ropes of cum fly into the air, covering her arm and your abdomen... and you swear one string even lands of her right breast along the way.  \"<i>That's it, sweetie, shoot all you have.</i>\"");
				else Render.text("  You pretty much cover yourself, Marble, and the surrounding ground with your enormous white load of cum.  She seems numbed from the sheer output you're capable of.  \"<i>Uh... sweetie...</i>\"");
				Render.text("\n\n\"<i>Well... that was fun.</i>\"  Marble slowly pulls you away from her breast");
				if(player.cumQ() >= 1000) Render.text(", then wipes the nipple clean on the other");
				Render.text(".  \"<i>Now, sweetie, if you'd be so kind to suck on the other one...</i>\"");
			}
			//[Female] 
			else {
				...
				if(player.wetness() >= 4) Render.text("  Messy as it is, Marble never stops, wanting to make the orgasm as intense as possible.");
				//([Squirter]
				if(player.wetness() > 4) Render.text("  Soon, you're shooting your femcum all over her arm, and she giggles at the intensity and visibility of your orgasm.");
		...
		//ORAL
			...
			Render.text("You walk up to your big-breasted lover, a feverish look in your eyes.  \"<i>Hmm... looks like you need to release some tension, don't you sweetie?</i>\"  You nod in approbation, ");
			if(player.lowerBody.cockSpot.hasCock()) Render.text("a tent growing in your " + player.armorName);
			else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("a moistness growing under your " + player.armorName);
			Render.text(" as you keep staring at her tender eyes; you instinctively know she's going to handle your junk with care.");
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				Render.text("[eachCock] throbs");
			}			
			else {
				Render.text("your " + clitDescript() + " quivers in arousal");
			}
			Render.text(".\n\n");
			Render.text("Marble gives you a kinky look and coos, \"<i>Do you like what you see?  Because I sure do like what I see.</i>\"  Her hand goes to your crotch and delivers some teasing squeezes until you can't do anything but moan under her gentle fondling.  She smiles at you, happy to see the effect her efforts have on you.  \"<i>Relax, sweetie... I really believe you need some more love...</i>\"  You trill in excitement as she starts going down on you.");
			Render.text("\n\nThe contact of her hands has been replaced with a sensation of sweet warmth.  Marble effortlessly licks your groin, ");
			if(player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock()) Render.text("probing her tongue in your " + vaginaDescript(0) + ", licking you in the most appropriate places");
			else Render.text("wrapping her tongue around your rod of dickflesh, squeezing and pumping beads of pre-cum up your shaft");
			Render.text(".  She's doing an amazing job, and you thrash wildly under her smothering tongue assaults.");
			//([vag only/has a cock]
			Render.text("  Marble keeps ");
			if(player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock()) Render.text("poking your cunt");
			else Render.text("suckling the juices out of your urethra and literally absorbing the drops of pre-cum as they spurt out of the tip of your junk.");
			//[if multicock]
			if(player.lowerBody.cockSpot.count() > 1) Render.text("  She alternatively sucks and jerks every cock you have, making sure there isn't a single meat-tower that isn't being taken care of.");
			if(player.lowerBody.balls > 0) Render.text("  Her hands softly rub your [balls], making them churn and swell in pure arousal.");
			...
			if(player.gender == 2) Render.text("tongue-fuck");
			else Render.text("deepthroat");
			Render.text(" you.  Your hips thrust back and forth, enjoying the fluid motion of her tongue working over your wet, saliva-slathered genitals.");
			//[if cock] 
			if(player.lowerBody.cockSpot.hasCock()) {
				Render.text("  Her lips tightly enclose your " + cockDescript(0));
				if(player.lowerBody.cockSpot.count() > 1) Render.text(" while she strokes another");
				Render.text(", acting as an organic cock-ring.  She looks up to you as you watch her lips undulating under the repeated pulsations of your spooge-pumping " + cockDescript(0) + ".  These cocksuckers are teasing you in the most infuriating way, and you savagely shove more of your prick down her throat, stretching her jaw and making her moan in delight, although her cries of pleasure are muffled by the pecker stuffing her mouth.");
			}
			//[if vagina]
			else Render.text("  Her flexible, yet firm tongue really feels like a dong and it keeps provoking you with its prolonged digging. With a mighty thrust, you shove her against your " + vaginaDescript() + ", forcing the entirety of her oral muscle inside.");
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				...
				if(player.perks.has("MarblesMilk")) Render.text("usually");
				else Render.text("used to");
				Render.text(" suckle her tits.");
				//[if high cum production] 
				if(player.cumQ() >= 500) Render.text("  Your " + cockDescript(0) + " keeps spouting more seed inside her, efficiently distending her belly");
				//[if cum production is massive]until she looks ready to give birth.[/]
				if(player.cumQ() > 1500) Render.text(" until she looks ready to give birth.");
				Render.text("  Her lips cling to your cum-tube as if her life depended on it; she sucks viciously, milking your junk of every drop of its baby-batter.");
			}
			//[if vagina]
			...
			Render.text("\"<i>You feeling antsy, sweetie?</i>\" Marble asks as you walk up to her.  \"<i>Don't worry; I'm ready and able to help.</i>\"  ", true);
			...
			if(player.stats.cor + player.statusAffects.get("Marble").value4 < 66) {
				Render.text("The two of you find a secluded place in the camp and slip off your clothes.\n\n", false);
			} 
			else if (player.stats.cor + player.statusAffects.get("Marble").value4 < 132) {
				Render.text("The two of you strip down right there in the middle of camp.\n\n", false);
			} 
			else {
				Render.text("You both look around for another member to join in. No one jumps at the offer, so the two of you strip down.\n\n", false);
			}
			//all the foreplay here
			Render.text("Marble smiles at you and playfully pushes her sizable breasts together a few times before ", false);
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) {
				Render.text("pressing them against your own " + biggestBreastSizeDescript() + ".  She giggles slightly as she puts one of them between her mounds and rubs them up and down around it, teasing your " + nippleDescript(0) + " with her own " + marbleNips() + ", a trickle of milk leaking out onto your locked chests.", false);
			} else {
				Render.text("pressing them against your chest.  She rubs them up and down against you as she moves around, making sure she has rubbed each part of your upper body at least three times over.  She is especially happy with moving your head around between her breasts.", false);
			}
			...
			if(player.lowerBody.cockSpot.count() > 0) {
				...
				if (player.lowerBody.cockSpot.count() > 1) {
					Render.text("  Yet still unsatisfied, Marble engulfs and rub the rest of your " + multiCockDescriptLight() + " within her ample mounds.  ", false);
				}
			}
			...
			if(player.lowerBody.cockSpot.count() > 0) {
				...
				if(player.cumQ() < 25) Render.text("  In moments, a small amount of your cum is covering Marble's breasts.", false);
				else if(player.cumQ() < 250) Render.text("  After a few moments, a fair amount of your cum is covering Marble's breasts and face.", false);
				else if(player.cumQ() < 500) Render.text("  You keep cumming and cumming, until eventually Marble is completely drenched in your cum.", false);
				else Render.text("  Your seemingly endless orgasm eventually results in the two of you becoming completely drenched in your cum, as well as a large part of the ground around you.", false);
			}
			...
			if(player.lowerBody.vaginaSpot.count() > 0) {
				...
				if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.WET) Render.text("  As you recover, you see that a small amount of your girly fluids has leaked onto Marble's breasts.", false);
				else if(player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.SLICK) Render.text("  As you recover, you see that Marble has been covered fairly liberally with your girly fluids.", false);
				else if(player.vaginas[0].vaginalWetness <  5) Render.text("  As you recover, you see that Marble is decently drenched in your girly fluids.", false);
				else Render.text("  It takes you a minute to recover, but upon doing so, you blush and realize just how soaked with fluid you've become.", false);
			}
			//finally some philosophy on what an orgasm would feel like for someone who is genderless
			if(player.gender == 0) Render.text("  It's not quite the same as when you had genitals, but it is undeniably an orgasm.  While it is essentially a pointless orgasm, it's also almost pure in a way.  There is not a feeling inside you but the explosion of pleasure.", false);
	...
	//NAGAS
		...
		if(player.stats.lib >= 50) Render.text("  Quietly, you reach down and hook your fingers into the hem of her clothing, allowing her to pull it half off with her next shift and expose her pussy");
		if(flags[FlagEnum.MARBLE_DICK_LENGTH] > 0) Render.text(" and " + marbleCock());
		Render.text(".  \"<i>Why, you shameless " + player.mf("pervert","hussy") + "!</i>\" the cow-girl says in mock-offense, winking at you.");
		...
		if(player.gender > 0) {
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(vaginaDescript());
			Render.text(".  \"<i>Well, sweetie, looks like I don't have to go digging for treasure; it's already right here for me,</i>\" she whispers to you.  With one hand she ");
			if(player.lowerBody.cockSpot.hasCock()) Render.text("grabs your shaft");
			else Render.text("penetrates your pussy");
			Render.text(", while using her long tongue to lick the ");
			if(player.lowerBody.cockSpot.hasCock()) Render.text("tip");
			else Render.text("clit");
			Render.text(".  You cry out in pleasure, and start to wrap your long tail about Marble's shapely body.  You can't hold back for long, and soon reach orgasmic bliss, all while tightly clinging to your cow-girl lover.");
			if(player.lowerBody.cockSpot.hasCock()) Render.text("  [EachCock] unleashes a load of semen right into her mouth, which she swallows with pink-faced, wanton gusto.");
			if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Your [pussy] runs and squirts as you moan, drooling your juices into the little valley where Marble's breasts press against your scales.");
		} 
		else {
...
private rapeDAHMARBLEZ():void {
	...
	Render.text("", true);
	Render.text("You decide that rather than helping her, you are going to roughly manhandle her breasts and rape her.  You suddenly grab at her breasts and squeeze them roughly, at which point she screams and ", false);
	...
	if(raped) {
		...
		//wrapping things up
		Render.text("You close your eyes and revel in the moment before feeling Marble roll over and grab one of your hands.  You open your eyes just in time to see a big hammer-head flying towards your face...  ", false);
...
private marbleRapeCorruptFantasy():void {
	spriteSelect(41);
	Render.text("Marble is helpless before your onslaught with your superior position, and you find it immensely enjoyable to have someone trapped under you like this.  You start to fantasize just what it would be like if everyone were like this to you, just from being in your presence.  You imagine a sea of asses and pussies all stuck up in the air for you to rape at your leisure, and none of the owners able to do a damn thing about it.\n\n", false);
	//do they really want to have this fantasy?  How far are they gone?
	if(player.stats.cor>=66) {
		Render.text("You shake your head, clearing the bizarre fantasy from your mind and returning to the more immediate enjoyment.\n\n", false);
	} 
	else {
		Render.text("You smile to yourself and think, \"<i>One day,</i>\" before returning to the more immediate enjoyment.\n\n", false);
	}
}
...
public marbleAfterRapeBattle():void {
	spriteSelect(41);
	Render.text("While exploring the farm, you notice the cow-girl that hit you earlier, Marble, coming out of the barn.  You could try confronting her if you want to, or you could just avoid her from now on.", true);
...
private marbleAfterRapeYes():void {
	spriteSelect(41);
	//If choice was yes 
	Render.text("Deciding to deal with her, you move towards the barn.  However, Marble spots you on your way over and quickly disappears inside.  Just as you get to the entrance, she re-emerges with a large two handed hammer in hand.  \"<i>Leave right now, or this hammer is going into your head,</i>\" she tells you with an angry look in her eyes and drops into a combat stance.  Will you fight her?", true);
...
private marbleAfterRapeStartFight():void {
	spriteSelect(41);
	Render.text("You drop into your own combat stance; it's time to get even with her for last time.  ", true);
...
private marbleAfterRapeNo():void {
	spriteSelect(41);
	Render.text("You shake your head. It's just not worth the headache to deal with this cow.  You turn around and leave; you aren't going to be seeing her anymore.", true);
	doNext(camp.returnToCampUseOneHour);
}
	...
	if (pregnancy.type == PregnancyType.PLAYER) {
		...
		if(flags[FlagEnum.MARBLE_PURIFIED] > 0 && rand(2) == 0)
		{
			...
			if (flags[FlagEnum.MARBLE_BOYS] ==0)
			{
				...
				Render.text("You ask her why she hesitated like that.  \"<i>Oh?</i>\" she starts, looking up at you, \"<i>Sorry sweetie, it's just that I never thought I would give birth to a boy.  It just never occurred to me.</i>\"  Then her expression changes.", false);
			}
			else
			{
		...
		else
		{
			...
			if(flags[FlagEnum.MARBLE_DICK_TYPE] > 0) Render.text(", but you notice that she does not have a cock of any kind. It seems that trait isn't passed on", false);
			Render.text(".  The little girl's face is a really pretty one; you're sure that she'll grow up to be like her mom.  You hand Marble the child and she puts the crying child to her chest. The little girl stops crying at once and starts eagerly gobbling down Marble's milk.\n\n", false);
			//If (PC is addicted to Marble)
			if(player.perks.has("MarblesMilk")) {
				Render.text("\"<i>Don't worry sweetie,</i>\" Marble tells you, \"<i>somehow I know that she won't get addicted.\"</i>  ", false);
			} 
			else {
				Render.text("\"<i>Oh my,\"</i> Marble says to you, \"<i>It's just as wonderful as when you suckled me when my milk was addictive; I'd forgotten the feeling.\"</i>  ", false);
			}
			Render.text("Marble looks at her other breast a moment before looking at you and saying \"<i>Still, I think I could use you on the other side.</i>\" You oblige her by suckling her other breast.\n\n", false);
			//since the PC must either be addicted, or have removed Marble's addictive trait in order to father a child with her, there is no need for a check for addiction here!
			Render.text("Once the two of you have had your fill, Marble puts the child inside the nursery.  The little girl is already starting to look like she is a few years old, and is already trotting around on her little hoofs.  Marble turns to look at you and asks \"<i>Ok, I'll name her ", false);
			...
			if(flags[FlagEnum.MARBLE_KIDS] == 0) {
				...
				switch(temp) {
					case 0:
						Render.text("Aura", false);
						break;
					case 1:
						Render.text("Miran", false);
						break;
					case 2:
						Render.text("Lin", false);
						break;
					case 3:
						Render.text("Mary", false);
						break;
					case 4:
						Render.text("Bess", false);
						break;
					case 5:
						Render.text("Tina", false);
						break;
					case 6:
						Render.text("Rill", false);
						break;
					case 7:
						Render.text("Wendy", false);
						break;
					case 8:
						Render.text("Rainy", false);
						break;
					case 9:
						Render.text("Nicky", false);
						break;
				}
				Render.text(", yes, that's a good name for her.</i>\"", false);

			}
			else {
				Render.text("...hmm, well Sweetie, I can't think of a good name right now, I'll figure one out tomorrow.</i>\"", false);
			}
		}
...
public marbleNightSleepFlavor():boolean {
	...
	if(flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] == 0 && player.pregnancyType == PregnancyType.MARBLE && (player.pregnancyIncubation <= 128)) {
		Render.text("<b>Citing your pregnant belly, Marble informs you she'll be getting to work on building a nursery for your coming cow-child soon.</b>\n\n", false);
		flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION]++;
	}
	//PREGGO ALERTZ
	switch (pregnancy.eventTriggered()) {
		case 2: Render.text("<b>You notice that Marble seems to have gained some weight.</b>\n\n", false); //3 days in
				break;
		case 3: Render.text("<b>Marble's belly has gotten a fair bit bigger; she may be pregnant.</b>\n\n", false); //8 days in
				break;
		case 4: Render.text("<b>Marble's belly has gotten bigger; she is obviously pregnant.", false); //12 days in
				...
				if (flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] == 0 && pregnancy.type != PregnancyType.OVIELIXIR_EGGS) {
					flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION]++;
					Render.text("  She lets you know that she'll be busy working on a nursery for her child for a while.", false);
				}
				Render.text("</b>\n\n", false);
				break;
		case 5: Render.text("<b>Marble's pregnancy has advanced further still, though the structure of her body keeps it from slowing her down.</b>\n\n", false); //18 days in
				break;
		case 6: Render.text("<b>Marble is probably getting close to giving birth, as her belly has gotten very large.</b>\n\n", false); //24 days in
		default:
	}
	...
	if(player.gender == 0) {
		Render.text("As you and Marble lie down for the night, she whispers into your ear, \"<i>Sweetie, if it isn't too much trouble, I'd really like it if you would grow a penis for me.  If you can't do that, then could you at least get a vagina?  I'll understand if it's too much trouble, I just wanted to get that off my chest...</i>\" After a moment of getting comfortable, the two of you go to sleep.\n\n", false);
		return false;
	}
	else if(flags[FlagEnum.MARBLE_LUST] < 50 || player.gender == 0) {
		Render.text("As you go to lie down for the night, you find that Marble has gotten there before you.  ");
		if(flags[FlagEnum.MARBLE_LUST] < 0) Render.text("She makes a point of turning to face away from you when you climb into your bedroll.  Looks like she's still annoyed at you.");
		else if(flags[FlagEnum.MARBLE_LUST] < 20) Render.text("She lies next to you contentedly when you climb into your bedroll.");
		else if(flags[FlagEnum.MARBLE_LUST] < 30) Render.text("Marble gives a soft sigh when you climb into your bedroll.");
		else if(flags[FlagEnum.MARBLE_LUST] < 40) Render.text("Marble fidgets around a bit when you climb into your bedroll.");
		else Render.text("Marble seems rather worked up when you climb into your bedroll.");
		if(player.lust < 33 && flags[FlagEnum.MARBLE_LUST] < 20) Render.text("  After a few moments of getting comfortable, you drift off to sleep.");
		else if(player.lust < 33) {
			Render.text("  You wonder for a moment if Marble is maybe looking for something more from you.");
			//(50% chance of this happening)
			if(rand(2) == 0) {
				marbleCuddlin();
			}
			else Render.text("  However, you're not aroused enough to consider sex.");
		}
		else {
			Render.text("  Do you ask Marble if she'd like to have sex?");
			//[Ask][Don't Ask]
	...
	//Replaces laying down texts when Marble is at or over 50 lust
		...
		Render.text("You lie down and notice that Marble hasn't gone to bed yet.  You decide not worry about it too much until you hear Marble come clopping up outside.  \"<i>Sweetie?</i>\" she calls out to you as she crawls in next to you.  \"<i>Sweetie, I'm feeling really horny ");
		//if (PC's lust is under minimum to request sex)
		if(player.lust < 33) {
			Render.text("and I know you really aren't in the mood, but could you please help me out?</i>\"  You sit up and ask her what it is.  She looks at you slightly embarrassed before asking you, \"<i>Could you please ", false);
			//If (Marble is a herm)
			if(flags[FlagEnum.MARBLE_DICK_TYPE] > 0)
				Render.text("suck my " + marbleCock() + "?  I really, really need it.</i>\"", false);
			else
				Render.text("lick my womanhood?  I really want to feel your tongue bring me to release...</i>\"", false);
			...
			Render.text(".  I can tell you want this too, so can we please have sex?</i>\"  You sit up and consider her request.  Will you have sex with Marble?  If not, you could always offer to give her oral instead.");
...
private marbleCuddlin():void {
	Render.text("  Before too long, Marble reaches over and ", false);
	...
	if(!pcPregWithMarblesKids() && !marblePregWithPCKids()) {
		...
		if(player.tallness <= 78) Render.text("pulls your head on top of her chest.  She idly strokes your " + hairDescript() + " and says, \"<i>Sweetie, how about I be your pillow for the night?</i>\"  You happily agree, and soon you drift off to the sound of her steady breathing while you lay between her sizable breasts.", false);
		else Render.text("wraps her arms around you, gently pulling you against her.  \"<i>Sweetie, do you mind if we stay like this for a while?</i>\"  There is no way you could refuse, and before long, the two of you drift off to sleep in each other's arms.", false);
	}
	//Preg-cuddles
	else {
		//if (only Marble is heavily pregnant)
		if(marblePregWithPCKids() && !pcPregWithMarblesKids())
			Render.text("gently pulls your head to her swollen belly.  \"<i>Listen, sweetie,</i>\" she whispers, \"<i>you can hear our child's heartbeat when it's quiet like this...</i>\"  You hear a gentle thump accompanying Marble's own powerful heartbeat, along with your child's occasional squirms inside Marble.  You sigh and snuggle up against Marble's chest, rubbing her belly with your hand before drifting off to sleep.");
		//PC be preggers only
		else if(pcPregWithMarblesKids())
			Render.text("gently pets your swollen belly.  \"<i>Sweetie,</i>\" she whispers, \"<i>I can feel our child.</i>\"  She puts her furry ears to your growing belly, gently rubbing it all the while.  After a few moments she snuggles up against your body and the two of you drift off to sleep.");
		else
			Render.text("pulls your hand to her belly, and puts her own onto yours.  \"<i>The miracle of two lovers carrying each other's child,</i>\" she says.  It certainly isn't something you ever expected to happen, but that doesn't make it any less wonderful.  The two of you drift off to sleep together, hands on each other.");
	}
}
//Ask for sex
private atNightAskMarbleForSomeSexMaybe():void {
	clearOutput();
	Render.text("You turn to face Marble, and ");
	//if (Marble's lust is negative)
	if(flags[FlagEnum.MARBLE_LUST] < 0) {
		Render.text("see the annoyed back of her head.  Oh right, she's probably not in the mood for this.  You'll have to find another way of satisfying your needs tomorrow.");
	}
	else if(flags[FlagEnum.MARBLE_LUST] < 20) {
		Render.text("gently ask her if she wants to fool around.  After a moment you hear her breathe out and she says, \"<i>Sorry sweetie, I'm not feeling up to it right now.</i>\"");
		//Marble cuddles the PC
		marbleCuddlin();
	} 
	else {
		Render.text("gently ask her if she wants to fool around.  After a moment you hear her breathe out and, with a lewd voice, she says, \"<i>I was worried you'd never ask, sweetie...</i>\"");
		//trigger Marble sex
...
private marbleNightSexIntro(clear:boolean = true):void {
	...
	//No sex if dude and too big!
		...
		Render.text("You lie down next to her, but you can't help wishing that the two of you could have sex.  Suddenly, you feel Marble's hand grab a hold of your " + cockDescript(x) + ".  She sighs and says, \"<i>Sweetie, I'm afraid that you won't fit inside me.  Do you think you could get a little smaller, just for me?</i>\"  After a moment of getting comfortable, the two of you go to sleep.", false);
		doNext(playerMenu);
	}
	//Ok lets light this fire!
	Render.text("At that moment, you feel Marble breathe in your ear, \"<i>I know you want this, don't you sweetie?</i>\"\n\n", false);
	//If (PC is under 5 feet tall) {
	if(player.tallness < 60) {
		Render.text("Marble rolls you onto your back and pulls your head up onto her ample cleavage.  \"<i>I want it too. Just relax and let me take care of this.</i>\"  Marble keeps your head locked in her cleavage as she uses her other hand to slowly free the two of you of your garments.\n\n", false);
		Render.text("Your head rests comfortably between Marble's breasts the entire time, her warm breath blowing softly on your hair.  You feel almost completely relaxed, yet incredibly turned on at the same time.  For some reason, it just feels right to lay there and breathe in her scent, enjoying the wonderful love pillows she is forcing you to relax between.  She runs her hand down your " + biggestBreastSizeDescript() + ", gently cooing to you.  Her hand slips down to your ", false);
		//If (PC has dick) {
		if(player.lowerBody.cockSpot.count() > 0) {
			Render.text(multiCockDescriptLight(), false);
		...
		Render.text(" and gently caresses you.  She whispers into your ear, \"<i>Soon sweetie, soon we'll both get what we want.</i>\"\n\n", false);
		Render.text("Finished with removing your clothes and her own, Marble releases her grip on your head and turns your body over.\n\n", false);
	} 
	else 
	{
		Render.text("She reaches over and puts her hand onto your waist, gently slipping her hand inside your undergarments to touch your ", false);
		//If (PC has dick) {
		if(player.lowerBody.cockSpot.count() > 0) {
			Render.text(multiCockDescriptLight(), false);
			//If (PC both dick and vag) 
		...
		Render.text(" while you slip your own hand into her clothes and ", false);
		//If (Marble is a herm) {
		if(flags[FlagEnum.MARBLE_DICK_TYPE] > 0) {
			Render.text("gently rub her " + marbleCock() + ".  After a moment, you move your hand down lower and ", false);
		}
		Render.text(" run your fingers down her moist womanhood.  The two of you quiver under each other's ministrations.  Both of you work to quickly free yourselves from your clothing, growing increasingly excited at where things are going.  Free of your garments, you move overtop of Marble.\n\n", false);
	}
	//If (player is a herm, and their dick can fit inside Marble)
	if(player.gender == 3) {
		if(player.cockArea(x) <= marbleCuntCapacity()) {
			Render.text("\"<i>So sweetie, what part will you be using today?</i>\" she asks you.", false);
			...
			Render.text("Marble says, \"<i>Well sweetie, since that " + cockDescript(x) + " of yours is too big to fit inside me, let's put that " + vaginaDescript(0) + " to use.</i>\"\n\n", false);
...
private marbleNightSexDudes():void {
	...
	flags[FlagEnum.MARBLE_LUST] = 0;
	Render.text("", true);
	Render.text("You gently lift yourself up and slide the tip of your " + cockDescript(x) + " into her waiting hole.  Ever so slowly, you push yourself further and further in.  Each inch gained brings gentle moans and coos from Marble, encouraging you onward.  ", false);
	//Can the PC's main cock go all the way?
	//If (cock 0 is longer than 8 inches) 
	if(player.lowerBody.cockSpot.list[x].cockLength > 8) {
		Render.text("There is only one thing that will stop your advance: when you reach the end of the line.  \"<i>Keep going sweetie, there is room left.  Make sure you fill me all the way.</i>\"  With a groan, your " + cockDescript(x) + " hits the furthest part of Marble's confines and you can go no further.\n\n", false);
	}
	//otherwise, Marble doesn't mind, she'll hold all of them instead
	else {
		Render.text("Nothing will stop your advance; your cock will be fully enveloped inside Marble's body.  \"<i>That's right sweetie... Ah, let me fully envelop you.</i>\"  With a sigh of satisfaction, your bodies come completely together.\n\n", false);
	}
	Render.text("You look Marble in the eye and the two of you relish the moment; you are two lovers, connected in the most enjoyable of ways.  ", false);
	...
	if(player.stats.cor + player.stats.lib < player.statusAffects.get("Marble").value4 + 10) {
		...
		//Fun, the PC's breasts fall onto Marble's Face!  If they have them, anyway...
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 4) {
				Render.text("landing your " + biggestBreastSizeDescript() + " into Marble's face!  You hear a slight muffled cry underneath you as Marble takes her hands off your hips and lifts you up off of her.  ", false);
			} 
			else {
				Render.text("crashing down on Marble and doing a good job of flattening her breasts under your size.  \"<i>Hey!</i>\" she cries out, letting go of your hips to lift your body off of her.  ", false);
			}
		}
		Render.text("You look down at her annoyed expression, before grinning at her and beginning to impale her moist and wonderful snatch yourself.  In a moment, her expression changes from a pouty face to one of ecstasy.  The two of you are nearing your peaks now.\n\n", false);
	}
	//nope the PC is the one
	else 
	{
		Render.text("It isn't enough to hold you back for more than a moment, though, and soon you're thrusting deep inside her at a comfortable rhythm for the two of you.  You smile at her and she smiles back as you start fondling and playing with Marble's lovely pillows.  She starts to run her fingers onto her love button, crying out in pleasure as you continue to thrust your " + cockDescript(x) + " inside her.\n\n", false);
		//If (PC's breasts are B cups or bigger)
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) Render.text("You pull your hands back up and begin to run them over your " + biggestBreastSizeDescript() + ", toying and teasing your nipples as your " + cockDescript(x) + " continues to pound away at Marble's confines.  Marble soon has her hands on the breasts you vacated, clearly enjoying teasing herself just as much as you are.  The two of you cry out in pleasure from the stimulation; you can feel your orgasm fast approaching.\n\n", false);
		else Render.text("The sensations are quickly becoming too much for you, and you're unable to do anything but focus on the feeling of your " + cockDescript(x) + " inside her wonderful confines.  Marble grins at your difficulties and teases, \"<i>Am I just too much for you, sweetie?  Ah!</i>\" She gasps; it seems that you're becoming too much for her too.\n\n", false);
	}
	//ORGASMO
	Render.text("You gasp as you feel your insides churning and the walls of Marble's sex collapsing tightly down on you.  Once again, you've brought each other to the point of blissful orgasm around the same time.  ", false);
	//let's talk about the PC's cum production
	if(player.cumQ() < 250) {
		Render.text("Marble cries out in joy as your " + cockDescript(x) + " unleashes its load within her insides.  ", false);
	} 
	else {
		Render.text("Marble gasps in both pleasure and pain as you fill up her insides to almost bursting, and large amounts of your jizz spill out around her slit.  ", false);
	}
	//Now, does the PC have more than one cock?  Cum comes out of those too.
	if(player.totalCocks() > 1) 
	{
		Render.text("Of course, ", false);
		if(player.totalCocks() > 2) { 
			Render.text("the rest of your " + multiCockDescriptLight(), false);
		} 
		else { 
			Render.text("your other tool", false);
		}
		Render.text(" unleashes its own load, liberally covering both of you.  ", false);
	}
	//does Marble spray her cock?
	if(flags[FlagEnum.MARBLE_DICK_TYPE] > 0) {
		Render.text("Trapped between your bodies, Marble's " + marbleCock() + " spasms and sprays the two of you.", false);
	}
...
private marbleNightSexChicks():void {
	...
	Render.text("", true);
	if(flags[FlagEnum.MARBLE_DICK_TYPE] == 0) {
		Render.text("You gently lift yourself up as Marble produces her double dildo and slips it inside her womanhood.  Smiling at her, you slowly lower your body and impale your " + vaginaDescript(0) + " onto the artificial rod.  ", false);
		...
		if(player.stats.cor + player.stats.lib < player.statusAffects.get("Marble").value4 + 10) {
			...
			Render.text("\"<i>Sweetie!</i>\" Marble gasps as you start to play with her breasts.  ", false);
			//Does Marble help out, or does she go for the PC's breasts?
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 4) {
				Render.text("She reaches her hands up and starts to play with your " + biggestBreastSizeDescript() + ", flicking and toying with them at the same pace that you play with hers.  The two of you start a friendly little contest to see who can stimulate the other's chest better; all the while, you continue to rise and fall above her.\n\n", false);
			} 
			else {
				Render.text("Marble reaches her hands up and grabs onto yours, pulling them down tightly against her chest.  She gets you to play with each of her " + marbleNips() + " in turn, making sure that you flick them and tease them in just the right way to get her to cry out in ecstasy again and again.\n\n", false);
			}
			Render.text("Within moments, your rapid pistoning on the dildo is pushing the two of you closer and closer to the edge.  ", false);
		}
		Render.text("The two of you cry out in orgasm as your " + vaginaDescript(0) + " clamps down on one end of her dildo and her womanhood clamps down on the other end.  ", false);
		//Does this PC have a cock too?
		//If (PC has at least 1 cock)
		if(player.totalCocks() > 0) {
			Render.text("At the same time, your " + multiCockDescriptLight() + " sprays its own load onto Marble's chest and stomach.  ", false);
		}
		Render.text("The two of you give one more shudder from the wonderful stimulation, before collapsing on top of one another.  ", false);
	}
	//PC is female, Marble is a herm
	else {
		player.slimeFeed();
		Render.text("You gently lift yourself up into the air and guide your " + vaginaDescript(0) + " to the tip of her waiting " + marbleCock() + ".  You slowly lower your " + vaginaDescript(0) + " onto her quivering " + marbleCock() + ", gaining inch by inch ", false);
		//is she satisfied?
		...
		if(player.vaginalCapacity() > flags[FlagEnum.MARBLE_DICK_LENGTH] * flags[FlagEnum.MARBLE_DICK_THICKNESS]) {
			Render.text("until she is fully within you.  The two of you shudder in pleasure, and Marble says, \"<i>Oh sweetie, you got it all in there.  Being inside another woman like this is something I'll never really get used to, but I can't deny how good it feels.</i>\"\n\n", false);
		}
		//Marble seems to think you can take more than maybe you should be able to take
		else {
			Render.text("until you bottom out and can take no more of her.  \"<i>No more?</i>\" Marble asks you a little sadly, \"<i>You sure you can't take a little more?</i>\" she adds as she starts pushing you down onto her " + marbleCock() + ". You are forced to stretch to accommodate more of her and you shudder slightly from the strain.  ", false);
			//(Message about vagina becoming looser)
			Render.text("Marble grins at you and says, \"<i>See?  I knew you that you could fit a little more.</i>\"\n\n", false);
		}
		Render.text("After adjusting to the feeling of her inside you, you can't deny what a wonderful feeling it is to be connected like this.  ", false);
		//Does Marble take the initiative?
		if(player.stats.cor + player.stats.lib < player.statusAffects.get("Marble").value4 + 10) {
			...
			Render.text("\"<i>Oh sweetie, am I going too fast for you?  Here, let me give you a chance to catch your breath.</i>\"  She offers as she slows down her rapid movement, letting your mind settle back into place.  ", false);
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 4) {
				Render.text("Marble takes her hands off your hips and lets you start to rise and fall on her in your own time.  She still rises up to meet you as you descend and lowers herself down each time you rise, matching your timing, but her hands have a much more pressing concern: your " + biggestBreastSizeDescript() + ".  Marble starts working her hands expertly to tease and play with your " + biggestBreastSizeDescript() + ". She seems to know all the right ways to move her fingers, clearly having a lot of experience with her own, and you can't help but moan at the attention.  She giggles and pants in pleasure herself; it seems she may be enjoying this play just as much as you!\n\n", false);
			} 
			else {
		...
		//no, no, the PC is much hornier than she is
			...
			Render.text("Within moments, your rapid pistoning on her " + marbleCock() + " is pushing the two of you closer and closer to the edge.  ", false);
		}
		Render.text("The two of you cry out in orgasm as your " + vaginaDescript(0) + " clamps down on her " + marbleCock() + " as it fills you with its seed.  ", false);
		//Does this PC have a cock too?
		if(player.totalCocks() > 0) {
			Render.text("At the same time, ", false);
			if(player.totalCocks() > 1) Render.text("each of ", false);
			Render.text("your " + multiCockDescriptLight() + " sprays its own load onto Marble's chest and stomach.  ", false);
		}
		Render.text("The two of you give one more shudder from the wonderful stimulation, before collapsing on top of one another.  ", false);
...
private marbleNomNoms():void {
	...
	//Render.text("You go to lie down and notice that Marble hasn't gone to bed yet.  You decide not worry about it too much until you hear Marble come clopping up beside you.  \"<i>Sweetie?</i>\" she calls out to you as she crawls in next to you.  \"<i>Sweetie, I'm feeling really horny and I know you really aren't in the mood, but could you please help me out?</i>\"  You sit up and ask her how you can help; you could never refuse her.  She looks at you slightly embarrassed before asking you, \"<i>Could you please ", false);
	//If (Marble is a herm) {
	if(flags[FlagEnum.MARBLE_DICK_TYPE] > 0) {
		player.slimeFeed();
		//Render.text("suck my " + marbleCock() + "?  I really, really need it.</i>\"", false);
		if(player.stats.cor < 30) {
			Render.text("You aren't really comfortable about doing this, but you can't really turn Marble down.  Especially after you were the one to give her that " + marbleCock() + ".\n\n", false);
		}
		else {
			Render.text("Sucking her tasty-looking " + marbleCock() + " sounds like it would be a lot of fun and you have no problem agreeing.\n\n", false);
		}
		//sucking her cock scene
		Render.text("Marble lifts up her skirt and gives you a good look at her very erect " + marbleCock() + ".  You position yourself overtop of her and take a deep breath, taking in her smell.  ", false);
...
public MarbleDigsDraftsYo():void {
	...
	Render.text("You hand Marble the bottle.  She looks at it for a moment before ", false);
	if(player.statusAffects.get("Marble").value4 > 60) {
		Render.text("giving you a smile and saying, \"<i>So you want me to partake in corruption, and to have a nice cock to stick my sweet with?</i>\"  You tell her that the bottle has been purified, so it won't give corruption, but otherwise, you're hoping it gives her the <i>additional</i> effect.  She grins at you and downs the bottle.\n\n", false);
	} 
	else {
		Render.text("looking at you uncertainly and saying, \"<i>Uh, sweetie, I'm not going to drink this demon stuff.  It'll warp my body, and I think this will give me a cock...</i>\"  You assure her that the draft has been purified, so it won't warp her body or corrupt her... aside from that last effect she mentioned, but that's what you want her to get.  She sighs before yielding, and says, \"<i>Ok sweetie, if you really want me to have one, I'll take it for you.</i>\"  She takes a deep breath before drinking the bottle, and grimaces at the taste.\n\n", false);
	}
	Render.text("Marble drops the potion and grabs at her crotch.  Turning away from you, she gives an excited cry that is a mix between a moo and a moan as her body convulses slightly.  After a moment, she turns back to you and shows her new 7\" long and 2\" wide human cock.  She gives you a ", false);
	if(player.statusAffects.get("Marble").value4 > 50) Render.text("big ", false);
	else Render.text("nervous ", false);
	Render.text("smile.  <b>Marble is now a hermaphrodite.</b>  ", false);
	doNext(camp.returnToCampUseOneHour);
...
//removes her dick, no other effect.  Can only be given if Marble has a dick.
	Render.text("You hand Marble the pink egg.  She looks at it thoughtfully for a moment before her eyes light up in recognition.  \"<i>This is one of those magic eggs made from those egg elixirs.  If I remember right, this one removes the male traits of those who eat them.  ", false);
	if(player.statusAffects.get("Marble").value4 > 50 ) {
		Render.text("So my sweet, tired of my cock?  Well, what if I like it?  Do you really want me to get rid of it?</i>\"  You assure her that yes, you do want her to get rid of it.  She frowns at you in annoyance, but ultimately agrees to take the egg.  \"<i>Eventually you'll want it back, I'm sure of it,</i>\" she says just before eating the egg.\n\n", false);
	} 
	else {
		Render.text("So does this mean you don't want me to have a cock anymore, sweetie?</i>\"  You assure her that yes, you don't want her to have it anymore.  She seems relieved by this and admits, \"<i>Thank you sweetie, I don't think I really liked having it,</i>\" before eating the egg.\n\n", false);
	}
	Render.text("She lifts up her skirt and the two of you watch as her " + marbleCock() + " is absorbed back into her body.  <b>Marble is now purely female.</b>  ", false);
...
public marbleOvulatesLikeMadDawg():void {
	...
	if (flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] < 100 && pregnancy.type == PregnancyType.PLAYER) {
		Render.text("Marble refuses to take the elixir to speed up her pregnancy until after she has finished the nursery. ", false);
		doNext(camp.returnToCampUseOneHour);
		return;
	}
	else if (pregnancy.isPregnant) {
		Render.text("After taking the elixir, Marble's belly rumbles slightly.  You can both tell that her pregnancy has advanced.  ", false);
		...
		Render.text("After taking the elixir, Marble tells you that her belly feels full.  It sounds like she'll be laying eggs soon.  ", false);
...
private giveMurbleProBova():void {
	...
	if(flags[FlagEnum.MARBLE_BOVA_LEVEL] == 0) {
		Render.text("You hand Marble the bottle of ProBova.  She considers it for a few moments before looking at you and saying, \"<i>Judging by the label, this is probably going to make me even more cowlike then I already am, sweetie.  I'm not really opposed to the idea, but it will be permanent.  Are you sure you want me to drink it?</i>\"");
		//player chooses yes/no
		doYesNo(giveMarbleTheProBovas4Sho, giveItem);
	}
	else {
		Render.text("You hand Marble another bottle of the ProBova.  She studies your face seriously for a few moments before saying, \"<i>Now, you're absolutely sure that you want me to drink this?  Remember that whatever happens will likely be permanent.</i>\"");
		//player chooses yes/no
		doYesNo(giveMarbleTheProBovas4Sho, giveItem);
	}
}
...
public giveMarbleTheProBovas4Sho():void {
	...
	if(flags[FlagEnum.MARBLE_BOVA_LEVEL] == 0) {
		Render.text("Marble nods and downs the contents.  Nothing happens for a moment... then she gasps and grabs at her chest, stumbling forward slightly.  The cow-girl straightens up and releases her grip, then pulls open her top to look at her breasts.  <b>Each is now decorated with sets of four nipples, like the teats of a cow.  She has also gained about 4 inches in height, judging against the backdrop of the camp.</b>  Marble takes a few minutes to test her new nipples, squeezing them gently and sighing as dribbles of milk decorate her areolae in fours instead of one, then looks at you and says, \"<i>This isn't really so bad.  Actually, it feels nice.  If you find another dose, I'm willing to drink it - just to see what happens, of course.</i>\"");
...
private marbleAppearance():void {
	...
	Render.text("Marble is a ");
	if(flags[FlagEnum.MARBLE_BOVA_LEVEL] == 0) Render.text("6 foot 4 ");
	else if(flags[FlagEnum.MARBLE_BOVA_LEVEL] == 1) Render.text("6 foot 8 ");
	else Render.text("7 foot 4 ");
	Render.text("inch tall cow-girl, and while she isn't very fast, she has very strong muscles; though they are difficult to spot under her skin's layer of fat. <b>She is currently wearing farm-hand clothes and using a large two-handed hammer as a weapon.</b>  ");
	if(flags[FlagEnum.MARBLE_BOVA_LEVEL] <= 1) Render.text("She has a fairly normal soft human face, with light-colored skin.  ");
	else Render.text("She has a soft face that is a mix of bovine and human features, with thick brown fur covering her body.  ");

	Render.text("Her shoulder-length brown hair is parted by a pair of rounded cow-ears that stick out sideways from her head.  ");
	
	if(player.statusAffects.get("Marble").value4 <= 20) Render.text("Two small horns grow from her forehead, similar in size and appearance to those on a young female bovine.  ", false);
	else if(player.statusAffects.get("Marble").value4 <= 50) Render.text("Two medium sized horns grow from her forehead, similar in size and appearance to those on a female bovine.  ", false);
	else Render.text("Two fairly large horns grow from her forehead, similar in appearance to those on a female bovine.  ", false);
	Render.text("She has wide womanly thighs that draw the attention of those around her, and her large butt fills out her clothing nicely.  A long cow-tail with a puffy tip swishes back and forth between her legs, as if swatting at flies. A pretty bow has been tied to her tail.  Two legs grow down from her waist");
	
	if(flags[FlagEnum.MARBLE_BOVA_LEVEL] <=1) Render.text(", human until about half-way down her thigh.  The lower portion of her legs is covered in thick dark brown fur and ends in a pair of bestial hooves.\n\n",false);
	else Render.text(" that are oddly jointed and end in a pair of bestial hooves.\n\n",false);

	if(player.statusAffects.get("Marble").value4 <=15) Render.text("She has two pillowy breasts, each supporting a 0.4-inch lactating " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + " bra.\n\n", false);
	else if(player.statusAffects.get("Marble").value4<=25) Render.text("She has two large breasts, each supporting a 0.6-inch lactating " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + ".\n\n", false);
	else if(player.statusAffects.get("Marble").value4<=35) Render.text("She has two basketball-sized breasts, each supporting a 0.8-inch milk-seeping " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + " bra.\n\n", false);
	else Render.text("She has two basketball-sized breasts, each supporting a 1-inch milk-seeping " + marbleNip() + ".  She could easily fill an " + marbleBreastSize() + " bra.\n\n",false);
	...
	switch (pregnancy.event) {
		case 2: Render.text("It looks like she's put on some weight since coming to your camp.\n\n", false); //3 to 8 days in
				break;
		case 3: Render.text("Her belly has swollen a bit; she may be pregnant.\n\n", false); //8 to 12 days in
				break;
		case 4: Render.text("Her belly is obviously swollen; she is almost certainly pregnant.\n\n", false); //12 to 18 days in
				break;
		case 5: Render.text("Her belly is very swollen; she is very pregnant.\n\n", false); //18 to 24 days in
				break;
		case 6: Render.text("Her belly is extremely swollen and occasionally quivers when whatever she is pregnant with moves around.\n\n", false); //24+ days in
		default:
	}
	
	if( (flags[FlagEnum.MARBLE_PURIFICATION_STAGE] < 5 && flags[FlagEnum.MARBLE_LUST] >= 20) || (flags[FlagEnum.MARBLE_PURIFICATION_STAGE] >= 5 && flags[FlagEnum.MARBLE_TIME_SINCE_NURSED_IN_HOURS] >= 4) )
	{
		Render.text( "<b>Marble is fidgeting around uncomfortably, perhaps she needs to be milked?</b>\n\n" );
	}
	
	if(flags[FlagEnum.MARBLE_DICK_TYPE] > 0) Render.text("She has grown a " + marbleCock() + " since you brought her to camp. It is " + num2Text(int(flags[FlagEnum.MARBLE_DICK_LENGTH])) + " inches long and " + num2Text(int(flags[FlagEnum.MARBLE_DICK_THICKNESS])) + " inches thick.\n\n", false);
	
	if(player.statusAffects.get("Marble").value4<=50) Render.text("She has a pussy, with a 0.5 inch clit.\n\n", false);
	else if(player.statusAffects.get("Marble").value4 <=75) Render.text("She has a cunt, with a 0.6 inch clit.  You can see moisture gleaming from it.\n\n", false);
	else Render.text("She has a fuck-hole, with a 0.7 inch clit.  Moisture gleams in her cunt, its lips slightly parted.\n\n", false);
	//since I assume Marble may become ass fucked in the future, Boolean Marble.analVirgin:
	if(player.statusAffects.get("MarbleSpecials").value1 == 0) Render.text("She has one virgin bum-hole, placed between her squeezable butt-cheeks where, you presume, it belongs.", false);
	else Render.text("She has one bum-hole, placed between her squeezable butt-cheeks where, you presume, it belongs.", false);
	doNext(interactWithMarbleAtCamp);
}
...
private giveMarbleTailjobRelease():void {
	clearOutput();
	Render.text("You tell Marble that while you're not in a mood to suck her off today, you're not just going to leave her alone with it, either, and ask for her permission to do an exotic alternative.  Slowly, she nods, apparently not so sure about what you mean or whether she should agree.");
	...
	if(player.stats.cor > 70) {
		...
		if(player.tongueType == TONUGE.SNAKE || player.tongueType == TONUGE.DEMONIC) Render.text("  Your tongue runs along the length of the end of your tail, tasting both Marble's feminine secretions and her semen.  She gives you a smoldering gaze as you lick her juices up.  You grin at her.");
		//(Demon Tongue) 
		if(player.tongueType == TONUGE.DEMONIC) Render.text("  You decide to put on a show for Marble, moving your tail as you drop out a large piece of your inhumanly long tongue, licking up her secretions sensuously while staring at her.  She quickly blushes under your gaze.");
...
private milkMarble():void
{
	...
	if( player.lowerBody.vaginaSpot.hasVagina() && flags[FlagEnum.MARBLE_DICK_TYPE] > 0 && (player.isBiped() || player.isGoo()) ) addButton( 2, "Take Her", milkMarbleTakeHerDick );
...
private milkMarbleCunnilingling():void
{
	...
	Render.text( "You suggest that maybe she'd like it if you licked her vagina while being milked." );
	
	Render.text( "[pg]A huge smile spreads across her face, \"<i>Oh sweetie, that sounds like it would be wonderful!  You're the best; you know that?</i>\"" );
	
	Render.text( "[pg]She takes a seat on the edge of the chair in the middle of the room and spreads her legs wide.  Her skirt slips up her " );
	if( flags[FlagEnum.MARBLE_BOVA_LEVEL] < 1 ) Render.text( "semi-human" );
	else Render.text( "inhuman" );
	Render.text( " legs, giving you a clear look at her waiting womanhood.  With a coy sideways glance, she wiggles her fingers in invitation for you to go down on her." );
	
	Render.text( "[pg]In order to get the show on the road, you " );
	if( player.isBiped() ) Render.text( "get down on your knees, and slowly crawl forward" );
	else Render.text( "lower your body close to the ground, and slowly inch forward" );
	Render.text( " like an animal who's found something they want.  When you get close to her, your lover runs a finger up the inside of her thigh as her tail sneaks its way up around her waist and drapes itself over your prize." );
	
	Render.text( "[pg]Arriving at a hard brown hoof, you wrap your arm about her coarsely furred leg and rest your head against the bestial appendage.  You stroke and run your fingers through her fur to a soft giggle above you while the machine continues to pump and whirl.  You tease her a bit more before raising yourself up, bit by bit, until you're at eye level with her tail-covered opening. A small tuft of hair decorated with a pink bow is all that hides your goal.  No, it's a present, gift wrapped for you." );
	
	Render.text( "[pg]You take hold of two strands of the bow and tell Marble she shouldn't have given you such a gift." );
	
	Render.text( "[pg]\"<i>Oh, not at all sweetie.  You deserve it!</i>\"" );
	
	Render.text( "[pg]You slip the bow open, unwrapping your present, and pull her tail aside to reveal her moist and inviting opening and exclaim that it was what you always wanted!" );
	
	Render.text( "[pg]\"<i>Oh, I'm so glad you like it.  Why don't you try it out right now?</i>\"" );
	
	Render.text( "[pg]She doesn't have to give you the suggestion twice.  You dive in at once, sticking your [tongue] deep into her recesses, and tasting her cow-girl nectar.  Her folds accept you easily, and an eager moan slips out of the woman above you.  Her fingers run over your [hair] while you withdraw from your initial sortie to scout out the entrance more." );
	
	Render.text( "[pg]In response to your tugging and toying with her outer folds, Marble's hips jerk forward slightly, and you see her love button pulse and engorge itself.  There is a happy moan, and her tail manages to find itself flicking your face.  You swat the pesky wrappings away and continue your efforts at bringing your mate to the height of pleasure.  The next target: her clit." );
	
	Render.text( "[pg]There is a long drawn out cry of joy when you bring your [tongue] to the source of her womanly pleasure.  " );
	if( flags[FlagEnum.MARBLE_DICK_TYPE] > 0 ) Render.text( "With the first flick across its pink flesh, Marble grabs ahold of her "+ marbleCock() +", and tightly squeezes it, slowly pushing her hand down it." );
	else Render.text( "With the first flick across its pink flesh, Marble grabs onto an arm of her chair, gripping it tightly." );
	Render.text( "  You can't help but laugh and ask her if your treatment is too much, suggesting that maybe you should save playing more for later." );
	
	Render.text( "[pg]\"<i>No more teasing me, [name], bite me now!</i>\"" );
	
	Render.text( "[pg]How can you refuse such a request?" );
	
	Render.text( "[pg]\"<i>Ahhhhooooo!</i>\"  The wonderful bovine women gives a grateful cry of pleasure as your teeth nip into her nub of engorged skin and a burst of liquid sprays out onto your chin.  " );
	if( flags[FlagEnum.MARBLE_DICK_TYPE] > 0 ) Render.text ("Of course, there is also a spurt of white which sprays out over her lap and your head.  " );
	Render.text( "Obviously, while the milkers don't normally get her aroused, they can seriously amplify the effects of any pleasure she gets." );
	
	Render.text( "[pg]You ask your lover if that was good." );
	
	Render.text( "[pg]\"<i>Lovely. Thanks for giving me the best start to being milked sweetie.</i>\"  She notices that you're all wet, \"<i>Oh, here, let me get that for you.</i>\" and she wipes the fluid off your [face] with a grin." );
	
	Render.text( "[pg]You then tie her bow back up on her tail and say you'll save the rest for later." );
	
	Render.text( "[pg]\"<i>Oh, I'll be looking forward to it.  See you later sweetie!</i>\"  She gives you a slightly awkward peck on the cheek around the milking tubes, that sends you into a blissful state." );
...
private milkMarbleFuckDatCowPussy():void
{
	...
	Render.text( "You wonder aloud if maybe she'd be interested in a little intercourse action. You'd be perfectly prepared to provide for her." );
	
	Render.text( "[pg]\"<i>Hmm, well, your offer is tempting, but I'm going to have to see what's available before I make a final decision.</i>\"" );
	
	Render.text( "[pg]With a flourish, you remove your [armor] and do a little sweep over your " + cockDescript(x) + ", asking her if everything is to her needs." );
	...
	{
		Render.text( "[pg]The haze given off by Marble's tainted fluids alights a familiar craving at the back of your mind, but you push it back and reach your hands up to her breasts and pinch her nipples.  Marble tenses for a moment, and looks down at you confused.  You tell her that you'll try to keep her entertained while you wait for Whitney to fix things." );
		
		Render.text( "[pg]She begins speaking once more, then goes quiet when you pull on her nipples and try your best to milk her by hand, much the same way that the cows were milked back home.  While nothing actually comes out of her breasts from your touch, she gives an appreciative moan and resumes her rocking against your crotch." );
		
		Render.text( "[pg]After rubbing against you for a bit, the stimulation on your length just isn't enough for you anymore, and try to start thrusting up into your lover's passage.  Marble seems to be in agreement in what she wants, and her motions switch from a gentle rocking into meeting each of your thrusts with a pumping of her own, trying to drain your manhood while you try to drain her breasts." );
		
		Render.text( "[pg]Finally the two of you cry out together in orgasmic pleasure, and you spray out your seed into her passage, mixing with a rush of her own fluids into a slurry of sex juices sloshing around your " + cockDescript(x) + ".  The two of you settle down, with Marble laying down on you contentedly for a moment." );
		
		Render.text( "[pg]Suddenly a voice with a southern drawl call out from above you, \"<i>You kids havin a lots of fun down there?</i>\"" );
		
		Render.text( "[pg]The two of you stare at each other in surprise and tense up." );
		
		Render.text( "[pg]\"<i>Next time, please try to be a bit gentler on the equipment, if ya don't mind.  I'd rather not have to do fix it every time you two get frisky.</i>\"  There is a slam of metal on metal as Whitney finishes working on the broken milker and it whirls back to life." );
		
		Render.text( "[pg]You and Marble each look each other in the eye and burst out laughing.  You then help her reattach the cups and excuse yourself.  Just as you're leaving, Marble blows you a kiss and says, \"<i>We should definitely do this again sometime.  Let's just give Whitney a break first.</i>\"" );
...
private milkMarbleTakeHerDick():void
{
	...
	Render.text( "You ask her if maybe she wants to penetrate you while she is using that milking machine." );
	
	Render.text( "[pg]She looks thoughtful for a moment, tapping her finger against her lips while her tail swishes back and forth.  With the tubes coming off of her nipples, it actually looks rather comical." );
	
	Render.text( "[pg]Spotting your look, she frowns at you, \"<i>Oh, you think this is silly do you, [name]?  I'll have you know that I'm seriously trying to figure out how to make this work.</i>\"  She indicates her rather physical attachment to the room, and the furnishings it has available to it." );
	
	Render.text( "[pg]You give it a little thought, then suggest that you could lean up against that railing while she fucks you from behind." );
	
	Render.text( "[pg]\"<i>Take you from behind, huh?  I'd really rather look into your eyes while I'm having sex, but there aren't many options here, are there?</i>\"" );
	
	Render.text( "[pg]To silence her complaining, you quickly strip" );
	if( player.tallness < 56 ) Render.text( ", grab a box," );
	Render.text( " and lean against the railing, then shake your [ass] in her direction with your vagina on full display, wet with arousal.  You look back at her to see if you're having the desired effect." );
	
	Render.text( "[pg]Marble is licking her lips with a nervous look on her face while holding her tail in her hand and running her fingers through it's brush.  \"<i>You make a very persuasive argument, sweetie.  Plus, if you're looking at me like that, I guess I really don't have anything to complain about, do I?</i>\"" );
	
	Render.text( "[pg]She strips off her short skirt and takes up a position behind you, hands on your hips and erect cock pressed against your entrance.  You look down between your legs and catch sight of her tail hanging down between her legs, the barest hint of movement to it.  \"<i>Well sweetie, are you ready for me?</i>\"" );
	
	Render.text( "[pg]You retort that if you weren't, you wouldn't have suggested this in the first place." );
	
	Render.text( "[pg]\"<i>Heh, good point.  Alright, here I go...</i>\"  She slowly works her way inside you, inch by inch.  " );
	if( player.vaginalCapacity() < flags[FlagEnum.MARBLE_DICK_LENGTH] * flags[FlagEnum.MARBLE_DICK_THICKNESS] ) Render.text( "With each new bit of length inserted, she waits a few moments for you to adjust to her size, letting you fill out to accommodate her.  " );
	Render.text( "With the final bit of her inserted into you, her tail suddenly shoots straight up out of your sight, and she lets out a long exaggerated sigh of pleasure as the milking machine continues to whirl above her." );
	player.cuntChange( 8, true, true, false );
	
	Render.text( "[pg]\"<i>Oh sweetie, you feel so wonderful.  I actually feel a bit strange right now, like I want to....</i>\"  She starts thrusting into your entrance with great gusto, eliciting a surprised gasp from you; Marble isn't normally this energetic when it comes to sex!  Being milked is probably making her more aggressive than usual, though given how good she is making you feel, you can't really complain." );
	
	Render.text( "[pg]She starts playing with your backside with her hand, massaging your " );
	if( player.lowerBody.butt.buttRating <= 4 && !player.isGoo() ) Render.text( "taut" );
	else Render.text( "jiggling" );
	Render.text( " flesh while her " + num2Text(Math.round(flags[FlagEnum.MARBLE_DICK_LENGTH])) + " inch length continues to push and pull into your womanhood.  Egged on by her enthusiasm, you start putting in some effort into the sex yourself by pushing your backside tight against her with each push forward and shaking it back and forth with each pull out of your interior." );
	
	Render.text( "[pg]The stimulation sends Marble wild, and in short order she is flooding you with her hermaphroditic seed.  However, she doesn't slow down at all and simply continues to lovingly abuse your backside and mound of pleasure.  You look down between your legs once more to see her tail has started swinging lengthwise with her body, every last inch of her body and mind are fixed on breeding with you, triggering a triumphant laugh from you as you cum yourself." );
	
	Render.text( "[pg]Nearly an hour later, Marble finally calms down and looks down at what she has managed to do to you.  \"<i>Oh my!  Sweetie, I'm sorry, I'm not sure what came over me...</i>\" she freezes when she sees your happy expression, and you inform her that you rather liked seeing that side of her. The two of you will have to do this again some time." );
...
private milkMarbleOnTheBar():void
{
	...
	Render.text( "You suggest that you'd be interested in playing with her while she's draped over the bar on the one side of the room." );
	
	Render.text( "[pg]She narrows her eyes at you, considering the proposition.  \"<i>I don't know sweetie, doesn't really sound like something I'd like that much.  I'd like to be in a position where I can do more than just take it...</i>\"" );
	
	Render.text( "[pg]You assure her that you'll make it good enough that it will be worth while to let you have complete control." );
	
	Render.text( "[pg]After a few more moments of thought, Marble gives you a very serious look, \"<i>You don't get complete control, I'll be giving the instructions here, oh, and you'd better do a good job.</i>\"  Suddenly she smiles, \"<i>Otherwise, I'll just have to turn things around!</i>\"" );
	
	Render.text( "[pg]You wonder if maybe you want her to turn things around while she rests her hands on the bar, then drapes herself over it with her breasts swinging underneath.  She turns her head towards you and swings her ribbon decorated tail back and forth a few times.  You'd best not keep her waiting when she actually took you up on your request, so you strip down and step up behind her." );
	
	Render.text( "[pg]\"<i>Alright sweetie, time to get me ready.  Let's see how you do.</i>\"  She turns her head back, and simply waits for you to start while the milking machine continues to hum above her." );
	
	Render.text( "[pg]You start by lifting up her skirt to expose her round ass to the air and the lips of her womanhood below it.  Of course, there is also the base of her tail just above it which continues to swish back and forth somewhat indignantly overtop of her openings.  Next, you take a handful of her generous asscheeks, rolling them around in your hands and kneading the soft bouncy flesh.  Though not as expansive as her bust by any means, Marble undeniably has quite the pleasant backside." );
	
	Render.text( "[pg]\"<i>Hmm,</i>\" comes a pleased moan from the cowgirl in front of you, and her tail lifts up ever so slightly as the first signs of moisture and arousal form around her womanhood." );
	
	Render.text( "[pg]Next, you run a finger across her entrance, teasing her ever so slightly.  A small jolt flies through her body, and her tail shoots straight up." );
	
	Render.text( "[pg]\"<i>Ah, that's a good start sweetie, now, put a finger inside me.</i>\"" );
	
	Render.text( "[pg]The words are barely out of her mouth, when you push your index finger into her lower lips.  There is a low squelch while it slips inside, which starts again when you stir your finger around in a few circles.  This causes Marble's tail to curl around your hand and try to pull your finger out.  Once a little ways out, her tail tries to force your hand back into her waiting snatch." );
	
	Render.text( "[pg]\"<i>Maybe you need my help after all.  Want to let my tail do the talking sweetie?  Let me show you how I like being penetrated.  Then again, your tongue might be nice to feel next too...\"</i>" );
	
	if( player.cockThatFits(marbleCuntCapacity()) > 0 ) Render.text( "[pg]You idly consider grabbing and pulling on her tail, so you could have a chance to take charge, though Marble would probably try and take control in that case for sure..." );
...
private milkMarbleBarFollowTail():void
{
	...
	else
	{
		...
		Render.text( "[pg]You ask Marble if there is any chance you've found what she wanted?" );
		
		Render.text( "[pg]\"<i>Why yes sweetie.  What gave it away?</i>\"" );
		
		Render.text( "[pg]It was just a hunch." );
		
		Render.text( "[pg]You adjust your grip on her personal toy and press one end against her waiting womanhood.  Just as you're about to ask her if she's ready for it, her tail suddenly jerks forward, sliding the tool inside her.  She gasps while the false cock quickly slides up to its centerpiece into her depths, then giggles when you try to pull it back out. Unfortunately, you find she's clapped down on it hard." );
		
		Render.text( "[pg]\"<i>What's wrong sweetie?  You haven't managed to get stuck have you?  Here, let me give you a hand....</i>\"" );
		
		Render.text( "[pg]Abruptly, her tail reverses it's pressure and allows you to extract her toy from its sheath - at least a little ways.  Then, you flip directions yourself and push it back in to the hilt.  There is a sharp intake of breath from your lovely lover, and you give her rear a small smack.  In response she tightens around your wrist, and it sounds like she might be about to get mad at you.  Instead she chuckles a bit." );
		
		Render.text( "[pg]\"<i>Alright, no more teasing.  You're more than just a plaything after all.  I've got to let you do something.</i>\"" );
		
		Render.text( "[pg]Finally, she allows you to freely build her pleasure and tease her body.  You can focus on bringing her to blissful sexual release by playing with her wonderful plump rump, twisting and pumping her with her dildo to the sound of lewd squelching that can just be heard over the hum of the milker and Marble's moans of pleasure, and lastly, the quivering of her tail.  The tail that serves as a subtle guide to the ways of your lover's tunnel." );
	}
	
	Render.text( "[pg]You're so engaged into the activity, that you barely register a long low call of ecstasy come out of cow-girl mate.  That same cry that is something between a moo and a long sigh that you've come to know very well.  At the same time, she manages to knock the hoses off her breasts, and there is a splash as her milk hits the floor of the stall, which steadily slows to a trickle, then only a few drops." );
	
	if( x > 0 )
	{
		Render.text( "[pg]Finally, the grip on your root relaxes, and you're finally able to explode inside her. Her tail slowly slips off of you and falls limp next to the connection between your bodies.  Several gasps and groans come out of your lips, and you ask Marble if she was satisfied." );
	}
	else
	{
		Render.text( "[pg]The hold on your hand tightens to a death grip, and you have no choice but to let go of her dildo and desperately struggle to free your constricted limb from her tail.  When you finally do manage to free yourself, you start rubbing your tingling fingers and ask Marble if putting your hand to sleep was a sign that she was satisfied." );
	}
	
	Render.text( "[pg]Your lover girl straightens up and turns around to face you.  She fixes you with a stern look for a few moments before pulling you into a deep french kiss.  \"<i>Well, if you're always that good at following directions, I'm not sure how I wouldn't be.</i>\"" );
...
private milkMarbleBarCunnilingling():void
{
	...
	Render.text( "You ask Marble to release your wrist so you can get into a better position for taking care of her.  You're feeling an ache to harvest her other womanly fluid." );
	
	Render.text( "[pg]\"<i>Of course sweetie, I'm feeling very wet down there.  I definitely think I'm going to need you to take care of that.</i>\"" );
	
	Render.text( "[pg]She released you, and allows you to kneel down behind her and get a good whiff of her rich womanly scent.  It throws your mind into a haze for a few moments, and you're brought back out with a swish of wind as her tail swings side to side over her entrance, the base lifted up eagerly.  You rest your head to the side of her entrance, and lean against the part of her body where ass meets thigh while letting one hand play across the other leg." );
	
	Render.text( "[pg]After a moment's climb, you find your fingers curling around the root of her bottom's bestial extension.  An idea strikes you, and you comment on how you need to brush her off before you start harvesting." );
	
	Render.text( "[pg]\"<i>Oh?  What do you mean by that?</i>\" comes a somewhat bemused response." );
	
	Render.text( "[pg]You don't reply, instead you make a small ring with your thumb and finger and slide her tail through the hole as it flicks and swings around.  When you reach the thick hair at the end, you tighten your grip and direct the brush to the entrance of her womanhood." );
	
	Render.text( "[pg]\"<i>Oh!  Ohhhhhhh...</i>\"" );
	
	Render.text( "[pg]A careful dab here, a deeper scope there, and you think she's ready to be harvested.  You get a drop of her fluids of off your brush and give it a taste test.  A quick word of approval to Marble of the quality of her fluids, and you dive into her snatch.  Her deep earthy aroma fills your head as you drink deeply of her fluids, licking around her folds and tasting everything she has to offer.  You hear soft moans of pleasure in approval of your efforts, just barely audible over the hum of the milking machine above you." );
	
	Render.text( "[pg]Now free to do as it wills, you find that Marble's tail has curled around the back of your head, and is trying its hardest to push you deeper into her rear as possible.  At the same time, your lover's hips are rocking side to side, dragging your face across her gash and spreading her fluids all over your [face].  Trying to steady yourself, you grab two big handfuls of her expansive touch, and lower your head slightly.  This act brings her love-button in line with your mouth, and her passage in line with your nose.  Once again you're overwhelmed by the olfactory sensations, and mindlessly lap away at her fluids and engorged clitoris." );
	
	Render.text( "[pg]\"<i>Oh yes, yes!  That's so wonderfuuuuuuul!</i>\"  Marble cries out several more times, as she rides out the orgasm you've brought her to through your efforts to harvest her.  Your reward is to be absolutely drenched in the sweet nectar you've been harvesting all this time, and every drop you catch in your mouth and on your [tongue] is swallowed in an instant.  In the end, you're actually a bit disappointed when her fluids run out." );
	
	Render.text( "[pg]You nuzzle in against her soft bum once more, and bat at her tail playfully for a few moments." );
	
	Render.text( "[pg]\"<i>Hmm, I'd say that you've made standing like this worth the while, [name].  I think I might be willing to go like this again, if you're able to keep up this level of performance.</i>\"" );
	
	Render.text( "[pg]You laugh and assure Marble that when it comes to her, you'll always be giving it your all.  Just before you leave, an impish thought passes through your head for a moment, and you slip her tail's brush to Marble's entrance and push it inside a short ways.  Marble starts from the unexpected sensation, and you explain that you're only cleaning up some stray drops of fluid that might still be hiding away." );
	
	Render.text( "[pg]After a moment, her tail flicks back out of her passage and she shakes her head at you.  \"<i>Thank you for the thought, sweetie, but please don't get me worked up again now, unless you're going to take care of me again.</i>\"" );
	
	Render.text( "[pg]Alright, you'll end the teasing, for now." );
...
private milkMarbleBarPullTail():void
{
	...
	Render.text( "Probably against your better judgment, you decide to take charge and grab onto Marble's tail, roughly untwisting it off of you and yanking it upwards." );
	
	Render.text( "[pg]\"<i>Ow!  What the hell sweetie?!</i>\"" );
	
	Render.text( "[pg]You ignore her protests and smack her ass once, then roughly penetrate her with your " + cockDescript(x) + " while yelling at her to take it." );
	
	Render.text( "[pg]There is a loud clop as Marble stomps her hoof on the ground angrily.  With a huff, she lifts herself up on the bar and kicks back with both her legs, sending you flying onto your back and knocking the wind from your lungs.  You gasp for breath a few times, then look up to see Marble looming over you with a wicked smile on her face." );
	
	Render.text( "[pg]\"<i>I warned you that I'd be taking charge if you couldn't behave yourself, didn't I [name]?</i>\"  She sits down on your stomach, and presses her hand against your [chest], squeezing you somewhat painfully.  \"<i>Now you're going to be punished!</i>\"  Given her expression, you get the impression that maybe she actually wanted you to misbehave!" );
	
	Render.text( "[pg]Your aggressive mate moves forward and straddles your face, shaking her snatch back and forth a few times just above your head.  A few drops of her lubricant drip down onto you, followed by a flick across the nose, delivered by the tail you just grabbed.  At the same time, you're bathed in the deep scent of her aroused sex." );
	if( flags[FlagEnum.MARBLE_DICK_TYPE] > 0 ) Render.text( "  She grabs her erect member, and rubs it against your cheek a few times, spreading around some of her pre-cum." );
	
	Render.text( "[pg]\"<i>You like it when I take charge, don't you?  Hmmhmm, yes...</i>\"" );
	
	if( flags[FlagEnum.MARBLE_DICK_TYPE] == 0 )
	{
		Render.text( "[pg]She drops her hips and roughly forces her womanhood down onto your face, cutting off your ability to breathe.  You sputter and gasp underneath her at the sudden suffocation, though it lasts only a few seconds before she releases you." );
		
		Render.text( "[pg]\"<i>Aww, was that too rough?  Don't worry.  I'll be a bit more gentle.</i>\"" );
		
		Render.text( "[pg]Marble then settles herself down a bit more gently, this time giving you plenty of room to breathe while also keeping your mouth tightly locked against her slick passage." );
		
		Render.text( "[pg]\"<i>Alright, let's put that rude tongue of yours to a much better job.  Lick me.</i>\"" );
		
		Render.text( "[pg]You do as you're bid and get an approving smile in response.  Digging deeper inside her, you feel under her folds and plunge into her depths.  " );
		if(player.tongueType > TONUGE.HUMAN) Render.text( "You take full advantage of your inhumanly long tongue to fill her up, running across her passage all the way to the entrance to her womb and then back to her lower lips." );
		else Render.text( "You send your tongue as far into her passage as it will go and wriggle around her lower lips, trying to please your aggressive bovine lover to the best of your abilities." );
	}
	else
	{
		Render.text( "[pg]She forces your mouth open and pushes her hips forward, roughly shoving her " + num2Text(Math.round(flags[FlagEnum.MARBLE_DICK_LENGTH])) + "-inch cock into your hapless mouth and down your throat.  You choke and sputter in surprise at the sudden suffocation, but it only lasts a few seconds before she pulls back out." );
		
		Render.text( "[pg]\"<i>Aww, was that too rough?  Don't worry, I'll be a bit more gentle.</i>\"" );
		
		Render.text( "[pg]Marble then thrusts forward much more gently, giving you plenty of time to get use to the sensation of her member inside your mouth.  She also is sure to keep herself well outside of your throat." );
		
		Render.text( "[pg]\"<i>Alright, let's put that rude mouth of yours to a much better job.  Suck me.</i>\"" );
		
		Render.text( "[pg]You do as you're bid, and get an approving smile in response.  You wrap tightly around her with your lips and slide your tongue up and down the bottom of her shaft while also playing across the urethra.  She pumps herself into you a few times, and you get a taste of some of her pre-cum." );
	}
	
	Render.text( "[pg]Marble lifts her hips back up after a few moments, with a curt nod.  \"<i>Now, you've got to take care of my tail.  It stings after you pulled on it like that, I want you to apologize to it and kiss it better.</i>\"  She then turns around and gives you a good view of that very tail, which seems to be swinging back and forth rather half-heartedly." );
	
	Render.text( "[pg]There, there, you tell the part of your mate's body you hurt before, gently stroking the short furred appendage.  Then you lean forward and repeatedly kiss it, running from the thick tuff on the tip, all the way up to the base where her spine meets the round cheeks of her ass.  When you reach the root, a quiver shoots through Marble's body, and she suddenly grabs you and forces her tail into your mouth.  You gasp in surprise and look up at your lover inquisitively." );
	
	Render.text( "[pg]\"<i>Hmm, sorry sweetie, I just thought I'd try something.  It didn't feel the way I expect, but I really like the way you look with my tail in your mouth like that.  If you suck on it for a bit, I might be able to forgive you for yanking it before.</i>\"" );
	
	Render.text( "[pg]You run your tongue around the sweaty fur and get an approving look from your bovine lover.  Then you shake your jaw back and forth a few times and slowly let her bush slide out of your mouth.  As the last of the hair slips out, you give a small kiss to the tip." );
	
	Render.text( "[pg]\"<i>Did you like that sweetie?" );
	if( silly() ) Render.text( "  Want to brush your teeth with my tail next time?" );
	Render.text( "</i>\"  She nods and considers you for a moment.  \"<i>Well, I think I'm feeling a lot better now.  I think I can forgive you for what you did.  In fact, you did such a good job that I think I'll give you a reward!</i>\"" );
	
	Render.text( "[pg]With a smile, you ask her what she's got planned for you." );
	
	Render.text( "[pg]She repositions herself overtop of your waist and eyes your male member.  \"<i>Obviously this thing here is in need of some treatment, since you were so eager to try and take charge with it before.</i>\"  She drops down and brings the flaps of her womanhood into contact with the front of your shaft and runs them over it a couple of times.  \"<i>Look how hard you are.  You really want me to take you inside me, don't you?  You want so much to give complete control of your long, hard, dick, to me to use as I see fit.</i>\"" );
	
	Render.text( "[pg]She doesn't let you respond.  She's already decided what is going to happen, and that's for you to go inside her and be used by her as she sees fit.  She strokes your " + cockDescript(x) + " with one finger, then pushes it into her waiting cunt.  You slip easily into her very well lubricated passage and groan as it clamps down on you.  Every part of your shaft is being tightly wrapped in the ring of her vaginal muscles, and you can't help but groan in pleasure." );
	
	Render.text( "[pg]She shakes her hips side to side a few times, spinning your shaft around her interior, then stops and considers you.  \"<i>You know, I was thinking that maybe I'm giving you too much of a reward, or maybe not enough of one?</i>\"  The same wicked look when she first knocked you over crosses her eyes once more.  \"<i>Let's try something new...</i>\"  She turns away from you and puts her hand somewhere behind her, outside of your view." );
	
	Render.text( "[pg]After a moment, you suddenly feel something pushing up against the entrance to your [vagOrAss]!  You try to ask what Marble has planned, but the pressure forces something bushy into your hole.  She just stuck her tail into your [vagOrAss]!" );
	
	Render.text( "[pg]\"<i>Well sweetie, what do you think?</i>\"  It quivers a few times as Marble resumes the rocking of her wide womanly hips.  \"<i>Never mind, if you don't like it, well you can just try and end this as soon as possible.  If you do like it, then I guess...</i>\"  She leans forward as she switches from a swinging movement to a bounce on your " + cockDescript(x) + ", and her furry tufft of a backend appendage starts to push its way in deeper sliding easily thanks to your saliva on it.  \"<i>You'll just have to bear it as long as you can!</i>\"" );
	
	Render.text( "[pg]You've never seen Marble this aggressive before, but the cause is rather obvious: the whirring milking machine that's currently attached to her breasts.  Obviously giving milk is triggering some kind of dominance instinct, and it's incredibly sexy!" );
	
	Render.text( "[pg]Your lover laughs uproariously as she continues to ride you hard and fast, bringing up the pleasure in your lower body just as hard and fast.  Whether or not you want it, you can feel an orgasm rapidly building up within you, easily detectable to your rider from the increased rate of your gasps and cries of pleasure.  For her part, Marble is rapidly approaching her peak as well, and with a practiced rhythm, the two of you cum in unison as your lover's tail pops out of your [vagOrAss]." );
	
	Render.text( "[pg]The bovine woman gasps a few times above you in exhaustion, her great breasts heaving with each breath.  She then reaches down and pulls your head up and cradles it to her chest, smiling very happily.  \"<i>Well sweetie, if you ever want me to play rough again, you know what to do.  I'm looking forward to any other adventures the two of us have in the bedroom, are you?</i>\"" );
	
	Render.text( "[pg]You snuggle up to her bosom, thinking something very similar.  Things certainly haven't been dull while you've been in Marble's milking stall, that's for certain." );
...
private milkMarbleLeaveAfterBar():void
{
	...
	Render.text( "You don't answer Marble and instead continue to finger her womanhood until she cries out in orgasm.  You tell her you'd be happy to help her out again whenever she'd like someone to accompany her to get milked." );
	
	Render.text( "[pg]She is a bit disappointed that you didn't go further after getting her on the bar, but doesn't have a problem with you leaving.  She does tell you that she's looking forward to having you in her milker again some time." );
...
{
	clearOutput();
	Render.text( "You shake your head and say that you think you'll be fine." );
	
	Render.text( "[pg]\"<i>Oh, alright then sweetie, I'll see you later.</i>\"  She takes a seat in the chair in the middle of the room and picks up where she left off in her book.  You excuse yourself and return back to camp." );
