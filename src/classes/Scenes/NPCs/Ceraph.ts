export default class Ceraph extends Monster {
		...
		if (findStatusAffect(StatusAffects.Uber) < 0) {
			...
			//FIRE!
				...
				//(AUTO-LOSE)
					...
					MainScreen.text("She throws her hands out, palms facing you, and a rush of pink flame washes towards you.  Too busy with your own attack to effectively dodge, you're hit full on by the pink fire.  Incredibly, it doesn't burn.  The fire actually seems to flow inside you, disappearing into your skin.  You stumble, confused for a second, but then it hits you.  Every inch of your body is buzzing with pleasure, practically squirming and convulsing with sexual delight.  You collapse, twitching and heaving, feeling the constant sensation of sexual release running from your head to your " + LowerBodyDescriptor.describeFeet(player) + ".  Too horny and pleasured to resist, you lie down and tremble, occasionally rubbing yourself to enhance the bliss.", false);
	...
	private ceraphSpecial2(): void {
		...
		//[SPECIAL WHILE PC RESTRAINED]
			...
			if (rand(2) == 0) {
				MainScreen.text("Ceraph cuddles up against you, embracing you tenderly.  Her more-than-ample bosom crushes against your flank, and her demonic prick grinds and rubs against your " + player.skinDesc + ", smearing it with her juices.  Her hands slide over your bound form, sneaking underneath your " + player.inventory.armor.displayName + " to caress you more intimately while you're at her mercy.", false);
			...
			//[SPECIAL 2 WHILE PC RESTRAINED]
				...
				if (player.stats.lust < 33) MainScreen.text("It makes you feel warm and flushed.", false);
				else if (player.stats.lust < 60) MainScreen.text("It gets inside you and turns you on, stoking the flames of your desire.", false);
				else if (player.stats.lust < 80) MainScreen.text("It makes you very horny, and you begin to wonder if it's worth resisting.", false);
	...
	public ceraphBindingStruggle(): void {
		...
		if (player.statusAffects.get("Bound").value1 - 1 <= 0) {
			...
			if (player.stats.str > 65 && rand(player.stats.str) > 45) {
