...
export default class ArianScene implements UpdateInterface {
	//Implementation of TimeAwareInterface
	public update() {
		if (Flags.list[FlagEnum.ARIAN_EGG_COUNTER] > 0) Flags.list[FlagEnum.ARIAN_EGG_COUNTER]++;
		if (model.time.hours > 23) {
			if (arianScene.arianFollower() && Flags.list[FlagEnum.ARIAN_VAGINA] > 0) Flags.list[FlagEnum.ARIAN_EGG_EVENT]++;
			Flags.list[FlagEnum.ARIAN_LESSONS] = 0;
			Flags.list[FlagEnum.ARIAN_TREATMENT] = 0;
		}
		return false;
	}

	private arianCockSize(): number {
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] < 0 || Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 3) return 0;
		else if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 1) return 9;
		else if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 2) return 16;
		else return 36;
	}

	public arianFollower(): boolean {
		return Flags.list[FlagEnum.ARIAN_FOLLOWER] > 0;
	}
	public arianMF(boy: string, girl: string): string {
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) return girl;
			else return boy;
		}
		return girl;
	}
	public arianHealth(arg: number = 0): number {
		if (arg != 0) {
			Flags.list[FlagEnum.ARIAN_HEALTH] += arg;
			if (Flags.list[FlagEnum.ARIAN_HEALTH] > 100) Flags.list[FlagEnum.ARIAN_HEALTH] = 100;
			else if (Flags.list[FlagEnum.ARIAN_HEALTH] < 0) Flags.list[FlagEnum.ARIAN_HEALTH] = 0;
		}
		return Flags.list[FlagEnum.ARIAN_HEALTH];
	}
	public arianChestAdjective(): string {
		let buffer: string = "";
		let temp: number = rand(10);
		if (Flags.list[FlagEnum.ARIAN_BREASTS] == 0) return "";
		else if (Flags.list[FlagEnum.ARIAN_BREASTS] == 1) {
			if (temp <= 4) buffer += "small";
			else if (temp <= 6) buffer += "petite";
			else if (temp <= 8) buffer += "perky";
			else buffer += "palm-filling";
		}
		else if (Flags.list[FlagEnum.ARIAN_BREASTS] == 2) {
	...
	public arianChest(): string {
		...
		if (Flags.list[FlagEnum.ARIAN_BREASTS] == 0) return "chest";
	...
	private helpArianWhenYouMeetHim(): void {
		...
		Flags.list[FlagEnum.ARIAN_PARK] = 1;
		Flags.list[FlagEnum.ARIAN_COCK_SIZE] = 1;
		Flags.list[FlagEnum.ARIAN_CAPACITY] = 50;

		MainScreen.text("You approach the hooded figure with caution, asking if they're all right; it feels a little silly to say that, but you can't think of much else to say.");
	...
	public visitThePark(): void {
		MainScreen.clearText();
		MainScreen.text("As you enter the ragged remnants of the park, you spot the sickly lizan, Arian, sitting at his usual bench, and greet him.  \"<i>Oh, hello there [name].  Good to see you.</i>\"  He waves lazily.");

		//Visit 1
		if (Flags.list[FlagEnum.ARIAN_PARK] == 1) {
			MainScreen.text("\n\nFeeling ");
			if (player.stats.cor < 50) MainScreen.text("curious");
			else MainScreen.text("bored");
			MainScreen.text(", you decide to ask him what his story is.");
			...
			if (player.hasSpells()) {
				MainScreen.text("  You thought spellcasting merely took fatigue and the proper mindset, not life force, and you express that sentiment to the lizan.");
			}
		...
		//Visit 2
		else if (Flags.list[FlagEnum.ARIAN_PARK] == 2) {
		...
		//Visit 3
		else if (Flags.list[FlagEnum.ARIAN_PARK] == 3) {
			...
			if (player.stats.cor > 66) MainScreen.text(", whilst privately thinking to yourself how useless that looks - no wonder they can't fight the demons if this is the best they're capable of");
			MainScreen.text(".");
			...
			if (player.stats.cor >= 66) MainScreen.text(", keeping your real reasons for bothering with him to yourself");
			MainScreen.text(".");
	...
	public visitAriansHouse(): void {
		...
		if (arianFollower()) {
			MainScreen.text("You approach the enchanted tent and slip easily inside the doors to the luxurious interior.  ");
			let temp: number = rand(10);
			if (temp == 0) {
				MainScreen.text("However, Arian isn't here right now, so you instead make yourself comfortable on the couch.  After a few minutes, Arian [Arian emself] walks in through the entrance.  \"<i>Oh, [name].  I wasn't aware you were here... have you been waiting for long?</i>\" [Arian ey] asks.  You tell [Arian em] not very long.  \"<i>That's good to hear.  So, what can I do for you?</i>\" [Arian ey] asks, with a smile.");
			}
			else if (temp == 1) {
				MainScreen.text("Inside, the lizan is sitting at a table, fastidiously drinking from a cup of something hot while pouring over an arcane-looking text.  You politely cough to draw [Arian eir] attention and [Arian ey] looks at you, smiling.  \"<i>Hello, [name].  I was just catching up on my studies.  Can I offer you a cup of tea, or maybe something else?</i>\" [Arian ey] asks.");
			}
			else if (temp <= 2) {
				MainScreen.text("The lizan is currently busy tinkering with some occult-looking paraphernalia when you find [Arian em].  You politely cough to attract [Arian eir] attention, then do so again when [Arian ey] fails to heed that.  It's only on the third attempt that [Arian ey] looks up apologetically from [Arian eir] work.  \"<i>Ah, [name]; I'm sorry, but I was preoccupied with something.</i>\" [Arian ey] states in an apologetic tone, indicating the mess on [Arian eir] desk.  \"<i>Was there something you wanted?</i>\" [Arian ey] asks.");
			}
			else if (temp <= 4) {
				MainScreen.text("The smell of fresh cooking fills the air and you can see Arian happily sitting down at his couch with a plate of something just cooked.  \"<i>Oh, [name]; I was just about to eat, can I offer you a dish?  Or if you'd rather do something else, this can wait,</i>\" the lizan tells you with a smile.");
			}
			else if (temp <= 6) {
				...
				MainScreen.text("A strange smell hits your nose as you enter the tent; it takes you a few moments, but then you place it, your ears pricking as you hear Arian letting out some very familiar groans of release.  With a smirk, you sneak up and lean over the couch, looking right into the eyes of Arian, [Arian eir] fingers still glistening with ");
				if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
					if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("mixed sexual fluids");
					else MainScreen.text("jizz");
				}
				else MainScreen.text("femjizz");
				MainScreen.text(" and, indeed, still hovering over [Arian eir] ");
				if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
					if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("twin sexual slits");
					else {
						MainScreen.text("cock");
						if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
					}
				}
				else MainScreen.text("pussy");
				MainScreen.text(".  For a long moment, [Arian ey] just stares back at you; if lizans could blush, you're certain [Arian ey]'d be red as a beet.  \"<i>[name]! I was- I was just....  Oh, this is embarrassing,</i>\" [Arian ey] mutters, looking at [Arian eir] feet.");
			...
			if (Flags.list[FlagEnum.ARIAN_PARK] == 4) {
				...
				if (player.stats.cor < 33) MainScreen.text("curiously");
				else if (player.stats.cor < 66) MainScreen.text("casually");
				else MainScreen.text("indifferently");
				MainScreen.text(" ask why he's thanking you.");
				...
				if (!player.lowerBody.isTaur()) MainScreen.text("pull up a chair");
				else MainScreen.text("seat your tauric body on the floor");
				MainScreen.text(".  You rack your brains for polite conversation, and finally ask how he's been since you saw him last.");
				...
				if (Flags.list[FlagEnum.ARIAN_HERM_CHAT] == 1) {
					...
					//ArianGirlHermChat++;
					Flags.list[FlagEnum.ARIAN_HERM_CHAT]++;
				}
				//0-9 health:
				else if (Flags.list[FlagEnum.ARIAN_HEALTH] < 10) {
				...
				//10-19 health:
				else if (Flags.list[FlagEnum.ARIAN_HEALTH] < 20) {
				...
				//20-29 health:
				else if (Flags.list[FlagEnum.ARIAN_HEALTH] < 30) {
					//Repeat this until the PC decides to sex Arian up somehow.
					if (Flags.list[FlagEnum.ARIAN_VIRGIN] == 0 && Flags.list[FlagEnum.ARIAN_S_DIALOGUE] == 2) {
						...
						MainScreen.addButton(3, "Leave", leaveFappingArian);
						return;
					}
					//(else)
					else {
				...
				//30-49 health:
				else if (Flags.list[FlagEnum.ARIAN_HEALTH] < 50) {
				...
				//50-74 health
				else if (Flags.list[FlagEnum.ARIAN_HEALTH] < 75) {
					...
					if (!player.lowerBody.isTaur()) MainScreen.text("take the offered chair");
					else MainScreen.text("the offered chair");
					MainScreen.text(".");
				...
				//75-100 health:
					...
					if (!player.lowerBody.isTaur()) MainScreen.text(" and offers you the chair nearby");
					MainScreen.text(".  Taking another for [Arian em]self.  \"<i>I love when you come visit, [name].  So, what are we going to do today?</i>\" [Arian ey] asks, expectantly.");
				}
				//(Display Options)
				arianHomeMenu();
			}
	...
	private arianHomeMenu(): void {
		...
		else if (Flags.list[FlagEnum.ARIAN_S_DIALOGUE] == 3 && arianHealth() >= 50) MainScreen.addButton(0, "Next", arianImbue);
		else if (Flags.list[FlagEnum.ARIAN_S_DIALOGUE] == 4 && arianHealth() >= 75) MainScreen.addButton(0, "Next", arianPlot4);
		else if (Flags.list[FlagEnum.ARIAN_S_DIALOGUE] == 5 && arianHealth() >= 100) MainScreen.addButton(0, "Next", arianPlot5);
		//If no story dialogue
		else {
			MainScreen.addButton(0, "Talk", talkToArianChoices);
			if (Flags.list[FlagEnum.ARIAN_S_DIALOGUE] >= 2) MainScreen.addButton(1, "Sex", arianSexMenu);
			if (Flags.list[FlagEnum.ARIAN_S_DIALOGUE] >= 1) MainScreen.addButton(3, "Give Item", giveArianAnItem);
			if (player.hasKeyItem("Arian's Talisman") >= 0 || player.hasKeyItem("Arian's Charged Talisman") >= 0)
				MainScreen.addButton(2, "Talisman", imbueTalisman);
			if (Flags.list[FlagEnum.ARIAN_S_DIALOGUE] >= 5) MainScreen.addButton(4, "Treat Corr.", treatCorruption);
			if (model.time.hours >= 17 && arianFollower()) MainScreen.addButton(8, "Sleep With", sleepWithArian, true);
			if (Flags.list[FlagEnum.SLEEP_WITH] == "Arian") MainScreen.addButton(8, "NoSleepWith", dontSleepWithArian);
			if (!arianFollower()) MainScreen.addButton(9, "Back", telAdre.telAdreMenu);
			else MainScreen.addButton(9, "Back", camp.campLoversMenu);
		}
	}

	private dontSleepWithArian(): void {
		MainScreen.clearText();
		MainScreen.text("You decide not to sleep with Arian at night, for now.");
	...
	private eavesDropOnArian(): void {
		MainScreen.clearText();
		MainScreen.text("You sidle up to the door, pressing your ear against the wood and start to listen intently.");
	...
	private peepOnArian(): void {
		MainScreen.clearText();
		MainScreen.text("Curious, you decide to take a little peek through the lock; you press yourself against it as best you can, looking through into the bedroom beyond.  True to what your ears heard, the sickly albino's health has improved enough for him to focus on more... carnal matters.  Naked from the waist down, he sits on the edge of his bed, groinal slit disgorging a single, average-sized phallus.  Maybe 6 inches long, it's a bright purple-red color, covered in strange lumps");
		if (player.lowerBody.cockSpot.countType(CockType.LIZARD) > 0) MainScreen.text(" just like yours");
		MainScreen.text(", though this isn't stopping him from enthusiastically stroking himself off.");
	...
	private leaveFappingArian(): void {
		MainScreen.clearText();
		MainScreen.text("You decide to let Arian have some privacy and leave for the moment... after all, what the lizan mage does in his free time is not really your business....");
	...
	private bargeInOnArian(): void {
		MainScreen.clearText();
		MainScreen.text("With a wry smirk you turn the knob and find that Arian's door is unlocked; without missing a beat, you open the door and step in right in time to see a sticky rope of pre paint Arian's slender belly as he scrambles to cover himself up.");
	...
	private hermsLikeMaleArian(): void {
		MainScreen.clearText();
		MainScreen.text("You tell him that's not the case for you; you don't have a problem with him being a guy.  In fact, you think he looks very cute, earning you a nervous smile.  Arian relaxes, letting you look over his body and decide what you want to do....");
		//(Should you penetrate him or mount him?)
		//Penetrate - (Proceed to appropriate scene)
		//Get Penetrated - (Proceed to appropriate scene)
		menu();
		if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(50) >= 0) MainScreen.addButton(0, "Penetrate", giveArianAnal);
		MainScreen.addButton(1, "Get Penetrated", getPenetratedByArianAndHisHitlerMustache);
	}
	//[=Prefer Female=]
	private hermsLikeFemaleArian(): void {
		MainScreen.clearText();
		MainScreen.text("You tell him that while you do like to play with guys once in a while, you prefer girls.");
	...
	private youDontMindBeingGayForArian(): void {
		MainScreen.clearText();
		MainScreen.text("You tell him that you don't have a problem with males, as long as they're cute.  You smile at him.  \"<i>You... do you really think I'm cute?</i>\"");
	...
	private youLikeGirlsNotSickLizardDudes(): void {
		MainScreen.clearText();
		MainScreen.text("You tell him that you prefer females.... Arian looks at you expectantly.  \"<i>So... if I was a girl... then you wouldn't mind?</i>\"");
	...
	private arianStoryDialogue1(): void {
		arianHealth(1);
		MainScreen.clearText();
		MainScreen.text("You feel like you'd like to know a bit more about Arian, so you ask if he would mind sharing some of [Arian eir] history with you.  After all, as a survivor from at least the early days of the demon war, and a wizard to boot, he's got to have some stories up [Arian eir] voluminous sleeves.");
		...
		//PC has met Marae: 
		if (player.statusAffects.has("MetMarae")) MainScreen.text("Marae herself told you they showed up about, what, 20-30 years ago?");
		else MainScreen.text("you'd guess a long while ago given the general mess they seem to have made of the world.");
	...
	private arianStoryDialogue2(): void {
		MainScreen.clearText();
		arianHealth(1);
		MainScreen.text("You look Arian over, remarking that he seems to be getting better after all.");
	...
	private arianStory2DropIt(): void {
		MainScreen.clearText();
		MainScreen.text("Though you do feel a little curious, you decide to stop making him uncomfortable, and tell him that it's okay, you'll let him get some sleep now.");
	...
	private arianStoryPry(): void {
		MainScreen.clearText();
		MainScreen.text("Oh, no, you're not letting him wriggle out of this that easily.  You playfully tap [Arian eir] nose and tell him he should come clean and confess");
		if (player.stats.cor < 40) MainScreen.text("; he'll sleep better with the burden off [Arian eir] conscience");
		MainScreen.text(".");
	...
	private arianDialogue3(): void {
		MainScreen.clearText();
		arianHealth(1);
		MainScreen.text("Before you can say anything, Arian asks you, \"<i>[name], I've been wondering....  Do you have any interest in magic?  You've done so much for me; I believe I should return the favor somehow.</i>\"");
		//ArianSDialogue++;
		Flags.list[FlagEnum.ARIAN_S_DIALOGUE]++;
		//[Yes] [No]
		menu();
	...
	private noArianShouldntMagicTeach(): void {
		MainScreen.clearText();
		MainScreen.text("You think it over for a moment, and then tell Arian that while you are flattered by the offer and willing to consider it, you can't say that you want to study magic right this moment.  You'd like to discuss it at some other time, please.");
	...
	private arianImbue(): void {
		MainScreen.clearText();
		arianHealth(1);
		MainScreen.text("Before you can say anything, Arian gasps, \"<i>Oh, [name].  I have a surprise for you.</i>\"  Arian says with a smile.");
	...
	private yesPlotSexArian(): void {
		MainScreen.clearText();
		MainScreen.text("You approach the awkwardly amorous lizan and place your arms around [Arian eir] neck.  Leaning in close, you whisper into [Arian eir] ear that [Arian ey] only had to ask.");
		//(Display Sex Menu)
		arianSexMenu(false);
	}
	...
	private noPlotSexNauArian(): void {
		MainScreen.clearText();
		MainScreen.text("You apologize to the lizan, telling [Arian em] that you aren't in the mood right now....");
	...
	private arianPlot4(): void {
		MainScreen.clearText();
		arianHealth(1);
		MainScreen.text("Before you can say anything, Arian says, \"<i>Oh, I have good news, [name]!</i>\"");
	...
	private arianPlot5(): void {
		MainScreen.clearText();
		arianHealth(1);
		MainScreen.text("Before you can say anything, Arian stops you.  \"<i>I've been meaning to ask you something, [name].  I've been feeling a lot better lately; in fact, I may be even better than I was before.</i>\"  Arian blushes.");
		...
		if (camp.companionsCount() > 1) MainScreen.text("  There are other people living with you already, so what's one more?  Right?");
		MainScreen.text("</i>\"");
	...
	private acceptArianMovingIntoCamp(): void {
		MainScreen.clearText();
		MainScreen.text("You tell Arian you'd be delighted to have [Arian em] move in with you.  Arian's face lights up like a kid's who's been given a bucket of candy.  \"<i>Really!?  Great!  I'll pack my stuff and we can go right away!</i>\"");
	...
	private denyAriansMoveIn(): void {
		MainScreen.clearText();
		MainScreen.text("You tell Arian you'd like some time to think about it.  Arian looks disappointed at first, but smiles at you all the same.  \"<i>I understand... no pressure....  So, what are we going to do today?</i>\"");
	...
	private talkToArianChoices(): void {
		MainScreen.clearText();
		MainScreen.text("You tell Arian you'd like to talk to [Arian em].  Arian smiles at the prospect of chatting with you.  \"<i>I love talking with you; so what do you want to talk about?</i>\"");
	...
	private arianMagicLessons(): void {
		MainScreen.clearText();
		arianHealth(1);
		MainScreen.text("You ask Arian if [Arian ey] wouldn't mind giving you some magic lessons.");

		//(if ArianMLesson >= 4)
		if (Flags.list[FlagEnum.ARIAN_LESSONS] >= 4) {
		...
		if (player.stats.int < 25) {
			...
			if (!arianFollower()) MainScreen.text("make your way back to your camp.");
			else MainScreen.text("return to your duties.");
		}
		//(else if PC int < 50 //Teach Charge Weapon if PC doesn't know.
			...
			if (!arianFollower()) MainScreen.text(", making your way back to camp.");
			else MainScreen.text(", exiting [Arian eir] tent and going about your business.");
			...
			if (!player.statusAffects.has("KnowsCharge")) {
				...
				MainScreen.text(" away, you start pondering Arian's lesson of the day.  You wonder if you could perhaps apply the channeling principle to strengthen your [weapon] in combat.");
			...
			MainScreen.text("<b>Game Note: This isn't canon and will be re-written soonish.</b>");
			...
			if (!arianFollower()) MainScreen.text("begin the trek home.");
			else MainScreen.text("exit [Arian eir] tent.");
			//(if PC doesn't know Whitefire)
			if (!player.statusAffects.has("KnowsWhitefire")) {
		...
		Flags.list[FlagEnum.ARIAN_LESSONS]++;
		player.stats.int += 1;
	...
	private arianSexingTalk(): void {
		MainScreen.clearText();
		arianHealth(1);
		MainScreen.text("You smirk knowingly at [Arian em] and ask how [Arian ey] feels about sex now that [Arian ey]'s had [Arian eir] first time?");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] != 0) {
			...
			//Big Cock.
			else if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 2) {
			...
			//Huge Cock.
			else if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 3) {
		...
		else if (Flags.list[FlagEnum.ARIAN_ANAL_XP] < 66) {
		...
		else if (Flags.list[FlagEnum.ARIAN_ANAL_XP] <= 100) {
	...
	private yesYouButtslutIllFuckYou(): void {
		MainScreen.clearText();
		MainScreen.text("How could you refuse such a request?  You tell [Arian em] to strip and get ready.");
	...
	private goddamnitNoYouButtSlut(): void {
		MainScreen.clearText();
		MainScreen.text("You apologize, but you really can't do that right now.  Arian looks a bit disappointed, but smiles at you all the same.  \"<i>Oh... okay.  Next time then?</i>\"");
		MainScreen.text("\n\nYou nod.");
		sexTalkFinish(false);
	}
	...
	private sexTalkFinish(newl: boolean = false): void {
		...
		MainScreen.text("Satisfied with your little chat, you pat the lizan's head and excuse yourself, heading back to camp.");
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private inviteArianToCamp(): void {
		MainScreen.clearText();
		MainScreen.text("You ask the lizan if [Arian ey] still really wants to leave [Arian eir] comfortable home in the city and come out to live with you in your little camp in the wastelands?");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			if (Flags.list[FlagEnum.ARIAN_VAGINA] == 0) MainScreen.text("man");
	...
	private takeYerLizardHomePartII(): void {
		MainScreen.clearText();
		MainScreen.text("Upon arriving at the camp, the first thing Arian notices is the shimmering portal.  \"<i>Is this... where you came from?</i>\" Arian asks.");
		...
		if (camp.followersCount() > 1) MainScreen.text(" and your other companions");
		MainScreen.text(".");
		...
		Flags.list[FlagEnum.ARIAN_FOLLOWER] = 1;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private arianSexMenu(output: boolean = true): void {
		if (output) {
			MainScreen.clearText();
			MainScreen.text("You ask Arian if [Arian ey] feels strong enough to do a little lovemaking.");
			//(if ArianHealth < 30)
			if (Flags.list[FlagEnum.ARIAN_HEALTH] < 30) {
			...
			//(if ArianHealth < 100)
			else if (Flags.list[FlagEnum.ARIAN_HEALTH] < 100) {
		...
		if (player.lowerBody.cockSpot.hasCock()) {
			...
			//Get Blown:
			//PC must have a cock.
			MainScreen.addButton(2, "Get Blown", getBlownByArian);
			//Penetrate:
			//Arian must be herm/female.
			//PC must have a cock that fits (cock area 50 or less)
			if (player.cockThatFits(Flags.list[FlagEnum.ARIAN_CAPACITY]) >= 0 && Flags.list[FlagEnum.ARIAN_VAGINA] > 0)
				MainScreen.addButton(4, "Fuck Vagina", penetrateArian);
			//Double Pen Arian:
			...
			if (player.cockThatFits(50) >= 0 && player.cockThatFits2(50) >= 0 && Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.addButton(8, "Double Pen", doublePenetrateArian);
			//Docking
			//ArianCockSize needs to be below 3. (ArianDblCock does not affect this decision.) 
			//PC cock area must be <= 30.
			if (player.smallestCockArea() <= 30 && Flags.list[FlagEnum.ARIAN_COCK_SIZE] < 3 && Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.addButton(7, "Docking", arianDocking);
		}
		//Get Anal:
		//Arian must have a cock.
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.addButton(1, "Anal - Catch", getButtWreckedByArian);
		//Blow:
		//Arian must have a cock.
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.addButton(3, "Blow " + arianMF("Him", "Her"), suckAriansDick);

		//PC must have a vagina.
		if (player.lowerBody.vaginaSpot.hasVagina()) {
			//Get Penetrated:
			//Arian must have a cock.
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.addButton(5, "Get Fucked", getPenetratedByArianAndHisHitlerMustache);
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0 && (player.hasKeyItem("Dildo") >= 0 || player.hasKeyItem("Deluxe Dildo") >= 0)) {
				MainScreen.addButton(6, "Dildo Fun", arianDildoFun);
			}
		}
		MainScreen.addButton(9, "Back", arianHomeMenu);
	}
	...
	private giveArianAnal(): void {
		...
		MainScreen.clearText();
		arianHealth(3);
		Flags.list[FlagEnum.ARIAN_ANAL_XP] += 10;
		if (Flags.list[FlagEnum.ARIAN_ANAL_XP] >= 100) Flags.list[FlagEnum.ARIAN_ANAL_XP] = 100;
		Flags.list[FlagEnum.ARIAN_VIRGIN] += 1;
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0)
			MainScreen.text(images.showImage("arianfemale-home-giveArianAnal"));
		else
			MainScreen.text(images.showImage("arianmale-home-giveArianAnal"));
		...
		MainScreen.text("You tell Arian that, if [Arian ey]'s willing, you'd like to take [Arian em] from behind.");
		//AnalXP < 33
		...
		if (Flags.list[FlagEnum.ARIAN_ANAL_XP] < 33) {
			...
			//(if ArianVagina < 1)
			if (Flags.list[FlagEnum.ARIAN_VAGINA] < 1) {
				...
				if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 0) MainScreen.text("girl");
				else MainScreen.text("herm");
				MainScreen.text(" isn't going to make it any smaller either.</i>\"  Arian fidgets and shudders as you stroke [Arian eir] behind.");
			}
			...
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) {
				MainScreen.text("the puffy lips of [Arian eir] drooling vagina");
				if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text(" and ");
			}
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
				MainScreen.text(arianMF("his", "her") + " erecting ");
				if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("pair of reptilian pricks");
				else MainScreen.text("reptilian prick");
			}
			...
			if (player.lowerBody.balls == 0) MainScreen.text("the base of your cock");
			else MainScreen.text("the bottom of your balls");
			MainScreen.text(" and you warn Arian that you're going to cum inside [Arian em] if you keep going.  Arian drools in pained pleasure, for a moment you wonder if [Arian ey] even heard you.  \"<i>Cum.  Cum inside me.  I want it... all of it! Ah!</i>\"");
			...
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text("spewing forth large gobs of cum of [Arian eir] own");
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0 && Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" and ");
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("wetting both your and her legs with a healthy serving of lizan pussy juice");
			MainScreen.text(".  You shudder and gasp until, utterly spent, you can't resist sinking down atop Arian and bearing [Arian em] into the comforting embrace of the bed.  You lay there, recovering your strength from your most delightful exertions, wriggling to give the lizan a proper snuggling while you lay there.");
		...
		//Loose, sensitive enough to make Arian scream in pleasure. (It's like a tight vagina, feels like one too, minor pain)
			...
			if (Flags.list[FlagEnum.ARIAN_VAGINA] < 1) {
				...
				if (Flags.list[FlagEnum.ARIAN_BREASTS] == 1) MainScreen.text(" perky");
				else if (Flags.list[FlagEnum.ARIAN_BREASTS] == 2) MainScreen.text(" rounded");
				else MainScreen.text(" pillowy");
				MainScreen.text(" mounds.");
			...
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
				MainScreen.text(" a");
				if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text(" pair of dripping cocks");
				else MainScreen.text(" dripping cock");
				if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" and");
			}
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" a sopping wet pussy");
			MainScreen.text(" greet");
			if ((Flags.list[FlagEnum.ARIAN_VAGINA] == 0 && Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] == 0) || (Flags.list[FlagEnum.ARIAN_VAGINA] == 1 && Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 0)) MainScreen.text("s");
			MainScreen.text(" your touch.  What a little slut Arian is turning into; [Arian ey]'s already raring to go, and even knowing it's going to be [Arian eir] ass that's getting fucked, too!");
			...
			if (Flags.list[FlagEnum.ARIAN_VAGINA] < 1) MainScreen.text("boypussy");
			else MainScreen.text("nether-pussy");
			MainScreen.text(".  \"<i>Yesssss.... Oh, sometimes it hurts a bit, but the feeling, the pleasure, the pain.... It's heavenly.</i>\"");
			...
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
				if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] == 0) MainScreen.text("cock spews its load");
				else MainScreen.text("twin cocks spew their loads");
				MainScreen.text(" on the bedsheets");
				if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" and [Arian eir] ");
			}
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" contracting pussy paints your lower body with lizan femcum");
			MainScreen.text(".  Arian is only capable of moaning and shuddering as [Arian eir] powerful orgasm rocks the poor lizan to [Arian eir] core.  The extra tightness of [Arian eir] contracting butthole increases the friction on your " + CockDescriptor.describeCock(player, x) + ", pushing you ever closer to the climax.");
			...
			if (player.lowerBody.balls == 0) MainScreen.text("base of your spine");
			else MainScreen.text("depths of your balls");
			MainScreen.text(".  Arian, completely blissed out, lays limply on [Arian eir] bed, [Arian eir] butt held up by your gripping hands.  With a final deep thrust you finally go over the edge.");
			...
			if (!arianFollower()) MainScreen.text("room");
			else MainScreen.text("tent");
			MainScreen.text(".");
		}
		//AnalXP <= 100
		//Very Loose, sensitive enough to make Arian cum just from insertion. (Feels better than anything else. Yep, Arian really becomes a buttslut at this point)
			...
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("wet gash is positively leaking lizan juices");
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0 && Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text(" and [Arian eir] ");
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text("erect cock is already leaking pre like a sieve");
			MainScreen.text(".  You can hardly believe how far Arian has come; you ask [Arian em] if it really feels that good?  \"<i>More than you can... ah... imagine, now fill me up!</i>\" [Arian ey] demands with a sense of urgency.");
			...
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("her pussy clenches, spilling a flood of juices; ");
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0 && Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text(" and ");
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text("his cock throbs and shoots rope after rope of cum onto the bedsheets");
			MainScreen.text(". Overwhelmed by your sudden intrusion, Arian collapses forward, burying [Arian eir] face on [Arian eir] pillow and dragging you on top of [Arian em].  \"<i>Ah... [name], you feel so good.  It's amazing.  I never thought buttsex could ever feel this good.</i>\"");
			...
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
				if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("cocks are still rock hard, and surprisingly, still leaking ropes of cum");
				else MainScreen.text("cock is still rock hard, and surprisingly, still leaking ropes of cum");
				if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(", and her");
			}
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" pussy squirting juices against your [legs] with each shuddering impact of your [hips]");
			MainScreen.text(".  \"<i>Ah!  Yes!  More, give me more!  Harder!  Faster!</i>\" Arian pleads, before biting on [Arian eir] pillow in pleasure.");
			...
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) {
				MainScreen.text("  Her cunt grips the air and blasts one last jet of wetness at your crotch.");
			}
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text("  [Arian Eir] throbbing cock flexes and shoots one last rope of lizan-cum against your belly.");
			MainScreen.text("  [Arian Eir] ass grips you forcefully, forming a vacuum and finally loosens.  \"<i>That... that wash da best,</i>\"  Arian slurs, before finally collapsing on [Arian eir] bed.");
			...
			//(ArianAssChat == 0)
			if (Flags.list[FlagEnum.ARIAN_ASS_CHAT] == 0) {
				...
				if (9999 == 9999) MainScreen.text(" as hypocritical as that may be");
				MainScreen.text("... besides, you think [Arian ey]'s kind of sexy when [Arian ey] gets like that.  At that Arian perks up.  \"<i>Really?</i>\"  Yes, really, you reply.  Arian smiles happily at you.  \"<i>So... do you want to go again?</i>\"");
				...
				if (!arianFollower()) MainScreen.text("back to Tel'Adre's streets, and from there back to camp.");
				else MainScreen.text("out of [Arian eir] tent.");
				Flags.list[FlagEnum.ARIAN_ASS_CHAT]++;
			}
			else {
	...
	private getBlownByArian(): void {
		...
		arianHealth(3);
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0)
			MainScreen.text(images.showImage("arianfemale-home-getbj"));
		else
			MainScreen.text(images.showImage("arianmale-home-getbj"));
		MainScreen.text("You trail your hand down your belly, pondering what to do.  Arian doesn't seem to notice, instead staring with anticipation at your erection.  You idly swing your hips from side to side, and notice with amusement that the lizan seems to follow it.  Building on that train of thought, you ask if [Arian ey] would be willing to suck you off.");

		//(if ArianHasBlown == 0)
		if (Flags.list[FlagEnum.ARIAN_HAS_BLOWN] == 0) {
			MainScreen.text("\n\nThe lizan averts [Arian eir] eyes, snapping out of [Arian eir] reverie.");
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 3) MainScreen.text("  You notice Arian's exposed shaft slowly hardening at your invitation.");
			MainScreen.text("  \"<i>I... Can I really?</i>\"");
		...
		if (player.cumQ() < 250) MainScreen.text("small");
		else if (player.cumQ() < 1500) MainScreen.text("big");
		else MainScreen.text("huge");
		MainScreen.text(" pot belly.  [Arian Ey] lets go of your dick with a sigh of pleasure and lays back on the bed.  \"<i>Ah... I'm full.</i>\"");

		MainScreen.text("\n\nYou tell [Arian em] that [Arian ey] looks full");
		if (player.cumQ() >= 1500) MainScreen.text(", in fact, you're amazed [Arian ey] didn't pop with how much [Arian ey] drank");
		MainScreen.text("; is [Arian ey] sure [Arian ey]'s all right?");
		...
		if (player.cumQ() >= 1000) MainScreen.text(" which, considering the fact [Arian ey] looks like a mother ready to birth a toddler from the size of [Arian eir] belly, isn't as easy as you'd think");
		MainScreen.text(".  Letting the lizan use your shoulder, you pull [Arian eir] sheets up and help [Arian em] back in the bed, where [Arian ey] flops down with a protest of springs");
		if (player.cumQ() >= 250) MainScreen.text(", [Arian eir] gut audibly sloshing as the cum inside is churned by the impact");
		MainScreen.text(".");
		...
		if (!arianFollower()) MainScreen.text(" back to camp, stopping only to notify Laika and Boon that Arian is sleeping.");
		else MainScreen.text(" out of Arian's tent.");
		player.orgasm();
		Flags.list[FlagEnum.ARIAN_HAS_BLOWN]++;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private penetrateArian(): void {
		...
		MainScreen.text(images.showImage("arianfemale-home-penetrate"));
		MainScreen.text("You admire the transgendered lizan's body, from her feminized features, down past her [arian chest], all the way to her shapely thighs.  You tell Arian that the change looks very good on her; few boys would really be so naturally pretty when turned into a ");
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text("dick");
		...
		MainScreen.text(".  \"<i>Err... readier?</i>\"  She asks, smiling nervously.");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 3) {
			MainScreen.text(", her exposed cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
			MainScreen.text(" hardening at what she's about to do");
		}
		else if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			MainScreen.text(", her bulbous cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s peeking out of their");
			else MainScreen.text(" peeking out of its");
			MainScreen.text(" hiding place, already at full mast in anticipation");
		}
		MainScreen.text(".  Arian swallows audibly.  \"<i>What now?</i>\" she asks, already panting in lust.");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 0) MainScreen.text("female");
		else MainScreen.text("herm");
		MainScreen.text(" lizan, until you have managed to hilt yourself inside of her.");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			MainScreen.text("  Her ");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("twin shafts");
			else MainScreen.text("shaft");
			MainScreen.text(" bobbing just out of your field of vision, leaking pre all over your belly.");
		}
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			MainScreen.text("  Her cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s twitch as they spurt ");
			else MainScreen.text(" twitches as it spurts ");
			MainScreen.text("a small rope of pre over her belly, anticipating what's to come as she braces herself for the inevitable pounding you're about to give her.");
		}
		...
		MainScreen.text("\n\n");
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			MainScreen.text("Arian's ");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("twin cocks throb");
			else MainScreen.text("cock throbs");
			MainScreen.text(" as she splashes her [arian chest] and face with her own futa-lizan seed.  ");
		}
		...
		if (player.lowerBody.balls == 0) MainScreen.text("you");
		else MainScreen.text("your [balls]");
		MainScreen.text(".");

		MainScreen.text("\n\nFinally having kept your promise, you give yourself over to the inevitable. ");
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" A wash of fluids spills down your body from your cunt as it spasms in release and y");
		else MainScreen.text("Y");
		MainScreen.text("ou unleash your essence right into the lizan's burning womb.");
		...
		if (player.cumQ() < 250) MainScreen.text("lean");
		else if (player.cumQ() < 1000) MainScreen.text("full");
		else MainScreen.text("overfilled");
		MainScreen.text(" belly.  \"<i>Stay with me, like this, a little longer?</i>\" Arian asks.");
	...
	private getButtWreckedByArian(): void {
		...
		arianHealth(3);
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0)
			MainScreen.text(images.showImage("arianfemale-home-getArianAnal"));
		else
			MainScreen.text(images.showImage("arianmale-home-getArianAnal"));
		MainScreen.text("With a lick of your lips with your tongue, you ask how Arian would like to be on the pitching end of anal?");
		//(if AnalXP < 33)
		if (Flags.list[FlagEnum.ARIAN_ANAL_XP] < 33) MainScreen.text("\n\n\"<i>I don't know... won't it hurt, are you sure?</i>\"");
		//(if AnalXP < 66)
		else if (Flags.list[FlagEnum.ARIAN_ANAL_XP] < 66) MainScreen.text("\n\n\"<i>I like it when you use my ass, but if you want me to use yours, I would gladly comply.  That is, if you're sure you want me to...?</i>\"");
		//(if AnalXP <= 100)
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] >= 3) {
			MainScreen.text("exposed cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s harden");
			else MainScreen.text(" hardens");
			MainScreen.text(" to full mast.");
		}
		else {
			MainScreen.text(arianMF("his", "her") + " cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s peek out of their");
			else MainScreen.text(" peeks out of its");
			MainScreen.text(" hiding place, rising to full mast.");
		}

		MainScreen.text("\n\nYou sashay yourself towards the prone lizan, straddling [Arian eir] legs and reaching out to grasp [Arian eir] ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("primary ");
		MainScreen.text("cock.  Slowly and gently you begin to stroke its strange, knobbly surface, your fingers eliciting moans and gasps from Arian as [Arian ey] shudders under your touch.  Precum begins to ooze from the reptilian prick's head, and you purposefully rub it into [Arian eir] shaft as a kind of makeshift lubricant.");

		MainScreen.text("\n\n\"<i>Ohm... T-this feels great, [name], b-but if you keep this up, I won't be able to hold back!</i>\" Arian pants, [Arian eir] shaft");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
		MainScreen.text(" throbbing against your hands.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) {
			MainScreen.text("  [Arian Eir] other shaft twiches and sprays your ");
			if (player.lowerBody.tailType == TailType.NONE) MainScreen.text("back");
			else MainScreen.text("tail");
			MainScreen.text(".");
		}
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("stroking [Arian eir] other cock and sliding it between your ass cheeks, ");
		MainScreen.text("caressing the base of [Arian eir] tail.  With a devious grin, you begin to gently but insistently slide your finger into [Arian eir] ");
		if (Flags.list[FlagEnum.ARIAN_ANAL_XP] > 66) MainScreen.text("eager ");
		MainScreen.text("ass, probing for [Arian eir] prostrate...");

		MainScreen.text("\n\nThe reaction is almost instantaneous; Arian moans into your kiss and you feel [Arian eir] ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("pair of lizan-cocks practically jump");
		else MainScreen.text("lizan-cock practically jump");
		MainScreen.text(" back to full mast, even surprising you with Arian's eagerness.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("sliding [Arian eir] neglected prick through your ass-cheeks, ");
		MainScreen.text("clenching your inner walls to grip and feel the excitingly unusual lumpy, bulbous texture of [Arian eir] shaft inside you.  You moan and gasp, telling [Arian em] that [Arian ey] feels so unique, and yet [Arian ey] stimulates you so wonderfully.");
		...
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("cunt dripping with arousal");
		if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" and ");
		if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text("cock");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
			MainScreen.text(" drooling precum onto [Arian eir] belly");
		}
		MainScreen.text(".  You're so close now, you can feel it.  You tell Arian you're going to cum, and beg [Arian em] to cum with you.");

		MainScreen.text("\n\nAs if on cue, Arian gasps and [Arian eir] shaft");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s twitch");
		else MainScreen.text(" twitches");
		MainScreen.text(", signaling [Arian eir] oncoming orgasm.  [Arian Ey] grabs the sheets on [Arian eir] bed and curls [Arian eir] toes.  \"<i>...[name]!</i>\"  Jet after jet of lizan spunk paints your bowels white");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text(", as well as your butt");
		MainScreen.text(", as Arian climaxes");
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(", her own pussy clenching and drizzling juices on the matted sheets beneath the two of you.");
		else MainScreen.text(".");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  [EachCock] spasms and shoots rope after rope of cum, painting Arian's [arian chest] white, as well as the groaning lizan's face.");
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Your [pussy], though unattended, joins in the perverted display, leaking copious amount of fluids on top of the prone lizan.");
		...
		//(if AnalXP <= 100)
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				...
				menu();
				if (player.cockThatFits(50) >= 0 && player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(0, "Yes", giveArianAnal);
				else MainScreen.text(".  You're too big to fit inside Arian's ass, though.");
				MainScreen.addButton(1, "No", camp.returnToCampUseOneHour);
				return;
			}
			else {
	...
	private suckAriansDick(): void {
		...
		arianHealth(3);
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0)
			MainScreen.text(images.showImage("arianfemale-home-suckariandick"));
		else
			MainScreen.text(images.showImage("arianmale-home-suckariandick"));
		MainScreen.text("You make a show of lewdly licking your lips and ask Arian if [Arian ey]'d be willing to let you have a little taste of lizan essence...?");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 3) {
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("exposed cocks hardening to full mast");
			else MainScreen.text("exposed cock hardening to full mast");
		}
		else {
			MainScreen.text("cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s emerging from their");
			else MainScreen.text(" emerging from its");
			MainScreen.text(" hiding place, already fully erect");
			MainScreen.text(".");
		}
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("uppermost ");
		MainScreen.text("cock, marveling at the strange textures of its bumpy, knobbly, bulbous surface.  It feels so strange to you, and you continue to stroke it eagerly.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("twin ");
		MainScreen.text("shaft");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s tremble");
		else MainScreen.text(" trembles");
		MainScreen.text(" and throb");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] == 0) MainScreen.text("s");
		MainScreen.text(" against your hand; pre quickly forming on the tip");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
		MainScreen.text(" only to slowly slide down the bulbous surface of Arian's lizan prick");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
		MainScreen.text(".  You continue to slide your hand along the increasingly slick surface, playfully asking if your hand really feels that good to [Arian em]?");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
		MainScreen.text(" a long, loving, slurpy lick.  The taste is surprising; a sort of sugary-tinted spice, not the usual salt of another creature.  It's actually quite pleasant, and you find yourself running your tongue up and down [Arian eir] prick as if it were some kind of candied stick, eagerly sucking and slobbering as you coax more of [Arian eir] yummy goo from its strangely-shaped tip.");
		...
		MainScreen.text(" one more time and erupts into your waiting mouth");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text(" and chin");
		MainScreen.text(".  [Arian Eir] legs close around your back, effectively keeping you from moving too far away as [Arian ey] finishes giving you all of [Arian eir] load.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text(", oblivious to the second cock spewing cum all over your [chest] in your eagerness");
		MainScreen.text(".  After [Arian ey] finally finishes, you continue to lick and suckle for a few minutes longer, making sure you've lapped up every last drop.  Then and then alone do you lift your head, flirtatiously licking your lips to tease the now-spent lizan.");
		...
		MainScreen.text(" rolling your neck in pleasure as [Arian ey] rubs it with [Arian eir] reptilian tail.");
		...
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" and tickle her cunt with a finger");
		MainScreen.text(" for emphasis, kiss [Arian em] gently on the tip of [Arian eir] nose, and then get your clothes together, planning on getting dressed and heading back out again.");
	...
	private getPenetratedByArianAndHisHitlerMustache(): void {
		...
		arianHealth(3);
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0)
			MainScreen.text(images.showImage("arianfemale-home-getpenetrated"));
		else
			MainScreen.text(images.showImage("arianmale-home-getpenetrated"));
		MainScreen.text("You look at Arian's ");
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] < 3) MainScreen.text("slit, and think of what it's hiding in there");
		else {
			MainScreen.text("exposed cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
		}
		MainScreen.text("; then you turn to look at Arian and ask [Arian em] how would [Arian ey] feel about giving [Arian eir] ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("twin cocks");
		else MainScreen.text("cock");
		MainScreen.text(" a bit of a workout?");
		...
		if (player.isBiped()) MainScreen.text("spread your [legs] and ");
		MainScreen.text("expose your [vagina] to [Arian eir] viewing pleasure.  The reaction is almost instantaneous; Arian's ");
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] >= 3) MainScreen.text("exposed");
		else MainScreen.text("hidden");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text(" twin shafts immediately fill");
		else MainScreen.text(" shaft immediately fills");
		MainScreen.text(" with blood, growing fully erect in ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("their");
		else MainScreen.text("its");
		MainScreen.text(" bulbous, throbbing glory.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
		MainScreen.text(" and get ready to thrust in; otherwise neither of you are going to be feeling good any time soon.");

		MainScreen.text("\n\nSnapping to [Arian eir] senses, Arian quickly ");
		MainScreen.text("kneels between your [legs]");
		MainScreen.text(", aligning the tip of ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("each of ");
		MainScreen.text(arianMF("his", "her") + " ");
		MainScreen.text("shaft");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
		MainScreen.text(" with your [vagina]");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text(" and [asshole]");
		MainScreen.text(".  [Arian Ey] looks at you, waiting for you to confirm that [Arian ey] should indeed get going.  You smile and nod");
		if (player.hasLongTail()) MainScreen.text(", looping your tail around [Arian eir] waist");
		MainScreen.text(".");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) {
			...
			if (player.hasLongTail()) MainScreen.text(", further encouraging [Arian em] to do so, by looping your tail around [Arian eir] waist");
			MainScreen.text(".  The lizan doesn't need much encouragement to comply, easily snuggling against you, breast to breast, and sighing softly.  \"<i>I'm sorry.  Sometimes I wonder why you bother doing something like this, with someone like me,</i>\" [Arian ey] says, a hint of sadness in [Arian eir] voice.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
		MainScreen.text(" sit inside you.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("twin bulbous shafts feel");
		else MainScreen.text("bulbous shaft feels");
		MainScreen.text(" exquisite, and ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("their");
		else MainScreen.text("its");
		MainScreen.text(" bumps massage your entrance");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
		MainScreen.text(" unlike anything else.  You praise the lizan on being a quick study, letting go of [Arian eir] tail and grabbing onto [Arian eir] scaly shoulders instead, giving [Arian em] a few more directions so [Arian ey] can catch your most sensitive spots.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("and [ass] ");
		MainScreen.text("with [Arian eir] sticky wet seed.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
		MainScreen.text(" swelling inside you, the lizan grabbing your shoulders and starting to thrust [Arian em]self back into you once more.");
		...
		if (Flags.list[FlagEnum.ARIAN_VAGINA] == 0) MainScreen.text("[Arian ey] himself ");
		else MainScreen.text("she ");
		MainScreen.text("is being overwhelmed by, spraying your ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] == 0) MainScreen.text("cavity");
		else MainScreen.text("cavities");
		MainScreen.text(" with a second helping of lizan spunk, dredging up every last drop of jizz left in [Arian eir] internal balls before, with a groan, [Arian ey] sinks down atop you.  Arian's orgasm pushes you over the edge, and you find your pussy ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("and ass ");
		MainScreen.text("contracting, trying ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] == 0) MainScreen.text("its");
		else MainScreen.text("their");
		MainScreen.text(" best to milk the poor lizan of all [Arian ey] is worth, until finally with one last spasm, you slump down and release the lizan shaft");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
		MainScreen.text(" deeply embedded inside you, some of your mixed juices running down to mat Arian's bed.");
	...
	private doublePenetrateArian(): void {
		...
		Flags.list[FlagEnum.ARIAN_VIRGIN] += 1;
		arianHealth(3);
		MainScreen.text(images.showImage("arianfemale-home-doublepenetrate"));
		...
		MainScreen.text("You look over your feminine lizard lover, and feel your " + CockDescriptor.describeMultiCockShort(player) + " starting to stir in your [armor].  Since you have enough tools for the job, and Arian has enough holes, you ask if Arian would be willing to let you fuck her in both her ass and pussy at the same time?");
		...
		if (Flags.list[FlagEnum.ARIAN_ANAL_XP] <= 33) MainScreen.text("tight");
		else if (Flags.list[FlagEnum.ARIAN_ANAL_XP] <= 66) MainScreen.text("loose");
		else MainScreen.text("throbbing");
		MainScreen.text(" ass and dripping wet vagina; a pair of clawed hands reach behind to spread her butt open for you.  \"<i>Okay, I'm ready.</i>\"  Arian says, looking behind at you.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_PENETRATION_CHAT] == 0) {
			...
			if (!arianFollower()) MainScreen.text("bedchambers");
			else MainScreen.text("tent");
			MainScreen.text(".");
		}
		else {
	...
	private arianDocking(): void {
		...
		arianHealth(3);
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0)
			MainScreen.text(images.showImage("arianfemale-home-docking"));
		else
			MainScreen.text(images.showImage("arianmale-home-docking"));
		MainScreen.text("You set your eyes on Arian's genital slit, and then smile at [Arian em].  You ask how [Arian ey]'d feel about ‘hiding' your cock, rather than [Arian eirs], inside [Arian eir] slit?");
		...
		//(if ArianDblCock == 0)
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] == 0) {
			MainScreen.text("didn't [Arian ey] always want to have two dicks?  Well, now it's [Arian eir] chance to find out what it'd be like.");
		}
		else {
			MainScreen.text("[Arian ey]'s already hiding two in there... an extra cock won't make such a huge difference, would it?");
		}
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("pair of ");
		MainScreen.text("pecker");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
		MainScreen.text(" from your prying eyes.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
		MainScreen.text(" of Arian's dick");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
		MainScreen.text(".  You brush your fingers against the tip");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
		MainScreen.text(", seeing what kind of reaction that will elicit from your scaly lover.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] == 0) MainScreen.text(" brushing up against [Arian eir] own lizan pecker");
		else MainScreen.text("sandwiched between [Arian eir] two lizard dicks");
		MainScreen.text(", the friction is absolutely incredible, sending sparks of pleasure cascading along your shaft.  You moan in delicious lust and tell Arian that this is absolutely incredible.");
		...
		MainScreen.text("slides across your intruding shaft as you go, and [Arian eir] sheathe-walls grip and squeeze as best they can, leaving you moaning and panting like a bitch in heat.  Arian's moans of pleasure join yours, [Arian eir] twitching bulbous shaft");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
		MainScreen.text(" massaging your [cock smallest] almost like a vagina would.  Shaky hands grab a hold of your shoulders, helping you down and up on Arian's male slit.  \"<i>Oh, Marae!  It feels like I'm getting an internal handjob... it feEels so... so good!  H - harder... almost cumming!</i>\" [Arian ey] pleads, as [Arian ey] breathes hotly on your face; panting like a bitch in heat.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
		MainScreen.text(" out before cumming.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("pair of cummy masts emerge");
		else MainScreen.text("cummy mast emerges");
		MainScreen.text(" to feebly shoot a couple more ropes of jism, before returning limply to ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("their");
		else MainScreen.text("its");
		MainScreen.text(" hiding place.  \"<i>Ahhh,</i>\" is all Arian says, before slumping down.");
	...
	private giveArianAnItem(): void {
		MainScreen.clearText();
		MainScreen.text("Thinking about the many items in your possession, you ask if Arian would be willing to take something for you?");

		//(if ArianHealth < 10) //May not give anything.
		if (Flags.list[FlagEnum.ARIAN_S_DIALOGUE] < 1) {
		...
		else if (Flags.list[FlagEnum.ARIAN_HEALTH] < 20) { //May only give Vitality T.
			...
			if (!player.inventory.items.has(consumables.VITAL_T)) {
		...
		else if (Flags.list[FlagEnum.ARIAN_S_DIALOGUE] == 3 && arianHealth() >= 50) arianImbue();
		else if (Flags.list[FlagEnum.ARIAN_S_DIALOGUE] == 4 && arianHealth() >= 75) arianPlot4();
		else if (Flags.list[FlagEnum.ARIAN_S_DIALOGUE] == 5 && arianHealth() >= 100) arianPlot5();
		else {
			if (player.inventory.items.has(consumables.VITAL_T)) MainScreen.addButton(0, "Vital Tinct", arianVitalityTincture);
			if (Flags.list[FlagEnum.ARIAN_HEALTH] >= 20) {
				if (player.inventory.items.has(consumables.P_DRAFT)) MainScreen.addButton(1, "P. Incubi D", giveIncubusDraftToArian);
				if (player.inventory.items.has(consumables.P_S_MLK)) MainScreen.addButton(2, "P. Suc.Milk", succubiMilkForArian);
				if (player.inventory.items.has(consumables.LACTAID)) MainScreen.addButton(3, "Lactaid", giveArianLactaid);
				if (player.inventory.items.has(consumables.REDUCTO)) MainScreen.addButton(4, "Reducto", giveArianReducto);
				if (player.inventory.items.has(consumables.REPTLUM)) MainScreen.addButton(5, "Reptilum", giveArianReptilum);
			}
			MainScreen.addButton(9, "Back", arianHomeMenu);
		}
	}
	...
	private arianVitalityTincture(): void {
		MainScreen.clearText();
		MainScreen.text("Fishing around amongst your pockets, you withdraw a vial of that strange potion Giacomo peddles and offer it to the sickly lizan, explaining it will bolster [Arian eir] constitution and fill [Arian em] with permanent vitality.");
	...
	private giveIncubusDraftToArian(): void {
		...
		MainScreen.text("Fishing around in your pockets, your hand closes on the vial of purified incubus draft.  You offer this to Arian, asking ");
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text("if [Arian ey] would like to make [Arian eir] cock bigger");
		else MainScreen.text("if she'd like to have a touch of her old malehood back");
		MainScreen.text(".");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 0) {
			Flags.list[FlagEnum.ARIAN_COCK_SIZE]++;
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("[vagina] and ");
			MainScreen.text("[asshole].");
			...
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] == 0) MainScreen.text("shaft emerges from its hiding place.  It throbs and grows, settling in its");
			else MainScreen.text("pair of shafts emerge from their hiding place.  They throb and grow, settling in their");
			MainScreen.text(" new size as Arian moans and cums all over [Arian em]self.");
			...
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s back into their");
			else MainScreen.text(" back into its");
			MainScreen.text(" slit.");
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 2) {
				Flags.list[FlagEnum.ARIAN_COCK_SIZE]++;
				dynStats("lus", 10 + player.stats.lib / 20);
				...
				if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("them");
				else MainScreen.text("it");
				MainScreen.text(" until two thirds are inside, then it doesn't looks like [Arian ey] can take in anymore.");

				if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("\n\n\"<i>W-Wow, they're so big I can't even manage to hide them,</i>\" Arian pants as the tip of [Arian eir] cocks hang limply, exposed for all to see.");
				...
				if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) {
					...
					if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
					MainScreen.text(" starts hardening, no longer able to hide Arian's excitement inside her protective slit.");
				}
				else {
		...
		if (Flags.list[FlagEnum.ARIAN_BREASTS] > 1 && (!Flags.list[FlagEnum.HYPER_HAPPY])) {
			Flags.list[FlagEnum.ARIAN_BREASTS]--;
		...
		else if (Flags.list[FlagEnum.ARIAN_BREASTS] == 1 && (!Flags.list[FlagEnum.HYPER_HAPPY])) { //Now you are male again.
			...
			Flags.list[FlagEnum.ARIAN_BREASTS] = 0;
			Flags.list[FlagEnum.ARIAN_VAGINA] = 0;
	...
	private succubiMilkForArian(): void {
		...
		MainScreen.text("Fishing out the bottle of purified demon's milk, you ask if Arian is willing to get ");
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("further ");
		MainScreen.text("in touch with [Arian eir] feminine side.  For you?");
		...
		if (Flags.list[FlagEnum.ARIAN_VAGINA] == 0) {
			Flags.list[FlagEnum.ARIAN_VAGINA]++;
			if (Flags.list[FlagEnum.ARIAN_BREASTS] == 0) Flags.list[FlagEnum.ARIAN_BREASTS]++;
			...
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
			MainScreen.text(" having already softened and retracted");
			//if ArianCockSize >= 3:
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 3) MainScreen.text(" as far as it'll go");
			MainScreen.text(" into its protective slit.");
		...
		else if (Flags.list[FlagEnum.ARIAN_BREASTS] < 3) {
			Flags.list[FlagEnum.ARIAN_BREASTS]++;
			...
			//(if ArianBreasts == 2)
			if (Flags.list[FlagEnum.ARIAN_BREASTS] == 2) {
				...
				if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] >= 3) MainScreen.text(" and her exposed shaft begins to grow hard");
				MainScreen.text(".");
			}
			else {
				...
				if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 3) MainScreen.text(" and you see her exposed shaft slowly rising to point at you");
				MainScreen.text(".");
		...
		//(else If ArianCockSize > 1) //Dick shrinkage.
		else if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 1 && (!Flags.list[FlagEnum.HYPER_HAPPY])) {
			...
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
			MainScreen.text(".  \"<i>Oh... that's good.  Right?</i>\" Arian looks at you nervously.  You think it over, and confess that while you are more attracted to women than men, you won't force Arian to become fully female if she doesn't want to.");
		...
		else if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 1 && Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] >= 1 && (!Flags.list[FlagEnum.HYPER_HAPPY])) {
			...
			Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] = 0;
		}
		else if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 1 && (!Flags.list[FlagEnum.HYPER_HAPPY])) {
			Flags.list[FlagEnum.ARIAN_COCK_SIZE] = 0;
	...
	private giveArianLactaid(): void {
		...
		MainScreen.text("Your hand closes around the vial of lactation-inducing potion that is Lactaid.  You almost reject it automatically, but then you stop and think.  There's odder things in this world, after all.  You remove the vial and ask Arian if [Arian ey] would be willing to let you see what lizan milk tastes like.");
		...
		if (Flags.list[FlagEnum.ARIAN_BREASTS] == 0) {
			...
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 3) MainScreen.text(", her exposed cock growing hard as she flushes with arousal");
			MainScreen.text(".  Her hands grip the sheets and you watch her ");
			if (Flags.list[FlagEnum.ARIAN_BREASTS] == 1) MainScreen.text("perky");
			else if (Flags.list[FlagEnum.ARIAN_BREASTS] == 2) MainScreen.text("luscious");
			else MainScreen.text("pillowy");
			MainScreen.text(" breasts grow before your eyes.  \"<i>My breasts... d-do something, [name]!</i>\" Arian pleads.  Unsure of what you should be doing, you grab her breasts and begin kneading them as they grow in your hands; her erect nipples poking your palms.  \"<i>Ah... that feels good!  Don't stop!</i>\"");
			...
			//(if ArianBreasts < 3)
			if (Flags.list[FlagEnum.ARIAN_BREASTS] < 3) {
				Flags.list[FlagEnum.ARIAN_BREASTS]++;
	...
	private giveArianReducto(): void {
		MainScreen.clearText();
		MainScreen.text("Eyeing Arian up and down, you fish your pouches for a tube of Reducto.  Once you've found it, you hand it over to Arian and tell [Arian em] you'd like [Arian em] to reduce something for you.");
	...
	private useReductoOnAriansBreasts(): void {
		...
		MainScreen.text("You point at Arian's ");
		if (Flags.list[FlagEnum.ARIAN_BREASTS] == 1) MainScreen.text("small");
		else if (Flags.list[FlagEnum.ARIAN_BREASTS] == 2) MainScreen.text("generous");
		else MainScreen.text("pillowy");
		MainScreen.text(" breasts.  \"<i>Okay... I suppose it would be good to lose a bit of weight up here and spare my back.</i>\"  Arian smiles, opening the tube of Reducto and her robes; then squeezing the contents of the paste her hands.");
		...
		//(if ArianBreasts > 1)
		if (Flags.list[FlagEnum.ARIAN_BREASTS] > 1) {
			...
			MainScreen.text("The two of you watch Arian's breasts, but nothing seems to be changing.  You reach forward to feel them, and indeed they haven't shrunk down at all.");
	...
	private useReductoOnArianCocks(): void {
		MainScreen.clearText();
		player.inventory.items.consumeItem(consumables.REDUCTO);
		MainScreen.text("You point at [Arian eir] crotch, mentioning that you'd like [Arian em] to be smaller.");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 3) {
			...
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("pair of exposed lizard cocks");
			else MainScreen.text("exposed lizard cock");
			MainScreen.text(" into full erection.  Finally [Arian ey] begins applying the paste.");

			MainScreen.text("\n\nThe changes are almost immediate; Arian groans and watches as [Arian eir] oversized dick");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
			MainScreen.text(" slowly shrink");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] == 0) MainScreen.text("s");
			MainScreen.text(" to a more manageable size.  Once the transformation is complete Arian tries to fully retract [Arian eir] cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s back into their");
			else MainScreen.text(" back into its");
			MainScreen.text(" hiding place; it's a tight fit, but it fits.");
			...
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s from their tight hiding place");
			else MainScreen.text(" from its tight hiding place");
			MainScreen.text(", and into full erection.  Finally [Arian ey] begins applying the Reducto.");

			MainScreen.text("\n\nIt takes mere moments for the changes to occur; Arian groans in slight discomfort as [Arian eir] cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s return to their");
			else MainScreen.text(" returns to its");
			MainScreen.text(" original size.  Once the transformation is complete, [Arian ey] retracts ");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("them");
			else MainScreen.text("it");
			MainScreen.text(" back into [Arian eir] genital slit.");
		...
		else if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) {
			MainScreen.text("Then, [Arian ey] coaxes [Arian eir] twin reptilian-peckers out of their hiding place.  Finally [Arian ey] applies the paste to both shafts in turn.");
	...
	private useReductoOnAriansAsshole(): void {
		MainScreen.clearText();
		player.inventory.items.consumeItem(consumables.REDUCTO);
		MainScreen.text("You ask Arian to hand the tube of reducto back over to you, telling [Arian em] that you want to make [Arian em] a little tighter when you do [Arian em] from behind.  The lizard-");
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			if (Flags.list[FlagEnum.ARIAN_VAGINA] == 0) MainScreen.text("man");
			else MainScreen.text("herm");
		}
		else MainScreen.text("woman");
		MainScreen.text(" does as you ask, allowing you to smear your fingers generously with the shrinking cream, even as [Arian ey] anticipates your next request and removes [Arian eir] clothes before lying down, bum up in the air and tail slowly swishing to and fro, [Arian eir] anus exposed and waiting for your ministrations.");
		//(if AnalXP == 1)
		if (Flags.list[FlagEnum.ARIAN_ANAL_XP] <= 1) {
		...
		//(else if AnalXP < 33)
		else if (Flags.list[FlagEnum.ARIAN_ANAL_XP] < 33) {
			...
			Flags.list[FlagEnum.ARIAN_ANAL_XP] = 1;
		}
		else if (Flags.list[FlagEnum.ARIAN_ANAL_XP] < 66) {
			Flags.list[FlagEnum.ARIAN_ANAL_XP] -= 33;
	...
	private giveArianReptilum(): void {
		...
		MainScreen.text("Fingering the vial of reptilium, you smirk to yourself.  Quickly wiping it off your face, you instruct Arian to close [Arian eir] eyes and open [Arian eir] mouth, as you have a special surprise for [Arian em].");
		...
		//(if ArianFirstRept == 1)
		if (Flags.list[FlagEnum.ARIAN_FIRST_REPTILUM] == 0) {
		...
		//(if ArianFirstRept == 1)
		if (Flags.list[FlagEnum.ARIAN_FIRST_REPTILUM] == 1) {
		...
		if (!player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock()) MainScreen.text("genderless crotch");
		MainScreen.text(".  \"<i>Hmm, that looks much tastier than candy actually.</i>\"  [Arian Ey] licks your ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("cock from base to tip");
		else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("pussy all the way from the back to the tip of your [clit]");
		else MainScreen.text("your sweaty crotch");
		MainScreen.text(".  \"<i>Yummy!  I could use seconds, but roleplay time is over; let's... masturbate each other!</i>\"  [Arian Ey] begins stroking [Arian eir] ");

		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0)
			MainScreen.text(images.showImage("arianfemale-home-mutualmasturbation"));
		else
			MainScreen.text(images.showImage("arianmale-home-mutualmasturbation"));

		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] == 1) {
			MainScreen.text("quickly erecting lizan cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" and her ");
		}
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("slavering vagina");
		MainScreen.text(".");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			MainScreen.text("shooting thick ropes of cum up into the air to land on [Arian eir] ");
			if (Flags.list[FlagEnum.ARIAN_BREASTS] == 0) MainScreen.text("chest");
			else if (Flags.list[FlagEnum.ARIAN_BREASTS] == 1) MainScreen.text("perky little breasts");
			else if (Flags.list[FlagEnum.ARIAN_BREASTS] == 2) MainScreen.text("soft breasts");
			else MainScreen.text("pillowy breasts");
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" while ");
		}
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("her vagina doing its best to soak her bed with lizan femcum");
		MainScreen.text(".");
		...
		if (rand(2) == 0 && Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] == 0 && Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
		...
		if (tfed) {
			...
			Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] = 1;
		}
		else {
	...
	private treatCorruption(): void {
		MainScreen.clearText();
		MainScreen.text("You ask Arian if [Arian ey] thinks [Arian ey] can help you reduce some of the taint that has infected your soul.");

		if (Flags.list[FlagEnum.ARIAN_TREATMENT] == 1) {
			...
			player.stats.cor += -1;
			fatigue(20);
			if (player.stats.cor == 0) MainScreen.text(" - in fact, you're quite sure that Arian has purified you entirely");
			MainScreen.text(".  You thank the lizan for [Arian eir] magical treatment.");
			...
			Flags.list[FlagEnum.ARIAN_TREATMENT]++;
			doNext(camp.returnToCampUseOneHour);
		}
	}
	...
	private imbueTalisman(): void {
		MainScreen.clearText();
		MainScreen.text("You tell Arian that, if it's not too much trouble, you'd like [Arian em] to ");
		if (player.hasKeyItem("Arian's Talisman") >= 0) MainScreen.text("place a spell in the enchanted talisman [Arian ey] created for you");
		else MainScreen.text("change the spell in the talisman [Arian ey] created for you");
	...
	private arianSpellPlace(spell: string): void {
		MainScreen.clearText();
		MainScreen.text("You tell Arian that you want [Arian em] to place the " + spell + " spell in your talisman for you.");
		...
		if (spell == "Shielding Spell") {
			...
			//Shielding Spell: 2x Black Chitin and 1x Tough Silk. - Increases defense for the duration of the battle.
			player.inventory.items.consumeItem(useables.B_CHITN, 2);
		...
		if (spell == "Immolation Spell") {
			...
			//Immolation Spell: 2x Goblin Ale and 1x Sweet Gossamer. - Deals damage over time.
			player.inventory.items.consumeItem(consumables.GOB_ALE, 2);
			player.inventory.items.consumeItem(consumables.S_GOSSR);
		}
	...
	*/
	...
	public sleepWithArian(newl: boolean = false): void {
		...
		MainScreen.text("Tired after a whole day of adventuring, you decide to retire and catch some shut-eye.  While going through the day's events, you recall Arian had offered to let you stay in [Arian eir] tent and sleep with [Arian em] in [Arian eir] bed.  Your tired body could surely use a soft bed today, and maybe a certain lizan to keep you company too.  With that in mind, you head to [Arian eir] tent.");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			MainScreen.text("You can clearly see [Arian eir] exposed cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
			MainScreen.text(" stiffening at the sight.  ");
		}
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("You can tell that [Arian ey] is aroused by the little show, despite [Arian eir] body doing a good job of hiding it.  ");
		MainScreen.text("You just let [Arian em] squirm, pretending you don't see [Arian eir] reactions, and then, indicating the bed, you ask which of you should get in first.");
	...
	private dontListenToLowAnalXPArian(): void {
		MainScreen.clearText();
		MainScreen.text("You gently stroke the lizan's side and plant a soft, gentle kiss on [Arian eir] neck.  At your ministrations, Arian sighs and slowly falls still again.  You wait, but it seems that whatever dreams [Arian ey] was having have faded and [Arian ey]'s truly asleep now, allowing you to go back to sleep yourself.");
		menu();
	...
	private listenToLowAnalXPArian(): void {
		...
		MainScreen.text("You turn you head to try and catch what Arian might be dreaming about.");
	...
	private listenToMediumAnalXPArian(): void {
		...
		if (player.lowerBody.cockSpot.hasCock()) {
			...
			MainScreen.text(" still worming its way into [Arian eir] fuckable ass, but it really does look like [Arian ey]'s fast asleep.  Weird.  \"<i>So many muffins... all [name] flavored...</i>\"  [Arian ey] calms down and begins snoring lightly.");
			...
			if (player.stats.cor >= 66) MainScreen.text(", despite the silly lizan being deserving of it");
			MainScreen.text(".  But as payback, [Arian ey]'s going to sleep with you inside, not that it seems like it'll be much of a problem to [Arian em]... Too bad you can't hold an erection all the time while asleep, so you know eventually you'll slip out...");
		}
		else {
	...
	private TeaseHighAnalXPArian(): void {
		...
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0)
			MainScreen.text(images.showImage("arianfemale-camp-dreamingArian"));
		else
			MainScreen.text(images.showImage("arianmale-camp-dreamingArian"));
		Flags.list[FlagEnum.ARIAN_MORNING] = 1;
	...
	public wakeUpAfterArianSleep(): void {
		...
		if (player.lowerBody.cockSpot.hasCock()) {
			...
			if (Flags.list[FlagEnum.ARIAN_ANAL_XP] < 33) {
				...
				MainScreen.text("Your eyes open, and you suddenly realize that whatever dreams you were having... are now gone.  You make note of the still sleeping lizan right next to you and recall last night's events.  You wonder if Arian will remember any of that...");
				...
				if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) {
					MainScreen.text(arianMF("his", "her") + " pussy is squirting her juices all over your lower body");
					if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
						MainScreen.text(" and ");
					}
					else MainScreen.text("; ");
					...
					MainScreen.text(arianMF("his", "her") + " ");
					if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("twin shafts are");
					else MainScreen.text("shaft is");
					MainScreen.text(" squirting white ropes of lizan cum all over the bedsheets;");
				}
				MainScreen.text(" what a naughty little lizan.");

				MainScreen.text("\n\nYou can't resist playfully teasing Arian that [Arian ey] is such a buttslut.  ");
				if (!player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("Did [Arian ey] always know [Arian ey] wanted a man to top [Arian em], or is this development new to [Arian em]?");
				else MainScreen.text("Does [Arian ey] really get off on having a girl with a dick all her own fuck [Arian em] like a girl, hmm?");
			...
			if (Flags.list[FlagEnum.ARIAN_ANAL_XP] < 33) {
				...
				if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
					MainScreen.text(arianMF("his", "her") + " ");
					if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] >= 3) MainScreen.text("exposed ");
					MainScreen.text("bulbous shaft");
					if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s begin");
					else MainScreen.text(" begins");
					MainScreen.text(" sporting an erection");
					if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] < 3) {
						MainScreen.text(", poking ");
						if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] == 1) MainScreen.text("its tip");
						else MainScreen.text("their tips");
						MainScreen.text(" out of Arian's genital slit");
					}
					MainScreen.text(".");
				}
				else if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("and her moist pussy, begins forming a stream of juices that gently run its course through the inside of Arian's thighs, evaporating before hitting the floor due to the lizan's currently elevated body heat.");
				MainScreen.text("  [Arian Ey] pants, eyes glazed.  \"<i>Uhh... [name]...</i>\"");
		...
		Flags.list[FlagEnum.ARIAN_MORNING] = 0;
		doNext(playerMenu);
	}
	...
	public arianEggingEvent(): void {
		...
		if (Flags.list[FlagEnum.ARIAN_EGG_CHAT] == 0) {
			Flags.list[FlagEnum.ARIAN_EGG_CHAT]++;
			MainScreen.text("As you are about to enter Arian's tent, you hear a moan emanate from within.  Those aren't moans of pleasure though.  They are moans of discomfort.  Wondering if the lizan is all right, you decide to enter her tent.");
			...
			MainScreen.text("As you are about to enter Arian's tent, you hear a moan emanate from within.  Seems like it's that time of the month again...");
		...
		MainScreen.addButton(3, "Pink", pickAnEggArian, "pink");
		MainScreen.addButton(4, "White", pickAnEggArian, "white");
		MainScreen.addButton(5, "Rubber", pickAnEggArian, "rubbery black");
		MainScreen.addButton(6, "No Thanks", leaveEggs);
	}
	...
	private pickAnEggArian(color: string = "pink"): void {
		...
		MainScreen.text("You tell Arian you'd like her to make you a " + color + " egg.");
		...
		Flags.list[FlagEnum.ARIAN_EGG_COUNTER] = 1;
		doNext(camp.returnToCampUseOneHour);
	}
	//Leave
	private leaveEggs(): void {
		MainScreen.clearText();
		MainScreen.text("You tell her that you don't want any colored eggs from her this month.  The lizan nods, deciding it's not necessary for her to spell out that this means she'll just produce ordinary eggs and eat them for breakfast after she's laid them.  \"<i>So, do you want something?</i>\" she asks.");
		doNext(camp.returnToCampUseOneHour);
	}
	...
	public arianLaysEggs(): void {
		...
		MainScreen.text("As you approach the tent, you wonder if Arian's ready to lay those eggs...  Your contemplations are promptly interrupted as you hear a pained moan coming from inside.  Seems like Arian is in labor!  Without delay, you rush inside, to be greeted by the sight of naked lizan-");
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text("herm");
		else MainScreen.text("girl");
		MainScreen.text(" squatting over a wooden basin, one hand between her legs, massaging her dripping treasure, while the other is rubbing her belly to try and coax the eggs out of her.");
		...
		let itype: ItemType;
		if (Flags.list[FlagEnum.ARIAN_EGG_COLOR] == "brown") itype = consumables.L_BRNEG;
		else if (Flags.list[FlagEnum.ARIAN_EGG_COLOR] == "purple") itype = consumables.L_PRPEG;
		else if (Flags.list[FlagEnum.ARIAN_EGG_COLOR] == "blue") itype = consumables.L_BLUEG;
		else if (Flags.list[FlagEnum.ARIAN_EGG_COLOR] == "pink") itype = consumables.L_PNKEG;
		else if (Flags.list[FlagEnum.ARIAN_EGG_COLOR] == "white") itype = consumables.L_WHTEG;
		else if (Flags.list[FlagEnum.ARIAN_EGG_COLOR] == "rubbery black") itype = consumables.L_BLKEG;
	...
	private arianDildoFun(): void {
		...
		MainScreen.clearText();
		//1st time:
		if (Flags.list[FlagEnum.TIMES_ARIAN_DILDOED] == 0) {
			MainScreen.text("Looking over your reptilian girlfriend, your eyes go down her body");
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
				MainScreen.text(", past her cock");
				if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
			}
			else MainScreen.text(",");
			MainScreen.text(" to her dripping wet pussy, clearly yearning for some action.  Reflecting that your own love-lips feel the same way, you decide to try something fun and present to Arian the dildo that you ");
			if (player.hasKeyItem("Dildo") >= 0) MainScreen.text("bought from Giacomo");
			else MainScreen.text("got from Tamani");
			MainScreen.text(", asking if she's willing to give this a try.");
			...
			MainScreen.text("You present your dildo to Arian again, telling her that this time, you want to play with your toys - after all, she enjoyed it so much the first time.");

			MainScreen.text("\n\nArian fidgets in embarrassment.  \"<i>Okay.</i>\"");
		}
		Flags.list[FlagEnum.TIMES_ARIAN_DILDOED]++;
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			MainScreen.text("Ignoring her cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
			MainScreen.text(", y");
		}
		else MainScreen.text("Y");
		MainScreen.text("ou slide first one finger into her slick pussy and then another, gently caressing her inner labia to see if you can find her sweet spots.  Arian's eyes open in surprise as the first pang of pleasure hits her, she moans into your kiss.  Her tongue darts to dance with your own in excitement.");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			MainScreen.text("brushing it tantalizingly across the sensitive skin of her pre-oozing cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
			MainScreen.text(" before");
		}
		MainScreen.text(" brushing it against her outer lips, stroking it up and down each labia, rubbing it against her clitoris, gently pushing the very tip of it into her cunt only to then withdraw it before it can really start to stretch her cunt.");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			MainScreen.text("Her cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s throb");
			else MainScreen.text("throbs");
			MainScreen.text(" throb as a jet of whiteness escape them.  ");
		}
		MainScreen.text("You can feel it... the suddenness of your penetration brought the poor lizan mage to an instant orgasm.");
