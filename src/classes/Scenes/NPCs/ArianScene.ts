...
public function visitThePark():void {
	...
	if (flags[FlagEnum.ARIAN_PARK] == 1) {
		outputText("\n\nFeeling ");
		if(player.stats.cor < 50) outputText("curious");
	...
	//Visit 3
		...
		if(player.stats.cor > 66) outputText(", whilst privately thinking to yourself how useless that looks - no wonder they can't fight the demons if this is the best they're capable of");
		outputText(".");
		...
		if(player.stats.cor >= 66) outputText(", keeping your real reasons for bothering with him to yourself");
		outputText(".");
...
public function visitAriansHouse():void {
	...
	if(arianFollower()) {
		...
		if(temp == 0) {
			...
			if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
				if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText("mixed sexual fluids");
				else outputText("jizz");
			}
			else outputText("femjizz");
			outputText(" and, indeed, still hovering over [Arian eir] ");
			if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
				if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText("twin sexual slits");
				else {
					outputText("cock");
					if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("s");
				}
			}
		...
		if(flags[FlagEnum.ARIAN_PARK] == 4) {
			...
			if(player.stats.cor < 33) outputText("curiously");
			else if(player.stats.cor < 66) outputText("casually");
...
private function arianStoryDialogue1():void {
	...
	//PC has met Marae: 
	if(player.statusAffects.has("MetMarae")) outputText("Marae herself told you they showed up about, what, 20-30 years ago?");
...
private function arianStoryPry():void {
	...
	if (player.stats.cor < 40) outputText("; he'll sleep better with the burden off [Arian eir] conscience");
	outputText(".");
...
private function inviteArianToCamp():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
		if(flags[FlagEnum.ARIAN_VAGINA] == 0) outputText("man");
...
private function giveArianAnal():void {
	...
	if(flags[FlagEnum.ARIAN_ANAL_XP] < 33) {
		...
		if(flags[FlagEnum.ARIAN_VAGINA] < 1) {
			...
			if(flags[FlagEnum.ARIAN_COCK_SIZE] == 0) outputText("girl");
			else outputText("herm");
		...
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) {
			...
			if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) outputText(" and ");
		}
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			outputText(arianMF("his","her") + " erecting ");
			if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("pair of reptilian pricks");
		...
		if(player.lowerBody.balls == 0) outputText("the base of your cock");
		...
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) outputText("spewing forth large gobs of cum of [Arian eir] own");
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0 && flags[FlagEnum.ARIAN_VAGINA] > 0) outputText(" and ");
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText("wetting both your and her legs with a healthy serving of lizan pussy juice");
	...
	//Loose, sensitive enough to make Arian scream in pleasure. (It's like a tight vagina, feels like one too, minor pain)
		...
		if(flags[FlagEnum.ARIAN_VAGINA] < 1) {
			...
			if(flags[FlagEnum.ARIAN_BREASTS] == 1) outputText(" perky");
			else if(flags[FlagEnum.ARIAN_BREASTS] == 2) outputText(" rounded");
		...
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			outputText(" a");
			if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText(" pair of dripping cocks");
			else outputText(" dripping cock");
			if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText(" and");
		}
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText(" a sopping wet pussy");
		outputText(" greet");
		if((flags[FlagEnum.ARIAN_VAGINA] == 0 && flags[FlagEnum.ARIAN_DOUBLE_COCK] == 0) || (flags[FlagEnum.ARIAN_VAGINA] == 1 && flags[FlagEnum.ARIAN_COCK_SIZE] == 0)) outputText("s");
		...
		if(flags[FlagEnum.ARIAN_VAGINA] < 1) outputText("boypussy");
		...
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			if(flags[FlagEnum.ARIAN_DOUBLE_COCK] == 0) outputText("cock spews its load");
			else outputText("twin cocks spew their loads");
			outputText(" on the bedsheets");
			if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText(" and [Arian eir] ");
		}
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText(" contracting pussy paints your lower body with lizan femcum");
		...
		if(player.lowerBody.balls == 0) outputText("base of your spine");
	...
	//Very Loose, sensitive enough to make Arian cum just from insertion. (Feels better than anything else. Yep, Arian really becomes a buttslut at this point)
		...
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText("wet gash is positively leaking lizan juices");
		if(flags[FlagEnum.ARIAN_VAGINA] > 0 && flags[FlagEnum.ARIAN_COCK_SIZE] > 0) outputText(" and [Arian eir] ");
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) outputText("erect cock is already leaking pre like a sieve");
		...
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText("her pussy clenches, spilling a flood of juices; ");
		if(flags[FlagEnum.ARIAN_VAGINA] > 0 && flags[FlagEnum.ARIAN_COCK_SIZE] > 0) outputText(" and ");
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) outputText("his cock throbs and shoots rope after rope of cum onto the bedsheets");
		...
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("cocks are still rock hard, and surprisingly, still leaking ropes of cum");
			else outputText("cock is still rock hard, and surprisingly, still leaking ropes of cum");
			if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText(", and her");
		}
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText(" pussy squirting juices against your [legs] with each shuddering impact of your [hips]");
		...
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) outputText("  [Arian Eir] throbbing cock flexes and shoots one last rope of lizan-cum against your belly.");
...
private function getBlownByArian():void {
	...
	if(flags[FlagEnum.ARIAN_HAS_BLOWN] == 0) {
		...
		if(flags[FlagEnum.ARIAN_COCK_SIZE] == 3) outputText("  You notice Arian's exposed shaft slowly hardening at your invitation.");
...
private function penetrateArian():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) outputText("dick");
	outputText("girl.");
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] == 3) {
		...
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("s peeking out of their");
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] == 0) outputText("female");
	else outputText("herm");
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
		outputText("  Her ");
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("twin shafts");
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
		outputText("  Her cock");
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("s twitch as they spurt ");
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
		outputText("Arian's ");
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("twin cocks throb");
	...
	if(player.lowerBody.balls == 0) outputText("you");
	else outputText("your [balls]");
	outputText(".");
	
	outputText("\n\nFinally having kept your promise, you give yourself over to the inevitable. ");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" A wash of fluids spills down your body from your cunt as it spasms in release and y");
	else outputText("Y");
...
private function getButtWreckedByArian():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] >= 3) {
		...
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("s harden");
		else outputText(" hardens");
		outputText(" to full mast.");
	}
	else {
		outputText(arianMF("his","her") + " cock");
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("s peek out of their");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("primary ");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("stroking [Arian eir] other cock and sliding it between your ass cheeks, ");
	...
	if(flags[FlagEnum.ARIAN_ANAL_XP] > 66) outputText("eager ");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("pair of lizan-cocks practically jump");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("sliding [Arian eir] neglected prick through your ass-cheeks, ");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("cunt dripping with arousal");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("s twitch");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText(", as well as your butt");
	outputText(", as Arian climaxes");
	if (flags[FlagEnum.ARIAN_VAGINA] > 0) outputText(", her own pussy clenching and drizzling juices on the matted sheets beneath the two of you.");
	else outputText(".");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  [EachCock] spasms and shoots rope after rope of cum, painting Arian's [arian chest] white, as well as the groaning lizan's face.");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your [pussy], though unattended, joins in the perverted display, leaking copious amount of fluids on top of the prone lizan.");
...
private function suckAriansDick():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 3) {
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("exposed cocks hardening to full mast");
		else outputText("exposed cock hardening to full mast");
	}
	else {
		outputText("cock");
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("s emerging from their");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("uppermost ");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("twin ");
	outputText("shaft");
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("s tremble");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText(" and chin");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText(", oblivious to the second cock spewing cum all over your [chest] in your eagerness");
	...
	if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText(" and tickle her cunt with a finger");
...
private function getPenetratedByArianAndHisHitlerMustache():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] < 3) outputText("slit, and think of what it's hiding in there");
	else {
		outputText("exposed cock");
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("s");
	}
	outputText("; then you turn to look at Arian and ask [Arian em] how would [Arian ey] feel about giving [Arian eir] ");
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("twin cocks");
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] >= 3) outputText("exposed");
	else outputText("hidden");
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText(" twin shafts immediately fill");
	else outputText(" shaft immediately fills");
	outputText(" with blood, growing fully erect in ");
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("their");
	else outputText("its");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("each of ");
	outputText(arianMF("his","her") + " ");
	outputText("shaft");
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("s");
	outputText(" with your [vagina]");
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText(" and [asshole]");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("twin bulbous shafts feel");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("and [ass] ");
	...
	if(flags[FlagEnum.ARIAN_VAGINA] == 0) outputText("[Arian ey] himself ");
	else outputText("she ");
	outputText("is being overwhelmed by, spraying your ");
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] == 0) outputText("cavity");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] == 0) outputText("its");
...
private function doublePenetrateArian():void {
	...
	if(flags[FlagEnum.ARIAN_ANAL_XP] <= 33) outputText("tight");
	else if(flags[FlagEnum.ARIAN_ANAL_XP] <= 66) outputText("loose");
	...
	//(Medium Cum Amount)
		...
		if(player.lowerBody.balls == 0) outputText("themselves");
		else outputText("your [balls]");
...
private function arianDocking():void {
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("pair of ");
	outputText("pecker");
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("s");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] == 0) outputText(" brushing up against [Arian eir] own lizan pecker");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("pair of cummy masts emerge");
...
private function giveIncubusDraftToArian():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) outputText("if [Arian ey] would like to make [Arian eir] cock bigger");
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] == 0) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("[vagina] and ");
		outputText("[asshole].");
		...
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] == 0) outputText("shaft emerges from its hiding place.  It throbs and grows, settling in its");
		...
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("s back into their");
...
private function succubiMilkForArian():void {
	...
	if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText("further ");
	...
	if(flags[FlagEnum.ARIAN_VAGINA] == 0) {
		...
		//if ArianCockSize >= 3:
		if(flags[FlagEnum.ARIAN_COCK_SIZE] == 3) outputText(" as far as it'll go");
		...
		if(flags[FlagEnum.ARIAN_BREASTS] == 2) {
			...
			if(flags[FlagEnum.ARIAN_COCK_SIZE] >= 3) outputText(" and her exposed shaft begins to grow hard");
			outputText(".");
		}
		else {
			...
			if(flags[FlagEnum.ARIAN_COCK_SIZE] == 3) outputText(" and you see her exposed shaft slowly rising to point at you");
			outputText(".");
...
private function giveArianLactaid():void {
	...
	if(flags[FlagEnum.ARIAN_BREASTS] == 0) {
		...
		if(flags[FlagEnum.ARIAN_COCK_SIZE] == 3) outputText(", her exposed cock growing hard as she flushes with arousal");
		outputText(".  Her hands grip the sheets and you watch her ");
		if(flags[FlagEnum.ARIAN_BREASTS] == 1) outputText("perky");
		else if(flags[FlagEnum.ARIAN_BREASTS] == 2) outputText("luscious");
		else outputText("pillowy");
...
private function useReductoOnAriansBreasts():void {
	...
	if(flags[FlagEnum.ARIAN_BREASTS] == 1) outputText("small");
	else if(flags[FlagEnum.ARIAN_BREASTS] == 2) outputText("generous");
	else outputText("pillowy");
...
private function useReductoOnArianCocks():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] == 3) {
		...
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("pair of exposed lizard cocks");
		...
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("s from their tight hiding place");
		...
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("s return to their");
...
private function useReductoOnAriansAsshole():void {
	...
	//(else if AnalXP < 33)
		...
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			...
			if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("s suddenly jolting from their sheathe");
			else outputText(" suddenly jolting from its sheathe");
			if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText(" and ");
		}
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) {
			if(flags[FlagEnum.ARIAN_COCK_SIZE] == 0) outputText(", ");
...
private function giveArianReptilum():void {
	...
	if(!player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock()) outputText("genderless crotch");
	outputText(".  \"<i>Hmm, that looks much tastier than candy actually.</i>\"  [Arian Ey] licks your ");
	if(player.lowerBody.cockSpot.hasCock()) outputText("cock from base to tip");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("pussy all the way from the back to the tip of your [clit]");
	...
	if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText("slavering vagina");
	outputText(".");
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
		...
		if(flags[FlagEnum.ARIAN_BREASTS] == 0) outputText("chest");
		else if(flags[FlagEnum.ARIAN_BREASTS] == 1) outputText("perky little breasts");
		else if(flags[FlagEnum.ARIAN_BREASTS] == 2) outputText("soft breasts");
		else outputText("pillowy breasts");
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText(" while ");
	}
	if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText("her vagina doing its best to soak her bed with lizan femcum");
	outputText(".");
...
private function treatCorruption():void {
	...
	if(flags[FlagEnum.ARIAN_TREATMENT] == 1) {
		...
		dynStats("cor", -1);
		fatigue(20);
		if(player.stats.cor == 0) outputText(" - in fact, you're quite sure that Arian has purified you entirely");
...
public function sleepWithArian(newl:boolean = false):void {
	...
	if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText("You can tell that [Arian ey] is aroused by the little show, despite [Arian eir] body doing a good job of hiding it.  ");
...
private function listenToMediumAnalXPArian():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.stats.cor >= 66) outputText(", despite the silly lizan being deserving of it");
	...
	if (player.lowerBody.cockSpot.hasCock()) {
		...
		if(flags[FlagEnum.ARIAN_ANAL_XP] < 33) {
			...
			if(flags[FlagEnum.ARIAN_VAGINA] > 0) {
				...
				outputText(arianMF("his","her") + " ");
				if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("twin shafts are");
			...
			if(!player.lowerBody.vaginaSpot.hasVagina()) outputText("Did [Arian ey] always know [Arian ey] wanted a man to top [Arian em], or is this development new to [Arian em]?");
		...
		if(flags[FlagEnum.ARIAN_ANAL_XP] < 33) {
			...
			if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
				...
				if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("s begin");
				else outputText(" begins");
				outputText(" sporting an erection");
				if(flags[FlagEnum.ARIAN_COCK_SIZE] < 3) {
					outputText(", poking ");
					if(flags[FlagEnum.ARIAN_DOUBLE_COCK] == 1) outputText("its tip");
					else outputText("their tips");
					outputText(" out of Arian's genital slit");
				}
				outputText(".");
			}
			else if(flags[FlagEnum.ARIAN_VAGINA] > 0) outputText("and her moist pussy, begins forming a stream of juices that gently run its course through the inside of Arian's thighs, evaporating before hitting the floor due to the lizan's currently elevated body heat.");
...
public function arianLaysEggs():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) outputText("herm");
...
private function arianDildoFun():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
		outputText("Her cock");
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) outputText("s throb");
