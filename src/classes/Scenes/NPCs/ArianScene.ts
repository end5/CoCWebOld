...
public meetArian():void {
	...
	menu();
	MainScreen.addButton(0,"Help",helpArianWhenYouMeetHim);
	MainScreen.addButton(1,"Don't Help",dontHelpArianWhenYouMeetHim);
	MainScreen.addButton(2,"Never Help",dontHelpArianWhenYouMeetHim,true);
}
...
public visitAriansHouse():void {
	...
	if(arianFollower()) {
		...
		if(temp == 0) {
			...
			if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
				if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("mixed sexual fluids");
				else MainScreen.text("jizz");
			}
			else MainScreen.text("femjizz");
			MainScreen.text(" and, indeed, still hovering over [Arian eir] ");
			if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
				if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("twin sexual slits");
				else {
					MainScreen.text("cock");
					if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
				}
			}
		...
		if(flags[FlagEnum.ARIAN_PARK] == 4) {
			...
			if(!player.isTaur()) MainScreen.text("pull up a chair");
			...
			//50-74 health
				...
				if(!player.isTaur()) MainScreen.text("take the offered chair");
			...
			//75-100 health:
				...
				if(!player.isTaur()) MainScreen.text(" and offers you the chair nearby");
...
private arianHomeMenu():void {
	menu();
	if(flags[FlagEnum.ARIAN_S_DIALOGUE] == 0 && arianHealth() >= 10) MainScreen.addButton(0,"Next",arianStoryDialogue1);
	else if(flags[FlagEnum.ARIAN_S_DIALOGUE] == 1 && arianHealth() >= 20) MainScreen.addButton(0,"Next",arianStoryDialogue2);
	else if(flags[FlagEnum.ARIAN_S_DIALOGUE] == 2 && arianHealth() >= 30) MainScreen.addButton(0,"Next",arianDialogue3);
	else if(flags[FlagEnum.ARIAN_S_DIALOGUE] == 3 && arianHealth() >= 50) MainScreen.addButton(0,"Next",arianImbue);
	else if(flags[FlagEnum.ARIAN_S_DIALOGUE] == 4 && arianHealth() >= 75) MainScreen.addButton(0,"Next",arianPlot4);
	else if(flags[FlagEnum.ARIAN_S_DIALOGUE] == 5 && arianHealth() >= 100) MainScreen.addButton(0,"Next",arianPlot5);
	//If no story dialogue
	else {
		MainScreen.addButton(0,"Talk",talkToArianChoices);
		if(flags[FlagEnum.ARIAN_S_DIALOGUE] >= 2) MainScreen.addButton(1,"Sex",arianSexMenu);
		if(flags[FlagEnum.ARIAN_S_DIALOGUE] >= 1) MainScreen.addButton(3,"Give Item",giveArianAnItem);
		...
		if(flags[FlagEnum.ARIAN_S_DIALOGUE] >= 5) MainScreen.addButton(4,"Treat Corr.",treatCorruption);
		if(model.time.hours >= 17 && arianFollower()) MainScreen.addButton(8,"Sleep With",sleepWithArian,true);
		if(flags[FlagEnum.SLEEP_WITH] == "Arian") MainScreen.addButton(8,"NoSleepWith",dontSleepWithArian);
...
private peepOnArian():void {
	...
	if(player.lizardCocks() > 0) MainScreen.text(" just like yours");
...
private talkToArianChoices():void {
	...
	menu();
	if(flags[FlagEnum.ARIAN_VIRGIN] > 0) MainScreen.addButton(0,"Sexy Talk",arianSexingTalk);
	if(flags[FlagEnum.ARIAN_S_DIALOGUE] >= 3) MainScreen.addButton(1,"Teach Magic",arianMagicLessons);
	if(!arianFollower() && flags[FlagEnum.ARIAN_S_DIALOGUE] >= 6) MainScreen.addButton(4,"Invite2Camp",inviteArianToCamp);
	if(flags[FlagEnum.ARIAN_VIRGIN] == 0 && flags[FlagEnum.ARIAN_S_DIALOGUE] < 3) MainScreen.text("\n\n<b>Arian doesn't have much to talk about right now.  Maybe you ought to just visit him from time to time or find him an item that would help combat [Arian eir] sickness.</b>");
...
private inviteArianToCamp():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
		if(flags[FlagEnum.ARIAN_VAGINA] == 0) MainScreen.text("man");
...
private arianSexMenu(output:boolean = true):void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.cockThatFits(50) >= 0 && player.cockThatFits2(50) >= 0 && flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.addButton(8,"Double Pen",doublePenetrateArian);
		//Docking
		//ArianCockSize needs to be below 3. (ArianDblCock does not affect this decision.) 
		//PC cock area must be <= 30.
		if(player.smallestCockArea() <= 30 && flags[FlagEnum.ARIAN_COCK_SIZE] < 3 && flags[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.addButton(7,"Docking",arianDocking);
	}
	//Get Anal:
	//Arian must have a cock.
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.addButton(1,"Anal - Catch",getButtWreckedByArian);
	//Blow:
	//Arian must have a cock.
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.addButton(3,"Blow " + arianMF("Him","Her"),suckAriansDick);
	
	//PC must have a vagina.
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		//Get Penetrated:
		//Arian must have a cock.
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.addButton(5,"Get Fucked",getPenetratedByArianAndHisHitlerMustache);
...
private giveArianAnal():void {
	...
	if(flags[FlagEnum.ARIAN_ANAL_XP] < 33) {
		...
		if(flags[FlagEnum.ARIAN_VAGINA] < 1) {
			...
			if(flags[FlagEnum.ARIAN_COCK_SIZE] == 0) MainScreen.text("girl");
			else MainScreen.text("herm");
		...
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) {
			...
			if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text(" and ");
		}
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			MainScreen.text(arianMF("his","her") + " erecting ");
			if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("pair of reptilian pricks");
		...
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text("spewing forth large gobs of cum of [Arian eir] own");
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0 && flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" and ");
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("wetting both your and her legs with a healthy serving of lizan pussy juice");
	...
	//Loose, sensitive enough to make Arian scream in pleasure. (It's like a tight vagina, feels like one too, minor pain)
		...
		if(flags[FlagEnum.ARIAN_VAGINA] < 1) {
			...
			if(flags[FlagEnum.ARIAN_BREASTS] == 1) MainScreen.text(" perky");
			else if(flags[FlagEnum.ARIAN_BREASTS] == 2) MainScreen.text(" rounded");
		...
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			MainScreen.text(" a");
			if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text(" pair of dripping cocks");
			else MainScreen.text(" dripping cock");
			if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" and");
		}
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" a sopping wet pussy");
		MainScreen.text(" greet");
		if((flags[FlagEnum.ARIAN_VAGINA] == 0 && flags[FlagEnum.ARIAN_DOUBLE_COCK] == 0) || (flags[FlagEnum.ARIAN_VAGINA] == 1 && flags[FlagEnum.ARIAN_COCK_SIZE] == 0)) MainScreen.text("s");
		...
		if(flags[FlagEnum.ARIAN_VAGINA] < 1) MainScreen.text("boypussy");
		...
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			if(flags[FlagEnum.ARIAN_DOUBLE_COCK] == 0) MainScreen.text("cock spews its load");
			else MainScreen.text("twin cocks spew their loads");
			MainScreen.text(" on the bedsheets");
			if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" and [Arian eir] ");
		}
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" contracting pussy paints your lower body with lizan femcum");
		MainScreen.text(".  Arian is only capable of moaning and shuddering as [Arian eir] powerful orgasm rocks the poor lizan to [Arian eir] core.  The extra tightness of [Arian eir] contracting butthole increases the friction on your " + cockDescript(x) + ", pushing you ever closer to the climax.");
	...
	//Very Loose, sensitive enough to make Arian cum just from insertion. (Feels better than anything else. Yep, Arian really becomes a buttslut at this point)
		...
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("wet gash is positively leaking lizan juices");
		if(flags[FlagEnum.ARIAN_VAGINA] > 0 && flags[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text(" and [Arian eir] ");
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text("erect cock is already leaking pre like a sieve");
		...
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("her pussy clenches, spilling a flood of juices; ");
		if(flags[FlagEnum.ARIAN_VAGINA] > 0 && flags[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text(" and ");
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text("his cock throbs and shoots rope after rope of cum onto the bedsheets");
		...
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("cocks are still rock hard, and surprisingly, still leaking ropes of cum");
			else MainScreen.text("cock is still rock hard, and surprisingly, still leaking ropes of cum");
			if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(", and her");
		}
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" pussy squirting juices against your [legs] with each shuddering impact of your [hips]");
		...
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text("  [Arian Eir] throbbing cock flexes and shoots one last rope of lizan-cum against your belly.");
...
private getBlownByArian():void {
	...
	if(flags[FlagEnum.ARIAN_HAS_BLOWN] == 0) {
		...
		if(flags[FlagEnum.ARIAN_COCK_SIZE] == 3) MainScreen.text("  You notice Arian's exposed shaft slowly hardening at your invitation.");
...
private penetrateArian():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text("dick");
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] == 3) {
		...
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s peeking out of their");
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] == 0) MainScreen.text("female");
	else MainScreen.text("herm");
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
		MainScreen.text("  Her ");
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("twin shafts");
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
		...
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s twitch as they spurt ");
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
		...
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("twin cocks throb");
...
private getButtWreckedByArian():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] >= 3) {
		...
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s harden");
		else MainScreen.text(" hardens");
		MainScreen.text(" to full mast.");
	}
	else {
		MainScreen.text(arianMF("his","her") + " cock");
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s peek out of their");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("primary ");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) {
		...
		if(player.tailType == TAIL.NONE) MainScreen.text("back");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("stroking [Arian eir] other cock and sliding it between your ass cheeks, ");
	...
	if(flags[FlagEnum.ARIAN_ANAL_XP] > 66) MainScreen.text("eager ");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("pair of lizan-cocks practically jump");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("sliding [Arian eir] neglected prick through your ass-cheeks, ");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s twitch");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text(", as well as your butt");
	MainScreen.text(", as Arian climaxes");
	if (flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(", her own pussy clenching and drizzling juices on the matted sheets beneath the two of you.");
	else MainScreen.text(".");
...
private suckAriansDick():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 3) {
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("exposed cocks hardening to full mast");
		else MainScreen.text("exposed cock hardening to full mast");
	}
	else {
		MainScreen.text("cock");
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s emerging from their");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("uppermost ");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("twin ");
	MainScreen.text("shaft");
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s tremble");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text(" and chin");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text(", oblivious to the second cock spewing cum all over your [chest] in your eagerness");
	...
	if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" and tickle her cunt with a finger");
...
private getPenetratedByArianAndHisHitlerMustache():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] < 3) MainScreen.text("slit, and think of what it's hiding in there");
	else {
		MainScreen.text("exposed cock");
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
	}
	MainScreen.text("; then you turn to look at Arian and ask [Arian em] how would [Arian ey] feel about giving [Arian eir] ");
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("twin cocks");
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] >= 3) MainScreen.text("exposed");
	else MainScreen.text("hidden");
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text(" twin shafts immediately fill");
	else MainScreen.text(" shaft immediately fills");
	MainScreen.text(" with blood, growing fully erect in ");
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("their");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("each of ");
	MainScreen.text(arianMF("his","her") + " ");
	MainScreen.text("shaft");
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s");
	MainScreen.text(" with your [vagina]");
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text(" and [asshole]");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("twin bulbous shafts feel");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("and [ass] ");
	...
	if(flags[FlagEnum.ARIAN_VAGINA] == 0) MainScreen.text("[Arian ey] himself ");
	else MainScreen.text("she ");
	MainScreen.text("is being overwhelmed by, spraying your ");
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] == 0) MainScreen.text("cavity");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] == 0) MainScreen.text("its");
...
private doublePenetrateArian():void {
	...
	MainScreen.text("You look over your feminine lizard lover, and feel your " + multiCockDescriptLight() + " starting to stir in your [armor].  Since you have enough tools for the job, and Arian has enough holes, you ask if Arian would be willing to let you fuck her in both her ass and pussy at the same time?");
	...
	if(flags[FlagEnum.ARIAN_ANAL_XP] <= 33) MainScreen.text("tight");
	else if(flags[FlagEnum.ARIAN_ANAL_XP] <= 66) MainScreen.text("loose");
...
private arianDocking():void {
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("pair of ");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] == 0) MainScreen.text(" brushing up against [Arian eir] own lizan pecker");
	...
	if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("pair of cummy masts emerge");
...
private giveArianAnItem():void {
	...
	if(flags[FlagEnum.ARIAN_S_DIALOGUE] == 0 && arianHealth() >= 10) arianStoryDialogue1();
		...
		if(player.hasItem(consumables.VITAL_T)) MainScreen.addButton(0,"Vital Tinct",arianVitalityTincture);
		if(flags[FlagEnum.ARIAN_HEALTH] >= 20) {
			if(player.hasItem(consumables.P_DRAFT)) MainScreen.addButton(1,"P. Incubi D",giveIncubusDraftToArian);
			if(player.hasItem(consumables.P_S_MLK)) MainScreen.addButton(2,"P. Suc.Milk",succubiMilkForArian);
			if(player.hasItem(consumables.LACTAID)) MainScreen.addButton(3,"Lactaid",giveArianLactaid);
			if(player.hasItem(consumables.REDUCTO)) MainScreen.addButton(4,"Reducto",giveArianReducto);
			if(player.hasItem(consumables.REPTLUM)) MainScreen.addButton(5,"Reptilum",giveArianReptilum);
		}
		MainScreen.addButton(9,"Back",arianHomeMenu);
	}
}
...
private giveIncubusDraftToArian():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text("if [Arian ey] would like to make [Arian eir] cock bigger");
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] == 0) {
		...
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] == 0) MainScreen.text("shaft emerges from its hiding place.  It throbs and grows, settling in its");
		...
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s back into their");
		...
		if(flags[FlagEnum.ARIAN_COCK_SIZE] == 2) {
			...
			if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("them");
...
private succubiMilkForArian():void {
	...
	if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("further ");
	...
	if(flags[FlagEnum.ARIAN_VAGINA] == 0) {
		...
		//if ArianCockSize >= 3:
		if(flags[FlagEnum.ARIAN_COCK_SIZE] == 3) MainScreen.text(" as far as it'll go");
		...
		if(flags[FlagEnum.ARIAN_BREASTS] == 2) {
			...
			if(flags[FlagEnum.ARIAN_COCK_SIZE] >= 3) MainScreen.text(" and her exposed shaft begins to grow hard");
			MainScreen.text(".");
		}
		else {
			...
			if(flags[FlagEnum.ARIAN_COCK_SIZE] == 3) MainScreen.text(" and you see her exposed shaft slowly rising to point at you");
			MainScreen.text(".");
...
private giveArianLactaid():void {
	...
	if(flags[FlagEnum.ARIAN_BREASTS] == 0) {
		...
		if(flags[FlagEnum.ARIAN_COCK_SIZE] == 3) MainScreen.text(", her exposed cock growing hard as she flushes with arousal");
		MainScreen.text(".  Her hands grip the sheets and you watch her ");
		if(flags[FlagEnum.ARIAN_BREASTS] == 1) MainScreen.text("perky");
		else if(flags[FlagEnum.ARIAN_BREASTS] == 2) MainScreen.text("luscious");
		else MainScreen.text("pillowy");
...
private giveArianReducto():void {
	...
	//(Display Options)
	menu();
	if(flags[FlagEnum.ARIAN_BREASTS] > 0) MainScreen.addButton(0,"Breasts",useReductoOnAriansBreasts);
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.addButton(1,"Cock",useReductoOnArianCocks);
...
private useReductoOnAriansBreasts():void {
	...
	if(flags[FlagEnum.ARIAN_BREASTS] == 1) MainScreen.text("small");
	else if(flags[FlagEnum.ARIAN_BREASTS] == 2) MainScreen.text("generous");
	else MainScreen.text("pillowy");
...
private useReductoOnArianCocks():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] == 3) {
		...
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("pair of exposed lizard cocks");
		...
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s from their tight hiding place");
		...
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s return to their");
...
private useReductoOnAriansAsshole():void {
	...
	//(else if AnalXP < 33)
		...
		if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			...
			if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s suddenly jolting from their sheathe");
			else MainScreen.text(" suddenly jolting from its sheathe");
			if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" and ");
		}
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) {
			if(flags[FlagEnum.ARIAN_COCK_SIZE] == 0) MainScreen.text(", ");
...
private giveArianReptilum():void {
	...
	if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("slavering vagina");
	MainScreen.text(".");
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
		...
		if(flags[FlagEnum.ARIAN_BREASTS] == 0) MainScreen.text("chest");
		else if(flags[FlagEnum.ARIAN_BREASTS] == 1) MainScreen.text("perky little breasts");
		else if(flags[FlagEnum.ARIAN_BREASTS] == 2) MainScreen.text("soft breasts");
		else MainScreen.text("pillowy breasts");
		if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text(" while ");
	}
	if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("her vagina doing its best to soak her bed with lizan femcum");
	MainScreen.text(".");
...
private imbueTalisman():void {
	...
	*/
	...
	menu();
	if(player.hasItem(useables.B_CHITN,2) && player.hasItem(useables.T_SSILK)) MainScreen.addButton(0,"Shielding",arianSpellPlace,"Shielding Spell");
	if(player.hasItem(consumables.GOB_ALE,2) && player.hasItem(consumables.S_GOSSR)) MainScreen.addButton(1,"Immolation",arianSpellPlace,"Immolation Spell");
	MainScreen.addButton(9,"Back",arianHomeMenu);
}
...
public sleepWithArian(newl:boolean = false):void {
	...
	if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("You can tell that [Arian ey] is aroused by the little show, despite [Arian eir] body doing a good job of hiding it.  ");
	...
	if (player.lowerBody.cockSpot.hasCock()) {
		...
		if(flags[FlagEnum.ARIAN_ANAL_XP] < 33) {
			...
			if(flags[FlagEnum.ARIAN_VAGINA] > 0) {
				...
				MainScreen.text(arianMF("his","her") + " ");
				if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("twin shafts are");
		...
		if(flags[FlagEnum.ARIAN_ANAL_XP] < 33) {
			...
			if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
				...
				if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s begin");
				else MainScreen.text(" begins");
				MainScreen.text(" sporting an erection");
				if(flags[FlagEnum.ARIAN_COCK_SIZE] < 3) {
					MainScreen.text(", poking ");
					if(flags[FlagEnum.ARIAN_DOUBLE_COCK] == 1) MainScreen.text("its tip");
					else MainScreen.text("their tips");
					MainScreen.text(" out of Arian's genital slit");
				}
				MainScreen.text(".");
			}
			else if(flags[FlagEnum.ARIAN_VAGINA] > 0) MainScreen.text("and her moist pussy, begins forming a stream of juices that gently run its course through the inside of Arian's thighs, evaporating before hitting the floor due to the lizan's currently elevated body heat.");
...
public arianLaysEggs():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) MainScreen.text("herm");
...
private arianDildoFun():void {
	...
	if(flags[FlagEnum.ARIAN_COCK_SIZE] > 0) {
		MainScreen.text("Her cock");
		if(flags[FlagEnum.ARIAN_DOUBLE_COCK] > 0) MainScreen.text("s throb");
