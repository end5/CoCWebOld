...
public function meetIzmaAtLake():void {
	...
	//(Izmacounter= 1)
		...
		if(player.statusAffects.get("TelAdre").value1 >= 1) outputText("outside the safety of Tel'Adre", false);
...
private function readSharkCuntManual2():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00238] != 1) outputText("head back to your camp.", false);
...
private function readSharkgirlPornzYouFuckingPervertAsshole():void {
	...
	if(player.stats.cor < 33) {
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00238] != 1) outputText(" back to your camp", false);
		outputText(".", false);		
	}
	//(31-69 Corruption)
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00238] != 1) outputText("you head back to camp.", false);
...
private function fightSharkCunt():void {
	...
	//(If Izmafight = 0) 
	if(flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] == 0) outputText("\"<i>All right, show me just what a Champion can do!</i>\" she says, entering a fighting stance.", false);
	//(If Izmafight = 1-2) 
	else if(flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] > 0 && flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] <= 2) outputText("Izma's eyes narrow at you, and she assumes a fighting stance.  \"<i>You won't get so lucky this time.</i>\"", false);
	//(If Izmafight = 3+)
	else if(flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] > 2) outputText("Izma seems uncertain with herself as she prepares for battle. \"<i>Go a little easier on me this time... please?</i>\"", false);
	//(If Izmafight = -1 or -2 )
	else if(flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] < 0 && flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= -2) outputText("\"<i>Hm, really?  Well, maybe you'll get lucky this time,</i>\" she mocks, gesturing at you to strike first.", false);
	//(If Izmafight = -3 to -4)
...
internal function IzmaWins():void {
	...
	//(without worms)
		...
		//(Izmafight =0)
		if(flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= 0) outputText("Izma chuckles slightly as she prowls around your defeated form.  \"<i>Well, as far as things around here stand, you made for a decent fight.  Still no match for me, though.</i>\"", false);
		//(Izmafight = -1 or -2) 
		else if(flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= -2) outputText("\"<i>Ya know, just because we're friends doesn't mean you need to hold back... you were holding back, right?</i>\" Izma asks, placing her hands on her hips.", false);
		//(Izmafight = -3 or -4) 
...
private function loseToIzma():void {
	...
	if(player.gender <= 2) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("cunt", false);
		else outputText("pucker", false);
		outputText(" of yours!</i>\"  ", false);
		//[(If Izmafight is -1 or -2)
		if(flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= -2) outputText("With some reluctance, but driven by your promise, you remove pieces of your " + player.armorName + " until you stand naked before the hungrily ogling tigershark.", false);
	...
	if(player.gender <= 1) {
		...
		if(player.analCapacity() < 26) {
			...
			//(If Izmafight <= -4)
			if(flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] <= -4) outputText("  You find yourself pushing back to speed up the process, desperate for Izma to fill you again.", false);
		...
		//(If player has middling anus:) 
			...
			if(flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] <= -4) outputText("  You find yourself wondering that as well...", false);
			player.buttChange(monster.cockArea(0),true,true,false);
		}
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText(", and your own male organ is hard and throbbing from the stimulation", false);
		outputText(".\n\n", false);
		...
		//(If Izmafight = -4)  
		if(flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] <= -4) outputText("  If this is how the sharks do it, you could really get used to it...", false);
		outputText("\n\n", false);
	...
	//Loss Scene- Herm 
		...
		//(1-10 inch penis)
		if(player.lowerBody.cockSpot.list[x].cockLength <= 10) outputText("Izma manages to supress a snort of laughter at the sight of your cock.  \"<i>Um... wow?  It's simply... heh, huge...</i>\"", false);
		//(10-19 inches)
		else if(player.lowerBody.cockSpot.list[x].cockLength <= 19) outputText("\"<i>Not bad, I'm actually impressed,</i>\" Izma says, nodding slightly in approval.", false);
		//(20+ inches)
		...
		//(If Izmafight <= -4) 
		if(flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] <= -4) outputText("  You're not even really ashamed to admit that such is the case anymore.", false);
		outputText("\n\n", false);
		...
		if(player.lowerBody.cockSpot.count() == 1) outputText("it", false);
	...
	//zmafight -1: 
	if(flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= -1) outputText("  You realize what you're thinking and shudder, forcing the submission-tinged desires down. Where did they come from, anyway?", false);
	//Izmafight -2: 
	else if(flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= -2) outputText("  Though you manage to force them away, the dreams of submitting to Izma are starting to haunt you, their power and seductive allure growing. But, still, you can control them.", false);
	//Izmafight -3: 
	else if(flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= -3) outputText("  You hum idly to yourself and enjoy the mental visions for a while, then, with some reluctance, you push them aside. Still, you're sure you can bring them out again when you want to enjoy them.", false);
	//Izmafight -4: 
	else if(flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= -4) outputText("  You have only the vaguest thought that maybe you shouldn't be thinking about Izma in that way, but it's so tempting to just immerse yourself in the sexiness of having a hot herm shark-girl dominate you so thoroughly. After all, it's not as if there's any harm in doing so, is there?", false);
	//Izmafight -5: 
...
private function finalIzmaSubmission():void {
	...
	if(player.lowerBody.cockSpot.hasCock() || player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your " + vaginaDescript(0) + " moisten considerably", false);
	}
	outputText(".\n\n", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina() || player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your " + vaginaDescript(0) + " starts squirting fluids liberally", false);
	}
	outputText(".\n\n", false);
...
private function victoryPenisIzma():void {
	...
	//[(If Izmafight = 3+)
	if(flags[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= 3) outputText("  Hell, maybe it was made for you, given just how eager Izma seems to be whenever she sees you.  It's like she loses to you on purpose.", false);
	outputText("\n\n", false);
	...
	if(player.gender == 1) {
		...
		//[(multi) 
		if(player.lowerBody.cockSpot.count() > 1) outputText(" and onto her groin", false);
...
private function nonFightIzmaSmexPAINUS():void {
	...
	if(player.lowerBody.cockSpot.count() == 1 && player.lowerBody.cockSpot.list[0].cockType.Index < 9) {
		...
		if(player.lowerBody.cockSpot.list[0].cockType == CockType.HUMAN) outputText("Izma seems surprised to see your genitalia is similar to hers.  \"<i>Huh. Thought that every land-dweller usually bumbled into a dick transformation around here.</i>\"", false);
		//[Horse cock in slot 0] 
		else if(player.lowerBody.cockSpot.list[0].cockType == CockType.HORSE) outputText("Seeing your " + cockDescript(0) + " causes Izma to lick her lips slowly. \"<i>Well... Equinum is pretty popular, isn't it?</i>\"", false);
		//[Dog cock]
		...
		//[Tentacle] 
		else if(player.lowerBody.cockSpot.list[0].cockType == CockType.TENTACLE) outputText("A puzzled look plays across Izma's face as she stares at your " + cockDescript(0) + ".  \"<i>Wow... I've read a lot about the corrupt plants.  Didn't think people could wind up like that.</i>\"", false);
		//[Demon Dick] 
		else if(player.lowerBody.cockSpot.list[0].cockType == CockType.DEMON) outputText("Izma looks shocked as she lays eyes on your perverted pecker, having never seen anything quite like it before. \"<i>Um... that looks... awkward. Like having a truncheon between your legs.</i>\"", false);
		//[cat dork] 
		else if(player.lowerBody.cockSpot.list[0].cockType == CockType.CAT) outputText("She glances at your barbed pecker and winces.  \"<i>You ate the fruit around here, didn't you?  One of my friends did that, too.</i>\"", false);
		//[lizardick]
		else if(player.lowerBody.cockSpot.list[0].cockType == CockType.LIZARD) outputText("Her eyes goggle as you reveal a bulbous purple pecker.  \"<i>Oh wow.  Do you have a green one too?</i>\"", false);
		//[Captain Nemo] 
		else if(player.lowerBody.cockSpot.list[0].cockType == CockType.ANEMONE) outputText("She grimaces as you unveil a " + cockDescript(0) + ".  \"<i>God, not one of those.  You wind up hornier after cumming than you were before you started touching it.</i>\"", false);
		//[Kangaroo Jack] 
		else if(player.lowerBody.cockSpot.list[0].cockType == CockType.KANGAROO) outputText("She looks on blankly as you reveal your sheath, then gives a start as your " + cockDescript(0) + " slides out of it after a few strokes.  \"<i>Woah... it's so... thin and pointy.</i>\"", false);
	}
	...
	if(player.lowerBody.cockSpot.list[0].cockType == CockType.TENTACLE) outputText("\"<i>Well, actually...</i>\" you begin, flexing your cock-tentacle.  She cuts you off with a glare.  ", false);
	//(other slot 0)
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText(" and hair", false);
	outputText(".  ", false);
	//[(big/mega skeet)
...
private function nonFightIzmaSmexCUNTPUSSYSNATCHQUIM():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" and " + multiCockDescriptLight(), false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" and " + sMultiCockDesc() + " shooting a weak load across her chest", false);
...
private function acceptIzmaAsYourBitch():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00236] == 0 && flags[FlagEnum.AMILY_FOLLOWER] == 1) {
		...
		if(player.perks.has("MarblesMilk")) outputText("You couldn't break your addiction, and now you'll die without a steady supply of her milk.", false);
		...
		//(Fatally addicted)
		if(player.perks.has("MarblesMilk")) outputText("turned you into some weak drug-addict", false);
		//(Broke addiction)
	...
	//Izma pops 'em out!
		...
		switch (pregnancy.event) {
			...
			case 6: outputText("Izma's certain the baby will come soon.  You're inclined to believe her; she's huge now.  She still wears no clothing, but there's definitely a practical reason for it");
					if (flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("- you don't think her skirt would be able to cope with how often she gets erect now, her huge cock rubbing along");
					...
					if (flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("the friction of her cock against her bulge, you think it has more to do with avoiding being splattered in sexual juices all day long.", false);
...
private function izmaPearance():void {
	...
	if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00235] == 0) outputText(" and wielding iron gauntlets as a weapon.");
	else outputText(".");
...
private function followerIzmaMountsPC(lastHalf:boolean = false):void {
	...
	if(!lastHalf) {
		...
		if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText(" instead of her cock");
		...
		if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("  Your stomach starts to feel wet, and as you look down you realize her pre is starting to soak into your " + player.skinFurScales() + ".\n\n", false);
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("Izma bites her lip in response, clearly wanting to enjoy her time on top just a little bit longer.  But the grunts and moans of ecstasy make it clear that the tigershark can't hold back much longer.  Her vaginal walls clamp down hard against your cock.  She squeals loudly and thick ropes of her jizz fly through the air, going to the point where a few even arc over your head. Most of it, though, hits your face and " + allBreastsDescript() + ", plastering the top half of your body in cum as she empties her quads.\n\n", false);
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("own ");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.SLICK) outputText("dripping", false);
		else if(player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.DROOLING) outputText("gushing", false);
...
private function followerIzmaTakesItInPooper():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		//[(if player has balls)
		if(player.lowerBody.balls > 0) outputText(" and " + sackDescript(), false);
		...
		//[(if player has balls)
		if(player.lowerBody.balls > 0) outputText(" against your own " + ballsDescriptLight(), false);
		//(if no balls and vag)
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" against your already-dripping " + vaginaDescript(0), false);
		//(else)
		else outputText(" against your perineum", false);
		outputText(", humiliating you more than she stimulates you.\n\n", false);

		//[if dick0 is <14\"] 
		if(player.lowerBody.cockSpot.list[0].cockLength < 14) outputText("\"<i>Hmm...</i>\" she sighs, as she looks at her glans sticking out past the tip of your dick.  \"<i>It's nice to know who's in charge.</i>\"", false);
		//[dick0 >14 but <16] 
		else if(player.lowerBody.cockSpot.list[0].cockLength < 16) outputText("\"<i>It's nice that we keep things fair around here,</i>\" murmurs Izma, smiling at your similarly-sized cocks.", false);
		//[If dick0 >16]
		else outputText("\"<i>Oh fuck,</i>\" she says, her eyes filling with lust as she sees your cock extending well past her own.  She tilts forward and rubs the base of her cock hard against the base of your " + Appearance.cockNoun(player.lowerBody.cockSpot.list[0].cockType) + ", jerking the both of you off together.  You moan helplessly when she stops, and she sighs, \"<i>It feels so good to be alpha over a powerful cock like yours.</i>\"", false);
		outputText("\n\n", false);
	}
	...
	//[if player has penis: 
	if(player.lowerBody.cockSpot.hasCock()) outputText("  You try to reach for your cock and get off, but her hands keep your wrists securely pinned to the ground.", false);
	outputText("\n\n", false);
	...
	if(!player.lowerBody.cockSpot.hasCock()) outputText("The feeling of her cock throbbing finally puts you over the edge and you orgasm, your tight pucker helping milk the last of her cum into you.\n\n", false);
	//(if player has penis)
	else {
		...
		if(player.lowerBody.cockSpot.list[0].cockLength < 14) {
			...
			if(player.lowerBody.cockSpot.count() > 1) outputText(" first, then your other " + cockDescript(1) + ", synchronizing her movements", false);
		...
		//[(else if penis 0 length >15 inches)
			...
			if(player.lowerBody.cockSpot.count() > 1) outputText(" and onto your combined bodies", false);
			outputText(".\n\n", false);
	...
	if(!player.lowerBody.cockSpot.hasCock()) outputText("After a few minutes, she pulls her shrinking cock out of you and gives you a deep kiss. \"<i>Thanks for that,</i>\" she says, \"<i>...my Alpha.</i>\"  She jogs off to the river to clean off.  After a minute you follow her, with her cum dripping out of your well-used anus.", false);
	player.orgasm();
	dynStats("sen", -1);
	doNext(camp.returnToCampUseOneHour);
}
...
private function followerIzmaTakesItInVagoo():void {
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText(", gently lifting her meaty quads up so");
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("the other holding her maleness out of the way, teasing her cock with her thumb and flexing her fingers to roll her four balls around in the palm of her hand.");
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText(", while Izma herself reaches around to jerk herself off.\n\n", false);
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText(", and gains speed as she jerks her meat pole");
	outputText(".\n\n", false);
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText(", a hot jet of spunk going straight into your mouth, and several subsequent shots hitting you in the lips and face.  ");
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 1) outputText("girl-");
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("licking some of her seed off of your face.  \"<i>Come fill me whenever you need to.</i>\"  You grin at Izma and order her to eat up every drop.\n\n", false);
	else outputText("licking you face affectionately.  \"<i>Come fill me whenever you need to.</i>\"  You grin back at Izma knowingly.\n\n", false);

	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("She giggles and runs her tongue over the mess she's made; her tongue lingers on your lips for a few moments before she gives you another long kiss and lets you go.", false);
...
private function RadarIzmaCumInYourChest():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("in between your " +  chestDesc() + " and coats ");
	outputText("against your chest");
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText(", coating it");
...
private function followerIzmaTakesIt69():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		//[(if herm)
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" while pushing two fingers into your " + vaginaDescript(0), false);
	}
	//(Female)
	else if(player.lowerBody.vaginaSpot.hasVagina()) {
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("You wrap your lips around Izma's own massive erection, sucking and gagging on her almost-two-foot cock as she jerks and twists her hips around.  Her muffled moans seem to assure you that you're doing a great job, and Izma responds by increasing the speed of her head-bobbing.  Your vision is a little obscured by Izma's quartet of balls repeatedly bumping against your forehead and the bridge of your nose, but if anything this serves to arouse you further.\n\n", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("to reach ", false);
	else outputText("to push your " + clitDescript(), false);
	outputText(" as far into her mouth as you can.\n\n", false);
 
	if(player.lowerBody.cockSpot.hasCock()) outputText("Eventually, the two of you manage to bring each other to a powerful simultaneous orgasm, Izma's hot cum shoots down your eager throat, and you ", false);
	...
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("drool a sticky mess over Izma's cheeks.", false);
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 1) outputText("lady-");
...
private function talkWivIzma():void {
	...
	//[Talk topic 4.]
		...
		if(flags[FlagEnum.IZMA_NO_COCK] == 1) outputText("she used to have a ");
		else outputText("she's got a ");
		outputText("perfectly functional set of male genitalia.\n\n", false);
		
		if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("\"<i>Oh, nah.  Pfft, as if that's a challenge,</i>\" she says.  It's interesting to see how Izma, as a hermaphrodite, thinks of herself when it comes to the most basic of gender roles.  She scratches her chin in thought.  \"<i>Well, sexually, I'm more accustomed to my penis, since it's a bit easier to dominate with and the lake life seems to fixate on it.  But for you, I'll use whichever you want.</i>\"\n\n", false);
	...
	//[Talk option 5]
		...
		if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("..  and patiently ignoring the fact you can feel her cock starting to stiffen against your thigh.");
...
private function chooseToFlirtWithIzma():void {
	...
	if (flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("her huge cock barely able to poke past her baby-filled belly, ");
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("  But she glances between her legs and sees her massive cock poking out of her skirt.");
...
private function IzmaPoopsBabies():void {
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("jerking me off or ");
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("Divvying your time between her raging erection, quartet of sizable balls and dilating cunt while she strains under you is an awkward experience, but you persevere; when Izma stops hissing and gasping in pain and starts instead moaning lightly, it seems more worthwhile.\n\n", false);
...
public function pcPopsOutASharkTot():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00238] != 1) {
		...
		if(player.upperBody.chest.count() > 1) outputText("  She then starts on your third breast, and while she slows down at the fourth breast, she still finishes it off.", false);
		//(If player has six breasts: 
		if(player.upperBody.chest.count() > 2) outputText("  She seems to almost be struggling to finish off your third pair, but she manages to suck all six of your breasts dry.", false);
...
private function izmaKidsPlaytime():void {
	...
	if (flags[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] + flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] == 1) outputText("There is ", false);
	else outputText("There are ", false);
	if (flags[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] > 0) {
		if (flags[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] == 1) outputText("one shark-girl", false);
		else outputText(num2Text(flags[FlagEnum.IZMA_CHILDREN_SHARKGIRLS]) + " shark-girls", false);
		if (flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] > 0) outputText(" and ", false);
	}
	if (flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] > 0) {
		if (flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] == 1) outputText("one tigershark", false);
		else outputText(num2Text(flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS]) + " tigersharks", false);
	}
	...
	//NOPE
	if(choice == 1) {
		if (flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] == 1) outputText("Your lone tigershark daughter is off by herself, experimentally stroking and fondling her already-impressive cock.  It doesn't look like it'll be long before she cums; her face is already screwed up in an expression of curious pleasure, gasping softly at the sensations.", false);
		//(Requires 2-3 Tigersharks: 
		else if (flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] < 4 && flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] > 0) outputText("Your tigershark daughters are seated together, each experimentally stroking and fondling her sizable erection.  It looks like they're competing to see who will cum first.", false);
		//(Requires 4+ Tigersharks:
		else if (flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] > 0) outputText("Your tigershark daughters are seated together, each experimentally jerking off either her own cock or one of her sisters.  Some look to be competing to see who can jerk themselves off first, others are evidently seeing who's better at handjobs.", false);
		if (flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] > 0) outputText("  Izma blushes and explains that your children have strong libidos, so it's natural for them to experiment like this.  She assures you that they won't do any real sexual penetration with a family member.\n\n", false);
	}
	...
	//NOPE
		...
		//Other children:
		if (flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] + flags[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] > 2) outputText("  Your other kids are gathered around, watching with amusement and a little envy as one of your tigershark offspring plows an excited goblin.  \"<i>Oh, yeah, baby, now you folks know how to live!</i>\" she cheers.", false);
		if (flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] > 2) outputText("  \"<i>Don't worry; you'll get a chance to use a beautiful big cock on me, too - just wait your turn!</i>\" she laughs looking around the circle.  Your other daughters look like they can't wait.", false);
	...
	if(choice == 4) {
		...
		if (flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] + flags[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] == 1) outputText("your daughter", false);
	...
	if(choice == 5) {
		...
		if (flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] + flags[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] == 1) outputText("child perks up to see Izma and comes running over to her, throwing herself into her arms", false);
		...
		if (flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] + flags[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] == 1) outputText("cries out in joy at the prospect of storytime and eagerly seats herself in front of her, listening as she starts to read.", false);
...
private function inCampRideIzmasDickDongTheWitchIsDead():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.balls > 0) outputText(" and " + ballsDescript(), false);
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		outputText("  Your " + Appearance.cockNoun(player.lowerBody.cockSpot.list[0].cockType) + " rubs against her shaft, sending delicious sparks surging through you.", false);
	}
	...
	if(player.stats.cor < 66) outputText("you've teased her enough", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("s", false);
	...
	if(player.stats.cor < 33) outputText("Feeling merciful, you tell her that you're almost there... just a little more.  ", false);
...
private function stephenHawkingPorn():void {
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("continuously to keep the beast in her skirt pinned down.\n\n", false);
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("but that's not exactly possible for someone who just had over a foot of dick slip from between her thighs and stick into the air.  You laugh openly and give Izma's silver hair a soft tug, before getting up and telling her you have business elsewhere.  Izma simply nods in taciturn response but keeps her gaze fixed on the lewd images before her.  Another laugh escapes your lips as soon as you think you're out of earshot.", false);
...
private function fuckIzmasPussyDominate():void {
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("cock ");
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("es");
	outputText(" now stand");
	if(flags[FlagEnum.IZMA_NO_COCK] == 1) outputText("s");
	outputText(" exposed to the cool air, drooling ");
	if(flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("their sexual lubricants");
	...
	if(flags[FlagEnum.IZMA_NO_COCK] == 1) outputText("shark-girl");
...
private function radarIzmaAnalDominant():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Your [clit] flushes with");
	else if(player.lowerBody.cockSpot.hasCock()) outputText("[EachCock] bobs in");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(clitDescript());
	else if(player.lowerBody.cockSpot.hasCock()) outputText(cockDescript(0));
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("feverish hands as you fingerfuck your " + vaginaDescript() + ".");
	//(If PC has balls: 
	else if(player.lowerBody.balls > 0) outputText("swinging [balls] with each gyration of your hips.");
	//(If PC is genderless: 
...
private function backJizzShot():void {
	...
	if(player.lowerBody.cockSpot.count() == 1) outputText("your shaft ");
	else if(player.lowerBody.cockSpot.count() == 2) outputText("two shafts ");
...
private function izmaDomsLatexyPartI():void {
	...
	if(flags[FlagEnum.GOO_DICK_LENGTH] > 0) outputText("cock");
	...
	outputText("  A particularly powerful thrust inside herself has " + flags[FlagEnum.GOO_NAME] + " whining, her eyelids fluttering closed as she cries, \"<i>Oh, [name]!</i>\"");
	...
	if(latexGirl.gooObedience()/5 + latexGirl.gooFluid()/5 + rand(20) < 10) {
		outputText("" + flags[FlagEnum.GOO_NAME] + " grows and sucks harder, cheeks hollowed with suckling hunger.  You can see them stretch and go even more concave with rhythmic pulsations, vacuum-tight around the orange shaft.  Izma moans and tries to pull out, but she barely manages to extract an inch before her whole length is noisily pulled back in.  The shark-girl tries to pry the slutty, hungry little goo off of herself again and again, but each time, all she winds up doing is getting sensuously devoured again and again.  " + flags[FlagEnum.GOO_NAME] + " gives her four balls another appreciative squeeze, and Izma's eyes suddenly roll back, helpless again the hungry sluts impressive oral talents.");
	...
	//OBEDIENCE SUCCESS
		...
		outputText("" + flags[FlagEnum.GOO_NAME] + " obediently relaxes, allowing the herm to take more effective control of the situation.  \"<i>Atta girl,</i>\" Izma comments as she extracts herself from the hungry, sucking mouth-hole, her dick shining with a fresh, wet, latex sheath.  \"<i>Ass up, slut.  You're getting fed via suppository today!</i>\"");
...
private function stopIzmaLatexy():void {
	clearOutput();
	outputText("You step into the middle of it before it can progress any farther.  " + flags[FlagEnum.GOO_NAME] + " is yours, and Izma is not to play at being a mistress to YOUR pet.");
...
private function izmaLatexyToggle():void {
	...
	if(flags[FlagEnum.IZMA_X_LATEXY_DISABLED] == 0) {
		outputText("You tell Izma you don't want her feeding " + flags[FlagEnum.GOO_NAME] + " any more.");
		...
		outputText("You tell Izma that you've changed your mind, and she can keep " + flags[FlagEnum.GOO_NAME] + " fluid topped off.  The shark-girl hugs you with a predatory grin splitting her face.  \"<i>I can't wait.</i>\"");
