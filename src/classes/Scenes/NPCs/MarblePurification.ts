...
/*
flags[FlagEnum.MARBLE_PURIFICATION_STAGE] : 0 = unstarted
flags[FlagEnum.MARBLE_PURIFICATION_STAGE] : 1 = Started, gathering labova
flags[FlagEnum.MARBLE_PURIFICATION_STAGE] : 2 = Potions gathered, need to talk to Rathazul
flags[FlagEnum.MARBLE_PURIFICATION_STAGE] : 3 = Started wiv Ratazul
flags[FlagEnum.MARBLE_PURIFICATION_STAGE] : 4 = Marble is out getting her sister
flags[FlagEnum.MARBLE_PURIFICATION_STAGE] : 5 = QUEST COMPLETE
*/
...
package classes.Scenes.NPCs {
	...
	public BLUHBLUH():void {
		...
		if(flags[FlagEnum.MARBLE_BOVA_LEVEL] <= 1) MainScreen.text("human");
		...
		if(amilyScene.amilyFollower() && !amilyScene.amilyCorrupt() || followerKiha() || flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00238] == 1)
		{
		...
		if(player.hasItem(consumables.PURHONY)) MainScreen.text("  You then recall that bottle of honey the bee-girl gave you, and ask if maybe it might work?");
		...
		flags[FlagEnum.MARBLE_PURIFICATION_STAGE] = 1;
	...
	public giveMarblePureQuestLabova():void
	{
		...
		//First time
		if(flags[FlagEnum.TIMES_GIVEN_MARBLE_PURE_LABOVA] == 0)
		{
		...
		else if(player.statusAffects.get("MarbleSpecials").value4 <= 30)
		{
		 	...
		 	flags[FlagEnum.MARBLE_PURIFICATION_STAGE] = 2;
		}
		//Every other time
		else
		{
			...
			if(flags[FlagEnum.MARBLE_BOVA_LEVEL] > 1) MainScreen.text(", fur covered");
		...
		flags[FlagEnum.TIMES_GIVEN_MARBLE_PURE_LABOVA]++;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	public pageTwoOfVisitingTheRatForMurblesPurification():void
	{
		MainScreen.clearText();
	 	...
	 	flags[FlagEnum.MARBLE_PURIFICATION_STAGE] = 3;
	 	flags[FlagEnum.MARBLE_RATHAZUL_COUNTER_1] = 24;
	...
	{
		MainScreen.clearText();
		flags[FlagEnum.MARBLE_RATHAZUL_COUNTER_1] = 0;
	...
	public murbleShouldGoRunAlongAndGetHerCuntySisterSoTheyCanBeCuntsTogether():void
	{
		...
		flags[FlagEnum.MARBLE_PURIFICATION_STAGE] = 4;
		flags[FlagEnum.MARBLE_RATHAZUL_COUNTER_2] = 240;
	...
	{
		MainScreen.clearText();
		flags[FlagEnum.MARBLE_RATHAZUL_COUNTER_2] = 0;
		...
		if(flags[FlagEnum.MARBLE_BOVA_LEVEL] <= 1) MainScreen.text("your mate has");
	...
	public partTwoOfClaraShowingUpAndBeingACunt():void 
	{
		...
		flags[FlagEnum.MURBLE_TEA_DRINKER_COUNT] = teaDrinkers;
	...
	public partTwoOfLowIntelligencePCsFallingForTheIncrediblyObviousCowCuntsTrap():void
	{
		...
		if(flags[FlagEnum.MARBLE_BOVA_LEVEL] <= 1) MainScreen.text("face");
	...
	public startCombatWithMarbleSister(inCamp:boolean = true):void
	{
		...
		if(inCamp && player.findStatusAffect(StatusAffects.ClaraFoughtInCamp) < 0) player.statusAffects.add(new StatusAffect("ClaraFoughtInCamp",0,0,0,0)));
	}
	//Chose submit
	...
	public loseToClara(combat:boolean = true):void
	{
		...
		if(player.tallness < 70) MainScreen.text("much too big for you.");
	 	...
	 	MainScreen.text("  You also find that you aren’t wearing your " + player.armorName + ", and your " + player.weaponName + " is nowhere to be seen.  ");
		...
		if((player.lowerBody.cockSpot.hasCock() && player.smallestCockArea() <= 40) && player.upperBody.chest.BreastRatingLargest[0].breastRating <= 25 && !player.isTaur() && !player.isNaga() && !player.isGoo() && !player.isDrider())
	...
	public cowCuntBadEndPartIII():void
	{
		...
		if(player.hairLength > 0) MainScreen.text("through your " + hairDescript());
	...
	public cowCuntBadEndPartIV():void
	{
		...
		{
		 	MainScreen.text("At this point your " + multiCockDescriptLight() + " is rock hard, and Clara gives it a few strokes.  She does seem to lose interest surprisingly quick though.");
		}
		else if (player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock())
		{
		 	MainScreen.text("You’ve become quite wet at this point, and Clara gives your " + vaginaDescript() + " a few rubs, before tugging on your " + clitDescript() + " once.");
		}
	...
	public defeatClaraCuntInAFight(cheated:boolean = false):void
	{
		...
		//(if Marble is a futa)
		if(flags[FlagEnum.MARBLE_DICK_TYPE] > 0) MainScreen.text("egg Marble into violating Clara with her cock, ");
		...
		if(flags[FlagEnum.MARBLE_DICK_TYPE] > 0) MainScreen.addButton(3,"Marble Cock",futaMarbleIsAHugeCowToCowCuntAndStuffsCowCuntsCuntFullOfCowCock);
	...
	//does not work with centaur, naga, drider, or goo bodies.  However, I don't particularly want to just say you can't do this scene if you have them.
	{
		...
		if((player.lowerBody.cockSpot.hasCock() && x >= 0) && flags[FlagEnum.MARBLE_DICK_TYPE] > 0)
		{
		...
		else if((player.lowerBody.cockSpot.hasCock() && x >= 0) && flags[FlagEnum.MARBLE_DICK_TYPE] == 0)
		{
			//n is the index of the PC's biggest cock under 20 area.
	...
	public NTRIsClearlyTheWorstFetishWhyWouldYouWriteThisOMG():void {
		...
		{
		 	MainScreen.text("she drops herself onto your " + cockDescript(x) + ".  Since your bovine mate isn't in the habit of wearing any form of undergarments, her skirt doesn't really do much to get in the way of penetration.  A shriek of dismay accompanies this act.");
		...
		else if(player.lowerBody.vaginaSpot.hasVagina() && flags[FlagEnum.MARBLE_DICK_TYPE] > 0)
		{
	...
	public calmMurblesFatAssDown():void
	{
		...
		if((player.tallness >= 80 && flags[FlagEnum.MARBLE_BOVA_LEVEL] < 2) || (player.tallness >= 86 && flags[FlagEnum.MARBLE_BOVA_LEVEL] >= 2))
		{
	...
	public rathazulFinishingMarblesPotion():void
	{
		...
		if(flags[FlagEnum.MARBLE_KIDS] > 0)
		{
			...
			if(flags[FlagEnum.MARBLE_KIDS] > 0) MainScreen.text("ren");
		 	MainScreen.text(".  ");
		}
	...
	public purifyMarbleComplete():void
	{
		...
		if(flags[FlagEnum.MARBLE_KIDS] > 0)
		{
			...
			if(flags[FlagEnum.MARBLE_KIDS] > 1) MainScreen.text("ren");
		 	MainScreen.text(" right away.  Give ");
			if(flags[FlagEnum.MARBLE_KIDS] == 1) MainScreen.text("her");
			...
			if(flags[FlagEnum.MARBLE_KIDS] > 1) MainScreen.text("each of ");
		 	MainScreen.text("your child");
			if(flags[FlagEnum.MARBLE_KIDS] > 1) MainScreen.text("ren");
		 	MainScreen.text(" two drops of the formula.  ");
			if(flags[FlagEnum.MARBLE_KIDS] > 1) MainScreen.text("They ");
			else MainScreen.text("She ");
		 	MainScreen.text("take");
			if(flags[FlagEnum.MARBLE_KIDS] == 1) MainScreen.text("s");
		 	MainScreen.text(" it easily enough, and while there are no outward effects, ");
			if(flags[FlagEnum.MARBLE_KIDS] == 1) MainScreen.text("your ");
			else MainScreen.text("each ");
		 	MainScreen.text("child does report feeling like something was lifted from ");
			if(flags[FlagEnum.MARBLE_KIDS] > 1) MainScreen.text("their");
	...
	public purifyMurblesSister():void
	{
		MainScreen.clearText();
	 	...
	 	flags[FlagEnum.CLARA_PURIFIED] = 1;
	...
	public cageDatCowCunt():void
	{
		MainScreen.clearText();
	 	...
	 	flags[FlagEnum.CLARA_IMPRISONED] = 1;
	...
	public purificationQuestFinal():void
	{
		...
		flags[FlagEnum.MARBLE_PURIFICATION_STAGE] = 5;
		flags[FlagEnum.MARBLE_PURIFIED] = 1;
	...
	public actuallyGrowPureMarblesTittiesForFunzies():void
	{
		...
		if(flags[FlagEnum.MARBLE_BREAST_SIZE] < 3)
		{
			...
			else if(flags[FlagEnum.MARBLE_BREAST_SIZE] == 2)
			{
	...
	public pureMurblePCChoosesYesToShrinkeyTits():void
	{
		...
		if (flags[FlagEnum.MARBLE_BREAST_SIZE] > 0)
		{
			...
			else if(flags[FlagEnum.MARBLE_BREAST_SIZE] == 1)
			{
			...
			flags[FlagEnum.MARBLE_LUST] -= 5;
			if(flags[FlagEnum.MARBLE_LUST] < 0) flags[FlagEnum.MARBLE_LUST] = 0;
		}
		else 
		{
	...
	public lactaidForPureMurble():void
	{
		...
		flags[FlagEnum.MARBLE_TIME_SINCE_NURSED_IN_HOURS] += 10;
	...
	public nursingFromPureMarble():void
	{
		...
		if(flags[FlagEnum.MARBLE_TIME_SINCE_NURSED_IN_HOURS] < 4)
		{
		...
		else
		{
			...
			flags[FlagEnum.MARBLE_TIME_SINCE_NURSED_IN_HOURS] = 0;
			if(flags[FlagEnum.MARBLE_LUST] > 0) flags[FlagEnum.MARBLE_LUST] = 100;
			doNext(camp.returnToCampUseOneHour);
		}
	}
	...
	public marbleWarnsPCAboutCorruption():void
	{
		MainScreen.clearText();
	 	...
	 	flags[FlagEnum.MARBLE_WARNED_ABOUT_CORRUPTION] = 1;
		//end event
		doNext(playerMenu);
	}
	...
	public marbleLeavesThePCOverCorruption():void
	{
		...
		if(flags[FlagEnum.MARBLE_KIDS] > 1) MainScreen.text(" , and with her children in tow");
		//else if (PC has one child with Marble)
		else if(flags[FlagEnum.MARBLE_KIDS] == 1) MainScreen.text(", and holding her child's hand");
		...
		flags[FlagEnum.MARBLE_LEFT_OVER_CORRUPTION] = 1;
	...
	public pureMarbleDecidesToBeLessOfABitch():void
	{
		...
		//if (PC has at least one child with Marble)
		if(flags[FlagEnum.MARBLE_KIDS] > 0) MainScreen.text(" and that of your child");
		...
		flags[FlagEnum.MARBLE_LEFT_OVER_CORRUPTION] = 0;
		flags[FlagEnum.MARBLE_WARNED_ABOUT_CORRUPTION] = 0;
	...
	public claraCampAddition():void
	{
		...
		if(flags[FlagEnum.CLARA_PURIFIED] > 0)
		{
