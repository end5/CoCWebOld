...
package classes.Scenes.NPCs {
	...
	public function BLUHBLUH():void {
		clearOutput();
	 	Render.text("When you return to camp, you find Marble seated at the campfire, staring gloomily into the ashes. Curious, you approach your bovine lover and ask her what the matter is. She looks up at you, her ");
		if(flags[FlagEnum.MARBLE_BOVA_LEVEL] <= 1) Render.text("human");
		else Render.text("bovine");
	 	Render.text(" head still in her hands, and sighs loudly. <i>\"I’m sorry, sweetie, I just... I’ve tried to ignore it, but I just don’t feel comfortable here.\"</i>");
		...
		{
		 	Render.text("  ...Has she been fighting with ");
			if(amilyScene.amilyFollower() && !amilyScene.amilyCorrupt()) Render.text("Amily");
			else if(followerKiha()) Render.text("Kiha");
			else Render.text("Izma");
		 	Render.text("?");
		}
		...
		if(player.hasItem(consumables.PURHONY)) Render.text("  You then recall that bottle of honey the bee-girl gave you, and ask if maybe it might work?");
		if(player.statusAffects.has("PureCampJojo")) Render.text("  You remember how Jojo helped you become pure, and suggest maybe his meditations could assist Marble?");
	...
	public function giveMarblePureQuestLabova():void
	{
		...
		player.consumeItem(consumables.P_LBOVA);
	 	Render.text("You tell Marble that you managed to get some LaBova alchemically purified, and you want her to drink some.");
		//First time
	...
	public function visitRathazulToPurifyMarbleAfterLaBovaStopsWorkin():void
	{
		...
		{
		 	Render.text("You spot the elderly alchemist's camp on the lake once more.\n\n");
		}
	 	Render.text("As you approach the rodentine alchemist, you remember Marble’s problems and resolve to ask him if he can possibly help you. Once he finishes greeting you, you politely return the greeting and ask if he would be willing to lend his opinion on a problem you have.");
	...
	{
		clearOutput();
	 	Render.text("Fortunately, Marble is at your camp rather than off at Whitney’s, and your bovine lover looks up at you hopefully as you approach. \"<i>Did he say he could help?</i>\" she asks, excitedly.");
	...
	public function rathazulsMurbelReport():void
	{
		...
		flags[FlagEnum.MARBLE_RATHAZUL_COUNTER_1] = 0;
	 	Render.text("When you wake and join Marble ");
		if(kGAMECLASS.camp.companionsCount() > 2) Render.text("and your other followers ");
	 	Render.text("for breakfast this morning, you find a bleary-eyed Rathazul waiting at the campfire. Marble clucks her tongue and gives him a mug of something strong, which he accepts gratefully, noisily slurping down a mouthful. He takes a seat, staring into his mug, and then sighs softly.");
	...
	{
		clearOutput();
	 	Render.text("You shake your head and tell Marble that you still need her at the camp; you’ll tell her when you can spare her.");
	...
	{
		clearOutput();
	 	Render.text("You tell Marble that if she needs some time off to go and see her family for this matter, it’s okay with you.");
	...
	public function claraShowsUpInCampBECAUSESHESACUNT():void
	{
		...
		if(flags[FlagEnum.MARBLE_BOVA_LEVEL] <= 1) Render.text("your mate has");
		else Render.text("what your mate use to have");
	 	Render.text("; cow legs, ears, horns, and large breasts.  However, she doesn’t have Marble’s brown fur, instead having black and white blotched fur across her legs and ears and a mane of white hair atop her head.  She is wearing a one piece thick cloth dress, with a great deal of fancy embroidery decorating it.  You also guess that she’s a bit smaller than Marble, in both the chest and height departments.");
		...
		if(player.stats.int >= 60) Render.text(" with a odd smile on her face");
		...
		if(player.stats.int >= 60) Render.text(" somewhat unconvincingly");
	 	Render.text(".");
	...
	{
		clearOutput();
	 	Render.text("You turn back to the camp and continue going about your morning routine.  By the time you’re ready to head out, the girls have come back from the farm, bearing a bottle of Clara’s milk.  The spotted bovine’s foul mood seems to have improved as well.");
		...
		if(kGAMECLASS.camp.companionsCount() >= 3)
		{
			...
			if (isabellaFollower())
			{
				...
				if (isabellaAccent())
				{
					Render.text("  Isabella jumps from her chair, “<i>Tea?  Ooh, ja!  I hear tea und milk iz sehr gut combination, ja?</i>”  Well, she seems rather enthusiastic if nothing else.");
				}
				else
				{
					Render.text("  Isabella jumps from her chair, “<i>Tea? Ooh, yes!  I hear tea and milk is a really good combination, yes? I've never had the opportunity to try it though...</i>”  Well, she seems rather enthusiastic if nothing else.");
				}
	...
	public function dumbshitCowCuntsTryToDrugYouBecauseTheyreCunts(teaDrinkers: number):void
	{
		clearOutput();
	 	Render.text("Upon returning to the center of activity, you find a rather annoyed looking Marble sitting off to the side while Clara busies herself with a large number of cups from her pack.  Curious over what happened, you sit down next to Marble while watching Clara check the temperature of the pot.  After a moment, you ask your bovine lover why she is just watching.  \"<i>After I got the fire started, Clara insisted that I’d get in the way and ruin the tea if I tried to help,</i>\" she grumbles in annoyance.");
		...
		//new page
		if(player.stats.int >= 60) addButton(0,"Next",highIntelligenceOutcomesVsCowCuntDrugs);
		else if(player.stats.int >= 30) addButton(0,"Next",mediumIntelligenceReactsToDrugging);
	...
	{
		clearOutput();
	 	Render.text("Once everyone has a cup, Clara smiles at all of you and holds her own cup up to her mouth and says, \"<i>To Marble’s purification and meeting new friends.</i>\"");
	...
	public function partTwoOfLowIntelligencePCsFallingForTheIncrediblyObviousCowCuntsTrap():void
	{
		...
		if(flags[FlagEnum.MARBLE_BOVA_LEVEL] <= 1) Render.text("face");
		else Render.text("muzzle");
	 	Render.text(", you try to figure out what the heck is going on.  You remember meeting Marble’s sister, then getting tea, and then...  What did Clara say to Marble?  You groan slightly, your body feeling an odd old craving.");
		...
		if(player.weaponName != "fists") Render.text("your [weapon] is nearby, and ");
	 	Render.text(" you’re still wearing your [armor].");
	...
	public function fightCowCuntAfterBeingTotesMcGoatsDrugged():void {
		clearOutput();
	 	Render.text("You steel yourself, ");
		if(player.weaponName == "fists") Render.text("raise your fists");
		else Render.text("grab your " + player.weaponName);
	 	Render.text(" and get ready to fight to free yourself from this mad milkmaid’s mammaries.  Realizing your intentions, Clara’s face turns from a crazed grin to an angry glare.  With barely contained fury, she screams at you, \"<i>Oh don’t you worry darling, I will <b>make</b> you love drinking my milk!</i>\" before standing up, hefting a nasty looking heavy mace, and charging at you.");
		//go to Clara’s fight
	...
	{
		clearOutput();
	 	Render.text("Giving into the desires that your addiction has granted you, you return to Clara’s breast, much to the young cow-girl’s moans and sighs of pleasure.  \"<i>Ahh, yes, that's a goood " + player.mf("boy","girl") + ",</i>\" is whispered in your ear.");
	...
	{
		clearOutput();
	 	Render.text("While Clara is handing out the cups, you can’t help but think how strange everything about this feels.  You can’t put your finger on what exactly is wrong, but neither can you put the unsettling feeling out of your mind.");
	...
	{
		clearOutput();
	 	Render.text("You ready yourself for battle as Clara pulls an angry looking heavy mace out and says, \"<i>Fine, we’ll doo this the hard way.</i>\"");
	...
	public function loseToClara(combat:boolean = true):void
	{
		...
		if(combat)
		{
			if(player.HP < 1) Render.text("Your body gives out from the strain, and you fall to the ground.  The last thing you hear before you pass out, is Clara’s triumphant laughter.");
			//proceed to loss scene
			//PC loses via lust
			else Render.text("You just can’t fight it anymore, and desperately start to masturbate in front of Clara.  Laughing triumphantly, Clara walks up to you and forces a pill down your throat.  A moment later, you pass out.");
			Render.text("\n\n");
		}
		//proceed to loss scene
		...
		//PC wakes up laying down on a bed roll.  They feel very groggy and weak.  After a moment, they realize that they’ve got a strong craving for milk and that they need to drink it if they’re going to survive.  Clara then shows up and proceeds to demonstrate her control over the PC, taking great pleasure in it.
	 	...
	 	Render.text("You dream of odd things, distorted images that you called out for help to.  Then you were dragged away.  You kept seeing more shapes that felt familiar and important somehow, but you couldn’t focus on them.  You’d call again and again, only to be dragged away by something else again.  Eventually your cries for help become weaker and halfhearted; you know what’s going to happen next, so why bother?  Suddenly you realize that it is a cow-girl that is dragging you away; her face looks like Marble’s at first, then it splits into a huge, wicked smile, and you panic.");
		...
		if(player.tallness < 70) Render.text("much too big for you.");
		else if(player.tallness <= 82) Render.text("just your size.");
		else Render.text("way too small for your stature.");
	 	Render.text("  You also find that you aren’t wearing your " + player.armorName + ", and your " + player.weaponName + " is nowhere to be seen.  ");
		...
		if((player.lowerBody.cockSpot.hasCock() && player.smallestCockArea() <= 40) && player.upperBody.chest.BreastRatingLargest[0].breastRating <= 25 && !player.isTaur() && !player.isNaga() && !player.isGoo() && !player.isDrider())
		 	Render.text("Instead a plain but rather slim form fitting two piece garb adorns your body.  Whoever dressed you in that definitely wanted your bodily assets to be fully visible to anyone looking at you.");
		else Render.text("You’re completely naked.");
	...
	{
		clearOutput();
	 	Render.text("It is then that you realize with a shock that a familiar urge is welling up inside you.  No, it is much stronger than you remember it being before.  You want breastmilk.  You <i>need</i> breastmilk, and if you don’t get it, you’ll die.  To make matters worse, you know whose breastmilk it is too.  Clara, Marble’s younger sister, has fatally addicted you to her milk.");
		...
		{
		 	Render.text("\"<i>Oh?  Wondering how I know about that?  Marble told me when she was explaining about you breaking free of the addiction.  Unlike her, I’m going to make sure you behave yourself.</i>\"  ");
		} 
		else
		{
		 	Render.text("\"<i>Confused?  That was one of the things Marble found out when she was researching the nature of her addiction.  She said she didn’t want to trouble you with that knowledge, but I don’t see why.</i>\"  ");
		}
	 	Render.text("This woman is nothing like her sister, she loves the power she has over you, and loves to show you just how much she has.");
		...
		if(kGAMECLASS.camp.loversCount() == 1) Render.text("Marble");
		else Render.text("the others");
	 	Render.text(".  Then your eyes drop down to Clara’s chest, and you think of your need.");
	...
	public function cowCuntBadEndPartIII():void
	{
		...
		if(player.hairLength > 0) Render.text("through your " + hairDescript());
		else Render.text("across your scalp");
	 	Render.text(", then they suddenly grip down tight.  \"<i>Let’s see if you can pick up the pace.  Drink faster, my love slave!</i>\"  You do as the voice asks, and loud moans of pleasure follow.");
	...
	public function cowCuntBadEndPartIV():void
	{
		...
		{
		 	Render.text("At this point your " + multiCockDescriptLight() + " is rock hard, and Clara gives it a few strokes.  She does seem to lose interest surprisingly quick though.");
		}
		else if (player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock())
		{
		 	Render.text("You’ve become quite wet at this point, and Clara gives your " + vaginaDescript() + " a few rubs, before tugging on your " + clitDescript() + " once.");
		}
		else if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina())
		{
		 	Render.text("Your arousal at this point is quite apparent on the whole of your genitals.  Clara gives you a few touches and caresses, but doesn’t seem all that interested.");
		}
		else
		{
		 	Render.text("Or rather, your lack of thereof.  Clara doesn’t seem to mind too much; Marble must have told her about this part of you too.");
		}
		...
		menu();
		if(!player.lowerBody.cockSpot.hasCock()) addButton(0,"Next",cocklessPCsGetAbusedByCowCunts);
	...
	{
		clearOutput();
	 	Render.text("\"<i>Now, we do have one problem.  ");
		if(player.lowerBody.cockSpot.count() > 1) 
		{
		 	Render.text("All of your penises are");
		}
		else Render.text("Your penis is");
	 	Render.text(" too big for a virgin like me to fit!  How the hell am I suppose to get that inside me?</i>\"  Clara stands up and starts stomping around the room.  ");
		...
		{
		 	Render.text("\"<i>‘It’s not a flaw, it’s a feature,’ she said, ‘This is why we train ourselves to take big endowments,’ she said!  Fuck you Marble!  I’m not sticking that stupid big thing inside me.</i>\"  ");
		}
		else
		{
	 	...
	 	Render.text("She continues ranting for several minutes before turning to you.  \"<i>YOOU!  Didn’t you ever stop to consider the needs of dainty girls like me before you started abusing transformatives?</i>\"  She sighs, apparently running out of steam.  \"<i>Who knew this plan would be so hard?");
	...
	public function defeatClaraCuntInAFight(cheated:boolean = false):void
	{
		...
		else
		{
			...
			{
			 	Render.text("Worried about the others, you head out of the cellar and drag Clara and her things with you.  Thanks to the strange way that travel works in this world, it doesn't take you long to find your way back to camp.  You catch your followers there just before they head out to go looking for you and dump your would be kidnaper before them.  Marble steps up next to you and looks down at her sister.");
		...
		//Raping or punishing Clara in camp with your followers
	 	Render.text("\"<i>Clara, what the hell were you trying to pull there?</i>\" Marble demands of her sister, her hands shaking in rage.");
		...
		{
		 	Render.text("I'm not even " + player.short + "'s only mate.  " + player.mf("He","She") + " can satisfy their needs, but can't satisfy mine?  "); //no new paragraph
		}
	 	Render.text("The frustration of not being able to nurse, someone, anyone, but especially my mate is tearing me apart!</i>\"  Marble stops screaming and her head drops.  All that's coming out of her now is heavy breathing.");
		...
		if((player.lowerBody.cockSpot.hasCock() && player.cockThatFits(20) >= 0) || player.lowerBody.vaginaSpot.hasVagina()) Render.text("use Marble's unstable state of mind to get her into a forceful threesome with Clara, ");
		//(if PC either has a cock that will fit Marble, or has a vagina)
		if((player.lowerBody.cockSpot.hasCock() && player.cockThatFits(marbleScene.marbleCuntCapacity()) >= 0) || player.lowerBody.vaginaSpot.hasVagina()) Render.text("have sex in front of Clara so she can see what your love really is, ");
		//(if Marble is a futa)
		if(flags[FlagEnum.MARBLE_DICK_TYPE] > 0) Render.text("egg Marble into violating Clara with her cock, ");
	 	Render.text(" or try to calm Marble down so that she doesn't do anything to her sister.");
		...
		if((player.lowerBody.cockSpot.hasCock() && player.cockThatFits(20) >= 0) || player.lowerBody.vaginaSpot.hasVagina()) addButton(1,"Threesome",pcAndMurbleDoubleTeamCowButt);
		if((player.lowerBody.cockSpot.hasCock() && player.cockThatFits(marbleScene.marbleCuntCapacity()) >= 0) || player.lowerBody.vaginaSpot.hasVagina()) addButton(2,"NTR",NTRIsClearlyTheWorstFetishWhyWouldYouWriteThisOMG);
		if(flags[FlagEnum.MARBLE_DICK_TYPE] > 0) addButton(3,"Marble Cock",futaMarbleIsAHugeCowToCowCuntAndStuffsCowCuntsCuntFullOfCowCock);
	...
	{
		clearOutput();
	 	Render.text("Marble seems to have a good idea on how to punish Clara already, so you just sit back and watch.");
	...
	//does not work with centaur, naga, drider, or goo bodies.  However, I don't particularly want to just say you can't do this scene if you have them.
	{
		clearOutput();
		let x: number = player.cockThatFits(20);
	 	Render.text("You step forward and suggest to Marble that the two of you should work together to punish Clara.  She nods and asks what you have in mind.  Well, you were thinking that a sexual punishment from the two of you would be appropriate.");
		...
		if((player.lowerBody.cockSpot.hasCock() && x >= 0) && flags[FlagEnum.MARBLE_DICK_TYPE] > 0)
		{
			//n is the index of the PC's biggest cock under 20 area.
		 	Render.text("Eventually you pull her up and sandwich her between your bodies.  Marble expresses an aversion to anal, so you take the backside while she takes the front.  \"<i>Hey, you can't do this to me!  I don't want to lose my virginity to my sister!</i>\" Clara cries out indignantly.");
	...
	public function NTRIsClearlyTheWorstFetishWhyWouldYouWriteThisOMG():void {
		...
		{
		 	Render.text("she drops herself onto your " + cockDescript(x) + ".  Since your bovine mate isn't in the habit of wearing any form of undergarments, her skirt doesn't really do much to get in the way of penetration.  A shriek of dismay accompanies this act.");
		...
		{
		 	Render.text("she drops herself onto the ground next to you.  Abruptly, she lifts you up and drops your womanhood onto the member you gave her.  Since your bovine mate isn't in the habit of wearing any form of undergarments, her skirt doesn't really do much to get in the way of penetration.  A shriek of dismay accompanies this act.");
	...
	{
		clearOutput();
	 	Render.text("You call out to Marble and suggest that she should use her new \"appendage\" to punish Clara.  She hesitates for a moment, then nods.  You doubt that you could have ever gotten her to do something like that if she wasn't as worked up as she is.");
	...
	public function calmMurblesFatAssDown():void
	{
		...
		{
		 	Render.text("tightly hugs herself against your body.  Gently stroking her hair, you're happy to be a shoulder for her to cry on, given both your affection for the gentle girl and the awareness that you're able to help her feel better.");
		} 
		//else if (PC taller than 5 feet)
		else if(player.tallness >= 60)
		{
		 	Render.text("returns the hug.  Rocking back and forth against one another, you're happy to be here for her, given both your affection for the lovely lady and the warm feeling of being so close to one you care so much for.");
		}
		else
		{
		 	Render.text("scoops you up, hugging you tightly into her chest.  Sandwiched between her massive mammaries, you're happy to serve as her temporary comfort doll, given both your affection for the big bovine and the pleasant accommodation of her bountiful breasts.");
		}
	...
	public function rathazulFinishingMarblesPotion():void
	{
		...
		{
		 	Render.text("Then we'll be ready to purify you and your child");
			if(flags[FlagEnum.MARBLE_KIDS] > 0) Render.text("ren");
		 	Render.text(".  ");
		}
	 	Render.text("So come back a little later.</i>\"  The rodent returns to his reactions.");
	...
	{
		clearOutput();
	 	Render.text("Rathazul tells you that the formula is ready, so you head over to Marble to let her know.");
	...
	{
		clearOutput();
	 	Render.text("The two of you arrive at Rathazul's workspace, and find the owner awaiting you, holding a bottle of whitish fluid with gold streaks running throughout.  \"<i>So is that the formula?</i>\"  Marble asks looking at the bottle.");
		...
		if(flags[FlagEnum.MARBLE_KIDS] > 0)
		{
			...
			if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text("ren");
		 	Render.text(" right away.  Give ");
			if(flags[FlagEnum.MARBLE_KIDS] == 1) Render.text("her");
			else Render.text("each");
		 	Render.text(" a two drop dose,</i>\" the alchemist instructs you and hands over the bottle of his formula.");

		 	Render.text("\n\nYou and Marble waste no time in heading over to the nursery and feed ");
			if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text("each of ");
		 	Render.text("your child");
			if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text("ren");
		 	Render.text(" two drops of the formula.  ");
			if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text("They ");
			else Render.text("She ");
		 	Render.text("take");
			if(flags[FlagEnum.MARBLE_KIDS] == 1) Render.text("s");
		 	Render.text(" it easily enough, and while there are no outward effects, ");
			if(flags[FlagEnum.MARBLE_KIDS] == 1) Render.text("your ");
			else Render.text("each ");
		 	Render.text("child does report feeling like something was lifted from ");
			if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text("their");
	...
	{
		clearOutput();
	 	Render.text("Now there is only one more loose end in this whole effort to purify Marble: her sister.");
	...
	{
		clearOutput();
	 	Render.text("You ask Marble for the bottle of Rathazul's formula.  She nods, hands it to you and says, \"<i>Yeah, I think that's the best choice too.</i>\"");
	...
	{
		clearOutput();
	 	Render.text("You say that you're going to be keeping Clara a prisoner in the camp.  She deserves to be imprisoned so she'll no longer be a threat to anyone anymore.  Marble looks uncertain at you and asks if you're sure she needs to be locked up now that she's been purified.  You say you're certain, and your mate nods to you and says that she trusts your judgment on this matter.");
	...
	{
		clearOutput();
	 	Render.text("You say that now that Clara is no danger to anyone any more, she is free to go, so long as she promises to never bother you or your followers again.  Marble nods at this and says she agrees with your decision.");
	...
	{
		clearOutput();
	 	Render.text("No, you'll offer her the mercy of letting her keep her precious corruption.  Marble tells you she will respect your decision on that matter.  Now the question is what to do with her.");
	 	Render.text("\n\nYou guess you could also build a cage in the camp if you really want to keep an eye on her yourself.  That will make her another mouth to feed, and you'll have to hear her ranting every day.  Still, the saying does go, \"keep your enemies closer.\"");
	 	Render.text("\n\nIf you're feeling especially generous, you could just let her go, with not more than a promise to never come back or try to kidnap you again.");
		//PC makes their choice: Tel'Adre, camp cage, let go."
		menu();
	...
	{
		clearOutput();
	 	Render.text("You say that you're going to be keeping Clara a prisoner in the camp.  She deserves to be imprisoned so she'll be no threat to anyone anymore.  Marble nods to you and says that she trusts your judgment on this matter.");
	...
	{
		clearOutput();
	 	Render.text("\"<i>It has been quite the morning, hasn't it sweetie?</i>\"  It certainly has been, but at least now, your lover is finally free of her corruption once and for all.");
	...
	{
		clearOutput();
	 	Render.text("Do you want to ask Marble to grow her breasts bigger?");
	...
	{
		clearOutput();
	 	Render.text("You offer Marble a bottle of purified succubus milk and tell her that you'd like her to make her bust bigger.");
		...
		if(flags[FlagEnum.MARBLE_BREAST_SIZE] < 3)
		{
			...
			Render.text(kGAMECLASS.marbleScene.marbleBreastSize() + " melons!");
	...
	{
		clearOutput();
	 	Render.text("Do you want to ask Marble to shrink her breasts down?");
		menu();
	...
	{
		clearOutput();
	 	Render.text("You offer Marble a jar of reducto and tell her you'd like her to reduce her bust's size a bit.");
		if (flags[FlagEnum.MARBLE_BREAST_SIZE] > 0)
		{
			...
			flags[FlagEnum.MARBLE_BREAST_SIZE]--;
			Render.text(kGAMECLASS.marbleScene.marbleBreastSize() + " sized breasts.");
			if(flags[FlagEnum.MARBLE_BREAST_SIZE] == 0)
			{
	...
	public function nursingFromPureMarble():void
	{
		...
		{
		 	Render.text("Feeling a bit thirsty, you ask Marble if there’s any chance she has some milk for you.  She gives you a sad look and informs you that no, she hasn't yet built up enough milk for you to have more.  Purifying her has made it take a bit longer for her to produce milk.");
			doNext(marbleScene.interactWithMarbleAtCamp);
		}
		else
		{
		 	Render.text("Feeling a bit thirsty, you ask Marble if theres any chance she has some milk for you.  She smiles at you and tells you she was worried you'd never ask, then leads you to a familiar, secluded part of the rocks around the camp.");
			...
			{
			 	Render.text("The tall cow-lady takes a seat on the rocks and you jump onto her lap, ");
			} 
			else if (player.tallness < 84)
			{
			 	Render.text("You lean forward and ");
			}
			else
			{
			 	Render.text("You lift up the cow-lady to get access, ");
			}
		 	Render.text("in seconds your lips are wrapped around her teats, and her wonderful milk is flowing forth.");
	...
	{
		clearOutput();
	 	Render.text("Marble comes up to you with a worried look on her face.  \"<i>Uh, sweetie?  I'm getting worried about you.  You're trying to put so much effort into freeing me of corruption, but you seem to be slipping away yourself...</i>\"  She looks at the ground, rubs her hands a few times desperately trying to spit the words out.  \"<i>I don't think I'll be able to stay by your side if you keep falling like that.  Please find a way to clear yourself of the corruption that's gripping you!</i>\"");
	...
	{
		clearOutput();
	 	Render.text("Marble approaches you with a somber expression on her face, her things strapped to a pack on her back");
		//if (PC has more than one child with Marble)
		if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text(" , and with her children in tow");
		//else if (PC has one child with Marble)
		else if(flags[FlagEnum.MARBLE_KIDS] == 1) Render.text(", and holding her child's hand");
	 	Render.text(".  \"<i>I'm sorry sweetie.  I tried to warn you, but-</i>\" she stops for a moment and takes a deep breath to steady herself.  ");
		//if (PC has at least one child with Marble)
		if(flags[FlagEnum.MARBLE_KIDS] > 0)
		{
		 	Render.text("\"<i>Mommy, what's wrong?</i>\"  The young one holding Marble's hand asks nervously.\"<i>Hush now, little one.  We have to go away for awhile,</i>\" the bovine mother responds and turns back to you.  ");
		}
	 	Render.text("\"<i>You've gotten too corrupted, and I can't stay here with you anymore.  Please find a way to free yourself from the taint within you so that I can come back.  Goodbye.</i>\"");
	...
	public function pureMarbleDecidesToBeLessOfABitch():void
	{
		...
		clearOutput();
	 	Render.text("While checking your traps to ensure that they're still working, you hear a voice call out from outside the camp, \"<i>Sweetie!</i>\"  You look up and see the smiling face of Marble");
		//if (PC has at least one child with Marble)
		if(flags[FlagEnum.MARBLE_KIDS] > 0) Render.text(" and that of your child");
		//if (PC has two or more children with Marble) 
		if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text("ren");
	 	Render.text(" just outside the camp.  She must have somehow found out about your purification.");
	...
	{
		Render.text("  Nearby the portal sits a large wooden cage for keeping Marble's sister Clara prisoner.  It is tall enough for her to stand in and long enough for her to lie down completely.  ");
		//if (Clara was purified)
		if(flags[FlagEnum.CLARA_PURIFIED] > 0)
		{
			if(rand(10)==0) Render.text("You catch Clara staring at you for a moment, then her gaze once again returns to some distant mountain top.  She has not taken her purification well.");
			else Render.text("As always, Clara sits within, staring off into the distance.  She has not taken her purification well.");
		}
		else
		{
			//if (time of day <= 7 OR >= 20)
			if(kGAMECLASS.time.hours <= 7 || kGAMECLASS.time.hours >= 20) Render.text("Clara is laying sprawled out on the ground of her cage, snoring loudly.");
			else
			{
				temp = rand(4);
				switch (temp)
				{
					case 0:
						Render.text("Clara is currently fidgeting with her clothing, it looks like she's doing some repairs or embroidery.  It's one of the few grateful moments of quiet you get from her.");
						break;
					case 1:
						Render.text("Clara is heckling and throwing insults at your followers right now.  It's very annoying, but everyone ignores her.");
						break;
					case 2:
						Render.text("Clara is currently engaged in trying to tempt you into letting her out of her cage and becoming addicted to her.  It's much the same as all of her previous attempts....");
						break;
					case 3:
						Render.text("Clara is currently loudly whining about the conditions of her confinement to anyone who is willing to listen, which is no one.");
						break;
				}
			}
		}
