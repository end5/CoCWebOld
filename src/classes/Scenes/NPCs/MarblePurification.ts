...
package classes.Scenes.NPCs {
	...
	public function BLUHBLUH():void {
		...
		if(flags[FlagEnum.MARBLE_BOVA_LEVEL] <= 1) outputText("human");
		...
		if(player.statusAffects.has("PureCampJojo")) outputText("  You remember how Jojo helped you become pure, and suggest maybe his meditations could assist Marble?");
	...
	public function giveMarblePureQuestLabova():void
	{
		...
		if(flags[FlagEnum.TIMES_GIVEN_MARBLE_PURE_LABOVA] == 0)
		{
			...
			if(player.statusAffects.get("MarbleSpecials").value2 == 4) outputText("eight");
			else outputText("two");
		...
		else
		{
			...
			if(flags[FlagEnum.MARBLE_BOVA_LEVEL] > 1) outputText(", fur covered");
	...
	public function claraShowsUpInCampBECAUSESHESACUNT():void
	{
		...
		if(flags[FlagEnum.MARBLE_BOVA_LEVEL] <= 1) outputText("your mate has");
		...
		if(player.stats.int >= 60) outputText(" with a odd smile on her face");
		...
		if(player.stats.int >= 60) outputText(" somewhat unconvincingly");
	 	outputText(".");
	...
	public function partTwoOfLowIntelligencePCsFallingForTheIncrediblyObviousCowCuntsTrap():void
	{
		...
		if(flags[FlagEnum.MARBLE_BOVA_LEVEL] <= 1) outputText("face");
	...
	public function defeatClaraCuntInAFight(cheated:boolean = false):void
	{
		...
		if((player.lowerBody.cockSpot.hasCock() && player.cockThatFits(20) >= 0) || player.lowerBody.vaginaSpot.hasVagina()) outputText("use Marble's unstable state of mind to get her into a forceful threesome with Clara, ");
		//(if PC either has a cock that will fit Marble, or has a vagina)
		if((player.lowerBody.cockSpot.hasCock() && player.cockThatFits(marbleScene.marbleCuntCapacity()) >= 0) || player.lowerBody.vaginaSpot.hasVagina()) outputText("have sex in front of Clara so she can see what your love really is, ");
		//(if Marble is a futa)
		if(flags[FlagEnum.MARBLE_DICK_TYPE] > 0) outputText("egg Marble into violating Clara with her cock, ");
	...
	public function rathazulFinishingMarblesPotion():void
	{
		...
		if(flags[FlagEnum.MARBLE_KIDS] > 0)
		{
			...
			if(flags[FlagEnum.MARBLE_KIDS] > 0) outputText("ren");
		 	outputText(".  ");
		}
	...
	public function purifyMarbleComplete():void
	{
		...
		if(flags[FlagEnum.MARBLE_KIDS] > 0)
		{
			...
			if(flags[FlagEnum.MARBLE_KIDS] > 1) outputText("ren");
		 	outputText(" right away.  Give ");
			if(flags[FlagEnum.MARBLE_KIDS] == 1) outputText("her");
			...
			if(flags[FlagEnum.MARBLE_KIDS] > 1) outputText("each of ");
		 	outputText("your child");
			if(flags[FlagEnum.MARBLE_KIDS] > 1) outputText("ren");
		 	outputText(" two drops of the formula.  ");
			if(flags[FlagEnum.MARBLE_KIDS] > 1) outputText("They ");
			else outputText("She ");
		 	outputText("take");
			if(flags[FlagEnum.MARBLE_KIDS] == 1) outputText("s");
		 	outputText(" it easily enough, and while there are no outward effects, ");
			if(flags[FlagEnum.MARBLE_KIDS] == 1) outputText("your ");
			else outputText("each ");
		 	outputText("child does report feeling like something was lifted from ");
			if(flags[FlagEnum.MARBLE_KIDS] > 1) outputText("their");
			else outputText("her");
		 	outputText(" mind.");
	...
	public function marbleLeavesThePCOverCorruption():void
	{
		...
		if(flags[FlagEnum.MARBLE_KIDS] > 1) outputText(" , and with her children in tow");
		//else if (PC has one child with Marble)
		else if(flags[FlagEnum.MARBLE_KIDS] == 1) outputText(", and holding her child's hand");
	...
	public function pureMarbleDecidesToBeLessOfABitch():void
	{
		...
		//if (PC has at least one child with Marble)
		if(flags[FlagEnum.MARBLE_KIDS] > 0) outputText(" and that of your child");
