...
export default class MarblePurification extends NPCAwareContent {
	...
	public BLUHBLUH(): void {
		...
		MainScreen.text("When you return to camp, you find Marble seated at the campfire, staring gloomily into the ashes. Curious, you approach your bovine lover and ask her what the matter is. She looks up at you, her ");
		if (Flags.list[FlagEnum.MARBLE_BOVA_LEVEL] <= 1) MainScreen.text("human");
		else MainScreen.text("bovine");
		MainScreen.text(" head still in her hands, and sighs loudly. <i>\"I’m sorry, sweetie, I just... I’ve tried to ignore it, but I just don’t feel comfortable here.\"</i>");
		...
		if (amilyScene.amilyFollower() && !amilyScene.amilyCorrupt() || followerKiha() || Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00238] == 1) {
			MainScreen.text("  ...Has she been fighting with ");
			if (amilyScene.amilyFollower() && !amilyScene.amilyCorrupt()) MainScreen.text("Amily");
			else if (followerKiha()) MainScreen.text("Kiha");
			else MainScreen.text("Izma");
			MainScreen.text("?");
		}
		...
		if (player.inventory.items.has(consumables.PURHONY)) MainScreen.text("  You then recall that bottle of honey the bee-girl gave you, and ask if maybe it might work?");
		if (player.statusAffects.has("PureCampJojo")) MainScreen.text("  You remember how Jojo helped you become pure, and suggest maybe his meditations could assist Marble?");
	...
	public giveMarblePureQuestLabova(): void {
		...
		MainScreen.text("You tell Marble that you managed to get some LaBova alchemically purified, and you want her to drink some.");
		//First time
		if (Flags.list[FlagEnum.TIMES_GIVEN_MARBLE_PURE_LABOVA] == 0) {
			//Marble’s corruption is reduced by 4
			player.statusAffects.get("Marble").value4 = -4;
		...
		else if (player.statusAffects.get("MarbleSpecials").value4 <= 30) {
	...
	public visitRathazulToPurifyMarbleAfterLaBovaStopsWorkin(): void {
		...
		if (!player.statusAffects.has("CampRathazul")) {
			MainScreen.text("You spot the elderly alchemist's camp on the lake once more.\n\n");
		}
		MainScreen.text("As you approach the rodentine alchemist, you remember Marble’s problems and resolve to ask him if he can possibly help you. Once he finishes greeting you, you politely return the greeting and ask if he would be willing to lend his opinion on a problem you have.");
	...
	public pageTwoOfVisitingTheRatForMurblesPurification(): void {
		MainScreen.clearText();
		MainScreen.text("Fortunately, Marble is at your camp rather than off at Whitney’s, and your bovine lover looks up at you hopefully as you approach. \"<i>Did he say he could help?</i>\" she asks, excitedly.");
	...
	public rathazulsMurbelReport(): void {
		...
		MainScreen.text("When you wake and join Marble ");
		if (kGAMECLASS.camp.companionsCount() > 2) MainScreen.text("and your other followers ");
		MainScreen.text("for breakfast this morning, you find a bleary-eyed Rathazul waiting at the campfire. Marble clucks her tongue and gives him a mug of something strong, which he accepts gratefully, noisily slurping down a mouthful. He takes a seat, staring into his mug, and then sighs softly.");
	...
	public stayHereAndDontGoGetYerSisterShesABitch(): void {
		MainScreen.clearText();
		MainScreen.text("You shake your head and tell Marble that you still need her at the camp; you’ll tell her when you can spare her.");
	...
	public murbleShouldGoRunAlongAndGetHerCuntySisterSoTheyCanBeCuntsTogether(): void {
		MainScreen.clearText();
		MainScreen.text("You tell Marble that if she needs some time off to go and see her family for this matter, it’s okay with you.");
	...
	public claraShowsUpInCampBECAUSESHESACUNT(): void {
		...
		if (Flags.list[FlagEnum.MARBLE_BOVA_LEVEL] <= 1) MainScreen.text("your mate has");
		else MainScreen.text("what your mate use to have");
		MainScreen.text("; cow legs, ears, horns, and large breasts.  However, she doesn’t have Marble’s brown fur, instead having black and white blotched fur across her legs and ears and a mane of white hair atop her head.  She is wearing a one piece thick cloth dress, with a great deal of fancy embroidery decorating it.  You also guess that she’s a bit smaller than Marble, in both the chest and height departments.");
		...
		if (player.stats.int >= 60) MainScreen.text(" with a odd smile on her face");
		...
		if (player.stats.int >= 60) MainScreen.text(" somewhat unconvincingly");
		MainScreen.text(".");
	...
	public partTwoOfClaraShowingUpAndBeingACunt(): void {
		MainScreen.clearText();
		MainScreen.text("You turn back to the camp and continue going about your morning routine.  By the time you’re ready to head out, the girls have come back from the farm, bearing a bottle of Clara’s milk.  The spotted bovine’s foul mood seems to have improved as well.");
		...
		if (kGAMECLASS.camp.companionsCount() >= 3) {
			...
			if (isabellaFollower()) {
				...
				if (isabellaAccent()) {
					MainScreen.text("  Isabella jumps from her chair, “<i>Tea?  Ooh, ja!  I hear tea und milk iz sehr gut combination, ja?</i>”  Well, she seems rather enthusiastic if nothing else.");
				}
				else {
					MainScreen.text("  Isabella jumps from her chair, “<i>Tea? Ooh, yes!  I hear tea and milk is a really good combination, yes? I've never had the opportunity to try it though...</i>”  Well, she seems rather enthusiastic if nothing else.");
				}
	...
	public lowIntelligenceGetsDruggedByCowSluts(): void {
		MainScreen.clearText();
		MainScreen.text("Once everyone has a cup, Clara smiles at all of you and holds her own cup up to her mouth and says, \"<i>To Marble’s purification and meeting new friends.</i>\"");
	...
	public partTwoOfLowIntelligencePCsFallingForTheIncrediblyObviousCowCuntsTrap(): void {
		...
		if (Flags.list[FlagEnum.MARBLE_BOVA_LEVEL] <= 1) MainScreen.text("face");
		else MainScreen.text("muzzle");
		MainScreen.text(", you try to figure out what the heck is going on.  You remember meeting Marble’s sister, then getting tea, and then...  What did Clara say to Marble?  You groan slightly, your body feeling an odd old craving.");
		...
		if (player.weaponName != "fists") MainScreen.text("your [weapon] is nearby, and ");
		MainScreen.text(" you’re still wearing your [armor].");
	...
	public fightCowCuntAfterBeingTotesMcGoatsDrugged(): void {
		MainScreen.clearText();
		MainScreen.text("You steel yourself, ");
		if (player.weaponName == "fists") MainScreen.text("raise your fists");
		else MainScreen.text("grab your " + player.weaponName);
		MainScreen.text(" and get ready to fight to free yourself from this mad milkmaid’s mammaries.  Realizing your intentions, Clara’s face turns from a crazed grin to an angry glare.  With barely contained fury, she screams at you, \"<i>Oh don’t you worry darling, I will <b>make</b> you love drinking my milk!</i>\" before standing up, hefting a nasty looking heavy mace, and charging at you.");
		//go to Clara’s fight
	...
	public startCombatWithMarbleSister(inCamp: boolean = true): void {
		...
		if (inCamp && !player.statusAffects.has("ClaraFoughtInCamp")) player.statusAffects.add(new StatusAffect("ClaraFoughtInCamp", 0, 0, 0, 0)));
	}
	//Chose submit
	public voluntarilyChooseCowCuntBadEndYouBad(): void {
		MainScreen.clearText();
		MainScreen.text("Giving into the desires that your addiction has granted you, you return to Clara’s breast, much to the young cow-girl’s moans and sighs of pleasure.  \"<i>Ahh, yes, that's a goood " + player.mf("boy", "girl") + ",</i>\" is whispered in your ear.");
	...
	public mediumIntelligenceReactsToDrugging(): void {
		MainScreen.clearText();
		MainScreen.text("While Clara is handing out the cups, you can’t help but think how strange everything about this feels.  You can’t put your finger on what exactly is wrong, but neither can you put the unsettling feeling out of your mind.");
	...
	public mediumIntelligenceDoesntDrinkCowCuntsTea(): void {
		...
		if (kGAMECLASS.camp.companionsCount()) MainScreen.text("\n\nYou rush over to your fallen companion");
		else MainScreen.text("\n\nYou rush over to one of your fallen companions");
		MainScreen.text(" to see what has happened, only to find that she has passed out.  Noticing the tea cup again, you turn around to face the person who prepared it.  Clara stands behind you with a disturbing smile on her face.  You demand to know what was in the tea, like Marble did before.");
	...
	public mediumIntelligencesFightCowCunts(): void {
		MainScreen.clearText();
		MainScreen.text("You ready yourself for battle as Clara pulls an angry looking heavy mace out and says, \"<i>Fine, we’ll doo this the hard way.</i>\"");
	...
	public loseToClara(combat: boolean = true): void {
		...
		//PC loses via health	
		if (combat) {
			if (player.HP < 1) MainScreen.text("Your body gives out from the strain, and you fall to the ground.  The last thing you hear before you pass out, is Clara’s triumphant laughter.");
			//proceed to loss scene
			//PC loses via lust
			else MainScreen.text("You just can’t fight it anymore, and desperately start to masturbate in front of Clara.  Laughing triumphantly, Clara walks up to you and forces a pill down your throat.  A moment later, you pass out.");
			MainScreen.text("\n\n");
		}
		//proceed to loss scene
		...
		MainScreen.text("You dream of odd things, distorted images that you called out for help to.  Then you were dragged away.  You kept seeing more shapes that felt familiar and important somehow, but you couldn’t focus on them.  You’d call again and again, only to be dragged away by something else again.  Eventually your cries for help become weaker and halfhearted; you know what’s going to happen next, so why bother?  Suddenly you realize that it is a cow-girl that is dragging you away; her face looks like Marble’s at first, then it splits into a huge, wicked smile, and you panic.");
		...
		if (player.tallness < 70) MainScreen.text("much too big for you.");
		else if (player.tallness <= 82) MainScreen.text("just your size.");
		else MainScreen.text("way too small for your stature.");
		MainScreen.text("  You also find that you aren’t wearing your " + player.inventory.armor.displayName + ", and your " + player.weaponName + " is nowhere to be seen.  ");
		...
		if ((player.lowerBody.cockSpot.hasCock() && player.smallestCockArea() <= 40) && player.upperBody.chest.BreastRatingLargest[0].breastRating <= 25 && !player.lowerBody.isTaur() && !player.lowerBody.isNaga() && !player.lowerBody.isGoo() && !player.lowerBody.isDrider())
			MainScreen.text("Instead a plain but rather slim form fitting two piece garb adorns your body.  Whoever dressed you in that definitely wanted your bodily assets to be fully visible to anyone looking at you.");
		else MainScreen.text("You’re completely naked.");
	...
	public losingToCowCuntsPageII(): void {
		MainScreen.clearText();
		MainScreen.text("It is then that you realize with a shock that a familiar urge is welling up inside you.  No, it is much stronger than you remember it being before.  You want breastmilk.  You <i>need</i> breastmilk, and if you don’t get it, you’ll die.  To make matters worse, you know whose breastmilk it is too.  Clara, Marble’s younger sister, has fatally addicted you to her milk.");
		...
		if (player.statusAffects.get("$1").value3 == 1) {
			MainScreen.text("\"<i>Oh?  Wondering how I know about that?  Marble told me when she was explaining about you breaking free of the addiction.  Unlike her, I’m going to make sure you behave yourself.</i>\"  ");
		}
		else {
			MainScreen.text("\"<i>Confused?  That was one of the things Marble found out when she was researching the nature of her addiction.  She said she didn’t want to trouble you with that knowledge, but I don’t see why.</i>\"  ");
		}
		MainScreen.text("This woman is nothing like her sister, she loves the power she has over you, and loves to show you just how much she has.");
		...
		if (kGAMECLASS.camp.loversCount() == 1) MainScreen.text("Marble");
		else MainScreen.text("the others");
		MainScreen.text(".  Then your eyes drop down to Clara’s chest, and you think of your need.");
	...
	public cowCuntBadEndPartIII(): void {
		...
		if (player.upperBody.head.hairLength > 0) MainScreen.text("through your " + HeadDescriptor.describeHair(player));
		else MainScreen.text("across your scalp");
		MainScreen.text(", then they suddenly grip down tight.  \"<i>Let’s see if you can pick up the pace.  Drink faster, my love slave!</i>\"  You do as the voice asks, and loud moans of pleasure follow.");
	...
	public cowCuntBadEndPartIV(): void {
		...
		if (player.lowerBody.cockSpot.hasCock() && !player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("At this point your " + CockDescriptor.describeMultiCockShort(player) + " is rock hard, and Clara gives it a few strokes.  She does seem to lose interest surprisingly quick though.");
		}
		else if (player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text("You’ve become quite wet at this point, and Clara gives your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " a few rubs, before tugging on your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " once.");
		}
		else if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("Your arousal at this point is quite apparent on the whole of your genitals.  Clara gives you a few touches and caresses, but doesn’t seem all that interested.");
		}
		else {
			MainScreen.text("Or rather, your lack of thereof.  Clara doesn’t seem to mind too much; Marble must have told her about this part of you too.");
		}
		//Clara fingers the PC’s asshole
		MainScreen.text("\n\nIt then becomes rather apparent exactly what Clara really wants to play with now, and you feel her fingers slip down and around your body.  She gets herself a handful of your " + ButtDescriptor.describeButt(player) + ", and then slips her fingers into your crevice.  That finger doesn't stop until it's pushed itself inside your " + ButtDescriptor.describeButthole(player) + " and triggered a response that makes Clara laugh with delight.  Feeling emboldened, the bovine girl thrusts another finger into your back entrance and starts wiggling them around.");
		...
		menu();
		if (!player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(0, "Next", cocklessPCsGetAbusedByCowCunts);
	...
	public badEndDickThatFitsClara(): void {
		...
		if ((player.lowerBody.cockSpot.hasCock() && player.smallestCockArea() <= 40) && player.upperBody.chest.BreastRatingLargest[0].breastRating <= 25 && !player.lowerBody.isTaur() && !player.lowerBody.isNaga() && !player.lowerBody.isGoo() && !player.lowerBody.isDrider()) {
			MainScreen.text("In seconds she’s freed you of the form fitting garment that you woke up in.  ");
		}
		MainScreen.text("You’re still erect from the arousal that nursing from Clara gave you.  The young cow-girl straddles your calves and gives your " + CockDescriptor.describeCock(player, x) + " a slight flick with her finger and continues to smile broadly at you.  \"<i>One thing that I was really worried about was that you woouldn’t have a penis that I could actually fit inside me.  Thankfully, that isn’t the case, and that pretty pecker is just my size!</i>\"  She pulls apart the lips of her womanhood before continuing, \"<i>This will be my first time ever putting anything in here, take note just how special of a slave you are to me!</i>\"");

		MainScreen.text("\n\nWell, she certainly isn’t wasting any time, as the very next instant she has lowered her opened labia onto the tip of your " + CockDescriptor.describeCock(player, x) + ".  The passage is already wet and slick, thanks to her previous orgasm.  Immediately, an expression of both pain and pleasure crosses over the bovine woman’s face and she starts to pant.  You lift up your hands, bClara issues a sharp command that you aren’t to do anything at all while she has the sex she wants.  You can’t do much else but just lie back and take it.");
	...
	public cockTooBigForCowCuntageBadEndage(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>Now, we do have one problem.  ");
		if (player.lowerBody.cockSpot.count() > 1) {
			MainScreen.text("All of your penises are");
		}
		else MainScreen.text("Your penis is");
		MainScreen.text(" too big for a virgin like me to fit!  How the hell am I suppose to get that inside me?</i>\"  Clara stands up and starts stomping around the room.  ");
		...
		if (player.cockThatFits(marbleScene.marbleCuntCapacity()) >= 0) {
			MainScreen.text("\"<i>‘It’s not a flaw, it’s a feature,’ she said, ‘This is why we train ourselves to take big endowments,’ she said!  Fuck you Marble!  I’m not sticking that stupid big thing inside me.</i>\"  ");
		}
		else {
		...
		MainScreen.text("She continues ranting for several minutes before turning to you.  \"<i>YOOU!  Didn’t you ever stop to consider the needs of dainty girls like me before you started abusing transformatives?</i>\"  She sighs, apparently running out of steam.  \"<i>Who knew this plan would be so hard?");
	...
	public cocklessPCsGetAbusedByCowCunts(): void {
		...
		if (player.upperBody.head.face.tongueType > 0) {
	...
	public defeatClaraCuntInAFight(cheated: boolean = false): void {
		if (cheated) MainScreen.clearText();
			...
			else if (!player.statusAffects.has("ClaraFoughtInCamp")) {
				MainScreen.text("Worried about the others, you head out of the cellar and drag Clara and her things with you.  Thanks to the strange way that travel works in this world, it doesn't take you long to find your way back to camp.  You catch your followers there just before they head out to go looking for you and dump your would be kidnaper before them.  Marble steps up next to you and looks down at her sister.");
		...
		MainScreen.text("\"<i>Clara, what the hell were you trying to pull there?</i>\" Marble demands of her sister, her hands shaking in rage.");
		...
		if (kGAMECLASS.camp.loversCount() > 1) {
			MainScreen.text("I'm not even " + player.short + "'s only mate.  " + player.mf("He", "She") + " can satisfy their needs, but can't satisfy mine?  "); //no new paragraph
		}
		MainScreen.text("The frustration of not being able to nurse, someone, anyone, but especially my mate is tearing me apart!</i>\"  Marble stops screaming and her head drops.  All that's coming out of her now is heavy breathing.");
		...
		if ((player.lowerBody.cockSpot.hasCock() && player.cockThatFits(20) >= 0) || player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("use Marble's unstable state of mind to get her into a forceful threesome with Clara, ");
		//(if PC either has a cock that will fit Marble, or has a vagina)
		if ((player.lowerBody.cockSpot.hasCock() && player.cockThatFits(marbleScene.marbleCuntCapacity()) >= 0) || player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("have sex in front of Clara so she can see what your love really is, ");
		//(if Marble is a futa)
		if (Flags.list[FlagEnum.MARBLE_DICK_TYPE] > 0) MainScreen.text("egg Marble into violating Clara with her cock, ");
		MainScreen.text(" or try to calm Marble down so that she doesn't do anything to her sister.");
		...
		if ((player.lowerBody.cockSpot.hasCock() && player.cockThatFits(20) >= 0) || player.lowerBody.vaginaSpot.hasVagina()) MainScreen.addButton(1, "Threesome", pcAndMurbleDoubleTeamCowButt);
		if ((player.lowerBody.cockSpot.hasCock() && player.cockThatFits(marbleScene.marbleCuntCapacity()) >= 0) || player.lowerBody.vaginaSpot.hasVagina()) MainScreen.addButton(2, "NTR", NTRIsClearlyTheWorstFetishWhyWouldYouWriteThisOMG);
	...
	public murbleSpanksCowCunt(): void {
		MainScreen.clearText();
		MainScreen.text("Marble seems to have a good idea on how to punish Clara already, so you just sit back and watch.");
	...
	//does not work with centaur, naga, drider, or goo bodies.  However, I don't particularly want to just say you can't do this scene if you have them.
	{
		MainScreen.clearText();
		let x: number = player.cockThatFits(20);
		MainScreen.text("You step forward and suggest to Marble that the two of you should work together to punish Clara.  She nods and asks what you have in mind.  Well, you were thinking that a sexual punishment from the two of you would be appropriate.");
		...
		if ((player.lowerBody.cockSpot.hasCock() && x >= 0) && Flags.list[FlagEnum.MARBLE_DICK_TYPE] > 0) {
			//n is the index of the PC's biggest cock under 20 area.
			MainScreen.text("Eventually you pull her up and sandwich her between your bodies.  Marble expresses an aversion to anal, so you take the backside while she takes the front.  \"<i>Hey, you can't do this to me!  I don't want to lose my virginity to my sister!</i>\" Clara cries out indignantly.");
			...
			MainScreen.text("\n\nMarble looks to the side of her sister's head, and gives you a solemn nod.  You take that as an invitation to begin the violation and grab a good grip of Clara's expansive rear.  Then you prod your " + CockDescriptor.describeCock(player, x) + " at the entrance to her anus, prompting a gasp and plea from the woman you're about to penetrate to stop.  Her sister doesn't heed her begging, and neither do you.");
		...
		else if ((player.lowerBody.cockSpot.hasCock() && x >= 0) && Flags.list[FlagEnum.MARBLE_DICK_TYPE] == 0) {
			//n is the index of the PC's biggest cock under 20 area.
			...
			MainScreen.text("\n\n\"<i>You will if you don't want me to twist your ears,</i>\" comes the response.  You prod your " + CockDescriptor.describeCock(player, x) + " up against her tight asshole and suggest that you might be taking a different entrance if she doesn't cooperate for her punishment.  \"<i>Of course, if you're sorry for what you tried to do to " + player.short + "...</i>\" Marble suggests.");
		...
		//else if (PC does not have a cock, Marble does)
			...
			MainScreen.text("\n\n\"<i>What?  Why the hell would I be sorry for that?  Moop!</i>\"  There is a low slurp as your mate pushes her newly acquired seven inch shaft deep inside her sister's womanhood.  \"<i>Hey!  Yo-mmph,</i>\"  You silence her with your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ", roughly forcing it into her face.  You give a slight twist to the young bovine's ears to remind her what she should be doing, and right away her tongue comes out and starts giving you experimental licks.");
			...
			MainScreen.text("\n\nYou express your disappointment to Marble at her sister's unfinished performance, so the fucking continues.  Thankfully, it seems that the bad bovine did get the message and has started to tease and suck on your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " as well as continuing with her teases of your folds and depths.  You'd almost feel bad for doing what you two are doing to her if she wasn't evidently enjoying it as much as she is.  Muffled but loud gasps and moos are constantly being uttered into your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " by the bound girl between you and your lover.  Her breath upon your womanhood only serves to make you feel even better.");
		...
		{
			MainScreen.text("\n\nEventually you roll her onto her back.  Then Marble sits down on her sister's face while you put your mouth to Clara's womanhood and raise your " + ButtDescriptor.describeButt(player) + " up into the air so that your mate can reach your own " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".");
	...
	public NTRIsClearlyTheWorstFetishWhyWouldYouWriteThisOMG(): void {
		...
		if (player.lowerBody.cockSpot.hasCock() && x >= 0) {
			MainScreen.text("she drops herself onto your " + CockDescriptor.describeCock(player, x) + ".  Since your bovine mate isn't in the habit of wearing any form of undergarments, her skirt doesn't really do much to get in the way of penetration.  A shriek of dismay accompanies this act.");
		...
		else if (player.lowerBody.vaginaSpot.hasVagina() && Flags.list[FlagEnum.MARBLE_DICK_TYPE] > 0) {
			MainScreen.text("she drops herself onto the ground next to you.  Abruptly, she lifts you up and drops your womanhood onto the member you gave her.  Since your bovine mate isn't in the habit of wearing any form of undergarments, her skirt doesn't really do much to get in the way of penetration.  A shriek of dismay accompanies this act.");
			MainScreen.text("\n\nThere is no hesitation after you've been invaded, instantly you're being roughly lifted and dropped by a beast that has been awakened in Marble, and there is nothing that can stop it.  A loud slapping sound fills the air with each drop of your hips, only for them to be lifted up and dropped once more.  Your " + LowerBodyDescriptor.describeHips(player) + " have been essentially turned into a jackhammer right now, and its operator shows no signs allowing you to stray from your place on her lap.");
		}
		//else if (PC has a vagina and Marble does not have a cock)
		else if (player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("she drops herself onto your legs, and grabs one of them.  Instantly, your two womanhoods are being grinded against one another with great force and urgency.  Since your bovine mate isn't in the habit of wearing any form of undergarments, her skirt doesn't really do much to get in the way of tribadism.  A shriek of dismay accompanies this act.");
	...
	public futaMarbleIsAHugeCowToCowCuntAndStuffsCowCuntsCuntFullOfCowCock(): void {
		MainScreen.clearText();
		MainScreen.text("You call out to Marble and suggest that she should use her new \"appendage\" to punish Clara.  She hesitates for a moment, then nods.  You doubt that you could have ever gotten her to do something like that if she wasn't as worked up as she is.");
	...
	public calmMurblesFatAssDown(): void {
		...
		if ((player.tallness >= 80 && Flags.list[FlagEnum.MARBLE_BOVA_LEVEL] < 2) || (player.tallness >= 86 && Flags.list[FlagEnum.MARBLE_BOVA_LEVEL] >= 2)) {
			MainScreen.text("tightly hugs herself against your body.  Gently stroking her hair, you're happy to be a shoulder for her to cry on, given both your affection for the gentle girl and the awareness that you're able to help her feel better.");
		}
		//else if (PC taller than 5 feet)
		else if (player.tallness >= 60) {
			MainScreen.text("returns the hug.  Rocking back and forth against one another, you're happy to be here for her, given both your affection for the lovely lady and the warm feeling of being so close to one you care so much for.");
		}
		else {
			MainScreen.text("scoops you up, hugging you tightly into her chest.  Sandwiched between her massive mammaries, you're happy to serve as her temporary comfort doll, given both your affection for the big bovine and the pleasant accommodation of her bountiful breasts.");
		}
	...
	public rathazulFinishingMarblesPotion(): void {
		...
		if (Flags.list[FlagEnum.MARBLE_KIDS] > 0) {
			MainScreen.text("Then we'll be ready to purify you and your child");
			if (Flags.list[FlagEnum.MARBLE_KIDS] > 0) MainScreen.text("ren");
			MainScreen.text(".  ");
		}
		MainScreen.text("So come back a little later.</i>\"  The rodent returns to his reactions.");
	...
	public theFormulaIsDone(): void {
		MainScreen.clearText();
		MainScreen.text("Rathazul tells you that the formula is ready, so you head over to Marble to let her know.");
	...
	public purifyMarbleComplete(): void {
		MainScreen.clearText();
		MainScreen.text("The two of you arrive at Rathazul's workspace, and find the owner awaiting you, holding a bottle of whitish fluid with gold streaks running throughout.  \"<i>So is that the formula?</i>\"  Marble asks looking at the bottle.");
		...
		if (Flags.list[FlagEnum.MARBLE_KIDS] > 0) {
			...
			if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) MainScreen.text("ren");
			MainScreen.text(" right away.  Give ");
			if (Flags.list[FlagEnum.MARBLE_KIDS] == 1) MainScreen.text("her");
			else MainScreen.text("each");
			MainScreen.text(" a two drop dose,</i>\" the alchemist instructs you and hands over the bottle of his formula.");

			MainScreen.text("\n\nYou and Marble waste no time in heading over to the nursery and feed ");
			if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) MainScreen.text("each of ");
			MainScreen.text("your child");
			if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) MainScreen.text("ren");
			MainScreen.text(" two drops of the formula.  ");
			if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) MainScreen.text("They ");
			else MainScreen.text("She ");
			MainScreen.text("take");
			if (Flags.list[FlagEnum.MARBLE_KIDS] == 1) MainScreen.text("s");
			MainScreen.text(" it easily enough, and while there are no outward effects, ");
			if (Flags.list[FlagEnum.MARBLE_KIDS] == 1) MainScreen.text("your ");
			else MainScreen.text("each ");
			MainScreen.text("child does report feeling like something was lifted from ");
			if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) MainScreen.text("their");
			else MainScreen.text("her");
			MainScreen.text(" mind.");
	...
	public dealingWithCowCuntPostPurification(): void {
		MainScreen.clearText();
		MainScreen.text("Now there is only one more loose end in this whole effort to purify Marble: her sister.");
	...
	public purifyMurblesSister(): void {
		MainScreen.clearText();
		MainScreen.text("You ask Marble for the bottle of Rathazul's formula.  She nods, hands it to you and says, \"<i>Yeah, I think that's the best choice too.</i>\"");
	...
	public cageDatCowCunt(): void {
		MainScreen.clearText();
		MainScreen.text("You say that you're going to be keeping Clara a prisoner in the camp.  She deserves to be imprisoned so she'll no longer be a threat to anyone anymore.  Marble looks uncertain at you and asks if you're sure she needs to be locked up now that she's been purified.  You say you're certain, and your mate nods to you and says that she trusts your judgment on this matter.");
	...
	public letGoOfMarblesCowCuntSister(): void {
		MainScreen.clearText();
		MainScreen.text("You say that now that Clara is no danger to anyone any more, she is free to go, so long as she promises to never bother you or your followers again.  Marble nods at this and says she agrees with your decision.");
	...
	public dontPurifyClara(): void {
		MainScreen.clearText();
		MainScreen.text("No, you'll offer her the mercy of letting her keep her precious corruption.  Marble tells you she will respect your decision on that matter.  Now the question is what to do with her.");
		MainScreen.text("\n\nYou guess you could also build a cage in the camp if you really want to keep an eye on her yourself.  That will make her another mouth to feed, and you'll have to hear her ranting every day.  Still, the saying does go, \"keep your enemies closer.\"");
		MainScreen.text("\n\nIf you're feeling especially generous, you could just let her go, with not more than a promise to never come back or try to kidnap you again.");
		//PC makes their choice: Tel'Adre, camp cage, let go."
		menu();
	...
	public stickUnpurifiedClaraInACage(): void {
		MainScreen.clearText();
		MainScreen.text("You say that you're going to be keeping Clara a prisoner in the camp.  She deserves to be imprisoned so she'll be no threat to anyone anymore.  Marble nods to you and says that she trusts your judgment on this matter.");
	...
	public purificationQuestFinal(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>It has been quite the morning, hasn't it sweetie?</i>\"  It certainly has been, but at least now, your lover is finally free of her corruption once and for all.");
	...
	public purifiedSuccubusMilkForPureMarbles(): void {
		MainScreen.clearText();
		MainScreen.text("Do you want to ask Marble to grow her breasts bigger?");
	...
	public actuallyGrowPureMarblesTittiesForFunzies(): void {
		MainScreen.clearText();
		MainScreen.text("You offer Marble a bottle of purified succubus milk and tell her that you'd like her to make her bust bigger.");
		...
		if (Flags.list[FlagEnum.MARBLE_BREAST_SIZE] < 3) {
			...
			MainScreen.text(kGAMECLASS.marbleScene.marbleBreastSize() + " melons!");
	...
	public pureMurbleUsesReducto(): void {
		MainScreen.clearText();
		MainScreen.text("Do you want to ask Marble to shrink her breasts down?");
		menu();
	...
	public pureMurblePCChoosesYesToShrinkeyTits(): void {
		MainScreen.clearText();
		MainScreen.text("You offer Marble a jar of reducto and tell her you'd like her to reduce her bust's size a bit.");
		if (Flags.list[FlagEnum.MARBLE_BREAST_SIZE] > 0) {
			...
			Flags.list[FlagEnum.MARBLE_BREAST_SIZE]--;
			MainScreen.text(kGAMECLASS.marbleScene.marbleBreastSize() + " sized breasts.");
			if (Flags.list[FlagEnum.MARBLE_BREAST_SIZE] == 0) {
	...
	public nursingFromPureMarble(): void {
		...
		if (Flags.list[FlagEnum.MARBLE_TIME_SINCE_NURSED_IN_HOURS] < 4) {
			MainScreen.text("Feeling a bit thirsty, you ask Marble if there’s any chance she has some milk for you.  She gives you a sad look and informs you that no, she hasn't yet built up enough milk for you to have more.  Purifying her has made it take a bit longer for her to produce milk.");
			doNext(marbleScene.interactWithMarbleAtCamp);
		}
		else {
			MainScreen.text("Feeling a bit thirsty, you ask Marble if theres any chance she has some milk for you.  She smiles at you and tells you she was worried you'd never ask, then leads you to a familiar, secluded part of the rocks around the camp.");
			...
			if (player.tallness < 65) {
				MainScreen.text("The tall cow-lady takes a seat on the rocks and you jump onto her lap, ");
			}
			else if (player.tallness < 84) {
				MainScreen.text("You lean forward and ");
			}
			else {
				MainScreen.text("You lift up the cow-lady to get access, ");
			}
			MainScreen.text("in seconds your lips are wrapped around her teats, and her wonderful milk is flowing forth.");
	...
	public marbleWarnsPCAboutCorruption(): void {
		MainScreen.clearText();
		MainScreen.text("Marble comes up to you with a worried look on her face.  \"<i>Uh, sweetie?  I'm getting worried about you.  You're trying to put so much effort into freeing me of corruption, but you seem to be slipping away yourself...</i>\"  She looks at the ground, rubs her hands a few times desperately trying to spit the words out.  \"<i>I don't think I'll be able to stay by your side if you keep falling like that.  Please find a way to clear yourself of the corruption that's gripping you!</i>\"");
	...
	public marbleLeavesThePCOverCorruption(): void {
		MainScreen.clearText();
		MainScreen.text("Marble approaches you with a somber expression on her face, her things strapped to a pack on her back");
		//if (PC has more than one child with Marble)
		if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) MainScreen.text(" , and with her children in tow");
		//else if (PC has one child with Marble)
		else if (Flags.list[FlagEnum.MARBLE_KIDS] == 1) MainScreen.text(", and holding her child's hand");
		MainScreen.text(".  \"<i>I'm sorry sweetie.  I tried to warn you, but-</i>\" she stops for a moment and takes a deep breath to steady herself.  ");
		//if (PC has at least one child with Marble)
		if (Flags.list[FlagEnum.MARBLE_KIDS] > 0) {
			MainScreen.text("\"<i>Mommy, what's wrong?</i>\"  The young one holding Marble's hand asks nervously.\"<i>Hush now, little one.  We have to go away for awhile,</i>\" the bovine mother responds and turns back to you.  ");
		}
		MainScreen.text("\"<i>You've gotten too corrupted, and I can't stay here with you anymore.  Please find a way to free yourself from the taint within you so that I can come back.  Goodbye.</i>\"");
	...
	public pureMarbleDecidesToBeLessOfABitch(): void {
		...
		MainScreen.clearText();
		MainScreen.text("While checking your traps to ensure that they're still working, you hear a voice call out from outside the camp, \"<i>Sweetie!</i>\"  You look up and see the smiling face of Marble");
		//if (PC has at least one child with Marble)
		if (Flags.list[FlagEnum.MARBLE_KIDS] > 0) MainScreen.text(" and that of your child");
		//if (PC has two or more children with Marble) 
		if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) MainScreen.text("ren");
		MainScreen.text(" just outside the camp.  She must have somehow found out about your purification.");
	...
	public claraCampAddition(): void {
		MainScreen.text("  Nearby the portal sits a large wooden cage for keeping Marble's sister Clara prisoner.  It is tall enough for her to stand in and long enough for her to lie down completely.  ");
		//if (Clara was purified)
		if (Flags.list[FlagEnum.CLARA_PURIFIED] > 0) {
			if (rand(10) == 0) MainScreen.text("You catch Clara staring at you for a moment, then her gaze once again returns to some distant mountain top.  She has not taken her purification well.");
			else MainScreen.text("As always, Clara sits within, staring off into the distance.  She has not taken her purification well.");
		}
		else {
			//if (time of day <= 7 OR >= 20)
			if (kGAMECLASS.time.hours <= 7 || kGAMECLASS.time.hours >= 20) MainScreen.text("Clara is laying sprawled out on the ground of her cage, snoring loudly.");
			else {
				temp = rand(4);
				switch (temp) {
					case 0:
						MainScreen.text("Clara is currently fidgeting with her clothing, it looks like she's doing some repairs or embroidery.  It's one of the few grateful moments of quiet you get from her.");
						break;
					case 1:
						MainScreen.text("Clara is heckling and throwing insults at your followers right now.  It's very annoying, but everyone ignores her.");
						break;
					case 2:
						MainScreen.text("Clara is currently engaged in trying to tempt you into letting her out of her cage and becoming addicted to her.  It's much the same as all of her previous attempts....");
						break;
					case 3:
						MainScreen.text("Clara is currently loudly whining about the conditions of her confinement to anyone who is willing to listen, which is no one.");
						break;
				}
			}
		}
