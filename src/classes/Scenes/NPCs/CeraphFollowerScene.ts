...
//[Actually Ceraph] - 
		public function ceraphFollowerAppearance(output:boolean = true):void
		{
			...
			if (output) {
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("  Her demonic prick forms a ridge in the tight fabric, its head peeking ever-so-slightly over the top", false);
...
//*Ceraph is Defeated #4 - Offers Funtimes (Zeddited)
		public function submissiveCeraphOffer():void
		{
			...
			if (player.stats.cor < 33) outputText("a nervous chuckle", false);
			else if (player.stats.cor < 66) outputText("a surprised laugh", false);
...
//Collar Ceraph After 4th Defeat + Rape: (Zeddited)
		private function collarCeraph():void
		{
			...
			if (player.stats.cor < 33) outputText("that you never expected making a demon into your slave would factor into your quest.  On one hand it seems wrong, but... she's a demon.  The fewer you have opposing you, the easier it will be to end their threat completely.", false);
			else if (player.stats.cor < 66) outputText("that taking a demon as a slave would've been abhorrent to you when you started this journey.  Now, it's just a means to a very pleasurable end.", false);
			else {
				outputText("on how much you'll enjoy using the former dom as your personal ", false);
				if (player.lowerBody.cockSpot.hasCock()) outputText("cum-dump", false);
...
//[Pierce] 
		private function getCeraphFollowerPiercing():void
		{
			...
			if (player.stats.cor < 40) outputText("a little hesitation.", false);
...
//Volunteer for new fetishes!
		private function CeraphHandsOutNewFetishesLikePervCandy():void
		{
			...
			if (flags[FlagEnum.PC_FETISH] == 0) {
				...
				if (player.lowerBody.cockSpot.count() > 0) outputText("cock bounce in time with your fluttering heartbeats", false);
				else if (player.lowerBody.vaginaSpot.hasVagina()) outputText("vagina get wetter and wetter", false);
			...
			//*Fetish level = 1
				...
				if (player.nipplesPierced == 0) {
					...
					if (player.upperBody.chest.hasFuckableNipples()) outputText("slutty", false);
					else if (player.upperBody.chest.BreastRatingLargest[0].nippleLength < 1) outputText("cute", false);
			...
			//*Fetish level = 2
				...
				else if (player.lowerBody.vaginaSpot.hasVagina()) outputText("feminine moisture drools from between your lips and your " + clitDescript() + " turns into a hard button.", false);
				...
				if (player.totalCocks() > 0) {
					...
					if (player.lowerBody.cockSpot.list[0].pierced > 0) outputText(", the old piercing clattering to the ground as it slides out, ", false);
...
//*Ceraph Actually Removes The Fetish (Zeddited)
		private function goThroughWithCeraphUnfetishification():void
		{
			...
			if (player.stats.cor < 33) outputText("It seems there's still a spark of Ceraph's fire under all her submission.", false);
			else if (player.stats.cor < 66) outputText("You sigh and wonder if you should punish her for giving you such lip.", false);
...
//*Fuck Ceraph's Pussy (Zeddited)
		private function fuckFollowerCeraphsVagoo():void
		{
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText(" and half-erect, throbbing cock", false);
			outputText(".  ", false);
			if (player.stats.cor < 33) outputText("She gets down on all fours, then lays her cheek in the dirt.  Her arms cross behind her back, suddenly bound there by the abrupt appearance of those same panties, tied in a knot.  Supported by only her knees, her tits, shoulders, and her face, she asks, \"<i>Grab me and fuck me " + player.mf("Master", "Mistress") + ", please grind your slave's face in the dirt!</i>\"\n\n", false);
			else if (player.stats.cor < 66) outputText("She gets down on all fours, then lays her cheek in the dirt.  Her arms cross behind her back, and you pin them there for her, smiling as her panties appear as if by magic, tied in a binding knot around her wrists.  With only her knees and upper body supporting her, Ceraph begs, \"<i>Be rough with me.</i>\"\n\n", false);
			else {
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("and her dick releasing a squirt of pre-cum ", false);
			...
			if (player.stats.cor < 33) outputText("Well, that wasn't quite what you would have thought of doing, but you've got a good idea what she wants, and you may as well go along with it.  The idea of taking her in such a rough, hard way stirs something primal within you, and you have an easy time slipping into a meaner, rougher persona.", false);
			else if (player.stats.cor < 66) outputText("Well, with a request like that, you'll have no problem fucking her rough and hard.  The situation evokes something primal within you, and the inviting delta between Ceraph's thighs practically beckons you to ravage it.", false);
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText(", ignoring the rope of dangling pre-cum that hangs from her bumpy prick.", false);
			else outputText(", feeling the heat washing off her mons and onto your " + player.cockHead(x) + ".", false);
			outputText("  Yanking back, you bury all " + num2Text(Math.round(player.lowerBody.cockSpot.list[x].cockLength)) + " inches of your " + cockDescript(x) + " into Ceraph's unholy, warm snatch, ", false);
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("Her cock dangles towards the ground, dripping thick ropes of pre-spunk as readily as her pussy.  ", false);
			...
			if (y >= 0) {
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("She squeals and sprays a thick rope of jizz from her bloated cock.  It seems a little prostate pressure was all it took to put her maleness over the edge.  ", false);
				...
				if (player.lowerBody.cockSpot.count() >= 3) {
					...
					if (player.lowerBody.cockSpot.count() > 3) outputText("es have nothing to do but slide across her cheeks, dripping pre-cum all over her smooth skin.", false);
			...
			//(Single Penn!) 
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText(" while her cock spews ropes of pre-cum into the dirt", false);
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText(", and her cock starts pumping more demonic spunk into the mud", false);
...
//*Ceraph TongueFucks The PC (Zeddited)
		private function followerCeraphTongueFucking():void
		{
			...
			if (player.lowerBody.vaginaSpot.list[0].clitLength >= 3) outputText("fellating", false);
...
//*Ceraph goes full tentacle and double penetrates herms (+ standard dick BJ if ceraph dick toggle is on) (Zeddited)
		private function ceraphTentacleGrape():void
		{
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("  You can vaguely see Ceraph's hard, demonic-dick underneath all the waving tentacles.  She must have taken your command to keep her dick out for your use quite literally, even though there's little chance you'll get to put it anywhere.", false);
			outputText("\n\n", false);
			...
			if (player.lowerBody.cockSpot.count() == 1) outputText("all of your girth", false);
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("Ceraph's cock sprays cum all over her belly as her climax winds on, her poor, ignored prick blasting seed with reckless abandon.\n\n", false);
...
//Siamese Catgirl Twins - TDM (Zeddited, nya)
		public function catgirlEncounter():void
		{
			...
			if (!player.perks.has("Evade") && player.stats.spe / 5 + rand(20) < 22) outputText("flies into you, knocking you to the ground.  After a moment, you find yourself faced with a pair of overeager cat-morphs grinning down at you.", false);
...
//SEX!
		private function fuckCeraphsCatgirls():void
		{
			...
			if (player.lowerBody.cockSpot.count() == 1) outputText("sliding her hand between the furry tits, down the base of your " + cockDescript(x) + ".", false);
...
//[=Yes=]
		private function ceraphLackeyCorruption():void
		{
			...
			if (!player.lowerBody.cockSpot.hasCock()) {
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText("your " + clitDescript() + " swells", false);
			...
			if (player.lowerBody.balls > 0) outputText("scrotum and ", false);
			...
			if (!demon && !player.lowerBody.cockSpot.hasCock()) outputText(", then the temporary phallus shrinks and disappears back into your crotch", false);
			if (!demon) outputText(".\n\n", false);
...
//(should probably add a cock-limit of like, whatever you want, cuz you're fucking her butt)
		private function sweetieNOOOO():void
		{
			...
			//([if first time]
			if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00294] == 0) outputText("Seeing your confused stare, she simply answers with, \"<i>Ah, do be patient... sweetie,</i>\" and goes back to her work.", false);
			...
			if (flags[FlagEnum.MARBLE_DICK_LENGTH] > 0 && flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("Her nubbly member presumably shifts its color from royal purple to a lighter, brownish tone as well, inflating to the familiar seven-inch measurement; at least, so you'd judge by the sudden bulge in the overalls.  ", false);
...
//[of course honey-buns] 
		...
		private function postUdderChoice(newl:boolean = false):void
		{
			...
			//([if udder] 
			if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00296] == 1) outputText(", making sure to prep four extra tubes for her udder", false);
			...
			//([if udder] 
			if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00296] == 1) outputText("  The four others quickly follow, the udder-cups sucking onto the nubs like hungry children.", false);
			outputText("\n\n", false);
			...
			if (player.lowerBody.balls > 0) outputText("your " + ballsDescriptLight() + " tightening", false);
			...
			if (player.lowerBody.cockSpot.count() == 2) outputText("  Your other cock sprays ", false);
			else if (player.lowerBody.cockSpot.count() > 2) outputText("  Your other cocks spray ", false);
			...
			if (cum >= 2000 && player.stats.cor >= 66) outputText(" and chuckle at the drizzle of semen that issues from her butt and mouth", false);
...
//After that is a scene tailored more specifically for the PC. Intention is to give nagas and centaurs and all that their own scene. Currently only scene is for PCs with a dick - should be naga-compatible.
		private function cerminika():void
		{
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText(" while her cock shrinks to what looks approximately like six inches of meat, an outright tiny penis for Mareth", false);
			outputText(".\n\n", false);
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				...
				//[CERAPH COCK: 
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("  Her dick presses up into your crotch.", false);
				...
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 0) outputText("She casually and playfully gives the gentlest squeeze to your " + chestDesc() + ".  ", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText(", and her prick has remained firm and hard", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("The very base of her prick teases the veins of your own shaft, but she seems focused on the pleasure of her cunt for now.  ", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("  The degree that her cock abruptly shoots straight up is perhaps the most amusing part of the act.", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText(", and the heat of her prick bouncing into your lap", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("  A single bead of pre-cum forms slowly at the tip of her prick, growing larger until it finally slips down into the curve of her head, sliding down her shaft and into your lap.", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("Her prick bounces between her belly and yours as you take her.  ", false);
				...
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText(" and slap at your own chest", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("A light spurt of pre-cum from her prick warms the flesh where your hips meet.  ", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("  You can practically hear her rock-hard cock kicking up the ground.", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("  Her prick has become engorged and infernal, covered with abnormal bumps and veins. It practically spits up pre-cum, gurgling with a greatly increased supply.", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("  The demonic prick placed between her legs stops bouncing only because of how rock-hard it is as it spews her own filthy fucksludge across her body, splattering onto her tits and across her face.  The volume of off-white cream it spews out takes even longer to finish than you do, painting her obscenely.", false);
				outputText("\n\n", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("  Her own filthy jizz has already been licked clean by her hungry mouth, so as not to dirty your handsome prick.", false);
...
//▶[FuckVagina] requires penis
		private function fuckZetsukosTonguepussy():void
		{
			...
			if (player.lowerBody.balls > 0) outputText("slithering across your " + ballsDescriptLight() + ", ", false);
			else if (player.lowerBody.vaginaSpot.hasVagina()) outputText("gently licking the outer edges of your labia, ", false);
			outputText("moving along to the back, ", false);
			if (player.lowerBody.balls > 0 || player.lowerBody.vaginaSpot.hasVagina()) outputText("and ", false);
...
//▶[Banana?] silly mode
		...
		private function giveFollowerBodyBits():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock() || player.lowerBody.vaginaSpot.hasVagina() || player.hasBreasts()) {
				if (player.lowerBody.cockSpot.hasCock()) outputText("  I could take your cock.");
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText("  I could remove your snatch.  Don't worry, once it's off of you, anything I put in it won't wind up in your womb.");
		...
		private function ceraphFollowerCockTaking(smallest:boolean = false):void
		{
			...
			if (player.lowerBody.cockSpot.list[x].cockThickness >= 4) outputText(", compressing your massive girth into the rigid shaft", false);
		...
		private function ceraphFollowerTitTaking(rowNum: number = 0):void
		{
			...
			if (x < player.upperBody.chest.count() - 1 && player.upperBody.chest.count() > 1) {
				//If only 1 row below
				if (x >= player.upperBody.chest.count() - 2) outputText("your torso has shifted and your " + breastDescript(x + 1) + " have moved up to fill the void.", false);
				//Multiple below
			...
			if (player.upperBody.chest.count() == 0) outputText("<b>ERROR!  ERROR!  Please contact Fenoxo and reload your game if you don't want your save messed up.</b>", false);
			outputText("\n\n", false);

			outputText("You gasp when Ceraph ", false);
			if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00217] == 3) outputText("rolls the nipples in her hands", false);
			else if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00217] == 4) outputText("pushes her fingertips inside the leaky nipple-cunts", false);
...
//NippleCunt Stuffing (Ceraph grows dick-nipples to plow your lusty twats!)
		private function stuffSomeNippleCunts():void
		{
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText(" and proud demon-dick");
			...
			if (player.upperBody.chest.countNipples() > 2) outputText("  She moves from pussy to pussy with ease, exploring every single one of your myriad holes, a gleeful smile on her face.");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("  [EachCock] is so hard by this point that it's smearing its pre-cum all over Ceraph's belly, but she doesn't mind.");
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("  As for your [vagina], you've grown more than wet enough for a hard dicking, but honestly, right now, all you want is your [nipples] to be perfectly and completely fucked.");
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("  The fetishistic slut's sloppy cock is dribbling on your hip, neglected and aroused.  Poor thing.");
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText(", her cock spouting its own ivory wetness onto your belly in sympathetic release.");
...
//Portal Fuck (AKA Ceraph Hung Out With Cinnabar, Req's PC dick)
		private function portalFuckWithFollowerCeraph():void
		{
			...
			if (flags[FlagEnum.TIMES_CERAPH_PORTAL_FUCKED] == 0) {
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 1) outputText("  Of course, you don't seem to like that appendage, so my ring won't get much use today, will it?");
				outputText("</i>\"");
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 1) outputText("  Ceraph twirls one of the rings into her palm, where it promptly vanishes in a puff of purple smoke.");
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 1) outputText("  She sadly teleports one of them away, knowing you've commanded her to stay female for you.");
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText(" and eager cock");
			...
			if (flags[FlagEnum.TIMES_CERAPH_PORTAL_FUCKED] == 0) outputText("  Well, your dick doesn't look any different, aside from the gilden adornment stuck near the bottom.  Now, if it were hilt-deep in the demon's cunt, that would be something!");
		...
		private function layEggsInSlaveCeraph():void
		{
			...
			//[if (cocks > 0) 
			if (player.lowerBody.cockSpot.hasCock()) outputText("  Ceraph glances at the bulge in your [armor] quickly before she demurely lowers her eyes.  A wicked smile plays across her beautiful face as she responds, \"<i>Yes, [master].</i>\"");
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("  The whole garment creaks and strains under the pressure of her hardening member.");
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("; unable to keep it penned up, her demonic cock spills out the side, waving back and forth obscenely as you saw the offending garment into her crotch");
			outputText(".");
...
//Adds "Submission" option to the roleplay menu.
		private function sumissivenessToCeraphFollower():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("[oneCock]");
			else outputText("your rigid clit");
			outputText(" with ticklishly-soft touches");
			if (player.lowerBody.balls > 0) outputText(" before gently squeezing your [sack]");
			outputText(", savoring your state.  You groan out loud at the sudden sensation, and a bead of ");
			if (player.lowerBody.cockSpot.hasCock()) outputText("pre-cum has begun to form on your [cockHead biggest]");
...
//[Either Once]
		private function barkToCeraphOnce(dog:boolean = true):void
		{
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) outputText("ease the pressure on her tightly-contained phallus");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("milked ");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("your twitching maleness");
			else if (player.lowerBody.vaginaSpot.hasVagina()) outputText("to your achingly empty pussy");
			if (player.lowerBody.cockSpot.count() > 1) outputText("es");

			outputText(".  Almost immediately, you squirt out a huge dollop of ");
			if (player.lowerBody.cockSpot.hasCock()) outputText("pre-cum");
			else outputText("lady spunk");
			outputText(".  It dangles heavily enough to ");
			if (player.lowerBody.cockSpot.hasCock()) outputText("make your [cock biggest] bob below");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("rams straight into your inner, cock-milk reserve");
			...
			outputText("\n\n");
			if (player.lowerBody.cockSpot.hasCock()) outputText("Dick-milk");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("sliding");
			else outputText("pouring");
			outputText(" smoothly out of it as your ");
			if (player.lowerBody.cockSpot.hasCock()) outputText("inner reservoir is squeezed");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("prostate");
			else outputText("walls");
			outputText(".  ");
			if (player.lowerBody.cockSpot.hasCock()) outputText("Ironically, [eachCock] has begun to lose some of its stiffness, and you grow limper and limper as more of your cum leaks from you");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("the 'pressure'");
			else outputText("a need within you");
			outputText(" feels released, you're still horny");
			if (player.lowerBody.cockSpot.hasCock()) {
				if (player.lowerBody.cockSpot.count() == 1) outputText(", even with a soft cock");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("semen");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("prostate is getting pushed up against each time");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("prostate is");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("straight onto your sore prostate");
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				if (player.lowerBody.cockSpot.count() > 1) outputText("each of your cocks");
...
//[Either Twice]
		private function barkOrMeowTwiceToCeraph(dog:boolean = true):void
		{
			...
			if (select == 0) {
				...
				outputText(".  ");
				if (player.lowerBody.cockSpot.hasCock()) outputText("It bounces against your tummy");
			...
			//Corrupt Jojo or Amily
				...
				if (amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2 && campCorruptJojo()) outputText("s");
				...
				if (campCorruptJojo()) {
					outputText("Jojo");
					if (amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2) outputText(" and ");
				}
				if (amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2) outputText("Amily");
				if (amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2 && campCorruptJojo()) outputText("look around uncertainly before darting back to their usual places, knowing you'll give the command if you want them to take care of your desires.");
			...
			//Generic
				...
				if (player.lowerBody.cockSpot.hasCock()) outputText("goblins");
				else outputText("imps");
				outputText(" seem intrigued by the sight of your compulsively ");
				if (player.lowerBody.cockSpot.hasCock()) outputText("erect maleness");
				...
				if (player.lowerBody.cockSpot.hasCock()) outputText("her");
				else outputText("his");
				outputText(" hand beneath your freely drooling ");
				if (player.lowerBody.cockSpot.hasCock()) outputText("erection");
				else outputText("pussy");
				if (player.lowerBody.cockSpot.count() > 1) outputText("s");
				outputText(" to collect a palmful of slippery ");
				if (player.lowerBody.cockSpot.hasCock()) outputText("boy");
				else outputText("girl");
				outputText("-honey.  ");
				if (player.lowerBody.cockSpot.hasCock()) outputText("She");
				else outputText("He");
				...
				if (player.lowerBody.cockSpot.hasCock()) outputText("cum this nice, I'd be milking " + player.mf("him", "her") + " dry every time " + player.mf("he", "she") + " could get it up");
				...
				if (player.lowerBody.cockSpot.hasCock()) outputText("pre-cum rockets");
				...
				if (player.lowerBody.cockSpot.hasCock()) outputText("goblin");
				else outputText("imp");
				...
				if (player.lowerBody.cockSpot.hasCock()) outputText("pre-ejaculate");
				...
				if (player.lowerBody.cockSpot.hasCock()) outputText("boner");
				else outputText("twat");
				if (player.lowerBody.cockSpot.count() > 1) outputText("s");
				if (player.lowerBody.cockSpot.count() > 1) outputText(" feel like they");
				else outputText(" feels like it");
				outputText(" could go off at any second.  The journey is murder for your poor, ");
				if (player.lowerBody.cockSpot.hasCock()) outputText("over-inflated maleness");
				...
				if (player.lowerBody.cockSpot.hasCock()) outputText("popping");
