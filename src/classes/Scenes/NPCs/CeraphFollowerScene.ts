...
//[Actually Ceraph] - 
		public function ceraphFollowerAppearance(output:boolean = true):void
		{
			...
			if (flags[FlagEnum.FOLLOWER_AT_FARM_CERAPH] == 0 && flags[FlagEnum.FARM_CORRUPTION_STARTED] == 1) addButton(1, "Farm Work", helpWithFarm);
		}
		...
		private function helpWithFarm():void
		{
			...
			Render.text("You ask Ceraph if she can help you with the farm you recently acquired.");
		...
		private function doFarmShit():void
		{
			...
			Render.text("“<i>I’ll go over there tonight then,</i>” she says. There’s a far-away gleam of excitement in her eyes as she tugs on a nipple; only now does it occur to you what ‘marked as being under demon protection’ might entail. “<i>Anything else, [master]?</i>”");
		...
		private function noFarmShitYet():void
		{
			...
			Render.text("“<i>Very well,</i>” she smirks. “<i>Anything else?</i>”");
		...
		private function ceraphSexMenu():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock() && player.lust >= 33) {
				Render.text("You could fuck her pussy.  ", false);
				maleFuck = fuckFollowerCeraphsVagoo;
			}
			if (player.lowerBody.vaginaSpot.hasVagina() && player.lust >= 33) {
				Render.text("You could make her lick your pussy.  ", false);
			...
			if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina() && player.lust >= 33) {
				Render.text("You could command her to please all of your organs.  ", false);
				hermFuck = ceraphTentacleGrape;
			}
			if (player.upperBody.chest.hasFuckableNipples()) {
				Render.text("You could have your slave please your nipplecunts.  ");
				nipFuck = stuffSomeNippleCunts;
			}
			if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(100) >= 0) {
				Render.text("You could use your penis but see if Ceraph has some magic to mix it up.  ");
				portalFuck = portalFuckWithFollowerCeraph;
			}
			if (maleFuck + femaleFuck + hermFuck + nipFuck + portalFuck == 0) Render.text("There's no sexual acts you can perform with Ceraph at present.");
		...
		{
			Render.text("", true);
			Render.text("You tell Ceraph you'd like to do a little roleplaying.  Her nipples turn hard under their latex bindings as she asks, \"<i>What will it be, " + player.mf("Master", "Mistress") + "?  Shall I pretend you've just teased me into sexual submission, or would you like to switch things up and have your bottom play at being top again?  Or maybe... you'd like me to shapeshift into some other girl, and do all the dirty, depraved things she never would?</i>\"", false);
...
//*Ceraph is Defeated #4 - Offers Funtimes (Zeddited)
		public function submissiveCeraphOffer():void
		{
			spriteSelect(7);
			Render.text("", true);
			Render.text("Once again, Ceraph ", false);
			if (monster.HP < 1) Render.text("drops on the ground in front of you, completely beaten.", false);
			else Render.text("drops down and starts masturbating, practically fisting her drooling pussy while she pumps her demonic dick with reckless abandon.", false);
			Render.text("  She ", false);
			if (monster.HP < 1) Render.text("looks up at you and asks", false);
			else Render.text("manages to stop fapping long enough to look up at you and ask", false);
			Render.text(", \"<i>Why do I even bother?</i>\"\n\n", false);

			Render.text("You're a bit surprised by her tone - depression and defeat aren't exactly her style.  The only thing you manage to respond with is ", false);
			if (player.stats.cor < 33) Render.text("a nervous chuckle", false);
			else if (player.stats.cor < 66) Render.text("a surprised laugh", false);
			else Render.text("a bemused smirk", false);
			Render.text(".  Ceraph presses on, \"<i>This whole time I've been trying to bring you into my harem, but I've ignored the obvious.  Each time we've tangled, you come out on top... in more ways than one.</i>\"  The demon looks up at you with meek, hooded eyes and says, \"<i>Perhaps I've had it backwards all along... I belong in your harem.</i>\"\n\n", false);

			Render.text("Letting your eyes play over the demon's exotic, sculpted skin, you can't help but be tempted by her offer... Ceraph sees you mulling it over and produces a collar as she purrs, \"<i>No pressure... " + player.mf("Master", "Mistress") + ".  You could always just take me here like usual, and perhaps, the next time you'll stop being so lucky...</i>\"\n\n", false);

			//[Display Rape Options + Collar Option]
			if (player.gender > 0) {
				Render.text("Do you fuck her? (And if so, which of your body parts do you do it with?)", false);
				...
				if (player.lowerBody.cockSpot.hasCock()) {
					...
					else Render.text("  <b>There's no way you could fit inside her ass - you're too big.</b>", false);
				}
...
//Collar Ceraph After 4th Defeat + Rape: (Zeddited)
		private function collarCeraph():void
		{
			Render.text("", true);
			spriteSelect(7);
			Render.text("You reach down and snatch the collar from Ceraph's shaking hand.  Turning it over in your grip, you get a feel for the soft, supple leather.  Blood-red studs poke out around the black strap's surface, vaguely reminding you of a dog's collar, though with the aggression cranked up to max.  The snap mechanism looks simple enough to connect, but you can't see any way to release the latch.  It makes sense that Ceraph would have one-way collars; slaves shouldn't be able to remove the symbol of their station.\n\n", false);

			Render.text("Leaning over, you slide the collar around your new slave's suddenly flush neck, feeling her heart hammering away just beneath the skin.  Snapping it closed, you muse ", false);
			if (player.stats.cor < 33) Render.text("that you never expected making a demon into your slave would factor into your quest.  On one hand it seems wrong, but... she's a demon.  The fewer you have opposing you, the easier it will be to end their threat completely.", false);
			else if (player.stats.cor < 66) Render.text("that taking a demon as a slave would've been abhorrent to you when you started this journey.  Now, it's just a means to a very pleasurable end.", false);
			else {
				Render.text("on how much you'll enjoy using the former dom as your personal ", false);
				if (player.lowerBody.cockSpot.hasCock()) Render.text("cum-dump", false);
				else Render.text("tongue-slave", false);
				Render.text(".", false);
			}
			Render.text("  Ceraph pulls herself up to her knees and kisses your " + player.feet() + ", a show of absolute submission and obedience.\n\n", false);

			Render.text("The defeated demon explains, \"<i>Though I am now and forever your slut, your slave, your bitch... those in my harem cannot be abandoned.  I am sad to say I cannot live with you, " + player.mf("Master", "Mistress") + ".</i>\"  She sees the look forming in your eyes and hastily adds, \"<i>Oh, I'll still be at your beck and call, but if I can't make it, I'll be sure to send you one of my pets.  Just rub this charm whenever you want my services, " + player.mf("Master", "Mistress") + ", and I'll be there.</i>\"  Ceraph holds out a tiny onyx bar tipped with rubies.  The gems shine and glitter with their own inner light, while the black shaft seems to drink in everything around it, leaving behind darkness.\n\n", false);

			Render.text("Well, with a harem as large as hers, it makes sense that she'd have to keep them in her lair and tend to them.  There's no way you could foster the people in your camp, and besides, since their Mistress is your slave, they're <b>now yours by extension, as well</b>.  Ceraph reaches down to ", false);
			if (monster.lust > 99) Render.text("resume stroking", false);
			else Render.text("stroke", false);
			Render.text(" her nodule-studded demon-dick with her free hand.  She whimpers, \"<i>Would my " + player.mf("Master", "Mistress") + " prefer to carry " + player.mf("his", "her") + " slave's token, or wear it as a belly-button piercing?</i>\"\n\n", false);
...
//[Carry]
		private function carryCarephsToken():void
		{
			Render.text("", true);
			spriteSelect(7);
			Render.text("You inform your living property that you've heard quite enough about her piercings and snatch the token from her hand.  Ceraph's eyes go wide and she nods, more than a little fearfully.  Seeing the Omnibus so cowed brings a smile to your face.\n\n", false);

			Render.text("Ceraph asks, \"<i>So, before my " + player.mf("Master", "Mistress") + " leaves, would you like to fuck your new slut one of the old ways, one last time?</i>\"\n\n", false);

			Render.text("<b>(Received Key Item: Onyx Token)</b>\n\n", false);
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00287] = 1;
			player.createKeyItem("Onyx Token - Ceraph's", 0, 0, 0, 0);
			//[Display Rape Options + Collar Option]
			if (player.gender > 0) {
				Render.text("Do you fuck her as a disobedient demon, one last time? (And if so, which of your body parts do you do it with?)", false);
				...
				Render.text("  You don't really have the equipment to.  Oh well.", false);
				cleanupAfterCombat();
			}
		}
...
//[Pierce] 
		private function getCeraphFollowerPiercing():void
		{
			...
			Render.text("You bare your midriff to your new slut with ", false);
			if (player.stats.cor < 40) Render.text("a little hesitation.", false);
			else Render.text("a smirk, secure in your knowledge of her defeat.", false);
			Render.text("  Ceraph shimmies forward and holds the oily stud against your midriff, pressing it into you with her palm.  There's a moment of tingling warmth, followed by an ache of unholy numbness.  When the demoness removes her hand, your belly-button is studded with the magical jewelry, glowing and not at the same time.\n\n", false);

			Render.text("There's no lingering compulsion, no mental assault, just a new piercing.  You're kind of awestruck by the gesture - even though she seemed sincere, a small part of you still believed the fetish-obsessed demon was using this whole thing as a setup to trick you.\n\n", false);

			Render.text("Ceraph asks, \"<i>" + player.mf("Master", "Mistress") + ", before you go, would you like to fuck your slut one of the old ways, one last time?</i>\"", false);
			//[Display Rape Options + Collar Option]
			if (player.gender > 0) {
...
//Volunteer for new fetishes!
		private function CeraphHandsOutNewFetishesLikePervCandy():void
		{
			...
			if (flags[FlagEnum.PC_FETISH] == 0) {
				Render.text("Ceraph comes forward on your command, whispering calmly as she ", false);
				if (player.earsPierced == 0) Render.text("pulls a pair of gleaming, green piercings from a pouch.  \"<i>Oh, don't worry " + player.mf("Master", "Mistress") + "; you're going to love this so much.  These piercings are special, and they'll give you exactly what you want.</i>\"", false);
				else Render.text("places her hands upon your pierced ears.  She whispers softly, \"<i>Don't worry " + player.mf("Master", "Mistress") + ", I can easily use the piercings you already have.  It won't hurt.</i>\"", false);
				Render.text("\n\n", false);
				//(NOT PIERCED) 
				if (player.earsPierced == 0) Render.text("The demon places a hand on your forehead and rubs your temples.  Numbness spreads through your body with every touch, until you can barely feel a thing.  She snaps an earring into your left ear, and dizziness washes over you.  A moment later she's piercing the other side, and the now-familiar vertigo that accompanies it seems to come and go quicker than before.  ", false);
				//(PIERCED) 
				else Render.text("The demon rubs your ears in her hands, numbing them slightly.  A gradual buzz builds behind your eyes, accompanied by a wave of dizziness.  You blink and try to shake your head, but as numb as you are, it's quite difficult.  After a few moments, the odd sensations pass, returning normal feeling to your ears and " + player.face() + ", much to your relief.  ", false);
				Render.text("You hope she doesn't take your " + player.armorName + " while you're paralyzed, leaving you to roam the realm totally exposed.  Confusion and waves of new desire battle in your mind as you try to come to grips with the odd thought.\n\n", false);

				Render.text("Ceraph watches your ", false);
				if (player.lowerBody.cockSpot.count() > 0) Render.text("cock bounce in time with your fluttering heartbeats", false);
				else if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("vagina get wetter and wetter", false);
				else Render.text("parted lips and confused expression", false);
				Render.text(" as the new thoughts and desires settle themselves inside you.  She gives you a gentle pat and explains, \"<i>It's ok " + player.mf("Master", "Mistress") + "; you're an exhibitionist now.  Would you like your piercing slave to give you even more?</i>\"\n\n", false);

				Render.text("Ceraph is right – <b>you're an exhibitionist now.</b>", false);
			...
			//*Fetish level = 1
				...
				Render.text("Ceraph giggles as she closes in on you once again.  Sighing, you lie there and allow your slave to massage your temples, using her magic to paralyze and numb your body.  ", false);
				if (player.nipplesPierced == 0) {
					Render.text("She's all too happy to build up the suspense as she pulls out a pair of shining black studs, \"<i>Oh, do you know what these are going to do?  Well, how about I slide them into your ", false);
					if (player.upperBody.chest.hasFuckableNipples()) Render.text("slutty", false);
					else if (player.upperBody.chest.BreastRatingLargest[0].nippleLength < 1) Render.text("cute", false);
					else Render.text("tight", false);
					Render.text(" nipples, and you tell me all about your fetishes and which one makes you the hottest.  Oh, you'll love it " + player.mf("Master", "Mistress") + "!</i>\"\n\n", false);
				}
				//If already pierced
				if (player.nipplesPierced > 0) Render.text("She's all too happy to build up the suspense as she lays  her hands on your pierced nipples, giving them a gentle tweak that you can barely feel.  \"<i>Don't worry, " + player.mf("Master", "Mistress") + ".  Imbuing your new fetish into you through these will be easy.  Just tell me all about which fetishes make you hottest while I do it, and see if you can guess your new kink.</i>\"\n\n", false);
				//Business as usual!
				Render.text("The demon doesn't give you a chance to reply; instead, she focuses on ", false);
				if (player.nipplesPierced > 0) Render.text("your " + nippleDescript(0) + "s, circling her fingers all around the fleshy nubs.  Goosebumps run over your body in a wave, accompanied by a similar chill and a pressure behind your temples.  You shudder, but it quickly fades.", false);
				else Render.text("aligning the business ends of the piercings with your sensitive nipple-flesh.  Your right " + nippleDescript(0) + " is pierced in one smooth motion, nearly making you scream in pain.  As she fastens it on, you feel goosebumps spread over your body in a wave.  The second piercing doesn't seem to hurt as bad, but the sensation of spreading goosebumps is far more noticeable.", false);
				Render.text("  Your eyes dart around, curious what fetish your demonic slave has given you this time.\n\n", false);

				Render.text("Ceraph smiles down at you and whimpers, \"<i>I hope you're pleased with the new fetish " + player.mf("Master", "Mistress") + ".  Just think about how similar being paralyzed is to being tied down and tell me if you like it.</i>\"\n\n", false);

				Render.text("Your body goes beet-red as it suddenly tries to struggle against the invisible binding of her magic.  It... it feels good!  You nearly cry out with lust as the restraint turns you on more and more.  Ceraph's magic has given you a fetish for being tied up!  You nearly faint when you think of all the strange things in this land that might try to restrain you, and you know you have no hope of resisting if they ever catch you.  <b>Though somehow you think you might enjoy being a bondage fetishist...</b>", false);
				dynStats("lus", 25, "cor", 1);
				if (player.nipplesPierced == 0) {
			...
			//*Fetish level = 2
				...
				Render.text("The demoness pulls out a diamond-studded piercing and closes in on you, her cock peeking out of her panties, her pussy moist, and her hips swaying seductively as she advances.  Ceraph gives you a serious look and warns you, \"<i>You realize you're not even going to be able to lift a hand against your foes after this?  You really love a challenge, don't you " + player.mf("Master", "Mistress") + "?</i>\"\n\n", false);

				Render.text("The idea of facing the denizens of this land without even so much as the ability to throw a punch turns you on immensely, and you pant and gasp as ", false);
				if (player.totalCocks() > 0) {
					Render.text("pre-cum oozes from ", false);
					if (player.totalCocks() > 1) Render.text("each of ", false);
					Render.text("your " + multiCockDescriptLight() + ".", false);
				}
				else if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("feminine moisture drools from between your lips and your " + clitDescript() + " turns into a hard button.", false);
				else Render.text("your body aches for release.", false);
				Render.text("  With an amused grin, Ceraph yanks down your gear and ", false);
				//[dicks]
				if (player.totalCocks() > 0) {
					Render.text("grabs your " + cockDescript(0), false);
					if (player.lowerBody.cockSpot.list[0].pierced > 0) Render.text(", the old piercing clattering to the ground as it slides out, ", false);
					Render.text(" and snaps the diamond stud through your sensitive flesh, making your vision haze red in pain.\n\n", false);
				...
				//[cunts]
					...
					Render.text("spreads your lips", false);
					if (player.vaginas[0].clitPierced > 0) Render.text(", the old piercing clattering to the ground as it slides out of your flesh, ", false);
					Render.text(", getting ahold of the flesh around the base of your " + clitDescript() + ".  With practiced ease, she snaps the piercing closed, attaching the diamond stud to you while the pain fuzzes your vision red.\n\n", false);
				...
				//[else]
					...
					Render.text("snaps the diamond stud into your eye-brow, piercing it", false);
					if (player.eyebrowPierced > 0) Render.text(" and discarding your old jewelry like a piece of garbage", false);
					Render.text(".  It hurts more than it ought to, fuzzing your vision red.\n\n", false);
				...
				Render.text("As she finishes, you get up and attempt to test your slut's work.  Raising your hand for a slap, you try to bring it down on Ceraph's face.  She flinches, but the blow stops a few inches away from her, melting into a gentle caress.  <b>You can no longer use basic physical attacks!</b>  She drops to her knees and asks, \"<i>Have I displeased you?  I can remove the compulsion in it if you like.</i>\"\n\n", false);

				Render.text("You rub a hand through the demon's pitch-black hair and let her know that it's exactly what you asked for.\n\n", false);

				Render.text("Ceraph gives an excited squeak and holds herself still, allowing you to pet her.  Once you stop, she gives a disappointed sigh, but holds her position.", false);
...
//*Request Ceraph Remove a Fetish. (Zeddited)
		private function unfetishifyYourselfWithFollowerCeraph():void
		{
			Render.text("", true);
			spriteSelect(87);
			Render.text("You ask Ceraph to remove one of the fetishes she generously donated earlier.  She sighs and nods, saying, \"<i>" + player.mf("Master", "Mistress") + ", are you sure? It isn't that easy to do, and I love knowing my owner is aroused by my piercings!</i>\"\n\n", false);
			Render.text("Growling in irritation, you tell her, \"<i>Yes, I would like a fetish removed.</i>\"\n\n", false);
			Render.text("The demoness slumps her shoulders and nods.  She explains, \"<i>I have to do them in the reverse order that I added them... just hold still, okay?</i>\"\n\n", false);

			Render.text("Do you go through with it?", false);
...
//*Ceraph Actually Removes The Fetish (Zeddited)
		private function goThroughWithCeraphUnfetishification():void
		{
			Render.text("", true);
			spriteSelect(87);
			Render.text("Ceraph steps closer, her shining outfit squeaking from the movement.  Her hands gently touch your forehead, though she tries to avoid meeting your gaze.  The submissive slut keeps her eyes downcast, as is proper for a slave, and she begins to rub at your temples, working her magic to undo her mischief.  Warmth surges out, rushing through your temples and leaving a slack looseness in its wake.  Ceraph grunts and lets go, staggering back and panting.  She mumbles, \"<i>So much harder... to take those without changing... something else.</i>\"\n\n", false);
			Render.text("After a few moments, she seems to recover, and she asks, \"<i>Was there something else you needed me here for, " + player.mf("Master", "Mistress") + ", or did you just want to waste my time?</i>\"\n\n", false);

			if (player.stats.cor < 33) Render.text("It seems there's still a spark of Ceraph's fire under all her submission.", false);
			else if (player.stats.cor < 66) Render.text("You sigh and wonder if you should punish her for giving you such lip.", false);
			else Render.text("You slap her across the face for her temerity.", false);
...
//*Fuck Ceraph's Pussy (Zeddited)
		private function fuckFollowerCeraphsVagoo():void
		{
			...
			Render.text("You let Ceraph know that you'll be using her pussy.  She sighs and says, \"<i>Yes, " + player.mf("Master", "Mistress") + ",</i>\" unable to hide the disappointment in her tone.  Ceraph shifts, the panties of her outfit fading away to reveal her dripping cunny", false);
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text(" and half-erect, throbbing cock", false);
			Render.text(".  ", false);
			if (player.stats.cor < 33) Render.text("She gets down on all fours, then lays her cheek in the dirt.  Her arms cross behind her back, suddenly bound there by the abrupt appearance of those same panties, tied in a knot.  Supported by only her knees, her tits, shoulders, and her face, she asks, \"<i>Grab me and fuck me " + player.mf("Master", "Mistress") + ", please grind your slave's face in the dirt!</i>\"\n\n", false);
			else if (player.stats.cor < 66) Render.text("She gets down on all fours, then lays her cheek in the dirt.  Her arms cross behind her back, and you pin them there for her, smiling as her panties appear as if by magic, tied in a binding knot around her wrists.  With only her knees and upper body supporting her, Ceraph begs, \"<i>Be rough with me.</i>\"\n\n", false);
			else {
				Render.text("You push her onto all fours, then grab her arms and fold them behind her back, grinding her face into the dirt.  The demoness groans, her pussy dripping ", false);
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text("and her dick releasing a squirt of pre-cum ", false);
				Render.text("as she gets more turned on by the rough, sexual play.  Suddenly, as if by magic, her panties appear around her wrists, and you tie them tightly to bind her securely.  With only her knees and upper body supporting her, Ceraph begs, \"<i>Be rough with me.</i>\"\n\n", false);
			}

			if (player.stats.cor < 33) Render.text("Well, that wasn't quite what you would have thought of doing, but you've got a good idea what she wants, and you may as well go along with it.  The idea of taking her in such a rough, hard way stirs something primal within you, and you have an easy time slipping into a meaner, rougher persona.", false);
			else if (player.stats.cor < 66) Render.text("Well, with a request like that, you'll have no problem fucking her rough and hard.  The situation evokes something primal within you, and the inviting delta between Ceraph's thighs practically beckons you to ravage it.", false);
			else Render.text("With an invitation like that, there's no way you'll decline.  Looking over her, a predatory thrill runs through you, urging you to utterly violate her.", false);
			Render.text("  You grab Ceraph's thighs", false);
			if (player.str < 60) Render.text(" and heave, lifting her up off the ground and forcing her to straddle you.", false);
			else Render.text(" and easily lift her, forcing her to straddle you.", false);
			Render.text("  Dragging the demon back, you bring her slutty, sodden puss up to your " + cockDescript(x), false);
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text(", ignoring the rope of dangling pre-cum that hangs from her bumpy prick.", false);
			else Render.text(", feeling the heat washing off her mons and onto your " + player.cockHead(x) + ".", false);
			Render.text("  Yanking back, you bury all " + num2Text(Math.round(player.lowerBody.cockSpot.list[x].cockLength)) + " inches of your " + cockDescript(x) + " into Ceraph's unholy, warm snatch, ", false);
			if (player.cockArea(x) > 150) Render.text("distorting her body around the sheer bulk of your massive member.\n\n", false);
			else Render.text("immersing yourself in the decadent wetness.\n\n", false);
			...
			Render.text("Your slut moans, \"<i>Ohhhh, yesssssss,</i>\" while you slide home, culminating in an inarticulate gurgle.  Heavy drops of drool hang from her lips, turning the earth below into a thin layer of viscous mud for your pet to pillow her head in while you rail her.  Starting slow, you gently work your whore's pussy over, gleefully watching her girlish lube drip from the puffy, purple lips of her sex.  ", false);
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text("Her cock dangles towards the ground, dripping thick ropes of pre-spunk as readily as her pussy.  ", false);
			Render.text("Somehow, she feels as tight around your " + cockDescript(x) + " as any virgin and three times as wet.  The gentle claps of genital against genital send ripples through Ceraph, smearing her cheek in the growing mud-puddle while her tits wobble dangerously inside the sheer latex bra.\n\n", false);

			Render.text("\"<i>M-more!  Harder, " + player.mf("Master", "Mistress") + "!</i>\" the demon pants as she crosses her legs behind your back, as if it could somehow stop you from pulling the whole way out.  You hold her one handed, just long enough to crack your palm against her tight, toned ass, and then you pick up the pace.  Now that you're fucking her faster, the slut isn't even trying to talk anymore.  Her hair is plastered against her scalp, stained brown by the mud, and her arms are slack in the restraints.  Ceraph gives every impression of being utterly resigned to being fucked like a toy, used without care for her own feelings or emotions.  Knowing that you've taken a powerful dominatrix and turned her into... this - it sends a chill up your back, invigorating your fast-pumping hips.\n\n", false);

			//(DOUBLE PENN!) 
			if (y >= 0) {
				Render.text("A wicked idea crosses your mind, and you reach down to grab your " + cockDescript(y) + ", aiming it at the flexing asshole just above the slave's squelching snatch.  Your next thrust plows it deep into Ceraph's anus, thankfully lubricated by the demon's constant squirting.  Growling at the mounting, doubling pleasure, you resume your tempo and slam both your cocks hilt-deep in Ceraph.  ", false);
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text("She squeals and sprays a thick rope of jizz from her bloated cock.  It seems a little prostate pressure was all it took to put her maleness over the edge.  ", false);
				Render.text("Your slave's reaction is to wiggle her backside at you and curl her tail about your waist, trying to pull you even further into her body.  Delirious and high on pleasure, you " + player.mf("chuckle", "giggle") + " and pound away, heedless of anything but your own pending climax.", false);
				if (player.lowerBody.cockSpot.count() >= 3) {
					Render.text("  Sadly, your extra penis", false);
					if (player.lowerBody.cockSpot.count() > 3) Render.text("es have nothing to do but slide across her cheeks, dripping pre-cum all over her smooth skin.", false);
					else Render.text(" has nothing to do but slide across her cheeks, dripping pre-cum all over her smooth skin.", false);
				}
				Render.text("\n\n", false);
			}
			//(Single Penn!) 
			else {
				Render.text("Working the bitch harder and harder, you start slamming your " + cockDescript(x) + " so violently into the captive puss that Ceraph's sliding a few inches through the mud with each push.  Her cunt starts squeezing and contracting, tightening even more than you would have thought imaginable.  With such a vice-like twat, you're having a hard time even pushing back inside.  Sighing, you hilt yourself and let the wringing, milking tightness work you.  Not wanting to let your slave assume control of the situation, you start spanking her, bringing your palm down hard enough on her ass to leave hand-prints behind from the blows.  Her tail curls about your waist protectively", false);
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text(" while her cock spews ropes of pre-cum into the dirt", false);
				Render.text(".  Holding still for the sucking, squeezing embrace, there's little on your mind but punishing your slut and enjoying the feel of your coming climax.\n\n", false);
			}

			Render.text("With a shuddering explosion of warmth, you cum.  Ceraph groans loud and low, her pussy happily caressing your " + cockDescript(x) + " as it spews its potent load into her demonic womb.  ", false);
			if (y >= 0) Render.text("Her asshole likewise gleefully devours the seed from your " + cockDescript(y) + ", flexing wildly from the fluid injection.  ", false);
			Render.text("You hold yourself there, deep inside the wanton hole", false);
			if (y >= 0) Render.text("s", false);
			Render.text(" and basting the corrupt slave's tunnel", false);
			if (y >= 0) Render.text("s", false);
			Render.text(" with sloppy spooge.  Ceraph screams, \"<i>Yes!  Yes!  Fuck me!  Fill me!  Use me " + player.mf("Master", "Mistress") + "!  Pump me full of cum while you grind my whorish face in the dirt!</i>\"  Her voice goes ragged, high pitched and screaming", false);
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text(", and her cock starts pumping more demonic spunk into the mud", false);
			Render.text(".  It trails off, though her pussy continues teasing your " + cockDescript(x) + ", wringing the last of your seed from your " + ballsDescriptLight() + ".\n\n", false);

			Render.text("Ceraph sighs, ", false);
			if (player.cumQ() >= 700) Render.text("rubbing her ", false);
			if (player.cumQ() >= 1400) Render.text("bloated, ", false);
			if (player.cumQ() >= 700) Render.text("bulging belly as she purrs, ", false);
			else Render.text("purring, ", false);
			Render.text("\"<i>Thank you... you're such a good dom for a horny bitch like me.</i>\"\n\n", false);

			Render.text("You smirk and drop her into her own muddy cum-puddle, then help her up to her feet.  She's filthy, debased, and dripping ropes of your goo.  Ceraph gives you a kiss on your ", false);
			if (player.tallness >= 80) Render.text("chest", false);
			else if (player.tallness >= 60) Render.text("lips", false);
			else Render.text("forehead", false);
			Render.text(" and mouths, \"<i>Thank you.</i>\"\n\n", false);

			Render.text("Nodding, you give her ass a slap and send her off, noting Ceraph has freed her hands at some point and returned them to their normal position.  She hasn't done anything about the sexual filth coating her body, but knowing her, she probably doesn't want to.", false);
			player.orgasm();
...
//*Ceraph TongueFucks The PC (Zeddited)
		private function followerCeraphTongueFucking():void
		{
			...
			Render.text("Desirous of being pleasured by your demonic slave, you spread out your " + player.legs() + " to allow easy access to your " + vaginaDescript(), false);
			if (player.lowerBody.cockSpot.hasCock()) Render.text(" and " + multiCockDescriptLight(), false);
			Render.text(".\n\n", false);

			Render.text("\"<i>Serve me with your tongue,</i>\" you command.\n\n", false);

			Render.text("Ceraph nods, the barest hint of a twinkle in her eyes as she drops down to her knees in order to examine your vulva.  She meekly kisses your slit, planting her wet lips against your " + vaginaDescript() + " for but a moment.  The demoness looks up you and licks you starting from your taint, through your labia, around your ", false);
			if (player.lust < 50) Render.text("still-hooded", false);
			else Render.text("engorged", false);
			Render.text(" clit, and stops over your sensitive pubic skin.  Then, her tongue begins to extend from her mouth, hanging further and further down as if it were unspooling from a hidden reel in her throat.  Ceraph doesn't stop letting out more of the slippery organ until she's got three feet of tongue dangling between her tits.  Just looking at it there, undulating below your mons, makes you wet.\n\n", false);

			Render.text("With a sharp intake of breath, she reels in the prodigious proboscis, shortening it down to a more usable ten or eleven inches.  Starting between her parted lips, the tongue thickens, becoming cylindrical in shape and widening until you're sure it's at least two inches across.  Veins pulsate and texture the demon's tongue as it fills, giving it a decidedly... phallic appearance.  The transformation finishes, capping Ceraph's cock-like tongue with a rounded, penile cap that completes the illusion.\n\n", false);

			Render.text("The tainted slut winks and licks at you with the crown, coating it in moist, feminine wetness, letting you feel the warmth of her tongue-cock upon your flesh.  It feels wonderful, like being licked by a thick, flexible prick that's been slathered in saliva.  Slowly, agonizingly slowly, it works its way into your passage, pushing past your engorged outer lips to snuggle into your ", false);
			if (player.wetness() < 3) Render.text("moist", false);
			else if (player.wetness() < 4) Render.text("wet", false);
			else Render.text("soaking", false);
			Render.text(" canal.  She twists her flexible mouth-dick inside you, making sure to rub over your g-spot, and then she slides another few inches in, forcing what feels like half of it inside you.\n\n", false);

			Render.text("Now breathing heavily, your " + hipDescript() + " begin trembling, aching to mount the invading member, fuck it, mate with it; anything to sate your growing desires.  Ceraph gives you a knowing wink and sidles forward, sliding the last several inches through your spread nether-lips into the velvety embrace of your " + vaginaDescript() + ".  You can feel it, squirming and rubbing inside you, twisting through your pussy with a slow, maddening purpose.  From time to time it brushes your cervix, but never hard, never painfully.  At the same time, it seems to always be in contact with your most sensitive places.  It makes you wonder if Ceraph has practiced this on herself at some point, and you briefly entertain the notion of the demon bent over, vigorously fucking her box with her perverted tongue-prick ravaging her purple pussy.\n\n", false);

			Render.text("A jolt of pleasure blasts the image from your mind and nearly takes your " + player.legs() + " out from under you.  With a start, you realize Ceraph has opened her lips wide enough to slurp your " + clitDescript() + " into her mouth, and somehow, she's produced a second tongue to service it.  With the stimulation of a tentacular tongue constantly hitting your g-spot and a second oral organ ", false);
			if (player.lowerBody.vaginaSpot.list[0].clitLength >= 3) Render.text("fellating", false);
			else Render.text("licking", false);
			Render.text(" at your " + clitDescript() + ", you start to shudder, trying to stave off what you know is coming.  You don't want Ceraph to get too uppity, thinking she can get you off this fast, but you're dangerously close, and her pumping, teasing mouth-cock is relentless.\n\n", false);

			Render.text("You grab Ceraph's head and mash it against your sodden, constricting box, as you command, \"<i>Drink my cum, bitch.  Swallow all of your Mistress' slick leavings.  And don't think for a minute I won't punish you if you miss any.</i>\"\n\n", false);

			Render.text("Ceraph closes her eyes and hums, her twin tongues redoubling their efforts inside you.  Every nerve ending inside your climaxing quim seems to explode at once, and with your back arched, you cum on your slave's face.  Ceraph gurgles in happiness, her voice muffled by the plush, feminine flesh quivering over her face.  She happily swallows every ounce of fluid you produce", false);
			if (player.wetness() >= 5) Render.text(", even though her cheeks are bulging and her throat struggles to devour all of the fountaining girl-spunk.", false);
			else if (player.wetness() >= 4) Render.text(", even though her cheeks are slightly bulged and she's gulping it down.", false);
			else if (player.wetness() >= 3) Render.text(", even though she has to gulp it down from time to time.", false);
			else if (player.wetness() >= 2) Render.text(", even though you produce enough for her to gulp.", false);
			else Render.text(", even though your pussy doesn't gush like most of the corrupted creatures in this realm.", false);
			Render.text("  Still shaking and clenching, you start to come down, still holding Ceraph in her proper place.  She doesn't show any sign of discomfort, and as a matter of fact, once you deign to look down at her, her eyes are twinkling happily and her face is flushed.  ", false);
			if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00289] == 0) Render.text("Did she... get off with her tongue?\n\nSeeing the confusion on your face, Ceraph releases your spit-slathered genitals, her tongue returns to normal, and she says, \"<i>Mmm, of course, dear.  If only men knew what they were missing... tasting a woman's pussy while it climaxes on your cock is divine.</i>\"\n\n", false);
			else Render.text("Shuddering, Ceraph returns her tongue to normal and slides it out of your tender quim with a knowing smile.\n\n\"<i>I'll never get tired of that, " + player.mf("Master", "Mistress") + ",</i>\" she quips.\n\n", false);

			Render.text("You pull her back to your " + vaginaDescript() + " to lick the last of your lady-spunk from your nethers, then send her on her way with a smile on your face.  Your expression widens when you see Ceraph stagger, still a bit shaky from her own orgasm.", false);
...
//*Ceraph goes full tentacle and double penetrates herms (+ standard dick BJ if ceraph dick toggle is on) (Zeddited)
		private function ceraphTentacleGrape():void
		{
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00290] == 0) {
				Render.text("You smirk and wonder if you should punish her for stuffing her cock down your throat.  Do you?", false);
				simpleChoices("Punish", punishCeraphForSurpriseThroatFuck, "", null, "", null, "", null, "Leave", camp.returnToCampUseOneHour);
			}
			//ELSE: 
			else {
				Render.text("You nod graciously and begin to clean up, dismissing your personal demon... for now.", false);
				doNext(camp.returnToCampUseOneHour);
			}
		}
...
//Siamese Catgirl Twins - TDM (Zeddited, nya)
		public function catgirlEncounter():void
		{
			...
			Render.text("You call on Ceraph, but are a bit taken aback when she doesn't appear right away.  You look around to see if you might have missed her, then spot something else streaking towards you from the wastes.  It is kicking up so much dust that you don't have time to see what it is before it ", false);
			if (!player.perks.has("Evade") && player.stats.spe / 5 + rand(20) < 22) Render.text("flies into you, knocking you to the ground.  After a moment, you find yourself faced with a pair of overeager cat-morphs grinning down at you.", false);
			else Render.text("just barely misses you and crashes into the ground behind you.  After a moment, two bodies disentangle themselves from the impact site.  Once they stand up, you can see that a pair of overeager cat morphs have arrived in your camp.", false);
			Render.text("\n\n", false);

			//describing your new friends
			Render.text("You study them for a few moments.  They have glossy, soft, and pliable fur covering most of their bodies, generally with a whitish or mother-of-pearl tone.  That color gives way to darker layers on their legs and face, giving them a mischievous, owlish look.  They have feline legs with paws, but their arms look more like fuzzy hands with pads.  Long, flexible tails swish behind them, and pointy cat ears adorn the tops of their heads.  Their chests bear generous E-cup breasts, with small bare patches further down where other pairs of nipples poke out.  They have kittenish, clear blue eyes that lend an air of innocence to them, but the small horns on their foreheads suggest anything but.  Being Ceraph's pets, they are predictably perforated with piercings on their ears and tails.  One thing that plays over and over in the back of your mind is that the two are completely identical; <b>you're facing a pair of siamese cat twins</b>!\n\n", false);

			Render.text("As a chorus, the two start to speak.  \"<i>Mistress Ceraph couldn't come, so she has sent us to help you with your needs; the sisters are here for your pleasure.</i>\"  The choice is yours; do you play with these furry, eager, cat-faced girls, or send them away?\n\n", false);
...
//No sex
		...
		{
			Render.text("", true);
			Render.text("You shake your head at the kitty sisters and tell them that you aren't interested in fucking cats; you wanted the sexy demoness you were promised.  The two mewl meekly before slumping away.", false);
			//to camp menu
			doNext(playerMenu);
		}
...
//SEX!
		private function fuckCeraphsCatgirls():void
		{
			...
			Render.text("You smile at them and say that you'd be happy to have them for your pleasure; their horns suggest that they'll be quite a trip.  The two purr happily and instruct you to lie on your back to start the fun.  You relax as directed and the cat slaves unfasten your " + player.armorName + " from your body.  As they work, they make sure to gently stroke every inch of your newly exposed flesh with their soft furry hands as it's revealed; all the while moving closer and closer to your most personal parts.  When " + oMultiCockDesc() + " finally tastes the air, it gets even more attention.\n\n", false);

			//purrfect tit fuck
			Render.text("You can't help but put your hands to their heads and start rubbing and scratching them behind their ears.  Suddenly, one of them steps back as the other moves down in front of your " + cockDescript(x) + " and pulls it inside her breasts.  In response, both your hands end up on her head, forcing it down onto your cock, and you feel your manhood start to vibrate as the catgirl begins to purr.  You cry out in pleasure at your shaft being massaged by soft-furred breastflesh while she hums into the tip.\n\n", false);

			//play with da boobies
			Render.text("The other sister has been looking for something else to rub herself on, and she seems to have decided on your " + chestDesc() + ".  While your lower half is being covered by one cat (which is fine too), the other moves to your top half and drapes her breasts over your head while she gropes and plays with your " + nippleDescript(0) + "s.", false);
			if (player.biggestLactation() > 1) Render.text("  When some of your milk seeps out, she leans forward and latches onto a nipple eagerly, alternating between sucking on the tip and licking the drops off of it.", false);
			else {
				Render.text("  She seems to delight in playing with her chest, modest though it may be, pushing it into your face and tweaking the fuzzy nipples just past your nose.  You blow a raspberry and shake your face into her cleavage, ", false);
				if (rand(5) == 0) Render.text(" but some of the fur tickles your nose a bit <i>too</i> deeply; you deliver a sudden sneeze into her bosom, causing it to heave and jiggle.", false);
				else Render.text("vibrating it wildly back and forth.", false);
			}
			Render.text("  Her sister, watching all this, shakes with muffled laughter delivered directly into your cockhead, sending rough jolts of sensation down the shaft and forcing out a drop of pre-cum.\n\n", false);

			Render.text("The cat lying on your face sits up as her sister's eyes glimmer desirously upon tasting the drop; apparently she has recognized the expression, because she looks down at you and says, \"<i>Please, don't give all your rich, tasty cream to my sister.  She always steals my fair share, the bad kitty!</i>\"  Her meaning is obvious in context, moreso when she moves around to your groin, trying to shoulder her sibling aside and ", false);
			if (player.lowerBody.cockSpot.count() == 1) Render.text("sliding her hand between the furry tits, down the base of your " + cockDescript(x) + ".", false);
			else Render.text("grabbing the lonely, neglected dickflesh left outside the warm embrace of her sister's breasts and shoving them into her own while caressing the tip with her tongue.", false);
			Render.text("\n\n", false);

			Render.text("Thanks to all the stimulation from before, their expert tongues almost immediately bring your body to a shuddering orgasm, and " + sMultiCockDesc() + " ", false);
			if (player.cumQ() >= 1000) Render.text("unleashes a torrent of ejaculate, coating the two girls liberally; each yowls in happiness, slurping at the nearby cock-head and swallowing mightily.", false);
			else if (player.cumQ() >= 300) Render.text("lets out a generous load of 'cream' which each girl sloppily gulps down, lapping at the jism and its source roughly.", false);
			else Render.text("squeezes out a few immodest squirts of semen, which the girls push and fight over, each vying to be the one to gulp down the next stroke.", false);
			Render.text("\n\n", false);

			Render.text("You relax on your back, spent from the treatment and the orgasm; the kitty twins", false);
			if (player.cumQ() >= 1000) Render.text(" clean the remnants off of their fur eagerly, mollified by the sheer amount into actually helping lick each other clean with their tongues, and", false);
			Render.text(" thank you for the 'cream'.  You nod weakly and they jump to their feet and swish their tails at you, then depart.\n\n", false);
...
//NOTE2: Fen you might want to store the variable for the PC's cock type and cock size.
		...
		{
			Render.text("", true);
			Render.text("You call on Ceraph, but rather than the familiar sight of the purple omnibus, you see a human girl being brought into the camp by a gang of imps.  They approach you and pull the girl's collar down, forcing her to kneel before you.\n\n", false);

			Render.text("One of the imps steps forward and opens a letter, then begins reading.  \"<i>Lady Ceraph apologizes to her " + player.mf("Master", "Mistress") + ", but she finds herself unable to service you.  So she has sent this human as an offering for the " + player.mf("Master", "Mistress") + " to corrupt.  To this end, she has prepared a concoction for you.  Drinking this will provide you with what you need for the job, " + player.mf("Master", "Mistress") + ".</i>\"\n\n", false);

			//(Very High Corruption)
			if (player.stats.cor >= 75) {
				Render.text("You glare at the imp, asking if Ceraph's implying you're not able to fuck this girl by yourself.\n\n", false);

				Render.text("The imp recoils and offers a quick apology.  \"<i>No, of course not, " + player.mf("Master", "Mistress") + ".  Forgive us, we did not wish to offend.</i>\"\n\n", false);

				Render.text("Gruffly, you dismiss him with a wave of your hand.  The imp bows, thankful for your mercy.\n\n", false);
			}
			Render.text("He closes the scroll and holds out a bubbling black vial labelled \"<i>Drink me!</i>\" to you.  The other imps form a line behind the girl.\n\n", false);

			Render.text("Do you accept the 'offering' of the girl and drink the potion?", false);
			//[Yes][No]
...
//[=No=]
		...
		{
			Render.text("", true);
			Render.text("You wave the imps away and tell them that you're not interested.  One of the imps protests, \"<i>But, " + player.mf("Master", "Mistress") + "-</i>\"  You cut him off before he has a chance to finish, saying that you wanted Ceraph, not some human girl!  Then, you toss the potion away and tell them to take the girl away.\n\n", false);

			Render.text("\"<i>Y-Yes, " + player.mf("Master", "Mistress") + "...</i>\" the imps reply meekly, pulling on the collar to drag the girl away.", false);
...
//[=Yes=]
		...
		{
			Render.text("", true);
			Render.text("You grin and tell the imps that you will accept Ceraph's offering.  Then you circle the girl, appraising her.\n\n", false);

			Render.text("She is quite beautiful... about 5'4\" tall, with shoulder-length blonde hair.  Her face is covered by a blindfold, which you forcefully yank from her.  She gasps in fear and looks at you; her eyes are blue like the ocean, while her lips are pink and full.\n\n", false);

			Render.text("You gaze lower to appraise her breasts, guessing that they're at least a fair D-cup, and see that she's wearing a pair of nipple clamps connected by a chain. Her hands are held behind her by a pair of leather cuffs.\n\n", false);

			Render.text("You reach down between her legs, spreading them to probe her pussy; she gasps as you do so. You feel her pussy and realize it's moist... ha! The bitch is enjoying her predicament! You show her your glistening fingers and she looks away in shame.\n\n", false);

			Render.text("The vial of black fluid the imps offered you tastes sour and thick, and as it slides down your throat you can feel it burning a path of liquid heat through your throat.  The liquid settles in your belly and the heat spreads through your body; then focuses on your crotch.\n\n", false);

			//Cock obtained from this is human-looking, so you'd trigger the next paragraph too.
			//(if PC has no cock)
			if (!player.lowerBody.cockSpot.hasCock()) {
				Render.text("Intense pleasure overcomes you as you feel blood rush to your groin; ", false);
				if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("your " + clitDescript() + " swells", false);
				else Render.text("a small bump forms on your mons", false);
				Render.text(", then develops into a huge 16-inch long, 3-inch thick erection!  The tip practically explodes from the foreskin vainly trying to contain it.  ", false);
			}
			let x:number = 0;
			...
			if (player.cockArea(player.biggestCockIndex()) < 48 && player.lowerBody.cockSpot.hasCock()) {
				Render.text("Your " + cockDescript(x) + " throbs, veins bulging as it grows larger, ballooning to a generous 20-inch long, 3-inch thick size.  ", false);
			}
			//(if PC's cock is not demonic or pc has/had no cock prior)
			if (!player.lowerBody.cockSpot.hasCock() || !demon) {
				Render.text("A heady, musky scent emanates from your cock, then its color changes abruptly to a shiny inhuman purple hue and tiny sensitive nodules form along the length of the shaft; the crown develops a circle of rubbery protrusions that grow larger as you become more aroused.\n\n", false);
			}
			else {
				Render.text("You stroke your demonic prick, bringing it to full mast; it throbs as if knowing what is coming.\n\n", false);
			}
			...
			Render.text("You order them to remove the girl's bindings and hold her down. They quickly oblige, removing the leather cuff and pinning the girl down, then spreading her legs to allow you better access to her moist tunnel.\n\n", false);

			Render.text("You grab her hips and tease the poor girl by rubbing your nubbly shaft against her clit, forcing moans of unwanted pleasure out of her; moments later she screams in orgasm, her pussy juices already splashing against your ", false);
			if (player.lowerBody.balls > 0) Render.text("scrotum and ", false);
			Render.text(player.legs() + ".  The imps on her extremities laugh at the girl as she relaxes and her head slumps into the ground; you motion for the imps to release her and step back, then align yourself with her pussy.\n\n", false);

			//(if PC is above 60 cock area)
			if (player.cockArea(x) >= 60) {
				Render.text("It's clear to see that if you push inside her with a member of your size, you will rip her apart; thankfully one of the imps step forward with a vial containing a bluish fluid and forces it down her throat.  She drinks without resistance, then gasps as she orgasms once more, juices splattering about as her cunt seemingly grows elastic and wet enough for you to push the tip of your massive demonic cock inside her effortlessly.\n\n", false);
			}
			Render.text("You plunge into her warm depths, and she moans as your shaft forcibly forces her walls apart.  When your hips finally collide she screams, \"<i>Yessss!</i>\" and orgasms once more, milking your shaft with powerful contractions even as you begin pounding her in earnest.  Something in Ceraph's concoction must be playing havoc with your nerve endings; the newly-found sensitiveness of your shaft and the stimulation from her pussy are too much to contain and you burst inside her, shooting jet after jet of cum inside the girl's stretched pussy.\n\n", false);

			Render.text("You empty ", false);
			if (player.lowerBody.balls > 0) Render.text("your balls", false);
			else Render.text("yourself", false);
...
//[=Never Again - Fuck this nerd shit=]
		...
		{
			Render.text("", true);
			Render.text("You tell her, loudly and in no uncertain terms, that you have no interest in playing make-believe with her, and that next time Ceraph can come herself or have an ACTUAL innocent brought for you to corrupt.\n\n", false);

			Render.text("Chagrined, she unfurls her wings and flies off, the imps quickly wilting and following suit.", false);
...
//[=No=]
		...
		{
			Render.text("", true);
			Render.text("You tell her you have no interest in granting release to lowly imps.  If they want pleasure, then they should earn it themselves.\n\n", false);

			Render.text("\"<i>Sorry boys, " + player.mf("Master's", "Mistress") + " orders.</i>\"  She extends her wings and flies away, and the horny imps follow suit, still busy masturbating.  A 'pit-pat-pat' sound follows them, the noise of their pre-cum hitting the dry dirt from on high.\n\n", false);
			doNext(camp.returnToCampUseOneHour);
		}
...
//[=Yes=]
		...
		{
			Render.text("", true);
			Render.text("You smirk, seeing that this might be interesting... so you order her to pleasure the imps, all of them at the same time.\n\n", false);

			Render.text("The imps' eyes glow at your command, and they only stop masturbating long enough to pounce on the succubus and drag her to the ground.  She just smiles, offering no resistance as the imps hurry to fill her mouth, pussy, and ass, not to mention keeping her hands busy.\n\n", false);

			Render.text("The sight is arousing; the imps tug, grope and pull at the succubus, all while brutally fucking her.  The huge deposit you made inside her tight vagina splatters about with each wet slap of the imp fucking her pussy; the one inside her ass pushes brutally, as if trying to climb up her anus cockfirst; the one on her mouth makes use of her breasts whenever he pulls out; and finally the ones using her hands splatter pre on top of her, painting her purple skin white.\n\n", false);

			Render.text("The show doesn't last long, however.  The imps quickly climax with echoing cries. The one using her mouth cums so hard that some ejaculate backflows out of the succubus' nose.  The ones using her ass and pussy fill their respective holes, pulling out in the last spurt to paint the succubus' body in spooge.  Her hands, of course, complete the job by painting whatever was left  with the last two imp dicks.  By the end of the ordeal, the succubus is coughing and sputtering.\n\n", false);

			Render.text("\"<i>Look at what happened to me... used and transformed, then forced to service a bunch of dirty imps...  Thank you, " + player.mf("Master", "Mistress") + ",</i>\" she moans with a lewd smile.\n\n", false);

			Render.text("Licking the cum off her body, she sashays towards you to give you a little peck on the cheek.  \"<i>Hmm, you're such a good " + player.mf("Master", "Mistress") + ", I might have to leave Ceraph's harem and join yours instead.  See you around, hot stuff.</i>\"  She rounds up the tired imps and extends her wings, setting off alongside them.", false);
...
//[in a pig's eye, pal]  
		private function noUdderPlz(perm:boolean = false):void
		{
			...
			Render.text("A sharp head-shake is the only declination she needs.  \"<i>Of course, Sweetie, that wouldn't be very... Marble-like, would it?</i>\"\n\n", false);
...
//After that is a scene tailored more specifically for the PC. Intention is to give nagas and centaurs and all that their own scene. Currently only scene is for PCs with a dick - should be naga-compatible.
		private function cerminika():void
		{
			...
			if (flags[FlagEnum.CERAPH_ROLEPLAY_AS_DOMINIKA_COUNT] == 0) {
				Render.text("\"<i>Who?</i>\" Ceraph asks.\n\n", false);

				Render.text("You explain about the dark-lipped witch to your demon whore.  \"<i>Mmm, tattoos, that's gonna be rough,</i>\" she muses, licking her lower lip in thought.  \"<i>Think about her for a moment?</i>\"  You were already thinking of Dominika given that you were explaining who she was to Ceraph, but you suppose dealing with her idiocy is the price you pay for the sex.  Still, you acknowledge that you are.  Ceraph places her hands on your forehead and closes her eyes for a few moments, before purring and grinning.  \"<i>Oo, she's cute. Ask her if she likes piercings.</i>\"\n\n", false);
			}

			Render.text("Taking a deep breath, Ceraph closes her eyes.  When she opens them again their darkness has been replaced by bright white sclera, and brilliant blue irises.  Starting at the roots her hair brightens and blondes, pulling itself up into a stylized bun.  The demon's face shifts and squirms beneath the flesh, bones and muscle settling into subtle differences and adapting the cabalist's features.  Eyelashes adjust and shrink, losing the excessive glamour of the demonic and gaining a more down to earth and natural appearance.  Throughout the process her skin gradually shifts in hue to a more tanned shade, now resembling a color someone could actually be born with.\n\n", false);

			Render.text("Her figure retracts somewhat, growing slighter and shorter.  Previously obscene breasts diminish in size, and Ceraph removes her outfit to prevent undue damage.  Before your eyes her areolae shrink and change, nipples becoming positively petite.  Arms slender, shifting to a physique more expected of a sorceress.  Her demonic heels pull into her feet, the soles paling far more than the rest of her skin.  Amusingly, her ass plumpens somewhat, achieving the jiggle of the non-athletic.  Between her legs Ceraph's pussy becomes dignified and neat", false);
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text(" while her cock shrinks to what looks approximately like six inches of meat, an outright tiny penis for Mareth", false);
			Render.text(".\n\n", false);

			Render.text("Though much of her shrinks, her lips grow.  While unsurprising, it remains very enjoyable to watch.  Her lower lip rounds and swells, while her upper grows further defined.  Like a bruise, they darken, until no color but a glossy void remains.  As though they were being drawn directly into her skin, the lines of her tattoos begin to spread, starting from a single circle across her breast until the latticework of ink covers her tanned body.\n\n", false);

			Render.text("Dominika stands before you, nude and unrestricted. She licks her lips slowly.\n\n", false);

			//[Cock'd PC]
			if (player.lowerBody.cockSpot.hasCock()) {
				Render.text("\"<i>Champion,</i>\" those dark lips purr, \"<i>You have done everything I could have asked you for.</i>\"  She runs a hand along your cheek, stepping in close and caressing your arm.  \"<i>I owe you more than I can express.</i>\"  Her hand roams up into your hair, brushing through it before cupping the back of your head.  Dominika pulls you into a deep kiss, pressing her lips to yours in a breath-stealing passion.  Her tongue rolls into your mouth as though it intends to steal yours away, her leg eventually rising up and curling around you.", false);
				//[CERAPH COCK: 
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text("  Her dick presses up into your crotch.", false);
				Render.text("  \"<i>I dare say,</i>\" she purrs between the kiss, pressing her forehead to yours, \"<i>You may have seduced me, champion.</i>\"\n\n", false);

				Render.text("Pressing her hands on your shoulders she eases you down, straddling your lap.  \"<i>I linger in chastity out of fear that my dignity shall be taken from me in this land,</i>\" she purrs softly, tracing her hand across your chest.  ", false);
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 0) Render.text("She casually and playfully gives the gentlest squeeze to your " + chestDesc() + ".  ", false);
				Render.text("\"<i>But if I can rely on you – perhaps I do not need to worry.</i>\"  She pulls your " + player.armorName + " away, letting her hands roam hungrily across your body.  Blatantly turned on, her pussy rubs its juices against your crotch", false);
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text(", and her prick has remained firm and hard", false);
				Render.text(".  Her tongue roams hungrily over her inky lips, and an almost predatory grin sets across her features as she pulls up your " + cockDescript(0) + ".  Her fingernails cup under your head delicately and carefully tease you, rubbing her palm into the underside of your shaft.\n\n", false);

				Render.text("\"<i>How long,</i>\" she purrs, \"<i>Have you wanted to really be inside me?</i>\"\n\n", false);

				Render.text("Dominika slides her body back, rubbing the softness of her folds against your " + cockDescript(0) + ".  ", false);
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text("The very base of her prick teases the veins of your own shaft, but she seems focused on the pleasure of her cunt for now.  ", false);
				Render.text("She grins at you as she reaches the top of your cock, grinding back and forth across it.  \"<i>Mmm,</i>\" she muses quietly, biting her lower lip for a moment.  \"<i>It's been a long time since I let someone have me.  I forgot just how... warm a prick can feel down there.</i>\"  Her pussy casually clenches, brushing herself across your tip.  \"<i>You should feel honored.</i>\"\n\n", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text("  The degree that her cock abruptly shoots straight up is perhaps the most amusing part of the act.", false);
				Render.text("  You simply smirk at her, starting to bounce her into your lap and shoving your " + cockDescript(0) + " deeper inside. Once more she bites her lip, fingers digging into your stomach. No matter how classy she wants to appear, you can quite easily feel how wet she is", false);
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text(", and the heat of her prick bouncing into your lap", false);
				Render.text(".  \"<i>Hey,</i>\" she gasps, \"<i>Don't you want to savor it?</i>\"\n\n", false);

				Render.text("You slap her tit.\n\n", false);

				Render.text("The inability to frame her expression and hang it on a tree in your campsite is the greatest injustice the world has ever known.  Her black lips sit in a stunned part as she stares at you, even as you hump away at her sopping mound.  She blinks a few times, shakes her head once, the entire disbelief package.  Eventually her hands slide down her side to find your own, rubbing over your fingers as they hold her hips.  Then, sharply and surprisingly painfully, she digs her fingernails into your wrists, forcing you to let go.  \"<i>No,</i>\" she says with a wicked grin as she brings your hands to the side of your head, leaning forward and slowly undulating her hips in your lap.  \"<i>Really. Enjoy me.</i>\"\n\n", false);

				Render.text("You give her a carefully crafted retort that demonstrates your intellectual superiority, but she just gives you a fake smile and pats your cheek.  Eyes half-closed she slowly slides herself along you, enjoying the sensation of each inch of your " + cockDescript(0) + " as it brushes along her walls.  \"<i>Mmmm,</i>\" she lets herself purr out, \"<i>Worth... the wait.</i>\"  Her toes curl slightly, one of her legs shifting back slightly to alter the angle you enter her at.  It seems to work how she wants it to, as a deep, satisfied breath accompanies the next downward slide of her hips.", false);
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text("  A single bead of pre-cum forms slowly at the tip of her prick, growing larger until it finally slips down into the curve of her head, sliding down her shaft and into your lap.", false);
				Render.text("  She brushes her hair back behind her ear, sitting up straighter. In her quest to indulge in and feel your prick against all the warm flesh inside, she's neglected to keep you pinned.\n\n", false);

				Render.text("Grabbing her shoulders, you surprise her once more by twisting and rolling on top of her.  \"<i>No!</i>\" she shouts immediately, knowing your plot, \"<i>You " + player.mf("bastard", "bitch") + "!  No one puts me on my own back!</i>\"  You remind her that facts need more proof than the assumption that things will never change.  She spits at you, but you take solace in the fact that your hands are over her wrists and your " + cockDescript(0) + " is shoved so deep in Dominika there's a chance she could spit up cum when you're done rather than swallow it.  Presuming, of course, that her vagina is connected to her stomach, which it probably is not.\n\n", false);

				Render.text("Yet.  You haven't decided what you'll make her put in her mouth now that she's here.\n\n", false);

				Render.text("\"<i>F-fuck!</i>\"  Those dark lips manage to stammer out, \"<i>Stop fucking me! Let me ride you, you piece of shit!</i>\"  She manages to pull her hands free from the grip you have on them, only to beat on your chest a few times and push futilely against it.  ", false);
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text("Her prick bounces between her belly and yours as you take her.  ", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text("  You can practically hear her rock-hard cock kicking up the ground.", false);
				Render.text("  \"<i>What the fuck are you-</i>\" she begins, before shouting \"<i>Don't you DARE!</i>\"\n\n", false);

				Render.text("Grinding your prick between her dusky cheeks, you had already decided that you very much dared and didn't particularly care about her inevitable protests.  You calmly taunt her and her helplessness, pushing your fingers into her blonde locks as you hold her down and completely destroy any semblance of a nice, ordered hairdo.  \"<i>Stay away from my butt!</i>\" she eventually explicitly shouts, ruining the mystique of intruding upon it.\n\n", false);

				Render.text("\"<i>No,</i>\" you respond.\n\n", false);

				Render.text("It's surprisingly easy to spear yourself into the cabalist's plush posterior.  You had expected a woman of her nature to be tighter than she is, but the pussy juices soaking your " + cockDescript(0) + " are sufficient for a slippery and enjoyable entry.  She grunts and gasps dissatisfaction, but there's a deepness in her breath that betrays the excitement building within.  At the same time, she seems curiously nervous, and shakes not just in pleasure.\n\n", false);

				Render.text("\"<i>P-please,</i>\" she stammers out, even as her asshole clenches around you.  Though it was easy to force yourself into Dominika's puckered button, you're impressed at how well her colon squeezes your shaft.  There's no blown-out anal whore here, just a petite fucktunnel the perfect size to be broken by your " + cockDescript(0) + ".  You part aside her bunghole, claiming her ass in your name – colonizing her colon.  The entire reason those cheeks exist is cushioning for your crotch as you pound away at her, the juices her cunt left on your cock squirting out as your prick dominates every inch of those fleshy walls.  \"<i>Please,</i>\" she gets out again, \"<i>I've held out... so long...</i>\" She turns her head to look at you.  Tucked just behind the part of her hair, two small horns push from her skull.  A moment of confusion takes you, before it is replaced by a knowing smirk.  Jeering at the corrupted sorceress, you mock her inability to defend herself.  Without her chastity belt to protect her from the pleasure of the demon realm, it is free to feed upon her body and devour what purity remained, leaving behind just another succubus.  \"<i>Don't... make me cum,</i>\" she gasps.\n\n", false);

				Render.text("Under most circumstances you would have no opposition to denying a slut like her an orgasm, but considering the precipice she dangles over, you don't feel bad giving her a push.  Your " + cockDescript(0) + " continues its obscene assault on her backside, splitting her juicy cheeks to either side as you bend her guts to your dick's will.  Dominika lets out a moan that seems as though she may have intended it to be a scream of frustration, but her body is too far gone into pleasure to protest.  Like ink running through paper a deep dark purple begins to spread from her tattoos.  The color runs into itself and grows larger, leaving behind no lines where it joins.  The tan of her skin begins to disappear under the corruption.  \"<i>You " + player.mf("bastard", "bitch") + ",</i>\" she pants, shuddering as the horns grow larger.  \"<i>You would destroy my humanity... just for your pleasure?</i>\"\n\n", false);

				Render.text("\"<i>Yes,</i>\" you answer calmly.\n\n", false);

				Render.text("She wails in mixed frustration and pleasure, juices splattering against the ground as her hungry cunt grows hornier.  A darkness overtakes her eyes, and her nails grow long and sharp, and through it all your " + cockDescript(0) + " continues to degrade her ass.  As though a heartbeat pumping blood through her body, each thrust spreads the color over her skin a little more.  Her feet push and scrabble lightly against the ground in tormented frustration, but she is helpless beneath you.  You go without stopping, only slowing when wings unexpectedly burst from her shoulder blades, dark and starry.\n\n", false);

				Render.text("Grabbing your sorceress-cum-succubus fucktoy firmly around the hips, you go upright, lifting her off the ground and bouncing her against your body.  Moaning and squirting her juices against the ground, Dominika lifts her legs to help you, swelling indigo tits bouncing from your forceful thrusts.", false);
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text("  Her prick has become engorged and infernal, covered with abnormal bumps and veins. It practically spits up pre-cum, gurgling with a greatly increased supply.", false);
				Render.text("  \"<i>Take it then,</i>\" she gasps, \"<i>Take me... j-just give me... moreeeee...</i>\"  Her long nails – more slutclaws than anything else – dig into your arms as you hold her up almost painfully.  Squealing ecstatically, a long demonic tongue slips past her full corrupted ebony lips, slapping against her collarbone as her eyes roll up in pleasure.  Blonder and fuller, her hair naturally pulls back into an exaggerated bimbo-esque bun, and her horns have grown large and sinister.\n\n", false);

				Render.text("Satisfied with the ruined chastity of the former scholar, you wrap your arms around her hips and give a few final gut-wrenching slams into her dominated pucker.  Each one makes her squeal and gasp, and each one brings you one step closer to the shuddering eruption that is that slut's purpose.  Groping one of the whorish slutbags on her chest, you hold her to your body not out of intimacy, but dominance.  There is no mistaking your absolute control over her as the first thick blasts of cum spew into her abused ass and clog her cockhole.  Whimpering and clutching at you, all she can do is squeal and gasp your name, spitting your praises for destroying her dignity.", false);
				//[Player has large cum production: 
				if (player.cumQ() >= 500) Render.text("  You dump loads in her as though she's an addict who hasn't fed for days, bloating and humiliating her with the sheer volume of spunk that now inflates her belly.  Not as though the demon whore has much humility left to take.", false);
				else Render.text("  She shudders as your " + cockDescript(0) + " fills her guts with warmth, hips seizing as she rides out her assgasm.", false);
				Render.text("  Her cunt nearly waterfalls her juices, squirting obscenely as she rides out the pleasure.", false);
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text("  The demonic prick placed between her legs stops bouncing only because of how rock-hard it is as it spews her own filthy fucksludge across her body, splattering onto her tits and across her face.  The volume of off-white cream it spews out takes even longer to finish than you do, painting her obscenely.", false);
				Render.text("\n\n", false);

				Render.text("Her hands drag across your chest as she slinks off you, moaning and shaking.  You shift back on your ass to sit down after the long fuck, only to smirk as she eventually crawls back up to you.", false);
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text("  Her own filthy jizz has already been licked clean by her hungry mouth, so as not to dirty your handsome prick.", false);
				Render.text("  The demoness laps at your " + cockDescript(0) + ", cleaning it of spunk, sweat, and all the various filth it accumulates, sucking and dragging her fat, midnight lips across them.  Dominika purrs her happiness, her joy at being forced into such a whorish form.\n\n", false);

				Render.text("\"<i>Was that good?</i>\" Ceraph asks in her own voice, immediately ruining the illusion. \"<i>", false);
				//[Player has huge dick: 
				if (player.cockArea(0) >= 75) Render.text("I doubt her ass could actually take you so well, but... I wanted my " + player.mf("master", "mistress") + " to enjoy a perfect asshole.  Sue me.  ", false);
				Render.text("I hope that little slut appreciates what she's missing out on.</i>\"  She laps at your dick a few more times in the corrupted cabalist's visage, eventually sitting upright and slipping around behind you to massage your shoulders.\n\n", false);

				Render.text("\"<i>That said,</i>\" she whispers in your ear, \"<i>If you get a chance to do that to that cunt, do invite me.</i>\"", false);
			}
			player.orgasm();
...
//(First Time):
		public function encounterZetsuko():void
		{
			...
			if (flags[FlagEnum.ZETSUKO_MET] == 1) {
				Render.text("You call on Ceraph, but rather than the purple-skinned demoness you were expecting, you are surprised to see a fox-eared girl appear in a wash of corrupted violet flames.  She steps out of the portal and shakes off the flames as if they are water, and you take a moment to look her over.  The first thing you notice is that her arms are bound in a white straitjacket that is open at the chest, allowing her F-cup breasts to swing freely.  Four large fox tails twist erratically in the air behind her, and you can see a pair of small horns poking up from her hair in front of her large triangular ears.  Her body and face are covered in tribal tattoos, and her somewhat maniacal amber eyes are framed nicely by a head of messy auburn hair with black highlights.  Like many of Ceraph's harem members, she is adorned with numerous piercings and studs — almost too many to count them all — as well as a large studded collar.  Once you have had time to take it all in, she speaks.\n\n", false);
				Render.text("\"<i>Zetsuko apologizes for Mistress's absence, " + player.mf("Master", "other Mistress") + ".  Mistress wasn't able to be here, so she sent Zetsuko instead.  She hopes Zetsuko will be to your liking...</i>\"\n\n", false);

				Render.text("She punctuates her sentence with a creepy grin that shows off her oversized canine teeth, an evil gleam in her eyes hinting at an inner turmoil boiling just under the surface.  As she licks her lips with an almost animalistic hunger, you see that her tongue is extremely long and sports a large stud as well.  You quirk your brow at her peculiar manner of speech, but try your best to parse the information.\n\n", false);

				Render.text("\"<i>Zetsuko was wondering if " + player.mf("Master", "Mistress") + " could perhaps... loosen her jacket?</i>\" she says, struggling in her straitjacket a bit.  \"<i>Mistress tied it a little too tightly...</i>\"\n\n", false);

				Render.text("When she turns to offer you her bindings, you spot a small note pinned to her back, and reach out to take it.  It seems to have been written to you by Ceraph.\n\n", false);

				Render.text("\"<i>" + player.mf("Master", "Mistress") + ",\n\nI'm deeply sorry that I cannot be there to serve you in person, but I have sent Zetsuko here in my stead.  If you are reading this note, she has likely already tried to get you to release her bindings.  Respectfully, I would advise you that under no circumstances should you oblige her request.  Zetsuko has 'discipline' problems, as you may soon find out, and while my piercings suppress most of her more defiant tendencies, she has a frustratingly stubborn will.  She should not be able to raise a hand against you without your consent, but that would not stop her from trying, believe me.  Despite her 'difficulties,' she does have some enhancements that I think you'll really enjoy, as long as you don't fall for any of her tricks.\n\n", false);

				Render.text("Your Loyal Servant,\nCeraph\n\nP.S. Just in case she does get out, the safety word is 'banana.'\nP.P.S. Don't stick anything important in her mouth.\nP.P.P.S. Just to clarify, I mean the one on her face.</i>\"\n\n", false);

				Render.text("You give a confused blink, re-reading the last line.  As you begin to wonder what it means, Zetsuko spins around and plops down on her cushy rear, looking up at you indignantly.\n\n", false);

				Render.text("\"<i>" + player.mf("Master", "Mistress") + " isn't going to let Zetsuko out, is " + player.mf("he", "she") + "? Hehe... Too smart... Zetsuko is a bad girl, yes... many bad habits, trying to trick " + player.mf("Master", "Mistress") + ".  Mistress tries to teach her, but Zetsuko has far too much fun being bad.</i>\"\n\n", false);

				Render.text("Her legs are splayed out lewdly, and at last you start to understand what Ceraph meant in her note as your eyes are drawn to Zetsuko's drooling cunt.  Drooling, in this case, being literal — as you watch, an obscenely long tongue lined with studs is slithering its way out of her vagina, licking along the outer edges of her labia and dripping with what appears to be a mixture of saliva and pussy juices.  Thankfully, there don't seem to be any teeth, but the way that enormous tongue is wriggling around is so disturbingly erotic that you can't seem to pull your eyes away long enough to check.\n\n", false);

				Render.text("\"<i>Is Zetsuko's pussy to " + player.mf("Master", "Mistress") + "'s liking?</i>\" she says, laying on her back and spreading her legs further to proudly display her freakish appendage to you, putting on a display as she begins to tongue her own clit, flicking her hood piercing gently.\n\n", false);

				Render.text("Will you make use of Zetsuko now, or send her back to Ceraph?", false);
				if (silly()) banana = zetsukoBanana;
			}
			//(Subsequent times): 
			else {
				Render.text("You call on Ceraph, but rather than the purple-skinned demoness you were expecting, Zetsuko appears in a wash of corrupted violet flames, turning to you with a wide, toothy grin.\n\n", false);

				Render.text("\"<i>Zetsuko apologizes for Mistress's absence, " + player.mf("Master", "other Mistress") + ".  Mistress wasn't able to be here, so she sent Zetsuko instead.  Zetsuko was hoping perhaps today " + player.mf("Master", "Mistress") + " would deign to loosen Zetsuko's bonds?</i>\"\n\n", false);

				Render.text("You tell her that you have no plans to let her loose today, which causes her to pout a little, though Ceraph's piercings seem to keep her from being actively defiant.  Your eyes are drawn down to watch as her obscenely long tongue unfurls from between the folds of her pussy, slathering slick juices along the insides of her thighs in anticipation.", false);

				Render.text("Will you make use of Zetsuko now, or send her back to Ceraph?", false);
				if (silly()) {
					Render.text("  (You also remember Ceraph's note had mentioned something about a safety word... what was it again?)", false);
...
//▶[FuckVagina] requires penis
		private function fuckZetsukosTonguepussy():void
		{
			Render.text("", true);
			let x:number = player.biggestCockIndex();
			Render.text("Your lust and curiosity get the best of you as the sight of that licentious tongue wriggling around between her legs sends a heat to your groin, blood rushing into your " + multiCockDescriptLight() + ".  Zetsuko lays flat on her back, grinning up at you as her pussy licks itself in anticipation, and as you lower yourself down, her tongue snakes out even more, beginning to wrap itself around " + oMultiCockDesc() + " and slathering it with her saliva-like juices.  Your sensitive member can feel that the surface of her tongue is covered in hundreds of tiny bumps and nubs that massage your shaft, and the multiple studs that line it rub you sensually as the bizarre organ coils around your " + cockDescript(x) + " like a snake, the very tip gently stimulating around the edges of your urethra.\n\n", false);

			Render.text("You marvel at the incredible degree of control she seems to have over her tongue, watching as it begins to stroke and squeeze along your shaft, coating you in slick saliva from base to tip.  Its grip around you tightens up a bit, and you can feel it tugging you down gently, drawing your " + cockDescript(x) + " ever closer to her entrance.  Her vagina lips spread open as you approach, gaping wide like a hungry mouth, and as it does so, you can see that the inner surface of her pussy has a number of soft bumpy nodules lining it in a downward spiral pattern.\n\n", false);

			Render.text("\"<i>Zetsuko can't wait to have " + player.mf("Master's", "Mistress's") + " dick in her pussy...  she knows " + player.mf("Master", "Mistress") + " will just love it...</i>\" she says, wiggling her hips a bit as she pulls you up against her entrance, her lips sucking on the tip gently.", false);
			if (player.cockArea(x) >= 100) Render.text("  You wonder just how she plans to fit you inside, but there's not much you can do about it at this point.  You'll just have to trust that she knows what she's doing, crazy or not.", false);
			Render.text("  As you begin to grope her enormous jiggling breasts, you keep looking down to watch as her tongue slithers along your length, stretching out across it and then squeezing tight.  It begins to contract, the motion squeezing the first inch or so of your cock inside, and she sends up a moan, repeating the motion again and again, each time pulling a bit more of you inside.\n\n", false);

			Render.text("In no time at all, you ", false);
			if (player.cockArea(x) >= 100) Render.text("somehow ", false);
			Render.text("find yourself up to the hilt inside the strangely-textured orifice, while her tongue slides along the underside of your cock.  It curls along underneath you, ", false);
			if (player.lowerBody.balls > 0) Render.text("slithering across your " + ballsDescriptLight() + ", ", false);
			else if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("gently licking the outer edges of your labia, ", false);
			Render.text("moving along to the back, ", false);
			if (player.lowerBody.balls > 0 || player.lowerBody.vaginaSpot.hasVagina()) Render.text("and ", false);
			Render.text("teasing your " + assholeDescript() + " gently.  You gather up a mound of her soft tit flesh in each hand and begin squeezing along them lustfully, pushing on her pierced nipples with your thumbs gently while her tongue continues to lick and pleasure everything within its reach.\n\n", false);

			Render.text("\"<i>Ooo-oohh! " + player.mf("Master's", "Mistress's") + " cock tastes so good in Zetsuko's pussy!</i>\" she moans, rolling her eyes back as you feel her begin to \"<i>suck</i>\" on your " + cockDescript(x) + " deeply, rocking her hips back and forth.  The suction inside her drooling cunt feels like one of the most intense deepthroats you've ever felt, and you forget for a moment that it's coming from her vagina until Zetsuko's loudest moan yet snaps you back to reality.\n\n", false);

			Render.text("Deciding to give the crazy slut a little payback for so rudely interrupting your fantasy, you reach down and begin to twist the studs in her nipples lightly, grinning like a maniac as her slavering cunt suddenly clenches up around you from the sensations.\n\n", false);

			Render.text("\"<i>Ahh! Yes, " + player.mf("Master", "Mistress") + "! Punish bad Zetsuko! More, more, please!</i>\"\n\n", false);

			Render.text("As you continue to torture her nipples, she arches her back in ecstasy and begins to buck wildly, her pussy-tongue slapping around erratically now, flinging saliva-juices everywhere.  The tongue in her normal mouth is hanging out now, long enough to reach her chest, her eyes rolled back with a blissful grin on her face.  You can feel your climax coming on, and apparently Zetsuko can too as she yells out between moans, \"<i>Ah! " + player.mf("Master", "Mistress") + ", give Zetsuko your tasty cum!</i>\"\n\n", false);

			Render.text("With the way her pussy and tongue are squeezing you now, it would be nearly impossible not to indulge her request.  Your " + cockDescript(x) + " twitches inside her, releasing a creamy deluge that causes Zetsuko to begin drooling from both ends.  \"<i>Oooh...  so good, so thick, so creamy...  Fill Zetsuko up, yes!</i>\"", false);
			if (player.cumQ() <= 100) Render.text("  Your orgasm ends quicker than it seems Zetsuko wanted it to, the last of your few thin streams of cum spraying into her pussy.  It seems to suck on you all the harder, as if trying to dredge out just a few more drops.", false);
			else if (player.cumQ() <= 500) Render.text("  You fire creamy ribbons of cum into her, feeding what seems to be a never-ending hunger for semen as her pussy clenches and squeezes you for every drop.  Her tongue wraps around your member and squeezes along the length inside her to force out the remaining contents of your urethra, and a strong swallowing motion inside her vagina carries your thick load away.", false);
			else Render.text("  The strong sucking motions of her cunt milk your shaft for everything it has, her pussy greedily swallowing load after load of your hot spunk.  Her walls continue to milk you even as her abdomen begins to swell with the volume of your seed, growing outward into a gravid, jiggling belly.  When your orgasm finally ends, Zetsuko looks like she's several months pregnant, and she has a look of utter satisfaction on her face.  \"<i>NNnnnaaaaahhhhh....  Zetsuko...  is full.</i>\"", false);
			Render.text("\n\n", false);

			Render.text("Her pussy continues to suck on you gently as you pull out, and when your " + cockDescript(x) + " finally comes free of her bizarre lovehole, her tongue gently swirls around the lips before retracting itself inside.  You back up, wondering if you should help the bound kitsune to her feet, but as you are considering this, she flashes you a crazed grin and is suddenly bathed in purple flames, disappearing before your eyes.\n\n", false);

			Render.text("\"<i>Zetsuko hopes to taste " + player.mf("Master", "Mistress") + " again sometime...</i>\"", false);
			player.orgasm();
...
//▶[GetTongued] requires vagina
		private function getTonguedByZetsuko():void
		{
			...
			Render.text("Heat rushes to your groin as you stare at the outlandish appendage wriggling between Zetsuko's legs, feeling yourself becoming wet with anticipation.  Seeking to entice you further, she rolls herself back, placing most of her weight on her shoulders as she curls her lower body up into the air, her tongue slithering back and forth along her thighs.\n\n", false);

			Render.text("\"<i>Pleeeease, let Zetsuko taste Mistress's pussy?</i>\" she says, grinning up at you as you approach.  \"<i>She's certain her tongue can please Mistress greatly...</i>\"\n\n", false);

			Render.text("The salacious organ slithers toward you, caressing the outer lips of your " + vaginaDescript(0) + " and gently prodding your " + clitDescript() + " before receding, making what can only be described as a \"<i>come hither</i>\" motion.  You take a step forward, pressing your hips up against the heavy cushions of her ass, and her tongue immediately curls toward your groin, ", false);
			if (x >= 0) Render.text("wrapping itself around your " + cockDescript(x) + " and squeezing momentarily before ", false);
			Render.text("heading straight toward your slit.\n\n", false);

			Render.text("As the tip begins to probe your depths, you can feel that the surface of her tongue is covered in hundreds of tiny nubs and bumps that give it an incredible texture.  You can't help letting out a lewd moan as the muscular organ pushes yet deeper still inside you, flexing and contorting into various shapes to augment the intense sensations.\n\n", false);

			if (x >= 0) {
				Render.text("In a somewhat impressive feat of contortion, Zetsuko brings her feet in toward her groin to stimulate your " + multiCockDescriptLight() + ", lifting ", false);
				if (player.lowerBody.cockSpot.count() == 1) Render.text("it", false);
				else Render.text("them", false);
				Render.text(" up using her toes and stroking along the underside of the shaft", false);
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s", false);
				Render.text(" with the soles of her feet.  Her toes are surprisingly dextrous, able to curl around and grip your " + cockDescript(x) + " lightly, gently pinching and tugging at the skin just under the head and manipulating your throbbing member", false);
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s", false);
				Render.text(" almost as skillfully as if she were using her hands!\n\n", false);
			}

			Render.text("You slide your hips forward, curling her body back further and resting your weight on the pillowy cushions of her bottom, riding down onto her tongue and shivering deliriously.  You can feel it plumbing your depths, slithering into you and filling every nook and cranny as it swells and flows along your walls to take up every last bit of room you have.  The degree of control she has over it is simply incredible, becoming thicker to stretch your " + vaginaDescript(0) + " and then suddenly retracting to swirl itself into a spiralling tendril that drills your hole lewdly.\n\n", false);

			Render.text("\"<i>Oooh, Mistress's pussy is delicious!</i>\"\n\n", false);

			Render.text("Grabbing hold of her thighs, you begin to rock yourself back and forth across her tongue, moaning powerfully as you feel it begin to thrust in and out of you now.  The numerous studs that line the center of the organ begin to press firmly against the roof of your " + vaginaDescript(0) + ", stimulating your G-spot and flicking your " + clitDescript() + " each time her tongue dives back inside you.  Your fingers dig into her flesh like a vice now as you grind yourself down on the indecent tendril, moaning salaciously as the waves of your climax begin to overtake you.\n\n", false);

			Render.text("Your " + vaginaDescript(0) + " cinches tight around Zetsuko's tongue, and it continues to wriggle inside you while you ride out your orgasm.", false);
			if (x >= 0) {
				Render.text("  " + SMultiCockDesc() + " twitches powerfully between her feet, ", false);
				if (player.cumQ() <= 100) Render.text("spurting a healthy spray of hot cum all over her face and chest.", false);
				else if (player.cumQ() <= 500) Render.text("distending visibly as it begins to pump out thick streams of spunk all over her waiting face and tits.", false);
				else Render.text("unleashing a heavy downpouring of cum all over her face and tits that soaks through her straitjacket and mats her hair down with your thick load.", false);
				Render.text("  She grins lasciviously and opens her mouth to reveal her other freakishly long studded tongue, using it to lap up your spunk hungrily and clean herself off.  \"<i>Mistress's cum is so tasty...</i>\"", false);
			}
			Render.text("\n\n", false);

			Render.text("The slick juices of your climax trickle down, pooling inside Zetsuko's gaping pussy", false);
			if (player.wetness() >= 4) Render.text(" and overflowing to dribble down her bountiful ass cheeks and stomach", false);
			Render.text(".  You hear an odd gurgling sound, and watch as the puddle of juices slowly begins to drain down into her vagina, a visible swallowing motion carrying it away inside her.  \"<i>MMmmmm...  Zetsuko loves the taste of Mistress's juices...</i>\"\n\n", false);

			Render.text("Fully spent, you pull yourself back, grasping Zetsuko's still-writhing tongue in your hand and sliding it out of you, letting her lower body thump to the ground.  You watch as her tongue gently swirls around the lips and then retracts within the bizarre orifice, looking for all the world to be an ordinary — if slightly overstretched — pussy.  You wonder if you should help her up for a moment, but as you are considering reaching down to bring her to her feet, she is suddenly bathed in corrupted flames, disappearing before your eyes.\n\n", false);
...
//▶[Banana?] silly mode
		...
		{
			Render.text("", true);
			Render.text("Banana? What the flying fuck is a banana?\n\n", false);
			Render.text("Banana.\n\nBanana banana banana.  Just saying it feels kind of funny, so you say it a few more times just to be sure.  Yep, still funny.\n\n", false);

			Render.text("\"<i>FUCKING MARAE PLEASE STOP SAYING BANANA — FUCK!!</i>\"\n\n", false);

			Render.text("You hear a thud and turn to see Zetsuko lying face-down on the ground, struggling in vain to get back up with her arms bound.  She finally manages to wrench herself into a kneeling position, panting from exertion.  Her forehead has a big blotch of dirt on it like she has been repeatedly slamming it on the ground.\n\n", false);

			Render.text("Banana.\n\n", false);

			Render.text("\"<i>FUCK!</i>\"\n\n", false);

			Render.text("As the word leaves your lips, you see the studs on Zetsuko's collar glow, and she is suddenly forced down to the ground again, face mashed in the dirt and her ass raised high while her pussy-tongue flails about wildly between her thighs.\n\n", false);

			Render.text("\"<i>Hah... hah...  Zetsuko dislikes... those fruit... immensely,</i>\" she says, avoiding speaking the word aloud herself as she tries to lift herself back up yet again.\n\n", false);

			Render.text("Oh, so a banana is a fruit? *THUD*\n\n", false);

			Render.text("\"<i>FUCK!</i>\"\n\n", false);

			Render.text("You'll take that as a yes.\n\n", false);
			doNext(camp.returnToCampUseOneHour);
		}
		...
		{
			Render.text("", true);
			Render.text("You tell her to be gone; you wanted Ceraph, not some lowly slave that can't even be bothered to show you proper respect.  You order her to tell Ceraph not to waste your time with undisciplined servants that she's too inept to properly break.\n\n", false);

			Render.text("\"<i>Oohoh, Mistress is certain to be FURIOUS with Zetsuko...</i>\" she says, licking her lips and showing off the numerous studs that line her normal tongue as well.  \"<i>Zetsuko cannot wait...</i>\"", false);
			doNext(camp.campSlavesMenu);
		}
		...
		private function giveFollowerBodyBits():void
		{
			spriteSelect(87);
			clearOutput();
			Render.text("You ask Ceraph just what all giving up a body-part would entail.  Your submissive demonic slut presses herself against you, stroking her hands under your " + player.armorName + " as she answers, \"<i>Well, [Master], I would use my body-shifting black magics to remove a choice portion of your 'fun-bits', if you know my meaning.");
			if (player.lowerBody.cockSpot.hasCock() || player.lowerBody.vaginaSpot.hasVagina() || player.hasBreasts()) {
				if (player.lowerBody.cockSpot.hasCock()) Render.text("  I could take your cock.");
				if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("  I could remove your snatch.  Don't worry, once it's off of you, anything I put in it won't wind up in your womb.");
				if (player.hasBreasts()) Render.text("  I could even make off with your tits if you like.");
			}
			else Render.text("  Sadly, you don't have anything that would be worth taking right now.  A true shame, my [Master].");
			Render.text("</i>\"");
		...
		private function ceraphFollowerCockTaking(smallest:boolean = false):void
		{
			...
			let y: number = x + 1;
			Render.text("You sigh and undress enough to point at your [cock " + y + "], indicating that Ceraph is welcome to have it.  Ceraph titters, \"<i>With pleasure, my [Master].</i>\"");
			...
			if (player.lowerBody.cockSpot.list[x].cockThickness >= 4) Render.text(", compressing your massive girth into the rigid shaft", false);
			Render.text(", slowly closing.  It should hurt but it doesn't, it just feels warmer and warmer, and then with a sudden 'PYOING!', Ceraph is holding your " + cockDescript(x), false);
			if (player.lowerBody.balls > 0 && player.lowerBody.cockSpot.count() == 1) {
				Render.text(" and balls", false);
				//Gotcher balls!
			...
			Render.text(" in her hand!  At the base there's smooth flesh and an arcane mark, somehow keeping the disembodied dick alive to pulse and squirm in her grasp.  The place on your groin is left completely smooth and featureless, as if it had never been there at all.", false);
		...
		private function ceraphFollowerCuntTaking():void
		{
			spriteSelect(87);
			clearOutput();
			Render.text("You undress, just enough to point at your [vagina].  Ceraph smiles happily and muses, \"<i>I have just the troublesome slut that could learn a thing or two by having her mouth replaced by a pussy.  Face-fucking is such an effective discipline technique, thank you dear.</i>\"");
		...
		private function ceraphFollowerTitTaking(rowNum: number = 0):void
		{
			spriteSelect(87);
			clearOutput();
			let x:number = rowNum;
			...
			Render.text("You pull down your [armor] to bare your bosom to Ceraph and indicate that you'd like her to take it.  She smiles like a cat who's gotten the cream and whispers, \"<i>Your wish is my command, [Master].</i>\"");
			...
			if (x < player.upperBody.chest.count() - 1 && player.upperBody.chest.count() > 1) {
				//If only 1 row below
				if (x >= player.upperBody.chest.count() - 2) Render.text("your torso has shifted and your " + breastDescript(x + 1) + " have moved up to fill the void.", false);
				//Multiple below
				else Render.text("your torso has shifted and your other breasts have moved up to fill the void.", false);
				player.removeBreastRow(x, 1);
			}
			//Top row is only row
			else if (x == 0) {
				Render.text("your now-flat chest slowly gains two tiny, quarter-inch nipples.", false);
			...
			//everybody else
				...
				Render.text("your torso leaves the now-empty spot as a flat, featureless void.", false);
				player.removeBreastRow(x, 1);
			}
			//Oh shit something went wrong.
			if (player.upperBody.chest.count() == 0) Render.text("<b>ERROR!  ERROR!  Please contact Fenoxo and reload your game if you don't want your save messed up.</b>", false);
			Render.text("\n\n", false);

			Render.text("You gasp when Ceraph ", false);
			if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00217] == 3) Render.text("rolls the nipples in her hands", false);
			else if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00217] == 4) Render.text("pushes her fingertips inside the leaky nipple-cunts", false);
			else Render.text("squeezes out a squirt of milk", false);
			Render.text(", going a little weak in the knees.  Ceraph laughs and lowers the bouncy orbs down between her legs, and you can feel every little bump and nodule of her corrupted cock as she tit-fucks herself on your disembodied breasts.\n\n", false);

			Render.text("\"<i>Oh oh, does someone like having a nice hard cock buried in her tits?  Maybe I should've left these on you so that I could get the full benefit before I left.  Maybe next time,</i>\" taunts the demoness while she continues to tit-fuck herself on your former bosom.  \"<i>Don't worry, [Master], I'll be sure to give them a good home.  You should get plenty of nice dreams out of these sweater puppies!</i>\"\n\n", false);

			Render.text("Ceraph turns and prances off through the rocks, tweaking your nipples HARD every few moments to remind you of your choice.  She vanishes before long, leaving you to deal with the slowly dwindling sensation of her cock in your tits.\n\n", false);
...
//NippleCunt Stuffing (Ceraph grows dick-nipples to plow your lusty twats!)
		private function stuffSomeNippleCunts():void
		{
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text(" and proud demon-dick");
			Render.text(".  She grabs your [chest] to examine, and her fingers probe hesitantly around the entrances of your budding chest-cunnies.  Ceraph smiles with wicked intent.  Her fingers thrust in forcefully, drawing a sensual moan from your lips as she violates ");
			if (player.upperBody.chest.countNipples() > 2) Render.text("two of ");
			Render.text("your tit-pussies.");
			...
			if (player.upperBody.chest.countNipples() > 2) Render.text("  She moves from pussy to pussy with ease, exploring every single one of your myriad holes, a gleeful smile on her face.");
			if (player.lactationQ() > 0) Render.text("  A squirt of breastmilk nearly takes your slave in the eye, your milk letting down uncontrollably from the tit-puckering finger-fuck.");
			if (player.lowerBody.cockSpot.hasCock()) Render.text("  [EachCock] is so hard by this point that it's smearing its pre-cum all over Ceraph's belly, but she doesn't mind.");
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("  As for your [vagina], you've grown more than wet enough for a hard dicking, but honestly, right now, all you want is your [nipples] to be perfectly and completely fucked.");
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text("  The fetishistic slut's sloppy cock is dribbling on your hip, neglected and aroused.  Poor thing.");
			...
			if (player.bRows() > 1) {
				Render.text("  A second later, ");
				if (player.bRows() == 2) Render.text("another row of female flesh begins");
				else Render.text("more rows of female flesh begin");
				Render.text(" to sprout beneath her top pair, the demoness's chest mimicking yours in size and structure.  While the top pair of super-sized nipples continue to grow into massive teats, Ceraph repeats her caresses lower, and the newly-formed nipples mimic their big brothers, filling out to obscene length.");
			}
			Render.text("  At first, the changes make her areola look more like something that belongs on an udder, but as they enlarge beyond even that, you realize they're taking on a distinctly masculine shape, with a slight ridge just below their bulbous tips.  After growing out almost seven inches, the upward-curving nipples stop filling out.  Ceraph has sprouted nipple-dicks!");
			if (player.averageNipplesPerBreast() > 1) Render.text("  She strokes them, one by one, and they split down the middle, dividing into a multitude of penises, a perfect match to your quad-nipplecunts.");
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text(", her cock spouting its own ivory wetness onto your belly in sympathetic release.");
			else Render.text(", her pussy dripping weakly in sympathetic release.");
			Render.text("  Her hands run through your [hair] and pull your [face] into her shoulder.  She holds you tightly, clingingly even.  If you weren't so enraptured by the sensation of your sperm-filled nipples, you might actually care.");
...
//Portal Fuck (AKA Ceraph Hung Out With Cinnabar, Req's PC dick)
		private function portalFuckWithFollowerCeraph():void
		{
			...
			if (flags[FlagEnum.TIMES_CERAPH_PORTAL_FUCKED] == 0) {
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 1) Render.text("  Of course, you don't seem to like that appendage, so my ring won't get much use today, will it?");
				Render.text("</i>\"");
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 1) Render.text("  Ceraph twirls one of the rings into her palm, where it promptly vanishes in a puff of purple smoke.");
				...
				Render.text("You suggest Ceraph retrieve her fantastically enchanted cock-rings, and with an impish smile, your slave produces a pair of them in puffs of purple smoke.");
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 1) Render.text("  She sadly teleports one of them away, knowing you've commanded her to stay female for you.");
				Render.text("  The mauve miscreant tosses your ring to you, still warm from her body heat and tingling with the embedded magic.  She requests, \"<i>Please, [Master], do me like you did before!  It was so hot for you to watch me like that!</i>\"");
			}
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text(" and eager cock");
			Render.text(".  She scoots her ass up so that it's resting against your [legs] and the front side of the chair, a tantalizing heart-shaped butt just within arms' reach.  You give it a slap, which garners a delighted coo from the tainted slut.");
			...
			if (flags[FlagEnum.TIMES_CERAPH_PORTAL_FUCKED] == 0) Render.text("  Well, your dick doesn't look any different, aside from the gilden adornment stuck near the bottom.  Now, if it were hilt-deep in the demon's cunt, that would be something!");
			else Render.text("  Well, no sense wasting time!  You look down at Ceraph's moist box and imagine your dick spearing through her folds.");
		...
		private function layEggsInSlaveCeraph():void
		{
			...
			Render.text("You ");
			if (!player.isTaur()) Render.text("absently rub the sensitive bulge in your abdomen as you ");
			Render.text("regard your slave.  There certainly is something she could do for you.  Imperiously, you let her know that you have a load you wish to get rid of, and you intend to plant it in her.");

			//[if (cocks > 0) 
			if (player.lowerBody.cockSpot.hasCock()) Render.text("  Ceraph glances at the bulge in your [armor] quickly before she demurely lowers her eyes.  A wicked smile plays across her beautiful face as she responds, \"<i>Yes, [master].</i>\"");
			else Render.text("  Ceraph frowns as she glances towards your crotch.  She is disciplined enough to answer you quickly, however, with a sharp \"<i>Yes, [master].</i>\"");
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text("  The whole garment creaks and strains under the pressure of her hardening member.");
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text("; unable to keep it penned up, her demonic cock spills out the side, waving back and forth obscenely as you saw the offending garment into her crotch");
			Render.text(".");
			...
			if (!player.isTaur()) Render.text(", and your fingers tremble slightly as you trace its segmented length, leaving a trail of lubricating mucus from tip to base");
			Render.text(".  When you order your demonic slut to present her pussy to you, she slides an impeccably manicured hand between her thighs, opening her dark, glistening lips for you.");
			...
			if (player.gender > 0) Render.text("Your orgasm spatters your degraded demon's rear, adding to the sopping mess your ovipositor is leaving.  ");
...
//Adds "Submission" option to the roleplay menu.
		private function sumissivenessToCeraphFollower():void
		{
			clearOutput();
			spriteSelect(87);
			Render.text("When you broach the idea of submitting to your fetish queen, delight washes across Ceraph's face.  Before another word can be spoken, she lashes out with her whip (where did she get that from?) and catches you around the neck with the taut leather, yanking hard enough to drag you to the ground and make you sputter for air.  The shining leather slides off as soon as your hands hit the ground, but the demoness is already standing above you, pushing on you with her stiletto-like heels to roll you aside.");
			...
			if (player.gender == 3) {
				Render.text(", both in the hardness of your manhood");
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
				Render.text(" and the moisture leaking from under your equipment");
			}
			else if (player.gender == 2) Render.text(", what with the beads of moisture forming between your legs");
			else Render.text(".  Your hardness is easily visible to anyone who knows where to look, and the demoness definitely knows where to look");
			Render.text(".");
			...
			if (player.lowerBody.cockSpot.hasCock()) Render.text("[oneCock]");
			else Render.text("your rigid clit");
			Render.text(" with ticklishly-soft touches");
			if (player.lowerBody.balls > 0) Render.text(" before gently squeezing your [sack]");
			Render.text(", savoring your state.  You groan out loud at the sudden sensation, and a bead of ");
			if (player.lowerBody.cockSpot.hasCock()) Render.text("pre-cum has begun to form on your [cockHead biggest]");
			else Render.text("girl-cum has begun to form at your quivering entrance");
			Render.text(".  The crystal-clear droplet of congealed lust slowly begins to dangle down, connected for a few seconds by a string of moisture before it snaps and disappears into the dirt.");
...
//[Apple]
		private function sayAppleToCeraph():void
		{
			clearOutput();
			spriteSelect(87);
			Render.text("Fuck this!  \"<i>Apple!</i>\"");
...
//[Either Once]
		private function barkToCeraphOnce(dog:boolean = true):void
		{
			...
			spriteSelect(87);
			Render.text("\"<i>");
			if (dog) Render.text("Arf!</i>\" you bark");
			else Render.text("Meowwwww!</i>\" you mew");
			Render.text(", loud and almost proudly");
			if (player.tailType > TAIL.NONE) Render.text(", wagging your tail excitedly");
			Render.text(".");
			Render.text("\n\nCeraph casually pulls her panties a bit lower to ");
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) Render.text("ease the pressure on her tightly-contained phallus");
			else Render.text("allow a penis to grow, fully-formed from where her clit was");
			Render.text(".  The stalk of her cock drips with the swampy cunt-lubricant her delta exudes, and she pumps it a few times to make it stand long and erect, nine inches of pulsating, demonic horror.  Tiny nodules decorate the shaft in concentric rings to stimulated anyone lucky enough to experience her cock.  As Ceraph eyes your backside, you realize that you're going to be feeling it before long.");

			Render.text("\n\n\"<i>");
			if (dog) Render.text("ARF!  ARF!");
			else Render.text("REOOOOW!");
			Render.text("</i>\" you ");
			if (dog) Render.text("bark");
			else Render.text("meow");
			Render.text(" in distress, trying to crawl away.  You didn't mean to get ");
			if (player.lowerBody.cockSpot.hasCock()) Render.text("milked ");
			else Render.text("fucked ");
			Render.text("like... like that!  Her whip snaps taut before dragging you back.  You scrabble in the dirt for a half-second, choking yourself before you give up and sag limply down at her feet.  Slowly, you lift your [butt] up into the air to allow Ceraph access, regretfully yielding to her like you should have initially.");
			...
			if (dog) Render.text("doggie");
			else Render.text("kitty-cat");
			Render.text(".</i>\"  Her hips press the hot spear harder against your [asshole].  ");
			if (player.ass.analLooseness == 0) {
				Render.text("Even though you're a virgin, the cunt-slick tool worms its way into your rectum with ease, the nodules massaging your sorely-stretched anal ring as it accommodates to handle the bigger width, forever ruined by Ceraph's boner.");
			}
			else if (player.analCapacity() < 20) Render.text("Even though you're pretty tight, the cunt-slick tool slides into your rectum with ease, the semi-soft nodules massaging your anal ring as it adjusts to the heated impalement.");
			else if (player.ass.analLooseness >= 4) Render.text("The cunt-slick tool has no problem wiggling inside your well-used and abused asshole, hilted in a split second, the hundreds of nubs gently caressing your anal ring like a lover as they nestle inside you.");
			else Render.text("The cunt-slick tool slides right on in thanks to your capacitative anus, the gentle nubs each massaging your snug ring as they slide on through.");
			Render.text("  You groan as you're taken by your owner, catching yourself halfway and arresting the pleasured grunt as soon as you can, but it doesn't matter - the damage is already done.");
			...
			if (player.lowerBody.cockSpot.hasCock()) Render.text("your twitching maleness");
			else if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("to your achingly empty pussy");
			if (player.lowerBody.cockSpot.count() > 1) Render.text("es");

			Render.text(".  Almost immediately, you squirt out a huge dollop of ");
			if (player.lowerBody.cockSpot.hasCock()) Render.text("pre-cum");
			else Render.text("lady spunk");
			Render.text(".  It dangles heavily enough to ");
			if (player.lowerBody.cockSpot.hasCock()) Render.text("make your [cock biggest] bob below");
			else Render.text("tantalize and tease your clit");
			Render.text(", and you give up another grunt of pleasure while your hips press harder against your owner's.");
			...
			if (player.lowerBody.cockSpot.hasCock()) Render.text("rams straight into your inner, cock-milk reserve");
			else Render.text("rubs against your quivering vaginal walls, stimulating you from the inside out");
			Render.text(", and fireworks start to go off inside your brain.");

			Render.text("\n\n");
			if (player.lowerBody.cockSpot.hasCock()) Render.text("Dick-milk");
			else Render.text("Love-juice");
			Render.text(" drizzles from ");
			if (player.lowerBody.cockSpot.hasCock()) Render.text("[eachCock]");
			else Render.text("your slightly parted twat");
			Render.text(" into a pool on the floor as you're expertly brought to a tingly, body-quivering, anal orgasm.  It can barely be called an ejaculation, really.  The ");
			if (player.gender == 2) Render.text("fem-");
			Render.text("cum is just ");
			if (player.lowerBody.cockSpot.hasCock()) Render.text("sliding");
			else Render.text("pouring");
			Render.text(" smoothly out of it as your ");
			if (player.lowerBody.cockSpot.hasCock()) Render.text("inner reservoir is squeezed");
			else Render.text("cunt walls are stimulated through your anal walls");
			Render.text(".  Ceraph continues to push, but her dicktip slips off its mark, and she quickly buries herself back inside, each nodule responsible for small blurts of jism as they slip across your orgasmically tightening ");
			if (player.lowerBody.cockSpot.hasCock()) Render.text("prostate");
			else Render.text("walls");
			Render.text(".  ");
			if (player.lowerBody.cockSpot.hasCock()) Render.text("Ironically, [eachCock] has begun to lose some of its stiffness, and you grow limper and limper as more of your cum leaks from you");
			else Render.text("Strangely, the more femcum that leaks from your winking, tender twat, the more your body tingles and shivers with barely-constrained delight");
			Render.text(".");

			Render.text("\n\nMost amazingly of all, though ");
			if (player.lowerBody.cockSpot.hasCock()) Render.text("the 'pressure'");
			else Render.text("a need within you");
			Render.text(" feels released, you're still horny");
			if (player.lowerBody.cockSpot.hasCock()) {
				if (player.lowerBody.cockSpot.count() == 1) Render.text(", even with a soft cock");
				else Render.text(", even with soft cocks");
			}
			Render.text(".  Ceraph purrs, \"<i>I told you that I was a good owner, didn't I?  Now just lie there and enjoy myself on your sensitive tush.</i>\"");

			Render.text("\n\nShe rapidly begins to pump her penis through your sphincter, battering your [asshole] with firm but gentle thrusts.  You ");
			if (dog) Render.text("bark in pleasure like the cum-slut doggy that you are");
			else Render.text("mewl in delight like the cock-loving kitty that you are");
			Render.text(" and start to push back against her before long.  Those nodules rub your insides so well, and they keep you dripping wasted ");
			if (player.lowerBody.cockSpot.hasCock()) Render.text("semen");
			else Render.text("juices");
			Render.text(" from your ");
			if (player.lowerBody.cockSpot.hasCock()) {
				Render.text("limp cock");
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
			}
			else Render.text("snatch");
			Render.text(" the entire time, tingling in ecstatic pleasure.  Soon, you're panting hard while Ceraph's hips slap off your [butt] hard enough to sound like miniature thunderclaps, and your poor ");
			if (player.lowerBody.cockSpot.hasCock()) Render.text("prostate is getting pushed up against each time");
			else Render.text(", depressingly empty cunt walls are getting teased with every thrust");
			Render.text(".");
			...
			if (player.lowerBody.cockSpot.hasCock()) Render.text("prostate is");
			else Render.text("vaginal walls are");
			Render.text(" being smashed flat.  Warmth explodes inside your intestines, slowly spreading out around the flaring, demonically enhanced dick as it orgasms.  You can hear your owner's lady-spunk splattering into the ground behind you as she whimpers, \"<i>G-g-good... " + player.mf("boy", "girl") + "...</i>\", her dick firmly spraying its thick, tainted milk ");
			if (player.lowerBody.cockSpot.hasCock()) Render.text("straight onto your sore prostate");
			else Render.text("putting more delicious pressure on your sensitive walls");
			Render.text(".  You scrunch your eyes closed as Ceraph claims your ass for her own, and ");
			if (player.lowerBody.cockSpot.hasCock()) {
				if (player.lowerBody.cockSpot.count() > 1) Render.text("each of your cocks");
				else Render.text("your cock");
				Render.text(" lets go the very last of your sticky, stinky, pet-cum");
			}
			else Render.text("your pussy lets go of the last of its sticky, slippery snatch secretions");
			Render.text(".");
			if (player.cumQ() >= 700) Render.text("  It floods out everywhere.  You're just so productive that your hands and [legs] sink partway into the mess.");

			Render.text("\n\nYour Mistress eventually finishes and pulls out, offering a ");
			if (dog) Render.text("tasty bone-shaped biscuit");
			else Render.text("fishy-smelling treat");
			Render.text(" for you to gobble down, and she pats you on the head as you gratefully accept your reward.");
...
//[Either Twice]
		private function barkOrMeowTwiceToCeraph(dog:boolean = true):void
		{
			...
			spriteSelect(87);
			Render.text("\"<i>");
			if (dog) Render.text("Arf! Arf!</i>\" you bark");
			else Render.text("Meow!  Meowwwww!</i>\" you mew");
			Render.text(", wiggling your ");
			if (player.tailType > TAIL.NONE) Render.text("tail");
			else Render.text("[butt]");
			Render.text(" happily.  For some reason, the idea of denying yourself release in exchange for more humiliation is appealing to you.");
			...
			if (dog) Render.text("whining");
			else Render.text("mewling");
			Render.text(" like a needy animal.");
			...
			if (select == 0) {
				...
				//{Intimidated Holli}
					...
					Render.text("\n\n\"<i>");
					if (dog) Render.text("Arf!?");
					else Render.text("Meow!?");
					Render.text("</i>\"");
					Render.text("\n\nHolli roughly slaps the back of your head with a tentacle in a crude approximation of petting.  \"<i>That's a good ");
					if (dog) Render.text("doggie");
					else Render.text("kitty");
					Render.text(",</i>\" she says, thrilled to see you so debased and no longer concerned with obeying you.  The shame is more potent than a minotaur's spunk, and when combined with the magic of your demonic 'collar', it twists in your head, twining inexorably with arousal to make you leak even harder.  It feels soooo good to wallow in the piteous, condescending touches.");
				...
				if (player.lowerBody.cockSpot.hasCock()) {
					...
					Render.text("your rebellious boner");
					if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
					Render.text(", which has the unintended side-effect of making it puff up even harder");
				}
				else Render.text("your rebellious clitoris, which has the unintended side-effect of making it and your drooling cuntlips puff up, redder than ever before");
				Render.text(".  ");
				if (player.lowerBody.cockSpot.hasCock()) Render.text("It bounces against your tummy");
				else Render.text("Your vaginal muscles clench and contract");
				Render.text(" with unfilled need, but there's no sating it.  Only after all your muscles are sore and exhausted does Ceraph take you home.  You collapse onto your belly, panting while the whip is untied.  When you turn to talk to Ceraph, you find she's left you there, aroused beyond measure.");
			}
			//Corrupt Jojo or Amily
				...
				if (amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2 && campCorruptJojo()) {
					...
					Render.text("Before long, your corrupt, mouse-like slave sees you awkwardly clambering after the purple-hued dominatrix.  ");
					if (jojo) Render.text("He");
					else Render.text("She");
					Render.text(" asks, \"<i>[Master], what are you doing?</i>\"");
					Render.text("\n\nCeraph looks down at you with her hand tight on your leash.");
					...
					if (!jojo) Render.text("cum-slut is");
					else Render.text("I guess I'm");
					Render.text(" her pet too...</i>\" ");
					if (!jojo) Render.text("Amily");
					else Render.text("Jojo");
					Render.text(" says, dropping to all fours and crawling up alongside you.  Ceraph tears a strip of their clothing off nonchalantly and fixes it into another improvised collar that she ties on to her whip.  The three of you then continue the walk, though you're scolded again and again when you and ");
					if (!jojo) Render.text("Amily");
					else Render.text("Jojo");
					Render.text(" rub up against each other.  Having a ");
					if (!jojo) Render.text("sister");
					else Render.text("brother");
					Render.text("-pet is somewhat comforting, but being constantly exposed to ");
					if (jojo) Render.text("his");
					else Render.text("her");
					Render.text(" sexual scent is more than distracting.  Ceraph squirts the two of you with water a half-dozen times to keep you from fucking, and each time the shame is greater.  You really are a pet, one that can't even keep " + player.mf("his", "her") + " most basic needs in check.");
				}
				//Mice finisher
				Render.text("\n\nYou go for an exhausting walk around the nearby wasteland with your companion");
				if (amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2 && campCorruptJojo()) Render.text("s");
				Render.text(", and you thoroughly enjoy the degrading experience.  Lust and humiliation are all jumbled up inside you into one big mass of sex, such that getting turned on from this humiliates you, and being humiliated gets you even more aroused");
				Render.text(".  It's an endless feedback loop of sexual excitement that has you about to blow, except you're back in camp.  The collar is untied, and your owner's sweet voice whispers, \"<i>Good walk, pet.</i>\" before vanishing into the wind.");

				Render.text("\n\n");
				if (campCorruptJojo()) {
					Render.text("Jojo");
					if (amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2) Render.text(" and ");
				}
				if (amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2) Render.text("Amily");
				if (amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2 && campCorruptJojo()) Render.text("look around uncertainly before darting back to their usual places, knowing you'll give the command if you want them to take care of your desires.");
				else Render.text("looks around uncertainly before darting back to the usual places, knowing you'll give the command if you want a slave to take care of your desires.");
			}
			//Generic
			else {
				...
				if (player.lowerBody.cockSpot.hasCock()) Render.text("goblins");
				else Render.text("imps");
				Render.text(" seem intrigued by the sight of your compulsively ");
				if (player.lowerBody.cockSpot.hasCock()) Render.text("erect maleness");
				else Render.text("quivering quim");
				if (player.lowerBody.cockSpot.count() > 1) Render.text("es");
				Render.text(", and often offer Ceraph a bounty of gems to fuck you.  Your mistress insists that you don't need any such sexual release, because you are a good ");
				if (dog) Render.text("doggie");
				else Render.text("kitty");
				Render.text(" and get all you need out of obedience.  Her words draw an exaggerated whimper from your lips along with a few fresh drops of pearly ");
				if (player.lowerBody.cockSpot.hasCock()) Render.text("cock");
				else Render.text("cunt");
				Render.text("-cream, but you hold still, [legs] quaking eagerly.  It feels like all it would take to get your release would be a few soft touches on your aching ");
				if (player.lowerBody.cockSpot.hasCock()) Render.text("cock");
				else Render.text("nethers");
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
				Render.text(".");
				...
				Render.text(" even notices your state and comments on it, holding ");
				if (player.lowerBody.cockSpot.hasCock()) Render.text("her");
				else Render.text("his");
				Render.text(" hand beneath your freely drooling ");
				if (player.lowerBody.cockSpot.hasCock()) Render.text("erection");
				else Render.text("pussy");
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
				Render.text(" to collect a palmful of slippery ");
				if (player.lowerBody.cockSpot.hasCock()) Render.text("boy");
				else Render.text("girl");
				Render.text("-honey.  ");
				if (player.lowerBody.cockSpot.hasCock()) Render.text("She");
				else Render.text("He");
				Render.text(" holds it up in front of her nose and sniffs it, sighing, \"<i>" + player.mf("He", "She") + " certainly does seem to be well tamed, miss demon.  You're lucky you caught " + player.mf("him", "her") + " before I did.  With ");
				if (player.lowerBody.cockSpot.hasCock()) Render.text("cum this nice, I'd be milking " + player.mf("him", "her") + " dry every time " + player.mf("he", "she") + " could get it up");
				else Render.text("a scent like this, I'd be fucking " + player.mf("him", "her") + " every chance I could get");
				Render.text(".  Maybe you'll change your mind sometime.</i>\"");
				...
				if (player.lowerBody.cockSpot.hasCock()) Render.text("pre-cum rockets");
				else Render.text("fem-juice spurts");
				Render.text(" onto the ground while you whine softly.  Your owner notices and commands, \"<i>Now, pet, we can't have you making this ");
				if (player.lowerBody.cockSpot.hasCock()) Render.text("goblin");
				else Render.text("imp");
				Render.text("'s home a mess.  Lick it up.</i>\"");
				...
				if (player.lowerBody.cockSpot.hasCock()) Render.text("pre-ejaculate");
				else Render.text("lady jizz");
				Render.text(" landed.  \"<i>Good enough.  Come on, ");
				if (dog) Render.text("doggy");
				else Render.text("cat");
				Render.text(", let's finish our walk!</i>\"");
				...
				if (player.lowerBody.cockSpot.hasCock()) Render.text("boner");
				else Render.text("twat");
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
				if (player.lowerBody.cockSpot.count() > 1) Render.text(" feel like they");
				else Render.text(" feels like it");
				Render.text(" could go off at any second.  The journey is murder for your poor, ");
				if (player.lowerBody.cockSpot.hasCock()) Render.text("over-inflated maleness");
				else Render.text("puffy, aching cleft");
				if (player.totalCocks() > 1) Render.text("es");
				Render.text(", but you somehow make the whole way home without ");
				if (player.lowerBody.cockSpot.hasCock()) Render.text("popping");
				else Render.text("tipping over the edge");
				Render.text(".  The trail of musky slime that clearly marks your journey is another thing though.");
