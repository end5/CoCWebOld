...
//Is Ceraph a follower?
		...
		{
			return flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00286] > 0 || flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00287] > 0;
		}
		...
		public ceraphFollowerEncounter(forceCeraph:boolean = false):void
		{
			...
			if (rand(24) == 0 && player.lowerBody.cockSpot.hasCock())
				catgirlEncounter();
			else if (rand(24) == 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00293] == 0)
...
//[Actually Ceraph] - 
		public ceraphFollowerAppearance(output:boolean = true):void
		{
			...
			if (output) {
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("  Her demonic prick forms a ridge in the tight fabric, its head peeking ever-so-slightly over the top", false);
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) dickToggle = "Go Female";
			else dickToggle = "Go Herm";
			let gainFetish:Function =null;
			let loseFetish:Function =null;
			if (flags[FlagEnum.PC_FETISH] < 3) gainFetish = CeraphHandsOutNewFetishesLikePervCandy;
			if (flags[FlagEnum.PC_FETISH] > 0) loseFetish = unfetishifyYourselfWithFollowerCeraph;
			...
			if (flags[FlagEnum.FOLLOWER_AT_FARM_CERAPH] == 0 && flags[FlagEnum.FARM_CORRUPTION_STARTED] == 1) MainScreen.addButton(1, "Farm Work", helpWithFarm);
		}
		...
		private doFarmShit():void
		{
			...
			flags[FlagEnum.FOLLOWER_AT_FARM_CERAPH] = 1;
		...
		private ceraphSexMenu():void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina() && player.lust >= 33) {
				MainScreen.text("You could make her lick your pussy.  ", false);
				femaleFuck = followerCeraphTongueFucking;
			}
			if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina() && player.lust >= 33) {
		...
		private followerCeraphRoleplay():void
		{
			...
			if (flags[FlagEnum.TIMES_FUCKED_URTA] > 0 && (player.lowerBody.cockSpot.hasCock() || player.lowerBody.vaginaSpot.hasVagina()) && player.lust >= 33) urta = ceraphUrtaRoleplay;
			if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(70) >= 0 && player.statusAffects.has("Marble") && player.lust >= 33) marbles = sweetieNOOOO;
			if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00150] > 0 && player.lust >= 33 && player.lowerBody.cockSpot.hasCock()) dominika = cerminika;
...
//Collar Ceraph After 4th Defeat + Rape: (Zeddited)
		private collarCeraph():void
		{
			...
			MainScreen.text("  Ceraph pulls herself up to her knees and kisses your " + player.feet() + ", a show of absolute submission and obedience.\n\n", false);
...
//[Carry]
		private carryCarephsToken():void
		{
			...
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00287] = 1;
...
//*Decision to Display Demonic Dick or Demur (pretty sure Fen mentioned wanting this -Z)
		private cawkTawgle():void
		{
			...
			spriteSelect(87);
			//Off
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) {
				...
				flags[FlagEnum.CERAPH_HIDING_DICK] = 1;
			}
			else {
				...
				flags[FlagEnum.CERAPH_HIDING_DICK] = 0;
			}
...
//Volunteer for new fetishes!
		private CeraphHandsOutNewFetishesLikePervCandy():void
		{
			...
			if (flags[FlagEnum.PC_FETISH] == 0) {
				...
				//(PIERCED) 
				else MainScreen.text("The demon rubs your ears in her hands, numbing them slightly.  A gradual buzz builds behind your eyes, accompanied by a wave of dizziness.  You blink and try to shake your head, but as numb as you are, it's quite difficult.  After a few moments, the odd sensations pass, returning normal feeling to your ears and " + player.face() + ", much to your relief.  ", false);
				MainScreen.text("You hope she doesn't take your " + player.armorName + " while you're paralyzed, leaving you to roam the realm totally exposed.  Confusion and waves of new desire battle in your mind as you try to come to grips with the odd thought.\n\n", false);
				...
				flags[FlagEnum.PC_FETISH] = 1;
				dynStats("lus", 25, "cor", 1);
			}
			//*Fetish level = 1
			else if (flags[FlagEnum.PC_FETISH] == 1) {
				...
				if (player.nipplesPierced > 0) MainScreen.text("your " + nippleDescript(0) + "s, circling her fingers all around the fleshy nubs.  Goosebumps run over your body in a wave, accompanied by a similar chill and a pressure behind your temples.  You shudder, but it quickly fades.", false);
				else MainScreen.text("aligning the business ends of the piercings with your sensitive nipple-flesh.  Your right " + nippleDescript(0) + " is pierced in one smooth motion, nearly making you scream in pain.  As she fastens it on, you feel goosebumps spread over your body in a wave.  The second piercing doesn't seem to hurt as bad, but the sensation of spreading goosebumps is far more noticeable.", false);
			...
			//*Fetish level = 2
			else if (flags[FlagEnum.PC_FETISH] == 2) {
				...
				if (player.totalCocks() > 0) {
					...
					MainScreen.text("your " + multiCockDescriptLight() + ".", false);
				}
				else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("feminine moisture drools from between your lips and your " + clitDescript() + " turns into a hard button.", false);
				...
				//[dicks]
				if (player.totalCocks() > 0) {
					MainScreen.text("grabs your " + cockDescript(0), false);
					if (player.lowerBody.cockSpot.list[0].pierced > 0) MainScreen.text(", the old piercing clattering to the ground as it slides out, ", false);
					MainScreen.text(" and snaps the diamond stud through your sensitive flesh, making your vision haze red in pain.\n\n", false);
					player.lowerBody.cockSpot.list[0].pierced = 1;
					player.lowerBody.cockSpot.list[0].pShortDesc = "diamond cock-stud";
					player.lowerBody.cockSpot.list[0].pLongDesc = "Diamond cock-stud";
				}
				//[cunts]
				else if (player.lowerBody.vaginaSpot.hasVagina()) {
					MainScreen.text("spreads your lips", false);
					if (player.vaginas[0].clitPierced > 0) MainScreen.text(", the old piercing clattering to the ground as it slides out of your flesh, ", false);
					MainScreen.text(", getting ahold of the flesh around the base of your " + clitDescript() + ".  With practiced ease, she snaps the piercing closed, attaching the diamond stud to you while the pain fuzzes your vision red.\n\n", false);
...
//*Ceraph Actually Removes The Fetish (Zeddited)
		private goThroughWithCeraphUnfetishification():void
		{
			...
			if (flags[FlagEnum.PC_FETISH] == 3) flags[FlagEnum.PC_FETISH] = 2;
			else if (flags[FlagEnum.PC_FETISH] == 2) flags[FlagEnum.PC_FETISH] = 1;
			else if (flags[FlagEnum.PC_FETISH] == 1) flags[FlagEnum.PC_FETISH] = 0;
...
//*Fuck Ceraph's Pussy (Zeddited)
		private fuckFollowerCeraphsVagoo():void
		{
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text(" and half-erect, throbbing cock", false);
			MainScreen.text(".  ", false);
			if (player.stats.cor < 33) MainScreen.text("She gets down on all fours, then lays her cheek in the dirt.  Her arms cross behind her back, suddenly bound there by the abrupt appearance of those same panties, tied in a knot.  Supported by only her knees, her tits, shoulders, and her face, she asks, \"<i>Grab me and fuck me " + player.mf("Master", "Mistress") + ", please grind your slave's face in the dirt!</i>\"\n\n", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("and her dick releasing a squirt of pre-cum ", false);
			...
			if (player.str < 60) MainScreen.text(" and heave, lifting her up off the ground and forcing her to straddle you.", false);
			else MainScreen.text(" and easily lift her, forcing her to straddle you.", false);
			MainScreen.text("  Dragging the demon back, you bring her slutty, sodden puss up to your " + cockDescript(x), false);
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text(", ignoring the rope of dangling pre-cum that hangs from her bumpy prick.", false);
			else MainScreen.text(", feeling the heat washing off her mons and onto your " + player.cockHead(x) + ".", false);
			MainScreen.text("  Yanking back, you bury all " + num2Text(Math.round(player.lowerBody.cockSpot.list[x].cockLength)) + " inches of your " + cockDescript(x) + " into Ceraph's unholy, warm snatch, ", false);
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("Her cock dangles towards the ground, dripping thick ropes of pre-spunk as readily as her pussy.  ", false);
			MainScreen.text("Somehow, she feels as tight around your " + cockDescript(x) + " as any virgin and three times as wet.  The gentle claps of genital against genital send ripples through Ceraph, smearing her cheek in the growing mud-puddle while her tits wobble dangerously inside the sheer latex bra.\n\n", false);
			...
			//(DOUBLE PENN!) 
			if (y >= 0) {
				MainScreen.text("A wicked idea crosses your mind, and you reach down to grab your " + cockDescript(y) + ", aiming it at the flexing asshole just above the slave's squelching snatch.  Your next thrust plows it deep into Ceraph's anus, thankfully lubricated by the demon's constant squirting.  Growling at the mounting, doubling pleasure, you resume your tempo and slam both your cocks hilt-deep in Ceraph.  ", false);
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("She squeals and sprays a thick rope of jizz from her bloated cock.  It seems a little prostate pressure was all it took to put her maleness over the edge.  ", false);
			...
			//(Single Penn!) 
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text(" while her cock spews ropes of pre-cum into the dirt", false);
			...
			MainScreen.text("With a shuddering explosion of warmth, you cum.  Ceraph groans loud and low, her pussy happily caressing your " + cockDescript(x) + " as it spews its potent load into her demonic womb.  ", false);
			if (y >= 0) MainScreen.text("Her asshole likewise gleefully devours the seed from your " + cockDescript(y) + ", flexing wildly from the fluid injection.  ", false);
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text(", and her cock starts pumping more demonic spunk into the mud", false);
			MainScreen.text(".  It trails off, though her pussy continues teasing your " + cockDescript(x) + ", wringing the last of your seed from your " + ballsDescriptLight() + ".\n\n", false);
...
//*Ceraph TongueFucks The PC (Zeddited)
		private followerCeraphTongueFucking():void
		{
			...
			MainScreen.text("Desirous of being pleasured by your demonic slave, you spread out your " + player.legs() + " to allow easy access to your " + vaginaDescript(), false);
			...
			if (player.lust < 50) MainScreen.text("still-hooded", false);
			...
			if (player.wetness() < 3) MainScreen.text("moist", false);
			else if (player.wetness() < 4) MainScreen.text("wet", false);
			...
			if (player.wetness() >= 5) MainScreen.text(", even though her cheeks are bulging and her throat struggles to devour all of the fountaining girl-spunk.", false);
			else if (player.wetness() >= 4) MainScreen.text(", even though her cheeks are slightly bulged and she's gulping it down.", false);
			else if (player.wetness() >= 3) MainScreen.text(", even though she has to gulp it down from time to time.", false);
			else if (player.wetness() >= 2) MainScreen.text(", even though you produce enough for her to gulp.", false);
			...
			MainScreen.text("You pull her back to your " + vaginaDescript() + " to lick the last of your lady-spunk from your nethers, then send her on her way with a smile on your face.  Your expression widens when you see Ceraph stagger, still a bit shaky from her own orgasm.", false);
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00289]++;
			player.orgasm();
			dynStats("sen", -2 ,"cor", .25);
			doNext(camp.returnToCampUseOneHour);
		}
...
//*Ceraph goes full tentacle and double penetrates herms (+ standard dick BJ if ceraph dick toggle is on) (Zeddited)
		private ceraphTentacleGrape():void
		{
			...
			MainScreen.text(".  Even better, there are what feels like thousands of wriggling cilia squirming in the syrupy tunnel, each of them caressing and licking at " + sMultiCockDesc() + " repeatedly.  Like thousands of hungry tongues, they seem to set off every nerve in your " + multiCockDescriptLight() + ", nearly making you forget the rhythmic, pulsating suction of the tendril as it fellates you.\n\n", false);
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00290] == 0) {
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("Ceraph's cock sprays cum all over her belly as her climax winds on, her poor, ignored prick blasting seed with reckless abandon.\n\n", false);
...
//[Punish Her]
		...
		{
			spriteSelect(87);
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00290] = 1;
			MainScreen.text("", true);
...
//SEX!
		private fuckCeraphsCatgirls():void
		{
			...
			MainScreen.text("You smile at them and say that you'd be happy to have them for your pleasure; their horns suggest that they'll be quite a trip.  The two purr happily and instruct you to lie on your back to start the fun.  You relax as directed and the cat slaves unfasten your " + player.armorName + " from your body.  As they work, they make sure to gently stroke every inch of your newly exposed flesh with their soft furry hands as it's revealed; all the while moving closer and closer to your most personal parts.  When " + oMultiCockDesc() + " finally tastes the air, it gets even more attention.\n\n", false);

			//purrfect tit fuck
			MainScreen.text("You can't help but put your hands to their heads and start rubbing and scratching them behind their ears.  Suddenly, one of them steps back as the other moves down in front of your " + cockDescript(x) + " and pulls it inside her breasts.  In response, both your hands end up on her head, forcing it down onto your cock, and you feel your manhood start to vibrate as the catgirl begins to purr.  You cry out in pleasure at your shaft being massaged by soft-furred breastflesh while she hums into the tip.\n\n", false);
			...
			if (player.biggestLactation() > 1) MainScreen.text("  When some of your milk seeps out, she leans forward and latches onto a nipple eagerly, alternating between sucking on the tip and licking the drops off of it.", false);
			else {
			...
			if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("sliding her hand between the furry tits, down the base of your " + cockDescript(x) + ".", false);
			...
			MainScreen.text("Thanks to all the stimulation from before, their expert tongues almost immediately bring your body to a shuddering orgasm, and " + sMultiCockDesc() + " ", false);
...
//[Drunk] (female/herm only. No centaurs)
		...
		private ceraphUrtaRoleplayDrunk2():void
		{
			...
			if (player.isGoo() || player.isNaga()) MainScreen.text("wriggle to a wider stance", false);
...
//[Sober]
		private ceraphUrtaRoleplaySober():void
		{
			...
			if (player.horns > 0) MainScreen.text("  Unable to keep her hands at her sides, but unwilling to stand between your mouth and her slit, the fox-girl takes hold of your horns, pulling your face tightly against her mound, her chest tight with a barely audible squeak of delight. Stroking the tip of your tongue at the curtain of her sex, you allow her the barest trace of penetration before drawing back and placing a wet kiss on her clit. Enough foreplay.", false);
			dynStats("lus", 200);
			//[Next]
...
//[=Yes=]
		private ceraphLackeyCorruption():void
		{
			...
			if (!player.lowerBody.cockSpot.hasCock()) {
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("your " + clitDescript() + " swells", false);
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				if (player.lowerBody.cockSpot.list[x].cockType == CockType.DEMON) demon = true;
			}
			//(else if PC's cock is below cock area 48)
			if (player.cockArea(player.biggestCockIndex()) < 48 && player.lowerBody.cockSpot.hasCock()) {
				MainScreen.text("Your " + cockDescript(x) + " throbs, veins bulging as it grows larger, ballooning to a generous 20-inch long, 3-inch thick size.  ", false);
			}
			...
			MainScreen.text(player.legs() + ".  The imps on her extremities laugh at the girl as she relaxes and her head slumps into the ground; you motion for the imps to release her and step back, then align yourself with her pussy.\n\n", false);
...
//[=Never Again - Fuck this nerd shit=]
		private iQuitCeraphCorruptionDemons():void
		{
			...
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00293] = 1;
			doNext(camp.returnToCampUseOneHour);
		}
...
//(should probably add a cock-limit of like, whatever you want, cuz you're fucking her butt)
		private sweetieNOOOO():void
		{
			...
			//([if first time]
			if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00294] == 0) MainScreen.text("Seeing your confused stare, she simply answers with, \"<i>Ah, do be patient... sweetie,</i>\" and goes back to her work.", false);
			...
			if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00295] == 1) doNext(postUdderChoice);
			else {
...
//[in a pig's eye, pal]  
		private noUdderPlz(perm:boolean = false):void
		{
			MainScreen.text("", true);
			spriteSelect(41);
			if (perm) flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00295] = 1;
			...
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00296] = 0;
			postUdderChoice();
		}
...
//[of course honey-buns] 
		private yesUdderPWEASE():void
		{
			...
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00296] = 1;
			postUdderChoice();
		}
		...
		private postUdderChoice(newl:boolean = false):void
		{
			...
			//([if udder] 
			if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00296] == 1) MainScreen.text(", making sure to prep four extra tubes for her udder", false);
			...
			//([if udder] 
			if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00296] == 1) MainScreen.text("  The four others quickly follow, the udder-cups sucking onto the nubs like hungry children.", false);
			MainScreen.text("\n\n", false);
			...
			if (player.lowerBody.balls > 0) MainScreen.text("your " + ballsDescriptLight() + " tightening", false);
...
//After that is a scene tailored more specifically for the PC. Intention is to give nagas and centaurs and all that their own scene. Currently only scene is for PCs with a dick - should be naga-compatible.
		private cerminika():void
		{
			...
			MainScreen.text("", true);
			//[first time]
			if (flags[FlagEnum.CERAPH_ROLEPLAY_AS_DOMINIKA_COUNT] == 0) {
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text(" while her cock shrinks to what looks approximately like six inches of meat, an outright tiny penis for Mareth", false);
			MainScreen.text(".\n\n", false);
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				...
				//[CERAPH COCK: 
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("  Her dick presses up into your crotch.", false);
				...
				MainScreen.text("\"<i>But if I can rely on you – perhaps I do not need to worry.</i>\"  She pulls your " + player.armorName + " away, letting her hands roam hungrily across your body.  Blatantly turned on, her pussy rubs its juices against your crotch", false);
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text(", and her prick has remained firm and hard", false);
				MainScreen.text(".  Her tongue roams hungrily over her inky lips, and an almost predatory grin sets across her features as she pulls up your " + cockDescript(0) + ".  Her fingernails cup under your head delicately and carefully tease you, rubbing her palm into the underside of your shaft.\n\n", false);

				MainScreen.text("\"<i>How long,</i>\" she purrs, \"<i>Have you wanted to really be inside me?</i>\"\n\n", false);

				MainScreen.text("Dominika slides her body back, rubbing the softness of her folds against your " + cockDescript(0) + ".  ", false);
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("The very base of her prick teases the veins of your own shaft, but she seems focused on the pleasure of her cunt for now.  ", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("  The degree that her cock abruptly shoots straight up is perhaps the most amusing part of the act.", false);
				MainScreen.text("  You simply smirk at her, starting to bounce her into your lap and shoving your " + cockDescript(0) + " deeper inside. Once more she bites her lip, fingers digging into your stomach. No matter how classy she wants to appear, you can quite easily feel how wet she is", false);
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text(", and the heat of her prick bouncing into your lap", false);
				...
				MainScreen.text("You give her a carefully crafted retort that demonstrates your intellectual superiority, but she just gives you a fake smile and pats your cheek.  Eyes half-closed she slowly slides herself along you, enjoying the sensation of each inch of your " + cockDescript(0) + " as it brushes along her walls.  \"<i>Mmmm,</i>\" she lets herself purr out, \"<i>Worth... the wait.</i>\"  Her toes curl slightly, one of her legs shifting back slightly to alter the angle you enter her at.  It seems to work how she wants it to, as a deep, satisfied breath accompanies the next downward slide of her hips.", false);
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("  A single bead of pre-cum forms slowly at the tip of her prick, growing larger until it finally slips down into the curve of her head, sliding down her shaft and into your lap.", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("Her prick bounces between her belly and yours as you take her.  ", false);
				MainScreen.text("For your part you have no interest in slowing down, gripping back onto her hips and pounding away at the delightfully tight cunt presently dominated by your " + cockDescript(0) + ".  More in charge than you were before, you have a chance to truly appreciate the sensation of her labia being shoved apart for your needs.  Absolutely soaked with her needy juices, it's easy to penetrate the sorceress.  Forcing so much of her sopping nectar out is making a terrible mess of her thighs, but given how stuck up she's been about finally letting you take her, she could stand to get a little messy.  Her fingernails claw at your collarbone", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("A light spurt of pre-cum from her prick warms the flesh where your hips meet.  ", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("  You can practically hear her rock-hard cock kicking up the ground.", false);
				...
				MainScreen.text("She wails in mixed frustration and pleasure, juices splattering against the ground as her hungry cunt grows hornier.  A darkness overtakes her eyes, and her nails grow long and sharp, and through it all your " + cockDescript(0) + " continues to degrade her ass.  As though a heartbeat pumping blood through her body, each thrust spreads the color over her skin a little more.  Her feet push and scrabble lightly against the ground in tormented frustration, but she is helpless beneath you.  You go without stopping, only slowing when wings unexpectedly burst from her shoulder blades, dark and starry.\n\n", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("  Her prick has become engorged and infernal, covered with abnormal bumps and veins. It practically spits up pre-cum, gurgling with a greatly increased supply.", false);
				...
				else MainScreen.text("  She shudders as your " + cockDescript(0) + " fills her guts with warmth, hips seizing as she rides out her assgasm.", false);
				MainScreen.text("  Her cunt nearly waterfalls her juices, squirting obscenely as she rides out the pleasure.", false);
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("  The demonic prick placed between her legs stops bouncing only because of how rock-hard it is as it spews her own filthy fucksludge across her body, splattering onto her tits and across her face.  The volume of off-white cream it spews out takes even longer to finish than you do, painting her obscenely.", false);
				MainScreen.text("\n\n", false);
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("  Her own filthy jizz has already been licked clean by her hungry mouth, so as not to dirty your handsome prick.", false);
				MainScreen.text("  The demoness laps at your " + cockDescript(0) + ", cleaning it of spunk, sweat, and all the various filth it accumulates, sucking and dragging her fat, midnight lips across them.  Dominika purrs her happiness, her joy at being forced into such a whorish form.\n\n", false);
			...
			flags[FlagEnum.CERAPH_ROLEPLAY_AS_DOMINIKA_COUNT]++;
			doNext(camp.returnToCampUseOneHour);
		}
...
//▶[FuckVagina] requires penis
		private fuckZetsukosTonguepussy():void
		{
			...
			if (player.lowerBody.balls > 0) MainScreen.text("slithering across your " + ballsDescriptLight() + ", ", false);
			...
			MainScreen.text("teasing your " + assholeDescript() + " gently.  You gather up a mound of her soft tit flesh in each hand and begin squeezing along them lustfully, pushing on her pierced nipples with your thumbs gently while her tongue continues to lick and pleasure everything within its reach.\n\n", false);
			...
			MainScreen.text("With the way her pussy and tongue are squeezing you now, it would be nearly impossible not to indulge her request.  Your " + cockDescript(x) + " twitches inside her, releasing a creamy deluge that causes Zetsuko to begin drooling from both ends.  \"<i>Oooh...  so good, so thick, so creamy...  Fill Zetsuko up, yes!</i>\"", false);
...
//▶[GetTongued] requires vagina
		private getTonguedByZetsuko():void
		{
			...
			if (x >= 0) {
				MainScreen.text("In a somewhat impressive feat of contortion, Zetsuko brings her feet in toward her groin to stimulate your " + multiCockDescriptLight() + ", lifting ", false);
				...
				if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s", false);
				MainScreen.text(" with the soles of her feet.  Her toes are surprisingly dextrous, able to curl around and grip your " + cockDescript(x) + " lightly, gently pinching and tugging at the skin just under the head and manipulating your throbbing member", false);
				if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s", false);
			...
			MainScreen.text("Grabbing hold of her thighs, you begin to rock yourself back and forth across her tongue, moaning powerfully as you feel it begin to thrust in and out of you now.  The numerous studs that line the center of the organ begin to press firmly against the roof of your " + vaginaDescript(0) + ", stimulating your G-spot and flicking your " + clitDescript() + " each time her tongue dives back inside you.  Your fingers dig into her flesh like a vice now as you grind yourself down on the indecent tendril, moaning salaciously as the waves of your climax begin to overtake you.\n\n", false);

			MainScreen.text("Your " + vaginaDescript(0) + " cinches tight around Zetsuko's tongue, and it continues to wriggle inside you while you ride out your orgasm.", false);
			if (x >= 0) {
				MainScreen.text("  " + SMultiCockDesc() + " twitches powerfully between her feet, ", false);
			...
			if (player.wetness() >= 4) MainScreen.text(" and overflowing to dribble down her bountiful ass cheeks and stomach", false);
...
//▶[Banana?] silly mode
		...
		private ceraphFollowerCockTaking(smallest:boolean = false):void
		{
			...
			if (player.lowerBody.cockSpot.list[x].cockThickness >= 4) MainScreen.text(", compressing your massive girth into the rigid shaft", false);
			MainScreen.text(", slowly closing.  It should hurt but it doesn't, it just feels warmer and warmer, and then with a sudden 'PYOING!', Ceraph is holding your " + cockDescript(x), false);
			...
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00218]++;
		...
		private ceraphFollowerCuntTaking():void
		{
			...
			player.orgasm();
			dynStats("lib", -2, "sen", -2, "cor", 5);
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00219]++;
			doNext(camp.returnToCampUseOneHour);
		}
		...
		private ceraphFollowerTitTaking(rowNum: number = 0):void
		{
			...
			if (player.upperBody.chest.list[x].fuckable) flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00217] = 4;
			else if (player.lactationQ() >= 100) flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00217] = 5;
			else flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00217] = 3;
			...
			if (x < player.upperBody.chest.count() - 1 && player.upperBody.chest.count() > 1) {
				//If only 1 row below
				if (x >= player.upperBody.chest.count() - 2) MainScreen.text("your torso has shifted and your " + breastDescript(x + 1) + " have moved up to fill the void.", false);
				//Multiple below
			...
			if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00217] == 3) MainScreen.text("rolls the nipples in her hands", false);
			else if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00217] == 4) MainScreen.text("pushes her fingertips inside the leaky nipple-cunts", false);
			...
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00220]++;
			doNext(camp.returnToCampUseOneHour);
		}
...
//NippleCunt Stuffing (Ceraph grows dick-nipples to plow your lusty twats!)
		private stuffSomeNippleCunts():void
		{
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text(" and proud demon-dick");
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("  The fetishistic slut's sloppy cock is dribbling on your hip, neglected and aroused.  Poor thing.");
			...
			if (player.bRows() > 1) {
				...
				if (player.bRows() == 2) MainScreen.text("another row of female flesh begins");
			...
			if (player.averageNipplesPerBreast() > 1) MainScreen.text("  She strokes them, one by one, and they split down the middle, dividing into a multitude of penises, a perfect match to your quad-nipplecunts.");
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text(", her cock spouting its own ivory wetness onto your belly in sympathetic release.");
...
//Portal Fuck (AKA Ceraph Hung Out With Cinnabar, Req's PC dick)
		private portalFuckWithFollowerCeraph():void
		{
			...
			let y: number = x + 1;
			//FIRST TIME:
			if (flags[FlagEnum.TIMES_CERAPH_PORTAL_FUCKED] == 0) {
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 1) MainScreen.text("  Of course, you don't seem to like that appendage, so my ring won't get much use today, will it?");
				MainScreen.text("</i>\"");
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 1) MainScreen.text("  Ceraph twirls one of the rings into her palm, where it promptly vanishes in a puff of purple smoke.");
				...
				if (flags[FlagEnum.CERAPH_HIDING_DICK] == 1) MainScreen.text("  She sadly teleports one of them away, knowing you've commanded her to stay female for you.");
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text(" and eager cock");
			...
			if (flags[FlagEnum.TIMES_CERAPH_PORTAL_FUCKED] == 0) MainScreen.text("  Well, your dick doesn't look any different, aside from the gilden adornment stuck near the bottom.  Now, if it were hilt-deep in the demon's cunt, that would be something!");
			...
			player.orgasm();
			dynStats("cor", 1);
			flags[FlagEnum.TIMES_CERAPH_PORTAL_FUCKED]++;
			doNext(camp.returnToCampUseOneHour);
		}
		...
		private layEggsInSlaveCeraph():void
		{
			...
			MainScreen.text("You ");
			if (!player.isTaur()) MainScreen.text("absently rub the sensitive bulge in your abdomen as you ");
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("  The whole garment creaks and strains under the pressure of her hardening member.");
			...
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("; unable to keep it penned up, her demonic cock spills out the side, waving back and forth obscenely as you saw the offending garment into her crotch");
			MainScreen.text(".");
			...
			if (!player.isTaur()) MainScreen.text(", and your fingers tremble slightly as you trace its segmented length, leaving a trail of lubricating mucus from tip to base");
...
//[Either Once]
		private barkToCeraphOnce(dog:boolean = true):void
		{
			...
			if (player.tailType > TAIL.NONE) MainScreen.text(", wagging your tail excitedly");
			MainScreen.text(".");
			MainScreen.text("\n\nCeraph casually pulls her panties a bit lower to ");
			if (flags[FlagEnum.CERAPH_HIDING_DICK] == 0) MainScreen.text("ease the pressure on her tightly-contained phallus");
			...
			else if (player.ass.analLooseness >= 4) MainScreen.text("The cunt-slick tool has no problem wiggling inside your well-used and abused asshole, hilted in a split second, the hundreds of nubs gently caressing your anal ring like a lover as they nestle inside you.");
...
//[Either Twice]
		private barkOrMeowTwiceToCeraph(dog:boolean = true):void
		{
			...
			if (amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2)
			...
			if (select == 0) {
				//{Normal Holli}
				if (flags[FlagEnum.HOLLI_SUBMISSIVE] == 0) {
				...
				//{Intimidated Holli}
					...
					flags[FlagEnum.HOLLI_SUBMISSIVE] = 0;
				}
				//Either Holli ending:
			...
			//Corrupt Jojo or Amily
				...
				//Both
				if (amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2 && campCorruptJojo()) {
				...
				if (amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2 && campCorruptJojo()) MainScreen.text("s");
				...
				if (campCorruptJojo()) {
					...
					if (amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2) MainScreen.text(" and ");
				}
				if (amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2) MainScreen.text("Amily");
				if (amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2 && campCorruptJojo()) MainScreen.text("look around uncertainly before darting back to their usual places, knowing you'll give the command if you want them to take care of your desires.");
			...
			//Generic
				...
				if (player.totalCocks() > 1) MainScreen.text("es");
