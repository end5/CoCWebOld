...
public function followerSophieMainScreen():void {
	...
	if(flags[FlagEnum.SOPHIE_CAMP_EGG_COUNTDOWN] > 0 && flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) outputText("\n\n<b>Sophie's egg is sitting nearby.</b>");
...
private function sleepWithSophieToggle():void {
	...
	if(flags[FlagEnum.SLEEP_WITH] != "Sophie") {
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) {
			...
			if(player.smallestCockArea() <= 5 && player.lowerBody.cockSpot.hasCock()) outputText(", but don't be surprised if I go a little rough on that tiny dick of yours in the morning");
			outputText("!</i>\" she calls.");
			if(player.smallestCockArea() <= 5 && player.lowerBody.cockSpot.hasCock()) outputText("\n\n<b>If you don't want her to tease you about your small penis, you had better tell her to stop sleeping with you.</b>");
		}
		//NORMAL
		else {
			...
			if(player.smallestCockArea() <= 5 && player.lowerBody.cockSpot.hasCock()) outputText("  \"<i>You won't mind me teasing your cute little dick in the mornings, will you [name]?  It's so cute...</i>\" she muses.\n\n<b>If you don't want her to tease you about your small penis, you had better not sleep with her.</b>");
		}
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] == 0) outputText("Well, it is your bed.  Let me know if you want me coming back for snuggles, okay?");
...
private function fuckFollowerSophie():void {
	...
	if(player.stats.int < 50) outputText("\n\nAbruptly");
...
private function sophieFollowerGivesBlowjobs():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText(", right between your [chest]");
	outputText(", and she slowly trails it up to your chin, which she gently grabs in her long-nailed fingers.  \"<i>");
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) outputText("You may have given me a bimbo's body and an insatiable need for sex,");
...
private function sophieBlowsSixtyNine():void {
	...
	outputText(" that ");
	if(player.lowerBody.cockSpot.count() > 1) outputText("are");
	else outputText("is");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("s, searching for her target of the day.  At last, she settles on your " + cockDescript(x) + ", giving it an affectionate squeeze");
	...
	if(player.tongueType > TONUGE.HUMAN && player.tongueType <= TONUGE.DRACONIC) outputText("Longer and longer, your tongue continually unspools from your mouth, plunging deeper into the slippery channel with each passing moment.  Her taste saturates your many taste buds, and the velvety folds of her interior feel wonderful as you repeatedly slither your inhuman muscle through that syrupy passage.");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.lowerBody.balls > 0) outputText(" reaches under your [balls] and");
	...
	if(player.cumQ() >= 1000 || player.lowerBody.cockSpot.count() > 1) outputText("  Warm goo covers your [hips] and [butt] once your virile deposit becomes too much for Sophie to handle.");
...
private function forceSophieBlowjob():void {
	...
	if(player.lowerBody.balls > 0) outputText("fondling your [balls] with almost eager attention,");
	...
	if(player.lowerBody.cockSpot.count() == 2) outputText("  Your extra cock flops against her cheek, catching on her ear and slowly leaking pre-cum into her down-soft hair.");
	else if(player.lowerBody.cockSpot.count() > 2) outputText("  Your extra cocks flop against her cheeks, catching on her ear and shoulder and slowly leaking pre-cum into her down-soft hair.");
	...
	if(player.lowerBody.balls > 0) outputText("  Your [balls] feel almost like they're filling up, taut with liquid weight that begs to pump out");
	...
	if(player.lowerBody.balls > 0) outputText(" your [sack] and over");
	...
	if(player.lowerBody.cockSpot.list[x].cockType != CockType.DOG) outputText("slip your spasming boner out of her golden gates, dripping the last few drops onto her pouty lower-lip.");
	...
	if(flags[FlagEnum.SOPHIE_FOLLOWER_IRRITATION] == 1) outputText("Ahhh... not... not as much as... I would if you'd reciprocate!");
	else if(flags[FlagEnum.SOPHIE_FOLLOWER_IRRITATION] < 4) outputText("Ahhh... I... I don't know how many times I'll put up with this kind of treatment!");
	else if(flags[FlagEnum.SOPHIE_FOLLOWER_IRRITATION] < 8) outputText("Ahhh... are... are you ever going to reciprocate?");
	else if(flags[FlagEnum.SOPHIE_FOLLOWER_IRRITATION] == 8) outputText("Ahhh... I think... I've had enough...");
...
public function sophieSmallDongTeases():void {
	...
	if(player.upperBody.chest.list[0].breastRating >= 1) outputText(" and squeezes your " + player.breastCup(0) + " breast roughy.");
	...
	if(flags[FlagEnum.TIMES_MORNING_SOPHIE_FEMDOMMED] > 0) outputText("In the back of your mind, you know that if her words weren't true you wouldn't have kept sleeping with her and letting her do this to you.");
	...
	if(player.stats.sens/5 + rand(20) + 1 <= 20) {
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) outputText("golden");
		else outputText("pink");
...
private function sophieSpecial():void {
	...
	if(player.lowerBody.balls > 0) outputText(", and your [balls] are just waiting to unload a virile, sticky package down her throat");
...
private function kissSophieRecruitment():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText(", your second one sliding up between her butt");
	if(player.lowerBody.cockSpot.count() > 3) outputText(" while the others press against her");
...
internal function sophieIncestInHerCooterOrSomethingIDunno():void {
	...
	if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] > 1) outputText("The prospect of their " + player.mf("father","mother") + "'s attention causes a small scuffle to break out between your feathery daughters. Thankfully, the small fight is soon ended and one of your daughters flies down and stands before you.  ");
	...
	//second daughter: 
		...
		if(flags[FlagEnum.DAUGHTER_TWO_BIMBO] > 0) outputText("bimbo ");
		...
		if(flags[FlagEnum.DAUGHTER_TWO_BIMBO] > 0 && flags[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] == 0) outputText("\"<i>Mmmm... doncha want to just like, pound my sexy ass, " + player.mf("Daddy","Mommy") + "?  It's so big, it's just begging to be spanked and squeezed till it's totally red and sensitive</i>\"");
	...
	//third daughter:
		...
		if(flags[FlagEnum.DAUGHTER_FOUR_BIMBO] > 0 && flags[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] == 0) outputText("\"<i>Come on " + player.mf("Daddy","Mommy") + ", can't you see how totally hot I am?  Don't you just want to let loose and like, just fuck me till I scream.</i>\"");
		...
		//bimbo speech
		if(flags[FlagEnum.DAUGHTER_FOUR_BIMBO] > 0 && flags[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] == 0) outputText("  \"<i>Ahh... ohhh... oh, " + player.mf("Daddy","Mommy") + "!  You like my butt, don't you; isn't it nice?  It's so good to grab and squeeze.  I bet it would be fun to, like, spank it too....</i>\"");
		//[normal speech:]
...
private function phaseTwoOfIncest(daughter: number):void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) {
		...
		//if small bust:
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 3) outputText(", perk up as the cool air brushes past your hard, aroused nipples.");
		else if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 10) outputText(", bounce perkily, nipples hard as your handful of breast flesh touches the cool air.");
