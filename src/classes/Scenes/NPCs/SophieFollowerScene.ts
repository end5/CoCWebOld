export default class SophieFollowerScene extends NPCAwareContent {
	...
	public followerSophieMainScreen(): void {
		...
		//Sophie is in season
		if (sophieBimbo.sophieIsInSeason() && player.lowerBody.cockSpot.hasCock() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) {
		...
		if (Flags.list[FlagEnum.SOPHIE_CAMP_EGG_COUNTDOWN] > 0 && Flags.list[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) MainScreen.text("\n\n<b>Sophie's egg is sitting nearby.</b>");
	...
	private sleepWithSophieToggle(): void {
		...
		if (Flags.list[FlagEnum.SLEEP_WITH] != "Sophie") {
			...
			if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] == 0) MainScreen.text("Well, it is your bed.  Let me know if you want me coming back for snuggles, okay?");
	...
	private fuckFollowerSophie(): void {
		...
		//Both
		MainScreen.text("  You strip out of your [armor] without a second thought and present your " + CockDescriptor.describeCock(player, x) + " to Sophie.");
		...
		MainScreen.text(", her words still affect you.  Her pussy just feels... so... exquisite.  It's so hot and wet that you almost feel like you could cum right now.  Your [legs] wobble, but Sophie just pushes back even harder, holding you up by your " + CockDescriptor.describeCock(player, x) + " and the friction of your [butt] on the boulder.  The harpy watches your expression as her hips continue their slow roll, feeling your fingers twitch and knead her immense backside when the pleasure begins to mount.  You try to stay composed, but it's just so gods-damned good!  Her silken slit is so warm, wet, and just plain... erotic.  Slowly, the point where you would normally orgasm approaches, but instead of the spasming clenches that signal release, your jizz stays bottled up inside you tighter than sperm in a goblin's womb.");
		...
		if (y >= 0) MainScreen.text("You shift to bring your " + CockDescriptor.describeCock(player, y) + " up in place, just in time for Sophie's wiggling rectum to spread around it.  It slides on in without so much as a whimper.");
		...
		MainScreen.text("  Then, you black out, with your " + CockDescriptor.describeCock(player, x));
	...
	private sophieFollowerGivesBlowjobs(): void {
		...
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) MainScreen.text("You may have given me a bimbo's body and an insatiable need for sex,");
	...
	private sophieBlowsSixtyNine(): void {
		...
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s, searching for her target of the day.  At last, she settles on your " + CockDescriptor.describeCock(player, x) + ", giving it an affectionate squeeze");
		...
		if (player.upperBody.head.face.tongueType > TongueType.HUMAN && player.upperBody.head.face.tongueType <= TongueType.DRACONIC) MainScreen.text("Longer and longer, your tongue continually unspools from your mouth, plunging deeper into the slippery channel with each passing moment.  Her taste saturates your many taste buds, and the velvety folds of her interior feel wonderful as you repeatedly slither your inhuman muscle through that syrupy passage.");
		...
		if (player.cumQ() >= 500) MainScreen.text("  She quickly gives up on trying to swallow it after she starts showing some cum-pudge, but her hand pumps you energetically as she milks your " + CockDescriptor.describeCock(player, x) + ", squeezing the voluminous ropes of cum all over your [chest].");
	...
	private forceSophieBlowjob(): void {
		...
		if (player.lowerBody.cockSpot.get(x).cockType != CockType.DOG) MainScreen.text("slip your spasming boner out of her golden gates, dripping the last few drops onto her pouty lower-lip.");
		...
		if (Flags.list[FlagEnum.SOPHIE_FOLLOWER_IRRITATION] == 1) MainScreen.text("Ahhh... not... not as much as... I would if you'd reciprocate!");
		else if (Flags.list[FlagEnum.SOPHIE_FOLLOWER_IRRITATION] < 4) MainScreen.text("Ahhh... I... I don't know how many times I'll put up with this kind of treatment!");
		else if (Flags.list[FlagEnum.SOPHIE_FOLLOWER_IRRITATION] < 8) MainScreen.text("Ahhh... are... are you ever going to reciprocate?");
		else if (Flags.list[FlagEnum.SOPHIE_FOLLOWER_IRRITATION] == 8) MainScreen.text("Ahhh... I think... I've had enough...");
	...
	public sophieSmallDongTeases(): void {
		...
		if (player.stats.str >= 70) MainScreen.text("  You could force her off you if you wanted, but you didn't bring her to bed for nothing.");
		...
		if (player.upperBody.chest.get(0).breastRating >= 1) MainScreen.text(" and squeezes your " + BreastDescriptor.breastCup(player.upperBody.chest.get(0)) + " breast roughy.");
		...
		if (Flags.list[FlagEnum.TIMES_MORNING_SOPHIE_FEMDOMMED] > 0) MainScreen.text("In the back of your mind, you know that if her words weren't true you wouldn't have kept sleeping with her and letting her do this to you.");
		...
		if (player.stats.sens / 5 + rand(20) + 1 <= 20) {
			...
			if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) MainScreen.text("golden");
			else MainScreen.text("pink");
	...
	private sophieSpecial(): void {
		...
		MainScreen.text(".  Sophie seems to sense it and chuckles, pulling her mouth off your " + CockDescriptor.describeCock(player, x) + ", causing you to whine as you seek out her mouth again.  \"<i>There, there, my little cum pump.  Momma Sophie wouldn't be so mean as to leave you and your ");
		...
		MainScreen.text(CockDescriptor.describeCock(player, x) + " inside Sophie's schlong-milking passage, causing her to moan in satisfaction as the first of your virile cum hits her inner walls.");
	...
	internal function sophieIncestInHerCooterOrSomethingIDunno(): void {
		...
		if (Flags.list[FlagEnum.SOPHIE_ADULT_KID_COUNT] > 1) MainScreen.text("The prospect of their " + player.mf("father", "mother") + "'s attention causes a small scuffle to break out between your feathery daughters. Thankfully, the small fight is soon ended and one of your daughters flies down and stands before you.  ");
		...
		//second daughter: 
			...
			if (Flags.list[FlagEnum.DAUGHTER_TWO_BIMBO] > 0) MainScreen.text("bimbo ");
			...
			if (Flags.list[FlagEnum.DAUGHTER_TWO_BIMBO] > 0 && Flags.list[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] == 0) MainScreen.text("\"<i>Mmmm... doncha want to just like, pound my sexy ass, " + player.mf("Daddy", "Mommy") + "?  It's so big, it's just begging to be spanked and squeezed till it's totally red and sensitive</i>\"");
		...
		//third daughter:
			...
			if (Flags.list[FlagEnum.DAUGHTER_FOUR_BIMBO] > 0 && Flags.list[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] == 0) MainScreen.text("\"<i>Come on " + player.mf("Daddy", "Mommy") + ", can't you see how totally hot I am?  Don't you just want to let loose and like, just fuck me till I scream.</i>\"");
			...
			//bimbo speech
			if (Flags.list[FlagEnum.DAUGHTER_FOUR_BIMBO] > 0 && Flags.list[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] == 0) MainScreen.text("  \"<i>Ahh... ohhh... oh, " + player.mf("Daddy", "Mommy") + "!  You like my butt, don't you; isn't it nice?  It's so good to grab and squeeze.  I bet it would be fun to, like, spank it too....</i>\"");
			//[normal speech:]
