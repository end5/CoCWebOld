...
public sophieFollower():boolean {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0 && flags[FlagEnum.SOPHIE_DEBIMBOED] > 0) return true;
...
internal function unbimboSophie():void {
	...
	MainScreen.text("Taking the potent mixture out of your pouch, you consider giving it to the bimbo harpy fluttering about in front of you.  She'd probably be pretty mad about the whole affair, but if you want to repair the damage you've inflicted, this is probably your best bet.  Do you un-bimbo Sophie?");
	//[Yes] [No]☼
	menu();
...
private noDontDebimbo():void {
	MainScreen.clearText();
	MainScreen.text("You shrug and put the potion back in your pack.  Maybe later...  A mad Sophie isn't something you particularly want to deal with right now.");
...
private yesDebimboSophie():void {
	...
	flags[FlagEnum.SOPHIE_DEBIMBOED] = 1;
	if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] > 0) flags[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] = 1;
	...
	if(player.hasItem(consumables.BIMBOLQ)) MainScreen.addButton(4,"Bimbo Again",bimboSophieAgain);
...
private bimboSophieAgain():void {
	...
	flags[FlagEnum.SOPHIE_DEBIMBOED] = 0;
	flags[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] = 0;
...
private beatSophieAroundYouMonster():void {
	MainScreen.clearText();
	MainScreen.text("Oh no, fuck this!  You ready your [weapon] to knock some sense into the harpy slut.  Seeing your hostile movements, Sophie draws a sharp breath, preparing to defend herself.");
	
	MainScreen.text("\n\n\"<i>I'm gonna make you pay for this!</i>\"");
	//(Go to normal Sophie Combat screen)
	startCombat(new Sophie());
	monster.statusAffects.add(new StatusAffect("BimboBrawl",0,0,0,0)));
...
internal function beatUpDebimboSophie():void {
	MainScreen.clearText();
	MainScreen.text("Sophie stumbles back, unable to withstand your furious assault.  You grab the harpy before she collapses, asking if she's ready to fucking listen now.  Defiantly, she looks away from you, though otherwise too weakened to resist you.");
...
private letDebimboSophieGo():void {
	MainScreen.clearText();
	MainScreen.text("You decide it's probably best to let Sophie go.  She remains still for a long moment, before finally looking at you with her big, blue eyes, a strange mixture of loathing and wonder in them.  Without further hesitation, she turns and rushes off, back toward the High Mountains you'd imagine.");
	...
	if(player.stats.lib >= 50) {
		...
		if(player.stats.cor < 40) {
			MainScreen.text("  You blink repeatedly, trying to clear your head of these idle thoughts, but are nonetheless aroused.");
			//{+10 AROUSAL}
			dynStats("lus", 10);
		}
		else {
...
private apologizeToDebimboSophie():void {
	MainScreen.clearText();
	MainScreen.text("You decide to apologize to Sophie, saying you didn't mean for things to get so out of hand... you didn't know she'd change quite so much!  You only thought the potion would enhance her physique, maybe make her a little more receptive to your advances.");
...
public followerSophieMainScreen():void {
	...
	if(sophieBimbo.sophieIsInSeason() && player.lowerBody.cockSpot.hasCock() && flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) {
		...
		if(sophieBimbo.bimboSophie()) MainScreen.text("restoring her intellect made her quite skilled at it again");
		else MainScreen.text("she's quite skilled at it");
		MainScreen.text(".  \"<i>Ohhh, come on, [name]!  Just give it to me like a good " + player.mf("boy","girl") + ", would you?  I could even give you a special treat.  <b>Just give. It. To. Me.</b></i>\"");
	...
	else if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) {
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) MainScreen.text("Sophie leans back in her nest as you approach, her prominent, bare F-cups on display.  She's completely nude, as always, and sits with her avian legs shamelessly spread, revealing the slick slit of her womanhood framed by downy-soft platinum blonde feathers.  \"<i>Hey there, " + player.mf("handsome","cutey") + ".  What can Momma Sophie do for you?  Come for a cuddle in my welcoming wings, maybe; or for something more... exciting?</i>\"");
		else MainScreen.text("Sophie leans back in her nest as you approach, her prominant, bare DD-cups on display.  She's completely nude, as always, and sits with her avian legs demurely crossed, concealing the slick slit of her womanhood from you.  \"<i>Hey there, " + player.mf("handsome","cutey") + ".  What can Momma Sophie do for you?</i>\"");
	}
	else
	{
		MainScreen.text("When you call her name, Sophie's gold-glossed lips curve upward in a smile. She takes her time picking her way out of her nest, and then glides down to you.");
	...
	menu();
	MainScreen.addButton(0,"Appearance",sophieAppearance);
	if(player.lust >= 33) MainScreen.addButton(1,"Sex",sexWithFollowerSophie);
	else MainScreen.text("\n\nYou aren't aroused enough to sleep with Sophie at the moment.");
	if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) MainScreen.addButton(8,"Sleep With",sleepWithSophieToggle);
	if(flags[FlagEnum.SLEEP_WITH] == "Sophie" && flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) {
		MainScreen.addButton(8,"NoSleepWith",sleepWithSophieToggle);
		MainScreen.text("\n\nYou're currently sharing your bed with Sophie at night, but you could kick her out, if you wanted.");
	}
	if(flags[FlagEnum.SOPHIE_CAMP_EGG_COUNTDOWN] > 0 && flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) MainScreen.text("\n\n<b>Sophie's egg is sitting nearby.</b>");
	if(flags[FlagEnum.SOPHIE_DAUGHTER_MATURITY_COUNTER] > 0) {
		MainScreen.addButton(7,"Daughter",sophieBimbo.daughterCheckup);
	}
	if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0) MainScreen.addButton(9, "Back", camp.campFollowers);
	else MainScreen.addButton(9, "Back", kGAMECLASS.farm.farmCorruption.rootScene);
	
	if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 0 && flags[FlagEnum.FARM_CORRUPTION_STARTED] == 1) MainScreen.addButton(2, "Farm Work", sendToFarm);
	if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 1) MainScreen.addButton(2, "Go Camp", backToCamp);
	
	if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 1 && flags[FlagEnum.FOLLOWER_PRODUCTION_SOPHIE] == 0) MainScreen.addButton(3, "Harvest Eggs", harvestEggs);
	if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 1 && flags[FlagEnum.FOLLOWER_PRODUCTION_SOPHIE] == 1) 
	{
...
private sendToFarm():void
{
	...
	MainScreen.text("You tell your live-in harpy that she is to head towards the lake, find a farm, present herself to the lady who works there and do as she says. Sophie isn’t exactly enthused by this idea.");
...
private backToCamp():void
{
	...
	MainScreen.text("You tell her she’s welcome to head back to camp now.");
...
private postEggSelector(selected:string):void
{
	...
	MainScreen.text("“<i>I’ll make some nice " + selected.toLowerCase() + "eggs for you then, hun,</i>” she says with a small grin. “<i>I can only produce one a week, though. I’ll put it with everything else you earn out of this hole in the ground. How’s that?</i>”");
...
private changeEggs():void
{
	...
	MainScreen.text("You ask Sophie if she will change the type of eggs that she is producing for you.");
...
private sexWithFollowerSophie():void {
	MainScreen.clearText();
	menu();
	MainScreen.text("\"<i>Oh, really? I was kind of hoping you'd want a taste of me soon.</i>\"  Sophie somewhat perks up at the mention of having sex with you.  \"<i>So, what'd you like? The usual, or should Momma do something special for you this time around?</i>\"  She licks her lips at the suggestion.");
	...
	if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] > 0 && flags[FlagEnum.SOPHIE_FAMILY_INCEST] > 0 && player.cockThatFits(sophieBimbo.sophieCapacity()) >= 0)
...
private sleepWithSophieToggle():void {
	MainScreen.clearText();
	if(flags[FlagEnum.SLEEP_WITH] != "Sophie") {
		MainScreen.text("You ask Sophie if she'd like to start sleeping with you at night.  Sophie cocks an eyebrow up at the suggestion.");
		//DEBIMBO'ed
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) {
			...
			MainScreen.text("\n\nYou ");
			if(player. cor < 33) MainScreen.text("sheepishly ");
			MainScreen.text("answer that you thought it might be nice to snuggle up with her at night, not even necessarily sexually.  Just having someone to hold during the long, cold nights would be nice, and you admit that Sophie would be a rather lovely body to cuddle up against.");
			...
			if(player.smallestCockArea() <= 5 && player.lowerBody.cockSpot.hasCock()) MainScreen.text(", but don't be surprised if I go a little rough on that tiny dick of yours in the morning");
			MainScreen.text("!</i>\" she calls.");
			if(player.smallestCockArea() <= 5 && player.lowerBody.cockSpot.hasCock()) MainScreen.text("\n\n<b>If you don't want her to tease you about your small penis, you had better tell her to stop sleeping with you.</b>");
		}
		//NORMAL
		else {
			...
			if(player.smallestCockArea() <= 5 && player.lowerBody.cockSpot.hasCock()) MainScreen.text("  \"<i>You won't mind me teasing your cute little dick in the mornings, will you [name]?  It's so cute...</i>\" she muses.\n\n<b>If you don't want her to tease you about your small penis, you had better not sleep with her.</b>");
		}
		flags[FlagEnum.SLEEP_WITH] = "Sophie";
	}
	else {
		//Don't Sleep WIth:*
		MainScreen.text("You tell Sophie that you'd rather not sleep with her for now.  She sighs heavily at the proclamation and says, \"<i>");
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] == 0) MainScreen.text("Well, it is your bed.  Let me know if you want me coming back for snuggles, okay?");
		else MainScreen.text("Couldn't handle all these curves, huh, [name]?  Don't fret, Momma Sophie will stick around until you can't stop yourself from cuddling up on her.");
		MainScreen.text("</i>\"");
...
private fuckFollowerSophie():void {
	...
	if(sophieBimbo.sophieIsInSeason()) {
		MainScreen.text("Sophie pulls you into her feathery embrace, her fingertips sensually exploring every inch of your form.  Her voice coos huskily, \"<i>So you want to help make a momma out of Momma Sophie, huh?</i>\"  She grabs your hand and stuffs it between her luscious thighs, right into the dripping-wet delta of her mons.  \"<i>Don't keep me waiting... you got me all... hot and bothered...</i>\"");
	}
	//Regular Intro
	else {
		MainScreen.text("Sophie smirks as she saunters closer, fingertips casually exploring your body.  She coos huskily, \"<i>So, you want to have a little fun with Momma Sophie, huh?</i>\"  She spreads her luscious thighs apart, just enough to show you the glittering moisture on her mound.  \"<i>Well, let's see how much of a " + player.mf("man","'man'") + " you are...</i>\"");
	}
	//Both
	MainScreen.text("  You strip out of your [armor] without a second thought and present your " + cockDescript(x) + " to Sophie.");
	...
	if(player.stats.int < 50) MainScreen.text("\n\nAbruptly");
	else MainScreen.text("\n\nEven though you know she's using her compulsion");
	MainScreen.text(", her words still affect you.  Her pussy just feels... so... exquisite.  It's so hot and wet that you almost feel like you could cum right now.  Your [legs] wobble, but Sophie just pushes back even harder, holding you up by your " + cockDescript(x) + " and the friction of your [butt] on the boulder.  The harpy watches your expression as her hips continue their slow roll, feeling your fingers twitch and knead her immense backside when the pleasure begins to mount.  You try to stay composed, but it's just so gods-damned good!  Her silken slit is so warm, wet, and just plain... erotic.  Slowly, the point where you would normally orgasm approaches, but instead of the spasming clenches that signal release, your jizz stays bottled up inside you tighter than sperm in a goblin's womb.");
	...
	if(y >= 0) MainScreen.text("fuck it");
	else MainScreen.text("finger my asshole");
	MainScreen.text(" while I fuck you, [name]!</i>\" she commands, fixing you with her eyes authoritatively.  ");
	if(y >= 0) MainScreen.text("You shift to bring your " + cockDescript(y) + " up in place, just in time for Sophie's wiggling rectum to spread around it.  It slides on in without so much as a whimper.");
	else MainScreen.text("You thumb at her asshole hesitantly, right up until her wiggling rectum presses on it, easily dilating around your invading finger.");
	MainScreen.text("  You squeeze tighter and begin to massage without knowing why, actually able to feel yourself through her thin, internal walls.");
	...
	if(y >= 0) MainScreen.text(" and anal walls");
	MainScreen.text(".  Your fingers dig into her plush butt, no longer kneading, but instead curling possessively, claiming absolute ownership of that supple behind and the rapidly-filling cunt below.");
	if(y >= 0) MainScreen.text("  You pack her intestines full of cum at the same time that you sperm up her clingy cunny tunnel.");
	if(sophieBimbo.sophieIsInSeason()) MainScreen.text("  Her pussy seems to drink the spunky deposit down, her cervix opening to allow in as much in as possible.  You can feel the suction, and if you weren't already riding out a mind-bending orgasm, you're sure you'd be cumming harder than normal right now.");
	MainScreen.text("  Your body slacks over her, the only muscles that are actually working are the ones spasming in your loins, pumping out fresh batches of sperm.  You whimper with primal pleasure, spittle frothing at the corners of your mouth.");
	if(player.cumQ() >= 500) {
		MainScreen.text("  Sophie's belly ");
		if(player.cumQ() < 1000) MainScreen.text("has begun to inflate");
		else MainScreen.text("has inflated to an absurd degree");
		MainScreen.text(" due to all the spooge you're squirting");
		if(player.cumQ() >= 4000) MainScreen.text(" and it's still growing larger");
		MainScreen.text(".");
	}
	if(player.cumQ() >= 5000 || (player.cumQ() >= 500 && sophieBimbo.sophieIsInSeason())) {
		MainScreen.text("  A huge wash of white rushes out from her well-sexed fuck-hole, soaking your legs with the mixed cum.");
	}
	MainScreen.text("  Then, you black out, with your " + cockDescript(x));
	if(y >= 0) MainScreen.text(" and " + cockDescript(y));
	MainScreen.text(" still pulsing in ecstasy.  It was such a good cum...");
	player.orgasm();
	dynStats("sen", -2);
...
private sophieVagFollowerFollowup():void {
	...
	MainScreen.text("You're roused a few moments later by a smiling Sophie, the proof of your union dripping between her thighs.  She quips, \"<i>");
	if(sophieBimbo.sophieIsInSeason()) MainScreen.text("If that didn't take, I don't know what will!");
	else if(player.cockArea(x) <= 5) MainScreen.text("Not too bad for a tiny girl-dick!");
	else MainScreen.text("It's always better when you have to wait for it, isn't it?");
	MainScreen.text("</i>\"");
...
private sophieFollowerGivesBlowjobs():void {
	...
	MainScreen.text("Sophie cocks her head to the side and chuckles, \"<i>You wanna feel Momma Sophie's lips around that ");
	if(player.cockArea(x) <= 5) MainScreen.text("tiny clit you call a penis");
	else if(player.cockArea(x) <= sophieBimbo.sophieCapacity()/2) MainScreen.text("needy boy-cock");
	else MainScreen.text("over-swollen engorgement you call a cock");
	MainScreen.text("?</i>\"  She saunters up with her hips swaying seductively, her juicy mound squelching noisily, webs of lubricant dangling between her thighs whenever they spread.  The harpy's fingertip punches into your chest");
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text(", right between your [chest]");
	MainScreen.text(", and she slowly trails it up to your chin, which she gently grabs in her long-nailed fingers.  \"<i>");
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0) MainScreen.text("You may have given me a bimbo's body and an insatiable need for sex,");
	else MainScreen.text("You may have seduced me into your camp with hot sex,");
	MainScreen.text("</i>\" she coos, \"<i>but that doesn't mean I'll wrap these beautiful lips around you for nothing.  How about you let me use that cute face of yours to grind my pussy on, and I'll return the favor?</i>\"  She blows you a kiss that culminates in her running her tongue over her swollen lips in a slow, wet circle.");
	//[Force Her] [Sixtynine]
	menu();
...
private sophieBlowsSixtyNine():void {
	...
	if(x < 0) x = player.smallestCockIndex();
	MainScreen.clearText();
	MainScreen.text("You nod eagerly, something that gets the busty harpy smiling as her whole body flushes.  She rushes at you, leaping and flapping, speeding towards you faster as old feathers kick up in a wake behind her.  You spread your arms and let her slam into you, the impact cushioned by her weighty breasts and large, soft thighs.  Both of you roll end over end through the dirt, eventually coming to rest with her straddling your [chest] and rubbing a finger through her slippery folds.  She adds another finger to spread them wide, the pink interior pulsing wetly and dripping with female moisture.");
	...
	if(player.tongueType > TONUGE.HUMAN && player.tongueType <= TONUGE.DRACONIC) MainScreen.text("Longer and longer, your tongue continually unspools from your mouth, plunging deeper into the slippery channel with each passing moment.  Her taste saturates your many taste buds, and the velvety folds of her interior feel wonderful as you repeatedly slither your inhuman muscle through that syrupy passage.");
	else MainScreen.text("Your tongue slides right in, the harpy's pussy seeming almost like a second home.  Her taste saturates your taste buds as you plunge in as deeply as possible, gliding over her velvet folds as it goes.");
	MainScreen.text("  Sophie's immense thighs squeeze tighter in response, shutting out any last vestiges of light and pushing you harder into her sopping-wet mound.  You let your lower lip graze her rigid clitty and savor the lubricating fluid that she leaks into your waiting mouth.");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		MainScreen.text("  She tenderly");
		if(player.lowerBody.balls > 0) MainScreen.text(" reaches under your [balls] and");
		MainScreen.text(" explores your [vagina] with two fingers, her thumb brushing across your [clit] every few seconds.  It seems the harpy is considerate of even your feminine needs - ones that serve to make you even harder, so full it almost hurts.");
	}
	//else
	else if(player.lowerBody.balls > 0) {
		MainScreen.text("  She tenderly caresses your [sack], allowing her fingers to dance over your [balls].  It seems the harpy is considerate of all your needs in a way that makes you even harder, so full it almost hurts.");
	}
	else MainScreen.text("She tenderly strokes a long nail across the sensitive skin of your taint.  It seems she's being very considerate to your pleasure, so considerate she has you so hard and full that it almost hurts.");
	MainScreen.text("  Her feathery hair dangles over your [legs], tickling you as she noisily slurps and sucks, while you moan directly into her bare-skinned twat.  The vibrations set her dripping even more fiercely than before.  You do your best to dutifully drink it down.");
	...
	if(player.cockArea(x) <= 60) MainScreen.text("to the [sheath]");
	else MainScreen.text("as far as she can, gagging slightly");
	MainScreen.text(".  Her thighs begin to tremble around you, hard enough that you begin to worry that she might somehow crush your head, yet it's more like being smothered with soft pads than anything truly worrisome.  You shudder in excitement of your own and begin to flick her clit, eager to make her share in the pleasure.");
	...
	if(player.cumQ() >= 1000 || player.lowerBody.cockSpot.count() > 1) MainScreen.text("  Warm goo covers your [hips] and [butt] once your virile deposit becomes too much for Sophie to handle.");
	if(player.cumQ() >= 500) MainScreen.text("  She quickly gives up on trying to swallow it after she starts showing some cum-pudge, but her hand pumps you energetically as she milks your " + cockDescript(x) + ", squeezing the voluminous ropes of cum all over your [chest].");
	if(player.cumQ() >= 3000) MainScreen.text("  You make a big white puddle with all the drippings, sinking into the cummy mud as you eat out the orgasmic harpy matron.");
	...
	flags[FlagEnum.SOPHIE_FOLLOWER_IRRITATION] = 0;
	//Slimefeed
	player.slimeFeed();
	//8 hours lust stick
...
private forceSophieBlowjob():void {
	...
	if(player.lowerBody.balls > 0) MainScreen.text("fondling your [balls] with almost eager attention,");
	else MainScreen.text("rubbing your length with almost eager attention,");
	...
	if(player.cockArea(x) <= 60) MainScreen.text("as far as it can go, even taking you part way into her throat before she runs out of room");
	else MainScreen.text("the whole way to your base, inch after inch of dick-flesh vanishing into her pliant throat");
	MainScreen.text(".");
	...
	if(player.lowerBody.cockSpot.count() == 2) MainScreen.text("  Your extra cock flops against her cheek, catching on her ear and slowly leaking pre-cum into her down-soft hair.");
	else if(player.lowerBody.cockSpot.count() > 2) MainScreen.text("  Your extra cocks flop against her cheeks, catching on her ear and shoulder and slowly leaking pre-cum into her down-soft hair.");
	...
	if(player.lowerBody.balls > 0) MainScreen.text("  Your [balls] feel almost like they're filling up, taut with liquid weight that begs to pump out");
	else MainScreen.text("  It feels tight, like you're filling up with a warm liquid weight that's just begging to be let out");
	MainScreen.text(" all over the motherly harpy's face.  You groan out loud with sheer excitement and fixate your eyes on Sophie's bodacious rump and thick thighs, partly obscured by her trembling tailfeathers.");
	
	MainScreen.text("\n\nSlobber and drool drip down");
	if(player.lowerBody.balls > 0) MainScreen.text(" your [sack] and over");
	MainScreen.text(" your legs.  Sophie's eyes roll back slightly - not all the way but enough to proudly display her penultimate pleasure as she cums.  Her syrupy girl-cum splatters off the waste-land earth below, though the sound changes to a wetter, muddy splashing after a few seconds.  Squeezing tightly, the harpy's throat convulses around you.  Her lips lock tight as she hums in ecstasy, languidly beginning to suckle your " + cockDescript(x) + " as her own orgasm dies down.  You force her to move faster, pushing her head to pop back and forth, too eager for release to care for her comfort.  Judging by the steady, squelching pumping that begins anew, she doesn't care that much either.");
	...
	if(player.hasKnot(x)) MainScreen.text("  The knot inflates just behind her lips, a golden orb that entraps you within her slick mouth and makes sure all your salty cremè is going to go down her throat.");
	...
	if(player.cumQ() >= 500) {
		MainScreen.text("  She gurgles noisily when her midriff begins to bulge from your liquid deposit");
		if(player.cumQ() >= 2000) MainScreen.text(", but that's just the beginning.  You bloat her tummy to an obscene, almost spherical shape");
		if(player.cumQ() >= 4000) MainScreen.text(", and then cum some more, the flooding jizz forced to escape from her nostrils after you finish flooding every inch of her digestive system");
		MainScreen.text(".");
	}
	
	MainScreen.text("\n\nAlmost regretfully, you ");
	if(player.lowerBody.cockSpot.list[x].cockType != CockType.DOG) MainScreen.text("slip your spasming boner out of her golden gates, dripping the last few drops onto her pouty lower-lip.");
	else MainScreen.text("pull against Sophie's cock-locked mouth, but your knot won't budge.  You sit there waiting for it to go down for what seems like forever, and her lipstick doesn't exactly help - it seems like your cum continues to flow and dribble the entire time, your animal-dong intent on inseminating the harpy's mouth.  Still, you manage to 'pop' free eventually, even though it stretches Sophie's jaw and echoes around your camp.");
	...
	if(flags[FlagEnum.SOPHIE_FOLLOWER_IRRITATION] == 1) MainScreen.text("Ahhh... not... not as much as... I would if you'd reciprocate!");
	else if(flags[FlagEnum.SOPHIE_FOLLOWER_IRRITATION] < 4) MainScreen.text("Ahhh... I... I don't know how many times I'll put up with this kind of treatment!");
	else if(flags[FlagEnum.SOPHIE_FOLLOWER_IRRITATION] < 8) MainScreen.text("Ahhh... are... are you ever going to reciprocate?");
	else if(flags[FlagEnum.SOPHIE_FOLLOWER_IRRITATION] == 8) MainScreen.text("Ahhh... I think... I've had enough...");
	else MainScreen.text("Ahhh... actually... I think... <b>I think if you don't treat me nicer, I might leave.</b>");
	MainScreen.text("</i>\"");
...
private sophieSixtyNine():void {
	MainScreen.clearText();
	MainScreen.text("Sophie cocks an eyebrow and asks, \"<i>A little sapphic delight for the horny champion?</i>\"  She snorts and rolls her eyes, \"<i>No thanks.  I'm not really that into girls.</i>\"  It doesn't seem like she's up for it.");
	menu();
...
public sophieSmallDongTeases():void {
	...
	if(player.str >= 70) MainScreen.text("  You could force her off you if you wanted, but you didn't bring her to bed for nothing.");
	MainScreen.text("  Her fluffy behind crashes down on your [legs], just below your waist.  With a familiar coo, she places her other foot on your chest");
	if(player.upperBody.chest.list[0].breastRating >= 1) MainScreen.text(" and squeezes your " + player.breastCup(0) + " breast roughy.");
	else MainScreen.text(" and lets the talons dig into your " + player.skinFurScales() + " just enough to hurt a little.");
	...
	if(flags[FlagEnum.TIMES_MORNING_SOPHIE_FEMDOMMED] > 0) MainScreen.text("In the back of your mind, you know that if her words weren't true you wouldn't have kept sleeping with her and letting her do this to you.");
	else MainScreen.text("In the back of your mind, you almost have to wonder if it's true, or if her lipstick is just now getting into your system...");
	...
	//{Low Sensitivity}
	if(player.stats.sens/5 + rand(20) + 1 <= 20) {
		...
		if(player.cumQ() >= 500) {
			MainScreen.text("  You cum so much that both you and Sophie wind up");
			if(player.cumQ() < 1000) MainScreen.text(" with huge wet spots over your bodies");
			else MainScreen.text(" soaked from the waist down");
			MainScreen.text(".  It feels so good that neither of you mind.");
		}
		if(player.cumQ() >= 2000) MainScreen.text("  Your bed is a reflective, sticky white as well, practically a lake of seed by now.");
	}
	//{High Sensitivity}
	else {
		...
		if(player.pregnancyIncubation > 0 && player.pregnancyIncubation < 150) MainScreen.text(", rolling off the pregnant dome you call your tummy");
		MainScreen.text(".");
		if(player.cumQ() >= 500) MainScreen.text("  You cum for what seems like hours, emptying your [balls] onto yourself, every drop of wasted seed a testament to your pussy-whipped status.");
		if(player.cumQ() >= 1000) MainScreen.text("  A puddle of jizz forms around you before long, mixing with the drippings from her pussy as she rides you into your own soaked bedding.");
		MainScreen.text("  When you're about empty, Sophie says, \"<i>Spurt all that nasty boy-cum out for me, then you can be my lady-pet.</i>\"  Your orgasm redoubles, surging to lance out several more long, thick dribbles of pleasure onto your soaked middle.");
	...
	if(player.cumQ() >= 1000) MainScreen.text("huge ");
	MainScreen.text("mess you made, wondering why you enjoyed that so much.");
	
	flags[FlagEnum.TIMES_MORNING_SOPHIE_FEMDOMMED]++;
	player.orgasm();
...
private sophieSpecial():void {
	...
	if(player.lowerBody.balls > 0) MainScreen.text(", and your [balls] are just waiting to unload a virile, sticky package down her throat");
	MainScreen.text(".  Sophie seems to sense it and chuckles, pulling her mouth off your " + cockDescript(x) + ", causing you to whine as you seek out her mouth again.  \"<i>There, there, my little cum pump.  Momma Sophie wouldn't be so mean as to leave you and your ");
	if(player.cockArea(x) <= 5) MainScreen.text("squirting clitty");
	else if(player.cockArea(x) <= 30) MainScreen.text("needy boy-cock");
	else MainScreen.text("giant piece of a meat rod");
	MainScreen.text(" without blowing a nice big load. It just can't go in my mouth... and you should be nice and compliant, not doing things on your own.</i>\"");
	...
	MainScreen.text(cockDescript(x) + " inside Sophie's schlong-milking passage, causing her to moan in satisfaction as the first of your virile cum hits her inner walls.");
	if(player.cumQ() < 250) MainScreen.text("  She milks you for all you're worth.");
	else if(player.cumQ() < 750) MainScreen.text("  She whimpers at how much you've discharged into her.");
	else if(player.cumQ() <= 1500) MainScreen.text("  Your cum soon gushes out into a small puddle from her pussy and onto your [hips].");
	else MainScreen.text("  Her ass is suddenly pushed out more towards you, cum gushing out of her wet cunt as she starts inflating from the sheer amount of cum in your first load.");
	
	MainScreen.text("\n\n\"<i>Good " + player.mf("boy","girl") + " with a nice, obedient ");
	if(player.cockArea(x) <= 5) MainScreen.text("clitty");
	else if(player.cockArea(x) <= 30) MainScreen.text("pretty cock");
	else MainScreen.text("humongous cunt-stretcher");
	MainScreen.text("... all for me... but you can't go soft, can you?  You just want to squirt more of your virile cum into Momma Sophie.</i>\"  Sophie rolls her hips atop of you, her jizz and girl-cum slickened ass hitting against your pelvis as she ruthlessly uses your overly sensitive " + cockDescript(x) + ", going from the tip to the base as she bounces atop of you.  The drugged lipstick and her constant, magic-powered demands don't allow you to go soft even for a moment as she rides you to completion, her hand working between her thighs at her own clit all the while when finally her pussy grips hard at you, causing you to thrust your hips and moan from sheer sensitivity, unable to achieve another orgasm so quickly.");
	
	MainScreen.text("\n\nWith a grin, Sophie rises from your " + cockDescript(x) + ", ");
	if(player.cumQ() < 250) MainScreen.text("her pussy dripping your cum down onto the ground");
	else MainScreen.text("your own semen drowning your [hips] and [butt] in a messy cum-puddle as it drips out of her");
	MainScreen.text(", and turns towards you, jiggling her tits right above your [face] as she bends over you, asking, \"<i>Now, wasn't that fun, [name]?  You think we should end now, shouldn't we?  Or do you want to give momma Sophie more of your thick cum so she's definitely pregnant twice over?</i>\"  Drugged and fucked to the edge of your sense of self-worth, you practically beg her to put you inside her again, your golden-tinted member standing proudly in the air, waiting for her hips and pussy to descend.  \"<i>Good answer, very good answer.</i>\"  She descends upon you again, rubbing her pussy against the tip of your " + cockDescript(x) + ", teasing both herself and you before drawing you inside and hilting you down to your [sheath] into her!");
	...
	if(player.cockArea(x) <= 5) MainScreen.text("dainty little fem-cock");
	else if(player.cockArea(x) <= 30) MainScreen.text("nice, sweet cum-squirter");
	else MainScreen.text("immense, gorgeous beast of a cock");
	MainScreen.text(" until it's almost broken!  Come on you sweet thing, you know you want to squirt it in me like you're a broken faucet!</i>\"");
	...
	if(player.cumQ() < 250) MainScreen.text("several ropes");
	else if(player.cumQ() < 750) MainScreen.text("a large, chaotic stream");
	else if(player.cumQ() < 2000) MainScreen.text("wave after wave");
	else MainScreen.text("massive amounts");
	MainScreen.text(" of cum into her, causing her to ");
	if(player.cumQ() < 500) MainScreen.text("moan in satisfaction");
	else if(player.cumQ() < 1000) MainScreen.text("groan as her cunny is unable to hold it all");
	else if(player.cumQ() < 2000) MainScreen.text("stretch atop of you, a cum-filled tummy added to the jiggling tits and soft butt");
	else MainScreen.text("erupt your cum violently around your shaft as you stretch her to pregnancy-looking visage with your cum alone");
	MainScreen.text(".");
	...
	MainScreen.text(" is probably ready to blow another load very soon, isn't it?  You just can't keep up with Momma Sophie's exquisite pussy, you have to keep cumming and cumming over and over again.</i>\"");
	...
	if(player.cumQ() < 250) MainScreen.text("  You only drop a meager few squirts inside of her");
	else if(player.cumQ() < 750) MainScreen.text("  Even your large cum production can't keep up with that pace");
	else MainScreen.text("  Inhuman as your cum factories are, you're only capable of delivering a few big spurts before it slows to a dribble");
	MainScreen.text(".  You thrash and moan and shudder under her, calling her name as she strokes your [hair] and coos, pressing her nipple against your mouth again.  \"<i>Shhh... you've done well for me, little cum-pump... give it all to Momma Sophie... It'll be over in just a moment...</i>\"");
...
private getOutSophieRecruitment():void {
	...
	dynStats("lus", -5);
	flags[FlagEnum.NO_PURE_SOPHIE_RECRUITMENT] = 1;
	doNext(playerMenu);
}
...
private kissSophieRecruitment():void {
	MainScreen.clearText();
	MainScreen.text("You shut her up the best way you know how - planting your lips upon her own, knowing full-well of the effects of her lustful lipstick.  Sophie returns the gesture, locking her tongue around yours while she tastes you, grinding her other set of lips against your attentive member.");
	...
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text(", your second one sliding up between her butt");
	if(player.lowerBody.cockSpot.count() > 3) MainScreen.text(" while the others press against her");
	MainScreen.text(".  Sophie cries out in pleasure, almost waking up the entire neighbourhood - which would be a bad thing, considering your neighbours.  You quickly take another taste of her lips, shutting her up while you start to pound upwards into her hungry quim.");
...
private sophieRecruitmentFinale():void {
	MainScreen.clearText();
	MainScreen.text("Sophie gives you a peck on the cheek, removing herself from your body to prepare for flight.");
	...
	if(camp.companionsCount() >= 3) MainScreen.text(", sure to introduce her to the other denizens along the way and smooth over any ruffled feathers before things escalate.  Sophie does a good job helping with that.  She seems to have a natural ability to defuse angry situations, something she attributes to raising her many daughters");
	MainScreen.text(".");
	MainScreen.text("\n\n(<b>Sophie is now available in the followers tab!</b>)");
	flags[FlagEnum.SOPHIE_RECRUITED_PURE] = 1;
	doNext(playerMenu);
}
...
internal function sophieIncestInHerCooterOrSomethingIDunno():void {
	MainScreen.clearText();
	MainScreen.text("Approaching the harpy nest you call out for one of your daughters.  ");
	if(flags[FlagEnum.SOPHIE_ADULT_KID_COUNT] > 1) MainScreen.text("The prospect of their " + player.mf("father","mother") + "'s attention causes a small scuffle to break out between your feathery daughters. Thankfully, the small fight is soon ended and one of your daughters flies down and stands before you.  ");
	else MainScreen.text("Your daughter, hearing your call, is more than happy to come to you.  ");
	MainScreen.text("The curvy girl smiles at you, her eyes bright and attentive, positively sparkling with the taboo affection she holds for her beloved daddy.  Smirking to her you give her an appraising look, your eyes wandering up and down her beautiful womanly body.");
	
	let daughterCap: number = flags[FlagEnum.SOPHIE_ADULT_KID_COUNT];
	...
	//second daughter: 
		...
		if(flags[FlagEnum.DAUGHTER_TWO_BIMBO] > 0) MainScreen.text("bimbo ");
		MainScreen.text("lips and blows you a kiss.  She slides her hands over her hips, pushing her taut butt out even more before spanking herself for your pleasure, her lips gasping at the feeling of her self-spank as she tries to work you into a breeding frenzy.");
		...
		if(flags[FlagEnum.DAUGHTER_TWO_BIMBO] > 0 && flags[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] == 0) MainScreen.text("\"<i>Mmmm... doncha want to just like, pound my sexy ass, " + player.mf("Daddy","Mommy") + "?  It's so big, it's just begging to be spanked and squeezed till it's totally red and sensitive</i>\"");
		else MainScreen.text("\"<i>Come on, " + player.mf("Daddy","Mommy") + ", touch me, spank me like the bad girl I am, lusting after my " + player.mf("Daddy","Mommy") + " like this.  Give it to me hard, spank my big round ass and pound my hot tight cunt like I know you want</i>\"");
	...
	//third daughter:
		...
		if(flags[FlagEnum.DAUGHTER_THREE_BIMBO] > 0 && flags[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] == 0) MainScreen.text("\"<i>L-like, did I do a good job, " + player.mf("Daddy","Mommy") + "?  I so totally did my best for you.  I know I'm not super sexy like Mom and my sisters... but, " + player.mf("Daddy","Mommy") + ".. I will like, still please you, like, totally the best that I can!</i>\"");
		else MainScreen.text("\"<i>W-well, what did you think, " + player.mf("Dad","Mommy") + "?  Did I do a good job?  I know I'm not as hot as Mom, but I promise I'll please you as best I can!</i>\"");
		MainScreen.text(" she says, wanting more than anything to be able to please you.");
		...
		if(flags[FlagEnum.DAUGHTER_FOUR_BIMBO] > 0 && flags[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] == 0) MainScreen.text("\"<i>Come on " + player.mf("Daddy","Mommy") + ", can't you see how totally hot I am?  Don't you just want to let loose and like, just fuck me till I scream.</i>\"");
		else MainScreen.text("\"<i>Just look at me, " + player.mf("Daddy","Mommy") + ", so curvy, so ready, so hot and wet for you.  Just let loose and show me your love, show me how much you love me, " + player.mf("Daddy","Mommy") + ", fuck me hard, make me scream and claim me as yours.</i>\"");
		MainScreen.text(" the horny birdgirl says with a lick of her plump, kissable lips, aching hunger for your incestuous seed.  She wraps her arms around you, pulling her body against yours and sliding a long downy leg around you.  Her position opens her up to you and presses her sodden sex against your crotch.  She's such a lusty, lovely woman, and she wants you so badly, her body begging for you to take her and to seed her fertile body with your potent virility.");
		...
		//bimbo speech
		if(flags[FlagEnum.DAUGHTER_FOUR_BIMBO] > 0 && flags[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] == 0) MainScreen.text("  \"<i>Ahh... ohhh... oh, " + player.mf("Daddy","Mommy") + "!  You like my butt, don't you; isn't it nice?  It's so good to grab and squeeze.  I bet it would be fun to, like, spank it too....</i>\"");
		//[normal speech:]
		else MainScreen.text("  \"<i>Mmmm... Oh, " + player.mf("Daddy","Mommy") + ", yessss...  That's it, touch me, touch me and play with me, do whatever you want with me.</i>\"");
		MainScreen.text(" she coos into your ear, temping you even more with the taboo of her incestuous flesh.  With such a demanding offer of pleasure, your daughter so open and ready to receive your pleasure, how could you say no to such a lovely girl?");
	}
	menu();
...
private phaseTwoOfIncest(daughter: number):void {
	...
	if(daughter == 2) MainScreen.text("massive, round, spankable ass");
	else MainScreen.text("gropable, soft ass");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) {
		MainScreen.text("  Your [chest], free from the confines of your gear");
		//if small bust:
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 3) MainScreen.text(", perk up as the cool air brushes past your hard, aroused nipples.");
		else if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 10) MainScreen.text(", bounce perkily, nipples hard as your handful of breast flesh touches the cool air.");
		else MainScreen.text(", spring forth in a wave of soft, squeezable bustiness; the kind of breasts one could curl up with and use as a pillow.");
	}
	...
	if(flags[FlagEnum.DAUGHTER_FOUR_BIMBO] > 0 && flags[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] == 0)
	
	let bimbo:boolean = (((daughter == 4 && flags[FlagEnum.DAUGHTER_FOUR_BIMBO] > 0) || (daughter == 3 && flags[FlagEnum.DAUGHTER_THREE_BIMBO] > 0) || (daughter == 2 && flags[FlagEnum.DAUGHTER_TWO_BIMBO] > 0) || (daughter == 1 && flags[FlagEnum.DAUGHTER_ONE_BIMBO] > 0)) && flags[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] == 0);
	
	if(bimbo) MainScreen.text("\n\n\"<i>Yessss!  Please, " + player.mf("Daddy","Mommy") + "!  Give it to me!  Like, fuck my brains out till I can't think of anything but your, like, totally amazing cock!</i>\"");
	...
	//[cum volume low:
	if(player.cumQ() < 250) MainScreen.text("  Squirting over and over into your sweet, lusty girl, you inseminate your fertile daughter, spilling as much of your cream right into her needy, hungry womb as possible.");
	//cum volume medium:
	else if(player.cumQ() < 500) MainScreen.text("  Gushing over and over into your sweet, lusty daughter, you hold yourself tighter against her, making sure you force as much of your spunk into her waiting, hungry womb as you can.");
	//[cum volume high:
	else if(player.cumQ() < 2000) MainScreen.text("  Gush after gush of thick rich cum flows from you, basting your daughter's womb with your virile spunk.  Wedging yourself as tightly against her as you can, you make sure that every drop packs into her fertile belly.  The potent flow of your seed quickly swells her stomach into a cute cum bloated paunch.");
	else MainScreen.text("  Your seed floods your daughter's tight, squeezing cunt like a flood, wave after wave of your thick, frothy spunk forces its way through her convulsing cunt and right into her eager, waiting womb.  Geysers of thick, frothy spunk force their way into her womb, filling it fuller and fuller until her belly swells and swells.  Her once tight, flat tummy rounds out into a healthy round cumbaby, your thick, yogurt-like spunk overflowing from her overstuffed snatch in thick, sticky streams.");
	MainScreen.text("  In a moment of lucidity, you look down at your daughter and see the blissed-out look on her face; her creamy skin red; her eyes rolling up into her head.  Her tongue is lolling out from her plump, cocksucking lips.  The sight of your daughter consumed by such mind bending pleasure, the ecstasy of having her " + player.mf("father","mother") + " cumming inside her, only gives you a sense of pride and affection for your sweet daughter.");
	...
	if(bimbo) MainScreen.text("  \"<i>Oh, " + player.mf("Daddy","Mommy") + ", oh, " + player.mf("Daddy","Mommy") + "...  So good!  My " + player.mf("Daddy","Mommy") + "'s such a hot, sexy " + player.mf("stud","herm") + ".  I so totally thought I was going to like, lose my mind for a second.  I love you, " + player.mf("Daddy","Mommy") + ".</i>\"");
	else MainScreen.text("  \"<i>Oh, " + player.mf("Daddy","Mommy") + ", oh, " + player.mf("Daddy","Mommy") + "... so good, you're such a fucking " + player.mf("stud","perfect breeder") + "!  I could just get addicted to you.  I love you, " + player.mf("Daddy","Mommy") + ",</i>\"");
	MainScreen.text(" she says to you as she kisses you with her soft lips.  Chuckling at your affectionate daughter, you snuggle up with her, spending some sweet, comfortable pillow time with your freshly seeded girl.");
