package classes.Scenes.NPCs
{
	...
	public class Clara extends Monster
	{
		...
		protected claraDrugAttack():void {
			...
			Render.text("Clara suddenly snatches something from a pouch at her belt. \"<i>Try this, little cutie!</i>\" She snarls, and throws a vial of potion at you.", false);
			//Dodge chance!
		...
		{
			//[cocked PCs only] 
			if(rand(3) == 0) Render.text("Clara hesitates, then lifts up her dress and shows you her womanhood.  Then she slowly utters, \"<i>You know, I’m still a virgin.  You’d be the first thing to ever enter inside this hole, something that Marble never could have offered you.</i>\"  What would it be like, you wonder for a moment, before catching yourself and trying to focus back on the fight.");
			else if(rand(2) == 0) Render.text("Clara seems to relax for a moment and bounces her breasts in her hands.  \"<i>Come on, you know how good it is to drink cow-girl milk, just give up!</i>\" she coos.  Despite yourself, you can’t help but remember what it was like, and find yourself becoming aroused.");
			else Render.text("Instead of attacking, Clara runs her hands up and down her body, emphasizing all the curves it has.  \"<i>You were made to be the milk slave of this, stop fighting it!</i>\" she says almost exasperated.  Even so, you find your gaze lingering on those curves against your will.");
			Render.text("\n");
		...
		{
			Render.text("Clara glares at you, clearly being worn down.  Then strange lights start dancing around her hand and she points it in your direction.");
			//Successful: 
		...
		override protected performCombatAction():void
		{
			...
			if(player.findStatusAffect(StatusAffects.ClaraCombatRounds) < 0) player.statusAffects.add(new StatusAffect("ClaraCombatRounds",1,0,0,0)));
		...
		public defeated(hpVictory:boolean):void
		{
			...
			if(player.statusAffects.has("ClaraFoughtInCamp") && player.statusAffects.get("ClaraCombatRounds").value1 >= 10) {}
				...
				clearOutput();
				//PC wins via health
				if(HP <= 0) Render.text("The pissed off cowgirl finally collapses to the ground.  She tries to stand up again, but finds that she can’t.  \"<i>Noo!</i>\" she cries out in frustration, \"<i>You were the perfect slave!  We were meant to be toogether!</i>\"\n\n");
				//PC wins via lust
				else Render.text("The fury and anger finally give out to the overwhelming lust that you’ve help Clara feel.  She can’t fight anymore, and falls onto her backside.  She starts feeling herself up, and desperately asks you to fuck her.\n\n");
			}
