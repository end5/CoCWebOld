...
private function RaphaelDressPtII():void {
	...
	//({If player has tail}
	if(player.tailType > TAIL.NONE) outputText("  Your tail peeks out through the cut.", false);
...
private function RaphaelEncounterIIDressFollowup():void {
	...
	//({If player does not meet the first encounter requirements:}
		...
		//({If player has above E cup breasts} 
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 7) outputText("\"<i>Your female curves... replaced with such... udders!</i>\" He looks at your bosom. \"<i>No woman could be elegant with such monstrosities up front!</i>\"\n\n", false);
		//({If player has below C cup breasts} 
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 3) outputText("\"<i>Your female curves... gone!</i>\" He looks at your bosom. \"<i>It's hard to tell you apart from a little girl!</i>\"\n\n", false); 
		//({If player has grown less than girly hips}
		if(player.lowerBody.hipRating < 6) outputText("\"<i>What happened to that fine hourglass shaped figure? Those comely hips?</i>\"\n\n", false);
		//({If player has gotten a massive butt} 
		if(player.lowerBody.butt.buttRating >= 13) outputText("\"<i>Oh... my... Marae! " + player.short + ", look at your butt. It is so big! You look one of those cat guy's girlfriends. Who understands those cat guys? You look like a total prostitute. I mean, your butt. It's just so big. I can't believe it's just so round and so out there! Gross!</i>\"  The fox shakes his head and breaks it down. \"<i>I hate big butts! So vulgar.</i>\"\n\n", false); 
		//({If female player has gotten bigger than 6 feet}
		...
		//({If player has grown ANY cock and balls}
		if(player.lowerBody.balls > 0 || player.lowerBody.cockSpot.count() > 0) outputText("\"<i>What is that bulge below the tight outfit I gave you?</i>\" The fox inspects your groin. \"<i>No, never mind. I don't want to know.</i>\"\n\n", false); 
		//(For now: 
		//({If player has lost all gender}
...
private function RaphaelChooseFrisk():void {
	...
	//({Int or Spe between 24-36 and Cor is not higher than that}
		...
		if(player.stats.cor <= 15) outputText("You have the stronge urge to grab hold of the oddly enlarged bulge between his legs and retrieve the necklace from within his pants, but the lewdness of the gesture keeps you from it.\n\n", false);
...
private function thieveryEnding():void {
	...
	if(flags[FlagEnum.RAPHAEL_RAPIER_TRANING] != 4) outputText("You try to reach for the magnificent ornament hanging off his hips, but as the fox continues to move, you lack the nimbleness to remove it.\n\n", false);
...
private function QuiksilverFawkesPtII():void {
	...
	if(flags[FlagEnum.REJECTED_RAPHAEL] == 1) outputText("<i>\"I can smell traces of him,\"</i> Hati states, nearing the waterbarrel, <i>\"but it's hard to tell.  There're too many other scents here.\"</i>\n\n", false);
...
private function coverForRaphael():void {
	...
	//({If cor is below 30}
	if(player.stats.cor < 30) outputText("  You slowly shake your head, tortured by the trusting twinkle in the wolf's soft yellow eyes.", false); 
	outputText("\n\n", false);
