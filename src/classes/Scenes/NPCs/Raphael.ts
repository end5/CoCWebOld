...
//- Player does not have a cock or balls, for now
		...
		public timeChange():boolean {
			...
			if (flags[FlagEnum.RAPHAEL_DRESS_TIMER] > 1 && player.stats.gems >= 5) flags[FlagEnum.RAPHAEL_DRESS_TIMER]--;
			if (flags[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] > 1 && player.stats.gems >= 5) flags[FlagEnum.RAPHEAL_COUNTDOWN_TIMER]--;
			//Fix 'hangs' - PC is at the bottom of the dress countdown
			if (flags[FlagEnum.RAPHAEL_DRESS_TIMER] == 1 && flags[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] == 0 && RaphaelLikes()) flags[FlagEnum.RAPHAEL_DRESS_TIMER] = 4;
			return false;
		}
	
		public timeChangeLarge():boolean {
			if (checkedRussetRogue++ == 0 && model.time.hours == 6 && flags[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] >= 0 && player.hasKeyItem("Camp - Chest") >= 0 && player.stats.gems >= 5 && player.statusAffects.get("TelAdre").value1 >= 1) {
				/*trace("RAPHAEL FINAL COUNTDOWN: " + flags[FlagEnum.RAPHEAL_COUNTDOWN_TIMER]);
				trace("RAPHAEL MET: " + flags[FlagEnum.RAPHAEL_MET]);
				trace("RAPHAEL DRESS TIMER: " + flags[FlagEnum.RAPHAEL_DRESS_TIMER]);
				trace("RAPHAEL DISGUSTED: " + flags[FlagEnum.RAPHAEL_DISGUSTED_BY_PC_APPEARANCE]);*/
				if (flags[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] == 0) { //Countdown to finale not currently engaged!
					//If the PC meets his criteria!
					if (RaphaelLikes()) { //Not yet met!  MEETING TIEM!
						if (flags[FlagEnum.RAPHAEL_MET] == 0) {
							Render.text("<b>\nSomething unusual happens that morning...</b>\n");
							doNext(meetRaphael);
							return true;
						}
						else { //Already met!
							if (flags[FlagEnum.RAPHAEL_DRESS_TIMER] == 0 && flags[FlagEnum.RAPHAEL_SECOND_DATE] == 0) { //Not given dress yet
								Render.text("<b>\nSomething unusual happens that morning...</b>\n");
								doNext(RaphaelDress);
								return true;
							}
							//Dress followup - Call picnic date prologue!
							if (player.armor == armors.R_BDYST && (flags[FlagEnum.RAPHAEL_DRESS_TIMER] > 1 && flags[FlagEnum.RAPHAEL_DRESS_TIMER] <= 4)) {
								Render.text("<b>\nSomething unusual happens that morning...</b>\n");
						...
						//Finale!
						if (flags[FlagEnum.RAPHAEL_DRESS_TIMER] == 1) {
							flags[FlagEnum.RAPHAEL_DRESS_TIMER] = -1;
							flags[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] = 7;
						}
						//PC get ready for the 2nd encounter and hasn't been
						//shot down yet?
						if (player.armor == armors.R_BDYST && flags[FlagEnum.RAPHAEL_DISGUSTED_BY_PC_APPEARANCE] == 0) {
							Render.text("<b>\nSomething unusual happens that morning...</b>\n");
							doNext(RaphaelEncounterIIDressFollowup);
							return true;
						}
				...
				else if (flags[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] == 1) { //FINALE
					Render.text("<b>\nSomething unusual happens that morning...</b>\n");
...
private meetRaphaelPtII():void {
	Render.text("", true);
	Render.text("Suddenly, Raphael's features grow soft and surprised as he looks down upon you. You get the feeling he's eyeing you up and catching a peek at your cleavage, but you can't be sure.\n\n", false);

	Render.text("\"<i>Marae must have cursed me for my audacity, for I am growing blind even at my young age.</i>\" The fox puts his hand to his forehead and pretends to faint. He rights himself just before he hits the ground, however and lands in a kneel before your feet.\n\n", false);
	
	Render.text("You can't help but smile a little at the amount of theatrical flourish.\n\n", false);

	Render.text("\"<i>Here I thought I had searched the entire camp, found every treasure, pilfered every gem...</i>\" He states while sauntering towards you in a disarming, wide stride. \"<i>... but it seems I've overlooked the greatest jewel of all!</i>\" Raphael kneels before you, taking you by the hand and planting a kiss upon it. \"<i>Can you ever forgive me for my blindness, my fair lady?</i>\" He takes your hand in both his paws, while looking deeply into your eyes. His own are a deep emerald green, contrasting sharply with his bright red coat. They are set below a sturdy brow that gives him playful maturity and a rough regal elegance.\n\n", false);
...
private RaphaelFirstMeetingSLAP():void {
	Render.text("", true);
	Render.text("With an offended scowl you throw a flat palm across his face. You make sure to catch his nose and sharp snout, sending him a clear message.\n\n", false);

	Render.text("It takes Raphael completely by surprise. The fox keeps his head in the wake of the blow for a good five seconds in disbelief, but tries once more when he recovers.\n\n", false);

	Render.text("\"<i>I can assure you señorita, that I had no intention of tainting your honor.</i>\" He pats your hand.\n\n", false);

	Render.text("This time you wrestle your hand from Raphael's hold and throw him the heavier back of your hand, sending him to fall on his back. You're actually getting angry at him. How dare he sneak into your camp, try to rob you and then seduce you! By the time it occurs to you to actually apprehend him, Raphael has already beaten a hasty retreat by hopping back up on the wall.\n\n", false);
	...
	Render.text("Raphael curtsies and tips his hat before making his escape. \"<i>You have my condolences.</i>\"\n\n", false);

	Render.text("You resolve to wash your hand. You're sure you've not seen the last of the russet rogue, but it will be time in coming before that happens with the severity of your rejection.", false);
	
	//{Game Removal}
	//No more meetings + endgame in 21 days
	flags[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] = 0;
	flags[FlagEnum.REJECTED_RAPHAEL] = 1;
	doNext(playerMenu);
}
...
private RaphaelFirstMeetingSWOON():void {
	Render.text("", true);
	Render.text("You snicker softly, shift your weight on one leg and blush a little. He's quite the charmer; almost good enough to forgive him for robbing you. The attention he showers you with doesn't leave you cold either.\n\n", false);

	Render.text("\"<i>Normally I rob opulent merchants and criminals. I have no idea what drew me into your camp. Perhaps destiny willed this fateful meeting?</i>\" He looks up and burrs with a suave accent. \"<i>Then again, I am attracted to extraordinary splendor.</i>\" He gazes at you with an emerald shimmer in his rich green eyes.\n\n", false);

	Render.text("Curious, you finally ask him who he really is.\n\n", false);

	Render.text("\"<i>Why I am Raphael!</i>\" He lisps affectionately and rises slowly. \"<i>Adventurer extraordinaire, redistributors of misplaced wealth and connoisseur of all things fine in life.</i>\" He strides into camp with the kind of respectful confidence that makes the place his own. He pops open the tightly locked trunk with the mere kick of the boot and places his ill-gotten gains back into your belongings. \"<i>The orphanage will have to wait. Tonight, I will regale the small ones with tales of exceptional beauty instead.</i>\"\n\n", false);

	Render.text("A faint, nondescript sound is heard and you instantly pull away from the surreality of your little meeting, to realize others might not be so smitten by the apparently world famous thief. What if others discover him? Your mind races to come up with excuses for your illicit rendezvous, but clearly, you're more worried about Raphael's presence than he is. He merely smiles at the noise, keeps his cool and takes you by the hand. One more time he plants a kiss on top of it by bending through crossed knees.\n\n", false);

	Render.text("\"<i>For now, do not worry yourself my shining jewel. The russet rogue never forgets a mark. I will answer all your questions in good stead, but for now, patience. I can already tell a woman like you is deserving of delicacy and finesse, like the blooming rose needs nurturing and time to reveal her innermost beauty.</i>\"\n\n", false);

	Render.text("In a blink of an eye, the red fox jumps back up the wall. \"<i>We will meet again!</i>\" He exclaims in a hushed tone, while slinking over to the other side of the wall.\n\n", false);

	Render.text("You hold the hand he touched close to your chest.", false);
...
private RaphaelFirstMeetingTALK():void {
	...
	Render.text("You squint your eyes at him, pulling on your arm to wrestle it from his hold. When you remark he stole from you and ask him to return your goods, Raphael simply throws you a vulpine smirk.\n\n", false);

	Render.text("\"<i>Mere souvenirs fair blossom, to remind me of your beauty!</i>\" He schmoozes and strides into camp with the kind of confidence that makes him own the place. He pops open the tightly locked trunk with the mere kick of a boot and places his ill-gotten gains back into your belongings. From within the large sack, he pinches a small pouch of your gems and quickly spirits it away. With his lightning quick fingers, you see him place it between his shoulders.\n\n", false);

	Render.text("You insist he return all of it, but the Russet Rogue merely smiles as he walks back towards you. \"<i>My, whatever do you mean, pretty señorita? Are you accusing me of being less than forthright?</i>\" He raises his shoulders.\n\n", false);

	Render.text("Tiring of his charades, you reach behind his neck to retrieve the pilfered pouch, but when you do, there is none to be found. It's the only place he could have put it, but it's gone and he isn't using his arms! You frown. It's clear that the fox is as good as his reputation. He's hiding the pouch with the skill of a pickpocket. You begin to consider how he does it, but before you do, the fox comments on your body posture.\n\n", false);

	Render.text("\"<i>My, aren't we frisky. What happened to foreplay? Shouldn't you be buying me dinner before you ravish me? Breakfast perhaps?</i>\" Raphael clucks. You jump away as you realize you've had your hands all over his body.\n\n", false);

	Render.text("\"<i>No, my lady. If you ever want to inspect these particular goods, we will have to meet again!</i>\"  Raphael hops up the same wall, spinning about on one leg. \"<i>Do not worry yourself, my fair flower. The russet rogue never leaves a lady wanting. For now, patience. I can already tell a woman like you is deserving of delicacy and finesse, like the blooming rose needs nurturing and time to present her full glory.</i>\"\n\n", false);

	Render.text("In a blink of an eye, the red fox jumps back up the wall. \"<i>We will meet again!</i>\" He exclaims in a hushed tone, while slinking over the wall to land on the other side.\n\n", false);
...
private RaphaelDress():void {
	Render.text("", true);
	Render.text("A small pebble hits the ground near you, waking you up. When a second one hits, you're sure someone is trying to draw your attention.\n\n", false);

	Render.text("Rubbing your eyes, you pull yourself out of bed, wondering what's going on. Sticking your head through the front wall of your tent, you take a curious peek outside, but find no one around.\n\n", false);

	Render.text("The first thing you do is open your storage chest, to see if the Russet Rogue has robbed you again. This doesn't appear to be the case, and instantly, your eye is drawn to an addition instead of a subtraction from its contents. You seem to have gained a new outfit! Upon a carefully folded fabric of rose red color, a note is left below a gorgeous ruby pendant upon a golden setting and carried by a filigree chain. Curiously, you open the note and read it.\n\n", false);

	Render.text("\"<i>I happened across this beautiful ensemble and was instantly reminded of the one thing in Mareth that makes its splendor pale in comparison. Fair trappings, to fit around a beautiful body.\n\n", false);

	Render.text("Yours faithfully,\n  - The Russet Rogue\n\n", false);

	Render.text("P.S. The clothes are my gift to you. I would delight to see you wear it whenever you desire to meet. Ownership of the ruby however, remains to be seen.</i>\"\n\n", false);

	Render.text("You hold the ruby pendant to the light. It must be priceless. Its crimson shimmer is enough to make you forget to ask yourself where Raphael could have gotten it from. You wonder what he means by ownership. It very clearly is in your possession now.\n\n", false);

	Render.text("Soon enough you put it down and begin to inspect the equally lustrous garment he left you. You unfold it and marvel at the Bordeaux colored clothes.", false);
	//nxt page
...
private RaphaelEncounterIIDressFollowup():void {
	...
	flags[FlagEnum.RAPHAEL_SECOND_DATE] = 1;
	//Clear dress countdown.  Its over and done with.
	flags[FlagEnum.RAPHAEL_DRESS_TIMER] = 7;
	Render.text("", true);
	
	Render.text("You awake to the soft patter of footsteps moving away from you. For a second you think nothing of it, but soon awake to the realization you might have been robbed again. When you sit up and notice a weight off your chest, you realize someone has made off with the priceless ruby pendant Raphael gifted you earlier. They swiped it straight off your neck!\n\n", false);

	Render.text("You rush out of your tent, but when you look around and spot something red lying on the small ruined wall on the outskirts of your camp, you realize that the situation isn't as urgent as you had feared. You begin to understand what the wily fox meant to imply with uncertain ownership of the pendant.\n\n", false);

	Render.text("Curious, you amble towards the Russet Rogue. Raphael, this time armed with a picnic basket and a bottle of fine wine, makes a nonchalant impression as he lies leisurely on top of the wall. His tail flicks about playfully, while he swirls a small amount of wine within a crystal glass. At first he looks at the fluid casually and takes a sip, before rolling his head sideways to look down upon you.\n\n", false);

	//({If player still meets the first encounter requirements:} 
	if(RaphaelLikes()) {
		Render.text("\"<i>How long ago was it, that you had a decent breakfast, hhhmmm?</i>\" He smirks. \"<i>One that didn't include imp gut and hellhound testicles. A worn out camp like this one is no place for a lady. Let me at least endeavor to give a woman of your caliber a taste of the good life.</i>\"\n\n", false);

		Render.text("When you inspect the rest of his body, it's clear that Raphael is still the sly fox. Your ruby pendant hangs from his belt. He has once again stolen it from you after gifting it earlier, like it was some game. When he notices your interest, he flicks his tail over it, causing the necklace to disappear.\n\n", false);

		Render.text("\"<i>The temporary price of admission for a moment of wonder.</i>\" He assures you. \"<i>Trust me that it'll be worth it. Join me, and I might even teach you the tricks of the trade. That is, unless you're cunning enough to frisk me for it.</i>\" He smiles playfully.\n\n", false);

		Render.text("What do you do?", false);
		flags[FlagEnum.RAPHAEL_DISGUSTED_BY_PC_APPEARANCE] = 0;
		//[Reject] [Frisk] [Date]
	...
	//({If player does not meet the first encounter requirements:}
		...
		Render.text("When he catches sight of you, he spurts out the sip.\n\n", false);

		Render.text("\"<i>Mon Dieu!</i>\" he states in shock and stands up on top of his wall.\n\n", false);

		Render.text("\"<i>What terrible tragedy! The land has taken its toll on the once so beautiful.</i>\" He looks down on you.\n\n", false);

		//({If player has no legs, or a centaur body.}
		if(player.lowerBody == LOWER_BODY.NAGA || player.lowerBody == LOWER_BODY.CENTAUR || player.lowerBody == LOWER_BODY.GOO || player.lowerBody == LOWER_BODY.PONY) 
			Render.text("\"<i>You're missing half your body!</i>\" He refers to your morphed legs.\n\n", false);
		//({If player has above E cup breasts} 
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 7) Render.text("\"<i>Your female curves... replaced with such... udders!</i>\" He looks at your bosom. \"<i>No woman could be elegant with such monstrosities up front!</i>\"\n\n", false);
		//({If player has below C cup breasts} 
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 3) Render.text("\"<i>Your female curves... gone!</i>\" He looks at your bosom. \"<i>It's hard to tell you apart from a little girl!</i>\"\n\n", false); 
		//({If player has grown less than girly hips}
		if(player.lowerBody.hipRating < 6) Render.text("\"<i>What happened to that fine hourglass shaped figure? Those comely hips?</i>\"\n\n", false);
		//({If player has gotten a massive butt} 
		if(player.lowerBody.butt.buttRating >= 13) Render.text("\"<i>Oh... my... Marae! " + player.short + ", look at your butt. It is so big! You look one of those cat guy's girlfriends. Who understands those cat guys? You look like a total prostitute. I mean, your butt. It's just so big. I can't believe it's just so round and so out there! Gross!</i>\"  The fox shakes his head and breaks it down. \"<i>I hate big butts! So vulgar.</i>\"\n\n", false); 
		//({If female player has gotten bigger than 6 feet}
		if(player.tallness > 72) Render.text("Raphael rolls his eyes across your giant body and looks intimidated. \"<i>I can forget about remaining unseen when I take someone of your size somewhere!</i>\"\n\n", false);
		//({If female player has gotten smaller than 4 feet}
		if(player.tallness < 48) Render.text("Raphael squints like he has trouble seeing you from there, because of your dimunitive size. \"<i>I've dated goblins once... didn't work out.</i>\"\n\n", false); 
		//({If player has grown ANY cock and balls}
		if(player.lowerBody.balls > 0 || player.lowerBody.cockSpot.count() > 0) Render.text("\"<i>What is that bulge below the tight outfit I gave you?</i>\" The fox inspects your groin. \"<i>No, never mind. I don't want to know.</i>\"\n\n", false); 
		//(For now: 
		//({If player has lost all gender}
		if(player.gender == 0) Render.text("\"<i>There's... something different about you today. Your smell, it has changed.</i>\"\n\n", false);

		Render.text("The fox looks dissapointed. \"<i>Beauty is in the eye of the beholder, but it certainly isn't gracing mine right now.  señorita... or what remains of you, please clean yourself up. Meanwhile, I just remembered: I have a sick mother to take care of. I hope you'll excuse me!</i>\" he mentions before hopping back of the wall and making a hasty retreat.\n\n", false);
				
		Render.text("You clench your jaw as he vanishes, more than a bit offended.", false);

		//{Game removal untill the PC complies with the requirements again.})
		doNext(playerMenu);
		flags[FlagEnum.RAPHAEL_DISGUSTED_BY_PC_APPEARANCE] = 1;
...
private RaphaelSelectDate():void {
	Render.text("", true);
	Render.text("You smile bashfully, not expecting this sort of gesture from anyone in Mareth. You hardly care about the pendant and you nod at him.\n\n", false);

	Render.text("\"<i>Ha-ha!</i>\" Raphael hops to his feet on top the wall and gloats at you. \"<i>You're sure you're ready for this journey? Because I'm going to show you the time of your life!</i>\" He kneels down on top his wall and extends you his hand on approach.\n\n", false);

	Render.text("It almost feels like you're consenting when you reach for his soft, cushioned padded furry paw, after which he pulls you up and helps you over the wall.", false);
	//{Unlocks Picnic}
...
private RaphaelChooseReject():void {
	Render.text("",true);
	Render.text("You sigh at the fox and shake your head. You're not interested in him or his advances.\n\n", false); 

	Render.text("He flicks his tail past his body again and within the blink of an aye, the glass of wine is exchanged for the stolen pendant.\n\n", false);

	Render.text("\"<i>The Russet Rogue is no thief, señora.</i>\" He states seriously and tosses the bauble back at your feet. You drop your frown.\n\n", false);

	Render.text("\"<i>Such a pity though... the flower who refused to bloom.</i>\" He stands atop his crumbled wall. \"<i>Take my advice fair lady. Life is too short and this world too fleeting, for missed opportunities. Let your hair drop, loosen your guard and enjoy the finer things in life.</i>\" He nods and winks at you. \"<i>You might discover it might not be so bad.</i>\"\n\n", false); 

	Render.text("In the two seconds it takes for you to pick up the pouch, you find that the fox has vanished as though he were never there. Amazed, you climb onto the wall and try to figure out where he'd gone that fast. Peeking over the wall confirms your suspicions of him simply letting himself fall off the wall to make an elegant exit. A single shattered glass can be found on the other side, but no trace of the fox.", false);
	//{Game Removal.}
	flags[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] = 14;
	flags[FlagEnum.REJECTED_RAPHAEL] = 1;
	doNext(playerMenu);
}
...
private RaphaelChooseFrisk():void {
	flags[FlagEnum.FUCK_YOU_GOT_MINE_RAPHAEL] = 0;
	Render.text("", true);
	Render.text("You flash a playful smile and express the desire to get your jewelry back.\n\n", false);

	Render.text("Challenged, Raphael hops from his perch and leans against the wall like a misbehaved rascal at your approach. With his back towards the boulders, he has his hands behind his head and one foot up against the stacked cobblestone; braving his body like he's eager for your touch. \"<i>My, whatever are you talking about, señorita? Know something I don't?</i>\"\n\n", false); 

	Render.text("Noticing how Raphael leaves more room between his hands and his head than necessary, you reach beind his neck to retrieve the pilfered pendant, but when you do, there is none to be found. It's the only place he could have put it, but it's gone and he isn't using his arms!\n\n", false);
	
	/// Int/Spe Variables ///
	//({Int or Spe below 24 and Cor is not higher than 19} 
	if((player.stats.int < 24 && player.stats.spe < 24) && player.stats.cor < 19) {
		Render.text("You frown, befuddled and eyeing the body you hold close to yours. He must be a magician, you conclude.\n\n", false); 

		Render.text("When you look up again, you realize you have your arms around his neck. His verdant green eyes peer into yours daringly and before the awkward moment has a chance to pass, the russet rogue steals a kiss. Putting the tip of his snout on your lips, he gives you a kiss more passionate and skilled than you thought possible with a sharp muzzle such as his. He discontinues before you have the time to either enjoy it or feel intimidated by it.\n\n", false);

		Render.text("\"<i>Forgive me...</i>\" He growls softly with low tone. \"<i>You're just so cute when consternated.</i>\"\n\n", false); 

		Render.text("\"<i>But the day is young!</i>\" He proclaims before slipping from your hold and up the wall, extending you his paw. \"<i>Join me! Together we shall paint the forest russet red!</i>\" He winks at you while pulling you up. \"<i>Maybe I could even show you how I do it, huh?</i>\"\n\n", false);
		//{Next scene picnic}
		doNext(RaphaelPicnic);
	}
	//({Int or Spe between 24-36 and Cor is not higher than that}
		...
		Render.text("Not outdone through wit, you notice how the fox is moving his tail about. The wileful rogue is using his fifth limb to move the pendant about his body! You quickly reach around and pat him across the back and flanks, but inspecting the tip of his tail leaves the pendant nowhere to be found. By then Raphael is moving his arms and legs to misplace it further. In a game of cat and mouse all through his red fur, you follow the movement from his lower back, to his feet and his thighs. The sly vulpine clearly enjoys leading you on your hunt across his skin and supple deerskin attire, as you graze over taut leather spanned across a contoured, masculine body. Raphael always appears one step ahead. That is, until the trail ends near his crotch.\n\n", false);

		//({If player corruption is below 15} 
		if(player.stats.cor <= 15) Render.text("You have the stronge urge to grab hold of the oddly enlarged bulge between his legs and retrieve the necklace from within his pants, but the lewdness of the gesture keeps you from it.\n\n", false);
		//{If player corruption is at or higher than 15} 
		else Render.text("With no intention to relent, you grab him by the oddly enlarged bulge in his tight leather pants and squeeze softly, discovering how several of his jewels are harder than others. He hid your gem amongst his own!\n\n", false);

		Render.text("\"<i>My, aren't we frisky. What happened to foreplay? Shouldn't you be buying me dinner before you ravish me? Breakfast perhaps?</i>\" Raphael clucks. \"<i>Although frankly.</i>\" He growls softly with low tone. \"<i>I would gladly suffer through a thousand indignities, for a mere touch from one such as you.</i>\"\n\n", false);
		
		Render.text("\"<i>But the day is young!</i>\" He proclaims before slipping from your hold and up the wall, extending you his paw. \"<i>Join me! Together we shall paint the forest russet red!</i>\" He winks at you while pulling you up. \"<i>You have skill though. Stick with me and maybe I can teach you.</i>\"\n\n", false);
		//{Next scene picnic}
		doNext(RaphaelPicnic);
	}
	//({Int or Spe are above 35 and Cor is not higher than that}
	else if(player.stats.cor < 35) {
		Render.text("Figuring it out easily, you notice how the fox is moving his tail about. The sneaky rogue is using his fifth limb to move the pendant about his body! Deducing the only way he could do that without moving his arms, you reach for the heel of his lifted foot and intercept the string before he can pass it onto his leg.\n\n", false);

		Render.text("\"<i>Nice try!</i>\" You comment with a triumphant smile, when you quickly retrieve the necklace and walk away from Raphael before he realizes you've outwitted him. You hug the jewelry tight.\n\n", false);

		Render.text("The fox seems like a gracious loser and the young man beams. \"<i>Well played!</i>\" He congratulates you behind your back. \"<i>You show potential. However, I urge you not to grow complacent. There's quite a few tricks that only a true master thief like I possesses. If you'd allow me, I could teach you even more about my craft, graduate you into a whole new way of living. One you never knew existed...</i>\"\n\n", false);
		
		Render.text("How do you respond?", false);
		//Reject] [Accept]
	...
	//{If player's corruption is higher than 19 and higher than Intelligence.}
		...
		Render.text("\"<i>My, such an eager little thing.</i>\" He comments, pleased.\n\n", false);

		Render.text("You ask yourself the age old question if he's just happy to see you, but you suspect differently when you reach out for the firm lump and cup it.\n\n", false);
...
private friskAcceptChoice():void {
	flags[FlagEnum.FUCK_OFF_THEIVING_RAPHAEL] = 1;
	Render.text("", true);
	Render.text("\"<i>Come on then!</i>\" He proclaims before shooting up the wall, extending you his paw. \"<i>Join me! Together we shall paint the forest russet red!</i>\"\n\n", false);

	Render.text("You waive his paw and choose to walk around the 8 foot long wall, instead of struggling to climb over it. Raphael flashes a grin of delight when you do. \"<i>I can see I've caught on to a smart one! Stick with me and I could even finish your education. I promise that graduation will be something... special.</i>\"\n\n", false); 
	//{Next scene picnic}
	doNext(RaphaelPicnic);
}
...
private friskRejectChoice():void {
	Render.text("", true);
	Render.text("You scoff, cross your arms and reject the offer by telling Raph your answer from across your shoulder, with your back still turned to him aloofly. Pleased with yourself, you add that he's nothing more than a carnie with an accent and that you won't be needing his help.\n\n", false);

	Render.text("\"<i>As you say, my desert rose. It is clear that I have no more to teach you then.</i>\"\n\n", false);

	Render.text("You startle when you hear the fox bang against your chest. When you turn around, he's once again stealing from you!\n\n", false);

	Render.text("\"<i>There's no teaching those of lesser stock a sense of grace and refinement. Those are still decidedly my domain!</i>\" He guffaws with a bag of goods across his back and runs off.\n\n", false);

	Render.text("\"<i>Even without a belt on?</i>\" You ask him, holding up the leather strap he wore across his waist a mere minute ago. You took the time to swipe it along with the pendant.\n\n", false);

	Render.text("As Raphael bolts, he has a look of surprise on his face. He doesn't last long before his pants drop to his ankles and he falls flat on his face.\n\n", false);

	Render.text("He yelps when he loses hold of the bag, which drops to the ground. The fox uses both hands to protect his modesty by holding up his pantaloons, slipping over the spilt gems a few good times and flailing about, before finally making it to the wall with nothing to show for it.\n\n", false);

	Render.text("\"<i>I will regain my honor!</i>\" He exclaims while waving his hands in a theatrical flourish, causing his pants to drop once more.\n\n", false);
	
	Render.text("You laugh as he blushes, beating a sound retreat. You're sure this won't be the last you'll see of him however.", false);
	//{Game removal}
	flags[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] = 14;
	flags[FlagEnum.REJECTED_RAPHAEL] = 1;
	doNext(playerMenu);
...
private friskFondleChoice():void {
	Render.text("", true);
	Render.text("You gently massage the leathery package. It increases in size until a distinct shape of some length forms above it. When Raphael moves his hips forward appreciatively, you look up, throw him a smile and slip your hand into his pants. He's not wearing any underwear, you notice as you rummage about. Struggling to restrain yourself, you slip past the throbbing meat of his naked cock and dig around his jewels instead, removing the ruby from behind them.\n\n", false);

	Render.text("\"<i>Such a good girl you are.</i>\" He smirks lewdly. \"<i>You have a lot of talent in those fingers. A distinct gift. There's a lot I could show a feisty little minx such as you.</i>\"\n\n", false);

	Render.text("You deftly remove the sweat stained, fogged up stones and tug them away between your breasts.\n\n", false);

	Render.text("\"<i>And I in turn, will have to retrieve those.</i>\" He growls low.\n\n", false);

	Render.text("\"<i>But the day is young!</i>\" He proclaims before shooting up the wall and extending you his paw. \"<i>Join me! Together we shall paint the forest russet red!</i>\" He winks at you while pulling you up. \"<i>Stick with me and I could finish your education. I will promise that your graduation will be... everything you hoped for. Like the wild mare is shown what is expected from her, the russet rogue never leaves a lady wanting.</i>\"\n\n", false); 
	dynStats("str", -1,"tou", -1, "spe", 3, "sen", 1, "lus", 25);
	//{Next scene Picnic}
	doNext(RaphaelPicnic);
}
...
private friskSqueezeChoice():void {
	Render.text("", true);
	Render.text("Vicious and eager to teach him some humility, you dig into the tender package and squeeze hard enough to drive the sharp edges of your pendant into Raphael's jewels. The fox immediately grabs you by the wrist, but this is one trap he isn't wriggling out of. You drive him to his knees while he lets out a high pitched, muffled squeal replete with a blank, agonized expression on his face.\n\n", false);

	Render.text("\"<i>You have some balls stealing from me fox.</i>\" You comment and grin. \"<i>Just checking. Is that too soft or should I squeeze harder?</i>\"\n\n", false);

	Render.text("Raphael shakes his head while a tear wells up in his eyes. When you roll his privates into the sharp edges, he finally lets out a cry loud enough to hear from miles away.\n\n", false);

	Render.text("You chuckle and look around you to see if anyone heard. When you look back to see him squirming between your fingers however, all you're holding onto is the ruby pendant. Raphael has pulled off an amazing switching trick and has escaped. Instead, he stands on top the wall panting and defiant.\n\n", false);

	Render.text("\"<i>I'll be sure to repay the favor.</i>\" He warns you with a high pitched voice while clasping his poor genitals. \"<i>Nobody touches my goods like that and gets away with it.</i>\"\n\n", false);

	Render.text("He raises just a single finger to the air and groans painfully. \"<i>We will meet again!</i>\"\n\n", false);
...
private RaphaelPicnicEnd():void {
	Render.text("", true);
	Render.text("The fox is a thief of more than just gems. The concept of time vanishes around him and before you know it, his antics have entertained you for nearly the entire early morning. Raphael is an amazing time sink. You notice how late it is by way of more light peeking in over the forest's treeline. You should return to the portal. It's not difficult to part with the fox however. Only now do you notice how much more anxious and guarded he has become with the increase in light, frequently looking over his shoulder. When you tell him you should be going, the young man smiles, relieved.\n\n", false);

	Render.text("\"<i>Are you sure, señorita? In your presence, I would gladly spend an eternity.</i>\"\n\n", false); 

	Render.text("You know he's just keeping up appearances and making you feel welcome, however. When you smile and assure him that you must be getting back, Raphael declares his regret one more time before literally scrambling to get away from the meadow, hastily cleaning up the picnic. He doesn't even bother to put things back in the basket. He just takes the four corners of the blanket and folds it into a bag slung over the shoulder. A bit perturbed, you look after him as he slips into the treeline.\n\n", false);
	
	Render.text("However, just before he vanishes, the fox gives you a wink and a bow. \"<i>My Lady, it has been a true delight, and it would be to my great fortune if you'd allow me to see you again.</i>\"\n\n", false); 

	Render.text("He seems so hasty that you'd begin to question it, but your doubt is taken away as the fox takes the time to stare patiently at you from the forest's edge. \"<i>How could I resist, when you're dressed and made so gorgeously?</i>\" He smiles, referring to your elegant red suit.\n\n", false); 

	if (flags[FlagEnum.FUCK_OFF_THEIVING_RAPHAEL] == 0)
	{
		Render.text("A minute of silence follows and just when it has been long enough to reassure you, he throws you something and slips away and vanishes.\n\n", false);

		Render.text("You catch the object: he has returned the priceless ruby pendant. You recollect yourself and head back to camp.", false);
	}
	else
	{
		flags[FlagEnum.FUCK_OFF_THEIVING_RAPHAEL] = 0;
		
		Render.text("A minute of silence follows and just when it has been long enough to reassure you, he makes his excuses and sets about heading back to... whatever it is the sly fox spends his days doing.\n\n", false);

		Render.text("You recollect yourself and head back to camp.", false);	
	}
	doNext(playerMenu);
}
...
private RaphaelPicnicSkill():void {
	...
	if(flags[FlagEnum.RAPHAEL_RAPIER_TRANING] == 0) {
		Render.text("You allow yourself to be distracted by the sheer opulence of the picnic. Crystal glasses, fine linen blanket, intricately woven basket... and then the clothes and pendant he put on you. Where does he get these things from? It almost leaves you jealous. While you peer across the arrangement, Raphael even manages to retrieve a small guitar. With his long slender fingers, he picks the snares and fine-tunes it. When the Russet Rogue frowns seriously and occupies himself with the small instrument - cradling in its arms focusing on its tortured plinks - you notice just how dedicated and dashing he is.\n\n", false);
		
		Render.text("Whatever else is on display however, your eye is consistently caught by the fox's rapier. Raphael has left it sheathed in a scabbard reinforced with silver upon the hard brown leather. The sword's guard sticks out from the sheath and is fashioned out of fine golden filigree that domes around a red hilt. The weapon makes you curious. It appears so delicate that you can hardly believe it to be serviceable in battle.\n\n", false);

		Render.text("When you ask Raphael if such a small weapon ever managed to impress anyone, he snaps one of the snares in shock and draws a long face. \"<i>It is not a thin, flimsy thing!</i>\" He balks in his unique accent and loses his cool for a moment. \"<i>I'll have you know it's perfectly average compared to those of others!</i>\" He defends himself with an excitable hand gesture.\n\n", false);

		Render.text("There's a moment of awkward silence as you realize the young man places more pride in his sword than you thought. However, Raphael recollects himself with a smile and chuckles at himself like he's been had by someone who just doesn't understand.\n\n", false);

		Render.text("\"<i>No, my lady. This is a man's weapon in every definition of the word.</i>\" He beams and picks it up, pulling the blade from the sheath with a cold iron swish. \"<i>Notice how its long hard length stands tall and fierce upon a sturdy hilt designed for deep thrusts.</i>\"\n\n", false);
		
		Render.text("Holding it up, it indeed gleams sensually in the sun. However, Raphael then points it downwards and directly at you, carefully tracing the point across your outfit. You can feel the cool edge glide across. \"<i>Notice the roving tip, how she searches for an opening upon a quivering body's wavering guard.</i>\"\n\n", false);
		
		Render.text("When Raphael places the point between your breasts, you cannot help but stare at the steely needle with fascination. Driven by long steel, the sharp tip feels heavy enough to slide into your flesh in a single thrust. The feeling is only stronger for wearing an outfit such as yours, leaving your body so acutely vulnerable against the outside world. You stand corrected; there is something very intimidating about it.\n\n", false);
		
		Render.text("\"<i>Could I learn to use it?</i>\" You ask Raphael, who then begins to snicker into his fist and snorts unseemly.\n\n", false);
		
		Render.text("When you don't flinch and just stare at him in earnest, the fox looks up with surprise. \"<i>Oh, you are serious about that?</i>\" He remarks and snaps out of it by adopting the coy frown typical of the wileful fox.\n\n", false);

		Render.text("When you ask him if he thinks that's weird, the vulpine rogue shakes his head. \"<i>Not at all mademoiselle. In fact, I am of the opinion that every woman of your remarkable beauty should know how to defend herself. Perish the thought of having to relinquish your womanhood to anything other than a true gentleman, or having it ravished by beasts instead. And if you must fight, I cannot think of a weapon more suited for a woman than the rapier. I would be honored to teach you.</i>\"\n\n", false); 

		Render.text("\"<i>Shall we?</i>\" He jumps to his feet and extends you an arm.\n\n", false);

		Render.text("When you ask him if you shouldn't be wearing armor instead of wafer-thin silk bodystockings, he laughs. \"<i>I assure you. For the Rapier, you are dressed perfectly.</i>\"\n\n", false); 
		//{Leads up to fencing variables}
	}
	//{Below 30 speed. PC chooses [Skill] Must play out at least once}
	if(player.stats.spe < 30 || flags[FlagEnum.RAPHAEL_RAPIER_TRANING] == 0) {
		Render.text("The two of you amble over to a mossy field. Raphael is up front and waving about his rapier playfully, grazing the tip through the flora and even slashing at a wasp. You can't seem to spot the insect flying around after he does that.\n\n", false);

		Render.text("When you reach the center of the field however, Raphael throws the weapon to the ground with temperament and tells you to prepare.\n\n", false);

		Render.text("\"<i>Aren't we going to use the weapon?</i>\" You ask him, causing him to smirk.\n\n", false);

		Render.text("\"<i>So eager.</i>\" He snarls exitedly. \"<i>Whatever happened to foreplay?</i>\"\n\n", false);

		Render.text("You try to explain that this isn't what you meant, but the fox shakes his head. \"<i>The sword is merely an extension of the rest of our body. The sharp of the edge is meaningless, when the arm that aims it and the feet that drive it suffer from inferior technique. It is... footwork.</i>\"\n\n", false);
		
		Render.text("\"<i>Observe!</i>\" He exclaims and motions you to keep your distance. For just a moment, Raphael stands still as though caught in a moment of intense preparation.\n\n", false);

		Render.text("The first thing Raphael does is to move his feet. He rapidly taps his boots down upon the moss below while he holds both arms wide in the air like a bird of prey, as though working towards a moment of instant momentum. When that moment comes, the fox turns into a true dervish. He spins about, squats down to pick up the rapier and with the long steel in hand, his swerves become only wider for it. The edge cleaves the air, until he draws it to a standstill and flicks the length around faster than you can follow it. He enacts a series of mock parries and dodges, before extending the long weapon forward with an outstretched hand and a straight spine. With the other arm far back, he adopts a posture of perfect poise.\n\n", false);
...
private fenceOfferChangeToDiscuss():void {
	Render.text("You blush, intimidated, and change the subject.\n\n", true);
...
private postRaphaelCoitus():void {
	Render.text("", true);
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00149] = 1;
	Render.text("When you wake up on a bed of soft moss, Raphael has disappeared completely.\n\n", false); 

	//({When player had reached the SPE fencing apex}
	if(flags[FlagEnum.RAPHAEL_RAPIER_TRANING] == 4) {
		Render.text("The only thing left behind is his rapier, sticking out of the moss.  He's bound it with his red sash around the length like a ribbon, as though he has now gifted it to you.  Perhaps it is his way of congratulating you.\n\n", false);
	...
	if(flags[FlagEnum.RAPHAEL_INTELLIGENCE_TRAINING] == 4) {
		Render.text("However, you realize he's left you with more than just pleasant memories of sitting with him around the picnic.  Realizing how skillfully you declined him and how deftly you led him around, you realize you may have mastered his art of keeping another's attention and leading them around with cunning and acting.  This misdirection could have great applications in battle.\n\n", false);
	...
	Render.text("You return to camp, having cleaned up the picnic and taking the rations that were left with you.  You can't help but wonder if you'll ever see him again though.\n\n", false);
	//[Removes Raph from the game. 7 days later, the Quicksilver scene plays out.]
	flags[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] = 7;
	//Next button if not taking Rapier
	if(flags[FlagEnum.RAPHAEL_RAPIER_TRANING] != 4) doNext(playerMenu);
}
...
private declinePuttingOutForRogues():void {
	Render.text("", true);
	Render.text("<i>\"No.\"</i> You shake your head, dropping the atonal monosyllable as if it were the last note in a musical play.\n\n", false);

	Render.text("The horny fox frowns.  It's like raising an invincible shield between you.  He reaches out towards your face and body, but he cannot touch anymore.\n\n", false); 

	Render.text("Some of the magic of the moment is still there in the fine young man leaning over your aroused body.  He could still take you right there and right now and you know you'd probably enjoy it too.  However, it would be unseemly.  It would be to rape an equal and to Raphael, that would deprive him too of the dignity he tries so hard to maintain.  You can tell it from his face; the agonized quandary.\n\n", false); 

	Render.text("When he gets off your body, you've won.  You've caught Raphael in his own game.  You've led him around.  He'd probably do anything for it, but cannot obtain it.\n\n", false); 

	Render.text("<i>\"Fine,\"</i> he mumbles.  He rolls off and sits sulking a few paces away from you.  It's the first time the young man fails to keep a cheerful attitude.\n\n", false);

	Render.text("You smile, a bit sorry for the way you broke him, but you try to convince him that he shouldn't mope.  The two of you had fun.  It's getting late and you occupy yourself with cleaning up the picnic.\n\n", false);

	Render.text("After a small while, the fox finally speaks.  <i>\"It seems like there isn't anything more I can teach you,\"</i> the fox claims.  <i>\"... I'm proud of you, my greatest student.\"</i>\n\n", false);
	
	Render.text("You turn around to smile at him.  However, Raphael has vanished.\n\n", false);

	//({When player had reached the SPE fencing apex}
	if(flags[FlagEnum.RAPHAEL_RAPIER_TRANING] == 4) {
		Render.text("The only thing left behind is his rapier, sticking out of the moss. He's bound it with his red sash around the length like a ribbon, like he has now gifted it to you. Perhaps it is his way of congratulating you.\n\n", false);
		//[Weapon: Rapier. Speed, instead of strength, influences the damage rating. Never as strong as the heavier weapons or sword, but works great with speed & evasion, encouraged by the rapier.])
		inventory.takeItem(weapons.RRAPIER, playerMenu);
	...
	//({When player has reached the INT Conversation apex}
	if(flags[FlagEnum.RAPHAEL_INTELLIGENCE_TRAINING] == 4) {
		Render.text("You realize he's left you with more than just pleasant memories of sitting with him around the picnic, though. Realizing how skillfully you declined him and how deftly you led him around, you realize you may have mastered his art of keeping another's attention and leading them around with cunning and acting. This misdirection could have great applications in battle.\n\n", false);
		//[Optional Perk: Misdirection. Intelligence adds to the chance to evade. Turns you into a true rogue together with the bodysuit.])
		player.createPerk(PerkLib.Misdirection,0,0,0,0);

	}
	
	Render.text("You return to camp, having cleaned up the picnic and taking the rations that were left with you.  You can't help but wonder if you'll ever see him again.\n\n", false); 
...
private RaphaelPicnicChooseThieving(newl:boolean = true):void {
	if(newl == true) Render.text("", true);
	//(Introduction; plays out only once)
	if(flags[FlagEnum.RAPHAEL_INTELLIGENCE_TRAINING] == 0) {
...
private RaphaelThieverySmexPtII():void {
	...
	if(flags[FlagEnum.RAPHAEL_INTELLIGENCE_TRAINING] == 4) {
		...
		//[Optional Perk: Misdirection. Intelligence adds to the chance to evade. Turns you into a true rogue together with the bodysuit.])
		player.createPerk(PerkLib.Misdirection,0,0,0,0);
		Render.text("(Gained Perk: Misdirection!)\n\n");
	}
	Render.text("You return to camp, having cleaned up the picnic and taking the rations that were left with you. You can't help but wonder if you'll ever see him again though.", false);
...
private quiksilverFawkesEndGame():void {
	Render.text("", true);
	Render.text("You wake up to the sound of an ominous cry in the distance, like that of a howling wolf.  It can only mean trouble and you jump out of bed.\n\n", false);

	Render.text("It is an eerily misty morning outside, with banks of fog covering your campsite.  It only makes the second howl more haunting as the danger appears to draw closer to camp.  You stand still and prepare yourself to face this menace, peering into the fog.  When a shadow creeps closer, you prime for combat.  However, its posture suggests it is weary and tired, slumping against a tree. When it struggles forward, it almost comes falling out of the curtaining mist.  It is only then that you notice the redness of its fur and the fineness of its clothes.  It is the fox and russet rogue, Raphael!\n\n", false);

	Render.text("<i>\"Please, you must help me!\"</i> he begs of you.  <i>\"They're trying to frame me!  I'm innocent!\"</i>\n\n", false);

	//({If player rejected him earlier}
	if(flags[FlagEnum.REJECTED_RAPHAEL] == 1) {
		Render.text("You're immediately confused and you ask him what's going on, but the fox is out of breath.  What's more, a similar howl now calls out from the opposite direction too!  Whatever these sounds are or this fog is, they have the camp surrounded!\n\n", false);
	}
	//({If player had an INT or SPE ending with him earlier}
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00149] == 1) {
		Render.text("You're a bit miffed at the fox.  You haven't heard from him for more than a week after he slept with you, and now he only shows up because he's looking for help.  The fox however, is out of breath.  What's more; a similar howl now calls out from the opposite direction too!  Whatever these sounds are or this fog is, they have the camp surrounded!\n\n", false); 
	}
	
	Render.text("<i>\"They hound me night and day!  I can't outrun them any longer!\"</i> he wheezes.  <i>\"Please!  You don't know what they'll do to me when they catch me!\"</i>\n\n", false);

	Render.text("You don't have the chance to respond before the sounds of snapping branches and rustling leaves near camp.  Raphael jumps up and dives into the barrel you keep in camp for the storage of water, hiding himself and leaving you to face the oncoming threats by yourself.\n\n", false);

	Render.text("In the hazy distance, you spot a massive shadow rushing through the mist and it dissapears before you get a good look.  An eerie silence follows and you sharpen your eyes and hearing to the danger.  When you look up, your heart sinks as the massive shape has jumped up and now descends on you from above.  It happens so fast that you barely realize what's going on.  Before you know it, you're sent sliding across the floor with the dark shape on top of you, pinning you down to the ground with its weight.\n\n", false); 

	Render.text("<i>\"Smelled of elderberries, huh?\"</i> it snarls.  <i>\"And what was that about my mother?\"</i> When you open your eyes again, you're greeted by a row of razor-sharp white teeth, set in a pitch-black face. Above them, two smoldering yellow eyes stare down on you, infuriated.\n\n", false);

	Render.text("<i>\"Hati!\"</i> an identical voice calls out from elsewhere.\n\n", false);

	Render.text("Suddenly, the teeth above you vanish and the eyes grow soft and surprised.  <i>\"Oh!\"</i> the shadow exclaims, its expression apologetic and startled.\n\n", false);

	Render.text("It removes itself from atop your body.  After having the breath knocked out of you, you take a moment to recover it and finally manage to get back up and meet the two monsters of the mist.", false); 
...
private coverForRaphael():void {
	Render.text("", true);
	Render.text("You stare back into the wolf's yellow eyes, keep a stiff back and answer him.  <i>\"I'm not seeing him right now.\"</i>\n\n", false);

	//({Player corruption below 30}
	if(player.stats.cor < 30) Render.text("You instantly feel bad for lying to the wolf. More so because he doesn't get troubled, disappointed or vexed.  He seems to instantly believe you... or at least, he wants to believe you, even though being a detective probably keeps him cynical.  You'll only be adding to that cynicism if he discovers your lie.\n\n", false);

	Render.text("<i>\"Cute answer,\"</i> his partner remarks, taking a break from staring at the water barrel.\n\n", false);
	Render.text("Your hair stands upright when he leans back in over the barrel and appears to look down into the water, but you struggle not to slake a sigh of relief when he merely starts drinking from it instead.  A few long laps with his tongue and the wolf is sent cringing and shaking his head at the taste.  <i>\"Even the water tastes of him.\"</i>\n\n", false);

	Render.text("<i>\"Hati, enough!\"</i> The wolf in front of you turns his head, throws back his cuneate ears and snarls at his partner.  <i>\"We're not in the habit of throwing around accusations without material proof.\"</i>  His anger starts to decrease. <i>\"Don't help Raphael turn us into the bad guys.\"</i>  Skoll turns back to you with just as much friendly patience as ever.  <i>\"I'm sure she would have told us if she knew anything about that.\"</i>", false);
	//({If cor is below 30}
	if(player.stats.cor < 30) Render.text("  You slowly shake your head, tortured by the trusting twinkle in the wolf's soft yellow eyes.", false); 
	Render.text("\n\n", false);

	Render.text("<i>\"The trail is strongest over towards the meadows.  He might have gone there.\"</i> Hati remarks after circling your camp one more time.\n\n", false);

	Render.text("<i>\"I hope you will let us know when you see him again...\"</i>  Skoll nods at you, his feral features pulling away from you.\n\n", false);

	Render.text("Skoll picks up Raphael's clothes.  The two brothers then get on all four of their limbs and speed off into the fog.  Fierce, dangerous beasts they are.\n\n", false);
	//~~~ Next page ~~~
...
private coverForRaphaelII():void {
	...
	Render.text("It doesn't take more than a minute for Raphael to make use of the wolves' departure.  Not even bothering to climb out with dignity, he tips the barrel over as he rushes out of the water.  Like a drowned cat, he climbs out coughing and gasping for air on all fours.\n\n", false); 

	Render.text("He hasn't lost his ways however.  The first words out of him are as suave and composed as ever.\n\n", false);

	Render.text("<i>\"Merci, mademoiselle,\"</i> he wheezes softly.  <i>\"You have whisked me away from the snatches of great injustice, as well as a fair share of humiliation.  Normally I am capable of outrunning them myself, but it seems that faith has blessed me with a guardian angel on the day I find myself less capable.\"</i>\n\n", false); 

	Render.text("You're upset towards him for asking you to lie to law enforcement, but Raphael's current state keeps you from being too furious.  He's not so bushy anymore.  His soaked hair falls flat against his body and causes him to lose quite a bit of his imposing physique. Strands of it run across his face. You spend a little time yelling at him, though.\n\n", false);

	Render.text("<i>\"No.  It is nothing like that,\"</i> he splutters, in response to one of your rants.  <i>\"We go way back.  The wolves and Urta, they carry out a personal vendatta against me that has no bearing on the law.\"</i>\n\n", false);

	Render.text("You spend some time considering the dubiosity of this revelation.\n\n", false);

	Render.text("<i>\"Please, señorita.\"</i>  He manages to rise to one knee.  <i>\"Is this the face of a hardened criminal?\"</i>  He looks up and into your eyes with his own brilliant green ones.  He's still a mess, hair disheveled and his features weather-beaten, but he'll dry up again eventually, to be dashing once again.  <i>\"Since when have I treated you anything less than courteously?  A handful of gems, certainly, yes, but do I deserve such police brutality because of that?  Do I deserve to be hunted down in the forests and carried through the streets in chains?  Just because my antics leave the world with a smile?\"</i>\n\n", false);

	Render.text("Before you manage to answer him, Raphael rushes back to his usual spot on the small crumbled wall on the outskirts of the camp proper.  Not as quick as previously, tired and weary perhaps, but still quite fast.  He doesn't seem so intimidating without his clothes on.  Knowing shame, unlike the wolves, the young man even covers his privates with a leg bent sideways.\n\n", false);
	
	Render.text("<i>\"Mademoiselle, this day, I owe you a great boon.  The russet rogue never forgets a debt.  Visit Tel'Adre after things die down and look for the orphanage.  You will be welcomed like a queen! Ah-ha!\"</i> he exclaims, before hopping down the wall.\n\n", false);

	Render.text("You blink as he fades off into the mist and you're left to absorb the morning's events by yourself.  For a minute you're concerned with the wolves, but at least you've managed to keep Raphael as a friend.  Though, you are curious what he means by 'orphanage'.", false);
	//Concludify things
	flags[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] = -2;
	doNext(playerMenu);
}
//{PC chooses to betray Raphael}
private betrayRaphael():void {
	Render.text("", true);
	Render.text("<i>\"The barrel,\"</i> you admit, the answer slipping from your lips in face of the wolf.\n\n", false);
	Render.text("Skoll, careful not to bare teeth, curls the corners of his lips and smiles at you.  <i>\"Thank you.\"</i>  He whispers a low growl and his eyes sparkle at you.  <i>\"I realize that might have been hard to do, but we will remember.\"</i>  Leaving you with that promise, he joins the other wolf in staring at the waterbarrel.  You feel guilty, but it's compensated by the feeling you might have gained two allies.\n\n", false); 

	Render.text("The two twins look at each other, then grin playfully.  One of them leans in over the barrel and puts his head directly over the water's surface, peering down.\n\n", false);

	Render.text("Suddenly, Raphael bursts from the water, no doubt in an effort to escape.  The wolves will have none of it.  One of them puts his paw on the head of the resurfaced fox and pushes him back into the water with just as much force.  Meanwhile, the other wolf fetches the lid and slides it onto the drum. Just before they close the barrel shut, they tip it over to discard the water. By now, Raphael is already banging on the wall from the inside, but his cries are muffled and weary.\n\n", false); 

	Render.text("<i>\"Tell it to Urta and the judge,\"</i> one of the wolves announces loudly, causing the other to chuckle.  <i>\"You have the right to remain silent. Anything you say will make us hold something against you, not necessarily in the court of law.\"</i> Sullenly, Raphael quiets down inside the sealed barrel.\n\n", false);

	Render.text("Effortlessly, one of the wolves lifts the barrel up onto his shoulder.  Before he carries it off, he throws one more look backwards and beams at you. <i>\"Check in with Urta.  She will reimburse you for the barrel, along with rewarding you with the outstanding bounty on our foxy friend.  Visit even later than that and you might even be allowed to enjoy his sentence.\"</i>\n\n", false);

	Render.text("The other spends a little longer regarding you, wagging his tail playfully and smiling a friendly smile at you.  He then looks back and sees his brother heading in the wrong way.  <i>\"We're not heading back to Tel'Adre right away?\"</i>  His voice is almost as low as his brother's.\n\n", false);

	Render.text("<i>\"We caught him in record time,\"</i> Skoll remarks.  <i>\"I'm sure Urta won't mind it if we keep him for ourselves for just a day.  You know... collect evidence... interrogate the culprit...\"</i>  His voice is carried off by the mist.\n\n", false); 

	Render.text("Hati throws you one more amicable glance before turning, getting on all fours, and running off.\n\n", false);

	Render.text("You blink as he fades and you're left to reflect on these morning's events by yourself.  For a minute you're concerned for the fox, but the wolves don't seem like the types to harm him... much.  You are curious what they plan to do with him and what his 'sentence' might be, though.", false); 
	flags[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] = -1;
	//1k gems?
	doNext(playerMenu);
}
...
private raphaelIntroPageTwo():void {
	clearOutput();
	Render.text("\"<i>This place isn't really an orphanage, is it?</i>\" you ascertain sagely, still waiting for a reply.");
...
private getTheFuckOutOfDodge():void {
	clearOutput();
	Render.text("You politely inform Raphael that you have to go, and though he seems crestfallen, he demurs, \"<i>Very well then, ma chère.  I shall await you in the hours of late evening, in case you change your mind.</i>\"");
	Render.text("\n\nWell, that went well.");
...
private cunnilingusWithRaphael():void {
	...
	if(player.wetness() < 3) Render.text("your cunt fluttering");
	//({Medium wetness multiplier}
	else if(player.wetness() < 4) Render.text("sending out a small spurt of femcum landing on your belly");
	//({High wetness multiplier}
	else Render.text("gushing a torrent of femcum into the air");
	Render.text(".  Raphael backs off a bit afterwards, giving you a few more laps of admiration around the throbbing fissure and cleaning you of any spillage. He ends with a tiny kiss, just inside your inner thigh.");
	player.orgasm();
	dynStats("sen", -2);
	doNext(camp.returnToCampUseOneHour);
}
