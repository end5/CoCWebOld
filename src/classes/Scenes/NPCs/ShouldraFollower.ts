...
private function pureValaAndShouldraMacroMax():void {
	...
	if(player.gender > 0) {
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText(", becoming firmly erect.  You happily piston your hips to grind your " + multiCockDescriptLight() + " along the pink-glossed walls, oozing your fluids in delight.");
...
public function shouldraMeetsCorruptVala():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  Your cock slips past the folds, pressed up against your stomach as the underside is massaged with the gentle tightness within.");
...
public function sandWitchGetsGhostly():void {
	...
	if(player.lowerBody.cockSpot.hasCock() && (!player.lowerBody.vaginaSpot.hasVagina() || rand(2) == 0)) {
		...
		if(!player.upperBody.chest.hasFuckableNipples()) outputText("\"<i>You know, maybe I should give you a couple of those chest-mounted treats.  They're pretty fun,</i>\" Shouldra teases, tweaking your [nipples].  ");
...
private function genderlessShouldraMasturbation():void {
	...
	//if event occurrence > 0
		...
		//[if (corruption < 50) 
		if(player.stats.cor < 50) outputText("There isn't an answer or outcome to that question that you're in favor of, honestly.");
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0 && flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0) outputText("Within lies a rather rustic yet well-built chest.");
	else if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 && flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0) outputText("The moment the light reveals what lies within, your [butthole] instinctively clenches shut.  \"<i>You didn't think I'd leave my most treasured possessions behind for some goblin to run into, did you?</i>\" It's the ghost girl's chest of dildos, as well-kept as the last time you laid worried eyes on it.");
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0) outputText("  Just what else did she pack in there since last you peered at its contents back at the ruins?");
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] > 0) outputText("familiar, ");
	...
	if(player.ass.analLooseness < 3) {
		...
		//[if (corruption < 50)
		if(player.stats.cor < 50) outputText("..  This only worries you further.");
	...
	//if analLooseness >= 3
		...
		if(player.stats.cor < 50) outputText(", which only serves to confuse you");
		outputText(".");
	}
	...
	if(flags[FlagEnum.ZETSUKO_MET] > 0) outputText("\"<i>Your run-in with a particular kitsune inspired me,</i>\" Shouldra teases");
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] > 0) outputText("You recognize it instantly as she pulls it free from her chest: the nearly foot-long floppy horse cock she impaled you on before.");
	//{if SHOULDRA_GENDERLESS_FUCK_COUNT = 0}
	else if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0) outputText("She holds her catch up clear for you to see; to your horror, you have clutched onto an enormous horse cock replica.  All 13 inches flop restlessly in your grip, seemingly taunting you with the foreboding future that lays before you.");
...
private function nongenderlessAnalShouldraMasturbation():void {
	...
	//if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 OR event occurrence > 0
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] > 0) outputText("You recognize it instantly as she pulls it free from her chest: the nearly 2-foot floppy horse cock she impaled you on before.");
	//if SHOULDRA_GENDERLESS_FUCK_COUNT = 0
...
private function shouldraTongueLicksPCs():void {
	...
	//if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 || event occurrence > 0
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || flags[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) outputText("  The consistency of your forgetfulness while traveling this path leads you to believe that your ethereal inhabitant would rather you not come back here on your own.");
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0 && flags[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) outputText("  Within lies a rather rustic yet well-built chest.");
	else if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 && flags[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) outputText("  \"<i>You didn't think I'd leave my most treasured possessions behind for some goblin to run into, did you?</i>\"  It's the ghost girl's chest of dildos, as well-kept as the last time you laid worried eyes on it.");
	else outputText("  Inside rests the ghost's familiar treasure trove of dildos and other toys you have yet to discover.");
	outputText("  She has you pull the container out from its hiding place, removing the clear tarp she used to protect it from the elements.  Despite the fact that you have yet to control your body once through this entire ordeal, an intrinsic urge to open the container has you bending over to pry into the contents within.");
	...
	if(flags[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) {
		...
		if(player.tongueType > TONUGE.HUMAN) outputText("some of ");
	...
	//if demon tongue
		...
		if(flags[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) outputText("  Almost shocked enough to wrest control away from your companion, you begin to babble dissent and try to reel your lengthening tongue back in to no avail.  Shouldra stops for a moment, quite frustrated by the interruption.  Sitting straight up, she consoles you, \"<i>Relax, Champ, it'll stop feeling weird in a minute and you know it's all temporary.</i>\"\n\nGranted a moment to adjust, you let yourself settle down and hand her the reins once more.  She happily goes back to ");
		else outputText("\n\n She happily continues ");
...
private function shouldraYappin():void {
	...
	if(flags[FlagEnum.SHOULDRA_TALK_NEEDED] == 1) {
		...
		if(flags[FlagEnum.SHOULDRA_PLOT_LEVEL] == 1) {
			...
			if( player.perks.has("Incorporeality")) outputText("  Shouldra bumps you to the side a little as she makes her next point, \"<i>Even with your hoaky quasi-possession power, you don't really know what it's like to actually possess someone.</i>\" You look at her funny, unaware that you weren't really getting the full effect.  Not that you care!");
			...
			if( player.perks.has("Incorporeality")) outputText(" - and I mean <b>really</b> possess someone -");
		...
		//Talk 3 - "Recent" Goblin Possession Discussion
			...
			if(player.lowerBody.cockSpot.hasCock()) outputText("</i>\"  Shouldra eyes your [cock] gingerly, acting as if she can stare right through your outfit at it.  \"<i>");
			else outputText("  ");
			...
			if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 200) outputText("Ones like yours?  ");
...
private function groBallsBiggaGHOSTYSTYLE():void {
	...
	//if ballsize > 10
	if(player.lowerBody.ballSize > 10) outputText("  You second guess your decision when your swinging cum barrels seem to impede walking.  Shouldra reassures you that the enhanced libido and baby batter supply is worth the wider gait.");
	dynStats("lus", 10);
	doNext(camp.returnToCampUseOneHour);
}
...
private function shouldraCockBloating101():void {
	...
	if(player.lowerBody.cockSpot.count() == 1) {
		...
		if(!player.lowerBody.cockSpot.hasSheath()) outputText("crotch");
...
public function nightTimeShouldraRapesThePC():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText(", circling their entire lengths around your [fullChest]");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("magically enlarged member");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("flailing cock");
...
public function shouldraWakesUpOrPokesPCsForShitsAndGigglesIdunnoHowLongCanIMakeThisFunctionNameQuestionMark():void {
	...
	switch(select) {
		...
		case 17:
			outputText("\"<i>");
			if(flags[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] + flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] == 1) outputText("That shark daughter of yours is");
...
private function shouldraAndWormsYoureGonnaHaveABadTime():void {
	...
	if(player.lowerBody.cockSpot.list[0].cockLength < 48) outputText(" and cuts right to the chase, ballooning your [cock] into a rigid, wide, four-foot tall slab of meat");
...
private function kickOutShouldra4YoWormyBuddies():void {
	...
	if(player.lowerBody.cockSpot.list[0].cockLength < 48) outputText("  With that little ordeal dealt with, you now are left to decide how best to manage the time until your cock returns to its normal size.");
...
public function shouldraAndExgartumonFightGottaCatchEmAll():void {
	...
	if(player.stats.cor < 50) outputText("work");
...
public function shouldraBakeryIntro():void {
	...
	//first time: 
	if(flags[FlagEnum.SHOULDRA_BAKERY_TIMES] == 0) outputText("You smirk, wondering how exactly a ghost manages to work up a hunger.  Your skepticism is met by an intense stabbing pain in your stomach, forcing you to buckle slightly from the cramping.  It lasts only a moment before it vanishes, however.  \"<i>You feel that, smartass?</i>\" The ghost girl sneers, \"<i>That's what decades of fasting will do to you.");
...
private function feedShouldraACake():void {
	...
	//after first time: 
	if(flags[FlagEnum.SHOULDRA_BAKERY_TIMES] > 0) outputText("Well, nothing <i>permanent</i>, anyway.  ");
...
private function goAlongWIthShouldrasEatingSpree():void {
	...
	spriteSelect(67);
	//if repeat scene: 
	if(flags[FlagEnum.SHOULDRA_BAKERY_TIMES] > 0) outputText("\"Man, you're not sure you want that kind of impact on your figure again...  \"");
	...
	if(flags[FlagEnum.PC_FETISH] > 0) outputText(", though not completely unwelcome");
	...
	if(flags[FlagEnum.PC_FETISH] > 0) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("trembling nether-lips");
	...
	if(flags[FlagEnum.PC_FETISH] > 0) outputText(", even with that kink of yours");
