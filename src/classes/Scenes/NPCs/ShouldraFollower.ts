...
private function shouldraSleeping(changes: number = 0, reset:boolean = false):boolean {
	//Adjust sleep timer
	if(reset) {
...
private function noFollowerShoulders():void {
	clearOutput();
	spriteSelect(67);
	Render.text("Somewhat bluntly, you inform her of your lack of interest regarding having a ghostly tagalong.  Though she looks a bit disappointed, she nods, scuffing her shoes against the ground and huffing a little sigh.  \"<i>At least I'll see you later, hey?</i>\"");
...
private function shouldraAndValaGetBigger():void {
	...
	Render.text("You decide to make your way down to the Wet Bitch to introduce Vala to her.  A few minutes into your stay, you see the fairy serving a few tables while holding a tray filled with drinks and empty glasses.  Pointing her out, Shouldra gives a delighted squeal.");
...
private function pureMacroValaCont():void {
	...
	//Female:
		...
		Render.text("Her finger starts pressing firmly against your chest.  You try to push it away, but the size difference makes the effort futile.  The ghost girl starts grinding against your chest with her fingertip, the friction against your breasts causing a small gasp to escape your lips, and your nips to become slightly firmer.  She runs her pinky up your leg, stopping at your vagina, and you reflexively clench your legs together. Unfazed, Shouldra forces them back open with her fingers, exposing you to the open air.  A small breeze blows through, tickling your cunt and causing you to bite your lip.");
...
private function pureValaAndShouldraMacroMax():void {
	...
	clearOutput();
	player.slimeFeed();
	Render.text("Before she can do anything, you shout out a rather unusual suggestion, though to you it sounds quite exciting.  Vala or Shouldra, whoever she is, smiles wickedly and nods, lifting you up towards the glittering gash between her legs.");
	...
	if(player.gender > 0) {
		Render.text("  Inspired by this, your own body has long since responded");
		if(player.lowerBody.cockSpot.hasCock()) Render.text(", becoming firmly erect.  You happily piston your hips to grind your " + multiCockDescriptLight() + " along the pink-glossed walls, oozing your fluids in delight.");
		else Render.text(", growing wet, though in this slick fluid, your contribution is a mere drop in the ocean.");
	}
	Render.text("  The warm fleshy dream is abruptly ended as Shouldra yanks you out by your [feet].  \"<i>I'm gonna put you the whole way in.  Do me a solid and get me off a few times, will ya?</i>\"");
	...
	Render.text(" against the slick walls.  You push your way towards the pinkish slit in the distance begin to slide your [legs] over the inner edges of her lips, pressing your arms up above you for stability.");
	...
	if(player.lowerBody.cockSpot.hasCock()) {  
		Render.text("  [EachCock] is quickly brought to orgasm by the exotic situation, and you fire off your own load into the deluge of sex-juice.");
		if(player.cumQ() >= 1000) Render.text("  Somehow, you pour out almost as much spunk as Vala's orgasm, turning the inside of her honeypot into an off-white mess.");
	}
	else if(player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("  Your [vagina] can scarcely resist the allure of the sweet, female flesh pressing against it, and with an explosion of hot bliss, you climax");
		if(player.wetness() >= 4) Render.text(", squirting with joy");
		Render.text(".");
	}
	...
	if(player.gender == 0) Render.text(" somehow");
	Render.text(", but she vanishes with a high-pitched giggle.");
...
public function shouldraMeetsCorruptVala():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  Your cock slips past the folds, pressed up against your stomach as the underside is massaged with the gentle tightness within.");
	Render.text("  Once you're up to your chest, Vala stops, instead grasping your shoulders between her thumb and forefingers as she begins to gently slide you in and out of her.");
...
public function sandWitchGetsGhostly():void {
	...
	if(player.lowerBody.cockSpot.hasCock() && (!player.lowerBody.vaginaSpot.hasVagina() || rand(2) == 0)) {
		...
		Render.text("Shouldra pokes her head out from your torso, rubbing her hands together menacingly.  \"<i>So, we're going to have some fun with this little gal, aren't we, Champ?</i>\"  The sand witch is paralyzed with fright, able only to gaze at the bizarre spectacle before her.  Looking down at the shade sticking out of you, you see her hands now poised against your chest.  The ghost girl is ready to fly out at a moment's notice.  You decide not to keep her waiting and give her the go ahead; the force of her excited exit about throws you to the sandy ground.  The eager spirit quickly hammers into the witch, digging her sitting body into the sand.  There's a little struggling, but it isn't long until the ghost subdues her prey.");
		...
		if(!player.upperBody.chest.hasFuckableNipples()) Render.text("\"<i>You know, maybe I should give you a couple of those chest-mounted treats.  They're pretty fun,</i>\" Shouldra teases, tweaking your [nipples].  ");
		Render.text("With a very telling exasperated sigh, you snatch up your [armor] and march back to camp.");
	}
...
private function genderlessShouldraMasturbation():void {
	clearOutput();
	spriteSelect(67);
	Render.text("Shouldra runs your fingers along your ");
	//[if (femininity < 50) 
	if(player.femininity < 50) Render.text("masculine ");
	else if(player.femininity > 50) Render.text("feminine ");
	Render.text("form while processing your request, your eyes glowing yellow as she mulls over her choices.");
	...
	//if event occurrence > 0
		...
		//[if (corruption < 50) 
		if(player.stats.cor < 50) Render.text("There isn't an answer or outcome to that question that you're in favor of, honestly.");
		else Render.text("You sure as hell wouldn't have bothered asking if you weren't.");
	}
	Render.text("  The ghost girl promptly discards your [armor] before hurrying your naked body along into the wilderness away from camp.");
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0 && flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0) Render.text("Within lies a rather rustic yet well-built chest.");
	else if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 && flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0) Render.text("The moment the light reveals what lies within, your [butthole] instinctively clenches shut.  \"<i>You didn't think I'd leave my most treasured possessions behind for some goblin to run into, did you?</i>\" It's the ghost girl's chest of dildos, as well-kept as the last time you laid worried eyes on it.");
	else Render.text("Inside rests the ghost's familiar treasure trove of dildos and other toys you have yet to discover.");
	Render.text("  She has you pull the container out from its hiding place, removing the clear tarp she used to protect it from the elements.  Despite the fact that you have yet to control your body once through this entire ordeal, an intrinsic urge to open the container has you bending over to pry into the contents within.");
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0) Render.text("  Just what else did she pack in there since last you peered at its contents back at the ruins?");
	...
	if(player.ass.analLooseness < 3) {
		Render.text("Your nerves screech out in pain as your rear revenant stretches out her puckered little \"jaw\" to something more fitting for what she has in store.  A few seconds later and the sensations begin to only register as pleasure.");
		//[if (corruption < 50)
		if(player.stats.cor < 50) Render.text("..  This only worries you further.");
		Render.text("  It doesn't take her long to <b>prepare your anus for any insidious ploy she's thought up.</b>");
	}
	//if analLooseness >= 3
	else {
		Render.text("\"<i>Looks as though you already get plenty of action back here, chief,</i>\" she jests.  Nevertheless, the rear revenant massages her \"jaw\" even more lax than you already are.  Even stranger, each gyration provides more and more pleasurable feedback");
		if(player.stats.cor < 50) Render.text(", which only serves to confuse you");
		Render.text(".");
	}
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] > 0) Render.text("You recognize it instantly as she pulls it free from her chest: the nearly foot-long floppy horse cock she impaled you on before.");
	//{if SHOULDRA_GENDERLESS_FUCK_COUNT = 0}
	else if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0) Render.text("She holds her catch up clear for you to see; to your horror, you have clutched onto an enormous horse cock replica.  All 13 inches flop restlessly in your grip, seemingly taunting you with the foreboding future that lays before you.");
	Render.text("  Shouldra smacks the dildo of doom onto a nearby boulder as she wearily rises you to your [feet].  The inferior, forgotten playthings slide out uselessly from the ghoul's gasping craw as she slowly clambers over top the wiggling horsey wang.");
...
private function nongenderlessAnalShouldraMasturbation():void {
	...
	//if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 OR event occurrence > 0
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] > 0) Render.text("You recognize it instantly as she pulls it free from her chest: the nearly 2-foot floppy horse cock she impaled you on before.");
	//if SHOULDRA_GENDERLESS_FUCK_COUNT = 0
	else Render.text("She holds her catch up clear for you to see; to your horror, you have clutched onto an enormous horse cock replica.  All 13 inches flop restlessly in your grip, seemingly taunting you with the foreboding future that lays before you.");
	Render.text("  Shouldra smacks the dildo of doom onto a nearby boulder as she wearily rises you to your [feet].  The inferior, forgotten playthings slide out uselessly from the ghoul's gasping craw as she slowly clambers over top the wiggling horsey wang.");
	Render.text("\n\nToo tired to object or plead, you meekly brace for impact as the wraith lines up its target.  Your immense cheeks part way.  Your rectum gapes wide.  Your anal tongue presses firmly against your taint.  \"<i>Down the hatch!</i>\"");
	Render.text("\n\nThe forest shudders with your scream.  The phantom easily downs six inches on the first go.  She wraps her tongue around the shaft, forcing more and more into you.  You swear your pelvis is bulging.  Muffled moans blended with laughter emit from your belabored behind.  You dare not guess how deep Shoudra drills the cock into you before she climaxes - hard.  Your voluptuous ass quavers with ferocity alongside her gagged yelping, throwing you from your perch atop the rock back down to the earth.  Her fun over, you begin to feel the magic subsiding.  Having no desire to learn of the consequences of leaving a massive wang lodged in your shrinking anus, you muster what little strength you can to plumb it free.  With the experience settling down, you can't help but laugh - closer to a timid chuckle, as its the best your exhausted body can offer.  You think you hear Shouldra saying something to you, but you're too worn out to listen to her boasting.");
...
private function maleMasturbationProper():void {
	clearOutput();
	spriteSelect(67);
	Render.text("You are nearly overwhelmed by Shouldra's delight when you allow her to focus on your [cock].  She instantly takes the lead, steering you behind a good-sized boulder and flinging your [armor] piece-by-piece as you go.");
	...
	//if cocksize <200: 
	if(player.cockArea(0) < 200) Render.text("  You stagger a bit at the unaccustomed heft of your junk altering your center of gravity, but you know that will wear off soon enough.");
	Render.text("  She reaches into her tunic and tosses you a full bottle of ectoplasm, giving you a wink before incorporealizing once more and floating into you.  All's well that ends well.");
	player.orgasm();
	dynStats("sen", -2);
	Render.text("\n\n", false);
	shouldraSleeping((10 + rand(26)), true);
...
private function girlyMasturbationWithGhosts():void {
	...
	Render.text("\"<i>Tsk!  Saying those kinds of things aloud!  Still... if you want to let it out so badly, I might let you if you beg a little more...</i>\"");
...
private function shouldraTongueLicksPCs():void {
	...
	//if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 || event occurrence > 0
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || flags[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) Render.text("  The consistency of your forgetfulness while traveling this path leads you to believe that your ethereal inhabitant would rather you not come back here on your own.");
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0 && flags[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) Render.text("  Within lies a rather rustic yet well-built chest.");
	else if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 && flags[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) Render.text("  \"<i>You didn't think I'd leave my most treasured possessions behind for some goblin to run into, did you?</i>\"  It's the ghost girl's chest of dildos, as well-kept as the last time you laid worried eyes on it.");
	else Render.text("  Inside rests the ghost's familiar treasure trove of dildos and other toys you have yet to discover.");
	Render.text("  She has you pull the container out from its hiding place, removing the clear tarp she used to protect it from the elements.  Despite the fact that you have yet to control your body once through this entire ordeal, an intrinsic urge to open the container has you bending over to pry into the contents within.");
	...
	if(flags[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) {
		...
		if(player.tongueType > TONUGE.HUMAN) Render.text("some of ");
		Render.text("your tongue out, examining it.  Already, you think you've got a pretty good idea of where she's going to take this.");
	...
	if(player.perks.has("Flexibility")) {
		...
		if(flags[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) {
			Render.text("  Almost shocked enough to wrest control away from your companion, you begin to babble dissent and try to reel your lengthening tongue back in to no avail.  Shouldra stops for a moment, quite frustrated by the interruption.  Sitting straight up, she consoles you, \"<i>Relax, Champ, it'll stop feeling weird in a minute and you know it's all temporary.</i>\"");
		...
		Render.text(" sliding fingers along the wet appendage, humming a little tune over your tongue until more than a foot of it droops from your mouth.  \"<i>Now we're getting somewhere,</i>\" you hear her thinking as she works her magic.  There doesn't seem to be a limit to how much she can pull out of your mouth, and soon she has more than three feet of pure tongue coiled in your lap.  Furrowing your brow in concentration, she tries to move your tongue about, but can't manage to wiggle more than the tip.  \"<i>Too thin,</i>\" she mutters, \"<i>much too weak. Doesn't feel right...</i>\"");
	...
	//if demon tongue
		...
		if(flags[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) Render.text("  Almost shocked enough to wrest control away from your companion, you begin to babble dissent and try to reel your lengthening tongue back in to no avail.  Shouldra stops for a moment, quite frustrated by the interruption.  Sitting straight up, she consoles you, \"<i>Relax, Champ, it'll stop feeling weird in a minute and you know it's all temporary.</i>\"\n\nGranted a moment to adjust, you let yourself settle down and hand her the reins once more.  She happily goes back to ");
		else Render.text("\n\n She happily continues ");
		Render.text("sliding fingers along the wet appendage, humming a little tune over your tongue until more than an additional foot of it droops from your mouth.  \"<i>Now we're getting somewhere,</i>\" you hear her thinking as she works her magic.  There doesn't seem to be a limit to how much she can pull out of your mouth, and soon she has more than three feet of pure tongue coiled in your lap.  Furrowing your brow in concentration, she tries to move your tongue about, but can't manage to wiggle more than the tip.  \"<i>Too thin,</i>\" she mutters, \"<i>much too weak. Doesn't feel right...</i>\"");
		Render.text("\n\nTaking the midsection of your tongue in your hand, she slowly starts to rub her hand along some of its length.  It feels... odd, somehow, but not entirely unpleasant.  Though you shudder at the first rub, they start to feel more natural as she pushes you on.  Actually, it feels sort of good.  Her strokes remain constant, though some part of you wishes she'd go faster.  It doesn't even register as odd in your addled mind that your tongue is swelling in your hand with every stroke.  Shouldra starts to make wider movements along your smooth stretch of muscle.  Its girth plumps up to fill your palm nicely wherever she brings your hand, and the strange sensations of your flesh on its nerves wrack your body with shudders of pleasure even through Shouldra's grip.");
...
public function shouldraDream1():void {
	...
	if(flags[FlagEnum.SHOULDRA_PLOT_LEVEL] == 0) {
		...
		Render.text("Few things more relaxing than heading home after a day of work.  But as you approach the dilapidated hut, some strange noises from within catch you off guard.  If it's that fucking mouse again...");
...
private function shouldraYappin():void {
	...
	if(flags[FlagEnum.SHOULDRA_TALK_NEEDED] == 1) {
		...
		if(flags[FlagEnum.SHOULDRA_PLOT_LEVEL] == 1) {
			...
			Render.text("You decide not to beat around the bush and ask about the strange dream you had the other night.  The spirit puts on a surprised reaction, your shoulder returning to normal as she casually walks outside of your body.");
			...
			if( player.perks.has("Incorporeality")) Render.text("  Shouldra bumps you to the side a little as she makes her next point, \"<i>Even with your hoaky quasi-possession power, you don't really know what it's like to actually possess someone.</i>\" You look at her funny, unaware that you weren't really getting the full effect.  Not that you care!");
			...
			Render.text(".  \"<i>No matter what treasure trove of knowledge I dig up from you, there's always something I'm more interested in.</i>\"  Your hand lifts back up, and you watch as Shouldra's hand splits off from your own, meeting below the wrist.  She holds your hand with her own ghostly appendage, resting her head on your shoulder.  The ghost girl sits there for a moment, silent, leaving you to wonder if maybe, just maybe, you're starting to mean a little more to her than just a sex toy.");
		...
		//Talk 2 - First Transformation Discussion
			...
			if(player.gender > 0) {
				Render.text("  Your [if (hasCock = true) [cock]][if (isHerm = true)  and ][if (hasVagina = true) [vagina]] start to swell up; looking up at the ghost, you realize that she hasn't uttered a single word!");
			}
			else Render.text("  A fleshy, penis-shaped bulge begins to swell out of your loins; looking up at the ghost, you realized she hasn't uttered a single word!");
			...
			if(player.gender == 0) Render.text("dick");
			Render.text(" [if (isHerm = false) continues its][if (isHerm = true) continue their] march of magnification while your fingers vigorously explore their new frontiers.");
			...
			if(player.gender == 0) Render.text("burgeoning dickflesh");
			Render.text(".  It takes some gumption for you to work through the arousal and contribute to the conversation, but you manage to point out how you're pretty sure there are other people in Mareth with the same sort of power, to some degree at least.");
		...
		//Talk 3 - "Recent" Goblin Possession Discussion
			...
			Render.text("You stare at Shouldra.  \"<i>Oh, <b>what</b>?</i>\"  You stare at Shouldra.  \"<i>She fucking pissed on my floor!</i>\"  You stare at Shouldra.  The ghost shakes her head, sinking it back inside your skin. The ghost girl quickly flies out of your chest.  \"<i>You don't fuck with my stuff!</i>\"");
			...
			if(player.lowerBody.cockSpot.hasCock()) Render.text("</i>\"  Shouldra eyes your [cock] gingerly, acting as if she can stare right through your outfit at it.  \"<i>");
			else Render.text("  ");
			Render.text("The half-millenium or so of scarcity has really shaped my interests.  I'm at the point now where I just lose it whenever I get these ghostly mitts on a slab of meat.  And the big ones?  ");
			if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 200) Render.text("Ones like yours?  ");
			Render.text("Then I'm just in heaven.</i>\"  Hold on.  She can turn any dick into a towering monument.  What's so special about ones that come pre-packaged, you ask?  \"<i>I kinda break the one rule I had at that point, but when I find someone with a big boner, it just... feels so good to me.  There's just something about an all-natural magnum opus that just lights my fire.</i>\"");
			...
			Render.text("\"<i>Oh,</i>\" she says, stepping out of you and sitting herself across from you.  \"<i>You saw that.</i>\"");
	...
	//Generic Oni Discussion!
		...
		Render.text("On a whim, you decide to have a chat with your spectral companion, who obligingly pops free of your torso to spare you the awkward sensation of conversing with one of your own body parts.");
...
public function shouldraFollowerScreen():void {
	spriteSelect(67);
	clearOutput();
	if(!shouldraSleeping()) Render.text("You pulse a desire for interaction with Shouldra, summoning her head to form from your shoulder. \"<i>What's on your mind, Champ? I could've found out on my own, but I was... preoccupied.</i>\"");
	//Follower Screen Quick Text - Shouldra asleep
	else {
		Render.text("You try and contact Shouldra, but the ghost is still sound asleep within you.  Or whatever ghosts consider sleep after an arousing round of debauchery. Either way, you garner no response.  Best to try again later.");
		doNext(playerMenu);
		return;
	}
...
private function shouldraSexMenu():void {
	clearOutput();
	spriteSelect(67);
	Render.text("Shouldra stirs from somewhere deep inside you, excitement evident in the swiftness of her reaction.  \"<i>Is it time for some fun, Champ?</i>\" she asks happily, her increased lust affecting you just a bit.  Do you oblige her?");
...
private function shouldraGroPlus():void {
	clearOutput();
	spriteSelect(67);
	Render.text("You decide to take up Shouldra's offer on natural body enhancement.  Before you tell her as much, the eager spectre quickly assumes control of your body.  \"<i>Alright, Champ, now we're in business.  So, what did you have in mind?</i>\"");
...
private function groBallsBiggaGHOSTYSTYLE():void {
	...
	clearOutput();
	spriteSelect(67);
	Render.text("\"<i>Interested in a bigger ballsack, huh?  Not only will it help pronounce your package, but a big ol' swinging sack of nuts feels great when you're really pounding.</i>\" Shouldra grasps your [balls] with both your hands and begins reciting the appropriate spell.");
	...
	//if ballsize > 10
	if(player.lowerBody.ballSize > 10) Render.text("  You second guess your decision when your swinging cum barrels seem to impede walking.  Shouldra reassures you that the enhanced libido and baby batter supply is worth the wider gait.");
	dynStats("lus", 10);
	doNext(camp.returnToCampUseOneHour);
}
...
private function shouldraGrowsYoTits():void {
	...
	clearOutput();
	spriteSelect(67);
	Render.text("\"<i>I'll give you a rack that could hypnotize anyone into a faithful tit slave,</i>\" Shouldra boasts, stretching your shoulders in preparation.  Your hands grope your [fullChest] as the spirit slips into the arcane language you've come to grow so familiar with.\n\n");
...
private function shouldraGrowsYoClit():void {
	...
	clearOutput();
	spriteSelect(67);
	Render.text("\"<i>Looking to give your joy buzzer a little jumpstart?</i>\" Shouldra teases, your hands sliding down to your inviting [cunt].  Your fingers tease your [clit] while the ghost girl spouts her arcane tongue from your mouth.  ");
	//clitLength + 1
	player.lowerBody.vaginaSpot.list[0].clitLength += 1;
	Render.text("She welcomes the new inch with a quick flick.  The sensitive [clit] twitches happily in response, confirming the success of the spell.");
...
private function shouldraGivesYaSomeFukkinTeats():void {
	...
	clearOutput();
	spriteSelect(67);
	Render.text("Shouldra rubs your hands together in anticipation, \"<i>A beautiful blossoming pair of nipples is what you need to really compliment a nice set of tits.</i>\"  Your hands gleefully tweak your [nipples] as the spirit's incantation gets underway.");
...
private function shouldrasButtBigginator():void {
	...
	clearOutput();
	spriteSelect(67);
	Render.text("Shouldra's haunting laugh works its way out from your mouth.  \"<i>Looking for a little more jiggle in your step, Champ?  I can get BEHIND that.</i>\"  This damnable ghost won't let you roll your eyes!  Is there no end to her torment?  After a quick crack of your knuckles, Shouldra directs your hands back onto your derriere as she transitions into spellcasting.");
...
private function shouldraReductosYourBallsUpInsideYa():void {
	...
	clearOutput();
	spriteSelect(67);
	Render.text("\"<i>Why would you want to shrivel these delicious cumquats?</i>\"  Shouldra's arsenal of puns doesn't affect your request in the slightest.  The ghost girl moves into a quick spell, savoring your testicles before finishing.");
...
private function shouldraReductosYourTits():void {
	...
	clearOutput();
	spriteSelect(67);
	Render.text("Your arms clasp themselves around your [fullChest].  \"<i>Not the gals!  What monster would want to deflate their knockers?</i>\"  This monster, evidently.  Shouldra sighs, holding your hands in a weak casting stance as she gets the heinous act over with.");
	player.shrinkTits();
...
private function clittyVanishingActShouldra():void {
	...
	clearOutput();
	spriteSelect(67);
	Render.text("\"<i>Aw, really, Champ?  The best part of your snatch?</i>\"  A nod of your head reasserts your desire.  The depressed phantom hastily works through the spell, the notion of shrinking your [clit] not sitting well with her.");
...
private function shouldraMakesCocksDisappear():void {
	...
	if(player.lowerBody.cockSpot.count() == 1) {
		...
		if(choice == 0) Render.text("murderer");
		else if(choice == 1) Render.text("bastard");
		else if(choice == 2) Render.text("jerk");
		else if(choice == 3) Render.text("monster");
		else if(choice == 4) Render.text("cretin");
		else if(choice == 5) Render.text("fiend");
		else if(choice == 6) Render.text("asshole");
		else if(choice == 7) Render.text("villain");
		else Render.text("dick");
		Render.text("!</i>\" it shouts.  The tiny voice fades away as your [cock] goes still, resting at two thirds of its original size.  A single tear rolls down your cheek; you suspect the melodramatic spirit is at fault here.");
...
private function shrinkDemNipplzForYoGhost():void {
	...
	clearOutput();
	spriteSelect(67);
	Render.text("\"<i>Are you kidding me, Champ?  Nipples are the most important-</i>\"  You cut the ghost girl's argument there, not wanting to leave your decision up for debate.  She instead leaves you with a deep sigh of disapproval before quickly casting the appropriate spell.");
	//Shrink
...
private function shrinkDatBootyForYoGhost():void {
	...
	clearOutput();
	spriteSelect(67);
	Render.text("Your hands quickly grasp at your bare [butt].  \"<i>You... you're desecrating a national treasure here!</i>\"  The rather odd declaration gets a chuckle from you, but you remain firm in your request to dull your derriere.  The ghost girl furthers her attempts to change your mind, but without literally changing your mind, you remain steadfast.  Defeated, your hands take one last trip around your posterior before the poltergeist recites her incantation.");
...
private function kickFollowerShouldraOut():void {
	...
	if(flags[FlagEnum.SHOULDRA_PLOT_LEVEL] < 4 || flags[FlagEnum.SHOULDRA_TALK_NEEDED] > 0) {
		Render.text("\"<i>Ah, you're booting me out, hey?</i>\"  Shouldra asks innocently, tumbling out of your body and corporealizing. \"<i>Wouldn't be the first, and won't be the last.  I do realize I have certain 'needs' that got cloying to many of my former hosts.</i>\"");
	...
	//She Fucking Explodes
		...
		Render.text("Are you sure you want to ask her to leave after she opened herself up to you?  It's just a hunch, but she might not take this very well.");
...
public function shouldersWarnings():boolean {
	...
	//7 days (168 hours) since last Shouldra scene
		...
		if(rand(3) == 0) {
			...
			if(player.gender == 0) Render.text("libido seems to be going haywire");
			Render.text(".  Shouldra's unfulfilled sexual needs are starting to blend into your own.  Nothing you do will satisfy your urging until she's seen to her own.\n\n<b>Minimum lust raised by 20.</b>\n");
		}
		else if(rand(2) == 0) {
...
public function nightTimeShouldraRapesThePC():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) Render.text(", circling their entire lengths around your [fullChest]");
	Render.text(".  It's quite amazing how ecstatic the tour feels, each pre-smeared exploration smothering you in sensual vibes.  Just moving the stiff little devils - particularly at your joints - sends shivers up your arm.");
	...
	Render.text(", softening the ");
	if(player.lowerBody.cockSpot.hasCock()) Render.text("magically enlarged member");
	else Render.text("magically gaped snatch");
	Render.text(" back to a much less craggy complexion.  Shouldra slowly drags you back to your [feet] after a minute or two of blissful respite and walks you closer to the cliffside to stare off into the moonlit horizon.  It never really dawned on you just how fucking high you are; just where the fuck did she take you?");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("flailing cock");
	else Render.text("moist vagina");
	Render.text(".");
	//if cock
	if(player.lowerBody.cockSpot.hasCock()) {
...
public function shouldraWakesUpOrPokesPCsForShitsAndGigglesIdunnoHowLongCanIMakeThisFunctionNameQuestionMark():void {
	...
	switch(select) {
		...
		case 5:
			...
			else if(subSelect == 2) Render.text("You almost don't realize your hand creeping its way into your [armor], trying to sneak a quick fingering. Looks like Shouldra is getting antsy.");
			else if(subSelect == 3) Render.text("What the... ?  Your [vagina] has started gushing fluids everywhere, absolutely soaking your [legs]!  \"<i>Don't mind me, Champ.  I'm just testing to see how far I can push this. Part of something I have in mind for later...</i>\"  No good can come of this...");
			else Render.text("You can sense Shouldra laughing to herself over something.  Though its usually a poor idea, you decide to ask and see what's on her mind.  \"<i>I was just scheming.  Would you get too upset if I spooked your next fuck?</i>\"  As she asks, you feel your [vagina] clenching hard, giving you a clear idea of what the trickster has in mind.  There's no way that-  On second thought, maybe it could be kind of fun?");
			break;
...
private function shouldraAndWormsYoureGonnaHaveABadTime():void {
	clearOutput();
	spriteSelect(67);
	Render.text("Your eyes glow yellow, signaling Shouldra's desire to stretch around a little.  You grant her mastery over your hands as you continue about your business.  The eager phantom decides to push things a little, however, systematically removing your [armor].  A break isn't that big a deal you figure, and you relinquish full control to the giddy ghost.  She directs you out of plain view");
	if(player.lowerBody.cockSpot.list[0].cockLength < 48) Render.text(" and cuts right to the chase, ballooning your [cock] into a rigid, wide, four-foot tall slab of meat");
	Render.text(".  Unable to contain her delight, she hastily hugs your enormous dick firmly against your body and squeezes vigorously, rubbing your face against your [cockHead].  A warm stream of pre oozes out and onto your possessed face.");
	...
	if(model.time.hours <= 12) Render.text("morning ");
	else if(model.time.hours <= 5) Render.text("afternoon ");
	else Render.text("evening ");
	Render.text("air.  Your hands clue the eidolon in to further activity, introducing her to your urethra all the way down to your [balls], every square wriggling inch chock-full of the little buggers, exploring their extended living quarters.");
...
private function kickOutShouldra4YoWormyBuddies():void {
	...
	if(player.lowerBody.cockSpot.list[0].cockLength < 48) Render.text("  With that little ordeal dealt with, you now are left to decide how best to manage the time until your cock returns to its normal size.");
	Render.text("  Oddly enough, you hope the little buggers appreciate you sticking up for them.");
...
public function shouldraAndExgartumonFightGottaCatchEmAll():void {
	...
	//Unlocks Argue 2
	spriteSelect(67);
	Render.text("Your bedroll looks mighty comfortable after a hard day's ");
	if(player.stats.cor < 50) Render.text("work");
	else Render.text("fucking");
	Render.text(".  Just when you're [armor] and about to tuck in for the night, however, your [cock] goes fully erect!  The throbbing demonic mass is flailing about, knocking into your shit and making a mess.  You clutch onto the bucking bronco in an attempt to prevent any further damage when Exgartuan begins to shout, \"<i>Don't FUCKING touch me, you goddamn cunt!</i>\"");
...
public function exgartuMonAndShouldraShowdown():void {
	...
	if(silly()) Render.text("  It's a man's right to choose what's done with his body.  Not anyone else's!");
	Render.text("  You shout loud enough to get the two to at least shut up and notice you're there.  It's time to end this little battle once and for all, starting with your first - and hopefully last - involvement.");
...
private function keepShouldraAndKickOutExgartuan():void {
	clearOutput();
	spriteSelect(67);
	Render.text("You decide to jump to Shouldra's side on this one, explaining to your demonic cock your desire to go your separate ways.  Exgartuan heartily laughs at you, your [cock] wiggling in the air with each mighty heave.");
...
public function keepShouldraPartIIExgartumonsUndeatH():void {
	...
	Render.text("While slipping into your bedroll to enjoy a peaceful night's sleep, a chill runs down your spine.  You look around to-");
...
private function keepAllTheGhosts():void {
	...
	if(player.cumQ() >= 1000) Render.text(", trails of your cum pouring out from them");
	Render.text(".");
...
public function shouldraBakeryIntro():void {
	clearOutput();
	spriteSelect(67);
	Render.text("You set off towards the bakery, and it's not long before the smell of freshly-baked pastries and sugary candies wafts through the air.  Something stirs in your mind as a familiar ghostly presence makes herself known; seems you're not the only one who noticed the scent of cinnamon.");
	...
	//first time: 
	if(flags[FlagEnum.SHOULDRA_BAKERY_TIMES] == 0) Render.text("You smirk, wondering how exactly a ghost manages to work up a hunger.  Your skepticism is met by an intense stabbing pain in your stomach, forcing you to buckle slightly from the cramping.  It lasts only a moment before it vanishes, however.  \"<i>You feel that, smartass?</i>\" The ghost girl sneers, \"<i>That's what decades of fasting will do to you.");
	else Render.text("You point out that she's already had something at the bakery.  \"<i>Yeah, I did.  And now I'm hungry again.  Deal with it.");
	Render.text("  Now, how about a snack?  Or two?</i>\" Her enthusiasm is clear...  and slightly worrying.");
...
private function bailOut():void {
	clearOutput();
	spriteSelect(67);
	Render.text("You quickly turn around and head back the way you came.  There's a moan of disappointment from Shouldra, before she settles down and goes back to doing whatever she usually does.  You try not to think about it as you leave the city and return to camp.");
	doNext(camp.returnToCampUseOneHour);
}
...
private function feedShouldraACake():void {
	clearOutput();
	spriteSelect(67);
	Render.text("You decide that there's no use getting cold feet now; you wanted to go the bakery anyway, no harm in letting Shouldra have some fun while you're there.  ");
	//after first time: 
	if(flags[FlagEnum.SHOULDRA_BAKERY_TIMES] > 0) Render.text("Well, nothing <i>permanent</i>, anyway.  ");
	Render.text("You press on, soon arriving at the ramp leading down into the bakery's interior.  It's as busy as ever, with patrons milling about, chatting between bites of pastry.");
...
private function shouldraAndMaddieSittingInATree():void {
	...
	if(player.tone >= 60) Render.text("well-toned");
	else if(player.thickness < 50) Render.text("trim");
	else Render.text("curvaceous");
	Render.text(" figure...");
...
private function goAlongWIthShouldrasEatingSpree():void {
	...
	spriteSelect(67);
	//if repeat scene: 
	if(flags[FlagEnum.SHOULDRA_BAKERY_TIMES] > 0) Render.text("\"Man, you're not sure you want that kind of impact on your figure again...  \"");
	Render.text("Ah, what the hell.  You were hungry anyway.  You let Shouldra guide the cake into your mouth.  Mmm, not bad; rich, sweet and delicate.  Shouldra chuckles in your mind as she releases her control slightly, letting you lick your fingers clean of any stray crumbs.  \"<i>Well, guess you're pretty hungry yourself!  Alright, Champ, I'll give you some freedom, but just remember you're not the only one with an appetite.</i>\" You wince as your hunger intensifies to the verge of pain; looks like you're eating for two.  Your eyes lock on to the feast before you and something snaps in your mind.  You need this food, all of this food, inside you.  <i>Now</i>.");
	...
	if(flags[FlagEnum.PC_FETISH] > 0) Render.text(", though not completely unwelcome");
	Render.text(".  You blush furiously and back into the corner of the (now rather cold) booth");
	if(flags[FlagEnum.PC_FETISH] > 0) {
		Render.text(" as your heart races and blood rushes down towards your ");
		if(player.lowerBody.cockSpot.hasCock()) {
			Render.text("drowsy member");
			if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and ");
		}
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("trembling nether-lips");
		if(player.gender == 0) Render.text("groin");
	}
	Render.text(", before you realize that nobody seems to notice your predicament.  Shouldra reads your thoughts and responds, \"<i>Thought you might appreciate some privacy");
	if(flags[FlagEnum.PC_FETISH] > 0) Render.text(", even with that kink of yours");
	Render.text(", Champ.  Here's something extra anyway, just in case.</i>\" One ghostly incantation later, and ");
	if(player.femininity <= 50) Render.text("a buttoned shirt and pair of belted jeans");
	else Render.text("a tight-fitting tank top and skirt");
	Render.text(" adorn your person.");
