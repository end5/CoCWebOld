export default class ShouldraFollower extends NPCAwareContent {
	...
	public followerShouldra(): boolean {
		return Flags.list[FlagEnum.SHOULDRA_FOLLOWER_STATE] == 1;
	}
	private shouldraSleeping(changes: number = 0, reset: boolean = false): boolean {
		//Adjust sleep timer
		if (reset) {
			Flags.list[FlagEnum.SHOULDRA_SLEEP_TIMER] = changes;
		}
		else Flags.list[FlagEnum.SHOULDRA_SLEEP_TIMER] += changes;
		if (Flags.list[FlagEnum.SHOULDRA_SLEEP_TIMER] < 0) Flags.list[FlagEnum.SHOULDRA_SLEEP_TIMER] = 0;
		if (Flags.list[FlagEnum.SHOULDRA_SLEEP_TIMER] > 48) Flags.list[FlagEnum.SHOULDRA_SLEEP_TIMER] = 48;
		//Is she sleeping?
		if (Flags.list[FlagEnum.SHOULDRA_SLEEP_TIMER] > 0) return true;
		return false;
	}
	...
	private noFollowerShoulders(): void {
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("Somewhat bluntly, you inform her of your lack of interest regarding having a ghostly tagalong.  Though she looks a bit disappointed, she nods, scuffing her shoes against the ground and huffing a little sigh.  \"<i>At least I'll see you later, hey?</i>\"");
	...
	private recruitShouldraForCampSexyTimes(): void {
		...
		Flags.list[FlagEnum.SHOULDRA_FOLLOWER_STATE] = .5;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	public morningShouldraAlert(): void {
		...
		Flags.list[FlagEnum.SHOULDRA_FOLLOWER_STATE] = 1;
		Flags.list[FlagEnum.SHOULDRA_PLOT_COUNTDOWN] = 7;
		doNext(playerMenu);
	}
	...
	private shouldraAndValaGetBigger(): void {
		MainScreen.clearText();
		spriteSelect(67);
		if (Flags.list[FlagEnum.SHOULDRA_MET_VALA] < 1) Flags.list[FlagEnum.SHOULDRA_MET_VALA] = 1;
		MainScreen.text("You decide to make your way down to the Wet Bitch to introduce Vala to her.  A few minutes into your stay, you see the fairy serving a few tables while holding a tray filled with drinks and empty glasses.  Pointing her out, Shouldra gives a delighted squeal.");
		...
		menu();
		MainScreen.addButton(0, "Next", pureMacroValaCont);
		MainScreen.addButton(1, "Be A Toy", pureValaAndShouldraMacroMax);
	}
	...
	private pureMacroValaCont(): void {
		...
		//Female:
			...
			MainScreen.text("Her finger starts pressing firmly against your chest.  You try to push it away, but the size difference makes the effort futile.  The ghost girl starts grinding against your chest with her fingertip, the friction against your breasts causing a small gasp to escape your lips, and your nips to become slightly firmer.  She runs her pinky up your leg, stopping at your vagina, and you reflexively clench your legs together. Unfazed, Shouldra forces them back open with her fingers, exposing you to the open air.  A small breeze blows through, tickling your cunt and causing you to bite your lip.");
	...
	private pureValaAndShouldraMacroMax(): void {
		...
		MainScreen.clearText();
		player.slimeFeed();
		MainScreen.text("Before she can do anything, you shout out a rather unusual suggestion, though to you it sounds quite exciting.  Vala or Shouldra, whoever she is, smiles wickedly and nods, lifting you up towards the glittering gash between her legs.");
		...
		if (player.gender > 0) {
			MainScreen.text("  Inspired by this, your own body has long since responded");
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(", becoming firmly erect.  You happily piston your hips to grind your " + CockDescriptor.describeMultiCockShort(player) + " along the pink-glossed walls, oozing your fluids in delight.");
			else MainScreen.text(", growing wet, though in this slick fluid, your contribution is a mere drop in the ocean.");
		}
		MainScreen.text("  The warm fleshy dream is abruptly ended as Shouldra yanks you out by your [feet].  \"<i>I'm gonna put you the whole way in.  Do me a solid and get me off a few times, will ya?</i>\"");
		...
		MainScreen.text(" against the slick walls.  You push your way towards the pinkish slit in the distance begin to slide your [legs] over the inner edges of her lips, pressing your arms up above you for stability.");
		...
		if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text("  [EachCock] is quickly brought to orgasm by the exotic situation, and you fire off your own load into the deluge of sex-juice.");
			if (player.cumQ() >= 1000) MainScreen.text("  Somehow, you pour out almost as much spunk as Vala's orgasm, turning the inside of her honeypot into an off-white mess.");
		}
		else if (player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("  Your [vagina] can scarcely resist the allure of the sweet, female flesh pressing against it, and with an explosion of hot bliss, you climax");
			if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 4) MainScreen.text(", squirting with joy");
			MainScreen.text(".");
		}
		...
		if (player.gender == 0) MainScreen.text(" somehow");
		MainScreen.text(", but she vanishes with a high-pitched giggle.");
	...
	public shouldraMeetsCorruptVala(): void {
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  Your cock slips past the folds, pressed up against your stomach as the underside is massaged with the gentle tightness within.");
		MainScreen.text("  Once you're up to your chest, Vala stops, instead grasping your shoulders between her thumb and forefingers as she begins to gently slide you in and out of her.");
	...
	public sandWitchGetsGhostly(): void {
		...
		if (player.lowerBody.cockSpot.hasCock() && (!player.lowerBody.vaginaSpot.hasVagina() || rand(2) == 0)) {
			...
			MainScreen.text("Shouldra pokes her head out from your torso, rubbing her hands together menacingly.  \"<i>So, we're going to have some fun with this little gal, aren't we, Champ?</i>\"  The sand witch is paralyzed with fright, able only to gaze at the bizarre spectacle before her.  Looking down at the shade sticking out of you, you see her hands now poised against your chest.  The ghost girl is ready to fly out at a moment's notice.  You decide not to keep her waiting and give her the go ahead; the force of her excited exit about throws you to the sandy ground.  The eager spirit quickly hammers into the witch, digging her sitting body into the sand.  There's a little struggling, but it isn't long until the ghost subdues her prey.");
			...
			if (!player.upperBody.chest.hasFuckableNipples()) MainScreen.text("\"<i>You know, maybe I should give you a couple of those chest-mounted treats.  They're pretty fun,</i>\" Shouldra teases, tweaking your [nipples].  ");
			MainScreen.text("With a very telling exasperated sigh, you snatch up your [armor] and march back to camp.");
		}
	...
	private shouldraFappinTimes(): void {
		spriteSelect(67);
		if (player.lowerBody.cockSpot.hasCock() && player.statusAffects.has("Infested")) {
			shouldraAndWormsYoureGonnaHaveABadTime();
		}
		else if (player.gender == 0) genderlessShouldraMasturbation();
		else if (player.lowerBody.cockSpot.hasCock() && (!player.lowerBody.vaginaSpot.hasVagina() || rand(2) == 0)) maleMasturbationProper();
		else {
	...
	private genderlessShouldraMasturbation(): void {
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("Shouldra runs your fingers along your ");
		//[if (femininity < 50) 
		if (player.femininity < 50) MainScreen.text("masculine ");
		else if (player.femininity > 50) MainScreen.text("feminine ");
		MainScreen.text("form while processing your request, your eyes glowing yellow as she mulls over her choices.");
		//(same paragraph follows)
		//{if SHOULDRA_GENDERLESS_FUCK_COUNT = 0 & event occurrence = 0}
		if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0 && Flags.list[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0) {
		...
		//if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 & event occurrence = 0
		else if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 && Flags.list[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0) {
		...
		//if event occurrence > 0
			...
			//[if (corruption < 50) 
			if (player.stats.cor < 50) MainScreen.text("There isn't an answer or outcome to that question that you're in favor of, honestly.");
			else MainScreen.text("You sure as hell wouldn't have bothered asking if you weren't.");
		}
		MainScreen.text("  The ghost girl promptly discards your [armor] before hurrying your naked body along into the wilderness away from camp.");
		...
		if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0 && Flags.list[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0) MainScreen.text("Within lies a rather rustic yet well-built chest.");
		else if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 && Flags.list[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0) MainScreen.text("The moment the light reveals what lies within, your [butthole] instinctively clenches shut.  \"<i>You didn't think I'd leave my most treasured possessions behind for some goblin to run into, did you?</i>\" It's the ghost girl's chest of dildos, as well-kept as the last time you laid worried eyes on it.");
		else MainScreen.text("Inside rests the ghost's familiar treasure trove of dildos and other toys you have yet to discover.");
		MainScreen.text("  She has you pull the container out from its hiding place, removing the clear tarp she used to protect it from the elements.  Despite the fact that you have yet to control your body once through this entire ordeal, an intrinsic urge to open the container has you bending over to pry into the contents within.");
		...
		if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0) MainScreen.text("  Just what else did she pack in there since last you peered at its contents back at the ruins?");
		...
		if (player.lowerBody.butt.analLooseness < 3) {
			MainScreen.text("Your nerves screech out in pain as your rear revenant stretches out her puckered little \"jaw\" to something more fitting for what she has in store.  A few seconds later and the sensations begin to only register as pleasure.");
			//[if (corruption < 50)
			if (player.stats.cor < 50) MainScreen.text("..  This only worries you further.");
			MainScreen.text("  It doesn't take her long to <b>prepare your anus for any insidious ploy she's thought up.</b>");
		}
		//if analLooseness >= 3
		else {
			MainScreen.text("\"<i>Looks as though you already get plenty of action back here, chief,</i>\" she jests.  Nevertheless, the rear revenant massages her \"jaw\" even more lax than you already are.  Even stranger, each gyration provides more and more pleasurable feedback");
			if (player.stats.cor < 50) MainScreen.text(", which only serves to confuse you");
			MainScreen.text(".");
		}
		...
		if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || Flags.list[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] > 0) MainScreen.text("You recognize it instantly as she pulls it free from her chest: the nearly foot-long floppy horse cock she impaled you on before.");
		//{if SHOULDRA_GENDERLESS_FUCK_COUNT = 0}
		else if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0) MainScreen.text("She holds her catch up clear for you to see; to your horror, you have clutched onto an enormous horse cock replica.  All 13 inches flop restlessly in your grip, seemingly taunting you with the foreboding future that lays before you.");
		MainScreen.text("  Shouldra smacks the dildo of doom onto a nearby boulder as she wearily rises you to your [feet].  The inferior, forgotten playthings slide out uselessly from the ghoul's gasping craw as she slowly clambers over top the wiggling horsey wang.");
		...
		player.lowerBody.butt.analLooseness = 5;
		MainScreen.text("\n\n", false);
		player.orgasm();
		player.stats.sens += -2;
		shouldraSleeping((10 + rand(26)), true);
		Flags.list[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT]++;
	...
	private nongenderlessAnalShouldraMasturbation(): void {
		...
		//if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 OR event occurrence > 0
		if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || Flags.list[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] > 0) MainScreen.text("You recognize it instantly as she pulls it free from her chest: the nearly 2-foot floppy horse cock she impaled you on before.");
		//if SHOULDRA_GENDERLESS_FUCK_COUNT = 0
		else MainScreen.text("She holds her catch up clear for you to see; to your horror, you have clutched onto an enormous horse cock replica.  All 13 inches flop restlessly in your grip, seemingly taunting you with the foreboding future that lays before you.");
		MainScreen.text("  Shouldra smacks the dildo of doom onto a nearby boulder as she wearily rises you to your [feet].  The inferior, forgotten playthings slide out uselessly from the ghoul's gasping craw as she slowly clambers over top the wiggling horsey wang.");
		MainScreen.text("\n\nToo tired to object or plead, you meekly brace for impact as the wraith lines up its target.  Your immense cheeks part way.  Your rectum gapes wide.  Your anal tongue presses firmly against your taint.  \"<i>Down the hatch!</i>\"");
		MainScreen.text("\n\nThe forest shudders with your scream.  The phantom easily downs six inches on the first go.  She wraps her tongue around the shaft, forcing more and more into you.  You swear your pelvis is bulging.  Muffled moans blended with laughter emit from your belabored behind.  You dare not guess how deep Shoudra drills the cock into you before she climaxes - hard.  Your voluptuous ass quavers with ferocity alongside her gagged yelping, throwing you from your perch atop the rock back down to the earth.  Her fun over, you begin to feel the magic subsiding.  Having no desire to learn of the consequences of leaving a massive wang lodged in your shrinking anus, you muster what little strength you can to plumb it free.  With the experience settling down, you can't help but laugh - closer to a timid chuckle, as its the best your exhausted body can offer.  You think you hear Shouldra saying something to you, but you're too worn out to listen to her boasting.");
	...
	private maleMasturbationProper(): void {
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("You are nearly overwhelmed by Shouldra's delight when you allow her to focus on your [cock].  She instantly takes the lead, steering you behind a good-sized boulder and flinging your [armor] piece-by-piece as you go.");
		...
		//if cocksize <200: 
		if (player.cockArea(0) < 200) MainScreen.text("  You stagger a bit at the unaccustomed heft of your junk altering your center of gravity, but you know that will wear off soon enough.");
		MainScreen.text("  She reaches into her tunic and tosses you a full bottle of ectoplasm, giving you a wink before incorporealizing once more and floating into you.  All's well that ends well.");
		player.orgasm();
		player.stats.sens += -2;
		MainScreen.text("\n\n", false);
		shouldraSleeping((10 + rand(26)), true);
	...
	private girlyMasturbationWithGhosts(): void {
		...
		MainScreen.text("\"<i>Tsk!  Saying those kinds of things aloud!  Still... if you want to let it out so badly, I might let you if you beg a little more...</i>\"");
	...
	private shouldraTongueLicksPCs(): void {
		...
		//if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 || event occurrence > 0
		if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || Flags.list[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) MainScreen.text("  The consistency of your forgetfulness while traveling this path leads you to believe that your ethereal inhabitant would rather you not come back here on your own.");
		...
		if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0 && Flags.list[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) MainScreen.text("  Within lies a rather rustic yet well-built chest.");
		else if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 && Flags.list[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) MainScreen.text("  \"<i>You didn't think I'd leave my most treasured possessions behind for some goblin to run into, did you?</i>\"  It's the ghost girl's chest of dildos, as well-kept as the last time you laid worried eyes on it.");
		else MainScreen.text("  Inside rests the ghost's familiar treasure trove of dildos and other toys you have yet to discover.");
		MainScreen.text("  She has you pull the container out from its hiding place, removing the clear tarp she used to protect it from the elements.  Despite the fact that you have yet to control your body once through this entire ordeal, an intrinsic urge to open the container has you bending over to pry into the contents within.");
		...
		//if event occurrence == 0
		if (Flags.list[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) {
			...
			if (player.upperBody.head.face.tongueType > TongueType.HUMAN) MainScreen.text("some of ");
			MainScreen.text("your tongue out, examining it.  Already, you think you've got a pretty good idea of where she's going to take this.");
		...
		if (player.perks.has("Flexibility")) {
			...
			if (Flags.list[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) {
				...
				MainScreen.text("\n\nShe sits you down and lets her dildos fall to the ground, forgotten for the moment.  \"<i>How's this one go?</i>\" she asks, leaning down in an awkward attempt to get your face in your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".  You start to explain the proper 'technique' to what she's trying to do, but before you can even finish your thought she's stuck a leg up in the air and has your mouth to your own snatch.  You doubt you'll ever get used to having her inside your thoughts.");
			}
			...
			if (Flags.list[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) {
				MainScreen.text("  Almost shocked enough to wrest control away from your companion, you begin to babble dissent and try to reel your lengthening tongue back in to no avail.  Shouldra stops for a moment, quite frustrated by the interruption.  Sitting straight up, she consoles you, \"<i>Relax, Champ, it'll stop feeling weird in a minute and you know it's all temporary.</i>\"");
			...
			MainScreen.text(" sliding fingers along the wet appendage, humming a little tune over your tongue until more than a foot of it droops from your mouth.  \"<i>Now we're getting somewhere,</i>\" you hear her thinking as she works her magic.  There doesn't seem to be a limit to how much she can pull out of your mouth, and soon she has more than three feet of pure tongue coiled in your lap.  Furrowing your brow in concentration, she tries to move your tongue about, but can't manage to wiggle more than the tip.  \"<i>Too thin,</i>\" she mutters, \"<i>much too weak. Doesn't feel right...</i>\"");
		...
		//if demon tongue
			...
			if (Flags.list[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) MainScreen.text("  Almost shocked enough to wrest control away from your companion, you begin to babble dissent and try to reel your lengthening tongue back in to no avail.  Shouldra stops for a moment, quite frustrated by the interruption.  Sitting straight up, she consoles you, \"<i>Relax, Champ, it'll stop feeling weird in a minute and you know it's all temporary.</i>\"\n\nGranted a moment to adjust, you let yourself settle down and hand her the reins once more.  She happily goes back to ");
			else MainScreen.text("\n\n She happily continues ");
			MainScreen.text("sliding fingers along the wet appendage, humming a little tune over your tongue until more than an additional foot of it droops from your mouth.  \"<i>Now we're getting somewhere,</i>\" you hear her thinking as she works her magic.  There doesn't seem to be a limit to how much she can pull out of your mouth, and soon she has more than three feet of pure tongue coiled in your lap.  Furrowing your brow in concentration, she tries to move your tongue about, but can't manage to wiggle more than the tip.  \"<i>Too thin,</i>\" she mutters, \"<i>much too weak. Doesn't feel right...</i>\"");
			MainScreen.text("\n\nTaking the midsection of your tongue in your hand, she slowly starts to rub her hand along some of its length.  It feels... odd, somehow, but not entirely unpleasant.  Though you shudder at the first rub, they start to feel more natural as she pushes you on.  Actually, it feels sort of good.  Her strokes remain constant, though some part of you wishes she'd go faster.  It doesn't even register as odd in your addled mind that your tongue is swelling in your hand with every stroke.  Shouldra starts to make wider movements along your smooth stretch of muscle.  Its girth plumps up to fill your palm nicely wherever she brings your hand, and the strange sensations of your flesh on its nerves wrack your body with shudders of pleasure even through Shouldra's grip.");
		...
		shouldraSleeping((10 + rand(26)), true);
		Flags.list[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES]++;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	public shouldraDream1(): void {
		...
		else if (Flags.list[FlagEnum.SHOULDRA_PLOT_LEVEL] == 1) {
		...
		else if (Flags.list[FlagEnum.SHOULDRA_PLOT_LEVEL] == 2) {
			//Dream 3 - "Recent" Goblin Possession
			//triggers one week after Dream/Talk 2
			MainScreen.text("Few things more relaxing than heading home after a day of work.  But as you approach the dilapidated hut, some strange noises from within catch you off guard.  If it's that fucking mouse again...");
		...
		if (Flags.list[FlagEnum.SHOULDRA_PLOT_LEVEL] == 3) {
			//Dream 4 - Backstory
		...
		Flags.list[FlagEnum.SHOULDRA_PLOT_LEVEL]++;
		Flags.list[FlagEnum.SHOULDRA_TALK_NEEDED] = 1;
		doNext(playerMenu);
	}
	...
	private shouldraYappin(): void {
		MainScreen.clearText();
		//Plot dreams!
		if (Flags.list[FlagEnum.SHOULDRA_TALK_NEEDED] == 1) {
			//unlocked with Dream 1
			if (Flags.list[FlagEnum.SHOULDRA_PLOT_LEVEL] == 1) {
				//Clear out that talk happened and start a countdown till next dream sequence.
				Flags.list[FlagEnum.SHOULDRA_TALK_NEEDED] = 0;
				spriteSelect(67);
				if (Flags.list[FlagEnum.SHOULDRA_PLOT_LEVEL] < 4) Flags.list[FlagEnum.SHOULDRA_PLOT_COUNTDOWN] = 7;
				...
				MainScreen.text("You decide not to beat around the bush and ask about the strange dream you had the other night.  The spirit puts on a surprised reaction, your shoulder returning to normal as she casually walks outside of your body.");
				...
				if (player.perks.has("Incorporeality")) MainScreen.text("  Shouldra bumps you to the side a little as she makes her next point, \"<i>Even with your hoaky quasi-possession power, you don't really know what it's like to actually possess someone.</i>\" You look at her funny, unaware that you weren't really getting the full effect.  Not that you care!");
				...
				MainScreen.text(".  \"<i>No matter what treasure trove of knowledge I dig up from you, there's always something I'm more interested in.</i>\"  Your hand lifts back up, and you watch as Shouldra's hand splits off from your own, meeting below the wrist.  She holds your hand with her own ghostly appendage, resting her head on your shoulder.  The ghost girl sits there for a moment, silent, leaving you to wonder if maybe, just maybe, you're starting to mean a little more to her than just a sex toy.");
			...
			//Talk 2 - First Transformation Discussion
				...
				if (player.gender > 0) {
					MainScreen.text("  Your [if (hasCock = true) [cock]][if (isHerm = true)  and ][if (hasVagina = true) [vagina]] start to swell up; looking up at the ghost, you realize that she hasn't uttered a single word!");
				}
				else MainScreen.text("  A fleshy, penis-shaped bulge begins to swell out of your loins; looking up at the ghost, you realized she hasn't uttered a single word!");
				...
				if (player.gender == 0) MainScreen.text("dick");
				MainScreen.text(" [if (isHerm = false) continues its][if (isHerm = true) continue their] march of magnification while your fingers vigorously explore their new frontiers.");
				...
				if (player.gender == 0) MainScreen.text("burgeoning dickflesh");
				MainScreen.text(".  It takes some gumption for you to work through the arousal and contribute to the conversation, but you manage to point out how you're pretty sure there are other people in Mareth with the same sort of power, to some degree at least.");
			...
			//Talk 3 - "Recent" Goblin Possession Discussion
			else if (Flags.list[FlagEnum.SHOULDRA_PLOT_LEVEL] == 3) {
				//Clear out that talk happened and start a countdown till next dream sequence.
				Flags.list[FlagEnum.SHOULDRA_TALK_NEEDED] = 0;
				spriteSelect(67);
				if (Flags.list[FlagEnum.SHOULDRA_PLOT_LEVEL] < 4) Flags.list[FlagEnum.SHOULDRA_PLOT_COUNTDOWN] = 7;
				//unlocked with Dream 3
				MainScreen.text("You stare at Shouldra.  \"<i>Oh, <b>what</b>?</i>\"  You stare at Shouldra.  \"<i>She fucking pissed on my floor!</i>\"  You stare at Shouldra.  The ghost shakes her head, sinking it back inside your skin. The ghost girl quickly flies out of your chest.  \"<i>You don't fuck with my stuff!</i>\"");
				...
				if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("</i>\"  Shouldra eyes your [cock] gingerly, acting as if she can stare right through your outfit at it.  \"<i>");
				else MainScreen.text("  ");
				MainScreen.text("The half-millenium or so of scarcity has really shaped my interests.  I'm at the point now where I just lose it whenever I get these ghostly mitts on a slab of meat.  And the big ones?  ");
				if (player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 200) MainScreen.text("Ones like yours?  ");
				MainScreen.text("Then I'm just in heaven.</i>\"  Hold on.  She can turn any dick into a towering monument.  What's so special about ones that come pre-packaged, you ask?  \"<i>I kinda break the one rule I had at that point, but when I find someone with a big boner, it just... feels so good to me.  There's just something about an all-natural magnum opus that just lights my fire.</i>\"");
				...
				MainScreen.text("\"<i>Oh,</i>\" she says, stepping out of you and sitting herself across from you.  \"<i>You saw that.</i>\"");
		...
		//Generic Oni Discussion!
			...
			MainScreen.text("On a whim, you decide to have a chat with your spectral companion, who obligingly pops free of your torso to spare you the awkward sensation of conversing with one of your own body parts.");
	...
	public shouldraFollowerScreen(): void {
		spriteSelect(67);
		MainScreen.clearText();
		if (!shouldraSleeping()) MainScreen.text("You pulse a desire for interaction with Shouldra, summoning her head to form from your shoulder. \"<i>What's on your mind, Champ? I could've found out on my own, but I was... preoccupied.</i>\"");
		//Follower Screen Quick Text - Shouldra asleep
		else {
			MainScreen.text("You try and contact Shouldra, but the ghost is still sound asleep within you.  Or whatever ghosts consider sleep after an arousing round of debauchery. Either way, you garner no response.  Best to try again later.");
			doNext(playerMenu);
			return;
		}
	...
	private shouldraSexMenu(): void {
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("Shouldra stirs from somewhere deep inside you, excitement evident in the swiftness of her reaction.  \"<i>Is it time for some fun, Champ?</i>\" she asks happily, her increased lust affecting you just a bit.  Do you oblige her?");
		let vala: Function = null;
		if (Flags.list[FlagEnum.FREED_VALA] != 0 && (model.time.hours >= 12 && model.time.hours <= 21)) {
	...
	private shouldraTalkMenu(): void {
		...
		let blowShitUp: Function = null;
		let shrinkShit: Function = null;
		if (Flags.list[FlagEnum.SHOULDRA_PLOT_LEVEL] >= 2 && Flags.list[FlagEnum.SHOULDRA_MAGIC_COOLDOWN] <= 0) {
			blowShitUp = shouldraGroPlus;
			shrinkShit = shouldraReductoMenu;
			MainScreen.text("\n\nShouldra could permanently expand or reduce part of your body.");
		}
		else if (Flags.list[FlagEnum.SHOULDRA_PLOT_LEVEL] >= 2) MainScreen.text("\n\nShouldra needs some time before she can grow or shrink one of your body parts permanently.");
	...
	private shouldraGroPlus(): void {
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("You decide to take up Shouldra's offer on natural body enhancement.  Before you tell her as much, the eager spectre quickly assumes control of your body.  \"<i>Alright, Champ, now we're in business.  So, what did you have in mind?</i>\"");
		//Balls     Breast     Clit     Cock     Nipples     Butt     Nevermind
		let balls: Function = null;
		let breast: Function = shouldraGrowsYoTits;
		let clit: Function = null;
		let cock: Function = null;
		let butt: Function = shouldrasButtBigginator;
		let nipples: Function = shouldraGivesYaSomeFukkinTeats;
		if (player.lowerBody.balls > 0) balls = groBallsBiggaGHOSTYSTYLE;
		if (player.lowerBody.cockSpot.hasCock()) cock = shouldraCockBloating101;
		if (player.lowerBody.vaginaSpot.hasVagina()) clit = shouldraGrowsYoClit;
	...
	private groBallsBiggaGHOSTYSTYLE(): void {
		Flags.list[FlagEnum.SHOULDRA_MAGIC_COOLDOWN] = 72;
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("\"<i>Interested in a bigger ballsack, huh?  Not only will it help pronounce your package, but a big ol' swinging sack of nuts feels great when you're really pounding.</i>\" Shouldra grasps your [balls] with both your hands and begins reciting the appropriate spell.");
		//ballsize + 2 + rand(4) 20% chance
		if (rand(5) == 0) {
			player.lowerBody.ballSize += 2 + rand(4);
		...
		//ballsize + 1 + rand(2) Else
			...
			player.lowerBody.ballSize += 1 + rand(2);
		...
		//if ballsize > 10
		if (player.lowerBody.ballSize > 10) MainScreen.text("  You second guess your decision when your swinging cum barrels seem to impede walking.  Shouldra reassures you that the enhanced libido and baby batter supply is worth the wider gait.");
	...
	private shouldraGrowsYoTits(): void {
		...
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("\"<i>I'll give you a rack that could hypnotize anyone into a faithful tit slave,</i>\" Shouldra boasts, stretching your shoulders in preparation.  Your hands grope your [fullChest] as the spirit slips into the arcane language you've come to grow so familiar with.\n\n");
		//same growtits() as Gro+
		if (player.upperBody.chest.count() == 1) player.growTits((1 + rand(5)), 1, true, 1);
		else player.growTits(1 + rand(2), player.upperBody.chest.count(), true, 1);
		player.stats.lust += 10;
		doNext(camp.returnToCampUseOneHour);
	}
	//Clit
	private shouldraGrowsYoClit(): void {
		...
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("\"<i>Looking to give your joy buzzer a little jumpstart?</i>\" Shouldra teases, your hands sliding down to your inviting [cunt].  Your fingers tease your [clit] while the ghost girl spouts her arcane tongue from your mouth.  ");
		//clitLength + 1
		player.lowerBody.vaginaSpot.get(0).clitLength += 1;
		MainScreen.text("She welcomes the new inch with a quick flick.  The sensitive [clit] twitches happily in response, confirming the success of the spell.");
		player.stats.sens += 5;
	...
	private shouldraGivesYaSomeFukkinTeats(): void {
		...
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("Shouldra rubs your hands together in anticipation, \"<i>A beautiful blossoming pair of nipples is what you need to really compliment a nice set of tits.</i>\"  Your hands gleefully tweak your [nipples] as the spirit's incantation gets underway.");
	...
	private shouldrasButtBigginator(): void {
		...
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("Shouldra's haunting laugh works its way out from your mouth.  \"<i>Looking for a little more jiggle in your step, Champ?  I can get BEHIND that.</i>\"  This damnable ghost won't let you roll your eyes!  Is there no end to her torment?  After a quick crack of your knuckles, Shouldra directs your hands back onto your derriere as she transitions into spellcasting.");
		//this.player.lowerBody.butt.buttRating = this.player.lowerBody.butt.buttRating + 1 + rand(3)); or so
		player.lowerBody.butt.buttRating += 1 + rand(3);
	...
	private shouldraReductoMenu(): void {
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("Shouldra's offer to shrink aspects of your body sounds right up your alley, and you pulse as much to your ghostly compatriot.  There's no response.  You remind her that it was HER suggestion in the first place.  \"<i>Come ooooon, Champ.  It was just a slip of the ol' spiritual tongue.</i>\"  You cross your arms and look up and off to the side, the best way you can figure to glare at something that isn't there. \"<i>Ugh, fine.  Let's just get this over with,</i>\" Shouldra concedes, removing your [armor].");
		//Balls     Breast     Clit     Cock     Nipples     Butt     Nevermind
		let balls: Function = null;
		let breasts: Function = null;
		let clit: Function = null;
		let cock: Function = null;
		let butt: Function = null;
		let nipples: Function = null;
		if (player.upperBody.chest.BreastRatingLargest[0].nippleLength > .25) nipples = shrinkDemNipplzForYoGhost;
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) breasts = shouldraReductosYourTits;
		if (player.lowerBody.butt.buttRating >= 2) butt = shrinkDatBootyForYoGhost;
		if (player.lowerBody.balls > 0 && player.lowerBody.ballSize > 1) balls = shouldraReductosYourBallsUpInsideYa;
		if (player.lowerBody.cockSpot.hasCock() && player.longestCockLength() > 4) cock = shouldraMakesCocksDisappear;
		if (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.vaginaSpot.get(0).clitLength > .25) clit = clittyVanishingActShouldra;
	...
	private shouldraReductosYourBallsUpInsideYa(): void {
		...
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("\"<i>Why would you want to shrivel these delicious cumquats?</i>\"  Shouldra's arsenal of puns doesn't affect your request in the slightest.  The ghost girl moves into a quick spell, savoring your testicles before finishing.");
		//this.player.lowerBody.ballSize = this.player.lowerBody.ballSize - (2 + this.rand(4));
		player.lowerBody.ballSize -= (2 + rand(4));
	...
	private shouldraReductosYourTits(): void {
		...
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("Your arms clasp themselves around your [fullChest].  \"<i>Not the gals!  What monster would want to deflate their knockers?</i>\"  This monster, evidently.  Shouldra sighs, holding your hands in a weak casting stance as she gets the heinous act over with.");
		player.shrinkTits();
	...
	private clittyVanishingActShouldra(): void {
		...
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("\"<i>Aw, really, Champ?  The best part of your snatch?</i>\"  A nod of your head reasserts your desire.  The depressed phantom hastily works through the spell, the notion of shrinking your [clit] not sitting well with her.");
		player.lowerBody.vaginaSpot.get(0).clitLength /= 1.7;
		//Set clitlength down to 2 digits in length
		player.lowerBody.vaginaSpot.get(0).clitLength = int(player.lowerBody.vaginaSpot.get(0).clitLength * 100) / 100;
	...
	private shouldraMakesCocksDisappear(): void {
		...
		if (player.lowerBody.cockSpot.count() == 1) {
			...
			let choice: number = rand(10);
			if (choice == 0) MainScreen.text("murderer");
			else if (choice == 1) MainScreen.text("bastard");
			else if (choice == 2) MainScreen.text("jerk");
			else if (choice == 3) MainScreen.text("monster");
			else if (choice == 4) MainScreen.text("cretin");
			else if (choice == 5) MainScreen.text("fiend");
			else if (choice == 6) MainScreen.text("asshole");
			else if (choice == 7) MainScreen.text("villain");
			else MainScreen.text("dick");
			MainScreen.text("!</i>\" it shouts.  The tiny voice fades away as your [cock] goes still, resting at two thirds of its original size.  A single tear rolls down your cheek; you suspect the melodramatic spirit is at fault here.");
			player.lowerBody.cockSpot.get(0).cockLength *= 2 / 3;
			player.lowerBody.cockSpot.get(0).cockThickness *= 2 / 3;
		}
		//MULTI
		else {
			...
			for (let ii: number = 0; ii < player.lowerBody.cockSpot.count(); ii++) {
				player.lowerBody.cockSpot.get(ii).cockLength *= 2 / 3;
				player.lowerBody.cockSpot.get(ii).cockThickness *= 2 / 3;
			}
		}
		player.stats.sens += -2;
		player.stats.lust += -10;
	...
	private shrinkDemNipplzForYoGhost(): void {
		...
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("\"<i>Are you kidding me, Champ?  Nipples are the most important-</i>\"  You cut the ghost girl's argument there, not wanting to leave your decision up for debate.  She instead leaves you with a deep sigh of disapproval before quickly casting the appropriate spell.");
		//Shrink
		if (player.upperBody.chest.BreastRatingLargest[0].nippleLength / 2 < .25) {
	...
	private shrinkDatBootyForYoGhost(): void {
		...
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("Your hands quickly grasp at your bare [butt].  \"<i>You... you're desecrating a national treasure here!</i>\"  The rather odd declaration gets a chuckle from you, but you remain firm in your request to dull your derriere.  The ghost girl furthers her attempts to change your mind, but without literally changing your mind, you remain steadfast.  Defeated, your hands take one last trip around your posterior before the poltergeist recites her incantation.");
	...
	private kickFollowerShouldraOut(): void {
		...
		if (Flags.list[FlagEnum.SHOULDRA_PLOT_LEVEL] < 4 || Flags.list[FlagEnum.SHOULDRA_TALK_NEEDED] > 0) {
			MainScreen.text("\"<i>Ah, you're booting me out, hey?</i>\"  Shouldra asks innocently, tumbling out of your body and corporealizing. \"<i>Wouldn't be the first, and won't be the last.  I do realize I have certain 'needs' that got cloying to many of my former hosts.</i>\"");
			...
			Flags.list[FlagEnum.SHOULDRA_FOLLOWER_STATE] = 0;
			doNext(camp.returnToCampUseOneHour);
		}
		//She Fucking Explodes
		else {
			MainScreen.text("Are you sure you want to ask her to leave after she opened herself up to you?  It's just a hunch, but she might not take this very well.");
	...
	public shouldersWarnings(): boolean {
		...
		if (Flags.list[FlagEnum.SHOULDRA_SLEEP_TIMER] == -96) {
			spriteSelect(67);
		...
		//7 days (168 hours) since last Shouldra scene
		else if (Flags.list[FlagEnum.SHOULDRA_SLEEP_TIMER] == -168) {
			spriteSelect(67);
			...
			if (rand(3) == 0) {
				...
				if (player.gender == 0) MainScreen.text("libido seems to be going haywire");
				MainScreen.text(".  Shouldra's unfulfilled sexual needs are starting to blend into your own.  Nothing you do will satisfy your urging until she's seen to her own.\n\n<b>Minimum lust raised by 20.</b>\n");
			}
			else if (rand(2) == 0) {
	...
	public nightTimeShouldraRapesThePC(): void {
		...
		Flags.list[FlagEnum.SHOULDRA_SLEEP_TIMER] = 0;
		spriteSelect(67);
		player.orgasm();
		player.stats.lib += 1;
		player.stats.sens += 4;
		...
		//event occurance = 0
		if (Flags.list[FlagEnum.SHOULDRA_TIMES_NIGHT_RAPED_PC] == 0) {
		...
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text(", circling their entire lengths around your [fullChest]");
		MainScreen.text(".  It's quite amazing how ecstatic the tour feels, each pre-smeared exploration smothering you in sensual vibes.  Just moving the stiff little devils - particularly at your joints - sends shivers up your arm.");
		...
		MainScreen.text(", softening the ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("magically enlarged member");
		else MainScreen.text("magically gaped snatch");
		MainScreen.text(" back to a much less craggy complexion.  Shouldra slowly drags you back to your [feet] after a minute or two of blissful respite and walks you closer to the cliffside to stare off into the moonlit horizon.  It never really dawned on you just how fucking high you are; just where the fuck did she take you?");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("flailing cock");
		else MainScreen.text("moist vagina");
		MainScreen.text(".");
		//if cock
		if (player.lowerBody.cockSpot.hasCock()) {
	...
	private edwardShouldraDickHandsPartII(): void {
		...
		Flags.list[FlagEnum.SHOULDRA_TIMES_NIGHT_RAPED_PC]++;
		doNext(playerMenu);
	}
	...
	public shouldraWakesUpOrPokesPCsForShitsAndGigglesIdunnoHowLongCanIMakeThisFunctionNameQuestionMark(): void {
		...
		//1 PC with cock - 1
		if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.cockSpot.biggestCocks[0].cockArea() < 200) {
			choices[choices.length] = 1;
			choices[choices.length] = 1;
			choices[choices.length] = 1;
			choices[choices.length] = 1;
		}
		//2 PC with BIG COCK - if (this.player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 200)
		...
		if (player.statusAffects.get("Exgartuan").value1 == 1 && 9999 == 9999) {
		...
		choices[choices.length] = 7;
		//8 (Tel'Adre unlocked)
		if (player.statusAffects.get("TelAdre").value1 >= 1) choices[choices.length] = 8;
		//9 (is preggers)
		...
		//13 (is Naga)
		if (player.lowerBody.isNaga()) choices[choices.length] = 13;
		//14 (Amily is a follower)
		if (kGAMECLASS.amilyScene.amilyFollower()) choices[choices.length] = 14;
		//15 (Pure Jojo is a follower)
		if (player.statusAffects.has("PureCampJojo")) choices[choices.length] = 15;
		//16 (has any of Marae's gift perks)
		if (player.perks.has("MaraesGiftStud") || player.perks.has("MaraesGiftFertility") || player.perks.has("MaraesGiftProfractory") || player.perks.has("MaraesGiftButtslut")) choices[choices.length] = 16;
		//17 (has had children with Izma)
		if (Flags.list[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] + Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] > 0) choices[choices.length] = 17;
		//18 (has Incorporeality perk)
		if (player.perks.has("Incorporeality")) choices[choices.length] = 18;
		//19 (possessed by Boobgartuan)
		if (player.statusAffects.get("Exgartuan").value1 == 2) choices[choices.length] = 19;
		//20 (PC knows any spells)
		if (player.hasSpells()) choices[choices.length] = 20;
		//21 (PC has anemone dick)
		if (player.lowerBody.cockSpot.countType(CockType.ANEMONE) > 0) choices[choices.length] = 21;
		...
		switch (select) {
			...
			case 6:
				subSelect = rand(6);
				if (subSelect == 0) MainScreen.text("While minding your business, you realize faint whispering is coming from your [cock].  Shouldra and Exgartuan are scheming something, probably something you wouldn't be a fan of.  Every so often your wang lets out a chuckle.  Maybe getting these two together wasn't in your best interest.");
				else if (subSelect == 1) MainScreen.text("Your [cock] is wiggling around inside your [armor], making you incredibly uncomfortable.  You go to undo your outfit when the demonic dong plows its way out anyway, scaring you half to death.  Shouldra and Exgartuan just laugh, too content with one another to pay you much mind.  You never expected them to get along THIS well.");
				else if (subSelect == 2) MainScreen.text("\"<i>So what other sorts of things can you concoct?</i>\" Exgartuan asks, shuffling around in your armor.  You're incredibly confused by the sudden question.\n\n\"<i>Honestly, that put me in the mood to make all of mother nature our bitch,</i>\" you respond... Shouldra responds.  The spirit is gabbing through your mouth to your possessed pecker.  The two prattle on like this for some time.");
				else if (subSelect == 3) MainScreen.text("Your hands slip inside your [armor], exploring your [cock].  Your fingers caress each little nub and nodule they come across.  \"<i>That's the ticket,</i>\" Exgartuan gleefully responds, getting rigid in the tight confines of your outfit.  Looks like your ghostly partner is out to make everyone happy.");
				else if (subSelect == 4) MainScreen.text("\"<i>Why don't you help out during battles, babe?</i>\" Exgartuan asks rather bluntly.\n\n\"<i>Hey, I've offered to help out with Champ's more seductive poses like you do...</i>\"\n\nIt isn't like you have much choice in the matter as far as Exgartuan is concerned!  The two continue their discussion, belittling your apparently meager combat performance.  Maybe your possessors could actually help deal some real damage for a change!");
				else MainScreen.text("Your [armor] has vanished!  You're butt naked!  It's pretty obvious who's at fau-\n\n\"<i>Really, [name], you don't even need it,</i>\" Exgartuan says, surprising you.\n\nShouldra laughs, \"<i>Right, Champ?  Two sexually-charged tyrants dwelling within you and you insist on sneaking around like common rabble?  Why not let it all hang out?</i>\"\n\nYour hands caress your [cock], exciting the demonic member into attention.  You remind the couple that your outfit isn't <b>just</b> for protection, citing many other-  Oh, your [armor] is back.  Guess they don't want to hear you prattle.");
				break;
			//General flavor text
			case 7:
				subSelect = rand(6);
				if (subSelect == 0) MainScreen.text("\"<i>Yoooo, Champ, are we gonna fuck something, or am I just gonna stagnate here?</i>\" Shouldra moans, managing to steal control of an arm long enough to give your [butt] a honk.  You sigh.");
				else if (subSelect == 1) MainScreen.text("Out of nowhere, a colossal yawn bursts forth from your craw.  \"<i>Come on, Champ. I'm dying to try out some new stuff I've cooked up.</i>\" What is this sense of dread consuming your being?");
				else if (subSelect == 2) MainScreen.text("It's been pretty quiet recently.  No doubt Shouldra is up to something -- she always is.  Honestly, you've forgotten what it's like to not having your body passing for a bloated something or other.");
				else if (subSelect == 3) MainScreen.text("As you are going about your business, you swear you make out muttering emanating from somewhere on your person. It sounds like the ghost girl is reciting some magic, but you don't feel any different. This only worries you even more.");
				else if (subSelect == 4) MainScreen.text("\"<i>Do foot fetishes still apply if it's your own foot?</i>\" Shouldra asks.  For a moment you wonder how such a feat would even be possible.  But only for a brief, forgetful moment...");
				else if (subSelect == 5) MainScreen.text("A familiar voice works its way from the back of your mind, \"<i>Hey, Champ.  You think there's any way to make armpits sexy?  What about knees or noses?</i>\"  This line of questioning could lead to some very troubling consequences...");
				else MainScreen.text("\"<i>I like what you did with those racks,</i>\" Shouldra says unexpectedly, her voice losing its mischievous edge for a moment.  \"<i>I'm glad you kept them.</i>\"  You both sit in silence for several moments as you consider her mood shift to tenderness, but the moment passes.  Shouldra chuckles softly.  \"<i>Just thought I'd mention.</i>\"");
				break;
			//Special Flavor Texts (misc. prereqs)
			//(Tel'Adre unlocked)
			case 8:
				MainScreen.text("Somehow, someway, you can feel Shouldra sifting through your memories.  \"<i>I'd love to visit the bakery in that desert town.  It's been at least a century since I tasted a decent sweet of some sort.</i>\"");
				break;
			//Preggo
			case 9:
				MainScreen.text("Your hands circle your bulging belly unbidden.  Shouldra seems to be enjoying your pregnancy.  \"<i>A good deal of the goblins I got a hold of were pregnant, but they don't really treat the event with anything other than their usual 'decorum.'</i>\"  The ghost girl lets out a deep sigh.  \"<i>There isn't anything wrong with enjoying it while you can, is there?</i>\"  Do the dead normally have this much interest in the birth of the living?");
				break;
			//(is preggers2)
			case 10:
				MainScreen.text("\"<i>Looks like the thing you're pregnant with is doing all right there, Champ,</i>\" Shouldra comments, your belly wiggling around a bit.  You ask that the ghost not go messing around with pregnancy, fearful of the consequences.  \"<i>Oh, please.  I wouldn't do anything crazy like that...</i>\" You can't see her, but you can picture the expression the ghost made as she trailed off.  And it worries you.");
				break;
			//(is butt pregnant)
			case 11:
				MainScreen.text("\"<i>I must admit, Champ, haven't run into too many instances of rear-ended pregnancies!</i>\"  Shouldra laughs, smacking your hand on your [butt].  \"<i>Just make sure that doesn't have to contend with all the other things that go in and out of your ass!</i>\"  You try not to dignify the crass remark with any sort of response.");
				break;
			//(can fly)
			case 12:
				MainScreen.text("You're taken by surprise when your wings act all on their own, lifting you off the ground with strong flaps.  \"<i>I'm trying to think of ways to use your own flight in some kinky shit, Champ,</i>\" Shouldra comments, \"<i>Nothing's coming to mind that's any better than me just floating your carcass instead.</i>\"  Well, we aren't all \"blessed\" enough to be deceased spirits!");
				break;
			//(is Naga)
			case 13:
				MainScreen.text("Your tail wiggles around on its own suddenly.  Catching on to how it appears to be sneaking to play around with something on your body, you calmly ask its spiritual captor to knock it off.  An otherworldly snicker emanates from it before it goes limp.");
				break;
			//(Amily is a follower)
			case 14:
				MainScreen.text("Your fingers move around a little, signaling Shouldra's stirring.  \"<i>It would seem that that mouse follows me everywhere I go,</i>\" she complains.  You figure she's talking about Amily.  You ask what her beef is.  \"<i>Too much baggage.  Don't want to deal with it.  I scared her a few times back at the town, but it wasn't all that satisfying.</i>\"  Looks as though Shouldra wasn't always the warmest of neighbors.");
				break;
			//(Pure Jojo is a follower)
			case 15:
				MainScreen.text("The silence bothers you a bit, so you decide to see if Shouldra is awake and ask her what she thinks about Jojo.  \"<i>You're talking about that holy mouse morph back at camp, right?  Not a fan,</i>\" she replies, shrugging your shoulders.  \"<i>Other than just not really being into people that remind me of rodents, I don't think he'd get along well with 'a spirit that has yet to abandon these mortal bonds.'</i>\"  she mocks.");
				break;
	...
	private shouldraAndWormsYoureGonnaHaveABadTime(): void {
		...
		if (player.lowerBody.cockSpot.get(0).cockLength < 48) MainScreen.text(" and cuts right to the chase, ballooning your [cock] into a rigid, wide, four-foot tall slab of meat");
		MainScreen.text(".  Unable to contain her delight, she hastily hugs your enormous dick firmly against your body and squeezes vigorously, rubbing your face against your [cockHead].  A warm stream of pre oozes out and onto your possessed face.");
		...
		if (model.time.hours <= 12) MainScreen.text("morning ");
		else if (model.time.hours <= 5) MainScreen.text("afternoon ");
		else MainScreen.text("evening ");
		MainScreen.text("air.  Your hands clue the eidolon in to further activity, introducing her to your urethra all the way down to your [balls], every square wriggling inch chock-full of the little buggers, exploring their extended living quarters.");
	...
	private kickOutShouldra4YoWormyBuddies(): void {
		...
		if (player.lowerBody.cockSpot.get(0).cockLength < 48) MainScreen.text("  With that little ordeal dealt with, you now are left to decide how best to manage the time until your cock returns to its normal size.");
		MainScreen.text("  Oddly enough, you hope the little buggers appreciate you sticking up for them.");
	...
	public shouldraAndExgartumonFightGottaCatchEmAll(): void {
		...
		//Unlocks Argue 2
		spriteSelect(67);
		MainScreen.text("Your bedroll looks mighty comfortable after a hard day's ");
		if (player.stats.cor < 50) MainScreen.text("work");
		else MainScreen.text("fucking");
		MainScreen.text(".  Just when you're [armor] and about to tuck in for the night, however, your [cock] goes fully erect!  The throbbing demonic mass is flailing about, knocking into your shit and making a mess.  You clutch onto the bucking bronco in an attempt to prevent any further damage when Exgartuan begins to shout, \"<i>Don't FUCKING touch me, you goddamn cunt!</i>\"");
		...
		Flags.list[FlagEnum.SHOULDRA_EXGARTUDRAMA] = 1;
		doNext(playerMenu);
	}
	...
	public exgartumonAndShouldraFightPartII(): void {
		...
		Flags.list[FlagEnum.SHOULDRA_EXGARTUDRAMA] = 2;
	}
	...
	public exgartumonAndShouldraFightPartIII(): void {
		...
		Flags.list[FlagEnum.SHOULDRA_EXGARTUDRAMA] = 3;
	...
	public exgartuMonAndShouldraShowdown(): void {
		...
		if (silly()) MainScreen.text("  It's a man's right to choose what's done with his body.  Not anyone else's!");
		MainScreen.text("  You shout loud enough to get the two to at least shut up and notice you're there.  It's time to end this little battle once and for all, starting with your first - and hopefully last - involvement.");
	...
	private keepShouldraAndKickOutExgartuan(): void {
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("You decide to jump to Shouldra's side on this one, explaining to your demonic cock your desire to go your separate ways.  Exgartuan heartily laughs at you, your [cock] wiggling in the air with each mighty heave.");
		...
		Flags.list[FlagEnum.SHOULDRA_EXGARTUDRAMA] = -0.5;
		doNext(playerMenu);
	}
	...
	public keepShouldraPartIIExgartumonsUndeatH(): void {
		...
		MainScreen.text("While slipping into your bedroll to enjoy a peaceful night's sleep, a chill runs down your spine.  You look around to-");
		...
		Flags.list[FlagEnum.SHOULDRA_EXGARTUDRAMA] = 0;
		//[next page]
	...
	private keepAllTheGhosts(): void {
		...
		if (player.cumQ() >= 1000) MainScreen.text(", trails of your cum pouring out from them");
		MainScreen.text(".");
		...
		Flags.list[FlagEnum.SHOULDRA_EXGARTUDRAMA] = 4;
		doNext(playerMenu);
	}
	...
	public shouldraBakeryIntro(): void {
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("You set off towards the bakery, and it's not long before the smell of freshly-baked pastries and sugary candies wafts through the air.  Something stirs in your mind as a familiar ghostly presence makes herself known; seems you're not the only one who noticed the scent of cinnamon.");
		...
		//first time: 
		if (Flags.list[FlagEnum.SHOULDRA_BAKERY_TIMES] == 0) MainScreen.text("You smirk, wondering how exactly a ghost manages to work up a hunger.  Your skepticism is met by an intense stabbing pain in your stomach, forcing you to buckle slightly from the cramping.  It lasts only a moment before it vanishes, however.  \"<i>You feel that, smartass?</i>\" The ghost girl sneers, \"<i>That's what decades of fasting will do to you.");
		else MainScreen.text("You point out that she's already had something at the bakery.  \"<i>Yeah, I did.  And now I'm hungry again.  Deal with it.");
		MainScreen.text("  Now, how about a snack?  Or two?</i>\" Her enthusiasm is clear...  and slightly worrying.");
	...
	private bailOut(): void {
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("You quickly turn around and head back the way you came.  There's a moan of disappointment from Shouldra, before she settles down and goes back to doing whatever she usually does.  You try not to think about it as you leave the city and return to camp.");
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private feedShouldraACake(): void {
		MainScreen.clearText();
		spriteSelect(67);
		MainScreen.text("You decide that there's no use getting cold feet now; you wanted to go the bakery anyway, no harm in letting Shouldra have some fun while you're there.  ");
		//after first time: 
		if (Flags.list[FlagEnum.SHOULDRA_BAKERY_TIMES] > 0) MainScreen.text("Well, nothing <i>permanent</i>, anyway.  ");
		MainScreen.text("You press on, soon arriving at the ramp leading down into the bakery's interior.  It's as busy as ever, with patrons milling about, chatting between bites of pastry.");
	...
	private shouldraAndMaddieSittingInATree(): void {
		...
		if (player.tone >= 60) MainScreen.text("well-toned");
		else if (player.thickness < 50) MainScreen.text("trim");
		else MainScreen.text("curvaceous");
		MainScreen.text(" figure...");
	...
	private goAlongWIthShouldrasEatingSpree(): void {
		...
		spriteSelect(67);
		//if repeat scene: 
		if (Flags.list[FlagEnum.SHOULDRA_BAKERY_TIMES] > 0) MainScreen.text("\"Man, you're not sure you want that kind of impact on your figure again...  \"");
		MainScreen.text("Ah, what the hell.  You were hungry anyway.  You let Shouldra guide the cake into your mouth.  Mmm, not bad; rich, sweet and delicate.  Shouldra chuckles in your mind as she releases her control slightly, letting you lick your fingers clean of any stray crumbs.  \"<i>Well, guess you're pretty hungry yourself!  Alright, Champ, I'll give you some freedom, but just remember you're not the only one with an appetite.</i>\" You wince as your hunger intensifies to the verge of pain; looks like you're eating for two.  Your eyes lock on to the feast before you and something snaps in your mind.  You need this food, all of this food, inside you.  <i>Now</i>.");
		...
		if (Flags.list[FlagEnum.PC_FETISH] > 0) MainScreen.text(", though not completely unwelcome");
		MainScreen.text(".  You blush furiously and back into the corner of the (now rather cold) booth");
		if (Flags.list[FlagEnum.PC_FETISH] > 0) {
			MainScreen.text(" as your heart races and blood rushes down towards your ");
			if (player.lowerBody.cockSpot.hasCock()) {
				MainScreen.text("drowsy member");
				if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" and ");
			}
			if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("trembling nether-lips");
			if (player.gender == 0) MainScreen.text("groin");
		}
		MainScreen.text(", before you realize that nobody seems to notice your predicament.  Shouldra reads your thoughts and responds, \"<i>Thought you might appreciate some privacy");
		if (Flags.list[FlagEnum.PC_FETISH] > 0) MainScreen.text(", even with that kink of yours");
		MainScreen.text(", Champ.  Here's something extra anyway, just in case.</i>\" One ghostly incantation later, and ");
		if (player.femininity <= 50) MainScreen.text("a buttoned shirt and pair of belted jeans");
		else MainScreen.text("a tight-fitting tank top and skirt");
		MainScreen.text(" adorn your person.");
		...
		Flags.list[FlagEnum.SHOULDRA_BAKERY_TIMES]++;
		player.slimeFeed();
	}
