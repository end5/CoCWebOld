...
public followerShouldra():boolean {
	return flags[FlagEnum.SHOULDRA_FOLLOWER_STATE] == 1;
}
private shouldraSleeping(changes: number = 0, reset:boolean = false):boolean {
	//Adjust sleep timer
	if(reset) {
		flags[FlagEnum.SHOULDRA_SLEEP_TIMER] = changes;
	}
	else flags[FlagEnum.SHOULDRA_SLEEP_TIMER] += changes;
	if(flags[FlagEnum.SHOULDRA_SLEEP_TIMER] < 0) flags[FlagEnum.SHOULDRA_SLEEP_TIMER] = 0;
	if(flags[FlagEnum.SHOULDRA_SLEEP_TIMER] > 48) flags[FlagEnum.SHOULDRA_SLEEP_TIMER] = 48;
	//Is she sleeping?
	if(flags[FlagEnum.SHOULDRA_SLEEP_TIMER] > 0) return true;
	return false;	
}
...
private recruitShouldraForCampSexyTimes():void {
	...
	flags[FlagEnum.SHOULDRA_FOLLOWER_STATE] = .5;
	doNext(camp.returnToCampUseOneHour);
}
...
public morningShouldraAlert():void {
	...
	flags[FlagEnum.SHOULDRA_FOLLOWER_STATE] = 1;
	flags[FlagEnum.SHOULDRA_PLOT_COUNTDOWN] = 7;
	doNext(playerMenu);
}
...
private shouldraAndValaGetBigger():void {
	MainScreen.clearText();
	spriteSelect(67);
	if(flags[FlagEnum.SHOULDRA_MET_VALA] < 1) flags[FlagEnum.SHOULDRA_MET_VALA] = 1;
...
private pureValaAndShouldraMacroMax():void {
	...
	if(player.gender > 0) {
		...
		if(player.lowerBody.cockSpot.hasCock()) MainScreen.text(", becoming firmly erect.  You happily piston your hips to grind your " + multiCockDescriptLight() + " along the pink-glossed walls, oozing your fluids in delight.");
	...
	else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(vaginaDescript());
	...
	if(player.lowerBody.cockSpot.hasCock()) {  
		...
		if(player.wetness() >= 4) MainScreen.text(", squirting with joy");
		MainScreen.text(".");
	}
...
private genderlessShouldraMasturbation():void {
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0 && flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0) {
	...
	//if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 & event occurrence = 0
	else if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 && flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0) {
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0 && flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0) MainScreen.text("Within lies a rather rustic yet well-built chest.");
	else if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 && flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0) MainScreen.text("The moment the light reveals what lies within, your [butthole] instinctively clenches shut.  \"<i>You didn't think I'd leave my most treasured possessions behind for some goblin to run into, did you?</i>\" It's the ghost girl's chest of dildos, as well-kept as the last time you laid worried eyes on it.");
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0) MainScreen.text("  Just what else did she pack in there since last you peered at its contents back at the ruins?");
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] > 0) MainScreen.text("familiar, ");
	...
	if(flags[FlagEnum.ZETSUKO_MET] > 0) MainScreen.text("\"<i>Your run-in with a particular kitsune inspired me,</i>\" Shouldra teases");
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] > 0) MainScreen.text("You recognize it instantly as she pulls it free from her chest: the nearly foot-long floppy horse cock she impaled you on before.");
	//{if SHOULDRA_GENDERLESS_FUCK_COUNT = 0}
	else if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0) MainScreen.text("She holds her catch up clear for you to see; to your horror, you have clutched onto an enormous horse cock replica.  All 13 inches flop restlessly in your grip, seemingly taunting you with the foreboding future that lays before you.");
	...
	flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT]++;
...
private nongenderlessAnalShouldraMasturbation():void {
	...
	//(same paragraph follows)
	//if event occurrence = 0
	if(flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0) {
	...
	//if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 OR event occurrence > 0
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || flags[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] > 0) MainScreen.text("You recognize it instantly as she pulls it free from her chest: the nearly 2-foot floppy horse cock she impaled you on before.");
	//if SHOULDRA_GENDERLESS_FUCK_COUNT = 0
...
private shouldraTongueLicksPCs():void {
	...
	//if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 || event occurrence > 0
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || flags[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) MainScreen.text("  The consistency of your forgetfulness while traveling this path leads you to believe that your ethereal inhabitant would rather you not come back here on your own.");
	...
	if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0 && flags[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) MainScreen.text("  Within lies a rather rustic yet well-built chest.");
	else if(flags[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 && flags[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) MainScreen.text("  \"<i>You didn't think I'd leave my most treasured possessions behind for some goblin to run into, did you?</i>\"  It's the ghost girl's chest of dildos, as well-kept as the last time you laid worried eyes on it.");
	else MainScreen.text("  Inside rests the ghost's familiar treasure trove of dildos and other toys you have yet to discover.");
	MainScreen.text("  She has you pull the container out from its hiding place, removing the clear tarp she used to protect it from the elements.  Despite the fact that you have yet to control your body once through this entire ordeal, an intrinsic urge to open the container has you bending over to pry into the contents within.");
	...
	//if event occurrence == 0
	if(flags[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) {
		...
		if(player.tongueType > TONUGE.HUMAN) MainScreen.text("some of ");
	...
	//if demon tongue
		...
		if(flags[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] == 0) MainScreen.text("  Almost shocked enough to wrest control away from your companion, you begin to babble dissent and try to reel your lengthening tongue back in to no avail.  Shouldra stops for a moment, quite frustrated by the interruption.  Sitting straight up, she consoles you, \"<i>Relax, Champ, it'll stop feeling weird in a minute and you know it's all temporary.</i>\"\n\nGranted a moment to adjust, you let yourself settle down and hand her the reins once more.  She happily goes back to ");
		else MainScreen.text("\n\n She happily continues ");
	...
	shouldraSleeping((10 + rand(26)), true);
	flags[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES]++;
	doNext(camp.returnToCampUseOneHour);
}
...
public shouldraDream1():void {
	...
	if(flags[FlagEnum.SHOULDRA_PLOT_LEVEL] == 0) {
	...
	else if(flags[FlagEnum.SHOULDRA_PLOT_LEVEL] == 1) {
	...
	else if(flags[FlagEnum.SHOULDRA_PLOT_LEVEL] == 2) {
	...
	if(flags[FlagEnum.SHOULDRA_PLOT_LEVEL] == 3) {
		//Dream 4 - Backstory
	...
	flags[FlagEnum.SHOULDRA_PLOT_LEVEL]++;
	flags[FlagEnum.SHOULDRA_TALK_NEEDED] = 1;
	doNext(playerMenu);
}
...
private shouldraYappin():void {
	MainScreen.clearText();
	//Plot dreams!
	if(flags[FlagEnum.SHOULDRA_TALK_NEEDED] == 1) {
		//unlocked with Dream 1
		if(flags[FlagEnum.SHOULDRA_PLOT_LEVEL] == 1) {
			//Clear out that talk happened and start a countdown till next dream sequence.
			flags[FlagEnum.SHOULDRA_TALK_NEEDED] = 0;
			spriteSelect(67);
			if(flags[FlagEnum.SHOULDRA_PLOT_LEVEL] < 4) flags[FlagEnum.SHOULDRA_PLOT_COUNTDOWN] = 7;
		...
		else if(flags[FlagEnum.SHOULDRA_PLOT_LEVEL] == 3) {
			//Clear out that talk happened and start a countdown till next dream sequence.
			flags[FlagEnum.SHOULDRA_TALK_NEEDED] = 0;
			spriteSelect(67);
			if(flags[FlagEnum.SHOULDRA_PLOT_LEVEL] < 4) flags[FlagEnum.SHOULDRA_PLOT_COUNTDOWN] = 7;
...
private shouldraSexMenu():void {
	...
	if(flags[FlagEnum.FREED_VALA] != 0 && (model.time.hours >= 12 && model.time.hours <= 21)) {
...
private groBallsBiggaGHOSTYSTYLE():void {
	flags[FlagEnum.SHOULDRA_MAGIC_COOLDOWN] = 72;
	MainScreen.clearText();
	spriteSelect(67);
...
private shouldraCockBloating101():void {
	...
	if(player.lowerBody.cockSpot.count() == 1) {
		...
		player.lowerBody.cockSpot.list[0].cockThickness += .25;
	}
	//MULTI
	else {
		...
		for(let i:number =0;i<player.lowerBody.cockSpot.count();i++) {
			...
			player.lowerBody.cockSpot.list[i].cockLength += 1;
			player.lowerBody.cockSpot.list[i].cockThickness += .25;
		}
	}
...
private shouldraReductoMenu():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.vaginaSpot.list[0].clitLength > .25) clit = clittyVanishingActShouldra;
...
private clittyVanishingActShouldra():void {
	...
	player.lowerBody.vaginaSpot.list[0].clitLength = int(player.lowerBody.vaginaSpot.list[0].clitLength * 100)/100;
...
private shouldraMakesCocksDisappear():void {
	...
	//MULTI
		...
		for(let ii:number =0;ii<player.lowerBody.cockSpot.count();ii++) {
			player.lowerBody.cockSpot.list[ii].cockLength  *= 2/3;
			player.lowerBody.cockSpot.list[ii].cockThickness  *= 2/3;
		}
	}
	dynStats("sen", -2, "lus", -10);
	doNext(camp.returnToCampUseOneHour);
...
private kickFollowerShouldraOut():void {
	...
	if(flags[FlagEnum.SHOULDRA_PLOT_LEVEL] < 4 || flags[FlagEnum.SHOULDRA_TALK_NEEDED] > 0) {
		...
		flags[FlagEnum.SHOULDRA_FOLLOWER_STATE] = 0;
		doNext(camp.returnToCampUseOneHour);
	}
...
public shouldersWarnings():boolean {
	...
	if(flags[FlagEnum.SHOULDRA_SLEEP_TIMER] == -96) {
		spriteSelect(67);
	...
	else if(flags[FlagEnum.SHOULDRA_SLEEP_TIMER] == -168) {
		spriteSelect(67);
	...
	if(flags[FlagEnum.SHOULDRA_SLEEP_TIMER] == -216) {
		spriteSelect(67);
...
public nightTimeShouldraRapesThePC():void {
	...
	flags[FlagEnum.SHOULDRA_SLEEP_TIMER] = 0;
	spriteSelect(67);
	player.orgasm();
	...
	//event occurance = 0
	if(flags[FlagEnum.SHOULDRA_TIMES_NIGHT_RAPED_PC] == 0) {
...
private edwardShouldraDickHandsPartII():void {
	...
	flags[FlagEnum.SHOULDRA_TIMES_NIGHT_RAPED_PC]++;
	doNext(playerMenu);
}
...
public shouldraWakesUpOrPokesPCsForShitsAndGigglesIdunnoHowLongCanIMakeThisFunctionNameQuestionMark():void {
	...
	if(flags[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] + flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] > 0) choices[choices.length] = 17;
	...
	switch(select) {
		...
		case 17:
			MainScreen.text("\"<i>");
			if(flags[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] + flags[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] == 1) MainScreen.text("That shark daughter of yours is");
...
private shouldraAndWormsYoureGonnaHaveABadTime():void {
	...
	if(player.lowerBody.cockSpot.list[0].cockLength < 48) MainScreen.text(" and cuts right to the chase, ballooning your [cock] into a rigid, wide, four-foot tall slab of meat");
...
private kickOutShouldra4YoWormyBuddies():void {
	...
	if(player.lowerBody.cockSpot.list[0].cockLength < 48) MainScreen.text("  With that little ordeal dealt with, you now are left to decide how best to manage the time until your cock returns to its normal size.");
...
public shouldraAndExgartumonFightGottaCatchEmAll():void {
	...
	flags[FlagEnum.SHOULDRA_EXGARTUDRAMA] = 1;
	doNext(playerMenu);
}
...
public exgartumonAndShouldraFightPartII():void {
	...
	flags[FlagEnum.SHOULDRA_EXGARTUDRAMA] = 2;
}
...
public exgartumonAndShouldraFightPartIII():void {
	...
	flags[FlagEnum.SHOULDRA_EXGARTUDRAMA] = 3;
...
private keepShouldraAndKickOutExgartuan():void {
	...
	flags[FlagEnum.SHOULDRA_EXGARTUDRAMA] = -0.5;
	doNext(playerMenu);
}
...
public keepShouldraPartIIExgartumonsUndeatH():void {
	...
	flags[FlagEnum.SHOULDRA_EXGARTUDRAMA] = 0;
	//[next page]
...
private keepAllTheGhosts():void {
	...
	flags[FlagEnum.SHOULDRA_EXGARTUDRAMA] = 4;
	doNext(playerMenu);
}
...
public shouldraBakeryIntro():void {
	...
	//first time: 
	if(flags[FlagEnum.SHOULDRA_BAKERY_TIMES] == 0) MainScreen.text("You smirk, wondering how exactly a ghost manages to work up a hunger.  Your skepticism is met by an intense stabbing pain in your stomach, forcing you to buckle slightly from the cramping.  It lasts only a moment before it vanishes, however.  \"<i>You feel that, smartass?</i>\" The ghost girl sneers, \"<i>That's what decades of fasting will do to you.");
...
private feedShouldraACake():void {
	...
	//after first time: 
	if(flags[FlagEnum.SHOULDRA_BAKERY_TIMES] > 0) MainScreen.text("Well, nothing <i>permanent</i>, anyway.  ");
...
private goAlongWIthShouldrasEatingSpree():void {
	...
	spriteSelect(67);
	//if repeat scene: 
	if(flags[FlagEnum.SHOULDRA_BAKERY_TIMES] > 0) MainScreen.text("\"Man, you're not sure you want that kind of impact on your figure again...  \"");
	...
	if(flags[FlagEnum.PC_FETISH] > 0) MainScreen.text(", though not completely unwelcome");
	...
	if(flags[FlagEnum.PC_FETISH] > 0) MainScreen.text(", even with that kink of yours");
	...
	flags[FlagEnum.SHOULDRA_BAKERY_TIMES]++;
	player.slimeFeed();
}
