	...
	else
	{
		...
		if (kGAMECLASS.farm.farmCorruption.hasTattoo("jojo"))
		{
			...
			else
			{
				...
				if (flags[FlagEnum.JOJO_TATTOO_COLLARBONE] != 0) outputText(flags[FlagEnum.JOJO_TATTOO_COLLARBONE] + "\n");
				if (flags[FlagEnum.JOJO_TATTOO_SHOULDERS] != 0) outputText(flags[FlagEnum.JOJO_TATTOO_SHOULDERS] + "\n");
				if (flags[FlagEnum.JOJO_TATTOO_LOWERBACK] != 0) outputText(flags[FlagEnum.JOJO_TATTOO_LOWERBACK] + "\n");
				if (flags[FlagEnum.JOJO_TATTOO_BUTT] != 0) outputText(flags[FlagEnum.JOJO_TATTOO_BUTT] + "\n");
				outputText("\n");
			}
		}
...
private function mutateJojo():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("bodies", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("caressing your aching twat ", false);
...
public function useTentacleJojo():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("You feel it split in half, dividing in order to take you from both ends.  ", false);
	...
	if(player.totalCocks() > 0) {
		...
		if(player.lowerBody.cockSpot.count() == 1) {
			...
			//(1 dick no balls = 
				...
				if(!player.lowerBody.vaginaSpot.hasVagina()) outputText("rubbing back and forth along your taint and smearing you with their slime.", false);
				else {
					...
					//(If big clit – 
					if(player.lowerBody.vaginaSpot.list[0].clitLength >= 4) outputText("  You nearly cum on the spot from the clitoral stimulation; it's like you have a super-sensitive cock being licked by a thousand tongues.", false);
				}
				outputText("\n\n", false);
			}
		}
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("pair of tentacles ", false);
	else outputText("tentacle ", false);
	outputText("hanging over your bottom decide", false);
	if(!player.lowerBody.vaginaSpot.hasVagina()) outputText("s", false);
	...
	//not virgin taking
		...
		//With cunt
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			if(player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.GAPING_WIDE) outputText(" practically falling into your oversized cum-receptacles", false);
			else {
				if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLICK) outputText(" sliding inside easily thanks to your copious lubrication", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("and pleasure ", false);
	outputText("as Jojo's tentacle", false);
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("s begin", false);
	...
	if(player.totalCocks() > 0) {
		...
		if(player.lowerBody.cockSpot.count() == 1) outputText("  The tentacle on your " + multiCockDescriptLight() + " seems appreciative, and begins sucking to swallow down all your pre.", false);
		if(player.lowerBody.cockSpot.count() > 1) outputText("  The tentacles on your " + multiCockDescriptLight() + " seem appreciative, and begin sucking to swallow down all your pre.", false);
	}
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your " + vaginaDescript(0) + " spasms around the invader, clenching uncontrollably as it's plowed by the thick vine-prick.", false);
	outputText("\n\n", false);
	...
	if(player.biggestLactation() >= 2) {
		...
		if(player.stats.cor < 80) outputText("being such a wanton slut", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING) outputText(", and soaks him with a wave of puss juice.  ", false);
		else if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLICK) outputText(", as your drooling cunt forms a slick puddle.  ", false);
	...
	if(player.biggestLactation() >= 2 && nippleSucking) {
		...
		if(player.upperBody.chest.BreastRatingLargest[0].nippleLength >= 2) outputText("even larger", false);
	...
	if(titFucking) {
		...
		if(player.stats.cor >= 80 && player.stats.lib >= 70) outputText("You lick your lips and swallow as much of it as possible, wallowing in the mouse-cream.  ", false);
...
private function milkJojoFirst():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00333] > 0) outputText(", even though it reeks of the spunk you've spilled into the milker's mechanical embrace", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("It starts to turn your way before it stops and rises, seeming to sniff at the rodent directly above.", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(x < 0) {
			...
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00333] > 0) outputText("You know from experience that machine usually takes an hour before it will allow release", false);
	...
	//(Vagoozle!)
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) outputText("Your other hand grabs your penis-sized 'button' and begins to jack it, the action sending tremors of sensation through your " + hipDescript() + ".  ", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", squeezing your " + cockDescript(x) + " with agonizingly pleasurable pressure", false);
	...
	if(!player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) outputText("  His balls tremble and shrink in your mouth, pumping his prodigious semen out, wasting it in exchange for a pittance of gems.", false);
	outputText("\n\n", false);
	...
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("You climax and clamp down on your fingers, squeezing your hand as your own cries of pleasure mix with your cock-toy's milked-out moans.", false);
...
private function repeatMilkJojo(tentacle:boolean = false):void {
	...
	if(!tentacle || !tentacleJojo()) {
		...
		if(!player.lowerBody.cockSpot.hasCock() && !player.lowerBody.vaginaSpot.hasVagina()) outputText(", even though to do so you're forced to stimulate your " + assholeDescript(), false);
	...
	//TENTACLE JOJO MILKING (edited)
		...
		if(flags[FlagEnum.PC_FETISH] > 0) outputText("An exhibitionist thrill shivers through you at the thought of being watched while engaged in such a carnal act.  ", false);
...
private function amilyTeachingJojoBJ():void {
	...
	if (player.lowerBody.cockSpot.hasCock()) outputText(" Do you surprise them both by allowing Amily to demonstrate on you?");
...
private function fillAmilysMouth():void {
	...
	if (player.lowerBody.balls > 0) outputText(" leans down to desperately suckle at your [sack], subserviently worshipping your bloated testes and their liquid bliss one after another; tonguing, sucking, and moaning into the spunk - bloated mass of your ambrosial scrotum.");
...
private function amilyAndJojoFuck():void {
	...
	if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" It isn't until you notice how wet your crotch is that you come to the realization that you've masturbated yourself through a few orgasms as well.");
...
//this.semiglobalReferencer.jojoRape = jojoRape;
		...
		private function jojosSecondRape():void {
			...
			if(player.gender == 1) {
				...
				if(player.lowerBody.cockSpot.count() > 1) outputText("  Your remaining equipment showers him with jizz, more than you ever thought you could produce.");
			...
			if(player.gender == 2 || player.gender == 3) {
				...
				if(player.lowerBody.cockSpot.count() == 1) outputText("Jizz sprays onto his chest from your " + cockDescript(0) + ".  ");
				if(player.lowerBody.cockSpot.count() == 2) outputText("A hail of jizz splatters over Jojo from your " + multiCockDescriptLight() + ".  ");
				if(player.lowerBody.cockSpot.count() == 3) outputText("A multitude of thick cum-streams splatter over Jojo from head to waist as your " + multiCockDescriptLight() + " hose him down.  ");
		...
		private function jojosThirdRape():void {
			...
			if(player.gender == 1) {
				...
				if(player.lowerBody.cockSpot.count() == 1) outputText("to ram your " + cockDescript(0) + " deep into his maw.  You roughly grab his ears, facefucking him hard, his tongue working hard to please.  ");
				if(player.lowerBody.cockSpot.count() > 1) outputText("to cram two cocks deep into his maw, making his jaw stretch obscenely.  You roughly grab his ears, facefucking him hard, his tongue working hard to please you.  ");
				...
				if(player.lowerBody.cockSpot.count() == 1) outputText("Your " + cockDescript(0) + " clenches tightly, erupting ");
				if(player.lowerBody.cockSpot.count() > 1) outputText("Your twin dicks clench tightly, erupting ");
			...
			if (player.gender == 2) {
				...
				if(player.biggestLactation() >= 1.5 && player.upperBody.chest.BreastRatingLargest[0].breastRating > 2 && player.mostBreastsPerRow() >= 2 && player.upperBody.chest.count() >= 1) outputText("Every sensual peak within you is mirrored with small spurts of milk from your nipples.  It eventually trickles down to Jojo's tongue, spurring his efforts on.  ");
				...
				if(player.lowerBody == LOWER_BODY.NAGA) outputText("your coils ");
				else if(player.lowerBody == LOWER_BODY.GOO) outputText("your jiggling goo ");
			...
			if(player.gender == 3) {
				...
				if(player.lowerBody.cockSpot.count() == 1) outputText("You sigh as your " + cockDescript(0) + " droops over his head.  ");
				if(player.lowerBody.cockSpot.count() > 1) outputText("You sigh as your " + multiCockDescriptLight() + " pile atop his head.  ");
				if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) {
				...
				if(player.lowerBody.cockSpot.count() > 0) outputText("  Thick runners of your pre dribble down his neck, sticking to his fur.  ");
				outputText("\n\nYou pull the cunt-dazed mouse away reluctantly, and bend over,");
				if (player.isBiped()) outputText(" spreading your legs and");
				outputText(" shaking your rump.  ");
				if(player.hasLongTail() > 0) outputText("Your tail swishes back and forth teasingly.  ");
				outputText("Your tantalizing backside is too much for him.  He pounces your ass in a second, penetrating your " + vaginaDescript(0) + " forcefully.  The knowledge that your demonic influence is responsible for his state nearly makes you cream all over his cock.  You fantasize about your cunt making his cock grow longer and fatter as it corrupts him further, transforming his morals and values into cock and cum.  Jojo thrashes against you, brutally pounding your " + vaginaDescript(0) + " as he orgasms.  His " + monster.cockDescriptShort(0) + " erupts inside you, the feeling of being filled with mouse-spunk drives you over the edge with him.  More and more of his cum sprays inside you, unnatural amounts for a creature his size.  Thick streams of the stuff escape down your thighs as your belly distends.  ");
				player.cuntChange(2, true);
				if(player.lowerBody.cockSpot.count() == 1) outputText("Your " + cockDescript(0) + " trembles in orgasm, squirting your load into the thick forest loam.  ");
				if(player.lowerBody.cockSpot.count() > 1) outputText("Your " + multiCockDescriptLight() + " tremble in orgasm, squirting their hot loads all over the thick forest loam.  ");
				outputText("\n\n");
				outputText("You let the drained mouse pop free, reveling in the sensation of cum pouring from ");
				if (player.isBiped()) outputText("between your thighs");
				else outputText("your cunt");
		...
		private function jojosFourthRape():void {
			...
			if(player.gender == 2 || player.gender == 3) {
				...
				if(player.lowerBody.cockSpot.count() == 1) outputText("Your " + cockDescript(0) + " drizzles cum in his hair, some dripping off by his ear.  ");
				if(player.lowerBody.cockSpot.count() > 1) outputText("Your " + cockDescript(0) + "s drizzle him with cum, covering his head with thick white streams of your jizz.  ");
				...
				if(player.lowerBody.cockSpot.count() > 0) {
					...
					if(player.lowerBody.cockSpot.count() == 1) {
						...
						if(player.cumQ() >= 25 && player.cumQ() < 250) {
							...
							if(player.lowerBody.cockSpot.list[0].cockType == CockType.HUMAN || player.lowerBody.cockSpot.list[0].cockType.Index > 2) outputText("Your body tenses and cums a thick eruption far beyond what a normal human could produce.  Jojo is splattered with the stuff.  ");
						}
						if(player.cumQ() >= 250) {
				...
				if(player.biggestLactation() >= 1 && player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) {
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating * player.biggestLactation() * player.upperBody.chest.count() < 8) outputText("Milk sprays from your " + breastDescript(0) + " in tiny streams, triggered by your orgasms.");
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating * player.biggestLactation() * player.upperBody.chest.count() < 20 &&
					player.upperBody.chest.BreastRatingLargest[0].breastRating * player.biggestLactation() * player.upperBody.chest.count() >= 8) outputText("Milk erupts from your " + breastDescript(0) + ", spraying out over the mouse, squirting out the contractions of each shuddering orgasm.");
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating * player.biggestLactation() * player.upperBody.chest.count() < 35 &&
					player.upperBody.chest.BreastRatingLargest[0].breastRating * player.biggestLactation() * player.upperBody.chest.count() >= 20) outputText("Milk erupts from your " + breastDescript(0) + ", spraying in pulsing blasts, soaking the mouse.  Each nerve-wracking orgasm seems to wring more and more milk from you, until it puddles around.");
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating * player.biggestLactation() * player.upperBody.chest.count() >= 35) outputText("Milk fountains from your " + breastDescript(0) + ", soaking the mouse with a continuous river of cream.  For every blob of cum you feel pushing into your over-filled uterus, another torrent of milk sprays out.  As your mind-bending orgasms drag on, a small lake of milk forms around you.");
				}
			...
			if (monk == 2 || monk == 3) {
				...
				if (player.lust >= 100) {
					if (player.gender == 1) {
						...
						if (player.lowerBody.cockSpot.count() == 1) outputText("Pre and cum drip from your " + player.cockDescript(0) + ", forced out of your prostate by the rough beating it's taking.  You feel a flash of warm wetness inside you, and realize Jojo is cumming.  A sense of relief washes over you as the last burst of cum squirts out from your cheeks, only to be replaced with a dawning sense of horror as he continues fucking you harder than ever.\n\nYou black out after a few dozen of his orgasms and one or two of your own, your gut painfully distended with semen.");
						if (player.lowerBody.cockSpot.count() > 1) outputText("Pre and cum drip from your " + player.cockDescript(0) + "s, forced out of your prostate by the rough beating it's taking.  You feel a flash of warm wetness inside you, and realize Jojo is cumming.  A sense of relief washes over you as the last burst of cum squirts out from your cheeks, only to be replaced with a dawning sense of horror as he continues fucking you harder than ever.\n\nYou black out after a few dozen of his orgasms and one or two of your own, your gut painfully distended with semen.");
					...
					if (player.gender >= 2) {
						...
						if (player.lowerBody.cockSpot.count() > 1) outputText("Your " + player.cockDescript(0) + " splatters the ground with cum repeatedly, until both your genders are raw and sore.  ");
		...
		private function corruptJojoBJCruel():void {
			...
			outputText("His eyes turn to you in fear and his body shudders for lack of breath, but it does nothing more than stoke the fires of your lust. You groan in pleasure as your balls draw up tight, churning with your corrupted seed, and in a rush you feed it to him, your orgasm overtaking you as surge after hot surge of cum flares through your flesh and into his throat.  " + (player.lowerBody.vaginaSpot.hasVagina() ? "A sympathetic orgasm hits your pussy, causing a surge of feminine juices to splash against his chest and dribble down your thighs lewdly.  " : ""));
		...
		private function corruptJojoBJGentle():void {
			...
			outputText("His eyes turn to you in fear and awe, pleading for release, and a slip of your foot to his own straining erection lets you know how in need of an orgasm he is, but this time is yours. You groan in pleasure as your balls draw up tight, churning with your corrupted seed, and in a rush you feed it to him, your orgasm overtaking you as surge after hot surge of cum flares through your flesh and into his throat.  " + (player.lowerBody.vaginaSpot.hasVagina() ? "A sympathetic orgasm hits your pussy, causing a surge of feminine juices to splash against his chest and dribble down your thighs lewdly.  " : ""));
			outputText("Your orgasm seems to last forever, filling his belly with your corrupted essence, causing his stomach to bulge slightly with the sheer volume of it. You pull away at last, letting him gasp for breath and fall to the ground, curling around his bloated belly.  ");
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2)
		...
		private function corruptJojoBreasts():void { //Should only be available to players with biggestBreastSize > 2
			...
			if (player.biggestLactation() > 1) { //For suckling the scene is the same regardless of player's gender
				...
				if (player.upperBody.chest.count() > 1) outputText("and then all the others, "); //Extra boob coverage
			...
			if (player.gender == 2 && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) {
				...
				if (selection == 0) {
					...
					if (player.biggestLactation() > 1) {
						outputText("He leans in and starts to kiss along your nipples before taking one into his mouth. He gives a firm suckle at the engorged teat, and you can see his eyes open wider in surprise at the sudden surge of milk that fills his muzzle. He shivers and starts to suckle in earnest, drinking from first one breast, then the other, ");
						//Extra boob coverage
						if (player.upperBody.chest.count() > 1) outputText("and then all the others, ");
						outputText("partaking of your blessing until his belly is full.  ");
					}
					else if (player.upperBody.chest.BreastRatingLargest[0].breastRating <= 5)
...
private function beeEggsInCorruptJojo():void {
	...
	if(!player.lowerBody.cockSpot.hasCock()) {
		if(flags[FlagEnum.TIMES_EGGED_JOJO] == 0) outputText("Jojo looks confused at this; he knows you don't have a cock!  ");
	...
	if(player.gender > 0) {
		...
		if(player.gender == 3 || player.lowerBody.cockSpot.count() > 1) outputText("are");
		else outputText("is");
...
public function lowCorruptionIntro():void
{
	...
	if (player.stats.cor <= 5) outputText("don't ");
...
public function jojoTalkSandCave():void
{
	...
	if (flags[FlagEnum.MET_MILK_SLAVE] == 1)
	{
		...
		{
			outputText("As the question leaves his lips you give Jojo a confused look and, with a glance, direct his gaze toward " + flags[FlagEnum.MILK_NAME] + ".\n\n");
...
public function apparantlyJojoDOESlift():void
{
	...
	if (!player.perks.has("ControlledBreath") && !player.perks.has("CleansingPalm") && !player.perks.has("Enlightened"))
	{
		...
		if (flags[FlagEnum.TIMES_TRAINED_WITH_JOJO] == 5)
		{
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating == 0) outputText(" rises and falls");
