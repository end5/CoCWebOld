export default class SheilaScene extends NPCAwareContent implements TimeAwareInterface {
	...
	public SheilaScene() {
		pregnancy = new PregnancyType(FlagEnum.SHEILA_PREGNANCY_TYPE, FlagEnum.SHEILA_PREGNANCY_INCUBATION, 0, 0);
	...
	public timeChange(): boolean {
		...
		if (model.time.hours > 23) {
			if (Flags.list[FlagEnum.SHEILA_CLOCK] < 0) Flags.list[FlagEnum.SHEILA_CLOCK]++;
		}
	...
	private sheilaPreg(reducedChance: boolean = false): void {
		...
		if (rand(100) <= chance)
			pregnancy.knockUpForce(PregnancyType.PLAYER, PregnancyType.INCUBATION_SHIELA);
	}

	public sheilaCorruption(arg: number = 0): number {
	/*if(Flags.list[FlagEnum.SHEILA_CORRUPTION] + arg > 100) Flags.list[FlagEnum.SHEILA_CORRUPTION] = 100;
	else*/ if (Flags.list[FlagEnum.SHEILA_CORRUPTION] + arg < 0) Flags.list[FlagEnum.SHEILA_CORRUPTION] = 0;
		else Flags.list[FlagEnum.SHEILA_CORRUPTION] += arg;
		return Flags.list[FlagEnum.SHEILA_CORRUPTION];
	}

	//Sheila's cup function!
	internal function sheilaCup(): string {
		if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 10) return "tiny, almost-absent";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 20) return "A-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 30) return "B-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 40) return "C-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 50) return "D-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 60) return "DD-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 70) return "E-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 80) return "EE-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 90) return "front-running";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 100) return "FF-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 110) return "G-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 120) return "GG-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 130) return "H-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 140) return "HH-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 150) return "'Holy Honkin' Hooters!'-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 160) return "I 'want to squeeze them'-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 170) return "II-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 180) return "Jiggle-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 190) return "JJ-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 200) return "K-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 210) return "KK-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 220) return "L-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 230) return "'Ladylove'-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 240) return "M-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 250) return "'Mega Milk'-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 260) return "N-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 270) return "NN-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 280) return "O-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 290) return "'so big it makes you say' OO-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 300) return "P-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 310 && player.lowerBody.cockSpot.count() > 0) return "'would feel great around your' PP-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 310 && player.lowerBody.cockSpot.count() == 0) return "PP-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 320) return "Q-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 330) return "QQ-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 340) return "R-u-serious?-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 350) return "RR-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 360) return "S-rank";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 370) return "'Shooting Star'-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 380) return "T-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 390) return "'damn, those are some big' TT-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 400) return "'bigger than' U-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 410) return "UU-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 420) return "V-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 430) return "'Va-Va-Voom'-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 440) return "'War Head'-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 450) return "World-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 460) return "X-Buster-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 470) return "XXXX-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 480) return "'Yay'-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 490) return "YY-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 500) return "Z-Saber-cup";
		else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 510) return "ZZ-cup";
		else return "fuckin' magic";
	}

	public sheilaEncounterRouter(): void {
		trace("Sheila Encounter:");
		trace("Sheila Corruption: " + Math.round(sheilaCorruption() * 10) / 10 + " Sheila Preg: " + Flags.list[FlagEnum.SHEILA_PREGNANCY_INCUBATION] + " Sheila XP: " + Flags.list[FlagEnum.SHEILA_XP]);
		//UNFUCK FLAGS
		if (Flags.list[FlagEnum.SHEILA_CLOCK] > 0)
			Flags.list[FlagEnum.SHEILA_CLOCK] = 0
		if (sheilaCorruption() >= 100 && Flags.list[FlagEnum.SHEILA_DEMON] == 0)
			Flags.list[FlagEnum.SHEILA_DEMON] = 1;
		if (Flags.list[FlagEnum.SHEILA_DEMON] == 1)
			Flags.list[FlagEnum.SHEILA_XP] = 4;

		//ACTUALLY PICK ENCOUNTER!
		if (Flags.list[FlagEnum.SHEILA_DEMON] == 0) {
			//PREG SHIT
			if (pregnancy.isPregnant && pregnancy.incubation == 0 && Flags.list[FlagEnum.SHEILA_XP] >= -2) {
				if (Flags.list[FlagEnum.SHEILA_JOEYS] == 0) normalSheilaPregNotifNumberOne();
				else normalSheilaPregNotifREPEATEDED();
				return;
			}
			if (Flags.list[FlagEnum.SHEILA_XP] == 0) sheilaFirstEncounter();
			else if (Flags.list[FlagEnum.SHEILA_XP] == -1) sheilaApology();
			//XP-2 Sheila's apology (sheila xp = -2 and demon sheila = 0):
			else if (Flags.list[FlagEnum.SHEILA_XP] == -2) sheilaIsSorrySheRapedYou();
			//XP-3 or XP-4 She-mad (Sheila XP <= -3 and demon sheila = 0):
			else if (Flags.list[FlagEnum.SHEILA_XP] == -3 || Flags.list[FlagEnum.SHEILA_XP] == -4) sheilaReallyMad();
			//XP1: Reconciliation encounter (sheila xp = 1 and demon sheila = 0):
			else if (Flags.list[FlagEnum.SHEILA_XP] == 1) sheilaReconcile();
			//XP2: Familiarizing (Sheila XP = 2; or Sheila XP = 3 AND time =/= 20:00 and demon sheila = 0):
			else if (Flags.list[FlagEnum.SHEILA_XP] == 2 || (Flags.list[FlagEnum.SHEILA_XP] == 3 && model.time.hours != 20)) sheilaGettingFamiliar();
			//XP3: Sexy Time (sheila xp = 3 AND time = 20:00 and demon sheila = 0):
			else if (Flags.list[FlagEnum.SHEILA_XP] == 3 && model.time.hours == 20) sheilaXPThreeSexyTime();
			//XP4: Fancy meeting you here (sheila xp >= 4 and demon sheila = 0):
			else if (Flags.list[FlagEnum.SHEILA_XP] == 4) fuckBuddySheilaMeeting();
			else {
				MainScreen.text("A BUG HAPPENED.  YOUR SHEILA_XP: " + Flags.list[FlagEnum.SHEILA_XP] + ". <b>Value should be between -3 to 4.</b>");
	...
	private sheilaCorruptionUpdate(): void {
		...
		if (player.stats.cor > 90) {
			...
			//Big change, dump ten.
			if (player.stats.cor - sheilaCorruption() >= 10) {
				player.stats.cor += -10;
				sheilaCorruption(10);
			}
			else {
				...
				while (Math.round(player.stats.cor) != Math.round(sheilaCorruption())) {
					player.stats.cor -= 1;
					if (Math.round(player.stats.cor) <= Math.round(sheilaCorruption())) {
						break;
					}
					else sheilaCorruption(1);
				}
				player.stats.cor = Math.round(player.stats.cor);
			...
			//Big Change!
			if (sheilaCorruption() - player.stats.cor >= 10) {
				player.stats.cor += 10;
				sheilaCorruption(-10);
			}
			else {
		...
		if (Flags.list[FlagEnum.SHEILA_DEMON] == 1) player.stats.cor += 1;
	}
	...
	private sheila1stEncStayHidden(): void {
		MainScreen.clearText();
		MainScreen.text("You regulate your breathing and hold yourself still to avoid rustling anything softly, waiting for the creature to finish its business and leave.");
	...
	private sheila1ndEncLookCloserPtII(): void {
		...
		//[(libido>=50)
		if (player.stats.lib >= 50) MainScreen.text("  Noting you still checking out her body, she smirks and says, \"<i>Enjoying your lookie, then?  Up here, cobber,</i>\" causing you to snap your head up sheepishly.");
		MainScreen.text("  You nod at the strange woman and introduce yourself.");
		if (player.short != "Sheila") MainScreen.text("  \"<i>" + player.short + ", eh?  My mates just call me Sheila.  Bunch of jokers, the bastards.</i>\"");
		else MainScreen.text("  \"<i>You too, eh?  Sheila's what my mates call me for a nickname.  Don't suppose you got yours from a bunch of adolescent-minded wankers, though.</i>\"");
		...
		//[(high demon score)
		if (player.demonScore() >= 4) MainScreen.text("  I mean, take a squizz at yourself.  It's like you're not even trying to hide it; I've seen imps that look more like normal people.");
		MainScreen.text("  Forget it.</i>\"  You deny the accusation, but she continues to glare at you.");
		...
		//[Talk][Leave]
		menu();
		MainScreen.addButton(0, "Talk", sheila1ndEncLookCloserPtIITalk);
		MainScreen.addButton(1, "Leave", sheila1ndEncLookCloserPtIILeave);
	...
	private sheila1ndEncLookCloserPtIILeave(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>Good onya.  Maybe I'll see you around.</i>\"  With that, you take your leave of the strange woman.");
		//set sheila xp = 1
		Flags.list[FlagEnum.SHEILA_XP] = 1;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private sheila1ndEncLookCloserPtIITalk(): void {
		...
		//[Leave][Fight]
		menu();
		MainScreen.addButton(0, "Fight", sheila1ndEncLookCloserPtIITalkFight);
		MainScreen.addButton(4, "Leave", sheila1ndEncLookCloserPtIITalkLeave);
	}

	//[XP0 - Look Closer - Talk - Leave]
	private sheila1ndEncLookCloserPtIITalkLeave(): void {
		MainScreen.clearText();
		MainScreen.text("Wordlessly, you get up and back away from the woman... though not without a dirty glance.  She follows you with her eyes until you judge yourself far enough away to turn your back to her.");
		//go to camp
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private sheilaApology(): void {
		MainScreen.clearText();
		MainScreen.text("As you search the plains for god-knows-what, you catch your [leg] on something; a rustling in the brush below you is the only warning you get before a cleverly hidden snare closes on it!");
		...
		menu();
		MainScreen.addButton(0, "Apologize", apologySheilaApology);
		MainScreen.addButton(1, "SlapNInsult", apologySheilaSlapSult);
		MainScreen.addButton(2, "Nothing", apologySheilaSayNothing);
		MainScreen.addButton(3, "Avoid Her", apologyAvoidSheila);
	}
	...
	private apologyAvoidSheila(): void {
		MainScreen.clearText();
		MainScreen.text("You shake your head at the leery girl, and tell her you'd be just as happy as she would if you never saw each other again.");
		...
		//set sheilapreg = -1
		Flags.list[FlagEnum.SHEILA_DISABLED] = 1;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private apologySheilaApology(): void {
		MainScreen.clearText();
		MainScreen.text("You scratch your " + player.skinFurScales() + " absently behind an ear, not sure how to broach the topic of reconciliation when you feel like she was the aggressor, but mumble something about being sorry things started off on the wrong foot.");
	...
	private apologySheilaSlapSult(): void {
		MainScreen.clearText();
		MainScreen.text("You say nothing as she finishes with the snare and stands back up.");
		...
		Flags.list[FlagEnum.SHEILA_XP] = -3;
		startCombat(new Sheila());
	}
	...
	private apologySheilaSayNothing(): void {
		MainScreen.clearText();
		MainScreen.text("You decide to keep your own counsel for now, folding your arms defensively and frowning.");
	...
	private sheilaIsSorrySheRapedYou(): void {
		...
		MainScreen.addButton(0, "Forgive", sheilaIsSorryAndYouForgive);
		MainScreen.addButton(1, "Fight", sheilaPologyFight);
		if (player.statusAffects.has("KnowsArouse")) MainScreen.addButton(2, "CastArouse", sheilaPologyCastArouse);
		MainScreen.addButton(3, "LemmeAlone", sheilaIsSorryButLeaveMeAlone);
	...
	private sheilaIsSorryButLeaveMeAlone(): void {
		MainScreen.clearText();
		MainScreen.text("You shake your head and turn away, dismissing Sheila with a pointed gesture.  \"<i>H-hey!</i>\" the girl calls angrily from behind you.  \"<i>Dammit, I said I was sorry, you bastard!  Screw you!  See if I talk to you again!</i>\"");
		//set sheilapreg = -1
		Flags.list[FlagEnum.SHEILA_DISABLED] = 1;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private sheilaIsSorryAndYouForgive(): void {
		...
		if (Flags.list[FlagEnum.SHEILA_JOEYS] > 0) Flags.list[FlagEnum.SHEILA_XP] = 4;
		else Flags.list[FlagEnum.SHEILA_XP] = 1;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private sheilaPologyFight(): void {
		MainScreen.clearText();
		MainScreen.text("Saying nothing, you raise your [weapon] and take a swing!  Sheila starts a bit and dodges just in time, then with a look of resignation, raises her fists as well.  \"<i>That'll be right, I guess.  For what it's worth, I really am sorry.</i>\"");
	...
	private sheilaPologyCastArouse(): void {
		...
		MainScreen.text("\n\nBetter and better.");
		dynStats("lus", (player.stats.lib / 5), "resisted", false);
		//[Let Her][Leave]
		menu();
		MainScreen.addButton(0, "Let Her", castArouseAndLetHer);
		MainScreen.addButton(4, "Leave", castArouseAndLeaveSheila);
	}

	//[XP-2 - Cast Arouse - Leave]
	private castArouseAndLeaveSheila(): void {
		MainScreen.clearText();
		MainScreen.text("You shake your head sadly.  \"<i>I'm sorry, I haven't the time right now.</i>\"");
		...
		Flags.list[FlagEnum.SHEILA_CORRUPTION] = 100;
		Flags.list[FlagEnum.SHEILA_DEMON] = 1;
		Flags.list[FlagEnum.SHEILA_CITE] = -1;
		player.stats.cor += 10;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private castArouseAndLetHer(): void {
		MainScreen.clearText();
		MainScreen.text("As soon as you nod, the girl bounds to her feet and jumps you, driving you slightly downward in her ardor.");
		//increase PC corruption +10, output loss scene consonant with player's sex, set sheila xp = 4
		player.stats.cor += 10;
		Flags.list[FlagEnum.SHEILA_XP] = 4;
		//Call appropriate loss router
		menu();
		MainScreen.addButton(0, "Next", sheilaLossRapeRouter);
	}

	//XP-3 or XP-4 She-mad (Sheila XP <= -3 and demon sheila = 0):
	private sheilaReallyMad(): void {
		MainScreen.clearText();
		MainScreen.text("A rustle in the bush claims your attention, and you turn your head to see Sheila, standing up suddenly.  Her eyes narrow when she recognizes you, and she takes off running!");
		...
		if (Flags.list[FlagEnum.SHEILA_CORRUPTION] >= 90) MainScreen.text("It's so hard to remember myself anymore, and the taint you've put in my body goes crazy when you touch me.  Do you want me to get on my hands and knees?</i>\"  The woman lowers herself to the ground.  \"<i>I beg you.  ");
		MainScreen.text("Please, please leave me alone.");
		if (Flags.list[FlagEnum.SHEILA_XP] == -3) MainScreen.text("  I know you... you're not as bad as the rest of the monsters out here, so... please.");
		MainScreen.text("</i>\"");
		//[Stand][Leave]
		menu();
		MainScreen.addButton(4, "Leave", sheilaReallyMadLeave);
		MainScreen.addButton(0, "Stand", sheilaReallyMadStandGround);
	...
	private sheilaReallyMadLeave(): void {
		MainScreen.clearText();
		MainScreen.text("Huffing, you throw the animals down and turn away.  What a display.");
	...
	private sheilaReallyMadStandGround(): void {
		...
		//[(normal mode)]
		if (!silly()) {
			MainScreen.text("You throw the animals behind you and fold your arms.");
			MainScreen.text("\n\nSheila shivers, then speaks.  \"<i>Alright, mate.  If that's the way it has to be.</i>\"  She raises her fists half-heartedly.");
		}
		//(silly mode)
		else {
			MainScreen.text("Throwing the bundle to the side dramatically, you call the name of your stand!  Sheila's eyes widen as your body surges with renewed energy, and she jumps back as a ghostly other self materializes next to you.");
	...
	private sheilaReconcile(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>Oy, it's you,</i>\" sings out a voice behind you.  \"<i>Back again?</i>\"");
		...
		menu();
		MainScreen.addButton(0, "Kay", sheilaReconcileKay);
		MainScreen.addButton(1, "Don't Wanna", sheilaReconcileDunWanna);
	}

	//[XP1 - Don't Wanna]
	private sheilaReconcileDunWanna(): void {
		MainScreen.clearText();
		MainScreen.text("Raising your hands in a palms-out gesture, you decline.  Sheila colors a bit, misinterpreting your reluctance.  \"<i>Hey, it's not like I'm cracking onto you or anything.  I was just looking for a reason to eat half a rabbit myself, is all.</i>\"");
	...
	private sheilaReconcileKay(): void {
		MainScreen.clearText();
		MainScreen.text("Some fresh meat does sound pretty tempting.  You nod at the woman and she climbs to her feet.  \"<i>Beaut!  Do me the favor and fossick about for some tinder.</i>\"");
		//--Next--
		menu();
		MainScreen.addButton(0, "Next", sheilaReconcileKay2);
	}

	private sheilaReconcileKay2(): void {
		MainScreen.clearText();
		MainScreen.text("Soon enough you've got a firepit going in a small clearing.  \"<i>Ahh, classic bush theater,</i>\" Sheila says, admiring it.  The woman dextrously spits two liberated and skinned rabbit quarters on a long stick and twirls them over the lapping flame, and soon the air is full of the sizzle and pop of meat juices dripping on the hot stones and the simple tune your cook hums.  You stare at ");
		if (player.stats.lib < 50) MainScreen.text("the beckoning hunks of meat");
		else MainScreen.text("her rising and falling breasts under the pretext of watching her work");
		MainScreen.text(", until she happens to look up and catch your eye.  \"<i>Sorry, no parsley,</i>\" she says, breaking into an obliviously happy smile.  Seems like her temperament preternaturally suits this lifestyle.");
		...
		//[(pc is kangaroo)
		if (player.race() == "kangaroo-morph") MainScreen.text(" - except you, I suppose");
		MainScreen.text(".  We trade our goods in prearranged op shops and dead drops but avoid bringing outsiders into our own settlements, even when they're right corkers.</i>\"");
		...
		//[(lib >50)]
		if (player.stats.lib >= 50) MainScreen.text(" like yourself - I saw you perving -");
		MainScreen.text(" make us a little hornier, and evil buggers make us a bit more evil.  Our bodies tend to change on their own to reflect how tainted we are... which is as much as saying there are a sad lot among us who flirt with demonic energy to up our sizes to suit our egos.  But versus the sexual appetite and limitless rapacity of the demons themselves...?</i>\"");
		...
		//set sheila xp to 2
		Flags.list[FlagEnum.SHEILA_XP] = 2;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private sheilaGettingFamiliar(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>Hey, it's [name].</i>\"");
		...
		menu();
		MainScreen.addButton(0, "Here Often?", sheilaFriendlyComeHereOften);
		MainScreen.addButton(1, "Her People", sheilaFriendlyHerPeoples);
		MainScreen.addButton(2, "Curfew?", sheilaFriendlyCurfew);
		MainScreen.addButton(3, "No Questions", sheilaFriendlyNoQuestions);
	...
	private sheilaFriendlyComeHereOften(): void {
		MainScreen.clearText();
		MainScreen.text("You ask her why she enjoys being on the plains so much.");
	...
	private sheilaFriendlyHerPeoples(): void {
		MainScreen.clearText();
		MainScreen.text("You lie down on the grass yourself, balance the parcel on your stomach, and ask about her people.");
		...
		if (player.femininity < 35) MainScreen.text("  She looks absently at your crotch.");
		...
		//(lib >=50)
			...
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(", and you raise the kids at home?  Or put them with a nanny");
			MainScreen.text("?</i>\"");

			MainScreen.text("\n\n\"<i>Yeah, I suppose that'll work");
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(", and I'll turn the kids over to");
			MainScreen.text(" - no, I mean, ");
			if (!player.lowerBody.cockSpot.hasCock()) MainScreen.text("if I decide");
			else MainScreen.text("I'm not ready for");
			MainScreen.text(" - er... fuck <b>you!</b></i>\"  This last exclamation comes in response to your pealing laughter.  \"<i>Wanker!</i>\"");
	...
	public sheilaXPThreeSexyTime(): void {
		MainScreen.clearText();
		MainScreen.text("You prowl through the twilit grasslands, lazily looking for anything interesting before the bleak, red moon brings on the night and its restrictions.  Nothing much catches your eye, except for a rounded shape cast in silhouette against the horizon.  Initially taking it for a rock, you're surprised when it shivers at the sound of your approach!  It unbends and raises a head, turning toward you and revealing the flushed face of the bush girl, Sheila.  Her eyes quickly change from terror to relief as they settle on you, and she cries out.  \"<i>[name]!  Oh thank god; please help me!</i>\"");
		MainScreen.text("\n\n\"<i>What's wrong?</i>\"");
		...
		menu();
		MainScreen.addButton(0, "Sex", sheilaXPThreeSexyTimeSex);
		MainScreen.addButton(1, "Guard Duty", sheilaXPThreeSexyTimeGuardDuty);
		MainScreen.addButton(2, "Ditch Her", sheilaXPThreeSexyTimeDitchHer);
	}

	//[XP3 - Ditch Her]
	private sheilaXPThreeSexyTimeDitchHer(): void {
		MainScreen.clearText();
		MainScreen.text("You shake your head, pulling away from the woman and telling her she'll be fine.  \"<i>No!</i>\" she shrieks.  \"<i>You gotta help me!</i>\"");
	...
	private sheilaXPThreeSexyTimeSex(): void {
		MainScreen.clearText();
		MainScreen.text("Sheila's eyes widen with your smile, and she pulls back from you.  \"<i>That's, um...</i>\"");
	...
	private sheilaXPThreeSexyTimePostSexLetHerBe(): void {
		MainScreen.clearText();
		MainScreen.text("Pulling away from you, Sheila gathers her clothing and departs, looking satisfied.  The glance she gives you over her shoulder is even a little warmer than before.");
		//i had snoo-snoo!
		Flags.list[FlagEnum.SHEILA_XP] = 4;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private shielaXPThreeSexyTimePostSexStay(): void {
		MainScreen.clearText();
		Flags.list[FlagEnum.SHEILA_XP] = 4;
		MainScreen.text("You grab her tail, provoking a curious, but not discouraging, look from the woman who was planning to spend the night alone.  \"<i>What is it, [name]?</i>\"");
		...
		//--Next--
		menu();
		MainScreen.addButton(0, "Next", shielaXPThreeSexyTimePostSexStayII);
	}
	//fork
	private shielaXPThreeSexyTimePostSexStayII(): void {
		MainScreen.clearText();
		//(has cockfit 32)
		if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(32) >= 0) {
			MainScreen.text("Her hand slips down to your groin and, overcoming its trepidation, grabs your [cockFit 32] and begins to pump languidly, slowly stimulating blood back into it.  \"<i>I don't know what's come over me,</i>\" she says, giggling.  \"<i>Maybe I'm just greedy, but... I want you again.</i>\"  She pushes you");
			//[(horse)]
			if (player.lowerBody.isTaur()) MainScreen.text(" and you playfully roll");
			MainScreen.text(" onto your back; Sheila slips her leg over you");
			if (player.lowerBody.isTaur()) MainScreen.text("r own");
			MainScreen.text(", positioning herself atop your crotch and guiding your tip to her pussy.  It pokes through the labia slightly, welcomed with warm wetness.  \"<i>Is... this ok, [name]?  Can I?</i>\"");
			...
			//[(not horse)]
			if (!player.lowerBody.isTaur()) MainScreen.text(" just above one " + BreastDescriptor.describeNipple(0));
			else MainScreen.text(" on the thigh");
			MainScreen.text(".  \"<i>I'm gonna have to punish you for it, you dag.</i>\"  Excusing herself thusly, she sits up and begins to slide up and down your [cockFit 32].  ");
			//[(not horse)]
			if (!player.lowerBody.isTaur()) MainScreen.text("Your hands squeeze her hips almost of their own volition, helping her slam down with force, and her head rolls back on her neck.  ");
			MainScreen.text("Her ");
			//[(sheila corruption < 30)]
			if (Flags.list[FlagEnum.SHEILA_CORRUPTION] < 30) MainScreen.text("little breasts rise and fall with increasing frequency atop her ribcage as her pace picks up and her breaths come short and shallow.");
			else if (Flags.list[FlagEnum.SHEILA_CORRUPTION] > 60) MainScreen.text("huge tits jiggle magnificently as she rocks, nearly slapping her in the face as she plunges down and then falling back into place with waves of rippling flesh.");
			else MainScreen.text("breasts quiver invitingly as she pumps your cock, but before you can think of grabbing one yourself, both of her hands are squeezing and caressing them, drawing sighs from the woman as she toys with her erect nipples.");
			...
			//(horse)
			if (player.lowerBody.isTaur()) MainScreen.text("barrel");
			else MainScreen.text("chest");
			MainScreen.text(" in her soft hair - but her hips keep pumping, incredibly.  ");
			//(pc corr >= 90 or > sheila corruption)
			if (player.stats.cor >= 90 || player.stats.cor > Flags.list[FlagEnum.SHEILA_CORRUPTION]) {
				MainScreen.text("  The tight, rippling muscles of her pussy clamp down on you, nearly holding your cock in place as she tries to ride you still but only manages to jerk your dick up and down and squeeze pre-cum from it.  \"<i>S-so full,</i>\" Sheila groans, \"<i>did you just get bigger in me?</i>\"  Her lungs inflate as she sucks in her breath, and her hard nipples push into your " + player.skinFurScales() + ".");
			}
			else if (player.stats.cor < Flags.list[FlagEnum.SHEILA_CORRUPTION]) MainScreen.text("  Her pussy clamps down around you, the rippling muscles so tight that she wouldn't even be able to pump if it weren't for the hot river of juice you feel around your tool.  \"<i>A-ah,</i>\" Sheila groans, \"<i>you thrust too!  Rail me silly!</i>\"  She tries to push against you with her hands, to return herself to an upright position, but can barely pull her chest apart from you - her nipples just narrowly recede from the surface of your " + player.skinFurScales() + " as her efforts peak.");
			//(else if magically equal but < 90)
			else MainScreen.text("  Her pussy squirts and sucks alternately, trying to find purchase on your cock even as her pumping hips deny it; the sensation drives you wild.  \"<i>[name], your old fella is amazing...</i>\"");
			MainScreen.text("  Your dick twitches and swells, as if to fulfill her words, and your pelvis rocks into Sheila's as you begin to ejaculate.  \"<i>No... pull out, or you'll get me pregnant,</i>\" she cries, when ");
			//[(single cox)
			if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("your dick twitches inside her");
			//(multicox)
			else MainScreen.text("she spies the first bead of pearly semen exiting");
			if (player.lowerBody.cockSpot.count() == 2) MainScreen.text(" your lonely, unused prick");
			if (player.lowerBody.cockSpot.count() > 2) MainScreen.text(" each of your lonely, unused pricks");
			MainScreen.text(".");
			...
			//[(multi)]
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(", to be joined shortly by a messy external eruption that covers her thighs and belly in blotches of gooey white");
			MainScreen.text(".");
			//[(big skeet)
			if (player.cumQ() >= 1000) MainScreen.text("  It rolls out of you in waves, packing Sheila's cunt until she fills to capacity and the excess is forced out, sliming your own crotch.");
			...
			if (player.stats.cor >= 90 && sheilaCorruption() >= 90) {
				menu();
				MainScreen.addButton(0, "Next", sheilaGoesDemon);
				return;
			}
		...
		//(if no cockFit 32 and yes vag)
			...
			//[(horse)]
			if (player.lowerBody.isTaur()) MainScreen.text("You turn around and straddle a leg, squatting to bring your [vagina] atop her crotch.  \"<i>Push up,</i>\" you order, and Sheila obeys eagerly, twisting her body to connect her excited pussy with yours.");
			//(else if naga)
			else if (player.lowerBody.isNaga()) MainScreen.text("You slide it under her and lift gently; Sheila plays along with a disquieted expression, rolling over and raising her pussy into the air, and you press your own against her butt as your tail slides between her legs.  \"<i>See, you just need to be more... flexible,</i>\" you quip.");
			//(else if not horse and not naga)
			else MainScreen.text("You straddle her legs with your own, pressing her pussy into yours; Sheila gasps as the hot flesh connects and you tentatively rock your hips, dragging your lower lips across.");
			...
			//(sheila corruption > 30)
			if (sheilaCorruption() < 30) MainScreen.text(" even though they're small and humble");
			//(else sheila corruption > 60)
			else if (sheilaCorruption() > 60) MainScreen.text("; I worry that they're getting too mountainous");
			MainScreen.text(".</i>\"  A hand drifts up to her nipple, pinching it as her mouth hangs open.  She rolls it in her palm, then presses the breast against her ribcage, and her shiver of arousal transmits to you through");
			if (player.lowerBody.isNaga()) {
				MainScreen.text(" her taut buttcheek");
				MainScreen.text(".");
				...
				MainScreen.text(" her heated, slippery vagina.");
			...
			//[(naga) 
			if (player.lowerBody.isNaga()) MainScreen.text(" tail, drawing it up the hot opening and coiling it so you can return the warm, lube-slicked length for another pass even as you hump her butt.");
			else MainScreen.text(" cunt, grinding her like a sex toy in search of your own climax.");
			MainScreen.text("  \"<i>Do it!</i>\" you order, anxious to feel her pussy convulsing against you, anxious to feel your own squeezing her.  \"<i>Come with your lesbian lover!</i>\"");
			...
			if (player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) {
				MainScreen.text("  Her vagina doesn't wait for romance, spasming and opening wide to try to suck you in, pulling at your ");
				if (player.lowerBody.isNaga()) MainScreen.text("scales");
				else MainScreen.text("pussy");
				MainScreen.text(" until you can take no more of the heat and come as well.");
				//[(naga)]
				if (player.lowerBody.isNaga()) MainScreen.text("  Her breasts swell in your hands as she sucks in her breath sharply, feeling your pussy drool its juice onto her ass.");
			}
			//(else if PC corr < sheila corruption)
			else if (player.stats.cor < sheilaCorruption()) {
				MainScreen.text("  Her vagina doesn't wait for symbolism, dumping a wave of orgasm that splashes almost audibly against you.  Your pussy answers with your own climax, and begins wetting the woman underneath");
				if (player.lowerBody.isNaga()) MainScreen.text("; it feels like her breasts are pulling away from you as she tries to push her ass further into your quivering quim");
				MainScreen.text(".");
			}
			else {
				MainScreen.text("  Her vagina drools and spasms, not interested in gestures so much as release, and coats ");
				if (player.lowerBody.isNaga()) MainScreen.text("your tail");
				else MainScreen.text("your own");
				MainScreen.text(" with a fresh coat of her hot fluids which trigger the expected response from you.");
			}
			//(if cock)
			if (player.lowerBody.cockSpot.hasCock()) {
				MainScreen.text("  Your neglected ");
				if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("prick quakes and spurts");
				else MainScreen.text("pricks quake and spurt");
				MainScreen.text(" weakly, drooling semen that pools on her skin.");
			}
			//[(not horse)
		...
		//(else if no cockFit 32 and no vag)
			...
			MainScreen.text("Her hand lowers to her crotch and slides her middle finger into her pussy.  \"<i>I don't know what's wrong with me,</i>\" Sheila says, nervously.  \"<i>I'm not even sure why I get hot when I think of you... you don't have anything I can use.</i>\"");

			MainScreen.text("\n\nYou smile at the admission");
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(", stroking your oversized tool despite her dismissal");
			MainScreen.text(".  \"<i>But you do?</i>\"");
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				...
				//(mans)
				if (!player.lowerBody.isTaur()) MainScreen.text("falls into your lap");
				//(horse)
				else MainScreen.text("drifts between your legs");
				MainScreen.text(", then crawls its way to your [cock].  She begins stroking it, torpidly at first, but with increasing excitement as a drop of pre-cum slides down and wets her palm.  \"<i>What's wrong, goblin girl?</i>\" you ask.  \"<i>Gotta get some sperm no matter what?</i>\"");

				MainScreen.text("Sheila winces as your probing fingers send a shudder through her.  \"<i>Shut up, you tosser,</i>\" she retorts, yanking on your dick slightly harder in her irritation.");
			}
			//(new PG)
			...
			//[(pc corr >= 90 or > sheila corruption)]
			if (player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) MainScreen.text("  Her vagina heats up, squeezing your fingers so hard that you nearly can't pull out to rub her again, and you have a clear view of her breasts swelling up - arguable proof of your prowess, considering what she told you earlier.  Her people must be terrible at faking orgasm.");
			//(pc corr < sheila corruption)
			else if (player.stats.cor < sheilaCorruption()) MainScreen.text("  Her vagina gushes with clear fluid, filling the bowl made by your hand with warmth, and you think you can see her breasts tightening up slightly as her back arches, though it's hard to be sure with how she's shaking.");
			else MainScreen.text("  Her pussy quivers around you, and you draw the fingers out and pop them into Sheila's mouth.  The woman sucks on them greedily, and you raise an eyebrow at her.  She blushes again, but it's evidently not the first time she's done the same thing!");
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				...
				//[(big skeet)
				if (player.cumQ() > 1000) MainScreen.text("  Her mouth fills quickly, and she writhes in your grip, managing to turn her head.  \"<i>Oh god, no way!</i>\" she coughs, spitting up enough semen to coat her chin as your ongoing orgasm spatters against her cheek.");
				//[(mega skeet)
				if (player.cumQ() >= 2000) MainScreen.text("  Not satisfied with that alone, you turn her head the other way for your finish, ejaculating on the rest of her face and completing her gooey, white beard as she squirms.");
				//[(multicock)
				if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("  The other strokes not coming from this cock do their part to decorate your lover as well, turning her hair into a slimy mess.");
				MainScreen.text("  She escapes from your hand as you spend yourself, leaning away and gagging up seed onto the dry grass.");
			}
			MainScreen.text("\n\nThe woman falls weakly backward");
			//(cock.length > 0)
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(", her face still covered in globby white smears");
			MainScreen.text(".  \"<i>Fuck...</i>\" she groans.  \"<i>You're a jerk, [name].");
			//[(cock.length > 0)
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  I can't believe you called me a goblin, you arsehole.");
			MainScreen.text("</i>\"");

			MainScreen.text("\n\nYou ");
			//[(cock.length > 0)
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("can't believe how much she acts like one sometimes... but you ");
			MainScreen.text("just laugh and tug her ear; she flicks it and pulls away from you, backhanding you softly in the gut.  \"<i>Knock it off!</i>\"");

			MainScreen.text("\n\n\"<i>Are you really so worried about it?</i>\" you ask.  No answer comes");
			//[(cock.length > 0)
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(", even after Sheila finishes wiping her face clean and stares at you");
			MainScreen.text(", so you just content yourself with lying next to the woman for a while.");
			...
			else dynStats("lus", player.stats.lib / 4);
			sheilaCorruptionUpdate();
			/*
			if(player.stats.cor > sheilaCorruption()) {
				player.stats.cor += -5;
	...
	private sheilaXPThreeSexyTimeGuardDuty(): void {
		MainScreen.clearText();
		MainScreen.text("You fold your arms and frown at Sheila, but consent to help.  She brightens, then looks confused when you make no further motion.  \"<i>So...</i>\" she begins, \"<i>what should I do?</i>\"");
		...
		//[(PC level >=8)
		if (player.level >= 8) MainScreen.text(" - which, you suppose, must include imps even though they're so far beneath you they might as well be underground");
		MainScreen.text(".  You stand there for so long that you nearly slip into a daydream, and are only roused by the noise of a leather belt being carefully unfastened.  Focused again, you can hear Sheila's ragged breathing and her thighs rubbing together, then a hand sliding against skin and fabric... seems like she's made her decision.");
		...
		//(else lib >= 40)
			...
			//[(cock.length > 0)
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("shift your pelvis forward");
			//(cock.length == 0)
			else MainScreen.text("stick out your tongue playfully");
			MainScreen.text(".  The girl giggles at that, relaxing and finally letting herself get into the masturbation.");
	...
	private fuckBuddySheilaMeeting(): void {
		...
		menu();
		MainScreen.addButton(0, "Chat", fuckBuddySheilaMeetingChat);
		MainScreen.addButton(1, "Sex", sheilaXP4Sex, true);
		MainScreen.addButton(2, "Just Hang", fuckBuddySheilaMeetingJustHangOut);
		MainScreen.addButton(3, "Maybe Later", fuckBuddySheilaMeetingMaybeLater);
	...
	private fuckBuddySheilaMeetingMaybeLater(): void {
		MainScreen.clearText();
		MainScreen.text("You tell her you haven't the time right now, but squeeze her knee as you get up again.");
	...
	private fuckBuddySheilaMeetingJustHangOut(): void {
		MainScreen.clearText();
		MainScreen.text("In answer, you reach down to pinch her butt and then ");
		//[(not horse or drider)
		if (!player.lowerBody.isTaur() && !player.lowerBody.isDrider()) MainScreen.text("scoot your own forward");
		else MainScreen.text("fold your legs under you");
		MainScreen.text(" to lay in the bush's shade.");
	...
	private fuckBuddySheilaMeetingChat(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>Feeling talkative?</i>\" Sheila asks.  \"<i>Have at yer.</i>\"");

		MainScreen.text("\n\nWhat do you bring up?");

		//[Family&Friends][Old Loves][Why Hide?][Live With Me][Kids(only if joeycount > 0)]
		menu();
		MainScreen.addButton(0, "Fam'N Friends", fuckBuddySheilaMeetingChatFamilyAndFriends);
		MainScreen.addButton(1, "Old Loves", fuckBuddySheilaMeetingChatOldLoves);
		MainScreen.addButton(2, "Why Hide?", fuckBuddySheilaMeetingChatWhyHide);
		MainScreen.addButton(3, "LiveWithMe", fuckBuddySheilaMeetingChatLiveWithMe);
		if (Flags.list[FlagEnum.SHEILA_JOEYS] > 0) MainScreen.addButton(4, "Kids", fuckBuddySheilaMeetingChatKids);
	}

	//[XP4 - Chat - Family&Friends]
	private fuckBuddySheilaMeetingChatFamilyAndFriends(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>Them?  They're doing fine.</i>\"  Sheila stares at you obtusely, trying to will you into changing the subject; you stare right back.  She sighs.  \"<i>No go, huh?</i>\"");
	...
	private fuckBuddySheilaMeetingChatOldLoves(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>[name], I don't want to talk about that,</i>\" Sheila says, \"<i>it leaves a bad taste in my mouth.</i>\"");
		...
		//(else lib >=50)
			...
			if (player.stats.cor >= 60) MainScreen.text(" - though it's probably meant to be a defensive gesture, all it does is push her tits together under her white shirt and frame them in light-brown skin");
			MainScreen.text(".  \"<i>[name], you horny whacker, I'm not gonna marry you.  Having to bull my way through a horde of monsters to see you and stand watch in shifts just to sleep together at night may be romantic once, but it's no kind of thing to do every day for the rest of my life.</i>\"");
			...
			//[(corr < 30)
			if (player.stats.cor < 30) MainScreen.text("; you lay a hand on her shoulder reassuringly");
			MainScreen.text(".  Eventually she pulls away again, nose slightly red.  \"<i>Sorry, mate; didn't mean to have a tantrum on you.</i>\"");
	...
	private fuckBuddySheilaMeetingChatWhyHide(): void {
		...
		//(if T'A)
		if (player.statusAffects.get("TelAdre").value1 >= 1) {
			MainScreen.text("  After all, the city in the desert seems to be bustling and bristling with weapons to resist the demons, whatever her feelings may be about it.");
	...
	private fuckBuddySheilaMeetingChatLiveWithMe(): void {
		MainScreen.clearText();
		MainScreen.text("You ask Sheila to move in with you at your camp, and she turns bright red.");
	...
	private fuckBuddySheilaMeetingChatKids(): void {
		MainScreen.clearText();
		//if joeycount = 1
		if (Flags.list[FlagEnum.SHEILA_JOEYS] == 1) {
			MainScreen.text("\"<i>Ah?</i>\" Sheila says, turning toward you sheepishly.");
			...
			if (player.femininity > 50) {
				MainScreen.text("I don't know how he'll react when he sees a pretty face like yours, though; he doesn't know that I'm-");
				//[(PC is herm, fem)
				if (player.gender >= 2) {
				...
				MainScreen.text(" you guess, teasing her as her blush deepens.");
				MainScreen.text("\n\n\"<i>[name], stop it.</i>\"");
			}
			//else if PC masculine or andro, no new PG)
			else {
				MainScreen.text("I'm not sure if he intends to shake your hand or punch you, though.</i>\"");
				//[(PC is male or neuter)]
				if (player.gender <= 1) MainScreen.text("\n\n\"<i>Sounds... protective.</i>\"");
		...
		//if 1 < joeycount < 10
		else if (Flags.list[FlagEnum.SHEILA_JOEYS] < 10) {
			MainScreen.text("\"<i>So how are the kids?</i>\"");
			...
			if (player.stats.lib > 50) MainScreen.text(" and tight, hot cunt");
			MainScreen.text(" so well...  \"<i>How long does it take them to grow up?</i>\"");
			...
			if (player.gender >= 3) MainScreen.text("nother");
			MainScreen.text(" lady's age, after all.</i>\"");
		...
		//if joeycount >= 10
			...
			MainScreen.text("\"<i>Kids doing all right?</i>\"");
			...
			if (player.stats.lib < 50) {
				...
				//[(silly)]
				if (silly()) {
					MainScreen.text(" and clutching the life-sized [name] bodypillow I made");
					MainScreen.text(".  But it's not going to happen right now.</i>\"");
			...
			//(else lib >=50)
				...
				dynStats("lus", player.stats.lib / 10 + 5, "resisted", false);
				sheilaXP4Sex(false);
				MainScreen.addButton(9, "RestForNow", fuckBuddySheilaMeetingChatKidsRest4Now);
				return;
			}
		}
		doNext(camp.returnToCampUseOneHour);
	...
	private fuckBuddySheilaMeetingChatKidsRest4Now(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>Always,</i>\" you answer, \"<i>but if you need to relax and sleep for a bit, I'm here for that too.</i>\"  You draw your hand back out, patting her on the cheek.");

		MainScreen.text("\n\nSheila kisses you on the forehead and then lays her head against your ");
		if (player.lowerBody.isTaur()) MainScreen.text("back");
		else MainScreen.text("lap");
		MainScreen.text(".  \"<i>You're sweet... thank you.  I will take you up on both of those offers - but maybe just the nap this arvo.  Would you wake me up in about an hour?</i>\"");
	...
	private sheilaXP4Sex(display: boolean = false): void {
		if (display) {
			MainScreen.clearText();
			MainScreen.text("You slip an arm around the bushranger and lean in.  \"<i>I was thinking we could have some sex... feel up to it?</i>\"");
		...
		menu();
		if (player.gender > 0) {
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(0, "69WithWang", consensualSheila69, true);
			if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.addButton(1, "69WithCooch", consensualSheila69, false);
		}
		//[Consensual - Vaginal Sex] (req. cock <= 32 area):
		if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(32) >= 0) MainScreen.addButton(2, "Fuck Her", consentacleVagSexForKangarooSlutBitches);
		//da scene
		MainScreen.addButton(3, "Masturbation", sheilaMutualMasturbation);
	}
	...
	private consensualSheila69(cock: boolean = true): void {
		...
		if (cock) {
			if (player.cockThatFits(32) >= 0) {
				MainScreen.text("  Though, she smiles openly when you produce your [cockFit 32].");
				...
				//(all big cox > 32)
				MainScreen.text("\n\nIncreasingly wide eyes offset her thin, pressed lips as you reveal " + CockDescriptor.describeMultiCockSimpleOne(player) + ", the voluminous flesh swaying softly to the breeze.");
				...
				MainScreen.text(" as she rubs her small, smooth labia vigorously.  \"<i>What do you expect me to do with one of those?</i>\"");
			}
		}
		//(vag)
		else {
			MainScreen.text("  When you finally drop your bottoms, revealing your wet " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ", she smiles softly.");
			...
			if (player.lowerBody.vaginaSpot.get(0).clitLength > 8) MainScreen.text(" and bring your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " to its erect state.  \"<i>... And quite a stem,</i>\" your lover concludes.  \"<i>You sure that's not a dick?</i>\"");
			else MainScreen.text(", producing a slight sigh and a wet finger.  \"<i>And I'm the eager one, right?</i>\" your lover asks, wryly - though her finger is just as wet.");
		}
		...
		if (cock && player.cockThatFits(32) < 0) MainScreen.text("  \"<i>[name], I'm telling you it won't fit!</i>\"");
		else MainScreen.text("  \"<i>Not talking, then?  Please yourself.</i>\"");
		MainScreen.text("  Her commentary cuts off as you ");
		if (!player.lowerBody.isTaur()) MainScreen.text("turn around and hang your ");
		else MainScreen.text("straddle her and squat down to dangle your ");
		if (cock) MainScreen.text("pre-seeping shaft");
		else MainScreen.text("wet pussy");
		MainScreen.text(" over her head, waggling it provocatively as you slip your fingers into her.");

		MainScreen.text("\n\n\"<i>Ah... ah... you could have just told me,</i>\" Sheila gasps, ");
		//[(cox)
		if (cock) {
			MainScreen.text("wrapping a hand around your shaft");
			//(big) 
			if (player.cockThatFits(32) < 0) MainScreen.text(" and guiding it to bend comfortably against her chest, drooling a trail of pre-cum as she slides it into position");
			MainScreen.text(".");
		}
		//(vag)
		else MainScreen.text("returning the favor and plunging into your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " with her hand.");
		MainScreen.text("  She pumps you once, eliciting a trickle of fluid, and then speaks once more.  \"<i>Bring it lower...</i>\"");

		MainScreen.text("\n\nYou oblige, ");
		if (!player.lowerBody.isTaur()) MainScreen.text("flattening your back");
		else MainScreen.text("squatting lower");
		MainScreen.text(" so your ");
		if (cock) {
			MainScreen.text("cock sinks down");
			//[(small cock) 
			if (player.cockThatFits(32) >= 0) MainScreen.text(" into her warm, waiting mouth");
			else MainScreen.text("; she lets go of it to grab her breasts instead, trying to push them around the sensitive shaft as she runs her tongue up the underside");
			MainScreen.text(".");
		}
		else {
			MainScreen.text("pussy dangles just over her face; she ");
			if (player.lowerBody.vaginaSpot.get(0).clitLength > 8) MainScreen.text("nips and sucks at the tip of your clit, rolling it with her tongue");
			else MainScreen.text("parts your labia with her tongue, stroking them gently and tickling your little clitoris");
			MainScreen.text(".");
		}
		MainScreen.text("  Not to be outdone in creativity, technique, or audacity, you ");
		if (!player.lowerBody.isTaur()) MainScreen.text("lean down and bury your own tongue all the way inside Sheila, forcing her labia open with your mouth");
		else MainScreen.text("reach as far as you can to bring both hands into play, stroking and pulling Sheila's labia apart with one as you tease her exposed clitoris or slip inside her vagina with the other");
		MainScreen.text("; your lover stops licking and you can feel her mouth slip away from your sensitive skin as her head rolls back.");
		...
		//[(ladyparts)
		if (!cock) {
			MainScreen.text("slipping her tongue back into you");
			//[(bigclit)
			if (player.lowerBody.vaginaSpot.get(0).clitLength > 8) MainScreen.text(" and stroking your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " rapidly with one hand");
			MainScreen.text(".");
		}
		//(smallcox)
		else if (player.cockThatFits(32) >= 0) MainScreen.text("taking you into her mouth once again and licking away a neglected drop of pre-cum.");
		else MainScreen.text("stroking your cock with one hand as she pushes an erect nipple against its side to trace a line.");
		...
		if (!player.lowerBody.isTaur()) {
			...
			if (cock && player.cockThatFits(32) < 0) MainScreen.text("shaking, teasing nipples and heaving chest");
			else MainScreen.text("mouth, humming in delight");
			MainScreen.text(".");
		}
		//(horse)
		else {
			...
			if (cock && player.cockThatFits(32) < 0) MainScreen.text("hard nipples");
			else MainScreen.text("throat");
			MainScreen.text(".");
		}
		//merge horse with rider
		//[(big cox)
		if (cock && player.cockThatFits(32) < 0) MainScreen.text("\n\n\"<i>[name]!  I'm coming!</i>\" Sheila cries.");
		else MainScreen.text("\n\n\"<i>Mmmh!  Mm mmnnn!</i>\" Sheila moans, muffled by your body.  The import doesn't hit you until a second later.");
		MainScreen.text("  Her vagina convulses, ");
		//(pc corr >= 90 or > sheila corruption
		if (player.stats.cor >= 90 || player.stats.cor > Flags.list[FlagEnum.SHEILA_CORRUPTION]) {
			MainScreen.text("trying to pull your ");
			if (!player.lowerBody.isTaur()) MainScreen.text("tongue");
			else MainScreen.text("fingers");
			MainScreen.text(" in ravenously.  You can literally feel her internal muscles rippling and undulating, stroking you as if they could coax out an ejaculation.  Sheila sucks in her breath as she climaxes, filling her chest; you can feel her tits pushing harder and harder against you.");
		}
		//(PC corr < sheila corruption)
		else if (player.stats.cor < Flags.list[FlagEnum.SHEILA_CORRUPTION]) {
			MainScreen.text("pushing out a gush of fluid.  For what seems like several minutes, her pussy deluges your ");
			if (!player.lowerBody.isTaur()) MainScreen.text("mouth, filling you with her taste.");
			else MainScreen.text("hands, completely soaking them in her orgasm.");
			MainScreen.text("   Her thighs tense, forcing her pelvis into the air and her cunt toward you as her chest slumps; slowly, you feel the perky nipples pressing into your stomach pull away.");
		}
		//(else if equal but below 90)
		else if (player.stats.cor == Flags.list[FlagEnum.SHEILA_CORRUPTION] && player.stats.cor < 90) {
			MainScreen.text("drooling a trickle of fluid and licking at you as she screams her ecstasy into your crotch.");
		}
		MainScreen.text("  Sheila jams her face upward, ");
		//[(big cox)
		if (cock && player.cockThatFits(32) < 0) MainScreen.text("pressing her lips and tongue to the base of your shaft as she masturbates it vigorously.");
		///(small cox)
		else if (cock) MainScreen.text("sinking your [cockHeadFit 32] into the warm recesses of her throat where her muscles caress and coax it.");
		else MainScreen.text("penetrating you deeply with her tongue as her hands reach up to stroke your clit furiously.");
		MainScreen.text("  Your body gives up under her frenzied assault, laying your crotch onto the woman's face as muscle control abandons you.");

		//[(if cock-focus)]
		if (cock) {
			MainScreen.text("\n\nStroke after stroke of semen passes down " + CockDescriptor.describeMultiCockSimpleOne(player) + ", ");
			//[(cock fits)
			if (player.cockThatFits(32) >= 0) {
				MainScreen.text("pouring out into your lover's waiting throat.  She gags and you can feel the gooey seed oozing back against your [cockHeadFit 32]");
				//[(small cum)
				if (player.cumQ() < 250) MainScreen.text("; you withdraw from her, allowing her throat to force your ejaculate back up and over her lips and cheeks with a splutter.");
				//(big cum)
				else if (player.cumQ() < 2000) MainScreen.text(", but your relentless orgasm forces it down again, dumping more jism into her throat.  You can feel her hands clawing at your [butt] as you relieve your buildup.");
				//[(mega cum)
				else MainScreen.text(".  Soon, her visibly-rounding stomach fills and the tip of your dick is once again washed in backflow; the next spurts force the cum all the way up, and she begins drooling the slimy semen-saliva blend from her mouth and nose, hands all the more frantic in her alarm.");
			}
			//(else doesn't fit)
			else {
				MainScreen.text(" squirting onto and between your intertwined bodies to soak your [chest] and neck, and Sheila's softly pumping crotch.");
				//(big skeet)
				if (player.cumQ() > 1000) MainScreen.text("  A wellspring of seed, your body continues producing until the fuzz on her thighs and tail is completely matted down with the whiteness drooling between you.");
				//(multicocks)
				if (player.lowerBody.cockSpot.count() > 1) {
					MainScreen.text("  Your other cock");
					if (player.lowerBody.cockSpot.count() == 2) MainScreen.text(" spasms");
					else MainScreen.text("s spasm");
					MainScreen.text(" as well, adding their own issue to the unholy mess covering the poor woman.");
				}
			}
		}
		//(else if vag-focus)
			...
			if (player.lowerBody.vaginaSpot.get(0).vaginalWetness <= 2) MainScreen.text("trickle out, and are quickly sucked up by her greedy mouth");
			else if (player.lowerBody.vaginaSpot.get(0).vaginalWetness <= 4) MainScreen.text("gush freely, leaving her face slick");
			else MainScreen.text("overflow, soaking her hair even as she shuts her eyes tight against the cataract");
			MainScreen.text(".  As you climax, you wrap around her head, squeezing so hard that you flatten her ears against her skull.  Her hands latch onto your butt, trying to pull you off, but having already spent herself on her throes of passion, she's no match for yours, still ongoing, and you hold tight until your body relaxes.");
		}

		MainScreen.text("\n\nSpent, you roll off of Sheila");
		//[(smallcox)
		if (cock && player.cockThatFits(32) >= 0) MainScreen.text(", pulling free of her throat.  She immediately turns over as well and forces herself to her hands and knees, then gags and coughs up a large amount of your jism.  When she's dumped enough to clear her passageway, she collapses forward, smearing the mess onto her chest as she spends a few minutes to catch her breath.  Once she's got it, she turns back over and sits up, strands of your slime linking her breasts to the grass.");
		//(else not smallcox)
		else MainScreen.text(".  She sits up, wiping sweat and stray fluids away from her eyes.");
		...
		//[(corruption threshold warning if sheila corruption >= 80 and PC corr > sheila corruption and sheila xp =/= 3)
		else if ((player.stats.cor > 90 || player.stats.cor > sheilaCorruption()) && sheilaCorruption() >= 80 && Flags.list[FlagEnum.SHEILA_XP] != 3) {
			sheilaCorruptionUpdate();
			/*if(player.stats.cor > 90 || player.stats.cor > sheilaCorruption()) {
				player.stats.cor += -10;
				sheilaCorruption(10);
			...
			menu();
			MainScreen.addButton(0, "Listen2Her", sheilaCorruptionWarningListenToHer);
			MainScreen.addButton(1, "Fondle Her", sheilaCorruptionWarningsChooseFondle);
			return;
		...
		if (cock && player.lowerBody.balls > 0) {
			MainScreen.text("What you see manages to pull an explosive laugh from you, tired as you are.  Sheila gives you the stink-eye until you've recovered enough to explain the source of your mirth: on her forehead, making a perfect ");
			if (player.lowerBody.balls == 2) MainScreen.text("infinity");
			else MainScreen.text("double infinity");
			MainScreen.text(" shape against the rest of her skin, is an imprint of your " + BallsDescriptor.describeBalls(true, true, player) + ".  The kangaroo girl rolls her eyes at the juvenile display and resumes her harangue.  ");
		}
		MainScreen.text("\"<i>You could give a girl a bloody warning before that sort of behavior.  I thought you were going to kill me!</i>\"");
		...
		//[(sheila xp = 3)
		if (Flags.list[FlagEnum.SHEILA_XP] == 3) {
			MainScreen.text("\"<i>[name]... thanks.  Even being covered in your mess is better than being stuck outside all night, too scared to touch myself and too horny to run.</i>\"  Sheila kisses you on the cheek.  \"<i>My sloppy hero, eh?  I'm okay now; I can make it 'til morning when the gate reopens.</i>\"");
			...
			menu();
			MainScreen.addButton(0, "Let Her Be", sheilaXPThreeSexyTimePostSexLetHerBe);
			MainScreen.addButton(1, "Stay With Her", shielaXPThreeSexyTimePostSexStay);
			return;
		}
		//(else if sheila xp =/= 3)
		else {
			MainScreen.text("\"<i>Forget it.  With the way you worked me, I forgive you.  Just be careful, you wanker.</i>\"  Sheila closes her eyes and pulls your arm around her shoulder");
			if (cock && player.cockThatFits(32) >= 0) MainScreen.text(", smearing your dangling hand into the mess on her tits");
			MainScreen.text(".  The two of you lie like that for a while, until she gets up.  \"<i>I should really go... need to cure my catch.  Not to mention I'll have to clean up before I can even put my damn clothes back on.</i>\"  Sheila picks up her stuff.  \"<i>Hehe... see you soon.</i>\"");
		}
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private consentacleVagSexForKangarooSlutBitches(): void {
		...
		if (sheilaCorruption() <= 30) MainScreen.text("pert");
		else MainScreen.text("jiggly");
		MainScreen.text(" breasts as she fiddles with your own clothing.  She manages to remove the top of your [armor], exposing your [chest]; she then returns your favor by ");
		if (!player.upperBody.chest.hasFuckableNipples()) MainScreen.text("squeezing and fondling");
		else MainScreen.text("plunging two fingers into");
		MainScreen.text(" your own " + BreastDescriptor.describeNipple(0) + "s.  You shiver as she paws at the sensitive nubs on your chest, but don't let up your assault, moving your mouth down to tickle one of hers with your teeth and tongue.  Soon she's quaking and twitching beneath your ministrations; rubbing at her crotch with one hand as she half-heartedly tries to tease you back with the other.");

		MainScreen.text("\n\n\"<i>Please... I can't take any more,</i>\" she whines.  \"<i>Stop pashing on my nips and put it in, mate.</i>\"  The hand at her crotch begins trying to unfasten her belt while the other grabs and fondles her breast; you smirk and complete your own undressing quickly with both hands.  As " + CockDescriptor.describeMultiCockSimpleOne(player) + " emerges, engorged with blood, from your clothing, she switches from undressing back to rubbing, breathing heavily and stroking herself through the fabric as she stares at your ");
		//[(if wangaroo)
		if (player.lowerBody.cockSpot.list[player.cockThatFits(32)].cockType == CockType.KANGAROO) MainScreen.text("tapered, fleshy penis.  \"<i>Wow... you never see those.</i>\"");
		else MainScreen.text(CockDescriptor.describeMultiCockShort(player) + ".");
		...
		MainScreen.text("\n\nYou grin.  ");
		//[(no centaur)
		if (!player.lowerBody.isTaur()) MainScreen.text("Taking hold of your " + CockDescriptor.describeCock(player, x));
		//(centard)
		else MainScreen.text("Attempting to maneuver your " + CockDescriptor.describeCock(player, x) + " into position");
		MainScreen.text(", you rub the head along her dripping snatch, smearing her fluid over it.  ");
		//[(non-cent)
		if (!player.lowerBody.isTaur()) MainScreen.text("Gripping her fuzzy thighs next, you lift her bottom and line");
		//(hoers)
		else MainScreen.text("She lifts her bottom and lines");
		MainScreen.text(" her slippery passage up with your cock.  \"<i>Here it comes!</i>\" you cry.  With that announcement, you thrust into her, parting her lips with your swollen " + player.cockHead(x) + ".  She moans in relief as you finally enter her, but said relief is short-lived as you begin pumping at her slick vagina.  Gasping weakly, she lies back on the ground; ");
		if (sheilaCorruption() <= 30) MainScreen.text("her perky breasts rock back and forth");
		else MainScreen.text("her jiggling breasts flatten into perfect domes and undulate on her chest");
		MainScreen.text(" as you draw in and out of her warm, moist pussy.");
		...
		//[(non-cent)]
		if (!player.lowerBody.isTaur()) {
			MainScreen.text("You pause briefly, adjusting your angle and leaning down to kiss her deeply once again and help caress her ");
			if (sheilaCorruption() <= 30) MainScreen.text("firm");
			else MainScreen.text("pillowy");
			MainScreen.text(" tits, then resume thrusting from your new position; her nipples now brush against your [chest] as you pump.  ");
		}
		//(if balls/vag)
		if (player.lowerBody.balls > 0 || player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("Her stubby tail curls inward toward you, rubbing up against your ");
			...
			MainScreen.text(" and tickling you as she writhes in pleasure.  ");
			//(if vag)
			if (player.lowerBody.balls == 0 && player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("Your cunt twitches and drools a bit of fluid on the furry intruder.  ");
		}
		MainScreen.text("You fuck harder, stuffing Sheila with such force that her ");
		if (sheilaCorruption() < 30) MainScreen.text("perky, erect nipples trace long arcs against your skin");
		else MainScreen.text("jiggly tits nearly slap into her face");
		MainScreen.text(".  \"<i>Ohhh,</i>\" she moans, \"<i>YES!</i>\"  At that moment you feel her vagina clamp around your [cockFit 32], wringing it frantically as she reaches her climax.  You continue pumping as best you're able, but you can't stand up to the siege of her spasming pussy for long; ");

		//[(pc corr >= 90 or > sheila corruption) 
		if (player.stats.cor >= 90 || player.stats.cor > Flags.list[FlagEnum.SHEILA_CORRUPTION]) {
			MainScreen.text("something builds to a painful intensity in your crotch.  It feels like two mind-blowing orgasms arriving together and fighting over which one gets to come out first.  You rise toward climax quickly and then are drawn back in moments of maddening frustration, over and over.  In response you intensify your thrusts even more, sawing in and out of the girl at a pace that would be blistering if the two of you weren't leaking enough pre-cum and pussy juice to put out a brushfire.  Eventually the pleasure overwhelms the strange reticence of your orgasm; you cry out loudly as you peak, and squeeze a tiny trickle of cum into her grasping vagina.");
			//[(multi)
			if (player.lowerBody.cockSpot.count() > 1) {
				MainScreen.text("  Your other ");
				if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("dick erupts");
				else MainScreen.text("dicks erupt");
				MainScreen.text(" as well, festooning her crotch with sticky strings.");
			}
			MainScreen.text("  The intense sensation begins to ascend your dick slowly; your body works itself into overdrive and your partner's moans increase sympathetically as it nears the tip.  Finally, it exits your body with an enormous squeeze, alongside a jet of jism.  Sheila gives a complementary gasp as this pulse enters her, and her pussy convulses in a second orgasm, causing her back to arch.  Her breasts seem to jiggle more wildly than before as she thrashes on the ground in the grip of her pleasure.");
		}
		//(PC corruption < sheila corruption)
		else if (player.stats.cor < Flags.list[FlagEnum.SHEILA_CORRUPTION]) {
			MainScreen.text("your resistance is condemned to die in failure as Sheila lets out a quavering scream of pleasure and wraps her arms and legs around you, pinning you inside.  Her vagina convulses around your cock powerfully; you could swear you feel yourself stretching longer and longer as her rippling pussy pulls you deeper in.  Just when you think that your dick must be a mile long, her whole body tenses up and her fingernails dig into your back.  Wearing a madwoman's expression, Sheila stares open-mouthed at you as her muscles lock and her pussy quivers out its fitful orgasm; even her ");
			if (sheilaCorruption() < 30) MainScreen.text("perky");
			else MainScreen.text("pillowy");
			MainScreen.text(" breasts seem to be held taut, compressed and motionless.  Suddenly she rams her hips into yours violently and throws her head back, and before you can issue a word of protest, her cunt wrings your " + CockDescriptor.describeCock(player, x) + " one last time with almost supernatural heat; your mind clouds over and a wracking orgasm shoots to the base of your spine, echoing back with the first wave of your ejaculation.");
		}
		//(else if equal but below 90)
		else {
			MainScreen.text("your orgasm quickly supersedes your control as she wrings you and rubs her nipples against your [chest].  " + CockDescriptor.describeMultiCockSimpleOne(player, true) + " begins to twitch as it unloads its first squirt of semen into her spasming pussy");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" or onto her thighs and belly");
			MainScreen.text(".");
		}
		...
		//[(non-dog cock)
		if (!player.hasKnot(x)) {
			MainScreen.text("You oblige shakingly, pulling your " + CockDescriptor.nounCock(player.lowerBody.cockSpot.get(x).cockType) + " out of her clenching pussy with a wet, sucking noise.  A trickle of semen drools from between her lips as you unburden yourself of the rest of your load, squirting it onto her groin");
			//[(big skeet)
			if (player.cumQ() > 500) MainScreen.text(", breasts, and face");
			MainScreen.text(".");
			//[(Huge skeet)
			if (player.cumQ() >= 1500) MainScreen.text("  More and more globs of semen shoot from the end of " + CockDescriptor.describeMultiCockSimpleOne(player) + ", pasting Sheila to the dry grass as you paint her body white.");
		}
		//(dog cock)
		else {
			MainScreen.text("You attempt to oblige her, but your swollen knot sticks firmly in her squeezing pussy, locking you into place.  After a few fruitless tugs, you give yourself up to your orgasm with an apology, slumping against her heaving chest and helplessly dumping the rest of your load into the twitching woman. ");
			//[(big skeet)
			if (player.cumQ() >= 1000) {
				MainScreen.text("Her belly begins to round as your overproductive gonads push out your massive store of semen");
				//[(huge skeet)
				if (player.cumQ() >= 3000) MainScreen.text(", until it's stretched as far as possible and each successive jet forces out squirts of frothy jism from the tight seal her pussy makes around the knot");
				MainScreen.text(". ");
			}
			//[(multi)
			if (player.lowerBody.cockSpot.count() > 2) MainScreen.text("Your other cocks erupt as well, soaking her crotch and matting down the fur on her thighs with whiteness. ");
		}
		MainScreen.text("Both of your orgasms finish up");
		//(dog-c) 
		if (player.hasKnot(x)) MainScreen.text(" and you finally manage to pull out of her");
		MainScreen.text("; the two of you lie there breathing raggedly for several minutes.  Sheila breaks the silence first.");
		...
		//(else sheila xp =/= 3)
			...
			//[(norm skeet)
			if (player.cumQ() < 500) MainScreen.text("as she strokes the " + player.skinFurScales() + " of your chest");
			else MainScreen.text("as she wipes off as much jizz as she can");
			MainScreen.text(", then get up and collect your gear.  \"<i>Um... see you later, ok?</i>\" she says.");
			doNext(camp.returnToCampUseOneHour);
		}
	...
	private sheilaMutualMasturbation(): void {
		...
		if (player.lowerBody.cockSpot.hasCock()) {
			if ((player.lowerBody.isTaur() && player.tallness * (5 / 6) < player.lowerBody.cockSpot.list[player.longestCock()].cockLength) || player.lowerBody.cockSpot.hasCock()) choices[choices.length] = "cock";
		}
		if (player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.isTaur()) choices[choices.length] = "vagina";
		if (player.upperBody.chest.hasFuckableNipples()) choices[choices.length] = "nipplecunt";
		if (player.gender == 0 || choices.length == 0) choices[choices.length] = "ass";

		if (choices.length == 0) {
			CoC_Settings.error("");
			MainScreen.text("ERROR - SHEILA MASTURBATE BROKE, SON");
			doNext(camp.returnToCampUseOneHour);
		}
		target = choices[rand(choices.length)];

		MainScreen.text("You snatch Sheila's hat and pack, tossing them away, then slide a hand into the girl's shirt, slipping one of her nipples between two fingers as you nibble on her neck, and use the gentle pressure to push her onto her back.  She sighs, moving a hand under her clothes to caress yours.  \"<i>Ah,</i>\" she pants, letting out a soft puff of air, \"<i>what're we gonna do on the grass, [name]");
		if (silly()) MainScreen.text("-chan");
		MainScreen.text("?</i>\"");
		...
		//[(horse)
		if (player.lowerBody.isTaur()) MainScreen.text(", folding your legs under you,");
		MainScreen.text(" and begin ");
		//[(adequate-length cock)
		if (target == "cock") {
			MainScreen.text("stroking your [cock], pointing it at her as you smear pre-cum onto your fingers");
		}
		//(non-horse vag)
		else if (target == "cunt") MainScreen.text(" sliding your hand to your [vagina], parting your lips with two fingers");
		//(gross-ass nipcunts)
		else if (target == "nipplecunt") MainScreen.text("plunging your fingers into one of your awkward, distended nipples");
		//(Unsexed non-centaur only)
		else if (target == "ass" && !player.lowerBody.isTaur()) MainScreen.text("licking a finger and teasing your asshole with shallow strokes, gently circling the ring");
		//(centaur who hasn't got shit)
		else MainScreen.text("sucking a single digit salaciously, fingerfucking your mouth and coating it with saliva");
		MainScreen.text(".  \"<i>Oh... are you going to touch me, or just play with that?</i>\" Sheila asks, grinning.");
		...
		if (target == "cock") MainScreen.text("You return your hand to your [cock], pumping vigorously to catch up to your lover, spurred onward by the smooth skin she presses into your lips.");
		//(else if nonhorse vag)
		else if (target == "cunt") MainScreen.text("You slide your hand between your [legs] again, intent on recovering the march Sheila's stolen on you, and begin rubbing your [clit] vigorously while sighing into her breast.");
		//(else if RNG disgusting nipplecunts)
		else if (target == "nipplecunt") MainScreen.text("Your hand returns to your chest, slipping inside the sensitive flesh of your nipple, and you groan into Sheila's as a trickle of moisture falls from your chest.");
		//(else if non-centaur asshole)
		else if (!player.lowerBody.isTaur()) MainScreen.text("Your hand reaches back to your [asshole], neglected for too long, and a finger spears your pucker, stroking the nerves inside as you hurry to bring your arousal in line with Sheila's.");
		///(else if fucking nothing)
		else {
			MainScreen.text("You wish you had something to play with as you watch your lover writhe under you");
			//[(silly), even just a paperclip].]
			if (silly()) MainScreen.text(", even just a paperclip");
			MainScreen.text(".");
		}
		...
		//[(if fucking nothing)
		if (target != "cock" && target != "cunt" && target != "nipplecunt") MainScreen.text(", and yours joins her, slipping into her entrance alongside");
		MainScreen.text(".  \"<i>Just... say I'm always yours!</i>\"  She turns you toward her face again, locking you in a kiss, and your ");
		//[(if cock)
		if (target == "cock") {
			if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("prick begins");
			else MainScreen.text("pricks begin");
			MainScreen.text(" to twitch, ejaculating a stroke onto Sheila's midriff and wrist.  The girl groans in your mouth at the new heat, and her body shakes in orgasm.");
		}
		//(vag)
		else if (target == "cunt") {
			MainScreen.text("pussy convulses, ");
			if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < 4) MainScreen.text("drooling");
			else MainScreen.text("squirting");
			MainScreen.text(" around your hand.  As your body shakes and your lips rattle against Sheila's, she shivers and her own orgasm begins.");
		}
		//(nipcunts)
		else if (target == "nipplecunt") {
			MainScreen.text("nipple constricts around your fingers in climax, oozing lubrication onto your partner.  As the drops from your " + BreastDescriptor.describeBreastRow(player.upperBody.chest.get(0)) + " hit her own, Sheila's back arches again and her own orgasm begins.");
		}
		//(jack shit)
		else MainScreen.text("fingers are pulled into her vagina by her muscles as her orgasm begins.");
		//[( pc corr >= 90 or > sheila corruption) 
		if (player.stats.cor >= 90 || player.stats.cor > Flags.list[FlagEnum.SHEILA_CORRUPTION]) {
		...
		//(else if equal corr but under 90)
			...
			MainScreen.text("She breaks the kiss, drawing her hand from her pussy, and holds your head in front of her; her eyes soften with affection as she shudders.");
			//[(not jack shit)
			if (target == "cunt" || target == "cock" || target == "nipplecunt") MainScreen.text("  Emptying your reserves, you allow yourself to fall into her arms, resting your head on her chest.");
		}
		...
		//(else no corruption warning triggered)
			...
			MainScreen.text("\n\nSheila strokes your " + HeadDescriptor.describeHair(player) + " as you lie ");
			if (target == "cock") MainScreen.text("there, staring at the load you left in her bellybutton");
			else MainScreen.text("together");
			MainScreen.text(".  \"<i>Oh, I had so much fun, mate.");
			//[(if sex =U and sillymode)
			if (player.gender == 0 && silly()) MainScreen.text("  I love you, my Ken doll.");
			MainScreen.text("</i>\"");
			//[(if sheila xp = 3)
			if (Flags.list[FlagEnum.SHEILA_XP] == 3) {
	...
	private sheilaCorruptionWarningsChooseFondle(): void {
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("erect");
		else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("wet");
		else MainScreen.text("aching");
		MainScreen.text(".  Even as she goes, you can still see the edges of her amazing, swaying rack around the sides of her torso.");

		MainScreen.text("\n\nBitch tease.");
		//set sheila xp to -3 and sheila corruption to 90, PC corr - 10
		Flags.list[FlagEnum.SHEILA_XP] = -3;
		Flags.list[FlagEnum.SHEILA_CORRUPTION] = 90;
		player.stats.cor += -10;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private sheilaCorruptionWarningListenToHer(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>So...</i>\" you answer, your feel frozen in mid-cop, \"<i>what?</i>\"");
		...
		player.stats.lust += -10;
		Flags.list[FlagEnum.SHEILA_CLOCK] = -4;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	internal function sheilaGotWhomped(): void {
		MainScreen.clearText();
		MainScreen.text("Sheila collapses, ");
		//[(HP)
		if (monster.HP < 1) MainScreen.text("completely spread-eagled from her injuries");
		///(lust)
		else MainScreen.text("rubbing her thighs together and smearing around the moisture from her pussy");
		MainScreen.text(".  As you stand over her, she forces herself to her elbows and tries to scoot away from you, fear written on her face.  \"<i>I'm sorry... please, no...");
		//[(sheila xp > -4)
		if (Flags.list[FlagEnum.SHEILA_XP] > -4) MainScreen.text("  I'll forgive you for everything else, but please, not this!");
		MainScreen.text("</i>\" she implores.");
		//[(sheila corruption >= 90)
		if (sheilaCorruption() >= 90) MainScreen.text("  \"<i>I-if you touch me again... god, I can't even think straight anymore!  Don't do this!</i>\"");
		//(lust >= 30)
		...
		if (player.stats.lust >= 33) {
			if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(48) >= 0) MainScreen.addButton(0, "Fuck Pussy", rapeSheilasCooter);
			if (player.lowerBody.cockSpot.hasCock()) {
				if ((!player.lowerBody.isTaur() && player.shortestCockLength() <= player.tallness * .35) || (player.lowerBody.isTaur() && player.tallness * 1.2 <= player.shortestCockLength() && player.shortestCockLength() <= player.tallness * 1.55)) {
					MainScreen.addButton(1, "Forced BJ", forcedSheilaOral, true);
				}
			}
			if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.addButton(2, "Forced Lick", forcedSheilaOral, false);
		}
		MainScreen.addButton(9, "Leave", letSheilaGo);
	}
	//if choosing Let Her Go and sheila xp > -3, set sheila xp = -1, then give xp/gems and Next button to camp
	private letSheilaGo(): void {
		if (Flags.list[FlagEnum.SHEILA_XP] >= -3) Flags.list[FlagEnum.SHEILA_XP] = -1;
		cleanupAfterCombat();
	}
	...
	private rapeSheilasCooter(): void {
		...
		MainScreen.text("You grab the girl by her fuzzy leg; she tries to kick away from you, but you pull her back, pinning her");
		//[(horse)
		if (player.lowerBody.isTaur()) MainScreen.text(" by planting your forehooves just above her hips");
		MainScreen.text(".  Reaching down, you take the neck of her shirt in your hand, then rip it open, exposing her ");
		//[(sheila corruption < 30)
		if (sheilaCorruption() < 30) MainScreen.text("tiny ");
		//sheila corruption > 60)
		else if (sheilaCorruption() > 60) MainScreen.text("huge, soft ");
		MainScreen.text("breasts, which jiggle from the force.  She tries to cover herself with her arms, but you grab and pull them aside, revealing her chest again.  As you appraise her naked flesh, she looks away, shame coloring her cheek.  Her mouth tightens and her eyes shut as you squeeze her tits roughly, flicking the nipples until they're erect.");
		...
		//[(monster HP < 1)
		if (monster.HP < 1) MainScreen.text("sliding it between her labia and producing a damp spot, then ");
		MainScreen.text("dragging the wet fabric around her vulva.");
		...
		if (player.stats.cor < 50) {
			...
			//[(big skeet)
			if (player.cumQ() >= 500) MainScreen.text("  Soon you've filled her as fully as the shallow penetration will allow, and the panties begin to press back against your cock, oozing onto you whatever semen the porous dam can't contain.");
			//(mega skeet)
			else if (player.cumQ() >= 1500) MainScreen.text("  It sags and bows around the tip of your spear, pulsing with each new stroke as the hemlines leak jizz onto her legs and butt.");
			MainScreen.text("  Idly, you imagine what it must look like inside, each squirt having its momentum arrested by the cloth, drooling lazily into her until the next one displaces it a bit further forward, and wonder if it'll be enough to get her pregnant.  You pull away from the girl, leaving her gasping and once again averting her gaze.");

			MainScreen.text("\n\nShe attempts to stand, holding her midriff and looking at you unhappily.  ");
			//[(big skeet)
			if (player.cumQ() >= 500) MainScreen.text("So much cum fills her pussy that it slips out, once again restoring the opacity, though not the dryness, of her panties.  ");
			MainScreen.text("You return her gaze, turning your nose up dismissively.  Conscious of her nudity, she pulls up her shorts and buttons her vest to conceal herself as you leave her behind.");
			...
			if (player.cockArea(x) > 32) MainScreen.text(" and the pain of taking a cock slightly too large for her");
			MainScreen.text(", and she actually rears back to scratch at you with her fingernails.  You catch her hand and hold it, and she responds by spitting at you!  \"<i>Fuck you, arsehole!</i>\" Sheila shrieks.  \"<i>Take it out!</i>\"  Reaching down, you slap her in the face.");
			...
			if (sheilaCorruption() > 10) MainScreen.text(" jiggles her breasts and");
			MainScreen.text(" clenches her jaw.  A bloom of red colors her cheeks, one brighter than the other from the blow... though she still refuses to look at you.  Doesn't matter; having sex.  You begin to pump her pussy vigorously, the wetness enough to allow quick, smooth thrusts without any warmup.  \"<i>You're a godawful liar,</i>\" you grunt");
			//[(not centaur)
			if (!player.lowerBody.isTaur()) MainScreen.text(", leaning down to kiss her long neck and collarbone");
			MainScreen.text(".  Sheila quivers again, still looking away, as you continue.  \"<i>I can actually feel you stroking me with your vagina already.  Are you going to come soon?  Will you tell me how much you're hating this when you do, and wrap your legs around me to punish me by holding me inside?</i>\"  The girl's blush deepens, and she bites her lip in embarrassment.  Confidently, you release her hand");
			//[(nothors)
			if (!player.lowerBody.isTaur()) MainScreen.text(", transferring yours to her breast while brushing her torn shirt to the side");
			MainScreen.text("; without struggling further, she slowly lowers it");
			//[(nothors)
			if (!player.lowerBody.isTaur()) MainScreen.text(" to rest atop your own");
			MainScreen.text(", confirming your suspicion.  Her eyes close and her chin raises resignedly");
			//[(nothors)
			if (!player.lowerBody.isTaur()) MainScreen.text(" as you triumphantly squeeze the flesh in your hand and kiss her under the jaw");
			MainScreen.text(".");
			...
			//[(mans)
			if (!player.lowerBody.isTaur()) MainScreen.text("place a hand along her face, tuning it toward you, and kiss her on the lips.");
			else MainScreen.text("bend down, lowering your torso until you can caress her cheek.");
			MainScreen.text("  \"<i>Of course I won't,</i>\" you answer sweetly.  \"<i>What would be the point?</i>\"  Sheila winces as you hilt yourself, sinking your [cockFit 48] so far into her that your crotch meets hers.  You shudder");
			//[(nonhors)
			if (!player.lowerBody.isTaur()) MainScreen.text(" and squeeze her breast");
			MainScreen.text(" as your climax arrives and the first of your seed pours into her grasping pussy.  While you fill her, her back arches");
			//[(nonhors) 
			if (!player.lowerBody.isTaur()) MainScreen.text(" and she presses your hand uncomfortably");
			MainScreen.text(", gasping as her own orgasm begins.  Her knees wrap around your sides, satisfying your predicition, and her vagina");
			//[(pc corr >= 90 or > sheila corruption)
			if (player.stats.cor >= 90 || sheilaCorruption() < player.stats.cor) {
				MainScreen.text(" pulls greedily at your [cockFit 48], drinking up your semen as fast as you can squirt it.  She sucks in breath as she climaxes, finally opening her eyes as her head tilts back");
				//[(nonhors)
				if (!player.lowerBody.isTaur()) MainScreen.text("; you can almost feel her breast heating up and swelling with blood under your hand, as the nipple pokes into your palm");
				MainScreen.text(".");
			}
			//(PC corr < sheila corruption)
			else if (player.stats.cor < sheilaCorruption()) {
				MainScreen.text(" floods you with juices, dumping so much around your twitching cock that you feel like you're lowering it into a hot spring.");
				//[(nonhors)
				if (!player.lowerBody.isTaur()) MainScreen.text("  She squeezes your hand so hard that you swear you can feel her breast compressing under it as she screams her pleasure.");
			}
			//(else equal but < 90)
			else {
				MainScreen.text(" drools eagerly as you mingle your juices with hers, until the fluid begins to thicken with white seed.");
			}
			//[(big skeet)
			if (player.cumQ() >= 500) MainScreen.text("  Overabundant as always, your semen quickly fills her to capacity and begins to squirt out around your shaft, staining her thighs.");
			//[(multicock)
			if (player.lowerBody.cockSpot.count() > 1) {
				MainScreen.text("  Your other ");
				if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("dick spits");
				else MainScreen.text("dicks spit");
				MainScreen.text(" as well, leaving trails all over her light-brown skin.");
			}
	...
	private forcedSheilaOral(dick: boolean = true): void {
		...
		//set sheila xp = -4
		Flags.list[FlagEnum.SHEILA_XP] = -4;
		MainScreen.text("You lean down and grab the girl by her shirt-front, raising her to her knees; her hands automatically go to yours.");
		...
		if (sheilaCorruption() >= 50) MainScreen.text("soft, pillowy");
		else MainScreen.text("small, perky");
		MainScreen.text(" breast.  \"<i>You don't want to rip your clothes, do you?  It must be difficult to get nice fabric like this out here in the middle of nowhere.</i>\"");
		...
		if (sheilaCorruption() < 20) MainScreen.text("little round things that trace gentle curves under her loose shirt.");
		//(else sheila corruption < 50)
		else if (sheilaCorruption() < 50) MainScreen.text("healthy mounds that fill out her shirt.");
		else MainScreen.text("swollen globes that bulge over her v-neck and strain at the seams of her shirt.");
		MainScreen.text("  Smiling at the sight, you grab the garment's hem and pull it up roughly.  Sheila blinks dumbly as you expose her erect nipples to the air, then blushes and covers them with her hands.");

		MainScreen.text("\n\nStill smiling, you unfasten your [armor] and reveal your crotch.  ");
		//[(cock.length > 0)
		if (dick) MainScreen.text(CockDescriptor.describeMultiCockSimpleOne(player, true) + " flops free, half-erect and swelling steadily with blood as you appraise the woman's body.");
		//(vag present)
		else MainScreen.text("Your [vagina] drools slightly, labia parting and [clit] poking up; you imagine what you want to do with it.");
		MainScreen.text("  Sheila watches your face as you slip into a fantasy, half-playing with her nipples - but just when you start to get into your daydream, she turns and tries to scramble to her feet!");
		...
		if (dick) {
			...
			//[(cock width <= 6")
			if (player.lowerBody.cockSpot.list[player.shortestCockIndex()].cockThickness <= 6) {
				MainScreen.text("parting her lips to pop neatly between them.  It feels good, but one-sided, and you order Sheila to get to work.");
			}
			else {
				MainScreen.text("jabbing at her undersized mouth with your enormous [cockHead shortest].  \"<i>It's too thick!</i>\" she complains, trying to turn away.");
			...
			MainScreen.text("  The reticent woman brings her lips into play, sucking gingerly, ");
			//start length fork, no new PG
			//[(dick length <= 8)
			if (player.lowerBody.cockSpot.list[player.shortestCockIndex()].cockLength <= 8) {
				MainScreen.text("and the warm embrace of her mouth is welcome stimulation.");
				...
				if (player.lowerBody.balls > 0) MainScreen.text("your [sack]");
				else MainScreen.text("the base of your shaft");
				MainScreen.text(", and the other hesitantly makes its way to her breasts as she gets into the sex.  The woman tweaks her nipple, softly at first but with mounting passion as her attentions on your dick become more lavish.  Catching this from the corner of your eye, you move ");
				if (!player.lowerBody.isNaga()) MainScreen.text("a [foot]");
				else MainScreen.text("the tip of your tail");
				MainScreen.text(" between her knees to her crotch, rubbing her pussy through her shorts; Sheila hums into your shaft as a wet spot appears on the cloth, and begins grinding her hips against the pressure.  You answer her enthusiasm by taking her head in your hands and stroking the surface of her long, pointed ears affectionately with your thumbs.  She flushes, then sinks down on your cock, rubbing the nerves on the slit with the back of her tongue and pulling you closer to orgasm.");
				...
				if (player.lowerBody.balls > 0) MainScreen.text(" and your [balls] tighten up");
				MainScreen.text(".  She pushes against you with her hands, trying to get free, but you just hold her against your crotch as you lean over her head.");
			}
			//(else dick length > 8)
			else {
				MainScreen.text("but this sort of half-assed attention won't get you off any time before sundown.");
				...
				///(else width > 6)
					...
					if (player.lowerBody.balls > 0) MainScreen.text(" and your [balls] draw closer to your body");
					MainScreen.text("; still intent on calling this oral sex, you squeeze Sheila's jaw, forcing her mouth open, then push your drooling slit right up to it as the first squirt of white peeks out.");
				}
			}
			//end width fork and length fork
			//begin corruption worms fork
			//(if corr >= 70 and Infest command is unlocked)
			if (player.stats.cor >= 70 && player.statusAffects.get("Infested").value1 == 5) {
				MainScreen.text("\n\nA wicked idea flits through your head, brought on by the squirming annoyances ");
				if (player.lowerBody.balls > 0) MainScreen.text("packing your [balls] and driving up your lust in their zeal to multiply");
				else MainScreen.text("deep inside your body, trailing sensation along the nerves in your most intimate places and ramping up your production");
				MainScreen.text(", and you decide to indulge it.  \"<i>Don't worry about rations for the day,</i>\" you cry out, \"<i>I've got more than enough to feed you!</i>\"  Sheila's resigned expression registers this as little more than masculine braggadocio - all the better for your surprise.  Pulling deep inside yourself, you force out a thick load of wormy jism, depositing it directly into the woman's throat; her body tenses at the texture and she tries again to pull away.  As the writhing mass is pushed down her gullet by yet more arriving after it, she begins to struggle violently, hitting you with tired fists and trying to wrench her head free of your grip, even at the cost of her ears.  Luckily, she lacks the energy to do permanent damage to either you or herself after the fight and the cruel face-fucking, and you pour your infested load into her with abandon.");
				//[(big skeet)
				if (player.cumQ() >= 2500) MainScreen.text("  The volume of semen and slimy worms you pack her with is so great that it strains her stomach; you can actually see dozens of the annelids slithering around in a tight ball under the taut skin of her swollen midriff.");
				MainScreen.text("  Eventually you finish and pull out, after pushing up every worm you can.  Your ");
				if (player.lowerBody.balls > 0) MainScreen.text("[sack]");
				else MainScreen.text("body");
				MainScreen.text(" actually feels lighter by a noticeable amount - though you can still feel some deep inside you, reminding you that you'll be full again soon.  On that note...");
			...
			//(else if corr < 70 or PC cannot use Infest at will)
				...
				//[(big skeet)
				if (player.cumQ() >= 1000) MainScreen.text(", and soon you can feel it lapping at the end of your shaft");
				MainScreen.text(".");
				//[(mega skeet)
				if (player.cumQ() >= 3000) MainScreen.text("  Your body's not done with that, though, and keeps pushing out cum until it spurts around your prick and out Sheila's nose in little squirts.");
				MainScreen.text("  Emptying yourself, you pull Sheila's mouth away from your [cock shortest] and drag the slimy member on her chest, smearing her tits with spit and semen and wiping away the last of the mess on her shirt.");

				MainScreen.text("\n\nYou pull the soiled garment down over Sheila's breasts");
				//[(big+ skeet)
				if (player.cumQ() >= 1000) MainScreen.text(", though it's so wet with seed now that you can see them almost as well through it");
				MainScreen.text(".  \"<i>This really is a nice fabric.  You should take better care of it.</i>\"  With that parting shot, you leave the queasy-looking woman to digest your load and the humiliation.");
			}
			//end worm fork
			...
			//[(sheila corruption >= 30)
			if (sheilaCorruption() >= 30) MainScreen.text("; her breasts jiggle pleasingly as they settle on her chest, filling you with the urge to play with them");
			MainScreen.text(".  Straddling her, you lower your [vagOrAss] to her face.  \"<i>Lick,</i>\" you order, rubbing your [butt] on Sheila's nose");
			//[(not horse)
			if (!player.lowerBody.isTaur()) MainScreen.text(" as you fondle her tits");
			MainScreen.text(".");
			...
			if ((player.lowerBody.vaginaSpot.hasVagina() && player.vaginalCapacity() >= 100) || (!player.lowerBody.vaginaSpot.hasVagina() && player.analCapacity() >= 100)) MainScreen.text("  She keeps hitting the same spot again, too - she's not even bothering to adjust her angle and touch the rest of your stretched opening.");
			...
			if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00141] > 0) MainScreen.text("  I know a nice boy in the mountains who keeps dogs, and I'm sure he'd love to have a womb like yours for their seed.  By this time tomorrow, you could have a cock in both holes, filling you with hellhound babies since you can't please a person.");
			MainScreen.text("</i>\"");

			MainScreen.text("\n\nYou can feel her face heating up under your [butt] as she flushes with shame; seems like you struck a nerve.  You ");
			//[(no horse)
			if (!player.lowerBody.isTaur()) MainScreen.text("pinch her nipple for emphasis, then grind your hips across her face, mauling her nose with your ass.  \"<i>I could fuck a tree and it wouldn't be as wooden as you in the sack");
			//[(PC has found corrupt glade)
			if (player.exploredForest >= 40) MainScreen.text(" - in fact, there are quite a few I've seen who look like better lovers than you.  Maybe I should carry you to the forest and tie you to a nice pussy-shaped giant flower to give you lessons");
			MainScreen.text(".  ");
			//[(minotaur addiction score =/= 0%)
			if (Flags.list[FlagEnum.MINOTAUR_CUM_ADDICTION_TRACKER] > 0 || player.perks.has("MinotaurCumAddict")) {
				MainScreen.text("Perhaps I'll turn you over to a minotaur; you're bound to get better at using your tongue when you're addicted to their amazing seed.  Would you like that?  The first thing you'd see in the morning would be a huge flared cock, right in front of you, dripping with heavenly precum and waiting for your mouth... or your cunt.  You could be just another minotaur-fucking slut of the plains.  ");
			}
			MainScreen.text("Or I could take you home and give you personal training myself, whenever I feel like it, and invite travelers to do the same");
			//[(PC has Izma with cock)
			if (izmaFollower() && Flags.list[FlagEnum.IZMA_NO_COCK] == 0) MainScreen.text("; I personally know fifteen inches of fat, orange dick and four heavy balls that'd love to get in at your other end while I ride your face");
			MainScreen.text(".</i>\"");
			...
			if (player.statusAffects.has("HairdresserMeeting") || Flags.list[FlagEnum.LUMI_MET] > 0) {
				MainScreen.text(" while imagining her turned over to the goblins for experiments, forced to drink potion after potion - most would be dedicated to growing cocks on her or increasing semen production, of course");
			}
			MainScreen.text(".  Your hole clenches and drools on her, and the woman's rude tongue retreats from the pressure, leaving you painfully empty.");
			//[(no horse)
			if (!player.lowerBody.isTaur()) MainScreen.text("  Even squeezing her breasts viciously can't compel her to reinsert it, though you amuse yourself trying.");
	...
	private sheilaGetsRapedWithADildont(): void {
		...
		if (player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) {
			MainScreen.text("Your face heats as she kisses you sloppily, and not all of the temperature is from her mouth.  It feels like she's pulling energy right out of you, and you can swear that her breasts are growing.");
		}
		//(else PC corr < sheila corruption)
		else if (player.stats.cor < sheilaCorruption()) {
			MainScreen.text("Her lips heat up and your body tingles; you can see her breasts shrinking as she shivers and slobbers on your face - or so it seems from the angle.");
		}
		//(else if magically equal but < 90)
		else MainScreen.text("The kiss is wet and indecent as she shudders through her anal orgasm; her lips don't stay in one place, instead tracking drool all over your face.");
		...
		//(cock)
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  \"<i>Perhaps next time you'd like to try a real cock in there?</i>\"  Even exhausted and messed up on goblin drugs, she has the presence of mind to shake her head at that.  You laugh.");
		...
		dynStats("lus", player.stats.lib / 3, "resisted", false);
		cleanupAfterCombat();
	}
	...
	internal function getBeatUpBySheila(): void {
		MainScreen.clearText();
		MainScreen.text("You slump down before Sheila, breathing heavily; the woman ");
		if (monster.lust < 75) {
			MainScreen.text("takes the opportunity to grab her catch and take off running!  Even if you wanted to, you couldn't catch her now... you collapse, head swimming in ");
			if (player.HP < 1) MainScreen.text("colorful spots");
			else MainScreen.text("fantasies of her, stark naked but for her hat and waiting with her legs open");
			MainScreen.text(".");
		...
		//(else monster lust >= 75)
			...
			MainScreen.text("stares at you, desire and nerve at war in her face.  Her thighs rub together involuntarily as she tries and fails to will herself away... slowly, she takes a step toward you.");
			...
			//[(sheila xp > -4)
			if (Flags.list[FlagEnum.SHEILA_XP] > -4) {
	...
	private sheilaLossRapeRouter(): void {
		...
		if ((!player.lowerBody.cockSpot.hasCock() || rand(3) == 0)) sheilaForcedOralGeneric();
	...
	private sheilaCowgirlRapesYou(): void {
		MainScreen.clearText();
		MainScreen.text("Sheila pushes insistently, and you allow her to force you to the ground and then onto your back");
		if (player.lowerBody.isTaur()) MainScreen.text("; your hooves stick awkwardly out to one side");
		MainScreen.text(".  She blushes furiously and won't meet your eyes, but it doesn't stop her from stripping off her vest and shirt in one smooth motion, revealing her " + sheilaCup() + " chest.  \"<i>T-touch me,</i>\" she demands, \"<i>rub me.</i>\"  Almost as an afterthought, she adds \"<i>... please.</i>\"");

		MainScreen.text("\n\nYou move your hands to her smooth skin, cupping a ");
		if (sheilaCorruption() < 30) MainScreen.text("petit ");
		else if (sheilaCorruption() > 70) MainScreen.text("massive ");
		...
		if (player.lowerBody.isTaur()) MainScreen.text("pulls your legs apart");
		else MainScreen.text("unfastens and strips off your [armor]");
		MainScreen.text(", revealing your " + CockDescriptor.describeMultiCockShort(player) + "; ");
		//[(if no cock fit 32)
		if (player.cockThatFits(32) < 0) {
			MainScreen.text("her eyes widen as " + CockDescriptor.describeMultiCockSimpleOne(player) + " bobs free, half-erect monster meat that fills her with frustration.");
			...
			//[(horse)
			if (player.lowerBody.isTaur()) MainScreen.text(", or at least as flat as you can get it");
			MainScreen.text(", then sits on your stomach");
			if (player.lowerBody.isTaur()) MainScreen.text(", sliding her long legs between yours");
			MainScreen.text(".  She scoots forward slowly, and gasps as her pussy comes to rest against your hot, blood-engorged shaft.");
			...
			if (player.lowerBody.isTaur()) MainScreen.text("forelegs fold over her torso affectionately");
			else MainScreen.text("hands search out her breasts, caressing and squeezing");
			MainScreen.text(" as her gasps and the picture in your head both intensify.  Sheila grabs one with her own hand, pulling it up to her mouth to smother it in kisses");
			if (!player.lowerBody.isTaur()) MainScreen.text("and suck on your fingers");
			MainScreen.text(".  \"<i>Oh... [name]...</i>\"");
			...
			MainScreen.text("Sheila shivers and leans back, hair dangling, as her hips buck and her thighs squeeze your shaft.  ");
			//(PC corr >=90 or > sheila corruption)
			if (player.stats.cor >= 90 || sheilaCorruption() < player.stats.cor) {
				MainScreen.text("You can feel your prick heating up where her pussy rubs against it... it almost feels like the rest is cooling down at the same time.  The woman atop you arches her back and sucks in her breath sharply, and her tits seem to claim your field of vision, swelling up - though that could just be the angle.");
			}
			else if (player.stats.cor < sheilaCorruption()) MainScreen.text("Her orgasm wets your prick, coating it with fluid as she comes.  Sheila wraps herself tighter in your limbs, squeezing her breasts against her chest, and you think you can feel them get a little smaller as your head fuzzes over with her excess emotion.");
			else MainScreen.text("Her hips lock up when she comes, leaving you to try and finish yourself as she heats up around your shaft.");

			//[(sens >=50)
			if (player.stats.sens >= 50) {
				MainScreen.text("\n\nThe warm, smooth, wetness of her thighs is enough to bring out your own climax, and " + CockDescriptor.describeMultiCockSimpleOne(player) + " unburdens itself, forcing globs of semen into the air to rain down on your entwined bodies.  Sheila laughs as you coat her midriff with your mess, and playfully strokes your sensitized [cock] with one hand, sending shivers through you as she helps force your cum up your shaft.");
				//[(mega skeet)
				if (player.cumQ() >= 2000) MainScreen.text("  The giggles and the strokes stop long before your orgasm, though; eventually Sheila just lies there, dumbfounded, as you deposit layer after layer of white lattice on her skin.  \"<i>Bloody hell,</i>\" she mumbles vaguely, either stone-sober or drunk from the sight of it.");
				MainScreen.text("  Once the rain of semen peters out, she pulls apart from you, slimy and dripping.");
		...
		//(else if cock fit 32)
			...
			MainScreen.text("as you lean over to catch her expression, you can see her eyes glimmer with desire.  Sheila notices you from her peripheral vision, and her face reddens again.  She turns her back to you.");
			...
			//[(multicock)]
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("separating out and ");
			MainScreen.text("stroking your [cockFit 32].  A drop of precum forms, and she touches it with the index finger of her free hand, sending a tingle through you; she pulls the finger away, drawing a string of slick moisture out.  Sheila passes the finger between her breasts, wiping the fluid away on her chest and shivering.  The display is too much for you, and you reach forward, navigating your hands around her waist quietly until you can reach the buckle of her belt.  The girl sighs as you begin to unfasten it, but an errant thought brings her back to reality - she grabs your hands abruptly.");
			...
			//(else lib >= 25)
				...
				if (player.lowerBody.isTaur()) MainScreen.text(", pushing your legs apart");
				MainScreen.text("; looking into your face, her frown evolves into a mischievous grin.  Sheila about-faces, turning her back to you.  \"<i>Blame yourself for this; it's rude to stare,</i>\" she giggles, hovering over your [cockHeadFit 32].  The woman sinks down with an audible sigh, parting her slick, smooth labia with your tip and swallowing your " + num2Text(Math.round(player.lowerBody.cockSpot.list[player.cockThatFits(32)].cockLength)) + " inches.  \"<i>Ohh...</i>\" Sheila moans.  She slips a hand down to your crotch to check how much of your prick is left to go, and starts a bit when she realizes she's taken the entire shaft on the first stroke!");
				...
				if (!player.lowerBody.isTaur()) MainScreen.text(", and your hands go to her waist, fairly pushing her up and slamming her back down in your eagerness");
				MainScreen.text(".  She rolls her head as the fuck gets wetter and faster, dangling her hair over her shoulders as her back muscles shift and flex with her strokes; her ears flick whenever she descends and rubs your neck against one of the sweet spots on her vulva.");
				...
				//(PC corr >= 90 or > sheila corruption)
				if (player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) MainScreen.text("You can feel her cum-slick inner walls pulling at you as she begins to orgasm, and your mind clears as you blow your load inside her, wondering when she'll likewise return to sense.");
				//(else PC corr < sheila corruption)
				else if (player.stats.cor < sheilaCorruption()) MainScreen.text("Her pussy gushes with her orgasm, pushing out your semen and soaking her inner lips even as she continues riding your cock, and your mind clouds as you think of little except sneaking as much of your cum inside her as possible.");
				//else equal but < 90)
				else MainScreen.text("Even as she comes mindlessly something makes you hold in your own moans, wondering how long it will take her to notice her pussy being filled with your egg-hunting sperm.");
				//[(small cum)]
				if (player.cumQ() <= 250) {
					MainScreen.text("  Sheila continues moving her hips even as she comes, lost in bliss; on one particularly lofty ascent, your cock flops out, squirting a line of jizz along ");
					if (player.lowerBody.tailType > TailType.NONE) MainScreen.text("your tail");
					else MainScreen.text("the grass");
					MainScreen.text(", and her eyes go wide.");
				}
				//(big cum)
				else {
					MainScreen.text("  With how much Sheila is still moving her hips, it's only a matter of time before she notices the mounting pressure in her cunt... on one particularly deep push, she forces out a squirt of white, and her eyes go wide as she pulls off and lets you finish on the grass.");
				}
				...
				MainScreen.text(" you groan.  \"<i>Can't believe you didn't notice...</i>\"");
		...
		sheilaCorruptionUpdate();
		/*if(player.stats.cor > sheilaCorruption() || player.stats.cor >= 90) {
			player.stats.cor += -10;
			sheilaCorruption(10);
		}
		else if(sheilaCorruption() > player.stats.cor) {
			player.stats.cor += 10;
			sheilaCorruption(-10);
		}*/
	}
	...
	private sheilaForcedOralGeneric(): void {
		MainScreen.clearText();
		MainScreen.text("Sheila kisses you once more, forcing her tongue into your mouth, and slides a hand into your [armor]; you stare wonderingly into her affectionate, hungry eyes at the sudden change.  It isn't until ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("her hand slips around your [cock]");
		else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("her fingers slip between your labia");
		else MainScreen.text("she rubs your bare crotch for a few seconds");
		MainScreen.text(" that her reverie breaks; her face colors and she looks away, averting her gaze in shame.  She draws her hand back out guiltily, then begins to doff her shorts and panties.  \"<i>S-sorry... didn't mean to get familiar.  I'll have this over with quickly.</i>\"");

		MainScreen.text("\n\n\"<i>That's-</i>\"  Your words are cut off as she suddenly climbs atop you, bringing her pussy to eye level");
		if (player.stats.str < 61) MainScreen.text("; her weight bears you slowly to the ground");
		MainScreen.text(".  Sheila rubs the smooth slit on your [face], trying to part her labia on you, and shivers a bit when your lips brush hers.");
		...
		if (player.upperBody.head.hairType == 4) MainScreen.text(" and flushing as your stingers release their payloads into her palm");
		MainScreen.text(".  You obediently stick out your tongue");
		if (player.stats.cor > 60) MainScreen.text(", motivated more by whimsy than the meager, almost playful pain,");
		MainScreen.text(" and shallowly penetrate Sheila's vagina with it.");
		...
		if (player.upperBody.head.face.tongueType > TongueType.HUMAN) {
			MainScreen.text("  A sly look crosses your face at her request; she notices it but has no time to dismount before your long tongue shoots into her, stroking the nerves around her entrance as you unspool.  \"<i>Ah-ahhh!</i>\" she moans, \"<i>What the hell... is with your mouth?</i>\"");
		...
		//[(PC corr >= 90 or > sheila corruption)]
		if (player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) {
			MainScreen.text("strangling your tongue with muscles attempting to lock onto it, trying to pull it in.");
			//[(demon/dragon tongue)
			if (player.upperBody.head.face.tongueType > TongueType.HUMAN) MainScreen.text("  As tired as you are, you simply go along with it, allowing her rippling walls to slowly draw your tip deeper inside until it bumps against her cervix; she shivers and kisses the top of your head as you groggily run your tongue over the entrance to her womb.");
			else MainScreen.text("  However, it doesn't get very far; you lazily flop it around inside her as she works through her arousal.");
			MainScreen.text("  Sheila groans loudly and your vision fades; through the fog, you can swear you feel her breasts growing larger against your forehead.");
		}
		//(else PC corr < sheila corruption)
		else if (player.stats.cor < sheilaCorruption()) {
			MainScreen.text("filling your mouth with her liquid love.  Your lips and throat heat up as you're forced to drink it from the nearness of her pussy; as she bucks her hips weakly into your face, trying to draw out her climax, you increasingly wish that you could return the favor, ");
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("face-fucking her and blowing a load into her mouth, then holding her jaw shut until she swallows it unhappily.");
			else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("smearing your gash on her face until you come and drown her in your squirting girl-jizz.");
			else MainScreen.text("forcing her to tongue your asshole until you come, writhing and pressing her head into the grass with your [butt].");
		}
		else MainScreen.text("and she pets your head affectionately as her walls squeeze your tongue.  \"<i>Oh, mate...</i>\" she whispers lovingly, completely forgetting the struggle that led to your tryst.");
		...
		//[(str < 61)
		if (player.stats.str < 61) MainScreen.text("climbing off and kneeling next to you.");
		else MainScreen.text("letting herself down your body until she can put her long legs on the ground once more.");
		MainScreen.text("  She looks into your eyes contentedly, rubbing your cheek... but then starts and looks away, guilty once more.  \"<i>Shit...</i>\" she mumbles.  \"<i>I did it again.  Even after I promised myself I wouldn't...</i>\"");
		...
		if (player.upperBody.head.face.tongueType > TongueType.HUMAN) fatigue(15);
		dynStats("lus", player.stats.lib / 3 + 30, "resisted", false);
	...
	private normalSheilaPregNotifNumberOne(): void {
		...
		MainScreen.clearText();
		MainScreen.text("Traipsing through the grass, you can see Sheila sitting under the shade of a low tree from a long way off.  The reverse is also true, as she waves to you from her seat, beckoning you over.  You make your way to her curiously.");
		...
		if (player.stats.cor < 50) MainScreen.text("  A hand placed around her shoulder seems to steady her a bit.");
		...
		MainScreen.addButton(0, "Yep, Issue", normalSheilaPregNotifNumberOneYepIssue);
		MainScreen.addButton(1, "Nah", normalSheilaPregNotifNumberOneCoolDeal);
	...
	private normalSheilaPregNotifNumberOneYepIssue(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>I see,</i>\" Sheila says, nonplussed.  \"<i>I'm sorry you feel that way...  Truth be told, I'm not keen myself on the idea of having a kid, but what's out is out.</i>\"");
		...
		Flags.list[FlagEnum.SHEILA_DISABLED] = 2;
		Flags.list[FlagEnum.SHEILA_JOEYS]++;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private normalSheilaPregNotifNumberOneCoolDeal(): void {
		...
		if (Flags.list[FlagEnum.SHEILA_XP] < 4) Flags.list[FlagEnum.SHEILA_XP] = 4;
		Flags.list[FlagEnum.SHEILA_JOEYS]++;
		doNext(camp.returnToCampUseOneHour);
	...
	private normalSheilaPregNotifREPEATEDED(): void {
		MainScreen.clearText();
		MainScreen.text("Sheila is here as you step into the plains, with a grass-wrapped parcel of food open on her lap and her hat sitting upturned next to her, and waves you over.");
		...
		if (rand(10) < 7) MainScreen.text("healthy.  It's eating a lot... frankly, it's a little hellraiser with a strong grip, and it cries well, too.</i>\"");
		else MainScreen.text("weak... we thought we were going to lose it for a bit.  It wouldn't drink from the nurse unless I was there, so I had to stay up with it all night myself.  Feeling a bit stuffed.</i>\"");
		MainScreen.text("  She looks over at you, giving you the benefit of her fatigue-lined eyes.");

		MainScreen.text("\n\nSheila slips her hand into yours");
		if (player.stats.cor < 40) MainScreen.text(", and you give it a squeeze");
		MainScreen.text(".  \"<i>I just wanted to tell you about your baby.  Can't stay... have to catch up to quota still.</i>\"  She drags to her feet and turns to go, tail listlessly hanging.  Even her ears are drooping; this girl is tired.");

		pregnancy.knockUpForce(); //Clear Pregnancy
		Flags.list[FlagEnum.SHEILA_JOEYS]++;
		if (sheilaCorruption() > 80) Flags.list[FlagEnum.SHEILA_CORRUPTION] = 80;
		menu();
		//[Help(requires >80 speed, centaur >= 5', naga, or big wings and archery)][Walk With Her][Let Her Go]
		if ((player.stats.spe > 80 && player.perks.has("Runner")) || (player.lowerBody.isTaur()) || player.lowerBody.isNaga() || (player.canFly && player.hasKeyItem("Bow") >= 0))
			MainScreen.addButton(0, "Help", normalSheilaPregNotifREPEATEDEDHelpABitchOut);
		MainScreen.addButton(1, "WalkWithHer", normalSheilaPregNotifREPEATEDEDWalkWithHer);
		MainScreen.addButton(2, "Let Her Go", normalSheilaPregNotifREPEATEDEDLetHerGo);
	}

	//[Normal Preg Subsequent - Let Her Go]
	private normalSheilaPregNotifREPEATEDEDLetHerGo(): void {
		MainScreen.clearText();
		MainScreen.text("Nothing you can do about that - even if you wanted to watch your baby for her, she's keeping it hidden in a hole in the ground.  You hand her the parcel back");
		if (player.stats.cor < 40) MainScreen.text(", imploring her to eat the rest and keep up her strength");
		MainScreen.text(".  Sheila smiles weakly and takes it, folding the grass around again.  \"<i>Thanks, [name].  I'll see you soon... ish.</i>\"");
	...
	private normalSheilaPregNotifREPEATEDEDWalkWithHer(): void {
		MainScreen.clearText();
		MainScreen.text("You pick up her forgotten hat and rise as well, carefully refolding the grass around Sheila's uneaten lunch and tucking it back into her pack for her.  She takes her hat back from you distantly.");

		MainScreen.text("\n\n\"<i>Thanks, mate... forget my own head next.</i>\"  She doesn't even put it on, simply holds it in her hand as she plods on.  You ");
		if (!player.lowerBody.isNaga()) MainScreen.text("walk");
		else MainScreen.text("slither");
		MainScreen.text(" next to her, easily keeping pace with the tired woman, and she raises her face to you inquiringly.  \"<i>What's up, [name]?</i>\"");
	...
	private normalSheilaPregNotifREPEATEDEDHelpABitchOut(): void {
		MainScreen.clearText();
		MainScreen.text("You pick up her forgotten hat and set it on her head, then fold her lunch back into its grass wrapper and tuck it into her pack.");
		...
		//(else spd >= 80 and Runner perk)
		else if (player.stats.spe >= 80 && player.perks.has("Runner")) {
			...
			if (player.stats.lib < 50) MainScreen.text("you pinch her ear, provoking an annoyed flick from it.");
			else MainScreen.text("you slap her on the ass, grabbing a handful and taking her mind off her own misery.");
			MainScreen.text("  \"<i>Dammit, [name]... not now,</i>\" she frowns, pulling away from you.");
			...
			if (!player.lowerBody.isGoo()) MainScreen.text("run");
			else MainScreen.text("glide");
			MainScreen.text(", making a long loop around to the rear of the wary animal, then begin to systematically drive it closer to your partner, who has strung a line to the base of a shrub and is gesturing at you to force the hare into it.");
		...
		if (model.time.hours + 4 < 21) doNext(camp.returnToCampUseFourHours);
			...
			menu();
			MainScreen.addButton(0, "StayWithHer", normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE);
			if (!(Flags.list[FlagEnum.FUCK_FLOWER_LEVEL] >= 4 && Flags.list[FlagEnum.FUCK_FLOWER_KILLED] == 0) && !vapulaSlave())
				MainScreen.addButton(1, "To Camp", normalSheilaPregNotifREPEATEDEDHelpABitchOutTOCAMP);
			MainScreen.addButton(2, "Welp", normalSheilaPregNotifREPEATEDEDHelpABitchOutANDWELP);
		}
	}
	...
	private normalSheilaPregNotifREPEATEDEDHelpABitchOutANDWELP(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>Welp,</i>\" you say softly.");
		//[(corr <50)
		if (player.stats.cor < 50) MainScreen.text("  You set a hand on Sheila's shoulder to steady her a bit as the tears start to fall.");
		MainScreen.text("  \"<i>It'll be alright.</i>\"");
	...
	private normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE(): void {
		MainScreen.clearText();
		MainScreen.text("With a wan smile, you pull the woman to her feet.  She doesn't resist, but continues looking into your face disconsolately.  \"<i>I'm so stonkered I can't even think what to do first...</i>\"");
		...
		if (player.lowerBody.isNaga()) MainScreen.text("coil");
		else MainScreen.text("seat");
		MainScreen.text(" yourself, then grabs your arms and pulls them around her waist.");

		//forks in order of priority, no new pg at start
		//[(full gooey body)
		if (player.lowerBody.isGoo()) {
			MainScreen.text("  She considers for a moment, rubbing away some of the slime you've deposited on her shorts, and pulls free from your slimy skin again.  \"<i>Um... do you... would you be offended if I took my kit off, so it doesn't get ruined?</i>\"");
			if (player.stats.lib >= 40) MainScreen.text("  The leer you display is so prurient that she seems on the verge of reconsidering, but you compose yourself and");
			else MainScreen.text("  You");
			MainScreen.text(" give assent with little more than a broad smile.  Sheila pulls her shirt over her head, revealing her " + sheilaCup() + " breasts, but raises her face to you hesitantly when her hands get to her belt.");
			...
			dynStats("lus", player.stats.lib / 5 + player.stats.sens / 3, "resisted", false);
		}
		//(naga)
		else if (player.lowerBody.isNaga()) {
			...
			if (player.stats.lib >= 40) MainScreen.text(" - but tighten the one sliding across her breasts just slightly.  Sheila's neck turns red and her ears twitch as you rub her nipples, and you plant a kiss on her nape");
			MainScreen.text(".  \"<i>Ahh,</i>\" she sighs, shifting in your coils.  \"<i>Hold it right there...</i>\"");
			//(apply libido-based lust damage if lib >= 40)
			if (player.stats.lib >= 40) dynStats("lus", player.stats.lib / 4, "resisted", false);
		}
		//(horse and >=5ft)
		else if (player.lowerBody.isTaur() && player.tallness >= 60) {
			...
			if (player.stats.sens >= 40) MainScreen.text("... or would be, if you weren't distracted by the nipples poking into you and the thighs squeezing your flank");
			MainScreen.text(".");
			//(apply sens-based lust damage if sens >=40)
			if (player.stats.sens >= 40) dynStats("lus", player.stats.sens / 4, "resisted", false);
		}
		//(harpy - minimum score for birdlegs at least)
		else if (player.lowerBody == LowerBodyType.HARPY && player.lowerBody.tailType == TailType.HARPY) {
		...
		//(eight-legged freaks)
			...
			if (player.lowerBody.tailVenom < 0) player.lowerBody.tailVenom = 1;
		}
		//(else)
		...
		if (!player.lowerBody.isGoo()) MainScreen.text("blood");
		else MainScreen.text("goo");
		MainScreen.text(" flowing, unsure if you'll be able to get to sleep, but Sheila has no such problems.  Within minutes, the woman is out, head nodding forward.  You suppose that means the first watch is yours... it passes slowly, despite the scenery, but eventually you nudge your partner awake when you can stay up no longer.");

		MainScreen.text("\n\n\"<i>Mmm... lemme sleep for fifteen minutes more, mate,</i>\" she mumbles.");

		//(corr <60)
		if (player.stats.cor < 60) MainScreen.text("\n\nYou nearly consider it, tired as you are, until you remember that a sleeping person's fifteen minutes are two hours in the waking world and shake her again until she gets up.");
		else {
		...
		if (!player.lowerBody.isTaur()) MainScreen.text("  She grudgingly repositions, allowing you to rest against her.");
		model.time.days++;
		model.time.hours = 2;
		statScreenRefresh();
		//--Next--
		menu();
		MainScreen.addButton(0, "Next", normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE2);
	}
	...
	private normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE2(): void {
		...
		model.time.hours = 5;
		statScreenRefresh();
		MainScreen.text("Your sleep is fitful, but not totally useless, and you yawn and stir a few hours later when Sheila wakes you");
		if (player.lowerBody.isGoo()) MainScreen.text("; she's already dressed and appears to have just been outside");
		MainScreen.text(".");
		...
		if (player.stats.lust < 30) MainScreen.text("  You give her a hand, then help her load up.  She gives you a peck on the cheek before she departs.  \"<i>Thanks, love.</i>\"");
		if (player.stats.lust >= 30) {
			MainScreen.text("  Part of you is actually sad to see your cozy hovel being disassembled if it means her warm body won't be pressed up against yours anymore, and your hands begin to rove over her as she works.");
			...
			menu();
			sheilaXP4Sex(false);
			MainScreen.addButton(9, "LeaveHerBe", normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDEREBUTLEAVEHERBE);
		}
	...
	private normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDEREBUTLEAVEHERBE(): void {
		MainScreen.clearText();
		MainScreen.text("You relent, allowing the woman to free herself with a peck on your cheek.  She loads up her catch, and then turns to you.  \"<i>Thanks, [name].  Owe you one.</i>\"");
	...
	private normalSheilaPregNotifREPEATEDEDHelpABitchOutTOCAMP(): void {
		MainScreen.clearText();
		MainScreen.text("Wordlessly, you lift the bundle of game animals off of Sheila's shoulders and sling it over your own.  She looks up at you, confused enough to forget about her tears.  \"<i>What're you doing?</i>\"");
		...
		//[(int >=60)]
		if (player.stats.int >= 60) MainScreen.text("\"<i>The baby will be fine,</i>\" you venture, hazarding a guess.  \"<i>It's what the nurses are there for.</i>\"");
		else MainScreen.text("You slip an arm around her, and ask if she's okay.");
		MainScreen.text("  She nods mutely and pulls the food out of the fire.");
		//[(if Jojo present)
		if (player.statusAffects.has("JojoNightWatch") || player.statusAffects.has("PureCampJojo")) MainScreen.text("  She extends one bird to you and another to Jojo.  He raises his hand and shakes his head, but politely.");
		else if (Flags.list[FlagEnum.ANEMONE_WATCH] > 0) MainScreen.text("  Sheila offers you one of the birds, then extends another to your odd tenant.  The anemone's mouth tightens and she shakes her head vigorously, blushing a deep blue.");
		MainScreen.text("  You eat in silence until the food is gone, and Sheila clears her throat.");
		...
		//[(if no watch)
		if (!player.statusAffects.has("JojoNightWatch") || !player.statusAffects.has("PureCampJojo") || Flags.list[FlagEnum.ANEMONE_WATCH] == 0) MainScreen.text("\n\nShe gets to her feet as you grin, then makes her way over to your bedroll.  It doesn't take her long to fall asleep; you amuse yourself watching her toss and turn... and drool on your pillow.");
		//(if watch)
		else {
			MainScreen.text("\n\nYou grin and get up, and Sheila tails you to your bed.  ");
			if (Flags.list[FlagEnum.SLEEP_WITH] != 0 && Flags.list[FlagEnum.SLEEP_WITH] != "") MainScreen.text("She looks with some distaste at the nearby sleeper, and steps past.  You pick up the bedroll and follow her, reasoning that if she keeps her promise to be gone before dawn, it shouldn't raise any questions.  \"<i>Making friends, I see,</i>\" Sheila mutters... but as you unroll the bed again, her eyes zero in on it like a drowning man's on a raft, and she says nothing else.  ");
			MainScreen.text("She lies down first, allowing you to cozy up to her as far as you dare.  Just as you're about to drift off, you feel Sheila take your hand and pull your arm over her body.");
		}
	...
	private sheilaGoesDemon(): void {
		...
		if (Flags.list[FlagEnum.SHEILA_XP] < -2) MainScreen.text("always so bloody eager to fuck me!");
		else MainScreen.text("me best mate!");
		MainScreen.text("  Help out!</i>\"");
	...
	private demonSheilaEncounter(): void {
		MainScreen.clearText();
		MainScreen.text("A shadow drops out of a low tree behind you with a rustle.  \"<i>Hey, lover; been a while... how about spending some time with me?</i>\"");
		...
		menu();
		MainScreen.addButton(0, "Talk", demonSheilaTalk);
		if (player.stats.lust >= 33) MainScreen.addButton(1, "LetHerFuck", sheilaLetHerFuckYou);
		MainScreen.addButton(2, "Resist", demonSheilaResist);
		//run simply returns to camp and should be the default spacebar option
		MainScreen.addButton(4, "Run", camp.returnToCampUseOneHour);
	}

	//[Demon Sheila - Resist]
	private demonSheilaResist(): void {
		MainScreen.clearText();
		MainScreen.text("You back away from the demon and raise your [weapon] with a dour glower.  Sheila purses her lips at you, then says, \"<i>Not in the mood, eh mate?  I'm so horny though... I bet I could get you into it.  Let me show you how much I love you...</i>\"  Her ears twitch furiously, belying her intentions as she");
		//[(sheila corruption < 30)
		if (sheilaCorruption() < 30) MainScreen.text(" thrusts her black spade forward and smears some of the constantly-oozing fluid from its tip down the length with her hand");
		else MainScreen.text(" kneads her breasts seductively, pinching her nipples");
		MainScreen.text(".  Your posture softens a bit as she reaches out to you");
		if (sheilaCorruption() < 30) MainScreen.text(", tilting her palm to dribble a handful of strong-scented dark oils");
		MainScreen.text(", but as she giggles sadistically and starts to approach, you tense up again.  It's pretty clear by the way she swaggers dismissively that she isn't taking the rejection well!");
		//go to fight
		startCombat(new Sheila());
	}
	...
	private demonSheilaTalk(): void {
		MainScreen.clearText();
		MainScreen.text("Questions nag at you, and you ask Sheila if she wouldn't mind answering a few of them.  She struts over and ");
		//[(not horse)
		if (!player.lowerBody.isTaur()) MainScreen.text("takes your arm in hers, then slips the other hand into your armor");
		//[(horse)
		else MainScreen.text("between your legs");
		MainScreen.text(" to touch your ");
		...
		else MainScreen.text("smooth skin");
		//[(lib <20)
		if (player.stats.lib < 20) MainScreen.text(", giggling as your expression petrifies");
		MainScreen.text(".  \"<i>Anything for you; I'll just find something to occupy my attention.  What's on your mind, love?</i>\"");
		//plus lust depending on sens
		dynStats("lus", 2 + player.stats.sens / 5, "resisted", false);
		//[The Change][Her Colony][What Now?][Fuck Off]
		menu();
		MainScreen.addButton(0, "The Change", askDemonSheilaAboutDahChange);
		MainScreen.addButton(1, "Her Colony", askDemonSheilaAboutHerColony);
		MainScreen.addButton(2, "What Now?", talkToDemonSheilaAboutWhatNow);
		MainScreen.addButton(3, "Fuck Off", tellSheilaDemonToFuckOff);
	}

	//[ Demon Sheila - Talk - Fuck Off]
	private tellSheilaDemonToFuckOff(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>Oh,</i>\" she says.  \"<i>Don't be like that.  You're my best mate, you know.  My beloved.  I'll never let you go, ever.</i>\"  Her eyes gloss over a bit, ");
		//[(RNG decides if dis bitch crazy, bias toward sane)
		if (rand(10) < 3) {
			MainScreen.text("and she blurts, \"<i>I know I can make you understand.  You can't go until you say it back!</i>\"  You pull her hand free and back away from her, but she advances again, trying to grab you!");
			startCombat(new Sheila());
			return;
		}
		else {
			MainScreen.text("but she remasters herself with effort.  \"<i>No worries.  I'm sorry to hear you don't have any time for love, my special one.  I'll be here until you do, thinking about you and touching myself.</i>\"  She releases you and steps away, then crudely shoves her spade through her thighs, grabs it, and half-moans as she thrusts her pelvis back and forth, jerking the thick black flesh in her hand.  With a wink, she abruptly releases it, then turns her back and departs.");
			doNext(camp.returnToCampUseOneHour);
		}
	...
	private askDemonSheilaAboutDahChange(): void {
		MainScreen.clearText();
		//[(if sheilacite >= 1)]
		if (Flags.list[FlagEnum.SHEILA_CITE] >= 1) {
			MainScreen.text("Sheila blushes deeply.  \"<i>Silly.  You were there.</i>\"  She looks at you, pupils dancing as she remembers the lovemaking session that led to her current form.  \"<i>I suppose that's not what you're asking, though.</i>\"");
			...
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(", and you feel a monument of your own being erected");
			MainScreen.text(".  \"<i>Every squeeze of my pussy compressed my focus, too, until my mind was clearer than the juice you made me flow with.  I realized my purpose there, with you.</i>\"");
		...
		player.stats.lust += 12;
		player.stats.resisted += false;
		menu();
		MainScreen.addButton(0, "Another Q.", demonSheilaTalkAnotherQuestion);
		if (player.stats.lust >= 33) MainScreen.addButton(1, "LetHerFuck", sheilaLetHerFuckYou);
		MainScreen.addButton(2, "Resist", demonSheilaResist);
	}
	...
	private askDemonSheilaAboutHerColony(): void {
		...
		MainScreen.text(", you ask what happened to him.  Sheila smiles serenely before answering.  \"<i>Unsurprisingly, his prick was all too eager to fill me with sperm after the countless sermons he preached about it being every woman's duty to expand the clan.  He barely lasted three minutes when I climbed atop him.  Oh, that little squirt was the most satisfying one.  Watching him melt under me drove home how much power I've always had... I wish I could have known when I was just an awkward little girl, sitting through his browbeating lectures on loyalty and servitude.</i>\"");
		...
		//(corr >=75)
		if (player.stats.cor >= 75) MainScreen.text("harvested that day, you sigh wistfully");
		else MainScreen.text("stolen that day, you wince");
		MainScreen.text(".  Sheila, peeking at you with one eye from under her bangs, interprets your expression correctly.  \"<i>Wondering where the others went, love?</i>\"  She releases her grip on your arm and holds out the hand that's not exploring your [armor], displaying several rings.  \"<i>The rings are my closest.  I kept them for sentimental reasons.  The rest, well... I ate them.</i>\"");
		...
		//[(cock)
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("she ties you down and rides you until she's so full of your spunk that she can push it back out just by pressing on her stomach");
		else MainScreen.text("she binds your hands and sits on you, teasing your helpless body with her oozing, throbbing spade until she comes all over you, spraying dark slimy fluid from it");
		MainScreen.text(".  You ");
		if (player.stats.cor < 50) MainScreen.text("break eye contact, blushing");
		else MainScreen.text("stare into her shining eyes, getting hotter and hotter as the vision plays out to the very end");
		MainScreen.text(".");
		...
		dynStats("lus", player.stats.lib / 3 + 5, "resisted", false);
		menu();
	...
	private talkToDemonSheilaAboutWhatNow(): void {
		...
		MainScreen.text("\"<i>What will I do now?  What else, my silly love?</i>\"  Sheila looks out over the plains.  \"<i>I'm always hot for you.  We could have a bit of a naughty, and make each other feel good.</i>\"");
		...
		//(lib>=30)
		if (player.stats.lib >= 30) MainScreen.text("\n\nYour eyebrows rise.  \"<i>Got your attention, did I?  But,</i>\"");
		else MainScreen.text("\n\nYour mouth hardens.  \"<i>You're right,</i>\"");
		MainScreen.text(" Sheila says, \"<i>I'm going to save that one for our honeymoon, instead.</i>\"  Her eyes twinkle with infatuation.  \"<i>I could even pull the taint of this land out of your body and use it to make mine more lewd for you, my " + player.mf("knight-champion", "great lady") + "...</i>\"");
		...
		if (player.stats.cor < 50) MainScreen.text("; even though you hide it, I've seen it lurking");
		MainScreen.text(".  Feel like flying your colors for a while?</i>\"");

		//plus libido-based lust damage, output choices [Another Question][Let Her Fuck(req lust >=30)][Resist]
		dynStats("lus", player.stats.lib / 15, "resisted", false);
		menu();
		MainScreen.addButton(0, "Another Q.", demonSheilaTalkAnotherQuestion);
		if (player.stats.lust >= 33) MainScreen.addButton(1, "LetHerFuck", sheilaLetHerFuckYou);
		MainScreen.addButton(2, "Resist", demonSheilaResist);
	...
	private demonSheilaTalkAnotherQuestion(): void {
		...
		if (player.stats.lust <= 99) {
			MainScreen.text("Sheila reaches farther down, ");
			//(cock)
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("slipping more of her finger and thumb around your penis");
			else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("parting your lips more");
			else MainScreen.text("rubbing your bare groin");
			MainScreen.text(" even as she smiles encouragingly.  \"<i>Sure, mate.  Amuse yourself.  I certainly intend to.</i>\"");
	...
	private sheilaLetHerFuckYou(): void {
		...
		if (sheilaCorruption() >= 40) MainScreen.text("pillowy ");
		else MainScreen.text("pert ");
		MainScreen.text("breasts against you.");
	...
	private demonImpChildren(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>[name]!</i>\"  A swishing in the grass behind you is the only prelude you get to a nude Sheila pressing herself against you in a hug, ");
		//(sheila corruption < 40)
		if (sheilaCorruption() < 40) MainScreen.text("her perky nipples rubbing into your arm as she snuggles up");
		else MainScreen.text("her enormous, pendulous breasts practically parting around you to wrap you up alongside her arms");
		MainScreen.text(".");
		...
		//(if no cock)
		if (!player.lowerBody.cockSpot.hasCock()) MainScreen.text("  Or, had...</i>\" she finishes with a frown as she reaches your bare groin.  \"<i>Where'd you leave that magnificent old fella?  I was lookin' forward to another go.</i>\"  She pulls her hand out, then leans toward you with a pout.  \"<i>I'm so horny, too...</i>\"");
		//(if cock)
		else {
			MainScreen.text("  Still, it's not the same as having you inside me...</i>\"  She squeezes the neck of your [cock] and then pulls her hand away, slipping it between her own legs.  \"<i>I was thinking you could help me make another, and maybe up the population of men worth a damn around here.</i>\"  Reclining, she spreads her thighs, pulling apart her small, smooth labia ");
			//[(sheila corruption <=80)]
			if (sheilaCorruption() <= 80) MainScreen.text("with her fingers to make sure the invitation is all the way open");
			//(else)
			else MainScreen.text("with her split tail while she cradles her massive tits to make sure you see the open invitation");
			MainScreen.text(".  Impatiently, she rubs her stubby, drooling, phallic spade against the hole, accentuating the imagery.  You can feel the blood rushing to your groin as she gives herself shallow, teasing strokes.");
		}
		pregnancy.knockUpForce(); //Clear Pregnancy
		Flags.list[FlagEnum.SHEILA_IMPS]++;
		//plus lust if PC has cock
		if (player.lowerBody.cockSpot.hasCock()) dynStats("lus", 10 + player.stats.lib / 10, "resisted", false);
		//[Other Sex][Knock Up(cock only)][Leave]
		menu();
		MainScreen.addButton(0, "Other Sex", pregDemonSheilaOtherSex);
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(1, "Knock Up", pregDemonSheilaKnockUp);
		//leave should be default spacebar option
		MainScreen.addButton(4, "Leave", leavePregDemonSheila);
	}

	//[Preg Demon Sheila - Leave]
	private leavePregDemonSheila(): void {
		MainScreen.clearText();
		MainScreen.text("You shake your head at her, backing away.  She sulks a bit, then speaks.  \"<i>I guess I'll let you off, since you sexed me up so well before.  Next time, though, you're gonna give me some... satisfaction.</i>\"  She runs a finger teasingly down her chest, giving you half a smile.");
		//(if cock)
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  Your refusal doesn't stop her from consummating the affair herself by slipping the oozing spade past her entrance and thrusting vigorously.  \"<i>What?</i>\" she says, shaking you from your distracted stare.  \"<i>I'm always randy now, and you're not going to spend time with me, so... move along.  Maybe someone fun will find me and join in.</i>\"  She throws her head back as her slippery tail makes a particularly deep push, crying her pleasure carelessly to the glaring sun.  The wet schlicks of her self-love follow you until the magic of the realm brings you abruptly back to camp.");

		//plus more lust if cock, go to camp
		if (player.lowerBody.cockSpot.hasCock()) player.stats.lust += 10;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private pregDemonSheilaOtherSex(): void {
		MainScreen.clearText();
		MainScreen.text("Her eyebrows knit as you propose an alternative.  ");
		//[(if cock)
		if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text("\"<i>Well now, I think I get a say, don't I?  I was really hoping to have your seed inside me... it's practically worth bottling.</i>\"  She ");
			if (sheilaCorruption() >= 80) MainScreen.text("heaves onto her feet, heavy breasts swaying");
			else MainScreen.text("nimbly hops to her clawed feet");
			MainScreen.text(", and runs her hands distractingly along the curves of her hips.  \"<i>You can do what you like with my body, love... if you can convince me.  Of course, I'll claim the same privilege!</i>\"  Sheila jumps at you, leaving no doubt what kind of 'convincing' she expects to do!");
			startCombat(new Sheila());
		}
		//(if no cock)
		else if (player.lowerBody.vaginaSpot.hasVagina()) {
	...
	internal function loseToSheila(consensual: boolean = false): void {
		...
		if (player.HP < 1 && !consensual) {
			MainScreen.text("Your erstwhile opponent's eyes glimmer with excitement as you collapse from your injuries, and she runs over to you.  The demon strips off your [armor] eagerly, but you can't stay awake for the fun.  Consciousness slips away and you pass out.");
			//--Next--
			model.time.hours += 8;
			if (model.time.hours > 23) {
				model.time.hours -= 24;
			...
			menu();
			MainScreen.addButton(0, "Next", loseToDemonSheila);
		}
		else {
			let choices: Array = new Array();
			//Loss - normal cocks get rode (for cockarea <= 56)
			if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(56) >= 0)
				choices[choices.length] = loseToNormalSheilaAndGetRidden;
			//Loss - tail-spade sounding for fuckhueg cock (for cockarea > 56)
			if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 56)
	...
	private loseToNormalSheilaAndGetRidden(): void {
		MainScreen.clearText();
		let x: number = player.cockThatFits(56);
		MainScreen.text("Sheila pushes you onto the ground");
		if (player.lowerBody.isTaur()) MainScreen.text(" and you're forced to put your forelegs down for support as you come to rest on your haunches");
		else MainScreen.text(", spreading your legs.  The demon unfastens your [armor] and strips it from your bottoms easily");
		MainScreen.text(", exposing your " + CockDescriptor.describeMultiCockShort(player) + ".");

		MainScreen.text("\n\n");
		//[(multi)
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("\"<i>All of these look ripper, but... I want this one today,</i>\" she says, selecting your [cockFit 56].  ");
		MainScreen.text("She wraps her hand around it and begins pumping up and down, squeezing out a drop of precum that she smears onto her palm.  \"<i>You're so eager,</i>\" Sheila giggles.  \"<i>Are you already about to burst?  That's no good... I wanted to play a bit more.</i>\"");

		MainScreen.text("\n\nThe demon releases your dick, watching raptly as it bobs and twitches, then scoots up to you and");
		if (player.lowerBody.isTaur()) MainScreen.text(", draping your forelegs over her shoulders,");
		MainScreen.text(" presses her " + sheilaCup() + " breasts");
		if (sheilaCorruption() < 100) MainScreen.text(" against you, dragging her nipples against yours.  ");
		else MainScreen.text(" against you so forcefully that they part around your chest.  ");
		MainScreen.text("\"<i>Such a " + player.mf("handsome bloke", "pretty sheila") + ", with such a nice dong,</i>\" she whispers, \"<i>I'm going to have to take you for a ride.  Try not to come as soon as you get in!</i>\"");

		MainScreen.text("\n\nSheila grins viciously and sinks down on your [cockFit 56], sliding it into her pussy at a painstaking pace");
		//[(multi)
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" and slipping the unused remainder of your manhood between her thighs");
		MainScreen.text("; you can feel every new inch of heat as she lowers her body.  The woman moans, gyrating her hips, and " + CockDescriptor.describeMultiCockSimpleOne(player) + " twitches in painful erectness.");
		...
		if (player.lowerBody.balls > 0) {
			...
			//(not horse)
			if (!player.lowerBody.isTaur()) MainScreen.text("  Sheila takes your face in her hands and presses it into her breasts as she reassures you.");
			...
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" and lap");
			MainScreen.text(" just like she's dumping feelings into your head.  \"<i>Lover!</i>\" she screams.  \"<i>Pack me with your babies!</i>\"");
			...
			if (player.lowerBody.isTaur()) MainScreen.text("forelegs");
			else MainScreen.text("hands");
			MainScreen.text(", displacing a glob of cum from her pussy that's soon followed by others as you ejaculate once more.  The jizz is so dense with sperm that it's thick, almost colloidal; it jiggles on her skin, barely sliding downward as your hips ram against hers.  It takes nearly ");
			//(normal skeet)
			if (player.cumQ() < 250) MainScreen.text("ten minutes and two");
			//(big skeet)
			else if (player.cumQ() < 1000) MainScreen.text("thirty minutes and four");
			else MainScreen.text("fifty minutes and eight");
			MainScreen.text(" more orgasms, but finally your balls empty of their demonically-induced load and your lover pulls away and stands up.");
		...
		//(else if no balls)
			...
			if (player.lowerBody.isTaur()) MainScreen.text("You force her downward with your forelegs to");
			else MainScreen.text("You grab her hips and");
			MainScreen.text(" help her impale herself on your tool.");
			...
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" and lap");
			//[(big skeet)
			if (player.cumQ() >= 1000) MainScreen.text("; she quickly reaches capacity and pulls off, jerking your messy, slimy [cockFit 56] and letting you squirt the last of your strokes all over her face and chest");
			if (player.cumQ() >= 6000) MainScreen.text(".  So much is left that she's practically underwater by the time you're finished; she giggles and bubbles form in the oozing liquid covering her lips");
			MainScreen.text(".");
	...
	private tailSpadeSoundingForFuckHugeDongsWithDemonSheila(): void {
		...
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("; unguided, the ");
		if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("other twitches and sprays");
		else if (player.lowerBody.cockSpot.count() > 2) MainScreen.text("others twitch and spray");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" all over, coating her back, you, the grass, and anything ");
		if (player.lowerBody.cockSpot.count() > 2) MainScreen.text("else they happen to point at");
		if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("else it happens to point at");
		MainScreen.text(".  Your hips automatically try to push your dick further into her warm hole but only succeed in unbalancing her, forcing the demon onto her hands.  As your climax winds down, your head leans back and your cock slips from her vulva, squirting the last stroke onto her ass.");
	...
	private clitSwellingDemonSheilaClitSoundingAnal(): void {
		MainScreen.clearText();
		//(no horse)
		if (!player.lowerBody.isTaur()) MainScreen.text("Sheila places her hand in the small of your back and leans in, drawing closer to your face even as she pulls your waist inward.");
		else MainScreen.text("Sheila sets her hand atop your withers, pressing down gently as she looks into your eyes.");
		MainScreen.text("  \"<i>Why don't you relax, [name]?  Lie down and let me take care of you...</i>\"  Her eyelids lower and her mouth spreads in a sly smile, and for just a moment you lose your nerve, looking into her devilish expression.  \"<i>Don't worry, love,</i>\" she says, straightening her face.  \"<i>I promise you'll enjoy this.</i>\"");

		MainScreen.text("\n\nBefogged but still reserved, you nonetheless allow Sheila to lower you to the grass and remove your [armor].  She ");
		//[(not horse)
		if (!player.lowerBody.isTaur()) MainScreen.text("turns around, then straddles your groin and looks over her shoulder.");
		//(Equus ferus caballus)
		else MainScreen.text("circles you until she reaches your rump, then turns around and kneels down herself, looking at you over her shoulder.");
		MainScreen.text("  Her black, spaded tail lifts from between her thighs and snakes toward your pussy,");
		//[(if manparts)
		if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text(" caressing ");
			if (player.lowerBody.balls > 0) MainScreen.text("your [balls]");
			else MainScreen.text("the underside of your [cock biggest], before");
		}
		MainScreen.text(" forcing your lips apart and lingering on your rapidly-swelling " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + ".  \"<i>Oh, my.  What's with this ");
		if (player.lowerBody.vaginaSpot.get(0).clitLength <= 6) MainScreen.text("lewd little");
		else MainScreen.text("grotesque");
		MainScreen.text(" thing doing growing from a pretty girl like you?  It's just the perfect size for some fun.</i>\"");

		MainScreen.text("\n\nYour eyes open in alarm at the same time as her mouth does in pleasure; she jams the slit of her spade onto your button");
		if (player.lowerBody.vaginaSpot.get(0).clitLength > 6) MainScreen.text(", taking it several inches deep");
		MainScreen.text(".  Your head swims as pleasure from your engulfed clitoris overwhelms you, then toes the line to genuine pain.  The copious demonic fluid backing up inside her tail begins to leak around the tip, coating your clit and drooling onto your labia; your muscles tighten as the heat escalates past comfortable levels, and you begin shifting violently, trying to pull your swollen chick-stick free from the demon's anatomy.  \"<i>Ah, ah, god, [name], stop!  Don't thrash so!  You're teasing me too fast!</i>\"  Sheila's words are lost on you, sunken as you are into a hell of oversensitized skin and constant stimulation.  It feels like the nerves exposed to the torment are even multiplying, as if skin you weren't even feeling before suddenly appears to be immersed into the tainted slime.");
		MainScreen.text("\n\nYour eyes roll in temporary shock as her tail squeezes around your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " tighter and tighter; the siege lifts slightly, allowing you to focus with supreme effort and see that the demon is similarly affected!  Her head sags between her shoulders, even as she places two hands on the ground in front of her to steady herself.  The two tails on her butt whip spastically, squeezing another wave of painful pleasure into your crotch.  Staring, you can only watch as Sheila's thrashing black spade describes a clipped arc, a long, thick rod of flesh growing out of it to join your bodies.");

		//[(<=6")
		if (player.lowerBody.vaginaSpot.get(0).clitLength <= 6) MainScreen.text("\n\nAwareness comes to you slowly; that flesh is your clitoris, gratuitously mutated by Sheila's fluids!");
		else MainScreen.text("Though you're by now accustomed to the sight of your clitoris, more like a cock than a dainty feminine bump, this is something else altogether!  The huge thing poking into her tail is almost twice the thickness of the slim growth you remember and bulges with blood vessels; as you watch it, transfixed, a trickle of Sheila's oily, dark secretion slides down it, then is absorbed into some flesh near your groin, adding another layer of width with a pulse!");
		MainScreen.text("  \"<i>Fuck, it's so tight!</i>\" the demon gasps, now resting on her elbows with her chest and face pressed to the grass.  Her tail jerks again, yanking your stalk this way and that and releasing more of her backed-up fluids.  They slide all the way to the bottom of your rod before sinking into your skin.");
		...
		//(clit<=6" initially)
		if (player.lowerBody.vaginaSpot.get(0).clitLength <= 6) {
			MainScreen.text("\n\nShe sits up on her knees, then turns to face you.  ");
			if (!player.lowerBody.isTaur()) MainScreen.text("Taking your face into her hand, she closes your jaw and then leans in to plant a kiss on your chin.");
			else MainScreen.text("Her eyes glitter with mirth.");
			MainScreen.text("  \"<i>Why the look, mate?  Seems like you're saturated for now, and just in time.  This... is...</i>\"  She pulls her tail up from the end of your clitoris, then slides it back down, slowly.  \"<i>... perfect!</i>\"  As her tight hole swallows your sensitive distaff staff once again, your eyes roll and her head drops, dragging her hair");
			//[(horse)
			if (player.lowerBody.isTaur()) MainScreen.text(" through the grass");
			else if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text(" between your " + BreastDescriptor.describeAllBreasts(player));
			else MainScreen.text(" along your flat chest");
			MainScreen.text(".  \"<i>Ffuck, even your girl parts are amazing.</i>\"  Sheila plants kiss after kiss on your ");
			if (!player.lowerBody.isTaur()) MainScreen.text("[chest]");
			else MainScreen.text(ButtDescriptor.describeButt(player));
			MainScreen.text(" as she shallowly pumps your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " in and out of her spade.  Your pelvis twitches uselessly; with her tail waving in the air instead of braced against her, controlling the pace of the fuck is beyond you.  \"<i>Naughty sheila,</i>\" she says, squeezing your wiggling hip with a hand.  You think privately that she could just as easily be talking about herself.  \"<i>Am I not going fast enough for you?  Let me speed up.</i>\"");
			...
			//[(PC corr<50)
			if (player.stats.cor < 50) {
				MainScreen.text("her tail quakes and you feel something hot licking at the tip of your clitoris.  The spade pulls back and then sinks down again, and the heat gains several inches; she repeats this twice more until you can feel what must be her cum all the way down to her tail-tip, but as she attempts to pull back again and force her climax past the raw, hot skin at the edge, her tail simply jerks at your clit and she falls weakly onto the ground.");

				MainScreen.text("\n\n\"<i>N-no... no!  So close!</i>\"  The black stalk thrashes weakly as her body loses muscle control everywhere except the parts responsible for forcing her cum out of it.  She moans piteously and babbles as her tail swells around your clit and engulfs you in her liquid climax, doubling in thickness behind the spade from the mounting fluids.  It's too much for you to watch, this arrogant woman trying to use you to get off but only managing to tease herself while suffusing your clit with her hot fluid, and combined with the temperature, pushes you over the edge you'd been flirting with.  Your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " squeezes and squirts, depositing your own orgasm onto the ");
				if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 4) MainScreen.text("demon's legs");
				else MainScreen.text("grass");
				MainScreen.text(", and your clit swells even further inside your tormented partner.  Sheila groans, \"<i>Fuck you, [name]...</i>\"");

				MainScreen.text("\n\nIrritated, she wraps her other frantic tail around the still-straining black one and squeezes, finally managing to get some of her climax past the ring of tight skin at the end; with the fluid forming a bridge, you feel the tension release as a wave of her black, slippery tail-cum forces her slit wide open and gushes out to soak your clit and then your pussy underneath.  \"<i>Ah!  Ah!  Finally!</i>\" she cries, great gouts of agitated slime sliding down your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " to bubble and lick at your pussy, as if trying to get inside.");
			}
			//(else PC corr>=50)
			else {
				MainScreen.text("her tail quakes, then pushes her pussy into yours.  \"<i>God, yes!</i>\"  A spatter of hot fluid hits your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " as your demon lover climaxes, and another begins to drool around the tip of your clit, still suspended inside her writhing tail.  The sudden labial heat is too much for you, and you answer her with a climax of your own, ");
				if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 4) MainScreen.text("drizzling");
				else MainScreen.text("copiously soaking");
				MainScreen.text(" her pussy with your own girl-jizz.  It feels like the nearness of her own cunt pulls your orgasm into obscene lengths, and you come repeatedly, drooling more fluid onto her wanton pussy as she grinds its lips against yours.  She rubs her breasts as she strokes her convulsing vagina against your own, pinching and pulling them until you swear you can see them lengthen.");
			}
		...
		//(else if clit >6" initially)
			...
			//[(horse)
			if (player.lowerBody.isTaur()) MainScreen.text(" and rolls onto her back");
			MainScreen.text(", then wraps her tails around your thickened clit.  They twist and pull, sending spirals of pleasure along the numerous nerves, and begin gently but insistently tugging you toward her tan butt.  Sheila directs the tip right between her cheeks, and you can feel her using it to rub the ring around her hole.  \"<i>I think this should be about the right size for you, love,</i>\" she teases, spearing herself on your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + ".  Inch after inch slides into her slowly, maddening you with warmth, until ");
			if (player.lowerBody.vaginaSpot.get(0).clitLength <= 12) MainScreen.text("all of it is in her bowels and her hot cunt rests against yours");
			else MainScreen.text("she's taken as much as she can");
			MainScreen.text(".  She half-smiles and half-winces at you.  \"<i>Don't usually take it in this hole, but I wanted to feel your pussy going crazy for me.</i>\"  ");
			if (player.lowerBody.vaginaSpot.get(0).clitLength <= 12) MainScreen.text("Her hips shift, dragging her lips across yours and stirring your clit through her asshole even as it squeezes you; your pussy twitches at the stimulation.");
			else {
				MainScreen.text("Her tails, one still swollen from being stretched and tormented by your growing clit, slip into you abruptly, bringing their mild burning sensation with them; if you hadn't been saturated with it already, you'd probably pass out.");
				//[(if virgin vagina)
				if (player.hasVirginVagina()) MainScreen.text("  \"<i>What's this?</i>\" Sheila teases.  \"<i>Why, your virtue is still intact, little girl!  I wouldn't want anything to happen to it!</i>\"  She pulls her tails out a bit, contenting herself with simply teasing your lips and driving you mad for penetration.");
				MainScreen.text("  The demon shifts around your clit, sending a wave through you that makes your lips wrap around the invaders.");
			}

			MainScreen.text("\n\nSheila places her long-taloned feet against ");
			//(not naga)
			if (!player.lowerBody.isNaga()) MainScreen.text("your thighs, spreading your [legs] wide and then hooking them over top to touch ");
			MainScreen.text("the ground next to your ass, and uses the leverage to begin pumping your clit out of and into her asshole, caressing your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " every time she slides home.  \"<i>I could get used to this, love.  What say I keep you dosed up like this and you become my dildo?  I'll ride you with a different hole every time!</i>\"  Your eyes roll back in your head, at least partially from the irregular pace at which she's stroking your clit.  Sheila grabs her breasts as she slows down drastically once again, mashing them with her hands and sinking onto you an inch at a time.  \"<i>Fuck... I love your junk, [name].  Maybe I should catch a goblin, too, and make her tease my pussy while you fill my ass.</i>\"  She raises back up and ");
			//[(clit>12")
			if (player.lowerBody.vaginaSpot.get(0).clitLength > 12) MainScreen.text("pulls her black tail free, dripping with your juice, then ");
			MainScreen.text("begins to work her drooling spade along your clit again, smearing her goo along your chunky chick-stick.  You can feel the edges of the stretched hole in her tip working on your skin, opening and closing ever so slightly as her copious tainted lube oozes out.  Your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " must be completely saturated already, because the slippery goo lingers on the surface instead of wicking into it.  \"<i>Gonna g-get you nice and wet, love...  Ohh,</i>\" Sheila says, affected to distraction by the pleasure of tracing the slick end of her stubby tail along you.");

			MainScreen.text("\n\nFinally coating you to satisfaction with the oily secretions, Sheila begins pumping again.  The heated friction from before evaporates, replaced with a dull, distributed warmth as she slides her asshole along your clit.  With less resistance, she's able to up the tempo to frantic, scraping along the ground so fast that she begins to tear up the grass.  You bear up as much as you can under the onslaught of pumping and gyrating hips, but eventually the sensation is too much, and your nerves take over as your climax begins.  Your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " spasms, drooling onto her ");
			if (player.lowerBody.vaginaSpot.get(0).clitLength <= 12) MainScreen.text("pussy; her smooth, small labia greedily lap at the fluid when they touch, brushing against yours and drawing further twitches and squirts.");
			else MainScreen.text("teasing tails; they shift from stroking your labia to holding them open, allowing your pussy to squirt as it pleases.");
			MainScreen.text("  Suddenly she stops mid-stroke as her asshole quivers, sending shivers up your orgasm-sensitized clitoris.  \"<i>Fffuck!  So soon!  I'm coming too, [name]!</i>\"  Sheila sinks down, burying your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " into her asshole.");
			//(clit>12")
			if (player.lowerBody.vaginaSpot.get(0).clitLength <= 12) {
				MainScreen.text("  Her tails sink into you as she squirms, parting your lips and pushing their way inside.");
				//[output hymen check, but don't remove virgin flag yet]  
				if (player.hasVirginVagina()) MainScreen.text("  <b>You've lost your virginity!</b>");
				MainScreen.text("  Buried in your pussy, the spade begins drooling its black fluids in greater and greater volume as Sheila is wracked by her orgasm.");
			}

			//(PC corr<50)
			if (player.stats.cor < 50) {
				MainScreen.text("  The demon's pussy gushes and pours with unnatural levels of fluids, soaking your crotch as a miniature cataract makes its way out of her lower lips.  Sheila spills herself onto you for at least a minute, producing more hot fluid than you thought possible, until she looks utterly dehydrated - even her breasts seem to be smaller and less jiggly.");
			}
			else MainScreen.text("  The demon's skin seems to parch as she grinds into you relentlessly, evaporating your fluids so fast that most of them don't even have time to fall.  Her hot flesh lays heavily across your netherlips, prolonging your climax and coaxing out more fem-spray than you thought was in you.  Even your mouth seems to dry as she drinks your essence up.");
			...
			if (player.lowerBody.vaginaSpot.get(0).clitLength > 12 && player.hasVirginVagina()) {
				MainScreen.text("  \"<i>Sorry about your hymen, love, but I just couldn't resist making a sweet little slut like you into my girl.  I hope you'll remember me - I only wish I had a proper donger so I could give you my baby, too.</i>\"");
			}
			MainScreen.text("  Even as tired as you are, what you see on the demon girl makes you laugh.  She starts at this, then follows the direction of your gaze; ");
			if (!player.lowerBody.isTaur()) MainScreen.text("her tits are covered by grass stains from her frenetic pumping.");
			else MainScreen.text("craning around shows her that her asscheeks are completely green and wet with torn grass from slamming into you so violently and repeatedly.");
			MainScreen.text("  \"<i>... Oh, god dammit,</i>\" Sheila gripes.  She walks over and forces your face into them with a long, slow shake, then giggles as a joke comes to her.  \"<i>There, [name]; I've stained you with my colors!</i>\"  She plants a long kiss on your grass-smeared forehead, then drops you back to the ground, straightens up, and departs.  Exhausted, you slip into a doze, wondering if you'll be able to fit your swollen clit back into your clothing when you wake or if you'll have to walk back to camp with it sticking out.  It slowly deflates as you sleep, but does retain a bit of additional length.");
		}
		...
		if (player.lowerBody.vaginaSpot.get(0).clitLength > 12 || player.stats.cor < 50) {
			player.cuntChange(12, false, false, false);
			player.slimeFeed();
		}
		player.lowerBody.vaginaSpot.get(0).clitLength++;
		if (player.perks.has("BigClit")) player.lowerBody.vaginaSpot.get(0).clitLength += .5;
		if (player.stats.cor < 50) {
			player.stats.cor += 10;
			sheilaCorruption(-10);
		}
	...
	private aintGotNoGenderAndKangarooRaped(): void {
		MainScreen.clearText();
		MainScreen.text("Sheila sashays over to you and brazenly sticks her hand in your featureless crotch.  \"<i>It's too bad you've mutilated yourself like this, love... or did you do it just to tease me?  You know, it doesn't bug me that much.</i>\"");
		...
		dynStats("lus", 20 + player.stats.lib / 4, "resisted", false);
		if (Game.inCombat)
			cleanupAfterCombat();
		else doNext(camp.returnToCampUseOneHour);
	}
	...
	internal function beatUpDemonSheila(output: boolean = true): void {
		if (output) {
			MainScreen.clearText();
			MainScreen.text("\"<i>Ahh, you win, mate.  I give up.</i>\"  Sheila slumps to the ground, ");
			if (monster.HP < 1) MainScreen.text("and her wounds begin to close with a soft lilac glow.  \"<i>So fuckin' strong.  I don't suppose you've got any sperm for me, do you?  I'd love to have some of your kids...</i>\"  She looks at you beseechingly, gently fingering herself.");
			//(lust)
			else MainScreen.text("frigging herself vigorously.  \"<i>Do with me what you will, lover; those were the terms.  Won't you take my body to satisfy yourself, you brute?</i>\"  The audacious woman sticks her tongue out, as if to goad you into treating her roughly.");
		}
		//[(lust < 30)]
		...
		if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(56) >= 0) MainScreen.addButton(0, "Missionary", missionaryForThePurposesOfCreatingImpsWithSheila);
		...
		if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(56) >= 0) MainScreen.addButton(1, "AnalHateFuck", analHateFuck);
		//Win - [Big Dick+Thighs] (usable for dicks with cockarea > 56) (all 10 of them)
		//penis envy, go!
		if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 56) MainScreen.addButton(2, "UseHerThighs", bigDickAndThighs);
		//Win - [Penetration, in Spades] - for self-loathing vaginas
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.addButton(3, "Clit-Fuck", winAgainstDemoNSheilaForVaginas);
	}
	...
	private missionaryForThePurposesOfCreatingImpsWithSheila(): void {
		MainScreen.clearText();
		MainScreen.text("You unfasten your [armor] and strip down completely, as Sheila's eyes drink up your bare flesh.  Denuded, you caress your cock with several strokes, producing a drop of precum.");
		...
		if (player.stats.cor < 40) MainScreen.text("Your ardor subsides a bit at her performance, but seeing your look of contrition, ");
		else MainScreen.text("Your scowl hardens at her shameless playacting, and ");
		MainScreen.text("she breaks into laughter.  \"<i>I'm just having you on, love.  You know my body's yours for the using.</i>\"");
		...
		if (!player.lowerBody.isTaur()) MainScreen.text("shoulders");
		else MainScreen.text("flanks");
		MainScreen.text(" and her pupils dance in excitement; for a moment you even lose yourself in her enthusiasm, forgetting her demonhood and seeing only an earnest, adoring girl under you.  \"<i>Now, push!</i>\"");
		...
		//(sheila corruption >=40)
		if (sheilaCorruption() >= 40) MainScreen.text("and her soft breasts jiggle ");
		MainScreen.text("as a twinge of sensation from your thrust provokes a gasp and forces her to reposition herself, to better find it again.  \"<i>That's the spot!  Your dick is the <b>best!</b>  Pound me, you fucking cum-hose!</i>\"");
		...
		if (!player.lowerBody.isTaur()) MainScreen.text("arms");
		else MainScreen.text("forelegs");
		...
		if (player.lowerBody.isTaur()) {
			MainScreen.text(", practically lifting herself off of the ground] as she impales her cunt on your [cockFit 56] to prevent you from pulling out.");
		}
		//[(silly)
		if (silly()) MainScreen.text("  \"<i>Make me feel like I'm fifteen years old again and playin' with the dog-morphs in the field!</i>\"  Heavens to Betsy!");
		else MainScreen.text("  \"<i>This is what you're here for!  Don't spill a single drop!</i>\"");
		...
		if (sheilaCorruption() < 40) MainScreen.text("tight");
		else MainScreen.text("soft");
		MainScreen.text(" chest against you");
		if (player.lowerBody.isTaur()) MainScreen.text(" as you slump forward");
		MainScreen.text(".  Your cock ");
		if (player.lowerBody.cockSpot.count() > 1) {
			MainScreen.text(" and its ");
			if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("brother");
			else MainScreen.text("brethren");
			MainScreen.text(" swell and squirt");
		}
		else MainScreen.text("swells and squirts");
		MainScreen.text(", pouring your semen into Sheila's ravenous pussy");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" and coating her crotch with a glaze of white");
		MainScreen.text("; her eyes glow and it briefly feels as though her body is sucking the ejaculate right out of you");
		if (player.cumQ() >= 500) MainScreen.text(", until she fills up and the rest of your orgasm is drooled out at its own lazy pace");
		MainScreen.text(".  She kisses you");
		if (player.lowerBody.isTaur()) MainScreen.text("r forelegs");
		MainScreen.text(" lovingly and deeply");
		if (player.lowerBody.cockSpot.count() > 1 || player.cumQ() >= 500) MainScreen.text(" as the spare seed dribbles from her crotch");
		MainScreen.text(", then breaks away and looks up into your face.");

		MainScreen.text("\n\n\"<i>[name], you ");
		if (player.cumQ() >= 500 || player.lowerBody.cockSpot.count() > 1) MainScreen.text("spilled some.  I can't have your baby if you're fucking the grass.  And you ");
		MainScreen.text("came too soon; I wasn't done.</i>\"");
		...
		//(else if RNG doesn't end scene)
			...
			//(if vag)
			if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" as her imp spawn take turns doing the same to you from behind");
			else MainScreen.text(" as her imp spawn gather around, stroking themselves and occasionally squirting their hot, tainted cum on her body or yours as orgasm takes them");
			MainScreen.text(".  ");
			if (player.stats.lib < 40) MainScreen.text("Shocked");
			else MainScreen.text("Flushing hotly");
			MainScreen.text(", you look away and break the connection, but Sheila's smile only widens.");
			...
			//[(PC corr <50)
			if (player.stats.cor < 50) MainScreen.text("Her pussy convulses as she orgasms, and you feel a wave of heat overtaking your [cock biggest], starting at the tip and moving down the shaft.  As it reaches the base, a gush of her fluids breaches the seal her pussy makes around you, forcing your jizz out along with it in a mixed white and clear squirt.  The demon's vagina squeezes and drools for half a minute, pouring her tainted liquid love out against your tightly-held body until your crotch is soaked with her sticky heat and responds with its own.");
			else MainScreen.text("Her pussy sucks you in as she climaxes, and you can feel larger and larger waves of precum being pulled from your shaft; it almost feels like she's tugging at your entire being.  It's not long before your cock twitches and begins giving her greedy sex a second helping of your semen.");
			MainScreen.text("  She goes limp as her twitching cunt takes over and begins drinking every drop it can get from your dick to replace the goo you pushed out");
			if (player.cumQ() >= 500) MainScreen.text("; you nearly manage to fit it all into her this time, with your production still flagging from the last session");
			MainScreen.text(".  Muscles weakening again, you lower yourself onto her once more and allow her to stroke your ");
			if (!player.lowerBody.isTaur()) MainScreen.text("back");
			else MainScreen.text("sides");
			MainScreen.text(" as she cools down.");
	...
	private analHateFuck(): void {
		MainScreen.clearText();
		MainScreen.text("Fucking bitch, playing with herself like nothing's the matter and you didn't just show her concrete proof of your superiority.  Part of you feels like punishing the insolent woman yourself, pounding her insides and dumping your load without a whit of regard for her own pleasure... but another part is thinking it might be fun to see her completely debased, to hammer home how pathetic she is - something like reducing her to a cock-sleeve for your personal cum-slave.");

		//[Tear Her Up][Call Jojo]
		menu();
		MainScreen.addButton(0, "TearHerUp", sheilaAnalHateFuckAGoGo);
		if (jojoScene.campCorruptJojo()) MainScreen.addButton(1, "Call Jojo", analHateFucksWithJojo);
	}
	...
	private sheilaAnalHateFuckAGoGo(): void {
		MainScreen.clearText();
		MainScreen.text("Your [cockFit 56] gets hard in your [armor] as you watch her diddle her pussy, but you set your jaw to resist giving her what she wants - a better idea is taking shape in your head.  \"<i>Turn around,</i>\" you demand.");

		MainScreen.text("\n\nSheila bites her lip and blushes, then turns her back to you, pressing her chest into the ground and waving her ass in the air, twin tails bobbing over her round cheeks and drooling, eager cunt.  \"<i>Heehee, I'm ready for you, mate... look how wet I am!</i>\"  You strip, releasing your " + CockDescriptor.describeMultiCockShort(player) + ", and approach.  She hums in anticipation as you rub against her smooth labia, smearing her moisture onto your [cockHeadFit 56].  \"<i>Come on; time to put it in already!  Don't tease me!</i>\"");

		MainScreen.text("\n\nYour smile widens, unseen, as you silently agree.  ");
		if (!player.lowerBody.isTaur()) MainScreen.text("Slipping one hand around your [cockFit 32]");
		else MainScreen.text("Straddling her");
		MainScreen.text(", you pull your hips back and line the [cockHeadFit 32] up, then thrust - right into her asshole.  Her head raises and her teeth grit as you force the slick tip past her ring.  \"<i>[name], what the fuck?!  That hurts!  Go easy if you're going to use that hole; I'm not good at stretching my body yet!</i>\"");

		MainScreen.text("\n\n\"<i>No.</i>\"  You ");
		//(not centaur)
		if (!player.lowerBody.isTaur()) MainScreen.text("grab her thrashing legs, using them for leverage as you ");
		MainScreen.text("force your dick into the angry woman.  \"<i>You talk a good game about how you love being full of cock, but when I shove one in you, you get all bitchy and act like you're not ready; why don't you just shut up and get fucked, slut?</i>\"");
		...
		if (sheilaCorruption() >= 90) MainScreen.text("; her enormous, cushiony breasts spread under her as your prick forces her weight heavily onto them");
		MainScreen.text(".  She pulls up tufts and pounds the earth impotently with her fists while you sink in as far as you can.  \"<i>Fuck you!  What the fuck is wrong with you, arsehole?!  Let me go, I don't want anything to do with you anymore!</i>\"");
		...
		if (player.stats.cor >= 80 && player.statusAffects.has("Infested")) {
			menu();
			//[No][Worms Suit You], else auto-output text from [No]
			MainScreen.addButton(0, "No", sheilaAnalHateFuckAGoGoNO);
			MainScreen.addButton(1, "WormsSuitYou", sheilaAnalHateFuckAGoGoGETYOUSOMEWORMS);
		}
		else {
			menu();
			//see above
			MainScreen.addButton(0, "Next", sheilaAnalHateFuckAGoGoNO);
		}
	}
	...
	private sheilaAnalHateFuckAGoGoNO(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>Go to hell.</i>\"  You bury yourself in the demon's ass as your dick begins to spasm.  Her tails writhe as the first trickle sinks into her.  \"<i>What?  Say something, I dare you.</i>\"");
		...
		if (player.cumQ() >= 500) MainScreen.text("  Your overproduction floods her insides, and Sheila groans as it begins to leak back out, drooling over her pussy.  \"<i>Hey, you got your wish,</i>\" you tease.  The wanton woman tries to tilt her ass higher, to tip more of your overflow into her cunt, but you laugh and slap her hard on the hip, sending her to one knee again.");
		...
		if (player.cumQ() < 250) MainScreen.text("trickle");
		else if (player.cumQ() < 1000) MainScreen.text("stream");
		else MainScreen.text("waterfall");
		MainScreen.text(" of semen made when her body pushes out some of the foreign substance.");
	...
	private sheilaAnalHateFuckAGoGoGETYOUSOMEWORMS(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>Mmm...</i>\" you answer, noncommittally.  Your ");
		if (player.lowerBody.balls > 0) MainScreen.text("[balls]");
		else MainScreen.text("insides");
		MainScreen.text(" squirm, giving you pause, and you pop your [cockFit 56] out.  \"<i>Actually... I don't think I suit you at all.</i>\"");
		...
		if (player.lowerBody.balls > 0) MainScreen.text("[sack]");
		else MainScreen.text("[cockFit 56]");
		MainScreen.text(", you can actually feel your pent-up pets, eager to be let out to play.  \"<i>No,</i>\" you say, \"<i>I know what's more your speed.  Trash like you should be cast down into the dirt, and should keep company with the creatures of it.</i>\"  Sheila seems alarmed, but as you slip into her grasping pussy, she forgets her question, grinding her hips into you to milk you dry.  You focus, forcing a wave of slimy annelids down your shaft.  \"<i>Here come your new babies!</i>\"");
		...
		player.orgasm();
		player.stats.cor += 10;
		dynStats("lus", player.stats.lib / 5);
		Flags.list[FlagEnum.SHEILA_DISABLED] = 3;
		if (Game.inCombat)
			cleanupAfterCombat();
		else doNext(camp.returnToCampUseOneHour);
	}
	...
	private analHateFucksWithJojo(): void {
		MainScreen.clearText();
		MainScreen.text("Watching her sit there and play with herself like nothing's the matter and you didn't just demonstrate her weakness for all the world to see touches a nerve deep inside you.  You scowl and focus your thoughts to call your mousey ex-monk, bent on showing this demon how a proper mendicant should behave.");
		...
		if (sheilaCorruption() < 40) MainScreen.text("\"<i>What's with these tiny titties?  Could any man possibly be aroused by your bug bites?  I've seen bigger breasts on incubi!</i>\"");
		else MainScreen.text("\"<i>These disgusting sacks of fat on your chest... are you under the impression that men like giant stretch marks and nipples that hang to your knees?  Why don't you get in shape, you messy bitch?</i>\"");
		...
		if (player.stats.cor >= 80) {
			menu();
			MainScreen.addButton(0, "Next", analHateFucksWithJojoNo, true);
			MainScreen.text("\n\n<b>You could really ruin them, but you'd probably never see Jojo again.</b>");
			MainScreen.addButton(1, "Ruin Them", jojoRuinsTheAnalHateFuck, true);
		}
		else {
	...
	private analHateFucksWithJojoNo(clear: boolean): void {
		...
		dynStats("lus", player.stats.lib / 3, "cor", 2);
		if (Game.inCombat)
			cleanupAfterCombat();
		else doNext(camp.returnToCampUseOneHour);
	}
	...
	private jojoRuinsTheAnalHateFuck(clear: boolean = true): void {
		...
		MainScreen.text("Your mouth spreads in a sickly smile as you contemplate granting her request anyway.  \"<i>Jojo...</i>\"");
		...
		if (sheilaCorruption() >= 90) MainScreen.text(", mashing her tits together as they thicken");
		MainScreen.text(".  The mouse himself seems to be growing, in fact.  His feet kick and thrash, sprouting dangerous-looking nails as his legs stretch prodigiously.  As his body mass increases, his white-furred skin begins to burst and peel like old rags, revealing new, dark flesh underneath.");
		...
		Flags.list[FlagEnum.SHEILA_DISABLED] = 4;
		Flags.list[FlagEnum.JOJO_DEAD_OR_GONE] = 1;
		if (Game.inCombat)
			cleanupAfterCombat();
		else doNext(camp.returnToCampUseOneHour);
	}
	...
	private bigDickAndThighs(): void {
		MainScreen.clearText();
		MainScreen.text("As Sheila diddles herself, you ");
		//[(not horse)
		if (!player.lowerBody.isTaur()) {
			MainScreen.text("doff your [armor]; the demon's eyebrows climb as you haul out " + CockDescriptor.describeMultiCockSimpleOne(player));
			if (player.lowerBody.cockSpot.count() > 2) MainScreen.text(" one at a time, practically stacking them atop each other");
			MainScreen.text(".");
		}
		//(horse)
		else {
			MainScreen.text("slowly draw nearer to her, your " + CockDescriptor.describeMultiCockShort(player) + " bobbing with each step.");
		}
		...
		if (player.stats.cor < 40) {
			...
			MainScreen.text(" toward her and aim underneath her cunt to rub your [cockHead biggest] against her palm, smearing a glob of precum on it.  You can see her shivering wordlessly, sliding her fingers together to play with the hot, clingy fluid, making translucent webbing between them.");
			//[(multi)
			if (player.lowerBody.cockSpot.count() > 1) {
				if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("  Both");
				else MainScreen.text("  All");
				MainScreen.text(" of them... slide them here, love,</i>\" the demon insists.  You line the remainder of your manhood up and force it between her thighs as well.");
			}
			MainScreen.text("  She takes the tip of your [cock biggest] in her hand and pulls gently, coaxing your forward; you close the distance until " + CockDescriptor.describeMultiCockSimpleOne(player) + " is peeking through her legs and your crotch is pressed against hers.");

			MainScreen.text("\n\n\"<i>I'm going to borrow a bit of your sensation...</i>\" Sheila announces quietly.  ");
			//[(not horse and not < 5"0)
			if (!player.lowerBody.isTaur() && player.tallness >= 60) MainScreen.text("She unbends and kisses you on the cheek, and your face feels a bit fuzzy where her lips touch.  ");
			...
			if (player.lowerBody.isTaur()) MainScreen.text(", unseen");
			MainScreen.text(".");

			MainScreen.text("\n\n\"<i>We'll see who finishes first.</i>\"  Resting herself against your " + CockDescriptor.describeMultiCockShort(player) + " for support, she brings her other hand up and touches you; almost the full sensation is there this time, and you sign in happiness.  Sheila begins stroking with the new hand as well as the old, grabbing ");
			if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("your dick with both");
			else MainScreen.text("a prick in each");
			MainScreen.text(" and angling upward so the precum drools downward and lubes her fingers.");
		}
		//(else corr >= 40)
			...
			if (player.lowerBody.cockSpot.count() > 1) {
				...
				if (player.lowerBody.cockSpot.count() == 2) MainScreen.text(", you slide it through her legs between your first cock and her cunt, plowing through her entrance like a furrow; she quakes and you can feel it contracting against your skin.  ");
				else MainScreen.text("s, you push them through the woman's legs one by one, smearing warm precum on her thighs as you do; her pussy squeezes against the top of your [cock biggest] as you coat her skin with your fluids.  ");
			}
			else MainScreen.text("\n\n");
			MainScreen.text("Sheila scoots backward, choking up on monster cock until her ass is pressed against your crotch, and then wraps her hands around and begins to masturbate you, using your drooling fluid and her own wetness for lubrication.");

			MainScreen.text("\n\nShe seems to get off on it as much as you do; as she strokes, her tongue hangs out slightly and she begins rubbing your " + CockDescriptor.describeMultiCockShort(player) + " between her " + sheilaCup() + " breasts, smearing your secretions down her entire torso.  Her skin is hot and feels good, but not as vivid as you'd expect.  The demon's pussy contracts wildly again, despite being at rest against your shaft, and you come to a realization: she's actually siphoning the pleasure from you!");
		}
		//end corr split
		MainScreen.text("\n\nThe woman quickly becomes lost in her own little world, searching out the nerves on your prick");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(" and teasing each one; you thrust weakly into her hand, helping guide her hands, but your lonely tip wants for stimulation.  As the feeling becomes so acute that you prepare to say something, Sheila uncannily anticipates you and scoots her pussy along your shaft, popping your [cockHead biggest] into her mouth.  Her tongue dabs at your slit, tasting your fluid, and her hands ");
		if (player.lowerBody.cockSpot.count() > 2) MainScreen.text("relinquish your other pricks and ");
		if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("relinquish your other prick and ");
		MainScreen.text("focus on squeezing more drops out of you.  She fairly coats her lips with it, rubbing your hole against them before popping the whole [cockHead biggest] back into her mouth.  A moan escapes you just as another one returns from Sheila's throat, transmitted through your [cock biggest].");

		MainScreen.text("\n\nAs her attentions refocus on your [cockHead biggest], a pang of loss arises from the middle of your " + CockDescriptor.describeMultiCockShort(player) + " - and is quickly smothered with her chest.");
		//[(sheila corruption < 50)
		if (sheilaCorruption() < 50) MainScreen.text("  Though her modest breasts can't even begin to encompass your girth, Sheila rubs her cleavage along your skin with gusto while she works your tip with her tongue.");
		else if (sheilaCorruption() < 150) MainScreen.text("  Her large " + sheilaCup() + " breasts encompass your girth easily, and she pushes the soft, flushed skin together as she slides it up and down your cock.");
		else {
			MainScreen.text("  The demon's ridiculously enormous breasts easily cover the entire length of your cock");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
			MainScreen.text(" falling between her crotch and her mouth, making her whole body feel like a huge, wet fuckhole");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" built for " + num2Text(player.lowerBody.cockSpot.count()) + " pricks");
			MainScreen.text(".");
		}
		...
		if (sheilaCorruption() >= 150) {
			MainScreen.text("Her enormous breasts dangle all the way to the ground, forming a second, softer hole to push into with your swollen prick");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
			MainScreen.text(".  ");
		}
		MainScreen.text("\"<i>Y-yes,</i>\" she mumbles, \"<i>gonna fuck her... harder...</i>\"");

		MainScreen.text("\n\nThe apparent answer to your question passes you by as you obliviously force your tool");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(" through the vise of her legs; losing yourself further and further to pleasure, you don't even notice when Sheila's hips begin to buck in time with your own, pressing her mons down against your ");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("topmost ");
		MainScreen.text("dick.  With an enormous, final thrust, you mash your crotch up against hers and begin to ejaculate.  ");
		if (player.stats.cor < 50) MainScreen.text("The sensation seems to reawaken Sheila; she angles her body up to press her pussy close and your embezzled sensation floods back into you - along with an unexpected gift.  You can feel your... no, <i>her</i> demonic pussy convulsing repeatedly, trying to suck up sperm in vain as her own orgasm comes off, and she looks at you with sly eyes.  \"<i>A little reciprocity for my love,</i>\" the woman coos, \"<i>to show you how much I care.</i>\"");
		else MainScreen.text("\"<i>Oh god!</i>\" Sheila shrieks.  \"<i>I'm coming!  Jizz is going everywhere!</i>\"  Indeed, as your body releases streams of cum, Sheila directs them by aiming " + CockDescriptor.describeMultiCockSimpleOne(player) + ", covering herself and moaning as her hands press out as much as possible.");

		MainScreen.text("\n\nFinished, you pull your sensitive, soaked shaft");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(" from between her still-squeezing thighs with a shudder; Sheila shudders with you until you finally draw free and break skin contact, then lowers herself carefully to the ground.  The demon turns toward you, exposing a face and chest splattered with your semen.  She stares at your deflating prick");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(", looking almost wistful to see ");
		...
		if (!player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("to grow a nice pussy for me so that ");
		MainScreen.text("I can put it in you any time I want.</i>\"  She sticks her tongue out at you, and the seed dripping from her lips oozes onto it.");
	...
	private winAgainstDemoNSheilaForVaginas(): void {
		MainScreen.clearText();
		MainScreen.text("You drop onto the woman's lap without hesitation, ");
		if (!player.lowerBody.isTaur()) MainScreen.text("mauling her tits roughly with your hands, ");
		MainScreen.text("and grind your pussy desperately against her midriff.  She slips two fingers into you obligingly, but you ");
		if (!player.lowerBody.isTaur()) MainScreen.text("grab her face, plunging one thumb into the corner of her mouth in your haste, and");
		else MainScreen.text("turn your head and");
		MainScreen.text(" stare right into her eyes.  Focusing as hard as you can on your fantasy, you try to transmit your desire for something more to the demon by sheer force of will.  A sly smile parts her lips as she stares back - could this actually work?");
		...
		if (!player.lowerBody.isTaur()) {
			MainScreen.text("crawl away on your hands");
			//[(tail or naga)
			if (player.lowerBody.tailType > TailType.NONE && player.lowerBody.isNaga()) MainScreen.text(" and raise your tail");
		}
		else MainScreen.text("clamber to your hooves and bend your forelegs");
		MainScreen.text(" to present your eager [vagina].  Sheila sits up and places her hands on your [hips], then slides her pelvis up, teasing your lower lips with something bulbous and warm.  They sting slightly as it smears its slime along them, a not-entirely-pleasant sensation.");
		...
		MainScreen.text("  Oh, god, it burns!  You shake violently as your pussy is assaulted by the fluid from Sheila's black tail, waves of pain twisting through it and forcing its walls to clamp down on the invader.  The demon gasps as you squeeze and leans forward onto your back");
		if (sheilaCorruption() > 0) MainScreen.text(", squashing her tits into you");
		MainScreen.text(".  \"<i>Calm down!</i>\" she barks, grabbing your arms and pulling them toward her.  With hands held behind you like this, you can do little to check her enthusiasm... but Sheila doesn't begin thrusting yet.  \"<i>It'll fade soon, love, and you'll start feeling it,</i>\" she promises, kissing your ");
		if (!player.lowerBody.isTaur()) MainScreen.text("neck");
		else MainScreen.text("withers");
		MainScreen.text(" as her strange, still shaft continues drooling into your pussy.  Your groin heats up in response to the pain, and her words ring hollow.  The burning doesn't seem to be getting any better; you look back over your shoulder at her, fear and hurt evident on your tightened features.  She smiles nervously at you, but the flow of goo doesn't stop.  It feels wetter and slicker in your pussy every second, and soon you can feel the slime begin to seep back out of your [vagina].  Trickle after trickle of fluid escapes you, falling to the ground underneath.");
		...
		//[(lib <20)
		if (player.stats.lib < 20) MainScreen.text("even as proud as you are, ");
		MainScreen.text("there's something appealing about the picture, something powerful in being cherished and used in equal measure.  Sheila screams and releases your hair, digging her fingers into your hips, spraying the inside of your pussy with a thick, oozing heat that spreads out and then dulls as your own pouring juices force her cum out.  Peering between your legs, you can see it falling to the ground; viscous black sludge that bubbles and steams as if it were trying to reach back up to your sex.");
		if (player.stats.cor < 50) {
			MainScreen.text("  She pumps and pumps again, forcing more of the muck into your [vagina] even as she calls you the most filthy things she can think of");
			if (sheilaCorruption() > 0) MainScreen.text("; her tits wobble violently as she thrusts, and then still as she arches her back and jams her spade all the way in");
			MainScreen.text(".  Sheila howls to the sky as she pours her demonic slime into you in one final, solid stream.  You can feel it pushing eagerly through even your copious fluids until it's at the entrance to your womb... and then inside.");
			...
			if (player.fertility < 50) {
				MainScreen.text("  There is a tingle deep inside you, making you more fertile.");
				player.fertility++;
			}
		}
		else {
			MainScreen.text("  She stops pumping, falling against your back and panting as the last of her goo seeps out and is ejected around her shaft by your fountainhead of girl-cum.  You are clearer-headed than before as your orgasm dwindles, though, illogically, you could swear you felt her tits growing against you.");
		}
