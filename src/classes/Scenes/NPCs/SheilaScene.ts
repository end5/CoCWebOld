...
//-sheilaforge: counts up 1 per day once PC has turned Sheila's lethicite over to the T'A weaponsmith, modifies [Weapons] button outputs in that city's menu if sheilacite = 3
		...
		public timeChange():boolean
		{
			...
			if (model.time.hours > 23) {
				if (flags[FlagEnum.SHEILA_CLOCK] < 0) flags[FlagEnum.SHEILA_CLOCK]++;
			}
...
public sheilaCorruption(arg:number = 0):number {
	/*if(flags[FlagEnum.SHEILA_CORRUPTION] + arg > 100) flags[FlagEnum.SHEILA_CORRUPTION] = 100;
	else*/ if(flags[FlagEnum.SHEILA_CORRUPTION] + arg < 0) flags[FlagEnum.SHEILA_CORRUPTION] = 0;
	else flags[FlagEnum.SHEILA_CORRUPTION] += arg;
	return flags[FlagEnum.SHEILA_CORRUPTION];
}

//Sheila's cup function!
internal function sheilaCup():string {
	if(flags[FlagEnum.SHEILA_CORRUPTION]  < 10) return "tiny, almost-absent";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 20) return "A-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 30) return "B-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 40) return "C-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 50) return "D-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 60) return "DD-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 70) return "E-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 80) return "EE-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 90) return "front-running";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 100) return "FF-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 110) return "G-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 120) return "GG-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 130) return "H-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 140) return "HH-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 150) return "'Holy Honkin' Hooters!'-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 160) return "I 'want to squeeze them'-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 170) return "II-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 180) return "Jiggle-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 190) return "JJ-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 200) return "K-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 210) return "KK-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 220) return "L-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 230) return "'Ladylove'-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 240) return "M-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 250) return "'Mega Milk'-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 260) return "N-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 270) return "NN-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 280) return "O-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 290) return "'so big it makes you say' OO-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 300) return "P-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 310 && player.lowerBody.cockSpot.count() > 0) return "'would feel great around your' PP-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 310 && player.lowerBody.cockSpot.count() == 0) return "PP-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 320) return "Q-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 330) return "QQ-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 340) return "R-u-serious?-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 350) return "RR-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 360) return "S-rank";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 370) return "'Shooting Star'-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 380) return "T-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 390) return "'damn, those are some big' TT-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 400) return "'bigger than' U-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 410) return "UU-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 420) return "V-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 430) return "'Va-Va-Voom'-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 440) return "'War Head'-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 450) return "World-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 460) return "X-Buster-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 470) return "XXXX-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 480) return "'Yay'-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 490) return "YY-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 500) return "Z-Saber-cup";
	else if(flags[FlagEnum.SHEILA_CORRUPTION] < 510) return "ZZ-cup";
	else return "fuckin' magic";
}

public sheilaEncounterRouter():void {
	trace("Sheila Encounter:");
	trace("Sheila Corruption: " + Math.round(sheilaCorruption() * 10)/10 + " Sheila Preg: " + flags[FlagEnum.SHEILA_PREGNANCY_INCUBATION] + " Sheila XP: " + flags[FlagEnum.SHEILA_XP]);
	//UNFUCK FLAGS
	if(flags[FlagEnum.SHEILA_CLOCK] > 0)
		flags[FlagEnum.SHEILA_CLOCK] = 0
	if(sheilaCorruption() >= 100 && flags[FlagEnum.SHEILA_DEMON] == 0)
		flags[FlagEnum.SHEILA_DEMON] = 1;
	if(flags[FlagEnum.SHEILA_DEMON] == 1)
		flags[FlagEnum.SHEILA_XP] = 4;
	
	//ACTUALLY PICK ENCOUNTER!
	if(flags[FlagEnum.SHEILA_DEMON] == 0) {
		//PREG SHIT
		if (pregnancy.isPregnant && pregnancy.incubation == 0 && flags[FlagEnum.SHEILA_XP] >= -2) {
			if (flags[FlagEnum.SHEILA_JOEYS] == 0) normalSheilaPregNotifNumberOne();
			else normalSheilaPregNotifREPEATEDED();
			return;
		}
		if(flags[FlagEnum.SHEILA_XP] == 0) sheilaFirstEncounter();
		else if(flags[FlagEnum.SHEILA_XP] == -1 ) sheilaApology();
		//XP-2 Sheila's apology (sheila xp = -2 and demon sheila = 0):
		else if(flags[FlagEnum.SHEILA_XP] == -2) sheilaIsSorrySheRapedYou();
		//XP-3 or XP-4 She-mad (Sheila XP <= -3 and demon sheila = 0):
		else if(flags[FlagEnum.SHEILA_XP] == -3 || flags[FlagEnum.SHEILA_XP] == -4) sheilaReallyMad();
		//XP1: Reconciliation encounter (sheila xp = 1 and demon sheila = 0):
		else if(flags[FlagEnum.SHEILA_XP] == 1) sheilaReconcile();
		//XP2: Familiarizing (Sheila XP = 2; or Sheila XP = 3 AND time =/= 20:00 and demon sheila = 0):
		else if(flags[FlagEnum.SHEILA_XP] == 2 || (flags[FlagEnum.SHEILA_XP] == 3 && model.time.hours != 20)) sheilaGettingFamiliar();
		//XP3: Sexy Time (sheila xp = 3 AND time = 20:00 and demon sheila = 0):
		else if(flags[FlagEnum.SHEILA_XP] == 3 && model.time.hours == 20) sheilaXPThreeSexyTime();
		//XP4: Fancy meeting you here (sheila xp >= 4 and demon sheila = 0):
		else if(flags[FlagEnum.SHEILA_XP] == 4) fuckBuddySheilaMeeting();
		else {
			MainScreen.text("A BUG HAPPENED.  YOUR SHEILA_XP: " + flags[FlagEnum.SHEILA_XP] + ". <b>Value should be between -3 to 4.</b>");
...
private sheilaCorruptionUpdate():void {
	...
	if(flags[FlagEnum.SHEILA_DEMON] == 1) dynStats("cor", 1);
}
...
private sheilaFirstEncounter():void {
	...
	flags[FlagEnum.SHEILA_CORRUPTION] = 30;
...
private apologyAvoidSheila():void {
	...
	//set sheilapreg = -1
	flags[FlagEnum.SHEILA_DISABLED] = 1;
	doNext(camp.returnToCampUseOneHour);
}
...
private sheilaIsSorryAndYouForgive():void {
	...
	if(flags[FlagEnum.SHEILA_JOEYS] > 0) flags[FlagEnum.SHEILA_XP] = 4;
	else flags[FlagEnum.SHEILA_XP] = 1;
	doNext(camp.returnToCampUseOneHour);
}
...
private castArouseAndLeaveSheila():void {
	...
	flags[FlagEnum.SHEILA_CORRUPTION] = 100;
	flags[FlagEnum.SHEILA_DEMON] = 1;
	flags[FlagEnum.SHEILA_CITE] = -1;
	dynStats("cor", 10);
	doNext(camp.returnToCampUseOneHour);
}
...
private sheilaReallyMad():void {
	...
	if(flags[FlagEnum.SHEILA_CORRUPTION] >= 90) MainScreen.text("It's so hard to remember myself anymore, and the taint you've put in my body goes crazy when you touch me.  Do you want me to get on my hands and knees?</i>\"  The woman lowers herself to the ground.  \"<i>I beg you.  ");
	MainScreen.text("Please, please leave me alone.");
	if(flags[FlagEnum.SHEILA_XP] == -3) MainScreen.text("  I know you... you're not as bad as the rest of the monsters out here, so... please.");
	MainScreen.text("</i>\"");
	//[Stand][Leave]
	menu();
...
private shielaXPThreeSexyTimePostSexStayII():void {
	...
	if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(32) >= 0) {
		...
		//[(horse)]
		if(player.isTaur()) MainScreen.text(" and you playfully roll");
		MainScreen.text(" onto your back; Sheila slips her leg over you");
		if(player.isTaur()) MainScreen.text("r own");
		...
		//[(not horse)]
		if(!player.isTaur()) MainScreen.text(" just above one " + nippleDescript(0));
		...
		//[(not horse)]
		if(!player.isTaur()) MainScreen.text("Your hands squeeze her hips almost of their own volition, helping her slam down with force, and her head rolls back on her neck.  ");
		MainScreen.text("Her ");
		//[(sheila corruption < 30)]
		if(flags[FlagEnum.SHEILA_CORRUPTION] < 30) MainScreen.text("little breasts rise and fall with increasing frequency atop her ribcage as her pace picks up and her breaths come short and shallow.");
		else if(flags[FlagEnum.SHEILA_CORRUPTION] > 60) MainScreen.text("huge tits jiggle magnificently as she rocks, nearly slapping her in the face as she plunges down and then falling back into place with waves of rippling flesh.");
		...
		//(horse)
		if(player.isTaur()) MainScreen.text("barrel");
		...
		if(player.stats.cor >= 90 || player.stats.cor > flags[FlagEnum.SHEILA_CORRUPTION]) {
		...
		else if(player.stats.cor < flags[FlagEnum.SHEILA_CORRUPTION]) MainScreen.text("  Her pussy clamps down around you, the rippling muscles so tight that she wouldn't even be able to pump if it weren't for the hot river of juice you feel around your tool.  \"<i>A-ah,</i>\" Sheila groans, \"<i>you thrust too!  Rail me silly!</i>\"  She tries to push against you with her hands, to return herself to an upright position, but can barely pull her chest apart from you - her nipples just narrowly recede from the surface of your " + player.skinFurScales() + " as her efforts peak.");
	...
	//(if no cockFit 32 and yes vag)
		...
		//[(horse)]
		if(player.isTaur()) MainScreen.text("You turn around and straddle a leg, squatting to bring your [vagina] atop her crotch.  \"<i>Push up,</i>\" you order, and Sheila obeys eagerly, twisting her body to connect her excited pussy with yours.");
		//(else if naga)
		else if(player.isNaga()) MainScreen.text("You slide it under her and lift gently; Sheila plays along with a disquieted expression, rolling over and raising her pussy into the air, and you press your own against her butt as your tail slides between her legs.  \"<i>See, you just need to be more... flexible,</i>\" you quip.");
		...
		//[(naga) 
		if(player.isNaga()) MainScreen.text(" tail, drawing it up the hot opening and coiling it so you can return the warm, lube-slicked length for another pass even as you hump her butt.");
		...
		if(player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) {
			...
			if(player.isNaga()) MainScreen.text("scales");
			else MainScreen.text("pussy");
			MainScreen.text(" until you can take no more of the heat and come as well.");
			//[(naga)]
			if(player.isNaga()) MainScreen.text("  Her breasts swell in your hands as she sucks in her breath sharply, feeling your pussy drool its juice onto her ass.");
		}
		//(else if PC corr < sheila corruption)
			...
			if(player.isNaga()) MainScreen.text("; it feels like her breasts are pulling away from you as she tries to push her ass further into your quivering quim");
			MainScreen.text(".");
		}
		else {
			MainScreen.text("  Her vagina drools and spasms, not interested in gestures so much as release, and coats ");
			if(player.isNaga()) MainScreen.text("your tail");
	...
	//(else if no cockFit 32 and no vag)
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			...
			//(mans)
			if(!player.isTaur()) MainScreen.text("falls into your lap");
			//(horse)
...
private sheilaXPThreeSexyTimeGuardDuty():void {
	...
	flags[FlagEnum.SHEILA_XP] = 4;
	if(player.stats.lib < 40) dynStats("lus", 5);
...
private fuckBuddySheilaMeetingJustHangOut():void {
	...
	//[(not horse or drider)
	if(!player.isTaur() && !player.isDrider()) MainScreen.text("scoot your own forward");
...
private fuckBuddySheilaMeetingChat():void {
	...
	if(flags[FlagEnum.SHEILA_JOEYS] > 0) MainScreen.addButton(4,"Kids", fuckBuddySheilaMeetingChatKids);
}
...
private fuckBuddySheilaMeetingChatKids():void {
	MainScreen.clearText();
	//if joeycount = 1
	if(flags[FlagEnum.SHEILA_JOEYS] == 1) {
	...
	//if 1 < joeycount < 10
	else if(flags[FlagEnum.SHEILA_JOEYS] < 10) {
	...
	//if joeycount >= 10
		...
		if(player.stats.lib < 50) {
			...
			if(player.stats.cor < 40) dynStats("cor", -1);
		}
		//(else lib >=50)
		else {
...
private fuckBuddySheilaMeetingChatKidsRest4Now():void {
	...
	if(player.isTaur()) MainScreen.text("back");
...
private consensualSheila69(cock:boolean = true):void {
	...
	//(vag)
		...
		MainScreen.text("  When you finally drop your bottoms, revealing your wet " + vaginaDescript(0) + ", she smiles softly.");
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 8) MainScreen.text(" and bring your " + clitDescript() + " to its erect state.  \"<i>... And quite a stem,</i>\" your lover concludes.  \"<i>You sure that's not a dick?</i>\"");
	...
	if(!player.isTaur()) MainScreen.text("turn around and hang your ");
	...
	//(vag)
	else MainScreen.text("returning the favor and plunging into your " + vaginaDescript(0) + " with her hand.");
	MainScreen.text("  She pumps you once, eliciting a trickle of fluid, and then speaks once more.  \"<i>Bring it lower...</i>\"");
	
	MainScreen.text("\n\nYou oblige, ");
	if(!player.isTaur()) MainScreen.text("flattening your back");
	...
	if(cock) {
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 8) MainScreen.text("nips and sucks at the tip of your clit, rolling it with her tongue");
	...
	if(!player.isTaur()) MainScreen.text("lean down and bury your own tongue all the way inside Sheila, forcing her labia open with your mouth");
	...
	if(!cock) {
		...
		//[(bigclit)
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 8) MainScreen.text(" and stroking your " + clitDescript() + " rapidly with one hand");
		MainScreen.text(".");
	}
	//(smallcox)
	...
	if(player.stats.cor >= 90 || player.stats.cor > flags[FlagEnum.SHEILA_CORRUPTION]) {
		...
		if(!player.isTaur()) MainScreen.text("tongue");
	...
	//(PC corr < sheila corruption)
	else if(player.stats.cor < flags[FlagEnum.SHEILA_CORRUPTION]) {
		MainScreen.text("pushing out a gush of fluid.  For what seems like several minutes, her pussy deluges your ");
		if(!player.isTaur()) MainScreen.text("mouth, filling you with her taste.");
	...
	else if(player.stats.cor == flags[FlagEnum.SHEILA_CORRUPTION] && player.stats.cor < 90) {
	...
	//(else if vag-focus)
		...
		if(player.wetness() <= 2) MainScreen.text("trickle out, and are quickly sucked up by her greedy mouth");
		else if(player.wetness() <= 4) MainScreen.text("gush freely, leaving her face slick");
	...
	else if((player.stats.cor > 90 || player.stats.cor > sheilaCorruption()) && sheilaCorruption() >= 80 && flags[FlagEnum.SHEILA_XP] != 3) {
		sheilaCorruptionUpdate();
		/*if(player.stats.cor > 90 || player.stats.cor > sheilaCorruption()) {
			dynStats("cor", -10);
			sheilaCorruption(10);
	...
	if(cock && player.lowerBody.balls > 0) {
		...
		MainScreen.text(" shape against the rest of her skin, is an imprint of your " + ballsDescriptLight() + ".  The kangaroo girl rolls her eyes at the juvenile display and resumes her harangue.  ");
	}
...
private consentacleVagSexForKangarooSlutBitches():void {
	...
	else MainScreen.text(multiCockDescriptLight() + ".");
	...
	MainScreen.text("\n\nYou grin.  ");
	//[(no centaur)
	if(!player.isTaur()) MainScreen.text("Taking hold of your " + cockDescript(x));
	//(centard)
	else MainScreen.text("Attempting to maneuver your " + cockDescript(x) + " into position");
	MainScreen.text(", you rub the head along her dripping snatch, smearing her fluid over it.  ");
	//[(non-cent)
	if(!player.isTaur()) MainScreen.text("Gripping her fuzzy thighs next, you lift her bottom and line");
	//(hoers)
	...
	if(player.lowerBody.balls > 0 || player.lowerBody.vaginaSpot.hasVagina()) {
		...
		else MainScreen.text(vaginaDescript(0));
	...
	//(else if equal but below 90)
		...
		MainScreen.text("your orgasm quickly supersedes your control as she wrings you and rubs her nipples against your [chest].  " + SMultiCockDesc() + " begins to twitch as it unloads its first squirt of semen into her spasming pussy");
	...
	//[(non-dog cock)
	if(!player.hasKnot(x)) {
		MainScreen.text("You oblige shakingly, pulling your " + Appearance.cockNoun(player.lowerBody.cockSpot.list[x].cockType) + " out of her clenching pussy with a wet, sucking noise.  A trickle of semen drools from between her lips as you unburden yourself of the rest of your load, squirting it onto her groin");
		//[(big skeet)
		if(player.cumQ() > 500) MainScreen.text(", breasts, and face");
		MainScreen.text(".");
		//[(Huge skeet)
		if(player.cumQ() >= 1500) MainScreen.text("  More and more globs of semen shoot from the end of " + sMultiCockDesc() + ", pasting Sheila to the dry grass as you paint her body white.");
	}
	//(dog cock)
	else {
...
private sheilaMutualMasturbation():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		if((player.isTaur() && player.tallness * (5/6) < player.lowerBody.cockSpot.list[player.longestCock()].cockLength) || player.lowerBody.cockSpot.hasCock()) choices[choices.length] = "cock";
	}
	if(player.lowerBody.vaginaSpot.hasVagina() && !player.isTaur()) choices[choices.length] = "vagina";
	...
	//[(horse)
	if(player.isTaur()) MainScreen.text(", folding your legs under you,");
	...
	else if(target == "ass" && !player.isTaur()) MainScreen.text("licking a finger and teasing your asshole with shallow strokes, gently circling the ring");
	...
	else if(!player.isTaur()) MainScreen.text("Your hand reaches back to your [asshole], neglected for too long, and a finger spears your pucker, stroking the nerves inside as you hurry to bring your arousal in line with Sheila's.");
	...
	//(vag)
		...
		if(player.wetness() < 4) MainScreen.text("drooling");
	...
	//(nipcunts)
		...
		MainScreen.text("nipple constricts around your fingers in climax, oozing lubrication onto your partner.  As the drops from your " + breastDescript(0) + " hit her own, Sheila's back arches again and her own orgasm begins.");
	}
	//(jack shit)
...
private sheilaCorruptionWarningsChooseFondle():void {
	...
	flags[FlagEnum.SHEILA_XP] = -3;
	flags[FlagEnum.SHEILA_CORRUPTION] = 90;
	dynStats("cor", -10);
	doNext(camp.returnToCampUseOneHour);
}
...
internal function sheilaGotWhomped():void {
	...
	//[(sheila xp > -4)
	if(flags[FlagEnum.SHEILA_XP] > -4) MainScreen.text("  I'll forgive you for everything else, but please, not this!");
...
private letSheilaGo():void {
	if(flags[FlagEnum.SHEILA_XP] >= -3) flags[FlagEnum.SHEILA_XP] = -1;
	cleanupAfterCombat();
}
...
private rapeSheilasCooter():void {
	...
	//[(horse)
	if(player.isTaur()) MainScreen.text(" by planting your forehooves just above her hips");
	...
	if(player.stats.cor < 50) {
		...
		if(player.isTaur()) MainScreen.text(" with your belly");
		...
		//[(not centaur)
		if(!player.isTaur()) MainScreen.text(", leaning down to kiss her long neck and collarbone");
		...
		//[(nothors)
		if(!player.isTaur()) MainScreen.text(", transferring yours to her breast while brushing her torn shirt to the side");
		MainScreen.text("; without struggling further, she slowly lowers it");
		//[(nothors)
		if(!player.isTaur()) MainScreen.text(" to rest atop your own");
		MainScreen.text(", confirming your suspicion.  Her eyes close and her chin raises resignedly");
		//[(nothors)
		if(!player.isTaur()) MainScreen.text(" as you triumphantly squeeze the flesh in your hand and kiss her under the jaw");
		MainScreen.text(".");
		...
		//[(mans)
		if(!player.isTaur()) MainScreen.text("place a hand along her face, tuning it toward you, and kiss her on the lips.");
		...
		//[(nonhors)
		if(!player.isTaur()) MainScreen.text(" and squeeze her breast");
		...
		//[(nonhors) 
		if(!player.isTaur()) MainScreen.text(" and she presses your hand uncomfortably");
		...
		if(player.stats.cor >= 90 || sheilaCorruption() < player.stats.cor) {
			...
			//[(nonhors)
			if(!player.isTaur()) MainScreen.text("; you can almost feel her breast heating up and swelling with blood under your hand, as the nipple pokes into your palm");
			MainScreen.text(".");
		}
		//(PC corr < sheila corruption)
		else if(player.stats.cor < sheilaCorruption()) {
			...
			//[(nonhors)
			if(!player.isTaur()) MainScreen.text("  She squeezes your hand so hard that you swear you can feel her breast compressing under it as she screams her pleasure.");
		}
		//(else equal but < 90)
		else {
...
private forcedSheilaOral(dick:boolean = true):void {
	...
	//[(cock.length > 0)
	if(dick) MainScreen.text(SMultiCockDesc() + " flops free, half-erect and swelling steadily with blood as you appraise the woman's body.");
	//(vag present)
	...
	if(dick) {
		...
		if(player.lowerBody.cockSpot.list[player.shortestCockIndex()].cockLength <= 8) {
			...
			if(!player.isNaga()) MainScreen.text("a [foot]");
		...
		//[(not horse)
		if(!player.isTaur()) MainScreen.text(" as you fondle her tits");
		MainScreen.text(".");
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00141] > 0) MainScreen.text("  I know a nice boy in the mountains who keeps dogs, and I'm sure he'd love to have a womb like yours for their seed.  By this time tomorrow, you could have a cock in both holes, filling you with hellhound babies since you can't please a person.");
		MainScreen.text("</i>\"");
		
		MainScreen.text("\n\nYou can feel her face heating up under your [butt] as she flushes with shame; seems like you struck a nerve.  You ");
		//[(no horse)
		if(!player.isTaur()) MainScreen.text("pinch her nipple for emphasis, then grind your hips across her face, mauling her nose with your ass.  \"<i>I could fuck a tree and it wouldn't be as wooden as you in the sack");
		...
		if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_TRACKER] > 0 || player.perks.has("MinotaurCumAddict")) {
		...
		if(izmaFollower() && flags[FlagEnum.IZMA_NO_COCK] == 0) MainScreen.text("; I personally know fifteen inches of fat, orange dick and four heavy balls that'd love to get in at your other end while I ride your face");
		MainScreen.text(".</i>\"");
		...
		if(player.statusAffects.has("HairdresserMeeting") || flags[FlagEnum.LUMI_MET] > 0) {
		...
		//[(no horse)
		if(!player.isTaur()) MainScreen.text("  Even squeezing her breasts viciously can't compel her to reinsert it, though you amuse yourself trying.");
...
private sheilaCowgirlRapesYou():void {
	...
	if(player.isTaur()) MainScreen.text("; your hooves stick awkwardly out to one side");
	...
	if(player.isTaur()) MainScreen.text("pulls your legs apart");
	else MainScreen.text("unfastens and strips off your [armor]");
	MainScreen.text(", revealing your " + multiCockDescriptLight() + "; ");
	//[(if no cock fit 32)
	if(player.cockThatFits(32) < 0) {
		MainScreen.text("her eyes widen as " + sMultiCockDesc() + " bobs free, half-erect monster meat that fills her with frustration.");
		...
		//[(horse)
		if(player.isTaur()) MainScreen.text(", or at least as flat as you can get it");
		MainScreen.text(", then sits on your stomach");
		if(player.isTaur()) MainScreen.text(", sliding her long legs between yours");
		...
		if(player.isTaur()) MainScreen.text("forelegs fold over her torso affectionately");
		...
		if(!player.isTaur()) MainScreen.text("and suck on your fingers");
	...
	//(else if cock fit 32)
		...
		//(else lib >= 25)
			...
			if(player.isTaur()) MainScreen.text(", pushing your legs apart");
			...
			if(!player.isTaur()) MainScreen.text(", and your hands go to her waist, fairly pushing her up and slamming her back down in your eagerness");
			...
			if(player.cumQ() <= 250) {
				...
				if(player.tailType > TAIL.NONE) MainScreen.text("your tail");
...
private sheilaForcedOralGeneric():void {
	...
	if(player.str < 61) MainScreen.text("; her weight bears you slowly to the ground");
	...
	if(player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) {
		...
		//[(demon/dragon tongue)
		if(player.tongueType > TONUGE.HUMAN) MainScreen.text("  As tired as you are, you simply go along with it, allowing her rippling walls to slowly draw your tip deeper inside until it bumps against her cervix; she shivers and kisses the top of your head as you groggily run your tongue over the entrance to her womb.");
	...
	//[(str < 61)
	if(player.str < 61) MainScreen.text("climbing off and kneeling next to you.");
	...
	if(player.tongueType > TONUGE.HUMAN) fatigue(15);
...
private normalSheilaPregNotifNumberOneYepIssue():void {
	...
	flags[FlagEnum.SHEILA_DISABLED] = 2;
	flags[FlagEnum.SHEILA_JOEYS]++;
	doNext(camp.returnToCampUseOneHour);
}
...
private normalSheilaPregNotifNumberOneCoolDeal():void {
	...
	if(flags[FlagEnum.SHEILA_XP] < 4) flags[FlagEnum.SHEILA_XP] = 4;
	flags[FlagEnum.SHEILA_JOEYS]++;
	doNext(camp.returnToCampUseOneHour);
...
private normalSheilaPregNotifREPEATEDED():void {
	...
	pregnancy.knockUpForce(); //Clear Pregnancy
	flags[FlagEnum.SHEILA_JOEYS]++;
	if(sheilaCorruption() > 80) flags[FlagEnum.SHEILA_CORRUPTION] = 80;
	menu();
	//[Help(requires >80 speed, centaur >= 5', naga, or big wings and archery)][Walk With Her][Let Her Go]
	if((player.stats.spe > 80 && player.perks.has("Runner")) || (player.isTaur()) || player.isNaga() || (player.canFly && player.hasKeyItem("Bow") >= 0))
...
private normalSheilaPregNotifREPEATEDEDWalkWithHer():void {
	...
	if(!player.isNaga()) MainScreen.text("walk");
...
private normalSheilaPregNotifREPEATEDEDHelpABitchOut():void {
	...
	//(else spd >= 80 and Runner perk)
		...
		if(!player.isGoo()) MainScreen.text("run");
	...
	if(model.time.hours + 4 < 21) doNext(camp.returnToCampUseFourHours);
		...
		if(!(flags[FlagEnum.FUCK_FLOWER_LEVEL] >= 4 && flags[FlagEnum.FUCK_FLOWER_KILLED] == 0) && !vapulaSlave())
...
private normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE():void {
	...
	if(player.isNaga()) MainScreen.text("coil");
	...
	else if(player.lowerBody == LOWER_BODY.HARPY && player.tailType == TAIL.HARPY) {
	...
	//(bees with boots)
	else if(player.lowerBody == LOWER_BODY.BEE) {
	...
	if(!player.isGoo()) MainScreen.text("blood");
	...
	if(!player.isTaur()) MainScreen.text("  She grudgingly repositions, allowing you to rest against her.");
	model.time.days++;
	model.time.hours = 2;
	statScreenRefresh();
	//--Next--
...
private normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE2():void {
	...
	if(player.isGoo()) MainScreen.text("; she's already dressed and appears to have just been outside");
	MainScreen.text(".");
	...
	if(player.lust < 30) MainScreen.text("  You give her a hand, then help her load up.  She gives you a peck on the cheek before she departs.  \"<i>Thanks, love.</i>\"");
	if(player.lust >= 30) {
...
private normalSheilaPregNotifREPEATEDEDHelpABitchOutTOCAMP():void {
	...
	else if(flags[FlagEnum.ANEMONE_WATCH] > 0) MainScreen.text("  Sheila offers you one of the birds, then extends another to your odd tenant.  The anemone's mouth tightens and she shakes her head vigorously, blushing a deep blue.");
...
private sheilaGoesDemon():void {
	...
	if(flags[FlagEnum.SHEILA_XP] < -2) MainScreen.text("always so bloody eager to fuck me!");
	...
	if(player.tongueType > TONUGE.HUMAN) MainScreen.text("extending your oral muscle until it's all the way to the back of her pussy.  ");
...
private demonSheilaEncounter():void {
	...
	if(player.lust >= 33) MainScreen.addButton(1,"LetHerFuck",sheilaLetHerFuckYou);
...
private demonSheilaTalk():void {
	...
	//[(not horse)
	if(!player.isTaur()) MainScreen.text("takes your arm in hers, then slips the other hand into your armor");
	//[(horse)
...
private askDemonSheilaAboutDahChange():void {
	MainScreen.clearText();
	//[(if sheilacite >= 1)]
	if(flags[FlagEnum.SHEILA_CITE] >= 1) {
	...
	else if(flags[FlagEnum.SHEILA_CITE] <= -1) {
...
private talkToDemonSheilaAboutWhatNow():void {
	...
	if(flags[FlagEnum.AMILY_OFFER_ACCEPTED] > 0) MainScreen.text(" save one.  Actually, the two are a bit reminiscent of each other..");
...
private leavePregDemonSheila():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) dynStats("lus", 10);
	doNext(camp.returnToCampUseOneHour);
}
...
private loseToNormalSheilaAndGetRidden():void {
	...
	if(player.isTaur()) MainScreen.text(" and you're forced to put your forelegs down for support as you come to rest on your haunches");
	else MainScreen.text(", spreading your legs.  The demon unfastens your [armor] and strips it from your bottoms easily");
	MainScreen.text(", exposing your " + multiCockDescriptLight() + ".");
	...
	if(player.isTaur()) MainScreen.text(", draping your forelegs over her shoulders,");
	...
	MainScreen.text("; you can feel every new inch of heat as she lowers her body.  The woman moans, gyrating her hips, and " + sMultiCockDesc() + " twitches in painful erectness.");
	...
	if(player.lowerBody.balls > 0) {
		...
		//(not horse)
		if(!player.isTaur()) MainScreen.text("  Sheila takes your face in her hands and presses it into her breasts as she reassures you.");
		...
		if(player.isTaur()) MainScreen.text("forelegs");
	...
	//(else if no balls)
		...
		if(player.isTaur()) MainScreen.text("You force her downward with your forelegs to");
...
private clitSwellingDemonSheilaClitSoundingAnal():void {
	MainScreen.clearText();
	//(no horse)
	if(!player.isTaur()) MainScreen.text("Sheila places her hand in the small of your back and leans in, drawing closer to your face even as she pulls your waist inward.");
	...
	//[(not horse)
	if(!player.isTaur()) MainScreen.text("turns around, then straddles your groin and looks over her shoulder.");
	...
	MainScreen.text(" forcing your lips apart and lingering on your rapidly-swelling " + clitDescript() + ".  \"<i>Oh, my.  What's with this ");
	if(player.lowerBody.vaginaSpot.list[0].clitLength <= 6) MainScreen.text("lewd little");
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength > 6) MainScreen.text(", taking it several inches deep");
	...
	//(clit<=6" initially)
	if(player.lowerBody.vaginaSpot.list[0].clitLength <= 6) {
		MainScreen.text("\n\nShe sits up on her knees, then turns to face you.  ");
		if(!player.isTaur()) MainScreen.text("Taking your face into her hand, she closes your jaw and then leans in to plant a kiss on your chin.");
		...
		//[(horse)
		if(player.isTaur()) MainScreen.text(" through the grass");
		else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text(" between your " + allBreastsDescript());
		...
		if(!player.isTaur()) MainScreen.text("[chest]");
		else MainScreen.text(buttDescript());
		MainScreen.text(" as she shallowly pumps your " + clitDescript() + " in and out of her spade.  Your pelvis twitches uselessly; with her tail waving in the air instead of braced against her, controlling the pace of the fuck is beyond you.  \"<i>Naughty sheila,</i>\" she says, squeezing your wiggling hip with a hand.  You think privately that she could just as easily be talking about herself.  \"<i>Am I not going fast enough for you?  Let me speed up.</i>\"");
		...
		if(player.stats.cor < 50) {
			...
			if(player.wetness() >= 4) MainScreen.text("demon's legs");
		...
		//(else PC corr>=50)
			...
			if(player.wetness() >= 4) MainScreen.text("drizzling");
	...
	//(else if clit >6" initially)
		...
		//[(horse)
		if(player.isTaur()) MainScreen.text(" and rolls onto her back");
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength <= 12) MainScreen.text("all of it is in her bowels and her hot cunt rests against yours");
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength <= 12) MainScreen.text("Her hips shift, dragging her lips across yours and stirring your clit through her asshole even as it squeezes you; your pussy twitches at the stimulation.");
		else {
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength <= 12) MainScreen.text("pussy; her smooth, small labia greedily lap at the fluid when they touch, brushing against yours and drawing further twitches and squirts.");
		...
		MainScreen.text("  Suddenly she stops mid-stroke as her asshole quivers, sending shivers up your orgasm-sensitized clitoris.  \"<i>Fffuck!  So soon!  I'm coming too, [name]!</i>\"  Sheila sinks down, burying your " + clitDescript() + " into her asshole.");
		//(clit>12")
		if(player.lowerBody.vaginaSpot.list[0].clitLength <= 12) {
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 12 && player.hasVirginVagina()) {
		...
		if(!player.isTaur()) MainScreen.text("her tits are covered by grass stains from her frenetic pumping.");
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength > 12 || player.stats.cor < 50) {
		player.cuntChange(12,false,false,false);
		player.slimeFeed();
	}
	player.lowerBody.vaginaSpot.list[0].clitLength++;
	if(player.perks.has("BigClit")) player.lowerBody.vaginaSpot.list[0].clitLength += .5;
	if(player.stats.cor < 50) {
		dynStats("cor", 10);
		sheilaCorruption(-10);
	}
...
private missionaryForThePurposesOfCreatingImpsWithSheila():void {
	...
	if(!player.isTaur()) MainScreen.text("shoulders");
	...
	if(!player.isTaur()) MainScreen.text("arms");
	...
	if(player.isTaur()) MainScreen.text(" as you slump forward");
	...
	if(player.isTaur()) MainScreen.text("r forelegs");
	...
	//(else if RNG doesn't end scene)
		...
		MainScreen.text("\n\nIt's too much.  ");
		if(!player.isTaur()) MainScreen.text("You kiss Sheila on the lips, pushing your tongue through to play with hers, as your arms push your spent body into position to thrust again.");
...
private sheilaAnalHateFuckAGoGo():void {
	...
	if(!player.isTaur()) MainScreen.text("Slipping one hand around your [cockFit 32]");
	...
	MainScreen.text("\n\n\"<i>No.</i>\"  You ");
	//(not centaur)
	if(!player.isTaur()) MainScreen.text("grab her thrashing legs, using them for leverage as you ");
...
private sheilaAnalHateFuckAGoGoGETYOUSOMEWORMS():void {
	...
	flags[FlagEnum.SHEILA_DISABLED] = 3;
	if (getGame().inCombat)
		cleanupAfterCombat();
	else doNext(camp.returnToCampUseOneHour);
}
...
private jojoRuinsTheAnalHateFuck(clear:boolean = true):void {
	...
	flags[FlagEnum.SHEILA_DISABLED] = 4;
	flags[FlagEnum.JOJO_DEAD_OR_GONE] = 1;
	if (getGame().inCombat)
		cleanupAfterCombat();
	else doNext(camp.returnToCampUseOneHour);
}
...
private bigDickAndThighs():void {
	...
	//[(not horse)
	if(!player.isTaur()) {
		MainScreen.text("doff your [armor]; the demon's eyebrows climb as you haul out " + sMultiCockDesc());
		if(player.lowerBody.cockSpot.count() > 2) MainScreen.text(" one at a time, practically stacking them atop each other");
		MainScreen.text(".");
	}
	//(horse)
	else {
		MainScreen.text("slowly draw nearer to her, your " + multiCockDescriptLight() + " bobbing with each step.");
	}
	...
	if(player.stats.cor < 40) {
		...
		MainScreen.text("  She takes the tip of your [cock biggest] in her hand and pulls gently, coaxing your forward; you close the distance until " + sMultiCockDesc() + " is peeking through her legs and your crotch is pressed against hers.");
		
		MainScreen.text("\n\n\"<i>I'm going to borrow a bit of your sensation...</i>\" Sheila announces quietly.  ");
		//[(not horse and not < 5"0)
		if(!player.isTaur() && player.tallness >= 60) MainScreen.text("She unbends and kisses you on the cheek, and your face feels a bit fuzzy where her lips touch.  ");
	...
	else MainScreen.text("\"<i>Oh god!</i>\" Sheila shrieks.  \"<i>I'm coming!  Jizz is going everywhere!</i>\"  Indeed, as your body releases streams of cum, Sheila directs them by aiming " + sMultiCockDesc() + ", covering herself and moaning as her hands press out as much as possible.");
...
private winAgainstDemoNSheilaForVaginas():void {
	...
	if(!player.isTaur()) MainScreen.text("mauling her tits roughly with your hands, ");
	MainScreen.text("and grind your pussy desperately against her midriff.  She slips two fingers into you obligingly, but you ");
	if(!player.isTaur()) MainScreen.text("grab her face, plunging one thumb into the corner of her mouth in your haste, and");
	...
	if(!player.isTaur()) {
		...
		//[(tail or naga)
		if(player.tailType > TAIL.NONE && player.isNaga()) MainScreen.text(" and raise your tail");
	}
	...
	if(!player.isTaur()) MainScreen.text("neck");
