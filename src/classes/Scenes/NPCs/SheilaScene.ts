...
public function sheilaEncounterRouter():void {
	...
	if(flags[FlagEnum.SHEILA_DEMON] == 0) {
		...
		//XP4: Fancy meeting you here (sheila xp >= 4 and demon sheila = 0):
			...
			outputText("A BUG HAPPENED.  YOUR SHEILA_XP: " + flags[FlagEnum.SHEILA_XP] + ". <b>Value should be between -3 to 4.</b>");
...
private function sheila1ndEncLookCloserPtII():void {
	...
	//[(libido>=50)
	if(player.stats.lib >= 50) outputText("  Noting you still checking out her body, she smirks and says, \"<i>Enjoying your lookie, then?  Up here, cobber,</i>\" causing you to snap your head up sheepishly.");
...
private function sheilaReallyMad():void {
	...
	if(flags[FlagEnum.SHEILA_CORRUPTION] >= 90) outputText("It's so hard to remember myself anymore, and the taint you've put in my body goes crazy when you touch me.  Do you want me to get on my hands and knees?</i>\"  The woman lowers herself to the ground.  \"<i>I beg you.  ");
	outputText("Please, please leave me alone.");
	if(flags[FlagEnum.SHEILA_XP] == -3) outputText("  I know you... you're not as bad as the rest of the monsters out here, so... please.");
	outputText("</i>\"");
	//[Stand][Leave]
	menu();
...
private function sheilaReconcileKay2():void {
	...
	if(player.stats.lib < 50) outputText("the beckoning hunks of meat");
	...
	//[(lib >50)]
	if(player.stats.lib >= 50) outputText(" like yourself - I saw you perving -");
...
private function sheilaFriendlyHerPeoples():void {
	...
	//(lib >=50)
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText(", and you raise the kids at home?  Or put them with a nanny");
		outputText("?</i>\"");
		
		outputText("\n\n\"<i>Yeah, I suppose that'll work");
		if(player.lowerBody.cockSpot.hasCock()) outputText(", and I'll turn the kids over to");
		outputText(" - no, I mean, ");
		if(!player.lowerBody.cockSpot.hasCock()) outputText("if I decide");
...
private function shielaXPThreeSexyTimePostSexStayII():void {
	...
	if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(32) >= 0) {
		...
		if(flags[FlagEnum.SHEILA_CORRUPTION] < 30) outputText("little breasts rise and fall with increasing frequency atop her ribcage as her pace picks up and her breaths come short and shallow.");
		else if(flags[FlagEnum.SHEILA_CORRUPTION] > 60) outputText("huge tits jiggle magnificently as she rocks, nearly slapping her in the face as she plunges down and then falling back into place with waves of rippling flesh.");
		...
		else if(player.stats.cor < flags[FlagEnum.SHEILA_CORRUPTION]) outputText("  Her pussy clamps down around you, the rippling muscles so tight that she wouldn't even be able to pump if it weren't for the hot river of juice you feel around your tool.  \"<i>A-ah,</i>\" Sheila groans, \"<i>you thrust too!  Rail me silly!</i>\"  She tries to push against you with her hands, to return herself to an upright position, but can barely pull her chest apart from you - her nipples just narrowly recede from the surface of your " + player.skinFurScales() + " as her efforts peak.");
		...
		//[(single cox)
		if(player.lowerBody.cockSpot.count() == 1) outputText("your dick twitches inside her");
		//(multicox)
		else outputText("she spies the first bead of pearly semen exiting");
		if(player.lowerBody.cockSpot.count() == 2) outputText(" your lonely, unused prick");
		if(player.lowerBody.cockSpot.count() > 2) outputText(" each of your lonely, unused pricks");
		outputText(".");
		...
		//[(multi)]
		if(player.lowerBody.cockSpot.count() > 1) outputText(", to be joined shortly by a messy external eruption that covers her thighs and belly in blotches of gooey white");
		outputText(".");
		//[(big skeet)
	...
	//(if no cockFit 32 and yes vag)
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			...
			if(player.lowerBody.cockSpot.count() == 1) outputText("prick quakes and spurts");
	...
	//(else if no cockFit 32 and no vag)
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText(", stroking your oversized tool despite her dismissal");
		...
		else if(player.stats.cor < sheilaCorruption()) outputText("  Her vagina gushes with clear fluid, filling the bowl made by your hand with warmth, and you think you can see her breasts tightening up slightly as her back arches, though it's hard to be sure with how she's shaking.");
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			...
			//[(multicock)
			if(player.lowerBody.cockSpot.count() > 1) outputText("  The other strokes not coming from this cock do their part to decorate your lover as well, turning her hair into a slimy mess.");
		...
		//(cock.length > 0)
		if(player.lowerBody.cockSpot.hasCock()) outputText(", her face still covered in globby white smears");
		outputText(".  \"<i>Fuck...</i>\" she groans.  \"<i>You're a jerk, [name].");
		//[(cock.length > 0)
		if(player.lowerBody.cockSpot.hasCock()) outputText("  I can't believe you called me a goblin, you arsehole.");
		outputText("</i>\"");
		
		outputText("\n\nYou ");
		//[(cock.length > 0)
		if(player.lowerBody.cockSpot.hasCock()) outputText("can't believe how much she acts like one sometimes... but you ");
		...
		//[(cock.length > 0)
		if(player.lowerBody.cockSpot.hasCock()) outputText(", even after Sheila finishes wiping her face clean and stares at you");
...
private function sheilaXPThreeSexyTimeGuardDuty():void {
	...
	//(else lib >= 40)
		...
		//[(cock.length > 0)
		if(player.lowerBody.cockSpot.hasCock()) outputText("shift your pelvis forward");
		//(cock.length == 0)
...
private function fuckBuddySheilaMeetingChatOldLoves():void {
	...
	//(else lib >=50)
		...
		if(player.stats.cor >= 60) outputText(" - though it's probably meant to be a defensive gesture, all it does is push her tits together under her white shirt and frame them in light-brown skin");
		...
		//[(corr < 30)
		if(player.stats.cor < 30) outputText("; you lay a hand on her shoulder reassuringly");
...
private function fuckBuddySheilaMeetingChatKids():void {
	...
	//if 1 < joeycount < 10
		...
		if(player.stats.lib > 50) outputText(" and tight, hot cunt");
	...
	//if joeycount >= 10
		...
		if(player.stats.lib < 50) {
			...
			if(player.stats.cor < 40) outputText("  You pat her shoulder carefully and suggest, \"<i>I could watch them for a while.</i>\"");
...
private function consensualSheila69(cock:boolean = true):void {
	...
	//(vag)
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 8) outputText(" and bring your " + clitDescript() + " to its erect state.  \"<i>... And quite a stem,</i>\" your lover concludes.  \"<i>You sure that's not a dick?</i>\"");
	...
	if(cock) {
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 8) outputText("nips and sucks at the tip of your clit, rolling it with her tongue");
	...
	if(!cock) {
		...
		//[(bigclit)
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 8) outputText(" and stroking your " + clitDescript() + " rapidly with one hand");
		outputText(".");
	}
	//(smallcox)
	...
	if(cock) {
		...
		//(else doesn't fit)
			...
			if(player.lowerBody.cockSpot.count() > 1) {
				...
				if(player.lowerBody.cockSpot.count() == 2) outputText(" spasms");
	...
	if(cock && player.lowerBody.balls > 0) {
		...
		if(player.lowerBody.balls == 2) outputText("infinity");
...
private function consentacleVagSexForKangarooSlutBitches():void {
	...
	if(!player.upperBody.chest.hasFuckableNipples()) outputText("squeezing and fondling");
	...
	//[(if wangaroo)
	if(player.lowerBody.cockSpot.list[player.cockThatFits(32)].cockType == CockType.KANGAROO) outputText("tapered, fleshy penis.  \"<i>Wow... you never see those.</i>\"");
	...
	if(player.lowerBody.balls > 0 || player.lowerBody.vaginaSpot.hasVagina()) {
		...
		//(if vag)
		if(player.lowerBody.balls == 0 && player.lowerBody.vaginaSpot.hasVagina()) outputText("Your cunt twitches and drools a bit of fluid on the furry intruder.  ");
	}
	...
	if(player.stats.cor >= 90 || player.stats.cor > flags[FlagEnum.SHEILA_CORRUPTION]) {
		...
		if(player.lowerBody.cockSpot.count() > 1 ) {
			...
			if(player.lowerBody.cockSpot.count() == 2) outputText("dick erupts");
	...
	//(else if equal but below 90)
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText(" or onto her thighs and belly");
		outputText(".");
	}
	...
	//[(non-dog cock)
	if(!player.hasKnot(x)) {
		outputText("You oblige shakingly, pulling your " + Appearance.cockNoun(player.lowerBody.cockSpot.list[x].cockType) + " out of her clenching pussy with a wet, sucking noise.  A trickle of semen drools from between her lips as you unburden yourself of the rest of your load, squirting it onto her groin");
		//[(big skeet)
	...
	//(dog cock)
		...
		//[(multi)
		if(player.lowerBody.cockSpot.count() > 2) outputText("Your other cocks erupt as well, soaking her crotch and matting down the fur on her thighs with whiteness. ");
	}
...
private function sheilaMutualMasturbation():void {
	...
	//[(if cock)
	if(target == "cock") {
		if(player.lowerBody.cockSpot.count() == 1) outputText("prick begins");
...
private function sheilaCorruptionWarningsChooseFondle():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("erect");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("wet");
...
internal function sheilaGotWhomped():void {
	...
	//[(sheila xp > -4)
	if(flags[FlagEnum.SHEILA_XP] > -4) outputText("  I'll forgive you for everything else, but please, not this!");
...
private function rapeSheilasCooter():void {
	...
	if(player.stats.cor < 50) {
		...
		if(player.lowerBody.cockSpot.count() > 1 ) {
			outputText("  Your other ");
			if(player.lowerBody.cockSpot.count() == 2) outputText("dick spits");
...
private function forcedSheilaOral(dick:boolean = true):void {
	...
	if(dick) {
		...
		if(player.lowerBody.cockSpot.list[player.shortestCockIndex()].cockLength <= 8) {
			...
			if(player.lowerBody.balls > 0) outputText("your [sack]");
			...
			if(player.lowerBody.balls > 0) outputText(" and your [balls] tighten up");
		...
		//(else dick length > 8)
			...
			if(player.lowerBody.cockSpot.list[player.shortestCockIndex()].cockThickness <= 6) {
				...
				if(player.lowerBody.balls > 0) outputText(" and your [balls] begin to rise");
			...
			///(else width > 6)
				...
				if(player.lowerBody.balls > 0) outputText(" and your [balls] draw closer to your body");
		...
		if(player.stats.cor >= 70 && player.statusAffects.get("Infested").value1 == 5) {
			...
			if(player.lowerBody.balls > 0) outputText("packing your [balls] and driving up your lust in their zeal to multiply");
		...
		if((player.lowerBody.vaginaSpot.hasVagina() && player.vaginalCapacity() >= 100) || (!player.lowerBody.vaginaSpot.hasVagina() && player.analCapacity() >= 100)) outputText("  She keeps hitting the same spot again, too - she's not even bothering to adjust her angle and touch the rest of your stretched opening.");
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00141] > 0) outputText("  I know a nice boy in the mountains who keeps dogs, and I'm sure he'd love to have a womb like yours for their seed.  By this time tomorrow, you could have a cock in both holes, filling you with hellhound babies since you can't please a person.");
		outputText("</i>\"");
		...
		if(izmaFollower() && flags[FlagEnum.IZMA_NO_COCK] == 0) outputText("; I personally know fifteen inches of fat, orange dick and four heavy balls that'd love to get in at your other end while I ride your face");
		outputText(".</i>\"");
...
private function sheilaGetsRapedWithADildont():void {
	...
	//(cock)
	if(player.lowerBody.cockSpot.hasCock()) outputText("  \"<i>Perhaps next time you'd like to try a real cock in there?</i>\"  Even exhausted and messed up on goblin drugs, she has the presence of mind to shake her head at that.  You laugh.");
...
private function sheilaCowgirlRapesYou():void {
	...
	if(player.cockThatFits(32) < 0) {
		...
		else if(player.stats.cor < sheilaCorruption()) outputText("Her orgasm wets your prick, coating it with fluid as she comes.  Sheila wraps herself tighter in your limbs, squeezing her breasts against her chest, and you think you can feel them get a little smaller as your head fuzzes over with her excess emotion.");
	...
	//(else if cock fit 32)
		...
		//[(multicock)]
		if(player.lowerBody.cockSpot.count() > 1) outputText("separating out and ");
		...
		//(else lib >= 25)
			...
			//(PC corr >= 90 or > sheila corruption)
			if(player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) outputText("You can feel her cum-slick inner walls pulling at you as she begins to orgasm, and your mind clears as you blow your load inside her, wondering when she'll likewise return to sense.");
			//(else PC corr < sheila corruption)
			else if(player.stats.cor < sheilaCorruption()) outputText("Her pussy gushes with her orgasm, pushing out your semen and soaking her inner lips even as she continues riding your cock, and your mind clouds as you think of little except sneaking as much of your cum inside her as possible.");
			//else equal but < 90)
...
private function sheilaForcedOralGeneric():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("her hand slips around your [cock]");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("her fingers slip between your labia");
	...
	if(player.upperBody.head.hairType == 4) outputText(" and flushing as your stingers release their payloads into her palm");
	outputText(".  You obediently stick out your tongue");
	if(player.stats.cor > 60) outputText(", motivated more by whimsy than the meager, almost playful pain,");
	...
	if(player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) {
		...
		//[(demon/dragon tongue)
		if(player.tongueType > TONUGE.HUMAN) outputText("  As tired as you are, you simply go along with it, allowing her rippling walls to slowly draw your tip deeper inside until it bumps against her cervix; she shivers and kisses the top of your head as you groggily run your tongue over the entrance to her womb.");
	...
	//(else PC corr < sheila corruption)
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("face-fucking her and blowing a load into her mouth, then holding her jaw shut until she swallows it unhappily.");
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("smearing your gash on her face until you come and drown her in your squirting girl-jizz.");
...
private function normalSheilaPregNotifNumberOne():void {
	...
	if(player.stats.cor < 50) outputText("  A hand placed around her shoulder seems to steady her a bit.");
...
private function normalSheilaPregNotifREPEATEDED():void {
	...
	if(player.stats.cor < 40) outputText(", and you give it a squeeze");
...
private function normalSheilaPregNotifREPEATEDEDLetHerGo():void {
	...
	if(player.stats.cor < 40) outputText(", imploring her to eat the rest and keep up her strength");
...
private function normalSheilaPregNotifREPEATEDEDHelpABitchOut():void {
	...
	//(else spd >= 80 and Runner perk)
		...
		if(player.stats.lib < 50) outputText("you pinch her ear, provoking an annoyed flick from it.");
...
private function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDWELP():void {
	...
	//[(corr <50)
	if(player.stats.cor < 50) outputText("  You set a hand on Sheila's shoulder to steady her a bit as the tears start to fall.");
...
private function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE():void {
	...
	if(player.isGoo()) {
		...
		if(player.stats.lib >= 40) outputText("  The leer you display is so prurient that she seems on the verge of reconsidering, but you compose yourself and");
		else outputText("  You");
	...
	//(naga)
		...
		if(player.stats.lib >= 40) outputText(" - but tighten the one sliding across her breasts just slightly.  Sheila's neck turns red and her ears twitch as you rub her nipples, and you plant a kiss on her nape");
	...
	//(horse and >=5ft)
		...
		if(player.stats.sens >= 40) outputText("... or would be, if you weren't distracted by the nipples poking into you and the thighs squeezing your flank");
		outputText(".");
...
private function normalSheilaPregNotifREPEATEDEDHelpABitchOutTOCAMP():void {
	...
	//[(int >=60)]
	if(player.stats.int >= 60) outputText("\"<i>The baby will be fine,</i>\" you venture, hazarding a guess.  \"<i>It's what the nurses are there for.</i>\"");
	...
	//[(if Jojo present)
	if(player.statusAffects.has("JojoNightWatch") || player.statusAffects.has("PureCampJojo")) outputText("  She extends one bird to you and another to Jojo.  He raises his hand and shakes his head, but politely.");
	else if(flags[FlagEnum.ANEMONE_WATCH] > 0) outputText("  Sheila offers you one of the birds, then extends another to your odd tenant.  The anemone's mouth tightens and she shakes her head vigorously, blushing a deep blue.");
...
private function sheilaGoesDemon():void {
	...
	if(flags[FlagEnum.SHEILA_XP] < -2) outputText("always so bloody eager to fuck me!");
	...
	if(player.tongueType > TONUGE.HUMAN) outputText("extending your oral muscle until it's all the way to the back of her pussy.  ");
...
private function demonSheilaTalk():void {
	...
	//[(lib <20)
	if(player.stats.lib < 20) outputText(", giggling as your expression petrifies");
...
private function askDemonSheilaAboutDahChange():void {
	...
	if(flags[FlagEnum.SHEILA_CITE] >= 1) {
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText(", and you feel a monument of your own being erected");
...
private function askDemonSheilaAboutHerColony():void {
	...
	//(corr >=75)
	if(player.stats.cor >= 75) outputText("harvested that day, you sigh wistfully");
	...
	//[(cock)
	if(player.lowerBody.cockSpot.hasCock()) outputText("she ties you down and rides you until she's so full of your spunk that she can push it back out just by pressing on her stomach");
	...
	outputText(".  You ");
	if(player.stats.cor < 50) outputText("break eye contact, blushing");
...
private function talkToDemonSheilaAboutWhatNow():void {
	...
	if(player.stats.cor < 50) {
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("my pussy");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("give them to me");
	...
	if(flags[FlagEnum.AMILY_OFFER_ACCEPTED] > 0) outputText(" save one.  Actually, the two are a bit reminiscent of each other..");
	...
	if(player.stats.cor < 50) outputText("; even though you hide it, I've seen it lurking");
...
private function demonSheilaTalkAnotherQuestion():void {
	...
	if(player.lust <= 99) {
		...
		//(cock)
		if(player.lowerBody.cockSpot.hasCock()) outputText("slipping more of her finger and thumb around your penis");
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("parting your lips more");
...
private function demonImpChildren():void {
	...
	//(if no cock)
	if(!player.lowerBody.cockSpot.hasCock()) outputText("  Or, had...</i>\" she finishes with a frown as she reaches your bare groin.  \"<i>Where'd you leave that magnificent old fella?  I was lookin' forward to another go.</i>\"  She pulls her hand out, then leans toward you with a pout.  \"<i>I'm so horny, too...</i>\"");
	//(if cock)
	else {
...
private function loseToNormalSheilaAndGetRidden():void {
	...
	outputText("\n\n");
	//[(multi)
	if(player.lowerBody.cockSpot.count() > 1) outputText("\"<i>All of these look ripper, but... I want this one today,</i>\" she says, selecting your [cockFit 56].  ");
	...
	//[(multi)
	if(player.lowerBody.cockSpot.hasCock()) outputText(" and slipping the unused remainder of your manhood between her thighs");
	...
	if(player.lowerBody.balls > 0) {
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText(" and lap");
...
private function tailSpadeSoundingForFuckHugeDongsWithDemonSheila():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("; unguided, the ");
	if(player.lowerBody.cockSpot.count() == 2) outputText("other twitches and sprays");
	else if(player.lowerBody.cockSpot.count() > 2) outputText("others twitch and spray");
	if(player.lowerBody.cockSpot.count() > 1) outputText(" all over, coating her back, you, the grass, and anything ");
	if(player.lowerBody.cockSpot.count() > 2) outputText("else they happen to point at");
	if(player.lowerBody.cockSpot.count() == 2) outputText("else it happens to point at");
...
private function clitSwellingDemonSheilaClitSoundingAnal():void {
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength <= 6) outputText("lewd little");
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength > 6) outputText(", taking it several inches deep");
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength <= 6) {
		...
		else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText(" between your " + allBreastsDescript());
	...
	//(else if clit >6" initially)
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength <= 12) outputText("all of it is in her bowels and her hot cunt rests against yours");
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength <= 12) outputText("Her hips shift, dragging her lips across yours and stirring your clit through her asshole even as it squeezes you; your pussy twitches at the stimulation.");
		else {
		...
		//[(clit>12")
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 12) outputText("pulls her black tail free, dripping with your juice, then ");
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength <= 12) outputText("pussy; her smooth, small labia greedily lap at the fluid when they touch, brushing against yours and drawing further twitches and squirts.");
...
private function missionaryForThePurposesOfCreatingImpsWithSheila():void {
	...
	if(player.stats.cor < 40) outputText("Your ardor subsides a bit at her performance, but seeing your look of contrition, ");
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		outputText(" and its ");
		if(player.lowerBody.cockSpot.count() == 2) outputText("brother");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText(" and coating her crotch with a glaze of white");
	...
	if(player.lowerBody.cockSpot.count() > 1 || player.cumQ() >= 500) outputText(" as the spare seed dribbles from her crotch");
	outputText(", then breaks away and looks up into your face.");
	
	outputText("\n\n\"<i>[name], you ");
	if(player.cumQ() >= 500 || player.lowerBody.cockSpot.count() > 1) outputText("spilled some.  I can't have your baby if you're fucking the grass.  And you ");
	...
	//(else if RNG doesn't end scene)
		...
		//(if vag)
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" as her imp spawn take turns doing the same to you from behind");
		...
		outputText(".  ");
		if(player.stats.lib < 40) outputText("Shocked");
...
private function bigDickAndThighs():void {
	...
	if(!player.isTaur()) {
		...
		if(player.lowerBody.cockSpot.count() > 2) outputText(" one at a time, practically stacking them atop each other");
		outputText(".");
	}
	//(horse)
	else {
	...
	if(player.stats.cor < 40) {
		...
		if(player.lowerBody.cockSpot.count() == 1) outputText("your dick with both");
	...
	//(else corr >= 40)
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			...
			if(player.lowerBody.cockSpot.count() == 2) outputText(", you slide it through her legs between your first cock and her cunt, plowing through her entrance like a furrow; she quakes and you can feel it contracting against your skin.  ");
	...
	if(player.lowerBody.cockSpot.count() > 2) outputText("relinquish your other pricks and ");
	if(player.lowerBody.cockSpot.count() == 2) outputText("relinquish your other prick and ");
	...
	if(sheilaCorruption() < 50) outputText("  Though her modest breasts can't even begin to encompass your girth, Sheila rubs her cleavage along your skin with gusto while she works your tip with her tongue.");
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText(" built for " + num2Text(player.lowerBody.cockSpot.count()) + " pricks");
		outputText(".");
	}
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) outputText("to grow a nice pussy for me so that ");
...
private function winAgainstDemoNSheilaForVaginas():void {
	...
	//[(lib <20)
	if(player.stats.lib < 20) outputText("even as proud as you are, ");
