...
public function sheilaEncounterRouter():void {
	...
	if(flags[FlagEnum.SHEILA_DEMON] == 0) {
		...
		//XP4: Fancy meeting you here (sheila xp >= 4 and demon sheila = 0):
			...
			Render.text("A BUG HAPPENED.  YOUR SHEILA_XP: " + flags[FlagEnum.SHEILA_XP] + ". <b>Value should be between -3 to 4.</b>");
...
private function sheila1stEncStayHidden():void {
	clearOutput();
	Render.text("You regulate your breathing and hold yourself still to avoid rustling anything softly, waiting for the creature to finish its business and leave.");
...
private function sheila1ndEncLookCloserPtII():void {
	...
	//[(libido>=50)
	if(player.stats.lib >= 50) Render.text("  Noting you still checking out her body, she smirks and says, \"<i>Enjoying your lookie, then?  Up here, cobber,</i>\" causing you to snap your head up sheepishly.");
	Render.text("  You nod at the strange woman and introduce yourself.");
	if(player.short != "Sheila") Render.text("  \"<i>" + player.short + ", eh?  My mates just call me Sheila.  Bunch of jokers, the bastards.</i>\"");
	else Render.text("  \"<i>You too, eh?  Sheila's what my mates call me for a nickname.  Don't suppose you got yours from a bunch of adolescent-minded wankers, though.</i>\"");
	...
	//[(high demon score)
	if(player.demonScore() >= 4) Render.text("  I mean, take a squizz at yourself.  It's like you're not even trying to hide it; I've seen imps that look more like normal people.");
	Render.text("  Forget it.</i>\"  You deny the accusation, but she continues to glare at you.");
...
private function sheila1ndEncLookCloserPtIILeave():void {
	clearOutput();
	Render.text("\"<i>Good onya.  Maybe I'll see you around.</i>\"  With that, you take your leave of the strange woman.");
	//set sheila xp = 1
...
private function sheila1ndEncLookCloserPtIITalkLeave():void {
	clearOutput();
	Render.text("Wordlessly, you get up and back away from the woman... though not without a dirty glance.  She follows you with her eyes until you judge yourself far enough away to turn your back to her.");
	//go to camp
	doNext(camp.returnToCampUseOneHour);
}
...
private function sheilaApology():void {
	clearOutput();
	Render.text("As you search the plains for god-knows-what, you catch your [leg] on something; a rustling in the brush below you is the only warning you get before a cleverly hidden snare closes on it!");
...
private function apologyAvoidSheila():void {
	clearOutput();
	Render.text("You shake your head at the leery girl, and tell her you'd be just as happy as she would if you never saw each other again.");
...
private function apologySheilaApology():void {
	clearOutput();
	Render.text("You scratch your " + player.skinFurScales() + " absently behind an ear, not sure how to broach the topic of reconciliation when you feel like she was the aggressor, but mumble something about being sorry things started off on the wrong foot.");
...
private function apologySheilaSlapSult():void {
	clearOutput();
	Render.text("You say nothing as she finishes with the snare and stands back up.");
...
private function apologySheilaSayNothing():void {
	clearOutput();
	Render.text("You decide to keep your own counsel for now, folding your arms defensively and frowning.");
...
private function sheilaIsSorrySheRapedYou():void {
	...
	if(player.statusAffects.has("KnowsArouse")) addButton(2,"CastArouse",sheilaPologyCastArouse);
...
private function sheilaIsSorryButLeaveMeAlone():void {
	clearOutput();
	Render.text("You shake your head and turn away, dismissing Sheila with a pointed gesture.  \"<i>H-hey!</i>\" the girl calls angrily from behind you.  \"<i>Dammit, I said I was sorry, you bastard!  Screw you!  See if I talk to you again!</i>\"");
	//set sheilapreg = -1
	flags[FlagEnum.SHEILA_DISABLED] = 1;
	doNext(camp.returnToCampUseOneHour);
}
...
private function sheilaPologyFight():void {
	clearOutput();
	Render.text("Saying nothing, you raise your [weapon] and take a swing!  Sheila starts a bit and dodges just in time, then with a look of resignation, raises her fists as well.  \"<i>That'll be right, I guess.  For what it's worth, I really am sorry.</i>\"");
...
private function castArouseAndLeaveSheila():void {
	clearOutput();
	Render.text("You shake your head sadly.  \"<i>I'm sorry, I haven't the time right now.</i>\"");
...
private function castArouseAndLetHer():void {
	clearOutput();
	Render.text("As soon as you nod, the girl bounds to her feet and jumps you, driving you slightly downward in her ardor.");
...
private function sheilaReallyMad():void {
	clearOutput();
	Render.text("A rustle in the bush claims your attention, and you turn your head to see Sheila, standing up suddenly.  Her eyes narrow when she recognizes you, and she takes off running!");
	...
	if(flags[FlagEnum.SHEILA_CORRUPTION] >= 90) Render.text("It's so hard to remember myself anymore, and the taint you've put in my body goes crazy when you touch me.  Do you want me to get on my hands and knees?</i>\"  The woman lowers herself to the ground.  \"<i>I beg you.  ");
	Render.text("Please, please leave me alone.");
	if(flags[FlagEnum.SHEILA_XP] == -3) Render.text("  I know you... you're not as bad as the rest of the monsters out here, so... please.");
	Render.text("</i>\"");
	//[Stand][Leave]
	menu();
...
private function sheilaReallyMadLeave():void {
	clearOutput();
	Render.text("Huffing, you throw the animals down and turn away.  What a display.");
...
private function sheilaReallyMadStandGround():void {
	...
	//[(normal mode)]
	if(!silly()) {
		Render.text("You throw the animals behind you and fold your arms.");
		Render.text("\n\nSheila shivers, then speaks.  \"<i>Alright, mate.  If that's the way it has to be.</i>\"  She raises her fists half-heartedly.");
	}
	//(silly mode)
	else {
		Render.text("Throwing the bundle to the side dramatically, you call the name of your stand!  Sheila's eyes widen as your body surges with renewed energy, and she jumps back as a ghostly other self materializes next to you.");
...
private function sheilaReconcile():void {
	clearOutput();
	Render.text("\"<i>Oy, it's you,</i>\" sings out a voice behind you.  \"<i>Back again?</i>\"");
...
private function sheilaReconcileDunWanna():void {
	clearOutput();
	Render.text("Raising your hands in a palms-out gesture, you decline.  Sheila colors a bit, misinterpreting your reluctance.  \"<i>Hey, it's not like I'm cracking onto you or anything.  I was just looking for a reason to eat half a rabbit myself, is all.</i>\"");
...
private function sheilaReconcileKay():void {
	clearOutput();
	Render.text("Some fresh meat does sound pretty tempting.  You nod at the woman and she climbs to her feet.  \"<i>Beaut!  Do me the favor and fossick about for some tinder.</i>\"");
	//--Next--
	menu();
...
private function sheilaReconcileKay2():void {
	clearOutput();
	Render.text("Soon enough you've got a firepit going in a small clearing.  \"<i>Ahh, classic bush theater,</i>\" Sheila says, admiring it.  The woman dextrously spits two liberated and skinned rabbit quarters on a long stick and twirls them over the lapping flame, and soon the air is full of the sizzle and pop of meat juices dripping on the hot stones and the simple tune your cook hums.  You stare at ");
	if(player.stats.lib < 50) Render.text("the beckoning hunks of meat");
	else Render.text("her rising and falling breasts under the pretext of watching her work");
	Render.text(", until she happens to look up and catch your eye.  \"<i>Sorry, no parsley,</i>\" she says, breaking into an obliviously happy smile.  Seems like her temperament preternaturally suits this lifestyle.");
	...
	//[(pc is kangaroo)
	if(player.race() == "kangaroo-morph") Render.text(" - except you, I suppose");
	Render.text(".  We trade our goods in prearranged op shops and dead drops but avoid bringing outsiders into our own settlements, even when they're right corkers.</i>\"");
	...
	//[(lib >50)]
	if(player.stats.lib >= 50) Render.text(" like yourself - I saw you perving -");
	Render.text(" make us a little hornier, and evil buggers make us a bit more evil.  Our bodies tend to change on their own to reflect how tainted we are... which is as much as saying there are a sad lot among us who flirt with demonic energy to up our sizes to suit our egos.  But versus the sexual appetite and limitless rapacity of the demons themselves...?</i>\"");
...
private function sheilaFriendlyComeHereOften():void {
	clearOutput();
	Render.text("You ask her why she enjoys being on the plains so much.");
...
private function sheilaFriendlyHerPeoples():void {
	clearOutput();
	Render.text("You lie down on the grass yourself, balance the parcel on your stomach, and ask about her people.");
	...
	if(player.femininity < 35) Render.text("  She looks absently at your crotch.");
	...
	//(lib >=50)
		...
		if(player.lowerBody.cockSpot.hasCock()) Render.text(", and you raise the kids at home?  Or put them with a nanny");
		Render.text("?</i>\"");
		
		Render.text("\n\n\"<i>Yeah, I suppose that'll work");
		if(player.lowerBody.cockSpot.hasCock()) Render.text(", and I'll turn the kids over to");
		Render.text(" - no, I mean, ");
		if(!player.lowerBody.cockSpot.hasCock()) Render.text("if I decide");
		else Render.text("I'm not ready for");
		Render.text(" - er... fuck <b>you!</b></i>\"  This last exclamation comes in response to your pealing laughter.  \"<i>Wanker!</i>\"");
...
public function sheilaXPThreeSexyTime():void {
	clearOutput();
	Render.text("You prowl through the twilit grasslands, lazily looking for anything interesting before the bleak, red moon brings on the night and its restrictions.  Nothing much catches your eye, except for a rounded shape cast in silhouette against the horizon.  Initially taking it for a rock, you're surprised when it shivers at the sound of your approach!  It unbends and raises a head, turning toward you and revealing the flushed face of the bush girl, Sheila.  Her eyes quickly change from terror to relief as they settle on you, and she cries out.  \"<i>[name]!  Oh thank god; please help me!</i>\"");
	Render.text("\n\n\"<i>What's wrong?</i>\"");
...
private function sheilaXPThreeSexyTimeDitchHer():void {
	clearOutput();
	Render.text("You shake your head, pulling away from the woman and telling her she'll be fine.  \"<i>No!</i>\" she shrieks.  \"<i>You gotta help me!</i>\"");
...
private function sheilaXPThreeSexyTimeSex():void {
	clearOutput();
	Render.text("Sheila's eyes widen with your smile, and she pulls back from you.  \"<i>That's, um...</i>\"");
...
private function sheilaXPThreeSexyTimePostSexLetHerBe():void {
	clearOutput();
	Render.text("Pulling away from you, Sheila gathers her clothing and departs, looking satisfied.  The glance she gives you over her shoulder is even a little warmer than before.");
	//i had snoo-snoo!
	flags[FlagEnum.SHEILA_XP] = 4;
	doNext(camp.returnToCampUseOneHour);
}
...
private function shielaXPThreeSexyTimePostSexStay():void {
	clearOutput();
	flags[FlagEnum.SHEILA_XP] = 4;
	Render.text("You grab her tail, provoking a curious, but not discouraging, look from the woman who was planning to spend the night alone.  \"<i>What is it, [name]?</i>\"");
...
private function shielaXPThreeSexyTimePostSexStayII():void {
	...
	if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(32) >= 0) {
		Render.text("Her hand slips down to your groin and, overcoming its trepidation, grabs your [cockFit 32] and begins to pump languidly, slowly stimulating blood back into it.  \"<i>I don't know what's come over me,</i>\" she says, giggling.  \"<i>Maybe I'm just greedy, but... I want you again.</i>\"  She pushes you");
		//[(horse)]
		if(player.isTaur()) Render.text(" and you playfully roll");
		Render.text(" onto your back; Sheila slips her leg over you");
		if(player.isTaur()) Render.text("r own");
		Render.text(", positioning herself atop your crotch and guiding your tip to her pussy.  It pokes through the labia slightly, welcomed with warm wetness.  \"<i>Is... this ok, [name]?  Can I?</i>\"");
		...
		//[(not horse)]
		if(!player.isTaur()) Render.text(" just above one " + nippleDescript(0));
		else Render.text(" on the thigh");
		Render.text(".  \"<i>I'm gonna have to punish you for it, you dag.</i>\"  Excusing herself thusly, she sits up and begins to slide up and down your [cockFit 32].  ");
		//[(not horse)]
		if(!player.isTaur()) Render.text("Your hands squeeze her hips almost of their own volition, helping her slam down with force, and her head rolls back on her neck.  ");
		Render.text("Her ");
		//[(sheila corruption < 30)]
		if(flags[FlagEnum.SHEILA_CORRUPTION] < 30) Render.text("little breasts rise and fall with increasing frequency atop her ribcage as her pace picks up and her breaths come short and shallow.");
		else if(flags[FlagEnum.SHEILA_CORRUPTION] > 60) Render.text("huge tits jiggle magnificently as she rocks, nearly slapping her in the face as she plunges down and then falling back into place with waves of rippling flesh.");
		else Render.text("breasts quiver invitingly as she pumps your cock, but before you can think of grabbing one yourself, both of her hands are squeezing and caressing them, drawing sighs from the woman as she toys with her erect nipples.");
		...
		//(horse)
		if(player.isTaur()) Render.text("barrel");
		else Render.text("chest");
		Render.text(" in her soft hair - but her hips keep pumping, incredibly.  ");
		//(pc corr >= 90 or > sheila corruption)
		if(player.stats.cor >= 90 || player.stats.cor > flags[FlagEnum.SHEILA_CORRUPTION]) {
			Render.text("  The tight, rippling muscles of her pussy clamp down on you, nearly holding your cock in place as she tries to ride you still but only manages to jerk your dick up and down and squeeze pre-cum from it.  \"<i>S-so full,</i>\" Sheila groans, \"<i>did you just get bigger in me?</i>\"  Her lungs inflate as she sucks in her breath, and her hard nipples push into your " + player.skinFurScales() + ".");
		}
		else if(player.stats.cor < flags[FlagEnum.SHEILA_CORRUPTION]) Render.text("  Her pussy clamps down around you, the rippling muscles so tight that she wouldn't even be able to pump if it weren't for the hot river of juice you feel around your tool.  \"<i>A-ah,</i>\" Sheila groans, \"<i>you thrust too!  Rail me silly!</i>\"  She tries to push against you with her hands, to return herself to an upright position, but can barely pull her chest apart from you - her nipples just narrowly recede from the surface of your " + player.skinFurScales() + " as her efforts peak.");
		//(else if magically equal but < 90)
		else Render.text("  Her pussy squirts and sucks alternately, trying to find purchase on your cock even as her pumping hips deny it; the sensation drives you wild.  \"<i>[name], your old fella is amazing...</i>\"");
		Render.text("  Your dick twitches and swells, as if to fulfill her words, and your pelvis rocks into Sheila's as you begin to ejaculate.  \"<i>No... pull out, or you'll get me pregnant,</i>\" she cries, when ");
		//[(single cox)
		if(player.lowerBody.cockSpot.count() == 1) Render.text("your dick twitches inside her");
		//(multicox)
		else Render.text("she spies the first bead of pearly semen exiting");
		if(player.lowerBody.cockSpot.count() == 2) Render.text(" your lonely, unused prick");
		if(player.lowerBody.cockSpot.count() > 2) Render.text(" each of your lonely, unused pricks");
		Render.text(".");
		...
		//[(multi)]
		if(player.lowerBody.cockSpot.count() > 1) Render.text(", to be joined shortly by a messy external eruption that covers her thighs and belly in blotches of gooey white");
		Render.text(".");
		//[(big skeet)
		if(player.cumQ() >= 1000) Render.text("  It rolls out of you in waves, packing Sheila's cunt until she fills to capacity and the excess is forced out, sliming your own crotch.");
	...
	//(if no cockFit 32 and yes vag)
		...
		//[(horse)]
		if(player.isTaur()) Render.text("You turn around and straddle a leg, squatting to bring your [vagina] atop her crotch.  \"<i>Push up,</i>\" you order, and Sheila obeys eagerly, twisting her body to connect her excited pussy with yours.");
		//(else if naga)
		else if(player.isNaga()) Render.text("You slide it under her and lift gently; Sheila plays along with a disquieted expression, rolling over and raising her pussy into the air, and you press your own against her butt as your tail slides between her legs.  \"<i>See, you just need to be more... flexible,</i>\" you quip.");
		//(else if not horse and not naga)
		else Render.text("You straddle her legs with your own, pressing her pussy into yours; Sheila gasps as the hot flesh connects and you tentatively rock your hips, dragging your lower lips across.");
		...
		//(sheila corruption > 30)
		if(sheilaCorruption() < 30) Render.text(" even though they're small and humble");
		//(else sheila corruption > 60)
		else if(sheilaCorruption() > 60) Render.text("; I worry that they're getting too mountainous");
		Render.text(".</i>\"  A hand drifts up to her nipple, pinching it as her mouth hangs open.  She rolls it in her palm, then presses the breast against her ribcage, and her shiver of arousal transmits to you through");
		if(player.isNaga()) {
			Render.text(" her taut buttcheek");
			Render.text(".");
			...
			Render.text(" her heated, slippery vagina.");
		...
		//[(naga) 
		if(player.isNaga()) Render.text(" tail, drawing it up the hot opening and coiling it so you can return the warm, lube-slicked length for another pass even as you hump her butt.");
		else Render.text(" cunt, grinding her like a sex toy in search of your own climax.");
		Render.text("  \"<i>Do it!</i>\" you order, anxious to feel her pussy convulsing against you, anxious to feel your own squeezing her.  \"<i>Come with your lesbian lover!</i>\"");
		...
		if(player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) {
			Render.text("  Her vagina doesn't wait for romance, spasming and opening wide to try to suck you in, pulling at your ");
			if(player.isNaga()) Render.text("scales");
			else Render.text("pussy");
			Render.text(" until you can take no more of the heat and come as well.");
			//[(naga)]
			if(player.isNaga()) Render.text("  Her breasts swell in your hands as she sucks in her breath sharply, feeling your pussy drool its juice onto her ass.");
		}
		//(else if PC corr < sheila corruption)
		else if(player.stats.cor < sheilaCorruption()) {
			Render.text("  Her vagina doesn't wait for symbolism, dumping a wave of orgasm that splashes almost audibly against you.  Your pussy answers with your own climax, and begins wetting the woman underneath");
			if(player.isNaga()) Render.text("; it feels like her breasts are pulling away from you as she tries to push her ass further into your quivering quim");
			Render.text(".");
		}
		else {
			Render.text("  Her vagina drools and spasms, not interested in gestures so much as release, and coats ");
			if(player.isNaga()) Render.text("your tail");
			else Render.text("your own");
			Render.text(" with a fresh coat of her hot fluids which trigger the expected response from you.");
		}
		//(if cock)
		if(player.lowerBody.cockSpot.hasCock()) {
			Render.text("  Your neglected ");
			if(player.lowerBody.cockSpot.count() == 1) Render.text("prick quakes and spurts");
			else Render.text("pricks quake and spurt");
			Render.text(" weakly, drooling semen that pools on her skin.");
		}
		//[(not horse)
	...
	//(else if no cockFit 32 and no vag)
		...
		Render.text("Her hand lowers to her crotch and slides her middle finger into her pussy.  \"<i>I don't know what's wrong with me,</i>\" Sheila says, nervously.  \"<i>I'm not even sure why I get hot when I think of you... you don't have anything I can use.</i>\"");
		
		Render.text("\n\nYou smile at the admission");
		if(player.lowerBody.cockSpot.hasCock()) Render.text(", stroking your oversized tool despite her dismissal");
		Render.text(".  \"<i>But you do?</i>\"");
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			...
			//(mans)
			if(!player.isTaur()) Render.text("falls into your lap");
			//(horse)
			else Render.text("drifts between your legs");
			Render.text(", then crawls its way to your [cock].  She begins stroking it, torpidly at first, but with increasing excitement as a drop of pre-cum slides down and wets her palm.  \"<i>What's wrong, goblin girl?</i>\" you ask.  \"<i>Gotta get some sperm no matter what?</i>\"");
		
			Render.text("Sheila winces as your probing fingers send a shudder through her.  \"<i>Shut up, you tosser,</i>\" she retorts, yanking on your dick slightly harder in her irritation.");
		}
		//(new PG)
		...
		//[(pc corr >= 90 or > sheila corruption)]
		if(player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) Render.text("  Her vagina heats up, squeezing your fingers so hard that you nearly can't pull out to rub her again, and you have a clear view of her breasts swelling up - arguable proof of your prowess, considering what she told you earlier.  Her people must be terrible at faking orgasm.");
		//(pc corr < sheila corruption)
		else if(player.stats.cor < sheilaCorruption()) Render.text("  Her vagina gushes with clear fluid, filling the bowl made by your hand with warmth, and you think you can see her breasts tightening up slightly as her back arches, though it's hard to be sure with how she's shaking.");
		else Render.text("  Her pussy quivers around you, and you draw the fingers out and pop them into Sheila's mouth.  The woman sucks on them greedily, and you raise an eyebrow at her.  She blushes again, but it's evidently not the first time she's done the same thing!");
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			...
			//[(big skeet)
			if(player.cumQ() > 1000) Render.text("  Her mouth fills quickly, and she writhes in your grip, managing to turn her head.  \"<i>Oh god, no way!</i>\" she coughs, spitting up enough semen to coat her chin as your ongoing orgasm spatters against her cheek.");
			//[(mega skeet)
			if(player.cumQ() >= 2000) Render.text("  Not satisfied with that alone, you turn her head the other way for your finish, ejaculating on the rest of her face and completing her gooey, white beard as she squirms.");
			//[(multicock)
			if(player.lowerBody.cockSpot.count() > 1) Render.text("  The other strokes not coming from this cock do their part to decorate your lover as well, turning her hair into a slimy mess.");
			Render.text("  She escapes from your hand as you spend yourself, leaning away and gagging up seed onto the dry grass.");
		}
		Render.text("\n\nThe woman falls weakly backward");
		//(cock.length > 0)
		if(player.lowerBody.cockSpot.hasCock()) Render.text(", her face still covered in globby white smears");
		Render.text(".  \"<i>Fuck...</i>\" she groans.  \"<i>You're a jerk, [name].");
		//[(cock.length > 0)
		if(player.lowerBody.cockSpot.hasCock()) Render.text("  I can't believe you called me a goblin, you arsehole.");
		Render.text("</i>\"");
		
		Render.text("\n\nYou ");
		//[(cock.length > 0)
		if(player.lowerBody.cockSpot.hasCock()) Render.text("can't believe how much she acts like one sometimes... but you ");
		Render.text("just laugh and tug her ear; she flicks it and pulls away from you, backhanding you softly in the gut.  \"<i>Knock it off!</i>\"");
		
		Render.text("\n\n\"<i>Are you really so worried about it?</i>\" you ask.  No answer comes");
		//[(cock.length > 0)
		if(player.lowerBody.cockSpot.hasCock()) Render.text(", even after Sheila finishes wiping her face clean and stares at you");
		Render.text(", so you just content yourself with lying next to the woman for a while.");
...
private function sheilaXPThreeSexyTimeGuardDuty():void {
	clearOutput();
	Render.text("You fold your arms and frown at Sheila, but consent to help.  She brightens, then looks confused when you make no further motion.  \"<i>So...</i>\" she begins, \"<i>what should I do?</i>\"");
	...
	//[(PC level >=8)
	if(player.level >= 8) Render.text(" - which, you suppose, must include imps even though they're so far beneath you they might as well be underground");
	Render.text(".  You stand there for so long that you nearly slip into a daydream, and are only roused by the noise of a leather belt being carefully unfastened.  Focused again, you can hear Sheila's ragged breathing and her thighs rubbing together, then a hand sliding against skin and fabric... seems like she's made her decision.");
	...
	//(else lib >= 40)
		...
		//[(cock.length > 0)
		if(player.lowerBody.cockSpot.hasCock()) Render.text("shift your pelvis forward");
		//(cock.length == 0)
		else Render.text("stick out your tongue playfully");
		Render.text(".  The girl giggles at that, relaxing and finally letting herself get into the masturbation.");
...
private function fuckBuddySheilaMeetingMaybeLater():void {
	clearOutput();
	Render.text("You tell her you haven't the time right now, but squeeze her knee as you get up again.");
...
private function fuckBuddySheilaMeetingJustHangOut():void {
	clearOutput();
	Render.text("In answer, you reach down to pinch her butt and then ");
	//[(not horse or drider)
	if(!player.isTaur() && !player.isDrider()) Render.text("scoot your own forward");
	else Render.text("fold your legs under you");
	Render.text(" to lay in the bush's shade.");
...
private function fuckBuddySheilaMeetingChat():void {
	clearOutput();
	Render.text("\"<i>Feeling talkative?</i>\" Sheila asks.  \"<i>Have at yer.</i>\"");
	...
	if(flags[FlagEnum.SHEILA_JOEYS] > 0) addButton(4,"Kids", fuckBuddySheilaMeetingChatKids);
}

//[XP4 - Chat - Family&Friends]
private function fuckBuddySheilaMeetingChatFamilyAndFriends():void {
	clearOutput();
	Render.text("\"<i>Them?  They're doing fine.</i>\"  Sheila stares at you obtusely, trying to will you into changing the subject; you stare right back.  She sighs.  \"<i>No go, huh?</i>\"");
...
private function fuckBuddySheilaMeetingChatOldLoves():void {
	clearOutput();
	Render.text("\"<i>[name], I don't want to talk about that,</i>\" Sheila says, \"<i>it leaves a bad taste in my mouth.</i>\"");
	...
	//(else lib >=50)
		...
		if(player.stats.cor >= 60) Render.text(" - though it's probably meant to be a defensive gesture, all it does is push her tits together under her white shirt and frame them in light-brown skin");
		Render.text(".  \"<i>[name], you horny whacker, I'm not gonna marry you.  Having to bull my way through a horde of monsters to see you and stand watch in shifts just to sleep together at night may be romantic once, but it's no kind of thing to do every day for the rest of my life.</i>\"");
		...
		//[(corr < 30)
		if(player.stats.cor < 30) Render.text("; you lay a hand on her shoulder reassuringly");
		Render.text(".  Eventually she pulls away again, nose slightly red.  \"<i>Sorry, mate; didn't mean to have a tantrum on you.</i>\"");
...
private function fuckBuddySheilaMeetingChatWhyHide():void {
	...
	//(if T'A)
	if(player.statusAffects.get("TelAdre").value1 >= 1) {
		Render.text("  After all, the city in the desert seems to be bustling and bristling with weapons to resist the demons, whatever her feelings may be about it.");
...
private function fuckBuddySheilaMeetingChatLiveWithMe():void {
	clearOutput();
	Render.text("You ask Sheila to move in with you at your camp, and she turns bright red.");
...
private function fuckBuddySheilaMeetingChatKids():void {
	...
	if(flags[FlagEnum.SHEILA_JOEYS] == 1) {
		Render.text("\"<i>Ah?</i>\" Sheila says, turning toward you sheepishly.");
		...
		if(player.femininity > 50) {
			Render.text("I don't know how he'll react when he sees a pretty face like yours, though; he doesn't know that I'm-");
			//[(PC is herm, fem)
			if(player.gender >= 2) {
			...
			Render.text(" you guess, teasing her as her blush deepens.");
			Render.text("\n\n\"<i>[name], stop it.</i>\"");
		}
		//else if PC masculine or andro, no new PG)
		else {
			Render.text("I'm not sure if he intends to shake your hand or punch you, though.</i>\"");
	...
	//if 1 < joeycount < 10
		...
		Render.text("\"<i>So how are the kids?</i>\"");
		...
		if(player.stats.lib > 50) Render.text(" and tight, hot cunt");
		Render.text(" so well...  \"<i>How long does it take them to grow up?</i>\"");
		...
		if(player.gender >= 3) Render.text("nother");
		Render.text(" lady's age, after all.</i>\"");
	...
	//if joeycount >= 10
		...
		Render.text("\"<i>Kids doing all right?</i>\"");
		...
		if(player.stats.lib < 50) {
			...
			if(player.stats.cor < 40) Render.text("  You pat her shoulder carefully and suggest, \"<i>I could watch them for a while.</i>\"");
			...
			//[(silly)]
			if(silly()) {
				Render.text(" and clutching the life-sized [name] bodypillow I made");
				Render.text(".  But it's not going to happen right now.</i>\"");
...
private function fuckBuddySheilaMeetingChatKidsRest4Now():void {
	clearOutput();
	Render.text("\"<i>Always,</i>\" you answer, \"<i>but if you need to relax and sleep for a bit, I'm here for that too.</i>\"  You draw your hand back out, patting her on the cheek.");
	
	Render.text("\n\nSheila kisses you on the forehead and then lays her head against your ");
	if(player.isTaur()) Render.text("back");
	else Render.text("lap");
	Render.text(".  \"<i>You're sweet... thank you.  I will take you up on both of those offers - but maybe just the nap this arvo.  Would you wake me up in about an hour?</i>\"");
...
private function sheilaXP4Sex(display:boolean = false):void {
	if(display) {
		clearOutput();
		Render.text("You slip an arm around the bushranger and lean in.  \"<i>I was thinking we could have some sex... feel up to it?</i>\"");
	...
	menu();
	if(player.gender > 0) {
		if(player.lowerBody.cockSpot.hasCock()) addButton(0,"69WithWang",consensualSheila69,true);
		if(player.lowerBody.vaginaSpot.hasVagina()) addButton(1,"69WithCooch",consensualSheila69,false);
	}
	//[Consensual - Vaginal Sex] (req. cock <= 32 area):
	if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(32) >= 0) addButton(2,"Fuck Her",consentacleVagSexForKangarooSlutBitches);
	//da scene
...
private function consensualSheila69(cock:boolean = true):void {
	...
	if(cock) {
		if(player.cockThatFits(32) >= 0) {
			Render.text("  Though, she smiles openly when you produce your [cockFit 32].");
			...
			Render.text(" as she rubs her small, smooth labia vigorously.  \"<i>What do you expect me to do with one of those?</i>\"");
		}
	}
	//(vag)
	else {
		Render.text("  When you finally drop your bottoms, revealing your wet " + vaginaDescript(0) + ", she smiles softly.");
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 8) Render.text(" and bring your " + clitDescript() + " to its erect state.  \"<i>... And quite a stem,</i>\" your lover concludes.  \"<i>You sure that's not a dick?</i>\"");
		else Render.text(", producing a slight sigh and a wet finger.  \"<i>And I'm the eager one, right?</i>\" your lover asks, wryly - though her finger is just as wet.");
	}
	...
	if(cock && player.cockThatFits(32) < 0) Render.text("  \"<i>[name], I'm telling you it won't fit!</i>\"");
	else Render.text("  \"<i>Not talking, then?  Please yourself.</i>\"");
	Render.text("  Her commentary cuts off as you ");
	if(!player.isTaur()) Render.text("turn around and hang your ");
	else Render.text("straddle her and squat down to dangle your ");
	if(cock) Render.text("pre-seeping shaft");
	else Render.text("wet pussy");
	Render.text(" over her head, waggling it provocatively as you slip your fingers into her.");
	
	Render.text("\n\n\"<i>Ah... ah... you could have just told me,</i>\" Sheila gasps, ");
	//[(cox)
	if(cock) {
		Render.text("wrapping a hand around your shaft");
		//(big) 
		if(player.cockThatFits(32) < 0) Render.text(" and guiding it to bend comfortably against her chest, drooling a trail of pre-cum as she slides it into position");
		Render.text(".");
	}
	//(vag)
	else Render.text("returning the favor and plunging into your " + vaginaDescript(0) + " with her hand.");
	Render.text("  She pumps you once, eliciting a trickle of fluid, and then speaks once more.  \"<i>Bring it lower...</i>\"");
	
	Render.text("\n\nYou oblige, ");
	if(!player.isTaur()) Render.text("flattening your back");
	else Render.text("squatting lower");
	Render.text(" so your ");
	if(cock) {
		Render.text("cock sinks down");
		//[(small cock) 
		if(player.cockThatFits(32) >= 0) Render.text(" into her warm, waiting mouth");
		else Render.text("; she lets go of it to grab her breasts instead, trying to push them around the sensitive shaft as she runs her tongue up the underside");
		Render.text(".");
	}
	else {
		Render.text("pussy dangles just over her face; she ");
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 8) Render.text("nips and sucks at the tip of your clit, rolling it with her tongue");
		else Render.text("parts your labia with her tongue, stroking them gently and tickling your little clitoris");
		Render.text(".");
	}
	Render.text("  Not to be outdone in creativity, technique, or audacity, you ");
	if(!player.isTaur()) Render.text("lean down and bury your own tongue all the way inside Sheila, forcing her labia open with your mouth");
	else Render.text("reach as far as you can to bring both hands into play, stroking and pulling Sheila's labia apart with one as you tease her exposed clitoris or slip inside her vagina with the other");
	Render.text("; your lover stops licking and you can feel her mouth slip away from your sensitive skin as her head rolls back.");
	...
	//[(ladyparts)
	if(!cock) {
		Render.text("slipping her tongue back into you");
		//[(bigclit)
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 8) Render.text(" and stroking your " + clitDescript() + " rapidly with one hand");
		Render.text(".");
	}
	//(smallcox)
	else if(player.cockThatFits(32) >= 0) Render.text("taking you into her mouth once again and licking away a neglected drop of pre-cum.");
	else Render.text("stroking your cock with one hand as she pushes an erect nipple against its side to trace a line.");
	...
	if(!player.isTaur()) {
		...
		if(cock && player.cockThatFits(32) < 0) Render.text("shaking, teasing nipples and heaving chest");
		else Render.text("mouth, humming in delight");
		Render.text(".");
	}
	//(horse)
	else {
		...
		if(cock && player.cockThatFits(32) < 0) Render.text("hard nipples");
		else Render.text("throat");
		Render.text(".");
	}
	//merge horse with rider
	//[(big cox)
	if(cock && player.cockThatFits(32) < 0) Render.text("\n\n\"<i>[name]!  I'm coming!</i>\" Sheila cries.");
	else Render.text("\n\n\"<i>Mmmh!  Mm mmnnn!</i>\" Sheila moans, muffled by your body.  The import doesn't hit you until a second later.");
	Render.text("  Her vagina convulses, ");
	//(pc corr >= 90 or > sheila corruption
	if(player.stats.cor >= 90 || player.stats.cor > flags[FlagEnum.SHEILA_CORRUPTION]) {
		Render.text("trying to pull your ");
		if(!player.isTaur()) Render.text("tongue");
		else Render.text("fingers");
		Render.text(" in ravenously.  You can literally feel her internal muscles rippling and undulating, stroking you as if they could coax out an ejaculation.  Sheila sucks in her breath as she climaxes, filling her chest; you can feel her tits pushing harder and harder against you.");
	}
	//(PC corr < sheila corruption)
	else if(player.stats.cor < flags[FlagEnum.SHEILA_CORRUPTION]) {
		Render.text("pushing out a gush of fluid.  For what seems like several minutes, her pussy deluges your ");
		if(!player.isTaur()) Render.text("mouth, filling you with her taste.");
		else Render.text("hands, completely soaking them in her orgasm.");
		Render.text("   Her thighs tense, forcing her pelvis into the air and her cunt toward you as her chest slumps; slowly, you feel the perky nipples pressing into your stomach pull away.");
	}
	//(else if equal but below 90)
	else if(player.stats.cor == flags[FlagEnum.SHEILA_CORRUPTION] && player.stats.cor < 90) {
		Render.text("drooling a trickle of fluid and licking at you as she screams her ecstasy into your crotch.");
	}
	Render.text("  Sheila jams her face upward, ");
	//[(big cox)
	if(cock && player.cockThatFits(32) < 0) Render.text("pressing her lips and tongue to the base of your shaft as she masturbates it vigorously.");
	///(small cox)
	else if(cock) Render.text("sinking your [cockHeadFit 32] into the warm recesses of her throat where her muscles caress and coax it.");
	else Render.text("penetrating you deeply with her tongue as her hands reach up to stroke your clit furiously.");
	Render.text("  Your body gives up under her frenzied assault, laying your crotch onto the woman's face as muscle control abandons you.");
	...
	if(cock) {
		...
		//[(cock fits)
		if(player.cockThatFits(32) >= 0) {
			Render.text("pouring out into your lover's waiting throat.  She gags and you can feel the gooey seed oozing back against your [cockHeadFit 32]");
			//[(small cum)
			if(player.cumQ() < 250) Render.text("; you withdraw from her, allowing her throat to force your ejaculate back up and over her lips and cheeks with a splutter.");
			//(big cum)
			else if(player.cumQ() < 2000) Render.text(", but your relentless orgasm forces it down again, dumping more jism into her throat.  You can feel her hands clawing at your [butt] as you relieve your buildup.");
			//[(mega cum)
			else Render.text(".  Soon, her visibly-rounding stomach fills and the tip of your dick is once again washed in backflow; the next spurts force the cum all the way up, and she begins drooling the slimy semen-saliva blend from her mouth and nose, hands all the more frantic in her alarm.");
		}
		//(else doesn't fit)
		else {
			Render.text(" squirting onto and between your intertwined bodies to soak your [chest] and neck, and Sheila's softly pumping crotch.");
			//(big skeet)
			if(player.cumQ() > 1000) Render.text("  A wellspring of seed, your body continues producing until the fuzz on her thighs and tail is completely matted down with the whiteness drooling between you.");
			//(multicocks)
			if(player.lowerBody.cockSpot.count() > 1) {
				Render.text("  Your other cock");
				if(player.lowerBody.cockSpot.count() == 2) Render.text(" spasms");
				else Render.text("s spasm");
				Render.text(" as well, adding their own issue to the unholy mess covering the poor woman.");
			}
		}
	}
	//(else if vag-focus)
		...
		if(player.wetness() <= 2) Render.text("trickle out, and are quickly sucked up by her greedy mouth");
		else if(player.wetness() <= 4) Render.text("gush freely, leaving her face slick");
		else Render.text("overflow, soaking her hair even as she shuts her eyes tight against the cataract");
		Render.text(".  As you climax, you wrap around her head, squeezing so hard that you flatten her ears against her skull.  Her hands latch onto your butt, trying to pull you off, but having already spent herself on her throes of passion, she's no match for yours, still ongoing, and you hold tight until your body relaxes.");
	}
	
	Render.text("\n\nSpent, you roll off of Sheila");
	//[(smallcox)
	if(cock && player.cockThatFits(32) >= 0) Render.text(", pulling free of her throat.  She immediately turns over as well and forces herself to her hands and knees, then gags and coughs up a large amount of your jism.  When she's dumped enough to clear her passageway, she collapses forward, smearing the mess onto her chest as she spends a few minutes to catch her breath.  Once she's got it, she turns back over and sits up, strands of your slime linking her breasts to the grass.");
	//(else not smallcox)
	else Render.text(".  She sits up, wiping sweat and stray fluids away from her eyes.");
	...
	if(cock && player.lowerBody.balls > 0) {
		Render.text("What you see manages to pull an explosive laugh from you, tired as you are.  Sheila gives you the stink-eye until you've recovered enough to explain the source of your mirth: on her forehead, making a perfect ");
		if(player.lowerBody.balls == 2) Render.text("infinity");
		else Render.text("double infinity");
		Render.text(" shape against the rest of her skin, is an imprint of your " + ballsDescriptLight() + ".  The kangaroo girl rolls her eyes at the juvenile display and resumes her harangue.  ");
	}
	Render.text("\"<i>You could give a girl a bloody warning before that sort of behavior.  I thought you were going to kill me!</i>\"");
	...
	if(flags[FlagEnum.SHEILA_XP] == 3) {
		Render.text("\"<i>[name]... thanks.  Even being covered in your mess is better than being stuck outside all night, too scared to touch myself and too horny to run.</i>\"  Sheila kisses you on the cheek.  \"<i>My sloppy hero, eh?  I'm okay now; I can make it 'til morning when the gate reopens.</i>\"");
	...
	//(else if sheila xp =/= 3)
		...
		Render.text("\"<i>Forget it.  With the way you worked me, I forgive you.  Just be careful, you wanker.</i>\"  Sheila closes her eyes and pulls your arm around her shoulder");
		if(cock && player.cockThatFits(32) >= 0) Render.text(", smearing your dangling hand into the mess on her tits");
		Render.text(".  The two of you lie like that for a while, until she gets up.  \"<i>I should really go... need to cure my catch.  Not to mention I'll have to clean up before I can even put my damn clothes back on.</i>\"  Sheila picks up her stuff.  \"<i>Hehe... see you soon.</i>\"");
	}
	doNext(camp.returnToCampUseOneHour);
}
...
private function consentacleVagSexForKangarooSlutBitches():void {
	...
	if(sheilaCorruption() <= 30) Render.text("pert");
	else Render.text("jiggly");
	Render.text(" breasts as she fiddles with your own clothing.  She manages to remove the top of your [armor], exposing your [chest]; she then returns your favor by ");
	if(!player.upperBody.chest.hasFuckableNipples()) Render.text("squeezing and fondling");
	else Render.text("plunging two fingers into");
	Render.text(" your own " + nippleDescript(0) + "s.  You shiver as she paws at the sensitive nubs on your chest, but don't let up your assault, moving your mouth down to tickle one of hers with your teeth and tongue.  Soon she's quaking and twitching beneath your ministrations; rubbing at her crotch with one hand as she half-heartedly tries to tease you back with the other.");
	...
	//[(if wangaroo)
	if(player.lowerBody.cockSpot.list[player.cockThatFits(32)].cockType == CockType.KANGAROO) Render.text("tapered, fleshy penis.  \"<i>Wow... you never see those.</i>\"");
	else Render.text(multiCockDescriptLight() + ".");
	...
	Render.text("\n\nYou grin.  ");
	//[(no centaur)
	if(!player.isTaur()) Render.text("Taking hold of your " + cockDescript(x));
	//(centard)
	else Render.text("Attempting to maneuver your " + cockDescript(x) + " into position");
	Render.text(", you rub the head along her dripping snatch, smearing her fluid over it.  ");
	//[(non-cent)
	if(!player.isTaur()) Render.text("Gripping her fuzzy thighs next, you lift her bottom and line");
	//(hoers)
	else Render.text("She lifts her bottom and lines");
	Render.text(" her slippery passage up with your cock.  \"<i>Here it comes!</i>\" you cry.  With that announcement, you thrust into her, parting her lips with your swollen " + player.cockHead(x) + ".  She moans in relief as you finally enter her, but said relief is short-lived as you begin pumping at her slick vagina.  Gasping weakly, she lies back on the ground; ");
	if(sheilaCorruption() <= 30) Render.text("her perky breasts rock back and forth");
	else Render.text("her jiggling breasts flatten into perfect domes and undulate on her chest");
	Render.text(" as you draw in and out of her warm, moist pussy.");
	...
	//[(non-cent)]
	if(!player.isTaur()) {
		Render.text("You pause briefly, adjusting your angle and leaning down to kiss her deeply once again and help caress her ");
		if(sheilaCorruption() <= 30) Render.text("firm");
		else Render.text("pillowy");
		Render.text(" tits, then resume thrusting from your new position; her nipples now brush against your [chest] as you pump.  ");
	}
	//(if balls/vag)
	if(player.lowerBody.balls > 0 || player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("Her stubby tail curls inward toward you, rubbing up against your ");
		if(player.lowerBody.balls > 0) Render.text("[balls]");
		else Render.text(vaginaDescript(0));
		Render.text(" and tickling you as she writhes in pleasure.  ");
		//(if vag)
		if(player.lowerBody.balls == 0 && player.lowerBody.vaginaSpot.hasVagina()) Render.text("Your cunt twitches and drools a bit of fluid on the furry intruder.  ");
	}
	Render.text("You fuck harder, stuffing Sheila with such force that her ");
	if(sheilaCorruption() < 30) Render.text("perky, erect nipples trace long arcs against your skin");
	else Render.text("jiggly tits nearly slap into her face");
	Render.text(".  \"<i>Ohhh,</i>\" she moans, \"<i>YES!</i>\"  At that moment you feel her vagina clamp around your [cockFit 32], wringing it frantically as she reaches her climax.  You continue pumping as best you're able, but you can't stand up to the siege of her spasming pussy for long; ");
	
	//[(pc corr >= 90 or > sheila corruption) 
	if(player.stats.cor >= 90 || player.stats.cor > flags[FlagEnum.SHEILA_CORRUPTION]) {
		Render.text("something builds to a painful intensity in your crotch.  It feels like two mind-blowing orgasms arriving together and fighting over which one gets to come out first.  You rise toward climax quickly and then are drawn back in moments of maddening frustration, over and over.  In response you intensify your thrusts even more, sawing in and out of the girl at a pace that would be blistering if the two of you weren't leaking enough pre-cum and pussy juice to put out a brushfire.  Eventually the pleasure overwhelms the strange reticence of your orgasm; you cry out loudly as you peak, and squeeze a tiny trickle of cum into her grasping vagina.");
		//[(multi)
		if(player.lowerBody.cockSpot.count() > 1 ) {
			Render.text("  Your other ");
			if(player.lowerBody.cockSpot.count() == 2) Render.text("dick erupts");
			else Render.text("dicks erupt");
			Render.text(" as well, festooning her crotch with sticky strings.");
		}
		Render.text("  The intense sensation begins to ascend your dick slowly; your body works itself into overdrive and your partner's moans increase sympathetically as it nears the tip.  Finally, it exits your body with an enormous squeeze, alongside a jet of jism.  Sheila gives a complementary gasp as this pulse enters her, and her pussy convulses in a second orgasm, causing her back to arch.  Her breasts seem to jiggle more wildly than before as she thrashes on the ground in the grip of her pleasure.");
	}
	//(PC corruption < sheila corruption)
	else if(player.stats.cor < flags[FlagEnum.SHEILA_CORRUPTION]) {
		Render.text("your resistance is condemned to die in failure as Sheila lets out a quavering scream of pleasure and wraps her arms and legs around you, pinning you inside.  Her vagina convulses around your cock powerfully; you could swear you feel yourself stretching longer and longer as her rippling pussy pulls you deeper in.  Just when you think that your dick must be a mile long, her whole body tenses up and her fingernails dig into your back.  Wearing a madwoman's expression, Sheila stares open-mouthed at you as her muscles lock and her pussy quivers out its fitful orgasm; even her ");
		if(sheilaCorruption() < 30) Render.text("perky");
		else Render.text("pillowy");
		Render.text(" breasts seem to be held taut, compressed and motionless.  Suddenly she rams her hips into yours violently and throws her head back, and before you can issue a word of protest, her cunt wrings your " + cockDescript(x) + " one last time with almost supernatural heat; your mind clouds over and a wracking orgasm shoots to the base of your spine, echoing back with the first wave of your ejaculation.");
	}
	//(else if equal but below 90)
	else {
		Render.text("your orgasm quickly supersedes your control as she wrings you and rubs her nipples against your [chest].  " + SMultiCockDesc() + " begins to twitch as it unloads its first squirt of semen into her spasming pussy");
		if(player.lowerBody.cockSpot.count() > 1) Render.text(" or onto her thighs and belly");
		Render.text(".");
	}
	...
	//[(non-dog cock)
	if(!player.hasKnot(x)) {
		Render.text("You oblige shakingly, pulling your " + Appearance.cockNoun(player.lowerBody.cockSpot.list[x].cockType) + " out of her clenching pussy with a wet, sucking noise.  A trickle of semen drools from between her lips as you unburden yourself of the rest of your load, squirting it onto her groin");
		//[(big skeet)
		if(player.cumQ() > 500) Render.text(", breasts, and face");
		Render.text(".");
		//[(Huge skeet)
		if(player.cumQ() >= 1500) Render.text("  More and more globs of semen shoot from the end of " + sMultiCockDesc() + ", pasting Sheila to the dry grass as you paint her body white.");
	}
	//(dog cock)
	else {
		Render.text("You attempt to oblige her, but your swollen knot sticks firmly in her squeezing pussy, locking you into place.  After a few fruitless tugs, you give yourself up to your orgasm with an apology, slumping against her heaving chest and helplessly dumping the rest of your load into the twitching woman. ");
		//[(big skeet)
		if(player.cumQ() >= 1000) {
			Render.text("Her belly begins to round as your overproductive gonads push out your massive store of semen");
			//[(huge skeet)
			if(player.cumQ() >= 3000) Render.text(", until it's stretched as far as possible and each successive jet forces out squirts of frothy jism from the tight seal her pussy makes around the knot");
			Render.text(". ");
		}
		//[(multi)
		if(player.lowerBody.cockSpot.count() > 2) Render.text("Your other cocks erupt as well, soaking her crotch and matting down the fur on her thighs with whiteness. ");
	}
	Render.text("Both of your orgasms finish up");
	//(dog-c) 
	if(player.hasKnot(x)) Render.text(" and you finally manage to pull out of her");
	Render.text("; the two of you lie there breathing raggedly for several minutes.  Sheila breaks the silence first.");
	...
	//[(sheila xp = 3)
	if(flags[FlagEnum.SHEILA_XP] == 3) {
		Render.text("  Sheila looks over, thankfulness written on every inch of her brow.  \"<i>[name]... I really appreciate what you did.  I'd rather be knocked up than caught by some nightmare because I was too horny to hide properly until the colony opened - er, not that I want to be knocked up by you, sweet as you are.</i>\"");
	...
	//(else sheila xp =/= 3)
		...
		//[(norm skeet)
		if(player.cumQ() < 500) Render.text("as she strokes the " + player.skinFurScales()  + " of your chest");
		else Render.text("as she wipes off as much jizz as she can");
		Render.text(", then get up and collect your gear.  \"<i>Um... see you later, ok?</i>\" she says.");
		doNext(camp.returnToCampUseOneHour);
	}
...
private function sheilaMutualMasturbation():void {
	...
	{
		CoC_Settings.error("");
		Render.text("ERROR - SHEILA MASTURBATE BROKE, SON");
		doNext(camp.returnToCampUseOneHour);
	}
	target = choices[rand(choices.length)];
	
	Render.text("You snatch Sheila's hat and pack, tossing them away, then slide a hand into the girl's shirt, slipping one of her nipples between two fingers as you nibble on her neck, and use the gentle pressure to push her onto her back.  She sighs, moving a hand under her clothes to caress yours.  \"<i>Ah,</i>\" she pants, letting out a soft puff of air, \"<i>what're we gonna do on the grass, [name]");
	if(silly()) Render.text("-chan");
	Render.text("?</i>\"");
	...
	//[(horse)
	if(player.isTaur()) Render.text(", folding your legs under you,");
	Render.text(" and begin ");
	//[(adequate-length cock)
	if(target == "cock") {
		Render.text("stroking your [cock], pointing it at her as you smear pre-cum onto your fingers");
	}
	//(non-horse vag)
	else if(target == "cunt") Render.text(" sliding your hand to your [vagina], parting your lips with two fingers");
	//(gross-ass nipcunts)
	else if(target == "nipplecunt") Render.text("plunging your fingers into one of your awkward, distended nipples");
	//(Unsexed non-centaur only)
	else if(target == "ass" && !player.isTaur()) Render.text("licking a finger and teasing your asshole with shallow strokes, gently circling the ring");
	//(centaur who hasn't got shit)
	else Render.text("sucking a single digit salaciously, fingerfucking your mouth and coating it with saliva");
	Render.text(".  \"<i>Oh... are you going to touch me, or just play with that?</i>\" Sheila asks, grinning.");
	...
	if(target == "cock") Render.text("You return your hand to your [cock], pumping vigorously to catch up to your lover, spurred onward by the smooth skin she presses into your lips.");
	//(else if nonhorse vag)
	else if(target == "cunt") Render.text("You slide your hand between your [legs] again, intent on recovering the march Sheila's stolen on you, and begin rubbing your [clit] vigorously while sighing into her breast.");
	//(else if RNG disgusting nipplecunts)
	else if(target == "nipplecunt") Render.text("Your hand returns to your chest, slipping inside the sensitive flesh of your nipple, and you groan into Sheila's as a trickle of moisture falls from your chest.");
	//(else if non-centaur asshole)
	else if(!player.isTaur()) Render.text("Your hand reaches back to your [asshole], neglected for too long, and a finger spears your pucker, stroking the nerves inside as you hurry to bring your arousal in line with Sheila's.");
	///(else if fucking nothing)
	else {
		Render.text("You wish you had something to play with as you watch your lover writhe under you");
		//[(silly), even just a paperclip].]
		if(silly()) Render.text(", even just a paperclip");
		Render.text(".");
	}
	...
	//[(if fucking nothing)
	if(target != "cock" && target != "cunt" && target != "nipplecunt") Render.text(", and yours joins her, slipping into her entrance alongside");
	Render.text(".  \"<i>Just... say I'm always yours!</i>\"  She turns you toward her face again, locking you in a kiss, and your ");
	//[(if cock)
	if(target == "cock") {
		if(player.lowerBody.cockSpot.count() == 1) Render.text("prick begins");
		else Render.text("pricks begin");
		Render.text(" to twitch, ejaculating a stroke onto Sheila's midriff and wrist.  The girl groans in your mouth at the new heat, and her body shakes in orgasm.");
	}
	//(vag)
	else if(target == "cunt") {
		Render.text("pussy convulses, ");
		if(player.wetness() < 4) Render.text("drooling");
		else Render.text("squirting");
		Render.text(" around your hand.  As your body shakes and your lips rattle against Sheila's, she shivers and her own orgasm begins.");
	}
	//(nipcunts)
	else if(target == "nipplecunt") {
		Render.text("nipple constricts around your fingers in climax, oozing lubrication onto your partner.  As the drops from your " + breastDescript(0) + " hit her own, Sheila's back arches again and her own orgasm begins.");
	}
	//(jack shit)
	else Render.text("fingers are pulled into her vagina by her muscles as her orgasm begins.");
	//[( pc corr >= 90 or > sheila corruption) 
	if(player.stats.cor >= 90 || player.stats.cor > flags[FlagEnum.SHEILA_CORRUPTION]) {
	...
	//(else if equal corr but under 90)
		...
		Render.text("She breaks the kiss, drawing her hand from her pussy, and holds your head in front of her; her eyes soften with affection as she shudders.");
		//[(not jack shit)
		if(target == "cunt" || target == "cock" || target == "nipplecunt") Render.text("  Emptying your reserves, you allow yourself to fall into her arms, resting your head on her chest.");
	}
	...
	//(else no corruption warning triggered)
		...
		if(target == "cock") Render.text("there, staring at the load you left in her bellybutton");
		else Render.text("together");
		Render.text(".  \"<i>Oh, I had so much fun, mate.");
		//[(if sex =U and sillymode)
		if(player.gender == 0 && silly()) Render.text("  I love you, my Ken doll.");
		Render.text("</i>\"");
		//[(if sheila xp = 3)
		if(flags[FlagEnum.SHEILA_XP] == 3) {
...
private function sheilaCorruptionWarningsChooseFondle():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("erect");
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("wet");
	else Render.text("aching");
	Render.text(".  Even as she goes, you can still see the edges of her amazing, swaying rack around the sides of her torso.");
...
private function sheilaCorruptionWarningListenToHer():void {
	clearOutput();
	Render.text("\"<i>So...</i>\" you answer, your feel frozen in mid-cop, \"<i>what?</i>\"");
...
internal function sheilaGotWhomped():void {
	clearOutput();
	Render.text("Sheila collapses, ");
	//[(HP)
	if(monster.HP < 1) Render.text("completely spread-eagled from her injuries");
	///(lust)
	else Render.text("rubbing her thighs together and smearing around the moisture from her pussy");
	Render.text(".  As you stand over her, she forces herself to her elbows and tries to scoot away from you, fear written on her face.  \"<i>I'm sorry... please, no...");
	//[(sheila xp > -4)
	if(flags[FlagEnum.SHEILA_XP] > -4) Render.text("  I'll forgive you for everything else, but please, not this!");
	Render.text("</i>\" she implores.");
	//[(sheila corruption >= 90)
	if(sheilaCorruption() >= 90) Render.text("  \"<i>I-if you touch me again... god, I can't even think straight anymore!  Don't do this!</i>\"");
	//(lust >= 30)
	...
	menu();
	if(player.lust >= 33) {
		if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(48) >= 0) addButton(0,"Fuck Pussy",rapeSheilasCooter);
		if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) addButton(2,"Forced Lick",forcedSheilaOral,false);
	}
...
private function rapeSheilasCooter():void {
	...
	Render.text("You grab the girl by her fuzzy leg; she tries to kick away from you, but you pull her back, pinning her");
	//[(horse)
	if(player.isTaur()) Render.text(" by planting your forehooves just above her hips");
	Render.text(".  Reaching down, you take the neck of her shirt in your hand, then rip it open, exposing her ");
	//[(sheila corruption < 30)
	if(sheilaCorruption() < 30) Render.text("tiny ");
	//sheila corruption > 60)
	else if(sheilaCorruption() > 60) Render.text("huge, soft ");
	Render.text("breasts, which jiggle from the force.  She tries to cover herself with her arms, but you grab and pull them aside, revealing her chest again.  As you appraise her naked flesh, she looks away, shame coloring her cheek.  Her mouth tightens and her eyes shut as you squeeze her tits roughly, flicking the nipples until they're erect.");
	...
	//[(monster HP < 1)
	if(monster.HP < 1) Render.text("sliding it between her labia and producing a damp spot, then ");
	Render.text("dragging the wet fabric around her vulva.");
	...
	if(player.stats.cor < 50) {
		...
		//[(big skeet)
		if(player.cumQ() >= 500) Render.text("  Soon you've filled her as fully as the shallow penetration will allow, and the panties begin to press back against your cock, oozing onto you whatever semen the porous dam can't contain.");
		//(mega skeet)
		else if(player.cumQ() >= 1500) Render.text("  It sags and bows around the tip of your spear, pulsing with each new stroke as the hemlines leak jizz onto her legs and butt.");
		Render.text("  Idly, you imagine what it must look like inside, each squirt having its momentum arrested by the cloth, drooling lazily into her until the next one displaces it a bit further forward, and wonder if it'll be enough to get her pregnant.  You pull away from the girl, leaving her gasping and once again averting her gaze.");
		
		Render.text("\n\nShe attempts to stand, holding her midriff and looking at you unhappily.  ");
		//[(big skeet)
		if(player.cumQ() >= 500) Render.text("So much cum fills her pussy that it slips out, once again restoring the opacity, though not the dryness, of her panties.  ");
		Render.text("You return her gaze, turning your nose up dismissively.  Conscious of her nudity, she pulls up her shorts and buttons her vest to conceal herself as you leave her behind.");
		...
		if(player.cockArea(x) > 32) Render.text(" and the pain of taking a cock slightly too large for her");
		Render.text(", and she actually rears back to scratch at you with her fingernails.  You catch her hand and hold it, and she responds by spitting at you!  \"<i>Fuck you, arsehole!</i>\" Sheila shrieks.  \"<i>Take it out!</i>\"  Reaching down, you slap her in the face.");
		...
		if(sheilaCorruption() > 10) Render.text(" jiggles her breasts and");
		Render.text(" clenches her jaw.  A bloom of red colors her cheeks, one brighter than the other from the blow... though she still refuses to look at you.  Doesn't matter; having sex.  You begin to pump her pussy vigorously, the wetness enough to allow quick, smooth thrusts without any warmup.  \"<i>You're a godawful liar,</i>\" you grunt");
		//[(not centaur)
		if(!player.isTaur()) Render.text(", leaning down to kiss her long neck and collarbone");
		Render.text(".  Sheila quivers again, still looking away, as you continue.  \"<i>I can actually feel you stroking me with your vagina already.  Are you going to come soon?  Will you tell me how much you're hating this when you do, and wrap your legs around me to punish me by holding me inside?</i>\"  The girl's blush deepens, and she bites her lip in embarrassment.  Confidently, you release her hand");
		//[(nothors)
		if(!player.isTaur()) Render.text(", transferring yours to her breast while brushing her torn shirt to the side");
		Render.text("; without struggling further, she slowly lowers it");
		//[(nothors)
		if(!player.isTaur()) Render.text(" to rest atop your own");
		Render.text(", confirming your suspicion.  Her eyes close and her chin raises resignedly");
		//[(nothors)
		if(!player.isTaur()) Render.text(" as you triumphantly squeeze the flesh in your hand and kiss her under the jaw");
		Render.text(".");
		...
		//[(mans)
		if(!player.isTaur()) Render.text("place a hand along her face, tuning it toward you, and kiss her on the lips.");
		else Render.text("bend down, lowering your torso until you can caress her cheek.");
		Render.text("  \"<i>Of course I won't,</i>\" you answer sweetly.  \"<i>What would be the point?</i>\"  Sheila winces as you hilt yourself, sinking your [cockFit 48] so far into her that your crotch meets hers.  You shudder");
		//[(nonhors)
		if(!player.isTaur()) Render.text(" and squeeze her breast");
		Render.text(" as your climax arrives and the first of your seed pours into her grasping pussy.  While you fill her, her back arches");
		//[(nonhors) 
		if(!player.isTaur()) Render.text(" and she presses your hand uncomfortably");
		Render.text(", gasping as her own orgasm begins.  Her knees wrap around your sides, satisfying your predicition, and her vagina");
		//[(pc corr >= 90 or > sheila corruption)
		if(player.stats.cor >= 90 || sheilaCorruption() < player.stats.cor) {
			Render.text(" pulls greedily at your [cockFit 48], drinking up your semen as fast as you can squirt it.  She sucks in breath as she climaxes, finally opening her eyes as her head tilts back");
			//[(nonhors)
			if(!player.isTaur()) Render.text("; you can almost feel her breast heating up and swelling with blood under your hand, as the nipple pokes into your palm");
			Render.text(".");
		}
		//(PC corr < sheila corruption)
		else if(player.stats.cor < sheilaCorruption()) {
			Render.text(" floods you with juices, dumping so much around your twitching cock that you feel like you're lowering it into a hot spring.");
			//[(nonhors)
			if(!player.isTaur()) Render.text("  She squeezes your hand so hard that you swear you can feel her breast compressing under it as she screams her pleasure.");
		}
		//(else equal but < 90)
		else {
			Render.text(" drools eagerly as you mingle your juices with hers, until the fluid begins to thicken with white seed.");
		}
		//[(big skeet)
		if(player.cumQ() >= 500) Render.text("  Overabundant as always, your semen quickly fills her to capacity and begins to squirt out around your shaft, staining her thighs.");
		//[(multicock)
		if(player.lowerBody.cockSpot.count() > 1 ) {
			Render.text("  Your other ");
			if(player.lowerBody.cockSpot.count() == 2) Render.text("dick spits");
			else Render.text("dicks spit");
			Render.text(" as well, leaving trails all over her light-brown skin.");
		}
...
private function forcedSheilaOral(dick:boolean = true):void {
	...
	//set sheila xp = -4
	flags[FlagEnum.SHEILA_XP] = -4;
	Render.text("You lean down and grab the girl by her shirt-front, raising her to her knees; her hands automatically go to yours.");
	...
	if(sheilaCorruption() >= 50) Render.text("soft, pillowy");
	else Render.text("small, perky");
	Render.text(" breast.  \"<i>You don't want to rip your clothes, do you?  It must be difficult to get nice fabric like this out here in the middle of nowhere.</i>\"");
	...
	if(sheilaCorruption() < 20) Render.text("little round things that trace gentle curves under her loose shirt.");
	//(else sheila corruption < 50)
	else if(sheilaCorruption() < 50) Render.text("healthy mounds that fill out her shirt.");
	else Render.text("swollen globes that bulge over her v-neck and strain at the seams of her shirt.");
	Render.text("  Smiling at the sight, you grab the garment's hem and pull it up roughly.  Sheila blinks dumbly as you expose her erect nipples to the air, then blushes and covers them with her hands.");
	
	Render.text("\n\nStill smiling, you unfasten your [armor] and reveal your crotch.  ");
	//[(cock.length > 0)
	if(dick) Render.text(SMultiCockDesc() + " flops free, half-erect and swelling steadily with blood as you appraise the woman's body.");
	//(vag present)
	else Render.text("Your [vagina] drools slightly, labia parting and [clit] poking up; you imagine what you want to do with it.");
	Render.text("  Sheila watches your face as you slip into a fantasy, half-playing with her nipples - but just when you start to get into your daydream, she turns and tries to scramble to her feet!");
	...
	if(dick) {
		...
		if(player.lowerBody.cockSpot.list[player.shortestCockIndex()].cockThickness <= 6) {
			Render.text("parting her lips to pop neatly between them.  It feels good, but one-sided, and you order Sheila to get to work.");
		}
		else {
			Render.text("jabbing at her undersized mouth with your enormous [cockHead shortest].  \"<i>It's too thick!</i>\" she complains, trying to turn away.");
		...
		Render.text("  The reticent woman brings her lips into play, sucking gingerly, ");
		//start length fork, no new PG
		//[(dick length <= 8)
		if(player.lowerBody.cockSpot.list[player.shortestCockIndex()].cockLength <= 8) {
			Render.text("and the warm embrace of her mouth is welcome stimulation.");
			...
			if(player.lowerBody.balls > 0) Render.text("your [sack]");
			else Render.text("the base of your shaft");
			Render.text(", and the other hesitantly makes its way to her breasts as she gets into the sex.  The woman tweaks her nipple, softly at first but with mounting passion as her attentions on your dick become more lavish.  Catching this from the corner of your eye, you move ");
			if(!player.isNaga()) Render.text("a [foot]");
			else Render.text("the tip of your tail");
			Render.text(" between her knees to her crotch, rubbing her pussy through her shorts; Sheila hums into your shaft as a wet spot appears on the cloth, and begins grinding her hips against the pressure.  You answer her enthusiasm by taking her head in your hands and stroking the surface of her long, pointed ears affectionately with your thumbs.  She flushes, then sinks down on your cock, rubbing the nerves on the slit with the back of her tongue and pulling you closer to orgasm.");
			...
			if(player.lowerBody.balls > 0) Render.text(" and your [balls] tighten up");
			Render.text(".  She pushes against you with her hands, trying to get free, but you just hold her against your crotch as you lean over her head.");
		}
		//(else dick length > 8)
		else {
			Render.text("but this sort of half-assed attention won't get you off any time before sundown.");
			...
			///(else width > 6)
				...
				if(player.lowerBody.balls > 0) Render.text(" and your [balls] draw closer to your body");
				Render.text("; still intent on calling this oral sex, you squeeze Sheila's jaw, forcing her mouth open, then push your drooling slit right up to it as the first squirt of white peeks out.");
			}
		}
		...
		if(player.stats.cor >= 70 && player.statusAffects.get("Infested").value1 == 5) {
			...
			if(player.lowerBody.balls > 0) Render.text("packing your [balls] and driving up your lust in their zeal to multiply");
			else Render.text("deep inside your body, trailing sensation along the nerves in your most intimate places and ramping up your production");
			Render.text(", and you decide to indulge it.  \"<i>Don't worry about rations for the day,</i>\" you cry out, \"<i>I've got more than enough to feed you!</i>\"  Sheila's resigned expression registers this as little more than masculine braggadocio - all the better for your surprise.  Pulling deep inside yourself, you force out a thick load of wormy jism, depositing it directly into the woman's throat; her body tenses at the texture and she tries again to pull away.  As the writhing mass is pushed down her gullet by yet more arriving after it, she begins to struggle violently, hitting you with tired fists and trying to wrench her head free of your grip, even at the cost of her ears.  Luckily, she lacks the energy to do permanent damage to either you or herself after the fight and the cruel face-fucking, and you pour your infested load into her with abandon.");
			//[(big skeet)
			if(player.cumQ() >= 2500) Render.text("  The volume of semen and slimy worms you pack her with is so great that it strains her stomach; you can actually see dozens of the annelids slithering around in a tight ball under the taut skin of her swollen midriff.");
			Render.text("  Eventually you finish and pull out, after pushing up every worm you can.  Your ");
			if(player.lowerBody.balls > 0) Render.text("[sack]");
			else Render.text("body");
			Render.text(" actually feels lighter by a noticeable amount - though you can still feel some deep inside you, reminding you that you'll be full again soon.  On that note...");
		...
		//(else if corr < 70 or PC cannot use Infest at will)
			...
			//[(big skeet)
			if(player.cumQ() >= 1000) Render.text(", and soon you can feel it lapping at the end of your shaft");
			Render.text(".");
			//[(mega skeet)
			if(player.cumQ() >= 3000) Render.text("  Your body's not done with that, though, and keeps pushing out cum until it spurts around your prick and out Sheila's nose in little squirts.");
			Render.text("  Emptying yourself, you pull Sheila's mouth away from your [cock shortest] and drag the slimy member on her chest, smearing her tits with spit and semen and wiping away the last of the mess on her shirt.");
			
			Render.text("\n\nYou pull the soiled garment down over Sheila's breasts");
			//[(big+ skeet)
			if(player.cumQ() >= 1000) Render.text(", though it's so wet with seed now that you can see them almost as well through it");
			Render.text(".  \"<i>This really is a nice fabric.  You should take better care of it.</i>\"  With that parting shot, you leave the queasy-looking woman to digest your load and the humiliation.");
		}
		//end worm fork
		...
		//[(sheila corruption >= 30)
		if(sheilaCorruption() >= 30) Render.text("; her breasts jiggle pleasingly as they settle on her chest, filling you with the urge to play with them");
		Render.text(".  Straddling her, you lower your [vagOrAss] to her face.  \"<i>Lick,</i>\" you order, rubbing your [butt] on Sheila's nose");
		//[(not horse)
		if(!player.isTaur()) Render.text(" as you fondle her tits");
		Render.text(".");
		...
		if((player.lowerBody.vaginaSpot.hasVagina() && player.vaginalCapacity() >= 100) || (!player.lowerBody.vaginaSpot.hasVagina() && player.analCapacity() >= 100)) Render.text("  She keeps hitting the same spot again, too - she's not even bothering to adjust her angle and touch the rest of your stretched opening.");
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00141] > 0) Render.text("  I know a nice boy in the mountains who keeps dogs, and I'm sure he'd love to have a womb like yours for their seed.  By this time tomorrow, you could have a cock in both holes, filling you with hellhound babies since you can't please a person.");
		Render.text("</i>\"");
		
		Render.text("\n\nYou can feel her face heating up under your [butt] as she flushes with shame; seems like you struck a nerve.  You ");
		//[(no horse)
		if(!player.isTaur()) Render.text("pinch her nipple for emphasis, then grind your hips across her face, mauling her nose with your ass.  \"<i>I could fuck a tree and it wouldn't be as wooden as you in the sack");
		//[(PC has found corrupt glade)
		if(player.exploredForest >= 40) Render.text(" - in fact, there are quite a few I've seen who look like better lovers than you.  Maybe I should carry you to the forest and tie you to a nice pussy-shaped giant flower to give you lessons");
		Render.text(".  ");
		//[(minotaur addiction score =/= 0%)
		if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_TRACKER] > 0 || player.perks.has("MinotaurCumAddict")) {
			Render.text("Perhaps I'll turn you over to a minotaur; you're bound to get better at using your tongue when you're addicted to their amazing seed.  Would you like that?  The first thing you'd see in the morning would be a huge flared cock, right in front of you, dripping with heavenly precum and waiting for your mouth... or your cunt.  You could be just another minotaur-fucking slut of the plains.  ");
		}
		Render.text("Or I could take you home and give you personal training myself, whenever I feel like it, and invite travelers to do the same");
		//[(PC has Izma with cock)
		if(izmaFollower() && flags[FlagEnum.IZMA_NO_COCK] == 0) Render.text("; I personally know fifteen inches of fat, orange dick and four heavy balls that'd love to get in at your other end while I ride your face");
		Render.text(".</i>\"");
		...
		if(player.statusAffects.has("HairdresserMeeting") || flags[FlagEnum.LUMI_MET] > 0) {
			Render.text(" while imagining her turned over to the goblins for experiments, forced to drink potion after potion - most would be dedicated to growing cocks on her or increasing semen production, of course");
		}
		Render.text(".  Your hole clenches and drools on her, and the woman's rude tongue retreats from the pressure, leaving you painfully empty.");
		//[(no horse)
		if(!player.isTaur()) Render.text("  Even squeezing her breasts viciously can't compel her to reinsert it, though you amuse yourself trying.");
...
private function sheilaGetsRapedWithADildont():void {
	...
	if(player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) {
		Render.text("Your face heats as she kisses you sloppily, and not all of the temperature is from her mouth.  It feels like she's pulling energy right out of you, and you can swear that her breasts are growing.");
	}
	//(else PC corr < sheila corruption)
	else if(player.stats.cor < sheilaCorruption()) {
		Render.text("Her lips heat up and your body tingles; you can see her breasts shrinking as she shivers and slobbers on your face - or so it seems from the angle.");
	}
	//(else if magically equal but < 90)
	else Render.text("The kiss is wet and indecent as she shudders through her anal orgasm; her lips don't stay in one place, instead tracking drool all over your face.");
	...
	//(cock)
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  \"<i>Perhaps next time you'd like to try a real cock in there?</i>\"  Even exhausted and messed up on goblin drugs, she has the presence of mind to shake her head at that.  You laugh.");
...
internal function getBeatUpBySheila():void {
	clearOutput();
	Render.text("You slump down before Sheila, breathing heavily; the woman ");
	if(monster.lust < 75) {
		Render.text("takes the opportunity to grab her catch and take off running!  Even if you wanted to, you couldn't catch her now... you collapse, head swimming in ");
		if(player.HP < 1) Render.text("colorful spots");
		else Render.text("fantasies of her, stark naked but for her hat and waiting with her legs open");
		Render.text(".");
	...
	//(else monster lust >= 75)
		...
		Render.text("stares at you, desire and nerve at war in her face.  Her thighs rub together involuntarily as she tries and fails to will herself away... slowly, she takes a step toward you.");
...
private function sheilaCowgirlRapesYou():void {
	clearOutput();
	Render.text("Sheila pushes insistently, and you allow her to force you to the ground and then onto your back");
	if(player.isTaur()) Render.text("; your hooves stick awkwardly out to one side");
	Render.text(".  She blushes furiously and won't meet your eyes, but it doesn't stop her from stripping off her vest and shirt in one smooth motion, revealing her " + sheilaCup() + " chest.  \"<i>T-touch me,</i>\" she demands, \"<i>rub me.</i>\"  Almost as an afterthought, she adds \"<i>... please.</i>\"");
	
	Render.text("\n\nYou move your hands to her smooth skin, cupping a ");
	if(sheilaCorruption() < 30) Render.text("petit ");
	else if(sheilaCorruption() > 70) Render.text("massive ");
	Render.text("breast in each, squeezing them softly and rubbing the hard nipples with your thumbs.  Sheila shudders as you amuse yourself with her body, placing her hands atop yours and guiding you to her favorite spots.  You watch her as she moans; your gazes meet as she rolls her head, and she quickly restrains herself and looks away again.  \"<i>Stop staring at me...</i>\"");
	
	Render.text("\n\nEager to see anything but your curious face, Sheila pushes your hands away from her breasts and turns to your crotch.  She ");
	if(player.isTaur()) Render.text("pulls your legs apart");
	else Render.text("unfastens and strips off your [armor]");
	Render.text(", revealing your " + multiCockDescriptLight() + "; ");
	//[(if no cock fit 32)
	if(player.cockThatFits(32) < 0) {
		Render.text("her eyes widen as " + sMultiCockDesc() + " bobs free, half-erect monster meat that fills her with frustration.");
		...
		//[(horse)
		if(player.isTaur()) Render.text(", or at least as flat as you can get it");
		Render.text(", then sits on your stomach");
		if(player.isTaur()) Render.text(", sliding her long legs between yours");
		Render.text(".  She scoots forward slowly, and gasps as her pussy comes to rest against your hot, blood-engorged shaft.");
		...
		if(player.isTaur()) Render.text("forelegs fold over her torso affectionately");
		else Render.text("hands search out her breasts, caressing and squeezing");
		Render.text(" as her gasps and the picture in your head both intensify.  Sheila grabs one with her own hand, pulling it up to her mouth to smother it in kisses");
		if(!player.isTaur()) Render.text("and suck on your fingers");
		Render.text(".  \"<i>Oh... [name]...</i>\"");
		...
		Render.text("Sheila shivers and leans back, hair dangling, as her hips buck and her thighs squeeze your shaft.  ");
		//(PC corr >=90 or > sheila corruption)
		if(player.stats.cor >= 90 || sheilaCorruption() < player.stats.cor) {
			Render.text("You can feel your prick heating up where her pussy rubs against it... it almost feels like the rest is cooling down at the same time.  The woman atop you arches her back and sucks in her breath sharply, and her tits seem to claim your field of vision, swelling up - though that could just be the angle.");
		}
		else if(player.stats.cor < sheilaCorruption()) Render.text("Her orgasm wets your prick, coating it with fluid as she comes.  Sheila wraps herself tighter in your limbs, squeezing her breasts against her chest, and you think you can feel them get a little smaller as your head fuzzes over with her excess emotion.");
		else Render.text("Her hips lock up when she comes, leaving you to try and finish yourself as she heats up around your shaft.");
		...
		if(player.stats.sens >= 50) {
			...
			//[(mega skeet)
			if(player.cumQ() >= 2000) Render.text("  The giggles and the strokes stop long before your orgasm, though; eventually Sheila just lies there, dumbfounded, as you deposit layer after layer of white lattice on her skin.  \"<i>Bloody hell,</i>\" she mumbles vaguely, either stone-sober or drunk from the sight of it.");
			Render.text("  Once the rain of semen peters out, she pulls apart from you, slimy and dripping.");
	...
	//(else if cock fit 32)
		...
		Render.text("as you lean over to catch her expression, you can see her eyes glimmer with desire.  Sheila notices you from her peripheral vision, and her face reddens again.  She turns her back to you.");
		...
		//[(multicock)]
		if(player.lowerBody.cockSpot.count() > 1) Render.text("separating out and ");
		Render.text("stroking your [cockFit 32].  A drop of precum forms, and she touches it with the index finger of her free hand, sending a tingle through you; she pulls the finger away, drawing a string of slick moisture out.  Sheila passes the finger between her breasts, wiping the fluid away on her chest and shivering.  The display is too much for you, and you reach forward, navigating your hands around her waist quietly until you can reach the buckle of her belt.  The girl sighs as you begin to unfasten it, but an errant thought brings her back to reality - she grabs your hands abruptly.");
		...
		//(else lib >= 25)
			...
			if(!player.isTaur()) Render.text(", and your hands go to her waist, fairly pushing her up and slamming her back down in your eagerness");
			Render.text(".  She rolls her head as the fuck gets wetter and faster, dangling her hair over her shoulders as her back muscles shift and flex with her strokes; her ears flick whenever she descends and rubs your neck against one of the sweet spots on her vulva.");
			...
			//(PC corr >= 90 or > sheila corruption)
			if(player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) Render.text("You can feel her cum-slick inner walls pulling at you as she begins to orgasm, and your mind clears as you blow your load inside her, wondering when she'll likewise return to sense.");
			//(else PC corr < sheila corruption)
			else if(player.stats.cor < sheilaCorruption()) Render.text("Her pussy gushes with her orgasm, pushing out your semen and soaking her inner lips even as she continues riding your cock, and your mind clouds as you think of little except sneaking as much of your cum inside her as possible.");
			//else equal but < 90)
			else Render.text("Even as she comes mindlessly something makes you hold in your own moans, wondering how long it will take her to notice her pussy being filled with your egg-hunting sperm.");
			//[(small cum)]
			if(player.cumQ() <= 250) {
				Render.text("  Sheila continues moving her hips even as she comes, lost in bliss; on one particularly lofty ascent, your cock flops out, squirting a line of jizz along ");
				if(player.tailType > TAIL.NONE) Render.text("your tail");
				else Render.text("the grass");
				Render.text(", and her eyes go wide.");
			}
			//(big cum)
			else {
				Render.text("  With how much Sheila is still moving her hips, it's only a matter of time before she notices the mounting pressure in her cunt... on one particularly deep push, she forces out a squirt of white, and her eyes go wide as she pulls off and lets you finish on the grass.");
			}
			...
			Render.text(" you groan.  \"<i>Can't believe you didn't notice...</i>\"");
...
private function sheilaForcedOralGeneric():void {
	clearOutput();
	Render.text("Sheila kisses you once more, forcing her tongue into your mouth, and slides a hand into your [armor]; you stare wonderingly into her affectionate, hungry eyes at the sudden change.  It isn't until ");
	if(player.lowerBody.cockSpot.hasCock()) Render.text("her hand slips around your [cock]");
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("her fingers slip between your labia");
	else Render.text("she rubs your bare crotch for a few seconds");
	Render.text(" that her reverie breaks; her face colors and she looks away, averting her gaze in shame.  She draws her hand back out guiltily, then begins to doff her shorts and panties.  \"<i>S-sorry... didn't mean to get familiar.  I'll have this over with quickly.</i>\"");
	
	Render.text("\n\n\"<i>That's-</i>\"  Your words are cut off as she suddenly climbs atop you, bringing her pussy to eye level");
	if(player.str < 61) Render.text("; her weight bears you slowly to the ground");
	Render.text(".  Sheila rubs the smooth slit on your [face], trying to part her labia on you, and shivers a bit when your lips brush hers.");
	...
	if(player.upperBody.head.hairType == 4) Render.text(" and flushing as your stingers release their payloads into her palm");
	Render.text(".  You obediently stick out your tongue");
	if(player.stats.cor > 60) Render.text(", motivated more by whimsy than the meager, almost playful pain,");
	Render.text(" and shallowly penetrate Sheila's vagina with it.");
	...
	if(player.tongueType > TONUGE.HUMAN) {
		Render.text("  A sly look crosses your face at her request; she notices it but has no time to dismount before your long tongue shoots into her, stroking the nerves around her entrance as you unspool.  \"<i>Ah-ahhh!</i>\" she moans, \"<i>What the hell... is with your mouth?</i>\"");
	...
	//[(PC corr >= 90 or > sheila corruption)]
	if(player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) {
		Render.text("strangling your tongue with muscles attempting to lock onto it, trying to pull it in.");
		//[(demon/dragon tongue)
		if(player.tongueType > TONUGE.HUMAN) Render.text("  As tired as you are, you simply go along with it, allowing her rippling walls to slowly draw your tip deeper inside until it bumps against her cervix; she shivers and kisses the top of your head as you groggily run your tongue over the entrance to her womb.");
		else Render.text("  However, it doesn't get very far; you lazily flop it around inside her as she works through her arousal.");
		Render.text("  Sheila groans loudly and your vision fades; through the fog, you can swear you feel her breasts growing larger against your forehead.");
	}
	//(else PC corr < sheila corruption)
	else if(player.stats.cor < sheilaCorruption()) {
		Render.text("filling your mouth with her liquid love.  Your lips and throat heat up as you're forced to drink it from the nearness of her pussy; as she bucks her hips weakly into your face, trying to draw out her climax, you increasingly wish that you could return the favor, ");
		if(player.lowerBody.cockSpot.hasCock()) Render.text("face-fucking her and blowing a load into her mouth, then holding her jaw shut until she swallows it unhappily.");
		else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("smearing your gash on her face until you come and drown her in your squirting girl-jizz.");
		else Render.text("forcing her to tongue your asshole until you come, writhing and pressing her head into the grass with your [butt].");
	}
	else Render.text("and she pets your head affectionately as her walls squeeze your tongue.  \"<i>Oh, mate...</i>\" she whispers lovingly, completely forgetting the struggle that led to your tryst.");
	...
	//[(str < 61)
	if(player.str < 61) Render.text("climbing off and kneeling next to you.");
	else Render.text("letting herself down your body until she can put her long legs on the ground once more.");
	Render.text("  She looks into your eyes contentedly, rubbing your cheek... but then starts and looks away, guilty once more.  \"<i>Shit...</i>\" she mumbles.  \"<i>I did it again.  Even after I promised myself I wouldn't...</i>\"");
...
private function normalSheilaPregNotifNumberOne():void {
	...
	clearOutput();
	Render.text("Traipsing through the grass, you can see Sheila sitting under the shade of a low tree from a long way off.  The reverse is also true, as she waves to you from her seat, beckoning you over.  You make your way to her curiously.");
	...
	if(player.stats.cor < 50) Render.text("  A hand placed around her shoulder seems to steady her a bit.");
...
private function normalSheilaPregNotifNumberOneYepIssue():void {
	clearOutput();
	Render.text("\"<i>I see,</i>\" Sheila says, nonplussed.  \"<i>I'm sorry you feel that way...  Truth be told, I'm not keen myself on the idea of having a kid, but what's out is out.</i>\"");
...
private function normalSheilaPregNotifREPEATEDED():void {
	clearOutput();
	Render.text("Sheila is here as you step into the plains, with a grass-wrapped parcel of food open on her lap and her hat sitting upturned next to her, and waves you over.");
	...
	if(rand(10) < 7) Render.text("healthy.  It's eating a lot... frankly, it's a little hellraiser with a strong grip, and it cries well, too.</i>\"");
	else Render.text("weak... we thought we were going to lose it for a bit.  It wouldn't drink from the nurse unless I was there, so I had to stay up with it all night myself.  Feeling a bit stuffed.</i>\"");
	Render.text("  She looks over at you, giving you the benefit of her fatigue-lined eyes.");
	
	Render.text("\n\nSheila slips her hand into yours");
	if(player.stats.cor < 40) Render.text(", and you give it a squeeze");
	Render.text(".  \"<i>I just wanted to tell you about your baby.  Can't stay... have to catch up to quota still.</i>\"  She drags to her feet and turns to go, tail listlessly hanging.  Even her ears are drooping; this girl is tired.");
...
private function normalSheilaPregNotifREPEATEDEDLetHerGo():void {
	clearOutput();
	Render.text("Nothing you can do about that - even if you wanted to watch your baby for her, she's keeping it hidden in a hole in the ground.  You hand her the parcel back");
	if(player.stats.cor < 40) Render.text(", imploring her to eat the rest and keep up her strength");
	Render.text(".  Sheila smiles weakly and takes it, folding the grass around again.  \"<i>Thanks, [name].  I'll see you soon... ish.</i>\"");
...
private function normalSheilaPregNotifREPEATEDEDWalkWithHer():void {
	clearOutput();
	Render.text("You pick up her forgotten hat and rise as well, carefully refolding the grass around Sheila's uneaten lunch and tucking it back into her pack for her.  She takes her hat back from you distantly.");
	...
	Render.text(" next to her, easily keeping pace with the tired woman, and she raises her face to you inquiringly.  \"<i>What's up, [name]?</i>\"");
...
private function normalSheilaPregNotifREPEATEDEDHelpABitchOut():void {
	clearOutput();
	Render.text("You pick up her forgotten hat and set it on her head, then fold her lunch back into its grass wrapper and tuck it into her pack.");
	...
	//(else spd >= 80 and Runner perk)
		...
		if(player.stats.lib < 50) Render.text("you pinch her ear, provoking an annoyed flick from it.");
		else Render.text("you slap her on the ass, grabbing a handful and taking her mind off her own misery.");
		Render.text("  \"<i>Dammit, [name]... not now,</i>\" she frowns, pulling away from you.");
		...
		if(!player.isGoo()) Render.text("run");
		else Render.text("glide");
		Render.text(", making a long loop around to the rear of the wary animal, then begin to systematically drive it closer to your partner, who has strung a line to the base of a shrub and is gesturing at you to force the hare into it.");
...
private function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDWELP():void {
	clearOutput();
	Render.text("\"<i>Welp,</i>\" you say softly.");
	//[(corr <50)
	if(player.stats.cor < 50) Render.text("  You set a hand on Sheila's shoulder to steady her a bit as the tears start to fall.");
	Render.text("  \"<i>It'll be alright.</i>\"");
...
private function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE():void {
	clearOutput();
	Render.text("With a wan smile, you pull the woman to her feet.  She doesn't resist, but continues looking into your face disconsolately.  \"<i>I'm so stonkered I can't even think what to do first...</i>\"");
	...
	if(player.isNaga()) Render.text("coil");
	else Render.text("seat");
	Render.text(" yourself, then grabs your arms and pulls them around her waist.");
	
	//forks in order of priority, no new pg at start
	//[(full gooey body)
	if(player.isGoo()) {
		Render.text("  She considers for a moment, rubbing away some of the slime you've deposited on her shorts, and pulls free from your slimy skin again.  \"<i>Um... do you... would you be offended if I took my kit off, so it doesn't get ruined?</i>\"");
		if(player.stats.lib >= 40) Render.text("  The leer you display is so prurient that she seems on the verge of reconsidering, but you compose yourself and");
		else Render.text("  You");
		Render.text(" give assent with little more than a broad smile.  Sheila pulls her shirt over her head, revealing her " + sheilaCup() + " breasts, but raises her face to you hesitantly when her hands get to her belt.");
	...
	//(naga)
		...
		if(player.stats.lib >= 40) Render.text(" - but tighten the one sliding across her breasts just slightly.  Sheila's neck turns red and her ears twitch as you rub her nipples, and you plant a kiss on her nape");
		Render.text(".  \"<i>Ahh,</i>\" she sighs, shifting in your coils.  \"<i>Hold it right there...</i>\"");
	...
	//(horse and >=5ft)
		...
		if(player.stats.sens >= 40) Render.text("... or would be, if you weren't distracted by the nipples poking into you and the thighs squeezing your flank");
		Render.text(".");
	...
	if(!player.isGoo()) Render.text("blood");
	else Render.text("goo");
	Render.text(" flowing, unsure if you'll be able to get to sleep, but Sheila has no such problems.  Within minutes, the woman is out, head nodding forward.  You suppose that means the first watch is yours... it passes slowly, despite the scenery, but eventually you nudge your partner awake when you can stay up no longer.");
	...
	if(!player.isTaur()) Render.text("  She grudgingly repositions, allowing you to rest against her.");
	model.time.days++;
	model.time.hours = 2;
	statScreenRefresh();
	//--Next--
...
private function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE2():void {
	...
	model.time.hours = 5;
	statScreenRefresh();
	Render.text("Your sleep is fitful, but not totally useless, and you yawn and stir a few hours later when Sheila wakes you");
	if(player.isGoo()) Render.text("; she's already dressed and appears to have just been outside");
	Render.text(".");
	...
	if(player.lust < 30) Render.text("  You give her a hand, then help her load up.  She gives you a peck on the cheek before she departs.  \"<i>Thanks, love.</i>\"");
	if(player.lust >= 30) {
		Render.text("  Part of you is actually sad to see your cozy hovel being disassembled if it means her warm body won't be pressed up against yours anymore, and your hands begin to rove over her as she works.");
...
private function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDEREBUTLEAVEHERBE():void {
	clearOutput();
	Render.text("You relent, allowing the woman to free herself with a peck on your cheek.  She loads up her catch, and then turns to you.  \"<i>Thanks, [name].  Owe you one.</i>\"");
...
private function normalSheilaPregNotifREPEATEDEDHelpABitchOutTOCAMP():void {
	clearOutput();
	Render.text("Wordlessly, you lift the bundle of game animals off of Sheila's shoulders and sling it over your own.  She looks up at you, confused enough to forget about her tears.  \"<i>What're you doing?</i>\"");
	...
	//[(int >=60)]
	if(player.stats.int >= 60) Render.text("\"<i>The baby will be fine,</i>\" you venture, hazarding a guess.  \"<i>It's what the nurses are there for.</i>\"");
	else Render.text("You slip an arm around her, and ask if she's okay.");
	Render.text("  She nods mutely and pulls the food out of the fire.");
	//[(if Jojo present)
	if(player.statusAffects.has("JojoNightWatch") || player.statusAffects.has("PureCampJojo")) Render.text("  She extends one bird to you and another to Jojo.  He raises his hand and shakes his head, but politely.");
	else if(flags[FlagEnum.ANEMONE_WATCH] > 0) Render.text("  Sheila offers you one of the birds, then extends another to your odd tenant.  The anemone's mouth tightens and she shakes her head vigorously, blushing a deep blue.");
	Render.text("  You eat in silence until the food is gone, and Sheila clears her throat.");
	...
	//[(if no watch)
	if(player.findStatusAffect(StatusAffects.JojoNightWatch) < 0 || player.findStatusAffect(StatusAffects.PureCampJojo) < 0 || flags[FlagEnum.ANEMONE_WATCH] == 0) Render.text("\n\nShe gets to her feet as you grin, then makes her way over to your bedroll.  It doesn't take her long to fall asleep; you amuse yourself watching her toss and turn... and drool on your pillow.");
	//(if watch)
	else {
		Render.text("\n\nYou grin and get up, and Sheila tails you to your bed.  ");
		if(flags[FlagEnum.SLEEP_WITH] != 0 && flags[FlagEnum.SLEEP_WITH] != "") Render.text("She looks with some distaste at the nearby sleeper, and steps past.  You pick up the bedroll and follow her, reasoning that if she keeps her promise to be gone before dawn, it shouldn't raise any questions.  \"<i>Making friends, I see,</i>\" Sheila mutters... but as you unroll the bed again, her eyes zero in on it like a drowning man's on a raft, and she says nothing else.  ");
		Render.text("She lies down first, allowing you to cozy up to her as far as you dare.  Just as you're about to drift off, you feel Sheila take your hand and pull your arm over her body.");
	}
...
private function sheilaGoesDemon():void {
	...
	if(flags[FlagEnum.SHEILA_XP] < -2) Render.text("always so bloody eager to fuck me!");
	else Render.text("me best mate!");
	Render.text("  Help out!</i>\"");
...
private function demonSheilaEncounter():void {
	clearOutput();
	Render.text("A shadow drops out of a low tree behind you with a rustle.  \"<i>Hey, lover; been a while... how about spending some time with me?</i>\"");
...
private function demonSheilaResist():void {
	clearOutput();
	Render.text("You back away from the demon and raise your [weapon] with a dour glower.  Sheila purses her lips at you, then says, \"<i>Not in the mood, eh mate?  I'm so horny though... I bet I could get you into it.  Let me show you how much I love you...</i>\"  Her ears twitch furiously, belying her intentions as she");
	//[(sheila corruption < 30)
	if(sheilaCorruption() < 30) Render.text(" thrusts her black spade forward and smears some of the constantly-oozing fluid from its tip down the length with her hand");
	else Render.text(" kneads her breasts seductively, pinching her nipples");
	Render.text(".  Your posture softens a bit as she reaches out to you");
	if(sheilaCorruption() < 30) Render.text(", tilting her palm to dribble a handful of strong-scented dark oils");
	Render.text(", but as she giggles sadistically and starts to approach, you tense up again.  It's pretty clear by the way she swaggers dismissively that she isn't taking the rejection well!");
	//go to fight
	startCombat(new Sheila());
}
...
private function demonSheilaTalk():void {
	clearOutput();
	Render.text("Questions nag at you, and you ask Sheila if she wouldn't mind answering a few of them.  She struts over and ");
	//[(not horse)
	if(!player.isTaur()) Render.text("takes your arm in hers, then slips the other hand into your armor");
	//[(horse)
	else Render.text("between your legs");
	Render.text(" to touch your ");
	if(player.lowerBody.cockSpot.hasCock()) Render.text("[cock]");
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("[vagina]");
	else Render.text("smooth skin");
	//[(lib <20)
	if(player.stats.lib < 20) Render.text(", giggling as your expression petrifies");
	Render.text(".  \"<i>Anything for you; I'll just find something to occupy my attention.  What's on your mind, love?</i>\"");
...
private function askDemonSheilaAboutDahChange():void {
	...
	if(flags[FlagEnum.SHEILA_CITE] >= 1) {
		Render.text("Sheila blushes deeply.  \"<i>Silly.  You were there.</i>\"  She looks at you, pupils dancing as she remembers the lovemaking session that led to her current form.  \"<i>I suppose that's not what you're asking, though.</i>\"");
		...
		if(player.lowerBody.cockSpot.hasCock()) Render.text(", and you feel a monument of your own being erected");
		Render.text(".  \"<i>Every squeeze of my pussy compressed my focus, too, until my mind was clearer than the juice you made me flow with.  I realized my purpose there, with you.</i>\"");
...
private function askDemonSheilaAboutHerColony():void {
	...
	Render.text(", you ask what happened to him.  Sheila smiles serenely before answering.  \"<i>Unsurprisingly, his prick was all too eager to fill me with sperm after the countless sermons he preached about it being every woman's duty to expand the clan.  He barely lasted three minutes when I climbed atop him.  Oh, that little squirt was the most satisfying one.  Watching him melt under me drove home how much power I've always had... I wish I could have known when I was just an awkward little girl, sitting through his browbeating lectures on loyalty and servitude.</i>\"");
	...
	//(corr >=75)
	if(player.stats.cor >= 75) Render.text("harvested that day, you sigh wistfully");
	else Render.text("stolen that day, you wince");
	Render.text(".  Sheila, peeking at you with one eye from under her bangs, interprets your expression correctly.  \"<i>Wondering where the others went, love?</i>\"  She releases her grip on your arm and holds out the hand that's not exploring your [armor], displaying several rings.  \"<i>The rings are my closest.  I kept them for sentimental reasons.  The rest, well... I ate them.</i>\"");
	...
	//[(cock)
	if(player.lowerBody.cockSpot.hasCock()) Render.text("she ties you down and rides you until she's so full of your spunk that she can push it back out just by pressing on her stomach");
	else Render.text("she binds your hands and sits on you, teasing your helpless body with her oozing, throbbing spade until she comes all over you, spraying dark slimy fluid from it");
	Render.text(".  You ");
	if(player.stats.cor < 50) Render.text("break eye contact, blushing");
	else Render.text("stare into her shining eyes, getting hotter and hotter as the vision plays out to the very end");
	Render.text(".");
...
private function talkToDemonSheilaAboutWhatNow():void {
	...
	Render.text("\"<i>What will I do now?  What else, my silly love?</i>\"  Sheila looks out over the plains.  \"<i>I'm always hot for you.  We could have a bit of a naughty, and make each other feel good.</i>\"");
	...
	//(lib>=30)
	if(player.stats.lib >= 30) Render.text("\n\nYour eyebrows rise.  \"<i>Got your attention, did I?  But,</i>\"");
	else Render.text("\n\nYour mouth hardens.  \"<i>You're right,</i>\"");
	Render.text(" Sheila says, \"<i>I'm going to save that one for our honeymoon, instead.</i>\"  Her eyes twinkle with infatuation.  \"<i>I could even pull the taint of this land out of your body and use it to make mine more lewd for you, my " + player.mf("knight-champion","great lady") + "...</i>\"");
	...
	if(player.stats.cor < 50) Render.text("; even though you hide it, I've seen it lurking");
	Render.text(".  Feel like flying your colors for a while?</i>\"");
...
private function demonSheilaTalkAnotherQuestion():void {
	...
	//(if lust <=99)
	if(player.lust <= 99) {
		Render.text("Sheila reaches farther down, ");
		//(cock)
		if(player.lowerBody.cockSpot.hasCock()) Render.text("slipping more of her finger and thumb around your penis");
		else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("parting your lips more");
		else Render.text("rubbing your bare groin");
		Render.text(" even as she smiles encouragingly.  \"<i>Sure, mate.  Amuse yourself.  I certainly intend to.</i>\"");
...
private function sheilaLetHerFuckYou():void {
	...
	if(sheilaCorruption() >= 40) Render.text("pillowy ");
	else Render.text("pert ");
	Render.text("breasts against you.");
...
private function demonImpChildren():void {
	clearOutput();
	Render.text("\"<i>[name]!</i>\"  A swishing in the grass behind you is the only prelude you get to a nude Sheila pressing herself against you in a hug, ");
	//(sheila corruption < 40)
	if(sheilaCorruption() < 40) Render.text("her perky nipples rubbing into your arm as she snuggles up");
	else Render.text("her enormous, pendulous breasts practically parting around you to wrap you up alongside her arms");
	Render.text(".");
	...
	//(if no cock)
	if(!player.lowerBody.cockSpot.hasCock()) Render.text("  Or, had...</i>\" she finishes with a frown as she reaches your bare groin.  \"<i>Where'd you leave that magnificent old fella?  I was lookin' forward to another go.</i>\"  She pulls her hand out, then leans toward you with a pout.  \"<i>I'm so horny, too...</i>\"");
	//(if cock)
	else {
		Render.text("  Still, it's not the same as having you inside me...</i>\"  She squeezes the neck of your [cock] and then pulls her hand away, slipping it between her own legs.  \"<i>I was thinking you could help me make another, and maybe up the population of men worth a damn around here.</i>\"  Reclining, she spreads her thighs, pulling apart her small, smooth labia ");
		//[(sheila corruption <=80)]
		if(sheilaCorruption() <= 80) Render.text("with her fingers to make sure the invitation is all the way open");
		//(else)
		else Render.text("with her split tail while she cradles her massive tits to make sure you see the open invitation");
		Render.text(".  Impatiently, she rubs her stubby, drooling, phallic spade against the hole, accentuating the imagery.  You can feel the blood rushing to your groin as she gives herself shallow, teasing strokes.");
	}
	pregnancy.knockUpForce(); //Clear Pregnancy
	...
	if(player.lowerBody.cockSpot.hasCock()) addButton(1,"Knock Up",pregDemonSheilaKnockUp);
...
private function pregDemonSheilaOtherSex():void {
	clearOutput();
	Render.text("Her eyebrows knit as you propose an alternative.  ");
	//[(if cock)
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("\"<i>Well now, I think I get a say, don't I?  I was really hoping to have your seed inside me... it's practically worth bottling.</i>\"  She ");
		if(sheilaCorruption() >= 80) Render.text("heaves onto her feet, heavy breasts swaying");
		else Render.text("nimbly hops to her clawed feet");
		Render.text(", and runs her hands distractingly along the curves of her hips.  \"<i>You can do what you like with my body, love... if you can convince me.  Of course, I'll claim the same privilege!</i>\"  Sheila jumps at you, leaving no doubt what kind of 'convincing' she expects to do!");
		startCombat(new Sheila());
	}
	//(if no cock)
	else if (player.lowerBody.vaginaSpot.hasVagina()){
...
internal function loseToSheila(consensual:boolean = false):void {
	...
	if(player.HP < 1 && !consensual) {
		Render.text("Your erstwhile opponent's eyes glimmer with excitement as you collapse from your injuries, and she runs over to you.  The demon strips off your [armor] eagerly, but you can't stay awake for the fun.  Consciousness slips away and you pass out.");
		//--Next--
		model.time.hours += 8;
		if(model.time.hours > 23) {
			model.time.hours -= 24;
...
private function loseToNormalSheilaAndGetRidden():void {
	...
	Render.text("Sheila pushes you onto the ground");
	if(player.isTaur()) Render.text(" and you're forced to put your forelegs down for support as you come to rest on your haunches");
	else Render.text(", spreading your legs.  The demon unfastens your [armor] and strips it from your bottoms easily");
	Render.text(", exposing your " + multiCockDescriptLight() + ".");
	
	Render.text("\n\n");
	//[(multi)
	if(player.lowerBody.cockSpot.count() > 1) Render.text("\"<i>All of these look ripper, but... I want this one today,</i>\" she says, selecting your [cockFit 56].  ");
	Render.text("She wraps her hand around it and begins pumping up and down, squeezing out a drop of precum that she smears onto her palm.  \"<i>You're so eager,</i>\" Sheila giggles.  \"<i>Are you already about to burst?  That's no good... I wanted to play a bit more.</i>\"");
	
	Render.text("\n\nThe demon releases your dick, watching raptly as it bobs and twitches, then scoots up to you and");
	if(player.isTaur()) Render.text(", draping your forelegs over her shoulders,");
	Render.text(" presses her " + sheilaCup() + " breasts");
	if(sheilaCorruption() < 100) Render.text(" against you, dragging her nipples against yours.  ");
	else Render.text(" against you so forcefully that they part around your chest.  ");
	Render.text("\"<i>Such a " + player.mf("handsome bloke","pretty sheila") + ", with such a nice dong,</i>\" she whispers, \"<i>I'm going to have to take you for a ride.  Try not to come as soon as you get in!</i>\"");
	
	Render.text("\n\nSheila grins viciously and sinks down on your [cockFit 56], sliding it into her pussy at a painstaking pace");
	//[(multi)
	if(player.lowerBody.cockSpot.hasCock()) Render.text(" and slipping the unused remainder of your manhood between her thighs");
	Render.text("; you can feel every new inch of heat as she lowers her body.  The woman moans, gyrating her hips, and " + sMultiCockDesc() + " twitches in painful erectness.");
	...
	if(player.lowerBody.balls > 0) {
		...
		//(not horse)
		if(!player.isTaur()) Render.text("  Sheila takes your face in her hands and presses it into her breasts as she reassures you.");
		...
		if(player.lowerBody.cockSpot.count() > 1) Render.text(" and lap");
		Render.text(" just like she's dumping feelings into your head.  \"<i>Lover!</i>\" she screams.  \"<i>Pack me with your babies!</i>\"");
		...
		if(player.isTaur()) Render.text("forelegs");
		else Render.text("hands");
		Render.text(", displacing a glob of cum from her pussy that's soon followed by others as you ejaculate once more.  The jizz is so dense with sperm that it's thick, almost colloidal; it jiggles on her skin, barely sliding downward as your hips ram against hers.  It takes nearly ");
		//(normal skeet)
		if(player.cumQ() < 250) Render.text("ten minutes and two");
		//(big skeet)
		else if(player.cumQ() < 1000) Render.text("thirty minutes and four");
		else Render.text("fifty minutes and eight");
		Render.text(" more orgasms, but finally your balls empty of their demonically-induced load and your lover pulls away and stands up.");
	...
	//(else if no balls)
		...
		if(player.isTaur()) Render.text("You force her downward with your forelegs to");
		else Render.text("You grab her hips and");
		Render.text(" help her impale herself on your tool.");
		...
		if(player.lowerBody.cockSpot.count() > 1) Render.text(" and lap");
		//[(big skeet)
		if(player.cumQ() >= 1000) Render.text("; she quickly reaches capacity and pulls off, jerking your messy, slimy [cockFit 56] and letting you squirt the last of your strokes all over her face and chest");
		if(player.cumQ() >= 6000) Render.text(".  So much is left that she's practically underwater by the time you're finished; she giggles and bubbles form in the oozing liquid covering her lips");
		Render.text(".");
...
private function tailSpadeSoundingForFuckHugeDongsWithDemonSheila():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("; unguided, the ");
	if(player.lowerBody.cockSpot.count() == 2) Render.text("other twitches and sprays");
	else if(player.lowerBody.cockSpot.count() > 2) Render.text("others twitch and spray");
	if(player.lowerBody.cockSpot.count() > 1) Render.text(" all over, coating her back, you, the grass, and anything ");
	if(player.lowerBody.cockSpot.count() > 2) Render.text("else they happen to point at");
	if(player.lowerBody.cockSpot.count() == 2) Render.text("else it happens to point at");
	Render.text(".  Your hips automatically try to push your dick further into her warm hole but only succeed in unbalancing her, forcing the demon onto her hands.  As your climax winds down, your head leans back and your cock slips from her vulva, squirting the last stroke onto her ass.");
...
private function clitSwellingDemonSheilaClitSoundingAnal():void {
	clearOutput();
	//(no horse)
	if(!player.isTaur()) Render.text("Sheila places her hand in the small of your back and leans in, drawing closer to your face even as she pulls your waist inward.");
	else Render.text("Sheila sets her hand atop your withers, pressing down gently as she looks into your eyes.");
	Render.text("  \"<i>Why don't you relax, [name]?  Lie down and let me take care of you...</i>\"  Her eyelids lower and her mouth spreads in a sly smile, and for just a moment you lose your nerve, looking into her devilish expression.  \"<i>Don't worry, love,</i>\" she says, straightening her face.  \"<i>I promise you'll enjoy this.</i>\"");
	
	Render.text("\n\nBefogged but still reserved, you nonetheless allow Sheila to lower you to the grass and remove your [armor].  She ");
	//[(not horse)
	if(!player.isTaur()) Render.text("turns around, then straddles your groin and looks over her shoulder.");
	//(Equus ferus caballus)
	else Render.text("circles you until she reaches your rump, then turns around and kneels down herself, looking at you over her shoulder.");
	Render.text("  Her black, spaded tail lifts from between her thighs and snakes toward your pussy,");
	//[(if manparts)
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text(" caressing ");
		if(player.lowerBody.balls > 0) Render.text("your [balls]");
		else Render.text("the underside of your [cock biggest], before");
	}
	Render.text(" forcing your lips apart and lingering on your rapidly-swelling " + clitDescript() + ".  \"<i>Oh, my.  What's with this ");
	if(player.lowerBody.vaginaSpot.list[0].clitLength <= 6) Render.text("lewd little");
	else Render.text("grotesque");
	Render.text(" thing doing growing from a pretty girl like you?  It's just the perfect size for some fun.</i>\"");
	
	Render.text("\n\nYour eyes open in alarm at the same time as her mouth does in pleasure; she jams the slit of her spade onto your button");
	if(player.lowerBody.vaginaSpot.list[0].clitLength > 6) Render.text(", taking it several inches deep");
	Render.text(".  Your head swims as pleasure from your engulfed clitoris overwhelms you, then toes the line to genuine pain.  The copious demonic fluid backing up inside her tail begins to leak around the tip, coating your clit and drooling onto your labia; your muscles tighten as the heat escalates past comfortable levels, and you begin shifting violently, trying to pull your swollen chick-stick free from the demon's anatomy.  \"<i>Ah, ah, god, [name], stop!  Don't thrash so!  You're teasing me too fast!</i>\"  Sheila's words are lost on you, sunken as you are into a hell of oversensitized skin and constant stimulation.  It feels like the nerves exposed to the torment are even multiplying, as if skin you weren't even feeling before suddenly appears to be immersed into the tainted slime.");
	...
	//[(<=6")
	if(player.lowerBody.vaginaSpot.list[0].clitLength <= 6) Render.text("\n\nAwareness comes to you slowly; that flesh is your clitoris, gratuitously mutated by Sheila's fluids!");
	else Render.text("Though you're by now accustomed to the sight of your clitoris, more like a cock than a dainty feminine bump, this is something else altogether!  The huge thing poking into her tail is almost twice the thickness of the slim growth you remember and bulges with blood vessels; as you watch it, transfixed, a trickle of Sheila's oily, dark secretion slides down it, then is absorbed into some flesh near your groin, adding another layer of width with a pulse!");
	Render.text("  \"<i>Fuck, it's so tight!</i>\" the demon gasps, now resting on her elbows with her chest and face pressed to the grass.  Her tail jerks again, yanking your stalk this way and that and releasing more of her backed-up fluids.  They slide all the way to the bottom of your rod before sinking into your skin.");
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength <= 6) {
		...
		if(!player.isTaur()) Render.text("Taking your face into her hand, she closes your jaw and then leans in to plant a kiss on your chin.");
		else Render.text("Her eyes glitter with mirth.");
		Render.text("  \"<i>Why the look, mate?  Seems like you're saturated for now, and just in time.  This... is...</i>\"  She pulls her tail up from the end of your clitoris, then slides it back down, slowly.  \"<i>... perfect!</i>\"  As her tight hole swallows your sensitive distaff staff once again, your eyes roll and her head drops, dragging her hair");
		//[(horse)
		if(player.isTaur()) Render.text(" through the grass");
		else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) Render.text(" between your " + allBreastsDescript());
		else Render.text(" along your flat chest");
		Render.text(".  \"<i>Ffuck, even your girl parts are amazing.</i>\"  Sheila plants kiss after kiss on your ");
		if(!player.isTaur()) Render.text("[chest]");
		else Render.text(buttDescript());
		Render.text(" as she shallowly pumps your " + clitDescript() + " in and out of her spade.  Your pelvis twitches uselessly; with her tail waving in the air instead of braced against her, controlling the pace of the fuck is beyond you.  \"<i>Naughty sheila,</i>\" she says, squeezing your wiggling hip with a hand.  You think privately that she could just as easily be talking about herself.  \"<i>Am I not going fast enough for you?  Let me speed up.</i>\"");
		...
		if(player.stats.cor < 50) {
			...
			if(player.wetness() >= 4) Render.text("demon's legs");
			else Render.text("grass");
			Render.text(", and your clit swells even further inside your tormented partner.  Sheila groans, \"<i>Fuck you, [name]...</i>\"");
	...
	//(else if clit >6" initially)
		...
		//[(horse)
		if(player.isTaur()) Render.text(" and rolls onto her back");
		Render.text(", then wraps her tails around your thickened clit.  They twist and pull, sending spirals of pleasure along the numerous nerves, and begin gently but insistently tugging you toward her tan butt.  Sheila directs the tip right between her cheeks, and you can feel her using it to rub the ring around her hole.  \"<i>I think this should be about the right size for you, love,</i>\" she teases, spearing herself on your " + clitDescript() + ".  Inch after inch slides into her slowly, maddening you with warmth, until ");
		if(player.lowerBody.vaginaSpot.list[0].clitLength <= 12) Render.text("all of it is in her bowels and her hot cunt rests against yours");
		else Render.text("she's taken as much as she can");
		Render.text(".  She half-smiles and half-winces at you.  \"<i>Don't usually take it in this hole, but I wanted to feel your pussy going crazy for me.</i>\"  ");
		if(player.lowerBody.vaginaSpot.list[0].clitLength <= 12) Render.text("Her hips shift, dragging her lips across yours and stirring your clit through her asshole even as it squeezes you; your pussy twitches at the stimulation.");
		else {
			Render.text("Her tails, one still swollen from being stretched and tormented by your growing clit, slip into you abruptly, bringing their mild burning sensation with them; if you hadn't been saturated with it already, you'd probably pass out.");
			//[(if virgin vagina)
			if(player.hasVirginVagina()) Render.text("  \"<i>What's this?</i>\" Sheila teases.  \"<i>Why, your virtue is still intact, little girl!  I wouldn't want anything to happen to it!</i>\"  She pulls her tails out a bit, contenting herself with simply teasing your lips and driving you mad for penetration.");
			Render.text("  The demon shifts around your clit, sending a wave through you that makes your lips wrap around the invaders.");
		}
		
		Render.text("\n\nSheila places her long-taloned feet against ");
		//(not naga)
		if(!player.isNaga()) Render.text("your thighs, spreading your [legs] wide and then hooking them over top to touch ");
		Render.text("the ground next to your ass, and uses the leverage to begin pumping your clit out of and into her asshole, caressing your " + vaginaDescript(0) + " every time she slides home.  \"<i>I could get used to this, love.  What say I keep you dosed up like this and you become my dildo?  I'll ride you with a different hole every time!</i>\"  Your eyes roll back in your head, at least partially from the irregular pace at which she's stroking your clit.  Sheila grabs her breasts as she slows down drastically once again, mashing them with her hands and sinking onto you an inch at a time.  \"<i>Fuck... I love your junk, [name].  Maybe I should catch a goblin, too, and make her tease my pussy while you fill my ass.</i>\"  She raises back up and ");
		//[(clit>12")
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 12) Render.text("pulls her black tail free, dripping with your juice, then ");
		Render.text("begins to work her drooling spade along your clit again, smearing her goo along your chunky chick-stick.  You can feel the edges of the stretched hole in her tip working on your skin, opening and closing ever so slightly as her copious tainted lube oozes out.  Your " + clitDescript() + " must be completely saturated already, because the slippery goo lingers on the surface instead of wicking into it.  \"<i>Gonna g-get you nice and wet, love...  Ohh,</i>\" Sheila says, affected to distraction by the pleasure of tracing the slick end of her stubby tail along you.");
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength <= 12) Render.text("pussy; her smooth, small labia greedily lap at the fluid when they touch, brushing against yours and drawing further twitches and squirts.");
		else Render.text("teasing tails; they shift from stroking your labia to holding them open, allowing your pussy to squirt as it pleases.");
		Render.text("  Suddenly she stops mid-stroke as her asshole quivers, sending shivers up your orgasm-sensitized clitoris.  \"<i>Fffuck!  So soon!  I'm coming too, [name]!</i>\"  Sheila sinks down, burying your " + clitDescript() + " into her asshole.");
		//(clit>12")
		if(player.lowerBody.vaginaSpot.list[0].clitLength <= 12) {
			Render.text("  Her tails sink into you as she squirms, parting your lips and pushing their way inside.");
			//[output hymen check, but don't remove virgin flag yet]  
			if(player.hasVirginVagina()) Render.text("  <b>You've lost your virginity!</b>");
			Render.text("  Buried in your pussy, the spade begins drooling its black fluids in greater and greater volume as Sheila is wracked by her orgasm.");
		}
		
		//(PC corr<50)
		if(player.stats.cor < 50) {
			Render.text("  The demon's pussy gushes and pours with unnatural levels of fluids, soaking your crotch as a miniature cataract makes its way out of her lower lips.  Sheila spills herself onto you for at least a minute, producing more hot fluid than you thought possible, until she looks utterly dehydrated - even her breasts seem to be smaller and less jiggly.");
		}
		else Render.text("  The demon's skin seems to parch as she grinds into you relentlessly, evaporating your fluids so fast that most of them don't even have time to fall.  Her hot flesh lays heavily across your netherlips, prolonging your climax and coaxing out more fem-spray than you thought was in you.  Even your mouth seems to dry as she drinks your essence up.");
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 12 && player.hasVirginVagina()) {
			Render.text("  \"<i>Sorry about your hymen, love, but I just couldn't resist making a sweet little slut like you into my girl.  I hope you'll remember me - I only wish I had a proper donger so I could give you my baby, too.</i>\"");
		}
		Render.text("  Even as tired as you are, what you see on the demon girl makes you laugh.  She starts at this, then follows the direction of your gaze; ");
		if(!player.isTaur()) Render.text("her tits are covered by grass stains from her frenetic pumping.");
		else Render.text("craning around shows her that her asscheeks are completely green and wet with torn grass from slamming into you so violently and repeatedly.");
		Render.text("  \"<i>... Oh, god dammit,</i>\" Sheila gripes.  She walks over and forces your face into them with a long, slow shake, then giggles as a joke comes to her.  \"<i>There, [name]; I've stained you with my colors!</i>\"  She plants a long kiss on your grass-smeared forehead, then drops you back to the ground, straightens up, and departs.  Exhausted, you slip into a doze, wondering if you'll be able to fit your swollen clit back into your clothing when you wake or if you'll have to walk back to camp with it sticking out.  It slowly deflates as you sleep, but does retain a bit of additional length.");
	}
...
private function aintGotNoGenderAndKangarooRaped():void {
	clearOutput();
	Render.text("Sheila sashays over to you and brazenly sticks her hand in your featureless crotch.  \"<i>It's too bad you've mutilated yourself like this, love... or did you do it just to tease me?  You know, it doesn't bug me that much.</i>\"");
...
internal function beatUpDemonSheila(output:boolean = true):void {
	if(output) {
		clearOutput();
		Render.text("\"<i>Ahh, you win, mate.  I give up.</i>\"  Sheila slumps to the ground, ");
		if(monster.HP < 1) Render.text("and her wounds begin to close with a soft lilac glow.  \"<i>So fuckin' strong.  I don't suppose you've got any sperm for me, do you?  I'd love to have some of your kids...</i>\"  She looks at you beseechingly, gently fingering herself.");
		//(lust)
		else Render.text("frigging herself vigorously.  \"<i>Do with me what you will, lover; those were the terms.  Won't you take my body to satisfy yourself, you brute?</i>\"  The audacious woman sticks her tongue out, as if to goad you into treating her roughly.");
	}
	//[(lust < 30)]
	...
	if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(56) >= 0) addButton(0,"Missionary",missionaryForThePurposesOfCreatingImpsWithSheila);
	...
	if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(56) >= 0) addButton(1,"AnalHateFuck",analHateFuck);
	//Win - [Big Dick+Thighs] (usable for dicks with cockarea > 56) (all 10 of them)
	//penis envy, go!
	if(player.lowerBody.cockSpot.hasCock() && player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 56) addButton(2,"UseHerThighs",bigDickAndThighs);
	//Win - [Penetration, in Spades] - for self-loathing vaginas
	if(player.lowerBody.vaginaSpot.hasVagina()) addButton(3,"Clit-Fuck",winAgainstDemoNSheilaForVaginas);
}
...
private function missionaryForThePurposesOfCreatingImpsWithSheila():void {
	clearOutput();
	Render.text("You unfasten your [armor] and strip down completely, as Sheila's eyes drink up your bare flesh.  Denuded, you caress your cock with several strokes, producing a drop of precum.");
	...
	if(player.stats.cor < 40) Render.text("Your ardor subsides a bit at her performance, but seeing your look of contrition, ");
	else Render.text("Your scowl hardens at her shameless playacting, and ");
	Render.text("she breaks into laughter.  \"<i>I'm just having you on, love.  You know my body's yours for the using.</i>\"");
	...
	if(!player.isTaur()) Render.text("shoulders");
	else Render.text("flanks");
	Render.text(" and her pupils dance in excitement; for a moment you even lose yourself in her enthusiasm, forgetting her demonhood and seeing only an earnest, adoring girl under you.  \"<i>Now, push!</i>\"");
	...
	//(sheila corruption >=40)
	if(sheilaCorruption() >= 40) Render.text("and her soft breasts jiggle ");
	Render.text("as a twinge of sensation from your thrust provokes a gasp and forces her to reposition herself, to better find it again.  \"<i>That's the spot!  Your dick is the <b>best!</b>  Pound me, you fucking cum-hose!</i>\"");
	...
	if(!player.isTaur()) Render.text("arms");
	else Render.text("forelegs");
	Render.text(" wobble and it takes all your focus not to collapse onto the woman and melt into a boneless puddle; there's little left to prevent your twitching hips from granting her request, and you slam your pelvis into hers repeatedly, provoking delighted cries from your shameless partner.  \"<i>Ohh, yes!  Fill me with your baby batter, asshole!</i>\"  Sheila wraps her arms and legs around your back");
	if(player.isTaur()) {
		Render.text(", practically lifting herself off of the ground] as she impales her cunt on your [cockFit 56] to prevent you from pulling out.");
	}
	//[(silly)
	if(silly()) Render.text("  \"<i>Make me feel like I'm fifteen years old again and playin' with the dog-morphs in the field!</i>\"  Heavens to Betsy!");
	else Render.text("  \"<i>This is what you're here for!  Don't spill a single drop!</i>\"");
	...
	if(sheilaCorruption() < 40) Render.text("tight");
	else Render.text("soft");
	Render.text(" chest against you");
	if(player.isTaur()) Render.text(" as you slump forward");
	Render.text(".  Your cock ");
	if(player.lowerBody.cockSpot.count() > 1) {
		Render.text(" and its ");
		if(player.lowerBody.cockSpot.count() == 2) Render.text("brother");
		else Render.text("brethren");
		Render.text(" swell and squirt");
	}
	else Render.text("swells and squirts");
	Render.text(", pouring your semen into Sheila's ravenous pussy");
	if(player.lowerBody.cockSpot.count() > 1) Render.text(" and coating her crotch with a glaze of white");
	Render.text("; her eyes glow and it briefly feels as though her body is sucking the ejaculate right out of you");
	if(player.cumQ() >= 500) Render.text(", until she fills up and the rest of your orgasm is drooled out at its own lazy pace");
	Render.text(".  She kisses you");
	if(player.isTaur()) Render.text("r forelegs");
	Render.text(" lovingly and deeply");
	if(player.lowerBody.cockSpot.count() > 1 || player.cumQ() >= 500) Render.text(" as the spare seed dribbles from her crotch");
	Render.text(", then breaks away and looks up into your face.");
	
	Render.text("\n\n\"<i>[name], you ");
	if(player.cumQ() >= 500 || player.lowerBody.cockSpot.count() > 1) Render.text("spilled some.  I can't have your baby if you're fucking the grass.  And you ");
	Render.text("came too soon; I wasn't done.</i>\"");
	...
	//(else if RNG doesn't end scene)
		...
		//(if vag)
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(" as her imp spawn take turns doing the same to you from behind");
		else Render.text(" as her imp spawn gather around, stroking themselves and occasionally squirting their hot, tainted cum on her body or yours as orgasm takes them");
		Render.text(".  ");
		if(player.stats.lib < 40) Render.text("Shocked");
		else Render.text("Flushing hotly");
		Render.text(", you look away and break the connection, but Sheila's smile only widens.");
		...
		//[(PC corr <50)
		if(player.stats.cor < 50) Render.text("Her pussy convulses as she orgasms, and you feel a wave of heat overtaking your [cock biggest], starting at the tip and moving down the shaft.  As it reaches the base, a gush of her fluids breaches the seal her pussy makes around you, forcing your jizz out along with it in a mixed white and clear squirt.  The demon's vagina squeezes and drools for half a minute, pouring her tainted liquid love out against your tightly-held body until your crotch is soaked with her sticky heat and responds with its own.");
		else Render.text("Her pussy sucks you in as she climaxes, and you can feel larger and larger waves of precum being pulled from your shaft; it almost feels like she's tugging at your entire being.  It's not long before your cock twitches and begins giving her greedy sex a second helping of your semen.");
		Render.text("  She goes limp as her twitching cunt takes over and begins drinking every drop it can get from your dick to replace the goo you pushed out");
		if(player.cumQ() >= 500) Render.text("; you nearly manage to fit it all into her this time, with your production still flagging from the last session");
		Render.text(".  Muscles weakening again, you lower yourself onto her once more and allow her to stroke your ");
		if(!player.isTaur()) Render.text("back");
		else Render.text("sides");
		Render.text(" as she cools down.");
...
private function analHateFuck():void {
	clearOutput();
	Render.text("Fucking bitch, playing with herself like nothing's the matter and you didn't just show her concrete proof of your superiority.  Part of you feels like punishing the insolent woman yourself, pounding her insides and dumping your load without a whit of regard for her own pleasure... but another part is thinking it might be fun to see her completely debased, to hammer home how pathetic she is - something like reducing her to a cock-sleeve for your personal cum-slave.");
...
private function sheilaAnalHateFuckAGoGo():void {
	clearOutput();
	Render.text("Your [cockFit 56] gets hard in your [armor] as you watch her diddle her pussy, but you set your jaw to resist giving her what she wants - a better idea is taking shape in your head.  \"<i>Turn around,</i>\" you demand.");
	...
	if(!player.isTaur()) Render.text("Slipping one hand around your [cockFit 32]");
	else Render.text("Straddling her");
	Render.text(", you pull your hips back and line the [cockHeadFit 32] up, then thrust - right into her asshole.  Her head raises and her teeth grit as you force the slick tip past her ring.  \"<i>[name], what the fuck?!  That hurts!  Go easy if you're going to use that hole; I'm not good at stretching my body yet!</i>\"");
	
	Render.text("\n\n\"<i>No.</i>\"  You ");
	//(not centaur)
	if(!player.isTaur()) Render.text("grab her thrashing legs, using them for leverage as you ");
	Render.text("force your dick into the angry woman.  \"<i>You talk a good game about how you love being full of cock, but when I shove one in you, you get all bitchy and act like you're not ready; why don't you just shut up and get fucked, slut?</i>\"");
	...
	if(sheilaCorruption() >= 90) Render.text("; her enormous, cushiony breasts spread under her as your prick forces her weight heavily onto them");
	Render.text(".  She pulls up tufts and pounds the earth impotently with her fists while you sink in as far as you can.  \"<i>Fuck you!  What the fuck is wrong with you, arsehole?!  Let me go, I don't want anything to do with you anymore!</i>\"");
...
private function sheilaAnalHateFuckAGoGoNO():void {
	clearOutput();
	Render.text("\"<i>Go to hell.</i>\"  You bury yourself in the demon's ass as your dick begins to spasm.  Her tails writhe as the first trickle sinks into her.  \"<i>What?  Say something, I dare you.</i>\"");
	...
	if(player.cumQ() >= 500) Render.text("  Your overproduction floods her insides, and Sheila groans as it begins to leak back out, drooling over her pussy.  \"<i>Hey, you got your wish,</i>\" you tease.  The wanton woman tries to tilt her ass higher, to tip more of your overflow into her cunt, but you laugh and slap her hard on the hip, sending her to one knee again.");
	...
	if(player.cumQ() < 250) Render.text("trickle");
	else if(player.cumQ() < 1000) Render.text("stream");
	else Render.text("waterfall");
	Render.text(" of semen made when her body pushes out some of the foreign substance.");
...
private function sheilaAnalHateFuckAGoGoGETYOUSOMEWORMS():void {
	clearOutput();
	Render.text("\"<i>Mmm...</i>\" you answer, noncommittally.  Your ");
	if(player.lowerBody.balls > 0) Render.text("[balls]");
	else Render.text("insides");
	Render.text(" squirm, giving you pause, and you pop your [cockFit 56] out.  \"<i>Actually... I don't think I suit you at all.</i>\"");
	...
	Render.text(" your ");
	if(player.lowerBody.balls > 0) Render.text("[sack]");
	else Render.text("[cockFit 56]");
	Render.text(", you can actually feel your pent-up pets, eager to be let out to play.  \"<i>No,</i>\" you say, \"<i>I know what's more your speed.  Trash like you should be cast down into the dirt, and should keep company with the creatures of it.</i>\"  Sheila seems alarmed, but as you slip into her grasping pussy, she forgets her question, grinding her hips into you to milk you dry.  You focus, forcing a wave of slimy annelids down your shaft.  \"<i>Here come your new babies!</i>\"");
...
private function analHateFucksWithJojo():void {
	clearOutput();
	Render.text("Watching her sit there and play with herself like nothing's the matter and you didn't just demonstrate her weakness for all the world to see touches a nerve deep inside you.  You scowl and focus your thoughts to call your mousey ex-monk, bent on showing this demon how a proper mendicant should behave.");
	...
	if(sheilaCorruption() < 40) Render.text("\"<i>What's with these tiny titties?  Could any man possibly be aroused by your bug bites?  I've seen bigger breasts on incubi!</i>\"");
	else Render.text("\"<i>These disgusting sacks of fat on your chest... are you under the impression that men like giant stretch marks and nipples that hang to your knees?  Why don't you get in shape, you messy bitch?</i>\"");
...
private function jojoRuinsTheAnalHateFuck(clear:boolean = true):void {
	...
	Render.text("Your mouth spreads in a sickly smile as you contemplate granting her request anyway.  \"<i>Jojo...</i>\"");
	...
	if(sheilaCorruption() >= 90) Render.text(", mashing her tits together as they thicken");
	Render.text(".  The mouse himself seems to be growing, in fact.  His feet kick and thrash, sprouting dangerous-looking nails as his legs stretch prodigiously.  As his body mass increases, his white-furred skin begins to burst and peel like old rags, revealing new, dark flesh underneath.");
...
private function bigDickAndThighs():void {
	clearOutput();
	Render.text("As Sheila diddles herself, you ");
	//[(not horse)
	if(!player.isTaur()) {
		Render.text("doff your [armor]; the demon's eyebrows climb as you haul out " + sMultiCockDesc());
		if(player.lowerBody.cockSpot.count() > 2) Render.text(" one at a time, practically stacking them atop each other");
		Render.text(".");
	}
	//(horse)
	else {
		Render.text("slowly draw nearer to her, your " + multiCockDescriptLight() + " bobbing with each step.");
	}
	...
	if(player.stats.cor < 40) {
		...
		Render.text(" toward her and aim underneath her cunt to rub your [cockHead biggest] against her palm, smearing a glob of precum on it.  You can see her shivering wordlessly, sliding her fingers together to play with the hot, clingy fluid, making translucent webbing between them.");
		//[(multi)
		if(player.lowerBody.cockSpot.count() > 1) {
			if(player.lowerBody.cockSpot.count() == 2) Render.text("  Both");
			else Render.text("  All");
			Render.text(" of them... slide them here, love,</i>\" the demon insists.  You line the remainder of your manhood up and force it between her thighs as well.");
		}
		Render.text("  She takes the tip of your [cock biggest] in her hand and pulls gently, coaxing your forward; you close the distance until " + sMultiCockDesc() + " is peeking through her legs and your crotch is pressed against hers.");
		
		Render.text("\n\n\"<i>I'm going to borrow a bit of your sensation...</i>\" Sheila announces quietly.  ");
		//[(not horse and not < 5"0)
		if(!player.isTaur() && player.tallness >= 60) Render.text("She unbends and kisses you on the cheek, and your face feels a bit fuzzy where her lips touch.  ");
		...
		if(player.isTaur()) Render.text(", unseen");
		Render.text(".");
		...
		if(player.lowerBody.cockSpot.count() == 1) Render.text("your dick with both");
		else Render.text("a prick in each");
		Render.text(" and angling upward so the precum drools downward and lubes her fingers.");
	}
	//(else corr >= 40)
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			...
			if(player.lowerBody.cockSpot.count() == 2) Render.text(", you slide it through her legs between your first cock and her cunt, plowing through her entrance like a furrow; she quakes and you can feel it contracting against your skin.  ");
			else Render.text("s, you push them through the woman's legs one by one, smearing warm precum on her thighs as you do; her pussy squeezes against the top of your [cock biggest] as you coat her skin with your fluids.  ");
		}
		else Render.text("\n\n");
		Render.text("Sheila scoots backward, choking up on monster cock until her ass is pressed against your crotch, and then wraps her hands around and begins to masturbate you, using your drooling fluid and her own wetness for lubrication.");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" and teasing each one; you thrust weakly into her hand, helping guide her hands, but your lonely tip wants for stimulation.  As the feeling becomes so acute that you prepare to say something, Sheila uncannily anticipates you and scoots her pussy along your shaft, popping your [cockHead biggest] into her mouth.  Her tongue dabs at your slit, tasting your fluid, and her hands ");
	if(player.lowerBody.cockSpot.count() > 2) Render.text("relinquish your other pricks and ");
	if(player.lowerBody.cockSpot.count() == 2) Render.text("relinquish your other prick and ");
	Render.text("focus on squeezing more drops out of you.  She fairly coats her lips with it, rubbing your hole against them before popping the whole [cockHead biggest] back into her mouth.  A moan escapes you just as another one returns from Sheila's throat, transmitted through your [cock biggest].");
	...
	//[(sheila corruption < 50)
	if(sheilaCorruption() < 50) Render.text("  Though her modest breasts can't even begin to encompass your girth, Sheila rubs her cleavage along your skin with gusto while she works your tip with her tongue.");
	else if(sheilaCorruption() < 150) Render.text("  Her large " + sheilaCup() + " breasts encompass your girth easily, and she pushes the soft, flushed skin together as she slides it up and down your cock.");
	else {
		Render.text("  The demon's ridiculously enormous breasts easily cover the entire length of your cock");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" falling between her crotch and her mouth, making her whole body feel like a huge, wet fuckhole");
		if(player.lowerBody.cockSpot.count() > 1) Render.text(" built for " + num2Text(player.lowerBody.cockSpot.count()) + " pricks");
		Render.text(".");
	}
	...
	if(sheilaCorruption() >= 150) {
		Render.text("Her enormous breasts dangle all the way to the ground, forming a second, softer hole to push into with your swollen prick");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(".  ");
	}
	Render.text("\"<i>Y-yes,</i>\" she mumbles, \"<i>gonna fuck her... harder...</i>\"");
	
	Render.text("\n\nThe apparent answer to your question passes you by as you obliviously force your tool");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" through the vise of her legs; losing yourself further and further to pleasure, you don't even notice when Sheila's hips begin to buck in time with your own, pressing her mons down against your ");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("topmost ");
	Render.text("dick.  With an enormous, final thrust, you mash your crotch up against hers and begin to ejaculate.  ");
	if(player.stats.cor < 50) Render.text("The sensation seems to reawaken Sheila; she angles her body up to press her pussy close and your embezzled sensation floods back into you - along with an unexpected gift.  You can feel your... no, <i>her</i> demonic pussy convulsing repeatedly, trying to suck up sperm in vain as her own orgasm comes off, and she looks at you with sly eyes.  \"<i>A little reciprocity for my love,</i>\" the woman coos, \"<i>to show you how much I care.</i>\"");
	else Render.text("\"<i>Oh god!</i>\" Sheila shrieks.  \"<i>I'm coming!  Jizz is going everywhere!</i>\"  Indeed, as your body releases streams of cum, Sheila directs them by aiming " + sMultiCockDesc() + ", covering herself and moaning as her hands press out as much as possible.");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(", looking almost wistful to see ");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) Render.text("to grow a nice pussy for me so that ");
	Render.text("I can put it in you any time I want.</i>\"  She sticks her tongue out at you, and the seed dripping from her lips oozes onto it.");
...
private function winAgainstDemoNSheilaForVaginas():void {
	clearOutput();
	Render.text("You drop onto the woman's lap without hesitation, ");
	if(!player.isTaur()) Render.text("mauling her tits roughly with your hands, ");
	Render.text("and grind your pussy desperately against her midriff.  She slips two fingers into you obligingly, but you ");
	if(!player.isTaur()) Render.text("grab her face, plunging one thumb into the corner of her mouth in your haste, and");
	else Render.text("turn your head and");
	Render.text(" stare right into her eyes.  Focusing as hard as you can on your fantasy, you try to transmit your desire for something more to the demon by sheer force of will.  A sly smile parts her lips as she stares back - could this actually work?");
	...
	if(!player.isTaur()) {
		Render.text("crawl away on your hands");
		//[(tail or naga)
		if(player.tailType > TAIL.NONE && player.isNaga()) Render.text(" and raise your tail");
	}
	else Render.text("clamber to your hooves and bend your forelegs");
	Render.text(" to present your eager [vagina].  Sheila sits up and places her hands on your [hips], then slides her pelvis up, teasing your lower lips with something bulbous and warm.  They sting slightly as it smears its slime along them, a not-entirely-pleasant sensation.");
	...
	Render.text("  Oh, god, it burns!  You shake violently as your pussy is assaulted by the fluid from Sheila's black tail, waves of pain twisting through it and forcing its walls to clamp down on the invader.  The demon gasps as you squeeze and leans forward onto your back");
	if(sheilaCorruption() > 0) Render.text(", squashing her tits into you");
	Render.text(".  \"<i>Calm down!</i>\" she barks, grabbing your arms and pulling them toward her.  With hands held behind you like this, you can do little to check her enthusiasm... but Sheila doesn't begin thrusting yet.  \"<i>It'll fade soon, love, and you'll start feeling it,</i>\" she promises, kissing your ");
	if(!player.isTaur()) Render.text("neck");
	else Render.text("withers");
	Render.text(" as her strange, still shaft continues drooling into your pussy.  Your groin heats up in response to the pain, and her words ring hollow.  The burning doesn't seem to be getting any better; you look back over your shoulder at her, fear and hurt evident on your tightened features.  She smiles nervously at you, but the flow of goo doesn't stop.  It feels wetter and slicker in your pussy every second, and soon you can feel the slime begin to seep back out of your [vagina].  Trickle after trickle of fluid escapes you, falling to the ground underneath.");
	...
	//[(lib <20)
	if(player.stats.lib < 20) Render.text("even as proud as you are, ");
	Render.text("there's something appealing about the picture, something powerful in being cherished and used in equal measure.  Sheila screams and releases your hair, digging her fingers into your hips, spraying the inside of your pussy with a thick, oozing heat that spreads out and then dulls as your own pouring juices force her cum out.  Peering between your legs, you can see it falling to the ground; viscous black sludge that bubbles and steams as if it were trying to reach back up to your sex.");
	if(player.stats.cor < 50) {
		Render.text("  She pumps and pumps again, forcing more of the muck into your [vagina] even as she calls you the most filthy things she can think of");
		if(sheilaCorruption() > 0) Render.text("; her tits wobble violently as she thrusts, and then still as she arches her back and jams her spade all the way in");
		Render.text(".  Sheila howls to the sky as she pours her demonic slime into you in one final, solid stream.  You can feel it pushing eagerly through even your copious fluids until it's at the entrance to your womb... and then inside.");
		...
		if(player.fertility < 50) {
			Render.text("  There is a tingle deep inside you, making you more fertile.");
			player.fertility++;
		}
	}
	else {
		Render.text("  She stops pumping, falling against your back and panting as the last of her goo seeps out and is ejected around her shaft by your fountainhead of girl-cum.  You are clearer-headed than before as your orgasm dwindles, though, illogically, you could swear you felt her tits growing against you.");
	}
