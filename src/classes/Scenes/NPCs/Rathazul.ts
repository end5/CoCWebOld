...
public encounterRathazul():void {
	...
	if(player.statusAffects.has("MetRathazul")) {
		if(player.statusAffects.has("CampRathazul"))
			Render.text("You walk over to Rathazul's corner of the camp.  He seems as busy as usual, with his nose buried deep in some tome or alchemical creation, but he turns to face you as soon as you walk within a few paces of him.\n\n", true);
		else
			Render.text("You spy the familiar sight of the alchemist Rathazul's camp along the lake.  The elderly rat seems to be oblivious to your presence as he scurries between his equipment, but you know him well enough to bet that he is entirely aware of your presence.\n\n", true);
	}
	else {
		Render.text("You encounter a hunched figure working as you come around a large bush.  Clothed in tattered robes that obscure most his figure, you can nontheless see a rat-like muzzle protruding from the shadowy hood that conceals most of his form.  A simple glance behind him confirms your suspicions - this is some kind of rat-person.  He seems oblivious to your presence as he stirs a cauldron of viscous fluid with one hand; a neat stack of beakers and phials sit in the dirt to his left.  You see a smile break across his aged visage, and he says, \"<i>Come closer child.  I will not bite.</i>\"\n\nApprehensive of the dangers of this unknown land, you cautiously approach.\n\n\"<i>I am Rathazul the Alchemist.  Once I was famed for my miracle cures.  Now I idle by this lake, helpless to do anything but measure the increasing amounts of corruption that taint its waters,</i>\" he says as he pulls back his hood, revealing the entirety of his very bald and wrinkled head.\n\n", true);
	...
	if(player.statusAffects.get("MetRathazul").value2 >= 3 && player.statusAffects.get("$1").value3 != 1 && player.stats.cor < 75) {
		Render.text("\"<i>You know, I think I might be able to do this worn-out world a lot more good from your camp than by wandering around this lake.  What do you say?</i>\" asks the rat.\n\n(Move Rathazul into your camp?)", false);
	...
	if(!offered) {
		Render.text("He sighs dejectedly, \"<i>I am not sure what I can do for you, youngling.  This world is fraught with unimaginable dangers, and you're just scratching the surface of them.</i>\"\n\nYou nod and move on, leaving the depressed alchemist to his sadness.", false);
...
public campRathazul():void {
	...
	//Introduction
	Render.text("Rathazul looks up from his equipment and gives you an uncertain smile.\n\n\"<i>Oh, don't mind me,</i>\" he says, \"<i>I'm just running some tests here.  Was there something you needed, " + player.short + "?</i>\"\n\n", true);
	//player.statusAffects.add(new StatusAffect("metRathazul",0,0,0,0)));
	offered = rathazulWorkOffer();
	if (!offered) {
		Render.text("He sighs dejectedly, \"<i>I don't think there is.  Why don't you leave me be for a time, and I will see if I can find something to aid you.</i>\"", false);
		if (player.statusAffects.has("CampRathazul"))
...
private rathazulWorkOffer():boolean {
	...
	if(player.hasItem(consumables.BLACKEG) || player.hasItem(consumables.L_BLKEG)) {
		...
		spoken = true;
		Render.text("He eyes the onyx egg in your inventory and offers a little advice.  \"<i>Be careful with black eggs.  They can turn your skin to living latex or rubber.  The smaller ones are usually safer, but everyone reacts differently.  I'd get rid of them, if you want my opinion.</i>\"\n\n");
	}
	//Item crafting offer
	if(player.hasItem(useables.GREENGL, 2)) {
		if(player.findStatusAffect(StatusAffects.RathazulArmor) < 0) Render.text("He pipes up with a bit of hope in his voice, \"<i>I can smell the essence of the tainted lake-slimes you've defeated, and if you'd let me, I could turn it into something a bit more useful to you.  You see, the slimes are filled with the tainted essence of the world-mother herself, and once the taint is burned away, the remaining substance remains very flexible but becomes nearly impossible to cut through.  With the gel of five defeated slimes I could craft you a durable suit of armor.</i>\"\n\n", false);
		else Render.text("He pipes up with a bit of excitement in his voice, \"<i>With just five pieces of slime-gel I could make another suit of armor...</i>\"\n\n", false);
		spoken = true;
		if (player.hasItem(useables.GREENGL,5)) {
			...
			Render.text("You realize you're still a bit short of gel.\n\n", false);
		}
	}
	//Item crafting offer
	if(player.hasItem(useables.B_CHITN)) {
		Render.text("The elderly rat looks at you intently and offers, \"<i>I see you've gathered a piece of chitin from the giant bees of the forests.  If you bring me five pieces I could probably craft it into some tough armor.</i>\"\n\n", false);
		spoken = true;
		if (player.hasItem(useables.B_CHITN, 5)) {
			...
			Render.text("(You need five pieces of chitin for Rathazul to make you the chitinous armor.)\n\n", false);
		}
	}
	...
	//Single Offer
	if(pCounter == 1) {
		Render.text("The rat mentions, \"<i>I see you have at least one tainted item on you... for 20 gems I could remove most of the taint, making it a good deal safer to use.  Of course, who knows what kind of freakish transformations it would cause...</i>\"\n\n", false);
		spoken = true;
		totalOffers++;
	}
	if(pCounter > 1) {
		Render.text("The rat mentions, \"<i>I see you have a number of demonic items on your person.  For 20 gems I could attempt to remove the taint from one of them, rendering it a good deal safer for consumption.  Of course it would not remove most of the transformative properties of the item...</i>\"\n\n", false);
		spoken = true;
		totalOffers+=2;
	}
	//Offer dyes if offering something else.
	if(player.stats.gems >= 50) {
		Render.text("Rathazul offers, \"<i>Since you have enough gems to cover the cost of materials for my dyes as well, you could buy one of my dyes for your hair.  I will need 50 gems up-front.</i>\"\n\n", false);
		spoken = true;
		totalOffers++;
		dyes = buyDyes;
	}
	...
	if(player.statusAffects.has("CampRathazul") && player.statusAffects.get("MetRathazul").value2 >= 4) {
		Render.text("The rat hurries over to his supplies and produces a container of paste, looking rather proud of himself, \"<i>Good news everyone!  I've developed a paste you could use to shrink down any, ah, oversized body parts.  The materials are expensive though, so I'll need ");
		if(flags[FlagEnum.AMILY_MET_RATHAZUL] >= 2) Render.text("50");
		else Render.text("100");
		Render.text(" gems for each jar of ointment you want.</i>\"\n\n");
		totalOffers++;
		spoken = true;
		reductos = buyReducto;
	}
	...
	if (player.statusAffects.has("CampRathazul") && player.hasItem(useables.T_SSILK) && flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] + flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00275] == 0) {
		...
		spoken = true;
		totalOffers++;
		Render.text("\"<i>Oooh, is that some webbing from a giant spider or spider-morph?  Most excellent!  With a little bit of alchemical treatment, it is possible I could loosen the fibers enough to weave them into something truly magnificent - armor, or even a marvelous robe,</i>\" offers Rathazul.\n\n", false);
	}
	//Vines
	...
	if(player.statusAffects.has("CampRathazul")) {
		if(flags[FlagEnum.RATHAZUL_DEBIMBO_OFFERED] == 0 && (sophieBimbo.bimboSophie() || player.perks.has("BimboBrains") || player.perks.has("FutaFaculties"))) {
			...
			Render.text("You recall that Rathazul is willing to make something to cure bimbo liqueur for 250 gems and five Scholar's Teas.");
			if(player.hasItem(consumables.SMART_T,5) && player.stats.gems >= 250) {
				totalOffers++;
				debimbo = 1; 
			}
			else if(!player.hasItem(consumables.SMART_T,5)) Render.text("  You should probably find some if you want that...");
			else Render.text("  You need more gems to afford that, though.");
			Render.text("\n\n");			
		}
	}
	...
	if(totalOffers > 0) {
		Render.text("Will you take him up on an offer or leave?", false);
...
public rathazulArmorMenu():void {
	...
	Render.text("Which armor project would you like to pursue with Rathazul?");
	if (player.statusAffects.has("CampRathazul") && player.hasItem(useables.T_SSILK) && flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] + flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00275] == 0) {
		silk = craftSilkArmor;
	}
...
private craftSilkArmor():void {
	...
	if(!player.hasItem(useables.T_SSILK, 5)) {
		Render.text("The rat shakes his head and hands it back to you.  \"<i>This isn't enough for me to make anything with.  I'll need at least five bundles of this stuff total, so you'll need to find more,</i>\" he explains.\n\n", false);
		//(optional spider bonus: 
		if(player.tailType == TAIL.SPIDER_ADBOMEN) {
			Render.text("You show him your spider-like abdomen in response, offering to produce more webbing for him.  Rathazul chuckles dryly, a sound that reminds you of hot wind rushing through a dead valley.  \"<i>Dear child, this would never do.  Silk this tough can only be produced by a true-born spider.  No matter how you change yourself, you'll always be a human at heart.</i>\"\n\n", false);
			Render.text("The old rat shakes his head and adds, \"<i>Well, now that I think about it, the venom of a red widow might be able to transform you until you are a spider to the core, but I have absolutely no idea what that would do to you.  If you ever try such a dangerous, reckless idea, let me know.  I want to have my notebooks handy, for SCIENCE!</i>\"\n\n", false);
		}
		doNext(returnToRathazulMenu);
		return;
	}
	Render.text("The rat limps over to his equipment, spider-silk in hand.  With efficient, practiced motions, he runs a few tests.  As he finishes, he sighs and explains, \"<i>This will be harder than I thought.  The webbing is highly resistant to most of my alchemic reagents.  To even begin to work with such material I will need a number of rare, expensive elements.  I would need 500 gems to even start such a project.</i>\"\n\n", false);
	Render.text("You can't help but sigh when he names such a sizable figure.  Do you give him the 500 gems and spider-silk in order for him to create you a garment?", false);
	if(player.stats.gems < 500) {
		Render.text("  <b>Wait... you don't even have 500 gems.  Damn.</b>", false);
		doNext(returnToRathazulMenu);
		return;
	}
	//[Yes] [No]
...
private collectRathazulArmor():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00275] == 1) Render.text("armor", false);
	else Render.text("robe", false);
	Render.text(" is finished!</i>\"\n\n", false);
	//Robe
	let itype:ItemType;
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00275] == 2) {
		Render.text("Hanging from a small rack is a long, flowing robe.  It glitters brightly in the light, the pearl-white threads seeming to shimmer and shine with every ripple the breeze blows through the soft fabric.  You run your fingers over the silken garment, feeling the soft material give at your touch.  There's a hood with a golden border embroidered around the edge.  For now, it hangs limply down the back, but it would be easy to pull up in order to shield the wearer's eyes from harsh sunlight or rainy drizzle.  The sleeves match the cowl, circled with intricate threads laid out in arcane patterns.\n\n", false);
		
		Render.text("Rathazul gingerly takes down the garment and hands it to you.  \"<i>Don't let the softness of the material fool you.  This robe is tougher than many armors, and the spider-silk's properties may even help you in your spell-casting as well.</i>\"\n\n", false);
		itype = armors.SS_ROBE;
	}
	//(Armor)
	else {
		Render.text("A glittering white suit of armor sits atop a crude armor rack, reflecting the light that plays across its surface beautifully.  You definitely didn't expect anything like this!  It looks nearly identical to a set of light platemail, though instead of having a cold metal surface, the armor feels slightly spongy, with just a little bit of give in it.\n\n", false);
		
		Render.text("While you marvel at the strange equipment, Rathazul explains, \"<i>When you said you wanted armor, I realized I could skip a few of the alchemical processes used to soften material.  The savings let me acquire a cheap metal set of armor to use as a base, and I molded half the armor around each piece, then removed it and created the outer, defensive layers with the rest of the webbing.  Unfortunately, I didn't have enough silk for a solid codpiece, but I did manage to make a you thin loincloth from the leftover scraps  - for modesty.</i>\"\n\n", false);
...
private buyReducto():void {
	...
	if (player.stats.gems >= cost) {
		Render.text("Rathazul hands you the Reducto with a nod before returning to his work.\n\n");
		...
		Render.text("\"<i>I'm sorry, but you lack the gems I need to make the trade,</i>\" apologizes Rathazul.");
		doNext(returnToRathazulMenu);
	}
}
...
public craftCarapace():void {
	...
	if(player.gender >= 2) Render.text(" with stockings and garters", false);
	Render.text(".  He still manages to look somewhat pleased with himself in spite of the blunder, even bragging a little bit, \"<i>Let me show you the different lengths of string I used.</i>\"\n\n", false);
	if(player.lowerBody.cockSpot.count() > 0 && player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 40) Render.text("The silken material does little to hide the bulge of your groin, if anything it looks a little lewd.  Rathazul mumbles and looks away, shaking his head.\n\n", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 8) Render.text("Your " + biggestBreastSizeDescript() + " barely fit into the breastplate, leaving you displaying a large amount of jiggling cleavage.\n\n", false);
