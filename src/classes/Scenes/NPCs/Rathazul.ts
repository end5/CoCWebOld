...
private function rathazulWorkOffer():boolean {
	...
	if(player.statusAffects.has("CampRathazul") && player.statusAffects.get("MetRathazul").value2 >= 4) {
		...
		if(flags[FlagEnum.AMILY_MET_RATHAZUL] >= 2) outputText("50");
		else outputText("100");
...
private function collectRathazulArmor():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00275] == 1) outputText("armor", false);
...
public function craftCarapace():void {
	...
	if(player.lowerBody.cockSpot.count() > 0 && player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 40) outputText("The silken material does little to hide the bulge of your groin, if anything it looks a little lewd.  Rathazul mumbles and looks away, shaking his head.\n\n", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 8) outputText("Your " + biggestBreastSizeDescript() + " barely fit into the breastplate, leaving you displaying a large amount of jiggling cleavage.\n\n", false);
