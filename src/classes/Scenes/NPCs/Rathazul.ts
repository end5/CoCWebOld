...
export default class Rathazul extends NPCAwareContent implements TimeAwareInterface {
	...
	public encounterRathazul(): void {
		...
		if (Flags.list[FlagEnum.MARBLE_PURIFICATION_STAGE] == 2 && player.statusAffects.has("MetRathazul")) {
	...
	public campRathazul(): void {
		...
		if (rand(6) == 0 && Flags.list[FlagEnum.RATHAZUL_CAMP_INTERACTION_COUNTDOWN] == 0) {
			Flags.list[FlagEnum.RATHAZUL_CAMP_INTERACTION_COUNTDOWN] = 3;
			//Pure jojo
			if (Flags.list[FlagEnum.JOJO_RATHAZUL_INTERACTION_COUNTER] == 0 && player.statusAffects.has("PureCampJojo") && Flags.list[FlagEnum.JOJO_DEAD_OR_GONE] == 0) {
	...
	private rathazulWorkOffer(): boolean {
		...
		if (player.statusAffects.has("CampRathazul") && player.statusAffects.get("MetRathazul").value2 >= 4) {
			...
			if (Flags.list[FlagEnum.AMILY_MET_RATHAZUL] >= 2) MainScreen.text("50");
		...
		//SPOIDAH
		if (player.statusAffects.has("CampRathazul") && player.inventory.items.has(useables.T_SSILK) && Flags.list[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] + Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00275] == 0) {
			showArmorMenu = true;
			spoken = true;
			totalOffers++;
		...
		//Vines
		if (player.hasKeyItem("Marae's Lethicite") >= 0 && player.keyItemv2("Marae's Lethicite") < 3 && !player.statusAffects.has("DefenseCanopy") && player.statusAffects.has("CampRathazul")) {
		...
		if (player.statusAffects.has("CampRathazul")) {
			if (Flags.list[FlagEnum.RATHAZUL_DEBIMBO_OFFERED] == 0 && (sophieBimbo.bimboSophie() || player.perks.has("BimboBrains") || player.perks.has("FutaFaculties"))) {
				...
				else if (!player.inventory.items.has(consumables.SMART_T, 5)) MainScreen.text("  You should probably find some if you want that...");
	...
	private collectRathazulArmor(): void {
		...
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00275] == 1) MainScreen.text("armor", false);
	...
	private craftOozeArmor(): void {
		...
		if (!player.statusAffects.has("RathazulArmor")) player.statusAffects.add(new StatusAffect("RathazulArmor", 0, 0, 0, 0)));
	}
