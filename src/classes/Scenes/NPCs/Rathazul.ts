	...
	//After he has crafted 3 things for the player, option to move into camp.
		...
		public timeChange():boolean
		{
			if (flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] > 1) {
				...
				if (flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] < 1) flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] = 1;
				if (flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] > 300) flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] = 24;
			}
			if (flags[FlagEnum.RATHAZUL_CAMP_INTERACTION_COUNTDOWN] > 0) {
				flags[FlagEnum.RATHAZUL_CAMP_INTERACTION_COUNTDOWN]--;
				if (flags[FlagEnum.RATHAZUL_CAMP_INTERACTION_COUNTDOWN] < 0) flags[FlagEnum.RATHAZUL_CAMP_INTERACTION_COUNTDOWN] = 0;
			}
			return false;
		}
...
public encounterRathazul():void {
	...
	//Introduction
	if(player.statusAffects.has("MetRathazul")) {
		if(player.statusAffects.has("CampRathazul"))
			MainScreen.text("You walk over to Rathazul's corner of the camp.  He seems as busy as usual, with his nose buried deep in some tome or alchemical creation, but he turns to face you as soon as you walk within a few paces of him.\n\n", true);
		else
			MainScreen.text("You spy the familiar sight of the alchemist Rathazul's camp along the lake.  The elderly rat seems to be oblivious to your presence as he scurries between his equipment, but you know him well enough to bet that he is entirely aware of your presence.\n\n", true);
	}
	else {
		MainScreen.text("You encounter a hunched figure working as you come around a large bush.  Clothed in tattered robes that obscure most his figure, you can nontheless see a rat-like muzzle protruding from the shadowy hood that conceals most of his form.  A simple glance behind him confirms your suspicions - this is some kind of rat-person.  He seems oblivious to your presence as he stirs a cauldron of viscous fluid with one hand; a neat stack of beakers and phials sit in the dirt to his left.  You see a smile break across his aged visage, and he says, \"<i>Come closer child.  I will not bite.</i>\"\n\nApprehensive of the dangers of this unknown land, you cautiously approach.\n\n\"<i>I am Rathazul the Alchemist.  Once I was famed for my miracle cures.  Now I idle by this lake, helpless to do anything but measure the increasing amounts of corruption that taint its waters,</i>\" he says as he pulls back his hood, revealing the entirety of his very bald and wrinkled head.\n\n", true);
		player.statusAffects.add(new StatusAffect("MetRathazul",0,0,0,0)));
	}
	//Camp offer!
	if(player.statusAffects.get("MetRathazul").value2 >= 3 && player.statusAffects.get("$1").value3 != 1 && player.stats.cor < 75) {
		MainScreen.text("\"<i>You know, I think I might be able to do this worn-out world a lot more good from your camp than by wandering around this lake.  What do you say?</i>\" asks the rat.\n\n(Move Rathazul into your camp?)", false);
	...
	if(!offered) {
		MainScreen.text("He sighs dejectedly, \"<i>I am not sure what I can do for you, youngling.  This world is fraught with unimaginable dangers, and you're just scratching the surface of them.</i>\"\n\nYou nod and move on, leaving the depressed alchemist to his sadness.", false);
...
private rathazulMoveToCamp():void {
	...
	MainScreen.text("Rathazul smiles happily back at you and begins packing up his equipment.  He mutters over his shoulder, \"<i>It will take me a while to get my equipment moved over, but you head on back and I'll see you within the hour.  Oh my, yes.</i>\"\n\nHe has the look of someone experiencing hope for the first time in a long time.");
...
public campRathazul():void {
	...
	if(flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] == 1 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00275] > 0) {
		collectRathazulArmor();
		return;
	}
	//Special rathazul/follower scenes scenes.
	if(rand(6) == 0 && flags[FlagEnum.RATHAZUL_CAMP_INTERACTION_COUNTDOWN] == 0) {
		flags[FlagEnum.RATHAZUL_CAMP_INTERACTION_COUNTDOWN] = 3;
		//Pure jojo
		if(flags[FlagEnum.JOJO_RATHAZUL_INTERACTION_COUNTER] == 0 && player.statusAffects.has("PureCampJojo") && flags[FlagEnum.JOJO_DEAD_OR_GONE] == 0) {
			finter.jojoOffersRathazulMeditation();
			return;
		}
		if(flags[FlagEnum.AMILY_MET_RATHAZUL] == 0 && flags[FlagEnum.AMILY_FOLLOWER] == 1 && amilyScene.amilyFollower()) {
			finter.AmilyIntroducesSelfToRathazul();
			return;
		}
		if(flags[FlagEnum.AMILY_MET_RATHAZUL] == 1 && flags[FlagEnum.AMILY_FOLLOWER] == 1 && amilyScene.amilyFollower()) {
			finter.amilyIngredientDelivery();
			return;
		}
		if(flags[FlagEnum.AMILY_MET_RATHAZUL] == 2 && flags[FlagEnum.AMILY_FOLLOWER] == 1 && amilyScene.amilyFollower()) {
	...
	//Introduction
	MainScreen.text("Rathazul looks up from his equipment and gives you an uncertain smile.\n\n\"<i>Oh, don't mind me,</i>\" he says, \"<i>I'm just running some tests here.  Was there something you needed, " + player.short + "?</i>\"\n\n", true);
	//player.statusAffects.add(new StatusAffect("metRathazul",0,0,0,0)));
	offered = rathazulWorkOffer();
	if (!offered) {
		MainScreen.text("He sighs dejectedly, \"<i>I don't think there is.  Why don't you leave me be for a time, and I will see if I can find something to aid you.</i>\"", false);
		if (player.statusAffects.has("CampRathazul"))
...
private rathazulWorkOffer():boolean {
	...
	if(player.hasItem(consumables.BLACKEG) || player.hasItem(consumables.L_BLKEG)) {
		...
		spoken = true;
		MainScreen.text("He eyes the onyx egg in your inventory and offers a little advice.  \"<i>Be careful with black eggs.  They can turn your skin to living latex or rubber.  The smaller ones are usually safer, but everyone reacts differently.  I'd get rid of them, if you want my opinion.</i>\"\n\n");
	}
	//Item crafting offer
	if(player.hasItem(useables.GREENGL, 2)) {
		if(player.findStatusAffect(StatusAffects.RathazulArmor) < 0) MainScreen.text("He pipes up with a bit of hope in his voice, \"<i>I can smell the essence of the tainted lake-slimes you've defeated, and if you'd let me, I could turn it into something a bit more useful to you.  You see, the slimes are filled with the tainted essence of the world-mother herself, and once the taint is burned away, the remaining substance remains very flexible but becomes nearly impossible to cut through.  With the gel of five defeated slimes I could craft you a durable suit of armor.</i>\"\n\n", false);
		else MainScreen.text("He pipes up with a bit of excitement in his voice, \"<i>With just five pieces of slime-gel I could make another suit of armor...</i>\"\n\n", false);
		spoken = true;
		if (player.hasItem(useables.GREENGL,5)) {
			...
			MainScreen.text("You realize you're still a bit short of gel.\n\n", false);
		}
	}
	//Item crafting offer
	if(player.hasItem(useables.B_CHITN)) {
		MainScreen.text("The elderly rat looks at you intently and offers, \"<i>I see you've gathered a piece of chitin from the giant bees of the forests.  If you bring me five pieces I could probably craft it into some tough armor.</i>\"\n\n", false);
		spoken = true;
		if (player.hasItem(useables.B_CHITN, 5)) {
			...
			MainScreen.text("(You need five pieces of chitin for Rathazul to make you the chitinous armor.)\n\n", false);
		}
	}
	...
	//Single Offer
	if(pCounter == 1) {
		MainScreen.text("The rat mentions, \"<i>I see you have at least one tainted item on you... for 20 gems I could remove most of the taint, making it a good deal safer to use.  Of course, who knows what kind of freakish transformations it would cause...</i>\"\n\n", false);
		spoken = true;
		totalOffers++;
	}
	if(pCounter > 1) {
		MainScreen.text("The rat mentions, \"<i>I see you have a number of demonic items on your person.  For 20 gems I could attempt to remove the taint from one of them, rendering it a good deal safer for consumption.  Of course it would not remove most of the transformative properties of the item...</i>\"\n\n", false);
		spoken = true;
		totalOffers+=2;
	}
	//Offer dyes if offering something else.
	if(player.stats.gems >= 50) {
		MainScreen.text("Rathazul offers, \"<i>Since you have enough gems to cover the cost of materials for my dyes as well, you could buy one of my dyes for your hair.  I will need 50 gems up-front.</i>\"\n\n", false);
		spoken = true;
		totalOffers++;
		dyes = buyDyes;
	}
	...
	if(player.statusAffects.has("CampRathazul") && player.statusAffects.get("MetRathazul").value2 >= 4) {
		MainScreen.text("The rat hurries over to his supplies and produces a container of paste, looking rather proud of himself, \"<i>Good news everyone!  I've developed a paste you could use to shrink down any, ah, oversized body parts.  The materials are expensive though, so I'll need ");
		if(flags[FlagEnum.AMILY_MET_RATHAZUL] >= 2) MainScreen.text("50");
		else MainScreen.text("100");
		MainScreen.text(" gems for each jar of ointment you want.</i>\"\n\n");
		totalOffers++;
		spoken = true;
		reductos = buyReducto;
	}
	//SPOIDAH
	if (player.statusAffects.has("CampRathazul") && player.hasItem(useables.T_SSILK) && flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] + flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00275] == 0) {
		showArmorMenu = true;
		spoken = true;
		totalOffers++;
		MainScreen.text("\"<i>Oooh, is that some webbing from a giant spider or spider-morph?  Most excellent!  With a little bit of alchemical treatment, it is possible I could loosen the fibers enough to weave them into something truly magnificent - armor, or even a marvelous robe,</i>\" offers Rathazul.\n\n", false);
	}
	//Vines
	if(player.hasKeyItem("Marae's Lethicite") >= 0 && player.keyItemv2("Marae's Lethicite") < 3 && player.findStatusAffect(StatusAffects.DefenseCanopy) < 0 && player.statusAffects.has("CampRathazul")) {
		MainScreen.text("His eyes widen in something approaching shock when he sees the Lethicite crystal you took from Marae.  Rathazul stammers, \"<i>By the goddess... that's the largest piece of lethicite I've ever seen.  I don't know how you got it, but there is immense power in those crystals.  If you like, I know a way we could use its power to grow a canopy of thorny vines that would hide the camp and keep away imps.  Growing such a defense would use a third of that lethicite's power.</i>\"\n\n");
		totalOffers++;
		spoken = true;
		lethiciteDefense = growLethiciteDefense;
	}
	if(player.statusAffects.has("CampRathazul")) {
		if(flags[FlagEnum.RATHAZUL_DEBIMBO_OFFERED] == 0 && (sophieBimbo.bimboSophie() || player.perks.has("BimboBrains") || player.perks.has("FutaFaculties"))) {
			rathazulDebimboOffer();
			return true;
		}
		else if(flags[FlagEnum.RATHAZUL_DEBIMBO_OFFERED] > 0) {
			MainScreen.text("You recall that Rathazul is willing to make something to cure bimbo liqueur for 250 gems and five Scholar's Teas.");
			if(player.hasItem(consumables.SMART_T,5) && player.stats.gems >= 250) {
				totalOffers++;
				debimbo = 1; 
			}
			else if(!player.hasItem(consumables.SMART_T,5)) MainScreen.text("  You should probably find some if you want that...");
			else MainScreen.text("  You need more gems to afford that, though.");
			MainScreen.text("\n\n");			
		}
	}
	...
	if(totalOffers > 0) {
		MainScreen.text("Will you take him up on an offer or leave?", false);
...
private purifySomething():void {
	spriteSelect(49);
	MainScreen.clearText();
	MainScreen.text("Rathazul asks, \"<i>What would you like me to purify?</i>\"");
	menu();
	//Item purification offer
	if (player.hasItem(consumables.INCUBID)) {
...
private rathazulPurifyIncubiDraft():void {
	...
	if (player.stats.gems < 20) {
		MainScreen.text("Rathazul says, \"<i>You do not have enough gems for that service.</i>\"");
...
private rathazulDebimboOffer():void {
	...
	if(flags[FlagEnum.RATHAZUL_DEBIMBO_OFFERED] == 0) {
		...
		flags[FlagEnum.RATHAZUL_DEBIMBO_OFFERED]++;
	}
	//Rath menu
	menu();
...
public rathazulArmorMenu():void {
	...
	let gelArmor:Function = (player.hasItem(useables.GREENGL, 5) ? craftOozeArmor : null);
	let silk:Function = null;
	MainScreen.text("Which armor project would you like to pursue with Rathazul?");
	if (player.statusAffects.has("CampRathazul") && player.hasItem(useables.T_SSILK) && flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] + flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00275] == 0) {
		silk = craftSilkArmor;
	}
...
private craftSilkArmor():void {
	...
	MainScreen.text("You hand the bundled webbing to Rathazul carefully, lest you damage the elderly mouse.  He gives you a bemused smile and snatches the stuff from your grasp while he mutters, \"<i>I'm not falling apart you know.</i>\"\n\n", false);
	//(Not enough webs: 
	if(!player.hasItem(useables.T_SSILK, 5)) {
		MainScreen.text("The rat shakes his head and hands it back to you.  \"<i>This isn't enough for me to make anything with.  I'll need at least five bundles of this stuff total, so you'll need to find more,</i>\" he explains.\n\n", false);
		//(optional spider bonus: 
		if(player.tailType == TAIL.SPIDER_ADBOMEN) {
			MainScreen.text("You show him your spider-like abdomen in response, offering to produce more webbing for him.  Rathazul chuckles dryly, a sound that reminds you of hot wind rushing through a dead valley.  \"<i>Dear child, this would never do.  Silk this tough can only be produced by a true-born spider.  No matter how you change yourself, you'll always be a human at heart.</i>\"\n\n", false);
			MainScreen.text("The old rat shakes his head and adds, \"<i>Well, now that I think about it, the venom of a red widow might be able to transform you until you are a spider to the core, but I have absolutely no idea what that would do to you.  If you ever try such a dangerous, reckless idea, let me know.  I want to have my notebooks handy, for SCIENCE!</i>\"\n\n", false);
		}
		doNext(returnToRathazulMenu);
		return;
	}
	MainScreen.text("The rat limps over to his equipment, spider-silk in hand.  With efficient, practiced motions, he runs a few tests.  As he finishes, he sighs and explains, \"<i>This will be harder than I thought.  The webbing is highly resistant to most of my alchemic reagents.  To even begin to work with such material I will need a number of rare, expensive elements.  I would need 500 gems to even start such a project.</i>\"\n\n", false);
	MainScreen.text("You can't help but sigh when he names such a sizable figure.  Do you give him the 500 gems and spider-silk in order for him to create you a garment?", false);
	if(player.stats.gems < 500) {
		MainScreen.text("  <b>Wait... you don't even have 500 gems.  Damn.</b>", false);
		doNext(returnToRathazulMenu);
		return;
	}
	//[Yes] [No]
...
private commissionSilkArmorForReal():void {
	spriteSelect(49);
	MainScreen.text("", true);
	MainScreen.text("You sort 500 gems into a pouch and toss them to Rathazul, along with the rest of the webbing.  The wizened alchemist snaps the items out of the air with lightning-fast movements and goes to work immediately.  He bustles about with enormous energy, invigorated by the challenging task before him.  It seems Rathazul has completely forgotten about you, but as you turn to leave, he calls out, \"<i>What did you want me to make?  A mage's robe or some nigh-impenetrable armor?</i>\"\n\n", false);
...
private declineSilkArmorCommish():void {
	spriteSelect(49);
	MainScreen.text("", true);
	MainScreen.text("You take the silk back from Rathazul and let him know that you can't spend 500 gems on a project like that right now.  He sighs, giving you a crestfallen look and a slight nod of his hooded muzzle.", false);
	doNext(returnToRathazulMenu);
}

public chooseArmorOrRobes(robeType: number):void {
	spriteSelect(49);
	MainScreen.text("Rathazul grunts in response and goes back to work.  You turn back to the center of your camp, wondering if the old rodent will actually deliver the wondrous item that he's promised you.", true);
	doNext(camp.returnToCampUseOneHour);
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00275] = robeType;
...
private collectRathazulArmor():void {
	spriteSelect(49);
	MainScreen.text("", true);
	MainScreen.text("Rathazul beams and ejaculates, \"<i>Good news everyone!  Your ", false);
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00275] == 1) MainScreen.text("armor", false);
	else MainScreen.text("robe", false);
	MainScreen.text(" is finished!</i>\"\n\n", false);
	//Robe
	let itype:ItemType;
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00275] == 2) {
		MainScreen.text("Hanging from a small rack is a long, flowing robe.  It glitters brightly in the light, the pearl-white threads seeming to shimmer and shine with every ripple the breeze blows through the soft fabric.  You run your fingers over the silken garment, feeling the soft material give at your touch.  There's a hood with a golden border embroidered around the edge.  For now, it hangs limply down the back, but it would be easy to pull up in order to shield the wearer's eyes from harsh sunlight or rainy drizzle.  The sleeves match the cowl, circled with intricate threads laid out in arcane patterns.\n\n", false);
		
		MainScreen.text("Rathazul gingerly takes down the garment and hands it to you.  \"<i>Don't let the softness of the material fool you.  This robe is tougher than many armors, and the spider-silk's properties may even help you in your spell-casting as well.</i>\"\n\n", false);
		itype = armors.SS_ROBE;
	}
	//(Armor)
	else {
		MainScreen.text("A glittering white suit of armor sits atop a crude armor rack, reflecting the light that plays across its surface beautifully.  You definitely didn't expect anything like this!  It looks nearly identical to a set of light platemail, though instead of having a cold metal surface, the armor feels slightly spongy, with just a little bit of give in it.\n\n", false);
		
		MainScreen.text("While you marvel at the strange equipment, Rathazul explains, \"<i>When you said you wanted armor, I realized I could skip a few of the alchemical processes used to soften material.  The savings let me acquire a cheap metal set of armor to use as a base, and I molded half the armor around each piece, then removed it and created the outer, defensive layers with the rest of the webbing.  Unfortunately, I didn't have enough silk for a solid codpiece, but I did manage to make a you thin loincloth from the leftover scraps  - for modesty.</i>\"\n\n", false);
		itype = armors.SSARMOR;
	}
	//Reset counters
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00275] = 0;
	flags[FlagEnum.RATHAZUL_SILK_ARMOR_COUNTDOWN] = 0;
...
private buyDyes():void {
	spriteSelect(49);
	MainScreen.clearText();
	MainScreen.text("Rathazul smiles and pulls forth several vials of colored fluids.  Which type of dye would you like?");
	MainScreen.text("\n\n<b>(-50 Gems)</b>");
...
private buyDyeNevermind():void {
	spriteSelect(49);
	MainScreen.clearText();
	MainScreen.text("You change your mind about the dye, and Rathazul returns your gems.\n\n(<b>+50 Gems</b>)");
	player.stats.gems += 50;
	statScreenRefresh();
	doNext(returnToRathazulMenu);
}
...
private buyReducto():void {
	...
	if (player.stats.gems >= cost) {
		MainScreen.text("Rathazul hands you the Reducto with a nod before returning to his work.\n\n");
		...
		MainScreen.text("\"<i>I'm sorry, but you lack the gems I need to make the trade,</i>\" apologizes Rathazul.");
		doNext(returnToRathazulMenu);
	}
}
...
private growLethiciteDefense():void {
	spriteSelect(49);
	MainScreen.clearText();
	MainScreen.text("Rathazul asks, \"<i>Are you absolutely sure?  Growing this thorn canopy as a defense will use one third of the crystal's power.</i>\"\n\n(Do you have Rathazul use the crystal to grow a defensive canopy?)");
...
private growLethiciteDefenseGuessNot():void {
	spriteSelect(49);
	MainScreen.clearText();
	MainScreen.text("Rathazul nods sagely, \"<i>That may be wise.  Perhaps there will be another use for this power.");
	doNext(returnToRathazulMenu);
}

public craftCarapace():void {
	spriteSelect(49);
	MainScreen.text("The rat takes the scales and works on his bench for an hour while you wait.  Once he has finished, Ratzhul is beaming with pride, \"<i>I think you'll be pleased. Go ahead and take a look.</i>\"\n\nHe hands you the armor.  ", true);
	MainScreen.text("The plates shine and shimmer like black steel.  He has used the yellow chitin to add accents and embroidery to the plates with a level of detail and craftsmanship rarely seen back home. A yellow fur neck lining has been fashioned from hairs found on the pieces.  The armor includes a breastplate, shoulder guards, full arm guards, and knee high boots.  You notice there are no pants.  As you turn to ask him where the pants are, you see him scratching his head and hastily rustling in drawers.  He mutters under his breath, \"<i>I'm sorry, I'm sorry, I got so focused on working on the pauldrons that I forgot to make any leg coverings!  Here, this should look good with it, and it won't restrict your movements.</i>\"  He hands you a silken loincloth", false);
	if(player.gender >= 2) MainScreen.text(" with stockings and garters", false);
	MainScreen.text(".  He still manages to look somewhat pleased with himself in spite of the blunder, even bragging a little bit, \"<i>Let me show you the different lengths of string I used.</i>\"\n\n", false);
	if(player.lowerBody.cockSpot.count() > 0 && player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 40) MainScreen.text("The silken material does little to hide the bulge of your groin, if anything it looks a little lewd.  Rathazul mumbles and looks away, shaking his head.\n\n", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 8) MainScreen.text("Your " + biggestBreastSizeDescript() + " barely fit into the breastplate, leaving you displaying a large amount of jiggling cleavage.\n\n", false);
