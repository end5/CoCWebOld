...
internal function loseToSalamander():void {
	...
	//Player Loss – Rape – Female
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText(", your cock shooting a massive rope of spunk onto her face as your vag cums from the tailfucking", false);
		outputText(".\n\n", false);
...
internal function beatUpHelAndStealHerWalletFromHerVagina():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" while releasing her breasts to finger your " + vaginaDescript(0), false);
...
internal function helBlowsYou():void {
	...
	if(player.lowerBody.cockSpot.list[x].cockLength >= 6) outputText(", making her deepthroat you", false);
...
internal function dpHel():void {
	...
	if(flags[FlagEnum.TIMES_HELIA_DOUBLE_DONGED] == 0) outputText("You're my first multi-cock.  Damn, never thought it'd be like THIS!");
...
internal function helTailPegging():void {
	...
	if(player.gender > 0) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("vagina", false);
...
private function nagaCoilsUpHel():void {
	...
	if(flags[FlagEnum.HEL_FUCKBUDDY] == 0) outputText("battle and ", false);
...
private function mountHel():void {
	...
	outputText("You're a little nervous about outright breaking her, but she asked for it.  You trot up and swing your front hooves over her shoulders, resting part of your weight on her as your " + cockDescript(x) + " presses urgently against her firm ass cheeks.  She gasps and groans under your weight and at the prodding of your cock, so desperately seeking a hole to fill.  Finally, you slip between her legs and enter her cunt, ramming your full " + num2Text(int(player.lowerBody.cockSpot.list[x].cockLength)) + " inches into her sweltering depths.  You gasp and shudder at the intense heat of her inner walls, but your lover can only cry with exaltation as your cock fills her to the hilt.\n\n", false);
	...
	if(player.stats.cor < 50) outputText("Sadly, your duties as Champion prevent you from accepting her offer, even if you wanted to.  You tell her as much, and she sighs heavily.", false);
...
private function helVaginaTaur69():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" from around your encapsulated cock", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" and swelling member", false);
	outputText(".\n\n", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" and shaft", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" and filling her throat with jism", false);
	outputText(".\n\n", false);
...
private function receiveCorruptRimjobsFromHel():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Her tongue begins to zero in on your " + vaginaDescript(0) + ", but you give her a quick penalty swat on the tits.  You've got other plans!", false);
	outputText("\n\n", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText(", groping your tits,", false);
...
private function helMinotaurThreesome():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("I've got another hole needs filling!</i>\"", false);
...
private function helMinoThreeSomeLickItsDick():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("crotch", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" while " + sMultiCockDesc() + " ejects its load into the mess on the 'taur's chest", false);
	outputText(".\n\n", false);
...
private function berserkMode():void {
	...
	if(flags[FlagEnum.HEL_ISABELLA_THREESOME_ENABLED] == 0) outputText(", that fat cow Isabella");
...
private function salamanderXIsabellaDiplomacy2():void {
	...
	if(player.stats.cor < 50) outputText("fall silent under their considering gazes.", false);
...
private function nomOnIzzyTitWithSallyMancer():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", and your " + player.armorName + " is tenting visibly, displaying your excitement as you take her soft, delightfully warm and full breast in hand", false);
...
private function noThreesomeSexWithSallyAndIssyLastMinute():void {
	...
	if(player.lowerBody.balls > 0) outputText(" and balls bluer than the lake", false);
	outputText(".", false);
	if(model.time.hours < 6) doNext(playerMenu);
	else doNext(camp.returnToCampUseOneHour);
}
...
private function izzySallyThreeSomeVagoozlaz():void {
	...
	//(IF HERM: 
	if(player.lowerBody.cockSpot.hasCock()) outputText(" and Hel jerking your " + cockDescript(0) + " off", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" and salamander double-team", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", wrapping her warm tail around your " + cockDescript(0) + " lovingly", false);
	outputText(".\n\n", false);
...
private function telHelToGetOffTheMInoCock():void {
	...
	if(player.statusAffects.has("CampMarble")) outputText(", just like how Cowgirls have addictive breastmilk");
	outputText(".");
...
private function satisfyHelSoSheStopsMinoFucking():void {
	...
	if(player.stats.lib >= 50) {
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("[cock] stiffening, ready to go again");
		if(player.gender == 3) outputText(" and your ");
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("[vagina] already soaked with your lustful secretions");
	...
	//{PC has low libido}
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("another dick's ");
