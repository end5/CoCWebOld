...
private valeriaGetFucked():void {
	...
	if(flags[FlagEnum.VELARIA_FUTA] == 0) MainScreen.text("forms a thick, gooey cock");
	...
	//[if PC has cock: \"<i>
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("her gooey ass devouring your " + cockDescript(0) + " as she slides down your belly");
	...
	//(If Male/Herm)
		...
		MainScreen.text(" and fucking your " + cockDescript(0) + " with one fluid, passionate motion.  You're both moaning openly now, nearly overwhelmed by a multitude of pleasures.  You can feel your impending orgasm rising, and by the heightening pitch of Valeria's moans, she seems to be just as close to the edge."); 
...
*/
	...
	MainScreen.text("\n\nYou ");
	if(flags[FlagEnum.TIMES_VALERIA_GOO_THREESOMED] == 0) MainScreen.text("demand that she release you");
...
private valeriaGooRapeII():void {
	...
	if(flags[FlagEnum.TIMES_VALERIA_GOO_THREESOMED] == 0) MainScreen.text(", unexpected");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.totalCocks() > 1) MainScreen.text("s");
		MainScreen.text(". You inadvertantly clench your muscles in a way that makes your " + multiCockDescriptLight() + " so very swollen, in turn making it that much easier for the gelatinous fiend to have her way with your cumslit");
		if(player.totalCocks() > 1) MainScreen.text("s");
		MainScreen.text(". You feel that warm, wet fluid slowly peel it open and push inside. It doesn't hurt at all, but it is a little weird having your " + multiCockDescriptLight() + " slowly stuffed in such a way. The warmth travels the whole way down your dick");
		if(player.totalCocks() > 1) MainScreen.text("s");
	...
	if(flags[FlagEnum.TIMES_VALERIA_GOO_THREESOMED] > 0) MainScreen.text("You grin in eager anticipation, waiting to feel the flood of fluid that you know is coming. It won't take long.... You feel it in your butt first.");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.wetness() <= 2) MainScreen.text("leaking");
		else if(player.wetness() <= 3) MainScreen.text("dripping");
	...
	if(player.upperBody.chest.hasFuckableNipples() && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) {
		...
		if(player.bRows() > 1) MainScreen.text("top row of breasts");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5 && player.upperBody.chest.hasFuckableNipples()) {
		...
		MainScreen.text(" Your tits have grown much larger, " + player.breastCup(0) + "-cups at least.");
	}
...
public birthOutDatGooSlut():void {
	...
	if(player.skinType == SKIN.FUR) MainScreen.text("furry ");
	...
	if(player.statusAffects.get("GooStuffed").value4 > 0) {
		...
		if(player.bRows() > 1) MainScreen.text(" and the next row down");
		if(player.bRows() > 2) MainScreen.text(", and so on");
