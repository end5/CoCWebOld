...
internal function pcWinsValeriaSpar():void {
	clearOutput();
	spriteSelect(79);
	Render.text("\"<i>Oof!</i>\" Valeria grunts, nearly losing her human shape as she tumbles to the hard-packed dirt.  \"<i>All right, all right, you win!  Take it easy, partner,</i>\" she laughs, letting her greatsword dissipate back into her body.");
...
public function pcWinsValeriaSparDefeat():void {
	...
	if(player.HP < 1) Render.text("in too much pain");
	else Render.text("too turned on");
	Render.text(" to fight any longer.  Valeria's on you in a heartbeat, her gooey greatsword pressed to your throat.");
	...
	if(player.HP < 1 || player.gender == 0) {
		Render.text(", offering you a hand up.");
	...
	//[else; If PC lost via lust & has a gender: 
		...
		Render.text(".  \"<i>Well, since you're so... eager... I might as well get my daily fluids while we're here.</i>\"");
...
private function followersValeriaSex(display:boolean = true):void {
	...
	if(display) {
		clearOutput();
		Render.text("With a lusty grin, you ask your gooey friend if ");
		if(player.gender > 0) Render.text("she's interested in a little fluid exchange");
		else Render.text("she wouldn't mind helping you get some release, despite your genderlessness");
		Render.text(".");
	...
	if(flags[FlagEnum.VELARIA_FUTA] == 1) {
		dickText = "Lose Dick";
	}
...
private function valeriaDickToggle():void {
	...
	if(flags[FlagEnum.VELARIA_FUTA] == 1) {
		Render.text("Before you do anything, you ask Valeria if she wouldn't mind hiding her facsimile of a dick, at least when you aren't having sex.");
		...
		flags[FlagEnum.VELARIA_FUTA] = 0;
	}
	//[If Valeria doesn't have a dick:] 
	else {
		flags[FlagEnum.VELARIA_FUTA] = 1;
		Render.text("Struggling to find a less-than-awkward way of phrasing this, you ask Valeria if she wouldn't mind growing a certain extra appendage for you.");
...
private function valeriaGetFucked():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("stroking the slick lips of your [vagina], ask Valeria to fuck you.");
	else Render.text("posing seductively, ask Valeria to fuck you.");
	Render.text("\n\nShe grins.  \"<i>You just lie back and let me take care of everything, partner.</i>\"  You do as she asks, flopping onto your back ");
	if(player.isBiped()) Render.text("and spreading your legs ");
	Render.text("as Valeria ");
	if(flags[FlagEnum.VELARIA_FUTA] == 0) Render.text("forms a thick, gooey cock");
	else Render.text("grabs her thick, gooey cock");
	Render.text(" and looms over you.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) Render.text("bumping over your [chest] and stopping for a brief moment to tickle your sensitive tits with her goopy bottom before ");
	Render.text("coming to a stop over your crotch.  She pours herself into your lap, ");
	//[if PC has cock: \"<i>
	if(player.lowerBody.cockSpot.hasCock()) Render.text("her gooey ass devouring your " + cockDescript(0) + " as she slides down your belly");
	if(player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and ");
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(" letting bits of her goo-body slither into your wet, waiting [vagina], ever so slightly peeling your walls apart");
	//If PC has no gender 
	if(player.gender == 0) Render.text("poking the shaft of her cock gingerly, teasingly against your [asshole]");
	Render.text(".");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text(", as she rhythmically flexes her ass cheeks, shifting her fluidity around your shaft within her with a feeling like lotion-covered hands gently stroking you");
	Render.text(".");
	...
	if(!player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) {
		...
		Render.text(" deeper inside you than ever before.  You feel streaks of goo pouring into you as she cums, pouring her essence into your womb ");
		if(player.upperBody.chest.hasFuckableNipples()) Render.text("and [chest] ");
		Render.text("until slime overflows from inside you, pooling under her hips.  Filled beyond your capacity, you scream your pleasure and cum, clamping down on Valeria's cock");
		if(player.upperBody.chest.hasFuckableNipples()) Render.text("s");
		Render.text(" until they literally pop inside you, flowing back out in goopy streaks.");
	}
	//(If Male/Herm)
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			...
			Render.text(", the double-attack's pleasure is so great that you barely even notice her creating a second cock above the first and pouring it into your unused [vagina], filling your last hole up with a firm, gooey cock");
		}
		Render.text(".");
		player.buttChange(10,true,true,false);
		
		Render.text("\n\nValeria begins to buck her hips in your lap, stuffing your hole");
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("s");
		Render.text(" and fucking your " + cockDescript(0) + " with one fluid, passionate motion.  You're both moaning openly now, nearly overwhelmed by a multitude of pleasures.  You can feel your impending orgasm rising, and by the heightening pitch of Valeria's moans, she seems to be just as close to the edge."); 
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("s");
		Render.text(" literally popping as she climaxes, filling your stretched ass ");
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("and vag ");
		Render.text("with warm, sticky goop.");
	}
	...
	if(player.gender > 0) Render.text("patting her belly full of your juices, ");
	Render.text("and looms over you.  \"<i>That was fun, partner,</i>\" she says, leaning down to give you a wet peck on the cheek. \"<i>Let's do that again soon, all right?</i>\"");
...
private function gooFlation(clearText:boolean = true):void {
	...
	if(clearText) {
		clearOutput();
		Render.text("\"<i>Just relax, partner,</i>\" she grins, reaching up to run a hand through your [hair].  \"<i>Just let ol' Valeria take care of everything...</i>\"");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) Render.text("one lonely ");
	Render.text("hole.  You gulp as the tentacle-fuck begins, her long tendrils pouring one by one into your [asshole], each so small and soft it easily slips inside you.  One piles onto another, tickling and teasing your anal walls as more and more of her slithers into you.");
	...
	if(clearText) {
		Render.text("\n\nYou collapse, goop flowing freely from your abused body.  Laughing, Valeria pours out of your lap, ");
		if(player.gender > 0) Render.text("patting her belly full of your juices, ");
		Render.text("and looms over you.  \"<i>That was fun, partner,</i>\" she says, leaning down to give you a wet peck on the cheek. \"<i>Let's do that again soon, alright?</i>\"");
		player.orgasm();
		dynStats("sen", 1);
		HPChange(25,false);
		doNext(camp.returnToCampUseOneHour);
...
private function valeriaSexDominated():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina() && (!player.lowerBody.cockSpot.hasCock() || rand(2) == 0)) {
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) {
			...
			if(player.lactationQ() > 0) Render.text("  To her delight, a spray of warm milk jets out of your sore nipples, milky white mixing into blue goo like oil in water.  \"<i>Mmm, tasty!</i>\" she teases, massaging more and more from you.");
		}
		...
		Render.text(player.mf("boy","girl") + " ready to cum?</i>\" she withdraws herself from you, leaving you feeling suddenly very, very empty.  \"<i>I didn't say you could cum yet, slut! I'm not done with you yet.</i>\"");
		...
		if(player.vaginalCapacity() > 80 || player.analCapacity() > 80) Render.text(" and your own receptibility");
		Render.text(".  Hugging your back, the goo-girl grabs at your [chest], squeezing your tits and pinching at your nipples.  \"<i>Moan for me, bitch,</i>\" Valeria growls into your ear, giving your [nipples] a particularly violent tug.  You let out a lewd, whorish moan.  The goo-girl grins, her tongue lashing out to lick your ear as she rewards you with a single long thrust into your depths, filling and stretching your [vagina] and [asshole] completely before withdrawing.");
		...
		if(player.lactationQ() > 0) Render.text("and milk ");
		Render.text("swirling around inside her.  \"<i>We'll do this again sometime,</i>\" she adds, walking off to another part of camp with a wink.");
	}
	//(PC has Dick)
	else {
		...
		Render.text(" " + player.mf("boy","girl") + " ready to cum? Well, go on then - we've got time for more.</i>\"");
...
private function talkWithValeria():void {
	...
	if(player.race() != "human") Render.text(" were");
	Render.text(".  I just, I dunno, changed, I guess.</i>\"");
	Render.text("\n\nChanged?");
...
private function flirtWithValeria():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("brushing along the length of your rapidly-hardening cock");
	else Render.text("brushing a wet, warm knuckle across your [clit]");
	Render.text(".");
...
private function acceptValeriasNeeds():void {
	...
	if(player.statusAffects.has("SlimeCraving")) Render.text(" after all, you have the same ones");
	Render.text(", as long as they don't interfere with your mission as Champion.");
...
public function valeriaAndGooThreeStuff():void {
	...
	list.push("[asshole]");
	list.push("[nipples]");
	Render.text(formatStringArray(list) + " with liquid-soft caresses, almost ephemeral and yet still so perfectly all-consuming, filling every tiny imperfection in your [skin] with blue, gooey goodness and teasing whatever entrances she can get at. ");
/*
	clearList();
	addToList("[hips]");
	...
	addToList("[asshole]");
	addToList("[nipples]");
	Render.text(outputList() + " with liquid-soft caresses, almost ephemeral and yet still so perfectly all-consuming, filling every tiny imperfection in your [skin] with blue, gooey goodness and teasing whatever entrances she can get at. ");
*/
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("You sigh as [eachCock] stands at full erection. Bluish coating");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" barely thicker than a condom stretch");
		if(player.lowerBody.cockSpot.count() == 1) Render.text("es");
		Render.text(" around ");
		if(player.lowerBody.cockSpot.count() == 1) Render.text("it as it distends");
		else Render.text("them as they distend");
		Render.text(" Valeria's increasingly liquescent form. Dripping off the tip");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(", sapphire slime falls in long gooey stands, yet the whole time you feel as if a dozen hands are dragging lubricant-coated feathers over the whole of your sensitive organ");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(".");
	}
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("You sigh your as [vagina] is tenderly caressed by what feels like dozens of lubricant-soaked feathers, tickling at the skin in a way that makes your entrance grow hot enough to make you wonder why Valeria hasn't been cooked into steam. The airtight seal over your mons is steadily shifting around and about to stimulate you. Sometimes, a narrow band of semi-solid goo slips between your lips like an oh-so-narrow thong, the slime mixing with your own suddenly-copious sexual juices to form a salacious, barely restrained soup of girlish need. She holds herself back from diving into your entrance... for now.");
	else Render.text("You sigh as your [asshole] is tenderly licked and tickled by what feels like dozens of inhumanly long tongues, slathering over it in such wide circles that your cheeks get a thorough lashing as well. Bands as firm and strong as leather straps slip into place around your [butt] to spread you wide open, and one of the slender, slime-crafted organs slithers deeper to taste your rosebud, never quite pushing in.");
	...
	if(flags[FlagEnum.TIMES_VALERIA_GOO_THREESOMED] == 0) Render.text("\n\nUnfortunately for you, the fuck-hungry goo-slut is intent on taking command here.");
	else Render.text("\n\nJust like last time, the fuck-hungry goo-slut is taking command.");
	Render.text("  She surprises you by contorting your body with pressure from an unexpected angle, tipping you head over heels, right toward the wide-eyed goo-girl you just managed to defeat!");
	...
	Render.text("\n\nThe bluish");
	if (monster.skinTone != "blue") Render.text(" " + (monster as GooGirl).gooColor6());
	Render.text(" juices slowly meld together until you cannot tell one from the other, and they firm up into a semi-translucent, flat-bottomed sphere, trapping you there. Your debauched and hopelessly aroused state is visible to any who would wander by, stroked by dark-colored currents that wrap around your ");
	if(player.lowerBody.cockSpot.hasCock() || player.lowerBody.vaginaSpot.hasVagina()) Render.text("genitals");
	else Render.text("[asshole]");
	Render.text(" with sinful tightness. An involuntary shiver of excitement traverses up your spine, though your body is held almost completely stock-still in its sloshing vice.");
	...
	Render.text("\n\nYou ");
	if(flags[FlagEnum.TIMES_VALERIA_GOO_THREESOMED] == 0) Render.text("demand that she release you");
	else Render.text("playfully comment that she really ought to release you");
	Render.text(", but the troublesome, liquid armor shakes her head negatively.");
...
private function valeriaGooRapeII():void {
	...
	Render.text(" once more, and this time you catch a glimpse of it out of the corner of your eye. It's the goo-girl's core, clasped in a fist of deep-blue. It pulses faintly as it's held there, just before you, while subtle vibrations and temperature fluctuations alert you to some kind of secret communications between the two women. Just what are they planning?");
	Render.text("\n\nYou get your answer in the form of a sudden");
	if(flags[FlagEnum.TIMES_VALERIA_GOO_THREESOMED] == 0) Render.text(", unexpected");
	else Render.text(" but expected");
	Render.text(" pressure at your [asshole]. It's so... so slick that you have to wonder how they can manage to push with such authority at your rearmost orifice, but they manage it all the same. You clench to deny them entrance, squeezing your sphincter shut tighter than locked door. Valeria's giggling voice coos from somewhere below, \"<i>Oh, don't be a spoilsport. Come on, it'll be fun.</i>\"");
	...
	if(player.upperBody.chest.hasFuckableNipples() && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(" as your snatch");
		else Render.text(" as the randiest slut's snatch");
		Render.text(". Valeria takes this as an invitation, and " + num2Text(player.upperBody.chest.countNipples()) + " phallic objects greedily push at your cunt-nipples, eagerly sliding themselves inside your welcome chest-pussies. Looking down, you can see the navy-blue outlines of the phallic tendrils waving and writhing as they push their way deep, embedding themselves tit-deep in blissful warmth.");
	}
	//Dick
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("\n\nValeria's voice calls out once more, \"<i>You didn't think I'd forget ");
		if(player.lowerBody.cockSpot.count() == 1) Render.text("this little guy");
		else Render.text("these little guys");
		Render.text(", did you? ");
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 50) Render.text("...Maybe big guy is a bit more appropriate. Whatever. ");
		Render.text("I was just saving the best for last. Hold on tight, tiger.</i>\"");
		...
		Render.text(" in place, and something even warmer than the slippery container");
		if(player.totalCocks() > 1) Render.text("s");
		Render.text(" pushes at your head");
		if(player.totalCocks() > 1) Render.text("s");
		Render.text(", circling your urethra");
		if(player.totalCocks() > 1) Render.text("s");
		Render.text(". You inadvertantly clench your muscles in a way that makes your " + multiCockDescriptLight() + " so very swollen, in turn making it that much easier for the gelatinous fiend to have her way with your cumslit");
		if(player.totalCocks() > 1) Render.text("s");
		Render.text(". You feel that warm, wet fluid slowly peel it open and push inside. It doesn't hurt at all, but it is a little weird having your " + multiCockDescriptLight() + " slowly stuffed in such a way. The warmth travels the whole way down your dick");
		if(player.totalCocks() > 1) Render.text("s");
		Render.text(" to your middle, where you can feel it travel a bit further before it finally stops at your [balls].");
	}
	//Merge
	Render.text("\n\nGiggling, the sapphire slut does something that makes the body-wrapping sphere jiggle and contract slightly. ");
	if(flags[FlagEnum.TIMES_VALERIA_GOO_THREESOMED] > 0) Render.text("You grin in eager anticipation, waiting to feel the flood of fluid that you know is coming. It won't take long.... You feel it in your butt first.");
	else Render.text("At first, you don't realize what it is. You're floating in a bit of a sexual daze by now and barely cogent of what's going on, but it does feel quite nice, whatever it is. The spreading heat in your posterior is what finally clues you in to what's happening.");
	Render.text(" Pumping deep into your bowels, the hardened protrusion is no longer simply stuffing you. It's pumping something slick and wet and oh so gooey straight into your [asshole], filling you up so strangely that you aren't sure what to make of beyond how... well, good it feels to be stuffed like this.");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.wetness() <= 2) Render.text("leaking");
		else if(player.wetness() <= 3) Render.text("dripping");
		else Render.text("squirting");
		Render.text(" your girlcum in feverish releases of ecstasy, repeatedly getting off on this perverse, doubled slime-flation.");
	}
	//{titties}
	if(player.upperBody.chest.hasFuckableNipples() && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) {
		...
		if(player.bRows() > 1) Render.text("top row of breasts");
		else Render.text("breasts");
		Render.text(" pulling away from your body, gaining cup sizes in a span of seconds, your skin stretching taut under the moist deluge. Your tit-twats quiver with delight, feeling nothing but obscene, ever-growing pleasure until they're squeezing down on the sloppy tentacle-cocks, milking them to cum harder and faster, which in turn bloats your ecstatic tits into blissful, bloated orbs.");
	}
	//{cocks!}
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("\n\nThe sealed, cock-plugging tube");
		if(player.totalCocks() > 1) Render.text("s");
		Render.text(" in [eachCock] ");
		if(player.totalCocks() > 1) Render.text("do not");
		else Render.text("does not");
		Render.text(" differ greatly from Valeria's other liquid attentions. ");
		if(player.totalCocks() > 1) Render.text("They start");
		else Render.text("It starts");
		Render.text(" by thickening, stretching your cumvein");
		if(player.totalCocks() > 1) Render.text("s");
		Render.text(" as wide as ");
		if(player.totalCocks() > 1) Render.text("they");
		else Render.text("it");
		Render.text("'ll go without pain and then rolling thick pulses of gelatinous matter into your urethra");
		if(player.totalCocks() > 1) Render.text("s");
		Render.text("s");
		Render.text(". The sleeve");
		if(player.totalCocks() > 1) Render.text("s");
		Render.text(" squeeze");
		if(player.totalCocks() == 1) Render.text("s");
		Render.text(" down on your length");
		if(player.totalCocks() > 1) Render.text("s");
		Render.text(" harder, starting to slide around in ways that would be impossible for a vagina, and you find yourself helplessly blowing your load straight into the dick-plug");
		if(player.totalCocks() > 1) Render.text("s");
		Render.text(", spurting bliss that can't even hope to escape from your loins. You feel ");
		if(player.lowerBody.balls == 0) Render.text("something expanding as you cum, filling up with spunk or slime or some mixture of the two");
		else Render.text("your [balls] expanding as you cum, filling up with spunk or slime or some mixture of the two, weighing down your sack with ever-increasing heaviness. Tender licks of tongue roll across the surface as you fill beyond completely");
		Render.text(".");
	}
	//Butt fillinggasm
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5 && player.upperBody.chest.hasFuckableNipples()) Render.text(", pausing to handle your hefty, swelling tits");
	Render.text(", moaning and groaning as Valeria's shape gradually coalesces back around you. The goo-girl's core vanishes from view and reappears down south, pressing against your [asshole] as the \"pipe\" that filled you so full melts into slime. You scream in anal ecstasy as it slips past your ring, lodging firmly in your butt while the jellied mass inside convulses, rubbing against your walls in all the right ways, almost like it's experiencing an orgasm all its own.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5 && player.upperBody.chest.hasFuckableNipples()) {
		for(let x: number = 0; x < player.bRows(); x++) {
			player.upperBody.chest.list[x].breastRating += 3 + rand(3);
		}
		Render.text(" Your tits have grown much larger, " + player.breastCup(0) + "-cups at least.");
	}
	if(player.lowerBody.cockSpot.hasCock() && player.lowerBody.balls > 0) {
		player.lowerBody.ballSize += 3 + rand(2);
		Render.text(" Your balls look positively swollen with libidinous juices.");
	}
	Render.text(" Your belly is stuffed into a parody of a pregnant woman's");
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(", so gravid from both holes that a real mother would be shamed by your obscene \"pregnancy.\"");
	else Render.text(".");
	...
	flags[FlagEnum.TIMES_VALERIA_GOO_THREESOMED]++;
	player.orgasm();
	//v1 = time till birth.
...
public function birthOutDatGooSlut():void {
	...
	if(player.skinType == SKIN.FUR) Render.text("furry ");
	Render.text("bitch in heat. Gods, there's so much!");
	if(player.statusAffects.get("GooStuffed").value4 > 0) {
		Render.text("\n\n");
		if(!player.upperBody.chest.hasFuckableNipples()) Render.text("Stiffening up");
		else Render.text("Opening up");
		Render.text(", your [nipples], suddenly disgorge rivers of gelatinous slime, adding to growing puddle below with, odd-colored obscene lactation. You grab one with one hand and squeeze, shooting out huge streamers of milky-goo until your tit is sore and aching. You repeat on the other side");
		if(player.bRows() > 1) Render.text(" and the next row down");
		if(player.bRows() > 2) Render.text(", and so on");
		Render.text(", working out every drop of thick, body-filling slime while your ass is still busy draining. Even after they empty, they don't seem to lose a single bit of the size they've gained....");
	}
