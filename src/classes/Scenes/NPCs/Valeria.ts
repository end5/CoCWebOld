...
private function valeriaGetFucked():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("stroking the slick lips of your [vagina], ask Valeria to fuck you.");
	...
	if(flags[FlagEnum.VELARIA_FUTA] == 0) outputText("forms a thick, gooey cock");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) outputText("bumping over your [chest] and stopping for a brief moment to tickle your sensitive tits with her goopy bottom before ");
	outputText("coming to a stop over your crotch.  She pours herself into your lap, ");
	//[if PC has cock: \"<i>
	if(player.lowerBody.cockSpot.hasCock()) outputText("her gooey ass devouring your " + cockDescript(0) + " as she slides down your belly");
	if(player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) outputText(" and ");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" letting bits of her goo-body slither into your wet, waiting [vagina], ever so slightly peeling your walls apart");
	//If PC has no gender 
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", as she rhythmically flexes her ass cheeks, shifting her fluidity around your shaft within her with a feeling like lotion-covered hands gently stroking you");
	outputText(".");
	...
	if(!player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.upperBody.chest.hasFuckableNipples()) outputText("and [chest] ");
	...
	//(If Male/Herm)
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("s");
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("and vag ");
...
private function gooFlation(clearText:boolean = true):void {
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) outputText("one lonely ");
...
private function flirtWithValeria():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("brushing along the length of your rapidly-hardening cock");
...
private function acceptValeriasNeeds():void {
	...
	if(player.statusAffects.has("SlimeCraving")) outputText(" after all, you have the same ones");
...
*/
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		outputText(" around ");
		if(player.lowerBody.cockSpot.count() == 1) outputText("it as it distends");
	...
	if(player.lowerBody.cockSpot.hasCock() || player.lowerBody.vaginaSpot.hasVagina()) outputText("genitals");
	...
	outputText("\n\nYou ");
	if(flags[FlagEnum.TIMES_VALERIA_GOO_THREESOMED] == 0) outputText("demand that she release you");
...
private function valeriaGooRapeII():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) outputText("the valley of your cleavage");
	...
	if(flags[FlagEnum.TIMES_VALERIA_GOO_THREESOMED] == 0) outputText(", unexpected");
	...
	if(player.upperBody.chest.hasFuckableNipples() && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" as your snatch");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.count() == 1) outputText("this little guy");
		else outputText("these little guys");
		outputText(", did you? ");
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 50) outputText("...Maybe big guy is a bit more appropriate. Whatever. ");
	...
	if(flags[FlagEnum.TIMES_VALERIA_GOO_THREESOMED] > 0) outputText("You grin in eager anticipation, waiting to feel the flood of fluid that you know is coming. It won't take long.... You feel it in your butt first.");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.balls == 0) outputText("something expanding as you cum, filling up with spunk or slime or some mixture of the two");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5 && player.upperBody.chest.hasFuckableNipples()) outputText(", pausing to handle your hefty, swelling tits");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(", so gravid from both holes that a real mother would be shamed by your obscene \"pregnancy.\"");
	else outputText(".");
...
public function birthOutDatGooSlut():void {
	...
	if(player.statusAffects.get("GooStuffed").value4 > 0) outputText(" Your tits are even wobbling around wildly, shaking and jiggling obscenely inside your [armor] in a way that makes you your [nipples] more than a little leaky.");
	...
	if(player.statusAffects.get("GooStuffed").value4 > 0) {
		outputText("\n\n");
		if(!player.upperBody.chest.hasFuckableNipples()) outputText("Stiffening up");
	...
	if(player.lowerBody.cockSpot.hasCock() && player.statusAffects.get("GooStuffed").value2 > 0) {
		...
		if(player.lowerBody.balls > 0) outputText("ball-clenching");
		...
		if(player.statusAffects.get("GooStuffed").value2 >= 2) outputText("  Your balls keep their enhanced sized. Odd.");
	}
