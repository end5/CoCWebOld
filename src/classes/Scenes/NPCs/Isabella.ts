	...
	public class Isabella extends Monster
	{
		...
		public isabellaAttack():void {
			...
			if(statusAffects.has("Blind") && rand(3) < 2) {
				Render.text("Isabella blindly tries to charge at you, but misses completely.\n", false);
			}
			//Determine if dodged!
			else if(player.stats.spe - spe > 0 && int(Math.random()*(((player.stats.spe-spe)/4)+80)) > 80) {
				Render.text("You duck aside at the last moment, relying entirely on your speed.\n", false);
			}
			//Determine if evaded
			else if(player.perks.has("Evade") && rand(100) < 10) {
				Render.text("You easily evade her incredibly linear attack.\n", false);
			}
			//("Misdirection"
			else if(player.perks.has("Misdirection") && rand(100) < 10 && player.armorName == "red, high-society bodysuit") {
				Render.text("You easily misdirect her and step aside at the last moment.\n", false);
			}
			//Determine if cat'ed
			else if(player.perks.has("Flexibility") && rand(100) < 6) {
				Render.text("You throw yourself out of the way with cat-like agility at the last moment, avoiding her attack.\n", false);
			}
			else {
				let damage:number;
				damage = Math.round((weaponAttack + str + 20) - rand(player.tou+player.armorDef));
				if(damage < 0) {
					Render.text("You brace yourself and catch her shield in both hands, dragging through the dirt as you slow her charge to a stop.  She gapes down, completely awestruck by the show of power.", false);
				}
				else {
					damage = player.takeDamage(damage);
					Render.text("She's coming too fast to dodge, and you're forced to try to stop her.  It doesn't work.  Isabella's shield hits you hard enough to ring your ears and knock you onto your back with bruising force. (" + damage + ")\n", false);
				}
			}
			combatRoundOver();
		}
		...
		public isabellaStun():void {
			...
			//Determine if cat'ed
				...
				Render.text("You bend backward with cat-like agility to avoid her attack.\n", false);
			}
			else {
				let damage:number = 0;
				damage = Math.round((weaponAttack + str) - rand(player.tou+player.armorDef));
				if(damage < 0) {
					Render.text("You deflect her blow away, taking no damage.\n", false);
					damage = 0;
				}
				else if(player.perks.has("Resolute") && player.tou >= 75) {
					Render.text("You resolutely ignore the blow thanks to your immense toughness.\n");
					damage = 0;
				}
				else {
					damage = player.takeDamage(damage);
					Render.text("You try to avoid it, but her steely attack connects, rocking you back.  You stagger about while trying to get your bearings, but it's all you can do to stay on your feet.  <b>Isabella has stunned you!</b> (" + damage + ")\n", false);
		...
		public isabellaThroatPunch():void {
			...
			//Determine if cat'ed
				...
				if(damage <= 0) {
					Render.text("You manage to block her with your own fists.\n", false);
				}
				else if(player.perks.has("Resolute") && player.tou >= 75) {
					Render.text("You resolutely ignore the blow thanks to your immense toughness.\n");
				}
