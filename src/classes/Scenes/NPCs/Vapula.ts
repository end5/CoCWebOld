package classes.Scenes.NPCs
{
	...
	public class Vapula extends NPCAwareContent implements TimeAwareInterface
	{
		...
		public timeChange():boolean
		{
			if (model.time.hours > 23) {
				if (vapulaSlave() && flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 0) {
					if (flags[FlagEnum.VAPULA_HAREM_FUCK] == 0) flags[FlagEnum.VAPULA_DAYS_SINCE_FED]++;
					else flags[FlagEnum.VAPULA_DAYS_SINCE_FED] = 0;
				}
				if (flags[FlagEnum.VAPULA_FOLLOWER] == .5 || flags[FlagEnum.VAPULA_FOLLOWER] == 1.5) flags[FlagEnum.VAPULA_FOLLOWER]++;
				flags[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS]++;
			}
			if (vapulaSlave() && player.hasKeyItem("Demonic Strap-On") < 0 && player.gender == 2 && flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 0) {
		...
		public timeChangeLarge():boolean {
			if (flags[FlagEnum.VAPULA_FOLLOWER] >= 2.5 && model.time.hours == 6 && flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 0) {
				femaleVapulaRecruitmentPartII();
				return true;
			}
			if (model.time.hours == 2 && vapulaSlave() && flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 0 && flags[FlagEnum.VAPULA_DAYS_SINCE_FED] >= 5 && (player.lowerBody.cockSpot.hasCock() || (player.hasKeyItem("Demonic Strap-On") >= 0 && player.lowerBody.vaginaSpot.hasVagina()))) {
				vapulaForceFeeds();
				return true;
			}
			return false;
		...
		public vapulaSlave():boolean {
			return flags[FlagEnum.VAPULA_FOLLOWER] == 1;
...
//Camp lines
		public vapulaSlaveFlavorText():void
		{
			...
			if (choice == 2) {
				if (flags[FlagEnum.VAPULA_HAREM_FUCK] == 0) choice = 3;
			...
			if (choice == 5) {
				if (flags[FlagEnum.VAPULA_HAREM_FUCK] == 0 || !(sophieBimbo.bimboSophie())) choice = 6;
			...
			if (choice == 6) {
				if (flags[FlagEnum.VAPULA_HAREM_FUCK] == 0 || flags[FlagEnum.IZMA_NO_COCK] > 0 || !izmaFollower()) choice = 7;
			...
			if (choice == 7) {
				...
				if (flags[FlagEnum.VAPULA_HAREM_FUCK] == 0 || !jojoScene.campCorruptJojo()) choice = 8;
				else {
			...
			if (choice == 8) {
				if (flags[FlagEnum.VAPULA_HAREM_FUCK] == 0 || !(amilyScene.amilyFollower() && amilyScene.amilyCorrupt())) choice = 9;
...
//switch follower names depending on conditions
		public mouseWaifuFreakout(amily:boolean = false, jojo:boolean = false):void
		{
			...
			if (jojo) {
				...
				flags[FlagEnum.JOJO_MOVE_IN_DISABLED] = 1;
...
//Follower Summoning text (Z)
		public callSlaveVapula(output:boolean = true):void
		{
			if (output) {
				MainScreen.clearText();
				if (flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 0)
				{
				...
				else
				{
					...
					if (flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA_GIBS_MILK] == 1)
					{
						flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA_GIBS_MILK] = 2;
			...
			if (flags[FlagEnum.VAPULA_EARNED_A_SPANK] > 0 && !player.isTaur() && !player.isDrider()) spank = spankVapulaLikeABoss;
			...
			if (flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 0 && flags[FlagEnum.FARM_CORRUPTION_STARTED] == 1) MainScreen.addButton(6, "Farm Work", sendToFarm);
			if (flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 1) MainScreen.addButton(6, "Go Camp", backToCamp);
			
			if (flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 1 && flags[FlagEnum.FOLLOWER_PRODUCTION_VAPULA] == 0) MainScreen.addButton(7, "Harvest Milk", harvestMilk);
			if (flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 1 && flags[FlagEnum.FOLLOWER_PRODUCTION_VAPULA] == 1) MainScreen.addButton(7, "Stop Harvest", stopHarvest);
					
			if (flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 1) MainScreen.addButton(9, "Back", kGAMECLASS.farm.farmCorruption.rootScene);
		}
		...
		private sendToFarm():void
		{
			...
			flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] = 1;
		...
		private backToCamp():void
		{
			...
			flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] = 0;
		...
		private harvestMilk():void
		{
			...
			if (flags[FlagEnum.FARM_UPGRADES_REFINERY] == 0) MainScreen.text("“<i>It’ll sure as hell beat whatever disgusting thing the bitch would have me doing instead. Why don’t you make her build a concentration machine? Lethice has loads; makes one dose of cream go so much further.</i>”");
			else MainScreen.text("“<i>As you insist, [master]. It’ll sure as hell beat working.</i>”");
			
			flags[FlagEnum.FOLLOWER_PRODUCTION_VAPULA] = 1;
		...
		private stopHarvest():void
		{
			...
			flags[FlagEnum.FOLLOWER_PRODUCTION_VAPULA] = 0;
		...
		private fapulaFapfapfapAppearance():void
		{
			...
			if (kGAMECLASS.farm.farmCorruption.hasTattoo("vapula"))
			{
				...
				else
				{
					...
					if (flags[FlagEnum.VAPULA_TATTOO_COLLARBONE] != 0) MainScreen.text(flags[FlagEnum.VAPULA_TATTOO_COLLARBONE] + "\n");
					if (flags[FlagEnum.VAPULA_TATTOO_SHOULDERS] != 0) MainScreen.text(flags[FlagEnum.VAPULA_TATTOO_SHOULDERS] + "\n");
					if (flags[FlagEnum.VAPULA_TATTOO_LOWERBACK] != 0) MainScreen.text(flags[FlagEnum.VAPULA_TATTOO_LOWERBACK] + "\n");
					if (flags[FlagEnum.VAPULA_TATTOO_BUTT] != 0) MainScreen.text(flags[FlagEnum.VAPULA_TATTOO_BUTT] + "\n");
				}
			}
...
//Talk (Z)
		private talkToVapulaForSomeReason():void
		{
			...
			if (flags[FlagEnum.VAPULA_HAREM_FUCK] == 0) {
			...
			if (player.statusAffects.has("RepeatSuccubi")) {
				if (flags[FlagEnum.VAPULA_THREESOMES] == 0) {
		...
		private toggleCeruleanVapulaAssist():void
		{
			...
			if (flags[FlagEnum.VAPULA_THREESOMES] == 0) {
				...
				flags[FlagEnum.VAPULA_THREESOMES] = 1;
			}
			//Turn off
			else {
				...
				flags[FlagEnum.VAPULA_THREESOMES] = 0;
			}
			//Return to talk menu
		...
		private toggleVapulaHaremFucks():void
		{
			MainScreen.clearText();
			//Stop Fucking Harem
			if (flags[FlagEnum.VAPULA_HAREM_FUCK] == 1) {
				...
				flags[FlagEnum.VAPULA_HAREM_FUCK] = 0;
			}
			else {
				//Fucking Harem OK
...
//Feed
		...
		{
			MainScreen.clearText();
			MainScreen.text("You tell Vapula you're going to reward her for being such a good slut; idly stroking [oneCock] into erection, you command her to get on her knees so she can receive her meal.  The demoness eagerly complies and grabs hold of [oneCock].  She then proceeds to lick it, coating the entirety of your junk with her warm, sweet saliva. With an expert precision, she massages all your most sensitive points, pumping pre-cum up your urethra with ruthless efficiency.  As drops of your juices dribble from the tip of your " + cockDescript(0) + " she voraciously slurps them with a gourmand's smile.");
			...
			if (player.lowerBody.balls > 0) MainScreen.text("Your " + ballsDescriptLight() + " churn");
			else MainScreen.text("Your " + cockDescript(0) + " twitches");
			...
			MainScreen.text(".  At last, the cum-flow spilling through your urethra starts to ebb and your " + cockDescript(0) + " stops throbbing; only then does she removes your junk from her mouth with a loud POP.  Your " + cockDescript(0) + " appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.");
			player.orgasm();
			dynStats("cor", .5);
			flags[FlagEnum.VAPULA_DAYS_SINCE_FED] = 0;
			flags[FlagEnum.VAPULA_TEASE_COUNT] = 0;
			doNext(camp.returnToCampUseOneHour);
		}
...
//Threesome
		private vapulaThreesomeMenu():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock() || (player.lowerBody.vaginaSpot.hasVagina() && player.hasKeyItem("Demonic Strap-On") >= 0)) {
				...
				if (izmaFollower() && flags[FlagEnum.IZMA_NO_COCK] == 0) izma = vapulaAndIzmaThreeSome;
			}
...
//Option: Please both.
		private vapulaCeraphThreesomePleaseBoth():void
		{
			...
			MainScreen.text(".");
			MainScreen.text("\n\nYou feel your ");
			if (player.lowerBody.balls > 0) MainScreen.text(ballsDescriptLight() + " twitch and your " + cockDescript(x) + " pulsate as they unload their");
			else MainScreen.text("your " + cockDescript(x) + " twitch and pulsate as it unloads its");
			MainScreen.text(" essence into your omnibus pet, bringing her to orgasm as she blubbers incoherently, \"<i>Oh yessssssssss, just like that... Yeah... my ass... cum... yessssssssss...</i>\"  Her voice goes so high pitched it trails off, letting you hear Vapula's soft moans below.  You quickly pull out your " + cockDescript(x) + ", still orgasming.  You hear Ceraph feebly protest at the sudden lack of cock filling her hole, \"<i>What are you doing? Put it back! Put it back already!</i>\"");
			...
			if (player.lowerBody.balls > 0) MainScreen.text(ballsDescriptLight() + " hitting her butt-hole with a faint thud");
			...
			MainScreen.text("  Your thoughts are quickly drowned in waves of pleasure, and soon your only thought is to ram more of your " + cockDescript(x) + " down your sluts' throats, pumping their heads alternatively around your shaft");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
			if (player.lowerBody.balls > 0) MainScreen.text(" as the girls keep fondling your " + ballsDescriptLight());
			...
			player.orgasm();
			dynStats("cor", 1);
			if (player.stats.lib > 10) dynStats("lib", -1);
			if (player.stats.lib > 50) dynStats("lib", -1);
			if (player.stats.lib > 70) dynStats("lib", -1);
			flags[FlagEnum.VAPULA_DAYS_SINCE_FED] = 0;
			flags[FlagEnum.VAPULA_TEASE_COUNT] = 0;
			doNext(camp.returnToCampUseOneHour);
		}
...
//Ceraph-Vapula Buttfuck train
		private vapulaAndCeraphButtfuckTrainYeehaw():void
		{
			...
			//[if balls]
			if (player.lowerBody.balls > 0) MainScreen.text("  As you relentlessly smash your groin against Ceraph's plump rump, your " + ballsDescriptLight() + " repeatedly hit her quivering vagina; they are soon utterly coated in fem-spunk.");
			MainScreen.text("  The entirety of your crotch ends up drenched with a various mix of sexual juices, and your " + cockDescript(x) + " makes squelching noises at each thrust in and out of your omnibus' little ass.");
...
//Vapula-Sophie threesome
		...
		private vapulaSophieThreeSome():void
		{
			...
			MainScreen.text(buttDescript() + ".");
			//[if third cock]
			if (player.totalCocks() > 2) MainScreen.text("  Using her extra flexibility, she bends over and start jerking your third pole, the sweet handjob adding to your pure cocktail of pleasure.");
...
//Vapula-Amily threesome
		private vapulaAndAmilyThreesome():void
		{
			...
			//[if not defurred]
			if (flags[FlagEnum.AMILY_NOT_FURRY] == 0) MainScreen.text(" and her fur starts shrinking, letting her rosy skin shine below");
...
//Vapula/Jojo Butt-fuck train
		private jojoButtFuckTrain():void
		{
			...
			if (player.cockArea(x) > 100) MainScreen.text("  You can actually feel his internal organs being bumped against your meat as your " + cockDescript(x) + " stretches his colon beyond its normal size and thrusts in and out like a charging mammoth.");
			...
			if (jojoScene.tentacleJojo()) {
				...
				//[if vagina]
				if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Last but not least, a soft cock-knock at the entrance of your " + vaginaDescript() + " signals an incoming intrusion.  If your mouth weren't so full of cock you would sigh in relief as your aching pussy is thoroughly penetrated.");
			}
			...
			while (temp > 0) {
				temp--;
				if (player.lowerBody.cockSpot.list[temp].cockType == CockType.TENTACLE && temp != x) {
					if (n1 == -1) n1 = temp;
...
//Feed (as female)
		private chixFeedVapulaBlehblehIVantToZuckYourSpooo():void
		{
			...
			//Feed/Tease
			let tease:Function = null;
			if (flags[FlagEnum.VAPULA_HAREM_FUCK] == 0) tease = teaseVapula;
...
//Tease
		private teaseVapula():void
		{
			...
			if (flags[FlagEnum.VAPULA_HAREM_FUCK] == 0 && (flags[FlagEnum.VAPULA_DAYS_SINCE_FED] == 0)) {
				...
				flags[FlagEnum.VAPULA_TEASE_COUNT]++;
			}
			//Second Tease
			//Requires: Vapula not fucking harem, teased the day before
			else if (flags[FlagEnum.VAPULA_HAREM_FUCK] == 0 && flags[FlagEnum.VAPULA_DAYS_SINCE_FED] == 1) {
				...
				//[Hooves:
				if (player.lowerBody == LOWER_BODY.HOOFED || player.isTaur()) MainScreen.text("\"<i>I would worship your feet, mistress.  I would clean your hooves with my tongue to a shine, tease out every scrap of dirt from every crevice.  And I would thank you for the privilege.</i>\"");
				//[Naga:
			...
			//Third Tease
				...
				//Naga:
				if (player.isNaga()) MainScreen.text("you slide your tail under and over her trunk, wrapping your coils around her and drawing her face into your stuffed crotch.");
				//Other:
				else if (player.isTaur() || player.isGoo() || player.isDrider()) MainScreen.text("you sit the monstrous half of your body down comfortably and draw her face into your stuffed cotch.");
...
//Feed (as female)
		private chicksFeedVapula():void
		{
			...
			if (flags[FlagEnum.VAPULA_TEASE_COUNT] > 1) {
...
//Forcefeed
		public vapulaForceFeeds():void
		{
			if (player.lowerBody.vaginaSpot.hasVagina() && player.hasKeyItem("Demonic Strap-On") >= 0 && !player.lowerBody.cockSpot.hasCock()) {
				...
				flags[FlagEnum.VAPULA_DAYS_SINCE_FED] = 0;
				flags[FlagEnum.VAPULA_TEASE_COUNT] = 0;
				flags[FlagEnum.VAPULA_EARNED_A_SPANK] = 1;
			}
			//[if no fuck harem is on and PC hasn't fed/fucked Vapula for 5 days that scene will trigger at night]
				...
				if (player.lowerBody.balls > 0) MainScreen.text("  Your " + ballsDescriptLight() + " churn");
				else MainScreen.text("  Your " + cockDescript(x) + " twitches");
				...
				if (player.cumQ() >= 1500) MainScreen.text(" until she looks 6 months pregnant.  At last, the cum-flow spilling through your urethra starts to ebb and your " + cockDescript(x) + " stops throbbing; only then does she removes your junk from her mouth with a loud POP.  Your tool appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.");
				player.orgasm();
				dynStats("cor", 1);
				flags[FlagEnum.VAPULA_DAYS_SINCE_FED] = 0;
				flags[FlagEnum.VAPULA_TEASE_COUNT] = 0;
...
//Requires bipedal-ness
		private spankVapulaLikeABoss():void
		{
			...
			flags[FlagEnum.VAPULA_EARNED_A_SPANK] = 0;
			doNext(camp.returnToCampUseOneHour);
		}
	}
}
