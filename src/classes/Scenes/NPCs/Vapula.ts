export default class Vapula extends NPCAwareContent implements TimeAwareInterface {
	public Vapula() {
	...
	public timeChange(): boolean {
		if (model.time.hours > 23) {
			if (vapulaSlave() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 0) {
				if (Flags.list[FlagEnum.VAPULA_HAREM_FUCK] == 0) Flags.list[FlagEnum.VAPULA_DAYS_SINCE_FED]++;
				else Flags.list[FlagEnum.VAPULA_DAYS_SINCE_FED] = 0;
			}
			if (Flags.list[FlagEnum.VAPULA_FOLLOWER] == .5 || Flags.list[FlagEnum.VAPULA_FOLLOWER] == 1.5) Flags.list[FlagEnum.VAPULA_FOLLOWER]++;
			Flags.list[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS]++;
		}
		if (vapulaSlave() && player.hasKeyItem("Demonic Strap-On") < 0 && player.gender == 2 && Flags.list[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 0) {
	...
	public timeChangeLarge(): boolean {
		if (Flags.list[FlagEnum.VAPULA_FOLLOWER] >= 2.5 && model.time.hours == 6 && Flags.list[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 0) {
			femaleVapulaRecruitmentPartII();
			return true;
		}
		if (model.time.hours == 2 && vapulaSlave() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 0 && Flags.list[FlagEnum.VAPULA_DAYS_SINCE_FED] >= 5 && (player.lowerBody.cockSpot.hasCock() || (player.hasKeyItem("Demonic Strap-On") >= 0 && player.lowerBody.vaginaSpot.hasVagina()))) {
			vapulaForceFeeds();
			return true;
		}
		return false;
	...
	public vapulaSlave(): boolean {
		return Flags.list[FlagEnum.VAPULA_FOLLOWER] == 1;
	...
	public femaleVapulaRecruitmentPartII(): void {
		...
		Flags.list[FlagEnum.VAPULA_HAREM_FUCK] = 1;
		//[Demonic dildo added]
	...
	public vapulaSlaveFlavorText(): void {
		...
		if (choice == 0) MainScreen.text("Vapula is currently resting on a very rough bed of leaves she's gathered herself.  She sleeps quietly, her hand absent-mindedly laid on her vagina and the other one resting on her breasts.  A thin trickle of her juice is leaking out of her crotch.");
		if (choice == 1) MainScreen.text("Your succubus slut, Vaplua, is furiously stroking herself on her bed of leaves, her purple body shining in sweat.  She moans raggedly, often looking up to you with lust-consumed eyes.");
		if (choice == 2) {
			if (Flags.list[FlagEnum.VAPULA_HAREM_FUCK] == 0) choice = 3;
			else MainScreen.text("Vapula is currently absent, probably getting screwed by some of the imps surrounding your camp in order to get her semen rations.  You're sure she would come back instantly if you ever needed to give her YOUR cum instead.");
		}
		if (choice == 3) MainScreen.text("Vapula is busy drinking a vial of milk that looks very much like the kind you'd usually find on imps.  Her purple cheeks seem to blush darker when she sees you; she awkwardly gulps the rest of the bottle, hiccups, and giggles softly, eyeing you with a coy smile and a lust-filled gaze.  The potion seems to have made her even more aroused than usual.");
		if (choice == 4) {
			MainScreen.text("Vapula is currently toying with ");
			if (player.hasKeyItem("Deluxe Dildo") >= 0) MainScreen.text("your deluxe dildo");
			else MainScreen.text("a dildo");
			MainScreen.text(", pumping as fast as she can.  In her feverish display of lust she doesn't even notice you.");
		}
		if (choice == 5) {
			if (Flags.list[FlagEnum.VAPULA_HAREM_FUCK] == 0 || !(sophieBimbo.bimboSophie())) choice = 6;
			//if harem fucking is on)
			else MainScreen.text("Vapula is fingering herself while eating out Sophie, your harpy slut.  The dumb avian matron keeps moaning like she's in heat while the succubus furiously assaults her lush fuck-hole; both of their pair of wings flap in rhythm as they feel their pleasure rising in waves.");
		}
		if (choice == 6) {
			if (Flags.list[FlagEnum.VAPULA_HAREM_FUCK] == 0 || Flags.list[FlagEnum.IZMA_NO_COCK] > 0 || !izmaFollower()) choice = 7;
			//if Izma has cock and harem fucking is on)
			else MainScreen.text("Your succubus is currently very busy with Izma, your tigershark lover.  Izma is standing and hiding Vapula from your sight and you don't notice what Vapula is doing to her until you see her frantic bobbing movements, and hear Izma's hisses and growls of pleasure.  Obviously your succubus must get the cum she needs from somewhere.");
		}
		if (choice == 7) {
			//if C. Jojo and harem fucking is on)
			if (Flags.list[FlagEnum.VAPULA_HAREM_FUCK] == 0 || !jojoScene.campCorruptJojo()) choice = 8;
			else {
				MainScreen.text("You routinely hear screams of pleasure outside the camp, as well as some frantic male squeaks coming from Jojo, your ");
				if (jojoScene.tentacleJojo()) MainScreen.text("tentacled ");
				MainScreen.text("mouse pet.  Vapula seems to be having a lot of fun with your fuck-toy!");
			}
		}
		if (choice == 8) {
			if (Flags.list[FlagEnum.VAPULA_HAREM_FUCK] == 0 || !(amilyScene.amilyFollower() && amilyScene.amilyCorrupt())) choice = 9;
			//if C. Amily and harem fucking on)
			else MainScreen.text("Vapula is busy caressing your corrupted mousette, Amily while softly kissing her breasts.  Her fingers are buried in your fuck-toy's pussy, and Amily is moaning whorishly and doesn't even bother to stop when she looks at you, her helpless eyes betraying her lack of control over her own lust.");
		}
		if (choice == 9) {
			if (!player.statusAffects.has("CampRathazul")) choice = 10;
			else MainScreen.text("Vapula is having a very active conversation with Rathazul, the alchemist.  They seem to be discussing the chemical properties of demonic mixtures and the various taints that could occur from their ingestion.  The succubus is holding a bottle of a purplish white fluid that appears to be her own milk, probably waiting for an analysis.");
		}
		if (choice == 10) MainScreen.text("Vapula is resting on her stomach, a buttplug shoved into her ass.  She doesn't seem to notice you watching her butt jiggle under the effect of her unconscious anal contractions.");
	}
	...
	public mouseWaifuFreakout(amily: boolean = false, jojo: boolean = false): void {
		...
		if (amily) {
			...
			if (jojo) MainScreen.text("and ");
		}
		if (jojo) MainScreen.text("Jojo ");
		MainScreen.text("walk");
		if (!(amily && jojo)) MainScreen.text("s");
		MainScreen.text(" up to you, worried, as Vapula struts around the camp.");
		...
		//[if libido >=50]
		if (player.stats.lib >= 50) MainScreen.text("\"<i>Well, as long as she swallows, I don't care,</i>\" you quip.");
		MainScreen.text("\n\n\"<i>That's it!</i>\"  The ");
		if (jojo && amily) MainScreen.text("mice are");
		else MainScreen.text("mouse is");
		MainScreen.text(" raving.  \"<i>You've clearly given in to her demonic lust.</i>\"");
		if (jojo) {
			...
			Flags.list[FlagEnum.JOJO_MOVE_IN_DISABLED] = 1;
	...
	public callSlaveVapula(output: boolean = true): void {
		...
		//Option: Threesome
		//Option: Leave
		let mFeed: Function = null;
		let fFeed: Function = null;
		if (player.lowerBody.cockSpot.hasCock()) {
			if (player.stats.lust >= 33) mFeed = feedVapulaACupOfJizz;
			else if (output) MainScreen.text("\n\nYou aren't suitably aroused to feed Vapula right now.");
		}
		if (player.lowerBody.vaginaSpot.hasVagina() && player.hasKeyItem("Demonic Strap-On") >= 0) {
			if (player.stats.lust >= 33) fFeed = chixFeedVapulaBlehblehIVantToZuckYourSpooo;
			else if (output && !player.lowerBody.cockSpot.hasCock()) MainScreen.text("\n\nYou aren't suitably aroused to feed Vapula right now.");
		}
		...
		if (Flags.list[FlagEnum.VAPULA_EARNED_A_SPANK] > 0 && !player.lowerBody.isTaur() && !player.lowerBody.isDrider()) spank = spankVapulaLikeABoss;
		...
		if (Flags.list[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 0 && Flags.list[FlagEnum.FARM_CORRUPTION_STARTED] == 1) MainScreen.addButton(6, "Farm Work", sendToFarm);
		if (Flags.list[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 1) MainScreen.addButton(6, "Go Camp", backToCamp);

		if (Flags.list[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 1 && Flags.list[FlagEnum.FOLLOWER_PRODUCTION_VAPULA] == 0) MainScreen.addButton(7, "Harvest Milk", harvestMilk);
		if (Flags.list[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 1 && Flags.list[FlagEnum.FOLLOWER_PRODUCTION_VAPULA] == 1) MainScreen.addButton(7, "Stop Harvest", stopHarvest);

		if (Flags.list[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 1) MainScreen.addButton(9, "Back", kGAMECLASS.farm.farmCorruption.rootScene);
	}

	private sendToFarm(): void {
		MainScreen.clearText();

		MainScreen.text("You tell your succubus concubine that she is to head towards the lake, find a farm, present herself to the lady who works there and do as she says. Vapula laughs long and hard at this.");
		...
		Flags.list[FlagEnum.FOLLOWER_AT_FARM_VAPULA] = 1;
	...
	private backToCamp(): void {
		...
		MainScreen.text("You tell her to head back to camp; there are things you need to do to her you can’t do whilst she’s here. Repeatedly. Vapula fist pumps the sky.");
		...
		Flags.list[FlagEnum.FOLLOWER_AT_FARM_VAPULA] = 0;
	...
	private harvestMilk(): void {
		...
		MainScreen.text("You tell Vapula that you want her hooked up to a milking machine whenever possible; you need her fluids.");
		...
		if (Flags.list[FlagEnum.FARM_UPGRADES_REFINERY] == 0) MainScreen.text("“<i>It’ll sure as hell beat whatever disgusting thing the bitch would have me doing instead. Why don’t you make her build a concentration machine? Lethice has loads; makes one dose of cream go so much further.</i>”");
		else MainScreen.text("“<i>As you insist, [master]. It’ll sure as hell beat working.</i>”");

		Flags.list[FlagEnum.FOLLOWER_PRODUCTION_VAPULA] = 1;
	...
	private stopHarvest(): void {
		...
		MainScreen.text("You tell Vapula to stop producing succubus milk; you’re practically drowning in the stuff.");
		...
		Flags.list[FlagEnum.FOLLOWER_PRODUCTION_VAPULA] = 0;
	...
	private fapulaFapfapfapAppearance(): void {
		...
		if (kGAMECLASS.farm.farmCorruption.hasTattoo("vapula")) {
			...
			if (kGAMECLASS.farm.farmCorruption.vapulaFullTribalTats()) {
				...
				if (kGAMECLASS.farm.farmCorruption.numTattoos("vapula") > 1) MainScreen.text("She has the following tattoos emblazoned across her body:\n");
				else MainScreen.text("She has ");

				if (Flags.list[FlagEnum.VAPULA_TATTOO_COLLARBONE] != 0) MainScreen.text(Flags.list[FlagEnum.VAPULA_TATTOO_COLLARBONE] + "\n");
				if (Flags.list[FlagEnum.VAPULA_TATTOO_SHOULDERS] != 0) MainScreen.text(Flags.list[FlagEnum.VAPULA_TATTOO_SHOULDERS] + "\n");
				if (Flags.list[FlagEnum.VAPULA_TATTOO_LOWERBACK] != 0) MainScreen.text(Flags.list[FlagEnum.VAPULA_TATTOO_LOWERBACK] + "\n");
				if (Flags.list[FlagEnum.VAPULA_TATTOO_BUTT] != 0) MainScreen.text(Flags.list[FlagEnum.VAPULA_TATTOO_BUTT] + "\n");
			}
		}
	...
	private talkToVapulaForSomeReason(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>You want to talk?  Well it's quite rare to see " + player.mf("studs", "girls") + " looking for conversation with the likes of me, but go ahead.  What do you want to talk about?</i>\"");
		//Option: Stop Fucking Harem OR Fucking Harem OK (if toggled on off)
		//Option: Cerulean Threesome On/Off
		let threesomeT: string = "";
		let threesomeB: Function = null;
		let haremT: string = "";
		let haremB: Function = toggleVapulaHaremFucks;
		if (Flags.list[FlagEnum.VAPULA_HAREM_FUCK] == 0) {
		...
		if (player.statusAffects.has("RepeatSuccubi")) {
			if (Flags.list[FlagEnum.VAPULA_THREESOMES] == 0) {
	...
	private feedVapulaACupOfJizz(): void {
		MainScreen.clearText();
		MainScreen.text("You tell Vapula you're going to reward her for being such a good slut; idly stroking [oneCock] into erection, you command her to get on her knees so she can receive her meal.  The demoness eagerly complies and grabs hold of [oneCock].  She then proceeds to lick it, coating the entirety of your junk with her warm, sweet saliva. With an expert precision, she massages all your most sensitive points, pumping pre-cum up your urethra with ruthless efficiency.  As drops of your juices dribble from the tip of your " + CockDescriptor.describeCock(player, 0) + " she voraciously slurps them with a gourmand's smile.");
		MainScreen.text("\n\nHer ministrations feel good, but she keeps tickling you with her soft hands and pulpy lips, and her flexible tongue gives you the most infuriating tingles of pleasure.  With a groan of impatience, you grab her head and stuff your whole " + CockDescriptor.describeCock(player, 0) + " into her mouth, literally stuffing her full of your junk.  She nearly chokes at first but her old reflexes acquired from a whole life devoted to lewdness and licentiousness start kicking in; Vapula gluttonously deepthroats your meat, softly moaning as she fingers herself with one hand and caresses your ");
		if (player.lowerBody.balls > 0) MainScreen.text(BallsDescriptor.describeBalls(true, true, player));
		else MainScreen.text("groin");
		MainScreen.text(" with the other.  She sucks like a cock-hungry whore, treating your shaft like a straw as she slurps down your juices; her demonic throat feels incredibly hot and her saliva seems to make your " + CockDescriptor.describeCock(player, 0) + " grow even harder; her lips squeeze your meat in the most exquisite manner, sending you into short spasms of pleasure every time.");
		MainScreen.text("\n\nShe works on it for what seems like hours.  You look down at your slut; she keeps staring at you with avid yet playful eyes, never breaking eye contact as she relentlessly throatfucks herself; you feel her lips curve a little as the shape of a ravenous smile appears on her face, and her eyes glitter with an insatiable need.  The naughty bitch! She knows she's going to get what she wants, whether you like it or not.  You wish you were able to control yourself, but the covetous succubus never lets go of your " + CockDescriptor.describeCock(player, 0) + ", clinging to it like an animal.");
		MainScreen.text("\n\nYour stamina is rapidly overwhelmed and you decide to reward her effort; you savagely press Vapula's head against your groin, effectively burying all " + num2Text(Math.round(player.lowerBody.cockSpot.get(0).cockLength)) + " inches of your " + CockDescriptor.describeCock(player, 0) + " in her waiting throat; her tongue keeps teasing your rod for a few seconds, and then you cum, brutally.  ");
		if (player.lowerBody.balls > 0) MainScreen.text("Your " + BallsDescriptor.describeBalls(true, true, player) + " churn");
		else MainScreen.text("Your " + CockDescriptor.describeCock(player, 0) + " twitches");
		MainScreen.text(" as you release a sticky spooge fountain in her stomach.  Her lips keep squeezing your junk in order to milk everything you've got, and in your shuddering orgasm you're too eager to comply.  With a surrendering sigh, you abandon yourself to your climax and keep cumming, squirting your baby-batter to the last drop as the succubus keeps gulping it.  Her eyes are closed in an expression of complete satisfaction; she enjoys her meal to its full extent, filling her belly with your spooge");
		if (player.cumQ() >= 1500) MainScreen.text(" until she looks six months pregnant");
		MainScreen.text(".  At last, the cum-flow spilling through your urethra starts to ebb and your " + CockDescriptor.describeCock(player, 0) + " stops throbbing; only then does she removes your junk from her mouth with a loud POP.  Your " + CockDescriptor.describeCock(player, 0) + " appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.");
		player.orgasm();
		player.stats.cor += .5;
		Flags.list[FlagEnum.VAPULA_DAYS_SINCE_FED] = 0;
		Flags.list[FlagEnum.VAPULA_TEASE_COUNT] = 0;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private vapulaThreesomeMenu(): void {
		MainScreen.clearText();
		MainScreen.text("Who do you invite?");
		//Option: Amily
		let amily: Function = null;
		//Option: Ceraph
		let ceraph: Function = null;
		//Option: Sophie
		let sophie: Function = null;
		//Option: Jojo
		let jojo: Function = null;
		//Option: Izma
		let izma: Function = null;
		if (player.lowerBody.cockSpot.hasCock() || (player.lowerBody.vaginaSpot.hasVagina() && player.hasKeyItem("Demonic Strap-On") >= 0)) {
			if (jojoScene.campCorruptJojo()) jojo = vapulaJojoThreesomes;
			if (amilyScene.amilyFollower() && amilyScene.amilyCorrupt() && player.lowerBody.cockSpot.hasCock()) amily = vapulaAndAmilyThreesome;
			if (ceraphFollowerScene.ceraphIsFollower() && player.lowerBody.cockSpot.hasCock()) ceraph = vapulaCeraphThreesome;
			if (sophieBimbo.bimboSophie()) sophie = vapulaSophieThreesomeSelect;
			if (izmaFollower() && Flags.list[FlagEnum.IZMA_NO_COCK] == 0) izma = vapulaAndIzmaThreeSome;
		}
	...
	private vapulaCeraphThreesomePleaseBoth(): void {
		...
		MainScreen.text("You order your girls to turn around; you want to give them a good dicking.  They comply while giggling like a pair of whores, eager to get some cock up their holes.");
		MainScreen.text("\n\nYou push Vapula onto the floor, making her fall on her back, legs spread.  You then grab hold of Ceraph's shoulders and put her on all fours, her warm snatch facing Vapula's.  Already moist, the succubus and the omnibus naturally start caressing each other, kissing and groping each other's breasts.  You watch them giving in to their own depravity before joining the show; grabbing Ceraph's plump butt, you press the tip of your " + CockDescriptor.describeCock(player, x) + " against her backdoor.  The unexpected intrusion makes the omnibus cry, her gasps of joy only enticing you to push further, even at the cost of stretching her anal ring.  A few seconds later you're all the way in, enjoying the boiling and tight recesses of her pucker.  You roughly seize Ceraph's fuck-pillows and proceed to relentlessly assault her ass, making her yell under the conjugated efforts of your throbbing junk and the hot succubus fingering her from below.  Completely dominated by the pleasure you're inflicting on her, your slut starts muttering dirty words, often interrupted by sudden screams, \"<i>Fuck... fuck... fuck my ass... " + player.mf("Master", "Mistress") + "...feels so- AAAAAH! Oh FUCK YEAH! Harder, " + player.mf("Master", "Mistress") + "! Fuck your bitch!</i>\"");

		MainScreen.text("\n\nAnnoyed by the noise she's making, you slap her firm rump repeatedly, your hands imprinting marks on her red ass-cheeks.  Your buttslut doesn't seem to care, completely blissful from being used by her rightful " + player.mf("Master", "Mistress") + ".  She groans, grinds harder and squirms in Vapula's embrace in her efforts to impale herself further on your " + CockDescriptor.describeCock(player, x) + ".  The sight of your sluts' love-tunnels mixing their fluids together as they squirt as well as the insane motion of Ceraph's wobbling ass sending the sweetest sensations to your rod prove too much for you; despite your best efforts to hold back, you blast Ceraph's ass with your baby-batter, the globs effectively packing her anal entrance");
		if (player.cumQ() >= 1500) MainScreen.text(" and swelling her belly until she looks ready to give birth to a full horde of imps");
		MainScreen.text(".");
		MainScreen.text("\n\nYou feel your ");
		if (player.lowerBody.balls > 0) MainScreen.text(BallsDescriptor.describeBalls(true, true, player) + " twitch and your " + CockDescriptor.describeCock(player, x) + " pulsate as they unload their");
		else MainScreen.text("your " + CockDescriptor.describeCock(player, x) + " twitch and pulsate as it unloads its");
		MainScreen.text(" essence into your omnibus pet, bringing her to orgasm as she blubbers incoherently, \"<i>Oh yessssssssss, just like that... Yeah... my ass... cum... yessssssssss...</i>\"  Her voice goes so high pitched it trails off, letting you hear Vapula's soft moans below.  You quickly pull out your " + CockDescriptor.describeCock(player, x) + ", still orgasming.  You hear Ceraph feebly protest at the sudden lack of cock filling her hole, \"<i>What are you doing? Put it back! Put it back already!</i>\"");
		MainScreen.text("\n\nWith another slap you tell her to shut up and learn to share; her partner hasn't received any cock yet, and you intend to remedy it.  Grabbing Vapula's thighs this time, you lunge toward her wet fuck-hole and bury your " + CockDescriptor.describeCock(player, x) + " to the hilt, ");
		if (player.lowerBody.balls > 0) MainScreen.text(BallsDescriptor.describeBalls(true, true, player) + " hitting her butt-hole with a faint thud");
		else MainScreen.text("thudding up against her cushy butt-cheeks");
		MainScreen.text(".  The purple sex-goddess squeals, your abrupt dick-digging utterly ravaging her cunt; her unholy warmth and impossibly tight confines are almost enough to make you cum instantly as her vaginal walls wrap in the most sensitive way around your rod, but you manage to hold off the flow of semen churning in your groin.");
		//[if 2nd cock]
		...
		//[if multicocks]
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("  She expertly jerks each of your rods in turn, gently polishing them as they grow back into an erect state.");
		MainScreen.text("  She's obviously used to this kind of practice; even though [eachCock] felt numb after cumming so hard, you feel your lust build up again as your genitals throb once more with need.  You grab Ceraph's head and impale her on your crotch, forcefully throat-fucking her.");
		...
		MainScreen.text("  Your thoughts are quickly drowned in waves of pleasure, and soon your only thought is to ram more of your " + CockDescriptor.describeCock(player, x) + " down your sluts' throats, pumping their heads alternatively around your shaft");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		if (player.lowerBody.balls > 0) MainScreen.text(" as the girls keep fondling your " + BallsDescriptor.describeBalls(true, true, player));
		MainScreen.text(", bringing you extra tingles of pleasure to your body.");
		...
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(", their lips doing their best to milk ");
		if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("it");
		else MainScreen.text("them");
		MainScreen.text(" of all ");
		if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("its ");
		else MainScreen.text("their ");
		MainScreen.text("cream.  Once your junk feels completely empty, [oneCock] is released with a loud POP.");
	...
	private vapulaAndCeraphButtfuckTrainYeehaw(): void {
		...
		let x: number = player.cockThatFits(100);
		if (x < 0) x = player.smallestCockIndex();
		MainScreen.text("Grinning lewdly, you dispose your omnibus and your succubus in a front line such that Ceraph's cock points toward Vapula's anus.  Ceraph pants expectantly.  \"<i>What do you have in mind this time, Master?</i>\" Without a word, you place yourself behind Ceraph's back and start stroking [eachCock] against her butt-cheeks, your tip dribbling pre-cum teasingly; you shove a few fingers into Ceraph's warm snatch, stimulating her until she lets out a muffled moan as her own cock grows to full erectness.  At last, you ready yourself, your cock knocking on Ceraph's backside.  You gently explain to your girls that you're going to initiate a butt-fuck train.  Vapula giggles softly, \"<i>Ooooh, I love this!  But... wait, I'm in the front of the -</i>\"");
		MainScreen.text("\n\nWithout any warning, you harshly push your " + CockDescriptor.describeCock(player, x) + " into Ceraph's tight anal recesses, making her clench and tense in arousal.  She immediately responds by grasping Vapula's squeezable thighs and digs her own, broad prick inside the succubus' colon.  The totally unexpected rectal assault makes both hell-girls scream in surprise and pleasure mixed, their anal walls saturated with intense dick-friction.  You stand here comfortably, easily lodging your " + CockDescriptor.describeCock(player, x) + " inside Ceraph's backdoor while letting her have a better grip on Vapula's buttocks.  The omnibus' pucker is insanely hot!  Your rod of flesh feels like it's going to melt under her demonic warmth, and her astonishingly narrow colon squeezes it in the most harsh and delightful way.  You decide to speed up the pace and start sliding back and forth, ");
		if (silly()) MainScreen.text("screaming at the top of your lungs: \"<i>BUTT-FUCK! BUTT-FUCK! BUTT-FUCK!</i>\", soon joined by your horny anal partners");
		else MainScreen.text("groaning and panting as you ravage Ceraph's pucker with your junk");
		MainScreen.text(".\n\n");
		MainScreen.text("Between two mad thrusts, you order Ceraph to imitate you in every movement, treating Vapula the same way you treat her.  You start slapping her jiggly ass, sending ripples across her red shivering flesh.  Entranced by your violent ministrations, the omnibus complies to your orders, spanking Vapula as hard as she can.  Soon moans are heard all across the butt-fuck train; the more you slap and thrust, the harder Ceraph hits the purple sex-goddess, as if she wanted to prove she was a better servant, worthier of your " + CockDescriptor.describeCock(player, x) + ".  You reach around and knead Ceraph's tits, playing with her nipples as you devastate her backdoor with ferocious dick assaults, and she hesitantly grasps Vapula's bouncy tits, unable to match your thrusts in intensity.  She jerks her head backwards and moans, almost forgetting her own prick impaling the poor succubus in front of her.  Seeing that the poor bitch has lost all control over her own body, you stretch your arms further and reach Vapula's breasts.  The succubus is literally drooling in ecstasy; forced to feel the mighty thrusts of a strong omnibus as well as yours, she is bent over submissively, her tongue hanging and her eyes rolling under the combined strength of your anal-dickings.  You pound harder, squeezing her tits at each cock-nudge and muffling her pleasure-filled shrieks when you can.");

		MainScreen.text("\n\nAs for Ceraph, the poor thing isn't even able to think anymore.  Her rhythmical moans have been gradually replaced with some incoherent dirty babble, \"<i>Oh fuckfuckfuck... good.. fuck...  YES! Fuck... yes... fuckfuckfu-aaaAAAAAH!</i>\"  You keep impaling the omnibus' butthole with your " + CockDescriptor.describeCock(player, x) + ", pounding harder in hope the anal tearing will shut her up, but she only seems to get even more pleasure from such a violation of her insides.  Her mind isn't able to reason properly, torn between her ferocious butt-penetration and the pleasure her own cock is getting as it moves of its own volition in and out of Vapula's tight little pucker.");
		...
		//[if balls]
		if (player.lowerBody.balls > 0) MainScreen.text("  As you relentlessly smash your groin against Ceraph's plump rump, your " + BallsDescriptor.describeBalls(true, true, player) + " repeatedly hit her quivering vagina; they are soon utterly coated in fem-spunk.");
		MainScreen.text("  The entirety of your crotch ends up drenched with a various mix of sexual juices, and your " + CockDescriptor.describeCock(player, x) + " makes squelching noises at each thrust in and out of your omnibus' little ass.");

		MainScreen.text("\n\nEventually, you feel your stamina decreasing as a pressure builds in your loins and you brace yourself for the imminent cumshot. With a savage, desperate last thrust, you thoroughly impale the purple demoness on your junk and paint her interior walls with an explosion of spooge.  A mighty spray of cum spurt out of your " + CockDescriptor.describeCock(player, x) + ", completely stuffing Ceraph with your seed.");
		if (player.cumQ() >= 1500) MainScreen.text("  She automatically clenches her anus in order to keep as much of your baby-batter inside as possible, but to no avail; torrents of white goo start spurting out of her rectum, splattering her legs and your groin.");
		MainScreen.text("  This sudden extra pressure on her colon sends Ceraph over the edge, and she cums too; bleating in ecstasy, she grabs Vapula's hips and shoves her cock all the way inside her backdoor as she releases a milky flood of her own.  For several seconds you all stand there, fluids steadily flowing in and out of Ceraph as you both climax.  Vapula, literally mindbroken by the intense butt-fuck, utterly abandons herself to her pleasure and her pussy explodes in an abundant spray of fem-juices.  Ceraph's pussy quivers and convulses, her mad arousal making her squirt juices in thick torrents from her feminine counterpart.  The whole wagon of peckers and butts stands still, frozen in pure ecstasy as fluids keep dripping of every hole and dick.  A dirty puddle, formed by all the sexual juices spurted by your depraved embrace, grows and thickens as your " + CockDescriptor.describeCock(player, x) + " as well as Ceraph's keep pulsating in orgasm. At last, when every cock has been milked of all its seed, you relax.");
		MainScreen.text("\n\nYou dump your buttsluts, letting them splash in the cum-puddle, exhausted. Triumphantly exhibiting your " + CockDescriptor.describeCock(player, x) + ", you tell them to finish their job.  Vapula and Ceraph growl reluctantly as they grovel in the spunk-tainted mud and crawl toward your junk before licking it with delectation.  You let them enjoy their meal, groaning in pleasure as they squeeze their pittance out of your " + CockDescriptor.describeCock(player, x) + " and suck out the last globs of semen out of your urethra.  When you're all absolutely positive that no residual cum has been wasted, you smirk in satisfaction; you give your sluts a hard cock-slap for good measure and send them away.");
		//Lust set to 0
		player.orgasm();
		player.stats.sens += -2;
		player.stats.cor += 1;
	...
	private vapulaSophieThreesomeSelect(): void {
		if (player.lowerBody.cockSpot.hasCock()) vapulaSophieThreeSome();
	...
	private vapulaSophieThreeSome(): void {
		...
		MainScreen.text("\n\nWithout ceremony you grab the avian slut's fuckpillows and eagerly sandwich your meat between them, pressing the tip of your " + CockDescriptor.describeCock(player, x) + " against her tight anus.  As soon as the first inches start sliding in, the bimbo slut flaps her wings frenetically as she squawks in pleasure, her anal muscles contracting on their own as an invitation to slide in further.  Before you even think of it ");
		if (player.cockArea(x) > 75) MainScreen.text("you've filled the entirety of her colon");
		else MainScreen.text("your cock is already buried to the hilt, its mammoth girth bulging between her broad hips");
		MainScreen.text("; the bulge even grinds a bit against Vapula's own butt and the succubus reaches toward you, entranced with lust.  She wraps her arms around you, letting you feel her tender boobs being squeezed against your [chest] as she gives you a mad kiss filled with passion.  Your lips remain locked for a while as you explore each other's mouths with your tongues; you almost forget the burning hot and roughly tight hole.  It entraps your " + CockDescriptor.describeCock(player, x) + " as it continues massaging with the harpy's vice-like anal muscles.  The bimbo keeps wriggling arond your rod in order to get more of it inside, pleasantly smashing her fat rear against your groin.");
		MainScreen.text("\n\nStill not breaking the kiss, you groan silently and pull out your " + CockDescriptor.describeCock(player, x) + ", all smeared in pre-cum.  Sophie whimpers and wails softly until you thrust all the way back in, literally jamming her with your junk.  Her pupils widen, her mouth opens and her gigantic tits keep swinging in the dirt as her whole body trembles and shudders from the brutal anal violation.  Her cry of pleasure is so high-pitched you barely hear her subsequent moan; satisfied, you start thrusting at a more convenient rhythm, enjoying the lush depths of the avian slut as the succubus keeps making out with you.  You keep butt-fucking Sophie, unable to focus on the ministrations of the expert succubus or the extremely warm recesses of the harpy's interior.  You regularly slap her plush, fat buttocks, sending ripples across her whole fleshy body. Needless to say, the bimbo's pussy is completely drenched in fem-spunk, her wide pussy drooling trickles after trickles of her juices, betraying her utter arousal.");
		//[if 2nd cock]
		...
		if (player.lowerBody.balls > 0) MainScreen.text("balls and massages your ");
		MainScreen.text(ButtDescriptor.describeButt(player) + ".");
		//[if third cock]
		if (player.lowerBody.cockSpot.count() > 2) MainScreen.text("  Using her extra flexibility, she bends over and start jerking your third pole, the sweet handjob adding to your pure cocktail of pleasure.");
		MainScreen.text("  You can feel her hard nipples poke at your skin as they gently scrape across your torso.  Vapula hardly ever interrupts her embrace, only pulling back from your lips to take care of other intimate parts of your body.  You keep riding the bimbo senseless, making her squirm and squawk in ecstasy as your own cries of pleasure are muffled by Vapula's voracious lips.");
		...
		if (player.lowerBody.cockSpot.count() < 3) MainScreen.text("hastily pull out and ");
		MainScreen.text("wave ");
		if (player.lowerBody.cockSpot.count() >= 3) MainScreen.text("one of ");
		MainScreen.text("your still pulsing dick");
		if (player.lowerBody.cockSpot.count() >= 3) MainScreen.text("s");
	...
	private vapulaSophieThreesomeCunts(): void {
		...
		MainScreen.text("\n\nWithout ceremony you grab the avian slut's fuckpillows and eagerly sandwich your dildo between them, pressing the tip of it against her tight anus.  As soon as the first inches start sliding in, the bimbo slut flaps her wings frenetically as she squawks in pleasure, her anal muscles contracting on their own as an invitation to slide in further.  Before you even think of it your strap-on is already buried to its leather hilt, its mammoth erection bulging between her broad hips; the bulge even grinds a bit against Vapula's own butt and the succubus reaches toward you, entranced with lust. She wraps her arms around you, her tender boobs against your own " + BreastDescriptor.describeAllBreasts(player) + " as she gives you a mad kiss filled with passion. Your lips remain locked for a while as you explore each other's mouths with your tongue; you almost forget the tight hole wrapped around your dildo.  You are brought back to it by the bimbo wriggling around the rod rammed in her rear in an attempt to squeeze more of it into her, twisting your end around in your sopping " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " as she pleasantly mashes her fat rear against your groin.");
		...
		MainScreen.text("\n\nVapula shifts her attention to your breasts, her long tongue running lasciviously over your " + BreastDescriptor.describeNipple(0) + "s, tightly circling and gently teasing their tips until they are as sensitive and erect as any clitoris.  Her soft lips edge you further and further into your fuck daze, riding the bimbo senseless, making her squirm and squawk in ecstasy as your own cries of pleasure are muffled by Vapula's voracious lips.");
	...
	private vapulaAndAmilyThreesome(): void {
		...
		if (player.lowerBody.balls > 0) MainScreen.text(": her drooling pussy needs to be prepared because you're going to take her next");
		MainScreen.text(".  Vapula smirks in excitement and eagerly shoves her tumescent pussy in front of the mousette's waiting lips; Amily keeps wagging her butt impatiently, looking forward to being used as a disposable anal-toy.  You wink at Vapula; she's going to like it.");
		MainScreen.text("\n\nGrabbing hold of your mouse's plush buttcheeks, you stretch her anal opening and in a brutal, powerful thrust, you slide all of your " + CockDescriptor.describeCock(player, x) + " down her interior.  Amily immediately squeaks, her mouth drooling and her whole body twitching as she adapts to the erect prick inside her.  Her cries of pleasure are suddenly muffled as Vapula, too turned on to care about your mousette, brutally grabs her head and squeezes her face between her legs.  In the meantime, you start pumping at her ass, effortlessly moving your hips backwards and forwards as your " + CockDescriptor.describeCock(player, x) + " ravages the mousegirl's insides.  The poor creature, already driven over the edge by your relentless anal pounding, is now forced to lick the succubus' luscious pussy, her tongue being forced into probing her lush depths at each thrust received from behind.  She keeps moving back and forth between Vapula and you, roughly mishandled as both ends of her body are being abused.  However, this does not seem to decrease in the least the pleasure she's getting. She joyfully slams back her ass against your groin at every thrust, her insides always hungry for more dickflesh.  Your hips are quasi-immobile as she impales herself on her own in an insatiable cock-hunger.");
	...
	public vapulaAssistsCeruleanSuccubus(): void {
		...
		MainScreen.text("\n\nYou tell the blue woman not to worry about that and gently pat Vapula's head; the purple demoness coughs as she interrupts her divine blowjob and giggles when she notices one of her own kind.  She slowly gets up and is soon joined by your demon guest, awaiting your next orders.  The blue succubus and the purple succubus stand before you, their eyes filled with voracious lust and impatient need.  Two girls are eager for your cum, and there's only one " + player.mf("man", "herm") + " to satisfy them both... You don't know if you'll be up to the task, but it sure can't hurt to try.  Smiling to both, you remove your " + player.inventory.armor.displayName + " and lazily lie down, your erect " + CockDescriptor.describeCock(player, x) + " standing provocatively.  \"<i>Well, come on ladies.  Please me.  Can't you see I need some satisfaction?</i>\"");
		MainScreen.text("\n\nAt this invitation, both luscious creatures practically jump you.  In no time you're covered with blue and purple flesh, arms and lips caressing you everywhere but focusing on your most sensitive spots.  You shiver several times as the hell-girls give you tingles of pleasure that tease you almost unbearably.  You feel hands and tongues being pressed against your " + CockDescriptor.describeCock(player, x) + " and smearing it with saliva and pre-cum; you are kissed several times, your mouth is being padded with a flexible demonic tongue as your palate is filled with a sweet unholy taste; the scent of horny succubi makes you giddy and all the more aroused.  You feel nipples being stuffed in your mouth and you instinctively suck, gulping the succubi's essences like a newborn.");
		MainScreen.text("\n\nDrops of some fluid start falling on your face; looking up, you see nothing but a drooling, gushing pink pussy being lowered down.  Idly wondering whose it might be, you start probing your tongue into that moist vagina, eagerly sucking and licking as the whore atop you moans and bucks wildly; from her hoarse, ragged voice you can recognize Vapula.  Gods, she's such a horny succubus slut!  You keep eating her out until she screeches madly in pleasure, probably waking up every living creature around your camp.  At the same time, you feel your " + CockDescriptor.describeCock(player, x) + " is being handled and inserted into a nice warm snatch.  The brutal temperature change and amazing tightness drives you near the edge, and you actually feel your spooge flowing up and down your urethra as you contract your hips in a desperate effort to control yourself.  Your groin harshly hits the blue succubus', and your " + CockDescriptor.describeCock(player, x) + " digs even deeper in her innermost depths.  She sighs in contentment and slowly whispers, \"<i>Guests first.</i>\"");
		MainScreen.text("\n\nShe starts riding you ferociously, clamping you down and gripping your hips with claw-like hands.  Her vaginal muscles implacably contract around your " + CockDescriptor.describeCock(player, x) + ", acting as an boiling-hot and inescapable cock-ring.  Still trying your best to explore Vapula's love-tunnel, you reach around to get some of her tits, only to find the blue succubus' hands pinching your slut's nipples, making her shriek in torturesome pleasure.  You both keep groping her voluptuous body, making her shudder under your abusive massages.  Your head is now entirely soaked with Vapula's juices - that whore seems to orgasm ridiculously often.  Every time you consider pulling back your tongue and gasping for some fresh air, her twat squirts another crazy jet of fem-spunk down your face, making your vision blurry and leaving you with no choice but to lick her interior clean.  However, your efforts only stimulate her all the more, leading you to wonder how she can generate such an absurd amount of vaginal juices.");
		//[if multicock]
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("  She suddenly seems to have an idea: leaning down to reach your second cock, she starts licking it, accompanying the savage thrusting of the blue demoness as she slurps your extra junk, giving you even more pleasure.");
		else MainScreen.text("  The blue demoness leans down and starts licking Vapula's moist pussy, intertwining her flexible tongue with yours and pleasuring the purple slut even more.");
		...
		MainScreen.text("\n\nYou keep pounding away, coating your succubi with a mix of newly formed pre-cum, old cum from your previous ejaculation and vaginal juices being spilled all over your " + CockDescriptor.describeCock(player, x) + ".  The succubus' inhuman recesses start to feel too hot for you, and with another powerful shot, you cum.  The contact of your globs of spooge hitting Vapula's backdoor makes her squirt another of her insane orgasms, but you keep releasing your milk inside her.");
		//[if second cock]
		if (player.lowerBody.cockSpot.count() > 1) {
			MainScreen.text("  You pack her tight snatch ");
			//[if third cock]
			if (player.lowerBody.cockSpot.count() > 2) MainScreen.text("and your blue lover's mouth ");
			MainScreen.text("likewise.");
		}
		MainScreen.text("  Jism is being spilled all over Vapula's backside and spurts in sticky jets down her baby-maker and the blue slut's waiting mouth.  Vapula's holes tremble and contract in order to keep as much spooge as possible, and the blue succubus tries to feed as well, frustrated to feel her own semen being eaten by Vapula's gluttonous tongue.  Cum keeps circulating in and out of the hell-girls, with your " + CockDescriptor.describeCock(player, x) + " as a source.  Due to their mouths being occupied gulping semen from some hole, they only utter slow, lewd moans as they release their own juices in orgasm. The echo of your trio of bodies abandoning themselves to this debauched orgy is resonating all around your camp.  It should be wiser to end this quickly, lest you attract a horde of horny imps willing to join the party.");
		MainScreen.text("\n\nYou abruptly pop out your " + CockDescriptor.describeCock(player, x) + " and rudely order your sluts to finish their work.  Still recovering from the hard fuck, they lazily obey and lick all the cum from your soaked junk.  They work quickly, not willing to let any drop of cum be stolen by their concurrent.  Sometimes their tongues will claim the same deposit of spooge and they will fight over it, their tongues entwining until they lewdly kiss each other.  The display is quite erotic but you'd prefer they'd kiss your " + CockDescriptor.describeCock(player, x) + ", so you call them to order every time it happens.  When they're finally done emptying you from all your sperm, you give your lovers a tender kiss, thanking them for your efforts.  The blue succubus, still dazed by this intense session, hands you distractedly a bottle of her own milk.  \"<i>Woah.  That was... amazing. I don't know what she does to you but you taste much, much better when you keep this girl around.  We should definitely repeat this soon.</i>\"  Giving your dick a goodbye kiss, she staggers out of your camp, probably drunk from your huge cumload.");
		MainScreen.text("\n\nYou ask Vapula if she agrees with what the blue succubus said. As a response, your pet hugs you tightly");
		if (player.tallness < 60) MainScreen.text(", enclosing your head in her cleavage");
		MainScreen.text(".  \"<i>That was fun! I can't wait to have another party like this! You should invite her more often!</i>\"");
	...
	private vapulaJojoThreesomes(): void {
		...
		let tease: Function = null;
		let train: Function = null;
		if (player.lowerBody.vaginaSpot.hasVagina() || player.hasKeyItem("Demonic Strap-On") >= 0) tease = NTRSomeJojos;
		if (!player.lowerBody.cockSpot.hasCock()) { } else {
			tease = NTRSomeJojos;
	...
	private NTRSomeJojos(): void {
		...
		if (player.lowerBody.cockSpot.hasCock()) {
			let x: number = player.cockThatFits(100);
			if (x < 0) x = player.smallestCockIndex();
			MainScreen.text("\"<i>Admit it, you'd love to use that smoking hot body, wouldn't you?  Don't lie to me.  You'd give up your soul in order to fuck her brains out.  Don't you want to plunge your dick inside her?  Don't you want to fuck my beautiful succubus wife?</i>\"");
			...
			MainScreen.text("\n\n\"<i>These hips... these thighs...</i>\"  Grabbing hold of Vapula's waist, you shove your " + CockDescriptor.describeCock(player, x) + " between her legs and start grinding your junk against her vaginal entrance, teasing her in the most lustful manner. You keep grinding until your purple slut utters a lubricious moan.");
			MainScreen.text("\n\n\"<i>No, really, her whole body is perfect.</i>\"  You run your hands all over the succubus curves, pinching here and caressing here.  \"<i>And gods, that tight anus!  Oh, Jojo, if you knew how cock-hungry she is...</i>\"  With a wicked, wanton smile, you rail Vapula's ass with your " + CockDescriptor.describeCock(player, x) + ", filling the entirety of her interior with your dickflesh as you watch the monk.  The poor mousy slut has completely given in to his lust; he has fallen on his knees and is now stroking himself feverishly, unable to break your gaze.  You start pounding Vapula's butt; with each thrust you grunt ostensibly, letting your mouse pet know how much you enjoy it.  As for Vapula, the slutty demoness is clearly enjoying her anal treatment, the sight of a poor bitch masturbating to her being all the more arousing to her.  She also stares at him with longing eyes, as if to provoke him with her nude, horny body being fucked by someone else.  Soon she is screaming in utter ecstasy and starts babbling meaningless dirty talk, \"<i>Fuck, fuck me... oh yessssssssssssss... more!  MORE!</i>\"  She's clearly acting as a naughty bitch!  You wonder if the presence of a shy creature is helping to release her inner wantonness... Shrugging it off, you decide to focus on the nice purple body impaling itself on your " + CockDescriptor.describeCock(player, x) + " and pound her harder, your meat releasing streams of sex juices as it pounds in and out of Vapula's pucker.");
			if (player.lowerBody.cockSpot.count() > 1) {
				...
				//[if third cock]
				if (player.lowerBody.cockSpot.count() > 2) MainScreen.text("  Since the horny slut has a demonic vaginal capacity, you might as well fit more dick in it: in no time your third dick joins its colleague in this infernal vaginal assault.");
				MainScreen.text("  ");
			}
			else MainScreen.text("\n\n");
			MainScreen.text("You push in and out of her, claiming both her holes and snickering at the poor mouse thing that can get neither. With a mighty push, you put Vapula on all fours and lean down in order to touch more of her supple body.  Your purple slut is now dying with pleasure, her tongue hanging and her eyes staring behind Jojo as the monk squeaks pitifully in his pathetic self-stroking.");
			...
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
			MainScreen.text(" being filled with cock.");
			...
			if (jojoScene.tentacleJojo()) MainScreen.text("his tentacles start flying toward his ass, ");
			MainScreen.text("his-");
			MainScreen.text("\n\n\"<i>No.</i>\"");
	...
	private jojoButtFuckTrain(): void {
		...
		let x: number = player.cockThatFits(100);
		if (x < 0) x = player.smallestCockIndex();
		MainScreen.text("You tell Jojo it's his lucky day - you're going to have a joyful butt-fuck session with him and your purple pet.");
		MainScreen.text("\n\n\"<i>Admit it, you'd love to use that smoking hot body, wouldn't you?  Don't lie to me.  You'd give up your soul in order to fuck her brains out.  Don't you want to plunge your dick inside her?  Don't you want to fuck my beautiful succubus wife?</i>\"");
		MainScreen.text("\n\nJojo shakes his head in denial, but he can hardly keep any shred of credibility when his rock-hard mouse cock speaks for him.");
		...
		MainScreen.text("\n\nYou seize your " + CockDescriptor.describeCock(player, x) + " and press its tip against the mouse's sphincter, slowly pushing.  Startled by the sudden anal invasion, Jojo reflexively grabs Vapula's hips and inserts his rodent prick into her.  The simultaneous penetrations make your sluts squeak and moan in ecstasy, their bodies entranced as their anal walls provide them one thousand subtle sensations.  You grip his arms, using him as leverage to pull out, and slam it back home again.  Used like the fucktoy he truly is, Jojo doesn't resist; his dick moves of its own volition in and out of Vapula's interior as you relentlessly push him back and forth.  You secretly rejoice: his tight little ass is clearly adapting to your " + CockDescriptor.describeCock(player, x) + " and it somehow feels even better than usual.  You pound hard, making him groan and beg for more; you eagerly give him what he wants and stuff him full of your dickflesh.");
		if (player.cockArea(x) > 100) MainScreen.text("  You can actually feel his internal organs being bumped against your meat as your " + CockDescriptor.describeCock(player, x) + " stretches his colon beyond its normal size and thrusts in and out like a charging mammoth.");
		...
		if (jojoScene.tentacleJojo()) {
			...
			//[if vagina]
			if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Last but not least, a soft cock-knock at the entrance of your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " signals an incoming intrusion.  If your mouth weren't so full of cock you would sigh in relief as your aching pussy is thoroughly penetrated.");
		}
		...
		while (temp > 0) {
			temp--;
			if (player.lowerBody.cockSpot.get(temp).cockType == CockType.TENTACLE && temp != x) {
				if (n1 == -1) n1 = temp;
		...
		if (n1 >= 0) MainScreen.text("\n\nA deliciously depraved idea crosses your mind; using your dick muscles, you move your " + CockDescriptor.describeCock(player, n1) + " toward Vapula.  It squirms for a moment as you look for her wet snatch, poking her everywhere with your tip and teasing her with your accidental ministrations, but at last you find her vaginal entrance.  Using the rubbery flexibility of your plant-like prick, you brutally shove it down her love-tunnel, trying to force inside as many inches as you can.  You can feel her instinctively altering her own body in order to accommodate more dickflesh inside her, and you keep pushing until your " + CockDescriptor.describeCock(player, n1) + " is stretched to maximum capacity.  Somehow your tentacle rod is entirely buried in Vapula's warm depths, and her unbelievable tightness is driving you mad with lust.  It twists on its on volition inside her, its vegetable-like texture being more sensitive and receptive to pleasure than a normal prick and the sensations it sends to you are abnormally exquisite.  You almost forgot the asshole you were pounding with your " + CockDescriptor.describeCock(player, x) + " but the way Jojo wriggles against it in order to fit more of your length inside him reminds you of your job.");
		//[if PC has another tentacle cock, variable is n2]
		if (n2 >= 0) MainScreen.text("\n\nSince your sluts still have free holes left, you decide to fill some more fuck-holes with your junk.  Guiding  your " + CockDescriptor.describeCock(player, n2) + " toward Vapula's mouth, you think you could use a proper blowjob from a trained succubus and wait for her to let out another moan before filling your junk completely and thrusting up and down brutally, literally mouth-fucking her and treating her lips as a tight cock-ring.  Her expert tongue wraps around your girth, squeezing it and clinging to it like it's the most precious thing in her life.  She keeps jerking back and forth due to the hard butt-fucking she's receiving, and the jolts are transmitted to your " + CockDescriptor.describeCock(player, n2) + " in the most teasing manner.");
		//[if PC has another tentacle cock, variable is n3]
		if (n3 >= 0) MainScreen.text("\n\nSince Vapula is already sucking you off, why not Jojo?  You still have appropriate junk left after all.  You promptly send your wriggling " + CockDescriptor.describeCock(player, n3) + " and push it between the mousey's lips.  However, the slut won't open them!  You pound him ragingly, slamming the entirety of your " + CockDescriptor.describeCock(player, x) + " down his colon and bruising his firm butt until he lets out a deep howl of pleasure and pain mixed; you profit from the opening and stuff his mouth so full of your " + CockDescriptor.describeCock(player, n3) + " you think he's going to choke.  However, his mouth seems to be accustomed to take monstrous lengths - probably a side-effect of his corruption - and he quickly accommodates to your size.  Although he can't provide a blowjob as sweet as Vapula, you can definitely tell he's skilled at sucking dick.  The cute little slut!  You ram his asshole faster to reward him; this increased butt-fucking pace seems to have the desired effect, and soon your " + CockDescriptor.describeCock(player, n3) + " is throbbing in need as is bathes in the mousey's saliva.");
	...
	private vapulaAndIzmaThreeSome(): void {
		MainScreen.clearText();
		if (player.lowerBody.cockSpot.hasCock()) vapulaAndIzma(false);
	...
	private vapulaAndIzma(girls: boolean = true): void {
		...
		if (x < 0) x = player.smallestCockIndex();
		if (!girls) {
			MainScreen.text("You call your beta tigershark, telling her you'll have some use for her body.  Izma eagerly approaches you, although she frowns a bit when she sees the towering succubus accompanying you.  \"<i>What can I do for my alp-... wait, what are you doing with her?</i>\"  Vapula only smirks at Izma's puzzled reaction.");
			MainScreen.text("\n\nYou gently explain that you intend to have some extra fun, that's why you brought this gorgeous creature to please you both.  Izma isn't really convinced, but still walks up to you, not willing to antagonize her alpha.  You tell to remove her cloth and turn around so you can have a clear view of her toned butt.  \"<i>What do you have in mind, " + player.short + "?</i>\"\n\n\"<i>Shhh... just do as I say.</i>\"  With a discreet gesture, you order Vapula to get on her knees in front of Izma; she's going to feed from your tigershark this time.  With a conniving smile, Vapula obeys and before your shark lover can react, you're already grinding your " + CockDescriptor.describeCock(player, x) + " against her muscular butt-cheeks while the purple succubus grabs hold of her 15 inch long pecker.  You rub the tip of your " + CockDescriptor.describeCock(player, x) + " against her butt crack.  \"<i>W-wait! What are you guys doing? Are you going to fuck my... I don't want to-</i>\"");
			...
			MainScreen.text("\n\nYour taunts don't seem to affect Izma; if anything, they only turn her on all the more.  You tease Izma's butthole with your hands and " + CockDescriptor.describeCock(player, x) + ", stretching her fuckpillows to make room for your incoming junk.  The poor tigershark doesn't even resist anymore, completely overwhelmed by your teasing ministrations and Vapula's dexterous fondling.  Her shark tail is wagging in apprehension and excitement; her loose pussy is now freely gushing juices and her tight asshole is occasionally leaking some lubricant.  She's finally getting ready!");
			MainScreen.text("\n\nWith a roar of triumph, you grab her toned hips and force your " + CockDescriptor.describeCock(player, x) + " inside her, tearing her asshole and stretching it further.  Izma screeches, obviously not used to anal violation.  Her shark tail twists and wriggles nervously above her ass, nearly hitting you.  With one hand, you immobilize her inconvenient appendage; with your free hand, you grasp again on her toned buttocks, gripping the hard muscles as your herm lover contracts them; her anal walls are squeezing your " + CockDescriptor.describeCock(player, x) + " so tightly it actually hurts.  You tell Vapula to suck harder in order to relieve your tigershark's tension.  Your skilled succubus nods and starts clamping her pink lips around Izma's mammoth prick, effectively trapping it in an organic cock-ring.  At last you feel some of the intense pressure on your " + CockDescriptor.describeCock(player, x) + " being released, allowing you to pull out and slam back, almost bruising your groin as it mashes against her hard butt-muscles.  This time her ass completely gives in to your ferocious dick-assault.  You pull back and stuff her again, increasing the pace of your repeated thrusts as the both of you gain confidence.  Soon you're butt-fucking her senseless, devastating her rectal entrance with your towering rod.");
			MainScreen.text("\n\nNeedless to say, Izma's mind is being torn in pleasure; the sweet blowjob she's receiving from your voracious succubus contrasts with your rough anal ploughing and brings her the weirdest mix of sensation. She growls and moans, her hands grasping Vapula's head as its bobs up and down the length of her long prick.  Her pussy is dripping drop after drop of fem-juice.  You notice her asshole is getting wetter and wetter as you keep penetrating it; could it be that your tigershark lover is a buttslut in denial?  Excited at the idea of turning the prude tigershark into an eager anal beta, you intensify the pace of your butt-fucking, determined to bring her to ecstasy by the sole power of your " + CockDescriptor.describeCock(player, x) + ". You want her to cum from her butt being devastated, certainly not from a petty blowjob, even from a succubus.  Vapula rapidly takes note of your sudden change of rhythm; she grins wickedly and starts taking Izma's whole dickflesh into her mouth, effortlessly deepthroating her.  Izma, being utterly crushed between her two lovers, squirms and thrashes savagely; it won't be long before she cums from your combined throat- and butt-fucking.");
			MainScreen.text("\n\nAs you give your tigershark lover another raging thrust, her groin hits Vapula's face, and the monstrously thick rod bumps against the back of her throat, making her cough unintentionally.  With a triumphal cry, you seize Izma's breasts, kneading them harshly as you pinch her nipples, caring little about her comfort in your fiery passion.  The extra stimulation is enough to make the tigershark cum, and her gaping pussy releases torrents of fem-spunk, soaking the floor with her juices.  Her orgasm is sending shivers all across her body and the way her body is trembling in your arms is enough to send you over the edge: you release a milky flood in her anus, packing her interior with seed; the contact of your goo being shot across her colon triggers her masculine orgasm, and she lets out a torrent of her own spunk into the succubus' waiting stomach, unloading her load into your purple slut as you unload yours into her.  The flow of fluids flowing in and out of her body makes her thrill in ecstasy until your " + CockDescriptor.describeCock(player, x) + " stops vibrating, running out of spooge to shoot. Vapula keeps gluttonously sucking her subsistence out of Izma's traitorously long cock, squeezing her balls in order to extract as much semen as she can.");
			MainScreen.text("\n\nNodding in approval, you pull out your own " + CockDescriptor.describeCock(player, x) + " out of your tigershark's ass and tell Vapula to keep going.  With your rod still in hand, you walk around Vapula and grab hold of her plush, jiggly ass; without warning, you plunge your still-dripping rod into her tight entrance.  Unsurprisingly, the anal penetration is much easier with the purple succubus.  She jolts a bit but this doesn't reduce the intensity of her ministrations in the least; her tainted saliva appears to have an aphrodisiac effect on Izma's cock, never letting her go completely soft.  You pump your " + CockDescriptor.describeCock(player, x) + " in and out of Vapula's plump rump while she still sucks off your tigershark lover, letting her balls fill back up with cum.  The three of you go at it for what seems like hours, Vapula wobbling back and forth between your hard butt-fuck and Izma's dick.  Her asshole is somehow as tight as Izma's, but so wonderfully hot!  The sudden change of temperature is making you shiver in pleasure.  You keep pounding away, not really caring about a slutty creature used to butt-dickings.");
			MainScreen.text("\n\nYou stare at Izma, the two of you keeping eye contact as you both penetrate your fucktoy from both ends.  Then, at last, you wink at Izma as you both cum a second time, releasing simultaneously streams of spunk in the succubus' horny body.  She happily devours all you've got, her lips and anal-lips contracting mercilessly in order to milk both cocks fresh baby-batter.  You cum and cum, your " + CockDescriptor.describeCock(player, x) + " and Izma's pulsating in rhythm as they paint the succubus' inner depths white.  You then pull out of her ass and walk toward Izma.  The poor tigershark is completely drained from the double-blowjob, and staggers her way toward the stream to get hydrated, pulling back her dick with a loud POP as it escapes from Vapula's needy lips.  The succubus looks disappointed to see her new sperm feeder running away, but you remind her your own " + CockDescriptor.describeCock(player, x) + " still needs to be cleaned. With a happy sigh, she works on your junk for a while, tingling you in the most pleasant way as her tongue runs across your dickflesh and slurps all the remaining goo. When you're as empty as she's full, you both rest side to side, recovering from the intense fuck.");
		}
		//Vapula/Izma threesome (as female)
		else {
			MainScreen.text("You call your beta tigershark, telling her you'll have some use for her body.  Izma eagerly approaches you as you slip into your strap-on, only starting to frown when she sees the towering succubus accompanying you.  \"<i>What can I do for my alp-... wait, what are you doing with her?</i>\"  Vapula only smirks at Izma's puzzled reaction.");
			...
			MainScreen.text("\n\nNodding in approval, you pull the dildo out of your tigershark's ass with a wet pop and tell Vapula to keep going.  With your rod still in hand, you walk around Vapula and grab hold of her plush, jiggly ass and without warning stuff your still-dripping strap-on in her ass; unsurprisingly, the anal penetration is much easier with the purple succubus.  She jolts a bit but this doesn't reduce the intensity of her ministrations in the least; her tainted saliva appears to have an aphrodisiac effect on Izma's cock, never letting her get completely soft.  You pump your dildo in and out of Vapula's plump rump, enjoying the pink end stroking your dripping " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " while she still sucks off your tigershark lover, letting her balls fill back up with cum.");
			...
			MainScreen.text("\n\nShe happily devours all you've got, her lips and nether-lips contracting mercilessly in order to milk both cocks for fresh baby-batter.  The sight of the shameless cumbucket getting what she wants pushes you to a new height and you orgasm and orgasm, your end of the strap-on vibrating furiously in your streaming depths as the other end and Izma pulsate in rhythm and paint the succubus' inner depths white.  You then pull out of her ass and walk toward Izma. The poor tigershark is completely drained from the double-blowjob, and staggers her way toward the stream to get hydrated, pulling back her dick with a loud POP as it escapes from Vapula's needy lips.  The succubus looks disappointed to see her new sperm feeder running away, but you remind her you still need to be cleaned.  With a happy sigh, she works on your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " for a while, tingling you in the most pleasant way as her tongue runs slowly across your drooling sex and slurps up all the girl slime she can possibly tease out of you.  When you're as dry as she is full, you both rest side to side, recovering from the intense fuck.");
		}
		player.orgasm();
		player.stats.sens += -2;
		player.stats.cor += 2;
	...
	private chixFeedVapulaBlehblehIVantToZuckYourSpooo(): void {
		MainScreen.clearText();
		MainScreen.text("You produce the demonic dildo and hold it by the hilt of its giant purple end, wagging it lazily by your side with a playful grin.  Vapula knows what that means; she is on her knees in an instant, staring at you with wolfish anticipation.");
		//Feed/Tease
		let tease: Function = null;
		if (Flags.list[FlagEnum.VAPULA_HAREM_FUCK] == 0) tease = teaseVapula;
	...
	private teaseVapula(): void {
		...
		if (Flags.list[FlagEnum.VAPULA_HAREM_FUCK] == 0 && (Flags.list[FlagEnum.VAPULA_DAYS_SINCE_FED] == 0)) {
			MainScreen.text("You decide you aren't going to give her what she wants so easily . You're also genuinely curious about how badly she needs cum, and how far you can push her.  You don't say anything as you slowly buckle the strap-on, sighing as you slip the small, pink end into your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".  You make Vapula wait as you close your eyes and make the dildo pulse inside you, slowly filling you up and then withdrawing, letting it push up against your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + ".");
			...
			Flags.list[FlagEnum.VAPULA_TEASE_COUNT]++;
		}
		//Second Tease
		//Requires: Vapula not fucking harem, teased the day before
		else if (Flags.list[FlagEnum.VAPULA_HAREM_FUCK] == 0 && Flags.list[FlagEnum.VAPULA_DAYS_SINCE_FED] == 1) {
			MainScreen.text("You smile as once again Vapula gets into position on her knees, and you buckle on the strap-on and stand over her.  This time you make the pink end grow long, closing your eyes as it inches up your tunnel, before slowly thrusting your " + LowerBodyDescriptor.describeHips(player) + " backwards and forwards in front of your succubus, letting it shrink and grow so it climbs up and down your wet walls.");
			...
			//[Hooves:
			if (player.lowerBody == LowerBodyType.HOOFED || player.lowerBody.isTaur()) MainScreen.text("\"<i>I would worship your feet, mistress.  I would clean your hooves with my tongue to a shine, tease out every scrap of dirt from every crevice.  And I would thank you for the privilege.</i>\"");
			//[Naga:
		...
		//Third Tease
			...
			MainScreen.text("Once again you buckle up, sighing with pleasure as you sink the pink dildo into your moist hole, and once again you wait until Vapula gets onto her knees before you.  She looks at you silently and sullenly as you droop the monstrous purple end over her face.");
			...
			//Naga:
			if (player.lowerBody.isNaga()) MainScreen.text("you slide your tail under and over her trunk, wrapping your coils around her and drawing her face into your stuffed crotch.");
			//Other:
			else if (player.lowerBody.isTaur() || player.lowerBody.isGoo() || player.lowerBody.isDrider()) MainScreen.text("you sit the monstrous half of your body down comfortably and draw her face into your stuffed cotch.");
			else MainScreen.text("you open your [legs] and wrap them around her head, drawing her face into your stuffed crotch.");

			MainScreen.text("\n\nShe works on it for what seems like hours.  You look down at your slut; she keeps staring at you with avid yet playful eyes, never breaking eye contact as she relentlessly throatfucks herself; you feel her lips curve a little as the shape of a ravenous smile appears on her face, and her eyes are glittering with an insatiable need.  The naughty bitch!  She knows she's going to get what she wants, whether you like it or not.  You wish you were able to control yourself, but through the sheer force of her obscene hunger the succubus is somehow able to control your end of the strap-on, making the dildo shrink, pulsate and push into your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " involuntary, which soon has you panting, groaning, and thrusting back into her to sate your own growing need.");
			MainScreen.text("\n\nYour stamina is rapidly overwhelmed and you decide to reward her effort; you savagely press Vapula's head against your groin, burying all eight inches of your false cock into her waiting throat; in symphony the two of you will your throbbing dildo to expand to pack you mercilessly tight, making you cum brutally.  Your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " pulses as your vagina contracts ecstatically around the bulging sex toy; vaguely you feel release at the other end, and you hear Vapula loudly and shamelessly enjoy what the dildo pumps out.  You abandon yourself to the climax and keep thrusting your " + LowerBodyDescriptor.describeHips(player) + " into her face, girlcum spurting around your harness.");
	...
	private chicksFeedVapula(): void {
		...
		if (Flags.list[FlagEnum.VAPULA_TEASE_COUNT] > 1) {
			MainScreen.text("You push the pink end of the dildo into you with a sigh, then strap yourself into the harness so that the monstrous purple end bobs out in front of you.  You stand over a tense Vapula and tease her with it first, lightly brushing her face with the tip for a while.  She swallows thickly and licks her lips as you caress her with your cum fountain, but makes no movements towards it.  Eventually you giggle at her furious concentration and tell her you're going to reward her for being such a good slut, before pushing the end against her lips.  A flood of ecstatic thank-you-mistresses are swiftly muffled out as the dildo disappears into her mouth.");
		}
		else {
			MainScreen.text("You tell Vapula you're going to reward her for being such a good slut; pushing the pink end of the dildo into you with a sigh, you strap yourself into the harness so that the monstrous purple end bobs out in front of you.  You giggle as Vapula grabs hold of it and proceeds to lick it, coating the entirety of your artificial junk with her warm, sweet saliva.");
		}
	...
	public vapulaForceFeeds(): void {
		if (player.lowerBody.vaginaSpot.hasVagina() && player.hasKeyItem("Demonic Strap-On") >= 0 && !player.lowerBody.cockSpot.hasCock()) {
			...
			Flags.list[FlagEnum.VAPULA_DAYS_SINCE_FED] = 0;
			Flags.list[FlagEnum.VAPULA_TEASE_COUNT] = 0;
			Flags.list[FlagEnum.VAPULA_EARNED_A_SPANK] = 1;
		}
		//[if no fuck harem is on and PC hasn't fed/fucked Vapula for 5 days that scene will trigger at night]
			...
			MainScreen.text("\n\nShe works on it for what seems like hours. You look down at your slut; she keeps staring at you with avid yet playful eyes, never breaking eye contact as she relentlessly throatfucks herself; you feel her lips curve a little as the shape of a ravenous smile appears on her face, and her eyes are glittering with an insatiable need. The naughty bitch!  She knows she's going to get what she wants, whether you like it or not.  You wish you were able to control yourself, but the covetous succubus never lets go of your " + CockDescriptor.describeCock(player, x) + ", clinging to it like an animal.");
			MainScreen.text("\n\nYour stamina is rapidly overwhelmed and you decide to reward her effort; you savagely press Vapula's head against your groin, effectively burying all of your " + CockDescriptor.describeCock(player, x) + " in her waiting throat; her tongue keeps teasing your rod for a few seconds, and then you cum, brutally.");
			if (player.lowerBody.balls > 0) MainScreen.text("  Your " + BallsDescriptor.describeBalls(true, true, player) + " churn");
			else MainScreen.text("  Your " + CockDescriptor.describeCock(player, x) + " twitches");
			MainScreen.text(" as you release a sticky spooge fountain in her stomach.");
			...
			if (player.cumQ() >= 1500) MainScreen.text(" until she looks 6 months pregnant.  At last, the cum-flow spilling through your urethra starts to ebb and your " + CockDescriptor.describeCock(player, x) + " stops throbbing; only then does she removes your junk from her mouth with a loud POP.  Your tool appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.");
			player.orgasm();
			player.stats.cor += 1;
			Flags.list[FlagEnum.VAPULA_DAYS_SINCE_FED] = 0;
			Flags.list[FlagEnum.VAPULA_TEASE_COUNT] = 0;
	...
	private spankVapulaLikeABoss(): void {
		...
		if (camp.companionsCount() > 2) MainScreen.text(", drawing the eyes of your other companions.");
		else if (camp.companionsCount() == 2) MainScreen.text(", drawing the attention of your other companion.");
		else MainScreen.text(", echoing off the sentinel-like rocks that seem to ring your encampment.");
		MainScreen.text("  Her tail goes wild at the impact, whipping around in a frenzy and actually managing to slap your cheek!  That bitch!");
		...
		if (player.perks.has("Sadist")) MainScreen.text("  Dishing out all this suffering is getting you a little hot under the collar, and you squirm a bit in place in anticipation of the pain to come.");
		...
		//{+20ish lust}
		dynStats("lus", (10 + player.stats.lib / 7));
		//{Sadist: + 20 lust}
		if (player.perks.has("Sadist")) dynStats("lus", (10 + player.stats.lib / 7));
		Flags.list[FlagEnum.VAPULA_EARNED_A_SPANK] = 0;
		doNext(camp.returnToCampUseOneHour);
	}
}
}
