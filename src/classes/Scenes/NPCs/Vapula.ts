...
//switch follower names depending on conditions
		public function mouseWaifuFreakout(amily:boolean = false, jojo:boolean = false):void
		{
			...
			//[if libido >=50]
			if (player.stats.lib >= 50) outputText("\"<i>Well, as long as she swallows, I don't care,</i>\" you quip.");
			outputText("\n\n\"<i>That's it!</i>\"  The ");
...
//Follower Summoning text (Z)
		public function callSlaveVapula(output:boolean = true):void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina() && player.hasKeyItem("Demonic Strap-On") >= 0) {
				...
				else if (output && !player.lowerBody.cockSpot.hasCock()) outputText("\n\nYou aren't suitably aroused to feed Vapula right now.");
			}
		...
		private function harvestMilk():void
		{
			...
			if (flags[FlagEnum.FARM_UPGRADES_REFINERY] == 0) outputText("“<i>It’ll sure as hell beat whatever disgusting thing the bitch would have me doing instead. Why don’t you make her build a concentration machine? Lethice has loads; makes one dose of cream go so much further.</i>”");
		...
		private function fapulaFapfapfapAppearance():void
		{
			...
			if (kGAMECLASS.farm.farmCorruption.hasTattoo("vapula"))
			{
				...
				else
				{
					...
					if (flags[FlagEnum.VAPULA_TATTOO_COLLARBONE] != 0) outputText(flags[FlagEnum.VAPULA_TATTOO_COLLARBONE] + "\n");
					if (flags[FlagEnum.VAPULA_TATTOO_SHOULDERS] != 0) outputText(flags[FlagEnum.VAPULA_TATTOO_SHOULDERS] + "\n");
					if (flags[FlagEnum.VAPULA_TATTOO_LOWERBACK] != 0) outputText(flags[FlagEnum.VAPULA_TATTOO_LOWERBACK] + "\n");
					if (flags[FlagEnum.VAPULA_TATTOO_BUTT] != 0) outputText(flags[FlagEnum.VAPULA_TATTOO_BUTT] + "\n");
				}
			}
...
//Feed
		private function feedVapulaACupOfJizz():void
		{
			...
			if (player.lowerBody.balls > 0) outputText("Your " + ballsDescriptLight() + " churn");
...
//Option: Please both.
		private function vapulaCeraphThreesomePleaseBoth():void
		{
			...
			outputText(".");
			outputText("\n\nYou feel your ");
			if (player.lowerBody.balls > 0) outputText(ballsDescriptLight() + " twitch and your " + cockDescript(x) + " pulsate as they unload their");
			...
			if (player.lowerBody.balls > 0) outputText(ballsDescriptLight() + " hitting her butt-hole with a faint thud");
			...
			if (player.lowerBody.cockSpot.count() > 4) outputText("  Sadly, your extra junk does not have any hole left to fill.  It just hangs there, rubbing against Ceraph's backside and teasing her all the more.");
			...
			//[if multicocks]
			if (player.lowerBody.cockSpot.count() > 1) outputText("  She expertly jerks each of your rods in turn, gently polishing them as they grow back into an erect state.");
			...
			if (player.lowerBody.balls > 0) outputText(" as the girls keep fondling your " + ballsDescriptLight());
...
//Ceraph-Vapula Buttfuck train
		private function vapulaAndCeraphButtfuckTrainYeehaw():void
		{
			...
			//[if balls]
			if (player.lowerBody.balls > 0) outputText("  As you relentlessly smash your groin against Ceraph's plump rump, your " + ballsDescriptLight() + " repeatedly hit her quivering vagina; they are soon utterly coated in fem-spunk.");
...
//Vapula-Sophie threesome
		...
		private function vapulaSophieThreeSome():void
		{
			...
			if (player.lowerBody.balls > 0) outputText("balls and massages your ");
			...
			if (player.lowerBody.cockSpot.count() < 3) outputText("hastily pull out and ");
			outputText("wave ");
			if (player.lowerBody.cockSpot.count() >= 3) outputText("one of ");
			outputText("your still pulsing dick");
			if (player.lowerBody.cockSpot.count() >= 3) outputText("s");
...
//Vapula-Amily threesome
		private function vapulaAndAmilyThreesome():void
		{
			...
			if (player.lowerBody.balls > 0) outputText(": her drooling pussy needs to be prepared because you're going to take her next");
			...
			//[if not defurred]
			if (flags[FlagEnum.AMILY_NOT_FURRY] == 0) outputText(" and her fur starts shrinking, letting her rosy skin shine below");
...
//Vapula/Night Succubus
		public function vapulaAssistsCeruleanSuccubus():void
		{
			...
			//[if multicock]
			if (player.lowerBody.cockSpot.count() > 1) outputText("  She suddenly seems to have an idea: leaning down to reach your second cock, she starts licking it, accompanying the savage thrusting of the blue demoness as she slurps your extra junk, giving you even more pleasure.");
			...
			if (player.lowerBody.cockSpot.count() > 1) {
				...
				//[if third cock]
				if (player.lowerBody.cockSpot.count() > 2) outputText("and your blue lover's mouth ");
				outputText("likewise.");
			}
		...
		private function NTRSomeJojos():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				...
				if (player.lowerBody.cockSpot.count() > 1) {
					...
					//[if third cock]
					if (player.lowerBody.cockSpot.count() > 2) outputText("  Since the horny slut has a demonic vaginal capacity, you might as well fit more dick in it: in no time your third dick joins its colleague in this infernal vaginal assault.");
					outputText("  ");
				}
				else outputText("\n\n");
				...
				//[if second cock]
				if (player.lowerBody.cockSpot.count() > 1) outputText("  Your extra junk shoved down her love-tunnel also explodes in a milky torrent, blasting her fuck-hole full of spooge too.");
...
//Vapula/Jojo Butt-fuck train
		private function jojoButtFuckTrain():void
		{
			...
			if (jojoScene.tentacleJojo()) {
				...
				//[if vagina]
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText("  Last but not least, a soft cock-knock at the entrance of your " + vaginaDescript() + " signals an incoming intrusion.  If your mouth weren't so full of cock you would sigh in relief as your aching pussy is thoroughly penetrated.");
			}
...
//Tease
		private function teaseVapula():void
		{
			...
			//Requires: Vapula not fucking harem, teased the day before
				...
				//[Hooves:
				if (player.lowerBody == LOWER_BODY.HOOFED || player.isTaur()) outputText("\"<i>I would worship your feet, mistress.  I would clean your hooves with my tongue to a shine, tease out every scrap of dirt from every crevice.  And I would thank you for the privilege.</i>\"");
				//[Naga:
...
//Requires bipedal-ness
		private function spankVapulaLikeABoss():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText(" and a healthy surge of blood to [eachCock]");
			else if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" and a teltale moistness to your nethers");
			outputText(".");
			...
			if (player.perks.has("Sadist")) outputText("  Dishing out all this suffering is getting you a little hot under the collar, and you squirm a bit in place in anticipation of the pain to come.");
