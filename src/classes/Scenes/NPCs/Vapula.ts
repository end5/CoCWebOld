...
//Camp lines
		public vapulaSlaveFlavorText():void
		{
			...
			if (choice == 0) Render.text("Vapula is currently resting on a very rough bed of leaves she's gathered herself.  She sleeps quietly, her hand absent-mindedly laid on her vagina and the other one resting on her breasts.  A thin trickle of her juice is leaking out of her crotch.");
			if (choice == 1) Render.text("Your succubus slut, Vaplua, is furiously stroking herself on her bed of leaves, her purple body shining in sweat.  She moans raggedly, often looking up to you with lust-consumed eyes.");
			if (choice == 2) {
				if (flags[FlagEnum.VAPULA_HAREM_FUCK] == 0) choice = 3;
				else Render.text("Vapula is currently absent, probably getting screwed by some of the imps surrounding your camp in order to get her semen rations.  You're sure she would come back instantly if you ever needed to give her YOUR cum instead.");
			}
			if (choice == 3) Render.text("Vapula is busy drinking a vial of milk that looks very much like the kind you'd usually find on imps.  Her purple cheeks seem to blush darker when she sees you; she awkwardly gulps the rest of the bottle, hiccups, and giggles softly, eyeing you with a coy smile and a lust-filled gaze.  The potion seems to have made her even more aroused than usual.");
			if (choice == 4) {
				Render.text("Vapula is currently toying with ");
				if (player.hasKeyItem("Deluxe Dildo") >= 0) Render.text("your deluxe dildo");
				else Render.text("a dildo");
				Render.text(", pumping as fast as she can.  In her feverish display of lust she doesn't even notice you.");
			}
			if (choice == 5) {
				if (flags[FlagEnum.VAPULA_HAREM_FUCK] == 0 || !(sophieBimbo.bimboSophie())) choice = 6;
				//if harem fucking is on)
				else Render.text("Vapula is fingering herself while eating out Sophie, your harpy slut.  The dumb avian matron keeps moaning like she's in heat while the succubus furiously assaults her lush fuck-hole; both of their pair of wings flap in rhythm as they feel their pleasure rising in waves.");
			}
			if (choice == 6) {
...
//switch follower names depending on conditions
		public mouseWaifuFreakout(amily:boolean = false, jojo:boolean = false):void
		{
			...
			if (amily) {
				Render.text("Amily ");
				if (jojo) Render.text("and ");
			}
			if (jojo) Render.text("Jojo ");
			Render.text("walk");
			if (!(amily && jojo)) Render.text("s");
			Render.text(" up to you, worried, as Vapula struts around the camp.");
			...
			//[if libido >=50]
			if (player.stats.lib >= 50) Render.text("\"<i>Well, as long as she swallows, I don't care,</i>\" you quip.");
			Render.text("\n\n\"<i>That's it!</i>\"  The ");
			if (jojo && amily) Render.text("mice are");
			else Render.text("mouse is");
			Render.text(" raving.  \"<i>You've clearly given in to her demonic lust.</i>\"");
			if (jojo) {
...
//Follower Summoning text (Z)
		public callSlaveVapula(output:boolean = true):void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina() && player.hasKeyItem("Demonic Strap-On") >= 0) {
				...
				else if (output && !player.lowerBody.cockSpot.hasCock()) Render.text("\n\nYou aren't suitably aroused to feed Vapula right now.");
			}
			...
			if (flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 0 && flags[FlagEnum.FARM_CORRUPTION_STARTED] == 1) addButton(6, "Farm Work", sendToFarm);
			if (flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 1) addButton(6, "Go Camp", backToCamp);
			
			if (flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 1 && flags[FlagEnum.FOLLOWER_PRODUCTION_VAPULA] == 0) addButton(7, "Harvest Milk", harvestMilk);
			if (flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 1 && flags[FlagEnum.FOLLOWER_PRODUCTION_VAPULA] == 1) addButton(7, "Stop Harvest", stopHarvest);
					
			if (flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 1) addButton(9, "Back", kGAMECLASS.farm.farmCorruption.rootScene);
		}
		...
		private sendToFarm():void
		{
			...
			Render.text("You tell your succubus concubine that she is to head towards the lake, find a farm, present herself to the lady who works there and do as she says. Vapula laughs long and hard at this.");
		...
		private backToCamp():void
		{
			...
			Render.text("You tell her to head back to camp; there are things you need to do to her you can’t do whilst she’s here. Repeatedly. Vapula fist pumps the sky.");
		...
		private harvestMilk():void
		{
			...
			Render.text("You tell Vapula that you want her hooked up to a milking machine whenever possible; you need her fluids.");
			...
			if (flags[FlagEnum.FARM_UPGRADES_REFINERY] == 0) Render.text("“<i>It’ll sure as hell beat whatever disgusting thing the bitch would have me doing instead. Why don’t you make her build a concentration machine? Lethice has loads; makes one dose of cream go so much further.</i>”");
			else Render.text("“<i>As you insist, [master]. It’ll sure as hell beat working.</i>”");
		...
		private stopHarvest():void
		{
			...
			Render.text("You tell Vapula to stop producing succubus milk; you’re practically drowning in the stuff.");
...
//Vapula Appearance - this bitch is purpler than a Nigerian (Z)
		private fapulaFapfapfapAppearance():void
		{
			...
			if (kGAMECLASS.farm.farmCorruption.hasTattoo("vapula"))
			{
				...
				{
					if (kGAMECLASS.farm.farmCorruption.numTattoos("vapula") > 1) Render.text("She has the following tattoos emblazoned across her body:\n");
					else Render.text("She has ");

					if (flags[FlagEnum.VAPULA_TATTOO_COLLARBONE] != 0) Render.text(flags[FlagEnum.VAPULA_TATTOO_COLLARBONE] + "\n");
					if (flags[FlagEnum.VAPULA_TATTOO_SHOULDERS] != 0) Render.text(flags[FlagEnum.VAPULA_TATTOO_SHOULDERS] + "\n");
					if (flags[FlagEnum.VAPULA_TATTOO_LOWERBACK] != 0) Render.text(flags[FlagEnum.VAPULA_TATTOO_LOWERBACK] + "\n");
					if (flags[FlagEnum.VAPULA_TATTOO_BUTT] != 0) Render.text(flags[FlagEnum.VAPULA_TATTOO_BUTT] + "\n");
				}
			}
...
//Talk (Z)
		...
		{
			clearOutput();
			Render.text("\"<i>You want to talk?  Well it's quite rare to see " + player.mf("studs", "girls") + " looking for conversation with the likes of me, but go ahead.  What do you want to talk about?</i>\"");
...
//Feed
		...
		{
			clearOutput();
			Render.text("You tell Vapula you're going to reward her for being such a good slut; idly stroking [oneCock] into erection, you command her to get on her knees so she can receive her meal.  The demoness eagerly complies and grabs hold of [oneCock].  She then proceeds to lick it, coating the entirety of your junk with her warm, sweet saliva. With an expert precision, she massages all your most sensitive points, pumping pre-cum up your urethra with ruthless efficiency.  As drops of your juices dribble from the tip of your " + cockDescript(0) + " she voraciously slurps them with a gourmand's smile.");
			...
			if (player.lowerBody.balls > 0) Render.text("Your " + ballsDescriptLight() + " churn");
			else Render.text("Your " + cockDescript(0) + " twitches");
			Render.text(" as you release a sticky spooge fountain in her stomach.  Her lips keep squeezing your junk in order to milk everything you've got, and in your shuddering orgasm you're too eager to comply.  With a surrendering sigh, you abandon yourself to your climax and keep cumming, squirting your baby-batter to the last drop as the succubus keeps gulping it.  Her eyes are closed in an expression of complete satisfaction; she enjoys her meal to its full extent, filling her belly with your spooge");
			if (player.cumQ() >= 1500) Render.text(" until she looks six months pregnant");
			Render.text(".  At last, the cum-flow spilling through your urethra starts to ebb and your " + cockDescript(0) + " stops throbbing; only then does she removes your junk from her mouth with a loud POP.  Your " + cockDescript(0) + " appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.");
			player.orgasm();
			dynStats("cor", .5);
...
//Option: Please both.
		private vapulaCeraphThreesomePleaseBoth():void
		{
			...
			Render.text("You order your girls to turn around; you want to give them a good dicking.  They comply while giggling like a pair of whores, eager to get some cock up their holes.");
			...
			if (player.cumQ() >= 1500) Render.text(" and swelling her belly until she looks ready to give birth to a full horde of imps");
			Render.text(".");
			Render.text("\n\nYou feel your ");
			if (player.lowerBody.balls > 0) Render.text(ballsDescriptLight() + " twitch and your " + cockDescript(x) + " pulsate as they unload their");
			else Render.text("your " + cockDescript(x) + " twitch and pulsate as it unloads its");
			Render.text(" essence into your omnibus pet, bringing her to orgasm as she blubbers incoherently, \"<i>Oh yessssssssss, just like that... Yeah... my ass... cum... yessssssssss...</i>\"  Her voice goes so high pitched it trails off, letting you hear Vapula's soft moans below.  You quickly pull out your " + cockDescript(x) + ", still orgasming.  You hear Ceraph feebly protest at the sudden lack of cock filling her hole, \"<i>What are you doing? Put it back! Put it back already!</i>\"");
			...
			if (player.lowerBody.balls > 0) Render.text(ballsDescriptLight() + " hitting her butt-hole with a faint thud");
			else Render.text("thudding up against her cushy butt-cheeks");
			Render.text(".  The purple sex-goddess squeals, your abrupt dick-digging utterly ravaging her cunt; her unholy warmth and impossibly tight confines are almost enough to make you cum instantly as her vaginal walls wrap in the most sensitive way around your rod, but you manage to hold off the flow of semen churning in your groin.");
			//[if 2nd cock]
			...
			if (player.lowerBody.cockSpot.count() > 4) Render.text("  Sadly, your extra junk does not have any hole left to fill.  It just hangs there, rubbing against Ceraph's backside and teasing her all the more.");
			...
			//[if multicocks]
			if (player.lowerBody.cockSpot.count() > 1) Render.text("  She expertly jerks each of your rods in turn, gently polishing them as they grow back into an erect state.");
			Render.text("  She's obviously used to this kind of practice; even though [eachCock] felt numb after cumming so hard, you feel your lust build up again as your genitals throb once more with need.  You grab Ceraph's head and impale her on your crotch, forcefully throat-fucking her.");
			...
			Render.text("  Your thoughts are quickly drowned in waves of pleasure, and soon your only thought is to ram more of your " + cockDescript(x) + " down your sluts' throats, pumping their heads alternatively around your shaft");
			if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
			if (player.lowerBody.balls > 0) Render.text(" as the girls keep fondling your " + ballsDescriptLight());
			Render.text(", bringing you extra tingles of pleasure to your body.");
			...
			if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
			Render.text(", their lips doing their best to milk ");
			...
			Render.text("cream.  Once your junk feels completely empty, [oneCock] is released with a loud POP.");
...
//Ceraph-Vapula Buttfuck train
		private vapulaAndCeraphButtfuckTrainYeehaw():void
		{
			...
			//[if balls]
			if (player.lowerBody.balls > 0) Render.text("  As you relentlessly smash your groin against Ceraph's plump rump, your " + ballsDescriptLight() + " repeatedly hit her quivering vagina; they are soon utterly coated in fem-spunk.");
			Render.text("  The entirety of your crotch ends up drenched with a various mix of sexual juices, and your " + cockDescript(x) + " makes squelching noises at each thrust in and out of your omnibus' little ass.");
...
//Vapula-Sophie threesome
		...
		private vapulaSophieThreeSome():void
		{
			...
			if (player.lowerBody.balls > 0) Render.text("balls and massages your ");
			Render.text(buttDescript() + ".");
			//[if third cock]
			if (player.totalCocks() > 2) Render.text("  Using her extra flexibility, she bends over and start jerking your third pole, the sweet handjob adding to your pure cocktail of pleasure.");
			Render.text("  You can feel her hard nipples poke at your skin as they gently scrape across your torso.  Vapula hardly ever interrupts her embrace, only pulling back from your lips to take care of other intimate parts of your body.  You keep riding the bimbo senseless, making her squirm and squawk in ecstasy as your own cries of pleasure are muffled by Vapula's voracious lips.");
...
//Vapula-Amily threesome
		private vapulaAndAmilyThreesome():void
		{
			...
			if (player.lowerBody.balls > 0) Render.text(": her drooling pussy needs to be prepared because you're going to take her next");
			Render.text(".  Vapula smirks in excitement and eagerly shoves her tumescent pussy in front of the mousette's waiting lips; Amily keeps wagging her butt impatiently, looking forward to being used as a disposable anal-toy.  You wink at Vapula; she's going to like it.");
			...
			if (player.lowerBody.balls > 0) {
				...
				if (player.cumQ() >= 1500) Render.text("  Sticky globs of white goo are shot from the tip of your rod, effectively stuffing Vapula so full of your spooge that some of it drips out and rains down on Amily's face, sopping her with fluids. The succubus quivers in your arms, giving in to her lust before squirting a powerful spray of fem-spooge, splattering Amily even more.");
				Render.text("  Once your orgasm starts fading, you drop Vapula and tell her to imitate Amily; the dilligent succubus gets on her knees and quickly licks your " + cockDescript(x) + ", feeding her cum-thirst as she cleans your junk of its spooge.  Your two sluts work quickly, easily sucking up every bit of goo until your " + cockDescript(x) + " eventually shrinks to a half-erect state, your lust consumed.");
			}
...
//Vapula/Night Succubus
		public vapulaAssistsCeruleanSuccubus():void
		{
			...
			//[if multicock]
			if (player.lowerBody.cockSpot.count() > 1) Render.text("  She suddenly seems to have an idea: leaning down to reach your second cock, she starts licking it, accompanying the savage thrusting of the blue demoness as she slurps your extra junk, giving you even more pleasure.");
			else Render.text("  The blue demoness leans down and starts licking Vapula's moist pussy, intertwining her flexible tongue with yours and pleasuring the purple slut even more.");
			...
			if (player.tallness < 60) Render.text(", enclosing your head in her cleavage");
			Render.text(".  \"<i>That was fun! I can't wait to have another party like this! You should invite her more often!</i>\"");
...
//Vapula/Jojo Netorare
		private NTRSomeJojos():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				let x: number = player.cockThatFits(100);
				if (x < 0) x = player.smallestCockIndex();
				Render.text("\"<i>Admit it, you'd love to use that smoking hot body, wouldn't you?  Don't lie to me.  You'd give up your soul in order to fuck her brains out.  Don't you want to plunge your dick inside her?  Don't you want to fuck my beautiful succubus wife?</i>\"");
				...
				if (player.lowerBody.cockSpot.count() > 1) {
					...
					//[if third cock]
					if (player.lowerBody.cockSpot.count() > 2) Render.text("  Since the horny slut has a demonic vaginal capacity, you might as well fit more dick in it: in no time your third dick joins its colleague in this infernal vaginal assault.");
					Render.text("  ");
				}
				else Render.text("\n\n");
				Render.text("You push in and out of her, claiming both her holes and snickering at the poor mouse thing that can get neither. With a mighty push, you put Vapula on all fours and lean down in order to touch more of her supple body.  Your purple slut is now dying with pleasure, her tongue hanging and her eyes staring behind Jojo as the monk squeaks pitifully in his pathetic self-stroking.");
				...
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
				Render.text(" being filled with cock.");
				...
				if (jojoScene.tentacleJojo()) Render.text("his tentacles start flying toward his ass, ");
				Render.text("his-");
				Render.text("\n\n\"<i>No.</i>\"");
...
//Vapula/Jojo Butt-fuck train
		private jojoButtFuckTrain():void
		{
			...
			let x: number = player.cockThatFits(100);
			if (x < 0) x = player.smallestCockIndex();
			Render.text("You tell Jojo it's his lucky day - you're going to have a joyful butt-fuck session with him and your purple pet.");
			Render.text("\n\n\"<i>Admit it, you'd love to use that smoking hot body, wouldn't you?  Don't lie to me.  You'd give up your soul in order to fuck her brains out.  Don't you want to plunge your dick inside her?  Don't you want to fuck my beautiful succubus wife?</i>\"");
			Render.text("\n\nJojo shakes his head in denial, but he can hardly keep any shred of credibility when his rock-hard mouse cock speaks for him.");
			...
			if (player.cockArea(x) > 100) Render.text("  You can actually feel his internal organs being bumped against your meat as your " + cockDescript(x) + " stretches his colon beyond its normal size and thrusts in and out like a charging mammoth.");
			...
			if (jojoScene.tentacleJojo()) {
				...
				//[if vagina]
				if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Last but not least, a soft cock-knock at the entrance of your " + vaginaDescript() + " signals an incoming intrusion.  If your mouth weren't so full of cock you would sigh in relief as your aching pussy is thoroughly penetrated.");
			}
			...
			//[if second multicock]
			if (n1 >= 0) Render.text(", as well as Vapula's love-tunnel");
			if (n2 >= 0) Render.text(" and throat, ");
			if (n3 >= 0) Render.text("and Jojo's too");
			Render.text(".  You feel that Jojo's release is coming too.  With a powerful thrust, he squirts his own mouse semen down Vapula's waiting ass");
			//[if tentacle Jojo]
			if (jojoScene.tentacleJojo()) Render.text("; his tentacles absolutely drench your every hole with greenish mousey-cum");
			Render.text(".  Vapula's own orgasm also starts kicking in, the feeling of hot cum flooding her interior being too much for her horny body to bear.");
...
//Vapula/Izma threesome(F)
		private vapulaAndIzma(girls:boolean = true):void
		{
			...
			if (x < 0) x = player.smallestCockIndex();
			if (!girls) {
				Render.text("You call your beta tigershark, telling her you'll have some use for her body.  Izma eagerly approaches you, although she frowns a bit when she sees the towering succubus accompanying you.  \"<i>What can I do for my alp-... wait, what are you doing with her?</i>\"  Vapula only smirks at Izma's puzzled reaction.");
			...
			//Vapula/Izma threesome (as female)
				...
				Render.text("You call your beta tigershark, telling her you'll have some use for her body.  Izma eagerly approaches you as you slip into your strap-on, only starting to frown when she sees the towering succubus accompanying you.  \"<i>What can I do for my alp-... wait, what are you doing with her?</i>\"  Vapula only smirks at Izma's puzzled reaction.");
...
//Feed (as female)
		...
		{
			clearOutput();
			Render.text("You produce the demonic dildo and hold it by the hilt of its giant purple end, wagging it lazily by your side with a playful grin.  Vapula knows what that means; she is on her knees in an instant, staring at you with wolfish anticipation.");
			//Feed/Tease
			let tease:Function = null;
...
//Tease
		private teaseVapula():void
		{
			...
			//Requires: Vapula not fucking harem, teased the day before
				...
				//[Hooves:
				if (player.lowerBody == LOWER_BODY.HOOFED || player.isTaur()) Render.text("\"<i>I would worship your feet, mistress.  I would clean your hooves with my tongue to a shine, tease out every scrap of dirt from every crevice.  And I would thank you for the privilege.</i>\"");
				//[Naga:
			...
			//Third Tease
				...
				Render.text("Once again you buckle up, sighing with pleasure as you sink the pink dildo into your moist hole, and once again you wait until Vapula gets onto her knees before you.  She looks at you silently and sullenly as you droop the monstrous purple end over her face.");
				...
				//Naga:
				if (player.isNaga()) Render.text("you slide your tail under and over her trunk, wrapping your coils around her and drawing her face into your stuffed crotch.");
				//Other:
				else if (player.isTaur() || player.isGoo() || player.isDrider()) Render.text("you sit the monstrous half of your body down comfortably and draw her face into your stuffed cotch.");
				else Render.text("you open your [legs] and wrap them around her head, drawing her face into your stuffed crotch.");
...
//Feed (as female)
		private chicksFeedVapula():void
		{
			...
			if (flags[FlagEnum.VAPULA_TEASE_COUNT] > 1) {
				Render.text("You push the pink end of the dildo into you with a sigh, then strap yourself into the harness so that the monstrous purple end bobs out in front of you.  You stand over a tense Vapula and tease her with it first, lightly brushing her face with the tip for a while.  She swallows thickly and licks her lips as you caress her with your cum fountain, but makes no movements towards it.  Eventually you giggle at her furious concentration and tell her you're going to reward her for being such a good slut, before pushing the end against her lips.  A flood of ecstatic thank-you-mistresses are swiftly muffled out as the dildo disappears into her mouth.");
			}
			else {
				Render.text("You tell Vapula you're going to reward her for being such a good slut; pushing the pink end of the dildo into you with a sigh, you strap yourself into the harness so that the monstrous purple end bobs out in front of you.  You giggle as Vapula grabs hold of it and proceeds to lick it, coating the entirety of your artificial junk with her warm, sweet saliva.");
			}
...
//Forcefeed
		public vapulaForceFeeds():void
		{
			...
			//[if no fuck harem is on and PC hasn't fed/fucked Vapula for 5 days that scene will trigger at night]
				...
				if (player.lowerBody.balls > 0) Render.text("  Your " + ballsDescriptLight() + " churn");
				else Render.text("  Your " + cockDescript(x) + " twitches");
				Render.text(" as you release a sticky spooge fountain in her stomach.");
				...
				if (player.cumQ() >= 1500) Render.text(" until she looks 6 months pregnant.  At last, the cum-flow spilling through your urethra starts to ebb and your " + cockDescript(x) + " stops throbbing; only then does she removes your junk from her mouth with a loud POP.  Your tool appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.");
				player.orgasm();
				dynStats("cor", 1);
				flags[FlagEnum.VAPULA_DAYS_SINCE_FED] = 0;
				flags[FlagEnum.VAPULA_TEASE_COUNT] = 0;
...
//Requires bipedal-ness
		private spankVapulaLikeABoss():void
		{
			...
			if (camp.companionsCount() > 2) Render.text(", drawing the eyes of your other companions.");
			else if (camp.companionsCount() == 2) Render.text(", drawing the attention of your other companion.");
			else Render.text(", echoing off the sentinel-like rocks that seem to ring your encampment.");
			Render.text("  Her tail goes wild at the impact, whipping around in a frenzy and actually managing to slap your cheek!  That bitch!");
			...
			if (player.perks.has("Sadist")) Render.text("  Dishing out all this suffering is getting you a little hot under the collar, and you squirm a bit in place in anticipation of the pain to come.");
