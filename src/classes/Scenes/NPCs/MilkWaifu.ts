export default class MilkWaifu extends NPCAwareContent {

	//New Variables
	//" + Flags.list[FlagEnum.MILK_NAME] + ": This slut's name. Can't call her Bath Slut forever. Unless you do.
	...
	public milkSlave(): boolean {
		return Flags.list[FlagEnum.MILK_NAME] is String;
	}
	...
	private nameZeMilkBath(): void {
		...
		MainScreen.clearText();
		Flags.list[FlagEnum.MILK_NAME] = mainView.nameBox.text;
		mainView.nameBox.text = "";
		...
		//Variable: " + Flags.list[FlagEnum.MILK_NAME] + "
		//Having Named the Girl (Didn't name her Bath Slut)
		else {
			MainScreen.text("You crouch down beside the newly-named girl, brushing a few stray strands of her dark hair from her cheeks.  \"<i>No more Bath Slut.  You're " + Flags.list[FlagEnum.MILK_NAME] + " now.  " + Flags.list[FlagEnum.MILK_NAME] + "'s your name.</i>\"");

			MainScreen.text("\n\n\"<i>" + Flags.list[FlagEnum.MILK_NAME] + "</i>\" she hesitantly repeats.  Looks like she gets it!  \"<i>" + Flags.list[FlagEnum.MILK_NAME] + "!</i>\" she says, more confidently as you encourage her.");
		}
		//Arrival: Part 2 (PC has Rath in camp)
		if (player.statusAffects.has("CampRathazul") && player.statusAffects.get("MetRathazul").value2 >= 4) {
			MainScreen.text("\n\nAs you finish deciding on what to call " + Flags.list[FlagEnum.MILK_NAME] + ", you hear footsteps shuffling over to the two of you.  Looking up, you see the old rat alchemist Rathazul approaching, nose buried in an ancient-looking tome.  \"<i>Good news, [name]!</i>\" he calls, just before tripping over the prone milkmaid, going sprawling across the ground.");

			MainScreen.text("\n\n\"<i>Gah!  Help, I can't get up!</i>\" he shouts, flailing around until you rush over and pull him to his feet.");

			MainScreen.text("\n\n\"<i>Ah, thank you, youngling.  But...  egad, [name], what have you dragged in this time?</i>\" he mumbles, fishing a pair of spectacles out of his pocket to examine " + Flags.list[FlagEnum.MILK_NAME] + ".  \"<i>Why, it's a girl...  a very, um, busty girl at that.  Gods be good, how the devil does she stand with all that...  those...  those things on her chest?</i>\"");

			MainScreen.text("\n\n\"<i>Milk time?</i>\" " + Flags.list[FlagEnum.MILK_NAME] + " mewls, reaching back to rub her full, round ass where Rath bumped into her.  Sheepishly, you explain that she doesn't stand, per se.  You're not sure what the Sand Witches did to her, but she's not quite capable of taking care of herself.");

			MainScreen.text("\n\n\"<i>I see.  Poor dear,</i>\" Rath says, shakily kneeling down beside " + Flags.list[FlagEnum.MILK_NAME] + ".  He brushes her cheek sympathetically, a sorrowful smile playing across his aged features.  \"<i>She's hardly the first to be changed so drastically by these awful times, my friend.  Aside from counseling and lessons, I do not know what I can do to help a mind that's been subjected to the horrors this girl has doubtless seen, but at the very least, my Reducto concoction may be able to help her live with some semblance of normality.</i>\"");
			...
			MainScreen.text("\n\nWith that, Rath hobbles off back to his little laboratory, leaving you with " + Flags.list[FlagEnum.MILK_NAME] + ".");
	...
	public ratducto(): void {
		...
		MainScreen.text("\n\n\"<i>I see.  Poor dear,</i>\" Rath says, shakily kneeling down beside " + Flags.list[FlagEnum.MILK_NAME] + ".  He brushes her cheek sympathetically, a sorrowful smile playing across her aged features.  \"<i>She's hardly the first to be changed so drastically by these awful times, my friend.  Aside from counseling and lessons, I do not know what I can do to help a mind that's been subjected to the horrors this girl has doubtless seen, but at the very least, my Reducto concoction may be able to help her live with some semblance of normality.</i>\"");
	...
	public milkyMenu(): void {
		...
		if (Flags.list[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] == 0) {
			MainScreen.text("You wander over to " + Flags.list[FlagEnum.MILK_NAME] + "'s pool, and find the dusky girl sitting at its rim, ");
			if (Flags.list[FlagEnum.MILK_SIZE] == 0) MainScreen.text("lying face-down on her massive rack, her plump little ass sticking up in the air for all to see.  Seeing you approach, she brightens up and shifts her titanic bust around to face you, all bright-eyes and smiles.  \"<i>Milk time?</i>\"");
			else if (Flags.list[FlagEnum.MILK_SIZE] == 1) MainScreen.text("arms crossed under her hefty, milky bust to support their still-sizable weight.  She smiles as you approach, able to stand up under her own power to give you a hug, milky tits pressed tight against you.  \"<i>I-is it milk time?</i>\" she asks, cupping her tits for you.");
		...
		if (Flags.list[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] == 0 && Flags.list[FlagEnum.FARM_CORRUPTION_STARTED] == 1) {
			if (Flags.list[FlagEnum.MILK_SIZE] == 0) {
		...
		if (Flags.list[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] == 1 && Flags.list[FlagEnum.MILK_SIZE] == 0) {
		...
		if (Flags.list[FlagEnum.MILK_SIZE] < 2 && player.hasKeyItem("Super Reducto") >= 0) MainScreen.addButton(3, "Reducto", superReductoUsage);
		if (Flags.list[FlagEnum.MILK_SIZE] > 0 && player.stats.lust >= 33 && player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(4, "Titfuck", titFuckDatMilkSlut);

		if (Flags.list[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] == 0 && Flags.list[FlagEnum.MILK_SIZE] > 0 && Flags.list[FlagEnum.FARM_CORRUPTION_STARTED] == 1) MainScreen.addButton(5, "Farm Work", sendToFarm);
		if (Flags.list[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] == 1 && Flags.list[FlagEnum.MILK_SIZE] > 0) MainScreen.addButton(5, "Go Camp", backToCamp);

		if (Flags.list[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] == 0) MainScreen.addButton(9, "Back", camp.campSlavesMenu);
	...
	private backToCamp(): void {
		...
		Flags.list[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] = 0;
	...
	private milkWriteFuAppearance(): void {
		MainScreen.clearText();
		MainScreen.text("" + Flags.list[FlagEnum.MILK_NAME] + " is a five-foot-five human female, with darkly tanned skin and ebony hair.  ");
		if (Flags.list[FlagEnum.MILK_SIZE] >= 2) MainScreen.text("Over her supple body, she wears a simple cloth shirt which cuts off just above her knees, hiding her newly-gained modesty, with a pair of silk painties and a simple bra beneath it.  ");
		...
		//{If GIANT boobs: 
		if (Flags.list[FlagEnum.MILK_SIZE] == 0) MainScreen.text("\n\nHer most distinguishing feature, though, is her massive bust.  Enough to drag her down onto all fours without support, " + Flags.list[FlagEnum.MILK_NAME] + "'s rack is absolutely gigantic.  Her areola are the size of plates, constantly dribbling motherly fluids with every breath she takes.  The slightest touch is enough to get a stream flowing from " + Flags.list[FlagEnum.MILK_NAME] + "'s titanic teats, enough to fill her entire pool with more to spare.  Those can't be too comfortable, you reckon, even if they are almost mesmerizing in their beauty and potential for sexual pleasure.");
		else if (Flags.list[FlagEnum.MILK_SIZE] == 1) MainScreen.text("\n\nThough you've reduced them <i>considerably</i>, " + Flags.list[FlagEnum.MILK_NAME] + "'s most eye-catching feature is still her tremendous bosom.  Those huge tits of hers would look more normal on a cowgirl than a human girl, and a rather petite one at that.  Her areola are palm-sized and tipped with a pair of prominent nipples, always eager to release their motherly fluids.  She can easily fill a tub with those things, and she alone could provide enough to feed a small village, producing more than the all the cows of Ingnam combined every day.");
		else {
			MainScreen.text("\n\nThanks to your intervention, " + Flags.list[FlagEnum.MILK_NAME] + "'s breasts aren't so cripplingly large.  Reduced to a DD-cup, you've even managed to scrape together some clothes that actually fit her.  In her current state, you might even call her bust perky, her milk-laden breasts both firm and yet so delightfully soft; you can easily sink your fingers into them, or bury your face");
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" or cock");
			MainScreen.text(" between her pillowy bosoms.  Despite the reduction in size, " + Flags.list[FlagEnum.MILK_NAME] + " is happy to tell you that she's still got plenty of milk inside her, though perhaps not quite as much as before: she can't quite fill the pool all by herself, but now even short milkings leave her blissfully empty for hours afterwards.  She's not an entire farm unto herself now and she seems fairly pleased by this, all things considered.");
		}
		...
		if (kGAMECLASS.farm.farmCorruption.hasTattoo("milky")) {
			...
			if (kGAMECLASS.farm.farmCorruption.milkyFullTribalTats()) {
				...
				if (Flags.list[FlagEnum.MILKY_TATTOO_COLLARBONE] != 0) MainScreen.text(Flags.list[FlagEnum.MILKY_TATTOO_COLLARBONE] + "\n");
				if (Flags.list[FlagEnum.MILKY_TATTOO_SHOULDERS] != 0) MainScreen.text(Flags.list[FlagEnum.MILKY_TATTOO_SHOULDERS] + "\n");
				if (Flags.list[FlagEnum.MILKY_TATTOO_LOWERBACK] != 0) MainScreen.text(Flags.list[FlagEnum.MILKY_TATTOO_LOWERBACK] + "\n");
				if (Flags.list[FlagEnum.MILKY_TATTOO_BUTT] != 0) MainScreen.text(Flags.list[FlagEnum.MILKY_TATTOO_BUTT] + "\n");
			}
		}
	...
	private superReductoUsage(): void {
		...
		MainScreen.text("\n\n\"<i>Come on, you'll feel a lot better afterwards, I promise,</i>\" you say, crouching down beside her, locking onto " + Flags.list[FlagEnum.MILK_NAME] + "'s big brown eyes.");

		MainScreen.text("\n\nIf by nothing else than your soothing tone, " + Flags.list[FlagEnum.MILK_NAME] + " stops fussing about the Reducto and does her best to curl up in your lap, giving you free access to her big ol' rack.  You tussle her hair and start thinking about how to go about this.");
		...
		if (Flags.list[FlagEnum.MILK_SIZE] == 0) {
			...
			MainScreen.text("\n\nYou shift " + Flags.list[FlagEnum.MILK_NAME] + " around in your lap, giving yourself the best angle on her giant tits.  That done, you lather up with Reducto, covering your hands with the stuff before reaching around and grabbing your friend's tits, smearing the first dollops around her leaky nipples.  She winces, shivering as the cool substance smears onto her teats, but almost immediately you can see it going to work: her breasts shudder, flesh quivering as they begin to shrink like balloons.");
			MainScreen.text("\n\n\"<i>[name]!</i>\" " + Flags.list[FlagEnum.MILK_NAME] + " cries out, whimpering and moaning as you massage her tits, rubbing a good amount of reducto all around her bust.  Milk pours out of her like a pair of hoses, spraying her motherly fluids everywhere as her carrying capacity shrinks and shrinks.  Down and down she goes, breasts shrinking and leaking until they're about HHH-cups - big enough for a cowgirl, but a lot smaller than they were.");
			MainScreen.text("\n\n\"<i>That's enough,</i>\" you say, leaning around to put your hands into the spray of milk " + Flags.list[FlagEnum.MILK_NAME] + "'s making, washing the reducto from your hands.  When you're done, you wipe her teats off with spare milk, rubbing off the reducto until her dusky skin is as bare as can be, nice and shiny.");
			MainScreen.text("\n\nExperimentally, " + Flags.list[FlagEnum.MILK_NAME] + " tries to stand.  She nearly falls forward, leaden down by her still-prodigious bosoms, but you catch her, steadying the lightened girl until she's standing on her own power, swaying slightly, but standing.");

			MainScreen.text("\n\n\"<i>I...  I can...</i>\" she groans, suddenly clutching her head.  You jump up, steadying her again as " + Flags.list[FlagEnum.MILK_NAME] + " moans in pain, tugging at her dark hair until suddenly, she's silent.  A long moment passes before she turns to you, all smiles.  \"<i>I can...  walk,</i>\" she says, struggling to form the words.  Before you can react, " + Flags.list[FlagEnum.MILK_NAME] + " grabs you, hugging you into her hefty bosom as hard as she can, nearly crying with joy.  Laughing, you hug her back, holding her milk-soaked body tight until she's ready to stand.  Giving you a long, loving kiss, the girl stumbles off, trying to get her land legs back after being on all fours for so long.");
		}
		else {
			MainScreen.text("\n\n" + Flags.list[FlagEnum.MILK_NAME] + "'s got cowgirl-sized tits right now");
			...
			MainScreen.text("\n\n\"<i>[name]!</i>\" " + Flags.list[FlagEnum.MILK_NAME] + " cries out, whimpering and moaning as you massage her tits, rubbing a goodly amount of reducto all around her bust.  Milk pours out of her like a pair of hoses, spraying her motherly fluids everywhere as her carrying capacity shrinks and shrinks.  Down and down she goes, breasts shrinking and leaking more and more.  You smear more Reducto on her, using every last drop you've got to help the poor milky slave girl.  She cries out again as her bust seems to explode with milk, a white tidal wave pouring from her shrinking nipples, nearly blinding you.");

			MainScreen.text("\n\nBy the time you can see again, you and " + Flags.list[FlagEnum.MILK_NAME] + " are whiter than snow, completely covered in milk.  You look down, and see that your hands are cupping a beautiful, firm pair of DD breasts.  She's stopped shrinking right where you predicted, leaving her with a shapely bust that any girl could be proud of.  And more importantly, one that she could easily move around with.  Wiping the milk from your eyes, you stroke " + Flags.list[FlagEnum.MILK_NAME] + "'s hair, gently urging her to try and stand.  Shakily, she allows herself to be stood up, and you let her go.");

			MainScreen.text("\n\n" + Flags.list[FlagEnum.MILK_NAME] + " nearly falls, but catches herself at the last moment, waving her arms like a bird's to steady herself as she adjusts to her new center of gravity.  She takes a few experimental steps, and finds herself relatively unburdened.  Smiling from ear to ear, the girl turns to you and shouts out, \"<i>I...  I can WALK!</i>\" before leaping into your arms, crying tears of joy as she holds you tight against herself, perky tits dribbling milk onto your chest.  It seems no matter how flat she gets, " + Flags.list[FlagEnum.MILK_NAME] + " can't stop producing milk; but now she doesn't have those massive tits to deal with, and for now, that's enough.  The poor slave girl nuzzles her cheek into your neck, shivering as she sobs, fingers clutching at your [armor].");

			MainScreen.text("\n\nYou let her cry it out for a good long while, stroking her hair and whispering soothing words until finally, she steps back, drying her eyes.  " + Flags.list[FlagEnum.MILK_NAME] + " rubs her temples, groaning with a brief moment of pain before looking up at you with bright brown eyes and whispering, \"<i>Thank you...  so very much, [name].</i>\"");
	...
	private milkBathTime(): void {
		MainScreen.clearText();
		MainScreen.text("Tussling " + Flags.list[FlagEnum.MILK_NAME] + "'s hair, you tell her her breasts look a bit full.  She smiles up at you eagerly as you disrobe.  Once nude, you hop down inside and say, \"<i>Bath Time.</i>\"");
		MainScreen.text("\n\nWith trembling anticipation, " + Flags.list[FlagEnum.MILK_NAME] + " reaches down for her ");
		if (Flags.list[FlagEnum.MILK_SIZE] == 0) MainScreen.text("teat-like");
		...
		if (Flags.list[FlagEnum.MILK_SIZE] == 0) MainScreen.text("gigantic");
		...
		else if (player.gender == 3) MainScreen.text(CockDescriptor.describeMultiCockShort(player) + " and [vagina]");
		...
		MainScreen.text("\n\n" + Flags.list[FlagEnum.MILK_NAME] + " pulls back, licking her puffy lips and smelling strongly of female arousal, though obviously satisfied.  She whimpers, \"<i>I love bath time,</i>\" before ");
		if (Flags.list[FlagEnum.MILK_SIZE] == 0) MainScreen.text("starting to shift her breasts' bulk toward a comfortable spot for a nap");
		else MainScreen.text("struggling to her feet, legs shaky with the afterglow of a might boobgasm");
		MainScreen.text(".");
		if (player.stats.lust >= 33) MainScreen.text("  You could probably masturbate in the tub if you wanted to, or maybe pull the dusky milk-maid in for company.");
		...
		if (Flags.list[FlagEnum.VALARIA_AT_CAMP] == 1) count++;
		if (amilyScene.amilyFollower() && !amilyScene.amilyCorrupt()) count++;
		if (helScene.followerHel()) count++;
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00238] == 1) count++;
	...
	private fuckTheMilkWaifu(): void {
		...
		MainScreen.text("\n\nSubmerged beneath a sea of creamy milk, it's so very, very easy to slide into " + Flags.list[FlagEnum.MILK_NAME] + ", ");
	...
	private drinkAndFapturbate(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>Wait,</i>\" you call out to the ebony woman, letting the milk obscure your hands as you begin to masturbate, \"I want a drink.</i>\"  Sheepishly, " + Flags.list[FlagEnum.MILK_NAME] + " obligingly shifts back toward you, presenting her huge teats.");
		...
		if (Flags.list[FlagEnum.MILK_SIZE] == 0) MainScreen.text("a dinner plate");
		...
		MainScreen.text("\n\nAn excited moan worms out of " + Flags.list[FlagEnum.MILK_NAME] + "'s puffy lips, a testament to the raw sensitivity of her milk-bloated jugs.  As your tongue swirls over the leaky nozzle's pebbly skin, she releases another breathy pant of delight.  The vocal tremors seem to coo all the way down to your loins, joining with your fingers' caresses to stir you to aching, trembling arousal.  ");
		//{Fork, no new PG}
		//(DA HERMS)
		if (player.gender == 3) {
		...
		if (Flags.list[FlagEnum.MILK_SIZE] == 0) MainScreen.text("drags her gigantic tits over the puddly, milk-slicked floor");
	...
	private nyanCatMilkTime(): void {
		...
		if (Flags.list[FlagEnum.MILK_SIZE] == 1) {
			MainScreen.text("\"<i>Milk time!</i>\" you say, giving " + Flags.list[FlagEnum.MILK_NAME] + " a playful swat on her upturned ass, walking around her to the pool.  You jump in and toss your [armor] back out as the milky witch gets situated, well aware of just how much milk this poor girl's about to blast you with, even with her much reduced bosom.  Still, she's gotta be milked, and you're thirsty.  Milk time indeed.");

			MainScreen.text("\n\nOnce you've disrobed, and " + Flags.list[FlagEnum.MILK_NAME] + "'s got her giant jugs situated at the edge of the pool, you get to work.  Rubbing your hands together, you place each over top one of her palm-sized areola, gently running your fingers around the bases of her teat-like nipples, though with every motion your hands seem to be engorged into the massive mounds, sucked into a blackhole of titflesh.  With every touch, though, " + Flags.list[FlagEnum.MILK_NAME] + " shudders and moans, biting her lower lip as her arms quiver with pleasure.");

			MainScreen.text("\n\nIn a matter of moments, you coax out the first explosive spurt of milk from your friend's teats, two hot streams of white cream blasting you in the face and drenching you before you can blink.  \"<i>S-sorryyyyyyyy,</i>\" she whines, voice turning into an ecstatic moan as you grip her nipples and start working vigorously, not letting the milky stream die down for an instant.  " + Flags.list[FlagEnum.MILK_NAME] + " moans and groans as you roughly milk her, rewarded with gallon after gallon of delightful motherly milk.");

			//If PC has Smart/Normal Sophie:
			if (sophieFollowerScene.sophieFollower()) MainScreen.text("\n\nAs you milk " + Flags.list[FlagEnum.MILK_NAME] + ", you see a shadow flash overhead.  You look up in time to see your harpy broodmother perching at the rim of the pool, just beside " + Flags.list[FlagEnum.MILK_NAME] + "'s quivering body.  \"<i>Fresh milk!</i>\" Sophie laughs, cupping up a handful and bringing it to her mouth.  \"<i>You don't mind if Momma Sophie has a taste, do you honey?</i>\"\n\nThe milky girl shakes her head, barely paying attention as Sophie flops down beside her, avian tongue lapping at the stream pouring down into the pool.");
			...
			if (isabellaFollowerScene.isabellaFollower()) {
				...
				MainScreen.text(" done.</i>\"  You find yourself roughly pushed aside as Isabella jumps in with you, tossing her skirt and blouse aside as she grasps " + Flags.list[FlagEnum.MILK_NAME] + "'s teats, massaging them much more gently.  The milk girl's eyes roll back in her head as Isabella sets to work, though it gets harder and harder as the milk flows up to her thighs and chest.");
			}

			//If PC has ONLY KIha or if PC dun have Izzy, Sophie, OR Kiha: 
			if (!isabellaFollowerScene.isabellaFollower() || !sophieFollowerScene.sophieFollower()) MainScreen.text("\n\nIt takes a good long while to get " + Flags.list[FlagEnum.MILK_NAME] + "'s bloated tits under control, but nearly an hour later you've milked her as well as you can for now.  Sopping wet, you pull yourself out of the pool and grab a towel.  With her tits lightened for the moment, " + Flags.list[FlagEnum.MILK_NAME] + " reaches up and pulls you down to her, just long enough to plant a kiss on your cheek and whisper, \"<i>Thank you, [name].  That felt good.</i>\"");

			//If PC has Sophie or Isabella:
			else MainScreen.text("\n\nThough you didn't intend for this to turn communal, " + Flags.list[FlagEnum.MILK_NAME] + " certainly has more than enough to share.  You relax as the last trickles of milk pour into the pool, her breasts looking positively deflated.  You decide to leave the plug in for your friends as you clamber out, figuring they'll want to stockpile a little for the day before you drain it.  With her tits lightened for the moment, " + Flags.list[FlagEnum.MILK_NAME] + " reaches up and pulls you down to her, just long enough to plant a kiss on your cheek and whisper, \"<i>Thank you, [name].  That felt good.</i>\"");
			fatigue(-50);
			dynStats("lus", 10 + player.stats.sens / 10, "resisted", false);
			doNext(camp.returnToCampUseOneHour);
		}
		//Milk Time!  (DD Boobs Ver.)
			...
			MainScreen.text("You sit yourself down with " + Flags.list[FlagEnum.MILK_NAME] + " and ask if she'd like a good milking.  Her eyes brighten at the suggestion, and she whispers \"<i>Yes please,</i>\" already pulling off her shirt.  You help her get her top and bra off, leaving her full, perky breasts bare in the cool sun.  You slip behind her, running your hands across her sun-kissed skin, tracing your fingers from hips to chest; " + Flags.list[FlagEnum.MILK_NAME] + " shivers at your slightest touch, whining lustily as your hands wrap around her milk-laden mounds.  Her reddened nipples are rock-hard by the time your fingers brush up against them, perspiring clear pearlescent drops as you circle her wide areola.");
			MainScreen.text("\n\n" + Flags.list[FlagEnum.MILK_NAME] + " whimpers as you squeeze a tiny trickle from her teats, the white fluid squirting out in a powerful arc, staining the campground for the briefest moment before the hungry earth swallows it up, leaving no trace but a darkened patch at the girl's knees.  Gently, you start to squeeze and knead " + Flags.list[FlagEnum.MILK_NAME] + "'s breasts, holding her back against you as you massage the heavy burden from her seemingly unending reserves.  A steady stream of milk pours from her chest, dribbling down between your fingers and her flat belly.  Soon a pool's formed around your [legs], the barren earth not nearly fast enough to consume the bounty flowing from your friend.");

			MainScreen.text("\n\nWith each caress and squeeze, " + Flags.list[FlagEnum.MILK_NAME] + " shivers and shakes, her chest soon heaving with her ragged, pleasure-strained breath.  You can see her lustful secretions stain her paintes as her hidden cunt quivers, desperate for attention as you milk her over-sensitive mammaries.  Her breathing becomes more irregular, her long legs quaking and eyes rolling back as you squeeze out gallon after gallon.  Her whimpers of pleasure become outright cries, piercing the quiet of camp with orgasmic moans and shrill screams as your fingers work their magic on her, sending shockwaves of pleasure from hefty teats to sex-addled mind  that make her entire body convulse and writhe at your touch.");
	...
	private suckleDatMilk(): void {
		...
		MainScreen.text("\n\nOver a few short minutes, " + Flags.list[FlagEnum.MILK_NAME] + "'s breath becomes increasingly erratic, her chest heaving hard, pushing your face from her with every heavy breath she takes.  The girl's voice soon gives out, giving way to husky moans and whispers of praise to your oral skills, thanking you again and again for the gentle, tender release you're bringing her to.  She runs her fingers through your hair as her body undulates atop you, responding to every suckle and caress with more milk and shudders of pleasure.");

		MainScreen.text("\n\nHer orgasm is as powerful as it is inevitable.  " + Flags.list[FlagEnum.MILK_NAME] + " throws her head back, a silent cry escaping her lips as her fingers dig into your flesh, gripping you as tight as she can as the stream of milk passing your lips grows and grows and grows in power until you're nearly drowning.  White rivers from your mouth as you struggle to keep up with the unforgiving flow, swallowing fast to keep from drowning in her orgasmic release.  " + Flags.list[FlagEnum.MILK_NAME] + " cries and squeals as her body is rocked by boobgasm, the breast not firmly in your grasp spraying wildly, creating a lake around the two of you.  You can see her thin panties darken with fem-cum, a few of her fingers darting down to rub her hidden nub, masturbating to the rhythm of the cream spurting from her breasts.");

		MainScreen.text("\n\nEventually, the boobgasm subsides, leaving " + Flags.list[FlagEnum.MILK_NAME] + " a quivering, panting pile of lust in your arms, her fingers absently rubbing through her panties as the last dribbles of milk trickle down her chest and your chin.  Taking the first deep breath you've managed to get in the last few minutes, you grab a towel from nearby and try to dry yourself and the leaky girl off as best you can, brushing off the gallons of milk that have washed over you both.  When you're done, " + Flags.list[FlagEnum.MILK_NAME] + " leans over and plants a long, lusty kiss on your lips, her tongue lapping up little drops of her own milk still inside your mouth.  She breaks the kiss after a long, pleasant moment, whispering \"<i>Thank you, [name].</i>\"");
		fatigue(-50);
		dynStats("lus", 10 + player.stats.sens / 10, "resisted", false);
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private titFuckDatMilkSlut(): void {
		MainScreen.clearText();
		MainScreen.text("You sit down beside " + Flags.list[FlagEnum.MILK_NAME] + " and give her a little push onto her back.  A tiny gasp escapes her lips, but is silenced as you move over top her and giver her a long, tongue-filled kiss.  Her surprise turns into a lusty moan as you ");
		if (Flags.list[FlagEnum.MILK_SIZE] == 2) MainScreen.text("pull off her shirt and bra");
		else MainScreen.text("sink your fingers into her udder-like teats");
		MainScreen.text(", caressing her sensitive breasts until a gentle stream of milk runs down her chest.  Her breath catches in anticipation as you back off a moment, freeing your [cock] from your [armor] and letting it flop onto her bare belly.  However, your hips slide forward away from her already-wet cunt and towards the enticing valley of her cleavage.  To your delight, the attention you're about to lavish on her breasts seems to excite " + Flags.list[FlagEnum.MILK_NAME] + " as much as a good fucking, and her breath picks up excitedly as your cock slips between her teats.  With a little urging, she cups her breasts and squeezes them together around your rock-hard member, enveloping it in the warm embrace of her dusky flesh.");

		MainScreen.text("\n\nBefore you can start to thrust into her chest, you reach down and pinch her nipples, rolling the two prominent peaks between your fingers, coaxing out a bigger and bigger flood of milk from her deep reserves.  White rivers run down from her mounds, a deluge flooding down into the valley around your [cock].  You shudder as the warm, thick cream pours in around your member.  " + Flags.list[FlagEnum.MILK_NAME] + " reaches in and rubs it into your cockflesh with slow, sensuous motions, gently stroking you off as more and more whiteness submerges your wang. Overcome by your lactic lust, you lean in and lap up the run-off from the lake of milk between " + Flags.list[FlagEnum.MILK_NAME] + "'s leaky breasts.  Your senses hum at the of taste sweet cream tinted with lust, and as if on their own accord, your [hips] start to slide forward, your [cock] gliding between her tits like it would a well-lubed cunt.  " + Flags.list[FlagEnum.MILK_NAME] + " giggles at the sensation, squeezing her tits tightly together around your wang.");

		MainScreen.text("\n\nYou grab her shoulders for support and start to really get going.  Your hips move faster and faster, groin pounding into the undersides of " + Flags.list[FlagEnum.MILK_NAME] + "'s tits as you hammer into her milk-drenched cleavage.  She feels just as good as a warm, wet pussy around your cock, but with an added bonus: as the [cockHead] of your [cock] peaks out between the tops of her breasts, " + Flags.list[FlagEnum.MILK_NAME] + " leans her head up and wraps her full lips around it, her tongue flicking across the slit of your urethra and sending shivers of pleasure down your spine.  A low, husky moan escapes you as " + Flags.list[FlagEnum.MILK_NAME] + " sucks your cock at the crest of every thrust, putting all her suckling skill to work on your brim until your [cock] feels practically afire with pleasure.");

		MainScreen.text("\n\nCoaxed on by " + Flags.list[FlagEnum.MILK_NAME] + "'s oral ministrations, you abandon her breasts for a moment, ");
		//If not naga:
		if (!player.lowerBody.isNaga()) MainScreen.text("straddling her shoulders and shoving");
		else MainScreen.text("wrapping yourself around her to shove");
		MainScreen.text(" your [cock] deep into her mouth.  " + Flags.list[FlagEnum.MILK_NAME] + " accepts it eagerly, opening wide as you stuff inch after inch of your manhood down her throat.  Now she can really go to work: her lips wrap tight around your base, tongue lapping at your underside like candy as her throat muscles massage the shaft.  You slide in and out, watching her throat bulge and contract with each thrust");
		if (player.lowerBody.balls > 0) MainScreen.text(", shivering as her tongue darts out around your cock to flick your nads");
		MainScreen.text(".");

		MainScreen.text("\n\nJust as soon as you're settling into a rhythm, though, " + Flags.list[FlagEnum.MILK_NAME] + " surprises you with a sudden shift, rolling over onto all fours and taking your with her.  You flop onto your back, yelping in surprise as the busty maid tops you, her cheek resting in your thigh.  You start to protest, but are quickly silenced as her tongue wraps around the base of your cock, sliding up your fuckpole with languid ease.  You shudder and relax, content to let her put her tremendous oral proficiency to good use; and use it she does, soon getting onto all fours over your [cock], dragging her huge tits across the upright length until the sheer weight of her rack weighs your mighty penis down, pinning it to your belly.  Gently, she rocks her body forward, dragging her well-lubed tits up and down your length, only saving the [cockHead] for her mouth.  She leans down until you can feel every hot breath on your sensitive skin.  " + Flags.list[FlagEnum.MILK_NAME] + " takes her time, only slowly rolling her tongue around your crown, flicking her tip across your urethra and lapping up the pre that's now spurting liberally from your [cock].  You groan and clench your fists, trying to bear through the combination titfuck and blowjob, but you just can't withstand her ministrations.");

		MainScreen.text("\n\nWith one last surge of sexual energy, you wrap your [legs] around " + Flags.list[FlagEnum.MILK_NAME] + "'s shoulders and roll her over again, putting yourself back on top with cock held firmly between her hefty tits.  You jackhammer your [hips] into her underboobs, digging your fingers into her soft flesh and savaging her nipples until she's breathing hard and moaning, milk sloshing down into her cleavage again to perfectly lubricate your last thrusts until, with a feral roar, the first thick globs of your creamy spunk burst forth, mixing momentarily with the sea of milk around your [cock] before spattering onto " + Flags.list[FlagEnum.MILK_NAME] + "'s neck, giving her an instant pearl necklace.  Your cock spasms again and again, shooting jizz into the lake of milk until her breast-valley is a frothy admixture of cum and cream around your prick.  With a mischievous grin, you reach in and scoop up a double-handful of the solution you've made, and hold it up to " + Flags.list[FlagEnum.MILK_NAME] + "'s lips.  She smiles lustily up at you before suckling it all up, drinking every offered drop of sweet cream and seed.");

		MainScreen.text("\n\nYou sigh contentedly and give " + Flags.list[FlagEnum.MILK_NAME] + " an affectionate pat on the boob, watching as her flesh quivers at your touch.  She smiles at you with a lusty warmth, blowing you a kiss as you grab a towel to dry yourself off; you look back to see her happily masterbating, finishing the job you started with one hand knuckle-deep inside her as the other idly plays with the frothy cream you've left between her boobs.");
		player.orgasm();
		player.stats.sens += -1;
		fatigue(-10);
		doNext(camp.returnToCampUseOneHour);
