...
private function nameZeMilkBath():void 
{
	...
	//Having Named the Girl (Didn't name her Bath Slut)
		...
		outputText("You crouch down beside the newly-named girl, brushing a few stray strands of her dark hair from her cheeks.  \"<i>No more Bath Slut.  You're " + flags[FlagEnum.MILK_NAME] + " now.  " + flags[FlagEnum.MILK_NAME] + "'s your name.</i>\"");
...
public function milkyMenu():void {
	...
	{
		outputText("You wander over to " + flags[FlagEnum.MILK_NAME] + "'s pool, and find the dusky girl sitting at its rim, ");
		if(flags[FlagEnum.MILK_SIZE] == 0) outputText("lying face-down on her massive rack, her plump little ass sticking up in the air for all to see.  Seeing you approach, she brightens up and shifts her titanic bust around to face you, all bright-eyes and smiles.  \"<i>Milk time?</i>\"");
		else if(flags[FlagEnum.MILK_SIZE] == 1) outputText("arms crossed under her hefty, milky bust to support their still-sizable weight.  She smiles as you approach, able to stand up under her own power to give you a hug, milky tits pressed tight against you.  \"<i>I-is it milk time?</i>\" she asks, cupping her tits for you.");
...
private function milkWriteFuAppearance():void {
	clearOutput();
	outputText("" + flags[FlagEnum.MILK_NAME] + " is a five-foot-five human female, with darkly tanned skin and ebony hair.  ");
	if(flags[FlagEnum.MILK_SIZE] >= 2) outputText("Over her supple body, she wears a simple cloth shirt which cuts off just above her knees, hiding her newly-gained modesty, with a pair of silk painties and a simple bra beneath it.  ");
	...
	if (kGAMECLASS.farm.farmCorruption.hasTattoo("milky"))
	{
		...
		else
		{
			...
			if (flags[FlagEnum.MILKY_TATTOO_COLLARBONE] != 0) outputText(flags[FlagEnum.MILKY_TATTOO_COLLARBONE] + "\n");
			if (flags[FlagEnum.MILKY_TATTOO_SHOULDERS] != 0) outputText(flags[FlagEnum.MILKY_TATTOO_SHOULDERS] + "\n");
			if (flags[FlagEnum.MILKY_TATTOO_LOWERBACK] != 0) outputText(flags[FlagEnum.MILKY_TATTOO_LOWERBACK] + "\n");
			if (flags[FlagEnum.MILKY_TATTOO_BUTT] != 0) outputText(flags[FlagEnum.MILKY_TATTOO_BUTT] + "\n");
		}
	}
...
private function superReductoUsage():void {
	clearOutput();
	outputText("You tell " + flags[FlagEnum.MILK_NAME] + " that it's about time she got rid of those massive tits of hers.  She cocks her head to the side, looking at you without comprehension.  You fish the giant vial of Reducto Rath gave you from your pack and hold it up for her to examine.  She hesitantly takes it from your hand and rolls it around, sniffing at the foul-smelling stuff.  She turns up her nose at it.");
	...
	if(flags[FlagEnum.MILK_SIZE] == 0) {
		...
		if(isabellaFollowerScene.isabellaFollower()) {
			...
			if(player.statusAffects.has("CampMarble")) outputText(" and Marble");
			outputText(" pause");
		}
		else if(player.statusAffects.has("CampMarble")) outputText(", enough to give Marble pause");
...
private function milkBathTime():void {
	clearOutput();
	outputText("Tussling " + flags[FlagEnum.MILK_NAME] + "'s hair, you tell her her breasts look a bit full.  She smiles up at you eagerly as you disrobe.  Once nude, you hop down inside and say, \"<i>Bath Time.</i>\"");
	outputText("\n\nWith trembling anticipation, " + flags[FlagEnum.MILK_NAME] + " reaches down for her ");
	if(flags[FlagEnum.MILK_SIZE] == 0) outputText("teat-like");
	...
	if(flags[FlagEnum.MILK_SIZE] == 0) outputText("gigantic");
	...
	if(flags[FlagEnum.MILK_SIZE] == 0) outputText("starting to shift her breasts' bulk toward a comfortable spot for a nap");
...
private function relaxWithMilkWaifu():void {
	clearOutput();
	outputText("You sit in the tub for a while, letting the fragrant fluids soak into your " + player.skinFurScales() + ".  You spend the better part of an hour lounging, letting your cares float away in " + flags[FlagEnum.MILK_NAME] + "'s endless white bounty.  Finally though, you pull out the tub's plug and climb out, finding a towel nearby.  Thankfully, the milk doesn't seem to leave behind any residue, and you feel clean and refreshed, if a bit horny.");
	//(+Lust, -Fatigue)
...
private function pullInZeMilkGirl():void {
	clearOutput();
	outputText("You call out to " + flags[FlagEnum.MILK_NAME] + " before she can wander away, and wade over to the side of the tub, leaving your face a few scant inches from her hefty milkers.  She smiles as you reach up, nuzzling her cheek into your hand, purring your name happily.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 7) outputText(", pressing deep into your own huge rack, your nipples flicking across her own leaky tits");
	outputText(".");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.balls > 0) outputText(", her other hand cupping your [balls], rolling the " + num2Text(player.lowerBody.balls) + " orbs in her palm with delightful dexterity");
...
private function drinkAndFapturbate():void {
	clearOutput();
	outputText("\"<i>Wait,</i>\" you call out to the ebony woman, letting the milk obscure your hands as you begin to masturbate, \"I want a drink.</i>\"  Sheepishly, " + flags[FlagEnum.MILK_NAME] + " obligingly shifts back toward you, presenting her huge teats.");
	...
	if(flags[FlagEnum.MILK_SIZE] == 0) outputText("a dinner plate");
	...
	//(DA DUDES)
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("  You make sure to fondle each of your members equally, caressing, squeezing, and stroking to the tempo of your swelling passion.");
		if(player.lowerBody.balls > 0) outputText("  With your offhand, you cradle your [sack], hefting your [balls] as you feel your desire churning to new levels.");
	}
	//(TOGETHER)
	...
	if(flags[FlagEnum.MILK_SIZE] == 0) outputText("drags her gigantic tits over the puddly, milk-slicked floor");
...
private function nyanCatMilkTime():void {
	...
	{
		outputText("\"<i>Milk time!</i>\" you say, giving " + flags[FlagEnum.MILK_NAME] + " a playful swat on her upturned ass, walking around her to the pool.  You jump in and toss your [armor] back out as the milky witch gets situated, well aware of just how much milk this poor girl's about to blast you with, even with her much reduced bosom.  Still, she's gotta be milked, and you're thirsty.  Milk time indeed.");
		...
		if(isabellaFollowerScene.isabellaFollower()) {
			...
			outputText(" done.</i>\"  You find yourself roughly pushed aside as Isabella jumps in with you, tossing her skirt and blouse aside as she grasps " + flags[FlagEnum.MILK_NAME] + "'s teats, massaging them much more gently.  The milk girl's eyes roll back in her head as Isabella sets to work, though it gets harder and harder as the milk flows up to her thighs and chest.");
		}
	...
	//Milk Time!  (DD Boobs Ver.)
		...
		if(player.cockThatFits(50) >= 0 && player.lowerBody.cockSpot.hasCock()) outputText("  Then again, maybe you could hike her panties down and give her a good fucking to send her over the edge.  In her state, she certainly won't mind!");
		menu();
...
private function titFuckDatMilkSlut():void {
	clearOutput();
	outputText("You sit down beside " + flags[FlagEnum.MILK_NAME] + " and give her a little push onto her back.  A tiny gasp escapes her lips, but is silenced as you move over top her and giver her a long, tongue-filled kiss.  Her surprise turns into a lusty moan as you ");
	if(flags[FlagEnum.MILK_SIZE] == 2) outputText("pull off her shirt and bra");
