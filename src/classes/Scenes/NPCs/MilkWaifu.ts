...
private nameZeMilkBath():void 
{
	...
	{
		clearOutput();
		Render.text("<b>You must give her a name.</b>", false);
		menu();
		addButton(0,"Next",nameZeMilkBath);
	...
	if(mainView.nameBox.text == "Bath Slut") Render.text("Fuck it, Bath Slut it is.  At least she won't get confused.");
	else if(mainView.nameBox.text == "Biscuit") Render.text("Fuck it, you may as well butter her buns!");
	//Variable: " + flags[FlagEnum.MILK_NAME] + "
	//Having Named the Girl (Didn't name her Bath Slut)
	else {
		Render.text("You crouch down beside the newly-named girl, brushing a few stray strands of her dark hair from her cheeks.  \"<i>No more Bath Slut.  You're " + flags[FlagEnum.MILK_NAME] + " now.  " + flags[FlagEnum.MILK_NAME] + "'s your name.</i>\"");
...
public milkyMenu():void {
	...
	{
		Render.text("You wander over to " + flags[FlagEnum.MILK_NAME] + "'s pool, and find the dusky girl sitting at its rim, ");
		if(flags[FlagEnum.MILK_SIZE] == 0) Render.text("lying face-down on her massive rack, her plump little ass sticking up in the air for all to see.  Seeing you approach, she brightens up and shifts her titanic bust around to face you, all bright-eyes and smiles.  \"<i>Milk time?</i>\"");
		else if(flags[FlagEnum.MILK_SIZE] == 1) Render.text("arms crossed under her hefty, milky bust to support their still-sizable weight.  She smiles as you approach, able to stand up under her own power to give you a hug, milky tits pressed tight against you.  \"<i>I-is it milk time?</i>\" she asks, cupping her tits for you.");
		else Render.text("her long, tanned legs dangling into the tub.  She jumps up when you approach, and though still a bit unsteady with such easy movements, she's quick to leap into your arms, nuzzling into your neck and pressing her firm, perky DD-cup breasts against you, little trickles of milk staining your chest through the little shirt she's wearing.  \"<i>[name],</i>\" she purrs happily.");
	}
	else
	{
		if (flags[FlagEnum.MILK_SIZE] == 0)
		{
			Render.text("[bathgirlname] shakes vaguely out of her boob daze as you pick your way over to her.");

			Render.text("\n\n“<i>Bath time?</i>”");
		}
		else
		{
			Render.text("[bathgirlname] smiles at you as you pick your way over to her.");
	...
	if(flags[FlagEnum.MILK_SIZE] > 0) addButton(1,"Milk Time!",nyanCatMilkTime);
	if(flags[FlagEnum.MILK_SIZE] < 2) addButton(2,"Milk Bath",milkBathTime);
	if(flags[FlagEnum.MILK_SIZE] < 2 && player.hasKeyItem("Super Reducto") >= 0) addButton(3,"Reducto",superReductoUsage);
	if(flags[FlagEnum.MILK_SIZE] > 0 && player.lust >= 33 && player.lowerBody.cockSpot.hasCock()) addButton(4,"Titfuck",titFuckDatMilkSlut);
	
	if (flags[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] == 0 && flags[FlagEnum.MILK_SIZE] > 0 && flags[FlagEnum.FARM_CORRUPTION_STARTED] == 1) addButton(5, "Farm Work", sendToFarm);
	if (flags[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] == 1 && flags[FlagEnum.MILK_SIZE] > 0) addButton(5, "Go Camp", backToCamp);
	
	if (flags[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] == 0) addButton(9, "Back", camp.campSlavesMenu);
...
private sendToFarm():void
{
	...
	Render.text("You describe to [bathgirlname] the lake, and the farm which is situated close to it. Gently you tell her you want her to go there, present herself to the dog woman who owns it, and do as she says.");

	Render.text("“<i>Ok,</i>” says [bathgirlname], cautiously testing the idea out. “<i>You’ll come and visit sometimes, right?</i>” Of course. Mollified, the former sand witch slave gets to her feet and cautiously picks her way towards the lake. She won’t be much use protection-wise but she’ll give your milk production a boost.");
...
private backToCamp():void
{
	...
	//TODO
	Render.text("“<i>I want you to head on back to camp,</i>” you tell her. “<i>You’ll be more useful to me there.</i>” [bathgirlName]’s brow crinkles but she seems to accept your instruction.");

	Render.text("“<i>As you wish.</i>” She wipes her hands before walking slowly down and out of the farm’s gate.");
...
private milkWriteFuAppearance():void {
	...
	if (kGAMECLASS.farm.farmCorruption.hasTattoo("milky"))
	{
		...
		{
			Render.text("She is covered from head to tail in tattooed butterflies, as if the pretty insects are attracted to her chocolate skin. When she moves she does it with an extra bounce and flick of the head, admiring how she looks as she goes.");
		}
		else
		{
			if (kGAMECLASS.farm.farmCorruption.numTattoos("milky") > 1) Render.text("She has the following tattoos emblazoned across her body:\n");
			else Render.text("She has ");

			if (flags[FlagEnum.MILKY_TATTOO_COLLARBONE] != 0) Render.text(flags[FlagEnum.MILKY_TATTOO_COLLARBONE] + "\n");
			if (flags[FlagEnum.MILKY_TATTOO_SHOULDERS] != 0) Render.text(flags[FlagEnum.MILKY_TATTOO_SHOULDERS] + "\n");
			if (flags[FlagEnum.MILKY_TATTOO_LOWERBACK] != 0) Render.text(flags[FlagEnum.MILKY_TATTOO_LOWERBACK] + "\n");
			if (flags[FlagEnum.MILKY_TATTOO_BUTT] != 0) Render.text(flags[FlagEnum.MILKY_TATTOO_BUTT] + "\n");
		}
	}
...
private milkBathTime():void {
	...
	if(flags[FlagEnum.MILK_SIZE] == 0) Render.text("teat-like");
	else Render.text("prominent");
	Render.text(" nipples.  Her areola bead with white perspiration in anticipation, and the woman's hands eagerly set to work on them, starting to draw out the first hints of lactation, pouring them into the pool around you.  You drop the plug into the drain and look up.  Cooing in delight, the huge-breasted girl finally manages to get her shivering fingertips around each of her aching milk-spouts.  She massages her nipple-flesh for a moment, her eyes lidded and heavy from pleasure, and she releases the first heavy torrent of white into the tub.  As she milks herself, " + flags[FlagEnum.MILK_NAME] + "'s eyes seem to go vacant, overwhelmed by pleasure, and her mouth is too busy making sighs of relief to speak.");
	...
	if(flags[FlagEnum.MILK_SIZE] == 0) Render.text("gigantic");
	else Render.text("cow-like");
	Render.text(" breasts as you watch, and you have to admit, you feel a sexual thrill sliding down your spine as you watch her heafty bosoms work to fill your tub.");
	...
	else if(player.gender == 3) Render.text(multiCockDescriptLight() + " and [vagina]");
	...
	if(flags[FlagEnum.MILK_SIZE] == 0) Render.text("starting to shift her breasts' bulk toward a comfortable spot for a nap");
	else Render.text("struggling to her feet, legs shaky with the afterglow of a might boobgasm");
	Render.text(".");
	if(player.lust >= 33) Render.text("  You could probably masturbate in the tub if you wanted to, or maybe pull the dusky milk-maid in for company.");
	...
	if(count >= 3) {
		Render.text("  Then again, if you're getting a bath, maybe the rest of your friends might like to join you?");
...
private pullInZeMilkGirl():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 7) Render.text(", pressing deep into your own huge rack, your nipples flicking across her own leaky tits");
	Render.text(".");
	...
	else Render.text("sexless crotch");
	Render.text(", still burning hotly with your desires");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.balls > 0) Render.text(", her other hand cupping your [balls], rolling the " + num2Text(player.lowerBody.balls) + " orbs in her palm with delightful dexterity");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) {
			Render.text("  She cups your breasts, having to reach so far around both your rack and hers that she's straining her arms to rub your [nipples], but she does so valiantly, stroking them with her incredibly deft fingers.");
			if(player.lactationQ() >= 100) Render.text("  A spurt of milk escapes your own full jugs, joining the pool-full of your friend's.  She gasps with delight, quickly nuzzling herself into your back and going to work.  She milks you just as she would herself, letting the hefty flow of your motherly fluids pour into the pool, odd trickles smearing down your chest, staining your chest as white as her own.");
		}
	}
	//[Fuck Her](PC must have gender; if cooch, also C+ cups) [Don't]
	menu();
	addButton(4,"Don't",dontFuckTheMilkWaifu);
	if(player.lowerBody.cockSpot.hasCock()) addButton(0,"Fuck Her",fuckTheMilkWaifu);
	if(player.lowerBody.vaginaSpot.hasVagina() && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) addButton(1,"LesboFuck",beARugMunchingMilkDyke);
...
private fuckTheMilkWaifu():void {
	...
	if(player.cockArea(0) < 20) Render.text("pushing your few inches into her until your hips join, her nice and loose cunt easily taking your length");
	else if(player.cockArea(0) < 50) Render.text("hilting her in one long stroke");
...
private beARugMunchingMilkDyke():void {
	...
	if(player.lactationQ() >= 200) Render.text(", milking you with skill beyond anything you've ever experienced before; and why not, when her entire existence revolved around that self-same skill?");
	else Render.text(".");
	...
	Render.text(".  You run your fingers through the girl's hair, urging her sexual skills on as your chest heaves and quavers, riding out the massive boobgasm as femcum spurts from your cunt and into the milky pool below.");
...
private drinkAndFapturbate():void {
	...
	if(flags[FlagEnum.MILK_SIZE] == 0) Render.text("a dinner plate");
	else Render.text("an open palm");
	Render.text(" but far more exciting.  After taking a few swallows of her body's milky treat, you reach down to your ");
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("[vagina] and idly stroke your puffy, lust-engorged vulva");
	else {
		Render.text("[cock] and idly stroke the turgid mass");
	}
	...
	//(DA SCHLICKS)
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("Your pussy juices mix freely with the tub's white-colored 'waters', allowing your cream-lubed fingers to plunge into your [vagina] with ease.  You stroke your lips and caress the interior of your birth canal with the intimate familiarity of a skilled lover, playing with your body until you feel your control slipping, so wound up with lust that you feel like an over-tightened guitar string vibrating out of control.");
	//(DA DUDES)
	else {
		Render.text("[OneCock] throbs painfully in your hand, so hot and hard that you're sure you must have begun to leak precum, but any fluid is swiftly washed away by the ever-present milk.");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("  You make sure to fondle each of your members equally, caressing, squeezing, and stroking to the tempo of your swelling passion.");
		if(player.lowerBody.balls > 0) Render.text("  With your offhand, you cradle your [sack], hefting your [balls] as you feel your desire churning to new levels.");
	}
	//(TOGETHER)
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("  [EachCock] release its own gooey load, sputtering and spurting to add to the pearly deluge.");
		if(player.cumQ() >= 1500) Render.text("  With every torrent of seed you release, you can see it lift partway out of the tub, propelled by your incredible virility towards the nearest female specimen.");
		if(player.cumQ() >= 4000) Render.text("  Soon, the tub's fluid contents break out of their confines to stain your companion's dusky flesh white, an alabaster glaze that would entice you to further feats of debauchery were it not for the pleased contentment your maleness now radiates.");
	}
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Meanwhile, your juiced-up cunny feels like it's doing backflips inside you, contorting and squeezing as it gushes with fluid, feminine joy.  One brush on your [clit] knocks your [legs] out from underneath you, but thankfully, you float out the rest of your orgasm.");
...
private nyanCatMilkTime():void {
	...
	{
		Render.text("\"<i>Milk time!</i>\" you say, giving " + flags[FlagEnum.MILK_NAME] + " a playful swat on her upturned ass, walking around her to the pool.  You jump in and toss your [armor] back out as the milky witch gets situated, well aware of just how much milk this poor girl's about to blast you with, even with her much reduced bosom.  Still, she's gotta be milked, and you're thirsty.  Milk time indeed.");
		...
		if(isabellaFollowerScene.isabellaFollower()) {
			...
			Render.text("</i>\" You look up from your task to see Isabella looming over the pool, hands on her wide hips.  \"<i>You are going MUCH too hard on her, poor ");
			if(isabellaFollowerScene.isabellaAccent()) Render.text("zing");
			else Render.text("thing");
			Render.text(".  Let Isabella show you how ");
			if(isabellaFollowerScene.isabellaAccent()) Render.text("es ist");
			else Render.text("it's");
			Render.text(" done.</i>\"  You find yourself roughly pushed aside as Isabella jumps in with you, tossing her skirt and blouse aside as she grasps " + flags[FlagEnum.MILK_NAME] + "'s teats, massaging them much more gently.  The milk girl's eyes roll back in her head as Isabella sets to work, though it gets harder and harder as the milk flows up to her thighs and chest.");
		}
		
		//If PC has ONLY KIha or if PC dun have Izzy, Sophie, OR Kiha: 
		if(!isabellaFollowerScene.isabellaFollower() || !sophieFollowerScene.sophieFollower()) Render.text("\n\nIt takes a good long while to get " + flags[FlagEnum.MILK_NAME] + "'s bloated tits under control, but nearly an hour later you've milked her as well as you can for now.  Sopping wet, you pull yourself out of the pool and grab a towel.  With her tits lightened for the moment, " + flags[FlagEnum.MILK_NAME] + " reaches up and pulls you down to her, just long enough to plant a kiss on your cheek and whisper, \"<i>Thank you, [name].  That felt good.</i>\"");
	...
	//Milk Time!  (DD Boobs Ver.)
		...
		if(player.cockThatFits(50) >= 0 && player.lowerBody.cockSpot.hasCock()) Render.text("  Then again, maybe you could hike her panties down and give her a good fucking to send her over the edge.  In her state, she certainly won't mind!");
		menu();
		addButton(0,"Normal",finishMilkNormall);
		addButton(1,"Suckle",suckleDatMilk);
		if(player.cockThatFits(50) >= 0 && player.lowerBody.cockSpot.hasCock()) addButton(2,"Fuck", fuckDatMilkSlat);
	}
}
...
private titFuckDatMilkSlut():void {
	...
	//If not naga:
	if(!player.isNaga()) Render.text("straddling her shoulders and shoving");
	else Render.text("wrapping yourself around her to shove");
	Render.text(" your [cock] deep into her mouth.  " + flags[FlagEnum.MILK_NAME] + " accepts it eagerly, opening wide as you stuff inch after inch of your manhood down her throat.  Now she can really go to work: her lips wrap tight around your base, tongue lapping at your underside like candy as her throat muscles massage the shaft.  You slide in and out, watching her throat bulge and contract with each thrust");
	if(player.lowerBody.balls > 0) Render.text(", shivering as her tongue darts out around your cock to flick your nads");
	Render.text(".");
