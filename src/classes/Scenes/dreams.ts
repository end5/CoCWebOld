...
import Flags, { FlagEnum } from "../classes/Game/Flags";
import Utils from "../classes/Utilities/Utils";
import Game from "../classes/Game/Game";
import { CockType } from "../classes/Body/Cock";
import RaceScore from "../classes/RaceScore";

export default class Dreams {
    public static dreamSelect(): boolean {
        let daydream: number = 0;
        let choices: number[] = [];
        let temp: number = 0;
        //dream quantity
        let dreamQuantity: number = Utils.rand(Game.player.stats.lib / 10 + Game.player.stats.cor / 20) + Game.player.stats.cor / 20;

        //BUILD UP CHOICES ARRAY
        temp = RaceScore.humanScore(Game.player);
        //Add human numbers to choices
        while (temp > 0) {
            choices[choices.length] = 0;
            temp--;
        }
        temp = RaceScore.dogScore(Game.player);
        while (temp > 0) {
            choices[choices.length] = 2;
            temp--;
        }
        temp = RaceScore.horseScore(Game.player);
        while (temp > 0) {
            choices[choices.length] = 1;
            temp--;
        }
        temp = RaceScore.cowScore(Game.player);
        while (temp > 0) {
            choices[choices.length] = 3;
            temp--;
        }
        temp = RaceScore.catScore(Game.player);
        while (temp > 0) {
            choices[choices.length] = 4;
            temp--;
        }
        temp = RaceScore.demonScore(Game.player);
        while (temp > 0) {
            choices[choices.length] = 5;
            temp--;
        }
        //Two chances if addicted, three if with perk!
        if (Game.player.minotaurAddicted()) {
        ...
        //Exgartuboobs
        if (Game.player.statusAffects.get("Exgartuan").value1 == 2) {
            choices[choices.length] = 8;
            choices[choices.length] = 8;
            choices[choices.length] = 8;
        }
        //Exgartucock
        if (Game.player.statusAffects.get("Exgartuan").value1 == 1 && Game.player.lowerBody.cockSpot.hasCock()) {
            choices[choices.length] = 9;
            choices[choices.length] = 9;
            choices[choices.length] = 9;
        }
        //Latexy Skinz
        if (Game.player.skinAdj == "latex" || Game.player.skinAdj == "rubber") {
        ...
        if (Flags.list[FlagEnum.NUMBER_OF_TIMES_MET_SCYLLA] > 0 && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00150] > 0) {
            ...
            choices[choices.length] = 12;
            choices[choices.length] = 12;
            if (Game.player.lowerBody.cockSpot.hasCock() && Game.player.stats.cor > 50) {
        ...
        //Sand trap
        if (RaceScore.sandTrapScore(Game.player) >= 2) {
            choices[choices.length] = 16;
            choices[choices.length] = 16;
            choices[choices.length] = 16;
            choices[choices.length] = 16;
        }
        if (Game.player.mouseScore() >= 3) {
        ...
        if (dreamQuantity > 5) {
            ...
            //normal fantasies...
            if (daydream == 0) {
                if (Game.player.stats.cor <= 33) {
                    //randomly 1 of two simple fantasies.
                    if (Utils.rand(2) == 0) MainScreen.text("tender sex with a sweetheart back home.  ", false);
                    else {
                        MainScreen.text("beautiful nude ", false);
                        if (Game.player.gender <= 1 || (Game.player.gender == 3 && Utils.rand(2) == 0)) MainScreen.text("women ", false);
                        else MainScreen.text("men ", false);
                        MainScreen.text("massaging you, slowly moving their hands over your most intimate places.  ", false);
                    }
                }
                if (Game.player.stats.cor > 33 && Game.player.stats.cor <= 66) {
                    MainScreen.text("being violently raped and used by demons.  ", false);
                }
                if (Game.player.stats.cor > 66) {
                    dreamQuantity = Utils.rand(5);
                    if (dreamQuantity <= 3) MainScreen.text("giving yourself to the demons fully, allowing yourself to be tied down and owned as your body is warped for their twisted pleasures.  ", false);
                    else MainScreen.text("being captured and taken to a rusted building with dark smokestacks that belch sweet purplish smoke.  Inside are rows of multi-dicked, huge breasted humans, approximately thirty of them.  Each is shackled with their legs spread and torso bent over, and each has tight fitting suction tubes fitting over their tits and cocks.  Every set of tubes is pulsing with suction, drawing rivers of sticky white fluids from its slave's over-endowed breasts and balls.  You shudder in horror and arousal as you realize the victims seem to be arranged by age, ending with an empty machine next to the youngest slut-cow.  The inhuman strength of your captors easily overpowers your struggles as you are forced into your shackles, the metal locks clicking with finality.  A funnel is forced into your mouth, force-feeding you slick corrupted fluids that taste like sex and make your head swim.  Your vision fades as you feel heat in your chest and groin, making you swoon from the drugged cocktail and pleasure of your new best friends – the suction tubes. All you can hear is your own desperate moans... no, wait... that was a dream... but it was so twisted and hot that you're still panting with lust.  ", false);
                }
            }
            //canine
            else if (daydream == 2) {
                //Male-ish dreams
                if (Game.player.gender <= 1 || (Game.player.gender == 3 && Utils.rand(2) == 0)) {
                    if (Utils.rand(2) == 0) MainScreen.text("locking a thick knotted cock inside a female, the pheromones of her heat making your maleness twitch and flex, the knot bulging obscenely as you begin to impregnate her.  ", false);
                    else MainScreen.text("stroking a knotted doggie-prick, gently stroking and squeezing it, yipping in pleasure as your pointed dog-cock leaks steady streams of fluids.  ", false);
                }
                //female
                else {
                    //heat dream!
                    if (Game.player.inHeat) {
                        MainScreen.text("being pregnant, your belly bulging obscenely with the fruits of all your frequent copulations, and your breasts swollen with breast milk.  You imagine your condition enhancing your libido, driving you seek out sexual partners willing to pleasure your distorted form.  ", false);
                    }
                    //normal dream
                    else {
                        MainScreen.text("being in heat, your cunt sopping wet with moisture and desire, intense pheromones pouring off you to make all the males rigid and ready.  ", false);
                    }
                }
            }
            //horse
            else if (daydream == 1) {
                if (Game.player.gender == 1 || (Game.player.gender == 3 && Utils.rand(2) == 0)) MainScreen.text(" running the plains with a harem of beautiful centaur fillies.  Your bloated equine endowments swelling with blood at the sight of their fertile backsides and potent pheromones.   You dream of staying up half the night to service your animalistic brides insatiable desires, plugging them over and over until your baby-batter is running in steady streams from their backsides.  ", false);
                else MainScreen.text("running your own farm, complete with a stable of docile, horse-morphed studs.   Of course each had to be captured as he came through the portal, until you had enough studs to keep your cunny happy and your womb filled with equine-spunk.  You saunter out to the barn and smile at the immediate reaction your presence has on your livestock, their leering eyes glued to every curve of your nude body.   Each of them lines up, guiding their rapidly hardening shafts through specially crafted holes in the stalls.  You giggle as your oldest acquisition struggles, trying to get the massive flare of his head through the hole before it gets any bigger.  You dream of walking down the aisle, granting each stud the release he so desires, taking the largest and most worthy to feed your tainted womb's thirst for hot spunk.  ", false);
            }
            //cow
            else if (daydream == 3) {
                ...
                else MainScreen.text("roaming the mountain-sides while you hunt for a mate, your turgid shaft and swelling balls aching with the need for release.  ", false);
            }
            //cat
            else if (daydream == 4) {
                //FEMALE
                if (Game.player.lowerBody.vaginaSpot.hasVagina() && (!Game.player.lowerBody.cockSpot.hasCock() || Utils.rand(2) == 0))
                    MainScreen.text("being a full cat and getting pounded by another as you mewl with pleasure.  He comes and pulls out, the barbs on his cock rake your insides as you yowl from the sensation. You clean yourself before searching for another cat to pound you, then another, and another...  ", false);
                //MALE
                else
                    MainScreen.text("prowling through the forest as a cat.  One dream in particular has you encountering a female cat in heat, and filling her womb to the brim with cum as you rake her insides with the barbs on your cat-cock. You dream that you impregnate every female you come across, making sure you fight off any competing males...  ", false);
            }

            //demon
            else if (daydream == 5) {
                MainScreen.text("being used and abused by demons of all varieties.  ", false);
            }
            //minotaur cum
            else if (daydream == 6) {
                ...
                MainScreen.text("You awaken from the vivid dream at the sensation of penetration, lurid images slowly fading in the morning light.  Yet, somehow, you can still just barely smell the blissful odor of minotaur spunk.", false);
                MainScreen.text("\n", false);
            }
            else if (daydream == 7) {
                MainScreen.text("grinding your rear into his thick cock, your hand reaching behind you to caress Akbal's cheek.  Having his prick between your ass is almost too distracting, but you know you have a duty to perform.  You raise your hands and address the acolytes before you, kneeling and observing.\n\n", false);
                ...
                MainScreen.text("You cum.  You scream in glorious and divine pleasure, granted by your immaculate god.  The acolytes watching masturbate themselves, in awe of the raptured orgasm Akbal has granted you.  Your tongue lolls out as he pounds into you, the haze of delight briefly devouring your thoughts in all-consuming green fire.\n\n", false);

                MainScreen.text("\"<i>Glory unto Akbal,</i>\" Buttslut whispers.\n\n", false);

                MainScreen.text("You awake with a start, grumbling slightly before turning over and trying to get back to sleep.  Your asshole twitches.", false);
            }
            //Exgartuboobs
            else if (daydream == 8) {
                MainScreen.text("drifting through darkness to arrive at your lover's bed.  She purrs and beckons you over, almost entirely hidden behind her massive chest.  You grunt slightly, and between your legs your cock rises, rises, rises - growing in measurements of feet rather than inches.  Excited and eager she moans as you enter her cleavage, thrusting into the ample soft flesh eagerly.\n\n", false);

                MainScreen.text("Her hands stroke her tits, encouraging you to push in deeper, harder.  She jiggles the overabundant flesh, sending comfortable ripples down all of her obscene cleavage.  \"<i>Harder,</i>\" she pants out eagerly, \"<i>harder!</i>\"  You grunt and pound, gripping onto her huge nipples to brace yourself.  Under her chin your cockhead bursts up, grinding into her neck and making her gasp excitedly.\n\n", false);
            ...
            //Scylla + dom + dong
                ...
                if (Game.player.gender <= 1) {
                    ...
                    //Male Trap Dream #2: 
                        ...
                        if (Game.player.lowerBody.cockSpot.hasCock()) MainScreen.text("  [EachCock] is fully erect and throbbing.");
                        else if (Game.player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Your [vagina] dripping wet with need.");
                        MainScreen.text("  You shudder as you recall the erotic dream you were just having, a hand caressing your aching loins.");
                        //end dream
                        //lust increase
                    }
                ...
                //Dream about living in a borrow and having lots and lots of kids.  Many furred, big eared bodies are running around and you feel the pregnant belly of your mate, or feel your own pregnant belly.
                    ...
                    MainScreen.text("finding yourself in an underground burrow, with many small furred bodies with large ears scurrying around all over.  The place is warm and cozy, while also filled with the smell of mice and sex.  You love it down here, and you love seeing all your energetic children running around and having fun.\n\n", false);
                    MainScreen.text("You have a great larder, able to feed everyone as much as they need.  It has never been a problem to keep it full.  It's a veritable paradise for a rodent such as yourself.  Then you find your way into the master bedroom and see its massive bed, the site where your great family was brought into existence.\n\n", false);

                    if (Game.player.gender == 1 || (Game.player.gender == 3 && Utils.rand(2) == 0)) {
                        MainScreen.text("Things blur for a moment, and you find your head resting against a pregnant belly, covered in fine fur.  The results of your virility as a father, and soon to be added to the great brood that you helped bring into this world...\n\n", false);
                    } else {
                        MainScreen.text("Things blur for a moment, and you find yourself laying on the bed with your hands wrapped around your heavily pregnant belly.  The product of your fertility and the virility of your great mate, and soon to be added to the great brood you've already brought into the world...\n", false);
                    }
                }
            }
    ...
    public static dayTenDreams(): void {
        ...
        if (Game.player.lowerBody.cockSpot.hasCock()) {
            if (Game.player.stats.cor < 50) {
                MainScreen.text("Your task completed, you return through the portal to your home.  Exhausted but happy, the sight of your village fills you with a surge of emotions.  Most prominently, and perhaps most enjoyably: pride.  You did it.\n\n", false);

                MainScreen.text("You drink and celebrate with your people long into the night, feasting and receiving accolades and praise.  It is a party the likes of which you have never seen, and rightfully so.  Never again will your village be threatened by the demons.  Late in the evening, you arrive at the home of your childhood and greet your mother.\n\n", false);

                MainScreen.text("\"<i>Welcome back,</i>\" she smiles.\n\n", false);

                MainScreen.text("\"<i>Thank you, Mom,</i>\" you respond, wrapping your arms around her and kissing her deeply.  Your hands take hold of her rear, gripping-\n\n", false);

                MainScreen.text("Shocked awake, you blink as the light of your dying fire flickers dimly across the campsite.  That was...\n\n", false);

                MainScreen.text("This place is getting to you.", false);
            }
            else {
                MainScreen.text("Your task completed, you return through the portal to your home.  Exhausted but happy, the sight of your village fills you with a surge of emotions.  Most prominently, and perhaps most enjoyably: pride.  You did it.\n\n", false);
                ...
                MainScreen.text("\"<i>Thank you, Mom,</i>\" you respond, wrapping your arms around her and kissing her deeply.  Your hands take hold of her rear, gripping it in great handfuls and squeezing her possessively.  Nearly throwing her in the bed, you delight in her playful squeals.  Mounting her, you tear away her dress and plunge into her cunt.\n\n", false);

                MainScreen.text("\"<i>Oooh, god!</i>\" she exclaims,  \"<i>Such a good boy!  Such a big boy!</i>\"\n\n", false);

                MainScreen.text("\"<i>That's right, Mom,</i>\" you grunt, taking hold of her breast and sucking harshly on it.  \"<i>All grown up.</i>\"  You continue to grope her roughly as you pound into her cunt, slapping your balls into her thighs.  Her pussy squeezes you with a mother's love, and you soon find yourself dumping heaping loads of your spunk within her.  You rise, and bring your dick to her mouth so that she can clean you.\n\n", false);

                MainScreen.text("As your mother laps at your shaft, coated in her juices, you look confidently down at her belly.  Though you love your mother, you're looking forward to having a daughter, and training her to love you just as much as a mother does. One day, perhaps even to replace her.\n\n", false);

                MainScreen.text("You awaken in a sweat.  Fuck.", false);
            }
        }
        else {
            if (Game.player.stats.cor < 50) {
                ...
                MainScreen.text("\"<i>Welcome back,</i>\" he smiles.\n\n", false);

                MainScreen.text("\"<i>Thank you, daddy,</i>\" you respond, wrapping your arms around him and kissing him deeply.  Your hands slip into his pants, lowering-\n\n", false);
                ...
                MainScreen.text("\"<i>Thank you, daddy,</i>\" you respond, wrapping your arms around him and kissing him deeply.  Your hands slip into his pants, lowering them and exposing his crotch.  Too excited to control yourself, you drag your lips down his chest before wrapping them around his cock and taking him into your mouth.  You pull him deep, teasing his balls with your lower lip.  Though it isn't necessary to gag - you could have swallowed him easily - you do anyway.  Daddy deserves a show.\n\n", false);

                MainScreen.text("His cum splatters into your throat.\n\n", false);

                MainScreen.text("You smile with glazed teeth as you straddle him in his bed and mount him.  Some disappointment lingers in you that you didn't just let daddy take your ass then and there, but the prospect of his cum flooding your womb and fertilizing you is too delightful to pass up.\n\n", false);

                MainScreen.text("\"<i>Do you like it?</i>\" you ask, before moaning his name.\n\n", false);

                MainScreen.text("\"<i>You're a good girl,</i>\" he answers.\n\n", false);

                MainScreen.text("His balls gurgle as they fill you.  You squeal in delight and lick his chest, demonstrating your affection.  Clenching him tightly, you make sure not to spill daddy's seed, and imagine giving birth to a sexy, busty daughter, teaching her to love him.  One day, teaching her to replace you.\n\n", false);

                MainScreen.text("You wake up in a sweat.  Fuck.", false);
            }
        }
        if (Game.player.stats.cor < 50) Game.player.stats.lust += 10;
        else Game.player.stats.lust += 25;
        doNext(Game.playerMenu);
    }
    ...
    public static displacerDreamII(): void {
        ...
        if (silly()) MainScreen.text("k");
        MainScreen.text("not a drop of jism has escaped.");
    ...
    public static displacerDreamIII(): void {
        MainScreen.clearText();
        MainScreen.text("The details of the dream immediately begin to slip away.  You strain your brain, trying to commit everything to memory, but it slips out of your mind like water through a sieve.");
