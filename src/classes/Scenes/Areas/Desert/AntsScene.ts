...
//[Explore Desert]
		private function firstAntColonyEncounter():void
		{
			...
			if (player.stats.cor > 75 && player.lust > 50) Render.text(" looking for something to slake your unquenchable lusts, ");
			Render.text("you come across an abandoned merchant's cart.  The cart looks eerily disheveled, with a majority of its contents strewn about the sand.  Whoever or whatever deserted this thing did so in a hurry, and for seemingly no reason. Curious, you decide to inspect it for anything worthwhile.");
...
//►[Keep Hidden]
		private function keepHidden():void
		{
			...
			//►(If Over 41 - Male)
				...
				Render.text(images.showImage("ants-desert-male-firstencounter"));
				Render.text("You know exactly what's going to happen once the demons 'subdue' their prey.  The growing throbbing in your loins begs for release, and you quickly and quietly take off your " + player.armorName + ", making sure the demons don't notice you.  You might as well watch the show as you're not gonna be able to leave unnoticed until the demons are done anyways.");
				...
				if (player.lowerBody.cockSpot.list[x].cockLength >= 20) Render.text("  Licking the head of your [cock " + (x + 1) + "] in anticipation of what's to come only adds to your growing lust.");
				//If player cock size tiny:
				else if (player.cockArea(x) < 6) Render.text("  Though your cock isn't the most impressive, it's 'got it where it counts' and you intend to make the most of it.  You lovingly tease yourself until you're fully erect; running your fingers down the ridge causes a bead of pre-cum to form on your tip.");
				//If player cock size normal:
				else if (player.cockArea(x) < 12) Render.text("  You hold your cock like the hilt of a sword and start to pump your shaft slowly.  Your breathing becomes heavier and labored, as you stroke yourself with mounting passion.");
				//If player cock size large:
				else if (player.cockArea(x) < 30) Render.text("  The fear of having your cock give away your position quickly passes through your mind, but it's too late now.  The only way out of this is to cum, so you might as well enjoy it.  You firmly grasp your cock and start jacking off, hoping that you'll finish sooner rather than later.");
				//If player cock size MASSIVE:
				else Render.text("  The towering meat log jutting from your crotch gives away your 'hiding' spot, but the demons don't seem to mind, as they're too busy with their new toy.  You doubt even if they did care that any of them would be able to satisfy you. You wrap your arms around your fully erect cock and start to pump, hoping to release your load soon.");
				...
				if (player.cumQ() < 100) Render.text("sizable");
				else if (player.cumQ() < 1000) Render.text("huge");
				else if (player.cumQ() < 10000) Render.text("massive");
				else Render.text("titanic");
				Render.text(" load.  Once you can refocus your eyes, you see the hermaphrodite pull out, and as it does a burst of cum sprays out of the ant-morph's vagina as her stomach deflates slightly.  She's been filled with so much corrupt seed that it's stretched her belly to the point it resembles a third-trimester pregnancy.");
			...
			//►If Over 41 - Female
				...
				Render.text(images.showImage("ants-desert-female-firstencounter"));
				Render.text("Watching from your hiding spot, you consider what's going to happen and feel your pussy immediately moisten.  Your practiced hands move with deftness, slipping into your [armor].");
				...
				//If PC is lactating:
				if (player.lactationQ() > 100) Render.text("  You feel your warm milk flow out of your hardened arola, as your body begs to release its sweet nectar.");
				//If PC has dick(s):
				if (player.lowerBody.cockSpot.hasCock()) Render.text("  Feeling [eachCock] throb between your legs, you run one of your hands down your chest and start to stroke lovingly.");
...
//►[Play Hero]
		private function playHero():void
		{
			...
			Render.text("As the demons bear down on the ant-girl, you burst from your hiding place, raising your [weapon] to the air and uttering an impressive war cry.  Nobody, ant or otherwise, is getting raped if you have any say in the matter!");
...
//(Explore the Desert)
		private function enterTheColony():void
		{
			...
			//[(corruption < 40)
			if (player.stats.cor < 40) Render.text("\"<i>'Champion', you've been sent to this land to resist the demons and your efforts thus far, insignificant though they be, have not escaped my gaze.  ");
			else Render.text("\"<i>Despite your obvious taint, you seem to have retained your valor as the 'Champion' you proclaim yourself to be, and not as a rutting beast.  Something to be admired, for sure.  ");
			Render.text("In saving my daughter, Phylla...</i>\"  She pauses and gestures, and the four-armed girl you saved from the demon horde walks slowly and shyly out of a nearby tunnel to stand next to her mother.  'Phylla' looks very embarrassed and is blushing deeply, avoiding eye contact from both of you by looking at the ground as Queen Chylla continues her dry monologue.  \"<i>... you have been chosen to become her potential...'mate'; much to my dismay.</i>\"  Wait, mate?  \"<i>My daughter has shown me how you saved her from the demons that inhabit this land.  Perhaps you were just lucky and caught them with their cocks in their hands while her back was turned?</i>\"");
...
//The Challenges
		private function antColonyChallenge():void
		{
			...
			if (flags[FlagEnum.MET_ANT_ARENA] == 0) {
				...
				else if (player.str > 50) Render.text("\n\n\"<i>Well, you certainly look the part.  But you're going to need more than brute strength to win here.</i>\"");
				//Else if intelligence over 50:
				else if (player.stats.int > 50) Render.text("\n\nYou make a witty quip about how he might be making puppy eyes at someone who saved him from a horde of giant demon cocks, too, but he doesn't receive it well.");
				Render.text("  After a moment he looks down and scribbles something on his chart.");
				Render.text("\n\n\"<i>So, we're ready to start when you are.</i>\"");
				flags[FlagEnum.MET_ANT_ARENA]++;
			}
			//Introduction Subsequent Times
			else {
				Render.text("Your feet bring you back to the ant colony and the guard motions you down the only lit tunnel once more.  You enter the colosseum, and as you step out into the cavern, you are greeted, again, by a thin male ant-morph holding a clipboard.  He looks up at you.");
...
//►[Leave]
		...
		private function antColiseumFight():void
		{
			...
			//►[Fight #3]
				...
				//(Gnoll - Intro)
				if (flags[FlagEnum.MET_ANT_ARENA_GNOLL] == 0) Render.text("With due ceremony, the manager leads you down to one of the staging areas for the arena.  You watch through the bars as the stadium fills far past capacity.  There must be a thousand ants here... maybe even the whole colony.  Gazing out into the crowd, you pick out the royal seating and in it, Princess Phylla.  The princess is in much finer attire than you're used to seeing - her chest is actually covered by a fine red and blue dress and her hair is combed, framing her face nicely.  Her mother is nowhere to be seen, and you can tell that Phylla is taking full advantage.  She waves and cheers for you just like the crowd is and, perhaps by her orchestration, you hear the crowd pick up a chant in your name.  Seeing Phylla chant with them makes you feel invigorated.  As the gates raise, you charge out into the center of the arena, roaring your finest battle cry; your opponent is taken somewhat aback but finds resolve and braces for your charge.");
				//►(Gnoll - Intro Repeat - for people that lost once)
				else Render.text("You move through the network of tunnels and caves and come to the colosseum once more.  You are greeted by the event manager and he ushers you down to one of the staging areas for the arena.  The noise intensifies as the colosseum fills to the brim with spectators; Phylla herself is up in her box waving to you.  You look across the arena floor and the gnoll at the other side of the arena seems just as excited to fight as you are.  The gates open and you charge each other!");
...
//►(Minotaur- Loss)
		...
		public function phyllaPCBeatsGnoll():void
		{
			...
			//►(Gnoll - Win Repeat) Standard Gnoll Win Scene. +
				...
				Render.text("This isn't the first time you've accomplished this, but it still brings the crowd to their feet.  You simply stand in the center of the arena, taking it all in.  Phylla makes her way to you, blushing as the rowdy audience follows her with catcalls and demands for her to give her champion a kiss.  She tends to you as always and, when she's done, takes your hand in hers and looks at you expectantly.  You smile at her, brushing the hair from her face with your other hand; she colors deeply at the contact as two guards appear to lead you both off.  Having no more reason to stay, you allow your guide to return you to the surface.");
			}
			//(+1 Win Score)
			flags[FlagEnum.ANT_ARENA_WINS]++;
			cleanupAfterCombat();
...
//Good End / Waifu Content
		private function antGirlGoodEnd():void
		{
			...
			menu();
			if (player.lowerBody.cockSpot.hasCock()) addButton(0, "Use Penis", gigititigitigitigitigityAntGirl);
			if (player.lowerBody.vaginaSpot.hasVagina()) addButton(1, "Use Vagina", femalePhyllaFirstFuckGooooo);
		}
...
//►Male Continuation
		private function gigititigitigitigitigityAntGirl():void
		{
			...
			if (player.cockArea(x) >= 72) {
				...
				if (player.tentacleCocks() > 0) {
					...
					Render.text(" forward and wrap them tightly around Phylla, taking her hand and tugging her in close.");
				}
				...
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
				Render.text(" from around her and hang it in front of her face, grinning your antics on your face as Phylla stares at ");
				if (player.lowerBody.cockSpot.count() == 1) Render.text("it");
				else Render.text("them");
				Render.text(", slightly intimidated.");
				...
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
				Render.text(" looking for something more sizeable.  You can tell Phylla is more scared of what might happen to her than what might happen to you.");
		...
		private function malePhyllaContinuation(x: number):void
		{
			...
			//PC is goo morph:
			if (player.isGoo()) Render.text(", leaving traces of your slimy lust along her body for her to wear, allowing your detached bits of mass to ignite her sexual fires");
			Render.text(".");
			...
			if (player.stats.cor < 75) addButton(0, "Next", phyllaFirstTimePureBabiesFuckEnding);
...
//PURE ENDING!
		private function phyllaFirstTimePureBabiesFuckEnding():void
		{
			...
			if (player.cumQ() < 50) Render.text("thin");
			else if (player.cumQ() < 500) Render.text("hefty");
			else Render.text("thick");
			Render.text(" ejaculate far into her womb.  Phylla responses in kind, working her hips and squeezing your body with her legs in an almost milking fashion, trying to draw as much seed as she can into her womb.");
			...
			if (player.cumQ() >= 100 && player.cumQ() < 1000) Render.text("  Truly amazing considering how much your loads usually contain.  You think you see a little bulge in her stomach that wasn't there before.");
			//If cum milliliters over 1000:
			else if (player.cumQ() >= 1000) Render.text("  As you look down at her nethers your mind struggles to understand just how she could... then looking up her body, you see.  Her belly is bloated with your sperm, almost to the point of already looking pregnant.");
			...
			if (player.canOvipositSpider()) {
				...
				if (player.stats.int / 20 + rand(20) + 1 < 16) {
					...
					if (pregnancy.isPregnant) Render.text("  I just can't hold anything else inside me. I'm sorry! Please don't be mad... I mean I will!  Just a-after... this batch...");
				...
				//Persuasion success:
				{
					...
					if (player.stats.cor >= 75) Render.text("  Little does poor Phylla know, you're going to just use her as a baby maker to ease the tension of your own sacks.");
...
//Corruption greater than 75 (Corrupt Ending):
		private function phyllaCorruptMascEnding():void
		{
			...
			Render.text(", but it will only serve to help you.  You will need a brace for what you're about to pull and her rigid abdomen will do just fine.  Phylla makes a confused noise, glancing over her shoulder with a look of concern growing on her face.");
			...
			if (y >= 0) {
				...
				if (player.lowerBody.cockSpot.count() > 2) Render.text("one of your free cocks");
				else Render.text("your free cock");
				Render.text(" against her tight ass. Feeling this new intruder, she turns back to you.");
			...
			if (player.lowerBody.cockSpot.count() > 1) Render.text("s twitch, ");
			else Render.text(" twitches, ");
			Render.text("ushering forth your corrupted seed into her womb");
			if (y >= 0) Render.text(" and her ass.");
			else Render.text(", and onto the bedspread below.");
			Render.text("  As her pulsing cunt works to suck the semen out of your ");
			if (player.lowerBody.balls > 0) Render.text("balls");
			else Render.text("shaft");
			Render.text(", Phylla retaliates with her own orgasm, flooding your groin with her sticky girl cum.  You glance down to your joined nethers and find a massive amount of Phylla's cum has pooled around your knees and more seems to be flowing every second.  Clearly, she's a squirter.");
...
//Cunnilingus Scene: (Triggered if PC does NOT have a dick that is suitable for vaginal sex with Phylla.)
		private function cuntmuffinLingusPhyllaDickBig():void
		{
			...
			if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
			Render.text(" isn't going to work.  Phylla's eyes begin to tear up as she also comes to the realization that her first time is a non-starter.");
			...
			if (player.longestCockLength() < 69) {
				Render.text("angle your hips so that ");
				if (player.lowerBody.cockSpot.count() == 1) Render.text("it points");
				else Render.text("they point");
				Render.text(" at the ceiling, placing them out of the way for what you have in mind.");
			}
			//Dick size Over 70 inches::
				...
				Render.text(" aimed out the doorway.");
			}
			Render.text("  You wickedly beckon Phylla over to you with a hooked finger.  Phylla looks at you quizzically for a moment before it dawns on her.  Her eyes widen and cooing she scrambles over to you.");
			...
			if (player.lowerBody.cockSpot.count() == 1) Render.text(" switches and bobs");
			else Render.text("s switch and bob");
			Render.text(", loudly brushing against the ");
			if (player.longestCockLength() < 69) Render.text("ceiling");
			else Render.text("wall in the hallway");
			Render.text(".");
			...
			//If Snake Tongue:
			if (player.tongueType == TONUGE.SNAKE) Render.text("  You run the split of your forked mouth muscle from the tip of her clit to the base.  As your textured feeler makes its way down, you wrap around the rest of her long clitorus, like a boa constrictor.  Once your split reaches the hood of her clit you roll and flick the tip of your tongue rapidly.  Pulling at your hair she gasps for breath in between teeth grinding moans.");
			...
			if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
			Render.text(" and pants heavily, having just gone through the equivalent of an orgasm triathlon.  As you sigh, you can hear her giggle in what sounds like disbelief.  \"<i>Heh... I didn't think I had it in me to do... that,</i>\" she coos, staring mischievously up at you.  \"<i>Just... could you shrink ");
			if (player.lowerBody.cockSpot.count() == 1) Render.text("this monster");
			else Render.text("these monsters");
			Render.text(" down?  I mean, I'm not complaining!  But I... just... I mean... you... inside me,</i>\" she shyly remarks, obviously wanting something more... traditional.");
...
//►Female Continuation
		private function femalePhyllaFirstFuckGooooo():void
		{
			...
			if (!player.lowerBody.cockSpot.hasCock()) addButton(0, "Next", girlFiller);
		...
		private function girlFiller():void
		{
			...
			Render.text("\"<i>I-I've... never,</i>\" she stammers, but you just want her to experience this before she ruins the moment.");
...
//If PC Herm/Has (a) cock(s):
		private function femalePhyllaFirstTimePlusCock():void
		{
			...
			Render.text(" has taken");
			Render.text(" hold of ");
			//One dick only:
			if (player.lowerBody.cockSpot.count() == 1) Render.text("your " + cockDescript(0) + ".");
			else Render.text("your " + cockDescript(0) + " leaving the lucky member to bask in affectionate glory as the rest of your manhood laments being left out.  \"<i>Don't worry, I have four hands...</i>\" Phylla coos as she works your shaft.");
			...
			if (player.smallestCockArea() > phyllaCapacity()) {
				...
				if (player.isTaur()) Render.text("Seeing your impending predicament, Phylla quickly scrambles with all four of her arms to collect a bunch of cushions.  She stacks them in a makeshift 'resting place' and helps you into a comfortable position as your cocks precede to grow uncontrollably.");
				//Drider body:
				else if (player.isDrider()) Render.text("Phylla immediately realizes this, and flips you over so that you are resting on your head and neck, ducking in past your " + multiCockDescriptLight() + " and coming to rest on the border between your spider carapace and soft clit.");
				//Goo body:
				else if (player.isGoo()) Render.text("Phylla immediately realizes this, and flips you over so that you are resting on your head and neck, and she works her way through your goopy mass, connecting her clit with yours in little time or effort. \"<i>This is a little weird... but I like it...</i>\" she flirts. Hardening your mass slightly, you snugly secure Phylla against your body, telling her that things \"might\" get rough.");
				//Human/Harpy/Naga legs:
				else Render.text("Phylla immediately realizes this, and flips you over so that you are resting on your head and neck, and curls your [legs] towards her.");

				Render.text("  Your " + multiCockDescriptLight() + " now resting comfortably out of the open room, Phylla positions herself to scissor your pussy.");
			...
			//If PC has an appropriately sized dick(s):
				...
				if (player.tentacleCocks() > 0) {
					...
					Render.text("your tentacle dick");
					if (player.tentacleCocks() > 1) Render.text("s");
					Render.text(".  Clearly she's never seen anything like it before as she doesn't know what to do.  Laughing, you tell Phylla she doesn't need to do anything except hold still.  You snake your tentacle cock");
					if (player.tentacleCocks() > 1) Render.text("s");
					Render.text(" up her body, making sure to caress her most tender places on their way.  She screams out loudly in shock only to realize what happened when she looks down.  \"<i>God, you scared me!</i>\" she yells, embarrassed that she got frightened half to death by your dick");
					if (player.tentacleCocks() > 1) Render.text("s");
					Render.text(".  Laughing at her display, you motion to your prick");
					if (player.tentacleCocks() > 1) Render.text("s and wiggle them ");
					else Render.text(" and wiggle it ");
					Render.text("in front of her, like a worm to a bird in her face.  \"<i>Aw, does someone need some attention?</i>\" Phylla teases, taking ");
					//PC has:
					//one tentacle dick:
					if (player.tentacleCocks() == 1) Render.text("your cock into her mouth as she continues to rock her hips into yours.");
					//two:
					else if (player.tentacleCocks() == 2) Render.text("your cocks into her mouth and between her breasts as she continues to rock her hips into yours.");
					else  Render.text("your cocks into her mouth, between her breasts, and along her body.  The ones on her body tightly wrap around her insectile joints, using her muscular curves to get you off.");
					...
					if (player.lowerBody.cockSpot.count() == 2 && (player.cockArea(player.smallestCockIndex2()) + player.cockArea(player.smallestCockIndex()) <= phyllaCapacity()))
						Render.text("two of your " + multiCockDescriptLight() + ", stroking them apologetically as she scissors you.  You feel her almost insatiable appetite for all parts of your body.  You feel Phylla's mind slowly becoming lost in pleasure.");
					...
					else if (player.lowerBody.cockSpot.list[player.smallestCockIndex()].cockLength < 4) Render.text("your " + cockDescript(player.smallestCockIndex()) + ", stroking it apologetically as she scissors you.  You feel her almost insatiable appetite for all parts of your body.  You feel Phylla's mind slowly becoming lost in pleasure.");
					//PC only has a dick(s) that('s) (are) more than 4 inches in width:
					else Render.text("your " + cockDescript(player.smallestCockIndex()) + ", but she simply can't fit any number of her hands around the entirety of your glans; instead, she takes to caressing the head of your " + cockDescript(player.biggestCockIndex()) + ".");
...
//If PC returns to colony after telling her to stay with her mother:
		...
		public function introductionToPhyllaFollower():void
		{
			...
			if (flags[FlagEnum.PHYLLA_EGG_LAYING] == 0) addButton(2, "Lay Eggs", phyllaLaysEggsToggle);
			else addButton(2, "No Eggs", phyllaLaysEggsToggle);
			if (flags[FlagEnum.ANT_KIDS] > 0) addButton(3, "Children", phyllasKidsChildren);
		...
		private function phyllaSexMenu():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				...
				if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0 && flags[FlagEnum.ANT_KIDS] >= 10) addButton(1, "Fuck Her", dudesFuckEggLayingBitches);
				if (flags[FlagEnum.ANT_KIDS] > 10 && player.stats.cor >= 75) addButton(3, "Orgy (Male)", orgyWithDatColonyCorruptDudes);
			}
			//Straight Sex (Lesbian/Fisting) - Written
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				addButton(2, "Lesbian Sex", lesbianFisting);
				//While Giving Birth (Female) - Written
				if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0 && flags[FlagEnum.ANT_KIDS] >= 10) addButton(2, "Lesbian Sex", birfingSexWithAntsForDasLadies);
				//Orgy w/ Colony (Female)
				//You tell Phylla you're interested in 'inspecting' your children.
				if (flags[FlagEnum.ANT_KIDS] > 10 && player.stats.cor >= 75) addButton(4, "Orgy (Female)", antColonyOrgy4Ladies);
			}
...
//..(Ant Morph History)
		private function talkAboutAntHistory():void
		{
			...
			Render.text("You relax on one of the many available pillows and ask Phylla about her race's history. She pulls a pillow up next to yours and cuddles in.  \"<i>It's not a very happy story, are you sure you want to hear it?</i>\" You nod and move your hand to hold one of hers.");
...
//(Ant Morph Mating Ritual / Inherited Knowledge)
		private function talkAboutAntMatingAndRituals():void
		{
			...
			//If corruption more than 60:
				...
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 4) Render.text("\n\n\"<i>At least not like yours... I mean, yours are nicer!</i>\"");
				...
				if (player.stats.cor >= 80) {
					...
					if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0) Render.text(", sending jets of milk shooting out onto the floor");
					Render.text(".  With a loud moan she orgasms, covering her smaller two forearms, her inner thighs, and the floor below her in thick, sweet-smelling, girl cum.");
...
//(Phylla's Life Past & Future)
		private function phyllasLifePastAndFuture():void
		{
			...
			if (kGAMECLASS.izmaScene.izmaFollower() && flags[FlagEnum.PHYLLA_IZMA_TALK] == 0) {
				...
				Render.text("Phylla seems to be a little distant today as you sit down to talk.  You point it out to her and she cautiously asks about Izma.");
				...
				Render.text("You take a seat on one of the many fluffy cushions in her room and she proceeds to cuddle in close, pressing her head against your chest and wrapping her abdomen around you.  She then looks up at you and asks what's on your mind.");
...
//First Time Blowjob:
		private function phyllaBeeeJays():void
		{
			...
			//Subsequent Blowjobs:
				...
				Render.text("Approaching Phylla, you cross your arms and lean against the wall of her bedchamber while sporting a tell-tale tent in your pants.  Phylla stares down at your bulge and picks up immediately on your intentions.");
				Render.text("\n\n\"<i>Y-You gonna ask me nicely, or let your penis");
				if (player.lowerBody.cockSpot.count() > 1) Render.text("es");
				Render.text(" do all the convincing?</i>\"  She teases.  Shrugging, you inform her that whatever works for her works for you.");
			}
			//Continued...
			//If Phylla is not Laying Eggs:
			...
			if (player.cockArea(x) < 60) {
				...
				if (player.lowerBody.cockSpot.count() > 1) Render.text("all ");
				Render.text("bobbing up");
				//Dick size less than 5 inches:
				if (player.lowerBody.cockSpot.list[x].cockLength < 5) Render.text(" \"tall\" and proud, rock hard and ready for action.");
				else Render.text(" tall and proud, rock hard and ready for action.");
			}
			else {
				...
				if (flags[FlagEnum.PHYLLA_BLOWJOBS] == 1) {
					...
					if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
					Render.text(" from your pants, but apparently doesn't realize just how big ");
					if (player.lowerBody.cockSpot.count() == 1) Render.text("this");
					else Render.text("these");
					Render.text(" battering ram");
					if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
					Render.text(" can get.  Chuckling at her ignorance, you tell Phylla she'd better back up.  Phylla rolls her eyes and looks up at you, giving you an unamused look.");
					...
					Render.text(" up to a full raging erection, achieving your maximum state of \"<i>readiness</i>\" in no time at all.");
				...
				//Dick size more than 60 inches (subsequent blowjobs):
					...
					if (flags[FlagEnum.PHYLLA_BLOWJOBS] == 2) Render.text("last time");
					else Render.text("our first time");
					Render.text(".  I'll help to free ");
					if (player.lowerBody.cockSpot.count() == 1) Render.text("that monster");
					else Render.text("those monsters");
					Render.text(", but p-please be careful about my valuables.</i>\"  Phylla asks nervously.");
					...
					Render.text(" freed once more, ready and raring to \"re-acquaint\" with Phylla.  Taking the hint, Phylla closes in and takes the ");
					//Dick size less than 16 inches:
					if (player.lowerBody.cockSpot.list[x].cockLength < 16) Render.text("base");
					else Render.text("middle");
					Render.text(" of your shaft against her tongue.  You hear a mix of a giggle and hum as she drags her tongue towards the head of your " + cockDescript(x) + ".  Phylla teases your shaft as she reaches the peak and suckles at your urethral opening.  You feel her drawing up as much pre-cum as she can and lapping it up like she's dying of thirst.  She continues this for a bit, until it becomes apparent that she's either not too sure of what else to do, or that she's never heard of how to properly fellate someone.");
			...
			if (player.stats.cor < 75) addButton(0, "Next", purePhyllaBJOver);
...
//Pure BJ:
		private function purePhyllaBJOver():void
		{
			clearOutput();
			let x: number = player.smallestCockIndex();
			Render.text("You quickly and softly tell her to calm down, she needn't worry; you'll be more than happy to help her learn how as surely as she would help you if your situations were reversed.  At the mention of counter positions she nods cheerfully.");
			...
			Render.text("Taking your directions, Phylla takes the head of your cock along the top of her wet and humid tongue, leisurely batting around before trapping it with her lips.");
			...
			if (player.lowerBody.cockSpot.count() > 1) {
				...
				if (player.lowerBody.cockSpot.count() == 2) Render.text("your other dick");
				else Render.text("the rest of your dicks");
				Render.text("; if she would be \"<i>so kind.</i>\" With gusto, her four hands shoot up and take your ");
				if (player.lowerBody.cockSpot.count() == 2) Render.text("cocks into their firm grips");
				else Render.text("cock into their firm grips");
				Render.text(".  Phylla mumbles something but absentmindedly realizes she still has your cock in her mouth and gives up on whatever it was she was trying to say.");
			}
			//Vagina:
			if (player.lowerBody.vaginaSpot.hasVagina()) {
...
//[Sure]
		private function surePhyllaLetsFuck():void
		{
			clearOutput();
			let x: number = player.smallestCockIndex();
			Render.text("\"<i>We'll have to... kiss...  I mean, I can still taste you... inside my mouth...  I could go wash or... ah~!</i>\" You cut her off by pulling her mouth on to yours.");
...
//[Nope]
		private function nopeNotOnMouthOrWhateverFuckThisNoise():void
		{
			...
			if (!player.canAutoFellate()) Render.text(" or anything like it would feel like.");
			else Render.text(" feels like with Phylla's mind linked to your own.");
...
//Pure BJ Ending
		private function pureBJEnding(linked:boolean = true):void
		{
			...
			if (!linked) Render.text(".");
				...
				Render.text("leading to you jesting through the link and calling her a damn dirty cheater.");
			}
			if (flags[FlagEnum.TIMES_LINKED_BJ_SUCK] > 0) Render.text("  You can feel her relying less and less on the link with every subsequent time; there's no doubt in your mind that she will no longer \"<i>cheat</i>\" her way into making you cum before you know it.");
			let x: number = player.smallestCockIndex();
			...
			if (player.cumQ() < 250) Render.text("thin");
			else if (player.cumQ() > 1000) Render.text("thick");
			Render.text(" strands of cum hang wildly between her lips and your prick.");
			//If cum volume over 1000ml::
			if (player.cumQ() > 1000) Render.text("  Though she might have a mouthful, you're not finished yet!  You continue to unload the remainder of your hot load on her breasts and stomach.");
			if (player.lowerBody.cockSpot.count() > 1) {
				//If multi cock::
				Render.text("  Your other cock");
				if (player.lowerBody.cockSpot.count() > 2) Render.text("s shower");
				else Render.text(" showers");
				Render.text(" her hands and body in your white goo as she continues to pump ");
				if (player.lowerBody.cockSpot.count() == 2) Render.text("on each of ");
				Render.text("them lovingly.");
			}

			Render.text("\n\nWith a heavy sigh, and significantly lighter ");
			if (player.lowerBody.balls > 0) Render.text("balls");
			else Render.text("in the pelvis");
			Render.text(", you rub Phylla's hair playfully");
			if (flags[FlagEnum.PHYLLA_BLOWJOBS] != 1) Render.text(".");
			//If first time:
			else Render.text(" and congratulate her on her first time.");
...
//Corrupt BJ Ending
		private function corruptPhyllaEndings():void
		{
			clearOutput();
			let x: number = player.smallestCockIndex();
			Render.text("\"<i>A slut that can't suck a dick... how can this get any worse?</i>\"  You ponder to yourself while rubbing your nasal bridge. You tell the innocent little ant that she'll learn how to suck a mean cock, even if it kills her.  You're knee deep at this point - might as well turn her into something salvageable.  Commanding her attention, you lift her face up and hover your " + cockDescript(x) + " close to her lips, smothering her lips with your precum.  She attempts to turn away but you react before she can.  Holding her head in place you comment on how this is what you want from her.");
			...
			//If first time:
			if (flags[FlagEnum.PHYLLA_BLOWJOBS] == 1) Render.text("achieved the desired effect.");
			//Every other time:
			else Render.text("improved since the last time.");
			...
			if (player.lowerBody.cockSpot.count() > 1) {
				...
				Render.text(" if she would be \"<i>so kind</i>\".  With gusto, her four hands shoot up and take your cocks into their firm grips.  Phylla mumbles something but absentmindedly realizes she still has your cock in her mouth and gives up on whatever it was she was trying to say.");
			}
			//Vagina:
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				Render.text("\n\nYou ask Phylla if she can take care of your womanhood while she's at it.  Phylla moves one of her small arms in between your legs and rubs against your " + clitDescript() + ".  She caresses your labia in between her fingers before inserting them inside of you.  Phylla begins working her wrist diligently in conjunction with her head bobbing.");
			...
			if (player.cumQ() < 10) Render.text("tiny");
			else if (player.cumQ() < 100) Render.text("respectable");
			else Render.text("massive");
			Render.text(" facial, covering her petite features in cum.");
			...
			if (silly()) {
				...
				Render.text(".  Somehow, you succeed your fortitude roll and remain conscious without passing out even after plummeting to the ground from sheer exhaustion.  You and Phylla can't help but slowly drift off to sleep in a pool of your cum to dream about the bizarre pleasures you two have indulged in.");
			}
...
//\"<i>Use Dick</i>\"
		private function dickPhylla():void
		{
			...
			//3-4 times:
			if (flags[FlagEnum.PHYLLA_FUCKS] >= 3) Render.text(" she asks playfully, knowing full well that that's what you came here for.");
			//(First time)
			else if (flags[FlagEnum.PHYLLA_FUCKS] == 0) Render.text(" You nod quickly.");
			//(Subsequent fucks)
			if (flags[FlagEnum.PHYLLA_FUCKS] > 0) Render.text("\n\n\"<i>Do you have to ask?</i>\"  You ask her while grinning mischievously; something that causes her to smirk back at you.");

			//3-4 times:
			if (flags[FlagEnum.PHYLLA_FUCKS] >= 3) Render.text("\n\nShe seems like she's losing some of her shyness, or at the very least feels comfortable enough around you to assert more of her personality.");
			...
			Render.text("in her sexual liquids.");
...
//While Giving Birth (Male) - Written
		private function dudesFuckEggLayingBitches():void
		{
			...
			if (player.stats.cor < 50) Render.text("don't care how she looks right now. Besides, some creatures around here appreciate a little carnal love; it helps with the birthing process.");
			//If Corruption is more than 50:
			else Render.text("could care less for her pathetic attempts at concealing her pregnant form; you're horny, and she's going to put out, regardless of her insecurities.");

			Render.text("\n\nYou walk over to her and ");
			if (player.stats.cor < 50) Render.text("lovingly");
			else Render.text("apathetically");
			Render.text(" pull her hands away from her breasts.  She resists you at first, but once she sees there's no stopping you she shyly resigns herself to you.");
			...
			//(If PC has balls:
			if (player.lowerBody.balls > 0) Render.text(" your balls slap against the smoothness of her pelvis.");
			else Render.text(" your knees grind along the edge of her makeshift throne.");
...
//While Giving Birth (Female) - Written
		private function birfingSexWithAntsForDasLadies():void
		{
			...
			if (player.isNaga()) Render.text("  You slither almost whorishly with a slow, measured wriggle,");
			else Render.text("  You walk almost whorishly with a slow, sensual gait,");
			Render.text(" allowing her to see the sensual swivel of your " + buttDescript() + " from left to right and back again as you visibly entice Phylla's mind to lustful thoughts.  Once you're facing Phylla once again, you see her lower two hands are still covering her breasts and the upper two are gripping the stone chair she sits in.");
			...
			if (player.stats.cor <= 50) Render.text("  You probe her mind and are almost taken aback.\"<i>NO!  Just please keep going!</i>\"  You hear her voice ring out in your head.");
			...
			if (player.lowerBody.cockSpot.count() > 0 && player.cockThatFits(42) >= 0) {
				...
				if (player.lowerBody.cockSpot.count() == 1) Render.text("it");
				else if (player.lowerBody.cockSpot.count() == 2) Render.text("them");
				else Render.text("as many as you can");
				Render.text(" as fiercely as your body will let you.");
			}
			...
			if (player.lowerBody.cockSpot.count() > 0 && player.cockThatFits(42) >= 0) {
				...
				if (player.cockThatFits2(42) >= 0) Render.text("two of ");
				Render.text("your " + multiCockDescriptLight() + " into her mouth and mashes ");
				if (player.cockThatFits2(42) >= 0) Render.text("them");
				else Render.text("it");
				Render.text(" against the back of her throat.  Unable to contain the pleasure from this, you howl and blow your hot load into her throat.");
			}
			...
			if (player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 500) {
				...
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
				Render.text(".  With a heavy heave, you feel the hot and sticky ejaculate launch from your dick, hitting the interior decorations as you do so; what exactly you hit, you can't say... but you'll likely be tripping over your mess on the way out.");
			}
...
//Orgy w/ Colony (Male) - Written
		private function orgyWithDatColonyCorruptDudes():void
		{
			...
			Render.text("You tell Phylla to call in your strongest and most virile children for 'inspection.'");
			...
			if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0) Render.text("large, lactating ");
			...
			if (player.lowerBody.cockSpot.count() > 1 && player.lowerBody.cockSpot.count() <= 3) {
				...
				if (player.lowerBody.cockSpot.count() == 2) Render.text(" is");
				else Render.text("s are");
				Render.text(" getting sucked off by two of your children, who themselves are being sucked off.");
			}
			//If player has four-6 six dicks:
			if (player.lowerBody.cockSpot.count() >= 4 && player.lowerBody.cockSpot.count() <= 6) Render.text("\n\nEven though you have so many cocks, it won't be a problem.  Just standing over Phylla you look down as hands, mouths, and Phylla's breasts do their best to please every one of your cocks in turn.");
			...
			if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
			Render.text(" sucked, stroked, inserted, and finally orgasming so many times that you lose track. Hours pass and you eventually pass out sitting atop Phylla's throne watching the writhing mass before you.");
...
//You tell Phylla you're interested in 'inspecting' your children.
		private function antColonyOrgy4Ladies():void
		{
			...
			if (flags[FlagEnum.TIMES_CORRUPT_FEMALE_ANT_ORGY] == 1) {
				Render.text("She gives you a confused look, but does as you command. Tilting her head back and closing her eyes you watch as she silently 'communicates' to her children.");
			...
			if (player.isBiped()) Render.text("  Setting one foot on his shoulder, you present your [vagina] to him.");
			else Render.text(" Now at the perfect height, you present your [vagina] to him.");
			...
			if (player.isBiped()) Render.text(" from between your legs.");
			else Render.text(" from your cunt.");
			Render.text("  You beckon him to follow you as you strut over to where Phylla is.");
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				Render.text("  You feel hands run up and down your shaft");
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
				Render.text(", causing you to spurt bursts of pre all over the floor.");
			}
			...
			//If PC has dick(s):
			if (player.lowerBody.cockSpot.hasCock()) {
				Render.text("  Not forgetting you have 'other needs' the ant mounting you reaches down with his larger arms and grabs hold of [eachCock].  Keeping his smaller arms on your hips to keep stable with each thrust, he pumps vigorously at your cock");
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
				Render.text(".");
			}
			...
			if (player.lowerBody.cockSpot.count() > 0) {
				...
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
				Render.text(".  You feel your load");
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
				Render.text(" splash into your [chest] and chest as ");
				if (player.lowerBody.cockSpot.count() == 1) Render.text("it");
...
//►[Lay Eggs / Don't Lay Eggs]
		private function phyllaLaysEggsToggle():void
		{
			...
			if (flags[FlagEnum.PHYLLA_EGG_LAYING] == 0) {
				...
				Render.text("Phylla seems confused as to what you're thinking as you gaze deeply into her eyes.  You gather all four of her hands between the two of yours and tell her that children are wanted.  Not just one or two; you want as many as she can muster.");
				//If you've talked to her about history:
				if (flags[FlagEnum.TALKED_WITH_PHYLLA_ABOUT_HISTORY] > 0) Render.text("  You tell her you want a colony like those she spoke of in the times before the corruption.");
			...
			//(Stop Laying Eggs)
				...
				Render.text("You sigh to yourself; you never really wanted this conversation but you knew it might happen someday.  You make a stern face and put a determined fatherly tone in your voice.  Phylla immediately senses you mean business and you have her full attention.  You try to tell her as gently as possible that you don't think it is safe for her to keep having children.");
...
//DON'T LICK THAT!
		private function dontLickAntButt():void
		{
			...
			if (flags[FlagEnum.ANT_KIDS] > 1) Render.text("; the children can tend to the eggs and I can really use the rest");
			Render.text(".  You should get back to your duties...  B-but I wouldn't mind if you came back later...</i>\"");
...
//Phylla lays Drider eggs
		private function phyllaLaysSomeDriderEggs():void
		{
			...
			if (player.stats.cor < 75) {
				...
				if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0) Render.text("both eggs are just fine on their own");
				else Render.text("the egg is just fine on its own");
				Render.text(", you redirect your focus to helping Phylla through the labor.  Each subsequent egg that finds its way to freedom causes Phylla's once pain filled screams to turn into pleasurable moans of euphoric delight as her pussy becomes overwhelmed by the stimulation of each of your eggs working their way out.  At long last, the final egg slithers out of her vagina and lands gently on the bedding.  Her ordeal over, Phylla breathes a long sigh of relief.");
				//First Time:
...
//(Leads to - Failure)
		private function phyllaDigsForGems():void
		{
			...
			if (flags[FlagEnum.PHYLLA_GEMS_HUNTED_TODAY] == 0 && rand(20) + kidsMod > 10) {
				...
				//If Phylla IS NOT Laying Eggs
				if (flags[FlagEnum.PHYLLA_EGG_LAYING] == 0) {
					Render.text("You ask Phylla is she's found any gems while digging out her colony.  She nods happily and runs over to a small stone chest and rifles though it.  After a moment, she runs back over to you and holds up all four of her hands.");
			...
			//Failure
				...
				Render.text("You ask Phylla if she's found any more gems while digging.  \"<i>No, I'm sorry.  I haven't.  Not since the last time you asked.  I'll do better!  I mean, I know you like them.  They're rare... even down here.  I'll do better...</i>\"");
				//If corruption is less  than 50
...
//Sex > [Egg Phylla]
		private function eggDatBitch():void
		{
			clearOutput();
			//PC is a Female/Herm Drider:
			if (player.canOvipositSpider()) Render.text("While Phylla appears to be sexually sated, the heaviness in your spider abdomen begs for release.\n\n");
			//First Time:
			if (flags[FlagEnum.TIMES_EGG_IMPREGNATING_PHYLLA] == 0) {
				Render.text("Turning to Phylla, you tell her that you can help to make her wishes of being a brood-mother come true.  She looks at you a bit taken back, as if what you already did wasn't enough already.");
			...
			//Subsequent Times:
			else Render.text("You give Phylla a knowing look then glance back at your swollen spider abdomen.  Playfully you thrust your ovipositor at her.  Phylla giggles a little, knowing what you want to do to her.");
