...
//[Explore Desert]
		private function firstAntColonyEncounter():void
		{
			...
			if (player.stats.cor > 75 && player.lust > 50) outputText(" looking for something to slake your unquenchable lusts, ");
...
//►[Keep Hidden]
		private function keepHidden():void
		{
			...
			//►(If Over 41 - Male)
				...
				if (player.lowerBody.cockSpot.list[x].cockLength >= 20) outputText("  Licking the head of your [cock " + (x + 1) + "] in anticipation of what's to come only adds to your growing lust.");
			...
			//►If Over 41 - Female
				...
				//If PC has dick(s):
				if (player.lowerBody.cockSpot.hasCock()) outputText("  Feeling [eachCock] throb between your legs, you run one of your hands down your chest and start to stroke lovingly.");
...
//After whuppin' demons!
		internal function consolePhylla():void
		{
			...
			if (player.stats.lib >= 50) outputText("After taking an eyeful of her smooth, enticing pussy, revealed in her current position by the inadequacies of her loincloth, y");
			else outputText("Y");
...
//(Explore the Desert)
		private function enterTheColony():void
		{
			...
			//[(corruption < 40)
			if (player.stats.cor < 40) outputText("\"<i>'Champion', you've been sent to this land to resist the demons and your efforts thus far, insignificant though they be, have not escaped my gaze.  ");
...
//►[Leave]
		...
		private function antColiseumFight():void
		{
			...
			//►[Fight #3]
				...
				//(Gnoll - Intro)
				if (flags[FlagEnum.MET_ANT_ARENA_GNOLL] == 0) outputText("With due ceremony, the manager leads you down to one of the staging areas for the arena.  You watch through the bars as the stadium fills far past capacity.  There must be a thousand ants here... maybe even the whole colony.  Gazing out into the crowd, you pick out the royal seating and in it, Princess Phylla.  The princess is in much finer attire than you're used to seeing - her chest is actually covered by a fine red and blue dress and her hair is combed, framing her face nicely.  Her mother is nowhere to be seen, and you can tell that Phylla is taking full advantage.  She waves and cheers for you just like the crowd is and, perhaps by her orchestration, you hear the crowd pick up a chant in your name.  Seeing Phylla chant with them makes you feel invigorated.  As the gates raise, you charge out into the center of the arena, roaring your finest battle cry; your opponent is taken somewhat aback but finds resolve and braces for your charge.");
...
//►Male Continuation
		...
		private function malePhyllaContinuation(x: number):void
		{
			...
			outputText("\n\nYou tug ");
			if (player.stats.cor < 75) outputText("playfully");
			...
			if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0) outputText("nipples and ");
			...
			if (player.stats.cor < 75) outputText("a gentle amount of force, ");
		...
		private function phyllaFirstTimePureBabiesFuckEnding():void
		{
			...
			if (player.canOvipositSpider()) {
				...
				//Persuasion success:
				{
					...
					if (player.stats.cor >= 75) outputText("  Little does poor Phylla know, you're going to just use her as a baby maker to ease the tension of your own sacks.");
...
//Corruption greater than 75 (Corrupt Ending):
		private function phyllaCorruptMascEnding():void
		{
			...
			if (player.lowerBody.balls == 0) outputText("thighs");
			...
			if (y >= 0) {
				...
				if (player.lowerBody.cockSpot.count() > 2) outputText("one of your free cocks");
			...
			if (player.lowerBody.cockSpot.count() > 1) outputText("s twitch, ");
...
//Cunnilingus Scene: (Triggered if PC does NOT have a dick that is suitable for vaginal sex with Phylla.)
		private function cuntmuffinLingusPhyllaDickBig():void
		{
			...
			if (player.longestCockLength() < 69) {
				...
				if (player.lowerBody.cockSpot.count() == 1) outputText("it points");
			...
			if (player.lowerBody.cockSpot.count() == 1) outputText(" switches and bobs");
			...
			//If Snake Tongue:
			if (player.tongueType == TONUGE.SNAKE) outputText("  You run the split of your forked mouth muscle from the tip of her clit to the base.  As your textured feeler makes its way down, you wrap around the rest of her long clitorus, like a boa constrictor.  Once your split reaches the hood of her clit you roll and flick the tip of your tongue rapidly.  Pulling at your hair she gasps for breath in between teeth grinding moans.");
			...
			if (player.lowerBody.cockSpot.count() == 1) outputText("this monster");
...
//If PC Herm/Has (a) cock(s):
		private function femalePhyllaFirstTimePlusCock():void
		{
			...
			outputText(" hold of ");
			//One dick only:
			if (player.lowerBody.cockSpot.count() == 1) outputText("your " + cockDescript(0) + ".");
			...
			//If PC has an appropriately sized dick(s):
				...
				if (player.tentacleCocks() > 0) {
					...
					else if (player.lowerBody.cockSpot.list[player.smallestCockIndex()].cockLength < 4) outputText("your " + cockDescript(player.smallestCockIndex()) + ", stroking it apologetically as she scissors you.  You feel her almost insatiable appetite for all parts of your body.  You feel Phylla's mind slowly becoming lost in pleasure.");
...
//(Ant Morph Mating Ritual / Inherited Knowledge)
		private function talkAboutAntMatingAndRituals():void
		{
			...
			//If corruption more than 60:
				...
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 4) outputText("\n\n\"<i>At least not like yours... I mean, yours are nicer!</i>\"");
				...
				if (player.stats.cor >= 80) {
					...
					if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0) outputText("milk dripping ");
					...
					if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0) outputText(", sending jets of milk shooting out onto the floor");
...
//First Time Blowjob:
		private function phyllaBeeeJays():void
		{
			...
			if (player.cockArea(x) < 60) {
				...
				if (player.lowerBody.cockSpot.count() == 1) outputText("its");
				...
				if (player.lowerBody.cockSpot.count() > 1) outputText("all ");
				outputText("bobbing up");
				//Dick size less than 5 inches:
				if (player.lowerBody.cockSpot.list[x].cockLength < 5) outputText(" \"tall\" and proud, rock hard and ready for action.");
				...
				//Dick size more than 60 inches (subsequent blowjobs):
					...
					if (flags[FlagEnum.PHYLLA_BLOWJOBS] == 2) outputText("last time");
					else outputText("our first time");
					outputText(".  I'll help to free ");
					if (player.lowerBody.cockSpot.count() == 1) outputText("that monster");
					...
					if (player.lowerBody.cockSpot.list[x].cockLength < 16) outputText("base");
...
//Pure BJ:
		private function purePhyllaBJOver():void
		{
			...
			if (player.lowerBody.cockSpot.count() > 1) {
				...
				if (player.lowerBody.cockSpot.count() == 2) outputText("your other dick");
				...
				if (player.lowerBody.cockSpot.count() == 2) outputText("cocks into their firm grips");
...
//Pure BJ Ending
		private function pureBJEnding(linked:boolean = true):void
		{
			...
			if (flags[FlagEnum.TIMES_LINKED_BJ_SUCK] > 0) outputText("  You can feel her relying less and less on the link with every subsequent time; there's no doubt in your mind that she will no longer \"<i>cheat</i>\" her way into making you cum before you know it.");
			let x: number = player.smallestCockIndex();
			...
			if (player.lowerBody.cockSpot.count() > 1) {
				...
				if (player.lowerBody.cockSpot.count() > 2) outputText("s shower");
				else outputText(" showers");
				outputText(" her hands and body in your white goo as she continues to pump ");
				if (player.lowerBody.cockSpot.count() == 2) outputText("on each of ");
...
//Corrupt BJ Ending
		private function corruptPhyllaEndings():void
		{
			...
			//If first time:
			if (flags[FlagEnum.PHYLLA_BLOWJOBS] == 1) outputText("achieved the desired effect.");
			//Every other time:
...
//\"<i>Use Dick</i>\"
		private function dickPhylla():void
		{
			...
			//3-4 times:
			if (flags[FlagEnum.PHYLLA_FUCKS] >= 3) outputText(" she asks playfully, knowing full well that that's what you came here for.");
			//(First time)
			else if (flags[FlagEnum.PHYLLA_FUCKS] == 0) outputText(" You nod quickly.");
			//(Subsequent fucks)
			if (flags[FlagEnum.PHYLLA_FUCKS] > 0) outputText("\n\n\"<i>Do you have to ask?</i>\"  You ask her while grinning mischievously; something that causes her to smirk back at you.");

			//3-4 times:
			if (flags[FlagEnum.PHYLLA_FUCKS] >= 3) outputText("\n\nShe seems like she's losing some of her shyness, or at the very least feels comfortable enough around you to assert more of her personality.");
...
//While Giving Birth (Male) - Written
		private function dudesFuckEggLayingBitches():void
		{
			...
			if (player.stats.cor < 50) outputText("don't care how she looks right now. Besides, some creatures around here appreciate a little carnal love; it helps with the birthing process.");
			...
			if (player.stats.cor < 50) outputText("lovingly");
			...
			//(If corruption is more than 50:
			if (player.stats.cor > 50) outputText("\n\n(Oh, great, here we go again...)");
			...
			//(If PC has balls:
			if (player.lowerBody.balls > 0) outputText(" your balls slap against the smoothness of her pelvis.");
...
//While Giving Birth (Female) - Written
		private function birfingSexWithAntsForDasLadies():void
		{
			...
			outputText("\n\nYou ");
			if (player.stats.cor < 75) outputText("lovingly");
			...
			if (player.stats.cor <= 50) outputText("  You probe her mind and are almost taken aback.\"<i>NO!  Just please keep going!</i>\"  You hear her voice ring out in your head.");
			...
			if (player.lowerBody.cockSpot.count() > 0 && player.cockThatFits(42) >= 0) {
				...
				outputText(", stroking ");
				if (player.lowerBody.cockSpot.count() == 1) outputText("it");
				else if (player.lowerBody.cockSpot.count() == 2) outputText("them");
...
//Orgy w/ Colony (Male) - Written
		private function orgyWithDatColonyCorruptDudes():void
		{
			...
			if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0) outputText("large, lactating ");
			...
			if (player.lowerBody.cockSpot.count() > 1 && player.lowerBody.cockSpot.count() <= 3) {
				...
				if (player.lowerBody.cockSpot.count() == 2) outputText(" is");
			...
			if (player.lowerBody.balls > 0) outputText(" along with your own");
...
//You tell Phylla you're interested in 'inspecting' your children.
		private function antColonyOrgy4Ladies():void
		{
			...
			if (flags[FlagEnum.TIMES_CORRUPT_FEMALE_ANT_ORGY] == 1) {
				...
				if (player.lowerBody.cockSpot.hasCock()) outputText(" and [eachCock] harden");
...
//►[Lay Eggs / Don't Lay Eggs]
		private function phyllaLaysEggsToggle():void
		{
			...
			if (flags[FlagEnum.PHYLLA_EGG_LAYING] == 0) {
				...
				if (flags[FlagEnum.TALKED_WITH_PHYLLA_ABOUT_HISTORY] > 0) outputText("  You tell her you want a colony like those she spoke of in the times before the corruption.");
...
//DON'T LICK THAT!
		private function dontLickAntButt():void
		{
			...
			if (flags[FlagEnum.ANT_KIDS] > 1) outputText("; the children can tend to the eggs and I can really use the rest");
...
//Phylla lays Drider eggs
		private function phyllaLaysSomeDriderEggs():void
		{
			...
			if (player.stats.cor < 75) {
				...
				if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0) outputText("both eggs are just fine on their own");
