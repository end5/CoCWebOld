...
//  DIDNT_FUCK_PHYLLA_ON_RECRUITMENT: number = 925;
		...
		public timeChange():boolean
		{
			...
			if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0 && rand(5) == 0 && flags[FlagEnum.ANT_KIDS] < 5000) flags[FlagEnum.ANT_KIDS]++;
			if (model.time.hours > 23) {
				...
				if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0) flags[FlagEnum.DAYS_PHYLLA_HAS_SPENT_BIRTHING]++;
				if (flags[FlagEnum.PHYLLA_GEMS_HUNTED_TODAY] > 0) flags[FlagEnum.PHYLLA_GEMS_HUNTED_TODAY] = 0;
				if (phyllaWaifu()) flags[FlagEnum.DAYS_PHYLLA_IN_CAMP]++;
			}
			return false;
		}
		...
		public antColonyEncounter():void
		{
			...
			if (flags[FlagEnum.ANT_ARENA_WINS] - flags[FlagEnum.ANT_ARENA_LOSSES] >= 2 && flags[FlagEnum.ANT_ARENA_WINS] >= 4 && player.gender > 0) {
				if (flags[FlagEnum.PHYLLA_STAY_HOME] > 0) bumpIntoTheAntColonyAfterStayHomePhylla();
				else antGirlGoodEnd();
			}
			else if (flags[FlagEnum.PC_READY_FOR_ANT_COLONY_CHALLENGE] == 1)
				antColonyChallenge();
			else if (flags[FlagEnum.PHYLLA_SAVED] == 1)
				enterTheColony();
...
//[Explore Desert]
		private firstAntColonyEncounter():void
		{
			...
			if (player.stats.cor > 75 && player.lust > 50) MainScreen.text(" looking for something to slake your unquenchable lusts, ");
...
//►[Keep Hidden]
		private keepHidden():void
		{
			...
			MainScreen.clearText();
			//If Under 40
			if ((player.stats.lib < 41 && player.lust < 50) || player.stats.cor < 66 || player.gender == 0) {
			...
			//►(If Over 41 - Male)
				...
				MainScreen.text("You know exactly what's going to happen once the demons 'subdue' their prey.  The growing throbbing in your loins begs for release, and you quickly and quietly take off your " + player.armorName + ", making sure the demons don't notice you.  You might as well watch the show as you're not gonna be able to leave unnoticed until the demons are done anyways.");
				...
				if (player.lowerBody.cockSpot.list[x].cockLength >= 20) MainScreen.text("  Licking the head of your [cock " + (x + 1) + "] in anticipation of what's to come only adds to your growing lust.");
...
//(Explore the Desert)
		private enterTheColony():void
		{
			...
			flags[FlagEnum.PC_READY_FOR_ANT_COLONY_CHALLENGE] = 1;
			doNext(camp.returnToCampUseOneHour);
		}
...
//The Challenges
		private antColonyChallenge():void
		{
			...
			if (flags[FlagEnum.MET_ANT_ARENA] == 0) {
...
//►[Leave]
		...
		private antColiseumFight():void
		{
			MainScreen.clearText();
			//►[Fight #1]
			if (flags[FlagEnum.ANT_ARENA_WINS] + flags[FlagEnum.ANT_ARENA_LOSSES] == 0) {
			...
			//►[Fight #2]
			else if (flags[FlagEnum.ANT_ARENA_WINS] + flags[FlagEnum.ANT_ARENA_LOSSES] == 1) {
				//►(Minotaur - Intro)
			...
			//►[Fight #3]
				...
				//(Gnoll - Intro)
				if (flags[FlagEnum.MET_ANT_ARENA_GNOLL] == 0) MainScreen.text("With due ceremony, the manager leads you down to one of the staging areas for the arena.  You watch through the bars as the stadium fills far past capacity.  There must be a thousand ants here... maybe even the whole colony.  Gazing out into the crowd, you pick out the royal seating and in it, Princess Phylla.  The princess is in much finer attire than you're used to seeing - her chest is actually covered by a fine red and blue dress and her hair is combed, framing her face nicely.  Her mother is nowhere to be seen, and you can tell that Phylla is taking full advantage.  She waves and cheers for you just like the crowd is and, perhaps by her orchestration, you hear the crowd pick up a chant in your name.  Seeing Phylla chant with them makes you feel invigorated.  As the gates raise, you charge out into the center of the arena, roaring your finest battle cry; your opponent is taken somewhat aback but finds resolve and braces for your charge.");
				...
				flags[FlagEnum.MET_ANT_ARENA_GNOLL]++;
...
//►(Tentacle Beast - Loss) Standard Tentacle Beast Loss Scene.
		public phyllaTentaclePCLoss():void
		{
			...
			// (+1 Loss Score)
			flags[FlagEnum.ANT_ARENA_LOSSES]++;
			if (flags[FlagEnum.ANT_ARENA_LOSSES] > 3) {
				antastrophyBadEnd();
				return;
			}
			cleanupAfterCombat();
...
//►(Minotaur- Loss)
		...
		public phyllaPCBeatsGnoll():void
		{
			...
			if (flags[FlagEnum.ANTS_PC_BEAT_GNOLL] == 0) {
				...
				flags[FlagEnum.ANTS_PC_BEAT_GNOLL]++;
			}
			else if (flags[FlagEnum.ANT_ARENA_WINS] - flags[FlagEnum.ANT_ARENA_LOSSES] >= 2 && flags[FlagEnum.ANT_ARENA_WINS] >= 3 && player.gender > 0) {
		...
		public phyllaGnollBeatsPC():void
		{
			...
			if (flags[FlagEnum.ANTS_PC_LOST_TO_GNOLL] == 0) {
				flags[FlagEnum.ANTS_PC_LOST_TO_GNOLL]++;
...
//►Male Continuation
		...
		{
			MainScreen.clearText();
			MainScreen.text("Phylla stares down at your " + multiCockDescriptLight() + " and starts nervously rubbing her hands together.  \"<i>It's my first time... I mean, I hope it isn't going to be bad for you...</i>\" she says, before trailing off; uncertain of her sexual prowess.");
			...
			//PC is goo morph:
			if (player.isGoo()) MainScreen.text(", leaving traces of your slimy lust along her body for her to wear, allowing your detached bits of mass to ignite her sexual fires");
			MainScreen.text(".");
			...
			if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0) MainScreen.text("nipples and ");
...
//Corruption greater than 75 (Corrupt Ending):
		private phyllaCorruptMascEnding():void
		{
			...
			else MainScreen.text(ballsDescriptLight());
			...
			//PC has Goo Body:
			if (player.isGoo()) MainScreen.text("  Perhaps the poor dear could use some tender attention, you ponder to yourself.  Reducing the solidity of your body, you work her abdomen deep into your sticky, goopy body, re-solidifying once she's firmly in there and lowering your hands down to her nipples, pinching them hard as you continue your pumping of her rump.");
...
//Cunnilingus Scene: (Triggered if PC does NOT have a dick that is suitable for vaginal sex with Phylla.)
		...
		{
			MainScreen.clearText();
			MainScreen.text("Sadly, as you gaze down at your " + multiCockDescriptLight() + ", you realize that trying to penetrate Phylla with your freakishly large prick");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
			...
			//If Snake Tongue:
			if (player.tongueType == TONUGE.SNAKE) MainScreen.text("  You run the split of your forked mouth muscle from the tip of her clit to the base.  As your textured feeler makes its way down, you wrap around the rest of her long clitorus, like a boa constrictor.  Once your split reaches the hood of her clit you roll and flick the tip of your tongue rapidly.  Pulling at your hair she gasps for breath in between teeth grinding moans.");
			...
			if (player.isGoo()) MainScreen.text(", tumbling out of your mass as she tears free");
			...
			dynStats("lus=", 100);
			flags[FlagEnum.DIDNT_FUCK_PHYLLA_ON_RECRUITMENT] = 1;
...
//►Female Continuation
		...
		{
			MainScreen.clearText();
			MainScreen.text("Eager to get to some good old fashion fun, you direct your thoughts to Phylla, telling her to 'remove her clothing'.  After all of the pieces of her royal attire are removed, she closes in and rubs her naked body against yours.  Her fingers move fluidly as she slowly traces her ever hardening nipples along your chest and along the ridges of your back, sending a sensual chill up and down your spine.  Your " + nippleDescript(0) + "s harden in response, and with surprising speed she shifts her body downward and locks her mouth around your nipple, firmly suckling at your hardened areola.");
			//If PC Lactating:
...
//If PC Herm/Has (a) cock(s):
		private femalePhyllaFirstTimePlusCock():void
		{
			...
			//One dick only:
			if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("your " + cockDescript(0) + ".");
			else MainScreen.text("your " + cockDescript(0) + " leaving the lucky member to bask in affectionate glory as the rest of your manhood laments being left out.  \"<i>Don't worry, I have four hands...</i>\" Phylla coos as she works your shaft.");
			...
			if (player.smallestCockArea() > phyllaCapacity()) {
				...
				if (player.isTaur()) MainScreen.text("Seeing your impending predicament, Phylla quickly scrambles with all four of her arms to collect a bunch of cushions.  She stacks them in a makeshift 'resting place' and helps you into a comfortable position as your cocks precede to grow uncontrollably.");
				//Drider body:
				else if (player.isDrider()) MainScreen.text("Phylla immediately realizes this, and flips you over so that you are resting on your head and neck, ducking in past your " + multiCockDescriptLight() + " and coming to rest on the border between your spider carapace and soft clit.");
				//Goo body:
				else if (player.isGoo()) MainScreen.text("Phylla immediately realizes this, and flips you over so that you are resting on your head and neck, and she works her way through your goopy mass, connecting her clit with yours in little time or effort. \"<i>This is a little weird... but I like it...</i>\" she flirts. Hardening your mass slightly, you snugly secure Phylla against your body, telling her that things \"might\" get rough.");
				...
				MainScreen.text("  Your " + multiCockDescriptLight() + " now resting comfortably out of the open room, Phylla positions herself to scissor your pussy.");
			...
			//If PC has an appropriately sized dick(s):
				...
				if (player.tentacleCocks() > 0) {
					...
					if (player.tentacleCocks() > 1) MainScreen.text("s and wiggle them ");
					...
					//PC has:
					//one tentacle dick:
					if (player.tentacleCocks() == 1) MainScreen.text("your cock into her mouth as she continues to rock her hips into yours.");
					//two:
					else if (player.tentacleCocks() == 2) MainScreen.text("your cocks into her mouth and between her breasts as she continues to rock her hips into yours.");
					...
					if (player.lowerBody.cockSpot.count() == 2 && (player.cockArea(player.smallestCockIndex2()) + player.cockArea(player.smallestCockIndex()) <= phyllaCapacity()))
						MainScreen.text("two of your " + multiCockDescriptLight() + ", stroking them apologetically as she scissors you.  You feel her almost insatiable appetite for all parts of your body.  You feel Phylla's mind slowly becoming lost in pleasure.");
					...
					else if (player.lowerBody.cockSpot.list[player.smallestCockIndex()].cockLength < 4) MainScreen.text("your " + cockDescript(player.smallestCockIndex()) + ", stroking it apologetically as she scissors you.  You feel her almost insatiable appetite for all parts of your body.  You feel Phylla's mind slowly becoming lost in pleasure.");
					//PC only has a dick(s) that('s) (are) more than 4 inches in width:
					else MainScreen.text("your " + cockDescript(player.smallestCockIndex()) + ", but she simply can't fit any number of her hands around the entirety of your glans; instead, she takes to caressing the head of your " + cockDescript(player.biggestCockIndex()) + ".");
...
//►[Stay Here]
		private tellPhyllaToStayTheFuckAtHomeThatCunt():void
		{
			...
			flags[FlagEnum.PHYLLA_STAY_HOME] = 1;
			doNext(camp.returnToCampUseOneHour);
		}
...
//If PC returns to colony after telling her to stay with her mother:
		...
		public introductionToPhyllaFollower():void
		{
			...
			if (flags[FlagEnum.PHYLLA_CAMP_VISITS] == 0) {
				...
				if (flags[FlagEnum.DIDNT_FUCK_PHYLLA_ON_RECRUITMENT] == 1 && player.lowerBody.cockSpot.hasCock()) {
			...
			if (player.lust >= 33) MainScreen.addButton(1, "Sex", phyllaSexMenu);
			if (flags[FlagEnum.PHYLLA_EGG_LAYING] == 0) MainScreen.addButton(2, "Lay Eggs", phyllaLaysEggsToggle);
			else MainScreen.addButton(2, "No Eggs", phyllaLaysEggsToggle);
			if (flags[FlagEnum.ANT_KIDS] > 0) MainScreen.addButton(3, "Children", phyllasKidsChildren);
		...
		private phyllaSexMenu():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				...
				if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0 && flags[FlagEnum.ANT_KIDS] >= 10) MainScreen.addButton(1, "Fuck Her", dudesFuckEggLayingBitches);
				if (flags[FlagEnum.ANT_KIDS] > 10 && player.stats.cor >= 75) MainScreen.addButton(3, "Orgy (Male)", orgyWithDatColonyCorruptDudes);
			}
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				...
				if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0 && flags[FlagEnum.ANT_KIDS] >= 10) MainScreen.addButton(2, "Lesbian Sex", birfingSexWithAntsForDasLadies);
				//Orgy w/ Colony (Female)
				//You tell Phylla you're interested in 'inspecting' your children.
				if (flags[FlagEnum.ANT_KIDS] > 10 && player.stats.cor >= 75) MainScreen.addButton(4, "Orgy (Female)", antColonyOrgy4Ladies);
			}
		...
		private talkAboutAntMatingAndRituals():void
		{
			...
			//If corruption more than 60:
				...
				if (player.stats.cor >= 80) {
					...
					if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0) MainScreen.text("milk dripping ");
					...
					if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0) MainScreen.text(", sending jets of milk shooting out onto the floor");
			...
			flags[FlagEnum.PHYLLA_INHERITED_KNOWLEDGE] = 1;
			doNext(camp.returnToCampUseOneHour);
		}
...
//(Phylla's Life Past & Future)
		private phyllasLifePastAndFuture():void
		{
			MainScreen.clearText();
			//(If Izma at Camp)
			if (kGAMECLASS.izmaScene.izmaFollower() && flags[FlagEnum.PHYLLA_IZMA_TALK] == 0) {
				flags[FlagEnum.PHYLLA_IZMA_TALK] = 1;
				...
				if (player.stats.cor < 40) {
					...
					if (flags[FlagEnum.PHYLLA_INHERITED_KNOWLEDGE] == 1) {
...
//First Time Blowjob:
		private phyllaBeeeJays():void
		{
			...
			if (flags[FlagEnum.PHYLLA_BLOWJOBS] == 1) {
			...
			if (player.cockArea(x) < 60) {
				...
				if (player.lowerBody.cockSpot.list[x].cockLength < 5) MainScreen.text(" \"tall\" and proud, rock hard and ready for action.");
				...
				//Dick size more than 60 inches (subsequent blowjobs):
					...
					if (flags[FlagEnum.PHYLLA_BLOWJOBS] == 2) MainScreen.text("last time");
					...
					if (player.lowerBody.cockSpot.list[x].cockLength < 16) MainScreen.text("base");
					else MainScreen.text("middle");
					MainScreen.text(" of your shaft against her tongue.  You hear a mix of a giggle and hum as she drags her tongue towards the head of your " + cockDescript(x) + ".  Phylla teases your shaft as she reaches the peak and suckles at your urethral opening.  You feel her drawing up as much pre-cum as she can and lapping it up like she's dying of thirst.  She continues this for a bit, until it becomes apparent that she's either not too sure of what else to do, or that she's never heard of how to properly fellate someone.");
...
//Pure BJ Ending
		private pureBJEnding(linked:boolean = true):void
		{
			...
			else if (flags[FlagEnum.TIMES_LINKED_BJ_SUCK] == 0) {
				MainScreen.text(", ");
			...
			if (flags[FlagEnum.TIMES_LINKED_BJ_SUCK] > 0) MainScreen.text("  You can feel her relying less and less on the link with every subsequent time; there's no doubt in your mind that she will no longer \"<i>cheat</i>\" her way into making you cum before you know it.");
			let x: number = player.smallestCockIndex();
...
//Corrupt BJ Ending
		private corruptPhyllaEndings():void
		{
			MainScreen.clearText();
			let x: number = player.smallestCockIndex();
			MainScreen.text("\"<i>A slut that can't suck a dick... how can this get any worse?</i>\"  You ponder to yourself while rubbing your nasal bridge. You tell the innocent little ant that she'll learn how to suck a mean cock, even if it kills her.  You're knee deep at this point - might as well turn her into something salvageable.  Commanding her attention, you lift her face up and hover your " + cockDescript(x) + " close to her lips, smothering her lips with your precum.  She attempts to turn away but you react before she can.  Holding her head in place you comment on how this is what you want from her.");
			...
			//If first time:
			if (flags[FlagEnum.PHYLLA_BLOWJOBS] == 1) MainScreen.text("achieved the desired effect.");
			//Every other time:
...
//\"<i>Use Dick</i>\"
		private dickPhylla():void
		{
			...
			//(NO BJ experience)
			if (flags[FlagEnum.PHYLLA_BLOWJOBS] == 0) {
			...
			//3-4 times:
			if (flags[FlagEnum.PHYLLA_FUCKS] >= 3) MainScreen.text(" she asks playfully, knowing full well that that's what you came here for.");
			//(First time)
			else if (flags[FlagEnum.PHYLLA_FUCKS] == 0) MainScreen.text(" You nod quickly.");
			//(Subsequent fucks)
			if (flags[FlagEnum.PHYLLA_FUCKS] > 0) MainScreen.text("\n\n\"<i>Do you have to ask?</i>\"  You ask her while grinning mischievously; something that causes her to smirk back at you.");

			//3-4 times:
			if (flags[FlagEnum.PHYLLA_FUCKS] >= 3) MainScreen.text("\n\nShe seems like she's losing some of her shyness, or at the very least feels comfortable enough around you to assert more of her personality.");
...
//While Giving Birth (Female) - Written
		private birfingSexWithAntsForDasLadies():void
		{
			...
			if (player.lowerBody.cockSpot.count() > 0 && player.cockThatFits(42) >= 0) {
				...
				MainScreen.text("your " + multiCockDescriptLight() + " into her mouth and mashes ");
...
//Orgy w/ Colony (Male) - Written
		private orgyWithDatColonyCorruptDudes():void
		{
			...
			//First Time:
			if (flags[FlagEnum.TIMES_CORRUPT_MALE_ANT_ORGY] == 1) {
			...
			if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0) MainScreen.text("large, lactating ");
...
//You tell Phylla you're interested in 'inspecting' your children.
		private antColonyOrgy4Ladies():void
		{
			...
			//(First Time)
			if (flags[FlagEnum.TIMES_CORRUPT_FEMALE_ANT_ORGY] == 1) {
...
//►[Lay Eggs / Don't Lay Eggs]
		private phyllaLaysEggsToggle():void
		{
			MainScreen.clearText();
			//(Lay Eggs)
			if (flags[FlagEnum.PHYLLA_EGG_LAYING] == 0) {
				flags[FlagEnum.PHYLLA_EGG_LAYING] = 1;
				...
				if (flags[FlagEnum.TALKED_WITH_PHYLLA_ABOUT_HISTORY] > 0) MainScreen.text("  You tell her you want a colony like those she spoke of in the times before the corruption.");
			...
			//(Stop Laying Eggs)
				...
				flags[FlagEnum.PHYLLA_EGG_LAYING] = 0;
...
//LICK THAT!
		private lickThatAntButt():void
		{
			...
			//First Time:
			if (flags[FlagEnum.ANTS_BIRTHED_FROM_LICKING] == 0) {
			...
			flags[FlagEnum.PHYLLA_COOLDOWN] = 12;
			flags[FlagEnum.ANTS_BIRTHED_FROM_LICKING]++;
			if (flags[FlagEnum.ANT_KIDS] < 5000) flags[FlagEnum.ANT_KIDS] += 5;
			doNext(camp.returnToCampUseOneHour);
		}
...
//DON'T LICK THAT!
		private dontLickAntButt():void
		{
			...
			if (flags[FlagEnum.ANT_KIDS] > 1) MainScreen.text("; the children can tend to the eggs and I can really use the rest");
			...
			flags[FlagEnum.PHYLLA_COOLDOWN] = 6;
			flags[FlagEnum.ANT_KIDS]++;
			doNext(camp.returnToCampUseOneHour);
		}
...
//Phylla lays Drider eggs
		private phyllaLaysSomeDriderEggs():void
		{
			...
			if (player.stats.cor < 75) {
				...
				if (flags[FlagEnum.PHYLLA_EGG_LAYING] > 0) MainScreen.text("both eggs are just fine on their own");
			...
			flags[FlagEnum.PHYLLA_TIMES_DRIDER_EGG_LAYED]++;
			flags[FlagEnum.PHYLLA_DRIDER_BABIES_COUNT] += 5 + rand(4);
		}
...
//(Leads to - Failure)
		private phyllaDigsForGems():void
		{
			...
			if (flags[FlagEnum.ANT_KIDS] > 10) kidsMod++;
			if (flags[FlagEnum.ANT_KIDS] > 50) kidsMod++;
			if (flags[FlagEnum.ANT_KIDS] > 150) kidsMod++;
			if (flags[FlagEnum.ANT_KIDS] > 300) kidsMod++;
			if (flags[FlagEnum.ANT_KIDS] > 600) kidsMod++;
			if (flags[FlagEnum.ANT_KIDS] > 1000) kidsMod++;
			if (flags[FlagEnum.ANT_KIDS] > 2000) kidsMod++;
			//Success
			if (flags[FlagEnum.PHYLLA_GEMS_HUNTED_TODAY] == 0 && rand(20) + kidsMod > 10) {
				let gems: number = 0;
			...
			flags[FlagEnum.PHYLLA_GEMS_HUNTED_TODAY] = 1;
			doNext(camp.returnToCampUseOneHour);
		}
...
//Sex > [Egg Phylla]
		private eggDatBitch():void
		{
			...
			//First Time:
			if (flags[FlagEnum.TIMES_EGG_IMPREGNATING_PHYLLA] == 0) {
			...
			//Persuasion Success
				...
				if (!pregnancy.isPregnant) pregnancy.knockUpForce(PregnancyType.DRIDER_EGGS, 8 * 24); //Supposed to be eight days, not eight hours
				flags[FlagEnum.TIMES_EGG_IMPREGNATING_PHYLLA]++;
				player.orgasm();
				player.dumpEggs();
			}
			doNext(camp.returnToCampUseOneHour);
