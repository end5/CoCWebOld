 ...
 */
export default class AntsScene implements TimeAwareInterface {
	...
	public timeChange(): boolean {
		...
		if (Flags.list[FlagEnum.PHYLLA_EGG_LAYING] > 0 && rand(5) == 0 && Flags.list[FlagEnum.ANT_KIDS] < 5000) Flags.list[FlagEnum.ANT_KIDS]++;
		if (model.time.hours > 23) {
			...
			if (Flags.list[FlagEnum.PHYLLA_EGG_LAYING] > 0) Flags.list[FlagEnum.DAYS_PHYLLA_HAS_SPENT_BIRTHING]++;
			if (Flags.list[FlagEnum.PHYLLA_GEMS_HUNTED_TODAY] > 0) Flags.list[FlagEnum.PHYLLA_GEMS_HUNTED_TODAY] = 0;
			if (phyllaWaifu()) Flags.list[FlagEnum.DAYS_PHYLLA_IN_CAMP]++;
		}
		return false;
	}
	...
	public phyllaWaifu(): boolean {
		return Flags.list[FlagEnum.ANT_WAIFU] > 0;
	}

	public antColonyEncounter(): void {
		//WAIFU GET!
		trace("ANT WINS: " + Flags.list[FlagEnum.ANT_ARENA_WINS] + " ANT LOSSES: " + Flags.list[FlagEnum.ANT_ARENA_LOSSES]);
		if (Flags.list[FlagEnum.ANT_ARENA_WINS] - Flags.list[FlagEnum.ANT_ARENA_LOSSES] >= 2 && Flags.list[FlagEnum.ANT_ARENA_WINS] >= 4 && player.gender > 0) {
			if (Flags.list[FlagEnum.PHYLLA_STAY_HOME] > 0) bumpIntoTheAntColonyAfterStayHomePhylla();
			else antGirlGoodEnd();
		}
		else if (Flags.list[FlagEnum.PC_READY_FOR_ANT_COLONY_CHALLENGE] == 1)
			antColonyChallenge();
		else if (Flags.list[FlagEnum.PHYLLA_SAVED] == 1)
			enterTheColony();
		else firstAntColonyEncounter();
	}

	private phyllaCapacity(): number {
		return Flags.list[FlagEnum.PHYLLA_CAPACITY];
	}
	...
	private firstAntColonyEncounter(): void {
		...
		MainScreen.text("While traversing an unfamiliar part of this desert wasteland, ");
		if (player.stats.cor > 75 && player.stats.lust > 50) MainScreen.text(" looking for something to slake your unquenchable lusts, ");
		MainScreen.text("you come across an abandoned merchant's cart.  The cart looks eerily disheveled, with a majority of its contents strewn about the sand.  Whoever or whatever deserted this thing did so in a hurry, and for seemingly no reason. Curious, you decide to inspect it for anything worthwhile.");
	...
	private keepHidden(): void {
		Flags.list[FlagEnum.ANT_COLONY_KEPT_HIDDEN] = 1;
		...
		//►(If Over 41 - Male)
			...
			MainScreen.text(images.showImage("ants-desert-male-firstencounter"));
			MainScreen.text("You know exactly what's going to happen once the demons 'subdue' their prey.  The growing throbbing in your loins begs for release, and you quickly and quietly take off your " + player.inventory.armor.displayName + ", making sure the demons don't notice you.  You might as well watch the show as you're not gonna be able to leave unnoticed until the demons are done anyways.");
			MainScreen.text("\n\nWatching from your hiding spot you start to stroke your " + CockDescriptor.describeCock(player, 0) + ".");
			//Start Dick Size Check
			let x: number = player.biggestCockIndex();
			//If player can auto-fallatio:
			if (player.lowerBody.cockSpot.get(x).cockLength >= 20) MainScreen.text("  Licking the head of your [cock " + (x + 1) + "] in anticipation of what's to come only adds to your growing lust.");
			//If player cock size tiny:
			else if (player.cockArea(x) < 6) MainScreen.text("  Though your cock isn't the most impressive, it's 'got it where it counts' and you intend to make the most of it.  You lovingly tease yourself until you're fully erect; running your fingers down the ridge causes a bead of pre-cum to form on your tip.");
			//If player cock size normal:
			else if (player.cockArea(x) < 12) MainScreen.text("  You hold your cock like the hilt of a sword and start to pump your shaft slowly.  Your breathing becomes heavier and labored, as you stroke yourself with mounting passion.");
			//If player cock size large:
			else if (player.cockArea(x) < 30) MainScreen.text("  The fear of having your cock give away your position quickly passes through your mind, but it's too late now.  The only way out of this is to cum, so you might as well enjoy it.  You firmly grasp your cock and start jacking off, hoping that you'll finish sooner rather than later.");
			//If player cock size MASSIVE:
			else MainScreen.text("  The towering meat log jutting from your crotch gives away your 'hiding' spot, but the demons don't seem to mind, as they're too busy with their new toy.  You doubt even if they did care that any of them would be able to satisfy you. You wrap your arms around your fully erect cock and start to pump, hoping to release your load soon.");
			...
			MainScreen.text("\n\nThe hermaphrodite struggles to cram itself inside the ant-girl and eventually succeeds.  With each stomach-contorting thrust you hear the ant's muffled moans of pain and ecstasy from between the legs of the female demon.  The hermaphrodite quickens his thrusts and, in turn, you keep speed.  With near-precision timing, you and the demon finish together.  " + CockDescriptor.describeMultiCockSimpleOne(player, true) + " throbs and a wave of euphoria washes over you.  Groaning in pleasure you release your ");
			//[no new PG, Start Cum Check
			if (player.cumQ() < 100) MainScreen.text("sizable");
			else if (player.cumQ() < 1000) MainScreen.text("huge");
			else if (player.cumQ() < 10000) MainScreen.text("massive");
			else MainScreen.text("titanic");
			MainScreen.text(" load.  Once you can refocus your eyes, you see the hermaphrodite pull out, and as it does a burst of cum sprays out of the ant-morph's vagina as her stomach deflates slightly.  She's been filled with so much corrupt seed that it's stretched her belly to the point it resembles a third-trimester pregnancy.");
		...
		//►If Over 41 - Female
			...
			MainScreen.text(images.showImage("ants-desert-female-firstencounter"));
			MainScreen.text("Watching from your hiding spot, you consider what's going to happen and feel your pussy immediately moisten.  Your practiced hands move with deftness, slipping into your [armor].");
			...
			//If PC is lactating:
			if (player.lactationQ() > 100) MainScreen.text("  You feel your warm milk flow out of your hardened arola, as your body begs to release its sweet nectar.");
			//If PC has dick(s):
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  Feeling [eachCock] throb between your legs, you run one of your hands down your chest and start to stroke lovingly.");
	...
	private playHero(): void {
		...
		MainScreen.text("As the demons bear down on the ant-girl, you burst from your hiding place, raising your [weapon] to the air and uttering an impressive war cry.  Nobody, ant or otherwise, is getting raped if you have any say in the matter!");
		MainScreen.text("\n\nYou are now fighting demons!");
		startCombat(new DemonPack());
		monster.statusAffects.add(new StatusAffect("phyllafight", 0, 0, 0, 0)));
		doNext(playerMenu);
	}
	...
	internal function consolePhylla(): void {
		...
		Flags.list[FlagEnum.PHYLLA_SAVED] = 1;
		cleanupAfterCombat();
	}
	...
	private enterTheColony(): void {
		...
		//[(corruption < 40)
		if (player.stats.cor < 40) MainScreen.text("\"<i>'Champion', you've been sent to this land to resist the demons and your efforts thus far, insignificant though they be, have not escaped my gaze.  ");
		else MainScreen.text("\"<i>Despite your obvious taint, you seem to have retained your valor as the 'Champion' you proclaim yourself to be, and not as a rutting beast.  Something to be admired, for sure.  ");
		MainScreen.text("In saving my daughter, Phylla...</i>\"  She pauses and gestures, and the four-armed girl you saved from the demon horde walks slowly and shyly out of a nearby tunnel to stand next to her mother.  'Phylla' looks very embarrassed and is blushing deeply, avoiding eye contact from both of you by looking at the ground as Queen Chylla continues her dry monologue.  \"<i>... you have been chosen to become her potential...'mate'; much to my dismay.</i>\"  Wait, mate?  \"<i>My daughter has shown me how you saved her from the demons that inhabit this land.  Perhaps you were just lucky and caught them with their cocks in their hands while her back was turned?</i>\"");
		...
		Flags.list[FlagEnum.PC_READY_FOR_ANT_COLONY_CHALLENGE] = 1;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private antColonyChallenge(): void {
		...
		if (Flags.list[FlagEnum.MET_ANT_ARENA] == 0) {
			...
			else if (player.stats.str > 50) MainScreen.text("\n\n\"<i>Well, you certainly look the part.  But you're going to need more than brute strength to win here.</i>\"");
			//Else if intelligence over 50:
			else if (player.stats.int > 50) MainScreen.text("\n\nYou make a witty quip about how he might be making puppy eyes at someone who saved him from a horde of giant demon cocks, too, but he doesn't receive it well.");
			MainScreen.text("  After a moment he looks down and scribbles something on his chart.");
			MainScreen.text("\n\n\"<i>So, we're ready to start when you are.</i>\"");
			Flags.list[FlagEnum.MET_ANT_ARENA]++;
		}
		//Introduction Subsequent Times
		else {
			MainScreen.text("Your feet bring you back to the ant colony and the guard motions you down the only lit tunnel once more.  You enter the colosseum, and as you step out into the cavern, you are greeted, again, by a thin male ant-morph holding a clipboard.  He looks up at you.");
	...
	private leaveAntColony(): void {
		MainScreen.clearText();
		MainScreen.text("Deciding to better prepare yourself first, you inform the thin fight manager that you will return later.  You leave the colony, heading back to camp.");
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private antColiseumFight(): void {
		...
		//►[Fight #3]
			...
			//(Gnoll - Intro)
			if (Flags.list[FlagEnum.MET_ANT_ARENA_GNOLL] == 0) MainScreen.text("With due ceremony, the manager leads you down to one of the staging areas for the arena.  You watch through the bars as the stadium fills far past capacity.  There must be a thousand ants here... maybe even the whole colony.  Gazing out into the crowd, you pick out the royal seating and in it, Princess Phylla.  The princess is in much finer attire than you're used to seeing - her chest is actually covered by a fine red and blue dress and her hair is combed, framing her face nicely.  Her mother is nowhere to be seen, and you can tell that Phylla is taking full advantage.  She waves and cheers for you just like the crowd is and, perhaps by her orchestration, you hear the crowd pick up a chant in your name.  Seeing Phylla chant with them makes you feel invigorated.  As the gates raise, you charge out into the center of the arena, roaring your finest battle cry; your opponent is taken somewhat aback but finds resolve and braces for your charge.");
			//►(Gnoll - Intro Repeat - for people that lost once)
			else MainScreen.text("You move through the network of tunnels and caves and come to the colosseum once more.  You are greeted by the event manager and he ushers you down to one of the staging areas for the arena.  The noise intensifies as the colosseum fills to the brim with spectators; Phylla herself is up in her box waving to you.  You look across the arena floor and the gnoll at the other side of the arena seems just as excited to fight as you are.  The gates open and you charge each other!");
			MainScreen.text("\n\nYou're fighting a gnoll!");
			Flags.list[FlagEnum.MET_ANT_ARENA_GNOLL]++;
	...
	public phyllaTentacleDefeat(): void {
		...
		fatigue(10);
		//(+1 Win Score)
		Flags.list[FlagEnum.ANT_ARENA_WINS]++;
		cleanupAfterCombat();
	}
	...
	public phyllaTentaclePCLoss(): void {
		MainScreen.clearText();
		MainScreen.text("After your defeat in the arena, you set off to leave but are stopped.  Princess Phylla is standing with a troupe of armed guards by the exit, holding a vial of clear liquid.  She doesn't look like she wants to give it to you but she extends it to you all the same.");
		...
		// (+1 Loss Score)
		Flags.list[FlagEnum.ANT_ARENA_LOSSES]++;
		if (Flags.list[FlagEnum.ANT_ARENA_LOSSES] > 3) {
			antastrophyBadEnd();
			return;
		}
		cleanupAfterCombat();
	...
	public phyllaBeatAMino(): void {
		MainScreen.text("As you exit the arena, amidst the cheers and roars of the crowd, you are met by Princess Phylla and slightly fewer guards than the last time.  She sees you coming and her face lights up, then runs over to you and starts doting over you. You smile and let her do her thing.  Once she's done, she looks deep into your eyes.");
	...
	public phyllaPCLostToMino(): void {
		MainScreen.text("After your staggering defeat and subsequent humiliation in the arena two guards approach you as you try to leave.  One of them holds out a small vial.  Princess Phylla is off in the corner of the room crying, and you try to move closer to her but one of the guards steps in front of you.  \"<i>Drink!</i>\" he commands, uncorking the vial.");
	...
	public phyllaPCBeatsGnoll(): void {
		...
		if (Flags.list[FlagEnum.ANTS_PC_BEAT_GNOLL] == 0) {
			...
			Flags.list[FlagEnum.ANTS_PC_BEAT_GNOLL]++;
		}
		else if (Flags.list[FlagEnum.ANT_ARENA_WINS] - Flags.list[FlagEnum.ANT_ARENA_LOSSES] >= 2 && Flags.list[FlagEnum.ANT_ARENA_WINS] >= 3 && player.gender > 0) {
			Game.inCombat = false;
			Flags.list[FlagEnum.ANT_ARENA_WINS]++;
			antGirlGoodEnd();
			return;
		...
		//►(Gnoll - Win Repeat) Standard Gnoll Win Scene. +
			...
			MainScreen.text("This isn't the first time you've accomplished this, but it still brings the crowd to their feet.  You simply stand in the center of the arena, taking it all in.  Phylla makes her way to you, blushing as the rowdy audience follows her with catcalls and demands for her to give her champion a kiss.  She tends to you as always and, when she's done, takes your hand in hers and looks at you expectantly.  You smile at her, brushing the hair from her face with your other hand; she colors deeply at the contact as two guards appear to lead you both off.  Having no more reason to stay, you allow your guide to return you to the surface.");
		}
		//(+1 Win Score)
		Flags.list[FlagEnum.ANT_ARENA_WINS]++;
		cleanupAfterCombat();
	...
	private antastrophyBadEnd(): void {
		MainScreen.clearText();
		MainScreen.text("As you go to leave the arena queen Chylla and four bodyguards approach you.");
	...
	private antGirlGoodEnd(): void {
		...
		menu();
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(0, "Use Penis", gigititigitigitigitigityAntGirl);
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.addButton(1, "Use Vagina", femalePhyllaFirstFuckGooooo);
	}

	//►Male Continuation
	private gigititigitigitigitigityAntGirl(): void {
		MainScreen.clearText();
		MainScreen.text("Sporting a painfully obvious tent, your body betrays its desire for sexual gratification and it causes Phylla to smile with interest.  \"<i>I-I didn't think I had that ability to... to...  I mean I'm not like you, so I thought...</i>\" she begins, but trails off as she takes in the rough, obscured shape given off by your bulge.");
		...
		MainScreen.text("\n\nToo late!  With a final groan and a rip, your pants give way to the force and power of your quickly hardening " + CockDescriptor.describeMultiCockShort(player) + ", sending a cursed button flying asunder from your waistline. The button connects with Phylla's forehead with a mighty 'thwack' and careens off her skull.  Fortunately for you, and her, she's more fixated on what's coming out of your pants rather than the pants themselves.");
		//Dick Size huge:
		if (player.cockArea(x) >= 72) {
			...
			//PC has more than one dick:
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("\n\nOverwhelmed by the sight of ONE of your members, Phylla arguably goes catatonic when she sees all of what your packing.  She probably won't be able to handle the one, let alone all of your " + CockDescriptor.describeMultiCockShort(player) + "!");
			...
			if (player.lowerBody.cockSpot.countType(CockType.TENTACLE) > 0) {
				...
				MainScreen.text(" forward and wrap them tightly around Phylla, taking her hand and tugging her in close.");
			}
			...
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
			MainScreen.text(" from around her and hang it in front of her face, grinning your antics on your face as Phylla stares at ");
			if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("it");
			else MainScreen.text("them");
			MainScreen.text(", slightly intimidated.");
			...
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
			MainScreen.text(" looking for something more sizeable.  You can tell Phylla is more scared of what might happen to her than what might happen to you.");
		...
		if (player.cockArea(x) <= phyllaCapacity()) {
			MainScreen.addButton(0, "Next", malePhyllaContinuation, x);
		}
		//If PC has dick(s) that won't fit: Jump to - Cunnilingus Scene Continuation
		else {
			MainScreen.addButton(0, "Next", cuntmuffinLingusPhyllaDickBig);
		}
	}

	private malePhyllaContinuation(x: number): void {
		MainScreen.clearText();
		MainScreen.text("Phylla stares down at your " + CockDescriptor.describeMultiCockShort(player) + " and starts nervously rubbing her hands together.  \"<i>It's my first time... I mean, I hope it isn't going to be bad for you...</i>\" she says, before trailing off; uncertain of her sexual prowess.");
		//Corruption less than 75:
		if (player.stats.cor < 75) MainScreen.text("\n\nYou assure her that she shouldn't worry so much about her first time.");
		...
		//PC is goo morph:
		if (player.lowerBody.isGoo()) MainScreen.text(", leaving traces of your slimy lust along her body for her to wear, allowing your detached bits of mass to ignite her sexual fires");
		MainScreen.text(".");
		...
		MainScreen.text("\n\nSimply dragging your " + CockDescriptor.describeCock(player, x) + " along the entrance her of moist pussy causes her to moan and wiggle under you. Parting her pink little lips with the head of your cock, you finally hit the sweet spot for penetration. With ");
		if (player.stats.cor < 75) MainScreen.text("a gentle amount of force, ");
		else MainScreen.text("an uncaring, brutish amount of force, ");
		MainScreen.text("you push past the tight opening of Phylla's folds.  You hear Phylla inhale sharply as she stretches to accommodate your width. In what seems to be a prolonged amount of time, you finally manage to ease the head of your " + CockDescriptor.describeCock(player, x) + " into her.  The act takes a loud and pained turn as Phylla moans and shifts in obvious discomfort.  You feel her squeeze her vaginal walls tightly around your cock as her considerable strength almost cuts off blood flow.  After a moment, Phylla realizes how much pressure she's exerting and her body relaxes, realizing the potential for injuring her partner.  Sighing heavily, she moves her hips down to take even more of you in. As you slide further in, her eyes flash with pain as she bites down on her lip, it becomes glaringly clear that something has yielded inside of Phylla.");
		...
		if (player.stats.cor < 75) MainScreen.addButton(0, "Next", phyllaFirstTimePureBabiesFuckEnding);
		//(Jump to Corruption more than 75 - Corrupt Ending)
		else MainScreen.addButton(0, "Next", phyllaCorruptMascEnding);
	}
	...
	private phyllaFirstTimePureBabiesFuckEnding(): void {
		...
		if (player.cumQ() >= 100 && player.cumQ() < 1000) MainScreen.text("  Truly amazing considering how much your loads usually contain.  You think you see a little bulge in her stomach that wasn't there before.");
		//If cum milliliters over 1000:
		else if (player.cumQ() >= 1000) MainScreen.text("  As you look down at her nethers your mind struggles to understand just how she could... then looking up her body, you see.  Her belly is bloated with your sperm, almost to the point of already looking pregnant.");
		...
		if (player.canOvipositSpider()) {
			...
			if (player.stats.int / 20 + rand(20) + 1 < 16) {
				//Persuasion failure:
				...
				if (pregnancy.isPregnant) MainScreen.text("  I just can't hold anything else inside me. I'm sorry! Please don't be mad... I mean I will!  Just a-after... this batch...");
			...
			//Persuasion success:
			{
				...
				if (player.stats.cor >= 75) MainScreen.text("  Little does poor Phylla know, you're going to just use her as a baby maker to ease the tension of your own sacks.");
				...
				player.dumpEggs();
				if (!pregnancy.isPregnant) pregnancy.knockUpForce(PregnancyType.DRIDER_EGGS, 8 * 24); //Supposed to be eight days, not eight hours
			}
		...
		player.orgasm();
		menu();
		MainScreen.addButton(0, "Next", waifuQuestOver);
	}
	...
	private phyllaCorruptMascEnding(): void {
		...
		MainScreen.text(", but it will only serve to help you.  You will need a brace for what you're about to pull and her rigid abdomen will do just fine.  Phylla makes a confused noise, glancing over her shoulder with a look of concern growing on her face.");
		MainScreen.text("\n\n\"<i>W-What are you doing?</i>\" she asks nervously. \"<i>This isn't how I imag~</i>\"");
		let x: number = player.cockThatFits(phyllaCapacity());
		if (x < 0) x = player.smallestCockIndex();

		MainScreen.text("\n\nWithout letting her finish, you plunge your " + CockDescriptor.describeCock(player, x) + " into her, causing her to yell out in sheer surprise and discomfort.  \"<i>This is how <b>I</b> mate!</i>\" you inform her, telling her that she won't EVER forget her first time with you.  Her mouth goes open slightly before your subsequent gyrations put an end to whatever she was going to sputter out.  Smirking like a mad jester at the position you have Phylla in, you begin to work your hips as you dominate her from behind.  You hear your ");
		if (player.lowerBody.balls == 0) MainScreen.text("thighs");
		else MainScreen.text(BallsDescriptor.describeBalls(true, true, player));
		MainScreen.text(" slap loudly off of her rump with every aggressive thrust against her clit.  She wails out in frustration at the lack of intimacy that you apparently have in mind.  The moaning that is mixed into her pathetic protests, however, suggests that the naive little woman is getting off on being treated so roughly; a hard slap on the ass affirms this as she whines out in annoyed pleasure.  You feel the growing heat from her pussy as it gets wetter and wetter, continuing your pressure on her abdomen as you use it to support your brutal fucking of her cunt.");
		//PC has Goo Body:
		if (player.lowerBody.isGoo()) MainScreen.text("  Perhaps the poor dear could use some tender attention, you ponder to yourself.  Reducing the solidity of your body, you work her abdomen deep into your sticky, goopy body, re-solidifying once she's firmly in there and lowering your hands down to her nipples, pinching them hard as you continue your pumping of her rump.");
		...
		if (player.lowerBody.cockSpot.count() < 2) y = -1;
		if (y >= 0) {
			MainScreen.text("\n\nIntent on increasing your own pleasures, you decide to work ");
			if (player.lowerBody.cockSpot.count() > 2) MainScreen.text("one of your free cocks");
			else MainScreen.text("your free cock");
			MainScreen.text(" against her tight ass. Feeling this new intruder, she turns back to you.");
			...
			if (player.lowerBody.cockSpot.biggestCocks[0].cockArea() > phyllaCapacity()) {
		...
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s twitch, ");
		else MainScreen.text(" twitches, ");
		MainScreen.text("ushering forth your corrupted seed into her womb");
		if (y >= 0) MainScreen.text(" and her ass.");
		else MainScreen.text(", and onto the bedspread below.");
		MainScreen.text("  As her pulsing cunt works to suck the semen out of your ");
		if (player.lowerBody.balls > 0) MainScreen.text("balls");
		else MainScreen.text("shaft");
		MainScreen.text(", Phylla retaliates with her own orgasm, flooding your groin with her sticky girl cum.  You glance down to your joined nethers and find a massive amount of Phylla's cum has pooled around your knees and more seems to be flowing every second.  Clearly, she's a squirter.");
	...
	private cuntmuffinLingusPhyllaDickBig(): void {
		MainScreen.clearText();
		MainScreen.text("Sadly, as you gaze down at your " + CockDescriptor.describeMultiCockShort(player) + ", you realize that trying to penetrate Phylla with your freakishly large prick");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(" isn't going to work.  Phylla's eyes begin to tear up as she also comes to the realization that her first time is a non-starter.");
		...
		MainScreen.text("\n\n\"<i>W-What are you doing?</i>\" she asks, somewhat upset that she won't be getting the traditional first time sex.  Arcing your " + CockDescriptor.describeMultiCockShort(player) + ", you ");
		//Dick Size Over 48 but Under 69 inches:
		if (player.longestCockLength() < 69) {
			MainScreen.text("angle your hips so that ");
			if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("it points");
			else MainScreen.text("they point");
			MainScreen.text(" at the ceiling, placing them out of the way for what you have in mind.");
		}
		//Dick size Over 70 inches::
			...
			MainScreen.text(" aimed out the doorway.");
		}
		MainScreen.text("  You wickedly beckon Phylla over to you with a hooked finger.  Phylla looks at you quizzically for a moment before it dawns on her.  Her eyes widen and cooing she scrambles over to you.");
		...
		if (player.lowerBody.cockSpot.count() == 1) MainScreen.text(" switches and bobs");
		else MainScreen.text("s switch and bob");
		MainScreen.text(", loudly brushing against the ");
		if (player.longestCockLength() < 69) MainScreen.text("ceiling");
		else MainScreen.text("wall in the hallway");
		MainScreen.text(".");
		...
		//If Snake Tongue:
		if (player.upperBody.head.face.tongueType == TongueType.SNAKE) MainScreen.text("  You run the split of your forked mouth muscle from the tip of her clit to the base.  As your textured feeler makes its way down, you wrap around the rest of her long clitorus, like a boa constrictor.  Once your split reaches the hood of her clit you roll and flick the tip of your tongue rapidly.  Pulling at your hair she gasps for breath in between teeth grinding moans.");
		...
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(" and pants heavily, having just gone through the equivalent of an orgasm triathlon.  As you sigh, you can hear her giggle in what sounds like disbelief.  \"<i>Heh... I didn't think I had it in me to do... that,</i>\" she coos, staring mischievously up at you.  \"<i>Just... could you shrink ");
		if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("this monster");
		else MainScreen.text("these monsters");
		MainScreen.text(" down?  I mean, I'm not complaining!  But I... just... I mean... you... inside me,</i>\" she shyly remarks, obviously wanting something more... traditional.");
		//(Player lust increases to 100)
		dynStats("lus=", 100);
		Flags.list[FlagEnum.DIDNT_FUCK_PHYLLA_ON_RECRUITMENT] = 1;
	...
	private femalePhyllaFirstFuckGooooo(): void {
		MainScreen.clearText();
		MainScreen.text("Eager to get to some good old fashion fun, you direct your thoughts to Phylla, telling her to 'remove her clothing'.  After all of the pieces of her royal attire are removed, she closes in and rubs her naked body against yours.  Her fingers move fluidly as she slowly traces her ever hardening nipples along your chest and along the ridges of your back, sending a sensual chill up and down your spine.  Your " + BreastDescriptor.describeNipple(0) + "s harden in response, and with surprising speed she shifts her body downward and locks her mouth around your nipple, firmly suckling at your hardened areola.");
		//If PC Lactating:
		if (player.lactationQ() > 250) MainScreen.text("\n\nYou can feel her suck some of the milk from your breasts, slating her weak thirst in the process as she becomes immensely aroused at the sensation of \"nursing\" from her lover's " + BreastDescriptor.describeBreastRow(player.upperBody.chest.get(0)) + ".  Weird... you can taste the nutritious warmth of your nectar as it flows down her throat and into her stomach.  Drinking and tasting one's own breast milk could definitely weird someone out, but the foreignness of the act is an odd turn-on for you, and you let her continue to suckle.");
	...
	private girlFiller(): void {
		...
		MainScreen.text("\"<i>I-I've... never,</i>\" she stammers, but you just want her to experience this before she ruins the moment.");
		...
		menu();
		MainScreen.addButton(0, "Next", scissorContinue, true);
	}
	...
	private femalePhyllaFirstTimePlusCock(): void {
		...
		MainScreen.text(" has taken");
		MainScreen.text(" hold of ");
		//One dick only:
		if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("your " + CockDescriptor.describeCock(player, 0) + ".");
		else MainScreen.text("your " + CockDescriptor.describeCock(player, 0) + " leaving the lucky member to bask in affectionate glory as the rest of your manhood laments being left out.  \"<i>Don't worry, I have four hands...</i>\" Phylla coos as she works your shaft.");
		...
		if (player.smallestCockArea() > phyllaCapacity()) {
			//Dicks too Big:
			MainScreen.text("\n\nDespite your best efforts to try and get into a comfortable position to scissor, your painfully erect  " + CockDescriptor.describeMultiCockShort(player) + " prevent you from doing so, given that they are now compressed against the ceiling.  While the display of rainbows is pretty to watch on your " + CockDescriptor.describeMultiCockShort(player) + ", the crushing pain and the unyielding nature of your " + CockDescriptor.describeMultiCockShort(player) + " is too much.  ");

			//Horse: (Cont from ...is too much.)
			if (player.lowerBody.isTaur()) MainScreen.text("Seeing your impending predicament, Phylla quickly scrambles with all four of her arms to collect a bunch of cushions.  She stacks them in a makeshift 'resting place' and helps you into a comfortable position as your cocks precede to grow uncontrollably.");
			//Drider body:
			else if (player.lowerBody.isDrider()) MainScreen.text("Phylla immediately realizes this, and flips you over so that you are resting on your head and neck, ducking in past your " + CockDescriptor.describeMultiCockShort(player) + " and coming to rest on the border between your spider carapace and soft clit.");
			//Goo body:
			else if (player.lowerBody.isGoo()) MainScreen.text("Phylla immediately realizes this, and flips you over so that you are resting on your head and neck, and she works her way through your goopy mass, connecting her clit with yours in little time or effort. \"<i>This is a little weird... but I like it...</i>\" she flirts. Hardening your mass slightly, you snugly secure Phylla against your body, telling her that things \"might\" get rough.");
			//Human/Harpy/Naga legs:
			else MainScreen.text("Phylla immediately realizes this, and flips you over so that you are resting on your head and neck, and curls your [legs] towards her.");

			MainScreen.text("  Your " + CockDescriptor.describeMultiCockShort(player) + " now resting comfortably out of the open room, Phylla positions herself to scissor your pussy.");
		...
		//If PC has an appropriately sized dick(s):
			...
			MainScreen.text("\n\n[EachCock] hungers for attention, having been let go by Phylla when you shifted into a scissoring position.  Reaching down to start taking care of yourself; Phylla interrupts you abruptly.  \"<i>Oh, I'm sorry! I mean... I forgot... just feels... so gooood,</i>\" she coos between moans of ecstasy.  After a moment she seems to forget as waves of pleasure wash over her.  You make a playful throat clearing noise and gesture to your " + CockDescriptor.describeMultiCockShort(player) + ".  Phylla quickly snaps back into reality and takes hold of ");
			//Tentacle dick check
			if (player.lowerBody.cockSpot.countType(CockType.TENTACLE) > 0) {
				//PC has (a) Tentacle dick(s):
				MainScreen.text("your tentacle dick");
				if (player.lowerBody.cockSpot.countType(CockType.TENTACLE) > 1) MainScreen.text("s");
				MainScreen.text(".  Clearly she's never seen anything like it before as she doesn't know what to do.  Laughing, you tell Phylla she doesn't need to do anything except hold still.  You snake your tentacle cock");
				if (player.lowerBody.cockSpot.countType(CockType.TENTACLE) > 1) MainScreen.text("s");
				MainScreen.text(" up her body, making sure to caress her most tender places on their way.  She screams out loudly in shock only to realize what happened when she looks down.  \"<i>God, you scared me!</i>\" she yells, embarrassed that she got frightened half to death by your dick");
				if (player.lowerBody.cockSpot.countType(CockType.TENTACLE) > 1) MainScreen.text("s");
				MainScreen.text(".  Laughing at her display, you motion to your prick");
				if (player.lowerBody.cockSpot.countType(CockType.TENTACLE) > 1) MainScreen.text("s and wiggle them ");
				else MainScreen.text(" and wiggle it ");
				MainScreen.text("in front of her, like a worm to a bird in her face.  \"<i>Aw, does someone need some attention?</i>\" Phylla teases, taking ");
				//PC has:
				//one tentacle dick:
				if (player.lowerBody.cockSpot.countType(CockType.TENTACLE) == 1) MainScreen.text("your cock into her mouth as she continues to rock her hips into yours.");
				//two:
				else if (player.lowerBody.cockSpot.countType(CockType.TENTACLE) == 2) MainScreen.text("your cocks into her mouth and between her breasts as she continues to rock her hips into yours.");
				else MainScreen.text("your cocks into her mouth, between her breasts, and along her body.  The ones on her body tightly wrap around her insectile joints, using her muscular curves to get you off.");
				...
				if (player.lowerBody.cockSpot.count() == 2 && (player.cockArea(player.smallestCockIndex2()) + player.cockArea(player.smallestCockIndex()) <= phyllaCapacity()))
					MainScreen.text("two of your " + CockDescriptor.describeMultiCockShort(player) + ", stroking them apologetically as she scissors you.  You feel her almost insatiable appetite for all parts of your body.  You feel Phylla's mind slowly becoming lost in pleasure.");
				//TODO: "player.lowerBody.cockSpot.list[player.smallestCockIndex()] < 4" changed to cockLength. Not sure if it is the appropriate attribute.
				else if (player.lowerBody.cockSpot.list[player.smallestCockIndex()].cockLength < 4) MainScreen.text("your " + CockDescriptor.describeCock(player, player.smallestCockIndex()) + ", stroking it apologetically as she scissors you.  You feel her almost insatiable appetite for all parts of your body.  You feel Phylla's mind slowly becoming lost in pleasure.");
				//PC only has a dick(s) that('s) (are) more than 4 inches in width:
				else MainScreen.text("your " + CockDescriptor.describeCock(player, player.smallestCockIndex()) + ", but she simply can't fit any number of her hands around the entirety of your glans; instead, she takes to caressing the head of your " + CockDescriptor.describeCock(player, player.biggestCockIndex()) + ".");
	...
	private waifuQuestOver(): void {
		...
		menu();
		MainScreen.addButton(0, "Come2Camp", getAntWaifuYoShit);
		MainScreen.addButton(1, "Stay Here", tellPhyllaToStayTheFuckAtHomeThatCunt);
	}

	//[Come to Camp]
	private getAntWaifuYoShit(): void {
		MainScreen.clearText();
		MainScreen.text("You smile at her and tell her you would love for her to join you at your camp.  Her face brightens like the sun and she quickly gathers the very few possessions she owns - mostly clothing, the pillows, and some jewelry.  Together you promptly leave the colony and head back to camp.");
		MainScreen.text("\n\n(<b>Phylla has moved in!  She can be found in the lovers tab!</b>)");
		Flags.list[FlagEnum.ANT_WAIFU] = 1;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private tellPhyllaToStayTheFuckAtHomeThatCunt(): void {
		...
		Flags.list[FlagEnum.PHYLLA_STAY_HOME] = 1;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private bumpIntoTheAntColonyAfterStayHomePhylla(): void {
		MainScreen.clearText();
		MainScreen.text("You make your way down a strangely familiar path that leads to Phylla's room.  Her head quickly snaps around upon seeing your shadow in her doorway.");
	...
	public introductionToPhyllaFollower(): void {
		if (Flags.list[FlagEnum.PHYLLA_CAPACITY] < 50) Flags.list[FlagEnum.PHYLLA_CAPACITY] = 50;
		MainScreen.clearText();
		...
		if (Flags.list[FlagEnum.PHYLLA_CAMP_VISITS] == 0) {
			...
			if (Flags.list[FlagEnum.DIDNT_FUCK_PHYLLA_ON_RECRUITMENT] == 1 && player.lowerBody.cockSpot.hasCock()) {
		...
		menu();
		MainScreen.addButton(0, "Talk", phyllaTalkChoices);
		if (player.stats.lust >= 33) MainScreen.addButton(1, "Sex", phyllaSexMenu);
		if (Flags.list[FlagEnum.PHYLLA_EGG_LAYING] == 0) MainScreen.addButton(2, "Lay Eggs", phyllaLaysEggsToggle);
		else MainScreen.addButton(2, "No Eggs", phyllaLaysEggsToggle);
		if (Flags.list[FlagEnum.ANT_KIDS] > 0) MainScreen.addButton(3, "Children", phyllasKidsChildren);
		MainScreen.addButton(4, "Appearance", phyllaPearance);
		MainScreen.addButton(5, "Find Gems", phyllaDigsForGems);
		MainScreen.addButton(9, "Back", camp.campLoversMenu);

		Flags.list[FlagEnum.PHYLLA_CAMP_VISITS]++;
	}

	private phyllaSexMenu(): void {
		menu();
		if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.addButton(0, "Get BJ", phyllaBeeeJays);
			//\"<i>Use Dick</i>\"
			MainScreen.addButton(1, "Fuck Her", dickPhylla);
			//[While Giving Birth]
			//(Note: The above option will only be available if Phylla is 'Laying Eggs.')
			//While Giving Birth (Male) - Written
			if (Flags.list[FlagEnum.PHYLLA_EGG_LAYING] > 0 && Flags.list[FlagEnum.ANT_KIDS] >= 10) MainScreen.addButton(1, "Fuck Her", dudesFuckEggLayingBitches);
			if (Flags.list[FlagEnum.ANT_KIDS] > 10 && player.stats.cor >= 75) MainScreen.addButton(3, "Orgy (Male)", orgyWithDatColonyCorruptDudes);
		}
		//Straight Sex (Lesbian/Fisting) - Written
		if (player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.addButton(2, "Lesbian Sex", lesbianFisting);
			//While Giving Birth (Female) - Written
			if (Flags.list[FlagEnum.PHYLLA_EGG_LAYING] > 0 && Flags.list[FlagEnum.ANT_KIDS] >= 10) MainScreen.addButton(2, "Lesbian Sex", birfingSexWithAntsForDasLadies);
			//Orgy w/ Colony (Female)
			//You tell Phylla you're interested in 'inspecting' your children.
			if (Flags.list[FlagEnum.ANT_KIDS] > 10 && player.stats.cor >= 75) MainScreen.addButton(4, "Orgy (Female)", antColonyOrgy4Ladies);
		}
		//Drider/Bee impregnation scene for Phylla (universal unless otherwise specified, which will include varied intros and stuff.
		//Sex > [Egg Phylla]
		if (player.canOvipositSpider()) MainScreen.addButton(5, "Oviposit", eggDatBitch);
		MainScreen.addButton(9, "Back", introductionToPhyllaFollower);
	}

	private phyllaTalkChoices(): void {
		menu();
		MainScreen.addButton(0, "History", talkAboutAntHistory);
		//(Ant Morph Mating Ritual / Inherited Knowledge)
		MainScreen.addButton(1, "Mating", talkAboutAntMatingAndRituals);
		//(Phylla's Life Past & Future)
		MainScreen.addButton(2, "Her Life", phyllasLifePastAndFuture);
		MainScreen.addButton(9, "Back", introductionToPhyllaFollower);
	}
	...
	private talkAboutAntHistory(): void {
		MainScreen.clearText();
		Flags.list[FlagEnum.TALKED_WITH_PHYLLA_ABOUT_HISTORY] = 1;
		MainScreen.text("You relax on one of the many available pillows and ask Phylla about her race's history. She pulls a pillow up next to yours and cuddles in.  \"<i>It's not a very happy story, are you sure you want to hear it?</i>\" You nod and move your hand to hold one of hers.");
	...
	private talkAboutAntMatingAndRituals(): void {
		MainScreen.clearText();
		MainScreen.text("You've always wondered how Phylla can project thoughts and images of her life into your mind. Seeing as how she's not busy, you ask her about it.  She tilts her head, trying to think of a way to explain it.  You can see this is taking some thought as she stays silent for almost a minute.");
		...
		//If corruption more than 60:
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 4) MainScreen.text("\n\n\"<i>At least not like yours... I mean, yours are nicer!</i>\"");
			...
			if (player.stats.cor >= 80) {
				...
				if (Flags.list[FlagEnum.PHYLLA_EGG_LAYING] > 0) MainScreen.text(", sending jets of milk shooting out onto the floor");
				MainScreen.text(".  With a loud moan she orgasms, covering her smaller two forearms, her inner thighs, and the floor below her in thick, sweet-smelling, girl cum.");
		...
		Flags.list[FlagEnum.PHYLLA_INHERITED_KNOWLEDGE] = 1;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private phyllasLifePastAndFuture(): void {
		MainScreen.clearText();
		//(If Izma at Camp)
		if (kGAMECLASS.izmaScene.izmaFollower() && Flags.list[FlagEnum.PHYLLA_IZMA_TALK] == 0) {
			Flags.list[FlagEnum.PHYLLA_IZMA_TALK] = 1;
			MainScreen.text("Phylla seems to be a little distant today as you sit down to talk.  You point it out to her and she cautiously asks about Izma.");
			...
			if (player.stats.cor < 40) {
				...
				if (Flags.list[FlagEnum.PHYLLA_INHERITED_KNOWLEDGE] == 1) {
			...
			MainScreen.text("You take a seat on one of the many fluffy cushions in her room and she proceeds to cuddle in close, pressing her head against your chest and wrapping her abdomen around you.  She then looks up at you and asks what's on your mind.");
	...
	private phyllaBeeeJays(): void {
		MainScreen.clearText();
		Flags.list[FlagEnum.PHYLLA_BLOWJOBS]++;
		if (Flags.list[FlagEnum.PHYLLA_BLOWJOBS] == 1) {
			MainScreen.text("Turning to Phylla, you can't help but smile as you flash her a seductive look, making sure you have her undivided attention before glancing down at the bulge in your pants.  Raising a hand to her mouth in sultry intrigue, Phylla giggles and shoots you a devilish look back.  Turning towards the bed, Phylla presumptuously begins to disrobe...  close!  But not what you had in mind.");
		...
		//Subsequent Blowjobs:
			...
			MainScreen.text("Approaching Phylla, you cross your arms and lean against the wall of her bedchamber while sporting a tell-tale tent in your pants.  Phylla stares down at your bulge and picks up immediately on your intentions.");
			MainScreen.text("\n\n\"<i>Y-You gonna ask me nicely, or let your penis");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("es");
			MainScreen.text(" do all the convincing?</i>\"  She teases.  Shrugging, you inform her that whatever works for her works for you.");
		}
		//Continued...
		//If Phylla is not Laying Eggs:
		...
		//First time:
		if (Flags.list[FlagEnum.PHYLLA_BLOWJOBS] == 1) MainScreen.text("\n\n\"<i>Oh!  I get the meal delivered to me.  H-How... kind of you.</i>\"");
		//Subsequent times:
		...
		if (player.cockArea(x) < 60) {
			MainScreen.text("\n\nWithout even having to utter a word, Phylla jumps right in and starts to work your " + CockDescriptor.describeMultiCockShort(player) + " free of ");
			if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("its");
			...
			MainScreen.text("\n\nWith both sets of fingers placed in the fabric band of your undergarments, Phylla pulls downward and releases your " + CockDescriptor.describeMultiCockShort(player) + ", ");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("all ");
			MainScreen.text("bobbing up");
			//Dick size less than 5 inches:
			if (player.lowerBody.cockSpot.get(x).cockLength < 5) MainScreen.text(" \"tall\" and proud, rock hard and ready for action.");
			else MainScreen.text(" tall and proud, rock hard and ready for action.");
		}
		else {
			...
			if (Flags.list[FlagEnum.PHYLLA_BLOWJOBS] == 1) {
				...
				if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
				MainScreen.text(" from your pants, but apparently doesn't realize just how big ");
				if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("this");
				else MainScreen.text("these");
				MainScreen.text(" battering ram");
				if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
				MainScreen.text(" can get.  Chuckling at her ignorance, you tell Phylla she'd better back up.  Phylla rolls her eyes and looks up at you, giving you an unamused look.");
				...
				MainScreen.text("\n\nYou work to release yourself from your clothing.  Once you're completely free you watch as Phylla's eyes widen in horror.  Smiling, you start to stroke your " + CockDescriptor.describeMultiCockShort(player) + " and work ");
				if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("it");
				else MainScreen.text("them");
				MainScreen.text(" up to a full raging erection, achieving your maximum state of \"<i>readiness</i>\" in no time at all.");

				MainScreen.text("\n\nDumbfounded, Phylla stares in awe at how big your " + CockDescriptor.describeMultiCockShort(player) + " can really get; you can see the look of complete intimidation on her face and who could blame her?  You've seen ship cannons that pale in comparison to your man meat.");
				//If multiple cocks:
				if (player.lowerBody.cockSpot.count() > 1) {
					MainScreen.text("\n\nShe's staring down a mighty series of threatening fleshy spears.");
				}
				MainScreen.text("\n\n\"<i>I don't know if I can... wow...</i>\" She sputters out.  Laughing out loud, you grin down at her and inform her that, in this instance, her slight ignorance IS bliss.  Twitching the muscles in your " + CockDescriptor.describeMultiCockShort(player) + " to bob a bit in her face you let her know that you rose to the challenge; now it's time for her to \"<i>fulfill</i>\" her end of the bargain: the cock end that is.");
			}
			//Dick size more than 60 inches (subsequent blowjobs):
			else {
				MainScreen.text("\n\n\"<i>Okay, you're gonna have to be careful so we don't have a repeat of ");
				if (Flags.list[FlagEnum.PHYLLA_BLOWJOBS] == 2) MainScreen.text("last time");
				else MainScreen.text("our first time");
				MainScreen.text(".  I'll help to free ");
				if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("that monster");
				else MainScreen.text("those monsters");
				MainScreen.text(", but p-please be careful about my valuables.</i>\"  Phylla asks nervously.");
				MainScreen.text("\n\nYou tell her that as long as she's helping, there shouldn't be any \"<i>accidents.</i>\"  With synchronous effort, your hulking " + CockDescriptor.describeMultiCockShort(player) + " ");
				if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("is");
				else MainScreen.text("are");
				MainScreen.text(" freed once more, ready and raring to \"re-acquaint\" with Phylla.  Taking the hint, Phylla closes in and takes the ");
				//Dick size less than 16 inches:
				if (player.lowerBody.cockSpot.get(x).cockLength < 16) MainScreen.text("base");
				else MainScreen.text("middle");
				MainScreen.text(" of your shaft against her tongue.  You hear a mix of a giggle and hum as she drags her tongue towards the head of your " + CockDescriptor.describeCock(player, x) + ".  Phylla teases your shaft as she reaches the peak and suckles at your urethral opening.  You feel her drawing up as much pre-cum as she can and lapping it up like she's dying of thirst.  She continues this for a bit, until it becomes apparent that she's either not too sure of what else to do, or that she's never heard of how to properly fellate someone.");
				MainScreen.text("\n\nWith a chuckle, you pull your " + CockDescriptor.describeCock(player, x) + " away from her mouth and ask her to look up at you. Clearly embarrassed at your realization of her skills, she begins to tremor softly.  \"<i>S-sorry! I mean, I've never done this!  This act is uncommon for my people.  Such things are not proper for a queen to be...  taught, or learn.</i>\"  She apologizes, begging your forgiveness with her body language and demeanor.");
			}
		}
		menu();
		//(If Corruption less than 75 transitions to - Pure BJ Ending)
		if (player.stats.cor < 75) MainScreen.addButton(0, "Next", purePhyllaBJOver);
		//(If Corruption more than 75 transitions to - Corrupt BJ Ending)
		else MainScreen.addButton(0, "Next", corruptPhyllaEndings);
	}
	...
	private purePhyllaBJOver(): void {
		MainScreen.clearText();
		let x: number = player.smallestCockIndex();
		MainScreen.text("You quickly and softly tell her to calm down, she needn't worry; you'll be more than happy to help her learn how as surely as she would help you if your situations were reversed.  At the mention of counter positions she nods cheerfully.");
		MainScreen.text("\n\n\"<i>You... know I'd d-do anything for you... I'll get good, I will!</i>\"");

		MainScreen.text("\n\nHappy and secure that you're willing to tolerate her lack of experience, Phylla lowers herself back down to your " + CockDescriptor.describeCock(player, x) + " and gives you a tender lick on your head, slurping in all the pre-cum you've been accumulating.  You opt to help her with a more direct approach; you lower your hand down to her chin and tilt her head up so her eyes meet yours.  The tension in Phylla's jaw is fierce, and you tell her that she's putting too much thought and effort into this: just relax. You move your thumb over her lips and part her mouth slightly.  A calm washes over her as your directive registers.");
		MainScreen.text("\n\nPeacefully, you tell her to open her mouth just a bit and to let your " + CockDescriptor.describeCock(player, x) + " slide into her mouth, advising her to watch the teeth.  Once she's got it in there, you tell her she can massage your head with her tongue and lips to begin with.  Eventually she'll work her way up to experimenting with the interior of her mouth.  Then once she gets that down, she can take you into her throat, but that's a more 'advanced technique.'");
		MainScreen.text("Taking your directions, Phylla takes the head of your cock along the top of her wet and humid tongue, leisurely batting around before trapping it with her lips.");
		...
		if (player.lowerBody.cockSpot.count() > 1) {
			...
			if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("your other dick");
			else MainScreen.text("the rest of your dicks");
			MainScreen.text("; if she would be \"<i>so kind.</i>\" With gusto, her four hands shoot up and take your ");
			if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("cocks into their firm grips");
			else MainScreen.text("cock into their firm grips");
			MainScreen.text(".  Phylla mumbles something but absentmindedly realizes she still has your cock in her mouth and gives up on whatever it was she was trying to say.");
		}
		//Vagina:
		if (player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("\n\nYou ask Phylla if she can take care of your womanhood while she's at it.  Phylla moves one of her small arms in between your legs and rubs against your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + ".  She caresses your labia in between her fingers before inserting them inside of you.  Phylla begins working her wrist diligently in conjunction with her head bobbing.");
		}
		...
		//[Sure]  [Nope]
		menu();
		MainScreen.addButton(0, "Sure", surePhyllaLetsFuck);
		MainScreen.addButton(1, "Nope", nopeNotOnMouthOrWhateverFuckThisNoise);
	}
	...
	private nopeNotOnMouthOrWhateverFuckThisNoise(): void {
		MainScreen.clearText();
		MainScreen.text("You pass on her offer, not wanting to know what THAT sensation");
		//If PC can't Autofellatio:
		if (!player.canAutoFellate()) MainScreen.text(" or anything like it would feel like.");
		else MainScreen.text(" feels like with Phylla's mind linked to your own.");
	...
	private pureBJEnding(linked: boolean = true): void {
		...
		if (!linked) MainScreen.text(".");
		else if (Flags.list[FlagEnum.TIMES_LINKED_BJ_SUCK] == 0) {
			MainScreen.text(", ");
			//If PC linked, first time occurring:
			MainScreen.text("leading to you jesting through the link and calling her a damn dirty cheater.");
		}
		if (Flags.list[FlagEnum.TIMES_LINKED_BJ_SUCK] > 0) MainScreen.text("  You can feel her relying less and less on the link with every subsequent time; there's no doubt in your mind that she will no longer \"<i>cheat</i>\" her way into making you cum before you know it.");
		let x: number = player.smallestCockIndex();
		MainScreen.text("\n\nShe giggles up at you as she bobs back and forth along your shaft, sensing the telltale signs of your pending orgasm with her mouth.  When you feel the moment of climax finally arrive, you grab onto the back of her head with your hands. She jumps at the unexpected feeling, but before she can react you roar, thrusting your hips forward.  Phylla, like a pro, times her motions with your release, drawing a steady stream of semen out of your rod.  With each spasm of your cock you shoot your cum into her mouth.  She doesn't relent until she's got a mouthful, pulling free of your " + CockDescriptor.describeCock(player, x) + " as ");
		if (player.cumQ() < 250) MainScreen.text("thin");
		else if (player.cumQ() > 1000) MainScreen.text("thick");
		MainScreen.text(" strands of cum hang wildly between her lips and your prick.");
		//If cum volume over 1000ml::
		if (player.cumQ() > 1000) MainScreen.text("  Though she might have a mouthful, you're not finished yet!  You continue to unload the remainder of your hot load on her breasts and stomach.");
		if (player.lowerBody.cockSpot.count() > 1) {
			//If multi cock::
			MainScreen.text("  Your other cock");
			if (player.lowerBody.cockSpot.count() > 2) MainScreen.text("s shower");
			else MainScreen.text(" showers");
			MainScreen.text(" her hands and body in your white goo as she continues to pump ");
			if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("on each of ");
			MainScreen.text("them lovingly.");
		}

		MainScreen.text("\n\nWith a heavy sigh, and significantly lighter ");
		if (player.lowerBody.balls > 0) MainScreen.text("balls");
		else MainScreen.text("in the pelvis");
		MainScreen.text(", you rub Phylla's hair playfully");
		if (Flags.list[FlagEnum.PHYLLA_BLOWJOBS] != 1) MainScreen.text(".");
		//If first time:
		else MainScreen.text(" and congratulate her on her first time.");
	...
	private corruptPhyllaEndings(): void {
		MainScreen.clearText();
		let x: number = player.smallestCockIndex();
		MainScreen.text("\"<i>A slut that can't suck a dick... how can this get any worse?</i>\"  You ponder to yourself while rubbing your nasal bridge. You tell the innocent little ant that she'll learn how to suck a mean cock, even if it kills her.  You're knee deep at this point - might as well turn her into something salvageable.  Commanding her attention, you lift her face up and hover your " + CockDescriptor.describeCock(player, x) + " close to her lips, smothering her lips with your precum.  She attempts to turn away but you react before she can.  Holding her head in place you comment on how this is what you want from her.");
		...
		//If first time:
		if (Flags.list[FlagEnum.PHYLLA_BLOWJOBS] == 1) MainScreen.text("achieved the desired effect.");
		//Every other time:
		else MainScreen.text("improved since the last time.");
		...
		MainScreen.text("\n\nPhylla groans at the rough treatment, not used to being treated so coarsely as your " + CockDescriptor.describeCock(player, x) + " lunges back and forth in her mouth, grinding along her cheek and tongue. Your cock seems to have a mind of its own as you somehow manage to wrap her tongue around yourself, knowing full well Phylla lacks the technique required to do it on her own.  The little queen is about to receive a rude awakening about pleasing her master.");
		MainScreen.text("\n\nMoan after muffled moan escapes from Phylla's mouth as you continue your callous fucking of her delicate little face.  Your little slut is trying her best to keep up with your thrusts but is ultimately failing.  Perhaps she needs a little motivation?  Withdrawing your " + CockDescriptor.describeCock(player, x) + ", you tell her she doesn't get to suck your dick anymore unless she fingers her tight little hole. Desperate to fulfill your desires, she quickly darts a hand to her pussy and fingers that little fuckhole like her life depends on it.  Within seconds you see her long clit protruding, begging to be touched.");
		...
		if (player.lowerBody.cockSpot.count() > 1) {
			...
			MainScreen.text(" if she would be \"<i>so kind</i>\".  With gusto, her four hands shoot up and take your cocks into their firm grips.  Phylla mumbles something but absentmindedly realizes she still has your cock in her mouth and gives up on whatever it was she was trying to say.");
		}
		//Vagina:
		if (player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("\n\nYou ask Phylla if she can take care of your womanhood while she's at it.  Phylla moves one of her small arms in between your legs and rubs against your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + ".  She caresses your labia in between her fingers before inserting them inside of you.  Phylla begins working her wrist diligently in conjunction with her head bobbing.");
		...
		//Continuation:
		MainScreen.text("\n\nCongratulating her for her hasty heeding of your wishes, you ram your " + CockDescriptor.describeCock(player, x) + " back into her mouth and continue the face fucking once more.  Phylla picks up the effort as she starts working her head and tongue in tandem with your hips.  Soon she's driving you perilously close to an orgasm.  With one last directive, you ask Phylla if she's ready to receive your seed.  Though since you continue your brutal fucking of her mouth, she has no opportunity to reply.");
		...
		if (player.cumQ() < 10) MainScreen.text("tiny");
		else if (player.cumQ() < 100) MainScreen.text("respectable");
		else MainScreen.text("massive");
		MainScreen.text(" facial, covering her petite features in cum.");
		...
		if (silly()) {
			...
			MainScreen.text(".  Somehow, you succeed your fortitude roll and remain conscious without passing out even after plummeting to the ground from sheer exhaustion.  You and Phylla can't help but slowly drift off to sleep in a pool of your cum to dream about the bizarre pleasures you two have indulged in.");
		}
	...
	private dickPhylla(): void {
		...
		if (Flags.list[FlagEnum.PHYLLA_BLOWJOBS] == 0) {
			...
			MainScreen.text("\n\nCutting her off, you gently place your hand on the back of her head as " + CockDescriptor.describeMultiCockSimpleOne(player) + " hardens even further.  With a slight pressure you force Phylla's head down onto the head of [oneCock]. At this point she gets the picture.");
		}
		...
		//3-4 times:
		if (Flags.list[FlagEnum.PHYLLA_FUCKS] >= 3) MainScreen.text(" she asks playfully, knowing full well that that's what you came here for.");
		//(First time)
		else if (Flags.list[FlagEnum.PHYLLA_FUCKS] == 0) MainScreen.text(" You nod quickly.");
		//(Subsequent fucks)
		if (Flags.list[FlagEnum.PHYLLA_FUCKS] > 0) MainScreen.text("\n\n\"<i>Do you have to ask?</i>\"  You ask her while grinning mischievously; something that causes her to smirk back at you.");

		//3-4 times:
		if (Flags.list[FlagEnum.PHYLLA_FUCKS] >= 3) MainScreen.text("\n\nShe seems like she's losing some of her shyness, or at the very least feels comfortable enough around you to assert more of her personality.");
		...
		MainScreen.text("\n\nHer eyes widen in surprise as your " + CockDescriptor.describeCock(player, x) + " fills her.  She howls a blissful scream as she drools forth her girl fluids, coating your cock ");
		if (player.lowerBody.balls > 0) MainScreen.text("and balls ");
		MainScreen.text("in her sexual liquids.");
		...
		if (player.lowerBody.cockSpot.count() >= 2 && player.lowerBody.cockSpot.count() <= 5) {
			if (player.lowerBody.vaginaSpot.hasVagina()) {
	...
	private lesbianFisting(): void {
		...
		MainScreen.text("\n\nMoaning loudly into the air, the aroma of earth, sweat, and sex fills your nostrils.  You can't help but feel a little embarrassed to be so fully exposed to her, but that quickly passes as one of her fingers slips along your labia and brushes past the opening of your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".  Guided by the wetness of your ever eager pussy, her hard fingertips slide forward into your body.  You moan again at this slow insertion.  Encouraged, Phylla rubs up and down the walls of your vagina, leisurely at first but with mounting vigor, searching for your G-spot.");
	...
	private dudesFuckEggLayingBitches(): void {
		MainScreen.clearText();
		MainScreen.text("Phylla looks completely taken aback when you suggest you want to have sex with her.  Obviously self conscious about how she looks right now, she covers her enlarged breasts with all four of her hands as she sits back in her seat.");
		...
		if (player.stats.cor < 50) MainScreen.text("don't care how she looks right now. Besides, some creatures around here appreciate a little carnal love; it helps with the birthing process.");
		//If Corruption is more than 50:
		else MainScreen.text("could care less for her pathetic attempts at concealing her pregnant form; you're horny, and she's going to put out, regardless of her insecurities.");

		MainScreen.text("\n\nYou walk over to her and ");
		if (player.stats.cor < 50) MainScreen.text("lovingly");
		else MainScreen.text("apathetically");
		MainScreen.text(" pull her hands away from her breasts.  She resists you at first, but once she sees there's no stopping you she shyly resigns herself to you.");
		...
		//(If PC has balls:
		if (player.lowerBody.balls > 0) MainScreen.text(" your balls slap against the smoothness of her pelvis.");
		else MainScreen.text(" your knees grind along the edge of her makeshift throne.");
	...
	private birfingSexWithAntsForDasLadies(): void {
		MainScreen.clearText();
		MainScreen.text("Phylla looks completely taken aback when you suggest that you want to have sex with her.  You can tell she is self conscious about the way she looks right now by the way she covers her enlarged breasts with all four of her hands as she sits back in her seat.");
		...
		if (player.lowerBody.isNaga()) MainScreen.text("  You slither almost whorishly with a slow, measured wriggle,");
		else MainScreen.text("  You walk almost whorishly with a slow, sensual gait,");
		MainScreen.text(" allowing her to see the sensual swivel of your " + ButtDescriptor.describeButt(player) + " from left to right and back again as you visibly entice Phylla's mind to lustful thoughts.  Once you're facing Phylla once again, you see her lower two hands are still covering her breasts and the upper two are gripping the stone chair she sits in.");
		...
		if (player.stats.cor <= 50) MainScreen.text("  You probe her mind and are almost taken aback.\"<i>NO!  Just please keep going!</i>\"  You hear her voice ring out in your head.");
		...
		//If Corruption is over 50:
			...
			MainScreen.text("\n\nSensing that she is nearing an orgasm, you pull back and wrestle out from betwixt her legs.  Finally free, you see she's sprawled out on the bed, her massive abdomen hanging off to the side.  Settling yourself over the top of her, you slide your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " along her nose, intent on getting your scent entrenched in her body, wanting to make her crave your pussy, something that occurs as she takes a long whiff of air and moans like a whore in heat.");

			//If PC has loose pussy:
			if (player.lowerBody.vaginaSpot.get(0).vaginalLooseness >= VaginaLooseness.GAPING) MainScreen.text("\n\nYour loose pussy lips cause your juices to drip onto Phylla's face as you hover over her, allowing her to lather in the wetness of your arousal.");
		...
		//EAT PUSSY:
		MainScreen.text("\n\nLike a starving animal, Phylla's mouth waters as she takes hold of your thighs and spreads your legs apart.  You grab hold of her hair and shove her head inbetween your legs, impacting your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " and causing you to moan slightly.  She certainly isn't shy now, you think, as her tongue starts to work its way into you.  You can \"<i>hear</i>\" Phylla giggle back in response. Thoughts of grinding your pussy across her mouth and face flood your mind, and instinct takes over as your hips start to rock in response to Phylla's eager attack.  Running your hands through her hair,  you grind Phylla's pretty face harder and deeper into your soaking " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".  The poor ant can only respond by moaning fiercely as she eagerly sucks and kisses your love button, completely overwhelmed by the desire that has overtaken her.");

		//If PC has (a) Dick(s), that are less than 42 inches:
		if (player.lowerBody.cockSpot.count() > 0 && player.cockThatFits(42) >= 0) {
			...
			if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("it");
			else if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("them");
			else MainScreen.text("as many as you can");
			MainScreen.text(" as fiercely as your body will let you.");
		}

		MainScreen.text("\n\nAs you feel your stiff " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " glide along her lips, you can begin to taste bits of her almost primal hunger for you.  As well, you sense that the growing contractions in her abdomen are getting quicker and stronger.");
		...
		MainScreen.text("\n\nYour " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " comes to rest on her lips again as her head starts to bob between her own strong legs.  Returning the sentiment, you dart in-between her legs, tongue first.  You hit that sweet tasting little pussy of hers with force.  Phylla moans, your intentions crystal clear as she bears down on your pussy hard, intently working for your sexual release.  She reaches up with her lower set of arms and gives both your " + BreastDescriptor.describeNipple(0) + "s a firm squeeze, the twist of which is nothing short of exquisite.  Muffled moans of bliss are unintelligible, but in your shared mind-state, you can hear one another screaming words of encouragement and direction.");

		//If PC has (a) Dick(s), that are less than 42 inches:
		if (player.lowerBody.cockSpot.count() > 0 && player.cockThatFits(42) >= 0) MainScreen.text("\n\nWithout warning, Phylla begins to alternate between giving your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " oral and sucking your " + CockDescriptor.describeMultiCockShort(player) + "; the dual stimulation overwhelms you as she devilishly teases your sexes.");
		...
		if (player.lowerBody.cockSpot.count() > 0 && player.cockThatFits(42) >= 0) {
			...
			if (player.cockThatFits2(42) >= 0) MainScreen.text("two of ");
			MainScreen.text("your " + CockDescriptor.describeMultiCockShort(player) + " into her mouth and mashes ");
			if (player.cockThatFits2(42) >= 0) MainScreen.text("them");
			else MainScreen.text("it");
			MainScreen.text(" against the back of her throat.  Unable to contain the pleasure from this, you howl and blow your hot load into her throat.");
		}
		else if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("\n\nFeeling your own " + CockDescriptor.describeMultiCockShort(player) + " about to erupt, you quickly roll off your lover.  Just in time, too - you fire your jizzum up at the ceiling, coating it quite well before your salty load falls back down on top of the two of you.");

		//PC dick is out in the hallway:
		if (player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 500) {
			MainScreen.text("\n\nYour " + CockDescriptor.describeMultiCockShort(player) + " twitches with orgasmic release, bulging as your load works its way through your urethra");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
			MainScreen.text(".  With a heavy heave, you feel the hot and sticky ejaculate launch from your dick, hitting the interior decorations as you do so; what exactly you hit, you can't say... but you'll likely be tripping over your mess on the way out.");
		}

		MainScreen.text("\n\nYou struggle to breathe for a moment as a massive amount of warm girl cum seems to flood from Phylla's vagina to completely cover your face and soak most of your " + HeadDescriptor.describeHair(player) + ".  While you catch your breath, you can hear Phylla noisily slurping up your fluids before compressing her face against your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " in an eager effort to lather her face in your juices for just a little longer.  Contentment and fulfilled desire - not only your own, but Phylla's as well - spreads like wildfire through your brain.  You grin to yourself, satisfied at the effect you've had on Phylla during your session.");
	...
	private orgyWithDatColonyCorruptDudes(): void {
		Flags.list[FlagEnum.TIMES_CORRUPT_MALE_ANT_ORGY]++;
		MainScreen.clearText();

		MainScreen.text("You tell Phylla to call in your strongest and most virile children for 'inspection.'");

		//First Time:
		if (Flags.list[FlagEnum.TIMES_CORRUPT_MALE_ANT_ORGY] == 1) {
		...
		if (Flags.list[FlagEnum.PHYLLA_EGG_LAYING] > 0) MainScreen.text("large, lactating ");
		else MainScreen.text("small ");
		MainScreen.text("orbs.");
		...
		if (player.lowerBody.cockSpot.count() > 1 && player.lowerBody.cockSpot.count() <= 3) {
			...
			if (player.lowerBody.cockSpot.count() == 2) MainScreen.text(" is");
			else MainScreen.text("s are");
			MainScreen.text(" getting sucked off by two of your children, who themselves are being sucked off.");
		}
		//If player has four-6 six dicks:
		if (player.lowerBody.cockSpot.count() >= 4 && player.lowerBody.cockSpot.count() <= 6) MainScreen.text("\n\nEven though you have so many cocks, it won't be a problem.  Just standing over Phylla you look down as hands, mouths, and Phylla's breasts do their best to please every one of your cocks in turn.");
		...
		let x: number = player.cockThatFits(phyllaCapacity());
		if (x < 0) x = player.smallestCockIndex();
		MainScreen.text("\n\nWhen you've felt your mind completely sync up with everyone else in the room, it's like being drunk on sexual euphoria.  You almost lose yourself in it for a moment, but focusing your mind, you snap your eyes open. You see Phylla in front of you looking like a drugged slut; she runs all four of her hands over your " + CockDescriptor.describeCock(player, x) + " at the entrance of her flooded cunt while her children play with her nipples and suck each other off.  Aggressively, you grab her arms and pin her down while simultaneously shoving your cock into her.");
		...
		MainScreen.text("\n\nOne of your brood leans down and starts to feverishly lick the base of Phylla's clit as your cock slides in and out of her. His warm saliva coupled with Phylla's natural lubricant warms your cock and sends even more pleasure throughout your " + CockDescriptor.describeCock(player, x) + ".");
		...
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(" sucked, stroked, inserted, and finally orgasming so many times that you lose track. Hours pass and you eventually pass out sitting atop Phylla's throne watching the writhing mass before you.");
	...
	private antColonyOrgy4Ladies(): void {
		MainScreen.clearText();
		Flags.list[FlagEnum.TIMES_CORRUPT_FEMALE_ANT_ORGY]++;
		//(First Time)
		if (Flags.list[FlagEnum.TIMES_CORRUPT_FEMALE_ANT_ORGY] == 1) {
			MainScreen.text("She gives you a confused look, but does as you command. Tilting her head back and closing her eyes you watch as she silently 'communicates' to her children.");
		...
		if (player.isBiped()) MainScreen.text("  Setting one foot on his shoulder, you present your [vagina] to him.");
		else MainScreen.text(" Now at the perfect height, you present your [vagina] to him.");
		...
		if (player.isBiped()) MainScreen.text(" from between your legs.");
		else MainScreen.text(" from your cunt.");
		MainScreen.text("  You beckon him to follow you as you strut over to where Phylla is.");
		...
		if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text("  You feel hands run up and down your shaft");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
			MainScreen.text(", causing you to spurt bursts of pre all over the floor.");
		}
		...
		//If PC has dick(s):
		if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text("  Not forgetting you have 'other needs' the ant mounting you reaches down with his larger arms and grabs hold of [eachCock].  Keeping his smaller arms on your hips to keep stable with each thrust, he pumps vigorously at your cock");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
			MainScreen.text(".");
		}
	...
	private phyllaLaysEggsToggle(): void {
		MainScreen.clearText();
		//(Lay Eggs)
		if (Flags.list[FlagEnum.PHYLLA_EGG_LAYING] == 0) {
			Flags.list[FlagEnum.PHYLLA_EGG_LAYING] = 1;
			MainScreen.text("Phylla seems confused as to what you're thinking as you gaze deeply into her eyes.  You gather all four of her hands between the two of yours and tell her that children are wanted.  Not just one or two; you want as many as she can muster.");
			//If you've talked to her about history:
			if (Flags.list[FlagEnum.TALKED_WITH_PHYLLA_ABOUT_HISTORY] > 0) MainScreen.text("  You tell her you want a colony like those she spoke of in the times before the corruption.");
			...
			menu();
			MainScreen.addButton(0, "Lick That", lickThatAntButt);
			MainScreen.addButton(1, "Don't Lick", dontLickAntButt);
		}
		//(Stop Laying Eggs)
		else {
			Flags.list[FlagEnum.PHYLLA_EGG_LAYING] = 0;
			MainScreen.text("You sigh to yourself; you never really wanted this conversation but you knew it might happen someday.  You make a stern face and put a determined fatherly tone in your voice.  Phylla immediately senses you mean business and you have her full attention.  You try to tell her as gently as possible that you don't think it is safe for her to keep having children.");
	...
	private lickThatAntButt(): void {
		MainScreen.clearText();
		MainScreen.text("You tilt your head slightly.  You thought she said she could start to lay any time after the first time you had sex with her.");
		...
		//First Time:
		if (Flags.list[FlagEnum.ANTS_BIRTHED_FROM_LICKING] == 0) {
		...
		Flags.list[FlagEnum.PHYLLA_COOLDOWN] = 12;
		Flags.list[FlagEnum.ANTS_BIRTHED_FROM_LICKING]++;
		if (Flags.list[FlagEnum.ANT_KIDS] < 5000) Flags.list[FlagEnum.ANT_KIDS] += 5;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private dontLickAntButt(): void {
		...
		if (Flags.list[FlagEnum.ANT_KIDS] > 1) MainScreen.text("; the children can tend to the eggs and I can really use the rest");
		MainScreen.text(".  You should get back to your duties...  B-but I wouldn't mind if you came back later...</i>\"");
		...
		Flags.list[FlagEnum.PHYLLA_COOLDOWN] = 6;
		Flags.list[FlagEnum.ANT_KIDS]++;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private phyllaLaysSomeDriderEggs(): void {
		...
		if (player.stats.cor < 75) {
			...
			if (Flags.list[FlagEnum.PHYLLA_EGG_LAYING] > 0) MainScreen.text("both eggs are just fine on their own");
			else MainScreen.text("the egg is just fine on its own");
			MainScreen.text(", you redirect your focus to helping Phylla through the labor.  Each subsequent egg that finds its way to freedom causes Phylla's once pain filled screams to turn into pleasurable moans of euphoric delight as her pussy becomes overwhelmed by the stimulation of each of your eggs working their way out.  At long last, the final egg slithers out of her vagina and lands gently on the bedding.  Her ordeal over, Phylla breathes a long sigh of relief.");
			//First Time:
		...
		//PC has more than 75 corruption:
			...
			if (player.canOvipositSpider()) {
				...
				MainScreen.addButton(0, "LetHerRecover", letPhyllaRecover);
				MainScreen.addButton(1, "Mount", driderDoublePhllaMount);
			}
			else {
				menu();
				//[Leave her to recover]
				MainScreen.addButton(0, "Next", letPhyllaRecover);
			}
		}
		Flags.list[FlagEnum.PHYLLA_TIMES_DRIDER_EGG_LAYED]++;
		Flags.list[FlagEnum.PHYLLA_DRIDER_BABIES_COUNT] += 5 + rand(4);
	}

	//Mount Phylla:
	private driderDoublePhllaMount(): void {
		MainScreen.clearText();
		MainScreen.text("No longer able to control yourself, you rip free of your [armor] and charge up to Phylla's bed, catching her by surprise as she looks up at you.  With a seductive smile, you pin her down and slather your ovipositor against her fuck hole.");
	...
	private letPhyllaRecover(): void {
		MainScreen.clearText();
		MainScreen.text("Working up all of your self control, you decide that Phylla could use the rest.  You wink at Phylla as you leave, telling her that you'll be back to fuck her brains out shortly... once she feeds your children.  She only musters the strength to smile and mutter something about motherhood but you're already halfway out the door.");
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private phyllaPearance(): void {
		...
		menu();
		MainScreen.addButton(0, "Next", introductionToPhyllaFollower);
	}
	...
	private phyllaDigsForGems(): void {
		...
		if (Flags.list[FlagEnum.ANT_KIDS] > 10) kidsMod++;
		if (Flags.list[FlagEnum.ANT_KIDS] > 50) kidsMod++;
		if (Flags.list[FlagEnum.ANT_KIDS] > 150) kidsMod++;
		if (Flags.list[FlagEnum.ANT_KIDS] > 300) kidsMod++;
		if (Flags.list[FlagEnum.ANT_KIDS] > 600) kidsMod++;
		if (Flags.list[FlagEnum.ANT_KIDS] > 1000) kidsMod++;
		if (Flags.list[FlagEnum.ANT_KIDS] > 2000) kidsMod++;
		//Success
		if (Flags.list[FlagEnum.PHYLLA_GEMS_HUNTED_TODAY] == 0 && rand(20) + kidsMod > 10) {
			let gems: number = 0;
			...
			//If Phylla IS NOT Laying Eggs
			if (Flags.list[FlagEnum.PHYLLA_EGG_LAYING] == 0) {
				MainScreen.text("You ask Phylla is she's found any gems while digging out her colony.  She nods happily and runs over to a small stone chest and rifles though it.  After a moment, she runs back over to you and holds up all four of her hands.");
		...
		//Failure
			...
			MainScreen.text("You ask Phylla if she's found any more gems while digging.  \"<i>No, I'm sorry.  I haven't.  Not since the last time you asked.  I'll do better!  I mean, I know you like them.  They're rare... even down here.  I'll do better...</i>\"");
			//If corruption is less  than 50
		...
		Flags.list[FlagEnum.PHYLLA_GEMS_HUNTED_TODAY] = 1;
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private eggDatBitch(): void {
		MainScreen.clearText();
		//PC is a Female/Herm Drider:
		if (player.canOvipositSpider()) MainScreen.text("While Phylla appears to be sexually sated, the heaviness in your spider abdomen begs for release.\n\n");
		//First Time:
		if (Flags.list[FlagEnum.TIMES_EGG_IMPREGNATING_PHYLLA] == 0) {
			MainScreen.text("Turning to Phylla, you tell her that you can help to make her wishes of being a brood-mother come true.  She looks at you a bit taken back, as if what you already did wasn't enough already.");
		...
		//Persuasion Success
			...
			if (!pregnancy.isPregnant) pregnancy.knockUpForce(PregnancyType.DRIDER_EGGS, 8 * 24); //Supposed to be eight days, not eight hours
			Flags.list[FlagEnum.TIMES_EGG_IMPREGNATING_PHYLLA]++;
			player.orgasm();
			player.dumpEggs();
		}
		doNext(camp.returnToCampUseOneHour);
