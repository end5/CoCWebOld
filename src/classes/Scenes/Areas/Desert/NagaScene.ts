...
public function nagaEncounter():void {
	...
	if(player.lowerBody == LOWER_BODY.NAGA) {
		...
		//[If player has fangs]
		if(player.faceType == FACE.SNAKE_FANGS) outputText("your own.", false);
		//[player has no fangs]
		...
		if(player.gender == 1 || (player.gender == 3 && rand(2) == 0)) {
			...
			//[Cock isn't too big]
				...
				if(player.lowerBody.cockSpot.count() == 1) outputText("it starts ", false);
		...
		//chick secks
			...
			//(if player has fangs)
			if(player.faceType == FACE.SNAKE_FANGS) outputText("carefully nibbling at it so as to not break the skin.  ", false);
...
    outputText("You decide that you've played with her enough and withdraw your hand, now wet with her juices, from her pussy. A hiss escapes the naga's lips and she tries to bring your hand back to her needy cunt. You pull your hand out of her grip and instead shift your gooey form over her lips and start to push some of your lower half into her. She starts to coo as you shove the first few inches in, her hips thrusting into the air. You continue to push more of your slimy form into her, sliding deeper and deeper into her as her walls convulse around you.\n\n", false);
	...
	//(if corrupt)
	if(player.stats.cor > 50) outputText("but immediately after, you decide that this is going to be too much fun to not push on.", false);
	//(if not corrupt)
...
    
	...
	if(player.totalCocks() > 0) {
		if(player.lowerBody.cockSpot.count() == 1) outputText("your " + cockDescript(0), false);
...
    outputText("\n\nYou continue to relentlessly push yourself into the naga, stretching her wide.  ", false);
	...
	//(if female)
		...
		//(if squirter)
		if(player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING) outputText(", your girlcum gushing out and pooling on the stomach of the naga", false);
		outputText(".", false);
	}
	//(if herm)
	if(player.gender == 3) {
		outputText("You scream in ecstasy as you hit your peak, your girlcum ", false);
		if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLAVERING) outputText("leaking out", false);
...
   			//(Loss by Lust)
		...
		//b) Corrupted
			...
			if(player.str > 50) {
				...
				//(If high corr)
				if(player.stats.cor > 80) outputText("  As a parting gift, you kick sand in her face, and walk away without a care in the world.", false);
				//(If low corr )
...
internal function nagaFUCKSJOOOOOO():void {
	...
	//too)
		...
		if(player.stats.cor > 75) outputText("  The thought somehow makes your " + cockDescript(0) + " throb even more.", false);
		outputText("\n\n", false);
...
     	outputText("A grin stretches across the naga's face as she slithers over to your collapsed form. She coils her tail tightly around your prone form, squeezing hard enough to make it difficult to breathe. She brings her hand down to the crotch of your " + player.armorName + " and a look of confusion crosses her face before she realizes that there is nothing down there.\n\n", false);
		...
		if(player.stats.cor < 50) outputText("You try to turn your head away, but she forcefully shoves it down into her soft folds.  ", false);
...
private function eggUpANagaSpiderLike():void {
	...
	if(player.gender > 0) {
		outputText(", ");
		if(player.lowerBody.cockSpot.hasCock()) outputText("jizz");
...
private function beePositANagaPlease():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  [EachCock] erupts to splatter her with white");
	if(player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) outputText(" and y");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText(" as well");
	}
