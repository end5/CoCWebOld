...
public nagaEncounter():void {
	...
	if(player.lowerBody == LOWER_BODY.NAGA) {
		...
		//[If player has fangs]
		if(player.faceType == FACE.SNAKE_FANGS) MainScreen.text("your own.", false);
		//[player has no fangs]
		...
		if(player.gender == 1 || (player.gender == 3 && rand(2) == 0)) {
			...
			if(player.cockArea(0) > 35) {
				...
				MainScreen.text("\"<i>You're quite the big boy, aren't you?</i>\" she says as she wraps both of her hands around your now throbbing " + cockDescript(0) + ". She starts to slide her hands up and down your length. Sticking out her tongue, she wraps it around the tip and licks at the pre that is starting to leak out.\n\n", false);
			...
			//[Cock isn't too big]
				...
				MainScreen.text("The kiss continues and both of your bodies rub together sensually, your tails continuing to wrap around one another. There is a pulsing in your " + multiCockDescriptLight() + " as ", false);
				if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("it starts ", false);
				else MainScreen.text("they start ", false);
				MainScreen.text("to grow hard against the naga's soft belly. Feeling this, the naga gently pulls away, slowly letting your tongue out of her mouth as she does so. Her hands make their way over your shoulders, down your abs, and stop at your " + multiCockDescriptLight() + ".\n\n", false);
				//[player has one dick]
				if(player.totalCocks() == 1) MainScreen.text("Gently she starts to stroke the length of your " + cockDescript(0) + " with one hand while circling the tip with the other. ", false);
				//[player has two dicks]
				if(player.totalCocks() == 2) MainScreen.text("She takes one in each of her hands and gently strokes them up and down. Every few seconds she stops at the tip to rub her palms over them. ", false);
				//[player has three or more dicks]
				if(player.totalCocks() >= 3) MainScreen.text("She takes one in each hand, stroking them slowly and making sure to pay attention to the tip. Every so often she switches to a different dick to make sure that each and every one of your throbbing cocks has some love given to them. ", false);
				MainScreen.text("A hiss of pleasure escapes your lips as the naga strokes your " + multiCockDescriptLight() + ", her talented fingers bringing you further into a state of arousal. She stops her caress and brings her hand to a scaly covering at her crotch, spreading it wide to reveal her soft pussy.\n\n", false);
				
				//[player has one dick] 
				if(player.totalCocks() == 1) MainScreen.text("She carefully lines it up with your member and starts to tease the tip before gently inserting the first few inches. ", false);
				//[player has two dicks]
				if(player.totalCocks() == 2) MainScreen.text("She carefully lines it up with both of your members and starts to tease their tips before gently inserting the first few inches. ", false);
				//[player has three or more dicks]
				if(player.totalCocks() >= 3) MainScreen.text("She carefully lines it up with one of your members and starts to tease the tip before gently inserting the first few inches. ", false);
...
    		MainScreen.text("You slide your fingers out of her and lift yourself up to look into her flushed face. \"<i>N-no,</i>\" she pants. \"<i>Don't stop... So close...</i>\"\n\n", false);
			...
			MainScreen.text("You untangle the end of your tail from hers and bring it up between the two of you, sliding it between your " + breastDescript(0) + " and inserting it sensually into your mouth. You suck on the end, making sure to coat it liberally before bringing it down to the naga's awaiting slit.\n\n", false);
...
private gooNagaRape():void {
	...
	if(player.stats.cor > 50) {
		//(if corrupt male)
		if(player.gender == 1) {
			MainScreen.text("your " + cockDescript(0) + " growing harder at the thought.", false);
		}
		//(if corrupt female)
...
    
	...
	if(player.totalCocks() > 0) {
		if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("your " + cockDescript(0), false);
		else MainScreen.text("one of your " + multiCockDescriptLight(), false);
	...
	//(if herm) 
		...
		if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("your " + cockDescript(0), false);
		else MainScreen.text("your " + multiCockDescriptLight(), false);
...
    MainScreen.text("\n\nYou continue to relentlessly push yourself into the naga, stretching her wide.  ", false);
	...
	//(if female)
		...
		//(if squirter)
		if(player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING) MainScreen.text(", your girlcum gushing out and pooling on the stomach of the naga", false);
		MainScreen.text(".", false);
	}
	//(if herm)
	if(player.gender == 3) {
		MainScreen.text("You scream in ecstasy as you hit your peak, your girlcum ", false);
		if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLAVERING) MainScreen.text("leaking out", false);
		else MainScreen.text("gushing out to form a pool on the naga's stomach", false);
		MainScreen.text(" and your " + multiCockDescriptLight(), false);
		//(normal)
...
private nagaVictoryMale():void {
	...
	if(player.totalCocks() > 0) {
		...
		if(player.lowerBody == LOWER_BODY.CENTAUR) {
			...
			//[SUPER CUM]
			else MainScreen.text("you explode with huge fountains of cum spurting from your " + cockDescript(0) + ", gushing out from where the two of you are joined to rain on the sands below, ", false);
		...
		//Uncorrupted  
			...
			MainScreen.text("You almost feel pity as you behold the defeated creature lying before you, but your growing arousal pushes the thought from your mind. Leaning forward to get a closer look, your thoughts wander, and you find yourself wondering what her flexible body could do. Unable to contain your lust, you remove your " + player.armorName + " and slowly lay on top of her, moving your hands across her scaly form in search of her pussy. As you rub your hands across the underside of her scaly belly, you notice that she is beginning to blush and moan softly. She is visibly trying to hold the feelings back, but whatever it is you're doing to her is definitely making that difficult. For the moment, you decide to abandon the search, content with simply running your hands along her tail's soft underside and watching her face as she tries in vain to hold her passion back. As you run your fingers up the center of her tail's underside, you notice a strange bump just a few inches below where her skin and scales meet. On further inspection, you realize that this is the thing you have been searching for - her warm, wet lovehole. Your touches seem to have had an effect on her; she is already rather moist, ready to take your " + cockDescript(0) + ". You gingerly slide one of your fingers inside her, feeling for depth as well as for wetness. The naga coos softly, unable to withhold her moans.\n\n", false);
			...
			if(player.cockArea(0) <= monster.vaginalCapacity()) {
				...
				//[Spermageddon]
				else MainScreen.text("your " + cockDescript(0) + " erupting, blasting a violent torrent of semen deep inside her petite frame, filling her up completely as both your voices rise in screams of pleasure.", false);
			}
			//[If cock too large]
			else {
				...
				MainScreen.text("  The naga removes your " + cockDescript(0) + " from her mouth", false);
				//[lots of cum OR more]
...
   			//(Loss by Lust)
			if(monster.lust > 99) {
				...
				if(player.gender == 3) MainScreen.text("  As her tail gently squeezes your body, it draws your own " + multiCockDescriptLight() + " up against your body, her soft underbelly stimulating the sensitive skin.", false);
				...
				//(If PC is Herm i.e. Has a cock(s)){
				if(player.gender == 3) MainScreen.text(" and your " + multiCockDescriptLight() + " constantly stimulated by the tail wrapped around your body", false);
...
internal function nagaFUCKSJOOOOOO():void {
	...
	if((player.perks.has("BimboBrains") || player.perks.has("FutaFaculties")) && player.lowerBody.vaginaSpot.hasVagina()) {
		...
		//[If Lactating]
		if(player.lactationQ() >= 50 && player.biggestLactation() >= 1) MainScreen.text("Your warm milk is leaking from your " + nippleDescript(0) + "s the entire time, coating your skin and soaking her hands. At one point, she withdraws a hand and licks the milk from it. You can only assume that she liked it, as she begins to focus even more on your breasts.\n\n", false);
		//-------
		
		MainScreen.text("The naga lifts you from her body with her tail, placing you onto the warm sand where she had been laying. She moves her own body above yours, her tail buried deep inside your " + vaginaDescript(0) + " the entire time. She holds you in place with one hand, her other hand moving to resume playing with your " + player.allBreastsDescript() + ". She moves her head in, her hot breath washing over your neck, and looks up into your eyes for a moment. She smiles, giggles, and then wraps her mouth around your " + nippleDescript(0) + ", letting her slender tongue tickle your sensitive areola.\n\n", false);
		//-----
		
		//[If lactating] 
		if(player.lactationQ() >= 50 && player.biggestLactation() >= 1) MainScreen.text("As her mouth sucks and plays with your nipple, your warm, sweet milk begins to flow forth once again, filling her mouth with your delectable cream. Her hand is also absolutely covered by it, and the excess drips down to the dry sand below, absorbed in seconds by the thirsty earth.\n\n", false);
		//------
		...
		//----
		//[If Lactating]
		if(player.lactationQ() >= 50 && player.biggestLactation() >= 1) MainScreen.text("She shares a mouthful of your delicious milk with you.\n\n", false);
		//------
	...
	//too)
		...
		//(If defeated by lust)
		if(player.lust > 99) MainScreen.text("You fall to your knees, your body shaking with desire.\n\n", false);
		//(If defeated by KO)
		...
		//(if defeated by lust)
		if(player.lust > 99) MainScreen.text("all your resistance seems to do is make your " + cockDescript(0) + " harder.", false);
		//if defeated by KO)
		...
		if(player.stats.cor > 75) MainScreen.text("  The thought somehow makes your " + cockDescript(0) + " throb even more.", false);
		MainScreen.text("\n\n", false);
...
    		//(If you've been a bad boy during victory scenes)
	...
	//b) female //http://nekomimichan.org/mg/src/12844585127.jpg
		...
		//(if uncorrupted)
		else MainScreen.text("and while you know you probably won't like it, your " + vaginaDescript(0) + " says otherwise.  ", false);
		...
		if(player.lust > 99) 
		{
			MainScreen.text("and you can feel the need to have sex with her burn inside you. You quickly strip yourself of your " + player.armorName + " and throw yourself on her awaiting pussy.", false);
		} 
		//(If defeated by KO)
		else 
		{
			MainScreen.text("and you're too weak to resist anyway; you might as well just comply. You attempt to remove your " + player.armorName + " before the naga grabs you in pity and decides to help you with it. Now naked,  she lays you down on her tail and you let her guide you to her awaiting pussy.", false);
		}
...
   		
		...
		if(player.biggestLactation() > 1) { 
			MainScreen.text("  Your warm milk is leaking from your " + nippleDescript(0) + " the entire time, coating your " + player.skinDesc + " and soaking her hands. At one point, she withdraws a hand and licks the milk from it. You can only assume that she liked it, as she begins to focus even more on your breasts.", false);
		}
		MainScreen.text("\n\n", false);
...
    	MainScreen.text("The naga lifts you from her body with her tail, placing you onto the warm sand where she had been laying. She moves her own body above yours, her tail buried deep inside your " + vaginaDescript(0) + " the entire time. She holds you in place with one hand, her other hand moving to resume playing with your " + biggestBreastSizeDescript() + ". She moves her head in, her hot breath washing over your other breast, and looks up into your eyes for a moment. She smiles, giggles, and then wraps her mouth around your " + nippleDescript(0) + ", letting her slender tongue tickle your sensitive skin there.", false);
		//(If fuckable nipples)
		if(player.upperBody.chest.hasFuckableNipples()) {
			MainScreen.text(  "It isn't long before she decides to take it one step further, pushing her tongue inside your " + nippleDescript(0) + " and wriggling it around inside. Meanwhile, her other hand begins to work its way into your other " + nippleDescript(0) + ", finger-fucking it for all it's worth.", false);
		} 
		//(If lactating)
		if(player.biggestLactation() > 1) {
...
private beePositANagaPlease():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.wetness() < 3) MainScreen.text("drools");
		else if(player.wetness() < 5) MainScreen.text("gushes");
