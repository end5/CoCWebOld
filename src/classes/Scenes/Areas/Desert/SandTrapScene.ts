...
private function dickwieldersLoseToSandTarps():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Ignored entirely, your " + vaginaDescript(0) + " quivers and orgasms in tandem, flexing and wetting fruitlessly against the dry water pressing against it.");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" and cunt");
...
private function chicksLoseToSandTarp():void {
	...
	if(monster.statusAffects.has("Fertilized")) outputText("  \"<i>A cute little wwworker hasz fallen into my home.  Wwwell, that isz ok.  Fortunately for hwyou, I already have some eggsz ready.  Hhyou will get to be a queen! Iszn't that exciting?</i>\"");
...
private function genderlessLoss():void {
	...
	if(monster.statusAffects.has("Fertilized")) outputText("\"<i>Not a worker OR a drone! Wwwell, that isz ok.  Even ants who hhhave nothing can be someone for somebody.  And hhyou can be mommy for my childrrhen!</i>\"");
...
public function birfSandTarps():void {
	...
	if(player.gender == 1) {
		...
		if(!player.isTaur() || (player.tallness * (5/6) < player.lowerBody.cockSpot.list[player.longestCock()].cockLength)) outputText("  Your oily hands descend upon your cock, and you massage your shaft as you feel the pressure in your bowels intensify again.");
	}
	//Female: 
...
private function useSandTarpsHands():void {
	...
	if(player.gender == 1) {
		...
		//Three cocks: 
			...
			if(player.lowerBody.balls > 0) outputText("  It grins up at you shyly and, with its final hand, cups your [balls].  ]You laugh with an almost hysterical edge to the sky at how mindboggling getting a triple handjob feels, and the trap giggles slightly as well despite itself.");
...
private function rideDatSandTarpLikeIts1999():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  [EachCock] flexes in delicious concert with your quim, spurting jizz onto the trap's chest and face.");
...
private function stickWangInSandgina():void {
	...
	if(player.lowerBody.balls > 0) outputText(", makes your " + ballsDescriptLight() + " swell in readiness,");
	...
	//Megacock: 
		...
		if(player.lowerBody.balls > 0) outputText("  Your " + ballsDescriptLight() + " slap against the creature's wet opening.  ");
	...
	if(player.lowerBody.balls > 0) outputText("balls ache and your ");
...
private function nagaThreesomeWithSandTrap():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.list[0].cockLength < 14) {
			...
			if(player.lowerBody.balls > 0) outputText("slapping your " + ballsDescriptLight() + " into its nose, forcing your cock all the way to the top of its tight throat.");
		}
		//14 inches or more: 
		else {
			...
			if(player.lowerBody.balls > 0) outputText("your " + ballsDescriptLight() + " swinging heavily in front of the sandtrap's nose, ");
		...
		if(player.lowerBody.cockSpot.list[0].cockLength >= 14) outputText("  You're not sure which is the more pleasing sight; her, or the rhythmic bulging in the sandtrap's neck.");
...
private function sandTrapBadEnd():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("[EachCock] hardens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap's will.  ");
	//Female: 
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Your [vagina] moistens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap's will.  ");
...
private function desztiny():void {
	...
	//Male: 
	if(player.lowerBody.cockSpot.hasCock()) outputText("Your cock strains upwards and you moan; feeling as sensitive as you do, you are both desperate to be touched and desperate not to be.  ");
	//Female:
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("You feel but barely hear the drip of moisture on the sand, both your [asshole] and [vagina] beading with oily need.  ");
...
private function sandTrapBadEndFinale():void {
	...
	if(player.gender > 0) {
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("cock harden");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" and masturbating [oneCock] desperately");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" and fingering your [vagina] desperately");
