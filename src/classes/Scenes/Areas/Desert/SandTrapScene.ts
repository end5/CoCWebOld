export default class SandTrapScene {
	...
	public encounterASandTarp(): void {
		...
		spriteSelect(97);
		if (Flags.list[FlagEnum.SANDTRAP_LOSS_REPEATS] >= 2 && player.upperBody.head.face.eyeType == EyeType.BLACK_EYES_SAND_TRAP && player.upperBody.wingType == WingType.GIANT_DRAGONFLY) {
			sandTrapBadEnd();
			return;
		}
		Flags.list[FlagEnum.TIMES_ENCOUNTERED_SAND_TRAPS]++;
		if (Flags.list[FlagEnum.TIMES_ENCOUNTERED_SAND_TRAPS] == 1) {
			//femininity <= 65: 
			if (player.femininity <= 65) {
				MainScreen.text("You are exploring the endlessly undulating landscape of the desert when you hear an agonized cry for help over the nearest dune.  You quickly scramble up it and are confronted by the sight of a small, flat lake of exceptionally fine sand, overlooked on each side by tall dunes.  Trapped in the middle of it is a young woman with black hair, up to her armpits in the sand.  \"<i>Oh, thank the Gods!</i>\" she cries, spotting you.  \"<i>I'm sinking, please help me!</i>\"  The word 'quicksand' flashes through your mind like fire and you take a nervous step back, attempting to quickly evaluate the situation.");
				if (player.stats.int > 65) MainScreen.text("  Something about this scene puts you on edge.  How come you've never encountered quicksand in this desert before, with no water and its steady, stubborn, but not violent winds?  There's something odd about the woman, too - although she can't be a sand witch with that hair, you can't quite make her features out clearly through the heat haze.");
				MainScreen.text("  You think you could just about reach her with your hand if you were careful.  If you're going to save her, it has to be now; even in the short time you've been here she has sunk down to her collarbone.  She stares at you with plaintive despair.");
			}
			else {
				MainScreen.text("You are exploring the endlessly undulating landscape of the desert when you hear an agonised cry for help over the nearest dune.  You quickly scramble up it and are confronted by the sight of a small, flat lake of exceptionally fine sand, overlooked on each side by tall dunes.  Trapped in the middle of it is a young man with black hair, up to his armpits in the sand.  \"<i>Oh, thank the Gods!</i>\" he cries, spotting you.  \"<i>I'm sinking, please help me!</i>\"  The word 'quicksand' flashes through your mind like fire and you take a nervous step back, attempting to quickly evaluate the situation.  ");
	...
	private saveTheSandTarps(): void {
		...
		//(femininity <= 65): 
		if (player.femininity <= 65) {
			MainScreen.text("You carefully kneel at the edge of the quicksand and stretch your hand out towards the stricken woman, commanding her to grab hold.   With effort she pulls an arm clear of the treacherous powder and, straining, reaches out to grab the very tips of your fingers... before, with a triumphant sneer, pulling as hard as she can.  Thrown off-balance, you fall face first into the quicksand.  Coughing, you right yourself and struggle desperately to get back up the side, but you have no leverage, and your thrashing causes you to sink waist deep into what feels like dry, impossibly heavy water.");
		...
		//(else fem > 65): 
			...
			MainScreen.text("You carefully kneel at the edge of the quicksand and stretch your hand out towards the stricken man, commanding him to grab hold.   With effort he pulls an arm clear of the treacherous powder and, straining, reaches out to grab the very tips of your fingers... before, with a triumphant sneer, pulling as hard as he can.  Thrown off-balance, you fall face first into the quicksand.  Coughing, you right yourself and struggle desperately to get back up the side, but you have no leverage, and your thrashing causes you to sink waist deep into what feels like dry, impossibly heavy water.");
	...
	private dontSaveTheTarps(): void {
		MainScreen.clearText();
		spriteSelect(97);
		MainScreen.text("You carefully step backwards from the quicksand, not taking your eyes off the stricken ");
		if (player.femininity <= 65) MainScreen.text("woman");
		else MainScreen.text("man");
		MainScreen.text(".  \"<i>Please!</i>\" ");
		if (player.femininity <= 65) MainScreen.text("she");
		else MainScreen.text("he");
		MainScreen.text(" cries, neck sinking into the drift.  \"<i>Don't let me die like this... oh, very well.  Have it YOUR way!</i>\"  The ");
		if (player.femininity <= 65) MainScreen.text("woman");
		else MainScreen.text("man");
		MainScreen.text(" suddenly thrusts upwards and plunges a fist into the fluid sand surrounding ");
		if (player.femininity <= 65) MainScreen.text("her");
		else MainScreen.text("him");
		MainScreen.text(".  The dune you are standing on caves inwards, turning into a shifting slide to the morass below you.  You stumble and hop backwards as fast as you can, managing to stay just ahead of the ");
		if (player.femininity <= 65) MainScreen.text("woman");
		else MainScreen.text("man");
		MainScreen.text("'s spell.  In front of you, sand collapses downwards into a great conical depression, at the epicentre of which is your supposed rescuee.  With ");
		if (player.femininity <= 65) MainScreen.text("her");
		else MainScreen.text("his");
		MainScreen.text(" midriff exposed and glamour gone, the creature looks quite different; six eyes as black as its hair look at you hungrily from its beautiful androgynous face, whilst four slender arms trail patterns in the sand around its willowy, flat-chested midsection.  It wriggles its body tauntingly at you, making the sand beneath it move ponderously.  You glimpse insect chitin beneath its flat humanoid belly; the buried half of this creature must be monstrous.");
	...
	internal function pcBeatsATrap(): void {
		MainScreen.clearText();
		spriteSelect(97);
		Flags.list[FlagEnum.SANDTRAP_LOSS_REPEATS] = 0;
		//PC HP victory: 
		if (monster.HP < 1) MainScreen.text("The sandtrap groans and collapses forwards into the dirt.  With the creature no longer controlling it, the sand underneath you feels a great deal more stable... climbing out won't be too difficult.");
		//PC lust victory: 
		else MainScreen.text("The sand around you stops sinking.  Overpowered with lust, the sandtrap moans and gives up control over itself and its pit to desperately run one set of hands over its flat chest whilst the other pushes into the sand to masturbate... whatever it has down there.  With your foe in this state, climbing out won't be so difficult now.");

		let nagaThreeSome: Function = null;
		let putYourDickInIt: Function = null;
		let rideDatSantTrap: Function = null;
		let useSandTarpsHand: Function = null;
		...
		if (player.lowerBody.isNaga() && player.statusAffects.has("Naga") && player.gender > 0 && player.upperBody.head.face.faceType == FaceType.SNAKE_FANGS) nagaThreeSome = nagaThreesomeWithSandTrap;

		//Requires: Penis and str requirement
		if (player.lowerBody.cockSpot.hasCock() && player.stats.str >= 60) putYourDickInIt = stickWangInSandgina;

		//\"<i>Ride</i>\" (Z)
		if (player.lowerBody.vaginaSpot.hasVagina()) rideDatSantTrap = rideDatSandTarpLikeIts1999;
		//\"<i>Hands</i>\" (Z)
	...
	private dickwieldersLoseToSandTarps(): void {
		...
		if (!player.lowerBody.isTaur()) MainScreen.text("armpits");
		else MainScreen.text("forelegs");
		MainScreen.text(".  You try and turn your head to see what it is doing but it gently but insistently pushes your face into the yielding sand.  \"<i>You arrrrhe a fine little drone.  I wwill thank you for yourrh hard work... with the fruits of ssame hard work.</i>\" What does it mean by...?");
		...
		MainScreen.text("  The creature pushes your head down, making soothing shushing noises as it feeds more of its bulbous length into your ass.");
		if (player.analCapacity() < 40) MainScreen.text("  Although the oil which coats your thoughts and reflexes with sensual supplication makes things easier, the feeling is still incredibly intense, and you grit your teeth as something round travels down the creature's ovipositor and stretches your tight hole wide.");
		else MainScreen.text("  Your accommodating ass feels like it was made for this purpose, and with the creature's oil soothing your thoughts and reflexes you savor the creature taking you like this as if it were your quim.  You moan as something round travels down the creature's ovipositor and into your wet ass, the sensation making [eachCock] hard again.");
		...
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" and cunt");
		MainScreen.text(" flexing out more moisture into the sand beneath you.  The oil-induced serenity and the sexual marathon the sandtrap has put you through are too much and, even with the creature still relentlessly pumping away at you, you pass out.");
		...
		monster.statusAffects.add(new StatusAffect("Fertilized", 0, 0, 0, 0)));
		sandTrapPregChance();
		player.orgasm();
		player.stats.lib += 1;
		player.slimeFeed();
	...
	private chicksLoseToSandTarp(): void {
		MainScreen.clearText();
		spriteSelect(97);
		MainScreen.text("The sandtrap holds your hands whilst it pushes you further down with its other set of arms, until only your head is above the sand.  Below the surface you try to weakly move your limbs, try to work yourself out of this situation, but it is impossible; the sand feels impossibly heavy and is packed against you.  The sandtrap seems to have no such difficulty.  It sinks gracefully downwards until its face is almost level with yours.  You feel something wet touch your thigh and you try to flinch, but aside from flexing your muscles you cannot move.");
		...
		if (monster.statusAffects.has("Fertilized")) MainScreen.text("  \"<i>A cute little wwworker hasz fallen into my home.  Wwwell, that isz ok.  Fortunately for hwyou, I already have some eggsz ready.  Hhyou will get to be a queen! Iszn't that exciting?</i>\"");
		else MainScreen.text("  \"<i>A cute little wwworker hasz fallen into my home, but I have no fertile eggsz to givve it.  Wwwell, that isz ok.  Hwwe can still have fun, can't we?</i>\"");
		...
		if (player.analCapacity() < 30) MainScreen.text("Although the oil which coats your thoughts and reflexes with sensual supplication makes things easier, the feeling is still incredibly intense, and you grit your teeth as something round travels down the creature's ovipositor and stretches your tight ass wide.");
		else MainScreen.text("Your accommodating ass feels like it was made for this purpose, and with the creature's oil soothing your thoughts and reflexes you savor the creature taking you like this as if it were your second quim.  You moan as something round travels down the creature's ovipositor and into your wet ass.");
		MainScreen.text("  Slowly the creature begins to pick up its rhythm again, sliding in and out of your pussy as it breeds you.");
		...
		if (monster.statusAffects.has("Fertilized")) sandTrapPregChance();
		player.orgasm();
		player.stats.lib += 1;
		player.slimeFeed();
		//reduce lust, increase lib, slimefeed, reset hours since cum
	...
	private genderlessLoss(): void {
		...
		if (monster.statusAffects.has("Fertilized")) MainScreen.text("\"<i>Not a worker OR a drone! Wwwell, that isz ok.  Even ants who hhhave nothing can be someone for somebody.  And hhyou can be mommy for my childrrhen!</i>\"");
		else MainScreen.text("\"<i>Not a worker OR a drone! Wwwell, that isz ok.  Hwwe can still have fun, can't we?</i>\"");
		...
		MainScreen.text("  Slowly the creature begins to pick up its rhythm again, sliding in and out of your butt as it breeds you.");
	...
	private sandTrapPregChance(): void {
		...
		if (monster.statusAffects.has("Fertilized"))
			player.buttKnockUp(PregnancyType.SANDTRAP_FERTILE, PregnancyType.INCUBATION_SANDTRAP, 1, 1);
		else
			player.buttKnockUp(PregnancyType.SANDTRAP, PregnancyType.INCUBATION_SANDTRAP, 1, 1);
	}

	public birfSandTarps(): void {
		spriteSelect(97);
		MainScreen.text("\nYour eyes widen as a gout of oil suddenly gushes from your ass.  Before panic can set in, an incredible light-headedness overtakes you.  Dreamily, you discard your [armor] and squat.  More oil oozes out of you, and in your hazy euphoria, you scoop some of it up and rub it dreamily into your " + BreastDescriptor.describeNipple(0) + "s.  Part of you is disgusted at yourself, questioning what you are doing, but that is one voice in a million-strong chorus crooning you into total relaxation... the oil clings to your skin and seems to radiate warmth and softness.  Something round stretches your rectum wide, but in your state the sensation is practically orgasmic.");
		//[Male: 
		if (player.gender == 1) {
			MainScreen.text("  You roll your eyes to the sky and moan, [eachCock] growing hard as you push out the egg.");
			//[(mans and qualified horses only)]
			if (!player.lowerBody.isTaur() || (player.tallness * (5 / 6) < player.lowerBody.cockSpot.list[player.longestCock()].cockLength)) MainScreen.text("  Your oily hands descend upon your cock, and you massage your shaft as you feel the pressure in your bowels intensify again.");
		}
		//Female: 
		else if (player.gender == 2) {
			MainScreen.text("  You roll your eyes to the sky and moan, your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " moistening as you push out the egg.");
			//[(no fukken horses from here)]
			if (!player.lowerBody.isTaur()) MainScreen.text("  Your oily hands push softly into your cleft, fingering your needy " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " as you feel the pressure in your bowels intensify again.");
		}
		//Herm: 
		else if (player.gender == 3) {
			MainScreen.text("  You roll your eyes to the sky and moan, [eachCock] growing hard and your [vagina] moistening as you push out the egg.");
			//[(no horses)]
			if (!player.lowerBody.isTaur()) MainScreen.text("  Your oily hands descend upon your genitals and you begin to slowly pump your shaft and finger your needy " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " as you feel the pressure in your bowels intensify again.");
		}
	...
	private useSandTarpsHands(): void {
		...
		if (player.gender == 1) {
			...
			if (player.lowerBody.cockSpot.get(0).cockLength < 6) {
				// balls:
				if (player.lowerBody.balls == 0) MainScreen.text("\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " + CockDescriptor.describeCock(player, 0) + " to stimulate you even further.");
				else MainScreen.text("\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " + CockDescriptor.describeCock(player, 0) + " to incite you even further, whilst it uses its free hand to gently cup your [balls].");
				MainScreen.text("  Your small size doesn't seem to bother it; if anything, judging from the red rising in its thin cheeks and its shortening breath, the girliness of your cock actually seems to excite it.  It pauses and, whilst you are both panting lightly, it slowly and gently circles your head with its index finger whilst it still holds the entirety of your cock with the rest of its hand.");
			}
			//Cock 6-16 inches: 
			else if (player.lowerBody.cockSpot.get(0).cockLength < 16) {
				//[no balls:
				if (player.lowerBody.balls == 0) MainScreen.text("\n\nThe sandtrap glides its hand up and down your " + CockDescriptor.describeCock(player, 0) + ", picking up a warm smooth rhythm.");
				//Balls:
				else MainScreen.text("\n\nThe sandtrap glides its hand up and down your " + CockDescriptor.describeCock(player, 0) + ", picking up a warm smooth rhythm as it gently cups your [balls] with its free hand.]  It slows down intermittently to move its palm around your length, letting every inch of your dick feel the tips of its delicate fingers.");
			}
			//Cock more than 16 inches: 
			else MainScreen.text("\n\nThe sandtrap quickly comes to the realisation it needs both hands for this.  Still cupping your behind, it moves one oily hand up the top end of your massive shaft, while the other works the other half.  You sigh with deep satisfaction, enjoying the all-too-rare sensation of having every inch of your " + CockDescriptor.describeCock(player, 0) + " immersed in pleasure.  The creature screws its face up and looks away in distaste at the size of your straining dick, which is in danger of poking it in the nose.  As you enjoy the warm, slippery sensation inundating you, you find you couldn't care less what it thinks.");
			//number of cock fork
			//One cock: 
			if (player.lowerBody.cockSpot.count() == 1) {
				MainScreen.text("  The sandtrap pumps you faster and faster until its slippery hands are a blur of motion.  It clutches onto your butt cheeks so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.");
			...
			//Two cocks: 
				...
				MainScreen.text("  As the sandtrap begins to pump you faster, your disregarded " + CockDescriptor.describeCock(player, 1) + " bobs in hard, eager concert with your " + CockDescriptor.describeCock(player, 0) + ".  You gasp as the trap slows down momentarily to grasp your second sex.  It is clumsy at first as it moves its lubricated palms in tandem, then with increasing confidence as it finds a position and rhythm that suits it, until it is jerking you off furiously, clutching your butt cheeks so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.");
				MainScreen.text("\n\nThe warm, oily double sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a wordless bark, spurt out line after line of jizz.  The trap flinches slightly when your first cumshots hit it in the face and chest, but then goes back to gently rubbing you, guiding you up and down your warm, slippery orgasm.");
			}
			//Three cocks: 
			else if (player.lowerBody.cockSpot.count() == 3) {
				MainScreen.text("\n\nAs the sandtrap begins to pump you faster, your two other, disregarded cocks bob in hard, eager concert with your " + CockDescriptor.describeCock(player, 0) + ".  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as  one of its hands clutching your ass releases and snakes back around to take hold of your " + CockDescriptor.describeCock(player, 2) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking all three of your cocks in rhythm.");
				if (player.lowerBody.balls > 0) MainScreen.text("  It grins up at you shyly and, with its final hand, cups your [balls].  ]You laugh with an almost hysterical edge to the sky at how mindboggling getting a triple handjob feels, and the trap giggles slightly as well despite itself.");
			...
			//Four cocks: 
			else if (player.lowerBody.cockSpot.count() == 4) {
				MainScreen.text("\n\nAs the sandtrap begins to pump you faster, your three other, disregarded cocks bob in hard, eager concert with your " + CockDescriptor.describeCock(player, 0) + ".  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as both of its hands clutching your ass release and snake back around to take hold of your " + CockDescriptor.describeCock(player, 2) + " and " + CockDescriptor.describeCock(player, 3) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking all four of your cocks in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels; it glances up at you and giggles shyly as you look into each other's eyes and share a thought.  \"<i>At last, I've found somebody who complements my body perfectly.</i>");
			...
			//More than four cocks: 
				...
				MainScreen.text("\n\nAs the sandtrap begins to pump you faster, your many other, disregarded cocks bob in hard, eager concert with your " + CockDescriptor.describeCock(player, 0) + ".  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as its two hands clutching your ass snake back around to take hold of your " + CockDescriptor.describeCock(player, 2) + " and " + CockDescriptor.describeCock(player, 3) + ".  It is clumsy at first as it works its way around the incredibly complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking four of your cocks in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels, and the trap giggles slightly as well despite itself.  Hazily you wonder if it is too much to wish that this creature, with its excellent wrist action, had just one or two more hands to its name.");
				MainScreen.text("\n\nThe warm, oily quadrupled sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a wordless bark, spurt out line after line of jizz.  The trap flinches slightly when your first shots hit it in the face and chest, but then goes back to gently rubbing you, guiding you up and down your warm, slippery multiple orgasm as you spurt like a cum sprinkler in front of it.");
			}
		}
		//Female: 
			...
			if (player.tone < 60) MainScreen.text("  It picks up the pace gradually and soon it is doubly frigging you as fast as its fingers will allow, its other hands sinking deep into your soft, pillowy behind as it clutches onto you.");
			else MainScreen.text("  It picks up the pace gradually and soon it is doubly frigging you as fast as its fingers will allow, its other hands clutching onto your tight, muscled behind so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.");
		...
		//Herm: 
			...
			MainScreen.text("\n\nOne pair of the creature's hands slowly brush up your inner thighs whilst the other two feel around your \"<i>armor</i>\", clumsily loosening clasps and belts as it finds them.  One hand finds your " + CockDescriptor.describeCock(player, 0) + " and, after pausing for a split second, slides its fingers around your shaft and begins to slowly rub up and down.  You sigh as you quickly harden in the creature's smooth, dry, enveloping grasp.  You look into the sandtrap's face as it works; blushing, it looks down from your gaze and concentrates upon what it is doing.  It begins to pump you faster and you hiss slightly at the abrasion this causes.  At this the sandtrap makes a throaty sound.  Curiously you look down in time to see it bring its other pair of hands up to its mouth and deposit a great dollop of clear ooze into its cupped palms.  Your disgust turns to delight as it swaps its hands and begins to massage its oil into your length, the warm lubricant allowing the creatures' fingers to glide around your shaft and head like they were made of silk.  As it moves its first hand up and down, it tentatively moves its second further between your legs.  It finds your [vagina] and, after pausing for a split second, slides its slender fingers into your moistness.  Although it is clumsy at first, the oil which coats its digits make them glide over your shaft and labia as if they were entirely frictionless.  It quickly gains confidence and begins to gently frig you as it slides its warm, slippery grasp up and down your " + CockDescriptor.describeCock(player, 0) + ", circling your [clit] whilst occasionally dipping into your dripping hole.  You sigh and begin to lose yourself in the wet, silky sensation.");
			//Cock less than 6 inches:  
			if (player.lowerBody.cockSpot.get(0).cockLength < 6) {
				//[no balls: 
				if (player.lowerBody.balls == 0) MainScreen.text("\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " + CockDescriptor.describeCock(player, 0) + " to stimulate you even further.");
				else MainScreen.text("\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " + CockDescriptor.describeCock(player, 0) + " to incite you even further, whilst it uses its free hand to gently cup your [balls].");
				MainScreen.text("  Your small size doesn't seem to bother it; if anything, judging from the red rising in its thin cheeks and its shortening breath, the girliness of your cock actually seems to excite it.  It pauses and, while you are both panting lightly, it slowly and gently circles your head with its index finger, still holding the entirety of your cock with the rest of its hand.");
			}
			//Cock 6-16 inches: 
			else if (player.lowerBody.cockSpot.get(0).cockLength < 16) {
				if (player.lowerBody.balls == 0) MainScreen.text("\n\nThe sandtrap glides its hand up and down your " + CockDescriptor.describeCock(player, 0) + ", picking up a warm smooth rhythm.");
				else MainScreen.text("\n\nThe sandtrap glides its hand up and down your " + CockDescriptor.describeCock(player, 0) + ", picking up a warm smooth rhythm as it gently cups your [balls] with its free hand.]  It slows down intermittently to move its fingers around your length, letting every inch of your dick feel the tips of its delicate fingers.");
			}
			else {
			...
			//Two cocks: 
				...
				MainScreen.text("\n\nAs the sandtrap begins to pump you faster, your disregarded " + CockDescriptor.describeCock(player, 1) + " bobs in eager concert with your " + CockDescriptor.describeCock(player, 0) + ".  You gasp as the trap slows down momentarily to reach one of the hands gripping your ass back around to grasp your second manhood.  It is clumsy at first as it moves its lubricated palms in tandem, then with increasing confidence as it finds a position and rhythm that suits it, it is jerking and flicking you off furiously, clutching your butt cheek with its final hand for what seems like dear life.");
				MainScreen.text("\n\nThe warm, oily triple sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a harsh moan, spurt out line after line of jizz whilst gushing femcum coat its hands and your thighs.  The trap flinches slightly when your first shots hit it in the face and chest, but then goes back to gently rubbing and frigging you as you ride your glorious orgasms out.");
			}
			//hree cocks: 
			else if (player.lowerBody.cockSpot.count() == 3) {
				MainScreen.text("\n\nAs the sandtrap begins to pump you faster, your two other, disregarded cocks bob in eager concert with your " + CockDescriptor.describeCock(player, 0) + ".  You gasp as the trap slows down momentarily to reach both hands clutching your ass back around to take hold of your " + CockDescriptor.describeCock(player, 1) + " and " + CockDescriptor.describeCock(player, 2) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking three of your cocks whilst fingering you in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels; it glances up at you and giggles shyly as you look into each other's eyes and share a thought.  At last, I've found somebody who compliments my body perfectly.");
			...
			//More than three cocks: 
				...
				MainScreen.text("\n\nAs the sandtrap begins to pump you faster, your many other, disregarded cocks begin to harden in eager concert with your " + CockDescriptor.describeCock(player, 0) + ".  You gasp as the trap slows down momentarily to reach both of its hands clutching your ass back around to take hold of your " + CockDescriptor.describeCock(player, 1) + " and " + CockDescriptor.describeCock(player, 2) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking three of your cocks whilst fingering you in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels, and the trap giggles slightly as well despite itself.  Hazily you wonder if it is too much to wish that this creature, with its excellent wrist action, had just one or two more hands to its name.");
	...
	private rideDatSandTarpLikeIts1999(): void {
		...
		MainScreen.text("\n\n\"<i>You like that, don't you, sand sissy?</i>\"  It is breathing heavily but it still won't look into your face. \"<i>Look me in the eye!</i>\" As it finally does so you push your oil-covered fingers into its mouth and it moans around them as you swab its tongue and cheeks with its own grease.  You press your body against it, your " + BreastDescriptor.describeAllBreasts(player) + " pushing into its own flat chest; it is powerless to resist you as its own pheromones take effect upon it, and you slowly push it backwards onto the flat of its back on the sloping sand.  You sit your moist " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " at the base of its pseudo-cock and then slowly lead yourself back, luxuriating in pushing your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " up this oily slide.   It moans and shifts itself upwards needily, but you tut mockingly and stop your slow slide inches away from its tip.");
		MainScreen.text("\n\n\"<i>Ask for it.</i>\"");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  [EachCock] flexes in delicious concert with your quim, spurting jizz onto the trap's chest and face.");
	...
	private stickWangInSandgina(): void {
		...
		MainScreen.text("\n\nYou take a moment to look your strange conquest over.  Even up close you can't tell from its thin, fey beauty whether it is male or female.  Although it is affecting to look angry at you, its sculpted cheeks are quite flushed and there is a definite undercurrent of desire in its eyes; you suspect your display of strength and dominance appeals to it on some perverse, animalistic level.  Still smiling victoriously, you bend your face into it, deliberately invading its personal space, silently demonstrating who is in charge here.  It can't hold your gaze and, blushing, looks downwards.  This submissive demureness sends heat rushing towards your " + CockDescriptor.describeMultiCockShort(player) + "; the desire to fuck builds in you, to plough, to sink your cock into this strange, eerily beautiful creature, to establish once and for all who is giving and who is receiving around here... the only question is, how?  Its slim, androgyne upper body is devoid of obvious sexual features.  Your eyes roam steadily downwards.  Where its insect half meets its human half, where its cock should be, it instead has a genital slit; from this is emerging a long, black prong, dripping with clear oil.  Your show of dominance is turning this creature on, and it is powerless to disguise it.  It looks up at you with timid hopefulness... but you are in no mood for that.  There is a definite air of femininity to this creature, a pheromone scent which speaks directly to your " + CockDescriptor.describeCock(player, 0) + ", makes you hard");
		if (player.lowerBody.balls > 0) MainScreen.text(", makes your " + BallsDescriptor.describeBalls(true, true, player) + " swell in readiness,");
		MainScreen.text(" and fills you with a deep, red urge; you grit your teeth in frustration at its essentially male upper half.  Its female half must be what is buried beneath the sand.  <i>Well,</i> says your dick.  <i>You've come this far...</i>");
	...
	private nagaThreesomeWithSandTrap(): void {
		...
		//Female: 
			...
			MainScreen.text("\n\nThe naga is soon deep within the sandtrap's oil fuge and she slows her thrusting down as it relaxes her and amplifies what she can feel, smiling euphorically as she slowly rotates on the creature's leaking pseudo-cock.  You pace yourself in sympathy; having already reached one ecstatic high you're more than happy to simply sit back and enjoy the sandtrap's worshipful ministrations, tending to every corner of your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".  You've also had enough oil rubbed into you to feel quite dreamy yourself; the sand, the sun and even your own vagina seem bigger and more encompassing than they were before.");
			...
			if (player.cumQ() > 1000) MainScreen.text("  The long, sensuous tongue bath has pushed your body into overdrive and the waterfall you unleash is even more uncontrollable than the last; you wonder dizzily if you haven't just lost control of your bladder.  The hypnotised sandtrap gladly drinks what you give it, but it's far too much and soon it is coughing your girlcum back up, through its nose and back onto your still contracting " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".");
		}
	...
	private sandTrapBadEnd(): void {
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("[EachCock] hardens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap's will.  ");
		//Female: 
		else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("Your [vagina] moistens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap's will.  ");
		MainScreen.text("You inhale hard and try to clear your head, forcing yourself not to sleepwalk downwards into the Sandtrap's waiting embrace.  The creature doesn't seem to mind your hesitation; it smiles softly and confidently, waiting for you with the infinite patience of its kind.  You need to make a choice here fast, before your burning body makes it for you.");
		//Fight/Desztiny/Leave
		menu();
		MainScreen.addButton(0, "Fight", sandTrapBadEndFight);
		MainScreen.addButton(1, "Desztiny", desztiny);
		MainScreen.addButton(4, "Leave", leaveSandTrapBadEnd);
	...
	private sandTrapBadEndFight(): void {
		MainScreen.clearText();
		spriteSelect(97);
		MainScreen.text("You shake yourself out of your fog, clench your [weapon] and grit your teeth.  You are your own person, dammit, and you will show this creature just what that means!  The Sandtrap slowly lowers its arms and looks at you with almost parental disappointment as you march into its pit with the obvious intention of beating the crap out of it.  \"<i>You want to dance again, Flytrap?</i>\" it sighs.  \"<i>I would prefer that you look at yourself and accept what you are, but I will force you to accept it if that is what you wish.</i>\"");
	...
	private leaveSandTrapBadEnd(): void {
		MainScreen.clearText();
		spriteSelect(97);
		MainScreen.text("With some effort you break your stare with the Sandtrap, turn away and step back towards camp, resolving to leave this disturbing scenario with its disturbing thoughts behind.");
	...
	private desztiny(): void {
		MainScreen.clearText();
		spriteSelect(97);
		MainScreen.text("The Sandtrap is right.  What your body and subconscious are telling you is so right.  At your acceptance of what the creature is saying warmth floods through your body; your mind bubbles as blood rushes towards your skin, making you feel incredibly sensitive, incredibly sexual.  ");
		//Male: 
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("Your cock strains upwards and you moan; feeling as sensitive as you do, you are both desperate to be touched and desperate not to be.  ");
		//Female:
		else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("You feel but barely hear the drip of moisture on the sand, both your [asshole] and [vagina] beading with oily need.  ");
	...
	private sandTrapBadEndFinale(): void {
		...
		if (player.gender > 0) {
			...
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("cock harden");
			else MainScreen.text("vagina dribble");

		}
		MainScreen.text(".  Yes.  You remember your quest.");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" and masturbating [oneCock] desperately");
		else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" and fingering your [vagina] desperately");
		MainScreen.text(".  But, of course, as a fresh dominant urge takes you and drags you reluctantly from your needy genitals, there is much still left to be done.");
