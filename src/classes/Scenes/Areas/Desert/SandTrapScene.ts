...
private saveTheSandTarps():void {
	...
	//(femininity <= 65): 
	if(player.femininity <= 65) {
		Render.text("You carefully kneel at the edge of the quicksand and stretch your hand out towards the stricken woman, commanding her to grab hold.   With effort she pulls an arm clear of the treacherous powder and, straining, reaches out to grab the very tips of your fingers... before, with a triumphant sneer, pulling as hard as she can.  Thrown off-balance, you fall face first into the quicksand.  Coughing, you right yourself and struggle desperately to get back up the side, but you have no leverage, and your thrashing causes you to sink waist deep into what feels like dry, impossibly heavy water.");
	...
	//(else fem > 65): 
		...
		Render.text("You carefully kneel at the edge of the quicksand and stretch your hand out towards the stricken man, commanding him to grab hold.   With effort he pulls an arm clear of the treacherous powder and, straining, reaches out to grab the very tips of your fingers... before, with a triumphant sneer, pulling as hard as he can.  Thrown off-balance, you fall face first into the quicksand.  Coughing, you right yourself and struggle desperately to get back up the side, but you have no leverage, and your thrashing causes you to sink waist deep into what feels like dry, impossibly heavy water.");
...
private dontSaveTheTarps():void {
	clearOutput();
	spriteSelect(97);
	Render.text("You carefully step backwards from the quicksand, not taking your eyes off the stricken ");
	if(player.femininity <= 65) Render.text("woman");
	else Render.text("man");
	Render.text(".  \"<i>Please!</i>\" ");
	if(player.femininity <= 65) Render.text("she");
	else Render.text("he");
	Render.text(" cries, neck sinking into the drift.  \"<i>Don't let me die like this... oh, very well.  Have it YOUR way!</i>\"  The ");
	if(player.femininity <= 65) Render.text("woman");
	else Render.text("man"); 
	Render.text(" suddenly thrusts upwards and plunges a fist into the fluid sand surrounding ");
	if(player.femininity <= 65) Render.text("her");
	else Render.text("him");
	Render.text(".  The dune you are standing on caves inwards, turning into a shifting slide to the morass below you.  You stumble and hop backwards as fast as you can, managing to stay just ahead of the ");
	if(player.femininity <= 65) Render.text("woman");
	else Render.text("man");
	Render.text("'s spell.  In front of you, sand collapses downwards into a great conical depression, at the epicentre of which is your supposed rescuee.  With ");
	if(player.femininity <= 65) Render.text("her");
	else Render.text("his");
	Render.text(" midriff exposed and glamour gone, the creature looks quite different; six eyes as black as its hair look at you hungrily from its beautiful androgynous face, whilst four slender arms trail patterns in the sand around its willowy, flat-chested midsection.  It wriggles its body tauntingly at you, making the sand beneath it move ponderously.  You glimpse insect chitin beneath its flat humanoid belly; the buried half of this creature must be monstrous.");
...
internal function pcBeatsATrap():void {
	...
	//PC HP victory: 
	if(monster.HP < 1) Render.text("The sandtrap groans and collapses forwards into the dirt.  With the creature no longer controlling it, the sand underneath you feels a great deal more stable... climbing out won't be too difficult.");
	//PC lust victory: 
	else Render.text("The sand around you stops sinking.  Overpowered with lust, the sandtrap moans and gives up control over itself and its pit to desperately run one set of hands over its flat chest whilst the other pushes into the sand to masturbate... whatever it has down there.  With your foe in this state, climbing out won't be so difficult now.");
...
private dickwieldersLoseToSandTarps():void {
	...
	//[High cum: 
	if(player.cumQ() > 1000) Render.text("  The sandtrap gasps and coos as you ejaculate over and over again, and beneath you feel your cum wet the sands as it beads around the clenching grip it has you in.");
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Ignored entirely, your " + vaginaDescript(0) + " quivers and orgasms in tandem, flexing and wetting fruitlessly against the dry water pressing against it.");
	...
	if(!player.isTaur()) Render.text("armpits");
	else Render.text("forelegs");
	Render.text(".  You try and turn your head to see what it is doing but it gently but insistently pushes your face into the yielding sand.  \"<i>You arrrrhe a fine little drone.  I wwill thank you for yourrh hard work... with the fruits of ssame hard work.</i>\" What does it mean by...?");
	...
	Render.text("  The creature pushes your head down, making soothing shushing noises as it feeds more of its bulbous length into your ass.");
	if(player.analCapacity() < 40) Render.text("  Although the oil which coats your thoughts and reflexes with sensual supplication makes things easier, the feeling is still incredibly intense, and you grit your teeth as something round travels down the creature's ovipositor and stretches your tight hole wide.");
	else Render.text("  Your accommodating ass feels like it was made for this purpose, and with the creature's oil soothing your thoughts and reflexes you savor the creature taking you like this as if it were your quim.  You moan as something round travels down the creature's ovipositor and into your wet ass, the sensation making [eachCock] hard again.");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and cunt");
	Render.text(" flexing out more moisture into the sand beneath you.  The oil-induced serenity and the sexual marathon the sandtrap has put you through are too much and, even with the creature still relentlessly pumping away at you, you pass out.");
...
private chicksLoseToSandTarp():void {
	clearOutput();
	spriteSelect(97);
	Render.text("The sandtrap holds your hands whilst it pushes you further down with its other set of arms, until only your head is above the sand.  Below the surface you try to weakly move your limbs, try to work yourself out of this situation, but it is impossible; the sand feels impossibly heavy and is packed against you.  The sandtrap seems to have no such difficulty.  It sinks gracefully downwards until its face is almost level with yours.  You feel something wet touch your thigh and you try to flinch, but aside from flexing your muscles you cannot move.");
	...
	if(monster.statusAffects.has("Fertilized")) Render.text("  \"<i>A cute little wwworker hasz fallen into my home.  Wwwell, that isz ok.  Fortunately for hwyou, I already have some eggsz ready.  Hhyou will get to be a queen! Iszn't that exciting?</i>\"");
	else Render.text("  \"<i>A cute little wwworker hasz fallen into my home, but I have no fertile eggsz to givve it.  Wwwell, that isz ok.  Hwwe can still have fun, can't we?</i>\"");
	...
	if(player.analCapacity() < 30) Render.text("Although the oil which coats your thoughts and reflexes with sensual supplication makes things easier, the feeling is still incredibly intense, and you grit your teeth as something round travels down the creature's ovipositor and stretches your tight ass wide.");
	else Render.text("Your accommodating ass feels like it was made for this purpose, and with the creature's oil soothing your thoughts and reflexes you savor the creature taking you like this as if it were your second quim.  You moan as something round travels down the creature's ovipositor and into your wet ass.");
	Render.text("  Slowly the creature begins to pick up its rhythm again, sliding in and out of your pussy as it breeds you.");
...
private genderlessLoss():void {
	...
	if(monster.statusAffects.has("Fertilized")) Render.text("\"<i>Not a worker OR a drone! Wwwell, that isz ok.  Even ants who hhhave nothing can be someone for somebody.  And hhyou can be mommy for my childrrhen!</i>\"");
	else Render.text("\"<i>Not a worker OR a drone! Wwwell, that isz ok.  Hwwe can still have fun, can't we?</i>\"");
	...
	Render.text("  Slowly the creature begins to pick up its rhythm again, sliding in and out of your butt as it breeds you.");
...
public birfSandTarps():void {
	...
	//[Male: 
	if(player.gender == 1) {
		Render.text("  You roll your eyes to the sky and moan, [eachCock] growing hard as you push out the egg.");
		//[(mans and qualified horses only)]
		if(!player.isTaur() || (player.tallness * (5/6) < player.lowerBody.cockSpot.list[player.longestCock()].cockLength)) Render.text("  Your oily hands descend upon your cock, and you massage your shaft as you feel the pressure in your bowels intensify again.");
	}
	//Female: 
	else if(player.gender == 2) {
		Render.text("  You roll your eyes to the sky and moan, your " + vaginaDescript(0) + " moistening as you push out the egg.");
		//[(no fukken horses from here)]
		if(!player.isTaur()) Render.text("  Your oily hands push softly into your cleft, fingering your needy " + clitDescript() + " as you feel the pressure in your bowels intensify again.");
	}
	//Herm: 
	else if(player.gender == 3) {
		Render.text("  You roll your eyes to the sky and moan, [eachCock] growing hard and your [vagina] moistening as you push out the egg.");
		//[(no horses)]
		if(!player.isTaur()) Render.text("  Your oily hands descend upon your genitals and you begin to slowly pump your shaft and finger your needy " + clitDescript() + " as you feel the pressure in your bowels intensify again.");
	}
...
private useSandTarpsHands():void {
	...
	if(player.gender == 1) {
		...
		if(player.lowerBody.cockSpot.list[0].cockLength < 6) {
			...
			Render.text("  Your small size doesn't seem to bother it; if anything, judging from the red rising in its thin cheeks and its shortening breath, the girliness of your cock actually seems to excite it.  It pauses and, whilst you are both panting lightly, it slowly and gently circles your head with its index finger whilst it still holds the entirety of your cock with the rest of its hand.");
		}
		//Cock 6-16 inches: 
		...
		if(player.lowerBody.cockSpot.count() == 1) {
			Render.text("  The sandtrap pumps you faster and faster until its slippery hands are a blur of motion.  It clutches onto your butt cheeks so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.");
		...
		//Three cocks: 
			...
			if(player.lowerBody.balls > 0) Render.text("  It grins up at you shyly and, with its final hand, cups your [balls].  ]You laugh with an almost hysterical edge to the sky at how mindboggling getting a triple handjob feels, and the trap giggles slightly as well despite itself.");
	...
	//Female: 
		...
		if(player.tone < 60) Render.text("  It picks up the pace gradually and soon it is doubly frigging you as fast as its fingers will allow, its other hands sinking deep into your soft, pillowy behind as it clutches onto you.");
		else Render.text("  It picks up the pace gradually and soon it is doubly frigging you as fast as its fingers will allow, its other hands clutching onto your tight, muscled behind so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.");
	...
	//Herm: 
		...
		if(player.lowerBody.cockSpot.list[0].cockLength < 6) {
			...
			Render.text("  Your small size doesn't seem to bother it; if anything, judging from the red rising in its thin cheeks and its shortening breath, the girliness of your cock actually seems to excite it.  It pauses and, while you are both panting lightly, it slowly and gently circles your head with its index finger, still holding the entirety of your cock with the rest of its hand.");
		}
		//Cock 6-16 inches: 
		else if(player.lowerBody.cockSpot.list[0].cockLength < 16) {
			if(player.lowerBody.balls == 0) Render.text("\n\nThe sandtrap glides its hand up and down your " + cockDescript(0) + ", picking up a warm smooth rhythm.");
...
private rideDatSandTarpLikeIts1999():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  [EachCock] flexes in delicious concert with your quim, spurting jizz onto the trap's chest and face.");
...
private stickWangInSandgina():void {
	...
	if(player.lowerBody.balls > 0) Render.text(", makes your " + ballsDescriptLight() + " swell in readiness,");
	Render.text(" and fills you with a deep, red urge; you grit your teeth in frustration at its essentially male upper half.  Its female half must be what is buried beneath the sand.  <i>Well,</i> says your dick.  <i>You've come this far...</i>");
	...
	//Megacock: 
		...
		if(player.lowerBody.balls > 0) Render.text("  Your " + ballsDescriptLight() + " slap against the creature's wet opening.  ");
		Render.text("  Any coherence flies straight out of the sandtrap's head and it can do nothing but whine, buzz and moan as you begin to thrust in and out of it, penetrating it to its very core, the [cockHead] bumping against strange bulbous objects which give and push back against your sensitive end delightfully.");
	}
...
private nagaThreesomeWithSandTrap():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.list[0].cockLength < 14) {
			...
			if(player.lowerBody.balls > 0) Render.text("slapping your " + ballsDescriptLight() + " into its nose, forcing your cock all the way to the top of its tight throat.");
		}
		//14 inches or more: 
		else {
			...
			if(player.skinType == SKIN.PLAIN) Render.text("beading sweat, ");
			Render.text("pistoning into its submissive opening with fervor, ");
			if(player.lowerBody.balls > 0) Render.text("your " + ballsDescriptLight() + " swinging heavily in front of the sandtrap's nose, ");
			Render.text("pushing as much of your lubricated dick down the creature's gullet as you can.");
		}
		...
		if(player.lowerBody.cockSpot.list[0].cockLength >= 14) Render.text("  You're not sure which is the more pleasing sight; her, or the rhythmic bulging in the sandtrap's neck.");
		Render.text("  Mindlessly, the sandtrap's four arms clutch and stroke the warm scales which swaddle its upper body as the two of you continue to fuck it.");
	...
	//Female: 
		...
		if(player.cumQ() > 1000) Render.text("  The long, sensuous tongue bath has pushed your body into overdrive and the waterfall you unleash is even more uncontrollable than the last; you wonder dizzily if you haven't just lost control of your bladder.  The hypnotised sandtrap gladly drinks what you give it, but it's far too much and soon it is coughing your girlcum back up, through its nose and back onto your still contracting " + vaginaDescript(0) + ".");
	}
...
private sandTrapBadEnd():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("[EachCock] hardens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap's will.  ");
	//Female: 
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("Your [vagina] moistens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap's will.  ");
	Render.text("You inhale hard and try to clear your head, forcing yourself not to sleepwalk downwards into the Sandtrap's waiting embrace.  The creature doesn't seem to mind your hesitation; it smiles softly and confidently, waiting for you with the infinite patience of its kind.  You need to make a choice here fast, before your burning body makes it for you.");
...
private sandTrapBadEndFight():void {
	clearOutput();
	spriteSelect(97);
	Render.text("You shake yourself out of your fog, clench your [weapon] and grit your teeth.  You are your own person, dammit, and you will show this creature just what that means!  The Sandtrap slowly lowers its arms and looks at you with almost parental disappointment as you march into its pit with the obvious intention of beating the crap out of it.  \"<i>You want to dance again, Flytrap?</i>\" it sighs.  \"<i>I would prefer that you look at yourself and accept what you are, but I will force you to accept it if that is what you wish.</i>\"");
...
private leaveSandTrapBadEnd():void {
	clearOutput();
	spriteSelect(97);
	Render.text("With some effort you break your stare with the Sandtrap, turn away and step back towards camp, resolving to leave this disturbing scenario with its disturbing thoughts behind.");
...
private sandTrapBadEndFinale():void {
	...
	if(player.gender > 0) {
		...
		if(player.lowerBody.cockSpot.hasCock()) Render.text("cock harden");
		else Render.text("vagina dribble");
		
	}
	Render.text(".  Yes.  You remember your quest.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text(" and masturbating [oneCock] desperately");
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and fingering your [vagina] desperately");
	Render.text(".  But, of course, as a fresh dominant urge takes you and drags you reluctantly from your needy genitals, there is much still left to be done.");
