...
public encounterASandTarp():void {
	...
	spriteSelect(97);
	if(flags[FlagEnum.SANDTRAP_LOSS_REPEATS] >= 2 && player.eyeType == EYES.BLACK_EYES_SAND_TRAP && player.upperBody.wingType == WING.GIANT_DRAGONFLY) {
		sandTrapBadEnd();
		return;
	}
	flags[FlagEnum.TIMES_ENCOUNTERED_SAND_TRAPS]++;
	if(flags[FlagEnum.TIMES_ENCOUNTERED_SAND_TRAPS] == 1) {
		//femininity <= 65: 
...
public sandtrapmentLoss(clear:boolean = false):void {
	...
	flags[FlagEnum.SANDTRAP_LOSS_REPEATS]++;
...
internal function pcBeatsATrap():void {
	MainScreen.clearText();
	spriteSelect(97);
	flags[FlagEnum.SANDTRAP_LOSS_REPEATS] = 0;
	//PC HP victory: 
	...
	if(player.isNaga() && player.statusAffects.has("Naga") && player.gender > 0 && player.faceType == FACE.SNAKE_FANGS) nagaThreeSome = nagaThreesomeWithSandTrap;
	
	//Requires: Penis and str requirement
	if(player.lowerBody.cockSpot.hasCock() && player.str >= 60) putYourDickInIt = stickWangInSandgina;
...
private dickwieldersLoseToSandTarps():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Ignored entirely, your " + vaginaDescript(0) + " quivers and orgasms in tandem, flexing and wetting fruitlessly against the dry water pressing against it.");
	...
	if(!player.isTaur()) MainScreen.text("armpits");
...
public birfSandTarps():void {
	...
	if(player.gender == 1) {
		...
		if(!player.isTaur() || (player.tallness * (5/6) < player.lowerBody.cockSpot.list[player.longestCock()].cockLength)) MainScreen.text("  Your oily hands descend upon your cock, and you massage your shaft as you feel the pressure in your bowels intensify again.");
	}
	//Female: 
	else if(player.gender == 2) {
		MainScreen.text("  You roll your eyes to the sky and moan, your " + vaginaDescript(0) + " moistening as you push out the egg.");
		//[(no fukken horses from here)]
		if(!player.isTaur()) MainScreen.text("  Your oily hands push softly into your cleft, fingering your needy " + clitDescript() + " as you feel the pressure in your bowels intensify again.");
	}
	//Herm: 
		...
		//[(no horses)]
		if(!player.isTaur()) MainScreen.text("  Your oily hands descend upon your genitals and you begin to slowly pump your shaft and finger your needy " + clitDescript() + " as you feel the pressure in your bowels intensify again.");
	}
...
private useSandTarpsHands():void {
	...
	if(player.gender == 1) {
		...
		if(player.lowerBody.cockSpot.list[0].cockLength < 6) {
			// balls:
...
private stickWangInSandgina():void {
	...
	if(player.lowerBody.balls > 0) MainScreen.text(", makes your " + ballsDescriptLight() + " swell in readiness,");
	...
	//Megacock: 
		...
		if(player.lowerBody.balls > 0) MainScreen.text("  Your " + ballsDescriptLight() + " slap against the creature's wet opening.  ");
...
private nagaThreesomeWithSandTrap():void {
	...
	//First encounter: 
	if(flags[FlagEnum.SANDTRAP_NAGA_3SOME] == 0) {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.list[0].cockLength < 14) {
			...
			if(player.lowerBody.balls > 0) MainScreen.text("slapping your " + ballsDescriptLight() + " into its nose, forcing your cock all the way to the top of its tight throat.");
		}
		//14 inches or more: 
		else {
			...
			if(player.skinType == SKIN.PLAIN) MainScreen.text("beading sweat, ");
			MainScreen.text("pistoning into its submissive opening with fervor, ");
			if(player.lowerBody.balls > 0) MainScreen.text("your " + ballsDescriptLight() + " swinging heavily in front of the sandtrap's nose, ");
		...
		if(player.lowerBody.cockSpot.list[0].cockLength >= 14) MainScreen.text("  You're not sure which is the more pleasing sight; her, or the rhythmic bulging in the sandtrap's neck.");
	...
	//Female: 
		...
		if(player.wetness() < 4) MainScreen.text("  Its gentle worship of your " + vaginaDescript(0) + " soon has you streaming rivulets of girlcum from your oversexed hole, dribbling down the sandtrap's face as you eagerly press yourself down for more.");
