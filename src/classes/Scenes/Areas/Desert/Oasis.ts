...
private oasisTalkAccept():void {
	...
	if(player.lowerBody == LOWER_BODY.CENTAUR) MainScreen.text("your rear legs losing their balance and sending you crashing on your flank.  ", false);
...
internal function oasisSexing():void {
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		...
		MainScreen.text("your " + multiCockDescriptLight() + " and begin to slide up and down before another demon pushes them out of the way and you ", false);
		if (player.lowerBody.cockSpot.list[0].cockLength > 25) MainScreen.text("feel your " + cockDescript(0) + " slide into a hell-girl's sloppy cunt. The demoness moans and pants in pleasure as your " + cockDescript(0) + " bottoms out in her gaping pussy and she begins to ride you like the motionless whore you momentarily are, her gigantic breasts threatening to concuss you as she slides the huge distance up and down your cock in her thirst to fill her cavelike cunt.  ", false);
		else {
			MainScreen.text("see a smaller and younger demon lowering her very tight pussy onto your " + cockDescript(0) + ". It's a tight fit, but her almost-virginal pussy is dripping wet. She bottoms her pussy out ", false);
			if(player.lowerBody.cockSpot.list[0].cockLength > 17) MainScreen.text("with hardly any of you inside her ", false);
			else if(player.lowerBody.cockSpot.list[0].cockLength > 12) MainScreen.text("about halfway down your " + cockDescript(0) + " ", false);
			else if(player.lowerBody.cockSpot.list[0].cockLength > 6) MainScreen.text("with almost all of you inside of her ", false);
			else if(player.lowerBody.cockSpot.list[0].cockLength <= 6) MainScreen.text("as her pussy lips touch the base of your " + cockDescript(0) + " ", false);
			...
			if(player.lowerBody.cockSpot.list[0].cockLength > 6) {
				MainScreen.text("The realisation that her pussy is slowly stretching over your " + cockDescript(0) + " gives you a thrill that can only be described as positively evil.  ", false);
				dynStats("cor", 1);
			}
			MainScreen.text("Soon the slender cock-whore begins to cum and she starts on what is about to be the first of several shuddering orgasms. You smile evilly as she cums around your " + cockDescript(0) + " and when her eyes roll back to the front of her head you know that there is more to come.  ", false);
		}
		//Orgasmzzzzzzz
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			...
			if(player.lowerBody.cockSpot.count() == 2) MainScreen.text(cockDescript(1) + " is", false);
	...
	if(player.lowerBody.vaginaSpot.count() > 0) {
		...
		//Not gaping!
			...
			MainScreen.text("The two cocks pump you hard until you feel one of them shooting your " + vaginaDescript(0) + " full of hot demon cum. Eventually both the dicks release inside you and slip out one after the other, but each time another takes their place so that your pussy is never empty.", false);
		}
		//Orgasmzzzzz
	...
	MainScreen.text("Soon you feel hands and dicks grabbing and jabbing at your " + buttDescript() + ", edging inwards bit by bit and jostling for position as a slippery cock positions itself at the opening of your " + assholeDescript() + ". It pauses for a second and then dives straight into your waiting asshole.  ", false);
	...
	if(player.upperBody.chest.count() > 0) {
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) {
			MainScreen.text("  Demonic hands grab and squeeze at your " + allBreastsDescript() + " and lips fasten around your " + nippleDescript(0) + " and begin to lick and suck like there's no tomorrow.", false);
		}
	}
	//Newline for oral!
...
private oasisBadEnd():void {
	...
	MainScreen.text("After passing out from your latest orgy with the desert tribe, you wake up to find yourself still naked and laying on your back. ", false);
		//[If female/herm]
		if(player.lowerBody != LOWER_BODY.CENTAUR) MainScreen.text("Your feet are locked up in shackles, though with a chain long enough to leave you room to move or walk without problems. ", false);
		if(player.lowerBody == LOWER_BODY.CENTAUR) MainScreen.text("Your four legs are locked up in shackles, though with a chain long enough to leave you room to move or walk without problems. ", false);
