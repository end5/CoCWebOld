...
private function oasisTalkAccept():void {
	...
	if(player.lowerBody == LOWER_BODY.CENTAUR) outputText("your rear legs losing their balance and sending you crashing on your flank.  ", false);
...
internal function oasisSexing():void {
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		...
		if (player.lowerBody.cockSpot.list[0].cockLength > 25) outputText("feel your " + cockDescript(0) + " slide into a hell-girl's sloppy cunt. The demoness moans and pants in pleasure as your " + cockDescript(0) + " bottoms out in her gaping pussy and she begins to ride you like the motionless whore you momentarily are, her gigantic breasts threatening to concuss you as she slides the huge distance up and down your cock in her thirst to fill her cavelike cunt.  ", false);
			...
			if(player.lowerBody.cockSpot.list[0].cockLength > 17) outputText("with hardly any of you inside her ", false);
			else if(player.lowerBody.cockSpot.list[0].cockLength > 12) outputText("about halfway down your " + cockDescript(0) + " ", false);
			else if(player.lowerBody.cockSpot.list[0].cockLength > 6) outputText("with almost all of you inside of her ", false);
			else if(player.lowerBody.cockSpot.list[0].cockLength <= 6) outputText("as her pussy lips touch the base of your " + cockDescript(0) + " ", false);
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			...
			if(player.lowerBody.cockSpot.count() == 2) outputText(cockDescript(1) + " is", false);
	...
	if(player.lowerBody.vaginaSpot.count() > 0) outputText("You feel the cocks in your ass and pussy rubbing into each other as your lower body becomes a temple of friction. The feeling of so much demon stuffing you as full as possible is almost unbearable, and you are inches away from blacking out as you come again.  ", false);
...
private function oasisBadEnd():void {
	...
	outputText("After passing out from your latest orgy with the desert tribe, you wake up to find yourself still naked and laying on your back. ", false);
		//[If female/herm]
		if(player.lowerBody != LOWER_BODY.CENTAUR) outputText("Your feet are locked up in shackles, though with a chain long enough to leave you room to move or walk without problems. ", false);
		if(player.lowerBody == LOWER_BODY.CENTAUR) outputText("Your four legs are locked up in shackles, though with a chain long enough to leave you room to move or walk without problems. ", false);
