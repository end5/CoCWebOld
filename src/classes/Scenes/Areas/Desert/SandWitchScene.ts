...
private function sandwitchSex():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText(num2Text(player.lowerBody.cockSpot.count()), false);
	...
	if(player.lowerBody.balls >= 2) outputText("balls ache and your ", false);
...
private function sandwitchCentaurBoning():void {
	...
	if(player.stats.cor < 70) {
		...
		//[has breasts]
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) outputText("; your " + nippleDescript(0) + "s  become stiff", false);
		//[cock only]
		if(player.totalCocks() > 0 && !player.lowerBody.vaginaSpot.hasVagina()) outputText(" and your " + cockDescript(0) + " hardens.", false);
		//[cunt only]
		if(player.lowerBody.vaginaSpot.hasVagina() && player.totalCocks() == 0) outputText(" and your " + vaginaDescript(0) + " begins to drip with moisture.", false);
		//[cock and cunt]
		if(player.lowerBody.vaginaSpot.hasVagina() && player.totalCocks() > 0) outputText(" and your " + cockDescript(0) + " hardens as your " + vaginaDescript(0) + " begins to drip with moisture.", false);
		...
		//[has breasts]
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) outputText(", eyes fixed on your " + player.allBreastsDescript(), false);
		outputText(".\n\n", false);
		...
		//[has breasts]
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) outputText("fondle your " + player.allBreastsDescript() + " and ", false);
...
private function SWCentaurMore(argument:number):boolean {
	...
	if(argument == 2) {
		...
		if(player.lowerBody.cockSpot.count() > 0 || (player.gender == 3 && rand(2) == 0)) {
			...
			//[1 cock, non-tentacle]
			if(player.lowerBody.cockSpot.count() == 1 && player.tentacleCocks() == 0) outputText("With a single thrust, you push deep into one of her cunts, ", false);
			//[1 cock, tentacle]
			if(player.lowerBody.cockSpot.count() == 1 && player.tentacleCocks() == 1) outputText("Your " + cockDescript(0) + " caresses her anus teasingly, causing her to whimper in a mixture of arousal and denial.  With a single savage thrust, you push past her clenched muscles, ", false);
			//[two cocks] 
			if(player.lowerBody.cockSpot.count() == 2) outputText("After aligning your " + multiCockDescriptLight() + " to her twin cunts, you push yourself deeply into her, ", false);
			//[3+ cocks]
			if(player.lowerBody.cockSpot.count() >= 3) outputText("Lining up two of your cocks to her twin cunts and another to her anus, you thrust into her without pre-amble.  Her anal muscles try to keep you out, but they are no match for the strength of your legs.  You tear into her, ", false);
			...
			if(player.lowerBody.cockSpot.list[0].cockLength >= 12) {
				if(player.lowerBody.cockSpot.count() == 1) outputText("Her body surrenders more and more as your " + multiCockDescriptLight() + " presses against her cervix. ", false);
			...
			//[1 cock, tentacle]
			if(player.lowerBody.cockSpot.count() == 1 && player.tentacleCocks() == 1) outputText("anus", false);
			else {
				//[largest cock is not wide and/or long]
				if(player.lowerBody.cockSpot.list[0].cockLength >= 12 || player.lowerBody.cockSpot.list[0].cockThickness >= 3) {
					//[1 cock]
					if(player.lowerBody.cockSpot.count() == 1) outputText("cunt", false);
					//[2+ cocks]
					if(player.lowerBody.cockSpot.count() >= 2) outputText("cunts", false);
				}
				//[largest cock is wide and/or long]
				else {
					outputText("womb", false);
					if(player.lowerBody.cockSpot.count() >= 3) outputText(" and bowels", false);
				}
			}
		...
		//[% no cocks]
			...
			//[has cunt] 
			if(player.lowerBody.vaginaSpot.hasVagina()) outputText("and your " + vaginaDescript(0) + " dripping with desire", false);
...
private function ovipositSandWitches():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("hardened  " + multiCockDescriptLight());
	...
	if(player.totalCocks() > 1) outputText(num2Text(player.lowerBody.cockSpot.count() + 1));
	else outputText("two");
...
private function eggwitchForeplay():void {
	...
	if(player.lowerBody.cockSpot.count() > 1 || player.gender == 3) outputText("es");
	...
	if(player.stats.cor > 50) outputText("enough ");
...
private function laySomeEggsInThatWitchFinally():void {
	...
	if(player.lowerBody.cockSpot.count() > 1 || player.gender > 2) outputText("es");
	outputText(" is the only point of dissatisfaction, a point you make clear when you ");
	if(player.lowerBody.cockSpot.hasCock()) outputText("take hold of your " + multiCockDescriptLight() + " and bring it");
	...
	if(player.gender == 3) outputText(" and ");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your " + vaginaDescript(0));
...
public function sammitchBirthsDriders():void {
	...
	if(player.stats.cor < 50) outputText("Intending to help");
...
private function sandwitchBirthsYourMonstrosities():void {
	...
	if(player.stats.cor < 60) outputText(", with a quick kiss on the lips");
...
private function sandwitchSpanking():void {
	...
	if(player.stats.cor < 33) outputText("and though you feel kind of bad about using her this way, she likely would have done worse to you.");
	else if(player.stats.cor < 66) outputText("and though you aren't sure you ought to be using her this way, she likely would have done worse to you.");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("Worse still, [eachCock] is now unprotected and pressed against the rough stone by your body-weight.  It's less than pleasant.  ");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Worse still, your [vagina] is pressed up against the rough stone.  It's less than pleasant.  ");
	outputText("You bristle at your helplessness");
	if(player.perks.has("FireLord") || player.perks.has("Hellfire") || player.perks.has("Dragonfire")) outputText(", breathing out a gout of flame in rage, but you can't even direct the blast properly like this.  All it does is melt some of the sand below into glass");
	...
	if(player.stats.cor < 50) outputText("She sounds almost fanatical in her dedication to her order.");
