export default class SandWitchScene implements TimeAwareInterface {
	...
	public encounter(): void {
		...
		MainScreen.text("A strange woman seems to appear from the dunes themselves.  She identifies herself as a sand witch, and politely asks if she can cast a spell on you.");
		if (player.statusAffects.get("Exgartuan").value1 == 1 && player.cockArea(0) > 100 && player.statusAffects.get("Exgartuan").value2 == 0) {
			MainScreen.text("\n\nThe " + player.inventory.armor.displayName + " covering your lower half hits the ground, as if yanked down by magic.  Your " + CockDescriptor.describeCock(player, 0) + " pulsates darkly, growing rigid in seconds as the demon within you takes over.  It barks, \"<i>Fuck, how about I cast my spell on you baby?</i>\"\n\n");
			MainScreen.text("The sandwitch ");
			if (player.stats.cor < 50)
				MainScreen.text("and you both turn crimson", false);
			else MainScreen.text("turns crimson", false);
			MainScreen.text(" as you yank your " + player.inventory.armor.displayName + " back into place.  You're in charge here, not some possessed appendage!   Exgartuan yells something snide, but it's muffled too much to understand.  You look up in time to sidestep an attack from the Sand Witch.  It looks like you'll have to fight her!");
	...
	private allowSandWitchMagic(): void {
		...
		if (player.upperBody.head.hairColor == "sandy blonde") {
			MainScreen.text("She smiles wickedly and intones, \"<i>Tresed eht retaw llahs klim ruoy.</i>\"\n\n", false);
			if (player.upperBody.chest.count() == 0 || player.upperBody.chest.BreastRatingLargest[0].breastRating == 0) {
				...
				if (player.upperBody.chest.get(0).nipplesPerBreast < 1) player.upperBody.chest.get(0).nipplesPerBreast = 1;
				player.stats.sens += 2;
				player.stats.lust += 1;
			}
			...
			if (player.upperBody.chest.get(0).nipplesPerBreast < 1) {
				MainScreen.text("Two dark spots appear on your chest, rapidly forming into sensitive nipples.  ", false);
			...
			if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier > 0) {
				...
				if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier < 2)
					MainScreen.text("powerful jets of milk spray from your nipples, spraying thick streams over the desert sands.  You moan at the sensation and squeeze your tits, hosing down the tainted earth with an offering of your milk.  You blush as the milk ends, quite embarassed with your increased milk production.  ", false);
				if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier >= 2 && player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier <= 2.6)
				...
				if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier > 2.6 && player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier < 3)
			...
			if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier == 0) {
				MainScreen.text("A pleasurable release suddenly erupts from your nipples!  Streams of milk are spraying from your breasts, soaking into the sand immediately.  It stops all too soon, though the witch assures you that you can lactate quite often now.  ", false);
				player.boostLactation(1);
				player.stats.lib += .5;
				player.stats.sens += 1;
				player.stats.lust += 10;
			}
			MainScreen.text("The sand-witch smiles and thanks you for your offering.  You notice her dress is damp in four spots on the front.  ", false);
			if (Game.sand == 0)
				MainScreen.text("You wonder at what her robes conceal as she vanishes into the dunes.", false);
			if (Game.sand == 1) {
				if (player.stats.cor <= 33)
					MainScreen.text("You are glad to avoid servicing her again as she vanishes into the dunes.", false);
				else if (player.stats.cor <= 66)
					MainScreen.text("You wonder if you should've resisted and tried for some sex as she departs.", false);
				else MainScreen.text("You wish you had said no, so you could fuck with her and her magnificent quartet of breasts some more.", false);
			}
		}
		else {
			MainScreen.text("She smiles wickedly and intones, \"<i>nuf erutuf rof riah ydnas, nus tresed eht sa ydnas.</i>\"\n\nYou feel a tingling in your scalp, and realize your hair has become a sandy blonde!", false);
	...
	private refuseSandWitchMagic(): void {
		spriteSelect(50);
		MainScreen.clearText();
		MainScreen.text("With an inarticulate scream of rage, the Sand Witch attacks!");
		startCombat(new SandWitch());
	}
			...
			if (player.upperBody.head.hairColor.indexOf("sandy blonde") != -1) {
				...
				if (player.upperBody.chest.averageNipplesPerBreast() < 1) {
					...
					while (temp > 0) {
						...
						if (player.upperBody.chest.get(0).nipplesPerBreast < 1) player.upperBody.chest.get(0).nippleLength = .2;
						player.upperBody.chest.get(0).nipplesPerBreast = 1;
					}
					player.stats.sens += 2;
					player.stats.lust += 1;
				...
				if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier > 0) {
					MainScreen.text("A strong pressure builds in your chest, painful in its intensity.  You yank down your top as ", false);
					if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier < 2) MainScreen.text("powerful jets of milk spray from your nipples, spraying thick streams over the desert sands.  You moan at the sensation and squeeze your tits, hosing down the tainted earth with an offering of your milk.  You blush as the milk ends, quite embarassed with your increased milk production.  ", false);
					if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier >= 2 && player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier <= 2.6) MainScreen.text("eruptions of milk squirt from your nipples, hosing thick streams everywhere.  The feeling of the constant gush of fluids is very erotic, and you feel yourself getting more and more turned on.  You start squeezing your breasts as the flow diminishes, anxious to continue the pleasure, but eventually all good things come to an end.  ", false);
					if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier > 2.6 && player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier < 3) MainScreen.text("thick hoses of milk erupt from your aching nipples, forming puddles in the sand.  You smile at how well you're feeding the desert, your milk coating the sand faster than it can be absorbed.  The constant lactation is pleasurable... in a highly erotic way, and you find yourself moaning and pulling on your nipples, totally outside of your control.  In time you realize the milk has stopped, and even had time to soak into the sands.  You wonder at your strange thoughts and pull your hands from your sensitive nipples.  ", false);

					if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier >= 3) MainScreen.text("you drop to your knees and grab your nipples.  With a very sexual moan you begin milking yourself, hosing out huge quantities of milk.  You pant and grunt, offering as much of your milk as you can.  It cascades down the dune in a small stream, and you can't help but blush with pride... and lust.  The erotic pleasures build as you do your best to feed the desert all of your milk.  You ride the edge of orgasm for an eternity, milk everywhere.  When you come to, you realize you're kneeling there, tugging your dry nipples.  Embarrassed, you stop, but your arousal remains.  ", false);
					if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier < 3) {
				...
				if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier == 0) {
					MainScreen.text("A pleasurable release suddenly erupts from your nipples!  Twin streams of milk are spraying from your breasts, soaking into the sand immediately.  It stops all too soon, though the witch assures you that you can lactate quite often now.  ", false);
					player.boostLactation(1);
					player.stats.lib += .5;
					player.stats.sens += 1;
					player.stats.lust += 10;
	...
	private sandWitchBadEndPartThree(): void {
		MainScreen.clearText();
		MainScreen.text("Something breaks within your soul, accompanied by a strangled sob. Tears of humiliation trickle down your face as you part your quivering lips. \"<i>Please milk me, Mistress. Your slave needs to be milked.</i>\"\n\n");
		if (player.perks.has("MarblesMilk")) {
			...
			MainScreen.text("Trying to figure out why you feel so incredibly weak, you think back on what might have caused this.  When was the last time you had some of Marble's milk?  Feeling your body start to wither away, the realization hits you.  You chuckle softly at your captor as the world starts to go dark; looks like you'll be the one to have the last laugh...\n\n");
			MainScreen.text("The last thing you hear is the Sand Witch screaming \"<i>What the hell is going on?  I ALWAYS get what I want!</i>\"");
		}
		else {
	...
	private sandwitchRaped(): void {
		...
		MainScreen.text("Even as you rip her sand-colored robes from her body, her eyes hold power and defiance. Her chest holds four large breasts, each leaking milk upon the sands.  ", false);
		//Malllllleeeee (or has penis)
		if (player.gender == 1 || player.gender == 3) {
			//Not so corrupt
			if (player.stats.cor < 50) {
				MainScreen.text("You smile at the sand witch, trying to seem enticing. A tiny smile plays across her face even though she supports a worried frown. She reaches up to you, and you allow her to gently pull you down to her. You marvel at her multiple breasts, each perfectly rounded. Each has a nipple approximately two inches long, enough to suck if you wanted to.  ", false);
				MainScreen.text("Your attention wanders down to the damp sand underneath her legs, and her musky scent hits your nose. Which do you go after?\n\n", false);
			...
			//Really corrupt
				...
				if (player.lowerBody.cockSpot.count() == 1) {
					//DOGGIES
					if (player.lowerBody.cockSpot.countType(CockType.DOG) > 0 && player.lowerBody.cockSpot.get(0).knotMultiplier > 1.4) knotSandwitch();
					else {
						MainScreen.text("You place your " + player.foot() + " in the wet sand, pulling her hair and making her look you in the eyes. The sand witch looks shocked at your display of power, only turning you on more. You undo your " + player.inventory.armor.displayName + " and grope her nethers, only to find a surprise. She has " + num2Text(player.lowerBody.cockSpot.count() + 1) + " pussies and each seems to be made to fit you exactly. Pushing her to the ground, you bring the seeping head of your " + CockDescriptor.describeCock(player, 0) + " to one of her nethers, slowly easing into her depths.  ", false);
						MainScreen.text("Thrusting deep, you hit bottom, and marvel at the sensations as you feel the head of your " + CockDescriptor.describeCock(player, 0) + " rub against the end of her slick cunts, only making you lust after her more. As though responding to your will, your " + CockDescriptor.describeCock(player, 0) + " grows just a tiny bit more in length and thickness.  ", false);
						MainScreen.text("With a satisfied growl of lust from yourself, and a screech of pain from the sand witch, you break past her cervix and into her womb. Pulling out, you rub yourself against her wet outer lips, bringing yourself closer and closer to your peak as the sand witch struggles, her eyes turning wide with fright as she starts to understand you hold more power than her.  ", false);
						MainScreen.text("With a bestial cry of lust, you shove your shaft deep within one of her vaginas, past her cervix and into her womb where you blast your potent, corrupted, demon-tainted seed.  ", false);
						MainScreen.text("\n\nThe sand witch screams in savage denial as you plant your seed deep within her body, your corruption no doubt making it extra potent. You gaze into her eyes, reveling in her expression of horror. Without uttering a word, you continue to fuck her, making sure your seed is well-entrenched within her womb, feeling it splatter and bubble around your " + CockDescriptor.describeCock(player, 0) + ".  You wonder if she will remember the pleasure her body felt today, and if she will be more likely to drop to her knees before you than to fight... ", false);
						MainScreen.text("Finally finished, you pull your " + CockDescriptor.describeCock(player, 0) + " from one of her well-used slits, your " + CockDescriptor.describeCock(player, 0) + " drooling cum even now. With a grin, you walk away, happy with your little encounter.  ", false);
						player.orgasm();
					}
				}
				//Multiprick
				if (player.lowerBody.cockSpot.count() > 1) {
					...
					MainScreen.text("Using the corruption in your blood, you place your " + player.foot() + " upon the budding shoots, and wipe them from existence. The sand witch looks shocked at your display of power, only turning you on more. You undo your loin cloth and grope her nethers, only to find a surprise. She has " + num2Text(player.lowerBody.cockSpot.count() + 1) + " loose wet cunts and each seems to be made to fit you exactly. Pushing her to the ground, you bring the seeping heads of your " + CockDescriptor.describeMultiCockShort(player) + " to her nethers, slowly easing into her depths.  ", false);
					MainScreen.text("Thrusting deep, you hit bottom, and marvel at the sensations as you feel the heads of your " + CockDescriptor.describeMultiCockShort(player) + " rub against the end of her wet pussies, only making your desire to penetrate increase. As though responding to your will, your " + CockDescriptor.describeMultiCockShort(player) + " grow just a tiny bit more in length and thickness.  ", false);
					MainScreen.text("With a satisfied growl of lust from yourself, and a screech of pain from the sand witch, you break past her cervixes and into her wombs. Pulling the heads of your " + CockDescriptor.describeMultiCockShort(player) + " from her wombs, you rub and rut at the outside of her cunts, bringing yourself closer and closer to your peak as the sand witch struggles, her eyes turning wide with fright as she starts to understand you hold more power than her.  ", false);
					MainScreen.text("With a bestial cry of lust, you shove your shafts deep within her vaginas, past her cervices and into her womb where you blast your potent, corrupted, demon-tainted seed.  ", false);
					//A few dicks...
					MainScreen.text("\n\nThe sand witch screams in savage denial as you plant your seed deep within her body, your corruption no doubt making it extra potent. You gaze into her eyes, reveling in her expression of horror. Without uttering a word, you continue to fuck her, making sure your seed is well-entrenched within her wombs, feeling it splatter and bubble around your " + CockDescriptor.describeMultiCockShort(player) + ".  You wonder if she will remember the pleasure her body felt today, and if she will be more likely to drop to her knees before you than to fight... ", false);
					MainScreen.text("Finally finished, you pull your " + CockDescriptor.describeMultiCockShort(player) + " from her well used slits, your " + CockDescriptor.describeMultiCockShort(player) + " drooling cum even now. With a grin, you walk away, happy with your little encounter.  ", false);
				}
				cleanupAfterCombat();
			}
		}
		//Femfem
			...
			MainScreen.text("You smile at the sand witch, trying to seem enticing. A tiny smile plays across her face even though she supports a worried frown. She reaches up to you, and you allow her to gently pull you down to her. You marvel at her multiple breasts, each perfectly rounded. Each has a nipple approximately two inches long, enough to suck if you wanted to.  ", false);
			MainScreen.text("Your attention wanders down to the damp sand underneath her legs, and her musky scent hits your nose.\n\n", false);
			sandwitchBewbs();
		}
	}
	...
	private sandwitchBewbs(): void {
		spriteSelect(50);
		MainScreen.text("", true);
		MainScreen.text("You move yourself atop the defeated sand witch, and bring your lips to her breasts.  The moment the first drop of milk touches your tongue, you drink, hungrily, needfully, demandingly. You grope and massage her breasts as she leans back and cries out, her eyes wide in surprise. You guzzle and drink her sweet cream, the small seedlings underneath you appearing to rustle and move as you drink the alabaster flow once intended for the desert sands.  ", false);
		MainScreen.text("Warmth fills your gluttonous belly, and the sand witch places a hand to your stomach, rubbing it lightly as you drink from her breasts, moving on when the one you're drinking from is drained.  ", false);
		MainScreen.text("You burp, and flush scarlet, but the sand witch lays a hand against your face, leans close, and kisses your lips sweetly. Her tongue snakes out and dives into your mouth. She pulls your face against hers, grinding her breasts against your chest. You can feel fresh milk dribbling against your " + player.skinDesc + " and the scents are driving you wild with lust.  ", false);
		MainScreen.text("She breaks the kiss and smiles at you, whispering. Your eyes slowly close as you grow drowsy; you realize too late that she's casting a spell. You fall asleep, the sand witch's milk in your stomach churning, gurgling.  ", false);
		MainScreen.text("You wake up in the desert, immediately grabbing at your body to see if anything's changed...", false);
		player.slimeFeed();
	...
	private sandwitchSex(): void {
		spriteSelect(50);
		MainScreen.text("", true);
		MainScreen.text("With a charming, disarming smile, you reach between her legs and touch against the slick lips of her pussies. She has ", false);
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(num2Text(player.lowerBody.cockSpot.count()), false);
		else MainScreen.text("two", false);
		MainScreen.text(" of them!  The sand witch appears frightened, but as you gently tease her lips with your fingers, she slowly starts to breathe faster, starting to moan as her legs spread themselves against her better judgment. Grinning in satisfaction, you bring your rigid " + CockDescriptor.describeCock(player, 0) + " against her lips, and push slowly, gently.  ", false);
		MainScreen.text("The sand witch cries out, her hands grabbing your arms, pressing hard against your " + player.skinDesc + " as you slowly and teasingly ease your " + CockDescriptor.describeCock(player, 0) + " into her tight depths. You almost feel like a virgin, your " + CockDescriptor.describeCock(player, 0) + " throbbing, the need to cum almost beyond your ability to control.  ", false);
		MainScreen.text("You rest against her chest, her large breasts squirting milk against your own chest as your weight appears to excite her enough to make her lactate, if what you're doing already wasn't enough.  ", false);
		MainScreen.text("You kiss the sand witch, getting accustomed to her unbelievable tightness. You hump backwards and forwards, grinning like a fool with his first girlfriend. Not even a succubus would be able to rival her lush depths.  ", false);
		MainScreen.text("You finally get serious and ease yourself into fucking the sand witch, slowly gaining speed and depth as your groins slap together with such ferocity that a normal person would be bruised upon their next waking. You pound the sand witch, making her scream in ecstasy, her already tight depths turning vice-like as you grunt your lust, cumming and filling her depths. Your ", false);
		if (player.lowerBody.balls >= 2) MainScreen.text("balls ache and your ", false);
		MainScreen.text(CockDescriptor.describeCock(player, 0) + " throbs with need, even as you empty your lust into the defeated sand witch.", false);
		MainScreen.text("\n\nWith a satisfied smirk, you lay atop your prize, your " + CockDescriptor.describeCock(player, 0) + " still twitching within her depths. One of the sand witch's hands rubs at your head, teasing your " + player.skinDesc + ". Like a fresh youth, you fall asleep, " + CockDescriptor.describeCock(player, 0) + " buried deep in the sand witch.  ", false);
		MainScreen.text("With a start you wake up, only to find the sand witch still passed out beside you.  Almost thinking it was a dream, you touch your groin tenderly and rub softly. There's a faint trace of soreness, and you remember the ferocity with which you mashed your pelvises together.  You get up and get ready.  ", false);
		player.orgasm();
		cleanupAfterCombat();
	}
	/*
	...
	private sandwitchCentaurBoning(): void {
		spriteSelect(50);
		player.orgasm();
		MainScreen.text("The Sand Witch is panting before you, her clothes in disarray ", false);
		//[win via HP]
		if (monster.HP < 1) MainScreen.text("and torn in many places. ", false);
		//[win via lust]
		else MainScreen.text("and her fingers squeezing her nipples as lust overwhelms her. ", false);
		//[has breasts]
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) {
			MainScreen.text("As you approach, her eyes drift up to your " + player.BreastDescriptor.describeAllBreasts(player) + " and she ", false);
			//[win via HP]
			if (monster.HP < 1) MainScreen.text("is unable to stop herself from slowly licking her lips. ", false);
			//[win via lust]
			else MainScreen.text("sensually licks her parted lips. ", false);
		}
		//[lactating]
		if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier > 1) {
			MainScreen.text("Her eyes widen when she sees the beads of milk already dripping from your " + BreastDescriptor.describeNipple(0) + "s as her hand drops between her legs and she begins to slowly masturbate.", false);
		}
		MainScreen.text("\n\n", false);
		//[Corruption < 70]
		if (player.stats.cor < 70) {
			MainScreen.text("You smile as you approach", false);
			//[has breasts]
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) MainScreen.text("; your " + BreastDescriptor.describeNipple(0) + "s  become stiff", false);
			//[cock only]
			if (player.lowerBody.cockSpot.count() > 0 && !player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" and your " + CockDescriptor.describeCock(player, 0) + " hardens.", false);
			//[cunt only]
			if (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.cockSpot.count() == 0) MainScreen.text(" and your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " begins to drip with moisture.", false);
			//[cock and cunt]
			if (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.cockSpot.count() > 0) MainScreen.text(" and your " + CockDescriptor.describeCock(player, 0) + " hardens as your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " begins to drip with moisture.", false);
			if (player.gender == 0) MainScreen.text(" her.", false);
			MainScreen.text(" She seems hesitant at first, but soon approaches and begins to run her hands along your stomach and your " + LowerBodyDescriptor.describeHips(player), false);
			//[has breasts]
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) MainScreen.text(", eyes fixed on your " + player.BreastDescriptor.describeAllBreasts(player), false);
			MainScreen.text(".\n\n", false);
			//[GoTo: SW_3 and return] 
			if (SWCentaurMore(3)) return;
			MainScreen.text("\n\n", false);
			MainScreen.text("Delicate hands reach around to ", false);
			//[has breasts]
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) MainScreen.text("fondle your " + player.BreastDescriptor.describeAllBreasts(player) + " and ", false);
			MainScreen.text("tease your " + BreastDescriptor.describeNipple(0) + "s, and you can feel her grinding herself against your strong shoulders. ", false);
			//[orb penetrated player during combat] 
			MainScreen.text("With a start you realize that the witch's orb is still inside you as it suddenly begins to pulse within your " + LowerBodyDescriptor.assholeOrPussy(player) + ", causing you to cry out and hasten your pace across the dunes. ", false);
			MainScreen.text("You lose track of time and location as you ride, feeling her orgasm over and over against you, her milk pouring down your back and spilling onto the sands. ", false);
			//[has breasts]
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) {
				//[lactating]
				if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier >= 1) {
					MainScreen.text("Your own " + BreastDescriptor.describeNipple(0) + "s  are leaking as well, ", false);
					//[light-med lactation]
					if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier < 3) MainScreen.text("small streams running out ", false);
					//[heavy lactation] 
					else MainScreen.text("torrents gushing out ", false);
					MainScreen.text("under her expert hands. ", false);
				}
			}
			MainScreen.text("You can somehow feel her arousal pouring through her milk and into your skin, filling you with her own sensations and making your entire body vibrate like a sexual tuning fork.  As the sensations grow to be overwhelming, you feel yourself stumbling and slowing until you finally come to a stop. Panting, you listen to the sand witch's voice whispering sweet nothings into your ear... but the nothings seem to be having an effect.\n\n", false);
			//[GoTo: SW-1]
			if (SWCentaurMore(1)) return;
		}
		//[Corruption > 70]
		else {
			MainScreen.text("When she sees the look on your face, the sand witch comes to her senses. Quickly gathering herself up, she tries to run in fear.  You grin and out-pace her easily, galloping around her and steering her in circles, occasionally knocking her over for fun.  When she starts to slow down, you grab her sand-colored robes and ", false);
			//[STR > 50]
			if (player.stats.str >= 50) MainScreen.text("easily tear them off of her. ", false);
			//[STR < 50]
			else MainScreen.text("drag her a bit until they come off. ", false);
	...
	private SWCentaurMore(argument: number): boolean {
		...
		//[SW_1]
		if (argument == 1) {
			MainScreen.text("You realize too late that she has been casting a spell for the past few minutes. Before you can react, everything grows dark.", false);
		...
		if (argument == 2) {
			...
			if (player.lowerBody.cockSpot.count() > 0 || (player.gender == 3 && rand(2) == 0)) {
				MainScreen.text("Finished with your games, pre-cum starts pooling under your throbbing " + CockDescriptor.describeCock(player, 0) + ". ", false);
				//[largest cock is wide]
				if (player.lowerBody.cockSpot.get(0).cockThickness >= 3) {
					if (player.hasKnot(0)) MainScreen.text("Seeing its sheer size combined with your swollen knot causes her to gasp in terror and try to run again.  ", false);
					else MainScreen.text("Seeing its sheer size causes her to gasp in fear and turn to run again.  ", false);
				}
				MainScreen.text("You grab the witch and knock her down into the sand, quickly lowering your " + CockDescriptor.describeCock(player, 0) + " to be against her buttocks. ", false);
				//[1 cock, non-tentacle]
				if (player.lowerBody.cockSpot.count() == 1 && player.lowerBody.cockSpot.countType(CockType.TENTACLE) == 0) MainScreen.text("With a single thrust, you push deep into one of her cunts, ", false);
				//[1 cock, tentacle]
				if (player.lowerBody.cockSpot.count() == 1 && player.lowerBody.cockSpot.countType(CockType.TENTACLE) == 1) MainScreen.text("Your " + CockDescriptor.describeCock(player, 0) + " caresses her anus teasingly, causing her to whimper in a mixture of arousal and denial.  With a single savage thrust, you push past her clenched muscles, ", false);
				//[two cocks] 
				if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("After aligning your " + CockDescriptor.describeMultiCockShort(player) + " to her twin cunts, you push yourself deeply into her, ", false);
				//[3+ cocks]
				if (player.lowerBody.cockSpot.count() >= 3) MainScreen.text("Lining up two of your cocks to her twin cunts and another to her anus, you thrust into her without pre-amble.  Her anal muscles try to keep you out, but they are no match for the strength of your legs.  You tear into her, ", false);
				MainScreen.text("eliciting a scream as your hind quarters push her forcefully over the sand. ", false);
				//[largest cock is wide] 
				if (player.lowerBody.cockSpot.get(0).cockThickness >= 3) {
					MainScreen.text("It is hard to believe just how tight she is, though if her cries serve as any indication, she will not be after you are through with her.  Turned on even more, you thrust in with increasing vigor and try to widen her as much as possible. ", false);
				}
				//[2+ cocks]
				if (player.lowerBody.cockSpot.count() >= 2) {
					MainScreen.text("The feeling of your " + CockDescriptor.describeMultiCockShort(player) + " touching each other through the narrow layers of her body causes you to grunt in pleasure, experiencing the sensation anew during every thrust. ", false);
				}
				//[largest cock is long, non-tentacle]
				if (player.lowerBody.cockSpot.get(0).cockLength >= 12) {
					if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("Her body surrenders more and more as your " + CockDescriptor.describeMultiCockShort(player) + " presses against her cervix. ", false);
					else MainScreen.text("Her body surrenders more and more as your " + CockDescriptor.describeMultiCockShort(player) + "  press against her cervixes. ", false);
					MainScreen.text("Her mouth opens in a soundless and agonizing cry when you finally push past. ", false);
				}
				//[1 cock, tentacle] 
				if (player.lowerBody.cockSpot.countType(CockType.TENTACLE) == 1 && player.lowerBody.cockSpot.count() == 1) {
					MainScreen.text("Her body surrenders more and more as your " + CockDescriptor.describeCock(player, 0) + " pushes deep into her bowels, snaking its way further into her body as it fucks her insides. ", false);
				}
				MainScreen.text("It feels like you have been pushing yourself into her for hours", false);
				//[largest cock is wide and/or long]
				if (player.lowerBody.cockSpot.get(0).cockLength >= 12 || player.lowerBody.cockSpot.get(0).cockThickness >= 3) {
					MainScreen.text("; her cries have long since died down to whimpers. ", false);
				}
				//[largest cock is not wide and/or long]
				else MainScreen.text(", and her cries have long since turned into groans of pleasure. ", false);
				MainScreen.text("You cannot hold off your orgasm any longer. Your " + CockDescriptor.describeMultiCockShort(player) + " explode", false);
				if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("s", false);
				MainScreen.text(", ", false);
				//[large cum production] 
				if (player.cumQ() >= 250) MainScreen.text("gushing massive amounts of your cum ", false);
				//[regular cum production] 
				else MainScreen.text("pushing your sperm ", false);
				MainScreen.text("deep into her ", false);
				//[1 cock, tentacle]
				if (player.lowerBody.cockSpot.count() == 1 && player.lowerBody.cockSpot.countType(CockType.TENTACLE) == 1) MainScreen.text("anus", false);
				else {
					//[largest cock is not wide and/or long]
					if (player.lowerBody.cockSpot.get(0).cockLength >= 12 || player.lowerBody.cockSpot.get(0).cockThickness >= 3) {
						//[1 cock]
						if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("cunt", false);
						//[2+ cocks]
						if (player.lowerBody.cockSpot.count() >= 2) MainScreen.text("cunts", false);
					}
					//[largest cock is wide and/or long]
					else {
						MainScreen.text("womb", false);
						if (player.lowerBody.cockSpot.count() >= 3) MainScreen.text(" and bowels", false);
					}
				}
				MainScreen.text(".  With a satisfied groan, you pull out and let your " + CockDescriptor.describeMultiCockShort(player) + " dribble the last remnants of your cum over the ravished witch. Satisfied, you ride off into the desert.", false);
			}
			//[% no cocks]
			else {
				MainScreen.text("Finished with your games ", false);
				//[has cunt] 
				if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("and your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " dripping with desire", false);
				MainScreen.text(", you push the witch unceremoniously to the ground and deliver a slap to her breasts.  She cries out in pain as milk splashes out onto the sand.  A cruel smile is brought to your face and you start slapping them even harder, alternatively smacking her pair of cunts for good measure.  Surprisingly, her cries of pain begin to turn into moans of pleasure with every slap.  She might even be pushing into the blows slightly, though it is difficult to tell.  After a sizable pool of milk and her juices has drained into the sands beneath her, you reach down and start to drive your fingers hard into one of the sand witch's cunts.  The first few thrusts are enough to violently bring her to orgasm.  You trot forward slowly until you are over her and the witch suddenly hops up. Driving her face into your ", false);
				//[has cunt]
				if (player.lowerBody.vaginaSpot.hasVagina()) {
					MainScreen.text(VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ", she fingers herself even harder than you had been a moment ago.  Her lips lock unto your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " and she suckles on it, drawing a pained gasp from you and causing your pussy juices to gush over her face. You feel ", false);
					//[cunt size is small]
					if (player.vaginalCapacity() < 10) MainScreen.text("one of her fingers ", false);
					//[cunt size is normal]
					else if (player.vaginalCapacity() < 20) MainScreen.text("several of her fingers ", false);
					//[cunt size is large] 
					else MainScreen.text("a fist ", false);
					MainScreen.text("pushing deep into your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ", and you're surprised as ", false);
					//[anus size is small]
					if (player.analCapacity() < 10) MainScreen.text("her tongue ", false);
					//[anus size is normal]
					else if (player.analCapacity() < 20) MainScreen.text("two of her fingers ", false);
					//[anus size is large]
					else MainScreen.text("a fist ", false);
					MainScreen.text("penetrates your " + ButtDescriptor.describeButthole(player), false);
					MainScreen.text(".", false);
				}
				//[does not has cunt]
				else {
					MainScreen.text(ButtDescriptor.describeButthole(player) + ", she fingers herself even harder than you had been a moment ago.  Her tongue drives into your " + ButtDescriptor.describeButthole(player) + ", licking enthusiastically as she squeezes her nipples. ", false);
					//[anus size is small]
					if (player.analCapacity() < 10) MainScreen.text("The sensations are intense, and you could almost swear that her tongue is tracing a pattern as her mouth mutters strange sounds. ", false);
					//[anus size is normal] 
					else if (player.analCapacity() < 20) MainScreen.text("Unsatisfied with just her buffet, you soon feel her fingers thrust hard into you and hear her muttering what sounds like obscene words of depraved lust. ", false);
					//[anus size is large]
					else MainScreen.text("Unsatisfied with just her buffet, the witch grabs one of her breasts and squirts milk into your massive hole.  You realize she was lubricating it even as you feel her entire hand driving into you.  The sensations are too intense, and your screams of pleasure drown out her lustful muttering. ", false);
					MainScreen.text("Her mumbling finishes even as an orgasm overwhelms you.", false);
					MainScreen.text("\n\n", false);
					//[SW_1]
					SWCentaurMore(1);
		...
		//[SW_3]
		if (argument == 3) {
			MainScreen.text("As she begins to seem less skittish, you reach your hands into her robe and push it off of her shoulders, revealing a beautiful body underneath.  A pair of pussies is betraying the sand witch's arousal - her thighs are already dripping wet.  You begin to tease her four breasts as you lower your mouth to her lips for a passionate kiss.  Breaking off the kiss and grinning, you ", false);
			//[STR > 50]
			if (player.stats.str > 50) MainScreen.text("pick her up and toss her onto your back. ", false);
			//[STR < 50]
			else MainScreen.text("offer her your hand and help her onto your back. ", false);
			MainScreen.text("Your strong hooves carry you across the clear desert at a light trot.  Without her robes to protect her, you can feel the witch's bare pussies grinding against your back as she presses her quad of breasts against your shoulders.  Her moans intensify more and more as you pick up the pace.", false);
		}
		return false;
	}
	...
	private knotSandwitch(): void {
		spriteSelect(50);
		MainScreen.text("", true);
		MainScreen.text("You press the tip of your " + CockDescriptor.describeCock(player, 0) + " at the entrance of her fragent flower.  It slides in, thanks to the pre already dribbling from your cock, your eyes rolling back in your head from the touch of her slippery folds.  You can't help but suddenly stab the length of your " + CockDescriptor.describeCock(player, 0) + " to the hilt within her, moaning as the tip pushes against her cervix.\n\n", false);
		MainScreen.text("As your " + CockDescriptor.describeCock(player, 0) + " stirs within her, she begins to push back, her velvet walls gripping you tightly as you get a extra bit inside.  She pulls free and pounds in again, causing her to grunt from the force of your knot stretching her.  Your balls swing back and forth, slapping and bouncing against her ass in a delicious way.  You can feel the beginnings of the orgasm building within your loins, pulsing up through the tip of your " + CockDescriptor.describeCock(player, 0) + ". As the pleasure increases, your body readies itself for the spill, and your knot begins to swell.  ", false);
		MainScreen.text("You begin to buck into her at a frenzied pace, your body's need to release your motivation.  At first, your knot swelling is an extra sensation, a tightness that squeezes deliciously as more and more blood is forced inside by sheer stimulation.\n\n", false);
		MainScreen.text("Then it begins to get in the way, preventing your length from going all the way in, causing the pointed tip of your " + CockDescriptor.describeCock(player, 0) + " to lust for the unattainable depths of her cunt.  You press in, and get the sensation of the knot being squeezed in, crushing you with warm wet pleasure within her spasming cunt, only to release as you pull free.  As it continues to swell, you can no longer get your dick all the way inside her, your sperm-swollen knot bloated beyond her capacity.  Your desire is almost at its peak; within your grasp, but without that vice-like tightness around your knot it will be hard to cum.  ", false);
		MainScreen.text("You grunt and push hard, desperate to force the bulbous cock-flesh inside, and you're rewarded as it pops in with a wet squelch. The tightening pressure around the base of your " + CockDescriptor.describeCock(player, 0) + " pushes you over the edge, and the cum begins to shoot in her tight hole, with your " + CockDescriptor.describeCock(player, 0) + " trapped inside!\n\n", false);
		//ORGAZMO
		if (player.cumQ() < 25) MainScreen.text("The cum slicks your dick as you continue trying to hump, easing the over-sensation of your dick while you're glued together by the bulbous knot locked in her snatch.", false);
		if (player.cumQ() >= 25 && player.cumQ() < 100) MainScreen.text("Your cum expands her pussy, stretching her out and leaving your " + CockDescriptor.describeCock(player, 0) + " swimming in your spunk, trapped within her tight-stretched walls by your massive knot.", false);
		if (player.cumQ() >= 100 && player.cumQ() < 500) MainScreen.text("Your cum begins to pump into her corked body, filling her hole to the brim.  Surely the pressure is piling the jizz deep into her womb, and after what feels like forever, the overfull spunk shoves back on your knot, a little dribbling out around the edges of her taut feminine folds as her pussy reaches its limit.", false);
		if (player.cumQ() >= 500) MainScreen.text("Your cum spurts into her in rapid jets, coating the inside of her walls briefly white as more continues to pour into her. Her cunt begins to fill, and as she strugles to relieve the pressure that continues to pump into her, she only succeeds in spurring your orgasm to a new height.  You continue to blast within her, your knot holding her fast as she squirms, each shot of cum from your prick managing to squirt a tiny bit of jizz from your locked genitals.", false);
		player.orgasm();
	}
	...
	internal function beatSandwitch(): void {
		spriteSelect(50);
		if (monster.lust > 99) MainScreen.text("You smile in satisfaction as the " + monster.short + " drops down on all fours and begins masturbating feverishly.  Sadly you realize your own needs have not been met.  Of course you could always fuck the horny witch...\n\nDo you rape her?", true);
		else MainScreen.text("You smile in satisfaction as the " + monster.short + " drops down on all fours and struggles to rise.  Sadly you realize your own needs have not been met.  Of course, you could always fuck the witch...", true);
		MainScreen.text("  Of course, just taunting, teasing, and humiliating her for her arrogance would be equally amusing, <b>but it would give her plenty of time to turn the tables...</b>");
	...
	private missingoSex(): void {
		spriteSelect(50);
		MainScreen.text("", true);
		MainScreen.text("You stride proudly towards your latest conquest, the prone witch starting at you with a look of pure and delightful helplessness. She backs away on all fours, but a simple \"Stop.\" uttered from your lips is enough to freeze her in place. You look do/n at her, your \" + multiCockDescript(0) + \" throbbing within your clothes. What clothes? Oh, those clothes th$t you happen to be tearing from your body, much to her dismay.\n\n", false);

		MainScreen.text("Clearly, this bitch needs the dick. And you're just the ^&a*pion to give it to her. You lick your lips and drop to your @nees, taking hold of her le-s and forcibly spreading them apart. Looking down, you see that her body is quite ready for you, her exposed fuckhole pouted and moist. You cannot help but laugh a- the situation, the motions making your \" + multiCockDescript(0) + \" bounce in the open air as you co-vulse with laugh-er. Her res$$tance only serves to arouse you, making you savor that choice ass before you claim it for your own.\n\n", false);

		MainScreen.text("You want to embarrass her further, complete her humiliation upon the sands she once thought to be her domain. You tear her robes into little shreds, exposing her four generously sized breasts. Your eyes follow her dri-^ling nipples, the pupils bouncing as they do. \"Wh-what are you going to do? Rape me?!\" cries the witch, her tone equal parts terrified and wishful. Her thighs attempt to close, though you're sure it's more t$ grind them together than any atte-pt at preserving her modesty. Leaning down, y-u open up to let your tongue lap at her swollen clit. She arches her back and moans, thrusting her hips towa-ds you as bes- she can.\n\n", false);

		MainScreen.text("0)w is t$e time. But no, she won't get the dick. She hasn't EAR*ED the dick. She isn't even wor$h your \" + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + \" yet. No n- no, you need -o warm that ho_ cunt up. You whistle to g-t her attention, and as you do, you present your -i%-s ^a_=e* up to*--h-r. Her eyes wi_en, the wo$an's jaw going slack. \"N-no.. not t-e whole thing.\" you nod _n respo-se to her plea. \"The WHO-E THING?!\"\n\n", false);

		MainScreen.text("Now she begins to panic. She shake- her head rapidly as you draw bac- your %s99=, and thrust them towards her >pen sl1t...", false);
	...
	private missingoSex3(): void {
		spriteSelect(50);
		MainScreen.text("", true);
		MainScreen.text("_at_r@_ly, you %i-+. Ho@-@@ly, her h_le c_n't take @_ch _ po_ndi_g. Yo_ can t_ll t-is b_ loo_ing _t her pr_ne,',, v@^@erable b@dy, #ipe _or y-ur a_use. Y_ur *i- isn't ev|n en_ug_ to do this! Y_ur-i*))s fall sho_t, la@d@ng i( the sa_d, sp_as_i_g he- bo^y wit@ a @ave @f the -tuff. She gasps in su!2!i!e, ba_el_ a?le to k_ep it fr_m g_tti_g into h_r m_uth. @er @it@ har_ly fa_e as well, bei@g c@_t_d ge_ero_sl_, ^er milk mi$ing w--h it. T_e re_ult_ng co@cocti@n sti-ks to her c_est, l@a_in@ it q_ite t_e m_s_.\n\n", false);

		MainScreen.text("\"@ @a@'t b@lie^e y_u j@st _rie_ th_t.\" s@e sa_s @?th a )_=f. \"Yo_'re @oi(g to g_t a_d in all s))ts o& u_c_mfo_table pla_es ^& yo_ ke-_p t@at u@.\"\n\n", false);

		MainScreen.text("\"@_i@_...\" s_e says, ##o@%7g @t yo_ s_@_y. \"W_@ d_n't @o_ (ull b=c( an_ @r_ _h_@ a_@in....\"\n\n", false);
	...
	private missingoSex6(): void {
		...
		statScreenRefresh();
		MainScreen.text("", true);
		MainScreen.text("You successfully restore your game. Everything's intact. At least, you thought it was. You must have messed up the password somewhere along the line, but oddly enough, the only difference is the clock being set an hour ahead and a few extra gems and EXP in your pocket.\n\n", false);

		MainScreen.text("It is like fucking christmas up in here.", false);
	...
	private ovipositSandWitches(): void {
		MainScreen.clearText();
		MainScreen.text("As you glance down at the ");
		if (monster.HP < 1) MainScreen.text("bruised and battered");
		else MainScreen.text("lusty");
		MainScreen.text(" sand witch, you feel the familiar tug of arousal in your loins.  Yet your current... situation offers quite the opportunity to relieve some tension through the use of your 'other' sexual appendage.  Licking your lips in anticipation, you lower yourself down to the obstinate spell caster, who immediately reacts with ");
		if (monster.HP < 1) MainScreen.text("fright as she clamors to get away.");
		else MainScreen.text("an uneasy curiosity to see what exactly you have planned.");

		//PC won through HP victory: 
		if (monster.HP < 1) MainScreen.text("\n\nRolling your eyes, you offer a sympathetic hand to the defeated witch, showing her that you don't mean to hurt her any further, that you have something more... pleasurable in mind.  ")
		MainScreen.text("The sand witch slows to a dead stop as she assesses your intentions, which are made all the more clear as you disrobe and toss your [armor] aside, exposing your ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("hardened  " + CockDescriptor.describeMultiCockShort(player));
		...
		MainScreen.text(" for her viewing consumption.  Making quite the persuasive argument of non-hostility as you do, the sand witch finally relents, removing her cloak and exposing her four-breasted, ");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(num2Text(player.lowerBody.cockSpot.count() + 1));
		else MainScreen.text("two");
		MainScreen.text("-sexed form, allowing the sun to glisten off her bare body.");
		...
		if (player.canOvipositBee()) MainScreen.text("bee-morphs");
		else MainScreen.text("driders");
		MainScreen.text(" can do to their partners and unsure if this is really what she wants.  The poor girl seems to need a little 'encouragement'... of course, you could just skip all that regardless if you really wanted.");
	...
	private laySomeEggsInThatWitchFinally(): void {
		...
		MainScreen.text("Before long the intense need to culminate the act grows too lucid to ignore, and with a hum of satisfaction you finally bore your ovipositor through the tight ");
		if (cunt) MainScreen.text("folds of her vulva");
		else MainScreen.text("ring of her anus");
		MainScreen.text(".  Like something out of a fever dream, the bizarre appendage you wield burrows further and further into her ");
		if (cunt) MainScreen.text("sex");
		else MainScreen.text("ass");
		MainScreen.text(", the ");
		if (cunt) MainScreen.text("ribbed contours of her pussy");
		else MainScreen.text("tight fit of her innards");
		MainScreen.text(" enrapturing your egg-tube with every passing inch.  You begin the process of lubricating the antsy witch's ");
		if (cunt) MainScreen.text("love-");
		else MainScreen.text("anal ");
		MainScreen.text("canal for the delivery of your young; the witch barely contains her moans and utter glee at being filled with such a hot, humid substance.  In perverted delight the sand witch coos between kisses, her moans betraying a curiosity to explore just how pleasant mating this way can truly be.");
		...
		if (cunt) MainScreen.text("opening of your lover's cervix");
		else MainScreen.text("deepest recesses of her sphincter");
		MainScreen.text(".  The joining of oviduct to ");
		if (cunt) MainScreen.text("womb");
		else MainScreen.text("anus");
		MainScreen.text(" now complete, you prepare yourself for the inevitable convulsion of orgasm along your bizarre appendage, grinning down at the sand witch with a smirk that can only convey that the fun's about to begin.  With a sudden, violent quake your ovipositor rumbles with the first of many eggs, squeezing your unborn brood down your shaft and pushing ");
		if (cunt) MainScreen.text("at the waiting lips of the sand witch's vagina");
		else MainScreen.text(" through her rectal walls");
		MainScreen.text(".  With a meticulous push, the forerunner of your young manages to infiltrate her passage, working its way valiantly into her depths with every convulsion.");
		...
		if (cunt) MainScreen.text("into her waiting womb");
		else MainScreen.text("as far as you can safely tuck it");
		MainScreen.text(", nestled and sound until the scramble for life can begin.");

		MainScreen.text("\n\n\"<i>More... give me more...</i>\" the sand witch whimpers, caressing your ");
		//(Bee, non-horse:
		if (player.canOvipositBee() && !player.lowerBody.isTaur()) MainScreen.text("torso");
		else MainScreen.text("pelvis");
		MainScreen.text(" to spur you on.  Of course you don't have a problem with fulfilling her wishes; the neglect of your throbbing sex");
		if (player.lowerBody.cockSpot.count() > 1 || player.gender > 2) MainScreen.text("es");
		MainScreen.text(" is the only point of dissatisfaction, a point you make clear when you ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("take hold of your " + CockDescriptor.describeMultiCockShort(player) + " and bring it");
		else MainScreen.text("bring your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)));
		MainScreen.text(" to her face.  The cute stare of her half-glazed eyes turns to the lunge of lips toward your sex, the sand witch driven like a woman possessed in her efforts to get the next batch going...");

		MainScreen.text("\n\nFor what feels like an eternity you sit atop the pliant sorceress, having her tend to your sex");
		if (player.lowerBody.cockSpot.count() > 1 || player.gender == 3) MainScreen.text("es");
		MainScreen.text(" while you slowly pump her full of eggs, her girlish little moans resonating through your body as she orally stimulates your ");
		...
		MainScreen.text(".  At long last her enthusiastic efforts bring you to a more 'traditional' release, lathering ");
		let facial: number = rand(2);
		if (facial == 0) MainScreen.text("her facial features with proof of your climax");
		else if (facial == 1) MainScreen.text("the interior of her mouth with sweltering streams of your cum");
		MainScreen.text(".  Having finally achieved release, and also having run out of eggs, you pull yourself free of the desert woman with a loud pop, the ");
		if (player.canOvipositBee()) MainScreen.text("honey-like");
		else MainScreen.text("goopy");
		MainScreen.text(" lubricant drooling out of her in the absence of your intimate plug.  Utterly overwhelmed and exhausted, the sand witch resigns herself to gathering strength in the bright, blistering gaze of the desert sun.  Smiling to yourself, you take to re-donning your discarded [armor], remarking as you leave that you look forward to the next encounter; a sentiment the sorceress must agree with as she weakly waves to you, holding her stomach with her arms and filled with motherly delight at the thought of carrying your brood.");
		//Give her ze eggs!
	...
	public sammitchBirthsDriders(): void {
		...
		MainScreen.text("As you struggle with the loose footing of the hot desert sand, a weak howl reaches your ears.  Curiously, you stop and listen, trying to locate the potential source of the sound.  Yet all you can really hear is the wind against the sand-blasted, desolate terrain, giving you cause to shrug it off and continue on your merry way.");
		...
		if (player.stats.cor < 50) MainScreen.text("Intending to help");
		else MainScreen.text("Curious");
		MainScreen.text(", you slide down the dune that you are on and come to a stop next to the distressed woman.  However, upon seeing her face you realize that this is the very sand witch that you had a hot and steamy tumble with!");
	...
	private helpZeWithBirfBabies(): void {
		...
		if (player.lowerBody.isTaur() || player.lowerBody.isDrider()) {
			...
			if (player.lowerBody.isDrider()) MainScreen.text("skitter");
			else MainScreen.text("gallop");
			MainScreen.text(" across the dry desert sea...");
		}
	...
	private reluctantlyHelpZeWitch(): void {
		...
		//PC is a drider/centaur/whatever:  
		if (player.lowerBody.isDrider() || player.lowerBody.isTaur()) {
	...
	private sandwitchBirthsYourMonstrosities(): void {
		...
		if (player.stats.cor < 60) MainScreen.text(", with a quick kiss on the lips");
		MainScreen.text(", whisper in her ear that you'll see her again...");
	...
	private sandwitchSpanking(): void {
		MainScreen.clearText();
		MainScreen.text("Feeling euphoric after your clear victory, you lazily circle the panting witch and laugh at her.  \"<i>Look at you,</i>\" you taunt, \"<i>A mighty witch of the sands, laid low by a helpless traveller.");
		if (monster.HP < 1) MainScreen.text("  You can't even stand up!");
		else MainScreen.text("  You can't even stop masturbating, can you?");
		MainScreen.text("</i>\"  You kick her hand away, and she tumbles down into a distraught heap, crying softly");
		if (monster.lust > 99) MainScreen.text(" but still masturbating");
		MainScreen.text(".  It's time for some fun!");
		...
		if (player.stats.cor < 33) MainScreen.text("and though you feel kind of bad about using her this way, she likely would have done worse to you.");
		else if (player.stats.cor < 66) MainScreen.text("and though you aren't sure you ought to be using her this way, she likely would have done worse to you.");
		else MainScreen.text("and though you used to find things like this distasteful, you don't really give a shit anymore.");
		MainScreen.text("  You tweak and tease with a goofy smile, laughing as you aim one of the tits up into the witch's face, splattering her with her own alabaster juices.  You're so involved in it that you fail to notice the increasingly angry gaze that settles over your captive's face.  Her tits are just so goddamn fun to tease that you don't really waste any time paying attention to her from the neck up.  That's a mistake.");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("Worse still, [eachCock] is now unprotected and pressed against the rough stone by your body-weight.  It's less than pleasant.  ");
		else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("Worse still, your [vagina] is pressed up against the rough stone.  It's less than pleasant.  ");
		MainScreen.text("You bristle at your helplessness");
		if (player.perks.has("FireLord") || player.perks.has("Hellfire") || player.perks.has("Dragonfire")) MainScreen.text(", breathing out a gout of flame in rage, but you can't even direct the blast properly like this.  All it does is melt some of the sand below into glass");
		else MainScreen.text(", but there's nothing you can do");
		MainScreen.text(".  Fingers tickle the " + player.skin() + " of your bottom, and then, without warning, your [butt] is slapped!  The sound of the hit rings out across the desert, hard enough to make moisture bead at the corners of your eyes.");
		...
		if (player.stats.cor < 50) MainScreen.text("She sounds almost fanatical in her dedication to her order.");
		else MainScreen.text("What the hell is she on about?  Does she actually think her little torture session is going change your attitude?");
