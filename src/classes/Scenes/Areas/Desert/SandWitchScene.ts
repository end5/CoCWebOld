...
//const EGG_WITCH_COUNTER: number = 588;
		...
		public encounter():void {
			...
			if (player.statusAffects.get("Exgartuan").value1 == 1 && player.cockArea(0) > 100 && player.statusAffects.get("Exgartuan").value2 == 0) {
				...
				MainScreen.text(" as you yank your " + player.armorName + " back into place.  You're in charge here, not some possessed appendage!   Exgartuan yells something snide, but it's muffled too much to understand.  You look up in time to sidestep an attack from the Sand Witch.  It looks like you'll have to fight her!");
...
internal function sandwitchRape():void {
	...
	if(player.lust > 99) {
		...
		if(player.hairColor.indexOf("sandy blonde") != -1) {
			...
			if(player.biggestLactation() > 0) {
				MainScreen.text("A strong pressure builds in your chest, painful in its intensity.  You yank down your top as ", false);
				if(player.biggestLactation() < 2) MainScreen.text("powerful jets of milk spray from your nipples, spraying thick streams over the desert sands.  You moan at the sensation and squeeze your tits, hosing down the tainted earth with an offering of your milk.  You blush as the milk ends, quite embarassed with your increased milk production.  ", false);
				...
				if(player.biggestLactation() > 2.6 && player.biggestLactation() < 3) MainScreen.text("thick hoses of milk erupt from your aching nipples, forming puddles in the sand.  You smile at how well you're feeding the desert, your milk coating the sand faster than it can be absorbed.  The constant lactation is pleasurable... in a highly erotic way, and you find yourself moaning and pulling on your nipples, totally outside of your control.  In time you realize the milk has stopped, and even had time to soak into the sands.  You wonder at your strange thoughts and pull your hands from your sensitive nipples.  ", false);
		...
		else 
		{
			...
			if(player.lowerBody.cockSpot.count() == 1) {
				...
				if(player.dogCocks() > 0 && player.lowerBody.cockSpot.list[0].knotMultiplier > 1.4) knotSandwitch();
					...
					MainScreen.text("Thrusting deep, you hit bottom, and marvel at the sensations as you feel the head of your " + cockDescript(0) + " rub against the end of her slick cunts, only making you lust after her more. As though responding to your will, your " + cockDescript(0) + " grows just a tiny bit more in length and thickness.  ", false);
					...
					MainScreen.text("Finally finished, you pull your " + cockDescript(0) + " from one of her well-used slits, your " + cockDescript(0) + " drooling cum even now. With a grin, you walk away, happy with your little encounter.  ", false);
					player.orgasm();
				}
			}
			//Multiprick
			if(player.lowerBody.cockSpot.count() > 1) {
				...
				MainScreen.text("With a satisfied growl of lust from yourself, and a screech of pain from the sand witch, you break past her cervixes and into her wombs. Pulling the heads of your " + multiCockDescriptLight() + " from her wombs, you rub and rut at the outside of her cunts, bringing yourself closer and closer to your peak as the sand witch struggles, her eyes turning wide with fright as she starts to understand you hold more power than her.  ", false);
				...
				MainScreen.text("Finally finished, you pull your " + multiCockDescriptLight() + " from her well used slits, your " + multiCockDescriptLight() + " drooling cum even now. With a grin, you walk away, happy with your little encounter.  ", false);
			}
			cleanupAfterCombat();
		}
	}
...
private sandwitchSex():void {
	...
	MainScreen.text(" of them!  The sand witch appears frightened, but as you gently tease her lips with your fingers, she slowly starts to breathe faster, starting to moan as her legs spread themselves against her better judgment. Grinning in satisfaction, you bring your rigid " + cockDescript(0) + " against her lips, and push slowly, gently.  ", false);
	MainScreen.text("The sand witch cries out, her hands grabbing your arms, pressing hard against your " + player.skinDesc + " as you slowly and teasingly ease your " + cockDescript(0) + " into her tight depths. You almost feel like a virgin, your " + cockDescript(0) + " throbbing, the need to cum almost beyond your ability to control.  ", false);
	...
	MainScreen.text(cockDescript(0) + " throbs with need, even as you empty your lust into the defeated sand witch.", false);
...
private sandwitchCentaurBoning():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) {
		MainScreen.text("As you approach, her eyes drift up to your " + player.allBreastsDescript() + " and she ", false);
		//[win via HP]
	...
	if(player.biggestLactation() > 1) {
		MainScreen.text("Her eyes widen when she sees the beads of milk already dripping from your " + nippleDescript(0) + "s as her hand drops between her legs and she begins to slowly masturbate.", false);
	}
	MainScreen.text("\n\n", false);
	//[Corruption < 70]
	if(player.stats.cor < 70) {
		...
		//[has breasts]
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) MainScreen.text("; your " + nippleDescript(0) + "s  become stiff", false);
		//[cock only]
		if(player.totalCocks() > 0 && !player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" and your " + cockDescript(0) + " hardens.", false);
		//[cunt only]
		if(player.lowerBody.vaginaSpot.hasVagina() && player.totalCocks() == 0) MainScreen.text(" and your " + vaginaDescript(0) + " begins to drip with moisture.", false);
		//[cock and cunt]
		if(player.lowerBody.vaginaSpot.hasVagina() && player.totalCocks() > 0) MainScreen.text(" and your " + cockDescript(0) + " hardens as your " + vaginaDescript(0) + " begins to drip with moisture.", false);
		if(player.gender == 0) MainScreen.text(" her.", false);  
		MainScreen.text(" She seems hesitant at first, but soon approaches and begins to run her hands along your stomach and your " + hipDescript(),false);
		//[has breasts]
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) MainScreen.text(", eyes fixed on your " + player.allBreastsDescript(), false);
		MainScreen.text(".\n\n", false);
		...
		//[has breasts]
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) MainScreen.text("fondle your " + player.allBreastsDescript() + " and ", false);
		MainScreen.text("tease your " + nippleDescript(0) + "s, and you can feel her grinding herself against your strong shoulders. ", false);
		//[orb penetrated player during combat] 
		MainScreen.text("With a start you realize that the witch's orb is still inside you as it suddenly begins to pulse within your " + player.assholeOrPussy() + ", causing you to cry out and hasten your pace across the dunes. ", false);
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) {
			//[lactating]
			if(player.biggestLactation() >= 1) {
				MainScreen.text("Your own " + nippleDescript(0) + "s  are leaking as well, ", false);
				//[light-med lactation]
				if(player.biggestLactation() < 3) MainScreen.text("small streams running out ", false);
				//[heavy lactation] 
	...
	//[Corruption > 70]
		...
		//[STR > 50]
		if(player.str >= 50) MainScreen.text("easily tear them off of her. ", false);
		//[STR < 50]
...
private SWCentaurMore(argument:number):boolean {
	...
	if(argument == 2) {
		...
		if(player.lowerBody.cockSpot.count() > 0 || (player.gender == 3 && rand(2) == 0)) {
			MainScreen.text("Finished with your games, pre-cum starts pooling under your throbbing " + cockDescript(0) + ". ", false);
			...
			MainScreen.text("You grab the witch and knock her down into the sand, quickly lowering your " + cockDescript(0) + " to be against her buttocks. ", false);
			//[1 cock, non-tentacle]
			if(player.lowerBody.cockSpot.count() == 1 && player.tentacleCocks() == 0) MainScreen.text("With a single thrust, you push deep into one of her cunts, ", false);
			//[1 cock, tentacle]
			if(player.lowerBody.cockSpot.count() == 1 && player.tentacleCocks() == 1) MainScreen.text("Your " + cockDescript(0) + " caresses her anus teasingly, causing her to whimper in a mixture of arousal and denial.  With a single savage thrust, you push past her clenched muscles, ", false);
			//[two cocks] 
			if(player.lowerBody.cockSpot.count() == 2) MainScreen.text("After aligning your " + multiCockDescriptLight() + " to her twin cunts, you push yourself deeply into her, ", false);
			//[3+ cocks]
			...
			if(player.lowerBody.cockSpot.count() >= 2) {
				MainScreen.text("The feeling of your " + multiCockDescriptLight() + " touching each other through the narrow layers of her body causes you to grunt in pleasure, experiencing the sensation anew during every thrust. ", false);
			}
			//[largest cock is long, non-tentacle]
			if(player.lowerBody.cockSpot.list[0].cockLength >= 12) {
				if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("Her body surrenders more and more as your " + multiCockDescriptLight() + " presses against her cervix. ", false);
				else MainScreen.text("Her body surrenders more and more as your " + multiCockDescriptLight() + "  press against her cervixes. ", false);
			...
			if(player.tentacleCocks() == 1 && player.lowerBody.cockSpot.count() == 1) {
				MainScreen.text("Her body surrenders more and more as your " + cockDescript(0) + " pushes deep into her bowels, snaking its way further into her body as it fucks her insides. ", false);
			}
			...
			MainScreen.text("You cannot hold off your orgasm any longer. Your " + multiCockDescriptLight() + " explode", false);
			...
			//[1 cock, tentacle]
			if(player.lowerBody.cockSpot.count() == 1 && player.tentacleCocks() == 1) MainScreen.text("anus", false);
			else {
				//[largest cock is not wide and/or long]
				if(player.lowerBody.cockSpot.list[0].cockLength >= 12 || player.lowerBody.cockSpot.list[0].cockThickness >= 3) {
					//[1 cock]
			...
			MainScreen.text(".  With a satisfied groan, you pull out and let your " + multiCockDescriptLight() + " dribble the last remnants of your cum over the ravished witch. Satisfied, you ride off into the desert.", false);
		}
		//[% no cocks]
		else {
			MainScreen.text("Finished with your games ", false);
			//[has cunt] 
			if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("and your " + vaginaDescript(0) + " dripping with desire", false);
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) {
				MainScreen.text(vaginaDescript(0) + ", she fingers herself even harder than you had been a moment ago.  Her lips lock unto your " + clitDescript() + " and she suckles on it, drawing a pained gasp from you and causing your pussy juices to gush over her face. You feel ", false);
				//[cunt size is small]
				...
				MainScreen.text("pushing deep into your " + vaginaDescript(0) + ", and you're surprised as ", false);
				//[anus size is small]
				...
				MainScreen.text("penetrates your " + assholeDescript(), false);
				MainScreen.text(".", false);
			}
			//[does not has cunt]
			else {
				MainScreen.text(assholeDescript() + ", she fingers herself even harder than you had been a moment ago.  Her tongue drives into your " + assholeDescript() + ", licking enthusiastically as she squeezes her nipples. ", false);
				//[anus size is small]
	...
	if(argument == 3) {
		...
		//[STR > 50]
		if(player.str > 50) MainScreen.text("pick her up and toss her onto your back. ", false);
		//[STR < 50]
...
private knotSandwitch():void {
   ...
   if(player.cumQ() >= 25 && player.cumQ() < 100) MainScreen.text("Your cum expands her pussy, stretching her out and leaving your " + cockDescript(0) + " swimming in your spunk, trapped within her tight-stretched walls by your massive knot.", false);
...
private ovipositSandWitches():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("hardened  " + multiCockDescriptLight());
	...
	if(player.totalCocks() > 1) MainScreen.text(num2Text(player.lowerBody.cockSpot.count() + 1));
...
private laySomeEggsInThatWitchFinally():void {
	...
	//(Bee, non-horse:
	if(player.canOvipositBee() && !player.isTaur()) MainScreen.text("torso");
	...
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("take hold of your " + multiCockDescriptLight() + " and bring it");
	else MainScreen.text("bring your " + vaginaDescript(0));
	...
	if(player.gender == 3) MainScreen.text(" and ");
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("your " + vaginaDescript(0));
...
private helpZeWithBirfBabies():void {
	...
	if(player.isTaur() || player.isDrider()) {
		...
		if(player.isDrider()) MainScreen.text("skitter");
