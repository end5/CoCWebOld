...
private function firstMinervaEncounter():void {
	...
	//If Sophie Bimbo=0:
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] == 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00283] == 0 && !kGAMECLASS.sophieFollowerScene.sophieFollower()) Render.text(" that matronly harpy, Sophie");
	else Render.text(" something interesting");
	Render.text(", or get some good plunder from some of the local beasts");
	//if minotaur cum addict:
	if(player.minotaurAddicted()) Render.text(".  With any luck, you might even find a hunky minotaur to scratch that burning itch in your mind");
	Render.text(".  Trudging on, you make your way up and up the mountain, passing the occasional harpy nest as you climb and traverse along the mountain paths.  Just when it looks like you're going to hit the clouds, you catch sight of a trail that you had never really noticed before.");
	//Choice
...
private function takeMinervasPath():void {
	...
	if(player.tallness <= 96) Render.text(" one hand");
	else Render.text(" both hands");
	Render.text(", bodily swinging you out over the abyss, with only her grip keeping you from plunging back down the mountainside.");
	//Low corruption: 
	if(player.stats.cor < 50) Render.text("  \"<i>All right, you!  Who are you and what are you doing here?!  I don't get any visitors besides those horny harpies, so what are you after?  Hm?");
	//High corruption:
	else Render.text("  \"<i>All right, you corrupted beast!  Who are you and what are you doing here?  Your presence is not welcome in this sanctuary... answer quickly or I'll drop you to your death!");
	//Demon Score=3>:
	if(player.demonScore() >= 3) Render.text("  Filthy demon!  Come to finish your work, have you?!  You won't take me without a fight!  Maybe if you beg for your life I'll give you a chance. Speak!  Convince me to not simply kill you here and now!");
	Render.text("</i>\" the creature demands, clearly threatening to drop you if you don't answer to her liking.");
	...
	if(player.isGoo()) {
		...
		//Corruption=75>:
		if(player.stats.cor >= 75) Render.text("... \"<i>yet</i>\", you think to yourself.");
	...
	//High corruption:
	if(player.stats.cor > 66) Render.text("  She is wise not to fully trust you.");
	
	Render.text("\n\nWith");
	if(player.isGoo()) Render.text(" your own");
	else Render.text(" a");
	Render.text(" relieved sigh, you take in the sight of the strange shark... harpy... harpy-shark... sharpy?  What an odd combination!  Looking the woman up and down, you take in her whole appearance.");
...
private function repeatEncounterMinerva():void {
	...
	//No romance: 
	if(!minervaRomanced()) Render.text("  \"<i>Ah, [name], you have returned!  What can I do for you?</i>\"");
	else Render.text("  \"<i>Mmm... my lover has come to see me!  Oh, how I have missed you, my sweet!  Won't you join me for a bath?  Or... is there anything else I can do for you?</i>\"");
...
private function minervaThirdPlusEncounter():void {
	...
	//No romance:
	if(!minervaRomanced()) Render.text("  \"<i>Welcome back, [name]!  It's good to see you again.  Have you come to give me some company, or would you like to partake in some of the fruit?  Or are you maybe here to sate your thirst?</i>\"");
	else Render.text("  \"<i>Darling!  Coming to visit me all the way out here!  How sweet of you!</i>\"");
	Render.text("  Minerva says as she strides toward you, her generous hips swaying back and forth, as if to entice you into her arms.");
...
private function minervaAfterFiveTalksRomancing():void {
	...
	if(!player.isTaur() && !player.isDrider()) {
		...
		Render.text("  Looking up, you see the grinning face of Minerva, clearly playing with you.  \"<i>Well, hello there, [name]!  I was wondering when you would come and visit me.  I can be away from the oasis for a little while, so... want to go for a flight, or are you scared of heights?</i>\"  she asks playfully.");
		//[Flight] [Land]
		menu();
...
private function rejectMinervasLove():void {
	...
	if(player.stats.cor < 45) Render.text("The look in the siren's eyes only makes this harder for you as you look into them; you can't help but feel bad that you really don't feel anything special for her.");
	else Render.text("You can tell she really wants this - the siren is almost desperate for your attention.  Unfortunately for the love starved herm, she won't find the love she craves with you.");
	Render.text("  You look away from her and pull back a bit before speaking.  You confess to her that, while she is a nice person, you do not feel that sort of bond with her, and you apologize if she was misled by your actions, but that you would like to be friends with her still.");
...
private function talkingToMinervaAboutBackstory():void {
	...
	//PART 3
		...
		//No Romance: 
		if(!minervaRomanced()) Render.text("  \"<i>You're a good friend to have, and you know that, [name].</i>\"");
		//Romance:
		else Render.text("  \"<i>You're a wonderful lover, you know.  I couldn't be happier that you found me.</i>\"");
		// PC returns to camp.
...
private function talkToMinervaAboutHerCorruption():void {
	...
	if(flags[FlagEnum.MINERVA_BACKSTORY_LEARNED] == 1) Render.text("  You remember my firstborn, the shark girl I had?  Well... not long after she reached maturity, she... well, she got a bit amorous...  She seduced me and I... I mated with her.  I mated with her over and over that night.  I enjoyed it... it was like I couldn't get enough...  I probably got her pregnant...  So shameful...  I impregnated my own daughter and made her bear my grandchild,");
	Render.text("</i>\" she declares sourly as she stares into the pure waters of the spring.");
...
private function minervaMotherhood():void {
	...
	if(player.gender == 1) Render.text(" and those goblins");
	//PC vagina: 
	else if(player.gender == 2) Render.text(" and that bee-girl");
	//PC herm:
	if(player.gender == 3) Render.text(", goblins and that bee-girl");
	Render.text(", if the stories were real, I'm sure you gave them plenty of children.  I wish to one day have kids... to have a great, loving family to call my own.</i>\"");
	
	Render.text("\n\nShaking your head, you grin and shuffle over to Minerva.  Sitting down with her, you");
	//Romance: 
	if(minervaRomanced()) Render.text(" hug her tightly and kiss her cheek, telling her that she will have children one day, and perhaps you will have to see about helping her with that 'problem.'");
	//No romance:
	else Render.text(" place a hand on her shoulder and give her a comforting smile before telling her that she will have that one day - you're sure of it.");
	Render.text(" Hearing your words brings a smile to her black glossy lips.  \"<i>Thank you so much; that means a lot to me, really,</i>\" she coos, her hands gingerly stroking yours before pulling away.  \"<i>But... I guess we have been here for a while.  I'm sure you have a great deal of work to do as such a brave champion.</i>\"");
...
private function minervaSexMenu(display:boolean = true):void
{
	if (display)
	{
		clearOutput();
		spriteSelect(95);
		Render.text("You tell Minerva that, this time, you'd like to have sex with her.");
		//Default, first time:
		if (flags[FlagEnum.TIMES_MINERVA_SEXED] == 0)
		{
			...
			//PC cock, add: 
			if (player.lowerBody.cockSpot.hasCock()) Render.text(" just... not in my vagina, all right?  That's... special.");
			Render.text("  Did you have anything specific in mind?");
			//PC female, add: "
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Would you mind how we do it?  You're the one asking, so... would you want me to fuck your ass?  Or... do you want me to try to fit between those sexy hips of yours?");
			Render.text("</i>\"");
		}
		//Dick Too Big - PC exceed capacity
		else if (rand(2) == 0 && player.lowerBody.cockSpot.hasCock() && player.cockThatFits(minervaACapacity()) < 0)
			Render.text("Looking at the monstrous mass of your mammoth member, Minerva blushes hard and stares at it with wide eyes.  \"<i>Oh... oh my, that... that's </i>big</i>... I'm sorry, I don't think that's going to fit inside me, how about we try something else?  I'm sure we can come up with something!</i>\"  Minerva says with confidence.");
		//{Repeated/No romance: 
		else if (!minervaRomanced())
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Since you don't seem to mind... would you like me to fuck your ass or to squeeze into your cute cunny?");
			Render.text("</i>\"");
		}
		//Romanced: 
		else
		{
			...
			//PC cock, add:
			if (player.lowerBody.cockSpot.hasCock()) Render.text("  Since we're together, I'll even let you have my pussy, I bet you like the sound of that!");
			//PC female, add:
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("  I'd be happy to pound that cute butt of yours and stuff your hot pussy - which would you rather have, my dear?");
			Render.text("</i>\"");
		}
	}
	menu();
...
private function fuckMinervasAsshole():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and your [vagina]");
	Render.text(" to the open air.");
	Render.text("\n\nFeeling Minerva's bare ass rubbing against you helps coax your ");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("shafts");
	else Render.text("shaft");
	Render.text(" to erectness, and you begin to teasingly drag [oneCock] through the crack of her curvy, spankable ass, hesitating to actually penetrate her until you're ready.  Your vexation brings a brighter blush to her cheeks, her tail squirming gently against you as she takes the hint.  Her hips start pushing back and forth, using her soft rounded rump to massage [eachCock], wanting to get you just as needy as she is.");
...
private function minervaCowgirlSex():void {
	...
	if(x < 0) x = player.smallestCockIndex();
	flags[FlagEnum.TIMES_MINERVA_SEXED]++;
	Render.text("You look the attractive siren up and down and");
	if(player.tallness <= 60) Render.text(", with some trepidation, given the difference in size,");
	Render.text(" ask if Minerva would like to give her female parts some pleasure, promising to allow her control of the pace.");
	//First time:
	if(flags[FlagEnum.TIMES_MINERVA_COWGIRLED] == 0) {
		Render.text("  Minerva looks unsure, the siren biting her lip and looking a little away from you, \"<i>I don't know.  That's... special...  After the things I have been through, I don't want to just do it with anyone.  You... </i>have<i> been really nice to me though... I guess I can trust you.</i>\"");
	}
	//Repeated:
	else Render.text("  Minerva runs a hand through her hair and smiles, \"<i>Well... the last time was quite nice, I think it could be fun.  Sure, let's do it.</i>\"");
	...
	if(player.cumQ() >= 500) {
		...
		if (player.lowerBody.balls > 0) Render.text(" your balls and");
		Render.text(" the base of your " + cockDescript(x) + ", stinging you again and again like an overzealous jellyfish, pumping more of her aphrodisiac venom into your genitals in mind-numbing quantities.");
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  A few even slither down into your [vagina] to sting you there as well, making sure you are, at least temporarily, turned into a sex-hungry beast.");
	}
	...
	if(player.cumQ() < 500) Render.text(", Minerva's belly visibly bloated by the time you are finished");
	Render.text(".");
	if(player.cumQ() < 500) {
	...
	if(player.cumQ() >= 500 && player.cumQ() < 4000) {
		...
		if(minervaRomanced()) Render.text(" with your child");
		Render.text(".");
	}
	//Very High: 
	if(player.cumQ() >= 4000) {
		...
		if(!minervaRomanced()) Render.text("  \"<i>Oohhh...I'm so... so big... It's almost like you've gotten me pregnant.</i>\"");
		else Render.text("  \"<i>Oohhh... I'm so... so big... cumming so much, I didn't know you were so desperate to get me pregnant.</i>\"");
	}
	//Low-to-moderate: 
...
private function minervaLapSex():void {
	...
	//PC herm: 
	if(player.gender == 3) Render.text("  One of her hands trail down to your hard herm " + cockDescript(x) + ", stroking you softly.");
	...
	//PC herm: 
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text(", your own throbbing phallus");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("es");
		Render.text(" cumming alongside,");
		//{Low-moderate cum: 
		if(player.cumQ() < 500) Render.text(" gushing your cum all over both of your bellies");
		//High cum volume:
		else if(player.cumQ() < 4000) Render.text(" splattering thick splooge all over you and Minerva, caking your torsos in fertile jizz");
		//Very high cum:
		else Render.text(" exploding out onto your unsuspecting bodies, thick ropes of starkly white sperm gushing over your bodies, quickly dousing both of you in buckets of cum and pooling on the ground");
	}
	Render.text(".");
...
private function goDownOnAHermAndLoveItYouDirtySlutYou():void {
	...
	if(player.tallness < 96) Render.text("your fingers not even able to fit around the herm's hefty girth");
	else Render.text("your fingers barely wrapping around the meaty member, the fat thing throbbing gently");
	Render.text(" as you stroke it slowly.  Your touches are clearly adding to your partner's pleasure, her thick masculinity pulsating under your grip, her tentacles reacting to your presence, the dexterous things curling around your hands and fingers, touching you gently, but not stinging you.");
...
private function noBallsMinervaAndContinuation(clear:boolean = true):void {
	...
	spriteSelect(95);
	Render.text("Your careful ministrations soon have the poor siren shuddering and squirming, looking about ready to pop from your skilled hands and mouth.");
	if(!clear) Render.text("  Retreating from her throbbing balls, you");
	else Render.text("  You");
	Render.text(" move back up to her now soaking snatch, your tongue eagerly lapping up all the tasty juices from her lips before delving deep into her refreshingly cool cunt, hungrily licking up every slick drop of her succulent sweetness.  The act of worming your tongue back inside her hungry hole again drives Minerva ever closer to the brink of her climax, each slurp and stroke of your tongue and hands pushing the near-climactic herm right to the edge and sending her off into the blissful, pleasure-filled abyss of a mind-blowing double orgasm.");
	...
	//if demonic tongue:
	if(player.tongueType == TONUGE.DEMONIC) Render.text(", your perversely prehensile tongue slowly retracting from her quivering quim, rubbing and teasing as it comes out and nearly sending the overly sensitive herm into another climax");
	Render.text(".");
...
private function letMinervaSuckYouOff():void {
	...
	//If naga: 
	if(player.isNaga()) Render.text(" lounge against the rock, spreading open your protective slit and letting your " + cockDescript(x) + " hang free.");
	else Render.text(" sit down, spreading your [legs] and opening up the crotch of your [armor] to the air, letting your " + cockDescript(x) + " hang free.");
	...
	//If balls:
	if(player.lowerBody.balls > 0) Render.text("  Her hand on your hip moves down between your legs, sliding inwards to gently caress and massage your cum-filled [balls] to increase your pleasure even more, her hands trying their best to get your nuts worked up and prepare a good load for her at the end.");
	...
	//If balls:
	if(player.lowerBody.balls > 0) Render.text(" before licking and teasing your [balls]");
	Render.text(".");
	...
	//If cockarea 50+:
	if(player.cockArea(x) >= 50) Render.text(", your huge cock deep inside her mouth, and soon far down her throat.  With some difficulty, her slightly angular nose presses down against your crotch, every inch of your " + cockDescript(x) + " trapped inside her cool, wet mouth and throat");
	Render.text(".");
	...
	if(player.lowerBody.balls > 0) Render.text(", while your [balls] lurch and swell, rising up as you reach your climactic explosion");
	Render.text(".");
...
private function fuckMinervaWithHerHandsBehindHerBack():void {
	...
	//If balls: 
	if(player.lowerBody.balls > 0) Render.text("  As you continue to pound the siren, your [balls] swing between your legs, the sperm-filled orbs churning with your fertility, preparing to fill the needy siren with every drop you can muster when you let loose your eventual orgasm.");
	...
	//Holy shit!:
		...
		if (player.lowerBody.balls > 0) Render.text("  Minerva groans and lets out mindless gurgles as you unload your overproductive balls into her.");  
		Render.text("  To even your amazement, her hungry womb is able to contain much of your sperm before she can take no more, each blast of potent cum now gushing from between your bodies, the floor soon looking like someone dumped a barrel full of raw milk on the ground before you're finally done.");
...
private function pcGetsEatenOutByMinerva():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text(", and, perhaps, your more male parts while she's down there");
	Render.text(".");
	...
	if(player.lowerBody.balls > 0) Render.text(", one of her hands lifting your [balls] up before giving each of the fertile nuts a kiss");
	Render.text(" and moving down to your sweet cunt, just waiting for her attentions.");
...
private function eatSomethingYouCunt():void {
	...
	spriteSelect(95);
	//Acquiring: 
	Render.text("You walk over to the fruit trees surrounding the spring, examining the strange treats.  You decide that one ripe fruit, one resembling a peach, is the best choice, and pluck it from the tree.  Thanking Minerva for letting you have it, you stow it away safely and head back to camp.\n\n");
	inventory.takeItem(consumables.PURPEAC, camp.returnToCampUseOneHour);
}
