...
private function firstMinervaEncounter():void {
	...
	//If Sophie Bimbo=0:
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] == 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00283] == 0 && !kGAMECLASS.sophieFollowerScene.sophieFollower()) outputText(" that matronly harpy, Sophie");
...
private function takeMinervasPath():void {
	...
	//Low corruption: 
	if(player.stats.cor < 50) outputText("  \"<i>All right, you!  Who are you and what are you doing here?!  I don't get any visitors besides those horny harpies, so what are you after?  Hm?");
	//High corruption:
	...
	if(player.isGoo()) {
		...
		//Corruption=75>:
		if(player.stats.cor >= 75) outputText("... \"<i>yet</i>\", you think to yourself.");
	...
	//High corruption:
	if(player.stats.cor > 66) outputText("  She is wise not to fully trust you.");
...
private function rejectMinervasLove():void {
	...
	if(player.stats.cor < 45) outputText("The look in the siren's eyes only makes this harder for you as you look into them; you can't help but feel bad that you really don't feel anything special for her.");
...
private function minervaSexMenu(display:boolean = true):void
{
	if (display)
	{
		...
		if (flags[FlagEnum.TIMES_MINERVA_SEXED] == 0)
		{
			...
			//PC cock, add: 
			if (player.lowerBody.cockSpot.hasCock()) outputText(" just... not in my vagina, all right?  That's... special.");
			outputText("  Did you have anything specific in mind?");
			//PC female, add: "
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("  Would you mind how we do it?  You're the one asking, so... would you want me to fuck your ass?  Or... do you want me to try to fit between those sexy hips of yours?");
			outputText("</i>\"");
		}
		...
		else if (!minervaRomanced())
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("  Since you don't seem to mind... would you like me to fuck your ass or to squeeze into your cute cunny?");
			outputText("</i>\"");
		}
		//Romanced: 
		else
		{
			...
			//PC cock, add:
			if (player.lowerBody.cockSpot.hasCock()) outputText("  Since we're together, I'll even let you have my pussy, I bet you like the sound of that!");
			//PC female, add:
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("  I'd be happy to pound that cute butt of yours and stuff your hot pussy - which would you rather have, my dear?");
			outputText("</i>\"");
		}
	}
	menu();
...
private function fuckMinervasAsshole():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" and your [vagina]");
	outputText(" to the open air.");
	outputText("\n\nFeeling Minerva's bare ass rubbing against you helps coax your ");
	if(player.lowerBody.cockSpot.count() > 1) outputText("shafts");
...
private function minervaCowgirlSex():void {
	...
	if(player.cumQ() >= 500) {
		...
		if (player.lowerBody.balls > 0) outputText(" your balls and");
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  A few even slither down into your [vagina] to sting you there as well, making sure you are, at least temporarily, turned into a sex-hungry beast.");
	}
...
private function noBallsMinervaAndContinuation(clear:boolean = true):void {
	...
	//if demonic tongue:
	if(player.tongueType == TONUGE.DEMONIC) outputText(", your perversely prehensile tongue slowly retracting from her quivering quim, rubbing and teasing as it comes out and nearly sending the overly sensitive herm into another climax");
	outputText(".");
...
private function letMinervaSuckYouOff():void {
	...
	//If balls:
	if(player.lowerBody.balls > 0) outputText("  Her hand on your hip moves down between your legs, sliding inwards to gently caress and massage your cum-filled [balls] to increase your pleasure even more, her hands trying their best to get your nuts worked up and prepare a good load for her at the end.");
	...
	//If balls:
	if(player.lowerBody.balls > 0) outputText(" before licking and teasing your [balls]");
	outputText(".");
	...
	if(player.lowerBody.balls > 0) outputText(", while your [balls] lurch and swell, rising up as you reach your climactic explosion");
	outputText(".");
...
private function fuckMinervaWithHerHandsBehindHerBack():void {
	...
	//If balls: 
	if(player.lowerBody.balls > 0) outputText("  As you continue to pound the siren, your [balls] swing between your legs, the sperm-filled orbs churning with your fertility, preparing to fill the needy siren with every drop you can muster when you let loose your eventual orgasm.");
	...
	//Holy shit!:
		...
		if (player.lowerBody.balls > 0) outputText("  Minerva groans and lets out mindless gurgles as you unload your overproductive balls into her.");  
...
private function pcGetsEatenOutByMinerva():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", and, perhaps, your more male parts while she's down there");
	outputText(".");
	...
	if(player.lowerBody.balls > 0) outputText(", one of her hands lifting your [balls] up before giving each of the fertile nuts a kiss");
	...
	outputText("\n\n");
	//If dick:
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Thanks to her handy stimulation, your musky maleness throbs and dilates along your femininity, the thick cockvein feeding your bloating member as your seed rushes out, splattering up and onto Minerva's back, defiling her smooth skin with your fertility.  ");
