...
private minervaAppearance():void {
	...
	if(player.lust >= 33) MainScreen.addButton(2,"Sex",minervaSexMenu);
...
public encounterMinerva():void {
	...
	else if(flags[FlagEnum.MINERVA_BACKSTORY_LEARNED] > 0 && player.gender > 0 && flags[FlagEnum.MINERVA_LOVE] == 0 && rand(2) == 0) minervaAfterFiveTalksRomancing();
	else {
...
private firstMinervaEncounter():void {
	...
	//If Sophie Bimbo=0:
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] == 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00283] == 0 && !kGAMECLASS.sophieFollowerScene.sophieFollower()) MainScreen.text(" that matronly harpy, Sophie");
...
private takeMinervasPath():void {
	...
	MainScreen.text("\n\nWith");
	if(player.isGoo()) MainScreen.text(" your own");
...
private goForASkyRideWithMinervaHeartBaring():void {
	...
	if(player.isGoo()) MainScreen.text(" cling");
	...
	//Goo: 
	if(player.isGoo()) MainScreen.text(" while you're barely able to maintain your slimy cohesion");
...
private minervaTalkSelect(bath:boolean = true):void {
	...
	if(flags[FlagEnum.MINERVA_BACKSTORY_LEARNED] == 0) {
		...
		if(flags[FlagEnum.MINERVA_BACKSTORY] == 0) {
	...
	if(minervaRomanced() && flags[FlagEnum.MINERVA_BACKSTORY_LEARNED] == 1) choices[choices.length] =  talkWithMinervaAboutSharkGirlDaughter;
...
private talkingToMinervaAboutBackstory():void {
	...
	//PART 1
	if(flags[FlagEnum.MINERVA_BACKSTORY] == 0) {
		flags[FlagEnum.MINERVA_BACKSTORY] = 1;
	...
	//PART 2
	else if(flags[FlagEnum.MINERVA_BACKSTORY] == 1) {
		flags[FlagEnum.MINERVA_BACKSTORY] = 2;
	...
	//PART 3
		...
		flags[FlagEnum.MINERVA_BACKSTORY] = 0;
		...
		// PC returns to camp.
		flags[FlagEnum.MINERVA_BACKSTORY_LEARNED] = 1;
	}
	doNext(camp.returnToCampUseOneHour);
}
...
private talkToMinervaAboutHerCorruption():void {
	...
	if(flags[FlagEnum.MINERVA_BACKSTORY_LEARNED] == 1) MainScreen.text("  You remember my firstborn, the shark girl I had?  Well... not long after she reached maturity, she... well, she got a bit amorous...  She seduced me and I... I mated with her.  I mated with her over and over that night.  I enjoyed it... it was like I couldn't get enough...  I probably got her pregnant...  So shameful...  I impregnated my own daughter and made her bear my grandchild,");
...
private minervaSexMenu(display:boolean = true):void
{
	if (display)
	{
		...
		//Default, first time:
		if (flags[FlagEnum.TIMES_MINERVA_SEXED] == 0)
		{
...
private fuckMinervasAsshole():void {
	...
	if(!minervaRomanced()) {
		//First time ending:
		if(flags[FlagEnum.TIMES_BUTTFUCKED_MINERVA] == 0) {
	...
	flags[FlagEnum.TIMES_BUTTFUCKED_MINERVA]++;
	player.orgasm();
	dynStats("sen", -1);
	if (getGame().inCombat)
		cleanupAfterCombat();
...
private minervaCowgirlSex():void {
	...
	//First time:
	if(flags[FlagEnum.TIMES_MINERVA_COWGIRLED] == 0) {
	...
	if(player.cumQ() >= 500) {
		...
		MainScreen.text(" the base of your " + cockDescript(x) + ", stinging you again and again like an overzealous jellyfish, pumping more of her aphrodisiac venom into your genitals in mind-numbing quantities.");
	...
	flags[FlagEnum.TIMES_MINERVA_COWGIRLED]++;
	player.orgasm();
	dynStats("sen", -1);
	if (getGame().inCombat)
		cleanupAfterCombat();
...
private minervaLapSex():void {
	...
	//PC herm: 
	if(player.gender == 3) MainScreen.text("  One of her hands trail down to your hard herm " + cockDescript(x) + ", stroking you softly.");
	...
	if(!minervaRomanced()) {
		//Default, first time:
		if(flags[FlagEnum.TIMES_MINERVA_LAPSEXED] == 0) {
			MainScreen.text("\n\nMinerva actually seems a little sad that you're leaving.  \"<i>I guess you are a busy person.  But I'm glad we did this, it was a lot of fun.  And... you know...  I wouldn't mind if we did this again, soon,</i>\" She suggests with a small, hopeful smile on her face.  You promise her that you'll try to come back again soon.");
		}
		//Default, repeated:
		else {
	...
	player.slimeFeed();
	flags[FlagEnum.TIMES_MINERVA_LAPSEXED]++;
	player.orgasm();
	dynStats("sen", -1);
	if (getGame().inCombat)
		cleanupAfterCombat();
...
private noBallsMinervaAndContinuation(clear:boolean = true):void {
	...
	//if demonic tongue:
	if(player.tongueType == TONUGE.DEMONIC) MainScreen.text(", your perversely prehensile tongue slowly retracting from her quivering quim, rubbing and teasing as it comes out and nearly sending the overly sensitive herm into another climax");
	MainScreen.text(".");
...
private letMinervaSuckYouOff():void {
	...
	//If naga: 
	if(player.isNaga()) MainScreen.text(" lounge against the rock, spreading open your protective slit and letting your " + cockDescript(x) + " hang free.");
	else MainScreen.text(" sit down, spreading your [legs] and opening up the crotch of your [armor] to the air, letting your " + cockDescript(x) + " hang free.");
	...
	//If cockarea 50+:
	if(player.cockArea(x) >= 50) MainScreen.text(", your huge cock deep inside her mouth, and soon far down her throat.  With some difficulty, her slightly angular nose presses down against your crotch, every inch of your " + cockDescript(x) + " trapped inside her cool, wet mouth and throat");
	MainScreen.text(".");
...
private pcGetsEatenOutByMinerva():void {
	...
	flags[FlagEnum.MINERVA_LEZZES_OUT]++;
	// PC returns to camp
	dynStats("sen", -2);
	player.orgasm();
	if (getGame().inCombat)
...
private getADrinkYouBitch():void {
	...
	if(player.stats.cor > 75) dynStats("cor", -1);
	doNext(camp.returnToCampUseOneHour);
}
/*
