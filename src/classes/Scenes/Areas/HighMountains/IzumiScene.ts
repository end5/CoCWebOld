package classes.Scenes.Areas.HighMountains 
{
	...
	public class IzumiScene extends BaseContent
	{
		...
		protected function enterFirstTimePartII():void
		{
			...
			if (player.stats.cor <= 33) outputText(" exposing an uncomfortable amount of creamy-white thigh flesh.");
			else if (player.stats.cor > 33 && player.stats.cor < 66) outputText(" exposing a tantalising view of creamy-white thigh flesh.");
		...
		protected function surrenderSmallCock():void
		{
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 3) outputText(", your [chest] dwarfed by the larger womans own pair.");
			else if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 5) outputText(", your [chest] moulding themselves against the underside of the Oni’s larger pair.");
			else if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5 && player.upperBody.chest.BreastRatingLargest[0].breastRating <= 8) outputText(", your [chest] compressing against the Oni’s larger pair, forming a delicious valley of flesh between your bodies.");
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText(", sticky moisture dribbling across your thigh, your [vagina] clenching in lust - fueled harmony with your cock");
			...
			if (player.lowerBody.balls > 0) outputText(" and her free fingers reach out to gently stroke against your [balls].");
			else if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" and her free fingers reach out to gently stroke against your [vag].");
			else outputText(".");
		...
		protected function surrenderMediumCockObey():void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" mixed puddle of your own fem-slime and");
		...
		protected function surrenderMediumCockRefuse():void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText(", [vagina] spasming in sympathy");
		...
		protected function surrenderLargeCock():void
		{
			...
			if (player.lowerBody.balls > 0 && !player.lowerBody.vaginaSpot.hasVagina()) outputText(", occasionally sliding down to massage your [balls] before returning to your cock.");
			else if (player.lowerBody.balls == 0 && player.lowerBody.vaginaSpot.hasVagina()) outputText(", occasionally sliding down to tease at the lips of your [vagina].");
			else if (player.lowerBody.balls > 0 && player.lowerBody.vaginaSpot.hasVagina()) outputText(", occasionally sliding down, alternating between a gentle massage of your [balls] and teasing caresses of your [vagina].");
			outputText("\n\n");
			...
			if (player.lowerBody.balls > 0) outputText("  You groan, your aching balls coming to the same conclusion as you have, twitching under a passing stroke from one of Izumi’s soles and tensing up in readiness.");
		...
		protected function surrenderOhGodTheFemaleSurrenderContentIsFuckingHugeSendHelp():void
		{
			...
			if (flags[FlagEnum.IZUMI_TIMES_SUBMITTED] == 0)
			{
				...
				if (player.stats.cor <= 33) outputText("  You can’t help but feel a crippling sense of embarrassment as the strange woman undresses you, and you look away, embarrassed, while Izumi’s hands explore your body.");
				else if (player.stats.cor >= 66) outputText("  As Izumi slowly reveals your naked body to the world, you begin to feel an oh-so-familiar heat building up inside of you.  You look down at Izumi with pride as she inspects your body, running a skillful hand over your chest with a thoughtful look on her face.");
				outputText("\n\n");
			...
			if (flags[FlagEnum.PC_FETISH] >= 1)
			{
				...
				if (player.lowerBody.cockSpot.hasCock()) outputText(", your [cock] slowly thickening under her scrutiny");
		...
		protected function lossSceneStay():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("“<i>Or maybe,</i>”  She whispers seductively into your ear, “<i>You wanna bury that little Oni of yours in </i>me<i>.  Hmmmm?</i>”  You shudder as you feel her hand snake into your underwear and run sinuously across your crotch, readying you for the ordeal ahead...\n\n");
		}
		...
		public function touchThatFluffyHorn():void
		{
			...
			else
			{
				...
				else if (player.stats.lib >= 40 && (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2 || player.biggestCockLength() >= 5))
				{
					...
					if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText("maybe giving her face a little personal time nestled between your [chest] will finally distract the Oni enough.  You reach out for the bright red curve of Izumi’s horn, ");
		...
		protected function letGoAndFuck():void
		{
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 3) outputText(" smothering your [chest] with her own, much larger, pair.\n\n");
			else if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 5) outputText(" your [chest] moulding themselves against the Oni’s larger pair.\n\n");
			else if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5 && player.upperBody.chest.BreastRatingLargest[0].breastRating <= 8) outputText(" your [chest] compressing against the Oni’s larger pair, forming a delicious valley of flesh.\n\n");
			else if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 8) outputText(" your [chest] dominating the Oni’s own in the battle for space between the two of you.\n\n");
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) outputText(" against your own");
		...
		protected function gonnaGetMeSomeRevengeButtsexin():void
		{
			...
			if (player.biggestCockLength() <= 4)
			{
				...
				if (player.stats.cor >= 50) outputText("  You hate to admit it, but in a messed up way, her apathy is beginning to turn you on a bit by itself.  You knew you weren’t the most well-endowed adventurer in the first place, and to someone physically huge like Izumi the problem must be magnified.  For some reason, you find that idea hot as hell....\n\n");
		...
		protected function sayThatsAPrettyFaceBeAShameIfSomebodyJizzedAllOverIt():void
		{
			...
			if (player.lowerBody.cockSpot.list[cockIndex].cockType == CockType.HORSE) outputText("  Visibly unsure of where to put herself, Izumi finally settles for brushing her lips against the exact center of the flared mass that makes up the head of your stallion-prick.");
			else if (player.lowerBody.cockSpot.list[cockIndex].cockType == CockType.DOG) outputText("  Her eyes darting nervously from side to side, Izumi leans forwards and takes the very tip of your dogcock into her mouth, pressing her lips against the main bulk of what passes for your head.");
		...
		protected function tribThatRockyHornGirl():void
		{
			...
			if (player.stats.cor > 80) outputText("“<i>Whatever.  Just try not to cum too hard while I’m making you my bitch, you little slut.</i>” You shoot back, returning");
