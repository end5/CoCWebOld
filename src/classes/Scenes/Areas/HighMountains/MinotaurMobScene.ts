...
public function meetMinotaurSons():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00325] == 1) {
		...
		if(!player.minotaurAddicted()) {
			...
			if(player.stats.cor < 33) outputText("enjoyed their affections more than you ought to", false);
			else if(player.stats.cor < 66) outputText("indulged yourself in their glorious cum more than a few times", false);
			...
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] > 3) outputText("some of ", false);
		...
		//(Addicted) 
			...
			if(player.stats.cor < 33) outputText("You wouldn't fuck your own offspring just for another hit of cum... would you?", false);
			else if(player.stats.cor < 66) outputText("You're not sure how to feel about this morally, but you can't keep from licking your lips, fantasizing about the fix these wonderful kids of yours are so willing to give you.", false);
	...
	//Repeat meetings
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] < 20) {
			...
			if(!player.minotaurAddicted()) {
				...
				if (player.pregnancyIncubation > 0 && player.pregnancyIncubation <= 216 && player.pregnancyType == PregnancyType.MINOTAUR) outputText("Pregnant with another brother for us, and already you come back for more?", false);
			...
			//[Addicted Repeat] 
				...
				if(player.pregnancyIncubation > 0 && player.pregnancyIncubation <= 216 && player.pregnancyType == PregnancyType.MINOTAUR) outputText("Pregnant with another brother for us, and you're already back for another fix?  Why even leave in the first place?", false);
		...
		//Tribe sized
			...
			if(!player.minotaurAddicted()) {
				...
				else if (player.pregnancyType == PregnancyType.MINOTAUR) outputText("Come back to show off the lil' bro yer growin'?", false);
				...
				outputText("No! ", false);
				if(player.stats.cor < 33) outputText("You can't think that way! Sure, you've birthed minotaurs enough to create this huge tribe of horny monsters, but you're a champion!", false);
				else if(player.stats.cor < 66) outputText("You shouldn't think that way! Sure, you've had enough minotaurs inside you to spawn this tribe of muscular beast-men, but you've got a mission to focus on.", false);
			...
			//[Repeat Tribe-Size Addict] 
				...
				else if (player.pregnancyType == PregnancyType.MINOTAUR) outputText("Come back for some more spunk? I bet your next son comes out as a spooge-slurping minitaur.  We could use another cock-sleeve around here.", false);
...
internal function minotaurDeFeet():void {
	...
	//(Lust) 
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("  " + SMultiCockDesc() + " hardens and dangles above the ground, leaking in sympathy with your hungry quim.  Of course it will likely be ignored - rutting minotaurs have little use for someone else's dick.", false);
...
private function nonAddictMinotaurGangBang():void {
	...
	if(player.isTaur()) {
		...
		if(player.tailType == TAIL.NONE) outputText("asshole", false);
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] >= 10) outputText("The assembled beasts snicker at the noise, stroking themselves slowly while awaiting a chance at your hungry holes.", false);
	...
	if(player.stats.cor >= 66) outputText(".  You couldn't be happier about it", false);
	...
	else if (player.pregnancyType == PregnancyType.MINOTAUR) outputText("I hope you don't mind me saying hi to my little brother.", false);
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] <= 3) outputText(" the minotaur that gave you your first dose.  Maybe he's ready to give you more? He smirks and slaps you across the face with his soft shaft.  You giggle drunkenly and start to lick his balls - they HAVE to have more for you!  ", false);
	else {
...
private function loseToMinoMobVeryPregnant():void {
	...
	if (player.pregnancyType != PregnancyType.MINOTAUR) outputText("She doesn't know how to keep her hungry twat closed.  You can see the vermin squirming in her belly.  Disgusting.", false);
	...
	if(player.stats.cor < 33) outputText("irritation at being addressed so insultingly", false);
	else if(player.stats.cor < 66) outputText("annoyance at their familiarity", false);
	...
	if (player.pregnancyType != PregnancyType.MINOTAUR) outputText("isn't", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  The pre-cum leaking from your " + multiCockDescriptLight() + " reminds you of your poor, ignored maleness, but you'll have to settle for servicing your sons' delectable organs.\n\n", false);
	...
	outputText("You look up and oogle the " + num2Text((flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] - 1)) + " dicks hovering just inches away from you.  Even the minitaur is there, his smaller dick dripping the sweet cream of his need at the sight of his slutty, utterly debased mother.  It turns you on so much, even more than the cream in your belly.  You reach out and grab two of the dicks to stroke, lubricating fingers with hot, wet pre-cum.", false);
	if(player.isNaga() || player.tailType == TAIL.DEMONIC || player.tailType == TAIL.COW || player.tailType == TAIL.SHARK || player.tailType == TAIL.LIZARD || player.tailType == TAIL.KANGAROO) outputText("  Even your tail reaches up to curl around a third, stroking the thick meat.", false);
	outputText("\n\n", false);
	...
	if (player.pregnancyType == PregnancyType.MINOTAUR) outputText("That baby is probably mine too.  You let your son father his brother.", false);
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] < 8) {
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] > 3) {
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] == 4) outputText("Sorry bro, it looks like Mom's done.  You'll have to snag a shot at her later - she's pretty fucking pregnant, and I don't want her broken.", false);
	...
	//(Long) 
		...
		if (player.pregnancyType == PregnancyType.MINOTAUR) outputText("we wouldn't want the next member of our tribe injured.", false);
...
private function analSpearSemiPregMinotaurGangbang():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] >= 20) outputText("to make the tribe even bigger", false);
	...
	if (player.pregnancyType == PregnancyType.MINOTAUR) outputText("our little brother", false);
	...
	if(player.upperBody.chest.hasFuckableNipples()) {
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] < 7) outputText("shakes his head.  \"<i>You're only saving up trouble for later, little cunt.  Your hole after Mom's, remember.</i>\"  The minitaur wistfully withdraws his cock and crawls a few paces off, where he sits in a sulk.", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3 && player.biggestLactation() >= 1) outputText(", your pregnant, milky tits swaying underneath you while they're suckled", false);
	...
	if(!player.upperBody.chest.hasFuckableNipples() && player.biggestLactation() >= 1) outputText(", and the minitaur's lips disentangle from your " + nippleDescript(0), false);
	outputText(".  Hands gently lower you to the ground", false);
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] >= 5) outputText(", but you're positioned with your ass in the air.  A voice calls out, \"<i>Okay, Mom's pretty out of it.  One at a time and don't damage her, okay?</i>\"\n\nGiggling dreamily, you're fucked repeatedly, spunk squirting from your anus with every thrust.  You pass out like that, feeling quite satisfied.", false);
...
private function victoryMinotaurGangTitFuck():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] >= 10) {
		...
		if(player.perks.has("BimboBrains") || player.perks.has("FutaFaculties")) outputText("like, ", false);
	...
	//(SMALL) 
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] == 3) outputText("one minotaur", false);
		else outputText(num2Text(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326]-2) + " minotaurs", false);
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] == 3) outputText("You grab the minitaur's cock and drag the shy beast-man over your cum-slicked tits, wasting no time in burying the shaft in mommy's mammaries, where it belongs.", false);
		else {
			outputText("You grab the minitaur and ", false);
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] == 4) outputText("the remaining brother", false);
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] > 3) outputText("His glaze spurts up along the opening created by his larger brother's shaft, and soon he too is cumming, obediently orgasming for you.  ", false);
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] < 5) outputText("The minotaurs let you get dressed while they sigh happily, clearly pleased with the outcome.", false);
		else {
			...
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] > 5) {
				...
				if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] >= 6) outputText("s", false);
			}
...
private function victoryAllThePenetrationsMinotaurGangBang():void {
	...
	if(player.upperBody.chest.hasFuckableNipples() && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] >= 4) outputText("  There's still more you could be doing.", false);
	outputText("\n\n", false);
	...
	if(player.upperBody.chest.hasFuckableNipples() && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] > 4) {
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] == 4) outputText("One stud assembles next to ", false);
		else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] >= 5 && player.totalBreasts() == 2) outputText("Two studs assemble around ", false);
		else {
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] == 4) outputText("He excitedly fingers", false);
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] == 4) outputText("He licks his fingers clean, and a moment later, a cock is spearing one of your nipples, disappearing deep into the tingling passages concealed within your chest.\n\n", false);
	...
	//Dicks)
	if(player.lowerBody.cockSpot.hasCock()) outputText(SMultiCockDesc() + " hangs limply on one of your boys' bellies, dribbling weakly at your need, but there's not any way for it to get involved in the crowd of orgiastic bodies.\n\n", false);
	...
	if(player.stats.cor < 33) outputText(" that you cringe a little when you realize you're having an incestuous orgy, but the rising tide of minotaur-drug in your bloodstream washes the errant thought away.", false);
	...
	if(player.upperBody.chest.hasFuckableNipples() && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] > 4) outputText("your nipples squirt, ", false);
	...
	if(player.upperBody.chest.hasFuckableNipples() && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] > 4) {
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] > 4) outputText("Meanwhile, your " + nippleDescript(0) + "s get a similar feeling from the studs arranged around them", false);
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] < 20) outputText("gang", false);
...
private function forceMinitaurToGiveOral(choice:number = 0):void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] < 20) outputText("mob", false);
	else outputText("tribe", false);
	...
	if(choice == 0) {
		...
		if(player.isTaur()) {
			...
			if(player.tailType > TAIL.NONE) outputText("and lift your tail ", false);
			outputText("to expose the glistening pinkness of your womanhood.", false);
		}
		//ELSE
		else {
			if(player.lowerBody.balls > 0) outputText("lift your balls and ", false);
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength >= 8) outputText(", until the huge size of it is revealed.", false);
		else if(player.lowerBody.vaginaSpot.list[0].clitLength >= 3) outputText(", until the cock-like length of your feminine organ is revealed.", false);
		else if(player.lowerBody.vaginaSpot.list[0].clitLength >= 1) outputText(", until the large, womanly organ is revealed.", false);
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength < 3) outputText(", even sucking your clit into his mouth and running his tongue along its length.", false);
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength >= 6) outputText("He jacks your huge clit like a cock, and the incredible stimulation is too much for you to take.", false);
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("  Salty cum dribbles down his back from your spurting " + multiCockDescriptLight() + ", the male part of you orgasming along in a perverse symphony with your girl-parts.\n\n", false);
	}
	//(PENISCOCKSUCKERFUCK) 
	else {
		...
		if(player.lowerBody.balls > 0) outputText("around your " + sackDescript() + " before curling ", false);
		outputText("around the base of your " + multiCockDescriptLight() + ".  It circles up ", false);
		if(player.lowerBody.cockSpot.count() > 1) outputText("a", false);
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Humming happily, the minitaur slips a digit inside your " + vaginaDescript() + ", and while you didn't ask for it, the added stimulation only heightens your need.  ", false);
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			...
			if(player.lowerBody.cockSpot.count() > 2) outputText("one of your ignored penises", false);
			else outputText("on your ignored penis", false);
		}
		outputText(".", false);
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  You begin to cum into your son's mouth, your pussy hugging his finger tightly inside its depths.", false);
		outputText("\n\n", false);
...
private function victoryBJNippleFuckMinotaurGang():void {
	...
	if(player.upperBody.chest.countNipples() == 2) outputText("each", false);
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] == 3) outputText("the minitaur, much to the smaller monster's chagrin.", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 2) outputText("a few inches", false);
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 4) outputText("inches", false);
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 8) outputText("most of a foot", false);
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 14) outputText("over a foot", false);
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 30) outputText("nearly two feet", false);
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] >= 7) outputText(", chased by jeers from the remainder of your brood", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		outputText("  " + SMultiCockDesc(), false);
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" and " + vaginaDescript() + " drool", false);
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] < 5) outputText("relax and fan themselves while giving you lusty looks", false);
...
private function minotaurGangBadEnd():void {
	...
	if(player.upperBody.chest.hasFuckableNipples()) outputText("slide inside your nipple-cunts and finger your chest pussies in a small approximation of what's happening below.", false);
