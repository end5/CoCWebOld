...
//Requires pussy or cock small enough for harpy!
		private function harpyLossLust():void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina() && x < 0) {
				...
				if (player.skinType == SKIN.PLAIN) outputText(" and quickly alleviating the pain by brushing their wings across your hide", false);
				...
				if (player.skinType == SKIN.PLAIN) outputText(", always followed by a soothing brush of their feathers", false);
				...
				if (player.skinType == SKIN.PLAIN) outputText(" across your reddening skin", false);
			...
			//(Reminds me of what my own Succubus does in my novel)
				...
				if (player.lowerBody.balls > 0) outputText("You can feel her little tongue rolling across your balls and the base of your ", false);
...
//No genderless folks.
		private function harpyDamageLoss():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock() && x >= 0) {
				...
				if (!player.lowerBody.vaginaSpot.hasVagina()) {
					...
					if (player.lowerBody.balls > 0) outputText(", craning their necks to slurp and massage your " + ballsDescriptLight() + " eagerly, trying to coax up your cum production as much as they can", false);
					...
					//(If breasts)
					if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("her petite bosom grinding across your " + player.allBreastsDescript() + ", her nipples tickling at your " + nippleDescript(0) + ", ", false);
				...
				//[(Herm)
					...
					//(If balls)
					if (player.lowerBody.balls > 0) outputText("their hands moving up to grope and fondle your " + ballsDescriptLight() + ", massaging them to boost your cum production as much as they can, ", false);
					...
					//(If breasts)
					if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText(", causing your " + player.allBreastsDescript() + " to bounce and sway violently", false);
					outputText(".  She slowly leans down, her eyes locked on yours, her lips glinting a malicious gold", false);
					//(if breasts)
					if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText(" her toned chest and pert breasts squishing your " + breastDescript(0) + " beneath them", false);
					outputText(".\n\n", false);
				}
				//Apply harpy status.
			...
			//[(Female) 
				...
				//(If breasts)
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) outputText(" and causing your " + player.allBreastsDescript() + " to jiggle, slapping into one another", false);
				outputText(". She leans slowly over you, her gilded lips glistening with saliva as they approached yours", false);
				//{(If breasts)
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText(", her petite chest squashing your " + breastDescript(0) + " against hers", false);
				outputText(".\n\n", false);
			...
			if (x >= 0 && player.lowerBody.cockSpot.count() == 1) {
				...
				if (player.lowerBody.balls > 0) outputText(", her huge feathery bottom coming to rest on your " + ballsDescriptLight(), false);
				...
				else if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText(" while your " + player.allBreastsDescript() + " are shaking and jiggling around helplessly.", false);
				else outputText(".", false);
			...
			//[(Female)
				...
				//{(If breasts)
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("her belly squishing your " + player.allBreastsDescript() + " beneath her weight, ", false);
		...
		private function victoryHarpyGetsHerPussyRaped():void
		{
			...
			//(If balls)
			if (player.lowerBody.balls > 0) outputText("You eventually bottom out, your " + ballsDescriptLight() + " coming to rest between her powerful, gelatinous thighs, her soft feathers tickling at your " + sackDescript() + " maddeningly.\n\n", false);
			//(If balls-less) 
			...
			outputText(".  ", false);
			//{(If balls)
			if (player.lowerBody.balls > 0) outputText("Your " + sackDescript() + " slaps wetly into her immense, wobbling thighs.  ", false);
		...
		private function winAndRapeHarpyAnally():void
		{
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText(" Her petite bust squishes against your " + player.allBreastsDescript() + ". Her downy tits tickle, and grind against your chest maddeningly, tickling and scratching them gently as the lust-fueled bird-girl seems to ache for you.", false);
			outputText("\n\n", false);
			...
			//(if balls)
			if (player.lowerBody.balls > 0) outputText("  Your " + ballsDescriptLight() + " come to rest against her thick, wiggly rear.", false);
			outputText("\n\n", false);
			...
			//[(Herms/Shemales) 
				...
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText(", the motions causing your " + player.allBreastsDescript() + " to bounce and sway wildly, slapping into one another lewdly, adding to your growing lust", false);
				...
				if (player.gender == 3) {
					...
					//{(If balls)
					if (player.lowerBody.balls > 0) outputText("  She even makes your " + ballsDescript() + " slap against your " + vaginaDescript(0) + ", a hot, moist SPLAT accompanying every tiny motion the harpy makes.", false);
				}
				outputText("\n\n", false);
			}
			...
			//(if breasts)
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("her chest mashing against yours as she becomes still, ", false);
		...
		private function WinOnHarpyAndOralRape():void
		{
			...
			//(if boobs)
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText(", but soon she presses her toned, petite chest against your " + player.allBreastsDescript() + ", that stubby tongue tangling around yours lustily.", false);
			...
			if (player.lowerBody.cockSpot.count() == 1) {
				...
				//(If balls) 
				if (player.lowerBody.balls > 0) outputText("  Her hands creep down to softly squeeze and massage your " + ballsDescriptLight() + ", the harpy's eager head bobbing back and forth along the sweet tasting shaft of your " + cockDescript(0) + ", sending waves of powerful, overwhelming pleasure resonating through your body.", false);
				outputText("\n\n", false);
			}
			//[(Male, Shemale, Herm, multicock)
				...
				//(If balls)
				if (player.lowerBody.balls > 0) outputText("  She moves that hand to softly squeeze and pet at your " + ballsDescriptLight() + ", massaging those bloated cum-factories with expertise. You can feel a distinct rumble as your " + sackDescript() + " really ups its production, almost feeling heavier.", false);
				//(If herm)
			...
			if (player.gender == 2) {
				...
				//(If breasts)
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("You grasp onto your " + player.allBreastsDescript() + " violently, kneading and tweaking your " + nippleDescript(0) + " in a horny haze as the harpy's fingers tease around your pussy.  ", false);
			...
			if (player.lowerBody.cockSpot.count() == 1) {
				...
				//(If balls) 
				if (player.lowerBody.balls > 0 && player.lowerBody.cockSpot.list[0].cockLength <= 16) outputText("She even deep throats you, taking your length into her expertly-controlled mouth, that tongue creeping out to tickle at your " + ballsDescriptLight() + ".  ", false);
			...
			if (player.lowerBody.cockSpot.count() > 1) {
				...
				//(if balls)
				if (player.lowerBody.balls > 0) outputText(", dribbling all over your " + sackDescript(), false);
...
//{New page}
		...
		private function spoidahsLegEggsInHarpeis():void
		{
			...
			if (player.gender > 0) {
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" your [vagina]");
