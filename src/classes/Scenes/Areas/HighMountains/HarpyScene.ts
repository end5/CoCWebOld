...
//*If genderless, rape options are Finger Her*
		public harpyVictoryuuuuu():void
		{
			...
			if (monster.HP < 1) Render.text("The harpy screams out in one last, pained cry before her wings give way, the feathered woman collapsing into a weary heap.", true);
			//(Enemy defeated by lust)
			else Render.text("The harpy can't contain her lust anymore and crumples to the ground before you, on her knees with her plush, heavy ass resting on her feet. She coos pathetically, with one hand between her legs furiously fingering herself, and the other pressed against your crotch, a needy look in her eyes.", false);
			...
			if (player.canOvipositSpider() && (player.faceType == FACE.SNAKE_FANGS || player.faceType == FACE.SPIDER_FANGS)) eggs = spoidahsLegEggsInHarpeis;
			let anal:Function =null;
			let pussy:Function =null;
			let scissor:Function =null;
			let clitFuck:Function =null;

			if (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.vaginaSpot.list[0].clitLength >= 3.5) clitFuck = clitFuckAHarpy;
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				if (player.isNaga()) Render.text("  If you weren't a naga, you could scissor her.");
		...
		public harpyLossU():void
		{
			...
			//No fitu
				...
				Render.text("Though you've been defeated by the harpy, it doesn't seem she wants anything to do with you, and gives you a whack upside your head before departing.", true);
				cleanupAfterCombat();
			}
		}
		...
		{
			Render.text("", true);
			Render.text("The triumphant harpy looks down at your goopy form, ready to take you.  She seems a little confused though, and begins poking at your gelatinous body.\n\n", false);

			Render.text("\"<i>Penis?</i>\" she eventually asks.\n\n", false);

			Render.text("You tell her no.\n\n", false);

			Render.text("\"<i>Pussy?</i>\"\n\n", false);

			Render.text("Nope.\n\n", false);

			Render.text("\"<i>Oh...</i>\"\n\n", false);

			Render.text("She grabs a handful of you and releases it, watching your body jiggle about. This seems to amuse her greatly and she plunges both hands inside you then shakes them about. Retracting them, she takes a few steps back and gives you an evil grin, then lets out a blood-curdling shriek.\n\n", false);

			Render.text("Two more harpies flap down beside her and the trio huddles together, periodically pointing at you.  Most of what they're saying is inaudible to you, though you're fairly sure you heard 'goop' somewhere in there.  You're not given much time to stew on it though, as they seem to agree on something and approach.\n\n", false);

			Render.text("The smallest of the bunch walks right up to you and punches your chest, laughing and chittering as your body undulates in response.  The other two stick their arms inside you and begin shaking them about, also laughing.  This goes on for quite a while, with the bird women trying various positions and levels of force (including a particularly odd one in which two attempted to shove your entire body up the third's vagina; not exactly a situation you want to find yourself in again...). Eventually they get bored of you and flap off, leaving your exhausted body in a pile until you get enough energy to pull yourself together and head back.", false);
		}
...
//No genderless folks.
		private harpyDamageLoss():void
		{
			...
			Render.text("You collapse in a broken heap, gasping for breath as your wounds sting and throb, a testament to the sharpness of the harpy's claws. Turning your aching head to look at the ever-circling harpy, you see her mad-looking eyes narrow as she gains height, flapping her two pairs of wings frantically.\n\n", false);

			Render.text("She lets out a psychotic cry that seems to echo throughout the area around you, carrying into the hills and back again. You could swear that more cries come back than the one that started the rolling echo, but before you can dwell on this, the crazed bird-girl dives at you with incredible speed, her thick frame colliding with you. You are sent spinning and rolling across the ground, your wounds singing a hymn of pain and weariness. After a few complex moments, the pair of you are dumped in a heap on the floor, with the horny harpy clawing brutally at your " + player.armorName + ", tearing it away from your body in no time at all.\n\n", false);

			//[(Male, shemale, herm)
			if (player.lowerBody.cockSpot.hasCock() && x >= 0) {
				Render.text("As her crazed, brutal hands clutch at every part of your body, grasping every ounce of flesh they can reach, your hazed, weary mind seemed to be receiving more signals than you thought, more hands clawing at you, more crazed avian cries. Your vision swims from the pain and, as you look down, conscious for just a few seconds, ", false);
				if (player.lowerBody.cockSpot.count() > 1) Render.text("each of ", false);
				Render.text("your " + multiCockDescriptLight() + " flopping from side to side from the ministrations, you see not one, but THREE deranged, randy bird-girls, all squabbling over your genitals.\n\n", false);

				//[(Male/shemale)
				if (!player.lowerBody.vaginaSpot.hasVagina()) {
					Render.text("The hierarchy is eventually settled, it seems, because two of those heavy-hipped bird babes wrap their lips around your " + cockDescript(x) + ", sucking and pecking at the hot flesh", false);
					if (player.lowerBody.balls > 0) Render.text(", craning their necks to slurp and massage your " + ballsDescriptLight() + " eagerly, trying to coax up your cum production as much as they can", false);
					Render.text(". The third, estranged harpy leaps at your torso, straddling your navel with her weighty rear, knocking the breath out of you and leaning down, ", false);
					//(If breasts)
					if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) Render.text("her petite bosom grinding across your " + player.allBreastsDescript() + ", her nipples tickling at your " + nippleDescript(0) + ", ", false);
					Render.text("her golden lips glinting in the light as they make for your own.\n\n", false);
				}
				//[(Herm)
				else {
					Render.text("The hierarchy is settled between the warring bird-girls, and two of them grasp onto your thighs with their clawed hands, those gold-lipped mouths starting to hungrily suckle and taste every inch of your " + cockDescript(x) + ", their squawking, feathered heads occasionally diving down to probe their short, stubby tongues into your " + vaginaDescript(0) + ", sending sharp spikes of pleasure rippling through your body, ", false);
					//(If balls)
					if (player.lowerBody.balls > 0) Render.text("their hands moving up to grope and fondle your " + ballsDescriptLight() + ", massaging them to boost your cum production as much as they can, ", false);
					Render.text("tiny tingles left on your body at every spot their gilded lips caress. The third harpy dives at your chest, her thick hips SLAMMING into your stomach, knocking the breath out of you", false);
					//(If breasts)
					if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) Render.text(", causing your " + player.allBreastsDescript() + " to bounce and sway violently", false);
					Render.text(".  She slowly leans down, her eyes locked on yours, her lips glinting a malicious gold", false);
					//(if breasts)
					if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) Render.text(" her toned chest and pert breasts squishing your " + breastDescript(0) + " beneath them", false);
					Render.text(".\n\n", false);
				}
				//Apply harpy status.
		...
		private victoryHarpyGetsHerPussyRaped():void
		{
			...
			Render.text("Once your flow of seed subsides, you pull free of the fat-reared harpy.  She collapses into an orgasm-wracked pile on the ground, her plump ass and tender thighs waving in the air for whomever comes along after you.  You wipe yourself down and continue on your way, pleased with the brutal fuck and looking forward to your NEXT encounter...", false);
		...
		private winAndRapeHarpyAnally():void
		{
			...
			Render.text("Grinning to yourself and stripping free of your " + player.armorName + ", you lean down and grasp the harpy's pointed chin, turning it from side to side, your eyes roaming across her sharp, beautiful features as you inspect your prize. Her body just begs to be touched!  You gaze at those marvelous, tickly feathers coating her slender chest and those awe-inspiring hips, each supporting a fat, jiggling buttock that's nearly as large as a centaur's whole rear!  Without hesitation, the stricken harpy raises herself and starts to press that sweet, feathery body against yours, squawking and crooning pathetically.", false);
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) Render.text(" Her petite bust squishes against your " + player.allBreastsDescript() + ". Her downy tits tickle, and grind against your chest maddeningly, tickling and scratching them gently as the lust-fueled bird-girl seems to ache for you.", false);
			Render.text("\n\n", false);

			Render.text("Not wishing to keep her fantastic butt waiting, you grasp onto her shoulders and pull her back with you, laying back against the hard ground with the squawking avian slut grinding and bouncing her bean-bag ass against your " + multiCockDescriptLight() + ".  Blood flows through your " + cockDescript(x) + " as it is sandwiched between her hot, caressing, luxurious ass cheeks, and you grind the head brutally against her impossibly tight pucker, which seems to make her panic a bit. She starts squawking and flapping her arm-wings in a vain attempt to escape your grasp, and the imminent butt-fucking she is about to receive! You smirk to yourself as the fluids from her womanhood drool and spill over your entrapped " + cockDescript(x) + ", your slowly-pumping " + hipDescript() + " smearing her own sticky sexual juices over those downy butt cheeks.\n\n", false);

			Render.text("The bird-girl's attempts to escape only turn you on more, and you grab HARD onto those gelatinous orbs, guiding your " + cockDescript(x) + " into her gloriously tight and hot rear entrance; it feels harsh and rough, practically scratching every inch of your member as it's driven hard against her resisting inner walls.  She's far tighter and hotter than any human girl could possibly be! Your " + cockDescript(x) + " pushes so viciously and painfully into her clenching tight depths that the harpy squeals and cries out, trying to fly away from you again.  However, she only succeeds in clenching and waving her enormous ass, sending viciously sharp sensations through your blood-engorged dick. Inch by inch you force your " + cockDescript(x) + " into her, her movements getting more frantic as you plumb her most lustful and forbidden hole.  Finally, you bottom out and stop for breath, her powerful, jiggling thighs pressing against yours.", false);
			//(if balls)
			if (player.lowerBody.balls > 0) Render.text("  Your " + ballsDescriptLight() + " come to rest against her thick, wiggly rear.", false);
			Render.text("\n\n", false);

			Render.text("The harpy girl seems to freeze above you, wincing and trilling in pain. You are bordering on pain yourself; her crushing ass is so tight and so hot it nearly burns your " + cockDescript(x) + "! You smirk and raise your hand, before bringing it down onto her blubbery feminine rear.  With a loud, ringing slap, her ass jiggles as you scream \"<i>Giddy-up!</i>\"\n\n", false);

			//[(Males, no breasts)
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating < 1 && !player.lowerBody.vaginaSpot.hasVagina()) {
				Render.text("Her pupils constrict, and the feathery fat-reared fowl squawks loudly, going completely crazy atop you! She writhes and bounces this way and that, growing more and more desperate and insane as her pain increases.  Your " + cockDescript(x) + " slams this way and that into her rough, gripping walls, as moans and grunts escape your lips. She flaps frantically from side to side, almost slipping her clutching rump free of your " + cockDescript(x) + " before you grasp hard onto her meaty thighs, forcing yourself back into the distressed harpy with a lewd grin.  It is clear that some part of her is loving this brutal intrusion; her feminine fluids are spilling free, coating your torso with her strong scent.\n\n", false);
			}
			//[(Herms/Shemales) 
			else {
				Render.text("Her pupils constrict, and the feathery fat-reared fowl squawks loudly, going completely crazy atop you! Struggling to escape from your impaling shaft, she drives her clutching, ribbed inner walls into the sides of your throbbing dick, her gelatinous ass wobbling this way and that", false);
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) Render.text(", the motions causing your " + player.allBreastsDescript() + " to bounce and sway wildly, slapping into one another lewdly, adding to your growing lust", false);
				Render.text(". You keep a firm grip on her rump and thighs, ensuring that every time she nearly wriggles her hot anus free of your cock, you can drive your hot meat straight back into her lush depths, laughing at her helplessness.", false);
				//(If herm)
				if (player.gender == 3) {
					Render.text("  Her struggles are driving you wild, and your " + vaginaDescript(0) + " is seeping fluids, growing more and more aroused as she bounces and slamming that immense, rippling rear into you, the force squishing your moist lips together, releasing yet more sticky juices and adding an extra little tone of pleasure.", false);
					//{(If balls)
					if (player.lowerBody.balls > 0) Render.text("  She even makes your " + ballsDescript() + " slap against your " + vaginaDescript(0) + ", a hot, moist SPLAT accompanying every tiny motion the harpy makes.", false);
				}
				Render.text("\n\n", false);
			}
			Render.text("With an ear-splitting squawk, the impaled harpy's drooling pussy absolutely explodes.  It fires huge streams of her sticky feminine cum onto your ever-thrusting " + hipDescript() + ", her muscles locking in ecstasy, before the exhausted bird-girl collapses over you, ", false);
			//(if breasts)
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) Render.text("her chest mashing against yours as she becomes still, ", false);
			Render.text("submitting to your will. Growling, you start to fuck her in earnest and grab a handful of her squishy rear in each hand, pounding her up and down on your " + cockDescript(x) + ". Though the harpy has collapsed, her body is still panicking, forcing her rough inner passage to clutch and massage your shaft, feeling harsh and grating, but oh-so wonderful! Pathetic squawks and trills of pain escape her golden lips as you roughly pound her tender ass, and her hips pound back against you, ass bouncing on your lap with a dull SLAP every time, which spatters pre-cum across your legs as well as her enormous thighs.\n\n", false);

			Render.text("Throwing your head back and crying out in sheer lust-consumed ecstasy, you hilt your " + cockDescript(x) + " deep inside her thick, tight anus, releasing a flood of hot, sticky seed into her butt. The sudden surge of cum into her rough depths causes the exhausted harpy to tense up once more, and she releases a second, more pathetic wave of her fluids, a little less than last time. Her creamy feminine cum leaks out over your thighs as you lock with her. Your eyes roll back from the intensity of your orgasm, and you bite down on your bottom lip; for what seems like weeks you cum into her plump rump, her hands pressed on your chest as if trying to push you away. Eventually, your flow subsides, and the harpy collapses on top of you, sound asleep.\n\n", false);

			Render.text("Still leaking seed, you pull free of her gaping ass, and dump the worthless bird-slut on the floor in a heap. Passed out and woefully exposed, her lush holes await whatever horny beast or demon will come alone after you.", false);
			cleanupAfterCombat();
			player.orgasm();
		}
		...
		private WinOnHarpyAndOralRape():void
		{
			...
			Render.text("You stand threateningly over the helpless harpy, stroking yourself through your " + player.armorName + " before grasping onto her chin, turning it this way and that, inspecting your prize thoroughly. Those golden lips glisten in the light as she trills feebly, her eyes looking soulfully up at you.\n\n", false);

			Render.text("Grinning lewdly you squeeze her cheeks, the stricken harpy opening her mouth obediently. Her tongue is short and stubby like that of a normal bird, glistening with saliva and looking terribly inviting. You strip off the lower portion of your " + player.armorName, false);
			//(if pussy)
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				Render.text(", fluids seeping from your plush " + vaginaDescript(0) + " all the while", false);
			}
			//(if cock)
		...
		{
			clearOutput();
			Render.text("You stare down at the defeated harpy, watching as ");
			if (monster.HP < 1) Render.text("her arms tremble from the effort of trying to keep her body from collapsing to the floor.");
			else Render.text("she leans back from her kneeling position, spreading her legs and digging her fingers into her soaking cunt. Her other hand darts up to her ample bosom, giving it a hard squeeze before starting to tease her erect nipple, pinching and flicking as she continues to fingerfuck herself ever more forcefully.");
			...
			if (monster.HP < 1) Render.text("vulnerable");
			else Render.text("lust-mad");
			Render.text(" harpy.");
			...
			if (monster.HP < 1) Render.text("worry");
			else Render.text("desire");
			Render.text(", but before she can even begin to talk you grab the mess of short hair atop her head and yank her forwards, roughly grinding her face into your soaked pussy[if (isHerm = true) \", leaving your stiff cock resting on her head where the stimulation from her feathery hair causes it to steadily drip pre-cum down the back of her neck\"].  She struggles furiously, her hands shooting up to your thighs as she tries to push away from your slick, aching entrance.  Your grip is far too strong and she's too exhausted from the fight though, and you demonstrate the pointlessness of her attempts at freedom by slowly dragging her face up and down against your [vagina], smearing even more femcum over her increasingly distraught visage.");
...
//{New page}
		...
		private spoidahsLegEggsInHarpeis():void
		{
			...
			Render.text("Perched above the downed bird-bitch, you look her over as you weigh up your options.  Your eyes slide across her petite breasts, tracing her curves until your gaze rests upon her flared hips and curvaceous ass.  Unfortunately your appreciation of the finer things in life is interrupted by the feather-brained eyecandy; she simply will not stop squawking.");
			...
			if (player.tallness > 78) Render.text("up");
			else if (player.tallness == 78) Render.text("forward");
			else Render.text("down");
			Render.text(", eyes closed and lips puckered seductively, ready to deliver a heady dose of her drugged lipstick.");
...
//Fuck a harpy with the players throbbing clit.
		...
		{
			clearOutput();
			Render.text("Eyeing the hapless bird-woman up and down, you feel your lusts kindling.  Moisture seeps from your [vagina] as you watch the harpy ");
			if (monster.lust > 99) Render.text("moan and masturbate, dipping her fingers into her honey-pot with wild abandon.");
			else Render.text("moan and struggle to rise, jiggling in the cutest way each time she slumps down, defeated.");
			Render.text("  You wrench her thick thighs apart to get a better view of her avian snatch.  Her lips are shrouded in a downy, feathery fuzz, but you can easily make out the heavy, lust-glossed labia.  The thought of tasting her dripping honey makes your mouth water and inside your [armor], your [clit] has emerged from its hood, straining as blood rushes to your loins.   The sensitive organ is so swollen that it's more lady-cock than joy-buzzer, the thick hood resembling an animal's sheath more than a natural flesh fold.");
			...
			if (player.wetness() < 3) Render.text("  Runnels of your lubricant are dripping down your [legs] in streams,");
			else if (player.wetness() < 4) Render.text("  Streams of lubricant are soaking down your [legs] in waves,");
			else Render.text("  Squirts of lubricant are gushing down your [legs] to puddle on the floor,");
			Render.text(" evidence of just how hungry your clenching, empty hole is.");
			...
			if (player.wetness() < 3) Render.text("  A fresh gush of fluid washes out of your pussy at the peak of your pleasure even though you're far from a squirter");
			else if (player.wetness() < 4) Render.text("  A fresh spout of girlspunk washes out of your pussy at the peak of your pleasure");
			else Render.text("  A torrential splash of girlspunk shoots out of your pussy in a river at the peak of your pleasure");
			Render.text(", timed with a full-body seizure that causes you and your partner to slump down into sticky pile.");
			...
			if (player.tou / 20 + rand(20) < 6) {
				Render.text(" your endurance gives out, and you slip into unconsciousness.  This time it seems you met your sexual match.");
				monster.lust = 98;
				monster.HP = 2;
				player.lust = 100;
				flags[FlagEnum.COMBAT_BONUS_XP_VALUE] = monster.XP;
				...
				Render.text(" the harpy's eyes close, and she slips into unconsciousness.  You give her round bottom a little pat before you depart, still shivering from the cascades of pleasure it brought you.");
				player.orgasm();
				dynStats("sen", -1);
				cleanupAfterCombat();
			}
