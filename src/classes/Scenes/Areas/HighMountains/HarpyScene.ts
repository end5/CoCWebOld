...
//*If genderless, rape options are Finger Her*
		public harpyVictoryuuuuu():void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				if (player.isNaga()) MainScreen.text("  If you weren't a naga, you could scissor her.");
...
//Requires pussy or cock small enough for harpy!
		private harpyLossLust():void
		{
			...
			//if long hair
			if (player.hairLength > 12) MainScreen.text(" and moves it out of your face", false);
			...
			if (player.lowerBody.vaginaSpot.hasVagina() && x < 0) {
				...
				if (player.skinType == SKIN.PLAIN) MainScreen.text(" and quickly alleviating the pain by brushing their wings across your hide", false);
				...
				if (player.skinType == SKIN.PLAIN) MainScreen.text(", always followed by a soothing brush of their feathers", false);
				MainScreen.text(". They line the curve of your " + breastDescript(0) + ", brush around your aurela, and harden your " + nippleDescript(0) + "s. They trace your inner thigh, and not even your " + player.feet() + " are safe. It does not tickle, but your nerves are lit up like a Christmas tree and a tear wells up in your eyes as you're forced to lie still in the face of such a soft, yet overpowering sensation.\n\n", false);

				MainScreen.text("Suddenly, there's a different feeling altogether. You look down and through the feathered fans, spot one of the harpies with her face between your spread legs. She looks at you, a sultry expression in her black-yellow eyes as her black, stout tongue runs up and down your " + vaginaDescript(0) + ". You close your eyes and focus on that, instead of the maddening sensation of the rolling waves of talons", false);
				if (player.skinType == SKIN.PLAIN) MainScreen.text(" across your reddening skin", false);
				MainScreen.text(". Her little black tongue feels divine as it bumps up against your " + clitDescript() + ". WHen you let out a small, alleviated coo, the other harpies once again follow suit. The two previously holding your arms, bend down and put their stiff lips on top of your antagonized nipples, rewarding them after having brushed past them so many times.\n\n", false);

				MainScreen.text("You feel your " + player.legs() + " being lifted up and out of the way. Feathered arms curl around either side of your thighs to keep them in place as the harpy teasing and suckling your " + clitDescript() + " gives the last harpy enough room to scoot in below her. A teasing of soft harpy hair feathers against your " + buttDescript() + " is the only warning you receive before you feel the wet warmth of that final black tongue pressing against your " + assholeDescript() + ".\n\n", false);
			...
			//(Reminds me of what my own Succubus does in my novel)
				...
				MainScreen.text(cockDescript(x) + " whenever the one on top of you doesn't take you up to the hilt.\n\n", false);
...
//No genderless folks.
		private harpyDamageLoss():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock() && x >= 0) {
				...
				MainScreen.text("your " + multiCockDescriptLight() + " flopping from side to side from the ministrations, you see not one, but THREE deranged, randy bird-girls, all squabbling over your genitals.\n\n", false);

				//[(Male/shemale)
				if (!player.lowerBody.vaginaSpot.hasVagina()) {
					MainScreen.text("The hierarchy is eventually settled, it seems, because two of those heavy-hipped bird babes wrap their lips around your " + cockDescript(x) + ", sucking and pecking at the hot flesh", false);
					if (player.lowerBody.balls > 0) MainScreen.text(", craning their necks to slurp and massage your " + ballsDescriptLight() + " eagerly, trying to coax up your cum production as much as they can", false);
					...
					//(If breasts)
					if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text("her petite bosom grinding across your " + player.allBreastsDescript() + ", her nipples tickling at your " + nippleDescript(0) + ", ", false);
				...
				//[(Herm)
					...
					//(If balls)
					if (player.lowerBody.balls > 0) MainScreen.text("their hands moving up to grope and fondle your " + ballsDescriptLight() + ", massaging them to boost your cum production as much as they can, ", false);
					...
					//(If breasts)
					if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) MainScreen.text(", causing your " + player.allBreastsDescript() + " to bounce and sway violently", false);
					MainScreen.text(".  She slowly leans down, her eyes locked on yours, her lips glinting a malicious gold", false);
					//(if breasts)
					if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) MainScreen.text(" her toned chest and pert breasts squishing your " + breastDescript(0) + " beneath them", false);
					MainScreen.text(".\n\n", false);
				}
				//Apply harpy status.
			...
			if (x >= 0 && player.lowerBody.cockSpot.count() == 1) {
				...
				if (player.lowerBody.balls > 0) MainScreen.text(", her huge feathery bottom coming to rest on your " + ballsDescriptLight(), false);
				...
				if (player.lowerBody.balls >= 0) {
					MainScreen.text(".  Your " + ballsDescriptLight() + " bounce with such force as to fetch the cock-speared harpy a stinging slap on the rear with every brutal, mechanical fuck", false);
					//{(If herm)
					if (player.gender == 3) MainScreen.text(" before swinging back to spanking your " + vaginaDescript(0) + " wetly.", false);
					else MainScreen.text(".", false);
				}
				else if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) MainScreen.text(" while your " + player.allBreastsDescript() + " are shaking and jiggling around helplessly.", false);
				else MainScreen.text(".", false);
			...
			//[(Female)
				...
				//{(If lactating)
				if (player.biggestLactation() >= 1) MainScreen.text("You hear some subtle slurps around the big feathery earmuffs crushing your head, and feel your " + breastDescript(0) + " begin to drain, sending motherly shivers through your whole being.  The monster-girls drink and drink, those stubby tongues circling each nipple while the drugged golden lips make you tingle sensually.  ", false);
				MainScreen.text("Their hands find their way to your " + vaginaDescript(0) + ", and two sets of fingers start to explore your lush, velvety depths.  They seem intent on making their captive quiver and scream in ecstasy beneath the stifling birdy bottom. The riding harpy leans down, ", false);
				//{(If breasts)
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) MainScreen.text("her belly squishing your " + player.allBreastsDescript() + " beneath her weight, ", false);
			...
			if (x >= 0 && player.lowerBody.cockSpot.hasCock()) {
				...
				//{(If herm)
				if (player.gender == 3) MainScreen.text(", and the number of times your " + vaginaDescript(0) + " has dripped your sweet feminine cum over your thighs", false);
			...
			//[(Female)
				...
				//{(If lactating)
				if (player.biggestLactation() >= 1) MainScreen.text("  Their bellies seem a little pudgier than when they'd started, a result of the endless drinking from your " + nippleDescript(0) + "s.  ", false);
			...
			if (x >= 0 && player.lowerBody.cockSpot.hasCock()) {
				...
				MainScreen.text("your " + multiCockDescriptLight() + " shakes and quivers under the attentions of the merciless bird-girls, releasing a powerful blast of hot, sticky seed, coating the three cruel harpies.  They're still rubbing and probing their pussies with delicate fingers while you cum. You hear some angry squawking and a few syllables of recognizable language before you finally give in to unconsciousness, fading into a warm dream of soft, velvet thighs...\n\n", false);
			}

			MainScreen.text("After a few hours, when you wake, every muscle in your body is aching as though you've just run a marathon. Looking down at your " + player.legs() + " in a weary haze, you see signs that even after you'd blacked out, your body had continued to be abused by the three lust-crazed harpies.", false);
			fatigue(20);
			cleanupAfterCombat();
			player.orgasm();
		...
		private victoryHarpyGetsHerPussyRaped():void
		{
			...
			//(If balls)
			if (player.lowerBody.balls > 0) MainScreen.text("You eventually bottom out, your " + ballsDescriptLight() + " coming to rest between her powerful, gelatinous thighs, her soft feathers tickling at your " + sackDescript() + " maddeningly.\n\n", false);
			//(If balls-less) 
			else MainScreen.text("Leaning low over her back, you adjust to drive yourself ever deeper, forcing the whole length of your " + cockDescript(x) + " into her, the soft down on her gelatinous thighs prickling at your thighs maddeningly.\n\n", false);

			if (player.gender == 1) {
				//(Male, no breasts)
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) {
					MainScreen.text("Taking care not to waste your opportunity with an immense butt by cumming too soon, you slowly start to push and pull her fat, quivering cheeks up and down your " + cockDescript(x) + ", feeling her rough walls and clutching, gasping convulsions massage your manhood", false);
					if (player.totalCocks() > 1) MainScreen.text("s", false);
					MainScreen.text(" along ", false);
					if (player.totalCocks() > 1) MainScreen.text("their", false);
			...
			MainScreen.text("It seems the exposed girl is enjoying herself just as much.  Every tiny motion of your " + hipDescript() + " causes her to writhe and moan, her moans punctuated with birdlike squawks as she pushes her huge egg-bearing hips back at you in sheer, undisguised desire to have you fuck her harder and deeper.\n\n", false);

			MainScreen.text("You decide to give her what she so clearly wants and plant both your hands onto her massive bottom, squashing and groping it around your impaling " + multiCockDescriptLight() + " as you speed up the pace of your thrusts, driving the full length of your " + cockDescript(x), false);
			if (player.totalCocks() > 1) MainScreen.text(" and " + cockDescript(y), false);
			MainScreen.text(" into her grinding, overwhelming hole", false);
			if (player.totalCocks() > 1) MainScreen.text("s", false);
			MainScreen.text(".  ", false);
			//{(If balls)
			if (player.lowerBody.balls > 0) MainScreen.text("Your " + sackDescript() + " slaps wetly into her immense, wobbling thighs.  ", false);
		...
		private winAndRapeHarpyAnally():void
		{
			...
			MainScreen.text("Grinning to yourself and stripping free of your " + player.armorName + ", you lean down and grasp the harpy's pointed chin, turning it from side to side, your eyes roaming across her sharp, beautiful features as you inspect your prize. Her body just begs to be touched!  You gaze at those marvelous, tickly feathers coating her slender chest and those awe-inspiring hips, each supporting a fat, jiggling buttock that's nearly as large as a centaur's whole rear!  Without hesitation, the stricken harpy raises herself and starts to press that sweet, feathery body against yours, squawking and crooning pathetically.", false);
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) MainScreen.text(" Her petite bust squishes against your " + player.allBreastsDescript() + ". Her downy tits tickle, and grind against your chest maddeningly, tickling and scratching them gently as the lust-fueled bird-girl seems to ache for you.", false);
			MainScreen.text("\n\n", false);

			MainScreen.text("Not wishing to keep her fantastic butt waiting, you grasp onto her shoulders and pull her back with you, laying back against the hard ground with the squawking avian slut grinding and bouncing her bean-bag ass against your " + multiCockDescriptLight() + ".  Blood flows through your " + cockDescript(x) + " as it is sandwiched between her hot, caressing, luxurious ass cheeks, and you grind the head brutally against her impossibly tight pucker, which seems to make her panic a bit. She starts squawking and flapping her arm-wings in a vain attempt to escape your grasp, and the imminent butt-fucking she is about to receive! You smirk to yourself as the fluids from her womanhood drool and spill over your entrapped " + cockDescript(x) + ", your slowly-pumping " + hipDescript() + " smearing her own sticky sexual juices over those downy butt cheeks.\n\n", false);

			MainScreen.text("The bird-girl's attempts to escape only turn you on more, and you grab HARD onto those gelatinous orbs, guiding your " + cockDescript(x) + " into her gloriously tight and hot rear entrance; it feels harsh and rough, practically scratching every inch of your member as it's driven hard against her resisting inner walls.  She's far tighter and hotter than any human girl could possibly be! Your " + cockDescript(x) + " pushes so viciously and painfully into her clenching tight depths that the harpy squeals and cries out, trying to fly away from you again.  However, she only succeeds in clenching and waving her enormous ass, sending viciously sharp sensations through your blood-engorged dick. Inch by inch you force your " + cockDescript(x) + " into her, her movements getting more frantic as you plumb her most lustful and forbidden hole.  Finally, you bottom out and stop for breath, her powerful, jiggling thighs pressing against yours.", false);
			//(if balls)
			if (player.lowerBody.balls > 0) MainScreen.text("  Your " + ballsDescriptLight() + " come to rest against her thick, wiggly rear.", false);
			MainScreen.text("\n\n", false);
			...
			//[(Herms/Shemales) 
				...
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) MainScreen.text(", the motions causing your " + player.allBreastsDescript() + " to bounce and sway wildly, slapping into one another lewdly, adding to your growing lust", false);
				...
				//(If herm)
				if (player.gender == 3) {
					MainScreen.text("  Her struggles are driving you wild, and your " + vaginaDescript(0) + " is seeping fluids, growing more and more aroused as she bounces and slamming that immense, rippling rear into you, the force squishing your moist lips together, releasing yet more sticky juices and adding an extra little tone of pleasure.", false);
					//{(If balls)
					if (player.lowerBody.balls > 0) MainScreen.text("  She even makes your " + ballsDescript() + " slap against your " + vaginaDescript(0) + ", a hot, moist SPLAT accompanying every tiny motion the harpy makes.", false);
				}
				MainScreen.text("\n\n", false);
			}
			MainScreen.text("With an ear-splitting squawk, the impaled harpy's drooling pussy absolutely explodes.  It fires huge streams of her sticky feminine cum onto your ever-thrusting " + hipDescript() + ", her muscles locking in ecstasy, before the exhausted bird-girl collapses over you, ", false);
			//(if breasts)
			...
			MainScreen.text("submitting to your will. Growling, you start to fuck her in earnest and grab a handful of her squishy rear in each hand, pounding her up and down on your " + cockDescript(x) + ". Though the harpy has collapsed, her body is still panicking, forcing her rough inner passage to clutch and massage your shaft, feeling harsh and grating, but oh-so wonderful! Pathetic squawks and trills of pain escape her golden lips as you roughly pound her tender ass, and her hips pound back against you, ass bouncing on your lap with a dull SLAP every time, which spatters pre-cum across your legs as well as her enormous thighs.\n\n", false);

			MainScreen.text("Throwing your head back and crying out in sheer lust-consumed ecstasy, you hilt your " + cockDescript(x) + " deep inside her thick, tight anus, releasing a flood of hot, sticky seed into her butt. The sudden surge of cum into her rough depths causes the exhausted harpy to tense up once more, and she releases a second, more pathetic wave of her fluids, a little less than last time. Her creamy feminine cum leaks out over your thighs as you lock with her. Your eyes roll back from the intensity of your orgasm, and you bite down on your bottom lip; for what seems like weeks you cum into her plump rump, her hands pressed on your chest as if trying to push you away. Eventually, your flow subsides, and the harpy collapses on top of you, sound asleep.\n\n", false);
		...
		private WinOnHarpyAndOralRape():void
		{
			...
			MainScreen.text("You stand threateningly over the helpless harpy, stroking yourself through your " + player.armorName + " before grasping onto her chin, turning it this way and that, inspecting your prize thoroughly. Those golden lips glisten in the light as she trills feebly, her eyes looking soulfully up at you.\n\n", false);

			MainScreen.text("Grinning lewdly you squeeze her cheeks, the stricken harpy opening her mouth obediently. Her tongue is short and stubby like that of a normal bird, glistening with saliva and looking terribly inviting. You strip off the lower portion of your " + player.armorName, false);
			//(if pussy)
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				MainScreen.text(", fluids seeping from your plush " + vaginaDescript(0) + " all the while", false);
			}
			//(if cock)
			if (player.lowerBody.cockSpot.hasCock()) {
				...
				if (player.totalCocks() > 1) MainScreen.text("each of ", false);
				MainScreen.text("your " + multiCockDescriptLight() + " jumps to attention, throbbing and begging to be touched", false);
			}
			...
			//(if boobs)
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) MainScreen.text(", but soon she presses her toned, petite chest against your " + player.allBreastsDescript() + ", that stubby tongue tangling around yours lustily.", false);
			...
			if (player.lowerBody.cockSpot.count() == 1) {
				...
				//(If balls) 
				if (player.lowerBody.balls > 0) MainScreen.text("  Her hands creep down to softly squeeze and massage your " + ballsDescriptLight() + ", the harpy's eager head bobbing back and forth along the sweet tasting shaft of your " + cockDescript(0) + ", sending waves of powerful, overwhelming pleasure resonating through your body.", false);
				MainScreen.text("\n\n", false);
			}
			//[(Male, Shemale, Herm, multicock)
				...
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) {
					MainScreen.text(", and your " + player.allBreastsDescript() + " jiggle slightly from the sheer intensity of the pins-and-needles feeling coating your body.  Your " + nippleDescript(0) + "s perk up and harden.", false);
					if (player.biggestLactation() >= 1) MainScreen.text("  Milk dribbles out gently onto the areola, trickling sensually between your cleavage.", false);
				}
				else MainScreen.text(".", false);
				...
				//(If balls)
				if (player.lowerBody.balls > 0) MainScreen.text("  She moves that hand to softly squeeze and pet at your " + ballsDescriptLight() + ", massaging those bloated cum-factories with expertise. You can feel a distinct rumble as your " + sackDescript() + " really ups its production, almost feeling heavier.", false);
				//(If herm)
			...
			if (player.lowerBody.cockSpot.count() == 1) {
				...
				//(if herm)
				if (player.gender == 3) MainScreen.text(", her other hand doing the exact same to your own " + vaginaDescript(0) + ", digging those perfect fingers deep into the wet, hyper-sensitive flesh, spreading those lips wide", false);
				...
				//(If balls) 
				if (player.lowerBody.balls > 0 && player.lowerBody.cockSpot.list[0].cockLength <= 16) MainScreen.text("She even deep throats you, taking your length into her expertly-controlled mouth, that tongue creeping out to tickle at your " + ballsDescriptLight() + ".  ", false);
			...
			if (player.lowerBody.cockSpot.count() > 1) {
				...
				//(if balls)
				if (player.lowerBody.balls > 0) MainScreen.text(", dribbling all over your " + sackDescript(), false);
				...
				//(If herm)
				if (player.gender == 3) MainScreen.text("  Her fingers never stop their sweet teasing of your " + vaginaDescript(0) + ", using all four fingers on her hand to plumb your velvet depths.", false);
...
//Fuck a harpy with the players throbbing clit.
		private clitFuckAHarpy():void
		{
			...
			if (player.wetness() < 3) MainScreen.text("  Runnels of your lubricant are dripping down your [legs] in streams,");
			else if (player.wetness() < 4) MainScreen.text("  Streams of lubricant are soaking down your [legs] in waves,");
			...
			if (player.wetness() < 3) MainScreen.text("  A fresh gush of fluid washes out of your pussy at the peak of your pleasure even though you're far from a squirter");
			else if (player.wetness() < 4) MainScreen.text("  A fresh spout of girlspunk washes out of your pussy at the peak of your pleasure");
