export default class MinervaScene {
	...
	private minervaRomanced(): boolean {
		return (Flags.list[FlagEnum.MINERVA_LOVE] == 1);
	}
	...
	public encounterMinerva(): void {
		if (Flags.list[FlagEnum.MET_MINERVA] > 0) Flags.list[FlagEnum.MET_MINERVA]++; //Increment counter by 1 if previously met Minerva.
		if (Flags.list[FlagEnum.MET_MINERVA] == 0) firstMinervaEncounter();
		else if (Flags.list[FlagEnum.MET_MINERVA] == 3) minervaThirdPlusEncounter();
		else if (Flags.list[FlagEnum.MINERVA_BACKSTORY_LEARNED] > 0 && player.gender > 0 && Flags.list[FlagEnum.MINERVA_LOVE] == 0 && rand(2) == 0) minervaAfterFiveTalksRomancing();
		else {
	...
	private firstMinervaEncounter(): void {
		MainScreen.clearText();
		MainScreen.text("Returning to the mountains, you begin to climb; the usual search for loot and, perhaps, some harpies driving your upward efforts.  Perhaps you'll run into");
		//If Sophie Bimbo=0:
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] == 0 && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00283] == 0 && !kGAMECLASS.sophieFollowerScene.sophieFollower()) MainScreen.text(" that matronly harpy, Sophie");
		else MainScreen.text(" something interesting");
		MainScreen.text(", or get some good plunder from some of the local beasts");
		//if minotaur cum addict:
		if (player.minotaurAddicted()) MainScreen.text(".  With any luck, you might even find a hunky minotaur to scratch that burning itch in your mind");
		MainScreen.text(".  Trudging on, you make your way up and up the mountain, passing the occasional harpy nest as you climb and traverse along the mountain paths.  Just when it looks like you're going to hit the clouds, you catch sight of a trail that you had never really noticed before.");
		//Choice
		//[Ignore Path] [Take Path]
		menu();
		MainScreen.addButton(1, "Take Path", takeMinervasPath);
		MainScreen.addButton(2, "Ignore Path", ignoreMinervasPath);
	}
	//[Ignore Path]
	private ignoreMinervasPath(): void {
		MainScreen.clearText();
		MainScreen.text("Deciding to play it safe, you turn away from the new path and continue your search elsewhere.");
		// PC returns to camp.
	...
	private takeMinervasPath(): void {
		MainScreen.clearText();
		spriteSelect(95);
		MainScreen.text("Knowing that a new path could lead to great things, you can't help but wonder where this one could lead.");
		...
		if (player.tallness <= 96) MainScreen.text(" one hand");
		else MainScreen.text(" both hands");
		MainScreen.text(", bodily swinging you out over the abyss, with only her grip keeping you from plunging back down the mountainside.");
		//Low corruption: 
		if (player.stats.cor < 50) MainScreen.text("  \"<i>All right, you!  Who are you and what are you doing here?!  I don't get any visitors besides those horny harpies, so what are you after?  Hm?");
		//High corruption:
		else MainScreen.text("  \"<i>All right, you corrupted beast!  Who are you and what are you doing here?  Your presence is not welcome in this sanctuary... answer quickly or I'll drop you to your death!");
		//Demon Score=3>:
		if (player.demonScore() >= 3) MainScreen.text("  Filthy demon!  Come to finish your work, have you?!  You won't take me without a fight!  Maybe if you beg for your life I'll give you a chance. Speak!  Convince me to not simply kill you here and now!");
		MainScreen.text("</i>\" the creature demands, clearly threatening to drop you if you don't answer to her liking.");
		...
		if (player.lowerBody.isGoo()) {
			...
			//Corruption=75>:
			if (player.stats.cor >= 75) MainScreen.text("... \"<i>yet</i>\", you think to yourself.");
		...
		//High corruption:
		if (player.stats.cor > 66) MainScreen.text("  She is wise not to fully trust you.");

		MainScreen.text("\n\nWith");
		if (player.lowerBody.isGoo()) MainScreen.text(" your own");
		else MainScreen.text(" a");
		MainScreen.text(" relieved sigh, you take in the sight of the strange shark... harpy... harpy-shark... sharpy?  What an odd combination!  Looking the woman up and down, you take in her whole appearance.");
		...
		Flags.list[FlagEnum.MET_MINERVA] = 1;
		//[Follow] [Leave]
		menu();
		MainScreen.addButton(0, "Follow", followMinerva);
		MainScreen.addButton(1, "Don'tFollow", leaveMinervasFirstEncounter);
	}
	...
	private followMinerva(): void {
		MainScreen.clearText();
		spriteSelect(95);
		MainScreen.text("Deciding that you want to know more about Minerva, you follow her into the tower.  \"<i>So, now that you're inside, would you like to talk?  Or perhaps to partake in some of the peaches that grow on my trees?</i>\" she asks, looking at you with interest.");
	...
	private repeatEncounterMinerva(): void {
		...
		//No romance: 
		if (!minervaRomanced()) MainScreen.text("  \"<i>Ah, [name], you have returned!  What can I do for you?</i>\"");
		else MainScreen.text("  \"<i>Mmm... my lover has come to see me!  Oh, how I have missed you, my sweet!  Won't you join me for a bath?  Or... is there anything else I can do for you?</i>\"");
	...
	private minervaThirdPlusEncounter(): void {
		...
		//No romance:
		if (!minervaRomanced()) MainScreen.text("  \"<i>Welcome back, [name]!  It's good to see you again.  Have you come to give me some company, or would you like to partake in some of the fruit?  Or are you maybe here to sate your thirst?</i>\"");
		else MainScreen.text("  \"<i>Darling!  Coming to visit me all the way out here!  How sweet of you!</i>\"");
		MainScreen.text("  Minerva says as she strides toward you, her generous hips swaying back and forth, as if to entice you into her arms.");
	...
	private minervaAfterFiveTalksRomancing(): void {
		MainScreen.clearText();
		spriteSelect(95);
		Flags.list[FlagEnum.MET_MINERVA]++;
		...
		if (!player.lowerBody.isTaur() && !player.lowerBody.isDrider()) {
			...
			MainScreen.text("  Looking up, you see the grinning face of Minerva, clearly playing with you.  \"<i>Well, hello there, [name]!  I was wondering when you would come and visit me.  I can be away from the oasis for a little while, so... want to go for a flight, or are you scared of heights?</i>\"  she asks playfully.");
			//[Flight] [Land]
			menu();
			MainScreen.addButton(0, "Flight", goForASkyRideWithMinervaHeartBaring);
			MainScreen.addButton(1, "Land", dontFlyWithWithMeYouCrazyBitch);
		}
		else {
			...
			menu();
			MainScreen.addButton(0, "Next", minervaOpensHerHeartAndDisplaysHerVentricals);
		}
	}
	...
	private minervaOpensHerHeartAndDisplaysHerVentricals(): void {
		...
		//[Kiss] [Reject]
		menu();
		MainScreen.addButton(0, "Kiss Her", yeahSureMinervaILikeYouAndShit);
		MainScreen.addButton(1, "Reject", rejectMinervasLove);
	}
	...
	private yeahSureMinervaILikeYouAndShit(): void {
		...
		Flags.list[FlagEnum.MINERVA_LOVE] = 1;
		menu();
		//{Male: [NEXT] / Female/Herm: [NEXT]}
		//Sex Menu
		if (player.gender == 1) {
			if (player.cockThatFits(minervaVCapacity()) >= 0) MainScreen.addButton(0, "Next", minervaCowgirlSex);
			else MainScreen.addButton(0, "Next", letMinervaSuckYouOff);
		}
		else MainScreen.addButton(0, "Next", minervaLapSex);
	}
	...
	private rejectMinervasLove(): void {
		MainScreen.clearText();
		spriteSelect(95);
		MainScreen.text("You stare at Minerva awkwardly as she waits for your response, her electric blue eyes looking at you with hope.  ");
		if (player.stats.cor < 45) MainScreen.text("The look in the siren's eyes only makes this harder for you as you look into them; you can't help but feel bad that you really don't feel anything special for her.");
		else MainScreen.text("You can tell she really wants this - the siren is almost desperate for your attention.  Unfortunately for the love starved herm, she won't find the love she craves with you.");
		MainScreen.text("  You look away from her and pull back a bit before speaking.  You confess to her that, while she is a nice person, you do not feel that sort of bond with her, and you apologize if she was misled by your actions, but that you would like to be friends with her still.");
	...
	private minervaTalkSelect(bath: boolean = true): void {
		...
		if (Flags.list[FlagEnum.MINERVA_BACKSTORY_LEARNED] == 0) {
			//Force first time to talk about her early backstory!
			if (Flags.list[FlagEnum.MINERVA_BACKSTORY] == 0) {
		...
		if (minervaRomanced() && Flags.list[FlagEnum.MINERVA_BACKSTORY_LEARNED] == 1) choices[choices.length] = talkWithMinervaAboutSharkGirlDaughter;
	...
	private talkingToMinervaAboutBackstory(): void {
		...
		//PART 2
		else if (Flags.list[FlagEnum.MINERVA_BACKSTORY] == 1) {
			Flags.list[FlagEnum.MINERVA_BACKSTORY] = 2;
		...
		//PART 3
			...
			Flags.list[FlagEnum.MINERVA_BACKSTORY] = 0;
			...
			//No Romance: 
			if (!minervaRomanced()) MainScreen.text("  \"<i>You're a good friend to have, and you know that, [name].</i>\"");
			//Romance:
			else MainScreen.text("  \"<i>You're a wonderful lover, you know.  I couldn't be happier that you found me.</i>\"");
			// PC returns to camp.
			Flags.list[FlagEnum.MINERVA_BACKSTORY_LEARNED] = 1;
		}
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private talkToMinervaAboutHerCorruption(): void {
		MainScreen.clearText();
		spriteSelect(95);
		MainScreen.text("Telling Mineva that you just want to talk brings a smile to her face, the siren eager to speak with you.  Guiding you over to a soft spot of moss by the spring, she sits next to you and looks into the pond, her smile slowly fading as she looks into the clear waters.");
		...
		if (Flags.list[FlagEnum.MINERVA_BACKSTORY_LEARNED] == 1) MainScreen.text("  You remember my firstborn, the shark girl I had?  Well... not long after she reached maturity, she... well, she got a bit amorous...  She seduced me and I... I mated with her.  I mated with her over and over that night.  I enjoyed it... it was like I couldn't get enough...  I probably got her pregnant...  So shameful...  I impregnated my own daughter and made her bear my grandchild,");
		MainScreen.text("</i>\" she declares sourly as she stares into the pure waters of the spring.");
	...
	private minervaMotherhood(): void {
		...
		if (player.gender == 1) MainScreen.text(" and those goblins");
		//PC vagina: 
		else if (player.gender == 2) MainScreen.text(" and that bee-girl");
		//PC herm:
		if (player.gender == 3) MainScreen.text(", goblins and that bee-girl");
		MainScreen.text(", if the stories were real, I'm sure you gave them plenty of children.  I wish to one day have kids... to have a great, loving family to call my own.</i>\"");

		MainScreen.text("\n\nShaking your head, you grin and shuffle over to Minerva.  Sitting down with her, you");
		//Romance: 
		if (minervaRomanced()) MainScreen.text(" hug her tightly and kiss her cheek, telling her that she will have children one day, and perhaps you will have to see about helping her with that 'problem.'");
		//No romance:
		else MainScreen.text(" place a hand on her shoulder and give her a comforting smile before telling her that she will have that one day - you're sure of it.");
		MainScreen.text(" Hearing your words brings a smile to her black glossy lips.  \"<i>Thank you so much; that means a lot to me, really,</i>\" she coos, her hands gingerly stroking yours before pulling away.  \"<i>But... I guess we have been here for a while.  I'm sure you have a great deal of work to do as such a brave champion.</i>\"");
	...
	private bathTimeTalkWithMinerva(): void {
		...
		//[Yes] [No]
		menu();
		MainScreen.addButton(0, "Yes", bathTimeWithMinerva);
		MainScreen.addButton(1, "No", noBathTime);
	}

	//[No] // add and take to another talking scene
	private noBathTime(): void {
		MainScreen.clearText();
		MainScreen.text("Looking down, she sighs, a brief moment passes before she returns her attention to you.  \"<i>Well, that's all right, maybe next time.  So you wanted to talk; what would you like to know?</i>\"");
		menu();
		MainScreen.addButton(0, "Next", minervaTalkSelect, false);
	}
	...
	private minervaSexMenu(display: boolean = true): void {
		if (display) {
			MainScreen.clearText();
			spriteSelect(95);
			MainScreen.text("You tell Minerva that, this time, you'd like to have sex with her.");
			//Default, first time:
			if (Flags.list[FlagEnum.TIMES_MINERVA_SEXED] == 0) {
				...
				//PC cock, add: 
				if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" just... not in my vagina, all right?  That's... special.");
				MainScreen.text("  Did you have anything specific in mind?");
				//PC female, add: "
				if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Would you mind how we do it?  You're the one asking, so... would you want me to fuck your ass?  Or... do you want me to try to fit between those sexy hips of yours?");
				MainScreen.text("</i>\"");
			}
			//Dick Too Big - PC exceed capacity
			else if (rand(2) == 0 && player.lowerBody.cockSpot.hasCock() && player.cockThatFits(minervaACapacity()) < 0)
				MainScreen.text("Looking at the monstrous mass of your mammoth member, Minerva blushes hard and stares at it with wide eyes.  \"<i>Oh... oh my, that... that's </i>big</i>... I'm sorry, I don't think that's going to fit inside me, how about we try something else?  I'm sure we can come up with something!</i>\"  Minerva says with confidence.");
			//{Repeated/No romance: 
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Since you don't seem to mind... would you like me to fuck your ass or to squeeze into your cute cunny?");
				MainScreen.text("</i>\"");
			}
			//Romanced: 
			else {
				...
				//PC cock, add:
				if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  Since we're together, I'll even let you have my pussy, I bet you like the sound of that!");
				//PC female, add:
				if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  I'd be happy to pound that cute butt of yours and stuff your hot pussy - which would you rather have, my dear?");
				MainScreen.text("</i>\"");
			}
		}
		menu();
		let btnIdx: number = 0;
		if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(minervaACapacity()) >= 0)
			MainScreen.addButton(btnIdx++, "FuckHerButt", fuckMinervasAsshole);
		if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(minervaVCapacity()) >= 0 && minervaRomanced()) {
			MainScreen.addButton(btnIdx++, "FuckCowgirl", minervaCowgirlSex);
			MainScreen.addButton(btnIdx++, "RestrainFuck", fuckMinervaWithHerHandsBehindHerBack);
		}
		if (player.lowerBody.vaginaSpot.hasVagina())
			MainScreen.addButton(btnIdx++, "TakeHerDick", minervaLapSex);
		MainScreen.addButton(btnIdx++, "EatHerOut", goDownOnAHermAndLoveItYouDirtySlutYou);
		if (player.lowerBody.cockSpot.hasCock())
			MainScreen.addButton(btnIdx++, "Get BJ", letMinervaSuckYouOff);
		MainScreen.addButton(9, "Leave", repeatEncounterMinerva);
	}
	...
	private fuckMinervasAsshole(): void {
		...
		MainScreen.text("\n\nYou draw yourself back, dragging your " + CockDescriptor.describeCock(player, x) + " inch by inch through her grinding, squeezing walls, then thrust yourself firmly back inside her.  You repeat the process again, and then again, picking up speed as you pull and push.  The siren's lyrical moans slowly start to fill the air, her body rocking with your thrusts, pressing and grinding her pierced nipples against the ground as you pound her slick rear.  The longest of her cock tentacles manage to reach back and grabs hold of your " + CockDescriptor.describeCock(player, x) + ", feverishly stroking you as you thrust into her.  The naughty little things sting you hard, oozing their aphrodisiac venom into your " + CockDescriptor.describeCock(player, x) + " instinctively.  You cry out, shivering as the potent venom immediately streams through your body, setting your blood aboil with lust, and causing you to piston into her ass with ever-increasing speeds.  More and more toxin floods you, combining with the incredible sensations of her anal walls gripping and squeezing you to turn you into a mindlessly rutting beast.");
		...
		if (!minervaRomanced()) {
			//First time ending:
			if (Flags.list[FlagEnum.TIMES_BUTTFUCKED_MINERVA] == 0) {
		...
		Flags.list[FlagEnum.TIMES_BUTTFUCKED_MINERVA]++;
		player.orgasm();
		player.stats.sens += -1;
		if (Game.inCombat)
			cleanupAfterCombat();
	...
	private minervaCowgirlSex(): void {
		...
		if (x < 0) x = player.smallestCockIndex();
		Flags.list[FlagEnum.TIMES_MINERVA_SEXED]++;
		MainScreen.text("You look the attractive siren up and down and");
		if (player.tallness <= 60) MainScreen.text(", with some trepidation, given the difference in size,");
		MainScreen.text(" ask if Minerva would like to give her female parts some pleasure, promising to allow her control of the pace.");
		//First time:
		if (Flags.list[FlagEnum.TIMES_MINERVA_COWGIRLED] == 0) {
			MainScreen.text("  Minerva looks unsure, the siren biting her lip and looking a little away from you, \"<i>I don't know.  That's... special...  After the things I have been through, I don't want to just do it with anyone.  You... </i>have<i> been really nice to me though... I guess I can trust you.</i>\"");
		}
		//Repeated:
		else MainScreen.text("  Minerva runs a hand through her hair and smiles, \"<i>Well... the last time was quite nice, I think it could be fun.  Sure, let's do it.</i>\"");

		MainScreen.text("\n\nWith a smile, you gently remove your clothes and put them aside, giving Minerva a chance to see your naked body.  Your " + CockDescriptor.describeCock(player, x) + " already starting to stiffen, you lie back down on the ground, patiently waiting for the siren to make the first move.  You don't have to do so for long, as soon, Minerva's tight tube-top and short shorts are pulled off, her firm, pierced DD's bouncing free of their fabric prison.  Clearly excited by the thrill of actually having regular sex, you can see her male endowment quickly growing hard between her legs, the forearm-length appendage ready to go despite the fact it won't be used for the main event.  Looking down at you, Minerva approaches, swaying her hips to and fro.  She kneels down, crawling over you like some predator, one of her hands briefly stroking your " + CockDescriptor.describeCock(player, x) + " before sliding up to feel your [chest].  \"<i>I promise not to get too excited,</i>\" she says before grabbing hold of your maleness and stroking it again.  Bringing her own male organ up against yours, her tentacles encircle your " + CockDescriptor.describeCock(player, x) + ", stinging it with her potent venom to really get you fired up as she teases you.  \"<i>This stuff is always floating around in me - that's why I get so damn hard so fast...  I hope you like my special venom.</i>\"");

		MainScreen.text("\n\nYou groan softly, feeling the venom already coursing through you.  Your " + CockDescriptor.describeCock(player, x) + " surges erect, the venom-induced lust making you swell larger than usual, ready for her to do with as she pleases, your limbs numb and useless from the lust-inducing toxins coursing through your body.  You are completely helpless and at her mercy...  And part of you seems to like that.");

		MainScreen.text("\n\nMinerva seems to enjoy seeing you helpless like this, as her dick seems to just get harder and harder while her tentacles caress your member, though they no longer sting; instead, they just tease you with their slick and gentle touch.  \"<i>You like that, don't you?  You're so hard I can feel your need growing more and more.  You want my tight pussy so badly,</i>\" she says with a lecherous grin before rising her body up and pressing against your " + CockDescriptor.describeCock(player, x) + " with her surprisingly cool cunt.  Already her lips are soaked, her slick juices running down your dick as she teases you even further.");
		...
		if (player.cumQ() >= 500) {
			...
			if (player.lowerBody.balls > 0) MainScreen.text(" your balls and");
			MainScreen.text(" the base of your " + CockDescriptor.describeCock(player, x) + ", stinging you again and again like an overzealous jellyfish, pumping more of her aphrodisiac venom into your genitals in mind-numbing quantities.");
			if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  A few even slither down into your [vagina] to sting you there as well, making sure you are, at least temporarily, turned into a sex-hungry beast.");
		}
		...
		if (player.cumQ() < 500) MainScreen.text(", Minerva's belly visibly bloated by the time you are finished");
		MainScreen.text(".");
		if (player.cumQ() < 500) {
		...
		if (player.cumQ() >= 500 && player.cumQ() < 4000) {
			...
			if (minervaRomanced()) MainScreen.text(" with your child");
			MainScreen.text(".");
		}
		//Very High: 
		if (player.cumQ() >= 4000) {
			...
			if (!minervaRomanced()) MainScreen.text("  \"<i>Oohhh...I'm so... so big... It's almost like you've gotten me pregnant.</i>\"");
			else MainScreen.text("  \"<i>Oohhh... I'm so... so big... cumming so much, I didn't know you were so desperate to get me pregnant.</i>\"");
		}
		//Low-to-moderate: 
		...
		Flags.list[FlagEnum.TIMES_MINERVA_COWGIRLED]++;
		player.orgasm();
		player.stats.sens += -1;
		if (Game.inCombat)
			cleanupAfterCombat();
	...
	private minervaLapSex(): void {
		...
		//PC herm: 
		if (player.gender == 3) MainScreen.text("  One of her hands trail down to your hard herm " + CockDescriptor.describeCock(player, x) + ", stroking you softly.");
		...
		//Nonvirgin: 
		if (!player.hasVirginVagina()) MainScreen.text("  The thick siren prick plunges into your soaked cunt and fills your sweltering hole with her oddly cool cock, both of you groaning in delightful pleasure.");
		//Virgin:
		else MainScreen.text("  The thick siren prick plunges into your soaked cunt, filling your sweltering pussy with her oddly cool cock, in the process tearing right through your hymen, claiming your first fuck for herself as you bite back a pained yelp, though it is quickly displaced by the pleasure from the stimulation and venom.");
		...
		//PC herm: 
		if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text(", your own throbbing phallus");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("es");
			MainScreen.text(" cumming alongside,");
			//{Low-moderate cum: 
			if (player.cumQ() < 500) MainScreen.text(" gushing your cum all over both of your bellies");
			//High cum volume:
			else if (player.cumQ() < 4000) MainScreen.text(" splattering thick splooge all over you and Minerva, caking your torsos in fertile jizz");
			//Very high cum:
			else MainScreen.text(" exploding out onto your unsuspecting bodies, thick ropes of starkly white sperm gushing over your bodies, quickly dousing both of you in buckets of cum and pooling on the ground");
		}
		MainScreen.text(".");
		...
		if (!minervaRomanced()) {
			//Default, first time:
			if (Flags.list[FlagEnum.TIMES_MINERVA_LAPSEXED] == 0) {
				MainScreen.text("\n\nMinerva actually seems a little sad that you're leaving.  \"<i>I guess you are a busy person.  But I'm glad we did this, it was a lot of fun.  And... you know...  I wouldn't mind if we did this again, soon,</i>\" She suggests with a small, hopeful smile on her face.  You promise her that you'll try to come back again soon.");
			}
			//Default, repeated:
			else {
		...
		player.slimeFeed();
		Flags.list[FlagEnum.TIMES_MINERVA_LAPSEXED]++;
		player.orgasm();
		player.stats.sens += -1;
		if (Game.inCombat)
			cleanupAfterCombat();
	...
	private goDownOnAHermAndLoveItYouDirtySlutYou(): void {
		...
		if (player.tallness < 96) MainScreen.text("your fingers not even able to fit around the herm's hefty girth");
		else MainScreen.text("your fingers barely wrapping around the meaty member, the fat thing throbbing gently");
		MainScreen.text(" as you stroke it slowly.  Your touches are clearly adding to your partner's pleasure, her thick masculinity pulsating under your grip, her tentacles reacting to your presence, the dexterous things curling around your hands and fingers, touching you gently, but not stinging you.");
		...
		menu();
		MainScreen.addButton(0, "Balls", lickMinervasBalls);
		MainScreen.addButton(1, "No Balls", noBallsMinervaAndContinuation);
	}
	...
	private noBallsMinervaAndContinuation(clear: boolean = true): void {
		...
		spriteSelect(95);
		MainScreen.text("Your careful ministrations soon have the poor siren shuddering and squirming, looking about ready to pop from your skilled hands and mouth.");
		if (!clear) MainScreen.text("  Retreating from her throbbing balls, you");
		else MainScreen.text("  You");
		MainScreen.text(" move back up to her now soaking snatch, your tongue eagerly lapping up all the tasty juices from her lips before delving deep into her refreshingly cool cunt, hungrily licking up every slick drop of her succulent sweetness.  The act of worming your tongue back inside her hungry hole again drives Minerva ever closer to the brink of her climax, each slurp and stroke of your tongue and hands pushing the near-climactic herm right to the edge and sending her off into the blissful, pleasure-filled abyss of a mind-blowing double orgasm.");
		...
		//if demonic tongue:
		if (player.upperBody.head.face.tongueType == TongueType.DEMONIC) MainScreen.text(", your perversely prehensile tongue slowly retracting from her quivering quim, rubbing and teasing as it comes out and nearly sending the overly sensitive herm into another climax");
		MainScreen.text(".");
	...
	private letMinervaSuckYouOff(): void {
		...
		//If naga: 
		if (player.lowerBody.isNaga()) MainScreen.text(" lounge against the rock, spreading open your protective slit and letting your " + CockDescriptor.describeCock(player, x) + " hang free.");
		else MainScreen.text(" sit down, spreading your [legs] and opening up the crotch of your [armor] to the air, letting your " + CockDescriptor.describeCock(player, x) + " hang free.");

		MainScreen.text("\n\nAnxious and ready, Minerva looks down at you before kneeling, placing a hand on your [hips] as she takes hold of your " + CockDescriptor.describeCock(player, x) + " and slowly starts to stroke it.  Her eyes looking up at you, she leans in and starts to gently lick your hardening flesh with the tip of her tongue.  Under her careful, deliberate attentions your manhood is quickly brought to full attention.");
		...
		MainScreen.text("\n\nTaking a relaxing breath, Minerva dives right into it, her long tongue slithering out to lick your hard, sensitive flesh, wrapping around the tip and giving the most sensitive part of your dick a good squeeze.  The heat-quenching chill of her wet tongue feels remarkable on your hard prick, the tight squeeze pulling a gasp from your mouth.  Your hands move over her head and into her dark red hair, and you're suddenly relieved that she can't envenom you this way.  Wanting to show you what she's made of, your siren leans in, dragging her cool wet tongue up your " + CockDescriptor.describeCock(player, x) + " from base to tip before planting a kiss right on the " + player.cockHead(x) + " of your throbbing, needy dick.");

		//If balls:
		if (player.lowerBody.balls > 0) MainScreen.text("  Her hand on your hip moves down between your legs, sliding inwards to gently caress and massage your cum-filled [balls] to increase your pleasure even more, her hands trying their best to get your nuts worked up and prepare a good load for her at the end.");
		...
		//If balls:
		if (player.lowerBody.balls > 0) MainScreen.text(" before licking and teasing your [balls]");
		MainScreen.text(".");

		MainScreen.text("\n\nWith your " + CockDescriptor.describeCock(player, x) + " firmly captured by her tongue, Minerva leans down, opening her mouth wide and taking you inside its cool, wet confines, even as her tongue squeezes you and slides around your warm, sensitive flesh.  Looking up at you with a playful twinkle in her eyes, she starts to suck and squirm her tongue around on you, the long, prehensile appendage switching between licking over your throbbing cockmeat and squeezing it in a snakelike embrace.");

		MainScreen.text("\n\nPuckering her glossy black lips, Minerva starts to bob her head, taking in more and more of you with each downwards motion");
		//If cockarea 50+:
		if (player.cockArea(x) >= 50) MainScreen.text(", your huge cock deep inside her mouth, and soon far down her throat.  With some difficulty, her slightly angular nose presses down against your crotch, every inch of your " + CockDescriptor.describeCock(player, x) + " trapped inside her cool, wet mouth and throat");
		MainScreen.text(".");
		...
		if (player.lowerBody.balls > 0) MainScreen.text(", while your [balls] lurch and swell, rising up as you reach your climactic explosion");
		MainScreen.text(".");

		MainScreen.text("\n\nGripping Minerva's head tightly, you let out a long deep moan, your " + CockDescriptor.describeCock(player, x) + " bloating with each ejaculation.  Eager to please, Minerva does her best to drink down every drop you give her, her hands gripping your [hips] and pressing her nose against your crotch, puckering her full black lips around your [sheath].");
	...
	private fuckMinervaWithHerHandsBehindHerBack(): void {
		...
		MainScreen.text("\n\nDespite your brutal thrust, you slide in with ease, Minerva's tight walls so slickened with her arousal.  You squeeze into the deliciously tight herm like butter, easily ramming every inch of your " + CockDescriptor.describeCock(player, x) + " into her with only a few hard, deep thrusts.  With your hips pressed against Minerva's accepting body, you grind against her, making sure she can feel every inch moving inside her clinging cunt.");
		//If balls: 
		if (player.lowerBody.balls > 0) MainScreen.text("  As you continue to pound the siren, your [balls] swing between your legs, the sperm-filled orbs churning with your fertility, preparing to fill the needy siren with every drop you can muster when you let loose your eventual orgasm.");
		...
		//Holy shit!:
			...
			if (player.lowerBody.balls > 0) MainScreen.text("  Minerva groans and lets out mindless gurgles as you unload your overproductive balls into her.");
			MainScreen.text("  To even your amazement, her hungry womb is able to contain much of your sperm before she can take no more, each blast of potent cum now gushing from between your bodies, the floor soon looking like someone dumped a barrel full of raw milk on the ground before you're finally done.");
	...
	private pcGetsEatenOutByMinerva(): void {
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(", and, perhaps, your more male parts while she's down there");
		MainScreen.text(".");
		...
		if (player.lowerBody.balls > 0) MainScreen.text(", one of her hands lifting your [balls] up before giving each of the fertile nuts a kiss");
		MainScreen.text(" and moving down to your sweet cunt, just waiting for her attentions.");
		...
		MainScreen.text("\n\n");
		//If dick:
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("Thanks to her handy stimulation, your musky maleness throbs and dilates along your femininity, the thick cockvein feeding your bloating member as your seed rushes out, splattering up and onto Minerva's back, defiling her smooth skin with your fertility.  ");
		MainScreen.text("Her mouth is closed over your hot quaking cunt, catching every drop and sucking hard on you as she slowly retracts her perversely prehensile tongue.  Minerva's mind wanders as she muses your unique flavors, her tongue still squirming inside you, making sure to lick up as much of your feminine juices as she can before finally pulling out from your body and letting go of you.");
		...
		Flags.list[FlagEnum.MINERVA_LEZZES_OUT]++;
		// PC returns to camp
		player.stats.sens += -2;
		player.orgasm();
		if (Game.inCombat)
	...
	private eatSomethingYouCunt(): void {
		...
		spriteSelect(95);
		//Acquiring: 
		MainScreen.text("You walk over to the fruit trees surrounding the spring, examining the strange treats.  You decide that one ripe fruit, one resembling a peach, is the best choice, and pluck it from the tree.  Thanking Minerva for letting you have it, you stow it away safely and head back to camp.\n\n");
		inventory.takeItem(consumables.PURPEAC, camp.returnToCampUseOneHour);
	}
	...
	*/
	...
	private getADrinkYouBitch(): void {
		MainScreen.clearText();
		spriteSelect(95);
		MainScreen.text("You ask Minerva if she's okay with allowing you to drink from her spring.  She replies with a nod, a smile and a casual wave towards the pool.  Making it quite clear that you can drink your fill.");
		//[Bottle] [Drink]
		//[Drink]
