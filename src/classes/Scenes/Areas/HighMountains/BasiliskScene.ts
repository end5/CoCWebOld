	...
	public class BasiliskScene extends BaseContent
	{
		...
		private function defeatBasiliskAndAnal():void {
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				...
				if(player.lowerBody.cockSpot.list[x].cockLength <= 13) {
					...
					if(player.lowerBody.balls > 0) outputText(" your " + ballsDescriptLight() + " slapping against it,", false);
				...
				//(More than 13 inches:)
					...
					//[(if balls)
					if(player.lowerBody.balls > 0) outputText("Your " + ballsDescriptLight() + " swing heavily underneath your shaft, swelling as your rhythm picks up.  ", false);
				...
				//<(13 inches or less:) 
				if(player.lowerBody.cockSpot.list[x].cockLength <= 13) outputText("gripping its hips and pushing every inch of yourself in and reaching a glorious, skin tingling peak.", false);
				//(More than 13 inches:)
		...
		private function defaultBasiliskRape():void {
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				...
				if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" and sticks some of the smaller fingers of its other hand in your " + vaginaDescript(0), false);
		...
		private function basiliskHasVagEggStuff():void {
			...
			//(egg preg:
			if(player.pregnancyIncubation > 1 && player.pregnancyType == PregnancyType.OVIELIXIR_EGGS) outputText("  The basilisk licks your bulging belly hungrily, pushing against and testing for the eggs you are carrying.  Your sensitive cargo shifts around under its hungry attention; you'd squirm, but that is, of course, impossible.", false);
			//(heat or perk:
			if(player.inHeat || player.perks.has("Oviposition") || player.perks.has("BasiliskWomb")) outputText("  The basilisk licks your belly hungrily, its sticky tongue crawling like a warm tentacle across your sensitive underside.  You'd squirm, but that is impossible.  The creature is making you feel everything it is forcing upon you.", false);
			outputText("\n\n", false);
		...
		public function basiliskBirth():void {
			...
			if(player.perks.has("BasiliskWomb")) {
				...
				if(player.lowerBody.cockSpot.hasCock()) outputText(" and a downpour of hermcum");
				outputText(".");
				...
				if(flags[FlagEnum.BENOIT_EGGS] > 0) outputText(" as well");
				...
				if(player.stats.cor < 33) outputText("You find yourself shaken by the experience, and deeply disquieted by the thought of the clutch of monsters you have unleashed on this world.  You pick yourself up, rub yourself down and leave, promising yourself fervently you'll be more careful around basilisks in the future.\n\n", false);
				else if(player.stats.cor < 66) outputText("You pick yourself up, rub yourself down and leave.  You feel conflicted about what just happened; on the one hand you feel disquieted about the dozen monsters you just unleashed on this world, on the other you cannot help feel oddly proud of them and yourself.\n\n", false);
		...
		private function basiliskAdvantageHarpy():void {
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				...
				//Multicock: 
					...
					if(player.lowerBody.cockSpot.count() >= 3) outputText(", crowded out by the bodies of the others despite her transparent desire for the unoccupied man meat flopping against them,", false);
		...
		private function basiliskAdvantageGoblin():void {
			...
			//Unsexed: 
				...
				//(Addict: 
				if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] > 0 || player.perks.has("MinotaurCumAddict")) outputText("  You're incredibly frustrated that you can't clench yourself and hold the magical substance deep inside you, so you can savor the wonderful, soft elation it blossoms inside of you for as long as you can.", false);
				outputText("\n\n", false);
		...
		private function driderPCEggLaysBasilisk():void {
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) outputText("already-dripping pussy");
			...
			if(player.lowerBody.cockSpot.hasCock()) outputText("wrap tightly around your waiting prick");
			...
			//(if corruption >= 50 
			if(player.stats.cor >= 50) outputText(" before heading off towards your camp. The harpies will probably free him from your webs... eventually.");
		...
		private function layBeeEggsInABasilisk():void {
			...
			if(player.upperBody.wingType == WING.BEE_LIKE_SMALL || player.upperBody.wingType == WING.BEE_LIKE_LARGE ) outputText("buzz lightly over to the basilisk's writhing body");
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				...
				if(player.lowerBody.balls > 0) outputText("and inside your balls ");
