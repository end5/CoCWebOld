 ...
 */
export default class BasiliskScene {
	...
	public defeatBasilisk(): void {
		...
		if (player.cockThatFits(monster.analCapacity()) >= 0 && player.stats.cor >= 66) evil = defeatBasiliskAndAnal;
		//Player HP victory: 
		if (monster.HP < 1) MainScreen.text("Unable to stand anymore, the basilisk shakily sinks down on one knee, drops its head and looks at the ground, evidently demonstrating submission.", false);
		//Player Lust victory: 
		else MainScreen.text("No longer able to control its raging erection, the basilisk closes its eyes and sinks to one knee.  It would probably be attempting to signal its submission to you if it weren't furiously masturbating its long, purple cock, which has emerged straining from the creature's genital slit.", false);

		//If victory and Player Lust above 30: 
		if (player.stats.lust >= 33 && player.gender > 0) {
			MainScreen.text("  Certain that the creature won't dare try and turn its eyes on you again, you take your time to look the tall reptile over directly for the first time.  Perhaps you could use it to satisfy your baser urges. If so, what part of it do you choose?", false);
			//[Tongue][Ass]
	...
	private tongueBasiliskSmex(): void {
		spriteSelect(75);
		MainScreen.text("", true);
		MainScreen.text("You tap your jaw as you stare down at the defeated reptile, smiling at its oddly formal gesture of surrender.  You'd very much like to take some satisfaction from the basilisk, extract a bit of pleasurable payment for what it tried to do to you: but the more you get to look at it properly, the more difficult doing that seems. The tall, thin creature is all angles, tough scales and pointed edges. You don't like the thought of putting your genitals anywhere near its sickle claws or curved fangs.\n\n", false);

		MainScreen.text("It is as you are taking in this latter article that the basilisk swallows nervously, and you get a glimpse inside its mouth at a long, sticky, pale purple tongue, another gift of its reptilian ancestors. It's the only part you've seen of it so far which looks... soft.  Pliable.  Your smile widens and you step forward, slowly loosening your " + player.inventory.armor.displayName + ".\n\n", false);
	...
	private basiliskHasVagEggStuff(): void {
		...
		if (player.pregnancyType == PregnancyType.OVIELIXIR_EGGS) {
			...
			player.knockUpForce(PregnancyType.BASILISK, PregnancyType.INCUBATION_BASILISK - 150); //Convert Ovi Elixir eggs to Basilisk eggs
		}
	...
	public basiliskBirth(): void {
		...
		if (player.perks.has("BasiliskWomb")) {
			...
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" and [eachCock] starting to leak pre-cum");
			MainScreen.text(" as you find yourself being moved to climax by the birthing.  You see no point in resisting and reach down to begin fiddling with yourself, moaning in pain-spiked pleasure as the stimulus overwhelms you. With an orgasmic cry, you release your eggs into the world amidst a gush of femcum");
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" and a downpour of hermcum");
			MainScreen.text(".");

			MainScreen.text("\n\nWhen you find yourself able to stand, you examine what it is you have birthed; ");
			//(eggNumber) 
			MainScreen.text(num2Text(Math.floor(player.totalFertility() / 10)));
			MainScreen.text(" large, jade-colored eggs, the unmistakable shape of reptile eggs. You pick up one and hold it gently against your ear; inside, you can hear a little heart, beating strong and quick.  You put it down carefully with its fellows and stare at your clutch, a queasy tangle of emotions tugging at you.");

			//First time:
			if (Flags.list[FlagEnum.BENOIT_EGGS] + Flags.list[FlagEnum.BENOIT_GENERIC_EGGS] == 0) {
			...
			if (Flags.list[FlagEnum.BENOIT_EGGS] > 0) MainScreen.text(" as well");
			MainScreen.text(".  You place the egg back down and gather them all up, moving them closer to the campfire to stay warm while you recover from your exertions.");
			...
			//First Time:  
			if (Flags.list[FlagEnum.BENOIT_GENERIC_EGGS] == 0) {
	...
	private basiliskAdvantageHarpy(): void {
		spriteSelect(75);
		MainScreen.text("Time stretches by at an agonizingly slow pace as you stand there, a bizarre, motionless flesh statue.  You have no way of measuring how much time is passing; the sun is not in your direct line of vision.   You try to move any and every part of yourself in turn, but it is hopeless.  Your body is a cage, and you begin to hate the basilisk less because it paralyzed you and more because it left your mind entirely aware of it.  Every so often another unbidden backwash of erotic memories overwhelms your senses, keeping you helplessly aroused and reminded of who did this to you.  Coupled with the unscratchable itches and the aching in your limbs the experience is one of sensational hell.\n\n", false);
		MainScreen.text("You hear a shrill cry from above you, half eagle scream and half mocking, female laughter.  With a fluttering flap of feathers, a harpy lands at your side before proceeding to stalk around you, taking in your helpless, frozen form with stiff, jerky movements.  You reflect bitterly that if the big bottomed bird woman had turned up fifteen minutes ago she would probably have scared the basilisk off.  As it is, you are going to have to take whatever she can throw at you... with a stiff upper lip, as it were.\n\n", false);

		//Male/Herm: 
		if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text("The harpy's eyes zero in on your erect cock greedily.  Stepping back from you she raises her head and lets out a screech which echoes around the mountains; as answering calls roll back to her she closes in, threads her arms around your neck and scalp and kisses you roughly. She pushes her golden lips against yours and squeezes her rough bird tongue into your mouth.  You feel your lips tingle and you raggedly moan against the savage frenching, her hot breath pushing down your throat as she circles your still tongue with her own, before exploring further down towards your tonsils.  By the time she has finished with you your whole body feels like it is glowing red from the effect of her lipstick, " + CockDescriptor.describeMultiCockSimpleOne(player) + " straining.\n\n", false);

			MainScreen.text("You wonder vaguely how she and her no-doubt-soon-to-arrive sisters are going to take advantage of you as you are; you find out a moment later when with no preamble whatsoever the harpy shoves you roughly in the chest.  ", false);
			if (player.isBiped()) MainScreen.text("You teeter horribly on your frozen heels for a moment and then fall onto your back like a collapsing statue.  ", false);
			else MainScreen.text("You feel yourself slowly and horribly lose your balance before flopping onto your side like a collapsing statue.  ", false);
			MainScreen.text("Unable to brace yourself, you bang your head painfully; as you are lying there dazed, you feel something build at the back of your mind.  Involuntary sensations prickle your skin and groin as, once again, the intermittent mental backwash that the basilisk's hypnosis has forced upon your mind hits you.  Memory after memory of sexually-charged encounters, daydreams or fantasies crowd your consciousness.  It is made worse, much worse by the pheromones the harpy has pushed into your mouth; your body rides the chemical glow at the same time as imaginings of soft skin, tight muscle and musk overload your senses.  You groan raggedly as " + CockDescriptor.describeMultiCockSimpleOne(player) + " bulges and leaks pre-cum, almost screaming for attention.  When you finally, woozily come to your senses, you find that all vision has been blotted out by a big, wobbly harpy bum, her moistening lips rubbing impatiently against your mouth.\n\n", false);

			MainScreen.text("The bang on the head you took, the harpy lipsticks trilling in your bloodstream and the involuntary tide of erotic memories which ebbs and flows over you mean the next couple of hours goes by for you in a haze of forceful sex.  You can't control your cock", false);
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s", false);
			MainScreen.text(", your sex drive or what you are thinking about; you are a prisoner of lust and you quickly subside under the sexual concussion, hoping eventually the harpies and your own body will stop fucking you.  It does sink in around your second involuntary orgasm, your aching " + CockDescriptor.describeCock(player, 0) + " spurting ribbon after ribbon of jizz into a harpy's clenching warmth, that the two arriving harpies are not happy with the first one; with your mouth and hands frozen you can't pleasure them with anything but your manhood, which leaves two of them fighting each other for time with your groin whilst a third grinds frustratedly at your face, mainly using your nose to rub at her inner walls and clit, forcing her juices down your nostrils and making you cough raggedly, struggling to breathe.\n\n", false);

			//Single cock: 
			if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("Their inability to properly satisfy themselves on your frame raises their tempers and frustration to the point where the three of them are physically fighting each other, treading all over you as they scream, bite and tear, their feathers flying all over the place.  Eventually the biggest of the trio drives the other two off, flapping and screeching into the mountains, before throwing herself onto her scratched and battered prize, her eyes lit up and wild.  She fucks you with a gusto born of bloodlust, her petit breasts bouncing up and down as she slams her powerful thighs into your " + LowerBodyDescriptor.describeHips(player) + ", picking up the pace.  She quickly forces you to another achingly pleasurable peak, your " + CockDescriptor.describeCock(player, 0) + " spurting more of your seed deep into her, and then just keeps on working you. Your cock seems incapable of going soft.  You feel the involuntary erotic backwash build in your skull again...\n\n", false);
			//Multicock: 
			else {
				MainScreen.text("The three of them do eventually work out a compromise however, once they discover you have more than one manpole to your name.  One pulls your " + CockDescriptor.describeCock(player, 0) + " forwards painfully until it is almost pointing towards your face before squatting over it, whilst another clutches at your " + CockDescriptor.describeCock(player, 1) + " as she spreads her legs and works her way inwards until she is spearing herself on you. You can't see them - you can't see anything except pink wobbly flesh and lavender feathers - but you can hear them shift impatiently around each other as they find a position which is comfortable to them both, their inner walls rubbing you from every direction as they move.  The third continues to thrust her needy sex into your face as best she can", false);
				if (player.lowerBody.cockSpot.count() >= 3) MainScreen.text(", crowded out by the bodies of the others despite her transparent desire for the unoccupied man meat flopping against them,", false);
				MainScreen.text(" as the other two begin to push and pull your cocks into them, their overcharged libidos taking hold.  Your first cock feels like it is being pulled off your body at the same time as its end is being pumped; the sensation of being doubly and brutally fucked like this is unbearably pleasurable and with a series of ragged gasps you tumble helplessly over a sweat-beading peak, your two cocks spurting jizz deep into the two harpies.  The third harpy coos as she feels your hot breath on her gaping vagina, and she begins to buck her frustrated sex against your face faster.  Trapped in the harpies' clenching holes and incapable of going soft, your cocks continue to get worked as if nothing happened.  You feel the involuntary erotic backwash build in your skull again and with a lost moan you fall comatose, incapable of even computing how fucked you are anymore.\n\n", false);
			}

			MainScreen.text("Eventually you notice that your dick is no longer trapped in sucking wet, and coming out of your daze you find yourself without harpy companions, leaving your petrified form in a mingled pool of harpy juices and your own jizz.  After another ten or twenty minutes of being forced to lie there and marinate in your own shameful memories, you find with great relief you can begin to move your fingers again.  Eventually with some effort you manage to work power into each corner of your body and finally shake free of the basilisk's curse; quickly, you pick yourself up and redress before anything else finds you and woozily begin to make your way back down the mountain.  The whole experience feels like it may have been a lucid sex nightmare to your sluggish mind and you could almost believe it- if you didn't reek of sticky harpy sex and your own musk.", false);
		...
		//Genderless: 
			...
			MainScreen.text("You feel something build at the back of your mind.  Involuntary sensations prickle your skin as, once again, the intermittent mental backwash that the basilisk's hypnosis has forced upon your mind hits you.  Memory after memory of sexually-charged encounters, daydreams or fantasies crowd your consciousness.  It is made worse, much worse by the pheromones the harpy has pushed into your mouth; your body rides the chemical glow at the same time as imaginings of soft skin, tight muscle and musk overload your senses.  The denial the petrification has forced upon you is becoming unbearable.\n\n", false);
			...
			MainScreen.text("The harpy manages to cum twice on your hand, gobbling with excitement as she spatters your arm with her juices.  During this time you are forced to ride the potent cocktail of hypnotic sexual compulsion and the harpy's golden lipstick again and again, until you feel you would have collapsed in a pool of steaming sex long ago if your knees would only allow it.  Once she is finished with you the harpy clambers down, taking care to wipe her leaking twat on your naked front as she does so, before flapping off with a winsome smirk, entirely ignoring your own plight.\n\n", false);
	...
	private basiliskAdvantageGoblin(): void {
		...
		//Unsexed: 
			...
			MainScreen.text("\"<i>A " + player.race() + "!</i>\" she yelps.  Hesitantly at first, then with increasing confidence as how vulnerable you are sinks in, she does a round circuit of you, taking in every angle of your frozen, helpless flesh.\n\n", false);

			MainScreen.text("\"<i>Get caught with our pants down by a basilisk, did we?</i>\" the goblin purrs.  \"<i>Well, not to worry. I'll take good c- hey, what gives?!</i>\" From behind you, you feel her hands thread their way around your " + LowerBodyDescriptor.describeLegs(player) + " before feeling and slapping all around your featureless groin.  \"<i>This is fucking bullshit!</i>\" she howls.  \"<i>I get a toy that can't stop me all to my lonesome, and it's some colossal jerkoff who thinks it's funny to have no sex!</i>\"  She stomps around to your front and glares at you, simmering with rage.  \"<i>I bet you think you're a real smartass, you bastard... you bitch... you... whatever!  Ooh, I'll fix you!</i>\"  With a look of complete disgust she storms off.  Having faced down this bizarre rant, you allow yourself to feel a tiny bit of relief.  You were worried for a moment there she'd go for your ass... a short distance away, you hear the goblin calling to someone.\n\n", false);

			MainScreen.text("\"<i>Hey you!  Yeah you, shit-for-brains!  Wanna free assfuck?  Come and get it then, you ugly, dumb muscle-bound moron!</i>\"  A moment later you feel a slap on your calf as the goblin runs past you, giggling.  \"<i>Have fun, smartass!</i>\"  A huge, angry bellow vibrates the air around you and the earth shakes as something big approaches you from behind.  You strain with every sinew of your being to escape, but you are, as ever, glued in place.  You manage a raspy moan as the enraged minotaur grasps you roughly by both arms, his animal musk filling your nostrils. <i>Fucking goblins.</i>\n\n", false);

			MainScreen.text("The huge bull-man is not one to look a gift fuck in the mouth.  Without bothering to take you in, bar a long wet sniff of your " + HeadDescriptor.describeHair(player) + ", he sticks his cock between your ass cheeks.  He grunts as he forces his head past your sphincter, squirting pre-cum into your passage as he does so.  Your body is incapable of clenching instinctively against the invasion, but there is no escaping how huge the dong feels as he impatiently forces your ass open.", false);
			player.buttChange(60, true, true, false);
			if (player.analCapacity() < 60) MainScreen.text("  The giant cock stretches you out painfully, and everything else blots out as your body attempts to accommodate the beast.  As he begins to thrust more of his length up you, he grunts and beads more of his drugged pre-cum, lubricating your anus.  This thankfully makes his cock easier to take, but also increases the pace of his thrusting as your hole becomes more receptive to it.", false);
			//V loose/Buttslut: 
			else MainScreen.text("  Your well-worn ass is a perfect fit for the giant cock and accepts it eagerly, every bit as welcoming as a moist vagina.  As the minotaur rubs against your tender inner walls he grunts and beads more of his drugged pre-cum, turning your hole into a helplessly wet, clenching ass cunt.", false);
			MainScreen.text("  You begin to pant from exertion and the overpowering sensation of the fuck as you feel first one ring and then a second push past your sphincter, then out again, then in and out as the minotaur picks up the pace.  As rut takes hold of him, he picks you up by your arms and uses his strength to force you up and down his dick, using you as a cocksleeve to sate his animalistic desires.  Your ass is slick with his pre-cum by now however, and the drugged slime has bumped you upwards into a hazy high; only the height of his downward thrust when his dick is almost completely buried in your bowels brings you out of it, the painful intensity of it dragging you down of your cloud with a gasp.\n\n", false);

			MainScreen.text("Eventually the minotaur's balls swell against your " + ButtDescriptor.describeButt(player) + " and with a long, satisfied moo, he reaches his peak, holding you down so his cum jets forth deep inside you.  The sensual high it has already instilled in you amplifies by tenfold as the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, your anus helplessly milking the creature for all it can get.\n\n", false);

			MainScreen.text("The minotaur holds onto you until he has finished spurting his last into you, before abruptly setting you back down on your frozen feet, pulling his slimy, receding member out of your abused anus and with a satisfied snort, takes his leave.  You feel his cum drooling out of you and down your " + LowerBodyDescriptor.describeHips(player) + ", but in your hazy, druggy state the feeling is almost sensual.", false);
			//(Addict: 
			if (Flags.list[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] > 0 || player.perks.has("MinotaurCumAddict")) MainScreen.text("  You're incredibly frustrated that you can't clench yourself and hold the magical substance deep inside you, so you can savor the wonderful, soft elation it blossoms inside of you for as long as you can.", false);
			MainScreen.text("\n\n", false);

			MainScreen.text("Eventually, after another thirty or so minutes of being forced to stand there and savor the cum trickling down your legs, you find with great relief you can begin to move your fingers again. With some effort you manage to work power into each corner of your body and finally shake free of the basilisk's curse; quickly, you shake the aching out of your " + LowerBodyDescriptor.describeLegs(player) + " and re-dress before anything else finds you and woozily begin to make your way back down the mountain, trying to ignore the feeling of ooze dripping out of you.", false);
			player.orgasm();
			player.slimeFeed();
		}
		//Female: 
			...
			MainScreen.text("\"<i>Get caught with our pants down by a basilisk, did we slut?</i>\" she purrs.  \"<i>Well, not to worry.  I'll take good care of you.</i>\"  You somehow doubt her good intentions, and your suspicions are confirmed when, after completing her sauntering circumference of you, she shoves your bottom half as hard as she can. You fall slowly like a wooden board; being unable to brace yourself, the sensation of dropping is horrible, and, once your skull connects with the ground, painful.  Before you can clear your swimming vision you feel the goblin's soft, dense weight on your chest, and then her eager lips upon yours, sucking and tonguing every inch of your frozen mouth that she can reach.  A tingling sensation spreads from your mouth downwards as her pheromone-laced lipstick takes effect.  By the time she has finished with your mouth and worked her way downwards, your paralyzed body feels warm and receptive, and your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " is wet with need.  You feel small fingers caressing your mons before slipping their way inside, circling your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " before testing your depths.  You wish you could cringe away or even thrust yourself forward to try and make the goblin better address your growing need, but that is impossible; you stare in front of you, petrified, as the fingers continue their lazy tour of your sex until your juices are running like a river in spring.\n\n", false);

			MainScreen.text("\"<i>You are well up for this, aren't you hun?</i>\" coos the goblin as she begins to flick your sopping clit, sounding delighted with the involuntary heat she has stoked within you.  \"<i>It's such a shame that I don't have the big cock you obviously want and need.  Don't worry though; I've got the next best thing!</i>\"  You hear her rummage around in her satchel, which is followed by the sounds of her wetly sucking on something with obvious enthusiasm.  You shrink inwardly as you make a strong guess as to what she's holding and what she intends to do with it.  Sure enough, a moment later you feel a dildo pushing unceremoniously into your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".", false);
			player.cuntChange(30, true, true, false);
			//[(tight)
			if (player.vaginalCapacity() < 30) MainScreen.text("  You didn't get the wet end, and you groan as you feel the thing begin to puff up as it reacts with your eager juices until it is pushing almost painfully against your inner walls.", false);
			else MainScreen.text("  You didn't get the wet end, and you groan as you feel the thing begin to puff up as it reacts with your eager juices until it fills your well-used passage almost perfectly.", false);
			MainScreen.text("  You feel the goblin crawl on top of you, pulling the dildo upwards so it rubs against your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " and then with a squealing giggle impales herself on the other end.  Her lustful movements translate through the artificial cock buried in your dripping sex, sending tremors and waves of pleasure to your very core.  A gush of female fluid splatters over your own sex as the goblin works herself right down the shaft connecting you until she is squelching against you wetly.  The goblin twists, grinding and scissoring her thighs, the hard bud of her clit rubbing back and forth over your own.  By now your passage feels as stuffed as it ever has been, crammed totally full of the squishy expanded double-dong.  Every motion the tiny slut makes is amplified directly into the fuck-stick plugging your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".  Judging by how wonderful it feels rubbing and twisting against your sensitive walls, the aphrodisiac it is leaking into you is definitely having an effect.  You whine at the goblin twists herself and her dildo around, rubbing and riding you so effectively, the involuntary tide of erotic memories building again...\n\n", false);

			MainScreen.text("After cumming over and over again in tandem with the dominant little slut she eventually takes mercy upon you, leaving you lying in a pool of mingled girl cum, dazed by the hypnotic backwash and the drugs which have left you so hazy and sensitive.  After another ten or twenty minutes of being forced to lie there and marinate in your own shameful memories, you find with great relief you can begin to move your fingers again.  Eventually with some effort you manage to work power into each corner of your body and finally shake free of the basilisk's curse; quickly, you pick yourself up and redress before anything else finds you and woozily begin to make your way back down the mountain.  The smell of horny goblin on you is a lingering reminder of what just happened to you.\n\n", false);
	...
	private basiliskAdvantageMinotaur(): void {
		...
		MainScreen.text("Eventually the minotaur's balls swell against your " + ButtDescriptor.describeButt(player) + " and with a long, satisfied moo, it reaches its peak, holding you down so its cum jets forth deep inside you.  ", false);
		if (player.gender == 1) MainScreen.text("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously ejaculate, ropes of your own cum spattering on the ground and your own nerveless " + LowerBodyDescriptor.describeFeet(player) + ".", false);
		else if (player.gender == 2) MainScreen.text("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, your juices spattering your thighs and calves.", false);
		else if (player.gender == 3) MainScreen.text("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, ropes of your own jizz spattering on the ground and girl cum dripping down your thighs.", false);
		else MainScreen.text("The sensual high it has already instilled in you amplifies by tenfold as the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, your anus helplessly milking the creature for all it can get.", false);
		MainScreen.text("\n\n", false);
	...
	private driderPCEggLaysBasilisk(): void {
		...
		MainScreen.text("Leaning back on your carapace-clad abdomen, you try and look over your defeated opponent.  Even slumped over in ");
		if (monster.HP < 1) MainScreen.text("exhaustion");
		else MainScreen.text("arousal");
		MainScreen.text(", the basilisk is still striving to meet your eyes with its own.  Clenching your teeth, you avert your eyes and stomp forward blindly, colliding roughly with the spined lizard, knocking him flat on his back on the rocky ground.");
		...
		MainScreen.text(" up against his half-open mouth, smearing your free-running juices across his scaled face.  You wait a few seconds, just enjoying the feeling of his hot breath against your oversensitive sex, before you state your intentions in an imperious tone.  \"<i>Lick.</i>\"");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("wrap tightly around your waiting prick");
		else MainScreen.text("burrow deep into your needy slit");
		MainScreen.text(".  You coo in delight at the sensation, and begin to grind yourself against his face as you wait for your egg-laying to begin.");
		MainScreen.text("\n\nThe monster's stomach bloats as a final rush of goo fills his bowels in preparation for your ");
		if (player.fertilizedEggs() == 0) MainScreen.text("un");
		MainScreen.text("fertilized orbs, providing you with a moment's warning before the ecstasy-inducing ripple of an egg sliding deep into him rolls your eyes backwards in pleasure.  You twitch and squirm atop the bound basilisk, mashing your ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("dick");
		else MainScreen.text("cunt");
		MainScreen.text(" against his face while your egg-laden tube pumps away, each pulse forcing another egg through his squeezing ring and sending trills of intoxicating euphoria up your spine.  You lose yourself in the act, your heartbeat racing in time with contractions of your ovipositor, spraying ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("semen");
		else MainScreen.text("femcum");
		MainScreen.text(" across your partner's face and neck.");
		...
		//(if corruption >= 50 
		if (player.stats.cor >= 50) MainScreen.text(" before heading off towards your camp. The harpies will probably free him from your webs... eventually.");
		else MainScreen.text(".  You reach up and slice him free from the webbing, carefully placing him on the ground below.  The blindfold stays, though.  You're kind, not stupid.  With your good deed for the day complete, you gather yourself up and head back to camp.");
		player.dumpEggs();
		player.orgasm();
		cleanupAfterCombat();
	}
	...
	private layBeeEggsInABasilisk(): void {
		...
		if (player.lowerBody.isTaur() || player.gender == 0) MainScreen.text("\n\nYou drool messily on your hand and cup it to the basilisk's hole; he hisses in displeasure - though he must have had an inkling of what was coming, the visceral sensation of having his asshole lubed for entry is probably sending his imagination careening off in reckless, tormenting anticipation.");
			...
			if (player.lowerBody.cockSpot.count()) MainScreen.text("s");
			MainScreen.text("; rubbing enthusiastically, you start at the base but end in a gentle stroke at the sensitive tip.  With you doing your best to milk as much pre as you can, the viscous ooze that dribbles out of your dong");
			if (player.lowerBody.cockSpot.count()) MainScreen.text("s");
			MainScreen.text(" lands with soft pitters on the overgrown lizard's back, the intense mountain sun baking the gunk onto the scales.  You sigh in languor, but remember you're doing this for a reason.");
		}
		//(else if pc has vag:
		...
		if (player.lowerBody.isTaur()) MainScreen.text("above");
		else MainScreen.text("behind");
		MainScreen.text(" the desperately-squirming lizard, delighting in the fear-scented air around the beast as your ovipositor slaps his cheeks in search of his tight little black hole.  Aside from the difference in feel, you know once you hit the sweet spot by the rigid jolt from the basilisk as his worst fear is confirmed.  You openly laugh this time, and enjoy the feel of the lizard squirming under you as your bulbous appendage painstakingly, slowly works its way inside his anus.  Dry and tight, you really have to force your way, but the feeling is amazing, and it just keeps getting better.  It probably doesn't feel too great for the basilisk, but that's not exactly your problem.  The slit at the end of your ovipositor widens, leaking golden liquid, and the squirming basilisk halts its struggles and relaxes as the narcotics in the slime work their way into the bloodstream.  You begin humming in a fashion similar to the giant bees you now resemble as you enjoy the blissful sensation of the eggs progressing through the long tubular organ, stretching the reptile's pucker to accommodate your ");
		if (player.fertilizedEggs()) MainScreen.text("fertilized offspring");
		else MainScreen.text("burdensome load");
		MainScreen.text(".");
		...
		if (player.lowerBody.isTaur() || player.gender == 0) {
			MainScreen.text("\n\nThere's nothing to distract you from the pleasure; ");
			//[(U)
			if (player.gender == 0) MainScreen.text("no needy cock or cunt demanding a caress - except the basilisk's, and you care not a bit about that one.");
			else if (player.lowerBody.isTaur()) MainScreen.text("even if you wanted to reach your genitals, your arms resolutely refuse to bend that way, so you focus on the slippery, silky feelings travelling along your ovipositor.");
		}
		if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text("\n\nYour seeping member");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
			MainScreen.text(" demand");
			if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("s");
			MainScreen.text(" attention, and you don't deny them, as you resume the pleasurable stroking you started earlier.  More white cream leaks out, landing on the earth and the basilisk's back.  The combined titillation of your ovipositor massaging the overgrown lizard's anus from the inside out and the stroking of " + CockDescriptor.describeMultiCockSimpleOne(player) + "  is more than your overly-sensitised body can handle for long.  Soon, the tell-tale feeling at the base of your spine ");
			if (player.lowerBody.balls > 0) MainScreen.text("and inside your balls ");
			MainScreen.text("is more than you can take, and ropes of thick semen coat the basilisk from the cheeks of its ass to the back of its head.  You give a guttural groan as your orgasm hurries the last of your eggs up the long black organ that is your ovipositor to be deposited into the tightly packed cavern that you've made the basilisk's intestines into.");
		}
		//(no-horse vag: 
		else if (player.lowerBody.vaginaSpot.hasVagina()) {
