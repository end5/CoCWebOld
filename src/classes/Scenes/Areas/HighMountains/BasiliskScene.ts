	...
	public class BasiliskScene extends BaseContent
	{
		...
		private tongueBasiliskSmex():void {
			...
			//Female: 
				...
				MainScreen.text("\"<i>Lick me,</i>\" you tell the basilisk brusquely, as you push your hips out and present your " + vaginaDescript(0) + " to its tongue.  You decide you'll take your reward standing up; the sight of the beast on its knees in front of you is quite pleasing.\n\n", false);
		...
		private defeatBasiliskAndAnal():void {
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				...
				if(player.lowerBody.cockSpot.list[x].cockLength <= 13) {
					MainScreen.text("Soon your " + hipDescript() + " are clapping a staccato rhythm against its warm, muscly butt,", false);
					if(player.lowerBody.balls > 0) MainScreen.text(" your " + ballsDescriptLight() + " slapping against it,", false);
				...
				//(More than 13 inches:)
					...
					//[(if balls)
					if(player.lowerBody.balls > 0) MainScreen.text("Your " + ballsDescriptLight() + " swing heavily underneath your shaft, swelling as your rhythm picks up.  ", false);
				...
				//<(13 inches or less:) 
				if(player.lowerBody.cockSpot.list[x].cockLength <= 13) MainScreen.text("gripping its hips and pushing every inch of yourself in and reaching a glorious, skin tingling peak.", false);
				//(More than 13 inches:)
		...
		private defaultBasiliskRape():void {
			MainScreen.text("Working briskly, the basilisk tears off your " + player.armorName + " until you are entirely naked.  It then rummages through your pockets; it carelessly discards everything it finds without apparent interest.  It grabs a handful of gems from your purse and then prowls back to you.\n\n", false);
		
			//Male/Herm: 
			if(player.lowerBody.cockSpot.hasCock()) {
				MainScreen.text("With surprising gentleness and deftness, the basilisk rubs your " + cockDescript(0) + " with one palm", false);
				if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" and sticks some of the smaller fingers of its other hand in your " + vaginaDescript(0), false);
		...
		private basiliskHasVagEggStuff():void {
			spriteSelect(75);
			player.slimeFeed();
			MainScreen.text("The basilisk is breathing heavily as it tears your " + player.armorName + " from your body, its warm exhalations rolling over your naked flesh.  It seems to be having difficulty controlling itself; from your frozen gaze you can see it constantly shifting its dreadful slit eyes back to your frame as it searches through your pockets with claws that tremble.  Eventually it throws down your attire and stares back into your eyes.  There is something else in there now; a pulsing lust, hints of red at the edges of that great, grey sea, a rapacious tide gathering.  You wish you could look away but there is more chance of you moving mountains.", false);
			//(Heat: 
			...
			//<(Tight: 
			if(player.looseness() < 4) MainScreen.text("Though it is not girthy, it is a perfect fit for your tight hole, and its long length coupled with your drawn up position has you panting as it touches your deepest, most sensitive depths.", false);
			//(Loose: 
		...
		public basiliskBirth():void {
			...
			if(player.perks.has("BasiliskWomb")) {
				...
				if(flags[FlagEnum.BENOIT_EGGS] > 0) MainScreen.text(" as well");
		...
		private basiliskBadEnd():void {
			...
			if(player.gender == 3) MainScreen.text("  You moan through your still throat as you feel blood rushing to your groin, your " + multiCockDescriptLight() + " stiffening and your " + vaginaDescript(0) + " beginning to drip.", false);
			if(player.gender == 1) MainScreen.text("  You moan through your still throat as you feel blood rushing to your groin, your " + multiCockDescriptLight() + " stiffening.", false);
			if(player.gender == 2) MainScreen.text("  You moan through your still throat as you feel blood rushing to your groin, your " + vaginaDescript(0) + " beginning to drip.", false);
		...
		private basiliskAdvantageHarpy():void {
			...
			if(player.lowerBody.cockSpot.hasCock()) {
				...
				//Multicock: 
					...
					MainScreen.text("The three of them do eventually work out a compromise however, once they discover you have more than one manpole to your name.  One pulls your " + cockDescript(0) + " forwards painfully until it is almost pointing towards your face before squatting over it, whilst another clutches at your " + cockDescript(1) + " as she spreads her legs and works her way inwards until she is spearing herself on you. You can't see them - you can't see anything except pink wobbly flesh and lavender feathers - but you can hear them shift impatiently around each other as they find a position which is comfortable to them both, their inner walls rubbing you from every direction as they move.  The third continues to thrust her needy sex into your face as best she can", false);
			...
			//Genderless: 
				...
				MainScreen.text("The harpy comes to a halt behind you and begins to eagerly run her cold but soft hands over your bottom half, stroking your thighs and squeezing your " + buttDescript() + ", as if appraising a piece of meat.  Whilst caressing your neck she runs her hands between your legs and grabs around your groin, searching for something that isn't there.  She lets loose a squawk of pure frustration and wheels around you to glare in your eyes angrily.  The sex-crazed harridan is clearly deeply irritated about finding such a helpless victim only to further discover they lack a cock which she can abuse.  She shifts her eyes to your mouth, but quickly arrives at the same conclusion you've already come to: unable to move your mouth, you can't even be forced to give oral pleasure.  You feel a bizarre sense of triumph over the creature; you stare into space smugly as the harpy paces in front of you, glaring, thwarted but unwilling to give up her prize.  Perhaps eventually she will leave you alone...?\n\n", false);
		...
		private basiliskAdvantageGoblin():void {
			...
			//Unsexed: 
				...
				MainScreen.text("Eventually the minotaur's balls swell against your " + buttDescript() + " and with a long, satisfied moo, he reaches his peak, holding you down so his cum jets forth deep inside you.  The sensual high it has already instilled in you amplifies by tenfold as the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, your anus helplessly milking the creature for all it can get.\n\n", false);
				...
				//(Addict: 
				if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] > 0 || player.perks.has("MinotaurCumAddict")) MainScreen.text("  You're incredibly frustrated that you can't clench yourself and hold the magical substance deep inside you, so you can savor the wonderful, soft elation it blossoms inside of you for as long as you can.", false);
				MainScreen.text("\n\n", false);
		...
		private basiliskAdvantageMinotaur():void {
			...
			MainScreen.text("Eventually the minotaur's balls swell against your " + buttDescript() + " and with a long, satisfied moo, it reaches its peak, holding you down so its cum jets forth deep inside you.  ", false);
			if(player.gender == 1) MainScreen.text("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously ejaculate, ropes of your own cum spattering on the ground and your own nerveless " + player.feet() + ".", false);
		...
		private layBeeEggsInABasilisk():void {
			...
			if(player.upperBody.wingType == WING.BEE_LIKE_SMALL || player.upperBody.wingType == WING.BEE_LIKE_LARGE ) MainScreen.text("buzz lightly over to the basilisk's writhing body");
			...
			if(player.isTaur() || player.gender == 0) MainScreen.text("\n\nYou drool messily on your hand and cup it to the basilisk's hole; he hisses in displeasure - though he must have had an inkling of what was coming, the visceral sensation of having his asshole lubed for entry is probably sending his imagination careening off in reckless, tormenting anticipation.");
				...
				if(player.totalCocks()) MainScreen.text("s");
			...
			if(player.isTaur()) MainScreen.text("above");
			...
			if(player.isTaur() || player.gender == 0) {
				...
				else if(player.isTaur()) MainScreen.text("even if you wanted to reach your genitals, your arms resolutely refuse to bend that way, so you focus on the slippery, silky feelings travelling along your ovipositor.");
			}
			if(player.lowerBody.cockSpot.hasCock()) {
				MainScreen.text("\n\nYour seeping member");
