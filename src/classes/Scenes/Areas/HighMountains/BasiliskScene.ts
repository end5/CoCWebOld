...
package classes.Scenes.Areas.HighMountains
{
	...
	public class BasiliskScene extends BaseContent
	{
		...
		public defeatBasilisk():void {
			...
			//Player HP victory: 
			if(monster.HP < 1) Render.text("Unable to stand anymore, the basilisk shakily sinks down on one knee, drops its head and looks at the ground, evidently demonstrating submission.", false);
			//Player Lust victory: 
			else Render.text("No longer able to control its raging erection, the basilisk closes its eyes and sinks to one knee.  It would probably be attempting to signal its submission to you if it weren't furiously masturbating its long, purple cock, which has emerged straining from the creature's genital slit.", false);
		
			//If victory and Player Lust above 30: 
			if(player.lust >= 33 && player.gender > 0) {
				Render.text("  Certain that the creature won't dare try and turn its eyes on you again, you take your time to look the tall reptile over directly for the first time.  Perhaps you could use it to satisfy your baser urges. If so, what part of it do you choose?", false);
				//[Tongue][Ass]
		...
		private tongueBasiliskSmex():void {
			spriteSelect(75);
			Render.text("", true);
			Render.text("You tap your jaw as you stare down at the defeated reptile, smiling at its oddly formal gesture of surrender.  You'd very much like to take some satisfaction from the basilisk, extract a bit of pleasurable payment for what it tried to do to you: but the more you get to look at it properly, the more difficult doing that seems. The tall, thin creature is all angles, tough scales and pointed edges. You don't like the thought of putting your genitals anywhere near its sickle claws or curved fangs.\n\n", false);
		
			Render.text("It is as you are taking in this latter article that the basilisk swallows nervously, and you get a glimpse inside its mouth at a long, sticky, pale purple tongue, another gift of its reptilian ancestors. It's the only part you've seen of it so far which looks... soft.  Pliable.  Your smile widens and you step forward, slowly loosening your " + player.armorName + ".\n\n", false);
		...
		private basiliskHasVagEggStuff():void {
			spriteSelect(75);
			player.slimeFeed();
			Render.text("The basilisk is breathing heavily as it tears your " + player.armorName + " from your body, its warm exhalations rolling over your naked flesh.  It seems to be having difficulty controlling itself; from your frozen gaze you can see it constantly shifting its dreadful slit eyes back to your frame as it searches through your pockets with claws that tremble.  Eventually it throws down your attire and stares back into your eyes.  There is something else in there now; a pulsing lust, hints of red at the edges of that great, grey sea, a rapacious tide gathering.  You wish you could look away but there is more chance of you moving mountains.", false);
			//(Heat: 
			if(player.inHeat) Render.text("  You are more aware than ever of an invisible scent simmering off you, of your wet vagina clenching and wetting itself in anticipation, your body begging this male creature to fulfil its genetic objective upon you.  Your eyes have betrayed you, your body is betraying you, and whatever else you are is a tiny, ignored voice screaming in between.", false);
			Render.text("  The basilisk suddenly breaks away and kneels down in front of you.  Out of sight of your petrified eyes you cannot see what it is doing; however a moment later, you can feel, as a warm, sticky sensation slavers over your abdomen.", false);
			//(egg preg:
			if(player.pregnancyIncubation > 1 && player.pregnancyType == PregnancyType.OVIELIXIR_EGGS) Render.text("  The basilisk licks your bulging belly hungrily, pushing against and testing for the eggs you are carrying.  Your sensitive cargo shifts around under its hungry attention; you'd squirm, but that is, of course, impossible.", false);
			//(heat or perk:
			if(player.inHeat || player.perks.has("Oviposition") || player.perks.has("BasiliskWomb")) Render.text("  The basilisk licks your belly hungrily, its sticky tongue crawling like a warm tentacle across your sensitive underside.  You'd squirm, but that is impossible.  The creature is making you feel everything it is forcing upon you.", false);
			Render.text("\n\n", false);
			
			Render.text("The basilisk gets up and again stares back into your eyes and you feel its will press indomitably against your pliable mind.  At the very edge of your vision, underneath the creature you can see a hint of shiny purple; the creature's cock has slid out of its genital slit.  How long it is and what it looks like you cannot see, but as with another raspy moan you finally accept what is about to happen, there is no doubt you are going to get to know about it very personally.\n\n", false);
		...
		public basiliskBirth():void {
			...
			if(player.lowerBody.vaginaSpot.count() == 0) {
				Render.text("You feel a terrible pressure in your groin... then an incredible pain accompanied by the rending of flesh.  <b>You look down and behold a new vagina</b>.\n\n", false);
				player.createVagina();
				player.genderCheck();
			}
			if(player.perks.has("BasiliskWomb")) {
				...
				if(player.lowerBody.cockSpot.hasCock()) Render.text(" and [eachCock] starting to leak pre-cum");
				Render.text(" as you find yourself being moved to climax by the birthing.  You see no point in resisting and reach down to begin fiddling with yourself, moaning in pain-spiked pleasure as the stimulus overwhelms you. With an orgasmic cry, you release your eggs into the world amidst a gush of femcum");
				if(player.lowerBody.cockSpot.hasCock()) Render.text(" and a downpour of hermcum");
				Render.text(".");
				
				Render.text("\n\nWhen you find yourself able to stand, you examine what it is you have birthed; ");
				//(eggNumber) 
				Render.text(num2Text(Math.floor(player.totalFertility() / 10)));
				Render.text(" large, jade-colored eggs, the unmistakable shape of reptile eggs. You pick up one and hold it gently against your ear; inside, you can hear a little heart, beating strong and quick.  You put it down carefully with its fellows and stare at your clutch, a queasy tangle of emotions tugging at you.");
		
				//First time:
				if(flags[FlagEnum.BENOIT_EGGS] + flags[FlagEnum.BENOIT_GENERIC_EGGS] == 0) {
				...
				if(flags[FlagEnum.BENOIT_EGGS] > 0) Render.text(" as well");
				Render.text(".  You place the egg back down and gather them all up, moving them closer to the campfire to stay warm while you recover from your exertions.");
				...
				//First Time:  
				if(flags[FlagEnum.BENOIT_GENERIC_EGGS] == 0) {
		...
		private basiliskAdvantageHarpy():void {
			...
			//Genderless: 
				...
				Render.text("You feel something build at the back of your mind.  Involuntary sensations prickle your skin as, once again, the intermittent mental backwash that the basilisk's hypnosis has forced upon your mind hits you.  Memory after memory of sexually-charged encounters, daydreams or fantasies crowd your consciousness.  It is made worse, much worse by the pheromones the harpy has pushed into your mouth; your body rides the chemical glow at the same time as imaginings of soft skin, tight muscle and musk overload your senses.  The denial the petrification has forced upon you is becoming unbearable.\n\n", false);
				...
				Render.text("The harpy manages to cum twice on your hand, gobbling with excitement as she spatters your arm with her juices.  During this time you are forced to ride the potent cocktail of hypnotic sexual compulsion and the harpy's golden lipstick again and again, until you feel you would have collapsed in a pool of steaming sex long ago if your knees would only allow it.  Once she is finished with you the harpy clambers down, taking care to wipe her leaking twat on your naked front as she does so, before flapping off with a winsome smirk, entirely ignoring your own plight.\n\n", false);
		...
		private basiliskAdvantageGoblin():void {
			...
			//Unsexed: 
				...
				Render.text("\"<i>A " + player.race() + "!</i>\" she yelps.  Hesitantly at first, then with increasing confidence as how vulnerable you are sinks in, she does a round circuit of you, taking in every angle of your frozen, helpless flesh.\n\n", false);
		...
		private basiliskAdvantageMinotaur():void {
			...
			Render.text("Eventually the minotaur's balls swell against your " + buttDescript() + " and with a long, satisfied moo, it reaches its peak, holding you down so its cum jets forth deep inside you.  ", false);
			if(player.gender == 1) Render.text("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously ejaculate, ropes of your own cum spattering on the ground and your own nerveless " + player.feet() + ".", false);
			else if(player.gender == 2) Render.text("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, your juices spattering your thighs and calves.", false);
			else if(player.gender == 3) Render.text("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, ropes of your own jizz spattering on the ground and girl cum dripping down your thighs.", false);
			else Render.text("The sensual high it has already instilled in you amplifies by tenfold as the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, your anus helplessly milking the creature for all it can get.", false);
			Render.text("\n\n", false);	
		...
		private driderPCEggLaysBasilisk():void {
			spriteSelect(75);
			clearOutput();
			Render.text("Leaning back on your carapace-clad abdomen, you try and look over your defeated opponent.  Even slumped over in ");
			if(monster.HP < 1) Render.text("exhaustion");
			else Render.text("arousal");
			Render.text(", the basilisk is still striving to meet your eyes with its own.  Clenching your teeth, you avert your eyes and stomp forward blindly, colliding roughly with the spined lizard, knocking him flat on his back on the rocky ground.");
			...
			Render.text(" up against his half-open mouth, smearing your free-running juices across his scaled face.  You wait a few seconds, just enjoying the feeling of his hot breath against your oversensitive sex, before you state your intentions in an imperious tone.  \"<i>Lick.</i>\"");
			...
			if(player.lowerBody.cockSpot.hasCock()) Render.text("wrap tightly around your waiting prick");
			else Render.text("burrow deep into your needy slit");
			Render.text(".  You coo in delight at the sensation, and begin to grind yourself against his face as you wait for your egg-laying to begin.");
			Render.text("\n\nThe monster's stomach bloats as a final rush of goo fills his bowels in preparation for your ");
			if(player.fertilizedEggs() == 0) Render.text("un");
			Render.text("fertilized orbs, providing you with a moment's warning before the ecstasy-inducing ripple of an egg sliding deep into him rolls your eyes backwards in pleasure.  You twitch and squirm atop the bound basilisk, mashing your ");
			if(player.lowerBody.cockSpot.hasCock()) Render.text("dick");
			else Render.text("cunt");
			Render.text(" against his face while your egg-laden tube pumps away, each pulse forcing another egg through his squeezing ring and sending trills of intoxicating euphoria up your spine.  You lose yourself in the act, your heartbeat racing in time with contractions of your ovipositor, spraying ");
			if(player.lowerBody.cockSpot.hasCock()) Render.text("semen");
			else Render.text("femcum");
			Render.text(" across your partner's face and neck.");
			...
			//(if corruption >= 50 
			if(player.stats.cor >= 50) Render.text(" before heading off towards your camp. The harpies will probably free him from your webs... eventually.");
			else Render.text(".  You reach up and slice him free from the webbing, carefully placing him on the ground below.  The blindfold stays, though.  You're kind, not stupid.  With your good deed for the day complete, you gather yourself up and head back to camp.");
			player.dumpEggs();
			player.orgasm();
			cleanupAfterCombat();
		}
		...
		private layBeeEggsInABasilisk():void {
			...
			if(player.isTaur() || player.gender == 0) Render.text("\n\nYou drool messily on your hand and cup it to the basilisk's hole; he hisses in displeasure - though he must have had an inkling of what was coming, the visceral sensation of having his asshole lubed for entry is probably sending his imagination careening off in reckless, tormenting anticipation.");
				...
				if(player.totalCocks()) Render.text("s");
				Render.text("; rubbing enthusiastically, you start at the base but end in a gentle stroke at the sensitive tip.  With you doing your best to milk as much pre as you can, the viscous ooze that dribbles out of your dong");
				if(player.totalCocks()) Render.text("s");
				Render.text(" lands with soft pitters on the overgrown lizard's back, the intense mountain sun baking the gunk onto the scales.  You sigh in languor, but remember you're doing this for a reason.");
			}
			//(else if pc has vag:
			...
			if(player.isTaur()) Render.text("above");
			else Render.text("behind");
			Render.text(" the desperately-squirming lizard, delighting in the fear-scented air around the beast as your ovipositor slaps his cheeks in search of his tight little black hole.  Aside from the difference in feel, you know once you hit the sweet spot by the rigid jolt from the basilisk as his worst fear is confirmed.  You openly laugh this time, and enjoy the feel of the lizard squirming under you as your bulbous appendage painstakingly, slowly works its way inside his anus.  Dry and tight, you really have to force your way, but the feeling is amazing, and it just keeps getting better.  It probably doesn't feel too great for the basilisk, but that's not exactly your problem.  The slit at the end of your ovipositor widens, leaking golden liquid, and the squirming basilisk halts its struggles and relaxes as the narcotics in the slime work their way into the bloodstream.  You begin humming in a fashion similar to the giant bees you now resemble as you enjoy the blissful sensation of the eggs progressing through the long tubular organ, stretching the reptile's pucker to accommodate your ");
			if(player.fertilizedEggs()) Render.text("fertilized offspring");
			else Render.text("burdensome load");
			Render.text(".");
			...
			if(player.isTaur() || player.gender == 0) {
				...
				//[(U)
				if(player.gender == 0) Render.text("no needy cock or cunt demanding a caress - except the basilisk's, and you care not a bit about that one.");
				else if(player.isTaur()) Render.text("even if you wanted to reach your genitals, your arms resolutely refuse to bend that way, so you focus on the slippery, silky feelings travelling along your ovipositor.");
			}
			if(player.lowerBody.cockSpot.hasCock()) {
				Render.text("\n\nYour seeping member");
