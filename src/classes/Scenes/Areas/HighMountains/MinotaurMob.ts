	...
	public class MinotaurMob extends Monster 
	{
		private precumTease():void {
			...
			if(damage <= 8) {
				...
				if(player.lust > 99) MainScreen.text("  Still, you're too horny to fight any longer.", false);
			}
			//(Medium damage taken)
			else if(damage <= 14) {
				MainScreen.text("The powerful pheromones and scents hanging in the air around you make your body flush hotly.  Your " + player.nippleDescript(0) + "s grow harder", false);
				if(player.lust > 70) MainScreen.text(", though you didn't think such a thing was possible", false);
				else MainScreen.text(", feeling like two bullets scraping along the inside of your " + player.armorName, false);
				...
				if(player.lust > 99) MainScreen.text("  Fuck it, they smell so good.  You want, no, NEED more.", false);
			...
			//(high damage taken)
				...
				if(player.wetness() <= 1) MainScreen.text("moisten", false);
				else if(player.wetness() <= 2) MainScreen.text("drip", false);
				else if(player.wetness() <= 3) MainScreen.text("drool", false);
				...
				if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("  " + SMultiCockDesc() + " twitches and dribbles its own pre-seed, but it doesn't smell anywhere near as good!", false);
				...
				if(player.lust <= 99) MainScreen.text("You pull back from the brink with a start.  It'll take more than a little drugged pre-cum to bring you down!", false);
		...
		private minotaurGangGropeAttack():void {
			...
			if(player.lust >= 80) MainScreen.text(", and your pussy is absolutely soaking wet", false);
