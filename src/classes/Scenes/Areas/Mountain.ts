...
export default class Mountain {
	...
	public exploreMountain(): void {
		...
		if (isHolidays()) {
			//Gats xmas adventure!
			if (rand(5) == 0 && player.gender > 0 && isHolidays() && Flags.list[FlagEnum.GATS_ANGEL_DISABLED] == 0 && Flags.list[FlagEnum.GATS_ANGEL_GOOD_ENDED] == 0 && (Flags.list[FlagEnum.GATS_ANGEL_QUEST_BEGAN] > 0 && player.hasKeyItem("North Star Key") < 0)) {
				kGAMECLASS.gatsSpectacularRouter();
				return;
			}
			if (rand(6) == 0 && Flags.list[FlagEnum.JACK_FROST_YEAR] < date.fullYear && silly()) {
		...
		if (rand(100) <= 77) {
			if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00141] < 3) {
				...
				if (player.upperBody.head.face.faceType == FaceType.DOG && (player.lowerBody.cockSpot.countType(CockType.DOG) >= 2 || (player.lowerBody.vaginaSpot.hasVagina() && player.pregnancyType == PregnancyType.HELL_HOUND)) && player.stats.cor >= 60 && player.lowerBody.tailType == TailType.DOG && (player.lowerBody == LowerBodyType.DOG || player.upperBody.head.hairColor == "midnight black")) {
		...
		if (rand(10) == 0 && !player.statusAffects.has("HairdresserMeeting")) chooser = 4;
		if ((rand(8) == 0 && player.statusAffects.has("MetMarae"))
			&& !player.statusAffects.has("FoundFactory")) {
		...
		if (Flags.list[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] > 0 && rand(10) == 0) {
			chooser = 1;
		}
		...
		if (chooser == 1) {
			spriteSelect(44);
			if (!player.statusAffects.has("TF2") && player.level <= 1 && player.stats.str <= 40) {
				if (silly()) {
			...
			//Mino gangbang
			if (!player.statusAffects.has("MinoPlusCowgirl") || rand(10) == 0) {
				if (Flags.list[FlagEnum.HAS_SEEN_MINO_AND_COWGIRL] == 1 && player.upperBody.head.horns > 0 && player.upperBody.head.hornType == HornType.COW_MINOTAUR && player.upperBody.head.earType == EarType.COW && player.lowerBody.tailType == TailType.COW && player.lactationQ() >= 200 && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3 && player.minotaurAddicted()) {
				...
				if (!player.statusAffects.has("MinoPlusCowgirl")) player.statusAffects.add(new StatusAffect("MinoPlusCowgirl", 0, 0, 0, 0)));
		...
		if (chooser == 2) {
			...
			if (player.statusAffects.has("WormsOn") && !player.statusAffects.has("Infested")) {
	...
	private joinBeingAMinoCumSlut(): void {
		...
		if (player.upperBody.chest.count() > 1) MainScreen.text("uppermost ");
