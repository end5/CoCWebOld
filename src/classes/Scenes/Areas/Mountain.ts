	...
	public class Mountain extends BaseContent
	{
		...
		public function exploreMountain():void
		{
			...
			if ((player.level >= 5 || player.exploredMountain >= 40) && flags[FlagEnum.DISCOVERED_HIGH_MOUNTAIN] == 0) {
				Render.text("While exploring the mountain, you come across a relatively safe way to get at its higher reaches.  You judge that with this route you'll be able to get about two thirds of the way up the mountain.  With your newfound discovery fresh in your mind, you return to camp.\n\n(<b>High Mountain exploration location unlocked!</b>)", true);
			...
			if (chooser == 0) {
				...
				if (rand(10) < impGob) {
					if (player.level >= 8 && rand(2) == 0) {
						...
						Render.text("An imp leaps out from behind a rock and attacks!", true);
						startCombat(new Imp());
					}
					spriteSelect(29);
					return;
			...
			if (chooser == 1) {
				...
				if (player.findStatusAffect(StatusAffects.TF2) < 0 && player.level <= 1 && player.str <= 40) {
					if (silly()) {
						...
						Render.text("Crossing over the treacherous mountain paths, you walk past an ominous cave.  The bones and the smell of death convince you to hasten your pace.  However, as you walk by, you hear a deep bellow and a snort as a monstrous man with a bull's head steps out.  With hell in his eyes and a giant ax in his hand, he begins to approach you in clear rage.  As he comes out into the light, you see that he is completely naked and sports a monstrous erection as angry as the minotaur himself, freely leaking a steady stream of pre-cum as he stalks you.\n\n", true);
						Render.text("You stumble in your attempt to escape and realize that you are completely helpless.  The minotaur towers over you and heaves his ax for a <i>coup de grace</i>.  As he readies the blow, a monstrous explosion rocks the entire mountainside, causing the bull-man to stumble before he can finish you off. You look around, bewildered, trying to understand this strange new turn of events, and notice a group of maybe half a dozen people approaching from further up the path.  They appear to be a motley crew clad in blue and carrying monstrous weapons.  The tallest man holds a weapon made of multiple rotating tubes, and begins spinning the barrels.  A second later, while screaming in a language you do not understand, a rain of lead begins shredding the minotaur into a cloud of blood and flesh.\n\n", false);
						Render.text("An equally imposing black man with a patch over one eye begins firing canisters at the beast, which explode violently.  \"<i>Ya ragged-arsed beast man!</i>\" he taunts.  \"<i>Ye should pick on someone yer own size, BOY-O! HEHEHE!</i>\"\n\n", false);
						Render.text("Coming up the path next is a freak of a person clad in a contained shiny suit with a weapon that burns with flame.  He freely walks into the explosions and gunfire and begins igniting the beast.\n\n", false);
						Render.text("\"<i>MRPHHUMFHRUFH!!!!! HUMFHUFMMRUF!</i>\" the freak mumbles through his mask.\n\n", false);
						Render.text("\"<i>I like me steak well done, ye crazy bugger!</i>\" yells the black man.\n\n", false);
						Render.text("The beast collapses in a charred and bloody heap.   As you stand back up, you hear a strange noise behind you.  You turn around to find a well-dressed man wearing a ski mask and smoking a cigarette.  \"<i>Don't you know ze mountains are dangereuse,</i>\" the man says with a thick accent.  \"<i>You will get FUCKED up here if you are not careful.</i>\"\n\n", false);
						Render.text("You thank the man and his team, but they brush off your gratitude.  \"<i>Non, non!</i>\" the man with the accent says.  \"<i>As zey say, everyone gets ONE.</i>\" With that, he touches the watch on his wrist and disappears.  The rest of the group continues on their way.\n\n", false);
						Render.text("As they leave, the giant with the chain gun yells in a horribly accented manner, \"<i>YOU LEAVE SANDVICH ALONE! SANDVICH IS MINE!</i>\"\n\n", false);
						Render.text("With that, another hail of bullets break the scene as they walk away, leaving you safe from the minotaur, but utterly baffled as to what in hell just happened.", false);
					}
					else {
						Render.text("Crossing over the treacherous mountain paths, you walk past an ominous cave.  The bones and the smell of death convince you to hasten your pace.  However, as you walk by, you hear a deep bellow and a snort as a monstrous man with a bull's head steps out.  With hell in his eyes and a giant ax in his hand, he begins to approach you in clear rage.  As he comes out into the light, you see that he is completely naked and sports a monstrous erection as angry as the minotaur himself, freely leaking a steady stream of pre-cum as he stalks you.\n\n", true);
						Render.text("You stumble in your attempt to escape and realize that you are completely helpless.  The minotaur towers over you and heaves his ax for a <i>coup de grace</i>.  As he readies the blow, another beast-man slams into him from the side.  The two of them begin to fight for the honor of raping you, giving you the opening you need to escape.  You quietly sneak away while they fight – perhaps you should avoid the mountains for now?\n\n", false);
					}
				...
				if (player.findStatusAffect(StatusAffects.MinoPlusCowgirl) < 0 || rand(10) == 0) {
					if (flags[FlagEnum.HAS_SEEN_MINO_AND_COWGIRL] == 1 && player.horns > 0 && player.hornType == HORNS.COW_MINOTAUR && player.upperBody.head.earType == EARS.COW && player.tailType == TAIL.COW && player.lactationQ() >= 200 && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3 && player.minotaurAddicted()) {
						...
						Render.text("As you pass a shadowy cleft in the mountainside, you hear the now-familiar call of a cowgirl echoing from within.  Knowing what's in store, you carefully inch closer and peek around the corner.");
						...
						if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("dampens");
						else Render.text("twinges");
						Render.text(", aching to be filled");
						if (player.lowerBody.cockSpot.hasCock()) Render.text(", while [eachCock] rises to attention, straining at your [armor]");
						Render.text(".");
				...
				if (player.statusAffects.has("WormsOn") && player.findStatusAffect(StatusAffects.Infested) < 0) {
					if (player.statusAffects.has("WormsHalf") && rand(2) == 0) {
						if (player.stats.cor < 90) {
							Render.text("Your hike in the mountains, while fruitless, reveals pleasant vistas and provides you with good exercise and relaxation.", true);
							dynStats("tou", .25, "spe", .5, "lus", player.stats.lib / 10 - 15);
						}
						else {
							Render.text("During your hike into the mountains, your depraved mind keeps replaying your most obcenely warped sexual encounters, always imagining new perverse ways of causing pleasure.\n\nIt is a miracle no predator picked up on the strong sexual scent you are emitting.", true);
		...
		private function joinBeingAMinoCumSlut():void
		{
			...
			if (player.tallness <= 96) Render.text(" despite your immense height");
			Render.text(" as they look you up and down.  The entire area goes silent, even the goblins and the imps that are no doubt watching seem to be holding their breath, wondering what will happen to you.");
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				Render.text("ignoring your cock");
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
				Render.text(" entirely, ");
			}
			Render.text("slipping a finger into your moistening ");
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("pussy");
			else Render.text("asshole");
			Render.text(".  A low moan escapes your lips as a second finger slips in while the busty bovine woman's thumb ");
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("swirls around your clitoris");
			else Render.text("presses against your perineum");
			Render.text(".");
			...
			if (player.lowerBody.cockSpot.hasCock()) Render.text(", once again ignoring your manhood completely");
			Render.text(", dives tongue first into your wet ");
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("quim");
			else Render.text("back door");
			Render.text(".  The movement is so quick that you can't even suppress the sudden, perverted moan that leaves your lips... a moan that sounds shockingly like a cow's moo.  The surprise at your sudden bovine outburst quickly dissipates as the cow-girl's large tongue dips in and out of your ");
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("sodden box");
			else Render.text("moist butthole");
			Render.text(".  Any remaining fears of joining this very public sex show are gone, and you wonder why you didn't join in sooner.");
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("cow-pussy");
			else Render.text("ass");
			Render.text(" right there,</i>\" he groans, more of his bombastic cock slipping deep inside you.  The minotaur hooks an arm under each of your knees, lifting up your lower body, pressing even deeper.  Powerful sensations drift up from your ");
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("g-spot");
			else Render.text("prostate");
			Render.text(" as the minotaur's wide flare strokes it through your ");
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("vaginal");
			else Render.text("anal");
			Render.text(" walls.  Biting your lip with barely contained pleasure, you bring your hands to your breasts, playing with your milk-sodden nipples in between each orgasmic thrust of the bull's hips.");
			...
			Render.text("tongue inside her, eagerly licking out and guzzling down the remnants of the minotaur's present.");
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("womb");
			else Render.text("back door");
			Render.text(" increases slowly, and you can feel your limbs tingling at the prospect of your mino-cum-induced orgasm.");
			...
			Render.text(" like a Christmas tree.  You're acutely aware of every single movement, every pulse, every little bit of contact between you and the huge cock living inside you.");
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("womb");
			else Render.text("bowels");
			Render.text(".  The moment that long-awaited jism hits your walls, it's like another lightning bolt hits.  It travels up your spine and sets your entire brain aglow.  Ecstasy wrapped in bliss with a side of euphoric rapture consumes your thoughts.  Your vision goes white, pearly white like the seed filling your body, and your lips part as a primal \"<i>moo</i>\" slips out.");
			...
			if (!player.lowerBody.vaginaSpot.hasVagina()) Render.text("new ");
			Render.text("womb with his divine dick.");
			...
			if (player.bRows() > 1) Render.text("uppermost ");
			Render.text("breasts around his monolithic shaft.  Your faces meet and her soft lips press against yours, each of you earnestly pressing your tongues into the other's mouths, swapping the juices you've collected over the past hour or so.  The bull beneath you groans, awakening to the feeling of four breasts surrounding his love muscle.");
			...
			if (flags[FlagEnum.PC_FETISH] > 0) {
				Render.text("  A thrill runs through you.  Even though you were brought to such a satisfying climax, the whole thought that goblins and imps were watching you and getting off on it... it just makes you hornier than you were before.");
				dynStats("lus=", 100);
			}
