...
//[RANDOM SCENE IF CHARACTER HAS AT LEAST ONE COCK LARGER THAN THEIR HEIGHT, AND THE TOTAL COMBINED WIDTH OF ALL THEIR COCKS IS TWELVE INCHES OR GREATER]
		internal function bigJunkForestScene(lake:boolean = false):void
		{
			...
			if (player.lowerBody.cockSpot.count() == 1) {
				if (lake) outputText("  As it drags through the lakeside mud, the sensation forces you to imagine the velvety folds of a monstrous pussy sliding along the head of your " + Appearance.cockNoun(player.lowerBody.cockSpot.list[x].cockType) + ", gently attempting to suck it off.", false);
				else outputText("  As it drags across the grass, twigs, and exposed tree roots, the sensation forces you to imagine the fingers of a giant hand sliding along the head of your " + Appearance.cockNoun(player.lowerBody.cockSpot.list[x].cockType) + ", gently jerking it off.", false);
			}
			else if (player.lowerBody.cockSpot.count() >= 2) {
				if (lake) outputText("  With all of your " + multiCockDescriptLight() + " dragging through the mud, they begin feeling as if the lips of " + num2Text(player.lowerBody.cockSpot.count()) + " different cunts were slobbering over each one.", false);
				else outputText("  With all of your " + multiCockDescriptLight() + " dragging across the grass, twigs, and exposed tree roots, they begin feeling as if the rough fingers of " + num2Text(player.lowerBody.cockSpot.count()) + " different monstrous hands were sliding over each shaft, gently jerking them off.", false);
			}
			outputText("\n\n", false);
		...
		private function ignoreSatyr():void {
			...
			if(player.stats.cor < 50) outputText("disgusted by the strange thoughts this place seems to put into your mind", false);
		...
		private function rapeSatyr():void {
			...
			//(Low Corruption)
			if(player.stats.cor < 33) outputText("For a moment you hesitate... taking someone from behind without their consent seems wrong... but then again you doubt a satyr would pass on the opportunity if you were in his position.", false);
			//(Medium Corruption)
			else if(player.stats.cor < 66) outputText("You smirk; normally you would have given this some thought, but the idea of free booty is all you need to make a decision.", false);
			//High Corruption
			...
			if(player.lowerBody.balls > 0) outputText("your balls churn, ", false);
			outputText("and you grunt as you feel the first shots of cum flowing along " + sMultiCockDesc() + ", only to pour out into", false);
			if(player.lowerBody.cockSpot.count() > 1) outputText(" and onto", false);
