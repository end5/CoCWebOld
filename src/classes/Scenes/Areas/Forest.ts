...
export default class Forest {
	...
	public exploreDeepwoods(): void {
		...
		if (player.statusAffects.get("ExploredDeepwoods").value1 % 10 == 0 && isHalloween()) {
			...
			if (!player.perks.has("FerasBoonBreedingBitch") && !player.perks.has("FerasBoonAlpha")) {
				if (date.fullYear > Flags.list[FlagEnum.PUMPKIN_FUCK_YEAR_DONE]) {
			...
			//Fera is free!
				...
				if (Flags.list[FlagEnum.FERAS_TRAP_SPRUNG_YEAR] == 0) {
					if (date.fullYear > Flags.list[FlagEnum.FERAS_GLADE_EXPLORED_YEAR]) {
		...
		if (Flags.list[FlagEnum.DISCOVERED_DUNGEON_2_ZETAZ] == 0 && player.statusAffects.get("ExploredDeepwoods").value1 % 5 == 0 && player.statusAffects.has("DungeonShutDown")) {
			...
			Flags.list[FlagEnum.DISCOVERED_DUNGEON_2_ZETAZ]++;
			return;
		}
		//Tamani 20% encounter rate
		if (Flags.list[FlagEnum.TAMANI_TIME_OUT] == 0 && rand(5) == 0 && player.gender > 0 && (player.lowerBody.cockSpot.count() > 0 || player.hasKeyItem("Deluxe Dildo") < 0)) {
			if (player.lowerBody.cockSpot.count() > 0 && Flags.list[FlagEnum.TAMANI_DAUGHTER_PREGGO_COUNTDOWN] == 0 && Flags.list[FlagEnum.TAMANI_NUMBER_OF_DAUGHTERS] >= 24) {
		...
		if (Flags.list[FlagEnum.ERLKING_DISABLED] == 0 && Flags.list[FlagEnum.ERLKING_ENCOUNTER_COUNTER] == 4) {
			Flags.list[FlagEnum.ERLKING_ENCOUNTER_COUNTER] = 0;
			erlkingScene.encounterWildHunt();
			return;
		}
		else {
			Flags.list[FlagEnum.ERLKING_ENCOUNTER_COUNTER]++;
		}
		...
		if (chooser == 1) {
			...
			if (player.gender > 0) Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00247] = 0;
	...
	public exploreForest(): void {
		...
		if (player.statusAffects.has("PureCampJojo") || Flags.list[FlagEnum.JOJO_DEAD_OR_GONE] == 1) {
			chooser = rand(3);
			if (chooser >= 1) chooser++;
		}
		//Chance to discover deepwoods
		if ((player.exploredForest >= 20) && !player.statusAffects.has("ExploredDeepwoods")) {
		...
		if ((rand(100) <= 1) && player.gender > 0 && (Flags.list[FlagEnum.ESSY_MET_IN_DUNGEON] == 0 || Flags.list[FlagEnum.TOLD_MOTHER_TO_RELEASE_ESSY] == 1)) {
		...
		//Marble randomness
		if (player.exploredForest % 50 == 0 && player.exploredForest > 0 && !player.statusAffects.has("MarbleRapeAttempted") && !player.statusAffects.has("NoMoreMarble") && player.statusAffects.has("Marble") && Flags.list[FlagEnum.MARBLE_WARNING] == 0) {
		...
		if (chooser == 0) {
			...
			if (player.lowerBody.cockSpot.count() > 0) impGob--;
			...
			//Encounter Gobbalin!
				...
				if (rand(4) <= 0 && Flags.list[FlagEnum.TAMANI_TIME_OUT] == 0 && player.gender > 0 && (player.lowerBody.cockSpot.count() > 0 || player.hasKeyItem("Deluxe Dildo") < 0)) {
					if (player.lowerBody.cockSpot.count() > 0 && Flags.list[FlagEnum.TAMANI_DAUGHTER_PREGGO_COUNTDOWN] == 0 && Flags.list[FlagEnum.TAMANI_NUMBER_OF_DAUGHTERS] >= 24) {
						tamaniDaughtersScene.encounterTamanisDaughters();
					}
					else
		...
		if (chooser == 1) {
			...
			if (kGAMECLASS.monk == 0) {
				...
				MainScreen.text("While marvelling at the strange trees and vegetation of the forest, the bushes ruffle ominously.  A bush seems to explode into a flurry of swirling leaves and movement.  Before you can react you feel your " + LowerBodyDescriptor.describeFeet(player) + " being swept out from under you, and land hard on your back.\n\n", false);
				...
				if (player.gender > 0) {
					trace("Gender != 0");
					simpleChoices("Accept", Game.jojoScene.meditateInForest, "Rape Him", Game.jojoScene.jojoRape, "BWUH?", null, "Decline", camp.returnToCampUseOneHour, "", null);
				}
				else {
					trace("Gender == 0");
					simpleChoices("Accept", Game.jojoScene.meditateInForest, "Rape Him", null, "BWUH?", null, "Decline", camp.returnToCampUseOneHour, "", null);
				}
				return;
			}
			if (kGAMECLASS.monk == 1) {
				if (player.statusAffects.has("Infested")) {
					...
					if (player.gender > 0) simpleChoices("Purge", Game.jojoScene.wormRemoval, "Meditate", Game.jojoScene.meditateInForest, "Rape", Game.jojoScene.jojoRape, "", null, "Leave", camp.returnToCampUseOneHour);
					else simpleChoices("Purge", Game.jojoScene.wormRemoval, "Meditate", Game.jojoScene.meditateInForest, "Rape", null, "", null, "Leave", camp.returnToCampUseOneHour);
					return;
				}
				kGAMECLASS.jojoScene.jojoSprite();
				...
				if (player.gender > 0) simpleChoices("Yes", Game.jojoScene.meditateInForest, "No", camp.returnToCampUseOneHour, "BWUH", null, "Rape Him", Game.jojoScene.jojoRape, "", null);
				else simpleChoices("Yes", Game.jojoScene.meditateInForest, "No", camp.returnToCampUseOneHour, "BWUH", null, "Rape Him", null, "", null);
			}
		...
		if (chooser == 2) {
			...
			if (temp == 1) {
				if (player.stats.cor < 80) {
					...
					if (player.gender == 1) MainScreen.text("stroking your half-erect " + CockDescriptor.describeMultiCockShort(player) + " as you daydream about fucking all kinds of women, from weeping tight virgins to lustful succubi with gaping, drooling fuck-holes.", false);
					if (player.gender == 2) MainScreen.text("idly toying with your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " as you daydream about getting fucked by all kinds of monstrous cocks, from minotaurs' thick, smelly dongs to demons' towering, bumpy pleasure-rods.", false);
					if (player.gender == 3) MainScreen.text("stroking alternatively your " + CockDescriptor.describeMultiCockShort(player) + " and your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " as you daydream about fucking all kinds of women, from weeping tight virgins to lustful succubi with gaping, drooling fuck-holes, before, or while, getting fucked by various monstrous cocks, from minotaurs' thick, smelly dongs to demons' towering, bumpy pleasure-rods.", false);
	...
	internal function bigJunkForestScene(lake: boolean = false): void {
		...
		MainScreen.text(", you find yourself increasingly impeded by the bulk of your " + CockDescriptor.describeCock(player, x) + " dragging along the ", false);
		...
		if (player.lowerBody.cockSpot.count() == 1) {
			if (lake) MainScreen.text("  As it drags through the lakeside mud, the sensation forces you to imagine the velvety folds of a monstrous pussy sliding along the head of your " + CockDescriptor.nounCock(player.lowerBody.cockSpot.get(x).cockType) + ", gently attempting to suck it off.", false);
			else MainScreen.text("  As it drags across the grass, twigs, and exposed tree roots, the sensation forces you to imagine the fingers of a giant hand sliding along the head of your " + CockDescriptor.nounCock(player.lowerBody.cockSpot.get(x).cockType) + ", gently jerking it off.", false);
		}
		else if (player.lowerBody.cockSpot.count() >= 2) {
			if (lake) MainScreen.text("  With all of your " + CockDescriptor.describeMultiCockShort(player) + " dragging through the mud, they begin feeling as if the lips of " + num2Text(player.lowerBody.cockSpot.count()) + " different cunts were slobbering over each one.", false);
			else MainScreen.text("  With all of your " + CockDescriptor.describeMultiCockShort(player) + " dragging across the grass, twigs, and exposed tree roots, they begin feeling as if the rough fingers of " + num2Text(player.lowerBody.cockSpot.count()) + " different monstrous hands were sliding over each shaft, gently jerking them off.", false);
		}
		MainScreen.text("\n\n", false);
		...
		if (!player.lowerBody.isTaur()) {
			MainScreen.text("The impending erection can't seem to be stopped.  Your sexual frustration forces stiffness into your " + CockDescriptor.describeMultiCockShort(player) + ", which forces your torso to the ground.  Normally your erection would merely raise itself skyward, but your genitals have grown too large and heavy for your " + LowerBodyDescriptor.describeHips(player) + " to hold them aloft.  Instead, you feel your body forcibly pivoting at the hips until your torso is compelled to rest face down atop your " + CockDescriptor.describeMultiCockShort(player) + ".", false);
			//IF CHARACTER HAS GIANT BREASTS ADD SENTENCE
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 35) {
				if (lake) MainScreen.text("  Your " + chestDesc() + " hang lewdly off your torso to rest in the lakeside mud, covering much of the ground to either side of you.  Their immense weight anchors your body, further preventing your torso from lifting itself up.  Mud cakes against their undersides and coats your " + BreastDescriptor.describeNipple(0) + "s.", false);
				else MainScreen.text("  Your " + chestDesc() + " hang lewdly off your torso to rest on the twings and dirt, covering up much of the ground to either side of you.  Their immense weight anchors your body, further preventing your torso from lifting itself up.  The rough texture of the bark on various tree roots teases your " + BreastDescriptor.describeNipple(0) + "s mercilessly.", false);
			}
			//IF CHARACTER HAS A BALLS ADD SENTENCE
			if (player.lowerBody.balls > 0) {
				MainScreen.text("  Your " + player.skinTone + " " + BallsDescriptor.describeSack(player) + " rests beneath your raised " + ButtDescriptor.describeButt(player) + ".  Your " + BallsDescriptor.describeBalls(true, true, player) + " pulse with the need to release their sperm through your " + CockDescriptor.describeMultiCockShort(player) + " and ", false);
			...
			if (player.lowerBody.vaginaSpot.count() >= 1) {
				MainScreen.text("  Your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " and " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " are thoroughly squashed between the bulky flesh where your male genitals protrude from between your hips and the " + ButtDescriptor.describeButt(player) + " above.", false);
		...
		//FOR CENTAURS
			...
			MainScreen.text("  The impending erection can't seem to be stopped.  Your sexual frustration forces stiffness into your " + CockDescriptor.describeMultiCockShort(player) + ", which forces the barrel of your horse-like torso to the ground.  Normally your erection would merely hover above the ground in between your centaurian legs, but your genitals have grown too large and heavy for your " + LowerBodyDescriptor.describeHips(player) + " to hold them aloft.  Instead, you feel your body being forcibly pulled down at your hind legs until your equine body is resting on top of your " + CockDescriptor.describeMultiCockShort(player) + ".", false);
			//IF CHARACTER HAS GIANT BREASTS ADD SENTENCE
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 35) {
				if (lake) MainScreen.text("  Your " + chestDesc() + " pull your human torso forward until it also is forced to face the ground, obscured as it is in boob-flesh.  Your tits rest on the wet earth to either side of you.  Their immense weight anchors you, further preventing any part of your equine body from lifting itself up.  Mud cakes their undersides and coats your " + BreastDescriptor.describeNipple(0) + "s.", false);
				else MainScreen.text("  Your " + chestDesc() + " pull your human torso forward until it also is forced to face the ground, obscured as it is in boob-flesh.  Your tits rest on the dirt and twigs to either side of you.  Their immense weight anchors you, further preventing any part of your equine body from lifting itself up.  The rough texture of the bark on various tree roots teases your " + BreastDescriptor.describeNipple(0) + "s mercilessly.", false);
			}
			//IF CHARACTER HAS A BALLS ADD SENTENCE
			if (player.lowerBody.balls > 0) {
				MainScreen.text("  Your " + player.skinTone + BallsDescriptor.describeSack(player) + " rests beneath your raised " + ButtDescriptor.describeButt(player) + ".  Your " + BallsDescriptor.describeBalls(true, true, player) + " pulse with the need to release their sperm through your " + CockDescriptor.describeMultiCockShort(player) + " and ", false);
		...
		if (lake) {
			...
			//Taurs
			else if (player.lowerBody == LowerBodyType.CENTAUR) MainScreen.text("  You struggle and work your equine legs against the wet ground.  Your " + LowerBodyDescriptor.describeFeet(player) + " have consistent trouble finding footing as the mud fails to provide enough leverage to lift your bulk.  You breath in deeply and lean side to side, trying to find some easier vertical leverage beneath your feet.  Eventually, with a crude crawl, your centaur legs manages to push the bulk of your body onto more solid ground.  With great difficulty, you spend the next hour shuffling your genitals back to camp.", false);
			//SCENE END = FOR ALL OTHER CHARACTERS
			else MainScreen.text("  You struggle and push with your " + LowerBodyDescriptor.describeLegs(player) + " as hard as you can, but it's no use.  You do the only thing you can and begin stroking your " + CockDescriptor.describeMultiCockShort(player) + " with as much vigor as you can muster.  Eventually, your body tenses and a light load of jizz erupts from your body, but the orgasm is truly mild compared to what you need.  You're far too weary from struggling to give yourself the masturbation you truly need, but you continue to try.  Nearly an hour later, " + CockDescriptor.describeMultiCockSimpleOne(player) + " has softened enough to allow you to stand again, and you make your way back to camp, still dragging your genitals through the mud.", false);
		}
		else {
			//SCENE END = IF CHARACTER HAS FULL WINGS ADD SENTENCE
			...
			else if (player.lowerBody == LowerBodyType.CENTAUR) MainScreen.text("  You struggle and work your equine legs against the soft dirt.  Your " + LowerBodyDescriptor.describeFeet(player) + " have consistent trouble finding footing as the ground fails to provide enough leverage to lift your bulk.  You breath in deeply and lean side to side, until eventually, your feet brace against the various roots of the trees around you.  With a crude crawl, your centaur legs manage to shuffle your body and genitals out of the forest and back to camp.", false);
			//SCENE END = FOR ALL OTHER CHARACTERS
			else MainScreen.text("  You struggle and push with your " + LowerBodyDescriptor.describeLegs(player) + " as hard as you can, but it's no use.  You do the only thing you can and begin stroking your " + CockDescriptor.describeMultiCockShort(player) + " with as much vigor as you can muster.  Eventually, your body tenses and a light load of jizz erupts from your loins, but the orgasm is truly mild compared to what you need.  You're far too weary from struggling to give yourself the masturbation you truly need, but you continue to try.  Nearly an hour later, " + CockDescriptor.describeMultiCockSimpleOne(player) + " has softened enough to allow you to stand again, and you make your way back to camp, still dragging your genitals across the forest floor.", false);
		}
	...
	private rapeSatyr(): void {
		...
		MainScreen.text("  You silently strip your " + player.inventory.armor.displayName + " and ", false);
		if (player.lowerBody.isNaga()) MainScreen.text("slither", false);
		else MainScreen.text("sneak", false);

		MainScreen.text(" towards the distracted satyr; stopping a few feet away, you stroke your " + CockDescriptor.describeCock(player, x) + ", urging it to full erection and coaxing a few beads of pre, which you smear along your " + player.cockHead(x) + ".  With no warning, you lunge forward, grabbing and pulling his hips towards your " + CockDescriptor.describeCock(player, x) + " and shoving as much of yourself inside his tight ass as you can.\n\n", false);
		...
		else MainScreen.text("stretching it to fit your " + CockDescriptor.describeCock(player, x) + " like a condom.", false);
		MainScreen.text("  Your groin throbs, ", false);
		if (player.lowerBody.balls > 0) MainScreen.text("your balls churn, ", false);
		MainScreen.text("and you grunt as you feel the first shots of cum flowing along " + CockDescriptor.describeMultiCockSimpleOne(player) + ", only to pour out into", false);
		...
		MainScreen.text("As you watch the lewd display, you feel your arousal building and your " + CockDescriptor.describeCock(player, x) + " growing back into full mast. Figuring you already have a willing slut readily available, you consider using him to relieve yourself once more... What do you do?", false);
		player.orgasm();
		//[Again][Leave]
	...
	private dontRepeatFuckSatyr(): void {
		MainScreen.text("", true);
		spriteSelect(99);
		MainScreen.text("You've had your fun, and you don't really want to fool around in the forest all day, so you grab your " + player.inventory.armor.displayName + " and leave the rutting satyr behind.\n\n", false);
		doNext(camp.returnToCampUseOneHour);
	}
	//[=Again=]
	private secondSatyrFuck(): void {
		...
		MainScreen.text("", true);
		MainScreen.text("There's no harm in using the helpless goat once more... This time though, you decide you'll use his mouth.  With a yank on his horns, you forcefully dislodge him from the breast-plant and force him to his knees, turning his head towards you; he doesn't put up much resistance and when you present your erect shaft to him, he licks his lips in excitement and latches onto your " + CockDescriptor.describeCock(player, x) + ".\n\n", false);
		...
		MainScreen.text("You give your sensitive member a few trembling, almost-painful strokes... maybe you overdid it a bit.  Shrugging, you gather your " + player.inventory.armor.displayName + " and leave the passed-out satyr behind as you go back to your camp.", false);
		player.orgasm();
		player.stats.lib += 1;
		player.stats.sens += -5;
		doNext(camp.returnToCampUseOneHour);
