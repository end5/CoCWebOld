...
package classes.Scenes.Areas.Bog
{
	...
	public class PhoukaScene extends BaseContent implements TimeAwareInterface {
		...
		internal static let phoukaForm: number = 0; //This keeps track of the form of the phouka across different scenes and through combat
		internal static const PHOUKA_FORM_FAERIE: number = 0;
		internal static const PHOUKA_FORM_BUNNY: number = 1;
		internal static const PHOUKA_FORM_GOAT: number = 2;
		internal static const PHOUKA_FORM_HORSE: number = 3;
		...
		{ //Helper function, For any text that depends on whether or not the player knows what to call a phouka
			return (flags[FlagEnum.PHOUKA_LORE] == 0 ? unknown : known);
		}
		...
		{ //General entry point for everything except halloween special encounter
			...
			if (flags[FlagEnum.PHOUKA_ENCOUNTER_STATUS] == 0) { //Guarantee first Phouka encounter is with faerie fire
				flags[FlagEnum.TREACLE_MINE_YEAR_DONE] = date.fullYear - 1; //If you've never encountered phoukas before then we can safely set all this stuff here
				flags[FlagEnum.BIRTHS_PHOUKA] = 0;
				flags[FlagEnum.BIRTHS_FAERIE] = 0;
			}
		...
		{ //In old Irish legends the Phouka are responsible for Will-o’-the-wisps. In the deep woods it’s Kitsune, but in the bog it’s a Phouka
			...
			if (flags[FlagEnum.PHOUKA_ENCOUNTER_STATUS] != 0) {
			...
			menu();
			if (flags[FlagEnum.PHOUKA_ENCOUNTER_STATUS] != 0 || player.hasKeyItem("Traveler's Guide") >= 0) MainScreen.addButton(4, "Leave", phoukaFaerieFireLeave);
		...
		{
			MainScreen.clearText();
			let hasAlcohol:boolean = player.hasItem(consumables.P_WHSKY, 1);
			...
			if(player.isNaga()) MainScreen.text("tail is");
			...
			if (flags[FlagEnum.PHOUKA_ENCOUNTER_STATUS] == 0) flags[FlagEnum.PHOUKA_ENCOUNTER_STATUS] = 1;
		...
		protected phoukaStuckOfferWhiskey():void
		{
			...
			if (flags[FlagEnum.PHOUKA_LORE] == 0) flags[FlagEnum.PHOUKA_LORE] = 1; //Now you know what to call them
		...
		{ //In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
			...
			flags[FlagEnum.PREGNANCY_CORRUPTION]++; //Faerie or phouka babies become more corrupted, no effect if the player is not pregnant or on other types of babies
		...
		{ //In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
			...
			if ((player.pregnancyIncubation == 0) && (player.buttPregnancyIncubation == 0) && (player.pregnancyIncubation <= 100)) { //Pregnancy is obvious to the phouka
				...
				if ((player.lust > 66) || ((player.stats.cor > 50) && (player.lust > 33))) {
		...
		{
			if (flags[FlagEnum.PHOUKA_LORE] == 1) {
				flags[FlagEnum.PHOUKA_LORE]++;
			...
			else if (flags[FlagEnum.PHOUKA_LORE] == 2) {
				flags[FlagEnum.PHOUKA_LORE]++;
			...
			else if (flags[FlagEnum.PHOUKA_LORE] == 3) {
				flags[FlagEnum.PHOUKA_LORE] += 1;
			...
			else if (flags[FlagEnum.PHOUKA_LORE] == 4) {
				//More can come later, right now we don’t increment here //flags[FlagEnum.PHOUKA_LORE] += 1;
            ...
            let playerIsInebriated:boolean = (playerNotDrinking == false) && (player.tou < 70);
		...
		{
			flags[FlagEnum.TREACLE_MINE_YEAR_DONE] = date.fullYear;
			MainScreen.clearText();
		...
		public phoukaPregBirth():void
		{
			...
			if (flags[FlagEnum.PREGNANCY_CORRUPTION] > 6) { //You’ve been drinking like a fish, haven’t you?
				...
				fatigue(75);
				flags[FlagEnum.BIRTHS_PHOUKA]++;
				player.orgasm();
			}
			else if (flags[FlagEnum.PREGNANCY_CORRUPTION] > 0) {
		...
		{ //Belly size doesn't change, instead you get updates on what's going on
			if (player.pregnancyIncubation == 170) { //Stage 1:
				if (flags[FlagEnum.PREGNANCY_CORRUPTION] > 0)
			...
			if (player.pregnancyIncubation == 60) { //Stage 4:
				if (flags[FlagEnum.PREGNANCY_CORRUPTION] > 6)
		...
		{ //Player must have a vagina for bunny-morph sex
			...
			if ((player.lust > 80) || (player.stats.cor > 50))
		...
		protected phoukaSexFaerieFemale(postCombat:boolean):void
		{
			...
			if ((flags[FlagEnum.FREED_VALA] == 1) || (flags[FlagEnum.TIMES_PC_DEFEATED_VALA] > 0) || (flags[FlagEnum.TIMES_FUCKED_VALA_IN_DUNGEON] > 0) || (flags[FlagEnum.INVESTIGATED_VALA_AFTER_ZETAZ_DEFEATED] > 0)) //If any of these is true you've had some contact with Vala
		...
		{ //Whether by horse, bunny or (male) faerie sex it all ends up here if the PC has a vagina
			...
			if (player.isPregnant()) {
				let isFirstTime:boolean = (flags[FlagEnum.BIRTHS_PHOUKA] == 0) && (flags[FlagEnum.BIRTHS_FAERIE] == 0);
				flags[FlagEnum.PREGNANCY_CORRUPTION] = 2; //Start off with a corrupt little phouka. It will take three drinks of pure honey to turn it into a faerie
		...
		{ //Everything after the sex. Handles awards, gem loss and text for player leaving the bog
			...
			if (phoukaForm == PHOUKA_FORM_FAERIE) { //In this case postCombat means you need an award because you must have won to get faerie sex
				...
				MainScreen.text("\n\nYou ");
				if (phoukaForm == PHOUKA_FORM_HORSE && !player.isTaur()) MainScreen.text("wait for the throbbing pain in your pelvis to subside.  Then you ");
				if (postCombat) {
