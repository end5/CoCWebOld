 ...
 */
export default class PhoukaScene implements TimeAwareInterface {
	...
	public timeChange(): boolean {
		if (player.statusAffects.get("PhoukaWhiskeyAffect").value1 > 0) {
			player.statusAffects.get("PhoukaWhiskeyAffect").value1 = -1; //Count down hours until player is not drunk
			if (player.statusAffects.get("PhoukaWhiskeyAffect").value1 <= 0) {
	...
	protected phoukaStuckOfferWhiskey(): void {
		MainScreen.clearText();
		MainScreen.text("You pull out the bottle and ask the " + phoukaName() + " if he might be interested in a drink.  He looks at the bottle and says <i>“Well we both know where ya picked that up, hmmm?”</i>  Then he smiles and adds <i>“Course that’s some other phouka’s problem if he let ye beat him.”</i>");
	...
	protected phoukaTalk(): void {
		MainScreen.clearText();
		MainScreen.text("You ask the " + phoukaName());
		if (player.level < 10)
			MainScreen.text(" if perhaps there's some way he could release you.")
		else MainScreen.text(" if he really wants a taste of what you're going to give him.");

		if (player.stats.cor + rand(125) <= 110) { //Players with 0% corruption have a 12% chance that the Phouka is willing to talk, players with 100% corruption have a 92% chance
			//The phouka is not in the mood for talk, start the fight gagged as punishment for trying to talk
			MainScreen.text("  You open your mouth to say something more to the ");
			if (player.stats.cor < 34)
				MainScreen.text("vile little monster")
			else if (player.stats.cor < 67)
				MainScreen.text("perverted " + phoukaName())
			else MainScreen.text("fuckable little " + phoukaName());
			MainScreen.text(" but he must not feel like listening.  He launches a ball of black mud at your face, sealing your mouth with sticky and [if (corruption <= 50)ewww -][if (corruption > 50)mmmm -] salty muck.");
			startCombat(new Phouka(phoukaName()));
			player.statusAffects.add(new StatusAffect("WebSilence", 0, 0, 0, 0)));
		}
			...
			switch (player.level < 10 ? rand(8) : rand(12)) { //Low level characters don't get the chance to be offered whiskey
				...
				case 6: //Offers you a chance to submit, which leads to the various defeat sex scenes
					MainScreen.text(" considers for a moment and then says, <i>“You can't get away from me right now, so why don't you enjoy it?  Otherwise, maybe I bring in some of my friends an we all have at ya.”</i> The confidence in the " + phoukaName() + "'s gaze suggests he isn't bluffing.  He slowly circles you, hovering a few feet out of reach.  His eyes seem to be drinking in every detail of your body in a way that [if (corruption < 50)makes shivers run down your spine][if (corruption >= 50)makes your heart beat faster].\n\nThe " + phoukaName() + " continues, ");
					if (player.lowerBody.vaginaSpot.hasVagina())
						MainScreen.text("<i>“Maybe yer inta bunnies, hrm?  Big bunny with a nice hard cock and strong legs, I can do that fer ya.  Fill that tight little muff o' yers up with a big load o' spunk!”\n\n“Or maybe ");
					else
						MainScreen.text("<i>“");
					MainScreen.text("I could change inta a goat for ya.  That suit yer fancy?  Is it a fond dream o' yers to let a goat blow its load in yer cornhole?");
					if (player.lowerBody.vaginaSpot.hasVagina())
						...
						MainScreen.text("  If it ain't then tough shite, cause that's what I want.”</i>");
					menu();
	...
	protected phoukaDrinkWhilePregnant(playerOfferedTheBooze: boolean): void {
		...
		if (acceptable < 0) {
			...
			if (acceptable == -4) MainScreen.text("  One of them seems to crave the whiskey, but the other is a more normal child.");
			MainScreen.text("  Sure, some of the elders around the village said a beer or some light cider was all right for a woman heavy with child, but you doubt that whiskey would be acceptable.  You politely refuse the drink, hoping the phouka won't be offended.");
		}
		else {
			MainScreen.text("\n\nYou put one hand on your belly, about to refuse the drink.  Then you realize that yes, this is exactly what your ");
			if (acceptable <= 2)
				MainScreen.text("baby needs.  Some light kicks inside your " + (acceptable == 2 ? "distended colon" : "womb") + " suggest that your child knows");
			else
				MainScreen.text("babies need.  Some light kicks inside your womb and your distended colon suggest that your children know");
			MainScreen.text(" what's up for grabs.  <i>“Go on and have some already. I try not to drink alone.  Always end up fucked by an imp or a drider or something that way.”</i>");
		}
		menu();
	...
	protected phoukaDrinkAccept(): void { //In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
		...
		switch (consumables.P_WHSKY.phoukaWhiskeyDrink(player)) {
			...
			case 1: //Child is a phouka or satyr, loves alcohol
				MainScreen.text("A warm feeling in your belly lets you know your passenger enjoyed the meal as well.");
				break;
		...
		if (player.stats.tou < 70) {
			MainScreen.text("  It looks like the phouka is holding his liquor about as well as you are.");
		}
		else {
			MainScreen.text("  It seems you can handle your liquor better than the phouka.");
		}
		MainScreen.text("  Soon both of you start to share stories about your adventures in Mareth.");
	...
	protected phoukaDrinkRefuse(): void { //In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
		...
		if ((player.pregnancyIncubation == 0) && (player.buttPregnancyIncubation == 0) && (player.pregnancyIncubation <= 100)) { //Pregnancy is obvious to the phouka
			MainScreen.text("The rabbit morph sits down heavily on the wet ground and sips his booze.  <i>“Don't want to fuck, don't want to drink.  What gives?  Live a little and let me have fun with your pregnant pussy.  I’ll get ya off, and it’s not like I can knock you up again.  Course, that'd be fun too.”</i>");
			menu();
			phoukaSexAddStandardMenuChoices();
			MainScreen.addButton(0, "Leave", phoukaDrinkRefusePregnantLeave);
		}
		else { //Less obvious pregnancy or player not pregnant
			MainScreen.text("<i>“You damn prude.  That does it! I don't have to sit around listening to this.  I try to be friendly like some people say ya should instead of just raping ya.  What's it get me, huh?”</i>");
	...
	protected phoukaDrinkRefuseLeave(): void { //In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
		MainScreen.clearText();
		MainScreen.text("You extract yourself from the mud and get out of the bog before the phouka changes its mind.");
		doNext(camp.returnToCampUseOneHour);  //Return to camp, 1 hour used
	}
	...
	protected phoukaDrinkTalk(playerNotDrinking: boolean): void {
		...
		if (playerIsInebriated) {
			...
			MainScreen.text("  He lets out a little giggle and begins to shrink. Tendrils of black smoke rise from the bunny morph’s black fur as he shrinks back to the size and shape of a faerie.");
			phoukaForm = PHOUKA_FORM_FAERIE;
			if (player.stats.lust < 33)
				MainScreen.text("  It looks like he needs a nap and you take that as your cue to get out of here.");
			else {
				MainScreen.text("\n\nYou look at the sleeping phouka");
				if (player.lowerBody.cockSpot.hasCock())
					MainScreen.text(", [eachCock] beginning to swell" + (player.lowerBody.vaginaSpot.hasVagina() ? " and your [vagina] starting to moisten" : "") + ".")
				else if (player.lowerBody.vaginaSpot.hasVagina())
					MainScreen.text(", your [vagina] starting to moisten.")
				else MainScreen.text("and you feel your sphincter twitch as your brain thinks about what you could do with this phouka now."); //Genderless
			}
			menu();
			MainScreen.addButton(0, "Leave", phoukaDrinkLeave);
			if (player.stats.lust >= 33) {
				MainScreen.addButton(1, "Ride Faerie", phoukaSexFaerieNonCombat); //Works for all
				if (player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(2, "Fuck Faerie", phoukaSexFaerieFemaleNonCombat); //Male or Herm Only
			}
		}
	}
	...
	protected phoukaDrinkLeave(): void {
		if (player.stats.lust < 33)
			...
			MainScreen.text("\n\nIf you wanted");
		MainScreen.text(" you could take advantage of the phouka’s predicament, but right now you’d rather go and check on the portal.  Thanks to the whiskey, the water soaking your [legs] doesn't matter to you quite so much as you slog back towards camp.");
		doNext(camp.returnToCampUseOneHour);  //Return to camp, 1 hour used
	}
	...
	public phoukaHalloween(): void {
		...
		if (player.stats.str < 40) {
			MainScreen.text("You find it very difficult to operate and have to squeeze it with your thighs, but you manage to pump a respectable amount before passing it on.");
		}
		else if (player.stats.str < 80) {
			MainScreen.text("You find that treacle pumping is as hard as the phouka said, but you keep at it.  With what you pumped you must be as strong as they are.");
		}
		else {
			MainScreen.text("You don't know why they said this was difficult.  You have no problem pumping the treacle; in fact they have to stop you partway through to change barrels.");
		}
	...
	protected phoukaLeaveOnLustWin(newScreen: boolean = true): void {
		...
		MainScreen.text("The lusty " + phoukaName() + " continues to pound his cock into the earth as you prepare to go, oblivious to your presence.");
	...
	internal function phoukaPlayerWins(hpVictory: boolean): void {
		...
		if (hpVictory) { //You win by physical damage, the phouka cheats and runs
			MainScreen.text("The seriously injured " + phoukaName() + " stumbles backward, but before you can strike again it twists and stretches in mid-air, dropping to the ground in the form of a long black eel.  You're pretty sure you hear the eel curse at you as it dives into the mire and sinks out of view.\n\nWith your attacker gone you struggle and strain to get yourself free of the thick mass of roots and muck around your [legs].  Your lower half is soaked and you decide to head home.");
			kGAMECLASS.clearStatuses(false);
			kGAMECLASS.awardPlayer();  //This will provide loot and return to camp, 1 hour used
		}
		else { //You win by lust and have the chance to fuck the phouka if you’re horny
			MainScreen.text("The " + phoukaName() + " collapses to the ground and begins to jab his cock into the peat.");
			if (phoukaForm != PHOUKA_FORM_FAERIE) {
				MainScreen.text("  As you watch, wisps of black smoke start to rise from him and his body starts to shrink.  In moments the ");
				if (phoukaForm == PHOUKA_FORM_BUNNY)
					MainScreen.text("bunny morph");
				else if (phoukaForm == PHOUKA_FORM_GOAT)
					MainScreen.text("goat morph");
				else if (phoukaForm == PHOUKA_FORM_HORSE)
					MainScreen.text("stallion");
				MainScreen.text(" has shrunk back down to the size and shape of a " + phoukaNameText("phouka", "faerie") + ".");
				phoukaForm = PHOUKA_FORM_FAERIE;
			}
			if (player.stats.lust < 33) {
				MainScreen.text("  The threat dealt with you scoop, dig and finally extract yourself from the mire.  Your lower half is soaked and you long to sit in front of the fire at your camp.  Hefting your supplies you turn back the way you came.");
				phoukaLeaveOnLustWin(false);
			}
			else {
				MainScreen.text("  While he's busy, you extract yourself from the muck, and once you're free, you look over at the lusty " + phoukaName());
				if (player.stats.cor < 34)
					MainScreen.text(".  You could sate yourself just to be sure you're not distracted on the way home.");
				else if (player.stats.cor < 67)
					MainScreen.text(".  You could take advantage of the situation to slake your lust.");
				else MainScreen.text(".  It would be so easy to rape this perverted faerie, how can you resist?");
				menu();
				...
				if (player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(2, "Fuck Faerie", phoukaSexFaerieFemalePostCombat); //Male or Herm Only
			}
		}
	}
	...
	public phoukaPregUpdate(): boolean { //Belly size doesn't change, instead you get updates on what's going on
		...
		if (player.pregnancyIncubation == 100) { //Stage 3:
			...
			MainScreen.text(" You've noticed that this pregnancy doesn't seem to be affecting your breasts. It's as if the child inside you has no use for your milk.\n");
			return true;
		}
		...
		if (player.pregnancyIncubation == 36) { //Stage 5:
			...
			if (Flags.list[FlagEnum.PREGNANCY_CORRUPTION] > 6)
				MainScreen.text("Despite the small size of your belly you spend most of your time feeling deeply ill and you can't wait to force this thing out.  You can feel a constant dull pain from your womb and ovaries, probably the result of the tainted fluid inside you.  Only drinking alcohol settles your stomach.\n");
			else if (Flags.list[FlagEnum.PREGNANCY_CORRUPTION] > 0)
				MainScreen.text("Despite the relatively small size of your belly, you spend most of your time feeling mildly ill and you can't wait to force this thing out.\n");
			else MainScreen.text("Despite being pregnant you still don't feel restricted or slowed in any way.  It's as if the tiny life inside is living in harmony with you.\n");
			return true;
		}
		return false;
	}
	...
	internal function phoukaSexBunny(postCombat: boolean, lustLoss: boolean): void { //Player must have a vagina for bunny-morph sex
		MainScreen.clearText();
		if (postCombat) {
			MainScreen.text("As you collapse the " + phoukaName());
			if (phoukaForm == PHOUKA_FORM_BUNNY)
				MainScreen.text(" hops over and stands in front of you");
			else
				MainScreen.text(" begins to shapeshift in front of you.  He changes into various forms, looks you over and finally changes into a black furred bunny-morph");
			MainScreen.text(".  You watch as the bunny's erection stiffens, your ");
			if (lustLoss)
				MainScreen.text("mind so overcome with lust you can't");
			else MainScreen.text("body too bruised to");
			MainScreen.text(" resist any further.");
		}
		else {
			if (player.stats.cor <= 50) {
				MainScreen.text("It might not be the wisest move, but you decide to let the " + phoukaName() + " have his way with you.  You tell him you would prefer a little bunny loving and the " + phoukaName() + " grins from ear to ear. <i>“Comin' right up,”</i> he says");
				if (phoukaForm != PHOUKA_FORM_BUNNY) MainScreen.text(", and morphs into a rabbit");
				MainScreen.text(".  Hopefully he'll be gentle with a willing partner.");
			}
			else {
				MainScreen.text("You can't think of any good reason to refuse a roll in the hay (or the mire) with this " + phoukaName() + ".  You tell him that you want to feel his big bunny cock deep inside you.  The " + phoukaName() + " grins as you begin seductively stripping off you armor.");
				if (phoukaForm != PHOUKA_FORM_BUNNY) MainScreen.text("  He goes through a short series of warm up exercises and then transforms himself into the rabbit you desire.");
			}
		}
		phoukaForm = PHOUKA_FORM_BUNNY;
		...
		if ((player.stats.lust > 80) || (player.stats.cor > 50))
			MainScreen.text(" You can only moan in response.  The thought of this rabbit stuffing you has your [vagina] leaking already.");
		else
			MainScreen.text(" You are powerless to stop the bunny-morph as he undoes the last few straps holding your [armor] in place.");
		...
		if (player.hasVirginVagina())
			MainScreen.text("You scream as the " + phoukaName() + "'s cock tears your hymen apart.  He stops thrusting and says <i>“A virgin? Here? Good goddess, missy, ya should have told me.”</i> In a lower tone he adds <i>“so I could have enjoyed it more.”</i> Your whole vagina burns as every fold is stretched and widened by the passage of the rabbit's cock.");
		else if (postCombat && (player.lowerBody.vaginaSpot.get(0).vaginalLooseness < 2))
			MainScreen.text("You nearly pass out from the pain as the " + phoukaName() + "'s cock lengthens and thickens inside you.");
		else if (!postCombat || (player.stats.cor > 50))
			MainScreen.text("Even as the invader lengthens and thickens you push back against it. <i>“Good girl. Good little fuck slut,”</i> whispers the " + phoukaName());
	...
	internal function phoukaSexGoat(postCombat: boolean, lustLoss: boolean): void { //Player does not have a vagina or has chosen anal
		MainScreen.clearText();
		if (postCombat) {
			if (lustLoss)
				MainScreen.text("Panting with need, you begin to strip off your [armor].  The " + phoukaName() + " watches the show with an evil smile.");
			else
				MainScreen.text("Your body falls to the ground, too weak to fight back against the " + phoukaName() + " any longer.");
			if (phoukaForm != PHOUKA_FORM_GOAT) MainScreen.text("  As you lay there, the " + phoukaName() + "'s body stretches and morphs until it takes the form of a large, black furred goat-morph.");
		}
		else {
			MainScreen.text("You tell the " + phoukaName() + " you're willing to let him take your ass in his goat form.  He smiles and lands near you.  His body warps into the form of a black furred goat morph.  ");
			if (player.stats.cor <= 50)
				MainScreen.text("You steel yourself for an asspounding while you remove your [armor].");
			else MainScreen.text("Then you put on a show, taking your clothes off piece by piece.  You strip off your final undergarment, bend over and wink your asshole at the goat.");
		}
		...
		if (postCombat)
			MainScreen.text("He mounts you immediately, giving you no chance to adjust to his invading cock.  The goat-morph's first powerful thrust drives your face into the muck as his tries to force his narrow cockhead into your ass.");
		else
			MainScreen.text("The goat rubs his cockhead against your pucker, spreading a thick layer of pre over your hole before slowly pressing home his cock.  His first few thrusts are gentle enough that you have the chance to spread your ass and get used to this invader.");
		MainScreen.text("  Disturbingly you feel his front 'hooves' split into hard little fingers that he wraps around your [if (isTaur = true)belly][if (isTaur = false)[chest]] to hold onto you even tighter.\n\n");
		if (postCombat)
			MainScreen.text("The " + phoukaName() + " shows no interest in you except as a receptacle for his cum.  After stabbing his hips at you several times you feel the full length of his cock penetrate your pucker.");
		else
			MainScreen.text("Once your [butt] has adjusted, the goat morph begins to thrust more quickly.  You are surprised when you feel his balls slapping against your [if (isNaga = true)tail][if (isNaga = false)inner thighs] - he can't already be in that deep, can he?");
		MainScreen.text("  Once the goat's cock is inside, he stops moving and tenses up.  You feel his cock growing larger and larger until you fear your abused anus is going to split apart.");
		...
		if (postCombat)
			MainScreen.text("The goat-morph waits only a few seconds before unceremoniously yanking his deflating cock from your ass.");
		else
			MainScreen.text("Realizing that you haven't yet cum, the goat morph continues to rock in and out of your ass.  The air fills with the sucking and slurping noises of the goat's seed alternately being sucked deeper or forced out of your ass.  Finally you reach your limit and cum[if (hasCock = true), your own cock turning the mud white beneath you].");
		dynStats("cor", rand(1) + (postCombat ? 1 : 3)); //Extra two corruption for being enough of a pervert to want to fuck the phouka
		player.orgasm();
		if (postCombat) MainScreen.text("  While you're recovering the goat-morph reaches into your gem pouch and takes a handful.");
	...
	internal function phoukaSexHorse(postCombat: boolean, lustLoss: boolean): void { //Player has a vagina
		MainScreen.clearText();
		if (postCombat) {
			MainScreen.text("As you collapse the " + phoukaName());
			if (phoukaForm == PHOUKA_FORM_HORSE)
				MainScreen.text(" circles you in a slow trot, his top half shaping into a centaur form before finally stopping directly in front of you.");
			else
				MainScreen.text(" begins to shapeshift in front of you.  It grows and grows, finally taking the form of a massive, black, centaur stallion with an equally massive prick.");
		}
			...
			if (player.stats.cor <= 50)
				...
				MainScreen.text("The more you think about it the more you decide you could use a real fucking.  You smile at the " + phoukaName() + ", telling him you want the biggest cock he can muster.  The " + phoukaName() + "morphs into the form you expected, that of a black, centaur stallion [if (tallness <= 96)taller than you are][if (tallness > 96)that's about your height].  Maybe that tree trunk between his legs can scratch your itch.");
		}
		phoukaForm = PHOUKA_FORM_HORSE;
		MainScreen.text("  On closer inspection the centaur is a bit unusual.  For a start, he has shiny green eyes.  Parts of his flanks look like thin wings pressed against its sides.  His nose is a little shorter and his face a lot more human than you would have expected.  He sees you looking him over and says <i>[if (isTaur = true)“I've been waitin for this.  A nice big mare to fuck the daylights out of][if (isTaur = false)“Yer gonna be sorry ye weren't born with four legs and a cunt you could lose an arm in, princess].”</i>\n\n");
		if (!postCombat || lustLoss)
			MainScreen.text("Considering the fire in your vagina that's crying out to be quenched, you don't care how big this horse's cock is.  You just need something to fill you NOW.");
		else
			MainScreen.text("You're hardly in any condition to refuse.  This stallion is going to pound you with that meatstick no matter what you do[if (corruption <= 50), so you steel yourself for what is to come][if (corruption > 50).  The thought of being wrecked by such an enormous staff has your cunt dripping girl-cum already.]");
		...
		if (player.lowerBody.isTaur())
			MainScreen.text("He walks up behind you and mounts you.  You feel his front legs rubbing against your flanks and have to admit to yourself that this feels right.  The " + phoukaName() + "'s equine cock was built to force its way inside a mare[if (vagCapacity < 40), but it has difficulty with your tight cuntlips.  The stallion has to try twice more before his cock slides into place between your labia][if (vagCapacity >= 40) and it does just that].");
		else {
			if (player.lowerBody.isNaga())
				MainScreen.text("He walks in front of you, turns your way and then simply walks over you, forcing your upper body to bend backward over your tail.  Thankfully your snake-like spine means bending into a 'U' shape isn't that uncomfortable.");
			else
				MainScreen.text("He walks up behind you and then simply walks right over top of you, his chest knocking you to the ground.  You find yourself on all fours with a massive cockhead grinding against your ass.");
			MainScreen.text("  You hear the " + phoukaName() + " say <i>“Now line me up with yer cunt lass, or I'll lay down on top of ya and have a nap.”</i> Not wanting to be crushed under that kind of bulk, you reach [if (isNaga = true)up][if (isNaga = false)behind you] and grab his member.  You place his cock at the entrance to your [vagina] and the stallion steps forward.  [if (vagCapacity < 50)His cock bends and then snaps free of your cunt, slapping against his belly. <i>“Again! And do it right this time!”</i> he shouts.  You line it up once more and press against it with your hands so it has nowhere to go but in.]");
		}
		MainScreen.text("  You're convinced you're going to be split in half by this monster horse cock.\n\n");
		if (player.hasVirginVagina())
			MainScreen.text("You scream as the " + phoukaName() + "'s cock tears your hymen apart.  He stops mid-thrust and asks <i>“A virgin? Here? Good goddess missy ya should have told me.”</i> In a lower tone he adds <i>“so I could have enjoyed it more.”</i> Your whole vagina burns as every fold is stretched and widened by the passage of the " + phoukaName() + "'s huge horse cock.  You think he's delighted to have taken a woman from virgin to gaping in just one thrust.  ");
		else if ((postCombat) && (player.lowerBody.vaginaSpot.get(0).vaginalLooseness < 3))
			MainScreen.text("You nearly pass out from the pain as the " + phoukaName() + "'s cock drives deeper and deeper inside you.  With such a wide head forcing its way in, you're sure the stretching will be permanent.  ");
		else if (!postCombat || (player.stats.cor > 50))
			MainScreen.text("Even as the invader thrusts deeper you push back against it. <i>“Good girl, ya love horse cock, don't ya?”</i> whispers the " + phoukaName() + ".  ");
		...
		if ((player.stats.lust > 80) || (player.stats.cor > 50))
			MainScreen.text("You pant and moan, waiting for the inevitable flood from his cantaloupe-sized nuts.  You're ready to get off just from the feeling of being so stuffed, so violated.");
		else {
			MainScreen.text("You whimper in pain, hoping the " + phoukaName() + " will soon be done with you.  ");
			if (player.lowerBody.isTaur())
				MainScreen.text("You feel so stretched and full.  What worries you is that the pain is giving way to pleasure.  Some base instinct of your centaur body is telling you that as a mare you were meant to be filled like this.  Your cunt was built for cocks like this so you could carry lots of little centaur foals.");
			else
				MainScreen.text("You're so stretched that your blood flow is being cut off and you can't feel [if (isNaga = true)the end of your tail][if (isNaga = false)your legs] anymore.");
		}
		MainScreen.text("  You feel those cum-stuffed balls twitch once, twice, three times.  The pressure must be building in the " + phoukaName() + "'s prostate.  He groans <i>“[if (isPregnant = true)Time te feed the baby.][if (isPregnant = false)Maybe today I'll leave ye a present.]”</i>  When he cums it's not a series of ripples moving up his urethra, it's a single long blast of stallion seed.");
		phoukaSexPregnate(postCombat);
	}
	...
	protected phoukaSexFaerie(postCombat: boolean): void {
		...
		if (player.lowerBody.vaginaSpot.hasVagina()) {
			...
			if (player.lowerBody.isTaur())
				MainScreen.text("You sigh in frustration as the " + phoukaName() + " pulls free from your ass, obviously about to leave.  He notices your condition and thinks better of it. <i>“Just gimme a sec.  I’ll get ye off,”</i> he says. <i>“After all, I do want ye to come back fer more.”</i>\n\nHe starts jacking himself off, whispering something to himself under his breath.  When his cock looks nice and hard he grabs your flanks and drives the full length back inside you.  As he pistons into your rectum, you feel that denied orgasm building and at last you cum.");
			else
				MainScreen.text("Not yet satisfied, you wait for the flood to stop, then begin to really ride your man-sized faerie.\n\nYou don't give him the chance to go soft.  Your [if (isNaga = true)tail][if (isNaga = false)legs] feel rubbery and your muscles are burning when you finally cum.");
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  [EachCock] fires several blasts of cum into the bog [if (isTaur = true)beneath you][if (isTaur = false)and all over your faerie partner].");
	...
	protected phoukaSexFaerieFemale(postCombat: boolean): void {
		...
		if ((Flags.list[FlagEnum.FREED_VALA] == 1) || (Flags.list[FlagEnum.TIMES_PC_DEFEATED_VALA] > 0) || (Flags.list[FlagEnum.TIMES_FUCKED_VALA_IN_DUNGEON] > 0) || (Flags.list[FlagEnum.INVESTIGATED_VALA_AFTER_ZETAZ_DEFEATED] > 0)) //If any of these is true you've had some contact with Vala
			MainScreen.text(" and she reminds you quite a bit of Vala, though Vala's chest is far larger.");
		else
			MainScreen.text(", but she still looks more human than most of the people you've met in Mareth.");
		...
		if (player.biggestCockLength() < 8)
			MainScreen.text("You push the last inch of your " + int(player.biggestCockLength()) + " prick into the moaning slut.");
		else if (player.biggestCockLength() < 15)
			MainScreen.text("You feed the last inch of your " + int(player.biggestCockLength()) + " cock into her.  Your cockhead bumps against her cervix and the slut beneath you begs for more.");
		else {
			MainScreen.text("You drive as much of your cock as you can into the little faerie slut, but there's just too much meat for her to take in.");
			if (player.stats.cor <= 50)
				MainScreen.text("  It still feels amazing and you give her even more pleasure by rubbing your cockhead against her cervix.");
			else
		...
		if (player.stats.cor <= 33)
			MainScreen.text("You certainly don't want a phouka to learn where your campsite is.  You help her into the bushes and tell her she just needs to hide out for a few days.");
		else if (player.stats.cor <= 66)
			MainScreen.text("You laugh and remind her that's what she's wanted to do to you.");
		else
			MainScreen.text("Why shouldn't the other phouka get the chance to enjoy her?  Before she can fully recover you grab some tree limbs and vines.  You pin her to the ground and use them to tie her up tight.  With her arms and legs lashed together and her ass in the air she's an inviting target.");
		MainScreen.text("  Satisfied, you march back to camp.");
		dynStats("cor", rand(1) + 3); //Extra two corruption for being enough of a pervert to want to fuck the phouka
		if (postCombat) {
			kGAMECLASS.clearStatuses(false);
			kGAMECLASS.awardPlayer();  //This will provide loot and return to camp, 1 hour used
	...
	protected phoukaSexPregnate(postCombat: boolean): void { //Whether by horse, bunny or (male) faerie sex it all ends up here if the PC has a vagina
		if (player.isPregnant()) {
			...
			if (player.stats.cor > 50)
				MainScreen.text("  The feeling of all that seed flowing into you and then out of you again finally brings you to your own orgasm.[if (hasCock = true)  [EachCock] fires long strands of cum into the bog water beneath you.]");
			else
				MainScreen.text("  As the thick grey cum leaks from your cunt you finally give in and get yourself off.  There's no way you could have made it back to camp feeling like that.");
			if (phoukaForm == PHOUKA_FORM_FAERIE) MainScreen.text("\n\nNow that you've got your fill you [if (isTaur = true)pull away from][if (isTaur = false)roll off] your faerie lover.  He shrinks back to his normal size and launches himself into the air.  You're too tired to stop him from buzzing into the bushes and you quickly lose track of him.");
		...
		if (phoukaForm == PHOUKA_FORM_FAERIE)
			...
			MainScreen.text("\n\nHe");
		if (postCombat) MainScreen.text(" rummages around in your gem pouch and then");
		MainScreen.text(" takes to the air and hovers nearby as if waiting for something.\n\n");
		if (player.isPregnant()) {
			...
			MainScreen.text("You're about to lift yourself up when you feel " + (isFirstTime ? "an unusual" : "a familiar") + " sensation inside your belly.  All that cum is hardening!  You can feel it set, almost like mortar left out in the sun!  You stand and some clear water drains out, your belly shrinking in size to that of a watermelon.  Whatever's left inside your belly is solid, like you're carrying a smooth ball of wood inside you.\n\nYou run a hand over your belly.  Thankfully it isn't any heavier than a normal pregnancy, nor does it hurt at all.\n\n<i>“Yes!”</i> shouts the " + phoukaName() + ". <i>“I'm gonna be a daddy!  Thanks fer letting me fill ya up girl, there’s plenty o’ space in there fer the little guy.  I gotta tell all the other phouka about ya");
			if (Flags.list[FlagEnum.PHOUKA_LORE] == 0) Flags.list[FlagEnum.PHOUKA_LORE] = 1; //Now you know what to call them
			if (!postCombat || (player.stats.cor > 50)) MainScreen.text(", yer a real prize slut");
			MainScreen.text(".”</i> The phouka gains more altitude and flies off deeper into the bog, whistling happily to himself.  ");
			if (isFirstTime)
				MainScreen.text("You have to wonder how long this pregnancy will last. If this were a human child people would judge you to be at least four months along.");
			else MainScreen.text("At least you know what to expect from this form of pregnancy now.");
		}
		else MainScreen.text("When you pull yourself up your belly begins to deflate and leak its gooey grey payload into the mud.  The " + phoukaName() + " says <i>“Ah well, there's always next time,”</i> and buzzes away to some other part of the bog.");
		phoukaSexPregnateEnd(postCombat);
	}
	...
	protected phoukaSexPregnateEnd(postCombat: boolean): void { //Everything after the sex. Handles awards, gem loss and text for player leaving the bog
		...
		if (phoukaForm == PHOUKA_FORM_FAERIE) { //In this case postCombat means you need an award because you must have won to get faerie sex
			...
			if (player.stats.cor > 50) MainScreen.text("  But either way you plan to return and give all of them that lesson.");
			if (postCombat) {
				kGAMECLASS.clearStatuses(false);
				kGAMECLASS.awardPlayer();  //This will provide loot and return to camp, 1 hour used
			}
			...
			if (postCombat && player.stats.gems > 0) MainScreen.text("  While you're recovering the " + phoukaName() + " reaches into your gem pouch and takes a handful.");
			...
			MainScreen.text("\n\nYou ");
			if (phoukaForm == PHOUKA_FORM_HORSE && !player.lowerBody.isTaur()) MainScreen.text("wait for the throbbing pain in your pelvis to subside.  Then you ");
			if (postCombat) {
				MainScreen.text("pull your [if (isNaga = true)tail][if (isNaga = false)legs] free from the cool muck before you get chilled to the bone.");
				cleanupAfterCombat();
			}
			else {
				MainScreen.text("collect your clothes and begin the long march out of the bog.  ");
				doNext(camp.returnToCampUseOneHour); //Return to camp, 1 hour used
			}
			if (player.stats.cor <= 50)
				MainScreen.text("As you trudge back to camp you have to wonder - why did you decide to visit the bog again?");
			else if (phoukaForm == PHOUKA_FORM_HORSE && player.lowerBody.isTaur())
				MainScreen.text("  It was quite the ride and you find yourself looking forward to your next trip into the bog.");
			else MainScreen.text("Not what you expected, but at least you got off.");
		}
	}
