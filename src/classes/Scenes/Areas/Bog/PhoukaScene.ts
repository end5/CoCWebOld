	...
	public class PhoukaScene extends BaseContent implements TimeAwareInterface {
			...
			if (playerIsInebriated) {
				...
				if (player.lust < 33)
					...
					if (player.lowerBody.cockSpot.hasCock())
						outputText(", [eachCock] beginning to swell" + (player.lowerBody.vaginaSpot.hasVagina() ? " and your [vagina] starting to moisten" : "") + ".")
		...
		protected function phoukaSexFaerie(postCombat:boolean):void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				...
				if (player.lowerBody.cockSpot.hasCock()) outputText("  [EachCock] fires several blasts of cum into the bog [if (isTaur = true)beneath you][if (isTaur = false)and all over your faerie partner].");
		...
		{ //Whether by horse, bunny or (male) faerie sex it all ends up here if the PC has a vagina
			...
			if (player.isPregnant()) {
				...
				if (flags[FlagEnum.PHOUKA_LORE] == 0) flags[FlagEnum.PHOUKA_LORE] = 1; //Now you know what to call them
				if (!postCombat || (player.stats.cor > 50)) outputText(", yer a real prize slut");
		...
		{ //Everything after the sex. Handles awards, gem loss and text for player leaving the bog
			...
			if (phoukaForm == PHOUKA_FORM_FAERIE) { //In this case postCombat means you need an award because you must have won to get faerie sex
					...
					if (player.stats.cor > 50) outputText("  But either way you plan to return and give all of them that lesson.");
					if (postCombat) {
						kGAMECLASS.clearStatuses(false);
						kGAMECLASS.awardPlayer();  //This will provide loot and return to camp, 1 hour used
					}
				...
				if (postCombat && player.stats.gems > 0) outputText("  While you're recovering the " + phoukaName() + " reaches into your gem pouch and takes a handful.");
