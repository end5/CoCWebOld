...
// TIMES_MET_CHAMELEON: number = 561;
		...
		public encounterChameleon():void
		{
			...
			spriteSelect(89);
			if (flags[FlagEnum.TIMES_MET_CHAMELEON] == 0) {
		...
		public loseToChameleonGirl():void
		{
			...
			//-Lose by lust
			if (player.lust > 99) {
				MainScreen.text("Losing control to your own growing arousal, you fall to your knees and desperately start working to get at your needy body beneath your " + player.armorName + ".");
			}
			//Lose by HP
		...
		private dudesLoseToChamChams():void
		{
			...
			if (player.lust > 99) MainScreen.text("overwhelmed by lust");
		...
		public defeatChameleonGirl():void
		{
			...
			if (monster.lust > 99) {
				...
				if (player.lust >= 33) MainScreen.text("  Do you help the horny girl to get you both off?  Or maybe you could punish her bad attitude with something from your bag...");
			}
			//-Win by HP
			else {
				...
				if (player.lust >= 33) MainScreen.text("  Do you use the girl in her weakened state to get yourself off?  Or maybe you could punish her bad attitude with an item from your bags...");
			}
			...
			//let PC use item
			if ((player.hasItem(consumables.SUCMILK) || player.hasItem(consumables.P_S_MLK)) && player.lowerBody.cockSpot.hasCock()) item = useAnItemOnTheChamcham;
		...
		private fuckDatChameleonAsACoolGuyGirlHerm():void
		{
			...
			MainScreen.text("roll her onto her back and get up to collect your things, rinsing yourself with the water and shaking the mud out before putting your " + player.armorName + " back on and heading back to camp.");
			player.orgasm();
			cleanupAfterCombat();
		}
		...
		private manFucksChameleonWithBiggishWang():void
		{
			...
			if (player.lowerBody.balls > 0) MainScreen.text("your " + ballsDescriptLight() + " clench and ");
			MainScreen.text("the semen erupts from the end of your " + cockDescript(x) + " spilling out hot spunk over her folded tongue.  She gasps audibly and arches her back as the thick ropes of your jizz coat her tongue, shoving her long fingers as far as she possibly can into her dripping pussy, and flashes so brightly pink she nearly glows.");
			...
			MainScreen.text("  Satisfied, you put your " + player.armorName + " back on and head back to camp.");
		...
		private femaleHasWinSexWithChamCham():void
		{
			MainScreen.clearText();
			spriteSelect(89);
			MainScreen.text("You shed your " + player.armorName + " and approach the fallen chameleon girl");
			...
			MainScreen.text("roll her onto her back and get up to collect your things, shaking out the mud and water before putting your " + player.armorName + " back on and heading to camp.");
			//send player back to camp, reset hours since cum, add gems and time
			player.orgasm();
			cleanupAfterCombat();
		}
		...
		private useAnItemOnTheChamcham():void
		{
			...
			if ((player.hasItem(consumables.SUCMILK) || player.hasItem(consumables.P_S_MLK)) && player.lowerBody.cockSpot.hasCock()) milk = giveTheChameleonASuccubiMilk;
		...
		private giveTheChameleonASuccubiMilk():void
		{
			...
			MainScreen.text(" your cock aimed for her pillowy tits.  She pushes and squirms beneath you, but cannot get enough leverage to move you.  You take her breasts in your hands and shimmy up her torso towards them, squeezing them around your " + cockDescript(x) + ".");
