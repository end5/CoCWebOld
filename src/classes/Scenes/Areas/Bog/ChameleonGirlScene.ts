 ...
 */
export default class ChameleonGirlScene {
	...
	public encounterChameleon(): void {
		...
		spriteSelect(89);
		if (Flags.list[FlagEnum.TIMES_MET_CHAMELEON] == 0) {
			MainScreen.text("You work your way through the dense foliage of the bog, pushing aside branches and slogging through the thick mud in search of something new.  Feeling exhausted, you slow down and look for a place to rest; finding a small clearing with shallow water no more than a couple inches deep and firmer ground, you sit back against a tree to catch your breath.  You're so soaked by now that you hardly notice the murky water beneath you and the slick mud on the trunk seeping into your " + player.inventory.armor.displayName + ".  You lean your head back and close your eyes, enjoying a few moments of peace.");
			...
			//ENCOUNTER INTRO; REPEAT (Z edited)
			MainScreen.text("You work your way through the dense foliage of the bog, pushing aside branches and slogging through the thick mud in search of something new.  Feeling exhausted, you slow down and look for a place to rest; a small clearing with shallow water and firmer ground seems to fit the bill, and you sit back against a tree to catch your breath.  You're so soaked by now that you hardly notice the murky water beneath you and the slick mud on the trunk seeping into your " + player.inventory.armor.displayName + ".");
		...
		Flags.list[FlagEnum.TIMES_MET_CHAMELEON]++;
		doNext(playerMenu);
	}//LOSS SCENES (Intro) (Z edited)
	public loseToChameleonGirl(): void {
		...
		if (player.stats.lust > 99) {
			MainScreen.text("Losing control to your own growing arousal, you fall to your knees and desperately start working to get at your needy body beneath your " + player.inventory.armor.displayName + ".");
		}
		//Lose by HP
		else MainScreen.text("Too weak to keep fighting, you stagger back and fall to your knees.");
		...
		else if (player.lowerBody.vaginaSpot.hasVagina()) loseToChamChamWithPCCunt();
		else {
	...
	private loseToChameleonWithCockAnBallsAnCunt(): void {
		...
		MainScreen.text("\n\n\"<i>Um, strip for me!</i>\" she commands, as forcefully as she can manage.  Still, it almost sounds like a question.  Too weak to do anything but play along, you comply, shedding your " + player.inventory.armor.displayName + ".");

		MainScreen.text("\n\nAs you set the last bit of your " + player.inventory.armor.displayName + " aside in the water, you're suddenly struck in your chest and pushed to the ground.  The chameleon girl's wide foot keeps you pinned down, and she looks at you almost apologetically.  She shakes the worry from her face, and tries to appear stern once again.  \"<i>I'll... I'll show you who's the boss around here!</i>\"  Her tail darts towards you and uncoils at its end.  The tip starts probing around your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ", brushing across your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " as it feels around for your entrance.");
	...
	private dudesLoseToChamChams(): void {
		let x: number = rand(player.lowerBody.cockSpot.count());
		spriteSelect(89);
		MainScreen.text("\n\nRecognizing that you're no danger, her skin shifts back into its natural " + monster.skinTone + " coloration as she walks towards you.  She swings her hips seductively as she splashes slowly through the water and licks her lips with a loud smack of her tongue, then reaches out and forcefully pulls your " + player.inventory.armor.displayName + " above your waist before you can even try to resist her advances, and pushes you back against a tree.  With a few quick pulls of fabric she's stripped her own clothes, and she tosses her thong over a low-hanging branch.  The other band of silk she uses to quickly bind your wrists.  Were you in any shape to fight back you could pull free, but you're too ");
		if (player.stats.lust > 99) MainScreen.text("overwhelmed by lust");
		else MainScreen.text("battered");
		MainScreen.text(" to even rip the makeshift binding.");

		MainScreen.text("\n\nShe makes short work of your " + player.inventory.armor.displayName + " in her haste to see her prize, then stands up and, still looking down, lets her tongue fall out of her mouth and find its way to [oneCock].  In an amazing display of control, she manages to wrap the warm pink appendage around your man-meat, sliding the slick length of her warm tongue up and down your shaft almost as if she were massaging you.  You shudder softly as she tickles the underside of your cock with just the tip of her tongue, trailing sticky saliva around its head.  Your member is soon erect and throbbing with need; satisfied, she slurps the slender length of muscle back into her mouth.");
	...
	public defeatChameleonGirl(): void {
		...
		//-Win by lust
		if (monster.lust > 99) {
			MainScreen.text("Unable to control her arousal, the chameleon girl collapses to her knees and begins masturbating underneath her thong, having lost all capacity to fight you; she moans and throws her head back as her hand splashes in and out of the water she's kneeling in.  Her skin returns to its usual " + monster.skinTone + " and then keeps going, shifting closer and closer to pink as her moans increase in both volume and volubility.");
			if (player.stats.lust >= 33) MainScreen.text("  Do you help the horny girl to get you both off?  Or maybe you could punish her bad attitude with something from your bag...");
		}
		//-Win by HP
		else {
			MainScreen.text("Too weak to continue fighting, the chameleon girl drops to her knees, exhausted.  Her skin returns to its usual " + monster.skinTone + ", unable to maintain the camouflage.");
			if (player.stats.lust >= 33) MainScreen.text("  Do you use the girl in her weakened state to get yourself off?  Or maybe you could punish her bad attitude with an item from your bags...");
		}
		...
		if (player.lowerBody.cockSpot.hasCock()) dick = manFucksChameleonWithBiggishWang;
		if (player.lowerBody.vaginaSpot.hasVagina()) pussy = femaleHasWinSexWithChamCham;
		if (player.gender == 3) herm = fuckDatChameleonAsACoolGuyGirlHerm;
		//let PC use item
		if ((player.inventory.items.has(consumables.SUCMILK) || player.inventory.items.has(consumables.P_S_MLK)) && player.lowerBody.cockSpot.hasCock()) item = useAnItemOnTheChamcham;
	...
	private fuckDatChameleonAsACoolGuyGirlHerm(): void {
		...
		if (player.stats.cor < 50) MainScreen.text("and guiltily ");
		MainScreen.text("roll her onto her back and get up to collect your things, rinsing yourself with the water and shaking the mud out before putting your " + player.inventory.armor.displayName + " back on and heading back to camp.");
		player.orgasm();
		cleanupAfterCombat();
	}
	...
	private manFucksChameleonWithBiggishWang(): void {
		...
		if (player.lowerBody.balls > 0) MainScreen.text("your " + BallsDescriptor.describeBalls(true, true, player) + " clench and ");
		MainScreen.text("the semen erupts from the end of your " + CockDescriptor.describeCock(player, x) + " spilling out hot spunk over her folded tongue.  She gasps audibly and arches her back as the thick ropes of your jizz coat her tongue, shoving her long fingers as far as she possibly can into her dripping pussy, and flashes so brightly pink she nearly glows.");
		if (player.cumQ() > 750) MainScreen.text("  The torrent of cum bursting from your cock just doesn't stop, and her eyes roll back into her head as your semen coats her face and runs off her tongue, drenching her breasts and belly in white.  Her head falls back and her tongue goes limp as she's sent into ecstasy by the flood.");
		...
		if (player.cumQ() > 750) MainScreen.text("  Even the murky water around her is turned white as the pools of your cum on her body run off into it.");
		MainScreen.text("  Satisfied, you put your " + player.inventory.armor.displayName + " back on and head back to camp.");
	...
	private femaleHasWinSexWithChamCham(): void {
		MainScreen.clearText();
		spriteSelect(89);
		MainScreen.text("You shed your " + player.inventory.armor.displayName + " and approach the fallen chameleon girl");
		if (monster.lust > 99) MainScreen.text(", her moans of pleasure growing more desperate as you approach");
		MainScreen.text(".  You look her up and down, following the smooth curves of her thin body with your eyes.  Squatting down beside her, you take her weak body by the shoulders in one arm and trail your fingertips lightly up her torso with the other, feeling the soft skin of her belly.");
		if (monster.lust > 99) MainScreen.text("  She coos lustily at the sensation.");
		else MainScreen.text("  Her eyes widen, and she looks from your hand to your face and back again.");
		MainScreen.text("  When your hand reaches the silk cloth covering her chest, you pull it off with a light tug and cast it off to the side.  You cup her small breast in your palm, and as you squeeze it her skin tone changes perceptibly, flashing to a much brighter shade of the colors it was.");
		...
		if (player.stats.cor < 50) MainScreen.text("and somewhat guiltily ");
		MainScreen.text("roll her onto her back and get up to collect your things, shaking out the mud and water before putting your " + player.inventory.armor.displayName + " back on and heading to camp.");
		//send player back to camp, reset hours since cum, add gems and time
		player.orgasm();
		cleanupAfterCombat();
	}
	...
	private useAnItemOnTheChamcham(): void {
		MainScreen.clearText();
		spriteSelect(89);
		MainScreen.text("Looking at the poor girl kneeling pathetically before you, you almost feel sorry for her.  But, she did try to rape you and claim that you were on 'her' territory.  You rummage through your bags, trying to find a fun way to punish her.  Fun for you, at least.");
		...
		let drafts: Function = null;
		if ((player.inventory.items.has(consumables.SUCMILK) || player.inventory.items.has(consumables.P_S_MLK)) && player.lowerBody.cockSpot.hasCock()) milk = giveTheChameleonASuccubiMilk;
		if (player.inventory.items.has(consumables.SENSDRF) && (player.inventory.items.has(consumables.L_DRAFT) || player.inventory.items.has(consumables.F_DRAFT))) drafts = doseDatChameleonWithLustAndSensitivityDrafts;
	...
	private giveTheChameleonASuccubiMilk(): void {
		MainScreen.clearText();
		spriteSelect(89);
		MainScreen.text("You find a bottle of Succubi Milk tucked away in a pouch.  A wicked smile comes to your face as you imagine filling her so full of the stuff that her tits grow to the point where she can't even move, but you ultimately decide that just one dose will suffice for now.");
		...
		if (player.hasLongTail() > 0) MainScreen.text(" with your tail,");
		else MainScreen.text(" by straddling her,");
		MainScreen.text(" your cock aimed for her pillowy tits.  She pushes and squirms beneath you, but cannot get enough leverage to move you.  You take her breasts in your hands and shimmy up her torso towards them, squeezing them around your " + CockDescriptor.describeCock(player, x) + ".");
		...
		player.orgasm();
		if (player.inventory.items.has(consumables.P_S_MLK)) player.inventory.items.consumeItem(consumables.P_S_MLK);
		else {
	...
	private doseDatChameleonWithLustAndSensitivityDrafts(): void {
		MainScreen.clearText();
		spriteSelect(89);
		MainScreen.text("You dig through your bags and come across a particularly tortuous combination of fluids.  With a Lust Draft in one hand and a Sensitivity Draft in the other, you return to the chameleon girl, swirling them and smiling at her menacingly.  She starts to protest, but you cut her off by shoving the opening of the Lust Draft vial into her open mouth.");
		...
		MainScreen.text("\n\nToo turned on to just sit there, she falls forward, catching herself on you, then looks up, pleading with her eyes.  You laugh mirthlessly at her grovelling form, and tell her to beg if she wants you.  \"<i>Please!</i>\" she squeals, her eyes watering, \"<i>I need this!</i>\"  She tugs insistently at your " + player.inventory.armor.displayName + ", begging for you to fill her and crying out that she can't stand to feel so empty.  You grin, and wonder if the sensitivity draft has started to take effect.  Two fingers run lightly up her forearm produce shivers at the obviously amplified sensation, and a tweak of her nipple elicits a shrill shriek.  \"<i>Why does it feel so good?!</i>\"  She puts her face into your lap and begs, \"<i>I don't even know what's going on, please, just fuck me, pleasepleaseplease...</i>\" trailing off as she grinds her pussy into you.");
		...
		if (!player.lowerBody.cockSpot.hasCock() || player.cockArea(player.smallestCockIndex()) > monster.vaginalCapacity()) {
		...
		player.inventory.items.consumeItem(consumables.SENSDRF);
