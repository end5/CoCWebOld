...
// TIMES_MET_CHAMELEON: number = 561;
		...
		public loseToChameleonGirl():void
		{
			...
			//-Lose by lust
			if (player.lust > 99) {
				Render.text("Losing control to your own growing arousal, you fall to your knees and desperately start working to get at your needy body beneath your " + player.armorName + ".");
			}
			//Lose by HP
			else Render.text("Too weak to keep fighting, you stagger back and fall to your knees.");
		...
		private loseToChameleonWithCockAnBallsAnCunt():void
		{
			...
			//[if(cockarea <40)
			if (player.cockArea(x) < 40) Render.text(" and occasionally lifting it to take the shaft between two dexterous toes and work it more delicately");
			Render.text(".  She blushes as she watches your breaths turn to pants from the pleasure.");
		...
		private dudesLoseToChamChams():void
		{
			...
			if (player.lust > 99) Render.text("overwhelmed by lust");
			else Render.text("battered");
			Render.text(" to even rip the makeshift binding.");
			...
			if (player.cumQ() > 500) Render.text(", filling her with so much cum that it leaks out of her and drips to the water below");
			Render.text(".  The chameleon girl's pace slows once again, and both of your breaths come in deeply as you each recover from orgasm.  She sucks her hanging tongue most of the way back into her mouth, grimacing as she wipes some mud from it.");
		...
		public defeatChameleonGirl():void
		{
			...
			//-Win by lust
			if (monster.lust > 99) {
				Render.text("Unable to control her arousal, the chameleon girl collapses to her knees and begins masturbating underneath her thong, having lost all capacity to fight you; she moans and throws her head back as her hand splashes in and out of the water she's kneeling in.  Her skin returns to its usual " + monster.skinTone + " and then keeps going, shifting closer and closer to pink as her moans increase in both volume and volubility.");
				if (player.lust >= 33) Render.text("  Do you help the horny girl to get you both off?  Or maybe you could punish her bad attitude with something from your bag...");
			}
			//-Win by HP
			else {
				Render.text("Too weak to continue fighting, the chameleon girl drops to her knees, exhausted.  Her skin returns to its usual " + monster.skinTone + ", unable to maintain the camouflage.");
				if (player.lust >= 33) Render.text("  Do you use the girl in her weakened state to get yourself off?  Or maybe you could punish her bad attitude with an item from your bags...");
			}
		...
		private fuckDatChameleonAsACoolGuyGirlHerm():void
		{
			...
			if (player.stats.cor < 50) Render.text("and guiltily ");
			Render.text("roll her onto her back and get up to collect your things, rinsing yourself with the water and shaking the mud out before putting your " + player.armorName + " back on and heading back to camp.");
			player.orgasm();
			cleanupAfterCombat();
		}
		...
		private manFucksChameleonWithBiggishWang():void
		{
			...
			if (player.lowerBody.balls > 0) Render.text("your " + ballsDescriptLight() + " clench and ");
			Render.text("the semen erupts from the end of your " + cockDescript(x) + " spilling out hot spunk over her folded tongue.  She gasps audibly and arches her back as the thick ropes of your jizz coat her tongue, shoving her long fingers as far as she possibly can into her dripping pussy, and flashes so brightly pink she nearly glows.");
			if (player.cumQ() > 750) Render.text("  The torrent of cum bursting from your cock just doesn't stop, and her eyes roll back into her head as your semen coats her face and runs off her tongue, drenching her breasts and belly in white.  Her head falls back and her tongue goes limp as she's sent into ecstasy by the flood.");
			...
			if (player.cumQ() > 750) Render.text("  Even the murky water around her is turned white as the pools of your cum on her body run off into it.");
			Render.text("  Satisfied, you put your " + player.armorName + " back on and head back to camp.");
		...
		private femaleHasWinSexWithChamCham():void
		{
			clearOutput();
			spriteSelect(89);
			Render.text("You shed your " + player.armorName + " and approach the fallen chameleon girl");
			if (monster.lust > 99) Render.text(", her moans of pleasure growing more desperate as you approach");
			Render.text(".  You look her up and down, following the smooth curves of her thin body with your eyes.  Squatting down beside her, you take her weak body by the shoulders in one arm and trail your fingertips lightly up her torso with the other, feeling the soft skin of her belly.");
			if (monster.lust > 99) Render.text("  She coos lustily at the sensation.");
			else Render.text("  Her eyes widen, and she looks from your hand to your face and back again.");
			Render.text("  When your hand reaches the silk cloth covering her chest, you pull it off with a light tug and cast it off to the side.  You cup her small breast in your palm, and as you squeeze it her skin tone changes perceptibly, flashing to a much brighter shade of the colors it was.");
			...
			if (player.stats.cor < 50) Render.text("and somewhat guiltily ");
			Render.text("roll her onto her back and get up to collect your things, shaking out the mud and water before putting your " + player.armorName + " back on and heading to camp.");
			//send player back to camp, reset hours since cum, add gems and time
			player.orgasm();
			cleanupAfterCombat();
		}
		...
		private useAnItemOnTheChamcham():void
		{
			clearOutput();
			spriteSelect(89);
			Render.text("Looking at the poor girl kneeling pathetically before you, you almost feel sorry for her.  But, she did try to rape you and claim that you were on 'her' territory.  You rummage through your bags, trying to find a fun way to punish her.  Fun for you, at least.");
		...
		private giveTheChameleonASuccubiMilk():void
		{
			clearOutput();
			spriteSelect(89);
			Render.text("You find a bottle of Succubi Milk tucked away in a pouch.  A wicked smile comes to your face as you imagine filling her so full of the stuff that her tits grow to the point where she can't even move, but you ultimately decide that just one dose will suffice for now.");
			...
			if (player.hasLongTail() > 0) Render.text(" with your tail,");
			else Render.text(" by straddling her,");
			Render.text(" your cock aimed for her pillowy tits.  She pushes and squirms beneath you, but cannot get enough leverage to move you.  You take her breasts in your hands and shimmy up her torso towards them, squeezing them around your " + cockDescript(x) + ".");
		...
		private doseDatChameleonWithLustAndSensitivityDrafts():void
		{
			clearOutput();
			spriteSelect(89);
			Render.text("You dig through your bags and come across a particularly tortuous combination of fluids.  With a Lust Draft in one hand and a Sensitivity Draft in the other, you return to the chameleon girl, swirling them and smiling at her menacingly.  She starts to protest, but you cut her off by shoving the opening of the Lust Draft vial into her open mouth.");
