...
private function serviceLowCorruptionHighLust():void
{
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		...
		if (player.lowerBody.cockSpot.count() > 1) outputText("Your own erect " + multiCockDescriptLight() + " rub against the creature's as you work, getting in the way somewhat.  ", false);
		//Big single cocked folk get in their own way too!
		else if(player.lowerBody.cockSpot.list[0].cockThickness >= 36) outputText("Your own erect " + cockDescript(0) + " rubs against the creature's as you work, getting in the way somewhat.  ", false);
		//Single dick contemplation
		if(player.lowerBody.cockSpot.count() == 1) outputText("You feel your " + cockDescript(0) + " begin to throb and entertain the idea of trying to penetrate it somehow.  You turn a bit and look around the creature for an orifice, but find nothing.  After a few frustrated moments of searching you give up and decide to handle things yourself.", false);
	...
	if(player.tallness < 82)
	{
		...
		if (player.gender == 1 || (player.gender == 3 && rand(2) == 0))
		{
			...
			{
				if(player.lowerBody.cockSpot.count() > 0) outputText("Eventually your own arousal becomes unbearable and you reach down to work your one of your " + multiCockDescriptLight() + ", massaging the monster's swollen, throbbing head with your other hand.  You gasp slightly as soon as you wrap your hand around your " + cockDescript(1) + ", the creature's fluid making you exceptionally sensitive.  You moan in spite of yourself and start to thrust your hips a bit against your fist, feeling both yourself and the creature begin to throb harder and harder.  Before long you can hold out no longer, and feel your body racked by an orgasmic spasm, squeezing tightly with both hands as you blow your " + multiCockDescriptLight() + " cover the creature with cum.  ", false);
				//Herms - vegemita vaga-cum.
				if(player.lowerBody.vaginaSpot.count() > 0) outputText("Your " + vaginaDescript(0) + " quivers and shares in the sensation as you grind yourself against the slime.  ", false);
				//Create cumz
		...
		else if (player.gender == 2 || player.gender == 3)
		{
			...
			//Big clitties
			if(player.lowerBody.vaginaSpot.list[0].clitLength >= 2) outputText("You moan and lean back slightly as you grip your " + clitDescript() + " between your thumb and forefinger, biting your lip at the intense sensations as you rub your thumb over your " + clitDescript() + " and shuddering in anticipation and delight.  ", false);
			//Small clitties
			...
			if (player.upperBody.chest.count() > 0 && player.upperBody.chest.BreastRatingLargest[0].breastRating > 0)
			{
				...
				//Tig ol' bitties
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 8) outputText("barely contains them.", false)
				//Middling Boobiliciousness
				else if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) outputText("gently covers them.", false)
				//Tiny ta-tas.
			...
			if (player.lowerBody.cockSpot.count() > 0)
			{
				...
				if(player.lowerBody.cockSpot.count() > 1)
				{
					...
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating == 0) outputText("You delicately grasp the head of one of your cocks, your hand slick with the fluids of yourself and of the creature.  The combination of the creature's movements and your own quickly overwhelms you.", false);
				}
				//Single prick
				else
				{
					...
					if(player.upperBody.chest.BreastRatingLargest[0].breastRating == 0) outputText("You delicately grasp the head of your " + cockDescript(0) + ", your hand slick with the fluids of yourself and of the creature.  The combination of the creature's movements and your own quickly overwhelms you.", false);
				}
			}
			...
			{
				//Multidicked herms
				if (player.lowerBody.cockSpot.count() > 1)	outputText("You let out a strangled gasp as your body, already straining from exertion, shudders again as your " + multiCockDescript() + " explodes into orgasm, showering both you and the creature in jizz.  ", false);
				//Single cocked herms
	...
	else if(player.tallness >= 82)
	{
		...
		if(player.gender == 1 || (player.gender == 3 && rand(2) == 0))
		{
			...
			else
			{
				...
				//Compare cocks
				if(player.lowerBody.cockSpot.list[0].cockType == CockType.HUMAN) outputText("a near exact replica of your own " + cockDescript(0) + "!  ", false);
		...
		else if(player.gender == 2 || player.gender == 3)
		{
			...
			if(player.upperBody.chest.count() > 0 && player.upperBody.chest.BreastRatingLargest[0].breastRating > 0)
			{
				...
				if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 8) outputText("Its hand expands, struggling to cover all of your " + allBreastsDescript() + ", ", false);
				//Middling ta-tas get covered
				else if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) outputText("Its hand expands and gently covers your " + allBreastsDescript() + ", ", false);
				//Small tits?  Pwned.
			...
			if(player.lowerBody.cockSpot.count() > 0)
			{
				...
				if(player.upperBody.chest.BreastRatingLargest[0].breastRating == 0 || player.upperBody.chest.count() == 0)
				{
					...
					//multi!
					if(player.lowerBody.cockSpot.count() > 1) outputText("penii ", false);
					//Single
...
private function maleRapesOoze():void {
	...
	{
		//MULTICAWK
		if(player.lowerBody.cockSpot.count() > 1) outputText("Shrugging, you decide to go with the closest thing available and walk towards its head.  You spend a moment deciding which of your organs to ravage the creature with, eventually settling on the largest,  ", false);
		...
		//Balls ftw
		if(player.lowerBody.balls > 1) outputText("anointing the slime's face with your " + sackDescript() + " as you do.  ", false);
		//Ball-less bitches
		...
		if(player.lowerBody.cockSpot.list[0].cockLength >= 12) outputText(", down its throat,", false);
		if(player.lowerBody.cockSpot.list[0].cockLength >= 24) outputText(" into its torso,", false);
		if(player.lowerBody.cockSpot.list[0].cockLength >= 48) outputText(" and all the way through to its hips,", false);
		...
		if(player.stats.cor >= 60) outputText("You wonder for a moment if you've broken the creature's membranous skin, then shrug and decide you don't care.", false);
		...
		//Balls ftw
		if(player.lowerBody.balls > 1) outputText(" as your " + ballsDescriptLight() + " slap against its face", false);
		...
		//vaginal orgasm text
		if(player.lowerBody.vaginaSpot.count() > 0) outputText("  This is quickly followed by a quivering between your nether-lips as your " + vaginaDescript(0) + " quivers in orgasm shortly afterwards.", false);
	...
	else
	{
		...
		if(player.lowerBody.cockSpot.count() > 1)
		{
			...
			if(player.lowerBody.cockSpot.count() == 2) outputText("larger", false);
		...
		if(player.lowerBody.cockSpot.list[0].cockLength >= 24) outputText("By the time you finally bottom out, your " + cockDescript(0) + " is nearly halfway through its torso.", false);
		else if(player.lowerBody.cockSpot.list[0].cockLength >= 48) outputText("By the time you finally bottom out, your " + cockDescript(0) + " reaches nearly into the slime's neck.", false);
		else if(player.lowerBody.cockSpot.list[0].cockLength >= 60) outputText("Your penis keeps going deeper and deeper, through the chest, neck, and head, before finally stretching the slime as far as it needs to go to accommodate the full length of your " + cockDescript(0) + ".", false);
		...
		//vaginal arousal text
		if(player.lowerBody.vaginaSpot.count() > 0) outputText("Your " + vaginaDescript(0) + " grows wet as you work the thing's hole, every slap of your hips against the thing's ass sending a pang of pleasure through your nethers.  ", false);
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("your " + multiCockDescriptLight() + " explodes into orgasm, your load filling the space inside the creature and spraying in thick streams across its back.", false);
		else outputText("exploding inside it, your load filling the space around your cock with a white, milky fluid.", false);
		//vaginal orgasm text
		if(player.lowerBody.vaginaSpot.count() > 0) outputText("  As you pump your jizz into the creature an almost electrical shock runs through your body, starting from your " + vaginaDescript(0) + " and radiating outwards.", false);
...
private function femaleRapesOoze():void {
	...
	else
	{
		...
		outputText("<<ERROR: breast size check returned invalid number -" + String(player.upperBody.chest.BreastRatingLargest[0].breastRating) + "- at green slime' player win + vagina + high lust;>>", false);
	}*/
	...
	if(player.lowerBody.cockSpot.count() > 0)
	{
		if(player.lowerBody.cockSpot.count() > 1)
		{
			...
			outputText(num2Text(player.lowerBody.cockSpot.count()), false);
...
private function oozeButtRapesYou():void 
{
	...
	//Girls get fondled. 
	if(player.lowerBody.vaginaSpot.count() > 0) outputText(vaginaDescript(0)+ " and " + assholeDescript(), false);
	//No coochie
	else {
		//Ballsy
		if(player.lowerBody.balls >= 2) outputText(assholeDescript()+ " and " + ballsDescript(), false);
	...
	if(player.analCapacity() < 18 || player.ass.analLooseness == 0)
	{
		...
		if(player.stats.cor <= 50) outputText("You feel a bit of fear as you tremble under the creature's weight, worried about the damage that its massive size might do to your ass.  ", false);
		else if(player.stats.cor < 100) outputText("In spite of your worries about the creature's size, you can't help but feel a little bit excited at the idea of having this beast take you from behind.  ", false);
	...
	//cock arousal
	if(player.lowerBody.cockSpot.count() > 0) outputText("You feel pangs of pleasure run through your body as its member repeatedly strokes your prostate from the inside, stimulating it.  ", false);
	...
	{
		//Single
		if(player.lowerBody.cockSpot.count() == 1) outputText("You quickly grow erect under the creature's embrace as it pistons into your ass, and before long you stand at full attention.  You weakly rub yourself with one hand as it pumps you, and quickly approach orgasm.  ", false);
		//Multi!
	...
	//mangasm
	else if(player.lowerBody.cockSpot.count() > 0) outputText("The additional pressure against your prostate finally pushes you over the edge and you come hard, spraying your load against the ground.  Thick ropes of jizm spray onto the ground as the feeling wracks your body.  ", false) 
...
private function oozeRapesYouOrally():void
{
	...
	if(player.stats.cor < 20) outputText("you blush as ", false);
	...
	if(player.stats.cor >= 70)
	{
		...
		if(player.lowerBody.vaginaSpot.count() > 0) {
			...
			if(player.lowerBody.cockSpot.count() > 0) outputText("also ", false);
			...
			if(player.lowerBody.cockSpot.count() < 1 || player.lust <= 40) outputText("You try to reach for yourself to masturbate, but find yourself unable to reach around the slime's massive bulk.", false);
		}
	...
	else
	{
		...
		if(player.stats.int > player.tou) outputText("You force yourself to keep your mouth shut in spite of the lack of air, holding out for as long as you can.  Eventually you become light headed, and pass out.  When you wake up there is no trace of the creature, save for the fluid covering your mouth, chest, and the ground around you.", false);
		else {
...
private function oozeRapesYouVaginally():void
{
	...
	if(player.stats.cor <= 20)
	{
		...
		if(player.lowerBody.cockSpot.count() > 0)
		{
			...
			if(player.lowerBody.cockSpot.count() > 1)
			{
				...
				if(player.lowerBody.cockSpot.count() > 2) outputText("switching between cocks as you rapidly stroke yourself in time with the creature's movements.  ", false);
		...
		{
			//Multi
			if(player.lowerBody.cockSpot.count() > 1) outputText("Your " + multiCockDescript() + " climaxes as well, sending rope after rope of hot, thick cum onto your face and chest.", false);
			//Single
	...
	else
	{
		...
		{
			//multi
			if(player.lowerBody.cockSpot.count() > 1) outputText("Your " + multiCockDescript() + " climaxes as well, and you open your mouth as you spray rope after rope of hot, thick jism onto your face and chest.  You relish its salty taste, continuing to stroke yourself even as you come.  ", false);
			//single cock
