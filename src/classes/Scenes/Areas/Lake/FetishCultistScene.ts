	...
	public class FetishCultistScene extends AbstractLakeContent
	{
		...
		public function cultistRapesYou():void
		{
			...
			if (rand(2) == 0 && !sexed) {
				...
				if (player.lowerBody == LOWER_BODY.CENTAUR) {
					...
					if (player.lowerBody.cockSpot.count() > 0) outputText("  As you see your " + multiCockDescriptLight() + ", 'incredibly large' is the only qualifier you can think of.", false);
				}
				outputText("\n\n", false);
				...
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 1 && !changed) {
					...
					if (player.upperBody.chest.count() == 2) outputText("Once about a quarter of the fluid has been injected into the first breast, she moves on to the second and you feel a similar sharp pain followed by the glowing pleasure.  She continues this until all of your " + player.allBreastsDescript() + " have been injected.  ", false);
					//[if the player has three rows of breasts]
					if (player.upperBody.chest.count() > 2)  outputText("Once about a sixth of the fluid has been injected into the first breast, she moves on to the second and you feel a similar sharp pain followed by the glowing pleasure.  She continues this until all of your " + player.allBreastsDescript() + " have been injected.  ", false);
				...
				if (player.lowerBody.vaginaSpot.count() > 0 && player.lowerBody.cockSpot.count() > 0) {
					...
					if (player.lowerBody.cockSpot.count() == 2) outputText("Your " + cockDescript(1) + " rubs against her skin pleasantly.  ", false);
					//[if player has more than 2 dicks] 
					if (player.lowerBody.cockSpot.count() > 2) outputText("The rest of your " + multiCockDescriptLight() + " rubs against her skin pleasantly.  ", false);
					...
					if (player.lowerBody.cockSpot.count() > 1) outputText("  You take special care to make sure that the rest of your cum does not get on your mistress in any way.", false);
					...
					if (player.lowerBody.cockSpot.count() > 0 && (player.lowerBody.vaginaSpot.count() == 0 || rand(2) == 0)) {
						...
						if (player.lowerBody.vaginaSpot.count()) outputText(", your " + vaginaDescript(0) + ",", false);
						...
						if (player.lowerBody.vaginaSpot.count() > 0) outputText("You feel desire start to well up in your " + vaginaDescript(0) + ", but you resist the urge to act on it; that's not why your mistress keeps you. ", false);
						outputText("She starts to rock back and forth, your " + cockDescript(0) + " bringing her to the height of pleasure. ", false);
						if (player.lowerBody.cockSpot.count() == 2) outputText("Your " + cockDescript(1) + " rubs against her skin pleasantly.  ", false);
						if (player.lowerBody.cockSpot.count() > 2) outputText("The rest of your " + multiCockDescriptLight() + " rub against her skin pleasantly.  ", false);
					...
					//(variant 2: maid, requires vagina)
						...
						if (player.lowerBody.cockSpot.count() > 0) outputText(", your " + multiCockDescriptLight() + ",", false);
						...
						//[if player has a dick]
						if (player.lowerBody.cockSpot.count() > 0) outputText("You feel a desire rising in your erect " + multiCockDescriptLight() + " but you ignore it; that's not what your mistress told you to play with.  ", false);
		...
		public function playerRapesCultist():void
		{
			...
			if (monster.armorName == "pervy nun's clothing") {
				...
				if (player.stats.cor < 40) outputText("You are rather taken aback by this odd declaration, but in the end you decide: <i>what the hell, I'm horny.</i>\n\n", false);
				...
				if (player.lowerBody.cockSpot.count() > 0 && (player.gender != 3 || rand(2))) {
					...
					if (player.lowerBody == LOWER_BODY.CENTAUR) outputText("You command her to get on all fours, then cover her and push ", false);
					else outputText("You grab hold of and lift her legs, then kneel down and push ", false);
					if (player.lowerBody.cockSpot.count() > 1) outputText("the first of ", false);
					...
					if (player.lowerBody.cockSpot.count() > 1) {
						...
						if (player.lowerBody.cockSpot.count() > 2) {
							...
							if (player.lowerBody.cockSpot.count() == 3) outputText("last", false);
							if (player.lowerBody.cockSpot.count() > 3) outputText("rest", false);
				...
				if (flags[FlagEnum.PC_FETISH] > 0) outputText("Given your experience with Ceraph, you can definitely understand why she is having so much fun showing off her body.  In fact, you think you'll start to do the same.", false);
				...
				//corruption check!
				if (player.stats.cor < 33) outputText("Well, she obviously wants this, so you guess there isn't anything wrong with enjoying what's on offer, is there?\n\n", false);
				else if (player.stats.cor < 66) outputText("You think to yourself how awesome this is.  It isn't often a defeated foe is so eager.\n\n", false);
				...
				if (player.lowerBody.cockSpot.hasCock()) {
					...
					//if (PC has a vagina)
					if (player.lowerBody.vaginaSpot.hasVagina()) outputText("Then, she hesitates for some reason, before saying, \"<i>Um, I'm not sure why I'm asking this since you've got a good tool there already, but... I've got some vibrators, if you want to play with them instead...</i>\"\n\n", false);
				...
				if (player.stats.cor < 40) outputText("You stop, thinking that maybe you should let her go, but she grabs your hand and gives you a look of desperation.  Deciding to finish the job, you push her to her knees and the look of fear returns to her face.  ", false);
				...
				if (player.lowerBody.cockSpot.count() > 0 && (player.gender != 3 || rand(2))) {
					...
					//[if not corrupted] 
					if (player.stats.cor < 40) outputText("Feeling uncomfortable at this, you start to pull away, but her expression turns instantly into a look of pure horror and her body becomes completely tense.  As you move back to her, her body relaxes and her expression returns to the tear-stricken face.  Obviously this is an act and she would be far more bothered by you if you left her without finishing the job.  ", false);
		...
		private function plugSwimsuitVag():void
		{
			...
			outputText("", true);
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("You decline the vibrators; you'd much rather take her incredibly wet pussy.  ", false);
			...
			if (player.lowerBody.cockSpot.count() > 1) {
				...
				if (player.lowerBody.cockSpot.count() == 2) outputText(" lets loose its load", false);
