...
//[Lake]
		public function encounterGooGirl():void
		{
			Render.text("", true);
			spriteSelect(69);
			Render.text("As you walk around the lake, you notice a pale red light pulsing in the ", false);
			if (player.findStatusAffect(StatusAffects.FactoryOverload) < 0) Render.text("sapphire ", false);
			else Render.text("murky ", false);
			Render.text("waters. You pause, trying to figure out what the shape might be. Just under the surface of the water, there appears to be a fist-sized heart shedding a crimson glow. Leaning closer, you gaze down into your reflection only to find your face rising up with pursed lips, trying to kiss you! You jerk backwards and the pseudo-head quivers, resolving its face into a gooey-looking girl, her ", false);
			startCombat(new GooGirl());
			Render.text(gooColor() + " slime body sculpting itself into a humanoid shape. The girl curiously tilts her head to one side, as if trying to figure out why you're backing away, before she happily surges forward!", false);
			doNext(playerMenu);
		}
...
//New Perk – Slime Core (requires goo player, random drop rate?)
		private function coreDropChance():void
		{
			if (rand(4) == 0 && player.statusAffects.has("SlimeCraving") && !player.perks.has("SlimeCore") && player.isGoo() && player.gooScore() >= 4) {
				...
				Render.text("(<b>Gained New Perk: Slime Core - Moisture craving builds at a greatly reduced rate.</b>)", false);
...
//VICTORY
		public function beatUpGoo():void
		{
			...
			if (player.gender == 0 || player.lust < 33) {
				Render.text("The goo-girl, while an unusual creature, seems unable to communicate and clearly has nothing of value.  Of no particular use in your quest, you shoo the dripping blob back to the shore. She seems disappointed at first, but bounces back quickly enough, spotting movement in the lake.  She splashes in and takes off at top speed, " + gooColor9() + " blur while you take your leave.", false);
				cleanupAfterCombat();
			}
			else {
				Render.text("With the goo-girl defeated, her unusual body is at your mercy.  What do you do?", false);
				let sex1S:string = "";
				...
				if ((flags[FlagEnum.GOO_TFED_MEAN] == 0 && flags[FlagEnum.GOO_TFED_NICE] == 0) && flags[FlagEnum.TIMES_FUCKED_NORMAL_GOOS] >= 2) {
					if (player.stats.cor < 50 && (player.hasItem(consumables.SUCMILK) || player.hasItem(consumables.P_S_MLK)) && (player.hasItem(consumables.BLACKEG) || player.hasItem(consumables.L_BLKEG))) {
						...
						if ((player.hasItem(consumables.SUCMILK) || player.hasItem(consumables.P_S_MLK)) && (player.hasItem(consumables.BLACKEG) || player.hasItem(consumables.L_BLKEG))) {
							Render.text("  Good thing you have those handy!");
				...
				if (valeria != null) {
					...
					if (flags[FlagEnum.TIMES_VALERIA_GOO_THREESOMED] == 0) Render.text("Do you offer a threesome with the girl to Valeria? It could get a little weird....");
					else Render.text("Do you offer a threesome with the girl to Valeria? She'll likely try flood with you with more sloshing, shuddering pleasure than your body can handle.");
				}
