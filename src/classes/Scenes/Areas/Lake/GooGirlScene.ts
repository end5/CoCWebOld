...
//[Lake]
		public encounterGooGirl():void
		{
			...
			spriteSelect(69);
			MainScreen.text("As you walk around the lake, you notice a pale red light pulsing in the ", false);
			if (player.findStatusAffect(StatusAffects.FactoryOverload) < 0) MainScreen.text("sapphire ", false);
			else MainScreen.text("murky ", false);
			MainScreen.text("waters. You pause, trying to figure out what the shape might be. Just under the surface of the water, there appears to be a fist-sized heart shedding a crimson glow. Leaning closer, you gaze down into your reflection only to find your face rising up with pursed lips, trying to kiss you! You jerk backwards and the pseudo-head quivers, resolving its face into a gooey-looking girl, her ", false);
			startCombat(new GooGirl());
			MainScreen.text(gooColor() + " slime body sculpting itself into a humanoid shape. The girl curiously tilts her head to one side, as if trying to figure out why you're backing away, before she happily surges forward!", false);
			doNext(playerMenu);
		}
...
//New Perk – Slime Core (requires goo player, random drop rate?)
		private coreDropChance():void
		{
			if (rand(4) == 0 && player.statusAffects.has("SlimeCraving") && !player.perks.has("SlimeCore") && player.isGoo() && player.gooScore() >= 4) {
				...
				MainScreen.text("There is a momentary pressure in your chest and a few memories that are not your own flicker before your eyes. The dizzying sight passes and the slime core settles within your body, imprinted with your personality and experiences. There is a comforting calmness from your new nucleus and you feel as though, with your new memories, you will be better able to manage your body's fluid requirements.\n\n", false);
				//(Reduces Fluid Addiction to a 24 hour intake requirement).
				MainScreen.text("(<b>Gained New Perk: Slime Core - Moisture craving builds at a greatly reduced rate.</b>)", false);
...
//goo-girl Bad End – If the player loses to 3-5 goo encounters while under full goo Transformation
		...
		private slimeBadEnd2():void {
			...
			MainScreen.text("One year later...");
...
//VICTORY
		public beatUpGoo():void
		{
			...
			if (player.gender == 0 || player.lust < 33) {
				MainScreen.text("The goo-girl, while an unusual creature, seems unable to communicate and clearly has nothing of value.  Of no particular use in your quest, you shoo the dripping blob back to the shore. She seems disappointed at first, but bounces back quickly enough, spotting movement in the lake.  She splashes in and takes off at top speed, " + gooColor9() + " blur while you take your leave.", false);
				cleanupAfterCombat();
			}
			else {
				MainScreen.text("With the goo-girl defeated, her unusual body is at your mercy.  What do you do?", false);
				let sex1S:string = "";
				let sex1N:Function =null;
				let sex2S:string = "";
				let sex2N:Function =null;
				let sex3S:string = "";
				let sex3N:Function =null;
				let sex4S:string = "";
				let sex4N:Function =null;
				...
				if (player.lowerBody.cockSpot.hasCock()) {
					if (player.lowerBody.cockSpot.list[player.smallestCockIndex()].cockLength < 24) {
						sex1S = "DickSex";
				...
				let gooTF:Function = null;
				//corrupt chances
				if ((flags[FlagEnum.GOO_TFED_MEAN] == 0 && flags[FlagEnum.GOO_TFED_NICE] == 0) && flags[FlagEnum.TIMES_FUCKED_NORMAL_GOOS] >= 2) {
					if (player.stats.cor < 50 && (player.hasItem(consumables.SUCMILK) || player.hasItem(consumables.P_S_MLK)) && (player.hasItem(consumables.BLACKEG) || player.hasItem(consumables.L_BLKEG))) {
						kGAMECLASS.latexGirl.pureGooRecruitmentStart();
						return;
					}
					else if (flags[FlagEnum.PC_KNOWS_ABOUT_BLACK_EGGS] > 0) {
						//Recruitment:
						//Notice After Victory:
						if (flags[FlagEnum.TIMES_THOUGHT_ABOUT_GOO_RECRUITMENT] == 0) {
							...
							flags[FlagEnum.TIMES_THOUGHT_ABOUT_GOO_RECRUITMENT]++;
						}
						//(Repeat)
						else {
						...
						if ((player.hasItem(consumables.SUCMILK) || player.hasItem(consumables.P_S_MLK)) && (player.hasItem(consumables.BLACKEG) || player.hasItem(consumables.L_BLKEG))) {
							MainScreen.text("  Good thing you have those handy!");
				...
				if (valeria != null) {
					...
					if (flags[FlagEnum.TIMES_VALERIA_GOO_THREESOMED] == 0) MainScreen.text("Do you offer a threesome with the girl to Valeria? It could get a little weird....");
					else MainScreen.text("Do you offer a threesome with the girl to Valeria? She'll likely try flood with you with more sloshing, shuddering pleasure than your body can handle.");
				}
