...
private function letKaijuHaveWayWithYou():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", [eachCock] stiffening in your [armor] from the stimulation as your whole body is used in what seems more and more like a massive boob job");
	outputText(".");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your [vagina] moistens from your body's growing arousal.");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		//[if PC has balls]
		if(player.lowerBody.balls > 0) outputText("  Your [balls] begin to ache as you slowly empty your load onto green flesh.");
		player.orgasm();
	}
...
private function flirtWithKaiju():void {
	...
	if(player.stats.sens < 50) {
		...
		//[If PC has cock]
		if(player.lowerBody.cockSpot.hasCock()) outputText("  [EachCock] spasms as you release your load onto her pussy walls, making them just a bit more slick with your contribution.");
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your own pussy releases its own feminine juices, soaking your thighs even further in wetness.");
	...
	if(kaijuCock()) {
		...
		if(player.gender > 0) {
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("  You press your [chest] against her cock flesh as well, giving her penis as much a boob job as such a behemoth prick can be given.");
		...
		if(player.gender > 0) {
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) outputText("feminine juices");
			if(player.gender == 3) outputText(" and ");
			if(player.lowerBody.cockSpot.hasCock()) outputText("thick cum");
...
private function fuckThisGiantYouDumbCunt():void {
	...
	if(player.stats.sens >= 50) {
		...
		if(kaijuCock()) {
			...
			//[If PC has boobs]
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("You press your [fullChest] against her cock flesh as well, giving her penis as much a boob job as such a behemoth prick can be given.");
			if(player.lactationQ() >= 200) outputText("  Milk spurts and flows from your [nipples], making your breast slick and shiny and helping you slide more easily along the throbbing meat-pole.");
...
private function urethraFuckDatGiantCock():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" and your " + multiCockDescriptLight());
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  So much jizz fills your ass it can’t help but to press in on your prostate, making [eachCock] stiffen with a seriously growing urge.");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your ass isn’t alone in the feeling of being flooded as your [vagina] and womb are packed with the hermaphrodite’s sperm.");
...
private function talkToKaiju():void {
	...
	//Dialog two: 
		...
		if(player.statusAffects.has("DungeonShutDown") && player.findStatusAffect(StatusAffects.FactoryOverload) < 0) outputText("  But the flow of yummy fluids seems to have been stopped.");
...
private function mockDatTurtleGirl():void {
	...
	if(player.lowerBody.balls > 0 && player.lowerBody.ballSize >= 6) outputText("Your [balls] rest on the hermaphrodite's hand, bouncing loudly with each thunderous stroke from the speed and force she is employing to get herself off.  ");
	//[end balls] [If player has cock]
	if(player.lowerBody.cockSpot.hasCock()) outputText("You can feel your [armor] getting tighter as [eachCock] begins to stiffen under your clothing from all the constant pressure of the cock squeezing down on your member.  ");
	//[If player has vagina]
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("You begin to feel a wetness between your legs as your cunt begins leaking as your arousal spikes from the intense rubbing the herm's penis is giving your whole body.  ");
	//[If genderless]
	if(player.gender == 0) outputText("Your cheeks blush in arousal from all this heavy rubbing.  ");
	if(player.stats.cor < 33) outputText("You can't believe that you're being aroused by this.  Aren't you suppose to be a champion?");
	else if(player.stats.cor < 66) outputText("Wow, this is arousing you.  You wonder if you really are a champion.");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" as you cum in your [armor]");
...
private function helpNewFutaKaijuGetOff():void {
	...
	//[If PC has cock]
	if(player.lowerBody.cockSpot.hasCock()) outputText("  [EachCock] quickly hardens now as it is rubbed up against the giant's massive shaft.");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  The warm heat that radiates off of the titanic member seems to seep into your nether region, making your [vagina] leak out moisture onto the massive prick.");
	...
	if(player.stats.cor < 50 && player.stats.lib < 75) outputText("  You knead the cock head with your hands, massaging it quickly and roughly as you grind up against it, rock back and forth on the giant member.  Some of the pre dribbles down and you begin to use it as sticky massage oil, working it into the cock flesh.  The cooing and lustful moaning coming from the giantess increases in intensity with your every movement.");
...
private function badEndPartTwo():void {
	...
	if(player.tongueType == TONUGE.SNAKE) outputText("  Your snaky forked tongue begins lashing about, flicking at the nipple tip in your mouth while rubbing at the underside of it, urging the tender spot to give up a drop of the heavenly white liquid.");
	...
	if(kaijuCock()) {
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("  You prostate is pressed down like a button, and there’s just no letting up!");
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Even if your cunt is empty, there is no free space left there, your cunt walls forced together like glue.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("  Another tendril snakes its way up your abdomen to nestle between your [chest], slicking your mounds with pre as it slides between your tits, giving itself a titty-fuck with your titties!");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  More tentacles arrive and begin to wrap around your " + multiCockDescriptLight() + ", slowly jacking you off to the tempo set by the cocks in your ass and mouth, wracking you with further pleasure.");
