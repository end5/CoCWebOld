export default class Kaiju extends AbstractLakeContent {
	...
	private meetDatKaijuYo(): void {
		...
		if (player.statusAffects.has("FactoryOverload")) Flags.list[FlagEnum.KAIJU_COCK] = 1;
	...
	private letKaijuHaveWayWithYou(): void {
		...
		if (player.gender == 3) MainScreen.text(CockDescriptor.describeMultiCockShort(player) + " and [vagina]");
		else if (player.gender == 1) MainScreen.text(CockDescriptor.describeMultiCockShort(player));
	...
	private urethraFuckDatGiantCock(): void {
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" and your " + CockDescriptor.describeMultiCockShort(player));
	...
	private talkToKaiju(): void {
		...
		//Dialog two: 
			...
			if (player.statusAffects.has("DungeonShutDown") && !player.statusAffects.has("FactoryOverload")) MainScreen.text("  But the flow of yummy fluids seems to have been stopped.");
	...
	private badEndPartTwo(): void {
		...
		if (player.upperBody.head.face.tongueType == TongueType.SNAKE) MainScreen.text("  Your snaky forked tongue begins lashing about, flicking at the nipple tip in your mouth while rubbing at the underside of it, urging the tender spot to give up a drop of the heavenly white liquid.");
		//[If PC has demonic/dragon tongue]
		else if (player.upperBody.head.face.tongueType > TongueType.HUMAN) {
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  More tentacles arrive and begin to wrap around your " + CockDescriptor.describeMultiCockShort(player) + ", slowly jacking you off to the tempo set by the cocks in your ass and mouth, wracking you with further pleasure.");
