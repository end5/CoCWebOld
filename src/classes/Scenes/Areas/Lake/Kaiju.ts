...
public function kaijuMeeting():void {
	...
	if(flags[FlagEnum.KAIJU_MEETINGS] == 0) {
		Render.text("Your explorations take you to a small island you haven't seen before.  It appears to be a large, smooth rock hill jutting out of the water.  Do you explore it?");
		//[Yes/No]
		menu();
...
private function noMeetingKaijus():void {
	clearOutput();
	Render.text("You continue rowing on, away from the hilly island.");
...
private function insultTheKaijuFirstMeeting():void {
	...
	if(kaijuCock()) {
		...
		Render.text("In perhaps not your brightest idea, you decide to make fun of the giant green girl in front of you, mocking her obvious insecurity over her figure.  The word 'fat' barely exits your mouth before her face goes red in anger and you realize your mistake.");
...
private function corruptKaijuInsertion():void {
	clearOutput();
	spriteSelect(103);
	Render.text("She practically shoves your face into her dark green cock-head, your nose snug against the long slit.  It smells of the sexual corruption of the lake, of demonic jizz and tainted aphrodisiacs.  \"<i>Well go on,</i>\" she says, \"<i>lick it!</i>\"");
...
private function complimentKaiju():void {
	clearOutput();
	spriteSelect(103);
	Render.text("You decide to compliment her figure, stating it is proportional and very pleasing.");
...
private function stopItPlease():void {
	clearOutput();
	spriteSelect(103);
	Render.text("You cry out and wave the inquisitive hand away.  You tell her that you mean no offense, but at such a size and strength disparity you are afraid of what a grip from such a woman could do accidentally.  She seems a bit saddened at that, but makes no further attempt to grab you.");
...
private function letKaijuHaveWayWithYou():void {
	...
	if(player.gender == 3) Render.text(multiCockDescriptLight() + " and [vagina]");
	else if(player.gender == 1) Render.text(multiCockDescriptLight());
	else if(player.gender == 2) Render.text("[vagina]");
	else Render.text("bare groin");
	Render.text(".  \"<i>Let's have some fun!</i>\"");
...
private function flirtWithKaiju():void {
	clearOutput();
	spriteSelect(103);
	Render.text("You decide to try and flirt up the giantess.  You tell her that her figure is perfect and statuesque, and that her curves are quite magnificent to behold.  It seems to work if her spreading look of pleasure is any indication.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) {
		Render.text("  Your [fullChest] slide across the slick yet firm vaginal flesh of her surprisingly dexterous cunt, your [nipples] stiffening from the constant teasing massage.");
	}
	//[If PC has cock]
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("  [EachCock] ");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("are");
		else Render.text("is");
		Render.text(" throbbing from the sexual stimulants of her aphrodisiac moisture and the pressing walls of her cunt flesh, pre forming at your tip");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(".");
	}
	...
	if(player.stats.sens < 50) {
		...
		//[If PC has cock]
		if(player.lowerBody.cockSpot.hasCock()) Render.text("  [EachCock] spasms as you release your load onto her pussy walls, making them just a bit more slick with your contribution.");
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Your own pussy releases its own feminine juices, soaking your thighs even further in wetness.");
	...
	if(kaijuCock()) {
		...
		if(player.gender > 0) {
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) Render.text("  You press your [chest] against her cock flesh as well, giving her penis as much a boob job as such a behemoth prick can be given.");
			if(player.lactationQ() >= 200) Render.text("  Milk spurts and flows from your [nipples], making your breast slick and shiny and helping you slide more easily along the throbbing meat-pole.");
		}
		...
		if(player.gender > 0) {
			...
			Render.text("  Undeterred, you continue grinding upon the spurting pole until you reach your own smaller, far less impressive orgasm, releasing your own ");
			if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("feminine juices");
			if(player.gender == 3) Render.text(" and ");
			if(player.lowerBody.cockSpot.hasCock()) Render.text("thick cum");
			Render.text(".  You look out upon the lake to see much of it has taken on a remarkably more milky white color.  Even for a giant that was a prodigious amount of cum she unleashed!");
...
private function kaijuRepeatBoobHug():void {
	clearOutput();
	spriteSelect(103);
	Render.text("You decide it would be fun to play with her titanic titties for a while.");
	...
	if(player.gender == 0) Render.text("bare groin");
	Render.text(".  \"<i>Let's have some fun!</i>\"");
...
private function fuckThisGiantYouDumbCunt():void {
	clearOutput();
	spriteSelect(103);
	Render.text("You decide it’d be fun to fuck the giantess, even if at her size you’re really more of a living dildo than a rutting stud.  She seems pretty happy with the thought if her spreading look of pleasure is any indication.");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("\n\nYour hard cock");
		if(player.totalCocks() > 1) Render.text("s are");
		else Render.text(" is");
		Render.text(" throbbing from the sexual stimulants of her aphrodisiac moisture and the pressing walls of her cunt flesh, pre forming at your tip");
	...
	if(player.stats.sens >= 50) {
		...
		if(kaijuCock()) {
			...
			//[If vagina and/or cock is present]
			if(player.gender > 0) {
				Render.text("You grind your privates against her member, determined to get yourself off as well.  ");
			}
			//[If PC has boobs]
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) Render.text("You press your [fullChest] against her cock flesh as well, giving her penis as much a boob job as such a behemoth prick can be given.");
			if(player.lactationQ() >= 200) Render.text("  Milk spurts and flows from your [nipples], making your breast slick and shiny and helping you slide more easily along the throbbing meat-pole.");
...
private function urethraFuckDatGiantCock():void {
	clearOutput();
	spriteSelect(103);
	Render.text("You look at her with an inquisitive stare.  You’re interested in that big green cock of hers, but how can you use such a giant tool?");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text(" and your " + multiCockDescriptLight());
	Render.text(".  \"<i>Wow, you look absolutely gorgeous in my cock!</i>\" the giantess says with glee, perverse desire in her eyes as she shoves you in further up to your [chest], her trembling, squeezing cock seeming to give you an almost relaxing massage.  You can barely believe it, but this unnatural giantess has as much control over her urethral walls as the most whorish succubus has over her cunt!  Only your arms and head are free of her cock as her unusual member starts squeezing and rippling all along your body.  It seems her dick wants to give you a happy ending.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  So much jizz fills your ass it can’t help but to press in on your prostate, making [eachCock] stiffen with a seriously growing urge.");
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Your ass isn’t alone in the feeling of being flooded as your [vagina] and womb are packed with the hermaphrodite’s sperm.");
	Render.text("  You manage to reach your own orgasmic climax right before the pressure finally pops you out of her cock like a cork from a wine bottle.  You black out somewhere above the lake.");
...
private function leaveRepeatKaiju():void {
	clearOutput();
	spriteSelect(103);
	Render.text("You politely decline any options and bid the green girl goodbye as you row away.");
	doNext(camp.returnToCampUseOneHour);
}
...
private function talkToKaiju():void {
	...
	flags[FlagEnum.KAIJU_BAD_END_COUNTER]--;
	Render.text("\"<i>Oh?  You want to talk?</i>\" she says, a bit taken by surprise, \"<i>What about?</i>\"");
...
private function peekAtSomePhatAssKaijuButt():void {
	...
	Render.text(".  She pumps her clenched fist furiously as deep into herself as she can reach, going faster and faster as she quickly reaches orgasm, her pussy juices squirting out around her fist, soaking her forearm in the feminine fluid.  She slowly pulls her hand out with an audible 'Plop' and brings both hands back to her breasts, massaging her giant sized fuck pillows until two thick steady streams of milk begin flowing out her nipples and down her breasts.");
	//[Lust is increased]
...
private function noKaijuPeepShows():void {
	clearOutput();
	spriteSelect(103);
	Render.text("\"<i>My goodness, it's so much better with a captivated audience!</i>\" she says, breathing heavily.  You thank her for the show as she places you back into your boat and giving it a push.  You row away, considering perhaps coming back for another show.");
...
private function dontGetFutaTurtlesOffToday():void {
	clearOutput();
	spriteSelect(103);
	Render.text("You shake your head no, politely responding that you do not want to get her off right now.  \"<i>Oh, okay. I understand,</i>\" the giantess states, looking somewhat crestfallen.  You row away, leaving her and her new addition to themselves for the time being.");
	doNext(camp.returnToCampUseOneHour);
}
...
private function mockDatTurtleGirl():void {
	...
	if(player.lowerBody.balls > 0 && player.lowerBody.ballSize >= 6) Render.text("Your [balls] rest on the hermaphrodite's hand, bouncing loudly with each thunderous stroke from the speed and force she is employing to get herself off.  ");
	//[end balls] [If player has cock]
	if(player.lowerBody.cockSpot.hasCock()) Render.text("You can feel your [armor] getting tighter as [eachCock] begins to stiffen under your clothing from all the constant pressure of the cock squeezing down on your member.  ");
	//[If player has vagina]
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("You begin to feel a wetness between your legs as your cunt begins leaking as your arousal spikes from the intense rubbing the herm's penis is giving your whole body.  ");
	//[If genderless]
	if(player.gender == 0) Render.text("Your cheeks blush in arousal from all this heavy rubbing.  ");
	if(player.stats.cor < 33) Render.text("You can't believe that you're being aroused by this.  Aren't you suppose to be a champion?");
	else if(player.stats.cor < 66) Render.text("Wow, this is arousing you.  You wonder if you really are a champion.");
	else Render.text("You just want to get off now.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text(" as you cum in your [armor]");
	Render.text(".  You can feel the spasming of the giant's cock as you feel the pressure from her shaft increase as a load of cum rushes through it, erupting out of her tip like a volcano with globs of white lava.  \"<i>Well, that's a hell of a load,</i>\" she says, seeming quite proud of herself.  She sets you back down in your boat and roughly shoves it out further into the lake.  \"<i>Learn some manners you meanie!</i>\" she calls out.");
...
private function helpNewFutaKaijuGetOff():void {
	...
	//[If PC has cock]
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  [EachCock] quickly hardens now as it is rubbed up against the giant's massive shaft.");
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  The warm heat that radiates off of the titanic member seems to seep into your nether region, making your [vagina] leak out moisture onto the massive prick.");
	Render.text("  She squirms under your touch, falling on her back and wriggling almost uncontrollably.  You're quite surprised at the power you hold over such a behemoth.");
...
private function yesTurnKaijuFuta():void {
	...
	Render.text("You wouldn’t have offered if you didn’t want to see a giant green futa-cock dangling between her legs.  Your declaration seems to have deepened her blush to almost beet red, but the lustful look in her eyes betrays her desire.");
...
private function flyAwayFromBadEnd():void {
	clearOutput();
	spriteSelect(103);
	Render.text("Flapping your wings at max speed you beat a hasty retreat!");
	doNext(camp.returnToCampUseOneHour);
}
...
private function badEndPartTwo():void {
	...
	if(kaijuCock()) {
		...
		if(player.lowerBody.cockSpot.hasCock()) Render.text("  You prostate is pressed down like a button, and there’s just no letting up!");
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Even if your cunt is empty, there is no free space left there, your cunt walls forced together like glue.");
	...
	Render.text("  Inches slide in at a time as it spasms inside of you, attempting to hit every sensitive spot your ass may have as it slowly tries to stretch you out.\n\n");
	
	//[If PC has a cock] 
	if(player.lowerBody.cockSpot.hasCock()) Render.text("[EachCock] stiffens as the tree tendril pushes against your prostate, forcing a thick glob of pre up your shaft.  ");
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("As the first vine-cock works its way up your rectum a second has found its way up to your snatch!  The plant-like head rubs against your folds almost lovingly, tenderly, before the head starts to prod past your pink lips, butting up against your cunt as part of the shaft rears up to rub against your clit, sending shockwaves through your body, and the vine-cock chooses this moment to plunge into your [vagina].  Now you have two cocks ramming into your holes, pressing up against your walls as if to feel each other through you.  The plant dick in your pussy slides up and down through your folds, each thrust out going ever so slowly, only to ram back in at full speed, more inches sliding in every time until the head is bumping up against your cervix, leaky pre-seed slowly filling up your cunt to leak out between your thighs.  ");
	Render.text("  The next sinuous cock threads itself between your legs, up your belly, past your chest to lightly poke at your lips.  You open your mouth to shout when the cock in your [asshole] hits a sensitive spot and the cock at your mouth slides right in, dripping seed into your mouth, your tongue suddenly awash in a salty, yet pleasantly sweet flavor.");
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) Render.text("  Another tendril snakes its way up your abdomen to nestle between your [chest], slicking your mounds with pre as it slides between your tits, giving itself a titty-fuck with your titties!");
