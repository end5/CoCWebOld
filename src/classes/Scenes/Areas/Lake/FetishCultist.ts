	...
	public class FetishCultist extends Monster
	{
		...
		private function cultistLustTransfer():void {
			if(lust <= 30 || rand(2) == 0) {
				...
				if(player.stats.cor < 40) outputText("and you're almost afraid to know ", false);
				...
				if(player.lowerBody.vaginaSpot.count() > 0) {
					...
					if(player.lowerBody.cockSpot.count() > 0)  outputText(", while a sudden influx of pre-cum blurts out and streams down your " + player.multiCockDescriptLight() + ", painfully hardened by a vast amount of blood rushing to your groin", false);
					outputText(".", false);
				}
				else if(player.lowerBody.cockSpot.count() > 0) outputText("  A sudden influx of pre-cum blurts out and streams down your " + player.multiCockDescriptLight() + ", painfully hardened by a vast amount of blood rushing to your groin.", false);
