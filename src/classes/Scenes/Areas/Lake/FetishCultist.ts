export default class FetishCultist extends Monster {
	...
	private cultistLustTransfer(): void {
		if (lust <= 30 || rand(2) == 0) {
			...
			if (player.lowerBody.vaginaSpot.count() > 0) {
				MainScreen.text("  You feel your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " soaking itself in a sudden burst", false);
				if (player.lowerBody.cockSpot.count() > 0) MainScreen.text(", while a sudden influx of pre-cum blurts out and streams down your " + player.CockDescriptor.describeMultiCockShort(player) + ", painfully hardened by a vast amount of blood rushing to your groin", false);
				MainScreen.text(".", false);
			}
			else if (player.lowerBody.cockSpot.count() > 0) MainScreen.text("  A sudden influx of pre-cum blurts out and streams down your " + player.CockDescriptor.describeMultiCockShort(player) + ", painfully hardened by a vast amount of blood rushing to your groin.", false);
