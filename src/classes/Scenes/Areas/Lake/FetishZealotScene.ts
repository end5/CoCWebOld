		...
		public function zealotLossRape():void
		{
			...
			if (broseph == "dude") {
				...
				if (player.gender != 0) {
					...
					if (player.lowerBody.vaginaSpot.hasVagina()) outputText("You slip one of your fingers inside your " + vaginaDescript(0) + ", wishing that it could get some attention, but this lesson is about pleasing <i>men</i>.  ", false);
...
//Raped by the player
		private function zealotWinRape():void
		{
			...
			//If player has a vagina, again, if both, chose one at random
				...
				if (player.vaginas[0].vaginalLooseness < VAGINA_LOOSENESS.LOOSE) outputText("tight", false);
