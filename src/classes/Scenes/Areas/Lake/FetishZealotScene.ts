...
//Raping the player
		public function zealotLossRape():void
		{
			...
			if (player.HP < 1)
				Render.text("You collapse from the pain of the zealot's attacks.  You feel your head swimming from the aphrodisiac; it seems to be having a stronger and stronger effect on you.  Soon your head is swimming with images of fetish scenes of all kinds.  The zealot walks up to you and puts his hand on your forehead.  Suddenly, all the images cascade into one.\n\n", false);
			//Pre Rape Scene – lose by lust
			else
				Render.text("The constant images of fetishes overwhelm you and you fall to your knees, unable to resist the temptation. The zealot walks up to you and puts his hand on your forehead.  Suddenly all the images cascade into one.\n\n", false);
			...
			Render.text("You stand up facing the chalkboard in your classroom.  For some reason you think you should be bothered by this, but you're in your classroom just after class.  Nothing seems to be out of place or missing.  You turn back to the rest of the classroom and notice that everyone but one boy seems to have left already.  You recognize that he is the cute nervous boy with the rich parents.  You shudder at the thought of his parents; last week they got a teacher fired for yelling at him to speak up.  ", false);
			...
			if (broseph == "dude") {
				...
				Render.text("You smooth down your detached pants, and look at your exposed dick for a few moments, wondering if there was anything you said that may have upset him.  ", false);
			}
			//no its a girl, or it looks like one
				...
				Render.text("You smooth out your half-skirt, trying to busy yourself as you try to remember if there was anything you said to upset him.  ", false);
			}
			Render.text("\"<i>Professor?</i>\"  You hear him call out to you.  You look up and see him standing in front of your desk looking at the ground nervously.  \"<i>Yes, what is it?</i>\" you reply.  ", false);
			//same as before
			if (broseph == "dude") {
				Render.text("He starts to look up, but then his gazes fixes directly on your ", false);
				if (player.lowerBody.cockSpot.count() > 0) {
					Render.text(multiCockDescriptLight() + ".  You feel yourself grow hard ", false);
					if (player.lowerBody.vaginaSpot.hasVagina()) {
						Render.text("and wet ", false);
					}
					Render.text("under his gaze, ", false);
				}
				else Render.text("bare crotch, ", false);
				Render.text("and you silently wish that the dress code allowed for a small flap for covering your ", false);
				if (player.gender == 0) {
					Render.text("lack of ", false);
				}
				else Render.text("genitals.  ", false);
			}
			else {
				Render.text("He starts to look up, but his gazes fixes on your skirt, just barely covering your ", false);
				if (player.lowerBody.vaginaSpot.hasVagina()) {
					...
					Render.text("bare crotch ", false);
				}
				Render.text("from this angle.  You feel hot under his gaze ", false);
				if (player.lowerBody.cockSpot.count() > 0) {
					Render.text(", and your " + multiCockDescriptLight() + " start", false);
					if (player.lowerBody.cockSpot.count() == 1) {
						Render.text("s", false);
					}
					Render.text(" to get hard, as your ", false);
				}
				Render.text("cleft starts to moisten.  ", false);
			}
			Render.text("You clear your throat nervously and he starts, finally raising his eyes to your face.\n\n", false);
			Render.text("\"<i>Ah, professor,</i>\" he says again, \"<i>I just got a " + player.mf("boyfriend", "girlfriend") + " and I have no idea how to have sex.  Could you please give me a lesson?</i>\"  You stare at him in disbelief.  This boy still hadn't had a " + player.mf("man", "woman") + " at this school?  It would almost be laughable, if the threat of his parents didn't loom in the back of your mind.\n\n", false);
			Render.text("\"<i>Ok,</i>\"  you say nervously, \"<i>I can give you a lesson on pleasing " + player.mf("men", "women") + ".</i>\"  He jumps up excitedly and runs around to the other side of the desk saying \"<i>Ok, what do I have to do?</i>\"  Feeling a bit put on the spot, you stop for a moment to take in the situation.  ", false);
			//again, is that a guy?  Or just look like one?
			if (broseph == "dude") {
				Render.text("You're standing in front of your student behind your desk in your empty classroom.  You are in regular teacher's attire: an open vest and tie over your otherwise bare " + player.allBreastsDescript() + ", with formal dress pant legs attached to your vest with suspenders, plus your black dress shoes for dancing.  Your student is wearing the standard male student uniform, a collection of pieces of material held together with straps, that will all fall away easily when one gets pulled; but otherwise gives full coverage.\n\n", false);
				...
				if (player.gender != 0) {
					...
					Render.text("Throughout the whole conversation, your student gets more and more excited before finally saying \"<i>Can we do it now?!</i>\" and pulling his waist strings, causing his crotch piece to fall open.  At the site of his impressive erection, you eagerly kneel down in front of him as he says \"<i>I want you, I want you to suck my cock <b>so</b> much!</i>\"  You waste no time in obliging him.\n\n", false);
					//blowjob
					Render.text("You take a hold of his nicely sized balls in one hand and the base of his cock in the other.  You give the tip a few licks before running your tongue down its length,  making sure to coat every bit of it in your saliva along the way.  His aroused groans let you know that you are giving a good demonstration.  You give his balls a soft rub as you stick the end inside your mouth and run your tongue around it; the taste of his pre is almost like candy to you.\n\n", false);
					//cunalinguss
					Render.text("You stand up and indicate that it is now his turn, and while he is disappointed that the blowjob ended before he came, he still eagerly kneels down in front of your " + vaginaDescript(0) + ".  You gingerly lift up your skirt ", false);
					if (player.lowerBody.balls > 0) {
						Render.text("and move your " + ballsDescriptLight() + " out of the way ", false);
					}
					Render.text("so he has access to your " + vaginaDescript(0) + ".  He sets to work, and you give him careful instructions on where to lick, how hard, and such.  He easily catches on, and soon he is probing all the right places, and giving the right amount of attention to every part of you.  ", false);
					if (player.lowerBody.cockSpot.count() > 0) {
						Render.text("You give your " + multiCockDescriptLight() + " a few gentle strokes, wishing he would give that a little attention too, but this is a lesson on how to please a <i>woman</i>.  ", false);
					}
					Render.text("You feel yourself getting close to the edge and tell him to stop.  He looks up at you uncertain for a moment, and you tell him \"<i>It's time for the main event.</i>\"\n\n", false);
					//prick in the twat
					Render.text("He eagerly stands up and starts to push you back against your desk, his fine cock already inside your " + vaginaDescript(0) + ".  He pushes you onto your back and grabs ahold of your legs, starting to make strong thrusts deep inside you, his cock seeming to fit your " + vaginaDescript(0) + " perfectly.  Any pretense of professionalism is lost in the moment, as all you can care about is the feeling of his exquisite cock perfectly filling you, pushing in and out.  All too soon, you hear him gasp, and the wonderful feeling of sweet release fills you.  ", false);
					if (player.lowerBody.cockSpot.count() > 0) {
						Render.text("Your " + multiCockDescriptLight() + " spasms and covers you with a liberal amount of your own fluids.  ", false);
					}
					Render.text("After a moment, your student pulls out of you and helps you back up.\n\n", false);
				}
			}
			//Wrapping things up
			if (player.gender != 0) {
				Render.text("The two of you stand looking at each other again for a moment.  Then you help your student get his clothes back together.  Once you have him fixed up, you notice that he is looking at the ground again.  \"<i>Professor?  I have a confession to make.</i>\" he says very quietly.  \"<i>What is it?</i>\" you ask him.  \"<i>I lied to you about getting a " + player.mf("boyfriend", "girlfriend") + ", I actually just wanted you.</i>\"  Before you have a chance to respond, he quickly gives you a kiss on the mouth, before rushing out the door, blushing furiously.  You can't help but smile at how he is growing up.\n\n", false);
			}
			if (player.gender == 0) {
				Render.text("Throughout the whole conversation, your student gets more and more excited.  Half way though he starts masturbating, and by the end he is ready to go over the edge.  \"<i>Thank you so much for the lesson professor, gaahh.</i>\" he tells you as he begins to cum. The sight of him cumming in front of you is enough to drive you to your own orgasm.  Breathing a sigh of relief, you let your student out of the room and go back to cleaning up.\n\n", false);
			}
			...
			if (player.stats.int < 10) {
				Render.text("You find that your mind is unable to return to reality, and it moves on to another, then another.  Later you feel a female body come and pick you up, but you are too messed up to react to it...", false);
			...
			//continue on to Cultist bad end.  I'll expand these later when where doing the cathedral.
				...
				Render.text("A few hours later your mind finally returns to reality.  You look around and realize that you are no longer in the same place you were before, and the zealot is nowhere to be seen.", false);
			}
		...
		public function zealotDefeated():void
		{
			...
			//Defeated by lust
			else Render.text("The zealot quivers for a moment before collapsing, his desires becoming too great for even him to control.", false);
...
//Raped by the player
		private function zealotWinRape():void
		{
			...
			Render.text("The zealot's attire seems to have settled on an outfit similar to those commonly worn by members of religious orders, though you aren't too surprised to see that it has a slit running down the front and back of the outfit that gives you full access to his sizable cock and asshole.\n\n", false);
