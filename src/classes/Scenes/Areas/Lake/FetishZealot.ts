package classes.Scenes.Areas.Lake
{
	...
	public class FetishZealot extends Monster
	{
		...
		public combatRoundUpdate():void
		{
			...
			//Talk abouts it mang!
			if(changed) Render.text("The fetish zealot's clothing shifts and twists, until he is wearing " + armorName + ".\n\n", false);
			lust += lustVuln * 5;
		}
...
//Special1: Tease
	...
	private function zealotSpecial1():void {
		...
		if(armorName == RELIGIOUS_CLOTHES) {
			...
			Render.text("The zealot cries out \"<i>Child, are you ready to present your offering to the holy rod?</i>\" while indicating his cock sliding between his robes.  The whole scene leaves you distracted for a few moments and significantly aroused.", false);
		}	
		//A pirate costume; 
		if(armorName == PIRATE_CLOTHES) {
			...
			Render.text("The zealot turns to the side holding his prosthetic towards you and doing something that sends the devices spinning and clicking.  <i>So that's how that would work...<i> you find yourself thinking for a few moments before realizing that he had both distracted and aroused you.", false);
		}
		//Military attire;
		if(armorName == MILITARY_CLOTHES) {
			...
			Render.text("He suddenly barks, \"<i>Let's see those genitals, soldier!</i>\" ", false);
			//[player is corrupt] 
			if(player.stats.cor > 50) Render.text("You eagerly cry out \"<i>Yes, sir!</i>\" and show yourself off to the best of your ability.  The whole act is extremely arousing.", false);
			//[player is not corrupt] 
			else Render.text("You have no idea why, but you promptly display yourself in the most provocative way possible.  After a moment you realize what you're doing and quickly stop, flushed with embarrassment and arousal.", false);
		}
		//Gimp gear;
		if(armorName == LEATHER_CLOTHES) {
			...
			Render.text("The Zealot turns around and gives you a full view of his tight leather clad body.  He smacks his ass and says \"<i>You like what you see, don't you " + player.mf("stud","slut") + "?</i>\"  You can't help but be incredibly aroused by the scene.", false);
		}
		//Well dressed and well groomed student in uniform;
		if(armorName == STUDENTS_CLOTHES) {
			//The Zealot seems to have taken on the appearance of a young adult wearing a student uniform of sorts; of course, this isn't any less perverted than any of the other costumes this man wears.  This one includes a number of loose straps that you're certain would cause large sections of his clothes to fall off if somebody pulled on them.
			Render.text("The Zealot student looks at you a little shyly and sticks a pencil in his mouth while pushing a hand in front of his groin, trying to hide a rather obvious bulge.  The whole scene is rather cute, and you feel incredibly aroused afterwards.", false);
		}
