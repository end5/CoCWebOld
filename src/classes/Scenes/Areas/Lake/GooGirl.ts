		...
		private gooGalAttack():void
		{
			...
			if (damage <= 0) {
				...
				if (findPerk(PerkLib.Acid) >= 0) {
					if (rand(player.armorDef + player.tou) < player.armorDef) MainScreen.text("tries to slap you, but the acid-bearing slap spatters weakly off your " + player.armorName + ".", false);
					else MainScreen.text("tries to slap you with an acid-loaded hand, but it splatters off you ineffectually.", false);
				}
				else {
					//Due to toughness or amor...
					if (rand(player.armorDef + player.tou) < player.armorDef) MainScreen.text(", her attack slapping fruitlessly against your " + player.armorName + ".", false);
			...
			//everyone else
				...
				if (findPerk(PerkLib.Acid) >= 0) {
					...
					if (player.findStatusAffect(StatusAffects.AcidSlap) < 0) player.statusAffects.add(new StatusAffect("AcidSlap", 0, 0, 0, 0)));
				}
				else MainScreen.text(", painfully smacking her gooey limbs against your head.  You shake your " + player.hairDescript() + ", clearing your head of the dazing slap.", false);
...
//Throw – 
		...
		{
			MainScreen.text("The girl reaches into her torso, pulls a large clump of goo out, and chucks it at you like a child throwing mud. The slime splatters on your chest and creeps under your " + player.armorName + ", tickling your skin like fingers dancing across your body.", false);
			let damage:number = 1;
...
//Engulf – 
		private gooEngulph():void
		{
			...
			if (player.findStatusAffect(StatusAffects.GooBind) < 0) player.statusAffects.add(new StatusAffect("GooBind", 0, 0, 0, 0)));
			combatRoundOver();
		}
