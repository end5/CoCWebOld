...
//Acid Slap (Only after player's fire attack) – Her body quivering from your flames, the goo-girl delivers a painful slap across your cheek. You gasp when the light stinging becomes a searing burn that seems to get worse as time goes on! (heavily damages hit points and puts Acid Burn on the player)
		private gooGalAttack():void
		{
			...
			if (findPerk(PerkLib.Acid) >= 0) Render.text("Her body quivering from your flames, the goo-girl ", false);
			else Render.text("The slime holds its hands up and they morph into a replica of your " + player.weaponName + ".  Happily, she swings at you", false);
			//Determine if dodged!
			if (player.stats.spe - spe > 0 && int(Math.random() * (((player.stats.spe - spe) / 4) + 80)) > 80) {
				if (findPerk(PerkLib.Acid) >= 0) Render.text("tries to slap you, but you dodge her attack.", false);
				else Render.text(", missing as you dodge aside.", false);
				return;
			}
			//Determine if evaded
			if (short != "Kiha" && player.perks.has("Evade") && rand(100) < 10) {
				if (findPerk(PerkLib.Acid) >= 0) Render.text("tries to slap you, but you evade her attack.", false);
				else Render.text(", but you evade the clumsy attack.", false);
				return;
			}
			//("Misdirection"
			if (player.perks.has("Misdirection") && rand(100) < 10 && player.armorName == "red, high-society bodysuit") {
				if (findPerk(PerkLib.Acid) >= 0) Render.text("tries to slap you.  You misdirect her, avoiding the hit.", false);
				else Render.text(", missing as you misdirect her attentions.", false);
				return;
			}
			//Determine if cat'ed
			if (player.perks.has("Flexibility") && rand(100) < 6) {
				if (findPerk(PerkLib.Acid) >= 0) Render.text("tries to slap you, but misses due to your cat-like evasion.", false);
				else Render.text(", missing due to your cat-like evasion.", false);
				return;
			}
			...
			if (damage <= 0) {
				...
				if (findPerk(PerkLib.Acid) >= 0) {
					if (rand(player.armorDef + player.tou) < player.armorDef) Render.text("tries to slap you, but the acid-bearing slap spatters weakly off your " + player.armorName + ".", false);
					else Render.text("tries to slap you with an acid-loaded hand, but it splatters off you ineffectually.", false);
				}
				else {
					//Due to toughness or amor...
					if (rand(player.armorDef + player.tou) < player.armorDef) Render.text(", her attack slapping fruitlessly against your " + player.armorName + ".", false);
					else Render.text(", her attack splattering ineffectually against you.", false);
				}
			}
			//everyone else
			else {
				if (findPerk(PerkLib.Acid) >= 0) {
					Render.text("delivers a painful slap across your cheek.  You gasp when the light stinging becomes a searing burn that seems to get worse as time goes on!", false);
					if (player.findStatusAffect(StatusAffects.AcidSlap) < 0) player.statusAffects.add(new StatusAffect("AcidSlap", 0, 0, 0, 0)));
				}
				else Render.text(", painfully smacking her gooey limbs against your head.  You shake your " + player.hairDescript() + ", clearing your head of the dazing slap.", false);
...
//Throw – 
		...
		{
			Render.text("The girl reaches into her torso, pulls a large clump of goo out, and chucks it at you like a child throwing mud. The slime splatters on your chest and creeps under your " + player.armorName + ", tickling your skin like fingers dancing across your body.", false);
			let damage:number = 1;
...
//Engulf – 
		...
		{
			Render.text("The goo-girl gleefully throws her entire body at you and, before you can get out of the way, she has engulfed you in her oozing form! Tendrils of " + skinTone + " slime slide up your nostrils and through your lips, filling your lungs with the girl's muck. You begin suffocating!", false);
