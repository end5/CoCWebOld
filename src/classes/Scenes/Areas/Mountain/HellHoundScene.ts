...
package classes.Scenes.Areas.Mountain
{
	...
	public class HellHoundScene extends BaseContent
	{
		...
		public function hellhoundRapesPlayer():void
		{
			...
			if (player.lowerBody == LOWER_BODY.CENTAUR) outputText("hocks and knees", false);
			...
			if (player.lowerBody.vaginaSpot.count() > 0 && player.lowerBody != LOWER_BODY.NAGA) outputText("slamming his twin dicks into your " + vaginaDescript(0) + " and your " + assholeDescript() + ".  ", false);
			...
			if (player.lowerBody.vaginaSpot.count() > 0 && player.lowerBody != LOWER_BODY.NAGA) {
				...
				if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS.WET) outputText("The pause gives your " + vaginaDescript(0) + " time to moisten, ensuring the next thrust won't be quite as rough.  ", false);
				...
				if (player.stats.cor < 40) outputText("You moan while trying to break free to get the heat off your back... but the beast keeps a firm grip on you as more and more of its fiery seed is pumped into and onto your body. After what feels like an eternity, the beast pulls out of you. He gives your " + assholeDescript() + " a single extended lick with his long dog-like tongue before running off out of sight. You are finally able to roll around and get the heat off of your back, but you can do nothing about the tainted warmth inside you. The heat proves to be too much and you pass out. After some time passes, you wake up to find the fire inside you has thankfully faded away. You're able to stand up again, but the damage is done and the creature's seed has left you feeling rather weak.", false);
...
//Merae's Lethicite
		...
		public function hellHoundPropahRape():void
		{
			...
			//--- ELSE (CORRUPTION >= 20) ---
				...
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText("  Your " + chestDesc() + " jiggle in exquisite ways along with your bumping, grinding motions.", false);
				...
				//--- IF PC HAS BALLS ---
				if (player.lowerBody.balls > 0) outputText("  Whenever you lower your hips to your mates belly, your " + ballsDescriptLight() + " touches his. The magical flames fill your scrotum with unnatural heat but no pain. You literally feel the cum in your balls boiling upwards, building up immense pressure.", false);
				outputText("\n\n", false);
				...
				if (player.totalCocks() > 0) {
					// --- IF ONE PENIS ---
					if (player.lowerBody.cockSpot.count() == 1) outputText("Your " + cockDescript(0) + " twitches as it ejects its sticky load.", false);
					//--- ELSE ---
