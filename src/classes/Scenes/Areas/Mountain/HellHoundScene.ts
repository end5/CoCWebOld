...
package classes.Scenes.Areas.Mountain
{
	...
	public class HellHoundScene extends BaseContent
	{
		...
		public hellhoundRapesPlayer():void
		{
			...
			if (player.lust >= 100) MainScreen.text("Overcome by sexual desire, you submit to the hungry hellhound.\n\n", true);
			...
			if (player.lowerBody == LOWER_BODY.CENTAUR) MainScreen.text("hocks and knees", false);
			else MainScreen.text("hands and knees", false);
			MainScreen.text(" and positions himself behind you. He grips you with his clawed hands on your shoulders and easily holds you in place. The powerful creature is easily able to get past your " + player.armorName + " and you feel one of his rods brush against your " + buttDescript() + ".  A shudder shoots through your body as he brings his two heads down to either side of your own, his hot canine breath blowing past your ears. He stays there for a few moments, completely still... before suddenly pulling his heads back and ", false);
			if (player.lowerBody.vaginaSpot.count() > 0 && player.lowerBody != LOWER_BODY.NAGA) MainScreen.text("slamming his twin dicks into your " + vaginaDescript(0) + " and your " + assholeDescript() + ".  ", false);
			else MainScreen.text("slamming his lower dick into your " + assholeDescript() + ", while rubbing the other between the cheeks of your " + buttDescript() + ".  ", false);
			if (player.lowerBody.vaginaSpot.count() > 0 && player.lowerBody != LOWER_BODY.NAGA) {
				if (player.vaginalCapacity() < monster.cockArea(0)) MainScreen.text("You feel an intense mixture of sensations in your lower body as your " + assholeDescript() + " feels like it is being torn apart, while your " + vaginaDescript(0) + " is filled with an intense pleasure at being filled with the creature's large member.  ", false);
				else MainScreen.text("Your lower body explodes with pain as the hellhound forces himself in too quickly for either your " + assholeDescript() + " or your " + vaginaDescript(0) + " to handle.  ", false);
				if (player.vaginas[0].virgin) MainScreen.text("<b>You are no longer a virgin!  </b>", false);
				if (player.cuntChange(monster.cockArea(0), false)) MainScreen.text("The beast howls as your " + vaginaDescript(0) + " is stretched to accommodate the large shaft.  ", false);
				MainScreen.text("The hellhound pants for a few seconds before continuing.  ", false);
				if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS.WET) MainScreen.text("The pause gives your " + vaginaDescript(0) + " time to moisten, ensuring the next thrust won't be quite as rough.  ", false);
				MainScreen.text("This time the beast starts pumping in and out more gently, only a short distance at a time as the hellhound continues panting.  As the pain in your " + assholeDescript() + " fades, you start feeling pleasure to match the sensations rising from your " + vaginaDescript(0) + ". Each thrust of the beast's twin manhoods bring you closer and closer to your peak.\n\n", false);
				//Cum
				...
				if (player.stats.cor < 40) MainScreen.text("You moan while trying to break free to get the heat off your back... but the beast keeps a firm grip on you as more and more of its fiery seed is pumped into and onto your body. After what feels like an eternity, the beast pulls out of you. He gives your " + assholeDescript() + " a single extended lick with his long dog-like tongue before running off out of sight. You are finally able to roll around and get the heat off of your back, but you can do nothing about the tainted warmth inside you. The heat proves to be too much and you pass out. After some time passes, you wake up to find the fire inside you has thankfully faded away. You're able to stand up again, but the damage is done and the creature's seed has left you feeling rather weak.", false);
			...
			player.orgasm();
			//[if not corrupt]
			if (player.stats.cor < 40) dynStats("tou", -2, "cor", 1);
			//[if corrupt]
...
//HELLHOUND MASTER+ BAD ENDZZZZZ
		public HellHoundMasterEncounter():void
		{
			...
			if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00141] == 0) {
			...
			else if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00141] == 1) {
				//Second Encounter (D)
...
//choose yes (C)
		...
		private acceptCrazyPresencesII():void
		{
			...
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00141] = 1;
			doNext(camp.returnToCampUseOneHour);
		}
...
//Regular Lethicite
		private giveLethicitePiece():void
		{
			...
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00141]++;
			doNext(camp.returnToCampUseOneHour);
		}
...
//Merae's Lethicite
		private giveALLTHELETHICITES():void
		{
			...
			if (player.dogCocks() >= 2) {
				...
				MainScreen.text("Instantly Cremera turns around and presents to you her waiting sloppy twin cunts.  The hellhound master walks behind you and slaps you on the ass.  \"<i>Don't keep her waiting.</i>\"  He says as a shudder passes through you and your " + player.armorName + " falls to the ground around you.\n\n", false);
				...
				if (player.totalCocks() > 2 || player.cockThatFits(60) < 0) {
					MainScreen.text("He then reaches around your waist and takes a hold of your " + multiCockDescriptLight() + ".  \"<i>Before we get started, let's make sure you're just right for Cremera.</i>\"  He then reforms your body to have twin doggy pricks of appropriately sized.  \"<i>Now.</i>\"\n\n", false);
					...
					player.lowerBody.cockSpot.list[1].cockLength = 14;
				...
				MainScreen.text("After what feels like hours of painful pleasure without release, you're smacked on your rear one last time, and it feels like Cimer bursts through your body completely.  This finally frees the floodgates, and you feel like you're releasing everything in your body in the most powerful orgasm you've ever felt.  You pant for a moment and look down in front of you.  You can see your " + player.allBreastsDescript() + " and your pregnant belly in front of you. You also see a package of demonic dicks sticking out in front of you, with a quad of hot balls resting underneath.  In front of you is a large pool of white steaming fluid, with a pink crystal resting in the middle.\n\n", false);
		...
		public hellHoundPropahRape():void
		{
			...
			//--- ELSE (CORRUPTION >= 20) ---
				...
				if (player.lowerBody.cockSpot.hasCock()) {
					...
					//--- IF ONE PENIS ---
					if (player.totalCocks() == 1) MainScreen.text("  Soon your own " + cockDescript(0) + " reacts, quickly growing into a full throbbing erection. You leave your member alone, and it keeps flapping against the hellhound's belly while you ride him.", false);
					//--- ELSE ---
					else MainScreen.text("  Soon " + sMultiCockDesc() + " reacts, quickly growing into a throbbing erection.  You leave your members alone, and they flap against the hellhound's belly while you ride him.", false);
				}
				//--- IF PC HAS BALLS ---
				if (player.lowerBody.balls > 0) MainScreen.text("  Whenever you lower your hips to your mates belly, your " + ballsDescriptLight() + " touches his. The magical flames fill your scrotum with unnatural heat but no pain. You literally feel the cum in your balls boiling upwards, building up immense pressure.", false);
				MainScreen.text("\n\n", false);
				...
				if (player.totalCocks() > 0) {
					// --- IF ONE PENIS ---
					if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("Your " + cockDescript(0) + " twitches as it ejects its sticky load.", false);
					//--- ELSE ---
					else MainScreen.text("Your " + multiCockDescriptLight() + " twitch as they eject their sticky loads.", false);
