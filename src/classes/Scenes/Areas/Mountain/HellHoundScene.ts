...
package classes.Scenes.Areas.Mountain
{
	...
	public class HellHoundScene extends BaseContent
	{
		...
		public function hellhoundRapesPlayer():void
		{
			...
			else Render.text("Too weak to continue fighting, you fall to your knees.\n\n", true);
			Render.text("The heads of the hellhound grin with desire as he rises onto his hind legs. As the hellhound stands before you, your eyes widen at the sight of its equipment: a pair of eight-inch long black rods, one above the other, with a sack containing a quad of flaming balls underneath. As the hound swaggers closer, you gulp at the thought of what those fiery balls are going to be pumping into you. You are helpless against the hellhound as he pushes you onto your ", false);
			if (player.lowerBody == LOWER_BODY.CENTAUR) Render.text("hocks and knees", false);
			else Render.text("hands and knees", false);
			Render.text(" and positions himself behind you. He grips you with his clawed hands on your shoulders and easily holds you in place. The powerful creature is easily able to get past your " + player.armorName + " and you feel one of his rods brush against your " + buttDescript() + ".  A shudder shoots through your body as he brings his two heads down to either side of your own, his hot canine breath blowing past your ears. He stays there for a few moments, completely still... before suddenly pulling his heads back and ", false);
			if (player.lowerBody.vaginaSpot.count() > 0 && player.lowerBody != LOWER_BODY.NAGA) Render.text("slamming his twin dicks into your " + vaginaDescript(0) + " and your " + assholeDescript() + ".  ", false);
			else Render.text("slamming his lower dick into your " + assholeDescript() + ", while rubbing the other between the cheeks of your " + buttDescript() + ".  ", false);
			if (player.lowerBody.vaginaSpot.count() > 0 && player.lowerBody != LOWER_BODY.NAGA) {
				if (player.vaginalCapacity() < monster.cockArea(0)) Render.text("You feel an intense mixture of sensations in your lower body as your " + assholeDescript() + " feels like it is being torn apart, while your " + vaginaDescript(0) + " is filled with an intense pleasure at being filled with the creature's large member.  ", false);
				else Render.text("Your lower body explodes with pain as the hellhound forces himself in too quickly for either your " + assholeDescript() + " or your " + vaginaDescript(0) + " to handle.  ", false);
				if (player.vaginas[0].virgin) Render.text("<b>You are no longer a virgin!  </b>", false);
				if (player.cuntChange(monster.cockArea(0), false)) Render.text("The beast howls as your " + vaginaDescript(0) + " is stretched to accommodate the large shaft.  ", false);
				Render.text("The hellhound pants for a few seconds before continuing.  ", false);
				if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS.WET) Render.text("The pause gives your " + vaginaDescript(0) + " time to moisten, ensuring the next thrust won't be quite as rough.  ", false);
				Render.text("This time the beast starts pumping in and out more gently, only a short distance at a time as the hellhound continues panting.  As the pain in your " + assholeDescript() + " fades, you start feeling pleasure to match the sensations rising from your " + vaginaDescript(0) + ". Each thrust of the beast's twin manhoods bring you closer and closer to your peak.\n\n", false);
				//Cum
				Render.text("As you reach your climax, the beast howls and you feel its incredibly hot seed pour into you.  ", false);
				if (player.stats.cor < 40) Render.text("You moan as your insides begin to heat up. The uncomfortably hot sensation only grows as more and more of its fiery seed is pumped into your body. After what feels like an eternity, the beast pulls out of you. He gives your " + vaginaDescript(0) + " and your " + assholeDescript() + " a single extended lick with its long dog-like tongue before running off out of sight. The tainted heat inside you proves to be too much and you pass out. After some time passes, you wake up to find the corrupt warmth inside you has thankfully faded away. You're able to stand up again, but the damage is done and the creature's seed has left you feeling rather weak.", false);
				else Render.text("His flaming seed brings about a pleasure you had not expected; your insides feel like they are burning with passion and power.  It is an incredible and fiery experience, one that you don't think you could have had if it wasn't for the power of corruption that you've gained since you got here. Too soon, the beast pulls out of you.  He gives your " + vaginaDescript(0) + " and your " + assholeDescript() + " a single extended lick with his long dog-like tongue before he runs off. You quickly look over and manage to catch a glimpse of its tail before it disappears from view, just before your body falls into a deep sleep. When you wake, you can still feel the afterglow of the hot seed inside you.", false);
				//Preggers chance!
				player.knockUp(PregnancyType.HELL_HOUND, PregnancyType.INCUBATION_HELL_HOUND, 101);
			}
			else {
				Render.text("Your lower body explodes with pain as the hellhound forces himself in too quickly for your " + assholeDescript() + " to handle. He pauses and pants for a few seconds, but you barely have enough time to catch your breath before the beast starts thrusting in and out again. However, this time the beast starts pumping in and out more gently, only a short distance at a time as the hellhound continues panting.  As the pain in your " + assholeDescript() + " fades, you start feeling strong pleasure from it. Each thrust of the beast's lower manhood, combined with the upper one tickling and teasing your " + buttDescript() + " above it brings you closer and closer to your peak.\n\n", false);
				//Cum
				Render.text("As you reach your climax, the beast howls and you feel its incredibly hot seed pour into you and onto your back.  ", false);
				if (player.stats.cor < 40) Render.text("You moan while trying to break free to get the heat off your back... but the beast keeps a firm grip on you as more and more of its fiery seed is pumped into and onto your body. After what feels like an eternity, the beast pulls out of you. He gives your " + assholeDescript() + " a single extended lick with his long dog-like tongue before running off out of sight. You are finally able to roll around and get the heat off of your back, but you can do nothing about the tainted warmth inside you. The heat proves to be too much and you pass out. After some time passes, you wake up to find the fire inside you has thankfully faded away. You're able to stand up again, but the damage is done and the creature's seed has left you feeling rather weak.", false);
		...
		public function hellHoundGetsRaped():void
		{
			...
			if (player.gender == 0) Render.text("The two heads stare at your lack of sexual endowments for a few seconds before looking up at you sadly.  With a whine, the hellhound wanders off.  Feeling rather unsatisfied, you think that you should probably fix your lack of sexual 'parts'...", false);
			else player.orgasm();
			cleanupAfterCombat();
		}
...
//HELLHOUND MASTER+ BAD ENDZZZZZ
		public function HellHoundMasterEncounter():void
		{
			...
			if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00141] == 0) {
				Render.text("As you're wandering the mountains, you feel an odd presence in the back of your mind.  It calls out to you with promise of power and safety, but part of you feels a bit apprehensive at the prospect of giving yourself over to it.  Do you let the presence take over?", false);
...
//Choose yes (F)
		private function giveUpLethicite():void
		{
			...
			Render.text("Another succubus stands guard at the entrance with another hellhound standing next to her.  She gives you a bit of a disdainful look before her hound steps forward.  \"<i>My mistress doesn't like dealing with lower hounds.  The grand master will be glad to see you, however.  Please go right in.</i>\"  One of its heads intones before directing you inside the fort.  Without hesitation you rush in.\n\n", false);

			Render.text("You barely spare a thought at the interior in your hurry to get to the master.  As you burst into the room, the master's eyes light up at the sight of you.  \"<i>Ah!  It is my favorite fan.  Have you brought me a piece of that pink crystal I asked you for?</i>\"\n\n", false);
...
//Regular Lethicite
		private function giveLethicitePiece():void
		{
			...
			Render.text("You produce the pink crystal and hand it to the master.  As the crystal touches his hand, he gives a low growl of pleasure and holds the crystal up to his eyes.  \"<i>Ah yes, excellent!  Thank you so much for this tasty treat of power.</i>\"\n\n", false);
			//if (PC gave ⅔ Merae's crystal)
			if (player.keyItemv2("Marae's Lethicite") == 1) {
				Render.text("He looks at the crystal for a moment, before biting off half of it and giving the other half back to you.  \"<i>This is enough for the hellfire, I think.  Go ahead and keep the rest, my favorite fan!</i>\"\n\n", false);
...
//Merae's Lethicite
		private function giveALLTHELETHICITES():void
		{
			...
			if (player.dogCocks() >= 2) {
				...
				if (player.totalCocks() > 2 || player.cockThatFits(60) < 0) {
					Render.text("He then reaches around your waist and takes a hold of your " + multiCockDescriptLight() + ".  \"<i>Before we get started, let's make sure you're just right for Cremera.</i>\"  He then reforms your body to have twin doggy pricks of appropriately sized.  \"<i>Now.</i>\"\n\n", false);
		...
		public function hellHoundPropahRape():void
		{
			...
			else Render.text("Unable to bear hurting you anymore, the hellhound's flames dim as he stops its attack. The two heads look at you, whining plaintively. After all the trouble you had with him, you want to have some fun in exchange. \"<i>Bad boy, roll over!</i>\" you command as if the hellhound was just an ordinary domestic dog. To your surprise the creature actually does as you say. Accepting your victory he rolls onto his back, exposing his human-like abs and throat, signaling submissiveness. Looking down on your foe, your lustful gaze rests upon his privates. You examine the unusual ballsack with its quadruple testes, but the real treasures are pulsating just above it. Pulling on his sheath, you smile as his twin members twitch in response. An obsessive idea comes to your mind and you are determined to put his magnificent tools to a good use.\n\n", false);

			Render.text("You start caressing his pair of wonderful canine cocks. Although the hellhound is pretty out of it, his dicks ", false);
			if (monster.HP < 1) Render.text("spring to life, extending rapidly from the sheath. Tentatively you give one of them a gentle lick, being rewarded with a drop of pre-cum.\n\n", false);
			else Render.text("still manage to leak plenty of hot, steamy pre-cum all over his belly. Tentatively you give one of them a gentle lick, being rewarded with a dollop of the stuff.\n\n", false);
			//--- IF CORRUPTION < 20 ---
			if (player.stats.cor < 20) {
				Render.text("The corrupt juice burns on your tongue, far worse than the hottest spicy dish you have ever had. You instantly back off from his member, cursing this abomination of nature. Leaving the submissive creature as it is, you head back for your camp.", false);
				dynStats("lus", -99);
				cleanupAfterCombat();
				return;
			}
			//--- ELSE (CORRUPTION >= 20) ---
				...
				//--- ELSE (CORRUPTION >= 40) ---
				Render.text("His magical natural flames fill your body with fierce warmth, arousing you further. You pick yourself up, letting the hellhound's cocks nearly escape your nethers just to sit down again, taking the duo of dog members to the hilt. Grabbing his hind legs like the handlebars of an exercise machine, you bring yourself into position for a good amount of training. Gradually, you pick up the pace, rocking your hips up and down, enjoying the awesome double penetration.", false);
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) Render.text("  Your " + chestDesc() + " jiggle in exquisite ways along with your bumping, grinding motions.", false);
				Render.text("  Riding the hound for a couple of minutes, you feel the dick in your " + assholeDescript() + " extending, giving you a good stretching. Its brother in your " + vaginaDescript(0) + " does the same, pushing more and more girl-fluids out of your slippery slit while stimulating your sensitive clit from the inside. You lean forward, running your hands through your fuckpet's dense midnight black fur, feeling his animal body warmth. The hellhound is moaning and panting beneath you, unsure whether to fear you or to enjoy what you are doing to him.\n\n", false);

				//--- IF PC HAS PENIS ---
				if (player.lowerBody.cockSpot.hasCock()) {
					Render.text("His canine cock in your anus stimulates your prostate perfectly. His exotic black dong applies pressure in parts of your body you wouldn't expect to get such pleasure from.", false);
					//--- IF ONE PENIS ---
					if (player.totalCocks() == 1) Render.text("  Soon your own " + cockDescript(0) + " reacts, quickly growing into a full throbbing erection. You leave your member alone, and it keeps flapping against the hellhound's belly while you ride him.", false);
					//--- ELSE ---
					else Render.text("  Soon " + sMultiCockDesc() + " reacts, quickly growing into a throbbing erection.  You leave your members alone, and they flap against the hellhound's belly while you ride him.", false);
				}
				//--- IF PC HAS BALLS ---
				if (player.lowerBody.balls > 0) Render.text("  Whenever you lower your hips to your mates belly, your " + ballsDescriptLight() + " touches his. The magical flames fill your scrotum with unnatural heat but no pain. You literally feel the cum in your balls boiling upwards, building up immense pressure.", false);
				Render.text("\n\n", false);

				Render.text("Way too early, the two doggie pricks stuffed inside you start to twitch, unloading four balls worth of spunk into you. In ecstasy, the hellhound's eyes look in four different directions. The dual eruptions in your love-canal and rectum feel amazing as they pulse in and out of sync, dumping cum into your body over and over.  The hellhound's contractions ebb away as he finishes, but you're not done yet. You slowly start tilting your hips back and forth, continuing to fiercely fuck the prime member while the other rests nearly motionless in your ass. Soon the hellhound whines from the unwanted stimulation of his now sensitive pricks, but he is too defeated to put up much of a struggle.\n\n", false);

				Render.text("Slowly but steadily you bring yourself to the verge of orgasm. The hellhound under you squirms and tries to get free, his hind legs flailing through the air behind your back. You keep riding his magnificent member, grinding it up and down in your moist cave. Finally you feel the familiar rippling of your vaginal walls, milking his cock and intensifying your pleasant sensations. Clamping down on his members, you keep moving your hips in a circling motion, working your clit, vagina and butt in equal measure. The sensations eventually grant you an intense orgasm while the hellhound's still-distended members apply the perfect pressure to all of your sensitive spots and prolong your bliss.", false);
				//--- IF PC HAS PENIS ---
				if (player.totalCocks() > 0) {
					// --- IF ONE PENIS ---
					if (player.lowerBody.cockSpot.count() == 1) Render.text("Your " + cockDescript(0) + " twitches as it ejects its sticky load.", false);
					//--- ELSE ---
					else Render.text("Your " + multiCockDescriptLight() + " twitch as they eject their sticky loads.", false);
					Render.text("  Keeping up your motions, you scatter your cum across the hellhound's body and the ground nearby.", false);
					//--- IF MESSY ORGASM PERK OR CUM MULTIPILER > 5 ---
					if (player.cumQ() > 700) Render.text("  As your orgasm drags on, you practically drench the hellhound in your seed. An especially large load lands on his snouts and the heads greedily start licking your juice with their canine toungues. Your mate's black fur is soaked with your cum, and he'll definitely be busy licking himself clean for some time.", false);
				}


				Render.text("\n\n", false);

				Render.text("Busy with your own orgasm, you barely notice the twitching of the dual pricks buried inside you.  The hellhound launches into another orgasm or two, his dongs aching for peace and wanting to go soft, but forced to orgasm by your body.  You clench your muscles, locking the blood in the throbbing members, trying to preserve the sensations forever.\n\n", false);

				Render.text("Finally having finished, you relax your anal sphincter and allow his member to leave your anus. His members rise from the feel of the incredible amount of cum his quadruple balls pumped into your holes squirting back out to wash over them. The hellhound curls up, spent, his tail covering his privates. His rods look a little red from the intense fuck, and you're sure that in spite of his fiery nature he is experiencing a little more than a burning sensation.", false);
