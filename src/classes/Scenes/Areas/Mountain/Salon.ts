export default class Salon implements TimeAwareInterface {
	...
	public hairDresser(): void {
		MainScreen.text("While exploring the mountain, you find a cleverly concealed doorway.  From inside you can hear the sound of blades being sharpened.  Do you enter the doorway?", true);
	...
	public salonGreeting(): void {
		if (player.statusAffects.has("HairdresserMeeting")) {
			...
			player.statusAffects.add(new StatusAffect("HairdresserMeeting", 0, 0, 0, 0)));
			hairDresserGreeting();
		}
	}
	...
	private salonFavoritesPaymentMenu(): void {
		...
		menu();
		if (Flags.list[FlagEnum.LYNNETTE_PREGNANCY_CYCLE] >= 4 && player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(5, "Fuck Goblin", fuckLynnette);
	...
	private buyMinoCum(): void {
		if (player.stats.gems < 60) {
			MainScreen.text("You can't afford any minotaur cum right now!", true);
			...
			player.stats.gems -= 60;
			MainScreen.text("You happily give Lynnette 60 gems and pick up the bottle full of glistening, heavenly cum.  ", true);
			statScreenRefresh();
	...
	public salonPurchaseMenu(): void {
		...
		let cutMedium2: Function = null;
		let cutLong2: Function = null;
		let lengthening: Function = null;
		let minoCum: Function = null;
		let mudFacial2: Function = null;
		let sandFacial2: Function = null;
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00142] > 0) minoCum = buyMinoCum;
		if (player.upperBody.head.hairLength > 2) cutShort2 = cutShort;
		if (player.upperBody.head.hairLength > 13) cutMedium2 = cutMedium;
		if (player.upperBody.head.hairLength >= 26) cutLong2 = cutLong;
		if (player.upperBody.head.hairLength < player.tallness) lengthening = hairGrow;
		...
		else if (player.femininity < 100 && player.perks.has("Androgyny")) mudFacial2 = mudFacial;
		...
		else if (player.femininity > 0 && player.perks.has("Androgyny")) sandFacial2 = sandFacial;
	...
	private hairDresserGreeting(): void {
		...
		MainScreen.text("   \"<i>I apologize for my daughters,</i>\" she says as she presses herself against you.  \"<i>They're a bunch of brainless hussies for the most part.  My name is Lynnette, and welcome to my salon!  You want your hair cut or lengthened?  We've got you covered, and we don't ask for much - just a shot of cum.", false);
		if (player.lowerBody.cockSpot.count() == 0) {
			MainScreen.text("  You look like you don't got any of your own, but we've got glory holes in the back if you need to get some.  Just don't swallow too much, ok?</i>\"\n\nShe shows you to the back of the cave, which is boarded-up.  There are about 20 holes in boards, and most are empty.  While you watch, a few new dicks slide in, and just as quickly the goblin's daughters commence sucking and fucking them.  There are only a few you could take a crack at - do you blow one (and if so which one)?", false);
		}
		else {
			MainScreen.text("  I'll even do you the favor of letting you blow it in my mouth, I've already got a bun in the oven.  So what do you say?  Want a spooge and a haircut?  Or would you rather go get your payment from one of the gloryholes in the back, you " + player.mf("kinky boy", "naughty girl") + "?</i>\"\n\n", false);
			MainScreen.text("In the very back of the salon you can see a boarded-up wall with holes cut in it, some of which are currently plugged by various monstrous penises.  Do you let the goblin blow you, or do you go suck your payment from one of them?", false);
		}
		salonPaymentMenu();
	...
	private hairDresserRepeatGreeting(): void {
		...
		if (rand(5) == 0 && (player.perks.has("MinotaurCumAddict") || Flags.list[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] > 0)) {
		...
		//Favorite Greeting 
			...
			if (Flags.list[FlagEnum.LYNNETTE_PREGNANCY_CYCLE] < 4 && Flags.list[FlagEnum.LYNNETTE_CARRYING_COUNT] > 0) {
				MainScreen.text("Lynnette has a little difficulty ushering you in with her swollen belly in the way. She's tremendously heavy with child already, likely due to some goblin-engineered chemicals that accelerate her pregnancy. Her tremendously fertile hips roll back and forth as she waddles a little deeper into the salon, greeting, \"<i>Welcome back, honey-bunch. Here to make a donation to the cause or get some freebies from your favorite pregnant slut?</i>\" She twists her ass in your direction and slaps it, making it jiggle in her sheer dress.");
			}
			...
			//Nonpreggers
				...
				MainScreen.text("Pulling the door the rest of the way open before you've the latch more than half-turned, Lynnette has you inside with disorienting speed.  She presses her well-endowed, dress-straining tits against your ");
				if (player.tallness <= 48) MainScreen.text("side");
				else if (player.tallness <= 68) MainScreen.text("[hips]");
				else MainScreen.text("[leg]");
				MainScreen.text(", compressing her mountainous mammaries against you hard enough to leave milky stains in her wake.\n\n\"<i>How's my favorite breeder doing?</i>\" comes the honeyed words from her mouth, and as she begins to feel at the crotch of your [armor], she asks, \"<i>I can cut your hair, if you like, but why do that when there's a woman with a quim in front of you, waiting to get stuffed?</i>\"  She smirks. \"<i>So what'll it be?</i>\"");
			}
			if (player.lowerBody.cockSpot.count() == 0) {
		...
		//Non-Pregnant Greetings:
			...
			if (Flags.list[FlagEnum.LYNNETTE_MET_UNPREGNANT] == 0) {
				...
				MainScreen.text("Lynnette's familiar face greets you at the door to her salon once more, and she waves you in with a beatific smile plastered upon her face. Clinging to her like a second skin, the businessgoblin's dress highlights her surprisingly flat midsection; she's not pregnant!");
		...
		//Standard repeats
			...
			MainScreen.text("Lynnette the goblin answers the door and lets you in, waving you deeper into her shop.  Her shining black dress barely contains her fertile-hips and jiggling chest as she greets you, \"<i>Welcome back, honey!  ", true);
			if (player.lowerBody.cockSpot.count() == 0) {
				MainScreen.text("I see you still can't make payment for our services on your own.  No worries, the glory holes in the back are pretty popular with the demons and monsters, just go back there and catch some spooge and we'll see about helping with your hair, ok?</i>\"\n\n(There are a number of dicks in the glory hole, which do you want to deal with (if you're willing at all)?", false);
			}
			else {
				MainScreen.text("Will you be getting our normal services?  Just one BJ per hair treatment!  Or would you rather get it from the gloryholes in back?</i>\"", false);
			}
		}
		salonPaymentMenu();
	}
	...
	private gloryholeDoggie(): void {
		player.slimeFeed();
		MainScreen.text("", true);
		MainScreen.text("You sigh and kneel down to bring yourself level with the dog-dick hanging out of the wall.  It's pointed at the tip with a swollen circular bulb at the base.  As a matter of fact, the dog-dick's owner must be pretty excited to be here - it's dripping cum and the knot has swollen so large that it can't fit back through the hole.\n\n", true);
		if (player.stats.cor < 33) {
			MainScreen.text("It's a struggle to force your lips apart and take the strange thing in your mouth.  You feel like a freak, but make yourself get to work servicing the male who'll be covering your hair treatment today.  It tastes salty and sweaty and has a potent musky scent that excites you in spite of your better judgment.  You find yourself starting to get into it as you lick and slurp, humming with pleasure.  You reach up to touch the knot and the dog-cock jumps in your mouth, spurting a wad of thick cum into your throat.  You pull back most of the way and pump the cock, collecting the jism in your mouth.\n\n", false);
			player.stats.lust += 15;
			player.stats.cor += .25;
		}
		else if (player.stats.cor < 66) {
			MainScreen.text("You open wide and take as much of the pointed dick as you can into your mouth.  A few times you nearly gag yourself, but you manage to get most of the way down to the thick swollen knot.  You start sliding back and forth, breathing in the musky animalistic scent of your 'client' and become aroused by the situation more and more.  You reach up and caress the knot, squeezing gently and stroking the dick until it unloads a thick wad of cum into your mouth.  You pull back and catch the fruits of your labor in your mouth, milking the dick with your hands until it's finished and the dick pops back through the hole.\n\n", false);
			player.stats.lust += 25;
			player.stats.cor += .25;
		}
		else {
			MainScreen.text("You throw yourself onto the swollen dog-cock and easily engulf the whole thing, spreading your lips wide around the thick knot.  You can feel it bouncing the uvula in your throat around but you manage to supress your gag reflex like a good slut while you work the sweaty dick, giving it a thorough tongue bath.  You wrap your lips around the base of the knot, feeling it start to stretch your jaws apart.  You pull back before it can lock you like that, and squeeze the knot in your hands.  It cums, unloading thick wads of gooey cum into your mouth.\n\n", false);
			player.stats.lust += 30;
			player.stats.cor += .3;
		}
		MainScreen.text("A young goblin comes by with a bowl for you to make your payment into.  You spit out the gunk and wipe your mouth, as the goblin carries the seed away.  You notice a trail of clear drops on the ground behind her.  She must be anticipating something...", false);
		doNext(hairDressingMainMenu);
	}
	private gloryholeIncubus(): void {
		player.slimeFeed();
		MainScreen.text("", true);
		MainScreen.text("You kneel down in front of the throbbing demonic dick, ready to earn your pay.\n\n", false);
		if (player.stats.cor < 33) {
			MainScreen.text("You picked this dick hoping that a male lust demon would be the most pleasurable to service, and judging by the pleasant spicy scent the member gives off, you're right.  You open wide and take it inside your mouth, feeling the ridges around the crown pressing back against your lips. A drop of pre-cum hits your tongue and it tastes divine, spurring you on to please the tainted member.  You grab it with both hands and pump it, feeling the corrupted nodules under your fingers as you lick and suck on the head, eager for more.  You get your reward soon enough, and you eagerly start drinking down the copious waves of fluid.  Thankfully, the incubus' orgasm seems to last forever, and when he finishes, you have a full belly and a mouth full of cream.  You manage to resist this last swallow, as you still need to pay.\n\n", false);
			player.stats.lust += 1;
			player.stats.cor += 1.5;
			player.stats.lust = 99;
		}
		else {
			MainScreen.text("You take the corrupted cock in your hands and guide it into your waiting mouth, eager to try a walk on the wild side.  The incubus-cock does not disappoint, quickly dripping a steady flow of wonderful tasting pre-cum that only whets your appetite for the demon's cum.  You lean forwards, taking most of the bumpy purple dick inside your mouth and running your tongue all over it, paying special attention to the ring of nodules around the bottom edge of his crown.  You're quickly rewarded with a blast of heavenly demonic seed.  You gulp it down, craving more of it, and thankfully the incubus has plenty for you.  You're shivering and horny by the time he finishes, but thankfully you have enough willpower to keep a mouthful of cum for payment.\n\n", false);
			player.stats.lust += 1;
			player.stats.cor += 1;
			player.stats.lust = 99;
	...
	private gloryholeMinotaur(): void {
		player.slimeFeed();
		MainScreen.text("Your eyes are drawn to the huge minotaur cock, and you instinctively kneel down in front of it.\n\n", true);
		MainScreen.text("It's the largest available member in the room by a considerable margin, and from your position on your knees, it looks even more massive; it's over two feet long, and has three rings of prepuce spaced around its length. You can just barely span your entire hand around its thickness. You open your mouth wide and lean forward, taking the thick, spongy head into your mouth. The taste is incredibly strong, and its musk is as thick as the minotaur's dick itself. You moan in pleasure, and start bobbing your head back and forth, taking more and more of the dick into your mouth. You stroke the rest of the cock in time with your sucking. You can hear muffled grunting and snorting coming from the other side of the wall, obvious sounds of approval from your partner.\n\n", false);
		MainScreen.text("Driven on by the minotaur's reactions, you take more and more of the minotaur cock into your mouth. As you work your mouth around the first ring, you open up the top section of your " + player.inventory.armor.displayName + ", freeing up your " + BreastDescriptor.describeAllBreasts(player) + ". With each pass, you're able to take more and more of the bull dick down your throat. You caress your " + biggestBreastSizeDescript() + " while moaning around the dick. When you get your mouth around the second ring, the minotaur's cock throbs sharply, and his cum explodes in your mouth. Your eyes bulge in shock, and you pull off the cock as quickly as you can. You swallow the first several shots of thick, rich minotaur cum, while the rest shoots all over your face and tits. You ");
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) MainScreen.text("hold up ");
		else MainScreen.text("arch your back to expose ");
		MainScreen.text("your [chest] as cum rains down upon you. By the time his orgasm stops and his dick goes limp, your face and [chest] are covered in a thick layer of spooge.\n\n", false);
		MainScreen.text("Your head spins from the minotaur's musk, and you idly mop up and swallow the cum on your " + HeadDescriptor.describeFace(player) + ". A goblin aide comes in with a bowl, and gently scrapes the cum off your tits with a smooth, flat rock. Once you're cleaned up and you're dressed, the aide leads you back to Lynnette.\n\n", false);
	...
	private goblinHairDresserFacefuck(): void {
		spriteSelect(38);
		MainScreen.text("Lynnette licks her lips and practically tears her way into your " + player.inventory.armor.displayName + ", having your crotch exposed in seconds.  Your " + CockDescriptor.describeCock(player, 0) + " flops out immediately, slapping her on the nose as it grows hard.  She wraps both hands around you and begins pumping with practiced ease, flicking her tongue over your crown and wrapping her lips ", true);
		if (player.lowerBody.cockSpot.get(0).cockThickness >= 4) MainScreen.text("around as much of you as she can", false);
		else MainScreen.text("around your first few inches", false);
		MainScreen.text(".  She slips her tongue down your urethra, and arches her back, popping the straps on her dress and letting her oversized tits engulf your shaft.  Milk sputters and dribbles from the lactating goblin, lubricating her breasts and hands as she pleasures every inch of you.\n\n", false);
		MainScreen.text("It's impossible to hold back while receiving such expertly given stimulation, and you blow your load, bulging her cheeks out like a squirrel's.", false);
		if (player.cumQ() >= 250) MainScreen.text(" She can't hold it all and suddenly starts swallowing as runnels of it escape her nose and a wave of it rolls down her lips.", false);
		MainScreen.text("  In time it ends, and she pops back, spitting most of the cum into a funnel.  It washes down a pipe and you have to wonder if there's some horny goblin girl at the other end with her cunt spread wide.", false);
	...
	private hairDressingMainMenu(): void {
		spriteSelect(38);
		MainScreen.text("Lynnette offers and explains their options, \"<i>So what'll it be hun?  We could cut it down or give you a lengthening treatment. Or you can get a hair-dye to use on your own.  Just remember to come back in a few days for a touchup.</i>\"", false);
	...
	private cutShort(): void {
		...
		if (player.upperBody.head.hairType == 4) {
			MainScreen.text("Lynnette stares at you when you ask for a cut.  \"<i>Nothing doing, hon; that stuff looks alive and I don't want blood all over my nice floor.  Thanks for contributing to the white file, though; maybe we can do something else?</i>\"\n\n", false);
			salonPurchaseMenu();
			return;
		}
		MainScreen.text("Lynnette and her daughters crowd around you with razor-sharp scissors, effortlessly paring down your " + HeadDescriptor.describeHair(player) + ".  When they've finished, you're left with ", true);
		player.upperBody.head.hairLength = 1;
		MainScreen.text(HeadDescriptor.describeHair(player) + ".", false);
	...
	private cutMedium(): void {
		...
		MainScreen.text("Lynnette and her daughters crowd around you with razor-sharp scissors, effortlessly paring down your " + HeadDescriptor.describeHair(player) + ".  When they've finished, you're left with ", true);
		player.upperBody.head.hairLength = 10;
		MainScreen.text(HeadDescriptor.describeHair(player) + ".", false);
		doNext(camp.returnToCampUseOneHour);
	}
	private cutLong(): void {
		...
		//-trying to get a goblin to cut tentacle hair:
		if (player.upperBody.head.hairType == 4) {
			MainScreen.text("Lynnette stares at you when you ask for a cut.  \"<i>Nothing doing, hon; that stuff looks alive and I don't want blood all over my nice floor.  Thanks for the contributing to the white file, though; maybe we can do something else?</i>\"\n\n", false);
			salonPurchaseMenu();
			return;
		}
		MainScreen.text("Lynnette and her daughters crowd around you with razor-sharp scissors, effortlessly paring down your " + HeadDescriptor.describeHair(player) + ".  When they've finished, you're left with ", true);
		player.upperBody.head.hairLength = 25;
		MainScreen.text(HeadDescriptor.describeHair(player) + ".", false);
		doNext(camp.returnToCampUseOneHour);
	}
	private hairGrow(): void {
		...
		if (player.upperBody.head.hairType == 4) {
			MainScreen.text("Lynnette looks dubiously at you when you ask for a lengthening treatment.  \"<i>No offense hon, but that stuff is basically like an arm or an organ, not hair.  I'm not a goblin chirurgeon, and I wouldn't try to lengthen it even if one of my disobedient daughters were here to donate some parts.  Sorry to make you shoot and scoot, but I can't help you.  Maybe we could do something else?</i>\"\n\n", false);
			salonPurchaseMenu();
			return;
		}
		MainScreen.text("Lynnette grabs a bottle and squirts a white fluid into your hair.  You really hope it isn't your payment.  But it must not be, as within short order you feel the added weight of ", true);
		temp = rand(3) + 3;
		Flags.list[FlagEnum.HAIR_GROWTH_STOPPED_BECAUSE_LIZARD] = 0;
		player.upperBody.head.hairLength += temp;
		MainScreen.text(num2Text(temp) + " more inches of " + player.upperBody.head.hairColor + " hair.", false);
		doNext(camp.returnToCampUseOneHour);
	}
	...
	private dyeMenu(): void {
		spriteSelect(38);
		MainScreen.text("", true);
		MainScreen.text("Lynnette pulls open a cabinet in the corner, displaying a wide array of exotic hair-dyes.  Which kind do you want?", false);
		menu();
	...
	private minotaurCumBukkakeInSalon(): void {
		...
		player.minoCumAddiction(10);
		player.slimeFeed();
		MainScreen.text("As the salon door swings closed behind you, a familiar, heavenly scent catches your nose and wicks into your brain, flooding you with need and molten-hot lust.  Lynnette saunters over with her lips slightly pursed and her body jiggling, but you brush her aside.  She's completely forgotten as you close in on the source of your olfactory bliss.  Your sigh dreamily while your pupils slowly dilate from the familiar chemicals pounding through your bloodstream", false);
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" and puffing up your twat with liquid arousal.\n\n", false);
		else if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" and turning " + CockDescriptor.describeMultiCockSimpleOne(player) + " into a turgid, pulsating mass.\n\n", false);
		else MainScreen.text("\n\n", false);

		MainScreen.text("You push your way past a few more staring goblin sluts, tearing off your " + player.inventory.armor.displayName + " as you go.  The animal part of your brain recognizes that such needless trapping would just get in the way of all the thick, dripping, minotaur spunk just waiting to pump inside you.  Another door swings closed behind you while the scent drags you heedlessly deeper into the salon, closing in on the secluded glory-holes the goblins keep hidden away in the back.  You drool spittle down your neck while you lose yourself in the memory of that taste on your tongue, letting your body seek it out on autopilot.", false);
		if (player.lowerBody.vaginaSpot.hasVagina() || player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text("  A trail of ", false);
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				MainScreen.text("female slime ", false);
				if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("and ", false);
			}
			if (player.lowerBody.cockSpot.hasCock()) {
				if (player.cumQ() < 100) MainScreen.text("pre-cum ", false);
				else if (player.cumQ() < 500) MainScreen.text("pre-cum a few inches wide ", false);
				else MainScreen.text("pre-cum over a foot wide ", false);
			}
			MainScreen.text("winds over the rough-hewn floor behind you, clearly marking your passage to the overpowering musk.", false);
		}
		MainScreen.text("\n\n", false);

		MainScreen.text("Time passes in a slow, torturous manner, but you turn the corner and sigh.  Salvation is at hand – you've reached the glory-holes, and to your delight there's nearly a dozen hard, dripping, delicious... mmmm... minotaur cocks crammed through the holes, some bulging out around the seams and leaking with need.  You saunter forwards, hips swaying sensually as the drug-like desire of potent minotaur musk pulls you ever closer.  Mewling happily, you take one of them in hand and stroke along its length, giggling as it pulses and leaks a stream of heavenly goo down your arm.   You lick it from your arm in one long, languid motion before pursing your lips around the minotaur's flared tip and sucking it hard as you quest for more of its heady ambrosia.\n\n", false);

		MainScreen.text("You hear a deep, strangled sigh as more and more delicious pre floods your mouth, lighting your senses up with a fireworks show of pleasure", false);
		if (player.gender > 0) MainScreen.text(" and increasing the size of the puddle you're leaving on the floor", false);
		MainScreen.text(".  In between gulps of the fragrant fluid, you suck the swollen rod deeper and deeper into your mouth, gleefully suppressing your gag reflex as it pushes aside your uvula.  You work your throat, feverishly swallowing to suck it down.  A flare briefly distends your neck as it's taken deeper and deeper inside your form until your lips are puckered through the hole and you're sniffing the minotaur's pheromone-laced crotch.\n\n", false);

		MainScreen.text("The beast pulls back and starts pounding the hole, letting out deep, rumbling sighs of pleasure with each throat abusing glory-hole fuck.  If it weren't for his constant, bubbling preseed turning your throat to a slippery rut-hole you'd be rubbed raw, but instead you continue to lean forward, sniffing at his matted pubes every time he crushes them against your dick-stretched lips.  The other horny studs grunt with displeasure, but you reach out with your hands and grab hold two eager brutes' throbbing horsecocks.  Three out-of-sync heartbeats hammer through your hands and throat while you do your best to get them all off.\n\n", false);

		MainScreen.text("You let your eyes drift closed and lash your tongue back and forth across the minotaur's lowest medial ring, feeling his flare bulge larger inside you in response.  The beast-man batters at the wall, grunting in equal measures of passion and pent-up desire as his cock's tip stretches wide and locks itself inside you.  His urethra bulges in rhythmic contractions, stretching your lips around the already swollen shaft and signaling to your musk-addled mind that you're about to get what you desire.  You hum with blissful delight as your belly begins to gurgle, accepting long bursts of sticky minotaur drug.\n\n", false);

		MainScreen.text("Tied down by the cum-spurting flare locked in your gullet, you pump on the other two dicks with feverish speed and sway on your " + LowerBodyDescriptor.describeFeet(player) + " as the narcotic spooge intoxicates your already-addled mind.  You can feel each muscular contraction pulsing through the bestial shaft while it finishes depositing the heavy, sticky load, and your eyes cross from the viscous inebriation that's pooling in your belly.  Drizzles of pre-cum soak into your arms and palms, drawn out from the frenzied pumping of your fists.  They won't come from just a hand though.  They need something... tighter.\n\n", false);

		MainScreen.text("You start moaning in drug-induced bliss, but your vocalized pleasure is interrupted by the squelching slurp of the softening shaft being pulled from your dick-puckered lips.  It drips a rope of cum over your mouth and chin as it pulls free from the wall, leaving behind one vacancy among the swarm of ready minotaur dicks.  You lick up your stud's leavings and purr in bliss, reaching through the hole to cup the departing minotaur's balls teasingly.  He grunts and walks out of your grip – sated for now.  You curl your fingers into a come-hither gesture, retract your arm through the sweat-lubricated glory-hole, and wait for one of the other minotaurs to come over.\n\n", false);

		MainScreen.text("The beast who kept your left hand so busy repositions himself at the now-vacant opening, and you decide to reward him for moving so quickly.  You lick the last of the salty cream from your lips and muse that he isn't the only one getting a reward, but the monstrous cow-man doesn't need to know that.", false);
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) {
			MainScreen.text("  You wrap the pillowy flesh of your " + BreastDescriptor.describeAllBreasts(player) + " around the new member, pleasantly surprised by its girth and wide, already-flared tip.  Maybe you could have gotten him off with your hands after all?  ", false);
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating < 6) MainScreen.text("Even so, you can't quite get your breasts the entire way around him, so you make up for it by pressing it harder into you with your busy hands.  ", false);
			MainScreen.text("Runnels of pre cover the shaft and squish wetly between your tits, turning your body into a slip-n-slide that reeks of hot, sticky minotaur sex.", false);
		}
		else {
			MainScreen.text("You crush the member tightly against your chest, smearing your torso with the copious pre until you're like a hot, wet slip-n-slide of minotaur sex.  You make up for it by wrapping your arms around his prodigious thickness and squeeze him tightly with your hands, stroking him off with your whole body with enough tightness to make the poor mino' think he's in some poor adventurer's asshole.", false);
		}
		MainScreen.text("\n\n", false);

		MainScreen.text("You lower and raise yourself, bouncing up and down on your " + LowerBodyDescriptor.describeLegs(player) + " to enhance the ", false);
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) MainScreen.text("tit-fucking ", false);
		else MainScreen.text("full-body handjob ", false);
		MainScreen.text("you're giving out.  In such a position, you're given the perfect view to watch as your strokes draw forth large bubbles of pre, and before you can lose your high, you latch onto the minotaur's vulnerable urethra and suck, tonguing in wide circles around it since you can't open wide enough to accommodate his flare.  Of course, all the attention just makes him flare wider, not just at the tip, but through the whole shaft.  ", false);
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) MainScreen.text("The sudden girth change sends an enticing ripple through your " + BreastDescriptor.describeAllBreasts(player) + " that's pleasurable enough to make you moan into his steadily-widening urethra.  ", false);
		MainScreen.text("You pull off and bounce faster, lost to your lust and the haze of sex-musk permeating the air, intent on seeing just how much this huge stud can spray onto you.\n\n", false);

		MainScreen.text("The minotaur does not disappoint.  His hole dilates from the size of the approaching cum-blast, and you sink down his shaft slowly until it's aimed directly at your face.  You close your eyes and feel the first explosion splatter over your " + HeadDescriptor.describeHair(player) + " and forehead.  The next takes you full in the face, making it difficult to breathe through the mask of drug-like goo, but a few quick licks gives you a fix and makes it easy to breathe again.  On and on, the minotaur pumps fat ropes of spooge over your body until you're a syrupy, sticky mess that reeks of minotaur pheromones so strongly that dizziness overwhelms you and you fall free of the still-orgasming mino-cock, taking a few final blasts of seed on your " + chestDesc() + " and crotch.  Your hands instinctively shovel a few loads into your " + assholeOrPussy(), false);
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" while the animal part of your brain hopes it makes you pregnant with an equally girthy son", false);
		MainScreen.text(".\n\n", false);
		//ADD PREG CHECK
		//Preggers chance!
		player.knockUp(PregnancyType.MINOTAUR, PregnancyType.INCUBATION_MINOTAUR, 70);

		MainScreen.text("Giggling, you stagger over to the next cock in line and turn around, possessed with the idea of taking its spooge in the most direct way possible – anally.   You pull your butt-cheeks apart and lean back, surprising one of the horny beasts with the warmth of your " + ButtDescriptor.describeButthole(player) + " as you slowly relax, spreading over his flare.  He actually squirts ropes of something inside of you, but you've been around minotaurs enough to know that it can't be cum, at least not yet.  The slippery gouts of preseed make it nice and easy to rock back and spear yourself on the first few inches, ", false);
		if (player.analCapacity() < 80) {
			MainScreen.text("delighting in the drug-numbed pain of stretching yourself beyond your normal capacity.", false);
		}
		else if (player.analCapacity() < 140) MainScreen.text("delighting in the feeling of perfect fullness.", false);
		else MainScreen.text("delighting in realizing that you could take far larger than even this virile specimen!", false);
		//(buttchange here: 90)
		player.buttChange(90, true, false);

		MainScreen.text("\n\nYou slide down the twitching bull-shaft until your " + ButtDescriptor.describeButt(player) + " slaps the wall, and you draw slowly away, but you push back harder, turned into a lewd, wanting whore by the massive quantity of minotaur seed in your belly, on your skin, and fogging up the air.  The beast pulls out and you whine plaintively, feeling empty and useless until he plunges back inside and reminds you of your purpose.  He starts to fuck you hard, not caring for your pleasure at all, slamming his horse-cock deep and fast.  Each of his three rings of prepuce ", false);
		if (!player.lowerBody.cockSpot.hasCock()) MainScreen.text("drags through your body, touching sensitive nerves you didn't even know you had until you cum, shuddering and shaking like a wanton whore.", false);
		else MainScreen.text("presses on your prostate as it squeezes by, making " + CockDescriptor.describeMultiCockSimpleOne(player) + " drip and spurt freely until you can bear it no longer and cum, shaking and shuddering like a wanton whore.  Jizz drips and pours from " + CockDescriptor.describeMultiCockSimpleOne(player) + " in a steady stream that pools on the floor, slowly rolling towards a drain that doubtless empties into a tank or greedy goblin cunt.", false);
		MainScreen.text("\n\n", false);

		MainScreen.text("The strength goes completely out of your " + LowerBodyDescriptor.describeLegs(player) + ", but you manage to hold yourself up long enough for your stud to flex his cock inside you and fill up your backdoor with more potent addiction.  You slide off, nerveless and still orgasming as jizz rains on your back from the abandoned cock, rolling off you to add to the ever-widening puddle of fluids on the floor.  Smiling vacantly and addled on more minotaur-cum than any one person has reason to handle, you stumble up on all fours and crawl to the next one.  More...", false);
		//[NEXT]
		player.orgasm();
		player.stats.lib += 2;
		player.stats.sens += 2;
	...
	private minotaurSalonFollowUp(): void {
		...
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00142] == 0) {
			...
			//text goez here
			MainScreen.text("Lynnette slaps your face, waking you from your stupor.  What?  Where are you?  You look around and realize you're strapped into a barber's chair and caked with white goop, but why?\n\n", false);
			MainScreen.text("The goblin answers before you can vocalize your query, \"<i>Baby, you're so cum-hungry you make my daughters look like chaste virgins!  I haven't seen anyone go to town on minotaurs like that since my mother passed away, Marae guide her soul.  Normally we don't have that much use for the minotaurs outside of a bit of recreational drug-play, but you milked out so much cum that we can start selling it!  Isn't that great!?</i>\"\n\n", false);
			MainScreen.text("You nod, a little shocked by your behavior, but already licking your lips in anticipation.  Lynnette smiles and says, \"<i>You can buy some for the usual price, plus 60 gems, but for right now, how about some hair service?  It's on the house!</i>\"", false);
		}
		else MainScreen.text("You're woken up with a slap to the face, and still muzzy from your cum-induced glory-hole orgy, you stagger up to your " + LowerBodyDescriptor.describeFeet(player) + ".  Lynnette shakes her stinging palm and apologizes, \"<i>Sorry I have to keep doing that, but you're damned hard to wake after you go on these binges!  Anyways, you've earned some hair treatment if you want it.  Of course you could always buy more of your favorite fluid...</i>\"\n\n", false);
		//Menu
		MainScreen.text("Lynnette offers and explains their options, \"<i>So what'll it be hun?  We could cut it down or give you a lengthening treatment. Or you can get a hair-dye to use on your own.  Just remember to come back in a few days for a touchup.</i>\"", false);
		salonPurchaseMenu();
	}
	...
	private mudFacial(): void {
		spriteSelect(38);
		MainScreen.text("", true);
		MainScreen.text("You sit back in a comfortable chair and pull on a lever to recline it.  The goblins buzz around you, gathering up 'special mud'.  You close your eyes, letting them plaster your " + HeadDescriptor.describeFace(player) + " with the stuff in hopes that it will improve your complexion as much as you've been promised.  A pair of cucumber slices are laid out on your eyes, obscuring your view.\n\n", false);

		MainScreen.text("With that finished, the crowd of busty, green-skinned women disperses to leave you in peace.  Time drags on, but eventually the mud hardens and cracks.  As if on cue, tiny hands emerge with wet rags to scrub your face clean.  Once they've finished, you feel like a whole new you! (+10 femininity)", false);
	...
	private sandFacial(): void {
		spriteSelect(38);
		MainScreen.text("", true);
		MainScreen.text("You sit back in a comfortable chair and pull on a lever to recline it.  The goblins buzz around you, gathering up 'special sand'.  You close your eyes, letting them splatter your " + HeadDescriptor.describeFace(player) + " with the rough, textured goop.  It doesn't feel very good, but that won't matter if it makes you as handsome as it's supposed to.\n\n", false);

		MainScreen.text("After a while the goblin girls come back and clean the stuff from your face. (+10 masculinity)", false);
		player.modFem(0, 10);
		doNext(camp.returnToCampUseOneHour);
	}
	/*
	public static const LYNNETTE_PREGNANCY_CYCLE: number                                    = 1022; //0-3 = pregnant. 4-6 = not.
	public static const LYNNETTE_APPROVAL: number		                                     = 1023;
	public static const LYNNETTE_BABY_COUNT: number                                         = 1024;
	public static const LYNNETTE_CARRYING_COUNT: number                                     = 1025;
	*/
	...
	private fuckLynnette(): void {
		...
		if (Flags.list[FlagEnum.LYNNETTE_FUCK_COUNTER] == 0) {
			MainScreen.text("At your suggestion, Lynnette's eyelashes flutter dangerously low. She gives you a smokey look and asks, \"<i>Is that so?</i>\" She circles around you, looking you up and down with eyes that seem to bore right through your [armor]. She must see something she likes, because she dips forward, parting her weighty melons around your ");
			if (player.tallness >= 72) MainScreen.text("[leg]");
			else if (player.tallness >= 55) MainScreen.text("[hips]");
			else MainScreen.text("side");
			MainScreen.text(" so that she can slip a manicured hand inside your underclothes. She handles [eachCock] ");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("in turn ");
			MainScreen.text("with the skill of a practiced snake-charmer before ");
			if (player.lowerBody.balls == 0) MainScreen.text("pressing down near the [sheath]");
			else MainScreen.text("curling her petite fingers around your [sack] to heft it");
			MainScreen.text(".");
			//IF FAIL!
		...
		//Repeat cum within 24 hours check
			...
			MainScreen.text("Lynnette licks her lips at your decision and sashays forward, her immense hips rocking with enough sensuality that her plush ass is practically having a sexquake.  \"<i>You'll understand if we do a quick check then, right, [name]?</i>\" She doesn't wait for an answer, pressing herself against you so that you can get a good feel of her soft, sopping-wet tits conform to the shape of your body, letting the moist heat that seems to radiate from every pore of Lynnette's petite shape soak into your exposed " + player.skinFurScales() + ".  Her hand dives right into your underwear without asking, seizing tight around [oneCock] to heft it.");
			MainScreen.text("\n\nYou give up an appreciative groan, [eachCock] twitching in lewd enjoyment as ");
			if (player.stats.lust <= 75) MainScreen.text("it fills to full size almost immediately");
			else MainScreen.text("it's fully-erect state makes itself known");
			MainScreen.text(". Lynnette strokes a few times to make sure you're suitably engorged and then shifts lower, squeezing ");
			if (player.lowerBody.balls == 0) MainScreen.text("around your [sheath] to get a feel for how pent-up you are");
			else MainScreen.text("gently at your [balls], weighing them in her palm to get a feel for just how pent-up you are");
			MainScreen.text(".");
			//NOT ENOUGH!
			if (player.hoursSinceCum < 24) {
		...
		MainScreen.text(" with hunger in her eyes.");
		...
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("es have");
		else MainScreen.text(" has");
		MainScreen.text(" begun to bead clear droplets of precum. They run down the underside of your distended, eager cumvein");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(" and race down towards your ");
		if (player.lowerBody.balls > 0) MainScreen.text("[balls]");
		else MainScreen.text("[sheath]");
		MainScreen.text(", leaving a glossy, sexual shine in their wake, and they're only coming on faster and faster the longer you gaze upon Lynnette's emerald twat and inhale her prick-stiffening scent.");
		...
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(" won't take no for an answer.");
		...
		if (player.cockArea(x) > 50) {
			MainScreen.text("  Her tummy bulges with the outline of your swollen mass, clearly displaying the shape of your bitch-breaking dong as it travels up ");
			if (player.cockArea(x) <= 80) MainScreen.text("her belly.");
			else MainScreen.text("into her ribcage, penetrating farther than any lesser race could ever hope to handle.");
		}
		MainScreen.text(" The breeder grabs hold of your [hips] and tugs as hard as her meager strength will allow.  ");
		if (player.cockArea(x) <= 90) {
			MainScreen.text("Slapping against her hard enough to release a splash of girlish cum, you hilt yourself completely");
			if (player.lowerBody.balls > 0) MainScreen.text(", [balls] resting on her soaked, juicy ass-cheeks.");
		}
		else {
			MainScreen.text("Eventually stopping, you look down to realize that she hasn't taken all of you. Going any farther would likely hurt her and ruin the fun, so you'll have to make to with partial strokes.");
			if (player.lowerBody.balls > 0) MainScreen.text("  Her feet wrap around your [sack] to squeeze your [balls] affectionately, compensating quite nicely for her own shortcomings.");
		}

		MainScreen.text("\n\nLynette is so goddamned wet! Her twat is a sopping-wet furnace around your " + CockDescriptor.describeCock(player, x) + ", clenching down tightly to hold you still while she adjusts to the shape and size of hard-throbbing inseminator.  A happy, brainless smile spreads across her face in reaction to the mounting, though her hands remain stubbornly on your hips, helping to keep you from pounding away until she's ready.  While you're immobilized, you decide to avail yourself of the goblin's other features, gripping as much of one titanic tit as you possibility can.  Your hand barely covers a quarter of the swollen bosom, and your fingers sink deeply into the forgiving green flesh, eliciting a gasp of pleasure from the hairdresser when you shift your grope to place her nipple within your reach.");
		MainScreen.text("\n\nLosing control of her abdominal muscles from the forcible, nipple-bound bliss, Lynnette's cock-arresting grip collapses, and you're free to saw away at her gushing nethers, dragging your " + CockDescriptor.describeCock(player, x) + " out until only the head remains embedded within her purple-tinged interior and then, slamming it back in just as hard as your initial penetration.  Her hands slip off your [hips] and down to the sheets, where they gather up fistfuls of the increasingly sex-stained fabric and clench.  The honey hole around you flutters uncontrollably, clenching wildly as you thrust powerfully through it, mixing your pre-cum with its own copious leavings until there's a whitish slurry leaking from the increasingly puffy entrance.");
		...
		if (player.lowerBody.balls > 0) MainScreen.text(", making your sack feel tight and your testes bloated and tender.");
		else MainScreen.text(", making your gut clench and spasm with near-orgasmic contractions.");
		MainScreen.text("  There's a tsunami of sperm building up inside you to the point where holding it in is actually starting to hurt you, but at the same time, your body refuses to give in and climax yet either.");
		...
		if (player.cockArea(x) > 24) MainScreen.text("up against her slightly-yielding cervix");
		else MainScreen.text("in as close to the cervix as you possibly can");
		MainScreen.text(", grunting and growling as you lose yourself in the feel of her soaked, velvet tunnel.  Your focus collapses entirely down to the feel of that slick tunnel around you and how far you can push into it, how strongly you can make it climax and squeeze around you.");
		...
		MainScreen.text("\n\nYour body has been ready to go off for some time now, and the words act like an irresistible trigger for your climax, causing you to lurch your " + CockDescriptor.describeCock(player, x) + " ");
		if (player.cockArea(x) <= 90) MainScreen.text("completely inside once more");
		else MainScreen.text("as deeply inside as you can");
		MainScreen.text(" and hold it there.");
		if (player.lowerBody.cockSpot.count() > 1) {
			MainScreen.text("  Your other member");
			if (player.lowerBody.cockSpot.count() > 2) MainScreen.text("s are");
			else MainScreen.text(" is");
			MainScreen.text(" spasming ");
			if (player.longestCockLength() <= 12) MainScreen.text("on the greedy green slut's belly");
			else MainScreen.text("in the milk-stained valley between her tits, absolutely buried in squishy-soft breast");
			MainScreen.text(", jizzing in wild spurts.  The cum mixes with all the goblin-cream into a sticky-slick morass that paints Lynnette white.");
		}
		MainScreen.text("  Screaming with relief, you can feel the cum rushing out from your [balls], through your middle, and down the length of your quim-buried tool, distending it as blobs of cum roll through the honeyed folds to explode against the well-endowed goblin's cervix. The touch of cum at the womb's entrance sends your lover back into another body-shaking climax; her eyes roll back, her pussy clamps down, and most importantly, her cervix dilates into a wide-open hole, sucking the cum straight into her womb as fast as you can shoot it.");
		...
		else MainScreen.text("\n\nOh gods!  Every torrential outflow of cum is so substantial that it's literally stretching the poor goblin out as it rushes into her womb.  The first two blasts are enough to turn her once-taut middle into a sperm-stuffed dome, and you just keep shooting after that, each time making her gravid tummy jiggle and swell, rounding fuller and fuller until her bulging midsection seems almost as big as her.  Her belly button long ago popped out into an outtie, and the smooth, emerald skin shines with its overstretched tightness.  Lynnette whimpers softly, and a sudden torrent of spunk rolls out around your " + CockDescriptor.describeCock(player, x) + ".  It seems she just couldn't hold it all in, and you aren't even done cumming yet!  You resume sliding in and out as you jizz, washing your soon-to-be-pregnant fucktoy's tunnel with generous waves of spooge until you finally exhaust yourself.");
		MainScreen.text("\n\nLynnette holds you tight until she's absolutely sure you've spent every drop");
		if (player.cumQ() >= 3000) {
			MainScreen.text(", even if ");
			if (player.cumQ() >= 10000) MainScreen.text("most");
			else MainScreen.text("some");
			MainScreen.text(" of it is on the bed at this point");
		}
		MainScreen.text(".  Only then does she release you, allowing you to slide out");
		if (player.hasKnot(x)) MainScreen.text(" with a wet-sounding 'pop'.  Your knot does a terrific job of gaping her lips during its exit, and you cannot help but marvel at how well you've broken this bitch in.  A shame she'll probably tighten up again in short order");
		else MainScreen.text(" with a gush of girlspunk to chase you");
		MainScreen.text(".  She groans, ");
		//BELOW ONE LITER!
		if (player.cumQ() < 1000) {
			MainScreen.text("\"<i>[name], that was fun, but you barely even came!  How am I supposed to birth a dozen daughters at once if you barely coat the surface of my womb!?</i>\"  She shudders.  \"<i>Go on. I've got to find some drugs to fix this, else I won't get a decent brood out of this term.</i>\"\n\nYou leave her feeling a little underappreciated.  Maybe you could find some cum-boosting supplements out there?  Still, you are owed a haircut...");
			lynnetteApproval(-2);
		}
		//"GOOD"
		else if (player.cumQ() <= 10000) {
			if (Flags.list[FlagEnum.LYNNETTE_ANNOUNCED_APPROVAL] == 1) MainScreen.text("\"<i>Wow, [name].  You cum like a minotaur... and without drugging me out of my mind to boot!  Gods, I can feel the little swimmers hunting down my eggs already.</i>\"  She giggles and retrieves a vial, downing it in a single gulp.  \"<i>I better make sure there's enough eggs for them all, huh?</i>\"\n\nYou leave with a happy smile. It seems Lynnette approves of you as a mate.");
			//"GOOD + LYNNETTE FAVORITE"
			else MainScreen.text("\"<i>Mmmm, [name], I love the way you pack me full.</i>\"  She tosses back a vial of fertility-boosting chemicals and sighs.  \"<i>Do me a favor and come back soon.  I'd rather be pouring out swarms of your babies than a slurry of weakling sluts.</i>\"");
			lynnetteApproval(10);
		}
		//"UBUR"
		else {
			if (Flags.list[FlagEnum.LYNNETTE_ANNOUNCED_APPROVAL] == 0) MainScreen.text("\"<i>Ohhh... oh gods....  W-wha?  How...?  I'm so fucking full, [name]!  I can feel it, like I'm pregnant and about to pop already, and I haven't even given my babies a chance to fertilize!</i>\"  She gathers some of your leaking spooge to smear across the titanic dome with one hand and a bottle of fertility enhancers with the other.  \"<i>Go on baby, I'm just going to soak in it for a little while and make sure I get as many girls out of this as I can.</i>\"  Lynnette knocks back the vial and drops it at her side, focusing entirely on wallowing in your cum.\n\nYou swagger out into the main room with a happy smile, noting the envious looks you get from her daughters when they try to offer you 'services'.");
			//"UBUR + LYNNETTE FAVORITE"
			else MainScreen.text("\"<i>Fuck me, honey! I... you're the only one that can fill me like this, you know?  Short of a machine, I mean.</i>\"  Lynnette colors a deep green as she struggles to reach for a bottle of a fertility-enhancing chemical, half-pinned by her own cum-stuffed weight.  You easily hand it to her with a smile, and she downs it before gazing gratefully your way.  \"<i>I fucking love just lying here, fucking into oblivion, lying in a lake of my stud's lusts, wallowing in it until I pass out from pleasure.</i>\"\n\nShe sounds almost lovey-dovey in her declaration and must realize it, because her tone changes when she says, \"<i>Just make sure you come back in a few days to give me another proper fucking.  You'll do it if you know what's good for your dick!</i>\"\n\nYou silence her with a kiss that leaves her swooning and stroll out, dressing on the way.  Lynnette's satiated moans seem to haunt you the entire time you remain in the salon.");
			lynnetteApproval(25);
		}
		Flags.list[FlagEnum.LYNNETTE_FUCK_COUNTER]++;
		player.orgasm();
		Flags.list[FlagEnum.LYNNETTE_CARRYING_COUNT] = 3 + rand(3);
		if (player.cumQ() >= 1000) Flags.list[FlagEnum.LYNNETTE_CARRYING_COUNT] += 1 + rand(3);
		if (player.cumQ() >= 2000) Flags.list[FlagEnum.LYNNETTE_CARRYING_COUNT] += 1 + rand(3);
		if (player.cumQ() >= 3000) Flags.list[FlagEnum.LYNNETTE_CARRYING_COUNT] += 2 + rand(3);
		if (player.cumQ() >= 4000) Flags.list[FlagEnum.LYNNETTE_CARRYING_COUNT] += 2 + rand(3);
		if (player.cumQ() >= 6000) Flags.list[FlagEnum.LYNNETTE_CARRYING_COUNT] += 2 + rand(3);
		if (player.cumQ() >= 10000) Flags.list[FlagEnum.LYNNETTE_CARRYING_COUNT] += 2 + rand(3);
