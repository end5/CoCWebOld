...
private salonFavoritesPaymentMenu():void {
	...
	menu();
	if(flags[FlagEnum.LYNNETTE_PREGNANCY_CYCLE] >= 4 && player.lowerBody.cockSpot.hasCock()) addButton(5,"Fuck Goblin",fuckLynnette);
	...
	//choices("Goblin Blow",blow,"Canine",gloryholeDoggie,"Imp",gloryholeImp,"Minotaur",gloryholeMinotaur,"Incubus",gloryholeIncubus,"",0,"",0,"",0,"Buy MinoCum",minoCum,"Leave",13);
		...
		private buyMinoCum():void{
			...
			else
			{
				...
				Render.text("You happily give Lynnette 60 gems and pick up the bottle full of glistening, heavenly cum.  ", true);
				statScreenRefresh();
...
private hairDresserGreeting():void {
	...
	Render.text("   \"<i>I apologize for my daughters,</i>\" she says as she presses herself against you.  \"<i>They're a bunch of brainless hussies for the most part.  My name is Lynnette, and welcome to my salon!  You want your hair cut or lengthened?  We've got you covered, and we don't ask for much - just a shot of cum.", false);
	if(player.lowerBody.cockSpot.count() == 0) {
		Render.text("  You look like you don't got any of your own, but we've got glory holes in the back if you need to get some.  Just don't swallow too much, ok?</i>\"\n\nShe shows you to the back of the cave, which is boarded-up.  There are about 20 holes in boards, and most are empty.  While you watch, a few new dicks slide in, and just as quickly the goblin's daughters commence sucking and fucking them.  There are only a few you could take a crack at - do you blow one (and if so which one)?", false);
	}
	else {
		Render.text("  I'll even do you the favor of letting you blow it in my mouth, I've already got a bun in the oven.  So what do you say?  Want a spooge and a haircut?  Or would you rather go get your payment from one of the gloryholes in the back, you " + player.mf("kinky boy","naughty girl") + "?</i>\"\n\n", false);
		Render.text("In the very back of the salon you can see a boarded-up wall with holes cut in it, some of which are currently plugged by various monstrous penises.  Do you let the goblin blow you, or do you go suck your payment from one of them?", false);
	}
	salonPaymentMenu();
...
private hairDresserRepeatGreeting():void {
	...
	if(flags[FlagEnum.LYNNETTE_PREGNANCY_CYCLE] == 4 && flags[FlagEnum.LYNNETTE_CARRYING_COUNT] > 0) {
		Render.text("As soon as you enter the Salon, Lynnette is beaming ");
		if(player.tallness >= 48) Render.text("up ");
		Render.text("at you. \"<i>Hey there, honey. Come for some more service or to check up on the kids?</i>\" She pats her now-flat midsection. \"<i>I popped them out last night. We had " + num2Text(flags[FlagEnum.LYNNETTE_CARRYING_COUNT]) + " kids... all girls, of course.</i>\" She tucks a lock of beautifully styled hair behind an elfin ear. \"<i>Now you be sure and let me know when you want to contribute to my baby-bank again.</i>\"");
		if(flags[FlagEnum.LYNNETTE_BABY_COUNT] > 0) Render.text("\n\nA few new, green faces can be seen looking your way and blushing as they work the gloryholes, probably from a previous litter. They grow up so fast.");
	...
	else if(flags[FlagEnum.LYNNETTE_PREGNANCY_CYCLE] >= 4) 
	{
		if(flags[FlagEnum.LYNNETTE_MET_UNPREGNANT] == 0) {
			...
			Render.text("Lynnette's familiar face greets you at the door to her salon once more, and she waves you in with a beatific smile plastered upon her face. Clinging to her like a second skin, the businessgoblin's dress highlights her surprisingly flat midsection; she's not pregnant!");
	...
	//Standard repeats
		...
		Render.text("Lynnette the goblin answers the door and lets you in, waving you deeper into her shop.  Her shining black dress barely contains her fertile-hips and jiggling chest as she greets you, \"<i>Welcome back, honey!  ", true);
		if(player.lowerBody.cockSpot.count() == 0) {
			Render.text("I see you still can't make payment for our services on your own.  No worries, the glory holes in the back are pretty popular with the demons and monsters, just go back there and catch some spooge and we'll see about helping with your hair, ok?</i>\"\n\n(There are a number of dicks in the glory hole, which do you want to deal with (if you're willing at all)?", false);
		}
		else {
			Render.text("Will you be getting our normal services?  Just one BJ per hair treatment!  Or would you rather get it from the gloryholes in back?</i>\"", false);
		}
	}
	salonPaymentMenu();
}
...
private gloryholeDoggie():void {
	...
	if(player.stats.cor < 33) {
		Render.text("It's a struggle to force your lips apart and take the strange thing in your mouth.  You feel like a freak, but make yourself get to work servicing the male who'll be covering your hair treatment today.  It tastes salty and sweaty and has a potent musky scent that excites you in spite of your better judgment.  You find yourself starting to get into it as you lick and slurp, humming with pleasure.  You reach up to touch the knot and the dog-cock jumps in your mouth, spurting a wad of thick cum into your throat.  You pull back most of the way and pump the cock, collecting the jism in your mouth.\n\n", false);
		dynStats("lus", 15, "cor", .25);
	}
	else if (player.stats.cor < 66) {
		Render.text("You open wide and take as much of the pointed dick as you can into your mouth.  A few times you nearly gag yourself, but you manage to get most of the way down to the thick swollen knot.  You start sliding back and forth, breathing in the musky animalistic scent of your 'client' and become aroused by the situation more and more.  You reach up and caress the knot, squeezing gently and stroking the dick until it unloads a thick wad of cum into your mouth.  You pull back and catch the fruits of your labor in your mouth, milking the dick with your hands until it's finished and the dick pops back through the hole.\n\n", false);
		dynStats("lus", 25, "cor", .25);
	}
	else {
		Render.text("You throw yourself onto the swollen dog-cock and easily engulf the whole thing, spreading your lips wide around the thick knot.  You can feel it bouncing the uvula in your throat around but you manage to supress your gag reflex like a good slut while you work the sweaty dick, giving it a thorough tongue bath.  You wrap your lips around the base of the knot, feeling it start to stretch your jaws apart.  You pull back before it can lock you like that, and squeeze the knot in your hands.  It cums, unloading thick wads of gooey cum into your mouth.\n\n", false);
...
private gloryholeMinotaur(): void {
	...
	Render.text("Driven on by the minotaur's reactions, you take more and more of the minotaur cock into your mouth. As you work your mouth around the first ring, you open up the top section of your " + player.armorName + ", freeing up your " + allBreastsDescript() + ". With each pass, you're able to take more and more of the bull dick down your throat. You caress your " + biggestBreastSizeDescript() + " while moaning around the dick. When you get your mouth around the second ring, the minotaur's cock throbs sharply, and his cum explodes in your mouth. Your eyes bulge in shock, and you pull off the cock as quickly as you can. You swallow the first several shots of thick, rich minotaur cum, while the rest shoots all over your face and tits. You ");
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) Render.text("hold up ");
	else Render.text("arch your back to expose ");
	Render.text("your [chest] as cum rains down upon you. By the time his orgasm stops and his dick goes limp, your face and [chest] are covered in a thick layer of spooge.\n\n", false);
	Render.text("Your head spins from the minotaur's musk, and you idly mop up and swallow the cum on your " + player.face() + ". A goblin aide comes in with a bowl, and gently scrapes the cum off your tits with a smooth, flat rock. Once you're cleaned up and you're dressed, the aide leads you back to Lynnette.\n\n", false);
...
private goblinHairDresserFacefuck():void {
	...
	else Render.text("around your first few inches", false);
	Render.text(".  She slips her tongue down your urethra, and arches her back, popping the straps on her dress and letting her oversized tits engulf your shaft.  Milk sputters and dribbles from the lactating goblin, lubricating her breasts and hands as she pleasures every inch of you.\n\n", false);
	Render.text("It's impossible to hold back while receiving such expertly given stimulation, and you blow your load, bulging her cheeks out like a squirrel's.", false);
	if(player.cumQ() >= 250) Render.text(" She can't hold it all and suddenly starts swallowing as runnels of it escape her nose and a wave of it rolls down her lips.", false);
	Render.text("  In time it ends, and she pops back, spitting most of the cum into a funnel.  It washes down a pipe and you have to wonder if there's some horny goblin girl at the other end with her cunt spread wide.", false);
...
private cutShort():void {
	...
	if(player.upperBody.head.hairType == 4) {
		Render.text("Lynnette stares at you when you ask for a cut.  \"<i>Nothing doing, hon; that stuff looks alive and I don't want blood all over my nice floor.  Thanks for contributing to the white file, though; maybe we can do something else?</i>\"\n\n", false);
		salonPurchaseMenu();
		return;
	}
	Render.text("Lynnette and her daughters crowd around you with razor-sharp scissors, effortlessly paring down your " + hairDescript() + ".  When they've finished, you're left with ", true);
	player.hairLength = 1;
...
private cutLong():void {
	...
	//-trying to get a goblin to cut tentacle hair:
	if(player.upperBody.head.hairType == 4) {
		Render.text("Lynnette stares at you when you ask for a cut.  \"<i>Nothing doing, hon; that stuff looks alive and I don't want blood all over my nice floor.  Thanks for the contributing to the white file, though; maybe we can do something else?</i>\"\n\n", false);
		salonPurchaseMenu();
		return;
	}
	Render.text("Lynnette and her daughters crowd around you with razor-sharp scissors, effortlessly paring down your " + hairDescript() + ".  When they've finished, you're left with ", true);
...
private hairGrow():void {
	...
	if(player.upperBody.head.hairType == 4) {
		Render.text("Lynnette looks dubiously at you when you ask for a lengthening treatment.  \"<i>No offense hon, but that stuff is basically like an arm or an organ, not hair.  I'm not a goblin chirurgeon, and I wouldn't try to lengthen it even if one of my disobedient daughters were here to donate some parts.  Sorry to make you shoot and scoot, but I can't help you.  Maybe we could do something else?</i>\"\n\n", false);
		salonPurchaseMenu();
		return;
	}
	Render.text("Lynnette grabs a bottle and squirts a white fluid into your hair.  You really hope it isn't your payment.  But it must not be, as within short order you feel the added weight of ", true);
	temp = rand(3) + 3;
...
private minotaurCumBukkakeInSalon():void {
	...
	player.minoCumAddiction(10);
	player.slimeFeed();
	Render.text("As the salon door swings closed behind you, a familiar, heavenly scent catches your nose and wicks into your brain, flooding you with need and molten-hot lust.  Lynnette saunters over with her lips slightly pursed and her body jiggling, but you brush her aside.  She's completely forgotten as you close in on the source of your olfactory bliss.  Your sigh dreamily while your pupils slowly dilate from the familiar chemicals pounding through your bloodstream", false);
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and puffing up your twat with liquid arousal.\n\n", false);
	else if(player.lowerBody.cockSpot.hasCock()) Render.text(" and turning " + sMultiCockDesc() + " into a turgid, pulsating mass.\n\n", false);
	else Render.text("\n\n", false);
	
	Render.text("You push your way past a few more staring goblin sluts, tearing off your " + player.armorName + " as you go.  The animal part of your brain recognizes that such needless trapping would just get in the way of all the thick, dripping, minotaur spunk just waiting to pump inside you.  Another door swings closed behind you while the scent drags you heedlessly deeper into the salon, closing in on the secluded glory-holes the goblins keep hidden away in the back.  You drool spittle down your neck while you lose yourself in the memory of that taste on your tongue, letting your body seek it out on autopilot.", false);
	if(player.lowerBody.vaginaSpot.hasVagina() || player.lowerBody.cockSpot.hasCock()) {
		Render.text("  A trail of ", false);
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			Render.text("female slime ", false);
			if(player.lowerBody.cockSpot.hasCock()) Render.text("and ", false);
		}
		if(player.lowerBody.cockSpot.hasCock()) {
			if(player.cumQ() < 100) Render.text("pre-cum ", false);
			else if(player.cumQ() < 500) Render.text("pre-cum a few inches wide ", false);
			else Render.text("pre-cum over a foot wide ", false);
		}
		Render.text("winds over the rough-hewn floor behind you, clearly marking your passage to the overpowering musk.", false);
	}
	Render.text("\n\n", false);
	
	Render.text("Time passes in a slow, torturous manner, but you turn the corner and sigh.  Salvation is at hand – you've reached the glory-holes, and to your delight there's nearly a dozen hard, dripping, delicious... mmmm... minotaur cocks crammed through the holes, some bulging out around the seams and leaking with need.  You saunter forwards, hips swaying sensually as the drug-like desire of potent minotaur musk pulls you ever closer.  Mewling happily, you take one of them in hand and stroke along its length, giggling as it pulses and leaks a stream of heavenly goo down your arm.   You lick it from your arm in one long, languid motion before pursing your lips around the minotaur's flared tip and sucking it hard as you quest for more of its heady ambrosia.\n\n", false); 
	
	Render.text("You hear a deep, strangled sigh as more and more delicious pre floods your mouth, lighting your senses up with a fireworks show of pleasure", false);
	if(player.gender > 0) Render.text(" and increasing the size of the puddle you're leaving on the floor", false);
	Render.text(".  In between gulps of the fragrant fluid, you suck the swollen rod deeper and deeper into your mouth, gleefully suppressing your gag reflex as it pushes aside your uvula.  You work your throat, feverishly swallowing to suck it down.  A flare briefly distends your neck as it's taken deeper and deeper inside your form until your lips are puckered through the hole and you're sniffing the minotaur's pheromone-laced crotch.\n\n", false);
	
	Render.text("The beast pulls back and starts pounding the hole, letting out deep, rumbling sighs of pleasure with each throat abusing glory-hole fuck.  If it weren't for his constant, bubbling preseed turning your throat to a slippery rut-hole you'd be rubbed raw, but instead you continue to lean forward, sniffing at his matted pubes every time he crushes them against your dick-stretched lips.  The other horny studs grunt with displeasure, but you reach out with your hands and grab hold two eager brutes' throbbing horsecocks.  Three out-of-sync heartbeats hammer through your hands and throat while you do your best to get them all off.\n\n", false); 
	
	Render.text("You let your eyes drift closed and lash your tongue back and forth across the minotaur's lowest medial ring, feeling his flare bulge larger inside you in response.  The beast-man batters at the wall, grunting in equal measures of passion and pent-up desire as his cock's tip stretches wide and locks itself inside you.  His urethra bulges in rhythmic contractions, stretching your lips around the already swollen shaft and signaling to your musk-addled mind that you're about to get what you desire.  You hum with blissful delight as your belly begins to gurgle, accepting long bursts of sticky minotaur drug.\n\n", false);
	
	Render.text("Tied down by the cum-spurting flare locked in your gullet, you pump on the other two dicks with feverish speed and sway on your " + player.feet() + " as the narcotic spooge intoxicates your already-addled mind.  You can feel each muscular contraction pulsing through the bestial shaft while it finishes depositing the heavy, sticky load, and your eyes cross from the viscous inebriation that's pooling in your belly.  Drizzles of pre-cum soak into your arms and palms, drawn out from the frenzied pumping of your fists.  They won't come from just a hand though.  They need something... tighter.\n\n", false);
	
	Render.text("You start moaning in drug-induced bliss, but your vocalized pleasure is interrupted by the squelching slurp of the softening shaft being pulled from your dick-puckered lips.  It drips a rope of cum over your mouth and chin as it pulls free from the wall, leaving behind one vacancy among the swarm of ready minotaur dicks.  You lick up your stud's leavings and purr in bliss, reaching through the hole to cup the departing minotaur's balls teasingly.  He grunts and walks out of your grip – sated for now.  You curl your fingers into a come-hither gesture, retract your arm through the sweat-lubricated glory-hole, and wait for one of the other minotaurs to come over.\n\n", false);
	
	Render.text("The beast who kept your left hand so busy repositions himself at the now-vacant opening, and you decide to reward him for moving so quickly.  You lick the last of the salty cream from your lips and muse that he isn't the only one getting a reward, but the monstrous cow-man doesn't need to know that.", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) {
		Render.text("  You wrap the pillowy flesh of your " + allBreastsDescript() + " around the new member, pleasantly surprised by its girth and wide, already-flared tip.  Maybe you could have gotten him off with your hands after all?  ", false);
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 6) Render.text("Even so, you can't quite get your breasts the entire way around him, so you make up for it by pressing it harder into you with your busy hands.  ", false);
		Render.text("Runnels of pre cover the shaft and squish wetly between your tits, turning your body into a slip-n-slide that reeks of hot, sticky minotaur sex.", false);
	}
	else {
		Render.text("You crush the member tightly against your chest, smearing your torso with the copious pre until you're like a hot, wet slip-n-slide of minotaur sex.  You make up for it by wrapping your arms around his prodigious thickness and squeeze him tightly with your hands, stroking him off with your whole body with enough tightness to make the poor mino' think he's in some poor adventurer's asshole.", false);
	}
	Render.text("\n\n", false);
	
	Render.text("You lower and raise yourself, bouncing up and down on your " + player.legs() + " to enhance the ", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) Render.text("tit-fucking ", false);
	else Render.text("full-body handjob ", false);
	Render.text("you're giving out.  In such a position, you're given the perfect view to watch as your strokes draw forth large bubbles of pre, and before you can lose your high, you latch onto the minotaur's vulnerable urethra and suck, tonguing in wide circles around it since you can't open wide enough to accommodate his flare.  Of course, all the attention just makes him flare wider, not just at the tip, but through the whole shaft.  ", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) Render.text("The sudden girth change sends an enticing ripple through your " + allBreastsDescript() + " that's pleasurable enough to make you moan into his steadily-widening urethra.  ", false);
	Render.text("You pull off and bounce faster, lost to your lust and the haze of sex-musk permeating the air, intent on seeing just how much this huge stud can spray onto you.\n\n", false);

	Render.text("The minotaur does not disappoint.  His hole dilates from the size of the approaching cum-blast, and you sink down his shaft slowly until it's aimed directly at your face.  You close your eyes and feel the first explosion splatter over your " + hairDescript() + " and forehead.  The next takes you full in the face, making it difficult to breathe through the mask of drug-like goo, but a few quick licks gives you a fix and makes it easy to breathe again.  On and on, the minotaur pumps fat ropes of spooge over your body until you're a syrupy, sticky mess that reeks of minotaur pheromones so strongly that dizziness overwhelms you and you fall free of the still-orgasming mino-cock, taking a few final blasts of seed on your " + chestDesc() + " and crotch.  Your hands instinctively shovel a few loads into your " + assholeOrPussy(), false);
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(" while the animal part of your brain hopes it makes you pregnant with an equally girthy son", false);
	Render.text(".\n\n", false);
	//ADD PREG CHECK
	//Preggers chance!
	player.knockUp(PregnancyType.MINOTAUR, PregnancyType.INCUBATION_MINOTAUR, 70);

	Render.text("Giggling, you stagger over to the next cock in line and turn around, possessed with the idea of taking its spooge in the most direct way possible – anally.   You pull your butt-cheeks apart and lean back, surprising one of the horny beasts with the warmth of your " + assholeDescript() + " as you slowly relax, spreading over his flare.  He actually squirts ropes of something inside of you, but you've been around minotaurs enough to know that it can't be cum, at least not yet.  The slippery gouts of preseed make it nice and easy to rock back and spear yourself on the first few inches, ", false);
	if(player.analCapacity() < 80) {
		Render.text("delighting in the drug-numbed pain of stretching yourself beyond your normal capacity.", false);
	}
	else if(player.analCapacity() < 140) Render.text("delighting in the feeling of perfect fullness.", false);
	else Render.text("delighting in realizing that you could take far larger than even this virile specimen!", false);
	//(buttchange here: 90)
	...
	if(!player.lowerBody.cockSpot.hasCock()) Render.text("drags through your body, touching sensitive nerves you didn't even know you had until you cum, shuddering and shaking like a wanton whore.", false);
	else Render.text("presses on your prostate as it squeezes by, making " + sMultiCockDesc() + " drip and spurt freely until you can bear it no longer and cum, shaking and shuddering like a wanton whore.  Jizz drips and pours from " + sMultiCockDesc() + " in a steady stream that pools on the floor, slowly rolling towards a drain that doubtless empties into a tank or greedy goblin cunt.", false);
	Render.text("\n\n", false);
	
	Render.text("The strength goes completely out of your " + player.legs() + ", but you manage to hold yourself up long enough for your stud to flex his cock inside you and fill up your backdoor with more potent addiction.  You slide off, nerveless and still orgasming as jizz rains on your back from the abandoned cock, rolling off you to add to the ever-widening puddle of fluids on the floor.  Smiling vacantly and addled on more minotaur-cum than any one person has reason to handle, you stumble up on all fours and crawl to the next one.  More...", false);
	//[NEXT]
	player.orgasm();
...
private minotaurSalonFollowUp():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00142] == 0) {
		...
		//text goez here
		Render.text("Lynnette slaps your face, waking you from your stupor.  What?  Where are you?  You look around and realize you're strapped into a barber's chair and caked with white goop, but why?\n\n", false);
		Render.text("The goblin answers before you can vocalize your query, \"<i>Baby, you're so cum-hungry you make my daughters look like chaste virgins!  I haven't seen anyone go to town on minotaurs like that since my mother passed away, Marae guide her soul.  Normally we don't have that much use for the minotaurs outside of a bit of recreational drug-play, but you milked out so much cum that we can start selling it!  Isn't that great!?</i>\"\n\n", false);
		Render.text("You nod, a little shocked by your behavior, but already licking your lips in anticipation.  Lynnette smiles and says, \"<i>You can buy some for the usual price, plus 60 gems, but for right now, how about some hair service?  It's on the house!</i>\"", false);
	}
	else Render.text("You're woken up with a slap to the face, and still muzzy from your cum-induced glory-hole orgy, you stagger up to your " + player.feet() + ".  Lynnette shakes her stinging palm and apologizes, \"<i>Sorry I have to keep doing that, but you're damned hard to wake after you go on these binges!  Anyways, you've earned some hair treatment if you want it.  Of course you could always buy more of your favorite fluid...</i>\"\n\n", false);
	//Menu
	Render.text("Lynnette offers and explains their options, \"<i>So what'll it be hun?  We could cut it down or give you a lengthening treatment. Or you can get a hair-dye to use on your own.  Just remember to come back in a few days for a touchup.</i>\"", false);
	salonPurchaseMenu();
}
...
private mudFacial():void {
	...
	Render.text("With that finished, the crowd of busty, green-skinned women disperses to leave you in peace.  Time drags on, but eventually the mud hardens and cracks.  As if on cue, tiny hands emerge with wet rags to scrub your face clean.  Once they've finished, you feel like a whole new you! (+10 femininity)", false);
...
private sandFacial():void {
	...
	Render.text("After a while the goblin girls come back and clean the stuff from your face. (+10 masculinity)", false);
	player.modFem(0,10);
	doNext(camp.returnToCampUseOneHour);
}
/*
...
private fuckLynnette():void {
	...
	if(flags[FlagEnum.LYNNETTE_FUCK_COUNTER] == 0) {
		Render.text("At your suggestion, Lynnette's eyelashes flutter dangerously low. She gives you a smokey look and asks, \"<i>Is that so?</i>\" She circles around you, looking you up and down with eyes that seem to bore right through your [armor]. She must see something she likes, because she dips forward, parting her weighty melons around your ");
		if(player.tallness >= 72) Render.text("[leg]");
		else if(player.tallness >= 55) Render.text("[hips]");
		else Render.text("side");
		Render.text(" so that she can slip a manicured hand inside your underclothes. She handles [eachCock] ");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("in turn ");
		Render.text("with the skill of a practiced snake-charmer before ");
		if(player.lowerBody.balls == 0) Render.text("pressing down near the [sheath]");
		else Render.text("curling her petite fingers around your [sack] to heft it");
		Render.text(".");
		//IF FAIL!
	...
	//Repeat cum within 24 hours check
		...
		Render.text("Lynnette licks her lips at your decision and sashays forward, her immense hips rocking with enough sensuality that her plush ass is practically having a sexquake.  \"<i>You'll understand if we do a quick check then, right, [name]?</i>\" She doesn't wait for an answer, pressing herself against you so that you can get a good feel of her soft, sopping-wet tits conform to the shape of your body, letting the moist heat that seems to radiate from every pore of Lynnette's petite shape soak into your exposed " + player.skinFurScales() + ".  Her hand dives right into your underwear without asking, seizing tight around [oneCock] to heft it.");
		Render.text("\n\nYou give up an appreciative groan, [eachCock] twitching in lewd enjoyment as ");
		if(player.lust <= 75) Render.text("it fills to full size almost immediately");
		else Render.text("it's fully-erect state makes itself known");
		Render.text(". Lynnette strokes a few times to make sure you're suitably engorged and then shifts lower, squeezing ");
		if(player.lowerBody.balls == 0) Render.text("around your [sheath] to get a feel for how pent-up you are");
		else Render.text("gently at your [balls], weighing them in her palm to get a feel for just how pent-up you are");
		Render.text(".");
		//NOT ENOUGH!
		if(player.hoursSinceCum < 24) 
		{
	...
	Render.text(" with hunger in her eyes.");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("es have");
	else Render.text(" has");
	Render.text(" begun to bead clear droplets of precum. They run down the underside of your distended, eager cumvein");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" and race down towards your ");
	if(player.lowerBody.balls > 0) Render.text("[balls]");
	else Render.text("[sheath]");
	Render.text(", leaving a glossy, sexual shine in their wake, and they're only coming on faster and faster the longer you gaze upon Lynnette's emerald twat and inhale her prick-stiffening scent.");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" won't take no for an answer.");
	...
	{
		Render.text("  Her tummy bulges with the outline of your swollen mass, clearly displaying the shape of your bitch-breaking dong as it travels up ");
		if(player.cockArea(x) <= 80) Render.text("her belly.");
		else Render.text("into her ribcage, penetrating farther than any lesser race could ever hope to handle.");
	}
	Render.text(" The breeder grabs hold of your [hips] and tugs as hard as her meager strength will allow.  ");
	if(player.cockArea(x) <= 90) {
		Render.text("Slapping against her hard enough to release a splash of girlish cum, you hilt yourself completely");
		if(player.lowerBody.balls > 0) Render.text(", [balls] resting on her soaked, juicy ass-cheeks.");
	}
	else {
		Render.text("Eventually stopping, you look down to realize that she hasn't taken all of you. Going any farther would likely hurt her and ruin the fun, so you'll have to make to with partial strokes.");
		if(player.lowerBody.balls > 0) Render.text("  Her feet wrap around your [sack] to squeeze your [balls] affectionately, compensating quite nicely for her own shortcomings.");
	}
	...
	if(player.lowerBody.balls > 0) Render.text(", making your sack feel tight and your testes bloated and tender.");
	else Render.text(", making your gut clench and spasm with near-orgasmic contractions.");
	Render.text("  There's a tsunami of sperm building up inside you to the point where holding it in is actually starting to hurt you, but at the same time, your body refuses to give in and climax yet either.");
	...
	if(player.cockArea(x) > 24) Render.text("up against her slightly-yielding cervix");
	else Render.text("in as close to the cervix as you possibly can");
	Render.text(", grunting and growling as you lose yourself in the feel of her soaked, velvet tunnel.  Your focus collapses entirely down to the feel of that slick tunnel around you and how far you can push into it, how strongly you can make it climax and squeeze around you.");
	...
	if(player.cockArea(x) <= 90) Render.text("completely inside once more");
	else Render.text("as deeply inside as you can");
	Render.text(" and hold it there.");
	if(player.lowerBody.cockSpot.count() > 1) {
		Render.text("  Your other member");
		if(player.lowerBody.cockSpot.count() > 2) Render.text("s are");
		else Render.text(" is");
		Render.text(" spasming ");
		if(player.longestCockLength() <= 12) Render.text("on the greedy green slut's belly");
		else Render.text("in the milk-stained valley between her tits, absolutely buried in squishy-soft breast");
		Render.text(", jizzing in wild spurts.  The cum mixes with all the goblin-cream into a sticky-slick morass that paints Lynnette white.");
	}
	Render.text("  Screaming with relief, you can feel the cum rushing out from your [balls], through your middle, and down the length of your quim-buried tool, distending it as blobs of cum roll through the honeyed folds to explode against the well-endowed goblin's cervix. The touch of cum at the womb's entrance sends your lover back into another body-shaking climax; her eyes roll back, her pussy clamps down, and most importantly, her cervix dilates into a wide-open hole, sucking the cum straight into her womb as fast as you can shoot it.");
