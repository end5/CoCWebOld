...
private function hairDresserRepeatGreeting():void {
	...
	if(flags[FlagEnum.LYNNETTE_PREGNANCY_CYCLE] == 4 && flags[FlagEnum.LYNNETTE_CARRYING_COUNT] > 0) {
		...
		outputText("at you. \"<i>Hey there, honey. Come for some more service or to check up on the kids?</i>\" She pats her now-flat midsection. \"<i>I popped them out last night. We had " + num2Text(flags[FlagEnum.LYNNETTE_CARRYING_COUNT]) + " kids... all girls, of course.</i>\" She tucks a lock of beautifully styled hair behind an elfin ear. \"<i>Now you be sure and let me know when you want to contribute to my baby-bank again.</i>\"");
		if(flags[FlagEnum.LYNNETTE_BABY_COUNT] > 0) outputText("\n\nA few new, green faces can be seen looking your way and blushing as they work the gloryholes, probably from a previous litter. They grow up so fast.");
...
private function gloryholeMinotaur(): void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText("hold up ");
...
private function goblinHairDresserFacefuck():void {
	...
	if(player.lowerBody.cockSpot.list[0].cockThickness >= 4) outputText("around as much of you as she can", false);
...
private function minotaurCumBukkakeInSalon():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" and puffing up your twat with liquid arousal.\n\n", false);
	else if(player.lowerBody.cockSpot.hasCock()) outputText(" and turning " + sMultiCockDesc() + " into a turgid, pulsating mass.\n\n", false);
	else outputText("\n\n", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina() || player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			...
			if(player.lowerBody.cockSpot.hasCock()) outputText("and ", false);
		}
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) {
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 6) outputText("Even so, you can't quite get your breasts the entire way around him, so you make up for it by pressing it harder into you with your busy hands.  ", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("tit-fucking ", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("The sudden girth change sends an enticing ripple through your " + allBreastsDescript() + " that's pleasurable enough to make you moan into his steadily-widening urethra.  ", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" while the animal part of your brain hopes it makes you pregnant with an equally girthy son", false);
	outputText(".\n\n", false);
	//ADD PREG CHECK
	//Preggers chance!
	...
	if(!player.lowerBody.cockSpot.hasCock()) outputText("drags through your body, touching sensitive nerves you didn't even know you had until you cum, shuddering and shaking like a wanton whore.", false);
...
private function fuckLynnette():void {
	...
	if(flags[FlagEnum.LYNNETTE_FUCK_COUNTER] == 0) {
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("in turn ");
		outputText("with the skill of a practiced snake-charmer before ");
		if(player.lowerBody.balls == 0) outputText("pressing down near the [sheath]");
	...
	//Repeat cum within 24 hours check
		...
		if(player.lowerBody.balls == 0) outputText("around your [sheath] to get a feel for how pent-up you are");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("es have");
	else outputText(" has");
	...
	if(player.cockArea(x) <= 90) {
		...
		if(player.lowerBody.balls > 0) outputText(", [balls] resting on her soaked, juicy ass-cheeks.");
	}
	else {
		...
		if(player.lowerBody.balls > 0) outputText("  Her feet wrap around your [sack] to squeeze your [balls] affectionately, compensating quite nicely for her own shortcomings.");
	}
	...
	if(player.lowerBody.balls > 0) outputText(", making your sack feel tight and your testes bloated and tender.");
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() > 2) outputText("s are");
		else outputText(" is");
		outputText(" spasming ");
	...
	//"GOOD"
		...
		if(flags[FlagEnum.LYNNETTE_ANNOUNCED_APPROVAL] == 1) outputText("\"<i>Wow, [name].  You cum like a minotaur... and without drugging me out of my mind to boot!  Gods, I can feel the little swimmers hunting down my eggs already.</i>\"  She giggles and retrieves a vial, downing it in a single gulp.  \"<i>I better make sure there's enough eggs for them all, huh?</i>\"\n\nYou leave with a happy smile. It seems Lynnette approves of you as a mate.");
