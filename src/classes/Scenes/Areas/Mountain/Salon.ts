	...
	public class Salon extends BaseContent implements TimeAwareInterface {
		...
		public timeChange():boolean
		{
			...
			if (model.time.hours > 23) {
				if (flags[FlagEnum.LYNNETTE_CARRYING_COUNT] == 0 || flags[FlagEnum.LYNNETTE_PREGNANCY_CYCLE] != 4)
				{
					flags[FlagEnum.LYNNETTE_PREGNANCY_CYCLE]++;
				}
				if (flags[FlagEnum.LYNNETTE_PREGNANCY_CYCLE] == 7) {
...
private salonFavoritesPaymentMenu():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00142] > 0) minoCum = goblinHairDresserFacefuck;
	menu();
	if(flags[FlagEnum.LYNNETTE_PREGNANCY_CYCLE] >= 4 && player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(5,"Fuck Goblin",fuckLynnette);
...
private salonPaymentMenu():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) blow = goblinHairDresserFacefuck;
	let minoCum:Function = null;
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00142] > 0) minoCum = buyMinoCum;
	menu();
	if(flags[FlagEnum.LYNNETTE_PREGNANCY_CYCLE] >= 4 && player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(5,"Fuck Goblin",fuckLynnette);
	MainScreen.addButton(0,"Goblin Blow",blow);
	MainScreen.addButton(1,"Canine",gloryholeDoggie);
	...
	//choices("Goblin Blow",blow,"Canine",gloryholeDoggie,"Imp",gloryholeImp,"Minotaur",gloryholeMinotaur,"Incubus",gloryholeIncubus,"",0,"",0,"",0,"Buy MinoCum",minoCum,"Leave",13);
		...
		private buyMinoCum():void{
			if (player.stats.gems < 60)
			{
				...
				if (flags[FlagEnum.SALON_PAID] == 0)
...
private hairDresserRepeatGreeting():void {
	...
	if(rand(5) == 0 && (player.perks.has("MinotaurCumAddict") || flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] > 0)) {
		minotaurCumBukkakeInSalon();	
		return;
	}
	//Had babies announcement!
	if(flags[FlagEnum.LYNNETTE_PREGNANCY_CYCLE] == 4 && flags[FlagEnum.LYNNETTE_CARRYING_COUNT] > 0) {
		MainScreen.text("As soon as you enter the Salon, Lynnette is beaming ");
		if(player.tallness >= 48) MainScreen.text("up ");
		MainScreen.text("at you. \"<i>Hey there, honey. Come for some more service or to check up on the kids?</i>\" She pats her now-flat midsection. \"<i>I popped them out last night. We had " + num2Text(flags[FlagEnum.LYNNETTE_CARRYING_COUNT]) + " kids... all girls, of course.</i>\" She tucks a lock of beautifully styled hair behind an elfin ear. \"<i>Now you be sure and let me know when you want to contribute to my baby-bank again.</i>\"");
		if(flags[FlagEnum.LYNNETTE_BABY_COUNT] > 0) MainScreen.text("\n\nA few new, green faces can be seen looking your way and blushing as they work the gloryholes, probably from a previous litter. They grow up so fast.");
		flags[FlagEnum.LYNNETTE_BABY_COUNT] += flags[FlagEnum.LYNNETTE_CARRYING_COUNT];
		flags[FlagEnum.LYNNETTE_CARRYING_COUNT] = 0;
		//If favorite!
		if(flags[FlagEnum.LYNNETTE_ANNOUNCED_APPROVAL] == 1) {
			favoriteSalonMenu();
			return;
		}
		else {
	...
	//Favorite Announcement
	else if(flags[FlagEnum.LYNNETTE_APPROVAL] >= 100 && flags[FlagEnum.LYNNETTE_ANNOUNCED_APPROVAL] == 0) {
		...
		flags[FlagEnum.LYNNETTE_ANNOUNCED_APPROVAL] = 1;
		//Custom menu with options to "pay anyway"
		favoriteSalonMenu();
		return;
	}
	//Favorite Greeting 
	else if(flags[FlagEnum.LYNNETTE_ANNOUNCED_APPROVAL] == 1) {
		...
		else if(flags[FlagEnum.LYNNETTE_PREGNANCY_CYCLE] < 4) {
	...
	{
		if(flags[FlagEnum.LYNNETTE_MET_UNPREGNANT] == 0) {
			flags[FlagEnum.LYNNETTE_MET_UNPREGNANT] = 1;
...
private gloryholeMinotaur(): void {
	...
	MainScreen.text("Your head spins from the minotaur's musk, and you idly mop up and swallow the cum on your " + player.face() + ". A goblin aide comes in with a bowl, and gently scrapes the cum off your tits with a smooth, flat rock. Once you're cleaned up and you're dressed, the aide leads you back to Lynnette.\n\n", false);
...
private goblinHairDresserFacefuck():void {
	spriteSelect(38);
	MainScreen.text("Lynnette licks her lips and practically tears her way into your " + player.armorName + ", having your crotch exposed in seconds.  Your " + cockDescript(0) + " flops out immediately, slapping her on the nose as it grows hard.  She wraps both hands around you and begins pumping with practiced ease, flicking her tongue over your crown and wrapping her lips ", true);
	if(player.lowerBody.cockSpot.list[0].cockThickness >= 4) MainScreen.text("around as much of you as she can", false);
...
private cutShort():void {
	...
	MainScreen.text("Lynnette and her daughters crowd around you with razor-sharp scissors, effortlessly paring down your " + hairDescript() + ".  When they've finished, you're left with ", true);
	player.hairLength = 1;
	MainScreen.text(hairDescript() + ".", false);
...
private hairGrow():void {
	...
	MainScreen.text(num2Text(temp) + " more inches of " + player.hairColor + " hair.", false);
	doNext(camp.returnToCampUseOneHour);
}
...
private minotaurCumBukkakeInSalon():void {
	...
	else if(player.lowerBody.cockSpot.hasCock()) MainScreen.text(" and turning " + sMultiCockDesc() + " into a turgid, pulsating mass.\n\n", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) {
		MainScreen.text("  You wrap the pillowy flesh of your " + allBreastsDescript() + " around the new member, pleasantly surprised by its girth and wide, already-flared tip.  Maybe you could have gotten him off with your hands after all?  ", false);
	...
	MainScreen.text("You lower and raise yourself, bouncing up and down on your " + player.legs() + " to enhance the ", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) MainScreen.text("The sudden girth change sends an enticing ripple through your " + allBreastsDescript() + " that's pleasurable enough to make you moan into his steadily-widening urethra.  ", false);
	...
	else MainScreen.text("presses on your prostate as it squeezes by, making " + sMultiCockDesc() + " drip and spurt freely until you can bear it no longer and cum, shaking and shuddering like a wanton whore.  Jizz drips and pours from " + sMultiCockDesc() + " in a steady stream that pools on the floor, slowly rolling towards a drain that doubtless empties into a tank or greedy goblin cunt.", false);
	MainScreen.text("\n\n", false);
	
	MainScreen.text("The strength goes completely out of your " + player.legs() + ", but you manage to hold yourself up long enough for your stud to flex his cock inside you and fill up your backdoor with more potent addiction.  You slide off, nerveless and still orgasming as jizz rains on your back from the abandoned cock, rolling off you to add to the ever-widening puddle of fluids on the floor.  Smiling vacantly and addled on more minotaur-cum than any one person has reason to handle, you stumble up on all fours and crawl to the next one.  More...", false);
	//[NEXT]
	player.orgasm();
...
private minotaurSalonFollowUp():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00142] == 0) {
		...
		flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00142]++;
		//text goez here
	...
	else MainScreen.text("You're woken up with a slap to the face, and still muzzy from your cum-induced glory-hole orgy, you stagger up to your " + player.feet() + ".  Lynnette shakes her stinging palm and apologizes, \"<i>Sorry I have to keep doing that, but you're damned hard to wake after you go on these binges!  Anyways, you've earned some hair treatment if you want it.  Of course you could always buy more of your favorite fluid...</i>\"\n\n", false);
	//Menu
	MainScreen.text("Lynnette offers and explains their options, \"<i>So what'll it be hun?  We could cut it down or give you a lengthening treatment. Or you can get a hair-dye to use on your own.  Just remember to come back in a few days for a touchup.</i>\"", false);
	salonPurchaseMenu();
}
...
private fuckLynnette():void {
	...
	//Repeat cum within 24 hours check
		...
		if(player.lust <= 75) MainScreen.text("it fills to full size almost immediately");
	...
	//"GOOD"
		...
		if(flags[FlagEnum.LYNNETTE_ANNOUNCED_APPROVAL] == 1) MainScreen.text("\"<i>Wow, [name].  You cum like a minotaur... and without drugging me out of my mind to boot!  Gods, I can feel the little swimmers hunting down my eggs already.</i>\"  She giggles and retrieves a vial, downing it in a single gulp.  \"<i>I better make sure there's enough eggs for them all, huh?</i>\"\n\nYou leave with a happy smile. It seems Lynnette approves of you as a mate.");
	...
	flags[FlagEnum.LYNNETTE_FUCK_COUNTER]++;
	player.orgasm();
	flags[FlagEnum.LYNNETTE_CARRYING_COUNT] = 3 + rand(3);
	if(player.cumQ() >= 1000) flags[FlagEnum.LYNNETTE_CARRYING_COUNT] += 1 + rand(3);
	if(player.cumQ() >= 2000) flags[FlagEnum.LYNNETTE_CARRYING_COUNT] += 1 + rand(3);
	if(player.cumQ() >= 3000) flags[FlagEnum.LYNNETTE_CARRYING_COUNT] += 2 + rand(3);
	if(player.cumQ() >= 4000) flags[FlagEnum.LYNNETTE_CARRYING_COUNT] += 2 + rand(3);
	if(player.cumQ() >= 6000) flags[FlagEnum.LYNNETTE_CARRYING_COUNT] += 2 + rand(3);
	if(player.cumQ() >= 10000) flags[FlagEnum.LYNNETTE_CARRYING_COUNT] += 2 + rand(3);
...
public lynnetteApproval(arg: number = 0):number {
	...
	return flags[FlagEnum.LYNNETTE_APPROVAL];
}
}
}
