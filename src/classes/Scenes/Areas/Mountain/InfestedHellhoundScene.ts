...
package classes.Scenes.Areas.Mountain
{
	...
	public class InfestedHellhoundScene extends BaseContent
	{
		...
		public infestedHellhoundEncounter():void
		{
			...
			if (player.lowerBody == LOWER_BODY.GOO) MainScreen.text("making your jello-like body jiggle", false);
		...
		public infestedHellhoundLossRape():void
		{
			MainScreen.text("", true);
			//[BOTH INFESTED]
			if (player.totalCocks() > 0 && player.statusAffects.has("Infested")) {
				//(LUST)
				if (player.lust > 99) {
					...
					MainScreen.text("you remove your " + player.armorName + " and roll onto your back, exposing your vulnerable groin to the beast.\n\n", false);
				}
				//(HP) 
				else MainScreen.text("Too wounded to stand, you drop down to all fours in order to keep yourself off the ground.   Too late you realize your mistake – the snarling beast is behind you and its razor-sharp teeth easily sink into your " + player.armorName + " before tearing away the offending armor, exposing your " + assDescript() + " and " + multiCockDescriptLight() + ".  A cold mountain breeze blows across your now-exposed " + player.skinDesc + ", reminding you just how utterly vulnerable you are to the alien lusts of this symbiotic monstrosity. With a brutal lunge it knocks you off your " + player.feet() + " and onto your back.\n\n", false);
			...
			//[PLAYER'S COCKS ARE BIG ENOUGH TO BE INFECTED]
			else if (player.findStatusAffect(StatusAffects.Infested) < 0 && player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 40 && player.lowerBody.cockSpot.hasCock()) {
				//(LUST)
				if (player.lust > 99) {
					MainScreen.text("No amount of shame from the act of submitting to such a beast can overpower the furnace of lust raging in your loins.  ", false);
					if (player.stats.cor < 33) MainScreen.text("In spite of your revulsion ", false);
				...
				//(HP)
					...
					MainScreen.text("Too wounded to stand, you drop down to all fours in order to keep yourself off the ground.   Too late you realize your mistake – the snarling beast is behind you and its razor-shark teeth easily sink into your " + player.armorName + " before tearing it off, exposing your " + assDescript() + " and " + multiCockDescriptLight() + ".  A cold mountain breeze blows across your now exposed " + player.skinDesc + ", reminding you just how utterly vulnerable you are to the alien lusts of this symbiotic monstrosity.  With a brutal lunge it knocks you off your " + player.feet() + " and onto your back.\n\n", false);
				}
				if (player.totalCocks() > 1) {
					...
					MainScreen.text("each of your " + multiCockDescriptLight() + ".  It feels good, better than it has any right to.  ", false);
					MainScreen.text("Every single one of your " + multiCockDescriptLight() + " hardens under the stimulation, happy to be so well-treated.\n\n", false);
					...
					MainScreen.text("Gods, it's shoving its infected cocks up your urethras!  It's trying to infect you with those worms!  The dog-demon keeps pushing further and further, sliding deep inside you, the outline of its members easily visible through the skin of your " + multiCockDescriptLight() + ".  It starts feeling good, the wriggling parasite-infested dicks begin sliding in and out, fucking your urethras in earnest and depositing their wormy cargo deep inside you.\n\n", false);

					MainScreen.text("Your ", false);
					if (player.lowerBody.balls > 0) MainScreen.text(ballsDescriptLight() + " shiver as hot doggie-cum and wiggling worms are pumped directly into them.  ", false);
					...
					MainScreen.text("The infested hound repositions itself, blocking out the sun with its dark fur, leaving you with only the pale flames surrounding its fuzzy sack to look at.   The warm wetness of its smooth tongue starts sliding over ", false);
					MainScreen.text("your " + multiCockDescriptLight() + ".  It feels good, better than it has any right to.  ", false);
					MainScreen.text("Your " + multiCockDescriptLight() + " hardens under the stimulation, happy to be so well-treated.\n\n", false);
					...
					MainScreen.text("Gods, it's shoving its infected cock up your urethra!  It's trying to infect you with those worms!  The dog-demon keeps pushing further and further, sliding deep inside you, the outline of its member easily visible through the skin of your " + multiCockDescriptLight() + ".  It starts feeling good, the wriggling parasite-infested dick begins sliding in and out, fucking your urethra in earnest and depositing its wormy cargo deep inside you.\n\n", false);
				...
				if(flags[FlagEnum.EVER_INFESTED] == 0) {
					flags[FlagEnum.EVER_INFESTED] = 1;
					if(player.stats.cor < 25) player.stats.cor = 25;
				}
				cleanupAfterCombat();
			}
			//[HAS PUSSY AND NO DICK BIG ENOUGH TO BE INFECTED]
			else if (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.cockSpot.biggestCocks[0].cockArea() < 40 && player.lowerBody != LOWER_BODY.NAGA) {
				//(LUST) 
				if (player.lust > 99) {
					MainScreen.text("No amount of shame from the act of submitting to such a beast can overpower the furnace of lust raging in your loins.  ", false);
					if (player.stats.cor < 33) MainScreen.text("In spite of your revulsion ", false);
					...
					MainScreen.text("you remove your " + player.armorName + " and drop to all fours, mimicking what you think a dog would do.\n\n", false);
				}
				//(HP) 
				else {
					MainScreen.text("Too wounded to stand, you drop down to all fours in order to keep yourself off the ground.   Too late you realize your mistake – the snarling beast is behind you and its razor-shark teeth easily sink into your " + player.armorName + " before tearing it off, exposing your " + assDescript() + " and " + vaginaDescript(0) + ".  A cold mountain breeze blows across your now exposed " + player.skinDesc + ", reminding you just how utterly exposed you are to the alien lusts of this symbiotic monstrosity.\n\n", false);
				}
			...
			//[GENDERLESS OR MALE WITH DICK TOO SMALL]
				...
				if (player.lust > 99) {
					...
					MainScreen.text("you remove your " + player.armorName + " and drop to your elbows, mimicking what you think a dog would do.\n\n", false);
				}
				//(HP) 
				else {
					MainScreen.text("Too wounded to stand, you drop down to on your elbows in order to keep yourself off the ground.   Too late you realize your mistake – the snarling beast is behind you and its razor-shark teeth easily sink into your " + player.armorName + " before tearing it off, exposing your " + assDescript() + ".  A cold mountain breeze blows across your now exposed " + player.skinDesc + ", reminding you just how utterly exposed you are to the alien lusts of this symbiotic monstrosity.\n\n", false);
				}
				MainScreen.text("The hell-hound rises up, looking nearly humanoid in shape for a moment before it drops down, planting its paws on your shoulders.  ", false);
				...
				MainScreen.text("Incredible heat radiates from the thick black rod inside you, making your whole body break out in a cold sweat in an attempt to deal with the situation.  You're rocked back and forth as the beast begins fucking you in earnest, slamming its fur-covered hips against your " + assDescript() + " with animalistic intensity.  Thankfully, the fucking isn't that painful, but the small mercy is ruined by the hot fluid you can feel starting to pour into your " + assholeDescript() + " and onto your back.\n\n", false);
