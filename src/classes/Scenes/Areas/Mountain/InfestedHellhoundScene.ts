...
package classes.Scenes.Areas.Mountain
{
	...
	public class InfestedHellhoundScene extends BaseContent
	{
		...
		public infestedHellhoundEncounter():void
		{
			...
			if (player.lowerBody == LOWER_BODY.GOO) Render.text("making your jello-like body jiggle", false);
			else Render.text("rattling your teeth", false);
			Render.text(".  Peeking fearfully over your shoulder, you see the black-furred form of a hell-hound emerging from behind a rock.  Its eyes narrow as its gaze locks onto your " + player.assholeOrPussy() + ", a pair of black, shiny dog-cocks emerge underneath him, dangling freely.  His balls look bloated and distended, the skin around them crawling and wriggling.  A few worms drip from its over-sized peckers, crawling on the ground under the infested beast.\n\n", false);
			Render.text("Somehow you know this thing isn't going to let you just walk away.", false);
		...
		public infestedHellhoundLossRape():void
		{
			...
			if (player.totalCocks() > 0 && player.statusAffects.has("Infested")) {
				//(LUST)
				if (player.lust > 99) {
					Render.text("No amount of shame from the act of submitting to such a beast can overpower the furnace of lust raging in your loins.  ", false);
					if (player.stats.cor < 33) Render.text("In spite of your revulsion ", false);
					else if (player.stats.cor < 66) Render.text("In spite of your better sense ", false);
					else Render.text("With a perverse sense of anticipation ", false);
					Render.text("you remove your " + player.armorName + " and roll onto your back, exposing your vulnerable groin to the beast.\n\n", false);
				}
				//(HP) 
				else Render.text("Too wounded to stand, you drop down to all fours in order to keep yourself off the ground.   Too late you realize your mistake – the snarling beast is behind you and its razor-sharp teeth easily sink into your " + player.armorName + " before tearing away the offending armor, exposing your " + assDescript() + " and " + multiCockDescriptLight() + ".  A cold mountain breeze blows across your now-exposed " + player.skinDesc + ", reminding you just how utterly vulnerable you are to the alien lusts of this symbiotic monstrosity. With a brutal lunge it knocks you off your " + player.feet() + " and onto your back.\n\n", false);

				Render.text("The beast takes a sniff at your groin, then backs away, looking confused.  You glance down and realize just how hard you've become.  A few of your worms are hanging from the " + player.cockHead() + " of your " + cockDescript(0) + ", starting to flow out in a steady stream.  It feels better than it has any right to.   A shadow falls across you as the hellhound moves over you, its imposing twin members hard and pulsating above you.  Hot splatters of jism drip onto your chest as the beast's worms begin escaping, forcing thick globules of dog-semen out along with them.\n\n", false);

				Render.text("Overcome by the worms, both you and the beast begin orgasming, without external stimulation of any kind.  Worms and cum mix together on top of you, slowly building into a large mound that covers the better part of your torso.  Exhausted and drained, you both squirt weakly, emptying the last of your smallest worms into the pile.   Your eyes close as the beast lies down with you, and together the two of you lose consciousness as your newly birthed worm colony squirms away.", false);
				player.orgasm();
			...
			//[PLAYER'S COCKS ARE BIG ENOUGH TO BE INFECTED]
				...
				//(HP)
					...
					Render.text("Too wounded to stand, you drop down to all fours in order to keep yourself off the ground.   Too late you realize your mistake – the snarling beast is behind you and its razor-shark teeth easily sink into your " + player.armorName + " before tearing it off, exposing your " + assDescript() + " and " + multiCockDescriptLight() + ".  A cold mountain breeze blows across your now exposed " + player.skinDesc + ", reminding you just how utterly vulnerable you are to the alien lusts of this symbiotic monstrosity.  With a brutal lunge it knocks you off your " + player.feet() + " and onto your back.\n\n", false);
				}
				if (player.totalCocks() > 1) {
					Render.text("The infested hound repositions itself, blocking out the sun with its dark fur, leaving you with only the pale flames surrounding its fuzzy sack to look at.   The warm wetness of its smooth tongue starts sliding over ", false);
					Render.text("each of your " + multiCockDescriptLight() + ".  It feels good, better than it has any right to.  ", false);
					Render.text("Every single one of your " + multiCockDescriptLight() + " hardens under the stimulation, happy to be so well-treated.\n\n", false);

					Render.text("Stopping its licking, the beast begins to shuffle forwards, pre-cum coated worms plopping out of its double-dicks onto your belly.  They crawl lamely around as the beast works to line itself up, though you're unsure what it could possibly be aiming for.  Your questions are answered as the slightly pointed tips of its twin members press forwards, bumping against the crowns of your own cocks.   It pushes forward with an intense sort of care, slowly forcing itself into your urethras, dribbling painfully hot cum directly into your cock-passages.\n\n", false);

					Render.text("Gods, it's shoving its infected cocks up your urethras!  It's trying to infect you with those worms!  The dog-demon keeps pushing further and further, sliding deep inside you, the outline of its members easily visible through the skin of your " + multiCockDescriptLight() + ".  It starts feeling good, the wriggling parasite-infested dicks begin sliding in and out, fucking your urethras in earnest and depositing their wormy cargo deep inside you.\n\n", false);

					Render.text("Your ", false);
					if (player.lowerBody.balls > 0) Render.text(ballsDescriptLight() + " shiver as hot doggie-cum and wiggling worms are pumped directly into them.  ", false);
					else Render.text("body feels uncomfortably full as hot doggie-cum and wriggling worms are pumped directly into your prostate.  ", false);
					...
					Render.text("The demonic dog backs away with what looks like a grin on its face after filling you with worms and boiling spooge, your urethras stretched and dripping with white squirming goop.  Pushed beyond your endurance, you start blacking out, your last thought a lamentation on how you'll be a carrier for these parasites, just like this demon-dog.", false);
				}
				else {
					Render.text("The infested hound repositions itself, blocking out the sun with its dark fur, leaving you with only the pale flames surrounding its fuzzy sack to look at.   The warm wetness of its smooth tongue starts sliding over ", false);
					Render.text("your " + multiCockDescriptLight() + ".  It feels good, better than it has any right to.  ", false);
					Render.text("Your " + multiCockDescriptLight() + " hardens under the stimulation, happy to be so well-treated.\n\n", false);
					...
					Render.text("Gods, it's shoving its infected cock up your urethra!  It's trying to infect you with those worms!  The dog-demon keeps pushing further and further, sliding deep inside you, the outline of its member easily visible through the skin of your " + multiCockDescriptLight() + ".  It starts feeling good, the wriggling parasite-infested dick begins sliding in and out, fucking your urethra in earnest and depositing its wormy cargo deep inside you.\n\n", false);
					...
					Render.text("The demonic dog backs away with what looks like a grin on its face after filling you with worms and boiling spooge, your urethra stretched and dripping with white squirming goop.  Pushed beyond your endurance, you start blacking out, your last thought a lamentation on how you'll be a carrier for these parasites, just like this demon-dog.", false);
				}
				//(+infested)
			...
			//[GENDERLESS OR MALE WITH DICK TOO SMALL]
				...
				if (player.lust > 99) {
					...
					Render.text("you remove your " + player.armorName + " and drop to your elbows, mimicking what you think a dog would do.\n\n", false);
				}
				//(HP) 
				else {
					Render.text("Too wounded to stand, you drop down to on your elbows in order to keep yourself off the ground.   Too late you realize your mistake – the snarling beast is behind you and its razor-shark teeth easily sink into your " + player.armorName + " before tearing it off, exposing your " + assDescript() + ".  A cold mountain breeze blows across your now exposed " + player.skinDesc + ", reminding you just how utterly exposed you are to the alien lusts of this symbiotic monstrosity.\n\n", false);
				}
				Render.text("The hell-hound rises up, looking nearly humanoid in shape for a moment before it drops down, planting its paws on your shoulders.  ", false);
				if (player.stats.cor < 80) Render.text("You beg, \"<i>Please, no, no!  I don't want it... in me!</i>\"\n\n", false);
				else Render.text("You start to beg it not to put it inside you, but your protests drop off when you realize how good they could feel squirming inside you.\n\n", false);

				Render.text("The demon-dog tries to penetrate you all at once, but it isn't lined up properly, and all it manages to do is bump against your taint and rub its other dick on your back.   You gasp in pain, momentarily thankful not to be impaled by such a bestial implement, but knowing all too well the brief reprieve will be over all too soon.   As if it can read your thoughts, the demon-hound repositions itself and slams forward, this time managing to line its dog-dicks up with your " + assholeDescript() + " correctly.  It feels far larger than it looked, filling you with a painful suddenness that brings tears to your eyes.\n\n", false);
				Render.text("Incredible heat radiates from the thick black rod inside you, making your whole body break out in a cold sweat in an attempt to deal with the situation.  You're rocked back and forth as the beast begins fucking you in earnest, slamming its fur-covered hips against your " + assDescript() + " with animalistic intensity.  Thankfully, the fucking isn't that painful, but the small mercy is ruined by the hot fluid you can feel starting to pour into your " + assholeDescript() + " and onto your back.\n\n", false);
