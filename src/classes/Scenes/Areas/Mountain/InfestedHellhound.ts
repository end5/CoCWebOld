		...
		private function hellHoundWormCannon():void
		{
			...
			if (rand(2) == 0) {
				...
				if (player.statusAffects.has("Infested") && player.totalCocks() > 0) {
					...
					if (player.lowerBody.balls > 0) outputText("  Your " + ballsDescriptLight() + " grow weightier as the worms settle into their new home, arousing you beyond measure.", false);
				...
				else if (player.lowerBody.vaginaSpot.hasVagina()) outputText("  Thankfully, the worms don't seem to want anything to do with you, and rapidly drop down to the ground.", false);
			}
			//Sidestep
			else {
				...
				if (player.statusAffects.has("Infested")  && player.lowerBody.cockSpot.hasCock()) {
					if (player.lowerBody.cockSpot.hasCock()) {
						...
						if (player.lowerBody.balls == 0 && player.lowerBody.cockSpot.count() == 1) outputText("s", false);
