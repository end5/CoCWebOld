...
internal function minoVictoryRapeChoices():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		while(counter < player.lowerBody.cockSpot.count()) {
			if(player.lowerBody.cockSpot.list[counter].cockThickness <= 4 && player.lowerBody.cockSpot.list[counter].cockLength >= 14) urethralPen = minoUrethralPen;
			counter++;
		}
	}
	if(player.lowerBody.cockSpot.hasCock()) {
	...
	if(x >= 0 && player.lowerBody.cockSpot.count() > 1 && player.tentacleCocks() > 0) tentaRape = rapeMinotaurTentacles;
	...
	if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] >= 1 || player.perks.has("MinotaurCumAddict")) {
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5 && !player.isNaga()) {
	...
	if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] >= 1) {
		Render.text("Smiling down at your vanquished foe, you feel a familiar hunger growing within you.  What do you do?", true);
	}
	//Not an addict
	else if((player.lust >= 33 && player.gender > 0) || feedposit == "Lay Eggs") {
		if(monster.lust > 99) Render.text("You smile in satisfaction as the " + monster.short + " drops down on all fours and begins masturbating feverishly.  Sadly you realize your own needs have not been met.  Of course you could always fuck the eager bull...\n\nWhat do you do?", true);
		else Render.text("You smile in satisfaction as the " + monster.short + " collapses, unable to continue fighting.  Sadly you realize your own needs have not been met.  Of course you could always rape the poor thing...\n\nWhat do you do?", true);
	}
	//Not able to rape but a feeder
	else if(player.statusAffects.has("Feeder") && feedposit == "B. Feed") {
		if(monster.lust > 99) Render.text("You smile in satisfaction as the " + monster.short + " collapses, unable to continue fighting.  Sadly you realize your own need to breastfeed has not been met.  You could always let the poor thing have a drink...\n\nDo you?", true);
		else Render.text("You smile in satisfaction as the " + monster.short + " collapses, unable to continue fighting.  Sadly you realize your own need to breastfeed has not been met.  You could always let the poor thing have a drink...\n\nWhat do you do?", true);
	}
	//No rape, no feeder
	else {
		if(monster.lust > 99) Render.text("You smile in satisfaction as the " + monster.short + " drops down on all fours and begins masturbating feverishly.", true);
		else Render.text("You smile in satisfaction as the " + monster.short + " collapses, unable to continue fighting.", true);
		cleanupAfterCombat();
		return;
	}
...
private rapeMinotaurTentacles():void {
	...
	if(player.lowerBody.cockSpot.count() > 1 && player.tentacleCocks() == 1) {
	...
	let temp2:number = 0;
	Render.text("", true);
	if(monster.HP <= 0) Render.text("The minotaur collapses backwards after your last blow. Defeated and tired, he can barely stand up as you approach.  ", false);
	else {
		Render.text("The minotaur drops his arm", false);
		if(monster.weaponName == "axe") Render.text(", letting his axe clatter to the floor", false);
		Render.text(". Transfixed by your " + multiCockDescript() + " in front of his face, he barely notices as you walk closer.  ", false);
	}
	Render.text("You grab his arm firmly, making certain there will be no retaliation during the pleasure. By the look on his face, and the state of his torn, rolled up loincloth, though, you doubt that will be a problem.  ", false);
	if(player.stats.cor < 25) Render.text("You cry, petting his hair, smoothing it across his horns, justifying to yourself that you are raping this beast because this is how this world works as you maneuver your " + multiCockDescript() + " towards his tailhole.  ", false);
	if(player.stats.cor >= 25 && player.stats.cor < 50) Render.text("You smoothly guide your " + multiCockDescript() + " toward his tailhole.  ", false);
	if(player.stats.cor >= 50 && player.stats.cor < 80) Render.text("You roughly grab him, slamming him close to your " + multiCockDescript() + " - just how this beast likes it.  ", false);
	if(player.stats.cor >= 80) Render.text("You grin and roughly pull his horns towards your " + multiCockDescript() + ", ready to roughly ride this beast.  ", false);
	//Mixed dix
	if(player.lowerBody.cockSpot.count() != player.tentacleCocks()) {
		mismatched = true;
		...
		while(temp > 0 && temp2 == -1) {
			temp--;
			if(player.lowerBody.cockSpot.list[temp].cockType != CockType.TENTACLE) temp2 = temp;
		}
		//failsafe
		if(temp2 == -1) temp2 = 0;
		...
		while(temp > 0) {
			...
			if(player.lowerBody.cockSpot.list[temp].cockLength > player.lowerBody.cockSpot.list[temp2].cockLength && player.lowerBody.cockSpot.list[temp].cockType != CockType.TENTACLE) {
				temp2 = temp;
			}
		}
		Render.text("Your " + cockDescript(temp2) + " slams into his tailhole, spreading it wider as you thrust in and out of the opening, relishing in the feeling of the rough, yet lubricated insides of the minotaur.  ", false);
	}
	//Not mismatched
	else {
		temp2 = 0;
		Render.text("One of your prehensile dicks coils in on itself as it nears the minotaur's tailhole, readying itself. It then suddenly springs itself into his fleshy orifice, wiggling and writhing in deeper and deeper until all " + num2Text(int(player.lowerBody.cockSpot.list[0].cockLength)) + " inches of it have submerged itself inside his intestines. You and he both shudder as your cock starts weaving from side to side, making slithering motions inside him.  ", false);
	}
	//Vaginassss
	if(player.lowerBody.vaginaSpot.count() >= 1) {
		Render.text("With your " + cockDescript(temp2) + " successfully embedded in the minotaur, you twist around, positioning your " + vaginaDescript(0) + " to take the minotaur's enormous length into yourself.  You feel his member deliciously slide in, and feel the beast's hot breath radiate as he starts panting in lust.  ", false);
		player.cuntChange(monster.cockArea(0),true,false,true);
	}
	//Multivaginas...wtf
	if(player.lowerBody.vaginaSpot.count() > 1 && !mismatched && player.lowerBody.cockSpot.count() > 2) Render.text("Your tentacle cocks writhe upon themselves, each turning to an unattended " + vaginaDescript(1) + " on your body, and quickly filling up the cavernous depths. You groan in ecstasy at the self-fucking you are receiving as all your pussies start leaking their wonderful fluids.  ", false);
	//more dicks than available holes
	if((player.tentacleCocks() == 2 || player.tentacleCocks() == 3)) Render.text("Your next tentacle dick, saddened by not having anything to writhe around, turns to the minotaurs protruding member. It darts out from your crotch, wrapping around his monstrous meat and constricting, like a snake might around its prey. Your tongue lolls out as you feel that cock twist and grab his dick, pulsing with both your blood and the beast's twitching member.  ", false);
	if(player.tentacleCocks() > 3) Render.text("Your remaining tentacle dicks wave around the minotaur, tending to his balls, weaving around his limbs, and generally rubbing and throbbing all over him, spreading pre-cum around and through him, leaving both of you moaning in pleasure.  ", false);
	//Cum
	Render.text("Your tentacles throb and pulse, quickening in pace as you can feel the cum swelling in your prostate. They wave madly, and then, just as their motion makes you dizzy, you feel them stiffen suddenly, and start spewing their load all in and across the minotaur. You gasp and pause, collapsing on the strong back of the minotaur, basking in the afterglow.", false);
	player.orgasm();
	cleanupAfterCombat();
}
...
private rapeMinotaurTentacle():void {
	...
	Render.text("", true);
	Render.text("You stand before the defeated minotaur and your tentacle dick begins to lash back and forth quickly, showing your excitement.  You see the hardness of his dick, and decide to do something about it... for both of you.  ", false);
	Render.text("You come up behind him and wrap your tentacle dick around his huge member, squeezing him tightly at his dick's base pump even more blood to it.  Then you wrap yourself around his massive erection a few more times and you're ready. You jerk him up and down, varying the pace, place, and strength of your grip, milking his dick for all he is worth.  ", false);
	Render.text("The minotaur's balls begin to spasm, so you wrap the base of your tentacle around him tightly, preventing his " + monster.cockDescriptShort(0) + " from cumming, driving the minotaur mad with desire.  Your own " + cockDescript(x) + " delights in the feeling of the fur of his balls and the hardness of his " + monster.cockDescriptShort(0) + ".  ", false);
	//Reset for upcoming stuff
	temp = 0;
	//if multi with pony dick!
	if(player.horseCocks() == 1) {
    	Render.text("But it isn't enough for you. You take your pony prick, and with your hands, pull his anus wide. The flare at your tip takes some effort to get in, but once it's inside, you ram right to the hilt, knocking the wind from the minotaur with a solid grunt.  ", false);
		temp = 1;
	}
	//if multi with dogdick
	if(player.dogCocks() == 1 && temp != 1) {
		temp = 1;
		Render.text("But it isn't enough for you. You take your bitch knotter, and with your hands, pull his anus wide. You slide in easily, right up to your knot, but he's too tight for you to slip that in.  ", false);
	}
	//If single leftover dick
	if(player.lowerBody.cockSpot.count() == 2 && temp != 1) {
		temp = 1;
		Render.text("But you avail yourself of his free rear end, to further your own delights. A quick push, and you're in, buried to your hilt.  ", false);
	}
	if(player.lowerBody.cockSpot.count() > 2 && temp != 1) {
		temp = 1;
		Render.text("It's not enough! Your other dicks demand attention too! Fortunately, he has a large hole for you to abuse... you stuff as many of your " + multiCockDescript() + " as you can, the rest hanging underneath him, slapping against his sack. ");
	}
	if(player.lowerBody.vaginaSpot.list[0].clitLength > 6) {
		Render.text("Eager for more stimulation, you muse that your clit's big enough to fuck with, so why not!? You take your fingers, spit on them, and slide them into place, opening him up wider to accept your " + clitDescript() + ". It's so sensitive, sliding into that tight hole, and the sensations are driving you wild as you hump against him with your dick-like clit.  ", false);
	}
	//Free pussy!
	if(player.lowerBody.vaginaSpot.count() > 0) {
		temp = 2;
		Render.text("You're a woman, and it's about time you got some pussy action too!  You slide your " + vaginaDescript(0) + " onto " + player.cockDescript(x) + "-wrapped minotaur-prick, further driving the beast mad with lust.  He plaintatively groans, struggling to get his dick free or to cum, you aren't quite sure.  ", false);
		Render.text("Eventually, the moment is right.  You smile as you feel a boiling heat building in your groin, surging through your " + multiCockDescript() + " in a tide of white-hot pleasure.  You groan and shiver, lost in the pleasure until you begin to calm down.  ", false);
		if(player.cumQ() < 50) Render.text("Your cock made quite a mess of things, and when you pull him out, your " + vaginaDescript(0) + " makes a wet sucking sound.  ", false);
		if(player.cumQ() >= 50 && player.cumQ() < 400) Render.text("Your cock made quite a mess of things inside your " + vaginaDescript(0) + ".  As you pull free, cum drips freely down your thighs.  The heavenly scent of minotaur spunk fills your nose, and begin scooping the mixed jizz up to sample.  ", false);
		if(player.cumQ() >= 400) Render.text("Your cock made quite a mess of things inside your " + vaginaDescript(0) + ".  As you pull free, a river of cum pours from betwixt your thighs.  The heavenly scent of minotaur spunk fills your nose, and you begin scooping the mixed jizz up to sample.  ", false);
	}
	//Free ass!
	else {
		temp = 2;
		Render.text("You're feeling a little kinky so you step in front of him, offering him a little bit more, pushing yourself down onto his meaty dick.  You slowly push down, enjoying every inch as it travels down your " + assholeDescript() + ", the pre leaking from it acting as a lubricant as his tentacle-wrapped cock works its way inside you.  ", false);
		//Ass cum
		Render.text("Eventually, the moment is right.  You smile as you feel a boiling heat building in your groin, surging through your " + multiCockDescript() + " in a tide of white-hot pleasure.  You groan and shiver, lost in the pleasure until you begin to calm down.  ", false);
		if(player.cumQ() < 50) Render.text("Your cock in his ass made a mess of things, and when you pull out, it makes a wet sucking sound, trying to keep you inside.  ", false);
		if(player.cumQ() >= 50 && player.cumQ() < 400) Render.text("Your cock in his ass spills forth, creating a puddle which will mat his fur later.  ", false);
		if(player.cumQ() >= 400) Render.text("Your cock is blasted free from his ass by the force of your ejaculation, splattering rivers of spoo over the ground.  A river of the stickiness pours from his violated asshole, sticking in the fur of his thighs.  ", false);
	}
	//Cum tiem
	if(temp < 2) {
		if(player.cumQ() < 25) Render.text("Eventually, the moment is right... you position yourself accordingly to do what you have in mind.  Your needs are both at their peak, and release will be soon!  Once you're properly set up behind him, you shove his head forward, making him take both your, and his own, dick into his mouth. His tongue trying to shove you out, slipping in between his and your own dick pushes you over the edge, and you release his balls, causing both of your dicks to fill his mouth with cum.", false);
		//Big Cum
		if(player.cumQ() >= 25 && player.cumQ() < 250) Render.text("Eventually, the moment is right... you position yourself accordingly to do what you have in mind. Your needs are both at their peak, and release will be soon! Once you're properly set up behind him, you shove his head forward, making him take both your and his own dick into his mouth. His tongue tries to shove you out, slipping in between his and your own dick.  It pushes you over the edge, and you release his balls, causing both of your dicks to flood his mouth with cum, which dribbles out around his lips, and splatters onto his chest.", false);
		if(player.cumQ() >= 250 && player.cumQ() < 500) Render.text("Eventually, the moment is right... you position yourself accordingly to do what you have in mind. Your needs are both at their peak, and release will be soon! Once you're properly set up behind him, you shove his head forward, making him take both your and his own dick into his mouth. His tongue tries to shove you out, slipping in between his and your own dick.  It pushes you over the edge, and you release his balls, causing both of your dicks to overflow his mouth with cum, spraying out his nose, forcing him to swallow from sheer volume.", false);
		if(player.cumQ() >= 500) Render.text("Eventually, the moment is right... you position yourself accordingly to do what you have in mind. Your needs are both at their peak, and release will be soon! Once you're properly set up behind him, you shove his head forward, making him take both your, and his own dick into his mouth. His tongue tries to shove you out, slipping in between his and your own dick.  It pushes you over the edge, and you release his balls, allowing him to cum. However, your own cum dwarfs his pathetic volume... forcing him to swallow load after load of your hot sticky jizz. Even so, the volume exceeds his ability to down it, and cum sprays out of his mouth, an arc shooting out of his nose every now and again further commenting on the capacity of your massive ejaculation.", false);
	}
	cleanupAfterCombat();
}
...
private minoUrethralPen():void {
	...
	while(counter < player.lowerBody.cockSpot.count()) {
		if(player.lowerBody.cockSpot.list[counter].cockThickness <= 4 && player.lowerBody.cockSpot.list[counter].cockLength >= 14) {
			x = counter;
			break;
		}
		counter++;
...
private minoRapeIntro():void {	
	...
	if(monster.HP <= 0) {
		Render.text("You roll the barely conscious beast over, lifting his ass up above his knees.  The loincloth flops open, giving you a perfect view of his hardening member.  In most ways it looks like a horse's dick, though it seems to be textured with multiple rings of prepuce along its length.  The scent boiling off that marvelously thickening endowment is pleasant yet musky, and overpoweringly strong.", true);
		//Vagina or not flavor texts		
		if(player.lowerBody.vaginaSpot.count() > 0) Render.text("  Glancing back at you with hope in his eyes, the minotaur's " + monster.cockDescriptShort(0) + " twitches as he lays his gaze upon your " + vaginaDescript(0) + ".", false);
		else Render.text("  Glancing back at you with eyes full of despair, the minotaur seems to realize you won't be helping the predicament his " + monster.ballsDescriptLight() + " are in.", false);
	}
	//Monster lost to lust
	else {
		Render.text("You rip off the minotaur's loincloth with a savage grin, slapping his ass and admiring the jiggle of his " + monster.ballsDescriptLight() + " as they wobble from the force of your blow.  The cow-man moos softly with desire, ropes of pre splattering the ground from his arousal.   The musk pouring off him and his puddle of slick pre-cum is overpowering, making it difficult to think as you feel your blood rush to your groin ", false);
		//NIPPULAR ENJOYMENT
		if(player.upperBody.chest.countNipples() > 1) Render.text(" and nipples", false);
		Render.text(".", false);
		//Vagina or not flavor texts
		if(player.lowerBody.vaginaSpot.count() > 0) Render.text("  Glancing back at you with hope in its eyes, the minotaur's " + monster.cockDescriptShort(0) + " twitches as he lays his gaze upon your " + vaginaDescript(0) + ".", false);
		else Render.text("  Glancing back at you with eyes full of despair, the minotaur doubts you'll help relieve the predicament his " + monster.ballsDescriptLight() + " are in.", false);
	}
}
...
private girlRapeAMinotaur():void {
	...
	if(player.lowerBody == LOWER_BODY.CENTAUR) Render.text("hindquarters, forcefully grinding your vagina along his length like a toy.  ", false);
	else Render.text(hipDescript() + ", forcefully grinding up and down along his length like a cheap sex-toy.  ", false);
	//Size variants
	//Too small
	if(player.vaginalCapacity() < monster.cockArea(0)) Render.text("Every ridge of his " + monster.cockDescriptShort(0) + " presses tightly against your walls, smearing more of his wondrous pre-seed into your body with every stroke.  ", false);
	//Just right
	if(player.vaginalCapacity() >= monster.cockArea(0) && player.vaginalCapacity() * 1.5 < monster.cockArea(0)) Render.text("Every stroke of his " + monster.cockDescriptShort(0) + " deposits a bit more of his wondrous preseed into your body, spreading a relaxing warmth throughout your groin.  ", false);
	//Too big
	if(player.vaginalCapacity() *1.5 >= monster.cockArea(0)) Render.text("Every stroke of his " + monster.cockDescriptShort(0) + " slides easily into your " + vaginaDescript(0) + ", allowing him to slowly build up a puddle of his wondrous druggy pre-seed into your aching womb.  ", false);
	//Resume!
	Render.text("The minotaur picks up the pace, heartbeat thudding through his rigid meat, making it convulse and vibrate as he slams you down on it over and over again.  You feel his balls twitching against your ass as his cock visibly thickens, bulging out at the base and slowly fattening inside you until it feels fit to burst.  With a throaty roar he cums at last, fountaining seed directly into your womb.  Your eyes roll back as it fills you, swooning as you feel the liquid pleasure seeping into you.  Your " + vaginaDescript(0) + " clamps down, trapping his seed inside, his orgasm dragging on as his balls begin to shrink.  The copious fluid fills your womb, distending your belly painfully.  The minotaur gives one last powerful spurt, the force pushing you off his " + monster.cockDescriptShort(0) + ", just as you both reach total sexual satisfaction.\n\n", false);
	//Check for stetchiness!
	player.cuntChange(monster.cockArea(0), true);

	//ITS OVER
	Render.text("He passes out with a contented sigh, balls shrunk down to the size of baseballs and his cock still dribbling cum.  You pull yourself to your feet unsteadily, drunk with pleasure as you stagger away, clamping your " + vaginaDescript(0) + " down unconsciously to hold in his wonderful seed.", false);
	if(player.vaginalCapacity() < monster.cockArea(0)) Render.text("\n\nMixed with the sheer sexual pleasure, you feel a diffuse pain and numbness coming from your much-abused " + vaginaDescript(0) + ", but decide with a smile that it was well worth it as his cum numbs away the last of the pain.", false);
	player.orgasm();
...
public minoPheromones():void {
	spriteSelect(44);
	Render.text("The minotaur smiles at you and lifts his loincloth, flicking it at you.  Thick ropes of pre-cum fly through the air, ", false);
	//sometimes get hit with the pre for stronger effect!
	if(rand(3) == 0) {
		Render.text("slapping into your face before you can react!  You wipe the slick snot-like stuff out of your eyes and nose, ", false);
		if(player.lust > 75) {
			Render.text("swallowing it into your mouth without thinking.  ", false);
			dynStats("lus", 10 + player.stats.lib/10);
		}
		else {
			Render.text("feeling your heart beat with desire as your tongue licks the residue from your lips.  ", false);
			dynStats("lus", 5 + player.stats.lib/20);
		}
	}
	else Render.text("right past your head.  ", false);
	Render.text("The animalistic scent of it seems to get inside you, the musky aroma burning a path of liquid heat to your groin.", false);
	dynStats("lus", 10 + player.stats.lib/20);
	if(player.perks.has("MinotaurCumAddict") || flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] == 2) {
...
public getRapedByMinotaur(autoRape:boolean = false):void {
	...
	if (!autoRape)
	{
		...
		//Oral rape chance
		if(rand(2) == 0 && player.lowerBody != LOWER_BODY.CENTAUR) {
	...
	{
		Render.text("As you take the winding path up through the rocky trail, you come upon the opening to a cave. Peering inside, the stench of an overpowering musk washes over you. The primal scent excites you, causing you to become aroused almost immediately.  Not thinking as clearly as you normally might, you slowly sneak your way into the cave. Signs of life litter the cave floor.\n\n", true);
		dynStats("lus", 10 + player.stats.lib / 5);
		
		//Detect minotaur coming
		if (rand(30) + player.stats.int / 5 > 18)
		{
			Render.text("You spot a shadow moving and spin around to see a minotaur lumbering after you from the back of the cave!", false);
			startCombat(new Minotaur());
			return;
		}
		
		Render.text("Suddenly you're grabbed from behind, your arms held together by a single massive, furry hand. A heavy, snorting breath brushes the top of your head. You turn your neck to see a massive bull-man. His impressive dick presses ", false);
		
		if (player.lowerBody == LOWER_BODY.CENTAUR)
		{
			Render.text("against your buttocks", false);
		}
		else
		{
			
			Render.text("into the small of your back", false);
		}
		
		Render.text(" as it grows larger and harder, smearing its pre-cum into your skin and making you shiver.  ", false);
		//High str escape
	...
	//Normal RAEP
	if(player.lowerBody.vaginaSpot.count() > 0) {
		Render.text("The bull-man roughly grabs your hair and begins rubbing the flared head of his penis along your " + vaginaDescript(0) + ".  ", false);
		if(player.averageVaginalWetness() < 2) Render.text("You aren't very wet, and fear the damage this beast will inflict on your " + vaginaDescript(0) + ".  ", false);
		else Render.text("You're shamefully wet by this point, and your knees are ready to buckle.  ", false);
		minoCumAddiction(5);
	}
	//BUTTSECKS!
	else {
		Render.text("The bull-man roughly grabs your hair and repositions himself to press against your asshole, slowly building the pressure until suddenly, the flared head pops into you.  ", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0 && player.mostBreastsPerRow() > 1 && player.upperBody.chest.count() > 0) {
		Render.text("He manhandles your tits as he does so, almost violently squeezing and stretching them to his enjoyment.  ", false);
		if(player.biggestLactation() > 1.5) Render.text("He then gives a grunt in what you take to be approval as your milk begins to squirt out. He licks a milk-coated finger in satisfaction.  ", false);
	}
	if(player.lowerBody.cockSpot.count() == 1) Render.text("The bull-man bends forward a little, and grabs your " + cockDescript(0) + " in a crushing grip. He makes short jerking motions as he keeps thrusting into you.", false);
	if(player.lowerBody.cockSpot.count() > 1) 	Render.text("The bull-man bends forward a little, and grabs one of your " + cockDescript(0) + "s in a crushing grip. He makes short jerking motions as he keeps thrusting into you.", false);
	if(player.lowerBody.vaginaSpot.count() > 0) {
	...
	if(player.lowerBody.vaginaSpot.count() > 0) {
		if(player.averageVaginalWetness() >= 2) {
			if(player.averageVaginalWetness() < 4) Render.text("You squirm and quiver, orgasming from the beast's rough usage.  ", false);
			if(player.averageVaginalWetness() == 4) Render.text("You squirm and quiver, orgasming from the beast's rough usage, soaking him with your " + vaginaDescript(0) + ".  ", false);
			if(player.averageVaginalWetness() == 5) Render.text("You orgasm on his massive rod, splattering the beast with girlcum.  ", false);
			player.orgasm();
			dynStats("lib", .5, "sen", 1, "cor", 1);
		}
		if(player.averageVaginalWetness() < 2) {
			Render.text("You gasp in pain, your cunt rubbed raw by the rough and violent penetration.  ", false);
	...
	if(player.lowerBody.cockSpot.count() > 0 && (player.stats.sens + rand(40) > 50)) {
		Render.text("You orgasm, ", false);
		if(player.cumQ() < 25) Render.text("spurting your seed helplessly from the brutal rape.  ", false);
		if(player.cumQ() >= 25 && player.cumQ() < 250) Render.text("squirting thick ropes of cum over the cave.  ", false);
		if(player.cumQ() >= 250 && player.cumQ() < 500) Render.text("erupting potent ropes of seed in thick bursts, splattering the cave walls and floors.  ", false);
		if(player.cumQ() >= 500) Render.text("erupting a thick torrent of seed that seems to go on forever, spurred by the constant pressure of the huge minotaur cock.  You paint the cave wall with cum, the beast egging you on as it roughly jacks your " + cockDescript(0) + ".  You are vaguely aware of your orgasm dragging on and on, until eventually your orgasm stops, leaving a sizable puddle of cum on the floor.  ", false);
		player.orgasm();
		dynStats("lib", .5, "sen", 1, "cor", 1);
	}
	Render.text("The bull-man relaxes for a moment, then shoves you off of him and to the cold ground. You pass out as a strange sense of euphoria washes over you while copious quantities of monstrous cum escape your distended ", false);
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("pussy.", false);
	else Render.text("asshole.", false);
...
private getOralRapedByMinotaur():void {
	spriteSelect(44);
	Render.text("", true);
	Render.text("The huge bull-man grabs you around the waist with one hand and flips you over. You hang upside down in his grip, your " + player.legs() + " splayed in the air.  The minotaur rubs your face across his sweat-slick balls, smearing his musk into your skin. Each of the minotaur's testicles is bigger than a man's head, and the skin of his sack is pulled taut over his cum-filled balls.\n\n", false);
	Render.text("The minotaur traps your " + player.legs() + " in one hand and slams your back against the cold cave wall. You gasp, spraying drops of musky sweat from your lips. Dragging you against the rough stone, the minotaur lifts you higher and bends your " + player.legs() + " back over a rock ledge. You are pinned against the cave wall, " + player.legs() + " held in one of the minotaur's hands while his other roughly gropes your stretched-out body.", false);
	if(player.hasBreasts()) Render.text("  The bull-man grabs your " + biggestBreastSizeDescript() + ", squeezing and mauling them with his calloused hand.", false);
	if(player.lowerBody.cockSpot.count() >= 1) {
		Render.text("  Your " + multiCockDescriptLight() + " slap", false);
		if(player.lowerBody.cockSpot.count() == 1) Render.text("s", false);
		Render.text(" painfully against your belly as the minotaur drags you along the wall.", false);
	}
	Render.text("  All you can see is the minotaur's huge shaft, the tip grinding against your mouth. The minotaur slaps his cock-head against your face, rubbing his musk and pre-cum into your cheeks and hair.\n\n", false);
	//new pg
	Render.text("The minotaur reaches down and grabs your jaw, yanking it open. You know what's coming next but decide not to fight it. Submitting to this beast is the safest choice. You spread your jaws as wide as you can and bend your mouth towards the minotaur's monstrous cock-head...\n\n", false);
	//new pg
	Render.text("Wasting no time, the minotaur grinds his glans past your lips and tries to force the tip of his " + monster.cockDescriptShort(0) + " into your mouth. Snorting and grunting, he pushes your head against the cave wall as he shoves the tip past your teeth. The minotaur's pre-cum pools in your mouth and dribbles out the corners, leaving trails of slime down your face.  You breathe through your nose, inhaling the minotaur's musk, as the brute forces another fraction of an inch of bull-cock into your mouth. Taking yet another quarter-inch of slimy meat, you wonder if it wouldn't be better to let yourself pass out while the minotaur uses you.\n\n", false);
	//new pg
	Render.text("You hear a <i>POP</i> and feel a sharp pain in your jaw as the minotaur forces its full cock-head into your mouth.  The head slides past your jaws and into your mouth, pinning your tongue down and jabbing against the opening of your throat. The flared end of the shaft wedges itself behind your teeth. The minotaur moans, a sound somewhere between a bull calling to its herd and a man groaning in ecstasy. You grab the bull-man's shaft in both hands and brace yourself, trying to pull your head away from the cave wall. The minotaur starts grinding its hips, and with one hand grabs your head and twists it from side to side, rubbing its huge tip around in your mouth.\n\n", false);
	//new pg
	Render.text("You arch your back off the wall, resting your weight on the minotaur's " + monster.cockDescriptShort(0) + ". Your chest and belly rub against the minotaur's massive torso", false);
	if(player.hasBreasts()) Render.text(", painfully grinding your " + nippleDescript(0) + " against his wiry fur", false);
	if(player.lowerBody.balls > 0) Render.text(", and you can feel the minotaur's hot breath on your " + ballsDescriptLight(), false);
	Render.text(". The bull-man snorts and jerks his hips as you twist your hands around his shaft, hoping to make him cum sooner. You tickle your tongue against his cock-head and start sucking as hard as you can. You bend your neck as the minotaur grinds itself in your mouth, hoping to make the tip easier to take. The minotaur jerks his hips in short spasms and you can hear its grunts coming closer and closer together. You take a deep breath through your nose, filling your lungs with air before the beast floods your throat with its cum...\n\n", false);
	//new pg
...
private minoGetsTitFucked():void {
	...
	Render.text("", true);
	Render.text("The helpless minotaur lays in the dirt, ", false);
	if(monster.HP < 1) Render.text("defeated.  ", false);
	else Render.text("masturbating uncontrollably.  ", false);
	Render.text("He's yours for the moment, and for right now, you want to see if that throbbing horse-cock will splatter its heavenly spunk all over your " + player.allBreastsDescript() + " and " + player.face() + ".  You really could use a fix, just enough to fuzz over all the worry and stress you've had to deal with.\n\n", false);
	if(monster.lust > 99) Render.text("You push his hands away and y", false);
	else Render.text("Y", false);
	Render.text("ou lean over him, feeling your generous breast-flesh jiggle, your " + nippleDescript(0) + "s tingling with desire as they rub across the horse-like minotaur dong.   The beast's pre starts forming into large drops along his flared head, and you waste no time smearing it through your cleavage, lubricating the space between your tits and breathing deeply of its perfect scent.", false);
	if(player.totalCocks() > 0) Render.text("  Your " + cockDescript(0) + " grows harder and more aroused than ever before while your ", false);
	else Render.text("  Your body aches with arousal while your ", false);
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(vaginaDescript(0) + " grows hotter and hotter, begging to be taken.", false);
	else Render.text(assholeDescript() + " puckers slightly, as if begging to be taken by another monstrous beast.", false);
	Render.text("\n\n", false);
	Render.text("He moos out pitifully, clearly understanding what you're up to and frustrated he won't get to sate his desire in a tight mortal ", false);
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("cunt.  ", false);
	else Render.text("ass.  ", false);
	Render.text("Out to prove him wrong by letting him release all over your face, you wrap your " + player.allBreastsDescript() + " tightly around him and begin sliding your body along his length.  The constant drizzling pre-cum easily lubricates you, and soon your orbs are shiny and slick with his drug-like fluid.\n\n", false);
	Render.text("As you begin shaking your breasts up and down, stroking his length, you feel the skin of his three rings of prepuce sliding the soaked skin of your tits.   You lick around the edge of his crown when it presents itself to your lips, making sure to take in a nice fat dollop of his thick cream.  It makes you dizzy for a moment, and you nearly slide off, but the sensation passes.  You need more.  You squeeze your " + player.allBreastsDescript() + " tighter around him, squeezing out a few more drops as you feel the manhood begin to swell, growing harder and larger as the minotaur's orgasm approaches.\n\n", false);
	Render.text("The minotaur's balls clench tightly as the bull-man throws his head back and screams out in pleasure and relief, \"<i>MooooOOOOOOooooOOOOO!</i>\"\n\n", false);
	Render.text("You squeeze tightly and press down, watching his dick-head flare out, nearly doubling size before it unloads, drenching your face and hair with his thick narcotic cream.   You manage to get your lips against it in time for the next blast, and nearly drown when it overfills your mouth.   You pull back, coughing, starting to get high, and swaying woozily as more and more cream spatters your body, continuing to be squeezed out by your slick heaving breasts.\n\n", false);  
	Render.text("The bull-man relaxes at last, drenching you with one last wave before he passes out.  You stagger up and nearly trip over your legs, giggling and drunk on cow-spunk as you noisily slurp down the fluids drenching your face and hair.  You black out for a while...\n\n", false);
	Render.text("You wake up an hour later, unsure how you made it back to your camp, light-headed and euphoric with the lingering after-affects.  You also find a bottle on the ground next to you, filled to the brim with the fluid.  You don't remember filling it up, but you're already itching to pop it open and guzzle it down.  You manage to resist the urge.  Besides, you're still feeling GREAT from your last 'sample'.", false);
	minoCumAddiction(10);
	monster.short = "tit-fucked Minotaur";
	player.orgasm();
...
private takeMinoCumDirectly():void {
	spriteSelect(44);
	Render.text("", true);
	Render.text("Relishing your victory, you breathe deeply, shivering and stretching.  ", false);
	if(monster.HP < 1) Render.text("The beast-man has fallen on his back, comatose and insensate, though his shaft has risen to full erectness, displacing his loincloth.", false);
	else Render.text("The beast-man is on his back with his legs splayed, masturbating hopelessly.  His anguished moans tell you all you need to know – he'll never get off alone.", false);
	Render.text("  You smile, blushing in anticipation, readying yourself for the coming bliss.\n\n", false);
	
	Render.text("You shed your " + player.armorName + " and step over its waist, trembling slightly with anticipation of your next fix.  Taking his flared drug-nozzle in hand, you hold it steady and lower yourself down, feeling his slippery narcotic pre-cum smearing over your hole.  ", false);
	//Too tight!
	if(player.analCapacity() < monster.cockArea(0)) {
		Render.text("You have to push harder and harder to get it inside you, even with the copious lube's drug-like affects relaxing your sphincter.  The penetration is slow, but inch by inch you force it inside you.", false);
	}
	else {
		Render.text("Your muscles instantly relax on contact with the pre, and you feel the massive tool sliding up your spacious asshole with ease.", false);
	}
	if(!player.buttChange(monster.cockArea(0), true)) Render.text("  ", false);
	Render.text("The minotaur's eyes open wide as he lets out a lustful '<i>Moooooo</i>'.  He doesn't struggle or even move – you're giving him exactly what he wants, after all.\n\n", false);

	Render.text("You bottom out, sighing happily as you feel his hot fluids leaking inside you, radiating pure pleasure out from your core.   Bestial hands grab your " + nippleDescript(0) + "s and begin gently twisting them, taking surprising care for all their strength.  Spurred on by the feelings radiating from your ", false);
	...
	Render.text(" and hungry for even more bliss, you begin bouncing atop the defeated beast, repeatedly squeezing and relaxing yourself around him, trying to wring out the maximum possible amount of fluid.\n\n", false);
	...
	Render.text("You zone out, dimly aware of your hands roaming over your body in a wave of orgiastic sensations.  The uncomfortable feeling of being packed so full is completely masked by the artificial drug that fills every pore of your body, and you keep bouncing not realizing that the minotaur has finished cumming.  You find yourself wishing life could be like this for every moment of every day, blissful and pleasure-filled.\n\n", false);
	
	Render.text("An hour or so later, you pick yourself up out of a puddle of mud that reeks of sex.  Still feeling groggy and euphoric, you get dressed, not bothering to clean up.  You have to work to keep your " + assholeDescript() + " clenched, lest you lose some of the drug making you so happy.", false);
	...
	if(player.stats.sens < 60) dynStats("sen", 1);
	if(player.stats.sens < 40) dynStats("sen", 1);
	if(player.stats.sens < 20) dynStats("sen", 1);
	//+15 addiction
	minoCumAddiction(15);
	cleanupAfterCombat();
}
...
public minoCumUpdate():boolean {
	if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_TRACKER] == 0) return false;
	let output:boolean = false;
	//Your addicted enough for it to be official!
	if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] == 0 && flags[FlagEnum.MINOTAUR_CUM_ADDICTION_TRACKER] >= 50) {
		...
		flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] = 1;
		output = true;
	}
	//If under 50, clear addiction and stuff
	if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_TRACKER] < 50) {
		...
		if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] > 0) {
			...
			if(player.stats.cor > 60) Render.text("but miss the tingling apathy and fuzziness that filled your head every time one of them was squirting inside you.", false);
			else Render.text("and resolve to give the beasts a wide berth from now on.", false);
			Render.text("</b>\n", false);
			flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] = 0;
			output = true;
		}
	}
	//If over 50, addicted, become addicted, withdrawl, etc
		...
		if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] > 1 && flags[FlagEnum.TIME_SINCE_LAST_CONSUMED_MINOTAUR_CUM] < 24) {
			Render.text("<b>\nYou sigh happily before you return to camp.  You got just what you needed.</b>\n", false);
			flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] = 1;
			output = true;
		}
		//Go into 'need' if its time.
		if(flags[FlagEnum.TIME_SINCE_LAST_CONSUMED_MINOTAUR_CUM] >= 24 && flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] == 1) {
			flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] = 2;
			output = true;
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00330] = 12;
		...
		if(flags[FlagEnum.TIME_SINCE_LAST_CONSUMED_MINOTAUR_CUM] >= 48 && flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] == 2) {
			flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] = 3;
			output = true;
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00330] = 12;
		...
		if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_TRACKER] >= 100) {
			if(player.perks.has("MinotaurCumAddict")) {}
			else {
				player.createPerk(PerkLib.MinotaurCumAddict,0,0,0,0);
				Render.text("<b>You are now a Minotaur Cum Addict</b> (Bottled Minotaur Cum now heals 25% of your HP, but causes HP damage to boost lust for a few hours.)\n", false);
				output = true;
			}
		}
		//3 lust an hour if in withdrawl, 1 if in need
		if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] == 3 && flags[FlagEnum.TIME_SINCE_LAST_CONSUMED_MINOTAUR_CUM] >= 48) {
			dynStats("lus", 2);
		}
		if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] == 2 && flags[FlagEnum.TIME_SINCE_LAST_CONSUMED_MINOTAUR_CUM] >= 48) {
			dynStats("lus", 1);
		}
	}

	//Decrement addiction value as needed
	if(flags[FlagEnum.TIME_SINCE_LAST_CONSUMED_MINOTAUR_CUM] >= 48 && !player.perks.has("MinotaurCumAddict")) {
		if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_TRACKER] >= 0.15) flags[FlagEnum.MINOTAUR_CUM_ADDICTION_TRACKER] -= 0.15;
	}
	//Increment time since last imbibing cum...
	//used for withdrawal and reducing overall addiction.
	flags[FlagEnum.TIME_SINCE_LAST_CONSUMED_MINOTAUR_CUM]++;
	//Minotaur cum item's status affect decrement
	if(flags[FlagEnum.MINOTAUR_CUM_REALLY_ADDICTED_STATE] > 0) {
		flags[FlagEnum.MINOTAUR_CUM_REALLY_ADDICTED_STATE]--;
		if(flags[FlagEnum.MINOTAUR_CUM_REALLY_ADDICTED_STATE] == 0) {
			Render.text("\n<b>Pain and pleasure are no longer so indistinguishable to you.\n</b>", false);
			output = true;
		}
		if(flags[FlagEnum.MINOTAUR_CUM_REALLY_ADDICTED_STATE] < 0) flags[FlagEnum.MINOTAUR_CUM_REALLY_ADDICTED_STATE] = 0;
	}
	return output;
}
...
private minotaurBlowjob():void {
	...
	Render.text("You step towards the minotaur, ", false);
	if(!player.isTaur()) Render.text("peeling away your " + player.armorName + " and ", false);
	Render.text("revealing your " + multiCockDescriptLight() + " to the defeated bull.  The beast looks to you in both arousal and shock, mouth parting slightly at the idea of another creature taking charge.  Within the musky stench of its dripping shaft you nearly lose yourself, and ", false);
	if(!player.isTaur()) Render.text("taking the bull by the horns", false);
	else Render.text("pinning his horns in the crook of your forelegs", false);
	Render.text(", you guide its mouth closer to your member as if on autopilot.  \"<i>Let's see just how good you are at milking,</i>\" you blurt; the minotaur moos in defiance but is soon silenced when the head of your " + Appearance.cockNoun(player.lowerBody.cockSpot.list[x].cockType) + " presses into its bovine mouth.\n\n", false);

	Render.text("You grimace at the awkward feeling of the beast's snout against you, but soon regain confidence in your choice as the monster begins to take your prick into his mouth.  His eyes grow wide as his lips wrap around you, with ", false);
	if(monster.lust > 99) Render.text("his own burning lusts", false);
	else if(player.perks.has("ArousingAura")) Render.text("the power of your corruption", false);
	else Render.text("the salty taste of your pre-cum", false);
	Render.text(" leaving him hungry for more.\n\n", false);

	Render.text("At this realization he attempts to pull away, to save himself from the ordeal, but in his debilitated state his efforts are as weak as a kitten's. Held firmly by the horns, the bull-man slowly succumbs.  Unable to control himself in his lust, he licks at the cock within his mouth wildly, his thick cow-like tongue roughly wrapping around your length as you move yourself further within his sucking mouth.  From the combined effects of the creature's musk and maw you start to lose yourself as well, ", false);
	if(!player.isTaur()) Render.text("finding your hands leaving his horns for the back of his head ", false);
	Render.text("as your hips begin to thrust into his face.  In your daze you glance down, to see the minotaur roughly and futilely stroking his shaft, his eyes full of shame and desperation - full of wanting to not only release his pent-up arousal, but to taste your thick cum as well.\n\n", false);
	
	Render.text("You soon oblige the bull; your effort to fuck his face and the rapid bobbing of his head along your member quickly pass what little restraint you care to exercise on the monster's behalf.  Slamming the muzzle into your groin and leaning over his back, you begin layering the minotaur's tongue and tonsils with bursts of your seed.  Reaching down with ", false);
	if(!player.isTaur()) Render.text("your hand", false);
	else Render.text("a forehoof", false);
	Render.text(", you quickly put a stop to his unimportant attempts to pleasure himself by knocking his hand from his meaty cock.  The bull moos over your prick in response, leaving you to shudder while he swallows every last drop of your cum.  You are unable to tell if the beast is blushing from arousal or being reminded of his submission, but it does not matter.\n\n", false);
	
	Render.text("Pulling yourself free from the thing's mouth, you let the abused creature fall flat onto its chest and then gather your armor to leave.  Once, you turn back, only to catch sight of the minotaur lost with himself, desperatly trying to fuck the ground beneath him and mooing in overstimulated agony from the experience.", false); 
	player.orgasm();
	cleanupAfterCombat();
}
...
private layEggsInAMinotaurSpiderLike():void {
	...
	Render.text("The defeated minotaur turns and tries to flee.  You quickly fling a cord of sticky webbing that wraps around his massive horns, hauling him up short.  With a panicked moo, he begins to thrash his head, desperately trying to free himself.  You scurry forward and tackle the man-bull, forcing him onto his back.  Extruding more webbing, you wrap his wrists and bind them to his horns, then force his muscular legs upward, bending him almost double before lashing his legs to his horns as well.");
	...
	if(!silly()) Render.text("his bovine rosebud.");
	else Render.text("the entrance to his O.K. Corral.");
	Render.text("  Grinning, you lift your abdomen high behind you, then snap it forward, forcing more than half of your long ovipositor deep into your beef-bitch.");
	
	if(player.gender > 0) {
		Render.text("\n\n");
		if(player.lowerBody.cockSpot.hasCock()) Render.text("Forcing [oneCock] between his bound thighs, you frot his flared bull-cock, your mixed pre soaking into the fur of his belly as you thrust against him.");
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("Grasping his bull-cock, you pull it against your pussy, rubbing your juices up and down his impressive length as your lips and clit grind on him, adding to your pleasure.");
	}

	//(If male or herm: 
	Render.text("\n\n");
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("As your orgasm washes over you, you spray your cum, ");
		if(player.cumQ() < 100) Render.text("spattering");
		else if(player.cumQ() < 500) Render.text("drenching");
		else Render.text("nearly drowning");
		Render.text(" your helpless prey.  ");
	}
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("Your femcum sprays across his cock.  ");
	Render.text("Simultaneously, your ovipositor pulses and throbs, forcing egg after egg between his beefy buns and deep into him.  The ");
	if(player.gender > 0) Render.text("combination of the scent of your fluids and the ");
	Render.text("sensation of eggs forcing their way past his sphincter proves too much for your bovine bitch to resist, and he helplessly sprays his musky cum, stream after stream jetting up his chest and across his face as his eyes roll back in his head.");
	//[(mino addict)
	if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] >= 1) {
		Render.text("  The smell is too much for you as well, and as you come a second time, you point his flare at your mouth, gulping as much of his seed as you can get.");
		minoCumAddiction(3);
	}
...
private layBeeEggsInCowbutt():void {
	clearOutput();
	Render.text("Looking down at the defeated minotaur");
	if(monster.lust > 99) Render.text(", stroking his flared cock desperately"); 
	Render.text(" you feel a throbbing sensation in your apiary appendage.  A leering grin spreads across your face as you force the minotaur onto his stomach, trapping his musky manhood beneath him.");
	if(!player.isTaur()) Render.text("  Wrapping his tail around one hand, you lift his furred flanks up");
	else Render.text("  Straddling him with all four legs, you slide forward");
	Render.text(" while aiming your ovipositor.");
	...
	if(player.stats.cor < 50) Render.text("You hesitate, feeling a surge of pity for the once-mighty creature, but your new instincts drive your egg-layer further into his prone body.  ");
	else Render.text("Grinning cruelly, you grasp him by the horns and haul back with both hands until the minotaur is forced to look you in the eye.  You lean in close to his snout and buzz mockingly, \"<i>Whoz the bitch now?  Iz it you?  Yezz, it izz!</i>\"  ");
	Render.text("You begin to thrust back and forth, your strong forward strokes punctuated by grunting moos from your huge but helpless brood-bull.");
	
	Render.text("\n\nYour body, long-ready to lay, quickly achieves a climax; ");
	//(if male or herm:
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("you thrust your cock");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" through the air harder and faster, drenching his ");
		//(if cum volume low: 
		if(player.cumQ() < 250) Render.text("backside with your cum");
		else if(player.cumQ() < 500) Render.text("back with your cum, with ropes dripping slowly off his horns and down his face");
		else Render.text("back with your cum, before it covers him and spreads across the floor of his cave.");
	}
	//(if female: 
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("you rub your pussy hard against the minotaur's muscular ass, marking him with the scent of your fem-cum.");
...
private getMinoHawtDawged():void {
	clearOutput();
	if(player.lust > 99) {
		Render.text("You collapse to your knees, hands racing to your crotch and your ");
		//(If Male or Herm: 
		if(player.lowerBody.cockSpot.hasCock()) Render.text("[cock biggest], which you frantically try to stroke and rub through your [armor], its throbbing arousal clearly evident.");
		//If Herm: 
		if(player.gender == 3) Render.text("  Your other hand races to your ");
		//(If Female or Herm: 
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("[vagina], which is dripping and drooling your arousal down the insides of your [armor].  You manage to apply some pressure to your [clit] through your [armor], which sends a shock of pleasure through you.");
	}
	//If PC lost by damage, and Minotaur has axe: 
	else if(monster.weaponName == "axe") Render.text("The last great blow from the Minotaur's giant, man-slaughtering axe is just a little off, thankfully - the flat of the axe, rather than the blade, hits you upside the flat of your head, sending you tumbling into the dirt.");
	//(If PC lost by damage, and Minotaur does not:
	else Render.text("The last great blow from the Minotaur's giant, beefy fist hits you upside your head, the blow strong enough to send you tumbling into the dirt.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("[eachCock] twitching beneath you, painting the floor with dribbles of your pre.");
	if(player.gender == 3) Render.text("  You also feel ");
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("your [vagina] clenching and unclenching as your entire hips try to push against the Minotaur's searching tongue, dripping with his spit - along with your own juices.");
