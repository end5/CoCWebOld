...
private function rapeMinotaurTentacles():void {
	...
	if(player.stats.cor < 25) outputText("You cry, petting his hair, smoothing it across his horns, justifying to yourself that you are raping this beast because this is how this world works as you maneuver your " + multiCockDescript() + " towards his tailhole.  ", false);
	if(player.stats.cor >= 25 && player.stats.cor < 50) outputText("You smoothly guide your " + multiCockDescript() + " toward his tailhole.  ", false);
	if(player.stats.cor >= 50 && player.stats.cor < 80) outputText("You roughly grab him, slamming him close to your " + multiCockDescript() + " - just how this beast likes it.  ", false);
	if(player.stats.cor >= 80) outputText("You grin and roughly pull his horns towards your " + multiCockDescript() + ", ready to roughly ride this beast.  ", false);
	//Mixed dix
...
private function AddictNagaOnMinotaur():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("furiously stroke your own " + multiCockDescriptLight() + ". Your muffled vocalizations of pleaure weakly join his own.\n\n", false);
...
private function minoRapeIntro():void {	
	...
	if(monster.HP <= 0) {
		...
		if(player.lowerBody.vaginaSpot.count() > 0) outputText("  Glancing back at you with hope in his eyes, the minotaur's " + monster.cockDescriptShort(0) + " twitches as he lays his gaze upon your " + vaginaDescript(0) + ".", false);
	...
	//Monster lost to lust
		...
		//NIPPULAR ENJOYMENT
		if(player.upperBody.chest.countNipples() > 1) outputText(" and nipples", false);
		outputText(".", false);
		//Vagina or not flavor texts
		if(player.lowerBody.vaginaSpot.count() > 0) outputText("  Glancing back at you with hope in its eyes, the minotaur's " + monster.cockDescriptShort(0) + " twitches as he lays his gaze upon your " + vaginaDescript(0) + ".", false);
...
private function bumRapeaMinotaur():void {
	...
	//Regular or multi...
	if(player.lowerBody.cockSpot.count() == 1) outputText(cockDescript(x), false);
...
private function girlRapeAMinotaur():void {
	...
	if(player.lowerBody == LOWER_BODY.CENTAUR) outputText("hindquarters, forcefully grinding your vagina along his length like a toy.  ", false);
...
private function minotaurGetsRapedByHerms():void {
	...
	player.cuntChange(monster.cockArea(0), true);
	if(player.vaginas[0].vaginalLooseness < VAGINA_LOOSENESS.GAPING) outputText("A small suppressed part of your mind dimly registers that this should be fairly painful, but the lust and drug-like pre-cum wash that thought away with waves of pleasure.  ", false);
	if(player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.GAPING) outputText("The intense feelings seem to grow more and more powerful as he stays inside you, his drug-like pre-cum making your " + vaginaDescript(0) + " tingle from the inside-out.", false);
...
public function getRapedByMinotaur(autoRape:boolean = false):void {
	...
	if(player.lowerBody.cockSpot.count() == 1) outputText("The bull-man bends forward a little, and grabs your " + cockDescript(0) + " in a crushing grip. He makes short jerking motions as he keeps thrusting into you.", false);
	if(player.lowerBody.cockSpot.count() > 1) 	outputText("The bull-man bends forward a little, and grabs one of your " + cockDescript(0) + "s in a crushing grip. He makes short jerking motions as he keeps thrusting into you.", false);
	if(player.lowerBody.vaginaSpot.count() > 0) {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("pussy.", false);
...
private function getOralRapedByMinotaur():void {
	...
	if(player.lowerBody.balls > 0) outputText(", and you can feel the minotaur's hot breath on your " + ballsDescriptLight(), false);
...
private function minoGetsTitFucked():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript(0) + " grows hotter and hotter, begging to be taken.", false);
...
public function minoCumUpdate():boolean {
	...
	if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_TRACKER] < 50) {
		...
		if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] > 0) {
			...
			if(player.stats.cor > 60) outputText("but miss the tingling apathy and fuzziness that filled your head every time one of them was squirting inside you.", false);
...
public function minoAddictionBadEndEncounter():void {
	...
	if(flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] == 3) {
		...
		//(if smart)
		if(player.stats.int > 40) outputText("A tiny voice speaks up, warning you that it would be hard to get away from such a gathering.  ", false);
...
private function minoAddictionBadEnd2():void {
	...
	else if(player.lowerBody.cockSpot.count() > 0) outputText("hard ", false);
...
private function minotaurBlowjob():void {
	...
	outputText(", you guide its mouth closer to your member as if on autopilot.  \"<i>Let's see just how good you are at milking,</i>\" you blurt; the minotaur moos in defiance but is soon silenced when the head of your " + Appearance.cockNoun(player.lowerBody.cockSpot.list[x].cockType) + " presses into its bovine mouth.\n\n", false);
	...
	else if(player.perks.has("ArousingAura")) outputText("the power of your corruption", false);
...
private function layEggsInAMinotaurSpiderLike():void {
	...
	if(player.gender > 0) {
		outputText("\n\n");
		if(player.lowerBody.cockSpot.hasCock()) outputText("Forcing [oneCock] between his bound thighs, you frot his flared bull-cock, your mixed pre soaking into the fur of his belly as you thrust against him.");
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Grasping his bull-cock, you pull it against your pussy, rubbing your juices up and down his impressive length as your lips and clit grind on him, adding to your pleasure.");
	}
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Your femcum sprays across his cock.  ");
...
private function layBeeEggsInCowbutt():void {
	...
	if(player.stats.cor < 50) outputText("You hesitate, feeling a surge of pity for the once-mighty creature, but your new instincts drive your egg-layer further into his prone body.  ");
	...
	//(if female: 
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("you rub your pussy hard against the minotaur's muscular ass, marking him with the scent of your fem-cum.");
...
private function getMinoHawtDawged():void {
	...
	if(player.lust > 99) {
		...
		//(If Male or Herm: 
		if(player.lowerBody.cockSpot.hasCock()) outputText("[cock biggest], which you frantically try to stroke and rub through your [armor], its throbbing arousal clearly evident.");
		//If Herm: 
		if(player.gender == 3) outputText("  Your other hand races to your ");
		//(If Female or Herm: 
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("[vagina], which is dripping and drooling your arousal down the insides of your [armor].  You manage to apply some pressure to your [clit] through your [armor], which sends a shock of pleasure through you.");
	}
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("[eachCock] twitching beneath you, painting the floor with dribbles of your pre.");
	if(player.gender == 3) outputText("  You also feel ");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your [vagina] clenching and unclenching as your entire hips try to push against the Minotaur's searching tongue, dripping with his spit - along with your own juices.");
