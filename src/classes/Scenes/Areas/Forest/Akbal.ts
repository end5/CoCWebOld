export default class Akbal extends Monster {
	...
	public eAttack(): void {
		...
		if (statusAffects.has("Blind")) {
			MainScreen.text(capitalA + short + " seems to have no problem guiding his attacks towards you, despite his blindness.\n", false);
		}
		//Determine if dodged!
		if (player.stats.spe - spe > 0 && int(Math.random() * (((player.stats.spe - spe) / 4) + 80)) > 80) {
			if (player.stats.spe - spe < 8)
				MainScreen.text("You narrowly avoid " + a + short + "'s " + weaponVerb + "!", false);
			if (player.stats.spe - spe >= 8 && player.stats.spe - spe < 20)
				MainScreen.text("You dodge " + a + short + "'s " + weaponVerb + " with superior quickness!", false);
			if (player.stats.spe - spe >= 20)
				MainScreen.text("You deftly avoid " + a + short + "'s slow " + weaponVerb + ".", false);
		...
		if (player.perks.has("Flexibility") && rand(100) < 10) {
			MainScreen.text("Using your cat-like agility, you twist out of the way of " + a + short + "'s attack.", false);
			game.combatRoundOver();
			return;
		}
		...
		if (rand(2) == 0) {
			...
			if (damage <= 0) {
				MainScreen.text("Akbal lunges forwards but with your toughness", false);
				if (player.armorDef > 0)
					MainScreen.text(" and " + player.inventory.armor.displayName + ", he fails to deal any damage.", false);
				else
					MainScreen.text(" he fails to deal any damage.", false);
			}
			else {
				MainScreen.text("Akbal rushes at you, his claws like lightning as they leave four red-hot lines of pain across your stomach.", false);
				player.takeDamage(damage);
			}
		} else { //*Normal Attack B
			//(high HP damage)
			damage = int((str + 25 + weaponAttack) - Math.random() * (player.stats.tou) - player.armorDef);
			if (damage == 0) {
				MainScreen.text("Akbal lunges forwards but between your toughness ", false);
				if (player.armorDef > 0)
					MainScreen.text("and " + player.inventory.armor.displayName + ", he fails to deal any damage.", false);
			}
			else {
				MainScreen.text("Akbal snarls as he flies towards you, snapping his ivory teeth on your arm. You scream out in pain as you throw him off.", false);
				player.takeDamage(damage);
			}
		}
		game.combatRoundOver();
	...
	public akbalLustAttack(): void {
		//*Lust Attack - 
		if (!player.statusAffects.has("Whispered")) {
			MainScreen.text("You hear whispering in your head. Akbal begins speaking to you as he circles you, telling all the ways he'll dominate you once he beats the fight out of you.", false);
			//(Lust increase)
		...
		//Continuous Lust Attack - 
			...
			MainScreen.text("The whispering in your head grows, many voices of undetermined sex telling you all the things the demon wishes to do to you. You can only blush.", false);
			//(Lust increase)
	...
	public akbalSpecial(): void {
		...
		if (rand(2) == 0 && player.stats.spe > 20) {
			...
			MainScreen.text("Akbal's eyes fill with light, and a strange sense of fear begins to paralyze your limbs.", false);
			//(Speed decrease)
		...
		//*Special Attack B - 
			...
			MainScreen.text("Akbal releases an ear-splitting roar, hurling a torrent of emerald green flames towards you.\n", false);
			//(high HP damage)
			//Determine if dodged!
			if (player.stats.spe - spe > 0 && int(Math.random() * (((player.stats.spe - spe) / 4) + 80)) > 80) {
				if (player.stats.spe - spe < 8)
					MainScreen.text("You narrowly avoid " + a + short + "'s fire!", false);
				if (player.stats.spe - spe >= 8 && player.stats.spe - spe < 20)
					MainScreen.text("You dodge " + a + short + "'s fire with superior quickness!", false);
				if (player.stats.spe - spe >= 20)
					MainScreen.text("You deftly avoid " + a + short + "'s slow fire-breath.", false);
				game.combatRoundOver();
				return;
			}
			//Determine if evaded
			if (player.perks.has("Evade") && rand(100) < 20) {
				MainScreen.text("Using your skills at evading attacks, you anticipate and sidestep " + a + short + "'s fire-breath.", false);
				game.combatRoundOver();
				return;
			}
			//Determine if flexibilitied
			if (player.perks.has("Flexibility") && rand(100) < 10) {
				MainScreen.text("Using your cat-like agility, you contort your body to avoid " + a + short + "'s fire-breath.", false);
				game.combatRoundOver();
				return;
			}
			MainScreen.text("You are burned badly by the flames! (" + player.takeDamage(40) + ")", false);
			;
		}
		game.combatRoundOver();
	}
	...
	public akbalHeal(): void {
		if (HPRatio() >= 1)
			MainScreen.text("Akbal licks himself, ignoring you for now.", false);
		else
			MainScreen.text("Akbal licks one of his wounds, and you scowl as the injury quickly heals itself.", false);
		addHP(30);
		lust += 10;
		game.combatRoundOver();
	}
