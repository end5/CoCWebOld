...
class Attack implements CombatAction {
    ...
    public use(akbal: Character, enemy?: Character) {
        ...
        // Determine if dodged!
        if (enemy.stats.spe - akbal.stats.spe > 0 && Utils.rand(((enemy.stats.spe - akbal.stats.spe) / 4) + 80) > 80) {
        ...
        // Determine if evaded
        if (enemy.perks.has(PerkType.Evade) && Utils.rand(100) < 10) {
        ...
        if (enemy.perks.has(PerkType.Flexibility) && Utils.rand(100) < 10) {
...
class Special implements CombatAction {
    ...
    public use(akbal: Character, enemy?: Character) {
        // *Special Attack A -
        if (Utils.rand(2) === 0 && enemy.stats.spe > 20) {
        ...
        // *Special Attack B -
            ...
            // Determine if evaded
            if (enemy.perks.has(PerkType.Evade) && Utils.rand(100) < 20) {
...
class AkbalEndScenes extends EndScenes {
    ...
    protected victoryScene(howAkbalWon: DefeatType, enemy: Character): void {
        if (howAkbalWon === DefeatType.HP) {
            Game.scenes.forest.akbalScene.akbalWon(enemy, true, false);
        }
        else if (howAkbalWon === DefeatType.Lust) {
            Game.scenes.forest.akbalScene.akbalWon(enemy, false, enemy.statusAffects.has(StatusAffectType.Infested));
        }
    }
    ...
    protected defeatScene(howAkbalLost: DefeatType, enemy: Character): void {
        if (howAkbalLost === DefeatType.HP) {
            Game.scenes.forest.akbalScene.akbalDefeated(enemy, true);
        }
        else if (howAkbalLost === DefeatType.Lust) {
            Game.scenes.forest.akbalScene.akbalDefeated(enemy, false);
        }
    }

    public defeatAftermath(howAkbalLost: DefeatType, enemy: Character): void { }

    public victoryAftermath(howAkbalWon: DefeatType, enemy: Character): void { }
}
...
export default class Akbal extends Character {
    public constructor() {
        ...
        this.combat.rewards.drop = new WeightedDrop().
            ...
            add(weapons.PIPE, 1);
        this.combatContainer = new CombatContainer(this, new AkbalActions(), undefined, new AkbalEndScenes(this));
    }
}
