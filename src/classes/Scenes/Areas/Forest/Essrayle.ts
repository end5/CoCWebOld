export default class Essrayle {
	...
	public essrayleMeetingI(): void {
		...
		if (Flags.list[FlagEnum.MET_ESSY] == 0) {
			...
			if (player.stats.cor < 33) MainScreen.text("Despite however chivalric you think yourself");
			else if (player.stats.cor < 66) MainScreen.text("Despite your better sense");
			else MainScreen.text("Perhaps best of all");
			MainScreen.text(", your attention falls to her chest.  This emerald maiden sports a pair of breasts that'd make even a demon blush, gorgeous green globes that look like small watermelons resting upon her upper belly, capped with purple areolae.  Four, thick, teat-like nipples dangle a good three or four inches from each.");
			...
			if (player.stats.cor < 33) MainScreen.text("hastily looking away from her nude body");
			else if (player.stats.cor < 66) MainScreen.text("looking away out of courtesy");
			else MainScreen.text("letting your eyes roam over her as you search for even more depraved features on her nubile body");
			MainScreen.text(".");
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 10) MainScreen.text("own");
			else MainScreen.text("[chest]");
			MainScreen.text(".  \"<i>You must be one of those adventurers I've been hearing about, am I right?</i>\"");
		...
		Flags.list[FlagEnum.MET_ESSY]++;
		//[Yes] [No]
		menu();
		MainScreen.addButton(1, "Yes", plantsForMe);
		MainScreen.addButton(2, "No", noPlantsForMe);
	...
	private noPlantsForMe(): void {
		MainScreen.clearText();
		MainScreen.text("Essy pouts, ears flattening to the sides a bit.  \"<i>I see,</i>\" she nods, looking at you with thinly masked disappointment.  She seems to instantly put on a false smile and move aside, gesturing to the path ahead with a sweep of the arm.  \"<i>Well, in that case, you must be going somewhere important.  Best not keep you waiting.  I do wish you luck in your adventures!</i>\" She continues to beam as you head on off, leaving her behind you in no time.  Though as you leave, you swear you hear a mumbled, \"<i>Leave it to me to find the prudish ones.</i>\"");
		Flags.list[FlagEnum.TURNED_DOWN_ESSY_FIRST_MEETING] = 1;
	...
	private plantsForMe(): void {
		MainScreen.clearText();
		if (Flags.list[FlagEnum.ACCEPTED_ESSY_FIRST_MEETING] == 0) {
			Flags.list[FlagEnum.ACCEPTED_ESSY_FIRST_MEETING] = 1;
			MainScreen.text("Essrayle giggles, sending ripples through her ample breasts.  \"<i>Oh, why thank you!  It's nothing major - I doubt it'll be much of a burden to you.  But in my travels, I've grown wilted and depleted of the energy to go on.  I'm soooo hungry!  Would you please-?</i>\"  She leaves the question hanging, looking to you imploringly.  Hastily your hand goes to where your supplies are housed, but you stop as she waves a hand dismissively.  \"<i>No, no, I don't mean biscuits or the like.</i>\"  Confused, you frown, but soften as you feel her hand rest upon your [chest].  \"<i>I require a special kind of food to do what I do, one that won't exhaust your supplies in the least.</i>\"\n\n");
		}
		//(If breasts
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) MainScreen.text("Your nipples stiffen, breasts puffing out with a heated breath");
		else MainScreen.text("You arch your back gently, unconsciously pressing into her slender fingers a bit further");
		MainScreen.text(" as her hand begins to circle your chest.  \"<i>Though you, that's another story.</i>\"  She grins seductively as the air around you seems to thicken with a gentle pink mist.  Out of instinct, you look around, trying to discern the source - could she be one of the demons?");
		...
		if (player.upperBody.chest.get(0).breastRating >= 1) {
			...
			if (player.upperBody.chest.count() > 1) MainScreen.text(" of your uppermost pair");
			MainScreen.text(".  You moan, and this only seems to encourage her, as she takes your nipples between thumb and forefinger, twisting and turning them this way and that.  She giggles at how your moans seem to fluctuate by coincidence, continuing to tease them");
			if (player.upperBody.chest.count() > 1) MainScreen.text(", steadily working over the multitude of fleshy mounds on your body");
			MainScreen.text(".");
			if (player.upperBody.chest.hasFuckableNipples()) MainScreen.text("  Her fingers play with your nipples and press gently to tease at them.  She seems quite surprised as they sink in with minimal effort.  A grin spreads over her face as she begins to steadily thrust those digits in and out of your nipples, fingers swirling about the interiors all the while.");
			MainScreen.text("  \"<i>You certainly have a lovely pair, but they could always be better, don't you think?</i>\"");
			//[Yes] [No]
			menu();
			MainScreen.addButton(1, "Yes", plantsForMe2, 1);
			MainScreen.addButton(2, "No", plantsForMe2, 2);
		}
		//Else
		else {
			//[Next]
			menu();
			MainScreen.addButton(0, "Next", plantsForMe2, 0);
		}
	}
	...
	private plantsForMe2(BE: number = 0): void {
		...
		if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("find the dual sexes there.  Another hand joins the first, one gripping and tugging on your hardening cock as the other cups and squeezes over your pussy.");
		else if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text("circle about ");
			...
			MainScreen.text(", beginning to squeeze and slide about its length until your nursing is interrupted by moans of your own delight");
		}
		else MainScreen.text("stroke about the puffy, now-inflamed lips of your own flower, taking the lips between two fingers and stroking up and down across their length fully before switching to your moist core.");

		MainScreen.text("\n\nA firm hand cups your ");
		if (player.lowerBody.isTaur()) MainScreen.text("back");
		else MainScreen.text("butt");
		MainScreen.text(" and pulls you close, forcing your [legs] against her bulb.  They sink in slightly as you go down, reluctantly relinquishing her nipple from your mouth.  As it springs free, a thick glob of sap splatters over your lips and nose.  Essy giggles softly as she wipes them off with a hand.");
		...
		MainScreen.text("and [legs], ");
		if (!player.lowerBody.isTaur()) MainScreen.text("lifting you slowly into the air");
		else MainScreen.text("forcing you to rear up");
		MainScreen.text(".  Her lower hands grasp you, helping to raise you up until you're at the proper height, whence she abruptly plants her lips upon your crotch.");
		...
		if (player.lowerBody.cockSpot.hasCock()) {
			...
			if (player.upperBody.chest.get(0).breastRating >= 1) MainScreen.text(" and " + BreastDescriptor.breastCup(player.upperBody.chest.get(0)) + " breasts");
			MainScreen.text(".  The opaque cups pump and suck, Essy humming to herself as the base of her tongue continues to skillfully tease over every hot spot near the [cockHead biggest] of your [cock biggest], her length slithering about it skillfully.");
		}
		//(Pussy=
		else {
		...
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) {
			...
			if (player.lactationQ() >= 400) MainScreen.text("your milk and ");
			MainScreen.text("your pleasure.  In time, their pumping increases in force and tempo just as her lips and tongue do.");
		}
		...
		if (BE == 1) {
			...
			if (player.upperBody.chest.countNipples() > 2) MainScreen.text("s have");
			else MainScreen.text(" has");
			MainScreen.text(" been swallowed up as well.");
			...
			if (player.upperBody.chest.count() > 1) MainScreen.text("  She eventually dislodges with a wet pop, setting your new breasts to wobbling about as she moves down your body, repeating the gestures upon each set of breasts in return.");

			MainScreen.text("\n\nWeak from the pleasure, the last thing you see is her smiling face before you fall forward into her embrace.  The next thing you know, you're alone in the glade, staring at the canopy above.  There's a heavy weight on your chest and you struggle to sit up.  " + Num2Text(player.upperBody.chest.count() * 2) + " massive breasts jiggle and sway upon your chest, all full and aching to be milked");
			if (player.upperBody.chest.count() > 1) MainScreen.text(", the top pair being largest, descending in size with each subsequent pair");
			MainScreen.text(".");
			//Boost size, set lactation quantity.
			player.growTits(7, player.upperBody.chest.count(), false, 2);
	...
	private feedTrappedEssy(): void {
		MainScreen.clearText();
		MainScreen.text("Well, it's not like she's going anywhere right now, and she probably would love a good fuck from a friend anyway.");
	...
	private hasCockFeedEssy(newPage: boolean = true): void {
		...
		if (player.biggestCockLength() < 20) MainScreen.text("her lower pair of wobbling udders");
		else MainScreen.text("both sets of milk-bloated breasts");
		MainScreen.text(" faster and faster, the hot slapping of her bountiful bosom against your [cock]");
		if (player.lowerBody.balls > 0) MainScreen.text(" and your swollen, heavy balls against her fecund belly");
		MainScreen.text(" setting an inspiring drum beat to your feverish pace.");
	...
	public essyWitchVictory(): void {
		...
		Flags.list[FlagEnum.ESSRAYLE_ESCAPED_DUNGEON] = 1;
		menu();
		MainScreen.addButton(0, "Yes", acceptEssyPrizes);
		MainScreen.addButton(1, "No", declineEssyPrizes);
	}

	//[No]
	private declineEssyPrizes(): void {
		MainScreen.clearText();
		MainScreen.text("Essy sighs and shakes her head.  \"<i>What a let down!</i>\"  She shrugs, throwing all four arms into the air helplessly, and digs about in the pouch she keeps on her vine belt.  \"<i>Here, hope this tickles your fancy.</i>\"  She reaches out and places a number of glittering gems into your hand.  \"<i>They don't do me much good anyway.</i>\"");

		//(Player gains some more gems)
		player.stats.gems += 100 + rand(70);
		statScreenRefresh();
	...
	private acceptEssyPrizes(): void {
		...
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating <= 1) {
			MainScreen.text("four bulging tits erupt from your chest");
			player.upperBody.chest.get(0).breastRating = 4;
			if (player.upperBody.chest.count() == 1) player.createBreastRow();
			player.upperBody.chest.get(1).breastRating = 4;
		}
		//if two breasts:
		else if (player.upperBody.chest.count() == 1) {
			MainScreen.text("two more bulging tits erupt from your chest");
			if (player.upperBody.chest.get(0).breastRating < 4) {
				MainScreen.text(" while your top row expands");
				player.upperBody.chest.get(0).breastRating = 4;
			}
			if (player.upperBody.chest.count() == 1) player.createBreastRow();
			player.upperBody.chest.get(1).breastRating = player.upperBody.chest.get(0).breastRating;
		}
		//if four+ breasts: 
		else {
			MainScreen.text("your [fullChest] swell larger and larger under the expanding magic of the spell");
			player.growTits(6, player.upperBody.chest.count(), false, 2);
		}
		player.boostLactation(player.upperBody.chest.count());
		MainScreen.text(", filling with warm, fluid weight.  They jiggle and wobble against each other and your belly as they adjust to their brimming plumpness.  You now proudly sport [fullChest].");
	...
	private noGimmeGiantNipplesEssy(): void {
		MainScreen.clearText();
		MainScreen.text("She shrugs her shoulders, giving one bundle of milk-drooling nipples a soft squeeze.  \"<i>Eh, to each their own, I suppose.</i>\"");
		essyRewardEpilogueOUTTIES();
	}
	...
	public askMotherToReleaseEssy(): void {
		MainScreen.clearText();
		if (Flags.list[FlagEnum.SAND_WITCHES_COWED] == 0) {
			MainScreen.text("You point out that the witches have a friend of yours trapped here with magic and you'd like her released.");
			...
			MainScreen.text("You command the Sand Mother to release your friend, Essrayle from her magical bondage.  The sorcerous queen looks about to protest, but after a brief pause, she admits, \"<i>Fine, I'll see her released.  You can go pick her up.</i>\"  She practically spits the last sentence.");
			MainScreen.text("\n\nThat'll do.");
		}
		Flags.list[FlagEnum.TOLD_MOTHER_TO_RELEASE_ESSY] = 1;
		doNext(playerMenu);
	}
}
}
