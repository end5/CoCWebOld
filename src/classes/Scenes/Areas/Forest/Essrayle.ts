...
public function essrayleMeetingI():void {
	...
	if(flags[FlagEnum.MET_ESSY] == 0) {
		...
		if(player.stats.cor < 33) outputText("Despite however chivalric you think yourself");
		else if(player.stats.cor < 66) outputText("Despite your better sense");
		...
		if(player.stats.cor < 33) outputText("hastily looking away from her nude body");
		else if(player.stats.cor < 66) outputText("looking away out of courtesy");
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 10) outputText("own");
...
private function plantsForMe():void {
	...
	//(If breasts
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText("Your nipples stiffen, breasts puffing out with a heated breath");
	...
	if(player.upperBody.chest.list[0].breastRating >= 1) {
		...
		outputText(".");
		if(player.upperBody.chest.hasFuckableNipples()) outputText("  Her fingers play with your nipples and press gently to tease at them.  She seems quite surprised as they sink in with minimal effort.  A grin spreads over her face as she begins to steadily thrust those digits in and out of your nipples, fingers swirling about the interiors all the while.");
...
private function plantsForMe2(BE: number = 0):void {
	...
	if(player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) outputText("find the dual sexes there.  Another hand joins the first, one gripping and tugging on your hardening cock as the other cups and squeezes over your pussy.");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.upperBody.chest.list[0].breastRating >= 1) outputText(" and " + player.breastCup(0) + " breasts");
	...
	if(BE == 1) {
		...
		if(player.upperBody.chest.countNipples() > 2) outputText("s have");
		else outputText(" has");
...
private function hasCockFeedEssy(newPage:boolean = true):void {
	...
	if(player.lowerBody.balls > 0) outputText(" and your swollen, heavy balls against her fecund belly");
