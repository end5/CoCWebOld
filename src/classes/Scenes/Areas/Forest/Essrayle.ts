...
private noPlantsForMe():void {
	...
	flags[FlagEnum.TURNED_DOWN_ESSY_FIRST_MEETING] = 1;
...
private plantsForMe():void {
	MainScreen.clearText();
	if(flags[FlagEnum.ACCEPTED_ESSY_FIRST_MEETING] == 0) {
		flags[FlagEnum.ACCEPTED_ESSY_FIRST_MEETING] = 1;
	...
	if(player.upperBody.chest.list[0].breastRating >= 1) {
		...
		if(player.bRows() > 1) MainScreen.text(" of your uppermost pair");
		...
		if(player.bRows() > 1) MainScreen.text(", steadily working over the multitude of fleshy mounds on your body");
		MainScreen.text(".");
...
private plantsForMe2(BE: number = 0):void {
	...
	if(!player.isTaur()) MainScreen.text("lifting you slowly into the air");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.upperBody.chest.list[0].breastRating >= 1) MainScreen.text(" and " + player.breastCup(0) + " breasts");
	...
	if(BE == 1) {
		...
		if(player.bRows() > 1) MainScreen.text("  She eventually dislodges with a wet pop, setting your new breasts to wobbling about as she moves down your body, repeating the gestures upon each set of breasts in return.");
		...
		if(player.bRows() > 1) MainScreen.text(", the top pair being largest, descending in size with each subsequent pair");
		MainScreen.text(".");
...
public approachTrappedEssy():void {
	MainScreen.clearText();
	if(flags[FlagEnum.ESSY_DUNGEON_FUCKED] == 0) {
...
private feedTrappedEssy():void {
	...
	flags[FlagEnum.ESSY_DUNGEON_FUCKED]++;
...
private hasPussyFeedEssy(newPage:boolean = true):void {
	...
	if(player.looseness() < 2) MainScreen.text("a gasping grunt of effort");
	else if(player.looseness() < 4) MainScreen.text("little resistance");
...
private acceptEssyPrizes():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 1) {
		...
		player.upperBody.chest.list[1].breastRating = 4;
	}
	//if two breasts:
	else if(player.bRows() == 1) {
		MainScreen.text("two more bulging tits erupt from your chest");
		if(player.upperBody.chest.list[0].breastRating < 4) {
			MainScreen.text(" while your top row expands");
			player.upperBody.chest.list[0].breastRating = 4;
		}
		if(player.bRows() == 1) player.createBreastRow();
		player.upperBody.chest.list[1].breastRating = player.upperBody.chest.list[0].breastRating;
	}
	//if four+ breasts: 
	else {
...
public askMotherToReleaseEssy():void {
	...
	flags[FlagEnum.TOLD_MOTHER_TO_RELEASE_ESSY] = 1;
	doNext(playerMenu);
}
}
}
