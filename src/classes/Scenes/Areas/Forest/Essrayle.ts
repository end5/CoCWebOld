...
public function essrayleMeetingI():void {
	...
	if(flags[FlagEnum.MET_ESSY] == 0) {
		...
		if(player.stats.cor < 33) Render.text("Despite however chivalric you think yourself");
		else if(player.stats.cor < 66) Render.text("Despite your better sense");
		else Render.text("Perhaps best of all");
		Render.text(", your attention falls to her chest.  This emerald maiden sports a pair of breasts that'd make even a demon blush, gorgeous green globes that look like small watermelons resting upon her upper belly, capped with purple areolae.  Four, thick, teat-like nipples dangle a good three or four inches from each.");
		...
		if(player.stats.cor < 33) Render.text("hastily looking away from her nude body");
		else if(player.stats.cor < 66) Render.text("looking away out of courtesy");
		else Render.text("letting your eyes roam over her as you search for even more depraved features on her nubile body");
		Render.text(".");
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 10) Render.text("own");
		else Render.text("[chest]");
		Render.text(".  \"<i>You must be one of those adventurers I've been hearing about, am I right?</i>\"");
...
private function plantsForMe():void {
	...
	if(player.upperBody.chest.list[0].breastRating >= 1) {
		...
		if(player.bRows() > 1) Render.text(" of your uppermost pair");
		Render.text(".  You moan, and this only seems to encourage her, as she takes your nipples between thumb and forefinger, twisting and turning them this way and that.  She giggles at how your moans seem to fluctuate by coincidence, continuing to tease them");
		if(player.bRows() > 1) Render.text(", steadily working over the multitude of fleshy mounds on your body");
		Render.text(".");
		if(player.upperBody.chest.hasFuckableNipples()) Render.text("  Her fingers play with your nipples and press gently to tease at them.  She seems quite surprised as they sink in with minimal effort.  A grin spreads over her face as she begins to steadily thrust those digits in and out of your nipples, fingers swirling about the interiors all the while.");
		Render.text("  \"<i>You certainly have a lovely pair, but they could always be better, don't you think?</i>\"");
		//[Yes] [No]
		menu();
...
private function plantsForMe2(BE: number = 0):void {
	...
	if(player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) Render.text("find the dual sexes there.  Another hand joins the first, one gripping and tugging on your hardening cock as the other cups and squeezes over your pussy.");
	else if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("circle about ");
		...
		Render.text(", beginning to squeeze and slide about its length until your nursing is interrupted by moans of your own delight");
	}
	else Render.text("stroke about the puffy, now-inflamed lips of your own flower, taking the lips between two fingers and stroking up and down across their length fully before switching to your moist core.");
	
 	Render.text("\n\nA firm hand cups your ");
	if(player.isTaur()) Render.text("back");
	else Render.text("butt");
	Render.text(" and pulls you close, forcing your [legs] against her bulb.  They sink in slightly as you go down, reluctantly relinquishing her nipple from your mouth.  As it springs free, a thick glob of sap splatters over your lips and nose.  Essy giggles softly as she wipes them off with a hand.");
	...
	Render.text("and [legs], ");
	if(!player.isTaur()) Render.text("lifting you slowly into the air");
	else Render.text("forcing you to rear up");
	Render.text(".  Her lower hands grasp you, helping to raise you up until you're at the proper height, whence she abruptly plants her lips upon your crotch.");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.upperBody.chest.list[0].breastRating >= 1) Render.text(" and " + player.breastCup(0) + " breasts");
		Render.text(".  The opaque cups pump and suck, Essy humming to herself as the base of her tongue continues to skillfully tease over every hot spot near the [cockHead biggest] of your [cock biggest], her length slithering about it skillfully.");
	}
	//(Pussy=
	else {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) {
		...
		if(player.lactationQ() >= 400) Render.text("your milk and ");
		Render.text("your pleasure.  In time, their pumping increases in force and tempo just as her lips and tongue do.");
	}
	...
	if(BE == 1) {
		...
		if(player.upperBody.chest.countNipples() > 2) Render.text("s have");
		else Render.text(" has");
		Render.text(" been swallowed up as well.");
		...
		if(player.bRows() > 1) Render.text("  She eventually dislodges with a wet pop, setting your new breasts to wobbling about as she moves down your body, repeating the gestures upon each set of breasts in return.");
		...
		if(player.bRows() > 1) Render.text(", the top pair being largest, descending in size with each subsequent pair");
		Render.text(".");
...
private function feedTrappedEssy():void {
	clearOutput();
	Render.text("Well, it's not like she's going anywhere right now, and she probably would love a good fuck from a friend anyway.");
	...
	if(player.gender == 3) {
		Render.text("  How best to make use of this floral beauty?");
		menu();
...
private function hasCockFeedEssy(newPage:boolean = true):void {
	...
	if(player.biggestCockLength() < 20) Render.text("her lower pair of wobbling udders");
	else Render.text("both sets of milk-bloated breasts");
	Render.text(" faster and faster, the hot slapping of her bountiful bosom against your [cock]");
	if(player.lowerBody.balls > 0) Render.text(" and your swollen, heavy balls against her fecund belly");
	Render.text(" setting an inspiring drum beat to your feverish pace.");
...
private function acceptEssyPrizes():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 1) {
		Render.text("four bulging tits erupt from your chest");
	...
	//if two breasts:
		...
		Render.text("two more bulging tits erupt from your chest");
		if(player.upperBody.chest.list[0].breastRating < 4) {
			Render.text(" while your top row expands");
			player.upperBody.chest.list[0].breastRating = 4;
		}
		if(player.bRows() == 1) player.createBreastRow();
	...
	//if four+ breasts: 
		...
		Render.text("your [fullChest] swell larger and larger under the expanding magic of the spell");
		player.growTits(6, player.bRows(), false, 2);
	}
	player.boostLactation(player.bRows());
	Render.text(", filling with warm, fluid weight.  They jiggle and wobble against each other and your belly as they adjust to their brimming plumpness.  You now proudly sport [fullChest].");
...
private function noGimmeGiantNipplesEssy():void {
	clearOutput();
	Render.text("She shrugs her shoulders, giving one bundle of milk-drooling nipples a soft squeeze.  \"<i>Eh, to each their own, I suppose.</i>\"");
	essyRewardEpilogueOUTTIES();
}
...
public function askMotherToReleaseEssy():void {
	...
	if(flags[FlagEnum.SAND_WITCHES_COWED] == 0) {
		Render.text("You point out that the witches have a friend of yours trapped here with magic and you'd like her released.");
		...
		Render.text("You command the Sand Mother to release your friend, Essrayle from her magical bondage.  The sorcerous queen looks about to protest, but after a brief pause, she admits, \"<i>Fine, I'll see her released.  You can go pick her up.</i>\"  She practically spits the last sentence.");
		Render.text("\n\nThat'll do.");
	}
