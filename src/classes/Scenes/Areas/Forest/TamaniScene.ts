...
private function tamaniFemaleYes():void {
	...
	if(player.stats.cor < 50) outputText("Gross.\n\n", false);
...
private function tamaniFemaleNo(): void {
	...
	if(player.stats.cor < 33) outputText("What a slut.", false);
	else if(player.stats.cor < 66) outputText("How odd.", false);
...
private function tamaniFirstTimeConsentual():void {
	...
	if(player.lowerBody.balls > 0) outputText("Your " + ballsDescriptLight() + " swell with seed, spurring your desire to new heights.", false);
	...
	if(player.cockArea(0) <= 90) {
		...
		if(player.lowerBody.balls > 0) outputText(" and drips from your " + ballsDescriptLight(), false);
		...
		//[CORRUPT]
		if(player.stats.cor > 66) outputText("  Your " + cockDescript(0) + " twitches at the thought, ready and wanting more.", false);
	}
	//(TOO BIG)
	else {
		...
		if(player.lowerBody.cockSpot.hasSheath()) {
			...
			if(player.lowerBody.balls > 0) outputText("licks all over your balls", false);
			else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("sneaks between your folds to tease your now-hardening clit", false);
		...
		if(player.lowerBody.balls > 0 && player.hoursSinceCum > 200) outputText(", visibly draining your " + ballsDescriptLight() + " down to their normal size", false);
...
private function tamaniMaleRepeatEncounter():void {
	...
	if (pregnancy.isPregnant || flags[FlagEnum.TAMANI_NUMBER_OF_DAUGHTERS] > 0) outputText("While exploring, you're startled by the feeling of tiny hands stroking the insides of your thighs.  You look down and find Tamani there, grinning wolfishly,  \"<i>Ready for another fuck, big " + player.boyGirl() + "?\"\n\n", false);
	//(ELSE) 
...
internal function tamaniSexLetHer():void {
	...
	if (getGame().inCombat) {
		...
		//Not taur
			...
			if(player.lowerBody.cockSpot.count() == 1) outputText("an erection emerges", false);
	...
	if(player.lowerBody == LOWER_BODY.CENTAUR) {
		...
		if(player.lowerBody.cockSpot.list[0].cockLength <= 18) {
			...
			//No balls: 
			if(player.lowerBody.balls == 0) outputText("You find yourself wondering vaguely where a goblin learns these kinds of gymnastics.\n\n", false);
		...
		//If >18 Inches 
			...
			//High cum: 
				...
				if(player.lowerBody.balls > 0) outputText("Your " + ballsDescriptLight() + " ache and you feel very thirsty indeed.", false);
	...
	//Not-taurs
		...
		if(player.cockArea(0) <= 90) {
			...
			if(player.lowerBody.cockSpot.list[0].cockLength >= 12) outputText("Her body visibly stretches around you, and you silently thank whatever gods or demons adapted goblins to be able to fulfill this role.  ", false);
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("Your " + allBreastsDescript() + " bounce and jiggle obscenely as the goblin twists and pulls, abusing your nipples.", false);
			...
			if(player.lowerBody.balls > 0) outputText("Your " + ballsDescriptLight() + " twitch powerfully, venting their contents, releasing a jet of hot cum into the goblin.", false);
			...
			if(player.cumQ() < 250)outputText(".  I really enjoy our fucks " + player.mf("stud","hun") + ", and I can't wait to 'bump' into you again.", false);
				...
				if(player.lowerBody.balls > 0) outputText(ballsDescriptLight() + " fill back up with cum, ok?", false);
		...
		//(Doesnt fit)
			...
			if(player.lowerBody.balls > 0) outputText("lifts up your " + ballsDescriptLight(), false);
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) outputText("cock's base ", false);
...
internal function tamaniSexLost():void {
	...
	if(player.lowerBody.cockSpot.list[0].cockThickness >= 5) outputText("Impossibly, despite your titanic girth, the ring somehow expands to squeeze onto you, though it's painfully tight.\n\n", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 6) outputText("but can't manage to see your bloated prick past the wall of titflesh.\n\n", false);
	...
	if(player.lowerBody.balls > 0) outputText("cups your " + ballsDescriptLight() + " tightly", false);
	...
	if(player.lowerBody.balls > 0) outputText("Your balls are getting VERY swollen and tender.", false);
	else outputText("Your body aches inside, feeling bloated and ready to pop.", false);
	if(player.stats.cor < 50) outputText("  You have no choice, you'll say what she wants you to say.\n\n", false);
	...
	if(player.stats.cor > 50) outputText("blushing hotly and doing your best to look like a submissive toy.\n\n", false);
...
internal function tamaniSexWon():void {
	...
	if(player.cockArea(x) <= 90) {
		...
		if(player.lowerBody.balls > 0) outputText("sexy balls have ", false);
...
private function tamaniPregnantFuck():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText(allBreastsDescript(), false);
	else outputText("chest", false);
	outputText(".\n\n", false);
	...
	//(Very large: 
	if (flags[FlagEnum.TAMANI_TIMES_IMPREGNATED] > 6) outputText("  They're almost too big for you to handle; they probably make it hard for the poor girl to walk.", false);
	outputText("\n\n", false);
	...
	//(Doesn't Fit)
		...
		if(player.lowerBody.balls > 0) outputText("massaging your balls with her feet", false);
...
private function tamaniPoopsOutBabies():void {
	...
	if (flags[FlagEnum.TAMANI_NUMBER_OF_DAUGHTERS] > 0) outputText("with her daughters, ", false); //She always has at least two daughters if she has any
	...
	outputText("  She looks up at you and whispers breathily, \"<i>Come fuck me a little later, I'll give you even more beautiful little girls.  These " + num2Text(flags[FlagEnum.TAMANI_PREGNANCY_COUNT]) + " will be fully grown in a week or two and out man-hunting, but none of them will please you like me, I promise.</i>\"\n\n", false);
...
internal function getRapedByTamaniYouHypnoSlut():void {
	...
	if(primary >= 0) {
		...
		if(player.skinType == SKIN.FUR) outputText("matted", false);
		...
		else if(player.lowerBody.cockSpot.list[primary].cockType == CockType.HORSE) outputText("Your flare widens, actually holding itself inside Tamani's womb", false);
		else if(player.lowerBody.cockSpot.list[primary].cockType == CockType.DEMON) outputText("The corrupted nodules along your " + cockDescript(primary) + " pulsate in rippling, wave-like motions, massaging the interior of Tamani's twat", false);
		else if(player.lowerBody.cockSpot.list[primary].cockType == CockType.TENTACLE) outputText("Your tentacle-cock's tip widens, becoming mushroom-like inside Tamani's womb", false);
	...
	//[DOESNT FIT]
		...
		outputText(player.mf("strong man's","sexy lover's") + " " + Appearance.cockNoun(CockType.HUMAN) + "?</i>\"\n\n", false);
		
		outputText("You assure her that they do like what she is doing – very much so – and ", false);
		if(player.stats.cor < 33) outputText("beg", false);
		else if(player.stats.cor < 66) outputText("ask", false);
...
internal function tamaniBeaten():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("licks her lips in anticipation, eyes locked on [eachCock]");
	...
	if(flags[FlagEnum.TIMES_OVIPOSITED_TAMANI] == 0) outputText("my baby-" + player.mf("daddy","momma"));
	...
	if(flags[FlagEnum.TIMES_OVIPOSITED_TAMANI] > 0) outputText(", or in your case even more eggs");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  White coats the goblin from her shoulders up, the only evidence of your male satisfaction.  You were so focused on the act of impregnation and the unique pleasure that it provides that you barely noticed [eachCock] erupting in sympathetic pleasure.");
