...
public function encounterTamanisDaughters():void {
	...
	if(flags[FlagEnum.TIMES_ENCOUNTED_TAMANIS_DAUGHTERS] > 0 && rand(10) == 0) {
		tamaniPresent = true;
		outputText("While roaming along, you find your path ahead blocked by " + num2Text(int(flags[FlagEnum.TAMANI_NUMBER_OF_DAUGHTERS] / 4)) + " goblins.  At the forefront of the mob is Tamani", false);
		if(flags[FlagEnum.TAMANI_TIMES_HYPNOTISED] >= 10) outputText(", your wife", false);
	...
	outputText(num2Text(int(flags[FlagEnum.TAMANI_NUMBER_OF_DAUGHTERS] / 4)) + " goblins.  You ", false);
...
private function playDumbToTamanisDaughters():void {
	...
	if(player.stats.int/2 + 25 > rand(75)) {
		...
		if(flags[FlagEnum.TIMES_ENCOUNTED_TAMANIS_DAUGHTERS] > 1) outputText("They really must not be paying much attention to what you look like.", false);
	...
	if(flags[FlagEnum.TIMES_ENCOUNTED_TAMANIS_DAUGHTERS] == 1) outputText("first ", false);
...
private function fuckYoDaughtersHomie():void {
	...
	if(primary >= 0) {
		...
		if (tamaniPresent) {
			...
			if(player.lowerBody.balls > 0) outputText("cups your " + ballsDescriptLight() + ".", false);
		...
		if(player.stats.cor < 33) outputText("You were supposed to save the village from corruption, not breed tiny sluts to overrun it... y", false);
		else if(player.stats.cor < 66) outputText("You came here to make things better for everyone... well, at least you're helping these girls. Y", false);
		...
		if(daughters > 10) {
			...
			if(player.lowerBody.balls > 0) outputText("Your balls visibly puff up and slosh as one of your daughters plays with them, filling with more seed than ever before.", false);
			...
			if(player.lowerBody.balls > 0) outputText(ballsDescriptLight() + " empty", false);
	...
	//[DOESNT FIT]
		...
		if (tamaniPresent) {
			...
			if(player.lowerBody.balls > 0) outputText("  Your " + ballsDescriptLight() + " ache from the strain, but struggle to churn up more sperm.  ", false);
...
private function legTamanisDaughtersRAEPYou():void {
	...
	if(primary >= 0) {
		...
		if (tamaniPresent) {
			...
			if(flags[FlagEnum.TAMANI_TIMES_HYPNOTISED] < 10) outputText("'Wait... wife... mistress?' your mind wonders, rejecting the foreign thoughts.  You look up at Tamani, confused for a moment", false);
		...
		//NO TAMANI:  
			...
			//(MEDIUM PACK (or bigger))
				...
				if(player.lowerBody.balls > 0) outputText("balls begin swelling, trembling as they visibly inflate, preparing a massive load of seed.", false);
				...
				if(daughters < 40) {
					...
					if(player.lowerBody.balls > 0) outputText(ballsDescriptLight() + " hurt", false);
	...
	//[NO FIT]
		...
		//[Tamani is There]
			...
			if(flags[FlagEnum.TAMANI_TIMES_HYPNOTISED] >= 10) outputText("wife",false);
			...
			if(player.stats.cor > 66) outputText("sigh, actually anticipating what's about to come", false);
			else if(player.stats.cor > 33) outputText("don't resist, knowing there's no way to stop what's about to happen", false);
			...
			//(MULTI)
			if(cocks > 2) {
				outputText("Meanwhile your other " + Appearance.cockNoun(CockType.HUMAN) + "s are wasting their spunk over your belly, so some of the waiting girls grab them and pull them aside, ramming their tips deep inside their seemingly bottomless fuck-holes.  They giggle and run their manicured nails over your ", false);
				if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("chest", false);
				else outputText(allBreastsDescript(), false);
			...
			//(SINGLE EXTRA)
				...
				outputText("Meanwhile your other " + Appearance.cockNoun(CockType.HUMAN) + " is wasting its spunk over your belly, so one of the waiting girls grabs it and pulls it to the side, ramming its tip deep inside her seemingly bottomless fuck-hole.  She giggle and runs their manicured nails over your ", false);
				if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("chest", false);
				else outputText(allBreastsDescript(), false);
				outputText(", circling your sensitive nipples", false);
				if(player.biggestLactation() >= 1) {
			...
			if(flags[FlagEnum.TAMANI_TIMES_HYPNOTISED] > 10) outputText("She's the best wife ever.  You nuzzle deep into her cleavage, sighing happily.", false);
			else if(flags[FlagEnum.TAMANI_TIMES_HYPNOTISED] > 5) outputText("She really is a great wife... wait, wife? You shrug away the thought and enjoy slowly motorboating her breasts.", false);
...
private function tamaniDaughtersCombatLossDrain():void {
	...
	if(player.lust > 99) {
		...
		if(player.lowerBody.balls > 0) outputText("fondling your balls and ", false);
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00058] == 0) outputText("\"<i>Stop worrying.  I invented this while I was waiting on mom to bring me back more of your cum, just in case we ever got our hands on you.  I promise, after you've had a taste of my chair you'll never want to leave.</i>\"\n\n", false);
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00058] == 0) outputText("Time for your medicine!  We need you to cum enough for each of us, and maybe have a little left over to play with, so suck up the medicine, okay?  Just relax and let it fill you so that you give us all that yummy cummy!</i>\"\n\n", false);
	...
	//EXHIBITIONISTZ
	if(flags[FlagEnum.PC_FETISH] > 0) outputText("unimaginably turned on by being used and abused by such machinery in front of an audience of your own horny children.", false);
	else if(player.stats.cor > 60) outputText("turned on by being milked in such an obscene way.", false);
	...
	if(player.lowerBody.balls > 0) outputText("Your balls tighten inside your sack and swell up like sponges, slowly increasing in size in time with your desire.", false);
	...
	if(player.lowerBody.balls > 0) outputText("Straining and shaking, your " + ballsDescriptLight() + " clench against you, feeling tight as your body struggles to pump out the spooge they're producing.", false);
...
private function tamaniDaughtersBadEndChoice():void {
	...
	if(player.statusAffects.get("Exgartuan").value1 == 1) outputText("Exgartuan barks, \"<i>Hell yes I do!</i>\" but the goblin only smirks down for a moment before looking back at you.\n\n", false);
...
private function tamaniDaughtersYesBadEndMePlease():void {
	...
	if(player.statusAffects.get("Exgartuan").value1 == 1) outputText("Exgartuan moans, \"<i>Ohhhhhh yeeeeaaaaahhhh...</i>\" before slipping into silence.\n\n", false);
...
private function tamanisDaughtersFillIndividuallyBADEND():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("one of them", false);
...
private function loseToDaughtersWithTamaniThere():void {
	...
	if(flags[FlagEnum.TAMANI_TIMES_HYPNOTISED] < 2) outputText("It tastes syrupy-sweet", false);
	...
	if(flags[FlagEnum.TAMANI_TIMES_HYPNOTISED] > 10) outputText("Your body must remember how much it loves being my pussy-hungry husband.", false);
	...
	if(player.skinType == SKIN.FUR) outputText(", matting your " + player.hairColor + " fur", false);
	outputText(" as she continues ", false);
	if(flags[FlagEnum.TAMANI_TIMES_HYPNOTISED] < 10) outputText("filling your mind with truths", false);
	...
	if(player.lowerBody.cockSpot.count() == 1) outputText("grab your " + multiCockDescriptLight() + " and stroke it with long slow strokes, just like their mother.  Unlike her, they don't seem content to wait, and in seconds a slippery gash is stretching to accommodate your cock-head.\n\n", false);
	...
	//(FITS)
		...
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() < 30) outputText(" in spite of the large sizes they usually handle.", false);
