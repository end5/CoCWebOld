		...
		public tamaniHypnoTease():void {
			...
			//MEDIUM HYPNO VALUE:
				...
				if(selector == 0) {
					...
					if(player.lust > 80) MainScreen.text(" drips pre-cum", false);
					else if(player.lust > 40) MainScreen.text(" grows harder", false);
					...
					if(player.lust < 70) MainScreen.text("  Still, you don't want to fuck her right now!", false);
				}
				else {
					...
					MainScreen.text("your " + player.multiCockDescriptLight(), false);
					if(player.lust <= 41) MainScreen.text(" thicken perceptibly", false);
					else if(player.lust <= 81) MainScreen.text(" twitch eagerly", false);
					...
					if(player.lust < 70) MainScreen.text("  Regardless, you're resolute in your desire not to fuck her right now!", false);
				}
			}
			//HIGH HYPNO VALUE
			else {
				...
				if(selector == 0) {
					...
					if(player.totalCocks() > 1) MainScreen.text("each of ", false);
					MainScreen.text("your " + player.multiCockDescriptLight() + " aches with need.  You battle with the compulsion to kneel before your short, stacked mistress and perform your duties as her breeder husband.", false);
				}
				else {
					...
					MainScreen.text("our " + player.multiCockDescriptLight() + " pulses and dribbles pre-cum, aching to do its duty and fire load after load into Tamani's perfect pussy.", false);
				}
			}
