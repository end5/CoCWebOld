export default class TentacleBeastScene {
	...
	*/
	...
	public encounter(): void {
		...
		if (player.gender == 0 && rand(3) == 0 && !player.lowerBody.isNaga() && !player.lowerBody.isTaur() && !player.lowerBody.isGoo()) {
	...
	private tentacleEntice(): void {
		...
		//Spoiler for Entice Attack-Female: 
			...
			if (rand(2) == 0) {
				//GENDERLEZZ
				if (player.gender == 0) MainScreen.text("You brazenly turn your back on the creature and, glancing over your shoulder, begin bending over and presenting your " + ButtDescriptor.describeButt(player) + " to the beast. It pauses and observes while you bend over further, presenting a full view of both your back door and your " + ButtDescriptor.describeButthole(player) + ". You shift from side to side and observe the beast match your movements. You have obtained its attention to say the least.\n\n", false);
				//CHICKS
				else MainScreen.text("You brazenly turn your back on the creature and, glancing over your shoulder, begin bending over and presenting your " + ButtDescriptor.describeButt(player) + " to the beast. It pauses and observes while you bend over further, presenting a full view of both your back door and your honey hole. You shift from side to side and observe the beast match your movements. You have obtained its attention to say the least.\n\n", false);
				monster.lust += 10 + rand(5);
			}
			//Failure
			else {
				MainScreen.text("You begin shaking your hips and grabbing your " + BreastDescriptor.describeAllBreasts(player) + " to distract the creature. However, the near-miss from the tentacle it attempted to swat you with convinces you of its desire to beat your ass, rather than fuck it.\n\n", false);
			}
		}
	...
	internal function tentacleLossRape(): void {
		...
		//Horsecock surprise!
		if (player.lowerBody.cockSpot.countType(CockType.HORSE) > 0 && player.lowerBody.cockSpot.get(0).cockLength > 15 && player.lowerBody.cockSpot.get(0).cockThickness >= 3) {
			if (player.stats.cor < 75 && player.stats.lust < 100) MainScreen.text("It grabs you before you can get away!\n\nWhile you attempt to resist the abomination, its raw muscle mass is too much. ", false);
			...
			if (player.lowerBody.balls > 0) MainScreen.text(", and you feel your " + BallsDescriptor.describeBalls(false, true, player, true) + " rise and drop in unison to the muscular contractions pumping freshly made cum into position for release", false);
			MainScreen.text(".\n\n", false);
		...
		//Centaur madness!
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				if (player.cockArea(player.smallestCockIndex()) <= 50) {
					MainScreen.text("More aphrodisiac-toxin pours into you, causing " + CockDescriptor.describeMultiCockSimpleOne(player) + " to expand.  ", false);
					if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("  The creature seems surprised at first to discover such a large brace of cocks, testing their texture and wrapping around each individually.  Your " + CockDescriptor.describeMultiCockShort(player) + " responds by wriggling about and tempting the beast to continue its exploration, but the gesture is futile and they're abandoned, though not for long.", false);
					MainScreen.text("\n\n", false);

					MainScreen.text("A peculiar sensation rolls over it as an unseen tentacle engulfs you, rippling and milking your " + CockDescriptor.describeCock(player, 0) + ".  Your body naturally tries to drive into it but the tentacle isn't strong enough to provide resistance.  Your wild humping causes it to bump up and down against your underbelly, a surprisingly pleasurable feeling.  The tentacle pays no heed, continuing to ripple and constrict around you;  a suckling noise accompanies the sensation of your pre-cum being suctioned out.\n\n", false);

				}
				// has cock, not engulfable: 
				else {
					MainScreen.text("More aphrodisiac-toxin pours into you, causing " + CockDescriptor.describeMultiCockSimpleOne(player) + " to expand. Something bumps up against the tip but can't seem to fit around your " + CockDescriptor.describeCock(player, 0) + ".  It continues trying for a while, sending pangs of pleasure down the length.  The tentacle eventually gives up and latches onto the tip, positioned right at the opening to your urethra.  It sucks up your pre-cum as it drips from you, accompanied by a loud suckling noise.", false);
					//[With testicles: 
					if (player.lowerBody.balls > 0) MainScreen.text("The sucking reaches all the way to your " + BallsDescriptor.describeBalls(true, true, player) + ", a spectacularly strange sensation that nevertheless feels wonderful.", false);
					MainScreen.text("\n\n", false);
				}
			}
			// has vagina: 
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				MainScreen.text("A squirming tentacle forces its way inside your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ", undulating and squirming as it works its way deeper and deeper.  Your body responds by pumping out more fluid, making the passage of the monstrous thing easier.", false);
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1 && player.lactationQ() <= 0) {
				MainScreen.text("Roving tentacles latch onto your " + BreastDescriptor.describeAllBreasts(player) + "; tiny spikes jabbing into each " + BreastDescriptor.describeNipple(0) + " and injecting some sort of hot fluid.", false);
			...
			if (player.lowerBody.butt.analLooseness >= 4) {
				MainScreen.text("Your " + ButtDescriptor.describeButthole(player) + " makes an inviting target for the squirming mass and it's quick to capitalize.  A particularly bulbous appendage slides deep inside, roiling about in a way that not even your well-trained hole has been treated to.", false);
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				...
				MainScreen.text("The creature's desires are soon fulfilled as your " + CockDescriptor.describeCock(player, 0) + " starts to swell.  ", false);
				//[has testicles: 
				if (player.lowerBody.balls > 0) MainScreen.text("Your " + BallsDescriptor.describeBalls(true, true, player) + " tighten up against you in preparation for their inevitable release, ready to spray their boiling load into the beast.  ", false);
			...
			// has vagina:
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				MainScreen.text("Your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " ripples about the coiled intruder as you climax; fem-cum drips down the tentacle and fills the area with your musky scent.  You rear up as a surge of euphoria races through you, managing to overpower the tentacles holding your forelegs down for the briefest of moments.  But even with your forelegs free, the tentacle in your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " remains, rippling with waves of seed that spray inside you in massive, hot globules.  The sticky substance flooding your love canal pushes you over the edge and you orgasm again, spraying more as you cry out in pleasure.\n\n", false);
			}
			// has cock, normal cum amount, anus < gaping: 
			if (player.lowerBody.cockSpot.hasCock() && player.cumQ() < 1500 && player.lowerBody.butt.analLooseness < 4) {
				MainScreen.text("Just as you think it's over, another tentacle rams into your " + ButtDescriptor.describeButthole(player) + " and begins roughly massaging your prostate as it swells massively, causing another surge of cum to leave you, and another, and another.", false);
			...
			// has cock, normal cum amount, anus == gaping: 
			if (player.lowerBody.cockSpot.hasCock() && player.cumQ() < 1500 && player.lowerBody.butt.analLooseness >= 0) {
				MainScreen.text("Just as you think it's over, the tentacle inside your " + ButtDescriptor.describeButthole(player) + " begins to swell massively, causing another surge of cum to leave you, and another, and another.  It continues to violate your ass until you black out from exhaustion, the number of loads you've released no longer countable.", false);
				//end (loss)
				player.orgasm();
				player.stats.tou += 1;
				player.stats.int += -.5;
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				MainScreen.text("Just as you think it's over, a tentacle rams into your " + ButtDescriptor.describeButthole(player) + " and begins to swell massively, causing another surge of girlcum to leave you, and another, and another.", false);
				player.buttChange(40, true, true, false);
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				...
				if (player.lowerBody.balls > 0) MainScreen.text("r " + BallsDescriptor.describeBalls(true, true, player), false);
		...
		//Milk surprise!
		if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier >= 3.5 && player.gender > 0) {
			player.slimeFeed();
			...
			//Pussy
				...
				if (player.lowerBody.cockSpot.count() > 0) {
					MainScreen.text(VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " and " + CockDescriptor.describeMultiCockShort(player), false);
			...
			MainScreen.text("You shudder as your " + BreastDescriptor.describeAllBreasts(player) + " are quickly encircled and molested by the smaller tentacles.  Your swollen mammaries ache as the tentacles attach their orifices to your oozing nipples.  The tentacles begin a distinct milking pattern, alternating which nipple is milked first; you moan in delight and watch as your milk travels through the tentacle shaft and down to the shambling beast's body.\n\n", false);
			//(Optional Paragraphs)
			if (player.gender == 2) {
				//[Female/Virgin-Tight Cunt]
				if (player.vaginalCapacity() < 30) MainScreen.text("The beast senses your excitement and with beguiling speed swiftly impales your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " with one of its massive, knobbly tentacles.  You squeal in pain and pleasure as you feel every bumpy inch pound into you, your cunt being stretched to unbelievable proportions.  The tentacle quickly bottoms out in your shallow hole, pressing urgently against your cervix as it begins to rhythmically pound your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".\n", false);
				//[Female/Loose-Moist Cunt]
				else MainScreen.text("The beast senses your excitement and with beguiling speed swiftly impales your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " with one of its massive, knobbly tentacles.  You moan like a whore as the beast's knobbly cock slides into with ease, every bump sending shivers through your spine as it finally bottoms out deep in your cunt, pressing into your cervix urgently.  The monster begins to pound heartily at your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ", filling the air with lewd squishing sounds.\n", false);
				if (player.cuntChange(20, true)) MainScreen.text("\n", false);
				MainScreen.text("\n", false);
			}//HERMS
			if (player.gender == 3) {
				//[Herm/Virgin-Tight Cunt]
				if (player.vaginalCapacity() < 30) MainScreen.text("The beast senses your excitement and with beguiling speed swiftly impales your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " with one of its massive, knobbly tentacles.  You wail in excitement and pain, but before you can even digest the invasion, another tentacle impales itself on your " + CockDescriptor.describeCock(player, 0) + ".  The anus-like opening gapes to envelope you, slowly devouring your member.  The double assault drives your body wild, and you begin pumping back against the invader and thrusting your " + CockDescriptor.describeCock(player, 0) + " deeper into its tight fuck hole.\n\n", false);
				//[Herm/Loose-Wet Cunt]
				else MainScreen.text("The beast senses your excitement and with beguiling speed swiftly impales your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " with one of its massive, knobbly tentacles but before you can even digest the invasion another tentacle impales itself on your " + CockDescriptor.describeCock(player, 0) + ", the anus like opening gaping to envelope you.  The double assault drives your body wild, and you begin pumping back against the invader and thrusting your " + CockDescriptor.describeCock(player, 0) + " deeper into its tight fuck hole.\n\n", false);
				if (player.cuntChange(20, true)) MainScreen.text("\n", false);
				MainScreen.text("\n", false);
			}
			MainScreen.text("You slowly become aware that the beast has slowed its assault on your genitals and soon stops altogether, withdrawing entirely.  The beast lets out an audible gurgle and you smile as you feel the tentacles re-double their assault on your " + BreastDescriptor.describeNipple(0) + "s.  The beast slowly lifts you off the ground with its strong tentacles, suspending you about three feet off the ground before flipping you over.  You hang suspended in the air, your " + BreastDescriptor.describeAllBreasts(player) + " dangling lewdly under you.  Suddenly you feel the desire to \"<i>moo</i>\" as the attack on your aching " + BreastDescriptor.describeNipple(0) + "s continues.  The tentacles continue their assault for what seems like hours, but then you gradually sense the tentacles beginning to slow.  Another gurgling sound confirms your suspicions - the beast wants your milk, but it's obvious you have far too much to offer!  You grin wickedly when the beast's tentacles begin to sag, quickly reaching up to fondle and massage your " + BreastDescriptor.describeBreastRow(player.upperBody.chest.get(0)) + ".  The stimulation causes even more milk to gush down the tentacles length.  After a few moments of the increased assault the beast groans and releases you, the tentacles popping off your nipples audibly, spraying your milk about as they release you.\n\n", false);
			//[Female/Herm]
			if (player.gender >= 2) MainScreen.text("Your " + BreastDescriptor.describeAllBreasts(player) + " ache, but you can tell immediately they are not depleted.  More milk dribbles as the tentacles try to retreat, and you grin, hardly satisfied with the beast's attack.  You reach between your thighs, seizing the nearest knobbly tentacle.  The beast is so sated it offers no resistance as you begin to pound your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " with the living dildo.  The idea of turning the tables on the raping beast spurs you on to new heights and you cum quickly around the knobbly shaft, your cunt spasming and milking the bumpy tentacle hard.  As you finish with the tentacle the beast gives a final gurgle and retreats into the forest.", false);
			//[Male]
			else MainScreen.text("You feel your " + BreastDescriptor.describeNipple(0) + "s dribbling milk as the tentacles attempt their retreat.  You realize the beast has nowhere near drained you and you grin eagerly as your " + CockDescriptor.describeCock(player, 0) + " throbs mightily.  You reach back and seize the nearest knobby tentacle, the beast offering no resistance as you shove your " + CockDescriptor.describeCock(player, 0) + " into the tight, puckered orifice.  You moan in delight, grunting happily as you fuck the tight hole wildly.  The thought of turning the tables on the raping beast drives you closer to the edge; soon you bury all of your cock into the tight fuck tool and unload your massive torrent of cum into the tentacle.  Your hot cum gushes into the beast and you can feel the tentacle throb and squirm in protest as you fill the beast even more.  After your " + CockDescriptor.describeCock(player, 0) + " slips free the beast lets out a final gurgle of defeat and slithers away into the forest.", false);
			player.orgasm();
			dynStats("tou", .5, "spe", -.5, "int", -.5, "lib", 1, "sen", 1, "cor", 1);
			player.boostLactation(.5);
			monster.HP = 0;
		...
		if (player.gender == 1) {
			...
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("at the base of your " + CockDescriptor.describeMultiCockShort(player) + ".", false);
		...
		if (player.gender == 3) {
			...
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(CockDescriptor.describeMultiCockShort(player) + " immediately become fully erect and leak pre-cum liberally from their tips.  ", false);
			...
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(CockDescriptor.describeMultiCockShort(player) + ", now begging for release, and clamps down around your pubic mound, fully encapsulating your members.  ", false);
			else MainScreen.text(CockDescriptor.describeCock(player, 0) + ", now begging for release, and clamps down upon your pubic mound, fully encapsulating your member.", false);
		}
		//Call page 2!
	...
	private tentacleRapeContinuation(): void {
		...
		if (player.gender == 1) {
			...
			if (player.lowerBody.cockSpot.count() == 1)
				MainScreen.text("using a sucking-stroking motion on your helpless " + CockDescriptor.describeMultiCockShort(player) + ". The swelling of the ass tentacle pressures your prostate in a paradoxically pleasurable and painful manner. You realize, much to your terror, that this beast is MILKING you of your semen!", false);
			else
				MainScreen.text("using a sucking-stroking motion on your " + CockDescriptor.describeMultiCockShort(player) + ".  The swelling of the ass tentacle pressures your prostate in a paradoxical pleasurable and painful manner.  You realize, much to your terror, that this beast is MILKING you of your semen!", false);
			...
			if (player.lowerBody.cockSpot.count() == 1)
				MainScreen.text(CockDescriptor.describeMultiCockShort(player) + " and ass and in a very short time, your phallus explodes, launching stream upon stream of hot, thick cum into the horror. Your hips and pelvis buck violently with each thrust as the creature masterfully strokes your " + CockDescriptor.describeMultiCockShort(player) + "  and milks your prostate of your fluids. You cry with each orgasm, prompting the thing to milk you harder. After an eternity of successive ejaculations, the creature withdraws its unholy arms and leaves you in a bruised, lacerated, overfucked heap on the ground, discarded like a person throws away a corn cob after a meal.", false);
			else
				MainScreen.text(CockDescriptor.describeMultiCockShort(player) + " and ass and in a very short time, your dicks explode, launching stream upon stream upon stream of hot, thick cum into the horror.  Your hips and pelvis buck violently with each thrust as the creature masterfully strokes your " + CockDescriptor.describeMultiCockShort(player) + " and milks your prostate of your fluids.  You cry with each orgasm, prompting the thing to milk you harder. After an eternity of successive ejaculations, the creature withdraws its unholy arms and leaves you in a bruised, lacerated, overfucked heap on the ground, discarded like a person throws away a corn cob after a meal.", false);
		}
			...
			MainScreen.text("\n\nIn a coordination that can only signify higher intelligence, the monster fucks your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)), false);
			if (player.lowerBody.vaginaSpot.count() > 1)
				MainScreen.text("s", false);
			...
			if (player.lowerBody.cockSpot.count() == 1) {
				MainScreen.text("A sharp tug tells you that the creature has sealed itself upon your " + CockDescriptor.describeCock(player, 0) + ". You see " + player.totalBreasts() + " smaller tentacles latch onto your erect nipples. You feel milk begin to leak out as the creature makes a perfect seal around your areola. A thick, phallic tentacle probes underneath your trapped " + CockDescriptor.describeCock(player, 0) + " until it finds your vaginal opening. You cry out as the member punches past your opening and bottoms out in your womb. The tentacle swells up until it completely fills your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".  ");
				player.cuntChange(player.vaginalCapacity() * .76, true, false, true);
				MainScreen.text("With freakish coordination, the beast sucks your " + CockDescriptor.describeCock(player, 0) + " and tits while hammering away at your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ". The overwhelming pleasure courses through your body and triggers an immediate orgasm, sending gouts of cum into the tentacle sealed around your " + CockDescriptor.describeCock(player, 0) + ". The sensation of your fluids entering the creature prompts it to suck your " + CockDescriptor.describeCock(player, 0) + " harder as well as hammer your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " faster, leading to a chain of orgasms.\n\n", false);
				...
				MainScreen.text("A sharp tug tells you that the creature has sealed itself upon your " + CockDescriptor.describeMultiCockShort(player) + ". You see " + player.totalBreasts() + " smaller tentacles latch onto your erect nipples. You feel milk begin to leak out as the creature makes a perfect seal around your areola. A thick, phallic tentacle probes underneath your trapped cocks until it finds your vaginal opening. You cry out as the member punches past your opening and bottoms out in your womb. The tentacle swells up until it completely fills your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".");
				player.cuntChange(player.vaginalCapacity() * .76, true, true, false);
				MainScreen.text("  With freakish coordination, the beast sucks your " + CockDescriptor.describeMultiCockShort(player) + " and tits while hammering away at your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ". The overwhelming pleasure courses through your body and triggers an immediate orgasm, sending gouts of cum into the tentacles sealed around your pricks. The sensation of your fluids entering the creature prompts it to suck your throbbing cocks harder as well as hammer your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " faster, leading to a chain of orgasms.\n\n", false);
				MainScreen.text("Drunk with pleasure, you revel in the sensation of cumming into the creature while it breast feeds from you. All you can do is drown in the experience of being milked from top to bottom. The creature begins piledriving your box faster and you feel like the creature is going to impale you with its phallic tentacle.\n\n", false);
				MainScreen.text("The creature's milking tentacles stop moving and you feel the dick-tentacle press sharply against your womb. You feel the thunderous force of hot fluid lance into your body as the creature cums repeatedly inside you, triggering yet another orgasm. The creature cums in surges and shoots repeatedly inside you. Within moments, excess cum spews out of your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " as it cannot hold anymore, but the creature keeps cumming.\n\n", false);
				MainScreen.text("After a while the creature withdraws its tentacles from you. It poises the tentacle-cock over your face and lets out one last load, covering your face in hot, thick sperm. You reflexively open your mouth and allow loads of the salty juice down your throat. Once spent, the creature shambles off, leaving you well milked and cum-soaked.", false);
			}
	...
	private tentacleRapeContinuationForFemales(): void {
		...
		if (player.lowerBody.vaginaSpot.count() == 1) { //single coochie
			...
			if ((player.perks.has("BigClit") && player.lowerBody.vaginaSpot.get(0).clitLength > 2) || player.lowerBody.vaginaSpot.get(0).clitLength > 3)
				MainScreen.text("almost " + num2Text(Math.floor(player.lowerBody.vaginaSpot.get(0).clitLength * 1.75)) + " inches in length. ");
		...
		if (player.lowerBody.vaginaSpot.get(0).clitLength < 7 && player.lowerBody.vaginaSpot.get(0).clitLength >= 3.5 && player.perks.has("BigClit")) {
			player.lowerBody.vaginaSpot.get(0).clitLength += .1 + player.stats.cor / 100;
			MainScreen.text("Your massive clitty eventually diminishes, retaining a fair portion of its former glory.  It is now " + int(player.lowerBody.vaginaSpot.get(0).clitLength * 10) / 10 + " inches long when aroused, ");
			if (player.lowerBody.vaginaSpot.get(0).clitLength < 5)
				MainScreen.text("like a tiny cock.");
			if (player.lowerBody.vaginaSpot.get(0).clitLength >= 5 && player.lowerBody.vaginaSpot.get(0).clitLength < 7)
		...
		else if (player.lowerBody.vaginaSpot.get(0).clitLength < 3.5) {
		...
		if (player.lowerBody.vaginaSpot.get(0).vaginalLooseness == VaginaLooseness.TIGHT) player.lowerBody.vaginaSpot.get(0).vaginalLooseness = VaginaLooseness.NORMAL;
		player.slimeFeed();
		if (Game.inCombat)
			cleanupAfterCombat();
		else doNext(camp.returnToCampUseOneHour);
	...
	private centaurGenderlessRetardation(): void {
		MainScreen.text("", true);
		spriteSelect(100);
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00247] == 0 || player.lowerBody.balls == 0) {
			...
			if (player.stats.cor < 50 && player.stats.lust < 70) MainScreen.text("dread", false);
			else MainScreen.text("anticipation", false);
			MainScreen.text(", but stops when it reaches your haunches.  Another starts testing the same area, briefly touching your " + ButtDescriptor.describeButthole(player) + " but clearly not finding what it's looking for.\n\n", false);
			...
			MainScreen.text("A tentacle smacks your " + ButtDescriptor.describeButt(player) + " hard, and the voice returns to normal.\n\n", false);
		...
		//(Followup scene, if pc has seen above at least once, is unsexed centaur and has balls: -Z)
			...
			MainScreen.text(", slipping forward to probe between your haunches.  It arrives at and discovers your " + BallsDescriptor.describeSack(player) + " with some little ceremony, stroking and fondling it.\n\n", false);

			MainScreen.text("\"<i>Now THIS is what I'm talking about!</i>\" the creature's eerie voice sings out.  \"<i>Daddy needs his medicine!</i>\"\n\n", false);

			MainScreen.text("The tentacle, now joined by a second, hunts around your " + BallsDescriptor.describeBalls(true, true, player) + ", seeking any organs that might serve as a release valve for their contents.  You stare at it as it searches, quite certain you know what's coming next.\n\n", false);
