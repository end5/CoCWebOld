...
internal function tentacleLossRape():void {
	...
	if(player.gender == 0) {
		...
		if(player.isTaur()) {
			...
			if(player.stats.cor < 75) outputText("While you attempt to resist the abomination, its raw muscle mass is too much. ", false);
	...
	{
		if(player.stats.cor < 75 && player.lust < 100) outputText("It grabs you before you can get away!\n\nWhile you attempt to resist the abomination, its raw muscle mass is too much. ", false);
		...
		if(player.lowerBody.balls > 0) outputText(", and you feel your " + ballsDescript() + " rise and drop in unison to the muscular contractions pumping freshly made cum into position for release", false);
		outputText(".\n\n", false);
		...
		if(player.lowerBody.balls > 0) outputText("Your balls have overfilled the creature!\n\n", false);
	...
	//Centaur madness!
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			...
			// has cock, not engulfable: 
				...
				//[With testicles: 
				if(player.lowerBody.balls > 0) outputText("The sucking reaches all the way to your " + ballsDescriptLight() + ", a spectacularly strange sensation that nevertheless feels wonderful.", false);
				outputText("\n\n", false);
			}
		}
		// has vagina: 
		...
		if(player.ass.analLooseness >= 4) {
			...
			if(player.lowerBody.cockSpot.hasCock()) outputText("  A series of undulating lumps pass over your prostate, pushing out a splash of pre-cum.", false);
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			...
			//[has testicles: 
			if(player.lowerBody.balls > 0) outputText("Your " + ballsDescriptLight() + " tighten up against you in preparation for their inevitable release, ready to spray their boiling load into the beast.  ", false);
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			...
			if(player.lowerBody.balls > 0) outputText("r " + ballsDescriptLight(), false);
	...
	if(player.biggestLactation() >= 3.5 && player.gender > 0) {
		...
		if(player.gender == 1) {
			...
			if(player.lowerBody.cockSpot.count() > 1) outputText(" begin ", false);
	...
	if(player.gender == 1) 
	{
		...
		if(player.stats.cor < 75) outputText("It grabs you before you can get away!\n\nWhile you attempt to resist the abomination, its raw muscle mass is too much. ", false);
		outputText("It pins you to the ground easily. You immediately feel a sharp, horrible pain ", false);
		if(player.lowerBody.cockSpot.count() > 1) outputText("at the base of your " + multiCockDescriptLight() + ".", false);
		...
		if(player.lowerBody.cockSpot.count() == 1) outputText("your member immediately becomes fully erect and pre-cum leaks liberally from your tip.", false);
	...
	if(player.gender == 2)
	{
		...
		if(player.stats.cor < 75) outputText("You struggle to break free, but the creature only constricts you further, ensuring your immobility. ", false);
		outputText("A quick flex of the tentacles securing your legs leaves you spreadeagled before the maw of the horror.  ", false);
		if(player.stats.cor < 75) outputText("Fearing for your life, you scream and struggle for help, but only the apathetic sounds of nature respond.", false);
		...
		if(player.lowerBody.vaginaSpot.count() == 1) outputText("clit ", false);
	...
	if(player.gender == 3) 
	{
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("cocks", false);
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText(multiCockDescriptLight() + " immediately become fully erect and leak pre-cum liberally from their tips.  ", false);
		else outputText("member immediately becomes fully erect and pre-cum leaks liberally from your tip.  ", false);
		outputText("  " + Num2Text((player.upperBody.chest.countNipples())) + " thorny tentacles pierce your nipples, and you feel as if someone shot acid into your tits, which immediately begin to swell.", false);
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("furious cocks.  ", false);
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText(multiCockDescriptLight() + ", now begging for release, and clamps down around your pubic mound, fully encapsulating your members.  ", false);
...
private function tentacleRapeContinuationForFemales():void {
	...
	if (player.lowerBody.vaginaSpot.count() == 1) { //single coochie
		...
		if ((player.perks.has("BigClit") && player.lowerBody.vaginaSpot.list[0].clitLength > 2) || player.lowerBody.vaginaSpot.list[0].clitLength > 3)
			outputText("almost " + num2Text(Math.floor(player.lowerBody.vaginaSpot.list[0].clitLength * 1.75)) + " inches in length. ");
		//normal girls get big clits
		else
			outputText("almost four inches in length.  Bruised and sore, you pass into unconsciousness ");
	}
	else {
		outputText("Satisfied, the creature drops you smartly and withdraws its limbs from you and lumbers away.  Covered completely in cum, you see that your " + player.lowerBody.vaginaSpot.count() + " clits have swollen up to almost four inches in length.  Bruised and sore, you pass into unconsciousness, ");
	}
	//Not too corrupt
	if (player.stats.cor < 75)
	...
	if (player.lowerBody.vaginaSpot.list[0].clitLength < 7 && player.lowerBody.vaginaSpot.list[0].clitLength >= 3.5 && player.perks.has("BigClit")) {
		...
		outputText("Your massive clitty eventually diminishes, retaining a fair portion of its former glory.  It is now " + int(player.lowerBody.vaginaSpot.list[0].clitLength * 10) / 10 + " inches long when aroused, ");
...
private function centaurGenderlessRetardation():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00247] == 0 || player.lowerBody.balls == 0) {
		...
		if(player.stats.cor < 50 && player.lust < 70) outputText("dread", false);
