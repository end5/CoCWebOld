...
public function encounterFaerie():void {
	...
	if(player.lowerBody.cockSpot.count() > 0 && (!player.lowerBody.vaginaSpot.hasVagina() || rand(2) == 0)) {
		...
		if(player.lust >= 90) {
			...
			if(player.lowerBody.cockSpot.count() == 1) outputText("your dick", false);
...
private function faerieRAEP():void {
	...
	if(player.statusAffects.get("FaerieFemFuck").value1 == 1) outputText("She juices herself and screams, \"<i>Let me goooooooo,</i>\" trying to sound outraged instead of turned on, but the tiny girl's body gives away the lie.", false);
	else if(player.statusAffects.get("FaerieFemFuck").value1 <= 5) outputText("She juices herself and moans, \"<i>Stop teasing meeeeee,</i>\" doing her best to wriggle back against you, as if she could somehow impale herself on your digit.", false);
	...
	if(player.isTaur()) {
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength >= 3) {
			...
			//[Small amount of cum:
			if(player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.WET) outputText("The fae giggles more and more as the fluid seeps about her and your " + vaginaDescript(0) + " ripples. She hugs your " + clitDescript() + " tighter and starts gently gnawing at it, such a peculiar sensation that you cum suddenly, and wetly.  Her giggles quickly become all-out laughter, and she loses her grip on your clit, sprawling to the ground into a small puddle of femcum.\n\n", false);
			//[Normal amount of cum:
			else if(player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.DROOLING) outputText("The fae giggles more and more as the fluid squirts about her and your " + vaginaDescript(0) + " ripples. She hugs your " + clitDescript() + " tighter and starts gently gnawing at it, such a peculiar sensation that you cum suddenly, and wetly.  Her giggles quickly become all-out laughter, and she loses her grip on your clit, sprawling to the ground into a puddle of femcum.\n\n", false);
			//[Huge amount of cum:
		...
		//[All other clits:
			...
			//[Small amount of cum:
			if(player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.WET) outputText("You cum suddenly, and wetly. The fae giggles more and more as the fluid seeps about her and your " + vaginaDescript(0) + " ripples. Her giggles quickly become all-out laughter, and she loses her grip on your innards, sprawling to the ground into a small puddle of femcum.\n\n", false);
			//[Normal amount of cum:
			else if(player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.DROOLING) outputText("You cum suddenly, and wetly. The fae giggles more and more as the fluid squirts around her and your " + vaginaDescript(0) + " ripples. Her giggles quickly become all-out laughter, and she loses her grip on your innards, sprawling to the ground into a puddle of femcum.\n\n", false);
			//[Huge amount of cum:
	...
	//Non-Taurs
		...
		if(player.statusAffects.get("FaerieFemFuck").value1 < 4) outputText("Her mood immediately shifts from panic to desire, and she licks her lips hungrily, locking her eyes onto your feminine folds.", false);
		...
		if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLAVERING) outputText("releasing a tiny squirt", false);
		else if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) outputText("dribbling juices", false);
		else if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.WET) outputText("growing so slippery the faerie nearly loses her footing", false);
...
private function faerieDoNothing():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].nippleLength >= 1) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("With your other hand you start diddling your " + vaginaDescript(0) + ", adding your own soft moans to hers.  ", false);
		outputText("A few blissful moments later, she shudders and you feel her uncontrolled spasms around your nipple.  ", false);
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("You join her shortly after.  ", false);
	...
	if (player.lowerBody.vaginaSpot.list[0].clitLength >= 1.0 && player.lowerBody.vaginaSpot.list[0].clitLength <= 4.5 && player.lowerBody.vaginaSpot.hasVagina() && rand(2) == 0) {
		...
		if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) outputText("You drench the poor faerie completely in your female juices, soaking her hair and body. Overwhelmed and spent, you drop her to the ground and catch your breath. She licks up what's around her face, but is too weak to do anything else but lie in the dirt.\n\n", false);
...
private function faerieCaptureHJ():void {
	...
	if(player.isTaur()) {
		...
		if(player.lowerBody.cockSpot.count() > 0) outputText("largest " + Appearance.cockNoun(CockType.HUMAN), false);
		...
		//[No testicles:
		if(player.lowerBody.balls == 0) outputText("Your tiny globules of semen go straight into her open mouth and she sucks them down gleefully before falling with a splat onto the pre soaked ground.\n\n", false);
		else {
			//[Small amount of cum:
		...
		dynStats("lib", -.5);
		//Epilogue!
		if(player.statusAffects.get("FaerieFucked").value1 < 10) outputText("The faerie burps and giggles again before glaring up at you, accusing you with a mildly unfocused glare and asking, \"<i>Did you know we get drunk on cum?  Caushe I TRY SO HARRD not to get meshed up like this.</i>\"\n\n", false);
		else if(player.statusAffects.get("FaerieFucked").value1 < 15) outputText("The faerie burps and laughs drunkenly, patting the side of your " + player.leg() + " and slurring, \"<i>Oh by Marae's ripe titsh!  I needed that.  Do you thhink you could catsch me again?  I love feeling your cum coating my body.</i>\"\n\n", false);
		...
		if(player.statusAffects.get("FaerieFucked").value1 < 15) outputText("She licks her fingers and rolls around laughing, \"<i>Hehe, who caresh!  I'm happy! WHEEEEE!</i>\"\n\n", false);
	...
	//Non-taurs
		...
		else if(player.lowerBody.cockSpot.list[0].cockType == CockType.HORSE) outputText("climbs atop your " + cockDescript(0) + ", hanging onto your ring of prepuce and wrapping her legs as far around your horse-like maleness as she can.  ", false);
		else if(player.lowerBody.cockSpot.list[0].cockType == CockType.DEMON) outputText("climbs atop your " + cockDescript(0) + ", hanging on to the corrupted nubs and nodules as she threads her legs between them, squeezing you tightly as she hangs on.  You can feel her wet gash sitting atop a particularly sensitive bump, teasing you with a tiny cunt you'll never be able to penetrate.  ", false);
		else if(player.lowerBody.cockSpot.list[0].cockType == CockType.TENTACLE) outputText("climbs onto your squirming " + cockDescript(0) + ", wrapping her legs tightly around it as it wiggles and writhes with excitement.  Unbidden, it curls around and rubs its reddish-purple head against her face like an animal.  She gives it a gentle squeeze and licks it.  ", false);
