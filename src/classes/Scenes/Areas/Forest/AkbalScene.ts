...
import ImageName from '../../../display/Images/ImageName';
...
export default class AkbalScene {
    private akbal: Akbal;
    private data: AkbalData;
    public constructor() {
        this.akbal = Game.npcs.get(CharacterType.Akbal);
        ...
        if (Game.characterData.get(CharacterType.Akbal) === undefined)
            Game.characterData.set(CharacterType.Akbal, defaultData);
        this.data = Game.characterData.get(CharacterType.Akbal) as AkbalData;
    }
        ...
        else // [Victory via Lust]
        {
            ...
            if (character.stats.lust >= 33 && character.gender > 0) {
                ...
                if (character.torso.vaginas.count > 0 && character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 4 && character.inventory.equipment.armor.name === ArmorName.LustyMaidensArmor)
                    bikiniTits = (character.inventory.equipment.armor as LustyMaidensArmor).lustyMaidenPaizuri;
                if (character.torso.cocks.count > 0)
                    buttFuck = this.rapeAkbal;
                DisplayText("\n\nDo you rape him?");
                // Rape / Don't Rape
                MainScreen.displayChoices(["Butt-fuck", "Take Vaginally", "Force Lick", "B.Titfuck"], [buttFuck, vagoo, vagooLick, bikiniTits], ["Leave"], [cleanupAfterCombat]);
                return;
            }
        }
    }
    ...
    private rapeAkbal(character: Character) {
        ...
        if (character.torso.cocks.count > 0) {
            ...
            // [With Fertility/Lots of Jizz Perk]
            if (character.cumQ() > 1000) {
                DisplayText("You continue to slide your still-swollen " + CockDescriptor.describeCock(character, 0) + " inside his quivering hole as you pump massive tons of liquid into the false god's stomach and bowels.  Beneath the two of you, his belly begins to bulge as he is filled to a dangerous degree.\n\n");
    ...
    private girlsRapeAkbal(character: Character) {
        ...
        // {If Pussy is <loose}
            ...
            DisplayText("stuffing his own tail into his maw.  As he tries to push his tail back out, you remove your " + character.foot() + " pointedly, staring down at the demon. Lifting his hips back up weakly towards you, he meekly begins to suckle on his own appendage, closing his vibrant emerald eyes as you return your attention to his shaft.\n\n");
        }
        // {If Pussy >= Loose}
        else {
    ...
    private loseToAckballllllz(character: Character) {
        ...
        // Centaurs R SPECIALZ
            ...
            ButtModifier.displayStretchButt(character, Game.npcs.get(CharacterType.Akbal).torso.cocks.get(0).area, true);
            ...
            character.stats.cor += 5 + Utils.rand(10);
            // dynStats("lust+", 10 + character.stats.lib / 10, "cor+", 5 + Utils.rand(10));
            return;
            // Centaur over
        }
    ...
    public supahAkabalEdition(character: Character) {
        ...
        // Talk / Fight / Run
        MainScreen.displayChoices(["Talk", "Fight"], [this.superAkbalioTalk, this.startuAkabalFightomon], ["Leave"], [Game.scenes.camp.returnToCampUseOneHour]);
    }
    ...
    private ackbalRepeatAfterWin(character: Character) {
        ...
        // Fight / Flee
        MainScreen.displayChoices(["Fight"], [this.startuAkabalFightomon], ["Leave"], [Game.scenes.camp.returnToCampUseOneHour]);
    }
    ...
    private ackbalRepeatAfterLoss() {
        ...
        // Submit / Fight / Run
        MainScreen.displayChoices(["Submit", "Fight"], [this.akbalSubmit, this.startuAkabalFightomon], ["Leave"], [Game.scenes.camp.returnToCampUseOneHour]);
    }
    ...
    private akbalSubmit(character: Character) {
        ...
        if (character.torso.hips.legs.type === LegType.NAGA) {
            ...
            // [+ 4-12 Corruption]
            character.stats.cor += 4 + Utils.rand(8);
            // dynStats("cor", 4 + Utils.rand(8));
            // [+ 1-2 Speed]
