...
//[Talk] (C)
		private function talkAfterResistingKitsunellusion():void
		{
			...
			outputText("Self-preservation battles with curiosity " + ((player.lust > 50) ? "and lust " : "" ) + "as you consider her offer, " + ((player.stats.lib < 50) ? "weighing your chances against the possible dangers." : "eying the voluptuous curves that fill out her robes."));
...
//Illusory Mansion (C)
		private function mansion(willing:boolean, firstTime:boolean = false):void
		{
			...
			if (flags[FlagEnum.MANSION_VISITED] > 0) outputText(" back");
			outputText(",</i>\" you hear as you enter the courtyard, startling you a little.  There to greet you at the mansion's door stands ");
			if (flags[FlagEnum.MANSION_VISITED] > 0) outputText("the trio of familiar kitsune sisters.");
			else {
...
//fomerly letHer()
		private function kitSuneLetHerMansion(willing:boolean = true):void
		{
			...
			if (player.lowerBody.cockSpot.count() > 1) {
				...
				outputText("your remaining " + num2Text(player.lowerBody.cockSpot.count() - 1) + " cock");
			...
			outputText("Her warm " + ((player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 80) ? "cunt" : "asshole") + " still grips your cock tightly, dragging you down with her.  You collapse on top of her, sliding off of the redhead's cock as you tumble into the golden curls of the blonde's hair.");
			...
			outputText("  As your twitching cock relieves itself of the last of your seed inside the blonde's " + ((player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 80) ? "pussy" : "ass") + ", you feel your strength slipping away from you with each spasm, your eyelids growing heavy with an uncommon weariness.\n\n");
			player.orgasm();
...
//Formerly shoveHer()
		private function kitsuneShoveHerMansion(willing:boolean = true):void
		{
			...
			outputText("\"<i>F-fine...</i>\" she says quietly, rolling her eyes back in pleasure as you scratch behind her ears and run a finger coercively down her cheek." + ( flags[FlagEnum.redheadIsFuta] > 0 ? "" : "  Reaching down between her legs, you nod in satisfaction as you feel her cock slowly receding, shrinking down into a pert, solid pleasure bud between her puffy lips.") + "\n\n");
...
// For Males and Herms
		private function loseFightToHerms():void
		{
			...
			outputText("As her climax tears through her, her tail slackens its grip around your " + ((player.lowerBody.cockSpot.hasSheath()) ? "sheath" : "base" ) + ", and you are finally able to achieve release from your torment, your thick load rushing up your shaft to explode into the kitsune's quivering loins.  ");
			if (player.cumQ() <= 150) {
...
//Single cock < 9 inches long
		private function blondeKitsuneRapesSmallWangs():void
		{
			...
			outputText("Her lips rise and fall around your spasming prick, low moans vibrating through it each time she pulls back.  You can feel her cheeks pulling inward from the strength of her suction, loud smacks filling the air each time a gap forms in the almost airtight seal of her lips.  She squashes her breasts down as much as she can, lowering her lips to kiss your " + ((player.lowerBody.cockSpot.hasSheath()) ? "sheath" : "base" ) + ((player.lowerBody.balls > 0) ? " and reaching down to gently stroke your " + ballsDescriptLight() : "" ) + ((player.gender == 3) ? ", one of her long, dextrous fingers sliding into your " + vaginaDescript() : "" ) + "\n\n");
...
// lose, requires balls and cumQ() > 1000
		private function loseToKitsunesWithBallsAndHighCumQ():void
		{
			...
			outputText("\"<i>Smells like you've got a thick one brewing...</i>\"  she says, softly licking the " + ((player.lowerBody.cockSpot.hasSheath()) ? "sheath" : "base" ) + " with a giggle.  Six inky black tails slither forth to bind your limbs, twisting your wrists behind your back.  \"<i>Let's see if we can't help it along...</i>\"\n\n");
...
//Futa rape - chance increases as redhead's lust increases
		private function getRapedByRedHeadFutaKitsune():void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("entrance");
...
//Damage victory
		public function defeatTheKitsunes(display:boolean = true):void
		{
			if (display) {
				...
				kitsuneSprite();
				if (monster.HP < 1) {
					outputText("The kitsune hits the ground with an 'Oomph', landing roughly on her well-cushioned backside." + ((monster.hairColor == "red" && flags[FlagEnum.redheadIsFuta] == 0) ? "  The moment her rounded rump impacts the dirt, a swirling flame crackles to life between her legs, engulfing her exposed cock.  When it dies away, all that remains of her throbbing member is a pert cherry-colored bud between her dripping lips." : "") + "  She rubs her sore posterior, wincing in pain and pouting childishly.\n\n");
...
//[Vaginal] - requires cock
		private function fuckAKitsuneVaginally():void
		{
			...
			outputText("You only smirk, dragging your fingers across her side and leaning in to nibble the edge of her neck.  She practically melts in your arms, rolling her eyes back and moaning, her cheeks growing flushed with desire." + ((monster.hairColor == "red" && flags[FlagEnum.redheadIsFuta] > 0) ? "  You feel a light thump against your crotch and look down with a snicker, seeing a large tent in the lower half of her robes.  Teasingly, you run your hand down her front, wrapping your fingers around it through the fabric and pumping it slowly." : "" ) + "\n\n");
...
//[Anal] - requires cock area 144 or less
		private function putItInAKitsunesAssWin():void
		{
			...
			outputText("\"<i>Oh, no, such a big, strong cock...  what </i>ever<i> will I do?</i>\"  she says with a wry grin, beginning to slide her plump cheeks up and down your pole, one of her tails coiling down to gently wrap around the " + ((player.lowerBody.cockSpot.hasSheath()) ? "sheath" : "base" ) + ".\n\n");
			...
			outputText("Sensing your onrushing orgasm, she tightens the ring of her tail around your " + ((player.lowerBody.cockSpot.hasSheath()) ? "sheath" : "base" ) + ", blocking your orgasm from escaping and trapping blood in your swollen " + cockDescript(x) + ", making it that much more sensitive.\n\n");

			outputText("\"<i>Hey now, we're just getting started!  You wouldn't want to finish so soon, would you?</i>\"\n\n");

			outputText("You feel a swelling in your [balls] as your load begins to back up just behind your " + ((player.lowerBody.cockSpot.hasSheath()) ? "sheath" : "base" ) + ", its passage to release blocked by the mischievous fox-woman's tail.  Somehow even when defeated, she still finds a way to toy with you!  Your cock spasms inside her, trying to release its creamy load into her warm innards, but to no avail - on the plus side, however, you can somehow feel every single ripple and wave of her flesh magnified tenfold, gripping you tightly and milking your shaft pleasurably.\n\n");
			...
			outputText("\"<i>Heh...  not bad for...  a mortal,</i>\" she says, resuming her previous teasing, though you can tell her heart isn't in it by the satisfied look on her face" + ((monster.hairColor == "red" && flags[FlagEnum.redheadIsFuta] > 0) ? " and the lazy droop of her spent, flaccid member" : "") + ".\n\n");
...
//[Tail Job] - requires cock
		private function tailJobKitsuneWin():void
		{
			...
			outputText("It is at the height of one such cycle of pleasure and denial that you feel something cool and tingly washing over you, looking down to see the kitsune's lips pursed at " + ((player.lowerBody.balls > 0) ? "the underside of your " + ballsDescriptLight() : "the " + ((player.lowerBody.cockSpot.hasSheath()) ? "sheath" : "base" ) + " of your " + cockDescript(0) ) + ", gently blowing pale flames across your groin.\n\n");
...
//{all " + cockDescript(0) + " calls for this scene should prioritize dog cocks; if PC is in Rut, prioritize other animal dicks second, otherwise use the biggest cock.}
		private function fuckDraftBlond():void
		{
			...
			outputText("You " + ((player.stats.cor < 33) ? "stifle a laugh, observing her somewhat humorous plight." : "laugh uproariously, relishing the proud kitsune's utter humiliation.") + "  Her face twists into a beautiful grimace of climactic pleasure, her tongue lolling out shamelessly as she rolls her eyes back and orgasms, abruptly and violently.  Clear feminine cum pours from her convulsing pussy, splattering on the ground in a thick puddle.\n\n");
			...
			if (player.lowerBody.balls > 0) outputText(ballsDescriptLight());
...
//[Lactaid]
		private function lactaidDoseAKitSune():void
		{
			...
			outputText("You slowly begin to thrust [eachCock] through the slippery passage, revelling in the way her tremendous tits squeeze around your throbbing shaft" + ((player.lowerBody.cockSpot.count() > 1) ? "s" : "" ) + ".  The warmth of her flesh contrasts wonderfully with the cool tingling of her milk coating your cock" + ((player.lowerBody.cockSpot.count() > 1) ? "s" : "" ) + ", heightening your pleasure to incredible levels and making you groan and shiver.  Her chest rises and falls beneath you with every heaving breath she takes, shuddering with her needy moans.\n\n");
...
//[Hotdog Anal] - replaces regular Anal option only for the black-haired girl.
		private function hotdogAnalInKitsuneButtDontLetTailTickleYourNose():void
		{
			...
			outputText(((player.stats.cor > 45) ? "Without waiting to confirm if she is unharmed," : "After briefly confirming that she is for the most part unharmed,") + " you readjust your balance and resume your rhythm, holding onto her shoulders for support.  As you pull back for another thrust, she closes her thighs together gently, causing her prominent cheeks to become even more pronounced.  They serve well to cushion the blows as you pump downward powerfully, jiggling like gelatin with each pounding buck of your hips.\n\n");
...
//[GetBJ] - requires cock 108 area or less
		private function getABJFromAFoxGirl():void
		{
			...
			outputText("Grinning up at you, she dips underneath your shaft, lifting it up with her nose, flattening out her tongue around the underside of your " + ((player.lowerBody.cockSpot.hasSheath()) ? "sheath" : "base") + " and running it slowly upward.  Having her wrists restrained doesn't seem to be hampering her ability to pleasure you much at all - it even seems to be turning her on.  She pushes your " + cockDescript(x) + " up against your stomach using her nose, kissing along the underside longingly and making wet smacking sounds.\n\n");
			...
			if (player.lowerBody.cockSpot.list[x].cockLength > 9) {
				...
				outputText(".  She hungrily swallows along your length, moaning into your engorged prick as she finally presses her lips to your " + ((player.lowerBody.cockSpot.hasSheath()) ? "sheath" : "base") + " and rolls her eyes back in pleasure, her neck bulging slightly from the presence of your shaft.\n\n");
			}
			else outputText("her lips squeezing sensually around the " + ((player.lowerBody.cockSpot.hasSheath()) ? "sheath" : "base") + " each time she goes down you.  Moaning intensely, she closes her eyes and rolls your " + cockDescript(x) + " back and forth in her mouth, swirling her tongue sensually along it.\n\n");
