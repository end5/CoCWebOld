	...
	public class CorruptedGlade extends BaseContent {
		...
		private flowerFun():void {
			...
			if (player.lowerBody.cockSpot.hasCock()) { //Sex scenes for those with cawks
				...
				MainScreen.text("You lay there for some time until your muscle control returns, your cock" + (player.lowerBody.cockSpot.count() > 1 ? "s" : "") + " still slightly over-large " + (player.lowerBody.balls >= 2 ? "and your " + ballsDescriptLight() : "and") + " feeling sore from the exertion.  At least you hope it's just from the exertion and not from whatever the plant did.\n\n");
				if (player.cumQ() < 25)
		...
		private tentacleFun():void {
			...
			if (player.lowerBody.vaginaSpot.count() > 0 && rand(2) == 0) { //Vaginal Variant 50% of the time
				MainScreen.text("You saunter over to a dangling group of perverted looking vines, discarding your " + player.armorName + " along the way. Running your fingertips along the bulbous-tipped tentacle-like vines, you find one that looks ");
				...
				if (tentacle == 0) { //Small
					...
					switch (player.vaginas[0].vaginalLooseness) {
						case VAGINA_LOOSENESS.TIGHT:		tentacleSize =   0; break;
						case VAGINA_LOOSENESS.NORMAL:		tentacleSize =   4; break;
						case VAGINA_LOOSENESS.LOOSE:		tentacleSize =  16; break;
						case VAGINA_LOOSENESS.GAPING:		tentacleSize =  40; break;
						case VAGINA_LOOSENESS.GAPING_WIDE:	tentacleSize =  65; break;
				...
				if (tentacle == 1) { //Normal
					...
					switch (player.vaginas[0].vaginalLooseness) {
						case VAGINA_LOOSENESS.TIGHT:		tentacleSize =   3; break;
						case VAGINA_LOOSENESS.NORMAL:		tentacleSize =   7; break;
						case VAGINA_LOOSENESS.LOOSE:		tentacleSize =  26; break;
						case VAGINA_LOOSENESS.GAPING:		tentacleSize =  60; break;
						case VAGINA_LOOSENESS.GAPING_WIDE:	tentacleSize = 115; break;
				...
				if (tentacle == 2) { //Large
					...
					switch (player.vaginas[0].vaginalLooseness) {
						case VAGINA_LOOSENESS.TIGHT:		tentacleSize =   6; break;
						case VAGINA_LOOSENESS.NORMAL:		tentacleSize =   9; break;
						case VAGINA_LOOSENESS.LOOSE:		tentacleSize =  34; break;
						case VAGINA_LOOSENESS.GAPING:		tentacleSize =  78; break;
						case VAGINA_LOOSENESS.GAPING_WIDE:	tentacleSize = 135; break;
				...
				if (rand(4) == 0 && player.lowerBody.hipRating <= 10) { //+hip up to 10
					MainScreen.text("A strange shifting occurs below your waist, making your " + player.armorName + " feel tight.  <b>Your hips have grown larger</b>, becoming " + hipDescript() + ".  ");
				...
				if (player.lowerBody.cockSpot.count() > 0) { //Cocks (and maybe vagina)
					MainScreen.text(player.lowerBody.cockSpot.count() == 1 ? cockDescript(0) : multiCockDescriptLight());
					if (player.lowerBody.vaginaSpot.count() > 0) MainScreen.text(" and " + vaginaDescript(0));
				}
				else if (player.lowerBody.vaginaSpot.count() > 0) //Vagina
					MainScreen.text(vaginaDescript(0));
				...
				if (rand(4) == 0 && player.lowerBody.butt.buttRating <= 10) {
					MainScreen.text("A strange shifting occurs on your backside, making your " + player.armorName + " feel tight.  <b>Your butt has grown larger</b>, becoming a " + buttDescript() + ".  ");
				...
				if (rand(3) == 0 && player.lowerBody.cockSpot.count() > 0 && player.hairColor == "green") {
					if (player.tentacleCocks() < player.lowerBody.cockSpot.count()) {
						if (player.lowerBody.cockSpot.count() == 1) { //Single cawks
							MainScreen.text("Your feel your " + cockDescript(0) + " bending and flexing of its own volition... looking down, you see it morph into a green vine-like shape.  <b>You now have a tentacle cock!</b>  ");
						...
						if (player.lowerBody.cockSpot.count() > 1) { //multi
							MainScreen.text("Your feel your " + multiCockDescriptLight() + " bending and flexing of their own volition... looking down, you watch them morph into flexible vine-like shapes.  <b>You now have green tentacle cocks!</b>  ");
							for (let x: number = 0; x < player.lowerBody.cockSpot.count(); x++) player.lowerBody.cockSpot.list[x].cockType = CockType.TENTACLE;
						}
					}
				}
			}
