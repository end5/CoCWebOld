	...
	public class CorruptedGlade extends BaseContent {
		...
		private function flowerFun():void {
			...
			if (player.lowerBody.cockSpot.hasCock()) { //Sex scenes for those with cawks
				...
				Render.text("The tendrils encircling your genitals do not release; instead they pull tighter, one of " + (player.lowerBody.cockSpot.count() > 1 ? "each plant's tiny" : "the tiny plant's") + " appendages penetrating your urethra" + (player.lowerBody.cockSpot.count() > 1 ? "s" : "") + ", squirming up your cum slicked passage" + (player.lowerBody.cockSpot.count() > 1 ? "s" : "") + " with uncomfortable slowness.  You lay there, too weak to resist it or fight, hoping that whatever the plants are doing won't hurt much.  You feel it twisting and coiling inside you... until it stops.  You feel a sharp pinch, and then it withdraws, seemingly satisfied.  The tendrils unwrap, allowing the plants to spring back up, exposing your still over-engorged and sensitive member" + (player.lowerBody.cockSpot.count() > 1 ? "s" : "") + ".\n\n");
				
				Render.text("You lay there for some time until your muscle control returns, your cock" + (player.lowerBody.cockSpot.count() > 1 ? "s" : "") + " still slightly over-large " + (player.lowerBody.balls >= 2 ? "and your " + ballsDescriptLight() : "and") + " feeling sore from the exertion.  At least you hope it's just from the exertion and not from whatever the plant did.\n\n");
				if (player.cumQ() < 25)
					Render.text("As you depart, you notice the plants looking remarkably colorful and healthy...");
				else if (player.cumQ() < 250)
					Render.text("As you depart, you notice the plants' stalks bulging slightly from your deposit, their flowers wet with moisture and bright red.");
				else if (player.cumQ() < 500)
					Render.text("As you depart, you note the plants' stalks bulging obscenely, bits of your seed dripping from the flowers' opening.");
				else Render.text("As you depart, you note the plants' stalks bulging out obscenely, looking like overfull balloons.  They're stretched so thin as to be transparent, your cum sloshing about inside them as they attempt to digest their meals.  Steady streams of your jism leak from the flowers' lips, unable to keep it all inside.");
				//Stat changes!
				player.orgasm();
				dynStats("sen", 2);
				...
				Render.text("You walk away, your lips and tongue feeling slightly puffy and sensitive, but none the worse for the wear.");
				player.slimeFeed();
				player.orgasm();
		...
		private function tentacleFun():void {
			...
			if (player.lowerBody.vaginaSpot.count() > 0 && rand(2) == 0) { //Vaginal Variant 50% of the time
				Render.text("You saunter over to a dangling group of perverted looking vines, discarding your " + player.armorName + " along the way. Running your fingertips along the bulbous-tipped tentacle-like vines, you find one that looks ");
				let tentacle: number = rand(3); //Big, medium or small size tentacle
				let tentacleSize: number = 0;
				if (tentacle == 0) { //Small
					Render.text("a little small for your ");
				...
				if (tentacle == 1) { //Normal
					Render.text("well suited to your ");
					switch (player.vaginas[0].vaginalLooseness) {
				...
				if (tentacle == 2) { //Large
					Render.text("almost too big to cram in your ");
					switch (player.vaginas[0].vaginalLooseness) {
				...
				if (player.cuntChange(tentacleSize, true)) Render.text("\n\n"); //Stretch cuuuuunt and newline if it gets stretched
				...
				if (rand(3) == 0 && player.hairColor != "green") { //Change hair to green sometimes
					Render.text("You don't get far before you realize all the hair on your body has shifted to a verdant green color.  <b>You now have green hair.</b>  ");
					player.hairColor = "green";
				}
				if (rand(4) == 0 && player.lowerBody.hipRating <= 10) { //+hip up to 10
					Render.text("A strange shifting occurs below your waist, making your " + player.armorName + " feel tight.  <b>Your hips have grown larger</b>, becoming " + hipDescript() + ".  ");
				...
				Render.text("As you leave the corrupted plant-life behind a comforting warmth seems to radiate from your gut, suffusing you with gentle heat that makes your ");
				
				if (player.lowerBody.cockSpot.count() > 0) { //Cocks (and maybe vagina)
					Render.text(player.lowerBody.cockSpot.count() == 1 ? cockDescript(0) : multiCockDescriptLight());
					if (player.lowerBody.vaginaSpot.count() > 0) Render.text(" and " + vaginaDescript(0));
				}
				else if (player.lowerBody.vaginaSpot.count() > 0) //Vagina
					Render.text(vaginaDescript(0));
				else Render.text("nipples"); //Nipples
				//Finish sentance
				Render.text(" tingle.  ");
				...
				if (rand(4) == 0 && player.lowerBody.butt.buttRating <= 10) {
					Render.text("A strange shifting occurs on your backside, making your " + player.armorName + " feel tight.  <b>Your butt has grown larger</b>, becoming a " + buttDescript() + ".  ");
				...
				if (rand(3) == 0 && player.lowerBody.cockSpot.count() > 0 && player.hairColor == "green") {
					if (player.tentacleCocks() < player.lowerBody.cockSpot.count()) {
						if (player.lowerBody.cockSpot.count() == 1) { //Single cawks
							Render.text("Your feel your " + cockDescript(0) + " bending and flexing of its own volition... looking down, you see it morph into a green vine-like shape.  <b>You now have a tentacle cock!</b>  ");
						...
						if (player.lowerBody.cockSpot.count() > 1) { //multi
							Render.text("Your feel your " + multiCockDescriptLight() + " bending and flexing of their own volition... looking down, you watch them morph into flexible vine-like shapes.  <b>You now have green tentacle cocks!</b>  ");
		...
		private function treeBoobFun():void {
			...
			if (player.findStatusAffect(StatusAffects.LustyTongue) < 0) {
				if (rand(4) == 0) { //25% Chance of sensitive mouth status – increased lust gain/hour due to licking your lips :3
					Render.text("  The feeling doesn't seem to fade, only becoming more and more intense over the coming hour.  It will be hard to keep from getting turned on any time you lick your lips or eat some food.");
					player.statusAffects.add(new StatusAffect("LustyTongue", 24, 0, 0, 0)));
				}
				else Render.text("  Thankfully, the feeling goes away after a few minutes, leaving you feeling just a bit more sensitive.");
			}
			if (rand(2) == 0) { //50% Chance of breast-growth
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 5) {
					Render.text("  As you redress, you feel a sense of added weight on your chest.  After a few moments you realize your already-formidable chest has grown even larger.");
					player.growTits(1, 1 + rand(3), false, 1);
				}
				else {
					if (player.upperBody.chest.BreastRatingLargest[0].breastRating == 0) { //No tits yet
						Render.text("  As you redress, you realize you have grown a pair of luscious breasts!  Perhaps this was a good idea after all...");
						player.growTits(1, 2 + rand(3), false, 1);
					}
					else { //Small tits growth
						Render.text("  As you redress, you realize your breasts have gotten quite a bit larger!  Maybe you can come back later when you aren't so full and plump them up a bit more.");
						player.growTits(1, 1 + rand(3), false, 1);
					}
				}
			}
