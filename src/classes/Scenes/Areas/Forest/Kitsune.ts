...
export default class Kitsune extends Monster {
	...
	private kitsuneEntwine(): void {
		MainScreen.text("The kitsune closes in on you with a mischievous glint in her eyes.  You raise your guard, keeping your eyes trained on her to ensure that she doesn't try to pull anything.  Suddenly, you feel something coiling around your " + LowerBodyDescriptor.describeLeg(player) + ", and let out a yelp as you are suddenly lifted into the air, entangled in the kitsune's tails!");
	...
	public kitsuneStruggle(): void {
		...
		//Success
		if (rand(20) + player.stats.str / 20 + statusAffects.get("PCTailTangle").value1 >= 12) {
	...
	private kitsuneSealAttack(): void {
		...
		//Run:
			...
			MainScreen.text("\"<i>Tsk tsk, leaving so soon?</i>\"  the kitsune says, popping up in front of you suddenly as you attempt to make your escape.  Before you can react, she draws a small circle on your chest with her fingertip, leaving behind a glowing rune made of crackling blue flames.  You try to run the other way, but your " + LowerBodyDescriptor.describeLegs(player) + " won't budge!\n\n\"<i>Sorry baby, you'll just have to stay and play~.</i>\" she says in a singsong tone, appearing in front of you again.  <b>The kitsune's spell prevents your escape!</b>  You'll have to tough it out until the spell wears off.");
	...
	override protected performCombatAction(): void {
		...
		if (!player.statusAffects.has("Sealed")) moves.push(kitsuneSealAttack);
		if (!player.statusAffects.has("Sealed")) moves.push(kitsuneSealAttack);
	...
	public Kitsune(hairColor: string) {
		...
		if (rand(3) != 2) game.Flags.list[FlagEnum.redheadIsFuta] = 1;
		this.a = "a ";
		this.short = "kitsune";
		...
		// this.plural = false;
		if (hairColor == "red" && game.Flags.list[FlagEnum.redheadIsFuta] == 1) {
		...
		this.upperBody.head.hairLength = 13 + rand(20);
