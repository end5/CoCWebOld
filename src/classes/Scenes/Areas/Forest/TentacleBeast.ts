	...
	public class TentacleBeast extends Monster
	{
		private tentaclePhysicalAttack():void {
			...
			if(temp == 0 || (player.stats.spe - spe > 0 && int(Math.random()*(((player.stats.spe-spe)/4)+80)) > 80)) {
				Render.text("However, you quickly evade the clumsy efforts of the abomination to strike you.", false);
			}
			//Hit
			else {
				temp = player.takeDamage(temp);
				Render.text("The tentacles crash upon your body mercilessly for " + temp + " damage.", false);
			}
			combatRoundOver();
		}
		...
		{
			if (hpVictory) {
				Render.text("The creature lets out an ear-piercing screech as it collapses upon itself. Its green coloring quickly fades to brown as the life drains from it, leaving you victorious.", true);
			} else {
				Render.text("The tentacle beast's mass begins quivering and sighing, the tentacles wrapping around each other and feverishly caressing each other.  It seems the beast has given up on fighting.", false);
			}
			if (statusAffects.has("PhyllaFight")) {
				...
				if(!hpVictory && player.gender > 0) {
					Render.text("  Perhaps you could use it to sate yourself?", true);
		...
		{
			if (hpVictory) {
				Render.text("Overcome by your wounds, you turn to make a last desperate attempt to run...\n\n");
				if (statusAffects.has("PhyllaFight")) {
					statusAffects.remove("PhyllaFight");
					Render.text("...and make it into the nearby tunnel.  ");
					game.desert.antsScene.phyllaTentaclePCLoss();
				} else
					game.forest.tentacleBeastScene.tentacleLossRape();
			} else {
				Render.text("You give up on fighting, too aroused to resist any longer.  Shrugging, you walk into the writhing mass...\n\n");
				if(statusAffects.has("PhyllaFight")) {
					statusAffects.remove("PhyllaFight");
					Render.text("...but an insistent voice rouses you from your stupor.  You manage to run into a nearby tunnel.  ");
					game.desert.antsScene.phyllaTentaclePCLoss();
				} else
