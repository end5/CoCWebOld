export default class TentacleBeast extends Monster {
	private tentaclePhysicalAttack(): void {
		...
		let temp: number = int((str + weaponAttack) - Math.random() * (player.stats.tou) - player.armorDef);
		if (temp < 0) temp = 0;
		//Miss
	...
	private tentacleEntwine(): void {
		...
		if (!player.statusAffects.has("TentacleBind")) {
			...
			//Fail
				...
				//Male/Herm Version:
				if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("The creature, having immobilized you, coils a long tendril about your penis. You shudder as the creature begins stroking your cock like a maid at a dairy farm in an attempt to provoke a response from you. Unable to resist, your " + player.CockDescriptor.describeCock(player, 0) + " easily becomes erect, signaling to the creature that you are responsive to harsher stimulation.\n", false);
				//Female Version:
				else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("The creature quickly positions a long tentacle with a single sucker over your clitoris. You feel the power of the suction on you, and your body quickly heats up.  Your clit engorges, prompting the beast to latch the sucker onto your " + player.VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + ".\n", false);
				//Genderless
				else MainScreen.text("The creature quickly positions a long tentacle against your " + game.ButtDescriptor.describeButthole(player) + ". It circles your pucker with slow, delicate strokes that bring unexpected warmth to your body.\n", false);
	...
	public TentacleBeast() {
		...
		this.skinDesc = "bark";
		this.upperBody.head.hairColor = "green";
