	...
	public class BeeGirlScene extends BaseContent
	{
		...
		private function beeEncounterAfraidRepeatSex():void {
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				outputText("gently running her fingers along your " + multiCockDescriptLight() + " bringing " + (player.lowerBody.cockSpot.count() > 1 ? "them" : "it") + " to full hardness" + (player.lowerBody.vaginaSpot.hasVagina() ? ", " : " and "));
			}
			if (player.lowerBody.vaginaSpot.hasVagina()) {
		...
		private function beeEncounterWithWorms():void { //If she won't fuck infested players after combat then she shouldn't fuck them here either
			...
			outputText("The bee maiden puts her hands on your shoulders.  She draws you gently into a kiss, but as her sweet saliva prepares you for what is to come you feel one of your annelid passengers slither down " + (player.statusAffects.has("WormPlugged") ? "from your cervix and poke out of your cunt" : "your urethra and wrap itself around the tip of your cock") + ".  The bee girl pulls back from the kiss, looks down and jumps back, the color draining from her face.\n\n");
		...
		private function beeEncounterClassicSex(postCombat:boolean = true):void {
			...
			if (player.isTaur()) {
				...
				if (player.longestCockLength() >= 100 || player.gender == 0) {
					...
					if (player.gender == 1) {
						...
						//[Without testicles:
						if (player.lowerBody.balls == 0) outputText("With your cock quivering and spasming you continue to lap at the honey covered tree until the vibrating bee-girl finishes.  ", false);
						//[With testicles:
					...
					//[Herm (bypasses the bucking scene):
						...
						//[Without testicles:
						if (player.lowerBody.balls == 0) outputText("With your cock quivering and spasming and your untended to " + vaginaDescript(0) + " squirting repeatedly, you continue to lap at the honey covered tree until the vibrating bee-girl finishes.  ", false);
						//[With testicles:
						else outputText("With your cock quivering and spasming semen erupts from the tip.  ", false);
						//[Little-normal cum amount:
						if (player.cumQ() < 100) outputText("You spurt out more cum than you thought your " + ballsDescriptLight() + " could hold. White speckles the base of the tree and tiny puddles form underneath you as you groan in satisfaction.\n\n", false);
				...
				//Route 1 
					...
					if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) {
						...
						//[Fuckable breasts:
						if (player.upperBody.chest.hasFuckableNipples()) outputText("  Your fingers slip in and out of them, sending femcum flying everywhere. But the effects of the bee's honey appear to prevent you from getting off.", false);
						//[Other:
					...
					if (player.gender == 1) {
						...
						//[Without testicles:
						if (player.lowerBody.balls == 0) outputText("You cum violently, your " + multiCockDescriptLight() + " quivering and spasming.  ", false);
						//[With testicles:
					...
					//[Female:
						...
						//[Tiny-normal clit:
						if (player.lowerBody.vaginaSpot.list[0].clitLength < 3) outputText("Her awkward ministrations to your " + clitDescript() + " cause a gush of fluids from your " + vaginaDescript(0) + " and you let out a lewd moan of satisfaction.  ", false);
						//[Cock-like clit:
			...
			//Dicked version...
				...
				if (player.lowerBody.cockSpot.count() == 1) outputText("member", false);
				if (player.lowerBody.cockSpot.count() > 1) outputText("members", false);
			...
			//non-dicked version
				...
				if (player.lowerBody.vaginaSpot.count() == 0) {
					...
					if (player.upperBody.chest.countNipples() > 1) outputText(", your nipples swollen and red with desire.  ", false);
					else outputText(".", false);
				}
				//Coochie talk!
				else {
					if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS.WET)outputText("centering around your now puffy vulva.  ", false);
					if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.WET && player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING)outputText("growing more potent around your " + vaginaDescript(0) + " as it begins to leak with the honey of your desire.  ", false);
					if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) outputText("inflaming your " + vaginaDescript(0) + " with need, and allowing a river of your wetness to gush from between your puffy vulva.  ", false);
				}
				//Boobies!
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) {
				...
				if (player.lowerBody.vaginaSpot.count() > 0) outputText(" as your hand is guided to your " + vaginaDescript(0), false);
				outputText(".\n\n", false);
				//Cum text
				if (player.lowerBody.vaginaSpot.count() > 0) outputText("Your fingers dance over your clit as you lap up her honey", false);
				...
				if (player.lowerBody.vaginaSpot.count() > 0) outputText(", the constant aphrodisiac honey you're mindlessly devouring, and your fingers' instinctual ravaging of your " + vaginaDescript(0) + ", ", false);
		...
		public function beeSexForCocks(clearScreen:boolean = true):void {
			...
			if (giantCockIndex != -1) {
				...
				outputText("She takes one more longing look at your flesh before going to her bag and rummaging around in it.  She takes out a small bottle and looks back at you for a moment, before slipping the bottle into her dripping honeypot.  She straightens up and looks at you once more, her breath coming in short gasps as she runs a finger over her black luscious lips.  <i>“You’re zzzuch a wonderfully big boy!”</i>  she exclaims, <i>“" + (player.lowerBody.cockSpot.list[0].cockType == CockType.BEE ? "Keep drinking your zzzpezzzial honey and zzzoon you’ll be ready" : "With thizzz zzzpezzzial honey, you’ll zzzoon be even better") + ".”</i>  Given the state of your mind, you can’t do much else other than nod to her.\n\n");
				menu();
		...
		private function beeDroneBadEnd():void {
			...
			if (player.upperBody.wingType == WingType.BEE_LIKE_LARGE) {
				...
				if (player.upperBody.wingType == WingType.NONE) {
					...
					outputText("Your " + player.upperBody.wingDesc + " feel itchy for a moment before falling off entirely.  You don’t have much time to worry about that though, as almost immediately afterwards you sprout a large pair of bee wings that you use to lift yourself into the air.\n\n");
				}
		...
		private function beeTalk():void
		{
			...
			if (attitude < BEE_GIRL_TALKED) { //Replaced beeProgress
				...
				if (player.stats.cor < 33) outputText("You are sure no good can come of this, but your body is ready to say yes.");
				if (player.stats.cor >= 33 && player.stats.cor <= 66) outputText("Her offer intrigues you, and the arousing sweetness of her scent makes it difficult to resist.");
				if (player.stats.cor > 66) outputText("Looking at her through lust-tinted eyes, you're sure she can deliver on her offer.  Getting closer to her scent alone would be worth bearing a few eggs...");
				...
				//If you get unlucky you just get the choice of getting egg-laid.
					...
					if (player.stats.cor < 33) outputText("You are sure no good can come of this, but your body is urging you to agree.");
					if (player.stats.cor >= 33 && player.stats.cor <= 66) outputText("Her offer intrigues you, and the arousing sweetness of her scent makes it difficult to resist.");
					if (player.stats.cor > 66) outputText("Looking at her through lust-tinted eyes, you're sure she can deliver on her offer.  Getting closer to her scent alone would be worth bearing a few eggs...");
					outputText("\n\nDo you accept her offer?");
...
*/
		...
		private function seduceBeeGirl():void
		{
			...
			if (player.gender == 3) {
				...
				if (player.lowerBody.balls > 0) outputText(", sliding down and over your balls,", false);
			...
			if (player.isTaur()) {
				...
				//Lust:
					...
					if (player.gender == 1 && player.lowerBody.balls > 0) {
						...
						if (player.lowerBody.cockSpot.count() == 1) outputText("it as it's ", false);
		...
		private function rapeTheBeeMultiCockStuff():void
		{
			...
			if (player.lowerBody.cockSpot.count() >= 5 && player.upperBody.chest.BreastRatingLargest[0].breastRating > 2
				...
				outputText("The heads of your " + Appearance.cockNoun(CockType.HUMAN) + " rub back and forth against the bee-girl's lips and face, and finally curiosity leads her to begin tasting, her surprisingly long tongue lassoing and swirling around every tip she can reach in turn, darting from one to another as if tasting flowers to provide an experience almost as unique as her pussy.\n\n", false);
		...
		private function rapeTheBeeGirlWithADick():void
		{
			...
			if (player.isTaur()) {
				...
				if (player.cockArea(x) <= monster.vaginalCapacity()) {
					...
					//Has testicles:
					if (player.lowerBody.balls > 0) outputText("  Your " + ballsDescriptLight() + " really feel spectacular, the vibrations causing the hot spunk inside to roil about.", false);
					...
					//has testicles: 
					if (player.lowerBody.balls > 0) outputText(", and your " + ballsDescriptLight() + " constrict", false);
					outputText(".\n\n", false);

					outputText("Your seed is propelled through your pulsating " + Appearance.cockNoun(player.lowerBody.cockSpot.list[x].cockType) + " with surprising force.", false);
					//No testicles: 
					if (player.lowerBody.balls == 0) outputText("  It dribbles into the bee-girl as her honey squirts out of her once again. Her body trembles as the extreme exertions take their toll, and she slides off of your member with a damp plop, splashing into a puddle of her own fluids.", false);
					else {
						//Normal cum amount: 
				...
				//Cock too long to penetrate her:
					...
					//Has testicles: 
					if (player.lowerBody.balls > 0) outputText("  The vibrations do a number on your " + ballsDescriptLight() + " too, as the semen stored within starts to shake and dance about inside.", false);
				...
				if (player.lowerBody.cockSpot.count() == 1) outputText("penetrating her slick snatch, forcing in inch after inch.  ", false);
				if (player.lowerBody.cockSpot.count() == 2) outputText("cramming your " + multiCockDescript() + " into her, double-stuffing the poor bee-slut with your double dongs.  ", false);
				if (player.lowerBody.cockSpot.count() >= 3) outputText("cramming two out of your " + num2Text(player.lowerBody.cockSpot.count()) + " " + multiCockDescript() + "s into her, double-stuffing the poor bee-slut with double the dicks.  ", false);
				...
				if (player.gender == 3 && player.lowerBody != LOWER_BODY.CENTAUR) outputText("  You reach down and finger your " + vaginaDescript(0) + " roughly, lost in the throes of your orgasm.", false);
				outputText("\n\n", false);
				//wind down
...
//FEMALE sometimes herm
		private function rapeABeeGirlWithYourVagina():void
		{
			...
			if (player.isTaur()) {
				...
				//[Fuckable nipples: 
				if (player.upperBody.chest.hasFuckableNipples()) outputText("  Not even repeated penetration of your " + nippleDescript(0) + "s seems to bring the relief you need.", false);
				outputText("\n\n", false);
				...
				//[Breasts >D: 
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 4) outputText("Her legs release your head and she falls back, knocking into your " + player.allBreastsDescript() + " and causing them to jiggle lewdly. You grab her before she hits the ground, surprised at how light she is.", false);
				//[Breasts <=C: 
				...
				if (player.upperBody.chest.hasFuckableNipples() || player.lactationQ() >= 20) outputText(" and ooze fluids in a constant stream down your front", false);
				...
				if (player.upperBody.chest.count() > 1) outputText(" and " + player.allBreastsDescript(), false);
...
//FUTA Fallback
		private function futaRapesBeeGirl():void
		{
			...
			if (player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.NORMAL) outputText("Your " + vaginaDescript(0) + " becomes puffy and moist with excitement, ready for what you have planned.", false);
			if (player.vaginas[0].vaginalWetness > VAGINA_WETNESS.NORMAL && player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) outputText("Your " + vaginaDescript(0) + " squicks wetly from your gentle ministrations as your vulva become sensitive and engorged.", false);
			if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) outputText("Your " + vaginaDescript(0) + " gushes in response, dripping steadily over your victim's face, covering it with a spattering of girl-cum.", false);
			//New PG
			...
			if (player.lowerBody.balls >= 2) {
				if (player.lowerBody.ballSize < 5) outputText("Your " + ballsDescriptLight() + " droop into her eyes, blinding and humiliating her in equal measure.  ", false);
			...
			if (player.cumQ() >= 100) {
				...
				if (player.lowerBody.balls >= 2) outputText(ballsDescriptLight() + " and ", false);
				outputText("the bee-bitch's face.  ", false);
			}
			if (player.upperBody.chest.count() > 0) {
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("You free a hand to tend to your " + player.allBreastsDescript() + " and aching nipples, alternating between squeezing your female flesh and tugging on your erect nipples.  ", false);
			}
			outputText("\n\n", false);
			//New PG
			...
			if (player.lowerBody.balls >= 2) outputText("The warmth and pleasurable pressure of your male orgasm builds in your " + ballsDescriptLight() + ", growing stronger as it migrates up your groin to the base of your pulsing member.  You squeeze your " + cockDescript(0) + " tightly, trying to hold on a bit longer, but the squirming tongue inside you is too much.  ", false);
			//Mmmmmmangasm!
			...
			if (player.cumQ() < 25) {
				...
				if (player.lowerBody != LOWER_BODY.CENTAUR) outputText("Spent at last, you rise up off of her, shivering as her tongue retracts from your woman-hood.", false);
			}
			if (player.cumQ() >= 25 && player.cumQ() < 250) {
				...
				if (player.lowerBody.balls >= 2) outputText("with empty balls, ", false);
				if (player.lowerBody != LOWER_BODY.CENTAUR) outputText("you rise up off her, shivering as her slender and squirming tongue retracts from your " + vaginaDescript(0) + ".", false);
			...
			if (player.cumQ() >= 250 && player.cumQ() < 500) {
				...
				if (player.lowerBody != LOWER_BODY.CENTAUR) outputText("You rise up off of her in mid-orgasm, aiming lower and painting her with each blast of seed.  ", false);
			...
			if (player.cumQ() >= 500) {
				...
				if (player.lowerBody != LOWER_BODY.CENTAUR) outputText("You stand up off your victim, letting her tongue slip free from your soaked folds, and", false);
				...
				if (player.lowerBody.balls >= 2) outputText("Your balls eventually empty", false);
...
//(can replace normal rape victory scenes if corruption>75, and strength>60, and while player has naga tongue, dick, vagina, or d-cup or larger breasts)
		private function beeGirlRapeForTheDistinguishedGentleman():void
		{
			...
			if (select == 0) {
				...
				//[(if penis then)
				if (player.lowerBody.cockSpot.hasCock()) outputText(", spraying the fuzzy side of the bee's abdomen with your sticky come,", false);
			...
			//[(if dick)
			if (player.lowerBody.cockSpot.hasCock()) outputText("stroke your " + Appearance.cockNoun(CockType.HUMAN), false);
			else if (player.lowerBody.vaginaSpot.hasVagina()) outputText("rub your " + vaginaDescript(0), false);
			...
			if (player.lowerBody.cockSpot.hasCock() || player.lactationQ() >= 100 || player.wetness() >= 5) outputText(" and spray your sticky fluids all over her tortured groin", false);
			outputText(".\n\n", false);
		...
		private function beeAlternate():void
		{
			...
			if (rand(2) == 0) {
				...
				if (player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("You loosen your " + player.armorName + " and unleash your cocks.  The equine flare of the largest is already drooling pre as you enjoy the sight of the helpless, squirming bee-girl.  Swinging it like a flogger, you spank her ass, making the girl yelp and leaving trails of sticky pre across the globes of her ass.  Grinning, you drag your shaft back and forth across her pussy, slicking it in her honey.  Finally, you press the tip against her opening.  She's a tight fit for you, and she squeals more loudly as your shaft spreads her little cunny inexorably.  ", false);
				//Catchall scenario
				if (player.lowerBody.cockSpot.list[x].cockType == CockType.DEMON || player.lowerBody.cockSpot.list[x].cockType.Index > 4) outputText("You loosen your " + player.armorName + " and unleash your cocks.  The glans of the largest is already drooling pre as you enjoy the sight of the helpless, squirming bee-girl.  Guiding it forwards, you spank her ass, making the girl yelp and leaving trails of sticky pre across the globes of her ass.  Grinning, you drag your shaft back and forth across her pussy, slicking it in her honey.  Finally, you press the tip against her opening.  She's a tight fit for you, and she squeals more loudly as your shaft spreads her little cunny inexorably.  ", false);
				//CONTINUE
				...
				if (player.lowerBody.balls > 0) outputText(" with your " + ballsDescriptLight() + " hitting her abdomen hard enough to make smacking sounds ring out in the forest around you.", false);
				else outputText(".", false);
				//New PG
				outputText("\n\n", false);
				...
				if (player.lowerBody.cockSpot.count() >= 3) {
					...
					if (player.lowerBody.cockSpot.count() == 4) outputText("  The " + cockDescript(3) + " not inside her shakes wildly as you cum; jets of your semen let loose over her abused frame, coating her in white.", false);
					if (player.lowerBody.cockSpot.count() > 4) outputText("  The " + multiCockDescriptLight() + "s not inside her shake wildly as you cum; jets of your semen let loose over her abused frame, coating her in white.", false);
				}
				//New PG
				outputText("\n\n", false);
				...
				if (player.lowerBody != LOWER_BODY.CENTAUR) outputText("stuffing your " + multiCockDescriptLight() + " inside your " + player.armorName, false);
...
//Naga on Bee Scene
		...
		private function nagaRapesPt2TheExtremeContinuationOfAwesome():void
		{
			...
			if (player.gender == 1 || (player.gender == 3 && rand(3) == 0)) {
				...
				if (player.lowerBody.cockSpot.count() == 1) outputText(" is");
				else outputText(" are");
				...
				if (player.lowerBody.balls > 0) outputText("Not while her hands cup your " + sackDescript() + " and knead tenderly, infusing you with the craving for release; ", false);
				else outputText("not ", false);
				outputText("even when she takes your " + cockDescript(0) + " to the hilt, her eyes straining up in their sockets. You watch " + int(player.lowerBody.cockSpot.list[0].cockLength) + " inches of " + cockDescript(0) + " disappear inside her mouth, and the vibrations in her throat tell you that you'd hear her cooing if it wasn't packed tight.\n\n", false);
				...
				//[if player has testicles]
				if (player.lowerBody.balls > 0) outputText("your " + sackDescript() + " clenches as ", false);
			...
			//[Player is female]
				...
				if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS.WET) outputText("Damp", false);
				else if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) outputText("Wet", false);
				else if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLAVERING) outputText("Dripping", false);
				...
				//[if clit size => 1"]: 
				if (player.lowerBody.vaginaSpot.list[0].clitLength > 1) outputText("  Emergent from its sheath, she uses the base of her tongue as a bottom lip and begins to suck you off like a cock, unceasing in her lingual assault on the inside of your canal.", false);
				outputText("\n\n", false);
				...
				if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLICK) outputText("A few drops ", false);
				else if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) outputText("A gush ", false);
				else if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLAVERING) outputText("Wave after wave ", false);
			...
			//[Player is herm]
				...
				if (player.lowerBody.cockSpot.count() > 1) outputText("each of your shafts", false);
				...
				//[if Player cock => 12\"]
				if (player.lowerBody.cockSpot.list[0].cockLength > 12) outputText(", surprising you with its elasticity", false);
				outputText(".", false);
				...
				if (player.totalCocks() > 1) {
					...
					//[three or more]
					if (player.lowerBody.cockSpot.count() > 2) outputText("s, giving each a few pumps before moving on to the next", false);
				...
				//[if Player has multiple cocks]
				if (player.lowerBody.cockSpot.count() > 1) outputText("jacking, ", false);
		...
		public function beeGirlsGetsDildoed():void {
			...
			if(player.stats.cor < 50) outputText("You feel kind of bad for her, but y", false);
