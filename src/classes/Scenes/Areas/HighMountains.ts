 ...
 */
export default class HighMountains {
	...
	public exploreHighMountain(): void {
		...
		//25% minotaur sons!
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] >= 3 && rand(4) == 0 && player.lowerBody.vaginaSpot.hasVagina()) {
			spriteSelect(44);
	...
	public chickenHarpy(): void {
		...
		if (Flags.list[FlagEnum.TIMES_MET_CHICKEN_HARPY] == 0) {
			MainScreen.text("Taking a stroll along the mountains, you come across a peculiar-looking harpy wandering around with a large wooden cart in tow.  She's far shorter and bustier than any regular harpy you've seen before, reaching barely 4' in height but managing to retain some semblance of their thick feminine asses.  In addition to the fluffy white feathers decorating her body, the bird-woman sports about three more combed back upon her forehead like a quiff, vividly red in color.");
			...
			//Repeat Intro
			MainScreen.text("Taking a stroll along the mountains, you come across a familiar-looking shorty wandering around with a large wooden cart in tow.");
	...
	public giveTwoOviElix(): void {
		...
		player.inventory.items.consumeItem(consumables.OVIELIX);
		MainScreen.text("You hand over two elixirs, the harpy more than happy to take them from you.  In return, she unties a corner of the sheet atop the cart, allowing you to take a look at her collection of eggs.");
	...
	public giveThreeOviElix(): void {
		...
		MainScreen.text("You hand over three elixirs, the harpy ecstatic over the fact that you're willing to part with them.  In return, she unties a side of the sheet atop the cart, allowing you to take a look at a large collection of her eggs.");
		//[Black][Blue][Brown][Pink][Purple]
		menu();
	...
	public getHarpyEgg(itype: ItemType): void {
		...
		MainScreen.text("You take " + itype.longName + ", and the harpy nods in regards to your decision.  Prepping her cart back up for the road, she gives you a final wave goodbye before heading back down through the mountains.\n\n");
	...
	public leaveChickenx(): void {
		MainScreen.clearText();
		spriteSelect(90);
		MainScreen.text("At the polite decline of her offer, the chicken harpy gives a warm smile before picking her cart back up and continuing along the path through the mountains.");
