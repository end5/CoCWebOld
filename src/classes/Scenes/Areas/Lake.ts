	...
	public class Lake extends BaseContent
	{
		...
		public function exploreLake():void
		{
			...
			//Chance of dick-dragging! 10% + 10% per two foot up to 30%
				...
				if (player.stats.cor > 30 || player.lust > 60 || player.stats.lib > 40) outputText("Your imaginings increasingly seem to turn ", false);
				else dynStats("int", 1);
		...
		private function ottergirlLikesDongs():void
		{
			...
			//{(lil dicks)
			if (player.lowerBody.cockSpot.list[x].cockLength < 6) outputText("little, wanna-be cock");
			...
			//[equine]
			if (player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("to your flare, sucking, teasing and ");
			//[canine]
			else if (player.hasKnot(x)) outputText("to the base of your cock, planting sloppy kisses on your knot, ");
			//[demonic]
			else if (player.lowerBody.cockSpot.list[x].cockType == CockType.DEMON) outputText("to the demonic nodules ringing your cock, ");
			//[anemone]
			else if (player.lowerBody.cockSpot.list[x].cockType == CockType.ANEMONE) outputText("to the little wriggling tentacles ringing the head and base of your cock, ");
			...
			if (player.lowerBody.balls > 0 && player.lowerBody.vaginaSpot.hasVagina()) outputText("balls and cunt");
			else if (player.lowerBody.balls > 0) outputText("balls");
			...
			//(dicks 10" or less)
			if (player.lowerBody.cockSpot.list[x].cockLength < 10) outputText("until she comes to rest on your lap");
			//(10"-24")
			else if (player.lowerBody.cockSpot.list[x].cockLength < 24) outputText("slowly devouring your entire cock, until she finally comes to rest on your lap");
			...
			outputText("\n\nWith your entire ");
			if (player.lowerBody.cockSpot.list[x].cockThickness >= 3) outputText("impressive ");
		...
		private function ottersForGals():void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("folds");
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("quivering quim");
			...
			if (player.upperBody.chest.hasFuckableNipples()) outputText("You even slip a finger or two inside, stretching your nipple-cunts out with deliciously pleasurable results.  ");
			outputText("Combined with Callu's tender tongue ");
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("paying lip service to your wet cunt");
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("Your snatch clenches and clamps down on thin air, flooding Callu's face with your feminine juices in a tasty, refreshing spray.");
