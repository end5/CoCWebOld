	...
	public class Lake extends BaseContent
	{
			...
			//Chance of dick-dragging! 10% + 10% per two foot up to 30%
				...
				//OOZE!
					...
					if (player.stats.int >= 25) {
						...
						if (player.statusAffects.has("DungeonShutDown") && player.findStatusAffect(StatusAffects.FactoryOverload) < 0) MainScreen.text("\n\n<b>You are amazed to encounter a slime creature with the factory shut down - most of them have disappeared.</b>", false);
						return;
					}
				...
				if (player.stats.cor > 30 || player.lust > 60 || player.stats.lib > 40) MainScreen.text("Your imaginings increasingly seem to turn ", false);
				else dynStats("int", 1);
		...
		private ottahGirl():void
		{
			...
			if (flags[FlagEnum.MET_OTTERGIRL] == 1) {
				...
				if (player.findStatusAffect(StatusAffects.FactoryOverload) < 0) MainScreen.text("crystal clear, giving you a stunning view of the lakebed");
			...
			if (flags[FlagEnum.MET_OTTERGIRL] > 1) MainScreen.addButton(2, "Get Fish", getSomeFishYaFatty);
		...
		private ottergirlLikesDongs():void
		{
			...
			//{(lil dicks)
			if (player.lowerBody.cockSpot.list[x].cockLength < 6) MainScreen.text("little, wanna-be cock");
			...
			//[equine]
			if (player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) MainScreen.text("to your flare, sucking, teasing and ");
			//[canine]
			else if (player.hasKnot(x)) MainScreen.text("to the base of your cock, planting sloppy kisses on your knot, ");
			//[demonic]
			else if (player.lowerBody.cockSpot.list[x].cockType == CockType.DEMON) MainScreen.text("to the demonic nodules ringing your cock, ");
			//[anemone]
			else if (player.lowerBody.cockSpot.list[x].cockType == CockType.ANEMONE) MainScreen.text("to the little wriggling tentacles ringing the head and base of your cock, ");
			...
			//(dicks 10" or less)
			if (player.lowerBody.cockSpot.list[x].cockLength < 10) MainScreen.text("until she comes to rest on your lap");
			//(10"-24")
			else if (player.lowerBody.cockSpot.list[x].cockLength < 24) MainScreen.text("slowly devouring your entire cock, until she finally comes to rest on your lap");
			...
			MainScreen.text("\n\nWith your entire ");
			if (player.lowerBody.cockSpot.list[x].cockThickness >= 3) MainScreen.text("impressive ");
