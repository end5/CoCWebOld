...
//<Hyena Victorious – Anal> 
		public function hyenaSpearLossAnal():void
		{
			...
			//<Paragraph for Non-Taurs>
				...
				if (player.lowerBody.balls > 0) outputText("clamping onto your " + sackDescript() + " and squeezing until ", false);
				else if (player.lowerBody.vaginaSpot.hasVagina()) outputText("grabbing your " + clitDescript() + " until ", false);
			...
			if (player.tailType > TAIL.NONE) outputText("One furred paw wraps around your tail before pulling up sharply, exposing your " + assholeDescript() + " to the hot, savanna air.  Sharp fangs clamp onto the tail, holding it up to rob you of any remaining modesty.", false);
			...
			//<Heroes with Penises>
			if (player.lowerBody.cockSpot.hasCock()) outputText("  Each time the shaft slides into you, a jolt of urgent pleasure shoots through you as her thrusts slam against your prostate.", false);
			//<Others>
			...
			//<Male> 
			if (player.lowerBody.cockSpot.hasCock()) outputText("Your cum spurts from your " + multiCockDescriptLight() + ", spraying your seed over the dry ground, giving life to the savanna.", false);
			//<Females>
			else if (player.lowerBody.vaginaSpot.hasVagina()) outputText("Your own juices flow freely from your " + vaginaDescript() + ", coating your thighs and dripping onto the dry grass below you.", false);
			...
			//<For Slimes> 
			if (player.lowerBody == LOWER_BODY.GOO) outputText("  Even as your eyes slide closed, you see the hyena kicking open the circle of dust.", false);
...
//<Hyena Defeat - Fellatio>
		private function hyenaVictoryRapeFellatio():void
		{
			...
			if (player.lowerBody.cockSpot.list[x].cockLength > 7) outputText("The tip of your member presses hard against the back of her mouth for a long moment until she swallows, letting your " + cockDescript(x) + " slide into her tight throat.  ", false);
			...
			if (player.lowerBody.cockSpot.list[x].cockLength > 7) outputText("in the back of her mouth ", false);
			outputText("as it starts to be pulled from your encased member.\n\n", false);

			//<One Cock> 
			if (player.lowerBody.cockSpot.count() == 1) outputText("Even as the most incredible sensations flow through you from the feel of her muzzle, you see one of her spotted hands drop from her pendulous breast to grip her own shaft.  Her second hand follows, sliding two furred fingers into the tip of her strange shaft as she starts to move up and down the dark flesh in time to the suckling of her muzzle.  Her dark brown eyes slowly slide closed in bliss as she pleasures herself while servicing the shaft buried deep within her maw.\n\n", false);
			//<Two Cocks>
				...
				if (player.lowerBody.balls > 0) outputText("  Her second paw finally cups your " + ballsDescriptLight() + ", squeezing and massaging to add to the powerful sensations.", false);
				outputText("\n\n", false);
			}
			//Three or More Cocks>
			if (player.totalCocks() > 2) {
				...
				if (player.lowerBody.cockSpot.count() == 3) outputText("your other ", false);
			...
			//<Cock 0 is 7\" or greater> 
			if (player.lowerBody.cockSpot.list[x].cockLength >= 7) outputText("The muzzle starts to pull away from your " + cockDescript(x) + " until the tip finally slips from her throat, letting her gasp for breath.  ", false);
			...
			if (player.lowerBody.cockSpot.list[x].cockLength >= 7) {
				...
				//<Two Cocks> 
				if (player.lowerBody.cockSpot.count() == 2) outputText("  Your second " + cockDescript(y) + " throbs in her hand, spraying her face and throat with thick ropes of cum.", false);
				//<Three or More Cocks>
				if (player.lowerBody.cockSpot.count() >= 3) outputText("  Your " + multiCockDescriptLight() + " throb in her paws, coating her muzzle, neck, and breasts with thick ropes of cum.", false);
