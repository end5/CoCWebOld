...
public function bunnbunbunMeet():void {
	...
	if(flags[FlagEnum.MET_BUNBUN] == 0) {
		...
		if(player.stats.lib > 50) outputText("and you find yourself wondering what sort of treasures she conceals behind it.", false);
		else if(player.stats.cor < 40) outputText("and you find yourself thankful for her modesty.", false);
	...
	//Met her
		...
		else if(player.lowerBody.cockSpot.count() > 0) outputText("  <b>You're too to big fit inside her...</b>", false);
...
private function talkToBunnyBunBun():void {
	...
	else if(player.lowerBody.cockSpot.count() > 0) outputText("  <b>You're too big to fit inside her...</b>", false);
	if(player.gender > 0) {
		sixtyNine = bunbun69;
		outputText("  Sixty-nine her?", false);
	}
...
private function bunbunFucksYourVag():void {
	...
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) outputText("  Moisture leaks in a steady drizzle, mixing with bunny-pre to totally slick the soon-to-be invading member.", false);
	outputText("\n\n", false);
	...
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLICK) outputText(", not that you needed it", false);
	...
	if(player.tallness >= 72) {
		...
		if(player.stats.cor < 33) outputText("You'll have to try to slow your descent next time!  ", false);
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength >= 4) outputText("  Your " + clitDescript() + " bounces on her belly, tingling like mad every time it slaps into her tanned, sweat-slicked skin.", false);
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength >= 4) outputText("fondle your " + clitDescript() + " a moment before wrapping your hand around it and stroking the girl-cock excitedly.", false);
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		...
		if(player.lowerBody.cockSpot.list[0].cockLength < 10) outputText("on the bunny's belly", false);
		else if(player.lowerBody.cockSpot.list[0].cockLength < 18) outputText("between the bunny's tiny tits", false);
		else if(player.lowerBody.cockSpot.list[0].cockLength < 28) outputText("on the bunny's face", false);
		else outputText("past the bunny's head", false);
		outputText(", and your free hand wastes no time in tending to your male ");
		if(player.lowerBody.cockSpot.count() == 1) outputText("half's needs, slathering it in pre-cum and stroking hard and fast.", false);
...
private function bunbunFucksPCInAss():void {
	...
	if(player.analCapacity() < 20) outputText("it stretches you dangerously wide as you slide down the thick shaft.  The further down it gets, the more you have to work to relax your muscles and push yourself along.  With enough time, patience, and copious bunny-pre, you manage to take it completely inside you.  It twitches happily while you adjust to the intrusion, but you know you're just getting started.", false);
		...
		if(player.stats.cor < 50) outputText("girl", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLICK) outputText("Juices begin to run from your " + vaginaDescript(0) + ", but you ignore the empty feeling and focus on the throbbing meat inside your backdoor.  ", false);
	}
	...
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText(", and your " + vaginaDescript(0) + " clenching and dripping on the verge of orgasm", false);
	outputText(".", false);
	if(player.biggestLactation() >= 1) {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your " + vaginaDescript(0) + " clenches, empty, drooling, and neglected, wishing it something inside it.", false);
	outputText("\n\n", false);
...
private function bunbunGetsFucked():void {
	...
	if(y != -1) outputText("Her tight asshole slowly loosens around your " + cockDescript(y) + ", and so much lubricant splatters from her cunt with each thrust that the sensations of both your dicks become nigh-identical, save for the tight ring gripping the bottom one.", false);
		...
		if(player.lowerBody.balls > 0) outputText(", belly, and balls", false);
	...
	if(player.lowerBody.balls > 0) outputText(", slapping your balls against her ass", false);
	...
	if((player.lowerBody.cockSpot.count() == 2 && y == -1) || player.lowerBody.cockSpot.count() > 2) outputText("and onto ", false);
...
private function bunbun69():void {
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		...
		if(isEaster()) {
			...
			//[8"+ : 
			if(player.lowerBody.cockSpot.list[0].cockLength >= 8) outputText("  As she passes six inches into her throat, her sucking grows more anxious, her arms wrapping around your hips, fingers digging into your ass, drawing your groin down faster.", false);
			//[12"+ : 
			...
			if(!player.lowerBody.vaginaSpot.hasVagina()) outputText("prostate", false);
			...
			if(player.tailType != TAIL.RABBIT) {
				//[No Tail: 
				if(player.tailType == TAIL.NONE) outputText("  The heat around your lower back clutches at your tail bone and you feel a fluffy explosion swell around your butt cheeks as <b>a fluffy bunny's tail pops out of your back</b>, twitching excitedly!", false);
				//[Existing tail: 
		...
		//REGULAR SHOW
			...
			if(player.lowerBody.balls > 0) outputText("balls", false);
			else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("clit", false);
			...
			if(player.tailType != TAIL.RABBIT) {
				//[No Tail: 
				if(player.tailType == TAIL.NONE) outputText("  The heat around your lower back clutches at your tail bone and you feel a fluffy explosion swell around your butt cheeks as a fluffy bunny's tail pops out of your back, twitching excitedly!", false);
				//Existing tail: 
			...
			if(player.upperBody.head.earType != EARS.BUNNY) outputText("new ", false);
...
public function layEggsInBunbuns():void {
	...
	if(player.gender == 0) outputText(" against the empty place of your crotch.");
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText(" alongside [eachCock]");
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" and her hefty oval balls rubbing the [vagina] tucked between your thighs");
		 outputText(".");
	}
	...
	if(player.gender > 0) {
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("spunk");
		if(player.gender == 3) outputText(" and ");
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("femcum");
	}
	...
	if(player.gender > 0) {
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("spraying a long, thick load against the bunny's soft tits from your [eachCock]");
		if(player.gender == 3) outputText(" and ");
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("splashing her balls with your girlcum");
		outputText(".");
	}
...
private function fuckTheEggBoundBun():void {
	...
	if(player.stats.cor < 33) outputText("grimace at the vulgar goo that washes over your flesh");
	else if(player.stats.cor < 66) outputText("swallow a gulp as the lewd spoo squelches around your member");
...
private function getEggflated():void {
	...
	//Futa:
	if(player.lowerBody.cockSpot.hasCock()) outputText("  The steady movement of the orbs pushing past your prostate drills another, even messier orgasm from your " + multiCockDescriptLight() + ", cum arcing into the air to shower down on the two of you in pearl beads.");
	...
	//Futa:
	if (player.lowerBody.cockSpot.hasCock()) {
		outputText("Your exhausted " + multiCockDescriptLight() + " somehow find" + (player.lowerBody.cockSpot.count() > 1 ? "" : "s") + " the strength for a gushing orgasm of " + (player.lowerBody.cockSpot.count() > 1 ? "their" : "its") + " own, strands of spoo spilling across your own body, white hot spunk bringing a fresh sheen to your sweat-slick [chest].  ");
	}
