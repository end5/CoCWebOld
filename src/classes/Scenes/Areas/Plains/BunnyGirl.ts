...
public bunnbunbunMeet():void {
	...
	spriteSelect(13);
	if(flags[FlagEnum.MET_BUNBUN] == 0) {
	...
	//Met her
		...
		MainScreen.text("You lean forwards for a better look, dislodging a small pebble with your " + player.foot() + " while shifting position.  The bunny-girl leaps to her feet in a panic until her terrified irises lock onto your " + player.face() + ".  Her panic turns to embarrassment and in a flash every inch of her tanned skin is colored red.  She asks, \"<i>Ummm, since I'm still in s-season, c-could we b-breed or something?  Maybe just a little fuck?</i>\"\n\n", false);
...
private rapeBunBun():void {
	...
	if(player.stats.spe < 60) {
		MainScreen.text("You lunge forward off your " + player.feet() + ", trying to tackle and pin the poor girl, but at the first sign of movement from you, she bounds off in the other direction!  She's hopping so fast there's no way you could possibly catch her, and in a matter of seconds you're left totally alone.  Well, perhaps not TOTALLY alone – there's one small egg nestled in the grass.  It fell from the bunny's basket in her haste to flee!", false);
...
private bunbunFucksYourVag():void {
	spriteSelect(13);
	MainScreen.text("", true);
	MainScreen.text("A anticipatory grin widens across your " + player.face() + " as you speedily disrobe, discarding your " + player.armorName, false);
	
	if(player.weaponName != "fists") MainScreen.text(" and " + player.weaponName, false);
	MainScreen.text(" in a hurry.   Meanwhile the bunny looks on in a semi-aroused stupor, stroking her length with one hand while her other teases one of the hard nubs of her nipples.  You sigh with excitement as you position yourself above her, lining up her somewhat angular crown with the entrance to your " + vaginaDescript(0) + " before you start to drop.", false);
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) MainScreen.text("  Moisture leaks in a steady drizzle, mixing with bunny-pre to totally slick the soon-to-be invading member.", false);
	MainScreen.text("\n\n", false);
	...
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLICK) MainScreen.text(", not that you needed it", false);
	...
	MainScreen.text("While the bunny was content to remain passive up until this point, a fire lights in her eyes now that she's had a taste of pussy.  Her petite hands grab hold of your " + assDescript() + " while she pulls her powerful, oddly-jointed legs underneath of her.  A split second later she pushes up with enough force to bounce you a few inches into the air before you drop down on top of her.  ", false);
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength >= 4) MainScreen.text("  Your " + clitDescript() + " bounces on her belly, tingling like mad every time it slaps into her tanned, sweat-slicked skin.", false);
	if(player.biggestLactation() >= 1) MainScreen.text("  Milk begins to bead on your " + player.allBreastsDescript() + " from the sensation of the bunny's brutal, almost mechanical fucking.", false);
	...
	MainScreen.text("The girl underneath you is thrashing and moaning, chanting, \"<i>Yes... eggs eggs EGGS! YES!</i>\" while her ovipositor-like cock robs you of your strength and slides egg-shaped bulges inside you.  Amazingly, your " + vaginaDescript(0) + " is awash with pleasure, and you reach down to ", false);
	if(player.lowerBody.vaginaSpot.list[0].clitLength >= 4) MainScreen.text("fondle your " + clitDescript() + " a moment before wrapping your hand around it and stroking the girl-cock excitedly.", false);
	else MainScreen.text("caress your " + clitDescript() + " a moment before you start to circle it with a finger, teasing it expertly.", false);
	if(player.lowerBody.cockSpot.count() > 0) {
		MainScreen.text("  " + SMultiCockDesc() + " twitches and drools ", false);
		if(player.lowerBody.cockSpot.list[0].cockLength < 10) MainScreen.text("on the bunny's belly", false);
		else if(player.lowerBody.cockSpot.list[0].cockLength < 18) MainScreen.text("between the bunny's tiny tits", false);
		else if(player.lowerBody.cockSpot.list[0].cockLength < 28) MainScreen.text("on the bunny's face", false);
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		MainScreen.text("  At last, " + sMultiCockDesc() + " blasts sticky ropes of seed everywhere, cumming with you as your body slides sideways off the exhausted bunny into the dirt.  A few thick strands hit her tits and face, but judging by her half-closed eye-lids and questing tongue, she doesn't mind.", false);
		if(player.cumQ() > 500) MainScreen.text("  Not content with that, " + sMultiCockDesc() + " continues to pump until the bunny is coated with a generous layer of seed", false);
...
private bunbunFucksPCInAss():void {
	...
	else if(player.analCapacity() < 40) MainScreen.text("you have to keep pausing as you slide down to let more of her pre-cum bubble out and lubricate the fourteen inch shaft.  It fills you nicely, pressing on your innards in all the right ways as you slowly engulf the bunny-cock with your " + assholeDescript() + ".  After bottoming out, your nervous lover's prick pulsates happily inside you, but you're just getting started.", false);
	else {
		MainScreen.text("you wish it was a bit bigger.  A few seconds is all it takes to get her thick shaft completely inside your " + assholeDescript() + ".  You clench and squeeze your muscles around it as you sit on the bunny-", false);
	...
	if(player.totalCocks() > 0) MainScreen.text("Each time she bumps and slides past your prostate a dollop of your pre-cum spurts from " + sMultiCockDesc() + " onto the ground, and you have a hard time not using your new lover like an anal toy and masturbating yourself.  ", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLICK) MainScreen.text("Juices begin to run from your " + vaginaDescript(0) + ", but you ignore the empty feeling and focus on the throbbing meat inside your backdoor.  ", false);
	}
	...
	if(!player.upperBody.chest.hasFuckableNipples()) {
		...
		if(player.biggestLactation() >= 1) {
			...
			if(player.biggestLactation() >= 4) MainScreen.text("sprays", false);
	...
	MainScreen.text("  The bunny reaches down to your " + assDescript() + " and gives it a gentle caress and squeeze.  A moment later both her hands are holding your butt-cheeks, guiding you up and down as you bounce atop her faster and faster.\n\n", false);
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		MainScreen.text(", and " + sMultiCockDesc() + " pouring incredibly sticky pre-cum each time she squeezes your prostate", false);
	}
	else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(", and your " + vaginaDescript(0) + " clenching and dripping on the verge of orgasm", false);
	MainScreen.text(".", false);
	if(player.biggestLactation() >= 1) {
		MainScreen.text("  Milk ", false);
		if(player.biggestLactation() < 2) MainScreen.text("leaks", false);
		else if(player.biggestLactation() < 4) MainScreen.text("drips", false);
		else MainScreen.text("pours", false);
		MainScreen.text(" from your " + nippleDescript(0) + "s, turning your front to a leaky mess as you ride the very edge of pleasure.", false);
	}
	MainScreen.text("\n\n", false);
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		MainScreen.text("and as it squeezes past your prostate, " + sMultiCockDesc() + " spurts thick cum on the ground, though you don't QUITE orgasm.", false);
	}
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		MainScreen.text("  " + SMultiCockDesc() + " erupts in truth, spraying and splattering white goop over the grass and ground.", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Your " + vaginaDescript(0) + " clenches, empty, drooling, and neglected, wishing it something inside it.", false);
	MainScreen.text("\n\n", false);
...
private bunbunGetsFucked():void {
	...
	MainScreen.text("You disrobe and toss your " + player.armorName + " to the side, immediately forgetting about it as the bunny-girl lifts her sack to expose the bright pink flesh of her femininity.  It glistens, practically steaming up the air with her plentiful lubricants.  She runs a slender finger around the moist hole, beckoning you to plunge inside and fill her hungry flesh with your " + cockDescript(x) + ".  Lust burns through you as you drop to your knees and line your " + cockDescript(x) + " up with that ready opening", false);
	if(y != -1) MainScreen.text(", taking care to align your " + cockDescript(y) + " with her tightly puckered backdoor as well.", false);
	...
	MainScreen.text("and your " + cockDescript(x), false);
	if(y != -1) MainScreen.text(" and " + cockDescript(y) + " sink", false);
	...
	if(y != -1) MainScreen.text("Her tight asshole slowly loosens around your " + cockDescript(y) + ", and so much lubricant splatters from her cunt with each thrust that the sensations of both your dicks become nigh-identical, save for the tight ring gripping the bottom one.", false);
	else {
	...
	MainScreen.text(" rubs her passion-inflamed pussy-lips.  " + SMultiCockDesc() + " unloads, splattering out ropes of cum into ", false);
...
private bunbun69():void {
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		...
		if(isEaster()) {
			...
			//[8"+ : 
			if(player.lowerBody.cockSpot.list[0].cockLength >= 8) MainScreen.text("  As she passes six inches into her throat, her sucking grows more anxious, her arms wrapping around your hips, fingers digging into your ass, drawing your groin down faster.", false);
			//[12"+ : 
			...
			if(player.tailType != TAIL.RABBIT) {
				//[No Tail: 
				if(player.tailType == TAIL.NONE) MainScreen.text("  The heat around your lower back clutches at your tail bone and you feel a fluffy explosion swell around your butt cheeks as <b>a fluffy bunny's tail pops out of your back</b>, twitching excitedly!", false);
				//[Existing tail: 
		...
		//REGULAR SHOW
			...
			if(player.tailType != TAIL.RABBIT) {
				//[No Tail: 
				if(player.tailType == TAIL.NONE) MainScreen.text("  The heat around your lower back clutches at your tail bone and you feel a fluffy explosion swell around your butt cheeks as a fluffy bunny's tail pops out of your back, twitching excitedly!", false);
				//Existing tail: 
			...
			if(player.upperBody.head.earType != EARS.BUNNY) MainScreen.text("new ", false);
...
private getEggflated():void {
	...
	//Futa:
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("  The steady movement of the orbs pushing past your prostate drills another, even messier orgasm from your " + multiCockDescriptLight() + ", cum arcing into the air to shower down on the two of you in pearl beads.");
	...
	//Futa:
	if (player.lowerBody.cockSpot.hasCock()) {
		MainScreen.text("Your exhausted " + multiCockDescriptLight() + " somehow find" + (player.lowerBody.cockSpot.count() > 1 ? "" : "s") + " the strength for a gushing orgasm of " + (player.lowerBody.cockSpot.count() > 1 ? "their" : "its") + " own, strands of spoo spilling across your own body, white hot spunk bringing a fresh sheen to your sweat-slick [chest].  ");
	}
