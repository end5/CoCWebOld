		...
		public function getRapedByGnoll():void
		{
			...
			if (player.lowerBody.cockSpot.count() > 0) {
				...
				if (player.thickestCockThickness() > 2) {
					//(if cockTotal>1)
					if (player.lowerBody.cockSpot.count() > 1) outputText("The gnoll fishes into your " + multiCockDescriptLight() + " and quickly snaps up the thickest one.  ", false);
					//(if cockTotal=1)
					else outputText("The gnoll's hands dart down to your crotch and grabs " + cockDescript(0) + ".  ", false);
					outputText("\"<i>Yes, this will do nicely,</i>\" she says with relish, pumping your " + Appearance.cockNoun(player.lowerBody.cockSpot.list[player.thickestCock()].cockType) + " until it plumps up, filling with blood.  She cantilevers her body over yours, knees bent sharply, and brings the tip of her massive clitoris up against the head of your dick.  The moment a dollop of pre forms at your urethra, her hips surge forward, sinking her giant clit down the length of your " + Appearance.cockNoun(player.lowerBody.cockSpot.list[player.thickestCock()].cockType) + ".  ", false);
				...
				//(if cockTotal>0 AND thickestCock()<2")
					...
					//(if cockTotal>1)
					if (player.lowerBody.cockSpot.count() > 1) outputText("\"<i>This will have to do,</i>\" she says with the barest trace of disappointment as she grabs your thickest cock.  ", false);
					//(if cockTotal=1)
			...
			//(if breastRating>C)
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) outputText("  Your " + player.allBreastsDescript() + " bounce up and down as the gnoll grinds her anatomy into yours, slick with sweat under the hot sun.  Your " + nippleDescript(0) + "s tighten and flush as your whole body submits to the rough fuck administered by the hyena girl.", false);
			//(if cockTotal>0)
			if (player.lowerBody.cockSpot.count() > 0) {
...
//DickDownClit
		private function dickDownGnollClit():void
		{
			...
			if (player.lowerBody.cockSpot.list[x].cockLength > 9) outputText("Finally you can feel your cockhead push free into a deeper, wetter place.  You look down at her strange anatomy, the entire length of her tube bulging thicker to accommodate your " + cockDescript(x) + ".  You've made it all the way to her vagina!\n\n", false);
			...
			//(if balls>0)
			if (player.lowerBody.balls > 0) outputText("  It doesn't take long before your " + ballsDescriptLight() + " begin twitching, pumping cum down the length of your encased cock.", false);
		...
		private function dickInGnollAss():void
		{
			...
			if (rand(2) == 0) {
				...
				if (player.totalCocks() > 1 && player.cockArea(x) < monster.analCapacity() && y != -1) {
					outputText("With a smirk, you pound away for a few minutes to get her loosened up.  Then you reach down and slap another " + Appearance.cockNoun(CockType.HUMAN) + " alongside the first.  With a grunt and a thrust, you shove both of them inside.", false);
					//(if cockTotal>2)
					...
					//(if cockTotal>3)
					if (player.lowerBody.cockSpot.count() > 3 && player.cockArea(0) + player.cockArea(1) + player.cockArea(2) < monster.analCapacity()) outputText("  From there it's some time before her anus is stretched wide enough for more.  By now you can feel your orgasm on the horizon, so you're less careful with your fourth " + cockDescript(y) + ".  You slap it into the bundle of dicks and then ram it home.", false);
					//(if cockTotal>4)
					if (player.lowerBody.cockSpot.count() > 4 && player.cockArea(0) + player.cockArea(1) + player.cockArea(2) + player.cockArea(3) < monster.analCapacity()) outputText("  And then the next.", false);
					//(if cockTotal>5)
					if (player.lowerBody.cockSpot.count() > 5 && player.cockArea(0) + player.cockArea(1) + player.cockArea(2) + player.cockArea(4) + player.cockArea(5) < monster.analCapacity()) outputText("  And the next.", false);
					//(if cockTotal>6)
					if (player.lowerBody.cockSpot.count() > 6 && player.cockArea(0) + player.cockArea(1) + player.cockArea(2) + player.cockArea(4) + player.cockArea(5) + player.cockArea(6) < monster.analCapacity()) outputText("  And the next, until all of your " + multiCockDescriptLight() + " are inside her wide-spread ass.", false);
					outputText("\n\n", false);
				}
				...
				if (player.lowerBody.cockSpot.count() > 1) outputText(" while the remainder of your endowments slide along her bare buttocks", false);
				...
				if (player.cumQ() <= 250) {
					...
					if (player.lowerBody.cockSpot.count() > 1) {
						...
						if (player.lowerBody.cockSpot.count() > 2) outputText("s soak", false);
				...
				//Med CumQ):
					...
					if (player.lowerBody.cockSpot.count() > 1) outputText(", unleashing a torrent of semen that mats down the short fur on her back and rear as", false);
				...
				//(High CumQ):
					...
					if (player.lowerBody.cockSpot.count() > 1) {
						if (player.lowerBody.cockSpot.count() > 2) outputText(" while your remaining endowments soak her back thoroughly, covering her in a thick blanket of spunk", false);
...
//TakeHerClit
		...
		private function yoDawgHyenaBallz2():void
		{
			...
			if (player.lowerBody.balls > 0) outputText("your own " + ballsDescriptLight(), false);
			...
			if (player.lowerBody.balls > 0) outputText(" your distended sac, ballooning with the fatigued flood of the prostate-milking the girl is giving you", false);
