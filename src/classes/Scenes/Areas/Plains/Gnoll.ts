	...
	public class Gnoll extends Monster 
	{
		...
		private function gnollAttackText():void {
			...
			//Determine if dodged!
				...
				if(player.stats.spe - spe < 8) outputText("You narrowly avoid " + a + short + "'s " + weaponVerb + "!\n", false);
				else if(player.stats.spe - spe >= 8 && player.stats.spe-spe < 20) outputText("You dodge " + a + short + "'s " + weaponVerb + " with superior quickness!\n", false);
				else if(player.stats.spe - spe >= 20) outputText("You deftly avoid " + a + short + "'s slow " + weaponVerb + ".\n", false);
			}
			//Determine if evaded
