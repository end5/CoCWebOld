...
internal function loseToSatyr():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Upon finding one, he grins lustfully, his already impressively erect cock rising to full flag.");
	...
	if((player.lowerBody.vaginaSpot.hasVagina() && player.vaginalCapacity() < monster.cockArea(0)) || (!player.lowerBody.vaginaSpot.hasVagina() && player.analCapacity() < monster.cockArea(0))) outputText(", not even bothered by the fact that his cock doesn't even fit inside you");
	outputText(".");
	//[cunt/buttchange]
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  [EachCock] throbs as it send strings of cum flying straight into the satyr's chest, painting it white.");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  A veritable flood of girlcum escapes the small space between your vaginal walls and the satyr's massive shaft, covering his balls and legs with your female fluids.");
	...
	if(player.stats.cor < 50) outputText("shamefully ");
...
private function femaleTakesAdvantageOfSatyr():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", your own cock belching its usual load across his stomach and the surroundings");
	outputText(".");
...
private function willinglyBoneSatyr():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("hardening maleness, giving it a few strokes to bring it to full mast, then he moves on to your ");
	if(player.lowerBody.balls > 0) outputText("[balls], rolling each of your orbs around with reverence at the life-giving load they produce; finally he addresses your ");
	//((if PC has a pussy)
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("[vagina], spreading your labia wide to see the interior of your rapidly moistening walls.");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("[vagina], licking all over your netherlips and tasting your juices.");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) outputText("clenching butt");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("pussy gushes fluids down his massive manhood");
	if(player.gender == 3) outputText(" and ");
	if(player.lowerBody.cockSpot.hasCock()) outputText("cock spurts cream across your intertwined forms");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina() && flags[FlagEnum.SATYR_KIDS] == 0) {
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("males");
...
public function satyrBirth(vag:boolean):void {
	...
	if(player.stats.cor < 33) outputText(" and more than a bit disgusted");
