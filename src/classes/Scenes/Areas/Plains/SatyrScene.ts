...
export default class SatyrScene {
	...
	*/
	...
	public satyrEncounter(location: number = 0): void {
		...
		if (rand(2) == 0 || player.pregnancyIncubation > 0 || player.buttPregnancyIncubation > 0 || player.gender == 0) {
			MainScreen.text("As you cross the ");
			if (location == 0) MainScreen.text("grassy plains");
			else MainScreen.text("sodden expanse of the swamp");
			MainScreen.text(", you hear lewd bellowings and drunken curses.  From out of the expanse of green comes a humanoid figure with a set of goat-like horns curling from his head.  Seeing you, he lets out a randy bleat and charges, naked erection jabbing before him!");
		...
		//Pregnant PCs can't get this encounter.
			...
			MainScreen.text("You wander through the ");
			if (location == 0) MainScreen.text("grassy plains");
			else MainScreen.text("sodden expanse of the swamp");
			MainScreen.text(" when you hear strange music emanating not far from where you are.  Do you investigate?");
			//[Yes][No]
	...
	private keepDrinking(): void {
		MainScreen.clearText();
		spriteSelect(98);
		MainScreen.text("You grin at the satyr's encouragement and continue drinking, setting on a slower pace so you won't spill any more; shortly you pass him the empty skin and ask for more.");
		...
		//[hymen check]
		if (player.lowerBody.vaginaSpot.hasVagina()) player.cuntChange(25, true, true, false);
		...
		MainScreen.text(".  This must be the work of that satyr!  Mentally, you remind yourself to watch out for him next time.  You clean yourself up as best as you can and redress, then wobble your way towards your camp, trying to stifle the pain, in your head and elsewhere, along the way.");
	...
	private leavePartySatyr(): void {
		MainScreen.clearText();
		spriteSelect(98);
		MainScreen.text("You thank the satyr for his generosity, but you don't think you can handle this kind of booze, so you get up and start on your way back to your camp.");
	...
	private trickZeSatyr(): void {
		MainScreen.clearText();
		spriteSelect(98);
		MainScreen.text("You come up with a plan and pretend to start drinking again; once you notice the satyr is distracted, you quickly spill most of your drink on the floor and return an empty skin to him.");
	...
	internal function loseToSatyr(): void {
		...
		spriteSelect(98);
		//[Lust loss
		if (player.stats.lust > 99) MainScreen.text("You pant as you begin masturbating furiously, too horny to care about anything the grinning satyr before you has in mind.");
		else MainScreen.text("You try to steady yourself, clutching your body in pain as the satyr grins at you.");
		...
		if (player.lowerBody.isNaga()) MainScreen.text("pins down your serpentine body");
		else if (player.isBiped()) MainScreen.text("spreads your [legs]");
		else MainScreen.text("pins you down");
		MainScreen.text(" and reaches forward, probing for a vagina.  ");
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("Upon finding one, he grins lustfully, his already impressively erect cock rising to full flag.");
		else MainScreen.text("Finding you lacking that trait, he grunts disdainfully and probes roughly for an asshole, which he locates right between your buttcheeks where it belongs, then jerks himself with several strokes to help coax his masculinity to full erection.");
		MainScreen.text("He grabs your [ass] and roughly squeezes your cheeks, kneeling down while lifting you up so he can impale your [vagOrAss].");
		...
		if ((player.lowerBody.vaginaSpot.hasVagina() && player.vaginalCapacity() < monster.cockArea(0)) || (!player.lowerBody.vaginaSpot.hasVagina() && player.analCapacity() < monster.cockArea(0))) MainScreen.text(", not even bothered by the fact that his cock doesn't even fit inside you");
		MainScreen.text(".");
		//[cunt/buttchange]
		if (player.lowerBody.vaginaSpot.hasVagina()) player.cuntChange(monster.cockArea(0), true, true, false);
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  [EachCock] throbs as it send strings of cum flying straight into the satyr's chest, painting it white.");
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  A veritable flood of girlcum escapes the small space between your vaginal walls and the satyr's massive shaft, covering his balls and legs with your female fluids.");
		...
		else MainScreen.text("stomach");
		MainScreen.text("; your belly begins to swell and bulge as he pumps liters of semen into you.");
		...
		if (player.stats.cor < 50) MainScreen.text("shamefully ");
		else MainScreen.text("shamelessly ");
		MainScreen.text(" gather and re-dress in your garments and head back to camp, cum still dribbling from you as you go.");
	...
	internal function defeatASatyr(): void {
		...
		spriteSelect(98);
		//Lust Victory
		if (monster.lust > 99) MainScreen.text("The satyr collapses to its caprine knees, bleating in dismay as it paws frantically at its huge cock, oblivious to everything in its need to get off.  Already, pre-cum is fountaining from the goat-man's shaft, his jerking motions smearing the pungent sexual fluid across the crown.");
		//HP Victory
		else MainScreen.text("Beaten and dazed, the satyr collapses to its caprine knees, shaking his head in a futile attempt to recover himself from the brutal trouncing you've just given him.  The combination of the blows and his previous drunken state mean he's quite incapable of getting back, however.");
		let butt: Function = null;
		let faces: Function = null;
		if (player.stats.lust >= 33 && player.gender > 0) {
			...
			//[Male][Female][Leave]
			if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(monster.analCapacity()) >= 0) butt = malesTakeAdvantageOfSatyrs;
			else if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("\n\nYou're too big to fuck his ass...");
			if (player.lowerBody.vaginaSpot.hasVagina()) faces = femaleTakesAdvantageOfSatyr;
		}
		let bikiniTits: Function = null;
		if (player.lowerBody.vaginaSpot.hasVagina() && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 4 && player.armor == armors.LMARMOR) bikiniTits = (player.armor as LustyMaidensArmor).lustyMaidenPaizuri;
	...
	private femaleTakesAdvantageOfSatyr(): void {
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(", your own cock belching its usual load across his stomach and the surroundings");
		MainScreen.text(".");
	...
	private malesTakeAdvantageOfSatyrs(): void {
		...
		MainScreen.text("\n\nThe moment your fingers slide inside the helpless satyr he bleats, not in horror, but in delight.  His insistent struggles to get away from you make a complete U-turn, and instead he struggles to get more of your hand inside his gripping hole; his cock throbs and grows even harder, spurting pre on the ground below, as his balls churn and slosh eager for the prostate massage you'll be giving him with your own " + CockDescriptor.describeCock(player, x) + ".  You can't help but laugh at the satyr's change of heart; what a slut.");
		...
		//Low Cum Amount
		if (player.cumQ() < 250) MainScreen.text("  He tries to milk you for far more than you can give him, and at one point you feel like his anal muscles are going to tear your dick off, until finally they go lax and lets you slip out.");
		//(Medium Cum Amount)
		else if (player.cumQ() < 1000) MainScreen.text("  You dump far more than you usually do; the satyr's clenching asshole makes sure to drain you of every little drop of cum you have until finally it goes lax and lets you slip out, leaving the goat-man with a pudgy belly.");
		//(High Cum Amount)
		else MainScreen.text("  You shoot jet after jet of cum, far faster than the satyr's ass can milk you for it, and still he doesn't seem to stop trying.  His rippling muscles draw your cum inside his bowels in big bubbles.  When his ass finally goes lax, you're propelled halfway out of him by the force of your own backflowing seed; the satyr's ass looks like a wreck, and his belly is overflowing with so much spunk you're surprised it didn't come out of his mouth.");
	...
	private willinglyBoneSatyr(): void {
		MainScreen.clearText();
		spriteSelect(98);
		MainScreen.text("The satyr eyes you up and down hungrily; his hands move to grope your [chest], gently tweaking your [nipples], then he moves down towards your ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("hardening maleness, giving it a few strokes to bring it to full mast, then he moves on to your ");
		if (player.lowerBody.balls > 0) MainScreen.text("[balls], rolling each of your orbs around with reverence at the life-giving load they produce; finally he addresses your ");
		//((if PC has a pussy)
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("[vagina], spreading your labia wide to see the interior of your rapidly moistening walls.");
		else MainScreen.text("[ass], spreading your cheeks apart to gaze at your winking rosebud.");

		MainScreen.text("\"<i>Wonderful!  With a [vagOrAss] like this I'm sure you'll make a great " + player.mf("father", "mother") + " for our children,</i>\" he says, leaning closer to sniff at your ");
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("[vagina], licking all over your netherlips and tasting your juices.");
		else MainScreen.text("[butt], licking all around your hole to prepare it for what's coming.");
		...
		if (!player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("clenching butt");
		else MainScreen.text("spasming vagina");
		MainScreen.text(".  It's like someone shoved a hose into you and opened the nozzle, filling you with an endless stream of hot spooge; you can feel it pooling inside you, inflating your belly with his gigantic load.");
		...
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("pussy gushes fluids down his massive manhood");
		if (player.gender == 3) MainScreen.text(" and ");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("cock spurts cream across your intertwined forms");
		MainScreen.text(".  Moaning softly and heaving in great lungfuls of breath, you sink back down onto him, feeling well and truly sated.");
		...
		MainScreen.text("; making slow, short thrusts to ensure you've got all the seed that you need.  Panting, he grins at you from his prone position.  \"<i>Tell me then... did you like it?  Think that was enough baby batter to put a little satyr inside you?  Because if not I can go again.</i>\"  He grins confidently.");

		//(if male/genderless and 0 satyr children)
		if (!player.lowerBody.vaginaSpot.hasVagina() && Flags.list[FlagEnum.SATYR_KIDS] == 0) {
			...
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("males");
			else MainScreen.text("neuters");
			MainScreen.text(" like you, lover.</i>\"");
	...
	public satyrBirth(vag: boolean): void {
		...
		if (vag && !player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("newly grown vagina");
			player.createVagina();
		}
		else if (vag) MainScreen.text("[vagina]");
		else MainScreen.text("[butt]");
		MainScreen.text(", a gripping, writhing sensation as if something is thrashing around inside you and wants to get out.  Looks like it's time for whatever is inside you to emerge.");
		...
		if (vag) MainScreen.text("[vagina]");
		else MainScreen.text("[asshole]");
		MainScreen.text("; it's adorned with a pair of miniscule horns.  You groan in pain as its shoulders stretch you even wider, but the worst part of it is over and, within moments, you are delivered of your child.  ");
		if (vag) player.cuntChange(40, true, false, true);
		else player.buttChange(40, true, false, true);
		MainScreen.text("Long minutes pass as your body recovers from the strain, bouncing back with unnatural vitality from the exhaustion and pain of giving birth, but finally you are ready to look at it.");
		...
		if (player.stats.cor < 33) MainScreen.text(" and more than a bit disgusted");
		MainScreen.text(", you slip into a short, fitful sleep.");
		//badabingbadaboom
