	...
	public class FemaleSpiderMorphScene extends BaseContent implements TimeAwareInterface
	{
		...
		private function defeatFemale():void
		{
			...
			if (!getGame().inCombat) {
				...
				if (player.stats.cor < 33) outputText("nervously", false);
				else if (player.stats.cor < 66) outputText("playfully", false);
				...
				if (player.lowerBody.cockSpot.hasCock()) {
					if (player.lust < 70) {
						...
						if (player.lowerBody.cockSpot.count()) outputText("s", false);
			...
			//(Combat Intro: Lust Loss)
				...
				if (player.lowerBody.cockSpot.hasCock()) outputText(SMultiCockDesc() + " flop about lewdly, leaving trails of pre-cum everywhere that they touch.  ", false);
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("the other side of your chest", false);
			...
			if (player.tongueType == TONUGE.SNAKE) outputText("the snake-like length the whole way into her womb, slithering right past her cervix.", false);
			else if (player.tongueType == TONUGE.DEMONIC) outputText("the massive, demonic length into her vagina, curling it around the walls and stroking them all with even motions.", false);
		...
		private function spiderMorphFemaleRidesACawk():void
		{
			...
			if (player.lowerBody.cockSpot.count() > 1) {
				...
				if (player.lowerBody.cockSpot.count() == 2) outputText("a similar ring");
				else outputText("similar rings");
				outputText(" for your other penis", false);
				if (player.lowerBody.cockSpot.count() > 2) outputText("es", false);
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("dragging over your own " + nippleDescript(0) + "s", false);
		...
		private function femaleSpiderMorphTooBigWebRape():void
		{
			...
			if (!getGame().inCombat) {
				...
				if (player.lowerBody.cockSpot.count() > 1) outputText(", ignoring the other, smaller members", false);
			...
			if (player.lowerBody.balls == 0) outputText("body is completely empty", false);
		...
		private function fSpiderMorphRape():void
		{
			...
			if (player.lowerBody.vaginaSpot.list[0].clitLength < 4) outputText(", bumping and gliding against the black spider-cooch's sopping lips.", false);
			...
			if (player.lowerBody.vaginaSpot.list[0].clitLength >= 4) outputText("is seized by her velvet embrace just right, and you cum while fucking her with your 'tiny' nub.", false);
			...
			if (player.stats.cor < 33) outputText("In a moment of pity, you work to free her from the restraints before you leave.", false);
			else if (player.stats.cor < 66) outputText("You feel a little guilty, and before you go, you untie the bindings around her hands so that she'll be able to free herself.", false);
		...
		private function fSpiderMorphRapeDude():void
		{
			...
			if (player.lowerBody.cockSpot.count() > 1) {
				...
				if (player.lowerBody.cockSpot.count() == 2) outputText("cock flops about uselessly", false);
			...
			if (player.stats.cor < 50) outputText("teasing the eager twat", false);
			...
			if (player.lowerBody.balls > 0) outputText(" and slap your balls into her soaked taint", false);
			outputText(", " + cockDescript(x) + " twitching", false);
			if (player.lowerBody.cockSpot.count() > 1) {
				outputText(" while the ", false);
				if (player.lowerBody.cockSpot.count() > 2) outputText("rest spasm pointlessly", false);
			...
			if (player.stats.cor < 33) outputText("you wipe away her tears and kiss her, explaining that any new untainted life is a blessing.  She brightens at your kind words and returns her attention to her belly, still a bit dazed by it all.", false);
			else if (player.stats.cor < 66) outputText("you shrug and sigh, wiping the tears from her eyes and feeling a slight twinge of guilt.  She still sniffles and rubs her belly, obviously confused about the new development.", false);
		...
		private function evilSpiderGirlVictoryAnal():void
		{
			...
			if (player.lowerBody.balls > 0) outputText("with your balls squirming and unloading", false);
