...
export default class AbstractSpiderMorph extends Monster {
	override protected performCombatAction(): void {
		if (player.stats.spe >= 2 && rand(2) == 0) {
			spiderMorphWebAttack();
		}
		else if (!player.statusAffects.has("WebSilence") && rand(3) == 0) {
			spiderSilence();
		}
		else if (!player.statusAffects.has("Disarmed") && player.weaponName != "fists" && rand(3) == 0) {
			spiderDisarm();
		}
	...
	public spiderMorphWebAttack(): void {
		...
		//Got hit
			...
			if (!player.statusAffects.has("Web")) {
	...
	public getBitten(): void {
		...
		if (player.stats.spe < 2 && player.statusAffects.has("Web")) {
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				MainScreen.text(player.CockDescriptor.describeMultiCockSimpleOne(player, true) + " turns rock hard and squirts weakly, suddenly so aroused that it starts soaking your " + player.inventory.armor.displayName, false);
				if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" along with your " + player.VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)), false);
				MainScreen.text(".  ", false);
			}
			else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("Your " + player.VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " grows wet as hell and so sensitive that every step and movement reminds you of the powerful need for something between your sopping nether-lips.  ", false);
			MainScreen.text("While " + mf("his", "her") + " venom pours into you, the spider-" + mf("boy", "girl") + " reaches into your gear to play with your " + player.BreastDescriptor.describeNipple(0) + ", and you moan like a whore from the dual stimulation of " + mf("his", "her") + " venom and nipple-play.\n\n", false);
			...
			if (player.stats.lust > 99) MainScreen.text("wobble, utterly defeated and about to cave in to your lust.", false);
