...
public function encounterRogarSwamp():void {
	...
	if(flags[FlagEnum.ROGAR_PHASE] == 0) {
		...
		if(player.stats.cor < 70) outputText("You nod at him, wiping at the sludge on your body before frowning at yourself.  Ro'gar rumbles thoughtfully, eyeing you over again.  \"<i>My house's over yonder.  Ain't nobody but m'self 'round here, but I kin find ya somethin' to wipe yer face with.</i>\"  Ro'gar offers his friendly smile again, his formidable rows of teeth defeating the gesture.  Though vainly, you try to ignore the teeth and smile back at the generosity.  Ro'gar's thick finger points to the ground.  \"<i>Ya jus' wait right here an' I'll be back,</i>\" he says as he turns, trotting through the mud casually.  Should you really wait for him?\n\n", false);
		...
		//((Corruption 70 or more))
			...
			if(player.isNaga() || player.tailType > TAIL.NONE) outputText("  Your tail flicks about happily.", false);
		...
		//((intellect 30 or more))
			...
			//[(lib > 50)
			if(player.stats.lib >= 50) outputText("  Your loins burn", false);
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2 && player.stats.lib >= 50) {
			...
			if(player.stats.int <= 30) {
				...
				if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  When you witnessed him make that other beast submit with strength alone it created a demanding need in your " + vaginaDescript() + " during the match, and the memory is doing much the same now.", false);
				//((if player has cock(s))
				if(player.lowerBody.cockSpot.hasCock()) outputText("  You feel " + sMultiCockDesc() + " shift as you remember how hard you got from behind the tree, finding yourself absent mindedly stroking yourself with an open palm as pre-cum soaks through your clothing.", false);
		...
		//((high femininity or breasts >=B-cup, libido less than 50))
			...
			//[(int <30) 
			if(player.stats.int < 30) outputText("Ro'gar apologizes for wrestling for his territory in your sight.", false);
			//(int>=30) 
		...
		//((androgynous or masculine and breasts <= A-cup, libido less than 50))
			...
			if(player.stats.int < 30) outputText("leftovers from that fight in the swamp still linger on his body.", false);
	...
	//Not sure what determines this yet
		...
		//((androgynous or masculine and breasts <= A-cup))
			...
			//(Dirt Mc Girt = 2)
			if(flags[FlagEnum.ROGAR_DIRT] == 2) outputText("\"<i>" + player.short + ", y'all took off on me real quick like,</i>\" he says through a snicker.  The joke falls flat and the orc seems to blush lime green as you stare at him uncomprehendingly.  He rubs the back of his neck with a hand before continuing.  \"<i>Sorry about that.  I was plumb tuckered out afterwards.</i>\"\n\n", false);
			...
			if(player.stats.lib > 50) {
				...
				if(player.lowerBody.cockSpot.hasCock()) outputText("bulge", false);
...
private function waitForChunkyOrcLoe():void {
	...
	if(flags[FlagEnum.ROGAR_PHASE] == 0) {
		...
		if(player.stats.lib < 50) outputText("The hulking orc is surprisingly gentle as he cleans you like a young child.  The warm waters and careful touch soothe your tired muscles.  Your skin becomes textured with goose bumps as the tickles of droplets roll down your body and the filth is washed down to the waters below.  You find yourself sighing in relaxation from the treatment.  A smile curls your lips as you look down at Ro'gar, and he returns the pleased look with a toothy smirk.\n\n", false);
		//((If libido is 50 or more))
			...
			else if(player.lowerBody.cockSpot.hasCock()) outputText("  " + SMultiCockDesc() + " stirs to life, making it obvious how much you are enjoying the rub down, then begins to drool with lust.  Through lidded eyes you faintly make out Ro'gar's toothy smirk becoming a hungry grin.", false);
...
private function okayBroLetsHaveAGayCarwash():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(sMultiCockDesc(), false);
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your " + vaginaDescript(0), false);
	...
	//(int <30)
	if(player.stats.int < 30) outputText("  That fight between him and his opponent went on for quite a while, so it makes some sense.", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("erection", false);
	...
	//((androgynous or masculine and breasts <= A-cup))
		...
		//(else if Dirt Mc Girt = 2)
			...
			if(player.gender == 2 || player.gender == 0) {
				...
				if(player.stats.cor < 50) outputText("blush", false);
...
public function rogarPhaseFour():void {
	...
	if((player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) && flags[FlagEnum.ROGAR_WARNING] == 0) {
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText(" and your " + chestDesc(), false);
	...
	//(andro or masculine with tits <= A)
		...
		if(player.gender == 2 || player.gender == 0) {
			...
			if(flags[FlagEnum.ROGAR_DIRT] == 1) outputText("</i>\"I know ya probably ain't ever imagined hangin' out with a feller like me, but... I could really use the company tonight.</i>\"  ", false);
...
private function loseButtGinity():void {
	...
	if(player.ass.analLooseness == 0) {
		...
		if(player.stats.cor < 33) outputText("shyly ", false);
		else if(player.stats.cor > 66) outputText("eagerly ", false);
		...
		//(if player has balls) 
		if(player.lowerBody.balls > 0) outputText("  Ro'gar's dark green cockhead rubs over your " + ballsDescriptLight() + " and coats the plump sack in his musky goo, like an alpha male marking his territory.", false);
		outputText("\n\n", false);
	...
	//(([Get Anal] Scenario 1b: Get fucked (normal)))
		...
		if((player.thickness >= 75 && player.tone < 33) || player.tone >= 75) {
			...
			if(player.lowerBody.balls > 0) outputText(" and your " + ballsDescriptLight() + " clench tight as you feel your limit reached", false);
			outputText(".", false);
			//[(if player has toes)
		...
		//(else)
			...
			//[(If player has balls) 
			if(player.lowerBody.balls > 0) outputText("  You feel your " + ballsDescriptLight() + " pull up tight in your sack as your labored breath comes out in pants.", false);
...
private function fuckRogarsButtPussyBoyCuntManMoundSissySlitQueerQuim():void {
	...
	//((else If smallest cock is over 18 inches))
		...
		//(If player has balls) 
		if(player.lowerBody.balls > 0) outputText("  Ro'gar squirms under you, his chest heaving with labored breath while his musky scrotum presses against your own.", false);
	...
	//((If cock is between 18 and 4 inches))
		...
		if(player.lowerBody.cockSpot.list[0].cockType == CockType.CAT) outputText("  You grind around in his green pucker, the barbs working over Ro'gar's inner walls.  \"<i>W-what in tarnation?</i>\" Ro'gar grunts as you watch him squirm under you.  You buck your hips, digging deeper into orcflesh with each thrust, his muscled ass tensing with each slam of your hips.", false);
		//(If horse cock)
		else if(player.lowerBody.cockSpot.list[0].cockType == CockType.HORSE) outputText("  The flared head of your " + cockDescript(0) + " nestles in Ro'gar's hole.  His breath quickens as you push in, the girthy, flared head stretching Ro'gar's insides impossibly.  You simply thrust and buck you way deeper within the green pucker to force more of your lengthy shaft past that olive rosebud each time.\n\n", false);
		//(If dog cock)
		...
		if(player.lowerBody.cockSpot.count() > 2) outputText("  Your other cocks grind over that firm orc rump, drooling over his skin and making his rear glisten with your sex.", false);
		else if(player.lowerBody.cockSpot.count() > 1) outputText("  Your other cock grinds over that firm orc rump, drooling over his skin and making his rear glisten with your sex.", false);
		...
		//[(if player has balls)
		if(player.lowerBody.balls > 0) outputText("  With short jerks of your hips, you begin to unload your pent up orbs into the orc's innards.", false);
		...
		//[(if multiple cock)
		if(player.lowerBody.cockSpot.count() > 1) outputText("  The other meat rubbing over his olive skin outside adds to the coating of spunk, making a giant mess of Ro'gar's body.  In seconds he resembles a well-paid whore beneath your " + multiCockDescriptLight() + ".", false);
		outputText("\n\n", false);
	}
	//end right fit path
...
private function frotWithRogar():void {
	...
	//(If single cock)
		...
		//[(If player has balls)
		if(player.lowerBody.balls > 0) outputText("Your " + ballsDescriptLight() + " slap audibly against Ro'gar's heavy green sack.  ", false);
...
private function kinkyWithDaOrc():void {
	...
	if(player.stats.lib < 33) outputText("feigned confusion ", false);
	else if (player.stats.lib > 66) outputText("clear lust ", false);
