...
public function encounterRogarSwamp():void {
	Render.text("", true);
	spriteSelect(86);
	if(flags[FlagEnum.ROGAR_PHASE] == 0) {
		...
		//((androgynous or masculine and breasts <= A-cup, libido less than 50))
			...
			Render.text("As you look around the hut you notice something you hadn't seen before.  A bucket and towel has been tucked into a corner on the floor.  Reminded of your first encounter by it, you turn to Ro'gar and thank him for helping you out before, which makes him grin in turn.  \"<i>S'no problem at all.  It's easy to make a mess of yerself in this swamp.</i>\"  Ro'gar chuckles softly.  Your eyes wander over the filth caked on his skin; ", false);
			if(player.stats.int < 30) Render.text("leftovers from that fight in the swamp still linger on his body.", false);
			else Render.text("it makes you wonder what Ro'gar was up to.", false);
			Render.text("  Ro'gar follows your gaze, then his eyes glint oddly as he snickers.  \"<i>Care ta... return the favor?</i>\"  He slyly winks at you, eyes flicking from you to the bucket and back again.", false);
			//goto sexy choices
		}
...
private function dontWaitForRogar():void {
	Render.text("", true);
	spriteSelect(86);
	Render.text("Time passes with no sign of Ro'gar.  You decide it would be a waste of time to sit and wait for him, and head off in hopes of finding something interesting.\n\n", false);
	
	Render.text("Instead, you find only more mud.  You return to camp.", false);
	//<set Crying Game = 1>
	flags[FlagEnum.ROGAR_DISABLED] = 1;
	doNext(camp.returnToCampUseOneHour);
}
...
public function rogarThirdPhase():void {
	...
	if((player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2)) {
		if(flags[FlagEnum.ROGAR_WARNING] == 0) {
			...
			Render.text("You greet him warmly, but he seems a bit nervous around you.  \"<i>Well... after all ya tol' me about yer adventures, I, ah, thought I'd hit the road m'self.  An' so ya find me here.  Good ta see ya again, but I'm just finishin' up my drink afore I head on out.</i>\"  With this, he tips back the contents of his can and flashes you his standard grin, then departs.  He seemed a bit uneasy around you.", false);
			//set Ro'roh Raggy flag to 1
			flags[FlagEnum.ROGAR_WARNING] = 1;
		}
		//(high fem or breasts >=B-cup and Ro'roh Raggy =1)
		else {
			Render.text("Ro'gar the orc is here drinking again, but as you catch his eye, a pained expression flashes across his face.  It's clear he doesn't relish the prospect of talking to you right now, but knowing him, he's too polite to say so.  The wind spills from your sails and your icebreaker slips from your mind - answering him with a simple glance of resignation is the most you can manage.  He gives you a weak smile and returns to his drink.\n\n", false);
		}
...
public function rogarPhaseFour():void {
	...
	if((player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) && flags[FlagEnum.ROGAR_WARNING] == 0) {
		Render.text("Ro'gar the orc is drinking at the bar again, two crushed cans sitting by his half-empty one.  The bartender is casting irritated glances at them as though she'd like to throw them away, but doesn't dare take them from the enormous, burly orc until he finishes or leaves.  You tug on his cloak as you take a seat next to him, and he turns to you with a toothy smile.  That smile dims to a frown as he looks you over, eyes lingering on the womanly contours of your face", false);
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) Render.text(" and your " + chestDesc(), false);
		Render.text(".  \"<i>Gee, ya look... different.  Been samplin' some of the local foods, huh?</i>\"  You agree cautiously, and he flags the bartender over and buys you a drink.  \"<i>I'm just bein' polite since we're pals,</i>\" the orc ventures.  \"<i>Ya kin pay me back another time.</i>\"  You nod, sip your drink, and converse reservedly with him for a while, then go on your way.", false);
		//set Ro'roh Raggy = 1
		flags[FlagEnum.ROGAR_WARNING] = 1;
		doNext(camp.returnToCampUseOneHour);
	}
	//(high fem or breasts >=B-cup and Ro'roh Raggy =1)
	else if((player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) && flags[FlagEnum.ROGAR_WARNING] == 1) {
		Render.text("Ro'gar the orc is here drinking again, but as you catch his eye, a pained expression flashes across his face as he looks over your form.  It's clear he doesn't relish the prospect of talking to you right now, but knowing him, he's too polite to say so.  The wind spills from your sails and your icebreaker slips from your mind - answering him with a simple glance of resignation is the most you can manage.  He gives you a weak smile and returns to his drink.", false);
		//return to bar menu
		doNext(telAdre.barTelAdre);
	}
	//(andro or masculine with tits <= A)
	else {
		Render.text("Ro'gar is tipping his drink back when he catches sight of you, and waves you over.  \"<i>Ah, good ta see ya, " + player.short + "!", false);
		//[(Ro'roh Raggy = 1)
		if(flags[FlagEnum.ROGAR_WARNING] == 1) {
			Render.text("Yer lookin' much better today; ya been workin' out?</i>\"  You smile at the compliment, and ", false);
		}
		else Render.text("</i>\"  You ", false);
		Render.text("wave the bartender over for another round.  \"<i>So,</i>\" he begins once you've finished your drinks and she's out of earshot, \"<i>I was wonderin' if ya would like to hang out at my place later?  It ain't much, but it'd be a lot more... private... than here.</i>\"  He colors slightly and waggles an eyebrow at you, then slides a hand into your groin.", false);
		//[(if F or U)
		if(player.gender == 2 || player.gender == 0) {
			Render.text("  It reaches the center without interruption, and Ro'gar colors deeper.  \"<i>Uh... ferget it,</i>\" he mumbles.  \"<i>Didn't mean ta be indecent with you or anythin', miss.</i>\"  He nods to you and quickly departs before you can recover from the surprise.", false);
			doNext(camp.returnToCampUseOneHour);
		}
		else {
			Render.text("  " + SMultiCockDesc() + " trembles under his strong touch, as he increases the pressure and looks into your eyes meaningfully.  ", false);
			//[(if Dirt Mc Gurt flag = 1)
			if(flags[FlagEnum.ROGAR_DIRT] == 1) Render.text("</i>\"I know ya probably ain't ever imagined hangin' out with a feller like me, but... I could really use the company tonight.</i>\"  ", false);
			Render.text("He's clearly in the mood for some fun; do you want to head back to his place and do something?", false);
...
private function noTHanksRogarIAintGayDudeIjustLickedThatMudToBeNice():void {
	Render.text("", true);
	spriteSelect(86);
	Render.text("You smile at the orc, but tell him that you're not really in the mood to hang out right now.  He looks crestfallen, but nods at you.  \"<i>Well, I understan'.  I'll be seein' ya later, then.</i>\"", false);
	//Bar menu!
	doNext(telAdre.barTelAdre);
}
...
private function fuckRogarsButtPussyBoyCuntManMoundSissySlitQueerQuim():void {
	...
	//((If cock is between 18 and 4 inches))
		...
		if(player.lowerBody.cockSpot.list[0].cockType == CockType.CAT) Render.text("  You grind around in his green pucker, the barbs working over Ro'gar's inner walls.  \"<i>W-what in tarnation?</i>\" Ro'gar grunts as you watch him squirm under you.  You buck your hips, digging deeper into orcflesh with each thrust, his muscled ass tensing with each slam of your hips.", false);
		//(If horse cock)
		else if(player.lowerBody.cockSpot.list[0].cockType == CockType.HORSE) Render.text("  The flared head of your " + cockDescript(0) + " nestles in Ro'gar's hole.  His breath quickens as you push in, the girthy, flared head stretching Ro'gar's insides impossibly.  You simply thrust and buck you way deeper within the green pucker to force more of your lengthy shaft past that olive rosebud each time.\n\n", false);
		//(If dog cock)
		else if(player.hasKnot(0)) Render.text("  Your pointed tip slips in easily and you begin to plunge into Ro'gar's depths, and the burly orc gives satisfied grunts as his strong hand reaches down to touch himself.  You begin to fill his orc hole, the pre-slicked cock working as a lubing tool as you begin to build a nice rhythm within Ro'gar.  Your knot bulges as your climax approaches and begins to ram into Ro'gar's green pucker, the tight ring unable to allow entry as you swell bigger and bigger.", false);
		//(If other cock)
		else Render.text("  Breaking Ro'gar's orc hole open, you thrust deep within his hot, tight depths.  The burly orc pants with heated breaths, humid air fluttering out of his tusked mouth.  His musclegut heaves, each breath punctuating the thrusts of your hips.  You plow his ass roughly, pulling out until only the head remains inside him before plunging back in further than before.  Ro'gar grunts low in his chest with each bit of progress made, his strong, calloused hand stroking his drooling cock.  \"<i>Mmmf, " + player.short + ",</i>\" he groans as you finally hilt yourself within him.", false);
...
private function rogarIsDumb():void {
	Render.text("", true);
	spriteSelect(86);
	Render.text("You relinquish the can with a smile, slyly telling Ro'gar he can have it, then put your hands behind your back innocently.  He narrows his eyes, then shifts them to the can.  After a careful inspection, he thrusts the can toward you.  \"<i>Don't want it.  Ya probably put bimbo liqueur in it or somethin', ya sneaky devil.</i>\"\n\n", false);

	Render.text("The playful mood broken, you hang out for a while and shoot the breeze, then head back.", false);
	//end
	doNext(camp.returnToCampUseOneHour);
}
...
private function takeDatBroBrewFromDaBigMeanOlOrc():void {
	...
	if(changed) {
		Render.text("  You realize you look much stronger, bigger, and sexier", false);
		//(if dick grew)
		if(cocked) Render.text(", and you smile to yourself as your now-larger endowment rubs against garments not suited to it", false);
		Render.text(".  Who needs book smarts anyway?  Nerds.", false);
...
private function kinkyWithDaOrc():void {
	...
	Render.text("over the threshold, and then look around the apartment.  He points out the small kitchen area and then, rather sheepishly, the bedroom.  You're examining it when something catches your eye...\n\n", false);	
	Render.text("You bend over the bed to grip a small bottle lying in the soft fabric of the sheets.  Turning it over, you tilt your head as you read the label.  It's... a bottle of lubricant.  Suddenly, there is a loud thud behind you and the bottle drops from your hand as your torso is pressed into the cushioned surface of the bed.  Something presses into your back.  \"<i>Gotcha...</i>\" the familiar voice snickers in your ear.  Ro'gar's head leans over your shoulder and a long, orcish tongue licks your cheek.  \"<i>Was waitin' for this...</i>\" he purrs; the resonance of his voice rumbles against your ear.  Ro'gar chuckles and grips around your wrists.  \"<i>Now that yer here, it's time ta break in this here new bed with sumthin' I've been cravin' since I washed ya down in that swamp, boy.</i>\"  Ro'gar's tone suddenly takes a demanding, hissing tone.  His tongue flicks over your ear as he speaks.  Your skin begins to grow goosebumps as shivers crawl down your spine.\n\n", false);
	Render.text("He yanks your wrists behind your back, ignoring your faint gasps.  With a rustle of cloth, your wrists are bound together behind your back tightly; your fingers feel over the bonds but you struggle against them in vain.  Looking over your shoulder in ", false); 
	if(player.stats.lib < 33) Render.text("feigned confusion ", false);
	else if (player.stats.lib > 66) Render.text("clear lust ", false);
	Render.text("at Ro'gar, you only see his lust-ridden face dipping down to kiss at your lips.  His tusks press against your cheeks with a demanding snarl and his tongue into your mouth; your " + buttDescript() + " receives a smack, forcing you to yelp.  His tongue quickly invades, and his free hand unfastens your " + player.armorName + " to strip you forcefully.  Rough, calloused fingers dance over your body and fasten onto your " + nippleDescript(0) + "s, coaxing moans from your throat with each pinch.  Your pleased noises are muffled by Ro'gar's mouth, his tongue and lips hushing each whimper and gasp you breathe.  A bare, firm chest presses against your exposed back, and you pant as the heat from his body burns against you, only intensified by the thin layer of oil on his skin.  Ro'gar's blue eyes watch you like a predator as your face flushes.  He breaks the kiss and cups his hands under your chin to hold your face in place.  \"<i>Such a perdy face yer makin', " + player.short + ".  Yer lookin' at me like a proper slut, an' it's makin' me hard seein' ya so needy to be bred.</i>\"  He growls in his throat as his mouth nips over your neck, leaving red marks along your skin where his tusks rub against you.  You can feel the oil and sweat smearing over your back, and your senses go wild.  The room is soon filled with your needy whimpers as " + sMultiCockDesc() + " oozes and throbs against the sheets, making the fabric damp with your lust.\n\n", false);
	...
	Render.text(" with some resistance, but the relentless finger rubs over your walls as he begins to pull your rim open, forcing another finger inside and making you bite your lip from the stinging sensation.  Working the pair of fingers together inside of you, he slowly moves them apart and stretches your hole wide.", false);
	else if (player.ass.analLooseness < 5) 
	Render.text(" and rubs over your walls as he pulls your rim open, forcing another finger inside and making you moan like a bitch in heat.  Working the pair of fingers together inside of you, he slowly moves them apart and stretches your hole wide.", false);
		
	Render.text("  Fingers and " + player.feet() + " curl in pleasure as your boycunt is forced open by Ro'gar's steady coaxing.", false);
	...
	if (player.ass.analLooseness < 3) Render.text("the tight hole.  Your teeth clench as he stretches you wide around his length, though he continues without hesitation, and finally you cry out with the sting of the bulbous head squeezing into your tiny hole, though it falls on deaf ears as Ro'gar merely groans at your tightness.", false);
	else if (player.ass.analLooseness < 5) Render.text("your well-used hole. You moan with need as he stretches you wide around his length, continuing without hesitation until he's inside.", false);
	player.buttChange(25, true, true, false);
...
private function rubdubWithDaOrcWithoutDucky():void {
	...
	Render.text("", true);
	spriteSelect(86);
	Render.text("By the time you reach Ro'gar he has already reached his bedroom, his shorts on the floor. You eye over his olive green skin, all of it laid bare before you including his limp orc cock hanging over his hefty nutsack.\n\n", false);
	Render.text("\"<i>I purchased these oils to relax my muscles after spending so much time in the gym.</i>\" Your orc lover motions to a few bottle of oils on his night stand. \"<i>How about you give me a hand... or two.</i>\" Ro'gar grins at you, his eyebrows twitching suggestively. You need no further provocation as you strip from your " + player.armorName + ", you let it fall into a pile on the floor as you make for the bottle. Ro'gar grins and rolls onto his stomach.\n\n", false);
	Render.text("The bottles pop open easily and you pour a generous amount over your hand, you spill even more over Ro'gar back. He gasps feeling the cool liquid crawl down the spine of his back. Though it's not long after you warm his skin with your hands. Your fingers spread and smear the oils over his damp skin. You crawl up and straddle Ro'gar's hips, sitting on his muscled ass as you begin to rub the oils in.\n\n", false);
	Render.text("Ro'gar's skin under your fingertips feels stiff, his muscles resisting your insistent touches. Slowly you can feel them relax, they smooth out and grow warm under your palms. It's not long until Ro'gar is purring loud enough for you to hear. With a pleased growl Ro'gar shifts under your body briefly before letting you continue.\n\n", false);
        Render.text("\"<i>That's the spot.</i>\" Ro'gar grunts as you work your thumbs over a certain spot near his shoulders. You curve your fingers around his broad shoulder and gently pull the muscles to stretch away from the neck. Ro'gar's mouth bellows a moan as his head falls to the sheets. The oils mixing with the strange liquid secreted from the orc's skin making a curious concoction. You can feel your hands feeling so warm and soothed, the muscles in your hands relaxed and comforted. As you look along Ro'gar glistening back you wonder how it would feel to have such a large area affected by the slurry.\n\n", false);
	Render.text("You ask Ro'gar to flip over as you get off of his hips to get more oils. By the time you turn around your orc lover has already laid out on his back. Your eyes take in the sight of your muscled lover laid out for you, his muscled limbs bulging in anticipation, his swollen cock growing larger before your eyes. He grins at you as you pour oils over his chest, the cool liquid making him shiver as it drips down along his center. The clear oil crawling in between his pectorals, down his treasure trail and dipping into his navel before filling the gaps in his abdominals.\n\n", false); 
	Render.text("Without any hesitation you straddle the orc's hips again, his engorged orc cock brushing against the bare skin of your " + buttDescript () + ". Your hands get to work smearing the relaxing oil over Ro'gar's body. The orc stares at you with a lurid gaze as you massage his bulging pectorals, teasing his darkened nipples with your fingers. You grin feeling Ro'gar's thick cock pressing more against your " + buttDescript () + " as his chest rumbles under your hands.\n\n", false);
        Render.text("\"<i>You're good at this " + player.short + ", have you done this before?</i>\" Ro'gar purrs as you work over his abdominals, tracing over every gap with your fingertips, the oils glistening over your lover's body. Ro'gar's hips buck gently, bumping your " + buttDescript () + " with his throbbing cock with a chuckle, his mouth curls into a coy smile. You watch as Ro'gar reaches his arms back, resting his head on his hands as he bares his underarm and pits.\n\n", false);
