...
public encounterRogarSwamp():void {
	...
	if(flags[FlagEnum.ROGAR_PHASE] == 0) {
		...
		//((Corruption 70 or more))
			...
			if(player.isNaga() || player.tailType > TAIL.NONE) MainScreen.text("  Your tail flicks about happily.", false);
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2 && player.stats.lib >= 50) {
			...
			if(player.stats.int <= 30) {
				...
				if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  When you witnessed him make that other beast submit with strength alone it created a demanding need in your " + vaginaDescript() + " during the match, and the memory is doing much the same now.", false);
				//((if player has cock(s))
				if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("  You feel " + sMultiCockDesc() + " shift as you remember how hard you got from behind the tree, finding yourself absent mindedly stroking yourself with an open palm as pre-cum soaks through your clothing.", false);
	...
	//Not sure what determines this yet
		...
		//((androgynous or masculine and breasts <= A-cup))
			...
			//(Dirt Mc Girt = 2)
			if(flags[FlagEnum.ROGAR_DIRT] == 2) MainScreen.text("\"<i>" + player.short + ", y'all took off on me real quick like,</i>\" he says through a snicker.  The joke falls flat and the orc seems to blush lime green as you stare at him uncomprehendingly.  He rubs the back of his neck with a hand before continuing.  \"<i>Sorry about that.  I was plumb tuckered out afterwards.</i>\"\n\n", false);
	...
	if(flags[FlagEnum.ROGAR_PHASE] == 0) {
		...
		//((If libido is 50 or more))
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) {
				MainScreen.text("  Your " + vaginaDescript(0) + " starts to quiver as the warm water rolls over its lips, moistening in excitement as your skin is rubbed over by strong hands.  Your chest heaves as it relaxes to his touch, your " + nippleDescript(0) + "s becoming stiff under his rubbing.", false);
				MainScreen.text("  Even your " + player.feet() + " and fingers curl as you smell the sweat and musk of Ro'gar's closeness, the animalistic scent arousing you.", false);
			}
			else if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("  " + SMultiCockDesc() + " stirs to life, making it obvious how much you are enjoying the rub down, then begins to drool with lust.  Through lidded eyes you faintly make out Ro'gar's toothy smirk becoming a hungry grin.", false);
...
private okayBroLetsHaveAGayCarwash():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text(sMultiCockDesc(), false);
	else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("your " + vaginaDescript(0), false);
	else MainScreen.text("your " + assholeDescript(), false);
	...
	//((androgynous or masculine and breasts <= A-cup))
		...
		//(else if Dirt Mc Girt = 2)
			...
			if(player.gender == 2 || player.gender == 0) {
				...
				MainScreen.text("  " + SMultiCockDesc() + " gets a caress from him as he explores, arousing it to mild stiffness.  \"<i>" + player.short + ",</i>\" he grunts.  \"<i>Ah hope ah'll be seein' more of ya.</i>\"  Ro'gar breaks the contact and tips back the remains of his drink while you ", false);
				...
				if(player.hasLongTail() > 0 && player.isNaga()) MainScreen.text(", and you feel your tail flicking rapidly in excitement", false);
				MainScreen.text(".\n\n", false);
...
public rogarPhaseFour():void {
	...
	//(andro or masculine with tits <= A)
		...
		if(player.gender == 2 || player.gender == 0) {
			...
			MainScreen.text("  " + SMultiCockDesc() + " trembles under his strong touch, as he increases the pressure and looks into your eyes meaningfully.  ", false);
			//[(if Dirt Mc Gurt flag = 1)
			if(flags[FlagEnum.ROGAR_DIRT] == 1) MainScreen.text("</i>\"I know ya probably ain't ever imagined hangin' out with a feller like me, but... I could really use the company tonight.</i>\"  ", false);
...
private loseButtGinity():void {
	...
	if(player.ass.analLooseness == 0) {
		...
		if(player.hasLongTail() || player.isNaga()) MainScreen.text("  Your tail wraps around Ro'gar's arm for some sort of stability, the tip flicking lazily as it coils and squeezes around a bicep.", false);
		MainScreen.text("\n\n", false);
		...
		MainScreen.text("  Fingers and " + player.feet() + " curl in pleasure as your previously untouched boycunt is forced open by Ro'gar's steady coaxing.", false);
	...
	//(([Get Anal] Scenario 1b: Get fucked (normal)))
		...
		//(else)
			...
			MainScreen.text("  " +  SMultiCockDesc() + " lays out over your chest, dribbling pre-cum on your skin as you make a mess of your torso in lust.", false);
			//[(if player has tail) 
			if(player.hasLongTail() > 0 || player.isNaga()) MainScreen.text("  Your tail flicks helplessly below you, urgently wrapping around Ro'gar's thigh and trying to pull him deeper into you.", false);
			...
			//[(If player has balls) 
			if(player.lowerBody.balls > 0) MainScreen.text("  You feel your " + ballsDescriptLight() + " pull up tight in your sack as your labored breath comes out in pants.", false);
...
private fuckRogarsButtPussyBoyCuntManMoundSissySlitQueerQuim():void {
	...
	//((If cock is between 18 and 4 inches))
		...
		if(player.lowerBody.cockSpot.list[0].cockType == CockType.CAT) MainScreen.text("  You grind around in his green pucker, the barbs working over Ro'gar's inner walls.  \"<i>W-what in tarnation?</i>\" Ro'gar grunts as you watch him squirm under you.  You buck your hips, digging deeper into orcflesh with each thrust, his muscled ass tensing with each slam of your hips.", false);
		//(If horse cock)
		else if(player.lowerBody.cockSpot.list[0].cockType == CockType.HORSE) MainScreen.text("  The flared head of your " + cockDescript(0) + " nestles in Ro'gar's hole.  His breath quickens as you push in, the girthy, flared head stretching Ro'gar's insides impossibly.  You simply thrust and buck you way deeper within the green pucker to force more of your lengthy shaft past that olive rosebud each time.\n\n", false);
		//(If dog cock)
		...
		//[(if multiple cock)
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("  The other meat rubbing over his olive skin outside adds to the coating of spunk, making a giant mess of Ro'gar's body.  In seconds he resembles a well-paid whore beneath your " + multiCockDescriptLight() + ".", false);
		MainScreen.text("\n\n", false);
	}
	//end right fit path
...
private frotWithRogar():void {
	...
	//(If single cock)
		...
		MainScreen.text("Your cock presses tight against Ro'gar's fat orc cock. He growls low as he grinds himself over you, his thick, globbing pre-cum drooling onto your " + multiCockDescriptLight() + ".  Muffled groans escape Ro'gar's mouth as you kiss him roughly, tongues locked all the while in writhing combat.  ", false);
		//[(If player has balls)
		if(player.lowerBody.balls > 0) MainScreen.text("Your " + ballsDescriptLight() + " slap audibly against Ro'gar's heavy green sack.  ", false);
...
private kinkyWithDaOrc():void {
	...
	MainScreen.text("at Ro'gar, you only see his lust-ridden face dipping down to kiss at your lips.  His tusks press against your cheeks with a demanding snarl and his tongue into your mouth; your " + buttDescript() + " receives a smack, forcing you to yelp.  His tongue quickly invades, and his free hand unfastens your " + player.armorName + " to strip you forcefully.  Rough, calloused fingers dance over your body and fasten onto your " + nippleDescript(0) + "s, coaxing moans from your throat with each pinch.  Your pleased noises are muffled by Ro'gar's mouth, his tongue and lips hushing each whimper and gasp you breathe.  A bare, firm chest presses against your exposed back, and you pant as the heat from his body burns against you, only intensified by the thin layer of oil on his skin.  Ro'gar's blue eyes watch you like a predator as your face flushes.  He breaks the kiss and cups his hands under your chin to hold your face in place.  \"<i>Such a perdy face yer makin', " + player.short + ".  Yer lookin' at me like a proper slut, an' it's makin' me hard seein' ya so needy to be bred.</i>\"  He growls in his throat as his mouth nips over your neck, leaving red marks along your skin where his tusks rub against you.  You can feel the oil and sweat smearing over your back, and your senses go wild.  The room is soon filled with your needy whimpers as " + sMultiCockDesc() + " oozes and throbs against the sheets, making the fabric damp with your lust.\n\n", false);
	...
	MainScreen.text("  Fingers and " + player.feet() + " curl in pleasure as your boycunt is forced open by Ro'gar's steady coaxing.", false);
	...
	if (player.ass.analLooseness < 3) MainScreen.text("the tight hole.  Your teeth clench as he stretches you wide around his length, though he continues without hesitation, and finally you cry out with the sting of the bulbous head squeezing into your tiny hole, though it falls on deaf ears as Ro'gar merely groans at your tightness.", false);
	else if (player.ass.analLooseness < 5) MainScreen.text("your well-used hole. You moan with need as he stretches you wide around his length, continuing without hesitation until he's inside.", false);
	player.buttChange(25, true, true, false);
...
private rubdubWithDaOrcWithoutDucky():void {
	...
	MainScreen.text("\"<i>I purchased these oils to relax my muscles after spending so much time in the gym.</i>\" Your orc lover motions to a few bottle of oils on his night stand. \"<i>How about you give me a hand... or two.</i>\" Ro'gar grins at you, his eyebrows twitching suggestively. You need no further provocation as you strip from your " + player.armorName + ", you let it fall into a pile on the floor as you make for the bottle. Ro'gar grins and rolls onto his stomach.\n\n", false);
