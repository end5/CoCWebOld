 ...
 */
export default class CorruptedDriderScene {
	...
	public defeatDriderIntro(): void {
		spriteSelect(77);
		MainScreen.text("", true);
		MainScreen.text("All eight of the drider's black and " + monster.hairColor + " striped legs collapse under her weight while she struggles ", false);
		if (monster.HP < 1) MainScreen.text("to try to continue the fight, her movements slowed by pain and weakness.", false);
		else MainScreen.text("to masturbate her swollen pussy-lips and the thick, juicy demon-cock that's sprouted in place of her clit.  She's so focused on getting off that she utterly gives up on the idea of fighting back, let alone dominating you as she seemed to plan originally.", false);
		MainScreen.text("  The spider-herm lies in a miserable, defeated heap.", false);
		...
		if (player.lowerBody.cockSpot.hasCock() && player.stats.lust >= 33) {
			if (player.cockThatFits(monster.vaginalCapacity()) >= 0) {
				...
				if (player.lowerBody.tailType == TailType.SPIDER_ABDOMEN || player.inventory.items.has(useables.T_SSILK)) {
	...
	public loseToDrider(): void {
		...
		let select: Array = [];
		if (player.lowerBody.cockSpot.hasCock()) select[select.length] = 0;
		if (player.lowerBody.vaginaSpot.hasVagina()) select[select.length] = 1;
		//Genderless scene has low chance for gendered folks
		if (player.gender == 0 || (player.gender > 0 && rand(3) == 0)) select[select.length] = 2;

		//SELECT SCENE HERE
		let choice: number = select[rand(select.length)];
	...
	private winDriderPCDickInSpiderCunt(): void {
		...
		MainScreen.text("Eager to plumb the depths of this greedy spider's snatch, you practically tear your way out of your " + player.inventory.armor.displayName + " in your hurry to get naked.  Through it all ", false);
		if (monster.lust > 99) {
			MainScreen.text("she looks ", false);
			if (player.tallness > 72) MainScreen.text("up ", false);
			MainScreen.text("at you with her " + monster.skinTone + " eyes opened wide and her tongue hanging from her mouth, panting lewdly.  Carapace-clad digits sink into her dripping honey-pot over and over while she watches, deep in her own lusty haze.  She retracts her engorged demon-cock so as to have better access to that feminine hole.", false);
		}
		else {
			MainScreen.text("she looks ", false);
			if (player.tallness > 72) MainScreen.text("up ", false);
			MainScreen.text("at you with a look of disgust in her " + monster.skinTone + " eyes.  She folds her arms across her many-bosomed, quadratically endowed chest with disdain", false);
			if (monster.nipplesPierced > 0) MainScreen.text(", the chains jingling loudly as she does so", false);
			MainScreen.text(".  In spite of keeping a stiff upper lip about it, her lower ones visibly quiver while you strip.", false);
		}
		MainScreen.text("  You saunter over to the limp-legged drider and run your hands down her inhuman, arachnid body, exploring the smooth, shiny carapace.\n\n", false);

		MainScreen.text("The corrupted monster-girl sighs and shudders at your touches, as if she can somehow feel your finger-tips through the rock-hard exoskeleton.  From the way she's trying to hold in her exhalations, you can only guess that it must feel wonderful, so you keep it up, circling and touching every part of her exotic anatomy.  By the time you've circled around her entire body, she's ", false);
		if (monster.HP < 1) MainScreen.text("giving up little, lewd moans", false);
		else MainScreen.text("breathing out lurid moans", false);
		MainScreen.text(" of pleasure and her ", false);
		if (monster.nipplesPierced > 0) MainScreen.text("chain-pierced-", false);
		MainScreen.text("nipples and clit look as hard as her natural body-armor.  You boldly grope one of her ", false);
		if (monster.nipplesPierced > 0) MainScreen.text("pierced ", false);
		MainScreen.text("breasts, feeling the supple, soft flesh shift while you roughly palm it and rub your thumb down a dark nipple.  You let the sensitive bud go and watch it spring back up, enjoying the drider's gasp of surprised pleasure for the brief moment before she muffles herself.\n\n", false);

		MainScreen.text("Still fondling one of the four nicely-sized tits, you push the spider-bitch back, admiring the drider's flexibility as she leans back over her inhuman lower body, giving you an even better view of her intoxicatingly sweet-smelling pussy.  You take " + oMultiCockDesc() + " in hand and rub the " + player.cockHead(x) + " along the slick lips of her pussy, shuddering slightly as her wetness rubs the sensitive glans.  Gasping, you slowly spread her molten-hot pussy around your girth, reveling in the tight embrace of her slick walls.  The drider squirms in your grip with her breasts bouncing deliciously and whole body trembling.  Spreading as easily as her cunt, the drider's venom-glossed lips slowly open into a wordless 'o' of rapturous pleasure.\n\n", false);

		MainScreen.text("Half an inch at a time, you sink ever-deeper inside the seemingly bottomless pit that is the spider-girl's steamy sex.  Reveling in the taming of the aggressive predator's sticky snatch, you postpone the eventual hilting as long as possible.  For her part, she doesn't seem to mind you having your way with her.  All eight of her spindly legs are scrabbling pitifully in the dirt, doing little more than nervelessly convulsing in uncontrolled pleasure.  The spider-woman's arms close behind your back, the sharp tips of her nails trying to claw deep furrows into your " + player.skinFurScales() + ".  At long last, your waist presses against her own to feel the oozing slipperiness of her nethers.  Your " + CockDescriptor.describeCock(player, x) + " is totally buried in her tunnel, and now that you're holding still, you can feel the tiny folds inside her vagina slowly moving and gliding along your length, stroking your " + CockDescriptor.describeCock(player, x) + " like dozens of tongues.\n\n", false);

		MainScreen.text("Compelled by your body's instincts, you draw back your " + LowerBodyDescriptor.describeHips(player) + " to expose the bulk of your pussy-soaked shaft.  The spider-woman moans disappointedly, her voice filled with so much wanton desire that your " + player.cockHead(x) + " twitches in response.  You fondle the drider's lower pair of breasts with both hands, circling and squeezing the delectable mounds while you begin to repeatedly impale her squelching pussy, watching the upper row of pale breasts bounce and jiggle above your busy fingers.", false);
		if (monster.nipplesPierced > 0) MainScreen.text("  The chains binding her nipples make a delightful clinking noise with each squeeze and stroke.", false);
		MainScreen.text("\n\n", false);

		MainScreen.text("Meanwhile, the drider's swollen clit prods at your loins with each penetrating thrust, leaving lines of sticky spider-juice all over your crotch.  ", false);
		if (player.lowerBody.cockSpot.count() > 1) {
			MainScreen.text("Your extra penis", false);
			if (player.lowerBody.cockSpot.count() > 2) MainScreen.text("es fan out to the side, forgotten while you give yourself over to the silken caresses of her glorious vagina.", false);
			else MainScreen.text(" sticks out to the side, forgotten while you give yourself over to the silken caresses of her glorious vagina.", false);
		}
		else MainScreen.text("Your " + CockDescriptor.describeCock(player, x) + " seems to bulge inside her, giving itself over to the silken caresses of her glorious vagina.", false);
		MainScreen.text("  The more you fuck the arachnid hussy's wanton slit, the more animated she becomes, until she finally breaks her wordless silence by crying, \"<i>Yes " + player.mf("master", "mistress") + ", dominate me!  I've been such a naughty bitch.  Use me like your cheap whore!  Fuck my holes full of cum, and leave me defiled in a puddle of spunk-laced mud!</i>\"\n\n", false);

		MainScreen.text("Invigorated by the rising tide of desire inside her, the drider pulls her torso up, crushing her tits into your palms so hard that her flesh bulges out through your fingers.  You smile down at the slutty spider and squeeze them hard enough to give her a taste of pain in response.  She pants, and her four " + monster.skinTone + "-hued eyes gaze at you, so full of desire that you have a hard time not cumming on the spot while you lock onto those half-lidded, luminous orbs.  You let go of her breasts and reach back to squeeze the smooth skin of her butt, feeling the joint between her human and arachnid halves just under her curvy derriere.\n\n", false);

		MainScreen.text("Heaving mightily, you use the submissive spider's ass-cheeks as hand-holds for a fast, hard fuck.  Her four ", false);
		if (monster.nipplesPierced > 0) MainScreen.text("pierced", false);
		else MainScreen.text("white-skinned", false);
		MainScreen.text(" breasts mash against your " + BreastDescriptor.describeAllBreasts(player) + ", sweaty skin sliding back and forth and drawing gasps from each of you every time one of her nipples brushes one of yours.  She coos, \"<i>Please fuck my slutty drider-cunt harder, " + player.mf("master", "mistress") + "!  It was so naughty and cruel when I tried to rape you.  Don't let me cum... deny me over and over until I'm your good little spider-pet that'll do anything for release!</i>\"  She emphasizes every sentence by clamping her quim down on your " + CockDescriptor.describeCock(player, x) + ", the dozens of lubricant-soaked folds rolling back and forth, milking you until you cannot help but ejaculate inside her.\n\n", false);

		MainScreen.text("Your back arches in orgasmic pleasure, fingers squeezing her butt tightly enough that your pale lover's derriere will show your hand-prints for some time.  She shudders slightly as your " + CockDescriptor.describeCock(player, x) + " is forced as far inside her lewdly-textured vagina as possible, milking your penis with repeated strokes while your urethra bulges with the first load.  You release what feels like a torrent into her passage; the heavy spurt is quickly swallowed by the drider, disappearing deep inside her as if drawn away on an unseen current.  The sudden suction and increase in the movement of her folds makes your next rope of jizz feel even bigger, but it too disappears into the drider's seemingly-bottomless body.", false);
		if (player.cumQ() > 1000) MainScreen.text("  Amazingly, you actually manage to cum enough to fill her massive womb, and after the first few squirts, the cum slowly bubbles out around your shaft with each fresh deposit of jism.", false);
		MainScreen.text("\n\n", false);

		MainScreen.text("While you're distracted by the repeated contrast of tension and relief emanating from " + CockDescriptor.describeMultiCockSimpleOne(player) + ", the drider leans up and kisses your neck, humming against your throat as you squirt the last of your orgasmic goop into her vagina.  You slump into her arms, spent by the effort of fucking her so fast and cumming so hard.  As you lie there, her lips sucking hard enough to leave hickeys on your neck with every crushing, passionate kiss, you feel the patterns of glossy fluid tingling on your " + player.skin() + ".  A jolt of fear runs up your spine, and you try to push away from her envenomed affections.  It doesn't work!  You can barely lift your arms, let alone pull out of the amorous arachnid's embrace!\n\n", false);

		MainScreen.text("\"<i>Mmmmm,</i>\" hums the drider into your collarbone, smearing more of her toxic lip-gloss onto you as she goes.  \"<i>Such a good " + player.mf("master", "mistress") + "!  You punished me so well!  Now just relax... I've been dripping out a special venom that'll help you teach me the rest of my lesson, and I need to make sure that every... single... drop... slips inside that sexy bloodstream of yours.</i>\"\n\n", false);

		MainScreen.text("You wiggle futilely, using the last of your strength to try to break free, but it's no use - she's too strong, or the venom has made you too weak.  Dimly, you're aware of your " + CockDescriptor.describeCock(player, x) + " pulsating inside her, hard as iron and burning hot with lust, even though you just came.  The spider-woman continues to kiss you, and with every peck of her lips, you feel yourself sagging deeper into her embrace until you lay your head down on her breast, your only movements the slight twitches of your body as it tries to deal with the poison-induced need to copulate.  You feel the warmth of her breath on your neck when she stops, cradling you in her arms.\n\n", false);

		MainScreen.text("The drider cocks her head to the side so you can see her luminous " + monster.skinTone + " eyes boring into you while she whispers, \"<i>Don't try to fight it, dear.  While there's enough of my normal venom in there to keep your " + CockDescriptor.describeCock(player, x) + " hard enough to cum a few more times for me, the real magic is in the secondary chemicals my demonic friends were kind enough to teach me how to excrete.  You can feel it relaxing your muscles can't you?  Well, the beauty is that it isn't just your muscles relaxing... your mind is getting nice and relaxed too.  So open and pliable that you'll soon be a horny puddle of putty for me to play with.</i>\"\n\n", false);

		MainScreen.text("Gods, she's right!  You started to nod while she was talking, automatically agreeing with every sibilant syllable.  Trying to shake your head in denial, you only succeed at bouncing the jiggling tit-flesh beneath you.  You have to resist!  You beat her!  She can't do this!\n\n", false);

		MainScreen.text("As if reading your thoughts, the seductive spider answers, \"<i>Oh dear, don't struggle.  You won, remember?  You held me down and fucked me hard.  You mounted me like a fuck-hungry stud that does all his thinking with the big fat cock between his legs, and we both loved it.  You won't deny that will you?  It's the truth.</i>\"\n\n", false);

		if (player.stats.cor > 80 && player.stats.lib > 50) MainScreen.text("Of course it's the truth, the cunt's clearly got a good grasp on the obvious. Unwilling to lose your grip on dominance, however, you try to keep struggling to get the upper hand.\n\n", false);

		else MainScreen.text("No, that's not right.  You're not some rutting beast, you're the champion.  Sure, you've got your " + CockDescriptor.describeCock(player, x) + " locked hilt-deep inside her squirming slit, but that doesn't mean you're some kind of " + player.mf("stud", "slut") + " who jumps on a cunt without a second thought, does it?  You blink your eyes while you try to think.  She does have a point though, if you hadn't been in such a hurry for some sex, you wouldn't be sagging into her tits and feeling so awfully relaxed.\n\n", false);

		MainScreen.text("The drider croons, \"<i>Come on, don't delude yourself.  If you weren't such a dominating, rape-hungry beast you wouldn't be nuzzling against my tits and giving me little nods of agreement.  Honestly, how could you ever think of yourself as anything different when your brain just does whatever your dick tells it to?  You just got off and might have even knocked me up, but your penis is even harder than before.</i>\"\n\n", false);

		if (player.stats.cor > 80 && player.stats.lib > 50) MainScreen.text("You grumble out a denial.  You're not dumb, you just like to take charge and fuck. There's no reason to resist jumping on a nice wet pussy and making it squirt while you pack it full... but you can't quite get your limbs to move.\n\n", false);

		else MainScreen.text("You mumble out a denial.  You're not a beast... you just like to take charge and fuck.  It's just so hard to resist jumping on a nice wet pussy and making it squirt while you pack it full... no!  That's not what...\n\n", false);

		MainScreen.text("\"<i>Oooh, are you leaking out more pre-cum already?  You're insatiable!  I bet you start thinking about your next fuck before you even finish getting off, don't you?  You can deny it all you want, but I felt your dick puff up a little bit more as soon as I said it.  No matter what you think, your cock is a dumb, happy little tattle-tale that always tells the girls exactly what you're thinking.</i>\"", false);
		if (player.statusAffects.get("Exgartuan").value1 == 1) MainScreen.text("  Exgartuan mumbles telepathicaly, \"<i>Fuck yes I do, but I wanna see how this plays out.</i>\"", false);
		MainScreen.text("  She adds, \"<i>Face the music - there's nothing you love more than defeating someone and letting your dick tell you what to do.  You dumb... domineering... beast...</i>\"\n\n", false);

		MainScreen.text("Gods!  It's getting so hard to think with that tight cunt clutching at your cock and the soft, feminine enunciations constantly interrupting your thoughts.  How can she concentrate with the stink of sex hanging so thick in the air?  Your " + CockDescriptor.describeCock(player, x) + " is so horny, why aren't you fucking her yet?  You shudder weakly and remember ", false);
		if (player.stats.cor > 80 && player.stats.lib > 50) MainScreen.text("that it's because of her own venom paralyzing your muscles so effectively.\n\n", false);
		else MainScreen.text("there was something wrong about what she was saying... something important.\n\n", false);

		MainScreen.text("The drider pauses to gaze into your eyes with all four of hers.  She runs a hand through your hair while making gentle shushing noises at you before she resumes her diatribe.  \"<i>There there, I know how eager you are to fuck me into my place.  I'm just another wanton harlot waiting to be forced on her back and turned into a fresh cock-sleeve for you, aren't I?  You recognize the absolute truth of my words.  Every time I talk you can feel the words bouncing around inside that empty head of yours.</i>\"\n\n", false);

		MainScreen.text("You nod, actually moving your head against her pillowy breasts as you feel your strength starting to return.  Why is she wasting so much time telling you things you already know?  This bitch's cunt is like a white-hot furnace of lust, and your cock is so tired of having to sit still.  You can't wait to recover from the venom and fuck her so hard she'll black out from the ensuing orgasm.  Then you'll fuck her a few more times until your cock finally gets tired of her sloppy, cream-pied pussy.\n\n", false);

		MainScreen.text("\"<i>Oooh, that dick is just getting harder and harder isn't it?  I bet it feels like it's about to burst and cum, even though you're not able to punish me like you want to yet.  Maybe you secretly get turned on by being denied?  Oh!  I just felt your dick jump!  So you want to go around, letting your dick tell you to dominate every horny cunt you see, but the thing that turns you on more than anything else is having a woman turn the tables on you, isn't it?</i>\"\n\n", false);

		MainScreen.text("You moan, \"<i>N-no...</i>\" but a small burst of pre-cum chooses that moment to erupt from your " + CockDescriptor.describeCock(player, x) + ", still nestled inside the squirming spider-cunt.  It feels heavenly, and while you try to struggle in your weakened state, the warm embrace of her pussy's many cock-milking folds is turning your penis into a cum-drooling behemoth on the edge of exploding.  Oh gods, is she going to do it?  Damn, you want to put her in her place, but you're so weak right now.  Even worse, her pussy has you so close, so very close.\n\n", false);

		MainScreen.text("The chitinous, oh-so-right bitch pulls your head up to gaze deeply into her eyes.  \"<i>You have a horny, naughty cock, and you always listen to it.  This rebellious member gets you into so much trouble, but you'll never stop listening to it.  You'll keep chasing every wet pussy you see, because you're just a horny animal.  A rutting, horny animal.  But I know your secret.  Your dick really wants a girl to turn the tables on you, paralyze you, and tease your cock mercilessly.  It feels so good to let me touch your body, stroke it, tease it, and squeeze it.  You won't even fight any more.  My pussy has your cock so close, so very close, and you're going to cum when I tell you.  You'll blow your load on command for me, my servile, secretly submissive mount.</i>\"\n\n", false);

		MainScreen.text("A plaintive whining, almost exactly like that of an animal, vocalizes subconsciously.  Your " + CockDescriptor.describeCock(player, x) + " feels afire, constantly leaking pre-cum while you're held immobile in your spidery mistress' arms; you lick your lips and start to beg, mumbling out your desire to cum for her, anything for her, so long as she lets your dick orgasm again and again.  You want to climax so bad, but you want her to control your cock so much more.  It's always getting you in trouble, and it feels so good to let her use you as her cum-drooling mount.\n\n", false);

		MainScreen.text("The spider-woman giggles and surprises you by kissing you on the lips, holding you against her while her narrow tongue smears her venom all over your lips and tongue, staining them purple.  You exhale happily through your nose, feeling perfectly, utterly relaxed and yet aroused beyond any reasonable measure.  The squeezing caresses around your dick keep you happily squirting heavy dollops of pre-cum.  You've no doubt it must look like some kind of over-stuffed sausage by this point, bloated beyond reason.\n\n", false);

		MainScreen.text("When she pulls away, her face is covered in a huge blush, and she's panting out loud and interrupting her words with quiet moans as she talks, \"<i>That's a good " + player.mf("boy", "girl") + ".  Even though you're a stupid pussy-slave, bound to let your cock lead you into every hungry snatch you see, you know how to obey a smarter woman's cunt, don't you?  Let me hear you beg like the animal you are.  Whine for mistress while her cunt fucks your dick, show me with every fiber of your being how much you love to submit, to obey, and to beg to be allowed to cum.</i>\"\n\n", false);

		MainScreen.text("She lets your head hang limply, and all you can do is whine, and try to look up at her with the most pathetic expression in your eyes.  You're beaten - she's so much smarter than you and she's got you by your dick.  Why didn't you think?  ", false);

		if (player.stats.cor > 80 && player.stats.lib > 50) MainScreen.text("Gods it was a bad idea not to bind her lips... ", false);
		else MainScreen.text("Gods this was a bad idea... ", false);
		MainScreen.text("still, at least she's controlling your rebellious dick, teaching the little prick a lesson while letting you feel so good.\n\n", false);

		MainScreen.text("The drider moans louder and responds, \"<i>Yes, oooh, good " + player.mf("boy", "girl") + ".  You're so hard, so very hard.  You're oozing out cum even though you're not allowed to fuck me, and you love it.  I bet you don't even mind not getting to orgasm - so long as you're helpless and hilt-deep in a cunt, you're happy, aren't you?  Oohhh, I'm gonna... gonna... cum soon.  Luckily for you, I want you to squirt inside me.  I want that cock creaming my pussy as soon as I... ohhhh... so good... s-soon as I command it...  You'll do that for me, won't you?  You'll obey me completely, utterly, and pump every drop of your jism into my soon-to-be-pregnant pussy, right?</i>\"\n\n", false);

		MainScreen.text("You mewl out an affirmation, forgetting how to speak with all the venom pumping through you and the mind-crushing levels of arousal pouring out of your " + CockDescriptor.describeCock(player, x) + ".\n\n", false);

		MainScreen.text("The spider-woman shudders from head to all eight of her 'toes', her four eyelids closing while her mouth gasps out, \"<i>Cum for me.</i>\"\n\n", false);

		MainScreen.text("The first spurt of pent-up seed splatters the spider's womb before you even realize she gave you permission, painting the squirming walls white.  You growl and cum, releasing spurt after spurt of spunk while her pussy seems to drink it all down", false);
		if (player.cumQ() > 1000) MainScreen.text(", even though you've packed it full once already", false);
		MainScreen.text(".  Her legs skitter and tremble, her pussy convulses, and your dick gets the squeezing, sucking orgasm of a lifetime.  It feels like you're pumping out two or three orgasms worth of spooge, each dick-stretching explosion of cream dilating your slit almost painfully wide.", false);
		if (player.cumQ() > 1000) {
			MainScreen.text("  The stuff dribbles out around your tool along with a mix of girl-lube, too much even for her prodigious pussy to contain.", false);
			if (player.cumQ() > 2000) MainScreen.text("  The flow thickens into a river, and soon the puddle of cum looks more like a pond, turning the dirt into a soup of muddy cum.", false);
		}
		MainScreen.text("\n\n", false);

		MainScreen.text("While it seems to go on and on, your orgasm eventually ends, leaving you to hang limp while your still-hard dick is massaged by a much wetter spider-cunt.  The " + monster.hairColor + "-haired beauty dreamily says, \"<i>Mmmm... good job, pet.  Now, let that nice hard dick do the thinking for you and keep fucking my hot cunt until it goes dry.  Maybe a few hours after you finish, you'll be able to remember that you used to be a " + player.mf("man", "woman") + " before you became my livestock.</i>\"\n\n", false);

		MainScreen.text("The blissful arachnid rolls onto her side with you still locked inside her and falls asleep, her " + monster.hairColor + " tresses ", false);
		if (player.cumQ() <= 1000) MainScreen.text("splayed across the ground", false);
		else MainScreen.text("soaking in the white puddle of spooge", false);
		MainScreen.text(".  Even unconscious, her vagina continues its rhythmic contractions around your cock, working you towards another burst of orgasmic pleasure while you wait for your strength to come back...\n\n", false);
		//[NEXT]
	...
	private winDriderPCDickInSpiderCunt2(): void {
		...
		spriteSelect(77);
		MainScreen.text("", true);
		MainScreen.text("<b>Sometime Later...</b>\n", false);

		MainScreen.text("Pistoning your hips violently, you continue to ravage the unconscious spider, fucking her in a brutal, beast-like manner that centers entirely upon your own pleasure.  You cum, dumping a fresh batch of jizz into her already-stuffed twat, grinning when a few more rivulets of ooze roll over her inflamed pussy-lips to drip into the growing puddle.  Still hard, " + CockDescriptor.describeMultiCockSimpleOne(player) + " continues to twitch, hungry for more, and you don't deny it.  You grab the arachnid bitch and stuff your dick into her mouth, shivering while her venom seeps into your dick, keeping you hard and ready to blow.  Whatever special, immobilizing venom she had earlier, it seems exhausted, and the only response your body has to the onslaught of toxin is to erupt, spouting a huge load of semen into her mouth.\n\n", false);

		MainScreen.text("A low growl rips through your throat as you pull back and plaster her face white, coating the caked-on residue of a few previous loads with fresh moisture.  You shift position and force your " + CockDescriptor.describeCock(player, x) + " between her two pairs of breasts, sandwiching it in there for the titty-fuck of a lifetime.  Pistoning back and forth, humping the glorious orbs relentlessly, you give yourself over completely to your sore dick, letting your world dissolve into the feelings of each of the spider-woman's orifices around your member.\n\n", false);

		MainScreen.text("<b>Even later...</b>\n", false);

		MainScreen.text("You collapse into the dirt, not even caring that it stinks of dried cum and girl-jizz.  You've completely emptied out every speck of sperm from your " + BallsDescriptor.describeBalls(true, true, player) + ", and you even kept going for a while after that.  It wasn't until " + CockDescriptor.describeMultiCockSimpleOne(player) + " went soft at last that you found enough peace to lie down.  ", false);
		if (player.stats.cor > 80 && player.stats.lib > 50) MainScreen.text("The longer you sit there, the more your strength returns to you, and the more the submissiveness placed into you by her venom recedes. Still, you've tired yourself out too much to leave at the moment.", false);
		else MainScreen.text("The longer you sit there, the more you remember of yourself, and the more you forget the animal, the rut-beast that the spider-woman placed inside you.", false);
		MainScreen.text("  You shiver and wrap your arms around her, snuggling against her while a primal, buried part of your brain insists that she's YOURS.  You fall asleep like that, resting dreamlessly even through your mate's waking and departure.", false);
		hideUpDown();
		monster.lust = 98;
		monster.HP = 2;
	...
	private victoryVSDriderRideOviVaginal(): void {
		...
		player.slimeFeed();
		MainScreen.text("Eyeing the small lump of the spider's retracted ovipositor, you resolve to have a little fun with it, even if it means giving the spider exactly what she wants in a way that she doesn't.  You tear off your " + player.inventory.armor.displayName + " while the drider watches with ", false);
		if (monster.HP < 1) MainScreen.text("hate", false);
		else MainScreen.text("lust", false);
		MainScreen.text(" pouring from all four of her eyes.  Now nude, you saunter over with a gleam in your eyes and a telltale tingling in your loins.  You move around some of the eight, sprawled-out legs and close in on her side.  With a mighty heave, you help to lift her up onto her shaking legs, then keep pushing, tipping her dangerously to the side.  She cries out in surprise as she flips over, rolling onto her back while her spindly, chitinous legs flail uselessly at the muggy air.\n\n", false);
	...
	private victoryVSDriderRideOviAnal(): void {
		...
		MainScreen.text("The spider's look of ", false);
		if (monster.HP < 1) {
			MainScreen.text("hate fades slightly when she sees you eyeing her ovipositor, and with a sigh of annoyance, allows the segmented organ to slowly slide out from her body.  It grows to just over two feet in length, swaying sinuously in the air, though you can tell it isn't completely engorged yet.  Still, your " + ButtDescriptor.describeButthole(player), false);
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" and " + CockDescriptor.describeMultiCockSimpleOne(player) + " twitch", false);
			else MainScreen.text(" twitches", false);
			MainScreen.text(" at the thought of trying to pack all of that meaty, alien pole inside you.\n\n", false);
		}
		else {
			MainScreen.text("lust only deepens when she sees you eyeing her ovipositor, and with a twinkle in her eyes, she releases a hiss of pleasure.  The organ pulsates once, then practically explodes before your eyes, lancing just over two feet into the air and waving back and forth sinuously, like a snake.  In a way, the tubular, segmented shaft reminds you of a monstrously thick cock, something that might even give a minotaur pause. Your " + ButtDescriptor.describeButthole(player), false);
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" and  " + CockDescriptor.describeMultiCockSimpleOne(player) + " twitch", false);
			else MainScreen.text(" twitches", false);
			MainScreen.text(" in anticipation - you can't wait to feel that thing going inside you!\n\n", false);
		}

		MainScreen.text("You lean over and grab it in your hands, letting it pulse and squirm in response to your touches.  The copious slime it's covered in lets your fingers easily slide down its length.  Each stroke of the odd, feminine member elicits a pleasured groan from your defeated opponent, and you quickly surmise that she must like this just as much as you do using your own genitals.  Pumping and stroking it, you watch with perverse fascination as the ridges grow firmer, hardening in your hand.  A viscous green slime bubbles out of the wide slot at its tip as the spider-girl's moans reach a crescendo.  It's as lubricated as it's going to get.\n\n", false);

		MainScreen.text("Climbing over the drider's bulbous body, you bend over to present your " + ButtDescriptor.describeButt(player) + " to the twitching ovipositor, beckoning her to slide inside your twitching " + ButtDescriptor.describeButthole(player) + " as you stand atop its owner.  The sprawled-out spider looks up past all of her body to watch you, leaving you waiting.  Nothing happens at first.  You're just there, ass in the air and waiting to be violated, encouraging it on your own terms, but she doesn't do anything at all.  She simply watches you.  It isn't until a scowl of irritation makes its way across your face that she acts.\n\n", false);

		MainScreen.text("The ovipositor lurches over, bending and flexing to reach up to your exposed hole, barely touching you while it explores your cheeks.   You pant and grab your " + ButtDescriptor.describeButt(player) + ", pulling yourself wide to expose the clenching, dark circle that is your pucker.  It rubs across your hand before finally finding its way into your anal cleavage, ridges sliding between your backside and flopping against your ring while it blindly tries to get adjusted.  The moist, dripping slime it exudes quickly turns your ass into a green-glazed slip-'n-slide.  A sudden shock of sensation alerts you that yes, the ovipositor has finally found your asshole.\n\n", false);

		MainScreen.text("The tentacle-like tube prods forward, leaking ooze inside you as the first ring contracts to squeeze inside you.  You gasp when it pops into place, barely adjusting before the next ridge slides into the slightly-loosened sphincter.", false);
		player.buttChange(30, true, true, false);
		MainScreen.text("  Each segment somehow feels larger than the one preceding it, though it seems impossible given how widely stretched you feel by the time the fourth is inside you.", false);
		if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text("  The warm organ brushes past your prostate, the slippery caress making " + CockDescriptor.describeMultiCockSimpleOne(player) + " dribble a ", false);
			if (player.cumQ() < 500) MainScreen.text("drop", false);
			else MainScreen.text("stream", false);
			MainScreen.text(" of pre-cum down its length.", false);
		}
		if (player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("  Your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " quivers", false);
			if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 3) MainScreen.text(" and drips", false);
			MainScreen.text(" sympathetically with the sensation, and your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " peeks from the wet lips.", false);
		}
		MainScreen.text("  Yet you are not denied the anal pleasure you crave, as the widening bands now filling you are each pushing out harder than the one before.  At the stimulation, her own demonic clit swells as if posessed, nodulates, and hardens into dick form, poking up from her pussy and drooling a slime of pre-cum onto her abdomen.  The second ring rolls over your anal g-spot and you gasp, nearly losing your grip.  The third knocks your " + LowerBodyDescriptor.describeLegs(player) + " out from under you, and you collapse atop your partner, squeezing oozing dick", false);
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("s", false);
		MainScreen.text(" between you", false);
		if (player.lowerBody.vaginaSpot.get(0).clitLength >= 4 && player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(", and pressing your monster chick-stick into it; the gooey heat sends a tingle of shock up your spine and your pussy convulses", false);
		MainScreen.text(".", false);
		player.buttChange(50, true, true, false);
		MainScreen.text("\n\n", false);

		MainScreen.text("Shivering, you glance back up at the defeated woman; she has a passionate, prideful look plastered on her face, though it seems to waver each time another inch of ovipositor pushes past your stretched-out ring.\n\n", false);

		//([not mans]
		if (!player.lowerBody.cockSpot.hasCock()) MainScreen.text("\"<i>I never would've thought you were an anal-pregnancy slut,</i>\" hisses the spider in pleasure, flexing her organ harder while it curls through your guts, so deep inside you that you wonder how much further she can go.", false);
		else MainScreen.text("\"<i>Just because I have an exoskeleton doesn't mean I can't feel you dripping on me.  I thought I was the corrupt bitch here, but you... you take the cake.</i>\"", false);
		MainScreen.text("\n\n", false);

		MainScreen.text("With a final hard shove, the last few segments pop inside you", false);
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(", nearly crushing your prostate from the tight squeeze", false);
		MainScreen.text(".  ", false);
		if (!player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("You gasp and dribble a few strands of girl-goo, the contractions of your achingly empty pussy intensifying.  ", false);
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("You gasp and squirt a few weak ropes of cum, having a miniature orgasm on the spot.  ", false);
		MainScreen.text("Writhing happily, you sigh ", false);
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("while your " + CockDescriptor.describeMultiCockShort(player) + " dribbles white, wasted goo onto the black and " + monster.hairColor + " striped carapace below", false);
		else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("while your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " dribbles fem-slime onto the black and " + monster.hairColor + " striped carapace below", false);
		else MainScreen.text("while your fingers tweak your " + BreastDescriptor.describeNipple(0) + "s and your starved sphincter sucks at its slimy, squirming stuffing", false);
		MainScreen.text(".  Without warning, the ovipositor dilates, pulling your " + ButtDescriptor.describeButthole(player) + " even wider.  An explosion of warmth goes off in your guts, and you realize that your term as an incubator has begun with a sense of perverse delight.\n\n", false);

		MainScreen.text("The large, round egg pushes its way up your butt, squeezes past your ring", false);
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" and your prostate", false);
		MainScreen.text(", and travels deep inside you, bulging your belly the slightest bit once it's deposited.  With a groan of deep satisfaction, you start humping the fluid-slicked chitin beneath you, barely registering the long, seemingly unending series of eggs that infiltrate your ass", false);
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" and milk your prostate as they push past", false);
		MainScreen.text(".\n\n", false);

		MainScreen.text("The stream of slime only spreads as the stimulation of the sliding spheres slewing into your sphincter sets off", false);
		if (player.gender > 0) {
			MainScreen.text(", simultaneously, ", false);
			if (player.lowerBody.cockSpot.hasCock()) {
				MainScreen.text("a storm of semen-squirting spurts from your shaft", false);
				if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s", false);
			}
			else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("a siege of spray-scattering spasms from your snatch", false);
			MainScreen.text(" and", false);
		}
		MainScreen.text(" squirts of slippery spooge from her shuddering spunk-stick.  While you shriek, squirm and shiver shamelessly, you see the spider-slut suspended in the same, similarly a slave to the sensation of shoving and slamming her stalk into your straining sacrum and stuffing you with her strange spawn.\n\n", false);

		MainScreen.text("Gradually, you're lifted off the carapace by your inflating abdomen.  Your belly gurgles, making you burp as you're filled with enough eggs to make you feel uncomfortable and compressed.  The " + player.skinFurScales() + " of your stomach is drum-tight by the time you slide off the wilting ovipositor, and the bottom portion of it is soaked in the last of your partner's orgasm.  You look positively pregnant as you stagger upright and make for your garments, trying to ignore the exhausted drider's jibes.\n\n", false);

		MainScreen.text("By the time you finish getting dressed, she's fallen into a fitful slumber.  What an ordeal!", false);
		//GET KNOCKED UP
		player.buttKnockUp(PregnancyType.DRIDER_EGGS, PregnancyType.INCUBATION_DRIDER - 200, 10); //Butt pregnancy goes faster
		player.slimeFeed();
		player.orgasm();
		player.stats.cor += .5;
		cleanupAfterCombat();
	...
	private loseVSDriderGetAnalledOvi(): void {
		spriteSelect(77);
		MainScreen.text("", true);
		MainScreen.text("You fall to the ground, defeated by the agile arachnid-woman as she slowly advances towards you, her engorged clit growing larger and larger as it throbs in your direction.  You've managed to find yourself crumpled face down in the dirt, your " + ButtDescriptor.describeButt(player) + " in full display of the aroused drider while she grins lustfully at your broken figure.  Grunting, you begin to pull yourself off the ground, only to hear your attacker 'tsk' before feeling the warm stickiness of her web launch upon your limbs, gluing your hands to your ankles and leaving you completely vulnerable.  You struggle against the bindings, but to no avail.  Lifting your head, you turn to see the drider stroking her now fully-enlarged demon cock, replacing the hardened clit that once adorned her moist pussy.\n\n", false);
		MainScreen.text("\"<i>You just had to make it harder for yourself, didn't you?</i>\"\n\n", false);

		MainScreen.text("You grunt, too exhausted to move any further.  She chuckles at your disinterest, bending down to rest her thick member in between your cheeks and slowly grind along the cleft of your ass.  You feel the tiny nubs of her demonic cock rub along your eager asshole, pre-cum lubricating your hot entrance as she moans in unrequited pleasure.  Gritting your teeth, you force yourself to ", false);
		if (player.HP < 1) MainScreen.text("relax in order to reduce the discomfort", false);
		else MainScreen.text("calm your racing thoughts and resist showing how eager you are", false);
		MainScreen.text(" as she begins to enter inside of you.\n\n", false);

		MainScreen.text("Inch by inch, the bumps and curves of her nine-inch demonic dick massage your puckered hole as she plunges further in, working slowly to savor the feeling of being inside of you.  With a hearty grunt she fails to control her lust, driving in at full force and filling you with her corrupted member.", false);
		player.buttChange(monster.cockArea(0), true, true, false);
		MainScreen.text("\n\n", false);

		MainScreen.text("\"<i>Ungh, if I knew your ass was this eager to s-swallow my cock I wouldn't have wasted my time tying you up...</i>\"  She gasps, clearly the more impassioned of the two of you as she manages to bottom out inside of your " + ButtDescriptor.describeButt(player) + ".  You have very little time to regain your breath as the horny drider-woman begins to thrust in and out of your " + ButtDescriptor.describeButthole(player) + " with fevered abandon, tongue lolling out of her open mouth while you're forced to bounce against her crotch, your ", false);
	...
	private loseVSDriderAndVaginallyImpreggleOvi(): void {
		...
		//[LUST: Desperately (jerking your " + CockDescriptor.describeCock(player, 0) + \"<i>/grinding your fingers against your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + \"<i>/and shamelessly groping your " + biggestBreastSizeDescript() + \"<i>) you find yourself unable to continue fighting.  The carnal sexuality of the drider - her full chest, her gorgeous lips, and her pristine and gorgeous skin - has overwhelmed you.  Even if your mind was acutely aware that defeating your enemy was a necessity, your body would rebel. You need release.  More than that, you need her to provide it.] [BEATDOWN: With unceremonious indignity, you fall onto your " + ButtDescriptor.describeButt(player) + \"<i>.  It seems that you overestimated your capabilities, or underestimated the drider's strength, or perhaps both.  You have lost, and the expression that twinkles in her four eyes suggests that what awaits you is the violation you have come to expect from defeat.  Though weary, you steel yourself for what is to come. You may be her next victim, but you don't have to enjoy it.]
		MainScreen.text("The drider ignores you at first, lifting her head and glancing from side to side.  It's unclear why - perhaps she is worried that another predator will have been attracted by the sounds of combat, and attempt to steal her prey away.  Whatever her reasons she's apparently satisfied with what she sees.  Her gaze returns to you, eyes twinkling maliciously.  \"<i>Aren't you <b>precocious</b>,</i>\" she chuckles, walking slowly towards your prone body.  \"<i>Such an aggressive little morsel.</i>\"  A single strong spider leg places itself on your chest, pushing you ", false);
		if (player.isBiped() || player.lowerBody.isGoo()) MainScreen.text("down onto your back", false);
		else MainScreen.text("over, onto your side and then to your back", false);
		MainScreen.text(".  It seems that, regardless of their slenderness, she can still put an impressive amount of weight behind her legs.  They ", false);
		if (!player.lowerBody.isNaga()) MainScreen.text("pull your own " + LowerBodyDescriptor.describeFeet(player) + " apart", false);
		else MainScreen.text("scrape inquisitively at your scales", false);
		MainScreen.text(", her body climbing atop you.  Before you can realize her plans her spinneret is at work, ", false);
		if (!player.lowerBody.isNaga()) {
			MainScreen.text("coating your ", false);
			if (player.lowerBody.isTaur()) MainScreen.text("rear ", false);
			MainScreen.text(LowerBodyDescriptor.describeLegs(player) + " and pinning you in that prone, separated position", false);
		}
		else MainScreen.text("spraying webbing across your tail and weighing it down to pin it against the earth", false);
		MainScreen.text(".  She turns with the eerie speed of the arachnids of Mareth, quickly performing the same acts to your ", false);
		if (!player.lowerBody.isTaur()) MainScreen.text("wrists", false);
		else MainScreen.text("forelegs", false);
		MainScreen.text(" and leaving you pinned and spread-eagled on the ground.  \"<i>That should stem that.</i>\"\n\n", false);

		MainScreen.text("Turning, she makes as though to bite you, before pausing.  A thought runs through her head, and if the wicked curve of her dark lips are any indication, she likes it.  She gathers webbing on her fingers and bends down, inspecting your features carefully.  Placing said fingers on your chin she forces it open, quickly and expertly weaving sticky strands to connect your jaw and your chest.  The strands are just short enough to keep your chin held down, while strong enough that you can't seem to pull at them.  ", false);
		if (!player.lowerBody.isTaur()) MainScreen.text("Her fingers then move to the sides of your head, lacing webbing through your hair and sticking it firmly to the ground. More vulnerable than ever, you can't lift your head without ripping your hair from your scalp in chunks.", false);
		else {
			MainScreen.text("Her fingers then reach behind your head, pulling your upper body off of the damp dirt and stretching sticky strands between your skull and your immobile forelegs.  ", false);
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 4) MainScreen.text("With pain and then a pop, your " + player.BreastDescriptor.describeAllBreasts(player) + " squash out to the sides as she presses your torso into your knees.  ", false);
			MainScreen.text("Reaching back for more webbing, she ties your arms behind your back in a bundle of white thread.  Now pretzeled into an uncomfortable curled shape, you can't pull your torso away from your forelegs without wrenching your head off of your neck.  You feel completely helpless.", false);
		}
		MainScreen.text("  \"<i>Now, isn't that just precious,</i>\" the drider chuckles again.  She leans in low, her fingers trickling over your thighs like the steps of... well, of small spiders.  You gurgle in response, mouth held open uncomfortably.  She roams slowly up your body.", false);
		if (player.lowerBody.cockSpot.hasCock() && monster.nipplesPierced > 0) MainScreen.text("  Chains brush over " + CockDescriptor.describeMultiCockSimpleOne(player) + " as her chest passes your crotch.", false);
		MainScreen.text("  Her fangs trail teasingly along your belly, her fingers crawling up your sides.", false);
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) MainScreen.text("  Venom dribbles down the cleavage of your " + biggestBreastSizeDescript() + " as she approaches your vulnerable face.  Her own quartet of matronly tits comes to rest on them as her torso appears over their horizon.", false);
		MainScreen.text("\n\n", false);

		MainScreen.text("Drawing ever closer, at last her eyes blink slowly in front of yours.  Her lips seem to be pouting, then smirking, then - oh.  She's swirling something around in her mouth.  Finally it opens up above yours, her fangs extending.  Though you flinch in preparation for a bite, none comes - instead venom drips off her teeth and lips, every drop felt as it lands on your tongue.  You gurgle once more in protestation at this torture, but her hand pets the skin of your forehead maternally to remind you of your powerlessness.  Sadistic delight plays across her eyes as she watches your restrained struggles.  As the venom seeps, a heat grows.  Though subtle at first, you soon recognize it: lust.  Raw, burning need.  The poison coursing through you makes little effect on your heart, but a world of difference to your crotch.", false);
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  Worse still, its blood-swollen protuberance is met with no reaction from the drider.", false);
		MainScreen.text("  The slow drip serves no purpose - at least, none that serves you.  She is not teasing your body, nor whispering delicious torments.  All it does is force you to tremble more and more as she watches, knowing the effect that her venom will have. For no reason beyond her own sadistic amusement, she forces your body to put on the display of a helpless victim losing more and more control of their lust. You feel like a toy.\n\n", false);

		MainScreen.text("And the more you drink, the less you mind that.\n\n", false);

		MainScreen.text("Eventually she decides you've had enough, or perhaps grows impatient.  Her mouth closes, and her body rises.  Yours, incapable of either, merely burns.  The entirety of your body desires her, and your mind has a difficult time coming up with counterarguments.  Venom flows through you, commandeering your sensibilities and trading them in for a desire thus far unquenched.  She stands tall above you, straddling your body with her arachnid behind.  Something moist and ridged stretches out, caressing your thighs.  The degree to which you hope it's a cock is nearly immeasurable.  \"<i>I think you'll make a delectable little nest, morsel,</i>\" the drider croons.  \"<i>Let's start.</i>\"  Though you can't shift your head down to look at what she has in her behind, you can feel it.  It slaps messily against your thighs, searching out your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".  The caress the tentacle gives your slit is mockingly affectionate, but any illusions of gentility that may have persisted are soon ended by the thrust that delves greedily and deeply into your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".  If there is any small mercy, it's that the insatiable lust flooding your mind may wind up sated after all.\n\n", false);

		MainScreen.text("The thick, vile appendage twists inside of you as it advances.  Slick and obscene in its girth, each individual segmented ridge is palpable against your walls.  The prick practically breathes inside of you, pulsing with life and the ravenous desires that embody a phallus.  As it pushes it twists about in an attempt to wriggle in deeper, inadvertently showing the flexibility of its form.  \"<i>How many eggs have you lain, snack?</i>\" the drider coos.  Her fingers rub her sizeable swollen clit forebodingly.  It grows increasingly large, acquiring veins and bulges where no clit should.  \"<i>How swollen has your belly grown?</i>\"  Her tongue flicks across her venomous full lips.  \"<i>Time to enjoy it again.</i>\"  You can't see the tentacle shoved into your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ", but you are increasingly certain that it is in not, in fact, a dick.", false);
		player.cuntChange(50, true, true, false);
		MainScreen.text("  With her body in the way, you cannot clearly see what transpires in your loins, yet a sight emerges before your eyes that dominates your mind. Her clit has finished growing, and is no longer a clit.\n\n", false);

		MainScreen.text("From her crotch a cock has erupted, demonic and terrible.  It drips with sweat and musk, dribbling onto your collarbone.  The position you're in has never seemed more ominous - or more exciting.  Her cockhead smacks against your lower lip, pre-cum bouncing off and lightly spattering on your forehead.  Venom-addled, breathing in her fragrant spunk sends a quake up your body.  Inadvertently your hips start to seize, your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " shuddering in surprising pleasure.", false);
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  Even your " + CockDescriptor.describeCock(player, 0) + " spurts some pre-cum of its own.", false);
		MainScreen.text("  There's some shame in having a small orgasm from nothing more than the scent of sperm, but you place the blame solely on your intoxicated state.\n\n", false);
	...
	private loseVSDriderAndGetFuckedANDAnalImpreggegity(): void {
		...
		MainScreen.text("", true);
		if (player.stats.lust > 99) MainScreen.text("Unable to resist the desire coursing through your body, you strip off your " + player.inventory.armor.displayName + " and start stroking " + CockDescriptor.describeMultiCockSimpleOne(player) + ", displaying it in the hope that it will draw the drider's attention, while at the same time just trying to get yourself off and attain release.", false);
		else {
			MainScreen.text("You slump down, too beat up to mount an effective defense, let alone stay up on your " + LowerBodyDescriptor.describeFeet(player) + ".  The drider skitters over to you and effortlessly peels you out of your " + player.inventory.armor.displayName + ", exposing your ", false);
			if (player.stats.lust < 50) MainScreen.text("soft", false);
			else MainScreen.text("hard", false);
			MainScreen.text("member", false);
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s", false);
			MainScreen.text(" to the air.  She squeezes " + oMultiCockDesc() + " in her hand and pumps it, smiling while you stiffen and twitch precipitously in her grasp.", false);
		}
		MainScreen.text("  With a strength beyond what her lithe frame would imply, the spider-woman's shiny, exoskeletal arms slip under your shoulders and lift you up, hugging you into both pairs of breasts", false);
		if (monster.nipplesPierced > 0) MainScreen.text(", the crossed chains digging into your " + chestDesc(), false);
		MainScreen.text(".  The warmth of her nodule-covered cock pulsates on your belly, smearing you with a trail of pre-cum while she arranges your entwined bodies.\n\n", false);

		MainScreen.text("Lines of webbing burst from the arachnid abdomen, snapping into tree limbs before pulling taut.  The drider's eight legs clitter and clack, four of them folding around your back while your world rotates 90 degrees.  The two of you are pulled into the trees by the sticky strands, hanging sideways, almost upside-down, in the leaves.  The four-eyed monster-girl blinks asynchronously and smiles, shifting you slightly to bring " + oMultiCockDesc() + " against her moist lips.  You exhale, an intense, animalistic thrill shivering through you from the feeling of having your " + CockDescriptor.describeCock(player, x) + " lodged in a woman's entrance.  She leans down and bites into your neck, releasing a surge of toxin into your bloodstream.  It boils like fire, making you twitch and squirm in her many-limbed clutches.  The pain fades quickly, but in its place it leaves behind a molten, liquid lust, filling " + CockDescriptor.describeMultiCockSimpleOne(player) + " to its absolute maximum size, as hard as it can possibly be.\n\n", false);

		MainScreen.text("The " + monster.hairColor + "-haired dominatrix pulls back, a single drop of blood clinging to one of her fangs while she hisses, \"<i>Mmm, fresh meat is always so tasty... not to mention... firm...</i>\"\n\n", false);

		MainScreen.text("You look at her with fear, but your " + LowerBodyDescriptor.describeHips(player) + " start to twitch against her, rubbing your " + player.cockHead(x) + " up and down the slippery slit.  A slow exhale of pleasure escapes the spider's purple lips while her legs squeeze tighter against you, trying to push your " + CockDescriptor.describeCock(player, x) + " home.  The drider gasps when at last, you slip between her juicy outer labia, slurping deep into her cunny with a lurid squelch.  Finally held within her depths, your muscles all slacken, save for those you need to pump your hips against her, futilely trying to fuck her hot quim even though you have zero leverage.\n\n", false);

		MainScreen.text("\"<i>So eager!  I'm not going to let you fuck me, but I do want your sperm.  After all, I'll need to fertilize some more eggs.</i>\"\n\n", false);

		MainScreen.text("Wait, eggs?  While you try to chew over the thought, you're battling with your raging, venom-enhanced libido in order to think about something other than how good her pussy feels.  Fuck it!  You go back to grinding against your mistress, nuzzling at her neck while she twists her spider-half around you, holding you tightly in her chitin and bringing her abdomen up behind you.  A weird, wet appendage rubs over your exposed " + ButtDescriptor.describeButt(player) + ", slapping at your cheeks until it finds the cleft between them.  Sliding deeper, it bumps into your sphincter, spurts something warm, wet, and sticky against it, then pushes through.", false);
		player.buttChange(8, true, true, false);
		MainScreen.text("  You shudder from the anal violation and twist against the legs holding you in place, but with your " + CockDescriptor.describeCock(player, x) + " locked inside her, it's a useless endeavor.  It isn't shaped like a dick, and besides, that's currently dripping on your belly.  Oh gods... she mentioned eggs.\n\n", false);

		MainScreen.text("The strange organ keeps forcing its way deeper, slipping the steadily-widening, rib-like segments of its bulk into your " + ButtDescriptor.describeButthole(player) + ".  ", false);
		MainScreen.text("  Once it gets deep enough to push on your prostate, an upsurge of pleasure hits you hard enough to make " + CockDescriptor.describeMultiCockSimpleOne(player) + " dribble.  You moan against the drider's neck, pulsing pathetically inside the spider-woman's slit.  You know you'll soon be giving her exactly what she wants - all of it - and there's nothing you can do about it.\n\n", false);

		MainScreen.text("Another inch slides inside you, filling you uncomfortably full.  You shiver, clutching at your spidery mistress while she prepares both to deposit and to withdraw from the bank of you.   While you hang there, on the very edge of orgasm, it stops going deeper.  You've been given a reprieve, but it doesn't last long.  The drider kisses your forehead, her venomy lips spiking the sensitivity of your skin once again.  Suddenly, the fat tube in your " + ButtDescriptor.describeButthole(player) + " is dilating, stretching you wider.  Goo races up it, pumping into your guts, paving the way for the bigger bulk of the coming egg.  It stops at your pucker, battering against your muscles' resistance until it pops inside you, slamming into your prostate so hard that a huge rope of dick-milk erupts into the spider-woman's wet vagina.", false);
		player.buttChange(50, true, true, false);
		MainScreen.text("\n\n", false);

		MainScreen.text("The egg travels further up your butt, eventually flowing out the end of her stalk-like organ to rest deep in your digestive tract, ensconced in a bubble of slime.  Before you can adjust to this new sensation, another egg slips through your now-widened ring.  It squishes past your anal g-spot, rubbing it just as hard as the first, but before it even finishes traveling up your intestines, a third egg is forced inside you.  " + CockDescriptor.describeMultiCockSimpleOne(player, true) + " begins to cum wildly, squirting and leaking a torrent of jizz.  The first few ropes blast out with incredible force, hosing down her cunt", false);
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" and belly", false);
		MainScreen.text(".  Then, the constant application and release of pressure on your prostate utterly destroys the orgasmic timing of your muscles' contractions, making " + CockDescriptor.describeMultiCockSimpleOne(player) + " release a weak, but constant trickle of cum.\n\n", false);

		MainScreen.text("You quickly lose track of how many of the drider's eggs have gone inside you.  Feeling the numerous bulges in your abdomen, you give up entirely.  Your ass is going to be full of eggs and you've probably cum enough for her to make a whole 'nother batch.  Shuddering while the ovipositor is withdrawn, you sag, exhausted, barely realizing that the drider is hosing down your " + chestDesc() + " with her arachnid herm-spunk, the nodules ringing her dick pulsing in time with the blasts of thick spooge.  Some sticks to your chin, but you're so drained from your rough treatment that you barely react.\n\n", false);
		MainScreen.text("\"<i>Shhh, shhh... it's all right.  You'll be such a good mother for my young, and an even better father for the eggs you just fertilized.  Maybe if we ever meet again you can tell me how they looked when they were born.  For now, rest up.  I want you to take good care of our babies,</i>\" instructs the drider while she lowers herself towards the ground.  She sets you down on a soft spot, giving you a blanket of moss to recover in.  You struggle to stay awake but quickly slump into a deep slumber.", false);
		//(slimefeed)
		player.slimeFeed();
		//GET KNOCKED UP
		player.buttKnockUp(PregnancyType.DRIDER_EGGS, PregnancyType.INCUBATION_DRIDER - 200, 10); //Butt pregnancy goes faster
	...
	public birthSpiderEggsFromAnusITSBLEEDINGYAYYYYY(): void {
		...
		MainScreen.text("You shift position slightly, if only to get more comfortable, and relax as the eggs work their way through your guts.  With each egg's passage, your sphincter seems to be looser, more pliant, and more sensitive.  You blush and squeeze, hurrying the process to make yourself feel even better, and egg after egg splats wetly into the growing pile, each one slick and soaked in slime.  One after another, the heavy orbs exit your body, until the last one is sliding out of your rectum and you're having a strange anal orgasm that feels as relaxing as it is pleasant.\n\n", false);

		MainScreen.text("Clenching and relaxing, your slime-dripping asshole slowly narrows, gaping less visibly while you shudder and sigh, so happy and blissful that you don't even care about the crackling egg-shells hatching beneath you.  ", false);
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(CockDescriptor.describeMultiCockSimpleOne(player, true) + " stays limp, but drools a heavy flow of semen, puddling up a nice thick batch of your pleasure alongside your goo-pile.  ", false);
		if (player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("Your pussy clenches and ", false);
			if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < 3) MainScreen.text("drips", false);
			else if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < 4) MainScreen.text("dribbles", false);
			else if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < 5) MainScreen.text("gives a tiny squirt", false);
			else MainScreen.text("gushes a river of lady-spunk", false);
			MainScreen.text(" into the dirt, getting off along with your " + ButtDescriptor.describeButthole(player) + ".  ", false);
		}
		MainScreen.text("Once you come down from your high, you roll to the side and watch the tiny driders clambering out of their egg shells.\n\n", false);
		MainScreen.text("Your newborn children glance at you, chittering happily before they scurry off in the direction of the swamp.\n", false);
		player.orgasm();
	}
	...
	public driderPregVagBirth(): void {
		...
		MainScreen.text("After what seems like hours, you have lost count of the eggs pushed out.  ", false);
		if (player.lowerBody.isNaga()) MainScreen.text("At last, when no more eggs seem to be coming, you flop over onto your back, careful not to crush any of your dormant offspring.", false);
		else MainScreen.text("Looking between your legs you can see another slick, green egg slide from your body to join the sizable pile behind you.  At last, when no more seem to be following it, you flop over onto your back, careful not to crush any of your dormant offspring.", false);
		MainScreen.text("  As you lie panting, your revery is interrupted by a small cracking noise.  You sit up with a start and scan the area for someone approaching.  The cracking noise begins to multiply and you realize that it's coming from the pile of eggs nearby.  You begin watching with interest as your eggs start to hatch.\n\n", false);

		MainScreen.text("One egg rolls away from about the middle of the pile and you can see the little drider has almost made her way out of it.  With one last push she completely breaks out of the egg and falls face forward onto the ground.  The drider child seems to be having trouble coordinating all eight of her legs at the same time as she repeatedly picks herself up and stumbles a few steps before falling down again.  Looking back to the pile you can see many more of your drider children breaking out.  Soon enough, there are countless little drider girls, all about the size of your palm, skittering around.\n\n", false);

		//(If breasts)
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) {
			MainScreen.text("One drider girl cautiously makes her way over to you.  Her legs tickle your stomach as she climbs up to one of your " + BreastDescriptor.describeBreastRow(player.upperBody.chest.get(0)) + ".  Grasping the flesh tightly in her tiny hands she begins to suckle at your " + BreastDescriptor.describeNipple(0) + ".", false);
			if (player.upperBody.chest.count() > 1) MainScreen.text("  Following their sister's lead, more little driders take their place at each of your nipples and begin to drink.", false);
			else MainScreen.text("  Following her sister's lead, another little drider takes her place at your other nipple and begins to drink.", false);
			if (player.lactationQ() >= 3000) MainScreen.text("  One of your offspring seemed to be grasping your breast too tightly and drinking too greedily as suddenly you see one propelled away from you before she lands in a small puddle of milk nearby.  She seems to be struggling to right herself as she writhes around in the milk puddle, and so with a grin you reach over to pluck up the clumsy child and place her back at your waiting teat.", false);
			MainScreen.text("\n\n", false);
		}

		MainScreen.text("Around you, your many drider children fervently explore their surroundings.  One climbs up your back and perches atop your head.  She raises one hand to her forehead to block the sun's glare and stretches all four of her jewel-like eyes wide, delighting in the wondrous world she surveys.", false);
		if (player.upperBody.head.face.eyeType != EyeType.FOUR_SPIDER_EYES) MainScreen.text("  Suddenly she flops forward, leaning against your eyebrows and causing you to cross your eyes in an attempt to focus on her.  She seems confused by the way your eyes are different to hers.", false);
		if (player.lowerBody.isNaga()) MainScreen.text("  A few baby driders crawl ponderously across your scaled tail.  You give your tail a little wiggle and suddenly they go wild crawling around it and letting out the faintest, cute little squeals.", false);
		else MainScreen.text("  A few baby driders sit on each of your legs, facing each other.  Some gesture and squeak at each other in wonder while others pull faces.", false);
		MainScreen.text("\n\n", false);

		MainScreen.text("Nearby, two more of your little girls are in a stand-off, as each skitters back and forth in front of the other, ready to charge.  Suddenly one leaps forward and grabs the other before attempting to bite her arm.  Of course the attack is completely harmless - they are still much too young to have grown their fangs.  The other then turns the tables on her sister by grappling her sister to the ground, and soon, they're both letting out tiny giggles.\n\n", false);

		MainScreen.text("You lie back and your exhaustion finally begins to get the better of you when combined with the contentment from seeing your babies enjoying themselves.  Your eyelids begin to droop, and before you know it, you've slipped into unconsciousness.  You awaken later on, a sticky feeling between your legs to go with your wet pussy", false);
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" and raging hard on", false);
		MainScreen.text(".  After remembering where you are, the next thing you notice is the absence of your drider children.  Only a pile of empty, broken egg shells and some messy webbing in the trees remain.  As you stand up and prepare to move on, the sun reflecting off something in the corner of your eye catches your attention.\n\n", false);

		MainScreen.text("Between two nearby trees is a large spider web.  While not out of the ordinary, when the sunlight hits it at a certain angle the gossamer strands spell out a simple message:  \"<i>THANK YOU MOMMY.</i>\"", false);
		player.orgasm();
	}
	...
	private buttFuckADriderOhBaby(): void {
		...
		spriteSelect(77);
		MainScreen.text("", true);
		MainScreen.text("As soon as the drider-girl goes down, you leap into action.  Before she has a chance to recover, you run over and leap onto her back, balancing shakily as the drider stumbles back up onto her many legs, trying to shake you off.  You grab onto the drider's hair for support, pulling tight each time you're nearly displaced.  Yelping in agony, she soon learns her lesson and settles down, submitting to your dominance.\n\n", false);

		MainScreen.text("Now that she's calmed down, you give her a hard shove, putting her on her drider-belly so you're not nearly thrown off her every other moment.  With that done, you put your hands on her human back, pushing her forward and exposing her human-like bubble-butt, seated just above where woman meets spider. Greedily, you pull her cheeks apart, revealing the tiny star of her puckered asshole.  You pull your " + CockDescriptor.describeCock(player, x) + " out of your " + player.inventory.armor.displayName + ", already hardening from the thought of you pounding her tight little backdoor.\n\n", false);

		MainScreen.text("\"<i>Oh, you want that hole, do you? How very kinky,</i>\" the drider chides, but you aren't fooled by her tone – you can see her anus relaxing, preparing itself for your arrival.  With a grin, you grab your " + CockDescriptor.describeCock(player, x) + " and push it against her tight hole, leaning into the penetration until you feel it slip past her sphincter and into her hot, snug passage.  As you slide more and more of your cockmeat into her, you reach up from her cheeks and grope her breasts, squeezing her pert nipples between your fingers as you finally bottom out.  Since you can go no further up her butt, you start to piston your hips forward, slapping your hips to her ass with short, forceful strokes.\n\n", false);

		MainScreen.text("You're pleased to see the drider's tongue loll out of her mouth, her eyes rolled up into her head. Clearly she's enjoying this!  Encouraged, you start to fuck her butt harder, hammering into her ass with every bit of force your powerful muscles can summon up.  You're rewarded with a sharp, high moan from the drider.  Once she starts, she doesn't stop.  A symphony of moans, grunts, and pleasured screams erupt from the spider-girl, all in concert with your rough anal ravishing.\n\n", false);

		MainScreen.text("Without warning, the drider cums.  She cries out and grabs her fully-engaged corrupted cock, hosing down the trees in front of her with spider-cum as her butt squeezes down on your " + CockDescriptor.describeCock(player, x) + ", milking your dick with her anal passage.  You pick up the tempo of your fucking to break-pelvis speeds, ramming your cock into her backdoor and letting loose a torrent of your spunk, shooting it deep inside her", false);
		//(if Vagina: 
		if (player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text(" while your unused cunt spurts girlcum across her carapace", false);
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" and", false);
		}
		else if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" as", false);
		if (player.lowerBody.cockSpot.count() > 1) {
			MainScreen.text(" your spare cock", false);
			if (player.lowerBody.cockSpot.count() == 2) MainScreen.text(" coats", false);
			else MainScreen.text("s coat", false);
			MainScreen.text(" her human back with a thick white layer of spooge", false);
		}
		MainScreen.text(".\n\n", false);

		MainScreen.text("Finally done, you pull out of the drider, collapsing on her spider-back with a contented sigh.  Completely insensate after the anal ravaging you just gave her, the drider poses no threat as you leisurely gather " + CockDescriptor.describeMultiCockSimpleOne(player) + " back into your " + player.inventory.armor.displayName + " and lower yourself off her cum-soaked abdomen.  You give her a final, playful ass slap as you dismount before heading back to camp.", false);
	...
	private driderVagSmartFuck(): void {
		...
		MainScreen.text("You keep your distance from the downed spidermorph, still wary of her dripping, toxin-coated lips despite her weakened state and your own rising desire to breed her.  Trailing your eyes over her sweaty, splayed body you feel a familiar rush below the belt, your hardening [cock " + y + "] pressing against the inside of your [armor] as her breasts rise and fall seductively with each panting breath.  Desire threatens to overpower your caution as your gaze lingers on the vulnerable woman before you, her netherlips ");
		if (monster.HP < 1) MainScreen.text("parted slightly and glistening with beads of sweat from the adrenaline of battle.");
		else MainScreen.text("stuffed with her fingers, femcum streaming down the front of her spider-half as she desperately plays with herself.");
		MainScreen.text("  The need to bury your aching length inside that puffy snatch is almost unbearable, but you don't want to risk exposing yourself to any of her tricks, especially considering how close you'd be to her mouth as you take her.  Racking your brain for a solution, the mass of webbing that covers the canopy around this area catches your eye.");
		...
		if (player.lowerBody.tailType == TailType.SPIDER_ABDOMEN) {
			...
			if (player.lowerBody.isDrider()) MainScreen.text("scuttle");
			else MainScreen.text("saunter");
			MainScreen.text(" over to the collapsed drider.");
			driderTiedUpPartTwo(false);
		}
		//(Champion has some Tough Spider Silk on them)
		else if (player.inventory.items.has(useables.T_SSILK)) {
	...
	private dontSilkRapeDrider(): void {
		spriteSelect(77);
		MainScreen.clearText();
		MainScreen.text("Deciding against wasting the valuable material for little more than a quick fuck, you ponder your other options for using the defeated girl.");
		//(Return to "win" menu)
	...
	private driderTiedUpPartTwo(useItem: boolean): void {
		...
		if (!useItem) MainScreen.text("Shifting yourself, you withdraw your hand from her quivering slit, reaching back to collect another handful of stretchy silk.");
		else MainScreen.text("Reaching back into your pack, you grab the remainder of the bundle of silk that you tied the girl's hands with.  Looking over the mass of white-grey strands, you smile as you see that there seems to be more than enough left for what you have in mind.");
	...
	private driderTiedUpPartFour(): void {
		...
		//(if (30 =< corruption < 70)
			...
			MainScreen.text("You make sure she's comfortable before checking her for anything that could help your quest.  She stirs slightly as your hands caress her, eyes fluttering drunkenly as she tries to focus on you.  Something in the way her eyes bore into you fills you with a sense of shame at having taken advantage of her and you quickly sever her bonds, reasoning that she'll still be too weak to try anything.");
