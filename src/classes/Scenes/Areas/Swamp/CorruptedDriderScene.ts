...
//*Encounter Intro (done)
		public function driderEncounter():void
		{
			...
			//repeat intro:
				...
				if (player.tailType > TAIL.NONE) outputText("tail ", false);
...
//*Victory Rape: PC Penis goes in Vagina (done)
		private function winDriderPCDickInSpiderCunt():void
		{
			...
			if (player.lowerBody.cockSpot.count() > 1) {
				...
				if (player.lowerBody.cockSpot.count() > 2) outputText("es fan out to the side, forgotten while you give yourself over to the silken caresses of her glorious vagina.", false);
			...
			if (player.stats.cor > 80 && player.stats.lib > 50) outputText("Of course it's the truth, the cunt's clearly got a good grasp on the obvious. Unwilling to lose your grip on dominance, however, you try to keep struggling to get the upper hand.\n\n", false);
			...
			if (player.stats.cor > 80 && player.stats.lib > 50) outputText("You grumble out a denial.  You're not dumb, you just like to take charge and fuck. There's no reason to resist jumping on a nice wet pussy and making it squirt while you pack it full... but you can't quite get your limbs to move.\n\n", false);
			...
			if (player.statusAffects.get("Exgartuan").value1 == 1) outputText("  Exgartuan mumbles telepathicaly, \"<i>Fuck yes I do, but I wanna see how this plays out.</i>\"", false);
			...
			if (player.stats.cor > 80 && player.stats.lib > 50) outputText("that it's because of her own venom paralyzing your muscles so effectively.\n\n", false);
			...
			if (player.stats.cor > 80 && player.stats.lib > 50) outputText("Gods it was a bad idea not to bind her lips... ", false);
...
//Part 2 of the above
		private function winDriderPCDickInSpiderCunt2():void
		{
			...
			if (player.stats.cor > 80 && player.stats.lib > 50) outputText("The longer you sit there, the more your strength returns to you, and the more the submissiveness placed into you by her venom recedes. Still, you've tired yourself out too much to leave at the moment.", false);
...
//*Victory Rape: Ride Spider-Cawk in Vagoo (done)
		private function winVSDriderTakeItsCockInCunt():void
		{
			...
			if (player.tongueType > TONUGE.HUMAN) outputText(" as the rest of its demonic length lashes out and wraps around her pillowy bosom", false);
...
//*Victory Rape: Ride Ovi Vaginal (done)
		private function victoryVSDriderRideOviVaginal():void
		{
			...
			if (monster.HP < 1) {
				...
				if (player.lowerBody.cockSpot.hasCock()) outputText(" and " + sMultiCockDesc() + " twitch", false);
...
//*Victory Rape: Ride Ovi Anal sex-insenitive (done)
		private function victoryVSDriderRideOviAnal():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("s", false);
			outputText(" between you", false);
			if (player.lowerBody.vaginaSpot.list[0].clitLength >= 4 && player.lowerBody.vaginaSpot.hasVagina()) outputText(", and pressing your monster chick-stick into it; the gooey heat sends a tingle of shock up your spine and your pussy convulses", false);
			outputText(".", false);
			...
			//([not mans]
			if (!player.lowerBody.cockSpot.hasCock()) outputText("\"<i>I never would've thought you were an anal-pregnancy slut,</i>\" hisses the spider in pleasure, flexing her organ harder while it curls through your guts, so deep inside you that you wonder how much further she can go.", false);
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText(", nearly crushing your prostate from the tight squeeze", false);
			outputText(".  ", false);
			if (!player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) outputText("You gasp and dribble a few strands of girl-goo, the contractions of your achingly empty pussy intensifying.  ", false);
			if (player.lowerBody.cockSpot.hasCock()) outputText("You gasp and squirt a few weak ropes of cum, having a miniature orgasm on the spot.  ", false);
			outputText("Writhing happily, you sigh ", false);
			if (player.lowerBody.cockSpot.hasCock()) outputText("while your " + multiCockDescriptLight() + " dribbles white, wasted goo onto the black and " + monster.hairColor + " striped carapace below", false);
			else if (player.lowerBody.vaginaSpot.hasVagina()) outputText("while your " + vaginaDescript(0) + " dribbles fem-slime onto the black and " + monster.hairColor + " striped carapace below", false);
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText(" and your prostate", false);
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText(" and milk your prostate as they push past", false);
			outputText(".\n\n", false);
			...
			if (player.gender > 0) {
				...
				else if (player.lowerBody.vaginaSpot.hasVagina()) outputText("a siege of spray-scattering spasms from your snatch", false);
...
//(please view the spinneret as a spider-horsecock because for all intents and purposes that is really what it is) - shambadibs (1.5-inch width max because i'm racist against buur-bous american penis) (also probably a corruption minimum this is a pretty dick move) (I GET IT)
		private function victoryVSDriderStickDickInSpinneret():void
		{
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) outputText("  The semi-sticky stuff soaks into the cleavage of your " + player.allBreastsDescript() + ", the feeling surprisingly enjoyable.  You even knead the web in a bit, biting your bottom lip from the sensation.", false);
			...
			if (player.lowerBody.balls > 0) outputText(", even giggling a little as your " + ballsDescriptLight() + " slap against her nose", false);
			outputText(".\n\n", false);
			...
			if (player.lowerBody.cockSpot.count() > 1) {
				...
				if (player.lowerBody.cockSpot.count() > 2) outputText("cocks give ", false);
...
//*Loss Vaginal Impregnation and forced BJ - Symphonie (done)
		private function loseVSDriderAndVaginallyImpreggleOvi():void
		{
			...
			if (!player.isTaur()) outputText("Her fingers then move to the sides of your head, lacing webbing through your hair and sticking it firmly to the ground. More vulnerable than ever, you can't lift your head without ripping your hair from your scalp in chunks.", false);
				...
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 4) outputText("With pain and then a pop, your " + player.allBreastsDescript() + " squash out to the sides as she presses your torso into your knees.  ", false);
			...
			if (player.lowerBody.cockSpot.hasCock() && monster.nipplesPierced > 0) outputText("  Chains brush over " + sMultiCockDesc() + " as her chest passes your crotch.", false);
			outputText("  Her fangs trail teasingly along your belly, her fingers crawling up your sides.", false);
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) outputText("  Venom dribbles down the cleavage of your " + biggestBreastSizeDescript() + " as she approaches your vulnerable face.  Her own quartet of matronly tits comes to rest on them as her torso appears over their horizon.", false);
			outputText("\n\n", false);
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("  Worse still, its blood-swollen protuberance is met with no reaction from the drider.", false);
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("  Even your " + cockDescript(0) + " spurts some pre-cum of its own.", false);
...
//*Loss Ride & Impregnate Butt (done)
		private function loseVSDriderAndGetFuckedANDAnalImpreggegity():void
		{
			...
			if (player.lowerBody.cockSpot.count() > 1) outputText(" and belly", false);
...
//Summary: what goes up must come down
		public function birthSpiderEggsFromAnusITSBLEEDINGYAYYYYY():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText(SMultiCockDesc() + " stays limp, but drools a heavy flow of semen, puddling up a nice thick batch of your pleasure alongside your goo-pile.  ", false);
			if (player.lowerBody.vaginaSpot.hasVagina()) {
		...
		public function driderPregVagBirth():void
		{
			...
			if (player.eyeType != EYES.FOUR_SPIDER_EYES) outputText("  Suddenly she flops forward, leaning against your eyebrows and causing you to cross your eyes in an attempt to focus on her.  She seems confused by the way your eyes are different to hers.", false);
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText(" and raging hard on", false);
...
//Win Drider Male Anal (OH BABY)
		private function buttFuckADriderOhBaby():void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				...
				if (player.lowerBody.cockSpot.count() > 1) outputText(" and", false);
			}
			else if (player.lowerBody.cockSpot.count() > 1) outputText(" as", false);
			if (player.lowerBody.cockSpot.count() > 1) {
				outputText(" your spare cock", false);
				if (player.lowerBody.cockSpot.count() == 2) outputText(" coats", false);
