...
//*Victory Buttfucko
		private function victoryButtFuck():void
		{
			...
			if (player.lowerBody.cockSpot.count() > 1) {
				...
				if (player.lowerBody.cockSpot.count() == 2) outputText(" resting atop the small of his back", false);
			...
			if (player.lowerBody.balls > 0) outputText("r own");
			...
			if (player.lowerBody.cockSpot.count() == 2) outputText("  The leftover dick spurts out its half of your load over his ass, making it look more pearly-white than ever before.", false);
			else if (player.lowerBody.cockSpot.count() > 2) outputText("  The leftover dicks spurt out their portion of your load across his ass-cheeks, making them look more pearl-white than ever before.", false);
			if (player.cumQ() >= 500) {
...
//*Victory Frotting? (too biggo)
		private function victoryFrotTheSpoidah():void
		{
			...
			if (player.lowerBody.cockSpot.count() == 1) outputText("Dropping it squarely ", false);
...
//*Victory Cowgirl
		private function victoryCowgirlRidingOnSpiderBoi():void
		{
			...
			if (player.lowerBody.balls > 0) outputText("lifting your balls and ", false);
			...
			if (!player.upperBody.chest.hasFuckableNipples()) outputText("pinch", false);
			...
			if (player.stats.cor < 33) outputText("taking the time to clean the dripping spunk from your body.", false);
			else if (player.stats.cor < 66) outputText("casually wiping away the worst of the dripping spunk.", false);
...
//*Summary: Male only scene for maximum prostate pounding - possible random choice between two positions, plain doggy style or the 'lucky' one, where he will curl you over yourself and fuck you so you can watch his foreskinned spiderboy cock slam your asshole -while- you drip all over your face.
		private function spiderBoyLossMaleButtfuck():void
		{
			...
			if (player.HP < 1) {
				...
				if (player.stats.cor < 33) outputText("in fear", false);
				else if (player.stats.cor < 66) outputText("with worry", false);
			...
			if (player.lowerBody.cockSpot.count() == 1) outputText("A ribbon ", false);
			...
			if (player.stats.cor < 33) outputText("You blush shamefully and nod, utterly humiliated by the quick little orgasm.", false);
			else if (player.stats.cor < 66) outputText("You blush shamefully and nod, humiliated and aroused by how quickly you just came.", false);
...
//*Summary: Vagoozles only!
		private function loseToSpiderBoyVagFucked():void
		{
			...
			if (player.upperBody.chest.hasFuckableNipples()) outputText("into his mouth before plunging his tongue into the tit-pussy's passage", false);
			...
			if (player.stats.cor < 33) outputText(", half-stifled", false);
			...
			if (player.lowerBody.vaginaSpot.list[0].clitLength >= 3) outputText("frotting against him", false);
			...
			if (player.upperBody.chest.hasFuckableNipples()) outputText("drip their own slippery juices", false);
			else if (player.biggestLactation() >= 1) outputText("bead drops of milk", false);
			else outputText("fully engorge", false);
			outputText(", ", false);
			if (player.lowerBody.cockSpot.hasCock()) outputText(sMultiCockDesc() + " becomes so full and hard it feels like it could burst, ", false);
			outputText("and your " + clitDescript() + " ", false);
			if (player.lowerBody.vaginaSpot.list[0].clitLength >= 4) outputText("bounces up and down on your belly with each beat of your heart", false);
