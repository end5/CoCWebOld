		...
		public function driderKiss():void {
			...
			//Get hit 2nd time) 
				...
				if(player.statusAffects.get("DriderKiss").value1 == 1) {
					...
					if(player.lowerBody.cockSpot.hasCock()) outputText("  " + SMultiCockDesc() + " spews a rope of pre-cum into your " + player.armorName + ", desperate to get out and fuck.", false);
					if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Fem-cum dribbles down your " + player.legs() + " while your " + player.clitDescript() + " gets so hard you think it'll explode.", false);
