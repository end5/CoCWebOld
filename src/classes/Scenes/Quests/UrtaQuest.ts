	...
	if(flags[FlagEnum.URTA_QUEST_STATUS] > 0 && flags[FlagEnum.URTA_QUEST_STATUS] < 1) {
		if(player.short == "Urta") {
			if(player.lowerBody.cockSpot.hasCock()) {
				if(player.lowerBody.cockSpot.list[0].cockLength == 20 && player.lowerBody.cockSpot.list[0].cockThickness == 3) {
					if(player.lowerBody.vaginaSpot.hasVagina()) {
						if(player.upperBody.chest.list[0].breastRating == 7) return true;
					}
				}
			}
		}
...
public talkWithUrtaAboutFamFam():void {
	...
	flags[FlagEnum.URTA_QUEST_STATUS] = .5;
...
public startUrtaQuest():void {
	...
	player.lowerBody.cockSpot.list[0].cockLength = 20;
	...
	player.vaginas[0].vaginalLooseness = VAGINA_LOOSENESS.NORMAL;
...
//	player.armor = armors.URTALTA;
	...
	MainScreen.clearText();
	flags[FlagEnum.URTA_QUEST_STATUS] = .75;
...
private getHelBangedAsUrta():void {
	...
	if(flags[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] > 0) MainScreen.text(", and even how they worked together to bring down an evil sorceress and save her father. Your heart swells with pride as she recounts " + player2.short + " doing so a noble thing, and for a moment you know you made the right pick in a lover..");
...
private fuckHelAndKitsunesAsUrta():void {
	...
	flags[FlagEnum.URTA_FUCKED_HEL] = 1;
	player.orgasm();
	model.time.days++;
...
private visitPCPreUrtaQuest(truth:boolean):void {
	...
	if(flags[FlagEnum.ANEMONE_WATCH] > 0 && flags[FlagEnum.ANEMONE_KID] > 0)
		MainScreen.text("A confused-looking anemone with a " + ItemType.lookupItem(flags[FlagEnum.ANEMONE_WEAPON_ID]).longName + " nearly sees you, but you duck around a rock and escape her notice.  ");
	if(flags[FlagEnum.FUCK_FLOWER_LEVEL] >= 4 && flags[FlagEnum.FUCK_FLOWER_KILLED] == 0 && flags[FlagEnum.HOLLI_DEFENSE_ON] > 0)
	...
	if(truth)
	{
		...
		if(flags[FlagEnum.SLEEP_WITH] != "" && flags[FlagEnum.SLEEP_WITH] != 0) MainScreen.text("  " + player2.short + " suggests the two of you move off, so as not to wake " + flags[FlagEnum.SLEEP_WITH] + " in the night.");
	}
	//{LIE}
	else {
...
private putUrtasWangInPCsCunt(truth:boolean):void {
	...
	if(player.vaginaType() != 5) MainScreen.text("pink, ");
	...
	if(player2.isTaur())
	{
		...
		if(player2.tailType > TAIL.NONE) MainScreen.text("  You stroke her tail as you shift it aside");
...
private snuggleWithUrta(truth:boolean):void {
	...
	//[if PC Goo:
	else if(player.isGoo()) MainScreen.text("  " + player2.mf("His","Her") + " soft, squishy goo-body eagerly accepts you, the gelatinous mass of " + player2.mf("his","her") + " lower torso reaching out to softly envelop you in its cool, wet embrace.");
	//[Else:
...
private loseToGoblinsPartIVAsUrta():void {
	...
	flags[FlagEnum.URTA_GOBLIN_SLAVE] = 1;
	menu();
...
private urtaGameOver():void {
	...
	flags[FlagEnum.URTA_QUEST_STATUS] = -1;
	model.time.days++;
	resetToPC();
	model.time.hours = 6;
	statScreenRefresh();
...
//Gone	menuLoc = 3;
	...
	MainScreen.addButton(9, "Back", getGame().combatMenu, false);
}
...
public useSuccubiMilkOnGnollPrincesses():void {
	...
	if(flags[FlagEnum.URTA_FUCKED_HEL] == 1) MainScreen.text("that salamander slut's cunt");
...
private urtaFinishesWithBigOrange():void {
	...
	flags[FlagEnum.URTA_TENTACLE_GAPED] = 1;
	flags[FlagEnum.URTA_TENTACLE_RAPED] = 1;
	player.orgasm();
...
private submissiveMinoUrtaBadEndPtIII():void {
	...
	flags[FlagEnum.URTA_MINO_AND_SUCCUBI_SLAVE] = 1;
	menu();
	MainScreen.addButton(0,"Next",urtaGameOver);
}
...
public winRapeAMinoLordAsUrta():void {
	...
	flags[FlagEnum.URTA_RAPED_MINO_LORD] = 1;
...
public urtaLosesToMinotaurRoughVersion():void {
	...
	flags[FlagEnum.URTA_IS_VULQUINE_MENACE] = 1;
	menu();
...
public beatMinoLordOnToSuccubi():void {
	...
	//{If Urta raped Fido:}
	if(flags[FlagEnum.URTA_RAPED_MINO_LORD] > 0) {
...
public milkyUrtaTic():void {
	...
	if(player.lust < 50) MainScreen.text("Thankfully it hasn't become too distracting yet.");
	else if(player.lust < 75) MainScreen.text("You don't know how much longer you'll be able to withstand the effects of the milk.");
...
private getKnockedUpWithGodChild():void {
	...
	if(player.wetness() >= 4) MainScreen.text("wet ");
	...
	if((amilyScene.amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2) && jojoScene.campCorruptJojo()) ballWorship[ballWorship.length] = 0;
	if((amilyScene.amilyFollower() && flags[FlagEnum.AMILY_FOLLOWER] == 2) || jojoScene.campCorruptJojo()) ballWorship[ballWorship.length] = 1;
	...
	if(flags[FlagEnum.VALARIA_AT_CAMP] == 1) ballWorship[ballWorship.length] = 5;
	let choice: number;
	...
	if(player.tailType > TAIL.NONE) MainScreen.text(", though getting your tail angled right to allow her entrance is a bit of a challenge");
	else if(player.isTaur()) MainScreen.text(", though getting down on your hands and forelegs is a bit challenging for you");
	...
	if(player.wetness() >= 5) MainScreen.text("Lady-spunk showers out of your spasming twat, glazing Urta's bouncy ballsack with your lusty liquid.");
	else if(player.wetness() >= 3) MainScreen.text("Lady-spunk drools out of your spasming twat in thick streams, glazing Urta's bouncy ballsack with your liquid lust.");
	else if(player.wetness() >= 1) MainScreen.text("Lady-spunk drips from your spasming twat in steady trickles, glazing Urta's bouncy ballsack with your liquid lust.");
...
private urtaAndEdrynGodChild():void {
	...
	flags[FlagEnum.URTA_FERTILE]        = telAdre.edryn.pregnancy.type;       //Use these two flags to store the pregnancy that Taoth is overriding.
	flags[FlagEnum.URTA_PREG_EVERYBODY] = telAdre.edryn.pregnancy.incubation; //Since they can't be in use prior to Taoth being born this is fine.
