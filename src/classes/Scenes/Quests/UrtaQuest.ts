...
public function infertilityQuestions():void {
	...
	outputText("\n\nWell, ");
	if(player.stats.cor < 33) outputText("you have nothing to complain about there.  Your sexual romps with Urta are always a delight.");
	else if(player.stats.cor < 66) outputText("she's definitely a handful in the sack, you've got to give her that.");
...
private function getHelBangedAsUrta():void {
	...
	if(flags[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] > 0) outputText(", and even how they worked together to bring down an evil sorceress and save her father. Your heart swells with pride as she recounts " + player2.short + " doing so a noble thing, and for a moment you know you made the right pick in a lover..");
...
private function visitPCPreUrtaQuest(truth:boolean):void {
	...
	if(flags[FlagEnum.ANEMONE_WATCH] > 0 && flags[FlagEnum.ANEMONE_KID] > 0)
		outputText("A confused-looking anemone with a " + ItemType.lookupItem(flags[FlagEnum.ANEMONE_WEAPON_ID]).longName + " nearly sees you, but you duck around a rock and escape her notice.  ");
	...
	if(player.statusAffects.has("DungeonShutDown")) outputText(", no matter why " + player2.mf("he","she") + " was sent here");
	...
	if(truth)
	{
		...
		if(flags[FlagEnum.SLEEP_WITH] != "" && flags[FlagEnum.SLEEP_WITH] != 0) outputText("  " + player2.short + " suggests the two of you move off, so as not to wake " + flags[FlagEnum.SLEEP_WITH] + " in the night.");
	}
	//{LIE}
	else {
...
private function putUrtasWangInPCsCunt(truth:boolean):void {
	...
	if(player2.isTaur())
	{
		...
		if(player2.tailType > TAIL.NONE) outputText("  You stroke her tail as you shift it aside");
...
public function useSuccubiMilkOnGnollPrincesses():void {
	...
	if(flags[FlagEnum.URTA_FUCKED_HEL] == 1) outputText("that salamander slut's cunt");
...
public function urtaSubmitsToMinotaurBadEnd():void {
	...
	if(monster.statusAffects.has("MinotaurEntangled")) outputText("   Seeing the fire in your eyes change from a determined glare to a lusty look, the minotaur pulls you over, carefully unwinding the chain from around you, so as not to damage you.");
...
private function knockUpUrtaWithGodChild():void {
	...
	if(player.lowerBody.cockSpot.count() == 1) outputText(" your own");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText(", the mess made worse by your multiple-cocked virility.  There's so much that the clear juices roll off you to either side, puddling in your blankets below");
...
private function postFuckUrtaUp():void {
	...
	if(player.statusAffects.has("DungeonShutDown")) outputText(" - besides, given you know that you're nothing but a sacrifice the demons are too lazy to collect, it's not like they'll really send an invasion through");
...
private function preggedUrtaWithGodChildEpilogue():void {
	...
	if(player.stats.cor > 66) outputText(", what do you care, anyway?");
	else outputText(".");
...
private function getKnockedUpWithGodChild():void {
	...
	if(player.lowerBody.balls > 0) outputText(" and lifting your balls");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText("soft curve");
	else outputText("flat muscles");
	outputText(" until she hits your [nipple].  ");
	if(player.upperBody.chest.hasFuckableNipples()) outputText("Her tongue plunges inside, savoring the feminine lubricants");
	...
	if(player.lowerBody.balls > 0) outputText("both sets of");
	else outputText("her");
	...
	//{Bimbo Sophie ball worship here}
		...
		if(player.lowerBody.balls > 0) outputText("  She does the same to your own [balls], even giving your [asshole] a happy, sloppy kiss before returning to polishing the supple, skin-shrouded spheres just ahead.");
