...
private function comfortUrtaAfterFamFamTalk():void {
	...
	if(urta.urtaDrunk()) Render.text("boozy ");
	Render.text("kiss on your lips as she hugs you tight - tight enough that you feel every vein through the concealing silk of her outfit.  \"<i>I'm a lucky girl.</i>\"");
...
public function infertilityQuestions():void {
	...
	Render.text("\n\nWell, ");
	if(player.stats.cor < 33) Render.text("you have nothing to complain about there.  Your sexual romps with Urta are always a delight.");
	else if(player.stats.cor < 66) Render.text("she's definitely a handful in the sack, you've got to give her that.");
	else Render.text("she's a willing fuck, so no complaints there.  Now if she would just get on with it - if you wanted to flirt, you would've instigated it.");
...
//	player.armor = armors.URTALTA;
	...
	Render.text("<b>You are Urta, a hermaphroditic fox-morph.</b>");
...
private function getHelBangedAsUrta():void {
	...
	if(flags[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] > 0) Render.text(", and even how they worked together to bring down an evil sorceress and save her father. Your heart swells with pride as she recounts " + player2.short + " doing so a noble thing, and for a moment you know you made the right pick in a lover..");
	Render.text(".  You sit and listen for a long few minutes, sipping at the beer until the liquid courage starts to work its wonders, giving you a little tingle... and helping to open you up, too.  Soon, you're drawn into the conversation, joking and teasing with the other girls as if you'd been friends for ages.");
...
private function fuckHelAndKitsunesAsUrta():void {
	...
	//If PC has a cock:
	if(player2.hasCock()) {
		Render.text("  \"<i>I've got a better one ");
		if(helFollower.followerHel()) Render.text("at home");
		else Render.text("at my beck and call");
		Render.text(", you know...  But we've got that in common, don't we?  Mmm, maybe we can share it some day?  A horny girl like you'd just love that, woudn't you? Getting to worship " + player2.mf("his","her") + " cock beside me while my tail fills your ass so full you scream....</i>");
	}
...
private function visitPCPreUrtaQuest(truth:boolean):void {
	...
	if(player.statusAffects.has("JojoNightWatch") && player.statusAffects.has("PureCampJojo"))
		Render.text("You easily sneak past a mouse monk.  He's looking towards the sky mostly, perhaps watching for imps.  ");
	if(flags[FlagEnum.ANEMONE_WATCH] > 0 && flags[FlagEnum.ANEMONE_KID] > 0)
		Render.text("A confused-looking anemone with a " + ItemType.lookupItem(flags[FlagEnum.ANEMONE_WEAPON_ID]).longName + " nearly sees you, but you duck around a rock and escape her notice.  ");
	if(flags[FlagEnum.FUCK_FLOWER_LEVEL] >= 4 && flags[FlagEnum.FUCK_FLOWER_KILLED] == 0 && flags[FlagEnum.HOLLI_DEFENSE_ON] > 0)
		Render.text("A dangling tentacle nearly hits you in the dark.  You roll aside at the last moment, looking up to see dozens of the things idly dangling around.  When did this camp get such a corrupt tree in it?  You suppose it must have its uses in defending against foes from the sky...  ");
	if(camp.companionsCount() > 2) Render.text("There are a number of people in the camp, but you avoid them as you head towards " + player2.short + "'s bunk.  ");
	Render.text("The camp looks pretty nice actually.  Living out here must have given " + player2.mf("him","her") + " plenty of time to improve it.");
	...
	if(truth)
	{
		...
		if(player2.cor < 33) Render.text(player2.mf("he","she") + " argues that " + player2.mf("he","she") + " should come anyway.  You tell " + player2.mf("him","her") + " no, firmly, even though it breaks your heart.");
		else if(player2.cor < 66) Render.text(player2.mf("he","she") + " nods understandingly, not surprised that you have to handle this quest alone.  The two of you are alike in that way - each with their own burden to bear.");
		else Render.text(player2.mf("he","she") + " nods dismissively, trusting you to handle it alone you suppose.  " + player2.short + " always was hard like that, even though " + player2.mf("he","she") + " clearly likes you.");
		Render.text("  You hold on to " + player2.mf("him","her") + ", for comfort, dreading the long day ahead.  " + player2.mf("He","She") + " pats your back comfortingly as you cry, and when you break from the hug, " + player2.mf("he","she") + "'s looking at you expectantly.");
		if(flags[FlagEnum.SLEEP_WITH] != "" && flags[FlagEnum.SLEEP_WITH] != 0) Render.text("  " + player2.short + " suggests the two of you move off, so as not to wake " + flags[FlagEnum.SLEEP_WITH] + " in the night.");
	}
	//{LIE}
	else {
...
private function urtaPutsHerDickInPCsWithDicks(truth:boolean):void {
	...
	if(player2.tallness < 60) Render.text(" and let your tits hang over " + player2.mf("his","her") + " " + player2.face());
	else if(player2.tallness < 76) Render.text(" and let your tits swing between you");
	else if(player2.tallness < 84) Render.text(" and let your tits smash into " + player2.mf("his","her"));
	else Render.text(" and let your tits sway over " + player2.mf("his","her") + " belly");
	Render.text(", giving " + player2.mf("him","her") + " something to enjoy while you begin to shift position, dragging your dick down " + player2.mf("his","her") + " length");
	if(player2.hasVagina()) Render.text(", past " + player2.mf("his","her") + " pussy,");
	Render.text(" and towards the prize you've been wanting since you got here - " + player2.mf("his","her") + " asshole.");
	...
	if(player2.analCapacity() > 70) Render.text("  Thankfully, " + player2.mf("he","she") + "'s nice and stretched from all the butt-fucking " + player2.mf("he","she") + "'s had to take, and you easily pop inside.  It feels so good that your flare grows wider, pulling you deeper into that welcoming, tight anal heat.");
	else if(player2.analCapacity() > 50) Render.text("  Thankfully, " + player2.mf("he","she") + "'s got plenty of give in " + player2.mf("his","her") + " back-door, and you pop inside after applying a little more pressure.  It feels so good that your flare grows wider, slowing your descent into that tight, anal heat.");
	else Render.text("  Unfortunately, there's not much give in " + player2.mf("his","her") + " back-door, and you have to sit there smearing pre over it, again and again, each time pushing a little harder.  After what feels like the fiftieth try, you finally pop your fat flare into " + player2.mf("his","her") + " rectum, the tight anal heat causing you to flare from the force of " + player2.mf("his","her") + " pressure.  Slowly, you push inch after throbbing inch of your swollen, stallion-like erection into " + player2.mf("his","her") + " asshole.");
	if(player2.ass.analLooseness == 0) {
		Render.text("  <b>You've taken " + player2.mf("his","her") + " anal virginity!</b>");
		player2.ass.analLooseness = 1;
	}
	Render.text("  " + player2.short + " writhes as " + player2.mf("he","she") + " takes you, " + player2.mf("his","her") + " hips lifting slightly off the ground to make sure you grind into " + player2.mf("his","her") + " prostate.  A long strand of clear excitement oozes from " + player2.mf("his","her") + " tip");
	if(player2.cockTotal() > 1) Render.text("s");
	Render.text(", thick, viscous, and clinging to the drying mess you left behind.");
	...
	if(player2.cockTotal() > 1) Render.text("s");
	Render.text(" between your bodies, " + player2.mf("his","her") + " liquid love exploding all over your cock-locked forms.  Each time you cum, a fresh-squeezed spurt of semen splatters into your tits, " + player2.mf("his","her") + " prostate violently being milked by the heady flows of vixen-spunk you force inside " + player2.mf("his","her") + " rectum.");
	if(player2.hasVagina()) Render.text("  You don't notice the female fluids that drip from her sodden slit - they soak into your big balls, already forgotten.");
	...
	if(player2.cumQ() >= 500) {
		Render.text(", mixing with " + player2.mf("his","her") + " own ");
		if(player2.cumQ() < 1000) Render.text("puddly");
		else if(player2.cumQ() < 2500) Render.text("small lake-sized");
		else Render.text("massive, lake-sized");
		Render.text(" mess - only now do you realize just how much your lover could produce!  The");
	}
	else Render.text(", but the");
	Render.text(" semen-inundated fabric quickly begins to dry as the parched, wasteland dirt reclaims the moisture.");
...
private function putAPCDickInUrtaCoochies(truth:boolean):void {
	...
	if(player2.gender == 3) Render.text("...well, close enough... ");
	Render.text("to ever fuck you like a woman.  Grabbing " + player2.mf("his","her") + " face, you pull " + player2.mf("him","her") + " into a deep, passionate kiss, unthinkingly grinding your dripping cock and steadily-wettening gash into " + player2.mf("his","her") + " belly and rubbing your own pebbly onyx nipples against " + player2.mf("his","her") + " nipples.  " + player2.mf("His","Her") + " own " + player2.cockDescript(x) + " begins to drool precum");
	if(player2.hasVagina()) Render.text(", her cunt dripping wetly onto the ground from the stimulus");
	Render.text(", the fluid rolling down its length to puddle with yours on " + player2.mf("his","her") + " belly and filling your writhing, passionate embrace with hot, sticky sweetness.");
	...
	if(player2.isTaur() || player2.isDrider()) {
		...
		if(player2.cumQ() < 1000) Render.text("falling face-first into the jizz-mud");
		else Render.text("not even bothering trying to move your swollen bulk");
		Render.text(", and you smile dopily up at " + player2.mf("him","her") + ", love shining in your eyes.  Carefully, " + player2.mf("he","she") + " positions " + player2.mf("him","her") + "self beside you and strokes your face before closing " + player2.mf("his","her") + " eyes, making it quite clear " + player2.mf("he","she") + " intends to go to sleep right here.  You see no point in fighting the inevitable and instead give yourself over to slumber - wouldn't be the first time you woke up in dried cum-mud, after all...");
	}
	//Else:
	else {
		...
		if(player2.hasVagina()) Render.text(", her female jizz spattering unnoticed onto the ground beneath the pair of you");
		Render.text(".");
		...
		if(player2.cumQ() >= 1000) Render.text(" not to mention the weight of your overstuffed belly");
		Render.text(" you don't really feel inclined to move, either.  Instead, you yawn loudly, make yourself comfortable upon your lover's stomach, and allow yourself to drift off to sleep - already, you can hear the quiet breathing of " + player2.short + " as " + player2.mf("he","she") + " does the same thing.");
	}
	player.orgasm();
	dynStats("sen", -2);
	menu();
...
private function putUrtasWangInPCsCunt(truth:boolean):void {
	...
	Render.text("pulsing interior.  The sight is exciting in the extreme, immediately sending your heartbeat through the roof.  Your tremendous horse-cock is immediately stiffened by your ardor, along with your coal-black nipples, and your armor-plated skirt begins to rise, as if by magic.");
	...
	if(player2.pregnancyIncubation > 0 && player.pregnancyIncubation < 150) Render.text("the curve of her pregnant belly");
	else if(player2.tone >= 65) Render.text("her toned abdomen");
	else if(player2.tone < 30) Render.text("her soft, forgiving middle");
	else Render.text("her belly");
	Render.text(".  The attentions drive your attempts to dislodge the offending garments from your lusty frame to new levels.  Metal rivets slowly give before finally snapping apart to free your left shoulder.  Your left nipple tingles with pleasant, palpable relief, and you turn to the other side, intent on ripping it off, so that you can be with your lover, skin to skin.");
	...
	if(player2.isTaur())
	{
		Render.text("and climb off the 'taur-woman.  She smirks up at you, until you bid her to rise, telling her that you plan to mount her like the fertile filly she is. Reluctantly, " + player2.short + " clatters up onto her hooves.");
		if(player2.tailType > TAIL.NONE) Render.text("  You stroke her tail as you shift it aside");
		else Render.text("  You stroke her swollen sex as you admire it");
		Render.text(", making sure you've got the perfect view of her lube-slimed horse-cunt.  Unbidden, your cock thickens, disgorging a potent trail of animal-pre down its underside in anticipation.");
		if(player2.wetness() >= 4) Render.text("  Fuck, she's wetter than Edryn!  Just look at those heavy trails of slick moisture as they run down her rump and hindlegs.");
		else if(player2.wetness() >= 2) Render.text("  Fuck, she's as wet as Edryn!  Beads of moisture are forming on her clit, ready to drop to the ground.");
		else Render.text("  Fuck, she's almost as wet as Edryn!  Just getting a sniff of her horny horse-snatch has your head swimming!");
		Render.text("  You give it a lick and shudder, taking in a few deep lungfuls of her scent.");
		if(player2.tallness >= 84) Render.text("  Before you know what you're doing, you've pulled her back to a rock and climbed atop it, bringing your cock level with that heavenly hole.");
		else Render.text("  Before you know what you're doing, you've raised yourself up to your full height, bringing your cock level with that heavenly hole.");
	}
	//{goo}
	else if(player2.isGoo()) {
		Render.text("and roll the gooey woman so she's face down.  She holds herself up off the dry dirt with her palms, sweet, slimy bottom upraised above her goopy undercarriage.  Amazingly, there's a slight parting in the mass of slime, right between where her legs would be, if she had them.  A narrow, barely visible divide can be spotted inside, capped with the telltale bulge of her ");
		if(player2.clitLength < .5) Render.text("tiny ");
		else if(player2.clitLength >= 2.5) Render.text("massive ");
		Render.text("button.  You nuzzle up to that slick slime-cunt, sniffing at her super-wet snatch, taking the pheromones as deeply inside yourself as possible.  Her femininity is so strong that it makes your head spin and your dick ache.  Glancing down as you lick the moisture from your muzzle, you see the rigid, over-engorged veins standing out in harsh relief on your equine tool.  Your legs seem to move on their own, lining you up with the moisture-laden slit just inches away.");
	}
	//{nagas}
	else if(player2.isNaga()) {
		Render.text("and roll the snake-woman over so she's prostrate on the ground, barely holding her front half up with her arms.  Her loins are flat on the ground, undulating slightly as she grinds her moist clit against it for sensation.  Your lover's anatomy poses a problem for your choice of positions, but then, inspiration strikes.  You come up under the hind part of her snake-tail, and hoisting it up, you support half over your shoulder.  As expected, she has all the flexibility of the serpent she so resembles, and her tail bends down just far enough for her cunt to hang a mere inch above your rigid tool, right where her body changes from snake to person.");
		if(player2.wetness() >= 3) Render.text("  Her moisture drizzles from that wanton slit, basting you with her fragrant femininity, and you have a hard time not taking her here and now.");
		Render.text("  The tail you're holding up coils in, sliding around your arm and middle, wrapping you up in snakey snugness.  With impeccable control, she squeezes a little bit, bringing her cunt close enough to breathe foggy moisture on your equine tool.");
	}
	//{biped}
	else {
		Render.text("and roll her over on her hands and knees, lifting that sweet butt up in the air where you can admire it.  You kneel behind her and, pressing your nose into her fragrant quim, you extend your tongue, letting the smooth oral muscle taste her juices and tease at her engorging clit.  " + player2.short + " moans and presses back into you, crying out like a true bitch in heat, one in need of a thorough rutting.  The thought hits your cock like a lightning bolt, and it fills out to painful proportions, every vein and bulge standing out in stark relief, thumping obscenely to the tune of your insatiable lust.  You moan into her lovely-tasting snatch, gulping down ");
		if(player2.wetness() < 3) Render.text("every drop of nectar that she gives up");
		else if(player2.wetness() < 4) Render.text("the copious drops of nectar she gives up");
		else if(player2.wetness() < 5) Render.text("the drizzling moisture that oozes from her twat");
		else Render.text("the rivers of girl-lube that run unimpeded from her sopping twat");
		Render.text(".  You sniff in great lungfuls of her, your head swimming with need, and before you know it, you've risen behind her, your oozing flare just inches from that delectable opening.");
	}
	...
	if(player2.pregnancyIncubation > 0 && player2.pregnancyIncubation < 150) Render.text(" (more than it already is)");
	Render.text(" from your prodigious deposit.  The illusion of knocking her up takes hold of you, and your orgasm redoubles, fresh waves of horse-cum pouring out of you unceasingly.  " + player2.short + " moans, rubbing her taut");
	if(player2.pregnancyIncubation > 0 && player2.pregnancyIncubation < 150) Render.text(" pregnant");
	Render.text(" dome, until at last you finally spend the last of your spunk.");
...
private function snuggleWithUrta(truth:boolean):void {
	...
	if(player2.isTaur() || player2.isDrider()) {
		Render.text("  It's rather awkward, thanks to " + player2.mf("his","her") + " clunky ");
		if(player2.isTaur()) Render.text("centaur");
		else Render.text("drider");
		Render.text(" body, but you manage to press up against " + player2.mf("his","her") + " ");
		if(player2.isTaur()) Render.text("forehoofs");
		else Render.text("first two pairs of spidery limbs");
		Render.text(".");
	}
	//[if PC Naga:]
	else if(player2.isNaga()) Render.text("  " + player2.mf("His","Her") + " serpentine coils loop snugly around your waist, curling around your legs and holding you gently in place.");
	//[if PC Goo:
	else if(player.isGoo()) Render.text("  " + player2.mf("His","Her") + " soft, squishy goo-body eagerly accepts you, the gelatinous mass of " + player2.mf("his","her") + " lower torso reaching out to softly envelop you in its cool, wet embrace.");
	//[Else:
	else Render.text("  Knees to knees, it takes you a few moments to figure out how to properly position your legs, but you eventually manage it.");
	...
	if(player2.cor < 33) Render.text("playful pat");
	else if(player2.cor < 66) Render.text("gentle stroke");
	else Render.text("lascivious caress");
	Render.text(" before pulling you tighter into " + player2.mf("his","her") + " embrace.  Eager to forget your embarrassment, you focus on holding " + player2.mf("him","her") + " tight and close.  Your prick slowly slides back into its sheath from the lack of stimulus as you focus on how safe and content you feel to be here, being held like this.");
...
private function morningAfterCampVisitEmbark(truth:boolean):void {
	...
	model.time.days++;
	statScreenRefresh();
	Render.text("Sadly, morning comes, and your time with " + player2.short + " comes to an end.  You push " + player2.mf("his", "her") + " shoulder gently to rouse " + player2.mf("him", "her") + ", smiling at " + player2.mf("him", "her") + " when " + player2.mf("he", "she") + " wakes.  You kiss " + player2.mf("him", "her") + " on the lips before rolling out from under the blanket.  The two of you eat a quiet breakfast together, simply enjoying each other's company.  ");

	if (truth)
	{
		Render.text("" + player2.mf("He", "She") + " asks again about coming with you, but you tell " + player2.mf("him", "her") + " Gul's instructions were quite specific.  You'll need to tackle this alone.  The rest of breakfast passes in uncomfortable silence, broken when you rise to leave.");
	}
	...
	if (truth)
	{
		Render.text(" before you promise, \"<i> I'll come back to you, stronger and ready for kids, I promise.</i>\"\n\n");
	}
	else
	{
		Render.text(".  ");
	...
	Render.text(player2.short + " nods, and you leave " + player2.mf("him","her") + " behind.  You keep glancing over your shoulder as you leave, praying silently to any god who'll listen that you return safely to your lover.");
...
public function winFuckAGoblinBroodmotherAsUrta():void {
	...
	else Render.text("feverishly masturbating");
	Render.text(" form of your opponent and can't resist licking your black lips in anticipation - any guilt about fucking someone besides " + player2.short + " that might make it through the haze of lust consuming you is quickly extinguished by your conviction that " + player2.mf("he","she") + " would understand.  Your growing tower of fuck-meat is already lifting the hem of your armored skirt, making it even easier to grab its lower edge and flip it up out of the way as you advance towards your assailant, eager to see what you can do about blowing off some steam...");
...
//		doNext(getGame().combatMenu);
	...
	fatigue(-50);
	dynStats("lus", -50);
	Render.text("Closing your eyes for a moment, you focus all of your willpower on pushing yourself to your absolute limits, forcing your lusts down and drawing on reserves of energy you didn't know you had!\n\n");
	kGAMECLASS.enemyAI();
}
...
//		doNext(getGame().combatMenu);
	...
	//Blind
	if(player.statusAffects.has("Blind")) {
		Render.text("You attempt to dirt kick, but as blinded as you are right now, you doubt you'll have much luck!  ", false);
	}
	else Render.text("Spinning about, you drag your footpaw through the dirt, kicking a wave of debris towards " + monster.a + monster.short + "!  ");
	//Dodged!
	if(rand(20) + 1 + monster.spe/20 > 15 + player.stats.spe/20) {
		Render.text(monster.mf("He","She") + " manages to shield " + monster.mf("his","her") + " eyes.  Damn!\n\n");
		kGAMECLASS.enemyAI();
		return;
	}
	else if(monster.statusAffects.has("Blind")) {
		Render.text(monster.mf("He","She") + "'s already blinded.  What a waste.\n\n");
	}
	else {
		Render.text(monster.mf("He","She") + "'s blinded!\n\n");
...
//		doNext(getGame().combatMenu);
	...
	//Blind
	if(player.statusAffects.has("Blind")) {
		Render.text("You attempt to hit with a vicious blow to the side, but as blinded as you are right now, you doubt you'll have much luck!  ", false);
	}
	else Render.text("You make a wide swing to the side, hoping to stun your foe!  ");
	let damage:number;
	//Determine if dodged!
...
//		doNext(getGame().combatMenu);
	...
	if(player.statusAffects.has("Sealed") && player.statusAffects.get("Sealed").value2 == 0) {
		Render.text("You attempt to attack, but at the last moment your body wrenches away, preventing you from even coming close to landing a blow!  The seals have made normal attack impossible!  Maybe you could try something else?\n\n", false);
		kGAMECLASS.enemyAI();
		return;
	}
	//Blind
	if(player.statusAffects.has("Blind")) {
		Render.text("You attempt to make a high, vaulting attack, but as blinded as you are right now, you doubt you'll have much luck!  ", false);
	}
	else Render.text("You leap into the air, intent on slamming your " + player.weaponName + " into your foe!  ");
	let damage:number;
	//Determine if dodged!
	if((player.statusAffects.has("Blind") && rand(2) == 0) || (monster.spe - player.stats.spe > 0 && int(Math.random()*(((monster.spe-player.stats.spe)/4)+80)) > 80)) {
		if(monster.spe - player.stats.spe < 8) Render.text(monster.capitalA + monster.short + " narrowly avoids your attack!", false);
...
public function urtaBeatsUpSiriusRadio():void {
	...
	//Beaten via HP:
	else Render.text("Your latest blow lands with a crunch on his jaw and he collapses bodily onto the ground at your feet, clearly out cold.");
...
public function urtaLosesToSirriusSnakeRadio():void {
	...
	//Loss by HP:
	if(player.HP < 1) Render.text("You cry out in pain as the bruises and buffets you've taken overwhelm you.  You struggle to stay upright, but to no avail; your injured body has reached its limits and you collapse, helpless and at the mercy of your assailant.");
	//Loss by Lust:
	else Render.text("With a hollow moan of anguish, you cast aside your halberd and fall to your knees, tears stinging your eyes as you desperately jerk and maul at your huge horse-prick.  The cursed thing has betrayed you again, and now you're too addled by lust and need to defend yourself from whatever this naga has in store for you.");
...
public function winRapeHyenaPrincess():void {
	...
	if(monster.HP < 1) Render.text(".  In spite of her wounded state, she seems to be quite excited by the situation");
	Render.text(".");
...
public function useSuccubiMilkOnGnollPrincesses():void {
	...
	if(flags[FlagEnum.URTA_FUCKED_HEL] == 1) Render.text("that salamander slut's cunt");
	else Render.text("a mino-cum addicted whore's sodden cunt");
	Render.text(".  As you start to pick up the pace, bucking your hips into the thick cushion of her rump, the gnoll princess starts mewling pathetically, panting and moaning like a little slut.");
...
public function loseToGnollPrincessAndGetGangBanged():void {
	...
	if(player.lust > 99) {
		Render.text("Sinking to your knees, utterly overwhelmed by lust, you gaze up the clit-cocked amazon, licking your chops. She responds by disdainfully pushing you down with one of her soft, padded paws.  \"<i>Did you really think you could trespass in my mother's territory and go unpunished?</i>\"");
	...
	//Lose Via HP✓
		...
		Render.text("Flopping down to the ground, you struggle to rise.  You're badly hurt, and your muscles barely move at this point.  With a pain-wracked spasm, you roll onto your back, unable to even rise to your knees.  The gnoll smirks and proclaims, \"<i>You'll probably make a good slave once we've broken you in.</i>\"");
...
private function loseToGnollAsUrtaEpilogue():void {
	...
	//Loss Epilogue✓
	//[HP]
	if(player.HP < 1) Render.text("Ouch... what happened to you?  You blink a few times, ignoring the buzzing throb of pain that rises up like a dormant monster in the back of your head.");
	//[Lust]
	else Render.text("Oh, gods... what happened to you?  You blink a few times, ignoring the gentle throb coming from your well-used member.");
	Render.text("  There's a constant, loud thrum of many voices talking all at once that makes it hard to think, and when you finally come to focus in the dim lighting, you can see why.  There are dozens, if not hundreds, of gnolls here, assembled in the faint light of some kind of feast hall, most of them male.");
	...
	if(player.HP < 1) Render.text("I hope I wasn't too rough on you");
	else Render.text("I hope you enjoyed the journey here");
	Render.text(".</i>\"");
	...
	Render.text("\"<i>Good girl,</i>\" she says encouragingly.  ");
	if(player.HP >= 1) Render.text("You shudder at the memory of your meeting, leaking a little more lubricant.  ");
	Render.text("As if by magic, a knife appears in the gnoll princess' hand, and she swiftly lays into the bindings holding your feet.  With your feet free, you can flex your legs a little.  They ache from being immobile so long.  Just how long were you hanging there, anyway?  The gnoll princess - it bears to reason she can only be a princess, considering how she is dressed compared to the rest of the gnolls here - rises up on her paws to reach the leather around your arm.  Quickly, she slashes through both restraints.  There's a moment of panic when you begin to fall; you can't possibly catch yourself on your weakened legs.  The princess secures you easily as she discards the knife.  \"<i>Did you think I'd let my favorite piece of property get damaged?</i>\"");
...
public function urtaSubmitsToMinotaurBadEnd():void {
	...
	Render.text("  Your once foe removes his loincloth to fully expose the mammoth between his legs, three feet long and nearly twice as girthy as your own.  You're so dazed by his imposing manhood, that you totally miss him flinging his loincloth at you.  It smacks wetly into your face, smothering you in his syrupy spooge.");
...
public function milkyUrtaTic():void {
	...
	else Render.text("Those orbs full of milky goodness are starting to look more attractive by the minute... if you don't finish this quickly you might end up giving up!");
	Render.text("\n\n");
...
public function urtaBeatsUpCowcubi():void {
	...
	//HP:
	if(monster.HP < 1) {
		Render.text("The succubus trips and falls on her face, dropping her whip at your feet.  You quickly take her whip and use it to bind her hands behind her back.");
	...
	//Lust:
		...
		Render.text("The succubus drops her whip and begins masturbating furiously, jilling her dripping cunt with abandon.  You quickly run towards her, planting your foot on her back and sending her tumbling on her face, then you use her whip to bind her hands behind her back.");
...
public function urtaLosesToCowCubi():void {
	...
	else Render.text("you start pulling at your [armor], struggling with every strap and piece of restraining, constricting leather.  Your lust-clouded mind can barely focus to work your fumbling fingers against the snaps and straps.  In your haste, with balancing forgotten, you tumble onto your face, smearing your dick exquisitely between your fallen form and a patch of soft moss.  You mewl pitifully in protest.  You need to fuck!");
	...
	if(player.HP < player.maxHP()) Render.text(" and healing your wounds");
	Render.text(".  Your own nipples have long since become fully engorged, taut black tit-caps, pebbly and sensitive.  They drag up and down the fat bottoms of the demoness's swollen underbust with every swallow and gasping breath you take.");
...
private function urtaArrivesAtCampForFukks():void {
	...
	//if genderless:
	if(player.gender == 0) Render.text("seeing as how you can't make babies");
	else Render.text("if you'd rather not");
	Render.text(", I can always go and use Edryn...</i>\"");
	...
	if(player.gender > 0) Render.text(" fuck the horny fox and help her out of her predicament, or");
	Render.text(" send her to Tel'Adre to find a suitable partner, like Edryn?");
	/*if(player.lowerBody.cockSpot.hasCock()) {
		if(player.cockThatFits(urtaCapacity()) < 0) Render.text("  A shame you're too big to fit inside her.");
	}*/
	...
	if(player.lowerBody.cockSpot.hasCock()) addButton(0,"KnockUrta",knockUpUrtaWithGodChild);
	if(player.lowerBody.vaginaSpot.hasVagina()) addButton(1,"KnockMe",getKnockedUpWithGodChild);
...
private function knockUpUrtaWithGodChild():void {
	...
	if(player.lowerBody.cockSpot.count() == 1) Render.text(" your own");
	else Render.text(" [oneCock]");
	Render.text(".  The nub of her clit is a diamond-hard button that drags across your sensitive " + player.skin() + ", aggressively smashed into your genitals.  Still, somehow, she seems to be holding back, probing you with some last vestige of sanity for your consent.");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text(", the mess made worse by your multiple-cocked virility.  There's so much that the clear juices roll off you to either side, puddling in your blankets below");
	Render.text(".  Her nipples dig into yours");
	if(player.upperBody.chest.hasFuckableNipples()) {
		Render.text(", kissing into the ");
		if(player.lactationQ() > 50) Render.text("milky, ");
		Render.text("wet holes on your tits, drawing fresh gasps of delight from your overwhelmed brain");
	}
	else Render.text(", shooting tingling excitement through your nubs as you grind against each other");
	Render.text(".  Urta's artless humping would be cringeworthy if she weren't so incredibly wet or overly tight.");
	...
	Render.text(" tints off-white from your liquid, squirting out of her tunnel faster than ever when she climaxes with you");
	if(player.cumQ() > 1000) Render.text(", belly rounded with wobbly, womb-packing spunk");
	Render.text(".");
...
private function postFuckUrtaUp():void {
	...
	if(player.statusAffects.has("DungeonShutDown")) Render.text(" - besides, given you know that you're nothing but a sacrifice the demons are too lazy to collect, it's not like they'll really send an invasion through");
	Render.text(".  Urta's joyous expression makes it quite clear that you chose the right choice.  The two of you gather your things, dress Urta in her clothes as best you can (adding a blanket for extra protection and modesty), ");
	if(camp.followersCount() >= 2) Render.text("leave instructions to your fellow camp-dwellers to look after the place, ");
	Render.text("and set off into the wasteland in the direction of Tel'Adre.  Urta needs to stop and rest on several occasions, weighed down by her sudden and highly advanced pregnancy, but she soldiers on without complaint, taking whatever support you give to her.  You can feel the strange god-child kicking incessantly inside her belly as you walk together.");
...
private function preggedUrtaWithGodChildEpilogue():void {
	...
	if(player.stats.cor > 66) Render.text(", what do you care, anyway?");
	else Render.text(".");
...
private function getKnockedUpWithGodChild():void {
	...
	if(player.wetness() >= 4) Render.text("wet ");
	Render.text("squish, she lands nose-deep in your vagina, immediately licking at it and tickling you with her smooth, slippery tongue.  \"<i>Mmmmm,</i>\" she purrs as she suckles your [clit].  Inching upward, the dripping wet fox-girl licks higher and higher, her tongue smearing across the " + player.skinFurScales() + " that covers your ");
	if(player.pregnancyIncubation > 0 && player.pregnancyIncubation < 150) Render.text("rounded ");
	Render.text("belly.  Urta's rigid onyx nipples tickle your [legs] as they drag upward, each immensely erect and engorged, just a little pebbly in texture.  She reaches your [chest], kissing up the ");
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) Render.text("soft curve");
	else Render.text("flat muscles");
	Render.text(" until she hits your [nipple].  ");
	if(player.upperBody.chest.hasFuckableNipples()) Render.text("Her tongue plunges inside, savoring the feminine lubricants");
	else Render.text("Her tongue plays across the surface, the heat of her breath shooting tingles up your spine");
	Render.text(".  The further she climbs, the further up your [legs] you can feel her stallion-cock rising, a trail of clear pre-cum in its wake.");
	...
	if(player.vaginalCapacity() < 60) Render.text("a little painfully");
	else Render.text("deliciously");
	Render.text(".  Her hands roam over the rest of your body with unrestrained ardor, groping and squeezing, pulling and tweaking.   Meanwhile, her hips smack against your own, smushing her sheath up against your puffy lips, her steamy sex dripping girl-lube down the back of her balls into a thick puddle on your blanket.  SLAP!  The sweaty nutsack slaps loud enough to be heard all over as Urta crudely fucks you, humping you artlessly, her hips beating out a confused, staccato rhythm.");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text(", your own forgotten length");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" laying on your stomach, bouncing fitfully but ultimately ignored.");
	}
	else Render.text(", the slippery wetness of your [vagina] and the sheer hardness of your [clit] overwhelming.");
	Render.text("  This feels better than it really ought to, perhaps in part thanks to your partner's copious fluids and supernaturally warm, feverish dick.");
	...
	if(player.lowerBody.balls > 0) Render.text("both sets of");
	else Render.text("her");
	Render.text(" balls, slathering them with slick spunk.");
	...
	//{Bimbo Sophie ball worship here}
		...
		if(player.lowerBody.balls > 0) Render.text("  She does the same to your own [balls], even giving your [asshole] a happy, sloppy kiss before returning to polishing the supple, skin-shrouded spheres just ahead.");
		Render.text("  Helia seems thrilled to be egging you both on to new heights of pleasure, and when Urta turns back to you with a blissful smile on her face, you realize she's enjoying it too.");
	}
	//{Latexy}
	else if(choice == 4) {
	...
	if(player.tailType > TAIL.NONE) Render.text(", though getting your tail angled right to allow her entrance is a bit of a challenge");
	else if(player.isTaur()) Render.text(", though getting down on your hands and forelegs is a bit challenging for you");
	else Render.text(", [face] buried in your blankets soaked with Urta's musky fluid");
	Render.text(".");
	...
	if(player.wetness() >= 5) Render.text("Lady-spunk showers out of your spasming twat, glazing Urta's bouncy ballsack with your lusty liquid.");
	else if(player.wetness() >= 3) Render.text("Lady-spunk drools out of your spasming twat in thick streams, glazing Urta's bouncy ballsack with your liquid lust.");
	else if(player.wetness() >= 1) Render.text("Lady-spunk drips from your spasming twat in steady trickles, glazing Urta's bouncy ballsack with your liquid lust.");
	Render.text("  The hungry mouth between her legs wastes no time in cleaning them.");
	Render.text("  Again and again, muscular contractions wiggle through your canal, wringing Urta's swollen staff of impregnation, milking it for more, even though your womb is STILL slightly distended from her last deposit.");
...
private function getKnockedUpUrtaEpilogue():void {
	...
	if(player.pregnancyIncubation > 0 && player.pregnancyIncubation < 150) Render.text("than before");
	else Render.text("- very pregnant");
	Render.text(".  Whatever Urta just put inside you, it's developing fast.  Fast enough to give you a wobbling gait and a straining midriff.  You give the snoring fox a prod.  Her only reaction is to swat at your finger with her fluffy tail and roll over snoring, so this time, you jab her hard, in the butt-cheek.  She yelps, sitting upright immediately.");
...
private function getKnockedUpByUrtaEpilogueII():void {
	...
	if(player.pregnancyIncubation > 0 && player.pregnancyIncubation < 150) Render.text("  Somehow, you're still just as pregnant with something as you were before.");
	Render.text("  You're filled with vigor at the touch, the stress of the birth fading immediately.  Even your poor, tortured vagina tingles, the pain fading.  You can feel the pregnant gape receding, as if the birth never happened.");
...
private function urtaAndEdrynGodChildEpilogueII():void {
	...
	Render.text("horse-girl.");
	if (kGAMECLASS.telAdre.edryn.pregnancy.isPregnant) Render.text("  Just how did she get pregnant again when she was already knocked up?  ...Magic, maybe.");
	Render.text("  Urta leads you around a bend, squeezing your hand for comfort.  \"<i>Thank you for everything, " + player.short + ", most of all being so understanding.</i>\"  The gray-furred fox pushes past the sixth pair of guards you've seen since entering the tower, just inside to see Edryn splayed on the floor, groaning and heaving, her ");
	if (kGAMECLASS.telAdre.edryn.pregnancy.isPregnant) Render.text("massively ");
	Render.text("pregnant flanks rippling with muscular contractions.   Something is bulging against her nether-lips, stretching the massive horse-cunt wider and wider with each passing moment.");
	...
	if (kGAMECLASS.telAdre.edryn.pregnancy.isPregnant) Render.text("  The centaur guardswoman is still just as pregnant with your offspring as before.  Your child is unharmed by whatever just transpired.");
	Render.text("  Edryn's eyes immediately close, sinking into a deep, restful sleep.");
