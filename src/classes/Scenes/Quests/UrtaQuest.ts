...
export default class UrtaQuest extends NPCAwareContent {
 ...
 */
	...
	public let urtaQItems1:ItemSlotClass = new ItemSlotClass();
	public let urtaQItems2:ItemSlotClass = new ItemSlotClass();
	public let urtaQItems3:ItemSlotClass = new ItemSlotClass();
	...
	public urtaBusy(): boolean {
		return (telAdre.edryn.pregnancy.type == PregnancyType.TAOTH || Flags.list[FlagEnum.URTA_QUEST_STATUS] == -1);
	}
	public isUrta(): boolean {
		if (Flags.list[FlagEnum.URTA_QUEST_STATUS] > 0 && Flags.list[FlagEnum.URTA_QUEST_STATUS] < 1) {
			if (player.short == "Urta") {
				if (player.lowerBody.cockSpot.hasCock()) {
					if (player.lowerBody.cockSpot.get(0).cockLength == 20 && player.lowerBody.cockSpot.get(0).cockThickness == 3) {
						if (player.lowerBody.vaginaSpot.hasVagina()) {
							if (player.upperBody.chest.get(0).breastRating == 7) return true;
						}
					}
				}
			}
	...
	private comfortUrtaAfterFamFamTalk(): void {
		MainScreen.clearText();
		MainScreen.text("You lean over the table and take Urta's hands in your own.  Her downy fur ruffles as you give her a comforting squeeze, the tension oozing from her body at your touch.  Urta sniffles a little and wipes her moistened cheek on her shoulder, muttering, \"<i>Oh, " + player.short + ", you're too good for me.</i>\"  She pulls back and gives you a knowing smile.  \"<i>You don't have to worry, really.  You turned this down on her luck lush into a happy woman.</i>\"");
		...
		if (urta.urtaDrunk()) MainScreen.text("boozy ");
		MainScreen.text("kiss on your lips as she hugs you tight - tight enough that you feel every vein through the concealing silk of her outfit.  \"<i>I'm a lucky girl.</i>\"");
	...
	private yeeshUrtaAfterFamFamTalk(): void {
		MainScreen.clearText();
		MainScreen.text("You loudly exhale, \"<i>Yeesh,</i>\" with a bemused expression.");
	...
	private whoCaresUrtaAfterFamFamTalk(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>Who cares?</i>\" you ask, without a hint of compassion.");
	...
	public infertilityQuestions(): void {
		MainScreen.clearText();
		MainScreen.text("You ask her about her infertility - specifically, how someone could be infertile AND unable to use transformative items without adverse effects.  Urta stirs her drink before shrugging noncommittally.");
		...
		MainScreen.text("\n\nWell, ");
		if (player.stats.cor < 33) MainScreen.text("you have nothing to complain about there.  Your sexual romps with Urta are always a delight.");
		else if (player.stats.cor < 66) MainScreen.text("she's definitely a handful in the sack, you've got to give her that.");
		else MainScreen.text("she's a willing fuck, so no complaints there.  Now if she would just get on with it - if you wanted to flirt, you would've instigated it.");
		...
		MainScreen.addButton(1, "Maybe Later", camp.returnToCampUseOneHour);
	}
	...
	public startUrtaQuest(): void {
		...
		player.short = "Urta";
		player.tallness = 71;
		player.upperBody.head.hairColor = "gray";
		player.upperBody.head.hairLength = 12;
		player.skinType = SkinType.FUR;
		player.skinAdj = "silky";
		player.skinDesc = "fur";
		player.createBreastRow();
		player.upperBody.chest.get(0).breastRating = 7;
		...
		player.lowerBody.cockSpot.get(0).cockType = CockType.HORSE;
		player.lowerBody.cockSpot.get(0).cockLength = 20;
		player.lowerBody.cockSpot.get(0).cockThickness = 3;
		player.lowerBody.balls = 2;
		player.lowerBody.ballSize = 4;
		...
		player.lowerBody.butt.analLooseness = 2;
		player.statusAffects.add(new StatusAffect("BonusVCapacity", 58, 0, 0, 0)));
		player.createVagina();
		player.lowerBody.vaginaSpot.get(0).vaginalWetness = VaginaWetness.DROOLING;
		player.lowerBody.vaginaSpot.get(0).vaginalLooseness = VaginaLooseness.NORMAL;
		...
		MainScreen.clearText();
		Flags.list[FlagEnum.URTA_QUEST_STATUS] = .75;
		MainScreen.text("<b>You are Urta, a hermaphroditic fox-morph.</b>");
	...
	private towerOfTheCovenantII(): void {
		...
		if (Flags.list[FlagEnum.HEL_FUCKBUDDY] > 0) {
			MainScreen.addButton(0, "Stop4ADrink", getHelBangedAsUrta);
		}
	...
	private getHelBangedAsUrta(): void {
		...
		if (Flags.list[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] > 0) MainScreen.text(", and even how they worked together to bring down an evil sorceress and save her father. Your heart swells with pride as she recounts " + player2.short + " doing so a noble thing, and for a moment you know you made the right pick in a lover..");
		MainScreen.text(".  You sit and listen for a long few minutes, sipping at the beer until the liquid courage starts to work its wonders, giving you a little tingle... and helping to open you up, too.  Soon, you're drawn into the conversation, joking and teasing with the other girls as if you'd been friends for ages.");
	...
	private leaveB4HelFucksUrta(): void {
		MainScreen.clearText();
		MainScreen.text("As quickly as you can, you blather out some excuse and grab your gear, trying to cover your huge erection as you run toward the door, blurting an awkward thanks over your shoulder for the drink...");
	...
	private fuckHelAndKitsunesAsUrta(): void {
		...
		//If PC has a cock:
		if (player2.hasCock()) {
			MainScreen.text("  \"<i>I've got a better one ");
			if (helFollower.followerHel()) MainScreen.text("at home");
			else MainScreen.text("at my beck and call");
			MainScreen.text(", you know...  But we've got that in common, don't we?  Mmm, maybe we can share it some day?  A horny girl like you'd just love that, woudn't you? Getting to worship " + player2.mf("his", "her") + " cock beside me while my tail fills your ass so full you scream....</i>");
		}
		...
		Flags.list[FlagEnum.URTA_FUCKED_HEL] = 1;
		player.orgasm();
		model.time.days++;
	...
	private visitPCPreUrtaQuest(truth: boolean): void {
		MainScreen.clearText();
		MainScreen.text("You set out for " + player2.short + "'s camp, known to you thanks to the amazing efforts of your scouts.  Behind you, the tower slowly shrinks, less imposing now that you've finished that step on your journey.   Ahead lies uncertainty and struggle.  You know you'll likely wind up fighting the corrupted denizens of the lost regions of Mareth in your travels, and it's likely at least a demon or two will get in your way.  Still, as you exit the city gates, you give your home a forlorn gaze.  At least there's one bright patch ahead - your lover's camp.");
		MainScreen.text("\n\nThe sun has set by the time you get there, but the darkness conceals your movements thanks to your natural fur color.  ");
		if (player.statusAffects.has("JojoNightWatch") && player.statusAffects.has("PureCampJojo"))
			MainScreen.text("You easily sneak past a mouse monk.  He's looking towards the sky mostly, perhaps watching for imps.  ");
		if (Flags.list[FlagEnum.ANEMONE_WATCH] > 0 && Flags.list[FlagEnum.ANEMONE_KID] > 0)
			MainScreen.text("A confused-looking anemone with a " + ItemType.lookupItem(Flags.list[FlagEnum.ANEMONE_WEAPON_ID]).longName + " nearly sees you, but you duck around a rock and escape her notice.  ");
		if (Flags.list[FlagEnum.FUCK_FLOWER_LEVEL] >= 4 && Flags.list[FlagEnum.FUCK_FLOWER_KILLED] == 0 && Flags.list[FlagEnum.HOLLI_DEFENSE_ON] > 0)
			MainScreen.text("A dangling tentacle nearly hits you in the dark.  You roll aside at the last moment, looking up to see dozens of the things idly dangling around.  When did this camp get such a corrupt tree in it?  You suppose it must have its uses in defending against foes from the sky...  ");
		if (camp.companionsCount() > 2) MainScreen.text("There are a number of people in the camp, but you avoid them as you head towards " + player2.short + "'s bunk.  ");
		MainScreen.text("The camp looks pretty nice actually.  Living out here must have given " + player2.mf("him", "her") + " plenty of time to improve it.");
		...
		if (truth) {
			...
			if (player2.cor < 33) MainScreen.text(player2.mf("he", "she") + " argues that " + player2.mf("he", "she") + " should come anyway.  You tell " + player2.mf("him", "her") + " no, firmly, even though it breaks your heart.");
			else if (player2.cor < 66) MainScreen.text(player2.mf("he", "she") + " nods understandingly, not surprised that you have to handle this quest alone.  The two of you are alike in that way - each with their own burden to bear.");
			else MainScreen.text(player2.mf("he", "she") + " nods dismissively, trusting you to handle it alone you suppose.  " + player2.short + " always was hard like that, even though " + player2.mf("he", "she") + " clearly likes you.");
			MainScreen.text("  You hold on to " + player2.mf("him", "her") + ", for comfort, dreading the long day ahead.  " + player2.mf("He", "She") + " pats your back comfortingly as you cry, and when you break from the hug, " + player2.mf("he", "she") + "'s looking at you expectantly.");
			if (Flags.list[FlagEnum.SLEEP_WITH] != "" && Flags.list[FlagEnum.SLEEP_WITH] != 0) MainScreen.text("  " + player2.short + " suggests the two of you move off, so as not to wake " + Flags.list[FlagEnum.SLEEP_WITH] + " in the night.");
		}
		//{LIE}
		else {
		...
		if (player2.hasVagina())
			MainScreen.addButton(1, "FuckHerVag", putUrtasWangInPCsCunt, truth);
		//Put a PC dick in Urta's vag(C)*
		if (player2.cockThatFits(urta.urtaCapacity()) >= 0)
			MainScreen.addButton(2, "Ride Dick", putAPCDickInUrtaCoochies, truth);
		//Put Urta dick in the PC's asshole [Cocked PCs](C)*
		if (player2.hasCock())
			MainScreen.addButton(3, "Fuck Ass", urtaPutsHerDickInPCsWithDicks, truth);
		//PC blows Urta(C)*
		MainScreen.addButton(4, "Get BJ", pcBlowsUrtasDong, truth);
	}
	...
	private urtaPutsHerDickInPCsWithDicks(truth: boolean): void {
		MainScreen.clearText();
		MainScreen.text("You push " + player2.short + " down onto the ground, glad that " + player2.mf("he", "she") + "'s not wearing " + player2.mf("his", "her") + " gear right now.  Unfortunately, you've still got to get out of yours, an action made all the more difficult by the heat welling up in your groin.  Your rebellious cock is already half-stiff, lifting the hem of your armored skirt away from your thighs.  Fingers scrabbling, you struggle with your chestpiece's bindings.  Your nipples stiffen, scraping against the inside of it while you fight to remove it.  You straddle " + player2.short + "'s waist to hold " + player2.mf("him", "her") + " down while you work the bothersome, restraining material free, practically grinding your semi-rigid dick against " + player2.mf("his", "hers") + " in excitement.  " + player2.mf("His", "Her") + " hands begin aiding you in your task, allowing your tightly compressed breasts to finally, happily breathe.   They jiggle as they flop free, and to your delight, " + player2.short + "'s eyes lock on them.");
		MainScreen.text("\n\nGrabbing hold of " + player2.mf("his", "her") + " head, you pull " + player2.short + "'s lips to one of your puckered nipples.  " + player2.mf("He", "She") + " swallows it immediately, grabbing hold of your tits to squeeze and fondle them, openly enjoying the fruits of your womanly shape.  Tingles of electric enjoyment fire from your unrestrained areola when " + player2.mf("he", "she") + " squeezes down on the other with " + player2.mf("his", "her") + " fingers, expertly twisting the little nub around to give you the maximum pleasure.  In no time flat, you're humping " + player2.mf("his", "her") + " " + player2.CockDescriptor.describeCock(player, player2.biggestCockIndex()) + " openly, letting " + player2.mf("him", "her") + " guide your motions with deft, controlling touches to your onyx nipples.  Your pre-cum bubbles out unhindered, the thick gobs swaying and splattering on " + player2.mf("his", "her") + " belly and rigid tool, quickly lubricating the phallic embrace into a sticky, hot mess.");
		MainScreen.text("\n\n" + player2.short + " releases your tender tits after a few particularly sloppy thrusts.  You can feel " + player2.mf("his", "her") + " " + player2.CockDescriptor.describeCock(player, player2.biggestCockIndex()) + " releasing some of its own pre-cum onto your cock's thick spooge-vein, and " + player2.mf("his", "her") + " hands fall to grab at the blanket below, unintentionally clenching at the waves of pleasure you're giving " + player2.mf("him", "her") + ".  You bend now, onto all fours");
		if (player2.tallness < 60) MainScreen.text(" and let your tits hang over " + player2.mf("his", "her") + " " + player2.face());
		else if (player2.tallness < 76) MainScreen.text(" and let your tits swing between you");
		else if (player2.tallness < 84) MainScreen.text(" and let your tits smash into " + player2.mf("his", "her"));
		else MainScreen.text(" and let your tits sway over " + player2.mf("his", "her") + " belly");
		MainScreen.text(", giving " + player2.mf("him", "her") + " something to enjoy while you begin to shift position, dragging your dick down " + player2.mf("his", "her") + " length");
		if (player2.hasVagina()) MainScreen.text(", past " + player2.mf("his", "her") + " pussy,");
		MainScreen.text(" and towards the prize you've been wanting since you got here - " + player2.mf("his", "her") + " asshole.");
		...
		if (player2.analCapacity() > 70) MainScreen.text("  Thankfully, " + player2.mf("he", "she") + "'s nice and stretched from all the butt-fucking " + player2.mf("he", "she") + "'s had to take, and you easily pop inside.  It feels so good that your flare grows wider, pulling you deeper into that welcoming, tight anal heat.");
		else if (player2.analCapacity() > 50) MainScreen.text("  Thankfully, " + player2.mf("he", "she") + "'s got plenty of give in " + player2.mf("his", "her") + " back-door, and you pop inside after applying a little more pressure.  It feels so good that your flare grows wider, slowing your descent into that tight, anal heat.");
		else MainScreen.text("  Unfortunately, there's not much give in " + player2.mf("his", "her") + " back-door, and you have to sit there smearing pre over it, again and again, each time pushing a little harder.  After what feels like the fiftieth try, you finally pop your fat flare into " + player2.mf("his", "her") + " rectum, the tight anal heat causing you to flare from the force of " + player2.mf("his", "her") + " pressure.  Slowly, you push inch after throbbing inch of your swollen, stallion-like erection into " + player2.mf("his", "her") + " asshole.");
		if (player2.ass.analLooseness == 0) {
			MainScreen.text("  <b>You've taken " + player2.mf("his", "her") + " anal virginity!</b>");
			player2.ass.analLooseness = 1;
		}
		MainScreen.text("  " + player2.short + " writhes as " + player2.mf("he", "she") + " takes you, " + player2.mf("his", "her") + " hips lifting slightly off the ground to make sure you grind into " + player2.mf("his", "her") + " prostate.  A long strand of clear excitement oozes from " + player2.mf("his", "her") + " tip");
		if (player2.cockTotal() > 1) MainScreen.text("s");
		MainScreen.text(", thick, viscous, and clinging to the drying mess you left behind.");
		...
		if (player2.cockTotal() > 1) MainScreen.text("s");
		MainScreen.text(" between your bodies, " + player2.mf("his", "her") + " liquid love exploding all over your cock-locked forms.  Each time you cum, a fresh-squeezed spurt of semen splatters into your tits, " + player2.mf("his", "her") + " prostate violently being milked by the heady flows of vixen-spunk you force inside " + player2.mf("his", "her") + " rectum.");
		if (player2.hasVagina()) MainScreen.text("  You don't notice the female fluids that drip from her sodden slit - they soak into your big balls, already forgotten.");
		...
		if (player2.cumQ() >= 500) {
			MainScreen.text(", mixing with " + player2.mf("his", "her") + " own ");
			if (player2.cumQ() < 1000) MainScreen.text("puddly");
			else if (player2.cumQ() < 2500) MainScreen.text("small lake-sized");
			else MainScreen.text("massive, lake-sized");
			MainScreen.text(" mess - only now do you realize just how much your lover could produce!  The");
		}
		else MainScreen.text(", but the");
		MainScreen.text(" semen-inundated fabric quickly begins to dry as the parched, wasteland dirt reclaims the moisture.");
	...
	private putAPCDickInUrtaCoochies(truth: boolean): void {
		...
		if (player2.gender == 3) MainScreen.text("...well, close enough... ");
		MainScreen.text("to ever fuck you like a woman.  Grabbing " + player2.mf("his", "her") + " face, you pull " + player2.mf("him", "her") + " into a deep, passionate kiss, unthinkingly grinding your dripping cock and steadily-wettening gash into " + player2.mf("his", "her") + " belly and rubbing your own pebbly onyx nipples against " + player2.mf("his", "her") + " nipples.  " + player2.mf("His", "Her") + " own " + player2.CockDescriptor.describeCock(player, x) + " begins to drool precum");
		if (player2.hasVagina()) MainScreen.text(", her cunt dripping wetly onto the ground from the stimulus");
		MainScreen.text(", the fluid rolling down its length to puddle with yours on " + player2.mf("his", "her") + " belly and filling your writhing, passionate embrace with hot, sticky sweetness.");
		...
		if (player2.isTaur() || player2.isDrider()) {
			...
			if (player2.cumQ() < 1000) MainScreen.text("falling face-first into the jizz-mud");
			else MainScreen.text("not even bothering trying to move your swollen bulk");
			MainScreen.text(", and you smile dopily up at " + player2.mf("him", "her") + ", love shining in your eyes.  Carefully, " + player2.mf("he", "she") + " positions " + player2.mf("him", "her") + "self beside you and strokes your face before closing " + player2.mf("his", "her") + " eyes, making it quite clear " + player2.mf("he", "she") + " intends to go to sleep right here.  You see no point in fighting the inevitable and instead give yourself over to slumber - wouldn't be the first time you woke up in dried cum-mud, after all...");
		}
		//Else:
		else {
			...
			if (player2.hasVagina()) MainScreen.text(", her female jizz spattering unnoticed onto the ground beneath the pair of you");
			MainScreen.text(".");
			...
			if (player2.cumQ() >= 1000) MainScreen.text(" not to mention the weight of your overstuffed belly");
			MainScreen.text(" you don't really feel inclined to move, either.  Instead, you yawn loudly, make yourself comfortable upon your lover's stomach, and allow yourself to drift off to sleep - already, you can hear the quiet breathing of " + player2.short + " as " + player2.mf("he", "she") + " does the same thing.");
		}
		player.orgasm();
		player.stats.sens += -2;
		menu();
	...
	private putUrtasWangInPCsCunt(truth: boolean): void {
		MainScreen.clearText();
		MainScreen.text("You push " + player2.short + " down on her back, a task made all the more difficult by your growing arousal.  Thankfully, she's not wearing her usual " + player2.armorName + ".  Instead, her form is bared to your probing fingertips, pliant and exposed.  You cannot help but dip a finger into her moist delta, parting the oozing veil to enjoy the view of her ");
		if (player.lowerBody.vaginaSpot.get(0).vaginaType != 5) MainScreen.text("pink, ");
		MainScreen.text("pulsing interior.  The sight is exciting in the extreme, immediately sending your heartbeat through the roof.  Your tremendous horse-cock is immediately stiffened by your ardor, along with your coal-black nipples, and your armor-plated skirt begins to rise, as if by magic.");
		...
		if (player2.pregnancyIncubation > 0 && player.pregnancyIncubation < 150) MainScreen.text("the curve of her pregnant belly");
		else if (player2.tone >= 65) MainScreen.text("her toned abdomen");
		else if (player2.tone < 30) MainScreen.text("her soft, forgiving middle");
		else MainScreen.text("her belly");
		MainScreen.text(".  The attentions drive your attempts to dislodge the offending garments from your lusty frame to new levels.  Metal rivets slowly give before finally snapping apart to free your left shoulder.  Your left nipple tingles with pleasant, palpable relief, and you turn to the other side, intent on ripping it off, so that you can be with your lover, skin to skin.");
		...
		//{centaur}
		if (player2.isTaur()) {
			MainScreen.text("and climb off the 'taur-woman.  She smirks up at you, until you bid her to rise, telling her that you plan to mount her like the fertile filly she is. Reluctantly, " + player2.short + " clatters up onto her hooves.");
			if (player2.tailType > TailType.NONE) MainScreen.text("  You stroke her tail as you shift it aside");
			else MainScreen.text("  You stroke her swollen sex as you admire it");
			MainScreen.text(", making sure you've got the perfect view of her lube-slimed horse-cunt.  Unbidden, your cock thickens, disgorging a potent trail of animal-pre down its underside in anticipation.");
			if (player2.wetness() >= 4) MainScreen.text("  Fuck, she's wetter than Edryn!  Just look at those heavy trails of slick moisture as they run down her rump and hindlegs.");
			else if (player2.wetness() >= 2) MainScreen.text("  Fuck, she's as wet as Edryn!  Beads of moisture are forming on her clit, ready to drop to the ground.");
			else MainScreen.text("  Fuck, she's almost as wet as Edryn!  Just getting a sniff of her horny horse-snatch has your head swimming!");
			MainScreen.text("  You give it a lick and shudder, taking in a few deep lungfuls of her scent.");
			if (player2.tallness >= 84) MainScreen.text("  Before you know what you're doing, you've pulled her back to a rock and climbed atop it, bringing your cock level with that heavenly hole.");
			else MainScreen.text("  Before you know what you're doing, you've raised yourself up to your full height, bringing your cock level with that heavenly hole.");
		}
		//{goo}
		else if (player2.isGoo()) {
			MainScreen.text("and roll the gooey woman so she's face down.  She holds herself up off the dry dirt with her palms, sweet, slimy bottom upraised above her goopy undercarriage.  Amazingly, there's a slight parting in the mass of slime, right between where her legs would be, if she had them.  A narrow, barely visible divide can be spotted inside, capped with the telltale bulge of her ");
			if (player2.clitLength < .5) MainScreen.text("tiny ");
			else if (player2.clitLength >= 2.5) MainScreen.text("massive ");
			MainScreen.text("button.  You nuzzle up to that slick slime-cunt, sniffing at her super-wet snatch, taking the pheromones as deeply inside yourself as possible.  Her femininity is so strong that it makes your head spin and your dick ache.  Glancing down as you lick the moisture from your muzzle, you see the rigid, over-engorged veins standing out in harsh relief on your equine tool.  Your legs seem to move on their own, lining you up with the moisture-laden slit just inches away.");
		}
		//{nagas}
		else if (player2.isNaga()) {
			MainScreen.text("and roll the snake-woman over so she's prostrate on the ground, barely holding her front half up with her arms.  Her loins are flat on the ground, undulating slightly as she grinds her moist clit against it for sensation.  Your lover's anatomy poses a problem for your choice of positions, but then, inspiration strikes.  You come up under the hind part of her snake-tail, and hoisting it up, you support half over your shoulder.  As expected, she has all the flexibility of the serpent she so resembles, and her tail bends down just far enough for her cunt to hang a mere inch above your rigid tool, right where her body changes from snake to person.");
			if (player2.wetness() >= 3) MainScreen.text("  Her moisture drizzles from that wanton slit, basting you with her fragrant femininity, and you have a hard time not taking her here and now.");
			MainScreen.text("  The tail you're holding up coils in, sliding around your arm and middle, wrapping you up in snakey snugness.  With impeccable control, she squeezes a little bit, bringing her cunt close enough to breathe foggy moisture on your equine tool.");
		}
		//{biped}
		else {
			MainScreen.text("and roll her over on her hands and knees, lifting that sweet butt up in the air where you can admire it.  You kneel behind her and, pressing your nose into her fragrant quim, you extend your tongue, letting the smooth oral muscle taste her juices and tease at her engorging clit.  " + player2.short + " moans and presses back into you, crying out like a true bitch in heat, one in need of a thorough rutting.  The thought hits your cock like a lightning bolt, and it fills out to painful proportions, every vein and bulge standing out in stark relief, thumping obscenely to the tune of your insatiable lust.  You moan into her lovely-tasting snatch, gulping down ");
			if (player2.wetness() < 3) MainScreen.text("every drop of nectar that she gives up");
			else if (player2.wetness() < 4) MainScreen.text("the copious drops of nectar she gives up");
			else if (player2.wetness() < 5) MainScreen.text("the drizzling moisture that oozes from her twat");
			else MainScreen.text("the rivers of girl-lube that run unimpeded from her sopping twat");
			MainScreen.text(".  You sniff in great lungfuls of her, your head swimming with need, and before you know it, you've risen behind her, your oozing flare just inches from that delectable opening.");
		}
		...
		if (player2.pregnancyIncubation > 0 && player2.pregnancyIncubation < 150) MainScreen.text(" (more than it already is)");
		MainScreen.text(" from your prodigious deposit.  The illusion of knocking her up takes hold of you, and your orgasm redoubles, fresh waves of horse-cum pouring out of you unceasingly.  " + player2.short + " moans, rubbing her taut");
		if (player2.pregnancyIncubation > 0 && player2.pregnancyIncubation < 150) MainScreen.text(" pregnant");
		MainScreen.text(" dome, until at last you finally spend the last of your spunk.");
	...
	private snuggleWithUrta(truth: boolean): void {
		MainScreen.clearText();
		MainScreen.text("You think it over and decide that, for once, you'd like to focus more on love and less on lust.  \"<i>Well... I'd really just like to have a snuggle with you, " + player2.short + " - if that's alright?</i>\"  You ask quietly, tail instinctively wagging in an effort to be endearing.  Your lover nods " + player2.mf("his", "her") + " head and answers in the positive, opening " + player2.mf("his", "her") + " arms for further effect.  You waste no time and eagerly sidle into " + player2.mf("his", "her") + " arms.");
		//[If PC Centaur or Drider:
		if (player2.isTaur() || player2.isDrider()) {
			MainScreen.text("  It's rather awkward, thanks to " + player2.mf("his", "her") + " clunky ");
			if (player2.isTaur()) MainScreen.text("centaur");
			else MainScreen.text("drider");
			MainScreen.text(" body, but you manage to press up against " + player2.mf("his", "her") + " ");
			if (player2.isTaur()) MainScreen.text("forehoofs");
			else MainScreen.text("first two pairs of spidery limbs");
			MainScreen.text(".");
		}
		//[if PC Naga:]
		else if (player2.isNaga()) MainScreen.text("  " + player2.mf("His", "Her") + " serpentine coils loop snugly around your waist, curling around your legs and holding you gently in place.");
		//[if PC Goo:
		else if (player.lowerBody.isGoo()) MainScreen.text("  " + player2.mf("His", "Her") + " soft, squishy goo-body eagerly accepts you, the gelatinous mass of " + player2.mf("his", "her") + " lower torso reaching out to softly envelop you in its cool, wet embrace.");
		//[Else:
		else MainScreen.text("  Knees to knees, it takes you a few moments to figure out how to properly position your legs, but you eventually manage it.");
		...
		if (player2.cor < 33) MainScreen.text("playful pat");
		else if (player2.cor < 66) MainScreen.text("gentle stroke");
		else MainScreen.text("lascivious caress");
		MainScreen.text(" before pulling you tighter into " + player2.mf("his", "her") + " embrace.  Eager to forget your embarrassment, you focus on holding " + player2.mf("him", "her") + " tight and close.  Your prick slowly slides back into its sheath from the lack of stimulus as you focus on how safe and content you feel to be here, being held like this.");
	...
	private morningAfterCampVisitEmbark(truth: boolean): void {
		...
		model.time.days++;
		statScreenRefresh();
		MainScreen.text("Sadly, morning comes, and your time with " + player2.short + " comes to an end.  You push " + player2.mf("his", "her") + " shoulder gently to rouse " + player2.mf("him", "her") + ", smiling at " + player2.mf("him", "her") + " when " + player2.mf("he", "she") + " wakes.  You kiss " + player2.mf("him", "her") + " on the lips before rolling out from under the blanket.  The two of you eat a quiet breakfast together, simply enjoying each other's company.  ");

		if (truth) {
			MainScreen.text("" + player2.mf("He", "She") + " asks again about coming with you, but you tell " + player2.mf("him", "her") + " Gul's instructions were quite specific.  You'll need to tackle this alone.  The rest of breakfast passes in uncomfortable silence, broken when you rise to leave.");
		}

		MainScreen.text("\n\nYou embrace " + player2.short + ", kissing " + player2.mf("him", "her") + " on the lips");

		if (truth) {
			MainScreen.text(" before you promise, \"<i> I'll come back to you, stronger and ready for kids, I promise.</i>\"\n\n");
		}
		else {
			MainScreen.text(".  ");
		}

		MainScreen.text(player2.short + " nods, and you leave " + player2.mf("him", "her") + " behind.  You keep glancing over your shoulder as you leave, praying silently to any god who'll listen that you return safely to your lover.");
	...
	public winFuckAGoblinBroodmotherAsUrta(): void {
		MainScreen.clearText();
		MainScreen.text("You greedily eye the ");
		if (monster.HP < 1) MainScreen.text("submissive");
		else MainScreen.text("feverishly masturbating");
		MainScreen.text(" form of your opponent and can't resist licking your black lips in anticipation - any guilt about fucking someone besides " + player2.short + " that might make it through the haze of lust consuming you is quickly extinguished by your conviction that " + player2.mf("he", "she") + " would understand.  Your growing tower of fuck-meat is already lifting the hem of your armored skirt, making it even easier to grab its lower edge and flip it up out of the way as you advance towards your assailant, eager to see what you can do about blowing off some steam...");
	...
	private loseToGoblinsPartIIAsUrta(): void {
		MainScreen.clearText();
		MainScreen.text("<b>Hours, and many orgasms later...</b>");
	...
	private loseToGoblinsPartIIIAsUrta(): void {
		MainScreen.clearText();
		MainScreen.text("<b>Several days later...</b>");
	...
	private urtaGameOver(): void {
		MainScreen.clearText();
		MainScreen.text("<b>Urta has been lost to her fate...  Meanwhile, back at camp...</b>");
		Game.inCombat = false;
		Flags.list[FlagEnum.URTA_QUEST_STATUS] = -1;
		model.time.days++;
		resetToPC();
		model.time.hours = 6;
		statScreenRefresh();
	...
	public urtaSpecials(): void {
		//Gone	menuLoc = 3;
		if (Game.inCombat && player.statusAffects.has("Sealed") && player.statusAffects.get("Sealed").value2 == 5) {
			MainScreen.clearText();
		...
		MainScreen.addButton(3, "Dirt Kick", urtaDirtKick);
		MainScreen.addButton(4, "Metabolize", urtaMetabolize);
		MainScreen.addButton(5, "SecondWind", urtaSecondWind);
	...
	private urtaMetabolize(): void {
		...
		MainScreen.text("You work your body as hard as you can, restoring your fatigue at the cost of health. (" + damage + ")\nRestored 20 fatigue!\n\n");
		fatigue(-20);
		kGAMECLASS.enemyAI();
	}
	...
	private urtaSecondWind(): void {
		...
		if (monster.statusAffects.has("UrtaSecondWinded")) {
			MainScreen.text("You've already pushed yourself as hard as you can!");
			//Gone		menuLoc = 3;
		...
		monster.statusAffects.add(new StatusAffect("UrtaSecondWinded", 0, 0, 0, 0)));
		HPChange(Math.round(player.maxHP() / 2), false);
		fatigue(-50);
		player.stats.lust += -50;
		MainScreen.text("Closing your eyes for a moment, you focus all of your willpower on pushing yourself to your absolute limits, forcing your lusts down and drawing on reserves of energy you didn't know you had!\n\n");
		kGAMECLASS.enemyAI();
	}

	//Combo: 3x attack, higher miss chance, guaranteed hit vs blind
	private urtaComboAttack(): void {
		if (!player.statusAffects.has("Attacks")) {
			MainScreen.clearText();
			if (player.fatigue + 25 > 100) {
				MainScreen.text("You are too fatigued to use that attack!");
				//Gone			menuLoc = 3;
				//			doNext(Game.combatMenu);
				menu();
				MainScreen.addButton(0, "Next", kGAMECLASS.combatMenu, false);
		...
		if (!player.statusAffects.has("Attacks"))
			player.statusAffects.add(new StatusAffect("Attacks", 3, 0, 0, 0)));
	else {
			player.addStatusValue(StatusAffects.Attacks, 1, -1);
			trace("DECREMENDED ATTACKS");
			if (player.statusAffects.get("Attacks").value1 <= 1) {
				trace("REMOVED ATTACKS");
				player.statusAffects.remove("Attacks");
			}
		}
		//Blind
		...
		let damage: number;
		//Determine if dodged!
		if (monster.findStatusAffect(StatusAffects.Blind) < 0 && (rand(3) == 0 || (player.statusAffects.has("Blind") && rand(2) == 0) || (monster.spe - player.stats.spe > 0 && int(Math.random() * (((monster.spe - player.stats.spe) / 4) + 80)) > 80))) {
	...
	private urtaDirtKick(): void {
		...
		if (player.fatigue + 5 > 100) {
			MainScreen.text("You are too fatigued to use that ability!");
			//Gone		menuLoc = 3;
			//		doNext(Game.combatMenu);
			menu();
			MainScreen.addButton(0, "Next", kGAMECLASS.combatMenu, false);
		...
		//Blind
		if (player.statusAffects.has("Blind")) {
			MainScreen.text("You attempt to dirt kick, but as blinded as you are right now, you doubt you'll have much luck!  ", false);
		}
		else MainScreen.text("Spinning about, you drag your footpaw through the dirt, kicking a wave of debris towards " + monster.a + monster.short + "!  ");
		//Dodged!
		if (rand(20) + 1 + monster.spe / 20 > 15 + player.stats.spe / 20) {
			MainScreen.text(monster.mf("He", "She") + " manages to shield " + monster.mf("his", "her") + " eyes.  Damn!\n\n");
			kGAMECLASS.enemyAI();
			return;
		}
		else if (monster.statusAffects.has("Blind")) {
			MainScreen.text(monster.mf("He", "She") + "'s already blinded.  What a waste.\n\n");
		}
		else {
			MainScreen.text(monster.mf("He", "She") + "'s blinded!\n\n");
			monster.statusAffects.add(new StatusAffect("Blind", 2 + rand(3))), 0, 0, 0);
		}
		kGAMECLASS.enemyAI();
	}
	...
	private urtaSidewinder(): void {
		...
		//Blind
		if (player.statusAffects.has("Blind")) {
			MainScreen.text("You attempt to hit with a vicious blow to the side, but as blinded as you are right now, you doubt you'll have much luck!  ", false);
		}
		else MainScreen.text("You make a wide swing to the side, hoping to stun your foe!  ");
		let damage: number;
		//Determine if dodged!
	...
	private urtaVaultAttack(): void {
		...
		if (player.statusAffects.has("Sealed") && player.statusAffects.get("Sealed").value2 == 0) {
			MainScreen.text("You attempt to attack, but at the last moment your body wrenches away, preventing you from even coming close to landing a blow!  The seals have made normal attack impossible!  Maybe you could try something else?\n\n", false);
			kGAMECLASS.enemyAI();
			return;
		}
		//Blind
		if (player.statusAffects.has("Blind")) {
			MainScreen.text("You attempt to make a high, vaulting attack, but as blinded as you are right now, you doubt you'll have much luck!  ", false);
		}
		else MainScreen.text("You leap into the air, intent on slamming your " + player.weaponName + " into your foe!  ");
		let damage: number;
		//Determine if dodged!
		if ((player.statusAffects.has("Blind") && rand(2) == 0) || (monster.spe - player.stats.spe > 0 && int(Math.random() * (((monster.spe - player.stats.spe) / 4) + 80)) > 80)) {
			if (monster.spe - player.stats.spe < 8) MainScreen.text(monster.capitalA + monster.short + " narrowly avoids your attack!", false);
		...
		//Determine if critical hit!
		let crit: boolean = false;
		if (monster.statusAffects.has("Stunned") || rand(100) <= 4 || (player.perks.has("Tactician") && player.stats.int >= 50 && (player.stats.int - 50) / 5 > rand(100))) {
			crit = true;
			damage *= 2;
		}
		//Start figuring enemy damage resistance
	...
	private leaveSiriusBehind(): void {
		MainScreen.clearText();
		MainScreen.text("You decide that he's just not worth your time and leave to continue your journey.");
		menu();
	...
	private fuckSiriusRadioII(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>Oof... ugh... just... just one more... yeah!</i>\"  You gasp, moan, and shudder as your cock explodes yet again in your lover's mouth.  The snake-man is sprawled on the ground, supporting a belly like a centauress full-term with twins, and doesn't stir as you finally go limp and pull out of his mouth, drooling cum all over his face.");
	...
	public urtaLosesToSirriusSnakeRadio(): void {
		...
		//Loss by HP:
		if (player.HP < 1) MainScreen.text("You cry out in pain as the bruises and buffets you've taken overwhelm you.  You struggle to stay upright, but to no avail; your injured body has reached its limits and you collapse, helpless and at the mercy of your assailant.");
		//Loss by Lust:
		else MainScreen.text("With a hollow moan of anguish, you cast aside your halberd and fall to your knees, tears stinging your eyes as you desperately jerk and maul at your huge horse-prick.  The cursed thing has betrayed you again, and now you're too addled by lust and need to defend yourself from whatever this naga has in store for you.");
	...
	private gnollAlphaBitchIntro(): void {
		MainScreen.clearText();
		//Intro*
		MainScreen.text("The last two fights were wearying, but you still feel pretty good. Perhaps best of all, as your last defeated challenger fades into the distance, you become aware of the pendant glowing fairly brightly.  You must be headed in the right direction!  Setting off at a trot, you pick up your pace, covering great distance in the blink of an eye, eyes never leaving the rapidly approaching horizon.  Focus is key to travel in the uninhabited sections of Mareth, as you well know, but it also allows you to move with swiftness surpassing the fastest animal.");
	...
	public winRapeHyenaPrincess(): void {
		...
		if (monster.HP < 1) MainScreen.text(".  In spite of her wounded state, she seems to be quite excited by the situation");
		MainScreen.text(".");
	...
	public useSuccubiMilkOnGnollPrincesses(): void {
		...
		if (Flags.list[FlagEnum.URTA_FUCKED_HEL] == 1) MainScreen.text("that salamander slut's cunt");
		else MainScreen.text("a mino-cum addicted whore's sodden cunt");
		MainScreen.text(".  As you start to pick up the pace, bucking your hips into the thick cushion of her rump, the gnoll princess starts mewling pathetically, panting and moaning like a little slut.");
	...
	public loseToGnollPrincessAndGetGangBanged(): void {
		...
		if (player.stats.lust > 99) {
			MainScreen.text("Sinking to your knees, utterly overwhelmed by lust, you gaze up the clit-cocked amazon, licking your chops. She responds by disdainfully pushing you down with one of her soft, padded paws.  \"<i>Did you really think you could trespass in my mother's territory and go unpunished?</i>\"");
		...
		//Lose Via HP✓
			...
			MainScreen.text("Flopping down to the ground, you struggle to rise.  You're badly hurt, and your muscles barely move at this point.  With a pain-wracked spasm, you roll onto your back, unable to even rise to your knees.  The gnoll smirks and proclaims, \"<i>You'll probably make a good slave once we've broken you in.</i>\"");
	...
	private loseToGnollAsUrtaEpilogue(): void {
		...
		//Loss Epilogue✓
		//[HP]
		if (player.HP < 1) MainScreen.text("Ouch... what happened to you?  You blink a few times, ignoring the buzzing throb of pain that rises up like a dormant monster in the back of your head.");
		//[Lust]
		else MainScreen.text("Oh, gods... what happened to you?  You blink a few times, ignoring the gentle throb coming from your well-used member.");
		MainScreen.text("  There's a constant, loud thrum of many voices talking all at once that makes it hard to think, and when you finally come to focus in the dim lighting, you can see why.  There are dozens, if not hundreds, of gnolls here, assembled in the faint light of some kind of feast hall, most of them male.");
		...
		if (player.HP < 1) MainScreen.text("I hope I wasn't too rough on you");
		else MainScreen.text("I hope you enjoyed the journey here");
		MainScreen.text(".</i>\"");
		...
		MainScreen.text("\"<i>Good girl,</i>\" she says encouragingly.  ");
		if (player.HP >= 1) MainScreen.text("You shudder at the memory of your meeting, leaking a little more lubricant.  ");
		MainScreen.text("As if by magic, a knife appears in the gnoll princess' hand, and she swiftly lays into the bindings holding your feet.  With your feet free, you can flex your legs a little.  They ache from being immobile so long.  Just how long were you hanging there, anyway?  The gnoll princess - it bears to reason she can only be a princess, considering how she is dressed compared to the rest of the gnolls here - rises up on her paws to reach the leather around your arm.  Quickly, she slashes through both restraints.  There's a moment of panic when you begin to fall; you can't possibly catch yourself on your weakened legs.  The princess secures you easily as she discards the knife.  \"<i>Did you think I'd let my favorite piece of property get damaged?</i>\"");
	...
	private loseToGnollAsUrtaEpilogueBAM(): void {
		MainScreen.clearText();
		MainScreen.text("Ouch!  Something smacks your face, and it fucking hurts!  You look up, seeing a strong, spotted paw pulling away, trailing a web of spooge in its wake.");
	...
	private urtaSleepsArmored(): void {
		MainScreen.clearText();
		model.time.days++;
		MainScreen.text("You go to sleep wearing your armor.  It turns out to have been a good idea when you're woken by tentacles probing at your chest-plate, seeking to undo the numerous straps holding it in place.  You roll to the side and easily squeeze out of the slippery tendrils.  Your hand falls on your halberd with practiced ease, hefting its reassuring weight in your hands as you spring up off your paws.");
	...
	private urtaFinishesWithBigOrange(): void {
		...
		Flags.list[FlagEnum.URTA_TENTACLE_GAPED] = 1;
		Flags.list[FlagEnum.URTA_TENTACLE_RAPED] = 1;
		player.orgasm();
		player.stats.lib += 1;
		player.stats.sens += 2;
		menu();
	...
	private urtaFinishesWithTentacles(): void {
		MainScreen.clearText();
		MainScreen.text("Sighing, you pinch your nipples as your ride the dozens of tentacles, letting them completely envelop your body.  The lips that have teased your cock all this time sluggishly shift position, pressing back against your tip and slowly opening.  With sloth-like slowness, those soft lips encircle the fat, flat head of your horse-dick.  As the tight seal drifts down your length, you get your first taste of the mouth's interior.  The whole of the tentacle seems hollow and lubricated with some internal moisture.  Most interestingly of all, the walls are covered with wiggly little cilia, thousands of them.  Your over-aroused member is caressed and tickled from sheath to tip, buttoned up in the moist confines of the tentacle while it's plump and juicy lips are smacking against your balls.");
	...
	public urtaSubmitsToMinotaurBadEnd(): void {
		MainScreen.clearText();
		MainScreen.text("You mouth opens, drooling with hunger that you know only the sexy beast across from you can sate.");
		if (monster.statusAffects.has("MinotaurEntangled")) MainScreen.text("   Seeing the fire in your eyes change from a determined glare to a lusty look, the minotaur pulls you over, carefully unwinding the chain from around you, so as not to damage you.");
		MainScreen.text("  Your once foe removes his loincloth to fully expose the mammoth between his legs, three feet long and nearly twice as girthy as your own.  You're so dazed by his imposing manhood, that you totally miss him flinging his loincloth at you.  It smacks wetly into your face, smothering you in his syrupy spooge.");
	...
	private submissiveMinoUrtaBadEndPtII(): void {
		MainScreen.clearText();
		MainScreen.text("You wake up feeling so blissfully bloated and utterly full.  Memories of your encounter with the minotaur come streaming back.  You submitted to the beast, and it felt... it felt goood.  Your cunny clenches at the memory, and your rigid, oozing pole pulses needily below.  You burp, tasting the salty leavings in the back of your throat for the first time.  That was good!  Gradually, you become aware of a gentle forward and back motion, making your distended gut slosh and your tits sway pendulously.");
	...
	public beatMinoLordOnToSuccubi(): void {
		MainScreen.clearText();
		MainScreen.text("The succubus claps slowly as she gets up from her perch on a ruined column.  \"<i>I'm impressed.</i>\"  She says, stepping in the light as she walks towards you.  You gasp, this succubus looks like a cow!");
		...
		//{If Urta raped Fido:}
		if (Flags.list[FlagEnum.URTA_RAPED_MINO_LORD] > 0) {
	...
	public milkyUrtaTic(): void {
		MainScreen.text("Your body heats up with the effects of the succubus-cow milk...  ");
		if (player.stats.lust < 50) MainScreen.text("Thankfully it hasn't become too distracting yet.");
		else if (player.stats.lust < 75) MainScreen.text("You don't know how much longer you'll be able to withstand the effects of the milk.");
		else MainScreen.text("Those orbs full of milky goodness are starting to look more attractive by the minute... if you don't finish this quickly you might end up giving up!");
		MainScreen.text("\n\n");
		player.addStatusValue(StatusAffects.MilkyUrta, 1, -1);
		if (player.statusAffects.get("MilkyUrta").value1 <= 0) player.statusAffects.remove("MilkyUrta");
		player.stats.lust += 10;
	}
	...
	public urtaBeatsUpCowcubi(): void {
		...
		//HP:
		if (monster.HP < 1) {
			MainScreen.text("The succubus trips and falls on her face, dropping her whip at your feet.  You quickly take her whip and use it to bind her hands behind her back.");
		...
		//Lust:
			...
			MainScreen.text("The succubus drops her whip and begins masturbating furiously, jilling her dripping cunt with abandon.  You quickly run towards her, planting your foot on her back and sending her tumbling on her face, then you use her whip to bind her hands behind her back.");
	...
	private fuckTheCowCubi(): void {
		MainScreen.clearText();
		MainScreen.text("\"<i>Come on girl, strip down so I can see what you're hiding under that leather skirt of yours.  Besides, we both know you want me, I can see that tent,</i>\" she laughs, licking her lips seductively.");
	...
	public urtaLosesToCowCubi(): void {
		...
		if (player.HP < player.maxHP()) MainScreen.text(" and healing your wounds");
		MainScreen.text(".  Your own nipples have long since become fully engorged, taut black tit-caps, pebbly and sensitive.  They drag up and down the fat bottoms of the demoness's swollen underbust with every swallow and gasping breath you take.");
	...
	private deadGodsDreaming(): void {
		MainScreen.clearText();
		MainScreen.text("Your eyes spring open and you stare in dumbfounded shock at the statue as it starts to move, joints flexing with the awful sound of stone grating on stone.  Carved eyelids open to reveal flames of myriad colors, ever-shifting before your eyes.  \"<i>Welcome, vessel.  It is time that we spoke.</i>\"");
	...
	private urtaArrivesAtCampForFukks(): void {
		MainScreen.clearText();
		MainScreen.text("<b><u>Meanwhile, back at camp...</u></b>");
		...
		//if genderless:
		if (player.gender == 0) MainScreen.text("seeing as how you can't make babies");
		else MainScreen.text("if you'd rather not");
		MainScreen.text(", I can always go and use Edryn...</i>\"");
		...
		if (player.gender > 0) MainScreen.text(" fuck the horny fox and help her out of her predicament, or");
		MainScreen.text(" send her to Tel'Adre to find a suitable partner, like Edryn?");
		/*if(player.lowerBody.cockSpot.hasCock()) {
			if(player.cockThatFits(urtaCapacity()) < 0) MainScreen.text("  A shame you're too big to fit inside her.");
		}*/
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(0, "KnockUrta", knockUpUrtaWithGodChild);
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.addButton(1, "KnockMe", getKnockedUpWithGodChild);
		MainScreen.addButton(2, "Edryn", urtaAndEdrynGodChild);
	...
	private knockUpUrtaWithGodChild(): void {
		...
		if (player.lowerBody.cockSpot.count() == 1) MainScreen.text(" your own");
		else MainScreen.text(" [oneCock]");
		MainScreen.text(".  The nub of her clit is a diamond-hard button that drags across your sensitive " + player.skin() + ", aggressively smashed into your genitals.  Still, somehow, she seems to be holding back, probing you with some last vestige of sanity for your consent.");
		...
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(", the mess made worse by your multiple-cocked virility.  There's so much that the clear juices roll off you to either side, puddling in your blankets below");
		MainScreen.text(".  Her nipples dig into yours");
		if (player.upperBody.chest.hasFuckableNipples()) {
			MainScreen.text(", kissing into the ");
			if (player.lactationQ() > 50) MainScreen.text("milky, ");
			MainScreen.text("wet holes on your tits, drawing fresh gasps of delight from your overwhelmed brain");
		}
		else MainScreen.text(", shooting tingling excitement through your nubs as you grind against each other");
		MainScreen.text(".  Urta's artless humping would be cringeworthy if she weren't so incredibly wet or overly tight.");
		...
		MainScreen.text(" tints off-white from your liquid, squirting out of her tunnel faster than ever when she climaxes with you");
		if (player.cumQ() > 1000) MainScreen.text(", belly rounded with wobbly, womb-packing spunk");
		MainScreen.text(".");
	...
	private postFuckUrtaUp(): void {
		...
		if (player.statusAffects.has("DungeonShutDown")) MainScreen.text(" - besides, given you know that you're nothing but a sacrifice the demons are too lazy to collect, it's not like they'll really send an invasion through");
		MainScreen.text(".  Urta's joyous expression makes it quite clear that you chose the right choice.  The two of you gather your things, dress Urta in her clothes as best you can (adding a blanket for extra protection and modesty), ");
		if (camp.followersCount() >= 2) MainScreen.text("leave instructions to your fellow camp-dwellers to look after the place, ");
		MainScreen.text("and set off into the wasteland in the direction of Tel'Adre.  Urta needs to stop and rest on several occasions, weighed down by her sudden and highly advanced pregnancy, but she soldiers on without complaint, taking whatever support you give to her.  You can feel the strange god-child kicking incessantly inside her belly as you walk together.");
	...
	private preggedUrtaWithGodChildEpilogue(): void {
		...
		if (player.stats.cor > 66) MainScreen.text(", what do you care, anyway?");
		else MainScreen.text(".");
	...
	private getKnockedUpWithGodChild(): void {
		MainScreen.clearText();
		MainScreen.text("Smiling coyly, you remove your [armor] and crawl into your bed, beckoning for the lust-crazed fox to follow.  You trust her, and you're more than willing to let her put a nice little bun in your");
		if (player.isPregnant() && player.pregnancyIncubation < 150) MainScreen.text(" already filled");
		MainScreen.text(" oven.  Her green eyes bulge obscenely at the sight of your naked flesh, locked tight to every curve of your shape.  The vixen takes one shuffling step after another.  It's clear from her crazed panting and trembling fingertips that she's trying to keep herself from jumping you with limited success.  She's like an addict eyeing a cornucopia stuffed with her favorite drugs, knowing that she must take it slow but helpless against her desire to gluttonously gorge.");

		MainScreen.text("\n\nYou toss a little fuel on the fire by spreading your legs");
		if (player.lowerBody.balls > 0) MainScreen.text(" and lifting your balls");
		MainScreen.text(", exposing your puffy lips, and spreading them wide to expose your ");
		if (player.lowerBody.vaginaSpot.get(0).vaginaType != 5) MainScreen.text("pink, ");
		else MainScreen.text("dusky, ");
		MainScreen.text("glistening interior.  Urta whines, high pitched and keening, and she doubles over, pumping her cock with both hands, unashamedly masturbating at the sight of your exposed, vulnerable womanhood.  She grunts, swinging her hips lewdly back and forth, actually fucking her hands now, mouth open and panting, \"<i>Gonna... gonna fuck ya!  Fullofbabies!</i>\"");
		...
		if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 4) MainScreen.text("wet ");
		MainScreen.text("squish, she lands nose-deep in your vagina, immediately licking at it and tickling you with her smooth, slippery tongue.  \"<i>Mmmmm,</i>\" she purrs as she suckles your [clit].  Inching upward, the dripping wet fox-girl licks higher and higher, her tongue smearing across the " + player.skinFurScales() + " that covers your ");
		if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 150) MainScreen.text("rounded ");
		MainScreen.text("belly.  Urta's rigid onyx nipples tickle your [legs] as they drag upward, each immensely erect and engorged, just a little pebbly in texture.  She reaches your [chest], kissing up the ");
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) MainScreen.text("soft curve");
		else MainScreen.text("flat muscles");
		MainScreen.text(" until she hits your [nipple].  ");
		if (player.upperBody.chest.hasFuckableNipples()) MainScreen.text("Her tongue plunges inside, savoring the feminine lubricants");
		else MainScreen.text("Her tongue plays across the surface, the heat of her breath shooting tingles up your spine");
		MainScreen.text(".  The further she climbs, the further up your [legs] you can feel her stallion-cock rising, a trail of clear pre-cum in its wake.");
		...
		if (player.vaginalCapacity() < 60) MainScreen.text("a little painfully");
		else MainScreen.text("deliciously");
		MainScreen.text(".  Her hands roam over the rest of your body with unrestrained ardor, groping and squeezing, pulling and tweaking.   Meanwhile, her hips smack against your own, smushing her sheath up against your puffy lips, her steamy sex dripping girl-lube down the back of her balls into a thick puddle on your blanket.  SLAP!  The sweaty nutsack slaps loud enough to be heard all over as Urta crudely fucks you, humping you artlessly, her hips beating out a confused, staccato rhythm.");
		...
		if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text(", your own forgotten length");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
			MainScreen.text(" laying on your stomach, bouncing fitfully but ultimately ignored.");
		}
		else MainScreen.text(", the slippery wetness of your [vagina] and the sheer hardness of your [clit] overwhelming.");
		MainScreen.text("  This feels better than it really ought to, perhaps in part thanks to your partner's copious fluids and supernaturally warm, feverish dick.");
		...
		if (player.lowerBody.balls > 0) MainScreen.text("both sets of");
		else MainScreen.text("her");
		MainScreen.text(" balls, slathering them with slick spunk.");

		let ballWorship: Array = [];
		if ((amilyScene.amilyFollower() && Flags.list[FlagEnum.AMILY_FOLLOWER] == 2) && jojoScene.campCorruptJojo()) ballWorship[ballWorship.length] = 0;
		if ((amilyScene.amilyFollower() && Flags.list[FlagEnum.AMILY_FOLLOWER] == 2) || jojoScene.campCorruptJojo()) ballWorship[ballWorship.length] = 1;
		...
		if (Flags.list[FlagEnum.VALARIA_AT_CAMP] == 1) ballWorship[ballWorship.length] = 5;
		let choice: number;
		...
		if (player.lowerBody.tailType > TailType.NONE) MainScreen.text(", though getting your tail angled right to allow her entrance is a bit of a challenge");
		else if (player.lowerBody.isTaur()) MainScreen.text(", though getting down on your hands and forelegs is a bit challenging for you");
		else MainScreen.text(", [face] buried in your blankets soaked with Urta's musky fluid");
		MainScreen.text(".");
		...
		if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 5) MainScreen.text("Lady-spunk showers out of your spasming twat, glazing Urta's bouncy ballsack with your lusty liquid.");
		else if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 3) MainScreen.text("Lady-spunk drools out of your spasming twat in thick streams, glazing Urta's bouncy ballsack with your liquid lust.");
		else if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 1) MainScreen.text("Lady-spunk drips from your spasming twat in steady trickles, glazing Urta's bouncy ballsack with your liquid lust.");
		MainScreen.text("  The hungry mouth between her legs wastes no time in cleaning them.");
		MainScreen.text("  Again and again, muscular contractions wiggle through your canal, wringing Urta's swollen staff of impregnation, milking it for more, even though your womb is STILL slightly distended from her last deposit.");
	...
	private getKnockedUpUrtaEpilogue(): void {
		MainScreen.clearText();
		MainScreen.text("When you wake, you're ");
		if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 150) MainScreen.text("much more ");
		MainScreen.text("pregnant ");
		if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 150) MainScreen.text("than before");
		else MainScreen.text("- very pregnant");
		MainScreen.text(".  Whatever Urta just put inside you, it's developing fast.  Fast enough to give you a wobbling gait and a straining midriff.  You give the snoring fox a prod.  Her only reaction is to swat at your finger with her fluffy tail and roll over snoring, so this time, you jab her hard, in the butt-cheek.  She yelps, sitting upright immediately.");
	...
	private getKnockedUpByUrtaEpilogueII(): void {
		...
		if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 150) MainScreen.text("  Somehow, you're still just as pregnant with something as you were before.");
		MainScreen.text("  You're filled with vigor at the touch, the stress of the birth fading immediately.  Even your poor, tortured vagina tingles, the pain fading.  You can feel the pregnant gape receding, as if the birth never happened.");
	...
	private urtaAndEdrynGodChildEpilogueII(): void {
		MainScreen.clearText();
		MainScreen.text("As the double doors open, you hear a painful moan from the ");
		//{pre-pregnant }

		MainScreen.text("horse-girl.");
		if (kGAMECLASS.telAdre.edryn.pregnancy.isPregnant) MainScreen.text("  Just how did she get pregnant again when she was already knocked up?  ...Magic, maybe.");
		MainScreen.text("  Urta leads you around a bend, squeezing your hand for comfort.  \"<i>Thank you for everything, " + player.short + ", most of all being so understanding.</i>\"  The gray-furred fox pushes past the sixth pair of guards you've seen since entering the tower, just inside to see Edryn splayed on the floor, groaning and heaving, her ");
		if (kGAMECLASS.telAdre.edryn.pregnancy.isPregnant) MainScreen.text("massively ");
		MainScreen.text("pregnant flanks rippling with muscular contractions.   Something is bulging against her nether-lips, stretching the massive horse-cunt wider and wider with each passing moment.");
		...
		if (kGAMECLASS.telAdre.edryn.pregnancy.isPregnant) MainScreen.text("  The centaur guardswoman is still just as pregnant with your offspring as before.  Your child is unharmed by whatever just transpired.");
		MainScreen.text("  Edryn's eyes immediately close, sinking into a deep, restful sleep.");
