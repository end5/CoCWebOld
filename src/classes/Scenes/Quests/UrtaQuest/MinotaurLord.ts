export default class MinotaurLord extends Monster {
	...
	private minotaurDrankMalk(): void {
		MainScreen.text("The minotaur lord snorts audibly and turns to look at his mistress.  \"<i>What is it, Fido, boy?  You thirsty?</i>\"  The hulking minotaur nods.");
		//Success:*
		if (statusAffects.get("MinoMilk").value1 < 3) {
			MainScreen.text("\"<i>Catch!</i>\"  The succubus throws a bottle containing a milky-white substance to the minotaur.  He grabs it and uncorks the bottle, quickly chugging its contents with obvious enjoyment.  After he is done he looks even more energetic and ready to fight, and his cock looks even harder!");
			addHP(300);
			lust += 10;
		...
		//Failure:*
			...
			MainScreen.text("\"<i>Well too bad!  We're all out of milk... but don't worry, my dear pet, I'll let you drink as much as you want after you're done with this bitch.</i>\"  The succubus replies, idly checking her elongated nails.");
	...
	private minotaurLordEntangle(): void {
		MainScreen.text("The minotaur lord lashes out with his chain, swinging in a wide arc!\n");
		statusAffects.add(new StatusAffect("Timer", 2 + rand(4))), 0, 0, 0);
		//{dodge/whatever}
		if (combatMiss() || combatEvade() || combatFlexibility() || combatMisdirect()) {
			MainScreen.text("You leap over the clumsy swing, allowing the chain to fly harmlessly underneath you!");
		}
		else {
			MainScreen.text("You try to avoid it, but you're too slow, and the chain slaps into your hip, painfully bruising you with the strength of the blow, even through your armor.  The inertia carries the back half of the whip around you, and in a second, the chain has you all wrapped up with your arms pinned to your sides and your movement restricted.");
	...
	private minotaurPrecumTease(): void {
		MainScreen.text("The minotaur smiles at you and lifts his loincloth, flicking it at you.  Thick ropes of pre-cum fly through the air in a swarm,");
		if (rand(2) == 0) {
			MainScreen.text(" slapping into your face before you can react!  You wipe the slick snot-like stuff out of your eyes and nose, ");
			if (player.stats.lust >= 70) MainScreen.text("swallowing it into your mouth without thinking.  You greedily guzzle the potent, narcotic aphrodisiac down, even going so far as to lick it from each of your fingers in turn, sucking every drop into your waiting gullet.");
			else MainScreen.text("feeling your heart hammer lustily.");
			kGAMECLASS.dynStats("lus", 15 + player.stats.lib / 8 + player.stats.sens / 8);
		}
		else {
			MainScreen.text(" right past your head, but the smell alone is enough to make you weak at the knees.");
			MainScreen.text("  The animalistic scent of it seems to get inside you, the musky aroma burning a path of liquid heat to your groin, stiffening your horse-cock to absurd degrees.");
			kGAMECLASS.dynStats("lus", 11 + player.stats.lib / 10);
		}
		//(1)
		if (player.stats.lust <= 75) MainScreen.text("  You shiver with need, wanting nothing more than to bury your face under that loincloth and slurp out every drop of goopey goodness.");
		else MainScreen.text("  <b>You groan and lick your lips over and over, craving the taste of him in your mouth.</b>");
		kGAMECLASS.combatRoundOver();
	}

	public defeated(hpVictory: boolean): void {
		game.MainScreen.clearText();
		MainScreen.text("The minotaur lord is defeated!  ");
		MainScreen.text("  You could use him for a quick fuck to sate your lusts before continuing on.  Do you?");
		game.menu();
