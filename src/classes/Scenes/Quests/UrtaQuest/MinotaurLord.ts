...
//{dodge/whatever}
		...
		private minotaurCumPress():void
		{
			...
			if (player.lust > 99) MainScreen.text("you rise up, horny and hungry for more.");
			else {
				...
				if (player.lust >= 80) MainScreen.text(" and drippy");
		...
		private minotaurPrecumTease():void
		{
			...
			if (rand(2) == 0) {
				...
				if (player.lust >= 70) MainScreen.text("swallowing it into your mouth without thinking.  You greedily guzzle the potent, narcotic aphrodisiac down, even going so far as to lick it from each of your fingers in turn, sucking every drop into your waiting gullet.");
...
//(1)
			if (player.lust <= 75) MainScreen.text("  You shiver with need, wanting nothing more than to bury your face under that loincloth and slurp out every drop of goopey goodness.");
