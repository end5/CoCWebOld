...
export default class Giacomo implements TimeAwareInterface {
	...
	public timeChange(): boolean {
		...
		if (model.time.hours > 23) {
			if (Flags.list[FlagEnum.CERULEAN_POTION_BAD_END_FUTA_COUNTER] > 0) Flags.list[FlagEnum.CERULEAN_POTION_BAD_END_FUTA_COUNTER] -= 0.5; //Reduce bad-end for cerulean herms number
		}
		return false;
	}
	...
	public timeChangeLarge(): boolean {
		if (checkedSuccubi++ == 0 && model.time.hours == 4 && player.statusAffects.has("SuccubiNight") && (player.lowerBody.cockSpot.hasCock() || player.gender == 0)) { //Call secksins!
			if (player.statusAffects.has("RepeatSuccubi")) {
				if (Game.vapula.vapulaSlave() && player.lowerBody.cockSpot.hasCock() && Flags.list[FlagEnum.VAPULA_THREESOMES] > 0 && Flags.list[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 0) //VapulaSurprise
					Game.vapula.vapulaAssistsCeruleanSuccubus();
				else nightSuccubiRepeat(); //Normal night succubi shit
			}
			else {
				nightSuccubiFirstTime();
				player.statusAffects.add(new StatusAffect("RepeatSuccubi", 0, 0, 0, 0)));
			}
			//Lower count if multiples stacked up.
			if (player.statusAffects.get("SuccubiNight").value1 > 1)
				player.statusAffects.get("SuccubiNight").value1 = -1;
			else player.statusAffects.remove("SuccubiNight");
			return true;
		}
		return false;
	}
	...
	public giacomoEncounter(): void {
		...
		if (kGAMECLASS.giacomo == 0) {
		...
		else if (!player.statusAffects.has("WormOffer") && player.statusAffects.has("Infested")) { //If infested && no worm offer yet
			MainScreen.text("Upon walking up to Giacomo's wagon, he turns to look at you and cocks an eyebrow in curiosity and mild amusement.\n\n");
			MainScreen.text("\"<i>Been playing with creatures best left alone, I see</i>,\" he chuckles.  \"<i>Infestations of any kind are annoying, yet your plight is quite challenging given the magnitude of corrupt creatures around here.  It is not the first time I have seen one infested with THOSE worms.</i>\"\n\n");
			MainScreen.text("You ask how he knows of your change and the merchant giggles heartily.\n\n");
			MainScreen.text("\"<i>Do not look at me as if I am a mystic,</i>\" Giacomo heckles lightly.  \"<i>Your crotch is squirming.</i>\"\n\n");
			MainScreen.text("Looking down, you realize how right he is and attempt to cover yourself in embarrassment.\n\n");
			MainScreen.text("\"<i>Fear not!</i>\" the purveyor jingles.  \"<i>I have something that will cure you of those little bastards.  Of course, there is also a chance that it will purge your system in general.  This potion is not cheap.  I will trade it for 175 gems.</i>\"\n\n");
			player.statusAffects.add(new StatusAffect("WormOffer", 0, 0, 0, 0)));
			if (player.stats.gems < 175) { //Broke as a joke
				MainScreen.text("You realize you don't have enough gems for such a pricey potion, but perhaps there is something else in his inventory you can buy.");
			}
			else { //Can afford
				MainScreen.text("Do you purchase his cure?");
				//Remove/No
			...
			MainScreen.text("You spy the merchant Giacomo in the distance.  He makes a beeline for you, setting up his shop in moments.  ");
			MainScreen.text("Giacomo's grin is nothing short of creepy as he offers his wares to you. What are you interested in?");
		}
		let deworm: Function = (player.statusAffects.has("WormOffer") && player.statusAffects.has("Infested") ? wormRemovalOffer : null);
	...
	private firstEncounter(): void {
		MainScreen.text("As you travel, you see another person on the road.  He is tethered to a small cart that is overloaded with a hodgepodge of items.  He is dressed in a very garish manner, having a broad, multicolored hat, brocaded coat and large, striped pantaloons.  His appearance is almost comical and contrasts with his severe and hawkish facial features.  The man sees you, smiles and stops his cart.\n");
		MainScreen.text("\"<i>Greetings, traveler! My name is Giacomo.  I am, as you can see, a humble purveyor of items, curios and other accoutrements.  While I am not in a position to show you my full wares as my shop is packed on this push-cart, I do offer some small trinkets for travelers I meet.</i>\"\n\n");
		MainScreen.text("The merchant looks at you sharply and cracks a wide, toothy smile you find... unnerving.  The merchant twists his way around to access a sack he has around his back.  After a moment, he swings the sack from his back to have better access to its contents.  Inquisitively, the merchant turns back to you.\n");
		MainScreen.text("\"<i>So stranger, be you interested in some drafts to aid you in your travels, some quick pamphlets to warn you of dangers on journeys or...</i>\"\n\n");
		MainScreen.text("Giacomo pauses and turns his head in both directions in a mocking gesture of paranoid observation.  His little bit of theatrics does make you wonder what he is about to offer.\n");
		MainScreen.text("\"<i>...maybe you would be interested in some items that enhance the pleasures of the flesh?  Hmmm?</i>\"\n\n");
		MainScreen.text("Giacomo's grin is nothing short of creepy as he offers his wares to you.  What are you interested in?");
	...
	private potionMenu(): void {
		spriteSelect(23);
		MainScreen.clearText();
		MainScreen.text("Which potion or tincture will you examine?");
	...
	private bookMenu(): void {
		spriteSelect(23);
		MainScreen.clearText();
		MainScreen.text("Which book are you interested in perusing?");
		simpleChoices("Dangerous Plants", pitchDangerousPlantsBook, "Traveler's Guide", pitchTravellersGuide, "Hentai Comic", pitchHentaiComic,
			"Yoga Guide", (Flags.list[FlagEnum.COTTON_UNUSUAL_YOGA_BOOK_TRACKER] > 0 ? pitchYogaGuide : null), "Back", giacomoEncounter);
		statScreenRefresh();
	}
	...
	private buyVitailtyTincture(): void {
		...
		if (player.stats.gems < 15) {
			MainScreen.clearText();
			MainScreen.text("\n\nGiacomo sighs, indicating you need " + String(15 - player.stats.gems) + " more gems to purchase this item.");
			doNext(potionMenu);
		}
		else {
	...
	private buyCeruleanPotion(): void {
		...
		if (player.stats.gems < 75) {
			MainScreen.clearText();
			MainScreen.text("\n\nGiacomo sighs, indicating you need " + String(75 - player.stats.gems) + " more gems to purchase this item.");
			doNext(potionMenu);
		}
		else {
	...
	private buyDangerousPlantsBook(): void {
		...
		if (player.stats.gems < 10) {
			MainScreen.text("\n\nGiacomo sighs, indicating you need " + String(10 - player.stats.gems) + " more gems to purchase this item.");
			doNext(bookMenu);
		}
		else {
	...
	private buyTravellersGuide(): void {
		...
		if (player.stats.gems < 1) {
			...
			MainScreen.text("The crazy merchant said you might not need this and he was right.  Written at a simple level, this was obviously intended for a city-dweller who never left the confines of their walls.  Littered with childish illustrations and silly phrases, the book is informative in the sense that it does tell a person what they need and what to do, but naively downplays the dangers of the forest and from bandits.  Were it not so cheap, you would be pissed at the merchant.  However, he is right in the fact that giving this to some idiot ignorant of the dangers of the road saves time from having to answer a bunch of stupid questions.");
			doNext(bookMenu);
			player.stats.gems -= 1;
	...
	private buyYogaGuide(): void {
		...
		if (player.hasKeyItem("Yoga Guide") >= 0) {
			MainScreen.text("<b>You already own a yoga guide!</b>");
		}
		else if (player.stats.gems < 100) {
			MainScreen.text("You cannot afford a yoga guide!");
		}
		else {
			MainScreen.text("You exchange 100 gems for the tome.  Now you can finally enjoy a workout with Cotton!");
	...
	private pitchDildo(): void {
		...
		if (player.hasKeyItem("Dildo") >= 0) {
			MainScreen.text("<b>You already own a Dildo!</b>");
			doNext(eroticaMenu);
			return;
		}
		MainScreen.text("Giacomo takes out a slender tube roughly over half a foot in length.  \"<i>Since you seek pleasure, this is as simple and effective as it gets.  This dildo is a healthy seven inches long and is suitable for most women and even adventurous men.  Pick a hole, stick it in and work it to your heart's content or your partner's pleasure.  The single-piece construction makes it solid, sturdy and straightforward.  For 20 gems, you can take matters into your own hands.  How about it?</i>\"");
	...
	private buyDildo(): void {
		...
		if (player.stats.gems < 20) {
			MainScreen.text("\n\nGiacomo sighs, indicating you need " + String(20 - player.stats.gems) + " more gems to purchase this item.");
			doNext(eroticaMenu);
		}
		else {
			MainScreen.text("After making the payment, Giacomo hands you the Dildo");
			doNext(eroticaMenu);
	...
	private buySelfStimulationBelt(): void {
		...
		if (player.stats.gems < 30) {
			MainScreen.text("\n\nGiacomo sighs, indicating you need " + String(30 - player.stats.gems) + " more gems to purchase this item.");
			doNext(eroticaMenu);
			return;
		}
		MainScreen.text("After making the payment, Giacomo hands you the Self-Stimulation Belt");
	...
	private buyAllNaturalSelfStimulationBelt(): void {
		...
		if (player.stats.gems < 40) {
			MainScreen.text("\n\nGiacomo sighs, indicating you need " + String(40 - player.stats.gems) + " more gems to purchase this item.");
			doNext(eroticaMenu);
			return;
		}
		MainScreen.text("After making the payment, Giacomo hands you the All-Natural Self-Stimulation Belt");
	...
	private buyOnahole(): void {
		...
		MainScreen.text("After making the payment, Giacomo hands you the Plain Onahole");
	...
	private buyDeluxeOnahole(): void {
		...
		if (player.stats.gems < 50) {
			MainScreen.text("\n\nGiacomo sighs, indicating you need " + String(50 - player.stats.gems) + " more gems to purchase this item.");
			doNext(eroticaMenu);
			return;
		}
		MainScreen.text("After making the payment, Giacomo hands you the Deluxe Onahole");
	...
	private buyAllNaturalOnahole(): void {
		...
		if (player.stats.gems < 150) {
			MainScreen.text("\n\nGiacomo sighs, indicating you need " + String(150 - player.stats.gems) + " more gems to purchase this item.");
			doNext(eroticaMenu);
			return;
		}
		MainScreen.text("After making the payment, Giacomo hands you the All-Natural Onahole");
	...
	private buyDualStimulationBelt(): void {
		...
		MainScreen.clearText();
		if (player.stats.gems < 50) {
			MainScreen.text("You do not have enough gems to purchase this item.");
		}
		else {
			MainScreen.text("You are a bit dubious at the pleasure it could offer you, but it would be better than being raped by the creatures constantly... maybe to even work out some excess lusts... hesitantly, you reach into your bag and grab 50 gems, handing it to him.  He greedily snatches it from your palm and hands you with the belt with a smile.  \"<i>I promise you won't be disappointed.</i>\"  He counts the gems and waves goodbye.\n\n(<b>Dual Belt acquired!</b>)");
	...
	private wormRemoval(): void {
		...
		MainScreen.text("Upon seeing the fat worm, Giacomo displays a freakish celerity by jumping off his cart, grabbing an empty container and collecting the fat worm. Regaining your senses, you look at him with inquisitive shock at what he just did.\n\n");
		MainScreen.text("\"<i>You have to realize that I AM a merchant, after all.</i>\", he calmly replies. \"<i>Hell for you is heaven for someone else. This bugger will easily fetch 10,000 gems to some noble looking for a quick buzz. Don't WE know better!</i>\"\n\n");
		MainScreen.text("The merchant puts away his prize and comes back to help you up. \"<i>Here.</i>\", he says as he shoves a couple of bottles into your hand. \"<i>This is on the house. You probably need it after the shock of getting those things out.</i>\"\n\n");
	...
	private wormRemovalOffer(): void {
		...
		if (player.stats.gems < 175) {
			MainScreen.text("You realize you don't have enough gems for such a pricey potion, but perhaps there is something else in his inventory you can buy.", false);
			doNext(giacomoEncounter);
		}
		//Can afford
		else {
			MainScreen.text("Do you purchase his cure?", false);
			//Remove/No
			doYesNo(wormRemoval, giacomoEncounter);
		}
	}
	...
	private nightSuccubiFirstTime(): void {
		...
		if (player.gender == 0) {
			MainScreen.text("She quickly moves down to your crotch... only to discover no organs down there.\n\n");
			MainScreen.text("*record scratch*\n\n");
			MainScreen.text("\"<i>Wait a fucking minute,</i>\" the Succubus says, \"<i>Where's your dick?!</i>\"\n\n");
			MainScreen.text("As you state your genderless nature, the succubus hops off and from nowhere pulls out a large folder marked \"<i>Corruption of Champions-Script</i>\" and begins thumbing through the pages.  After finding the page she is looking for, she reads it and looks off into the distance in disgust.\n\n");
			MainScreen.text("\"<i>Hey Fenoxo and Dxasmodeus!!!!!!</i>\" the Succubus crows, \"<i>The goddamn script says that I should be milking someone's DICK!!!  Man, futa, herm, I don't give a shit.  YOUR OWN FUCKING SCRIPT SAYS I SHOULD BE MOUNTING AND MILKING A COCK!!!!  THIS IS A SEX GAME!!!!!!  THAT MEANS FUCKING!  WHAT THE HELL AM I SUPPOSED TO FUCK???!!!</i>\"\n\n");
			MainScreen.text("The Succubus looks at you with utter contempt, \"<i>THIS motherfucker doesn't have a DAMN thing!  What am I supposed to do?!  I can't exactly order a fucking Happy Meal!!!!!</i>\"\n\n");
			MainScreen.text("Throwing the script down in an utter rage, the tantrum continues, \"<i>Goddammit!  I can't believe this shit!  HEY!!!!!  INTERN!!!!  Bring me my robe, aspirins and cancer sticks!!!!</i>\"\n\n");
			MainScreen.text("The Succubus walks a few paces away where a plain-dressed woman with a clipboard hands the Succubus a pack of cigarettes and a small bottle of aspirin.  She takes a fistful of the painkillers and immediately lights up a smoke.  The Succubus takes a couple of drags off the cig and rubs her temples.\n\n");
			MainScreen.text("\"<i>You two are killing me!</i>\" she groans in clear frustration, \"<i>I come to work for you perverts based off the promise of MORE perverts to feed from and you do THIS to me!  I can't work like this!</i>\"\n\n");
			MainScreen.text("The plain woman hands the Succubus a robe, which she crudely puts on as she storms off into the night.\n\n");
			MainScreen.text("\"<i>I will discuss this horseshit with my agent,</i>\" the Succubus continues bitching, \"<i>THIS was NOT in my contract.</i>\"\n\n");
			MainScreen.text("The Succubus stops, turns and points to you in derision.  \"<i>And YOU!  You no-cock, no-cunt having pissant!  Take your ass back to the lab before they find out you escaped!!!!!</i>\"\n\n");
			MainScreen.text("The Succubus resumes her stormy exit.  You look at the bottle of Cerulean Potion and wonder if it REALLY had some psychotropics in it.  What the hell just happened?!");
			Flags.list[FlagEnum.CERULEAN_POTION_NEUTER_ATTEMPTED] = 1;
			doNext(playerMenu);
			return;
		}
		if (player.gender == 1)
			MainScreen.text("\"<i>Awwww!  Did my blue skin and pointy teeth scare you?</i>\" she says in a childish voice.  \"<i>Believe me stud, if I wanted to harm you, I would not have let you wake up at all.  I am here because you have 'called' me.</i>\"  She teases you with the empty blue bottle you bought from the merchant.  \"<i>My essence is in this bottle.  Any man who drinks this, I am compelled to return the pleasure by drinking his.</i>\"  The demon woman reaches her skinny hand down to your crotch where you see you have become fiercely erect.  The demon gently strokes your cock until you begin oozing generous amounts of your own natural lubricants.  The demon takes one of her massive breasts and teases you with her fat nipples.  \"<i>Open your mouth,</i>\" she demands.  \"<i>Take me into your mouth as I will soon take you into mine.</i>\"\n\n");
			...
			MainScreen.text("\"<i>Well, well, well!</i>\" the Succubus jingles.  \"<i>What have we here?!  A little girl with a big cock!</i>\"\n\n");
			MainScreen.text("As the Succubus looks down at your " + CockDescriptor.describeCock(player, 0) + ", you have quickly achieved one of the healthiest erections you have ever had.  The succubus quickly poises her hairy hole over your member and allows her weight to force your dick into her womb.  The demoness rests her weight in her lap as she allows you to fully penetrate her.  Her womb is hot and wet and her muscles have your prick in one of the strongest grips imaginable.  Even if you went totally limp, withdrawal would be an impossibility.  Wincing at the sudden crushing force of her vaginal muscles, the succubus giggles inhumanly.\n\n");
			MainScreen.text("\"<i>Quit whimpering,</i>\" the Succubus orders.  \"<i>I hope the rumors about you futas are true.  I need a good, fiery load of cum to get me going.  I haven't had one in a while and as much as I LOVE men, they can only feed me so much.</i>\"\n\n");
			MainScreen.text("You quickly try to struggle, but find the Succubus to be utterly dominating.  She wraps her arms around your back and entwines her lean legs around your hips.  The Succubus playfully licks your lips and grins.\n\n");
			MainScreen.text("\"<i>You are getting your dick milked,</i>\" the Succubus says flatly, \"<i>Accept it.  Trust me, when I am done, you will want more of me, anyway.</i>\"\n\n");
			MainScreen.text("As the Succubus finishes her ultimatum, you feel churning vaginal contractions stroking your massive cock.  Heavy, powerful, coordinated undulations work your dick as surely as the best handjob.  You quickly moan in shock and pleasure at such rough treatment.");
		}
	...
	private ceruleanSuccubusEncounterPart3(): void {
		...
		spriteSelect(8);
		if (player.gender == 1) {
			MainScreen.text("Rotating herself into a 69 position, she seizes your throbbing member and effortlessly begins deep throating.  Her thighs wrap around your head and confront you with her surprisingly hairy pussy.  Her clitoris is long and erect, begging for attention and the smell of her pheromones enslaves you.  You bury your face into her furry mound, ignoring your normal revulsion to such an unshaved state and begin eating her as well as any woman you have ever pleased.  The demon takes your cock out of her mouth to cry in delight.  \"<i>YES, LITTLE MAN!</i>\" she screams.  \"<i>LICK ME!  TEASE ME!  LOVE MY WOMB WITH YOUR TONGUE!</i>\"  She responds by clamping her mouth around the head of your penis and sucking smartly.  A sharp pain in your ass signals the entry of her bony fingers working their way to your inner manhood.  Finding the root of your sex easily, she mashes down to force you to cum.\n\n");
			MainScreen.text("Finding it impossible to resist such pleasure, you immediately begin cumming.  Glob after glob, stream after stream of your semen shoots into the woman's mouth.  Her timed sucking ensures that she swallows each drop as you launch it into her.  While you have been proud of the ability to cum in a woman for over a minute, you are wracked with both pain and pleasure as your ejaculations continue for almost ten.  Once you have spent your last, the demon releases your penis to bear down on your face with her thighs and unloads a massive squirting orgasm.  Your face is soaked with pussy juice as you see her cunt spasm from the force of her pleasure.  The sight of her rhythmic muscles is hypnotic.  She then promptly removes her finger from your ass.");
		}
		else {
			MainScreen.text("The Succubus rears up and pins your shoulders firmly to the ground.  The speed of her vaginal contractions becomes impossibly fast.  As you look down at the Succubus' crotch, you can clearly see her contractions working your penis despite the massive growth of fur between your legs.  The pressure deep behind your cock is crushing.  With one last spasmodic push, you release a thick, superhuman gout of semen deep in the Succubus' cunt.  She lets out an inhuman howl of pleasure as her womb clamps down upon you like a vise.  She literally squeezes each stream and drop of cum out of you.\n\n");
			MainScreen.text("After a couple of minutes, you feel your dick weaken and begin going limp.  Satisfied, you feel the Succubus' grip on your prick release and you quickly fall out of her womb.  Much to your surprise, despite the massive load you released, not a single drop of cum falls out or is left on your prick.  The Succubus begins rubbing herself all over, clearly in post-orgasmic ecstasy from absorbing your fluids.  After a couple of chain-orgasms, the Succubus sits down next to the empty jar.\n\n");
			MainScreen.text("She places the mouth of the jar next to one of her fat nipples and begins milking her tit into the jar, filling it back up with her fluids.  She places the jar next to you and stands up.\n\n");
			MainScreen.text("\"<i>It has been a long time since I soaked up a load that good,</i>\" she says, \"<i>Anytime you want more of that, just drink my milk again.  It's only fair.  I take your milk, you take mine.</i>\"\n\n");
			MainScreen.text("She smiles and flies off, leaving you with a fresh bottle of \"Cerulean Potion\".  As pleasing as the experience was, it has left you thoroughly exhausted.");
	...
	private nightSuccubiRepeat(): void {
		...
		if (player.gender == 0) {
			if (Flags.list[FlagEnum.CERULEAN_POTION_NEUTER_ATTEMPTED] == 0) {
				...
				MainScreen.text("The Succubus resumes her stormy exit. You look at the bottle of Cerulean Potion and wonder if it REALLY had some psychotropics in it. What the hell just happened?!", false);
				Flags.list[FlagEnum.CERULEAN_POTION_NEUTER_ATTEMPTED] = 1;
			}
			//REPEAT
			else {
				...
				MainScreen.text("You turn around, hoping for an exciting encounter only to find a rather short, plain-faced woman with horned-rim glasses and a purple dress on. She appears to be holding a stack of papers in her hand.\n\n", false);

				MainScreen.text("\"<i>Ahem.</i>\", the woman says meekly, \"<i>I hate to bother you, but I was sent by the CoC writers and staff to hand you this.</i>\"\n\n", false);

				MainScreen.text("Scratching your head, you inquire what the document is. The woman smiles shyly and hands it to you.\n\n", false);

				MainScreen.text("\"<i>This is the script and production notes for Corruption of Champions,</i>\" she says with a small bit of pride, \"<i>Apparently, you need to read the highlighted sections. They are important.</i>\"\n\n", false);

				MainScreen.text("You take the script, scratching your head at the surreal nature of the moment. You thumb through the pages, finding virtually every aspect of your life and encounters written as if foreseen by great mystics. The accuracy is nothing short of horrifying. You find a highlighted section that appears to be what the woman is referring to. The note is terse and outright blunt.\n\n", false);

				MainScreen.text("\"<i>GENDER NEUTRAL CHARACTERS ARE BUTT-MONKEYS. IF THE ENCOUNTER INVOLVES SEX, EXPECT SOMETHING FUCKED UP TO HAPPEN INSTEAD. ACTORS WHO PLAY NEUTER CHARACTERS SHOULD EXPECT TO PLAY ONLY FOR LULZ</i>.\"\n\n", false);

				MainScreen.text("The shock is overwhelming. The script basically says that you will never catch a break. As this reality drapes about you, the script disappears and you hear a cacophony of mocking laughter in all directions. The woman is nowhere to be found.\n\n", false);

				MainScreen.text("As the cacophony fades, you only hear one facetiously toned word,\n\n", false);

				MainScreen.text("\"<i><b>Problem?</b></i>\"", false);
			}
			doNext(playerMenu);
			return;
		}
		...
		if (player.gender == 1) {
			if (player.stats.cor < 66) {
				...
				if (player.lowerBody.isTaur()) MainScreen.text(" crouch between your legs and impale herself on your " + CockDescriptor.describeCock(player, 0) + " with a wet sound caused by her well-lubricated vulva. Y", false);
				else MainScreen.text(" open her womb and quickly consume your " + CockDescriptor.describeCock(player, 0) + ". She embraces you, entrapping your head in her cleavage as y", false);
				MainScreen.text("ou quickly feel her superhuman vaginal muscles work and stroke your " + CockDescriptor.describeCock(player, 0) + " better than any human woman or pair of hands could ever hope to accomplish. You are helpless as your unholy embrace milks the both of you in an infernal symphony of debauchery. The familiar cramp of an impending ejaculation grips you and your twitching signals the succubus of your approaching climax.\n\n", false);
				if (player.lowerBody.isTaur()) MainScreen.text("Pushing on your forelegs, she engulfs even more of your " + CockDescriptor.describeCock(player, 0), false);
				else MainScreen.text("Almost crushing your pelvis, she wraps her legs around your body", false);
				MainScreen.text(" and her muscles churn mercilessly demanding that you release your 'milk' as freely as she has released hers into you. Stimulated beyond any human ability to maintain control, you bear down and release a milky flood of your own inside the succubus. Moaning in ecstasy, she ", false);
				if (player.lowerBody.isTaur()) MainScreen.text("arches under your belly as you feel your " + CockDescriptor.describeCock(player, 0) + " bending pleasurably inside her, and", false);
				else MainScreen.text("releases you from her grip, allowing you to finally breathe deeply, and leans back, arching high to reveal your joined genitals in the moonlight. You visibly see", false);
				MainScreen.text(" her contractions milking your " + CockDescriptor.describeCock(player, 0) + " as fiercely as a maid milks a cow! Another torrent of cum pushes its way out of your body and you let out a moan of pleasure and exhaustion.\n\n", false);
				MainScreen.text("As you are passing out, you feel a deep kiss upon your lips from the succubus. \"You taste better each time we join. Call upon me soon, lest I take what I want from you anyway,\", says the lustful creature.\n\n", false);
				MainScreen.text("Fatigue takes you and you collapse into a deep sleep.  ", false);
			}
			else {
				...
				MainScreen.text("The succubus comes, as you predicted. Despite her obvious strength and size difference to you, you grab her and push her down to the ground and immediately push your angry cock into her hairy hole. The succubus, surprised and enthralled, laughs at your aggression.\n\n", false);
				MainScreen.text("\"<i>I thought I was the hungry one.</i>\", she chuckles. \"I am all yours, little man. FEED ME!\"\n\n", false);
				MainScreen.text("You begin bucking your ", false);
				if (player.lowerBody.isTaur()) MainScreen.text("flanks", false);
				else MainScreen.text("hips", false);
				MainScreen.text(" in the all-too-familiar rhythm, hammering away at the succubus' cunt. Impressed with your initiative, she chooses to remain submissive as you work up an impressive load of spunk. Trying with all of your might, you continue to hold off your orgasm, painfully, as you continue stimulating your inhuman lover.\n\n", false);
				MainScreen.text("However, she senses your control and immediately brings her own muscles into the little love game. With one good squeeze, she breaks down any control and resistance you have. Sensing you are about to explode, she ", false);
				if (player.lowerBody.isTaur()) MainScreen.text("pushes on your forelegs, impaling herself even deeper on your " + CockDescriptor.describeCock(player, 0), false);
				else MainScreen.text("wraps her legs around your hips and bears down", false);
				MainScreen.text(". You feel the head of your prick push past the dilated opening in her cervix, which immediately contracts around your head. Your penis is literally trapped and caught in her womb!\n\n", false);
				MainScreen.text("Groaning loudly, long muscle spasms release the painfully stored semen into the deepest parts of the succubus. The sensation of your hot cum so deep inside her body triggers her peak. ", false);

				if (player.lowerBody.isTaur()) MainScreen.text("She moans inhumanly, and reflexively digs her claws into your forelegs. Searing with lust, the pain means little to you as you only feel the sensation of your body forcing your fluids out of your body and into hers. You press your " + CockDescriptor.describeCock(player, 0) + " into her", false);
				else MainScreen.text("She embraces you, moaning inhumanly, and reflexively digs her claws into your back. Searing with lust, the pain means little to you as you only feel the sensation of your body forcing your fluids out of your body and into hers. You slam your pelvis into hers", false);
				MainScreen.text(", as if to force yourself to cum harder than you already are capable of, prompting an equally pleasurable reaction from her.\n\n", false);
				MainScreen.text("For the first time since you have had your 'visits', the succubus appears winded. Without another word, her muscles release your manhood, which she quickly licks clean of your intermingled juices.  She tongues your face in lustful approval and flies away. You quickly fall asleep, utterly spent.  ", false);
				player.stats.lib += -1;
			}
		}
		else if (player.gender == 3) {
			...
			if (Flags.list[FlagEnum.CERULEAN_POTION_BAD_END_FUTA_COUNTER] > 10 && player.stats.cor > 50) {
				...
				if (player.lowerBody.balls > 0) MainScreen.text("  Even your nuts are unbearably sore.", false);
				MainScreen.text("  As the milk begins to dry out of the Succubus' tit, you release it from your control and launch an impossible load of cum into the succubus. The demoness releases her hold of your cock and hops off your crotch and jumps to place her mouth over your erupting penis. Reflexively grabbing her head, you push your cock as deep as you can in her mouth and for minutes, pump stream after stream of hot lust into her gullet. After the last load leaves your dong, you pass out.\n\n", false);

				MainScreen.text("After a short time, you wake up sore from head to toe. The Succubus is sitting next to you with an utterly satisfied look on her face.\n\n", false);

				MainScreen.text("\"<i>Well, this was unexpected.</i>\", she says, \"<i>I did not expect you to change. Normally, men are susceptible to my milk, but apparently it works on herms, too.</i>\"\n\n", false);

				MainScreen.text("As you stand, you feel awkward as your body does not feel right. You look at the Succubus and she no longer appears as large as she once was. Quick to realize a problem, you look at your reflection in a small bucket at your campsite. Other than your own unique facial features, you see ANOTHER Cerulean Succubus looking back at you! You ARE a Cerulean Succubus!", false);
				//[(if the player has a large number of transformations) 
				if (player.horseScore() + RaceScore.dogScore(player) + player.nagaScore() + player.goblinScore() + player.sharkScore() + player.minoScore() + RaceScore.cowScore(player) > 5) MainScreen.text("  All of the other corruptions and changes to your body have faded away as your new form has taken shape.", false);
				MainScreen.text("  As the reality soaks in, you feel a sharp pain in your stomach and your cock. You NEED to feed. Cum, milk, it doesn't matter. Likewise, your dick is hard and you need to cum. Despite your need, you cannot bring yourself to masturbate. You want ANOTHER'S attention.\n\n", false);

				MainScreen.text("Without further acknowledgement, you take up your on your demonic wings to find your first \"meal\". The Succubus left behind simply giggles as she sees another of her kind take up the night in search for more meals and pleasure.", false);
				Game.gameOver();
				return;
			}
			else {
				...
				MainScreen.text("The Succubus restrains you without missing a stroke or disrupting your breastfeeding as the pangs of orgasmic pleasure swell up at the base of your cock. You wrap your arms forcefully around the succubus as you bear down upon your crotch, releasing the painfully stockpiled load of lust into the demoness' cunt for her own sustenance. The succubus lets out an inhuman howl of pleasure as her own orgasm begins to crush your cock, draining every last drop out of you.\n\n", false);

				MainScreen.text("Your consciousness begins to fade as the orgasm subsides. The succubus pops her tit out of your mouth and squeezes more of her essence into the empty bottle. She licks your lips and flies away just in time for you to pass out.  ", false);
				...
				if (Flags.list[FlagEnum.CERULEAN_POTION_BAD_END_FUTA_COUNTER] > 10) {
					player.statusAffects.remove("SuccubiNight");
				}
				fatigue(20);
