...
export default class Giacomo implements TimeAwareInterface, IEncounter {
    ...
    public timeChangeLarge(): boolean {
        if (this.checkedSuccubi++ === 0 && Game.time.hour === 4 && character.statusAffects.has(StatusAffectType.SuccubiNight) && (character.torso.cocks.count > 0 || character.gender === 0)) { // Call secksins!
            if (character.statusAffects.has(StatusAffectType.RepeatSuccubi)) {
                ...
                character.statusAffects.set(new StatusAffect("RepeatSuccubi", 0, 0, 0, 0));)
            }
            // Lower count if multiples stacked up.
            if (character.statusAffects.get("SuccubiNight").value1 > 1)
                character.statusAffects.get("SuccubiNight").value1 = -1;
            else character.statusAffects.remove("SuccubiNight");
            return true;
        }
        return false;
    }
    ...
    private bookMenu(character: Character) {
        ...
        MainScreen.displayChoices(["Dangerous Plants", "Traveler's Guide", "Hentai Comic", "Yoga Guide", "Back"], [this.pitchDangerousPlantsBook, this.pitchTravellersGuide, this.pitchHentaiComic, (Flags.list[FlagEnum.COTTON_UNUSUAL_YOGA_BOOK_TRACKER] > 0 ? this.pitchYogaGuide : undefined), this.encounter]);
    }
    ...
    private buyVitailtyTincture(character: Character) {
        ...
        if (character.inventory.gems < 15) {
            ...
            character.inventory.items.add(character, [ItemFactory.create(ItemType.Consumable, ConsumableName.VitalityTincture)], this.potionMenu);
        }
    }
    ...
    private buyScholarsTea(character: Character) {
        ...
        if (character.inventory.gems < 15) {
            ...
            character.inventory.gems -= 15;
            character.inventory.items.add(character, [ItemFactory.create(ItemType.Consumable, ConsumableName.ScholarsTea)], this.potionMenu);
        }
    }
    ...
    private buyCeruleanPotion(character: Character) {
        ...
        if (character.inventory.gems < 75) {
            ...
            character.inventory.items.add(character, [ItemFactory.create(ItemType.Consumable, ConsumableName.CeruleanPotion)], this.potionMenu);
    ...
    private ceruleanSuccubusEncounterPart4(character: Character) {
        ...
        character.inventory.items.add(character, [ItemFactory.create(ItemType.Consumable, ConsumableName.CeruleanPotion)], Menus.Player.display);
    }
    ...
    private nightSuccubiRepeat(character: Character) {
        ...
        if (character.gender === 1) {
            ...
            if (Flags.list[FlagEnum.CERULEAN_POTION_BAD_END_FUTA_COUNTER] > 10 && character.stats.cor > 50) {
        ...
        DisplayText("\n");
        character.orgasm();
        character.stats.str += Utils.rand(2);
        character.stats.tou += Utils.rand(2);
        character.stats.spe += Utils.rand(2);
        character.stats.int += Utils.rand(2);
        character.stats.cor += 1;
        character.inventory.items.add(character, [ItemFactory.create(ItemType.Consumable, ConsumableName.CeruleanPotion)], Menus.Player.display);
    }
}
