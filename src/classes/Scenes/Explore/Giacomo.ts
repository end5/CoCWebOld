	...
	public class Giacomo extends BaseContent implements TimeAwareInterface {
		...
		public function giacomoEncounter():void {
			...
			if (kGAMECLASS.giacomo == 0) {
				...
				if (player.stats.gems < 175) { //Broke as a joke
					Render.text("You realize you don't have enough gems for such a pricey potion, but perhaps there is something else in his inventory you can buy.");
				}
				else { //Can afford
					Render.text("Do you purchase his cure?");
					//Remove/No
				...
				Render.text("You spy the merchant Giacomo in the distance.  He makes a beeline for you, setting up his shop in moments.  ");
		...
		private function pitchDangerousPlantsBook():void {
			...
			if (player.hasKeyItem("Dangerous Plants") >= 0) {
				Render.text("<b>You already own the book 'Dangerous Plants'.</b>");
				doNext(bookMenu);
				return;
			}
		...
		private function pitchTravellersGuide():void {
			...
			if (player.hasKeyItem("Traveler's Guide") >= 0) {
				Render.text("<b>You already own the book 'Traveler's Guide'.</b>");
				doNext(bookMenu);
				return;
			}
		...
		private function buyYogaGuide():void {
			...
			if (player.hasKeyItem("Yoga Guide") >= 0) {
				...
				Render.text("You cannot afford a yoga guide!");
			}
			else {
				Render.text("You exchange 100 gems for the tome.  Now you can finally enjoy a workout with Cotton!");
		...
		private function pitchDildo():void {
			...
			if (player.hasKeyItem("Dildo") >= 0) {
				Render.text("<b>You already own a Dildo!</b>");
				doNext(eroticaMenu);
				return;
			}
		...
		private function buyDildo():void {
			...
			if (player.stats.gems < 20) {
				...
				Render.text("After making the payment, Giacomo hands you the Dildo");
				doNext(eroticaMenu);
		...
		private function pitchSelfStimulationBelt():void {
			...
			if (player.hasKeyItem("Self-Stimulation Belt") >= 0) {
				Render.text("<b>You already own a Self-Stimulation Belt!</b>");
				doNext(eroticaMenu);
				return;
			}
		...
		private function buySelfStimulationBelt():void {
			...
			Render.text("After making the payment, Giacomo hands you the Self-Stimulation Belt");
		...
		private function pitchAllNaturalSelfStimulationBelt():void {
			...
			if (player.hasKeyItem("All-Natural Self-Stimulation Belt") >= 0) {
				Render.text("<b>You already own an All-Natural Self-Stimulation Belt!</b>");
				doNext(eroticaMenu);
				return;
			}
		...
		private function buyAllNaturalSelfStimulationBelt():void {
			...
			Render.text("After making the payment, Giacomo hands you the All-Natural Self-Stimulation Belt");
		...
		private function pitchOnahole():void {
			...
			if (player.hasKeyItem("Plain Onahole") >= 0) {
				Render.text("<b>You already own a Plain Onahole!</b>");
				doNext(eroticaMenu);
				return;
			}
		...
		private function buyOnahole():void {
			...
			Render.text("After making the payment, Giacomo hands you the Plain Onahole");
		...
		private function pitchDeluxeOnahole():void {
			...
			if (player.hasKeyItem("Deluxe Onahole") >= 0) {
				Render.text("<b>You already own a Deluxe Onahole!</b>");
				doNext(eroticaMenu);
				return;
			}
		...
		private function buyDeluxeOnahole():void {
			...
			Render.text("After making the payment, Giacomo hands you the Deluxe Onahole");
		...
		private function pitchAllNaturalOnahole():void {
			...
			if (player.hasKeyItem("All-Natural Onahole") >= 0) {
				Render.text("<b>You already own an All-Natural Onahole!</b>");
				doNext(eroticaMenu);
				return;
			}
		...
		private function buyAllNaturalOnahole():void {
			...
			Render.text("After making the payment, Giacomo hands you the All-Natural Onahole");
		...
		private function pitchDualStimulationBelt():void {
			...
			if (player.hasKeyItem("Dual Belt") >= 0) {
				Render.text("<b>You already own a dual belt!</b>");
				doNext(eroticaMenu);
				return;
			}
		...
		private function buyDualStimulationBelt():void {
			...
			clearOutput();
			if (player.stats.gems < 50) {
				Render.text("You do not have enough gems to purchase this item.");
			}
			else {
		...
		private function nightSuccubiFirstTime():void {
			...
			if (player.gender == 0) {
				...
				Render.text("The Succubus resumes her stormy exit.  You look at the bottle of Cerulean Potion and wonder if it REALLY had some psychotropics in it.  What the hell just happened?!");
			...
			if (player.gender == 1)
				...
				Render.text("As the Succubus finishes her ultimatum, you feel churning vaginal contractions stroking your massive cock.  Heavy, powerful, coordinated undulations work your dick as surely as the best handjob.  You quickly moan in shock and pleasure at such rough treatment.");
			}
			dynStats("lus", 35);
		...
		private function ceruleanSuccubusEncounterPart2():void {
			...
			if (player.gender == 1) {
				...
				Render.text("\"<i>Ready to burst, are you?</i>\" the Succubus playfully challenges.  \"<i>Well, then.  No reason for you to hold back.</i>\"");
			}
		...
		private function ceruleanSuccubusEncounterPart3():void {
			...
			if (player.gender == 1) {
				...
				Render.text("She smiles and flies off, leaving you with a fresh bottle of \"Cerulean Potion\".  As pleasing as the experience was, it has left you thoroughly exhausted.");
		...
		private function ceruleanSuccubusEncounterPart4():void {
			...
			if (player.gender == 1) {
				clearOutput();
				Render.text("She stands up and helps you to your feet.  While dazed, ");
				if (player.tallness < 80)
					Render.text("you see that she towers over you.  She must stand well over seven feet in height.  ");
				if (player.tallness >= 80 && player.tallness < 90)
					Render.text("you see she is about as tall as you - around seven feet in height.  ");
				if (player.tallness >= 90)
					Render.text("you see she's definitely shorter than you, only about seven feet tall.  ");
				...
				Render.text("She gives a giggle and disappears before your eyes.  At that moment the fatigue from the massive fucking you received catches up with you and you pass out in a slump.");
		...
		private function nightSuccubiRepeat():void {
			...
			if(player.gender == 0) {
				...
				//REPEAT
					...
					Render.text("You turn around, hoping for an exciting encounter only to find a rather short, plain-faced woman with horned-rim glasses and a purple dress on. She appears to be holding a stack of papers in her hand.\n\n", false);
		
					Render.text("\"<i>Ahem.</i>\", the woman says meekly, \"<i>I hate to bother you, but I was sent by the CoC writers and staff to hand you this.</i>\"\n\n", false);
		
					Render.text("Scratching your head, you inquire what the document is. The woman smiles shyly and hands it to you.\n\n", false);
		
					Render.text("\"<i>This is the script and production notes for Corruption of Champions,</i>\" she says with a small bit of pride, \"<i>Apparently, you need to read the highlighted sections. They are important.</i>\"\n\n", false);
		
					Render.text("You take the script, scratching your head at the surreal nature of the moment. You thumb through the pages, finding virtually every aspect of your life and encounters written as if foreseen by great mystics. The accuracy is nothing short of horrifying. You find a highlighted section that appears to be what the woman is referring to. The note is terse and outright blunt.\n\n", false);
					
					Render.text("\"<i>GENDER NEUTRAL CHARACTERS ARE BUTT-MONKEYS. IF THE ENCOUNTER INVOLVES SEX, EXPECT SOMETHING FUCKED UP TO HAPPEN INSTEAD. ACTORS WHO PLAY NEUTER CHARACTERS SHOULD EXPECT TO PLAY ONLY FOR LULZ</i>.\"\n\n", false);
		
					Render.text("The shock is overwhelming. The script basically says that you will never catch a break. As this reality drapes about you, the script disappears and you hear a cacophony of mocking laughter in all directions. The woman is nowhere to be found.\n\n", false);
		
					Render.text("As the cacophony fades, you only hear one facetiously toned word,\n\n", false);
		
					Render.text("\"<i><b>Problem?</b></i>\"", false);
				}
				doNext(playerMenu);
				return;
			}
			...
			if(player.gender == 1) {
				if(player.stats.cor < 66) {
					...
					if(player.isTaur()) Render.text(" crouch between your legs and impale herself on your " + cockDescript(0) + " with a wet sound caused by her well-lubricated vulva. Y", false);
					else Render.text(" open her womb and quickly consume your " + cockDescript(0) + ". She embraces you, entrapping your head in her cleavage as y", false);
					Render.text("ou quickly feel her superhuman vaginal muscles work and stroke your " + cockDescript(0) + " better than any human woman or pair of hands could ever hope to accomplish. You are helpless as your unholy embrace milks the both of you in an infernal symphony of debauchery. The familiar cramp of an impending ejaculation grips you and your twitching signals the succubus of your approaching climax.\n\n", false);
					if(player.isTaur()) Render.text("Pushing on your forelegs, she engulfs even more of your " + cockDescript(0), false);
					else Render.text("Almost crushing your pelvis, she wraps her legs around your body", false);
					Render.text(" and her muscles churn mercilessly demanding that you release your 'milk' as freely as she has released hers into you. Stimulated beyond any human ability to maintain control, you bear down and release a milky flood of your own inside the succubus. Moaning in ecstasy, she ", false);
					if(player.isTaur()) Render.text("arches under your belly as you feel your " + cockDescript(0) + " bending pleasurably inside her, and", false);
					else Render.text("releases you from her grip, allowing you to finally breathe deeply, and leans back, arching high to reveal your joined genitals in the moonlight. You visibly see", false);
					Render.text(" her contractions milking your " + cockDescript(0) + " as fiercely as a maid milks a cow! Another torrent of cum pushes its way out of your body and you let out a moan of pleasure and exhaustion.\n\n", false);
					Render.text("As you are passing out, you feel a deep kiss upon your lips from the succubus. \"You taste better each time we join. Call upon me soon, lest I take what I want from you anyway,\", says the lustful creature.\n\n", false);
					Render.text("Fatigue takes you and you collapse into a deep sleep.  ", false);
				}
				else {
					...
					Render.text("The succubus comes, as you predicted. Despite her obvious strength and size difference to you, you grab her and push her down to the ground and immediately push your angry cock into her hairy hole. The succubus, surprised and enthralled, laughs at your aggression.\n\n", false);
					Render.text("\"<i>I thought I was the hungry one.</i>\", she chuckles. \"I am all yours, little man. FEED ME!\"\n\n", false);
					Render.text("You begin bucking your ", false);
					if(player.isTaur()) Render.text("flanks", false);
					else Render.text("hips", false);
					Render.text(" in the all-too-familiar rhythm, hammering away at the succubus' cunt. Impressed with your initiative, she chooses to remain submissive as you work up an impressive load of spunk. Trying with all of your might, you continue to hold off your orgasm, painfully, as you continue stimulating your inhuman lover.\n\n", false);
					Render.text("However, she senses your control and immediately brings her own muscles into the little love game. With one good squeeze, she breaks down any control and resistance you have. Sensing you are about to explode, she ", false);
					if(player.isTaur()) Render.text("pushes on your forelegs, impaling herself even deeper on your " + cockDescript(0), false);
					else Render.text("wraps her legs around your hips and bears down", false);
					Render.text(". You feel the head of your prick push past the dilated opening in her cervix, which immediately contracts around your head. Your penis is literally trapped and caught in her womb!\n\n", false);
					Render.text("Groaning loudly, long muscle spasms release the painfully stored semen into the deepest parts of the succubus. The sensation of your hot cum so deep inside her body triggers her peak. ", false);
					
					if(player.isTaur()) Render.text("She moans inhumanly, and reflexively digs her claws into your forelegs. Searing with lust, the pain means little to you as you only feel the sensation of your body forcing your fluids out of your body and into hers. You press your " + cockDescript(0) + " into her", false);
					else Render.text("She embraces you, moaning inhumanly, and reflexively digs her claws into your back. Searing with lust, the pain means little to you as you only feel the sensation of your body forcing your fluids out of your body and into hers. You slam your pelvis into hers", false);
					Render.text(", as if to force yourself to cum harder than you already are capable of, prompting an equally pleasurable reaction from her.\n\n", false);
				...
				if(flags[FlagEnum.CERULEAN_POTION_BAD_END_FUTA_COUNTER] > 10 && player.stats.cor > 50) {
					...
					if(player.lowerBody.balls > 0) Render.text("  Even your nuts are unbearably sore.", false);
					Render.text("  As the milk begins to dry out of the Succubus' tit, you release it from your control and launch an impossible load of cum into the succubus. The demoness releases her hold of your cock and hops off your crotch and jumps to place her mouth over your erupting penis. Reflexively grabbing her head, you push your cock as deep as you can in her mouth and for minutes, pump stream after stream of hot lust into her gullet. After the last load leaves your dong, you pass out.\n\n", false);
		
					Render.text("After a short time, you wake up sore from head to toe. The Succubus is sitting next to you with an utterly satisfied look on her face.\n\n", false);
		
					Render.text("\"<i>Well, this was unexpected.</i>\", she says, \"<i>I did not expect you to change. Normally, men are susceptible to my milk, but apparently it works on herms, too.</i>\"\n\n", false); 
		
					Render.text("As you stand, you feel awkward as your body does not feel right. You look at the Succubus and she no longer appears as large as she once was. Quick to realize a problem, you look at your reflection in a small bucket at your campsite. Other than your own unique facial features, you see ANOTHER Cerulean Succubus looking back at you! You ARE a Cerulean Succubus!", false);
					//[(if the player has a large number of transformations) 
					if(player.horseScore() + player.dogScore() + player.nagaScore() + player.goblinScore() + player.sharkScore() + player.minoScore() + player.cowScore() > 5) Render.text("  All of the other corruptions and changes to your body have faded away as your new form has taken shape.", false);
					Render.text("  As the reality soaks in, you feel a sharp pain in your stomach and your cock. You NEED to feed. Cum, milk, it doesn't matter. Likewise, your dick is hard and you need to cum. Despite your need, you cannot bring yourself to masturbate. You want ANOTHER'S attention.\n\n", false);
		
					Render.text("Without further acknowledgement, you take up your on your demonic wings to find your first \"meal\". The Succubus left behind simply giggles as she sees another of her kind take up the night in search for more meals and pleasure.", false);
					getGame().gameOver();
					return;
				}
				else {
					...
					Render.text("The Succubus restrains you without missing a stroke or disrupting your breastfeeding as the pangs of orgasmic pleasure swell up at the base of your cock. You wrap your arms forcefully around the succubus as you bear down upon your crotch, releasing the painfully stockpiled load of lust into the demoness' cunt for her own sustenance. The succubus lets out an inhuman howl of pleasure as her own orgasm begins to crush your cock, draining every last drop out of you.\n\n", false);
		
					Render.text("Your consciousness begins to fade as the orgasm subsides. The succubus pops her tit out of your mouth and squeezes more of her essence into the empty bottle. She licks your lips and flies away just in time for you to pass out.  ", false);
