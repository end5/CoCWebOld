...
export default class Valentines {
    ...
    public helpValentinesDay(): void {
        MainScreen.clearText();
        MainScreen.text("Deciding to try and improve the overall appreciation of romance and love in possibly the last pure city of Mareth, you smile to Scylla and tell her she can count on you.  Her happy expression makes you forget any doubts you might've had before.");
        menu();
    ...
    public leaveValentinesDayForever(): void {
        MainScreen.clearText();
        MainScreen.text("Ultimately, this is not worth your time.  You wish Scylla good luck, but tell her you won't help her make the holiday more popular in Tel'Adre, as you have other things to do.  Her expression turns sad and she nods in understanding as you turn away and go back into the streets of Tel'Adre.");
    ...
    public helpValentinesDayIII(): void {
        MainScreen.clearText();
        MainScreen.text("You spend the rest of the evening participating in holiday games and showing people around the stall, listening to Scylla giving a talk or two about love and the celebration of it.  All in all, the little popularization of the holiday turned out to be a moderate success, but a success after all thanks to the joint effort of the four of you.");
        ...
        if (player.cockThatFits(28) >= 0 || player.lowerBody.vaginaSpot.hasVagina()) MainScreen.addButton(2, "Pastie", goVisitPastyVDay);
        MainScreen.addButton(4, "Leave", goHomeFromVDay);
    }
    ...
    public goVisitScyllaVdayIII(): void {
        ...
        MainScreen.addButton(2, "Feed Her", feedScyllaVDay);
    }
    ...
    public cuddleWithScyllaVDay(): void {
        ...
        //[if (hasVagina = true)]
        if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Your [vagina] is starting to get moist, making a small mess in your underclothes.");
        //[if (hasCock = true)]
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  You realize it'd probably be a done deal if you didn't resist the pull of your hardening cock towards one of Scylla's orifices, but you decide to spare her any temptations and endure yourself.");
        MainScreen.text("  In time, your ignited lust slowly changes to constant arousal, and Scylla just sitting there, watching the sun with a beautiful smile on her face, clearly content to just have someone in her arms on this evening is helping you control yourself a lot better.");
    ...
    public makeOutWithScyllaVDay(): void {
        ...
        //[if (cocks = 1)
        if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("your [cock] is completely hard and stiff, forming a bulge in your clothing");
        else if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("[eachCock] is by now completely hard, eager to be touched and wanting to be caressed by the nun you're kissing");
        MainScreen.text(".");
    ...
    public goVisitAbby(): void {
        MainScreen.clearText();
        MainScreen.text("\"<i>You... you're kidding, right?  That fatty's words messing with yah head?  I always figured you were somewhat less... messed up in the head than Suck-a-Lot and Always-Drunk.  Was I wrong?</i>\"");
        ...
        //[if (hasVagina = false)
        else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("\"<i>Well, that's unexpected. I have certain doubts about you showing me such a good time,</i>\" she says.  You tell her you don't actually require a cock at all to do this, and she seems surprised.");
        MainScreen.addButton(0, "PleasureHer", pleasureAbbyVDay);
    ...
    public fuckAbbyVDay(): void {
        ...
        MainScreen.text("Figuring that the goblin girl's felt so out of place amid a celebration of love, you might as well show her a little affection the only way she's likely to understand.  Pulling aside your [armor] enough to let [eachCock] spring free, you step up behind the bent-over girl and rest your hands on her thick thighs.  The emerald girl's breath quickens as the heat of your manhood presses between the jiggling globes of her ass cheeks and a whimper escapes her throat before she realizes it.  Coughing, in an attempt to cover the moment of weakness, she turns her head to the side to give you a disapproving eye.  “What is this, romance?  Just jam it in, already!” she commands, gruffly.  Her face is flushed a deeper forest hue, pink irises twinkling in the dim light of her spartan home.");
    ...
    public goVisitPastyVDay(): void {
        MainScreen.clearText();
        MainScreen.text("You lean over the table Pastie is no longer dancing, but rather laying, on, and, conspiratorially, ask her if she'd like to go and get something to drink.");
    ...
    public pastieValentineIntro(choice: string = ""): void {
        MainScreen.clearText();
        if (choice == "vag") {
            MainScreen.text("You tell Pastie that indeed, she could get her fix, as you have something you're sure every fairy likes to drink not far away... between your legs.  \"<i>T-that's an... unusual drink proposition...  Umm, and I'm more susceptible to girls than most fairies, so... Ohhh, who am I kidding, that sounds pretty good right now.</i>\"");
    ...
    public valentineDrinkPastie(itype: ItemType): void {
        MainScreen.clearText();
        MainScreen.text("One you're away from any prying eyes, Pastie turns to you and rubs her little fairy hands together, apparently barely able to hold her enthusiasm to finally get a bit drunk.  You're afraid it may not end at one drink, either, and for a moment, wonder about whether this is all right or not...");
        //Corruption 0-20] 
        if (player.stats.cor < 33) MainScreen.text("  It probably isn't.");
        //[Corruption 21-75]
        else if (player.stats.cor < 75) MainScreen.text("  You guess as long as she's fine afterwards, it should be okay.");
        //[Corruption 76-100]
        else MainScreen.text("  As long as she's in the mood to drink more than just the alcohol afterwards, you don't really care.");
    ...
    public valentineDayGetPastieDrink(itype: ItemType): void {
        ...
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(1, "RubOnDick", rubPastieOnYourWangDawg);
        if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.addButton(2, "Go In Pussy", goForAPushayDivePasty);
    }
    ...
    public rubPastieOnYourWangDawg(): void {
        ...
        //high cum production:
        else MainScreen.text(" almost blasting her right out of your hand in the torrent of white that spills forth from your mighty member.");
        MainScreen.text("  The spunk-plastered pixie shudders in your palm, overwhelmed by the spoo drenching her.  She masturbates, feverishly, climaxing in seconds and then again a moment later.  You watch her for a few minutes, the batter-basted bimbo jilling herself silly from the heightened sensitivity your spoo shower has given her.");
    ...
    public goForAPushayDivePasty(): void {
        ...
        //[if (hascock = true)
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  All the while, you continue to jerk off [eachCock], preparing yourself for an orgasm on both sides.");
        MainScreen.text("  She finally takes a full dive inside, and you moan out in pleasure, feeling her little body roll and explore in there, her tongue tasting every bit of fluid she can from what's moisturizing your inner passage.");
