...
export default class Thanksgiving {
    ...
    public shooTurkeyAway(): void {
        ...
        MainScreen.text("You scowl and wave your arm at the strange turkey-girl, telling her to get lost.");
    ...
    public spitRoastThatTurkey(): void {
        MainScreen.clearText();
        MainScreen.text("As you contemplate what to do with the insatiable turkey-girl, you hear another rustling at the edge of camp.  For a moment both worried and aroused by the thought of a whole flock of these sluts, you glance up in time to see a certain salamander poking her way through your bevy of traps.");
    ...
    public femaleAndTurkeyMainCourse(): void {
        MainScreen.clearText();
        MainScreen.text("Fully disrobed, the girl runs her hands down her chest, fingers clamping around her dribbling nipples once again.  With another squeeze, gravy is rolling down her chest once more, slathering her fingers with her own mouthwatering juices.  Unable to hold yourself back, your mouth clamps onto her breast, hands shooting to her soft, plush rear as you pull her bodily into your lap.  Her eyes almost light up in her new position, her hands running through your [hair] as you suck for all you're worth.  Her thick, rich gravy fills your mouth with its delicious mix of flavours.  There's so many different, wonderful tastes mixing together that you could almost believe you were back in Ingnam, gorging yourself on the gigantic spread that the village always strives to put on, regardless of how well the harvest went.");
    ...
    public haveTurkeyGirlDesertInYourCunt(): void {
        MainScreen.clearText();
        MainScreen.text("Barely able to contain yourself, you nearly tear your [armor] from your burning body, desperate to feel the girl with your bare flesh.  She reclines lazily on the ground, legs spreading wide as a hand moves from her dribbling tits, sliding its way down her toned stomach to her swollen lips.  She continues to tease herself whilst you struggle with your clothes, little moans and grunts of pleasure leaving you weak at the knees as she dreamily circles her reddening snatch with a finger.  Her other hand busies itself with her chest, pinching and squeezing just hard enough to release another spurt of gravy that rolls its way down the curve of her breast, dripping onto her belly in a thick glob.");
    ...
    public helloAgain(): void {
        MainScreen.clearText();
        MainScreen.text("\"<i>Oh, hey, you,</i>\" you say as the seasonal little near-harpy waddles up.  You pat your thigh invitingly, letting Gobbles approach.");
    ...
    public gravyBoatDatTurkturk(): void {
        MainScreen.clearText();
        MainScreen.text("You cup the turkey's chin, pulling her up off your cock enough to get her looking up at you with her big, questioning eyes.  \"<i>Gobble?</i>\" she ventures nervously, straining against your hand to get back at your shaft.");
    ...
    public turkeyDesertBitches(): void {
        ...
        MainScreen.text(" as the venom takes control, flushing your skin and forcing out all thoughts but one: fuck that turkey! There's nothing for it, now: you grab Gobbles' hips and plunge in");
        if (player.lowerBody.cockSpot.count() == 2) MainScreen.text(" with both cocks, one ramming into her abused cunny and the other right up her vulnerable little bumhole");
        else if (player.lowerBody.cockSpot.count() > 2) MainScreen.text(" with every cock you've got, half in the pink and half in the stink, spreading her as wide as you are in both barrels");
        MainScreen.text(", shocking her so much that she can't hold back, screaming and cumming on the spot! You groan, thrusting forward as her spasming muscles contract around your cock");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(", milking you as hard as she can while her orgasm overtakes her.  Gravy geysers out of her swollen tits as fem-cum splashes out around your deep-buried cock");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        ...
        else MainScreen.text("your hips are slapping into hers, making her butt bounce with the impact");
        MainScreen.text(".  She cries her one word to the heavens as you pound into her hole");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(", thrusting your cock");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(" deep inside.  Her avian legs break free of her hold, wrapping around your [hips] and pulling you in, locking you in her spasming hole");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(" as she cums and cums, pleading, \"<i>Gobble!</i>\" at the top of her lungs.  You switch your grip to her bountiful bosom, roughly squeezing her tits as you hump your cock");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(" in and out of her slick slit{and quivering behind}, pinching her nipples to give the squealing turkey just a little more pleasure.");
        ...
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" and ass");
        MainScreen.text(", her legs spurring you on every time you withdraw, muscles clenching when you pause, never letting you rest.  Needy slut! But she's more than willing to do her part, thrusting her hips up to meet you, squeezing and milking your cock");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(" every time you pierce her, working your shaft");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(" so expertly that you know you won't be able to hold on for long - and neither will she, the way she's panting and gasping along with you!");

        MainScreen.text("\n\nYou draw nearer to the breaking point, hips unrelentingly slamming your shaft");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(" deep into Gobbles' eager depths, urged on by her happy squawks and gleeful cries.  Lust-venom-addled as the two of you are, orgasm approaches like a wave of relief, crashing through your bodies in beautiful unison.  \"<i>Gobbles!</i>\" she cries as her cunt ");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("and ass ");
        ...
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s deposit the last of their");
        else MainScreen.text(" deposits the last of its");
        MainScreen.text(" cream into her, fully stuffing your turkey-girl.  Panting, you let your head rest between her tits a moment long, shaft");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(" wilting inside her until like a broken dam, her cunt");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" and ass");
        MainScreen.text(" begin");
        if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("s");
        MainScreen.text(" to drool spunk, pooling onto the hungry ground with the gallons of spilt gravy and fem-spooge.");
    ...
    public pigSlutRoastingGreet(): void {
        MainScreen.clearText();
        MainScreen.text("As you enter The Wet Bitch you’re accosted by an incredibly busty waitress. You can’t really discern what exactly she is as your eyes are solely focused on the massive melons jutting out from her chest and jiggling proudly with mile deep cleavage to boot.");
    ...
    public getARoastPiggueOinkOinkOinkMotherfucker(): void {
        MainScreen.clearText();
        player.stats.gems -= 1;
        MainScreen.text("You quickly approach the gooey bucket and toss a single, shiny gem inside. Another waitress winks at you, almost equally as busty, nodding her head in the direction of the back rooms.");
        ...
        MainScreen.text("our nostrils flare, taking in great amounts of the salacious stench.");
    ...
    public tenderizeDatPigSloot(): void {
        MainScreen.clearText();
        MainScreen.text("\"<i>Turn over.</i>\" you say rather plainly, eager to see that jiggling booty of hers.");
...
player.stats.resisted += false;
    ...
    public tenderizeHerHardcore(): void {
        MainScreen.clearText();
        MainScreen.text("You can't help but maliciously smirk, your eyes resting on a fetish whip and some bondage equipment. You give the piggy one last double-handed spank on her ample ass, leaving her a pitifully moaning and quivering wreck. You quickly snatch up the whip, some rope, and a kinky ballgag.");
...
player.stats.resisted += false;
    ...
    public hamRoastTenderizeHerSoftcore(): void {
        MainScreen.clearText();
        MainScreen.text("You can't help but let a salacious smile grace your lips, your eyes resting on a paddle in the corner. You give the piggy one last double-handed spank on her ample ass, leaving her a pitifully moaning and quivering wreck. You quickly snatch the somewhat heavy paddle, feeling its heft and grinning like a madman.");
...
player.stats.resisted += false;
    ...
    public spankEpilogueColonYoureAHorriblePersonForAbusingADumbPigGirl(): void {
        MainScreen.clearText();
        MainScreen.text("Eventually you stop your ceaseless romping of her rear, the piggy whore blubbering into the now thoroughly torn pillow from her biting and tearing. Several strips of flesh from her reddened ass look practically raw from your activities. You lay an experimental finger on the tortured flesh, the blazing heat radiating from it telling you all you need to know.");
    ...
    public getDoneEatingOutPigSlut(): void {
        MainScreen.clearText();
        MainScreen.text("Finally you remove yourself from the still slightly dribbling juicebox, giving the whore one last spank as she seems to have passed out from the bliss. The bed is entirely a mess, covered with her sticky juices and torn from the spanking.");
        ...
        if (player.thickness < 100) {
            MainScreen.text("  You notice that you seem a bit more... jiggly, a slight paunch in your stomach now as you grow a bit more plump than you once were.");
        ...
        if (player.lowerBody.butt.buttRating < 35) {
            MainScreen.text("  You notice your ass filling out, gaining more mass as it visibly grows before your eyes.");
            if (player.lowerBody.butt.buttRating < 4) player.lowerBody.butt.buttRating += 3;
            if (player.lowerBody.butt.buttRating <= 10) player.lowerBody.butt.buttRating += 2;
            if (player.lowerBody.butt.buttRating <= 15) player.lowerBody.butt.buttRating += 1;
            if (player.lowerBody.butt.buttRating <= 20) player.lowerBody.butt.buttRating += 1;
            if (player.lowerBody.butt.buttRating <= 35) player.lowerBody.butt.buttRating += 1;
            changed = true;
        }
        if (player.lowerBody.hipRating < 35) {
            MainScreen.text(" Your hips grow too, widening and giving you more sexy curves.");
            if (player.lowerBody.hipRating < 4) player.lowerBody.hipRating += 3;
            if (player.lowerBody.hipRating < 10) player.lowerBody.hipRating += 2;
            if (player.lowerBody.hipRating < 15) player.lowerBody.hipRating += 1;
            if (player.lowerBody.hipRating < 20) player.lowerBody.hipRating += 1;
            if (player.lowerBody.hipRating < 35) player.lowerBody.hipRating += 1;
            changed = true;
        }
        ...
        MainScreen.text("\n\nYou sigh");
        if (changed) MainScreen.text(", feeling your body expand as you waddle out back towards camp with belly full of sweet syrup");
        MainScreen.text(". For only one gem, that was a pretty good time...");
        doNext(camp.returnToCampUseTwoHours);
    }
}