...
//1. Kirbster's Bunny Trap
//if(isHolidays() && Flags.list[FlagEnum.CANDY_CANE_YEAR_MET] < date.fullYear)

export default class XmasOther {
    public candyCaneTrapDiscovery(): void {
        MainScreen.clearText();
        MainScreen.text("Shortly after embarking into the plains, your thoughts drift back to your life and home in your village.  The slight chill in the air today reminds you of a time of cheer celebrated by your former friends and family.  As days grew shorter and colder, people exchanged humble trinkets, and great feasts were prepared.  ");
        if (player.stats.cor < 50) MainScreen.text("The memory brings a nostalgic smile to your [face].");
        else MainScreen.text("The memory reminds you of how bland your world truly was in comparison to Mareth.");
        ...
        menu();
        MainScreen.addButton(0, "Investigate", investigateCandyCaneBun);
        MainScreen.addButton(4, "Leave", leaveBeforeMeetingCandyCaneBun);
    ...
    public leaveBeforeMeetingCandyCaneBun(): void {
        MainScreen.clearText();
        MainScreen.text("You're not willing to run the risk of getting a foreign cock stuffed up your [butt].");
        //(If corruption or libido>50) 
        if (player.stats.cor > 50 || player.stats.lib > 50) MainScreen.text("  At least not right now.  You turn back, navigating your way back to your camp.");
    ...
    public investigateCandyCaneBun(): void {
        MainScreen.clearText();
        MainScreen.text("You make the decision to find out just what's going on.  You cautiously follow the small pools of goo, slightly on edge.  You approach a wall of low hanging vine which you quickly pull aside.  Sitting in the largest pudddle of semen you've seen yet, a trembling bunny-like figure kneels on its knees.");
        //(If corruption and libido<50) 
        if (player.stats.cor < 50 && player.stats.lib < 50) MainScreen.text("  Your eyes are quickly drawn to the rabbit morph's stark nudity.");
        //(If corruption or libido>50) 
        else MainScreen.text("  Your eyes are quickly drawn to the rabbit morph's finely curved ass.");
        MainScreen.text("  A pair of heavily hanging balls confirm the figure to be at least partially male.");
        MainScreen.text("\n\nYou make the move to ");
        //(If corruption and libido<50)
        if (player.stats.cor < 50 && player.stats.lib < 50) MainScreen.text("leave, ");
        //(If corruption or libido<50)
        else MainScreen.text("approach, ");
        MainScreen.text("but one of your [feet] snaps a twig and causes the bunny to turn at the sound. As its large eyes lock with your, you get your first good look at the figure.  It is predominantly human looking, but the floppy ears sprouting from its head and fluff adorning him from the thighs down indicates at least a little mutation.  Two small nipples lie on a flat chest, confirming him to be a male.  His eight inch dick and kiwi-sized balls leave even less to your imagination.  He is lightly tanned, his skin colored a non abrasive brown.  The hair on his body stands in a sharp contrast to his relatively regular skin.  A mop of shoulder length hair on his head is as white as freshly fallen snow, with sprinklings of red stripes highlighting it.  You can't help but notice how nicely it frames his surprisingly soft, feminine face.  The fur covering his legs is smiliary covered.  His hips jut out notably, adding even further to his overall girly appearance.");
        MainScreen.text("\n\n\"<i>You're not going to rape me?</i>\"  The bunny boy's remarkably high voice breaks your train of thought.  ");
        //(If corruption<50)
        if (player.stats.cor < 50) MainScreen.text("You calmly inform him that you didn't have the intention of violating him. ");
        //(If corruption>50)
        else MainScreen.text("Your train of thought is derailed, but you agree to let the bunny believe whatever he wants to.");
        //(If silly mode on) 
        if (silly()) MainScreen.text("  You're willing to say anything to calm him down.  This kid's voice is even more shrill than that asshole from Glee!");
        MainScreen.text("  A smile lights up his face, his shoulders visibly drop and lose their tension.   He sits like that for but a moment, before he realizes the full scope of the current situation.  His eyes grow wide while he throws down his arms with a rather unmanly yelp, covering his dripping cock.  Despite his embarrassment, it's evident that he's not going to get off unassisted.");
        Flags.list[FlagEnum.CANDY_CANE_YEAR_MET] = date.fullYear;
        MainScreen.text("\n\nDo you help him out?");
        //(Present \"<i>Yes</i>\" and \"<i>No</i>\" options)
        menu();
        MainScreen.addButton(0, "Yes", helpWithTheCandyCane);
        MainScreen.addButton(1, "No", declineCandyCaneCawks);
    ...
    public declineCandyCaneCawks(): void {
        MainScreen.clearText();
        MainScreen.text("Turning around a tad awkwardly, you stumble out of the glade.  Tempting as he is, you don't have time to help the random bunny orgasm.");
        doNext(camp.returnToCampUseOneHour);
    }
    ...
    public helpWithTheCandyCane(): void {
        ...
        //(If corruption<50) 
        if (player.stats.cor < 50) MainScreen.text("  Not wanting to disturb him, you make your way back to camp and attempt to ignore the burning fire in your loins.");
        //(If corruption>50) 
        else MainScreen.text("  Despite your rising urge to take advantage of the unknowing bunny, you decide to retire and return to camp.");
    ...
    public getAChristmasChicken(): void {
        Flags.list[FlagEnum.XMAS_CHICKEN_YEAR] = date.fullYear;
        ...
        menu();
        MainScreen.addButton(0, "Talk", talkToXmasChicken);
        MainScreen.addButton(4, "Leave", leaveXmasChicken);
    }

    //[Leave]
    public leaveXmasChicken(): void {
        MainScreen.clearText();
        MainScreen.text("Too exasperated by the plain absurdity of the situation to deal with it, you bury your face in your hands and decide to leave her to it.  Maybe some imps will shut her up for you.  You head back to camp with her piercing cries following you all the way, and prepare for the day ahead.  Her cries fade a while later; the snow thaws and the air warms soon after.");
        doNext(camp.returnToCampUseOneHour);
    }
    ...
    public talkToXmasChicken(): void {
        MainScreen.clearText();
        MainScreen.text("Figuring you should at least try asking her to pipe down, you continue your approach.  She finally takes notice of your footsteps crunching the snow beside her and goes still and silent for a moment as she looks up at you.  Now that you're closer, you can make out some more of her features against the snowy ground.  She wears a light dress, almost as pale as her skin and the snow around her, thin enough that you can just see her perky nipples through its white fabric.  A shiver of your own makes you wonder why she's wearing something like that on a day like this.  The thick, fluffy-looking feathers of her wings even manage to stand out as somehow even whiter than the frosty backdrop of the snow.  A faint, silvery glow seems to radiate from her, but it's probably just a trick of the light against the snowfall.");
        ...
        //(if male)
        if (player.gender == 1) MainScreen.text("How's a blowjob sound to you?");
        //(if female)
        else if (player.gender == 2) MainScreen.text("How about I eat you out?");
        //(if herm)
        else MainScreen.text("How about I give you a few minutes to do whatever you want with my mouth?");
        MainScreen.text("</i>\"  You raise an eyebrow, not entirely sure what to make of her suddenly lascivious nature.  It's certainly quite a change.  She giggles.  \"<i>Oh, don't be surprised.  Where do you think we are");
        //(if sillymode)
        if (silly()) MainScreen.text(", scrub");
        MainScreen.text("?</i>\"");
        menu();
        MainScreen.addButton(4, "Decline", declineChickenBlowjobs);
        MainScreen.addButton(0, "Accept", acceptChristmasBJs);
    }

    //[Decline]
    public declineChickenBlowjobs(): void {
        MainScreen.clearText();
        MainScreen.text("The harpy squints at you suspiciously.  \"<i>What do you mean, 'no thanks?'  Nobody says that around here!</i>\"  She rolls her eyes and sighs in an obviously exaggerated manner, then turns tail and flaps off into the distance.  You head back to camp, quite looking forward to the silence.  The air warms and the snow thaws soon after you return, leaving no trace of its presence but a few spots of slushy mud.");
        doNext(playerMenu);
    }
    ...
    public acceptChristmasBJs(): void {
        ...
        if (player.gender == 1) {
            ...
            MainScreen.clearText();
            MainScreen.text("Would you like her to service your male parts or lady bits?");
            menu();
            MainScreen.addButton(0, "Man Parts", acceptChristmasManBJ);
            MainScreen.addButton(1, "Lady Bits", acceptChristmasWomanBJ);
        }
    }
    ...
    public acceptChristmasManBJ(): void {
        ...
        if (player.cockArea(x) >= 80) {
            MainScreen.text("You free your [cock " + (x + 1) + "] from your [armor], signaling your acceptance.  Your monster flops free, and the harpy grabs at her throat reflexively when she looks at it.  She glances up to you with wide eyes and back down at it.  \"<i>A-a promise is a promise, but...</i>\"");
        ...
        //(else if cock would please Goldilocks) (area<= 30 or some shit i dunno whatever the standard is)
            ...
            MainScreen.text("You free your [cock " + (x + 1) + "] from your [armor], signaling your acceptance.  The immediate rush of cold air that greets it makes you hope she won't be wasting any time.  The pale harpy woman squeals at the sight, raising her hands to her chin and practically bouncing in place.  In a flash she's on her knees, stroking and licking at your meat until it's hard in front of her face.  Her hot breath against your shaft has you more than ready for the sanctuary from the biting cold.  She eyes your erect prick");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(" almost hungrily, licking her lips before swiftly engulfing almost its entire length in her mouth.  Her expert technique astounds you; you certainly didn't expect her to have any idea what she was doing but she's able to take you nearly to the base of your shaft in one motion.");
            ...
            //(if only one cock)
            if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("She grabs onto your hips, enabling herself to move all the quicker.");
            //(if two cocks)
            else if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("With her other hands she starts to work the shaft of the second cock, pumping in time with the motions of her head.");
            //(if 3+ cocks)
            else MainScreen.text("She grabs onto two of your other erect cocks with her hands and begins pumping away at them in time with the motions of her head.");
            ...
            if (player.cumQ() < 250) MainScreen.text("  She closes her eyes and looks quite contented while she swallows your spunk.");
            //(else if cum output is high)
            else if (player.cumQ() < 1000) MainScreen.text("  Semen gushes out of you for what seems like forever, but the harpy doesn't seem to mind.  She closes her eyes and looks quite contented while she swallows your still-torrenting spunk.");
            //(else if cum output is ridonkulous)
            else MainScreen.text("  Though at first she looks like she'll be able to handle it, the river of semen exploding from the rip of your cock doesn't stop.  Her eyes widen as the almost unending torrent of spunk fills her, rushing down her throat and into her belly.  You can almost see her bloating with the stuff before it starts to spill back out, dripping white from her lips around your shaft.");
            //(if multicock)
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("  Your cum sprays over her, coating her skin and plumage in a white to rival their own.  Strands of your semen dripping and running over her hardly even stand out on her bright body.");
            MainScreen.text("  When you finish, you let the harpy go.  She looks just as satisfied as you are.  A trail of cum and saliva still connects your [cockHead " + (x + 1) + "] to her; she breaks it with a lick of her lips and gives your softening member a longing glance before standing up, panting from exertion.");
    ...
    public approachKamiTheChristmasRoo(): void {
        ...
        if (Flags.list[FlagEnum.KAMI_ENCOUNTER] == 1) {
            ...
            menu();
            if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(70) >= 0) MainScreen.addButton(0, "Give Anal", giveKamiTheChristmasRooAnal);
            MainScreen.addButton(1, "Receive Anal", takeItRooButtStyle);
            MainScreen.addButton(2, "Give BJ", KamiBlowJob);
            if (player.lowerBody.cockSpot.count() > 1 && player.cockThatFits(70) >= 0 && player.cockThatFits2(70) >= 0) MainScreen.addButton(3, "Doube-Fuck", KamiDoubleDickFuck);
        }
        else {
            MainScreen.text("You summon the buxom waitress over, getting a much better view of her gentle face now that she's found the time to sit and talk with you.  With hazelnut-brown eyes, a gorgeous set of lips and tousled blonde hair, her faux-rebellious look is the icing on the festive cake for this busty waitress, her cute little santa hat being the strawberry on top.  Smooth, olive skin runs halfway down her meaty thighs before fading into soft fur, while her frail forearms follow the same pattern. Her huge kangaroo tail wags idly throughout your conversation, and you've gained a pretty good rhythm whilst engaging her in it. Determined to make things go well, you pause to pull out a charming one-liner when her index finger meets your ");
            if (player.femininity >= 75) MainScreen.text("feminine ");
            else if (player.femininity <= 25) MainScreen.text("masculine ");
            MainScreen.text("lips abruptly.");
            ...
            if (silly()) MainScreen.text(" and found space to scrawl a little penis in one corner. Clever");
            MainScreen.text(".");
            MainScreen.text("\n\nWow, well, that's pretty blunt.  What should you do now?");
            //[Present options, 'Let's go!' and 'Leave']
            menu();
            MainScreen.addButton(0, "Let's Go", chooseLetsGoKami);
            MainScreen.addButton(4, "Leave", skipOutAfterOpeningKamisBox);
        }
    }
    ...
    public skipOutAfterOpeningKamisBox(): void {
        MainScreen.clearText();
        MainScreen.text("You politely decline Kami's sexual advances, apologizing before making a turn for the door. You begin to wonder why you didn't buy anything at the bakery.");
        //[Return to Tel Adre, KamiEnc = 1]
        Flags.list[FlagEnum.KAMI_ENCOUNTER] = 1;
        doNext(camp.returnToCampUseOneHour);
    }
    ...
    public chooseLetsGoKami(): void {
        MainScreen.clearText();
        MainScreen.text("You decide to give Kami exactly what she wants - 'tis the season for giving, after all!  You grin approvingly before heading out into the back alley, taking your time to stare at her wobbling ass.");
        ...
        MainScreen.addButton(1, "Receive Anal", takeItRooButtStyle);
        MainScreen.addButton(2, "Give BJ", KamiBlowJob);
        if (player.lowerBody.cockSpot.count() > 1 && player.cockThatFits(70) >= 0 && player.cockThatFits2(70) >= 0) MainScreen.addButton(3, "DoubleFuck", KamiDoubleDickFuck);
    }
    ...
    public giveKamiTheChristmasRooAnal(): void {
        ...
        MainScreen.text("You head briskly towards the voluptuous waitress, rearing her around by the wrists and pressing her against the wall.  \"<i>H-Hey, wait!</i>\" Kami yelps, but you sense excitement in her delicate plea.");
        ...
        if (player.cumQ() < 1000) MainScreen.text("  \"<i>Mmm... just like that, babe,</i>\" Kami moans submissively, her asshole clenching one last time before you pull out and climb off your satiated conquest, your " + CockDescriptor.describeCock(player, x) + " still dripping creamy white spunk.");
        //{if cum quantity ≧ heavy}
        else
            MainScreen.text("  One of Kami's hands flies from her dick to her suddenly swollen belly as your mighty seed gushes forth.  The unexpectedly heavy girl sinks further into the ground, your load swishing back up and loosening her tunnel around you and forcing you to remove your still-spewing " + CockDescriptor.describeCock(player, x) + " before something unfortunate happens.  Semen starts tumbling out as soon as you dismount your kangaroo conquest, but judging by the slow pace, she's going to look heavily pregnant for a while.");
    ...
    public takeItRooButtStyle(): void {
        ...
        if (player.lowerBody.balls > 0) MainScreen.text("your own");
        //[Else if have vagina]
        else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("your pussy");
        //[Else if neither]
        else MainScreen.text("your taint");
        MainScreen.text(".  Each time, the waitress fondles your form with her eager hands.  ");

        MainScreen.text("\n\n");
        //[If player has pussy] 
        if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("Your cunt oozes in return, envious of the tenderized hole above it.  ");
        MainScreen.text("You dedicate a few more minutes to soaking up every moment of her animalistic romping, savoring the feeling of a hung rod in your punished posterior.  Your climax quickly comes to loom over you, however, the potent, ");
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("prostate");
        else MainScreen.text("ass");
        MainScreen.text("-numbing sensations at your rear becoming too much to bear.  Slamming back into Kami's pole one last time, you breathe a shuddered sigh of relief as your orgasm takes over, the sensation spreading throughout your entire body like electricity.");
        //[If male]
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  Your pent-up cock ejaculates there and then, shamelessly gushing sloppy semen against the wall in front of you.");
        //[If female]
        if (player.gender == 3) MainScreen.text("  Meanwhile, y");
        else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("Y");
        if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("our eager pussy buckles in arousal, gushing fem-juices on the floor below you.");
        MainScreen.text("  You're truly spent.");
    ...
    public KamiBlowJob(): void {
        ...
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  You think you might still need to find an outlet for that itch of yours, with [eachCock] still erect and needing release.");
        dynStats("lus", 20 + player.stats.lib / 10 + player.stats.sens / 10);
    ...
    public KamiDoubleDickFuck(): void {
        ...
        let x: number = player.cockThatFits(70);
        let y: number = player.cockThatFits2(70);
        MainScreen.clearText();
        ...
        MainScreen.text("\n\n\"<i>I could do this all goddamn day!</i>\"  Kami screams, your wicked thrashing leaving her pelvis moist with sheer bliss.  You can already feel your own orgasm approaching, so with due haste you grab Kami's ample bust and pick up your thrusts energetically, ramming your cocks home with as much speed as you can muster.  Your ass-engorged " + CockDescriptor.describeCock(player, x) + " cums first, packing Kami's tight hole with your sticky seed, shortly followed by your other " + CockDescriptor.describeCock(player, y) + " giving her delicate-yet-punished pussy the same treatment.");
    ...
    public getWinterPudding(): void {
        MainScreen.clearText();
        if (player.stats.gems < 35) {
            MainScreen.text("Sadly, the pudding isn't free, and it looks like you don't have the 35 gems you'd need to try a bite!");
            doNext(telAdre.bakeryScene.bakeryuuuuuu);
            return;
        }
        player.stats.gems -= 35;
        statScreenRefresh();
        MainScreen.text("You opt for a slice of the tasty treat - you've only got until the festivities wade away, so you might as well take one now!");
        ...
        if (player.thickness < 100 || player.tone > 0) {
            ...
            MainScreen.text(player.modTone(0, 2));
            MainScreen.text(player.modThickness(100, 2));
        }
        ...
        if (player.upperBody.head.horns == 0 && rand(2) == 0) {
            ...
            player.upperBody.head.hornType = HornType.ANTLERS;
            player.upperBody.head.horns = 4 + rand(12);
        }
        //[Show this description instead if the player already had horns when the transformation occurred.] 
        else if (player.upperBody.head.horns > 0 && player.upperBody.head.hornType != HornType.ANTLERS && rand(2) == 0) {
    ...
    public polarPete(): void {
        MainScreen.clearText();
        MainScreen.text("As you travel through the plains, something seems to be off the further you continue to walk.  Soon your breath mists in front of your mouth, and you hug yourself as the temperature seems to drop.  As you look up, flakes of pure white fall from above.  The ground at your feet is littered with frost and dusted with snow, and you feel a chill run up your spine as you adjust to the colder weather. Something unnatural seems to be affecting the area.");
        ...
        menu();
        MainScreen.addButton(0, "Unwrap", unwrapPolarPete);
        MainScreen.addButton(4, "Leave", leaveDisFukkinPolarBear);
    }
    //Unwrap
    public unwrapPolarPete(): void {
        MainScreen.clearText();
        MainScreen.text("The temptation too strong, you walk closer to Pete.  Sinking down to your knees before him as he adjusts his hips to your level.  Your hands on his crimson pants, pulling them down with bated breath.  His dark ebony length flops free, long and swollen with heat before your nose. Your hand rubs along it's smooth black surface coaxing a pleased groan from Pete.");
        ...
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  [EachCock] grows stiff and soaks your [armor] with precum.");
        //(if vagina) 
        if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Your [vagina] grows moist, dripping eager juices and soaking your [armor].");
        ...
        //[(if cock) 
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  His continued ramming of your prostate mixed with the addictive candy crammed in your mouth sends you over the edge, [eachCock] shooting out ropes of snow white all over your chest.  Pete eyes you over with a lurid gaze, your face covered in his cum and your body covered in your own.");
        else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Your [vagina] quivers, the rough thrusting into your [asshole] mixed with the addictive candy in your mouth sending you over the edge.  With a low muffled moan your mound spills your lust over your groin, dripping down to lube Pete's thrusts as well as your [legs].  Pete gives you a lurid gaze.");
    ...
    public leaveDisFukkinPolarBear(): void {
        MainScreen.clearText();
        MainScreen.text("You've decided not to trust the stranger, with a nod of your head you walk away from the man.  \"<i>Wait!  Before you go...</i>\" he speaks as he moves his massive sack in front of himself with a huff.  He leans forward, his upper body rummaging through the sack before reappearing.  He hands you a brightly wrapped gift.");
    ...
    public snowLadyActive(): void {
        ...
        Flags.list[FlagEnum.NIEVE_STAGE] = 1;
        doNext(playerMenu);
    }
    ...
    public nieveBuilding(): void {
        ...
        if (Flags.list[FlagEnum.NIEVE_STAGE] == 1) {
            ...
            MainScreen.text("You head back over to the snowy patch by your camp.  The crunch of snow again makes you feel just a little nostalgic, and you think of drinking hot cocoa topped with marshmallows.  With a smile on your face, you set about rolling up the soft powder-like crystals into a huge ball.  It takes some effort, but after a while you're left with a large ball of snow, which will serve as the perfect base.");
            ...
            menu();
            MainScreen.addButton(0, "Snowwoman", nieveSnowWoman);
            MainScreen.addButton(1, "Snowman", nieveSnowMan);
            //[Snowwoman] [Snowman] 
        }
        //Second Step: Eyes & Mouth
        //Alters Nieve's eyes in scenes
        else if (Flags.list[FlagEnum.NIEVE_STAGE] == 2) {
            MainScreen.text("You approach your snow" + nieveMF("man", "woman") + " again, looking it over.");
            ...
            menu();
            if (player.inventory.items.has(consumables.COAL___)) MainScreen.addButton(0, "Coal", nieveCoalEyes);
            MainScreen.addButton(1, "Gems", nieveGemEyes);
            MainScreen.addButton(4, "Back", camp.returnToCampUseOneHour);
        }
        //Fourth Step: The Nose
        else if (Flags.list[FlagEnum.NIEVE_STAGE] == 3) {
            MainScreen.text("You approach your nearly finished snowman. To others, it might be a perfectly acceptable creation, but not to you. It needs a carrot to finish it off.");
            ...
            if (player.hasKeyItem("Carrot") >= 0) {
                ...
                Flags.list[FlagEnum.NIEVE_STAGE] = 4;
                player.removeKeyItem("Carrot");
                MainScreen.text("\n\n(Removed Key Item: Carrot)");
            }
            //(Else) 
            else {
                MainScreen.text("Unfortunately, you've yet to find one in your adventures.  You suppose you'll have to look more carefully.  Who knows, there might be a farm right under your nose.");
            }
            doNext(camp.returnToCampUseOneHour);
        }
        else {
            MainScreen.text("Your snowman is done!  There's nothing more to add to it.  It looks mighty fine however, and just looking at it brings a nostalgia-fueled smile to your lips.");
            doNext(camp.returnToCampUseOneHour);
        }
    }
    //Coal
    ...
    public nieveCoalEyes(): void {
        MainScreen.clearText();
        player.inventory.items.consumeItem(consumables.COAL___);
        MainScreen.text("Luckily, you happen to have two lumps of coal.");
        ...
        Flags.list[FlagEnum.NIEVE_MOUTH] = "coal";
        Flags.list[FlagEnum.NIEVE_STAGE] = 3;
        doNext(camp.returnToCampUseOneHour);
    }
    //Gems
    //Add to previous text if possible?
    public nieveGemEyes(): void {
        MainScreen.clearText();
        if (player.stats.gems >= 9) {
            MainScreen.text("Taking a handful of gems out of your pouch, you spread them evenly around the Snowman's face, giving it a nice, vacant smile.  It still needs a nose, however, and for that, you'll need a carrot.  Perhaps there's a farm nearby?");
            player.stats.gems -= 9;
            statScreenRefresh();
            Flags.list[FlagEnum.NIEVE_MOUTH] = "gems";
            Flags.list[FlagEnum.NIEVE_STAGE] = 3;
        }
        //Too broke to use the gems option? What the fuck is wrong with you?
        else {
            MainScreen.text("You open up your pouch, and frown.  Unfortunately, you don't have enough gems to create the eyes and mouth. With a sigh you march your broke ass back to camp.");
        }
        doNext(camp.returnToCampUseOneHour);
    }
    ...
    public nieveSnowWoman(): void {
        ...
        MainScreen.text("You grin mischievously to yourself and set about making two more balls of powdery snow.  It takes less time than any of the others, and before you know it you've attached two icy-breasts to the snowman.  They aren't terribly big, any heavier and you're sure they'd fall off, but they get the point across.");
        ...
        doNext(camp.returnToCampUseOneHour);
        Flags.list[FlagEnum.NIEVE_STAGE] = 2;
        Flags.list[FlagEnum.NIEVE_GENDER] = 2;
    }
    //Snowman
    //Add to existing text if possible, rather than a new window?
    public nieveSnowMan(): void {
        MainScreen.clearText();
        MainScreen.text("You decide to leave it as is. Not everything has to have breasts, of course, even in Mareth.");
        ...
        doNext(camp.returnToCampUseOneHour);
        Flags.list[FlagEnum.NIEVE_STAGE] = 2;
        Flags.list[FlagEnum.NIEVE_GENDER] = 1;
    }
    ...
    public findACarrot(): void {
        MainScreen.clearText();
        MainScreen.text("As you explore the farm, you come across several rows of green plants.  Getting closer, you recognize them... carrots!  You pull one of them from the ground.  It's expectedly dirty, but bright orange, and straight enough to be the perfect nose for your snowman.");
    ...
    public nieveMF(Man: string = "", Woman: string = ""): string {
        if (Flags.list[FlagEnum.NIEVE_GENDER] == 1) return Man;
        else return Woman;
    }
    ...
    public nieveComesToLife(): void {
        MainScreen.clearText();
        Flags.list[FlagEnum.NIEVE_STAGE] = 5;
        ...
        if (Flags.list[FlagEnum.NIEVE_GENDER] == 1) MainScreen.text("  It appears to be male, judging by the modestly sized cock sprouting from its loins.");
        else MainScreen.text("  It appears to be female, judging by the modestly sized tits on its chest, and the lack of a penis, which is quite a feat in Mareth.");
        MainScreen.text("  Pure white hair spills down from its head, almost blending in with the surrounding snow.");
        ...
        if (Flags.list[FlagEnum.NIEVE_MOUTH] == "gems") MainScreen.text("glittering purple");
        else MainScreen.text("coal black");
        MainScreen.text(" orbs, fall on you, as if registering you for the first time.  Then a hint of recognition hits " + nieveMF("him", "her") + ".  \"<i>Oh!  [Master]!  You're my [master]!</i>\"  In a flash " + nieveMF("he", "she") + "'s standing, looking pleased as punch, with " + nieveMF("his", "her") + " hands clasped excitedly in front of " + nieveMF("himself", "herself") + ".");
        ...
        //[Y/N]
        menu();
        MainScreen.addButton(0, "Yes", yesKeepNieve);
        MainScreen.addButton(1, "No", noNoKeepNieve);
    }

    //Yes of course, what do you look like, an idiot?
    public yesKeepNieve(): void {
        MainScreen.clearText();
        MainScreen.text("You actually laugh at the question.  Of course you want " + nieveMF("him", "her") + " to stay!");
        //[Silly mode: 
        if (silly()) MainScreen.text("  A free follower, no worries about buying worthless dye or raising their affection?  Fuck yeah, you'll take " + nieveMF("him", "her") + "!");
        MainScreen.text("  " + nieveMF("He", "She") + " doesn't seem to be a threat, and indeed seems sincere in the fact that " + nieveMF("he", "she") + " was sent here to be your lover.");
    ...
    public noNoKeepNieve(): void {
        MainScreen.clearText();
        MainScreen.text("You shake your head.  Of course not!  While " + nieveMF("he", "she") + " certainly seems like a nice person on the surface, you can't help but think that's just the tip of the iceberg.  For all you know, " + nieveMF("he", "she") + "'s a frigid bitch underneath, a trap set by the demons to lure you into a false sense of security.  Clever bastards, you conclude.  They certainly know the best way to serve revenge, but you won't have any of it.");
        ...
        Flags.list[FlagEnum.NIEVE_STAGE] = -1;
        Flags.list[FlagEnum.NIEVE_GENDER] = 0;
        Flags.list[FlagEnum.NIEVE_MOUTH] = "";
        doNext(camp.returnToCampUseOneHour);
    }
    ...
    public nieveCampDescs(): void {
        //6:00
        if (model.time.hours == 6) MainScreen.text("Nieve is sitting cross-legged in the snowdrift, munching on what looks to be icicles.  As you watch, " + nieveMF("he", "she") + " reaches down into the glittering powder surrounding " + nieveMF("him", "her") + " and produces another one.");
        //7:00
        else if (model.time.hours == 7) MainScreen.text("Nieve is giggling and laughing as a fresh flurry flutters down on " + nieveMF("him", "her") + ".  You wonder idly if " + nieveMF("he", "she") + "'s causing it to happen.");
        //8:00
        else if (model.time.hours == 8) MainScreen.text("Nieve is sitting in the white wintery wonderland, carefully constructing a snowman.  It strikes you as strange, and almost masturbatory in a way.  " + nieveMF("He", "She") + " spots you and gives you a wave and a smile.  For a moment, you think the snowman has too, but you eventually conclude it's your imagination.");
        //9:00
        else if (model.time.hours == 9) MainScreen.text("Nieve is sprawled out in the icy field, creating a series of snow angels without a care in the world.  When " + nieveMF("he", "she") + " catches sight of you, she gives a friendly wave, then dives back into the snow.");
        //10:00
        else if (model.time.hours == 10) MainScreen.text("Nieve, the ice spirit, sits quietly at the edge of the snowdrift, looking out at the landscape beyond.  When " + nieveMF("he", "she") + " sees you looking, " + nieveMF("he", "she") + " gives a somber wave.");
        //11:00
        else if (model.time.hours == 11) MainScreen.text("Nieve is in your camp, poking around your supplies and water barrels. You notice " + nieveMF("he", "she") + " seems a little uncomfortable to be away from " + nieveMF("his", "her") + " snowdrift.");
        //12:00
        else if (model.time.hours == 12) MainScreen.text("Nieve is carefully compressing snowballs in " + nieveMF("his", "her") + " little camp.  To your surprise, " + nieveMF("he", "she") + " picks one up and begins eating it like you would an apple.  When " + nieveMF("he", "she") + " catches you looking mid-bite, " + nieveMF("he", "she") + " gives you a grin with her mouth full, " + nieveMF("his", "her") + " cheeks puffed out.");
        //13:00
        else if (model.time.hours == 13) MainScreen.text("Nieve is sitting cross-legged, in the middle of what appears to be an argument with a snowman " + nieveMF("he", "she") + "'s just made.");
        //14:00
        else if (model.time.hours == 14) MainScreen.text("Nieve is in " + nieveMF("his", "her") + " usual spot, seemingly deep in concentration.  Around " + nieveMF("him", "her") + " snow begins to fall slowly, then quicker and quicker, whipping " + nieveMF("his", "her") + " hair around wildly.  Although strangely, the air is still where you are.");
        //15:00
        else if (model.time.hours == 15) MainScreen.text("\n\nNieve the ice spirit is whirling around " + nieveMF("his", "her") + " area, practicing with what appears to be a translucent blue spear.  " + nieveMF("He", "She") + " jabs and thrusts, spins and swipes.  " + nieveMF("He", "She") + " may be new to Mareth, but you've got to hand it to " + nieveMF("him", "her") + ", it looks like " + nieveMF("he", "she") + " could take care of " + nieveMF("him", "her") + "self.");
        //16:00
        else if (model.time.hours == 16) MainScreen.text("\n\nNieve is at first nowhere to be seen.  Then you see " + nieveMF("his", "her") + " head pop out of a snowdrift.  " + nieveMF("He", "She") + " looks around quickly, then leaps into the air, diving head first into another pile of the powdery stuff.");
        //17:00
        else if (model.time.hours == 17) MainScreen.text("\n\nNieve appears to have constructed a firepit constructed from translucent blue logs and rocks.  A vivid blue flame roars in the pit, over which Nieve appears to be roasting... marshmallows?  As you watch " + nieveMF("he", "she") + " peels the burnt black skin off a marshmallow, gobbles it down, and begins roasting the rest.");
        //18:00
        else if (model.time.hours == 18) MainScreen.text("\n\nNieve seems to be working on a spear, sharpening it with a jagged piece of ice.  When " + nieveMF("he", "she") + " catches sight of you " + nieveMF("he", "she") + " waves and gives a friendly smile.");
        //19:00
        else if (model.time.hours == 19) MainScreen.text("\n\nNieve is sitting at the edge of " + nieveMF("his", "her") + " icy expanse, staring off at the distant mountains.  You wonder if " + nieveMF("he", "she") + "'s looking for home.");
        //20:00
        else if (model.time.hours == 20) MainScreen.text("\n\nNieve is, surprisingly, outside of " + nieveMF("his", "her") + " cold camp.  " + nieveMF("He", "She") + "'s dancing along the battered, parched ground, calling down snow.  Wherever a flake hits, it's immediately devoured by the thirsty earth.  Is Nieve perhaps trying to... water the ground?");
        //21:00
    ...
    public approachNieve(): void {
        MainScreen.clearText();
        MainScreen.text("You wave at Nieve, getting " + nieveMF("his", "her") + " attention and calling for " + nieveMF("him", "her") + ".  The icy spirit-" + nieveMF("man", "woman") + " happily walks over, smiling.  " + nieveMF("He", "She") + " takes " + nieveMF("his", "her") + " time, allowing you to take in " + nieveMF("his", "her") + " naked body.");
        ...
        //[Silly Mode:]
        if (silly()) MainScreen.text("  And you presume a butthole nestled between " + nieveMF("his", "her") + " cheeks, right where it belongs.");
        MainScreen.text("  " + nieveMF("His", "Her") + " stomach is flat and toned, " + nieveMF("as is his chest", "and she possesses a pair of perky B-cup breasts") + ". " + nieveMF("His", "Her") + " face is the same pale blue as the rest of " + nieveMF("his", "her") + " body, though it is off-set by " + nieveMF("his", "her") + " glittering, ");
        if (Flags.list[FlagEnum.NIEVE_MOUTH] == "coal") MainScreen.text("dusky black");
        else MainScreen.text("vibrant purple");
        MainScreen.text(" eyes and pure white hair, which " + nieveMF("barely goes past his ears", "tumbles down past her shoulders") + ".  Much of " + nieveMF("his", "her") + " body glimmers with a fine layer of powdered snow or ice.");
        MainScreen.text("\n\nNieve stops, gives you a friendly hug, and asks, \"<i>What can I do for you, [Master]?</i>\"");
        menu();
        MainScreen.addButton(0, "Sex", nieveSexMenu);
        MainScreen.addButton(4, "Back", camp.campLoversMenu);
    }

    //Sex Menu
    public nieveSexMenu(): void {
        MainScreen.clearText();
        MainScreen.text("What will you do with your oh-so-cool lover?");
        if (player.stats.lust < 33) MainScreen.text("  You aren't quite turned on enough for normal fucking.");
        menu();
        if (Flags.list[FlagEnum.NIEVE_GENDER] == 2) MainScreen.addButton(0, "Lick Her", lickNieve);
        if (Flags.list[FlagEnum.NIEVE_GENDER] == 1) MainScreen.addButton(0, "Suck Him", suckNieveOff);
        //Fuck Her
        //Female Nieve
        //Must have a penis or at least a 3.5 inch clit
        //Nieve's capacity is about 130.
        if ((player.lowerBody.cockSpot.hasCock() || (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.vaginaSpot.get(0).clitLength >= 3.5)) && player.stats.lust >= 33) {
            if (Flags.list[FlagEnum.NIEVE_GENDER] == 2) MainScreen.addButton(1, "Fuck Her", fuckNieve);
        }
        //Get Fucked by Gurumash
        //Male Nieve
        //Any Gender
        //Nieve's cock is 9x1.5
        if (Flags.list[FlagEnum.NIEVE_GENDER] == 1 && player.stats.lust >= 33) MainScreen.addButton(1, "Get Fucked", nieveFucksYou);
    ...
    public lickNieve(): void {
        MainScreen.clearText();
        MainScreen.text("You explain your intentions and how you'd like to be the one to give her pleasure.  She looks rather surprised at first, but then she smiles.  \"<i>You want to go down... on me?</i>\"  You nod and she giggles, \"<i>That would be wonderful!</i>\"");
        ...
        if (!silly()) {
            ...
            //(Normal/Snake Tongue: 
            if (player.upperBody.head.face.tongueType != TongueType.DEMONIC) MainScreen.text("It doesn't get far, but elicits squeals of surprise and pleasure from your elemental slave who's already grasping at the snow surrounding her for support.");
            //Demon Tongue: 
            else if (player.upperBody.head.face.tongueType == TongueType.DEMONIC) MainScreen.text("Her eyes widen as you snake your impressive, demonic tongue into her. She grunts and squeals with pleasure, hands grasping piles of snow, wishing they were bedsheets.");
            MainScreen.text("  After a minute of this, you realize you've lost the feeling in your tongue!  Pulling it back into your mouth, you find it warms back up quickly, and then you're at it again.");
            ...
            if (player.lowerBody.isTaur()) MainScreen.text(", or as close to it with your body,");
            MainScreen.text(" with Nieve straddling your face.  \"<i>Mm, this is much better.  Now you wanted to lick me, [Master]? So... lick,</i>\" she whispers, lowering her chilly cunt down onto your face.");
        ...
        //Silly Mode:
            ...
            //(no Kid A: 
            if (Flags.list[FlagEnum.ANEMONE_KID] != 1) MainScreen.text(", Nieve takes a ladle full of water and pours it onto her snatch");
            //Kid A: 
            else MainScreen.text(", your little anemone spawn giggles and laughs at your predicament before finally ladling some water onto your lover's snatch");
            MainScreen.text(", effectively allowing you to pull your tongue away.");
            MainScreen.text("\n\nThe both of you thoroughly flustered, Nieve returns to the winter paradise and you go back to your duties.");
            dynStats("lus", -5 - player.stats.sens / 5);
        }
        doNext(camp.returnToCampUseOneHour);
    }
    ...
    public fuckNieve(): void {
        ...
        if (x >= 0) {
            if (player.lowerBody.cockSpot.get(x).cockLength < 6) MainScreen.text("  \"<i>It's so cute, [Master],</i>\" Nieve says while catching a breath, \"<i>Like a little toy cock.</i>\"");
            //If overly large (>130): 
            else if (player.lowerBody.cockSpot.get(x).cockLength >= 24) MainScreen.text("  She has to take a breath almost immediately, saying, \"<i>By the fat man's beard this thing is huge.  You must make the rest of this world jealous.</i>\"");
            else MainScreen.text("  \"<i>Mmm,</i>\" she smacks her lips and takes a quick breath, \"<i>Such a nice example of a candycane right here.  Let's go in for another taste.</i>\"");
        }
        else {
            if (player.lowerBody.vaginaSpot.get(0).clitLength < 6) MainScreen.text("\"<i>It's so cute, [Master],</i>\" Nieve says while catching a breath, \"<i>Like a little toy cock.</i>\"");
            else if (player.lowerBody.vaginaSpot.get(0).clitLength >= 24) MainScreen.text("  She has to take a breath almost immediately, saying, \"<i>By the fat man's beard this thing is huge.  You must make the rest of this world jealous.</i>\"");
            else MainScreen.text("  \"<i>Mmm,</i>\" she smacks her lips and takes a quick breath, \"<i>Such a nice example of a candycane right here.  Let's go in for another taste.</i>\"");
        }
        MainScreen.text("  And with that she dives back down onto your " + cockClit(x) + ".  Her head bobs and bows, giving the sensation of an icecube running up and down your ");
        if (x >= 0) MainScreen.text("dick");
        else MainScreen.text("clit");
        MainScreen.text(" at great speed.");

        //(Variable depending on dick-type)
        if (x >= 0) {
            //(Human) 
            if (player.lowerBody.cockSpot.get(x).cockType == CockType.HUMAN) MainScreen.text("  Your normal, unaltered cock seems to take on a slight blue tint, although you can confirm it's not going numb.  Quite the contrary, it seems to be getting more sensitive as Nieve's lips roll over it.");
            //(Equine) 
            else if (player.lowerBody.cockSpot.get(x).cockType == CockType.HORSE) MainScreen.text("  Nieve fawns over your horsecock like a cultist praying to a god.  She sucks and kisses, running the tip of her tongue all along the sides until she reaches the sheathe, and then right back up the other side.");
            //(Canine) 
            else if (player.hasKnot(x)) {
                MainScreen.text("  Nieve eyes the knot on your cock as though it were a challenge.  She inches down your shaft, getting ever closer.");
                //(Cocksize >40)
                if (player.cockArea(x) >= 40) MainScreen.text("  Eventually, though, she admits defeat and goes back to working your shaft, though she eyes that knot from time to time with no small amount of anger.");
                //(Cocksize <40) 
                else MainScreen.text("  It takes her a few good minutes, but eventually she works her way down and takes the entire knot into her mouth.  She raises her hands in triumph and lets out a \"<i>Mmmmrrrrrpphhh!</i>\" before pulling back for a breath, inadvertently exposing your wet hard cock to the cold, wintery air.");
            }
            /*Cock types
            0 - human
            1 - horse
            ...
            */
            //(Demonic)
            else if (player.lowerBody.cockSpot.get(x).cockType == CockType.DEMON) MainScreen.text("  Your purple, demonic shaft pulses and throbs, seemingly immune to the unnatural cold of Nieve's mouth. Seemingly oblivious to its nature, Nieve slavers over each nodule, giving every ridge and bump a kiss and more than just a lick.");
            //(Tentacle) 
            else if (player.lowerBody.cockSpot.get(x).cockType == CockType.TENTACLE) MainScreen.text("  Nieve does not seem put off by your tentacle cock in the least, in fact she confesses, \"<i>It reminds me of a comic I read before coming here.</i>\"  As she fawns over your cock, you're certain you hear whispers of \"<i>Oh, you sexy tentacle beast...</i>\" coming from her mouth when she gets her breath.");
            //(Feline) 
            else if (player.lowerBody.cockSpot.get(x).cockType == CockType.CAT) MainScreen.text("  Though the barbs on your cock are soft and pliable, they reflexively harden up after Nieve's tongue washes over them.  She doesn't pay particular attention to the barbs, but she does seem rather careful not to get one hooked on her lip.");
            //(Lizard)
            else if (player.lowerBody.cockSpot.get(x).cockType == CockType.LIZARD) MainScreen.text("  Nieve fawns over your scaled and ridged reptilian cock, slurping and smooching up and down its length. It seems a bit more resistant to the biting cold of Nieve's mouth, which is lucky when she backs off, exposing your entire wet, frigid length to the wintery air surrounding you.");
            //(Anemone) 
            else if (player.lowerBody.cockSpot.get(x).cockType == CockType.ANEMONE) MainScreen.text("  Nieve giggles whenever one of your wriggling tentacles stings her, seemingly immune to its effects, and kisses it back.  Soon after she devours your whole length, delighted to feel the sting of your cock in the back of her throat.");
            //(Kangaroo) 
            else if (player.lowerBody.cockSpot.get(x).cockType == CockType.KANGAROO) MainScreen.text("  It takes Nieve a little while to catch on that your cock isn't quite human, and by the time she figures it out, she mutters, \"<i>Huh, that's strange... I thought kangaroos had balls above the cock,</i>\" before shrugging and getting back to work.");
            //(If you add dicks later, placeholder text:) 
            else MainScreen.text("  Nieve works on your mutant cock, spending time working every unique little facet of it.");
        }
        //(Clitoris) 
        else {
            MainScreen.text("  Nieve nuzzles and lavishes your elongated love button, showering it with attention. \"<i>I didn't know clits could get this big,</i>\" she says with wonder in her voice. \"<i>");
            if (player.lowerBody.vaginaSpot.get(0).clitLength < 5) MainScreen.text("It's just like a little cock");
            else if (player.lowerBody.vaginaSpot.get(0).clitLength < 9) MainScreen.text("It's just like a cock");
            else MainScreen.text("I bet even men get jealous of this bad girl");
            MainScreen.text(".</i>\"");
        }
        ...
        if (x >= 0) MainScreen.text("  Your " + cockClit(x) + " quivers and spurts cum directly into Nieve's eager mouth, who gulps it down as if she were drinking direct from the tap.");
        MainScreen.text("\n\nAs you come down from your little orgasmic high, you glance down at the ice woman, ");
        if (x < 0 || player.cumQ() <= 250) MainScreen.text("who has a bit of cum dribbling down her chin");
        else if (player.cumQ() <= 400) MainScreen.text("who has a steady stream of cum pouring from her mouth");
        else if (player.cumQ() < 600) MainScreen.text("who is patting her stomach as though she'd just had a large meal");
        else if (player.cumQ() < 1000) MainScreen.text("whose stomach appears slightly distended from the amount of cum you've poured into her");
        else MainScreen.text("whose stomach is rather large, like a pregnant woman's, from the sheer amount of cum you've pumped into her gullet");
        MainScreen.text(".  With a smirk, you tell her it's not over yet.  That was just the appetizer.");
        ...
        if (x >= 0) MainScreen.text("with your " + CockDescriptor.describeCock(player, x) + "");
        else MainScreen.text("with yours");
        MainScreen.text(", and just slipping the tip in before pulling out.  You do this until your snowbound lover is biting her lip, squirming with barely contained pleasure.");

        MainScreen.text("\n\nRight as she begins to open her mouth, to beg you to just stop it and fuck her, you slip your " + cockClit(x) + " in ");
        //(if too big: 
        if (x >= 0) {
            if (player.cockArea(x) >= 130) MainScreen.text("at least as far as it will go, ");
        }
        MainScreen.text("with a wet \"<i>schluck.</i>\"  The words coming out of her mouth are lost in a loud \"<i>ooohhhh</i>\" as her eyes roll backwards, closely followed by her head.  Her wet pussy grips you tightly, giving you a strange tingling sensation.  Your hot rod combined with her arctic nethers actually causes some steam to rise up, and you're a little captivated to see more tufts of steam rise with every slow, gentle thrust you make.");
        ...
        if (x >= 0) {
            //((cocksize 3/Clit 3)
            if (player.cockArea(x) <= 7) MainScreen.text("Your meager penis isn't much, but at this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes staring up at you, urging you on faster and faster.");
            //(Cocksize 30/Clit 12) 
            else if (player.cockArea(x) <= 30) MainScreen.text("At this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes tightly shut as she whispers dirty words you can't even make out.");
            //(Cocksize 70/Clit 24)
            else if (player.cockArea(x) <= 70) MainScreen.text("Nieve's cunt feels like an icy vice on your oversized cock, but even so, you manage to hit all the right places.  Soon she is wriggling around, eyes tightly shut as she pants for breath.");
            //(Cocksize 130+/Clit 48+)
            else MainScreen.text("Though you can't fit your entire " + cockClit(x) + " inside, you are amazed at how much she takes.  Her pale blue stomach bulges with every thrust.  Soon Nieve is wriggling, her eyes slightly open as she bites her lip in ecstasy.");
        }
        else {
            if (player.lowerBody.vaginaSpot.get(0).clitLength <= 4) MainScreen.text("Your meager clit isn't much, but at this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes staring up at you, urging you on faster and faster.");
            else if (player.lowerBody.vaginaSpot.get(0).clitLength <= 12) MainScreen.text("At this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes tightly shut as she whispers dirty words you can't even make out.");
            else if (player.cockArea(x) <= 24) MainScreen.text("Nieve's cunt feels like an icy vice on your oversized clitty, but even so, you manage to hit all the right places.  Soon she is wriggling around, eyes tightly shut as she pants for breath.");
            //(Cocksize 130+/Clit 48+)
            else MainScreen.text("Though you can't fit your entire " + cockClit(x) + " inside, you are amazed at how much she takes.  Her pale blue stomach bulges with every thrust.  Soon Nieve is wriggling, her eyes slightly open as she bites her lip in ecstasy.");
        }
        ...
        //((Clitfucking?)
        if (x < 0) MainScreen.text("  Your [clit] suddenly feels suddenly warm, like every nerve ending inside were on fire. It aches and pulses with the rest of your body, as though it were a little cock longing to cum.");
        else {
            MainScreen.text("  Your " + cockClit(x) + " aches and pulses with need before ");
            if (player.cumQ() < 100) MainScreen.text("spilling its meager offering into the ice spirit's womb");
            else if (player.cumQ() < 250) MainScreen.text("ejecting streams of white hot jizz into the ice spirit's hungry snatch");
            else if (player.cumQ() < 400) MainScreen.text("painting the ice spirit's insides with your cream until some begins to pour out");
            else if (player.cumQ() < 700) MainScreen.text("arcs of pearly white cum shoot forth, filling the ice spirit's womb so much her belly visibly inflates");
            else MainScreen.text("erupting inside, filling the ice spirit up with so much of your hot cum so much that she looks visibly pregnant");
            MainScreen.text(".");
        }
    ...
    public nieveFucksYou(): void {
        MainScreen.clearText();
        MainScreen.text("Your arrival to Nieve's part of the camp elicits a friendly smile from the ice man.  He notices the longing in your eyes with a grin, and speaks in a gentle, yet low tone, \"<i>[Master], you seem to need help from me...</i>\"");
        MainScreen.text("\n\nTo which you nod, ");
        if (player.stats.cor < 50) MainScreen.text("telling him that you find his form... appeasing to say the least, and would like to know how his 'parts' are working properly.");
        else MainScreen.text("leting your creation know that you are interested in 'how' it can please you, giving him no room to imagine what you're insinuating you then tell him bluntly that you want him to fuck you like the plaything he is.");
        ...
        //{Silly Mode:}
        if (silly()) MainScreen.text("   He did say that his kind were effectively love slaves, and boy does this prove it!");
        //[if (hasVagina = true) 
        if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Your [vag] is a sodden box so soon after he started, and you start to want more, much more!");
        //[if (hasCock = true) 
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  Your [cock biggest] is also rock hard in a short while and you quickly desire some more stimulation from your icy love slave.");
    ...
    public takeNieveAnal(): void {
        ...
        //[if (hasCock = true) 
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  Nieve then bends forward while keeping his pace to give you a reach-around.  Grabbing your own [cock biggest] in one hand, he starts to pump it to the speed of his thrusts, torturing you with the extra-stimulation in such a good way.");
        //[if (hasVagina = true)
        else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  As Nieve continues to pound away, he places one hand on your sopping wet cunt and begins to finger you in time with his assault, making you lust-drunk with the stimulation as he finger bangs you.");

        MainScreen.text("\n\nNieve continues to grunt as he prods and plunges into your ");
        //if (silly mode) 
        if (silly()) MainScreen.text("pooper");
        else MainScreen.text("[asshole]");
        MainScreen.text(" as you feel an oncoming climax.  Nieve, furiously impaling you with his long and cold cock, comes first, and in one final thrust drives his penis as deeply as possible, flooding your bowels with a crisp current of cum as his body rocks from the orgasm.  You join him in the throes of ecstasy as soon as his jizz hits your inner walls");
        if (player.gender > 0) {

            //[if (hasCock = true) 
            if (player.lowerBody.cockSpot.hasCock()) {
                MainScreen.text(", ");
                MainScreen.text("while your body undulates and seizes as cum starts to spew forth from [eachCock]");
                //(normal volume: 
                if (player.cumQ() < 200) MainScreen.text(", as you cum, you leave a small puddle on the floor below you");
                else if (player.cumQ() < 500) MainScreen.text(", as you cum you start to dress your arms and hands with your own cream as you shoot your stuff out onto the floor");
            }
            //[if (hasVagina = true)
            if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(".  Your soaking vagina makes your body quake and shudder as you orgasm, splashing your femcum all over Nieve and the ground");
        }
        MainScreen.text(".");
    ...
    public takeNieveVaginal(): void {
        MainScreen.clearText();
        MainScreen.text("You're already wet from the foreplay, and it shows.  Moreso, Nieve notices, with a grin similar to the one he gave you earlier.  The look on his face suggests he knows what you want, he's just merely waiting for his [master] to give the word.");
        ...
        //[if (hasBalls = true) 
        if (player.lowerBody.balls > 0) MainScreen.text(", gently moving your [balls] out of the way in the process");
        MainScreen.text(".  He slams his nine inch blue rod balls deep into your sodden box.  A loud squelch fills the air at the penetration, giving Nieve the go ahead to start in top gear, pistoning in and out at a hurried pace.");
        //[if (hasCock = true)
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  During the pounding Nieve grabs your dangling [cock biggest] and starts to pump it in time with his own thrusts, quite eager to please his [master].  Every time your feminine half cums, so does your male half, splashing its happy seed into the valley that your combined bodies have made.");
        //[if (isLactating = true) 
        if (player.lactationQ() >= 100) MainScreen.text("  Your ice man reaches out with a free hand to caress your [chest], twiddling his fingers around your delicate [nipples]. After some rough pulling and flicking, pounding away all the while, he feels milk dribble onto his hand.  He catches a quick glance of the liquid before diving head-first to suckle on your milk faucets, drinking heavily of your cream as he's about to give you his.");
    ...
    public nieveIsOver(): void {
        ...
        hideMenus();
        //Nieve Not Completed
        if (Flags.list[FlagEnum.NIEVE_STAGE] < 5) {
            MainScreen.text("You wake up with a yawn and a stretch. It feels just like any other day, though a bit warmer.  Quite a bit warmer in fact.  It feels like winter's grasp on Mareth has slipped, and it's entering Spring.  Curiously, you glance over towards the snow which had been recently deposited in your camp, and note it's melting away before your eyes.");
            MainScreen.text("\n\nOh well, not a huge loss, you figure.  It was fun while it lasted.");
            Flags.list[FlagEnum.NIEVE_STAGE] = 0;
        }
        //Nieve Completed
        else {
            MainScreen.text("You wake up with a yawn and a stretch.  It feels just like any other day, though a bit warmer.  Quite a bit warmer in fact.  It feels like winter's grasp on Mareth has slipped, and it's entering Spring.  You consider the fact absently at first, but then you stop dead in your tracks.  What about Nieve?!");
            ...
            if (player.stats.cor < 50) MainScreen.text("  You even tear up yourself, sad to see such a good friend leave, and so abruptly.");
            //Corruption >50: 
            else MainScreen.text("  You even feel a little sad yourself, it's such a pity to see a good, obedient fucktoy leave.");
            MainScreen.text("  The snowman collapses on itself, melting away before your very eyes.  In a matter of hours, this entire area will be nothing but a dried up, dehydrated desert once more.  But as the jingling fades out into the distance, you vow to hold on to the tear.");
    ...
    private fixNieve(): void {
        if (Flags.list[FlagEnum.NIEVE_GENDER] == 0) {
            MainScreen.clearText();
            MainScreen.text("(There was an error with stat tracking that cleared Nieve's stats out at the end of last year's event. <b>What gender do you want Nieve to be?</b>)");
            menu();
            MainScreen.addButton(0, "Male", fixNieveGender, 1);
            MainScreen.addButton(1, "Female", fixNieveGender, 2);
        }
        else if (Flags.list[FlagEnum.NIEVE_MOUTH] == "" || Flags.list[FlagEnum.NIEVE_MOUTH] == 0) {
            MainScreen.clearText();
            MainScreen.text("(There was an error with stat tracking that cleared Nieve's stats out at the end of last year's event. <b>What were Nieve's eyes and mouth made out of?</b>)");
            menu();
            MainScreen.addButton(0, "Gems", fixNieveMouth, 0);
            MainScreen.addButton(1, "Coal", fixNieveMouth, 1);
        }
        else nieveReturnsPartII();
    }

    private fixNieveGender(arg: number = 1): void {
        Flags.list[FlagEnum.NIEVE_GENDER] = arg;
        fixNieve();
    }
    private fixNieveMouth(arg: number = 1): void {
        if (arg == 0) Flags.list[FlagEnum.NIEVE_MOUTH] = "gems";
        else Flags.list[FlagEnum.NIEVE_MOUTH] = "coal";
        fixNieve();
    }
    ...
    public returnOfNieve(): void {
        MainScreen.clearText();
        MainScreen.text("As you awake in the morning you find yourself shivering slightly.  A cool breeze sweeps over your camp, while in the distance jingling bells can be heard.  How odd.  You haven't heard bells like that since...");
        ...
        //[Next]
        menu();
        MainScreen.addButton(0, "Next", fixNieve);
    }

    private nieveReturnsPartII(): void {
        MainScreen.clearText();
        MainScreen.text("You sigh, resigning yourself to your companion's fate.  However, as you rise from the snow to return to camp, you hear a soft, muffled voice.  Perplexed, you crawl forward in the snow, frantically seeking out the source.");
        ...
        if (Flags.list[FlagEnum.NIEVE_MOUTH] == "coal") MainScreen.text("coal black");
        else MainScreen.text("glittering purple");
        MainScreen.text(" eyes shine with crystallizing tears, not entirely unlike the ones " + nieveMF("he", "she") + " left with you on your last meeting.  \"<i>I'm so glad to be back!  I was worried I would be sent somewhere else, but I had so much fun here, and I told the big man that, and then Winter came and I wasn't sure you were still here, and then it turned out I might have gone to someone else, but oh my goodness I'm so happy I came back!</i>\" Nieve belts out without pause for breath before wrapping " + nieveMF("his", "her") + " arms around you once again.");
