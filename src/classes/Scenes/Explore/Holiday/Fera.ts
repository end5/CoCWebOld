...
export default class Fera {
    ...
    public pumpkinFuck(): void {
        ...
        if (player.cumQ() < 200) MainScreen.text("Your weak load disappears into the plant's sucking gullet, wicked away as fast as you fire.");
        else if (player.cumQ() < 1000) MainScreen.text("Your thick load disappears into the plant's sucking gullet almost as fast as you're firing it, though you can see a bit of white creampie forming around the edges of your cock.");
        else {
            MainScreen.text("Your massive load floods the pumping, a deluge of semen squirting around your [cock biggest] even as the pumpkin starts to enlarge.  Every titanic spooge-squirt pumps the gourd up another level.  Soon, the rind is rubbing your belly.  A few blasts later, it's lifting you from the ground, your still-spurting cock lodged firmly in its cunt.  The suction feels so much stronger now - you're not sure you could pull out now if you wanted to, and you don't!");
        }
        if (player.cumQ() < 1000) {
            ...
            if (player.weaponName == "fists") MainScreen.text("brandishing your fists");
            else MainScreen.text("picking up your [weapon]");
    ...
    public pumpkinFuckPartII(): void {
        ...
        MainScreen.text(" as she explains, \"<i>Almost a decade ago, my sister, Marae, bolstered by Tel'Adre's Covenant, sealed me away, bound me up in barren, lifeless soil.</i>\"  Her grip tightens, drawing a drop of blood and a wince of pain from you.  \"<i>She didn't approve of my decision to throw my lot in with Lethice's army.  The bitch always did think she knew best.</i>\"  The painful cage around your package relaxes, and she goes on, \"<i>You have no idea how hard it was to grow that pumpkin to the surface... but with your more than generous donation, you gave me the life I needed to break free.</i>\"");
        ...
        if (player.lowerBody.cockSpot.count() > 1) {
            MainScreen.text(", her free hand working ");
            if (player.lowerBody.cockSpot.count() == 2) MainScreen.text("your other");
            else MainScreen.text("another");
            MainScreen.text(" dick with child-like glee");
        }
        MainScreen.text(".  \"<i>");
        if (player.race() != "human") MainScreen.text("I know what you were, virile suitor.  ");
        MainScreen.text("There's never too much of a good thing.  Of course, my champion must have the proper train of thought...</i>\"");
    ...
    public pumpkinFuckPartIII(): void {
        MainScreen.clearText();
        MainScreen.text("SLAP!  The crack of a palm on your face rouses you, a red handprint left on your cheek.  Fera smirks at you as you stir woozily in your plant-woven prison.  \"<i>Did you enjoy yourself?</i>\"  You slowly nod, feeling... liberated, free.  The situation is less worrying than before.  Fera just wanted to thank you, right?");
        MainScreen.text("\n\nThe sultry orange-haired maiden ");
        if (player.lowerBody.balls > 0) MainScreen.text("pats your sloshing sack");
        else MainScreen.text("caresses a sensitive spot beneath your dick");
        MainScreen.text(" as the vines unwind, allowing you to stand on your own [feet].  You go to wipe the cum from your nose and upper lip as soon as you're free.  It's dry.  Did any of that even happen?");
    ...
    public mountPumpkin(): void {
        ...
        //Female characters don't have an easy analogue to cum production unless we count milk production, and female gender doesn't require either breasts or lactation to play.  So I just went for high vaginal wetness, not pregnant, and likely to become pregnant if she had sex with a male creature.  I hope that works out OK.
            ...
            MainScreen.text("\n\nA faint rumbling is the only warning you get (or need).  You jump back and ready yourself for anything, ");
            if (player.weaponName != "fists") MainScreen.text("picking up your [weapon]");
            else MainScreen.text("raising your guard");
            MainScreen.text(" as if it would somehow protect your nude body.");
    ...
    public pumpkinMountingLevelII(): void {
        MainScreen.clearText();
        MainScreen.text("The pumpkin's titanic stem shivers, the mighty shaft trembling immensely before the pumpkin splits from the top causing it to topple.  The crack goes around the rind in multiple directions, causing it to split open like a flower as strings of orangey-purple goo hang wetly between the pieces for a moment.  A moment later, the sectioned fruit falls apart, at last revealing the corrupt core.  No trace of the mammoth copulation remains.");
        ...
        if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) MainScreen.text(", her free hand working your [fullChest] with glee");
        MainScreen.text(".  <i>\"There's never too much of a good thing.  Of course, my champion must have the proper train of thought...\"</i>");
    ...
    public mountPumpkinsIII(): void {
        MainScreen.clearText();
        MainScreen.text("SLAP!  The crack of a palm on your face rouses you, a red handprint left on your cheek.  Fera smirks at you as you stir woozy in your plant-woven prison.  <i>\"Did you enjoy yourself?\"</i>  You slowly nod, feeling... liberated... free.  The situation is less worrying than before.  Fera just wanted to thank you, right?");
...
player.stats.cor += 1;
        ...
        if (player.stats.int >= 40) {
            MainScreen.text(", and with that accomplished, you could head back to camp.  Now, though, you'd rather stay; it smells and looks too good to leave this place.  If you focus on your better sense, you could still leave.");
            if (player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(0, "Continue", fuckDemCorruptFlowerLips);
            else MainScreen.addButton(0, "Continue", stickAStamenInIt);
            MainScreen.addButton(4, "Leave", leaveAfterFlowerHuffing);
        }
        else {
            MainScreen.text(", but that thought slips out of your head as it fills with more of that sinfully exciting aroma, making you feel so hot, horny, and eager.  You have to get more of it, submerse yourself deeper in its cloying clutches as you watch the pretty colors drain away your worries and concerns.");
            if (player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(0, "Next", fuckDemCorruptFlowerLips);
        ...
        if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) {
            MainScreen.text("\n\nThe only ");
            if (player.stats.int >= 40) MainScreen.text("other ");
            MainScreen.text("question left to you is, are you more concerned with slipping [oneCock] into those subliminally-whispering lips or guiding a writhing stamen into your [vagina] and [asshole] so that you can take in as much nectar as possible?");
...
player.stats.cor += 1;
    ...
    public leaveAfterFlowerHuffing(): void {
        MainScreen.clearText();
        MainScreen.text("The farther you get from that glade, the more your head clears and the more you realize how close you came to disaster.  You resolve to avoid it in the future, lest you fall prey to that entrancing pollen and the hypnotic petals once more.");
        doNext(camp.returnToCampUseOneHour);
    }
    //Fuck the Lips
    public fuckDemCorruptFlowerLips(): void {
        ...
        if (player.longestCockLength() >= 20) {
            MainScreen.text("easily ramming your huge endowment");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(" straight into the erotic-looking mouth");
        }
        else if (player.longestCockLength() >= 8) {
            MainScreen.text("slipping your large endowments");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(" right into the erotic-looking mouth");
        }
        else if (player.longestCockLength() >= 4) {
            MainScreen.text("easing the top hal");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("ves");
            else MainScreen.text("f");
            MainScreen.text(" of your phallus");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("es");
            MainScreen.text(" into the erotic-looking mouth");
        }
        else {
            MainScreen.text("barely managing to slip your tip");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(" into the erotic-looking mouth");
        }
        MainScreen.text(". The contact is everything you thought it would be: surprisingly warm, soaking-wet, and squeezing just tight enough to make your pillar");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(" of lust twitch in excitement.");
        ...
        if (player.lowerBody.balls > 0) MainScreen.text("[balls] against the cushy lower lip, wreathing them in syrupy sap or nectar.  You're not really sure what the appropriate name for such a substance is, and you're well past caring.");
        else MainScreen.text("loins against the cushy, sap-slicked lips, compressing their hot, smooth surfaces against your " + player.skin() + ".  You're not really sure if sap is the appropriate name for such a substance, but you're far beyond caring at this point.");
        MainScreen.text("  The fast-diminishing light doesn't even register, as your eyes have closed as you subsume yourself in pleasure.");
    ...
    public dickBoostingFuntimes(): void {
        ...
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(" just in time to take the first thrust from your hips, burying yourself into them and immediately pulling back, setting an easy, average rhythm.  At the same time, the suction starts, causing your ");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("heads");
        else MainScreen.text("[cockHead]");
        MainScreen.text(" to swell up a little, and when the rest of your length is plowed through the tight, slippery ring, the tugging vacuum increases, plumping your dick");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(" bigger, harder, and fatter than ");
        if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("it");
        else MainScreen.text("they");
        MainScreen.text(" should be.");
        ...
        MainScreen.text(" so that you can start the cycle anew.  You love that, and this is the perfect place for that kind of behavior, isn't it? Everybody in this world wants to fuck, they just like to fight to see who gets to be on top, and you're good at winning, aren't you, pet champion?</i>\" Fera ");
        if (player.lowerBody.balls > 0) MainScreen.text("cups your [sack] tenderly");
        else MainScreen.text("tickles your taint");
        MainScreen.text(" to spur you to thrust harder while she fills your head with lurid thoughts.");
        ...
        MainScreen.text(" as you do, whispering, \"<i>Good " + player.mf("boy", "girl") + ",</i>\" into your ears as dark energies flow into you, refilling your fast-emptying reservoirs faster than they can empty.  \"<i>Don't stop yet, [name].  You're going to be doing this a lot, so I'm giving you lots of practice.</i>\"");
...
player.stats.resisted += false;
    ...
    public stickAStamenInIt(): void {
        ...
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("or ");
        MainScreen.text("a tight, dick-milking rape-mother");
        if (!player.lowerBody.cockSpot.hasCock()) MainScreen.text(", or a libidinous dick-girl, sowing your seed everywhere you can and saving your cunt for only the strongest of mates");
        MainScreen.text("?</i>\"");
        ...
        if (!player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(2, "Dick", growADickForFera);
    }
    ...
    public gapeByFeraEpilogue(): void {
        MainScreen.clearText();
        MainScreen.text("You awaken fully dressed with a familiar ache between your legs. Damn, that was a good nap.  You rise, stretching as you rub your back, sore from lying against a tree for so long.  You suppose you could hunt down a tentacle monster and see if it'd really give you a filling");
        if (player.pregnancyIncubation == 0) MainScreen.text(", but you'd rather fuck something that'll get you good and pregnant with a well-hung fuck-monster, like a minotaur!");
...
player.stats.resisted += false;
    ...
    public tightLikeAToigar(): void {
        MainScreen.clearText();
        MainScreen.text("Fera smiles.  \"<i>I had hoped we might do that, little champion.  Now, you simply relax and enjoy the pleasure that only my normal weak-willed prey are allowed to experience.  You will be the first to taste it and be allowed free.  How wonderful!</i>\"  She pets your forehead.");
