...
export default class PlantGirl {
    ...
    public plantWomanStruggle(character: Character) {
        ...
        // (struggle: failure, or Intro 1.2.3) (< 40 STR)
            ...
            if (character.gender > 0) {
                ...
                if (character.gender === 3 || (character.torso.vaginas.count + character.torso.cocks.count > 1)) DisplayText("them");
                else DisplayText("it");
            ...
            MainScreen.doNext(this.sexFailureIntro);
        }
    }
    ...
    public seriouslyGetLostPlantLady(character: Character) {
        ...
        startCombat(monster); // TODO create PlantGirl
        monster.HP -= 20;
        if (character.torso.neck.head.horns.amount > 0 && character.torso.neck.head.horns.type > HornType.NONE) monster.HP -= 15;
        MainScreen.doNext(1);
    }
    ...
    public escapeFromPlantWomanAndDeclineSex(character: Character) {
        ...
        // Failure to persuade/intimidate (SD 1.1f)
            ...
            if (!character.torso.isTaur()) DisplayText("leap");
    ...
    public trappedFlowerSexBlowjobAhoy(character: Character) {
        ...
        // (If PC has a pussy:
        else if (character.torso.vaginas.count > 0) DisplayText("take hold of you VaginaDescriptor.describeClit(character, character.torso.vaginaSpot.get(0)) and tug hard");
        // (If PC is genderless:
    ...
    public trappedPlantSexPussyGOOO(character: Character) {
        ...
        if (x < 0) x = character.smallestCockIndex();
        // (If  cock(s) are 4 inches or less)
        if (character.torso.cocks.get(x).cockLength < 4) {
            ...
            character.torso.cocks.get(x).cockLength += 7;
        }
        // (if > 4 inches)
...
// (If height is 36 inches to 72 inches:
        ...
        // Vagina:
        if (character.torso.vaginas.count > 0) DisplayText("  Cumming hard, your " + VaginaDescriptor.describeClit(character, character.torso.vaginaSpot.get(0)) + " releases a liberal stream of juices down the vine that fucks you, dissolving into the appendage as you moan in furious release.");
    ...
    public trappedPlantSexBlowMe() {
        ...
        if (character.longestCockLength() < 24) {
            ...
            DisplayText("\n\nHaving to scoot back a fair bit to enjoy her catch, the plant woman kneels against the dirty ground and attempts to take your " + CockDescriptor.describeCock(character, 0) + " into her mouth.  You feel her attempt to fit your rod into her little mouth; teeth accidentally scraping along your head, the buckling of jaws, and soft lips that are rebutted are all the signs you need to understand that your " + CockDescriptor.describeMultiCockShort(character));
    ...
    public failStruggle69() {
        ...
        if (character.torso.cocks.count > 0) DisplayText(CockDescriptor.describeMultiCockShort(character));
        else if (character.torso.vaginas.count > 0) DisplayText(VaginaDescriptor.describeVagina(character, character.torso.vaginaSpot.get(0)));
        else DisplayText(ButtDescriptor.describeButthole(character) + " as she dives nose first into the under crevice of your " + ButtDescriptor.describeButt(character) + ", rubbing the bridge of it between your cheeks as she giggles");
        DisplayText(".");
    ...
    public struggle69Plant(pt: number = 1) {
        ...
        let target: number = 0;
        if (character.gender === 1) target = 1;
        if (character.gender === 3) target = 1 + rand(2);
        if (character.gender === 2) target = 2;
        ...
        if (pt === 2) {
            ...
            // PC pussy chosen:
            if (target === 2) DisplayText(", allowing her soft lips to surround your " + VaginaDescriptor.describeClit(character, character.torso.vaginaSpot.get(0)) + ", 'trapping' it for her much anticipated assault.");
            // PC cock chosen:
            else DisplayText(".  Wasting no time with 'formalities,' she stuffs your " + CockDescriptor.describeCock(character, 0) + " into her mouth, firmly sealing her lips around your shaft and suckling along the sensitive flesh of your glans.");
        }
        // [69!, TsI: 1.6a, give her oral]
            ...
            // PC cock chosen:
            else DisplayText(" as she wastes no time with 'formalities,' stuffing your " + CockDescriptor.describeCock(character, 0) + " into her mouth, firmly sealing her lips around your shaft and suckling along the sensitive flesh of your glans.");
        }
        ...
        // Cock(s):
            ...
            // (PC is herm:
            if (character.gender === 3) DisplayText("  In reward for remembering your “duties,” a set of fingers find their way along your feminine sex and into your honey pot, her teasing of your [vagina] ensuring that no sexes on your body are needlessly left out.");
    ...
    public struggleFailPlantPussyFuckNoWay() {
        ...
        // PC is virgin:
        if (character.torso.vaginaSpot.get(0).virgin) DisplayText("want to protect your chastity, or you ");
    ...
    public victoryVsPlant() {
        ...
        // PC HP victory:
        if (monster.HP < 1) DisplayText("shoulder and sides.");
        ...
        if (monster.HP < 1) DisplayText("pain");
    ...
    public defeatOrDomPlantAndCuntFuck() {
        ...
        if (inCombat && character.HP < maxHP()) {
        ...
        if (character.torso.vaginas.count > 0) DisplayText("; your VaginaDescriptor.describeVagina(character, character.torso.vaginaSpot.get(0)) quivers with the need for release, letting loose with its own torrent of lady fluids as you moan in orgasm");
        ...
        if (inCombat)
            cleanupAfterCombat();
        else MainScreen.doNext(camp.returnToCampUseOneHour);
    }
    ...
    public victoryDefeatPlantGirlGetBJMainSCREENON() {
        ...
        if (character.torso.cocks.countType(CockType.TENTACLE) > 0 && (character.torso.cocks.countType(CockType.TENTACLE) === character.torso.cocks.count || rand(2) === 0)) {
            plantGirlPCWinGetTentacleBJ();
            return;
        }
        else {
            // [if cock size is 4-14 inches]
            if (character.torso.cocks.list[character.biggestCockIndex()].cockLength < 14) DisplayText("takes the entirety of your [cock biggest] in her mouth and down her throat.  You lurch forward as your cock hits the back of her throat, moaning intensely at the plant girl's ability to take your whole penis in her mouth.");
        ...
        // ______________________________________________________________________________
        // (if Non-goo:
        if (!character.torso.isGoo()) DisplayText(".  She pulls your dick out of her mouth and closes her eyes as she gets ready to catch the rest of your load with her mouth.  Thick globs of semen erupt from your CockDescriptor.describeMultiCockShort(character) and strike her in the face.  She herself orgasms as your torrent of semen washes over her head and chest, covering both liberally.");
        // (If goo:
    ...
    public plantGirlPCWinGetTentacleBJ() {
        ...
        if (character.torso.cocks.get(x).cockLength < 48) {
		...
		else DisplayText(BreastDescriptor.describeNipple(0));
        ...
        // [If tentacle cock is longer than 48 inches]
            ...
            if (character.torso.balls.quantity > 0 || character.torso.vaginas.count > 0) {
                ...
                else DisplayText(VaginaDescriptor.describeVagina(character, character.torso.vaginaSpot.get(0)) + " slithers");
    ...
    public beatPlantGirlDomLetsFuckAnal() {
        ...
        // (Victory, V 1.3:
        if (inCombat) {
            if (monster.HP < 1) DisplayText("The plant woman lies defeated before you, clutching the side of her head and shaking it; no doubt she is seeing stars after the smack down you laid on her.");
    ...
    public beatPlantGirlAndTakePlantCockInButt() {
        ...
        if (character.gender === 3) DisplayText(" while ");
        if (character.torso.vaginas.count > 0) DisplayText("your pussy becomes incredibly wet");
        if (character.gender === 0) DisplayText("nipples become erect");
        ...
        if (character.gender === 0) DisplayText("your body quivers from the sensation");
        DisplayText(".  ");
        // PC has balls:
        ...
        if (character.gender === 3) DisplayText(" as she also ");
        if (character.torso.vaginas.count > 0) DisplayText("massages your labia");
        if (character.gender === 0) DisplayText("massages you");
        DisplayText(".");
        ...
        if (character.gender === 0) DisplayText("raises herself to deliver a passionate kiss");
        ...
        if (character.gender === 3) DisplayText(", ");
        ...
        if (character.gender === 0) DisplayText("twitching futilely around the plant-girl's swollen seeder");
    ...
    public defeatPlantgirlBlowAndButtfuckPure() {
        ...
        if (character.torso.isNaga()) DisplayText("now exposed ");
        DisplayText(VaginaDescriptor.describeClit(character, character.torso.vaginaSpot.get(0)) + ".  The plant woman yelps in surprise as the vine makes contact with your labia, causing you to jump a little.  You turn to face the plant woman, who is blushing profusely and resting her hand on her pussy.  Looking at the vegetation, you deduce that she must be able to feel you along her vines and that these creepers may also function as penile appendages.  A grin spreads across your face as the realization hits you, and you bring the vine back to your sex and tease your vulva.  Soft moans escape the plant girl as the head of the perverted vegetation brushes past your clit, and you ");
        // (pure:
        if (character.stats.cor < 50) DisplayText("giggle");
        // Corrupted:
        else DisplayText("smirk");
        DisplayText(" at the woman's predicament.  Deciding to stop teasing the poor girl, you take the head of the creeper and push it into your pussy, edging it upwards at a gradual pace so that your partner can feel the contours of your " + VaginaDescriptor.describeVagina(character, character.torso.vaginaSpot.get(0)) + ".");
...
[If you chose just the vine, V and D 1.6vp] You slip a hand to your
	...
	(if herm)) member and begin to (fondle your CockDescriptor.describeCock(character, 0)).
...
(if vagina and/or cock: (VaginaDescriptor.describeVagina(character, character.torso.vaginaSpot.get(0))/   /and/ CockDescriptor.describeMultiCockShort(character)/
...
(VaginaDescriptor.describeVagina(character, character.torso.vaginaSpot.get(0))/ CockDescriptor.describeMultiCockShort(character)/ assholeDescript(0) ).
...
(CockDescriptor.describeMultiCockShort(character)/ VaginaDescriptor.describeVagina(character, character.torso.vaginaSpot.get(0))/assholeDescript( ) )
...
(if genderless. Vc 1.7g: prods the crack of your ass as she parts your [butt], and licks your ButtDescriptor.describeButthole(character) with her energetic tongue; oddly enough, it resembles the texture of flower petals, and having that sensation against your hole causes you to quiver.)  Her moist tongue slaps against your hole, leaving it nice and wet as she prods and lathers your hole with her tongue. )
