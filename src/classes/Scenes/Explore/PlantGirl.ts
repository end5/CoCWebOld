...
export default class PlantGirl {
    ...
    public encounterPlantWoman(): void {
        if (rand(2) == 0) {
            monster.statusAffects.add(new StatusAffect("happy", 0, 0, 0, 0)));
        }
        else if (rand(2) == 0) {
            monster.statusAffects.add(new StatusAffect("horny", 0, 0, 0, 0)));
        }
        else {
            monster.statusAffects.add(new StatusAffect("grouchy", 0, 0, 0, 0)));
        }
        MainScreen.clearText();
        if (Flags.list[FlagEnum.TIMES_MET_PLANTGIRL] == 0) {
            MainScreen.text("While out exploring the deep woods you come across a thick bush full of vines and thorns that is obstructing your path.  Strange...that wasn't there a few days ago.  You remember the perverted, sexual vegetation, but not this.  You scan the brush for a way to get through to no avail.  As you begin to turn, you hear an eerie, ominous coo which causes you to freeze, listening for further movement or noise.  After a few moments, you sigh and begin to head back the way you came... only to hit another patch of dense foliage, blocking your advance once again.");
    ...
    public plantyLadySeeWhatHappens(): void {
        MainScreen.clearText();
        MainScreen.text("You decide to not offer any resistance as the vines begin to tighten around your body.  When they have you nearly constricted, they stop suddenly.  Looking downward confirms the vines relaxing and slowly uncoiling from around your legs." + (player.stats.cor >= 66 ? "  A sense of disappointment comes to you at the sight of the retreating vegetation, robbing you of potential fun." : "") + "  Standing there in utter confusion, you scratch the back of your head and survey the thick bush that now surrounds you, when a creature appears with a soft rustle to the left!");
        ...
        menu();
        MainScreen.addButton(0, "Let's Fuck!", plantLadyLetsFuck, 1.1);
        MainScreen.addButton(1, "Decline", declinePlantSmex);
        MainScreen.addButton(4, "Escape", plantWomanStruggle);
    }

    //[Let's fuck!]
    public plantLadyLetsFuck(from: number = 1.1): void {
        MainScreen.clearText();
        MainScreen.text("Throwing caution to the wind, you decide to engage in a little 'carnal' pleasure with this creature. How to do you want to sex the plant woman?");
    ...
    public plantWomanStruggle(): void {
        ...
        if (player.stats.str >= 40) {
            ...
            menu();
            MainScreen.addButton(0, "Sex", 9999);
            MainScreen.addButton(1, "Decline", declinePlantSmex);
            MainScreen.addButton(4, "Try to Run", 9999);
        }
        //(struggle: failure, or Intro 1.2.3) (< 40 STR)
        else {
            MainScreen.text("Despite your best efforts, you can't seem to break free from the vines as they tense around your limbs and then your waist.  You bite your lower lip and brace for whatever is coming to \"greet\" you.");
            //(leads to 1.2.3m)
    ...
    public sexFailureIntro(): void {
        ...
        menu();
        MainScreen.addButton(0, "Do It!", struggleFailDoItPlantSmex);
        MainScreen.addButton(1, "Decline", declinePlantSmex);
        MainScreen.addButton(2, "Get Lost", seriouslyGetLostPlantLady);
    }

    //[Decline, or Decline 1.0]
    public declinePlantSmex(): void {
        MainScreen.clearText();
        MainScreen.text("As she returns to teasing you again with a pseudo-kiss, you pull back and shake your head.  The rejection apparent in your facial features, the plant woman stops and stares at you with glassy eyes.  You signal (as best as you can while constricted) that you aren't into this and that you need to leave. Her eyes become watery as she turns away, quietly sobbing as her vines loosen up a bit.");
    ...
    public seriouslyGetLostPlantLady(): void {
        ...
        startCombat(monster);// TODO create PlantGirl
        monster.HP -= 20;
        if (player.upperBody.head.horns > 0 && player.upperBody.head.hornType > HornType.NONE) monster.HP -= 15;
        doNext(1);
    }
    ...
    public struggleFailDoItPlantSmex(): void {
        MainScreen.clearText();
        MainScreen.text("As she goes in for another pseudo kiss you lurch forward and connect with your lips, pressing them hard against her and fiercely returning her intimate gesture. She responds by snaking her tongue in your mouth, slapping hers against yours and twisting them around like a frustrated child with a knotted shoe lace.");
        ...
        //SHE RIDES UR DICK
        if (player.lowerBody.cockSpot.hasCock()) choices[choices.length] = trappedPlantSexPussy;
        //SHE BLOWS YA
        if (player.lowerBody.cockSpot.hasCock()) choices[choices.length] = trappedPlantSexWantsToBlowYa;
        //SHE WANTS UR DICK IN HER ASS
        if (player.lowerBody.cockSpot.hasCock()) choices[choices.length] = strugglePlantLadyFailAnal;
        //SHE WANTS 69 - ANY GENDER
        if (player.gender > 0) choices[choices.length] = failStruggle69;
        //SHE FUCKS YER CUNT, YA GIT
        if (player.lowerBody.vaginaSpot.hasVagina()) choices[choices.length] = struggleFailPlantFucksPussy;
    ...
    public escapeFromPlantWomanAndDeclineSex(): void {
        ...
        MainScreen.text("You stare at the plant woman and shake your head.  Sorry, you're not interested.  The plant woman continues to stare at you, uncertain of what you are saying.  You gesture to the monster that ");
        if (player.stats.cor < 50) MainScreen.text("you are taking your leave and don't wish for any trouble.");
        else MainScreen.text("following or attacking would be a bad idea.");
        //(Connects to failure to persuade)
        //(Success, SD 1.1s)
        if (9999 == 9999) {
            MainScreen.text("\n\nThe monster, from what you can tell, deliberates your gesture of ");
            if (player.stats.cor < 50) MainScreen.text("walking away");
            else MainScreen.text("intimidation");
            MainScreen.text(" and shrugs.  She promptly flees back into the woods.  You are left wondering just how messed up the local fauna and wildlife in Mareth is before promptly returning to camp.");
            doNext(camp.returnToCampUseOneHour);
        }
        //Failure to persuade/intimidate (SD 1.1f)
        else {
            MainScreen.text("\n\nThe plant monster scowls at you when you attempt to leave.  You try to ");
            if (player.stats.cor < 50) MainScreen.text("calm her down");
            else MainScreen.text("remind her that attacking you will earn her a world of pain");
            MainScreen.text(", but she isn't buying any of it.  You ");
            if (player.stats.cor < 50) MainScreen.text("shrug and turn you back, beginning the long trek back towards camp");
            else MainScreen.text("take a step forward and ready your weapon");
            MainScreen.text("; she responds by attempting to whip you with one of her vine appendages, causing you to ");
            if (!player.lowerBody.isTaur()) MainScreen.text("leap");
            else MainScreen.text("gallop");
            MainScreen.text(" out of the way.  She's misses, but it's obvious she will not take no for an answer. Looks like you have a fight on your hands...");
            //(leads to Fight 1.0]
            startCombat(monster);// TODO create PlantGirl
            doNext(1);
        }
    ...
    public fuckMyAssPlantLadyWhileImTrapped(): void {
        MainScreen.clearText();
        MainScreen.text("You look up into the plant girl's eyes and smile, signalling that you're ready for what she has in mind.  She returns your lustful leer and presses her lips against yours, tongues once again darting around in search of each other as the plant girl reaches down to both your chest and ");
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(CockDescriptor.describeMultiCockShort(player));
        if (player.gender == 3) MainScreen.text(" and ");
        ...
        MainScreen.text(", and begins to caress and ");
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("stroke");
        else MainScreen.text("finger");
        MainScreen.text(" you.");

        MainScreen.text("\n\nAs she takes ");
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("[oneCock]");
        else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)));
        ...
        if (player.analCapacity() < 25) MainScreen.text(", but the bulbous member sliding inside of you hurts nevertheless");
        MainScreen.text(".");
        ...
        if (player.analCapacity() < 20) MainScreen.text("  You moan again from the pain and clamp down, causing your captor to
shudder from the sensation of your [asshole]'s tightness.  Once the floral creature regains her composure, she begins to pump in and out of your ass at a gradually faster pace.");
	//PC asshole is loose:  
        ...
        if (player.shortestCockLength() < 48) {
            MainScreen.text("\n\nYour ");
            if (player.lowerBody.balls > 0) MainScreen.text("[balls] and ");
            MainScreen.text(CockDescriptor.describeCock(player, player.shortestCockIndex()) + " flap");
            if (player.lowerBody.balls > 0) MainScreen.text("s");
            MainScreen.text(" against your thighs as she fucks you harder and harder as her hand finds its way down to your prick");
            if (player.totalCock() > 1) MainScreen.text("s");
            MainScreen.text(".  The creature twists and jerks at the head of your cock and works diligently to stimulate your rock hard " + CockDescriptor.describeMultiCockShort(player) + ", intent on making you moan out for her.");
        }
        //PC has a vagina: 
        ...
        //(if long tongued: 
        if (player.upperBody.head.face.tongueType > TongueType.HUMAN) MainScreen.text("  In a cute but spiteful way, your tongue is pulled forcefully out of your mouth and into her lips as she pulls her head back; the plant woman doesn't get too far from your face before you run out of tongue to give.  This suits her just fine as she takes to sucking on your incredibly long tongue in a manner similar to fellatio.  Pursed on the bottom of yours, she darts her head back and forth along the length of it, edging further into her mouth as she nears closer and closer to your own.  Her dominance without question, she reaches your lips and slips her tongue in, sealing it in with a firm kiss.");
        //Not long tongued: 
        else MainScreen.text("  Your tongue is viciously attacked by the sudden intrusion of her own.  However, you are more than happy to return the favor as you counter attack, swapping each other's saliva in the process.");

        MainScreen.text("The feminine creature's hand then finds its way ");
        if (player.lowerBody.balls == 0) MainScreen.text("up");
        else MainScreen.text("down");
        MainScreen.text(" to your ");
        if (player.lowerBody.balls == 0) MainScreen.text(BreastDescriptor.describeNipple(0));
        else MainScreen.text("[balls]");
        MainScreen.text(", massaging and caressing them gently as she moves to kissing at your neck.  Her thrusts become shorter in length as her breathing becomes frantic, signaling that her orgasmic peak is near. The plant woman ");
        //PC has hair: (grabs you by the hair)
        if (player.upperBody.head.hairLength > 0) MainScreen.text("grabs you by the hair");
        //PC doesn't have hair: embraces you)
        else MainScreen.text("embraces you");
        MainScreen.text(" as she cries out in a fierce moan, her cock convulsing as she thrusts wildly against you and planting her hot seed deep into you.  The sensual act finally over, your head droops down as you pant heavily; the plant girl slumps her head down onto your shoulder and uses you to hold herself up.  She's breathing heavily in your ear while regaining some of her energy and composure.");
    ...
    public trappedFlowerSexBlowjobRejection(): void {
        MainScreen.text("You push against the plant girl's thighs and shake your head, telling her that you are interested in THAT. You comment on how you wouldn't mind playing with something else though, as you begin to stroke your finger around the plant woman's labia.  She stops and ponders for a moment, and then giggles as her appendage disappears back into her body.");
    ...
    public trappedFlowerSexBlowjobAhoy(): void {
        MainScreen.clearText();
        MainScreen.text("\"<i>MHHMMpffffrmPH!</i>\ you mumble as her cock hits the back of your mouth, the plant creature clearly intent on jumping into the rougher sex sooner rather than later. The plant girl giggles as you struggle to breathe and not vomit from the violent intrusion. Placing both of her hands on the back of your head she begins to pump her hips, gliding her cock along your (tongue descript). You gasp for air as you try to push yourself off of the girl and get some room to breathe, but she has a pretty firm grip. She continues thrusting, but is increasingly annoyed that you aren't putting much effort into it right now.  A pair a vines ");
        //(If PC has balls: 
        if (player.lowerBody.balls > 0) MainScreen.text("wrap themselves around your ([balls] and begin to squeeze");
        //(If PC has a cock(s): 
        else if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("wrap themselves around your " + CockDescriptor.describeMultiCockShort(player) + " and begin to squeeze");
        //(If PC has a pussy: 
        else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("take hold of you VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) and tug hard");
        //(If PC is genderless: 
        else MainScreen.text("take hold of your [nipples], fiercely pulling on them to try and convince you to put some work into it");
        MainScreen.text(".");
...
player.stats.resisted += false;
    ...
    public trappedPlantSexPussy(): void {
        MainScreen.clearText();
        MainScreen.text("Staring down at your " + CockDescriptor.describeMultiCockShort(player) + ", the plant woman's eyes become fixated and betray a sense of longing, of curiosity, and eagerness.  Approaching your well restrained body, she takes [oneCock] into her soft hand and brushes it's sensitive head against her smooth pelvis, fairly close to the creature's moist and sweet smelling pussy.  Proceeding to wrap its arms around you, the plant woman takes [oneCock] in-between her legs and against her clit, signaling the creature's intent to plunge its hungry cunt down on your dick.");
    ...
    public trappedPlantSexPussyGOOO(): void {
        MainScreen.clearText();
        MainScreen.text("The vines around you begin to shift, and you fear that she might have misinterpreted your preferences for sex, gritting your teeth as you await the inevitable violation of your body.  The continuing sound of cracking vines and growth intrigues you enough to open your eyes, witnessing both yourself and the plant woman being lifted off of the ground by her cabal of creepers.  Lifting you further and further into the air, the vegetation draw her close to your restrained, naked body.  The floral beauty's petal like vagina is soaked in anticipation and glistening in the warm glow of the sun.");
        let x: number = player.cockThatFits(9999); //Undefined capacity for now.
        if (x < 0) x = player.smallestCockIndex();
        //(If  cock(s) are 4 inches or less)  
        if (player.lowerBody.cockSpot.get(x).cockLength < 4) {
            MainScreen.text("\n\nThe plant woman goes to massage your " + CockDescriptor.describeCock(player, x) + ", but stops when she sees how small you are.  Rather than dumping you on your ass, one of the vines that holds you wraps itself around [eachCock] and then proceeds to pierce the sensitive flesh of your glans with what you assume are barbs.  Unconsciously, you scream out in pain; it isn't the worst agony you have ever felt, but the sharp stinging that floods your sensitive member is enough to be a mood killer. In increasing agony, you gaze down at your " + CockDescriptor.describeMultiCockShort(player) + " to witness your member");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(" swelling up. The head");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(" of your cock");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(" swell");
            if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("s");
            MainScreen.text(" to what seems to be double ");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("their ");
            else MainScreen.text("its ");
            MainScreen.text("original size, and your unsubstantial shaft");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s gain");
            else MainScreen.text(" gain");
            MainScreen.text(" some length in the process.  Is this some kind of allergic reaction?  A chemical?  Some kind of hormone? Regardless of how this is done, your once insignificant penis");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("es are now ");
            else MainScreen.text(" is now ");
            MainScreen.text("of sufficient length to penetrate this creature; though, you suspect the effects are temporary.");
            //(cock size temporarily increased to 7 inches)
            player.lowerBody.cockSpot.get(x).cockLength += 7;
        }
        //(if > 4 inches) 
        else MainScreen.text("\n\nShe presses her body against yours, causing your rock hard CockDescriptor.describeMultiCockShort(player) to jut upward against her leafy stomach.");
        MainScreen.text("\n\nRaising herself ever so slightly, the plant woman wraps her legs around and straddles you as she lowers her pussy onto the tip of " + CockDescriptor.describeCock(player, x) + ".  The female perversion of nature slowly takes the entirety of your cock in her, causing your prick to sensually ease through her tight cunt like a mole crawling through the fertile earth.  The sensation is too much, causing you to shudder and twitch uncontrollably.  Your dominant partner rests her ");
	//(If height is 36 inches to 72 inches: 
	else if (player.tallness < 72) MainScreen.text("raises her face to yours ");
        //If height is greater than 72 inches, but less than 96 inches: 
        else if (player.tallness < 84) MainScreen.text("head against your [chest] ");
        //(If Height is less than 96 inches: 
        else if (player.tallness < 96) MainScreen.text("forehead against your chest, sniffing you intensely as she takes in your musk.  The creature of flora and warped flesh moans and shivers as she acquires your " + player.mf("manly", "feminine") + " scent, and ");
        //(If height is greater than 96 inches: 
        else MainScreen.text("places her head against your stomach, eyes closing as the creature takes in the rising and descent of your stomach ");
        MainScreen.text(" as she slowly begins to work her hips, grinding the innards of her hot and moist sex against your " + CockDescriptor.describeCock(player, x) + ".");
        //(if one cock:
        if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("  The plant creature begins to speed up her thrusting against your cock, eager to drive you over the edge with the tightness of her hole.");
        else {
            //(If multi-cocked: 
        ...
        //(Multi-cocked: 
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("  Buried deep in the makeshift vaginas, [eachCock] thrusts forward and eagerly deposits the nourishment the creature craves.");
        //Vagina: 
        if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Cumming hard, your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " releases a liberal stream of juices down the vine that fucks you, dissolving into the appendage as you moan in furious release.");
    ...
    public trappedPlantSexWantsToBlowYa(): void {
        MainScreen.clearText();
        MainScreen.text("The plant woman looks your body up and down, seemingly torn between what tickles her fancy and how she can take your body.  Her almost childlike darting of her eyes stops when she lays her sight on your CockDescriptor.describeMultiCockShort(player), sending an aroused shiver through her body as she appears to finalize her decision.  Like a child walking to the local stream for a swim, she proceeds towards you in an almost skipping like fashion.  Reaching your naked body, she promptly sinks to her knees and takes your " + CockDescriptor.describeMultiCockShort(player) + " into her hand");
        //(if more than 4 cocks or PC cocks are more than 10 inches in diameter combined:
        if (player.lowerBody.cockSpot.count() >= 4 || player.totalCockThickness() >= 10) MainScreen.text("... or she tries to, as she fumbles with your freakish bundle of dicks.  Unable to fit them all into her hand she shrugs");
        MainScreen.text(", staring up at you with her leafy hair semi-obstructing her flirtatious and suggestive eyes.");
    ...
    public trappedPlantSexDontBlowMe(): void {
        MainScreen.clearText();
        MainScreen.text("It may seem she wants to take your [oneCock] and suck on it like a sugar cane, but you aren't keen on her choice of how to pleasure you.  Remaining flaccid, you stare down at the plant woman and tell her you aren't interest in a little oral love for your " + CockDescriptor.describeMultiCockShort(player) + ", aware of the fact that she can't understand you.  Despite your words likely sounding like jibberish, the plant woman understands your intent and motions for you to make up your mind; given the slightly annoyed expression on her face, you had better make it quick.");
    ...
    public trappedPlantSexBlowMe(): void {
        MainScreen.clearText();
        MainScreen.text("The thought of getting a blowjob from this rather attractive... um... plant lady excites you greatly, springing your flaccid maleness from it's slumber.  Before the plant woman's very eyes, [eachCock] expands like heated metal; only in a much more dramatic and erotic fashion.");
        //(if cock(s) are 3-24 inches:)
        if (player.longestCockLength() < 24) {
            MainScreen.text("  Now staring her straight in the eyes, your ");
            MainScreen.text(CockDescriptor.describeMultiCockShort(player) + " stare");
            if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("s");
            MainScreen.text(" defiantly at the plant woman; erect in a such a fashion that suggests insult at the attractive creature before ");
            if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("it");
            else MainScreen.text("them");
            MainScreen.text(" that will be tending to them today.  Eager to rise to said challenge, the plant woman attempts to take your " + CockDescriptor.describeMultiCockShort(player) + " into her mouth and ");

            //(if cock(s) do not exceed 6 inches in total diameter:)
            if (player.totalCockThickness() < 6) {
                MainScreen.text(" manages to fit ");
                if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("it");
                else MainScreen.text("them");
                MainScreen.text(" into her mouth with little difficulty");
                if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(".  What a champ");
            }
            //if total cocks exceed 6 inches total diameter, but up to two cocks fall under 6 inches: 
                ...
                MainScreen.text(" manages to take two of your pricks into her mouth without dislocating a jaw or showing any obvious of discomfort");
            }
            else MainScreen.text(" manages to swallow your dick into her mouth");
            MainScreen.text(".");
            //(If cock count is more than two: 
            if (player.lowerBody.cockSpot.count() > 2) {
                MainScreen.text("  The rest of your left out cocks ache for her touch; as she takes the two cocks she has further into her mouth, she raises her arms and attempts to take hold of your members, stroking them as best as she can as she works you with her mouth");
                //(cock(s) are 3-12 inches: 
                if (player.lowerBody.cockSpot.list[player.thinnestCockIndex()].cockLength < 12) MainScreen.text(" against her face, while planting her hands against your thighs");
                //Cocks are 13-24 inches: 
                else MainScreen.text(" in-between her shoulder and neck, using them both to form a tight hole for your other cocks to fuck");
                MainScreen.text(".");
            }
            //		(jump to Continued... (“normal” dick sized oral) )
            //Continued... (“normal” dick sized sexing)
            MainScreen.text("With your member");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(" inside of her mouth, she begins the long awaited sucking and stimulation; starting out slowly at first, she slides the top of her tongue along the bottom of your prick");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(", going so gradual that you believe she is savoring the taste of you in her mouth.  To your surprise, she pulls back and lets your cock");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(" pop out of her maw, smacking and licking her lips as she grunts her approval like someone who has just enjoyed a fine meal.  Shaking her now intrusive hair out of the way of her eyes, she darts her head forward and takes your ");
            if (player.lowerBody.cockSpot.count() >= 2) MainScreen.text("two ");
            MainScreen.text("saliva covered member");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(" into her mouth; with little regard for her gag reflex, she raises her hands on your ");
            if (player.lowerBody.cockSpot.count() >= 4) MainScreen.text("upper shafts");
            else if (player.lowerBody.cockSpot.count() >= 3) MainScreen.text("upper shaft");
            else MainScreen.text("[hips]");
            MainScreen.text(" as she better positions herself for the entirety of your ");
            MainScreen.text("cock");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s as they make");
            else MainScreen.text(" as it makes");
            MainScreen.text(" contact with the back of her throat.  You feel a bit of pre-cum leak out from the tip");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(" of your ");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("two ");
            MainScreen.text("dick");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(" and ooze against the back of her throat, causing her to moan in delight as your sticky fluids coat her throat.");

            //(If Naga: 
            if (player.lowerBody.isNaga()) MainScreen.text("\n\nYour snake tail finds its way to her body and begins to wrap itself around her torso, cusping her breasts tightly as you work your way up her body.  With her firmly in your grip, you edge the tip of your tail against the back of her head and 'encourage' her to mimic your motions, helping her time her movements with your pleasure.");
            //If Goo legs: 
                ...
                if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
                MainScreen.text(" as you assert a degree of control and influence on your pleasuring.  The sticky mess that is your body solicits a series of lustfully moans from the woman who tends to your loins with her mouth.");
            }
            MainScreen.text("\n\nBobbing her head back and forth against you, she sucks and roughs up your cock");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(" with her energetic efforts");
            if (player.lowerBody.cockSpot.count() == 1) MainScreen.text(".");
            else {
                MainScreen.text(", while she cheek fucks the remainder of your " + CockDescriptor.describeMultiCockShort(player) + ".");
            }

            MainScreen.text("\n\nHer rough treatment doesn't end there; taking a firm grip of your ");
            if (player.lowerBody.balls > 0) MainScreen.text("[balls]");
            else {
                MainScreen.text("shaft");
                if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            }
            //(If balls are present: 
            if (player.lowerBody.balls > 0) {
                MainScreen.text(" and the base of your shaft");
                if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            }
            MainScreen.text(", she squeezes with considerable force, almost as if she is attempting to bottle up pressure in your ");
            if (player.lowerBody.balls > 0) MainScreen.text("[sack]");
            else MainScreen.text("loins");
            MainScreen.text(" so that your creamy contents will explode forth into her craw.  Like a knotted up hose line, your cock");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(" begin");
            if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("s");
            MainScreen.text(" to ache due to the irritating discomfort in your shaft");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text("; you're unsure if you will be able to achieve orgasm with such a tense grip on your spear");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text("!  In a rhythmic fashion, the creature ascends your member");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(" and promptly descends back downward, flooding your nerves with pleasure and a growing need to cum as she darts back and forth like a grounded raven chasing after a worm.");
            ...
            if (player.lowerBody.balls > 0) {
                MainScreen.text("balls and shaft");
                if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            }
            else {
                MainScreen.text("shaft");
            ...
            MainScreen.text(" tightens considerably;  with gusto, she starts stroking the base of your shaft like a squeeze tube, eager to aid your sperm in their journey to her stomach as they unknowingly rush to their certain doom.  Bleat red, her face compresses against your loins as she takes load after load into her throat, moaning like a wanton harlot and swallowing every once in a while to ensure that there will be room for more splooge in her mouth.  Just when you think she is going to injure you with her fierce grip, she lets you go; and not a moment too soon it seems, judging from the redness in your ");
            if (player.lowerBody.balls > 0) MainScreen.text("scrotum's");
            else {
                if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("cock's");
                else MainScreen.text("cocks'");
            }
            MainScreen.text(" skin.  Content with your 'donation', the plant woman releases your cock");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(" from her mouth with a loud pop; leaving trails of drool and semen on her chin as she does so.  The vines holding you retract from around your limbs and allows you to suit up and leave, whilst she cradles her sated stomach and hums to herself.");
            player.orgasm();
        }
        else {
            //(If all cocks exceed 24 inches, and two to three cocks measure 6 inches in diameter or less total: 
            MainScreen.text("  Despite having to position herself a considerable distance away from your body, the plant woman manages to take [oneCock] and fits it into her mouth.  Due to the distance between you two, the plant woman has to ");
            if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("prop some of your length up on her average body to prevent it from straining towards the ground.");
            else MainScreen.text("prop your other dicks up with her average body to prevent them from hitting the dirt covered ground.  Obviously she has plans for them, and doesn't want her prize catches getting dirty.");
            //(jump to Continued... (huge dick sized oral) 
            //	Continued... (huge dick sized oral)
            //(All cocks exceed 4  inches in diameter individually: 
            MainScreen.text("\n\nHaving to scoot back a fair bit to enjoy her catch, the plant woman kneels against the dirty ground and attempts to take your " + CockDescriptor.describeCock(player, 0) + " into her mouth.  You feel her attempt to fit your rod into her little mouth; teeth accidentally scraping along your head, the buckling of jaws, and soft lips that are rebutted are all the signs you need to understand that your " + CockDescriptor.describeMultiCockShort(player));
            if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("is");
            else MainScreen.text("are");
            MainScreen.text(" much too large to fit inside of her mouth.  Undeterred by your freakish large member");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
            MainScreen.text(", the plant woman smothers her face against your " + CockDescriptor.describeCock(player, 0) + ", allowing your pre-cum to lather up her face well. Stopping to sheepishly grin up at you, you see that the plant woman has trails of pre-cum connecting between her face and the head of your dick.  Like a kitten with a ball of string, she chases after the strands of pre-cum and attempts to catch them in her mouth.  You grunt and cough at her childish display, to remind her of what she's SUPPOSED to be doing; odd, coming from someone who's trapped in a vine prison.  Embarrassed, the plant woman nods her head apologetically, and takes to your " + CockDescriptor.describeCock(player, 0) + " with ferocious effort, smearing her face along the head of your cock while dragging her tongue along it with wanton abandon.  You groan at the sensation, and tense up as your body is fluid with relaxing sexual chemicals, easing you up and getting you ready for the pleasure that follows.");
            ...
            MainScreen.text("\n\nWithout noticing, your orgasm arrives and launches forth from your " + CockDescriptor.describeMultiCockShort(player) + ", with enough force to startle the plant woman as it slaps her across the face.  A series of passionate moans rise up from under your dicks as the creature is liberally covered in your steamy jism.  Her tongue now free from your urethra, you breathe a sigh of relief as you finally get to relax; the sensation of tickling gone from your body.  Closing your eyes, you can't help but slump to the ground, mentally drained as the plant woman lunges after your torrent of cum like an orphan to food. With the last shot of cum, the plant woman rises to get feet, ");
            if (player.cumQ() < 500) MainScreen.text("covered");
            else MainScreen.text("drenched");
            MainScreen.text(" in your salty seed as she grins like a fool; obviously dumbfounded at her sheer dumb luck to find  a virile specimen such as yourself. The vines holding you retract, allowing your drained body to slowly fall to the ground. Panting heavily, you lie stomach first against the ground and collect your composure as the plant woman walks up to you and strokes your back and hair. She stays at your side until you muster the will and energy to pull yourself to your feet; at which point she helps you to your [armor] and helps you to put it back on. With a smile, she sees you off as you head back to camp.  “Strange that she didn't leave you and let something else have fun with me”, you wonder in your head as you leave the deep woods.");
            player.orgasm();
        }
        doNext(camp.returnToCampUseOneHour);
    }
    ...
    public plantAnalGivingTheDick(): void {
        MainScreen.clearText();
        MainScreen.text("The sight of this nice, ample ass and the thought of fucking a supple  little pucker piques your " + CockDescriptor.describeMultiCockShort(player) + ", causing ");
        if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("it");
        else MainScreen.text("them");
        MainScreen.text(" to become semi-erect.  As you reach her full ass, your prick");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s poke");
        else MainScreen.text(" pokes");
        MainScreen.text(" at her soft behind, signaling to her that now is the time guide you into her tight pucker.  Slowly, she edges your cock against the tight opening of her asshole.  Realizing that the asshole ");
        if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("it");
        else MainScreen.text("they");
        MainScreen.text(" will hopefully penetrate is a winner in the tightness department, your " + CockDescriptor.describeMultiCockShort(player) + " surge");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(" with vigor and anticipation.   The plant woman can barely contain her surprise as your dick");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(" become");
        if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("s");
        MainScreen.text(" rock hard and push");
        if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("es");
        MainScreen.text(" against her butt hole, her hand shooting up her mouth to contain a giggle.  Looking back over her shoulder at you, she smirks as she pushes against your cock head");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(", easing ");
        if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("it");
        else MainScreen.text("them");
        MainScreen.text(" into her tight pucker.  With a sudden and rough slide forward your cock");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(" satisfyingly penetrate");
        if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("s");
        MainScreen.text(" her anus, leading to a slight gasp as the plant woman takes you inside of her.");

        MainScreen.text("\n\nKeeping her comfort in mind, the plant creature stares at the trunk in front of her and takes her time as she gradually eases more and more of you inside of her hot innards.  You grunt as the walls of her ass painfully yield to your spear(s) of lust;  The rough stretching of her contours along your glans continuing until she loosens up, to where both of you can be comfortable with the act.  Throwing caution to the wind, the plant woman “tests the waters” by bounces her ass off of your thighs, eventually increasing in tempo as she adapts to the " + CockDescriptor.describeCock(player, 0) + " inside of her.  The initial throes of anal sex that your captor inflicts on herself begins to overwhelm your cock");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(", causing you to shudder in delicious pleasure as you moan loudly.");
        ...
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(" as she weakly bounces against you, squeezing down on you with just the right amount of force that it drives you past the point of no return, unleashing a ");
        //Cum output low: 
        if (player.cumQ() < 20) MainScreen.text("small amount of semen");
        else if (player.cumQ() < 250) MainScreen.text("generous amount of cum");
        else MainScreen.text("flood of sticky spunk");
        MainScreen.text(" from your cock");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(" and into her tight asshole.");

        //(If there are free cocks: 
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("\n\nThe rest of your " + CockDescriptor.describeMultiCockShort(player) + " erupt as well, coating her butt and thighs in sticky jizz.  Like a true slut, the plant woman grabs your ejaculating prick and crams it into her fuck-hole, her pussy milking you of all the cum you can muster up.");
        ...
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
        MainScreen.text(" from her body.  Surprisingly, no cum (yours or otherwise) leaks out of her love button... weird.  With your lust quenched, you decide to head back to camp and get back to... whatever you were doing.  As you put your [armor] back on, the plant woman slips in quietly and kisses you tenderly on the neck.  Smiling, you wave to the delicate plant woman and make for camp.");
        player.orgasm();
        doNext(camp.returnToCampUseOneHour);
    }
    ...
    public failStruggle69(): void {
        ...
        else MainScreen.text(ButtDescriptor.describeButthole(player) + " as she dives nose first into the under crevice of your " + ButtDescriptor.describeButt(player) + ", rubbing the bridge of it between your cheeks as she giggles");
        MainScreen.text(".");
        ...
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(CockDescriptor.describeMultiCockShort(player));
        else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("[clit]");
        else MainScreen.text("[asshole]");
        MainScreen.text(" with her hands as she moves in for the 'kill'.");

        menu();
        //[No way!, leads to TsI: 1.6r]     [69!, eat her pussy, leads to TsI: 1.6a]    [69! give her oral, leads to TsI: 1.6a]
        MainScreen.addButton(0, "No Way!", nowayStruggle69Plant);
        MainScreen.addButton(1, "69 EatPuss", struggle69Plant, 2);
        MainScreen.addButton(2, "69 EatDick", struggle69Plant, 1);
        MainScreen.addButton(3, "69 EatAll", struggle69Plant, 3);
    }
    //[No way!, TsI. 1.6r]
    public nowayStruggle69Plant(): void {
        MainScreen.clearText();
        MainScreen.text("The dizzying sensation of being hung upside down like a hunter's catch in a tree is bad enough; the idea of this thing forcing her fuck hole against your face and licking your genitalia is more than enough for you to try and break free.  Thrashing around in the vine prison like a fly caught in a spider web, you try to show the plant woman that you are not okay with her choice in 'having fun' with you.  Your erratic and frenzied rejection dawns on the plant woman, and she pouts as she dejectedly flips you up right.  Her stare carries with it a plea for you to reconsider, but you ensure that you will not be swayed on this matter.  With a sad expression on her face, she stares down at the ground, lamenting your denial; raising an eyebrow, you call out the plant creature and assure her through your ");
        ...
        MainScreen.text(" that you still want to fool around.  Conveying your preference, you gesture to her that she should... ");
    ...
    public struggle69Plant(pt: number = 1): void {
        ...
        if (pt == 2) {
            MainScreen.text("Abandoning yourself to the plant woman's desires, you proceed to stretch your tongue and take her sweet cunt, allowing her soft sex to drag across it.  The soft tongue of your lover against your ");
            if (target == 2) MainScreen.text("[clit]");
            else MainScreen.text("[cock]");
            MainScreen.text(" flushes your body with arousal as she teases you");

            //PC pussy chosen:
            if (target == 2) MainScreen.text(", allowing her soft lips to surround your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + ", 'trapping' it for her much anticipated assault.");
            //PC cock chosen: 
            else MainScreen.text(".  Wasting no time with 'formalities,' she stuffs your " + CockDescriptor.describeCock(player, 0) + " into her mouth, firmly sealing her lips around your shaft and suckling along the sensitive flesh of your glans.");
        }
        //[69!, TsI: 1.6a, give her oral] 
        else if (pt == 1) {
            MainScreen.text("Abandoning yourself to the plant woman's desires, you proceed to stretch your tongue and take her sweet, drooling cock along it, allowing her pre to seep along your mouth muscle and ignite your taste buds with her excitement. All too happy that you've chosen the rock hard prick between her hips, the plant woman is eager to reciprocate");
            //PC pussy chosen: 
            if (target == 2) MainScreen.text(" as she allows her soft lips to surround your [clit], 'trapping' it for her much anticipated assault.");
            //PC cock chosen: 
            else MainScreen.text(" as she wastes no time with 'formalities,' stuffing your " + CockDescriptor.describeCock(player, 0) + " into her mouth, firmly sealing her lips around your shaft and suckling along the sensitive flesh of your glans.");
        }
        //[69!, TsI: 1.6a, service both]
            ...
            if (target == 2) MainScreen.text(" as she allows her soft lips to surround your [clit], “trapping” it for her much anticipated assault.");
            //PC cock chosen: 
            else MainScreen.text(" as she wastes no time with 'formalities,' stuffing your [cock] into her mouth, firmly sealing her lips around your shaft and suckling along the sensitive flesh of your glans.");
            MainScreen.text("  Not to be an inconsiderate 'guest,' you decide to give a little love to her clit as well, soliciting a girlish little squeal of surprise for your lover.  Seems she lucked out this time with such an open-minded partner.");
        }
        ...
        if (target == 2) {
            ...
            if (player.lowerBody.cockSpot.hasCock()) {
                MainScreen.text("\n\nIf that wasn't bad enough, the unsporting woman also takes to your " + CockDescriptor.describeMultiCockShort(player) + " and strokes ");
                if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("it");
                else MainScreen.text("them");
                MainScreen.text(" teasingly.  The pompous creature gives you a stare down at your form and grins at the “superior” position she thinks she has you in.");
            }
            ...
            if (player.lowerBody.cockSpot.hasCock()) {
                MainScreen.text("  With your feminine release comes a slightly painful build up of pressure in your loins; then, the hot and heavy eruption of cum in the air as your prick");
                if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s explode");
                else MainScreen.text(" explodes");
                MainScreen.text(", coating the floral vixen in your seed.");
            }
        ...
        //Cock(s): 
            ...
            if (pt == 1) MainScreen.text("cock");
            else if (pt == 2) MainScreen.text("love button");
            else MainScreen.text("love button and cock");
            MainScreen.text(", trying your best to guide your tongue around her ");
            if (pt == 2) MainScreen.text("clit and vagina");
            else if (pt == 1) MainScreen.text("glans and shaft");
            else MainScreen.text("clit, vagina, glans, and shaft");
            MainScreen.text(" while you hang upside down.  No doubt loving every second of this, the plant woman starts gyrating and grinding her sex against your face, moaning and whimpering between mouthfuls of cock as you ");
            if (pt == 2) MainScreen.text("probe her depths");
            else MainScreen.text("do your best to service the stiff prick in your mouth");
            MainScreen.text(".");

            //(PC is herm: 
            if (player.gender == 3) MainScreen.text("  In reward for remembering your “duties,” a set of fingers find their way along your feminine sex and into your honey pot, her teasing of your [vagina] ensuring that no sexes on your body are needlessly left out.");
    ...
    public struggleFailPlantPussyFuckNoWay(): void {
        MainScreen.clearText();
        MainScreen.text("Whether it's because you ");
        //PC is virgin: 
        if (player.lowerBody.vaginaSpot.get(0).virgin) MainScreen.text("want to protect your chastity, or you ");
    ...
    public victoryVsPlant(): void {
        MainScreen.clearText();
        //(Victory, V 1.1L):
        MainScreen.text("The plant woman sinks to her knees, clutching her ");
        //PC HP victory: 
        if (monster.HP < 1) MainScreen.text("shoulder and sides.");
        else MainScreen.text("breasts and pussy, twisting at her teats and finger herself like a woman possessed.");
    ...
    public defeatOrDomPlantAndCuntFuck(): void {
        ...
        if (player.lowerBody.cockSpot.count() > 1) {
            MainScreen.text("  Your extra endowment");
            if (player.lowerBody.cockSpot.count() == 2) MainScreen.text(" flaps");
            else MainScreen.text("s flap");
            MainScreen.text(" wildly against her thighs and ass, quietly acknowledging the call to arms and prepare to do their part.");
        }
        ...
        MainScreen.text(".  The sensation is too much, and you cry out as you flood her vagina with your hot semen.  The plant woman senses your orgasmic release and begins to squeeze your sides with her legs, as if pumping you for all of your seed.  Your ");
        if (player.lowerBody.balls == 0) MainScreen.text(CockDescriptor.describeMultiCockShort(player));
        else MainScreen.text(BallsDescriptor.describeBalls(true, true, player));
        MainScreen.text(" acknowledge");
        if (player.lowerBody.balls == 0 && player.lowerBody.cockSpot.count() == 1) MainScreen.text("s");
        MainScreen.text(" her desire and continue to shoot thick streams of your seed into her");
        if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" as well as on her stomach and the ground beneath you");
        if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("; your VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) quivers with the need for release, letting loose with its own torrent of lady fluids as you moan in orgasm");
        MainScreen.text(".  After a few moments, you slump down on the plant woman's chest, completely spent. A few minutes rest does you good, and with time you rise back up and look down at your partner; a dazed, blissful expression has fallen on her features, the creature lost in pure nirvana as she cradles her stomach. Satisfied with how things turned out, you give her one last kiss and struggle to get back up.  Damn, your ");
        if (player.lowerBody.balls > 0) MainScreen.text("[balls] really hurt");
        else {
            MainScreen.text("dick really hurts");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s really hurt");
        }
        MainScreen.text(", but looking back at the heap of a woman behind you, you concur that it was totally worth it.");
        ...
        if (player.HP < hpMax()) {
            MainScreen.text("Upon returning to camp, you notice that some of the wounds the plant creature kissed have actually healed.  You're not sure how exactly this worked, but at least you don't have to put up with too many cut and bruises now.");
            //(HP restored +50).
            //9999 add HP
        }
        player.orgasm();
    ...
    public victoryOverPlantGalWithPCCockCorrupted(): void {
        ...
        if (player.lowerBody.isGoo()) {
            ...
            menu();
            MainScreen.addButton(0, "Yes", corruptDomWinFuckGooGal);
            MainScreen.addButton(1, "No", victoryOverPlantGalWithPCCockCorrupted, true);
        }
    ...
    public victoryOverPlantGalWithPCCockCorrupted(newl: boolean = false): void {
        if (newl) MainScreen.clearText();
        else MainScreen.text("\n\n");
        MainScreen.text("After an eternity of ravaging her pussy, you feel your load building up pressure and getting ready to explode.  You tighten your grip on the plant woman's hair and with one last thrust fill her womb with your corrupted semen.  You jerk her hair back, forcing her head back towards your mouth as you begin to neck her.  \"<i>AuuughhhAHHHHHHHHHHHHHHH!</i>\" she screams out as you pull on her hair, with her pussy clamping down on your " + CockDescriptor.describeCock(player, x) + ".  Her fuck hole ever so eager to receive your seed, her body convulses violently as you continue to fire more semen into the floral vixen's cervix.  The excretion is too much, and with a groan, you pass out on top of her.");
    ...
    public victoryDefeatPlantGirlGetBJMainSCREENON(): void {
        ...
        //PC has balls: (
        if (player.lowerBody.balls > 0) MainScreen.text(" cusps your [balls] in her hand, and suck on the sides of your [sack]. She licks upwards from your testicles and caresses them before");
        else MainScreen.text(" licks up from your gouch before");
        MainScreen.text(" slowly proceeding towards your glans and swirling her tongue around it like a tootsie roll.  It isn't long before the plant woman suddenly stops and looks up at you with mischievous eyes.  She raises her eyebrows twice, and then ");

        //[If cock(s) are tentacle dicks and greater than 24 inches in length, jump to (V and D 1.2tc)]
        if (player.lowerBody.cockSpot.countType(CockType.TENTACLE) > 0 && (player.lowerBody.cockSpot.countType(CockType.TENTACLE) == player.lowerBody.cockSpot.count() || rand(2) == 0)) {
            plantGirlPCWinGetTentacleBJ();
            return;
        }
        else {
            //[if cock size is 4-14 inches] 
            if (player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockLength < 14) MainScreen.text("takes the entirety of your [cock biggest] in her mouth and down her throat.  You lurch forward as your cock hits the back of her throat, moaning intensely at the plant girl's ability to take your whole penis in her mouth.");

            //[If cock size is greater than 14 inches]
            else MainScreen.text("attempts to take as much of your member as she possibly can.  Your [cock biggest] slams against the back of her throat, and the plant woman tries to take a little more of you to no avail.  She looks up at you apologetically, but you pet her head and ensure that she knows you aren't upset with her.");

            MainScreen.text("  She stares up at you with a flirtatious look and pulls your cock out of her throat, seeking to get a little more intimate with the head of your dick.  You grip on her hair tightly with every pleasurable sensation as her head bobs back and forth between your thighs.");
        ...
        //______________________________________________________________________________
        //(if Non-goo: 
        if (!player.lowerBody.isGoo()) MainScreen.text(".  She pulls your dick out of her mouth and closes her eyes as she gets ready to catch the rest of your load with her mouth.  Thick globs of semen erupt from your CockDescriptor.describeMultiCockShort(player) and strike her in the face.  She herself orgasms as your torrent of semen washes over her head and chest, covering both liberally.");
        //(If goo: 
        else MainScreen.text(", bucking hard against her face as you grunt furiously through gritted teeth and a fierce expression of release.  The plant woman inhales deeply just as you cum into her hot, moist mouth, producing a satisfied series of muffled groans as you deposit your seed.");
    ...
    public plantGirlPCWinGetTentacleBJ(): void {
        ...
        if (player.lowerBody.cockSpot.get(x).cockLength < 48) {
            ...
            if (player.lowerBody.balls > 0) "[sack]");
		else MainScreen.text(BreastDescriptor.describeNipple(0));
            MainScreen.text(" and begins to squeeze tighter and tighter.  “Owww! Owww! Okay, Okay! I get it!” you cry out as you once again ram your " + CockDescriptor.describeCock(player, x) + " down her throat.  The plant woman responds by massaging your ");
            if (player.lowerBody.balls > 0) MainScreen.text("balls");
            else MainScreen.text("nipples");
            MainScreen.text(", showing that this is what she wants.  You laugh out loud at the ridiculousness of the situation: you're using a plant creature's throat as a makeshift cunt, and she doesn't seem to be running out of air anytime soon.");
        ...
        //[If tentacle cock is longer than 48 inches]
            ...
            MainScreen.text("\n\nYour loins begin to ache for release, and you feel the muscles in your " + CockDescriptor.describeCock(player, x) + " tensing up,  cum bulging up your cock(s) as your load works its way down her throat and into her stomach.  The instant it hits, the plant woman bucks wildly, her sex spraying you in the face with her nectar-y pussy juice.  You bury your face against her vagina and lap up all of the juice you can as she orgasms.  As you both finish cumming, you and her collapse in a cum-stained heap, panting heavily as you both recover from the ordeal.");
        }
        //(V and D 1.2tc) conclusion
        //(V and D 1.2tc) conclusion: 
        MainScreen.text("After a few minutes, you gather enough strength to rise to your feet and pull your flaccid cock from her mouth.  Content, she gives it one last lick before passing out into a deep sleep.  You smile and put your [armor] back on, eager to get back to camp.");
        player.orgasm();
        cleanupAfterCombat();
    }
    ...
    public beatPlantGirlDomLetsFuckAnal(): void {
        ...
        if (inCombat) {
            ...
            //Testicle check:
            //([balls]/pelvis)
            if (player.lowerBody.balls > 0) ouptutText("Your [balls] slap");
            else MainScreen.text("Your pelvis slaps");
            MainScreen.text(" against her pussy, causing wet slapping sounds and coating your [sack] liberally with her sexual secretions.  The plant woman moans weakly");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" grabs a of your penis and guides it to her vagina.");
            else {
                MainScreen.text(", while she ");
                //PC has testicles: (
                if (player.lowerBody.balls > 0) MainScreen.text("fondles your testicles sensually; surprisingly she doesn't injure you by squeezing too hard despite the rough fucking.");
            }
            //Vagina check:
            if (player.lowerBody.vaginaSpot.hasVagina()) {
                if (player.lowerBody.cockSpot.count() == 1 && player.lowerBody.balls > 0) MainScreen.text("  Not content with just teasing your balls, the");
                else MainScreen.text("  The");
                MainScreen.text(" attractive creature darts her hand down to your [vagina], fingering you passionately as she takes the length of your meat into her hungry sex.");
            }
            ...
            MainScreen.text("\n\nYou grunt and moan as your " + CockDescriptor.describeCock(player, x) + " shoots its anal payload ");
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" along with its vagina-clad brother");
            MainScreen.text(".  She slumps against you, lost in pleasure and sensation as you drench her interior with hot, sticky cum.  The plant woman's nectar-flavored milk shoots forth from her breasts and coats the ground in front of the both of you, causing her to shake and moan furiously.");
            if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("Your [vagina] obliges, and a few steams of lady spunk flow from your [clit] and intertwine with the fauna creature's fluids on the ground.");
            MainScreen.text("  As the plant woman comes down from her climax, you lick your hands clean of her nectar and pull out from her.  You shoot her a look of appreciation, and she fires back a look of utter contentment and bliss.  You leave her to recuperate from the act and head back to camp.");
            player.orgasm();
            cleanupAfterCombat();
        }
        //(Dominant Let's Fuck! (giving anal) D 1.3
        else {
            MainScreen.text("Before the plant gets a chance to take two steps forward,you put your hand out towards her, signaling her to stop. Almost immediately the plant woman pauses her movement towards you, like a herbivore as it listens for the sounds of predators. Knuckles against your chin, you stare longingly at the creature's hips as she stands motionless, wondering why you wanted her to stop. Signaling with your index finger, you instruct the perplexed female to turn around. Reluctantly she spins around and exposes her ample rump to your gaze, causing a violent twitch in your pants as her full bottom comes into view. Satisfied, you proceed towards the plant woman, disrobing as you go.");
            MainScreen.text("\n\nThe sound of your gear hitting the ground causes the plant woman to look over her unimposing shoulder, and she grins as you embrace her from behind. Taking her left, average breast in your hand, you fondle both her mound and her anther-like nipple as you caress her stomach with your free hand.  The creature shudders from your touch, and you feel a thin stream of her moist pussy juice slowly run down your leg.  The need to fuck her becomes too overwhelming for you, and [eachCock] presses itself against her ample ass, pulsating with desire.  As you lower a hand to guide your CockDescriptor.describeCock(player, suitable cock) into her tight asshole, her hand darts out and grabs your dick instead, beating you to the punch.  Surprised at her speed, you slowly draw your hand back and gasp as the plant woman pushes against your CockDescriptor.describeCock(player, suitable cock)'s head, taking it past her tight pucker and into her anus.");
            ...
            if (player.lowerBody.balls > 0) MainScreen.text(" your balls [balls] and");
            MainScreen.text(" your [leg], and the nectar like scent of her pussy juices hits you with pungent force as you breathe it in.  Content and quite exhausted, you start the delicate task of pulling your " + CockDescriptor.describeCock(player, x) + " out of her rectum, taking a little bit more at a time out of her until you hear a loud, wet pop from her rump.  Your flaccid and battered penis flops out, signalling what should be the flood of semen from her lovely hole...yet nothing comes out. Curiously, you edge yourself under her and go to spread her cheeks, intent on seeing what the holdup is.  However, caution stops you from doing so; images of getting hit with your spunky drool flood your mind, and you opt to shrug it off. With a pleased wave to the creature, you gear back up and head back to camp. The floral creature gives a weak wave back, no doubt lost in the bliss of having her anus pleasured so well.");
            player.orgasm();
            doNext(camp.returnToCampUseOneHour);
        }
    ...
    public pcBeatsPlantgirlAndCorruptAssFuck(): void {
        ...
        //(If Naga:
            ...
            menu();
            MainScreen.addButton(0, "Penetrate", nagaPenetrateWithTailDuringPlantGirlMeanAnalPCWin);
            MainScreen.addButton(1, "Stick2Anal", pcBeatsPlantgirlAndCorruptAssFuckPt2);
            return;
        }
    ...
    public pcBeatsPlantgirlAndCorruptAssFuckPt2(tail: boolean = false): void {
        ...
        if (!tail) MainScreen.text("herself");
        else MainScreen.text("her labia while you fuck her with your tail");
        MainScreen.text(" and playing with her breasts as you pump her ass with all of your might.  You can no longer control the timing of your orgasm, and you howl in savage delight as you bottom out in this plant creature's sphincter.  She cries out so loud that you swear she almost ruptures your ear drums, slamming against you in a uncontainable thrashing orgasm.  You place your hands on the top of her thighs and push downward, intent on burying your cock");
        if (y >= 0) MainScreen.text("s");
        MainScreen.text(" deep in her ass as you fill her up with your tainted splooge.  She quivers violently; her body intent on getting some room but you will have none of it.  You keep her pinned against your lap until you have finished cumming and pull out of her, wiping yourself off with her hair as you proceed to replace your [armor].  The plant woman lies against the ground, now utterly dominated and unable to move.  As you walk away, you hear a rustling in the bush near her.  You let out a loud laugh as you picture an imp or tentacle monster emerging and proceeding to pick up where you left off.  As much as you would like to stick around and find out what fate lies in store for her, you have other things to do.  You turn and head back to camp.");
        player.orgasm();
        cleanupAfterCombat();
    }
    ...
    public victorySuckPlantgirlsDick(): void {
        ...
        if (inCombat) {
            ...
            if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("[eachCock] is rigidly erect");
            else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("your [vagina] is gazed over with your secretions");
            //Genderless: 
            else MainScreen.text("your body is flushed with sexual need");
    ...
    public beatPlantGirlAndTakePlantCockInButt(): void {
        ...
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("[eachCock] becomes rock solid");
        if (player.gender == 3) MainScreen.text(" while ");
        if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("your pussy becomes incredibly wet");
        if (player.gender == 0) MainScreen.text("nipples become erect");
        MainScreen.text(" in response to your own arousal and desire.  The tip of her penis presses against your cheeks and pushes its way into you as you allow it past your anus and into the depths of your ass.  The creature opens her mouth in an 'ohh' expression, enjoying the sensation of breaking past your [asshole].  As she bottoms out, ");
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("[eachCock] jerks upward, the last shot of blood filling and hitting them like a thunderous punch");
        if (player.gender == 3) MainScreen.text(", while ");
        if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("your pussy leaks a bit of girl cum.  It slowly drools out onto her pelvis");
        if (player.gender == 0) MainScreen.text("your body quivers from the sensation");
        MainScreen.text(".  ");
        //PC has balls: 
        if (player.lowerBody.balls > 0) MainScreen.text("Your [balls] laying on top of her pelvis, y");
        else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("Your vaginal muscles clenching up along with your sphincter, y");
        else MainScreen.text("Y");
        MainScreen.text("ou ");
        if (player.lowerBody.balls > 0) MainScreen.text("begin to massage your [sack] as you raise your hips upward");
        else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("relish in the moment and allow a hand to drop to your sex as you raise your hips, eager to please both ends during this act");
        else MainScreen.text("grab her nipples and begin to play with them");
        MainScreen.text(".  Slowly, you lower your hips back down once again onto the plant woman's penile member, causing your partner to moan in lustful appreciation.  A sense of fullness floods you as you allow yourself to take her entire member inside of you.  You ");
        if (!player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("place both hands on her shoulders");
        else MainScreen.text("continue to finger yourself");
        MainScreen.text(" as you bob your [asshole] against her lap, moaning loudly as you do.  The plant woman's hands shoot out and ");
        //PC has a pussy (and/or) a dick:
        if (player.gender > 0) {
            MainScreen.text("grab both your [chest] and ");
            if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("[oneCock]");
            if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("your vulva");
        }
        //PC is genderless:
        else MainScreen.text("your nipples, playfully tugging on them");
        MainScreen.text(", intent on magnifying your own pleasure.");
        ...
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("strokes [oneCock] with her hands");
        if (player.gender == 3) MainScreen.text(" as she also ");
        if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("massages your labia");
        if (player.gender == 0) MainScreen.text("massages you");
        MainScreen.text(".");
        MainScreen.text("  The plant woman thrusts in kind,  mashing her pelvis against your own as you bounce off the creature. You manage to time your descent with her thrusts to intensify both of your pleasures.  She ");
        if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("quickens her pace of stroking your cock");
        if (player.gender == 3) MainScreen.text(" and ");
        if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("fingers the interior of your pussy, causing you to roll your eyes into the back of your head as she sensually teases the inside of your sex");
        if (player.gender == 0) MainScreen.text("raises herself to deliver a passionate kiss");
        MainScreen.text(".  You feel her thrusts getting shorter now, and you know that her she going to blow her load soon.  With gradually closing eyes and increasingly frequent panting and moaning, she grabs your hips and holds them in place as she thrusts upward one last time.  With a lusty moan, her spunk shoots into your hot innards with significant force, allowing you to feel the impact of every shot of cum in your anus.");
        MainScreen.text("\n\nYou cry out in fierce orgasm, ");
        if (player.lowerBody.cockSpot.hasCock()) {
            MainScreen.text("[eachCock] erupting its own payload onto her face and chest");
        }
        if (player.gender == 3) MainScreen.text(", ");
        if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("vaginal muscles clamping down and drooling pussy juice all over the plant woman's lap");
        if (player.gender == 0) MainScreen.text("twitching futilely around the plant-girl's swollen seeder");
        MainScreen.text(".  With a groan, you collapse against the creature's chest, desperately trying to catch your breath from the taxing ordeal.  The plant woman hums approvingly, stroking your hair as your body ascends and descends from her own because of your heavy breathing.  After some time, you are able to muster the strength to pull yourself off of the plant creature, her cock slipping out of your ass and unleashing a flood of sappy jism from your anus.  Content, she passes out into a deep sleep.  You grin to yourself, putting your [armor] back on and heading back to camp.");
        player.orgasm();
        if (inCombat)
            cleanupAfterCombat();
        doNext(camp.returnToCampUseOneHour);
    ...
    public defeatPlantgirlBlowAndButtfuckPure(): void {
        //V 1.6 and D 1.6
        MainScreen.clearText();
        MainScreen.text("With your hands on your hips you " + player.mf("walk", "sway"));

        //(Victory, V 1.6:)
        if (inCombat)
            MainScreen.text(" towards the battered creature as she struggles to get back to her feet.  Her efforts are quite for naught as her arms give out, causing her to hit the ground hard.");
        //(Scene leads to V and D 1.6)
        //(Dominant, D 1.6: 
        else MainScreen.text(" towards the plant woman, causing her to giggle in response as she beckons you over to her.");
        //(Scene leads to V and D 1.6)
    }
    ...
    public defeatPlantgirlBlowAndButtfuckPure(): void {
        ...
        if (player.lowerBody.isNaga()) MainScreen.text("now exposed ");
        MainScreen.text(VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + ".  The plant woman yelps in surprise as the vine makes contact with your labia, causing you to jump a little.  You turn to face the plant woman, who is blushing profusely and resting her hand on her pussy.  Looking at the vegetation, you deduce that she must be able to feel you along her vines and that these creepers may also function as penile appendages.  A grin spreads across your face as the realization hits you, and you bring the vine back to your sex and tease your vulva.  Soft moans escape the plant girl as the head of the perverted vegetation brushes past your clit, and you ");
        //(pure: 
        if (player.stats.cor < 50) MainScreen.text("giggle");
        //Corrupted: 
        else MainScreen.text("smirk");
        MainScreen.text(" at the woman's predicament.  Deciding to stop teasing the poor girl, you take the head of the creeper and push it into your pussy, edging it upwards at a gradual pace so that your partner can feel the contours of your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".");
...
[If you chose just the vine, V and D 1.6vp] You slip a hand to your
 	...
 	(if female) BreastDescriptor.describeBreastRow(player.upperBody.chest.get(0)) and start to massage your hardening nippleDescriptAll().

	(if herm)) member and begin to (fondle your CockDescriptor.describeCock(player, 0)).
...
Both of you moan as your sexes touch one another, sensation running through your vulva like wildfire. Twitching ever so slightly, the plant woman grabs you by your LowerBodyDescriptor.describeHips(player) as she angles her cock to breach the mouth of your VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)).  Without wasting any more time you buck your hips downward, driving her member past your hungry lips and through the walls of your sex, flooding your body with endorphins and bringing on a sensation of giddiness.  Your partner squirms under you as her cock bottoms out inside of you, unable to control herself as her member takes the full blunt of your pussy. Grabbing her by the shoulders you begin to fuck the plant woman, bobbing your [butt] up and down against her lap; the melancholy of your intimate act ringing forth with the sounds of sloppy, wet sex. The nerves in your pussy light up as her member brushes back and forth inside of your fuck-hole, making you cry out occasionally as she “abuses” your cunt like it owes you money.  In response, you make the rough ride you're giving her faster and more savage.  Hands darting upward, the plant woman hungrily grabs your BreastDescriptor.describeBreastRow(player.upperBody.chest.get(0)),
...
(if herm: slips her hand down to your CockDescriptor.describeMultiCockShort(player), taking (it/one at a time) into her hands, stroking (it/them) furiously as you pound her moist love canal.  You let up on the rough fucking for a brief moment as she tends to your dick(s), taking in the euphoria of her soft, leaf like texture on your naughty bit(s). )
...
Eventually all good things must come to an end;  Your inevitable orgasm approaches, body trembling in anticipation at the thought of covering this creature in cum.  Sexual fluids rush out of you as you concentrate on pushing, forcing them out. Instinct takes hold as you mash your pelvis against the plant woman's lap. (Your CockDescriptor.describeMultiCockShort(player)) explode(s),
...
(if vagina and/or cock: (VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0))/   /and/ CockDescriptor.describeMultiCockShort(player)/ 
...
(VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0))/ CockDescriptor.describeMultiCockShort(player)/ assholeDescript(0) ). 
...
(CockDescriptor.describeMultiCockShort(player)/ VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0))/assholeDescript( ) ) 
against her face, slathering (it/them) down her forehead and past her nose as 
PC has cock(s): (gravity takes your CockDescriptor.describeMultiCockShort(player) and prods (it/them) against her lips) 
...
(if genderless. Vc 1.7g: prods the crack of your ass as she parts your [butt], and licks your ButtDescriptor.describeButthole(player) with her energetic tongue; oddly enough, it resembles the texture of flower petals, and having that sensation against your hole causes you to quiver.)  Her moist tongue slaps against your hole, leaving it nice and wet as she prods and lathers your hole with her tongue. )
