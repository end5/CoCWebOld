...
interface GargoyleData {
    name: string;
    confidence: number;
    ritualIntro: number;
    kinkyRitualsSpoken: number;
    history: number;
    cathedral: number;
    nameTalks: number;
}
...
export default class Gargoyle {
    private data: GargoyleData = {
        name: "",
        confidence: 0,
        ritualIntro: 0,
        kinkyRitualsSpoken: 0,
        history: 0,
        cathedral: 0,
        nameTalks: 0,
    };
    ...
    public constructor() {
        Game.characterData.set(CharacterType.Gargoyle, this.data);
    }
    ...
    private gargoyleConfidence(arg: number = 0): number {
        if (arg !== 0) {
            this.data.confidence += arg;
            if (this.data.confidence > 100) this.data.confidence = 100;
            else if (this.data.confidence < 0) this.data.confidence = 0;
        }
        return this.data.confidence;
    }

    // [b]Introduction, 1st Time:[/b]
    public gargoylesTheShowNowOnWBNetwork(character: Character) {
        DisplayText().clear();
        ...
        if (character.locations.get(LocationName.Desert).visited) DisplayText(", making even the desert seem healthy and full of life");
        ...
        if (character.locations.get(LocationName.TelAdre).visited) DisplayText("or the ruins of one of Tel'Adre's more distant neighbors, ");
        ...
        MainScreen.doNext(this.gargoyleMeeting2);
        Game.time.hour++;
    }

    // (Advance time by 1 hour)
    private gargoyleMeeting2(character: Character) {
        DisplayText().clear();
        ...
        if (character.stats.lib >= 40) DisplayText("  Maybe you could take this with you as a life-sized sex toy?");
        ...
        character.locations.get(LocationName.Cathedral).locationKnown = true;
        ...
        MainScreen.displayChoices(["Don't Break", "Break Chains"], [this.dontBreakThatShit, this.breakZeChains]);
    }

    // [b]Don't Break[/b]
    private dontBreakThatShit(character: Character) {
        DisplayText().clear();
    ...
    // [b]Break Chains[/b]
    private breakZeChains(character: Character) {
        DisplayText().clear();
        // Introduces two new values: " + this.data.name + ", the gargoyle's name, given by the Champion, and Confidence, a value that has a base score of 0 (submissive/slave) rising to 100 (equal). Negatives are possible.
        ...
        DisplayText("\n\n\"<i>I am your humble servant, Master.  Your tool.  Your plaything.  Your toy.  Ask, and I shall obey.</i>\"  She seems to pause for a moment, and adds in an emotionless whisper, \"<i>this one would be honored to pleasure Master, if " + character.desc.subjectivePronoun + " desires.</i>\"");
        ...
        // [Display a textbox, into which the character can type in a name. This new value will be called " + this.data.name + ", henceforth. ] (Confidence +10)
        mainView.nameBox.text = "";
        MainScreen.doNext(this.nameZeGargoyle);
    }

    private nameZeGargoyle(character: Character) {
        ...
        this.data.name = mainView.nameBox.text;
        mainView.nameBox.text = "";
        mainView.nameBox.visible = false;
        DisplayText().clear();
        DisplayText("\"<i>" + this.data.name + ",</i>\" she purrs, \"<i>" + this.data.name + ", " + this.data.name + ".  Master has interesting taste.</i>\"");
        ...
        this.gargoyleStarterMenu(character);
    }

    private gargoyleStarterMenu(character: Character) {
        // (Display options [Funny Order], [Carnal Order]. and [Talk])
        MainScreen.displayChoices(["Funny Order", "Carnal Order", "Talk"], [this.giveGargoyleAFunnyOrder, this.carnalOrder, this.firstGargoyleTalk]);
    }

    // [b]Funny Order[/b]
    private giveGargoyleAFunnyOrder(character: Character) {
        DisplayText().clear();
        ...
        DisplayText("\n\nTo her credit, " + this.data.name + " hesitates only for a moment before dropping to the floor and rolling belly-up, limbs upraised like a puppy in training.  Stifling a chuckle, you lean down and do just that, giving her a quick belly rub.  Her skin is cool to the touch, and impeccably smooth, as you might expect from a marble statue.  Seemingly pleased, " + this.data.name + " scrambles back to a low crouch before you, awaiting your next command.");
        this.gargoyleStarterMenu(character);
    }

    // [b]Carnal Order[/b]
    private carnalOrder(character: Character) {
        DisplayText().clear();
        DisplayText("Well, well. " + this.data.name + " seems willing to follow any order you give, no matter how carnal, and you could not help but notice her rather <i>arousing</i> attributes earlier.  Smirking wickedly, you command your new gargoyle to finger herself.");
    ...
    // [b]Talk (First Time)[/b]
    private firstGargoyleTalk(character: Character) {
        DisplayText().clear();
        DisplayText("You decide against giving " + this.data.name + " an order for now.  Instead, you right a nearby toppled pew and seat yourself across from the crouching gargoyle, returning her intense stare with one of your own.");
        ...
        DisplayText("\n\nSuddenly, " + this.data.name + " breaks the stare and looks away, as if in shame.  \"<i>The demons' doom that even now blights Mareth took its toll here, too.  The people of a town no longer here believed they were safe under Marae's roof.  Protected.  Their faith was. . . Misplaced.</i>\"");

        DisplayText("\n\nIt seems the events that led to the cathedral's destruction weigh heavily on " + this.data.name + "'s heart.  What do you do?");
        // (Display options: [Berate] and [Reassure])
        MainScreen.displayChoices(["Berate", "Reassure"], [this.berateTheGargoyle, this.reassureTheGargoyle]);
    }

    // [b]Berate[/b]
    private berateTheGargoyle() {
        DisplayText().clear();
        // (Confidence - 10)
        this.gargoyleConfidence(-10);
        DisplayText("A pillar of rage boils up inside you.  You are [name], Champion of Ingnam, you tell her, rising to your feet, fists clenched.  Your duty is the same as hers was, to protect your home.  And she failed.  She let all those people down.  You tell her that she's worthless, disgusting, a failure.");
        DisplayText("\n\n" + this.data.name + " recoils, flinching from you in expectation of assault. Instead, you spit on the ground and start for the exit.  You shout over your shoulder for her to stay here and wallow in her failure until you deign to use her.");
    ...
    // [b]Reassure[/b]
    private reassureTheGargoyle(character: Character) {
        DisplayText().clear();
        // (Confidence +10)
        ...
        DisplayText("\n\n" + this.data.name + " looks up at you, her ruby-red eyes shining in the dim hit of light in the cathedral.  You cup her cheek and give her a smile.  \"<i>Come on, now,</i>\" you reply, offering her a hand up.  \"<i>Let's get out of here.</i>\"");
        ...
        DisplayText("\n\nYou rustle " + this.data.name + "'s hair and assure her you will before you take your leave and return to camp.");
        // (Return to camp, advance time 1 hour)
        MainScreen.doNext(Game.scenes.camp.returnToCampUseOneHour);
    }

    public returnToCathedralNoWake(character: Character) {
        this.returnToCathedral(character);
    }

    public returnToCathedralWake(character: Character) {
        this.returnToCathedral(character, true);
    }
    ...
    public returnToCathedral(character: Character, woken: boolean = false) {
        ...
        if (Game.time.hour <= 9 || Game.time.hour >= 18 || woken) {
            if (!woken) {
                DisplayText("\n\nIn the dim ");
                if (Game.time.hour <= 9) DisplayText("early morning");
                else DisplayText("late evening");
                DisplayText(" light streaming in from the broken windows, you see " + this.data.name + " poking around some of the rubble, slowly but surely making efforts to repair the damage done to the Cathedral.  Seeing you, however, she immediately drops what she was doing and ");
                if (this.gargoyleConfidence() < 70) DisplayText("kneels before you, head bowed low.");
                else if (this.gargoyleConfidence() < 99) DisplayText("bows low before you, a small smile on her stone lips.");
                else DisplayText("with a sexy sway of her hips, approaches you and lays a small but tender kiss upon your cheek before bowing her head to you.");

                DisplayText("\n\n\"<i>Master has returned,</i>\" " + this.data.name + " says ");
                if (this.gargoyleConfidence() > 50) DisplayText("happily.");
                else DisplayText("solemnly.");
            ...
            if (character.stats.lust >= 33) MainScreen.getBottomButton(0).modify("Sex", this.gargoyleSexMenu);
            MainScreen.getBottomButton(1).modify("Rituals", this.ritualGargoyle);
            if (this.data.history < 3 || this.data.cathedral < 3 || this.data.nameTalks < 3) MainScreen.getBottomButton(2).modify("Talk", this.talkToGargoyleOutput);
            MainScreen.getBottomButton(3).modify("Appearance", this.gargoyleAppearance);
            if (this.gargoyleConfidence() >= 70 && this.data.ritualIntro > 0) MainScreen.getBottomButton(4).modify("Kinky Rituals", this.gargoyleKinkyRituals);
        }
        // [b]Cathedral Interior: 10:00 –> 17:00[/b]
        else {
            if (!woken) {
                DisplayText("\n\nWithin the ruins, you can see " + this.data.name + " perched atop her pedestal in a low, predatory crouch, much as you found her.  Waving a hand in front of her eyes, you confirm she's in some sort of dormant mode.  A simple touch from her Master could revive her");
                if (character.stats.lust >= 33) DisplayText(", or you could use her to satisfy your own urges, not unlike a life-size sex toy");
                DisplayText(".");
            }
            // (Display options: [Wake Her] [Use Her (if Lust >= 30)] [Appearance] and [Leave])
            MainScreen.getBottomButton(0).modify("Wake Her", this.returnToCathedralWake);
            if (character.stats.lust >= 33) {
                if (character.torso.cocks.count > 0) MainScreen.getBottomButton(1).modify("Use Her", this.useGargoyleMaleHerm);
                else if (character.torso.vaginas.count > 0) MainScreen.getBottomButton(1).modify("Use Her", this.useHerGargoyleFemale);
            }
            MainScreen.getBottomButton(3).modify("Appearance", this.gargoyleAppearance);
        }
        MainScreen.getBottomButton(9).modify("Leave", Game.scenes.camp.returnToCampUseOneHour);
    }

    // [b]Sex[/b]
    private gargoyleSexMenu(character: Character) {
        DisplayText().clear();
        DisplayText("You approach " + this.data.name + " and tell her you have some... pressure that needs relieving.");
        // (if Confidence =< 69:
        if (this.gargoyleConfidence() <= 69) DisplayText("  The gargoyle bows low before you, her face expressionless.  \"<i>By your command, Master.  How would you have me?</i>\"");
        ...
        if (character.gender === 1) {
            MainScreen.getBottomButton(1).modify("Anal", this.gargoyleAnal);
            MainScreen.getBottomButton(0).modify("Vaginal", this.gargoyleCoochiiGetsPlowed);
            MainScreen.getBottomButton(2).modify("Titfuck", this.titFuckTheGargoyle);
            MainScreen.getBottomButton(3).modify("Strap-On", this.strapOnGargoyle);
            MainScreen.getBottomButton(4).modify("Leave", this.returnToCathedralNoWake);
        }
        // (If Female, display options: [Tail Fuck] and [Ride Strap-on] [Leave])
        if (character.gender === 2) {
            MainScreen.getBottomButton(0).modify("Tail Fuck", this.tailFuckGargoyleScene);
            MainScreen.getBottomButton(1).modify("Strap-On", this.strapOnGargoyle);
            MainScreen.getBottomButton(4).modify("Leave", this.returnToCathedralNoWake);
        }
        ...
        if (character.gender === 3) {
            ...
            MainScreen.getBottomButton(2).modify("Titfuck", this.titFuckTheGargoyle);
            MainScreen.getBottomButton(3).modify("Strap-On", this.strapOnGargoyle);
            MainScreen.getBottomButton(4).modify("Tail Fuck", this.tailFuckGargoyleScene);
            MainScreen.getBottomButton(9).modify("Leave", this.returnToCathedralNoWake);
        }
    ...
    // [b]Anal[/b]
    private gargoyleAnal(character: Character) {
        DisplayText().clear();
        ...
        DisplayText("\n\n" + this.data.name + " makes a sharp little gasp as you push your finger into her.  You slide into her with surprising ease, reveling in how <i>cold</i> her interior is.  She's like ice inside!  You start thrusting your finger faster, trying to get her warmed up a little as your prepare her anus.  " + this.data.name + " squirms and moans softly as you finger-fuck her bum, slowly changing from moans of discomfort to little gasps of pleasure.");
        ...
        if (character.torso.cocks.count > 0 && character.torso.cocks.sort(Cock.LongestCocks)[0].length >= 50) {
            DisplayText("\n\nKnowing full well the living statue doesn't have much in the way of anatomy, you continue to push more and more of monstrous manhood into " + this.data.name + "'s asshole, watching with glee as her stomach begins to distend with the massive size of your prick.  Then her chest bulges, making her gasp in pain and clutch at her breast; that doesn't stop you, though.  With one mighty, massive thrust, you slam yourself up to the hilt in " + this.data.name + "'s ass, and watch with glee as the head bursts out of her mouth, squirting pre and drool all over the floor.  The poor gargoyle makes a muffled scream and starts flailing around helplessly, trying to do <i>something</i> about the massive member now protruding from her mouth.");

            DisplayText("\n\nWith gusto, you start hammering away at your living cock-sleeve, using her entire body as one great big ona-hole to take your gargantuan girth.  Resigned to her fate, " + this.data.name + " slumps forward and grasps the head of your [cock biggest], giving you a handy through her mouth as her innards milk the rest of your shaft.");
        ...
        // (if PC doesn't qualify for the above:)
            ...
            // if Balls:
            if (character.torso.balls.quantity > 0) DisplayText(", your balls slapping against her butt");
    ...
    // [b]Vaginal[/b]
    // no limits
    private gargoyleCoochiiGetsPlowed(character: Character) {
        DisplayText().clear();
        let cockThatFits: Cock;
        const cocksThatFit = character.torso.cocks.filter(Cock.CockThatFits(60));
        if (cocksThatFit.length > 0)
            cockThatFits = cocksThatFit[0];
        else
            cockThatFits = character.torso.cocks.sort(Cock.SmallestCockArea)[0];

        DisplayText("You gently push " + this.data.name + " onto her back and spread her long, marble legs, giving you a clear view of her tight little pussy.  You brush your hand along her thighs, making her gasp with the sensation as your knuckles run along her smooth, cool flesh.  Your fingers come to her small clit, and you caress her tiny pleasure buzzer, eliciting a tiny moan from the gargoyle.  Grinning, you give it another flick; " + this.data.name + " squirms, trying to either enhance or end the sensation, you can't be sure.  You start to finger her clitty harder, creating little circles around it as you slip another finger into her tight passage.");

        DisplayText("\n\nYou're rewarded for your efforts by " + this.data.name + " bucking her hips into your hands, moaning, \"<i>M-Master... please...</i>\"  Not done teasing her yet, you put your other hand on her flat belly and start to crawl it up, tickling her as you make your way toward her sizable breasts.  You glomp onto one of them, giving it a rough squeeze as you drive your finger into her a little further than before.  \"<i>Masterrrrr...</i>\" " + this.data.name + " whines, reaching up to stroke your cheek.  Deciding you've teased the poor girl enough, you prepare for the main course.");

        DisplayText("\n\nYou pull your " + CockDescriptor.describeCock(character, cockThatFits) + " from your [armor], letting it flop down atop her crotch.  Embarrassed, " + this.data.name + " looks away as you rub the underside of your cock across her mons, spitting on it for a bit of extra lubricant.  You lean back, lining the tip of your cockhead with the gargoyle's tight slit, and press forward.  She lets out a sharp gasp as your cock presses against her, pushing it past her lips and finally slipping into her depths.");
        ...
        else DisplayText("up to the hilt inside her.");
        ...
        if (this.gargoyleConfidence() >= 70) DisplayText("  Before you can stand, though, " + this.data.name + " hugs her arms tightly around you pulling you down on top of her and mashing your face between her soft tits.");
    ...
    // [b]Tit-Fuck[/b]
    private titFuckTheGargoyle(character: Character) {
        DisplayText().clear();
        DisplayText("You situate yourself on a pew and spread your legs.  You pat your thighs, telling " + this.data.name + " to sit between your legs as you fish out your [cock biggest].  She approaches, hesitantly placing herself on the floor between your legs as you recline and stroke your shaft to full hardness.  You let the gargoyle sit there, watching you wide-eyed as you continue to masturbate.  Though you're only preparing for the main act, you can't help but enjoy the sensation of such a basic sexual act, made all the more pleasurable by the creature at your feet, her face barely an inch from your cock's head as you stroke yourself.");

        DisplayText("\n\nOnce you're satisfied at your cock's rigid hardness, you tell " + this.data.name + " to put those big, soft tits of hers to work.  \"<i>M-Master?</i>\" she gasps, making a cute little “o” with her mouth just an inch from your shaft.  You repeat the order, and giver her a thump on the nose with your cock for emphasis.  Shyly, she cups her D-cups and leans into you, squeezing your [cock biggest] between them.");

        DisplayText("\n\nWith a little encouragement, " + this.data.name + " starts to rise and fall on her knees, dragging your shaft through her soft, cool mounds.  With a pleasured sigh, you reach down and stroke her hair, petting the girl as she begins to slowly but steadily titfuck you.  Still mumbling encouragements, you sit back and let the pleasure run over you, reveling in the incredible softness of her sizable chest massaging your cock, making steady progress toward stroking the cum out of you.");

        DisplayText("\n\nSuddenly, you feel an overwhelming wetness on your cock.  You look down, expecting to see pre flowing freely from you, but are instead greeted with the sight of " + this.data.name + " sliding the tip of your cock into her mouth, starting to fellate you on top of the tittyfuck.  You give her a few more words of encouragement and press her head down further onto your shaft, forcing her to take all that's left over from her breasts.  Despite her marble nature, her mouth is moist and surprisingly warm, and her long tongue is quick to wrap around your [cock biggest], wringing it in time with her chest-thrusts.");
        ...
        DisplayText("\n\nYou grab " + this.data.name + " and push her down hard, slamming your cock down her throat as you cum.  A human would surely have choked with this maneuver – instead, " + this.data.name + " squirms and flails, but you can see and feel her swallow your cum as it pours into her mouth, her throat massaging your flaring head, milking it harder than any tit-job ever could have.  Once her face has been ground into your pubes, you buck your hips sharply, thrusting into her throat as the last of your jizz spurts out and down her gizzard.");

        DisplayText("\n\nBy the time you're done, " + this.data.name + "'s face is an utter mess, with white lines trickling down her cheeks.  You drag your [cock biggest] out of her with a wet POP, unleashing a dam of jizz that quickly leaks out of her agape mouth to stain her breasts and belly.  You chuckle at the sight");
        if (this.gargoyleConfidence() >= 70) DisplayText(", and to your surprise, so does " + this.data.name + "");
        DisplayText(".");
    ...
    private strapOnGargoyle(character: Character) {
        DisplayText().clear();
        DisplayText("Giving a quick look around the ruins, you tell " + this.data.name + " to go find something that can bring you two a little <i>closer</i>.");

        DisplayText("\n\n\"<i>Closer?</i>\" she asks, confused.  You roll your eyes and tell her to grab the holy rod sitting on the altar and put it through a leather strap.  She hesitates for a moment, but dutifully does as you ask, returning a moment later with the smooth, silver cylinder and a leather thong.  With a bit of work, you've created a simple strap-on out of the holy rod, and instruct " + this.data.name + " to put it on.  Now she's getting it!  The gargoyle slips into the leather, giving a cute little shiver as one end of the rod slips inside her.  Before she has a chance to recompose herself, you grab her rod-cock in one hand and give her a gentle push with the other.");

        DisplayText("\n\nShe lays down with you atop her, straddling her waist as you stroke her rod, rewarded with sharp pleasure-gasps from the statue-girl.  You're hardly stimulating her pussy with these actions – she must be able to feel <i>through</i> the rod, as if it were a real cock.  Amused, you start to jerk the rod off, sliding it through your clenched fist with slow, measured strokes.  " + this.data.name + " squirms under the pleasurable attention, and is soon groping her big breasts and bucking her hips into your fists, shoving more of her rod-cock into your hand with each of your jerks.");

        DisplayText("\n\n\"<i>Well, someone's enjoying herself,</i>\" you tease, and immediately " + this.data.name + " freezes, her mouth agape with embarrassment.  With a little laugh, you give her a pat on the thigh and start to strip out of your [armor].  But you can't just leave " + this.data.name + " without SOME stimulation: ");
        // (If Human Legs/Feet [ie, needs toes], as new paragraph):
        if (!character.torso.hips.legs.isTaur() && !character.torso.hips.legs.isGoo() && !character.torso.hips.legs.isDrider() && !character.torso.hips.legs.isNaga()) DisplayText("you reveal your leg first, moving slowly and sensually, giving her a tantalizing look at your nethers before hiding them again.  With a wide grin, you creep your foot up to her thighs and gently run your heel along her smooth, tender flesh.  You come to the base of her rod, standing stark straight in the air like a flag-pole.  As you strip, you grasp her cock between your big and second toe, sliding them up the smooth shaft to the tip.  " + this.data.name + " moans, rolling her head back as you start to foot-fuck her cock, slowly putting more and more pressure on the steel member until it's pressed against her belly.  Your heel takes over, running quickly along the underside of her cock as you finally rid yourself of your [armor].");
        // Else, continued from above paragraph:
        else DisplayText("you giver her a good show, putting on a brief striptease for the gargoyle.  You strip your [armor] slowly, sensually, showing only the barest hints of your privates until the last moment, when the last piece of your clothing falls to the floor.");
        // (From Both, if has Vagina:)
        if (character.torso.vaginas.count > 0) {
            DisplayText("\n\n" + this.data.name + " is panting with lust by the time you're done teasing and stripping for her. Now nude, you drop back to your knees over her holy rod-cock, straddling her hips and lining her up with your [vagina]. You lower yourself until the tip of the rod is pressing against your lower lips – you're pleasantly surprised by the warm tingling sensation that quickly spreads out around the point of contact, and your cunny responds immediately by letting loose a little trickle of moisture that beads down on the gargoyle's shaft.");

            DisplayText("\n\nWithout further ado, you drop down on the strap-on, lowering yourself onto the tingling metal rod until your groin is pressed against " + this.data.name + "'s.  She, all the while, is biting her lower lip and whimpering with pleasure as your [vagina] swallows her length. Once you've taken all she has to offer, you begin grinding your hips against her, rocking back and forth atop the gargoyle.  To your surprise, " + this.data.name + " grabs you, squeezing her long fingers into your [hips] and pulling you up, almost off her rod. You decide to go with it; you grab her tits for support and start to bounce on her cock.");
            VaginaModifier.displayStretchVagina(character, 15, true, true, false);
            DisplayText("\n\nSoon, you're both moaning with pleasure, bucking and thrusting your hips to meet each other as you continue to ride " + this.data.name + "'s strap-on.  However, you can feel the clenching sensation of a coming orgasm building in your gut, and if the strained panting of your partner is any indication, she isn't far behind.  You start bouncing faster, increasing to a wild pace as your cunt contracts on the metal rod inside it, trying to milk the fake cock as waves of pleasure roll over you.  You don't let up throughout the orgasm, quite literally riding it out until " + this.data.name + " lets out a high-pitched shriek and grabs your back, smashing your face into her tits and holding you tight as she, too cums.  Suddenly she's the one pounding you, thrusting her hips into your [vagina].");

            DisplayText("\n\nBy the time she's done, both your hips and thighs are spattered with your sweat and girlcum. Still panting from the ordeal, you slowly drag yourself off of " + this.data.name + "'s cock and roll onto your back, exhausted. By the time you've recovered enough to stumble to your feet, " + this.data.name + " is fast asleep beside you, snoring cutely, her metal cock still standing straight up in the air.  You give her a gentle pat on the thigh and get dressed, leaving the gargoyle snoozing peacefully as you head back to camp.");
        }
        // (ELSE, PC has no vagina:)
        else {
            DisplayText("\n\n" + this.data.name + " is panting with lust by the time you're done teasing and stripping for her. Now nude, you drop back to your knees over her holy rod-cock, straddling her hips and lining her up with your [asshole].  You lower yourself until the tip of the rod is pressing against your clenched backdoor – and you're pleasantly surprised by the warm tingling sensation that quickly spreads out around the point of contact. Your anus responds immediately by utterly relaxing, letting just the tip of " + this.data.name + "'s strap-on slide inside you.");

            DisplayText("\n\nWithout further ado, you drop down on the strap-on, lowering yourself onto the tingling metal rod until your ass is pressed against " + this.data.name + "'s thighs.  She, all the while, is biting her lower lip and whimpering with pleasure as your ass swallows her length.  Once you've taken all she has to offer, you begin grinding your hips against her, rocking back and forth atop the gargoyle.  To your surprise, " + this.data.name + " grabs you, squeezing her long fingers into your [hips] and pulling you up, almost off her rod.  You decide to go with it; you grab her tits for support and start to bounce on her cock.");
            ButtModifier.displayStretchButt(character, 15, true, true, false);
            DisplayText("\n\nSoon, you're both moaning with pleasure, bucking and thrusting your hips to meet each other as you continue to ride " + this.data.name + "'s strap-on.  However, you can feel the clenching sensation of a coming anal orgasm building in your gut, and if the strained panting of your partner is any indication, she isn't far behind.  You start bouncing faster, increasing to a wild pace as your hole contracts on the metal rod inside it, instinctively trying to force out the intruder, but only serving to let waves of pleasure roll over you.  You don't let up throughout the orgasm, quite literally riding it out until " + this.data.name + " lets out a high-pitched shriek and grabs your back, smashing your face into her tits and holding you tight as she, too cums.  Suddenly she's the one pounding you, thrusting her hips into your [asshole] until you've both cum, hard, clutching each other as orgasmic pleasure slams through you.");

            DisplayText("\n\nStill panting from the ordeal, you slowly drag yourself off of " + this.data.name + "'s cock and roll onto your back, exhausted. By the time you've recovered enough to stumble to your feet, " + this.data.name + " is fast asleep beside you, snoring cutely, her metal cock still standing straight up in the air.  You give her a gentle pat on the thigh and get dressed, leaving the gargoyle snoozing peacefully as you head back to camp.");
        }
        character.orgasm();
        character.stats.sens += -1;
        MainScreen.doNext(Game.scenes.camp.returnToCampUseOneHour);
    ...
    private tailFuckGargoyleScene(character: Character) {
        ...
        DisplayText("\n\nBefore you can get used to the anal intruder, your eyes have rolled back in your head with pleasure.  Girlcum spurts from your cunt, splattering " + this.data.name + "'s face with your juices as you cum.  Your holes squeeze down on her appendages, trapping them inside you as your ride out the orgasm, bouncing on her rigid, rapidly-moistening tail until your body settles down.");

        DisplayText("\n\nWhen you've finished, you carefully extricate yourself from the gargoyle's tail and, a bit bow-legged, gather up your [armor].  By the time you're dressed, " + this.data.name + "'s cleaned herself off and is eagerly awaiting some token of your approval.");

        DisplayText("\n\n\"<i>Was Master satisfied?</i>\" she asks");
        if (this.gargoyleConfidence() > 70) DisplayText(" with just a hint of a smile");
        DisplayText(".");
    ...
    private useGargoyleMaleHerm(character: Character) {
        DisplayText().clear();
        ...
        DisplayText("\n\nBut you aren't quite satisfied just pounding an insensate hole.  Knowing that you have the power to \"revive\" " + this.data.name + " through touch, you decide to put this to rather devious use.  You reach around her body and grab one of her breasts, giving it a few hard squeezes until you hear the faintest gasp from the little gargoyle.  You release it immediately.  Your hand switches down to her taut ass and gives it a loud swat.  You repeat the action twice, and finally feel her cunt contract – just a bit – around your shaft.  You switch your attentions down to her thighs, gently running your hands along both expanses of gray skin beneath your thrusting [cock biggest].  You feel her shudder, ever so slightly, but just as she's on the verge of reviving, you pull back your hands.");

        DisplayText("\n\nYou return to thrusting into her, going fast and furious as your pleasure climbs steadily towards climax.  Only when you're bursting over the threshold of orgasm do you lay your hands on " + this.data.name + " again, grabbing her ass and giving one last massive thrust into her, shoving your [cock biggest] in to the hilt.  You cum inside her just as she awakens, moaning lustily into her ear as spunk drizzles out around your member.");

        DisplayText("\n\n\"<i>M-Master?</i>\" " + this.data.name + " gasps, giving a full-body shudder as you slowly pull out from inside her.  Your cum drools out of her snatch and pools beneath her.  Playfully, you jam your fingers up between her legs, damming your sperm inside her.  To your glee, this simple act sends " + this.data.name + " right over the edge: convulsing, she collapses on the floor with a heavy THUNK!");
    ...
    // [b]Use Her (Female)[/b]
    private useHerGargoyleFemale(character: Character) {
        DisplayText().clear();
        ...
        DisplayText("\n\nCompletely insensate, " + this.data.name + " remains in her low crouch ahead of you, not so much as breathing.  Still, she stretches you nice and wide as your bounce on her tail, spearing yourself over and over again on her smooth appendage.  By way of reward, you reach forward and slip a few fingers into her cool, tight cunny.  She can't respond as such, but you're confident she can still feel you sliding your fingers inside her, pushing in for each bounce you make atop her meaty tail.");

        DisplayText("\n\nYou're pleased to note that the gargoyle's tail is now dark and wet thanks to the liberal streams of lubricant pouring from your [vagina].  Now you're practically sliding on " + this.data.name + "'s tail, riding toward inevitable orgasm.  You cum at the apex of a bounce, slamming yourself down hard upon " + this.data.name + "'s tail until her tip breaches the entrance to your womb.  You scream in ecstasy, squirting your girl-cum freely onto the gargoyle's tail, back, and legs until you're completely and utterly spent.");

        DisplayText("\n\nContented, you spend a few minutes recovering before slowly dressing back up into your [armor]. You head on out, making sure to give " + this.data.name + " a little pat on the head on your way out.");
        character.orgasm();
        MainScreen.doNext(Game.scenes.camp.returnToCampUseOneHour);
    }

    // [b]Ritual (First Time)[/b]
    private ritualGargoyle(character: Character) {
        DisplayText().clear();
        if (this.data.ritualIntro === 0) {
            this.data.ritualIntro = 1;
            DisplayText("You note that the sanctuary's altar has been cleared of rubble, and a number of candles have been lit upon it. Resting atop the flat stone surface is a simple iron bowl filled with what appears to be water. Curious, you ask " + this.data.name + " what she's been up to.");
            ...
            if (this.gargoyleConfidence() < 50) DisplayText(" Forgive my impertinence, but");
        ...
        // [b]Ritual (Any after First)[/b]
            ...
            DisplayText("You walk to the church's alter, shortly followed by " + this.data.name + ".  You ask her if she's still able to perform the church's Rituals, to which she replies \"<i>Of course, Master.  Do you have need of one?</i>\"");

        }
        // (Display Options: [Body] [Mind] [Banish (only with Exgartuan or Worms)] and [Leave])
        MainScreen.hideBottomButtons();
        MainScreen.getBottomButton(0).modify("Body", this.bodyRitual);
        MainScreen.getBottomButton(1).modify("Mind", this.mindGargoyleRitual);
        if (character.statusAffects.has(StatusAffectType.Infested) || character.statusAffects.has(StatusAffectType.Exgartuan)) MainScreen.getBottomButton(2).modify("Banish", this.banishmentGargoyleRitual);
        MainScreen.getBottomButton(4).modify("Leave", this.returnToCathedralNoWake);
    }

    // [b]Body[/b]
    // (PC decreases Libido and Sensitivity. Suffers 20% Max HP damage, to a minimum of 1 pt. remaining)
    private bodyRitual(character: Character) {
        DisplayText().clear();
        DisplayText("You tell " + this.data.name + " that you could stand to reverse some of the corruption that has affected your body.  With an solemn nod, she takes you to a side alcove near the altar and instructs you to spread your arms and legs.  Hesitantly, you do so, and from the floor and ceiling " + this.data.name + " produces leather straps with rough looped straps.  Before you can react, she binds your arms and legs, spreading you eagle against the cold stone wall.");

        DisplayText("\n\n\"<i>With pain comes purity,</i>\" " + this.data.name + " says, so quietly you can barely hear her.  \"<i>Prepare yourself, Master,</i>\" she adds, and your heart nearly skips a beat when you hear the crack of a whip behind you.  Oh, shit, this is going to hu-CRACK!  You see stars as white-hot pain arcs through your body, making you scream into the wall.  You can feel the burning streak left across your back by the bullwhip, throbbing mercilessly in the moments before CRACK!  She hits you again, throwing you forward against your bonds with the force of the blow.");

        DisplayText("\n\nOver your ragged, pained breathing, you can just hear " + this.data.name + " chanting: \"<i>With pain comes purity... CRACK!  With agony comes enlightenment... CRACK!  Through torment comes holiness... CRACK!</i>\"  The last crack of the whip leaves you groaning, breathless, sagging in your bonds. Somehow, though, you do feel... cleaner?  Like a weight was just lifted from your heart.  And you know you're going to be thicker-skinned after that!  Quietly, reservedly, " + this.data.name + " releases you from your bonds, and you stagger into her waiting arms.  She helps you over to the altar and lays you down, letting you rest after your ordeal.  She stands over you, watching you pant and gasp.  You suppose you could get some vengeance for her brutality – or you could be kind.  What do you do?");
        ...
        MainScreen.hideBottomButtons();
        MainScreen.getBottomButton(0).modify("Nothing", this.noReactionToBodyRitual);
        MainScreen.getBottomButton(1).modify("Cuddle", this.gargoyleCuddleAfterBodyRitual);
        MainScreen.getBottomButton(2).modify("Revenge", this.gargoyleRevengeAfterBodyRitual);
    }

    // [b]Nothing[/b]
    private noReactionToBodyRitual() {
        DisplayText().clear();
        DisplayText("You simply lie still and try to recover from " + this.data.name + "'s \"gentle ministrations.\"  After a few minutes, you've caught your breath and, gathering your possessions, make your way out of the Cathedral without another word.  Damn, you're sore.");
        MainScreen.doNext(Game.scenes.camp.returnToCampUseOneHour);
    ...
    private mindGargoyleRitual(character: Character) {
        DisplayText().clear();
        DisplayText("You tell " + this.data.name + " that you've been plagued with dirty thoughts, and would like to undo some of the corruption that's spread to your mind.  " + this.data.name + " nods respectfully and tells you to kneel before the altar. You do so, assuming a low, supplicating stance before the tree-idol at the Cathedral's head as the gargoyle picks up the bowl of water and raises it on high.");
        ...
        DisplayText("  " + this.data.name + " brings the invocation to a quiet close, and rests the bowl back upon the altar. \"<i>Drink of this, Master,</i>\" she says, offering you the bowl, \"<i>and be healed of mind.</i>\"");

        DisplayText("\n\nYou do so, taking a long draught of the holy water.  By the time you've set the bowl down, you feel light-headed, as if a fog has receded from your mind.  You gasp happily, able to think more clearly than you have in days.  You thank " + this.data.name + ", give her a pat on the head, and head back to camp with a skip in your step.");
    ...
    private banishmentGargoyleRitual(character: Character) {
        DisplayText().clear();
        DisplayText("Talking about " + this.data.name + "'s rituals, you explain that you seem to have picked up ");
        if (character.statusAffects.has(StatusAffectType.Exgartuan)) DisplayText("a hitchhiker");
        else DisplayText("some hitchhikerss");
        DisplayText(" in your travels, and would like to rid yourself of them.  She tells you to strip and brace yourself against the altar.  \"<i>Unfortunately, as you have been invaded, so too is this a very... invasive ritual.</i>\"  You nod your assent – you really want to get ");
        if (character.statusAffects.has(StatusAffectType.Exgartuan)) DisplayText("this bastard");
        else DisplayText("these bastards");
        DisplayText(" out of your system.");

        DisplayText("\n\nFrom beneath the altar, " + this.data.name + " withdraws what looks like a perfectly cylindrical dildo, covered with dozens of dark, dimly-glowing blue runes.  Slowly, she makes her way around you and kneels behind your [butt].  \"<i>Take a deep breath, Master, and relax yourself.  It will be better this way.</i>\"");
        ...
        DisplayText("\n\n\"<i>Do not worry, Master,</i>\" " + this.data.name + " says");
        if (this.gargoyleConfidence() >= 50) DisplayText(" soothingly, using her free hand to stroke your thighs");
        DisplayText(".  \"<i>The pain is necessary.  Everything will be all right.</i>\"");
        ...
        // with Exgartuan:
        if (character.statusAffects.has(StatusAffectType.Exgartuan)) DisplayText("  Booming from within you, you hear the demon's voice: \"<i>Hey!  What the shit is this!?  What're you doing? Fucking stop, you ingrate!</i>\"");
        // with Worms:
        else DisplayText("  Gratifyingly, however, you can feel the infestation within you squirming and writhing, desperately seeking escape.");
        ...
        if (character.statusAffects.has(StatusAffectType.Exgartuan)) DisplayText("creature within you has");
        else DisplayText("creatures within you have");
        DisplayText(" been absorbed and purged by the rod, which " + this.data.name + " proceeds to chuck out a window.  Hopefully she has more, in case you ever get ");
        if (character.statusAffects.has(StatusAffectType.Exgartuan)) DisplayText("possessed");
        else DisplayText("infested");
        DisplayText(" again.");

        DisplayText("\n\nStill panting and gasping from your ordeal, you notice " + this.data.name + " looming over you, smiling.  You could be a dick for all that pain she caused you, or thank her for helping you.");
        if (character.statusAffects.has(StatusAffectType.Exgartuan)) character.statusAffects.remove(StatusAffectType.Exgartuan);
        else character.statusAffects.remove(StatusAffectType.Infested);
        character.stats.HP -= Math.round(character.stats.maxHP() / 3);
        character.stats.fatigue += 10;
        // (Display options: [Nothing] [Revenge] and [Cuddle])
        MainScreen.getBottomButton(0).modify("Nothing", this.dontFreakOutAfterBanishment);
        MainScreen.getBottomButton(1).modify("Cuddle", this.cuddleForBanishments);
        MainScreen.getBottomButton(2).modify("Revenge", this.getRevengeForBanishments);
    }

    // [b]Nothing[/b]
    private dontFreakOutAfterBanishment() {
        DisplayText().clear();
        DisplayText("You simply lie still and try to recover from " + this.data.name + "'s “gentle ministrations. ” After a few minutes, you've caught your breath and, gathering your possessions, make your way out of the Cathedral without another word. Damn, you're sore.");
        // (Return PC to camp, advance time 1 hour)
        MainScreen.doNext(Game.scenes.camp.returnToCampUseOneHour);
    ...
    // [b]Revenge[/b]
    // (Confidence -5)
    private getRevengeForBanishments() {
        DisplayText().clear();
        DisplayText("Seeing " + this.data.name + " just standing over you, gloating, causes you to sneer and lash out.  You strike her dead-center in the chest, throwing her back against the tree-idol at the church's head.  She slumps to the floor, looking meekly away from you.  While she's stunned, you gather your [armor] and stagger out of the Cathedral.");
        // (Return PC to camp, advance time 1 hour)
        this.gargoyleConfidence(-5);
    ...
    // (While Confidence 70+)
    private gargoyleKinkyRituals(character: Character) {
        DisplayText().clear();
        if (this.data.kinkyRitualsSpoken === 0) {
            DisplayText("You broach the subject of rituals to your gargoyle and are met with a surprisingly coy smile from her.  You ask her what's on her mind, to which " + this.data.name + " chuckles and says, \"<i>I've been thinking, Master. If you so desire, it may be possible to... enhance... the rituals that I've been using.</i>\"");
        ...
        // (While Confidence 70+)
            ...
            DisplayText("You walk to the church's alter, shortly followed by " + this.data.name + ".  You tell her you need one of her holy rituals, but that you'd prefer to have some fun as well.  Grinning, she says, \"<i>It would be my pleasure, my Master. What shall we do?</i>\"");
        }
        // (Display Options: [Body] [Mind] [Banish (only with Exgartuan or Worms)] and [Leave])
        MainScreen.hideBottomButtons();
        MainScreen.getBottomButton(0).modify("Body", this.kinkyBodyRitual);
        MainScreen.getBottomButton(1).modify("Mind", this.mindRitualPervy);
        if (character.statusAffects.has(StatusAffectType.Exgartuan) || character.statusAffects.has(StatusAffectType.Infested)) MainScreen.getBottomButton(2).modify("Banish", this.banishPervRitual);
        MainScreen.getBottomButton(4).modify("Leave", this.returnToCathedralNoWake);
    }

    // [b]Body[/b]
    private kinkyBodyRitual(character: Character) {
        DisplayText().clear();
        // (Confidence +2)
        // (PC decreases Libido and Sensitivity. Suffers 20% Max HP damage, to a minimum of 1 pt. remaining)
        DisplayText("You tell " + this.data.name + " that you could stand to reverse some of the corruption that has affected your body.  With an eager nod, she strips you down, takes you to a side alcove near the altar and instructs you to spread your arms and legs.  Putting your trust in your little gargoyle, you do so, allowing her to bind you spread-eagle with long leather straps.");
        ...
        DisplayText(".  She breaks the kiss and steps back, laying on one last time – CRACK! with the crop, stinging you again with an intense mix of pain and pleasure.  Gasping and panting from stimulation, you nearly collapse when " + this.data.name + " unbinds you, though she's quick to catch you before you fall.");

        DisplayText("\n\nShe guides you over to the altar and helps you lie atop it to rest.  Once you're comfortably situated, " + this.data.name + " slips up beside you, resting her cheek ");
        if (character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < 1) DisplayText("on your chest");
        else DisplayText("between your [chest]");
        DisplayText(".  Already, you feel more thick-skinned and less lusty, and so give an appreciative grin to your cute little gargoyle.  In response, she nuzzles against you and wraps her bat-like wings protectively around the two of you.");

        DisplayText("\n\nYou spend a few minutes cuddling with " + this.data.name + ", but sadly, you know you can't leave your camp unattended for too long.  Giving her a pat on the head and a kiss on the brow, you gather up your clothes and head on back to camp.");
    ...
    // (Confidence +2)
    // (PC Decreases Corruption and Lust, increases Intelligence)
    private mindRitualPervy(character: Character) {
        DisplayText().clear();
        this.gargoyleConfidence(2);
        DisplayText("You tell " + this.data.name + " that you've been plagued with dirty thoughts, and would like to undo some of the corruption that's spread to your mind.  With a deep nod conveying both a respect for your choice and an eagerness to engage in the ritual, " + this.data.name + " tells you to kneel before the altar.  You do so, assuming a low, supplicating stance before the tree-idol at the Cathedral's head as the gargoyle clears off the altar save for the humble bowl of water.");

        DisplayText("\n\n" + this.data.name + " lifts herself to perch atop the alter with a single beat of her wings, coming to rest cross-legged before you.  Grinning a sexy grin, she picks up the small waterbasin and lifts it up to her lips, taking a small drink of it.  \"<i>Blessed is " + character.desc.subjectivePronoun + " who seeks the pure,</i>\" she begins to chant, lowering the bowl from her thin lips. As she does so, she slowly raises her left leg, resting her toes upon your chin.  Locking eyes with you, " + this.data.name + " lowers the basin to her thigh and begins to pour, letting the water trickle sensually down her stone-smooth leg.  \"<i>Blessed is " + character.desc.subjectivePronoun + " who seeks wisdom,</i>\" she purrs as you instinctively open your mouth, allowing the holy water to trickle in from her foot.");

        DisplayText("\n\n" + this.data.name + " quickly slips her small toes into your mouth, letting you suckle the holy water from them like an infant.  She lets you remain like that for nearly a minute, nursing from the rivulets that come your way, before pulling her leg back and taking your head with it.  She leans backwards as you approach, balancing upon her bat-like wings as she finishes the chant: \"<i>And blessed is " + character.desc.subjectivePronoun + " who lusts after holiness and virtue before pleasures of the flesh.</i>\"  She pours from the basin once again, releasing the holy water directly onto the large, shapely mounds of her breasts.  The water runs down her chest and belly, creating a tiny waterfall over the nub of her clitoris to which she guides you.");

        DisplayText("\n\nYou do as she so obviously wishes, and spread her legs to get a good look at her now-sopping wet slit. Her vagina seems small, but an experimental touch proves it to be quite malleable – and your single touch causes " + this.data.name + " to arch her back, gasping with the sudden pleasure.  Smiling at the adorable, enraptured look on her face, you start lapping with a vengeance, grasping her thighs and burying your face between her legs.  You flick your tongue into her smooth, cool slit, lapping up a few drops of water with each flick of your dexterous tongue, and eliciting a gasp or moan each time.");

        DisplayText("\n\nSoon, " + this.data.name + " is clutching at her breasts and quickly fingering her cherry-nub clit as you lick her out, trying her best to keep an even rhythm with you, even as the waterfall comes to an end and you begin to feel light-headed, as if a fog has lifted from your mind.  By way of thanks, you redouble your efforts to pleasure her, ramming your tongue into her depths and fucking her mercilessly.  Quickly, she begins to tense, stopping her own actions to grasp at your " + HeadDescriptor.describeHair(character) + " and shove your face further into her crotch, locking her legs and tail around your back as she approaches orgasm.");

        DisplayText("\n\nScreaming, " + this.data.name + " arches her back and thrusts out her bat-wings to their full expanse, clamping down on your tongue as she cums.  You slow your pace, giving her a last few licks as she pants, gasps, and moans, slowly coming down from an orgasmic high.  She collapses backwards, clutching her chest and staring into the rafters.  Chuckling, you stand and give her a pleased pat on the inner thigh, letting her know she did a good job for you.  You leave her smiling atop the altar, slowly getting her breath under control.");
        // (Return PC to camp, advance time 1 hour)
    ...
    // [b]Banish[/b]
    // (Confidence +2)
    private banishPervRitual(character: Character) {
        DisplayText().clear();
        // (PC is cleared of Worms and/or Exgartuan)
        DisplayText("You explain to " + this.data.name + " that you seem to have picked up ");
        if (character.statusAffects.has(StatusAffectType.Exgartuan)) DisplayText("a hitchhiker");
        else DisplayText("some hitchhikers");
        DisplayText(" in your travels, and you're tired of ");
        if (character.statusAffects.has(StatusAffectType.Exgartuan)) DisplayText("it");
        else DisplayText("them");
        DisplayText(".  She gives you an understanding nod and tells you to strip.  As you do so, she dips beneath the altar, reappearing with a silver, blue-rune covered rod nestled into what looks like a leather thong.  " + this.data.name + " slips the thong on, letting a slight gasp escape her lips as the inner end of the rod enters her.");

        DisplayText("\n\nShe gives the shaft of the strap-on a few experimental strokes and, satisfied, says, \"<i>I've turned down the power of the runes, so it will not hurt you so much... though it will take longer than normal.  I suggest you lubricate it, Master.</i>\"  Smiling, she sits herself atop the alter and presents her silver “cock” to you.  You kneel and slip your face between her spread legs, giving the rod a quick lick.  It's incredibly smooth and quite cold, much like its wearer, but your tongue comes away with a warm tingle spreading through it – not pain, but more like numbness.  You take the rod into your mouth, running your tongue across its length and thrusting your face against it, blowing it like a cock while getting it sopping wet with your saliva.  You've only been at it for a minute when you notice " + this.data.name + " is bucking her hips slightly and moaning quietly, obviously getting more than a little enjoyment from the sensation.  Perhaps she can feel through it, like a real dick?  Grinning, you redouble your efforts to suck her off.");

        DisplayText("\n\n\"<i>Enough, Master,</i>\" she gasps, pushing you back.  Breathing heavily, she takes you by the hand and gets you to brace yourself against the altar.  Your entire mouth is numb now, tingling from the strange sensation given off by the holy rod-cock.  " + this.data.name + " slips behind you, putting a cool, soft hand on your [hips] as she lines up for penetration.  You grip down hard on the edge of the altar, and try your best to relax your sphincter as you begin to feel a tingling pressure against your back door.");
        if (character.torso.butt.wetness > 0) DisplayText("  She slips in easily, sliding her tingling holy rod into you until her hips are pressed against your [butt]");
        else DisplayText("  She slowly but surely coaxes the holy rod into your asshole, making long and slow thrusts into you until she finally bottoms out");
        DisplayText(".");

        DisplayText("\n\nWith her rod firmly inside you, " + this.data.name + " puts her hands on your [hips] for leverage and begins to pull out again, leaving a hot, intense tingling sensation in your rectum.  She thrusts in, this time much more quickly than you were expecting, making you gasp and moan as her hips slam into yours.");
        // If PC has Worms:
        if (!character.statusAffects.has(StatusAffectType.Exgartuan)) DisplayText("  You can feel her cock ram against your prostate, and shudder as the writhing mass of worms begin to panic as her cock pushes against them, obviously causing the creatures great pain.");
        DisplayText("  " + this.data.name + " begins to fuck you in earnest, hammering your [asshole] with the strap-on until you're both panting and gasping, nearly overwhelmed by sensation.");
        // (If PC has Exgartuan:
        if (character.statusAffects.has(StatusAffectType.Exgartuan)) DisplayText("  You can hear the demonic voice of Exgartuan inside your mind, yelling and cursing as he is rapidly drained from your body and sealed into the holy rod within you.");

        // (If PC has a cock:
        if (character.torso.cocks.count > 0) {
            DisplayText("\n\n" + this.data.name + " suddenly releases your hips and, pressing her soft breasts into your sweat-slicked back, grabs your [cock biggest] in her cold hand.  You let out a little gasp as she begins to jerk you off, rapidly pistoning your cock as she continues to pound your ass.  Between the dual stimulation, you feel a pressure begin to swell up inside your abused prostate, a tell-tale sign of orgasm.  She gives you a few last thrusts before you cum, splattering the altar with thick ropes of cum as you both yell in ecstasy.");
        }
        else {
            DisplayText("\n\n" + this.data.name + " reaches the peak of her speed, utterly reaming you as ");
            if (character.statusAffects.has(StatusAffectType.Exgartuan)) DisplayText("Exgartuan is");
            else DisplayText("the worms are");
            DisplayText(" sucked from your body. Suddenly, she leans in and presses her lips to yours, thrusting her tongue against yours, letting you feel her begin to convulse and shake with release. Cupping her cheek, you return the kiss with gusto, willing yourself toward an anal orgasm. Within moments, the incredible tingling and the rapid ass-fucking combine toward your own climax, and you squeeze down hard upon the rod inside you as you cum, shaking and shuddering in the gargoyle's grasp.");
        }

        DisplayText("\n\nSuddenly, the holy rod jammed up your [asshole] makes a loud hissing sound, and withdraws. Panting, you collapse to the ground, gripping your gut.  It's over, and though a part of you feels suddenly very empty, it's soon forgotten as " + this.data.name + " slips down beside you, having already tossed the holy rod out the window.  The creature");
        if (character.statusAffects.has(StatusAffectType.Exgartuan)) DisplayText(" within you has");
        else DisplayText("s within you have");
        DisplayText(" been absorbed and purged by the rod; and you can't help but hope she has more... just in case.");

        DisplayText("\n\nStill panting and gasping from your ordeal, you slip an arm around " + this.data.name + "'s shoulders, bringing her into a tight embrace.  Smiling cutely, she nuzzles her cheek against your chest, and winds her large bat-wings around the two of you, wrapping you together.  You give her a long, tender kiss on the brow, and let her remain like that for a good long while, basking in each others' presence.");

        DisplayText("\n\nEventually, though, you know you need to get back to camp.  You try to tell " + this.data.name + ", but to your surprise, she's fast asleep, snoring softly.  With a grin, you untangle yourself from her embrace and find something to cover her with before giving her another kiss and making your way home.");
        // (Return PC to camp, advance time 1 hour)
        if (character.statusAffects.has(StatusAffectType.Exgartuan)) character.statusAffects.remove(StatusAffectType.Exgartuan);
        else character.statusAffects.remove(StatusAffectType.Infested);
        character.stats.HP -= Math.round(character.stats.maxHP() / 3);
    ...
    private talkToGargoyleOutput() {
        DisplayText().clear();
        DisplayText("You take a seat on one of the sanctuary's pews, motioning for " + this.data.name + " to join you.");
        // If Confidence <50:
        if (this.gargoyleConfidence() < 50) DisplayText("  She meekly approaches her Master and kneels before you, head bowed submissively.");
        // if Confidence = 51-69:
        else if (this.gargoyleConfidence() < 70) DisplayText("  She nearly skips to follow you, assuming her low crouch at your feet and looking up at your expectantly.");
    ...
    // [b]Talk[/b]
    private talkToGargoyle() {
        // (Display Options: [History] [Cathedral] " + this.data.name + " [Leave])
        ...
        MainScreen.hideBottomButtons();
        if (this.data.history < 3) MainScreen.getBottomButton(0).modify("History", this.historyGo);
        if (this.data.cathedral < 3) MainScreen.getBottomButton(1).modify("Cathedral", this.cathedralTalks);
        if (this.data.nameTalks < 3) MainScreen.getBottomButton(2).modify("Her", this.talkAboutGarName);
        MainScreen.getBottomButton(4).modify("Back", this.returnToCathedralNoWake);
    }

    private historyGo(character: Character) {
        if (this.data.history === 0) {
            this.data.history = 1;
            this.talkToGargoyleHistoryA();
        }
        else if (this.data.history === 1) {
            this.historyOfGargoylesB();
            this.data.history = 2;
        }
        else {
            this.data.history = 3;
            this.gargoyleHistoryC(character);
        }
    }

    // [b]History A[/b]
    private talkToGargoyleHistoryA() {
        DisplayText().clear();

        DisplayText("You ask " + this.data.name + " what she can tell you about the history of Mareth.  She shrugs lightly.  \"<i>I can't tell you much, Master.  I've spent my entire life on these grounds.  Most of what I know was doctrine taught by the church. Is that acceptable?</i>\"");
    ...
    // [b]History C[/b]
    private gargoyleHistoryC(character: Character) {
        DisplayText().clear();
        ...
        if (kGAMECLASS.izmaScene.izmaFollower() && kGAMECLASS.isabellaFollowerScene.isabellaFollower()) {
            DisplayText("You ask " + this.data.name + " to tell you a little bit about the world of Mareth.  Giving it a few moments of quiet thought, she answers: \"<i>I do not have any experience outside of the church grounds, Master, but I remember the words of the last Master, and the parishioners before her.  There are... the world has a vast expanse of plains and woodlands, broken up by a mountain range and lake.  Those last two are populated by savage beasts, minotaurs, cow-girls, anemone, and shark-people, both of whom will rend your flesh or rape you half to death.</i>\"");
            DisplayText("\n\nWhoa, there. You stop " + this.data.name + " and explain to her that some of your lovers – mates, even – are just such beasts.  You tell her a little bit about Izma and Isabella, enforcing the fact that they're both not only quite friendly, but sweet as well.  They're just regular people");
            if (this.gargoyleConfidence() > 50) DisplayText(" just like her");
            DisplayText(".  If she could, you're sure " + this.data.name + " would be blushing bright crimson as you educate her on a little inter-special tolerance and understanding.  Chastened, she nods her comprehension when you're finished and, after a long pause, asks, \"<i>I-If Master really has many... girlfriends... does... no, it's silly, of course not...</i>\"");
            ...
            MainScreen.hideBottomButtons();
            MainScreen.getBottomButton(0).modify("Dodge", this.izmaAndIsabellaDodge);
            MainScreen.getBottomButton(1).modify("Slave", this.gargoylesAreSlavesYo);
            MainScreen.getBottomButton(2).modify("Girlfriend", this.gargoylesAreGirlfriends);
        }
        // (If PC has Marble as a follower)
        else if (character.statusAffects.has(StatusAffectType.CampMarble)) {
            DisplayText("You ask " + this.data.name + " to tell you a little bit about the world of Mareth.  Giving it a few moments of quiet thought, she answers: \"<i>I do not have any experience outside of the church grounds, Master, but I remember the words of the last Master, and the parishioners before her.  I believe there is a lake not far from here.  Beyond that, the world has a vast expanse of plains and woodlands, broken up by a great mountain range – strange monsters live there, but worst of all are savage bull-like beasts called Minotaurs.  And the cow-girls... Lacta-Bovines, I think they're called, who're just as nasty.</i>\"");
            DisplayText("\n\nYou cut " + this.data.name + " off right there, and explain that one of those 'savage beasts' is your lover, Marble, who's really quite sweet.  The gargoyle makes a horrified gasp and covers her mouth.  \"<i>I'm so sorry, Master! I didn't know...  All I'd heard were stories, years ago.  I didn't mean... but...  I-I didn't know Master had a girlfriend,</i>\" she whines pitifully.  Obviously, knowing that she's not your only lady-friend has upset the poor thing.");
            DisplayText("\n\nWell.  What do you say to that: that you're not exclusive with Marble, or that " + this.data.name + " doesn't count, being a statue and all?");
            // (Display Options: [Not Exclusive] [Doesn't Count])
            MainScreen.hideBottomButtons();
            MainScreen.getBottomButton(0).modify("Not Exclusive", this.marbleAintExclusiveBiatch);
            MainScreen.getBottomButton(1).modify("Doesn'tCount", this.gargoyleDoesntCountAsAWaifu);
        }
        // (If PC has Izma as a follower)
        else if (kGAMECLASS.izmaScene.izmaFollower()) {
            DisplayText("You ask " + this.data.name + " to tell you a little bit about the world of Mareth.  Giving it a few moments of quiet thought, she answers: \"<i>I do not have any experience outside of the church grounds, Master, but I remember the words of the last Master, and the parishioners before her.  There are... the world has a vast expanse of plains and woodlands, broken up by a great mountain range.  There is a lake not far from here, if I recall; a race of very dangerous shark-folk live there.</i>\"  With a chuckle, you tell " + this.data.name + " that you're more than well aware of that; indeed, you consider one such creature to be your mate, your beta.  She makes a slight gasp, and gushes out an apology.  \"<i>I'm so sorry, Master!  I hadn't realized... I thought...</i>\" she falls silent for a moment, then says shyly, \"<i>I wasn't aware Master had a girlfriend.</i>\"  Obviously, knowing that she's not your only lady-friend has upset the poor thing.");

            DisplayText("\n\nWell. What do you say to that: that you're not exclusive with Izma, or that she doesn't count, being a statue and all?");
            // (Display Options: [Not Exclusive] [Doesn't Count])
            MainScreen.hideBottomButtons();
            MainScreen.getBottomButton(0).modify("Not Exclusive", this.sharkgirlsArentExclusiveBiatch);
            MainScreen.getBottomButton(1).modify("Doesn'tCount", this.gargoylesDontCountAsSharkWaifus);
        }
        else {
    ...
    private gargoyleDoesntCountAsAWaifu(character: Character) {
        ...
        // (Return PC to " + this.data.name + "'s main menu)
        this.gargoyleConfidence(-5);
        this.returnToCathedral(character, true);
    }
    // IZMA
    // [b]Not Exclusive[/b]
    private sharkgirlsArentExclusiveBiatch(character: Character) {
        DisplayText().clear();
    ...
    // [b]Doesn't Count[/b]
    private gargoylesDontCountAsSharkWaifus(character: Character) {
        DisplayText().clear();
        // (Confidence -5)
        this.gargoyleConfidence(-5);
    ...
    // IZMA & ISABELLA
    // [b]Dodge[/b]
    private izmaAndIsabellaDodge(character: Character) {
        DisplayText().clear();
        // (Confidence -5)
        this.gargoyleConfidence(-5);
        DisplayText("You make a shitty joke and laugh awkwardly, trying to avoid the topic altogether.  " + this.data.name + " makes a pitiful little sniffle, but takes the hint and falls silent, allowing you to change the subject.");
    ...
    // [b]Girlfriend[/b]
    private gargoylesAreGirlfriends(character: Character) {
        // (Confidence +15)
        DisplayText().clear();

        this.gargoyleConfidence(15);
        DisplayText("You smile at " + this.data.name + " and tussle her hair.  Warmly, you tell her that, if she wants, she can be your girlfriend too.  Polyamory is just your thing, you suppose.  With a happy gasp, the little gargoyle leaps into your lap and wraps her arms around you, hugging you so tight you almost can't breathe.  She gives you a flurry of little kisses all along your neck and cheek before planting a long, deep one on your lips.  You kiss her back, soon laughing amicably as you return her embrace.  Once she's calmed down, " + this.data.name + " settles into your lap, resting her cheek against your chest and wrapping her tail and wings lovingly around you.  You give her a little peck on the forehead and nuzzle your chin into her hair, content to stay like this forever.");

        DisplayText("\n\nYet, you're soon surprised when " + this.data.name + " withdraws slightly from you, though her arms are still hooked about your neck.  \"<i>Master... if you want me to be your girlfriend, then... we should – if you want, Master, of course – maybe...  Master, will you make love to me?</i>\" she blurts, suddenly taken aback by her own forwardness.");
        ...
        MainScreen.hideBottomButtons();
        if (character.torso.cocks.count > 0) MainScreen.getBottomButton(0).modify("Vaginal", this.gargoyleCoochiiGetsPlowed);
        else MainScreen.getBottomButton(1).modify("Strap-on", this.strapOnGargoyle);
        MainScreen.getBottomButton(4).modify("Leave", Game.scenes.camp.returnToCampUseOneHour);
    }

    private cathedralTalks() {
        if (this.data.cathedral === 0) {
            this.data.cathedral++;
            this.talkCathedralA();
        }
        else if (this.data.cathedral === 1) {
            this.cathedralBTalk();
            this.data.cathedral = 2;
        }
        else {
            this.cathedralC();
            this.data.cathedral = 3;
        }
    }

    // [b]Cathedral A[/b]
    private talkCathedralA() {
        DisplayText().clear();
        DisplayText("You ask " + this.data.name + " to tell you a little about the Cathedral around you.  She visibly brightens at the idea.  \"<i>This is the greatest structure for miles around,</i>\" she claims proudly, motioning toward the vaulted ceiling and the remaining stained glass windows.  \"<i>Well, it isn't much to look at now, Master, but it was magnificent.  People came from all around to pray here.  It was wonderful...</i>\" she trails off, looking misty-eyed into the distance, remembering.");

        DisplayText("\n\n\"<i>Master?</i>\" she asks, looking you in the eye.  \"<i>I...  I want to rebuild the Cathedral.  I know there... there's no one to return to it, but...  I don't know.  It feels like repairing the damage that's been done would be a victory.  Just a little tiny one, but still a victory.</i>\"  " + this.data.name + " makes a sudden little gasp and says, \"<i>I-If that's all right with you, Master?</i>\"");

        DisplayText("\n\nYou could tell her it's pointless or encourage her.");
        // (Display Options: [Pointless] [Encourage])
        MainScreen.hideBottomButtons();
        MainScreen.getBottomButton(0).modify("Pointless", this.pointlessGargoylesArePointless);
        MainScreen.getBottomButton(1).modify("Encourage", this.encourageGargoyleWaifuToDoSomething);
    }

    // [b]Pointless[/b]
    // (Confidence -2)
    private pointlessGargoylesArePointless() {
        DisplayText().clear();
        this.gargoyleConfidence(-5);
    ...
    // [b]Encourage[/b]
    // (Confidence +2)
    private encourageGargoyleWaifuToDoSomething(character: Character) {
        DisplayText().clear();
        this.gargoyleConfidence(10);
        DisplayText("You give " + this.data.name + " a pat on the head and tell her it's a wonderful idea.  She makes a happy squeak and immediately flutters over to a pile of debris and starts cleaning it up.  Before you know it, the little gargoyle's on a roll, whistling a jaunty tune as she starts repairing some of the more obvious damage.  Seeing her so engrossed, you can't help but smile.  You wander over and start to give her a hand.");

        DisplayText("\n\nWithin the hour, the two of you have made a sizable dent in the debris of the Cathedral, and " + this.data.name + " has even managed to hang one of the candelabras back up in the rafters.  Exhausted, you say goodbye to the happy little gargoyle and head on back to camp.");
    ...
    private cathedralC() {
        DisplayText().clear();
        DisplayText("As you sit down with " + this.data.name + " to talk, you remember her words when you first met indicating that she could not – or would not – leave the Cathedral, even if you asked her to.   You ask her about that.");
        DisplayText("\n\n\"<i>Master,</i>\" " + this.data.name + " says, now looking you straight in the eye, \"<i>the Cathedral is my home, the only home I've ever known.  Even if I could leave, I would not want to");
        if (this.gargoyleConfidence() > 69) DisplayText(" unless it were to be with you, that is");
        DisplayText(".  But I am bound to this place, Master. I could not leave even if I wanted to.  My makers were wary of their creation, and chose to bind me here; they thought that if I ever rebelled, I would not do much harm if contained within the Cathedral.</i>\"");
    ...
    private comfortGargoyle() {
        ...
        DisplayText("\n\nTo your surprise, " + this.data.name + " follows you, running to catch up with you.  She takes your hand in hers and wraps one of her wings around you as you walk, holding you close as you make your way out the doors and onto the grounds.  Soon, though, you see " + this.data.name + " slowing and grimacing – approaching the edge of her boundary.  The two of you stop at the wrought-iron fence surrounding the Cathedral, and face each other.");
    ...
    // [b]Berate[/b]
    // (Confidence -10)
    private berateDatGargoyle4SomeSavin() {
        DisplayText().clear();
        DisplayText("Right.  Because becoming a living footstool for you was TOTALLY worth everyone she ever knew being raped out of their minds.  You give " + this.data.name + " a hard swat and remind her exactly how moronic she's being.  She recoils, shying away from you as you stand up and head for the door.  Over your shoulder, you tell her you're glad she's bound here – that way she can't fuck up and get you killed, too.");
        // (Return PC to camp, advance time 1 hour)
        this.gargoyleConfidence(-10);
        MainScreen.doNext(Game.scenes.camp.returnToCampUseOneHour);
    }

    private talkAboutGarName(character: Character) {
        if (this.data.nameTalks === 0) {
            this.data.nameTalks = 1;
            this.garNameA(character);
        }
        else if (this.data.nameTalks === 1) {
            this.data.nameTalks = 2;
            this.garNameB();
        }
        else {
            this.data.nameTalks = 3;
            this.garNameC();
        }
    }

    // [b]" + this.data.name + " A[/b]
    // (Confidence +3)
    private garNameA(character: Character) {
        DisplayText().clear();
        DisplayText("Curious about your gargoyle, you ask " + this.data.name + " to tell you a little bit about her life before you arrived.  She makes a nervous chuckle at the request, and shyly says, \"<i>Master is too kind...  I am no one special...</i>\"");
        ...
        if (this.gargoyleConfidence() > 69) DisplayText("  She continues, \"<i>though even were I not compelled to, I would still serve you,</i>\" she says with a playful wink.");
        ...
        // If Confidence =<69:
        if (this.gargoyleConfidence() <= 69) DisplayText("\n\nAfter what she did to me, I cannot enjoy sex; just the thought of it makes me shudder.</i>\"");
        else DisplayText("After what she did to me, I never thought I'd enjoy sex again.</i>\"  With a loving smile, " + this.data.name + " takes your hand in hers and brushes it against her cheek.  \"<i>But for you, Master, I think I could make an exception.</i>\"");
        ...
        // If Confidence =<69:
        if (this.gargoyleConfidence() <= 69) DisplayText("  \"<i>I'm sorry, Master, I can't... it hurts too much to think about it.</i>\"");
        else DisplayText("  \"<i>Sorry, Master. I just....Don't want to talk about it right now.  Ask me again some time.</i>\"");
    ...
    private garNameB() {
        ...
        DisplayText("\n\nYou don't need to prompt " + this.data.name + " this time.  \"<i>She left, perhaps ten years ago.  One day I was following orders to Master's pleasure, and the next I was bound to my pedestal, wearing the chains that Master broke to set me free.</i>\"");
    ...
    private garNameC() {
        ...
        MainScreen.hideBottomButtons();
        MainScreen.getBottomButton(0).modify("Comfort", this.comfortGarNameC);
        MainScreen.getBottomButton(1).modify("Berate", this.berateGargoyleC);
    }

    // [b]Comfort[/b]
    // (Confidence +15)
    private comfortGarNameC(character: Character) {
        DisplayText().clear();
        DisplayText("You pull " + this.data.name + " onto your lap and run your hand through her hair.  You tell her that she's wrong – dead wrong.  She is a person, and she is not helpless.  She's anything but.");
        ...
        DisplayText("\n\nShe smiles weakly, and rests her head on your chest.  \"<i>As you command, Master,</i>\" she says, and falls silent.  You sit there for a long while, stroking the gargoyle's soft hair and hoping you have made an impression on her.  But, eventually, you know you have to part.  You give " + this.data.name + " a little kiss on the forehead and tell her to do whatever she wants while you're gone.");
        ...
        DisplayText("\n\nSighing, you pat " + this.data.name + " on the head and tell her you'd be happy to stay for a while longer.  Beaming, she asks you what you'd like to do.");
    ...
    // [b]Appearance[/b]
    private gargoyleAppearance(character: Character) {
        DisplayText().clear();
        DisplayText("" + this.data.name + " is a gray marble statue that, through magics you can't quite grasp, has been animated.");
        ...
        if (this.gargoyleConfidence() < 70) DisplayText("glow timidly, reflecting a submissive and servile nature in the gargoyle.");
        else DisplayText("have a cool fire in them, showing her steadily-growing confidence and warmth toward you.");
        DisplayText("  Her skin is cold and smooth to the touch, a consequence of her marble material, yet she is quite soft, almost squishy.  " + this.data.name + " has a pair of cute little horns upon her brow, and has bat-like wings sprouting from her back which give a gentle flap every few seconds, an unconscious gesture on her part as you stare at her.  She has a long, spiked tail that curls ");
        if (this.gargoyleConfidence() < 70) DisplayText("timidly");
        else DisplayText("playfully");
        DisplayText(" about her thighs.  She has two normal human legs sculpted to end in normal human feet.");
