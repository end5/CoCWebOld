...
public function gargoylesTheShowNowOnWBNetwork():void {
	...
	if(player.statusAffects.get("TelAdre").value1 >= 1) outputText("or the ruins of one of Tel'Adre's more distant neighbors, ");
...
private function gargoyleMeeting2():void {
	...
	if(player.stats.lib >= 40) outputText("  Maybe you could take this with you as a life-sized sex toy?");
...
private function nameZeGargoyle():void {
	...
	clearOutput();
	outputText("\"<i>" + flags[FlagEnum.GAR_NAME] + ",</i>\" she purrs, \"<i>" + flags[FlagEnum.GAR_NAME] + ", " + flags[FlagEnum.GAR_NAME] + ".  Master has interesting taste.</i>\"");
...
private function carnalOrder():void {
	clearOutput();
	outputText("Well, well. " + flags[FlagEnum.GAR_NAME] + " seems willing to follow any order you give, no matter how carnal, and you could not help but notice her rather <i>arousing</i> attributes earlier.  Smirking wickedly, you command your new gargoyle to finger herself.");
	...
	if(player.stats.cor < 40) outputText("flustered");
...
private function firstGargoyleTalk():void {
	clearOutput();
	outputText("You decide against giving " + flags[FlagEnum.GAR_NAME] + " an order for now.  Instead, you right a nearby toppled pew and seat yourself across from the crouching gargoyle, returning her intense stare with one of your own.");
...
public function returnToCathedral(woken:boolean = false):void {
	...
	if(!woken) {
		clearOutput();
		if(rand(3) == 0) outputText("You make your way back to the ruined Cathedral in the wastelands.  You notice some of the debris has been cleared away from the courtyard, and some of the windows have been boarded up.  " + flags[FlagEnum.GAR_NAME] + "'s obviously been working since you last left, trying to make the building more habitable.  With your path less obstructed than before, you enter.");
	...
	if(model.time.hours <= 9 || model.time.hours >= 18 || woken) {
		if(!woken) {
			...
			outputText(" light streaming in from the broken windows, you see " + flags[FlagEnum.GAR_NAME] + " poking around some of the rubble, slowly but surely making efforts to repair the damage done to the Cathedral.  Seeing you, however, she immediately drops what she was doing and ");
...
private function gargoyleSexMenu():void {
	clearOutput();
	outputText("You approach " + flags[FlagEnum.GAR_NAME] + " and tell her you have some... pressure that needs relieving.");
	//(if Confidence =< 69:
...
private function gargoyleAnal():void {
	...
	//(if PC doesn't qualify for the above:)
		...
		//if Balls:
		if(player.lowerBody.balls > 0) outputText(", your balls slapping against her butt");
...
private function gargoyleCoochiiGetsPlowed():void {
	...
	if(gargoyleConfidence() >= 70) outputText("  Before you can stand, though, " + flags[FlagEnum.GAR_NAME] + " hugs her arms tightly around you pulling you down on top of her and mashing your face between her soft tits.");
...
private function titFuckTheGargoyle():void {
	...
	if(gargoyleConfidence() >= 70) outputText(", and to your surprise, so does " + flags[FlagEnum.GAR_NAME] + "");
	outputText(".");
...
private function strapOnGargoyle():void {
	clearOutput();
	outputText("Giving a quick look around the ruins, you tell " + flags[FlagEnum.GAR_NAME] + " to go find something that can bring you two a little <i>closer</i>.");
...
private function ritualGargoyle():void {
	...
	if(flags[FlagEnum.RITUAL_INTRO] == 0) {
		...
		outputText("You note that the sanctuary's altar has been cleared of rubble, and a number of candles have been lit upon it. Resting atop the flat stone surface is a simple iron bowl filled with what appears to be water. Curious, you ask " + flags[FlagEnum.GAR_NAME] + " what she's been up to.");
	...
	//[b]Ritual (Any after First)[/b]
		...
		outputText("You walk to the church's alter, shortly followed by " + flags[FlagEnum.GAR_NAME] + ".  You ask her if she's still able to perform the church's Rituals, to which she replies \"<i>Of course, Master.  Do you have need of one?</i>\"");
...
private function bodyRitual():void {
	clearOutput();
	outputText("You tell " + flags[FlagEnum.GAR_NAME] + " that you could stand to reverse some of the corruption that has affected your body.  With an solemn nod, she takes you to a side alcove near the altar and instructs you to spread your arms and legs.  Hesitantly, you do so, and from the floor and ceiling " + flags[FlagEnum.GAR_NAME] + " produces leather straps with rough looped straps.  Before you can react, she binds your arms and legs, spreading you eagle against the cold stone wall.");
...
private function noReactionToBodyRitual():void {
	clearOutput();
	outputText("You simply lie still and try to recover from " + flags[FlagEnum.GAR_NAME] + "'s \"gentle ministrations.\"  After a few minutes, you've caught your breath and, gathering your possessions, make your way out of the Cathedral without another word.  Damn, you're sore.");
	doNext(camp.returnToCampUseOneHour);
...
private function mindGargoyleRitual():void {
	...
	outputText("  " + flags[FlagEnum.GAR_NAME] + " brings the invocation to a quiet close, and rests the bowl back upon the altar. \"<i>Drink of this, Master,</i>\" she says, offering you the bowl, \"<i>and be healed of mind.</i>\"");
...
private function banishmentGargoyleRitual():void {
	clearOutput();
	outputText("Talking about " + flags[FlagEnum.GAR_NAME] + "'s rituals, you explain that you seem to have picked up ");
	if(player.statusAffects.has("Exgartuan")) outputText("a hitchhiker");
	...
	if(player.statusAffects.has("Exgartuan")) outputText("this bastard");
	...
	//with Exgartuan:
	if(player.statusAffects.has("Exgartuan")) outputText("  Booming from within you, you hear the demon's voice: \"<i>Hey!  What the shit is this!?  What're you doing? Fucking stop, you ingrate!</i>\"");
	//with Worms: 
	...
	if(player.statusAffects.has("Exgartuan")) outputText("creature within you has");
	else outputText("creatures within you have");
	outputText(" been absorbed and purged by the rod, which " + flags[FlagEnum.GAR_NAME] + " proceeds to chuck out a window.  Hopefully she has more, in case you ever get ");
	if(player.statusAffects.has("Exgartuan")) outputText("possessed");
...
private function gargoyleKinkyRituals():void {
	...
	//(While Confidence 70+)
		...
		outputText("You walk to the church's alter, shortly followed by " + flags[FlagEnum.GAR_NAME] + ".  You tell her you need one of her holy rituals, but that you'd prefer to have some fun as well.  Grinning, she says, \"<i>It would be my pleasure, my Master. What shall we do?</i>\"");
	}
	//(Display Options: [Body] [Mind] [Banish (only with Exgartuan or Worms)] and [Leave])
	menu();
...
private function kinkyBodyRitual():void {
	...
	//(Confidence +2)
	//(PC decreases Libido and Sensitivity. Suffers 20% Max HP damage, to a minimum of 1 pt. remaining)
	outputText("You tell " + flags[FlagEnum.GAR_NAME] + " that you could stand to reverse some of the corruption that has affected your body.  With an eager nod, she strips you down, takes you to a side alcove near the altar and instructs you to spread your arms and legs.  Putting your trust in your little gargoyle, you do so, allowing her to bind you spread-eagle with long leather straps.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText(", cupping your [chest] in her cold stone hands");
	...
	//if Male/Herm: 
	if(player.lowerBody.cockSpot.hasCock()) outputText(cockDescript(0));
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript());
	else outputText("barren crotch");
	outputText(".  She breaks the kiss and steps back, laying on one last time – CRACK! with the crop, stinging you again with an intense mix of pain and pleasure.  Gasping and panting from stimulation, you nearly collapse when " + flags[FlagEnum.GAR_NAME] + " unbinds you, though she's quick to catch you before you fall.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("on your chest");
...
private function banishPervRitual():void {
	clearOutput();
	//(PC is cleared of Worms and/or Exgartuan)
	outputText("You explain to " + flags[FlagEnum.GAR_NAME] + " that you seem to have picked up ");
	if(player.statusAffects.has("Exgartuan")) outputText("a hitchhiker");
	else outputText("some hitchhikers");
	outputText(" in your travels, and you're tired of ");
	if(player.statusAffects.has("Exgartuan")) outputText("it");
	else outputText("them");
	outputText(".  She gives you an understanding nod and tells you to strip.  As you do so, she dips beneath the altar, reappearing with a silver, blue-rune covered rod nestled into what looks like a leather thong.  " + flags[FlagEnum.GAR_NAME] + " slips the thong on, letting a slight gasp escape her lips as the inner end of the rod enters her.");
	...
	outputText("  " + flags[FlagEnum.GAR_NAME] + " begins to fuck you in earnest, hammering your [asshole] with the strap-on until you're both panting and gasping, nearly overwhelmed by sensation.");
	//(If PC has Exgartuan: 
	if(player.statusAffects.has("Exgartuan")) outputText("  You can hear the demonic voice of Exgartuan inside your mind, yelling and cursing as he is rapidly drained from your body and sealed into the holy rod within you.");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.statusAffects.has("Exgartuan")) outputText("Exgartuan is");
	...
	if(player.statusAffects.has("Exgartuan")) outputText(" within you has");
...
private function talkToGargoyle(output:boolean = true):void {
	if(output) {
		clearOutput();
		outputText("You take a seat on one of the sanctuary's pews, motioning for " + flags[FlagEnum.GAR_NAME] + " to join you.");
		//If Confidence <50:
...
private function talkToGargoyleHistoryA():void {
	...
	outputText("You ask " + flags[FlagEnum.GAR_NAME] + " what she can tell you about the history of Mareth.  She shrugs lightly.  \"<i>I can't tell you much, Master.  I've spent my entire life on these grounds.  Most of what I know was doctrine taught by the church. Is that acceptable?</i>\"");
...
private function comfortGargoyleDumbness():void {
	...
	//(Confidence +3)
	gargoyleConfidence(10);
	outputText("You sigh and reach over to " + flags[FlagEnum.GAR_NAME] + "; ");
	//if Confidence <50:
...
private function historyOfGargoylesB():void {
	clearOutput();
	outputText("You ask " + flags[FlagEnum.GAR_NAME] + " to tell you a little bit about the demons coming to Mareth");
...
private function gargoyleHistoryC():void {
	...
	if(kGAMECLASS.izmaScene.izmaFollower() && kGAMECLASS.isabellaFollowerScene.isabellaFollower()) {
		outputText("You ask " + flags[FlagEnum.GAR_NAME] + " to tell you a little bit about the world of Mareth.  Giving it a few moments of quiet thought, she answers: \"<i>I do not have any experience outside of the church grounds, Master, but I remember the words of the last Master, and the parishioners before her.  There are... the world has a vast expanse of plains and woodlands, broken up by a mountain range and lake.  Those last two are populated by savage beasts, minotaurs, cow-girls, anemone, and shark-people, both of whom will rend your flesh or rape you half to death.</i>\"");
	...
	//(If PC has Marble as a follower)
		...
		outputText("You ask " + flags[FlagEnum.GAR_NAME] + " to tell you a little bit about the world of Mareth.  Giving it a few moments of quiet thought, she answers: \"<i>I do not have any experience outside of the church grounds, Master, but I remember the words of the last Master, and the parishioners before her.  I believe there is a lake not far from here.  Beyond that, the world has a vast expanse of plains and woodlands, broken up by a great mountain range – strange monsters live there, but worst of all are savage bull-like beasts called Minotaurs.  And the cow-girls... Lacta-Bovines, I think they're called, who're just as nasty.</i>\"");
...
private function izmaAndIsabellaDodge():void {
	...
	//(Confidence -5)
	gargoyleConfidence(-5);
	outputText("You make a shitty joke and laugh awkwardly, trying to avoid the topic altogether.  " + flags[FlagEnum.GAR_NAME] + " makes a pitiful little sniffle, but takes the hint and falls silent, allowing you to change the subject.");
...
private function cathedralC():void {
	clearOutput();
	outputText("As you sit down with " + flags[FlagEnum.GAR_NAME] + " to talk, you remember her words when you first met indicating that she could not – or would not – leave the Cathedral, even if you asked her to.   You ask her about that.");
...
private function garNameA():void {
	clearOutput();
	outputText("Curious about your gargoyle, you ask " + flags[FlagEnum.GAR_NAME] + " to tell you a little bit about her life before you arrived.  She makes a nervous chuckle at the request, and shyly says, \"<i>Master is too kind...  I am no one special...</i>\"");
	...
	else outputText("After what she did to me, I never thought I'd enjoy sex again.</i>\"  With a loving smile, " + flags[FlagEnum.GAR_NAME] + " takes your hand in hers and brushes it against her cheek.  \"<i>But for you, Master, I think I could make an exception.</i>\"");
...
private function comfortGarNameC():void {
	clearOutput();
	outputText("You pull " + flags[FlagEnum.GAR_NAME] + " onto your lap and run your hand through her hair.  You tell her that she's wrong – dead wrong.  She is a person, and she is not helpless.  She's anything but.");
...
private function gargoyleAppearance():void {
	clearOutput();
	outputText("" + flags[FlagEnum.GAR_NAME] + " is a gray marble statue that, through magics you can't quite grasp, has been animated.");
