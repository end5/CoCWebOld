...
private gargoyleConfidence(arg:number = 0):number {
	if(arg != 0) {
		...
		else if(flags[FlagEnum.GAR_CONFIDENCE] < 0) flags[FlagEnum.GAR_CONFIDENCE] = 0;
	}
	return flags[FlagEnum.GAR_CONFIDENCE];
}
...
private breakZeChains():void {
	...
	//[Display a textbox, into which the player can type in a name. This new value will be called " + flags[FlagEnum.GAR_NAME] + ", henceforth. ] (Confidence +10)
...
private nameZeGargoyle():void {
	...
	flags[FlagEnum.GAR_NAME] = mainView.nameBox.text;
	mainView.nameBox.text = "";
	mainView.nameBox.visible = false;
	MainScreen.clearText();
	MainScreen.text("\"<i>" + flags[FlagEnum.GAR_NAME] + ",</i>\" she purrs, \"<i>" + flags[FlagEnum.GAR_NAME] + ", " + flags[FlagEnum.GAR_NAME] + ".  Master has interesting taste.</i>\"");
...
private carnalOrder():void {
	MainScreen.clearText();
	MainScreen.text("Well, well. " + flags[FlagEnum.GAR_NAME] + " seems willing to follow any order you give, no matter how carnal, and you could not help but notice her rather <i>arousing</i> attributes earlier.  Smirking wickedly, you command your new gargoyle to finger herself.");
...
private firstGargoyleTalk():void {
	MainScreen.clearText();
	MainScreen.text("You decide against giving " + flags[FlagEnum.GAR_NAME] + " an order for now.  Instead, you right a nearby toppled pew and seat yourself across from the crouching gargoyle, returning her intense stare with one of your own.");
	...
	MainScreen.text("\n\nSuddenly, " + flags[FlagEnum.GAR_NAME] + " breaks the stare and looks away, as if in shame.  \"<i>The demons' doom that even now blights Mareth took its toll here, too.  The people of a town no longer here believed they were safe under Marae's roof.  Protected.  Their faith was. . . Misplaced.</i>\"");
	
	MainScreen.text("\n\nIt seems the events that led to the cathedral's destruction weigh heavily on " + flags[FlagEnum.GAR_NAME] + "'s heart.  What do you do?");
...
private berateTheGargoyle():void {
	...
	MainScreen.text("\n\n" + flags[FlagEnum.GAR_NAME] + " recoils, flinching from you in expectation of assault. Instead, you spit on the ground and start for the exit.  You shout over your shoulder for her to stay here and wallow in her failure until you deign to use her.");
...
private reassureTheGargoyle():void {
	...
	MainScreen.text("\n\n" + flags[FlagEnum.GAR_NAME] + " looks up at you, her ruby-red eyes shining in the dim hit of light in the cathedral.  You cup her cheek and give her a smile.  \"<i>Come on, now,</i>\" you reply, offering her a hand up.  \"<i>Let's get out of here.</i>\"");
	...
	MainScreen.text("\n\nYou rustle " + flags[FlagEnum.GAR_NAME] + "'s hair and assure her you will before you take your leave and return to camp.");
...
public returnToCathedral(woken:boolean = false):void {
	...
	if(!woken) {
		MainScreen.clearText();
		if(rand(3) == 0) MainScreen.text("You make your way back to the ruined Cathedral in the wastelands.  You notice some of the debris has been cleared away from the courtyard, and some of the windows have been boarded up.  " + flags[FlagEnum.GAR_NAME] + "'s obviously been working since you last left, trying to make the building more habitable.  With your path less obstructed than before, you enter.");
	...
	if(model.time.hours <= 9 || model.time.hours >= 18 || woken) {
		if(!woken) {
			...
			MainScreen.text(" light streaming in from the broken windows, you see " + flags[FlagEnum.GAR_NAME] + " poking around some of the rubble, slowly but surely making efforts to repair the damage done to the Cathedral.  Seeing you, however, she immediately drops what she was doing and ");
			...
			MainScreen.text("\n\n\"<i>Master has returned,</i>\" " + flags[FlagEnum.GAR_NAME] + " says ");
		...
		if(player.lust >= 33) MainScreen.addButton(0,"Sex",gargoyleSexMenu);
		MainScreen.addButton(1,"Rituals",ritualGargoyle);
		if(flags[FlagEnum.GAR_HISTORY] < 3 || flags[FlagEnum.GAR_CATHEDRAL] < 3 || flags[FlagEnum.GAR_NAME_TALKS] < 3) MainScreen.addButton(2,"Talk",talkToGargoyle);
		MainScreen.addButton(3,"Appearance",gargoyleAppearance);
		if(gargoyleConfidence() >= 70 && flags[FlagEnum.RITUAL_INTRO] > 0) MainScreen.addButton(4,"Kinky Rituals",gargoyleKinkyRituals);
	}
	//[b]Cathedral Interior: 10:00 –> 17:00[/b]
	else {
		if(!woken) {
			MainScreen.text("\n\nWithin the ruins, you can see " + flags[FlagEnum.GAR_NAME] + " perched atop her pedestal in a low, predatory crouch, much as you found her.  Waving a hand in front of her eyes, you confirm she's in some sort of dormant mode.  A simple touch from her Master could revive her");
			if(player.lust >= 33) MainScreen.text(", or you could use her to satisfy your own urges, not unlike a life-size sex toy");
			MainScreen.text("."); 
		}
...
private gargoyleSexMenu():void {
	MainScreen.clearText();
	MainScreen.text("You approach " + flags[FlagEnum.GAR_NAME] + " and tell her you have some... pressure that needs relieving.");
	//(if Confidence =< 69:
...
private gargoyleAnal():void {
	...
	MainScreen.text("\n\n" + flags[FlagEnum.GAR_NAME] + " makes a sharp little gasp as you push your finger into her.  You slide into her with surprising ease, reveling in how <i>cold</i> her interior is.  She's like ice inside!  You start thrusting your finger faster, trying to get her warmed up a little as your prepare her anus.  " + flags[FlagEnum.GAR_NAME] + " squirms and moans softly as you finger-fuck her bum, slowly changing from moans of discomfort to little gasps of pleasure.");
	...
	if(player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockLength >= 50) {
		MainScreen.text("\n\nKnowing full well the living statue doesn't have much in the way of anatomy, you continue to push more and more of monstrous manhood into " + flags[FlagEnum.GAR_NAME] + "'s asshole, watching with glee as her stomach begins to distend with the massive size of your prick.  Then her chest bulges, making her gasp in pain and clutch at her breast; that doesn't stop you, though.  With one mighty, massive thrust, you slam yourself up to the hilt in " + flags[FlagEnum.GAR_NAME] + "'s ass, and watch with glee as the head bursts out of her mouth, squirting pre and drool all over the floor.  The poor gargoyle makes a muffled scream and starts flailing around helplessly, trying to do <i>something</i> about the massive member now protruding from her mouth.");
		
		MainScreen.text("\n\nWith gusto, you start hammering away at your living cock-sleeve, using her entire body as one great big ona-hole to take your gargantuan girth.  Resigned to her fate, " + flags[FlagEnum.GAR_NAME] + " slumps forward and grasps the head of your [cock biggest], giving you a handy through her mouth as her innards milk the rest of your shaft.");
...
private gargoyleCoochiiGetsPlowed():void {
	...
	MainScreen.text("You gently push " + flags[FlagEnum.GAR_NAME] + " onto her back and spread her long, marble legs, giving you a clear view of her tight little pussy.  You brush your hand along her thighs, making her gasp with the sensation as your knuckles run along her smooth, cool flesh.  Your fingers come to her small clit, and you caress her tiny pleasure buzzer, eliciting a tiny moan from the gargoyle.  Grinning, you give it another flick; " + flags[FlagEnum.GAR_NAME] + " squirms, trying to either enhance or end the sensation, you can't be sure.  You start to finger her clitty harder, creating little circles around it as you slip another finger into her tight passage.");
	
	MainScreen.text("\n\nYou're rewarded for your efforts by " + flags[FlagEnum.GAR_NAME] + " bucking her hips into your hands, moaning, \"<i>M-Master... please...</i>\"  Not done teasing her yet, you put your other hand on her flat belly and start to crawl it up, tickling her as you make your way toward her sizable breasts.  You glomp onto one of them, giving it a rough squeeze as you drive your finger into her a little further than before.  \"<i>Masterrrrr...</i>\" " + flags[FlagEnum.GAR_NAME] + " whines, reaching up to stroke your cheek.  Deciding you've teased the poor girl enough, you prepare for the main course.");
	
	MainScreen.text("\n\nYou pull your " + cockDescript(x) + " from your [armor], letting it flop down atop her crotch.  Embarrassed, " + flags[FlagEnum.GAR_NAME] + " looks away as you rub the underside of your cock across her mons, spitting on it for a bit of extra lubricant.  You lean back, lining the tip of your cockhead with the gargoyle's tight slit, and press forward.  She lets out a sharp gasp as your cock presses against her, pushing it past her lips and finally slipping into her depths.");
	...
	if(gargoyleConfidence() >= 70) MainScreen.text("  Before you can stand, though, " + flags[FlagEnum.GAR_NAME] + " hugs her arms tightly around you pulling you down on top of her and mashing your face between her soft tits.");
...
private titFuckTheGargoyle():void {
	MainScreen.clearText();
	MainScreen.text("You situate yourself on a pew and spread your legs.  You pat your thighs, telling " + flags[FlagEnum.GAR_NAME] + " to sit between your legs as you fish out your [cock biggest].  She approaches, hesitantly placing herself on the floor between your legs as you recline and stroke your shaft to full hardness.  You let the gargoyle sit there, watching you wide-eyed as you continue to masturbate.  Though you're only preparing for the main act, you can't help but enjoy the sensation of such a basic sexual act, made all the more pleasurable by the creature at your feet, her face barely an inch from your cock's head as you stroke yourself.");
	
	MainScreen.text("\n\nOnce you're satisfied at your cock's rigid hardness, you tell " + flags[FlagEnum.GAR_NAME] + " to put those big, soft tits of hers to work.  \"<i>M-Master?</i>\" she gasps, making a cute little “o” with her mouth just an inch from your shaft.  You repeat the order, and giver her a thump on the nose with your cock for emphasis.  Shyly, she cups her D-cups and leans into you, squeezing your [cock biggest] between them.");
	
	MainScreen.text("\n\nWith a little encouragement, " + flags[FlagEnum.GAR_NAME] + " starts to rise and fall on her knees, dragging your shaft through her soft, cool mounds.  With a pleasured sigh, you reach down and stroke her hair, petting the girl as she begins to slowly but steadily titfuck you.  Still mumbling encouragements, you sit back and let the pleasure run over you, reveling in the incredible softness of her sizable chest massaging your cock, making steady progress toward stroking the cum out of you.");
	
	MainScreen.text("\n\nSuddenly, you feel an overwhelming wetness on your cock.  You look down, expecting to see pre flowing freely from you, but are instead greeted with the sight of " + flags[FlagEnum.GAR_NAME] + " sliding the tip of your cock into her mouth, starting to fellate you on top of the tittyfuck.  You give her a few more words of encouragement and press her head down further onto your shaft, forcing her to take all that's left over from her breasts.  Despite her marble nature, her mouth is moist and surprisingly warm, and her long tongue is quick to wrap around your [cock biggest], wringing it in time with her chest-thrusts.");
	...
	MainScreen.text("\n\nYou grab " + flags[FlagEnum.GAR_NAME] + " and push her down hard, slamming your cock down her throat as you cum.  A human would surely have choked with this maneuver – instead, " + flags[FlagEnum.GAR_NAME] + " squirms and flails, but you can see and feel her swallow your cum as it pours into her mouth, her throat massaging your flaring head, milking it harder than any tit-job ever could have.  Once her face has been ground into your pubes, you buck your hips sharply, thrusting into her throat as the last of your jizz spurts out and down her gizzard.");
	
	MainScreen.text("\n\nBy the time you're done, " + flags[FlagEnum.GAR_NAME] + "'s face is an utter mess, with white lines trickling down her cheeks.  You drag your [cock biggest] out of her with a wet POP, unleashing a dam of jizz that quickly leaks out of her agape mouth to stain her breasts and belly.  You chuckle at the sight");
	if(gargoyleConfidence() >= 70) MainScreen.text(", and to your surprise, so does " + flags[FlagEnum.GAR_NAME] + "");
	MainScreen.text(".");
...
private strapOnGargoyle():void {
	MainScreen.clearText();
	MainScreen.text("Giving a quick look around the ruins, you tell " + flags[FlagEnum.GAR_NAME] + " to go find something that can bring you two a little <i>closer</i>.");
	
	MainScreen.text("\n\n\"<i>Closer?</i>\" she asks, confused.  You roll your eyes and tell her to grab the holy rod sitting on the altar and put it through a leather strap.  She hesitates for a moment, but dutifully does as you ask, returning a moment later with the smooth, silver cylinder and a leather thong.  With a bit of work, you've created a simple strap-on out of the holy rod, and instruct " + flags[FlagEnum.GAR_NAME] + " to put it on.  Now she's getting it!  The gargoyle slips into the leather, giving a cute little shiver as one end of the rod slips inside her.  Before she has a chance to recompose herself, you grab her rod-cock in one hand and give her a gentle push with the other.");
	
	MainScreen.text("\n\nShe lays down with you atop her, straddling her waist as you stroke her rod, rewarded with sharp pleasure-gasps from the statue-girl.  You're hardly stimulating her pussy with these actions – she must be able to feel <i>through</i> the rod, as if it were a real cock.  Amused, you start to jerk the rod off, sliding it through your clenched fist with slow, measured strokes.  " + flags[FlagEnum.GAR_NAME] + " squirms under the pleasurable attention, and is soon groping her big breasts and bucking her hips into your fists, shoving more of her rod-cock into your hand with each of your jerks.");
	
	MainScreen.text("\n\n\"<i>Well, someone's enjoying herself,</i>\" you tease, and immediately " + flags[FlagEnum.GAR_NAME] + " freezes, her mouth agape with embarrassment.  With a little laugh, you give her a pat on the thigh and start to strip out of your [armor].  But you can't just leave " + flags[FlagEnum.GAR_NAME] + " without SOME stimulation: ");
	//(If Human Legs/Feet [ie, needs toes], as new paragraph): 
	if(!player.isTaur() && !player.isGoo() && !player.isDrider() && !player.isNaga()) MainScreen.text("you reveal your leg first, moving slowly and sensually, giving her a tantalizing look at your nethers before hiding them again.  With a wide grin, you creep your foot up to her thighs and gently run your heel along her smooth, tender flesh.  You come to the base of her rod, standing stark straight in the air like a flag-pole.  As you strip, you grasp her cock between your big and second toe, sliding them up the smooth shaft to the tip.  " + flags[FlagEnum.GAR_NAME] + " moans, rolling her head back as you start to foot-fuck her cock, slowly putting more and more pressure on the steel member until it's pressed against her belly.  Your heel takes over, running quickly along the underside of her cock as you finally rid yourself of your [armor].");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		MainScreen.text("\n\n" + flags[FlagEnum.GAR_NAME] + " is panting with lust by the time you're done teasing and stripping for her. Now nude, you drop back to your knees over her holy rod-cock, straddling her hips and lining her up with your [vagina]. You lower yourself until the tip of the rod is pressing against your lower lips – you're pleasantly surprised by the warm tingling sensation that quickly spreads out around the point of contact, and your cunny responds immediately by letting loose a little trickle of moisture that beads down on the gargoyle's shaft.");
		
		MainScreen.text("\n\nWithout further ado, you drop down on the strap-on, lowering yourself onto the tingling metal rod until your groin is pressed against " + flags[FlagEnum.GAR_NAME] + "'s.  She, all the while, is biting her lower lip and whimpering with pleasure as your [vagina] swallows her length. Once you've taken all she has to offer, you begin grinding your hips against her, rocking back and forth atop the gargoyle.  To your surprise, " + flags[FlagEnum.GAR_NAME] + " grabs you, squeezing her long fingers into your [hips] and pulling you up, almost off her rod. You decide to go with it; you grab her tits for support and start to bounce on her cock.");
		player.cuntChange(15,true,true,false);
		MainScreen.text("\n\nSoon, you're both moaning with pleasure, bucking and thrusting your hips to meet each other as you continue to ride " + flags[FlagEnum.GAR_NAME] + "'s strap-on.  However, you can feel the clenching sensation of a coming orgasm building in your gut, and if the strained panting of your partner is any indication, she isn't far behind.  You start bouncing faster, increasing to a wild pace as your cunt contracts on the metal rod inside it, trying to milk the fake cock as waves of pleasure roll over you.  You don't let up throughout the orgasm, quite literally riding it out until " + flags[FlagEnum.GAR_NAME] + " lets out a high-pitched shriek and grabs your back, smashing your face into her tits and holding you tight as she, too cums.  Suddenly she's the one pounding you, thrusting her hips into your [vagina].");
		
		MainScreen.text("\n\nBy the time she's done, both your hips and thighs are spattered with your sweat and girlcum. Still panting from the ordeal, you slowly drag yourself off of " + flags[FlagEnum.GAR_NAME] + "'s cock and roll onto your back, exhausted. By the time you've recovered enough to stumble to your feet, " + flags[FlagEnum.GAR_NAME] + " is fast asleep beside you, snoring cutely, her metal cock still standing straight up in the air.  You give her a gentle pat on the thigh and get dressed, leaving the gargoyle snoozing peacefully as you head back to camp.");
	}
	//(ELSE, PC has no vagina:)
	else {
		MainScreen.text("\n\n" + flags[FlagEnum.GAR_NAME] + " is panting with lust by the time you're done teasing and stripping for her. Now nude, you drop back to your knees over her holy rod-cock, straddling her hips and lining her up with your [asshole].  You lower yourself until the tip of the rod is pressing against your clenched backdoor – and you're pleasantly surprised by the warm tingling sensation that quickly spreads out around the point of contact. Your anus responds immediately by utterly relaxing, letting just the tip of " + flags[FlagEnum.GAR_NAME] + "'s strap-on slide inside you.");
		
		MainScreen.text("\n\nWithout further ado, you drop down on the strap-on, lowering yourself onto the tingling metal rod until your ass is pressed against " + flags[FlagEnum.GAR_NAME] + "'s thighs.  She, all the while, is biting her lower lip and whimpering with pleasure as your ass swallows her length.  Once you've taken all she has to offer, you begin grinding your hips against her, rocking back and forth atop the gargoyle.  To your surprise, " + flags[FlagEnum.GAR_NAME] + " grabs you, squeezing her long fingers into your [hips] and pulling you up, almost off her rod.  You decide to go with it; you grab her tits for support and start to bounce on her cock.");
		player.buttChange(15,true,true,false);
		MainScreen.text("\n\nSoon, you're both moaning with pleasure, bucking and thrusting your hips to meet each other as you continue to ride " + flags[FlagEnum.GAR_NAME] + "'s strap-on.  However, you can feel the clenching sensation of a coming anal orgasm building in your gut, and if the strained panting of your partner is any indication, she isn't far behind.  You start bouncing faster, increasing to a wild pace as your hole contracts on the metal rod inside it, instinctively trying to force out the intruder, but only serving to let waves of pleasure roll over you.  You don't let up throughout the orgasm, quite literally riding it out until " + flags[FlagEnum.GAR_NAME] + " lets out a high-pitched shriek and grabs your back, smashing your face into her tits and holding you tight as she, too cums.  Suddenly she's the one pounding you, thrusting her hips into your [asshole] until you've both cum, hard, clutching each other as orgasmic pleasure slams through you.");
		
		MainScreen.text("\n\nStill panting from the ordeal, you slowly drag yourself off of " + flags[FlagEnum.GAR_NAME] + "'s cock and roll onto your back, exhausted. By the time you've recovered enough to stumble to your feet, " + flags[FlagEnum.GAR_NAME] + " is fast asleep beside you, snoring cutely, her metal cock still standing straight up in the air.  You give her a gentle pat on the thigh and get dressed, leaving the gargoyle snoozing peacefully as you head back to camp.");
	}
	player.orgasm();
	dynStats("sen", -1);
	doNext(camp.returnToCampUseOneHour);
...
private tailFuckGargoyleScene():void {
	...
	MainScreen.text("\n\nBefore you can get used to the anal intruder, your eyes have rolled back in your head with pleasure.  Girlcum spurts from your cunt, splattering " + flags[FlagEnum.GAR_NAME] + "'s face with your juices as you cum.  Your holes squeeze down on her appendages, trapping them inside you as your ride out the orgasm, bouncing on her rigid, rapidly-moistening tail until your body settles down.");
	
	MainScreen.text("\n\nWhen you've finished, you carefully extricate yourself from the gargoyle's tail and, a bit bow-legged, gather up your [armor].  By the time you're dressed, " + flags[FlagEnum.GAR_NAME] + "'s cleaned herself off and is eagerly awaiting some token of your approval.");
...
private useGargoyleMaleHerm():void {
	...
	MainScreen.text("\n\n\"<i>M-Master?</i>\" " + flags[FlagEnum.GAR_NAME] + " gasps, giving a full-body shudder as you slowly pull out from inside her.  Your cum drools out of her snatch and pools beneath her.  Playfully, you jam your fingers up between her legs, damming your sperm inside her.  To your glee, this simple act sends " + flags[FlagEnum.GAR_NAME] + " right over the edge: convulsing, she collapses on the floor with a heavy THUNK!");
...
private ritualGargoyle():void {
	...
	if(flags[FlagEnum.RITUAL_INTRO] == 0) {
		...
		MainScreen.text("You note that the sanctuary's altar has been cleared of rubble, and a number of candles have been lit upon it. Resting atop the flat stone surface is a simple iron bowl filled with what appears to be water. Curious, you ask " + flags[FlagEnum.GAR_NAME] + " what she's been up to.");
	...
	//[b]Ritual (Any after First)[/b]
		...
		MainScreen.text("You walk to the church's alter, shortly followed by " + flags[FlagEnum.GAR_NAME] + ".  You ask her if she's still able to perform the church's Rituals, to which she replies \"<i>Of course, Master.  Do you have need of one?</i>\"");
...
private noReactionToBodyRitual():void {
	MainScreen.clearText();
	MainScreen.text("You simply lie still and try to recover from " + flags[FlagEnum.GAR_NAME] + "'s \"gentle ministrations.\"  After a few minutes, you've caught your breath and, gathering your possessions, make your way out of the Cathedral without another word.  Damn, you're sore.");
	doNext(camp.returnToCampUseOneHour);
...
private mindGargoyleRitual():void {
	...
	MainScreen.text("  " + flags[FlagEnum.GAR_NAME] + " brings the invocation to a quiet close, and rests the bowl back upon the altar. \"<i>Drink of this, Master,</i>\" she says, offering you the bowl, \"<i>and be healed of mind.</i>\"");
	
	MainScreen.text("\n\nYou do so, taking a long draught of the holy water.  By the time you've set the bowl down, you feel light-headed, as if a fog has receded from your mind.  You gasp happily, able to think more clearly than you have in days.  You thank " + flags[FlagEnum.GAR_NAME] + ", give her a pat on the head, and head back to camp with a skip in your step.");
...
private banishmentGargoyleRitual():void {
	MainScreen.clearText();
	MainScreen.text("Talking about " + flags[FlagEnum.GAR_NAME] + "'s rituals, you explain that you seem to have picked up ");
	...
	MainScreen.text("\n\nFrom beneath the altar, " + flags[FlagEnum.GAR_NAME] + " withdraws what looks like a perfectly cylindrical dildo, covered with dozens of dark, dimly-glowing blue runes.  Slowly, she makes her way around you and kneels behind your [butt].  \"<i>Take a deep breath, Master, and relax yourself.  It will be better this way.</i>\"");
	...
	MainScreen.text("\n\n\"<i>Do not worry, Master,</i>\" " + flags[FlagEnum.GAR_NAME] + " says");
	...
	MainScreen.text(" been absorbed and purged by the rod, which " + flags[FlagEnum.GAR_NAME] + " proceeds to chuck out a window.  Hopefully she has more, in case you ever get ");
	if(player.statusAffects.has("Exgartuan")) MainScreen.text("possessed");
	else MainScreen.text("infested");
	MainScreen.text(" again.");
	
	MainScreen.text("\n\nStill panting and gasping from your ordeal, you notice " + flags[FlagEnum.GAR_NAME] + " looming over you, smiling.  You could be a dick for all that pain she caused you, or thank her for helping you.");
...
private gargoyleKinkyRituals():void {
	...
	//(While Confidence 70+)
		...
		MainScreen.text("You walk to the church's alter, shortly followed by " + flags[FlagEnum.GAR_NAME] + ".  You tell her you need one of her holy rituals, but that you'd prefer to have some fun as well.  Grinning, she says, \"<i>It would be my pleasure, my Master. What shall we do?</i>\"");
	}
	//(Display Options: [Body] [Mind] [Banish (only with Exgartuan or Worms)] and [Leave])
	menu();
...
private kinkyBodyRitual():void {
	...
	//(Confidence +2)
	//(PC decreases Libido and Sensitivity. Suffers 20% Max HP damage, to a minimum of 1 pt. remaining)
	MainScreen.text("You tell " + flags[FlagEnum.GAR_NAME] + " that you could stand to reverse some of the corruption that has affected your body.  With an eager nod, she strips you down, takes you to a side alcove near the altar and instructs you to spread your arms and legs.  Putting your trust in your little gargoyle, you do so, allowing her to bind you spread-eagle with long leather straps.");
	...
	//if Male/Herm: 
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text(cockDescript(0));
	else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(vaginaDescript());
	else MainScreen.text("barren crotch");
	MainScreen.text(".  She breaks the kiss and steps back, laying on one last time – CRACK! with the crop, stinging you again with an intense mix of pain and pleasure.  Gasping and panting from stimulation, you nearly collapse when " + flags[FlagEnum.GAR_NAME] + " unbinds you, though she's quick to catch you before you fall.");
	
	MainScreen.text("\n\nShe guides you over to the altar and helps you lie atop it to rest.  Once you're comfortably situated, " + flags[FlagEnum.GAR_NAME] + " slips up beside you, resting her cheek ");
	...
	MainScreen.text("\n\nYou spend a few minutes cuddling with " + flags[FlagEnum.GAR_NAME] + ", but sadly, you know you can't leave your camp unattended for too long.  Giving her a pat on the head and a kiss on the brow, you gather up your clothes and head on back to camp.");
...
private banishPervRitual():void {
	MainScreen.clearText();
	//(PC is cleared of Worms and/or Exgartuan)
	MainScreen.text("You explain to " + flags[FlagEnum.GAR_NAME] + " that you seem to have picked up ");
	if(player.statusAffects.has("Exgartuan")) MainScreen.text("a hitchhiker");
	...
	MainScreen.text(".  She gives you an understanding nod and tells you to strip.  As you do so, she dips beneath the altar, reappearing with a silver, blue-rune covered rod nestled into what looks like a leather thong.  " + flags[FlagEnum.GAR_NAME] + " slips the thong on, letting a slight gasp escape her lips as the inner end of the rod enters her.");
	
	MainScreen.text("\n\nShe gives the shaft of the strap-on a few experimental strokes and, satisfied, says, \"<i>I've turned down the power of the runes, so it will not hurt you so much... though it will take longer than normal.  I suggest you lubricate it, Master.</i>\"  Smiling, she sits herself atop the alter and presents her silver “cock” to you.  You kneel and slip your face between her spread legs, giving the rod a quick lick.  It's incredibly smooth and quite cold, much like its wearer, but your tongue comes away with a warm tingle spreading through it – not pain, but more like numbness.  You take the rod into your mouth, running your tongue across its length and thrusting your face against it, blowing it like a cock while getting it sopping wet with your saliva.  You've only been at it for a minute when you notice " + flags[FlagEnum.GAR_NAME] + " is bucking her hips slightly and moaning quietly, obviously getting more than a little enjoyment from the sensation.  Perhaps she can feel through it, like a real dick?  Grinning, you redouble your efforts to suck her off.");
	
	MainScreen.text("\n\n\"<i>Enough, Master,</i>\" she gasps, pushing you back.  Breathing heavily, she takes you by the hand and gets you to brace yourself against the altar.  Your entire mouth is numb now, tingling from the strange sensation given off by the holy rod-cock.  " + flags[FlagEnum.GAR_NAME] + " slips behind you, putting a cool, soft hand on your [hips] as she lines up for penetration.  You grip down hard on the edge of the altar, and try your best to relax your sphincter as you begin to feel a tingling pressure against your back door.");
	if(player.ass.analWetness > 0) MainScreen.text("  She slips in easily, sliding her tingling holy rod into you until her hips are pressed against your [butt]");
	...
	MainScreen.text("\n\nWith her rod firmly inside you, " + flags[FlagEnum.GAR_NAME] + " puts her hands on your [hips] for leverage and begins to pull out again, leaving a hot, intense tingling sensation in your rectum.  She thrusts in, this time much more quickly than you were expecting, making you gasp and moan as her hips slam into yours.");
	//If PC has Worms: 
	if(player.findStatusAffect(StatusAffects.Exgartuan) < 0) MainScreen.text("  You can feel her cock ram against your prostate, and shudder as the writhing mass of worms begin to panic as her cock pushes against them, obviously causing the creatures great pain.");
	MainScreen.text("  " + flags[FlagEnum.GAR_NAME] + " begins to fuck you in earnest, hammering your [asshole] with the strap-on until you're both panting and gasping, nearly overwhelmed by sensation.");
	//(If PC has Exgartuan: 
	...
	MainScreen.text("\n\nSuddenly, the holy rod jammed up your [asshole] makes a loud hissing sound, and withdraws. Panting, you collapse to the ground, gripping your gut.  It's over, and though a part of you feels suddenly very empty, it's soon forgotten as " + flags[FlagEnum.GAR_NAME] + " slips down beside you, having already tossed the holy rod out the window.  The creature");
	...
	MainScreen.text("\n\nStill panting and gasping from your ordeal, you slip an arm around " + flags[FlagEnum.GAR_NAME] + "'s shoulders, bringing her into a tight embrace.  Smiling cutely, she nuzzles her cheek against your chest, and winds her large bat-wings around the two of you, wrapping you together.  You give her a long, tender kiss on the brow, and let her remain like that for a good long while, basking in each others' presence.");
	
	MainScreen.text("\n\nEventually, though, you know you need to get back to camp.  You try to tell " + flags[FlagEnum.GAR_NAME] + ", but to your surprise, she's fast asleep, snoring softly.  With a grin, you untangle yourself from her embrace and find something to cover her with before giving her another kiss and making your way home.");
	//(Return PC to camp, advance time 1 hour)
	if(player.statusAffects.has("Exgartuan")) player.statusAffects.remove("Exgartuan");
	else player.statusAffects.remove("Infested");
	player.takeDamage(Math.round(player.maxHP()/3));
...
private talkToGargoyle(output:boolean = true):void {
	if(output) {
		MainScreen.clearText();
		MainScreen.text("You take a seat on one of the sanctuary's pews, motioning for " + flags[FlagEnum.GAR_NAME] + " to join you.");
		//If Confidence <50:
	...
	//(Display Options: [History] [Cathedral] " + flags[FlagEnum.GAR_NAME] + " [Leave])
	...
	menu();
	if(flags[FlagEnum.GAR_HISTORY] < 3) MainScreen.addButton(0,"History",historyGo);
	if(flags[FlagEnum.GAR_CATHEDRAL] < 3) MainScreen.addButton(1,"Cathedral",cathedralTalks);
	if(flags[FlagEnum.GAR_NAME_TALKS] < 3) MainScreen.addButton(2,"Her",talkAboutGarName);
...
private historyGo():void {
	...
	else if(flags[FlagEnum.GAR_HISTORY] == 1) {
		historyOfGargoylesB();
		flags[FlagEnum.GAR_HISTORY] = 2;
	}
	else {
		flags[FlagEnum.GAR_HISTORY] = 3;
		gargoyleHistoryC();
	}
}
...
private talkToGargoyleHistoryA():void {
	...
	MainScreen.text("You ask " + flags[FlagEnum.GAR_NAME] + " what she can tell you about the history of Mareth.  She shrugs lightly.  \"<i>I can't tell you much, Master.  I've spent my entire life on these grounds.  Most of what I know was doctrine taught by the church. Is that acceptable?</i>\"");
...
private historyOfGargoylesB():void {
	MainScreen.clearText();
	MainScreen.text("You ask " + flags[FlagEnum.GAR_NAME] + " to tell you a little bit about the demons coming to Mareth");
...
private gargoyleHistoryC():void {
	...
	if(kGAMECLASS.izmaScene.izmaFollower() && kGAMECLASS.isabellaFollowerScene.isabellaFollower()) {
		MainScreen.text("You ask " + flags[FlagEnum.GAR_NAME] + " to tell you a little bit about the world of Mareth.  Giving it a few moments of quiet thought, she answers: \"<i>I do not have any experience outside of the church grounds, Master, but I remember the words of the last Master, and the parishioners before her.  There are... the world has a vast expanse of plains and woodlands, broken up by a mountain range and lake.  Those last two are populated by savage beasts, minotaurs, cow-girls, anemone, and shark-people, both of whom will rend your flesh or rape you half to death.</i>\"");
		MainScreen.text("\n\nWhoa, there. You stop " + flags[FlagEnum.GAR_NAME] + " and explain to her that some of your lovers – mates, even – are just such beasts.  You tell her a little bit about Izma and Isabella, enforcing the fact that they're both not only quite friendly, but sweet as well.  They're just regular people");
		if(gargoyleConfidence() > 50) MainScreen.text(" just like her");
		MainScreen.text(".  If she could, you're sure " + flags[FlagEnum.GAR_NAME] + " would be blushing bright crimson as you educate her on a little inter-special tolerance and understanding.  Chastened, she nods her comprehension when you're finished and, after a long pause, asks, \"<i>I-If Master really has many... girlfriends... does... no, it's silly, of course not...</i>\"");
	...
	//(If PC has Marble as a follower)
		...
		MainScreen.text("You ask " + flags[FlagEnum.GAR_NAME] + " to tell you a little bit about the world of Mareth.  Giving it a few moments of quiet thought, she answers: \"<i>I do not have any experience outside of the church grounds, Master, but I remember the words of the last Master, and the parishioners before her.  I believe there is a lake not far from here.  Beyond that, the world has a vast expanse of plains and woodlands, broken up by a great mountain range – strange monsters live there, but worst of all are savage bull-like beasts called Minotaurs.  And the cow-girls... Lacta-Bovines, I think they're called, who're just as nasty.</i>\"");
		MainScreen.text("\n\nYou cut " + flags[FlagEnum.GAR_NAME] + " off right there, and explain that one of those 'savage beasts' is your lover, Marble, who's really quite sweet.  The gargoyle makes a horrified gasp and covers her mouth.  \"<i>I'm so sorry, Master! I didn't know...  All I'd heard were stories, years ago.  I didn't mean... but...  I-I didn't know Master had a girlfriend,</i>\" she whines pitifully.  Obviously, knowing that she's not your only lady-friend has upset the poor thing.");
		MainScreen.text("\n\nWell.  What do you say to that: that you're not exclusive with Marble, or that " + flags[FlagEnum.GAR_NAME] + " doesn't count, being a statue and all?");
...
private izmaAndIsabellaDodge():void {
	...
	//(Confidence -5)
	gargoyleConfidence(-5);
	MainScreen.text("You make a shitty joke and laugh awkwardly, trying to avoid the topic altogether.  " + flags[FlagEnum.GAR_NAME] + " makes a pitiful little sniffle, but takes the hint and falls silent, allowing you to change the subject.");
...
private gargoylesAreGirlfriends():void {
	...
	gargoyleConfidence(15);
	MainScreen.text("You smile at " + flags[FlagEnum.GAR_NAME] + " and tussle her hair.  Warmly, you tell her that, if she wants, she can be your girlfriend too.  Polyamory is just your thing, you suppose.  With a happy gasp, the little gargoyle leaps into your lap and wraps her arms around you, hugging you so tight you almost can't breathe.  She gives you a flurry of little kisses all along your neck and cheek before planting a long, deep one on your lips.  You kiss her back, soon laughing amicably as you return her embrace.  Once she's calmed down, " + flags[FlagEnum.GAR_NAME] + " settles into your lap, resting her cheek against your chest and wrapping her tail and wings lovingly around you.  You give her a little peck on the forehead and nuzzle your chin into her hair, content to stay like this forever.");
	
	MainScreen.text("\n\nYet, you're soon surprised when " + flags[FlagEnum.GAR_NAME] + " withdraws slightly from you, though her arms are still hooked about your neck.  \"<i>Master... if you want me to be your girlfriend, then... we should – if you want, Master, of course – maybe...  Master, will you make love to me?</i>\" she blurts, suddenly taken aback by her own forwardness.");
...
private cathedralTalks():void {
	...
	else if(flags[FlagEnum.GAR_CATHEDRAL] == 1) {
		cathedralBTalk();
		flags[FlagEnum.GAR_CATHEDRAL] = 2;
	}
	else {
		cathedralC();
		flags[FlagEnum.GAR_CATHEDRAL] = 3;
	}
}
...
private cathedralC():void {
	MainScreen.clearText();
	MainScreen.text("As you sit down with " + flags[FlagEnum.GAR_NAME] + " to talk, you remember her words when you first met indicating that she could not – or would not – leave the Cathedral, even if you asked her to.   You ask her about that.");
...
private comfortGargoyle():void {
	...
	MainScreen.text("\n\nTo your surprise, " + flags[FlagEnum.GAR_NAME] + " follows you, running to catch up with you.  She takes your hand in hers and wraps one of her wings around you as you walk, holding you close as you make your way out the doors and onto the grounds.  Soon, though, you see " + flags[FlagEnum.GAR_NAME] + " slowing and grimacing – approaching the edge of her boundary.  The two of you stop at the wrought-iron fence surrounding the Cathedral, and face each other.");
...
private talkAboutGarName():void {
	...
	else if(flags[FlagEnum.GAR_NAME_TALKS] == 1) {
		flags[FlagEnum.GAR_NAME_TALKS] = 2;
		garNameB();
	}
	else {
		flags[FlagEnum.GAR_NAME_TALKS] = 3;
		garNameC();
	}
}
...
private garNameA():void {
	MainScreen.clearText();
	MainScreen.text("Curious about your gargoyle, you ask " + flags[FlagEnum.GAR_NAME] + " to tell you a little bit about her life before you arrived.  She makes a nervous chuckle at the request, and shyly says, \"<i>Master is too kind...  I am no one special...</i>\"");
	...
	else MainScreen.text("After what she did to me, I never thought I'd enjoy sex again.</i>\"  With a loving smile, " + flags[FlagEnum.GAR_NAME] + " takes your hand in hers and brushes it against her cheek.  \"<i>But for you, Master, I think I could make an exception.</i>\"");
...
private garNameB():void {
	...
	MainScreen.text("\n\nYou don't need to prompt " + flags[FlagEnum.GAR_NAME] + " this time.  \"<i>She left, perhaps ten years ago.  One day I was following orders to Master's pleasure, and the next I was bound to my pedestal, wearing the chains that Master broke to set me free.</i>\"");
...
private comfortGarNameC():void {
	MainScreen.clearText();
	MainScreen.text("You pull " + flags[FlagEnum.GAR_NAME] + " onto your lap and run your hand through her hair.  You tell her that she's wrong – dead wrong.  She is a person, and she is not helpless.  She's anything but.");
	...
	MainScreen.text("\n\nShe smiles weakly, and rests her head on your chest.  \"<i>As you command, Master,</i>\" she says, and falls silent.  You sit there for a long while, stroking the gargoyle's soft hair and hoping you have made an impression on her.  But, eventually, you know you have to part.  You give " + flags[FlagEnum.GAR_NAME] + " a little kiss on the forehead and tell her to do whatever she wants while you're gone.");
	...
	MainScreen.text("\n\nSighing, you pat " + flags[FlagEnum.GAR_NAME] + " on the head and tell her you'd be happy to stay for a while longer.  Beaming, she asks you what you'd like to do.");
...
private gargoyleAppearance():void {
	MainScreen.clearText();
	MainScreen.text("" + flags[FlagEnum.GAR_NAME] + " is a gray marble statue that, through magics you can't quite grasp, has been animated.");
