...
public function gargoylesTheShowNowOnWBNetwork():void {
	...
	if(player.exploredDesert > 0) Render.text(", making even the desert seem healthy and full of life");
	Render.text(".  Your trip soon begins to seem unproductive, having found no new areas of Mareth or any contact with its inhabitants.  You sigh and turn back towards camp.");
	...
	if(player.statusAffects.get("TelAdre").value1 >= 1) Render.text("or the ruins of one of Tel'Adre's more distant neighbors, ");
	Render.text("or (even) some demon enclave that needs destroying.  With a quick check of your [weapon], you begin the long trek towards the shimmer in the distance.");
...
private function gargoyleMeeting2():void {
	...
	if(player.stats.lib >= 40) Render.text("  Maybe you could take this with you as a life-sized sex toy?");
...
private function nameZeGargoyle():void {
	if (kGAMECLASS.testingBlockExiting)
	{
		...
		clearOutput();
		Render.text("<b>You must name her.</b>", false);
		mainView.nameBox.text = "";
	...
	clearOutput();
	Render.text("\"<i>" + flags[FlagEnum.GAR_NAME] + ",</i>\" she purrs, \"<i>" + flags[FlagEnum.GAR_NAME] + ", " + flags[FlagEnum.GAR_NAME] + ".  Master has interesting taste.</i>\"");
...
private function carnalOrder():void {
	...
	if(player.stats.cor < 40) Render.text("flustered");
	else Render.text("excited");
	Render.text(" heartbeat.  The way she watches you, unblinking, as she fingers herself is rather unnerving, and you soon command her to stop.  She does so immediately, though with what you think might have been a sigh, and assumes a low crouch to await your next command.");
	gargoyleStarterMenu();
}
...
private function berateTheGargoyle():void {
	...
	Render.text("A pillar of rage boils up inside you.  You are [name], Champion of Ingnam, you tell her, rising to your feet, fists clenched.  Your duty is the same as hers was, to protect your home.  And she failed.  She let all those people down.  You tell her that she's worthless, disgusting, a failure.");
...
public function returnToCathedral(woken:boolean = false):void {
	...
	if(!woken) {
		clearOutput();
		if(rand(3) == 0) Render.text("You make your way back to the ruined Cathedral in the wastelands.  You notice some of the debris has been cleared away from the courtyard, and some of the windows have been boarded up.  " + flags[FlagEnum.GAR_NAME] + "'s obviously been working since you last left, trying to make the building more habitable.  With your path less obstructed than before, you enter.");
		//[b]Player Returns to the Cathedral B[/b]
		else if(rand(2) == 0) Render.text("You spot the Cathedral in the distance, and see plumes of smoke rising. Alarmed, you ready your [weaponName] and rush over.  To your relief, however, you discover the burning objects are just a few dead imps that were looking for easy pillage, but found the Cathedral's newly-awoken protector instead.  Stepping around their charred corpses, you enter.");
		//[b]Player Returns to the Cathedral C[/b]
		else Render.text("You wander into the Cathedral grounds, and are rather alarmed to see a pair of little green goblin sluts poking around outside, trying to cart off some of the stone tombstones for their nefarious schemes.  They see you approaching, however, and quickly flee – more interested in salvage than fighting or fucking today.  Chuckling to yourself, you enter.");
	}
	menu();
	...
	if(model.time.hours <= 9 || model.time.hours >= 18 || woken) {
		if(!woken) {
			Render.text("\n\nIn the dim ");
			if(model.time.hours <= 9) Render.text("early morning");
			else Render.text("late evening");
			Render.text(" light streaming in from the broken windows, you see " + flags[FlagEnum.GAR_NAME] + " poking around some of the rubble, slowly but surely making efforts to repair the damage done to the Cathedral.  Seeing you, however, she immediately drops what she was doing and ");
			if(gargoyleConfidence() < 70) Render.text("kneels before you, head bowed low.");
			else if(gargoyleConfidence() < 99) Render.text("bows low before you, a small smile on her stone lips.");
			else Render.text("with a sexy sway of her hips, approaches you and lays a small but tender kiss upon your cheek before bowing her head to you.");
			
			Render.text("\n\n\"<i>Master has returned,</i>\" " + flags[FlagEnum.GAR_NAME] + " says ");
			if(gargoyleConfidence() > 50) Render.text("happily.");
			else Render.text("solemnly.");
			Render.text("  \"<i>What would you have of me?</i>\"");
		}
		...
		if(flags[FlagEnum.GAR_HISTORY] < 3 || flags[FlagEnum.GAR_CATHEDRAL] < 3 || flags[FlagEnum.GAR_NAME_TALKS] < 3) addButton(2,"Talk",talkToGargoyle);
		addButton(3,"Appearance",gargoyleAppearance);
		if(gargoyleConfidence() >= 70 && flags[FlagEnum.RITUAL_INTRO] > 0) addButton(4,"Kinky Rituals",gargoyleKinkyRituals);
	}
	//[b]Cathedral Interior: 10:00 –> 17:00[/b]
		...
		if(!woken) {
			...
			if(player.lust >= 33) Render.text(", or you could use her to satisfy your own urges, not unlike a life-size sex toy");
			Render.text("."); 
		}
		//(Display options: [Wake Her] [Use Her (if Lust >= 30)] [Appearance] and [Leave])
		addButton(0,"Wake Her",returnToCathedral,true);
		if(player.lust >= 33) {
			if(player.lowerBody.cockSpot.hasCock()) addButton(1,"Use Her",useGargoyleMaleHerm);
			else if(player.lowerBody.vaginaSpot.hasVagina()) addButton(1,"Use Her",useHerGargoyleFemale);
		}
		addButton(3,"Appearance",gargoyleAppearance);
	}
...
private function gargoyleSexMenu():void {
	...
	//(if Confidence =< 69:
	if(gargoyleConfidence() <= 69) Render.text("  The gargoyle bows low before you, her face expressionless.  \"<i>By your command, Master.  How would you have me?</i>\"");
	else Render.text("  Her ruby-red eyes lock with yours and a sultry grin plays across her gray marble features.  She slides an arm around your waist and presses her sizable, surprisingly soft breasts against your chest.  \"<i>How would Master have me?</i>\"");
	menu();
...
private function gargoyleAnal():void {
	...
	//(if PC doesn't qualify for the above:)
		...
		//if Balls:
		if(player.lowerBody.balls > 0) Render.text(", your balls slapping against her butt");
		Render.text(".  \"<i>M-Master,</i>\" she pants, chest heaving after the experience.  You give her a playful push, leaving her face on the cold stone floor as you withdraw your cock to the tip from her cold stone insides.  She clenches up tight as you slam yourself back in, nearly making you cum on the spot.  Now you're the one moaning, and so decide to reward her with a sharp slap on the ass.  She makes a little squeak when you strike her, but she tightens up wonderfully, her anal passage trying to milk you as you withdraw.  You give her another swat as you push back in, and are treated to the same results.  Grinning, you pick up your pace, alternating between pounding her tight asshole and slapping her taut butt cheeks.");
...
private function gargoyleCoochiiGetsPlowed():void {
	...
	if(gargoyleConfidence() >= 70) Render.text("  Before you can stand, though, " + flags[FlagEnum.GAR_NAME] + " hugs her arms tightly around you pulling you down on top of her and mashing your face between her soft tits.");
...
private function titFuckTheGargoyle():void {
	...
	if(gargoyleConfidence() >= 70) Render.text(", and to your surprise, so does " + flags[FlagEnum.GAR_NAME] + "");
	Render.text(".");
...
private function strapOnGargoyle():void {
	...
	if(!player.isTaur() && !player.isGoo() && !player.isDrider() && !player.isNaga()) Render.text("you reveal your leg first, moving slowly and sensually, giving her a tantalizing look at your nethers before hiding them again.  With a wide grin, you creep your foot up to her thighs and gently run your heel along her smooth, tender flesh.  You come to the base of her rod, standing stark straight in the air like a flag-pole.  As you strip, you grasp her cock between your big and second toe, sliding them up the smooth shaft to the tip.  " + flags[FlagEnum.GAR_NAME] + " moans, rolling her head back as you start to foot-fuck her cock, slowly putting more and more pressure on the steel member until it's pressed against her belly.  Your heel takes over, running quickly along the underside of her cock as you finally rid yourself of your [armor].");
	//Else, continued from above paragraph:
	else Render.text("you giver her a good show, putting on a brief striptease for the gargoyle.  You strip your [armor] slowly, sensually, showing only the barest hints of your privates until the last moment, when the last piece of your clothing falls to the floor.");
...
private function tailFuckGargoyleScene():void {
	...
	Render.text("You shudder as you finally bottom out on her tail, taking it until your cunt is almost painfully stretched by her ever - thickening tail and you can feel the tip pressing against the entrance to your womb.");
	...
	if(gargoyleConfidence() > 70) Render.text(" with just a hint of a smile");
	Render.text(".");
...
private function ritualGargoyle():void {
	...
	if(flags[FlagEnum.RITUAL_INTRO] == 0) {
		...
		Render.text("You note that the sanctuary's altar has been cleared of rubble, and a number of candles have been lit upon it. Resting atop the flat stone surface is a simple iron bowl filled with what appears to be water. Curious, you ask " + flags[FlagEnum.GAR_NAME] + " what she's been up to.");
	...
	if(player.statusAffects.has("Infested") || player.statusAffects.has("Exgartuan")) addButton(2,"Banish",banishmentGargoyleRitual);
	addButton(4,"Leave",returnToCathedral);
}
...
private function mindGargoyleRitual():void {
	...
	Render.text("  " + flags[FlagEnum.GAR_NAME] + " brings the invocation to a quiet close, and rests the bowl back upon the altar. \"<i>Drink of this, Master,</i>\" she says, offering you the bowl, \"<i>and be healed of mind.</i>\"");
...
private function banishmentGargoyleRitual():void {
	...
	else Render.text("some hitchhikerss");
	Render.text(" in your travels, and would like to rid yourself of them.  She tells you to strip and brace yourself against the altar.  \"<i>Unfortunately, as you have been invaded, so too is this a very... invasive ritual.</i>\"  You nod your assent – you really want to get ");
	if(player.statusAffects.has("Exgartuan")) Render.text("this bastard");
	else Render.text("these bastards");
	Render.text(" out of your system.");
	...
	if(gargoyleConfidence() >= 50) Render.text(" soothingly, using her free hand to stroke your thighs");
	Render.text(".  \"<i>The pain is necessary.  Everything will be all right.</i>\"");
	...
	//with Exgartuan:
	if(player.statusAffects.has("Exgartuan")) Render.text("  Booming from within you, you hear the demon's voice: \"<i>Hey!  What the shit is this!?  What're you doing? Fucking stop, you ingrate!</i>\"");
	//with Worms: 
	else Render.text("  Gratifyingly, however, you can feel the infestation within you squirming and writhing, desperately seeking escape.");
	...
	if(player.statusAffects.has("Exgartuan")) Render.text("creature within you has");
	else Render.text("creatures within you have");
	Render.text(" been absorbed and purged by the rod, which " + flags[FlagEnum.GAR_NAME] + " proceeds to chuck out a window.  Hopefully she has more, in case you ever get ");
	if(player.statusAffects.has("Exgartuan")) Render.text("possessed");
	else Render.text("infested");
	Render.text(" again.");
...
private function gargoyleKinkyRituals():void {
	...
	//(While Confidence 70+)
		...
		Render.text("You walk to the church's alter, shortly followed by " + flags[FlagEnum.GAR_NAME] + ".  You tell her you need one of her holy rituals, but that you'd prefer to have some fun as well.  Grinning, she says, \"<i>It would be my pleasure, my Master. What shall we do?</i>\"");
	}
	//(Display Options: [Body] [Mind] [Banish (only with Exgartuan or Worms)] and [Leave])
	menu();
	addButton(0,"Body",kinkyBodyRitual);
	addButton(1,"Mind",mindRitualPervy);
	if(player.statusAffects.has("Exgartuan") || player.statusAffects.has("Infested")) addButton(2,"Banish",banishPervRitual);
	addButton(4,"Leave",returnToCathedral);
}
...
private function kinkyBodyRitual():void {
	...
	//(Confidence +2)
	//(PC decreases Libido and Sensitivity. Suffers 20% Max HP damage, to a minimum of 1 pt. remaining)
	Render.text("You tell " + flags[FlagEnum.GAR_NAME] + " that you could stand to reverse some of the corruption that has affected your body.  With an eager nod, she strips you down, takes you to a side alcove near the altar and instructs you to spread your arms and legs.  Putting your trust in your little gargoyle, you do so, allowing her to bind you spread-eagle with long leather straps.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) Render.text(", cupping your [chest] in her cold stone hands");
	Render.text(".  Your [nipples] go rock hard from her cold touch, and you shudder as she gently nips your ear and drags the length of the crop along your thighs.");
	...
	//if Male/Herm: 
	if(player.lowerBody.cockSpot.hasCock()) Render.text(cockDescript(0));
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(vaginaDescript());
	else Render.text("barren crotch");
	Render.text(".  She breaks the kiss and steps back, laying on one last time – CRACK! with the crop, stinging you again with an intense mix of pain and pleasure.  Gasping and panting from stimulation, you nearly collapse when " + flags[FlagEnum.GAR_NAME] + " unbinds you, though she's quick to catch you before you fall.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) Render.text("on your chest");
	else Render.text("between your [chest]");
	Render.text(".  Already, you feel more thick-skinned and less lusty, and so give an appreciative grin to your cute little gargoyle.  In response, she nuzzles against you and wraps her bat-like wings protectively around the two of you.");
...
private function banishPervRitual():void {
	...
	if(player.statusAffects.has("Exgartuan")) Render.text("a hitchhiker");
	else Render.text("some hitchhikers");
	Render.text(" in your travels, and you're tired of ");
	if(player.statusAffects.has("Exgartuan")) Render.text("it");
	else Render.text("them");
	Render.text(".  She gives you an understanding nod and tells you to strip.  As you do so, she dips beneath the altar, reappearing with a silver, blue-rune covered rod nestled into what looks like a leather thong.  " + flags[FlagEnum.GAR_NAME] + " slips the thong on, letting a slight gasp escape her lips as the inner end of the rod enters her.");
	...
	if(player.ass.analWetness > 0) Render.text("  She slips in easily, sliding her tingling holy rod into you until her hips are pressed against your [butt]");
	else Render.text("  She slowly but surely coaxes the holy rod into your asshole, making long and slow thrusts into you until she finally bottoms out");
	Render.text(".");
	...
	//If PC has Worms: 
	if(player.findStatusAffect(StatusAffects.Exgartuan) < 0) Render.text("  You can feel her cock ram against your prostate, and shudder as the writhing mass of worms begin to panic as her cock pushes against them, obviously causing the creatures great pain.");
	Render.text("  " + flags[FlagEnum.GAR_NAME] + " begins to fuck you in earnest, hammering your [asshole] with the strap-on until you're both panting and gasping, nearly overwhelmed by sensation.");
	//(If PC has Exgartuan: 
	if(player.statusAffects.has("Exgartuan")) Render.text("  You can hear the demonic voice of Exgartuan inside your mind, yelling and cursing as he is rapidly drained from your body and sealed into the holy rod within you.");
	...
	if(player.statusAffects.has("Exgartuan")) Render.text(" within you has");
	else Render.text("s within you have");
	Render.text(" been absorbed and purged by the rod; and you can't help but hope she has more... just in case.");
...
private function talkToGargoyle(output:boolean = true):void {
	if(output) {
		...
		//If Confidence <50:
		if(gargoyleConfidence() < 50) Render.text("  She meekly approaches her Master and kneels before you, head bowed submissively.");
		//if Confidence = 51-69: 
		else if(gargoyleConfidence() < 70) Render.text("  She nearly skips to follow you, assuming her low crouch at your feet and looking up at your expectantly.");
		else Render.text("  Happily, she takes a seat beside you and slips an arm around your shoulders, locking you in companionable closeness.");
		Render.text("  You tell her you'd like to talk for a few minutes, which immediately causes her eyes to brighten excitedly.  \"<i>Master honors me...  What would you like to talk about?</i>\"");
	}
	...
	menu();
	if(flags[FlagEnum.GAR_HISTORY] < 3) addButton(0,"History",historyGo);
	if(flags[FlagEnum.GAR_CATHEDRAL] < 3) addButton(1,"Cathedral",cathedralTalks);
	if(flags[FlagEnum.GAR_NAME_TALKS] < 3) addButton(2,"Her",talkAboutGarName);
...
private function historyOfGargoylesB():void {
	...
	Render.text(".  ");
	Render.text("Hesitantly, she nods her assent, though she says, \"<i>I'm sorry, Master, but I must warn you: I was greatly abused by the demons.  They are responsible for everything that is wrong with my world");
	//if Confidence >70: 
	if(gargoyleConfidence() > 70) Render.text(", and I despise them with every fiber of my being");
	Render.text(".  I may not be the most fair nor most balanced source of information.</i>\"");
...
private function gargoyleHistoryC():void {
	...
	if(kGAMECLASS.izmaScene.izmaFollower() && kGAMECLASS.isabellaFollowerScene.isabellaFollower()) {
		Render.text("You ask " + flags[FlagEnum.GAR_NAME] + " to tell you a little bit about the world of Mareth.  Giving it a few moments of quiet thought, she answers: \"<i>I do not have any experience outside of the church grounds, Master, but I remember the words of the last Master, and the parishioners before her.  There are... the world has a vast expanse of plains and woodlands, broken up by a mountain range and lake.  Those last two are populated by savage beasts, minotaurs, cow-girls, anemone, and shark-people, both of whom will rend your flesh or rape you half to death.</i>\"");
	...
	//(If PC has Marble as a follower)
		...
		Render.text("You ask " + flags[FlagEnum.GAR_NAME] + " to tell you a little bit about the world of Mareth.  Giving it a few moments of quiet thought, she answers: \"<i>I do not have any experience outside of the church grounds, Master, but I remember the words of the last Master, and the parishioners before her.  I believe there is a lake not far from here.  Beyond that, the world has a vast expanse of plains and woodlands, broken up by a great mountain range – strange monsters live there, but worst of all are savage bull-like beasts called Minotaurs.  And the cow-girls... Lacta-Bovines, I think they're called, who're just as nasty.</i>\"");
...
private function gargoylesDontCountAsSharkWaifus():void {
	...
	//(Confidence -5)
	gargoyleConfidence(-5);
	Render.text("You scoff and tell her not to worry – it's not like you're cheating on Izma by being around her, after all.  She's just a statue – a toy, really.  Her head sinks low, and the little gargoyle lets out a shudder that might just have been a sob.  You shake your head and change the subject.");
	//(Return PC to " + flags[FlagEnum.GAR_NAME] + "'s main menu)
	returnToCathedral(true);
}
...
private function izmaAndIsabellaDodge():void {
	...
	//(Confidence -5)
	gargoyleConfidence(-5);
	Render.text("You make a shitty joke and laugh awkwardly, trying to avoid the topic altogether.  " + flags[FlagEnum.GAR_NAME] + " makes a pitiful little sniffle, but takes the hint and falls silent, allowing you to change the subject.");
...
private function gargoylesAreGirlfriends():void {
	...
	menu();
	if(player.lowerBody.cockSpot.hasCock()) addButton(0,"Vaginal",gargoyleCoochiiGetsPlowed);
...
private function cathedralC():void {
	...
	if(gargoyleConfidence() > 69) Render.text(" unless it were to be with you, that is");
	Render.text(".  But I am bound to this place, Master. I could not leave even if I wanted to.  My makers were wary of their creation, and chose to bind me here; they thought that if I ever rebelled, I would not do much harm if contained within the Cathedral.</i>\"");
...
private function garNameA():void {
	...
	if(player.race() == "human") Render.text("are");
	else Render.text("once were");
	Render.text(", Master, and more.  We were to be their guardians, protectors – their perfect servants.</i>\"");
	...
	if(gargoyleConfidence() > 69) Render.text("  She continues, \"<i>though even were I not compelled to, I would still serve you,</i>\" she says with a playful wink.");
	...
	//If Confidence =<69:
	if(gargoyleConfidence() <= 69) Render.text("\n\nAfter what she did to me, I cannot enjoy sex; just the thought of it makes me shudder.</i>\"");
	else Render.text("After what she did to me, I never thought I'd enjoy sex again.</i>\"  With a loving smile, " + flags[FlagEnum.GAR_NAME] + " takes your hand in hers and brushes it against her cheek.  \"<i>But for you, Master, I think I could make an exception.</i>\"");
	...
	//If Confidence =<69:
	if(gargoyleConfidence() <= 69) Render.text("  \"<i>I'm sorry, Master, I can't... it hurts too much to think about it.</i>\"");
	else Render.text("  \"<i>Sorry, Master. I just....Don't want to talk about it right now.  Ask me again some time.</i>\"");
...
private function gargoyleAppearance():void {
	...
	if(gargoyleConfidence() < 70) Render.text("glow timidly, reflecting a submissive and servile nature in the gargoyle.");
	else Render.text("have a cool fire in them, showing her steadily-growing confidence and warmth toward you.");
	Render.text("  Her skin is cold and smooth to the touch, a consequence of her marble material, yet she is quite soft, almost squishy.  " + flags[FlagEnum.GAR_NAME] + " has a pair of cute little horns upon her brow, and has bat-like wings sprouting from her back which give a gentle flap every few seconds, an unconscious gesture on her part as you stare at her.  She has a long, spiked tail that curls ");
	if(gargoyleConfidence() < 70) Render.text("timidly");
	else Render.text("playfully");
	Render.text(" about her thighs.  She has two normal human legs sculpted to end in normal human feet.");
