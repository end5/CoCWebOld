export default class Lumi {
	public lumiEncounter(): void {
		...
		if (Flags.list[FlagEnum.LUMI_MET] == 0) {
			//placeholder text for outside the cathedral
			MainScreen.text("You spot an anomaly in the barren wastes; a door that seems to be standing out in the middle of nowhere. Somehow, you figure that it must lead to some other part of the world, and the only reason it's here is because you can't get to where the door should be right now.\n\n", false);
			MainScreen.text("Do you open it?", false);
			doYesNo(lumiLabChoices, camp.returnToCampUseOneHour);
		}
		else {
			//placeholder text for outside the cathedral
			MainScreen.text("You spot the door standing in the middle of nowhere again, and you guess that it will lead you back to Lumi's laboratory.  It swings open easily...", false);
	...
	public lumiShop(): void {
		...
		MainScreen.text("You ask Lumi if you can see her potions.  She smiles at you and pulls out several bottles from her desk and shows them to you.\n\n\"<i>Gawantied qwality, made by Lumi herself,</i>\" she says proudly.\n\n", false);
		MainScreen.text("Lust Draft - 15 gems\nGoblin Ale - 20 gems\nOviposition Elixir - 45 gems\n", false);
	...
	private lumiPitchGobboAle(): void {
		spriteSelect(37);
		MainScreen.clearText();
		MainScreen.text("You point at the flagon. \"<i>Oh? Oh thats Lumi's... actually no, dat tispsy stuff for 20 gems. You'll like if you want to be like Lumi. Do you like it?</i>\"\n\n", false);
		MainScreen.text("Will you buy the goblin ale?", false);
	...
	private lumiPurchase(itype: ItemType): void {
		...
		if (player.stats.gems >= cost) {
			MainScreen.text("You pay Lumi the gems, and she hands you " + itype.longName + " saying, \"<i>Here ya go!</i>\"\n\n", false);
			...
			MainScreen.text("You go to pay Lumi the gems, but then you realize that you don't have enough. Lumi seems to know what happened and tells you \"<i>Ok, is dere somefing you want to buy that you can affowd?</i>\"\n\n", false);
	...
	public lumiEnhance(justCheck: boolean = false): boolean {
		...
		if (player.inventory.items.has(consumables.FOXBERY))
			fox = lumiEnhanceFox;
		let laBova: Function = null;
		if (player.inventory.items.has(consumables.LABOVA_))
			laBova = lumiEnhanceLaBova;
		let succuDelight: Function = null;
		if (player.inventory.items.has(consumables.SDELITE))
			succuDelight = lumiEnhanceSDelight;
		let oviElix: Function = null;
		//if(player.inventory.items.has(consumables.OVIELIX))
		//	oviElix = lumiEnhanceOviElix;
		let lustDraft: Function = null;
		if (player.inventory.items.has(consumables.L_DRAFT))
			lustDraft = lumiEnhanceDraft;
		let seed: Function = null;
		if (player.inventory.items.has(consumables.GLDSEED))
			seed = lumiEnhanceGoldenSeed;
		let kanga: Function = null;
		if (player.inventory.items.has(consumables.KANGAFT))
			kanga = lumiEnhanceKanga;
		let kitsune: Function = null;
		if (player.inventory.items.has(consumables.FOXJEWL))
		...
		MainScreen.text("", true);
		MainScreen.text("\"<i>Do you have 100 gems for de enhancement?</i>\" asks Lumi.\n\n", false);
		//If (player has less than 100 gems)
		if (player.stats.gems < 100) {
			MainScreen.text("You shake your head no, and Lumi gives you a disappointed look and says, \"<i>Den Lumi can do no enhancement for you. Anyfing else?</i>\"\n\n", false);
			//Return to main Lumi menu
			doNext(lumiLabChoices);
			return false;
		}
		else {
			MainScreen.text("You nod and Lumi gives an excited yell, \"<i>Yay! Lumi loves to do enhancement, what you want to be bettar?</i>\"\n\n", false);
