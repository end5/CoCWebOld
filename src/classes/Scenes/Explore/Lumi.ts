package classes.Scenes.Explore{
	...
	if(flags[FlagEnum.LUMI_MET] == 0) {
		...
		Render.text("You spot an anomaly in the barren wastes; a door that seems to be standing out in the middle of nowhere. Somehow, you figure that it must lead to some other part of the world, and the only reason it's here is because you can't get to where the door should be right now.\n\n", false);
		Render.text("Do you open it?", false);
		doYesNo(lumiLabChoices,camp.returnToCampUseOneHour);
	}
	else {
		//placeholder text for outside the cathedral
		Render.text("You spot the door standing in the middle of nowhere again, and you guess that it will lead you back to Lumi's laboratory.  It swings open easily...", false);
...
public lumiShop():void {
	...
	clearOutput();
	Render.text("You ask Lumi if you can see her potions.  She smiles at you and pulls out several bottles from her desk and shows them to you.\n\n\"<i>Gawantied qwality, made by Lumi herself,</i>\" she says proudly.\n\n", false);
	Render.text("Lust Draft - 15 gems\nGoblin Ale - 20 gems\nOviposition Elixir - 45 gems\n", false);
...
private lumiLustDraftPitch():void {
	...
	Render.text("Will you buy the lust draft?", false);
...
private lumiPitchGobboAle():void {
	...
	Render.text("Will you buy the goblin ale?", false);
...
private lumiPitchOviElixer():void {
	...
	Render.text("Will you buy the Ovi Elixir?", false);
...
private lumiPurchase(itype:ItemType):void {
	...
	if(player.stats.gems >= cost) {
		Render.text("You pay Lumi the gems, and she hands you " + itype.longName + " saying, \"<i>Here ya go!</i>\"\n\n", false);
		...
		Render.text("You go to pay Lumi the gems, but then you realize that you don't have enough. Lumi seems to know what happened and tells you \"<i>Ok, is dere somefing you want to buy that you can affowd?</i>\"\n\n", false);
...
public lumiEnhance(justCheck:boolean = false):boolean {
	...
	Render.text("", true);
	Render.text("\"<i>Do you have 100 gems for de enhancement?</i>\" asks Lumi.\n\n", false); 
	//If (player has less than 100 gems)
	if(player.stats.gems < 100) {
		Render.text("You shake your head no, and Lumi gives you a disappointed look and says, \"<i>Den Lumi can do no enhancement for you. Anyfing else?</i>\"\n\n", false);
		//Return to main Lumi menu
		doNext(lumiLabChoices);
		return false;
	}
	else {
		Render.text("You nod and Lumi gives an excited yell, \"<i>Yay! Lumi loves to do enhancement, what you want to be bettar?</i>\"\n\n", false);
