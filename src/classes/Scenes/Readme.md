...
2. `enemyAI()` is called, which just contains call to...
    ...
    6. Description of `special`s was given before, supppose, `Monster.eAttack()` was called from `performCombatAction()`.
         By default:
    7. In case the monster represents a mob, `statusAffects.get("attacks").value1` is checked for number of attacks to perform.
         For each attack:
...
2. `CommonItem` contains handy methods `MainScreen.text`, `MainScreen.clearText` and a `game` property to use when performing item
   effects.
