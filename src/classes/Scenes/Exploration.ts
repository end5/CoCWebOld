...
export default class Exploration {
	...
	public doExplore(): void {
		...
		if (Flags.list[FlagEnum.TIMES_EXPLORED_PLAINS] > 0) MainScreen.addButton(5, "Plains", kGAMECLASS.plains.explorePlains);
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00272] > 0) MainScreen.addButton(6, "Swamp", kGAMECLASS.swamp.exploreSwamp);
	...
	private explorePageII(): void {
		...
		menu();
		if (Flags.list[FlagEnum.DISCOVERED_HIGH_MOUNTAIN] > 0) MainScreen.addButton(0, "High Mountain", kGAMECLASS.highMountains.exploreHighMountain);
		if (Flags.list[FlagEnum.BOG_EXPLORED] > 0) MainScreen.addButton(1, "Bog", kGAMECLASS.bog.exploreBog);
	...
	public tryDiscover(): void {
		...
		if (player.explored > 1) {
			...
			if (player.exploredLake >= 1 && rand(3) == 0 && player.exploredDesert == 0) {
				...
				if (player.lowerBody == LowerBodyType.HUMAN) MainScreen.text("inside your footwear, between your toes", false);
				if (player.lowerBody == LowerBodyType.HOOFED) MainScreen.text("in your hooves", false);
				if (player.lowerBody == LowerBodyType.DOG) MainScreen.text("in your paws", false);
				if (player.lowerBody == LowerBodyType.NAGA) MainScreen.text("in your scales", false);
			...
			if (player.exploredMountain >= 1 && rand(3) == 0 && Flags.list[FlagEnum.TIMES_EXPLORED_PLAINS] == 0) {
				Flags.list[FlagEnum.TIMES_EXPLORED_PLAINS] = 1;
				player.explored++;
			...
			//EXPLOOOOOOORE
			if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00272] == 0 && Flags.list[FlagEnum.TIMES_EXPLORED_PLAINS] > 0 && rand(3) == 0) {
				Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00272] = 1;
				player.explored++;
			...
			if (choosey == 2 && Flags.list[FlagEnum.FOUND_CATHEDRAL] == 1) {
				choosey = rand(5);
			...
			if (choosey == 0) {
				...
				player.explored++;
				if (Flags.list[FlagEnum.GAR_NAME] == 0) kGAMECLASS.gargoyle.gargoylesTheShowNowOnWBNetwork();
	...
	public bigJunkDesertScene(): void {
		...
		//PARAGRAPH 1
		MainScreen.text("Walking along the sandy dunes of the desert you find yourself increasingly impeded by the bulk of your " + CockDescriptor.describeCock(player, x) + " dragging along the sandscape behind you.  The incredibly hot surface of the desert causes your loins to sweat heavily and fills them with relentless heat.", false);

		if (player.lowerBody.cockSpot.count() == 1) MainScreen.text("  As it drags along the dunes, the sensation forces you to imagine the rough textured tongue of a monstrous animal sliding along the head of your " + CockDescriptor.nounCock(player.lowerBody.cockSpot.get(x).cockType) + ".", false);
		else if (player.lowerBody.cockSpot.count() >= 2) MainScreen.text("  With all of your " + CockDescriptor.describeMultiCockShort(player) + " dragging through the sands they begin feeling as if the rough textured tongues of " + num2Text(player.lowerBody.cockSpot.count()) + " different monstrous animals were slobbering over each one.", false);
		MainScreen.text("\n\n", false);
		...
		if (!player.lowerBody.isTaur()) {
			...
			if (player.lowerBody.vaginaSpot.count() >= 1) {
				...
				if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= VaginaWetness.DROOLING) MainScreen.text("  Juices stream from your womanhood and begin pooling on the hot sand beneath you.  Wisps of steam rise up into the air only to tease your genitals further.  ", false);
			}
		}
		//FOR CENTAURS
		else {
			MainScreen.text("The impending erection can't seem to be stopped.  Your sexual frustration forces stiffness into your " + CockDescriptor.describeMultiCockShort(player) + ", which forces the barrel of your horse-like torso to the ground.  Normally your erection would merely hover above the ground in between your centaurian legs, but your genitals have grown too large and heavy for your " + LowerBodyDescriptor.describeHips(player) + " to hold them aloft.  Instead, you feel your body being forcibly pulled down at your hindquarters until you rest atop your " + CockDescriptor.describeMultiCockShort(player) + ".", false);
			//IF CHARACTER HAS GIANT BREASTS ADD SENTENCE
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 35) MainScreen.text("  Your " + kGAMECLASS.BreastDescriptor.describeAllBreasts(player) + " pull your human torso forward until it also is forced to rest facedown, just like your horse half.  Your tits rest, pinned on the desert sand to either side of you.  Their immense weight anchors you, further preventing any part of your equine body from lifting itself up.  The burning heat of the desert teases your " + BreastDescriptor.describeNipple(0) + "s incessantly.", false);
			//IF CHARACTER HAS A BALLS ADD SENTENCE
			if (player.lowerBody.balls > 0) MainScreen.text("  Your " + player.skinTone + BallsDescriptor.describeSack(player) + " rests beneath your raised " + ButtDescriptor.describeButt(player) + ".  The airy warmth of the desert teases it, causing your " + BallsDescriptor.describeBalls(true, true, player) + " pulse with the need to release their sperm through your " + CockDescriptor.describeMultiCockShort(player) + ".", false);
			//IF CHARACTER HAS A VAGINA ADD SENTENCE
			if (player.lowerBody.vaginaSpot.count() >= 1) {
				MainScreen.text("  Your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " and " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " are thoroughly squashed between the bulky flesh where your male genitals protrude from between your hips and the " + ButtDescriptor.describeButt(player) + " above.", false);
				//IF CHARACTER HAS A DROOLING PUSSY ADD SENTENCE
				if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= VaginaWetness.DROOLING) MainScreen.text("  The desert sun beats down on your body, its fiery heat inflaming the senses of your vaginal lips.  Juices stream from your womanhood and begin pooling on the hot sand beneath you.", false);
			}
		}
		MainScreen.text("\n\n", false);
		//PARAGRAPH 3
		...
		else if (player.lowerBody.isTaur()) MainScreen.text("  You struggle and work your equine legs against the surface of the dune you are trapped on.  Your " + LowerBodyDescriptor.describeFeet(player) + " have consistent trouble finding footing, the soft sand failing to provide enough leverage to lift your bulk.  You breath in deeply and lean from side to side, trying to find some easier vertical leverage.  Eventually, with a crude crawl, your legs manage to push the bulk of your body onto more solid ground.  With great difficulty, you spend the next hour shuffling your genitals across the sandscape and back to camp.", false);
		//SCENE END = FOR ALL OTHER CHARACTERS
		else MainScreen.text("  You struggle and push with your " + LowerBodyDescriptor.describeLegs(player) + " as hard as you can, but it's no use.  You do the only thing you can and begin stroking your " + CockDescriptor.describeMultiCockShort(player) + " with as much vigor as you can muster.  Eventually your body tenses and a light load of jizz erupts from your body, but the orgasm is truly mild compared to what you need.  You're simply too weary from struggling to give yourself the masturbation you truly need, but you continue to try.  Nearly an hour later " + CockDescriptor.describeMultiCockSimpleOne(player) + " softens enough to allow you to stand again, and you make your way back to camp, still dragging your genitals across the warm sand.", false);
