		...
		public function tryDiscover():void
		{
			...
			if (player.explored > 1) {
				...
				if (player.exploredLake >= 1 && rand(3) == 0 && player.exploredDesert == 0) {
					...
					if (player.lowerBody == LOWER_BODY.HUMAN) outputText("inside your footwear, between your toes", false);
					if (player.lowerBody == LOWER_BODY.HOOFED) outputText("in your hooves", false);
					if (player.lowerBody == LOWER_BODY.DOG) outputText("in your paws", false);
					if (player.lowerBody == LOWER_BODY.NAGA) outputText("in your scales", false);
...
//AND THE TOTAL COMBINED WIDTH OF ALL THEIR COCKS IS TWELVE INCHES OR GREATER]
		public function bigJunkDesertScene():void
		{
			...
			if (player.lowerBody.cockSpot.count() == 1) outputText("  As it drags along the dunes, the sensation forces you to imagine the rough textured tongue of a monstrous animal sliding along the head of your " + Appearance.cockNoun(player.lowerBody.cockSpot.list[x].cockType) + ".", false);
			else if (player.lowerBody.cockSpot.count() >= 2) outputText("  With all of your " + multiCockDescriptLight() + " dragging through the sands they begin feeling as if the rough textured tongues of " + num2Text(player.lowerBody.cockSpot.count()) + " different monstrous animals were slobbering over each one.", false);
			outputText("\n\n", false);
			...
			if (!player.isTaur()) {
				...
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 35)  outputText("  Your " + kGAMECLASS.allBreastsDescript() + " hang lewdly off your torso to rest on the desert sands, seeming to bury the dunes on either side of you.  Their immense weight anchors your body, further preventing your torso from lifting itself up.  The burning heat of the desert teases your " + nippleDescript(0) + "s mercilessly as they grind in the sand.", false);
				//IF CHARACTER HAS A BALLS ADD SENTENCE
				if (player.lowerBody.balls > 0) outputText("  Your " + player.skinTone + sackDescript() + " rests beneath your raised " + buttDescript() + ".  The fiery warmth of the desert caresses it, causing your " + ballsDescriptLight() + " to pulse with the need to release their sperm through your " + multiCockDescriptLight() + ".", false);
				...
				if (player.lowerBody.vaginaSpot.count() >= 1) {
					...
					if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) outputText("  Juices stream from your womanhood and begin pooling on the hot sand beneath you.  Wisps of steam rise up into the air only to tease your genitals further.  ", false);
				}
			}
			//FOR CENTAURS
			else {
				...
				//IF CHARACTER HAS GIANT BREASTS ADD SENTENCE
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 35)  outputText("  Your " + kGAMECLASS.allBreastsDescript() + " pull your human torso forward until it also is forced to rest facedown, just like your horse half.  Your tits rest, pinned on the desert sand to either side of you.  Their immense weight anchors you, further preventing any part of your equine body from lifting itself up.  The burning heat of the desert teases your " + nippleDescript(0) + "s incessantly.", false);
				//IF CHARACTER HAS A BALLS ADD SENTENCE
				if (player.lowerBody.balls > 0) outputText("  Your " + player.skinTone + sackDescript() + " rests beneath your raised " + buttDescript() + ".  The airy warmth of the desert teases it, causing your " + ballsDescriptLight() + " pulse with the need to release their sperm through your " + multiCockDescriptLight() + ".", false);
				//IF CHARACTER HAS A VAGINA ADD SENTENCE
				if (player.lowerBody.vaginaSpot.count() >= 1) {
					outputText("  Your " + vaginaDescript() + " and " + clitDescript() + " are thoroughly squashed between the bulky flesh where your male genitals protrude from between your hips and the " + buttDescript() + " above.", false);
					//IF CHARACTER HAS A DROOLING PUSSY ADD SENTENCE
					if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) outputText("  The desert sun beats down on your body, its fiery heat inflaming the senses of your vaginal lips.  Juices stream from your womanhood and begin pooling on the hot sand beneath you.", false);
				}
			}
			outputText("\n\n", false);
			//PARAGRAPH 3
			...
			if (player.stats.cor < 33) outputText("  Panic slips into your heart as you realize that if any dangerous predator were to find you in this state, you'd be completely defenseless.  You must find a way to regain your mobility immediately!", false);
			else if (player.stats.cor < 66) outputText("  You realize that if any dangerous predator were to find you in this state you'd be completely defenseless.  You must find a way to regain your mobility... yet there is a certain appeal to imagining how pleasurable it would be for a sexual predator to take advantage of your obscene body.", false);
