	...
	public class Masturbation extends BaseContent {
		...
		public masturbateGo():void {
			...
			if (player.stats.cor < 15)
				...
				MainScreen.text("You happily remove your " + player.armorName + ", eager to pleasure yourself.\n\n");
			...
			if (player.lowerBody.cockSpot.count() > 0) {
				if (player.lowerBody.cockSpot.count() == 1) {
					MainScreen.text("You stroke your " + player.cockDescript());
			...
			if (player.lowerBody.vaginaSpot.count() > 0) {
				if (player.lowerBody.vaginaSpot.count() == 1) {
					...
					if (player.stats.lib < 45)
						MainScreen.text("You touch and play with your " + player.vaginaDescript() + ", ");
					else if (player.stats.lib < 70)
						MainScreen.text("You slap your pussy softly, ")
					else MainScreen.text("You touch your enflamed and aroused " + player.vaginaDescript() + ", ");
					switch (player.vaginas[0].vaginalWetness) {
						...
						case VAGINA_WETNESS.DROOLING:
							MainScreen.text("slicking your fingers in the juices that constantly dribble from " + player.vaginaDescript() + "  "); break;
						default:
				...
				if (player.lowerBody.vaginaSpot.count() > 1) {
					...
					else MainScreen.text("Touch your enflamed and aroused " + player.vaginaDescript() + "s, ");
					switch (player.vaginas[0].vaginalWetness) {
						case VAGINA_WETNESS.DRY:
							MainScreen.text("expertly arousing your female parts.  "); break;
						case VAGINA_WETNESS.NORMAL:
						...
						case VAGINA_WETNESS.SLICK:
							MainScreen.text("smiling as your fingers become coated in your slick fluids.  "); break;
						case VAGINA_WETNESS.DROOLING:
							MainScreen.text("slicking your fingers in the juices that constantly dribble from " + player.vaginaDescript() + "s  "); break;
						default:
			...
			if (player.lowerBody.cockSpot.count() == 1) {
				...
				if (player.lowerBody.cockSpot.list[0].cockThickness < 1.8)
					MainScreen.text("You easily wrap a hand around your " + player.cockDescript() + " and start masturbating.  ");
				else if (player.lowerBody.cockSpot.list[0].cockThickness < 3) {
					MainScreen.text("You have some difficulty fitting your hand around your " + player.cockDescript() + ", relishing the feelings of your ");
					...
					MainScreen.text("You use both hands to grip your " + player.cockDescript() + ", feeling the throbbing of your " + (player.hasKnot() ? "knot" : "member") + " as you begin to masturbate.  ");
				}
				else MainScreen.text("You grasp onto your " + player.cockDescript() + " with both hands, but fail to encircle it fully as you begin to masturbate.  ");
				//If length > 12 proud
				...
				if (player.lowerBody.cockSpot.list[0].cockLength < 12) {
					if (player.normalCocks() >= 1)
						...
						MainScreen.text("You stroke quickly, feeling the tiny 'barbs' of your " + player.cockDescript() + " sliding through your fingers, even darting down to circle the sensitive skin around your sheath.  ");
					else if (player.lizardCocks() >= 1)
						MainScreen.text("You stroke quickly, pleasuring your sensitive " + player.cockDescript() + ", sliding fingers over each ridge and bump that covers its knotty length.  ");
					else if (player.anemoneCocks() >= 1)
						MainScreen.text("You stroke quickly, gasping as your fingers are stung repeatedly with the aphrodisiac-laced tentacles around the base of your " + player.cockDescript() + " and under its crown.  ");
					...
					if (player.normalCocks() >= 1)
						MainScreen.text("You delight in teasing the crown of your " + player.cockDescript() + ", rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  It seems to pulse and twitch with each stroke, responding to every touch.  ");
					else if (player.horseCocks() >= 1)
						MainScreen.text("You delight in teasing the sensitive flared tip of your " + player.cockDescript() + ", rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  It seems to pulse and ripple with each stroke, responding to every touch.  ");
					else if (player.dogCocks() >= 1)
						MainScreen.text("You delight in teasing the pointed tip of your " + player.cockDescript() + ", rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  Your knot seems to pulse and twitch with each stroke, reacting to every touch.  ");
					else if (player.tentacleCocks() >= 1)
						...
						MainScreen.text("You delight in teasing the larger bumps that form a ring around the crown of your " + player.cockDescript() + ", watching as they twitch and spasm in time with the dollops of pre you're squeezing out and smearing over the entire length.  ");
					else if (player.catCocks() >= 1)
						MainScreen.text("You delight in teasing the sensitive nubs all along your " + player.cockDescript() + ", circling the cock-tip at the end of each stroke to gather pre and slather it over your entire length.  Each of the tiny 'barbs' provides bursts of pleasure with each stroke, driving you on.  ");
					else if (player.lizardCocks() >= 1)
						MainScreen.text("You delight in teasing the rounded bulbs that cover your " + player.cockDescript() + ", circling them with your fingertips before sliding up to the urethra and gathering a drop of pre.  You smear it over your sensitive reptile skin and revel in the pleasure radiating through you.  ");
					else if (player.anemoneCocks() >= 1)
						...
						MainScreen.text("You delight in teasing the opened tip of your " + player.cockDescript() + ", rubbing it at the end of each stroke, watching it squeeze out dollops of pre that you smear over it to tease yourself with.  Your knot seems to pulse and twitch with each stroke, reacting to every touch.  ");
					else MainScreen.text("You delight in teasing the crown of your " + player.cockDescript() + ", rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  It seems to pulse and twitch with each stroke, responding to every touch.  ");
				}
				else if (player.lowerBody.cockSpot.list[0].cockLength < 26) {			
					if (player.normalCocks() >= 1)
						MainScreen.text("The head of your " + player.cockDescript() + " wobbles towards your face as you masturbate, a dollop of pre slowly growing atop it.  ");
					else if (player.horseCocks() >= 1)
						MainScreen.text("The flared tip of your " + player.cockDescript() + " wobbles towards your face as you masturbate, a dollop of pre slowly growing atop it.  ");
					else if (player.dogCocks() >= 1)
						MainScreen.text("The pointed tip of your " + player.cockDescript() + " angles towards your face as you masturbate, a dollop of pre slowly leaking from it.  ");
					else if (player.tentacleCocks() >= 1)
						MainScreen.text("The overly wide head of your " + player.cockDescript() + " bumps against your lips as you masturbate, waving back and forth like a snake as it searches for an orifice.  ");
					else if (player.demonCocks() >= 1)
						MainScreen.text("The purplish head of your " + player.cockDescript() + " bumps against your lips as you masturbate, flushing darkly with every beat of your heart.  ");
					else if (player.catCocks() >= 1)
						MainScreen.text("The slightly pointed tip of your " + player.cockDescript() + " bumps against your lips as you masturbate, flushing with blood as your spines grow thicker in your hand.  ");
					else if (player.lizardCocks() >= 1)
						MainScreen.text("The pointed, purple tip of your " + player.cockDescript() + " bumps against your lips while its knotted surface flushes near-purple and seems to grow thicker.  ");
					else if (player.anemoneCocks() >= 1)
						MainScreen.text("The tentacle-ringed tip of your " + player.cockDescript() + " brushes against your lips, making them tingle with artificial heat while you stroke it.  ");
					else if (player.displacerCocks() >= 1)
						MainScreen.text("The blooming tip of your " + player.cockDescript() + " angles towards your face as you masturbate, a dollop of pre slowly leaking out of the outstretched top.  ");
					else MainScreen.text("The head of your " + player.cockDescript() + " wobbles towards your face as you masturbate, a dollop of pre slowly growing atop it.  ");
					...
					if (player.upperBody.chest.hasFuckableNipples() && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) {
						...
						MainScreen.text("You give in to temptation and swallow the tip, slurping greedily as you milk your " + player.cockDescript() + " of its pre-cum.  ");
						autofellatio = true;
					}
					if (player.canTitFuck() && player.upperBody.chest.BreastRatingLargest[0].breastRating > 3 && !nippleFuck) {
						MainScreen.text("Your hands migrate to your breasts of their own accord, wrapping your titflesh around your " + player.cockDescript() + ", jacking it up and down in your pillowy tits.  ");
						if (player.biggestLactation() > 0) MainScreen.text("Jets of milk squirt out of your nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ");
					}
				}
				else {
					if (player.normalCocks() >= 1)
						MainScreen.text("The head of your " + player.cockDescript() + " wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with your cock.  ");
					else if (player.horseCocks() >= 1)
						MainScreen.text("The flared tip of your " + player.cockDescript() + " wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with horsemeat.  ");
					else if (player.dogCocks() >= 1)
						MainScreen.text("The head of your " + player.cockDescript() + " wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with your " + player.cockDescript() + ".  ");
					else if (player.tentacleCocks() >= 1)
						MainScreen.text("The bulbous, mushroom-like head of your " + player.cockDescript() + " pushes against your face eagerly, smearing pre-cum over your lips as it seeks entrance to the nearest orifice.  You're unable to resist opening your mouth to suck it down, filling your mouth with slick rubbery cock-tentacle.  ");
					else if (player.demonCocks() >= 1)
						MainScreen.text("The tainted swollen head of your " + player.cockDescript() + " pushes against your face, smearing sweet pre-cum over your lips.  You're unable to resist opening wide and taking in the demonic member, submitting wholly to the desire to pleasure your corrupted body-parts.  ");
					else if (player.catCocks() >= 1)
						MainScreen.text("The pointed tip of your " + player.cockDescript() + " pushes against your face, smearing pre-cum over your lips and tickling you with the many barbs.  You're unable to resist opening wide and taking in the " + player.cockDescript() + ", half-humming half-purring in contentment.  ");
					else if (player.lizardCocks() >= 1)
						MainScreen.text("The slightly-pointed tip of your " + player.cockDescript() + " pushes against your face, smearing pre-cum over your eager lips.   You can't resist opening wide and slipping it inside as your hands caress the reptilian bulges along your length.  ");
					else if (player.anemoneCocks() >= 1)
						...
						MainScreen.text("The head of your " + player.cockDescript() + " wobbles over, bumping your face and smearing your lips with its copious pre-cum.  Wiggling against your lips, the various protrusions of the 'star' at the tip smear you with your heady emissions.   You are unable to resist opening your mouth and sucking it down, filling your mouth with your " + player.cockDescript() + ".  ");
					else MainScreen.text("The head of your " + player.cockDescript() + " wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with your cock.  ");
					//try to stick it in a titty!
					if (player.upperBody.chest.hasFuckableNipples() && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) {
						//In-between text
						//UNFINISHED
						...
						if (player.stats.cor > 60) {
							if (player.normalCocks() >= 1)
								...
								MainScreen.text("The sweet scent of your " + player.cockDescript() + " fills your nostrils as inch after inch of tentacle forces its way down your throat.  You struggle briefly with it, wrestling to keep it from pushing the whole way into your gut, swooning with the pleasure that fighting with your own cock induces.  ");
							else if (player.demonCocks() >= 1)
								MainScreen.text("The spicy demonic odor your " + player.cockDescript() + " radiates fills your nostrils as you force inch after inch of tainted meat down your throat.  You struggle briefly, but your " + player.cockDescript() + " quickly overwhelms your resistance and your gag reflex momentarily seems to disappear.  ");
							else if (player.catCocks() >= 1)
								...
								MainScreen.text("The salty, dry odor of your " + player.cockDescript() + " fills your nostrils as you force inch after inch into your mouth, swallowing as much of the bulgy shaft as you can.  ");
							else if (player.anemoneCocks() >= 1)
								MainScreen.text("The stinging, aphrodisiac-laced " + player.cockDescript() + " slowly works its way down your throat as you swallow it deeper and deeper, gurgling happily as your throat grows more sensitive.  ");
							else if (player.displacerCocks() >= 1)
								MainScreen.text("The strong animal scent of your cock fills your nostrils as you force inch after inch into your mouth, deepthroating as much of yourself as you can.  ");
					...
					if (player.canTitFuck() && player.upperBody.chest.BreastRatingLargest[0].breastRating > 3 && !nippleFuck) {
						MainScreen.text("Your hands reach up to your pillowy breasts, wrapping them around the shaft of your " + player.cockDescript() + ", causing you to let out muffled moans of excitment. ");
						if (player.biggestLactation() > 0) MainScreen.text("Jets of milk squirt out of your nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ");
					}
				}
			}
				...
				MainScreen.text("You grasp one of your " + player.multiCockDescriptLight() + " in each hand, ");
				...
				//Tittyfuck if D cup+
					...
					if (player.canTitFuck() && player.mostBreastsPerRow() > 2 && !nippleFuck) {
						...
						if (player.biggestLactation() > 0) MainScreen.text("Jets of milk squirt out of your many nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ");
					}
				...
				//Deepthroat yourself, maybe titfuck if equipped.
					...
					//autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs
					MainScreen.text("Your " + player.multiCockDescriptLight() + " brush against your face, throbbing with need.   You open your mouth and suck one of your pricks into your mouth, taking as much of it as possible, running your tongue along the underside.  You smile and pop it free, then take a different shaft into your maw.  ");
					//Try to stick it in a titty!
					if (player.upperBody.chest.hasFuckableNipples()) {
						//check for dogcocks
						if (player.dogCocks() > 0) {
				...
				if (player.averageCockLength() >= 20) {
					//autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs
					MainScreen.text("Your " + player.multiCockDescriptLight() + " brush against your face, throbbing with need.   You open your mouth and suck one of your pricks into your mouth, taking as much of it in as possible, running your tongue along the underside.  You smile and pop it free, then take a different shaft into your maw.  ");
					//Try to stick it in a titty!
					if (player.upperBody.chest.hasFuckableNipples()) {
						//check for dogcocks
						if (player.dogCocks() > 0) {
			...
			if (player.lowerBody.vaginaSpot.count() > 0) {
				...
				if (player.lowerBody.vaginaSpot.count() == 1) {
					...
					//Big clits
						...
						if (player.vaginas[0].vaginalWetness > VAGINA_WETNESS.DRY) MainScreen.text("glistening ");
					...
					switch (player.vaginas[0].vaginalWetness) {
						...
						case VAGINA_WETNESS.DROOLING:
							MainScreen.text("Warm wetness runs down your legs in thick streams, pouring from your " + player.vaginaDescript() + ".  You smile sheepishly and stroke up your now slickened legs to your pussy-lips, parting them and letting your fingers dive inside the wet channel.  "); break;
						default:
			...
			if (player.lowerBody.cockSpot.count() > 0) {
				...
				if (player.lowerBody.cockSpot.count() == 1) {
					if (player.stats.lib < 30)
						...
						MainScreen.text("You roll and begin to doze, your semi-erect " + player.cockDescript() + " flopping against you. ");
					else if (player.stats.lib <= 80) {
						MainScreen.text("As you close your eyes and relax, your " + player.cockDescript() + " surges back to erectness, ensuring ");
					...
					else MainScreen.text("You groan and drift to sleep, your rigid " + player.cockDescript() + " pulsing and throbbing with continual lust.");
				}
				//Multi Cock
				else {
					if (player.stats.lib < 30)
		...
		private genderlessMasturbate():void {
			...
			MainScreen.text("Your " + assholeDescript() + " begins to twitch. It's practically crying out for attention.\n\n");
			MainScreen.text("You lay down on your side and reach gingerly behind yourself.  The palm of your hand comes to rest on your " + player.buttDescript() + ".  You slide your finger into your crack and find your " + assholeDescript() + ".  You run your finger slowly around the sensitive ring of your hole and feel a tingle emanating from it. A smile creeps across your lips as you begin to imagine what is about to happen.\n\n");
			...
			if (player.ass.analWetness < 2) {
				...
				MainScreen.text(" in a generous helping of saliva and head back for your " + assholeDescript() + ".\n\n");
			}
			//If BioLube or continuing from no biolube - 
				...
				if (player.ass.analLooseness >= 3) MainScreen.text("s"); 
		...
		private titForeplay():void {
			...
			if (player.upperBody.chest.count() == 1 && player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) {
				if (player.stats.lib < 45)
					MainScreen.text("You caress your " + player.breastDescript(0) + " gently with your fingers");
				else if (player.stats.lib < 70)
					MainScreen.text("You grope your " + player.breastDescript(0) + " agressively with both hands");
				else MainScreen.text("You squeeze your " + player.breastDescript(0) + " brutally with both hands");
			}
				...
				if (player.stats.cor <= 25)
					MainScreen.text("You blink back tears of shame as you experience the tactile proof of your abnormal endowments, unable to resist mashing your " + player.allBreastsDescript() + " together");
				else if (player.stats.cor < 75)
					MainScreen.text("Though a small part of you is horrified by your abnormal endowments, it doesn't stop you from mashing your " + player.allBreastsDescript() + " together");
			...
			if (player.upperBody.chest.hasFuckableNipples() && player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) {
				...
				if (player.averageVaginalWetness() < 2)
					...
					MainScreen.text("you begin to slowly spread the slippery secretions leaking from your engorged nipples all over your " + player.breastDescript(0) + ". ");
				else {
					MainScreen.text("little spurts of ");
					//Lactating?
					if (player.averageLactation() > 0) MainScreen.text(" milky ");
				...
				if (player.averageLactation() > 0 && player.averageLactation() < 2)
					MainScreen.text("Your freakish mammaries drip a sticky combination of cunt-juice and milk, spattering your " + player.legs() + " and crotch.");
				else if (player.averageLactation() < 3)
					MainScreen.text("Your freakish mammaries leak thin streams of sticky girlcum mixed with milk all over your " + player.legs() + " and crotch.");
				else MainScreen.text("Your freakish mammaries constantly drool a steady stream of milky cunt-juice, drenching your " + player.legs() + " and crotch in milky goo.");
			}
			//If nipples aren't fuckable.
				...
				if (player.biggestLactation() > 1 && player.biggestLactation() < 2)
					MainScreen.text("Droplets of milk dribble from each nipple, spattering milk onto your " + player.legs() + " and crotch.  ");
				else if (player.biggestLactation() < 3)
					MainScreen.text("Thin squirts of milk spray from each nipple, spattering milk onto your " + player.legs() + " and crotch.  ");
				else if(player.biggestLactation() >= 3) MainScreen.text("A constant stream of milk drizzles from each teat, soaking your " + player.legs() + " and crotch.  ");
			}
		}
		...
		private titFuckSingle():void
		{
			...
			if(player.stats.lib < 45) MainScreen.text("You grip your " + cockDescript(0) + " and begin cautiously guiding it towards ", false);
			if(player.stats.lib >= 45 && player.stats.lib < 70) MainScreen.text("Shivering with anticipation, you place the tip of your " + cockDescript(0) + " against the opening of ", false);
			if(player.stats.lib >= 70) MainScreen.text("Without hesitation, you shove the tip of your " + cockDescript(0) + " into ", false);
			//More than one row?
			...
			if(player.averageNipplesPerBreast() > 1) MainScreen.text("many ", false);
			...
			if (player.stats.lib < 45)
				MainScreen.text("You grip your " + player.cockDescript() + " and begin cautiously guiding it towards ");
			else if (player.stats.lib < 70) {
				...
				MainScreen.text("tip of your " + player.cockDescript() + " against the opening of ");
		/* Old method
				if(player.horseCocks() > 0) MainScreen.text("Shivering with anticipation, you place the flared tip of your " + horseDescript(0) + " against the opening of ", false);
				else if(player.normalCocks() > 0) MainScreen.text("Shivering with anticipation, you place the tip of your " + cockDescript(0) + " against the opening of ", false);
				else if(player.dogCocks() > 0) MainScreen.text("Shivering with anticipation, you place the pointed tip of your " + dogDescript(0) + " against the opening of ", false);
				else if(player.tentacleCocks() > 0) MainScreen.text("Shivering with anticipation, you place the bulbous tip of your " + cockDescript(0) + " against the opening of ", false);
				else if(player.demonCocks() > 0) MainScreen.text("Shivering with anticipation, you place the bump-encircled tip of your " + cockDescript(0) + " against the opening of ", false);
				else if(player.catCocks() > 0) MainScreen.text("Shivering with anticipation, you place the tip of your " + cockDescript(0) + " against the opening of ", false);
				else if(player.anemoneCocks() > 0 || player.displacerCocks() > 0) MainScreen.text("Shivering with anticipation, you place the wriggling tip of your " + cockDescript(0) + " against the opening of ", false);
				else MainScreen.text("Shivering with anticipation, you place the tip of your " + cockDescript(0) + " against the opening of ", false);
		*/
			}
			else {
				...
				MainScreen.text("your " + player.cockDescript() + " into ");
		/* Old method
				if(player.normalCocks() > 0) MainScreen.text("Without hesitation, you shove the tip of your " + cockDescript(0) + " into ", false);
				else if(player.horseCocks() > 0) MainScreen.text("Without hesitation, you shove the engorged flare of your " + cockDescript(0) + " into ", false);
				else if(player.dogCocks() > 0) MainScreen.text("Without hesitation, you shove the pointed tip of your " + dogDescript(0) + " into ", false);
				else if(player.tentacleCocks() > 0) MainScreen.text("Without hesitation, you shove the over-sized mushroom that is your " + cockDescript(0) + " into ", false);
				else if(player.demonCocks() > 0) MainScreen.text("Without hesitation, you shove the bump-ringed monstrosity that is your " + cockDescript(0) + " into ", false);
				else if(player.anemoneCocks() > 0) MainScreen.text("Without hesitation, you shove the tentacle-laden mushroom that is your " + cockDescript(0) + " into ", false);
				else if(player.displacerCocks() > 0) MainScreen.text("Without hesitation, you shove the opened top of your " + cockDescript(x) + " into ");
				else MainScreen.text("Without hesitation, you shove the " + cockHead(0) + " of your " + cockDescript(0) + " into ", false);
		*/
			}
			//More than one row?
			...
			//More than 1 nip per boob?
			if (player.averageNipplesPerBreast() > 1) MainScreen.text("many ");
			//Different based on looseness (again)
			if (player.averageVaginalLooseness() < 2)
			...
			if (player.averageLactation() == 0) {
				if (player.averageVaginalWetness() < 2)
					...
					MainScreen.text("your " + player.cockDescript() + " and y");
		/* Old method
					if(player.normalCocks() > 0) MainScreen.text("Slick juices dribble down the skin of your " + cockDescript(0) + " and y", false);
					else if(player.horseCocks() > 0) MainScreen.text("Slick juices dribble down the mottled skin of your " + horseDescript(0) + " and y", false);
					else if(player.dogCocks() > 0) MainScreen.text("Slick juices dribble down the red, shiny skin of your " + dogDescript(0) + " and y", false);
					else if(player.tentacleCocks() > 0) MainScreen.text("Slick juices dribble down the rubbery skin of your " + cockDescript(0) + " and y", false);
					else if(player.demonCocks() > 0) MainScreen.text("Slick juices dribble down the shiny purplish skin and nodules of your " + cockDescript(0) + " and y", false);
					else if(player.lizardCocks() > 0) MainScreen.text("Slick juices dribble down the bumpy purple skin of your " + cockDescript(0) + " and y", false);
					else if(player.catCocks() > 0) MainScreen.text("Slick juices dribble over the pink, spiny protrusions that cover your " + cockDescript(0) + " and y", false);
					else if(player.anemoneCocks() > 0) MainScreen.text("Slick juices dribble over the nearly transparent skin of your " + cockDescript(0) + " and y", false);
					else if(player.displacerCocks() > 0) MainScreen.text("Slick juices dribble over the dusky purple of your " + cockDescript(0) + " and y", false);
					else MainScreen.text("Slick juices dribble down the skin of your " + cockDescript(0) + " and y", false);
		*/
				}
				else MainScreen.text("Slick juices squirt out from around your " + player.cockDescript() + " and y");
			}
			else {
				if (player.averageLactation() < 2) {
					...
					MainScreen.text("your " + player.cockDescript() + " and y");
		/* Old method
					if(player.normalCocks() > 0) MainScreen.text("Rivulets of milky girlcum drizzle down the skin of your " + cockDescript(0) + " and y", false);
					else if(player.horseCocks() > 0) MainScreen.text("Rivulets of milky girlcum drizzle down the mottled skin of your " + horseDescript(0) + " and y", false);
					else if(player.dogCocks() > 0) MainScreen.text("Rivulets of milky girlcum drizzle down the red, shiny skin of your " + dogDescript(0) + " and y", false);
					else if(player.tentacleCocks() > 0) MainScreen.text("Rivulets of milky girlcum drizzle down the rubbery skin of your " + cockDescript(0) + " and y", false);
					else if(player.demonCocks() > 0) MainScreen.text("Rivulets of milky girlcum drizzle down the shiny purplish skin and nodules of your " + cockDescript(0) + " and y", false);
					else if(player.catCocks() > 0) MainScreen.text("Rivulets of milky girlcum drizzle down the spiny, pink flesh of your " + cockDescript(0) + " and y", false);
					else if(player.lizardCocks() > 0 || player.displacerCocks()) MainScreen.text("Rivulets of milky girlcum drizzle over the purplish, knotted flesh of your " + cockDescript(0) + " and y", false);
					else if(player.anemoneCocks() > 0) MainScreen.text("Rivulets of milky girlcum drizzle and drip from the many tiny tentacles of your " + cockDescript(0) + " and y", false);
					else MainScreen.text("Rivulets of milky girlcum drizzle over the sensitive skin of your " + cockDescript(0) + " and y", false);
		*/
				}
				else {
					MainScreen.text("Milky girlcum squirts out from around your " + player.cockDescript() + ", staining ");
					switch (player.lowerBody.cockSpot.list[0].cockType) { //Different way of choosing cock to use, results in consistent description of cock[0]
						case CockType.ANEMONE:
		...
		/* Old method
					if(player.normalCocks() > 0) MainScreen.text("Milky girlcum squirts out from around your " + cockDescript(0) + ", staining it white.  Y", false);
					else if(player.horseCocks() > 0) MainScreen.text("Milky girlcum squirts out from around your " + horseDescript(0) + ", staining its mottled skin white.  Y", false);
					else if(player.dogCocks() > 0) MainScreen.text("Milky girlcum squirts out from around your " + dogDescript(0) + ", staining its shiny skin white.  Y", false);
					else if(player.tentacleCocks() > 0) MainScreen.text("Milky girlcum squirts out from around your " + cockDescript(0) + ", staining its rubbery skin white.  Y", false);
					else if(player.demonCocks() > 0) MainScreen.text("Milky girlcum squirts out from around your " + cockDescript(0) + ", staining its purplish-skin white.  Y", false);
					else if(player.catCocks() > 0) MainScreen.text("Milky girlcum squirts out from around your " + cockDescript(0) + ", staining the pink kitty-skin white.  Y", false);
					else if(player.lizardCocks() > 0 || player.displacerCocks() > 0) MainScreen.text("Milky girlcum squirts out from around your " + cockDescript(0) + ", staining the purple, knotty flesh white.  Y", false);
					else if(player.anemoneCocks() > 0) MainScreen.text("Milky girlcum squirts out from around your " + cockDescript(0) + ", staining the odd aquatic shaft wide.  Y", false);
					else MainScreen.text("Milky girlcum squirts out from around your " + cockDescript(0) + ", staining its length white.  Y", false);
		*/
				}
			}
			...
			if (Math.round(player.cockArea(0)) >= player.vaginalCapacity()) {
				...
				if (player.stats.sens < 45)
					MainScreen.text("ou grunt with exertion as you attempt to stuff your " + player.cockDescript() + " into ");
				else if (player.stats.sens < 70)
					MainScreen.text("ou blink back tears as you attempt to stuff your "  + player.cockDescript() + " into ");
				else MainScreen.text("ou scream with a combination of pain and pleasure as you attempt to stuff your "  + player.cockDescript() + " into ");
				...
				if (Math.round(player.cockArea(0)) == player.vaginalCapacity()) {
					if (player.averageVaginalLooseness() < 2)
						MainScreen.text("Your poor, tortured nipple is barely up to the task of accepting the " + player.cockDescript() + ", but accept it it does. ");
					else if (player.averageVaginalLooseness() < 4)
						MainScreen.text("The engorged opening at the end of your swollen nipple is stretched to its limit as you shove your " + player.cockDescript() + " home. ");
					else {
						switch (player.lowerBody.cockSpot.list[0].cockType) { //Different way of choosing cock to use, results in consistent description of cock[0]
						case CockType.ANEMONE:
							MainScreen.text("The swollen tips of your bloated nipple wrap around the stinging tentacles that surround your " + player.cockDescript() + "'s tip, convulsing with wet squishing sounds as they become red and enflamed with artificial lust.  ");
							break;
						case CockType.CAT:
							MainScreen.text("The swollen tips of your bloated nipples stretch around the barbed tip of your " + player.cockDescript() + ", swallowing it like an enormous mouth.  ");
							break;
						case CockType.DEMON:
							MainScreen.text("The swollen lips of your bloated nipple stretch around the nodule-ringed tip of your " + player.cockDescript() + ", swallowing it like an enormous mouth.  ");
							break;
						case CockType.DISPLACER:
							MainScreen.text("The swollen tips of your bloated nipple wrap around the outstretched head of your " + player.cockDescript() + ", convulsing with wet, squishing sounds as it wriggles inside you.  ");
							break;
						case CockType.DOG:
						case CockType.FOX:
							MainScreen.text("The swollen lips of your bloated nipple stretch around the pointed tip of your " + player.cockDescript() + " swallowing it like an enormous mouth.  ");
							break;
						case CockType.HORSE:
							MainScreen.text("The swollen lips of your bloated nipple stretch around the flared tip of your " + player.cockDescript() + " swallowing it like an enormous mouth.  ");
							break;
						case CockType.HUMAN:
							MainScreen.text("The swollen lips of your bloated nipple stretch around the tip of your " + player.cockDescript() + " swallowing it like an enormous mouth.  ");
							break;
						case CockType.LIZARD:
							MainScreen.text("The swollen tips of your bloated nipple wrap around the pointed tip of your " + player.cockDescript() + ", stretching oddly as it swallows the knot-covered appendage.  ");
							break;
						case CockType.TENTACLE:
							MainScreen.text("The swollen lips of your bloated nipple stretch around the rounded tip of your " + player.cockDescript() + ", swallowing it like an enormous mouth.  ");
							break;
							default:
								MainScreen.text("tips of your bloated nipples wrap around the " + player.cockHead() + " of your " + player.cockDescript() + ", swallowing it like an enormous mouth.  ");
						}
		/* Old method
						if(player.normalCocks() > 0) MainScreen.text("The swollen lips of your bloated nipple stretch around the tip of your " + cockDescript(0) + " swallowing it like an enormous mouth. ", false);
						else if(player.horseCocks() > 0) MainScreen.text("The swollen lips of your bloated nipple stretch around the flared tip of your " + horseDescript(0) + " swallowing it like an enormous mouth. ", false);
						else if(player.dogCocks() > 0) MainScreen.text("The swollen lips of your bloated nipple stretch around the pointed tip of your " + dogDescript(0) + " swallowing it like an enormous mouth. ", false);
						else if(player.tentacleCocks() > 0) MainScreen.text("The swollen lips of your bloated nipple stretch around the rounded tip of your " + cockDescript(0) + ", swallowing it like an enormous mouth. ", false);
						else if(player.demonCocks() > 0) MainScreen.text("The swollen lips of your bloated nipple stretch around the nodule-ringed tip of your " + cockDescript(0) + ", swallowing it like an enormous mouth. ", false);
						else if(player.catCocks() > 0) MainScreen.text("The swollen tips of your bloated nipples stretch around the barbed tip of your " + cockDescript(0) + ", swallowing it like an enormous mouth. ", false);
						else if(player.lizardCocks() > 0) MainScreen.text("The swollen tips of your bloated nipple wrap around the pointed tip of your " + cockDescript(0) + ", stretching oddly as it swallows the knot-covered appendage. ", false);
						else if(player.anemoneCocks() > 0) MainScreen.text("The swollen tips of your bloated nipple wrap around the stinging tentacles that surround your " + cockDescript(0) + "'s tip, convulsing with wet squishing sounds as they become red and enflamed with artificial lust.  ", false);
						else if(player.displacerCocks() > 0) MainScreen.text("The swollen tips of your bloated nipple wrap around the outstretched head of your " + cockDescript(0) + ", convulsing with wet, squishing sounds as it wriggles inside you.");
						else MainScreen.text("The swollen tips of your bloated nipples wrap around the " + cockHead(0) + " of your " + cockDescript(0) + ", swallowing it like an enormous mouth. ", false);
		*/
					}
					MainScreen.text("With each thrust, you bury your " + player.cockDescript() + " deeper into your greedy tit. Overwhelmed by the combined sensations, your mind is barely able to cope with the intense feeling of fullness where no such feeling should be possible.");
				}
				//if it doesnt really fit
				if (Math.round(player.cockArea(0)) > player.vaginalCapacity()) {
					if (player.averageVaginalLooseness() < 2)
						MainScreen.text("Your poor, tortured nipple is woefully insufficient compared to your " + player.cockDescript() + ", delerious with arousal, you keep trying anyway. ");
					else if (player.averageVaginalLooseness() < 4)
						MainScreen.text("The engorged opening at the end of your swollen nipple is stretched to its limits and beyond as you shove your " + player.cockDescript() + " home. ");
					else {
						switch (player.lowerBody.cockSpot.list[0].cockType) { //Different way of choosing cock to use, results in consistent description of cock[0]
						case CockType.ANEMONE:
							MainScreen.text("The swollen lips of your bloated nipple gape wide, but the stinging tip of your " + player.cockDescript() + " spreads them wider and fills them with artificial lust.  ");
							break;
						case CockType.CAT:
							MainScreen.text("The swollen lips of your bloated nipple gape wide, but the barbed tip of your " + player.cockDescript() + " spreads them even wider.  ");
							break;
						case CockType.DEMON:
							MainScreen.text("The swollen lips of your bloated nipple gape wide, but the tip of your " + player.cockDescript() + " spreads them even wider.  ");
							break;
						case CockType.DISPLACER:
							MainScreen.text("The swollen lips of your bloated nipple gape wide, but the wide head of your " + player.cockDescript() + " spreads them even wider.  ");
							break;
						case CockType.DOG:
						case CockType.FOX:
							MainScreen.text("The swollen lips of your bloated nipple gape wide, but the massive girth of your " + player.cockDescript() + " spreads them even wider.  ");
							break;
						case CockType.HORSE:
							MainScreen.text("The swollen lips of your bloated nipple gape wide, but the flared tip of your " + player.cockDescript() + " spreads them even wider.  ");
							break;
						case CockType.HUMAN:
							MainScreen.text("The swollen lips of your bloated nipple gape wide, but the tip of your " + player.cockDescript() + " spreads them even wider.  ");
							break;
						case CockType.LIZARD:
							MainScreen.text("The swollen lips of your bloated nipple gape wide, but the pointed tip of your " + player.cockDescript() + " slowly spreads them even wider.  ");
							break;
						case CockType.TENTACLE:
							MainScreen.text("The swollen lips of your bloated nipple gape wide, but the tip of your " + player.cockDescript() + " spreads them even wider.  ");
							break;
							default:
								MainScreen.text("The swollen lips of your bloated nipple gape wide, but the " + player.cockHead() + " of your " + player.cockDescript() + " spreads them even wider.  ");
						}
		/* Old method
						if(player.normalCocks() > 0) MainScreen.text("The swollen lips of your bloated nipple gape wide, but the tip of your " + cockDescript(0) + " spreads them even wider. ", false);
						else if(player.horseCocks() > 0) MainScreen.text("The swollen lips of your bloated nipple gape wide, but the flared tip of your " + horseDescript(0) + " spreads them even wider. ", false);
						else if(player.dogCocks() > 0) MainScreen.text("The swollen lips of your bloated nipple gape wide, but the massive girth of your " + dogDescript(0) + " spreads them even wider. ", false);
						else if(player.tentacleCocks() > 0) MainScreen.text("The swollen lips of your bloated nipple gape wide, but the tip of your " + cockDescript(0) + " spreads them even wider. ", false);
						else if(player.demonCocks() > 0) MainScreen.text("The swollen lips of your bloated nipple gape wide, but the tip of your " + cockDescript(0) + " spreads them even wider. ", false);
						else if(player.catCocks() > 0) MainScreen.text("The swollen lips of your bloated nipple gape wide, but the barbed tip of your " + cockDescript(0) + " spreads them even wider. ", false);
						else if(player.lizardCocks() > 0) MainScreen.text("The swollen lips of your bloated nipple gape wide, but the pointed tip of your " + cockDescript(0) + " slowly spreads them even wider. ", false);
						else if(player.anemoneCocks() > 0)  MainScreen.text("The swollen lips of your bloated nipple gape wide, but the stinging tip of your " + cockDescript(0) + " spreads them wider and fills them with artificial lust. ", false);
						else if(player.displacerCocks() > 0)  MainScreen.text("The swollen lips of your bloated nipple gape wide, but the wide head of your " + cockDescript(0) + " spreads them even wider. ", false);
						else MainScreen.text("The swollen lips of your bloated nipple gape wide, but the " + cockHead(0) + " of your " + cockDescript(0) + " spreads them even wider. ", false);
		*/
					}
					MainScreen.text("Grunting and sweating with effort, you stuff as much of your " + player.cockDescript() + " into your overstretched nipple as you can fit. The feeling of incredible tightness around your " + player.cockDescript() + " combines with the pain of your distended nipple to form a mindbending sensation that makes your head spin. ");
				}
				if (player.averageLactation() > 0)
					MainScreen.text("Milky ");
				else MainScreen.text("Slick ");
				if (player.averageVaginalWetness() < 2) MainScreen.text("girl-lube quickly coats the whole length of your " + player.cockDescript() + " in a glistening layer of fuck juice. " );
				else if (player.averageVaginalWetness() < 4) {
					MainScreen.text("girl-lube drizzles down the length of your " + player.cockDescript() + " in thick streams, ");
					switch (player.lowerBody.cockSpot.list[0].cockType) { //Different way of choosing cock to use, results in consistent description of cock[0]
						case CockType.ANEMONE:
		...
		/* Old method
					if(player.normalCocks() > 0) MainScreen.text("girl-lube drizzles down the length of your " + cockDescript(0) + " in thick streams, pooling at your crotch. ", false);
					else if(player.horseCocks() > 0) MainScreen.text("girl-lube drizzles down the length of your " + cockDescript(0) + " in thick streams, pooling in and around your sheath. ", false);
					else if(player.tentacleCocks() > 0) MainScreen.text("girl-lube drizzles down the length of your " + cockDescript(0) + " in thick streams, mixing with the tentacles own lubricants. ", false);
					else if(player.demonCocks() > 0) MainScreen.text("girl-lube drizzles down the length of your " + cockDescript(0) + " in thick streams, pooling around the base of your member. ", false);
					else if(player.catCocks() > 0) MainScreen.text("girl-lube drizzles down the length of your " + cockDescript(0) + " in thick streams, pooling around the spines near your base. ", false);
					else if(player.lizardCocks() > 0) MainScreen.text("girl-lube drizzles down the length of your " + cockDescript(0) + " in thick streams, pooling around the bulbs near your base. ", false);
					else if(player.anemoneCocks() > 0)  MainScreen.text("girl-lube drizzles down the length of your " + cockDescript(0) + " in thick streams, pooling around the wriggling tentacles near your base. ", false);
					else if(player.hasKnot()) MainScreen.text("girl-lube drizzles down the length of your " + cockDescript(0) + " in thick streams, pooling around the bulb near your base. ", false);
					else  MainScreen.text("girl-lube drizzles down the length of your " + cockDescript(0) + " in thick streams, pooling around the spines near your base. ", false);
		*/
				}
				else MainScreen.text("girl-lube spurts out of your tortured nipple with each thrust of your " + player.cockDescript() + ", spattering your arms and face with your secretions. ");
			}
			if (Math.round(player.cockArea(0)) < player.vaginalCapacity()) {
				//Different noises based on sensitivity
				if (player.stats.sens < 45)
				...
				MainScreen.text(" as your "  + player.cockDescript() + " slides into ");
				//Different based on looseness
				if (player.averageVaginalLooseness() < 2)
					MainScreen.text("the small, over-stretched opening of your swollen nipple. Your " + player.cockDescript() + " penetrates your swollen nipple easily, sliding halfway in on your first thrust. ");
				else if (player.averageVaginalLooseness() < 4)
					MainScreen.text("the engorged and distended opening of your fat, swollen nipple. Your " + player.cockDescript() + " plunges deeply into your freakishly engorged nipple, penetrating it easily. ");
				else {
					MainScreen.text("the gaping fuck-mouth of your inhuman nipple-cunt.  The swollen lips of your bloated nipple engulf the ");
					if (player.lowerBody.cockSpot.list[0].cockType == CockType.HORSE) MainScreen.text("flared ");
					MainScreen.text("tip of your " + player.cockDescript() + " and begin to slide down its length, ");
					switch (player.lowerBody.cockSpot.list[0].cockType) { //Different way of choosing cock to use, results in consistent description of cock[0]
						case CockType.ANEMONE:
		...
		/* Old method
					if(player.normalCocks() > 0) MainScreen.text("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the tip of your " + cockDescript(0) + " and begin to slide down its length, swallowing it completely. ", false);
					else if(player.horseCocks() > 0) MainScreen.text("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the flared tip of your " + horseDescript(0) + " and begin to slide down its length, swallowing it completely. ", false);
					else if(player.dogCocks() > 0 || player.displacerCocks() > 0) MainScreen.text("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the tip of your " + dogDescript(0) + " and begin to slide down its length, even swallowing your bulging knot without difficulty. ", false);
					else if(player.tentacleCocks() > 0) MainScreen.text("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the tip of your " + cockDescript(0) + " and begin to slide down its length, swallowing it completely as it twists and pulses on its own, fucking your nipple.  ", false);
					else if(player.demonCocks() > 0) MainScreen.text("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the tip of your " + cockDescript(0) + " and begin to slide down its length, swallowing it completely as each bump and nodule makes you quiver with unholy pleasures.  ", false);
					else if(player.catCocks() > 0) MainScreen.text("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the tip of your " + cockDescript(0) + " and begin to slide down its length, swallowing it completely as each springy barb makes you quiver with pleasure.  ", false);
					else if(player.lizardCocks() > 0) MainScreen.text("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the tip of your " + cockDescript(0) + " and begin to slide down its length, swallowing it completely as each of the bulgy knots along its length stretch the orifice further.  ", false);
					else if(player.anemoneCocks() > 0) MainScreen.text("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the tip of your " + cockDescript(0) + " and begin to slide down its length, swallowing it completely and leaving a searing trail of desire in its path.  ", false);
					else MainScreen.text("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the tip of your " + cockDescript(0) + " and begin to slide down its length, swallowing it completely. ", false);
		*/
				}
				MainScreen.text("You revel in the sensation as you slowly stroke your " + player.cockDescript() + " in and out of your distended nipple. Your shaft is enveloped in the warm, wet embrace of your freakish tit, and ");
				if (player.averageLactation() > 0)
					MainScreen.text("milky ");
				else MainScreen.text("slippery ");
				if (player.averageVaginalWetness() < 2)
					MainScreen.text("girl-lube quickly coats the whole length of your " + player.cockDescript() + " in a glistening layer of fuck juice. " );
				else if (player.averageVaginalWetness() < 4)
					MainScreen.text("girl-lube drizzles down the length of your " + player.cockDescript() + " in thick streams, pooling at your crotch. ");
				else {
					if (player.normalCocks() > 0)
						MainScreen.text("girl-lube pours out of your swollen nipple over your " + player.cockDescript() + " and hands, pooling on the ground below you. ");
					else if (player.lowerBody.cockSpot.hasSheath() > 0)
						MainScreen.text("girl-lube drizzles down the length of your " + player.cockDescript() + " in thick streams, pooling in and around your sheath. ");
					else if (player.tentacleCocks() > 0)
						MainScreen.text("girl-lube pours out of your swollen nipple over your " + player.cockDescript() + " and hands, pooling on the ground below you. ");
					else if (player.demonCocks() > 0)
						MainScreen.text("girl-lube pours out of your swollen nipple over your " + player.cockDescript() + " and hands, pooling on the ground below you. ");
					else MainScreen.text("girl-lube pours out of your swollen nipple over your " + player.cockDescript() + " and hands, pooling on the ground below you. ");
				}		
			}
		}
			...
			if (fittableCocks > 2 && holeTotal < fittableCocks) {
				...
				if (player.vaginalCapacity() >= 2 * player.cockArea(randomCock)) {
					...
					if (2 * holeTotal >= fittableCocks)
						MainScreen.text("You giggle with glee as you realize that you will be able to jam every single one of your " + player.multiCockDescriptLight() + " into your abused nipples, thanks to double-penetration! ");
		...
		private multiNippleFuckPrep(randomCock:number):void {
			...
			if (player.lowerBody.cockSpot.list[randomCock].cockType == CockType.HUMAN) {
				...
				//more than one?
				if (player.normalCocks() > 1)
					MainScreen.text("one of your " + num2Text(player.normalCocks()) + " " + player.cockDescript(randomCock) + "s ");
				else MainScreen.text("your " + player.cockDescript(randomCock) + " ");	
			}
			...
			if (player.lowerBody.cockSpot.list[randomCock].cockType == CockType.HORSE) {
				...
				if (player.horseCocks() > 1)
					MainScreen.text("one of your " + num2Text(player.horseCocks()) + " " + player.cockDescript(randomCock) + "s "); 
				else MainScreen.text("your " + player.cockDescript(randomCock) + " ");
			}
			...
			if (player.lowerBody.cockSpot.list[randomCock].cockType == CockType.DOG)
			{
				...
				if (player.dogCocks() > 1)
					MainScreen.text("one of your " + num2Text(player.dogCocks()) + " " + player.cockDescript(randomCock) + "s ");
				else MainScreen.text("your " + player.cockDescript(randomCock) + " ");
			}
		...
		private orgazmo(selfSucking:boolean, nippleFuck:boolean):void {
			...
			if (player.lowerBody.cockSpot.count() > 0) {
				if (player.lowerBody.cockSpot.count() == 1) {
					...
					if (player.normalCocks() == 1)
						...
						MainScreen.text("swelling, bulbous knot.  You feel it growing tighter and tighter until it's nearly twice the width of your " + player.cockDescript() + ".  The pressure is an agonizing pleasure that only builds higher and higher as you come closer and closer to orgasm.  ");
					else if (player.horseCocks() == 1)
						MainScreen.text("swollen equine sheath, slowly beginning to work its way up your shaft.  Pre-cum begins pouring from your " + player.cockDescript() + ", slicking your " + player.cockDescript() + " as you get ready to blow.  ");
					else if (player.tentacleCocks() == 1)
						MainScreen.text("wiggling vine-like cock, growing rapidly.  You feel the rubbery surface of your tentacle prick contracting as it nears release.  Thick bulges of fluids collect and travel along the length of your " + player.cockDescript() + ", the first of which is almost to your oversized tip.  ");
					else if (player.demonCocks() == 1)
						MainScreen.text("tainted man-meat, growing rapidly.  You feel the nodules close to the base swell and pulse, starting a rippling wave of pleasure that migrates upwards.  The ring of bumps around your crown double in size, flaring out as your " + player.cockDescript() + " prepares to unload.  ");
					else if (player.catCocks() == 1)
						MainScreen.text("full, cat-like sheath.  You feel it tingling and throbbing as the spines pulsate with arousal.  You feel the barbs at the bottom thickening as bulges of fluid migrate through your " + player.cockDescript() + " towards the tip.  ");
					else if (player.lizardCocks() == 1)
						MainScreen.text("swollen member.  You can feel it tingling and bulging strangly as it begins to contract and pulse.  Pre-cum leaks from your " + player.cockDescript() + " in a steady stream as each 'bulb' nearly doubles in size, and then one at a time, they deflate while your urethra dilates wide.  ");
					else if (player.anemoneCocks() == 1)
						MainScreen.text("base.  The tentacles surrounding your " + player.cockDescript() + " go nuts, constricting around it, inadvertently firing aphrodisiacs into the orgasmic flesh as they wring a potent, hip-jerking climax from you.  ");
					...
					MainScreen.text("Pleasurable spasms overwhelm you as cum erupts from your " + player.cockDescript());
					...
					if (selfSucking) {
						if (player.cumQ() < 25)
							...
							MainScreen.text("Pulse after pulse of cum erupts from your " + player.cockDescript() + " into your mouth.  You swallow what you can but it's too much for you.  Cum runs down your cock to pool on you as your orgasm drags on.  ");
						else MainScreen.text("Your orgasm never seems to end, and your world dissolves into the feelings from your " + player.cockDescript() + " as it erupts jet after jet of cum into your mouth.  You nearly gag, cum overflowing to spray out in a river, pooling around you.  ");
					}
					else if (nippleFuck) {
						if (player.cumQ() < 25)
						...
						else MainScreen.text("Your orgasm never seems to end, and your world dissolves into the feelings from your " + player.cockDescript() + " as it erupts jet after jet of cum into your abused tit.  With a *pop*, your cock is forced free by the pressure. It continues to squirt semen everywhere, while a river of cum pours out of your dialated nipple, pooling around you.  ");
					}
					else {
						if (player.cumQ() < 25)
				...
				if (player.lowerBody.cockSpot.count() > 1) {
					...
					if (player.dogCocks() > player.normalCocks() && player.dogCocks() > player.horseCocks()) { //Primary Dog
						...
						MainScreen.text("Your feel your knots bulging and swelling, growing tighter and tighter until they're nearly double the width of a " + player.cockDescript(dogIndex) + ".  The agonizing pressure builds higher and tighter with every passing second as you get closer and closer to orgasm.  ");
					}
						...
						MainScreen.text("You feel a pulsing in your sheath, slowly working its way up your " + player.cockDescript(horseIndex) + "s.  Pre-cum pours from your " + player.cockDescript(horseIndex) + "s, slicking the wobbly equine shafts as they get ready to blow.  ");
					}
					else { //Primary Normal
					...
					if (nippleFuck) {
						...
						else MainScreen.text("With an explosive release, you finally cum.  Your orgasm never seems to end, and your world dissolves into the feelings from your " + player.multiCockDescriptLight() + " as they erupt jet after jet of cum into your abused tits.  With a series of wet *pops*, your cocks are forced free by the pressure. They continue to squirt semen everywhere, while rivers of cum pour out of your dilated nipples, pooling around you.  ");
					}
		
					//Doing the perverted sprinkler after dicks have popped out.
					MainScreen.text("Pleasure overwhelms your fragile mind as cum erupts from your " + player.multiCockDescriptLight() + ".  Your hips jerk in the air in time with your eruptions, cum spraying from you as your body does its best impression of a perverted sprinkler.  ");
					if (selfSucking) {
						if (player.cumQ() < 25)
							...
							MainScreen.text("Pulse after pulse of cum erupts from your " + player.cockDescript() + " into your mouth.  You swallow what you can but it's too much for you.  Cum runs down your " + player.cockDescript() + " to pool on you as your orgasm drags on.  Jizz rains over you the entire time from the rest of your \"equipment\".");
						else MainScreen.text("Your orgasm never seems to end, and your world dissolves into the feelings from your " + player.cockDescript() + " as it erupts jet after jet of cum into your mouth.  You nearly gag, cum overflowing to spray out in a river, pooling around you.  Your other 'equipment' rains jizz upon you the whole while, soaking you in a cum-puddle.");
					}
			...
			if (player.lowerBody.vaginaSpot.count() > 0) {
				...
				if (player.lowerBody.vaginaSpot.count() == 1) {
					...
					//Wet orgasms
					if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING) MainScreen.text("A veritable gush of fluid explodes from your nethers, pulsing in time with the ripples of your " + player.vaginaDescript() + ".  ");
				}
		...
		private titCum(cumQuantity:number = 3):void {
			...
			if (player.averageLactation() >=2 && !player.upperBody.chest.hasFuckableNipples()) {
				...
				if (player.averageLactation() <= 2.6) MainScreen.text("thin streams of milk spray out of your ");
				if (player.averageLactation() > 2.6 && player.averageLactation() < 3) MainScreen.text("thick gouts of milk spurt out of your ");
				if (player.averageLactation() >= 3) MainScreen.text("huge torrents of milk blast out of your ");
				//different description based on size of nips
				...
				if (player.averageLactation() <= 2.6) MainScreen.text("spattering milk everywhere. ");
				if (player.averageLactation() > 2.6 && player.averageLactation() < 3) MainScreen.text("covering everything nearby. ");
				if (player.averageLactation() >= 3) MainScreen.text("drenching the entire area.");
			}
			...
			if (player.upperBody.chest.hasFuckableNipples()) {
				...
				//Lactating?
				if (player.averageLactation() > 0) MainScreen.text("milk and ");
		...
		private deluxeDildo():void {
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				...
				MainScreen.text("  A drop of pink aphrodisiac leaks from the tip, offering you a hint of the pleasure you're in for.   You make sure to catch it on your crotch, letting the fluid seep into your " + player.vaginaDescript() + ".  Warmth radiates outwards, spreading to your thighs as your " + player.clitDescript());
				...
				if (player.stats.cor < 50)
					MainScreen.text("before slowly working it inside you, gasping at the enhanced sensitivity of your " + player.vaginaDescript() + ".");
				else MainScreen.text("ramming it deep inside you, moaning as it rubs your now over-sensitive walls.");
				MainScreen.text("  You splay your " + player.legs() + " and lie there with it inside you, feeling it respond to your wetness, becoming more and more turned on by the second.\n\n");
				...
				//(Pretty wet)
					...
					if (player.lowerBody.cockSpot.count() > 0) {
						MainScreen.text("Even your " + player.multiCockDescript() + " ache");
		...
		private onaholeUse():void {
			...
			if (player.findStatusAffect(StatusAffects.PlainOnaholeUsed) < 0) {
				...
				if (player.lowerBody.cockSpot.count() > 1)
					MainScreen.text("With your free hand, you slap your " + player.multiCockDescriptLight() + " to 'attention' and ease the onahole over your cocks.  ");
		...
		private onaholeMulticockContinuation():void {
			MainScreen.text("You pull the sloppy toy from your dribbling dick and smile, shoving its slippery surface down on another of your " + player.multiCockDescriptLight() + ".  You rapidly work it around your cocks, orgasming until ");
			if (player.lowerBody.balls > 0)
				MainScreen.text("you pass out with aching, empty balls.");
			else MainScreen.text("you pass out with " + player.multiCockDescriptLight() + " sore from exertion.");
			dynStats("sen", -1);
			player.orgasm();
			doNext(camp.returnToCampUseOneHour);
		}
		...
		private onaholeFutaContinuation():void {
			...
			MainScreen.text(String(int(((Math.random() * player.str / 12) + player.str / 6) * 10) / 10));
		...
		private stimBeltUse():void {
			...
			if ((player.hasKeyItem("Self-Stimulation Belt") >= 0)) {
				...
				if (player.findStatusAffect(StatusAffects.used_self_dash_stim) < 0) {
					...
					MainScreen.text(player.breastCup(0));
		...
		private catAutoLick():void { //Male cat masturbation
			...
			if (flags[FlagEnum.TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY] == 0) {
				...
				MainScreen.text("You undress from your " + player.armorName + " and take a seat on the ground. You take a look at your transformed body, making notes of things you haven't noticed before. Suddenly, an idea pops into your head: the cats back at the village could reach any place on their body with their tongues! You wonder... closing your eyes and slowly bending down, you try to get as close as possible to your " + player.cockDescript() + ". It only takes a moment before you feel warm breath blowing against your dick. You open your eyes, coming face to face with your erect member. Your body is twisted and bent in a way that only cats can manage. You huff a cloud of hot air on your pecker, and the resulting sensation causes your eyes to roll back in your head. That was incredible and it's about to get better as another thought passes through your head, giving you a dirty smile.\n\n");
			...
			//[Repeatable]
				...
				if (player.countCockSocks("gilded") > 0 && flags[FlagEnum.GILDED_JERKED] < player.countCockSocks("gilded")) {
					...
					MainScreen.text("You once again feel a slight chill as you reach the point-of-no-return and you let your " + player.cockDescript() + " pop free of your mouth. You watch in glee as your seed slit parts to begin the sparkling shower you know is coming. Your cum, caught in the light of the golden cocksock, beads and twists in the light, crystallizing into a glittering shower. A ");
					if (player.cumQ() < 25)
						MainScreen.text("sprinkle of");
					else if (player.cumQ() < 250)
						MainScreen.text("rain of");
		...
		private dualBeltMasturbation():void {
			...
			if (player.tailType > TAIL.NONE) MainScreen.text("covering up your tail, ");
			MainScreen.text("and then going up your body, covering your " + player.allBreastsDescript() + " and neck. The only part of your body unclad by the suit is your head. The blackness feels slick and smooth, almost cold, a strange type of feeling washes over you until you realize that it is a rubber suit.\n\n");
			...
			MainScreen.text("Meanwhile the latex around your " + player.cockDescript() + " begins to pulse and ripple faster than ever before. You quake and quiver, " + player.legs() + " giving out as it teases and pulses around your " + player.allBreastsDescript() + ". Your hands go down your body helplessly and start stroking at your encased cock, rubbing up and down your length. Unfortunately, all things must come to an end as the pleasure gets to be way too much and you feel yourself cum. Your hips buck wildly as you feel cum spurt into the latex, the end swelling up and filling like a ");
			...
			//([If high lactation]
			if (player.biggestLactation() >= 2) MainScreen.text("Milk gushes out from your " + player.breastDescript(0) + " as you orgasm, filling the inside of the suit with a slick layer of milk and forming milk bubbles that hang lewdly off your chest. ");
			...
			MainScreen.text("When you wake, the black latex is no longer covering your body and the belt is silent around your waist. Cum drips from the tip of your cock and the top part of your " + player.legs() + " are coated with your feminine juices. ");
			if (player.biggestLactation() >= 2) MainScreen.text("Thin streams of creamy milk flow from your " + player.allBreastsDescript() + ", your torso and midsection dripping wet from the stuff. ");
		...
		private centaurNippleCunt():void {
			MainScreen.text("You shrug out of your " + player.armorName + ", ");
			if (flags[FlagEnum.PC_FETISH] > 0)
				MainScreen.text("panting lustily as you envision being caught masturbating your " + player.nippleDescript(0) + "s.");
			else if (player.stats.cor < 33)
				MainScreen.text("blushing a bit as you look down at your " + player.nippleDescript(0) + "s.");
			else if (player.stats.cor < 66)
				MainScreen.text("shivering as the air hits your exposed " + player.nippleDescript(0) + "s.");
			else MainScreen.text("smiling to yourself as you blatantly oogle your " + player.nippleDescript(0) + "s.");
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 2)
				MainScreen.text("  The sudden displacement makes your " + player.breastDescript(0) + " jiggle enticingly, and the sudden movement fires arcs of pleasure deep into your body.");
			...
			MainScreen.text("  Your free fingers slowly stroke around the outer edges of your " + player.nippleDescript(0) + "s' lips, pausing when you feel a tiny sensitive clit-like nub just inside the top of the opening.\n\n");
			
			MainScreen.text("Shivering with pleasure, you kneel down to prevent your legs from going out from underneath you.   Your " + player.face() + " flushes hotter as you toy with the slippery nipple-cunts, feeling them growing hard and puffy as you turn yourself on more and more.  ");
			...
			if (player.lowerBody.vaginaSpot.hasVagina() || player.lowerBody.cockSpot.hasCock()) {
				...
				if (player.lowerBody.cockSpot.hasCock()) {
					if (player.lowerBody.vaginaSpot.hasVagina())
						MainScreen.text(player.multiCockDescriptLight() + " and " + player.vaginaDescript());
					else MainScreen.text(player.cockDescript());
					//Set as plural if multi dick or dick and vag.
					if (player.lowerBody.vaginaSpot.hasVagina() || player.totalCocks() > 1) plural = true;
				}
				else MainScreen.text(player.vaginaDescript());
				if (plural)
				...
				if (player.totalCocks() > 1)
					MainScreen.text("  You feel each of your " + player.multiCockDescriptLight() + " tapping against your belly as your muscles clench, getting as much pleasure as possible from your male endowments.");
				else if (player.totalCocks() == 1)
					MainScreen.text("  You feel your " + player.multiCockDescriptLight() + " tapping against your belly as your muscles clench, getting as much pleasure as possible from your male endowment.");
				if (player.lowerBody.balls > 0) MainScreen.text("  Rocking slightly inside your sack, your " + player.ballsDescriptLight() + " repeatedly clench up against your body, ready to release.");
				MainScreen.text("\n\n");
			}
			//(ORGAZMO)
			MainScreen.text("A slow wave of contractions starts deep inside each of your " + player.nippleDescript(0) + "s.  It intensifies as it rises towards the surface of your " + player.breastDescript(0) + ", reaching a crescendo that brings you to the peak of pleasure.  Your eyes roll back as you slump down in orgasmic bliss, fingers pumping relentlessly at you slippery nipple-holes.  Noisy wet squelches and incessant moaning rouse you from your pleasure-induced coma, eventually waking you back to reality.\n\n");
			...
			if (player.lowerBody.cockSpot.hasCock()) {
				...
				MainScreen.text("released from your " + player.multiCockDescriptLight() + " by the intense climax.");
		...
		private centaurHugeCock():void {
			...
			MainScreen.text("You shrug out of your " + player.armorName + ", not that it does much to impede your " + player.multiCockDescriptLight() + " as a centaur.   ");
			...
			MainScreen.text("your " + player.multiCockDescriptLight() + " swells out underneath you, hanging down from your equine hindquarters as ");
			if (!plural) MainScreen.text("it grows full and hard.");
			else MainScreen.text("they grow full and hard.");
			if (plural)
				MainScreen.text("  After a moment they peek out from under your forelegs, proudly displaying your " + player.multiCockDescriptLight() + ".");
			else {
				MainScreen.text("  After a moment it peeks out from under your forelegs, proudly displaying your ");
				if (player.lowerBody.cockSpot.list[primary].cockType == CockType.HORSE) MainScreen.text("flared tip");
				else if (player.lowerBody.cockSpot.list[primary].cockType == CockType.DEMON) MainScreen.text("nodule-ringed head");
				else if (player.lowerBody.cockSpot.list[primary].cockType == CockType.DOG) MainScreen.text("pointed erection");
				else if (player.lowerBody.cockSpot.list[primary].cockType == CockType.TENTACLE) MainScreen.text("wriggling, mushroom-like tip");
				else if (player.lowerBody.cockSpot.list[primary].cockType == CockType.CAT) MainScreen.text("spiny head"); 
				else if (player.lowerBody.cockSpot.list[primary].cockType == CockType.LIZARD) MainScreen.text("pointed, purple tip");
				else if (player.lowerBody.cockSpot.list[primary].cockType == CockType.ANEMONE) MainScreen.text("tentacle-ringed head");
			...
			if (player.tallness * 1.5 < player.lowerBody.cockSpot.list[player.longestCock()].cockLength) {
				...
				MainScreen.text("your " + player.multiCockDescriptLight() + ".");
			}
			MainScreen.text("\n\n");	
			...
			if (player.tallness * 1.5 >= player.lowerBody.cockSpot.list[player.longestCock()].cockLength) {
				...
				MainScreen.text(player.cockDescript(primary) + " and lift up, pressing it tightly against your belly as you begin to stroke along the sensitive underside.  ");
				if (player.cumQ() > 500)
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Your neglected " + player.vaginaDescript() + " aches for penetration, but there's no way you'll be able to reach it in your current state.");
				...
				if (player.totalCocks() == 2)
					...
					MainScreen.text("  Your other " + player.multiCockDescriptLight() + " mimic their lucky brother's pleasure, even though they've been ignored in order for you to focus on your current 'toy'.");
			...
			//Get started (long)
				...
				MainScreen.text(player.cockDescript(primary) + " and lift it up, curving it slightly as you begin to stroke the underside with long, fluid caresses.  You even manage to bend down and give it a lick, forcing you into a pleasurable shudder.");
				if (player.cumQ() > 500)
					MainScreen.text("  Thick streams of pre-cum start to drizzle from your tip, forced out due to your prodigious production of sexual fluid.");
				else if (player.cumQ() > 100)
					MainScreen.text("  A dribble of pre-cum starts to leak from the tip as you get more into the act.");
			...
			if (player.lowerBody.balls > 0)
				MainScreen.text("your " + player.ballsDescriptLight());
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				...
				if (player.upperBody.chest.hasFuckableNipples()) {
					...
					MainScreen.text(" of your " + num2Text(player.upperBody.chest.countNipples()) + " " + player.nippleDescript(0) + "s quiver, ");
		...
		private centaurDudesGetHorseAids():void {
			...
			MainScreen.text("Seeing the toy's exposed, gaping genitals, you feel a stirring in your " + player.multiCockDescriptLight() + ".  You yearn to touch yourself, but your centaur lower body prevents you, as usual.  Grunting with annoyance, you trot up to the toy and give its wide cunt an experimental fisting.  ");
			//[If small cock: 
			...
			MainScreen.text(".  You rear your " + hipDescript() + " and slam yourself into the mare's waiting hole.\n\n");
			
			MainScreen.text("The toy's passage seems to shift and contract around your " + player.cockDescript(x) + ", molding itself to perfectly sheathe you.  What a marvelous little toy!  You slide on up until you hilt yourself, your crotch pressed against the mare's wide ass as your " + player.chestDesc() + " squeezes against her back.  Now fully mounted, you begin to rut on the mare toy, slapping your hips");
			if (player.lowerBody.balls > 0)
				MainScreen.text(" and " + player.ballsDescriptLight());
			...
			MainScreen.text("Finally able to get the release you weren't able to give yourself, you feel an orgasm mounting deep within you.  You let out a loud, equine whinny as you hump the toy faster and faster, pounding her with your " + player.cockDescript(x) + " until the pleasure overwhelms you.  You cry out as you cum, squirting your centaur-cum as far into the toy as you can shoot it");
			...
			if (player.countCockSocks("gilded") > 0 && flags[FlagEnum.GILDED_JERKED] < player.countCockSocks("gilded")) {
				...
				MainScreen.text("Sated, you spend a few blissful minutes enjoying the warmth and tightness of the mare-like onahole until your " + player.cockDescript(x) + " is soft inside it.  You scamper off of her, dropping back to your four equine feet.  With a contented yawn, you disassemble the toy and haul it off back to your stash, leaking your cum the entire way.");
			}
		...
		private getHugeEggsInCawk():void {
			...
			if (player.statusAffects.get("Exgartuan").value1 == 1 && player.statusAffects.get("Exgartuan").value2 == 0 && flags[FlagEnum.TIMES_EGGED_IN_COCK] == 0) {
				...
				if (getGame().shouldraFollower.followerShouldra()) MainScreen.text(", with Shouldra's assistance,");
				...
				MainScreen.text("\n\nYou struggle, ");
				if (getGame().shouldraFollower.followerShouldra()) MainScreen.text("ghost-assisted ");
				...
				if (player.gender == 3) {
					...
					if (player.str > 50) MainScreen.text("even with your considerable strength, ");
				...
				if (player.lowerBody.balls > 0) {
					...
					if (!player.isTaur()) MainScreen.text("More begin to follow, and you massage your quickly-bloating balls as they fill with egg after egg after egg.  ");
		...
		private layEggsInYerTits():void {
			...
			if (player.statusAffects.get("Exgartuan").value1 == 2 && player.statusAffects.get("Exgartuan").value2 == 0) {
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) {
					...
					if (player.wetness() >= 4) MainScreen.text("liberally ");
		...
		private tentacleSelfFuck():void {
			...
			else if (player.stats.cor < 60) MainScreen.text("You happily remove your " + player.armorName + ", eager to pleasure yourself.\n\n");
			else if (player.stats.cor < 80) MainScreen.text("You strip naked in an exaggerated fashion, hoping someone might be watching.\n\n");
			else MainScreen.text("You strip naked, fondling your naughty bits as you do so and casting seductive looks around, hoping someone or something is nearby to fuck you.\n\n");
			
			MainScreen.text("Almost immediately, your " + cockDescript(x) + " perks up like a pet expecting to be fed, and you have to admit that you plan to give that squirming tentacle exactly what it desires - a hot, slippery slit to nestle inside of.  Already, your [vagina] has grown ");
			if (player.wetness() <= 2)
			...
			if (y >= 0) MainScreen.text("  It loops around your " + cockDescript(y) + " while you fight with it, strangling the other phallus in tight coils of squeezing, floral friction.");
			...
			if (y >= 0) {
				MainScreen.text("  Your " + cockDescript(y) + " is getting jacked off by the engorged cock-coil's motions and slowly leaks creamy pre over the jerking length.");
			...
			if (player.tentacleCocks() >= 3) {
				...
				if (player.tentacleCocks() < player.lowerBody.cockSpot.count()) MainScreen.text(" and the open air");
				MainScreen.text(".");
			...
			if (player.tentacleCocks() >= 6) {
				...
				if (player.tentacleCocks() >= 7) MainScreen.text("  They pile in there, really squirming against each other as much as you, frotting against your sweat-slick flesh.  ");
			...
			if (player.tentacleCocks() >= 2) MainScreen.text(", while your [ass]'s interior is painted bright white");
			MainScreen.text(".");
			if (player.tentacleCocks() == 2) {
				MainScreen.text("  Your untended " + cockDescript(y) + " blows spunk over you ");
				if (player.cumQ() < 50)
				...
				if (player.tentacleCocks() >= 5) MainScreen.text("s");
			...
			if (player.tentacleCocks() == 2)
				MainScreen.text(" and ass");
			else if (player.tentacleCocks() > 2) MainScreen.text(", ass, and mouth");
			MainScreen.text(", leaving your orifice");
			if (player.tentacleCocks() > 1) MainScreen.text("s");
		...
		private tentacleGoesUpYerPooperNewsAtEleven():void {
			...
			MainScreen.text("  You groan and lazily remove your " + player.cockDescript(tentacle) + " from your anus as you give in to your pleasure-induced drowsiness.");
			if (player.lowerBody.cockSpot.count() > 0) {
				//Single Cock
				if (player.lowerBody.cockSpot.count() == 1) {
					if (player.stats.lib < 30)
