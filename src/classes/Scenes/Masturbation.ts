	...
	public class Masturbation extends BaseContent {
		...
		public function masturbateGo():void {
			...
			if (player.lowerBody.cockSpot.count() == 1) {
				...
				outputText("You grasp one of your " + player.multiCockDescriptLight() + " in each hand, ");
				if (player.averageCockThickness() <= 1.8)
					outputText("wrapping your fingers around each tool and just holding yourself, the feelings of your " + num2Text(player.lowerBody.cockSpot.count()) + " cocks making you feel giddy.  ");
			...
			if (player.lowerBody.vaginaSpot.count() > 0) {
				...
				if (player.lowerBody.vaginaSpot.count() == 1) {
					...
					//Big clits
						...
						if (player.vaginas[0].vaginalWetness > VAGINA_WETNESS.DRY) outputText("glistening ");
		...
		private function titForeplay():void {
			...
			if (player.upperBody.chest.hasFuckableNipples() && player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) {
				//more than one row?
				if (player.upperBody.chest.count() >= 1) outputText(" as you move your attention to your nipples");
			...
			//If nipples aren't fuckable.
				...
				if (player.upperBody.chest.countNipples() > 2) outputText("many ");
		...
		private function titFuckSingle():void
		{
			...
			if(player.stats.lib < 45) outputText("You grip your " + cockDescript(0) + " and begin cautiously guiding it towards ", false);
			if(player.stats.lib >= 45 && player.stats.lib < 70) outputText("Shivering with anticipation, you place the tip of your " + cockDescript(0) + " against the opening of ", false);
			if(player.stats.lib >= 70) outputText("Without hesitation, you shove the tip of your " + cockDescript(0) + " into ", false);
			//More than one row?
			if(player.upperBody.chest.count() > 1) outputText("one of the ",false);
			if(player.upperBody.chest.count() == 1) outputText("one of your ",false);
			...
			if(player.upperBody.chest.count() > 1) outputText(" on one of your lower breasts",false);
			outputText(". ", false);*/
		...
		*/
			...
			if (player.upperBody.chest.count() > 1) outputText(" on one of your lower breasts");
			outputText(". ");
		...
		*/
			...
			if (Math.round(player.cockArea(0)) < player.vaginalCapacity()) {
				...
				if (player.averageVaginalLooseness() < 2)
					...
					if (player.lowerBody.cockSpot.list[0].cockType == CockType.HORSE) outputText("flared ");
		...
		*/
		...
		private function multiTitFuck():boolean {			
			...
			if (fittableCocks > 2 && holeTotal < fittableCocks) {
				...
				if (player.vaginalCapacity() >= 2 * player.cockArea(randomCock)) {
					...
					else outputText("With a flash of irritation, you realize that even if you stick two cocks in each hole, you still won't be able to fit all " + num2Text(player.lowerBody.cockSpot.count()) + " of your dicks into your abused tits. Deciding to make the best of it, you prepare to stuff in as many as you can. ");
				} 
		...
		private function orgazmo(selfSucking:boolean, nippleFuck:boolean):void {
			...
			if (player.lowerBody.vaginaSpot.count() > 0) {
				...
				if (player.lowerBody.vaginaSpot.count() == 1) {
					...
					//Wet orgasms
					if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING) outputText("A veritable gush of fluid explodes from your nethers, pulsing in time with the ripples of your " + player.vaginaDescript() + ".  ");
				}
		...
		private function titCum(cumQuantity:number = 3):void {
			...
			if (player.upperBody.chest.hasFuckableNipples()) {
				...
				//Different noise based on sensitivity
				if (player.stats.sens < 70) outputText("moan with ecstacy as your freakishly swollen nipples ");
				if (player.stats.sens >= 70) outputText("scream with ecstacy as your freakishly swollen nipples ");
				//Different description based on wetness
		...
		private function deluxeDildo():void {
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				...
				outputText(".  You ");
				if (player.stats.cor > 50) outputText("don't ");
				outputText("hesitate ");
				if (player.stats.cor < 50)
		...
		private function centaurNippleCunt():void {
			...
			if (player.lowerBody.vaginaSpot.hasVagina() || player.lowerBody.cockSpot.hasCock()) {
				...
				if (player.lowerBody.balls > 0) outputText("  Rocking slightly inside your sack, your " + player.ballsDescriptLight() + " repeatedly clench up against your body, ready to release.");
				outputText("\n\n");
			}
			//(ORGAZMO)
		...
		private function centaurHugeCock():void {
			...
			if (plural)
				...
				if (player.lowerBody.cockSpot.list[primary].cockType == CockType.HORSE) outputText("flared tip");
				else if (player.lowerBody.cockSpot.list[primary].cockType == CockType.DEMON) outputText("nodule-ringed head");
				else if (player.lowerBody.cockSpot.list[primary].cockType == CockType.DOG) outputText("pointed erection");
				else if (player.lowerBody.cockSpot.list[primary].cockType == CockType.TENTACLE) outputText("wriggling, mushroom-like tip");
				else if (player.lowerBody.cockSpot.list[primary].cockType == CockType.CAT) outputText("spiny head"); 
				else if (player.lowerBody.cockSpot.list[primary].cockType == CockType.LIZARD) outputText("pointed, purple tip");
				else if (player.lowerBody.cockSpot.list[primary].cockType == CockType.ANEMONE) outputText("tentacle-ringed head");
			...
			if (player.tallness * 1.5 >= player.lowerBody.cockSpot.list[player.longestCock()].cockLength) {
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your neglected " + player.vaginaDescript() + " aches for penetration, but there's no way you'll be able to reach it in your current state.");
				outputText("  You sigh, delighted that you're able to caress at least one of your sexual organs with this body");
				if (player.stats.cor < 40) outputText(", but you're also worried that you're falling into the perversion that lurks in this strange world");
			...
			//Get started (long)
				...
				if (player.stats.cor < 33) outputText(", but you're also worried that you're falling into the perversion that lurks in this strange world");
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) {
				...
				if (player.upperBody.chest.hasFuckableNipples()) {
					...
					outputText(" of your " + num2Text(player.upperBody.chest.countNipples()) + " " + player.nippleDescript(0) + "s quiver, ");
		...
		private function layEggsInYerTits():void {
			...
			if (player.statusAffects.get("Exgartuan").value1 == 2 && player.statusAffects.get("Exgartuan").value2 == 0) {
				...
				//[if (hasVagina = true)]
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your [vagina] lets sticky fluids out slowly around your thighs; you are only able to tell by the damp feel of your [legs].  It feels fantastic!");
				...
				if (player.eggs() < 20) {
					...
					if (player.lowerBody.cockSpot.hasCock()) outputText("  Your [chest] and ovipositor meanwhile are slowly being coated in white from your languid pleasure.");
		...
		private function tentacleSelfFuck():void {
			...
			clearOutput();
			if (player.stats.cor < 15) outputText("You sheepishly find some rocks to hide in, where you remove your clothes.\n\n");
			else if (player.stats.cor < 30) outputText("You make sure you are alone and strip naked.\n\n");
			else if (player.stats.cor < 60) outputText("You happily remove your " + player.armorName + ", eager to pleasure yourself.\n\n");
			else if (player.stats.cor < 80) outputText("You strip naked in an exaggerated fashion, hoping someone might be watching.\n\n");
			else outputText("You strip naked, fondling your naughty bits as you do so and casting seductive looks around, hoping someone or something is nearby to fuck you.\n\n");
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText(", kneading the soft mammary");
			outputText(" and twisting your [nipple] as your body goes white-hot with pleasure");
			if (player.upperBody.chest.hasFuckableNipples()) outputText(", even stuffing a finger inside a sloppy, dripping nipplecunt");
			outputText(".");
			...
			if (player.tentacleCocks() >= 3) {
				...
				if (player.tentacleCocks() < player.lowerBody.cockSpot.count()) outputText(" and the open air");
				outputText(".");
				if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 4) outputText("  Laying right between your boobs, it doesn't take long for the shaft to extend far enough to loop around each tit, sliding slowly encircling each curvy mound as it presses them together around itself, getting a titfuck while it plows your unresisting mouth.");
			}
			//No bonus cawks!
			else {
		...
		private function midasCockJackingGemsRoll(): number {
