...
private function tellAmilyToFuckOffRuinedJojo():void {
	...
	if(player.stats.int >= 45) outputText("  Goblin ale and lust drafts might do the trick.");
...
private function agreeToHelpAmilyFixJojo():void {
	...
	clearOutput();
	outputText("You nod, ");
	if(player.stats.cor < 33) outputText("feeling genuinely sorry for what you've done.");
	else if(player.stats.cor < 66) outputText("unsure if you're making the right choice.");
...
public function fixJojoOOOOHYEEEEAHSNAPINTOASLIMJIM():void {
	...
	if(player.stats.cor > 66) outputText("already regretting your choice", false);
	else if(player.stats.cor > 33) outputText("a little confused by the whole situation", false);
...
public function amilyDiscoversJojoWithTentaclesAndShitOhBoy():void {
	...
	if(player.stats.cor > 66) {
		...
		if(player.stats.int >= 45) outputText("  Goblin ale and lust drafts might do the trick.", false);
...
public function amilyIngredientDelivery():void {
	...
	if(flags[FlagEnum.AMILY_NOT_FURRY] == 1) outputText("big-eared blur whizzes past you", false);
...
internal function marbleVsAmilyFreakout():void {
	...
	if(flags[FlagEnum.MARBLE_OR_AMILY_FIRST_FOR_FREAKOUT] <= 1) outputText("As you bring Amily into your camp, you see Marble look towards you with a smile for a moment, before that smile turns into a frown.  You realize that this might not have been such a good idea...\n\n", false);
...
public function BlameMarblezSweetVagoozle():void {
	...
	if(flags[FlagEnum.MARBLE_KIDS] == 1) outputText("and child", false);
	else if(flags[FlagEnum.MARBLE_KIDS] > 1) outputText("and her children", false);
...
public function askAboutAmily():void {
	...
	//before Urta goes on duty at 1400:
		...
		if(flags[FlagEnum.AMILY_WANG_LENGTH] > 0) outputText(" her " + amilyScene.amilyCock() + " half-erect and drooling pre and", false);
...
public function amilyUrtaSex():void {
	...
	if(player.stats.cor > 50 && flags[FlagEnum.AMILY_WANG_LENGTH] > 0) outputText("  Privately, you note that it's bullshit; she's nothing <b>but</b> self-pity.", false);
	outputText("\n\n", false);
...
private function amilyUrtaSexWatch():void {
	...
	if(flags[FlagEnum.AMILY_WANG_LENGTH] > 0) outputText(", up the underside of her penis,", false);
	...
	if(flags[FlagEnum.AMILY_WANG_LENGTH] > 0) outputText(", a bead of pre-cum stretching into a line connecting their two members", false);
