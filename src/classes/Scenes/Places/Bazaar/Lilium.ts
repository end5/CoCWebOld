...
private function approachLilium():void {
	...
	if(player.stats.gems < 200) outputText("<b>You remember that you haven't got the 200 gems for her services right now.  Maybe next time.</b>", false);
...
private function leaveLilium():void {
	outputText("", true);
	spriteSelect(93);
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00267] == 0) outputText("\"<i>I'm deathly allergic to fun, unfortunately,</i>\" you reply before moving on to examine more of the bazaar.", false);
...
private function buttFuckTongueJeorb():void {
	...
	if(player.lowerBody.cockSpot.list[x].cockLength > 8) outputText("  You can feel Lilium's smaller cock resting atop your " + cockDescript(x) + ".", false);
...
private function liliumDickRidah():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) {
		...
		//(if demonic tongue)
		if(player.tongueType == TONUGE.DEMONIC) outputText("  Letting your demonic tongue extend out from between your lips, you wrap it around one " + nippleDescript(0) + " and gently squeeze, sending an electric thrill that flows to your core and back out to the tips of your fingers.", false);
	...
	//(if little clit)
	if(player.lowerBody.vaginaSpot.list[0].clitLength < 2) outputText("With one finger you begin to knead your " + clitDescript() + ", which instantly increases the pleasure emanating from your lower body.", false);
	//(else if big clit)
...
private function liliumNippleFuck():void {
	...
	//(If cock is pierced)
	if(player.lowerBody.cockSpot.list[x].pierced > 0) outputText("You can feel a slight bump as her tongue passes over each piercing and she gives the head of your cock a flick with the tip of her tongue as she reaches the end.  ", false);
	//(else)
	...
	if(player.tailType > TAIL.NONE) outputText("your tail", false);
	else outputText("your " + player.feet(), false);
	outputText(" as you cum", false);
	if(player.lowerBody.cockSpot.count() > 2) outputText(", your unused cocks drenching her with seed", false);
	...
	if(player.cumQ() >= 700 && player.lowerBody.cockSpot.count() > 1) outputText("Rivers and rivers of cum pour out of your cocks, distending her nipple-holes as the backflow gushes from around the heads.", false);
	outputText("\n\n", false);
