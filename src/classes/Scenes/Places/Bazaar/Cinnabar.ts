...
private cinnabarGreeting():void {
	...
	if(flags[FlagEnum.CINNABAR_NUMBER_ENCOUNTERS] == 0) {
		...
		//(Big enough Cock) 
			...
			if(player.lowerBody.cockSpot.count() > 1 && player.lowerBody.cockSpot.biggestCocks[0].cockArea2() >= 75) MainScreen.text("those things", false);
			else MainScreen.text("that thing", false);
			MainScreen.text(" inside me... the name's Cinnabar, but I'd rather you plug me and call me whatever you want.</i>\"  Her voice trails off into a husky, barely heard vibration as she awaits your response.", false);
			//[Fuck] [Leave]
	...
	//Not first time
		...
		//(Big enough Cock) 
			...
			if(player.lowerBody.cockSpot.count() > 2 && player.lowerBody.cockSpot.biggestCocks[0].cockArea2() >= 75) MainScreen.text("those things", false);
			else MainScreen.text("that thing", false);
			MainScreen.text(" inside me... We could sit here and talk, but I'd rather you plug me and call me your slut.</i>\"  Her voice trails off into a husky, barely heard vibration as she awaits your response.", false);
			//[Fuck] [Leave]
			if(player.lowerBody.cockSpot.count() > 2 && player.lowerBody.cockSpot.biggestCocks[0].cockArea2() >= 75)
				simpleChoices("Fuck Her", cinnabarGetsFUKKKKED, "Multi-Fuck", cinnabarMultiCockPortalFuckFest, "", null, "", null, "Leave", bazaar.enterTheBazaar);
			else simpleChoices("Fuck Her", cinnabarGetsFUKKKKED, "", null, "", null, "", null, "Leave", bazaar.enterTheBazaar);
...
private cinnabarNonHugeDickings():void {
	...
	if(player.stats.gems < 50) {
		MainScreen.text("\"<i>Ooooh... I'm sorry, but you don't have the gems to hire a companion, babe.   You should get some practice spreading your thighs and earn a REAL living,</i>\" Cinnabar teases.", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("release their squeeze", false);
	else MainScreen.text("release the butt-stretching pressure", false);
	MainScreen.text(" and settle to the business of sliding back and forth, caressing you from the inside out.", false);
	//Finger poke
	...
	MainScreen.text("You shrug and pull yourself up by the fist in her cunt, eliciting an agonized moan from your whore while you plunge your other hand into her clenching, squeezing asshole.  Her knees dig deep into the cushions, now supporting her own weight as well as yours, but the quickening pace of Cinnabar's ", false);
	if(player.vaginalCapacity() < 30) MainScreen.text("fingers", false);
	else MainScreen.text("fingers - no, fist", false);
	MainScreen.text(" tells you just how much she likes it.  ", false);
	//CUNTZ
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		MainScreen.text("She even works ", false);
		if(player.analCapacity() < 30) MainScreen.text("her fingers", false);
	...
	MainScreen.text("The rat's ", false);
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		if(player.vaginalCapacity() < 30 && player.analCapacity() < 30) MainScreen.text("fingers", false);
		else if(player.vaginalCapacity() >= 30 && player.analCapacity() >= 30) MainScreen.text("fists", false);
		else MainScreen.text("fingers and fist", false);
	}
	else {
		if(player.analCapacity() < 30) MainScreen.text("fingers", false);
		else MainScreen.text("tightly-clenched fingers", false);
	}
	...
	MainScreen.text("Cinnabar rolls over to give you a kiss on the cheek once she's recovered.  Her voice is still rough from the volume of her pleasure-squeaks, but she manages to whisper, \"<i>That was nice for someone who didn't have a cock worth fucking.  Feel free to catch your breath, but I'm a working girl so don't take too long.  I've got to get the sheets cleaned", false);
	if(player.cumQ() >= 1000) MainScreen.text(" and the floor mopped", false);
...
private cinnabarGetsFUKKKKED():void {
	...
	else MainScreen.text("a dragoness cry", false);
	MainScreen.text("!  Look at the mirrors; go on, look,</i>\" she instructs.\n\n", false);
	
	MainScreen.text("Sure enough, when you look at the mirrors you get a surprise.  There, in all its glory, is Cinnabar's dripping, wet snatch.  Her lips are a ruddy-red near her thighs, but the closer they get to her slit, the blacker they become.  You note her vulva and labia are slightly parted, perhaps by your previous attempt at penetration, and her shining, black love-tunnel seems to almost beckon for your attentions.  \"<i>Mmmm, look how wet I am for you, " + player.mf("stud","slut") + "... I've been such a bad girl – look at my puny pussy.  Why doesn't " + player.mf("master","mistress") + " punish it?</i>\" she asks, pressing back hard enough to make her genitals bulge around the mirror's rim.\n\n", false);
	
	MainScreen.text("You smile and ball your hand into a fist, ", false);
	if(player.stats.cor > 66) MainScreen.text("enjoying the raunchiness of the act", false);
	else if(player.stats.cor > 33) MainScreen.text("a little worried by how much you're enjoying this", false);
	else MainScreen.text("a little amazed that you would ever do such a thing", false);
	MainScreen.text(", then push it forwards, battering through the silky slit and into her canal.  Her juices let you slide right in, squirting and dribbling over her vulva like a fresh fruit you've just bitten into.  You lean over to lick at clear, slimy droplets, feeling your arm sink deeper inside the velvet vice with each taste you get.  It's gone in up to the elbow, and Cinnabar is squeaking happily, without a hint of discomfort.\n\n", false);
...
private fuckCinnabarHugerAndCorruptedEr():void {
	...
	else if(player.cumQ() < 1500) MainScreen.text("makes her look pregnant and vents a puddle of syrupy-spooge on top of you.", false);
	else MainScreen.text("makes her look like a hugely-pregnant fuck-beast and dumps cum everywhere.", false);
	MainScreen.text("\n\n", false);
...
private cinnabarHuger():void {
	...
	MainScreen.text("You're almost there, so close to your orgasm, but you've got so much more cock to bury inside this flexible, stretched-out pussy, and you're determined to bury every last inch of your improbably-large prick inside her before you blast your load.  At this rate, you'll be pushing in all day, so you decide to speed things up.  You grab her by the haunches, feeling your fingers sink into her silky fur, and you pull her back, flexing your biceps as you yank her down with all your might.  She slides down fast, taking a foot of cock in seconds, fast enough to make her belly bulge out in a distinctly dick-like manner.\n\n", false);
	...
	if(player.cumQ() >= 500) {
		...
		else if(player.cumQ() < 1500) MainScreen.text("spooge-packed belly from all the cum.", false);
		else MainScreen.text("spooge-pregnancy from all the cum.  It swings underneath her, making her look ready to birth.", false);
		MainScreen.text("  ", false);
	}
	...
	MainScreen.text("You smile back and go get dressed; it's way past time you checked on your campsite.  As you open the door, you can hear Cinnabar humming happily and trying to clean all the mess from her sheets", false);
...
private cinnabarHuge():void {
	...
	if(player.cumQ() >= 1000) MainScreen.text(" and so much", false);
	MainScreen.text("... YES!  CLAIM ME!  I'm yours... your cock-addicted, stretched-out fucktoy!  More... more!</i>\"\n\n", false);
...
private cinnabarMultiCockPortalFuckFest():void {
	...
	if(player.cumQ() >= 500 && player.cumQ() < 1000) {
		MainScreen.text("Cinnabar's belly gurgles, her gut and womb packing with enough spunk to make her belly wobble slightly while you empty inside her.  Semen froths on her lips while she works to swallow it all.  It just adds to her already-impressive paunch.", false);
		if(player.lowerBody.cockSpot.count() >= 4) MainScreen.text("  The rat's tits are completely white, spooge leaking off her nipples like some kind of thick, sticky milk.", false);
		MainScreen.text("  You finish filling her with cream and hold your place long enough to admire your work from across the room.\n\n", false);
	}
	//(High cum production)
	else if(player.cumQ() >= 1000) {
		MainScreen.text("Cinnabar's belly gurgles, the skin pulling tight from all the seed you're pumping into her womb, gut, and belly.  She looks positively pregnant, and she even begins to slosh from each fresh deposit that you leave inside her.  The rat's eyes look up at you through a portal.  They're lidded with a mix of lust and pleasure, but your gaze goes lower to watch spooge bubble from her nose while her overtaxed throat fails to keep up with your heavy spunk-streams.", false);
		if(player.cumQ() >= 1500) MainScreen.text("  Best of all, her bed is completely soaked in the stuff, and she wallows in it like a pig in mud, her cleavage absolutely drenched with cum.", false);
		MainScreen.text("\n\n", false);
	}
