...
private function cinnabarGreeting():void {
	...
	if(flags[FlagEnum.CINNABAR_NUMBER_ENCOUNTERS] == 0) {
		...
		//(Big enough Cock) 
			...
			if(player.lowerBody.cockSpot.count() > 1 && player.lowerBody.cockSpot.biggestCocks[0].cockArea2() >= 75) outputText("those things", false);
	...
	//Not first time
		...
		//(Big enough Cock) 
			...
			if(player.lowerBody.cockSpot.count() > 2 && player.lowerBody.cockSpot.biggestCocks[0].cockArea2() >= 75) outputText("those things", false);
			else outputText("that thing", false);
...
private function cinnabarNonHugeDickings():void {
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) outputText(" and your " + assholeDescript(), false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("grab your " + clitDescript() + ", rolling it between her thumb-pads with a hair too much pressure", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("release their squeeze", false);
	...
	if(player.lowerBody.cockSpot.hasCock() && !player.lowerBody.vaginaSpot.hasVagina()) outputText("  Unbidden, " + sMultiCockDesc() + " begins to harden, stiffening with every brush against your prostate.", false);
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Femcum begins to soak the rat's practiced fingers, drenching her finger-pads with liquid lust.", false);
	outputText("  You rock back, slamming your " + buttDescript() + " against the wall and moaning with need.  She's quite skilled, and ", false);
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("caresses your passage in ways that ", false);
	...
	if(player.lowerBody.cockSpot.hasCock() && !player.lowerBody.vaginaSpot.hasVagina()) outputText("You gasp and moan when you feel her other hand wrap around your " + cockDescript(0) + ", pulling and tugging on it until it starts to leak a trail of squeezed-out man-slime on the mattress.  ", false);
...
private function cinnabarGetsFUKKKKED():void {
	...
	if(flags[FlagEnum.CINNABAR_FUCKED_CORRUPT_PREVIOUSLY] > 0) outputText("  Just be gentler than last time, okay " + player.mf("stud","baby") + "?", false);
	...
	if(player.stats.cor > 66) outputText("enjoying the raunchiness of the act", false);
	else if(player.stats.cor > 33) outputText("a little worried by how much you're enjoying this", false);
	...
	if(player.lowerBody.balls > 0) outputText(sackDescript() + " and ", false);
...
private function fuckCinnabarHugerAndCorruptedEr():void {
	...
	if(flags[FlagEnum.CINNABAR_FUCKED_CORRUPT_PREVIOUSLY] > 0) outputText("  You're doing it again – turning her into a cum-sleeve without a care for her pleasure, but you don't fucking care.  You're too horny and she's too sexy not to abuse.", false);
	outputText("\n\n", false);
	...
	if(!player.lowerBody.cockSpot.hasSheath()) outputText("crotch", false);
	...
	if(player.lowerBody.cockSpot.list[x].cockLength >= 60) outputText(", and only now do you realize that you're long enough that you should've gone the whole way through her.  Just what kinds of magics is she capable of!?", false);
	...
	if(player.stats.int >= 50) outputText("You dimly register it as an arousal spell, but you were already on edge, and your poor prostate is practically wringing itself dry in an effort to cum.", false);
...
private function cinnabarHuger():void {
	...
	if(player.lowerBody.cockSpot.list[x].cockLength < 24) outputText(num2Text(Math.round(player.lowerBody.cockSpot.list[x].cockLength)) + " inches", false);
	else outputText(num2Text(Math.round(player.lowerBody.cockSpot.list[x].cockLength/12)) + " feet", false);
	outputText(" all over again.", false);
	if(player.lowerBody.cockSpot.list[x].cockLength >= 60) outputText("  There's no way this should be possible, but it's happening and you couldn't be happier.", false);
	outputText("\n\n", false);
	...
	if(player.lowerBody.balls == 0) outputText("s", false);
	...
	if(player.lowerBody.cockSpot.list[x].cockLength >= 60) outputText("that it should be sticking up past her head, yet somehow isn't.", false);
...
private function cinnabarHuge():void {
	...
	if(!player.lowerBody.cockSpot.hasSheath()) outputText("the base of your twitching cock", false);
	...
	if(player.lowerBody.balls == 0) outputText("prostate has been squeezed dry.", false);
...
private function cinnabarMultiCockPortalFuckFest():void {
	...
	if(flags[FlagEnum.CINNABAR_HOUSE_VISITED] == 0) outputText("She leans, guiding you towards her home even as she lets herself be led.", false);
	...
	if(player.lowerBody.cockSpot.count() == 4) outputText("  The extra dick dives into the pillowy mounds, smearing a streamer of pre-cum over the tit's surface before it slides into that warm, welcoming cleavage.", false);
	else if(player.lowerBody.cockSpot.count() > 4) outputText("  The extra dicks dive into the pillowy mounds, smearing streamers of pre-cum all over the tits' surfaces before they slide home into that warm, welcoming cleavage.", false);
	outputText("\n\n", false);
	...
	if(player.lowerBody.cockSpot.count() >= 4) outputText(" while you tit-fuck her swollen breasts", false);
	...
	if(player.lowerBody.cockSpot.count() >= 4) {
		...
		if(player.lowerBody.cockSpot.count() == 4) outputText("extra dick", false);
	...
	if(player.lowerBody.cockSpot.count() >= 4) outputText(", her tits shining white from all the spooge you dump on to them", false);
	outputText(".\n\n", false);
	...
	if(player.cumQ() >= 500 && player.cumQ() < 1000) {
		...
		if(player.lowerBody.cockSpot.count() >= 4) outputText("  The rat's tits are completely white, spooge leaking off her nipples like some kind of thick, sticky milk.", false);
