...
private function Roxanne1stApproach():void {
	...
	Render.text("Suddenly, the alpha-lizan glances up and meets your eye, her expression turning into a leering sneer as she asks, \"<i>See something you like " + player.mf("buddy","girly") + "?  Come on over, tell us your story!</i>\"\n\n", false);
	Render.text("Do you approach?", false);
...
private function RoxanneChooseApproachOrRepeat():void {
	...
	//[Approach – lost last contest and she's huge]
		...
		Render.text("\"<i>Oooh, praise the sea god Ulrun, you've returned.  " + player.short + ", it's been a while, and I've gotten sooooo big,</i>\" cheers an ecstatic Roxanne.  To emphasize her point she shows the seam-splitting bulge in her pants, rubbing it from the base all the way down to the tip, which rests next to her knee.  She's so big, and a damp spot appears at her pant-leg's knee while the huge cock-sausage visibly inflates.  Captain Poisontail asks, \"<i>So, can I count on you to get sauced and help me take care of this beast again, or do you actually think you have a chance of winning now?</i>\"\n\n", false);
		Render.text("It looks like she wants to engage you in another drinking contest.  If you lose ", false);
		if(player.analCapacity() >= 100) Render.text("she's going to keep you stretched and gaping", false);
		else Render.text("she's probably going to stretch your ass beyond its normal limits", false);
		Render.text(".  Do you enter a drinking contest with Roxanne?", false);
	}
	//[Approach – lost last contest and she's not big]
	else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00224] == 2 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00225] < 200) {
		Render.text("\"<i>Welcome back " + player.short + ".  Did you miss my touch badly enough that you came back for more?</i>\" asks Roxanne.  She pats an empty chair and spreads her legs, shifting her position to get more comfortable while reminding you of your recent 'defeat'.  Thankfully it looks like she's had sex recently and her dick is a far more normal size.  The lizan asks, \"<i>So, ready for another drinking contest?  Standard rules – I win; I plug your sweet ass full.  You win; I lick you to climax.</i>\"\n\n", false);
		Render.text("Do you drink with Roxanne again?", false);
	}
...
private function roxanneDrinkingContest():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00225] >= 200) Render.text("stumbles over her huge manhood, working towards", false);
	else Render.text("saunters over to", false);
	Render.text(" the demonic-looking deer-taur working the tap and gives him a weighty gem-pouch, covering the cost of the contest and her mates' drinking in advance.  She lets the keg-keep top off the mug and throws it back, easily draining it with a few practiced swallows.  Her tail slaps the table in front of you as she teases, \"<i>Are you going to check me out all day or get smashed?  Come on!</i>\"\n\n", false);
	Render.text("You take the mug and hand it to the tainted 'taur working the tap.  ", false);
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00226] == 0) {
		Render.text("The 'taur at the tap quickly grows bored with the constant bantering between the scaly swashbuckler and yourself, only bothering to look your way when the two of you walk back for a refill.  The gluttonous chugging that started the contest gives way to a more languid pace as you and Roxanne become increasingly intoxicated, slowing down in hopes that the other will be judged unfit first.  It does get a little hard to focus with the way she's constantly eyeballing you, and her curvy figure and ", false);
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00225] >= 200) Render.text("massive, seam-ripping bulge", false);
		else Render.text("hard-to-hide bulge", false);
		Render.text(" give you more than an eyeful every time you return her leer.  Still, the scaly shemale must be feeling the same way, judging by the large damp spot her cock is making.\n\n", false);
		dynStats("lus", 25);
	}
	//[DRINKING CONTEST CONTINUES – losing intentionally] 
	else {
		Render.text("The 'taur at the tap quickly grows bored with the constant bantering from Roxanne and the flirting you shower the lizan in.  You down your drinks quickly, even sneaking refills while the lizan is distracted in order to speed your inevitable loss.  She looks at you, clearly checking you out while you unabashedly fixate on the pulsing mass of cock-flesh that strains her oh-so-tight pants.  Roxanne stops drinking and walks over to you, a little unsteady but still in control of herself, and pulls your head against her groin, letting you nuzzle it while she puts filled mugs in your hands.  \"<i>Go on and drink... good " + player.mf("boy","girl") + ",</i>\" she coos when you turn to the side and swallow more of the delicious brew.\n\n", false);
	}
	let score:number = 0;
	...
	if(score < (45 + rand(20))) {
		...
		Render.text("Giggling and nearly tripping up on your own " + player.feet() + ", you stumble up to the corrupted deer-taur.  He looks at your wobbling stance, nearly-vacant eyes, and dopey grin before he shakes his head from side to side and says, \"<i>No.</i>\"  ", false);
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00226] == 0) Render.text("Nooooo! You're cut off! That means Roxanne won...", false);
		else Render.text("Yessss!  You finally got so drunk that Roxanne has no excuse not to pack your drunk ass full of lizan-spoo!", false);
		Render.text("  A scaled hand slaps your " + buttDescript() + " spinning you around to fall drunkenly into the pirate's soft, cushy chest.  \"<i>Don't worry, I'll be gentle,</i>\" she whispers, hooking an arm around your sagging frame.", false);
		//CHOOSE SEX SCENE
	...
	//[WIN]
		...
		Render.text("Laughing uproariously, you watch with a bemused expression while Roxanne tries to stumble up to the tap, tripping over her tail three times before she finally manages to ask for another mug.  The partly-corrupted deer-taur shakes his head and folds his arms across his chest.  She's been cut off!  You win!  The lecherous lizan stamps her heeled boot in the dirt before tramping back over to you, hips swaying drunkenly.  She trips on her tail again, this time falling face-first into your lap.  Hiccuping drunkenly, Roxanne slurs, \"<i>Well at leasht I don't havta go far to give you your winningshh, huh?</i>\"\n\n", false);
		Render.text("The other lizans are looking at you with a watchful eye.  It looks like you'll have to stick by the terms of the contest.  What manner of oral service do you make her provide?", false);
...
private function roxanneGivesABlowjob():void {
	...
	Render.text("You open the lower portion of your " + player.armorName + " and, pulling it back, hang out your " + multiCockDescriptLight() + "; ", false);
	if(player.lust < 70) Render.text("it stiffens to a full, erect state.", false);
	else Render.text("the already-hard cock-flesh stands proud and erect.", false);
	Render.text("  Roxanne giggles drunkenly, her half-lidded eyes looking up as she wobbles back and forth on her knees.  \"<i>I don't do thish that much, but when you've got a dick that getsh as big as mine... you learn just where the good spotsh are.</i>\"\n\n", false);
	
	Render.text("Roxanne's smooth, scaled hand curls around the ", false);
	if(player.hasKnot(x)) Render.text("knot", false);
	else if(player.lowerBody.cockSpot.hasSheath()) Render.text("sheath", false);
	else Render.text("base", false);
	Render.text(", squeezing you softly while she steadies herself on your " + cockDescript(x) + ".  She titters, letting a few inches of tongue slip through her lips to envelop your sensitive " + player.cockHead(x) + ", circling the engorged cock-flesh with drunken slobbers.  ", false);
	if(player.lowerBody.balls > 0) Render.text("A set of warm, long-nailed fingers squeeze your " + ballsDescriptLight() + ", dragging sharp nail-tips along the underside of your sack to tease the poor, cum-packed orbs.  ", false);
	Render.text("The lizan glances back up at you, inebriated; her half-vacant eyes make love to you while her pink tongue worships your beer- and spit-covered member.\n\n", false);
	
	Render.text("You slump back in your chair, sprawling out your " + player.legs() + " around you, completely relaxed by the alcohol flowing through your veins and the exquisite oral service.  Roxanne returns her focus to your " + cockDescript(x) + ", more tongue spilling from her gaping maw to curl around your lust-engorged shaft.  The drunken lizard's oral organ devours ", false);
	if(player.cockArea(x) < 60) Render.text("the entire length of your " + cockDescript(x), false);
	else Render.text("as much of your over-sized member as she can encompass", false);
	Render.text(", constricting, snake-like, to hold you in a slippery, warm embrace.  Suddenly, a pointed nail drags over your taint, pressing just hard enough to hold you still while the shemale pirate works your " + cockDescript(x) + " with her tongue", false);
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(", stopping just short of your female entrance", false);
	Render.text(".\n\n", false);
	
	Render.text("Slurping and pumping, the pulsating tongue drags over your " + cockDescript(x) + ", molding into a perfect cock-sleeve.  At first it's a slow, gradual pump that smears your shaft with booze-flavored spit, but when the first dollop of pre-cum rolls onto the flexible fellatio-tool, the pirate goes into over-drive, pumping with wild abandon.  Her tail whips back and forth, passionately slapping the table, chairs, and dirt in her excitement.\n\n", false);
	
	Render.text("A few demons start to approach, but the lizan crew interposes themselves between their captain and the interlopers, shooing them off before they can start something.  You barely notice, so focused are you on the feel of saliva running down your " + cockDescript(x) + " and the soft, pliant mouth flesh trying to wring the jism from your shaft.   The warmth of orgasm starts to build in your loins, and you begin to pump at the tongue instinctively, hips rising up off the chair in spite of the sharp fingernail's warning.\n\n", false);
	
	Render.text("SMACK!  Roxanne's tail slaps into your " + buttDescript() + ", stinging the exposed " + player.skinDesc + ".  Her eyebrows narrow in irritation while she wraps her arms around your waistline, steadying her off-balance body while she spanks and pumps you in a drunken frenzy.  You hump her face, pressing your " + player.cockHead(x) + " against her lips to smear them with a glaze of leaky pre-cum.  Grabbing her horns, you cry out and pleasure and try to pull her down, but she spanks you, HARD.  You yelp in pain, dropping her horns and submitting completely to her tongue and the pleasure it brings.\n\n", false);
	
	Render.text("The wiggling, flexible tongue-tip presses down on your urethra, bottling the cum up inside you.  Tiny rivulets of white goo squirt and leak around the pink blockage, rolling over the many rings of tongue while your urethra bloats wide.  The lizan looks up, her dull, glazed eyes locking on to yours as she uncovers your cum-slit, then shivering as a ", false);
	if(player.cumQ() >= 1500) Render.text("huge torrent utterly drenches her face, horns, neck, and tight top", false);
	else if(player.cumQ() >= 500) Render.text("a huge spurt soaks her face and horns", false);
	else Render.text("big spurt splatters her face", false);
	Render.text(" with cum.  Her soft, oral organ squeezes the spit-slick surface, milking the rest of your cum from your " + cockDescript(x) + " and " + ballsDescriptLight() + ".\n\n", false);
	
	Render.text("The pirate pulls back, uncoiling her tongue to slurp the heavy load from her face", false);
	if(player.cumQ() >= 1500) Render.text(" and body", false);
	Render.text(".  \"<i>Jeeze, pent up much?</i>\" she laughs, struggling to stand while giggling at her own supposed cleverness.  Happy with how the contest worked out, you slip " + sMultiCockDesc() + " into your " + player.armorName + " and wobble off back to camp.", false);
	//(-100 lust, -1 int)
	player.orgasm();
	dynStats("int", -1);
...
private function roxanneCunnilingus():void {
	...
	Render.text("The pink length of Poisontail's tongue oozes out like a sentient creature, waggling slightly as if sniffing the air before it presses on the ", false);
	if(player.wetness() >= 4) Render.text("juice-dribbling ", false);
	else if(player.wetness() >= 2) Render.text("juicy ", false);
	Render.text("entrance of your loins.  You swoon, leaning against the chair's back and scooting your crotch forward in a fit of wanton need.  The tip of Roxanne's nose disappears into the moist lips, her slick, oral organ engulfed by your lusty tunnel.  It squirms and wriggles inside you, the thick pink protrusion lashing about, swirling in a circular motion to lap the juice from your walls.  Spit begins to drip down your " + player.legs() + " and you see Roxanne's saliva frothing at the corners of her mouth while she works you over.\n\n", false);
	
	Render.text("Face blushing red, you groan and hump at the lizan, grabbing her horns so that you can smear her nose into your " + clitDescript() + ".  She pulls back, resisting your insistent pulls with inebriated strength, but before you can let go she reverses direction and smashes her forehead into your gut, winding you.  Your fingers release immediately, clutching at your aching middle while the angry lizard snarls, \"<i>You're getting oral, NOT using me as a dildo!  ...but I am sorry I hit you that hard. I just needed to breathe.  Relax and let me make it up to you...</i>\"\n\n", false);
	
	Render.text("The drunken lizan puts her soft, lightly scaled fingers around you to squeeze at your " + buttDescript() + " and leans in to give your " + clitDescript() + " a tender kiss.  Her massive tongue ", false);
	if(player.lowerBody.vaginaSpot.list[0].clitLength < 16) Render.text("dwarfs", false);
	else Render.text("envelops", false);
	Render.text(" the ", false);
	if(player.lowerBody.vaginaSpot.list[0].clitLength < 1) Render.text("little pleasure buzzer", false);
	else if(player.lowerBody.vaginaSpot.list[0].clitLength < 3) Render.text("swollen love-button", false);
	else Render.text("cock-like lady-part", false);
	Render.text(", slobbering over it with a lewd tongue-kiss. Her frothy spit completely soaks it ", false);
	if(player.lowerBody.vaginaSpot.list[0].clitLength >= 3) Render.text("before she curls tightly around the female member, enveloping it with flexible, gooey tongue.  You catch your breath just in time to exhale the noisy sounds of your pleasure, and groan out loud.  Roxanne takes the auditory encouragement to heart and begins pumping her tongue while she places her lower lips against your " + vaginaDescript() + " and hums.  As if that wasn't enough, she begins twisting the curled tongue up and down your " + clitDescript() + " rotating it as she pumps you.", false);
	else Render.text("before she lets her spit-sloppy tongue push through the saliva-soaked entrance of your mound.  You catch your breath just in time to exhale noisy sounds of pleasure, groaning out loud while you're speared with flexible, gooey tongue. Roxanne takes the auditory encouragement to heart and begins to pump her tongue deep inside you, at least two feet of constantly-thickening appendage coiling over your entrance before sliding inside to kiss your cervix.  As if that wasn't enough, a moment later she starts to hum, nuzzling at your " + clitDescript() + " while she tongue-fucks you.", false);
	Render.text("\n\n", false);
	
	//(ORGASM HERE!)
	Render.text("Helplessly twitching in your place, you fight with your body's desire to copulate, trying not to impale your quivering snatch on the pirate's nose.  ", false);
	if(player.upperBody.chest.hasFuckableNipples()) {
		Render.text("Roxanne pulls her hands off your ass and reaches up, swaying unsteadily until she catches your " + nippleDescript(0) + "s.  A second later she slips her digits inside the welcoming tit-pussies, finger-fucking your breasts as ", false);
		if(player.biggestLactation() < 1.5) Render.text("expertly as the one below", false);
		else Render.text("they begin to squirt milk around her", false);
		Render.text(".  ", false);
	}
	Render.text("Unable to dam the pressure up inside you any longer, you cum with brain-breaking, mouth-babbling intensity.  The lizan doesn't relent in the slightest as you begin to spasm", false);
	if(player.wetness() >= 5) Render.text(" and squirt", false);
	Render.text(" against her.  She tightens her grip and speeds up her tongue's sensual massaging, nearly blacking you out while you writhe in her embrace.  Heat, pure, blissful heat, spreads through you, draining the strength from your uncontrollable twitches and leaving languid warmth in its place.  ", false);
	if(player.lowerBody.cockSpot.hasCock()) Render.text(SMultiCockDesc() + " bounces on her head, between her horns, spilling a trail of sticky seed down her back.  ", false);
	Render.text("You slouch down, pleasure-drunk and giggling while Roxanne disentangles herself.\n\n", false);
	
	Render.text("The cocky, drunken lizan slurs, \"<i>Boysh, I'ma need a while to schleep this one off.  Next time I plug her asssshh!</i>\"\n\n", false);
	
	Render.text("You smirk back, slowly putting your clothes back on.  Roxanne can do little to prevent you from noting the massive, unmistakable cock-outline in her pre-cum-soaked pants.  She gives you a smoky glare filled with lust and aggression, but she won't be getting her prize this time!", false);
	//(-100 lust, -1 int)
	player.orgasm();
	dynStats("int", -1);
	doNext(camp.returnToCampUseOneHour);
...
private function roxanneRimjob():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) Render.text(", your breasts cushioning you from the hard wood below", false);
	Render.text(".  Poisontail eyeballs the ring of your " + assholeDescript() + ", salivating as she grabs your " + buttDescript() + " in her scaled hands.  With anguished slowness, she pulls the cheeks as wide apart as possible.  You hear her gasp in delight a split-second before you do the same.  Her tongue launched from her mouth and found your " + assholeDescript() + " immediately!  Now she's sliding it in circles around the sensitive ring, making the skin of that pucker tighten and wink.\n\n", false);
	
	Render.text("Roxanne groans and gives your " + buttDescript() + " a raunchy squeeze, making love to your cheeks as her tongue worships your asshole.  She murmers, \"<i>Thish would be sho much better if it was jussht foreplay.  Why don't you lose next time, okay babe?</i>\"  You moan out loud when her tongue returns to its task, arching your back and whimpering from the intense, anal pleasure.  ", false);
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text(SMultiCockDesc() + " ", false);
		if(player.lust < 70) Render.text("hardens", false);
		else Render.text("pulses with need", false);
		Render.text(", a bead of pre-cum already hanging from the tip.  ", false);
	}
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("Meanwhile, your neglected box is getting wetter and wetter, with no sign of stopping.  The lizan's massive, questing tongue ignores it.  ", false);
	Render.text("Completely focused on your anal ring, Roxanne keeps her tongue exactly where it is, working the sparking nerves around your rectum incessantly.\n\n", false);
	
	//(Optional cock milking)
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("Surprisingly, one of the pirate's hands comes off your " + buttDescript() + " to tug on ", false);
		if(player.totalCocks() > 1) Render.text("one of ", false);
		Render.text("your member", false);
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s", false);
		Render.text(", milking the shaft like a cow's teat.  Smooth, scaled fingers circle it at the base and slowly slide down, tugging at your skin and squeezing out thick dollops of pre-cum with every pull.\n\n", false);
	}
	//(Optional pussy milking if no cock)
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("Surprisingly, one of the pirate's hands comes off your " + buttDescript() + " to play with your " + vaginaDescript() + ".  Long, sharp nails play over the sensitive skin of your vulva while the supple, scaled thumb presses between them.  Juicy girl-cum quickly coats her fingers, but it doesn't seem to bother her in the least.\n\n", false);
	
	Render.text("The long, wondrous tongue pushes forward, wriggling inside you.  At first it's just the tip, but the drunken lizard forces inch after inch inside you with constant intensity.  You can feel her spit frothing around your " + assholeDescript() + ", bubbling while she fills your backdoor with ever larger quantities of saliva-covered tongue.  Seeming to go on forever, the penetration robs you of strength, filling you with dozens more inches.  Roxanne hums at your dark hole, vibrating her tongue inside you while it arches to caress the most sensitive spots.\n\n", false);
	
	Render.text("Orgasm hits you powerfully, your " + assholeDescript() + " doing its best to choke the massive, pink tongue inside it.  You throw back your head and howl with pleasure, bumping your " + buttDescript() + " into your lizan lover's nose unconsciously.  She gives you a knowing squeeze while her tongue keeps up the attack, forcing waves of unfiltered, raw pleasure into your cerebellum.", false);
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("  You spurt in her hand, letting her squeeze out dollop after dollop of cum.  Somehow you keep your whimpering to a minimum while that long, perfect tongue rubs your prostate and her hand squeezes every last drop into the dirt.", false);
		if(player.cumQ() < 500) {}
		else if(player.cumQ() < 1000) Render.text("  It forms a nicely-sized pool before the ground wicks it up.", false);
		else if(player.cumQ() < 1500) Render.text("  It forms a large pool before the ground soaks it up.", false);
		else Render.text("  It forms a massive pool before the ground soaks it up.", false);
	}
	else if(player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("  You quiver and cum on her fingers, ", false);
		if(player.wetness() >= 4) Render.text("soaking them with your gushing orgasms ", false);
		else Render.text("sliming them with copious girl-cum ", false);
		Render.text("while she teases your " + clitDescript() + " with those terribly-slow touches.", false);
	}
	Render.text("\n\n", false);
	
	Render.text("Completely finished, you can do naught but tremble while Roxanne disentangles her tongue from your rectum, popping from your backside with a noisy slurp.  She immediately falls over and giggles, completely drunk, but she somehow staggers back up to give your ass a smack.  You follow her example and pull yourself to your feet, tired from the anal orgasm but feeling quite relaxed.  Once you've readied yourself, you realize that Roxanne's crew are arranged in a semi-circle around you.  You note a number of demons are on the prowl beyond that protective ring, though they seem to be dispersing now that the act has finished.  You make your way back to check on camp once it looks safe.", false);
	//(-100 lust, -1 int
	player.orgasm();
	dynStats("int", -1);
	doNext(camp.returnToCampUseOneHour);
...
private function roxanneReamsYouNormal():void {
	...
	Render.text("You giggle underneath the lizan, your face mopping up the frothy brew that spilled during the previous bout of binge drinking.  Lapping it up, you forget about your predicament as you adjust to the warm, scaled body overtop you.  Roxanne leaves you to your distractions while she wiggles out of her pants, her tail swaying to counterbalance the now-freed bulk of her member.  The sodden shaft rises to bump your butt, dripping with pre-cum as it wiggles between your cheeks.  It prods your " + assholeDescript() + ", rousing you from your drunken stupor and earning a look back, but the pirate begins to tongue the edge of your ear, making you giggle and relax into sexy, alcohol-lubricated acquiescence.\n\n", false);
	
	Render.text("With a slow, steady push, Captain Poisontail forces her drippy tip through your liquor-loosened ring and into your innards.  You grunt in discomfort from the sudden straightening of your colon, unable to take such a beast in silence.  It's enough to stir you from your semi-conscious silence, and you try to ask her to go slower, slobbering and slurring your protests through numbed lips.  \"<i>Relax, my helpless prey.  Don't fight.  Just lie there and take what's coming to you,</i>\" orders the still somewhat-sober lizan while she leers at you with an expression that borders on malice.  Her words ring true, you're drunk and helpless to resist the inches of thick shemale cock sliding into your stretched pucker.", false);
	player.buttChange(Math.floor(30+(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00225]/4)),true,true,false);
	Render.text("\n\n", false);
	
	Render.text("Too intoxicated to control yourself, your sphincter clenches repeatedly, drawing in a few more inches of Roxanne's thickness.  It packs you completely, filling you in a way that centers all of your limited thoughts on the feel of that bulbous mass inside your anus.  While the lizan is scaled from head to toe, her penis clearly isn't.  You judge by the shape stuffing you that her cock is like that of a human,  a long veiny shaft capped with a rounded crown.  Her balls slap ", false);
	if(player.lowerBody.balls > 0) Render.text("against your own, the " + num2Text(player.lowerBody.balls + 2) + " orbs rolling around each other in their sweaty sacks", false);
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("against your " + vaginaDescript() + ", the sweaty orbs growing slick with the moisture of your growing need", false);
	else Render.text("against your taint, the sweaty skin pressing hard between your bodies", false);
	Render.text(".\n\n", false);
	
	Render.text("There's nothing to do but lie there and accept it.  You're too drunk to run even if you did get the aggressive shemale out of your rectum.  Worse still, her 'crew' of male lizans are standing guard in a ring around the pair of you.  Even though they seem focused on protecting against external threats, you know you'd never get past them without tripping over a tail or being snatched by a strong, sober arm. You belch and relax, your face sliding over the booze-slicked table while Roxanne starts to pound away at your stretched ", false);
	if(player.tailType > TAIL.NONE) Render.text("tail-hole", false);
	else Render.text("anus", false);
	Render.text(".  It begins to feel good", false);
	if(player.lowerBody.cockSpot.hasCock()) Render.text(", very good,", false);
	Render.text(" as she bumps and grinds against your deepest places, spurting drops of pent-up need from her swollen shaft.\n\n", false);
	
	Render.text("\"<i>Unf... you're so fucking tight, " + player.short + "!  Gods, I hate Ceraph's curse and this wretched world... but your ass... your gorgeous, cock-slurping asshole... it's divine,</i>\" praises Roxanne, smacking your ass-pillows in between her slow, rump-filling pumps.  She continues ranting with her thrusts, picking up the pace while she says, \"<i>So hot... my beautiful, drunken anal slut.  Look at you... you're soaked in your spilt beer, panting while I ream your rump.  Did you even want to win?  I bet you secretly wanted this, didn't you?  Go on, nod and tell me how much you want this.</i>\"\n\n", false);
	
	Render.text("You shake your head no, and are rewarded for your disobedience with a violent, butt-jiggling tail-whip.  Gasping from the pain, your " + assholeDescript() + " contracts involuntarily, milking Roxanne's member for a few more drops of pre.  The aggressive lizard really gets into it, her massive, DD-cup tits pinning you to the table while she smacks your cheeks with her flexible lizan tail.   Pounding and slapping you, she abuses you for every dick-milking squeeze you'll give her, ", false);
	if(!player.perks.has("Masochist")) Render.text("until you're voluntarily working your abdominal muscles to avoid the pain.", false);
	else Render.text("until you're moaning with delight from every butt-reddening strike.", false);
	Render.text("  Breaking under the onslaught, you mewl drunkenly, \"<i>Fuck my assh... plug me with your cum,</i>\" and wonder if you actually meant it.\n\n", false);
	
	//(DICKS:  
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("The constant pressure of her thrusts seems to flow straight to " + sMultiCockDesc() + " as if each of Roxanne's prostate-pounding bumps is injecting you with her lust.  ", false);
		if(player.lowerBody.cockSpot.count() == 1) Render.text("It bounces against your belly with your heartbeats, loving the forced anal pressure.  ", false);
		else Render.text("They bounce against your belly with your heartbeats, loving the anal pressure.  ", false);
		Render.text("Sticky droplets of pre-cum leak from you, and you can't stifle the pleasured gasps that burst from your maw.", false);
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  To your delight and shame, your pussy is equally aroused by the action, musky and wet from the pirate's lewd anal battering.", false);
		Render.text("\n\n", false);
	}
	//(JUST CUNTS:  
	else if(player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("The constant pressure of her thrusts seems to flow straight to your " + vaginaDescript() + ", inflaming your moistening labia.  You can feel her mass compressing your feminine organs, rubbing your inner walls against each other.  The sublime internal friction releases your natural lubricants, ", false);
		if(player.wetness() < 2) Render.text("sliming your ready vulva.", false);
		else if(player.wetness() <= 3) Render.text("soaking your ready vulva.", false);
		else if(player.wetness() <= 4) Render.text("which drip from your ready vulva.", false);
		else Render.text("which leak in a steady stream from your ready vulva.", false);
		Render.text("\n\n", false);
	}
	
	if(player.tallness <= 50) Render.text("Roxanne shifts forward, placing her tits around your head, letting the smooth scales of her mammoth breasts envelop you in their soft embrace.", false);
	else Render.text("Roxanne shifts forward slightly, heaving the smooth scales of her mammoth breasts a little further up your back.", false);
	Render.text("  She doubles her pace suddenly, balls slapping loudly into you while she moans and howls lustily.  You feel the telltale bulging of an impending orgasm in your gut, and her balls drag up your " + player.skinDesc + ", preparing to disgorge their steamy cargo.  The lizan grabs your hair and twists her fingers through it while she hilts you, screaming out her pleasure for all to hear.\n\n", false);
	
	Render.text("Your insides froth and churn while they're filled with pent-up lizan cum.  The shemale's balls bounce against you, violently relaxing and contracting in time with the bursts of cream filling your intestines.  Wetness fills you completely, but the spurts go on relentlessly.  Suddenly, small squirts of hot cum escape your tender " + assholeDescript() + ", running down Roxanne's still-pulsing balls.  The action smears the cummy lather everywhere, and while the discomfort of being so completely filled rises, the pleasure grows in equal measure until you're twitching and moaning with whorish delight.\n\n", false);
	
	Render.text("Finishing her obscene orgasm, Roxanne sighs, depositing one last creamy dollop into your anal seed-lake.  She coos in your ear, \"<i>That was nice...  I can see you shaking.  Did you enjoy being a loser that much?</i>\"  You nod meekly, blubbering out your embarassed agreement.  \"<i>How pathetic.  You'll probably be back here tomorrow to lose again, won't you?  Such a shameful butt-slut.</i>\"\n\n", false);
	
	Render.text("Roxanne tugs back, but even with the pressure of her reproductive leavings inside you, you keep her pinned inside your depths, squeezing and writhing on her shaft.  She tugs harder and harder, muscles going taut under her shiny purple scales until, with a violent 'POP', she slides free.  You climax in that moment, getting off while a river of cum pours from your gaping asshole.  Crying and moaning, you tremble while your beer-stained lips drool in bliss.", false);
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("  ", false);
		if(player.cumQ() >= 1500) {
			Render.text("A torrential outpouring of spunk pours from " + sMultiCockDesc() + ", leaking in a steady, submissive flow to grow a ", false);
			if(player.cumQ() <= 2500) Render.text("puddle", false);
			else Render.text("lake", false);
			Render.text("below the table.", false);
		}
		else Render.text("An outpouring of spunk drools from " + sMultiCockDesc() + ", spurting submissively under the table.", false);
	}
	if(player.biggestLactation() >= 2) Render.text("  Bursts of milk erupt from your pinned " + nippleDescript(0) +"s, blasting out while you cum to stain the table white.", false);
	Render.text("  As it winds down, you fall into slumber, snoring happily after your violation.\n\n", false);
	
	Render.text("<b>LATER...</b>\n", false);
	Render.text("You wake in the lizan's bed<b> with a nasty hangover</b>, her arm curled around your gurgling belly.  From how sore your rear feels, she kept 'winning' at least two or three more times.  Your head is pounding, your " + player.legs() + " are weak, and you dribble cum with every movement.  It takes some doing to extricate yourself from Roxanne's slumbering form, but you find your equipment and leave, hanging your head in shame under the leering eyes of the caravan-goers.", false);
...
private function bigBootyRoxanneContestLoss():void {
	...
	Render.text("[if (hasVagina = true) \"Meanwhile, your [vagina] gets slicker and slicker, your lust permeating the air with potent female musk.  Oh, if only Roxanne would fuck you there too!\"]");
	Render.text("  Roxanne grunts, \"<i>That's a good girl...");
	if (!player.lowerBody.vaginaSpot.hasVagina()) Render.text(" \"<i>Oh, I know you don't have a pussy, but tonight, you're my big-bootied, cum-dumpster bitch!</i>\"");
	else Render.text("\"<i>Oh, you'll make such a great anal cum-dumpster for me.  Your poor pussy will be so cum-starved, but you'll be so stuffed with jizz that you'll look pregnant anyhow!</i>\"");
