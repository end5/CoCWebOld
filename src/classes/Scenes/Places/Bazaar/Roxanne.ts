...
private function RoxanneChooseApproachOrRepeat():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00221] == 0) {
		...
		if(player.stats.cor > 66) outputText("Yeah, you've got some corruption in you, but I don't think you're contagious like the demons and their ilk yet.  ", false);
		...
		if(player.stats.cor < 33) outputText(", but would a champion really do something like that?", false);
...
private function roxanneDrinkingContest():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00225] >= 200) outputText("stumbles over her huge manhood, working towards", false);
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00222] + flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00223] <= 0 || flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00224] == 2) {
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00224] == 2) outputText(" after your last humiliation.", false);
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00222] + flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00223] < 3) outputText("hesitantly sniff at the brew, taking in its dark color and heady, hoppy aroma before", false);
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00226] == 0) {
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00225] >= 200) outputText("massive, seam-ripping bulge", false);
	...
	if(score < (45 + rand(20))) {
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00226] == 0) outputText("Nooooo! You're cut off! That means Roxanne won...", false);
...
private function roxanneGivesABlowjob():void {
	...
	if(player.lowerBody.balls > 0) outputText("A set of warm, long-nailed fingers squeeze your " + ballsDescriptLight() + ", dragging sharp nail-tips along the underside of your sack to tease the poor, cum-packed orbs.  ", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(", stopping just short of your female entrance", false);
	outputText(".\n\n", false);
...
private function roxanneCunnilingus():void {
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength < 16) outputText("dwarfs", false);
	else outputText("envelops", false);
	outputText(" the ", false);
	if(player.lowerBody.vaginaSpot.list[0].clitLength < 1) outputText("little pleasure buzzer", false);
	else if(player.lowerBody.vaginaSpot.list[0].clitLength < 3) outputText("swollen love-button", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(SMultiCockDesc() + " bounces on her head, between her horns, spilling a trail of sticky seed down her back.  ", false);
...
private function roxanneRimjob():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText(", your breasts cushioning you from the hard wood below", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Meanwhile, your neglected box is getting wetter and wetter, with no sign of stopping.  The lizan's massive, questing tongue ignores it.  ", false);
...
private function roxanneReamsYouNormal():void {
	...
	if(player.lowerBody.balls > 0) outputText("against your own, the " + num2Text(player.lowerBody.balls + 2) + " orbs rolling around each other in their sweaty sacks", false);
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("against your " + vaginaDescript() + ", the sweaty orbs growing slick with the moisture of your growing need", false);
	...
	if(player.tailType > TAIL.NONE) outputText("tail-hole", false);
	else outputText("anus", false);
	outputText(".  It begins to feel good", false);
	if(player.lowerBody.cockSpot.hasCock()) outputText(", very good,", false);
	...
	if(!player.perks.has("Masochist")) outputText("until you're voluntarily working your abdominal muscles to avoid the pain.", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.count() == 1) outputText("It bounces against your belly with your heartbeats, loving the forced anal pressure.  ", false);
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  To your delight and shame, your pussy is equally aroused by the action, musky and wet from the pirate's lewd anal battering.", false);
		outputText("\n\n", false);
	}
	//(JUST CUNTS:  
	else if(player.lowerBody.vaginaSpot.hasVagina()) {
...
private function roxanneFucksYourAssOHGODITSHUGE():void {
	...
	if(player.tailType > TAIL.NONE) outputText("She lifts your tail ", false);
	...
	if(player.skinType == SKIN.PLAIN) outputText(", leaving red lines in your unblemished skin", false);
	...
	if(player.skinType == SKIN.SCALES) outputText("a roughly-scratched 'X' on your scales", false);
	...
	if(player.lowerBody.balls > 0) outputText("own", false);
	...
	if(player.lowerBody.cockSpot.hasCock() && player.cumQ() >= 1000) outputText(", drooling spit into the huge puddle of cum you made on her bed", false);
	else if(player.lowerBody.vaginaSpot.hasVagina() && player.wetness() >= 4) outputText(", drooling spit into the sloppy puddy of fem-cum you splattered on the mattress", false);
	outputText(".", false);
...
private function bigBootyRoxanneContestLoss():void {
	...
	if (!player.lowerBody.vaginaSpot.hasVagina()) outputText(" \"<i>Oh, I know you don't have a pussy, but tonight, you're my big-bootied, cum-dumpster bitch!</i>\"");
