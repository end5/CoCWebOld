...
private function benoitKnocksUpPCCheck():void {
	...
	if ((player.pregnancyType == PregnancyType.OVIELIXIR_EGGS || player.perks.has("HarpyWomb") || player.perks.has("Oviposition") || player.perks.has("BasiliskWomb")) && (player.pregnancyIncubation == 0 || player.pregnancyType == PregnancyType.OVIELIXIR_EGGS)) {
		if (player.perks.has("BasiliskWomb") && flags[FlagEnum.BENOIT_TESTED_BASILISK_WOMB] == 1) {
			...
			if (player.pregnancyIncubation > 0) player.knockUpForce(PregnancyType.BENOIT, player.pregnancyIncubation);
		}
		else {
...
public function benoitKnockUp():boolean
{
	...
	let bounty:boolean = (player.perks.has("ElvenBounty")) ? true : false;
	let stud:boolean = (player.perks.has("MaraesGiftStud")) ? true : false;
	let alpha:boolean = (player.perks.has("FerasBoonAlpha")) ? true : false;
...
public function benoitIntro():void {
	...
	else if (benoitInClutch() && flags[FlagEnum.FEMOIT_READY_FOR_EGGS] == 0 && (flags[FlagEnum.BENOIT_STATUS] == 1 || flags[FlagEnum.BENOIT_STATUS] == 2))
	{
		...
		Render.text("As you enter the stall Benoite looks up; though her blind, milky eyes make her harder to read, she looks excited. \"<i>Ah! [name]; it iz good you are here... I had hope zat you would come for a vizit today.</i>\"");
	...
	else if (benoitPreggers() && flags[FlagEnum.FEMOIT_INCUBATION] < 0)
	{
		...
		{
			Render.text("When you enter the stall, you notice that Benoite's stomach is flat again. She gives you a toothy grin when you enter her stall. \"<i>I have good newz [name]! You are a fazer! " + String(flags[FlagEnum.FEMOIT_EGGS]) + " timez over in fact! Oh I wish zat you could have helped; but I was ztrong, ztrong enough get through it on my own.</i>\" she gushes, speaking at a mile-a-minute. She's clearly pleased with herself; and your virility.");
	...
	else if (!benoitInClutch() && !benoitPreggers() && (this.getGame().model.time.days - flags[FlagEnum.FEMOIT_NEXTDAY_EVENT] >= 30) && (flags[FlagEnum.BENOIT_STATUS] > 0))
	{
		if (flags[FlagEnum.FEMOIT_FIRST_CLUTCH_MISSED] == 0)
		{
			...
			Render.text("When you enter the stall, you are greeted by the smell of something cooking. Investigating further brings you to the blind basilisk's small kitchen, where she is busy frying something. Her nose preoccupied with her meal, she doesn't realize you're approaching until you touch her shoulder, yelping in shock.");
		...
		{
			Render.text("The flat-bellied basilisk is tucking away into a plate laden with a heavy omlette when you arrive.");
	...
	else
	{
		...
		if(benoitAffection() <= 10) Render.text("Once again, you carefully enter the gloom of Benoit's salvage shop.  The proprietor sniffs the air as you enter, and then looks at you sourly.  \"<i>Well?</i>\" he rasps.");
...
public function benoitsBuyMenu():void {
	...
	{
		buyMod = 1.66;
		Render.text("\"<i>Some may call zis junk,</i>\" says Benoite, indicating her latest wares.  \"<i>Me... I call it garbage.</i>\"");
	}
	else 
	{
		Render.text("\"<i>Some may call zis junk,</i>\" says Benoit, indicating his latest wares.  \"<i>Me... I call it garbage.</i>\"");
	}
...
private function benoitSellMenu():void {
	...
	if (flags[FlagEnum.BENOIT_EXPLAINED_SHOP] == 0)
		...
		Render.text("\"<i>Let us feel what you are trying to palm off upon me zis time, zen,</i>\" sighs Benoit" + benoitMF("", "e") + ", sitting down and opening " + benoitMF("his","her") + " hand to you.");
...
private function benoitTransactBuy(slot: number = 1):void {
	clearOutput();
	let itype:ItemType;
	let buyMod:number = 2;
	...
	if(player.stats.gems < int(buyMod * itype.value)) {
		Render.text("You consider making a purchase, but you lack the gems to go through with it.");
		doNext(benoitsBuyMenu);
		return;
	}
	if(benoitLover()) Render.text("After examining what you've picked out with " + benoitMF("his","her") + " fingers, " + benoitMF("Benoit","Benoite") + " hands it over and accepts your gems with a grin.");
	else Render.text("After examining what you've picked out with " + benoitMF("his", "her") + " fingers, " + benoitMF("Benoit","Benoite") + " hands it over, names the price and accepts your gems with a curt nod.\n\n");
	//(+3 Affection)
	benoitAffection(3);
...
private function benoitSellTransact(slot: number, sellMod: number):void {
	clearOutput();
	if (benoitLover()) 
		Render.text("Benoit" + benoitMF("", "e") + " gives your object the briefest of goings-over with " + benoitMF("his", "her") + " fingers before stowing it away and handing over your gem reward with a trusting smile.");
	else Render.text("Following a painstaking examination of what you've given him with his hands and nose, Benoit grudgingly accepts it and carefully counts out your reward.");
...
private function benoitSellAllTransact(totalItems: number, sellMod: number):void {
	clearOutput();
	...
	if (benoitLover()) 
		Render.text("Benoit" + benoitMF("", "e") + " gives your objects the briefest of goings-over with " + benoitMF("his", "her") + " fingers before stowing them away and handing over your " + num2Text(itemValue) + " gem reward with a trusting smile.");
	else Render.text("Following a painstaking examination of the items you've given him with his hands and nose, Benoit grudgingly accepts them and carefully counts out your " + num2Text(itemValue) + " gem reward.");
...
private function talkToBenoit():void {
	...
	if(((flags[FlagEnum.BENOIT_TIMES_SEXED_FEMPCS] > 2 && player.stats.int >= 60 && flags[FlagEnum.BENOIT_WOMB_TALK_UNLOCKED] == 0) || flags[FlagEnum.BENOIT_TIMES_SEXED_FEMPCS] == 2) && player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("You ask " + benoitMF("Benoit","Benoite") + " if " + benoitMF("he","she") + " has ever thought about trying to do something to help his people's plight.");
	...
	else if (flags[FlagEnum.FEMOIT_TALKED_TO] == 0 && flags[FlagEnum.BENOIT_STATUS] != 0)
	{
		...
		Render.text("You ask Benoite if she intends to go back to the mountains now.  She laughs long and hard at this.  One thing the transformation has certainly gifted her is an extraordinarily filthy laugh.");
	...
	//Subsequent Talk
		...
		/* BUILD ZE CHOICES!*/
		let choices:Array = [0,1,2,3,4,5,6,7,8];
		...
		if(benoitLover() && benoitMF("he","she") == "he" && player.lowerBody.vaginaSpot.hasVagina()) {
		...
		//(Randomly generated)
		if(choice == 0) {
			Render.text("You ask if all basilisks talk as " + benoitMF("he","she") + " does.");
			...
			Render.text("You ask " + benoitMF("Benoit","Benoite") + " about the dog.");
			...
			Render.text("You ask " + benoitMF("him","her") + " about the sign above the shop.");
			...
			Render.text("You ask if " + benoitMF("he","she") + " has always been blind.");
			...
			Render.text("You ask " + benoitMF("Benoit","Benoite") + " how " + benoitMF("he","she") + " got into this line of work.");
			...
			Render.text("You ask Benoit if " + benoitMF("he","she") + " can tell you anything about the Bazaar.");
			...
			Render.text("You ask if " + benoitMF("he","she") + "'s ever had any trouble with the demons who frequent the Bazaar.");
			...
			Render.text("You ask " + benoitMF("Benoit","Benoite") + " what results when basilisks mate with harpies.");
			...
			Render.text("You ask if " + benoitMF("Benoit","Benoite") + " really can tell who you are just by smell.");
			...
			//[(male Benoit only)
			if(benoitMF("he","she") == "he") Render.text("  There's a hint of longing jealousy in the basilisk's voice and when he lapses into silence you decide not to push the subject.");
		}
		...
		//(Randomly generated.  Added to normal talk options after PC has had sex with Benoit two times or more regardless of womb quest.)
			...
			Render.text("You ask Benoit there is anything useful he can tell you about the demon strongholds.");
			...
			Render.text("You ask Benoit if he can suggest anything to help you fight his brethren in the high mountains.");
			...
			Render.text("You ask Benoit if he really, <b>really</b> can tell who you are just by smell.");
		...
		{
			Render.text("You ask Benoite how she’s getting on with being the opposite sex.  Benoite stops cleaning the tarnished silver plate in her hands to think.");
		...
		{
			Render.text("You ask Benoite if she really can tell who you are just by smell.");
...
private function benoitAndFemPCTalkAboutEggings():void {
	...
	Render.text("Benoit seems very on edge today.  He paces up and down, returns your greeting with a stiff nod, and fiddles endlessly with the stock in reach of his counter as you search for a topic.  You ask if there's something wrong.");
...
private function eggySuggest():void {
	...
	if(flags[FlagEnum.BENOIT_TIMES_SEXED_FEMPCS] > 0) {
		if(player.isTaur()) {
			Render.text("You silently reach across the counter to squeeze his hands again.  Benoit grins with deep affection at you and, hand in hand, the two of you quietly debunk to the store room again.");
			...
			Render.text("You silently reach across the counter to squeeze his hands again.  Benoit beams at you and, hand in hand and without a word, the two of you depart to the store room again.");
	...
	if(player.isTaur()) {
		//Suggest: 
		Render.text("You reach your fingers across the counter and lightly touch Benoit's hands, saying you don't mind working out a few natural urges, if he's in the mood.");
		...
		if(player.gender == 3) {
			...
			if(player.longestCockLength() <= 10) Render.text("  Although this is evidently an uncanny experience for him, he does manage a cocky smile as his hand moves around your male sex. “Mine is bigger,” he teases.  Unable to reward his cheek from where he is, you simply snort and shuffle your hooves, impatient now with need.");
			//[cock  >10 inches: 
			else Render.text("  This is evidently an uncanny experience for him, the alien nature of it deepening as his hands moves around your male sex. “Oly Gods [name], you are a monster,” he says thickly.  You smile and stamp your hooves, impatient now with need.");
		}
		...
		//[Lactation: 
		if(player.lactationQ() >= 50) Render.text("  The slow, sensual sex is enough for you to begin to bead milk from your sensitive [nipples]; you moan as the flow intensifies until you are instinctively kneading your [chest], spurting your sweet, warm fluids onto the floor.  Benoit starts in genuine amazement at the sound.  “Mammals are so damn weird,” he mutters, making you giggle.");
		...
		if(player.gender == 3) Render.text("  [EachCock] thickens and spurts in sympathy to your female high, spattering the floor with white paint.");
	}
	else {
		Render.text("You reach your fingers across the counter and lightly touch Benoit's hands, saying you don't mind working out a few natural urges, if he's in the mood.");
	...
	//[Demon: 
		...
		if(player.inHeat) Render.text(" By now your vagina is practically gushing, your body's own deep-seated pheromone need stoked to blazing heights by the basilisk's gentle, painstaking exploration.  You cannot stop yourself thrusting against his soaked hand, announcing how badly you want this with heavy moans.");
		...
		if(player.lactationQ() > 100) {
			Render.text("  You moan as your breasts reward his attention with spurts of milk; he starts in genuine amazement when they ");
			if(!player.isTaur()) Render.text("hit him in the face");
			else Render.text("trickle over his fingers");
			Render.text(".  \"<i>Mammals are so damn weird,</i>\" he mutters, making you giggle.");
		}
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			if(!player.isTaur()) Render.text("  Stuck between your two burning bodies, y");
			else Render.text("Y");
			Render.text("our " + cockDescript(0) + " spurts in sympathy to your female high, soaking ");
			if(!player.isTaur()) Render.text("both of you");
			else Render.text("the dry dirt");
			Render.text(" with white.");
		}
	}
	...
	player.orgasm();
	if ((player.pregnancyType == PregnancyType.OVIELIXIR_EGGS || player.perks.has("HarpyWomb") || player.perks.has("Oviposition")) && (player.pregnancyIncubation == 0 || player.pregnancyType == PregnancyType.OVIELIXIR_EGGS)) {
		Render.text("  I would not inflict my children upon you.  Ere, take as much as you like.</i>\"");
		simpleChoices("Take It", takeBenoitsContraceptives, "", null, "", null, "", null, "Leave", dontTakeEggtraceptives);
	}
	else {
		Render.text("  I cannot give you babies unless you 'ave eggs.  I guess I should think a bit more before I go digging for things...</i>\"");
...
private function takeBenoitsContraceptives():void {
	clearOutput();
	Render.text("You gladly accept the herbal contraceptive and push it into your mouth, enjoying the pleasantly sharp, citrus flavour.");
...
private function dontTakeEggtraceptives():void {
	clearOutput();
	Render.text("You smile and say you don't mind carrying and laying a few basilisk eggs. \"<i>You... you don't?</i>\" he says hesitantly.  He faces you and for a moment looks like he's going to say something else; but then he shakes his head and puts the bag back into the drawer.");
...
private function firstTimeAfterBoningEncounterBenoit():void {
	...
	Render.text("A strange, faint noise is emanating from the salvage shop.  It's only when you duck cautiously into the stall proper that you realize it's the sound of a basilisk humming.  Benoit stops to sniff the air when you come in, immediately puts down the mug he is polishing, and beckons you inside.");
...
private function repeatSexWithBenoitLetHim():void {
	...
	if(player.isTaur()) {
		...
		if(player.isBiped()) Render.text("knees");
		else Render.text("lower body");
		Render.text(".  His hands descend upon your [butt]; grinning, you poke it outwards invitingly, sighing with affected frustration as the blind basilisk painstakingly lines up his next shot with your drooling, sticky sex.");
...
private function repeatBenoitFuckTakeCharge():void {
	...
	if(player.isTaur()) {
		...
		Render.text("\n\n");
		//[Small capacity:]
		if(player.vaginalCapacity() < 30) Render.text("You slide back down onto him, cooing this time as you feel him bottom out with several inches of him still outside of you.  His thin, long length could not be more perfect for your tight sex; he rubs your walls up and down as you dreamily thrust in and out of his body, sending waves of pleasure flowing through you.");
		//Large capacity: 
		else Render.text("You slide back down onto him, cooing this time as your groin meets his muscular thighs with a wet kiss, your sex swallowing his cock whole.  You begin to ride him hard and slow, bending his cock upwards to push at your sensitive walls, waves of pleasure starting to flow through you.");
		...
		Render.text("\n\nYour lust ratchets up as his warm hands continue to move over your behind, and at last losing your discipline you begin to thrust into him with abandon, eager now for your peak.  Benoit pants as you really begin to fuck him and thrusts with you; the wetness of your arousal spatters against his thighs as your body slaps into the basilisk's tight muscles.  He makes a harsh, bestial noise when he cums; his blind eyes roll as he clutches your back and shoots surge after surge of cum into your churning depths.  The warmth of his jizz and his helpless bucking thrust you to your own orgasm; irrepressible pulses of pleasure overwhelm your mind and you can do nothing for long minutes except cry and squeal against the basilisk, clutching him back as you work his dick for every last drop.");
		//[Herm: 
		if(player.lowerBody.cockSpot.hasCock()) Render.text("  [EachCock] thickens and spurts in sympathy to your female high, spattering the floor with white paint.");
		...
		//begin copypasta
		Render.text("His hands trail further down, moving into your inner thighs, exploring your soft flesh until he touches your lips.  You close your eyes and sigh as he slowly parts them with his smooth fingers and slides into your [vagina].  Although his breath is becoming increasingly heavy he also seems genuinely curious about you; with surprising gentleness his fingers travel over and around your moistening sex, exploring your every fold, working deeper and deeper as he does.  You let him know what pleases you by sighing when he touches a sweet spot, moving deliberately with his finger's motions so he may give them better attention.  He soon finds your [clit], beginning to bulge with need; slowly he circles it and then flicks at it, gently frigging you.");
		if(player.inHeat) Render.text("  By now your vagina is practically gushing, your body's own deep-seated pheromone need stoked to blazing heights by the basilisk's gentle, painstaking exploration.  You cannot stop yourself thrusting against his soaked hand, announcing how badly you want this with heavy moans.");
...
private function tryToConvertToBassyWomb():void {
	...
	if(!(player.hasItem(consumables.OVIELIX,2) && player.hasItem(consumables.REPTLUM) && player.hasItem(consumables.GOB_ALE))) {
		Render.text("You don't have the necessary ingredients to attempt this yet.  You recall " + benoitMF("Benoit","Benoite") + " mentioning that you would need Reptilum, two Ovi Elixirs, and Goblin Ale.");
		doNext(benoitIntro);
	}
	/*else if(player.isTaur()) {
		Render.text("\"<i>Forgive me, [name],</i>\" Benoit says, clearly troubled, as you begin hauling out the ingredients and announcing your plan.  \"<i>I sink your body is already stressed enough wis 'aving to pump so little blood so far... I would razer you not take furzer risks on my account until your form is more... compact.  I cannot be a part of zis... 'owever much I would like to.  You mean too much to me, you see.</i>\"");
	...
	//Ingredients in inventory: 
		...
		Render.text("You ferret out the ingredients you have collected and begin to bang them onto the counter in front of Benoit, telling him that you've got what he needs.  Pierre barks excitedly at the noise.");
		...
		//[No oviposition:
		if(!player.perks.has("Oviposition")) Render.text("  You feel slightly bloated, but otherwise fine; you sense that you can lay eggs of your own volition now.");
		else Render.text("You feel slightly bloated, but otherwise fine; you sense that were you to get impregnated by basilisk seed, the eggs you produce would be pure basilisk.");
		...
		//[Lust 30 or more: 
		if(player.lust >= 33) {
			Render.text("  You grin and say you're not sure it worked, but you suppose there's only one real way of finding out...");
			if(player.tallness <= 78 && !player.isTaur()) Render.text("  The basilisk is still for a moment, and then with a sudden surge of movement, grabs you by the waist and hoists you over his shoulder.  You squeal in mock terror as he hauls you as fast as he can into the back room, knocking over half his stock as he does.");
			else Render.text("  The basilisk is still for a moment, and then with a sudden surge of movement, grabs you by the waist and frenetically attempts to hoist you over his shoulder.  You are far too big for him though; after several valiant attempts, he collapses against a shelf.  Laughing, you pick the stricken, panting reptile up, hoist him over your own shoulder, and navigate a path into the back room.");
			doNext(createCallBackFunction(suggestSexAfterBasiWombed,false));
			return;
		}
		else Render.text("  You grin and say you're not even sure it worked... but you'll be back at some point to try it out, and he'd better be ready for when you do.  You gently pry yourself out of his grip and leave as deliberately as you can, aware of the beguiling, invisible scent you are leaving for the stunned, silent basilisk to simmer in.");
		doNext(camp.returnToCampUseOneHour);
	}
}	
...
private function suggestSexAfterBasiWombed(later:boolean = true):void {
	...
	if(later) {
		Render.text("You smile coquettishly and ask if he's interested in taking your new body for a... test drive.  ");
		//(6'6 or less: 
		if(player.tallness <= 78 && !player.isTaur()) Render.text("The basilisk is still for a moment, and then with a sudden surge of movement, grabs you by the waist and hoists you over his shoulder.  You squeal in mock terror as he hauls you as fast as he can into the back room, knocking over half his stock as he does.");
		else Render.text("The basilisk is still for a moment, and then with a sudden surge of movement, grabs you by the waist and frenetically attempts to hoist you over his shoulder.  You are far too big for him though; after several valiant attempts, he collapses against a shelf.  Laughing, you pick the stricken, panting reptile up, hoist him over your own shoulder, and navigate a path into the back room.");
		Render.text("\n\n");
	...
	if(!player.perks.has("Oviposition")) {
...
public function popOutBenoitEggs():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text(" and a downpour of semen");
	Render.text(".");
	...
	if(flags[FlagEnum.BENOIT_EGGS] > 0) Render.text(" once more");
	Render.text(".  Well, he will be when this ");
	if(flags[FlagEnum.BENOIT_EGGS] > 0) Render.text("latest ");
	Render.text("clutch hatches, anyway.");
...
public function femoitInitialTalk():void
{
	...
	Render.text("You ask Benoit if he has ever thought about trying to do something to help his people's plight.");
...
public function benoitFeminise():void
{
	...
	{
		Render.text("You don't have the necessary ingredients to attempt this yet.");
...
public function femoitNextDayEvent():void
{
	...
	Render.text("A strange, faint sound is emanating from the salvage shop.  It's only when you duck cautiously into the stall proper that you realize it's the sound of a basilisk humming.  Benoit stops to sniff the air when you come in, immediately puts down the mug she is polishing, and beckons you inside.");
...
{
	clearOutput();
	Render.text("You let her down as kindly as you can.");
...
public function femoitFirstTimeYes():void
{
	...
	clearOutput();
	Render.text("Smiling, you reach across the counter and squeeze Benoite's hands until her nervous babble dies out and she smiles back.  Still holding her hand, you move behind the crates and then gently lead her behind the stall's canopy.");
	...
	if (player.isLactating()) Render.text(" The pressure and arousal makes your [nipples] dribble milk, spattering fluids across both of your chests.");
	Render.text("  Soon the two of you are thrashing into each other, Benoite moaning huskily as you batter her against the wagon wall, both seeking your peak together, each pushing the other a bit further upwards.  Your world is lost to everything but the warm tightness around your [cock] and the pressure against your chest.");
...
{
	clearOutput();
	Render.text("Once again, you take Benoite's hand and lead her into the back room.  Your free hand roams underneath her apron as you carefully pick out a path through the junk and Benoite is swiftly in on the game too; her fingers slide downwards to pinch and fondle at her favorite parts of you.  By the time you make it into the storage space you are practically falling over each other, laughing as you fumble off each other's clothes.");
	...
	if (!benoitPreggers())
	{
		...
		if (benoitInClutch()) Render.text("  Her stomach bulges with clutch and the air is thick with female musk; the sight and smell of overwhelming ripeness speaks to your body in a way which bypasses your higher cognition entirely, sending you growling on top her, barely even hearing her giggle.");
		...
		Render.text("\n\n");
 		if (player.biggestCockLength() < 15) Render.text("Soon you are hilting yourself in her depths, making her gasp as you slap into her.");
 		else Render.text("Soon you are bottoming out in her, making her gasp as your hulking length spreads her wide.");
 		Render.text("  You quickly pick up the pace as you enter rut, thrusting into your basilisk lover with red-flecked abandon, her powerful thighs working with yours to make each return plunge into her warm depths more gratifying than the last.");
		
		Render.text("\n\n");
 		if (!player.isTaur())
		{
			Render.text("Eventually, sweat dripping off you, you grab her thighs and heave them upwards so that you can really go to town, drawing yourself almost all the way out of her before smacking back into her,");
			if (!benoitInClutch()) Render.text(" your stomach beating out a slapping rhythm against her own flat abdomen.  ");
			else Render.text(" your stomach beating out a slapping rhythm against her bulging, gravid abdomen.  ");
		}
 		Render.text("Benoite moans, squeals and eventually screams to your exertions, her fluids spurting and spattering against your groin");
 		if (player.lowerBody.balls > 0) Render.text(" and [balls].  You tumble over your peak as her cunt suddenly tightens around yours, sending surge after surge of cum into her fertile depths, your body seized in a rictus of pleasure.");
 		if (player.cumQ() >= 2500) Render.text("  The quantity of it is such that it quickly dribbles back out around your cock and pools on the floor.");
	...
	{
		clearOutput();
		Render.text("Once you are both in the usual spot, neither of you waste any time undressing.  The pregnant basilisk stands there, staring blindly at you, and waiting for you to make the first move, tongue occasionally flicking past her lips to nervously wet them.");
		...
		if (player.cumQ() >= 2500) Render.text("  Because her womb is already so jam-packed with eggs, the bulk of your deposit simply oozes messily back out of her, leaving her well and truly creampied.");
	...
	else if (benoitVeryHeavyPreggers() || benoitExtremePreggers())
	{
		...
		if (player.isTaur())
		{
			Render.text("\"<i>No.  Non!  Absolutely non!</i>\" Benoite insists.  \"<i>You are my amazing stallion, [name], but... it is simply too much.  Wis all zis weight on me, zere is simply no way we can do it.</i>\"");
		...
		else
		{
			...
			{
				Render.text("Benoite smirks at you.  \"<i>Well, I guess ze last time was enjoyable enough.  Come, then; I am horny and you arrived in time to scratch my itch.</i>\"");
			}
			...
			if (player.lowerBody.balls == 0) Render.text(" with a smile, knowing damn well you don't, literally");
			Render.text(".");
...
{
	clearOutput();
	Render.text("As you enter Benoite's stall, you hear the sounds of gasps of pain from the backroom, mingling with the sounds of stock being knocked around. You race through and find Benoite, completely naked, leaning against a table and groaning in anguish, her tail slashing wildly through the air behind her.  \"<i>[name]!  It.. it iz good zat you are here.  Ze eggs!  Zey come!</i>\"");
...
// (First time: 
	...
	else
	{
		...
		if (benoitVeryHeavyPreggers() || benoitExtremePreggers()) Render.text("... which is good, because you can tell there're a lot of them coming");
		Render.text(".  That done, you squat down behind Benoite, keeping your head down to avoid having it slapped by her swishing tail, and reach up between her legs. She promptly lets out a shriek.");

		if (flags[FlagEnum.FEMOIT_HELPED_LAY] == 1)
		{
			Render.text("\"<i>What's the matter?</i>\" you blurt, alarmed.");

			Render.text("\"<i>You... you 'ave cold 'ands!</i>\" she squeals, shuddering in displeasure.");
		}
		else
		{
			Render.text("\"<i>What have I told you about warming zose hands?</i>\" she snaps angrily.");
		}
	}
	flags[FlagEnum.FEMOIT_HELPED_LAY]++;
		...
		if (silly()) Render.text(".  It's shaped like a complex rhomboidal polygon with 15 sides.");
		else Render.text(", the smoothly curved peak of a jade egg beginning to crest.");
		Render.text("  With a strangled cry of orgasm tinged with pained relief, Benoite pushes it from her passage into your hands. Slick with juices, the egg makes for quite a handful and you find yourself struggling to safely cradle it within your grasp. You quickly place it into the makeshift next at her feet.");
		...
		if (flags[FlagEnum.FEMOIT_EGGS] > 1) Render.text("s");
		Render.text(" she just laid.  You nod from behind her, and cautiously move around to give her a hug.  She is a very, very brave woman, and an even braver man for making the decision to do this in the first place.");
		...
		if (benoitRegularPreggers()) Render.text(" She sighs softly.  \"<i>A small clutch, this is... normal for my people, yes, but not very good for my mission, is it?  Still, a small step to freedom is still a step.</i>\"  She manages to smile.  \"<i>My children will be free, and that is something to celebrate.</i>\"");
		else if (benoitHeavyPreggers()) Render.text(" \"<i>My, a nice big clutch we had together, didn't we?</i>\" she smiles, proudly.  \"<i>Yes, these will be strong children, I zink.</i>\"");
		else if (benoitVeryHeavyPreggers()) Render.text(" \"<i>...Wow, you're quite ze stud, aren't you?</i>\" Benoite says, giving a throaty growl of lust.  \"<i>I chose well when I decided to let you fertilize my eggs... so many eggs, too.</i>\"");
		else if (benoitExtremePreggers()) Render.text(" The basilisk stops after her counting, visibly stunned.  She recounts her eggs again, and then again, shaking her head in amazement.  \"<i>Incredible... simply, incredible.  I didn't think it was possible for one woman to lay zo many eggs!  Zut alors!</i>\"  A sudden look of horrified realization washes over her features.  \"<i>I'm going to be run off my feet with all zese little monsters, aren't I?</i>\" she murmurs to herself.");
		...
		if (flags[FlagEnum.FEMOIT_EGGS] > 1) Render.text("s");
		Render.text(" away safely, though, and the blind reptilian clearly appreciates the help.  Leaving her to admire her new clutch you head back to camp.");
