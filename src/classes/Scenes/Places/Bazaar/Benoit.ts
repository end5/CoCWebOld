...
public function benoitIntro():void {
	...
	else if (benoitPreggers() && flags[FlagEnum.FEMOIT_INCUBATION] < 0)
	{
		...
		{
			outputText("When you enter the stall, you notice that Benoite's stomach is flat again. She gives you a toothy grin when you enter her stall. \"<i>I have good newz [name]! You are a fazer! " + String(flags[FlagEnum.FEMOIT_EGGS]) + " timez over in fact! Oh I wish zat you could have helped; but I was ztrong, ztrong enough get through it on my own.</i>\" she gushes, speaking at a mile-a-minute. She's clearly pleased with herself; and your virility.");
...
private function repeatSexWithBenoitLetHim():void {
	...
	if(player.isTaur()) {
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("and [fullChest] ");
...
private function repeatBenoitFuckTakeCharge():void {
	...
	if(player.isTaur()) {
		...
		outputText("\n\nYour lust ratchets up as his warm hands continue to move over your behind, and at last losing your discipline you begin to thrust into him with abandon, eager now for your peak.  Benoit pants as you really begin to fuck him and thrusts with you; the wetness of your arousal spatters against his thighs as your body slaps into the basilisk's tight muscles.  He makes a harsh, bestial noise when he cums; his blind eyes roll as he clutches your back and shoots surge after surge of cum into your churning depths.  The warmth of his jizz and his helpless bucking thrust you to your own orgasm; irrepressible pulses of pleasure overwhelm your mind and you can do nothing for long minutes except cry and squeal against the basilisk, clutching him back as you work his dick for every last drop.");
		//[Herm: 
		if(player.lowerBody.cockSpot.hasCock()) outputText("  [EachCock] thickens and spurts in sympathy to your female high, spattering the floor with white paint.");
...
private function tryToConvertToBassyWomb():void {
	...
	//Ingredients in inventory: 
		...
		//[No oviposition:
		if(!player.perks.has("Oviposition")) outputText("  You feel slightly bloated, but otherwise fine; you sense that you can lay eggs of your own volition now.");
...
private function suggestSexAfterBasiWombed(later:boolean = true):void {
	...
	if(!player.isTaur()) {
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("  He deliberately rubs himself up and down your body, and the [cock] trapped between your warm bodies quickly hardens against the warm, smooth friction.");
		...
		//[Herm:
		if(player.lowerBody.cockSpot.hasCock()) outputText("  He deliberately moves his hand over [oneCock] before trapping it in his warm grasp.   It quickly hardens against his warm, smooth friction.");
...
public function popOutBenoitEggs():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("and [eachCock] starting to leak pre-cum ");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" and a downpour of semen");
	outputText(".");
	...
	if(flags[FlagEnum.BENOIT_EGGS] > 0) outputText(" once more");
	outputText(".  Well, he will be when this ");
	if(flags[FlagEnum.BENOIT_EGGS] > 0) outputText("latest ");
...
public function femoitFirstTimeYes():void
{
	...
	if (player.biggestCockLength() < 15 && player.lowerBody.balls == 0) outputText("  Eventually you manage to hilt yourself entirely in her depths, your stomach pressing against her own tight belly.");
	else if (player.biggestCockLength() < 15 && player.lowerBody.balls > 0) outputText("  Eventually you manage to hilt yourself in her depths, your [balls] pressing into her sex as your stomach bumps into her own tight belly.");
	else if (player.biggestCockLength() >= 15 && player.lowerBody.balls == 0) outputText("  Eventually you manage to bottom out, your dick pressed against her cervix.");
	else if (player.biggestCockLength() >= 15 && player.lowerBody.balls > 0) outputText("  Eventually you manage to bottom out, your dick pressed against her cervix, your [balls] swinging heavily below your shaft.");
...
public function femoitSexIntro():void
{
	...
	if (!benoitPreggers())
	{
 		...
 		if (!player.isTaur())
		{
 		...
 		if (player.lowerBody.balls > 0) outputText(" and [balls].  You tumble over your peak as her cunt suddenly tightens around yours, sending surge after surge of cum into her fertile depths, your body seized in a rictus of pleasure.");
	...
	else if (benoitVeryHeavyPreggers() || benoitExtremePreggers())
	{
		...
		else
		{
			...
			if (player.lowerBody.balls == 0) outputText(" with a smile, knowing damn well you don't, literally");
			outputText(".");
...
// (First time: 
	...
	flags[FlagEnum.FEMOIT_HELPED_LAY]++;
		...
		if (flags[FlagEnum.FEMOIT_EGGS] > 1) outputText("s");
