...
public benoitIntro():void {
	...
	else if (!benoitInClutch() && !benoitPreggers() && (this.Game.model.time.days - Flags.list[FlagEnum.FEMOIT_NEXTDAY_EVENT] >= 30) && (Flags.list[FlagEnum.BENOIT_STATUS] > 0))
	{
		...
		{
			MainScreen.text("The flat-bellied basilisk is tucking away into a plate laden with a heavy omlette when you arrive.");
...
private benoitTransactBuy(slot: number = 1):void {
	...
	if(benoitLover()) MainScreen.text("After examining what you've picked out with " + benoitMF("his","her") + " fingers, " + benoitMF("Benoit","Benoite") + " hands it over and accepts your gems with a grin.");
	else MainScreen.text("After examining what you've picked out with " + benoitMF("his", "her") + " fingers, " + benoitMF("Benoit","Benoite") + " hands it over, names the price and accepts your gems with a curt nod.\n\n");
	//(+3 Affection)
	benoitAffection(3);
...
private benoitSellTransact(slot: number, sellMod: number):void {
	MainScreen.clearText();
	if (benoitLover()) 
		MainScreen.text("Benoit" + benoitMF("", "e") + " gives your object the briefest of goings-over with " + benoitMF("his", "her") + " fingers before stowing it away and handing over your gem reward with a trusting smile.");
	else MainScreen.text("Following a painstaking examination of what you've given him with his hands and nose, Benoit grudgingly accepts it and carefully counts out your reward.");
...
private benoitSellAllTransact(totalItems: number, sellMod: number):void {
	...
	if (benoitLover()) 
		MainScreen.text("Benoit" + benoitMF("", "e") + " gives your objects the briefest of goings-over with " + benoitMF("his", "her") + " fingers before stowing them away and handing over your " + num2Text(itemValue) + " gem reward with a trusting smile.");
	else MainScreen.text("Following a painstaking examination of the items you've given him with his hands and nose, Benoit grudgingly accepts them and carefully counts out your " + num2Text(itemValue) + " gem reward.");
...
private talkToBenoit():void {
	...
	//Subsequent Talk
		...
		//(Randomly generated)
		if(choice == 0) {
			MainScreen.text("You ask if all basilisks talk as " + benoitMF("he","she") + " does.");
			...
			MainScreen.text("You ask " + benoitMF("Benoit","Benoite") + " about the dog.");
			...
			MainScreen.text("You ask " + benoitMF("him","her") + " about the sign above the shop.");
			...
			MainScreen.text("You ask if " + benoitMF("he","she") + " has always been blind.");
			...
			MainScreen.text("You ask if " + benoitMF("he","she") + "'s ever had any trouble with the demons who frequent the Bazaar.");
			...
			MainScreen.text("You ask " + benoitMF("Benoit","Benoite") + " what results when basilisks mate with harpies.");
			...
			MainScreen.text("You ask if " + benoitMF("Benoit","Benoite") + " really can tell who you are just by smell.");
			...
			//[(male Benoit only)
			if(benoitMF("he","she") == "he") MainScreen.text("  There's a hint of longing jealousy in the basilisk's voice and when he lapses into silence you decide not to push the subject.");
		}
		...
		//(Randomly generated.  Added to normal talk options after PC has had sex with Benoit two times or more regardless of womb quest.)
			...
			MainScreen.text("You ask Benoit there is anything useful he can tell you about the demon strongholds.");
			...
			MainScreen.text("You ask Benoit if he can suggest anything to help you fight his brethren in the high mountains.");
			...
			MainScreen.text("You ask Benoit if he really, <b>really</b> can tell who you are just by smell.");
		...
		{
			MainScreen.text("You ask Benoite how she’s getting on with being the opposite sex.  Benoite stops cleaning the tarnished silver plate in her hands to think.");
		...
		{
			MainScreen.text("You ask Benoite if she really can tell who you are just by smell.");
...
private eggySuggest():void {
	...
	if(Flags.list[FlagEnum.BENOIT_TIMES_SEXED_FEMPCS] > 0) {
		if(player.lowerBody.isTaur()) {
			...
			MainScreen.text("You silently reach across the counter to squeeze his hands again.  Benoit beams at you and, hand in hand and without a word, the two of you depart to the store room again.");
	...
	if(player.lowerBody.isTaur()) {
		...
		if(player.gender == 3) {
			...
			if(player.longestCockLength() <= 10) MainScreen.text("  Although this is evidently an uncanny experience for him, he does manage a cocky smile as his hand moves around your male sex. “Mine is bigger,” he teases.  Unable to reward his cheek from where he is, you simply snort and shuffle your hooves, impatient now with need.");
			//[cock  >10 inches: 
			else MainScreen.text("  This is evidently an uncanny experience for him, the alien nature of it deepening as his hands moves around your male sex. “Oly Gods [name], you are a monster,” he says thickly.  You smile and stamp your hooves, impatient now with need.");
		}
		...
		//[Lactation: 
		if(player.lactationQ() >= 50) MainScreen.text("  The slow, sensual sex is enough for you to begin to bead milk from your sensitive [nipples]; you moan as the flow intensifies until you are instinctively kneading your [chest], spurting your sweet, warm fluids onto the floor.  Benoit starts in genuine amazement at the sound.  “Mammals are so damn weird,” he mutters, making you giggle.");
		...
		if(player.gender == 3) MainScreen.text("  [EachCock] thickens and spurts in sympathy to your female high, spattering the floor with white paint.");
	}
	else {
		MainScreen.text("You reach your fingers across the counter and lightly touch Benoit's hands, saying you don't mind working out a few natural urges, if he's in the mood.");
	...
	if ((player.pregnancyType == PregnancyType.OVIELIXIR_EGGS || player.perks.has("HarpyWomb") || player.perks.has("Oviposition")) && (player.pregnancyIncubation == 0 || player.pregnancyType == PregnancyType.OVIELIXIR_EGGS)) {
		MainScreen.text("  I would not inflict my children upon you.  Ere, take as much as you like.</i>\"");
		simpleChoices("Take It", takeBenoitsContraceptives, "", null, "", null, "", null, "Leave", dontTakeEggtraceptives);
	}
	else {
		MainScreen.text("  I cannot give you babies unless you 'ave eggs.  I guess I should think a bit more before I go digging for things...</i>\"");
...
private takeBenoitsContraceptives():void {
	MainScreen.clearText();
	MainScreen.text("You gladly accept the herbal contraceptive and push it into your mouth, enjoying the pleasantly sharp, citrus flavour.");
...
private dontTakeEggtraceptives():void {
	MainScreen.clearText();
	MainScreen.text("You smile and say you don't mind carrying and laying a few basilisk eggs. \"<i>You... you don't?</i>\" he says hesitantly.  He faces you and for a moment looks like he's going to say something else; but then he shakes his head and puts the bag back into the drawer.");
...
private repeatSexWithBenoitLetHim():void {
	...
	if(player.lowerBody.isTaur()) {
		...
		if(player.isBiped()) MainScreen.text("knees");
		else MainScreen.text("lower body");
		MainScreen.text(".  His hands descend upon your [butt]; grinning, you poke it outwards invitingly, sighing with affected frustration as the blind basilisk painstakingly lines up his next shot with your drooling, sticky sex.");
...
private repeatBenoitFuckTakeCharge():void {
	...
	if(player.lowerBody.isTaur()) {
		...
		MainScreen.text("\n\n");
		//[Small capacity:]
		if(player.vaginalCapacity() < 30) MainScreen.text("You slide back down onto him, cooing this time as you feel him bottom out with several inches of him still outside of you.  His thin, long length could not be more perfect for your tight sex; he rubs your walls up and down as you dreamily thrust in and out of his body, sending waves of pleasure flowing through you.");
		//Large capacity: 
		else MainScreen.text("You slide back down onto him, cooing this time as your groin meets his muscular thighs with a wet kiss, your sex swallowing his cock whole.  You begin to ride him hard and slow, bending his cock upwards to push at your sensitive walls, waves of pleasure starting to flow through you.");
		...
		MainScreen.text("\n\nYour lust ratchets up as his warm hands continue to move over your behind, and at last losing your discipline you begin to thrust into him with abandon, eager now for your peak.  Benoit pants as you really begin to fuck him and thrusts with you; the wetness of your arousal spatters against his thighs as your body slaps into the basilisk's tight muscles.  He makes a harsh, bestial noise when he cums; his blind eyes roll as he clutches your back and shoots surge after surge of cum into your churning depths.  The warmth of his jizz and his helpless bucking thrust you to your own orgasm; irrepressible pulses of pleasure overwhelm your mind and you can do nothing for long minutes except cry and squeal against the basilisk, clutching him back as you work his dick for every last drop.");
		//[Herm: 
		if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("  [EachCock] thickens and spurts in sympathy to your female high, spattering the floor with white paint.");
		...
		//begin copypasta
		MainScreen.text("His hands trail further down, moving into your inner thighs, exploring your soft flesh until he touches your lips.  You close your eyes and sigh as he slowly parts them with his smooth fingers and slides into your [vagina].  Although his breath is becoming increasingly heavy he also seems genuinely curious about you; with surprising gentleness his fingers travel over and around your moistening sex, exploring your every fold, working deeper and deeper as he does.  You let him know what pleases you by sighing when he touches a sweet spot, moving deliberately with his finger's motions so he may give them better attention.  He soon finds your [clit], beginning to bulge with need; slowly he circles it and then flicks at it, gently frigging you.");
		if(player.inHeat) MainScreen.text("  By now your vagina is practically gushing, your body's own deep-seated pheromone need stoked to blazing heights by the basilisk's gentle, painstaking exploration.  You cannot stop yourself thrusting against his soaked hand, announcing how badly you want this with heavy moans.");
...
private tryToConvertToBassyWomb():void {
	...
	//Ingredients in inventory: 
		...
		//[No oviposition:
		if(!player.perks.has("Oviposition")) MainScreen.text("  You feel slightly bloated, but otherwise fine; you sense that you can lay eggs of your own volition now.");
		else MainScreen.text("You feel slightly bloated, but otherwise fine; you sense that were you to get impregnated by basilisk seed, the eggs you produce would be pure basilisk.");
		...
		else MainScreen.text("  You grin and say you're not even sure it worked... but you'll be back at some point to try it out, and he'd better be ready for when you do.  You gently pry yourself out of his grip and leave as deliberately as you can, aware of the beguiling, invisible scent you are leaving for the stunned, silent basilisk to simmer in.");
		doNext(camp.returnToCampUseOneHour);
	}
}	
...
private suggestSexAfterBasiWombed(later:boolean = true):void {
	...
	if(!player.lowerBody.isTaur()) {
		...
		if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("  He deliberately rubs himself up and down your body, and the [cock] trapped between your warm bodies quickly hardens against the warm, smooth friction.");
...
public popOutBenoitEggs():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text(" and a downpour of semen");
	MainScreen.text(".");
...
public femoitInitialTalk():void
{
	...
	MainScreen.text("You ask Benoit if he has ever thought about trying to do something to help his people's plight.");
...
{
	MainScreen.clearText();
	MainScreen.text("You let her down as kindly as you can.");
...
public femoitFirstTimeYes():void
{
	...
	if (player.isLactating()) MainScreen.text(" The pressure and arousal makes your [nipples] dribble milk, spattering fluids across both of your chests.");
	MainScreen.text("  Soon the two of you are thrashing into each other, Benoite moaning huskily as you batter her against the wagon wall, both seeking your peak together, each pushing the other a bit further upwards.  Your world is lost to everything but the warm tightness around your [cock] and the pressure against your chest.");
...
{
	MainScreen.clearText();
	MainScreen.text("Once again, you take Benoite's hand and lead her into the back room.  Your free hand roams underneath her apron as you carefully pick out a path through the junk and Benoite is swiftly in on the game too; her fingers slide downwards to pinch and fondle at her favorite parts of you.  By the time you make it into the storage space you are practically falling over each other, laughing as you fumble off each other's clothes.");
	...
	if (!benoitPreggers())
	{
		...
		if (benoitInClutch()) MainScreen.text("  Her stomach bulges with clutch and the air is thick with female musk; the sight and smell of overwhelming ripeness speaks to your body in a way which bypasses your higher cognition entirely, sending you growling on top her, barely even hearing her giggle.");
		...
		MainScreen.text("\n\n");
 		if (player.biggestCockLength() < 15) MainScreen.text("Soon you are hilting yourself in her depths, making her gasp as you slap into her.");
 		else MainScreen.text("Soon you are bottoming out in her, making her gasp as your hulking length spreads her wide.");
 		...
 		if (!player.lowerBody.isTaur())
		{
			...
			else MainScreen.text(" your stomach beating out a slapping rhythm against her bulging, gravid abdomen.  ");
		}
 		MainScreen.text("Benoite moans, squeals and eventually screams to your exertions, her fluids spurting and spattering against your groin");
 		if (player.lowerBody.balls > 0) MainScreen.text(" and [balls].  You tumble over your peak as her cunt suddenly tightens around yours, sending surge after surge of cum into her fertile depths, your body seized in a rictus of pleasure.");
 		if (player.cumQ() >= 2500) MainScreen.text("  The quantity of it is such that it quickly dribbles back out around your cock and pools on the floor.");
	...
	else if (benoitRegularPreggers() && (!player.lowerBody.isTaur() || (player.lowerBody.isTaur() && (player.tallness * (5/6) < player.lowerBody.cockSpot.list[player.longestCock()].cockLength))))
	{
		...
		if (player.cumQ() >= 2500) MainScreen.text("  Because her womb is already so jam-packed with eggs, the bulk of your deposit simply oozes messily back out of her, leaving her well and truly creampied.");
	...
	else if (benoitVeryHeavyPreggers() || benoitExtremePreggers())
	{
		...
		else
		{
			...
			{
				MainScreen.text("Benoite smirks at you.  \"<i>Well, I guess ze last time was enjoyable enough.  Come, then; I am horny and you arrived in time to scratch my itch.</i>\"");
			}
			...
			if (player.lowerBody.balls == 0) MainScreen.text(" with a smile, knowing damn well you don't, literally");
			MainScreen.text(".");
...
{
	MainScreen.clearText();
	MainScreen.text("As you enter Benoite's stall, you hear the sounds of gasps of pain from the backroom, mingling with the sounds of stock being knocked around. You race through and find Benoite, completely naked, leaning against a table and groaning in anguish, her tail slashing wildly through the air behind her.  \"<i>[name]!  It.. it iz good zat you are here.  Ze eggs!  Zey come!</i>\"");
...
// (First time: 
	...
	else
	{
		...
		if (Flags.list[FlagEnum.FEMOIT_HELPED_LAY] == 1)
		{
			...
			MainScreen.text("\"<i>You... you 'ave cold 'ands!</i>\" she squeals, shuddering in displeasure.");
		}
		else
		{
	...
	Flags.list[FlagEnum.FEMOIT_HELPED_LAY]++;
		...
		if (silly()) MainScreen.text(".  It's shaped like a complex rhomboidal polygon with 15 sides.");
		...
		if (benoitRegularPreggers()) MainScreen.text(" She sighs softly.  \"<i>A small clutch, this is... normal for my people, yes, but not very good for my mission, is it?  Still, a small step to freedom is still a step.</i>\"  She manages to smile.  \"<i>My children will be free, and that is something to celebrate.</i>\"");
		else if (benoitHeavyPreggers()) MainScreen.text(" \"<i>My, a nice big clutch we had together, didn't we?</i>\" she smiles, proudly.  \"<i>Yes, these will be strong children, I zink.</i>\"");
		else if (benoitVeryHeavyPreggers()) MainScreen.text(" \"<i>...Wow, you're quite ze stud, aren't you?</i>\" Benoite says, giving a throaty growl of lust.  \"<i>I chose well when I decided to let you fertilize my eggs... so many eggs, too.</i>\"");
		else if (benoitExtremePreggers()) MainScreen.text(" The basilisk stops after her counting, visibly stunned.  She recounts her eggs again, and then again, shaking her head in amazement.  \"<i>Incredible... simply, incredible.  I didn't think it was possible for one woman to lay zo many eggs!  Zut alors!</i>\"  A sudden look of horrified realization washes over her features.  \"<i>I'm going to be run off my feet with all zese little monsters, aren't I?</i>\" she murmurs to herself.");
