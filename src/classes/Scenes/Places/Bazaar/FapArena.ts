...
public function fapArenaGOOOO():void {
	...
	if(player.longestCockLength() < 8) {
		...
		if(!player.lowerBody.cockSpot.hasCock()) outputText("you have a cock.", false);
	...
	//[if dick size >= 8 inches: 1st time] 
		...
		//[if Corr >40]
		if(player.stats.cor > 66) outputText("Sounds fun!", false);
		//[else]
	...
	//[if dick size > 8 inches: after first time] 
		...
		outputText("Everyone nods in agreement, hands and genitalia quivering in excitement.", false);
		//[if Corr >40]
		if(player.stats.cor > 66) outputText("  You look forward to this new masturbating session.", false);
...
private function fapArenaPageII():void {
	...
	if(player.stats.cor > 66) outputText("You eagerly seize the rod on your right side; squeezing it a bit in impatience.  A  little tingle alerts you to your left neighbor grabbing " + oMultiCockDesc() + " with an expert hand.  Everyone around you quickly grabs hold of the nearest cock on their right side.  Some of them are trembling because of the induced stress and arousal.\n\n", false);
	else if(player.stats.cor > 33) outputText("You nervously grab hold of the rod on your right side.  In your stress, you can't help but give it a little squeeze.  You tremble as your left neighbor grabs " + oMultiCockDesc() + " with an expert hand.  Most people are as nervous as you, their whole naked body trembling in stress.\n\n", false);
	else {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("other ", false);
...
private function fapResults(place:number = 3):void {
	...
	if(place == 3) {
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("  Several more gouts follow suit from the unstimulated dickflesh dangling from your groin.", false);
	...
	//[if first time]
	if(flags[FlagEnum.FAP_ARENA_RULES_EXPLAINED] == 1) outputText("You stand here, a little lost, wondering what will happen next.", false);
	...
	//[if corr < 50]
	if(player.stats.cor < 50) outputText("You find yourself yelling with them, astounded by everyone's enthusiasm and eagerness to be fucked in the ass, even after this rough masturbating session.  You wish you weren't so blindly following others, but they are so hypnotizing, their hard glistening cocks pointing at the air as they keep shouting over and over again, \"<i>BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!</i>\"  ", false);
	//[else, corr >50]
	...
	//[if you lost]
		...
		//([if corr < 50]
		if(player.stats.cor < 50) outputText("a whorish smile", false);
	...
	if(place <= 1) {
		...
		if((player.lowerBody.cockSpot.count() > 2) || (!tent && player.lowerBody.cockSpot.count() > 1)) {
			if(player.lowerBody.cockSpot.count() > 3 || (!tent && player.lowerBody.cockSpot.count() > 2)) outputText("  Your other cocks also spill ", false);
			if((tent && player.lowerBody.cockSpot.count() == 3) || (player.lowerBody.cockSpot.count() == 2 && !tent)) outputText("  Your other cock also spills ", false);
			outputText("a ", false);
			...
			if(player.lowerBody.cockSpot.count() > 3 || (!tent && player.lowerBody.cockSpot.count() > 2)) outputText("their ", false);
	...
	//[else, if you lost]
		...
		if(player.stats.lib > 50) outputText("  Maybe losing wasn't that bad? You feel like you could lose a few more times...", false);
		outputText("\n\n", false);
	}
