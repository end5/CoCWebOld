...
public fapArenaGOOOO():void {
	...
	//[1st time]
	if(Flags.list[FlagEnum.FAP_ARENA_RULES_EXPLAINED] == 0) MainScreen.text("As you approach the tent, you notice these people are actually waiting in line, pleasantly chatting with each other.  All varieties of species are there, from cute little goblins and mouse-faced people with rodent-like teeth, to towering centaurs and minotaurs, the latter easily noticed with their strong scent.   You spot the occasional sharp-toothed dog-morph or shark breeder, some traditional incubi and a few tentacled mutants, victims of consuming corrupted vegetation.  There are even a few humans, nonchalantly standing in the waiting line and making conversation, as if some of the others weren't monstrosities.  What strikes you is everyone's perfect civility.  Obviously everyone is used to the place.\n\n", false);

	MainScreen.text("The waiting line advances and you finally end up in front of the tent's entrance guarded by a surprisingly tall goblin.  She looks at you with assessing eyes, suddenly grabs your crotch with her left hand, rubs it for a second, and says, ", false);
	//[if dick size < 8 inches] 
	if(player.longestCockLength() < 8) {
		MainScreen.text("\"<i>I'm afraid you aren't well-endowed enough to take part in our sessions.  Please come back when ", false);
		if(!player.lowerBody.cockSpot.hasCock()) MainScreen.text("you have a cock.", false);
		else MainScreen.text("your cock is sufficiently long.", false);
		MainScreen.text("</i>\"", false);
		//THE FOLLOWING IS CUT - I AINT SELLING GRO+ HERE! NOAP
		//, or consider buying products in our local shop.</i>\"\n\n", false);
		//MainScreen.text("She redirects you to a small stall near the entrance with various potions stored on shelves.  A poster placed on it reads: "Your dick must qualify to the house's standards.  80 gems for a vial of Gro+, 80 for the session of your life!"
	...
	//[if dick size > 8 inches: after first time] 
		...
		MainScreen.text("\"<i>Welcome back.  Please undress yourself in the backroom and wait with the others in the main room.</i>\"\n\n", false);
		MainScreen.text("You do as you are told, quickly removing your " + player.inventory.armor.displayName + " and placing your naked self in the main room, waiting in circle with the others.  You have a pleasant and casual chat with other people, trying to recognize some faces.  You try your best to place yourself between people you don't know yet, in order to feel varied pleasures from one time to the next.  After a few minutes during which " + CockDescriptor.describeMultiCockSimpleOne(player) + " stands fully erect at the sight of so many naked people, the sexy referee walks in, naked as usual.\n\n", false);
		
		MainScreen.text("\"<i>Alright.  Welcome everyone; I see no new faces here, so I'll assume everyone knows the rules and we'll directly proceed to the masturbatory game, which I am sure you are all eager to play.  Remember: using alternative methods of stimulation is NOT allowed.  Do you studs understand?</i>\"\n\n", false);
		
		MainScreen.text("Everyone nods in agreement, hands and genitalia quivering in excitement.", false);
		//[if Corr >40]
		if(player.stats.cor > 66) MainScreen.text("  You look forward to this new masturbating session.", false);
...
private fapArenaPageII():void {
	...
	MainScreen.text("\"<i>READY!</i>\"\n\n", false);
	if(player.stats.cor > 66) MainScreen.text("You eagerly seize the rod on your right side; squeezing it a bit in impatience.  A  little tingle alerts you to your left neighbor grabbing " + oMultiCockDesc() + " with an expert hand.  Everyone around you quickly grabs hold of the nearest cock on their right side.  Some of them are trembling because of the induced stress and arousal.\n\n", false);
	else if(player.stats.cor > 33) MainScreen.text("You nervously grab hold of the rod on your right side.  In your stress, you can't help but give it a little squeeze.  You tremble as your left neighbor grabs " + oMultiCockDesc() + " with an expert hand.  Most people are as nervous as you, their whole naked body trembling in stress.\n\n", false);
	else {
		MainScreen.text("You hesitantly pick the rod on your right side with an awkward hand.  You nearly let it go because you're so nervous, and its owner smiles: you're obviously not very experienced with dicks.  ", false);
		MainScreen.text("You can't help but start as your left neighbor grabs " + oMultiCockDesc() + " with an unsettling expertise.  Other players are also nervous, but none as much as you.  The sight of all these trembling dicks makes you shiver.\n\n", false);
	}
	//end of global condition about PC's corr, display the following
	MainScreen.text("You firmly settle your " +  LowerBodyDescriptor.describeLegs(player)  + " on the ground, bracing yourself to the imminent handjob you're going to receive, and the one you're going to give.  Your old warrior reflexes start kicking in and you ease your grasp on the cock in your right hand and start calming yourself.  You feel ready when - ", false);
	MainScreen.text("\"<i>GO!</i>\"\n\n", false);

	MainScreen.text("You instantly wince a little as the hand holding your " + CockDescriptor.describeCock(player, x) + " starts rocking up and down energetically.  By the gods, he's good at this, whatever he is.  You suddenly remember your own hand has a job to do and you proceed to rub the shaft on your right, slowly at first and gaining more and more pace as you get more confident.  Soon you're furiously stroking it, trying to match your left partner's rhythm in intensity.  Since there wasn't any handjob stamina contest organized in Ingam, you're fairly inexperienced at this and try to imitate your left partner as best as you can, using the pleasure he's inflicting on you as a suggestion for how to treat your own victim.  You rub the sensitive points, you tickle the urethra and give gentle squeezes with your palm as your hand slowly works its way up and down the massive cock it's stroking.  Your best effort, however, goes into ignoring the hand expertly working your own " +  CockDescriptor.describeCock(player, x)  + ", massaging it viciously in order to pump the semen up in your urethra.  Your erection gets bigger, if that is even possible; but you manage to hold off the incoming orgasm.\n\n", false);
	
	MainScreen.text("You continue stroking the pecker vigorously while trying to ignore the handjob your own " + CockDescriptor.describeCock(player, x) + " is getting, and your efforts are rewarded: soon pre-cum starts dribbling from the tip of the tool you're jerking off, and your hand proceeds to smear that cock in its own juice; you notice it grows slightly bigger.  There is a profound silence in the tent; you only hear a few soft thuds resulting from dick skin being stretched: FAP FAP FAP FAP FAP FAP FAP FAP...  Occasionally a player will break the silence as their lips let a loud, pleasure-filled moan.\n\n", false);
	...
	let c:number = 0;
	if(player.lowerBody.cockSpot.get(x).cockType == CockType.DEMON) c = 10;
	else if(player.lowerBody.cockSpot.get(x).cockType == CockType.TENTACLE) c = 5;
	else if(player.lowerBody.cockSpot.get(x).cockType == CockType.LIZARD) c = 3;
	else if(player.lowerBody.cockSpot.get(x).cockType == CockType.HORSE) c = 2;
...
private fapResults(place:number = 3):void {
	...
	if(place == 3) {
		...
		MainScreen.text("Soon everyone else starts blowing their own loads, shouting their number.  You hear the pleasure-filled moans of a dozen of people giving in to their bodies, only arousing you more and increasing your cum-flow.  The referee records the results and writes them down on a little notepad but she has a hard time keeping it free of white stains because all players seem to be aiming their cumshots at her.  Players are moaning whorishly and louder than ever.  The whole room seems to be detonating as dozens of horny bodies release their powerful orgasms in an apotheosis of lust.  In a few minutes the entire room is filled with trails of cum, everyone being too eager to shoot their load after this fever of orgasm denial.\n\n", false);
	}
	//[else if the player doesn't lose, but doesn't win either - he cums neither first nor last]
	else if(place == 2) {
		MainScreen.text("For what seems like hours you try to hold on, resisting wave after wave of unbearable pleasure in your crotch.  Nevertheless, you feel your stamina gradually depleting; each orgasm is harder to deny.  You know you won't be able to hold on much longer, but you muster up your courage and count to ten in your head.  Surely someone will cum before you... they better do it now because your mind is about to collapse and you will no longer be able to control your body.  Gods, it feels too good.  That stroking, teasing hand is torturing you in a delicious way, but you keep denying the pleasure.  You have to.  It's so rough, so...\n\n", false);
		
		MainScreen.text("Your thoughts are suddenly interrupted.\n\n", false);
		
		MainScreen.text("Someone in the circle yells, \"<i>NUMBER " + num + " CUMMING!</i>\", immediately followed by another voice yelling even louder, \"<i>NUMBER " + (num-1) + " SCORING!</i>\"\n\n", false);
		
		MainScreen.text("Indeed, a player's cock has given up the stamina contest and sprays spunk all around in a hot creamy torrent.  That's one mighty cum-shot, you think in admiration.  Almost immediately after that, several players give up too and start shouting while shooting their own goo.\n\n", false);
		
		MainScreen.text("Your right partner suddenly shouts in a high-pitched voice, \"<i>NUMBER " + (num+5) + " CUMMING!</i>\"; you immediately understand and shout \"<i>NUMBER " + (num+4) + " SCORING!</i>\" in response.  You can feel the pressure releasing in the cock your hand is holding, the fluid being sprayed all around by the perverted pump.\n\n", false);
		
		MainScreen.text("A vicious yank on your own " + CockDescriptor.describeCock(player, x) + " reminds you that you also need to comply to your own genitals' needs.  Unhindered by the fear of losing, you completely give up to your orgasm and sigh in utter satisfaction and relief as you release one of the most abundant cumshots you've ever had.  Lost in your orgasm, you carelessly ", false);
		if(player.cumQ() >= 1000) MainScreen.text("soak", false);
		else if(player.cumQ() >= 500) MainScreen.text("pollute", false);
		else MainScreen.text("wet", false);
		MainScreen.text(" the floor with your sexual juices and yell in a whorish voice, \"<i>NUMBER " + (num+4) + " CUMMING!</i>\"; you're almost immediately followed by the scoring announcement from your left neighbor.  With a half-closed eye you notice the referee nodding in acknowledgement and you point your " + CockDescriptor.describeCock(player, x) + " at her, trying to soak her too.  She hardly dodges it and some lands on her arms, but she doesn't seem to notice.", false);
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("  Your thighs tense a few seconds before your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " squirts too, forming a ", false);
			if(player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 4) MainScreen.text("large puddle", false);
			else if(player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 2) MainScreen.text("puddle", false);
		...
		MainScreen.text("Quite satisfied, your left partner decides to stop torturing himself; he must have had a rough handjob too.  You watch in amazement as his cum-tube starts shooting globs of sticky goo, aiming at the referee with an expert's precision. He grins lewdly and yells: \"<i>NUMBER " + (num+3) + " CUMMING!</i>\"\n\n", false);
	...
	//[else, the player wins]
		...
		MainScreen.text("For what seems hours you try to hold on, resisting wave after wave of unbearable pleasure in your crotch.  Nevertheless, you feel your stamina gradually depleting; each orgasm is harder to deny.  You know you won't be able to hold on much longer, but you muster up your courage and count to ten in your head.  You must win.  All these efforts can't have been in vain.  Surely someone will cum before you... and someone else, and someone else.  They better do it soon though.  You don't know how long you will be able to control your body.  Gods, it feels too good.  That stroking, teasing hand is torturing you in a delicious way, but you keep denying the pleasure.  You have to.  It's so rough, so...\n\n", false);
		
		MainScreen.text("Your thoughts are suddenly interrupted.\n\n", false);
		
		MainScreen.text("Your right partner shouts in a high-pitched voice: \"<i>NUMBER "+ (num+1) + " CUMMING!</i>\"  You immediately understand and shout \"<i>NUMBER " + num + " SCORING!</i>\" in response.\n\n", false);
		
		MainScreen.text("Indeed, you can feel the pressure releasing in the cock your hand is holding, the fluid being sprayed all around in a hot creamy torrent.  That's one mighty cumshot, you think in admiration.  Almost immediately after that, several players give up too and start shouting while shooting their own goo.\n\n", false);
		
		MainScreen.text("Someone in the circle yells \"<i>NUMBER " + (num+7) + " CUMMING!</i>\" immediately followed by another voice yelling even louder \"<i>NUMBER " + (num+6) + " SCORING!</i>\"\n\n", false);
		...
		MainScreen.text("Tortured, your left partner decides to stop teasing himself; he must have had a rough handjob too.  You watch in amazement as his cum-tube starts shooting globs of sticky goo, aiming at the referee with an expert's precision.  He grins lewdly and yells: \"<i>NUMBER " + (num-1) + " CUMMING!</i>\"\n\n", false);
		...
		if(player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier > 1.5) MainScreen.text("  Your " + BreastDescriptor.describeAllBreasts(player) + " also release a milky flood of their own, the soft feeling of cream teasing your nipple only prolonging your orgasm.\n\n", false);
	}
	//Post fap
	MainScreen.text("You pant and relax, exhausted.  Your right arm is cramped and your " + CockDescriptor.describeCock(player, x) + " is sore.  People around you look similarly tired and are slowly recovering from this rough collective dick-stroking session.  Someone gently taps your shoulder.  It is your left neighbor: \"<i>Well played. I didn't expect you would be so... resistant.  Few players last that long with me, because their rods generally don't endure my handjobs.</i>\"  He proceeds to shake hands; both you are smeared with pre-cum and various cock juices, but you don't care.  You smile and thank him for the good game and the orgasm.  It comes as something of a relief that everyone seems to show good sportsmanship.  Everyone is congratulating each other; various comments reach your ears about the game they just had and how well their opponents did.  Your right partner stands before you.  \"<i>Whoa, you sure know how to give a handjob!  That was amazing, and you know what?  I don't care if I haven't won!</i>\"  You tell him that you're glad that he enjoyed your efforts.  Everyone is now laughing and chatting casually, as if their still erect cocks weren't dripping semen and they weren't busy giving furious handjobs a few minutes ago.  ", false);
		
	//[if first time]
	if(Flags.list[FlagEnum.FAP_ARENA_RULES_EXPLAINED] == 1) MainScreen.text("You stand here, a little lost, wondering what will happen next.", false);
	else MainScreen.text("You join them, congratulating some on the strength of their stroke, and commenting on the various aspects of the fapping game you've just had.\n\n", false);
		
	MainScreen.text("The referee, who had been busy cleaning herself up from all the spunk in a backroom, suddenly storms out and plants herself at the center of the circle, her nude red body trembling and shivering in arousal.  \"<i>Alright, everybody in a single line!  It's time for your butt-fuck reward.</i>\"\n\n", false);
	
	MainScreen.text("You jolt in surprise as everyone stops talking altogether and yells in a single, thundering, thrumming voice, \"<i>BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!</i>\"  It never seems to end.\n\n", false);
	
	//[if corr < 50]
	if(player.stats.cor < 50) MainScreen.text("You find yourself yelling with them, astounded by everyone's enthusiasm and eagerness to be fucked in the ass, even after this rough masturbating session.  You wish you weren't so blindly following others, but they are so hypnotizing, their hard glistening cocks pointing at the air as they keep shouting over and over again, \"<i>BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!</i>\"  ", false);
	//[else, corr >50]
	else MainScreen.text("Your body tenses and " + CockDescriptor.describeMultiCockSimpleOne(player) + " hardens at the perspective of a collective anal orgy.  You yell in lust like a minotaur in rut, \"<i>BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!</i>\" along with the others.  ", false);
	//end of global condition about PC's corr, display the following
	MainScreen.text("As they shout rhythmically, they start forming a straight line, their position in the line reflecting their rating.\n\n", false);
	
	//[if you won]
	if(place <= 1) MainScreen.text("Since you're the winner, you walk to the tail of the butt-fuck train, enjoying your prize.  ", false);
	//[if you didn't win, didn't lose]
	else if(place <= 2) MainScreen.text("You place yourself between two horny bodies.  The lust and confusion have obscured your senses and all you see is a rock-hard dick and a gaping anus, both waiting for you to play with.  ", false);
	//[if you lost]
	else {
		MainScreen.text("You meekly place yourself at the end of the butt-fuck wagon, knowing you'll have to endure the thrusts of everyone else.  In your shame and lust, all you can see is a member of enormous size, trembling in arousal, waiting to lodge between your ass-cheeks.  With ", false);
		//([if corr < 50]
		if(player.stats.cor < 50) MainScreen.text("a whorish smile", false);
		else MainScreen.text("apprehension", false);
		MainScreen.text(", you place yourself in front of the creature with the large cock.  It grinds against your back in impatience.  ", false);
	}
	MainScreen.text("You ready yourself up as everyone puts themselves into a proper butt-fucking position, cocks pointing to anuses.\n\n", false);
	
	MainScreen.text("The referee shouts, \"<i>LET THE BUTT-FUCK TRAIN BEGIN!</i>\"\n\n", false);
	
	//[if you won]
	if(place <= 1) {
		MainScreen.text("You can't believe you will get to fuck a whole train's worth of anuses and cocks.  This seems so unreal...  nevertheless, the tension and the scent of cum prove too much for you, and you give in to your lust.  ", false);
		//[1/2 chance of this]
		if(rand(2) == 0) {
			temp = 3;
			MainScreen.text("You eagerly grab the luxurious, gelatinous butt cheeks in front of you.  You notice a pussy underneath that fine ass, freely spilling sexual juices.  If the compelling order wasn't resonating deep in your mind, you would easily lose yourself in the herm's glorious depths. Nevertheless, your " + CockDescriptor.describeCock(player, x) + " remembers its job and starts pushing its way between these tender orbs, then into the herm's tight anus.\n\n", false);
		}
		else {
			MainScreen.text("You eagerly grab the firm, muscular ass cheeks in front of you, probably belonging to some vigorous male.  You grind your " + CockDescriptor.describeCock(player, x) + " against his back and it automatically starts spilling pre-cum; as soon as the seminal liquid trickles down his shapely butt, you can feel his body tensing, as if his lust reached a new level.  Nevertheless, your " + CockDescriptor.describeCock(player, x) + " remembers its job and starts pushing its way between these gorgeous buttocks, then into the man's tight anus.\n\n", false);
			temp = 1;
		}
		MainScreen.text("It lodges here comfortably for a few seconds, and then you remember you're the one initiating the butt-fucking train; so you quickly get down to business and give your dong an almighty thrust.  As soon as you start pushing, your sodomite partner grips ", false);
		if(temp == 1) MainScreen.text("his", false);
		else MainScreen.text("her", false);
		MainScreen.text(" own partner's butt and thrusts too under the pressure, who does the same to the one following, etc..  The last one is actually pushed face down onto the floor, but her butt bumps against the cum-tube fucking her, making the cock's owner bump in response, and it all propagates back to your own dick; you gasp in pleasure as the ass holding your " + CockDescriptor.describeCock(player, x) + " clenches all the more as it bumps against your thighs, and decide to give another thrust, and another, and another.  It is hard, laborious, and slow, but your " + CockDescriptor.describeCock(player, x) + " is rewarded beyond imagination: it feels like a gentle tingle every time a butt bumps into someone's ass.  It is like a giant anal wave coming and going through people's cocks, and you're the one riding the wave.  Soon people in the fuck train let out whorish moans, often interrupted when the wave hits them, before moaning all the more loudly afterwards.  Although you are not getting your " + ButtDescriptor.describeButthole(player) + " pounded like everyone else, you know you won't be able to keep up with such a crazy butt-fucking pace for long.  The ass you're fucking is almost moving on its own, jerking back and forth at each thrust, nearly beyond your control.  You know you're supposed to lead the whole fuck train, but the raw feelings are too much.  Every time you try to ease yourself into fucking that ass in a more comfortable way, it grinds against your thighs, driving your " + CockDescriptor.describeCock(player, x) + " crazy and crushing any attempt at coherent thought.  You decide to give up and let your " + CockDescriptor.describeCock(player, x) + " do the job itself.  Pleasure rises as you pound this ass relentlessly, making its owner join the chorus of moaning sounds.\n\n", false);
		...
		if((player.lowerBody.cockSpot.count() > 2) || (!tent && player.lowerBody.cockSpot.count() > 1)) {
			if(player.lowerBody.cockSpot.count() > 3 || (!tent && player.lowerBody.cockSpot.count() > 2)) MainScreen.text("  Your other cocks also spill ", false);
			if((tent && player.lowerBody.cockSpot.count() == 3) || (player.lowerBody.cockSpot.count() == 2 && !tent)) MainScreen.text("  Your other cock also spills ", false);
			MainScreen.text("a ", false);
			if(player.cumQ() > 1000) MainScreen.text("mighty", false);
			else if(player.cumQ() > 500) MainScreen.text("large", false);
			else MainScreen.text("good", false);
			MainScreen.text(" spray of ", false);
			if(player.lowerBody.cockSpot.count() > 3 || (!tent && player.lowerBody.cockSpot.count() > 2)) MainScreen.text("their ", false);
			else MainScreen.text("its ", false);
			MainScreen.text("own, unhindered by the tight colon holding your first " + CockDescriptor.describeCock(player, x) + ".", false);
		}
		//[if player has vagina]
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("  Your feminine part squirts a ", false);
			if(player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 5) MainScreen.text("titanic", false);
			else if(player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 3) MainScreen.text("massive", false);
			else MainScreen.text("nice", false);
			MainScreen.text(" load of girl-cum too, as if to compete with its male counterpart.", false);
		}
		//end of conditions
	...
	//[else if you didn't win]
		...
		MainScreen.text("You almost instantly feel firm arms groping your " + ButtDescriptor.describeButt(player) + " and the tip of an erect dick knocking at the door of your colon.  It slowly massages your rectum, and you can't help but giggle a bit as you feel a tingle of pleasure passing through your body.\n\n", false);
		
		//[1/2 chance of this]
		if(rand(2) == 0) MainScreen.text("You eagerly grab the luxurious, gelatinous butt cheeks in front of you.  You notice a pussy underneath that fine ass, freely spilling sexual juices.  If the compelling order weren't resonating deep in your mind, you would easily lose yourself in the herm's glorious depths.  Nevertheless, your " + CockDescriptor.describeCock(player, x) + " remembers its job and starts pushing its way between these tender orbs, then into the herm's tight anus.\n\n", false);
		else MainScreen.text("You eagerly grab the firm, muscular ass cheeks in front of you, probably belonging to some vigorous male.  You grind your " + CockDescriptor.describeCock(player, x) + " against his back and it automatically starts spilling pre-cum; as soon as the seminal liquid trickles down his shapely butt, you can feel his body tensing, as if his lust reached a new level.  Nevertheless, your " + CockDescriptor.describeCock(player, x) + " remembers its job and starts pushing its way between these gorgeous buttocks, then into the man's tight anus.\n\n", false);

		MainScreen.text("You suddenly hear a muffled moan from behind.  Someone is probably being fucked at the back of the train.  You brace yourself for the imminent anal penetration... and here it comes.  The dong is projected at full force into your " + ButtDescriptor.describeButt(player) + " then all the way inside your colon; you're protected only by the copious glaze of pre-cum its bearer had been applying.", false);
		player.buttChange(5+rand(15),true,true,false);
		MainScreen.text("  Instinctively, you reach for the butt awaiting your " + CockDescriptor.describeCock(player, x) + " and push as if your life depended on it.  You notice how your sodomite partner does the same to the ass in front of him, and so on.  Eventually the last one is pushed face down onto the floor, but her butt bumps against the dong fucking her, making the cock's owner bump in response, and it all propagates back to your own pecker; you wince in pleasure as your asshole impales itself further on the turgescent member behind you.  You are pushed and pulled rhythmically, your throbbing member and the one fucking you serving both as vectors in this slow wave of anal pleasure.  Unable to keep control of your thrusts, you let go of your body, enjoying the feeling of being shoved in both directions, each motion bringing either a jolt of pleasure in your crotch or your " + ButtDescriptor.describeButt(player) + ".  You are being slowly overwhelmed by the delicious tightness of the butt you're fucking as well as the fat piece of meat pounding your own.  The raw feelings eventually prove too much for your horny body, and you jerk your head back and moan like a whore, waiting for the incoming orgasm.\n\n", false);
		tentacleFapSpecial(place);
		MainScreen.text("A pressure builds at your crotch as a milky torrent of seed finds its way into your urethra, down your sodomite partner's colon.  Almost simultaneously, you feel an equally thick spray of spooge being released inside your own " + ButtDescriptor.describeButt(player) + ". At the peak of your orgasm, you are unable to think of anything but the steady flow of jism coming in and out of you.", false);
		tent = tentacleFapCum();
		//[if player has multicocks] 
		if((player.lowerBody.cockSpot.count() > 2) || (!tent && player.lowerBody.cockSpot.count() > 1)) {
			if(player.lowerBody.cockSpot.count() > 3 || (!tent && player.lowerBody.cockSpot.count() > 2)) MainScreen.text("  Your other cocks also spill ", false);
...
private tentacleFapCum():boolean {
	...
	if(player.lowerBody.cockSpot.get(x).cockLength >= 40 && player.lowerBody.cockSpot.get(x).cockType == CockType.TENTACLE) {
		MainScreen.text("  Your tentacle pecker starts vibrating on its own, and soon it also releases a voluminous amount of jizz inside the herm's vagina.  She is also reaching her climax; her pussy is drenching the end of your tentacled shaft with girl-juices.", false);
		return true;
	}
	return false;
}
