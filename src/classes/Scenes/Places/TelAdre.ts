...
private function encounterTelAdre():void {
	Render.text("", true);
	if(player.findStatusAffect(StatusAffects.TelAdre) < 0) {
		Render.text("You slog through the shifting sands for a long time, not really seeming to get that close.  Just when you're about to give up, you crest a large dune and come upon the walls of the city you saw before.  It's definitely NOT a mirage.  There are sandstone walls at least fifty feet tall ringing the entire settlement, and the only entrance you can see is a huge gate with thick wooden doors.  The entrance appears to be guarded by a female gray fox who's more busy sipping on something from a bottle than watching the desert.\n\n", false);
		Render.text("As if detecting your thoughts, she drops the bottle and pulls out a halberd much longer than she is tall.\n\n", false);
		Render.text("\"<i>Hold it!</i>\" barks the fox, her dark gray fur bristling in suspicion at your sudden appearance, \"<i>What's your business in the city of Tel'Adre?</i>\"\n\n", false);
		Render.text("You shrug and explain that you know nothing about this town, and just found it while exploring the desert.  The girl stares at you skeptically for a moment and then blows a shrill whistle.  She orders, \"<i>No sudden moves.</i>\"\n\n", false);
		Render.text("Deciding you've nothing to lose by complying, you stand there, awaiting whatever reinforcements this cute vulpine-girl has summoned.  Within the minute, a relatively large-chested centauress emerges from a smaller door cut into the gate, holding a massive bow with an arrow already nocked.\n\n", false);
		Render.text("\"<i>What's the problem, Urta?  A demon make it through the barrier?</i>\" asks the imposing horse-woman.\n\nUrta the fox shakes her head, replying, \"<i>I don't think so, Edryn.  " + player.mf("He's","She's") + " something else.  We should use the crystal and see if " + player.mf("he","she") + "'s fit to be allowed entry to Tel'Adre.</i>\"\n\n", false);
		Render.text("You watch the big centaur cautiously as she pulls out a pendant, and approaches you.  \"<i>Hold still,</i>\" she says, \"<i>this will do you no harm.</i>\"\n\n", false);
		Render.text("She places one hand on your shoulder and holds the crystal in the other.  Her eyes close, but her brow knits as she focuses on something.  ", false);
		telAdreCrystal();
	}
	else {
		Render.text("Once again you find the gray fox, Urta, guarding the gates.  She nods at you and whistles for her companion, Edryn once again.  The centauress advances cautiously, and you submit herself to her inspection as she once again produces her magical amulet.  ", false);
		telAdreCrystal();
	}
}
...
private function telAdreCrystal():void {
	...
	if(player.statusAffects.has("Exgartuan") || player.stats.cor >= 70) {
		Render.text("The crystal pendant begins to vibrate in the air, swirling around and glowing dangerously black.  Edryn snatches her hand back and says, \"<i>I'm sorry, but you're too far gone to step foot into our city.  If by some miracle you can shake the corruption within you, return to us.</i>\"\n\n", false);
		Render.text("You shrug and step back.  You could probably defeat these two, but you know you'd have no hope against however many friends they had beyond the walls.  You turn around and leave, a bit disgruntled at their hospitality.  After walking partway down the dune you spare a glance over your shoulder and discover the city has vanished!  Surprised, you dash back up the dune, flinging sand everywhere, but when you crest the apex, the city is gone.", false);
		doNext(camp.returnToCampUseOneHour);
		return;
	}
	//-50+ corruption or corrupted Jojo
	else if(player.stats.cor >= 50 || kGAMECLASS.monk >= 5) {
		Render.text("The crystal pendant shimmers, vibrating in place and glowing a purple hue.  Edryn steps back, watching you warily, \"<i>You've been deeply touched by corruption.  You balance on a razor's edge between falling completely and returning to sanity.  You may enter, but we will watch you closely.</i>\"\n\n", false);
	}
	//-25+ corruption or corrupted Marae
	else if(player.stats.cor >= 25 || player.statusAffects.has("FactoryOverload")) {
		Render.text("The crystal pendant twirls in place, glowing a dull red.  Edryn takes a small step back and murmers, \"<i>You've seen the darkness of this land first hand, but its hold on you is not deep.  You'll find sanctuary here.  The demons cannot find this place yet, and we promise you safe passage within the walls.</i>\"\n\n", false);
	}
	//-Low corruption/pure characters
	else {
		Render.text("The crystal shines a pale white light.  Edryn steps back and smiles broadly at you and says, \"<i>You've yet to be ruined by the demonic taint that suffuses the land of Mareth.  Come, you may enter our city walls and find safety here, though only so long as the covenant's white magic protects us from the demons' lapdogs.</i>\"\n\n", false);
	}
	Render.text("The vixen Urta gestures towards the smaller door and asks, \"<i>Would you like a tour of Tel'Adre, newcomer?</i>\"\n\n", false);
	Render.text("You remember your etiquette and nod, thankful to have a quick introduction to such a new place.  Urta leaves Edryn to watch the gate and leads you inside.  You do notice her gait is a bit odd, and her fluffy fox-tail seems to be permanently wrapped around her right leg.  The door closes behind you easily as you step into the city of Tel'Adre...", false);
	doNext(telAdreTour);
}
...
public function telAdreMenu():void {
	...
	spriteSelect( -1);
	Render.text("Tel'Adre is a massive city, though most of its inhabitants tend to hang around the front few city blocks.  It seems the fall of Mareth did not leave the city of Tel'Adre totally unscathed.  A massive tower rises up in the center of the city, shimmering oddly.  From what you overhear in the streets, the covenant's magic-users slave away in that tower, working to keep the city veiled from outside dangers.  There does not seem to be a way to get into the unused portions of the city, but you'll keep your eyes open.\n\n", true);
	Render.text("A sign depicting a hermaphroditic centaur covered in piercings hangs in front of one of the sandstone buildings, and bright pink lettering declares it to be the 'Piercing Studio'.  You glance over and see the wooden facade of Urta's favorite bar, 'The Wet Bitch'.  How strange that those would be what she talks about during a tour.  In any event you can also spot some kind of wolf-man banging away on an anvil in a blacksmith's stand, and a foppishly-dressed dog-man with large floppy ears seems to be running some kind of pawnshop in his stand.  Steam boils from the top of a dome-shaped structure near the far end of the street, and simple lettering painted on the dome proclaims it to be a bakery.  Perhaps those shops will be interesting as well.", false);
...
public function telAdreMenuShow():void { //Just displays the normal Tel'Adre menu options, no special events, no description. Useful if a special event has already played
	...
	if (flags[FlagEnum.ARIAN_PARK] > 0 && flags[FlagEnum.ARIAN_PARK] < 4) addButton(5, "Park", kGAMECLASS.arianScene.visitThePark);
...
public function houses():void {
	clearOutput();
	Render.text("Whose home will you visit?");
	...
	menu();
	if(flags[FlagEnum.ARIAN_PARK] >= 4 && !kGAMECLASS.arianScene.arianFollower()) addButton(0,"Arian's",kGAMECLASS.arianScene.visitAriansHouse);
	...
	if (flags[FlagEnum.KATHERINE_UNLOCKED] >= 5) addButton(3, "Kath's Apt", katherine.visitAtHome);
...
private function aboutYara():void {
	...
	Render.text("You introduce yourself and ask Yara about her past, noting that ", true);
	if(player.humanScore() <= 2) Render.text("you were once a human too.", false);
	else Render.text("you haven't seen many other humans about.", false);
...
private function pierceMenu():void {
	...
	Render.text("Yara asks, \"<i>Ok then, what would you like pierced " + player.mf("sir","cutie") + "?  Just keep in mind my piercings are special - they're permanent and CAN'T be removed.</i>\"", true);
...
private function dickPierce():void {
	spriteSelect(63);
	if(player.totalCocks() > 0) Render.text("\"<i>Ok, this is gonna hurt a LOT, but I've heard good things about it.  What kind of piercing do you want done?</i>\" Yara asks.", true);
	else {
		Render.text("You realize you don't have a dick to pierce.  Whoops!  Better pick something else...", true);
		doNext(pierceMenu);
		return;
	}
...
private function clitPierce():void {
	spriteSelect(63);
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("\"<i>Ohhh, that's going to be suckably cute!</i>\" exclaims Yara, blushing more than a little. \"<i>What kind of piercing would you like?</i>", true);
	else {
		Render.text("You realize you don't have a clit to pierce.  Whoops!  Better pick something else...", true);
		doNext(pierceMenu);
		return;
	}
	piercingLoc = 0;
...
private function earPierce():void {
	spriteSelect(63);
	piercingLoc = 2;
	Render.text("\"<i>Okay, just let me get my supplies and we can get started.  What kind of jewelry do you want in them?</i>\" asks Yara.", true);
...
private function eyebrowPierce():void {
	spriteSelect(63);
	piercingLoc = 3;
	Render.text("\"<i>Ah, that's a good look!  What do you want there?</i>\" asks Yara.", true);
	simpleChoices("Stud", chooseStud, "Ring", chooseRing, "", null, "Back", pierceMenu, "Nevermind", piercingStudio);
}
...
private function lipPierce():void {
	spriteSelect(63);
	piercingLoc = 4;
	Render.text("\"<i>Oh my, that'll be HAWT!  What kind of jewelry do you want there?</i>\" asks Yara.", true);
	simpleChoices("Stud", chooseStud, "Ring", chooseRing, "", null, "Back", pierceMenu, "Nevermind", piercingStudio);
}
...
private function nipplePierce():void {
	spriteSelect(63);
	piercingLoc = 5;
	Render.text("\"<i>Yeah, sure I can do those!  What kind of jewelry do you want there?  I'm partial to nipple-chains myself,</i>\" admits Yara, blushing bright red.", true);
...
private function nosePierce():void {
	spriteSelect(63);
	piercingLoc = 6;
	Render.text("Yara wrinkles her nose in distaste, \"<i>Really?  Well ok, what do you want there?</i>\"", true);
	simpleChoices("Stud", chooseStud, "Ring", chooseRing, "", null, "Back", pierceMenu, "Nevermind", piercingStudio);
}
...
private function tonguePierce():void {
	spriteSelect(63);
	piercingLoc = 7;
	Render.text("Yara happily purrs, \"<i>Oh my, I bet that'll be fun!  I'm afraid I can only put a stud there though, ok?</i>\"", true);
	simpleChoices("Ok", chooseStud, "", null, "", null, "Back", pierceMenu, "Nevermind", piercingStudio);
}
private function vulvaPierce():void {
	spriteSelect(63);
	piercingLoc = 8;
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("Yara explains, \"<i>This is gonna hurt a lot, but I think you'll love how it feels after.  I know I do!  Now what kind of jewelry do you want down-town?</i>\"", true);
	else {
		Render.text("You realize you don't have a pussy to pierce.  Whoops!  Better pick something else...", true);
		doNext(pierceMenu);
		return;
	}
	simpleChoices("Stud", chooseStud, "Ring", chooseRing, "", null, "Back", pierceMenu, "Nevermind", piercingStudio);
...
private function areYouSure():void {
	spriteSelect(63);
	Render.text("Yara says, \"<i>Ok, last chance to back out, are you sure you want to go ahead with this?  Remember, once I put it in, it's permanent.</i>\"", true);
...
private function chooseAdvancedMaterials():void {
	spriteSelect(63);
	Render.text("Yara goes back into the back and comes out with a gilded tray full of exotic materials.  She hands you a brochure and asks, \"<i>Ok, now what am I going to be working with?</i>\"", true);
...
private function normalPierceAssemble():void {
	spriteSelect(63);
	Render.text("Yara makes you comfortable and has you look away while she uses her piercing tools.  It hurts, but she's skilled and before you know it, your piercing is done!", true);
	//1. Amethyst (+1 int, +1 lib)
	//2. Diamond (+2 int, -1 cor)
	//3. Gold (+1 int, +1 sens)
	//4. Emerald (+1 spe)
...
private function piercingRemove():void {
	...
	if(clit == null && dick == null && ears == null && eyebrow == null && lip == null && nipples == null && nose == null && tongue == null && vulva == null) {
		Render.text("Yara giggles, \"<i>You don't have any piercings, silly!</i>\"", true);
		doNext(piercingStudio);
		return;
	}
	Render.text("\"<i>Really?</i>\" asks Yara, \"<i>I told you those piercings are permanent!  Well, I suppose they CAN be removed, but you're gonna hurt like hell afterwards.  If you really want me to, I can remove something, but it'll cost you 100 gems for the painkillers and labor.</i>\"", true);
	if(player.stats.gems < 100) {
	...
	if(player.tou <= 5.5) {
		Render.text("Yara looks you up and down before refusing you outright, \"<i>You don't look so good " + player.short + ".  I don't think your body could handle it right now.</i>\"", true);
		doNext(piercingStudio);
		return;
	}
	choices("Clit",clit,"Dick",dick,"Ears",ears,"Eyebrow",eyebrow,"Lip",lip,"Nipples",nipples,"Nose",nose,"Tongue",tongue,"Labia",vulva,"Back",piercingStudio);
...
private function removeClitPierce():void {
	spriteSelect(63);
	Render.text("Yara gives you something to drink and you swiftly black out.  You awake about an hour later, sore and weak, though thankfully not bleeding.", true);
...
public function oswaldPawn():void {
	...
	if(player.findStatusAffect(StatusAffects.Oswald) < 0) {
		Render.text("Upon closer inspection, you realize the pawnbroker appears to be some kind of golden retriever.  He doesn't look entirely comfortable and he slouches, but he manages to smile the entire time.  His appearance is otherwise immaculate, including his classy suit-jacket and tie, though he doesn't appear to be wearing any pants.  Surprisingly, his man-bits are retracted.  ", false);
		if(player.stats.cor < 75) Render.text("Who would've thought that seeing someone NOT aroused would ever shock you?", false);
		else Render.text("What a shame, but maybe you can give him a reason to stand up straight?", false);
		Render.text("  His stand is a disheveled mess, in stark contrast to its well-groomed owner.  He doesn't appear to be selling anything at all right now.\n\n", false);
		Render.text("The dog introduces himself as Oswald and gives his pitch, \"<i>Do you have anything you'd be interested in selling?  The name's Oswald, and I'm the best trader in Tel'Adre.</i>\"\n\n", false);
		Render.text("(You can sell an item here, but Oswald will not let you buy them back, so be sure of your sales.)", false);
		player.statusAffects.add(new StatusAffect("Oswald",0,0,0,0)));
	}
	else {
		Render.text("You see Oswald fiddling with a top hat as you approach his stand again.  He looks up and smiles, padding up to you and rubbing his furry hands together.  He asks, \"<i>Have any merchandise for me " + player.mf("sir","dear") + "?</i>\"\n\n", false);
		Render.text("(You can sell an item here, but Oswald will not let you buy them back, so be sure of your sales.)", false);
	}
...
private function buyCarrotFromOswald():void {
	...
	clearOutput();
	Render.text("Gems change hands in a flash, and you're now the proud owner of a bright orange carrot!\n\n(<b>Acquired Key Item: Carrot</b>)");
	menu();
...
private function oswaldPawnSell(slot: number):void { //Moved here from Inventory.as
	spriteSelect(47);
	let itemValue: number = int(player.itemSlots[slot].itype.value / 2);
	clearOutput();
	if (itemValue == 0)
		Render.text("You hand over " + player.itemSlots[slot].itype.longName + " to Oswald.  He shrugs and says, “<i>Well ok, it isn't worth anything, but I'll take it.</i>”");
	else Render.text("You hand over " + player.itemSlots[slot].itype.longName + " to Oswald.  He nervously pulls out " + num2Text(itemValue) + " gems and drops them into your waiting hand.");
...
private function oswaldPawnSellAll():void {
	...
	Render.text("You lay out all the items you're carrying on the counter in front of Oswald.  He examines them all and nods.  Nervously, he pulls out " + num2Text(itemValue) + " gems and drops them into your waiting hand.");
	player.stats.gems += itemValue;
	statScreenRefresh();
	doNext(oswaldPawn);
}

private function anotherButton(button: number, nam:string, func:Function, arg:* = -9000): number {
...
public function barTelAdre():void {
	...
	Render.text("The interior of The Wet Bitch is far different than the mental picture its name implied.  It looks like a normal tavern, complete with a large central hearth, numerous tables and chairs, and a polished dark wood bar.  The patrons all seem to be dressed and interacting like normal people, that is if normal people were mostly centaurs and dog-morphs of various sub-species.  The atmosphere is warm and friendly, and ");
	if (player.humanScore() <= 3) Render.text("despite your altered appearance, ");
	Render.text("you hardly get any odd stares.  There are a number of rooms towards the back, as well as a stairway leading up to an upper level.");
...
private function oldbarTelAdre():void {
	...
	Render.text("The interior of The Wet Bitch is far different than the mental picture its name implied.  It looks like a normal tavern, complete with a large central hearth, numerous tables and chairs, and a polished dark wood bar.  The patrons all seem to be dressed and interacting like normal people, that is if normal people were mostly centaurs and dog-morphs of various sub-species.  The atmosphere is warm and friendly, and ", false);
	if(player.humanScore() <= 3) Render.text("despite your altered appearance, ", false);
	Render.text("you hardly get any odd stares.  There are a number of rooms towards the back, as well as a stairway leading up to an upper level.", false);
...
private function buyClothes(itype:ItemType):void {
	Render.text("", true);
	spriteSelect(61);
	Render.text("Victoria nods and pulls a measuring tape off her shoulder.  She moves around you with practiced ease, taking measurements from every conceivable angle.  Thanks to her small stature, it's quite easy for her to take your inseam measurement, though Vicky manages to ", false);
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 30 || player.totalCocks() > 1) Render.text("fondle your bulging package", false);
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("rub against your outer lips", false);
	else Render.text("slip a finger along your crotch", false);
	Render.text(" more than a few times.  You could swear you catch her licking her lips when she stands up, but she quickly turns away, saying, \"<i>I've got one in the back that should fit perfectly!  Be right with you!</i>\"\n\n", false);
	Render.text("She disappears in the back for a few moments, then returns with " + itype.longName + " that looks as if it were tailor-made for you.\n\n", false);
	Render.text("\"<i>" + itype.value + " gems and it can be yours,</i>\" she says.  ", false);
	if(player.stats.gems < itype.value) {
		Render.text("You count out your gems and realize it's beyond your price range.", false);
		//Goto shop main menu
		doNext(tailorShoppe);
		return;
	}
...
private function weaponBuy(itype:ItemType):void {
	Render.text("", true);
	spriteSelect(80);
	Render.text("The gruff metal-working husky gives you a slight nod and slams the weapon down on the edge of his stand.  He grunts, \"<i>That'll be " + itype.value + " gems.</i>\"", false);
	if(player.stats.gems < itype.value) {
...
private function armorBuy(itype:ItemType):void {
	spriteSelect(64);
	Render.text("", true);
	Render.text("Yvonne gives you a serious look, then nods.  She pulls the armor off a rack and makes a few adjustments, banging away with her massive hammer to ensure a perfect fit.  The entire time, she's oblivious to the movements of her massive breasts, accidentally exposing her impressive nipples multiple times.\n\n", false);
	Render.text("She finishes and turns to you, smiling broadly, \"<i>Now, that will be " + itype.value + " gems, unless you want to change your mind?</i>\"", false);
	if(player.stats.gems < itype.value) {
		Render.text("\n\nYou count out your gems and realize it's beyond your price range.", false);
		//Goto shop main menu
		doNext(armorShop);
...
private function watchUrtaBeABadass():void {
	Render.text("", true);
	kGAMECLASS.urta.urtaSprite();
	Render.text("You shoulder past the bulky centaurs, ignore the rough fur of the nearby wolves and hounds as it brushes against you, and press your way through to the center of the crowd.  Eventually the throng parts, revealing the embattled combatants.  A snarling wolf, nearly eight feet tall, towers over Urta.  The comparatively diminutive fox-woman is girded in light leather armor and dripping with sweat.  The larger wolf-man is staggering about, and his dark brown fur is matted with blood.\n\n", false);

	Render.text("The bigger canid charges, snarling, with his claws extended.  Urta sidesteps and pivots, her momentum carrying her foot around in a vicious kick.  Her foot hits the side of the beast's knee hard enough to buckle it, and the wolf goes down on his knees with an anguished cry.  Urta slips under his arm and twists, turning his slump into a fall.  A cloud of dust rises from the heavy thud of the beast's body as it slams into the cobblestone street.\n\n", false);

	Render.text("Now that it's immobile, you get can get a better look at the defeated combatant, and you're ", false);
	if(player.statusAffects.has("Infested")) Render.text("aroused", false);
	else if(player.stats.cor < 50) Render.text("horrified", false);
	else Render.text("confused", false);
	Render.text(" by what you see.  A pair of thick, demonic horns curve back over the beast's head, piercing through the bottoms of its wolf-like ears.  Its entire body is covered in rippling muscle, leaving you in no doubt of its strength.  Even with a broken knee, the wolf-man is clearly aroused: protruding from a bloated sheath, his massive dog-dick is fully erect, solid black in color, with an engorged knot.  Small white worms crawl over the surface of his penis, wriggling out of the tip and crawling down the length, leaving trails of slime behind them.\n\n", false);

	Render.text("Urta kneels down onto the corrupted wolf's throat, cutting off its air as it foams and struggles under her.  With grim determination, she holds the weakening, demonically-tainted wolf underneath her, leaning all of her body-weight into her knee to keep it down.  It struggles for what seems like ages, but eventually the tainted wolf's eyes roll closed.  Urta nods and rises, watching closely as the beast's breathing resumes.\n\n", false);

	Render.text("She barks, \"<i>Get this one outside the walls before he wakes.  I won't have this corrupted filth in our city, and make sure you get the wards updated.  If he manages to find his way back, you sorry excuses for guards will be going out with him.</i>\"\n\n", false);
	Render.text("A few dog-morphs in similar armor to Urta approach and lash ropes around the wolf's legs.  They hand a line to a centaur, and together the party begins dragging the unconscious body away.  With the action over, the crowd begins dispersing.  More than a few males nod to Urta respectfully.  She keeps her expression neutral and excuses herself to resume her rounds, wiping her hands off on her armor-studded skirt as she leaves.", false);
	doNext(telAdreMenu);
}
...
public function gymDesc():void {
	...
	Render.text("", true);
	Render.text("Even though Ingnam, your hometown, was a large, prosperous village, you never saw a gym before coming to Tel'Adre.  The structure itself has numerous architectural differences from the surrounding buildings: short, waist-high walls, an arched ceiling supported by simple columns, and a sand-covered floor.  Perhaps the only 'normal' rooms inside are the changing stands and bathrooms, which ", false);
	if(player.stats.cor < 35) Render.text("thankfully ", false);
	else if(flags[FlagEnum.PC_FETISH] > 0 || player.stats.cor > 80) Render.text("unfortunately ", false);
	Render.text("have full sized walls to protect their users' privacy.  A breeze blows by, revealing that the open-air design provides great ventilation.  You note a wall of weights of different sizes and shapes, perfect for building muscle and bulking up.  There are also jogging tracks and even a full-sized, grass-covered track out back for centaurs to run on.  Though some of the equipment seems a bit esoteric in nature, you're sure you can make use of most of this stuff.\n\n", false);

	Render.text("Though the gym sees heavy use by the city guard and various citizens, it's not too busy at present.", false);
	...
	Render.text("  There's a centauress in a tank-top just inside the doorway with huge, rounded melons and perky nipples, but she merely coughs to get you to look up and says, \"<i>", false);
	if(flags[FlagEnum.LIFETIME_GYM_MEMBER] == 0) Render.text("10 gems an hour to use the facilities here, or 500 for a life-time membership.</i>\"  She has her hands on her hips, and it looks you'll have to pay ten gems to actually get to use any of this stuff.", false);
	else Render.text("Oh, welcome back " + player.short + ".  Have a nice workout!</i>\"", false);
...
private function buyGymLifeTimeMembership():void {
	...
	Render.text("You fish into your pouches and pull out 500 gems, dumping them into the centaur's hands.  Her eyes widen as she turns and trots towards a counter in the back.  She leans over as she counts, giving you a generous view down her low-cut top at the cleavage she barely bothers to conceal.", false);
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("  It brings a flush to your face that has nothing to do with exercise.  Maybe you'll be able to con her into some alone time later?", false);
...
private function weightLifting():void {
	...
	if(player.fatigue > 75) {
		Render.text("<b>There's no way you could exercise right now - you're exhausted!</b>  ", false);
		if(flags[FlagEnum.LIFETIME_GYM_MEMBER] == 0) Render.text("It'd be better to save your money and come back after you've rested.", false);
		doNext(telAdreMenu);
		return;
	}
	...
	fatigue(25);
	//TEXTS!
	Render.text("You walk up to the weights and begin your workout.  ", false);
	//(< 25 str)
	if(player.str < 25) Render.text("You have to start out on the smaller weights to the left side of the rack due to your strength, but even so, you manage to work up a good burn and a modest sweat.", false);
	//(< 40 str)
	else if(player.str < 40) Render.text("You heft a few of the weights and select some of the ones just to the left of the middle.  It doesn't take you long to work up a sweat, but you push on through a variety of exercises that leave your body feeling sore and exhausted.", false);
	//(< 60 str)
	else if(player.str < 60) Render.text("You smile when you grip a few of the heavier weights on the rack and start to do some lifts.  With a start, you realize you're probably stronger now than Ingnam's master blacksmith, Ben.  Wow!  This realization fuels you to push yourself even harder, and you spend nearly an hour doing various strength-building exercises with the weights.", false);
	//(<80 str)
	else if(player.str < 80) Render.text("You confidently grab the heaviest dumbbells in the place and heft them.  It doesn't take long for you to work up a lather of sweat and feel the burn thrumming through your slowly tiring form.  The workout takes about an hour, but you feel you made some good progress today.", false);
	//(<90)
	else if(player.str < 90) Render.text("You grab the heaviest weights they have and launch into an exercise routine that leaves you panting from exertion.  Setting the weights aside, you flex and marvel at yourself – you could probably arm wrestle a minotaur or two and come out victorious!", false);
	//(else)
	else Render.text("This place barely has anything left to challenge you, but you take the heaviest weights you can get your mitts on and get to it.  By the time an hour has passed, you've worked up a good sweat, but without heavier weights you probably won't get any stronger.", false);
	//Stat changes HERE!
	if(player.str < 90) dynStats("str", .5);
	if(player.tou < 40) dynStats("tou", .3);
	//Body changes here
	//Muscleness boost!
	Render.text(player.modTone(85,5+rand(5)), false);
...
private function goJogging():void {
	...
	fatigue(30);
	//Text!
	Render.text("You hit the jogging track, ", false);
	//(<25 tou)
	if(player.tou < 25) Render.text("but you get so winded you have to stop after a few minutes.  Determined to improve, you force yourself to stay at a fast walk until you can run again.", false);
	//(<40 tou)
	else if(player.tou < 40) Render.text("but your performance isn't that great.  You nearly stop jogging a few times but manage to push through until you're completely exhausted.", false);
	//(<60 tou)
	else if(player.tou < 60) Render.text("and you do quite well.  You jog around for nearly an hour, working up a healthy lather of sweat.  Even your " + player.legs() + " tingle and burn with exhaustion.", false);
	//(<80 tou)
	else if(player.tou < 80) Render.text("and it doesn't faze you in the slightest.  You run lap after lap at a decent clip, working yourself until you're soaked with sweat and fairly tired.", false);
	//(<90 tou)
	else if(player.tou < 90) Render.text("and you have a terrific time.  You can keep yourself just below your sprinting speed for the entire time, though you work up a huge amount of sweat in the process.", false);
	//else)
	else Render.text("and it barely challenges you.  You run at a sprint half the time and still don't feel like you're improving in the slightest.  Still, you do manage to burn a lot of calories.", false);
	//Stat changes HERE!
	...
	//Thickness decrease!
	Render.text(player.modThickness(1,5+rand(2)), false);
	//Muscleness boost!
	Render.text(player.modTone(100,2+rand(4)), false);
	Render.text("\n\nDo you want to hit the showers before you head back to camp?", false);
	if(flags[FlagEnum.BROOKE_MET] == 1) {
		menu();
		addButton(0,"\"Showers\"",sexMachine.exploreShowers);
...
private function yaraSex(girl:boolean = true):void {
	spriteSelect(63);
	Render.text("", true);
	Render.text("Yara makes you comfortable and has you look away while she uses her piercing tools.  It hurts, but she's skilled. Before you know it, your piercing is done!  You move to rise, retaining a bit of modesty", false);
	if(flags[FlagEnum.PC_FETISH] > 0) Render.text(" despite the guilty thrill", false);
	Render.text(".  \"<i>Hold it,</i>\" Yara commands softly, pressing her hand against your " + chestDesc() + " and pushing you back in your chair.  \"<i>Do you think I'll let you get away without some... field testing?</i>\"\n\n", false);

	Render.text("She seems intent on getting some loving - would you like to turn her down, or will you let nature run its course?", false);
...
private function letsDoYaraSex(girl:boolean = true):void {
	...
	//female
		...
		Render.text("Her head stops short, though.  Through your gasping relief, you managed to lock onto her legs.  \"<i>Nice... nice catch,</i>\" is about all Yara manages to share before resuming her purring contentment upside down, limp arms spread across the floor.  After a minute or so, the two of you regain some sort of composure, but the spectacular gymnast from before can only bare to slink around on the ground as she reorients herself.  The most you need to do is fix the back of your chair, lifting it to a more comfortable height.  \"<i>Can you spare one more helping hand here, friend?</i>\" Yara requests, now having at least managed to at least sit up straight.  The two of you exchange a knowing glance as you lift the metal-worker back to her feet.", false);
	}
	player.orgasm();
	doNext(piercingStudio);
}
...
private function yvonneFlirt():void {
	...
	{
		Render.text("Sorry, but you don't look like you'd be much fun.");
		Render.text("</i>\"");
		doNext(armorShop);
		return;
	}
	...
	if(flags[FlagEnum.YVONNE_FUCK_COUNTER] == 0) Render.text("Well, I could use a quick fuck.  If you meant what you said, go change the sign to say 'out' please.");
	else Render.text("You want to go again, huh?  I do love working up a sweat...");
	Render.text("</i>\"");
	//[Fuck] [Nevermind]
...
private function backOutOfYvonneFuck():void {
	spriteSelect(64);
	clearOutput();
	Render.text("You politely decline, not wanting to interrupt her work.  Yvonne sighs and begins to pump the bellows, muttering, \"<i>Then you'd better be buying something!</i>\"");
	doNext(armorShop);
}
//[Fuck]
private function fuckYvonneInZeBlacksmith():void {
	...
	if(player.cockArea(x) < 20) Render.text("runt");
	else if(player.cockArea(x) < 40) Render.text(player.mf("boy","girl"));
	else if(player.cockArea(x) < 60) Render.text("big " + player.mf("boy","girl"));
	else Render.text(player.mf("stud","breeder"));
	Render.text(".</i>\"");
...
private function flirtWithVictoria(itype:ItemType):void {
	...
	Render.text("You take the clothes from her and give them a look over.  Setting them on the counter, you tell her that they're quite nice, but if she's interested you might have something that could fit her quite nicely as well in the back.");
	...
	if(player.lowerBody.cockSpot.list[x].cockLength >= 5)
		Render.text(" until the tip is poking out the top");
	Render.text(".");
	if(player.lowerBody.cockSpot.list[x].cockThickness > 3)
		Render.text("  Her face scrunches up uncomfortably for a moment, your girth straining the seams of her shirt.  With a series of loud pops, her buttons all go flying in different directions, letting her ample, creamy flesh bounce free with a bountiful jiggle.  <i>\"Bloody hell, that was my favorite top...\"</i> she whines for a moment before squeezing her chest back together with her hands.");
	else Render.text("  She presses her arms inward to increase the pressure on your cock even further, and gives you another wide smile.  <i>\"Ready for this, love?\"</i>");

	Render.text("\n\nShe begins to slowly move her disproportionately massive chest up and down your cock, ");
	if(player.lowerBody.cockSpot.list[x].cockLength > 5)
		Render.text("making sure to give the " + player.cockHead(x) + " a quick suck every time it breaches her mounds.");
	else
		Render.text("pressing her mouth down into her ample cleavage so as to give your hidden tip a quick lick every time it draws near.");
