...
private function telAdreTour():void {
	...
	if(player.stats.cor < 25) outputText("blush", false);
...
private function clitPierce():void {
	spriteSelect(63);
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("\"<i>Ohhh, that's going to be suckably cute!</i>\" exclaims Yara, blushing more than a little. \"<i>What kind of piercing would you like?</i>", true);
	else {
...
private function vulvaPierce():void {
	spriteSelect(63);
	piercingLoc = 8;
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Yara explains, \"<i>This is gonna hurt a lot, but I think you'll love how it feels after.  I know I do!  Now what kind of jewelry do you want down-town?</i>\"", true);
	else {
...
public function oswaldPawn():void {
	...
	if(player.findStatusAffect(StatusAffects.Oswald) < 0) {
		...
		if(player.stats.cor < 75) outputText("Who would've thought that seeing someone NOT aroused would ever shock you?", false);
...
private function buyClothes(itype:ItemType):void {
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 30 || player.totalCocks() > 1) outputText("fondle your bulging package", false);
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("rub against your outer lips", false);
...
private function watchUrtaBeABadass():void {
	...
	if(player.statusAffects.has("Infested")) outputText("aroused", false);
	else if(player.stats.cor < 50) outputText("horrified", false);
...
public function gymDesc():void {
	...
	if(player.stats.cor < 35) outputText("thankfully ", false);
	else if(flags[FlagEnum.PC_FETISH] > 0 || player.stats.cor > 80) outputText("unfortunately ", false);
	...
	if(flags[FlagEnum.LIFETIME_GYM_MEMBER] == 0) outputText("10 gems an hour to use the facilities here, or 500 for a life-time membership.</i>\"  She has her hands on her hips, and it looks you'll have to pay ten gems to actually get to use any of this stuff.", false);
...
private function weightLifting():void {
	...
	if(player.fatigue > 75) {
		...
		if(flags[FlagEnum.LIFETIME_GYM_MEMBER] == 0) outputText("It'd be better to save your money and come back after you've rested.", false);
		doNext(telAdreMenu);
		return;
	}
...
private function yaraSex(girl:boolean = true):void {
	...
	if(flags[FlagEnum.PC_FETISH] > 0) outputText(" despite the guilty thrill", false);
...
private function letsDoYaraSex(girl:boolean = true):void {
	...
	if(!girl && player.lowerBody.cockSpot.hasCock()) outputText(" as your " + cockDescript(x) + " rises to attention, her eyes fixed upon the stiffened tool.  You resist the urge to grab her thin-yet-girlish hips and power into her right then and there, curious enough to allow her teasing.", false);
	...
	if(player.lowerBody.balls > 0) outputText("  Her dexterous fingertips brush against your " + ballsDescriptLight() + ", light and fluttery strokes that send shivers coursing through you.  The near-lack of contact is at least as titillating as the less-subtle Marethians you've come across.", false);
...
private function yvonneFlirt():void {
	...
	if(flags[FlagEnum.YVONNE_FUCK_COUNTER] == 0) outputText("Well, I could use a quick fuck.  If you meant what you said, go change the sign to say 'out' please.");
