...
private gangbangVillageFirstGoRound():void {
	...
	if(flags[FlagEnum.DECLINED_TO_VISIT_REBECCS_VILLAGE] == 0 && flags[FlagEnum.TIMES_REFUSED_REBECCS_OFFER] == 0 && flags[FlagEnum.TIMES_IN_DEMON_PIT] == 0) {
		//[Attitude is set to 50]
		flags[FlagEnum.OWCAS_ATTITUDE] = 50;
		flags[FlagEnum.VAPULA_SUBMISSIVENESS] = 50;
	...
	else if(flags[FlagEnum.TIMES_IN_DEMON_PIT] == 0) {
...
private agreeToFollowRebecFirstTime():void {
	...
	//Yes/No
	if(flags[FlagEnum.TIMES_IN_DEMON_PIT] == 0) doYesNo(createCallBackFunction2(acceptRebeccsPlea,true, true),declineRebeccsPlea);
...
private acceptRebeccsPlea(firstTime:boolean = false, sacrificed:boolean = false):void {
	...
	//if(player.armorName != "comfortable clothes") MainScreen.text("  The farmers almost eagerly help you out of your " +player.armorName+ ", supplying you with a set of shabby but comfortable peasant clothes.");
	//[[if corr >70 and have gender]
	if(player.stats.cor > 70 && player.gender > 0) {
		MainScreen.text("\n\nYou lick your lips in anticipation, your ");
		if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(vaginaDescript(0));
...
private intoTheDemonPit(sacrifice:boolean = true):void {
	...
	//[if first time]
	if(flags[FlagEnum.TIMES_IN_DEMON_PIT] == 0) MainScreen.text("\"<i>Awww, look at the poor little thing!  So this is what they gave us this time... Look at [him], everyone!  Do you know you'd be a cute pet?  People here call me Vapula, and I don't think you'll forget that name any time soon.</i>\"");
	//[else if Vapula Submissiveness >50]
	else if(flags[FlagEnum.VAPULA_SUBMISSIVENESS] > 50) MainScreen.text("\"<i>You again?  By Lethice, you must really enjoy it!  You love being a bitch for Vapula, don't you?  Yes you do!</i>\"");
	//[else]
	...
	flags[FlagEnum.TIMES_IN_DEMON_PIT]++;
	flags[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS] = 0;
...
private fightZeDemons(sacrifice:boolean = true):void {
	...
	if (sacrifice) {
		...
		if (player.str < 80 && player.stats.spe < 80) {
...
public loseOrSubmitToVapula():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(", followed by your "+vaginaDescript(0));
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text(", before finally wrapping around your " +cockDescript(0)+ " and stroking it; the friction uncontrollably arouses you, and you find yourself reaching full erectness");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.count() >= 7) {
			MainScreen.text("  Sadly, surrounded as you are, no one will be able to find extra room to ride your remaining cocks; fortunately, you feel warm, long demonic tongues being pressed against your " +cockDescript(6));
	...
	//[if cocks]
	if(player.lowerBody.cockSpot.hasCock()) {
		MainScreen.text("  " + SMultiCockDesc() + " pulsates and throbs");
		//[[if balls]
...
private rapeZeVapula():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		//[if vagina]
		if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("You stick a finger in your own " +vaginaDescript()+ " in order to lubricate it.  ");
		...
		MainScreen.text("!</i>\"  The poor demoness, cheek stained with semen, mutters an apology and proceeds to lick your " +cockDescript(0)+ ", suckling the tip, ");
		if(player.lowerBody.balls > 0) MainScreen.text("fondling your " +ballsDescriptLight()+ ", ");
		//[if multicocks] 
		...
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("In an unholy sex-frenzy, you want more; you start grabbing creatures by groups and alternatively pump in and out of their asses with your " +cockDescript(0)+ ", always hungry for more anal.");
	...
	//Victory rape using vagina/anus [Take Dicks in All Holes](Z)
		...
		//[if squirter] 
		if(player.wetness() >= 5) MainScreen.text("The sound of spattering fluid fills the air as your cunt spurts its juices around the incubus's cock, coating him in your fragrant slime.  ");
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) MainScreen.text(" and your " +breastDescript(0)+ " bounce gently");
...
private owcaMainScreenOn():void {
	...
	if(flags[FlagEnum.OWCAS_ATTITUDE] <= 5 && flags[FlagEnum.OWCA_ANGER_DISABLED] == 0 && flags[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) {
		desperateVillages();
		return;
	}
	//Desperate plea
	if(rand(10) <= 2 && flags[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS] >= 7 && flags[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) {
	...
	//[if attitude > 80]
	if(flags[FlagEnum.OWCAS_ATTITUDE] > 80) MainScreen.text("Villagers greet you as you arrive, praising your courage and your heroism.  People shake your hands or ask you to kiss them, as if they hoped your nobility were somehow contagious.  Once they're done complimenting and flattering you in every way possible, they leave you to your business.");
	//[if 60<=attitude<80]
	else if(flags[FlagEnum.OWCAS_ATTITUDE] > 60) MainScreen.text("Villagers greet you with a friendly smile as you arrive.  Some approach you and thank you for what you've done, before leaving awkwardly.  They're obviously not used to strangers.");
	//[if 40<=attitude<60]
	else if(flags[FlagEnum.OWCAS_ATTITUDE] > 40) MainScreen.text("Villagers seem to accept your presence without comment now; though none go out of their way to greet you, neither do they avoid doing so when you pass by.");
	//[if 20<attitude <= 40]
	else if(flags[FlagEnum.OWCAS_ATTITUDE] > 20) MainScreen.text("Villagers keep looking at you uncomfortably; no one seems to be willing to talk to you apart from Rebecc, your friend.");
	//[if attitude <= 20]
	...
	if(flags[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS] == 7 && flags[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) MainScreen.text("  A week");
	else if(flags[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS] > 7 && flags[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) MainScreen.text("  More than a week");
	if(flags[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS] >= 7 && flags[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) MainScreen.text(" has passed since the last offering to the demons; guarding the pit would certainly help improve your relations with the little town's denizens.");
	//Option: 
	let pit:Function = null;
	let herd:Function = null;
	let tavern:Function = null;
	if(model.time.hours >= 16 && flags[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) {
	...
	if(flags[FlagEnum.OWCAS_ATTITUDE] >= 50) {
	...
	if(flags[FlagEnum.OWCAS_ATTITUDE] >= 10) {
...
public owcaTavern():void {
	...
	MainScreen.text("\nSheep Milk Bottle: " + (180 - flags[FlagEnum.OWCAS_ATTITUDE]) + " gems");
	if((180 - flags[FlagEnum.OWCAS_ATTITUDE]) > player.stats.gems) milk = null;
	
	let goblin:Function = createCallBackFunction(owcaBuySetup,consumables.GOB_ALE);
	MainScreen.text("\nGoblin Ale: " + (60 - Math.round(flags[FlagEnum.OWCAS_ATTITUDE]/2)) + " gems");
	if((60 - Math.round(flags[FlagEnum.OWCAS_ATTITUDE]/2)) > player.stats.gems) goblin = null;
	
	let brew:Function = createCallBackFunction(owcaBuySetup,consumables.BROBREW);
	if(rand(10) > flags[FlagEnum.OWCAS_ATTITUDE]/10) {
		MainScreen.text("\nBro Brew: 2000 gems");
	...
	MainScreen.text("\nMinotaur Cum: " + (300 - flags[FlagEnum.OWCAS_ATTITUDE]) + " gems");
	if((300 - flags[FlagEnum.OWCAS_ATTITUDE]) > player.stats.gems) cum = null;
	MainScreen.text("</i>");
...
private herds():void {
	...
	//attitude +5, int +1
	flags[FlagEnum.OWCAS_ATTITUDE] += 5;
	if(flags[FlagEnum.OWCAS_ATTITUDE] > 100) flags[FlagEnum.OWCAS_ATTITUDE] = 100;
	dynStats("int", 1);
	//[if attitude > 70]
	if(flags[FlagEnum.OWCAS_ATTITUDE] > 70) {
...
private rebeccBathScene():void {
	...
	if(player.gender == 0) MainScreen.text("."); 
		...
		//[if cock]
		if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("; your " + cockDescript(0) + " grows to full erect size");
		if(player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.cockSpot.hasCock()) MainScreen.text(" and ");
		else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("; ");
		if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("your " + vaginaDescript() + " leaks a little trickle of girl-juice");
...
private rapeRebecc(outside:boolean = false):void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.list[0].cockLength > 12) MainScreen.text("  You keep burying more of your " + cockDescript(0) + " deep inside her rectum until you reach her limit and can't fit more cock in.");
		...
		//[if balls]
		if(player.lowerBody.balls > 0) MainScreen.text(", and your " + ballsDescriptLight() + " almost feel sore from colliding with her dribbling cunt");
...
public beatUpOwca():void {
	...
	if(player.stats.cor >= 60 && player.gender > 0 && player.lust >= 33) rape = createCallBackFunction(rapeRebecc,true);
...
private torchOwcaMotherFuckers():void {
	...
	flags[FlagEnum.OWCA_UNLOCKED] = -1;
	dynStats("cor", 15);
	if(player.stats.cor >= 60 && player.gender > 0 && player.lust >= 33) {
...
private forgiveOwca():void {
	...
	flags[FlagEnum.OWCAS_ATTITUDE] = 60;
	flags[FlagEnum.OWCA_ANGER_DISABLED] = 1;
	//To main owca menu
	if (getGame().inCombat)
...
private disbandHorde():void {
	...
	//Attitude set to 100
	flags[FlagEnum.OWCAS_ATTITUDE] = 100;
	flags[FlagEnum.OWCA_SACRIFICE_DISABLED] = 1;
	cleanupAfterCombat();
}
...
private enslaveVapulaWithYourWang():void {
	...
	flags[FlagEnum.OWCAS_ATTITUDE] = 100;
	flags[FlagEnum.VAPULA_HAREM_FUCK] = 1;
	flags[FlagEnum.OWCA_SACRIFICE_DISABLED] = 1;
...
private enslaveVapulaAsACuntWielder():void {
	...
	flags[FlagEnum.VAPULA_FOLLOWER] = .5;
