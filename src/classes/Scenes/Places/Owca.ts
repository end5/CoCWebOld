...
public gangbangVillageStuff():void {
	...
	if(Flags.list[FlagEnum.OWCA_UNLOCKED] == 1) owcaMainScreenOn();
	else gangbangVillageFirstGoRound()
}
//First encounter (Z)
private gangbangVillageFirstGoRound():void {
	MainScreen.clearText();
	/*if(Flags.list[FlagEnum.OWCAS_ATTITUDE] < 5) {
		desperateVillages();
		return;
	}*/
	if(Flags.list[FlagEnum.DECLINED_TO_VISIT_REBECCS_VILLAGE] == 0 && Flags.list[FlagEnum.TIMES_REFUSED_REBECCS_OFFER] == 0 && Flags.list[FlagEnum.TIMES_IN_DEMON_PIT] == 0) {
		//[Attitude is set to 50]
		Flags.list[FlagEnum.OWCAS_ATTITUDE] = 50;
		Flags.list[FlagEnum.VAPULA_SUBMISSIVENESS] = 50;
		MainScreen.text("You wander for quite a while in an endless field of grass before spotting a commotion in the distance.  A myriad of creatures have gathered in the path ahead, waving to you enthusiastically, some still holding tightly to their farm tools.  It's quite an odd sight, all these happy farmhands extending a warm welcome.  You simply accept it as a gesture of kindness and make your way toward the crowd of smiling faces.");
	...
	else if(Flags.list[FlagEnum.TIMES_IN_DEMON_PIT] == 0) {
		MainScreen.text("After wandering across the plains for a while, you spot Rebecc, the girl you met before, amongst a group of fellow farmers; she calls out to you.  \"<i>Hello again, traveler!  You seem lost... would you mind spending some time with us at our village?</i>\"");
	...
	//Second (and subsequent) encounter; only happens once PC accepted being sacrificed once (Z)
		...
		//[if corr >= 50] 
		if(player.stats.cor > 50) MainScreen.text("  You can't help but smile internally.  Let her think that, if she wants.");
...
private agreeToFollowRebecFirstTime():void {
	...
	if(Flags.list[FlagEnum.TIMES_REFUSED_REBECCS_OFFER] == 0) {
		...
		//[if silly mode on]
		if(silly()) MainScreen.text("I-I don't think there's a single grown man or woman in our village that still has their anal virginity.");
		//[if silly mode off]
		else MainScreen.text("Th-they abused us.  They mistreated us.  They would gather in circles and... use us in every possible way.");
		MainScreen.text("</i>\"");
		//[if Corr < 20]
		if(player.stats.cor < 20) {
			...
			//[[if silly mode on]
			if(silly()) MainScreen.text("she'll never have to worry about her asshole again.");
			//[if silly mode off] 
			else MainScreen.text("she will never be abused again.");
			MainScreen.text("  If there's anything that can be done to hold off these evil creatures, you will do it.");
		}
		//[else if corr and libido both > 60]
		else if(player.stats.cor > 60 && player.stats.lib > 60 && player.lowerBody.cockSpot.hasCock()) {
			...
			MainScreen.text("You are uncontrollably turned on by this revelation; your genitals actually hurt at your absurd level of arousal.  Just imagining groups of demons gathered around the helpless villagers, sometimes taking turns with each one and sometimes taking someone all at once, turns you on way more than it should.  You stupidly grin for a few seconds as your mind pictures vivid and lecherous scenes of collective ass-rape, but then you remember this is supposed to be bad news; you quickly recompose yourself and put on a false expression of worry.  Luckily she didn't notice the lust that temporarily filled your eyes; neither does she spot the massive tent growing in your " +player.inventory.armor.displayName+ ".  You ask her if there's anything you can do to meet these demons and 'handle' them on your own.");
		...
		if(player.stats.cor < 50 && player.stats.int >= 50) {
			...
			if(player.stats.lib > 50) MainScreen.text("... and considering what Rebecc meant by 'gratitude'");
			MainScreen.text(".  Maybe it wasn't all an act?");
		}
		//[if corr < 50 and int < 50]
	...
	//Subsequent pleas upon first refusal (Z)
		...
		MainScreen.text("\"<i>So, have you changed your mind yet?  Will you help us?  The demons will want another sacrifice soon, and we are afraid of what might happen if we don't comply!  Please, do accept this time!</i>\"");
		MainScreen.text("\n\nYou know very well what these people want: some meat to appease the demons.  Will you help them?");
	}
	//Yes/No
	if(Flags.list[FlagEnum.TIMES_IN_DEMON_PIT] == 0) doYesNo(createCallBackFunction2(acceptRebeccsPlea,true, true),declineRebeccsPlea);
	else doYesNo(createCallBackFunction2(acceptRebeccsPlea,false, true),declineRebeccsPlea);
}
//Refuse plea (Z)
private declineRebeccsPlea():void {
	MainScreen.clearText();
	MainScreen.text("You can't give in to her demand; you are a proud warrior fighting demons.  You were certainly not meant to serve as a scapegoat or sex-toy.  She sighs in disappointment.  \"<i>Well, I should have expected that.  I wasn't full of hope either.  No hard feelings.</i>\"  Everyone is looking at you with sad or apathetically hostile eyes, making you feel uncomfortable.  You quickly leave the village and return to your camp.");
	//[if more than 40 int, a warning after 4 consecutive refusals]
	//if(Flags.list[FlagEnum.TIMES_REFUSED_REBECCS_OFFER] >= 3 && player.stats.int >= 40) MainScreen.text("\n\nThe villagers are growing impatient of your repeated refusals; you can hear them exchange hushed whispers as you depart.  Failing to grant their request again will probably anger them considerably.");
	//[Attitude is set to 50]
	if(Flags.list[FlagEnum.OWCAS_ATTITUDE] > 5) Flags.list[FlagEnum.OWCAS_ATTITUDE] -= 5;
	Flags.list[FlagEnum.TIMES_REFUSED_REBECCS_OFFER]++;
	doNext(camp.returnToCampUseOneHour);
}
//Accept plea (Z)
private acceptRebeccsPlea(firstTime:boolean = false, sacrificed:boolean = false):void {
	MainScreen.clearText();
	Flags.list[FlagEnum.OWCAS_ATTITUDE] += 10;
	if(Flags.list[FlagEnum.OWCAS_ATTITUDE] > 100) Flags.list[FlagEnum.OWCAS_ATTITUDE] = 100;
	MainScreen.text("You tell the desperate girl that you're going to do as she asks.  As soon as you finish speaking, everyone cheers and applauds you, praising you as their savior.  Rebecc grabs hold of your shoulders and gives you a soft kiss on your lips; she whispers, \"<i>Thank you, champion.  We all owe you.  I promise that you will be rewarded for the heroic deed you're about to undertake.</i>\"");
	...
	//if(player.inventory.armor.displayName != "comfortable clothes") MainScreen.text("  The farmers almost eagerly help you out of your " +player.inventory.armor.displayName+ ", supplying you with a set of shabby but comfortable peasant clothes.");
	//[[if corr >70 and have gender]
	if(player.stats.cor > 70 && player.gender > 0) {
		...
		MainScreen.text(" already slickening with sexual juices; the sight of these restraints brings all kinds of perverted thoughts to your mind: dozens of ways to be used and deliciously abused like a sex-toy by wretched and well-endowed beings.");
	}
	//[else]
...
private intoTheDemonPit(sacrifice:boolean = true):void {
	...
	if(model.time.hours < 21) {
		let passed: number = 21 - model.time.hours;
		MainScreen.text("<b>" + Num2Text(passed) + " " + (passed > 1 ? "hours pass" : "hour passes") + "...</b>\n");
		model.time.hours = 21;
		statScreenRefresh();
	}
	MainScreen.text("You are awakened by a sudden cackling, and open your eyes; you are almost entirely surrounded by darkness, and the dim light provided by the menacing red moon only makes the landscape bleaker.");
	...
	if(sacrifice) MainScreen.text(", tangled in your restraints");
	MainScreen.text(".  She leans down and coos, ");
	//[if first time]
	if(Flags.list[FlagEnum.TIMES_IN_DEMON_PIT] == 0) MainScreen.text("\"<i>Awww, look at the poor little thing!  So this is what they gave us this time... Look at [him], everyone!  Do you know you'd be a cute pet?  People here call me Vapula, and I don't think you'll forget that name any time soon.</i>\"");
	//[else if Vapula Submissiveness >50]
	else if(Flags.list[FlagEnum.VAPULA_SUBMISSIVENESS] > 50) MainScreen.text("\"<i>You again?  By Lethice, you must really enjoy it!  You love being a bitch for Vapula, don't you?  Yes you do!</i>\"");
	//[else]
	else MainScreen.text("\"<i>Fuck, you again?  Tell me it's so!  I've been looking forward to putting you in your place as my newest slut!</i>\"");
	//[(if sacrificed)
	if(sacrifice) {
		//[if str >= 80]
		...
		MainScreen.text("  You're once again feeling regret over letting the villagers hold your equipment, but there's nothing for it now.");
	}
	Flags.list[FlagEnum.TIMES_IN_DEMON_PIT]++;
	Flags.list[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS] = 0;
...
private fightZeDemons(sacrifice:boolean = true):void {
	...
	if (sacrifice) {
		...
		monster.statusAffects.add(new StatusAffect("BowDisabled", 0, 0, 0, 0)));
		if (player.stats.str < 80 && player.stats.spe < 80) {
			monster.statusAffects.add(new StatusAffect("AttackDisabled", 0, 0, 0, 0)));
			monster.statusAffects.add(new StatusAffect("RunDisabled", 0, 0, 0, 0)));
			monster.statusAffects.add(new StatusAffect("PhysicalDisabled", 0, 0, 0, 0)));
		}
		else {
			if (!player.canFly()) monster.statusAffects.add(new StatusAffect("RunDisabled", 0, 0, 0, 0)));
		}
	}
...
public loseOrSubmitToVapula():void {
	MainScreen.clearText();
	MainScreen.text("Vapula taunts you as she circles around you.  \"<i>Look at the slutty pet!  Ain't you a slutty pet?  Yes, you are!  Don't pretend you're not hungry for some fat demon cock, I know you are.</i>\"  As she speaks, the crowd gathers closer.  A few creatures show some temerity, giving you pinches and gropes as they near.  The cock-belted imp unties his tentacle; the horror wriggles and squirms as it drops to the ground and slithers toward you.  The tip of the absurdly long pecker inspects your body, pressing itself against your flesh, massaging you in the most sensual places, wetting you with sap-like pre-cum and teasingly grinding itself against your mouth, and then your " +ButtDescriptor.describeButthole(player));
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(", followed by your "+VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)));
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text(", before finally wrapping around your " +CockDescriptor.describeCock(player, 0)+ " and stroking it; the friction uncontrollably arouses you, and you find yourself reaching full erectness");
	MainScreen.text(".  As more and more hands start playing with your flesh, the succubus grabs your head and gives you a fierce kiss, literally crushing your lips under hers; her mouth tastes like wine and her tongue is driving you over the edge as it intertwines with yours.  Her strong natural scent makes you dizzy and you gradually lose control over your body.  \"<i>Don't worry, darling, it'll be all right...</i>\"");
	MainScreen.text("\n\nSoon you find yourself completely overwhelmed by hot demon hands and dicks touching every part of your body.  In no time at all two imps bend you over and forcefully ram your " +ButtDescriptor.describeButthole(player)+ " with their mammoth peckers.");
	//[ass stretching check]
	player.buttChange(60,true,true,false);
	MainScreen.text("  The double penetration is brutal, unexpected and painful.  Your insides are protesting vigorously against this rough treatment, even though you feel a tingle of pleasure gently tickling your colon at every thrust.  ");
	//[(no Buttslut)
	if(!player.perks.has("Buttslut")) MainScreen.text("No!  You aren't supposed to enjoy it...  ");
	MainScreen.text("You try to cry out but as soon as your mouth opens it is filled with another dick, then a second one.  A third tries to push its way between the first two, stretching your cheeks and making you drool.  Seeing that the monstrous dong won't fit in your already double-stuffed mouth, its owner groans in frustration and proceeds to slap your cheek with it.  He is soon joined by other demons who find the idea very entertaining.");
	MainScreen.text("\n\nIt's a matter of minutes before a dozen hungry omnibuses and incubi are repeatedly cock-slapping your entire body, hitting every part of you with their heavy meat, grinding their rods against every fold and curve of your flesh and staining it with seminal fluids.  Your poor " +ButtDescriptor.describeButt(player)+ ", already abused by two giant pricks thrusting back and forth at an unnatural pace, is now the prey of numerous hands and full, erect dicks slapping it in every possible way, smearing it with pre-cum and sweat as they run across your tender skin.  You can't see anything: your eyesight has been blocked by a never-ending row of wriggling cocks.  Nor can you hear anything over the sound of a full horde of libidinous demons panting and moaning as they abuse their fuck-toy in an overwhelming orgy of pleasure; besides, a pair of imps are rubbing the tips of their dongs against your ears, as if they wanted to fill them with seed.");
	...
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("  It wraps around [eachCock] for a while, jerking it for a bit and rubbing its tip against yours.");
	//[if antennae]
	if(player.upperBody.head.antennae > AntennaeType.NONE) MainScreen.text("  Your antennae are being harshly pulled and twisted; some imps, in a crazy show of libertinism, start inserting your sensitive peduncles down their bloated urethrae. The intimate friction and the sudden jolts when the internal walls slather your appendages in slick, hot pre-cum are driving you mad with irregular shots of unbearable pleasure.");
	//[if horns]
	if(player.upperBody.head.horns > 0) MainScreen.text("  You feel your horns being used as leverage to slap your head with even more rock-hard dick.  The impacts stun you until your whole forehead feels numb and coated with a mixture of ballsweat and pre-cum.");
	//[if vagina
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		MainScreen.text("\n\nYou suddenly feel a sharp sensation: your womanhood is finally being penetrated.");
		player.cuntChange(60,true,true,false);
		MainScreen.text("  A dick found a way into your " +VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0))+ " and is pushing further inside.  Wait, what's this?  A second, and then a third!  There's no way these titanic columns of flesh will... this is too much... you fear you will be torn in half, but at the last moment, you feel someone spilling the content of a flask over your nether-lips.  As if you had lost control of your " +VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0))+ ", it automatically starts leaking girl-cum in prodigious amounts, and you let out a stifled moan as a delicious shiver runs teasingly across your body.  Your fuck-hole widens under the corrupting effect of the weird mixture you were administered.  ");
		...
		if(player.statusAffects.get("BonusVCapacity").value1 < 200) player.statusAffects.get("BonusVCapacity").value1 = 15;
		MainScreen.text("As soon as new space is created, it is immediately filled by a pussy-hungry cock.  The three cocks slide effortlessly inside you and start thrusting energetically, vying in intensity and ferocity with the peckers ramming your " +ButtDescriptor.describeButthole(player)+ ".  The penetration of both holes is almost too much too handle, but you finally get used to it as your own anal and vaginal muscles try their best to milk as much spunk as possible from these cum-tubes.  Even though you are being violated in every hole, the raw and powerful sensation is quite pleasurable; it feels so right to be used this way, so full of demon wang.  You don't have to focus on anything, just to enjoy the sheer amazing feeling of being pounded by many pistons at once.  You squirt over and over again as the dicks and the fluid force you into a series of wild female orgasms.");
		//[crank up vaginal wetness one level, why not?]
		if(player.lowerBody.vaginaSpot.get(0).vaginalWetness < 5) player.lowerBody.vaginaSpot.get(0).vaginalWetness++;
	}
	//[if cocks]
	if(player.lowerBody.cockSpot.hasCock()) {
		MainScreen.text("\n\nA soft brush against your " +CockDescriptor.describeCock(player, 0)+ " warns you that your crotch is going to get some more love.  At least four hands are caressing it, helping you rise and thicken until it can't grow anymore.  After a while of playful stroking, the hands are suddenly gone, only to be replaced by a fluid-gushing cunt.  It impales itself on your " +CockDescriptor.describeCock(player, 0)+ " at full force, driving a very whorish and high-pitched cry from its owner.");
		//[if more than 1 and less than 4 cocks]
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("  She is soon joined by other horny girls, too eager to wait for a piece of your crotch to play with.  They all comfortably position themselves, positioning their pussy or anus atop each of your cocks before riding you like there's no tomorrow.");
		//[if 4 to 7 cocks]
		if(player.lowerBody.cockSpot.count() >= 4) MainScreen.text("  Nevertheless, they aren't satisfied with a single insertion and most grab hold of one of your extra cocks before forcefully stuffing it in their other hole, howling and thrashing all the more as they are penetrated a second time.");
		//[if 7 cocks or more]
		if(player.lowerBody.cockSpot.count() >= 7) {
			MainScreen.text("  Sadly, surrounded as you are, no one will be able to find extra room to ride your remaining cocks; fortunately, you feel warm, long demonic tongues being pressed against your " +CockDescriptor.describeCock(player, 6));
			if(player.lowerBody.cockSpot.count() > 8) MainScreen.text(" as well as your remaining pricks");
			MainScreen.text(".  The tongues expertly lick your meat and wrap themselves around it, and questing lips give it multiple kisses before one pair suddenly loses patience and deepthroats you with voracity; the tightness of these lips makes them feel like a very efficient cock-ring.  However, it is hard to focus on the sweet warmth provided by these avid mouths when all your other cocks are being clamped by powerful, inhuman vaginal muscles.");
		}
		...
		MainScreen.text(", as if every inch had to be taken care of at every instant.  As you slide back into the pussy, the hands return to crawling over your body, caressing your groin and staining your belly with pre-cum - yours and others.");
	...
	if(player.lactationQ() > 0) MainScreen.text("\n\nLost in your feverish state of arousal, you feel your nipples harden and a thin trickle of milk comes out.  A couple of demons spot this new source of fluids and rush to your " +BreastDescriptor.describeAllBreasts(player)+ ", wolfishly suckling them.  Their dexterous tongues keep teasing your nipples, stimulating you further as they drink your essence.  Your milk-udders are being roughly groped and licked by these careless creatures, oblivious of everything but your mounds.");
	...
	//[if cocks]
	if(player.lowerBody.cockSpot.hasCock()) {
		MainScreen.text("  " + CockDescriptor.describeMultiCockSimpleOne(player, true) + " pulsates and throbs");
		//[[if balls]
		if(player.lowerBody.balls > 0) MainScreen.text(", and your balls swell and boil");
		MainScreen.text("; a gigantic spooge-flow pushes its way up your urethra and spurts outside in the waiting love-tunnels");
		if(player.lowerBody.cockSpot.count() > 7) MainScreen.text(" and mouths");
		MainScreen.text(".  The violent cumshot almost instantly produces a shiver of orgasm once again and vaginal walls contract furiously, determined to absorb as much of your baby-batter as possible.");
		//[if cum production is massive]
		if(player.cumQ() > 1500) MainScreen.text("  The effort appears to be vain, as the absurdly high volume you've ejected is enough to completely pack every hole; the remaining jism spills on the tainted ground, soaking it further.");
	}
	MainScreen.text("\n\nAfter what seems like hours of forceful penetration, the gigantic members inside you release a milky torrent of corrupted seed, completely packing your colon through your " +ButtDescriptor.describeButthole(player));
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" and filling your womb to its maximum capacity");
	MainScreen.text(".  The remaining cum spurts outside and dribbles onto your [legs].  Your asshole clenches, squeezing the dongs inside you and milking their very last drops of spunk.");
	//[if vagina]
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("\n\nYour own " +VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0))+ " clenches too, doing its best to waste as little semen as possible.  Your muscles quiver as they feel the incoming last release.  Then, in climax, you squirt your own girl-cum, splattering the demon dicks with your juice.");
	//[if lactating breasts]
	...
	Flags.list[FlagEnum.VAPULA_SUBMISSIVENESS] += 10;
	if(Flags.list[FlagEnum.VAPULA_SUBMISSIVENESS] >= 90 && Flags.list[FlagEnum.VAPULA_SUBMISSIVENESS] < 100) MainScreen.text("\n\n<b>You're starting to get dangerously used to this kind of treatment.  Your holes are being stretched to accommodate monstrous sizes and don't hurt that much anymore.  You feel like you could stand this as long as you need to with ease... maybe it's time to stop volunteering?</b>");
	if(Flags.list[FlagEnum.VAPULA_SUBMISSIVENESS] >= 100) doNext(slaveToVapulaBadEnd);
	else if(player.statusAffects.has("LostVillagerSpecial")) doNext(morningAfterRape);
	else doNext(wakeUpAfterDemonGangBangs);//WAKE UP
	player.orgasm();
	player.stats.lib += 1;
player.stats.sens += 2;
player.stats.cor += 3;
	Flags.list[FlagEnum.REBECCS_LAST_PLEA] = 0;
}
...
private wakeUpAfterDemonGangBangs():void {
	...
	model.time.hours = 7;
	model.time.days++;
	MainScreen.text("When you wake up, you are alone, and your restraints are broken.  You are sloshing in a pool of stinky juices; your mouth and ears are still full of it.  Your whole body is covered with a thin white layer that must certainly be dried spooge.  Underneath, you're nothing but bruises and every movement seems to hurt.  A few meters away, outside the pit, you notice your items and your gear.  The village itself appears to be empty... your best assumption is that the residents are hiding, either from shame at having sacrificed you or from awkwardness at the prospect of talking to a sloshing, crusty cumdumpster.  Wearily, you head back to your camp.");
...
public defeetVapulasHorde():void {
	MainScreen.clearText();
	Flags.list[FlagEnum.REBECCS_LAST_PLEA] = 0;
	if(Flags.list[FlagEnum.VAPULA_SUBMISSIVENESS] <= 0) {
		subdueVapula();
		return;
	}
	//Sacrificed and bound
	if(monster.statusAffects.has("AttackDisabled")) {
	   if(monster.HP < 1) MainScreen.text("You grin wickedly as the last demons fall, defeated.  Some of the errant blows have broken and mangled the links of your chains, and you find you can get free with a bit of additional effort.");
		//[if won by Lust]
		else MainScreen.text("You grin wickedly as the demons give up the fight, too turned on to care about you.  One even begins unfastening your bindings, hopeful desperation glinting in her eyes as she attempts to entice you with her long, thick nipples and enormous, dripping gash.");
	}
	//Not
	else {
		if(monster.HP < 1) MainScreen.text("You grin wickedly as the last demons fall, defeated.");
		//[if won by Lust]
		else MainScreen.text("You grin wickedly as the demons give up the fight, too turned on to care about you.  One even has hopeful desperation glinting in her eyes as she attempts to entice you with her long, thick nipples and enormous, dripping gash.");
	}
	//[(requires genitals and and corr >60)
	if(player.stats.cor > 60 && player.gender > 0) {
		...
		Flags.list[FlagEnum.VAPULA_SUBMISSIVENESS] -= 5;
		cleanupAfterCombat();
	}
...
private rapeZeVapula():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		MainScreen.clearText();
		MainScreen.text("The demon horde struggles before you.  With a disdainful smile, you fully expose your genitals for everyone to see.  ");
		//[if cocks]
		if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("\n\nYou gently stroke your " +CockDescriptor.describeCock(player, 0)+ ", bringing it to full erectness.  ");
		//[if vagina]
		if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("You stick a finger in your own " +            VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0))+ " in order to lubricate it.  ");
		MainScreen.text("The defeated horde watches you touch yourself with avid, almost desperate eyes.  You keep teasing yourself, saving your lust for the torrid storm of rape you're about to unleash on the poor creatures who dared attack you.  You walk among the battered bodies, looking for the prey that will best suit you; then you find her.  Vapula is lying here, her purple skin masking ");
		if(monster.HP < 1) MainScreen.text("the many bruises she's received");
		else MainScreen.text("the constant flushing due to intense arousal");
		MainScreen.text(".  You harshly grab her hand and grunt, \"<i>On your feet, cunt.</i>\"  She is too stunned to protest and weakly stands up, her leather straps torn to completely reveal her voluptuous body.  You feel the sudden urge to take her right now and fuck her like a ");
		if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("bitch in heat");
		else MainScreen.text("centaur in rut, abusing and filling her teasing cunt and ass with every protuberance you have");
		MainScreen.text(".  However, you have better plans for her.");
		...
		MainScreen.text("\n\nWith lazy nonchalance you grab hold of another demon crawling at your feet, another succubus; although not so tall or big-breasted as Vapula, her provocative curves are a delight to your eyes.  You bend her over and, without ceremony, start working her tight pucker, filling her insides with your " +CockDescriptor.describeCock(player, 0)+ ".");
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("  Seeing that she has extra room left, you grab another of your dongs and put it at the entrance of her dripping cunt before forcing it in.");
		...
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(" in front of the once-powerful dominatrix, grunting and panting as you accelerate the pace, until you finally reach your climax and release rope after rope of jism in your fuck-toy, completely packing her backdoor");
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text(" and her love-tunnel");
		MainScreen.text(".");
		//[if massive cum production]
		if(player.cumQ() > 1000) MainScreen.text("  Some of it spurts by little jets outside of her, even as she instinctively contracts her muscles to keep as much spooge as possible and her belly accomodates by bloating out absurdly.");
		MainScreen.text("  Needless to say, watching this rough ");
		if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("anal");
		else MainScreen.text("double-penetration");
		MainScreen.text(" session has brought Vapula to a new level of arousal; she is struggling to free her arms and finger herself, but her tight restraints only allow her to wriggle uncomfortably.  Her pussy is gushing of its own and she whimpers from time to time, unable to control her lust.");
		MainScreen.text("\n\nYou throw away your expendable");
		if(player.cumQ() > 1000) MainScreen.text(", expandable");
		MainScreen.text(" cumdumpster and grab another slut.  This time you feel like you could use a blowjob to clean off the demonic sweat and slime from your cock; you forcefully open her jaw and shove your " +CockDescriptor.describeCock(player, 0)+ " down her throat without any care.  The sudden influx of dickflesh blocking her air-flow makes her choke and cough, but a withdrawal and a good cock-slap reminds her of her place.  \"<i>You fucking bitch!  You goddamn worthless slut!  What good are you if you can't even give proper head?  Lick, you whore!  I want your tongue all over my crotch!  You can breathe when you've properly moistened my ");
		if(player.lowerBody.balls > 0) MainScreen.text("balls");
		else MainScreen.text(player.skin());
		MainScreen.text("!</i>\"  The poor demoness, cheek stained with semen, mutters an apology and proceeds to lick your " +CockDescriptor.describeCock(player, 0)+ ", suckling the tip, ");
		if(player.lowerBody.balls > 0) MainScreen.text("fondling your " +BallsDescriptor.describeBalls(true, true, player)+ ", ");
		//[if multicocks] 
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("alternatively deepthroating or jerking every cock of yours, ");
		MainScreen.text("warming up your dickflesh with her demonic tongue.  You enjoy her ministrations while staring Vapula in the eyes; she has trouble looking back at you as her body trembles and her eyes widen at your sheer display of depravity.  Your semen-dribbling shaft");
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s harden and thicken ");
		else MainScreen.text(" hardens and thickens ");
		MainScreen.text("again as the demoness sucks you off; Vapula can't help but take a quick glance at it, but you pretend not to notice.  After all, she's only teasing herself for you.");
		...
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("In an unholy sex-frenzy, you want more; you start grabbing creatures by groups and alternatively pump in and out of their asses with your " +CockDescriptor.describeCock(player, 0)+ ", always hungry for more anal.");
		...
		if(silly()) MainScreen.text("  As you look back, you snicker, \"<i>Ass for ass, assholes.</i>\"");
	...
	//Victory rape using vagina/anus [Take Dicks in All Holes](Z)
		...
		MainScreen.text("\n\nWith lazy nonchalance you grab hold of another demon crawling at your feet, an incubus; shorter and younger than Vapula, his tight, pert body is a delight to your eyes.  Without ceremony you shove him to the ground and grind your " +            VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0))+ " into his crotch.  He looks at you, semi-comatose and bewildered, as you get a rise out of his dick, which hardens against your damp strokes.  You pin him down as you sink onto his red length, sighing as you rapidly engulf him, then begin to ride him ruthlessly.  ");
		player.cuntChange(40,true,true,false);
		if(player.vaginalCapacity() > 80) MainScreen.text("His cock is lost in your vast cunt and you work him as hard as you can to feel him against your walls, his body convulsing and dazed eyes rolling against the wet, savage rape.  ");
		MainScreen.text("You keep thrusting back and forth, treating the worthless horned boy like a disposable dildo.  As you pump him, you look at Vapula, warning her, \"<i>Don't turn your head.  If you ever look away or close your eyes you will regret it.</i>\" Overwhelmed by your dominance and subdued by the defeat, Vapula can only nod.");
		MainScreen.text("\n\nYou keep working the incubus's hot cock in front of the once-powerful dominatrix, panting as you accelerate the pace until you finally reach your climax, clenching your thighs tightly to his fit body as your " +            VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0))+ " clenches and spasms.  ");
		//[if squirter] 
		if(player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 5) MainScreen.text("The sound of spattering fluid fills the air as your cunt spurts its juices around the incubus's cock, coating him in your fragrant slime.  ");
		MainScreen.text("Against your relentless milking pressure he stands no chance and he moans raggedly as he cums in tandem, still incapable of moving his body as your mixed juices dribble down him.");
		...
		MainScreen.text("\n\nYou throw your expendable cock toy to one side and grab another slut, this time a succubus.  Your semen-dribbling cunt is now in need of a cleaning, and without any ado you sit yourself straight down onto her face, banging the back of her head against the ground.  She whines into your " +            VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0))+ ", to which you respond by pulling her hair.  \"<i>You fucking bitch!  You worthless slut!  What good are you if you can't even munch rug properly?  Lick, you whore!  I want your tongue to clean up what your useless fuckhead of a friend couldn't keep to himself.</i>\"  The poor demoness mutters an apology and proceeds to lap up the juices drooling out of your hole, licking your labia and mons clean of what the incubus left, darting her wet muscle across your " +VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0))+ " before reaching into your vagina, carefully teasing out and drinking every drop of jizz she can find, making your own juices run afresh.  You enjoy her ministrations while staring Vapula in the eyes; she has trouble looking back at you as her body trembles and her eyes widen at your sheer display of depravity.  Your " +BreastDescriptor.describeNipple(0)+ "s harden");
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) MainScreen.text(" and your " +BreastDescriptor.describeBreastRow(player.upperBody.chest.get(0))+ " bounce gently");
		MainScreen.text(" as you grind your snatch into the succubus's face; Vapula can't help but take a quick glance at your oozing, slippery cunt, but you pretend not to notice.  After all, she's only teasing herself for you.");
		...
		MainScreen.text("\n\n\"<i>You!</i>\" you snap. \"<i>Fuck my ass.  Do a good job or I'll wring your neck.  The rest of you worthless spare pricks... gather around.</i>\"  You grip the incubus beneath you with your " +LowerBodyDescriptor.describeHips(player)+ " as small, uncertain hands grasp you around your waist and something hot oozes moisture into your " +ButtDescriptor.describeButthole(player)+ ".  You thrust forwards and then backwards, simultaneously working the incubus cock whilst driving the imp's meat into your bowels.");
		player.buttChange(60,true,true,false);
		MainScreen.text("  Around you more dicks present themselves as other demons slowly draw closer, and with the cock inside you rubbing your sensitive inner walls backwards and forwards you enthusiastically grab two other turgid members and pump them in tandem.  One of the dicks you have grabbed is a sensitive cat dick and the owner is quickly moaning as your fingers brush his hooks; as you feel him reach his peak you ruthlessly jerk him around so he is facing Vapula, and he helplessly spurts his load onto her jiggling frame.  The imp begins to pump harder as he finds his rhythm, stretching your ass wide as he takes you to his plug, rubbing against the huge incubus cock trapped in your " +VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0))+ ".  You laugh breathlessly and manically as the other demon in your grasp helplessly falls over his own peak, further soaking the horny ex-domniatrix.");
		//[cunt nipples] 
		if(player.upperBody.chest.hasFuckableNipples()) MainScreen.text("  Your freakish nipples moisten with excitement to the smell of man juice, and you feel your libido ratcheting up even higher.  Bending forward, you grab two other imps with smaller dicks, quickly guiding their lengths into your nipplecunts; they are small enough that you can mash their bodies against your soft flesh, doing all the work for them as your sensitive nubs are penetrated.");
...
private owcaMainScreenOn():void {
	...
	if(Flags.list[FlagEnum.REBECCS_LAST_PLEA] == 1 && !kGAMECLASS.vapula.vapulaSlave()) {
		rebeccsLastPlea();
		return;
	}
	if(Flags.list[FlagEnum.OWCAS_ATTITUDE] <= 5 && Flags.list[FlagEnum.OWCA_ANGER_DISABLED] == 0 && Flags.list[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) {
		desperateVillages();
		return;
	}
	//Desperate plea
	if(rand(10) <= 2 && Flags.list[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS] >= 7 && Flags.list[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) {
	...
	//[if attitude > 80]
	if(Flags.list[FlagEnum.OWCAS_ATTITUDE] > 80) MainScreen.text("Villagers greet you as you arrive, praising your courage and your heroism.  People shake your hands or ask you to kiss them, as if they hoped your nobility were somehow contagious.  Once they're done complimenting and flattering you in every way possible, they leave you to your business.");
	//[if 60<=attitude<80]
	else if(Flags.list[FlagEnum.OWCAS_ATTITUDE] > 60) MainScreen.text("Villagers greet you with a friendly smile as you arrive.  Some approach you and thank you for what you've done, before leaving awkwardly.  They're obviously not used to strangers.");
	//[if 40<=attitude<60]
	else if(Flags.list[FlagEnum.OWCAS_ATTITUDE] > 40) MainScreen.text("Villagers seem to accept your presence without comment now; though none go out of their way to greet you, neither do they avoid doing so when you pass by.");
	//[if 20<attitude <= 40]
	else if(Flags.list[FlagEnum.OWCAS_ATTITUDE] > 20) MainScreen.text("Villagers keep looking at you uncomfortably; no one seems to be willing to talk to you apart from Rebecc, your friend.");
	//[if attitude <= 20]
	else MainScreen.text("Villagers here appear to be openly hostile.  You keep hearing muffled whispers as you pass by, and people look at you rancorously.  They don't bother you, though.");
	//end variable display
	MainScreen.text("  The village is rather small.  A dozen humble thatched cottages are standing here and there, linked by dirt tracks and enclosed with small palisades.  There seems to be very little activity.");
	...
	if(Flags.list[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS] == 7 && Flags.list[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) MainScreen.text("  A week");
	else if(Flags.list[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS] > 7 && Flags.list[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) MainScreen.text("  More than a week");
	if(Flags.list[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS] >= 7 && Flags.list[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) MainScreen.text(" has passed since the last offering to the demons; guarding the pit would certainly help improve your relations with the little town's denizens.");
	//Option: 
	let pit:Function = null;
	let herd:Function = null;
	let tavern:Function = null;
	if(model.time.hours >= 16 && Flags.list[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) {
	...
	if(Flags.list[FlagEnum.OWCAS_ATTITUDE] >= 50) {
	...
	if(Flags.list[FlagEnum.OWCAS_ATTITUDE] >= 10) {
...
public owcaTavern():void {
	MainScreen.clearText();
	MainScreen.text("The tavern is nice and cozy; there are a few tables and chairs scattered around in no ordered pattern, and most clients here appear to belong to the same species.  By the crude wooden bar, you see a list of all the current drinks on sale:\n<i>");
	//SheepMk
	let milk:Function = createCallBackFunction(owcaBuySetup,consumables.SHEEPMK);
	MainScreen.text("\nSheep Milk Bottle: " + (180 - Flags.list[FlagEnum.OWCAS_ATTITUDE]) + " gems");
	if((180 - Flags.list[FlagEnum.OWCAS_ATTITUDE]) > player.stats.gems) milk = null;
	
	let goblin:Function = createCallBackFunction(owcaBuySetup,consumables.GOB_ALE);
	MainScreen.text("\nGoblin Ale: " + (60 - Math.round(Flags.list[FlagEnum.OWCAS_ATTITUDE]/2)) + " gems");
	if((60 - Math.round(Flags.list[FlagEnum.OWCAS_ATTITUDE]/2)) > player.stats.gems) goblin = null;
	
	let brew:Function = createCallBackFunction(owcaBuySetup,consumables.BROBREW);
	if(rand(10) > Flags.list[FlagEnum.OWCAS_ATTITUDE]/10) {
		MainScreen.text("\nBro Brew: 2000 gems");
		if((2000) > player.stats.gems) brew = null;
	}
	else brew = null;
	
	let cum:Function = createCallBackFunction(owcaBuySetup,consumables.MINOCUM);
	MainScreen.text("\nMinotaur Cum: " + (300 - Flags.list[FlagEnum.OWCAS_ATTITUDE]) + " gems");
	if((300 - Flags.list[FlagEnum.OWCAS_ATTITUDE]) > player.stats.gems) cum = null;
	MainScreen.text("</i>");
...
private owcaBuySetup(item:ItemType):void {
	if(item == consumables.SHEEPMK) buyOwcaShit(item,(180 - Flags.list[FlagEnum.OWCAS_ATTITUDE]));
	else if(item == consumables.GOB_ALE) buyOwcaShit(item,(60 - Math.round(Flags.list[FlagEnum.OWCAS_ATTITUDE]/2)));
	else if(item == consumables.BROBREW) buyOwcaShit(item,2000);
	else buyOwcaShit(item,(300 - Flags.list[FlagEnum.OWCAS_ATTITUDE]));
}
...
private buyOwcaShit(bleh:ItemType,price:number = 0):void {
	...
	statScreenRefresh();
	MainScreen.text("The bartender hands you a bottle and grabs your gems before attending other clients, leaving you to your own business.\n\n");
...
private herds():void {
	MainScreen.clearText();
	MainScreen.text("The herders greet you warmly.  \"<i>Welcome, champion!  It is indeed a pleasure to have the honor to talk to such a noble and revered hero.</i>\"  You reply with equal grace and ask in a more formal tone if there's anything you can do to help them out.");
	...
	//attitude +5, int +1
	Flags.list[FlagEnum.OWCAS_ATTITUDE] += 5;
	if(Flags.list[FlagEnum.OWCAS_ATTITUDE] > 100) Flags.list[FlagEnum.OWCAS_ATTITUDE] = 100;
	player.stats.int += 1;
	//[if attitude > 70]
	if(Flags.list[FlagEnum.OWCAS_ATTITUDE] > 70) {
...
private zePit():void {
	MainScreen.clearText();
	MainScreen.text("You announce to all the villagers present that you're going to guard the pit as well as the village's entrance against the demons, trusting them to spread the news; they seem glad that you took this initiative on your own.  Checking your equipment, you head toward the pit, ready to stand your ground against the lewd horde.");
...
private rebeccMenu():void {
	MainScreen.clearText();
	MainScreen.text("You knock and step into the old shack, accustoming yourself to the meager light.  As soon as you close the door behind you, you spot Rebecc stepping out of a little backroom.");
...
private rebeccBathScene():void {
	MainScreen.clearText();
	MainScreen.text("She grabs your hand and leads you to her bathroom, a tiny place almost entirely occupied by an enormous circular container acting as a bathtub, full of hot steaming milk.  You watch her, puzzled.");
	...
	if(player.gender == 0) MainScreen.text("."); 
		...
		//[if cock]
		if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("; your " + CockDescriptor.describeCock(player, 0) + " grows to full erect size");
		if(player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.cockSpot.hasCock()) MainScreen.text(" and ");
		else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("; ");
		if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("your " +             VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " leaks a little trickle of girl-juice");
		MainScreen.text(".  She doesn't seem to notice the effect she has on you - or so you think.  Suddenly, you feel a hand darting between your legs.");
	}
...
private rapeRebecc(outside:boolean = false):void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		MainScreen.text("\n\nThe sight of her glorious rear is too tempting a target to resist, and you quickly shove the tip of your " + CockDescriptor.describeCock(player, 0) + " between her plush cheeks, bracing yourself for torrid anal penetration.  She keeps whining, unable to break free of your iron grasp.  In her constant struggles you notice an opening between her buttocks - you thrust inside with all your might, devastating her backdoor entrance, stretching her holes, tearing her insides and grinding against her rugged anal walls in a extremely rough way.");
		//[if cock is too long, >12 inches]
		if(player.lowerBody.cockSpot.get(0).cockLength > 12) MainScreen.text("  You keep burying more of your " + CockDescriptor.describeCock(player, 0) + " deep inside her rectum until you reach her limit and can't fit more cock in.");
		else MainScreen.text("  Eventually the entirety of your prick is lodged inside her lush asshole.");
		MainScreen.text("  She trills and bleats in pain and flounders about in your arms, futilely trying to avoid the incoming butt-rape.  You lock her arms with yours, implacably kneading her tits, and her shivering flesh brings the most exquisite contact to your fingers; you toy with her nipples as well, making her scream savagely in forced pleasure.");
		MainScreen.text("\n\nAfter a few minutes of resistance, her body tenses up and you ease yourself into effectively butt-fucking her.  You pull her white fuzzy hair, seize her squeezable hips for more leverage and start raping her ass; the thrusts come hard and fast, almost bringing your " + CockDescriptor.describeCock(player, 0) + " out before jamming it all the way inside again.  This forceful and tigerish motion makes her shriek in excruciating ecstasy; she groans and moans rhythmically at the pace of your regular ass-pounding.  The torment you're inflicting on her poor interior walls is too much to bear and she twists violently to escape the dick-beast dominating her pucker; however, all she can do is wave her appreciable butt, bringing extra sensations to your " + CockDescriptor.describeCock(player, 0)+ " and adding extra spice to your already ardent anal-rape.");
		MainScreen.text("\n\nThe cadenced wobbling of her rump as well as your constant pumping is bringing you to a higher state of pleasure; the heavenly tingle your " + CockDescriptor.describeCock(player, 0) + " is receiving is making you heedless of anything else but the burning heat of her insides cooking your meat.  You feel your lust rise as you keep riding the buttslut below you, ignoring her muffled complaints and whining, her tight confines gradually taking over any control you have over your rod.  You pound Rebecc's ass as if your life depended on it, clutching her curvy hips instinctively like an imp in rut.  Your groin impacts her buttocks with a loud SLAP at every thrust");
		//[if balls]
		if(player.lowerBody.balls > 0) MainScreen.text(", and your " + BallsDescriptor.describeBalls(true, true, player) + " almost feel sore from colliding with her dribbling cunt");
		MainScreen.text(".  Finally, when you think you're about to pass out from exhaustion, you cum in a powerful jet, literally blasting Rebecc's ass with your spooge, globs of  goo being propelled like projectiles into her colon and splattering her backside with huge white smears.");
		if(player.cumQ() > 1500) MainScreen.text("  After completely filling up her rectum, your baby-batter erupts in thick spurts from her anus despite her instinctive ass-clenching efforts to hold as much semen as possible inside.  The sheer might of your cumshot is enough to send Rebecc over the edge, and she releases an absolutely torrential stream of girl-cum, squirting against her will all over your groins and mixing her own fluids with yours in a pathetic show of sluttiness.");
		MainScreen.text("  She unconsciously milks you of all your spunk, her anal muscles working on their own to massage your meat and extract every drop from your urethra.");
		MainScreen.text("\n\nWith a last, satisfied sigh, you fall over, " + CockDescriptor.describeCock(player, 0) + " deeply buried inside Rebecc's butt as you feel her warm, curvy body slowly waggling below you.  ");
		if(outside) MainScreen.text("Some of the villagers are apathetically watching you, too stunned and bruised to intervene.  ");
		MainScreen.text("You silently remove your prick from your girl's ass, watch her contentedly as her exposed rectum is literally oozing with spooge and walk back to your camp.");
		MainScreen.text("\n\nThis is a more-than-appropriate goodbye gift");
		if(outside) MainScreen.text(" to these miserable people, and even moreso");
		MainScreen.text(" to the manipulative slut who kept teasing you.  Serves her right, to use her body in the most provocative ways and never let you actually have your way with her.  You're unlikely to ever see her again.");
	...
	//Rebecc Female Rape Scene (Z)
		...
		if(outside) MainScreen.text("Some of the villagers are apathetically watching you, too stunned and bruised to intervene.  ");
		MainScreen.text("You silently pick yourself up and dress, admiring the quivering, crying handiwork at your feet, before walking back to your camp.  This is a more-than-appropriate goodbye gift");
		if(outside) MainScreen.text(" to these miserable people, and even moreso");
		MainScreen.text(" to the manipulative slut who kept teasing you.  Serves her right, to use her body in the most provocative ways and never let you actually have your way with her.  You're unlikely to ever see her again.");
		MainScreen.text("\n\n(<b>Owca removed from the places menu.</b>)");
	}
...
private desperateVillages():void {
	MainScreen.clearText();
	MainScreen.text("As you approach the group of huts, you hear a vague rumble, as if many people were talking at the same time.  Walking closer, you see all the villagers gathering outside and arguing violently; among them you spot your friend, Rebecc.  As soon as she sees you she hurries over with desperate, wet eyes.");
...
private fightZeVillagers():void {
	...
	monster.statusAffects.add(new StatusAffect("GenericRunDisabled",0,0,0,0)));
	playerMenu();
...
private torchUpVillagersAndLeave():void {
	MainScreen.clearText();
	MainScreen.text("You don't think you're going to see these villagers again, and you don't really want to.");
	//Corruption +15 (plus extra for rape), ~ 1000 gems, fatigue set to 100
	player.stats.gems += 900 + rand(150);
	fatigue(100);
	statScreenRefresh();
	cleanupAfterCombat();
}
...
public loseToOwca():void {
	MainScreen.clearText();
	MainScreen.text("You kneel down and then fall, overwhelmed by the villagers' fury and numbers.  Without hesitation, the angry mob jumps on you; you recoil in terror, persuaded that you're going to be beaten to death.  Instead, you are carelessly lifted and completely stripped off; ");
	if(player.weaponName != "fists") MainScreen.text("your " + player.weaponName + " is taken away and ");
	MainScreen.text("you are being uncomfortably transported to a destination you can guess easily.  Too dazed to resist or even worry about it; you are promptly brought to the dreaded pit, where the villagers tie you up and rudely shackle you.  Then, before you even realize how desperate your situation is, they're all gone.  Your numerous bruises and fatigue get the better of you and you quickly fall asleep.");
...
private forgiveOwca():void {
	MainScreen.clearText();
	MainScreen.text("Swallowing your pride, you tell her that it's all right and that you hold no grudge against her; the villagers were obviously desperate, to have resorted to such extreme measures.  You assure her");
	if(player.stats.cor >= 50) MainScreen.text(", lying through your teeth,");
	MainScreen.text(" that you don't feel any resentment at all; she tried her best to help you, and you're grateful to her.");
	...
	player.stats.int += 2;
player.stats.cor += -8;
	Flags.list[FlagEnum.OWCAS_ATTITUDE] = 60;
...
private fuckThisShit():void {
	MainScreen.clearText();
	MainScreen.text("You stare at the wretched, whimpering creature before you for a moment.  There's nothing to say.  Without a word, you head back to your camp, carefully closing Rebecc's door behind you as you leave.");
	Flags.list[FlagEnum.REBECCS_LAST_PLEA] = 1;
	if (Game.inCombat)
		cleanupAfterCombat();
	else doNext(camp.returnToCampUseOneHour);
...
private rebeccsLastPlea():void {
	MainScreen.clearText();
	MainScreen.text("As you arrive on the border of the cursed village, you see someone is running in your direction.  You recognize Rebecc instantly; she seems to stumble and stagger at every step, and her face is tear-stricken.  Intrigued, you catch the crying woman by the waist as she runs past you sightlessly; she falls into your arms, nearly tripping as she abandons herself to your embrace.  Unsure of what to do, you ask her why she is panicking, though it takes a while before your words reach her.");
...
private faceDownHordes():void {
	MainScreen.clearText();
	MainScreen.text("You tell Rebecc to go hide on the plains; you're going to handle the villagers as well as the demon horde on your own.  She cries, \"<i>But " + player.short + "!  You stand no chance against this ruthless horde!  Besides, the villagers won't trust you.  They'll think you want to cheat them... they'll tie you up to prevent you from getting away!</i>\"");
...
private subdueVapula():void {
	MainScreen.clearText();
	MainScreen.text("At last, the final demon falls, ");
	if(monster.HP < 1) MainScreen.text("overwhelmed by your might");
	else MainScreen.text("rendered insensate by his own lust");
	MainScreen.text(".  As usual, you walk among the forlorn bodies of your enemies.  Vapula is groveling on the floor again, looking up to you with imploring eyes.  Even beaten and broken, the luscious succubus remains the most preeminent figure of the motley rabble.");
...
private disbandHorde():void {
	...
	//Attitude set to 100
	Flags.list[FlagEnum.OWCAS_ATTITUDE] = 100;
	Flags.list[FlagEnum.OWCA_SACRIFICE_DISABLED] = 1;
	cleanupAfterCombat();
}
...
private enslaveVapulaWithYourWang():void {
	...
	MainScreen.text("You speak in a feverish voice.  \"<i>You're such a bitch.  Damn, why did you ever think you were going to make me your slut?  How many times do you need to be shown that I'm not the bottom in our relationship?  You deserve to be turned in to Lethice so she can make you her personal fuck-toy; at least you'll remember your place.</i>\"");
	...
	if(player.stats.lust < 80) player.stats.lust = 80;
...
private enslaveVapulaAsACuntWielder():void {
	...
	Flags.list[FlagEnum.VAPULA_FOLLOWER] = .5;
