...
private function gangbangVillageFirstGoRound():void {
	...
	//As long as you don't follow her once, display this on subsequent encounters (Z)
		...
		Render.text("After wandering across the plains for a while, you spot Rebecc, the girl you met before, amongst a group of fellow farmers; she calls out to you.  \"<i>Hello again, traveler!  You seem lost... would you mind spending some time with us at our village?</i>\"");
	...
	//Second (and subsequent) encounter; only happens once PC accepted being sacrificed once (Z)
		...
		//[if corr >= 50] 
		if(player.stats.cor > 50) Render.text("  You can't help but smile internally.  Let her think that, if she wants.");
...
private function agreeToFollowRebecFirstTime():void {
	...
	if(flags[FlagEnum.TIMES_REFUSED_REBECCS_OFFER] == 0) {
		...
		if(player.gender == 0) Render.text("regretfully bare ");
		Render.text("groin as you feel the contact of her tender curves against your body.  Taking advantage of the effect you seem to have on her, you start questioning her about her village, her people and how she was affected by demons.");
		...
		//[if silly mode on]
		if(silly()) Render.text("I-I don't think there's a single grown man or woman in our village that still has their anal virginity.");
		//[if silly mode off]
		else Render.text("Th-they abused us.  They mistreated us.  They would gather in circles and... use us in every possible way.");
		Render.text("</i>\"");
		//[if Corr < 20]
		if(player.stats.cor < 20) {
			...
			//[[if silly mode on]
			if(silly()) Render.text("she'll never have to worry about her asshole again.");
			//[if silly mode off] 
			else Render.text("she will never be abused again.");
			Render.text("  If there's anything that can be done to hold off these evil creatures, you will do it.");
		}
		...
		if(player.stats.cor < 50 && player.stats.int >= 50) {
			...
			if(player.stats.lib > 50) Render.text("... and considering what Rebecc meant by 'gratitude'");
			Render.text(".  Maybe it wasn't all an act?");
		}
		//[if corr < 50 and int < 50]
	...
	//Subsequent pleas upon first refusal (Z)
		...
		Render.text("\"<i>So, have you changed your mind yet?  Will you help us?  The demons will want another sacrifice soon, and we are afraid of what might happen if we don't comply!  Please, do accept this time!</i>\"");
...
private function acceptRebeccsPlea(firstTime:boolean = false, sacrificed:boolean = false):void {
	...
	Render.text("You tell the desperate girl that you're going to do as she asks.  As soon as you finish speaking, everyone cheers and applauds you, praising you as their savior.  Rebecc grabs hold of your shoulders and gives you a soft kiss on your lips; she whispers, \"<i>Thank you, champion.  We all owe you.  I promise that you will be rewarded for the heroic deed you're about to undertake.</i>\"");
	...
	//if(player.armorName != "comfortable clothes") Render.text("  The farmers almost eagerly help you out of your " +player.armorName+ ", supplying you with a set of shabby but comfortable peasant clothes.");
	//[[if corr >70 and have gender]
	if(player.stats.cor > 70 && player.gender > 0) {
		Render.text("\n\nYou lick your lips in anticipation, your ");
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(vaginaDescript(0));
		...
		Render.text(" already slickening with sexual juices; the sight of these restraints brings all kinds of perverted thoughts to your mind: dozens of ways to be used and deliciously abused like a sex-toy by wretched and well-endowed beings.");
	}
	//[else]
...
private function intoTheDemonPit(sacrifice:boolean = true):void {
	...
	if(model.time.hours < 21) {
		...
		Render.text("<b>" + Num2Text(passed) + " " + (passed > 1 ? "hours pass" : "hour passes") + "...</b>\n");
		model.time.hours = 21;
		statScreenRefresh();
	}
	Render.text("You are awakened by a sudden cackling, and open your eyes; you are almost entirely surrounded by darkness, and the dim light provided by the menacing red moon only makes the landscape bleaker.");
	...
	if(sacrifice) Render.text(", tangled in your restraints");
	Render.text(".  She leans down and coos, ");
	//[if first time]
	if(flags[FlagEnum.TIMES_IN_DEMON_PIT] == 0) Render.text("\"<i>Awww, look at the poor little thing!  So this is what they gave us this time... Look at [him], everyone!  Do you know you'd be a cute pet?  People here call me Vapula, and I don't think you'll forget that name any time soon.</i>\"");
	//[else if Vapula Submissiveness >50]
	else if(flags[FlagEnum.VAPULA_SUBMISSIVENESS] > 50) Render.text("\"<i>You again?  By Lethice, you must really enjoy it!  You love being a bitch for Vapula, don't you?  Yes you do!</i>\"");
	//[else]
	else Render.text("\"<i>Fuck, you again?  Tell me it's so!  I've been looking forward to putting you in your place as my newest slut!</i>\"");
	//[(if sacrificed)
	if(sacrifice) {
		//[if str >= 80]
		if(player.str >= 80) {
		...
		Render.text("  You're once again feeling regret over letting the villagers hold your equipment, but there's nothing for it now.");
	}
...
public function loseOrSubmitToVapula():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text(", before finally wrapping around your " +cockDescript(0)+ " and stroking it; the friction uncontrollably arouses you, and you find yourself reaching full erectness");
	Render.text(".  As more and more hands start playing with your flesh, the succubus grabs your head and gives you a fierce kiss, literally crushing your lips under hers; her mouth tastes like wine and her tongue is driving you over the edge as it intertwines with yours.  Her strong natural scent makes you dizzy and you gradually lose control over your body.  \"<i>Don't worry, darling, it'll be all right...</i>\"");
	...
	Render.text("  The double penetration is brutal, unexpected and painful.  Your insides are protesting vigorously against this rough treatment, even though you feel a tingle of pleasure gently tickling your colon at every thrust.  ");
	//[(no Buttslut)
	if(!player.perks.has("Buttslut")) Render.text("No!  You aren't supposed to enjoy it...  ");
	Render.text("You try to cry out but as soon as your mouth opens it is filled with another dick, then a second one.  A third tries to push its way between the first two, stretching your cheeks and making you drool.  Seeing that the monstrous dong won't fit in your already double-stuffed mouth, its owner groans in frustration and proceeds to slap your cheek with it.  He is soon joined by other demons who find the idea very entertaining.");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.count() > 1) Render.text("  She is soon joined by other horny girls, too eager to wait for a piece of your crotch to play with.  They all comfortably position themselves, positioning their pussy or anus atop each of your cocks before riding you like there's no tomorrow.");
		//[if 4 to 7 cocks]
		if(player.lowerBody.cockSpot.count() >= 4) Render.text("  Nevertheless, they aren't satisfied with a single insertion and most grab hold of one of your extra cocks before forcefully stuffing it in their other hole, howling and thrashing all the more as they are penetrated a second time.");
		//[if 7 cocks or more]
		if(player.lowerBody.cockSpot.count() >= 7) {
			Render.text("  Sadly, surrounded as you are, no one will be able to find extra room to ride your remaining cocks; fortunately, you feel warm, long demonic tongues being pressed against your " +cockDescript(6));
			if(player.lowerBody.cockSpot.count() > 8) Render.text(" as well as your remaining pricks");
			Render.text(".  The tongues expertly lick your meat and wrap themselves around it, and questing lips give it multiple kisses before one pair suddenly loses patience and deepthroats you with voracity; the tightness of these lips makes them feel like a very efficient cock-ring.  However, it is hard to focus on the sweet warmth provided by these avid mouths when all your other cocks are being clamped by powerful, inhuman vaginal muscles.");
		}
		Render.text("\n\nBetween every thrust, you feel hands working and polishing your shaft");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(", as if every inch had to be taken care of at every instant.  As you slide back into the pussy, the hands return to crawling over your body, caressing your groin and staining your belly with pre-cum - yours and others.");
	...
	//[if cocks]
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("  " + SMultiCockDesc() + " pulsates and throbs");
		//[[if balls]
		if(player.lowerBody.balls > 0) Render.text(", and your balls swell and boil");
		Render.text("; a gigantic spooge-flow pushes its way up your urethra and spurts outside in the waiting love-tunnels");
		if(player.lowerBody.cockSpot.count() > 7) Render.text(" and mouths");
		Render.text(".  The violent cumshot almost instantly produces a shiver of orgasm once again and vaginal walls contract furiously, determined to absorb as much of your baby-batter as possible.");
		//[if cum production is massive]
		if(player.cumQ() > 1500) Render.text("  The effort appears to be vain, as the absurdly high volume you've ejected is enough to completely pack every hole; the remaining jism spills on the tainted ground, soaking it further.");
	}
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and filling your womb to its maximum capacity");
	Render.text(".  The remaining cum spurts outside and dribbles onto your [legs].  Your asshole clenches, squeezing the dongs inside you and milking their very last drops of spunk.");
	//[if vagina]
...
private function wakeUpAfterDemonGangBangs():void {
	...
	model.time.hours = 7;
	model.time.days++;
	Render.text("When you wake up, you are alone, and your restraints are broken.  You are sloshing in a pool of stinky juices; your mouth and ears are still full of it.  Your whole body is covered with a thin white layer that must certainly be dried spooge.  Underneath, you're nothing but bruises and every movement seems to hurt.  A few meters away, outside the pit, you notice your items and your gear.  The village itself appears to be empty... your best assumption is that the residents are hiding, either from shame at having sacrificed you or from awkwardness at the prospect of talking to a sloshing, crusty cumdumpster.  Wearily, you head back to your camp.");
...
public function defeetVapulasHorde():void {
	...
	if(monster.statusAffects.has("AttackDisabled")) {
	   if(monster.HP < 1) Render.text("You grin wickedly as the last demons fall, defeated.  Some of the errant blows have broken and mangled the links of your chains, and you find you can get free with a bit of additional effort.");
		//[if won by Lust]
		else Render.text("You grin wickedly as the demons give up the fight, too turned on to care about you.  One even begins unfastening your bindings, hopeful desperation glinting in her eyes as she attempts to entice you with her long, thick nipples and enormous, dripping gash.");
	}
	//Not
	else {
		if(monster.HP < 1) Render.text("You grin wickedly as the last demons fall, defeated.");
		//[if won by Lust]
		else Render.text("You grin wickedly as the demons give up the fight, too turned on to care about you.  One even has hopeful desperation glinting in her eyes as she attempts to entice you with her long, thick nipples and enormous, dripping gash.");
	}
...
private function rapeZeVapula():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		clearOutput();
		Render.text("The demon horde struggles before you.  With a disdainful smile, you fully expose your genitals for everyone to see.  ");
		//[if cocks]
		if(player.lowerBody.cockSpot.hasCock()) Render.text("\n\nYou gently stroke your " +cockDescript(0)+ ", bringing it to full erectness.  ");
		//[if vagina]
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("You stick a finger in your own " +vaginaDescript()+ " in order to lubricate it.  ");
		Render.text("The defeated horde watches you touch yourself with avid, almost desperate eyes.  You keep teasing yourself, saving your lust for the torrid storm of rape you're about to unleash on the poor creatures who dared attack you.  You walk among the battered bodies, looking for the prey that will best suit you; then you find her.  Vapula is lying here, her purple skin masking ");
		if(monster.HP < 1) Render.text("the many bruises she's received");
		else Render.text("the constant flushing due to intense arousal");
		Render.text(".  You harshly grab her hand and grunt, \"<i>On your feet, cunt.</i>\"  She is too stunned to protest and weakly stands up, her leather straps torn to completely reveal her voluptuous body.  You feel the sudden urge to take her right now and fuck her like a ");
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("bitch in heat");
		else Render.text("centaur in rut, abusing and filling her teasing cunt and ass with every protuberance you have");
		Render.text(".  However, you have better plans for her.");
		...
		if(player.lowerBody.cockSpot.count() > 1) Render.text("  Seeing that she has extra room left, you grab another of your dongs and put it at the entrance of her dripping cunt before forcing it in.");
		...
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" in front of the once-powerful dominatrix, grunting and panting as you accelerate the pace, until you finally reach your climax and release rope after rope of jism in your fuck-toy, completely packing her backdoor");
		if(player.lowerBody.cockSpot.count() > 1) Render.text(" and her love-tunnel");
		Render.text(".");
		//[if massive cum production]
		if(player.cumQ() > 1000) Render.text("  Some of it spurts by little jets outside of her, even as she instinctively contracts her muscles to keep as much spooge as possible and her belly accomodates by bloating out absurdly.");
		Render.text("  Needless to say, watching this rough ");
		if(player.lowerBody.cockSpot.count() == 1) Render.text("anal");
		else Render.text("double-penetration");
		Render.text(" session has brought Vapula to a new level of arousal; she is struggling to free her arms and finger herself, but her tight restraints only allow her to wriggle uncomfortably.  Her pussy is gushing of its own and she whimpers from time to time, unable to control her lust.");
		Render.text("\n\nYou throw away your expendable");
		if(player.cumQ() > 1000) Render.text(", expandable");
		Render.text(" cumdumpster and grab another slut.  This time you feel like you could use a blowjob to clean off the demonic sweat and slime from your cock; you forcefully open her jaw and shove your " +cockDescript(0)+ " down her throat without any care.  The sudden influx of dickflesh blocking her air-flow makes her choke and cough, but a withdrawal and a good cock-slap reminds her of her place.  \"<i>You fucking bitch!  You goddamn worthless slut!  What good are you if you can't even give proper head?  Lick, you whore!  I want your tongue all over my crotch!  You can breathe when you've properly moistened my ");
		if(player.lowerBody.balls > 0) Render.text("balls");
		else Render.text(player.skin());
		Render.text("!</i>\"  The poor demoness, cheek stained with semen, mutters an apology and proceeds to lick your " +cockDescript(0)+ ", suckling the tip, ");
		if(player.lowerBody.balls > 0) Render.text("fondling your " +ballsDescriptLight()+ ", ");
		//[if multicocks] 
		if(player.lowerBody.cockSpot.count() > 1) Render.text("alternatively deepthroating or jerking every cock of yours, ");
		Render.text("warming up your dickflesh with her demonic tongue.  You enjoy her ministrations while staring Vapula in the eyes; she has trouble looking back at you as her body trembles and her eyes widen at your sheer display of depravity.  Your semen-dribbling shaft");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s harden and thicken ");
		else Render.text(" hardens and thickens ");
		Render.text("again as the demoness sucks you off; Vapula can't help but take a quick glance at it, but you pretend not to notice.  After all, she's only teasing herself for you.");
		...
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" and aim at Vapula, splattering her body with your spunk.  Words cannot describe the sheer triumph you feel.  You burst into laughter as you drench that unworthy bitch with your seed.");
		...
		if(player.lowerBody.cockSpot.count() > 1) Render.text("In an unholy sex-frenzy, you want more; you start grabbing creatures by groups and alternatively pump in and out of their asses with your " +cockDescript(0)+ ", always hungry for more anal.");
		...
		if(silly()) Render.text("  As you look back, you snicker, \"<i>Ass for ass, assholes.</i>\"");
	...
	//Victory rape using vagina/anus [Take Dicks in All Holes](Z)
		...
		if(player.vaginalCapacity() > 80) Render.text("His cock is lost in your vast cunt and you work him as hard as you can to feel him against your walls, his body convulsing and dazed eyes rolling against the wet, savage rape.  ");
		Render.text("You keep thrusting back and forth, treating the worthless horned boy like a disposable dildo.  As you pump him, you look at Vapula, warning her, \"<i>Don't turn your head.  If you ever look away or close your eyes you will regret it.</i>\" Overwhelmed by your dominance and subdued by the defeat, Vapula can only nod.");
		...
		//[if squirter] 
		if(player.wetness() >= 5) Render.text("The sound of spattering fluid fills the air as your cunt spurts its juices around the incubus's cock, coating him in your fragrant slime.  ");
		Render.text("Against your relentless milking pressure he stands no chance and he moans raggedly as he cums in tandem, still incapable of moving his body as your mixed juices dribble down him.");
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) Render.text(" and your " +breastDescript(0)+ " bounce gently");
		Render.text(" as you grind your snatch into the succubus's face; Vapula can't help but take a quick glance at your oozing, slippery cunt, but you pretend not to notice.  After all, she's only teasing herself for you.");
...
private function owcaMainScreenOn():void {
	...
	//[if attitude > 80]
	if(flags[FlagEnum.OWCAS_ATTITUDE] > 80) Render.text("Villagers greet you as you arrive, praising your courage and your heroism.  People shake your hands or ask you to kiss them, as if they hoped your nobility were somehow contagious.  Once they're done complimenting and flattering you in every way possible, they leave you to your business.");
	//[if 60<=attitude<80]
	else if(flags[FlagEnum.OWCAS_ATTITUDE] > 60) Render.text("Villagers greet you with a friendly smile as you arrive.  Some approach you and thank you for what you've done, before leaving awkwardly.  They're obviously not used to strangers.");
	//[if 40<=attitude<60]
	else if(flags[FlagEnum.OWCAS_ATTITUDE] > 40) Render.text("Villagers seem to accept your presence without comment now; though none go out of their way to greet you, neither do they avoid doing so when you pass by.");
	//[if 20<attitude <= 40]
	else if(flags[FlagEnum.OWCAS_ATTITUDE] > 20) Render.text("Villagers keep looking at you uncomfortably; no one seems to be willing to talk to you apart from Rebecc, your friend.");
	//[if attitude <= 20]
	else Render.text("Villagers here appear to be openly hostile.  You keep hearing muffled whispers as you pass by, and people look at you rancorously.  They don't bother you, though.");
	//end variable display
	Render.text("  The village is rather small.  A dozen humble thatched cottages are standing here and there, linked by dirt tracks and enclosed with small palisades.  There seems to be very little activity.");
	...
	if(flags[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS] == 7 && flags[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) Render.text("  A week");
	else if(flags[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS] > 7 && flags[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) Render.text("  More than a week");
	if(flags[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS] >= 7 && flags[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) Render.text(" has passed since the last offering to the demons; guarding the pit would certainly help improve your relations with the little town's denizens.");
	//Option: 
...
private function buyOwcaShit(bleh:ItemType,price:number = 0):void {
	...
	statScreenRefresh();
	Render.text("The bartender hands you a bottle and grabs your gems before attending other clients, leaving you to your own business.\n\n");
...
private function rebeccBathScene():void {
	...
	if(player.gender == 0) Render.text("."); 
		...
		//[if cock]
		if(player.lowerBody.cockSpot.hasCock()) Render.text("; your " + cockDescript(0) + " grows to full erect size");
		if(player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.cockSpot.hasCock()) Render.text(" and ");
		else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("; ");
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("your " + vaginaDescript() + " leaks a little trickle of girl-juice");
		Render.text(".  She doesn't seem to notice the effect she has on you - or so you think.  Suddenly, you feel a hand darting between your legs.");
	}
...
private function rapeRebecc(outside:boolean = false):void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.list[0].cockLength > 12) Render.text("  You keep burying more of your " + cockDescript(0) + " deep inside her rectum until you reach her limit and can't fit more cock in.");
		else Render.text("  Eventually the entirety of your prick is lodged inside her lush asshole.");
		Render.text("  She trills and bleats in pain and flounders about in your arms, futilely trying to avoid the incoming butt-rape.  You lock her arms with yours, implacably kneading her tits, and her shivering flesh brings the most exquisite contact to your fingers; you toy with her nipples as well, making her scream savagely in forced pleasure.");
		...
		//[if balls]
		if(player.lowerBody.balls > 0) Render.text(", and your " + ballsDescriptLight() + " almost feel sore from colliding with her dribbling cunt");
		Render.text(".  Finally, when you think you're about to pass out from exhaustion, you cum in a powerful jet, literally blasting Rebecc's ass with your spooge, globs of  goo being propelled like projectiles into her colon and splattering her backside with huge white smears.");
		if(player.cumQ() > 1500) Render.text("  After completely filling up her rectum, your baby-batter erupts in thick spurts from her anus despite her instinctive ass-clenching efforts to hold as much semen as possible inside.  The sheer might of your cumshot is enough to send Rebecc over the edge, and she releases an absolutely torrential stream of girl-cum, squirting against her will all over your groins and mixing her own fluids with yours in a pathetic show of sluttiness.");
		Render.text("  She unconsciously milks you of all your spunk, her anal muscles working on their own to massage your meat and extract every drop from your urethra.");
		...
		if(outside) Render.text("Some of the villagers are apathetically watching you, too stunned and bruised to intervene.  ");
		Render.text("You silently remove your prick from your girl's ass, watch her contentedly as her exposed rectum is literally oozing with spooge and walk back to your camp.");
		Render.text("\n\nThis is a more-than-appropriate goodbye gift");
		if(outside) Render.text(" to these miserable people, and even moreso");
		Render.text(" to the manipulative slut who kept teasing you.  Serves her right, to use her body in the most provocative ways and never let you actually have your way with her.  You're unlikely to ever see her again.");
	...
	//Rebecc Female Rape Scene (Z)
		...
		if(player.tone > 75) Render.text("  The sheep girl's plush softness against your own toughened, toned physique manages to both arouse and anger you.  \"<i>This is what you were bred to be, isn't it bitch?</i>\" you hiss, your hands roaming over her inviting curves.  \"<i>Your fat ass hasn't had to work a day in its life.  The town whore, the cock-tease they wheel out when your people need a dupe, that's what you are.  How does it feel to be in the hands of a real woman?</i>\"");
		else Render.text("  Gods, this girl has a fine ass; you give it a few slaps here and there as it jiggles in the most enticing way.");
		Render.text("  Rebecc doesn't say anything as you continue to grope her; she simply quivers with her eyes tightly shut, apparently unwilling to believe this is even happening.");
		...
		if(outside) Render.text("Some of the villagers are apathetically watching you, too stunned and bruised to intervene.  ");
		Render.text("You silently pick yourself up and dress, admiring the quivering, crying handiwork at your feet, before walking back to your camp.  This is a more-than-appropriate goodbye gift");
		if(outside) Render.text(" to these miserable people, and even moreso");
		Render.text(" to the manipulative slut who kept teasing you.  Serves her right, to use her body in the most provocative ways and never let you actually have your way with her.  You're unlikely to ever see her again.");
		Render.text("\n\n(<b>Owca removed from the places menu.</b>)");
	}
...
private function torchOwcaMotherFuckers():void {
	...
	if(player.stats.cor >= 60 && player.gender > 0 && player.lust >= 33) {
		Render.text("You notice Rebecc is still weeping among the scattered bodies of the beaten villagers.  Do you abuse her?");
...
private function forgiveOwca():void {
	...
	Render.text(" that you don't feel any resentment at all; she tried her best to help you, and you're grateful to her.");
...
private function subdueVapula():void {
	...
	else Render.text("rendered insensate by his own lust");
	Render.text(".  As usual, you walk among the forlorn bodies of your enemies.  Vapula is groveling on the floor again, looking up to you with imploring eyes.  Even beaten and broken, the luscious succubus remains the most preeminent figure of the motley rabble.");
