...
private function gangbangVillageFirstGoRound():void {
	...
	//Second (and subsequent) encounter; only happens once PC accepted being sacrificed once (Z)
		...
		//[if corr >= 50] 
		if(player.stats.cor > 50) outputText("  You can't help but smile internally.  Let her think that, if she wants.");
...
private function agreeToFollowRebecFirstTime():void {
	...
	if(flags[FlagEnum.TIMES_REFUSED_REBECCS_OFFER] == 0) {
		...
		if(player.stats.cor < 50 && player.stats.int >= 50) {
			...
			if(player.stats.lib > 50) outputText("... and considering what Rebecc meant by 'gratitude'");
...
private function acceptRebeccsPlea(firstTime:boolean = false, sacrificed:boolean = false):void {
	...
	if(player.stats.cor > 70 && player.gender > 0) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript(0));
...
private function intoTheDemonPit(sacrifice:boolean = true):void {
	...
	//[if first time]
	if(flags[FlagEnum.TIMES_IN_DEMON_PIT] == 0) outputText("\"<i>Awww, look at the poor little thing!  So this is what they gave us this time... Look at [him], everyone!  Do you know you'd be a cute pet?  People here call me Vapula, and I don't think you'll forget that name any time soon.</i>\"");
	//[else if Vapula Submissiveness >50]
	else if(flags[FlagEnum.VAPULA_SUBMISSIVENESS] > 50) outputText("\"<i>You again?  By Lethice, you must really enjoy it!  You love being a bitch for Vapula, don't you?  Yes you do!</i>\"");
	//[else]
...
public function loseOrSubmitToVapula():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(", followed by your "+vaginaDescript(0));
	if(player.lowerBody.cockSpot.hasCock()) outputText(", before finally wrapping around your " +cockDescript(0)+ " and stroking it; the friction uncontrollably arouses you, and you find yourself reaching full erectness");
	...
	//[(no Buttslut)
	if(!player.perks.has("Buttslut")) outputText("No!  You aren't supposed to enjoy it...  ");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("  She is soon joined by other horny girls, too eager to wait for a piece of your crotch to play with.  They all comfortably position themselves, positioning their pussy or anus atop each of your cocks before riding you like there's no tomorrow.");
		//[if 4 to 7 cocks]
		if(player.lowerBody.cockSpot.count() >= 4) outputText("  Nevertheless, they aren't satisfied with a single insertion and most grab hold of one of your extra cocks before forcefully stuffing it in their other hole, howling and thrashing all the more as they are penetrated a second time.");
		//[if 7 cocks or more]
		if(player.lowerBody.cockSpot.count() >= 7) {
			...
			if(player.lowerBody.cockSpot.count() > 8) outputText(" as well as your remaining pricks");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		//[[if balls]
		if(player.lowerBody.balls > 0) outputText(", and your balls swell and boil");
		outputText("; a gigantic spooge-flow pushes its way up your urethra and spurts outside in the waiting love-tunnels");
		if(player.lowerBody.cockSpot.count() > 7) outputText(" and mouths");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" and filling your womb to its maximum capacity");
...
private function rapeZeVapula():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		//[if vagina]
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("You stick a finger in your own " +vaginaDescript()+ " in order to lubricate it.  ");
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("bitch in heat");
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("  Seeing that she has extra room left, you grab another of your dongs and put it at the entrance of her dripping cunt before forcing it in.");
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText(" and her love-tunnel");
		outputText(".");
		...
		if(player.lowerBody.cockSpot.count() == 1) outputText("anal");
		...
		if(player.lowerBody.balls > 0) outputText("fondling your " +ballsDescriptLight()+ ", ");
		//[if multicocks] 
		if(player.lowerBody.cockSpot.count() > 1) outputText("alternatively deepthroating or jerking every cock of yours, ");
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("s harden and thicken ");
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("In an unholy sex-frenzy, you want more; you start grabbing creatures by groups and alternatively pump in and out of their asses with your " +cockDescript(0)+ ", always hungry for more anal.");
	...
	//Victory rape using vagina/anus [Take Dicks in All Holes](Z)
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText(" and your " +breastDescript(0)+ " bounce gently");
		...
		//[cunt nipples] 
		if(player.upperBody.chest.hasFuckableNipples()) outputText("  Your freakish nipples moisten with excitement to the smell of man juice, and you feel your libido ratcheting up even higher.  Bending forward, you grab two other imps with smaller dicks, quickly guiding their lengths into your nipplecunts; they are small enough that you can mash their bodies against your soft flesh, doing all the work for them as your sensitive nubs are penetrated.");
		...
		if(player.upperBody.chest.hasFuckableNipples()) outputText("s");
		...
		if(player.upperBody.chest.hasFuckableNipples()) outputText(", teats, ");
		else outputText(" ");
...
private function owcaMainScreenOn():void {
	...
	//[if attitude > 80]
	if(flags[FlagEnum.OWCAS_ATTITUDE] > 80) outputText("Villagers greet you as you arrive, praising your courage and your heroism.  People shake your hands or ask you to kiss them, as if they hoped your nobility were somehow contagious.  Once they're done complimenting and flattering you in every way possible, they leave you to your business.");
	//[if 60<=attitude<80]
	else if(flags[FlagEnum.OWCAS_ATTITUDE] > 60) outputText("Villagers greet you with a friendly smile as you arrive.  Some approach you and thank you for what you've done, before leaving awkwardly.  They're obviously not used to strangers.");
	//[if 40<=attitude<60]
	else if(flags[FlagEnum.OWCAS_ATTITUDE] > 40) outputText("Villagers seem to accept your presence without comment now; though none go out of their way to greet you, neither do they avoid doing so when you pass by.");
	//[if 20<attitude <= 40]
	else if(flags[FlagEnum.OWCAS_ATTITUDE] > 20) outputText("Villagers keep looking at you uncomfortably; no one seems to be willing to talk to you apart from Rebecc, your friend.");
	//[if attitude <= 20]
	...
	if(flags[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS] == 7 && flags[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) outputText("  A week");
	else if(flags[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS] > 7 && flags[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) outputText("  More than a week");
	if(flags[FlagEnum.DAYS_SINCE_LAST_DEMON_DEALINGS] >= 7 && flags[FlagEnum.OWCA_SACRIFICE_DISABLED] == 0) outputText(" has passed since the last offering to the demons; guarding the pit would certainly help improve your relations with the little town's denizens.");
	//Option: 
...
private function rebeccBathScene():void {
	...
	if(player.gender == 0) outputText("."); 
		...
		//[if cock]
		if(player.lowerBody.cockSpot.hasCock()) outputText("; your " + cockDescript(0) + " grows to full erect size");
		if(player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.cockSpot.hasCock()) outputText(" and ");
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("; ");
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your " + vaginaDescript() + " leaks a little trickle of girl-juice");
...
private function rapeRebecc(outside:boolean = false):void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.list[0].cockLength > 12) outputText("  You keep burying more of your " + cockDescript(0) + " deep inside her rectum until you reach her limit and can't fit more cock in.");
		...
		//[if balls]
		if(player.lowerBody.balls > 0) outputText(", and your " + ballsDescriptLight() + " almost feel sore from colliding with her dribbling cunt");
...
private function forgiveOwca():void {
	...
	if(player.stats.cor >= 50) outputText(", lying through your teeth,");
