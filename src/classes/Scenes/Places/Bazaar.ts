...
private function approachBazaarGuard():void {
	Render.text("", true);
	Render.text("You step from concealment and walk up to the strange man, calling out in greeting.  He folds his arms across his chest and looks you up and down, peering at you with intense, black eyes.  They aren't solid onyx, but his irises are just as dark as the seemingly bottomless depths of his pupils.  His appraising gaze watches you, unblinking as second after second ticks by.  Just when you start to wonder if he speaks your language, he interrupts you by saying, \"<i>", false);
	if(player.stats.cor < 33) Render.text("Leave at once.  You are not yet ready for the wonders of the Bazaar.", false);
	else Render.text("Welcome to the Bizarre Bazaar.  Enter, but be mindful of your actions within.", false);
	Render.text("</i>\"", false);
	if (player.stats.cor < 33) simpleChoices("", null, "", null, "", null, "", null, "Leave", camp.returnToCampUseOneHour);
...
private function theSlipperySqueeze():void {
	...
	//Cock Milker
		...
		Render.text("You notice Joey leaning on the counter, lost in thought.  The bunny boy frowns, troubled by something.  He jolts upright when he notices you approaching, his expression lifting into a polite grin.  \"<i>Hey, welcome!  Always nice to see a familiar face.  So, do you have some 'tension' that needs relieving?</i>\"  His eyes travel down to your crotch.  Shameless as ever but with a hesitant offer buried in his eyes...\n\n");
		flags[FlagEnum.JOEY_OFFERED_MILKER] = 1;
	}
	//(Repeat visit: 
	else {
		Render.text("Joey purses his glossed lips when you enter and coyly cocks one of his ears as he says, \"<i>Welcome back to 'The Slippery Squeeze', " + player.short + ".  Would you like me to give you a nice, salty rub-down?  Or would you prefer Sara do it?  Just remember, she can't make her own 'oil' like I can.</i>\"\n\n", false);
		//No scenes for Sara yet!
		Render.text("\"<i>I told you I'm not working until you give me a raise!  Do the damned massages yourself!</i>\"\n\nJoey blushes and apologies.  \"<i>I guess I'm the only one available for now.</i>\"\n\n", false);
		Render.text("The price list indicate it's 10 gems for a massage, though the gleefully illustrated 'oil' reminds you just what they plan to use on you.\n\n", false);
		if(!player.perks.has("Androgyny")) {
			Render.text("There also appears to be an option for a special 'Androgyny Treatment' that costs 500 gems.  Joey catches you looking at it and mutters, \"<i>That treatment isn't fun like our massages.  It would unlock the full potential of your visage, allowing it to be as masculine or feminine as possible.</i>\"\n\n", false);
...
private function noMilkerPlzJoey():void {
	clearOutput();
	Render.text("You decline; it's not really the sort of thing you need in your camp.  \"<i>Ah well,</i>\" Joey shrugs, \"<i>I'll just have to hang onto it for now I guess.  Shame.  Anyway,</i>\" he resumes his usual grin, \"<i>is there something else you need?  A massage, perhaps?</i>\"");
...
private function joeyMassage():void {
	...
	if(player.stats.gems < 10) {
		Render.text("Joey frowns when you realize you don't have the 10 gems.  He apologizes, \"<i>I'm sorry, " + player.short + " but I can't give freebies - our special potions cost us plenty.", false);
		doNext(enterTheBazaar);
		return;
	}
	player.slimeFeed();
...
private function joeysMassageWithEXTRASpooge():void {
	Render.text("", true);
	Render.text("The rabbit-eared fem-boy pulls the cork on another bottle and helps you to roll to your side to drink it.  He holds the lip of the bottle to your lips and raises the bottom slowly, giving you just enough time to guzzle it without drowning.  It's sweet and syrupy, though there's an undertone of spicy strangeness that you can't quite place.  Whatever the secret ingredients are, you'll never figure them out from taste alone.  You feel warmth once you've finished, and a tightness settles ", false);
	if(player.lowerBody.balls == 0) Render.text("inside you", false);
	else Render.text("in your " + ballsDescriptLight(), false);
	Render.text(" that reminds you of the sensation just before orgasm.", false);
	if(player.lowerBody.balls > 0) Render.text("  They even feel a little bigger.", false);
	Render.text("\n\n", false);
	
	Render.text("Joey's fingertips brush along your shaft, squeezing it with tender touches that make it stiffen and thicken.  He starts slowly jacking you off while his other hand traces one of your nipples.  \"<i>We've got to get you nice and hard now so that you can let out all that nice, creamy lotion,</i>\" explains the fem-boy.  You nod in understanding, blushing hard while he fondles " + sMultiCockDesc() + " with soft caresses.  He plays your manhood", false);
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s like fiddles", false);
	else Render.text(" like a fiddle", false);
	Render.text(", expertly running his fingertip around the sensitive " + player.cockHead() + " before tracing down along your rapidly filling urethra.  It feels good – better than it should, and the warmth inside you begins to leak into the bunny-boy's waiting hand in moments.\n\n", false);
	
	Render.text("You're rolled back on to your front, crushing " + sMultiCockDesc() + " between you and the sheets.  Joey leaps back atop you, straddling your back and facing your " + assDescript() + ", his hands locking onto the steam-moistened cheeks.   He slides forward slightly, placing his hardness between them, and it's then that you notice his discarded thong on the floor.  A moment later the first drops of Joey's own hot seed are dripping over your exposed derriere.  He slides himself through your buns, hotdogging the rapidly-slickening surface of your ass while his hands massage the tense flesh a little more enthusiastically than they ought to.\n\n", false);
	
	Render.text("Of course you don't complain, not with the growing puddle forming between you and the cum-soaked sheets.  Every time your masseuse shifts, the movement makes you slide in your sticky mess, the cum-lubed friction of the sheets helping the constant jizz-flow to thicken into a steady river of seed.  ", false);
	if(player.lowerBody.balls > 0) Render.text("Your balls pull and relax and pull tight, bouncing below you over and over, all while working hard to produce more juice for you to gush.  ", false);
	Render.text("Even though you're draining spunk at an alarming rate, it doesn't feel like a normal orgasm.  There's no wave of pressure and subsequent release, just a constant, pulsing contraction that makes you melt with ever-increasing feelings of satisfaction.\n\n", false);
	
	Render.text("Joey giggles, turning about to put his lotion-dispenser on the small of your back, and you feel the hot bunny-spoo pour onto you in a wave.   It's gathered up and pushed up your back.  Globs of it roll down your side, sliding over the semi-waterproofed sheets to mix with your growing spunk-lake.  With the slippery-seed aiding Joey's massage, he somehow manages to work out even more of your tension.  By this point your muscles feel like jello melting in the afternoon sun.  You feel like you could melt into your cum, and you sigh in bliss", false);
	if(player.cumQ() >= 1500) Render.text(".  The sound of your rivers of spooge splattering against the floor barely registers, but a part of you is proud to be so incredibly fertile", false);
	Render.text(".\n\n", false);
	
	Render.text("You're prodded insistently, the masseuse saying, \"<i>Come on, we've got so much more of your body to work on before you're completely emptied of all that nasty stress.  Now roll over, don't be shy about the mess.  I'll be making it messier.</i>\"  True to his word, Joey is still drooling his 'special lotion' over your " + player.legs() + ".  You oblige and gingerly roll over, the cummy sheets squishing lewdly under you as your slime-slicked belly and cum-drooling crotch are revealed.", false);
	if(player.cumQ() >= 1500) Render.text("  Joey's jaw drops from the unexpectedly large volume you're producing, and he watches in awe as each huge globule of cum rolls off you towards the floor.", false);
	Render.text("\n\n", false);
	
	Render.text("The bunny-boy springs back atop you, landing hard just below your hips.  His dripping seed washes over " + sMultiCockDesc() + ", and the sudden onslaught of fresh, liquid warmth on your groin sets off a small explosion of jism that splatters into your chin, leaving a long trail of slime behind like a snail.  The long-eared girly-boy smiles and shifts to rub his small cock against your " + cockDescript(0) + ", frotting you aggressively while you both spray cum like faucets with the knobs torn off.  You don't mind that he seems to have forgotten the massage, and you run your hands up and down your " + chestDesc() + " to smear the heavy loads ", false);
	if(player.skinType != SKIN.FUR) Render.text("over your " + player.skinDesc, false);
	else Render.text("through your fur", false);
	Render.text(".\n\n", false);
	
	Render.text("The special potion makes it so your orgasm is long and languid, oozing out for minutes instead of seconds, and the small chamber is filled with breathy, exultant cries of passion.  Joey leans forward and grabs hold of your " + nippleDescript(0) + "s, ", false);
	if(!player.upperBody.chest.hasFuckableNipples()) Render.text("squeezing and tugging on them", false);
	else Render.text("sliding cum-coated fingers into their slippery depths", false);
	Render.text(" while he grinds and cums on your " + cockDescript(0) + ".  \"<i>Ungg... yes... gotta... squeeze... allofitooooouuuuut!</i>\" he pants.  He lets go of one tender nipple to reach underneath you, and before you can react, his cum-soaked finger is inside your " + assholeDescript() + ", pressing on your prostate.\n\n", false);
	Render.text("Your eyes cross and you black out, twitching weakly as a regular orgasm bursts on top of your already-protracted potion-gasm.  A pleasant, heavily sexualized dream is interrupted by a finger poking your lotion-lubricated cheek and the best you can manage is an utterly contented \"<i>mmmm.</i>\"  A warm, moist towel is rubbed over your " + player.face() + ", wiping away the spooge you blasted over it, and now that you can see him, Joey says, \"<i>You had me worried for a minute there!  Keep the towel, it's on the house, and if you need to clean up, there's a shower in the back!", false);
	if(player.cumQ() >= 1500) Render.text("  You really flooded the room though, so you'd best walk on your tip-toes until we can get a succubi or a goblin in here to clean up.", false);
	Render.text("  Come back ANY time, " + player.short + ".  I'd love to help you release again.</i>\"\n\n", false);
	
	Render.text("Joey leaves, his poofy tail bobbing back and forth.  You can see his thong is distended, virtually packed with his own still-pumping spooge, and you marvel at his perverse ingenuity when you realize his thong is waterproofed and shaped to guide all the jizz between his thighs and into his back-door.  Kinky.", false);
...
private function overHearDemonsAboutSyrena():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00292] == 0) {
		Render.text("A whisper of conversation catches your ear while you're wandering the bazaar, and overcome by curiosity, you veer towards it.\n\n", false);
		Render.text("As you're closing in on the voices, the dialogue grows clear enough to understand.\n\n", false);
		Render.text("\"<i>-old him if he doesn't finish this week's experiments, she's going to drop him into a submission tank instead of the champion!</i>\" exclaims the first voice, sounding quite feminine.\n\n", false);
		Render.text("A baritone response rings out, \"<i>You're shitting me!  Syrena's off her fucking rocker if she thinks Lethice will let her turn another of our researchers into a gibbering slut.  We've got enough of those.</i>\"\n\n", false);
		...
		Render.text("The male laughs and says, \"<i>You belong underneath a fat dick, bitch.  Look at you, you've got strings of lube trailing on the ground behind you.  You're too busy slutting it up to accomplish anything.  I'm thinking I had better head back to the lab, though.  I don't want to wind up looking like one of the escaped factory rejects - gotta keep this perfect bod, ya know?</i>\"\n\n", false);
		Render.text("You get a good look at the two when the crowd parts, and wow, that incubus is right.  He's a curly-haired, chiseled Adonis of a man, with a bulge as over-the-top of the rest of him straining at his fashionable pants.  The demon fiddles with his collar, popping it up, and he turns to leave.  The other commiserating devil is a busty succubus in a lab coat... nothing else.  She's even wetter than her companion's words would indicate, painting a thick trail of slippery girl-slime in her wake as she moves.  The succubus' nipples are like two hard bullets, soaking two glorious wet patches into her coat as she argues back.\n\n", false);
		Render.text("\"<i>S-shut up.  That cunt tested some of her prototype compounds on me, remember?  That stuff turned my pussy into an insatiable furnace.  If I can get some free time... I'll... I'll... oh screw it, let's go rent a room and fuck a few times before we go back.  I'll want to be thinking clearly when I face her.  Syrena won't get to gloat about me jumping on her fat... juicy... mmm... cock... this time!</i>\"  The succubus takes the incubus' hand and drags him away in a hurry.\n\n", false);
		Render.text("Well, that explains a lot.  The demons seem to have an active research department, though the one called Syrena does not seem to please her underlings very much.  Then again, you doubt any demonic servants are pleased with their bosses.  You can't wait to put a stop to their labors, but for now, there's nothing to do but use the bazaar or go home.\n\n", false);
	}
	//[Listen in repeat]
	else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00292] == 1) {
		Render.text("Just as before, you spot the collar-popping incubus and his lab coat-clad lover complaining about their boss.\n\n", false);
		Render.text("\"<i>-still sore!  I can't believe she did that to me!</i>\" groans the male.\n\n", false);
		Render.text("Smiling cruelly, the slick-pussied succubus says, \"<i>You deserved it.  Honestly, you turned in cum-stained reports to your boss, and you're surprised that she took your ass for a ride as punishment?  If you ask me, you planned all this.  Who do you think had to clean up the huge mess you left on the floor?</i>\"\n\n", false);
		Render.text("For once, the incubus seems speechless.  He mutters, \"<i>Well, I didn't plan it.  I-I was just having fun with one of the new slaves - for inspiration - while I finished up the paperwork.</i>\"  He looks over and punches the leering female's arm.  \"<i>Besides, I thought you liked cleaning up my cum?  I've never seen you complain before.</i>\"\n\n", false);
		Render.text("The succubus laughs and slaps his ass as she answers, \"<i>Don't worry, you taste great... just not as good as the boss.  I dunno what concoction she made that did it, but her spunk is heavenly.  I could just... mmm... swallow that delicious cream all day long.  Do ya think she's part minotaur?</i>\"\n\n", false);
		Render.text("\"<i>Babe, you're sliming everywhere again.  Why don't we go blow off some steam?</i>\"  The two horny demons run off and disappear.\n\n", false);
	}
	//[Listen in Repeat 2]  
	else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00292] >= 2) {
		Render.text("This time, the two chatty demons are seated near the fire, and the reason for their altered location seems clear.  The succubus' belly is gravid to an unusual degree, utterly packed with some kind of corrupted offspring.  She's rubbing both her hands over the stretched skin-dome and moaning in discomfort, the packed womb squirming beneath her touches.\n\n", false);
		Render.text("Meanwhile, the male incubus is knocking back a beer, grumbling, \"<i>Could you take it down a notch?  It isn't like this is the first time you've had to lug around a load of imps - don't be so melodramatic.</i>\"\n\n", false);
		Render.text("His pregnant companion growls and tugs at her undersized labcoat, failing to conceal the blueberry-colored bulge of her belly from him as she retorts, \"<i>You didn't have to get all these fucked into you, now did you?  Hell, she even shot me up with fertility-plus first!  It feels like there's two dozen of the little bastards packed in there!  I figure in another day or two I won't even be able to walk.</i>\"\n\n", false);
		Render.text("\"<i>So does that mean you won't mind if I stop by whilst you're laid up?  I mean you can't get any more pregnant with imps, and it might pass the time,</i>\" suggests the well-muscled male.\n\n", false);
		Render.text("Smirking, the demoness retorts, \"<i>Sure, but next time you fuck up, I'm loaning Syrena some of my girly stuff to make you suffer just like this.</i>\"  Her hands both point at the imp-engorged skin of her midsection for emphasis.\n\n", false);
		Render.text("The incubus rises and taunts, \"<i>If I fucked up as often as you did, I'd be feeding the furnace in a factory somewhere or fed to the hellhounds.  Come on, let's get you home before I have to roll you back.</i>\"\n\n", false);
		Render.text("Unwilling to allow herself to be helped, the succubus staggers up and begins waddling away.  The incubus keeps his distance, wearing a predatory grin.\n\n", false);
	}
...
private function gretasGarments():void {
	...
	if(flags[FlagEnum.FOUND_SOCKS] == 1 && flags[FlagEnum.SOCK_COUNTER] == 0) {
		Render.text("As you enter the store, you're surprised to see that a variety of products line the shelves.  Clothing of various materials, sizes and shapes lay folded on shelves and tables around the little shop.  A lone, surprisingly anatomically correct mannequin stands by the counter nude except for a thin lacy piece of fabric held taut over its wooden penis.");
	...
	if(flags[FlagEnum.FOUND_SOCKS] < 2) {
		...
		if(flags[FlagEnum.OWN_MAIDEN_BIKINI] == 0) Render.text(", except maybe a super-skimpy chain bikini that's chased with white and gold highlights");
	}
	else {
		Render.text("The interior of Greta's Garments is surprisingly colorful.  Though it started off as an empty wagon filled with loose bolts of cloth and sewing tools, vivid fabrics now cover all the shelves.  Curtains hang from the walls in every color of the rainbow, and a single wooden mannequin stands near the counter, its surprisingly anatomically correct cock covered in a thin, lacey cock-sock.  Sitting nearby, behind the counter, is a pink-skinned succubus, busy knitting what looks like another such sock. Even with her slouching posture, you can see that her breasts are truly tremendous - mountainous mammaries that she actually rests her arms on while she knits.  She's completely nude, save for two thin black squares that stretch over her taut nipples (concealing absolutely nothing) and a soaked triangle that's even now threatening to disappear into her gushing crevice forever.");
		Render.text("\n\nNoticing your gaze, she sits up a little straighter and swivels on some kind of rotating chair to face you more directly.  Her jiggling breasts slowly bounce to a stop on the counter before her as she asks, \"<i>Can I interest you in something, honey?</i>\"");
	...
	menu();
	if(flags[FlagEnum.FOUND_SOCKS] == 0) addButton(4,"Low Stock",askGretaAboutInventory);
	else {
		if(flags[FlagEnum.FOUND_SOCKS] == 2 && player.lowerBody.cockSpot.count() > 0 && player.hasSockRoom()) addButton(1,"Browse Socks",browseDemSocksSon);
		if(player.hasSock()) addButton(2,"Remove Sock",takeOffDatSock);
	}
	if(flags[FlagEnum.OWN_MAIDEN_BIKINI] == 0) addButton(0,"Bikini",askGretaAboutZeBikini);
...
private function askGretaAboutInventory():void {
	clearOutput();
	Render.text("Curious about the lack of selection, you broach the topic with the slutty shopkeeper, asking just where the items she has for sale are at.");
...
private function askGretaAboutZeBikini():void {
	...
	//[Buy It] [Back]
	menu();
	if(player.stats.gems < 500) Render.text("\n\n<b>You can't afford it.</b>");
...
private function buyGretasBikini():void {
	...
	player.stats.gems -= 500;
	statScreenRefresh();
	Render.text("Greta's eyes widen in surprise.  \"<i>Really?</i>\"");
...
private function browseDemSocksSon():void {
	clearOutput();
	Render.text("What type of cock-sock do you want to look at?");
	//Cock-sock Menu
	menu();
...
private function woolCockSock():void {
	clearOutput();
	Render.text("You spy a thick, woolen sock sitting on a counter and take it up to Greta.  \"<i>Ah, yes.  That's our basic sock.  Warm and cozy, great for those chilly nights.  That one's a mere 10 gems.  A steal, of course.</i>\"");
...
private function pickACockForSock():void {
	//Buy Cock-sock
	clearOutput();
	Render.text("You take the cock-sock over to the counter where Greta sits, knitting even more garments and place down the gems required.  \"<i>Aha, good choice, honey!</i>\" the succubus says, snatching up the money and stashing it away.  \"<i>Now let's get that bad boy fitted on you.</i>\"");
	...
	if(player.lowerBody.cockSpot.count() == 1) {
		...
		while(button < player.lowerBody.cockSpot.count()) {
			if(player.lowerBody.cockSpot.list[button].sock == "") addButton(button,String(button+1),cockSockTarget,button);
			button++;
		}
	}
}

private function cockSockTarget(target: number):void {
	clearOutput();
	flags[FlagEnum.SOCKS_BOUGHT]++;
	//Putting it On - First Time
	if(flags[FlagEnum.SOCKS_BOUGHT] == 1) {
		Render.text("The gravity-defying succubus gestures towards your crotch.  \"<i>Well, come on then, let's see the tasty cock getting all dressed up,</i>\" she says, her voice becoming a deep purr.  You raise your eyebrow, questioning why she needs to see that.");
	...
	//Putting It On - Additional Times
		...
		Render.text("Greta motions with her hand, a movement that causes her mountainous cleavage to jiggle hypnotically.  \"<i>Well, come on then, let's see the tasty cock getting all dressed up,</i>\" she says, her voice becoming a deep purr.");
...
//Yes
private function yesPutDatSockOnMe(target: number):void {
	clearOutput();
	...
	Render.text("You nod to the busty succubus and strip off your [armor], revealing your naked body.  Greta's eyes light up as she looks over your body with barely-contained lust.  Finally her eyes settle onto your " + cockDescript(target) + ", and she licks her lips.  ");

	if (!conflict) { // There's no conflict. DO IT!!!
		player.lowerBody.cockSpot.list[target].sock = flags[FlagEnum.SOCK_HOLDING];
		statScreenRefresh();

		Render.text("With one hand she lifts your limp cock up, giving it a pleasant little stroke.");
...
private function noCockSock():void {
	...
	Render.text("You shake your head.  Greta sighs, \"<i>Figures.  Here's your money back, honey.  Come back when you change your mind.</i>\"");
	//(Back to menu)
	menu();
	addButton(0,"Next",gretasGarments);
}
...
private function takeOffDatSock():void {
	clearOutput();
	Render.text("Which cock-sock would you like to get removed?");
	//(display list of socked cocks)
	temp = 0;
	let button: number = 0;
	menu();
	while(button < player.lowerBody.cockSpot.count()) {
		if(player.lowerBody.cockSpot.list[button].sock != "") addButton(button,String(button+1),removeTargettedSock,button);
		button++;
	}
}

private function removeTargettedSock(index: number):void {
	clearOutput();
	//Select-A-Cock!
	Render.text("You walk up to the counter top.  Greta the succubus looks up at you over her latest creation, and you explain you'd like to remove a cocksock.");
	...
	if(player.cockArea(index) >= 100) Render.text(", almost knocking Greta over in the process");
	Render.text(", the cock-sock suddenly feeling unforgivingly tight.  With a light giggle, Greta gives your dick a soft kiss, and the burning arousal seems to dissipate, replaced with a cool, relaxing sensation that spreads throughout your body.");
...
private function joeySweetMassage():void {
	...
	statScreenRefresh();
	Render.text("Joey claps excitedly, his little puffy tail bouncing happily.  \"<i>Great!  It's a customer favorite, so I'm sure you'll just love, love, love it!</i>\"  He takes your hand and leads you towards the back rooms, letting the door shut behind the two of you with a soft click.  As usual, in the center of the room is a padded bed meant for you, and at Joey's insistence you disrobe to lie down on it.  You put your face through the rounded oval and relax, taking the moment to watch Joey's bare feet with their cute painted nails prance in and out of sight as he bustles around the room.  He pauses briefly by the coal-fired steam-oven, stoking it for a minute before turning back towards you.");
...
private function eggsInButt(eggButt:boolean = false):void {
	...
	Render.text(", spreading his sweet bounty all over.  His fingers to go work, rubbing up and down");
	if(!player.lowerBody.cockSpot.hasCock()) Render.text(", in and out");
	Render.text(", removing all that pent up tension from inside you.  ");
	if(player.lowerBody.cockSpot.hasCock()) Render.text("[EachCock] begins leaking pre-cum, mixing with Joey's on your belly to create what looks like a chocolate-white chocolate marbled confection.");
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("Your cunt grows ever wetter, and not just because of the new chocolate coating.  Despite Joey's femboy appearance, you've got to hand it to him, he does know his way around a pussy.");
	else Render.text("Your asshole winks and clenches, aching for release that Joey is kind, and skilled, enough to give.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("takes hold of [oneCock] and strokes gently, rubbing his thumb against the head");
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("reaches between his legs, slipping two fingers inside you while his thumb gently rubs your clit");
	else Render.text("reaches between his legs, slipping two fingers inside your ass while his thumb rubs against your perineum");
	Render.text(", and with his free hand he leans forward, tweaking a [nipple] softly.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("[eachCock] pulses and spasms, loosing rope after rope of white jizz through the air, splattering against the wall and floor messily");
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("your pussy tightens around his fingers, suddenly releasing a flood of femcum that splatters messily against the table");
	else Render.text("your asshole twitches and spasms, unable to do much more than grant you a vague half-orgasm");
	Render.text(".  Your entire body tingles as you slowly descend back to the table, practically melting into it as if you were nothing more than the bunny-spunk covering your skin.");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 30) Render.text(", especially compared to you.");
		else if(player.smallestCockArea() < 6) Render.text(", of course, if he's considered tiny, what does that make you?");
	}
	else Render.text(".");
	Render.text("  You suck and slurp, slipping your tongue up and down his length until not a trace of chocolate remains.  His penis continues to leak the chocolatey pre-cum, but the deeper you take him, the less it matters, until he's practically pumping the chocolate directly into your stomach.");
...
private function waitOnStuff():void {
	clearOutput();
	Render.text("Uncertainty plagues you as you wait to see what happens.  The large feline reaches the entrance of the large tent.  His free hand grips the tent and, as you let out a loud yelp, you are shoved forcefully through.");
...
private function assaultWinAndLeave():void {
	clearOutput();
	Render.text("You shove the large tiger man across the dirt.  His annoyed growls are muffled by the dirt his face is sliding across.  You issue a warning about picking fights with those stronger than he is and leave him lying in the dirt, his pride battered and bruised.  The dirt under your heel grinds in the darkness as you turn and casually continue down the path until you find yourself back in the bazaar.");
	//gtfo
	menu();
	addButton(0,"Next", enterTheBazaarAndMenu);
}
...
private function abuseHisAss():void {
	...
	//[(If multicock)
	if(player.lowerBody.cockSpot.count() > 1) Render.text("  Your hand strokes along your " + multiCockDescriptLight()  + " and press them to the warm cheeks exposed before you.");
	else Render.text("  You pump your " + multiCockDescriptLight() + " and milk a pearl of pre-cum out to drip onto the enticing ass before you.");
	Render.text("  \"<i>O-OI!</i>\"  The tiger man shouts out, his head turning to try to see behind him.  With a grin on your lips you grip the hilt of [oneCock] and slap its length over the tiger's rump.  You can see the tint to the tiger man's cheek as he hears and feels your dick smacking his exposed ass.  Pre-cum drips down [oneCock] as you line yourself up, jamming it between the tiger's lovely mounds. He gives a surprised shout as your cockhead presses to his tiny, puckered hole.");
	...
	if(player.smallestCockLength() >= 16) {
		...
		if(player.totalCocks() == 1) Render.text("  You squeeze your " + multiCockDescriptLight() + " between his warm ass cheeks, stroking yourself with his exposed bottom.  The tiger whimpers meekly under you.");
		else Render.text("  Your fattest cock grinds between the tiger man's rear, dripping spunk over his back and clothing while your other cocks make a sloppy mess of his ass.");
		Render.text("  The tiger man's muscled butt is soon glazed with your pre-cum.  Wet, slapping noises echo out into the alley as you grind your hips into his rear.  You can't help but think in the back of your head that the noise is stirring people inside the tent he came out of.");
	}
	//[[(If largest cock is smaller than 16)
		...
		//[(If multicock)
		if(player.lowerBody.cockSpot.count() > 1) Render.text("  One of your cocks slips into his hot, constricting confines, the remaining lengths pressed fast to the tiger man's rear and leaving glistening trails of eager fluids like markings on a bitch's ass.");
		else Render.text("  Your " + multiCockDescriptLight()  + " pushes in, your pre-cum-slicked length sliding in with a wet noise.");
		Render.text("  The burly man under you whimpers in protest as you begin to rock against his fine ass.  His firm rear dimples and twitches under your hips, each slam");
		if(player.lowerBody.balls > 0) Render.text(" swatting your " + ballsDescriptLight()  + " against him,");
		Render.text(" making him shout out in discomfort.  Your " + multiCockDescriptLight()  + " stretching him out");
		if(player.lowerBody.cockSpot.count() > 1) Render.text(" while beating his rear");
		Render.text(".  All his shouting stirs whoever resides in the tent he came out of.");
	}
...
private function collectSomeButtSmex():void {
	clearOutput();
	Render.text("Your body crashes into a wooden table and you choke, feeling the wind knocked out of you.  You struggle to lift your head and catch your breath, only to find yourself laying in... poker chips?");
	...
	Render.text("  Without warning the hand is gone");
	if(player.lowerBody.cockSpot.count() > 1) Render.text(" and your " + multiCockDescriptLight()  + " wetly slaps your stomach with need");
	Render.text(".  A whimper escapes your lips.  \"<i>This should be fine...</i>\"  The cool, collected voice of the satyr murmurs.");
	...
	if(player.stats.lib < 50) Render.text("fear");
	else Render.text("excitement");
	Render.text(".");
	...
	if(player.isBiped()) Render.text("  The bear shifts where he stands, grabbing your ankles and spreading your limbs almost painfully wide.  You wince from the sting of the strain on your leg muscles.");
	Render.text("  You hear the clopping of hooves on the floor as the satyr moves over to your " + buttDescript()  + ".  His hands grip over your " + hipDescript()  + " and drags you closer to the edge of the table so that your " + buttDescript()  + " hangs over the side of the table.  You raise your head to watch with a ");
	if(player.stats.lib < 50) Render.text("horrified");
	else Render.text("excited");
	Render.text(" expression as the satyr's hand strokes over his hard length.  It looks at least eight inches long, smooth like a human's cock and protruding from a forest of curly, brown fur.  The satyr lines himself up and rubs his stiff cockhead over your " + assholeDescript()  + ".");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text(" while [eachCock] twitches and leaks generously over your " + player.skinFurScales());
	Render.text(".");
...
private function finalGayFinallee(road: number = 0):void {
	...
	if(road == 0) {
		...
		if(player.lowerBody.cockSpot.hasCock()) Render.text("His hand dips down to play with your " + multiCockDescriptLight()  + ", stroking you off as he gives a few more brutal pounds of his hips.  ");
		else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("Fingers ram into your " + vaginaDescript(0)  + ", thrusting into you as he fucks your other hole.  ");
		Render.text("His face near yours, he licks your cheek and plants a brief kiss on your lips. You give him a confused look at the affection and he meets it with a wide grin.  \"<i>You're cute for a cocksucker.  Knew I had it made when I found you.</i>\" he snickers, causing you to blush.");
		if(player.gender > 0) Render.text("  Your lips part as you moan, feeling your climax building before coating the tiger's hand and your lower body in your sex.  The tiger man just brings his hand up to his lips to taste your juices before hilting into you one last time.");
		Render.text("  A third wave  of warmth floods you, the pressure growing even higher as you resemble an overstuffed, breeding bitch.  Exhaustion catches up with you and you begin to feel your eyes grow heavy.  The last thing you see is the feline's face contorted in pleasurable orgasm before you nod off.");
	...
	//((Ending 2 , suck bear, //crazy steel jaw))
		...
		Render.text("  His face near yours, he licks your cheek and plants a brief kiss on your lips.  You give him a confused look at the affection and he meets it with a wide grin.  \"<i>You're cute for a cocksucker.  Loved the way you went right for my dick.</i>\"  He snickers, causing you to blush.");
		if(player.gender > 0) Render.text("  Your lips part as you moan, feeling your climax building before coating the bear's hand and your lower body in your sex.  The bear just brings his hand up to his lips to taste your honey before hilting into you one last time.");
		Render.text("  A third wave of warmth floods you, the pressure growing even higher as you resemble an overstuffed, breeding bitch.  Exhaustion catches up with you and you begin to feel your eyes grow heavy.  The last thing you see is the bear's face contorted in pleasurable orgasm before you nod off.");
	...
	//((Ending 3 , suck none, //No furry plx))
		...
		if(player.lowerBody.cockSpot.hasCock()) Render.text("  His hand drips down to clumsily play with your " + multiCockDescriptLight()  + ", stroking you off as he gives you a few more brutal pounds of his hips.");
		else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Fingers dip into your " + vaginaDescript(0)  + ", ramming into you as he fucks your other hole.");
		if(player.gender > 0) Render.text("  Your lips part as you feel your climax building, and soon you coat the orc's hand and your lower body in your lust.  The orc just brings his hand up to taste your juices before hilting into you one last time.");
		Render.text("  A third wave of warmth floods you and the pressure grows even higher as you resemble an overstuffed breeding bitch.  Exhaustion catches up with you and your eyes begin to grow heavy.  The last thing you see is the orc's face twisted in orgasmic pleasure before you pass out.");
