...
private function approachBazaarGuard():void {
	...
	if(player.stats.cor < 33) outputText("Leave at once.  You are not yet ready for the wonders of the Bazaar.", false);
...
private function theSlipperySqueeze():void {
	...
	if(isEaster()) {
		...
		if(player.stats.gems < 20) outputText("  You can't afford it.");
...
private function joeyMassage():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  Would you like one?", false);
	outputText("</i>\"", false);
...
private function joeysMassageWifNoExtraJizz():void {
	...
	if(!player.upperBody.chest.hasFuckableNipples()) outputText("tweaks and teases", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.balls > 0) outputText("his hand firmly rubs your " + sackDescript() + ", and ", false);
	...
	//(FEM/GENDERLESS) 
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("presses his ruby lips into the glistening delta of your mons", false);
	...
	if(!player.lowerBody.cockSpot.hasCock()) {
		...
		//(Female orgasm)
			...
			if(player.lowerBody.vaginaSpot.list[0].clitLength >= 4) outputText("like a practiced whore fellating a john.", false);
...
private function joeysMassageWithEXTRASpooge():void {
	...
	if(player.lowerBody.balls == 0) outputText("inside you", false);
	else outputText("in your " + ballsDescriptLight(), false);
	outputText(" that reminds you of the sensation just before orgasm.", false);
	if(player.lowerBody.balls > 0) outputText("  They even feel a little bigger.", false);
	outputText("\n\n", false);
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("s like fiddles", false);
	...
	if(player.lowerBody.balls > 0) outputText("Your balls pull and relax and pull tight, bouncing below you over and over, all while working hard to produce more juice for you to gush.  ", false);
	...
	if(player.skinType != SKIN.FUR) outputText("over your " + player.skinDesc, false);
	...
	if(!player.upperBody.chest.hasFuckableNipples()) outputText("squeezing and tugging on them", false);
...
private function joeyBigBalls():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00348] == 0) {
		...
		if(player.stats.cor > 60) outputText(", with no more than a bit of wishful thinking,", false);
		...
		if(player.stats.cor > 70) outputText("; you won't get to watch him fountaining all that pearly spunk like a perverted statue", false);
...
private function suckOffJoeysGardenHose():void {
	...
	if(player.stats.cor < 33) outputText("all for a good cause, right?", false);
	else if(player.stats.cor < 66) outputText("also kind of fun, in a naughty way.", false);
...
private function gretasGarments():void {
	...
	if(flags[FlagEnum.FOUND_SOCKS] < 2) {
		...
		if(flags[FlagEnum.OWN_MAIDEN_BIKINI] == 0) outputText(", except maybe a super-skimpy chain bikini that's chased with white and gold highlights");
	}
	else {
...
private function askGretaAboutZeBikini():void {
	...
	//[Buy It] [Back]
	menu();
	if(player.stats.gems < 500) outputText("\n\n<b>You can't afford it.</b>");
...
private function eggsInButt(eggButt:boolean = false):void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("own");
	...
	if(!player.lowerBody.cockSpot.hasCock()) outputText(", in and out");
	outputText(", removing all that pent up tension from inside you.  ");
	if(player.lowerBody.cockSpot.hasCock()) outputText("[EachCock] begins leaking pre-cum, mixing with Joey's on your belly to create what looks like a chocolate-white chocolate marbled confection.");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Your cunt grows ever wetter, and not just because of the new chocolate coating.  Despite Joey's femboy appearance, you've got to hand it to him, he does know his way around a pussy.");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("takes hold of [oneCock] and strokes gently, rubbing his thumb against the head");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("reaches between his legs, slipping two fingers inside you while his thumb gently rubs your clit");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("[eachCock] pulses and spasms, loosing rope after rope of white jizz through the air, splattering against the wall and floor messily");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your pussy tightens around his fingers, suddenly releasing a flood of femcum that splatters messily against the table");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 30) outputText(", especially compared to you.");
...
private function abuseHisAss():void {
	...
	//[(If multicock)
	if(player.lowerBody.cockSpot.count() > 1) outputText("  Your hand strokes along your " + multiCockDescriptLight()  + " and press them to the warm cheeks exposed before you.");
	...
	//[[(If largest cock is smaller than 16)
		...
		//[(If multicock)
		if(player.lowerBody.cockSpot.count() > 1) outputText("  One of your cocks slips into his hot, constricting confines, the remaining lengths pressed fast to the tiger man's rear and leaving glistening trails of eager fluids like markings on a bitch's ass.");
		...
		if(player.lowerBody.balls > 0) outputText(" swatting your " + ballsDescriptLight()  + " against him,");
		outputText(" making him shout out in discomfort.  Your " + multiCockDescriptLight()  + " stretching him out");
		if(player.lowerBody.cockSpot.count() > 1) outputText(" while beating his rear");
...
private function collectSomeButtSmex():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your " + vaginaDescript(0)  + " grows slick in your excitement.");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" wrap around your " + multiCockDescriptLight()  + ".");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" and hard shaft");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText(" and your " + multiCockDescriptLight()  + " wetly slaps your stomach with need");
	...
	if(player.stats.lib < 50) outputText("fear");
	...
	if(player.stats.lib < 50) outputText("horrified");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" while [eachCock] twitches and leaks generously over your " + player.skinFurScales());
	outputText(".");
...
private function finalGayFinallee(road: number = 0):void {
	...
	if(road == 0) {
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("His hand dips down to play with your " + multiCockDescriptLight()  + ", stroking you off as he gives a few more brutal pounds of his hips.  ");
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Fingers ram into your " + vaginaDescript(0)  + ", thrusting into you as he fucks your other hole.  ");
	...
	//((Ending 3 , suck none, //No furry plx))
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("  His hand drips down to clumsily play with your " + multiCockDescriptLight()  + ", stroking you off as he gives you a few more brutal pounds of his hips.");
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Fingers dip into your " + vaginaDescript(0)  + ", ramming into you as he fucks your other hole.");
