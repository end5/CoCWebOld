...
private theSlipperySqueeze():void {
	...
	//(Repeat visit: 
		...
		MainScreen.text("Joey purses his glossed lips when you enter and coyly cocks one of his ears as he says, \"<i>Welcome back to 'The Slippery Squeeze', " + player.short + ".  Would you like me to give you a nice, salty rub-down?  Or would you prefer Sara do it?  Just remember, she can't make her own 'oil' like I can.</i>\"\n\n", false);
		//No scenes for Sara yet!
		MainScreen.text("\"<i>I told you I'm not working until you give me a raise!  Do the damned massages yourself!</i>\"\n\nJoey blushes and apologies.  \"<i>I guess I'm the only one available for now.</i>\"\n\n", false);
		MainScreen.text("The price list indicate it's 10 gems for a massage, though the gleefully illustrated 'oil' reminds you just what they plan to use on you.\n\n", false);
		if(!player.perks.has("Androgyny")) {
			MainScreen.text("There also appears to be an option for a special 'Androgyny Treatment' that costs 500 gems.  Joey catches you looking at it and mutters, \"<i>That treatment isn't fun like our massages.  It would unlock the full potential of your visage, allowing it to be as masculine or feminine as possible.</i>\"\n\n", false);
...
private noMilkerPlzJoey():void {
	MainScreen.clearText();
	MainScreen.text("You decline; it's not really the sort of thing you need in your camp.  \"<i>Ah well,</i>\" Joey shrugs, \"<i>I'll just have to hang onto it for now I guess.  Shame.  Anyway,</i>\" he resumes his usual grin, \"<i>is there something else you need?  A massage, perhaps?</i>\"");
...
private joeyAndrogyny():void {
	...
	if(player.stats.gems < 500) {
		MainScreen.text("You haven't got enough gems for that treatment!", false);
		doNext(theSlipperySqueeze);
		return;
	}
	player.stats.gems -= 500;
	statScreenRefresh();
	MainScreen.text("Joey takes your hand in his and insistently pulls towards the back rooms.  \"<i>Once we have this all done ", false);
	if(player.gender == 1) MainScreen.text("you'll be able to be as cute as me!", false);
	else if(player.gender == 2) MainScreen.text("you'll be able to be as masculine as you want!", false);
...
private joeyMassage():void {
	...
	if(player.stats.gems < 10) {
		MainScreen.text("Joey frowns when you realize you don't have the 10 gems.  He apologizes, \"<i>I'm sorry, " + player.short + " but I can't give freebies - our special potions cost us plenty.", false);
		doNext(enterTheBazaar);
		return;
	}
	player.slimeFeed();
...
private joeysMassageWithEXTRASpooge():void {
	...
	MainScreen.text("Even though you're draining spunk at an alarming rate, it doesn't feel like a normal orgasm.  There's no wave of pressure and subsequent release, just a constant, pulsing contraction that makes you melt with ever-increasing feelings of satisfaction.\n\n", false);
	
	MainScreen.text("Joey giggles, turning about to put his lotion-dispenser on the small of your back, and you feel the hot bunny-spoo pour onto you in a wave.   It's gathered up and pushed up your back.  Globs of it roll down your side, sliding over the semi-waterproofed sheets to mix with your growing spunk-lake.  With the slippery-seed aiding Joey's massage, he somehow manages to work out even more of your tension.  By this point your muscles feel like jello melting in the afternoon sun.  You feel like you could melt into your cum, and you sigh in bliss", false);
...
private joeyBigBalls():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00348] == 0) {
		...
		if(player.stats.cor > 60) MainScreen.text(", with no more than a bit of wishful thinking,", false);
...
private suckOffJoeysGardenHose():void {
	...
	MainScreen.text("Joey's rouged lips silently mouth, \"<i>Oh gods,</i>\" as he delivers the next, even bigger rope to your eager maw, stuffing your mouth so quickly that his jism squirts from the corners before you can react.  You shiver from the semen overload", false);
	if(player.minotaurAddicted()) MainScreen.text(" and find yourself wishing you had one of these potions to use on a minotaur.  It would be divine!", false);
...
private overHearDemonsAboutSyrena():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00292] == 0) {
		...
		MainScreen.text("A baritone response rings out, \"<i>You're shitting me!  Syrena's off her fucking rocker if she thinks Lethice will let her turn another of our researchers into a gibbering slut.  We've got enough of those.</i>\"\n\n", false);
	...
	//[Listen in repeat]
		...
		MainScreen.text("The succubus laughs and slaps his ass as she answers, \"<i>Don't worry, you taste great... just not as good as the boss.  I dunno what concoction she made that did it, but her spunk is heavenly.  I could just... mmm... swallow that delicious cream all day long.  Do ya think she's part minotaur?</i>\"\n\n", false);
	...
	//[Listen in Repeat 2]  
		...
		MainScreen.text("\"<i>So does that mean you won't mind if I stop by whilst you're laid up?  I mean you can't get any more pregnant with imps, and it might pass the time,</i>\" suggests the well-muscled male.\n\n", false);
		MainScreen.text("Smirking, the demoness retorts, \"<i>Sure, but next time you fuck up, I'm loaning Syrena some of my girly stuff to make you suffer just like this.</i>\"  Her hands both point at the imp-engorged skin of her midsection for emphasis.\n\n", false);
		MainScreen.text("The incubus rises and taunts, \"<i>If I fucked up as often as you did, I'd be feeding the furnace in a factory somewhere or fed to the hellhounds.  Come on, let's get you home before I have to roll you back.</i>\"\n\n", false);
		MainScreen.text("Unwilling to allow herself to be helped, the succubus staggers up and begins waddling away.  The incubus keeps his distance, wearing a predatory grin.\n\n", false);
	}
...
private askGretaAboutInventory():void {
	MainScreen.clearText();
	MainScreen.text("Curious about the lack of selection, you broach the topic with the slutty shopkeeper, asking just where the items she has for sale are at.");
...
private askGretaAboutZeBikini():void {
	...
	//[Buy It] [Back]
	menu();
	if(player.stats.gems < 500) MainScreen.text("\n\n<b>You can't afford it.</b>");
...
private browseDemSocksSon():void {
	MainScreen.clearText();
	MainScreen.text("What type of cock-sock do you want to look at?");
	//Cock-sock Menu
	menu();
...
private pickACockForSock():void {
	//Buy Cock-sock
	MainScreen.clearText();
	MainScreen.text("You take the cock-sock over to the counter where Greta sits, knitting even more garments and place down the gems required.  \"<i>Aha, good choice, honey!</i>\" the succubus says, snatching up the money and stashing it away.  \"<i>Now let's get that bad boy fitted on you.</i>\"");
...
private cockSockTarget(target: number):void {
	...
	//Putting It On - Additional Times
		...
		MainScreen.text("Greta motions with her hand, a movement that causes her mountainous cleavage to jiggle hypnotically.  \"<i>Well, come on then, let's see the tasty cock getting all dressed up,</i>\" she says, her voice becoming a deep purr.");
...
private takeOffDatSock():void {
	MainScreen.clearText();
	MainScreen.text("Which cock-sock would you like to get removed?");
...
private joeySweetMassage():void {
	...
	statScreenRefresh();
	MainScreen.text("Joey claps excitedly, his little puffy tail bouncing happily.  \"<i>Great!  It's a customer favorite, so I'm sure you'll just love, love, love it!</i>\"  He takes your hand and leads you towards the back rooms, letting the door shut behind the two of you with a soft click.  As usual, in the center of the room is a padded bed meant for you, and at Joey's insistence you disrobe to lie down on it.  You put your face through the rounded oval and relax, taking the moment to watch Joey's bare feet with their cute painted nails prance in and out of sight as he bustles around the room.  He pauses briefly by the coal-fired steam-oven, stoking it for a minute before turning back towards you.");
...
private eggsInButt(eggButt:boolean = false):void {
	...
	if(!player.lowerBody.cockSpot.hasCock()) MainScreen.text(", in and out");
	MainScreen.text(", removing all that pent up tension from inside you.  ");
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("[EachCock] begins leaking pre-cum, mixing with Joey's on your belly to create what looks like a chocolate-white chocolate marbled confection.");
	else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("Your cunt grows ever wetter, and not just because of the new chocolate coating.  Despite Joey's femboy appearance, you've got to hand it to him, he does know his way around a pussy.");
	else MainScreen.text("Your asshole winks and clenches, aching for release that Joey is kind, and skilled, enough to give.");
	...
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("takes hold of [oneCock] and strokes gently, rubbing his thumb against the head");
	else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("reaches between his legs, slipping two fingers inside you while his thumb gently rubs your clit");
	else MainScreen.text("reaches between his legs, slipping two fingers inside your ass while his thumb rubs against your perineum");
	MainScreen.text(", and with his free hand he leans forward, tweaking a [nipple] softly.");
	...
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("[eachCock] pulses and spasms, loosing rope after rope of white jizz through the air, splattering against the wall and floor messily");
	else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("your pussy tightens around his fingers, suddenly releasing a flood of femcum that splatters messily against the table");
	else MainScreen.text("your asshole twitches and spasms, unable to do much more than grant you a vague half-orgasm");
	MainScreen.text(".  Your entire body tingles as you slowly descend back to the table, practically melting into it as if you were nothing more than the bunny-spunk covering your skin.");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 30) MainScreen.text(", especially compared to you.");
		else if(player.smallestCockArea() < 6) MainScreen.text(", of course, if he's considered tiny, what does that make you?");
	}
	else MainScreen.text(".");
	MainScreen.text("  You suck and slurp, slipping your tongue up and down his length until not a trace of chocolate remains.  His penis continues to leak the chocolatey pre-cum, but the deeper you take him, the less it matters, until he's practically pumping the chocolate directly into your stomach.");
...
private waitOnStuff():void {
	MainScreen.clearText();
	MainScreen.text("Uncertainty plagues you as you wait to see what happens.  The large feline reaches the entrance of the large tent.  His free hand grips the tent and, as you let out a loud yelp, you are shoved forcefully through.");
...
private collectSomeButtSmex():void {
	MainScreen.clearText();
	MainScreen.text("Your body crashes into a wooden table and you choke, feeling the wind knocked out of you.  You struggle to lift your head and catch your breath, only to find yourself laying in... poker chips?");
	...
	if(player.stats.lib < 50) MainScreen.text("fear");
	else MainScreen.text("excitement");
	MainScreen.text(".");
...
private finalGayFinallee(road: number = 0):void {
	...
	if(road == 0) {
		...
		if(player.stats.gems > 1) {
			MainScreen.text(" with only a few gems missing");
