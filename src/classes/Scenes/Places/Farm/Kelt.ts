	...
	public class Kelt extends Monster
	{
		...
		private KellyuraAttack():void {
			...
			//(3)
				...
				if(player.lust >= 80) MainScreen.text("Your hand moves towards your groin seemingly of its own volition.");
		...
		override protected performCombatAction():void
		{
			if (statusAffects.get("BowCooldown").value1 > 0) {
				...
				if (rand(2) == 0 && flags[FlagEnum.KELT_BREAK_LEVEL] >= 2) dayDreamKelly();
				else keltShootBow();
			}
		...
		{
			if(game.flags[FlagEnum.KELT_BREAK_LEVEL] == 1) game.farm.kelly.defeatKellyNDBREAKHIM();
		...
		public Kelt()
		{
			...
			this.lowerBody = LOWER_BODY.CENTAUR;
			this.skinTone = "tan";
