package classes.Scenes.Places.Farm
{
	...
	public class FarmCorruption extends AbstractFarmContent
	{
		...
		public function corruptFollowers(): number
		{
			let count: number = 0;
		...
		public function whitneyCorruption(mod:number = 0): number
		{
			if (mod != 0)
			{
		...
		public function updateFarmCorruption(): number
		{
		...
		public function collectTheGoodies():void
		{
			...
			{
				Render.text("You stroll over to the big rock at the edge of the pepper patch, smiling as you see a small burlap sack shoved underneath a fold in the stone. You scoop it up, open it and count out " + flags[FlagEnum.FARM_CORRUPTION_GEMS_WAITING] + " gems.\n\n");
				
				if (farmValue() < 25) Render.text(" You frown; it seems like a feeble amount for such a large operation. Perhaps you could talk to Whitney about that.\n\n");
			...
			{
				Render.text("Your ‘farmers’ have been busy under the watchful eye of their assigned task mistress. A small bundle of goods have been stashed with the gems just awaiting your arrival.\n\n");
			}
			...
			{
				Render.text("<b>" + flags[FlagEnum.FARM_SUCCUMILK_STORED] + "x ");
				if (flags[FlagEnum.FARM_SUCCUMILK_STORED] == 1) Render.text("vial of Succubi milk.");
				else Render.text("vials of Succubi milk.");
				Render.text("</b>\n");
				addButton(0, getItemObj(FlagEnum.FARM_SUCCUMILK_STORED).shortName, takeItems, FlagEnum.FARM_SUCCUMILK_STORED);
			}
			if (flags[FlagEnum.FARM_INCUDRAFT_STORED] > 0)
			{
				Render.text("<b>" + flags[FlagEnum.FARM_INCUDRAFT_STORED] + "x ");
				if (flags[FlagEnum.FARM_INCUDRAFT_STORED] == 1) Render.text("vial of Incubi draft.");
				else Render.text("vials of Incubi draft.");
				Render.text("</b>\n");
				addButton(1, getItemObj(FlagEnum.FARM_INCUDRAFT_STORED).shortName, takeItems, FlagEnum.FARM_INCUDRAFT_STORED);
			}
			if (flags[FlagEnum.FARM_EGG_STORED] > 0)
			{
				Render.text("<b>" + flags[FlagEnum.FARM_EGG_STORED] + "x ");
				Render.text("large " + flags[FlagEnum.FOLLOWER_PRODUCTION_SOPHIE_COLORCHOICE] + " egg");
				if (flags[FlagEnum.FARM_EGG_STORED] > 1) Render.text("s");
				Render.text(".");
				Render.text("</b>\n");
				addButton(2, getItemObj(FlagEnum.FARM_EGG_STORED).shortName, takeItems, FlagEnum.FARM_EGG_STORED);
			}
			if (flags[FlagEnum.FARM_CONTRACEPTIVE_STORED] > 0)
			{
				Render.text("<b>" + flags[FlagEnum.FARM_CONTRACEPTIVE_STORED] + "x ");
				Render.text("bundle of Herbal Contraceptive.");
				Render.text("</b>\n");
				addButton(3, getItemObj(FlagEnum.FARM_CONTRACEPTIVE_STORED).shortName, takeItems, FlagEnum.FARM_CONTRACEPTIVE_STORED);
			}
			
			addButton(9, "Back", rootScene);
		}
		
		private function getItemObj(flag: number):SimpleConsumable
		{
		...
		private function takeItems(flag: number):void
		{
		...
		public function farmProtection(): number
		{
		...
		public function farmValue(): number
		{
			let fValue: number = 0;
		...
		public function takeoverPrompt():boolean
		{
			...
			if (player.stats.cor >= 70 && corruptFollowers() >= 2 && player.level >= 12)
			{
				...
				{	
					...
					else if (player.statusAffects.has("MarbleRapeAttempted"))
					{
		...
		protected function takeoverPromptKelly():void
		{
			...
			{
				Render.text(" and the rodent slut");
				if (kGAMECLASS.amilyScene.amilyCorrupt() && kGAMECLASS.jojoScene.campCorruptJojo())
				{
					Render.text("s");
				}
				Render.text(" before him");
			}
			Render.text("?");
		}
		...
		protected function takeoverPromptMerge(firstTime:boolean = false):void
		{
			...
			else
			{
				...
				Render.text("Again, you find yourself standing on the bluff overlooking the farm, and you feel yourself filled with unholy rage at the woman below who stands against you and your plans for this piece of property. Do you put your plan into motion now or later?");
			}
		...
		public function takeoverPromptNow():void
		{
			...
			Render.text("You stride down to the farm and leap over a gate. You move casually, swaggering towards the pepper field with no obvious intent. When Whitney spots you and slowly stands up from her weeding, you raise your hand in friendly greeting"); 
			if (player.weaponName != "fists") Render.text(", and to demonstrate your peacefulness, you theatrically hold up your [weapon] and then discard it with a careless swing of your arm.");
			else Render.text(".");
			...
			if (flags[FlagEnum.KELT_BREAK_LEVEL] >= 4 || player.statusAffects.has("MarbleRapeAttempted"))
			{
				...
				if (flags[FlagEnum.KELT_BREAK_LEVEL] >=  4) Render.text(" Kelt");
				else Render.text(" Marble");
				Render.text(" I cannot properly protect the place, so maybe I do need... insurance.</i>” She spits the last word. “<i>Just so long as you stay the fuck away from me, I will do as you say.</i>” You beam.");
			}
			else
			{
		...
		protected function takeoverPromptLater():void
		{
			...
			Render.text("You stare for a moment longer, then turn and head back to camp. You will show mercy she does not deserve... for now.");
		...
		public function rootScene():void
		{
			...
			if (!whitneyCorrupt())
			{
				Render.text("You stand on the rise you’ve taken to using to look down on the farm which you are invested in.")
				
				if (flags[FlagEnum.FARM_UPGRADES_REFINERY] == 1) Render.text(" A large machine, bulky and rotund with a conical top, has been built into the milking barn. Fat pipes crawl up onto the roof from it like metal ivy, and white smoke billows busily out of its whistle chimney.");
				
				if (flags[FlagEnum.FARM_UPGRADES_CONTRACEPTIVE] == 1) Render.text(" Next to the pepper patch another crop has been planted, a field of verdant green shrubs. Their thin stems bob idly in the breeze.");
			...
			{
				Render.text("You stand on your rise and take in your slave farm. ");
				if (silly())
				{
					Render.text(player.mf("You feel an irresistible hankering for a cigar.", "You feel an irresistible hankering for a cigarette holder."));
				}
				if (flags[FlagEnum.FARM_UPGRADES_REFINERY] == 1) Render.text(" A large machine, bulky and rotund with a conical top, has been built into the milking barn. Fat pipes crawl up onto the roof from it like metal ivy, and white smoke billows busily out of its whistle chimney.");
				if (flags[FlagEnum.FARM_UPGRADES_CONTRACEPTIVE] == 1) Render.text(" Next to the pepper patch another crop has been planted, a field of verdant green shrubs. Their thin stems bob idly in the breeze.");
			}
		...
		public function farmMenu():void
		{
			...
			if (player.findStatusAffect(StatusAffects.MarbleRapeAttempted) < 0 && player.findStatusAffect(StatusAffects.NoMoreMarble) < 0 && player.statusAffects.has("Marble") && flags[FlagEnum.MARBLE_WARNING] == 0) addButton(1, "Marble", farm.meetMarble);
			
			if (player.statusAffects.has("Kelt") && player.findStatusAffect(StatusAffects.KeltOff) < 0)
			{
				if (flags[FlagEnum.KELT_BREAK_LEVEL] >= 4) addButton(2, "Kelly", farm.kelly.breakingKeltOptions);
				else if (flags[FlagEnum.KELT_BREAK_LEVEL] == 0 && flags[FlagEnum.KELT_TALKED_FARM_MANAGEMENT] == 0) addButton(2, "Kelt", keltAChangeInManagement);
		...
		private function corruptingTheFarmExplore():void
		{
			...
			if (flags[FlagEnum.FARM_CORRUPTION_DAYS_SINCE_LAST_PAYOUT] >= 7 || flags[FlagEnum.FARM_SUCCUMILK_STORED] > 0 || flags[FlagEnum.FARM_INCUDRAFT_STORED] > 0 || flags[FlagEnum.FARM_EGG_STORED] > 0 || flags[FlagEnum.FARM_CONTRACEPTIVE_STORED] > 0) addButton(2, "Collect", collectTheGoodies);
		...
		private function keltAChangeInManagement():void
		{
			...
			Render.text("“<i>Hear there’s been a change in management,</i>” says Kelt, clopping to a halt in front of you. You confirm that that is the case. The big centaur looks at you thoughtfully. There’s something different in his dark eyes and rugged scowl than his usual wearied contempt. Grudging admiration?");

			Render.text("“<i>Find it difficult to believe someone like you could put a bitch in her place,</i>” he says eventually. “<i>Perhaps you’ve got bigger balls than I thought you had."); 
			if (player.lowerBody.balls == 0) Render.text(" So to speak, anyway.");
			Render.text("</i>” He snorts, and trots towards the butts. “<i>Just don’t expect me to treat you any different. As long as you’re getting free instruction from me, I’m the master and you’re the whelp. Got that?</i>”");
			...
			Render.text("“<i>Do I look like I want or need your sloppy seconds? Godsdamn, I can only imagine what kind of pathetic creatures would have </i>you<i> lording it over them. What a joke! Now, are we going to go watch you fail miserably to hit a target from five yards or what?</i>”");

			Render.text("It’s better than you were expecting from him in all honesty. You take your bow out and silently follow him to the archery range.");
		...
		private function numSlavesAtFarm(): number
		{
			let count: number = 0;
		...
		private function slavesAtFarmMenu():void
		{
			...
			if (flags[FlagEnum.FOLLOWER_AT_FARM_AMILY] == 1) addButton(0, "Amily", kGAMECLASS.amilyScene.amilyFollowerEncounter);
			
			if (flags[FlagEnum.FOLLOWER_AT_FARM_JOJO] == 1) addButton(1, "Jojo", kGAMECLASS.jojoScene.corruptCampJojo);
			
			if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 1 && kGAMECLASS.sophieBimbo.bimboSophie()) addButton(2, "Sophie", kGAMECLASS.sophieBimbo.approachBimboSophieInCamp);
			
			if (flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 1) addButton(3, "Vapula", kGAMECLASS.vapula.callSlaveVapula);
			
			if (flags[FlagEnum.FOLLOWER_AT_FARM_LATEXY] == 1) addButton(4, flags[FlagEnum.GOO_NAME], kGAMECLASS.latexGirl.approachLatexy);
			
			if (flags[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] == 1) addButton(5, flags[FlagEnum.MILK_NAME], kGAMECLASS.milkWaifu.milkyMenu);
			
			addButton(9, "Back", farmMenu);
		}
		
		private function numFollowersAtFarm(): number
		{
			let count: number = 0;
		...
		private function followersAtFarmMenu():void
		{
			...
			if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 1 && !kGAMECLASS.sophieBimbo.bimboSophie()) addButton(0, "Sophie", kGAMECLASS.sophieFollowerScene.followerSophieMainScreen);
			
			addButton(9, "Back", farmMenu);
		}
		
		private function numLoversAtFarm(): number
		{
			let count: number = 0;
		...
		private function loversAtFarmMenu():void
		{
			...
			if (flags[FlagEnum.FOLLOWER_AT_FARM_IZMA] == 1) addButton(0, "Izma", kGAMECLASS.izmaScene.izmaFollowerMenu);
			if (flags[FlagEnum.FOLLOWER_AT_FARM_IZMA] == 2) addButton(0, "Izmael", kGAMECLASS.gameOver);
			
			if (flags[FlagEnum.FOLLOWER_AT_FARM_ISABELLA] == 1) addButton(1, "Isabella", kGAMECLASS.isabellaFollowerScene.callForFollowerIsabella);
		...
		private function dogeNotCorruptYet():void
		{
			...
			{
				flags[FlagEnum.FARM_CORRUPTION_APPROACHED_WHITNEY] = 1;
				Render.text("You find Whitney sat in the cow field, hand milking one of her herd; her crossbow is propped up against a bucket. As you approach, she looks up sharply before slowly reaching for the weapon. You stop in your tracks, and she stops in kind, her hand frozen on the grip. You take a step forward and she picks it up, her finger finding the trigger, her expression entirely emotionless. You take several hasty steps back, she puts it down and goes back to milking peacefully.");
			...
			if (whitneyCorruption() >= 31 && whitneyCorruption() <= 60)
			{
				if (flags[FlagEnum.WHITNEY_MENU_31_60] == 0)
				{
					...
					Render.text("Once again you approach Whitney, this time as she is carrying full buckets of milk towards the bottling station. She sees you coming and quickens her step. You notice this time however she doesn’t have her crossbow, and this emboldens you.");
				...
				{
					// Subsequent 31-60: 
					Render.text("Whitney puts down her work as you stride towards her, resigned unhappiness in her expression.");
			...
			if (whitneyCorruption() >= 61 && whitneyCorruption() <= 90)
			{
				if (flags[FlagEnum.WHITNEY_MENU_61_90] == 0)
				{
					...
					Render.text("You find Whitney on the steps of the porch to her small house, adjoined to the storage barn. There’s a book next to her but she is simply sat drinking in the sun, her eyes closed. When she opens them and sees you standing there she smiles at you wanly; it’s the first time you’ve seen her do that for some time. There is something indefinable in her gaze and the long lines of her face: trepidation perhaps, coated patchily with coyness.");
				...
				{
					// 61-90 Subsequent: 
					Render.text("Whitney puts down her work and smiles at you vaguely as you approach her. You again sense that queasy mixture of anxiety, unhappiness and excitement in her jerky, cramped movements and expression.");
			...
			if (whitneyCorruption() >= 91 && whitneyCorruption() <= 119)
			{
				...
				{
					// Subsequent 91-119: 
					Render.text("Whitney lays down her work eagerly as you approach. ");
			...
			if (whitneyCorruption() == 120)
			{
				if (flags[FlagEnum.WHITNEY_CORRUPTION_COMPLETE] == 0)
				{
					...
					// 120 Farm Corruption 
					Render.text("You don’t need to seek out Whitney this time; almost the moment you arrive on the farm a sandy furred figure is hurrying over to you. There’s deep excitement in her face and her breast is heaving with more than just exertion.");
		...
		private function dogeNotCorruptLeaveFirstTime():void
		{
			...
			Render.text("As you turn to leave, you feel something tug on your [armor]. You turn back to look into Whitney’s searching eyes.");
		...
		private function dogeNotCorruptYetMenu():void
		{
			...
			if (whitneyCorruption() <= 60 && flags[FlagEnum.WHITNEY_LEAVE_0_60] == 0) addButton(9, "Back", dogeNotCorruptLeaveFirstTime);
			else if (whitneyCorruption() <= 90 && flags[FlagEnum.WHITNEY_LEAVE_61_90] == 0) addButton(9, "Back", dogeNotCorruptLeave6190);
		}
		...
		private function whitneyAppearanceCorrupt():void
		{
			...
			Render.text("Whitney is a 5’8” dog");
			if (whitneyDefurred()) Render.text("-girl. Her human transformation has rendered her pretty in a delicate, diffident kind of way.");
			else Render.text("-morph. Her muzzle is suggestive of a golden retriever but really she could be any breed.");
			if (whitneyMaxedOralTraining()) Render.text(" In sharp contrast to her otherwise fairly thin features, her lips are plump and a depthless black, shining like wet tar. She constantly moves her tongue over them unconsciously.");
			Render.text(" Although she continues to dress in the same modest cotton blouses and long skirts she always has, complete with holes cut in to allow her short, perky tail to poke through, the cut of it and the way she moves and swings herself about, livid with arousal and near-constant sexual frustration, makes the demure outfit more perverse than anything a demon could dream up.\n");

			if (whitneyHasTattoo())
			{
				if (numTattoos("whitney") > 1) Render.text("\nShe has the following tattoos emblazoned across her body:\n");
				else Render.text("\nShe has ")
				if (flags[FlagEnum.WHITNEY_TATTOO_COLLARBONE] != 0) Render.text(flags[FlagEnum.WHITNEY_TATTOO_COLLARBONE] + "\n");
				if (flags[FlagEnum.WHITNEY_TATTOO_SHOULDERS] != 0) Render.text(flags[FlagEnum.WHITNEY_TATTOO_SHOULDERS] + "\n");
				if (flags[FlagEnum.WHITNEY_TATTOO_LOWERBACK] != 0) Render.text(flags[FlagEnum.WHITNEY_TATTOO_LOWERBACK] + "\n");
				if (flags[FlagEnum.WHITNEY_TATTOO_BUTT] != 0) Render.text(flags[FlagEnum.WHITNEY_TATTOO_BUTT] + "\n");
			} 

			Render.text("\n");
			if (!whitneyDefurred()) Render.text("Her fur is sandy, dusking to black at her extremities");
			else Render.text("Her skin is a sandy color, and she wears black nail varnish");
			Render.text(". Her ears are floppy, her eyes are a dark brown which matches her shoulder-length hair, flecked now with deep, red desire. Whilst she is beyond the softness of youth, it is obvious from looking at her that she has never known childbirth; though hardened from many years of farm work her frame is relatively slim, her small breasts pert against her unprepossessing work-clothes. She has one anus, between her tight asscheeks where it belongs.");
		...
		private function prosperityGoNotCorrupt():void
		{
			...
			Render.text("You ask her how the help is coming on.");
		...
		{
			menu();
			if (player.hasKeyItem("Breast Milker - Installed At Whitney's Farm") < 0 && flags[FlagEnum.QUEUE_BREASTMILKER_UPGRADE] == 0) addButton(0, "Breast Milker", investmentBreastMilker);
			if(player.hasKeyItem("Cock Milker - Installed At Whitney's Farm") < 0 && flags[FlagEnum.QUEUE_COCKMILKER_UPGRADE] == 0) addButton(1, "Cock Milker", investmentCockMilker);
			if (flags[FlagEnum.FARM_UPGRADES_REFINERY] == 0 && flags[FlagEnum.QUEUE_REFINERY_UPGRADE] == 0) addButton(2, "Refinery", investmentRefinery);
			if (flags[FlagEnum.FARM_UPGRADES_CONTRACEPTIVE] == 0 && flags[FlagEnum.QUEUE_CONTRACEPTIVE_UPGRADE] == 0) addButton(3, "Contraceptive", investmentContraceptive);
			if (flags[FlagEnum.FARM_UPGRADES_MILKTANK] == 0 && kGAMECLASS.milkWaifu.milkSlave() && flags[FlagEnum.QUEUE_MILKTANK_UPGRADE] == 0) addButton(4, "MilkTank", investmentMilktank);
		...
		private function investmentBreastMilker():void
		{
			...
			if (!whitneyCorrupt())
			{
				Render.text("You say you’d like a breast milker installed, one that you could make use of. Whitney frowns, but doesn’t seem as thrown by this idea as you expected.");
			...
			{
				Render.text("You say you’d like a breast milker installed, one that you could make use of. Whitney lets her gaze sink down to your [chest].");
			...
			//[Do it][No]
			menu();
			if (player.stats.gems >= 1000) addButton(0, "Do it", doBreastMilkerInvestment);
		...
		private function doBreastMilkerInvestment():void
		{
			...
			Render.text("You silently hand over a hefty bag of gems. Whitney stows it away.");
		...
		private function investmentCockMilker():void
		{
			...
			if (!whitneyCorrupt())
			{
				Render.text("You say you’d like a semen extractor built, one that you could make use of. Whitney frowns, but doesn’t seem as thrown by this idea as you expected.");
			...
			{
				Render.text("You say you’d like a semen extractor built, one that you could make use of. Whitney bites her lip at the implication.");
			...
			menu();
			if (player.stats.gems >= 1000) addButton(0, "Do it", doCockMilkerInvestment);
			else addButton(0, "Do it", turnDownInvestment, true)
			addButton(1, "No", turnDownInvestment);
		}
		...
		private function investmentRefinery():void
		{
			...
			if (!whitneyCorrupt())
			{
				Render.text("You say you want a machine built that could concentrate the fluids your followers produce into actual transformatives. Whitney shudders, but is in no position to argue.");
			...
			{
				Render.text("You say you want a machine built that could concentrate the fluids your followers produce into actual transformatives.");
			...
			menu();
			if (player.stats.gems >= 1500) addButton(0, "Do it", doRefineryInvestment);
			else addButton(0, "Do it", turnDownInvestment, true);
			addButton(1, "No", turnDownInvestment);
		}
		...
		private function investmentContraceptive():void
		{
			...
			if (!whitneyCorrupt())
			{
				Render.text("You ask Whitney if she knows of any natural contraceptive that grows in Mareth.");
		...
		private function doContraceptiveInvestment():void
		{
			...
			if (!whitneyCorrupt())
			{
				Render.text("You hand over the dough. Whitney stows it away.");
		...
		private function investmentMilktank():void
		{
			...
			if (!whitneyCorrupt())
			{
				...
				{
					Render.text("You tell Whitney you want her to give [bathgirlName] an intensive course of Gro-plus and Lactaid, and to build a small swimming tank to house her.");
				...
				{
					Render.text("You tell Whitney you want her to build a swimming tank at the farm, then come to your camp, take away your expensively acquired milk slave and install her in it. The dog woman slowly absorbs this.");
			...
			menu();
			if (player.stats.gems >= 400) addButton(0, "Do it", doMilktankInvestment);
			else addButton(0, "Do it", turnDownInvestment, true);
			addButton(1, "No", turnDownInvestment);
		}
		...
		private function doMilktankInvestment():void
		{
			...
			if (!whitneyCorrupt()) Render.text("You press the gems into your taskmaster’s hand, turn and leave without another word. Your thoughts turn to huge, delightfully plush brown boobs and luxurious milky baths; it will be well worth it. ");
			else Render.text("You press the gems into your taskmaster’s hand, turn and leave without another word. Your thoughts turn to huge, delightfully plush, brown boobs, of luxurious milky baths; it will be well worth the cost. ");
		...
		private function turnDownInvestment(money:boolean = false):void
		{
			...
			{
				// Any “No” option:
				Render.text("Whitney shrugs, nonplussed.");

				if (money) Render.text("\n\n“<i>Come back with the money.</i>”");
				else Render.text("\n\n“<i>Come back if you change your mind. With the money.</i>”");
			}
			else
			{
				Render.text("Whitney shrugs with a simper. ");
		...
		private function deFurDoge():void
		{
			...
			Render.text("You put away the gem and move into Whitney, holding her gaze as you gently place your finger and thumb along her jaw line. She doesn’t pull away and lets out a shuddering sigh as you touch her.");
		...
		private function dontDeFurDoge():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) Render.text(" crotch into");
			else Render.text(" hips against");
			Render.text(" her firm ass and she moans, her breath starting to come in shallow gulps, her heart beating furiously against your skin. Although you keep hold of her you don’t have to do anything to make the hatchet rise again.");
		...
		private function makeDogeSubby():void
		{
			...
			Render.text("You again retrieve your gem, and once more begin to rhythmically turn it; this time, however, you walk steadily towards Whitney, your voice getting louder in her ears and the shining light in her unfocused eyes getting brighter as you go on.");
		...
		private function makeDogeDommy():void
		{
			...
			Render.text("You again retrieve your gem, and once more begin to rhythmically turn it. This time, however, you walk steadily towards Whitney, your voice getting louder in her ears, the shining light in her unfocused eyes getting brighter as you go on.");
		...
		private function dogeCorruptedMissionComplete(output:boolean = true):void
		{
			...
			{
				Render.text("Whitney is in front of you before you’ve finished calling her name.");
			...
			{
				Render.text("Whitney is knelt in front of you before you’ve finished calling her name. Her tail wags as her soulful brown eyes look up at you, lit up with depthless lust.");
			...
			if (flags[FlagEnum.FARM_CORRUPTION_BRANDING_MENU_UNLOCKED] == 1 || flags[FlagEnum.QUEUE_BRANDING_UPGRADE] < 1) addButton(2, "Branding", brandingMenu);
		...
		private function firstOralTraining():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) Render.text(" cocksucker");
			else Render.text(" pussy eater");
			...
			if (silly()) Render.text(" “<i>Do. Or do not. There is no try.</i>”");
			else Render.text(" “<i>You will.</i>”");
			Render.text(" Determination lines the dog woman’s mouth.");

			Render.text("\n\n“<i>Yes, [master]. I will tr- I will.</i>” She takes a long breath and transfers her gaze to " + ((player.lowerBody.cockSpot.hasCock()) ? "[eachCock]" : "[vagina]") +", looking for all the world like she’s about to embark on a marathon.");

			Render.text("\n\n“<i>First,</i>” you say, settling yourself down, “<i>Put one hand in your knickers. Find that nice, wet pussy of yours.</i>” You watch her, a smile twitching the corners of your mouth as, blushing furiously, she shifts around in front of you. “<i>You know how to do that, right? All those lonely nights on the farm... good. Nice and easy. Eyes down.</i>” You say the last part adamantly, opening your [hips] wider as you do. Whitney swallows a bit as she strokes at her tiny button whilst staring at your " + ((player.lowerBody.cockSpot.hasCock()) ? "[eachCock]" : "[vagina]") + ", already " + ((player.lowerBody.cockSpot.hasCock()) ? "semi-turgid" : "moist") + " from the display put on by your fresh, nervous slave. “<i>Good,</i>” you murmur. “<i>Now... reach forward, and begin to lick.</i>”");
		}
		...
		private function cockOralTrainingStageOne():void
		{
			...
			if (player.biggestCockLength() > 12) Render.text(" She still struggles a bit with her gag reflex but it only takes her a few tries to swallow your thick prick, allowing you to pump into the delightful tightness of her throat without issue.");
		...
		private function firstCockOralTrainingStageTwo():void
		{
			...
			//Herm w/ balls
			if (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.balls > 0)
			{
				...
				if (player.cumQ() >= 1500)  Render.text(" which is no mean feat, given it feels like you’ve fucked what feels like a gallon of it down her gullet. Once you’re almost done, as ever you spurt the last few creamy ropes across her face and clothes");
				Render.text(".");
			...
			//Herm without balls
			else if (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.balls == 0)
			{
			...
			else if (!player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.balls > 0)
			{
				...
				if (player.cumQ() >= 1500) Render.text(" which is no mean feat, given it feels like you’ve fucked what feels like a gallon of it down her gullet. Once you’re almost done, as ever you spurt the last few creamy ropes across her face and clothes");
				Render.text(".");
				if (player.lowerBody.cockSpot.count() > 1) Render.text(" The second cock trapped in her grasp flexes out cum just as eagerly as the first, sweat standing out on your forehead as you are clenched by your twin high; it paints her skirt with its generous discharge.");
			}
			//Male without balls:
			else if (!player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.balls == 0)
			{
			...
			if (!player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				if (player.lowerBody.cockSpot.count() > 1) Render.text(" and [cock biggest2]");
				Render.text(". There’s an eagerness on display here which you don’t think was there before as she laps at your cock [headplural], and you think she even stifles a disappointed whine when she’s finished. Smiling softly, you get up, dress and send her on her way.");
			}
		...
		private function cockOralTrainingStageTwo():void
		{
			...
			if (!player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				if (player.lowerBody.cockSpot.count() > 1) Render.text("She wraps her hand around your [cock biggest2] when she’s ready, giving it an experimental pump before beginning to softly and slowly work it, remembering exactly how to grip and bend its girth as she prepares for the main course.");
				...
				if (player.lowerBody.cockSpot.count() > 1) Render.text("she occasionally forgets the length bulging in her hand so eager is she to push her lips further down your [cock biggest] and work at her own clit, and you need to murmur to get her to concentrate.");
				else Render.text("she still occasionally forgets the length buried in her mouth, so eager is she to work keenly at her own clit, and you need to murmur to get her to concentrate.");
				Render.text(" Nonetheless, the worshipful pleasure inundating your groin is close to masterful and you find yourself drifting away on it, closing your eyes and forgetting about instruction, lost entirely in the waves of your bitch’s soft, exact, ecstatic movements, her sighs and slurps filling your ears.");
				...
				if (player.lowerBody.cockSpot.count() == 1) Render.text("Whitney cleans your [cock biggest] with her flat tongue, humming as she laps your oozing head clean, deliciously soothing your aching cock.");
				else Render.text("Whitney cleans first your [cock biggest2] and then your [cock biggest] with her flat tongue, humming as she laps each of your oozing heads clean, deliciously soothing your aching cock.");
		...
		private function firstCockOralTrainingStageThree():void
		{
			...
			if (!player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				else
				{
					Render.text("\n\n“<i>Put your hand here.</i>” Slowly she withdraws her fingers from her panties and wraps them around the base of your [cock biggest]. You sigh at the hot, damp pressure now ringing the very bottom of your saliva-glossed member. “<i>Work it right there... that’s it...</i>” You sigh as she swiftly picks up a steady beat");
						if (player.lowerBody.cockSpot.count() > 1) Render.text(" jerking both your cocks with her smooth, exacting grasp in tandem");
						Render.text(".");
				}
				...
				if (player.lowerBody.cockSpot.count() > 1)
				{
					Render.text(" all the while your [cock biggest2]");
					if (player.lowerBody.cockSpot.count() > 2) Render.text(" and [cock biggest3]");
					Render.text(" bulge and vibrate with pleasure in her pumping hand");
					if (player.lowerBody.cockSpot.count() > 2) Render.text("s");
				}
				Render.text(". Though it would be so easy to lose yourself entirely to her expert movements you keep abreast it for now, holding a close eye on the dog woman kneeling in front of you, now entirely devoted to pleasuring you.");
				...
				if (player.lowerBody.cockSpot.count() > 1 || player.cumQ() >= 1500) Render.text("\n\n");
				if (player.lowerBody.cockSpot.count() > 1)
				{
					Render.text("She doesn’t forget the cock");
					if (player.lowerBody.cockSpot.count() > 2) Render.text("s");
					Render.text(" trapped in her grasp for a moment; she milks");
					if (player.lowerBody.cockSpot.count() > 2) Render.text(" them");
					else Render.text(" it");
					Render.text(" as hard as you’re fucking her mouth, and");
					if (player.lowerBody.cockSpot.count() > 2) Render.text(" they flex");
					else Render.text(" it flexes");
					Render.text(" out cum just as eagerly as the first, delirium seizing you as you are clenched by your");
					if (player.lowerBody.cockSpot.count() == 2) Render.text(" twin");
					else if (player.lowerBody.cockSpot.count() == 3) Render.text(" triple");
					else Render.text(" transcendent");
					Render.text(" high;")
					if (player.lowerBody.cockSpot.count() > 2) Render.text(" they paint");
					else Render.text(" it paints");
					Render.text(" her skirt with")
					if (player.lowerBody.cockSpot.count() > 2) Render.text(" their");
					else Render.text(" its");
					Render.text(" generous discharge. ");
				}
				if (player.cumQ() >= 1500)
				{
					Render.text("Once you’re almost done, as ever you pull out and spurt the last few creamy ropes across her face and clothes. Red with her own huge high, the dog woman does her level best to catch them in her mouth.");
				}

				Render.text("\n\nOnce you have tensed your last you sigh beatifically and float high and formless on your post-blowjob haze. You close your eyes as, without any bidding, " + ((player.lowerBody.cockSpot.count() == 1) ? "Whitney cleans your [cock biggest] with her flat tongue, lapping your oozing head clean, deliciously soothing your aching cock" : "Whitney cleans all of your pricks with her flat tongue, lapping each of your oozing heads clean, deliciously soothing your aching cock") + ". She moans the low, blissed-out moan of an addict as she cleans you up, actually gently pumping your spent [prickplural] to make the last of it drool out. Gods... just these sounds and sensations, the gentle licking at your [cock biggest]’s tip and the little gasping exhale as she manages to coax a final drop onto her thirsty tongue, make you want to do it all over again... if only she hadn’t just drained you utterly. You pull her up before she makes your cock ache even more, pressing her firm, petite body into your [chest] and gaze proudly and complacently at your champion cocksucker.");
			...
			Render.text("\n\n“<i>Now get back to your main task.</i>” You hum with deep approval as once again your [cock biggest] is swaddled in mouth flesh, her tongue sliding up and down your shaft in the same masterful, alternating oscillation all the while your [vagina] " + ((player.lowerBody.cockSpot.count() >= 2) ? "and [cock biggest2] thrum and vibrate" : "thrums and vibrates") + " with pleasure from her teasing hands. Though it would be so easy to lose yourself entirely to her expert movements you keep abreast it for now, keeping a close eye on the dog woman kneeling in front of you, entirely devoted now to pleasuring you.");
			...
			if (player.biggestCockLength() < 8) Render.text("the back of her mouth");
			else Render.text("her gullet");
			Render.text(" as you spear decisively inwards.");

			Render.text("\n\nYou can’t find the words as you fly into your golden, ecstatic high; you throw your head back as you bury as much of your cock into her as you can, sweat standing out on your brow as you bark wordlessly with each bucket of jizz you fountain down her throat. There’s a series of shrill, strangled squeals below you, her body shakes around you and you know without needing to see Whitney is cumming too, purely from the slavering joy of swallowing every last drop of the thick seed thrust into her by her master. ");

			Render.text("\n\nHer fingers slip and slide around your [vagina] and it quivers and contracts in shared orgasm, ");
			if (player.wetness() > 2) Render.text("soaking her arm as it gutters your juices, ");
			Render.text("heightening the ecstasy you feel to mind-blowing heights. ");
			if (player.lowerBody.cockSpot.count() > 1) Render.text(" She doesn’t forget the cock trapped in her grasp for a moment either; she milks it as hard as you’re fucking her mouth, and it flexes out cum just as eagerly as the first, delirium seizing you as you are clenched by your triple high as it paints her skirt with its generous discharge.");
			if (player.cumQ() >= 1500) Render.text(" Once you’re almost done, as ever you pull out and spurt the last few creamy ropes across her face and clothes. Red with her own huge high, the dog woman does her level best to catch them in her mouth.");

			Render.text("\n\nOnce you have tensed your last you sigh beatifically and float high and formless on your post-blowjob haze. You close your eyes as, without any bidding, ");
			if (player.lowerBody.cockSpot.count() == 1) Render.text("Whitney first cleans your [vagina] and then your [cock biggest] with her flat tongue, polishing your dripping pussy and thighs to a shine before lapping your oozing head clean, deliciously soothing your aching cock.");
			else Render.text("Whitney uses her flat tongue to clean first your [vagina], polishing your dripping pussy and thighs to a shine, and then each of your pricks, lapping the oozing heads clean, deliciously soothing your aching cocks.");
			Render.text(" She moans the low, blissed-out moan of an addict as she cleans you up, actually gently pumping your spent [prickplural] to make the last of it drool out. Gods... just these sounds and sensations, the gentle licking at your [cock biggest]’s tip and the little gasping exhale as she manages to coax a final drop onto her thirsty tongue, make you want to do it all over again... if only she hadn’t so adroitly drained you utterly. You pull her up before she makes your cock ache even more, pressing her firm, petite body into your [chest] and gaze proudly and complacently at your champion cocksucker.");
		...
		private function cockOralTrainingStageThree():void
		{
			...
			if (player.lowerBody.cockSpot.count() >= 2) Render.text(", allowing your other cock");
			if (player.lowerBody.cockSpot.count() > 2) Render.text("s");
			if (player.lowerBody.cockSpot.count() >= 2) Render.text(" to unfurl and bob behind it");
			if (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.cockSpot.count() >= 2) Render.text(" and");
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(", leisurely exposing your moist [vagina]");
			if (player.lowerBody.cockSpot.count() >= 2 && player.lowerBody.vaginaSpot.hasVagina()) Render.text(" at the same time");
			Render.text(", perfuming the air around her with your heavy musk.");
			...
			if (!player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				if (player.lowerBody.cockSpot.count() > 2) Render.text("Finally, she snakes her other hand around your [cock biggest3], rolling her wrist as she gently wrings it with the same sliding thoroughness.} You open your mouth to the shifting delectation inundating your overgrowth of cocks.");
				...
				if (player.lowerBody.balls > 0) Render.text(", your saliva-glossed [balls] beating against her chin");
				Render.text(". If there is one deficiency to her right now it is her fairly thin, pale lips. No cocksucking champion has lips like that - they need to be big, bee-stung, constantly wet with the movements of a needy tongue. Luscious whore pillows worthy of having your cock slide between them. You push these succulent, evil thoughts deep into your groin, deep into the heat building inexorably there. ");
				...
				if (player.lowerBody.cockSpot.count() > 1)
				{
					...
					Render.text(" as hard as you’re fucking her mouth, and");
					if (player.lowerBody.cockSpot.count() == 2) Render.text(" it flexes");
					else Render.text(" they flex");
					Render.text(" out cum just as eagerly as the first, delirium seizing you as you are clenched by your");
					if (player.lowerBody.cockSpot.count() == 2) Render.text(" twin");
					else if (player.lowerBody.cockSpot.count() == 3) Render.text(" triple");
					else Render.text(" transcendent");
					Render.text(" high;");
					if (player.lowerBody.cockSpot.count() == 2) Render.text(" it paints");
					else Render.text(" they paint");
					Render.text(" her skirt with ");
					if (player.lowerBody.cockSpot.count() == 2) Render.text(" its");
					else Render.text(" their");
					Render.text(" generous discharge. ");
				}
				Render.text("She suddenly gasps in shock, and you quickly look down. Her lips look swollen, and they seem to be darkening.");
				...
				Render.text(", first kissing your head deep and then lapping you close; she moans like a whore. She thrusts her ass in the air, sending her tongue pushing all around her new cock pillows with the taste of your cum, gently pumping your spent prick");
				if (player.lowerBody.cockSpot.count() > 1) Render.text("s");
				Render.text(" to make the last of it drool out.");
			...
			else if (player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				if (player.lowerBody.cockSpot.count() > 1) Render.text("As she bathes your [cock biggest] in delicious sensation she wraps her hand around your [cock biggest2], softening and then tightening her grip, sliding up and down its stem until it is every bit as rock-hard as the one she is lapping at. ");
				Render.text(" You close your eyes as the pleasure inundating both your male and female sex twist around each other, a slow but brilliant chemical reaction consuming you from within.");
				...
				if (player.biggestCockLength() < 8) Render.text("mouth");
				else Render.text("throat");
				Render.text(". With some effort you stay afloat and consider the dog woman as she whimpers in delight to you hammering into her juicy mouth");
				if (player.lowerBody.balls > 0) Render.text(", your saliva-glossed [balls] beating against her chin");
				Render.text(". If there is one deficiency to her right now it is her fairly thin, pale lips. No cocksucking champion has lips like that - they need to be big, bee-stung, constantly wet with the movements of a needy tongue. Luscious whore pillows worthy of having your cock slide between them. You push these succulent, evil thoughts deep into your groin, deep into the heat building inexorably there. ");
				...
				Render.text(" as you spear decisively inwards. You can’t find the words as you fly into your golden, ecstatic high; you throw your head back as you bury as much of your cock into her as you can, sweat standing out on your brow as you bark and howl with each bucket of jizz you fountain down her throat. ");
				...
				if (player.lowerBody.cockSpot.count() > 1)
				{
					Render.text(" She doesn’t forget the cock trapped in her grasp for a moment; she milks it as hard as you’re fucking her mouth, and it flexes out cum just as eagerly as the first, delirium seizing you as you are clenched by your ");
					if (player.lowerBody.cockSpot.count() == 2) Render.text("triple");
					else Render.text("transcendent");
					Render.text(" high; it paints her skirt with its generous discharge.");
				}
		...
		private function cockOralTrainingMaxed():void
		{
			...
			if (!player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				if (player.lowerBody.balls > 0) Render.text("You sigh blissfully as her plump lips push into your [balls] before the flat, warm flannel-like tongue spreads itself across them, covering every inch, each sensitive orb first sensuously kissed and then taken into your slave’s mouth to be suckled gently until they are thrumming and bulging with packed heat. The loving attention laved on them is made all the better by the soft, lustful moans turning into outright panting sobs of pleasure drifting up from between your legs as she obsequiously polishes them.");

				Render.text("\n\nShe rises until her head is level with your semi-erect prick and with long, careful, exquisite licks makes it fiercely erect, running her tongue along its full length and then swirling it around your head, pressing into the sensitive spots she knows will make you dense and tight with need. She grips it steadily at the base as she does so, every so often giving it a tight squeeze to contrast with a particularly soft lap at your bulging head.");
				if (player.lowerBody.cockSpot.count() > 1) Render.text(" As she bathes your [cock biggest] in delicious sensation she wraps her hand around your [cock biggest2], softening and then tightening her grip, sliding up and down its stem until it is every bit as rock-hard as the one she is lapping at.");
				if (player.lowerBody.cockSpot.count() > 2) Render.text(" Finally, she snakes her other hand around your [cock biggest3], rolling her wrist as she gently wrings it with the same sliding thoroughness.");
				Render.text(" You open your mouth to the shifting delectation inundating [eachCock].");
				...
				if (player.lowerBody.cockSpot.count() > 1) Render.text("In the long, delicious daze that follows you move her around");
				if (player.lowerBody.cockSpot.count() == 2) Render.text(", switching her attention to your [cock biggest2] for a while");
				else if (player.lowerBody.cockSpot.count() >= 3) Render.text(" switching her attention first to one cock and then the other");
				if (player.lowerBody.cockSpot.count() > 1) Render.text(" so that each of your fiercely erect pricks has time to enjoy the tight, sucking embrace of her mouth.");
				...
				if (player.lowerBody.balls > 0) Render.text(" and her tongue once again sliding across your [balls], ");
				Render.text("before wrapping your hand through her silky hair and lifting her carefully but firmly all the way up, above your gleaming cock, at a height where she is just able to reach it with her tongue, telling her to cup her peachy breasts as you do. You enjoy the soft lapping at your beading head and the frustrated moans filling your ears at leisure, reveling in the sight of her, attempting to reconcile the memory of the calm, independent dog woman you once knew with the whimpering bitch dangling from your fist and creaming herself from worshiping your cock now.");

				Render.text("\n\nYou let her go and she immediately impales herself back onto your length");
				if (player.lowerBody.cockSpot.count() > 1) Render.text(", her hands wrapping back around your other cock");
				if (player.lowerBody.cockSpot.count() > 2) Render.text("s");
				if (player.lowerBody.cockSpot.count() > 1) Render.text(" as if they never left them");
				Render.text(" pulling and kneading you now with real need. The drowning intensity of the pleasure inundating your maleness increases and you begin to pump upwards to an irresistible high, grasping her head and beginning to thrust deep into the tight suction of her "+ ((player.biggestCockLength() < 8) ? "mouth" : "throat") +".");
				...
				Render.text(" Eventually you can give her no more and withdrawing you stumble back, stars swimming in your eyes from the force of it.");
			...
			else if (player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				Render.text("\n\nShe rises until her head is level with your semi-erect prick and with long, careful, exquisite licks makes it fiercely erect, running her tongue along its full length and then swirling it around your head, pressing into the sensitive spots she knows will make you dense and tight with need. She grips it steadily at the base as she does so, every so often giving it a tight squeeze to contrast with a particularly soft lap at your bulging head.");
				if (player.lowerBody.cockSpot.count() > 1) Render.text(" As she bathes your [cock biggest] in delicious sensation she wraps her hand around your [cock biggest2], softening and then tightening her grip, sliding up and down its stem until it is every bit as rock-hard as the one she is lapping at.");
				Render.text(" You open your mouth to the shifting delectation inundating both your male and female sex, her teasing digits moving with innate expertise to send twin pleasure twisting through you, a slow but brilliant chemical reaction consuming you from within.");
				...
				if (player.lowerBody.cockSpot.count() > 1) Render.text(" In the long, delicious daze that follows you move her around");
				if (player.lowerBody.cockSpot.count() == 2) Render.text(", switching her attention to your [cock biggest2] for a while");
				if (player.lowerBody.cockSpot.count() >= 3) Render.text(", switching her first to one cock then another");
				if (player.lowerBody.cockSpot.count() > 1) Render.text(" so that each of your fiercely erect pricks has time to enjoy the tight, sucking embrace of her mouth.");

				Render.text("\n\nPushing her all the way down to the base of your [cock biggest] so that the heavy breath through her nose warms your crotch and holding her there for a time, you enjoy the glorious closeness of the back of her "+ ((player.biggestCockLength() < 8) ? "mouth" : "throat"));
				if (player.lowerBody.balls > 0) Render.text(", and her tongue once again sliding across your [balls]");
				Render.text(" before wrapping your hand through her silky hair and lifting her carefully but firmly all the way up, above your [cock biggest] at a height where she is just able to reach it with her tongue, telling her to cup her peachy breasts as you do. You enjoy the soft lapping at your beading head and the frustrated moans filling your ears at leisure, reveling in the sight of her, attempting to reconcile the memory of the calm, independent dog woman you once knew with the whimpering bitch dangling from your fist and creaming herself from worshiping your cock now.");
				...
				if (player.wetness() > 2) Render.text("soaking her arm as it gutters your juices, ");
				Render.text("heightening the ecstasy you feel to mind-blowing heights.");
				if (player.lowerBody.cockSpot.count() > 1) Render.text(" She doesn’t forget the [cock biggest2] trapped in her grasp for a moment; she milks it as hard as you’re fucking her mouth, and it flexes out cum just as eagerly as the first, delirium seizing you as you are clenched by your triple high; it paints her skirt with its generous discharge.");
				Render.text(" Eventually you can give her no more and withdraw, stars swimming in your eyes from the force of your orgasm.");
		...
		private function vaginaOralTrainingStageOne():void
		{
			...
			else Render.text(" The dog woman coughs and splutters around the lubricant you dribble into her, some of it dripping out of her nose even as you continue to push yourself into her. This doesn’t stop her from furiously moving her hand in her knickers, flicking her clit for all she is worth as she attempts to cope with the pressing demand of your snatch. On the edge already it doesn’t take her long, and her eyes roll as she tenses and shudders.");
		...
		private function firstVaginaOralTrainingStageTwo():void
		{
			...
			if (player.lactationQ() > 0) Render.text(" dripping slightly");
			else Render.text(" semi-erect"); 
			Render.text(" from the arousal you already feel. “<i>A good slut knows her mistress’s breasts need to be attended to, her nipples and skin shining with a slave’s worship for all the world to see, before she’s deserving of drinking from her pussy. Let’s see how you do.</i>” ");
			...
			if (player.upperBody.chest.hasFuckableNipples()) Render.text(" obscene folds");
			else Render.text(" sensitive points");
			Render.text(", kneading them gently and bathing them in saliva. Given her initial awkwardness at cunnilingus it’s remarkable how good she is at this. “<i>Lick every inch... that’s good. Suckle on them a bit... very good!</i>” You gasp a bit as she envelopes one of your nipples and");
			if (player.upperBody.chest.hasFuckableNipples()) Render.text(" sends her tongue questing into its sensitive inside.");
			else Render.text("bites it ever so gently.");
			if (player.lactationQ() > 0)
			{
				Render.text(" The gentle kneading is easily enough to make your fecund teats dribble milk into her mouth, and after being slightly startled by it Whitney evidently enjoys the taste of it, because she begins to pull at your plump breast for more. You allow her another mouthful before gently pulling her away.");
			...
			if (player.wetness() <= 2) Render.text(" wetting both your slave’s hand and mouth");
			else Render.text(" absolutely soaking your slave’s face and arm");
			Render.text(" with femjizz as it does. She shudders around you and her eyes roll as she finds her own high rooting in her pussy; you are extremely gratified to note that, despite being on a plane of submissive pleasure far above you, she still manages to swallow every drop of cum that showers into her mouth");
			if (player.wetness() >= 5) Render.text(" which is no mean feat, given it feels like you gutter what feels like a gallon down her gullet. Once you’re almost done, as ever you get up and spurt the last of it on her hair and clothes");
			Render.text(".");
		...
		private function vaginaOralTrainingStageTwo():void
		{
			...
			Render.text("Not saying a word, you undo your [armor], exposing your whole body with a soft smile. Without any need for instruction Whitney gets to work, her hand delving into her underwear with a contented sigh, her eyes focused on the task in front of her as she steadily stokes the heat in her crotch with tight, urgent jerks of her slim body.");
			...
			if (player.lactationQ() > 0) Render.text(" and it quickly makes the liquid warmth in your breasts rise to a point, beading milk freely from your teats. Whitney laps and sucks it down eagerly, but gratifyingly she remembers your order and stops reluctantly after only taking a single pull");
			Render.text(", leaving your nipples");
			if (!player.upperBody.chest.hasFuckableNipples()) Render.text(" rock-hard and");
			Render.text(" shining and your [vagina] beading eagerly.");
			...
			if (player.wetness() >= 5) Render.text(", fountaining fem juice down her throat");
			...
			if (player.wetness() >= 5) Render.text(", soaking her again and again with warm, female approval");
			Render.text(".");
		...
		private function firstVaginaOralTrainingStageThree():void
		{
			...
			if (!player.upperBody.chest.hasFuckableNipples()) Render.text(" she takes each of your [nipples] into her mouth to bathe them in close, sucking attention");
			else Render.text(" she sends her tongue lapping into each of your cunt nipples, curling at their sensitive pink until they are wet with need");
			Render.text(". It’s a process which is beautifully sensuous, made all the more gratifying by the soft, lustful moans she makes as she polishes your softness.");
			if (player.lactationQ() > 0) Render.text(" It quickly makes the liquid warmth in your breasts rise to a point, beading milk freely from your teats. Whitney laps and sucks it down eagerly, but gratifyingly she remembers your order and stops reluctantly after only taking a single pull.");
			Render.text(" She sinks downwards leaving your nipples");
			if (!player.upperBody.chest.hasFuckableNipples()) Render.text(" rock-hard and");
			Render.text(" shining, your [vagina] beading eagerly.");
			...
			if (player.lowerBody.vaginaSpot.list[0].clitLength >= 4) Render.text(" She uses her new anchorage to take your bulging fem-dick entirely into her mouth, sucking on it avidly; she hums as she does it, sending ecstatic pleasure thrumming through you.");
			...
			if (player.wetness() >= 5) Render.text(", soaking her mouth and face again and again with warm, female approval");
			Render.text(". There’s a series of shrill, strangled squeals below you, her body shakes around you and you know without needing to see Whitney is cumming too, purely from the slavering joy of swallowing every last drop of the musky fruit guttered into her by her mistress.");
			if (player.wetness() >= 5) Render.text(" It is in a state of ecstatic momentum that you rise away from her mouth and spurt the last of it onto her hair and clothes. Red with her own huge high, the dog woman does her level best to catch your spotting fluids in her mouth.");
		...
		private function vaginaOralTrainingStageThree():void
		{
			...
			Render.text(". It’s a process which is beautifully sensuous.");
			if (player.lactationQ() > 0) Render.text(" It quickly makes the liquid warmth in your breasts rise to a point, beading milk freely from your teats. Whitney laps and sucks it down eagerly, whimpering as she withdraws after taking a single pull. You smile softly, knowing that the small, teasing taste of your sweetness only serves to make her thirstier.");
			Render.text(" She sinks downwards leaving your nipples");
			if (player.upperBody.chest.hasFuckableNipples()) Render.text(" rock-hard and");
			Render.text(" shining, your [vagina] beading eagerly.");
		...
		private function vaginaOralTrainingMaxed():void
		{
			...
			if (!player.upperBody.chest.hasFuckableNipples()) Render.text(" she envelopes each of your [nipples] with her lips, kissing them with their sumptuous sponginess before bathing them in close, sucking attention");
			else Render.text(" she pushes her lips against your cunt nipples before sending her tongue gently into each of them, curling at their sensitive pink until they are oozing wetness in delight");
			Render.text(". It’s a process which is beautifully sensuous, made all the better by the soft, lustful moans she makes as she polishes your bust.");
			...
			Render.text(" Having left your [chest] gleaming with warm, oily saliva and your nipples");
			if (!player.upperBody.chest.hasFuckableNipples()) Render.text(" rock-hard and");
			Render.text(" shining, she sinks down to the next stage.");
			...
			if (player.lowerBody.butt.buttRating < 4) Render.text("firm butt");
			if (player.lowerBody.butt.buttRating < 8) Render.text("supple rump");
			else Render.text("soft ass");
			Render.text(", oiling it to a sheen. She makes little gasping, stifled breaths as she goes about it, and a grin comes to your lips.");
		...
		{
			Render.text("You are slightly apprehensive about this, but you’ve gone to the trouble of crafting Whitney into a fairly unique slave, and it seems churlish not to test just how deep the dominating streak you’ve brought out in her goes. You tell her you could do with a bit of relaxation.");
		...
		{
			Render.text("You say you could do with some relaxation.");
		...
		private function whitneyDomBondageOral():void
		{
			...
			Render.text("Whitney climbs on top of your supine form, her hands pressing into your [chest], firm hips clenching around your abdomen as she considers you.");
			...
			if (player.lowerBody.cockSpot.hasCock() && !player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				if (player.totalCocks() > 1) Render.text(" “<i>So many things to play with tucked away in here!</i>” comes her merry voice again, as you feel her grasp your [cock smallest] at the same time; you groan into the confines of her flesh as she gives it a soft, teasing pump. It’s obvious your main member has really caught her attention though, and both her hands soon return to it.");
			...
			else if (player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock())
			{
				// Female: 
			...
			else Render.text("femcum");
			Render.text(", and only after relaxing for a long, heavy moment on your face does she finally slide off you. You heave and pant for breath, still cuffed to the bed, rivulets of femcum trickling down your face, your tongue and ");
			if (player.lowerBody.cockSpot.hasCock()) Render.text("[cock biggest]"); 
			if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and ");
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("[vagina]");
			Render.text(" aching, oozing. In a deep, post-coital haze you feel someone take a towel to your body before curling into you, throwing a dense thigh over you, holding and running her hands over you possessively as you recover. ");
		...
		private function whitneyDomStraponDoggy():void
		{
			...
			if (player.isBiped() || player.isTaur()) Render.text(" Next, she pushes your ankles apart, threads a leather strap around each and then pulls them tight with a sharp yank; you carefully test them and find with a cold thrill that there is a metal bar attached between them, forcing you to stay in this position.");
			else if (player.isDrider()) Render.text(" You don’t struggle as she then forces your spindly legs beneath the bed, looping a leather strap attached to the frame around them all, trapping them there.");
			else if (player.isNaga()) Render.text(" She then ties your reptilian coils around the bedposts, securing them with leather straps fitted for the purpose.");
			...
			if (player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock() && kGAMECLASS.vapula.vapulaSlave())
			{
			...
			if (capacity >= 36)
			{
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("sit and spread ‘em for anything going, don’t you?");
				else Render.text("I bet you get on your knees for anything with a nice, juicy cock, right?");
				Render.text(" What would your other slaves think of that? My secret little slut.</i>” ");
			...
			else
			{
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("take of your tight cunt");
				else Render.text("give of your tight ass");
				Render.text(", pushing the other end of the dildo into her. An even louder moan is forced out of your throat by the intensity of your tunnel being packed full of cock, albeit an imitation.");
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("servants, you tight, bossy little bitch");
				else Render.text("cattle, you tight, bossy little boy-bitch");
				Render.text(".</i>” ");
			...
			Render.text("\n\nA " + ((whitneyDefurred()) ? "hand" : "paw") + " reaches around and clutches your [chest] for anchorage as she finally begins to pound into your tenderized hole with all she’s got. She catches a [nipple] between her fingers and you find yourself being forced towards orgasm again, your ears full of her harsh breathing, your nose full of her heavy scent. She’s no longer thrusting into you for your pleasure though, if she ever was; the hard, feral thrusting of the strap-on into your " + ((player.lowerBody.vaginaSpot.hasVagina()) ? "oozing cunt" : "ass") + " is all about her, all about working the smaller, teasing end in her own pussy as hard as she can. You are simply a vehicle for her own lusts, and when you come again with a juddering cry into her panties, " + ((player.lowerBody.vaginaSpot.hasVagina()) ? "wetting the bed this time with the jackhammer force of it" : "") + " she keeps going, hammering into your hapless " + ((player.lowerBody.vaginaSpot.hasVagina()) ? "[vagina]" : "butt") + ".");
			...
			if (player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock() && kGAMECLASS.vapula.vapulaSlave()) Render.text(" You feel the end of the dildo tense deep within you before releasing, filling you full of warm demon spunk until it pools out onto the bed. The feeling is horribly, disgustingly satisfying, and for a moment you can see exactly how Vapula views the world.");
		...
		public function whitneyDomRide():void
		{
			...
			if (player.isBiped()) Render.text(" She does the same with your ankles, leaving you helplessly spread-eagled across the bed.");
			else if (player.isDrider()) Render.text(" She has long, leather straps attached to the underside of the bed; these are wrapped around the joining of your spindly legs to your heavy abdomen, leaving your chitinous feet pointing helplessly at the ceiling.");
			else if (player.isGoo()) Render.text(" “<i>I can’t secure you properly,</i>” she breathes, squeezing your gooey form, sinking her fingers into it. “<i>Owing to this nonsense you call a body. So I will jus’ say it again. Lie. Still.</i>”");
			else if (player.isNaga()) Render.text(" Your long, heavy tail is wrapped around the bedsteads at the foot of the bed and secured with more leather straps, leaving you helplessly spread-eagled across the bed.");
		...
		private function brandingMenu():void
		{
			...
			{
				Render.text("You idly put an arm around Whitney, drawing her into you. You want her in the right frame of mind before you lead her down this path of inquiry. Her breath is hot and heavy against your [chest], her desire-lit eyes unable to tear away from yours as your hand slides down the curve of her back and round around her enjoyably tight ass. You ask her what she knows about branding.");
				...
				menu();
				if (player.stats.gems >= 500) addButton(0, "Do it", getBrandingStuff);
			...
			else
			{
				...
				Render.text("Your thoughts turn to the ingenious magic of your tattooing gear, stashed away in the barn. Grinning, you consider who you will summon to brand.");

				if (player.stats.gems < 50) Render.text("\n\n<b>You don't have enough gems to afford a new brand for any of your slaves.</b>");
				...
				if (player.stats.gems >= 50)
				{
					...
					if (flags[FlagEnum.FOLLOWER_AT_FARM_AMILY] == 1 && hasFreeTattooSlot("amily")) addButton(1, "Amily", brandAmily);
					if (flags[FlagEnum.FOLLOWER_AT_FARM_JOJO] == 1 && hasFreeTattooSlot("jojo")) addButton(2, "Jojo", brandJojo);
					if (flags[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] == 1 && kGAMECLASS.sophieBimbo.bimboSophie() && hasFreeTattooSlot("sophie")) addButton(3, "Sophie", brandBimboSophie);
					if (flags[FlagEnum.FOLLOWER_AT_FARM_VAPULA] == 1 && hasFreeTattooSlot("vapula")) addButton(4, "Vapula", brandVapula);
					if (flags[FlagEnum.KELT_BREAK_LEVEL] >= 4 && hasFreeTattooSlot("kelly")) addButton(5, "Kelly", brandKelly);
					if (flags[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] == 1 && flags[FlagEnum.MILK_SIZE] > 0 && hasFreeTattooSlot("milky")) addButton(6, flags[FlagEnum.MILK_NAME], brandSmallMilky);
					if (flags[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] == 1 && flags[FlagEnum.MILK_SIZE] == 0 && hasFreeTattooSlot("milky")) addButton(6, flags[FlagEnum.MILK_NAME], brandBigMilky);
				}
		...
		public function numTattoos(name:string): number
		{
			let count: number = 0;
		...
		private function brandWhitney():void
		{
			...
			Render.text("Whitney knows exactly what it means when you head off to the barn where she stashed your “branding” equipment; the dog girl is already eagerly stripped down by the time you’ve returned. You hold the pots of ink and consider where, and what, to put on her.");
		...
		public function brandAmily():void
		{
			...
			Render.text("You retrieve the pots of ink and paper from the barn and, smiling, tell your pet mouse you’re going to give her a treat.");

			Render.text("\n\n“<i>How exciting!</i>” she squeaks. “<i>Is it your");
			if (player.lowerBody.cockSpot.hasCock()) Render.text(" dick");
			else if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" pussy");
			else Render.text(" body");
			Render.text(", [master]? That’s my favorite type of treat.</i>” You consider where, and what, to put on her.");
		...
		private function brandJojo():void
		{
			...
			Render.text("You retrieve the pots of ink and paper from the barn and, smiling brightly, tell your pet mouse you’re going to give him a special treat. Jojo knows all too well the nature of your treats. He closes his eyes and waits for the worst as you consider where and what to put on him.");
		...
		private function brandBimboSophie():void
		{
			...
			Render.text("You retrieve the pots of ink and paper from the barn and, smiling, tell your pet harpy you’re going to give her a treat.");
		...
		{
			Render.text("You retrieve the pots of ink and paper from the barn and, smiling, tell your succubus you’re going to give her a treat.");
		...
		{
			Render.text("You retrieve the pots of ink and paper from the barn and smilingly tell your centaur cumslut you’re going to give her a treat.");
		...
		{
			Render.text("You retrieve the pots of ink and paper from the barn and, smiling kindly, tell your ex Sand Witch slave you’re going to give her a treat. ");
		...
		private function dontGetBrandingStuff():void
		{
			...
			Render.text("You stroke at her tiny bulging button until you adjudge she’s on the edge, before withdrawing your hand completely. Maybe some other time, you say airily.");

			Render.text("\n\n“<i>Oh [master],</i>” groans Whitney, in a tone of deepest exasperation, her hands between her thighs.");
			if (whitneyDom()) Render.text(" “<i>You’ll pay for that later, I swear!</i>”");
			Render.text(" Grinning, you turn and leave.");
		...
		private function brandingAvailableTalk():void
		{
			...
			Render.text("“<i>[Master]! [Master], I’ve done as you asked,</i>” says Whitney, her tail wagging frantically as you approach. Her eyes are lit with glee and she’s got a cow with her on tether. You settle down and watch as she produces unassuming pots of black and red ink, and a sheaf of what looks like blotting paper.");
			...
			if (whitneyDefurred()) Render.text(" fingernail");
			else Render.text(" claw");
			Render.text(" into the pot, withdraws it and then, dripping black fluid, presses it against the cow’s ample backside. The animal moos and swishes its tail in mild protest, but it doesn’t seem in any grievous amount of pain as Whitney’s digit moves, artfully tracing a large number 12 with the ink.");
		...
		private function testBranding():void
		{
			...
			Render.text("You return her devilish grin with interest.");
		...
		{
			menu();
			if (flags[FlagEnum.WHITNEY_TATTOO_COLLARBONE] == 0) addButton(0, "Collarbone", brandSelect, 0)
			if (flags[FlagEnum.WHITNEY_TATTOO_SHOULDERS] == 0) addButton(1, "Shoulders", brandSelect, 1);
			if (flags[FlagEnum.WHITNEY_TATTOO_LOWERBACK] == 0) addButton(2, "Lower Back", brandSelect, 2);
			if (flags[FlagEnum.WHITNEY_TATTOO_BUTT] == 0) addButton(3, "Butt", brandSelect, 3);
		}

		public function amilyBrandSlotSelect():void
		{
			menu();
			if (flags[FlagEnum.AMILY_TATTOO_COLLARBONE] == 0) addButton(0, "Collarbone", amilyBrandSelect, 0)
			if (flags[FlagEnum.AMILY_TATTOO_SHOULDERS] == 0) addButton(1, "Shoulders", amilyBrandSelect, 1);
			if (flags[FlagEnum.AMILY_TATTOO_LOWERBACK] == 0) addButton(2, "Lower Back", amilyBrandSelect, 2);
			if (flags[FlagEnum.AMILY_TATTOO_BUTT] == 0) addButton(3, "Butt", amilyBrandSelect, 3);
		}

		public function jojoBrandSlotSelect():void
		{
			menu();
			if (flags[FlagEnum.JOJO_TATTOO_COLLARBONE] == 0) addButton(0, "Collarbone", jojoBrandSelect, 0)
			if (flags[FlagEnum.JOJO_TATTOO_SHOULDERS] == 0) addButton(1, "Shoulders", jojoBrandSelect, 1);
			if (flags[FlagEnum.JOJO_TATTOO_LOWERBACK] == 0) addButton(2, "Lower Back", jojoBrandSelect, 2);
			if (flags[FlagEnum.JOJO_TATTOO_BUTT] == 0) addButton(3, "Butt", jojoBrandSelect, 3);
		}

		public function bimboSophieSlotSelect():void
		{
			menu();
			if (flags[FlagEnum.SOPHIE_TATTOO_COLLARBONE] == 0) addButton(0, "Collarbone", bimboSophieBrandSelect, 0)
			if (flags[FlagEnum.SOPHIE_TATTOO_SHOULDERS] == 0) addButton(1, "Shoulders", bimboSophieBrandSelect, 1);
			if (flags[FlagEnum.SOPHIE_TATTOO_LOWERBACK] == 0) addButton(2, "Lower Back", bimboSophieBrandSelect, 2);
			if (flags[FlagEnum.SOPHIE_TATTOO_BUTT] == 0) addButton(3, "Butt", bimboSophieBrandSelect, 3);
		}

		public function vapulaSlotSelect():void
		{
			menu();
			if (flags[FlagEnum.VAPULA_TATTOO_COLLARBONE] == 0) addButton(0, "Collarbone", vapulaBrandSelect, 0)
			if (flags[FlagEnum.VAPULA_TATTOO_SHOULDERS] == 0) addButton(1, "Shoulders", vapulaBrandSelect, 1);
			if (flags[FlagEnum.VAPULA_TATTOO_LOWERBACK] == 0) addButton(2, "Lower Back", vapulaBrandSelect, 2);
			if (flags[FlagEnum.VAPULA_TATTOO_BUTT] == 0) addButton(3, "Butt", vapulaBrandSelect, 3);
		}

		public function kellySlotSelect():void
		{
			menu();
			if (flags[FlagEnum.KELLY_TATTOO_COLLARBONE] == 0) addButton(0, "Collarbone", kellyBrandSelect, 0)
			if (flags[FlagEnum.KELLY_TATTOO_SHOULDERS] == 0) addButton(1, "Shoulders", kellyBrandSelect, 1);
			if (flags[FlagEnum.KELLY_TATTOO_LOWERBACK] == 0) addButton(2, "Lower Back", kellyBrandSelect, 2);
			if (flags[FlagEnum.KELLY_TATTOO_BUTT] == 0) addButton(3, "Butt", kellyBrandSelect, 3);
		}

		public function smallMilkySlotSelect():void
		{
			menu();
			if (flags[FlagEnum.MILKY_TATTOO_COLLARBONE] == 0) addButton(0, "Collarbone", smallMilkyBrandSelect, 0)
			if (flags[FlagEnum.MILKY_TATTOO_SHOULDERS] == 0) addButton(1, "Shoulders", smallMilkyBrandSelect, 1);
			if (flags[FlagEnum.MILKY_TATTOO_LOWERBACK] == 0) addButton(2, "Lower Back", smallMilkyBrandSelect, 2);
			if (flags[FlagEnum.MILKY_TATTOO_BUTT] == 0) addButton(3, "Butt", smallMilkyBrandSelect, 3);
		}

		public function bigMilkySlotSelect():void
		{
			menu();
			if (flags[FlagEnum.MILKY_TATTOO_COLLARBONE] == 0) addButton(0, "Collarbone", bigMilkyBrandSelect, 0)
			if (flags[FlagEnum.MILKY_TATTOO_SHOULDERS] == 0) addButton(1, "Shoulders", bigMilkyBrandSelect, 1);
			if (flags[FlagEnum.MILKY_TATTOO_LOWERBACK] == 0) addButton(2, "Lower Back", bigMilkyBrandSelect, 2);
			if (flags[FlagEnum.MILKY_TATTOO_BUTT] == 0) addButton(3, "Butt", bigMilkyBrandSelect, 3);
		}

		public function brandSelect(slot: number):void
		{
			clearOutput();
			whitneySprite();

			Render.text("What will you draw on her " + slotNames[slot] + "?");
			...
			if (player.lowerBody.cockSpot.hasCock() && whitneyMaxedOralTraining()) addButton(4, "Cocksucker", champCocksuckerTattoo, slot);
			if (player.lowerBody.vaginaSpot.hasVagina() && whitneyMaxedOralTraining()) addButton(5, "Pussylicker", champPussylickerTattoo, slot);

			addButton(9, "Back", brandSlotSelect);
		}

		public function amilyBrandSelect(slot: number):void
		{
			clearOutput();
			...
			if (player.lowerBody.cockSpot.hasCock() && slot == 2) addButton(4, "Cock Here", amilyCockGoesHereTattoo, slot);
			if (player.lowerBody.vaginaSpot.hasVagina()) addButton(5, "Mommy's Girl", amilyMommysGirlTattoo, slot);

			addButton(9, "Back", amilyBrandSlotSelect);
		}

		public function jojoBrandSelect(slot: number):void
		{
			clearOutput();

			Render.text("What will you draw on his " + slotNames[slot] + "?");
			...
			if (player.lowerBody.cockSpot.hasCock() && slot == 2) addButton(4, "Cock Here", jojoCockGoesHereTattoo, slot);
			if (player.lowerBody.vaginaSpot.hasVagina()) addButton(5, "Mommy's Boy", jojoMommysBoyTattoo, slot);

			addButton(9, "Back", jojoBrandSlotSelect);
		}

		public function bimboSophieBrandSelect(slot: number):void
		{
			clearOutput();
			...
			if (player.lowerBody.cockSpot.hasCock() && slot == 2) addButton(6, "Cock Goes Here", bimboSophieCockGoesHereTattoo, slot);

			addButton(9, "Back", bimboSophieSlotSelect);
		}

		public function vapulaBrandSelect(slot: number):void
		{
			clearOutput();
			...
			if (!player.lowerBody.cockSpot.hasCock()) addButton(5, "Dildo Polisher", vapulaDildoPolisherTattoo, slot);

			addButton(9, "Back", vapulaSlotSelect);
		}

		public function kellyBrandSelect(slot: number):void
		{
			clearOutput();
		...
		public function smallMilkyBrandSelect(slot: number):void
		{
			clearOutput();
			...
			if (slot == 0 && player.lowerBody.cockSpot.hasCock()) addButton(6, "Cock Cozy", smallMilkyCockCozyTattoo, slot);

			addButton(9, "Back", smallMilkySlotSelect);
		}

		public function bigMilkyBrandSelect(slot: number):void
		{
			clearOutput();
			...
			if (slot == 0 && player.lowerBody.cockSpot.hasCock()) addButton(5, "Cock Cozy", bigMilkyCockCozyTattoo, slot);
		...
		{
			Render.text("You command her to kneel in front of you and be still. Your lithe, naked dog girl does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her, and then seal it on with the paper.");
		}

		private function amilyCollarboneIntro():void
		{
			Render.text("You command her to kneel in front of you and be still. The succubus mouse does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her, and then seal it on with the paper. ");
		}

		private function jojoCollarboneIntro():void
		{
			Render.text("You command him to kneel in front of you and be still. The mouse-demon does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on him, and then seal it on with the paper. ");
		...
		{
			Render.text("You command her to kneel in front of you and be still. It’s difficult to get her to stay still, but she goes into a trance-like state when you finally lay your ink-soaked finger on her, mesmerized by its movement until you lay the paper over the design you’ve drawn and seal the ink.");
		}

		private function vapulaCollarboneIntro():void
		{
			Render.text("You command her to kneel in front of you and be still. The succubus does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her, and then seal it on with the paper. ");
		}

		private function kellyCollarboneIntro():void
		{
			Render.text("You command her to kneel in front of you and be still. The centaur does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her, and then seal it on with the paper. ");
		}

		private function smallMilkyCollarboneIntro():void
		{
			Render.text("You command her to take her clothes off, kneel in front of you and be still. The dusky girl does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her and then seal it on with the paper.");
		}
		...
		{
			Render.text("You command her to kneel facing away from you and be still. Your lithe, naked dog girl does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her, and then seal it on with the paper. “<i>What did you draw, [master]?</i>” she says eagerly.");
		...
		{
			Render.text("You command him to kneel facing away from you and be still. The mouse-demon does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on him, and then seal it on with the paper. ");
		...
		{
			Render.text("You command her to kneel facing away from you and be still. Your succubus mouse does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her and then seal it on with the paper. ");
		...
		{
			Render.text("You command her to kneel facing away from you and be still. It’s difficult to get her to stay still, but she goes into a trance-like state when you finally lay your ink-soaked finger between her blonde wings, mesmerized by its movement until you lay the paper over the design you’ve drawn and seal the ink.");
		...
		{
			Render.text("You command her to kneel facing away from you and be still. Your succubus does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her, and then seal it on with the paper. ");
		...
		{
			Render.text("You command her to kneel facing away from you and be still. The centaur does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her shoulder blades, and then seal it on with the paper. ");
		...
		{
			Render.text("You command her to kneel facing away from you and be still. The dusky girl does so, keeping her back straight and still as you dip your finger into the ink, carefully draw your design on her, and then seal it on with the paper. ");
		}

		private function bigMilkyShouldersIntro():void
		{
			Render.text("You set yourself down behind [bathgirlName] before dipping your finger into the ink, carefully draw your design on her, and then seal it on with the paper. She barely seems to notice.");
		...
		{
			Render.text("You command her to set herself down on your [legs], as if she were about to receive a spanking. Your lithe, naked dog girl does so, her laughter at her own compromising permission turning to a sharp coo as you dip your finger into the ink and carefully draw your design on her, before sealing it on with the paper. “<i>What did you draw, [master]?</i>” she says eagerly.");
		...
		{
			Render.text("You command him to set himself face down across your [legs], as if he were about to receive a spanking. The mouse-demon does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on him and then seal it on with the paper. ");
		...
		{
			Render.text("You command her to set herself face down across your [legs], as if she were about to receive a spanking. Your succubus mouse does so, her giggles at her own compromising position turning to a sharp coo as you dip your finger into the ink and carefully draw your design on her, before sealing it on with the paper. ");
		...
		{
			Render.text("You command her to set herself face down across your [legs], as if she were about to receive a spanking. Your succubus does so, her put-upon sigh turning to a sharp coo as you dip your finger into the ink and carefully draw your design on her, before sealing it on with the paper. ");
		...
		{
			Render.text("You command her to stand perfectly still.  She can’t quite stop clopping her hooves fretfully as you dip your finger into the ink and carefully draw your design on her human lower back before sealing it on with the paper. ");
		...
		{
			Render.text("You command her to set herself down across your [legs]. The dusky girl does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her, and then seal it on with the paper.");
		}
		...
		{
			Render.text("You command her to set herself down on your [legs], as if she were about to receive a spanking. Your lithe, naked dog girl does so, her laughter at her own compromising permission turning to a sharp coo as you dip your finger into the ink and carefully draw your design on the softest part of her anatomy, before sealing it on with the paper.");
		...
		{
			Render.text("You command her to set herself face down across your [legs], as if she were about to receive a spanking. Your succubus mouse does so, her laughter at her own compromising position turning to a sharp coo as you dip your finger into the ink and carefully draw your design on the softest part of her anatomy, before sealing it on with the paper. ");
		...
		{
			Render.text("You command him to set himself face down across your [legs], as if he were about to receive a spanking. The mouse-demon does so, staying perfectly still as you dip your finger into the ink, carefully draw your design onto the softest part of his body and then seal it on with the paper. ");
		...
		{
			Render.text("You command her to set herself down across your [legs], as if she were about to receive a spanking. Your succubus does so, her put-upon sigh turning to a sharp coo as you dip your finger into the ink and carefully draw your design on the softest part of her anatomy, before sealing it on with the paper. ");
		...
		{
			Render.text("You command her to stand perfectly still.  She can’t quite stop clopping her hooves fretfully as you dip your finger into the ink and carefully draw your design on her smooth, brawny horse ass before sealing it on with the paper. ");
		...
		{
			Render.text("You command her to set herself down across your [legs]. The dusky girl does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her fine, round ass, and then seal it on with the paper.");
		}

		private function bigMilkyButtIntro():void
		{
			Render.text("You set yourself down behind [bathgirlName] before dipping your finger into the ink, carefully draw your design on her fine, round ass, and then seal it on with the paper. She barely seems to notice.");
		...
		private function tribalTattoo(slot: number):void
		{
			clearOutput();
			whitneySprite();
		...
		private function amilyTribalTattoo(slot: number):void
		{
			clearOutput();
		...
		private function jojoTribalTattoo(slot: number):void
		{
			clearOutput();
		...
		private function bimboSophieTribalTattoo(slot: number):void
		{
			clearOutput();
		...
		private function vapulaTribalTattoo(slot: number):void
		{
			clearOutput();
		...
		private function kellyTribalTattoo(slot: number):void
		{
			clearOutput();
			...
			if (slot == 0)
			{
				...
				Render.text("“<i>That’s- that’s actually really pretty, [master]!</i>” Kelly says when you’re done, looking down with delight. “<i>Thank you!</i>”");
				tText += "collarbone.";
		...
		private function smallMilkyTribalTattoo(slot: number):void
		{
			clearOutput();
			...
			if (slot == 0)
			{
				...
				Render.text("“<i>Oh hey, that’s quite pretty.</i>” [bathgirlName] smiles down at what you’ve drawn as you withdraw the pen. “<i>Thanks, [name]!</i>” You kiss her fondly on the forehead and send her on her way.");
				tText += "collarbone.";
		...
		private function bigMilkyTribalTattoo(slot: number):void
		{
			clearOutput();
		...
		private function bigMilkyHeartTattoo(slot: number):void
		{
			clearOutput();
		...
		private function bigMilkyPropertyOfTattoo(slot: number):void
		{
			clearOutput();
		...
		private function bigMilkyBathToyTattoo(slot: number):void
		{
			clearOutput();
		...
		private function bigMilkyMegaMilkTattoo(slot: number):void
		{
			clearOutput();
		...
		private function bigMilkyCockCozyTattoo(slot: number):void
		{
			clearOutput();
		...
		private function heartTattoo(slot: number):void
		{
			clearOutput();
			whitneySprite();
		...
		private function amilyHeartTattoo(slot: number):void
		{
			clearOutput();
		...
		private function jojoHeartTattoo(slot: number):void
		{
			clearOutput();
		...
		private function bimboSophieHeartTattoo(slot: number):void
		{
			clearOutput();
		...
		private function vapulaHeartTattoo(slot: number):void
		{
			clearOutput();
		...
		private function kellyHeartTattoo(slot: number):void
		{
			clearOutput();
		...
		private function smallMilkyHeartTattoo(slot: number):void
		{
			clearOutput();
		...
		private function smallMilkyPropertyOfTattoo(slot: number):void
		{
			clearOutput();
		...
		private function smallMilkyBathToyTattoo(slot: number):void
		{
			clearOutput();
		...
		private function smallMilkyMegaMilkTattoo(slot: number):void
		{
			clearOutput();
		...
		public function numMilkyButterflyTats(): number
		{
			let count: number = 0;
		...
		private function smallMilkyButterflyTattoo(slot: number):void
		{
			clearOutput();
		...
		private function kellyHorseshoeTattoo(slot: number):void
		{
			clearOutput();
		...
		private function kellyPropertyOfTattoo(slot: number):void
		{
			clearOutput();
		...
		private function kellyNo1FillyTattoo(slot: number):void
		{
			clearOutput();
		...
		private function kellyDickWonTattoo(slot: number):void
		{
			clearOutput();
		...
		private function propertyTattoo(slot: number):void
		{
			clearOutput();
			whitneySprite();
		...
		private function amilyPropertyTattoo(slot: number):void
		{
			clearOutput();
		...
		private function jojoPropertyTattoo(slot: number):void
		{
			clearOutput();
		...
		private function jojoSissySlutTattoo(slot: number):void
		{
			clearOutput();
		...
		private function bimboSophiePropertyOfTattoo(slot: number):void
		{
			clearOutput();
		...
		private function vapulaPropertyOfTattoo(slot: number):void
		{
			clearOutput();
		...
		private function vapulaCumAddictTattoo(slot: number):void
		{
			clearOutput();
		...
		private function vapulaButtslutTattoo(slot: number):void
		{
			clearOutput();
		...
		private function vapulaDildoPolisherTattoo(slot: number):void
		{
			clearOutput();
		...
		private function bimboSophieSwallowTattoo(slot: number):void
		{
			clearOutput();
		...
		private function bimboSophieBreedingBitchTattoo(slot: number):void
		{
			clearOutput();
		...
		private function bimboSophieCockGoesHereTattoo(slot: number):void
		{
			clearOutput();
		...
		private function bimboSophieWideLoadTattoo(slot: number):void
		{
			clearOutput();
		...
		private function no1Tattoo(slot: number):void
		{
			clearOutput();
			whitneySprite();
		...
		private function amilyBreedingBitchTattoo(slot: number):void
		{
			clearOutput();
		...
		private function champCocksuckerTattoo(slot: number):void
		{
			clearOutput();
			whitneySprite();
		...
		private function amilyCockGoesHereTattoo(slot: number):void
		{
			clearOutput();
		...
		private function jojoCockGoesHereTattoo(slot: number):void
		{
			clearOutput();
		...
		private function amilyMommysGirlTattoo(slot: number):void
		{
			clearOutput();
		...
		private function jojoMommysBoyTattoo(slot: number):void
		{
			clearOutput();
		...
		private function champPussylickerTattoo(slot: number):void
		{
			clearOutput();
			whitneySprite();
		...
		private function dontTestBranding():void
		{
			...
			Render.text("Maybe later, you say. Whitney looks disappointed.");
		...
		private function numAmilyTribalTats(): number
		{
			let count: number = 0;
		...
		private function numWhitneyTribalTats(): number
		{
			let count: number = 0;
		...
		private function numJojoTribalTats(): number
		{
			let count: number = 0;
		...
		private function numSophieTribalTats(): number
		{
			let count: number = 0;
		...
		private function numVapulaTribalTats(): number
		{
			let count: number = 0;
		...
		private function numKellyTribalTats(): number
		{
			let count: number = 0;
		...
		private function numMilkyTribalTats(): number
		{
			let count: number = 0;
			if (flags[FlagEnum.MILKY_TATTOO_COLLARBONE] is String)
			{
		...
		private function getOrgyRoom():void
		{
			...
			Render.text("It’s a heavy investment, but her description of what she intends has definitely piqued your interest. Slowly you turn and, with a flick of the head, indicate the rock where she leaves the farm’s profit each week. Whitney breaks out onto a wide grin, her hands scrunching up on her skirt.");
		...
		private function noOrgyRoomPlz():void
		{
			...
			Render.text("You tell her it’s a fine idea, but it will have to wait for now.");
		...
		private function orgyRoomTalk():void
		{
			...
			Render.text("You can hear Whitney humming to herself happily as you approach her outside the milking shed, and when she sees you she rushes over to clutch at your arm excitedly.");
			...
			if (whitneyDom()) Render.text("The sight sends a shuddering thrill down your spine. “<i>You didn’t think I’d forget those, did you [master]? We both know how important they are for properly relaxing you,</i>” Whitney murmurs, pushing her hot suppleness into your side. ");
			else Render.text("You raise an eyebrow at these and Whitney giggles and blushes. “<i>Just in case you’re feeling adventurous, [master]. Or, um, want help.</i>”");
			Render.text("You might be able to recognize those but you have no idea what the final object you can make out in the room’s dusky gloom is, stood against the far wall. It’s at least four foot tall and looks like an ornate stone urn, with a round bottom, a thin body, some sort of square contraption at the top, and a series of long tubes emerging from its stem. There’s a very small brazier next to it, with a few pebbles of charcoal huffing feebly at the bottom.");
			...
			if (!whitneyDom()) Render.text("“<i>It’s exactly what a harem owner needs, [master] - with this you could take care of me and your other sluts all night, every night!</i>”");
			else Render.text("“<i>I could do so many things to you [master], if you were on this stuff. And for so long. Just imagine it!</i>”");
			Render.text(" You don’t feel woozy or spaced out at all - if anything, you feel slightly more aware and in control of your surroundings, it’s just that your skin feels like it’s coated with something vaguely fuzzy. ");
		...
		private function orgyRoomSubMassage():void
		{
			...
			Render.text("You say in a grand, wearied tone that you’ve arrived from another hard day of bringing Mareth to its knees, and that you require the services of your pet masseuse. ");
			...
			if (player.isBiped()) Render.text(" you clamber onto the heated slab, placing your chin on your hands on the cushion at the end");
			else if (player.isDrider()) Render.text(" you tick-tack over and sink your spider underbelly onto the heated marble, before resting your human half down as well. Whitney was foresighted enough to make the slab quite long, and you complete the action in comfort");
			else if (player.isNaga()) Render.text(" you slither over and slide onto the heated slab, placing your chin on your hands on the cushion at the end. You allow your long, heavy tail to pool onto the floor below");
			Render.text(".");
			...
			if (player.lowerBody.butt.buttRating >= 6) Render.text("soft");
			else Render.text("brawny");
			Render.text(" rear. After she’s finished making your butt feel red and smooth she dips a heated, greasy hand between your [hips] and");
			if (player.lowerBody.cockSpot.hasCock()) Render.text(" grips your [cock]");
			else if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" traces the entrance of your [vagina]");
			Render.text(" for a moment... before sliding out with a throaty giggle, moving on down to your hips.");
			...
			if (player.isBiped())
			{
				...
				{
					Render.text(" Your mouth opens as a twinge of pure pleasure veins up your [leg] as she massages the soft arch of the sole, making your");
					if (player.lowerBody.cockSpot.hasCock()) Render.text(" cock thicken");
					if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and your");
					if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" pussy moisten");
					Render.text(".");

					Render.text(" There must be a direct nerve link leading right from the bottom of your body up to your groin because my word, that really shouldn’t feel as good as it does. She swirls her thumbs across the rougher pads of your feet, dipping her warm fingers in and around the valleys of your");
					if (player.lowerBody == LOWER_BODY.DEMONIC_CLAWS || player.lowerBody == LOWER_BODY.LIZARD || player.lowerBody == LOWER_BODY.HARPY || player.lowerBody == LOWER_BODY.DRAGON) Render.text(" claws");
					else Render.text(" toes")
					Render.text(", before returning deliberately to stroke at your arch, indulging that nervous link until you are deep in the unexpected bliss of it... before slowly releasing, leaving you to wallow delightfully in the knowledge that that same slow, delicious attention is about to be lavished on your other [foot].")
				}
				else
				{
			...
			else if (player.isNaga())
			{
				...
				if (player.tongueType == TONUGE.SNAKE) Render.text(" You grin, flicking your forked tongue out to smell her arousal.");
				Render.text(" “<i>I imagine watching your [master] move around is almost a masochistic experience for you. Something so alien and unnerving now linked forever with pleasure and obedience. Is that how it is? You can’t stop staring at my beautiful, deadly form because it’s almost like a slutty punishment for you?</i>”");
			...
			if (player.lust >= 33) Render.text(" The hot tub bubbles quietly behind her and it is not just oil which is making your skin glow. This would be the moment to pursue a happy ending if you so wished.");
			else Render.text(" You play with the idea of ‘rewarding’ her but... damn. She’s actually managed to relax you <i>too</i> much. You thank her regally, redress, and head out the door, enjoying the lightness of step and self-control which thrums through you.");
		...
		private function orgyRoomSubMassageHappyEnding():void
		{
			...
			Render.text("You silently rise from the slab and put your arm around your masseuse’s tight body. It radiates heat into you as you send a hand sliding down the curve of her back to squeeze at her tight, supple butt.");
			...
			{
				Render.text(" With a single movement you swing her hot, compact weight over your shoulder. You growl into her ear that she’s still got plenty to do if she’s to finish her job of thoroughly relaxing every part of you.");
			...
			{
				Render.text(" You tighten your grip on her pert behind and growl that she’s still got plenty to do if she’s to finish her job of thoroughly relaxing every part of you.");
			...
			if (player.lowerBody.cockSpot.hasCock() && player.cockThatFits(whitneyVagCapacity() * 1.33) != -1 && player.lowerBody.vaginaSpot.hasVagina())
			{
				Render.text(" You take a long moment to consider which of your sexes you will satisfy yourself with.");
			}

			//[Male][Female]/[Next]
			menu();
			if (player.cockThatFits(whitneyVagCapacity() * 1.33) != -1) addButton(0, "Male", subHappyEndingMale);
			if (player.lowerBody.vaginaSpot.hasVagina()) addButton(1, "Female", subHappyEndingFemale);
		}
		...
		private function subHappyEndingMale():void
		{
			...
			if (hasBiggerCock) Render.text(" [cock biggest]");
			else if (player.isBiped()) Render.text(" against your [hips]");
			else Render.text(" around your back");
			Render.text(" momentarily. You grin, and begin to pump into her a little harder, using the full breadth of movement afforded to you by your oiled, [hips] to do so. It has the effect you anticipated. After a few more moments gamely keeping time with your own heavy exhalations the dog girl you’re buried into gasps, unable to contain herself. You winch her legs up by the knees and go at her faster again, still using the full movement of your pelvis to withdraw and sink every inch of your [cock " + cockThatFits + "], pushing into her pink tightness vigorously enough for the water around to splash briskly, buried into her even deeper. ");
			...
			if (player.cumQ() >= 1500) Render.text(" Her small capacity is almost immediately filled and jizz spurts out with every rich thrust you make, polluting the water around you with your massive load.");
			if (hasBiggerCock) Render.text(" Whitney has stroked and tantalized your [cock biggest] with her hot hands and smooth belly intermittently this whole time, and as your orgasm builds and overwhelms you she gets her just reward for it; you jet cum from [eachCock], pasting first her face and then her tight breasts with your thick cream as you ride her hard.");
		...
		private function subHappyEndingFemale():void
		{
			...
			if (player.wetness() >= 5) Render.text(" Your pussy spurts excitement gleefully with each thrust, utterly drowning her own in your juices, polluting the water around you with your musk.");
			if (player.lowerBody.cockSpot.hasCock()) Render.text(" Whitney has stroked and tantalized your [cock biggest] with her hot hands and smooth belly intermittently this whole time, and as your orgasm builds and overwhelms you she gets her just reward for it; you jet cum from [eachCock], pasting first her face and then her tight breasts with your thick cream as you ride her hard.");
