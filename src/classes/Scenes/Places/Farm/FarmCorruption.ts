package classes.Scenes.Places.Farm
{
	...
	public class FarmCorruption extends AbstractFarmContent
	{
		...
		public function collectTheGoodies():void
		{
			...
			{
				outputText("You stroll over to the big rock at the edge of the pepper patch, smiling as you see a small burlap sack shoved underneath a fold in the stone. You scoop it up, open it and count out " + flags[FlagEnum.FARM_CORRUPTION_GEMS_WAITING] + " gems.\n\n");
			...
			{
				outputText("<b>" + flags[FlagEnum.FARM_SUCCUMILK_STORED] + "x ");
				if (flags[FlagEnum.FARM_SUCCUMILK_STORED] == 1) outputText("vial of Succubi milk.");
			...
			{
				outputText("<b>" + flags[FlagEnum.FARM_INCUDRAFT_STORED] + "x ");
				if (flags[FlagEnum.FARM_INCUDRAFT_STORED] == 1) outputText("vial of Incubi draft.");
			...
			if (flags[FlagEnum.FARM_EGG_STORED] > 0)
			{
				outputText("<b>" + flags[FlagEnum.FARM_EGG_STORED] + "x ");
				outputText("large " + flags[FlagEnum.FOLLOWER_PRODUCTION_SOPHIE_COLORCHOICE] + " egg");
				if (flags[FlagEnum.FARM_EGG_STORED] > 1) outputText("s");
				outputText(".");
				outputText("</b>\n");
			...
			{
				outputText("<b>" + flags[FlagEnum.FARM_CONTRACEPTIVE_STORED] + "x ");
		...
		public function takeoverPromptNow():void
		{
			...
			if (flags[FlagEnum.KELT_BREAK_LEVEL] >= 4 || player.statusAffects.has("MarbleRapeAttempted"))
			{
				...
				if (flags[FlagEnum.KELT_BREAK_LEVEL] >=  4) outputText(" Kelt");
		...
		public function rootScene():void
		{
			...
			if (!whitneyCorrupt())
			{
				...
				if (flags[FlagEnum.FARM_UPGRADES_REFINERY] == 1) outputText(" A large machine, bulky and rotund with a conical top, has been built into the milking barn. Fat pipes crawl up onto the roof from it like metal ivy, and white smoke billows busily out of its whistle chimney.");
				
				if (flags[FlagEnum.FARM_UPGRADES_CONTRACEPTIVE] == 1) outputText(" Next to the pepper patch another crop has been planted, a field of verdant green shrubs. Their thin stems bob idly in the breeze.");
		...
		private function keltAChangeInManagement():void
		{
			...
			if (player.lowerBody.balls == 0) outputText(" So to speak, anyway.");
		...
		private function whitneyAppearanceCorrupt():void
		{
			...
			if (whitneyHasTattoo())
			{
				...
				if (flags[FlagEnum.WHITNEY_TATTOO_COLLARBONE] != 0) outputText(flags[FlagEnum.WHITNEY_TATTOO_COLLARBONE] + "\n");
				if (flags[FlagEnum.WHITNEY_TATTOO_SHOULDERS] != 0) outputText(flags[FlagEnum.WHITNEY_TATTOO_SHOULDERS] + "\n");
				if (flags[FlagEnum.WHITNEY_TATTOO_LOWERBACK] != 0) outputText(flags[FlagEnum.WHITNEY_TATTOO_LOWERBACK] + "\n");
				if (flags[FlagEnum.WHITNEY_TATTOO_BUTT] != 0) outputText(flags[FlagEnum.WHITNEY_TATTOO_BUTT] + "\n");
			} 
		...
		private function dontDeFurDoge():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText(" crotch into");
		...
		private function firstOralTraining():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) outputText(" and");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText(" cocksucker");
		...
		private function firstCockOralTrainingStageTwo():void
		{
			...
			else if (!player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.balls > 0)
			{
				...
				if (player.cumQ() >= 1500) outputText(" which is no mean feat, given it feels like you’ve fucked what feels like a gallon of it down her gullet. Once you’re almost done, as ever you spurt the last few creamy ropes across her face and clothes");
				outputText(".");
				if (player.lowerBody.cockSpot.count() > 1) outputText(" The second cock trapped in her grasp flexes out cum just as eagerly as the first, sweat standing out on your forehead as you are clenched by your twin high; it paints her skirt with its generous discharge.");
			}
			//Male without balls:
			else if (!player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.balls == 0)
			{
			...
			if (!player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				if (player.lowerBody.cockSpot.count() > 1) outputText(" and [cock biggest2]");
		...
		private function cockOralTrainingStageTwo():void
		{
			...
			if (!player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				if (player.lowerBody.cockSpot.count() > 1) outputText("She wraps her hand around your [cock biggest2] when she’s ready, giving it an experimental pump before beginning to softly and slowly work it, remembering exactly how to grip and bend its girth as she prepares for the main course.");
				...
				if (player.lowerBody.cockSpot.count() > 1) outputText("she occasionally forgets the length bulging in her hand so eager is she to push her lips further down your [cock biggest] and work at her own clit, and you need to murmur to get her to concentrate.");
				...
				if (player.lowerBody.cockSpot.count() == 1) outputText("Whitney cleans your [cock biggest] with her flat tongue, humming as she laps your oozing head clean, deliciously soothing your aching cock.");
		...
		private function firstCockOralTrainingStageThree():void
		{
			...
			if (!player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				else
				{
					outputText("\n\n“<i>Put your hand here.</i>” Slowly she withdraws her fingers from her panties and wraps them around the base of your [cock biggest]. You sigh at the hot, damp pressure now ringing the very bottom of your saliva-glossed member. “<i>Work it right there... that’s it...</i>” You sigh as she swiftly picks up a steady beat");
						if (player.lowerBody.cockSpot.count() > 1) outputText(" jerking both your cocks with her smooth, exacting grasp in tandem");
						outputText(".");
				}
				...
				if (player.lowerBody.cockSpot.count() > 1)
				{
					...
					if (player.lowerBody.cockSpot.count() > 2) outputText(" and [cock biggest3]");
				...
				if (player.lowerBody.cockSpot.count() > 1 || player.cumQ() >= 1500) outputText("\n\n");
				if (player.lowerBody.cockSpot.count() > 1)
				{
					outputText("She doesn’t forget the cock");
					if (player.lowerBody.cockSpot.count() > 2) outputText("s");
					outputText(" trapped in her grasp for a moment; she milks");
					if (player.lowerBody.cockSpot.count() > 2) outputText(" them");
					else outputText(" it");
					outputText(" as hard as you’re fucking her mouth, and");
					if (player.lowerBody.cockSpot.count() > 2) outputText(" they flex");
					...
					if (player.lowerBody.cockSpot.count() == 2) outputText(" twin");
					else if (player.lowerBody.cockSpot.count() == 3) outputText(" triple");
					else outputText(" transcendent");
					outputText(" high;")
					if (player.lowerBody.cockSpot.count() > 2) outputText(" they paint");
					else outputText(" it paints");
					outputText(" her skirt with")
					if (player.lowerBody.cockSpot.count() > 2) outputText(" their");
					else outputText(" its");
			...
			if (player.lowerBody.cockSpot.count() > 1) outputText(" She doesn’t forget the cock trapped in her grasp for a moment either; she milks it as hard as you’re fucking her mouth, and it flexes out cum just as eagerly as the first, delirium seizing you as you are clenched by your triple high as it paints her skirt with its generous discharge.");
			...
			outputText("\n\nOnce you have tensed your last you sigh beatifically and float high and formless on your post-blowjob haze. You close your eyes as, without any bidding, ");
			if (player.lowerBody.cockSpot.count() == 1) outputText("Whitney first cleans your [vagina] and then your [cock biggest] with her flat tongue, polishing your dripping pussy and thighs to a shine before lapping your oozing head clean, deliciously soothing your aching cock.");
		...
		private function cockOralTrainingStageThree():void
		{
			...
			if (player.lowerBody.cockSpot.count() >= 2) outputText(", allowing your other cock");
			if (player.lowerBody.cockSpot.count() > 2) outputText("s");
			if (player.lowerBody.cockSpot.count() >= 2) outputText(" to unfurl and bob behind it");
			if (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.cockSpot.count() >= 2) outputText(" and");
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText(", leisurely exposing your moist [vagina]");
			if (player.lowerBody.cockSpot.count() >= 2 && player.lowerBody.vaginaSpot.hasVagina()) outputText(" at the same time");
			...
			if (!player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				if (player.lowerBody.cockSpot.count() > 2) outputText("Finally, she snakes her other hand around your [cock biggest3], rolling her wrist as she gently wrings it with the same sliding thoroughness.} You open your mouth to the shifting delectation inundating your overgrowth of cocks.");
				...
				if (player.lowerBody.balls > 0) outputText(", your saliva-glossed [balls] beating against her chin");
				...
				if (player.lowerBody.cockSpot.count() > 1)
				{
					...
					outputText(" as hard as you’re fucking her mouth, and");
					if (player.lowerBody.cockSpot.count() == 2) outputText(" it flexes");
					...
					outputText(" high;");
					if (player.lowerBody.cockSpot.count() == 2) outputText(" it paints");
					else outputText(" they paint");
					outputText(" her skirt with ");
					if (player.lowerBody.cockSpot.count() == 2) outputText(" its");
			...
			else if (player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				if (player.lowerBody.cockSpot.count() > 1) outputText("As she bathes your [cock biggest] in delicious sensation she wraps her hand around your [cock biggest2], softening and then tightening her grip, sliding up and down its stem until it is every bit as rock-hard as the one she is lapping at. ");
				...
				if (player.lowerBody.cockSpot.count() > 1)
				{
					...
					if (player.lowerBody.cockSpot.count() == 2) outputText("triple");
					else outputText("transcendent");
		...
		private function cockOralTrainingMaxed():void
		{
			...
			if (!player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				if (player.lowerBody.balls > 0) outputText("You sigh blissfully as her plump lips push into your [balls] before the flat, warm flannel-like tongue spreads itself across them, covering every inch, each sensitive orb first sensuously kissed and then taken into your slave’s mouth to be suckled gently until they are thrumming and bulging with packed heat. The loving attention laved on them is made all the better by the soft, lustful moans turning into outright panting sobs of pleasure drifting up from between your legs as she obsequiously polishes them.");

				outputText("\n\nShe rises until her head is level with your semi-erect prick and with long, careful, exquisite licks makes it fiercely erect, running her tongue along its full length and then swirling it around your head, pressing into the sensitive spots she knows will make you dense and tight with need. She grips it steadily at the base as she does so, every so often giving it a tight squeeze to contrast with a particularly soft lap at your bulging head.");
				if (player.lowerBody.cockSpot.count() > 1) outputText(" As she bathes your [cock biggest] in delicious sensation she wraps her hand around your [cock biggest2], softening and then tightening her grip, sliding up and down its stem until it is every bit as rock-hard as the one she is lapping at.");
				if (player.lowerBody.cockSpot.count() > 2) outputText(" Finally, she snakes her other hand around your [cock biggest3], rolling her wrist as she gently wrings it with the same sliding thoroughness.");
				...
				if (player.lowerBody.cockSpot.count() > 1) outputText("In the long, delicious daze that follows you move her around");
				if (player.lowerBody.cockSpot.count() == 2) outputText(", switching her attention to your [cock biggest2] for a while");
				else if (player.lowerBody.cockSpot.count() >= 3) outputText(" switching her attention first to one cock and then the other");
				if (player.lowerBody.cockSpot.count() > 1) outputText(" so that each of your fiercely erect pricks has time to enjoy the tight, sucking embrace of her mouth.");
				...
				if (player.lowerBody.balls > 0) outputText(" and her tongue once again sliding across your [balls], ");
				...
				if (player.lowerBody.cockSpot.count() > 1) outputText(", her hands wrapping back around your other cock");
				if (player.lowerBody.cockSpot.count() > 2) outputText("s");
				if (player.lowerBody.cockSpot.count() > 1) outputText(" as if they never left them");
			...
			else if (player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				if (player.lowerBody.cockSpot.count() > 1) outputText(" In the long, delicious daze that follows you move her around");
				if (player.lowerBody.cockSpot.count() == 2) outputText(", switching her attention to your [cock biggest2] for a while");
				if (player.lowerBody.cockSpot.count() >= 3) outputText(", switching her first to one cock then another");
				if (player.lowerBody.cockSpot.count() > 1) outputText(" so that each of your fiercely erect pricks has time to enjoy the tight, sucking embrace of her mouth.");
				...
				if (player.lowerBody.cockSpot.count() > 1) outputText(" and her other hand wrapping back around your [cock biggest2]");
				...
				if (player.wetness() > 2) outputText("soaking her arm as it gutters your juices, ");
				outputText("heightening the ecstasy you feel to mind-blowing heights.");
				if (player.lowerBody.cockSpot.count() > 1) outputText(" She doesn’t forget the [cock biggest2] trapped in her grasp for a moment; she milks it as hard as you’re fucking her mouth, and it flexes out cum just as eagerly as the first, delirium seizing you as you are clenched by your triple high; it paints her skirt with its generous discharge.");
		...
		private function firstVaginaOralTraining():void
		{
			...
			if (player.lowerBody.vaginaSpot.list[0].clitLength > 3) outputText(" despite the fact it must be practically poking her in the eye");
		...
		private function firstVaginaOralTrainingStageTwo():void
		{
			...
			if (player.upperBody.chest.hasFuckableNipples()) outputText(" obscene folds");
			...
			if (player.upperBody.chest.hasFuckableNipples()) outputText(" sends her tongue questing into its sensitive inside.");
		...
		private function vaginaOralTrainingStageTwo():void
		{
			...
			if (!player.upperBody.chest.hasFuckableNipples()) outputText(" rock-hard and");
		...
		private function firstVaginaOralTrainingStageThree():void
		{
			...
			if (!player.upperBody.chest.hasFuckableNipples()) outputText(" she takes each of your [nipples] into her mouth to bathe them in close, sucking attention");
			...
			if (player.lowerBody.vaginaSpot.list[0].clitLength >= 4) outputText(" She uses her new anchorage to take your bulging fem-dick entirely into her mouth, sucking on it avidly; she hums as she does it, sending ecstatic pleasure thrumming through you.");
		...
		private function vaginaOralTrainingMaxed():void
		{
			...
			if (!player.upperBody.chest.hasFuckableNipples()) outputText(" she envelopes each of your [nipples] with her lips, kissing them with their sumptuous sponginess before bathing them in close, sucking attention");
			...
			if (player.lowerBody.butt.buttRating < 4) outputText("firm butt");
			if (player.lowerBody.butt.buttRating < 8) outputText("supple rump");
		...
		private function whitneyDomBondageOral():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("cum");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("[cock biggest]"); 
			if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) outputText(" and ");
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("[vagina]");
		...
		private function whitneyDomStraponDoggy():void
		{
			...
			if (capacity >= 36)
			{
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText("deep, eager vagina");
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText("sit and spread ‘em for anything going, don’t you?");
			...
			else
			{
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText("take of your tight cunt");
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText("servants, you tight, bossy little bitch");
			...
			if (player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock() && kGAMECLASS.vapula.vapulaSlave()) outputText(" You feel the end of the dildo tense deep within you before releasing, filling you full of warm demon spunk until it pools out onto the bed. The feeling is horribly, disgustingly satisfying, and for a moment you can see exactly how Vapula views the world.");
		...
		public function brandAmily():void
		{
			...
			else if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" pussy");
		...
		private function orgyRoomSubMassage():void
		{
			...
			if (player.lowerBody.butt.buttRating >= 6) outputText("soft");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText(" grips your [cock]");
			else if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" traces the entrance of your [vagina]");
			...
			if (player.isBiped())
			{
				...
				if (!(player.lowerBody == LOWER_BODY.HOOFED || player.lowerBody == LOWER_BODY.NAGA || player.lowerBody == LOWER_BODY.GOO || player.lowerBody == LOWER_BODY.PONY))
				{
					...
					if (player.lowerBody.cockSpot.hasCock()) outputText(" cock thicken");
					if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) outputText(" and your");
					if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" pussy moisten");
					outputText(".");
					...
					if (player.lowerBody == LOWER_BODY.DEMONIC_CLAWS || player.lowerBody == LOWER_BODY.LIZARD || player.lowerBody == LOWER_BODY.HARPY || player.lowerBody == LOWER_BODY.DRAGON) outputText(" claws");
			...
			else if (player.isNaga())
			{
				...
				if (player.tongueType == TONUGE.SNAKE) outputText(" You grin, flicking your forked tongue out to smell her arousal.");
		...
		private function subHappyEndingFemale():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText(" Whitney has stroked and tantalized your [cock biggest] with her hot hands and smooth belly intermittently this whole time, and as your orgasm builds and overwhelms you she gets her just reward for it; you jet cum from [eachCock], pasting first her face and then her tight breasts with your thick cream as you ride her hard.");
