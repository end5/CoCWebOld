...
internal function defeatKellyNDBREAKHIM():void {
	...
	if(player.lowerBody.balls > 0) outputText(" and fondling your [balls] with great care");
...
outputText("\n\nKelly remains quiet for a couple of seconds before slowly muttering, \"<i>Please [Master], give me your cock.  I want nothing more in the world.  I'll squeeze you until you're milked dry.  Please, let me suck your dick!</i>\"");
	...
	if(player.lowerBody.balls > 0) outputText("and your [balls] churn ");
...
private function finalKeltBreaking():void {
	...
	if(player.lowerBody.balls > 0) outputText("Her free hands moves down on your groin and caresses your [balls] with expert care and precision; obviously her previous experience in groping has had an effect on her current skills.  ");
...
private function kellyAppearance():void {
	...
	if (flags[FlagEnum.KELLY_BONUS_BOOB_ROWS] > 0) outputText("  Beneath those, she has a second row of jiggly tits, just aching to be squeezed.");
	if (pregnancy.isPregnant) outputText("  Drops of milk often dribble from her tits, betraying her potent lactation.");
	//[enter vagina text] 
	outputText("\n\nKelly has a ");
	//[virgin]
	if(flags[FlagEnum.KELLY_VAGINALLY_FUCKED_COUNT] == 0) outputText("virgin, ");
	if(flags[FlagEnum.KELLY_CUNT_TYPE] == 0) outputText("pink pussy");
	...
	if (flags[FlagEnum.KELLY_HEAT_TIME] > 0 && !pregnancy.isPregnant) outputText("<b>  There is so much leaking from her that you think she might be in heat, rendering her more receptive to impregnation.</b>");
	...
	if (farm.farmCorruption.hasTattoo("kelly"))
	{
		...
		else
		{
			...
			if (flags[FlagEnum.KELLY_TATTOO_COLLARBONE] != 0) outputText(flags[FlagEnum.KELLY_TATTOO_COLLARBONE] + "\n");
			if (flags[FlagEnum.KELLY_TATTOO_SHOULDERS] != 0) outputText(flags[FlagEnum.KELLY_TATTOO_SHOULDERS] + "\n");
			if (flags[FlagEnum.KELLY_TATTOO_LOWERBACK] != 0) outputText(flags[FlagEnum.KELLY_TATTOO_LOWERBACK] + "\n");
			if (flags[FlagEnum.KELLY_TATTOO_BUTT] != 0) outputText(flags[FlagEnum.KELLY_TATTOO_BUTT] + "\n");
			outputText("\n");
		}
	}
...
private function approachKelly():void {
	...
	if (flags[FlagEnum.FARM_CORRUPTION_STARTED] == 0) outputText("You aren't welcome on the farm proper, but you can go visit Kelly's field.");
	...
	if(model.time.hours >= 9 && model.time.hours <= 11 && flags[FlagEnum.KELLY_KIDS] >= 2) {
		...
		if(flags[FlagEnum.KELLY_KIDS] > 2) outputText("two youngest ");
		outputText("children with ");
		if(flags[FlagEnum.KELLY_BONUS_BOOB_ROWS] > 0) outputText("the top row of ");
	...
	if(player.hasItem(consumables.CANINEP)) {
		...
		if(flags[FlagEnum.KELLY_BONUS_BOOB_ROWS] == 0) outputText(", but who knows how it will change her");
		outputText(".");
...
private function fuckKellysCunt():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() > 2) outputText("s hang");
	...
	if(player.lowerBody.balls > 0) outputText(" and roiling balls");
	outputText(" leaves no doubt that climax is fast approaching.  Kelly cums first, splattering your [hips] with her ");
	if(flags[FlagEnum.KELLY_CUNT_TYPE] > 0) outputText("fragrant ");
...
private function taurOnTaurSexKelly():void {
	...
	if(player.lowerBody.balls > 0) outputText("  Your [balls] tingle pleasantly from the attention, and you groan when she sucks one into her mouth, worshipping it with her lips.");
	...
	if(flags[FlagEnum.KELLY_CUNT_TYPE] > 0) outputText("musky ");
	...
	if(player.stats.cor < 50) outputText("  You feel a little bad for her and help her up - she did just milk your dick in the way that only a centaur can.");
...
private function tentaFuckKelly():void {
	...
	if(player.stats.cor < 33) outputText("but also a bit disgusted to watch such shameless, wanton behavior, ");
...
private function getARimjobFromKelly():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("s harden");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" and pussy");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("; she slides as many fingers as she can into your [vagina] and frigs you as best she can");
	...
	//[Pussy:]
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your female sex orgasms in tandem, your wet flesh seizing up around Kelly's fingers and spattering her arm and chest with girl juices.");
...
private function rideKellyForPunishment():void {
	...
	if(flags[FlagEnum.TIMES_RIDDEN_KELLY_FOR_PUNISHMENT] == 1) 
	{
		...
		if(player.tallness < 78 && !player.isTaur()) 
		{
			...
			if(flags[FlagEnum.KELLY_BONUS_TIT_ROWS] > 0) outputText("top row of ");
			outputText("breasts.");
			...
			if(flags[FlagEnum.KELLY_LACTATING] > 0) outputText(" hard enough to produce thin arcs of milk");
		...
		else 
		{
			...
			{
				if(flags[FlagEnum.KELLY_BONUS_TIT_ROWS] > 0) outputText("All four of h");
				else outputText("H");
	...
	//First repeat: 
		...
		if(flags[FlagEnum.KELLY_BONUS_TIT_ROWS] > 0) outputText("  Just as well there were four of them, eh?");
...
private function takeKellysVirginity():void {
	...
	if(flags[FlagEnum.KELLY_CUNT_TYPE] == 1) outputText("horse-");
	outputText("cunt.");
	...
	if(flags[FlagEnum.KELLY_CUNT_TYPE] == 1) outputText("lust-swollen, black-lipped fuck-hole");
	...
	if(flags[FlagEnum.KELLY_CUNT_TYPE] == 1) outputText(", equine");
	outputText(" cunt until her hymen is easily visible.  You rub every sensitive fold that you can get to");
	
	if(flags[FlagEnum.KELLY_CUNT_TYPE] == 1) outputText(", taking time to squeeze her plush, onyx vulva teasingly.  Kelly whinnies in response, overtaken by her potent equine instincts.");
	...
	if(player.cockArea(x) <= 300)
	{
		...
		if(flags[FlagEnum.KELLY_CUNT_TYPE] == 1) outputText("plump, equine cunt-lips");
	...
	if(flags[FlagEnum.KELLY_CUNT_TYPE] == 0) outputText("the redness of her well-used cunt.");
...
private function kellyPregSex():void {
	...
	if(player.lowerBody.cockSpot.list[x].cockType != CockType.HORSE) outputText("\n\nToo bad she gets yours instead.");
...
private function talkNHandToKelly():void {
	...
	if(flags[FlagEnum.KELLY_TALK_N_HAND_TIMES] == 0) {
		...
		if(!player.isTaur()) {
			...
			if(player.lowerBody.cockSpot.count() == 1) outputText("[oneCock], gently tugging it.  It quickly hardens with the pleasing friction she coils around it.");
		...
		//Centaur:
			...
			if(player.lowerBody.cockSpot.count() == 1) outputText("a warm hands wraps itself around your [cock] and then begins to gently tug, quickly hardening it with pleasing coils of friction.");
	...
	if(temp == 0) {
		...
		//1< cock:
		if(player.lowerBody.cockSpot.count() > 1) outputText("imagining thrusting every single one of your cocks into a wet, willing hole, ");
		//vagina:
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("imagining one of them tongue-fucking your [vagina] as you buck into another's tight cunt, ");
		...
		if(player.stats.cor < 70) outputText("  Plus, the thought of making Kelt's victims pay the price for his own crimes makes you feel uncomfortable.");
	...
	//2. 
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText(" and [cock 2]");
	...
	//4.
		...
		if(player.lowerBody.balls == 0) outputText("suckles and laps at the very base of your [cock] for a time");
		...
		if(flags[FlagEnum.KELLY_KIDS] > 0) outputText(" and bearing and raising your children");
...
private function giveKellyAppleSauce():void {
	...
	//Repeat:
		...
		if(flags[FlagEnum.KELLY_KIDS] > 1) outputText("  Somebody has even cleared an oblong patch of earth alongside the trees- a piece of card stuck into it is scrawled with a childish hand which declares it to be “CAROTS”.  You marvel at it all.  Who would have imagined your lust for horse pussy would have turned a pointless asshole into a loving mother who gardens with her children?  Really, you deserve a medal for your efforts.");
		...
		if(!player.isTaur()) {
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your [vagina] quivers and clenches, and once again, gasping with the dual orgasm which is clutching your body, you stand over the bowl and let some of your femcum drip into it.");
			//[High cum:
			if(player.cumQ() >= 750) outputText("  The bowl quickly begins to brim as your [cock] torrents out cum, and eventually you have to turn away, grunting out the last jets of filth into the undergrowth.");
		}
		//[Centaur:
	...
	if(!player.isTaur()) {
		...
		if(player.lowerBody.cockSpot.list[x].cockThickness >= 4) outputText("  Your girth spreads her fuck tunnel wide and you grunt at the delicious tightness of it as you pack her full of cock.");
		...
		if(player.lowerBody.balls > 0) outputText(", your [balls] slapping into her thighs demandingly ");
	...
	if(player.stats.cor < 60) outputText("The taste of cum glazing her apple breath is off-putting but what did you really expect?");
...
private function kellyBJsAhoy():void {
	...
	if(!player.isTaur()) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  You're in no hurry, and with a slight roll of your [hips] you direct her to start at the back and work her way up.");
		...
		if(player.lowerBody.balls > 0 && player.lowerBody.vaginaSpot.hasVagina()) {
			...
			if(player.lowerBody.ballSize <= 8) {
				...
				if(player.lowerBody.balls > 2) outputText(", then the third, then the fourth");
			...
			//[Huge balls:
				...
				//[Dominika dreams:
				if(flags[FlagEnum.NUMBER_OF_TIMES_MET_SCYLLA] > 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00150] > 0) outputText("  You are gripped by a sudden but thrilling velvet-edged sense of déjà vu.  Has this scene not repeated somewhere else?");
			}
		}
		//Vagina:
		...
		//Balls:
			...
			if(player.lowerBody.ballSize <= 8) {
				...
				if(player.lowerBody.balls >= 4) outputText(", then the third, then the fourth");
		...
		if(player.lowerBody.balls > 0) outputText(", as you feel your [balls] clench");
		...
		if(flags[FlagEnum.KELLY_BONUS_BOOB_ROWS] > 0) outputText(" onto all four of her tits");
	...
	//Centaur:
		...
		if(player.lowerBody.cockSpot.count() > 1 || player.lowerBody.vaginaSpot.hasVagina()) outputText("  You're in no hurry, and with a slight roll of your [hips] you direct her to start at the front and work her way down.");
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("pussy-licking, ");
		if(player.lowerBody.balls > 0) outputText("ball-sucking ");
		...
		//[Vagina:
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your female sex clenches and orgasms in sympathy, and you feel fluid dribble down your [hips] as you continue to paint Kelly.  Your only dim regret is that there's no way of soaking her with that, too.");
		...
		if(player.cumQ() < 750) outputText("  Kelly has done a sterling job of catching your load; you've frosted her boobs with long strokes quite convincingly.");
			...
			if(flags[FlagEnum.KELLY_BONUS_BOOB_ROWS] > 0) outputText("all four of ");
	...
	//Libido >50:
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText(", letting her sensually lick you out again until your [vagina] is gleaming with arousal");
		if(player.lowerBody.balls > 0) outputText(", letting her needily suckle on your [balls] again until they are inflamed with renewed pressure");
