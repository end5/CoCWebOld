...
private function keltEagerlyGetNaked():void {
	...
	if(player.gender == 1 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("\"<i>What are you, a girl?  Get on with it so I can see what I'm dealing with.</i>\"\r\r", false);
	else {
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating == 0) outputText("\"<i>Oops!  Could have sworn I'd find a pair of tits on you.  That's okay... I'm sure you'll hit puberty some day!</i>\"\r\r", false);
		//(Too small, A to DD: 
		else if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 5) outputText("\"<i>Ha! No wonder you cover up like a sniveling human!  Any centaur maiden would be ashamed to go out in public with " + num2Text(player.totalBreasts()) + " mosquito bites like that!  What do they call you back home... 'Tiny Tits'?  Or maybe they all just assume you're a guy!</i>\"\r\r",false);
		//(Medium, E to HHH:
		else if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 11) outputText("\"<i>Uh oh, boys... look out!  Miss Melons here has been putting on a little weight!  How did someone like you become an adventurer?  From the waist up, you're good for whoring and not much else!</i>\"\r\r", false);
	...
	if(player.lowerBody.butt.buttRating < 6) outputText("\"<i>Well, someone works out!  Got a nice, tight little ass there! Probably a little too tight, huh?  Don't worry, you can tell me... do the mean old monsters hurt when they fuck that poor little ass of yours? HA!</i>\"\r\r", false);
	//(Medium, Large to Heavy:
	else if(player.lowerBody.butt.buttRating < 13) outputText("\"<i>Hey, nice little cushion you got back here!  Do you get that from sitting around all day?  Nah, can't be that.  I'm sure you get more than a workout on this baby... of course, cock is no substitute for a little exercise, you know!</i>\"\r\r", false);
	...
	if(player.totalCocks() == 0) outputText("\"<i>Well, aren't you a pretty little thing, all together.  You and I will get along real nice, that's for sure!</i>\"\r\r", false);
		...
		//(Human Cock:
		if(player.lowerBody.cockSpot.list[0].cockType == CockType.HUMAN) outputText("\"<i>Heh.  Just sporting the normal model, huh?  You know there's a phrase around here... hung like a human!  Trust me... it's not a compliment.  ", false);
		//(Dog Cock:
		if(player.lowerBody.cockSpot.list[0].cockType == CockType.DOG) outputText("\"<i>Oh, lookie here!  We got ourselves a little doggie dong!  Well, cock or not, you'll always be a bitch to me, mutt.  ", false);
		//(Horse Cock: 
		if(player.lowerBody.cockSpot.list[0].cockType == CockType.HORSE) outputText("\"<i>Now that's just sad.  Getting a little envious of me, huh?  Had to go out and get a nice horsecock all your own?  Should have asked.  I'd have given you a taste of mine!  ", false);
		//(Tentacle Cock or other weirdness
		if(player.lowerBody.cockSpot.list[0].cockType.Index >= 3) outputText("\"<i>Hah!  I'll bite... what the fuck are you supposed to be?  That a cock, or just an ugly, misplaced tail?  HA!  ", false);
		//(Small Size, ?-10 inches:
		if(player.lowerBody.cockSpot.list[0].cockLength <= 10) outputText("\"So, when you poke a woman with that, does she notice?  Or do you have to tell her when to start faking her orgasm?</i>\"\r\r", false);
		//(Medium Size, 11-20 inches: 
		else if(player.lowerBody.cockSpot.list[0].cockLength <= 20) outputText("\"Still, at least you've got a decent sized cock.  You know, for a colt.  Maybe you'll get lucky, and the real women will take pity on you!</i>\"\r\r", false);
...
private function keltMainEncounter():void {
	...
	//(Submissive 0-30: 
	if(player.statusAffects.get("Kelt").value2 <= 30) outputText("You grind your teeth in irritation, but swallow your pride enough to ask him for help.  ", false);
	//(Submissive 30-70:
	else if(player.statusAffects.get("Kelt").value2 <= 70) outputText("You nod reluctantly, and Kelt grins.  He may be arrogant, but he is rather good.  ", false);
	//(Submissive 70-100: 
...
private function keltMainEncounter3():void {
	...
	//NAKERS
		...
		if (player.inHeat && player.gender > 1) {
			...
			//(Submissive, 0-30: 
			if(player.statusAffects.get("Kelt").value2 <= 30) outputText("You shiver and groan, unable to help yourself.  It is clear that Kelt has every intention of breeding you, and you are helpless to stop the urges of your body.  Terrifying images of being raped daily by this cruel beast fill your head... of your belly swelling with his young again and again.  You let out a moaning cry, and orgasm helplessly even as Kelt laughs.\r\r", false);
			//(Submissive, 30-70: 
			else if(player.statusAffects.get("Kelt").value2 <= 70) outputText("The thought of that fills you with a dreadful shiver of lust, from your head to your toes.  Your body longs to be bred, again and again, and the idea of submitting to this powerful creature is so powerfully erotic that you cum on the spot, orgasming with delightful abandon.  The thought of being this centaur's breeding slave feels so right!\r\r", false);
			//(Submissive, 70-100: 
		...
		if(temporary == 3) {
			...
			//(Small Size, A-DD: 
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 5) outputText("\"<i>Ha!  Even with your itty-bitty-titty, you have to admit to a certain... weakness, is it?  Awfully sensitive, aren't they?  Ooh, am I making the little girl wet?  Naughty slut!</i>\"\r\r", false);
			//(Medium Size, E-HHH:
			else if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 11) outputText("\"<i>Just look at these mommy melons!  You want to be an archer?  I'm amazed you don't slap yourself in the tits with every shot!  Easy target to grab onto.  But hey, I bet you like it that way.  Like it when people grope these fat titties of yours?</i>\"\r\r", false);
...
private function keltMainEncounterPostBlowjob():void {
	...
	if(player.statusAffects.has("BlowjobOn")) {
		...
		if (player.lust >= 75 || player.statusAffects.get("Kelt").value2 >= 90 && rand(2) == 0) {
			...
			//(Vagina:
			if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your poor, neglected pussy quivers with delight, convulsing without even being touched.  Thick juices run freely down your leg, dripping off to splatter the hay below you as you moan like a bitch in heat.", false);
			//(Genderless: 
...
private function keltResistance():void {
	...
	if(player.faceType == FACE.HORSE || player.faceType == FACE.DOG) outputText("a low growl building in the back of your throat", false);
...
private function fuckKeltsShitUp():void {
	...
	if (player.faceType == FACE.SNAKE_FANGS && player.tongueType == TONUGE.SNAKE && player.lowerBody == LOWER_BODY.NAGA) {
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			...
			if(player.lowerBody.cockSpot.list[0].cockLength <= 10) {
				if(player.lowerBody.balls > 0) outputText("Your " + ballsDescriptLight() + " bump into his chin as your cock finds the back of his throat. ", false);
	...
	//Standard anti-kelt scene
		...
		if(player.gender == 0) {
			...
			if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLICK) outputText("tasting them experimentally.  ", false);
			...
			if(player.lowerBody.cockSpot.list[0].cockLength < 10) outputText("taking the entire thing easily as his tongue licks you to ", false);
			else if(player.lowerBody.cockSpot.list[0].cockLength < 25) outputText("taking the entire thing deep into his throat.  Apparently centaurs lack a gag-reflex, and you can see his throat stretched around you as you're brought to ", false);
