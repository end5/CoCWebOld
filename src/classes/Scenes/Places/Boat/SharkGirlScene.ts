...
public sharkGirlEncounter(exploreLoc:number = 0):void {
	...
	//Rowboat
	if(exploreLoc == 1) {
		Render.text("While rowing the boat across the lake you spy a shark fin heading your way.  Worried it might damage the small boat, you hastily row back to shore, jumping out of the boat.  The shark shows no signs of slowing, and the fin disappears just before coming ashore.  ", false);
	}
	//Lake
	else {
		Render.text("You wander around the sandy shores of the lake before raising your eyebrow at a strange sight: A solitary fin, circling around in the water. You take a step back in surprise when the fin suddenly starts barrelling toward the shore at inhuman speeds.  ", false);
	}
	Render.text("A grey blur bursts from the water and lands on the ground a few feet away from you.\n\n", false);
	Render.text("It's a woman – a peculiarly corrupted woman, with shiny grey skin, silver hair, and a fin positioned between her shoulder blades. She's wearing some rather revealing black swimwear. The girl looks up at you and grins widely, showing rows of knife-like teeth. \"<i>Wanna play? Heads up though, I play 'rough'!</i>\"\n\n", false);
	Render.text("You're fighting a shark girl!", false);
...
internal function sharkWinChoices():void {
	...
	//HP Win
	if(monster.HP < 1) {
		Render.text("The shark-girl falls, clearly defeated.", true);
	}
	//Lust win
	else {
		Render.text("The shark-girl begins masturbating, giving up on dominating you.  The sight is truly entrancing.", true);
		dynStats("lus", 15);
	}
	if(player.lust >= 33 && player.gender > 0) {
		Render.text("  Do you have your way with her or leave?", false);
...
internal function sharkLossRape():void {
	...
	//Genderless:
	if(player.gender == 0) {
		Render.text("You slump down in defeat, too ", false);
		if(player.HP < 1) Render.text("hurt ", false);
		else Render.text("horny ", false);
		Render.text("to fight on.\n\n", false);
		Render.text("The shark girl does a little victory dance, swaying her hips to and fro before moving over to you. She quickly removes your " + player.armorName + ", but her smile fades to a blank expression when she notices you lack any genitalia. \"<i>What the...</i>\" she mumbles, poking you in the groin. Finding you completely useless, she growls in frustration and stomps on your face in anger. The sudden pain makes you pass out.", false);
		cleanupAfterCombat();
		dynStats("tou", -2);
		return;
	}
	...
	if(player.lowerBody.vaginaSpot.hasVagina() && (player.totalCocks() == 0 || rand(2) == 0)) {
		...
		Render.text("The shark girl giggles and moves over to you, tugging at your " + player.armorName + "  impatiently. Her tail swishes around and smacks your " + assDescript() + ". \"<i>You're gonna make me very happy, you hear? Otherwise...</i>\" she opens her mouth wide and you see her fangs glinting menacingly in the light. You gulp hard and nod, bringing a smile from the shark girl.\n\n", false); 
		Render.text("Wasting no time, she removes her skimpy swimwear and your own gear.  ", false);
		//[if herm]
		if(player.gender == 3) Render.text("Seeing your " + cockDescript(0) + " puts a smile on the shark girl's face as she takes a firm grip on your erection. \"<i>Well, you're just full of surprises, aren't you? Maybe I'll give this bad boy a whirl sometime. For now though...</i>\"  ", false);
		Render.text("Her gaze drifts over to your " + vaginaDescript(0) + " and she licks her lips in delight. \"<i>Now that's what I'm looking for! Tell you what dear, you get me wet and I might just give you some pleasure too.</i>\"\n\n", false);
		
		Render.text("She roughly grabs you by the hair and pulls your face into her drooling cunt, your tongue instinctively probing into her. \"<i>Mmm... don't you dare stop licking you dumb bitch, if you know what's good for you,</i>\" she orders. You reply by speeding up your tongue work. You're a little ashamed to admit it, but her dominant command makes you feel rather hot and bothered.\n\n", false);
		Render.text("The shark girl eventually sighs happily and relaxes her grip on your hair, pulling your head away a few inches. \"<i>Not bad bitch, not bad. Now get on your back.</i>\" You obey your mistress's command and flop onto your back. A sense of joy fills you as she positions her crotch in front of your face and moves her own head between your legs. You quickly resume eating her out, and this time she joins in the feast. It's not too long before the two of you orgasm, spraying girl-cum onto each other's faces.\n\n", false);
		Render.text("The shark girl stands to leave and winks at you before diving back into the water. You eventually pass out from the exertion.", false);
