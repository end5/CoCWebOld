...
public sharkGirlEncounter(exploreLoc:number = 0):void {
	...
	//Rowboat
	if(exploreLoc == 1) {
		MainScreen.text("While rowing the boat across the lake you spy a shark fin heading your way.  Worried it might damage the small boat, you hastily row back to shore, jumping out of the boat.  The shark shows no signs of slowing, and the fin disappears just before coming ashore.  ", false);
	}
	//Lake
	else {
		MainScreen.text("You wander around the sandy shores of the lake before raising your eyebrow at a strange sight: A solitary fin, circling around in the water. You take a step back in surprise when the fin suddenly starts barrelling toward the shore at inhuman speeds.  ", false);
	}
	MainScreen.text("A grey blur bursts from the water and lands on the ground a few feet away from you.\n\n", false);
	MainScreen.text("It's a woman – a peculiarly corrupted woman, with shiny grey skin, silver hair, and a fin positioned between her shoulder blades. She's wearing some rather revealing black swimwear. The girl looks up at you and grins widely, showing rows of knife-like teeth. \"<i>Wanna play? Heads up though, I play 'rough'!</i>\"\n\n", false);
	MainScreen.text("You're fighting a shark girl!", false);
...
internal function sharkWinChoices():void {
	...
	//HP Win
	if(monster.HP < 1) {
		MainScreen.text("The shark-girl falls, clearly defeated.", true);
	}
	//Lust win
	else {
