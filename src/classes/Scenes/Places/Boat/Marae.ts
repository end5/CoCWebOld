...
public function encounterMarae():void {
	...
	if(player.findStatusAffect(StatusAffects.DungeonShutDown) < 0) {
		...
		if(player.findStatusAffect(StatusAffects.MetMarae) < 0) {
			...
			if(player.stats.cor > 66) outputText("The woman bellows, \"<i>Begone demon.  You tread on the precipice of damnation.</i>\"  The tree's eyes flash, and you find yourself rowing back to camp.  The compulsion wears off in time, making you wonder just what that tree-woman was!", false);
			//Explain the dungeon scenario
				...
				if(player.stats.lib + player.stats.cor > 80) outputText("Your eyes can't help but take note of the tree-woman's shapely breasts, and wonder if they feel like tits or wood.  ", false);
...
private function maraeBadEnd():void {
	...
	if(player.statusAffects.get("$1").value3 > 0) outputText("Any thought or need to drink Marble's milk vanishes from your mind and body.  ", false);
...
public function level2MaraeEncounter():void {
	...
	if(player.stats.cor < 33) outputText("Knowing all too well what kinds of horrors await", false);
	else if(player.stats.cor < 66) outputText("Knowing all too well what kinds of sexual escapades lurk ahead", false);
	...
	if(player.stats.spe < 50) outputText("dropping you flat on your ass", false);
	...
	if(player.perks.has("MaraesGiftFertility") || player.perks.has("MaraesGiftStud")) outputText("second ", false);
...
private function MaraeIIStageII():void {
	...
	if(player.gender == 2) {
		...
		else if(player.stats.cor > 50) outputText("indecision", false);
		...
		if(player.stats.cor < 33) outputText("struggle to ", false);
		else if(player.stats.cor < 66) outputText("work to ", false);
	...
	//Marae uses a tree-tentacle to 'milk' male PC's.  Oral or Vajayjay? Not sure.
		...
		if(player.lowerBody.cockSpot.count() == 2) outputText(" while your other dangles against her thigh", false);
		else if(player.lowerBody.cockSpot.count() > 2) outputText(" while your others dangle against her thighs", false);
		...
		if(player.lowerBody.cockSpot.count() == 2) outputText("  Another vine that may as well be the first's twin snakes from between the goddess' legs and jumps onto your " + cockDescript(1) + ".", false);
		else if(player.lowerBody.cockSpot.count() > 2) outputText("  More 'open' vines shimmy forth from between Marae's legs and jump up onto your " + Appearance.cockNoun(CockType.HUMAN) + "s.", false);
		...
		if(player.lowerBody.balls == 0) outputText("body", false);
	...
	//[Hermz]  Marae grows vinecawks for DP under her flowercunt and sexes.
		...
		if(player.upperBody.chest.hasFuckableNipples()) outputText(", slipping inside your cunt-like nipples with ease.  You moan into her ear at the unexpected penetration and shiver from the tingling shocks of pleasure in your " + breastDescript(0) + ".", false);
		...
		if(player.lowerBody.cockSpot.count() == 2) outputText(" while your other cock rubs over her surface", false);
		else if(player.lowerBody.cockSpot.count() > 2) outputText(" while your other cocks rubs over her surface", false);
		outputText(".\n\n", false);
		...
		if(player.lowerBody.cockSpot.list[0].cockThickness > 5 || player.cockArea(0) > 100) outputText("With how big you are, there's no way it should be able to fit, but her body isn't even distorted by your girth.  Perhaps she changed you to fit her?  You pull back and your thickness seems unchanged.  You shake your head to clear the unwelcome thoughts and ram yourself back into her.  Fucking is what's important.  ", false);
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			if(player.lowerBody.cockSpot.count() > 2) outputText("  Another ", false);
			else outputText("Your other ", false);
			outputText(Appearance.cockNoun(CockType.HUMAN) + " prods at her tight pucker, and with a slight adjustment, you're able to line it up.  It's wet!  Inch after inch slides in with incredible ease, violating her slippery butthole until you've completely double-penetrated her.", false);
		}
		outputText("\n\n", false);

		if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.WET) outputText("Sticky wetness glistens between your thighs", false);
		else if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) outputText("Drops of feminine arousal run down your thighs", false);
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("Her asshole is squelching and dripping from your " + cockDescript(1) + ", actually squirting more lubricant than her pussy from the violent fucking!  ", false);
		...
		if(player.lowerBody.balls > 0) outputText("rapidly contracting balls", false);
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("The " + cockDescript(1) + " in her ass spasms and explodes with its brother, glazing her slippery colon with a coating of syrupy spunk.  ", false);
		if(player.lowerBody.cockSpot.count() > 2) {
			outputText("Neglected but orgasming, ", false);
			if(player.lowerBody.cockSpot.count() > 3) outputText("the remainder of ", false);
			outputText("your ", false);
			if(player.lowerBody.cockSpot.count() > 3) outputText(multiCockDescriptLight(), false);
...
private function MaraePt2RoundIIIPrizes():void {
	...
	//[HERMS]
		...
		if(!player.perks.has("MaraesGiftFertility") && !player.perks.has("MaraesGiftStud")) {
			...
			if(player.perks.has("MaraesGiftFertility")) outputText("<b>(New Perk Gained: Marae's Gift - Fertility)</b>", false);
