...
public encounterMarae():void {
	...
	if(!player.statusAffects.has("DungeonShutDown")) {
		...
		if(!player.statusAffects.has("MetMarae")) {
			...
			//Explain the dungeon scenario
				...
				MainScreen.text("\"<i>You seem so surprised by me, Champion.   I suppose that is inevitable.  Your origin is not of Mareth, our land, and few save for the demons remember me,</i>\" says the tree.\n\n", false);
				MainScreen.text("You take a step back, amazed to find such a creature, apparently uncorrupted.  ", false);
				if(player.stats.lib + player.stats.cor > 80) MainScreen.text("Your eyes can't help but take note of the tree-woman's shapely breasts, and wonder if they feel like tits or wood.  ", false);
				MainScreen.text("Feeling a bit confused, you introduce yourself and ask her who she is.\n\n", false);
				MainScreen.text("\"<i>Me?</i>\" she asks, \"<i>I am the life-goddess Marae.  I am Mareth, for my roots touch every part of it.   Or I was.  Before THEY came.</i>\"\n\n", false);
				MainScreen.text("You suggest, \"<i>The demons?</i>\"\n\n", false);
				MainScreen.text("She nods and continues, \"<i>The demons were once a tribe of magic-blessed humans, living in the mountains.  They had everything they could ever want:  peace, love, and the power to change reality.  But they grew dissatisfied, as men often do, and craved more.  They began using their magics to alter their bodies, seeking greater pleasure than ever before.  In time they became obsessed with it.  I let them be, believing their folly to be limited to their own village.   I was wrong.  While I focused on preventing famines and ensuring peace between the other villages, the humans twisted themselves into something else, something demonic.  They gave up their souls, crystallizing it into a magical energy source.    Of course they could not be satisfied with consuming the power of their own souls.  They wanted more.  They always want more.</i>\"\n\n", false);
				MainScreen.text("You nod in understanding, paying close attention to the tree-goddess as she explains just how this realm has fallen so far, \"<i>They came pouring out of the mountains in a wave, picking off villages left and right.  I lent many villages my power, but none had the strength to stand alone, and none would band together, resentful of their racial differences as they were.   All were consumed, enslaved, or filled with corruption.   My people were cut off from me, either by their new tainted outlook or by the demons' own machinations.  I was able to hide a few places from the enemy's sight, but I do not know how long it will last.</i>\"\n\n", false);
				...
				MainScreen.text("She practically begs, \"<i>Please champion, you must help me.  The demons have a factory at the foot of the mountains.  It produces much of the fluid they use to taint me.  If you could find a way to shut it down, I... all of Mareth, might stand a chance.</i>\"\n\n", false);
				MainScreen.text("You nod, understanding.  She commands, \"<i>Now go, there is nothing to be gained by your presence here.  Return if you manage to close that vile place.</i>\"\n\n", false);
				if (player.stats.lib + player.stats.cor > 80) {
					MainScreen.text("You could leave, but the desire to feel her breast will not go away.  What do you do?", false);
		...
		//Second meeting
			...
			MainScreen.text("You approach Marae's tree, watching the goddess flow out of the tree's bark as if it was made of liquid.   Just as before, she appears as the top half of a woman, naked from the waist up, with her back merging into the tree's trunk.\n\n", false);
			if(player.stats.cor > 66) {
				MainScreen.text("She bellows in rage, \"<i>I told you, begone!</i>\"\n\nYou turn tail and head back to your boat, knowing you cannot compete with her power directly.", false);
				doNext(camp.returnToCampUseOneHour);
			}
			else
			{
				...
				if(player.statusAffects.has("MaraesQuestStart")) {
					MainScreen.text("Marae reminds you, \"<i>You need to disable the demonic factory!  It's located in the foothills of the mountain.  Please, I do not know how long I can resist.</i>\"", false);
					doNext(camp.returnToCampUseOneHour);
				}
				//If not
				else {
					...
					MainScreen.text("She practically begs, \"<i>Please champion, you must help me.  The demons have a factory at the foot of the mountains.  It produces much of the fluids they use to taint me.  If you could find a way to shut it down, I... all of Mareth, might stand a chance.</i>\"\n\n", false);
					MainScreen.text("You nod, understanding.  She commands, \"<i>Now go, there is nothing to be gained by your presence here.  Return if you manage to close that vile place.</i>\"\n\n", false);
					if (player.stats.lib + player.stats.cor > 80) {
						MainScreen.text("You could leave, but the desire to feel her breast will not go away.  What do you do?", false);
						simpleChoices("Boob", grabHerBoob, "", null, "", null, "", null, "Leave", camp.returnToCampUseOneHour);
	...
	//Dungeon inoperable
		...
		if(!player.statusAffects.has("FactoryOverload")) {
			...
			MainScreen.text("Marae pushes the pearl into your hand, and closes your fingers over it gently.  \"<i>Go now, there is still much to be done.  With luck we will not need each other again,</i>\" commands the goddess as she slips back into her tree.  ", false);
...
private grabHerBoob():void {
	...
	MainScreen.text("You reach forward to cop a feel.  The goddess' eyes go wide with fury as a massive branch swings down, catching you in the sternum.  It hits you hard enough that you land in your boat and float back a few feet into the water.  Nothing to do but leave and hope for another chance at her breasts...");
...
private runFromPervertedGoddess():void {
	MainScreen.clearText();
	MainScreen.text("You turn and run for the boat, leaving the corrupt goddess behind.  High pitched laugher seems to chase you as you row away from the island.");
	doNext(camp.returnToCampUseOneHour);
}
...
private maraeStealLethicite():void {
	...
	{
		MainScreen.text("You dart to the side, diving into a roll that brings you up behind the tree.  You evade the gauntlet of grabbing tentacles that hang from the branches, snatch the large gem in both arms and run for the beach.  You do not hear the sounds of pursuit, only a disappointed sigh.", false);
	...
	//(FAIL)
		...
		player.slimeFeed();
		MainScreen.text("You dart to the side, diving into a roll that brings you up behind the tree.  You try to slip by the gauntlet of grabbing tentacles, but fail, getting tripped and ensnared in them like a fly in a spider's web.  You are pulled up and lifted to the other side of the tree, where you are slammed against it.  The tentacles pull your arms and legs wide, exposing you totally and locking you into a spread-eagle position.  You cringe as Marae strides around, free from the confines of her tree.\n\n", false);
		MainScreen.text("\"<i>Awwww, what a nasty deceitful little ", false);
		if(player.gender <= 1) MainScreen.text("boy ", false);
		else MainScreen.text("girl ", false);
		MainScreen.text("you are.  You turn me into a steaming hot sex-pot, then have the nerve to come here and try to walk off with my lethicite, all WITHOUT fucking me?  Tsk tsk tsk,</i>\" she scolds, \"<i>I appreciate your ambition, but I can't just let a mortal walk all over me like that.  I'll be taking that,</i>\" she says as she grabs the crystal, and lugs it to the tree underneath you.  She strokes the wood surface lovingly, and a knot dilates until it forms a hole large enough to contain the lethicite.  Marae shoves it inside, and strokes the wood like a pet creature, humming while the bark flows closed, totally concealing the crystal.\n\n", false);
		MainScreen.text("\"<i>Now, that should keep it safe from swift little play-toys like yourself.  What you tried was a bold move, and I respect that; initiative is to be rewarded.   So I'll let you go, just like that,</i>\" she says, snapping her fingers for emphasis.\n\n", false);
		MainScreen.text("The tentacles lower you to the ground, but do not release you from their tight embrace.\n\n", false);
		MainScreen.text("Marae steps closer, playing her fingers softly up your thigh, \"<i>BUT, you'll leave with a little something extra.  A gift from the new goddess of fertile unions...</i>\"\n\n", false);
		//DICK
...
public level2MaraeEncounter():void {
	...
	MainScreen.text("You advance on the twisted, arboreal orgy with reluctant determination.  Marae wants you here, and there's no way back without dealing with the sex-intoxicated goddess.  Her long, languid moans make it easy to find her.  The delirious deity's arms are entwined through the roots of her trees while a green-patterned tentacle goes diving into the petal-lined entrance of her sloppy sex.  She cranes her neck back at the sound of your footfalls and asks, \"<i>", false);
	//(FORK HERE BETWEEN STOLE LETHICITE AND LAZY ASSHOLEZ)\"
	//(STOLE)
	if(player.hasKeyItem("Marae's Lethicite") >= 0) {
		MainScreen.text("Welcome back, sneak-thief.  What kind of " + player.mf("gentleman","lady") + " is offered sex and then ransacks a god's soul?  Honestly, that right-right theeeeeree-oh yeah right there-is true depravity.</i>", false);
	}
	//(FAIL-STOLE)
	else if(player.perks.has("MaraesGiftStud") || player.perks.has("MaraesGiftFertility")) {
		MainScreen.text("Hey there " + player.short + ".  I didn't think I'd manage to snag you again so soon.  Are you enjoying my gifts?  I've been feeling kind of lonely without anyone around here to play with.</i>", false);
	}
	//(Left Like a Bitch)
	else {
		MainScreen.text("Well, look who came back!  I thought you were too afraid of a good time to come up here and fuck around with little ol' Marae.   I was actually going to let you row away if you were too scared to come here.  No, don't even glance back now, I've changed my mind.</i>", false);
	}
	MainScreen.text("\"\n\n", false);

	MainScreen.text("Featureless white irises glare at you from the goddess' lust-lidded eyes.  She commands you, \"<i>Come here.  It's time for a ", false);
	if(player.perks.has("MaraesGiftFertility") || player.perks.has("MaraesGiftStud")) MainScreen.text("second ", false);
	MainScreen.text("dose of Marae's tender affections.</i>\"\n\n", false);
	//Incase something breaks
	doNext(playerMenu);
	//Cant fly?  Stuck for sex!
	if(!player.canFly()) {
		MainScreen.text("You don't see any escape!", false);
		doNext(MaraeIIStageII);
	}
	//Can fly?  Choice to run
	else {
		MainScreen.text("You don't think she's counted on your wings.  If you tried to fly you could probably get out of the reach of her tentacles in short order.", false);
...
private MaraeIIStageII():void {
	...
	//Marae uses a tree-tentacle to 'milk' male PC's.  Oral or Vajayjay? Not sure.
		...
		MainScreen.text(".  Cum boils out from your ", false);
		if(player.lowerBody.balls == 0) MainScreen.text("body", false);
		else MainScreen.text("balls", false);
		MainScreen.text(", but it's quickly devoured by Marae's tree-based tentacle-beast without a sound.  ", false);
		if(player.cumQ() > 500) {
			MainScreen.text("The thick bulges of spooge actually distort the vines", false);
			if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s", false);
			MainScreen.text(", letting you get the barest glimpse of white through the over-stretched tentacle-tube", false);
			if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s", false);
	...
	//[Hermz]  Marae grows vinecawks for DP under her flowercunt and sexes.
		...
		if(player.cumQ() < 500) MainScreen.text("The orgasm is so much more massive than normal, and you wonder if her magic enhanced it.  ", false);
		if(player.cumQ() > 1000) MainScreen.text("She's actually surprised when you keep fountaining more seed into her.  Her nipples start to squirt out the excess seed, but you keep cumming until she's squirted enough to soak herself with your jism.  ", false);
		MainScreen.text("The goddess' tentacles never let up during it all, and you have a belly that matches Marae's perfectly.\n\n", false);

		MainScreen.text("You slide out and slump over, utterly exhausted by the breeding session.  The goddess pulls her tentacles from your abused openings, marveling at the outflow of plant-spunk while you relax and pass out.  You feel her fold your hands around your belly to cradle the pregnant bulge, and then you're snoring contentedly.\n\n", false);
	}
	//ONWARD TO NUMBER 3
...
private MaraePt2RoundIIIPrizes():void {
	...
	if(player.gender == 1) {
		...
		//(NON STUD)
		else MainScreen.text("You might be a little sore.  I did some work to make sure you'll be a perfect breeding stud for me.  No tiny cum-shots for you!  You'll squirt out enough to knock up anyone, and I even touched up your seed so it'll get through most contraceptives.  Aren't I the nicest?</i>\"  ", false);
		//(CONTINUED)
	...
	//[HERMS]
		...
		if(!player.perks.has("MaraesGiftFertility") && !player.perks.has("MaraesGiftStud")) {
			...
			//(RANDOM 2)
				...
				MainScreen.text("You might be a little sore.  I did some work to make sure you'll be a perfect breeding stud for me.  No tiny cum-shots for you!  You'll squirt out enough to knock up anyone, and I even touched up your seed so it'll get through most contraceptives.  Aren't I the nicest?</i>\"  ", false);
				player.createPerk(PerkLib.MaraesGiftStud,0,0,0,0);
			}
		...
		//(HAZ STUD)
			...
			MainScreen.text("I can't believe I didn't think of this last time!  I made you such a great stud that I didn't think to make you just as good at popping out your own kids!  Well I went ahead and fixed that while you were sleeping.  Your womb is nice and fertile, and you'll pop out kids a LOT quicker than before.  We'll be repopulating everything in Mareth in no time!  Just be sure to knock up the girls and let the boys fuck your pussy, okay?</i>\"  ", false);
...
private MaraeIIFlyAway():void {
	spriteSelect(40);
	MainScreen.text("", true);
	MainScreen.text("You launch into the air and beat your wings, taking to the skies.  The tentacle-tree lashes at you, but comes up short.  You've escaped!  Something large whooshes by, and you glance up to see your boat sailing past you.  She must have hurled it at you!  It lands with a splash near the mooring, somehow surviving the impact.  You dive down and drag it back to the dock before you return to camp.  That was close!", false);
	doNext(camp.returnToCampUseOneHour);
}
}
}
