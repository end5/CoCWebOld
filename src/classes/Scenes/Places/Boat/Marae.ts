...
public function encounterMarae():void {
	spriteSelect(40);
	Render.text(images.showImage("marae-first-encounter"));
	Render.text("Like a hidden emerald jewel, a small island appears in the distance.  You wager that you're somewhere near the center of this lake.  How coincidental.   You row closer, eager to get out of the boat and stretch your " + player.legs() + ".  The rowboat grounds itself in the moist earth of the island, coming to a dead stop.   You climb out, noting that this island is little more than a raised mound of earth and grass, with a small tree perched atop its apex.  ", false);
	//Dungeon operational
	if(player.findStatusAffect(StatusAffects.DungeonShutDown) < 0) {
		...
		if(player.findStatusAffect(StatusAffects.MetMarae) < 0) {
			...
			//Explain the dungeon scenario
				...
				Render.text("She practically begs, \"<i>Please champion, you must help me.  The demons have a factory at the foot of the mountains.  It produces much of the fluid they use to taint me.  If you could find a way to shut it down, I... all of Mareth, might stand a chance.</i>\"\n\n", false);
				Render.text("You nod, understanding.  She commands, \"<i>Now go, there is nothing to be gained by your presence here.  Return if you manage to close that vile place.</i>\"\n\n", false);
				if (player.stats.lib + player.stats.cor > 80) {
					Render.text("You could leave, but the desire to feel her breast will not go away.  What do you do?", false);
		...
		//Second meeting
			...
			Render.text("You approach Marae's tree, watching the goddess flow out of the tree's bark as if it was made of liquid.   Just as before, she appears as the top half of a woman, naked from the waist up, with her back merging into the tree's trunk.\n\n", false);
			if(player.stats.cor > 66) {
				Render.text("She bellows in rage, \"<i>I told you, begone!</i>\"\n\nYou turn tail and head back to your boat, knowing you cannot compete with her power directly.", false);
				doNext(camp.returnToCampUseOneHour);
			}
			else
			{
				//If youve taken her quest already
				if(player.statusAffects.has("MaraesQuestStart")) {
					Render.text("Marae reminds you, \"<i>You need to disable the demonic factory!  It's located in the foothills of the mountain.  Please, I do not know how long I can resist.</i>\"", false);
					doNext(camp.returnToCampUseOneHour);
				}
				//If not
				else {
					...
					Render.text("She practically begs, \"<i>Please champion, you must help me.  The demons have a factory at the foot of the mountains.  It produces much of the fluids they use to taint me.  If you could find a way to shut it down, I... all of Mareth, might stand a chance.</i>\"\n\n", false);
					Render.text("You nod, understanding.  She commands, \"<i>Now go, there is nothing to be gained by your presence here.  Return if you manage to close that vile place.</i>\"\n\n", false);
					if (player.stats.lib + player.stats.cor > 80) {
						Render.text("You could leave, but the desire to feel her breast will not go away.  What do you do?", false);
						simpleChoices("Boob", grabHerBoob, "", null, "", null, "", null, "Leave", camp.returnToCampUseOneHour);
	...
	//Dungeon inoperable
		...
		if(player.findStatusAffect(StatusAffects.FactoryOverload) < 0) {
			Render.text("Marae smiles broadly at you, and steps free from her tree.  The lithe plant-goddess gives you a warm hug and a kiss on the cheek.\n\n", false);
			Render.text("\"<i>Thank you,</i>\" she says, breaking the hug and turning back to her tree, \"<i>The onslaught has lessened, and I feel more myself already.  Let me thank you for your heroic deeds.</i>\"\n\n", false);
			Render.text("She plunges a hand inside the tree and pulls out a small pearl.  \"<i>This is a pearl from the very depths of the lake, infused with my purity.  If you eat it, it will grant you my aid in resisting the lust and corruption of this land.</i>\"\n\n", false);
			Render.text("Marae pushes the pearl into your hand, and closes your fingers over it gently.  \"<i>Go now, there is still much to be done.  With luck we will not need each other again,</i>\" commands the goddess as she slips back into her tree.  ", false);
			inventory.takeItem(consumables.P_PEARL, camp.returnToCampUseOneHour);
			player.statusAffects.add(new StatusAffect("MaraeComplete",0,0,0,0)));
		}
		//Corrupt!
		else {
			Render.text("You approach Marae's tree and note that the bark of the tree is smooth, practically wet looking.  The goddess's form is already exposed, as she leans out from the trunk and blows you a kiss.   Her breasts look as if they've filled out quite a bit since your first meeting, jiggling teasingly with every motion she makes.\n\n", true);
...
private function grabHerBoob():void {
	clearOutput();
	Render.text("You reach forward to cop a feel.  The goddess' eyes go wide with fury as a massive branch swings down, catching you in the sternum.  It hits you hard enough that you land in your boat and float back a few feet into the water.  Nothing to do but leave and hope for another chance at her breasts...");
...
private function runFromPervertedGoddess():void {
	clearOutput();
	Render.text("You turn and run for the boat, leaving the corrupt goddess behind.  High pitched laugher seems to chase you as you row away from the island.");
	doNext(camp.returnToCampUseOneHour);
}
...
private function maraeBadEnd():void {
	spriteSelect(40);
	Render.text("", true);
	Render.text(images.showImage("marae-bad-end"));
	if(player.findStatusAffect(StatusAffects.MetCorruptMarae) < 0) Render.text("The goddess flows out of the tree, stepping away from it as a living woman, curvy and nude.\n\n", false);
	Render.text("She approaches you, her breasts swinging pendulously and dripping sap.   Mesmerized by her swaying mammaries, you watch until she mashes you into them with an enormous hug.  A hand traces down your chest to your groin", false);
	if(player.gender == 0) Render.text(" where it pauses in momentary confusion", false);
	Render.text(".  She giggles and presses your face into her one of her verdant nipples.  You open your mouth to accept the purplish-green bud, licking and suckling it, encouraging her sweet sap to flow into your hungry mouth.  She gushes fluids and pulls you tightly against her tits, crushing you with soft flesh.\n\n", false);

	Render.text("The sap inside you makes your throat and belly tingle warmly, as if you had taken a strong drink.   Her milk-sap flows so quickly that you have to gulp it down to keep up.  Tiny burps escape your mouth every now and then as you work to gulp down the tainted treat.  You feel happy and secure, nestled in the bosom of a lust goddess.  ", false);
	if(player.statusAffects.get("$1").value3 > 0) Render.text("Any thought or need to drink Marble's milk vanishes from your mind and body.  ", false);
	Render.text("Foggy euphoria seems to float into your mind, making it difficult to think of anything but emptying the nipple in front of you.   You feel the last few drops splash on your tongue before unnatural strength breaks the seal, yanking you away and forcing a fresh dripping tit into your lips.\n\n", false);

	if(player.lowerBody.cockSpot.count() == 0) {
		Render.text("A building sense of pressure grows in your groin, bulging the flesh of your crotch out.  You ignore it, focusing on suckling more of the sweet fluids from your goddess' breasts.   The warmth in your middle feels like it's dripping down into that new bulge, making it tingle with sensitivity.  You ignore it, and lash your tongue across the slippery nipple in your mouth, being rewarded with another warm blast of syrupy sap.   Your mind fills with an impenetrable haze of lust, overcoming any logic with thoughts of raw sex intermingled with animal desire. You're pulled back again by that unholy strength, fighting to get one last lick on that nipple.  Your " + player.legs() + " and " + hipDescript() + " shake with lust, driven mad by sweet desire.  Marae reaches down to cup your groin, and blinds you with intense sensation.  She guides your gaze down to a new appendage that's sprouted from your needy groin – <b>a tentacle dick</b>!  It wavers to and fro, coiling on itself and tasting the air like a snake.\n\n", false);
...
private function maraeStealLethicite():void {
	...
	Render.text("", true);
	//(SUCCESS)
	if((player.stats.spe > 35 && (rand(player.stats.spe/3 + 30) > 20)) || (player.stats.spe > 35 && player.perks.has("Evade") && rand(3) < 2))
	{
		Render.text("You dart to the side, diving into a roll that brings you up behind the tree.  You evade the gauntlet of grabbing tentacles that hang from the branches, snatch the large gem in both arms and run for the beach.  You do not hear the sounds of pursuit, only a disappointed sigh.", false);
	...
	//(FAIL)
		...
		player.slimeFeed();
		Render.text("You dart to the side, diving into a roll that brings you up behind the tree.  You try to slip by the gauntlet of grabbing tentacles, but fail, getting tripped and ensnared in them like a fly in a spider's web.  You are pulled up and lifted to the other side of the tree, where you are slammed against it.  The tentacles pull your arms and legs wide, exposing you totally and locking you into a spread-eagle position.  You cringe as Marae strides around, free from the confines of her tree.\n\n", false);
		Render.text("\"<i>Awwww, what a nasty deceitful little ", false);
		if(player.gender <= 1) Render.text("boy ", false);
		else Render.text("girl ", false);
		Render.text("you are.  You turn me into a steaming hot sex-pot, then have the nerve to come here and try to walk off with my lethicite, all WITHOUT fucking me?  Tsk tsk tsk,</i>\" she scolds, \"<i>I appreciate your ambition, but I can't just let a mortal walk all over me like that.  I'll be taking that,</i>\" she says as she grabs the crystal, and lugs it to the tree underneath you.  She strokes the wood surface lovingly, and a knot dilates until it forms a hole large enough to contain the lethicite.  Marae shoves it inside, and strokes the wood like a pet creature, humming while the bark flows closed, totally concealing the crystal.\n\n", false);
		Render.text("\"<i>Now, that should keep it safe from swift little play-toys like yourself.  What you tried was a bold move, and I respect that; initiative is to be rewarded.   So I'll let you go, just like that,</i>\" she says, snapping her fingers for emphasis.\n\n", false);
		Render.text("The tentacles lower you to the ground, but do not release you from their tight embrace.\n\n", false);
		Render.text("Marae steps closer, playing her fingers softly up your thigh, \"<i>BUT, you'll leave with a little something extra.  A gift from the new goddess of fertile unions...</i>\"\n\n", false);
		//DICK
		if(player.gender == 1 || (player.gender == 3 && rand(2) == 1))
		{
			Render.text("She extends a hand expectantly, watching with detached concentration while a tentacle lowers from the tree into her palm.   Hips swaying sexually, she advances, peeling back the tentacle's outer layer.  It opens up to reveal a wet, gummy mouth.  She giggles and bumps the opening against your ", false);
			if(player.lowerBody.cockSpot.count() > 1) Render.text("largest ", false);
			Render.text(cockDescript(0), false);
			Render.text("'s tip.  Immediately a powerful suction draws your " + cockDescript(0) + " inside the tentacle-maw, burying you up to the base in squirming pleasure.   Marae watches the plant go to work, squeezing teasingly until you orgasm.  It takes mere moments for the gifted tentacle to achieve its goal.  Your cum makes a tasty treat for the plant-beast, and it sucks and sucks until your body feels empty and drained.\n\n", false);
			Render.text("\"<i>Oh that simply won't do,</i>\" Marae whispers, cupping your ", false);
			...
			Render.text(", \"<i>You'll be my prized breeder.</i>\"  The sharp point of a fingernail presses against your taint, scratching the skin.  \"<i>Just one tiny change to make,</i>\" exhales the goddess.  Pain explodes at the base of your crotch as it feels like her fingernail impales you, penetrating inches into your flesh.  You thrash in agony as it reaches something sensitive inside you.  You black out from pain and the shock of watching your blood flow down her arm.\n\n", false);
			Render.text("You feel warm, enclosed in comfort and pleasure.  Is this heaven?  No, your head is throbbing and your eyes are closed... you open them and discover you're still lying at the base of the tree.  That greedy tentacle is still locked around your " + cockDescript(0) + " pinning it in the throes of orgasm.  You watch thick bulges of cum pump up the tentacle, evidence of a truly garguantuan fluid output.  It goes on and on, and you realize the pleasure ought to drive you mad.\n\n", false);
			Render.text("Marae steps into your field of view, and pulls the tentacle free.  Your " + cockDescript(0) + " twitches pitifully, blasting a few massive loads onto your belly as your orgasm withers and dies from lack of stimulation.\n\n", false);
		...
		//FEM)
			...
			Render.text("She extends a hand expectantly, watching with detached concentration while a tentacle lowers from the tree into her palm.  A swift slash of her free hand cuts your " + player.armorName + " free, exposing your ", false);
			if(player.gender == 0) {
				Render.text("hairless crotch.  She holds the tentacle back a moment and raises her free hand.  It begins to glow and shimmer as she points to your groin.  Warmth explodes in your crotch as a wriggling wet gash opens up - <b>your new vagina</b>.  ", false);
				player.createVagina();
				player.genderCheck();
			}
			else Render.text(vaginaDescript(0) + ".  ", false);
			Render.text("She guides the tentacle forwards, letting it brush your nether-lips.  Without any guidance from its mistress, the bulbous plant-member buries itself inside you, sliding in easily until it's pushing hard against your womb.  A quick blast of fluid sends cramps spasming up your gut, forcing your cervix to dilate.  It wastes no time, flowing into your unprotected womb.  As soon as it reaches the back of your womb, thick bulges begin sliding down the exposed portion of the tentacle.  It stretches you wide, almost painfully so, as they pass through your lips and work up your passage.  They begin exploding in your cunt, one after the other, cum-bombs bursting in your womb, filling you to the brink.  Your belly swells out, giving you the appearance of a pregnant woman.  Finished with its nasty work, the plant-prick pulls free leaving your puffy pussy lips slightly agape.  A small runner of a thick green substance slowly slides out.", false);
			player.cuntChange(20,true,true,false);
			Render.text("\n\n", false);
			Render.text("Marae winks, \"<i>Sorry about making you look so pregnant my dear, it's a necessary part of the process.  All that sloshing seed is going to flow right into your tender little mortal ovaries, and remake them.  You'll be so fertile just looking at a hard cock could knock you up!</i>\"\n\n", false);
			Render.text("She giggles at your expression of horror, \"<i>No, not literally, but it won't take much to make you a mommy, and you'll find the gestation to be quite a bit... shorter.  Now get out of here before I change my mind and lock in an orgasm for the rest of your life.</i>\"\n\n", false);
			Render.text("You are dropped from the tree, and with little choice, you waddle to your boat, doing your best to cover up your violated " + vaginaDescript(0) + ".", false);
...
public function level2MaraeEncounter():void {
	...
	Render.text("", true);
	Render.text(images.showImage("marae-second-encounter"));
	Render.text("While rowing about the lake", false);
	if(player.str > 70) Render.text(" with ease", false);
	Render.text(", a familiar island resolves itself in the center of the lake.  There's a familiar tree perched atop it, though the wriggling tentacles' silhouette against the sky are a cruel reminder that this island will no longer be the peaceful haven it once was.  ", false);
	if(player.stats.cor < 33) Render.text("Knowing all too well what kinds of horrors await", false);
	else if(player.stats.cor < 66) Render.text("Knowing all too well what kinds of sexual escapades lurk ahead", false);
	else Render.text("Worried about enjoying the pleasures of the island overmuch", false);
	Render.text(", you dip your oars into the water and yank the boat around.  Something \"thunk\"s off the bottom of the boat, making the wood shiver and spurring you to row with renewed vigor.  You churn the water with your frenzied rowing, but the island never seems to get any further away.   Frustrated by the definitive lack of progress, you fearfully peer over the edge of the old boat's hull.  \"<i>Well, that explains it,</i>\" you muse.\n\n", false);

	Render.text("Squirming roots are crawling over the boat's underside.  They dig into every crease and crevice, binding the vessel tightly in place.  Just as you start to ponder swimming for the shore, the boat shifts, ", false);
	if(player.stats.spe < 50) Render.text("dropping you flat on your ass", false);
	else Render.text("nearly dropping you flat on your ass before you catch yourself and sit down", false);
	Render.text(".  The old dinghy's cutting through the water with amazing speed, leaving foot-high waves in its wake.  Marae's island grows larger with each passing second, almost taunting you with your inability to get away.   By now the root-like vines have crept over the gunwales, and they wriggle at you, seemingly in warning.  Getting in the water is definitely not a good idea right now.	The bone-chilling scrape of sand on wood grates at your ears.  You've arrived.  There's no point in putting off the inevitable.  You straighten up your " + player.armorName, false);
	if(player.weaponName != "fists") Render.text(", adjust your " + player.weaponName + ",", false);
	Render.text(" and step out of the boat onto the small, sandy beach that rings the island.  The tentacles that dragged your boat ashore are gone, leaving the well-used vessel in pristine condition, or as close as any such water-craft can be.  Looking up, you behold the monstrous, demonic-tree that sprouts from the island's apex.   It has no leaves, only small, teat-like protrusions that sprout from some of the 'branches', which in truth have more in common with tentacles than plant-life.  The squirming mass of sexual shrubbery stays in constant motion, and its intertwined tentacles occasionally bulge and flex as they spurt thick, jism-like sap over one-another.\n\n", false);

	Render.text("You advance on the twisted, arboreal orgy with reluctant determination.  Marae wants you here, and there's no way back without dealing with the sex-intoxicated goddess.  Her long, languid moans make it easy to find her.  The delirious deity's arms are entwined through the roots of her trees while a green-patterned tentacle goes diving into the petal-lined entrance of her sloppy sex.  She cranes her neck back at the sound of your footfalls and asks, \"<i>", false);
	//(FORK HERE BETWEEN STOLE LETHICITE AND LAZY ASSHOLEZ)\"
	//(STOLE)
	if(player.hasKeyItem("Marae's Lethicite") >= 0) {
		Render.text("Welcome back, sneak-thief.  What kind of " + player.mf("gentleman","lady") + " is offered sex and then ransacks a god's soul?  Honestly, that right-right theeeeeree-oh yeah right there-is true depravity.</i>", false);
	}
	//(FAIL-STOLE)
	else if(player.perks.has("MaraesGiftStud") || player.perks.has("MaraesGiftFertility")) {
		Render.text("Hey there " + player.short + ".  I didn't think I'd manage to snag you again so soon.  Are you enjoying my gifts?  I've been feeling kind of lonely without anyone around here to play with.</i>", false);
	}
	//(Left Like a Bitch)
	else {
		Render.text("Well, look who came back!  I thought you were too afraid of a good time to come up here and fuck around with little ol' Marae.   I was actually going to let you row away if you were too scared to come here.  No, don't even glance back now, I've changed my mind.</i>", false);
	}
	Render.text("\"\n\n", false);

	Render.text("Featureless white irises glare at you from the goddess' lust-lidded eyes.  She commands you, \"<i>Come here.  It's time for a ", false);
	if(player.perks.has("MaraesGiftFertility") || player.perks.has("MaraesGiftStud")) Render.text("second ", false);
	Render.text("dose of Marae's tender affections.</i>\"\n\n", false);
	//Incase something breaks
	doNext(playerMenu);
	//Cant fly?  Stuck for sex!
	if(!player.canFly()) {
		Render.text("You don't see any escape!", false);
		doNext(MaraeIIStageII);
	}
	//Can fly?  Choice to run
	else {
		Render.text("You don't think she's counted on your wings.  If you tried to fly you could probably get out of the reach of her tentacles in short order.", false);
...
private function MaraeIIStageII():void {
	...
	//Marae uses a tree-tentacle to 'milk' male PC's.  Oral or Vajayjay? Not sure.
		...
		Render.text("You watch, standing stock-still and paralyzed with ", false);
		if(player.lust > 80) Render.text("lust", false);
		else if(player.stats.cor > 50) Render.text("indecision", false);
		else Render.text("worry", false);
		Render.text(".  Marae's advance seems like an inexorable march to your eyes, and before you have a chance to react, she's crushed against you in a full-body hug.  Her tongue digs into your mouth, rooting out your tongue and melting your resistance in an overwhelming, lust-powered assault.  You vision swims for a moment when she releases you and pulls back.  It's hard to focus with the busty, nude image of fertility beckoning you.  It makes " + sMultiCockDesc() + " strain to reach her, but she dances back with a knowing smile and says, \"<i>Not yet my eager little subject.  Let me undress you, THEN you can worship me.</i>\"\n\n", false);

		Render.text("Vines whip out, sliding under your " + player.armorName + ", undoing clasps, and removing it until you stand naked and exposed to Marae.  She smirks and crooks her finger at you in a 'come hither' gesture.  You lurch forward, as if pulled on an invisible string until you're standing inches away from her, your " + cockDescript(0) + " rubbing her belly", false);
		if(player.lowerBody.cockSpot.count() == 2) Render.text(" while your other dangles against her thigh", false);
		else if(player.lowerBody.cockSpot.count() > 2) Render.text(" while your others dangle against her thighs", false);
		Render.text(".  The goddess gives you a cruel smile, as if she knows something you don't, and she commands, \"<i>Worship me with your cum, champion.  Submit to your goddess and spend your fertile seed for her.</i>\"\n\n", false);

		Render.text("Marae extends her arms, and the twin tentacles crawl forward like snakes as they wriggle down her appendages.  You thrust your crotch forwards, presenting it to the goddess, and only wonder why for a brief second before you toss away the nagging, useless thought.  Why would anything but pleasuring the living goddess before you matter?  Her corruptive aura floats in the air, filling you with the desire to submit to your queen and obey her every whim.  " + SMultiCockDesc() + " twitches and starts to drip pre-cum, itching to fulfill your deity's desires.\n\n", false);

		Render.text("The tentacle on her right arm convulses, then splits open along four joints.  The tip folds open to reveal a pink, wriggling interior that promises pleasures mortal minds weren't meant to comprehend.  Meanwhile, while you're distracted by the eager plant-hole, the other tentacle slips behind you and climbs up your " + player.leg() + ", leaving a trail of slime in its wake.   It slides between your cheeks and prods at your " + assholeDescript() + ".  You jerk forwards in surprise, but Marae pushes your " + hipDescript() + " back, allowing it to work its way inside.", false);
		player.buttChange(12,true,true,false);
		Render.text("  The open plant-hole dives for your groin while you're distracted, hits your " + cockDescript(0) + " and devours it with a greedy sluuuuurp.", false);
		if(player.lowerBody.cockSpot.count() == 2) Render.text("  Another vine that may as well be the first's twin snakes from between the goddess' legs and jumps onto your " + cockDescript(1) + ".", false);
		else if(player.lowerBody.cockSpot.count() > 2) Render.text("  More 'open' vines shimmy forth from between Marae's legs and jump up onto your " + Appearance.cockNoun(CockType.HUMAN) + "s.", false);

		Render.text("You grunt and pump your hips, shameless as you give in to the squeezing, textured tentacle", false);
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s", false);
		Render.text(".  Cum boils out from your ", false);
		if(player.lowerBody.balls == 0) Render.text("body", false);
		else Render.text("balls", false);
		Render.text(", but it's quickly devoured by Marae's tree-based tentacle-beast without a sound.  ", false);
		if(player.cumQ() > 500) {
			Render.text("The thick bulges of spooge actually distort the vines", false);
			if(player.lowerBody.cockSpot.count() > 1) Render.text("s", false);
			Render.text(", letting you get the barest glimpse of white through the over-stretched tentacle-tube", false);
			if(player.lowerBody.cockSpot.count() > 1) Render.text("s", false);
			Render.text(".  ", false);
		}
		Render.text("A hot, slippery pressure touches something inside your " + assholeDescript() + " and makes you squirt even harder.  Marae's minions have found your prostate!  You grunt and groan, but the orgasm doesn't seem to stop.  The goddess teases, \"<i>What?  You didn't think I'd actually let you cum on me did you?</i>\"\n\n", false);

		Render.text("It doesn't matter, you're giving her what she wants.  You cum until your " + player.legs() + " give out and you're sprawled on your back, " + sMultiCockDesc() + " being milked of its seed by the slurping, cunt-tentacles.  Every time you start to come down, the one in your backdoor rubs you just right and forces out another load.  Marae steps over your " + player.face() + " and drops down, allowing you to lick the nectar that drips from her sensitive, flower-like folds while you cum.  It's sweet, potent, and refreshing.  It makes it easy to keep cumming but hard to stay awake, and your eyes roll back as you pass out from an overload of pleasure.", false);
	}
	//[Hermz]  Marae grows vinecawks for DP under her flowercunt and sexes.
		...
		Render.text("The goddess closes with deliberately slow, hip-swaying steps that make your " + cockDescript(0) + " tremble and leave no doubt that she's the goddess of fertility.  Marae giggles and wraps her arms around you, planting a firm, wet kiss on your lips while she undoes your " + player.armorName + ".  Your gear hits the ground with a dull thump, and then her taut green nipples are pressed against your own", false);
		if(player.upperBody.chest.hasFuckableNipples()) Render.text(", slipping inside your cunt-like nipples with ease.  You moan into her ear at the unexpected penetration and shiver from the tingling shocks of pleasure in your " + breastDescript(0) + ".", false);
		else Render.text(".", false);
		Render.text("  Hammering in your chest, your heart beats fast enough to flush your whole body when your " + cockDescript(0) + " manages to slip between Marae's legs.  It doesn't penetrate, merely trapping itself between her fluid-lubed thighs", false);
		if(player.lowerBody.cockSpot.count() == 2) Render.text(" while your other cock rubs over her surface", false);
		else if(player.lowerBody.cockSpot.count() > 2) Render.text(" while your other cocks rubs over her surface", false);
		Render.text(".\n\n", false);

		Render.text("You swoon, your head buzzing with desire for more of this buzzing goddess' embrace.  The urge to kneel before her and worship her cunt rocks you to the core, blasting away the last of your feeble resistance, but before you can do so, Marae grabs you by the chin and commands, \"<i>No my child.  You can serve me better by breeding.</i>\"  Her fingers pull apart the petals of her flowery fuck-hole while she continues, \"<i>Go ahead, put it inside.  I'll show you how to practice the new faith of Marae.</i>\"\n\n", false);

		Render.text("She's easy to push down into the soft grasses of the island, and her legs part to allow you better access.  Your " + cockDescript(0) + " doesn't need to be told what to do, and it slips into her waiting wetness as if it was made for her.  ", false);
		if(player.lowerBody.cockSpot.list[0].cockThickness > 5 || player.cockArea(0) > 100) Render.text("With how big you are, there's no way it should be able to fit, but her body isn't even distorted by your girth.  Perhaps she changed you to fit her?  You pull back and your thickness seems unchanged.  You shake your head to clear the unwelcome thoughts and ram yourself back into her.  Fucking is what's important.  ", false);
		else Render.text("She feels perfect.  A velvet vice of hot, slippery wetness clutches tightly around your " + cockDescript(0) + ".  It almost feels like it's actually gripping you, cradling your cock in her ambrosia-slicked box.", false);
		if(player.lowerBody.cockSpot.count() > 1) {
			if(player.lowerBody.cockSpot.count() > 2) Render.text("  Another ", false);
			else Render.text("Your other ", false);
			Render.text(Appearance.cockNoun(CockType.HUMAN) + " prods at her tight pucker, and with a slight adjustment, you're able to line it up.  It's wet!  Inch after inch slides in with incredible ease, violating her slippery butthole until you've completely double-penetrated her.", false);
		}
		Render.text("\n\n", false);

		if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.WET) Render.text("Sticky wetness glistens between your thighs", false);
		else if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) Render.text("Drops of feminine arousal run down your thighs", false);
		else Render.text("Trails of viscous feminine fluid leak from your " + vaginaDescript(0), false);
		Render.text(", reminding you of your unused femsex.  Marae grunts underneath you, and while at first you assume it's from the penetration, the prodding of two cock-like protrusions at your lusty holes corrects your misguided assumptions.  You pull back and begin to fuck her in earnest, and with each long rock back, you can see she's grown tentacles from underneath her ass, like two prehensile tails.  They push forwards and spear you, arresting your movement while you try to cope with the sudden stretching of two of your orifices.  Warmth radiates from the twin intruders along with a slippery fullness.  They're pumping something inside you that tingles and makes " + sMultiCockDesc() + " bounce and drip.", false);
		player.cuntChange(12,true,true,false);
		player.buttChange(12,true,true,false);
		Render.text("\n\n", false);

		Render.text("Marae laughs and teases, \"<i>If this is how you fuck it's no wonder I haven't met your children yet.  If you're going to be my disciple you need to fuck your partners hard until you're stuffing them with cum.  Then you need to do it again.  Alternatively you should be bouncing on their cock and milking it with your " + vaginaDescript(0) + " until your womb is packed so full you can't walk.  Now show me how you'll do it, or I might keep you here until you're properly trained!</i>\"\n\n", false);

		Render.text("You happily thrust forwards, ramming your " + cockDescript(0) + " into her cunt with such force that a wet slap echoes over the lake and her fluids splatter your abdomen.  ", false);
		if(player.lowerBody.cockSpot.count() > 1) Render.text("Her asshole is squelching and dripping from your " + cockDescript(1) + ", actually squirting more lubricant than her pussy from the violent fucking!  ", false);
		Render.text("In spite of the obscene amount of pleasure " + sMultiCockDesc() + " is getting, you focus on obeying your goddess, and you work the muscles in your " + vaginaDescript(0) + " and " + assholeDescript() + " to pleasure her tentacles.  Muffled sloshes and spurts reach your ears, and you realize just how successful your efforts are.  Marae's pinching her nipples and arching her back, and a moment later a wave of pleasure hits you upside the head with the force of a hammer-blow.\n\n", false);

		Render.text("Cum boils out of your ", false);
		if(player.lowerBody.balls > 0) Render.text("rapidly contracting balls", false);
		else Render.text("tentacle-squeezed prostate", false);
		Render.text(" and erupts into Marae's womb.  Your hips rock forward, grazing her cervix with your " + player.cockHead() + " to better fill her uterus.  ", false);
		if(player.lowerBody.cockSpot.count() > 1) Render.text("The " + cockDescript(1) + " in her ass spasms and explodes with its brother, glazing her slippery colon with a coating of syrupy spunk.  ", false);
		if(player.lowerBody.cockSpot.count() > 2) {
			Render.text("Neglected but orgasming, ", false);
			if(player.lowerBody.cockSpot.count() > 3) Render.text("the remainder of ", false);
			Render.text("your ", false);
			if(player.lowerBody.cockSpot.count() > 3) Render.text(multiCockDescriptLight(), false);
			else Render.text(cockDescript(2), false);
			Render.text(" does its best to coat Marae's thighs with whiteness.  ", false);
		}
		Render.text("The goop from inside you never seems to end, and you pump Marae's belly up with it until she looks a little pregnant.  ", false);
		if(player.cumQ() < 500) Render.text("The orgasm is so much more massive than normal, and you wonder if her magic enhanced it.  ", false);
		if(player.cumQ() > 1000) Render.text("She's actually surprised when you keep fountaining more seed into her.  Her nipples start to squirt out the excess seed, but you keep cumming until she's squirted enough to soak herself with your jism.  ", false);
		Render.text("The goddess' tentacles never let up during it all, and you have a belly that matches Marae's perfectly.\n\n", false);

		Render.text("You slide out and slump over, utterly exhausted by the breeding session.  The goddess pulls her tentacles from your abused openings, marveling at the outflow of plant-spunk while you relax and pass out.  You feel her fold your hands around your belly to cradle the pregnant bulge, and then you're snoring contentedly.\n\n", false);
	}
	//ONWARD TO NUMBER 3
...
private function MaraePt2RoundIIIPrizes():void {
	...
	//[Chickzillas]
		...
		if(player.perks.has("MaraesGiftFertility")) {
			Render.text("<b>(New Perk Gained: Marae's Gift – Buttslut)</b>", false);
			player.createPerk(PerkLib.MaraesGiftButtslut,0,0,0,0);
			player.ass.analWetness = 2;
		}
		else {
			Render.text("<b>(New Perk Gained: Marae's Gift – Fertility)</b>", false);
			player.createPerk(PerkLib.MaraesGiftFertility,0,0,0,0);
		}
	}
	//[HERMS]
		...
		if(!player.perks.has("MaraesGiftFertility") && !player.perks.has("MaraesGiftStud")) {
			...
			//(RANDOM 2)
				...
				Render.text("You might be a little sore.  I did some work to make sure you'll be a perfect breeding stud for me.  No tiny cum-shots for you!  You'll squirt out enough to knock up anyone, and I even touched up your seed so it'll get through most contraceptives.  Aren't I the nicest?</i>\"  ", false);
				player.createPerk(PerkLib.MaraesGiftStud,0,0,0,0);
			}
			Render.text("The entire time she was speaking, you were trapped in orgasm, milked by her tree with unthinking intensity.\n\n", false);

			Render.text("Breathless and panting, you give Marae a nod of thanks as her tentacles lower you back towards your equipment.  They plant you on shaky " + player.feet() + " and uncoil slowly, stroking your body as they depart.  They must like you.  You get dressed in a hurry, but neither Marae nor the tree are paying you any attention any more.   The boat isn't far, and as you're climbing into it the goddess calls out her goodbyes, \"<i>Thanks for visiting and giving my tree so much of your sperm!  Once its fruit is ready I might come plant one at your camp!  Bye now, and don't forget to have lots of sex!</i>\"\n\n", false);
			if(player.perks.has("MaraesGiftFertility")) Render.text("<b>(New Perk Gained: Marae's Gift - Fertility)</b>", false);
			else Render.text("<b>New Perk Gained: Marae's Gift - Stud)</b>", false);
		}
		//(HAZ BREEDER)
		else if(player.perks.has("MaraesGiftFertility")) {
			Render.text("I can't believe I didn't think to do this last time!  I mean, I spent so much time making you a great baby-birther that I didn't bother to make you a stud too!  I fixed that this time though – you'll be squirting huge loads that are sure to knock up any of the pretty girls out there.  It'll even punch its way through most birth-controlling herbs.  Aren't I nice?</i>\"  ", false);
		...
		//(HAZ STUD)
			...
			Render.text("I can't believe I didn't think of this last time!  I made you such a great stud that I didn't think to make you just as good at popping out your own kids!  Well I went ahead and fixed that while you were sleeping.  Your womb is nice and fertile, and you'll pop out kids a LOT quicker than before.  We'll be repopulating everything in Mareth in no time!  Just be sure to knock up the girls and let the boys fuck your pussy, okay?</i>\"  ", false);
...
private function MaraeIIFlyAway():void {
	spriteSelect(40);
	Render.text("", true);
	Render.text("You launch into the air and beat your wings, taking to the skies.  The tentacle-tree lashes at you, but comes up short.  You've escaped!  Something large whooshes by, and you glance up to see your boat sailing past you.  She must have hurled it at you!  It lands with a splash near the mooring, somehow surviving the impact.  You dive down and drag it back to the dock before you return to camp.  That was close!", false);
	doNext(camp.returnToCampUseOneHour);
}
}
}
