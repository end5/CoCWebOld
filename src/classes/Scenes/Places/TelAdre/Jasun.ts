...
public changingRoom():void {
	...
	//[Look Around]
	if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00181] == 0)
...
private meetJasun():void {
	...
	spriteSelect(33);
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00179] == 0) {
		flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00179]++;
		...
		if(player.gender <= 1) {
			...
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00179] = 0;
	...
	//Met Jasun but not boned!
	else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00179] > 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00180] == 0) {
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00181] == 0) {
			...
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00181]++;
		}
		//If you're a dude.
		if(player.gender <= 1) {
		...
		if(player.tone < 75 || player.armorName != "slutty swimwear") {
			...
			else if(player.armorName != "slutty swimwear") MainScreen.text("properly attired", false);
...
private meetJasun2():void {
	...
	if(player.str < 50) MainScreen.text("that you're pretty sure you couldn't have moved on your own at all ", false);
	...
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00181] = 1;
	//[Accept] [Turn Away]
...
private jasunSecks():void {
	...
	player.slimeFeed();
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00180] == 0) {
	...
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00180]++;
	player.orgasm();
	dynStats("sen", -1);
	doNext(camp.returnToCampUseOneHour);
}
