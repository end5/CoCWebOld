...
private function meetJasun():void {
	...
	//Met Jasun but not boned!
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00181] == 0) {
			Render.text("While you look around the changing room, one of the gym's other patron asks you if you're looking for Jasun.  Seeing your confusion, they describe the hunky shark guy you met before.  You nod and smile, happy to have learned his name.\n\n", false);
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00181]++;
		}
		//If you're a dude.
		if(player.gender <= 1) {
			Render.text("Jasun walks out of a stall, his thick and muscular figure still as glorious as the last time you set eyes upon it.  He glances your way and smirks.\n\n", false);
			Render.text("\"<i>Eyes to yourself, boy. Try to play any games and I'll tear you apart limb from limb. Now get out of here before I do it anyway.</i>\" The shark-morph snarls threateningly before turning to walk away.\n\n", false);
			Render.text("You doubt he'd have the nerve to murder someone here, but you feel like you've wasted your time.", false);
			doNext(camp.returnToCampUseOneHour);
			return;
		}
		...
		if(player.tone < 75 || player.armorName != "slutty swimwear") {
			Render.text("Jasun walks out of a stall, his thick and muscular figure still as glorious as the last time you set eyes upon it. As he turns to greet you, though, you notice that his smirk suddenly turns around to a sense of shock. He's noticed that you're not quite the same as you were before, and you know it. He pauses a moment before turning back toward the mirror and addressing you without eye contact.\n\n", false);
			
			Render.text("\"<i>" + player.short + ", you know, I still don't think you're quite ready to swim with me. That's fine, and it's entirely up to you, but I think today I should probably swim alone. You can come and visit me anytime, of course, maybe when you're ", false);
			if(player.tone < 75) Render.text("fit enough", false);
			else if(player.armorName != "slutty swimwear") Render.text("properly attired", false);
			else Render.text("a little more compatible with me", false);
			Render.text(", we can go swim.</i>\" He hesitates for a moment at the mirror, and then walks past you quickly without making eye contact. Feeling dejected and suddenly very alone, you ignore the sensation caused by his skin brushing against yours and walk back to the gym's entrance. Maybe you can work yourself back into Jasun's graces with time.\n\n", false);
			//(-15 Lust, back to gym entrance)
			dynStats("lus", -15);
			doNext(camp.returnToCampUseOneHour);
			return;
		}
		Render.text("Jasun comes out of his stall and smiles when he sees you, already beginning to strain against the skimpy fabric of his bottom.  He asks, \"<i>Would you like to come swimming with me?</i>\" though his tone indicates that swimming may involve more than a few laps.", false);
	...
	//Repeat
		...
		if(player.gender <= 1) {
			Render.text("Jasun walks out of a stall, his thick and muscular figure still as glorious as the last time you set eyes upon it.  He glances your way and smirks.\n\n", false);
			Render.text("\"<i>Eyes to yourself, boy. Try to play any games and I'll tear you apart limb from limb. Now get out of here before I do it anyway.</i>\" The shark-morph snarls threateningly before turning to walk away.\n\n", false);
			Render.text("You doubt he'd have the nerve to murder someone here, but it's clear that so long as you lack a vagina Jasun will have no interest in you.  What a waste.", false);
			doNext(camp.returnToCampUseOneHour);
			return;
		}
		//IF YOU FAIL TO MEET ANY OF HIS CRITERIA NOW
