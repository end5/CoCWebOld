...
public function helpBrookeOut():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" and being so close to such a woman, you find it difficult to keep [eachCock] from getting too excited");
	outputText(".");
...
public function gropeDatBrooke():void {
	...
	//[if (hasCock = true)]
	if(player.lowerBody.cockSpot.hasCock()) outputText("  You can only imagine what it could feel like to drive your [cock] between those cheeks, just inches away from you.  The thought makes [eachCock] rouse.");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("[eachCock]");
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" and dragging her wet tail enticingly through the vulva of your [vagina]");
	}
...
public function penetrateBrooke():void {
	...
	if(player.lowerBody.balls > 0) outputText(" across your [sack] and");
	...
	//[if (cocks >= 3)
	if(player.lowerBody.cockSpot.count() > 1 && y >= 0) outputText(" while [eachCock] sprays your jizz all over the place: the floor and the walls of the small stall in particular, but also on her ass, onto her tail, and down her legs.  Most of it washes into the drain beneath you, but it'll definitely take a bit of scrubbing for her to get really clean after this.");
	//[if (cocks = 2)
	if(player.lowerBody.cockSpot.count() == 2 && y >= 0) outputText(".");
...
public function brookeAnal():void {
	...
	//[if {first time}]
	if(flags[FlagEnum.TIMES_IN_BROOKES_BUTT] == 0) outputText("  You wonder – is this the first time she's ever had anal before?");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText(" while your other member entertains itself between her thighs, bumping and sliding pleasurably across her vulva");
	...
	if(player.stats.cor >= 45) outputText("  With any luck, that someone will come to investigate – you're sure it wouldn't take much convincing to start a party.");

	outputText("\n\n<i>\"Harder!\"</i> she demands, thrusting her ass back onto you.  You're not sure how much harder you can go");
	if(player.lowerBody.cockSpot.count() >= 2) outputText(".  Any one of your dicks that's not inside her is battering painfully around her ass as she continues to hit you with it, making it harder to focus on fucking her this way");
	...
	if(player.lowerBody.balls > 0) outputText(" in your [balls]");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("  Your second dick flails and erupts with your " + cockDescript(x) + ", spraying onto the wall in front of Brooke and across the lips of her sex.");
	//[if (cocks >= 3)]
	if(player.lowerBody.cockSpot.count() > 2) outputText("  [EachCock] blasts in time with your " + cockDescript(x) + ", each of your dicks pointing in a different direction.  Your cum gets on everything: Brooke, yourself, the stall – some even reaches as high as the shower nozzle.");
	...
	//[if {first time}]
	if(flags[FlagEnum.TIMES_IN_BROOKES_BUTT] == 0) outputText("<i>\"I never knew anal could be such a thrill!  We'll have to do that again sometime!\"</i>  You chuckle politely but deep down you're almost dreading your next anal session with her.  You'll have to build up your stamina some more to keep up with her.");
...
public function tribadism():void {
	...
	if(!player.lowerBody.cockSpot.hasCock()) outputText(".  Brooke moves her hands up to your tits, her hands groping onto your [chest], her palms pressing onto your [nipples] mindlessly, and her tongue lolls and flags with each buck up against her you make");
	//[if {is not a herm and is lactating}
	if(!player.lowerBody.cockSpot.hasCock() && player.lactationQ() >= 200) outputText(".  Your milk streams from your [chest] as Brooke puts more of her weak pressure on them: it sprays from between her fingers and drips down the back of her hands on across her wrists");
	if(!player.lowerBody.cockSpot.hasCock() && player.upperBody.chest.hasFuckableNipples()) outputText(".  Feeling the texture of your [nipples] under her hands snaps her out of her reverie enough to think to stimulate you further by pressing into each with a thumb, fucking into and against you with each of your excited thrusts");
	outputText(".");
	if(player.lowerBody.cockSpot.hasCock()) outputText("  Brooke moves one hand to your [chest] and wraps her other around your stiff [cock], pumping it in time with your thrusts against her.");
...
public function goDownOnBrooke():void {
	...
	//[if (hasCock = true)
	if(player.lowerBody.cockSpot.hasCock()) outputText("  [EachCock] begins to fill with your desire, desperate to replace your fingers, and it takes no small amount of willpower to deny yourself.");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("and [eachCock] pulls forward, pointing towards her like a compass to magnetic north, leaking your pre at her scent and ready to fuck her, or something, anything.");
	...
	//[if (hasCock = true)]
	if(player.lowerBody.cockSpot.hasCock()) outputText("  [EachCock] has hardly gone unnoticed this whole while, still standing at attention and begging, absolutely pleading for some action.");
...
public function getLaidByBrooke():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("spearing your " + cockDescript(x) + " between her cheeks wickedly, feeling it rise effortlessly against her soft fur and the smooth, strong muscles of her glutes");
	//[if (hasCock = false)]
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("your " + cockDescript(x) + " continues to thrust and spear between her luscious ass, demanding attention be paid to it.  Brooke notices, and rocks her hips against you, squeezing your meat teasingly with each downstroke her ass makes on your " + cockDescript(x));
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("horndog");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("stroking along [eachCock], hissing inwardly at what she finds, loving the way her fingers, hurt as they are, just manage to glide across your skin");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" [eachCock], now fully erect and at attention, just for her");
	...
	if (player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.count() >= 3) outputText(" and all over the rest of her");
		outputText(".");
		...
		if(player.lowerBody.cockSpot.count() <= 2) outputText("enjoying the lips of your canine lover as they slide over [eachCock] and how she seems to enjoy herself so thoroughly");
		else if(player.lowerBody.cockSpot.count() >= 3) outputText("enjoying how she tries her damndest to make sure every little bit of you has enough attention and is pleased to the best of her ability, no matter how much effort she requires");
		outputText(".");
		if(player.gender != 3) outputText("\n\n");
			...
			if(player.lowerBody.balls > 0) outputText("burying her muzzle underneath your [sack] to push your [balls] out of her way, ");
		...
		//[if (cocks >= 3)]
		if(player.lowerBody.cockSpot.count() >= 3) outputText(".  Her hands are practically a blur, up and down on [eachCock] – she's pulled them each down and towards her, aiming them towards their proper target");
		//[if (isHerm = true)]
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText(".  Her fingers are straight as can be, pistoning into you roughly while her thumb frigs against your [clit], and the feel of her enthusiastic fingers inside your [vagina], massaging and caressing your walls, has you seeing stars");
	...
	//{female scene}
		...
		if(player.stats.cor <= 49) outputText("  That would almost be embarrassing enough to turn you off.");
	...
	if(player.lowerBody.cockSpot.count() >= 3) outputText(", and letting the excess wash from her fur half-heartedly");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", snowballing what she hasn't swallowed quite yet");
...
public function doubleDicked():void {
	...
	if(flags[FlagEnum.TIMES_IN_BROOKES_BUTT] > 0) outputText("  <i>\"Let's just hope this doesn't turn me any more into a buttfucking freak, huh?\"</i>");
	...
	if(player.lowerBody.cockSpot.count() >= 3) outputText("  Your other dicks rise as well, though not as quickly as the first two, jealous that they were chosen instead.");
	...
	//[if {first time anal}
	if(flags[FlagEnum.TIMES_IN_BROOKES_BUTT] == 0) outputText("it's certainly not a workout she's accustomed to,");
	...
	if(player.lowerBody.cockSpot.count() >= 3) outputText("The rest of you lamentably stands between you, leaking their pre uselessly between your bodies, pleading for some attention themselves.  ");
	...
	if(flags[FlagEnum.TIMES_IN_BROOKES_BUTT] == 0) outputText("Brooke's breathing slowly starts to even out, and she starts to moan enjoyably.  Maybe she's starting to like it?");
	...
	if(player.lowerBody.balls > 0) outputText(" down to your [balls]");
	...
	if(player.lowerBody.cockSpot.count() >= 3) outputText("  Your extra equipment, painfully erect, squashes between your bodies, pointing right at the underside of her boobs and your own [chest].");
	...
	if(player.lowerBody.cockSpot.count() >= 3) outputText("  Your [eachCock] sprays from between you, launching its own jizz up and uselessly between you, forming a warm, sticky blanket between your stomachs.  Some of it even crawls up between her cleavage, sticking them together from the inside, and with her pressed against you like this, it can't wash off right away.");
...
public function mediumAffectionOneTimeEvent():void {
	...
	//[if (hasCock = true)]
	if(player.lowerBody.cockSpot.hasCock()) outputText("  You feel [eachCock] beginning to wake from her proximity, and in just a few seconds, she'll know it.");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  Her thighs brush against your quickly rising [cock], and she's not even pretending it was an accident.");
...
public function brookeSpecialMediumSceneContinued():void {
	...
	if (x >= 0 && !player.lowerBody.vaginaSpot.hasVagina()) {
		...
		let y: number = player.cockThatFits2(brookeCapacity());
		if(player.lowerBody.cockSpot.count() == 2) outputText(" while your second phallus brushes higher, rubbing itself through the wet fur on her stomach between the gorge of her abs");
		//[if (cocks >= 3)
		if(y >= 0 && player.lowerBody.cockSpot.count() >= 3) outputText(" while [eachCock] finds themselves either driving themselves higher, through the wet fur on her stomach and across each of her tensed abs, or joining the first in between her legs, pressing and pulling with her needy humping");
		...
		if(player.lowerBody.balls > 0) outputText("[sack] resting gently against her crotch");
		...
		if(player.lowerBody.balls > 0) outputText("  Your [balls] swing away with you, slapping onto her pussy with every inward thrust.  You feel no pain, focused only on fucking this girl raw, knowing she's strong enough to take you.");
		if(player.lowerBody.cockSpot.count() == 2) outputText("  Your other dick swings freely, cold and alone in the shower between you, occasionally spurting pre into the air and onto Brooke's fur.  It's quickly washed away with the water.");
		if(player.lowerBody.cockSpot.count() >= 3) outputText("  Your other dicks swing freely, cold and alone in the shower between you, occasionally spurting pre into the air and onto Brooke's fur.  They're quickly washed away with the water.");
		...
		if(player.lowerBody.balls > 0) outputText("  Your [balls] begin to churn.");
		...
		if(player.lowerBody.cockSpot.count() == 2) outputText("  Your other prick tenses along with the first, pointing up and along the tight abs of the Shepherd girl, jizz erupting from the tip and blasting her on the underside of her perky tits.  If she even notices, she certainly doesn't seem to care.");
		if(player.lowerBody.cockSpot.count() >= 3) outputText("  [EachCock] spurts and flails along with the first, painting the stall white with your warm sperm.  It lands on the walls and the floor mostly, but more than a good deal lands on Brooke and into her fur.  She's so out of it from her own orgasm that she barely even notices.");
	...
	//{Herms only}
		...
		if(player.lowerBody.cockSpot.count() == 1) outputText("  You drill into that canine pussy, giving Brooke everything you have.  Your body burns and your cum churns the longer you go.  Her voice ascends from pitiful puppy whining to the jubilant cries of a woman well fucked, and the rise of her yells bring with them the rise of your cum.");
		if(player.lowerBody.cockSpot.count() == 2) outputText("  Your second penis swings, cold and alone, between your body and Brooke's as you continue to saw into her with your " + cockDescript(x) + ".  The tip points up to the cleft of her swinging, pounding breasts, ready to erupt and coat the underside of her tits with your offering.  She knows of it, and she does what she can to pleasure it, but even her well-defined six-pack can only do so much.");
		if(player.lowerBody.cockSpot.count() >= 3) outputText("  Unfortunately, as much as both you and Brooke want to pleasure as much of you as possible, she can only do so much.  [EachCock] thrusts into nothing between you, each of them stiff and pointed right up at her face – if they can't cum inside her, then they'll jealously paint her white on the outside.");
...
public function submitHeckelXBrookeThreesome():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("hard cock");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("engulfing your " + cockDescript(0) + " into her warm, busy mouth");
	if(player.lowerBody.cockSpot.count() == 2) outputText(" while she strokes your second dick with a free hand, wobbly balancing herself on her other, tired arm");
	if(player.lowerBody.cockSpot.count() >= 3) outputText(" while she does her best to please your [eachCock], massaging one with her fingers and shoving the others in her mouth, sometimes at once");
	if(player.lowerBody.cockSpot.hasCock()) outputText(".  Brooke and Heckel move at the same pace: the Shepherd girl, eager to please, does all she can with her mouth, sucking, bathing, and worshiping your tool, ");
	if(!player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) outputText("burying her muzzle into your [vagina] and immediately gets to work digging and searching your tunnel with her warm, moist, energetic tongue.  She licks, kisses, and worships everything from your vulva to your [clit] and when she gets bored of that, she thrusts her whole face into you once more, finding some new depth neither of you thought she could reach.  All the ");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  You might have taken offense to the idea you're not as good a fuck if you weren't too focused on the amazing blowjob Brooke's giving you.");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("you erupt, spraying your cum directly into Brooke's thirsty throat.  Despite her own ecstasy, she knows to swallow to avoid choking, taking hurried breaths through her nostrils when she can while you coat her mouth white several times over.  Her mouth and her tongue are mostly still, barely conscious enough to move much less pleasure you farther.  You don't mind, and you cum and cum into her mouth regardless.");
	...
	//[if (hasCock = true)]
	if(player.lowerBody.cockSpot.hasCock()) outputText("Based on how you know Brooke to be such a cum-hungry bitch when she's really into it, you doubt she'd rather have it any other way.  ");
...
public function brookeVHeckelBrookeWins2():void {
	...
	if(x >= 0) {
		...
		//[if (cocks >= 2)
		if(player.lowerBody.cockSpot.count() >= 2) outputText("  Your extra dick flops lamely up the crack of her ass, hotdogging between her cheeks with every thrust.  You consider the possibility of maybe double-dicking the cocky hyena, but you opt against it, not willing to break the good flow you've got between the three of you.");
		...
		if(player.lowerBody.cockSpot.count() == 2) outputText(".  Your second dick flails and pulses, ejaculating in time with its lucky brother, shooting up and coating Heckel's back with your jism");
		if(player.lowerBody.cockSpot.count() >= 3) outputText(" while [eachCock] pumps yet more of your virile jism every which way, staining the tile floor and tagging the lockers surrounding you with your cum");
		outputText(".");
	}
	//{Females only}
	else {
...
public function brookeVHeckelBrookeWins3():void {
	...
	if(!player.lowerBody.cockSpot.hasCock()) outputText(", muffled from your [vagina] still plugging her lips");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", and gently lay her down on her back, with Brooke awkwardly laying herself down with her");
