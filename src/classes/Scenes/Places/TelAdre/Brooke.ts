...
public function meetBrookeFirstTime():void {
	...
	if (player.isNaga()) Render.text("coil your tail nervously under yourself");
	else Render.text("shuffle your [feet]");
	Render.text(", and say that you're just about to shower yourself.  She steps aside, dropping the towel off her head and wrapping it around her body (almost teasingly), allowing you to pass.  \"<i>Plenty of stalls to pick from,</i>\" she laughs, and you thank her, walking on and avoiding eye contact.");
...
public function lowAffectionBrookeMeeting():void {
	clearOutput();
	Render.text("You head your way back to the showers, feeling the sweet ache of your muscles as you easily find your way there.  Brooke is there, already rinsing the day's work out of her muscles.  She hears you coming in, and turns to give you a warm greeting, which you happily return.");
...
public function mediumLowBrookeAffectionOneTime():void {
	clearOutput();
	Render.text("Another good workout and another fine sheen of sweat to wash off.  The day hasn't been so bad so far, and you start to whistle to yourself as you head to the showers, dropping your [armor] off in a locker beforehand.  Glancing out one of the windows, you guess that Brooke ought to be there around this time of day; and as you approach, you hear the sounds of rushing water.  Sure, it could actually be someone else, but you have a feeling it's the Shepherd girl.");
...
public function helpBrookeOut():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text(" and being so close to such a woman, you find it difficult to keep [eachCock] from getting too excited");
	Render.text(".");
	...
	if(player.gender == 1) Render.text("not caring about the stirring in [eachCock] in the privacy of the stall");
	//[if {PC is female}]
	if(player.gender == 2) Render.text("feeling a fire stoke in your [vagina] even under the constant water");
	if(player.gender == 3) Render.text("your loins beginning to stir, each and every part of you enticed");
	Render.text(" as you replay the memory of the Shepherd girl's taut, firm ass swaying, just for you.");
...
public function mediumLowBrookeAffection():void {
	...
	if(brookeAffection() >= 40 && brookeAffection() <= 70 && flags[FlagEnum.BROOKE_MEDIUM_SCENE] > 0) addButton(3,"Grope",gropeDatBrooke);
}
...
public function mediumBrookeAffectionMeetingAfterSex():void {
	...
	addButton(2,"Her Bod",brookesRockinBod);
	if(brookeAffection() >= 40 && flags[FlagEnum.BROOKE_MEDIUM_SCENE] > 0) addButton(3,"Grope",gropeDatBrooke);
}

//[=Working Out=]
public function talkWithBrookeAboutWorkingOut():void {
	clearOutput();
	Render.text("You ask Brooke how she manages to work out, given her condition.  You can understand the legs and the washboard abs, but she's still packing more heat in her arms and shoulders than most people do in their bodies.");
	...
	if(player.gender == 1) Render.text("[eachCock]");
	else if(player.gender == 2) Render.text("your [vagina]");
	else Render.text("[eachCock]");
	Render.text(".");
...
public function talkToBrookeAboutGymFolks():void {
	clearOutput();
	Render.text("You ask about what she thinks of the other gym members.  Surely she's seen them as often as you have, given how frequently she visits the place.");
...
public function brookesRockinBod():void {
	clearOutput();
	Render.text("You say that you recall her talking about wanting to get buff since a young age.  She even got a dumbbell for one of her birthdays, you recall.  What pushed her to want to become such a pinnacle of modern-day fitness?");
...
public function gropeDatBrooke():void {
	clearOutput();
	Render.text(images.showImage("brooke-grope-her"));
	Render.text("Brooke has an amazing body and she knows it.  She consciously chose to flaunt it in front of you by asking you to 'wash her back'.  The tightness of her muscles; the upright perkiness of her breasts; the taut, flawless features of her thighs... you bet if you smacked her ass, it'd be like slapping clay.");
	//[if (hasCock = true)]
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  You can only imagine what it could feel like to drive your [cock] between those cheeks, just inches away from you.  The thought makes [eachCock] rouse.");
	Render.text("  The tension is beginning to get a little unbearable and your resolve wavers.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("[eachCock]");
		...
		Render.text("your pelvis");
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and dragging her wet tail enticingly through the vulva of your [vagina]");
	}
	Render.text(".  <i>\"I'm game.  Hope you're ready, though; you're gonna need more than a few laps around the track to keep up with me.\"</i>");
	...
	menu();
	if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(brookeCapacity()) >= 0) addButton(0,"Penetrate",penetrateBrooke);
	if(player.lowerBody.cockSpot.hasCock()) addButton(1,"Anal",brookeAnal);
	if(player.lowerBody.vaginaSpot.hasVagina()) addButton(2,"Tribadism",tribadism);
	addButton(3,"Go Down On",goDownOnBrooke);
	if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(brookeCapacity()) >= 0) addButton(4,"Get Laid",getLaidByBrooke);
	if(player.lowerBody.cockSpot.count() >= 2) addButton(5,"Double-Dick",doubleDicked);
}
...
public function penetrateBrooke():void {
	...
	//[if (cocks >= 2)]
	if(y >= 2) Render.text(".  Your " + cockDescript(y) + " rubs up and down the crack of her ass lamely, though the way she clenches against you with every inward thrust makes every sensation worth the effort");
	//[if (isHerm = true)
	if(player.gender == 3) Render.text(".  Your [vagina] pulses longingly, jealous of the attention you're giving to Brooke's own, and hungers for something to suffocate the fire built inside it");
	Render.text(".  She moans appreciatively with you some more, loving the feel of your " + cockDescript(x) + " in her.");
	...
	if(player.lowerBody.balls > 0) Render.text(" across your [sack] and");
	Render.text(" down your thighs that she's getting wet enough for the both of you all the while.");
	...
	Render.text(".  You're not too far behind her, and with just a few more pumps, you're shooting your load deep into her, pumping and fucking the whole time, making sure to get it in deep.");
	if(y >= 0) Render.text("  Your " + cockDescript(y) + " shoots its own load up and onto her lower back and the base of her tail, snug between the cheeks of her ass");
	//[if (cocks >= 3)
	if(player.lowerBody.cockSpot.count() > 1 && y >= 0) Render.text(" while [eachCock] sprays your jizz all over the place: the floor and the walls of the small stall in particular, but also on her ass, onto her tail, and down her legs.  Most of it washes into the drain beneath you, but it'll definitely take a bit of scrubbing for her to get really clean after this.");
	//[if (cocks = 2)
	if(player.lowerBody.cockSpot.count() == 2 && y >= 0) Render.text(".");
	if(player.gender == 3) Render.text("  Your [vagina] cums along with the rest of you, spurting your femcum down your legs, washing the inside of your thighs in your juices.  The combined pleasure between your genitals makes your knees buckle – if only you had something, or someone, fucking you too!");
...
public function brookeAnal():void {
	...
	Render.text(", leaning her ass back and trapping your " + cockDescript(x) + " between her asscheeks expertly.  You moan with pleasure while she glides her globes up and down, feeling your manhood rise and swell with each bob.  Your hands are already on her butt, clenching the skin hard together while you thrust in the space between her buttocks.  <i>\"Well... all right.  But only because I like you so much.\"</i>");
	...
	//[if {first time}]
	if(flags[FlagEnum.TIMES_IN_BROOKES_BUTT] == 0) Render.text("  You wonder – is this the first time she's ever had anal before?");
	else Render.text("  She's tight as a vice, but you know she can take you.  She's just getting herself ready.");

	Render.text("\n\nThe " + player.cockHead(x) + " of your " + cockDescript(x) + " rubs against her rosebud in anticipation");
	if(player.lowerBody.cockSpot.count() > 1) Render.text(" while your other member entertains itself between her thighs, bumping and sliding pleasurably across her vulva");
	Render.text(".  She instinctively clenches her ass, and you give her a moment to relax before you stroke her there again.  She loosens, and, almost teasingly now, you prod gently at her hole, implying but not acting just yet.  She takes deep breaths, working to steady herself.  <i>\"Okay,\"</i> she says, looking over her shoulder to you with a smile.");

	Render.text(images.showImage("brooke-gym-male-analbrooke"));
	...
	Render.text(", burying yourself into her voracious ass.  With her body climbs her voice: a few pumps more and her moans become wails, loud enough that you're sure someone else must have heard.");
	if(player.stats.cor >= 45) Render.text("  With any luck, that someone will come to investigate – you're sure it wouldn't take much convincing to start a party.");

	Render.text("\n\n<i>\"Harder!\"</i> she demands, thrusting her ass back onto you.  You're not sure how much harder you can go");
	if(player.lowerBody.cockSpot.count() >= 2) Render.text(".  Any one of your dicks that's not inside her is battering painfully around her ass as she continues to hit you with it, making it harder to focus on fucking her this way");
	Render.text(".  <i>\"<b>Harder!</b>\"</i> she yells, almost viciously.  You just keep going, hoping that she won't notice you can't fuck her ass any harder; your orgasm isn't much farther off as it is, and you hope that Brooke's isn't either.  Her voice continues to rise even as she continues to make unreasonable demands, until finally, her words taper off into simple screams, and she orgasms, the wall, her inner thighs, and her lower stomach splattering with her cum.  Her anus squeezes down on your member, making it all the more difficult to keep up brutalizing her ass.");
	...
	if(player.lowerBody.balls > 0) Render.text(" in your [balls]");
	Render.text("; your cock stiffens");
	if(player.gender == 3) Render.text("; your lonely pussy clenches at nothing");
	Render.text("; and you cum into her, blasting rope after rope of your jism inside her.");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("  Your second dick flails and erupts with your " + cockDescript(x) + ", spraying onto the wall in front of Brooke and across the lips of her sex.");
	//[if (cocks >= 3)]
	if(player.lowerBody.cockSpot.count() > 2) Render.text("  [EachCock] blasts in time with your " + cockDescript(x) + ", each of your dicks pointing in a different direction.  Your cum gets on everything: Brooke, yourself, the stall – some even reaches as high as the shower nozzle.");
	...
	//[if {first time}]
	if(flags[FlagEnum.TIMES_IN_BROOKES_BUTT] == 0) Render.text("<i>\"I never knew anal could be such a thrill!  We'll have to do that again sometime!\"</i>  You chuckle politely but deep down you're almost dreading your next anal session with her.  You'll have to build up your stamina some more to keep up with her.");
	else Render.text("<i>\"Thanks for putting up with me being such a bossy bitch, [name].  But it's your fault, you know.  You're the one who awakened the anal-hungry monster within me.\"</i>  You're aware of the beast you've created, you tell her, but truth be told, since your first session, it's been a bit easier to keep up with her.  She's just that good of a workout partner.");
...
public function tribadism():void {
	clearOutput();
	Render.text(images.showImage("brooke-gym-female-tribadism"));
	Render.text("You fondle her full breasts, wet and heavy from the shower water beating down on them.  She moans and leans into your touch as you squish and fondle her flesh, flicking and pinching at her nipples.  You bend your head down and start kissing at her neck – trying to avoid getting any fur sticking to your tongue.  She loves it, and drags her ass over your pelvis some more, humping against");
	//[if (isHerm = true)
	if(player.gender == 3) Render.text(" your [cock], impishly wishing it were put to use.  You have some other plans this time, though.");
	else Render.text(" your " + player.skinFurScales() + ".  The tone of her ass almost makes you wish you had a dick to fuck her with, but you're sure she'll give you a hell of a ride all the same.");
	...
	if(player.gender == 3) Render.text(" and all the way to the base of your now-straining, pre-leaking cock");
	Render.text(".");
	...
	if(player.tone >= 50) Render.text(", and she coos sexily, gripping and pulling at each of your toned, chiseled asscheeks, even giving one as hard a slap as her hand can manage");
	Render.text(".  You start to fuck against her in earnest, pressing and pumping, sliding and bucking against her, loving and living for the sounds and expressions she makes as you love her the way only another " + player.mf("woman","'woman'") + " can.");
	...
	if(!player.lowerBody.cockSpot.hasCock()) Render.text(".  Brooke moves her hands up to your tits, her hands groping onto your [chest], her palms pressing onto your [nipples] mindlessly, and her tongue lolls and flags with each buck up against her you make");
	//[if {is not a herm and is lactating}
	if(!player.lowerBody.cockSpot.hasCock() && player.lactationQ() >= 200) Render.text(".  Your milk streams from your [chest] as Brooke puts more of her weak pressure on them: it sprays from between her fingers and drips down the back of her hands on across her wrists");
	if(!player.lowerBody.cockSpot.hasCock() && player.upperBody.chest.hasFuckableNipples()) Render.text(".  Feeling the texture of your [nipples] under her hands snaps her out of her reverie enough to think to stimulate you further by pressing into each with a thumb, fucking into and against you with each of your excited thrusts");
	Render.text(".");
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  Brooke moves one hand to your [chest] and wraps her other around your stiff [cock], pumping it in time with your thrusts against her.");
	...
	if(player.gender == 3) Render.text(".  Your [cock] explodes along with its sister, spurting your cum high between your pressed bodies, coating the underside of your breasts and your stomachs, but mostly onto Brooke, who keeps your flow pointed towards her as she pumps you");
	else Render.text(".  Brooke continues to drive against you as she mauls your breasts, using whatever strength is in her fingers to grope what she can, eager to reach her own peak not far off");
	Render.text(".  Brooke's moans rise and climb in her own ecstasy, until she climaxes along with you, returning the favor and spraying your crotch with her fluids, coating your [vagina] with her own gift.");

	Render.text("\n\nYou continue to thrust against each other for a bit, but eventually you wind down");
	//[if (isHerm = true)]
	if(player.gender == 3) Render.text(" and [eachCock] goes limp");
	...
	if(player.gender == 3) Render.text(".  <i>\"Lucky for me, we won't always have to choose.\"</i>  She gently strokes your waning erection some more, releasing you when the [cockHead] touches down on her skin limply");
	Render.text(".");
...
public function goDownOnBrooke():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("and [eachCock] pulls forward, pointing towards her like a compass to magnetic north, leaking your pre at her scent and ready to fuck her, or something, anything.");
	else Render.text("and you find yourself salivating – such a viable, powerful specimen like herself has you wishing you had a dick to fuck her with.");
	...
	//[if (hasCock = true)]
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  [EachCock] has hardly gone unnoticed this whole while, still standing at attention and begging, absolutely pleading for some action.");
	else Render.text("  Your own [vagina] stands out, puffy and needy against the cold air, and it's certainly not gone unnoticed.");
	Render.text("  Brooke offers to return the favor, more than once, but you decline each time, knowing that if she did, it'd be a cycle between you two that would last for hours.  She understands, but promises you that she owes you one.");
...
public function getLaidByBrooke():void {
	...
	Render.text("You step closer into her, wrapping your left arm around her torso diagonally, so that your hand rests on her right hip, and you reach over her chest with your right hand, grasping her left boob.  You pull her into you, and she coos at the affection, enjoying your loving embrace, but doesn't say anything, allowing you to have your way with her for now.");

	Render.text("\n\nYou roll your hips into her tight, toned ass, ");
	if(player.lowerBody.cockSpot.hasCock()) Render.text("spearing your " + cockDescript(x) + " between her cheeks wickedly, feeling it rise effortlessly against her soft fur and the smooth, strong muscles of her glutes");
	//[if (hasCock = false)]
	else Render.text("and with some clever maneuvering, you manage to wiggle her tail between your legs, and you feel the soft fur and the smooth, powerful muscles of her tail brush against your [vagina], stimulating you easily");
	Render.text(".  You kiss and nip at her neck, knowing just what she likes, and she moans, knowing that you like hearing her.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("your " + cockDescript(x) + " continues to thrust and spear between her luscious ass, demanding attention be paid to it.  Brooke notices, and rocks her hips against you, squeezing your meat teasingly with each downstroke her ass makes on your " + cockDescript(x));
	...
	Render.text(".  <i>\"Excited?\"</i> she asks, grinning back at you.  You let your actions speak for themselves, and you hump against her butt some more, dragging out your pleasure.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("horndog");
	else Render.text("whore");
	Render.text(" sometimes, [name],\"</i> she says, turning herself around in your arms.  She reaches down between you, ");
	if(player.lowerBody.cockSpot.hasCock()) Render.text("stroking along [eachCock], hissing inwardly at what she finds, loving the way her fingers, hurt as they are, just manage to glide across your skin");
	else Render.text("stroking along your [vagina], grinning at how your lower lips just manage to wrap around her fingers like they were made for each other.  Her thumb drags across your [clit] intentionally, several times, and she smiles wider at every feminine grunt you make");
	Render.text(".  <i>\"Feeling a little pent up, sweet thing?\"</i> she asks innocently, her fingers working harder on your genitals.  <i>\"It's okay.  Brooke's in a giving mood.\"</i>");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text(" [eachCock], now fully erect and at attention, just for her");
	else Render.text(" your [vagina], loving how you can hardly resist fucking your hips at nothing in anticipation");
	Render.text(", and, licking her drooling lips, leans in.");

	//{male/herm scene}[if (cocks = 1)
	if (player.lowerBody.cockSpot.hasCock()) {
		Render.text(images.showImage("brooke-gym-male-getLaid"));
		...
		if(player.lowerBody.cockSpot.count() >= 3) Render.text(" and all over the rest of her");
		Render.text(".");
		...
		if(player.lowerBody.cockSpot.count() <= 2) Render.text("enjoying the lips of your canine lover as they slide over [eachCock] and how she seems to enjoy herself so thoroughly");
		else if(player.lowerBody.cockSpot.count() >= 3) Render.text("enjoying how she tries her damndest to make sure every little bit of you has enough attention and is pleased to the best of her ability, no matter how much effort she requires");
		Render.text(".");
	...
	//{female scene}
		...
		Render.text(images.showImage("brooke-gym-female-getLaid"));
		...
		if(player.stats.cor <= 49) Render.text("  That would almost be embarrassing enough to turn you off.");
		else Render.text("  You want this sexy bitch all for yourself, and you'll be damned if you're sharing.  At least not until you're done.");
		Render.text("  Brooke, though, giggles through her mouthful of quim and thrusts again, enjoying the strained efforts you're making to not cry out.  Her hand jills hard and fast against her own neglected pussy, and from her actions, she's maybe as close as you are.");
	...
	if(player.lowerBody.cockSpot.count() >= 3) Render.text(", and letting the excess wash from her fur half-heartedly");
	Render.text(".  Moments pass in a completely undecipherable blur to you, and when you can finally make sense of your surroundings, Brooke's standing up and has her tongue in your mouth.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text(", snowballing what she hasn't swallowed quite yet");
	Render.text(".  She cradles you in her strong arms until she's done, and you pull away from the wall, still a bit wobbly.  <i>\"Was it good for you too, sweet thing?\"</i> she asks.  You barely nod.");
...
public function doubleDicked():void {
	...
	Render.text("\"</i> she mentions, almost casually, while you continue to masturbate yourself on her.  <i>\"");
	if(player.isNaga()) Render.text("Twice the dick means twice the fun... but twice the work, too.  I'm up to the challenge, though, if you are.");
	else Render.text("Haven't actually seen anyone that's not half-snake sporting more than one.  Mind if I put 'em to the test?");
	Render.text("\"</i>  She rocks her hips a little higher, brushing the ring of her anus across your " + cockDescript(y) + " while driving your " + cockDescript(x) + " further across her labia.");
	//[if {not first time anal}
	if(flags[FlagEnum.TIMES_IN_BROOKES_BUTT] > 0) Render.text("  <i>\"Let's just hope this doesn't turn me any more into a buttfucking freak, huh?\"</i>");
	else Render.text("  Brooke hesitates just a bit as she feels the heat of your dick against her asshole.  <i>\"Well, first time for everything,\"</i> she says to herself.");
	...
	if(player.lowerBody.cockSpot.count() >= 3) Render.text("  Your other dicks rise as well, though not as quickly as the first two, jealous that they were chosen instead.");
	...
	//[if {first time anal}
	if(flags[FlagEnum.TIMES_IN_BROOKES_BUTT] == 0) Render.text("it's certainly not a workout she's accustomed to,");
	else Render.text("no matter how much she loves getting fucked in the butt, it's still not a usual thing for her,");
	Render.text(" and for every inch her ass takes, another simultaneously slides into her pussy, stuffing her fuller than she's ever felt.");
	...
	if(player.lowerBody.cockSpot.count() >= 3) Render.text("The rest of you lamentably stands between you, leaking their pre uselessly between your bodies, pleading for some attention themselves.  ");
	if(player.gender == 3) Render.text("Your [vagina] clenches in time with its neighbor – you can easily imagine what it might feel like to have two dicks in you at once.  You almost wish for it while you're waiting for Brooke's go-ahead.  ");
	Render.text("Brooke's face contorts, not in pain but in concentration, while she fidgets with her ass and lower body, adjusting to both of her holes being so totally full.  After minutes of her positioning and repositioning herself, her eyes finally meet yours.  <i>\"Okay,\"</i> she whispers, <i>\"but... a bit gentler than what we're used to, okay?\"</i>");
	...
	if(flags[FlagEnum.TIMES_IN_BROOKES_BUTT] == 0) Render.text("Brooke's breathing slowly starts to even out, and she starts to moan enjoyably.  Maybe she's starting to like it?");
	else Render.text("Brooke rocks her hips onto both of your cocks, alternating from taking one deeper than the other.  As much as she just loves sex in general, it's the alien feeling of being so full that keeps her from going any harder.");
	Render.text("  She gasps and jerks with your every thrust, and you grunt, shoving yourself");
	if(player.lowerBody.balls > 0) Render.text(" down to your [balls]");
	Render.text(" deep into her.  You can practically feel your two dicks rubbing against each other inside her with each thrust.");
	...
	if(player.lowerBody.cockSpot.count() >= 3) Render.text("  Your extra equipment, painfully erect, squashes between your bodies, pointing right at the underside of her boobs and your own [chest].");
	Render.text("  You fuck her a bit more forcefully, but not any faster, for both your sakes.  She moans into you: her body's adjusted to the feel, and she's more receptive of your actions.");
	...
	//[if (cumQuantity >= 1000)
	if(player.cumQ() >= 10000) Render.text(" – she's probably going to have trouble doing much of anything for a day or two until she 'digests' her load");
	Render.text(".  Wearily, she pulls herself away, just enough to look you in the eye, and, without a moment, leans in to kiss you, tongue and all.");
...
public function mediumAffectionOneTimeEvent():void {
	clearOutput();
	Render.text("You wander your way back into the showers, stopping once again at the lockers to deposit your [armor].  Right on time, you hear the tell-tale sound of rushing water just up ahead, and as usual, you see Brooke washing at her fur lightly in the first shower stall.");
	...
	//[if (hasCock = true)]
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  You feel [eachCock] beginning to wake from her proximity, and in just a few seconds, she'll know it.");
...
public function brookeSpecialMediumSceneContinued():void {
	...
	if (x >= 0 && !player.lowerBody.vaginaSpot.hasVagina()) {
		...
		let y: number = player.cockThatFits2(brookeCapacity());
		if(player.lowerBody.cockSpot.count() == 2) Render.text(" while your second phallus brushes higher, rubbing itself through the wet fur on her stomach between the gorge of her abs");
		//[if (cocks >= 3)
		if(y >= 0 && player.lowerBody.cockSpot.count() >= 3) Render.text(" while [eachCock] finds themselves either driving themselves higher, through the wet fur on her stomach and across each of her tensed abs, or joining the first in between her legs, pressing and pulling with her needy humping");
		Render.text(", and you can feel the heat, the need, pour from Brooke's pussy as she continues to grind on you.  You can hear her pant as she continues to kiss your neck, whimpering at your denial to penetrate her and moaning with each brush across her vulva.");
		...
		if(player.lowerBody.balls > 0) Render.text("[sack] resting gently against her crotch");
		else Render.text("crotch resting gently against hers");
		Render.text(", her fur lightly tingling the skin.");
		...
		if(player.lowerBody.balls > 0) Render.text("  Your [balls] swing away with you, slapping onto her pussy with every inward thrust.  You feel no pain, focused only on fucking this girl raw, knowing she's strong enough to take you.");
		if(player.lowerBody.cockSpot.count() == 2) Render.text("  Your other dick swings freely, cold and alone in the shower between you, occasionally spurting pre into the air and onto Brooke's fur.  It's quickly washed away with the water.");
		if(player.lowerBody.cockSpot.count() >= 3) Render.text("  Your other dicks swing freely, cold and alone in the shower between you, occasionally spurting pre into the air and onto Brooke's fur.  They're quickly washed away with the water.");
		...
		if(player.hairLength >= 1) Render.text(", running her fingers through your " + hairDescript());
		Render.text(".  <i>\"Fuck me, [name]!  Just like that!  Give me that hard dick!  Fuck me!\"</i>");
		...
		if(player.lowerBody.balls > 0) Render.text("  Your [balls] begin to churn.");
		Render.text("  Your heart races, and your " + cockDescript(x) + " pulls and tightens in her pussy.  With some more encouraging, demanding words, and with a little more activity on her part, you finally cum, giving Brooke everything she asked for.");
		...
		if(player.lowerBody.cockSpot.count() == 2) Render.text("  Your other prick tenses along with the first, pointing up and along the tight abs of the Shepherd girl, jizz erupting from the tip and blasting her on the underside of her perky tits.  If she even notices, she certainly doesn't seem to care.");
		if(player.lowerBody.cockSpot.count() >= 3) Render.text("  [EachCock] spurts and flails along with the first, painting the stall white with your warm sperm.  It lands on the walls and the floor mostly, but more than a good deal lands on Brooke and into her fur.  She's so out of it from her own orgasm that she barely even notices.");
	...
	//{Female only}
		...
		Render.text(images.showImage("brooke-gym-female-specialscene"));
		Render.text("You can barely keep up with her sheer, wanton desire as her hands grope your body and her tongue conquers your mouth.  You can tell from her squeezing that she's trying to lift your left leg up and across her hips, but, unable to grip or lift with her hands, all she can do is grunt in frustration as she instead takes to humping into you.  You do her a favor and wrap your leg around her anyway.");
	...
	//{Herms only}
		...
		Render.text("Your hands waste no time in feeling and groping all over your new canine lover's body, appreciating every nuance of the tight muscles you feel under her short fur.  Her strong shoulders; her thick upper-back; her taut lower-back; and her tight ass.  [EachCock] rises in the excitement, reaching up between your bodies, scratching itself across her fine belly fur.  Your [vagina] likewise quivers in excitement, readying itself, though in Brooke's ravaging passion, you wonder how you'll be able to satisfy all of yourself, if you even can.");
		...
		Render.text("  She finally drops her hands down, one wrapping and caressing carefully, diligently, around your " + cockDescript(x) + " while the other trails the cleft of your [butt], a single claw running between them and towards your taint.  She doesn't waste a moment when she finds your [vagina] instead of plain skin, apparently not unfamiliar with multitasking.");
		...
		if(player.lowerBody.balls > 0)
		{
			...
			{
				Render.text("  Your [sack] swings with every pump, slapping onto the firm skin of Brooke\'s vulva every time you bottom out.  You feel no pain, and in fact the smoothness of her fur around her genitals tickles the cum out of your [balls] all the faster.");
			}
			else
			{
				Render.text("  Your [sack] is to tight and smooth to swing, but it still slaps against the firm skin of Brooke\'s vulva every time you bottom out.  The smoothness of her fur around her genitals tickles the cum out of your [balls] all the faster.");
			}
		}

		if(player.lowerBody.cockSpot.count() == 1) Render.text("  You drill into that canine pussy, giving Brooke everything you have.  Your body burns and your cum churns the longer you go.  Her voice ascends from pitiful puppy whining to the jubilant cries of a woman well fucked, and the rise of her yells bring with them the rise of your cum.");
		if(player.lowerBody.cockSpot.count() == 2) Render.text("  Your second penis swings, cold and alone, between your body and Brooke's as you continue to saw into her with your " + cockDescript(x) + ".  The tip points up to the cleft of her swinging, pounding breasts, ready to erupt and coat the underside of her tits with your offering.  She knows of it, and she does what she can to pleasure it, but even her well-defined six-pack can only do so much.");
		if(player.lowerBody.cockSpot.count() >= 3) Render.text("  Unfortunately, as much as both you and Brooke want to pleasure as much of you as possible, she can only do so much.  [EachCock] thrusts into nothing between you, each of them stiff and pointed right up at her face – if they can't cum inside her, then they'll jealously paint her white on the outside.");
...
public function specialHeckelAndBrookeEncounter():void {
	clearOutput();
	Render.text("You stroll into the gym, looking for Heckel, but she's not at her usual routine, running laps around the gym.  The gym itself is a little bare; there aren't a lot of people using the machines dotted around the room.  Perfectly good and ready machines, barbells, bench-presses, et al sit ready and waiting for someone to test their mettle on them.");
...
public function leaveHeckelEncounter():void {
	clearOutput();
	Render.text("Well, whatever.  You came here looking for Heckel, and she's not here, as far as you can tell.  You turn around and leave the gym, without investigating the cause of the commotion.");
...
public function nopeOutofBroke():void {
	clearOutput();
	Render.text("You shake your head at Heckel, not feeling like playing second fiddle today.");
...
public function submitHeckelXBrookeThreesome():void {
	clearOutput();
	Render.text(images.showImage("brooke-gym-heckelbrooke3some"));
	Render.text("Once you snap out of it, you comply, hastily removing your [armor] and leaving you just as naked as her.  Although you never really agreed to something like this in the first place, you <b>did</b> come here to see Heckel for a reason, and seeing the two of them like they are <b>is</b> really hot...");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("hard cock");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	if(player.gender == 2) Render.text("wet, waiting vagina");
	Render.text(", too out of it from the sudden penetration to do as Heckel said.  In response, Heckel thrusts forward much harder, roughly shoving a good third of her dick into Brooke's cunt.  <i>\"<b>Now!</b>\"</i> she demands, pushing her skull forward into your crotch, and finally, Brooke gets the sense to obey.");

	Render.text("\n\nResting her forearms on the bench for support, Brooke lunges forward, ");
	if(player.lowerBody.cockSpot.hasCock()) Render.text("engulfing your " + cockDescript(0) + " into her warm, busy mouth");
	if(player.lowerBody.cockSpot.count() == 2) Render.text(" while she strokes your second dick with a free hand, wobbly balancing herself on her other, tired arm");
	if(player.lowerBody.cockSpot.count() >= 3) Render.text(" while she does her best to please your [eachCock], massaging one with her fingers and shoving the others in her mouth, sometimes at once");
	if(player.lowerBody.cockSpot.hasCock()) Render.text(".  Brooke and Heckel move at the same pace: the Shepherd girl, eager to please, does all she can with her mouth, sucking, bathing, and worshiping your tool, ");
	if(!player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) Render.text("burying her muzzle into your [vagina] and immediately gets to work digging and searching your tunnel with her warm, moist, energetic tongue.  She licks, kisses, and worships everything from your vulva to your [clit] and when she gets bored of that, she thrusts her whole face into you once more, finding some new depth neither of you thought she could reach.  All the ");
	Render.text("while the hyena behind her pounds into her cunt with an unusual ferocity, atypical of how you know her.  Whether Heckel is just unusually aroused or she's taking the whole 'dominant' role more personally with Brooke, you're unsure and uncaring, if it means getting this sort of action.");
	if(player.gender == 3) Render.text("  Brooke, knowing of your duality, releases your cock from her mouth and dips lower, snaking her tongue below you and running it deliciously over your feminine sex.  Despite her weak hands, she does absolutely everything she can with what reach she's given, and you lean back, sliding your ass forward a bit to give her better reach to everything you have.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  You might have taken offense to the idea you're not as good a fuck if you weren't too focused on the amazing blowjob Brooke's giving you.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("you erupt, spraying your cum directly into Brooke's thirsty throat.  Despite her own ecstasy, she knows to swallow to avoid choking, taking hurried breaths through her nostrils when she can while you coat her mouth white several times over.  Her mouth and her tongue are mostly still, barely conscious enough to move much less pleasure you farther.  You don't mind, and you cum and cum into her mouth regardless.");
	if(player.gender == 2) Render.text("you cum into her mouth, spraying your feminine juices onto her tongue and wherever else Brooke didn't have the sense to cover.  It gets into her mouth; onto her muzzle; and down her face.  You coat her from your end as your pleased cunny quivers and cums again and again.  Brooke, for her part, swallows whatever happens to land into her maw, but she barely even seems to notice whenever your happen to get her anywhere else.");
...
public function brookeVHeckelBrookeWins2():void {
	...
	if(x >= 0) {
		...
		if(player.gender == 3) Render.text("  With every thrust into Heckel, you're reminded of your own empty pussy and you idly wish there was a spare cock around to fill you up too.");
		...
		if (flags[FlagEnum.TIMES_FUCKED_HECKEL_BLOWJOB] > 0 || flags[FlagEnum.TIMES_FUCKED_HECKEL_ANAL] > 0)
		{
			...
			if (bj && anal)	Render.text("  Every time you met Heckel, you either went down on her, or had to take her dick up your ass – now, with the rare opportunity to return the favor, y"); 
			else if (bj && !anal) Render.text("  Every time you met Heckel, you went down on her - now, with the rare opportunity to return the favour, y");
			else Render.text("  Every time you met Heckel, you had to take her dick up your ass - now, with the rare opportunity to return the favour, y");
		}
		else Render.text("Y");
		Render.text("ou savor every thrust into her tight velvet tunnel.  She's tight, far tighter than you had expected, but she's definitely no virgin either.  You draw your hands down to her hips, gripping and pulling delightedly at her tight muscles as you work her cunt, whetting your " + cockDescript(x) + " with the fruit you've been forbidden all this time.");
		//[if (cocks >= 2)
		if(player.lowerBody.cockSpot.count() >= 2) Render.text("  Your extra dick flops lamely up the crack of her ass, hotdogging between her cheeks with every thrust.  You consider the possibility of maybe double-dicking the cocky hyena, but you opt against it, not willing to break the good flow you've got between the three of you.");
		...
		if(player.hasKnot(x)) Render.text("  Likewise with Brooke, you pound your knot against Heckel's quim again and again, refusing to be denied entry, and with your orgasm, her pussy relents, taking in every inch of your " + cockDescript(x) + " with you.");
		Render.text("  You cum and you cum, thrusting hard, making sure to drench every possible nook and cranny within Heckel's needy tunnel, seeding her deeply with your every thrust, and from how she coos and rocks in time with each of your pulses rather than her own as she does the same to Brooke, it's safe to say she enjoys it");
		//[if (isHerm = true)]
		if(player.gender == 3) Render.text(".  Your [vagina] pulses in time with your " + cockDescript(x) + ", spraying your feminine juices down the inside of your tired legs uselessly, lonely and longing for a cock of its own to take");
		if(player.lowerBody.cockSpot.count() == 2) Render.text(".  Your second dick flails and pulses, ejaculating in time with its lucky brother, shooting up and coating Heckel's back with your jism");
		if(player.lowerBody.cockSpot.count() >= 3) Render.text(" while [eachCock] pumps yet more of your virile jism every which way, staining the tile floor and tagging the lockers surrounding you with your cum");
		Render.text(".");
	}
	//{Females only}
	else {
