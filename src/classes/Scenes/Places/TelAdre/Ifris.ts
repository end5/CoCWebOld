...
public function approachIfris():void {
	...
	if(flags[FlagEnum.MET_IFRIS] == 0) {
		flags[FlagEnum.MET_IFRIS] = 1;
		Render.text("The curious, appraising gaze on her pretty face turns into a pleased smile as you walk over, and her clawed toes click gently on the floor as she takes the last few steps to meet you. As you open your mouth to speak she preempts you, those pretty, black-painted lips parting, a voice too husky and smooth to be up to any good greeting you.\n\n", false);
	
		Render.text("\"<i>Hello, darling. I'm so glad you decided to join me... You can call me Ifris.</i>\" She tilts her head to the side cutely, gazing up at you through her lashes.\n\n", false);
	
		Render.text("You give her your name, mouth suddenly feeling a little too dry for some reason as she gives you that sultry smile and lingering gaze.\n\n", false);
	
		Render.text("\"<i>It's nice to meet you. Are you here to use the machines, or were you just hoping to watch the pretty bodies working up a sweat?</i>\" she giggles cutely, two of her clawed fingers covering her mouth demurely.\n\n", false);
	}
	//2a-Repeat approach, no sex-
	else if(flags[FlagEnum.TIMES_FUCKED_IFRIS_BLOWJOB] + flags[FlagEnum.TIMES_FUCKED_IFRIS_LICKED] == 0) {
		Render.text("Ifris grins as you walk over again, the devil girl's dark tongue dabbing at her even darker lips as she all but coos up at you.\n\n", false);

		if(flags[FlagEnum.IFRIS_SHOWED_OFF] == 0) Render.text("\"<i>Hello darling... here to actually give me a show?</i>\"\n\n", false);
		else Render.text("\"<i>Hello darling... here to give another show? I did so enjoy your last performance.</i>\"\n\n", false);
	}
	//2b-Repeat approach, had oral-
	else {
		Render.text("Ifris grins as you walk over again, the devil girl's dark tongue dabbing at her even darker lips. A small shudder runs through you as you imagine her remembering your flavor...\n\n", false);

		Render.text("\"<i>Hello darling... so lovely to see you. Are you here to give me a show this time? Or do you think you want to try to beat your last record?</i>\"\n\n", false);

		Render.text("A bit confused, you ask her what record she's talking about, only to evoke a small, cute laugh from her.\n\n", false);

		Render.text("\"<i>How long you can lift that much before my attentions are too distracting, of course.</i>\"\n\n", false);
	}
	Render.text("(You could go ahead and work out while she watches, ask her to join you, or leave.)", false);
...
private function workOutForIfris():void {
	...
	if(player.fatigue > 70) {
		Render.text("There's no way you could work out as tired as you are.  Maybe you could come back to flirt with the demonic-looking girl during your next workout.", false);
		doNext(telAdre.gymDesc);
		return;
	}
	Render.text("You smile to the devil-looking-girl and tell her you're just here to get your work-out on.\n\n", false);

	Render.text("\"<i>Oh, don't mind me, then. Please, by all means.</i>\" She gestures to the bench-press, stepping back and watching you with a smile.\n\n", false);

	Render.text("\"<i>I hope you don't mind if I keep you company. I'd love to... see you in action.</i>\"\n\n", false);
...
private function askIfrisToJoinYou():void {
	...
	Render.text("You ask Ifris if she'd like to join you in some exercises. Her eyes glint mischievously, obviously finding unintended meaning in your words, and you can't help but blush.\n\n", false);

	Render.text("\"<i>That's sweet of you, darling... but I don't want to get in the way. I'll just wait my turn. Don't worry about me at all.</i>\"\n\n", false);

	Render.text("You shrug at her response, but there's something about the way she eyes you now...", false);
	//WORK OUT or SHOW OFF?
	simpleChoices("Work Out", liftWhileIfrisWatches, "Show Off", showOffForIfris, "", null, "", null, "", null);
}
...
private function liftWhileIfrisWatches():void {
	spriteSelect(28);
	flags[FlagEnum.IFRIS_SHOWED_OFF]++;
	Render.text("", true);
...
private function showOffForIfris():void {
	...
	if(player.str < 75) {
		...
		Render.text("\"<i>Oh well...</i>\" She sighs, clearly crestfallen. Cocking one fist on her hip, she gives your " + player.leg() + " a light pat as she leaves your presence, vanishing out the door within moments.\n\n", false);

		Render.text("Well, at least you had the good sense to stop before you hurt yourself...\n\n", false);
		doNext(camp.returnToCampUseOneHour);
		flags[FlagEnum.IFRIS_SHOWED_OFF] = 0;
		return;
	}
	//4b5-PC masculinity > 60, corruption > 75, has cow features-
	if(player.gender == 0 || (player.femininity < 40 && player.stats.cor > 75 && player.faceType == FACE.COW_MINOTAUR)) {
		Render.text("Ifris watches you for a moment as you move down to the bench, but her eyes clearly wander elsewhere now and then. The pleasant smile never leaves her pretty face, but it's clear she's distracted or even disinterested for some reason. Soon enough she turns to leave, a bored little sigh leaving her. Her hips sway with a sexy gait as though it were natural, though nothing about her seems particularly excited at the moment...", false);
		//Stat changes HERE!
		if(player.str < 90) dynStats("str", .5);
		if(player.tou < 40) dynStats("tou", .3);
		//Body changes here
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		//First time
		if(flags[FlagEnum.TIMES_FUCKED_IFRIS_BLOWJOB] == 0) {
	...
	//4b4-PC succeeds! Is female!-
		...
		//5b-Repeat oral scene, female-
			...
			Render.text("\"<i>Always a pleasure, darling. I can't wait to see you again soon...</i>\" And she's leaving you once more, her butt swaying more than usual, moisture leaking down her long, slender legs...", false);
		}
		flags[FlagEnum.TIMES_FUCKED_IFRIS_LICKED]++;
	}
	//Stat changes HERE!
	if(player.str < 90) dynStats("str", .5);
	if(player.tou < 40) dynStats("tou", .3);
...
private function ifrisDP():void {
	...
	Render.text("After five big pushes, the hands disappear, but only long enough for her light, toned body to climb atop your own, the oiled thighs sliding up your sweat-glazed form towards your groin.  Ifris coos, \"<i>Mmm... keep at it, my darling.  Fitness is its own reward...</i>\"\n\n", false);
	
	Render.text("You gasp at the pleasant sensation of her moist swimsuit against " + oMultiCockDesc() + " and start to push yourself, straining harder, grunting as you begin to feel the onset of fatigue.  Just before the next lift, Ifris squeezes two of your " + multiCockDescriptLight() + ", angling them straight up.  You pause in confusion a moment, until you feel wet lips on one and tight, oiled skin squeezing the other.  Like you've been charged by lightning, energy fills your body, and you push yourself to keep going.  As the bar rises, Ifris' pussy and asshole lower, squeezing the two lucky dicks in her hot, clenching innards.  Holding that position, you struggle to control your muscles and adjust to the exquisite pleasure.  The bar lowers slowly, but Ifris' holes levitate in the same motion, coming just up off your sensitive, hungry shafts.\n\n", false);

	Render.text("The demonic-looking woman's tortuous orifices keep you hard, battling through the tide of pleasure so that you can keep lifting, and in return receive even more sensation.  Your arms are starting to feel sore, and lifting such enormous quantities of mass is getting harder and harder.  Ifris still seems pleased with your progress, and she murmurs words of encouragement.  \"<i>So... strong... mmmhmmm, go on.  You can do it.</i>\"  Her pussy squeezes and ripples around the main dick while her sphincter clamps tight on the other, massaging you to compensate for the slower speeds.  Droplets of sweat drip onto you, mingling with your own exhausted beads and falling to the floor, both of you soaked with the stink of exercise and sex.\n\n", false);

	Render.text("Ifris leans down and twines her arms around your shoulders, resting her head in the nape of your neck, her dark tongue licking the salt from you " + player.skin() + ".  Your whole body is shaking, trapped between the desire to cave in to the crimson beauty's body and the need to support the machine's tremendous load.  Hot and trembling, your arms feel like they're melting into weak, burning rubber.  Ifris senses this, and she says, \"<i>Set it down and release... release for me, darling.</i>\"\n\n", false);

	Render.text("You set the machine's bar back in place, letting your worn-out limbs go slack across Ifris's body, your hands coming to rest on her tight, squeezable butt.  The two slick holes start a whole new wave of contractations, and without the distraction of the weights, you're unable to resist.  ", false);
	//(Knotting - req's 2 dogdicks!)
	if(player.dogCocks() >= 2) {
		Render.text("Both your canine cocks bloat with seed, the knot's distending and stretching so wide that Ifris couldn't pull off if she wanted.  A moment later, the twin dog-dicks explode pillars of alabaster cream, painting the truly knotted woman's innards white.  ", false);
		if(player.cumQ() >= 1000) Render.text("She groans and moans as she's filled, her body distending to deal with the unholy amount of seed your cocks can spew.  ", false);
		Render.text("Throughout the massive release, you're softly grunting, held immobile by your animalistic body's instincts as your peckers try their damnedest to impregnate your overly fit mate.  You're still cumming, but your knots are starting to deflate at last.  ", false);
	}
	//(Nodule growing, unholy demonocity)
	else if(player.demonCocks() >= 2) {
		Render.text("Both your demonic cocks explode inside Ifris, the nodules along each shaft swelling out, thickening to stroke and caress your oily lover's inner walls.  A moment later, the tainted tips explode out, spurting demon-spunk into both silky tunnels.  ", false);
		if(player.cumQ() >= 1000) Render.text("She groans and moans as she's filled, her body distending to deal with the unholy amount of seed your cocks can spew.  ", false);
		Render.text("At the same time, your hands squeeze hard on her butt-cheeks and your hips start to jack-hammer against her, slamming into her as if she was a worthless piece of meat for you to impregnate.  You shiver, feeling not entirely in control of yourself, the demonic influence of your tainted members robbing you of control.  ", false);
	}
	//Flaring - req's 2 horsecocks!)
	else if(player.horseCocks() >= 2) {
		Render.text("Both your equine cocks explode inside Ifris, their tips swelling outward into thick flares that plug her stuffed channels.  A moment later, the swollen tips unleash pillars of alabaster cream, painting everything above the flares white.  ", false);
		if(player.cumQ() >= 1000) Render.text("She groans and moans as she's filled, her body distending to deal with the unholy amount of seed your cocks can spew.  ", false);
		Render.text("Your hips begin to rock and thrust, and no matter what you try to do, you cannot seem to still your body, your animal instincts owning your waist as you inseminate your seductive, teasing lover's body.  ", false);
	}
	//(Tentacle pistoning + head-mushrooming)
	else if(player.tentacleCocks() >= 2) {
		Render.text("Both your plant-like pricks explode inside Ifris, the purplish tips swelling up inside her to disproportionate sizes.  A moment later, the vine-dicks explode out twin pillars of ropey seed, painting her inner walls with spunk.  ", false);
		if(player.cumQ() >= 1000) Render.text("She groans and moans as she's filled, her body distending to deal with the unholy amount of seed your cocks can spew.  ", false);
		Render.text("At the same time, both tentacle-shafts curl and piston, winding around in other in the space between your bodies, curling and thrusting to rub every inch of your sensitive, prehensile shafts against Ifris' pussy-walls.  You shake and shudder, feeling more like a tentacle beast than a " + player.mf("man","woman") + ".  ", false);
	}
	//(Anemone - stinging funtimes)
	else if(player.anemoneCocks() >= 2) {
		Render.text("Both your unusual, aquatic cocks explode inside Ifris, their shafts thickening as the prepare for release.  A moment later, they explode out twin pillars of alabaster cum, painting her inner walls with gooey spunk.  ", false);
		if(player.cumQ() >= 1000) Render.text("She groans and moans as she's filled, her body distending to deal with the unholy amount of seed your cocks can spew.  ", false);
		Render.text("At the same time, your stinging cilia go wild, pricking her inside and out, uncontrollably forcing aphrodisiacs into your partner's shuddering, orgasmic body.  The drugs force her to climb to whole new plateaus of pleasure, making her squeal with unrepentant pleasure.  You convulse and get off even more forcefully from using your strange members in such a way.  ", false);
	}
	//(Standard)
	else {
		Render.text("Both your cocks explode inside Ifris, submitting alabaster pillars to her request for your release.  The red-hued sexpot exhales a happy hum and rocks her hips back and forth, her oily, lubricated holes squeezing your dicks for every drop of delectable seed.  ", false);
		if(player.lowerBody.cockSpot.count() > 2) {
			Render.text("The ", false);
			if(player.lowerBody.cockSpot.count() == 1) Render.text("one unbound prick remaining paints the muscular woman's lower back with goo, but she doesn't seem to mind.", false);
			else Render.text("other, unbound pricks paint the muscular woman's lower back in goo, but she doesn't seem to mind.", false);
			Render.text("  ", false);
		}
	}
	//(ALL GO HERE - NOT NEW PG) 
	Render.text("She purrs, \"<i>Delightful,</i>\" into your ear and plants a wet kiss on you, surely leaving a dusky, black lip-print on your neck.\n\n", false);
	
	Render.text("The last few contractions work through you, emptying the last of your load inside Ifris and allowing you to feel utterly, completely at peace.  Sliding up, she lets your slowly-deflating cocks ", false);
	if(player.dogCocks() >= 2) Render.text("noisily pop free", false);
	else Render.text("slide", false);
	Render.text(" from her sperm-glazed snatch and anus.  Her body's warmth vanishes as she stands, and with a wink, she reattaches her bikini bottom, the tight garment moulding to her soaked camel-toe and holding in all of your leavings.  She walks away without another word, ", false);
	if(player.cumQ() >= 1000) Render.text("her body bloated with cum and sloshing audibly with every step", false);
	else if(player.cumQ() >= 500) Render.text("her belly bloated and gurgling with every step", false);
	else Render.text("her body double-stuffed with seed", false);
	Render.text(".  You slump back and breathe heavy, feeling like you've just run a marathon.\n\n", false);
	
	Render.text("It is some time until you gain the energy to rise.", false);
	fatigue(10);
	//Stat changes HERE!
