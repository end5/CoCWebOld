...
public function approachIfris():void {
	...
	//2a-Repeat approach, no sex-
		...
		if(flags[FlagEnum.IFRIS_SHOWED_OFF] == 0) outputText("\"<i>Hello darling... here to actually give me a show?</i>\"\n\n", false);
...
private function showOffForIfris():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(flags[FlagEnum.TIMES_FUCKED_IFRIS_BLOWJOB] == 0) {
			...
			if(player.lowerBody.balls > 0) outputText("them and gently squeezes the sack.\n\n", false);
			else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your lower lips and gently squeezes.\n\n", false);
		...
		//5a-Repeat oral scene, male/herm-
			...
			if(player.lowerBody.balls > 0) outputText(", her hand cupping your " + ballsDescriptLight(), false);
	...
	//4b4-PC succeeds! Is female!-
		...
		//5b-Repeat oral scene, female-
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("breasts", false);
...
private function ifrisDP():void {
	...
	//(Standard)
		...
		if(player.lowerBody.cockSpot.count() > 2) {
			...
			if(player.lowerBody.cockSpot.count() == 1) outputText("one unbound prick remaining paints the muscular woman's lower back with goo, but she doesn't seem to mind.", false);
