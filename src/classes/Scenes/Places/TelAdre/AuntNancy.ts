...
public function auntNancy(display:boolean = true):boolean {
	if(model.time.hours >= 6 && model.time.hours <= 14) {
		if(display) {
			...
			//[If MetNancy = 0]
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00263] == 0) outputText("  She", false);
			else outputText("  Aunt Nancy", false);
			outputText(" glances up at you and smiles as you enter", false);
			//[(if unmet)
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00263] == 0) outputText(", and you see that she has additional red eyes spread across her forehead", false);
			outputText(".", false);
		}
		return true;
	}
...
private function lightStuff():void {
	...
	//[If Intelligence >= 20]
	if(player.stats.int >= 20) outputText("  You realize that what look like almost-shoulder-length gloves over her hands must actually be shell-like portions of this exoskeleton.", false);
	outputText("\n\n", false);
...
private function timeForAuntNancySpiderCooch():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText(" and taking the opportunity to grope your " + player.allBreastsDescript() + " right through your " + player.armorName, false);
	outputText(".\n\n", false);
...
private function auntNancyPoonPartII():void {
	...
	//[Intelligence > 40]
	if(player.stats.int > 40) outputText("realize that the spit must contain some kind of chemical originally meant to still prey", false);
	...
	if(player.tailType > TAIL.NONE) outputText(" to the top of your tail", false);
	outputText(".\n\n", false);
	...
	if(player.lowerBody.balls > 0) {
		...
		if(player.lowerBody.ballSize < 15) {
			if(player.lowerBody.ballSize > 5 && player.lowerBody.ballSize < 15) outputText("taking one", false);
			else outputText("both of them", false);
			outputText(" into her mouth to soak in her saliva as she wraps and caresses ", false);
			if(player.lowerBody.ballSize <= 5) outputText("them", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.lowerBody.balls > 0 && player.lowerBody.cockSpot.hasCock()) outputText("lifts away your other \"<i>equipment</i>\" and ", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("Aunt Nancy rises up above you, still smiling with gentle sensuality, and reaches out.  ", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.cockSpot.count() >= 2) {
			if(player.lowerBody.cockSpot.list[y].cockLength >= 22) {
				...
				if(player.lowerBody.cockSpot.count() == 2) outputText("other cock ", false);
				...
				if(player.lowerBody.cockSpot.list[y].cockLength >= 30) outputText("letting its head slip into her mouth, where she sucks and licks at it hungrily, ", false);
				...
				if(player.lowerBody.cockSpot.list[y].cockLength >= 30) outputText(" and mouth", false);
				outputText(", is incredible.  Aunt Nancy coos happily, clearly enjoying the taste.\n\n", false);
			}
			//[If there are still dicks left over:]
			if(player.lowerBody.cockSpot.count() > 2) {
				 if(player.lowerBody.cockSpot.count() == 3) outputText("Another of her hands wraps itself around your other cock", false);
				 ...
				 if(player.lowerBody.cockSpot.count() > 3) outputText("s", false);
				 outputText(" for all ", false);
				 if(player.lowerBody.cockSpot.count() == 3) outputText("it's", false);
		...
		if(!titFucked || (titFucked && player.lowerBody.cockSpot.count() <= 2)) {
			...
			if(player.lowerBody.balls > 0) outputText("gently kneading and caressing your " + ballsDescriptLight() + " as the cum inside churns and boils and ", false);
			//[If the player has a vagina]
			else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("exploring the folds and creases of your " + vaginaDescript(0) + " and ", false);
	...
	//[If player has vagina but no dicks: (Nancy prefers dicks and will always choose them over lesbian sex)]
		...
		if(player.tailType > TAIL.NONE) outputText("tail before rising to your ", false);
...
private function auntNancyPoonPartIII():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		//[If PC has balls]
		if(player.lowerBody.balls > 0) outputText(ballsDescriptLight() + " tense, churning and tightening.", false);
		//Else:]
	...
	//[Else:]
		...
		//[If PC has a vagina:]
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText(vaginaDescript(0) + " and ", false);
