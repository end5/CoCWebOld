...
public bakeryuuuuuu():void {
	if(isEaster() && player.lowerBody.cockSpot.hasCock() && (Flags.list[FlagEnum.LAST_EASTER_YEAR] < date.fullYear || rand(20) == 0)) {
		Flags.list[FlagEnum.LAST_EASTER_YEAR] = date.fullYear;
		easterBakeSale();
		return;
	}
	if(rand(10) <= 1 && kGAMECLASS.shouldraFollower.followerShouldra() && player.gender > 0 && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00242] == 4) {
...
public ingredientsMenu():void {
	MainScreen.clearText();
	MainScreen.text("Also try our special ingredients in your own baking!\n");
	MainScreen.text("Fox Berry - 5 gems.\n");
	MainScreen.text("Ringtail Fig - 5 gems.\n");
	MainScreen.text("Mouse Cocoa - 10 gems.\n");
	MainScreen.text("Ferret Fruit - 20 gems.\n");
	menu();
...
private talkToBaker():void {
	MainScreen.clearText();
	MainScreen.text("The minotaur snorts as you approach him, but waves you into the kitchen.  \"<i>What?</i>\" he asks, patiently watching you.  \"<i>Want to hear about baking?");
	//(Maddie 1 completed)
	if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00242] >= 4) MainScreen.text("  Or you want special order?");
	MainScreen.text("</i>\"");
...
private talkAboutBrownies():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>Like our brownies?</i>\" the baker asks.  \"<i>Recipe been handed down from chef to chef for years.  Original maker invented it at an inn, for guests to carry in their lunchboxes.</i>\"");
...
private talkAboutCupcakes():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>Cupcakes take work,</i>\" the baker intones, tilting his long face downward.  \"<i>Need butter, sugar, flour, and eggs for batter; gotta mix long time and add stuff slowly.  Candied berries get cut up, put inside batter in little pieces.  Bake batter in a special pan.</i>\"");
...
private talkAboutMouseCocoa():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>Mouse cocoa comes from warm side of the lake, by forest border.  Like the name says, mouse people used to grow and eat a lot of it.  No mice left, though... hard to get now and expensive.  Have to buy it from the farmer at the lake; she sends out gathering parties.  Same one we get milk from.  Less and less every year... going to have to raise prices soon.  Ten gems for one handful, now.</i>\"");
	//[Yes][No]
	menu();
...
private buyCocoa():void {
	...
	if(player.stats.gems < 10) {
		MainScreen.text("You can't afford one of those!");
		menu();
		MainScreen.addButton(0,"Next",ingredientsMenu);
		return;
	}
	MainScreen.text("You pay ten gems for some cocoa.  ");
...
private buyFerretFruit():void {
	...
	MainScreen.text("You pay twenty gems for a single ferret fruit.  ");
...
private buyFig():void {
	...
	MainScreen.text("You pay five gems for a fig.  ");
	player.stats.gems -= 5;
	statScreenRefresh();
...
private talkBakeryMenu():void {
	...
	MainScreen.clearText();
	MainScreen.text("Who will you talk to?\n");
...
public nomnomnom(name:string,price:number):void {
	...
	if(player.stats.gems < Flags.list[FlagEnum.TEMP_STORAGE_PASTRY_PRICE]) {
		MainScreen.text("You don't have enough gems to order that!", false);
		//doNext(bakeryuuuuuu);
		menu();
	...
	if(Flags.list[FlagEnum.TEMP_STORAGE_PASTRY_NAME] == "eclair") {
		MainScreen.text("You hand over 10 gems and ask for the 'special eclair'.  The centaur working the counter smirks ", false);
		if(player.tallness <= 52) MainScreen.text("down ", false);
		else if(player.tallness >= 84) MainScreen.text("up ", false);
		MainScreen.text("at you gives pulls a cream-filled pastry from a box concealed behind the counter.  It's warm... so very warm, and you try to steady your hands as you walk off to towards a table, sniffing in deep lungfuls of its 'special' scent.  The first bite is heaven, sating a craving you didn't even know you had.  You can't stop yourself from moaning with delight as you drain every drop and finish off the sweet doughnut shell.  The minotaur goo is all over your fingers, but you don't mind licking them all clean.  With the lust now you now feel burning inside you, you even try to make a show of it.  Though you make a few ", false);
		if(player.femininity >= 75) MainScreen.text("males fill their pants", false);
		else if(player.femininity <= 25) MainScreen.text("females squirm", false);
		else MainScreen.text("other patrons squirm and fill out their pants", false);
		MainScreen.text(", none of them tries to make a move.  Pity.", false);
		dynStats("lus", (20+player.stats.lib/10));
		player.minoCumAddiction(10);
	}
	else {
		MainScreen.text("You hand over " + num2Text(Flags.list[FlagEnum.TEMP_STORAGE_PASTRY_PRICE]) + " gems and get your " + Flags.list[FlagEnum.TEMP_STORAGE_PASTRY_NAME] + ".  A moment later you're at a table, licking the sugary residue from your fingertips and wondering just how they make the food so damned good.", false);
		if(Flags.list[FlagEnum.TEMP_STORAGE_PASTRY_NAME] == "doughnuts") {
			MainScreen.text(player.modTone(0,2), false);
			MainScreen.text(player.modThickness(100,1), false);
			if(rand(3) == 0 && player.lowerBody.butt.buttRating < 15) {
				MainScreen.text("\n\nWhen you stand back up your " + ButtDescriptor.describeButt(player) + " jiggles a little bit more than you'd expect.", false);
			...
			MainScreen.text(player.modTone(0,1), false);
			MainScreen.text(player.modThickness(100,2), false);
			...
			MainScreen.text(player.modThickness(100,4), false);
			if(rand(2) == 0 && player.lowerBody.hipRating < 30) {
				MainScreen.text("\n\nAfter finishing, you find your gait has changed.  Your " + LowerBodyDescriptor.describeHips(player) + " definitely got wider.", false);
				player.lowerBody.hipRating += 2;
			}
		}
		else if(Flags.list[FlagEnum.TEMP_STORAGE_PASTRY_NAME] == "cupcakes") {
			MainScreen.text(player.modTone(0,4), false);
			if(rand(2) == 0 && player.lowerBody.butt.buttRating < 30) {
				MainScreen.text("\n\nWhen you stand back up your " + ButtDescriptor.describeButt(player) + " jiggles with a good bit of extra weight.", false);
			...
			if(rand(3) == 0 && player.lowerBody.hipRating < 25) {
				MainScreen.text("\n\nAfter finishing, you find your gait has changed.  Did your " + LowerBodyDescriptor.describeHips(player) + " widen?", false);
				player.lowerBody.hipRating++;
			}
		}
	}
...
public buySlutCake():void {
	...
	if(player.stats.gems < 500) {
		MainScreen.text("You don't have enough gems for one of those!", false);
		//doNext(bakeryuuuuuu);
		menu();
		MainScreen.addButton(0,"Next",checkBakeryMenu);
		return;
	}
	MainScreen.text("The minotaur chef emerges from the backroom bearing a box that contains your cupcake.  It's too big to scarf down immediately.\n\n", false);
...
private buyFoxBerry():void {
	...
	MainScreen.text("You pay five gems for a fox berry.  ");
	player.stats.gems -= 5;
	statScreenRefresh();
...
private easterBakeSale():void {
	MainScreen.clearText();
	MainScreen.text("You make your way to the bakery only to find that it's so full you can barely squeeze inside.  ");
	if(telAdre.rubi.rubiAffection() >= 40) MainScreen.text("An extremely busy Rubi can only manage a wave in your direction before going back to attending customers.  ");
	MainScreen.text("Seeing all of the holiday bustle hits you with a pang of homesickness, remembering times from Ingnam.  Shaking these feelings off, you make your way to the front of the queue determined to see what the fuss is about.  The normally absent minotaurus chef greets you, adding fuel to your notion that they are understaffed.");
...
private easterBakeSaleHelp():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text("Your [fullChest] bounce at the vigor of your movement.  ");
	MainScreen.text("Going over the egg like an elaborate puzzle with its secrets only limited by your ability to unlock them, you are delighted to feel a stir of movement from within.  The heat is leaving not only your hands, but the entire room now, bringing the bristling heat down until you're sure it's cooler in here than outside with the swarm of customers.");
	...
	//(if the pc is male)
	if(player.gender == 1) MainScreen.text("[eachCock] jumps to full hardness.");
	//(if the pc is female)
	else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("your nipples stiffening noticeably, while your [vagina] prepares for what's to come.");
	//(if the pc is a herm)
	else MainScreen.text("[eachCock jumping to full hardness, your nipples and [vagina] not far behind in getting ready for your encounter.");
...
private malesHelpOutWithEaster():void {
	...
	//(large cum vol)
	else if(player.cumQ() < 1000) MainScreen.text("Your cock spews out a significant amount of seed, filling your partners deepness quickly while a small volume shoots out with some force.  You are happy to see that she seems to have gained a little weight from your baby-batter.");
	//(very large cum vol)
	else if(player.cumQ() < 5000) MainScreen.text("Your cock spews into your partner's deepness, filling it almost instantly while a significant volume splatters out.  You are happy to see she seems to have gained a little weight from your baby-batter.");
	else MainScreen.text("Your cock opens like a river, streaming into your partner with such force that her belly distends.  A spew begins to erupt from her vagina, empting the significant amount she could not take on to the floor.  You are happy to see she has gained some weight from your baby-batter.");
	MainScreen.text("  It's about all you can do to get to the floor before passing out.  So much for helping.  In the back of your mind you picture the minotaur with a smug grin as your consciousness fades.");
	MainScreen.text("\n\n<b>Later...</b>");
