...
private function talkToBaker():void {
	...
	//(Maddie 1 completed)
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00242] >= 4) outputText("  Or you want special order?");
	outputText("</i>\"");
...
public function nomnomnom(name:string,price:number):void {
	...
	if(flags[FlagEnum.TEMP_STORAGE_PASTRY_NAME] == "eclair") {
		...
		outputText("You hand over " + num2Text(flags[FlagEnum.TEMP_STORAGE_PASTRY_PRICE]) + " gems and get your " + flags[FlagEnum.TEMP_STORAGE_PASTRY_NAME] + ".  A moment later you're at a table, licking the sugary residue from your fingertips and wondering just how they make the food so damned good.", false);
...
private function easterBakeSaleHelp():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("Your [fullChest] bounce at the vigor of your movement.  ");
	...
	//(if the pc is female)
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your nipples stiffening noticeably, while your [vagina] prepares for what's to come.");
	//(if the pc is a herm)
...
private function malesHelpOutWithEaster():void {
	...
	if(player.lowerBody.balls > 0) outputText(", sticky drops of chocolate trailing down your [balls]");
