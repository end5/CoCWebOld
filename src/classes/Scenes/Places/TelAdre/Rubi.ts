...
public function rubiCock(lust: number = 50):string {
	let cumQ: number = 300;
...
public function rubiGetCockType():CockType {
...
private function specialRelationship20scene():void {
	clearOutput();
	rubiSprite();
	Render.text("As you're leaving the bakery, you hear the clicking of heels behind you.  Turning back you see Rubi rushing towards you shouting, \"<i>Hey, wait a second!</i>\"  You do, and wait for her to catch up.  Stopping in front of you, she puts her hands on her dainty knees and catches her breath.");
...
private function acceptRubi():void {
	clearOutput();
	rubiSprite();
	Render.text("You wrap your arms around and pull him into a hug.  You declare that you like him no matter what sort of genitals he happened to be born with.");
...
private function rejectRubi():void {
	clearOutput();
	rubiSprite();
	Render.text("With your hands on his shoulders you carefully keep him at arm's length.  Putting it as delicately as you can, you tell him that you're just not interested in him <i>that</i> way, not anymore at least.");
...
internal function rubiIntros():Function {
	...
	Render.text("\n");
	//(Haven't met yet)
	if(flags[FlagEnum.RUBI_INTRODUCED] == 0) Render.text("A cute human girl moves from table to table, bussing the empty ones and serving drinks to the occupied.  She's pretty flat-chested, and wears a pink and white frilly waitress uniform with an apron.  When she turns her back to you, you can tell she's not <i>entirely</i> human, what with the long red spaded tail coming out from under her dress.");
	//(Have met, relationship 0-19)
	else if (rubiAffection() < 30) Render.text("Rubi is here, moving from table to table efficiently clearing the empty ones and serving new customers.  Her long red tail swishes back and forth, sometimes lifting her dress enough to get a peek at her panties.");
	//(Have met, relationship 40, Bimbo Rubi; cheating scene)
	else if (rubiBimbo() && flags[FlagEnum.RUBI_PROSTITUTION] == 0 && rand(9) == 0) {
		Render.text("Strangely, even though you know it's time for Rubi's shift, she's not here.\n");
		//(Rubi button still active, goes to \"<i>Cheating Rubi</i>\" scene)
		//(Have met, relationship 40, Bimbo Rubi)
		return cheatingRubi;
	}
	else if (rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) Render.text("Rubi is here, moving from table to table, not quite as efficiently as [rubi ey] used to, but with a lot more exuberance.  [rubi Eir] long red tail swishes around, seemingly drawn towards the crotch of patrons and co-workers alike.");
	else Render.text("Rubi is here, moving from table to table efficiently clearing the empty ones and serving new customers.  [rubi Eir] long red tail swishes back and forth excitedly.");
	Render.text("\n");
...
public function approachRubiScenes():void {
	...
	//(Approach repeat, relationship 0-19)
		...
		Render.text("You take a seat at a table, and within seconds Rubi pulls up a chair next to you.  ");
	}
	//(Approach repeat, relationship 20+.  At this stage, randomly select an outfit from what the PC has given him, plus his waitress outfit.  Variants included for Bimbo /Incubus Rubi.)
		...
		if(select == 1) {
			...
			Render.text(rubiMF("He","She") + "'s dressed in [rubi eir] usual waitress uniform: a pink blouse and skirt covered by a frilly white apron.");
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) Render.text("  Rubi's breasts strain against the tight blouse, threatening to pop a button at any moment.");
			if(flags[FlagEnum.RUBI_COCK_SIZE] >= 9) Render.text("  You notice Rubi holds [rubi emself] a little awkwardly, and it's only when [rubi ey] sits down that you spy the slight bulge in [rubi eir] skirt.");
		}
		//(Suit Clothes (Normal))
		else if(select == 2) {
			//(Suitclothes)
			Render.text(rubiMF("He","She") + "'s dressed differently today, having foregone the normal waitress uniform for a professional black and white suit with a loose tie, though [rubi ey] still wears the waitress headband");
			if(flags[FlagEnum.RUBI_HORNTYPE] > 0) Render.text("just behind the horns");
			Render.text(".  In a sexy sort of way, [rubi ey] looks like a girl dressed in her boyfriend's clothes");
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) Render.text(", though with [rubi eir] large bosom, [rubi ey] can't quite button up all the way, giving the appearance of a sexy secretary");
			Render.text(".");
		}
		else if(select == 3) {
			//(Rubber Fetish Outfit (Bimbo))
			//(Rubber Fetish Clothes)
			Render.text("[rubi Ey]'s dressed differently today, having foregone the normal waitress uniform for a tight, black, rubber shirt, along with a skirt, and stockings.  The holes cut into the skirt, proudly displaying Rubi's ");
			if(flags[FlagEnum.RUBI_NIPPLE_TYPE] > 0) Render.text("black ");
			Render.text("nipples, which stand at perky attention in the cool air.  Rubber straps crisscross the outfit in odd places, and the whole ensemble looks like something out of one of Mareth's many fetish dungeons.  The outfit draws attention to [rubi eir]");
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) Render.text(" impressive bosom, and sort of makes [rubi em] look like the kind of rubber doll one might buy for those lonely nights.");
			else Render.text(" relatively flat chest, though with the rest of [rubi eir] figure, it's hard to imagine [rubi em] as anything but a woman.");
		}
		else if(select == 5) {
			//(Tube Top)
			Render.text("[rubi Ey]'s dressed differently today, having foregone the normal waitress uniform for a neon pink tube top and a pair of tiny denim shorts.");
			//(D breasts or higher)
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) Render.text("  [rubi Eir] breasts strain the fabric of the tube top, any wrong move threatening to have them burst forth.");
			//(dick 9" or higher)
			if(flags[FlagEnum.RUBI_COCK_SIZE] >= 4) Render.text("  [rubi Eir] rather large bulge can't be concealed, however, and you sometimes catch Rubi holding [rubi eir] notepad down across [rubi eir] crotch when [rubi ey] moves from table to table.");
			else Render.text("[rubi Eir] bulge is well concealed at least.  You're sure it would take a skilled eye, or someone who knew what to look for, to notice Rubi's little package.");
		}
		else if(select == 6) {
			//(Bodysuit)
			Render.text("[rubi Ey]'s dressed differently today, having foregone the normal waitress uniform for an almost sheer bodysuit covering [rubi em] from the groin up to the neck.  The lacy outfit is covered with flowers and swirling, colorful decorations in strategically placed locations.");
			if(flags[FlagEnum.RUBI_COCK_SIZE] >= 9) Render.text("  However, with an outfit this tight, it's almost impossible for [rubi em] to hide [rubi eir] impressive package.  Rubi's cock pokes straight upwards, creating a bulge from [rubi eir] crotch up towards [rubi eir] belly button.");
			else if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("  However, with an outfit this tight you can see a slight outline where [rubi eir] small package is tucked away.  You wonder how many other patrons have noticed the same thing.");
		}
		else if(select == 7) {
			//(Long Dress (Bimbo))
			Render.text("[rubi Ey]'s dressed differently today, having foregone the normal waitress uniform for an elegant, sequined ballroom gown.");
			if(rubiBimbo()) Render.text("  [rubi Ey]'s even modified the dress with false gems and a slit up the side to show off [rubi eir] leg.");
			Render.text("  You are stunned by the breathtaking combination of classy and carnal.  You're sure [rubi ey] would look great at a fancy ball, but here at the bakery [rubi ey] looks out of place, especially as a waitress.");
			//(Breasts flat or A cup)
			if(flags[FlagEnum.RUBI_BREAST_SIZE] <= 1) Render.text("  The dress must have come with a padded bra, because you're surprised to see Rubi with  breasts.  Though small, they help to accentuate [rubi eir] feminine figure.");
		}
		else if(select == 8) {
			//(Tight Leather Pants (Bimbo))
			Render.text("[rubi Ey]'s dressed differently today, having foregone the normal waitress uniform for a loose silk white shirt with tight black leather pants.");
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) Render.text("  With [rubi eir] large rack, [rubi ey] looks much like a busty piratess.  From the way [rubi eir] breasts strain at the top, you're certain that one wrong breath could send a button flying across the room.");
			else Render.text("  With [rubi eir] relatively flat chest, however, you wonder if perhaps it makes [rubi em] look a bit too much like a man.  An effeminate man, but a man nonetheless.");
			//Cock variants:
			//(9" or higher)
			if(flags[FlagEnum.RUBI_COCK_SIZE] >= 9) Render.text("  [rubi Eir] generous cock is on display, much to [rubi eir] chagrin, as a bulge running down the length of one pant leg.");
			else Render.text("Luckily, given [rubi eir] relatively small package, [rubi eir] bulge is kept nice and secret even with the tightness of the clothing.");
		}
		else if(select == 9) {
			//(Nurse Clothes (Bimbo))
			...
			//(D breasts or higher)
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) Render.text("  Rubi's large breasts strain the fabric, drawing the dress further up [rubi eir] body and showing off more of [rubi eir] legs and ass.");
		}
		//(Slutty Swimwear (Normal))
		else if(select == 10) {
			//(Slutty Swimwear)
			Render.text("[rubi Ey]'s dressed differently today, having foregone the normal waitress uniform for an impossibly skimpy black bikini.  It's a cute swimsuit, and Rubi looks damn sexy in it, but you can't help but think it's more suited for a day at the beach and not a day working in a bakery.");
			//(D breasts or higher)
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) Render.text("  Rubi's tremendous cleavage threatens to spill out of the skimpy bikini top with every movement, and from the red faces around the bakery, you wonder if that's already happened.");
			else if(flags[FlagEnum.RUBI_BREAST_SIZE] <= 1) Render.text("  The top only emphasizes Rubi's relatively flat chest, though with the rest of [rubi eir] figure, [rubi ey] still looks extremely feminine.");
		}
		//(Minidress Outfit (Bimbo))
			...
			if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("  [rubi Eir] skirt rides so high you catch glimpses of [rubi eir] unsurprisingly unpantied crotch, and the enticing package held within.  You wonder how many people have gotten a surprise with their drinks today!");
		}
		//(Bondage Straps)
		else if(select == 12) {
		...
		//(Risque Waitress Uniform)
			...
			if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text(rubiCock(33) + ", clad in a cute little frilly pink cocksock");
			if(flags[FlagEnum.RUBI_NO_CUNT] == 0 && flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text(" and ");
			if(flags[FlagEnum.RUBI_NO_CUNT] == 0) {
				Render.text("feminine mound trapped beneath a pair of pink panties");
				//(if Bimbo Brain)
				if(rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) Render.text(" trimmed feminine mons, free and open to the air");
			}
			Render.text(".");
			//(D breasts or higher)
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) Render.text("  Rubi's breasts, large as they are, show off a decent amount of sideboob and threaten to spill out at any moment.  Judging by the flushed faces of some customers, this may have already happened.");
			Render.text("  This outfit, as risque as it is, is clearly something Rubi never would have worn before [rubi ey] met you.  [rubi Ey] must be really comfortable in [rubi eir] body now!");
		}
		Render.text("\n\n[rubi Ey] hands you the usual menu and says, \"<i>");
		if(rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) Render.text("Like, h");
		else Render.text("H");
		Render.text("ere's the menu, babe.  Think about what you want and let me know when you're ready.</i>\"  [rubi Ey] stands and moves to clean off a nearby table, [rubi eir] red spaded tail swishing behind [rubi em] sensually.");
		...
		if(select == 4) {
			...
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) Render.text("  In an odd sort of way, [rubi ey] looks like an adventurer who met the wrong end of a witch's curse.");
		}
		...
		if(select == 1) {
			...
			if(rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) {
				Render.text("surprised to see [rubi ey]'s wearing nothing at all underneath, showing off [rubi eir] asshole");
				if(flags[FlagEnum.RUBI_NO_CUNT] == 0 && flags[FlagEnum.RUBI_COCK_SIZE] == 0) Render.text(" and wet cunt");
				else if(flags[FlagEnum.RUBI_NO_CUNT] > 0 && flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text(" and limp cock");
				else Render.text(", wet cunt, and limp cock");
				Render.text(" to anyone behind [rubi em]");
			}
			else Render.text("excited to see Rubi's choice of frilly pink panties clinging to [rubi eir] supple ass");
			Render.text(".");
		}
		//(Dashing Outfit)
		else if(select == 8) Render.text("\n\nAs [rubi ey] bends over, you get a good look at [rubi eir] ass, emphasized by the tight leather pants.");
		//(Adventurer's Outfit)
		else if(select == 15) {
			...
			//If Bimbo Body:
			if(rubiBimbo()) Render.text("  You even spy a wet patch in the crotch, with rivulets of moisture running down the legs, which [rubi ey] must have trouble hiding... if [rubi ey] even bothers to try.");
		}
		//(Long Dress)
		else if(select == 7) {
			Render.text("\n\nAs [rubi ey] moves to another table, Rubi sways back and forth, as if dancing with an invisible partner.  ");
			if(rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) Render.text("And then [rubi ey] begins to shake [rubi eir] ass, making spanking motions and effectively ruining the air of sophistication.");
		}
		//(Nurse Uniform)
		else if(select == 9) {
			Render.text("\n\nAs [rubi ey] bends over, you get a good look up [rubi eir] skirt, ");
			...
			else Render.text("excited to see Rubi's choice of frilly white panties clinging to [rubi eir] supple ass");
			Render.text(".");
		}
		//(Rubber Fetish Outfit)
			...
			else Render.text("excited to see a pair of translucent rubber panties clinging to [rubi eir] supple ass");
			Render.text(".");
		}
		//(Suitclothes)
		else if(select == 2) {
		...
		//(Bimbo Minidress)
			...
			else Render.text("excited to see a cute, little thong wedged into [rubi eir] supple ass");
			Render.text(".");
		}
		//(Bodysuit)
		else if(select == 6) {
			...
			//If Bimbo Body:
			if(rubiBimbo()) Render.text("  You even spy a moist patch in the crotch, which must be impossible to hide... if [rubi ey] even bothers to try.");
		}
		//(Bondage Straps)
		else if(select == 12) {
		...
		//(Risque Waitress Uniform)
			...
			else Render.text("excited to see a pair of pink frilly panties clinging to [rubi eir] supple ass");
			Render.text(".");
		}
	}
	...
	let tea:Function = null;
	let milk:Function = null;
	if(player.stats.gems >= 6) tea = getTeaFromRubi;
	else Render.text("\n<b>You cannot afford tea.</b>");
	if(player.stats.gems >= 3) milk = buyRubiMilk;
...
public function buyRubiMilk():void {
	...
	if(rubiAffection() >= 30 && flags[FlagEnum.RUBI_ADMITTED_GENDER] == 0) doNext(specialRelationship20scene);
...
public function getTeaFromRubi():void {
	...
	//(Relationship 0-19)
	if(rubiAffection() < 30) {
		Render.text("You flag Rubi down and tell her you'd like to order a cup of tea.  She nods and soon you've got a piping hot cup of fragrant tea in front of you.  \"<i>It's our relaxing blend, soothes the body and mind,</i>\" she says.  You take a sip and sigh.  It is indeed relaxing.");
	...
	//(Relationship 20+, normal Rubi)
		...
		Render.text("You flag Rubi down and tell [rubi em] you'd like to order a cup of tea.  [rubi Ey] nods and soon you've got a piping hot cup of fragrant tea in front of you.  \"<i>It's our relaxing blend, soothes the body and mind,</i>\" [rubi ey] says.  You take a sip and sigh.  It is indeed relaxing.");
...
public function chatWithRubi():void {
	...
	//(Relationship 40, Bimbo Rubi)
		...
		Render.text("You decide not to order anything to drink, instead taking the opportunity to hang out with Rubi whenever [rubi ey] has a free moment.  You talk of pleasant but inconsequential things.  Rubi spends some time discussing the fashion movements of Tel'Adre and how they're nothing like the fashion in [rubi eir] home village, an honest-to-goodness human village.  [rubi Ey] mentions how everyone back 'home' dressed so conservatively, not like the sexy body-baring fashions here.  Rubi even says how glad [rubi ey] is that demons attacked those years ago, so that [rubi ey] could come here and start her new life with hot new clothes and a new body.");
...
public function rubisFuckingHouseYouPervert():void {
	Render.text(images.showImage("rubi-at-house"), false);
	clearOutput();
	rubiSprite();
	//(Not available to those with a centaur, naga or drider body.)
	//(First Time?)
	if(flags[FlagEnum.RUBIS_HOUSE_FIRST_TIME] == 0) {
		Render.text("Rather than order anything, you stop Rubi as he's going by your table and pull [rubi em] close.  \"<i>How about we get out of here?</i>\" you whisper into [rubi eir] ear.");
	...
	//Bimbo Shopping!
	else if(rubiBimbo() && (player.perks.has("BimboBrains") || player.perks.has("FutaFaculties")) && rand(7) == 0) {
		flags[FlagEnum.RUBI_BIMBO_MINIDRESS] = 1;
		Render.text("Rather than order anything, you stop Rubi as she's going by your table and pull her close.  \"<i>How about we get out of here?</i>\"  you whisper into her ear.");
	...
	//(Repeat, Bimbo Rubi)
		...
		if(flags[FlagEnum.RUBI_PROSTITUTION] == 2 && flags[FlagEnum.RUBI_PROFIT] > 0) {
			...
			player.stats.gems += flags[FlagEnum.RUBI_PROFIT];
	...
	//Repeat unirubi variant
		...
		Render.text("Rather than order anything, you stop Rubi as [rubi ey]'s going by your table and pull [rubi em] close.  \"<i>How about we get out of here?</i>\" you whisper into [rubi eir] ear.");
...
private function rubiSexMenu():void {
	...
	menu();
	if (player.lust >= 33) {
		if (player.lowerBody.cockSpot.hasCock() || (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.vaginaSpot.list[0].clitLength >= 4)) addButton(0, "Fuck", fuckRubi);
		if (player.hasKeyItem("Deluxe Dildo") >= 0) addButton(1, "Dildo Fuck", dildoFuckRubi);
	}
	addButton(2, "Tease", teaseRubi);
	if (flags[FlagEnum.RUBI_BLU_BALLS] > 0 && flags[FlagEnum.RUBI_ORGASM_DENIAL] > 0 && !rubiBimbo() && flags[FlagEnum.RUBI_COCK_SIZE] > 0) addButton(3, "Release", releaseRubi);

	if (flags[FlagEnum.RUBI_AFFECTION] >= 75 && rubiCapacity() < player.lowerBody.cockSpot.biggestCocks[0].cockArea() && player.lowerBody.cockSpot.hasCock())
	...
	if (player.lowerBody.cockSpot.hasCock()) addButton(6, "Hotdogging", rubiHotdogging);
...
private function talkToRubiInHouse():void {
	clearOutput();
	rubiSprite();
	Render.text("You just want to talk with Rubi for now.  What do you want to talk about?");
	menu();
...
private function rubiIdentity():void {
	...
	if(flags[FlagEnum.TIMES_DISCUSSED_RUBIS_IDENTITY] == 0) {
		Render.text("You've noticed Rubi always refers to himself as a \"he\", despite ");
		if(flags[FlagEnum.RUBI_NO_CUNT] == 1)
			Render.text("his attempts to appear otherwise");
		else
			Render.text("his relatively recent transformation to the contrary");
		Render.text(".  Questioning him about it, he simply blushes.");
		...
		if(flags[FlagEnum.RUBI_NO_CUNT] == 0)
			Render.text("  Though now that I am, I absolutely love it!");
		Render.text("</i>\" he confides, fidgeting slightly.  \"<i>If you wanted to call me a \"she\", I'd be OK with that, just so you know.</i>\"");
		Render.text("\n\nWhich type of pronoun would you prefer Rubi to use?");
	}
	//(Repeating?)
	else {
		Render.text("You talk with Rubi at length about [rubi eir] gender identity again.  All in all, [rubi ey] seems pretty open to being either gender, with no real preference one way or another as long as [rubi ey] gets [rubi eir] pretty dresses.");
		Render.text("\n\nWhich type of pronoun would you prefer Rubi to use?");
	}
...
private function rubiIsAHe():void {
	...
	Render.text("Rubi nods, and will refer to himself as a \"he\" from now on.");
	menu();
	addButton(9,"Back",rubisFuckingHouseYouPervert);
}
//She
private function rubiIsAShe():void {
	...
	rubiSprite();
	flags[FlagEnum.RUBI_SHE] = 1;
	Render.text("Rubi nods, and will refer to herself as a \"she\" from now on.");
	menu();
	addButton(9,"Back",rubisFuckingHouseYouPervert);
}
//Sex Scenes Ahoy!
...
public function fuckRubi():void {
	Render.text(images.showImage("rubi-fuck"), false);
	let x: number;
	...
	rubiSprite();
	//(-100 Arousal)
	Render.text("You take Rubi by the hand and head into the bedroom, clutching [rubi em] close.  You sit on the edge of the soft fluffy bed and pull the little demon " + rubiMF("boy","girl") + " onto your lap.  The two of you kiss, letting your " + tongueDescript() + " explore the depths of your partner's mouth.");
	...
	if(player.lowerBody.cockSpot.hasCock())
	{
		...
		Render.text(cockDescript(x), false);
	}
	else
		Render.text(clitDescript(), false);
	Render.text(".");
	...
	if(size == 0) {
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] <= 5) Render.text("I guess we're in the same boat.</i>\"");
		else if(rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) Render.text("That's too bad, I was soooo looking forward to a real fucking.</i>\"");
		else Render.text("That's pretty sad, getting shown up by a dainty little thing like me...</i>\"");
	}

	else if(size == 1) Render.text("\n\n\"<i>Oh wow, you're quite the stud,</i>\" Rubi says, eyes sparkling with anticipation.");
	...
	Render.text(", Rubi slips off your lap and kneels on the floor.  [rubi Ey] pauses a moment, taking in your ");
	if(player.lowerBody.cockSpot.hasCock()) Render.text(cockDescript(x));
	else Render.text("[clit]");
	Render.text(" with [rubi eir] eyes before leaning in and giving it a tentative lick, which sends a shiver up your spine.  Seeing that, [rubi ey] gives a little smile and licks again, running [rubi eir] smooth tongue all along the length of your ");
	if(player.lowerBody.cockSpot.hasCock()) Render.text("swollen member");
	else Render.text("impressive love button");
	Render.text(".  Rubi's mouth dances along its length, at times flicking delicately and at others slurping loudly.  [rubi Eir] tongue ripples along the underside of ");
	if(player.lowerBody.cockSpot.hasCock()) Render.text("your cockhead");
	else Render.text("the tip of your clit");
	Render.text(", sending shivers of pleasure up your spine.");
	//(If Cock)
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  Amazingly, with the expert ministrations of your lover's mouth, you can't hold yourself in much longer, and begin to tense up, but at the last minute you pull yourself away from Rubi's grip and lips, halting yourself before you can cum.");
	...
	if(size < 2) {
		...
		Render.text(" at [rubi eir] tight ");
		if(flags[FlagEnum.RUBI_NO_CUNT] > 0) Render.text("tailhole");
		else Render.text("pussy");
		Render.text(", rubbing the mixture of [rubi eir] saliva and your juices into [rubi eir] cleft to get [rubi em] suitably lubricated before the real fun begins");
		if(rubiBimbo()) Render.text(", not that [rubi ey] needs the extra lubrication, as [rubi ey]'s practically dripping like a faucet");
		Render.text(".");
		Render.text("  You press forward, sinking your entire length into Rubi's depths, which clench and unclench sensually.  Aided by the additional lubrication, you slip easily in and out, pumping slowly at first.");
	}
	//(These two paragraphs for cocks over 24 area)
		...
		Render.text(" at the space just below [rubi eir] ");
		if(flags[FlagEnum.RUBI_NO_CUNT] > 0) Render.text("balls");
		else Render.text("cunt");
		Render.text(", rubbing the mixture of [rubi eir] saliva and your juices into [rubi eir] legs to get [rubi em] suitably lubricated before the real fun begins");
		//(if bimbo)
		if(rubiBimbo()) Render.text(", not that [rubi ey] needs the extra lubrication, as she's practically dripping like a faucet, which dribbles down [rubi eir] legs suitably enough");
		Render.text(".");
		Render.text("\n\nYou press forward, your enormous ");
		if(player.lowerBody.cockSpot.hasCock()) Render.text(cockDescript(x));
		else Render.text("[clit]");
		Render.text(" slipping between Rubi's thighs, which grip you tightly.  You feel [rubi eir] ");
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("hard cock");
		else Render.text("stiff clit");
		Render.text(" pressing against the top of ");
		if(player.lowerBody.cockSpot.hasCock()) Render.text("your dick");
		else Render.text("your clit");
		Render.text(", and know that with every thrust you give, [rubi ey]'ll get a good thrill too.");
	}
	...
	if(flags[FlagEnum.RUBI_ORGASM_DENIAL] > 0 && !rubiBimbo()) Render.text("  You give Rubi's ass a hard smack and remind [rubi em] that no matter what happens, [rubi ey] still can't cum yet.  [rubi Eir] eyes flutter open and [rubi ey] actually whimpers a little, but can't form the words to object.");
	//(If Orgasm Denial is NOT activated; male/herm Rubi)
	else if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("  Rubi’s body shivers and trembles, and almost collapses.  [rubi Ey] grasps at the blankets with wild abandon as ropes of pearly white cum erupt from [rubi eir] cock, splattering against [rubi eir] chest and the blankets.");
	//(Female Rubi)
	else Render.text("  Rubi's body shivers and trembles, and [rubi ey] almost collapses.  [rubi Ey] grasps at the blankets with wild abandon and gives and orgasmic scream as [rubi eir] cunt clenches and unclenches fiercely.  A spray of femjuices erupts outwards from [rubi em], coating your crotch and dripping down both yours and [rubi eir] legs.");
	...
	if(player.lowerBody.cockSpot.hasCock() && player.cockArea(x) <= rubiCapacity()) {
		Render.text("Your " + cockDescript(x) + " engorges for a second before erupting deep inside your demonic little fucktoy.  ");
		if(player.cumQ() < 250) Render.text("Cum dribbles out from [rubi eir] hole as you give a couple extra thrusts.");
		else if(player.cumQ() < 1000) Render.text("A generous amount of cum trickles from [rubi eir] depths as you give a couple extra thrusts.");
		else if(player.cumQ() < 2000) {
			Render.text("Cum practically floods from  [rubi eir] abused ");
			if(flags[FlagEnum.RUBI_NO_CUNT] > 0) Render.text("ass");
			else Render.text("cunt");
			Render.text(" as you give a couple extra thrusts.");
		}
	}
	else if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("Your " + cockDescript(x) + " engorges for a second before erupting beneath Rubi, splattering [rubi em] and the blankets ");
		if(player.cumQ() < 250) Render.text("with a fair amount of your seed.");
		else if(player.cumQ() < 1000) Render.text("with a generous amount of your spunk.");
		else Render.text("with a veritable torrent of potent pearlescent spooge.");
	}
	//(Player has clit?)
	else Render.text("  Your " + clitDescript() + " throbs and engorges briefly as your " + vaginaDescript(0) + " reflexively contracts and a warm feeling rocks through your entire body.  Femjuices run down your [legs] and relief overtakes you like a powerful wave upon a beach.");
	...
	if(rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) Render.text("like, totally ");
	else Render.text("absolutely ");
	Render.text("fantastic babe,</i>\" [rubi ey] murmurs before drifting off into a sex-filled dreamland.");
...
public function dildoFuckRubi():void {
	Render.text(images.showImage("rubi-dildo-fuck"), false);
	clearOutput();
	rubiSprite();
	//(If PC has no cock)
	if(!player.lowerBody.cockSpot.hasCock()) Render.text("You don't have the equipment to give Rubi a good fucking, but there is one thing you do have.");
	//(If PC has cock)
	else Render.text("While you do have the requisite equipment to give Rubi a good fucking, you'd much rather use something else.");
	//(Add)
	Render.text("  You produce the goblin-made dildo from your pack, and Rubi's eyes immediately go wide.  \"<i>Wow,</i>\" [rubi ey] says, reaching out to touch it.  [rubi Ey] runs [rubi eir] hands along its length, marveling at its color, shape and size.");
	...
	//(If PC has a cunt)
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("You press the tip into your cunt quickly, giving it a taste of what is to come.  The brief contact is enough to make you shudder slightly with pleasure.  ");
	//(If PC has fuckable nipples)
	if(player.upperBody.chest.hasFuckableNipples()) Render.text("You let the dildo meander up your body until it reaches your breasts, and with a bit of a grin you insert the tip right into one nipple, enjoying the feeling of your breasts swell up with arousal.  You do the same with the other nipple, and take a minute going back and forth.  ");
	Render.text("Finally you bring the dildo up to your mouth, licking the juices off of it, eagerly downing the goblin sex-drug.");
	...
	//(Normal/Incubus Rubi)
	if(!rubiBimbo()) Render.text("They part shakily and [rubi eir] tongue reaches out to taste the dildo.  ");
	//(Bimbo Rubi)
	else Render.text("They easily part, and [rubi eir] tongue takes a long slurp, just to taste the dildo.  ");

	Render.text("[rubi Ey] apparently finds it pleasing, because [rubi ey] immediately wraps [rubi eir] lips around the tip, sucking and bobbing on it like it was a real cock.");
	...
	if(flags[FlagEnum.RUBI_NO_CUNT] == 1) Render.text("against [rubi eir] tight ass");
	else Render.text("against [rubi eir] tight pussy");
	Render.text(", which resists a little at first, but once the drug has a moment to work, it slips right in.  Rubi actually coos a little as it slips inside.");
	...
	//(If player has pussy)
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Your " + clitDescript() + " stiffens, aching for attention.  It peeks from your folds like a shy little lewd faerie.");
	//(If player has a cock)
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  Meanwhile, [eachCock] stiffens, throbbing as the drug overtakes it.");
	if(player.lactationQ() > 0) Render.text("  As the warmth spreads up your body, not even your breasts are left alone.  Your nipples harden and begin leaking milk, which pours down the sides of your body and is soaked up into the cushions beneath you.");
	...
	//(Normal/Incubus Rubi, Orgasm Denial off)
		...
		if(!rubiIncubus()) Render.text("little cock");
		else Render.text("demonic cock");
		Render.text(" twitches, launching jets of cum onto [rubi eir] stomach, chest and face.  In [rubi eir] sex-filled stupor, [rubi ey] eagerly swallows down whatever lands in [rubi eir] mouth, and actually uses [rubi eir] fingers to collect the rest , gulping that down as well.");
	}
	//(Bimbo/Female Rubi)
	else {
	...
	//(PC has cock?)
	if(player.lowerBody.cockSpot.hasCock()) Render.text("[eachCock], stroking it fervently, in desperate need of release.");
	//(PC doesn't have cock, but has pussy?)
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("your " + clitDescript() + ", rubbing it frantically, in desperate need of release.");
	//(PC doesn't have either, but breasts above A-cup?)
	else Render.text("your [chest], rubbing your nipples vigorously, in desperate need of release.");
	//(Bonus, fuckable nipples?)
	if(player.upperBody.chest.hasFuckableNipples()) Render.text("  Your fingers easily slip inside each nipple, pumping in and out like little cocks.");
	//(No cock, no pussy, no breasts?)
	if(player.gender == 0 && player.upperBody.chest.BreastRatingLargest[0].breastRating == 0) Render.text("  Your hands roam all along your body, from your bare crotch to your nipples, even through your hair, all in search of something to give you the release you so desperately crave.");
	...
	//(PC has cock?)
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("  [EachCock] twitches and shudders, loosing several jets of cum onto your body, ");
		if(player.cumQ() < 250) Render.text("splattering messily");
		else if(player.cumQ() < 500) Render.text("coating you lightly");
		else if(player.cumQ() < 1000) Render.text("covering you in a copious layer of cum");
		else Render.text("covering you in a thick layer of your potent seed");
		Render.text(".");
	}
	//(PC has pussy?)
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Your " + vaginaDescript(0) + " clenches down on the enormous invader, which at this point feels like an enormous steel rod lodged firmly inside you, and a spray of fem-juices erupts from your nethers, coating the already slick dildo and Rubi.");
	//(PC lactating?)
	if(player.lactationQ() > 0) Render.text("  Your breasts shudder and quake as the orgasmic tsunami washes over them, spraying your sweet smelling milk all over yourself, Rubi and the couch.");
	...
	Render.text("\n\nRubi rubs [rubi eir] ");
	if(flags[FlagEnum.RUBI_NO_CUNT] == 0) Render.text("pussy");
	else Render.text("asshole");
	Render.text(" with a slight smile on [rubi eir] face, and says, \"<i>Well, that was interesting.</i>\"  You laugh at the understatement of the century, and stand, helping Rubi up off the couch as well.");
...
public function teaseRubi():void {
	Render.text(images.showImage("rubi-tease"), false);
	clearOutput();
	rubiSprite();
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) {
		Render.text("Having something quite different in mind, you order Rubi to strip down.  With a curious look on [rubi eir] face, [rubi ey] does so, slowly.  Each garment [rubi ey] peels from [rubi eir] soft ");
		if(flags[FlagEnum.RUBI_SKIN] == 0) Render.text("pale");
		else if(flags[FlagEnum.RUBI_SKIN] == 2) Render.text("fur-covered");
		else Render.text("red");
		Render.text(" skin seems to take an eternity, until finally [rubi ey]'s standing in front of you in naught but [rubi eir] panties.");
		...
		//(Normal Rubi)
		Render.text("  [rubi Ey] blushes, looking back at you with a curious, yet lustful expression.  [rubi Ey] gulps and lifts [rubi eir] tail, giving you free access to [rubi eir] pantied rump.");
	...
	//Girl Rubi!
		...
		Render.text("Having something quite different in mind, you order Rubi to strip down.  With a curious look on [rubi eir] face, [rubi ey] does so, eagerly.  Each garment [rubi ey] peels from [rubi eir] soft ");
		if(flags[FlagEnum.RUBI_SKIN] == 0) Render.text("pale");
		else if(flags[FlagEnum.RUBI_SKIN] == 1) Render.text("crimson");
		else if(flags[FlagEnum.RUBI_SKIN] == 2) Render.text("furred");
		else Render.text("striped");
		Render.text(" skin seems to take an eternity.  In fact, [rubi ey] seems to take great delight in stripping, giving little twirls and tossing [rubi eir] clothes towards you, until [rubi ey]'s standing in front of you in naught but [rubi eir] birthday suit.");
		...
		Render.text("  \"<i>Ooh, YES, right there,</i>\" Rubi gasps.  Your hand pinches [rubi eir] clit tight, eliciting a yelp and a coo of pleasure at the same time.");
		...
		if(rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) Render.text("like, ");
		Render.text("cum!</i>\"");
...
public function releaseRubi():void {
	Render.text(images.showImage("rubi-orgasm-denial-release"), false);
	clearOutput();
	rubiSprite();
	//Will deactivate Orgasm Denial.  To start Denying again, the PC will have to Tease.
	flags[FlagEnum.RUBI_ORGASM_DENIAL] = 0;
	Render.text("You order Rubi to strip down.  Knowing the drill, [rubi ey] does so slowly.  There's no striptease, no show, [rubi ey] just disrobes as quickly as possible until [rubi ey]'s standing in naught but [rubi eir] panties, ");
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("[rubi eir] [rubi cock] straining at the seams, trying to break free.");
	else Render.text("clinging tight to [rubi eir] flat little mound.");
	...
	if(flags[FlagEnum.RUBI_BLU_BALLS] <= 3) {
		Render.text("  Not to mention [rubi eir] tiny balls");
		if(flags[FlagEnum.RUBI_BALLS_TYPE] == 1) Render.text(", trapped in their little sissy sack");
		Render.text(", which you think have swelled somewhat, although you haven't been teasing [rubi em] long.");
	}
	//(if Blueballs 4-9)
	else if(flags[FlagEnum.RUBI_BLU_BALLS] <= 9) Render.text("  Not to mention [rubi eir] sizeable balls, which seem to have swelled up to twice their normal size, thanks to your relentless teasing.  They even have a slight bluish tinge to them, although it's not immediately visible.");
	else Render.text("  Not to mention [rubi eir] enormous balls, which, thanks to your endless teasing, are seemingly the size of baseballs.  They even have a noticeable blue tinge to them, obviously aching for relief.");
	...
	if(flags[FlagEnum.RUBI_BLU_BALLS] <= 3) Render.text("swollen");
	else if(flags[FlagEnum.RUBI_BLU_BALLS] <= 9) Render.text("slightly blue");
	else Render.text("incredibly swollen blue");
	Render.text(" balls.  Once it's properly lubed up, you press two fingers inside.");
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("P-spot");
	else Render.text("G-spot");
	//else Render.text("most sensitive spot");
	Render.text(".  When Rubi jerks and wiggles involuntarily, you know you've found it.  Pre-cum begins leaking from Rubi's [rubi cock] in copious amounts, running down your hand and arms, and forming a puddle on the coffee table.  Rubi's body trembles and shakes, wriggling on the table like a fish out of water.");

 	Render.text("\n\nFeeling a little more adventurous, you slip a third finger into the little demon " + rubiMF("boy","girl") + "'s ");
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("rectum");
	else Render.text("folds");
	Render.text(".  With all three fingers you press and massage [rubi eir] prostate, increasing the amount of pre-cum flowing from [rubi eir] tip.");
	...
	//(Blueballs 1-3)
	if(flags[FlagEnum.RUBI_BLU_BALLS] <= 3) Render.text("  It's quickly followed by another, less forceful jet of pearly white cum, which lands on [rubi eir] face, and several even smaller jets which splatter the area around [rubi em].  [rubi Eir] cock twitches and spasms, then slowly softens.");
	//(Blueballs 4-5)
	else if(flags[FlagEnum.RUBI_BLU_BALLS] <= 5) Render.text("  It's quickly followed by another, less forceful jet of pearly white cum, then another, and another, each landing closer to [rubi eir] face.  Finally, it gives a few more token jerks, spurting out tiny globules of cum onto the surrounding area.  [rubi Eir] cock twitches and spasms, squirting nothing but air, as if [rubi eir] body is still craving release.");
	//(Blueballs 6-7)
	else if(flags[FlagEnum.RUBI_BLU_BALLS] <= 7) Render.text("  It's quickly followed by another, more forceful jet of pearly white cum which splatters against the ceiling, dripping down onto the both of you.  Another spurt of cum bursts forth, then another, and another, each one liberally covering Rubi's stomach, chest and face.  [rubi Eir] cock finally gives a few last twitches and spasms, squirting nothing but air, as if [rubi eir] body is still craving release.");
	//(Blueballs 8-9)
	else if(flags[FlagEnum.RUBI_BLU_BALLS] <= 9) Render.text("  It's quickly followed by another, more forceful jet of pearly white cum, which splatters against the ceiling, dripping down onto the both of you, and then another, and another.  It isn't long before the ceiling has a huge white cum-covered blotch right above you, drizzling down on you like dew dripping from trees.  Rubi's cock doesn't stop there, however, it continues twitching and spurting [rubi eir] seed until [rubi ey] is completely covered in the stuff.  Finally, mercifully, it ends, though [rubi eir] [rubi cock] continues to spasm, squirting nothing but air, as if [rubi eir] body is still craving release.");
	//(Blueballs 10+)
	else Render.text("  It's quickly followed by another, more powerful jet of pearly white cum, which splatters against the ceiling, dripping down onto the both of you, and then another, and another.  It isn't long before the ceiling has a huge white cum-covered blotch right above you, drizzling down on you like dew dripping from the trees.  Rubi's cock doesn't stop there, however, it continues twitching and spurting [rubi eir] seed, liberally covering [rubi em]self, you and the surrounding area until everything appears to be in a white haze.");
	...
	//(Blueballs 1-5)
	if(flags[FlagEnum.RUBI_BLU_BALLS] <= 5) Render.text("You clean yourself up, leave Rubi a little \"<i>See you next time</i>\" note, and head out the door, back to camp.");
	//(Blueballs 6-9)
	else if(flags[FlagEnum.RUBI_BLU_BALLS] <= 9) Render.text("You head into the bathroom and wash the cum off you, taking a short shower.  Afterwards, you dry up, leave Rubi a little \"<i>See you next time</i>\" note, and head out the door, back to camp.");
	//(Blueballs 10+)
	else Render.text("You head into the bathroom and wash the cum off you, no easy feat to be sure.  Afterwards, you dry up, leave Rubi a little \"<i>See you next time</i>\" note, and head out the door, back to camp.  You certainly don't want to be there when [rubi ey] wakes up, lest you have to clean the entire house with [rubi em].");
...
public function cheatingRubi():void {
	...
	Render.text("You take a seat at a table, expectantly waiting for Rubi to slide into your lap...  but she never does.  You look around the place for her, but again, there's no sight of her.  You flag down Dia, the caninemorph waitress, who looks haggard, and inquire as to the location of her co-worker.");
...
private function dontCareAboutNoCheatingRubis():void {
	clearOutput();
	rubiSprite();
	Render.text("You shrug your shoulders.  It's not really your problem, is it?  You're free to fuck who you choose, and so is she.");
...
private function findBimboCheatster():void {
	clearOutput();
	rubiSprite();
	Render.text("You let out a disgruntled sigh.  Rubi's seeing someone else?!  Not on your watch!  You leave the bakery and make your way towards her neighborhood.  It doesn't take long, you've gone with her there enough times to memorize the path, even without her leading.");
...
private function interruptTheNTRsYouCrazyFool():void {
	clearOutput();
	rubiSprite();
	Render.text("No one's sticking their dick in your ");
	if(silly()) Render.text("waifu");
	else Render.text("slut");
	Render.text(" if you've got a say in it!  You get out of your hiding place, shout a word of warning and burst inside the vibrant pink house.");
...
private function waitAndGetNTRedLikeTheBoyBitchYouAre():void {
	...
	if(!player.isTaur()) {
		...
		else if(player.lowerBody.cockSpot.hasCock()) Render.text("[eachCock]");
		Render.text(".  ");
	}
	else Render.text("\n\n");
	Render.text("The two go on like this for five minutes, then ten, then fifteen.  Each position, you find, is more intriguing than the last.  ");
	if(silly()) Render.text("They're even doing positions that aren't available in the game!  ");
	Render.text("Half an hour later, Rubi and the wolfman sit exhausted on the couch, his seed pouring from each of her holes.");
...
private function NTRbimboBitchResolution():void {
	clearOutput();
	rubiSprite();
	Render.text("You look at Rubi and say that you're definitely going to have to talk about what just happened.");
...
private function noBimboNTR():void {
	clearOutput();
	rubiSprite();
	Render.text("Folding your arms, you shake your head.  You tell her that if this is going to work out, she has to remain faithful to you.  Sex toys and masturbation will have to do to keep her \"<i>urges</i>\" under control.  She looks a little sad, but nods as you speak.");
...
private function yesBimboNTR():void {
	clearOutput();
	rubiSprite();
	Render.text("You contemplate it for a moment, and then slowly nod your head.  What's the harm in letting her blow off steam?  You know she'll always come back to you.  You assent to her wishes, to which she squeals and hugs you tightly, tossing the pink cushion aside.");
...
private function breakUpWithRubi():void {
	clearOutput();
	rubiSprite();
	Render.text("You think about it for a long moment and then sigh sadly.  You look Rubi dead in the eyes and explain you can't be with a woman who wants to cheat on her partner.  Tears form in her eyes as she sniffles, but she nods with you.");
...
private function pimpOutRubi():void {
	clearOutput();
	rubiSprite();
	Render.text("You think about it for a long while, mulling over all the possibilities.  Then something else occurs to you. She wants help controlling her urges, why not make a little money in the meantime?  You explain that she's free to sleep with whoever she wants, but that she could charge them for her time.  After all, if you're going to do what you love, you may as well get paid for it.");
...
private function chocoRubiMassage():void {
	...
	if(player.stats.cor >= 66) Render.text("start sensually undressing, turning the simple act into a seductive show that causes Rubi's petite boner to jerk unpredictably beneath [rubi eir] clothes.  A small, damp stain appears at the tip by the time you finish your salacious tease.");
	else if(player.stats.cor >= 33) Render.text("get undressed without much pause or fanfare.  Rubi makes a show of turning [rubi eir] head, but you can see [rubi em] peeping glimpses out of the corners of [rubi eir] eyes, and [rubi eir] boner seems to be bigger and harder than it was moments before.");
	else {
		Render.text("make [rubi em] turn around while you undress.  You quickly shuck your clothes and ask [rubi em] what to do next, holding a hand across your [chest] and groin");
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 40 || player.upperBody.chest.BreastRatingLargest[0].breastRating >= 8) Render.text(", not that it does much good");
		Render.text(".");
	}
	...
	if(flags[FlagEnum.RUBI_BLU_BALLS] < 3) Render.text("small");
	else if(flags[FlagEnum.RUBI_BLU_BALLS] < 8) Render.text("big");
	else Render.text("huge, stretched");
	Render.text(" scrotum while allowing [rubi eir] delicate, aroused maleness to hang out freely, bouncing up in excitement as soon as you lay eyes on it.  Clinging to [rubi eir] svelte torso, a lacy corset compresses [rubi eir] middle slightly and amplifies [rubi eir] next-to-nonexistent chest, giving [rubi em] an even greater appearance of femininity than before.");
...
private function bottleUpRubiMassage():void {
	clearOutput();
	rubiSprite();
	Render.text("You bite the strawberry in half and mouth, \"<i>No, not yet.  Just let it build up.</i>\"  [rubi Ey] whines plaintively, but you cock your head to the side and say, \"<i>Come on, do it for me.  Just, don't let any out....  Here, come off of there and come into bed with me.  We can snuggle and let you relax a bit.</i>\"");
...
private function releaseRubiMassage():void {
	clearOutput();
	rubiSprite();
	Render.text("You figure after all the pampering [rubi ey] did, [rubi ey] deserves a treat.");
...
private function goInRubisClosetSoThatYouCanComeOutOfTheCloset():void {
	clearOutput();
	rubiSprite();
	Render.text("You ask to see Rubi's wardrobe.  [rubi Eir] eyes light up and [rubi ey] claps [rubi eir] hands together excitedly.  \"<i>");
	if(rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) Render.text("Oh my god!  ");
	Render.text("Really?  It's right over here!</i>\"  Rubi clasps your hand and drags you into the bedroom, then through another doorway.");
...
private function rubiAppearance():void {
	Render.text(images.showImage("rubi-inspect-appearance"), false);
	clearOutput();
	rubiSprite();
	Render.text("You look over your naked, demon-morph lover.");
	Render.text("\n\nRubi is about five feet tall, with a very lithe, feminine body.  [rubi Eir] skin is ");

	if(flags[FlagEnum.RUBI_SKIN] == 0) Render.text("pale, almost like porcelain");
	else if(flags[FlagEnum.RUBI_SKIN] == 1) Render.text("an exotic cherry red");
	else if(flags[FlagEnum.RUBI_SKIN] == 2) Render.text("covered with a fine layer of silky-smooth pink fur");
	else Render.text("an exotic cherry red which contrasts well with a grey belly and stripes that run up and down [rubi eir] legs and arms");

	if(flags[FlagEnum.RUBI_SKIN] != 2) Render.text(" and completely hairless, except for [rubi eir]");
	else Render.text(", and [rubi ey] sports");

	if(flags[FlagEnum.RUBI_HAIR.LENGTH] == 0) Render.text(" short");
	else if(flags[FlagEnum.RUBI_HAIR.LENGTH] == 1) Render.text(" shoulder-length");
	else Render.text(" long");
	if(flags[FlagEnum.RUBI_HAIR] == 0) Render.text(" black");
	else Render.text(" blonde");
	Render.text(" hair, which [rubi ey] keeps cut in a cute, feminine style.  Rubi's face is soft and rounded");
	if(flags[FlagEnum.RUBI_WHISKERS] > 0) Render.text(", with a cute set of whiskers sprouting from [rubi eir] cheeks");
	Render.text(", [rubi eir] lips painted a ");
	//(normal/furry skin)
	if(flags[FlagEnum.RUBI_SKIN] == 0 || flags[FlagEnum.RUBI_SKIN] == 2) Render.text("crimson red");
	//(red/siren skin)
	else Render.text("dusky black");
	Render.text(", [rubi eir] ");
	//if(flags[FlagEnum.RUBI_EYE_TYPE] == 1) Render.text("slitted ");
	if(flags[FlagEnum.RUBI_EYE_TYPE] == 0 || rubiBimbo()) Render.text("green");
	else if(flags[FlagEnum.RUBI_EYE_TYPE] == 1)
		Render.text("solid black");
	//else Render.text("black");
	Render.text(" eyes framed with eyeliner, belying [rubi eir] masculine origins.  ");
	if(flags[FlagEnum.RUBI_HORNTYPE] > 0) Render.text("Two tiny horns extend upwards from [rubi eir] hairline, solid black in color.  ");
	Render.text("A pair of ");
	if(flags[FlagEnum.RUBI_EAR_TYPE] == 0) Render.text("cute, normal ears protrude from the sides of [rubi eir] head, each one pierced with a small dangling earring");
	else if(flags[FlagEnum.RUBI_EAR_TYPE] == 1) Render.text("cute, cat-like ears pop up from the top of [rubi eir] head, each one seemingly able to move independently");
	else if(flags[FlagEnum.RUBI_EAR_TYPE] == 2) Render.text("feathered ears protrude from the sides of [rubi eir] head, each one fluttering cutely from time to time, and each pierced with a small dangling earring");
	Render.text(".");

	Render.text("\n\n[rubi Ey] has a ");
	if(flags[FlagEnum.RUBI_BREAST_SIZE] < 1) Render.text("flat-chest ");
	else Render.text("[rubi breasts]");
	Render.text(" supporting two perky ");
	if(flags[FlagEnum.RUBI_NIPPLE_TYPE] == 1) Render.text("black ");
	Render.text("nipples.");

	Render.text("\n\nBetween [rubi eir] legs is a ");
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) {
		Render.text(rubiCock() + ", which starts to perk up under your scrutiny. From experience, you know it can reach a impressive " + flags[FlagEnum.RUBI_COCK_SIZE] + " inches when erect.  ");
		Render.text("Under [rubi eir] cock swings a set of ");
		if(flags[FlagEnum.RUBI_BLU_BALLS] >= 4 && flags[FlagEnum.RUBI_BLU_BALLS] < 9) Render.text("cum-swollen ");
		else if(flags[FlagEnum.RUBI_BLU_BALLS] >= 9) Render.text("engorged, blue-tinged ");
		Render.text("testicles");
		if(flags[FlagEnum.RUBI_BALLS_TYPE] == 1) Render.text(", which cling tight to [rubi eir] body in a cute spherical pouch");
	}
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0 && flags[FlagEnum.RUBI_NO_CUNT] == 0) Render.text(" and a ");
	if(flags[FlagEnum.RUBI_NO_CUNT] == 0) {
		Render.text("cute ");
		if(flags[FlagEnum.RUBI_CUNTTYPE] > 0) Render.text("black ");
		Render.text("pussy topped off by a little love button");
		Render.text(".  Though you can't see it, you know Rubi's got a perfect little asshole, between [rubi eir] buttcheeks, right where it belongs.");
	}
	else
	{
		Render.text(".");
	...
	if(flags[FlagEnum.RUBI_FEET] == 1) Render.text("Finally, Rubi stands on a long black spike that makes [rubi em] walk as though [rubi ey] were wearing high heels.");
...
private function playDressUp():void {
	clearOutput();
	rubiSprite();
	Render.text("You look over Rubi's vast collection of clothes.  What outfit would you like Rubi to put on?\n\n");
	menu();
	let closet:Array = [];
	...
	if(closet.length > 0) {
		Render.text("<b>Rubi's Closet Contains:</b>\n");
		temp = 0;
		while(temp < closet.length) {
			Render.text(closet[temp] + "\n");
			temp++;
		}
		Render.text("\n");
	}
...
private function putRubiInAWaitressUniform():void {
	clearOutput();
	rubiSprite();
	Render.text("You tell Rubi you'd like to see [rubi em] in the waitress uniform.  \"<i>Hm, well all right, but it's just my work clothes,</i>\" [rubi ey] replies.");
	Render.text("\n\nRubi starts off with a white bra trimmed with lace");
	if(flags[FlagEnum.RUBI_BREAST_SIZE] < 1) Render.text(", though it's obvious [rubi ey] doesn't need it with [rubi eir] flat-chest");
	Render.text(".  [rubi Ey] slips [rubi eir] arms through the holes and expertly latches the back.  Next [rubi ey] takes a black garter belt and slips it up [rubi eir] slender ");
	if(flags[FlagEnum.RUBI_SKIN] != 2) Render.text("hairless");
	else Render.text("furred");
	Render.text(" legs, until it rests snugly around [rubi eir] waist.  A matching pair of sheer stockings quickly follow, held up by the garter.");

	if(!rubiBimbo() || flags[FlagEnum.RUBI_DEBIMBO] > 0) Render.text("\n\nYour demon-morphed lover next trots over to a chest of drawers and rummages through it until [rubi ey] produces a pair of frilly pink panties, complete with a little bow on the front.  [rubi Ey] steps into the underwear one stocking-clad leg at a time and pulls them up until they fit snugly around [rubi eir] round ass and [rubi cock].");
	...
	Render.text("  Rubi approaches a rack of clothes, searching quietly for a moment before picking two hangars off it: a pink blouse and a matching pink skirt.");

	Render.text("\n\nThe blouse goes on first, followed by the skirt.  Rubi only buttons the blouse about half way up");
	if(flags[FlagEnum.RUBI_BREAST_SIZE] < 1) Render.text(", showing off some of [rubi eir] flat chest");
	else Render.text(", showing off a generous portion of cleavage");
	Render.text(", and tucks the hem of the blouse into the skirt.  Finally, Rubi selects a pair of ");
	if(flags[FlagEnum.RUBI_FEET] == 1) Render.text("what appears to be flats, with a hole in the heel for [rubi eir] demonic heels");
	else Render.text("short heels");
	Render.text(" and slips them on.  [rubi Ey] gives a little twirl in the waitress uniform, and bends over several times to show off [rubi eir] ");
	if(!rubiBimbo() || flags[FlagEnum.RUBI_DEBIMBO] > 0) Render.text("girly panties");
	else Render.text("naked nethers");
	Render.text(".");
...
private function putOnADashingOutfitYouWhore():void {
	clearOutput();
	rubiSprite();
	Render.text("You tell Rubi you'd like to see [rubi em] in the dashing outfit.  \"<i>Ohh, feeling like plundering a little booty today?</i>\" [rubi ey] asks with a coy smile.");
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("tucking [rubi eir] cock firmly into the triangular strip of cloth covering [rubi eir] crotch, while ");
	Render.text("the string gets firmly lodged between Rubi's butt cheeks.  The next thing Rubi picks out is a pair of impossibly small leather pants.  [rubi Ey] puts them on one leg at a time, and struggles to pull the tight pants up [rubi eir] smooth thighs and shapely ass.  You get involved at some point, helping to hoist the pants up while Rubi buttons them properly.  The end result is a pair of leather pants that cling to Rubi's legs like a second skin, showing off every curve of [rubi eir] ass, thighs and calves");
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text(", not to mention the sizeable package in [rubi eir] crotch");
	Render.text(".");
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("  Combined with the sizeable bulge in the crotch, Rubi looks like quite the dashing, if slutty, piratess.");
...
private function goOnAnAnalAdventureRubiNotReallyJustAnAdventurersOutfit():void {
	clearOutput();
	rubiSprite();
	Render.text("You tell Rubi you'd like to see [rubi em] in the adventurer's outfit.  \"<i>Ah yes, am I going to be the Champion now?</i>\" Rubi asks, balling up [rubi eir] dainty hands and putting them on [rubi eir] hips.  \"<i>Rubi, Champion of Mareth!  Slayer of demons and ");
	if(silly()) Render.text("collector of waifus!");
	else Render.text("layer of ladies");
	Render.text("!  Has a bit of a ring to it doesn't it?</i>\"  [rubi Ey] giggles for a moment and then gathers up the clothes.");

	Render.text("\n\n[rubi Ey] starts with a simple, only slightly lacy white bra");
	if(flags[FlagEnum.RUBI_BREAST_SIZE] < 1) Render.text(", though it's obvious [rubi ey] doesn't need it with [rubi eir] flat-chest");
	Render.text(".  [rubi Ey] slips [rubi eir] arms through the holes and expertly latches the back.  Following that is a matching pair of panties that [rubi ey] slips up [rubi eir] long, smooth legs.  [rubi Ey] pulls them up so that they hug [rubi eir] round, cute ass tightly.");
	...
	if(flags[FlagEnum.RUBI_EAR_TYPE] == 1) Render.text(", covering [rubi eir] feline ears");
	Render.text(".  The boots that go with the outfit are mid-calf length and made of supple, fine leather");
	if(flags[FlagEnum.RUBI_FEET] == 1) Render.text(" with a hole cut out in the heel, apparently for Rubi's demonic heels to fit through");
	Render.text(".  Once those are on, Rubi completes the look by retrieving a wooden sword from somewhere while you were too distracted by [rubi eir] ass.");
...
private function giveRubiALongDressToHideHerHideousBody():void {
	clearOutput();
	rubiSprite();
	Render.text("You tell Rubi you'd like to see [rubi em] in the long dress.  \"<i>Oohh, I just </i>love<i> that one! One second!</i>\" [rubi ey] says as [rubi ey] pulls out the dress and several accessories.");
	Render.text("\n\nRubi starts with an off-pink strap-less bra");
	if(flags[FlagEnum.RUBI_BREAST_SIZE] < 1) Render.text(", despite the fact that [rubi ey] clearly doesn't need one");
	Render.text(", which [rubi ey] expertly fastens behind [rubi em]");
	Render.text(".  Next, [rubi ey] takes a white garter belt and slips it up [rubi eir] slender ");
	if(flags[FlagEnum.RUBI_SKIN] != 2) Render.text("hairless");
	else Render.text("furred");
	Render.text(" legs, until it rests snugly around [rubi eir] waist.  A matching pair of white stockings quickly follow, held up by the garter.  The next thing is an off-pink pair of panties that [rubi ey] slides up [rubi eir] legs and pulls up so that they cling tightly to [rubi eir] ample rump.");
	...
	if(flags[FlagEnum.RUBI_SKIN] == 1 || flags[FlagEnum.RUBI_SKIN] == 3) Render.text(" (or darkens at least)");
	Render.text(", and [rubi ey] returns to the task at hand.  The dress goes on neatly, and once it's in place you help to pull the strings on the built-in corset, tying them off with a neat little bow in back. To top it all off is a pair of pink pumps");
	if(flags[FlagEnum.RUBI_FEET] == 1) Render.text(" with the heels cut off and a hole for Rubi's demonic heel to fit through");
	Render.text(", which [rubi ey] slips on quickly.");
...
private function rubiPutsOnNursesClothesSoSheCanCheckYourTemperature():void {
	clearOutput();
	rubiSprite();
	Render.text("You tell Rubi you'd like to see [rubi em] in the nurse's uniform.  \"<i>Ohh, coming down with a bit of a fever?  Do you need a sponge bath?</i>\" [rubi ey] says coyly, before gathering up the clothing.");
	Render.text("\n\n[rubi Ey] starts with a simple, only slightly lacy white bra");
	if(flags[FlagEnum.RUBI_BREAST_SIZE] < 1) Render.text(", though it's obvious [rubi ey] doesn't need it with [rubi eir] flat-chest");
	Render.text(".  [rubi Ey] slips [rubi eir] arms through the holes and expertly latches the back.  Next [rubi ey] takes a white garter belt and slips it up [rubi eir] slender ");
	if(flags[FlagEnum.RUBI_SKIN] == 2) Render.text("furred ");
	else Render.text("hairless ");
	Render.text("legs, until it rests snugly around [rubi eir] waist.  A matching pair of white fishnet stockings quickly follow, held up by the garter.  The next thing is a pair of white cotton panties that [rubi ey] slides up [rubi eir] legs and pulls up so that they cling tightly to [rubi eir] ample rump.");
	...
	if(flags[FlagEnum.RUBI_BREAST_SIZE] < 1) Render.text("heart");
	else Render.text("left breast");
	Render.text(".  [rubi Ey] places the matching hat on [rubi eir] head and adjusts it before putting on a pair of white pumps");
	if(flags[FlagEnum.RUBI_FEET] > 0) Render.text(" with the heels removed and a hole for [rubi eir] demonic heels");
	Render.text(".");
...
private function rubiHasARubberFetish():void {
	clearOutput();
	rubiSprite();
	Render.text("You tell Rubi you'd like to see [rubi em] in the rubber fetish outfit.  \"<i>Ohoh,</i>\" [rubi ey] giggles and quirks an eyebrow at you, \"</i>feeling a bit kinky are we?</i>\"");
	...
	if(flags[FlagEnum.RUBI_SKIN] != 2) Render.text("skin");
	else Render.text("furred body");
	Render.text(", starting with [rubi eir] legs and moving up to the torso and arms.  Once Rubi's ");
	if(flags[FlagEnum.RUBI_SKIN] != 2) Render.text("skin ");
	else Render.text("fur ");
	Render.text("glistens with the oils it's time to put on the actual clothes.  ");
	//(If Not Bimbo)
	if(!rubiBimbo() || flags[FlagEnum.RUBI_DEBIMBO] > 0) Render.text("[rubi Ey] first takes a pair of transparent rubber panties and slides them up [rubi eir] legs until they lovingly cling around [rubi eir] ass.  ");
	Render.text("Rubi next slides into a transparent latex shirt, long sleeved, with holes cut out to accommodate [rubi eir] nipples.  It takes some work, and you have to help Rubi to put on the difficult garment, but once it's on, it looks fantastic, clinging to [rubi eir] body like a second skin.");
	...
	if(flags[FlagEnum.RUBI_SKIN] == 2) Render.text("furred ");
	Render.text("legs.  You wonder how [rubi ey] does this when you're not around!  Finally, Rubi slips on a pair of heels");
	if(flags[FlagEnum.RUBI_FEET] == 1) Render.text(", the actual heel cut off to make room for [rubi eir] demonic heel,");
	Render.text(" to complete the look.  All in all, [rubi ey] looks ready for a night in the dungeons.");
	Render.text("\n\nNow that Rubi is dressed up, what do you want to do?");
...
private function putOnASuitYouSlut():void {
	...
	if(flags[FlagEnum.RUBI_FEET] == 1) Render.text(", with the actual heel cut off to make room for [rubi eir] demonic heel,");
	Render.text(" and quickly combs back [rubi eir] hair.  All in all, [rubi ey] looks like a girl dressed in her boyfriend's clothes... which is to say, quite sexy.");
	Render.text("\n\nNow that Rubi is dressed up, what do you want to do?");
...
private function putOnATubeTopYouWhore():void {
	...
	if(flags[FlagEnum.RUBI_BREAST_SIZE] > 2) Render.text(", which strain the flimsy piece of material");
	Render.text(".  Completing the look is a simple pair of sandals");
	if(flags[FlagEnum.RUBI_FEET] == 1) Render.text(", with a hole cut in the sole to make room for Rubi's demonic heels");
	Render.text(".");
	Render.text("\n\nNow that Rubi is dressed up, what do you want to do?");
...
private function putOnSluttySwimwearYouSkank():void {
	clearOutput();
	rubiSprite();
	Render.text("You tell Rubi you'd like to see [rubi em] in the swimsuit.  \"<i>Oh, but there's no pool or lake nearby.  Or do you just want to see me in a bikini?</i>\"  Rubi giggles a moment.");
	...
	if(flags[FlagEnum.RUBI_BREAST_SIZE] < 3) Render.text(" sit loosely over Rubi's small chest");
	else Render.text("strain thanks to Rubi's ample busom");
	Render.text(".");

	Render.text("\n\nTopping it off, your demonic lover slips on a pair of sandals");
	if(flags[FlagEnum.RUBI_FEET] > 0) Render.text(", with a hole in the sole to make room for [rubi eir] demonic heels");
	Render.text(".");
...
private function putOnADressYouBimbo():void {
	clearOutput();
	rubiSprite();
	Render.text("You tell Rubi you'd like to see [rubi em] in the minidress.  \"<i>Oh that cute little thing we picked up together?  For sure!  It's so cute!</i>\"");
	...
	if(flags[FlagEnum.RUBI_FEET] > 0) Render.text(", with the heels removed to make room for [rubi eir] own demonic heels");
	Render.text(".");
...
private function putOnMyBodysuitYouWhore():void {
	clearOutput();
	rubiSprite();
	Render.text("You tell Rubi you'd like to see [rubi em] in the bodysuit.  \"<i>Oh, that thing is pretty cute,</i>\" [rubi ey] says, \"<i>Though there's not much to it.  But I suppose that's the point, huh?</i>\"");
	...
	if(flags[FlagEnum.RUBI_SKIN] != 2) Render.text("smooth");
	else Render.text("furred");
	Render.text(" legs, then up the rest of [rubi eir] body until [rubi ey] slips [rubi eir] arms into the long, sheer sleeves.  After a few adjustments, making sure ");
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("[rubi eir] package is neatly tucked away");
	else Render.text("a red rose is neatly covering up [rubi eir] pubic mound");
	Render.text(", and the butt of the suit is clinging perfectly to [rubi eir] own, [rubi ey] poses for you.");
...
private function putOnBondageStrapsYouBondageTrap():void {
	clearOutput();
	rubiSprite();
	Render.text("You tell Rubi you'd like to see [rubi em] in the bondage straps.  \"<i>Oh!  That outfit, huh?</i>\"  Rubi blushes brightly");
	if(flags[FlagEnum.RUBI_SKIN] > 0) Render.text(", or so you imagine");
	Render.text(".  \"<i>That one's so embarrassing... but for you, I'll wear it.</i>\"");
...
private function whyTheFuckIsRubiWearingAnInquisitorsCorset():void {
	clearOutput();
	rubiSprite();
	Render.text("You tell Rubi you'd like to see [rubi em] in the inquisitor's corset.  \"<i>Oh my, I really love that one.  It's so pretty and lacy!</i>\"");
	...
	if(flags[FlagEnum.RUBI_FEET] == 1) Render.text(", with the heels removed to make room for [rubi eir] own demonic heels");
	Render.text(".");
	Render.text("\n\nNow that Rubi is dressed up, what do you want to do?");
	//[Date (go to bar date)] [Back]
	menu();
...
private function putOnAWaitressUniformYouWhore():void {
	clearOutput();
	rubiSprite();
	Render.text("You tell Rubi you'd like to see [rubi em] in the risque version of [rubi eir] normal waitress uniform.  \"<i>Oh, you like that outfit, huh?  I thought you might,</i>\" Rubi says with a giggle.");
	Render.text("\n\nOf course, there's not much to this outfit, which starts off with a skirt so short it could be confused for a belt.  ");
	//(not bimbo)
	if(!rubiBimbo() || flags[FlagEnum.RUBI_DEBIMBO] > 0) {
		Render.text("Next [rubi ey] slips on a pair of pink frilly panties, sliding them up [rubi eir] slender legs until they come up to cling to [rubi eir] ample ass.  ");
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("Oddly enough, these panties only seem to cover Rubi's balls, leaving [rubi eir] cock right out in the open.  ");
	}
	Render.text("Taking a simple white apron, Rubi throws that on next, looping the halter-like top over [rubi eir] neck and tying off the straps around [rubi eir] waist.");
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("  To top it all off, Rubi takes a pink lacy cocksock and slips it down [rubi eir] [rubi cock], fastening it into place with a pink ribbon that [rubi ey] ties into a bow around the base of [rubi eir] cock.");

	Render.text("\n\nAfter all that, [rubi ey] simply puts on a pair of black, open-toed heels");
	if(flags[FlagEnum.RUBI_FEET] == 1) Render.text(", with the heels removed to make room for [rubi eir] own demonic heels");
	Render.text(", and poses for you in the skimpy outfit.");
	Render.text("\n\nNow that Rubi is dressed up, what do you want to do?");
...
private function dateIntro(date:Function):void {
	...
	if(flags[FlagEnum.TIMES_RUBI_DATED] == 0) {
		Render.text("An idea strikes you.  While you have talked with Rubi and gotten to know [rubi em] at work, you haven't really been out with [rubi em] anywhere else.  You wonder, and ask if Rubi would like to go out somewhere, maybe to pick up some food and talk.");
	...
	//(Repeats)
		...
		Render.text("Seeing Rubi all dressed up like this, you figure it might be time to go on another date.  You ask, and Rubi predictably makes a gleeful sound and assents.");
	}
	flags[FlagEnum.TIMES_RUBI_DATED]++;
	if(date == iceCreamDate) {
		if(player.stats.gems < 2) {//Can't Afford It?
...
private function fancyDate():void {
	...
	if(player.stats.gems >= 10) {
		...
		//(if PC has met Callu)
		if(flags[FlagEnum.MET_OTTERGIRL] == 1) Render.text(" though not as good as Callu's, the ottergirl fisherman you met at the lake.");
		else Render.text(", which makes you wonder who Rubi is thinking about.  Perhaps one of [rubi eir] parents?");
...
private function exhibitionistDate():void {
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text(rubiCock());
	else Render.text("glistening, wet pussy");
	Render.text(".");
...
private function anallyTrainYourBitchBoySlutHowToBeAnalForDCL():void {
	...
	if(flags[FlagEnum.RUBI_TIMES_ANALLY_TRAINED] == 1) {
		Render.text("You sit down on the couch next to Rubi, making small talk.  After a few minutes the two of you are laughing comfortably as you lounge on the cushions, discussing everything from life in Tel'Adre to your adventuring exploits.  Eventually the topic of conversation circles back to sex, and how you're a bit too much for [rubi em] to handle at the moment.");
		Render.text("\n\nRubi gulps visibly and nods, \"<i>I can't help it, babe.  That cock of yours looks so ");
		if(rubiBimbo() || flags[FlagEnum.RUBI_DEBIMBO] == 0) Render.text("totally ");
		Render.text("delicious, but I'm afraid it's too big!  I couldn't possibly fit that entire thing inside me... not comfortably at least.</i>\"");
		...
		if(flags[FlagEnum.RUBI_SKIN] == 1 || flags[FlagEnum.RUBI_SKIN] == 3) Render.text(", or redder at least");
		else Render.text(", what little of it you can see");
		Render.text(", and [rubi ey] sits knock-kneed behind the chest.");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text(", [rubi eir] dick wobbling enticingly");
		Render.text(".");
		...
		if(flags[FlagEnum.RUBI_NIPPLE_TYPE] == 1) Render.text("black ");
		Render.text("nipples, which jut straight up, straining against the sheer fabric of the negligee.  A soft gasp escapes Rubi's soft lips as [rubi ey] pinches one ");
		if(flags[FlagEnum.RUBI_NIPPLE_TYPE] == 1) Render.text("ebon ");
		Render.text("nipple, while [rubi eir] other hand flutters down between [rubi eir] thighs to stroke [rubi eir] ");
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text(rubiCock());
		else Render.text("quickly dampening pussy");
		Render.text(".");
		...
		Render.text(" lips, licking the ");
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("pre-cum");
		else Render.text("juices");
		Render.text(" off of them with a smile.  [rubi Eir] newly cleaned fingers move to the bottle of lube sitting nearby.  After pumping a handful into [rubi eir] palm, the rich scent of raspberries seems to fill the air, making your nostrils twitch.  Scented lube?");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] == 0) Render.text("slick vagina");
		else Render.text("hungry asshole");
		Render.text(".");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] == 0) Render.text("cunt");
		else Render.text("asshole");
		Render.text(" clinging so tightly to the toy you imagine it must be like a vice.");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text(rubiCock() + " is standing at full attention, bobbing up and down as Rubi fucks [rubi emself] on the dildo");
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0 && flags[FlagEnum.RUBI_NO_CUNT] == 0) Render.text(", and [rubi eir] ");
		if(flags[FlagEnum.RUBI_NO_CUNT] == 0) Render.text("cunt lips are glistening red with arousal, just begging for attention");
		Render.text(".  Rubi's cries grow louder, [rubi eir] lips parting and [rubi eir] tongue lolling out as [rubi ey] pants for breath.  You can tell [rubi ey]'s building up to something, something powerful at that.");

		Render.text("\n\n[rubi Eir] hips spin and oscillate, working vigorously to keep [rubi eir] ");
		if(flags[FlagEnum.RUBI_NO_CUNT] == 1) Render.text("backside");
		else Render.text("fuck hole");
		...
		if(flags[FlagEnum.RUBI_NO_CUNT] == 1) Render.text("asshole");
		else Render.text("cunt");
		Render.text(", seeking freedom from its sweet, fleshy prison.  A long supine sigh escapes your slender lover's mouth.  [rubi Ey]'s clearly exhausted and happy for this rapturous release.");
		...
		if(flags[FlagEnum.RUBI_NO_CUNT] == 1) Render.text("ass");
		else Render.text("pussy");
		Render.text(".");
		...
		if(flags[FlagEnum.RUBI_EAR_TYPE] == 0) Render.text("ear");
		else if(flags[FlagEnum.RUBI_EAR_TYPE] == 1) Render.text("feline ear");
		else Render.text("feathered ear");
		Render.text(" that [rubi ey] should keep that in for a few hours, just to make sure this training takes hold.  Rubi gives you a small smile in response along with a small peck on the cheek before slipping under the sheets for a quick nap.");
	...
	//Rubi's capacity won't go larger than 100.
		...
		Render.text("You bring up the subject of training Rubi's ass again.  [rubi Ey] looks up at you with a sheepish smile on [rubi eir] face and confesses, \"<i>I was hoping you'd say that, babe.  I've actually been practicing a little bit on my own.  I... Well, I've been itching to get that bad boy you've got inside me.</i>\"");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text(rubiCock());
		else Render.text("sweet pussy");
		Render.text(" is readily on display, and it's all for you.  Rubi's makeup is less than subtle, with bright red lips and a light dusting of silver eyeshadow, enough to make [rubi eir] eyes pop and not clash with the outfit.  Completing the look is a pair of white fishnet stockings that go up [rubi eir] legs, ending about mid-thigh.");
		...
		Render.text("'s mouth, exploring every inch and entwining with Rubi's own.");
		...
		if(flags[FlagEnum.RUBI_SKIN] == 0) Render.text("go bright red");
		else if(flags[FlagEnum.RUBI_SKIN] == 1 || flags[FlagEnum.RUBI_SKIN] == 3) Render.text("go a vivid purple");
		else Render.text("would be blushing bright red now, were it not for the fine layer of fur");
		Render.text(", and [rubi ey] snaps back to the present.  \"<i>Eheh... I guess I deserved that.</i>\"");
		...
		Render.text(", making sure to cover every inch.  You're certain you even see a few fingers press inside [rubi eir] hole.");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("asshole");
		else Render.text("cunt");
		Render.text(" is stretched nearly to its limits.  Finally [rubi ey] lets out a sigh of relief as the knot pops completely into place.  \"<i>Oh... fuck me it's so big,</i>\" [rubi ey] squeals, [rubi eir] body visibly shaking.  Rubi's tongue lolls out, dripping a bit of spittle down [rubi eir] chin.");
		...
		if(flags[FlagEnum.RUBI_NO_CUNT] == 1) Render.text("anus");
		else Render.text("cunt");
		Render.text(" just widened slightly.  Rubi's eye go wide as [rubi ey] looks up at you with a mouth full of cock, moaning something you can't understand.");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("You can feel your lover's cock straining against your leg, aching for some release.  You idly press your foot against it, feeling a slick layer of precum coating it from top to bottom already.  ");
		Render.text("A pleading look enters Rubi's eyes.  You hold up the pump and ask [rubi em] if [rubi ey] wants you to use it again.  [rubi Ey] gives [rubi eir] head a quick nod of assent, and reaffirms this by taking your [cock biggest] even deeper into [rubi eir] mouth.");

		Render.text("\n\nYou give it a quick pump, followed by a second, final one.  Rubi's ");

		if(flags[FlagEnum.RUBI_EYE_TYPE] == 0 || rubiBimbo()) Render.text("green");
		else if(flags[FlagEnum.RUBI_EYE_TYPE] == 1) Render.text("purple");
		else Render.text("shiny black");
		Render.text(" eyes close tight as [rubi ey] succumbs to a brain-melting orgasm.  [rubi Eir] entire body trembles, from [rubi eir] curling toes right up [rubi eir] twitching, painted fingers.");
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("  Even, no, <i>especially</i> [rubi eir] cock twitches and spasms jerking upwards and splattering globules of sticky white goo against the inside of Rubi's babydoll before dripping down onto the floor between [rubi eir] legs, creating a rapidly growing puddle.");
		...
		Render.text(", to make [rubi em] squeal from your actions, and not that of a lifeless dildo.  You imagine the " + rubiMF("demon","demoness") + "'s hot, wet walls clamped around you, milking you for everything you've got.");
...
private function getFuckedByRubi():void {
	Render.text(images.showImage("rubi-fucks-you"), false);
	clearOutput();
	rubiSprite();
	Render.text("Rather than wait for you to initiate something, Rubi takes you by the hands and leads you into the bedroom.  [rubi Ey] puts one arm on your shoulder and runs [rubi eir] fingers up through the hair on the back of your head before pulling you into a deep kiss.  Rubi's ");
	//(normal/furry skin)
	if(flags[FlagEnum.RUBI_SKIN] == 0 || flags[FlagEnum.RUBI_SKIN] == 2) Render.text("red");
	else Render.text("black");
	Render.text(" lips feel so delicate against yours, but they quickly part as [rubi eir] tongue slithers between them, exploring the depths of your warm, wet mouth.");
	...
	if(flags[FlagEnum.RUBI_EYE_TYPE] > 0) Render.text(" slitted");
	Render.text(" eyes constantly meet yours, looking for approval, before and after every act.  You're certain you're still the one in charge here... but you're simply letting Rubi take the lead.");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) Render.text("asshole");
	else Render.text("cunt");
	Render.text(" of yours all ready for me.</i>\"  Rubi's head descends between your legs");
	if(player.lowerBody.cockSpot.hasCock()) Render.text(", ignoring your " + multiCockDescriptLight() + " completely,");
	Render.text(" and presses [rubi eir] face into your [vagOrAss], inhaling deeply.  [rubi Eir] lips pucker as [rubi ey] plants a kiss directly onto your ");
	if(!player.lowerBody.vaginaSpot.hasVagina()) Render.text("ass");
	else Render.text("moistening pussy");
	Render.text(".  You let out a small moan and lean backwards, taking your eyes off the ");
	if(flags[FlagEnum.RUBI_HAIR] == 0) Render.text("black-haired");
	else Render.text("blonde");
	Render.text(" head betwixt your thighs and taking in the sensations.");
	...
	if(this.rubiGetCockType() == CockType.HUMAN) Render.text("pink, mushroom-like tip");
	else if(this.rubiGetCockType() == CockType.DEMON) Render.text("purple, nodule-laden tip");
	else if(this.rubiGetCockType() == CockType.HORSE) Render.text("blunted, musky tip");
	else if(this.rubiGetCockType() == CockType.TENTACLE) Render.text("tentacle-ringed crown");
	else Render.text("<b>ERROR: Rubi Cock Type set invalid. Currently: " + this.rubiGetCockType() + "</b>  ")
	Render.text(" emerges from the foreskin surrounding it as [rubi eir] cock strains, aching for something, anything to fill.");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) player.buttChange(flags[FlagEnum.RUBI_COCK_SIZE] * flags[FlagEnum.RUBI_COCK_SIZE]/6,true,true,false);
	else player.cuntChange(flags[FlagEnum.RUBI_COCK_SIZE] * flags[FlagEnum.RUBI_COCK_SIZE]/6,true,true,false);
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) Render.text("prostate");
	else Render.text("g-spot");
	Render.text(" briefly, but enough to send a hot shiver up your spine.  Seeing this reaction, Rubi's smile widens into a grin and begins to thrust in and out at a slow pace.  Every thrust brushes past your sensitive ");
	if(!player.lowerBody.vaginaSpot.hasVagina()) Render.text("prostate");
	else Render.text("g-spot");
	Render.text(", and one by one your limbs start to tingle.  In and out, [rubi ey] goes, and you can feel something building inside you.");
	...
	if(flags[FlagEnum.RUBI_EYE_TYPE] > 0) Render.text(" slitted");
	Render.text(" eyes, \"<i>Can I come inside you?</i>\"");
	...
	if(flags[FlagEnum.RUBI_BLU_BALLS] < 4) {}
		...
		Render.text("There's so much of it that it oozes out from your abused ");
		if(!player.lowerBody.vaginaSpot.hasVagina()) Render.text("ass");
		else Render.text("cunt");
		Render.text(", dribbling down your buttcheeks and pooling onto the bed and floor below.  ");
	}
	//(Blueballs 6-9)
	else if(flags[FlagEnum.RUBI_BLU_BALLS] <= 9) {
		Render.text("Your insides rumble suddenly as Rubi's impressive load begins to fill you up, [rubi eir] seed slipping into every available nook and cranny.  What doesn't fit slips out of your abused ");
		if(!player.lowerBody.vaginaSpot.hasVagina()) Render.text("ass");
		else Render.text("cunt");
		Render.text(" in a small torrent, pooling onto the bed and floor below.  ");
	}
	//(Blueballs 10)
	else Render.text("Your insides rumble dangerously and your stomach actually swells up, gaining a noticeable paunch as Rubi's lavish load coats your insides, filling every available nook and cranny and even creating new ones.  What doesn't fit slips out of your abused hole in a flood, coating the bed and floor beneath you in a slick, white glaze.  ");
	Render.text("Rubi doesn't stop there, however; [rubi ey] continues to assail your [vagOrAss], bringing you to several more miniature, body-shaking orgasms, until [rubi ey] just can't handle any more, and [rubi ey] pulls out, [rubi eir] cock still rather erect and looking cozy in its jacket-like foreskin.");
...
private function rubiHotdogging():void {
	...
	if(flags[FlagEnum.RUBI_SKIN] == 0) Render.text("pale skinned");
	else if(flags[FlagEnum.RUBI_SKIN] == 1 || flags[FlagEnum.RUBI_SKIN] == 3) Render.text("red skinned");
	else Render.text("fur-covered");
	Render.text(" " + rubiMF("boy","girl") + " lays naked on your lap.  Rubi giggles and dips a hand between [rubi eir] thighs ");
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("taking hold of [rubi eir] cock");
	else Render.text("rubbing a finger along [rubi eir] clit");
	Render.text(" while [rubi ey] says, \"<i>So babe, what do you intend to do?  Are you going to hold me down and ravish me?  What will it be?</i>\"");
	...
	if(flags[FlagEnum.RUBI_SKIN] != 2) Render.text("tender skin");
	else Render.text("silky fur");
	Render.text(".  Rubi's back arches and a soft gasp escapes [rubi eir] lips.  [rubi Eir] body reflexively jerks towards your hand, hoping for more than a simple teasing touch, but you pull your hand away and give [rubi eir] round ass a playful smack, chastising [rubi em].");
	...
	if(flags[FlagEnum.RUBI_NO_CUNT] == 0) Render.text("cunt moistening");
	if(flags[FlagEnum.RUBI_NO_CUNT] == 0 && flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text(" and ");
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("cock hardening");
	Render.text(".  [rubi Ey] obviously enjoys this light, playful teasing.  You give Rubi's ass another playful smack, watching [rubi eir] ");
	if(flags[FlagEnum.RUBI_SKIN] == 0) Render.text("pale skin");
	else if(flags[FlagEnum.RUBI_SKIN] == 1|| flags[FlagEnum.RUBI_SKIN] == 3) Render.text("crimson skin");
	else Render.text("fur-covered flesh");
	Render.text(" jiggle with a smile.");
	...
	if(flags[FlagEnum.RUBI_SKIN] != 2) Render.text("skin");
	else Render.text("coat");
	Render.text(" against your manhood as you slowly begin to pump in and out of Rubi's ass cheeks.");
	...
	Render.text(", while your index and middle fingers rub slowly around [rubi eir] ");
	if(flags[FlagEnum.RUBI_NO_CUNT] == 0) Render.text("clit");
	else Render.text("perineum");
	Render.text(".");
...
private function teaseButtjobs():void {
	Render.text(images.showImage("rubi-tease-buttjob"), false);
	clearOutput();
	rubiSprite();
	Render.text("Grinning to yourself, you decide to tease the little demon " + rubiMF("boy","girl") + " a little further.  You plunge your thumb further into [rubi eir] ");
	if(flags[FlagEnum.RUBI_NO_CUNT] == 1) Render.text("ass");
	else Render.text("pussy");
	Render.text(", finding [rubi eir] ");
	if(flags[FlagEnum.RUBI_NO_CUNT] == 1) Render.text("prostate");
	else Render.text("g-spot");
	Render.text(" and rubbing circles around it, drawing out a long moan from Rubi's smeared lips.  With every thrust into Rubi's soft ass-pillows you draw another loop around [rubi eir] most sensitive spot with your digit, until you can feel [rubi eir] body start to tense up.");
	...
	if(player.cumQ() <= 50) Render.text("in one go, with a small, proud look on [rubi eir] face");
	else if(player.cumQ() <= 150) Render.text("with one, throat-straining swallow, a small, proud smile taking [rubi eir] face when [rubi ey] stops panting for breath");
	else Render.text("as it comes, mouthful after mouthful of cum practically injected straight into [rubi eir] stomach. There's so much of it, [rubi ey] can't help but smile mid-way through, proud to take such a huge load");
	if(player.cumQ() >= 1000) Render.text(".  After guzzling down so much, you're certain Rubi's actually sporting a bit of a belly now, almost as if [rubi ey] were several months pregnant");
	Render.text(".");
...
private function popButtjobs():void {
	Render.text(images.showImage("rubi-pop-buttjob"), false);
	clearOutput();
	rubiSprite();
	Render.text("No, you figure Rubi has been so great lately [rubi ey] definitely deserves to cum.  You plunge your thumb further into [rubi eir] ");
	if(flags[FlagEnum.RUBI_NO_CUNT] == 1) Render.text("ass");
	else Render.text("pussy");
	Render.text(", finding [rubi eir] ");
	if(flags[FlagEnum.RUBI_NO_CUNT] == 1) Render.text("prostate");
	...
	Render.text("\n\nRubi's ");
	if(flags[FlagEnum.RUBI_NO_CUNT] == 0) Render.text("cunt clamps down on your thumb and gushes, coating your hand in [rubi eir] slick juices");
	else Render.text("asshole quivers and attempts to suck in more of your thumb before clamping down, leaking slick fluids all over your hand");
	Render.text(".  [rubi Eir] entire body spasms as the climax wracks it, one sexy leg suddenly jerking back and forth uncontrollably.  Rubi grips at a pillow, biting it and stifling another orgasmic moan.");
	//(If Rubi has Penis)
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) Render.text("  Beneath [rubi em], [rubi eir] cock jerks and twitches, loosing a spray of pearly white cum which splatters against the couch below");
	if(flags[FlagEnum.RUBI_BLU_BALLS] < 4) {}
	//((blueballs 4-5)
	else if(flags[FlagEnum.RUBI_BLU_BALLS] <= 5) Render.text(" which is quickly followed by another, more powerful stream of cum, which splashes against [rubi eir] stomach, coating [rubi eir] [rubi breasts] in semen");
	else if(flags[FlagEnum.RUBI_BLU_BALLS] <= 7) Render.text(" which is quickly followed by another, more powerful stream of cum, and a third, and a fourth.  Before you know it, Rubi's entire torso, including [rubi eir] [rubi breasts] are practically glazed with cum");
	else if(flags[FlagEnum.RUBI_BLU_BALLS] <= 9) Render.text(" which is followed up by several more, each seemingly more poweful than the one previous.  Rubi's body twitches and shudders with every jet, until [rubi eir] entire torso, and the couch, is covered in the musky jizz");
	else Render.text(" which is followed by a dozen more, each one thicker and more powerful than the last.  Rubi's cock twitches uncontrollably, just like [rubi eir] body, as it coats the entire couch, [rubi eir] entire upper body, and even some of the wall five feet away in the musky stuff");
	Render.text(".");
	...
	if(player.lowerBody.balls > 1) Render.text("churn");
	else Render.text("churns");
	Render.text(" pleasurably, your [cock biggest] stiffens suddenly, and you let loose a long, drawn-out sigh as your dick erupts, jetting cum upwards, which immediately splatters against Rubi's tail and back before dripping down [rubi eir] bountiful asscheeks.");
	...
	Render.text("You laugh and give the demon-" + rubiMF("boy","girl") + " a kiss, remarking how lucky you are to be with [rubi em].");
...
private function giveRubiATFItem(itype:ItemType):void {
	...
	if(flags[FlagEnum.RUBI_TIMES_GIVEN_AN_ITEM] == 0) {
		Render.text("Rubi looks at the offered item, curiously.  [rubi Ey] turns it over in [rubi eir] hands, carefully examining the liquid within.  Finally [rubi ey] asks apprehensively, \"<i>Is this going to... change my body?</i>\"");
	...
	if(itype == consumables.SUCMILK || itype == consumables.P_S_MLK) {
		if(itype == consumables.SUCMILK) {
			...
			Render.text("You hand over the bottle of milky white fluids to Rubi.  [rubi Ey] takes a moment to read over the pink label before pulling out the cork and downing the whole bottle in one gulp.");
		}
		else {
			...
			Render.text("You hand over the bottle of purified milky white fluids to Rubi.  [rubi Ey] takes a moment to read over the pink label before pulling out the cork and downing the whole bottle in one gulp.");
		}
		...
		if((flags[FlagEnum.RUBI_COCK_SIZE] > 4) && (!flags[FlagEnum.HYPER_HAPPY])) {
			...
			Render.text(Math.round(flags[FlagEnum.RUBI_COCK_SIZE]*10)/10 + "-inch penis.");
		}
		//(If Penis is removed)
		...
		if(rand(3) == 0 && flags[FlagEnum.RUBI_NO_CUNT] > 0) {
		...
		if(flags[FlagEnum.RUBI_FEET] == 0 && rand(3) == 0 && itype == consumables.SUCMILK) {
		...
		//(If Rubi gets horns)
		if(rand(3) == 0 && flags[FlagEnum.RUBI_HORNTYPE] == 0 && itype == consumables.SUCMILK) {
		...
		if(rand(3) == 0 && flags[FlagEnum.RUBI_SKIN] != 1 && itype == consumables.SUCMILK) {
			Render.text("\n\nRubi clutches [rubi eir] stomach suddenly as you notice red spots appearing on [rubi eir] ");
			if(flags[FlagEnum.RUBI_SKIN] != 2) Render.text("skin");
			else Render.text("coat of fur");
			Render.text(".  The blotches soon spread, rapidly overtaking [rubi eir] body.  ");
			if(flags[FlagEnum.RUBI_SKIN] == 2) Render.text("Meanwhile the silky soft fur that covered [rubi eir] body seems to be falling out as this red menace takes over.  ");
			Render.text("In no time at all, [rubi eir] smooth, hairless skin now appears to be a deep crimson, matching the color of [rubi eir] spade tail that [rubi ey] received long ago.");
		...
		if(itype == consumables.INCUBID) Render.text("You hand over the bottle of milky white fluids to Rubi.  [rubi Ey] takes a moment to read over the label before pulling out the cork and downing the whole bottle in one gulp.");
		...
		else Render.text("You hand over the bottle of purified milky white fluids to Rubi.  [rubi Ey] takes a moment to read over the pink label before pulling out the cork and downing the whole bottle in one gulp.");
		...
		//(If Rubi has no penis; instead)
			...
			if(flags[FlagEnum.RUBI_SKIN] == 1 || flags[FlagEnum.RUBI_SKIN] == 3) Render.text("red");
			else Render.text("pink");
			Render.text(" protrusion... a brand new cock!  Rubi shudders, [rubi eir] delicate little fingers wrapped around the new, five-inch cock.");
			flags[FlagEnum.RUBI_COCK_SIZE] = 5;
		}
		//(If Vagina removed)
		if(flags[FlagEnum.RUBI_NO_CUNT] == 0 && rand(3) == 0) {
		...
		//(If Rubi gets red skin)
		if(rand(3) == 0 && flags[FlagEnum.RUBI_SKIN] != 1 && itype == consumables.INCUBID) {
			Render.text("\n\nRubi clutches [rubi eir] stomach suddenly as you notice red spots appearing on [rubi eir] ");
			if(flags[FlagEnum.RUBI_SKIN] != 2) Render.text("skin");
			else Render.text("coat of fur");
			Render.text(".  The blotches soon spread, rapidly overtaking [rubi eir] body.  ");
		...
		//(If Rubi gets horns)
		if(rand(3) == 0 && flags[FlagEnum.RUBI_HORNTYPE] == 0 && itype == consumables.INCUBID) {
			Render.text("\n\nRubi's hands suddenly dart up, clutching [rubi eir] head as though [rubi ey] has a massive headache.  [rubi Ey] feels around blindly, trying to locate the source of the pain, and then you see it.  Two small bulges appear on [rubi eir] forehead, just at the hairline.  They grow larger and larger until the skin splits cleanly open revealing shiny black bone.  Horns.  They grow up and backwards, forming light ridges.  They don't quite grow completely past [rubi eir] head, however, before the growth stops.  Rubi calms and stands.  [rubi Eir] hands roam over every inch of [rubi eir] new addition as glances in the mirror, marveling at [rubi eir] demonic appearance.");
			flags[FlagEnum.RUBI_HORNTYPE] = 1;
		}
	}
	...
	//Bimbo Liqueur (Modified for variant genders)
		...
		Render.text("You hold out the bottle of Bimbo Liqueur.  Rubi takes it, and looks it over tentatively.  \"<i>Bimbo... liqueur?</i>\" [rubi ey] asks, quirking an eyebrow at you.  \"<i>But what does this do?  There's a huge warning label right here.  Effects are permanent, strong...</i>\" [rubi eir] voice drifts off as [rubi ey] reads.");
		...
		Render.text(" hair suddenly bursts outwards, long platinum blonde locks flow past [rubi eir] shoulders like a waterfall.  [rubi Eir] already dainty hands grow more feminine, as do [rubi eir] feet.");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) {
			...
			if(flags[FlagEnum.RUBI_NO_CUNT] == 0) Render.text("[rubi eir] vagina");
			else Render.text("a pristine new vagina");
			Render.text(".  It's pretty clear, this former male is now completely female.  Rubi collapses backwards onto the couch, the transformations seemingly done.");
			flags[FlagEnum.RUBI_COCK_SIZE] = 0;
			flags[FlagEnum.RUBI_NO_CUNT] = 0;
			flags[FlagEnum.RUBI_SHE] = 1;
		}
		...
		if(flags[FlagEnum.RUBI_BREAST_SIZE] < 7) {
			...
			if(flags[FlagEnum.RUBI_NIPPLE_TYPE] > 0) Render.text("black ");
			Render.text("nipples begins to tremble and then expand.  You swear you hear a whooshing sound, like someone is blowing up a balloon.");
			//(if flat-chested)
			if(flags[FlagEnum.RUBI_BREAST_SIZE] == 0) Render.text("  Within seconds, Rubi has the chest of a pubescent girl, within a minute they're at least a B-cup.");
			Render.text("  You both sit with morbid curiosity, watching the flesh grow outwards, bit by bit and cup by cup.  After five minutes the growth stops, and Rubi carefully cradles [rubi eir] new enormous E-cup breasts.");
			flags[FlagEnum.RUBI_BREAST_SIZE] = 7;
		}
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) Render.text("yours");
		else Render.text("you");
		Render.text(".  \"<i>Like, thank you so much baby!  You're the freaking best!  Now... you want to break this body in?</i>\"");
	...
	//Grows cock. (As Incubus Draft above)
		...
		if(this.rubiGetCockType() != CockType.HORSE) {
			...
			if(flags[FlagEnum.RUBI_BLU_BALLS] >= 4 && flags[FlagEnum.RUBI_BLU_BALLS] < 9) Render.text("swollen ");
			else if(flags[FlagEnum.RUBI_BLU_BALLS] >= 9) Render.text("engorged ");
			if(flags[FlagEnum.RUBI_BALLS_TYPE] == 1) Render.text("tight, trappy ");
			Render.text("ballsack.  The colors move forward, seemingly stopping at the thick ring that marks the edge of [rubi eir] sheathe... except patches of  dusky gray appear along the next few inches of [rubi eir] length, creating a nice contrast and pattern with the bright pink of [rubi eir] cockflesh.");
			...
			flags[FlagEnum.RUBI_COCK_SIZE] += 3 + rand(5);
		}
	}
	//Trap Oil
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) {
			Render.text("  Rubi's [rubi cock] twitches, already leaking precum from its tip as it attempts to reach the skies above");
		}
		else Render.text("  Rubi's cunt glistens, soaked through entirely thanks to your ministrations");
		if(flags[FlagEnum.RUBI_BREAST_SIZE] > 0) Render.text(", while [rubi eir] [rubi breasts] gently rock up and down with the motions of [rubi eir] breathing");
		Render.text(".");
	...
	//Progression of furriness: cat ears, then whiskers and paws, then cat penis, then furry body.
		...
		Render.text("You hand over the tiny peach-like fruit to Rubi.  [rubi Ey] turns it over in [rubi eir] hands and remarks, \"<i>How can you like these things?  They're so... furry!  Well, to each their own I suppose.</i>\"  Rubi gulps visibly, preparing [rubi em]self for the whiskery fruit before biting down.  Apparently, it's not as bad as [rubi ey] thought it was, as [rubi ey] makes an appreciative grunt and consumes the fruit down to the pit.");
	...
	if(itype == consumables.PURPEAC) {
		...
		if(flags[FlagEnum.RUBI_SKIN] == 0) Render.text("skin");
		else if(flags[FlagEnum.RUBI_SKIN] == 1) Render.text("red skin");
		else if(flags[FlagEnum.RUBI_SKIN] == 2) Render.text("furred skin");
		else Render.text("striped skin");
		Render.text(" underneath.  The demon-marked " + rubiMF("boy","girl") + " rubs [rubi eir] stomach appreciatively.  \"<i>Those were some great peaches, babe.</i>\"");
		...
		if (flags[FlagEnum.RUBI_EAR_TYPE] != 2 || flags[FlagEnum.RUBI_SKIN] != 3 || (flags[FlagEnum.RUBI_COCK_SIZE] > 0 && this.rubiGetCockType() != CockType.TENTACLE))
		{
			...
			if(flags[FlagEnum.RUBI_EAR_TYPE] != 2) {
				...
				if(flags[FlagEnum.RUBI_EAR_TYPE] == 0) Render.text("normal");
				else Render.text("feline");
				Render.text(" ears seem to pull into [rubi eir] skull before disappearing, leaving nothing but skin and hair behind.  You wonder if Rubi's lost [rubi eir] ears permanently, but as the thought finishes crossing your mind you notice two small nubs growing where normal ears would be.");
			...
			//(Shark Skin)
				...
				if(flags[FlagEnum.RUBI_SKIN] == 0) Render.text("its normal paleness replaced with a vibrant cherry red");
				else if(flags[FlagEnum.RUBI_SKIN] ==1 ) Render.text("which is barely noticeable on [rubi eir] already cherry red flesh");
				else if(flags[FlagEnum.RUBI_SKIN] == 2) Render.text("the fur rapidly retreating and replaced with bright red flesh underneath");
				Render.text(", but it doesn't seem to be even.  The majority of Rubi's body is cherry red, while [rubi eir] belly actually lightens, becoming a sweet, silvery gray.  A number of similarly silver stripes appear, seemingly at random, up and down Rubi's arms and legs, and even down [rubi eir] demonic tail, amazingly enough.");
			...
			//(Anemone Penis, must have a penis)
				...
				if(this.rubiGetCockType() != CockType.HUMAN) Render.text(", first shifting towards a more human-like appearance");
				Render.text(".  Small nodules appear, around the crown of the penis, and around its base.  Rubi's delicate hands run across two of the nodules, and [rubi eir] cock suddenly stiffens, harder than you've ever seen it!  Whatever those things are, they're quite sensitive.");
...
private function rubiGrowPlusBreasts():void {
	Render.text(images.showImage("rubi-gro-plus-tits"), false);
	clearOutput();
	rubiSprite();
	Render.text("You hand over the vial of blueish liquid.  Rubi takes a moment to look it over, before taking a deep breath and injecting half of the vial into one breast and the rest into [rubi eir] other breast.");
	...
	if(flags[FlagEnum.RUBI_BREAST_SIZE] < 7 || (flags[FlagEnum.RUBI_BREAST_SIZE] < 30 && flags[FlagEnum.HYPER_HAPPY])) {
		...
		flags[FlagEnum.RUBI_BREAST_SIZE] += 2 + rand(2);
		if (flags[FlagEnum.HYPER_HAPPY])
		{
...
private function rubiPenisGroPlus():void {
	Render.text(images.showImage("rubi-gro-plus-penis"), false);
	clearOutput();
	rubiSprite();
	Render.text("You hand over the vial of greenish liquid.  Rubi takes a moment to look it over, before taking a deep breath and injecting the entire thing into the base of [rubi eir] [rubi cock].");
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] < 12 || (this.rubiGetCockType() == CockType.HORSE && flags[FlagEnum.RUBI_COCK_SIZE] < 20) || (flags[FlagEnum.RUBI_COCK_SIZE] < 30 && flags[FlagEnum.HYPER_HAPPY])) {
		...
		flags[FlagEnum.RUBI_COCK_SIZE] += 2 + rand(2);
		...
		if(this.rubiGetCockType() == CockType.HORSE) Render.text("sheath-stuffing horse-dick");
		else Render.text("giant dick.");
	}
	player.consumeItem(consumables.GROPLUS);
	menu();
	addButton(0,"Next",pickAnItemToFeedRubi);
...
private function rubiBoobsReducto():void {
	clearOutput();
	rubiSprite();
	Render.text("You hand over the jar of paste.  Rubi looks it over and, after giving it a sniff, uses two fingers to scoop up some of the pale substance, smearing it over [rubi eir] breasts.  Before long, the jar is empty and [rubi eir] [rubi breasts] are covered in the stuff.");
...
private function rubiPenisReducto():void {
	clearOutput();
	rubiSprite();
	Render.text("You hand over the jar of paste.  Rubi looks it over and, after giving it a sniff, uses two fingers to scoop up some of the pale substance, smearing it over [rubi eir] penis.  Before long, the jar is empty and [rubi eir] [rubi cock] is covered in the stuff.");
...
private function pickAnItemToFeedRubi():void {
	...
	if (closet.length > 0) {
		Render.text("<b>Rubi's Closet:</b>");
		for (let x: number = 0; x < closet.length; x++) Render.text("\n" + closet[x]);
		Render.text("\n\n");
	}
	
	if (gifts.length > 0) {
		Render.text("After a quick glance around Rubi's things, you bet [rubi ey] could really use... " + gifts[rand(gifts.length)] + ".\n\n");
	}
	else {
		Render.text("You've given Rubi all the clothes [rubi ey] would want to make use of.");
		if (silly()) Render.text("  (Achievement Unlocked: Dress-tacular)");
		Render.text("\n\n");
	}
	
	if (!rubiBimbo() && !rubiIncubus() && rubiAffection() >= 100) {
		Render.text("You could give Rubi some incubi draft to make [rubi em] a little more manly but corrupt, or you could give Rubi a Bimbo Draft to turn [rubi em] into a smoking hot, female sex-bomb.\n\n");
	}
	Render.text("What will you give [rubi em]?");
...
public function giveRubiClothes(itype:ItemType = null):void {
	clearOutput();
	rubiSprite();
	Render.text("You hand over the spare set of clothes, and Rubi's eyes light up.  \"<i>For me?</i>\"  the little demon practically screams, ecstatic.  \"<i>ThankyouthankyouthankyouTHANKYOU!</i>\"");
...
{
	clearOutput();
	Render.text("You slither out of your [armor], flicking your tongue hungrily at the air, tasting the petite boy's desire. He is so attracted to you, and yet, he's trying so hard to be classy about it. It's cute in its own way, but you'd rather see him be more... honest about his drives. You want the cute, ");
	if(flags[FlagEnum.RUBI_BREAST_SIZE] < 1) Render.text("femmy boy");
	else if(flags[FlagEnum.RUBI_NO_CUNT] > 0) Render.text("dick-girl");
	else Render.text("little slut");
	Render.text(" ready to lust after your cock openly, to cling to you whenever you arrive and grope your bulge in public. He needs to be less worried about propriety and more focused on sexual satiation and appeal... yes.");
	...
	Render.text(" whipped out and swaying with your sinuous movements, and all he has is his comparatively undersized little pecker. You rock your whole body with the snake-like grace given to you by your naga body, swaying rhythmically as you meet his eyes. Knowing full well just what kinds of depravity you'd like to force him into, you feel a ");
	if(kGAMECLASS.monk >= 5) Render.text("familiar ");
	else Render.text("strange ");
	Render.text("dark power welling up within you.");
	...
	if(flags[FlagEnum.RUBI_NO_CUNT] > 0) Render.text("femboy");
	else Render.text("dickgirl");
	Render.text(" must have quite the mind on him if it's making him drip THAT fast.");
	...
	Render.text("\n\nBeyond words, the ");
	if(flags[FlagEnum.RUBI_NO_CUNT] == 0) Render.text("dick-");
	Render.text("girl");
	if(flags[FlagEnum.RUBI_NO_CUNT] > 0) Render.text("y-boy");
	Render.text(" simply gives the slightest of nods as a reply.");
	...
	Render.text("\n\n<b>Rubi is now a ");
	if(flags[FlagEnum.RUBI_NO_CUNT] > 0) Render.text("shemale ");
	Render.text("bimbo! She's going to be a lot more lusty and airheaded, but isn't that what you wanted?</b>");
	flags[FlagEnum.RUBI_BIMBO] = 1;
	flags[FlagEnum.RUBI_SHE] = 1;
