...
internal function rubiIntros():Function {
	...
	else if (rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) outputText("Rubi is here, moving from table to table, not quite as efficiently as [rubi ey] used to, but with a lot more exuberance.  [rubi Eir] long red tail swishes around, seemingly drawn towards the crotch of patrons and co-workers alike.");
...
public function approachRubiScenes():void {
	...
	//(Approach repeat, relationship 20+.  At this stage, randomly select an outfit from what the PC has given him, plus his waitress outfit.  Variants included for Bimbo /Incubus Rubi.)
		...
		if(select == 1) {
			...
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) outputText("  Rubi's breasts strain against the tight blouse, threatening to pop a button at any moment.");
			if(flags[FlagEnum.RUBI_COCK_SIZE] >= 9) outputText("  You notice Rubi holds [rubi emself] a little awkwardly, and it's only when [rubi ey] sits down that you spy the slight bulge in [rubi eir] skirt.");
		}
		//(Suit Clothes (Normal))
		else if(select == 2) {
			//(Suitclothes)
			...
			if(flags[FlagEnum.RUBI_HORNTYPE] > 0) outputText("just behind the horns");
			outputText(".  In a sexy sort of way, [rubi ey] looks like a girl dressed in her boyfriend's clothes");
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) outputText(", though with [rubi eir] large bosom, [rubi ey] can't quite button up all the way, giving the appearance of a sexy secretary");
			outputText(".");
		}
		else if(select == 3) {
			...
			if(flags[FlagEnum.RUBI_NIPPLE_TYPE] > 0) outputText("black ");
			...
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) outputText(" impressive bosom, and sort of makes [rubi em] look like the kind of rubber doll one might buy for those lonely nights.");
			...
			//(D breasts or higher)
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) outputText("  [rubi Eir] breasts strain the fabric of the tube top, any wrong move threatening to have them burst forth.");
			//(dick 9" or higher)
			if(flags[FlagEnum.RUBI_COCK_SIZE] >= 4) outputText("  [rubi Eir] rather large bulge can't be concealed, however, and you sometimes catch Rubi holding [rubi eir] notepad down across [rubi eir] crotch when [rubi ey] moves from table to table.");
			...
			if(flags[FlagEnum.RUBI_COCK_SIZE] >= 9) outputText("  However, with an outfit this tight, it's almost impossible for [rubi em] to hide [rubi eir] impressive package.  Rubi's cock pokes straight upwards, creating a bulge from [rubi eir] crotch up towards [rubi eir] belly button.");
			else if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("  However, with an outfit this tight you can see a slight outline where [rubi eir] small package is tucked away.  You wonder how many other patrons have noticed the same thing.");
		}
		else if(select == 7) {
			//(Long Dress (Bimbo))
			...
			if(flags[FlagEnum.RUBI_BREAST_SIZE] <= 1) outputText("  The dress must have come with a padded bra, because you're surprised to see Rubi with  breasts.  Though small, they help to accentuate [rubi eir] feminine figure.");
		}
		else if(select == 8) {
			...
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) outputText("  With [rubi eir] large rack, [rubi ey] looks much like a busty piratess.  From the way [rubi eir] breasts strain at the top, you're certain that one wrong breath could send a button flying across the room.");
			...
			//Cock variants:
			//(9" or higher)
			if(flags[FlagEnum.RUBI_COCK_SIZE] >= 9) outputText("  [rubi Eir] generous cock is on display, much to [rubi eir] chagrin, as a bulge running down the length of one pant leg.");
			...
			//(D breasts or higher)
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) outputText("  Rubi's large breasts strain the fabric, drawing the dress further up [rubi eir] body and showing off more of [rubi eir] legs and ass.");
		}
		//(Slutty Swimwear (Normal))
		else if(select == 10) {
			//(Slutty Swimwear)
			...
			//(D breasts or higher)
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) outputText("  Rubi's tremendous cleavage threatens to spill out of the skimpy bikini top with every movement, and from the red faces around the bakery, you wonder if that's already happened.");
			else if(flags[FlagEnum.RUBI_BREAST_SIZE] <= 1) outputText("  The top only emphasizes Rubi's relatively flat chest, though with the rest of [rubi eir] figure, [rubi ey] still looks extremely feminine.");
		}
		//(Minidress Outfit (Bimbo))
			...
			if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("  [rubi Eir] skirt rides so high you catch glimpses of [rubi eir] unsurprisingly unpantied crotch, and the enticing package held within.  You wonder how many people have gotten a surprise with their drinks today!");
		}
		//(Bondage Straps)
		else if(select == 12) {
		...
		//(Risque Waitress Uniform)
			...
			if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText(rubiCock(33) + ", clad in a cute little frilly pink cocksock");
			if(flags[FlagEnum.RUBI_NO_CUNT] == 0 && flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText(" and ");
			if(flags[FlagEnum.RUBI_NO_CUNT] == 0) {
				outputText("feminine mound trapped beneath a pair of pink panties");
				//(if Bimbo Brain)
				if(rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) outputText(" trimmed feminine mons, free and open to the air");
			}
			outputText(".");
			//(D breasts or higher)
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) outputText("  Rubi's breasts, large as they are, show off a decent amount of sideboob and threaten to spill out at any moment.  Judging by the flushed faces of some customers, this may have already happened.");
		...
		if(rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) outputText("Like, h");
		else outputText("H");
		...
		if(select == 4) {
			...
			if(flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) outputText("  In an odd sort of way, [rubi ey] looks like an adventurer who met the wrong end of a witch's curse.");
		}
		...
		if(select == 1) {
			...
			if(rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) {
				...
				if(flags[FlagEnum.RUBI_NO_CUNT] == 0 && flags[FlagEnum.RUBI_COCK_SIZE] == 0) outputText(" and wet cunt");
				else if(flags[FlagEnum.RUBI_NO_CUNT] > 0 && flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText(" and limp cock");
		...
		//(Long Dress)
			...
			if(rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) outputText("And then [rubi ey] begins to shake [rubi eir] ass, making spanking motions and effectively ruining the air of sophistication.");
		}
		//(Nurse Uniform)
		else if(select == 9) {
			outputText("\n\nAs [rubi ey] bends over, you get a good look up [rubi eir] skirt, ");
...
public function fuckRubi():void {
	...
	if(size == 0) {
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] <= 5) outputText("I guess we're in the same boat.</i>\"");
		else if(rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) outputText("That's too bad, I was soooo looking forward to a real fucking.</i>\"");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(cockDescript(x));
	else outputText("[clit]");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("swollen member");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("your cockhead");
	else outputText("the tip of your clit");
	outputText(", sending shivers of pleasure up your spine.");
	//(If Cock)
	if(player.lowerBody.cockSpot.hasCock()) outputText("  Amazingly, with the expert ministrations of your lover's mouth, you can't hold yourself in much longer, and begin to tense up, but at the last minute you pull yourself away from Rubi's grip and lips, halting yourself before you can cum.");
	...
	if(size < 2) {
		...
		if(flags[FlagEnum.RUBI_NO_CUNT] > 0) outputText("tailhole");
	...
	//(These two paragraphs for cocks over 24 area)
		...
		if(flags[FlagEnum.RUBI_NO_CUNT] > 0) outputText("balls");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("hard cock");
		else outputText("stiff clit");
		outputText(" pressing against the top of ");
		if(player.lowerBody.cockSpot.hasCock()) outputText("your dick");
	...
	if(flags[FlagEnum.RUBI_ORGASM_DENIAL] > 0 && !rubiBimbo()) outputText("  You give Rubi's ass a hard smack and remind [rubi em] that no matter what happens, [rubi ey] still can't cum yet.  [rubi Eir] eyes flutter open and [rubi ey] actually whimpers a little, but can't form the words to object.");
	//(If Orgasm Denial is NOT activated; male/herm Rubi)
	else if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("  Rubi’s body shivers and trembles, and almost collapses.  [rubi Ey] grasps at the blankets with wild abandon as ropes of pearly white cum erupt from [rubi eir] cock, splattering against [rubi eir] chest and the blankets.");
	//(Female Rubi)
	...
	if(player.lowerBody.cockSpot.hasCock() && player.cockArea(x) <= rubiCapacity()) {
		...
		if(player.cumQ() < 250) outputText("Cum dribbles out from [rubi eir] hole as you give a couple extra thrusts.");
			...
			if(flags[FlagEnum.RUBI_NO_CUNT] > 0) outputText("ass");
			else outputText("cunt");
	...
	if(rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) outputText("like, totally ");
...
public function dildoFuckRubi():void {
	...
	rubiSprite();
	//(If PC has no cock)
	if(!player.lowerBody.cockSpot.hasCock()) outputText("You don't have the equipment to give Rubi a good fucking, but there is one thing you do have.");
	//(If PC has cock)
	...
	//(If PC has a cunt)
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("You press the tip into your cunt quickly, giving it a taste of what is to come.  The brief contact is enough to make you shudder slightly with pleasure.  ");
	//(If PC has fuckable nipples)
	if(player.upperBody.chest.hasFuckableNipples()) outputText("You let the dildo meander up your body until it reaches your breasts, and with a bit of a grin you insert the tip right into one nipple, enjoying the feeling of your breasts swell up with arousal.  You do the same with the other nipple, and take a minute going back and forth.  ");
	...
	if(flags[FlagEnum.RUBI_NO_CUNT] == 1) outputText("against [rubi eir] tight ass");
	...
	//(If player has pussy)
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your " + clitDescript() + " stiffens, aching for attention.  It peeks from your folds like a shy little lewd faerie.");
	//(If player has a cock)
	if(player.lowerBody.cockSpot.hasCock()) outputText("  Meanwhile, [eachCock] stiffens, throbbing as the drug overtakes it.");
	...
	//(PC has cock?)
	if(player.lowerBody.cockSpot.hasCock()) outputText("[eachCock], stroking it fervently, in desperate need of release.");
	//(PC doesn't have cock, but has pussy?)
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your " + clitDescript() + ", rubbing it frantically, in desperate need of release.");
	...
	if(player.upperBody.chest.hasFuckableNipples()) outputText("  Your fingers easily slip inside each nipple, pumping in and out like little cocks.");
	//(No cock, no pussy, no breasts?)
	if(player.gender == 0 && player.upperBody.chest.BreastRatingLargest[0].breastRating == 0) outputText("  Your hands roam all along your body, from your bare crotch to your nipples, even through your hair, all in search of something to give you the release you so desperately crave.");
	...
	//(PC has pussy?)
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your " + vaginaDescript(0) + " clenches down on the enormous invader, which at this point feels like an enormous steel rod lodged firmly inside you, and a spray of fem-juices erupts from your nethers, coating the already slick dildo and Rubi.");
	//(PC lactating?)
	...
	outputText("\n\nRubi rubs [rubi eir] ");
	if(flags[FlagEnum.RUBI_NO_CUNT] == 0) outputText("pussy");
...
public function teaseRubi():void {
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) {
		...
		if(flags[FlagEnum.RUBI_SKIN] == 0) outputText("pale");
		else if(flags[FlagEnum.RUBI_SKIN] == 2) outputText("fur-covered");
		else outputText("red");
	...
	//Girl Rubi!
		...
		if(flags[FlagEnum.RUBI_SKIN] == 0) outputText("pale");
		else if(flags[FlagEnum.RUBI_SKIN] == 1) outputText("crimson");
		else if(flags[FlagEnum.RUBI_SKIN] == 2) outputText("furred");
		...
		if(rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) outputText("like, ");
		outputText("cum!</i>\"");
...
public function releaseRubi():void {
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("[rubi eir] [rubi cock] straining at the seams, trying to break free.");
	...
	if(flags[FlagEnum.RUBI_BLU_BALLS] <= 3) {
		...
		if(flags[FlagEnum.RUBI_BALLS_TYPE] == 1) outputText(", trapped in their little sissy sack");
		outputText(", which you think have swelled somewhat, although you haven't been teasing [rubi em] long.");
	}
	//(if Blueballs 4-9)
	else if(flags[FlagEnum.RUBI_BLU_BALLS] <= 9) outputText("  Not to mention [rubi eir] sizeable balls, which seem to have swelled up to twice their normal size, thanks to your relentless teasing.  They even have a slight bluish tinge to them, although it's not immediately visible.");
	...
	if(flags[FlagEnum.RUBI_BLU_BALLS] <= 3) outputText("swollen");
	else if(flags[FlagEnum.RUBI_BLU_BALLS] <= 9) outputText("slightly blue");
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("P-spot");
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("rectum");
	...
	//(Blueballs 1-3)
	if(flags[FlagEnum.RUBI_BLU_BALLS] <= 3) outputText("  It's quickly followed by another, less forceful jet of pearly white cum, which lands on [rubi eir] face, and several even smaller jets which splatter the area around [rubi em].  [rubi Eir] cock twitches and spasms, then slowly softens.");
	//(Blueballs 4-5)
	...
	//(Blueballs 1-5)
	if(flags[FlagEnum.RUBI_BLU_BALLS] <= 5) outputText("You clean yourself up, leave Rubi a little \"<i>See you next time</i>\" note, and head out the door, back to camp.");
	//(Blueballs 6-9)
	else if(flags[FlagEnum.RUBI_BLU_BALLS] <= 9) outputText("You head into the bathroom and wash the cum off you, taking a short shower.  Afterwards, you dry up, leave Rubi a little \"<i>See you next time</i>\" note, and head out the door, back to camp.");
	//(Blueballs 10+)
...
private function waitAndGetNTRedLikeTheBoyBitchYouAre():void {
	...
	if(!player.isTaur()) {
		...
		else if(player.lowerBody.cockSpot.hasCock()) outputText("[eachCock]");
		outputText(".  ");
	}
	else outputText("\n\n");
...
private function chocoRubiMassage():void {
	...
	if(player.stats.cor >= 66) outputText("start sensually undressing, turning the simple act into a seductive show that causes Rubi's petite boner to jerk unpredictably beneath [rubi eir] clothes.  A small, damp stain appears at the tip by the time you finish your salacious tease.");
	else if(player.stats.cor >= 33) outputText("get undressed without much pause or fanfare.  Rubi makes a show of turning [rubi eir] head, but you can see [rubi em] peeping glimpses out of the corners of [rubi eir] eyes, and [rubi eir] boner seems to be bigger and harder than it was moments before.");
	else {
		...
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 40 || player.upperBody.chest.BreastRatingLargest[0].breastRating >= 8) outputText(", not that it does much good");
		outputText(".");
	}
	...
	if(flags[FlagEnum.RUBI_BLU_BALLS] < 3) outputText("small");
	else if(flags[FlagEnum.RUBI_BLU_BALLS] < 8) outputText("big");
...
private function goInRubisClosetSoThatYouCanComeOutOfTheCloset():void {
	...
	if(rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] == 0) outputText("Oh my god!  ");
...
private function rubiAppearance():void {
	...
	if(flags[FlagEnum.RUBI_SKIN] == 0) outputText("pale, almost like porcelain");
	else if(flags[FlagEnum.RUBI_SKIN] == 1) outputText("an exotic cherry red");
	else if(flags[FlagEnum.RUBI_SKIN] == 2) outputText("covered with a fine layer of silky-smooth pink fur");
	...
	if(flags[FlagEnum.RUBI_SKIN] != 2) outputText(" and completely hairless, except for [rubi eir]");
	else outputText(", and [rubi ey] sports");

	if(flags[FlagEnum.RUBI_HAIR.LENGTH] == 0) outputText(" short");
	else if(flags[FlagEnum.RUBI_HAIR.LENGTH] == 1) outputText(" shoulder-length");
	else outputText(" long");
	if(flags[FlagEnum.RUBI_HAIR] == 0) outputText(" black");
	else outputText(" blonde");
	outputText(" hair, which [rubi ey] keeps cut in a cute, feminine style.  Rubi's face is soft and rounded");
	if(flags[FlagEnum.RUBI_WHISKERS] > 0) outputText(", with a cute set of whiskers sprouting from [rubi eir] cheeks");
	outputText(", [rubi eir] lips painted a ");
	//(normal/furry skin)
	if(flags[FlagEnum.RUBI_SKIN] == 0 || flags[FlagEnum.RUBI_SKIN] == 2) outputText("crimson red");
	//(red/siren skin)
	else outputText("dusky black");
	outputText(", [rubi eir] ");
	//if(flags[FlagEnum.RUBI_EYE_TYPE] == 1) outputText("slitted ");
	if(flags[FlagEnum.RUBI_EYE_TYPE] == 0 || rubiBimbo()) outputText("green");
	...
	if(flags[FlagEnum.RUBI_HORNTYPE] > 0) outputText("Two tiny horns extend upwards from [rubi eir] hairline, solid black in color.  ");
	outputText("A pair of ");
	if(flags[FlagEnum.RUBI_EAR_TYPE] == 0) outputText("cute, normal ears protrude from the sides of [rubi eir] head, each one pierced with a small dangling earring");
	else if(flags[FlagEnum.RUBI_EAR_TYPE] == 1) outputText("cute, cat-like ears pop up from the top of [rubi eir] head, each one seemingly able to move independently");
	else if(flags[FlagEnum.RUBI_EAR_TYPE] == 2) outputText("feathered ears protrude from the sides of [rubi eir] head, each one fluttering cutely from time to time, and each pierced with a small dangling earring");
	outputText(".");

	outputText("\n\n[rubi Ey] has a ");
	if(flags[FlagEnum.RUBI_BREAST_SIZE] < 1) outputText("flat-chest ");
	else outputText("[rubi breasts]");
	outputText(" supporting two perky ");
	if(flags[FlagEnum.RUBI_NIPPLE_TYPE] == 1) outputText("black ");
	outputText("nipples.");

	outputText("\n\nBetween [rubi eir] legs is a ");
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) {
		outputText(rubiCock() + ", which starts to perk up under your scrutiny. From experience, you know it can reach a impressive " + flags[FlagEnum.RUBI_COCK_SIZE] + " inches when erect.  ");
		outputText("Under [rubi eir] cock swings a set of ");
		if(flags[FlagEnum.RUBI_BLU_BALLS] >= 4 && flags[FlagEnum.RUBI_BLU_BALLS] < 9) outputText("cum-swollen ");
		else if(flags[FlagEnum.RUBI_BLU_BALLS] >= 9) outputText("engorged, blue-tinged ");
		outputText("testicles");
		if(flags[FlagEnum.RUBI_BALLS_TYPE] == 1) outputText(", which cling tight to [rubi eir] body in a cute spherical pouch");
	}
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0 && flags[FlagEnum.RUBI_NO_CUNT] == 0) outputText(" and a ");
	if(flags[FlagEnum.RUBI_NO_CUNT] == 0) {
		outputText("cute ");
		if(flags[FlagEnum.RUBI_CUNTTYPE] > 0) outputText("black ");
	...
	if(flags[FlagEnum.RUBI_FEET] == 1) outputText("Finally, Rubi stands on a long black spike that makes [rubi em] walk as though [rubi ey] were wearing high heels.");
...
private function putRubiInAWaitressUniform():void {
	...
	if(flags[FlagEnum.RUBI_BREAST_SIZE] < 1) outputText(", though it's obvious [rubi ey] doesn't need it with [rubi eir] flat-chest");
	...
	if(flags[FlagEnum.RUBI_SKIN] != 2) outputText("hairless");
	...
	if(flags[FlagEnum.RUBI_BREAST_SIZE] < 1) outputText(", showing off some of [rubi eir] flat chest");
	...
	if(flags[FlagEnum.RUBI_FEET] == 1) outputText("what appears to be flats, with a hole in the heel for [rubi eir] demonic heels");
	...
	if(!rubiBimbo() || flags[FlagEnum.RUBI_DEBIMBO] > 0) outputText("girly panties");
...
private function putOnADashingOutfitYouWhore():void {
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("tucking [rubi eir] cock firmly into the triangular strip of cloth covering [rubi eir] crotch, while ");
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText(", not to mention the sizeable package in [rubi eir] crotch");
	outputText(".");
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("  Combined with the sizeable bulge in the crotch, Rubi looks like quite the dashing, if slutty, piratess.");
...
private function goOnAnAnalAdventureRubiNotReallyJustAnAdventurersOutfit():void {
	...
	if(flags[FlagEnum.RUBI_EAR_TYPE] == 1) outputText(", covering [rubi eir] feline ears");
	outputText(".  The boots that go with the outfit are mid-calf length and made of supple, fine leather");
	if(flags[FlagEnum.RUBI_FEET] == 1) outputText(" with a hole cut out in the heel, apparently for Rubi's demonic heels to fit through");
...
private function giveRubiALongDressToHideHerHideousBody():void {
	...
	if(flags[FlagEnum.RUBI_BREAST_SIZE] < 1) outputText(", despite the fact that [rubi ey] clearly doesn't need one");
	...
	if(flags[FlagEnum.RUBI_SKIN] == 1 || flags[FlagEnum.RUBI_SKIN] == 3) outputText(" (or darkens at least)");
	...
	if(flags[FlagEnum.RUBI_FEET] == 1) outputText(" with the heels cut off and a hole for Rubi's demonic heel to fit through");
...
private function rubiPutsOnNursesClothesSoSheCanCheckYourTemperature():void {
	...
	if(flags[FlagEnum.RUBI_SKIN] == 2) outputText("furred ");
	...
	if(flags[FlagEnum.RUBI_BREAST_SIZE] < 1) outputText("heart");
	...
	if(flags[FlagEnum.RUBI_FEET] > 0) outputText(" with the heels removed and a hole for [rubi eir] demonic heels");
	outputText(".");
...
private function rubiHasARubberFetish():void {
	...
	if(flags[FlagEnum.RUBI_SKIN] != 2) outputText("skin");
	...
	//(If Not Bimbo)
	if(!rubiBimbo() || flags[FlagEnum.RUBI_DEBIMBO] > 0) outputText("[rubi Ey] first takes a pair of transparent rubber panties and slides them up [rubi eir] legs until they lovingly cling around [rubi eir] ass.  ");
	...
	if(flags[FlagEnum.RUBI_FEET] == 1) outputText(", the actual heel cut off to make room for [rubi eir] demonic heel,");
...
private function putOnASuitYouSlut():void {
	...
	if(flags[FlagEnum.RUBI_FEET] == 1) outputText(", with the actual heel cut off to make room for [rubi eir] demonic heel,");
...
private function putOnATubeTopYouWhore():void {
	...
	if(flags[FlagEnum.RUBI_BREAST_SIZE] > 2) outputText(", which strain the flimsy piece of material");
	outputText(".  Completing the look is a simple pair of sandals");
	if(flags[FlagEnum.RUBI_FEET] == 1) outputText(", with a hole cut in the sole to make room for Rubi's demonic heels");
	outputText(".");
	outputText("\n\nNow that Rubi is dressed up, what do you want to do?");
...
private function putOnSluttySwimwearYouSkank():void {
	...
	if(flags[FlagEnum.RUBI_BREAST_SIZE] < 3) outputText(" sit loosely over Rubi's small chest");
	else outputText("strain thanks to Rubi's ample busom");
	outputText(".");

	outputText("\n\nTopping it off, your demonic lover slips on a pair of sandals");
	if(flags[FlagEnum.RUBI_FEET] > 0) outputText(", with a hole in the sole to make room for [rubi eir] demonic heels");
	outputText(".");
...
private function putOnADressYouBimbo():void {
	...
	if(flags[FlagEnum.RUBI_FEET] > 0) outputText(", with the heels removed to make room for [rubi eir] own demonic heels");
	outputText(".");
...
private function putOnMyBodysuitYouWhore():void {
	...
	if(flags[FlagEnum.RUBI_SKIN] != 2) outputText("smooth");
	else outputText("furred");
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("[rubi eir] package is neatly tucked away");
...
private function putOnBondageStrapsYouBondageTrap():void {
	...
	if(flags[FlagEnum.RUBI_SKIN] > 0) outputText(", or so you imagine");
...
private function whyTheFuckIsRubiWearingAnInquisitorsCorset():void {
	...
	if(flags[FlagEnum.RUBI_FEET] == 1) outputText(", with the heels removed to make room for [rubi eir] own demonic heels");
	outputText(".");
	outputText("\n\nNow that Rubi is dressed up, what do you want to do?");
	//[Date (go to bar date)] [Back]
	menu();
...
private function putOnAWaitressUniformYouWhore():void {
	...
	if(!rubiBimbo() || flags[FlagEnum.RUBI_DEBIMBO] > 0) {
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("Oddly enough, these panties only seem to cover Rubi's balls, leaving [rubi eir] cock right out in the open.  ");
	}
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("  To top it all off, Rubi takes a pink lacy cocksock and slips it down [rubi eir] [rubi cock], fastening it into place with a pink ribbon that [rubi ey] ties into a bow around the base of [rubi eir] cock.");
...
private function fancyDate():void {
	...
	if(player.stats.gems >= 10) {
		...
		//(if PC has met Callu)
		if(flags[FlagEnum.MET_OTTERGIRL] == 1) outputText(" though not as good as Callu's, the ottergirl fisherman you met at the lake.");
...
private function exhibitionistDate():void {
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText(rubiCock());
...
private function anallyTrainYourBitchBoySlutHowToBeAnalForDCL():void {
	...
	if(flags[FlagEnum.RUBI_TIMES_ANALLY_TRAINED] == 1) {
		...
		if(rubiBimbo() || flags[FlagEnum.RUBI_DEBIMBO] == 0) outputText("totally ");
		...
		if(flags[FlagEnum.RUBI_SKIN] == 1 || flags[FlagEnum.RUBI_SKIN] == 3) outputText(", or redder at least");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText(", [rubi eir] dick wobbling enticingly");
		outputText(".");
		...
		if(flags[FlagEnum.RUBI_NIPPLE_TYPE] == 1) outputText("ebon ");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("pre-cum");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] == 0) outputText("slick vagina");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] == 0) outputText("cunt");
		else outputText("asshole");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText(rubiCock() + " is standing at full attention, bobbing up and down as Rubi fucks [rubi emself] on the dildo");
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0 && flags[FlagEnum.RUBI_NO_CUNT] == 0) outputText(", and [rubi eir] ");
		if(flags[FlagEnum.RUBI_NO_CUNT] == 0) outputText("cunt lips are glistening red with arousal, just begging for attention");
		...
		if(flags[FlagEnum.RUBI_NO_CUNT] == 1) outputText("backside");
		...
		if(flags[FlagEnum.RUBI_SKIN] != 2) outputText("bare");
		else outputText("furred");
		...
		if(flags[FlagEnum.RUBI_NO_CUNT] == 1) outputText("asshole");
		else outputText("cunt");
		...
		if(flags[FlagEnum.RUBI_NO_CUNT] == 1) outputText("ass");
		else outputText("pussy");
		outputText(".");
		...
		if(flags[FlagEnum.RUBI_EAR_TYPE] == 0) outputText("ear");
		else if(flags[FlagEnum.RUBI_EAR_TYPE] == 1) outputText("feline ear");
	...
	//Rubi's capacity won't go larger than 100.
		...
		if(flags[FlagEnum.RUBI_SKIN] == 0) outputText("go bright red");
		else if(flags[FlagEnum.RUBI_SKIN] == 1 || flags[FlagEnum.RUBI_SKIN] == 3) outputText("go a vivid purple");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("ass");
		else outputText("pussy");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("asshole");
		else outputText("cunt");
		...
		if(flags[FlagEnum.RUBI_NO_CUNT] == 1) outputText("anus");
		else outputText("cunt");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("You can feel your lover's cock straining against your leg, aching for some release.  You idly press your foot against it, feeling a slick layer of precum coating it from top to bottom already.  ");
		...
		if(flags[FlagEnum.RUBI_EYE_TYPE] == 0 || rubiBimbo()) outputText("green");
		else if(flags[FlagEnum.RUBI_EYE_TYPE] == 1) outputText("purple");
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("  Even, no, <i>especially</i> [rubi eir] cock twitches and spasms jerking upwards and splattering globules of sticky white goo against the inside of Rubi's babydoll before dripping down onto the floor between [rubi eir] legs, creating a rapidly growing puddle.");
...
private function getFuckedByRubi():void {
	...
	//(normal/furry skin)
	if(flags[FlagEnum.RUBI_SKIN] == 0 || flags[FlagEnum.RUBI_SKIN] == 2) outputText("red");
	else outputText("black");
	...
	if(flags[FlagEnum.RUBI_EYE_TYPE] > 0) outputText(" slitted");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) outputText("asshole");
	else outputText("cunt");
	outputText(" of yours all ready for me.</i>\"  Rubi's head descends between your legs");
	if(player.lowerBody.cockSpot.hasCock()) outputText(", ignoring your " + multiCockDescriptLight() + " completely,");
	...
	if(this.rubiGetCockType() == CockType.HUMAN) outputText("pink, mushroom-like tip");
	else if(this.rubiGetCockType() == CockType.DEMON) outputText("purple, nodule-laden tip");
	else if(this.rubiGetCockType() == CockType.HORSE) outputText("blunted, musky tip");
	else if(this.rubiGetCockType() == CockType.TENTACLE) outputText("tentacle-ringed crown");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) outputText("prostate");
	else outputText("g-spot");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) outputText("bowels");
...
private function rubiHotdogging():void {
	...
	if(flags[FlagEnum.RUBI_SKIN] == 0) outputText("pale skinned");
	else if(flags[FlagEnum.RUBI_SKIN] == 1 || flags[FlagEnum.RUBI_SKIN] == 3) outputText("red skinned");
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("taking hold of [rubi eir] cock");
	...
	if(flags[FlagEnum.RUBI_SKIN] != 2) outputText("tender skin");
	...
	if(flags[FlagEnum.RUBI_NO_CUNT] == 0) outputText("cunt moistening");
	if(flags[FlagEnum.RUBI_NO_CUNT] == 0 && flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText(" and ");
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("cock hardening");
	...
	if(flags[FlagEnum.RUBI_SKIN] == 0) outputText("pale skin");
	else if(flags[FlagEnum.RUBI_SKIN] == 1|| flags[FlagEnum.RUBI_SKIN] == 3) outputText("crimson skin");
	...
	if(flags[FlagEnum.RUBI_NO_CUNT] == 0) outputText("clit");
...
private function teaseButtjobs():void {
	...
	if(flags[FlagEnum.RUBI_NO_CUNT] == 1) outputText("prostate");
	else outputText("g-spot");
...
private function popButtjobs():void {
	...
	outputText("\n\nRubi's ");
	if(flags[FlagEnum.RUBI_NO_CUNT] == 0) outputText("cunt clamps down on your thumb and gushes, coating your hand in [rubi eir] slick juices");
	...
	//(If Rubi has Penis)
	if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) outputText("  Beneath [rubi em], [rubi eir] cock jerks and twitches, loosing a spray of pearly white cum which splatters against the couch below");
	if(flags[FlagEnum.RUBI_BLU_BALLS] < 4) {}
	//((blueballs 4-5)
	else if(flags[FlagEnum.RUBI_BLU_BALLS] <= 5) outputText(" which is quickly followed by another, more powerful stream of cum, which splashes against [rubi eir] stomach, coating [rubi eir] [rubi breasts] in semen");
	else if(flags[FlagEnum.RUBI_BLU_BALLS] <= 7) outputText(" which is quickly followed by another, more powerful stream of cum, and a third, and a fourth.  Before you know it, Rubi's entire torso, including [rubi eir] [rubi breasts] are practically glazed with cum");
	else if(flags[FlagEnum.RUBI_BLU_BALLS] <= 9) outputText(" which is followed up by several more, each seemingly more poweful than the one previous.  Rubi's body twitches and shudders with every jet, until [rubi eir] entire torso, and the couch, is covered in the musky jizz");
	...
	if(player.lowerBody.balls > 1) outputText("churn");
...
private function giveRubiATFItem(itype:ItemType):void {
	...
	if(itype == consumables.SUCMILK || itype == consumables.P_S_MLK) {
		...
		if((flags[FlagEnum.RUBI_COCK_SIZE] > 4) && (!flags[FlagEnum.HYPER_HAPPY])) {
			...
			outputText(Math.round(flags[FlagEnum.RUBI_COCK_SIZE]*10)/10 + "-inch penis.");
		}
		//(If Penis is removed)
		...
		if(rand(3) == 0 && flags[FlagEnum.RUBI_SKIN] != 1 && itype == consumables.SUCMILK) {
			...
			if(flags[FlagEnum.RUBI_SKIN] == 2) outputText("Meanwhile the silky soft fur that covered [rubi eir] body seems to be falling out as this red menace takes over.  ");
		...
		//(If Rubi has no penis; instead)
			...
			if(flags[FlagEnum.RUBI_SKIN] == 1 || flags[FlagEnum.RUBI_SKIN] == 3) outputText("red");
	...
	//Bimbo Liqueur (Modified for variant genders)
		...
		if(flags[FlagEnum.RUBI_COCK_SIZE] > 0) {
			...
			if(flags[FlagEnum.RUBI_NO_CUNT] == 0) outputText("[rubi eir] vagina");
		...
		if(flags[FlagEnum.RUBI_BREAST_SIZE] < 7) {
			...
			//(if flat-chested)
			if(flags[FlagEnum.RUBI_BREAST_SIZE] == 0) outputText("  Within seconds, Rubi has the chest of a pubescent girl, within a minute they're at least a B-cup.");
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("yours");
		else outputText("you");
	...
	//Grows cock. (As Incubus Draft above)
		...
		if(this.rubiGetCockType() != CockType.HORSE) {
			...
			if(flags[FlagEnum.RUBI_BLU_BALLS] >= 4 && flags[FlagEnum.RUBI_BLU_BALLS] < 9) outputText("swollen ");
			else if(flags[FlagEnum.RUBI_BLU_BALLS] >= 9) outputText("engorged ");
			if(flags[FlagEnum.RUBI_BALLS_TYPE] == 1) outputText("tight, trappy ");
	...
	//Trap Oil
		...
		if(flags[FlagEnum.RUBI_BREAST_SIZE] > 0) outputText(", while [rubi eir] [rubi breasts] gently rock up and down with the motions of [rubi eir] breathing");
		outputText(".");
	...
	if(itype == consumables.PURPEAC) {
		...
		if(flags[FlagEnum.RUBI_SKIN] == 0) outputText("skin");
		else if(flags[FlagEnum.RUBI_SKIN] == 1) outputText("red skin");
		else if(flags[FlagEnum.RUBI_SKIN] == 2) outputText("furred skin");
		...
		if (flags[FlagEnum.RUBI_EAR_TYPE] != 2 || flags[FlagEnum.RUBI_SKIN] != 3 || (flags[FlagEnum.RUBI_COCK_SIZE] > 0 && this.rubiGetCockType() != CockType.TENTACLE))
		{
			...
			if(flags[FlagEnum.RUBI_EAR_TYPE] != 2) {
				...
				if(flags[FlagEnum.RUBI_EAR_TYPE] == 0) outputText("normal");
			...
			//(Shark Skin)
				...
				if(flags[FlagEnum.RUBI_SKIN] == 0) outputText("its normal paleness replaced with a vibrant cherry red");
				else if(flags[FlagEnum.RUBI_SKIN] ==1 ) outputText("which is barely noticeable on [rubi eir] already cherry red flesh");
				else if(flags[FlagEnum.RUBI_SKIN] == 2) outputText("the fur rapidly retreating and replaced with bright red flesh underneath");
			...
			//(Anemone Penis, must have a penis)
				...
				if(this.rubiGetCockType() != CockType.HUMAN) outputText(", first shifting towards a more human-like appearance");
...
private function rubiPenisGroPlus():void {
	...
	if(flags[FlagEnum.RUBI_COCK_SIZE] < 12 || (this.rubiGetCockType() == CockType.HORSE && flags[FlagEnum.RUBI_COCK_SIZE] < 20) || (flags[FlagEnum.RUBI_COCK_SIZE] < 30 && flags[FlagEnum.HYPER_HAPPY])) {
		...
		if(this.rubiGetCockType() == CockType.HORSE) outputText("sheath-stuffing horse-dick");
...
private function pickAnItemToFeedRubi():void {
	...
	if (closet.length > 0) {
		...
		for (let x: number = 0; x < closet.length; x++) outputText("\n" + closet[x]);
		outputText("\n\n");
	}
...
public function hypnoBimboficationForRubiSloots():void
{
	...
	if(flags[FlagEnum.RUBI_BREAST_SIZE] < 1) outputText("femmy boy");
	else if(flags[FlagEnum.RUBI_NO_CUNT] > 0) outputText("dick-girl");
	...
	if(flags[FlagEnum.RUBI_NO_CUNT] > 0) outputText("femboy");
	...
	outputText("\n\nBeyond words, the ");
	if(flags[FlagEnum.RUBI_NO_CUNT] == 0) outputText("dick-");
	outputText("girl");
	if(flags[FlagEnum.RUBI_NO_CUNT] > 0) outputText("y-boy");
	...
	outputText("\n\n<b>Rubi is now a ");
	if(flags[FlagEnum.RUBI_NO_CUNT] > 0) outputText("shemale ");
