...
*/
		...
		private function tightTopDescption():void {
			switch (breasts.cupSize) {
				...
				case CoC.BREAST_CUP.B: outputText("tight enough to showcase even her small breasts"); break
				case CoC.BREAST_CUP.C:
				case CoC.BREAST_CUP.D:
				case CoC.BREAST_CUP.DD: outputText("just able to hold her bountiful breasts in check"); break;
				default:
			}
...
public function visitAtHome():void { //You go to Kath's place alone - she may or may not be home
	...
	if (isAt(KLOC_KATHS_APT)) {
		if (flags[FlagEnum.KATHERINE_URTA_AFFECTION] == 31) { //Special event that happens only once
			...
			outputText("Once you’re seated Urta laughs and says, “<i>so your big plan worked.  You got us fucking, " + (flags[FlagEnum.KATHERINE_URTA_DATE] == KDATE_LOTS ? "hell you encouraged us to fuck every chance we got!  So" : "so") + " it should be no surprise that we’ve become close.</i>”\n\n");
	...
	if (isAt(KLOC_URTAS_APT)) { //This option happens if you and Urta have no children
		...
		switch (rand(4)) {
			...
			default:
				...
				if (kids == 1)
					outputText("your child.  " + (flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1 ? "He" : "She") + "'s sitting in Katherine's lap, head mashed against her breasts like " + (flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1 ? "he" : "she") + " hasn't got a care in the world.\n\n");
				...
				outputText("When Kath spots you she gives you a big smile and you hear “<i>" + (player.lowerBody.vaginaSpot.hasVagina() ? "Mommy" : "Daddy") + "!</i>” from your offspring, who " + (kids > 1 ? "run over and hug" : "runs over and hugs") + " your legs.\n\n");
				outputText("Kath comes over and hugs you too, whispering, “<i>");
				if (kids == 1) 
					outputText(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1 ? "He's" : "She's");
				else outputText("They're");
				outputText(" so cute,</i>” in your ear.\n\n");
				outputText("Urta walks down from upstairs and tells ");
				if (kids == 1) 
					outputText(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1 ? "him" : "her");
				...
				if (kids == 1) 
					outputText(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1 ? "he races off to his room" : "she races off to her room");
...
private function urtaPlusKathCuddle():void {
	if (player.stats.spe > 75) {
		...
		if (getGame().urtaPregs.urtaKids() == 1) 
			outputText(flags[FlagEnum.URTA_FIRSTBORN_GENDER] == 1 ? "your son" : "your daughter");
...
public function barKathUrtaLoveAnnounce():void {
	...
	outputText("Urta offers you the seat next to her and Kath shifts over so she's on your other side.  “<i>So your big plan worked,</i>” says Urta with a grin that runs from ear to ear.  “<i>You got us fucking, " + (flags[FlagEnum.KATHERINE_URTA_DATE] == KDATE_LOTS ? "hell you encouraged us to fuck every chance we got!  So" : "so") + " it should be no surprise that we’ve become close.</i>”\n\n");
	outputText("“<i>Really close,</i>” says Katherine, grinding her hip up against yours.\n\n");
	outputText("“<i>So anyway,</i>” Urta continues, “<i>we both still love you, especially since we only met each other because of you.  But now we love each other too.  I figure you must have expected this, so don’t pretend to be surprised.</i>”\n\n");
	outputText("“<i>How do you want to celebrate?</i>” asks Kath.  You can see the matching pair of " + (hasCock() ? "bulges" : "smiles") + ", one to each side.  They both put their arms around you, pressing their breasts against you deliberately.  But hey, who are you to argue?  A threesome seems like an awfully good idea.");
	flags[FlagEnum.KATHERINE_URTA_AFFECTION] = 32;
...
private function talkClothes():void {
	...
	if (optionNurseOutfit == null)
		outputText((flags[FlagEnum.GIVEN_AMILY_NURSE_OUTFIT] > 0 ? "You remember what happened when you gave your mouse-girl one of those sexy nurse outfits" : "Out there in the wilds you’ve seen those sexy nurse outfits here and there") + ". You shiver at the thought of combining that obscene clothing with your " + catGirl("cat girl", "feline") + " lover and her sex drive.\n\n");
...
private function talkClothesNurseOutfitCompliment():void {
	...
	if (player.lowerBody.vaginaSpot.hasVagina()) outputText("  “<i>I can think of at least one other person I’d love to see in a nurse’s outfit</i>” she replies, tracing a finger down your back.");
	katSexMenu();
}
...
private function talkLoversVala():void {
	...
	if (flags[FlagEnum.KATHERINE_VALA_AFFECTION] == 0) {
		...
		if (flags[FlagEnum.TIMES_FUCKED_VALA_IN_DUNGEON] > 0) outputText("You decide it would be best not to mention " + (flags[FlagEnum.TIMES_FUCKED_VALA_IN_DUNGEON] == 1 ? "that time" : "those times") + " you took advantage of Vala, back when she was mindfucked and tied up.\n\n");
		if (isAt(KLOC_KATHS_APT))
...
private function useReductoOnKat():void {
	...
	if (cockLength <= dickMin && ballSize <= 1 && knotSize <= 2 && breasts.cupSize == BREAST_CUP.A) {
		...
		if (player.stats.lib > 50) outputText(" with a salacious grin");
...
private function useRedoctoOnKatsKnot():void {
	...
	if (cockNumber > 1) {
		...
		if (player.stats.lib > 50) outputText("joyously attempt to catch in your mouth like rain");
...
private function useReductoOnKatsKock():void {
	...
	if (player.stats.lib > 50) outputText("allow her to continue until " + (ballSize > 0 ? "her scrotum tightens up and " : "") + "it looks like she'll blow her load with one more touch, and then ");
	...
	if (player.stats.lib > 50) outputText(", then quivers again as the movement brings her a hair closer to her climax");
	outputText(".\n\n");
	...
	if (player.stats.lib > 50) outputText("staring at Katherine with a lewd smirk as she trembles and tries to maintain control, ");
	...
	if(player.stats.lib > 50) outputText("still pushing out the aftershocks of her climax onto her belly, ");
	...
	if (player.stats.lib > 50) outputText(", if fatigued,");
...
private function giveKatPureSuccubusMilk():void {
	...
	if (breasts.cupSize < CoC.BREAST_CUP.DD_BIG) { //She’s willing to go up to big DD-Cup normally
		...
		outputText("She swallows and asks, “<i>What have I done?</i>” before her " + (breasts.cupSize == CoC.BREAST_CUP.DD_BIG ? "tits" : "boobs") + " begin to swell beneath your fingers.  In just a few moments they " + (breasts.cupSize == CoC.BREAST_CUP.DD ? "are well and truly boobs" : "have expanded to epic proportions") + ".  “<i>Oh Marae!  I’m so top heavy.</i>”  Kath leans back and adjusts her posture to support her expanded rack.\n\n");
...
private function giveKatRefuzzFormulaChange():void {
	...
	if (player.lowerBody.cockSpot.hasCock())
		outputText("cock" + (player.lowerBody.cockSpot.count() > 1 ? "s" : ""));
	else outputText("crotch");
	outputText((player.lowerBody.vaginaSpot.hasVagina() ? " and moistening your cunt" : "") + ".\n\n");
...
private function giveKatClothesTubeTop():void {
	...
	else outputText((flags[FlagEnum.KATHERINE_CLOTHES_WORN] == KBIT_CLOTHES_C_CLOTH ? "blouse" : "clothes, quickly slips on her knee length skirt") + " and then struggles with the tube top.  When it’s finally on right she starts posing in the mirror, her " + breasts.adj() + " breasts on display in the stretchy, clingy fabric.");
...
private function katPenetrate():void {
	...
	if (isAt(KLOC_BAR) || isAt(KLOC_BAR_DRUNK) || isAt(KLOC_BAR_URTA_REFUSED)) { //At the bar
		...
		outputText("You take her hand and lead her outside, to " + (flags[FlagEnum.TIMES_FUCKED_URTA] > 0 ? "that familiar" : "the") + " alleyway behind the Wet Bitch.\n\n");
...
public function penetrateKatsVag():void {
	...
	if (player.lowerBody.vaginaSpot.hasVagina()) outputText("Your " + vaginaDescript() + " releases its own girl-cum in sympathy, even as y");
	else outputText("Y");
...
public function pcPenetratesKatDoubly():void {
	...
	clearOutput();
	let x:number = player.cockThatFits(70);
	outputText("You unfasten your " + player.armorName + ", exposing " + (player.lowerBody.cockSpot.count() == 2 ? "both your hardening cocks" : "your collection of hardening cocks"));
	if (isAt(KLOC_KATHS_APT))
	...
	outputText("he lets out a low, feral meow.  Taking that as your cue you raise your hips slightly and press your slick cockhead against her pucker. " + (player.lowerBody.cockSpot.count() == 2 ? "Your lower cock" : "One of your other cocks") + " nestles between the pink folds of her pussy.\n\n");
	...
	outputText("Your " + (player.lowerBody.balls > 0 ? "balls twitch" : "prostate twitches") + ", letting you know there’s plenty of cream for this sex kitten.\n\n");
	...
	if (hasCock()) {
		...
		outputText(" so there’s no sense in stopping now.  In fact you hope you can " + (player.stats.cor < 50 ? "encourage" : "force") + " your " + catGirl("pet cat girl", "feline friend") + " to add another helping to the mess.\n\n");
	}
	...
	outputText("The sensation overcomes you and your " + (player.lowerBody.balls > 0 ? "balls pull tight against your groin" : "engorged prostate squeezes") + " firing the first of many salvos into Katherine’s limp body.\n\n");

	outputText("You fire your load into both Kath’s holes");
	if (player.lowerBody.cockSpot.count() > 2) outputText(" while at the same time your other cock" + (player.lowerBody.cockSpot.count() > 3 ? "s " : " ") + (hasCock() ? (player.lowerBody.cockSpot.count() > 3 ? "add" : "adds") + " to the" : "create a") + " mess beneath her");
	if (player.cumQ() < 500)
		...
		outputText(".  Again and again your " + (player.lowerBody.balls > 0 ? "balls contract" : "prostate contracts") + ", fighting ever greater resistance as every void inside Kath is packed with your juices.  You no longer have to support Katherine’s hips - her cum stuffed belly rests against the ");
		if (isAt(KLOC_STREETS))
		...
		if (player.cumQ() >= 3000) {
			outputText("Even your iron erections are unable to hold back the flood from your unnaturally bountiful " + (player.lowerBody.balls > 0 ? "balls" : "prostate") + ". Kath’s skin is so taut " + catGirl("you can see stretch marks developing", "that her belly fur begins to lighten.  Looking closely you can see that each hair is being forced to stand on end") + ".\n\n");

			outputText("Then you feel the first trickles running back along your cocks.  Your " + (player.lowerBody.balls > 0 ? "balls pump" : "prostate pumps") + " again and the trickle becomes a torrent.  You now have to hold on to Kath’s hips just so she doesn’t launch off of you.\n\n");
		}
...
public function suckNFuck():void {
	...
	if (!player.perks.has("Flexibility")) outputText("  The sight is enough to make your spine wince in sympathy.");
	...
	if (player.lowerBody.balls > 0) outputText(", your balls,");
...
private function letKatKnotYouCommonDialogue(inAlleyBehindBar:boolean):void {
	...
	if (player.lowerBody.vaginaSpot.hasVagina()) outputText("  “<i>So, which hole do you want me to use?</i>”");
}
...
public function letKatKnotYourCuntPussyFuck():void {
	...
	if (cockNumber > 1) {
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText(multiCockDescriptLight());
		else if(player.lowerBody.balls > 0) outputText(sackDescript());
...
public function getPenetrated():void {
	...
	if (player.lowerBody.vaginaSpot.hasVagina()) outputText(", femcum splashing from your " + vaginaDescript());
	if (player.lowerBody.cockSpot.count() > 0) {
		outputText((player.lowerBody.vaginaSpot.hasVagina() ? " and " : ", ") + sMultiCockDesc() + " spurting ");
		if(player.cumQ() < 25) outputText("drops");
		else if(player.cumQ() < 100) outputText("splashes");
		else if(player.cumQ() < 250) outputText("puddles");
		else outputText("a veritable lake of spunk" + (isAt(KLOC_KATHS_APT) ? "onto the bed" : "into the alleyway"));
...
public function getDoublePennedByKat():void {
	...
	if (player.lowerBody.cockSpot.hasCock()) outputText("  " + SMultiCockDesc() + (player.lowerBody.cockSpot.count() > 1 ? " celebrate with arcs of their" : " celebrates with arcs of its") + " own semen, spraying them in a patter on her legs and the ground in front of you.");
...
public function suckedNFuckedByKat():void {
	...
	if (player.lowerBody.vaginaSpot.hasVagina()) outputText("your " + vaginaDescript());
	...
	if (player.lowerBody.vaginaSpot.hasVagina() && cockNumber > 1) outputText("s");
	...
	if (player.lowerBody.balls > 0) outputText("in your " + ballsDescriptLight());
	...
	if (cockNumber > 1 && player.lowerBody.vaginaSpot.hasVagina()) outputText("s");
...
public function giveKatOralPenisWingWang():void {
	...
	if (cockNumber == 1) {
		...
		if (player.stats.cor >= 70) outputText(" - if only to prove your skill and make the kitten ever more dependent on you for release");
		outputText(".\n\n");
		...
		if (player.tongueType == TONUGE.SNAKE) outputText("molesting her clit with your tongue");
		else if (player.tongueType == TONUGE.DEMONIC) outputText("penetrating her with your tongue as you give her head");
	...
	//Double Cock Scene
		...
		if (player.tongueType == TONUGE.SNAKE || player.tongueType == TONUGE.DEMONIC) outputText("  Said tongue wraps around one erection and then the other, stroking gently with its long, flexible length.");
		outputText("\n\n");
		...
		if (hasBalls()) {
			...
			if (player.tongueType == TONUGE.SNAKE || player.tongueType == TONUGE.DEMONIC) outputText("  Your long, flexible tongue once again slides out as you gently lift her testes up, easily dipping inside her wet pussy and stroking her inner walls as you suck.");
	...
	if (player.tongueType == TONUGE.DEMONIC) outputText("She runs her fingers along your lips.  “<i>I think I might be falling in love with the monster inside that pretty cave.</i>”\n\n");
...
private function katherineGoesDownOnTheGirlsOhYahBabyLesbo():void {
	...
	if (player.lowerBody.cockSpot.count() > 0)
		outputText("At the same time your cock" + (player.lowerBody.cockSpot.count() > 1 ? "s release their" : " releases its") + " load into the air, causing semen to rain down on the pair of you.\n\n");
...
private function katherineLicksAllTheBoyPenises():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.lowerBody.balls > 0) outputText(", past your " + ballsDescriptLight() + ",");
	...
	if(player.lowerBody.balls == 0) outputText("cock tingling");
...
private function katDoubleHelixCraziness():void {
	...
	if (player.lowerBody.balls > 0) outputText(", and even licking your " + ballsDescriptLight() + " clean");
	outputText(".  You moan and shudder and buck your hips at her ministrations; your ");
	if (player.lowerBody.balls > 0) outputText("balls are still depleted");
...
private function suckleFromKath():void {
	...
	if (breasts.lactationLevel == BreastStore.LACTATION_NONE) {
		...
		if (isAt(KLOC_BAR) || isAt(KLOC_BAR_DRUNK) || isAt(KLOC_BAR_URTA_REFUSED)) {
			...
			outputText("  After a little catnap she comes around and takes in the bar.  She puts " + clothesChoice("her bra and blouse back on", "on her bra and wriggles back into the bodysuit", "on her bra and does up the bodice of her dress", "her bra back on and closes up her robe", "her bra and tube top back on", "her bra and nurse's top back on") + (player.stats.cor < 75 ? "" : ", looking a little sheepish") + ".\n\n");
		}
		else if (isAt(KLOC_KATHS_APT)) {
			outputText("\n\n");
		}
	...
	if (isAt(KLOC_BAR) || isAt(KLOC_BAR_DRUNK) || isAt(KLOC_BAR_URTA_REFUSED)) {
		if (model.time.hours >= 8 && model.time.hours <= 16 && flags[FlagEnum.NIAMH_STATUS] == 0)
			outputText("You notice " + (flags[FlagEnum.MET_NIAMH] == 0 ? "the huge breasted cat girl" : "Niamh") + " rubbing her own mammaries enviously.  ");
...
private function suckleTacularKats():void {
	...
	if (isAt(KLOC_BAR) || isAt(KLOC_BAR_DRUNK) || isAt(KLOC_BAR_URTA_REFUSED)) { //At the bar
		...
		if (player.eyeType == EYES.FOUR_SPIDER_EYES) outputText("both sets of ");
	...
	if (!player.perks.has("Feeder") && player.lactationQ() < 1000) outputText("empties");
	else outputText("does her best to empty");
	outputText(" your first breast, and then moves to the next one.  Finally, she's drunk ");
	if (!player.perks.has("Feeder") && player.lactationQ() < 1000) outputText("you dry");
...
private function helpKathAfterInterruptu():void
{
	clearOutput();
	//(corruption <25)
	if(player.stats.cor < 25) outputText("You feel you just have to do something for her.  She needs food and she’s ashamed of her own body.  That’s rare in this land, but it’s not a good way to live your life.");
	else if(player.stats.cor < 75) outputText("Right now she needs food, no question.  If you help her out maybe she’ll be more willing to talk to you in future.  Maybe you can become a friend to her, maybe more.");
...
public function repeatAmbushKatherineRecruitMent():void
{
	...
	if(player.stats.cor < 75) outputText(" hope she’s here so you can get to know her better.");
	else 
	{
		if(player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) outputText("r dick swells and your pussy moistens");
		else if(player.lowerBody.cockSpot.hasCock()) outputText("r dick swells");
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("r pussy moistens");
...
private function donateToCatSlutsYouCatSlutPt2():void
{
	...
	//(corruption <25)
	if(player.stats.cor < 25) outputText("You tell her that of course you want to.  She seems like a nice person, and you want to get to know her better.");
	else if(player.stats.cor < 75) outputText("You say that yes, you would love to take her out on a date.  You don’t say that you’d like even more to press her vulnerable body against the wall and have your way with her.");
...
private function threatenCatSluts():void
{
	...
	//(corruption <25)
	if(player.stats.cor < 25) outputText("The poor girl is desperate for a friend.  You hope you can be a good enough person and help her out.");
	else if(player.stats.cor < 75) outputText("It’s a real shame Katherine is so lonely.  She shouldn’t be willing to trust a virtual stranger like you, but here she is and here you are.");
...
public function handjobbiesFurrDemCatFurries():void
{
	...
	if (player.stats.cor < 25) outputText("slowly caress her tender breasts");
	else if (player.stats.cor < 75) outputText("tease her lovely breasts");
	else outputText("roughly manhandle her knockers");
	if (flags[FlagEnum.KATHERINE_UNLOCKED] > 0 && breasts.milkIsFull()) outputText(".  Your fingers are soon coated in her cream and you offer them to Kath, who eagerly licks them clean");
	outputText(".  Her tail, trapped between your body and hers, flicks back and forth");
	if (player.lowerBody.cockSpot.hasCock()) outputText(", causing your " + player.multiCockDescriptLight() + " to rise to the occasion.");
	else if (player.lowerBody.vaginaSpot.hasVagina()) outputText(", teasing your hot little slit.");
	//(genderless)
	...
	if (!hasCock()) {
		...
		outputText("the tips of her hot shafts.  A few strokes and you’ve spread her copious precum over both of her cocks.  While one hand continues to pleasure her pricks the other goes lower, slipping " + (flags[FlagEnum.KATHERINE_UNLOCKED] < 1 ? "under her belt and into her pants." : "into her moist snatch."));
		...
		outputText("the tip of her hot shaft.  A few strokes and you’ve spread her copious precum over most of her cock.  While one hand pleasures her prick the other goes lower, slipping " + (flags[FlagEnum.KATHERINE_UNLOCKED] < 1 ? "under her belt and into her pants." : "into her moist snatch."));
	...
	if (player.stats.cor < 25) outputText("You hold her in your arms while she recovers.  You really enjoy seeing the happiness and contentment in her face.");
	else if (player.stats.cor < 75) outputText("While you wait for her to recover you pull her shirt up and play with her breasts a bit more.  By the time she can move again her nipples are sticking out like little berries.  She blushes, but clearly enjoys the attention.");
	else {
		outputText("Not one to pass up a helpless young " + (hasCock() ? "herm" : "girl") + " you " + (flags[FlagEnum.KATHERINE_UNLOCKED] < 1 ? "strip all Katherine’s clothes off.  You" : "") + " play with her tail, eventually growing bored and pushing the tip deep inside her pussy.\n\n");
		if (hasCock())
...
public function bathTime():void {
	...
	outputText((player.lowerBody.balls > 0 ? "Your balls try to retract, y" : "Y") + "our nipples harden and you can tell Kath’s are doing the same.");
	if (hasCock()) {
...
private function bathTimeCentaurRide():void {
	...
	outputText("Katherine’s feet begin to stroke your " + (player.lowerBody.cockSpot.count() > 1 ? "cocks" : "manhood") + " rapidly; if it weren’t for the river she would probably be rubbing you raw.  Then you feel her ");
	if (hasCock())
	...
	outputText("She only just succeeds.  As she collapses forward onto your back" + (hasCock() ? ", slapping wetly into her own spunk," : "") + " her feet weakly slide down towards the base of your " + (player.lowerBody.cockSpot.count() > 1 ? "largest " : "") + " cock one last time.  You’re so close that’s all you needed.\n\n");
	...
	if (player.cumQ() <= 500)
		...
		outputText("Your cum launches out of your cock" + (player.lowerBody.cockSpot.count() > 1 ? "s" : "") + " so forcefully that you can feel its warmth licking against the backs of your front knees.  With nothing to contain " + (player.lowerBody.cockSpot.count() > 1 ? "them your cocks bounce back and forth like out of control firehoses" : "it your penis bounces back and forth like an out of control firehose") + ".\n\n");
	else
		outputText("Your overproductive " + (player.lowerBody.balls > 0 ? "testes outdo themselves" : "prostate outdoes itself") + ".  Without an ass, mouth or vagina to contain " + (player.lowerBody.balls > 0 ? "their deluge they just keep" : "its deluge it just keeps") + " firing and contracting.  Your cock" + (player.lowerBody.cockSpot.count() > 1 ? "s jerk and bounce" : " jerks and bounces") + " so violently that your belly will probably sport some bruises after this.  The water grows warm around you and changes color noticeably downstream.  You have to wonder just how much magic is used up each time you cum.\n\n");
...
private function bathTimeCentaurPenetrated():void {
	...
	else outputText(player.lowerBody.vaginaSpot.hasVagina() ? "knucles grinding against the entrance to your womb" : "fingers flexing deep inside your colon");
	...
	if (player.lowerBody.cockSpot.hasCock()) {
		if (player.cumQ() <= 500)
			...
			outputText("  Your cum launches out of your cock" + (player.lowerBody.cockSpot.count() > 1 ? "s" : "") + " so forcefully that you can feel its warmth licking against the backs of your front knees.  With nothing to contain " + (player.lowerBody.cockSpot.count() > 1 ? "them your cocks bounce back and forth like out of control firehoses." : "it your penis bounces back and forth like an out of control firehose."));
		else outputText("  Your overproductive " + (player.lowerBody.balls > 0 ? "testes outdo themselves" : "prostate outdoes itself") + ".  Without an ass, mouth or vagina to contain " + (player.lowerBody.balls > 0 ? "their deluge they just keep" : "its deluge it just keeps") + " firing and contracting.  Your cock" + (player.lowerBody.cockSpot.count() > 1 ? "s jerk and bounce" : " jerks and bounces") + " so violently that your belly will probably sport some bruises after this.  The water grows warm around you and changes color noticeably downstream.  You have to wonder just how much magic is used up each time you cum.");
	}
...
private function bathTimeFuckKath():void {
	...
	if (player.cumQ() <= 500)
		...
		if (player.cumQ() > 3000)
			outputText("Your overproductive " + (player.lowerBody.balls > 0 ? "testes outdo themselves" : "prostate outdoes itself") + ".  ");
...
private function bathTimeFrustrated():void { //You took away her cock. This is your fault
	clearOutput();
	outputText("Kath hugs you tight and you feel her fingers sliding across your " + (player.lowerBody.vaginaSpot.hasVagina() ? "clit" : "nipples") + ".  You return the favor, eager to please your lusty cat " + catGirl("girl", "morph") + ".\n\n");
	outputText("The two of you grind your breasts together and your fingers race to see who will jill the other off first.\n\n");
	outputText("Seconds become minutes and there's no end in sight.  You realize that the glacial stream is hindering your efforts.  You're so cold you can barely feel Kath's body pressing against yours.  Her fingers are still rubbing you" + (player.lowerBody.vaginaSpot.hasVagina() ? "r clit but even that bundle of nerves" : ", but every bit of your skin") + " is growing numb.  Your own hand is starting to cramp up and the only reason you're certain it's still rubbing against her pussy is that her thighs are keeping it in place.\n\n");
...
private function bathTimePenetrated():void {
	if (!player.lowerBody.vaginaSpot.hasVagina()) { //Genderless or Male - get this out of the way since it will be a little different
		...
		if (furry)
			outputText("slick fur rubbing against your " + (player.skinType == SKIN.FUR ? "own" : "skin"));
		else
			outputText("wet skin " + (player.skinType == SKIN.FUR ? "rubbing against your fur" : "sliding over your body"));
		outputText(" overloads your senses.  " + (player.stats.cor < 75 ? "You close your eyes and lose yourself in the sensual feelings, the icy water forgotten." : "Your ability to fight back drains away and you curse your lack of planning - you’re so aroused that you can’t take advantage of Kath.  In fact she’s the one taking the lead!") + "\n\n");
		...
		if (player.lowerBody.cockSpot.hasCock()) {
			...
			if (player.cumQ() <= 500)
				...
				outputText("Your cum launches out of your cock" + (player.lowerBody.cockSpot.count() > 1 ? "s" : "") + " so forcefully that " + (player.lowerBody.cockSpot.count() > 1 ? "your cocks bounce back and forth like out of control firehoses." : "your penis bounces back and forth like an out of control firehose."));
			else
				outputText("Your overproductive " + (player.lowerBody.balls > 0 ? "testes outdo themselves" : "prostate outdoes itself") + ".  Without an ass, mouth or vagina to contain " + (player.lowerBody.balls > 0 ? "their deluge they just keep" : "its deluge it just keeps") + " firing and contracting.  Your cock" + (player.lowerBody.cockSpot.count() > 1 ? "s jerk and bounce" : " jerks and bounces") + " so violently that your belly will probably sport some bruises after this.  The water grows warm around you and changes color noticeably downstream.  You have to wonder just how much magic is used up each time you cum.");
		}
		...
		if (player.lowerBody.cockSpot.hasCock()) {
			outputText("The cold stream is starting to affect even your lust fueled shaft" + (player.lowerBody.cockSpot.count() > 1 ? "s" : "") + ".  It’s almost as if your body understands that right now your pussy is going to be doing all the work.  Your masculine member" + (player.lowerBody.cockSpot.count() > 1 ? "s flag" : " flags") + " as more and more blood is diverted to the muscles of your love tunnel" + (player.isPregnant() ? "" : ", your estrogen soaked ovaries and the walls of your fertile womb") + ".\n\n");
		}
...
private function bathTimePenetratedNormalEnd():void {
	...
	outputText(" carry your spent " + (player.stats.cor < 75 ? "girlfriend" : "fucktoy") + " to the shore and wait for her to recover.\n\n");

	if (player.lowerBody.cockSpot.hasCock())
		outputText("It's only when you flop onto the sandy ground that you realize your cock" + (player.lowerBody.cockSpot.count() > 1 ? "s" : "") + " never fired a shot.  The icy water and lack of stimulation resulted in only your female half cumming.  You " + (player.lowerBody.balls > 0 ? "feel your balls.  Although they're" : "insert a finger and massage your prostate.  Although it's") + " still filled with cum you feel no particular desire to get off.  How strange.  Perhaps in future you'll find some way to take advantage of this weird situation.  ");
	outputText("Katherine purrs and hugs you close, asking, “<i>Did you like it?  Did it feel great for you too?</i>”\n\n");

	outputText("You assure her it was good for you too" + (player.lowerBody.cockSpot.hasCock() ? ", despite the fact you didn't cum" : "") + ", then you both get dressed and head back to Tel’Adre.");
...
private function bedroomBondage():void {
	...
	if (player.lowerBody.cockSpot.hasCock() && x > -1) { //Males and herms (with small enough junk to fit)
		...
		outputText("You position yourself on the bed and line " + (player.lowerBody.cockSpot.count() > 1 ? "up one of your dicks" : "your cock") + " with Kath’s slit.  " + (hasCock() ? "Her cum is still dripping on your back when you push inside her.  " : "") + "She’s incredibly wet and her pussy opens itself before you, " + (hasCock() ? "almost like it’s jealous of the treatment her male side received" : "begging for more even though she's shaking her head") + ".\n\n");
		...
		if (player.cumQ() <= 500)
			...
			else outputText("Since her womb is already filled with a child it’s her vagina that takes the brunt of it.  Before your eyes a small hard bulge forms just above her cock" + cockMultiple("", "s") + " as the upper end of her birth canal is forced wide open by the flood from your " + (player.lowerBody.balls > 0 ? "balls" : "prostate"));
			outputText(".  The rest of your cum flows back around your cock, pouring out onto the bed.");
		}
		else {
			outputText("your " + (player.lowerBody.balls > 0 ? "overactive balls" : "unnatural prostate") + ", fueled by the strange magic of this world, inject" + (player.lowerBody.balls > 0 ? "" : "s") + " an impossible amount of sperm into your cat " + catGirl("girl", "morph") + " lover.  The breath is forced from Kath’s lungs and ");
			if (pregSize() == 0) {
				...
				outputText(".  Kath’s belly grows larger and larger, taking in all the cum you can give her and growing by leaps and bounds.  She soon looks five or six months pregnant and her belly button pops out.  But your cum " + (player.lowerBody.balls > 0 ? "factories aren't" : "factory isn't") + " done.\n\n");

				outputText("Your " + (player.lowerBody.balls > 0 ? "balls fire" : "prostate fires") + " even higher pressure blasts, overcoming all resistance from Kath’s tortured belly and inflating her taut tummy until she looks nine months pregnant.   ");
				if (player.stats.cor < 25)
		...
		outputText("You position yourself above Katherine and slowly lower your " + (player.lowerBody.vaginaSpot.hasVagina() ? "pussy" : "ass") + " onto her " + (dPen ? "upper shaft.  A moment later you feel her lower shaft start to force its way into your ass" : "shaft") + ".  Kath mewls as her oversensitive cock" + (dPen ? "s disappear into your holes" : " disappears into your cunt") + ".\n\n");
...
private function drunkFuck():void {
	...
	outputText("Once you’re naked she " + (player.lowerBody.vaginaSpot.hasVagina() ? "starts roughly playing with your clit" : "jabs a finger and then two inside your ass") + ".  She’s not gentle, aiming only to get you hotter and hotter.  Kath leans in close and whispers, “<i>It’s my turn now.  You wanted me - so for now I’m callin the shots.</i>”  She ");
	if (player.lowerBody.vaginaSpot.hasVagina())
	...
	outputText(".\n\nOnly when you’re having trouble even standing does Kath release your arm");
 	if (!player.lowerBody.vaginaSpot.hasVagina()) outputText(" and yank her " + (player.lowerBody.cockSpot.hasCock() ? "fingers" : "fist") + " from your ass");
	...
	if (player.lowerBody.cockSpot.hasCock())
		outputText("The constant battering overcomes you and " + (player.lowerBody.cockSpot.count() > 1 ? "your cocks start" : "your cock starts") + " to spray, coating the door " + (player.cumQ() > 1000 ? "and then the floor " : "") + "with your sticky semen");
	else if (player.lowerBody.vaginaSpot.hasVagina())
