		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00281] == 0) {
			...
			if(display) {
				...
				//[REPEAT INTROS]
					...
					if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 33) outputText("voluptuous", false);
					else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 66) outputText("plump", false);
					...
					if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 33) outputText("bountiful", false);
					else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 66) outputText("chubby", false);
					...
					if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 33) outputText("voluminous", false);
					else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 66) outputText("chubby", false);
					else outputText("cute", false);
					outputText(" rump.", false);
				}
...
public function encounterLottie(charge:boolean = false):void {
	...
	//[REPEAT ENCOUNTER]
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] >= 50) outputText("relaxing", false);
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 50) outputText("plump", false);
		else outputText("curvy", false);
		outputText(" figure.  ", false);
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00297] == 0) outputText("Lightly sipping", false);
		else outputText("Slurping down", false);
		outputText(" what appears to be ", false);
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00297] == 0) outputText("some juice", false);
		else outputText("a milkshake", false);
		outputText(", her eyes wander over the room when she spots you and waves", false);
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] >= 70) outputText(" excitedly", false);
		...
		//[Normal Encounter, Encouragement 60 - 100]
			...
			//[Figure 0 – 20]
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 20 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00297] == 0) outputText("\"<i>Oh, " + player.short + "! Let's do our best today, I really wanna lose those pounds! I know I can do my best when I'm with you!</i>\" Lottie beams, her massive tits pushed against the table. She seems to be excited for what you've got in store today.", false);
			//[Figure 0 - 20, convinced her to accept body]
			else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 20) outputText("\"<i>Hey, " + player.mf("stud","cutie") + "! You shouldn't keep a big girl like me waiting; unless you wanna get crushed by these!</i>\" Lottie grins and grabs her luscious breasts firmly, giggling when she notices you're at full attention now. She seems to be excited for what you've got in store today.", false);
			//Figure 20 - 40, haven't convinced her to accept body]
			else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 40 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00297] == 0) outputText("\"<i>Ah, " + player.short + "! We should probably get started soon; I'm gonna push myself to the limit today!</i>\" Lottie's arms are folded over her chest, subconsciously trying to hide her nervousness, though squishing her pillowy tits in the process. She seems to be excited for what you've got in store today.", false);
			//[Figure 20 - 40, convinced her to accept body]
			else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 40) outputText("\"<i>Hey, " + player.mf("stud","cutie") + "! You shouldn't keep me waiting; the only thing I'm losing right now is my patience!</i>\" Lottie grins and punches your arm playfully, squealing when you try to get her back. She seems to be excited for what you've got in store today.", false);
			//[Figure 40 - 60]
			else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 60) outputText("\"<i>Hey! I was looking for you earlier, y'know? I can't wait to get started.</i>\" She looks happy that you've arrived. Lottie leans over to grab her beverage, giving you an excellent view of her supple mounds. She seems to be excited for what you've got in store today.", false);
			//[Figure 60 - 80]
			else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 80) outputText("\"<i>Yo, " + player.mf("big guy","babe") + "! I've been waiting for you, so you better be pumped! Ready to get started?</i>\" Lottie leans over, her creamy tits pushed against the table. She seems to be excited for what you've got in store today.", false);
			//[Figure 80 - 100]
		...
		//[Normal Encounter, Encouragement 30 - 60]
			...
			//[Figure 0 - 20]
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 20) outputText("\"<i>Oh, " + player.short + ", hey. I think we should try and get into it today. I don't think I tried hard enough last time, y'know?</i>\" Lottie leans over to grab her beverage, her massive tits pushed against the table. She seems to be intent on trying her best today.", false);
			//[Figure 20 - 40]
			else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 40) outputText("\"<i>A-ah, " + player.short + "! We should probably get started soon; I... I want to push myself to the limit today!</i>\" Lottie's arms are folded over her chest, subconsciously trying to hide her nervousness, though squishing her pillowy tits in the process. She seems curious as to what you both plan to do today.", false);
			//[Figure 40 - 60]
			else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 60) outputText("\"<i>H-hey! I was looking for you earlier, y'know? I can't wait to get started.</i>\" She looks at ease now that you've arrived. Lottie leans over to grab her beverage, giving you an excellent view of her supple mounds. She seems curious as to what you both plan to do today.", false);
			//[Figure 60 - 80]
...
private function drinkWaterWithYourHamhock():void {
	...
	if(flags[FlagEnum.TIMES_HAD_YOGA] == 0) outputText("You've never had any experience with the yoga classes first hand, but you tell ", false);
...
private function drinkLustWithYourHamhock():void {
	...
	//[if balls]
	if(player.lowerBody.balls > 0) outputText(" while her fingers nervously trace around your balls", false);
...
public function analLottieMilkshake():void {
	...
	if(player.lowerBody.balls > 0) outputText(", her soaked pussy rubbing against your balls", false);
	outputText(".\n\n", false);

	//[If Multi]
	if(player.lowerBody.cockSpot.count() > 1) {
		outputText("Deciding you aren't done with her bouncy rear, you take ", false);
		if(player.lowerBody.cockSpot.count() == 2) outputText("your other cock", false);
		...
		if(player.lowerBody.cockSpot.count() == 2) outputText("your two ", false);
	...
	if(player.lowerBody.balls > 0) outputText("Your " + sackDescript() + " begins to swell", false);
...
public function getSuckedOffByAHamSandwich():void {
	...
	if(player.lowerBody.cockSpot.list[x].cockLength <= 4) {
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			...
			if(player.lowerBody.cockSpot.count() == 2) outputText(" pokes", false);
	...
	//[If the player has a cock between 5' – 12']
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText(" biggest head lightly", false);
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText(", jerking off your " + cockDescript(player.biggestCockIndex2()) + " with both hands", false);
	...
	//[If the player has a cock equal to or over 13']
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("biggest erection", false);
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText(" - using her hands to massage your leftover " + cockDescript(player.biggestCockIndex2()), false);
	...
	if(player.lowerBody.balls > 0) outputText(", your sack slapping against her chin ", false);
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText(" while you play with your " + biggestBreastSizeDescript(), false);
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		if(player.lowerBody.cockSpot.count() == 2) outputText("  Your extra cock flails wildly, slapping against her cheeks.", false);
		else if(player.lowerBody.cockSpot.count() > 2) outputText("  Your extra cocks flail wildly, slapping against her cheeks.", false);
	}
	...
	//[if Multi]
	if(player.lowerBody.cockSpot.count() > 1) outputText(" stops rubbing your " + cockDescript(x) + " to ", false);
	//(Otherwise)
	...
	//If Balls]
	if(player.lowerBody.balls > 0) outputText("knead your " + ballsDescriptLight(), false);
	...
	//[If the player has a cock equal to or over 13']
	if(player.lowerBody.cockSpot.list[x].cockLength > 12) outputText("your sensitive tip brushing against her eager mouth", false);
	//Otherwise]
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText(" and chokes on your meat", false);
...
private function lottieHighMoraleFatLoveSelfOneTimeEvent():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() == 2) outputText(" does", false);
...
private function lottieHighMoraleFatYoullGetThereOneTimeEvent():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		outputText("Your other cock", false);
		if(player.lowerBody.cockSpot.count() == 2) outputText(" twitches ", false);
...
private function lottieElleThreesomeFirstTime():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("of one ", false);
...
private function fuckElleInsteadOfThatFatSlutFirstTime():void {
	...
	if(player.lowerBody.cockSpot.count() == 1) {
		...
		if(player.lowerBody.balls > 0) outputText("massaging your balls", false);
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("massaging your pussy", false);
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() == 2) outputText(" throbs, spilling out its essence along the floor and onto Lottie.", false);
...
private function lottieSuccessOneShot():void {
	...
	//[Otherwise]
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("tits", false);
...
private function waitAndShowLottieYourPoolBoner():void {
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 15) outputText("pride and joy", false);
	else if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() < 5) outputText("secret shame", false);
...
public function lottieRepeatSUPERNEATORARES():void {
	...
	if(player.lowerBody.cockSpot.count() == 1) {
		...
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("pussy", false);
...
public function ifrisAndLottieGetsNTRedLiftingWeights():void {
	...
	if(player.lowerBody.balls > 0) outputText("  Your pace doesn't slow, even when your swollen balls smack loudly against her chin.", false);
	outputText("  You let out a deranged chuckle at the sight of Ifris being thrown about", false);
	if(player.lowerBody.balls > 0) outputText(", sweaty gonads colliding into her", false);
