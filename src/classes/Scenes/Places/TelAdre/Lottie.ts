	...
	if (model.time.hours >= 15 && model.time.hours <= 23) {
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00281] == 0) {
			...
			if(display) {
				//[RANDOM INTROS]
				//[Encouragement 0 – 30]
				if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] < 30 && rand(6) == 0) {
					...
					if(silly()) Render.text("  What can you say? You love making bacon.", false);
				}
				//[Encouragement 0 – 30]
				else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] < 30 && rand(5) == 0) {
					...
					if(silly()) Render.text("  What can you say? Her pork buns are the best.", false);
				}
				//[REPEAT INTROS]
				else if(rand(2) == 0) {
					...
					if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 33) Render.text("voluptuous", false);
					else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 66) Render.text("plump", false);
					else Render.text("curvy", false);
					Render.text(" pig girl jogs on the spot to wake herself up - unintentionally jiggling her " + lottieCup() + "-cup breasts and soft ass in the process. She walks over to the nearby cafe and sits down on her ", false);
					if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 33) Render.text("bountiful", false);
					else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 66) Render.text("chubby", false);
					else Render.text("cute", false);
					Render.text(" rump.", false);
				}
				else {
					...
					if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 33) Render.text("voluminous", false);
					else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 66) Render.text("chubby", false);
					else Render.text("cute", false);
					Render.text(" rump.", false);
				}
...
private lottieCup():string {
	//(d/e/f/g)
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] >= 75) return "D";
	else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] >= 50) return "E";
	else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] >= 25) return "F";
	else return "G";
}
...
public encounterLottie(charge:boolean = false):void {
	...
	//[REPEAT ENCOUNTER]
		...
		Render.text("You approach the pig girl ", false);
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] >= 50) Render.text("relaxing", false);
		else Render.text("sitting timidly", false);
		Render.text(" at a table, wearing her low-cut top and booty shorts that accentuate her ", false);
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 50) Render.text("plump", false);
		else Render.text("curvy", false);
		Render.text(" figure.  ", false);
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00297] == 0) Render.text("Lightly sipping", false);
		else Render.text("Slurping down", false);
		Render.text(" what appears to be ", false);
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00297] == 0) Render.text("some juice", false);
		else Render.text("a milkshake", false);
		Render.text(", her eyes wander over the room when she spots you and waves", false);
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] >= 70) Render.text(" excitedly", false);
		Render.text(".  You walk over to her and sit down, listening to Lottie update you on her progress.\n\n", false);

		//[Repeat If didn't meet requirements]
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00299] == 0 && (player.isNaga() || player.isTaur() || player.tone < 50)) {
			Render.text("\"<i>Hey, " + player.short + "! I...I'm really glad I talked about myself with you, you're a really great friend, y'know. I really think I can achieve something this time, if I work hard!</i>\" You smile and tell her how glad you are that she's doing this. Lottie responds with a delighted expression, and continues to talk. \"<i>Anyway, I've been trying to look for a personal trainer, but it's kinda difficult, heh. I mean... I'm sure there's lots of trainers here and all, but I – I'd prefer someone I know personally, right?</i>\" She looks up at you expectantly, but then looks back towards the floor. </i>\"But I guess we can't always have what we want.</i>\"\n\n", false);
		...
		else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00281] == 2 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00298] > 0) {
			Render.text("\"<i>Hey, " + player.short + "! I...I really enjoyed what we did. God, if that's what it takes to get healthy I wouldn't mind! I think I'm falling in love with you already, haha!</i>\" She forces a giggle at her last sentence, albeit rather nervously. You raise an eyebrow, but decide not to push the issue further.", false);
		}
		//[Second Encounter, If didn't fuck Lottie]
		else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00281] == 2) {
			Render.text("\"<i>Hey, " + player.short + "! I...I'm really glad we're doing this, y'know? I really think I can achieve something this time, if I work hard – and if you work hard with me!</i>\" You smile and tell her how glad you are that she's doing this. Lottie responds with a delighted expression as you both return to normal conversation.", false);
		...
		else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00301] == 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00300] > 30 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] <= 30) {
			Render.text("\"<i>Hey, " + player.short + ", I... I really don't know w-what's gotten into you lately. Please, please stop being so m-mean, ba- I mean, " + player.mf("sir","ma'am") + ". P-please don't be like this.</i>\" She forces a smile at her last sentence, though you can tell she's incredibly nervous. You raise an eyebrow, but decide not to push the issue further.", false);
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00301] = 1;
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00300] = flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280];
		}
		//[Encouragement moved from 31 to 30, regular]
		else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00301] > 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00300] > 30 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] <= 30) {
			Render.text("\"<i>H-hi " + player.short + "... I noticed that you're... y-you're acting like you do when I do something wrong. Did I do something? P-please, I really don't like seeing you so m-mean. Please don't be like this.</i>\" She forces a smile at her last sentence, though you can tell she's incredibly nervous. You raise an eyebrow, but decide not to push the issue further.", false);
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00300] = flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280];
		}
		//[Encouragement moved from 30 to 31
		else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00300] <= 30 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] > 30) {
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00300] = flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280];
			//first time
			if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00302] == 0) {
				Render.text("\"<i>H-hiya, " + player.short + "! I'm... I'm really glad you're being nice again. To be honest, when you're angry, it – I get really scared, y'know? But I'm glad even if we're just like this.</i>\" She throws you a genuine smile, she's confident that you won't mistreat her again. You smile back, silently assuring her that you won't, today.", false);
				flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00302] = 1;
			}
	...
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00300] = lottieMorale();
	if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00308] == 0) flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00308] = lottieTone();
}
...
private declineBeingLottiesGymHamsandwich():void {
	spriteSelect(36);
	Render.text("", true);
	Render.text("\"<i>O-oh, um... okay. I understand! You're probably busy what with all that lifting you're getting up to, haha!</i>\" The pig girl chuckles nervously, shifting her towel to cover more of her soft cleavage.  \"<i>I'll uh – I'll go get outta your hair then. I've got some stuff to do at home.</i>\"  You watch as she jogs off through the door and out into Tel'Adre, towel and all.  You decide you better return home too, and head back towards the camp.", false);
	//[PC no longer encounters Lottie.]
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00278] = 1;
	doNext(camp.returnToCampUseOneHour);
}
...
public gotInShapeComedyDeclineLottie():void {
	spriteSelect(36);
	Render.text("", true);
	Render.text("Haha, of course you fucking aren't, why would you be? Lottie stares at you, tears welling in her eyes. Oh, whoops – you must've said that out loud. Proceeding to not give two shits about the bawling pig-slut in front of you, you take a good squeeze of her cushiony tits before whipping your " + cockDescript(player.biggestCockIndex()) + " out and slapping her in the face with it.  \"<i>W-w-why... why are you... *sniff* p-please...!</i>\" The pig girl chokes in between sobs. It almost gives you a hard on - almost. You take one final step backwards, before announcing quite loudly to the other cafe-goers that you're going for a home run. Lottie squeaks before you take your " + cockDescript(player.biggestCockIndex()) + " and swing at her face, hitting dead on and managing to leave a bit of pre-cum on her newly-christened cheek. You stand triumphantly at your latest work of art, though Lottie seems to have other ideas, running out the door in complete misery, pre-cum and all. You decide you'd better return home too, and head back towards the camp – but not before spending the next hour or so chasing after Lottie and taking grabs at her plush ass.", false);
	//[PC no longer encounters Lottie.]
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00278] = 1;
	doNext(camp.returnToCampUseOneHour);
}
...
private gotInShapeAcceptTrainingOffer():void {
	...
	Render.text("You take this opportunity to get a good look at Lottie. She stands at around 4'8</i>\", considerably short on average, ", false);
	if(player.tallness > 60) Render.text("and shorter than you", false);
	else if(player.tallness > 53) Render.text("but you can't judge, being roughly the same height", false);
	else Render.text("taller than you", false);
...
private drinkLustWithYourHamhock():void {
	...
	Render.text("You tell Lottie you've decided you're going to take her for a vigorous work out. She can only pant with anticipation.", false);
	dynStats("lus", 99);
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00298]++;
	//[Oral][Anal]
	simpleChoices("Oral", getSuckedOffByAHamSandwich, "Anal", analLottieMilkshake, "", null, "", null, "", null);
...
public analLottieMilkshake():void {
	spriteSelect(36);
	Render.text("", true);
	Render.text(images.showImage("lottie-fuck-her-in-teh-butzor"));
	let x:number = player.cockThatFits(40);
	if(x < 0) x = 0;
	Render.text("You brush her grip off of " + oMultiCockDesc() + ", only to turn her around and fit your girth between her large cheeks", false);
	if(player.totalCocks() == 2) Render.text(", managing to squeeze in your other cock along with it", false);
	else if(player.totalCocks() > 2) Render.text(", the rest of your dicks poking and prodding against her soft flesh", false);
	Render.text(".  \"<i>You sure do like my butt, huh?</i>\"  She giggles softly, satisfied that at least some of her fat is being put to use.  You groan as she starts to move her ample hips, massaging your " + cockDescript(x) + " within her jiggling buttocks.  Your tip brushes against her asshole, smearing pre-cum all over the hot entrance.  Lottie starts to push back towards your cock, clearly overcome with lust.  You wait a few seconds as she aimlessly grinds against you before you spread her cheeks and plunge your " + cockDescript(x) + " into her tight backdoor. Lottie squeals against the wall in delight as you fill her stomach and begin pounding away at her relentlessly, her large tits bouncing rhythmically. She continues to drive herself into you, her tight, fiery insides squeezing your length whilst you watch her heavy butt ripple with every thrust.\n\n", false);

	Render.text("Lottie is a complete mess, gasping and convulsing underneath you as you plunge into her depths. Her mouth hangs wide open as she holds on to one of her plush tits, using her free hand to pleasure herself. You take hold of her lavish buttcheeks and force your length in deep, bottoming out inside her hot ass. You remain inside her and begin gyrating your hips, rubbing against the tight walls of her interior. Her legs collapse underneath you and you wrap one hand around her pot belly to hold her in place, resuming your thrusting.\n\n", false);

	Render.text("\"<i>A-aa-ah, I-I... AHH!</i>\" Lottie oinks noisily, her eyes rolling to the back of her head and her large thighs slapping against your own, pulling your " + cockDescript(x) + " further in. She shudders in orgasm", false);
	if(player.lowerBody.balls > 0) Render.text(", her soaked pussy rubbing against your balls", false);
	Render.text(".\n\n", false);

	//[If Multi]
	if(player.lowerBody.cockSpot.count() > 1) {
		Render.text("Deciding you aren't done with her bouncy rear, you take ", false);
		if(player.lowerBody.cockSpot.count() == 2) Render.text("your other cock", false);
		else Render.text("one of your cocks", false);
		Render.text(" and push it against her puckered hole. \"<i>H-hey, wait a mome- OHH!</i>\" You ease yourself back into her, with ", false);
		if(player.lowerBody.cockSpot.count() == 2) Render.text("your two ", false);
		else Render.text("two of your ", false);
		Render.text(" throbbing cocks buried deep in her rectum. The pressure against your hot rods is immense, though you continue to slowly push and pull her sweaty ass against you, enjoying the feeling of her meaty fuckpillows as they massage your cock", false);
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s", false);
		Render.text(".\n\n", false);
	}
	if(player.lowerBody.balls > 0) Render.text("Your " + sackDescript() + " begins to swell", false);
	else Render.text("You feel yourself about to explode", false);
	Render.text(" with the large amount of cum you've prepared to paint her inner walls with.  Your unrelenting fucking picks up the pace as you feel yourself coming to a climax.  Lottie's soft flesh starts to quiver, signaling her second orgasm.  You lift Lottie's plump body up straight and against yours as you groan loudly and thrust hard, the noisy broad ", false);
	if(player.cumQ() >= 1000) Render.text("overflowing", false);
	else if(player.cumQ() >= 500) Render.text("dripping", false);
	else Render.text("filled", false);
	Render.text(" with your hot, thick spunk. She squeals again as you bring her to orgasm, ramming herself into you to collect as much of your cum as possible. You continue to pound her bouncy buttocks as you release the rest of your seed inside of her", false);
	//[If large cum production]
	if(player.cumQ() >= 500) Render.text(", a heavy amount leaking from her ass", false);
	Render.text(".  Pulling yourself free, you wipe yourself against her plush butt and give it one final slap before walking out of the sauna.  \"<i>I-I'll see you again, hey?</i>\" Lottie asks, collapsed onto the floor in a mess of sweat and drool. You give her a thumbs up before turning around and heading out of the gym, making sure to grab a ", false);
	if(silly()) Render.text("bacon ", false);
	Render.text("shake on the way out.", false);
	//LOTTIE FIGURE +1, MUSCLE TONE +1, DRAINS PC'S LUST.
	lottieTone(1);
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00307] == 0) flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00307] = 1;
	Render.text(player.modTone(100,1), false);
	player.orgasm();
	doNext(camp.returnToCampUseOneHour);
}
...
public getSuckedOffByAHamSandwich():void {
	spriteSelect(36);
	Render.text("", true);
	Render.text(images.showImage("lottie-gives-you-a-blowjob"));
	let x:number = player.biggestCockIndex();

	Render.text("You grab Lottie's shoulders and firmly move her down towards your crotch. She kneels down, face to face with your " + cockDescript(x) + ".  Feeling her hot breath against your " + player.skinFurScales() + ", you tell her to suck it.\n\n", false);

	//[If player cock is at or below 4']
	if(player.lowerBody.cockSpot.list[x].cockLength <= 4) {
		Render.text("She stares at your " + cockDescript(x) + " and then up at you. \"<i>I... oh, well I guess this is okay.</i>\" She lazily slurps your member into her mouth, and moans at the taste of your " + cockDescript(x) + ". Her agile tongue moves over your entire length, licking gently over your sensitive head and along your shaft. Your " + cockDescript(x) + " sits comfortably inside her warm hole, covered in her saliva", false);
		if(player.lowerBody.cockSpot.count() > 1) {
			Render.text(" while your other cock", false);
			if(player.lowerBody.cockSpot.count() == 2) Render.text(" pokes", false);
			else Render.text("s poke", false);
			Render.text(" against her chin", false);
		}
		Render.text(".\n\n", false);
	}
	//[If the player has a cock between 5' – 12']
	else if(player.lowerBody.cockSpot.list[x].cockLength <= 12) {
		Render.text("She stares intently at your " + cockDescript(x) + " with hunger in her eyes. A small amount of drool escapes her luscious lips as she moves forward, kissing your ", false);
		if(player.lowerBody.cockSpot.count() > 1) Render.text(" biggest head lightly", false);
		else Render.text(" head lightly", false);
		Render.text(".  Her saliva leaks onto your " + cockDescript(x) + ", though she is quick to lick it up while she works her tongue around the rest of your shaft", false);
		if(player.lowerBody.cockSpot.count() > 1) Render.text(", jerking off your " + cockDescript(player.biggestCockIndex2()) + " with both hands", false);
		Render.text(".  She places small licks on the tip of your head, and then hungrily devours the rest of your length in one fell swoop.\n\n", false);
	}
	//[If the player has a cock equal to or over 13']
	else {
		Render.text("She gasps as you lift your " + cockDescript(x) + " and slap her face. \"<i>It's... wow, " + player.short + "!</i>\" You grin before poking her pudgy cheek with the tip of your ", false);
		if(player.lowerBody.cockSpot.count() > 1) Render.text("biggest erection", false);
		else Render.text("erection", false);
		Render.text(".  A small amount of drool escapes her luscious lips as she moves forward, kissing your head lightly. Her saliva leaks onto your " + cockDescript(x) + ", though she is quick to lick it up while she works her tongue around the rest of your lengthy shaft", false);
		if(player.lowerBody.cockSpot.count() > 1) Render.text(" - using her hands to massage your leftover " + cockDescript(player.biggestCockIndex2()), false);
		Render.text(". She places small licks on the tip of your head, and without warning stuffs as much of your thick girth as she can into her mouth, coughing around your " + cockDescript(x) + ".\n\n", false);
	}

	Render.text("She begins fellating your cock at an incredible rate, throatfucking herself on your " + cockDescript(x), false);
	if(player.lowerBody.balls > 0) Render.text(", your sack slapping against her chin ", false);
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) Render.text(" while you play with your " + biggestBreastSizeDescript(), false);
	Render.text(". You remark on what a hungry little piggy she must be, and the chubby girl moans in response, the vibrations massaging your member. Relishing the feeling of your " + cockDescript(x) + " in between her hungry lips, you start to thrust into her warm mouth hole, grabbing onto her head as you pound her face against your groin.", false);
	if(player.lowerBody.cockSpot.count() > 1) {
		if(player.lowerBody.cockSpot.count() == 2) Render.text("  Your extra cock flails wildly, slapping against her cheeks.", false);
		else if(player.lowerBody.cockSpot.count() > 2) Render.text("  Your extra cocks flail wildly, slapping against her cheeks.", false);
	}
	Render.text("  Lottie's slick tongue desperately wraps itself around your length as you drive in and out of her fuckhole relentlessly, drool beginning to leak from her mouth. She", false);
	//[if Multi]
	if(player.lowerBody.cockSpot.count() > 1) Render.text(" stops rubbing your " + cockDescript(x) + " to ", false);
	//(Otherwise)
	else Render.text(" reaches down towards her dripping pussy; plunging two fingers into her wetness while using the other hand to ", false);
	//If Balls]
	if(player.lowerBody.balls > 0) Render.text("knead your " + ballsDescriptLight(), false);
	else Render.text("reach around and squeeze your " + buttDescript(), false);
	Render.text(".  You grab the back of her head and pull her in further, bobbing her head while she slurps and sucks at your meatstick. The stimulation almost causes you to explode right then and there, but her bouncing tits give you an idea.\n\n", false);

	Render.text("You slowly take your " + cockDescript(x) + " out of her hot mouth, a small *pop* breaking the silence as it escapes her plump, puckered lips. Grabbing one of her hefty tits in your hands, you rub your cockhead against her erect nipple, eliciting a small moan from the sweaty slut. You slap your " + cockDescript(x) + " against her sweaty chest a few times before burying it in between her cleavage, ", false);
	//[If the player has a cock equal to or over 13']
	if(player.lowerBody.cockSpot.list[x].cockLength > 12) Render.text("your sensitive tip brushing against her eager mouth", false);
	//Otherwise]
	else Render.text("resting comfortably in between her cushiony fuckpillows", false);
	Render.text(". Lottie wraps her tits around your length and begins working at pleasuring your every inch.\n\n", false);

	//[If the player has a cock equal to or over 13']
	if(player.lowerBody.cockSpot.list[x].cockLength > 12) Render.text("Licking at your cockhead, Lottie squishes her breasts against the rest of your member, making sure to lap up the stray beads of pre-cum squirting from your tip. Relishing the taste of your " + cockDescript(x) + " in her mouth she begins to suck at your head, flicking her tongue around your urethra and pressing her full lips against it, creating a vacuum of pleasure around your manhood.\n\n", false);

	//[If Multi]
	if(player.lowerBody.cockSpot.count() > 1) {
		Render.text("You decide to make use of ", false);
		if(player.lowerBody.cockSpot.count() == 2) Render.text("your", false);
		else Render.text("an", false);
		Render.text(" extra cock as you push it between her pouty lips, her face brushing against it while she tries to titfuck your main member. Looking up at you, Lottie descends on you and begins fellating your " + cockDescript(player.biggestCockIndex2()) + ", not forgetting to knead her fat tits into your " + cockDescript(x) + ".\n\n", false);
	}

	Render.text("Raising her body up and down, you watch as the pig-slut massages her rack tightly around your rod whilst you plunge between her cleavage, thrusting slightly to match her movements. \"<i>" + player.short + ", p-please cum on me, god- I need this!</i>\" she moans, her saliva lubing her chest while she pumps your dick in between her melons", false);
	if(player.lowerBody.cockSpot.count() > 1) Render.text(" and chokes on your meat", false);
	Render.text(".  You throb against her hot flesh, feeling your spunk bubbling up inside you.  Throwing your head back, you give one final thrust before pulling back and exploding all over your squishy slut, ", false);
	if(player.cumQ() < 500) Render.text("thick ropes of cum splattering all over her face and pooling in between her tits", false);
	else Render.text("massive globules of cum splattering all over her face and painting her tits", false);
	Render.text(".  She continues to move up and down against the thick shaft between her breasts, determined to drain you of your seed. She pulls you out of her bosom and plunges your member back into her mouth, suckling on it for any stray milk left. After a while, you groan as she pops you back out, collecting the ", false);
	if(player.cumQ() > 500) Render.text("incredible amounts of ", false);
	Render.text("cum you've painted on her flesh and gulping it all down. \"<i>Thanks for the load, " + player.mf("big guy","babe") + ",</i>\" she remarks, sucking gently on her fingers. \"<i>I'll... I'll see you again, hey!</i>\" You pat her on the head and begin to make your way out of the gym, making sure to grab a ", false);
	if(silly()) Render.text("bacon ", false);
	Render.text("shake on the way out.", false);
	//LOTTIE FIGURE -1, MUSCLE TONE +1, DRAINS PC'S LUST.
	lottieTone(-1);
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00306] == 0) flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00306] = 1;
	Render.text(player.modTone(100,1), false);
	player.orgasm();
	doNext(camp.returnToCampUseOneHour);
}
...
public lottieTone(dif:number = 0):number {
	if(dif == 0) return flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279];
	if(debug) Render.text("\n<b>(DEBUG: Lottie Tone changed by " + dif + "!)</b>\n", false);
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] += dif;
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] > 100) flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] = 100;
	else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] < 0) flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] = 0;
	//Clear blowjob/anal change talk flags in case tone is getting out of whack.
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00306] == 1) flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00306] = 0;
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00307] == 1) flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00307] = 0;
	return flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00279];
}
public lottieMorale(dif:number = 0):number {
	if(dif == 0) return flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280];
	if(debug) Render.text("\n<b>(DEBUG: Lottie Morale changed by " + dif + "!)</b>\n", false);
	flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] += dif;
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] > 100) flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] = 100;
	else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] < 0) flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] = 0;
	return flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00280];
}
...
private talkToPigSlut():void {
	...
	else if(lottieMorale() <= 70 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00308] - 5 > lottieTone()) {
	...
	else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00308] - 5 > lottieTone()) {
	...
	else if(lottieMorale() <= 30 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00308] < lottieTone() - 5) {
	...
	else if(lottieMorale() <= 70 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00308] < lottieTone() - 5) {
	...
	else if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00308] < lottieTone() - 5) {
		Render.text("Her ears perk up at your eagerness to talk; she seems to be pretty excited about something. You ask her what's on her mind.  \"<i>That obvious, huh?</i>\" Lottie looks up at you and grins. \"<i>Y'see babe, since the last time we met, I've actually – I've lost weight!</i>\" She stands up, pumping her fist in the air and wiggling her plump ass. \"<i>I mean, I know it's not a big difference, but it's still a result! This is amazing!</i>\" She's clearly pleased with herself. You place a hand on her shoulder, and she beams at you. Do you encourage her, or break the bad news?\n\n", false);
...
private lottieMediumMoraleWeightGainEncourage():void {
	spriteSelect(36);
	Render.text("", true);
	Render.text("You tell her it isn't her fault; these things happen all the time for a variety of reasons. Her weight is bound to fluctuate now and then, it's completely natural. She looks up at you, her eyes welling up slightly. \"<i>Is that true? It's completely natural?</i>\" You nod your head. \"<i>I'm glad. I'm so glad.</i>\" She sniffs, wiping away her tears and taking a deep breath. You rub her shoulder as she places her hand upon yours. Lottie continues to talk about her day for a few more minutes before you both decide to hit the track.\n\n", false);
	Render.text("<b>...</b>\nAfter a good workout, you decide to return back to camp, exhausted.", false);
	//ENCOURAGEMENT +2
	lottieMorale(3);
	fatigue(10);
...
private lottieHighMoraleWeightGainEncourage():void {
	spriteSelect(36);
	Render.text("", true);
	Render.text("You tell her it's never her fault for something like that; and it's perfectly fine to be frustrated – as long as it doesn't detract from her main goal. Her weight is bound to fluctuate now and then, it's completely natural. She looks up at you, beaming at your kind words. \"<i>" + player.short + ", you really do know how to make people feel better, y'know?</i>\" You nod your head in agreement – you know. \"<i>I'm glad. I'm so glad.</i>\" She sighs, taking a deep breath. You rub her shoulder as she places her hand upon yours. Lottie continues to talk about her day for a few more minutes before you both decide to hit the track.\n\n", false);
	Render.text("<b>...</b>\nAfter a good workout, you decide to return back to camp, exhausted.", false);
	//ENCOURAGEMENT +2
	lottieMorale(3);
	fatigue(10);
...
private leaveLottieInsteadOfYogaSex():void {
	spriteSelect(36);
	Render.text("", true);
	Render.text("As excited as you are, your mind is telling you to leave. In blissful ignorance, you walk out the door of the cafe, through the pool area, into the main gym and out the main entrance. Once outside, you figure it's as good a time as any to take a friendly stroll back to the camp.\n\n", false);
	Render.text("As you head home, you have the strangest feeling you've missed out on incredibly sexy sex.", false);
	doNext(camp.returnToCampUseOneHour);
}
...
private boastAtLottie():void {
	spriteSelect(36);
	Render.text("", true);
	Render.text("You laugh as you pick up one of the larger ones and pump mercilessly, looking at her expectantly as if she had been failing out of laziness. She tries harder to pull the heavy object over her head, but eventually she begins to shake and drops it, landing in the sand with a ", false);
	if(silly()) Render.text("large *pomf =3*", false);
	else Render.text("loud clonk", false);
	Render.text(". She looks up at you and sniffles, but you just grunt in response, disappointed with her efforts. She sighs and tries her best to do it again.\n\n", false);

	Render.text("After a few more attempts at the weights, she eventually buckles under the pressure and gives up, walking home by herself. You snort and continue a few more well-earned reps before dropping down and recharging for the night.", false);
	fatigue(10);
	dynStats("str", .25);
	lottieMorale(-3);
	doNext(camp.returnToCampUseOneHour);
...
private trainWithLottie():void {
	...
	Render.text("After a few more attempts at the weights, you both decide you've had enough, going your separate ways and waving as you leave the gym.", false);
	dynStats("str", .1);
	lottieMorale(2);
	lottieTone(2);
	fatigue(10);
...
private sprintAndLeaveLottieBehind():void {
	...
	Render.text("A few rounds later, you've exhausted yourself so you decide to return home. You notice that Lottie's nowhere in sight, but it's her loss - not yours.", false);
...
private jogWithLottie():void {
	spriteSelect(36);
	Render.text("", true);
	Render.text("You decide that sprinting wouldn't really get either of you anywhere, though taking things a little faster wouldn't be any harm. You take the power walking to the next level with a quick jog, before reminding Lottie to take this as leisurely as possible - it isn't a race. Lottie quickly picks up the idea and starts to push herself further, trying her best to keep up with you, whilst maintaining a steady rhythm on the track. You both spend the good part of the evening jogging a fair distance, before deciding to retire for the night. You're both tired, but satisfied with today's efforts.\n\n", false);
...
private sprintAndLeaveFitLottieBehind():void {
	...
	Render.text("A few rounds later and you've exhausted yourself, so you decide to return home. You notice that Lottie's nowhere in sight, but it's her loss - not yours.", false);
	dynStats("tou", .2, "spe", .25);
	lottieMorale(-3);
	fatigue(10);
	doNext(camp.returnToCampUseOneHour);
...
private lottieYoga():void {
	...
	Render.text("", true);
	//[If Figure 0 – 100]
	Render.text("You know that Lottie has an affinity for yoga, so you decide to enter the dimly lit room at the end of the pool. She gladly follows, bouncing along with you as you take two of the mats out. A class has already started, and you can see many people gathering around a tanned, four-armed woman performing warm-up exercises. Knowing you both have a little time before they break out into the more complicated stretches; you scan around the room for optimal places to sit down without blocking your view of the tetra-armed instructor. By the time you've searched the room, a few more people had already blocked off most of the available spots. You look over to see Lottie already spread out and lying on her stomach – she looks over to you and giggles before patting the vacant area next to her. Throwing a quick smile, you also notice a small unused area right next to the teacher – you'd be able to take it if you went by yourself. Should you go at it alone, or stretch with Lottie?", false);
	//[Alone] [Together]
...
private lottieYogaAlone():void {
	spriteSelect(36);
	Render.text("", true);
	Render.text("Taking your mat, you walk over towards the four-armed yoga trainer, sitting down only a meter or two away from her tanned, muscular body. In the corner of your eye, you can see Lottie visibly disheartened as she watches you gazing at the fit, caramel-colored woman in front of you. As you take your position, the intense smell of coffee and desert flowers hits you as the instructor lays on her back and pulls both of her legs up, giving you a clear view of her large thighs and puffy mound. You swear you can see her smiling as the other surrounding members begin to imitate her position. Not wanting to disappoint, you lie on your rear and tilt your head backwards, confronted by another pair of legs lifted into the air. You can see Lottie attempting the same position, only to lean to her right and fall over, earning an annoyed stare from the person next to her.\n\n", false);
	Render.text("You continue to take on the vaguely sexual positions of the athletic yoga teacher, the class ending before you know it. Your head strangely dizzy from the smell of sweat and pheromones, you walk home by yourself, completely forgetting about Lottie.", false);
	Render.text(player.modThickness(30,1), false);
	Render.text(player.modTone(60,1), false);
	lottieMorale(-2);
	fatigue(5);
	doNext(camp.returnToCampUseOneHour);
}
...
private jumpInPoolBeforeLottieSeesYourWingWang():void {
	spriteSelect(36);
	Render.text("",true);
	Render.text("You can't risk it, so you decide to take a plunge into the water – it's freezing at first, but you quickly warm up. Lottie takes your avoidance to heart and looks visibly discouraged, though you wonder what her reaction would've been if your cockrocket", false);
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s", false);
	Render.text(" decided to say hello. Needless to say, you decide it'd be better if you swam alone – nothing's wrong with the occasional chubby-assed slut wandering now and then, but it's a pain to not be distracted. Lottie wades at the other end of the pool, doing the occasional stretch before breaking into laps. You take a page from her book and begin to work back and forth from either end, making sure not to stare at Lottie's massive chest as it bounces into the air every so often after a stroke. After a considerable amount of time, you both finish considerably more exhausted than before. She seems to have lightened up since your initial reaction to her body, so you shrug it off and head home.", false);
	//ENCOURAGEMENT -1, FIGURE +2, MUSCLE TONE +2, LUST +9001
	Render.text(player.modTone(100,2), false);
	dynStats("lus", 40);
	lottieTone(2);
	lottieMorale(-1);
	fatigue(5);
...
public hamSammitchTimeBIATCH():void {
	spriteSelect(36);
	Render.text("", true);
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00324] == 0) {
		flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00324]++;
		Render.text("You walk over to the cafe, Lottie watching as you order a sandwich from their menu. In a matter of seconds the collie-like clerk pulls one up from the glass display and hands it over to you", false);
		if(silly()) Render.text(", mumbling something about lifetime members receiving free pig-related food items", false);
		Render.text(". You exchange a smile before walking back to the table and handing Lottie the moist meal, explaining that you saw the ham sandwich the other day, figuring she might enjoy it. Lottie throws it on the floor. \"<i>What the heck is wrong with you?</i>\"\n\n", false);

		Render.text("You reach for a high five that never comes.", false);
	}
	//[If Ham Sandwich, Repeat Encounter, High Corruption]
	else if(player.stats.cor >= 75) {
		Render.text("You walk over to the cafe, Lottie watching as you order a sandwich from their menu. In a matter of seconds the collie-like clerk pulls one up from the glass display and hands it over to you, mumbling something about giving a pig-girl a ham sandwich being a stupid fucking idea. You exchange a smile before quickly walking out of the cafe and into the currently empty pool area. You hastily remove your " + player.armorName + " and take out your " + cockDescript(0) + ", jamming it straight into the sandwich and fucking it relentlessly. In moments you cum between the velvety slices of ham, leaving a large puddle inside and leaving the rest of your spunk leaking out the back and pooling onto the floor. You dress yourself before walking back to the table and handing Lottie the incredibly moist meal, explaining that you saw the ham sandwich the other day, figuring she might enjoy it – and that you've taken care into adding your own 'special mayonnaise'. Lottie throws it on the floor. \"<i>Don't cum inside ham sandwiches, asshole!</i>\"\n\n", false);
		Render.text("You reach for a high five that never comes.", false);
		player.orgasm();
	}
	//[If Ham Sandwich, Repeat Encounter]
	else {
		Render.text("You walk over to the cafe, Lottie watching as you order a sandwich from their menu. In a matter of seconds the collie-like clerk pulls one up from the glass display and hands it over to you, mumbling something about giving a pig-girl a ham sandwich being a stupid fucking idea. You exchange a smile before walking back to the table and handing Lottie the moist meal, explaining that you saw the ham sandwich the other day, figuring she might enjoy it. Lottie throws it on the floor. \"<i>Stop giving me ham sandwiches, asshole!</i>\"\n\n", false);
		Render.text("You reach for a high five that never comes.", false);
	}
	dynStats("cor", 1);
	doNext(camp.returnToCampUseOneHour);
...
public lottieFuck():void {
	...
	if(lottieMorale() <= 40) {
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00310] > 0) dog = lottieRepeatDoggieStyle;
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00311] > 0) walk = lottieRepeatWalkies;
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00316] > 0) three = lottieRepeatThreesome;
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00317] > 0) neatrares = lottieRepeatSUPERNEATORARES;
...
public lottieRepeatWalkies():void {
	spriteSelect(36);
	Render.text("", true);
	Render.text(images.showImage("lottie-fuck-her-in-public-repeat"));
	let x:number = player.cockThatFits(40);
	if(x < 0) x = 0;
	Render.text("A few minutes later and you've taken your whore outside of the gym and around to the back, where an equipment shed lies waiting. You make quick work of the lock, busting it open with a nearby metal pole. The two of you walk slowly inside, Lottie pausing behind you to gaze at the tools and exercise apparatuses littering the walls. Soon, you find what you're looking for. You've never been one to judge a person for their fetishes, but you make a mental note to thank the person who couldn't control theirs. A single leather collar attached to a leash dangles silently upon a hook attached to the walls of the shed. Grinning, you take it down, adjusting the length of the rope before handing it to Lottie. \"<i>W-what's this for, " + player.mf("sir","ma'am") + "?</i>\" she mumbles, but you pay no attention to her questioning as you loosen the collar and push it down on the girl's neck, tightening it before she can protest. \"<i>N-no! What are yo-!</i>\" Slap! You pelt your hand against her soft cheek, causing her to jerk on the rope and fall to the floor, miserable and in pain. Your slut slowly looks up at you with watering eyes, a hand to her face to quell the stinging sensation. \"<i>I... I understand.</i>\" She mutters quietly, picking up the leash and placing it in your hands, before standing on her knees and adopting a begging position in front of you. You grunt as the girl pulls out her small tongue, reluctantly licking at your groin to satisfy the beast behind your clothes. Just like a whore to assume that's what you're here for. You walk away from her attempts to fellate you, pulling on the leash and yanking your girl forward. She gets up and starts walking behind you and out into the sunlight - you want the world to know how well you've trained your fat little piggy.\n\n", false);

	Render.text("You look back to the shed to see if there's anything you might be missing. Weights, large poles, a whip, some rope, and – ah yes, the steel ring gag. 'Where there's a whip, there's a way', you chuckle to yourself as you take the strap and bring it over to Lottie. Your pig slut is naked and ready, waiting on your command as you pull out the ring-shaped device and stretch the strap over her head, placing the metal ring inside her mouth and forcing the pale slut to make a permanent 'O'-shape with her lips. You tighten the gag, causing Lottie to cough uncomfortably as drool forms at the base of her mouth. Taking the leash, you drag Lottie away from the shed, her large ass jiggling as she tries to catch up to you. You figure you'll take her to the backstreets of Tel'Adre – you want people to see your work of art, but you know only the scum of the city will truly appreciate her for what she is. You pull the leash, leading Lottie through an alleyway and into a smaller, dirtier part of the desert city, an enclosed area with several food stalls littered around.  A majority of the lower-class citizens wander these parts, shuffling about as they haggle, argue and steal – as is usual in the slums. Most people notice your pig-girl trotting behind you, though few offer a second glance. The majority stand and stare as you present your trophy – a few of the more courageous ones openly masturbate to the site, whereas the braver still walk up and slap your pig on the ass, eliciting a squeal or a moan. You decide to up the ante while you walk through the commons, placing a hand between Lottie's pale thighs and thrusting two fingers into her pussy, the heat enveloping your digits as you shamelessly fingerfuck the poor girl.\n\n", false);

	Render.text("On your walk, you find yourself in a familiar location – the back alleyway of the Wet Bitch. Figuring it wouldn't hurt to take your slut inside; you jerk your hand forward, forcing Lottie to walk towards the door – when you recognize the face of a furiously masturbating fox herm.  Urta looks at you while she pumps her thick horsecock relentlessly, winking at the pig-slut you've currently got on a leash. She doesn't seem to recognize you again, which could either mean she's somehow contracted serious amnesia, or she's a lot drunker than usual. \"<i>", false);
	if(!silly()) Render.text("Heyyy, is she like *hic* your shex slave er shumthin?", false);
	else Render.text("Heyyy, do I shmell bacon?  I fucking love bacon!  BACON BACON BACON!", false);
	Render.text("</i>\" You decide to give the drunk vixen a treat. Pulling your slick fingers out of her cunt, you push Lottie towards the gray fox-woman, tugging on the leash to jerk her backwards onto the floor. As she tries to get up, the girl squeals as Urta's large equine member slaps Lottie on the nose. You take out your own " + cockDescript(x) + ", barking at her to keep on all fours – as long as you own the backside, you couldn't care less what the fox-herm does to her face. Urta starts by rubbing the flat head of her length on the full lips of your slut, the scent of her intoxicating pre-cum enticing Lottie to place small licks along the wet tip. Not wanting to be left out, you forcefully grab her hips, gliding your " + cockDescript(x) + " straight into the slut's warm hole, groaning as you feel her walls tighten around you. You jerk as her hips suddenly thrust backwards, and you glance over to find Urta burying as much of her thick cock into the girl's mouth as she can. You both glance at each other, sharing a nod before you both plunge your lengths into the sobbing pig-whore, rocking back and forth. Urta moans while she holds onto the back of Lottie's head with one hand, fondling her own tits with the other. On the other end of your slave, you fuck relentlessly as you hold onto her fat asscheeks, panting heavily from the pleasure of them slapping against your groin. The fox-herm lets out a loud moan before pushing as much of Lottie's face into her groin as she can, her balls growing as she prepares to fill the slut's throat. You feel yourself about to explode as well, taking longer, and deeper thrusts into the girl before finally pushing forwards and filling her with your spunk, the fox-woman on the opposite end forcing her twitching, animalistic member into Lottie's throat and doing the same. The both of you slowly remove yourselves from your trophy, the pale, sweating girl now on the floor, thick seed leaking from both ends. With one more grunt you unload the rest on her ample butt, Urta painting her face.\n\n", false);

	Render.text("You decide to leave Lottie there while Urta jerks off for another round at her mouth, heading back to the gym to grab a ", false);
	if(silly()) Render.text("bacon ", false);
	Render.text("shake.", false);
	if(silly()) Render.text("  You sure do love spit roasting a pig, but nothing can beat a bacon shake.", false);
	player.orgasm();
...
public lottieRepeatCowGirl():void {
	spriteSelect(36);
	Render.text("", true);
	Render.text(images.showImage("lottie-fuck-her-cowgirl-repeat"));
	let x:number = player.cockThatFits(40);
	if(x < 0) x = 0;
	Render.text("A few minutes later and you've both gone to the yoga rooms – you've already placed a mat down while you wait for Lottie to change. Your rosy lover bounces into the empty, dim lit room, her ample thighs squishing out of a pair of super tight shorts, while her breasts fill over a sports bra. You're immediately turned on by the incredibly erotic display, Lottie catching scent of your increasingly hardening member and giggling while she crushes her boobs together in front of you. \"<i>I'm guessing you like what you see?</i>\" she purrs, walking over to you and pushing you down upon the gym mat. Lottie makes quick work of your lower clothes, your " + cockDescript(x) + " bouncing out and at attention, which she grabs hastily as she begins to jerk you off. The pig-girl presses your member against her moist slit, grinding it back and forth while her moans fill the empty room. You grunt in pleasure, jerking your hips slightly to help the process along, to which she replies by bending down to press her overflowing tits against your chest. You take your hands and slap them upon her ass, groping and squeezing while she moans into you. The girl shudders in bliss as the grinding sends her into an early orgasm. \"<i>I... I've got something better for you, babe...</i>\" she pants as she pulls apart the opening in her shorts to reveal her blushing pussy. Keeping a firm grip on your " + cockDescript(x) + ", Lottie lifts herself up and slowly lowers her body upon it, your tip parting her folds and entering her hot insides. You grunt at the sensation of her inner walls squeezing tightly against your member, ", false);
	if(player.cockArea(x) <= 40) Render.text("her ample cheeks resting on your groin as you bottom out in her", false);
	else Render.text("though you only reach so far before you've filled her up entirely with your length", false);
	Render.text(". You decide to take matters into your own hands as you grab her thick hips and drive upwards, Lottie squealing as you begin to pump in and out of her. The pig-girl's creamy breasts pop out of her bra and jump to your thrusts, her fat legs hugging either side while she bounces upon you. \"<i>God, I-I love you! Unh, I feel so hot, babe!</i>\" She yells as you pork your lover enthusiastically. The slaps of her full buttcheeks hitting your groin pervade the air, your grunts accompanied by Lottie's orgasmic moaning.\n\n", false);
	...
	Render.text("A while later, you've both said your goodbyes for the night. Before you leave, you make sure to head out to the cafe to buy yourself a ", false);
	if(silly()) Render.text("bacon ", false);
	Render.text("shake.", false);
	if(silly()) Render.text("  You love bacon shakes.", false);
	player.orgasm();
	dynStats("sen", -2);
...
public lottieRepeatQuickie():void {
	...
	Render.text("The nude pig-girl stands in front of you, turning red at your hungry gaze. \"<i>Will you f-fuck my pussy, " + player.short + "?</i>\"\n\n", false);

	Render.text("In moments flat you remove your clothes to jump the piggy, whipping out your " + cockDescript(x) + " and pushing it against Lottie's wet slit, the pig-girl lying on her back while her legs rest upon your shoulders. With a hearty thrust you drive yourself into her, eliciting a loud moan while you pork her insides with your meat. Her thick thighs slap against you as you bend over to take a lustful grab at her melons, sucking at her nipple and pinching the other, her sweet taste enveloping your mouth. Lottie buries you in her cleavage, yelping as you piledrive her tight pussy into submission – but you've got more in store. Flipping her over, Lottie lies face down with her soft butt jutting up in the air, your persistent fucking driving her wild. You continue gripping onto her tits as you pound her flesh, every thrust accompanied by her pleasured grunts. Back and forth you mount her until the familiar sensation in your loins signal your release. You increase your pace, pounding her pussy until you pull your head back and ejaculate inside her inner walls, slamming in harder every few seconds with each burst.", false);
	if(player.lowerBody.cockSpot.count() > 1) {
		Render.text("  Your other cock", false);
		if(player.lowerBody.cockSpot.count() == 2) Render.text(" twitches ", false);
		else Render.text("s twitch ", false);
...
public lottieRepeatSUPERNEATORARES():void {
	...
	let x:number = player.cockThatFits(40);
	if(x < 0) x = 0;
	Render.text("You both walk to the yoga rooms, to find Elle stretching in the empty area. She spots the both of you, waving hello -  but you all have other, dirtier intentions in mind. Elle strips down, her perky tits letting loose. You chuckle devilishly as you undress in front of your naked and wanting harem – but you only have eyes for one girl. You point at the spider-girl, informing the both of them that you only plan to fuck her. Lottie may have thought she could win you over with another slut, but you feel as if you should teach her a lesson about being too presumptuous about what you want. Elle walks towards you, her child-bearing hips swaying hypnotically as she ", false);
	if(player.lowerBody.cockSpot.count() == 1) {
		Render.text("begins to caress your " + cockDescript(x) + " with two of her hands, while the other sets to work on massaging your ", false);
		if(player.lowerBody.balls > 0) Render.text("balls", false);
		else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("pussy", false);
		else Render.text("ass", false);
	}
	else  Render.text("uses all four of her hands to handle " + sMultiCockDesc() + ", jerking each one with ease", false);
...
public lottieRepeatBackwardsAssCowgirl():void {
	...
	let x:number = player.cockThatFits(40);
	if(x < 0) x = 0;
	Render.text("You both walk into the yoga area, met with the smell of sweat and flowers pervading the room. Lottie gently asks for you to lie down on your back, to which you comply quite easily. You're comfortable, folding your arms behind your head to give you leverage as you watch your lover walk slowly to you. Lottie turns around, her thick thighs straddling you as she sits down on your groin with her back facing your direction, giving you  a perfect view of her curvy body and plump ass. Your " + cockDescript(x) + " nestles gently between her rosy cheeks, pulsing with lustful intent as the girl grabs her hips and begins to pleasure you through her ass. The silence of the room breaks at your fevered pants and grunts as Lottie's skillful assjob sends you through fits of ecstasy, every curve of her bottom jiggling against the heat of your member. Her pace picks up while she bounces upon you, squeezing and squashing her perfect ass until one final lift in the air is met with her plunging your length up into her hot interior. The feeling of your molten cock inside her immediately sends Lottie into fits of orgasmic bliss, the intense heat filling the pig-girl with pleasure. Her attempts to continue are thwarted by your " + cockDescript(x) + " scraping against her inner walls, her thick thighs clenched together while she slowly moves up and down upon you. Deciding to take things into your own hands, you thrust upwards, almost pushing the girl off with your strength and causing Lottie to squeal as you begin pounding her irresistible hole. Her lush buttcheeks jiggle with every movement you make, the rising and falling of her body moving in accordance to your own will. You pump in and out, keeping a steady rhythm inside the hot and bothered woman.\n\n", false);

	Render.text("Inevitably, you can feel your release approaching, and with one final push you grab a hold of the pig-girl's tight ass, slamming against her and pumping your spunk directly into her clenched pussy, your " + cockDescript(x) + " twitching as ropes of cum coat her insides. Lottie yelps, collapsing on the floor and full of your hot seed. You wipe your cock along her ass cleavage, leaving her as a sweaty, cum-filled mess on the floor. Walking out, you make sure to grab a ", false);
	if(silly()) Render.text("bacon ", false);
	Render.text("shake from the cafe. They're fantastic.", false);
	player.orgasm();
	dynStats("sen", -2);
	doNext(camp.returnToCampUseOneHour)
}
...
public lottieRepeatFEMDOMFEMFDOM():void {
	...
	let x:number = player.cockThatFits(40);
	if(x < 0) x = 0;
	Render.text("You both walk into the yoga area and are met with the smell of sweat and flowers pervading the room. You look around, unable to find your lover until –THUNK! A heavy weight is brought down upon your head, pulling you down to the ground and away from consciousness as everything fades to black.\n\n", false);
	...
	//[If Cum Production High]
		...
		Render.text("Unfortunately for her, you have far too much built up to stop now. With another thrust, your " + cockDescript(x) + " pulses in delight, exploding in orgasm as ropes of spunk splatter upon Lottie's face and tits. The girl gasps, the sheer amount of your seed painting her white. \"<i>N-no, stop! Oh god!</i>\" she yelps, before her mouth is filled with your hot essence. She forces herself to swallow, coughing as you grin at the defeated slut. \"<i>O-okay, y-you win, " + player.short + ".</i>\" she looks up at you and smiles, before untying your binds and setting you free. \"<i>But you have to admit, this was pretty fun, right?</i>\" You give her a thumbs up before walking out the door, leaving her to clean your thick mess. On the way out, a milkshake vendor waves over to you – it's the guy from the cafe. You buy a ", false);
		if(silly()) Render.text("bacon ", false);
		Render.text("shake before deciding to head home.", false);
		if(silly()) Render.text("  You fucking love bacon shakes.", false);
	}
...
public tentaDickLottieButtFuckbuttFuckButtFUCKBUTTFUCKBUTTFUCK():void {
	Render.text("", true);
	Render.text(images.showImage("lottie-fuck-her-with-tentacle-dick"));
	let x:number = -1;
	let y:number = -1;
	let z:number = -1;
	let zz:number = -1;
	...
	while(counter < player.lowerBody.cockSpot.count()) {
		if(player.lowerBody.cockSpot.list[counter].cockType == CockType.TENTACLE) {
			if(x == -1) x = counter;
...
private lottieAndIfrisNTREpilogue():void {
	Render.text("", true);
	Render.text("A dry wind across your face rouses you from your peaceful sleep.  It seems you're outside the walls of Tel'Adre.  All your equipment is sitting to the right of you, and when you gather the pile of your gear you find a note underneath.\n\n", false);

	Render.text("\"<i>Dear " + player.short + ",\n\nSleep well?  I hope so, considering I found you snoring on top of two luscious babes.  Don't worry about getting in trouble at the gym.  Most of the staff and customers were too busy getting themselves off to be angry or offended, though I have to admit I'm a bit jealous.  The little piggy and that tight piece of red ass are both safe in their homes.  ", false);
	if(!kGAMECLASS.urta.urtaLove()) {
		Render.text("Of course, I had to fine you for public indecency - you really ought to watch how you behave, lest someone take you for a demon agent.  Next time, try and keep it in your pants.\n\n-Urta, Captain of the Guard", false);
		if(player.stats.gems > 0) {
			player.stats.gems -= 200;
			if(player.stats.gems < 0) player.stats.gems = 0;
		}
	}
	else Render.text("I had to make a show of fining you for public indecency, but you'll note your money is all still there.  Pay me back sometime, okay, hun?\n\n-Always up for you, Urta", false);
	Render.text("</i>\"\n\n", false);

	Render.text("Satisfied, you get dressed and strut off towards camp.", false);
	if(silly()) Render.text("  You'll make damn sure to buy Urta a bacon shake tomorrow.", false);
	doNext(camp.returnToCampUseOneHour);
}
...
public lottiesThirtyMinutePigGasm():void {
	...
	Render.text("DAMN!  Did this pig bitch really just have a half-hour orgasm?  You're a little jealous.", false);
	player.orgasm();
	doNext(camp.returnToCampUseOneHour);
}
}
