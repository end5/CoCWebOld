...
public telAdreNiamh():void {
	if(flags[FlagEnum.MET_NIAMH] == 0) {
		...
		if(model.time.hours <= 8) Render.text("massive N-cup breasts, spilling over the table");
		else if(model.time.hours <= 9) Render.text("huge M-cup breasts, taking up much of the table");
		else if(model.time.hours <= 10) Render.text("great L-cup breasts, advancing far onto the table");
		else if(model.time.hours <= 11) Render.text("heavy K-cup breasts, lying on the table for support");
		else if(model.time.hours <= 12) Render.text("hefty J-cup breasts, resting comfortably on the table");
		else if(model.time.hours <= 13) Render.text("weighty I-cup breasts, dangling to lie on the table");
		else if(model.time.hours <= 14) Render.text("squeezable H-cup breasts, settling burdensomely on her chest");
		else Render.text("moderately manageable G-cups, trembling with every breath she takes");
		Render.text(".  Smooth, touchable skin covers her chest, face, hands and presumably the rest of her; it seems her animalistic accoutrements stop at kitty ears and tail. She takes a deep swig of her drink, following it up with a cute little burp.");
	}
	//This scene only plays if the PC has either Talked to Niamh or has gotten at least 1 mug of Black Cat Beer from her.
		...
		if(model.time.hours <= 8) Render.text("her titanic N-cup 'kegs' resting against the thankfully sturdy table in front of her.  Her teats are engorged with the beer she's cursed to carry, and the stuff dribbles out whenever she shifts.");
		else if(model.time.hours <= 14) {
			Render.text("her ");
			if(model.time.hours <= 9) Render.text("M-cup");
			else if(model.time.hours <= 10) Render.text("L-cup");
			else if(model.time.hours <= 11) Render.text("K-cup");
			else if(model.time.hours <= 12) Render.text("J-cup");
			else if(model.time.hours <= 13) Render.text("I-cup");
			else Render.text("H-cup");
			Render.text(" brew-burdened breasts have clearly gotten smaller as the day dragged on, although they're still uncomfortably sizeable and swollen.");
		}
		else Render.text("her G-cup beer-filled boobs far smaller than their morning volume, though still impressive in stature.  You can follow the progress she's made throughout the day by the beer stains running along the wooden surface, trailing from all the way off the table and ending where her boobs now lie.");
	}
}
...
public approachNiamh():void {
	...
	Render.text(images.showImage("niamh-approach-in-bar"));
...
public getANiamhBeer():void {
	clearOutput();
	Render.text(images.showImage("niamh-get-beer"));
	Render.text("\"<i>That'll be two gems,</i>\" she replies.  You hand over the payment, which she holds up to the light and scrutinizes thoroughly.  Satisfied, she drops the things into a sack at her waist.  \"<i>Sorry,</i>\" she mutters. \"<i>I do want to get this damned beer out o' me, but this is still my primary livelihood; I canno' afford to be cheated.  Anyway, here's your mug - you can keep that f'r another two gems; I get 'em cheap.  Go ahead and fill it up, but keep in mind what you're tweakin', right?</i>\"");
	//Feeder
	if(player.perks.has("Feeder")) Render.text("  You understand exactly what she's going through; the weight of a huge pair of breasts, the sensitivity as they engorge with fluids, the near-maddening sensation of the contents sloshing around underneath your straining skin...");
	//(Player has G-cups or bigger;
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 15)
	{
		if(player.lactationQ() == 0)
			Render.text("  Not terribly surprising; you know your tits are sensitive, and you don't even make milk in them.");
		else
			Render.text("  You can easily imagine just how much trouble she must have, especially given the bulk of her tits are fluid.");
	}
	else if(player.lactationQ() > 0)
		Render.text("  You know just how distracting it is to have your tits full of fluid waiting to get out.");
	...
	if(flags[FlagEnum.MET_NIAMH] > 0)
		...
		Render.text("the cat-girl ");
	Render.text("sighs in relief, biting her lower lip slightly.");
	//General Ending
...
private drinkNiamhsBeerInTelAdre():void {
	...
	if(player.lactationQ() >= 300 && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5 && rand(2) == 0 && flags[FlagEnum.MET_NIAMH] > 0) {
		...
		if(isHolidays() && date.date >= 31) {
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("pussy to wetten");
			if(player.gender == 3) Render.text(" and ");
			if(player.lowerBody.cockSpot.hasCock()) Render.text("[eachCock] to stiffen");
			Render.text(".  \"<i>Ye know, I got's an idea.  How's about ye stick by me for the next hour.  I'm sure ye must be diein' to drain those big'uns of yours.  We might as well sell your champagne ta the patrons.  Whaddaya say Lassie?</i>\"");
...
private blackCatBeerToGo():void {
	clearOutput();
	Render.text("\"<i>That'll be two more gems, then,</i>\" the girl says.  \"<i>Though, if your hands get any more skillful I might have to pay you to take it,</i>\" she flirts.  You pass over two more gems, and she gratefully bags them and beams you a smile.");
	player.stats.gems -= 2;
	statScreenRefresh();
	//both output
	if(model.time.hours <= 15) Render.text("\n\n\"<i>Thanks for the business, " + player.mfn("laddie","lassie","customer") + "!  Remember, Niamh sells her Black Cat Beer every day from 8 until 4.</i>\"  You nod your head in thanks and step away from the table.");
...
private talkToNiamh():void {
	...
	if(flags[FlagEnum.TALKED_NIAMH] == 0) {
		Render.text("You ask the cat-girl to tell you about her past.");
		...
		if(model.time.hours < 15) Render.text("who usually sits over yonder ");
...
private leaveNiamh():void {
	clearOutput();
	Render.text("You decide that you don't really want to talk to this strange cat-girl and, as politely as possible, excuse yourself.");
...
public blackCatBeerEffects(player:Player,clearScreen:boolean = true,newLine:boolean = false):void {
	...
	if(newLine) Render.text("\n\n");
	Render.text("Uncapping the mug, you swill the stuff down in a single swig, gasping as it burns a fiery trail into your belly.  It's rich and sweet, and damn, but it's strong stuff!");
...
private corruptOrBimboNiamhIntro():void {
	clearOutput();
	Render.text("\"<i>Heya, favored customer,</i>\" the busty kitty greets as you approach.  \"<i>I have a favor to ask!  My drink's empty, and I get bored of me own brew,</i>\" Niamh explains.  \"<i>Would ya mind fetchin' me somethin', or are ye gonna force me to sample me own goods once more?</i>\"");
	...
	if(player.hasItem(consumables.BIMBOLQ))
		Render.text("'bubbly' for her, in the form of the Bimbo Liqueur you got from the imp lord, or ");
	Render.text("maybe even a Succubus Milk?  You'll have to consider your options.  On the other hand, of course, something that corruptive would probably mutate her booze-filled boobs, most likely forcing her out of Tel'Adre altogether... and may fall back on your own head, to boot.  What do you do?");
...
public taintNiamh(bimbo:boolean = false):void {
	clearOutput();
	Render.text("You grab Niamh's glass and one more and head off, making sure to turn a corner before commandeering an empty table.  Plunking the mugs down in front of you, you mutter to yourself as she moves away.");
	//[if sucmilk]
	if(!bimbo) {
...
public niamhPostTelAdreMoveOut():void {
	...
	clearOutput();
	Render.text("Upon one of your routine searches, you're given pause as a half-coherent mess of a song comes tumbling across the grasses.  You move toward the cacophony of sound, fairly confident in what you will find.  Indeed, stumbling and shambling towards you comes a ");
	if(flags[FlagEnum.NIAMH_STATUS] == 1)
		Render.text("demonic-looking");
	else
		Render.text("hourglass-shaped");
	Render.text(" Niamh, and it's obvious she's completely hammered.  ");
	if(flags[FlagEnum.NIAMH_STATUS] == 1)
		Render.text("\"<i>W'hey there " + player.mf("boyo","big girl") + ", wanna...g'forrra goooo-gfh...time?</i>\"");
	else
		Render.text("\"<i>W'heeeey, honey!  I never thought I'd see yaaaagain!</i>\"");
	Render.text(" she slurs, struggling to stay upright between her impaired motor skills and her absolutely overfilled chest.  \"<i>Aaah've been drinkin' me 'shtash',</i>\" she explains, noticing your gaze.  \"<i>Canno' leddit go'tuh waste!</i>\" You nod your agreement, sneaking a glance behind her to observe the twin trails of ");
	if(flags[FlagEnum.NIAMH_STATUS] == 1)
		Render.text("dark");
	else
		Render.text("light");
	Render.text(" fluid she's leaving behind.");
...
public bimboChampagne(player:Player,clearScreen:boolean,intro:boolean):void {
	...
	if(intro) {
		if((player.perks.has("FutaFaculties") && player.perks.has("FutaForm")) || (player.perks.has("BimboBody") && player.perks.has("BimboBrains"))) {
			Render.text("You could've swore the stuff worked when you saw Niamh do it to others, but for some reason, it had, like, no effect on you. How weird!");
		}
		else if(player.findStatusAffect(StatusAffects.BimboChampagne) < 0) Render.text("You uncork the bottle and breathe in the fizzy, spicy aroma of the sparkling liquor.  Breathing deeply, you open your mouth and begin pouring the ever-effervescent fluid inside.  It's sweet and slightly gooey, and the feel of it sliding down your throat is intensely... awesome?  Like, totally!");
		else Render.text("You find yourself falling even further into the dense bimbo mindset.  You do feel, like, super-good and all, though!\n\nMoaning lewdly, you begin to sway your hips from side to side, putting on a show for anyone who might manage to see you.   You just feel so... sexy.  Too sexy to hide it.  Your body aches to show itself and feel the gaze of someone, anyone upon it.  Mmmm, it makes you so wet!  You sink your fingers into your sloppy cunt with a groan of satisfaction.  Somehow, you feel like you could fuck anyone right now!");
	}
	if(player.statusAffects.has("BimboChampagne")) {
		...
		if(!player.lowerBody.vaginaSpot.hasVagina()) {
			...
			player.genderCheck();
			Render.text("\n\nYou can feel ");
			if(player.lowerBody.cockSpot.hasCock()) Render.text("the flesh under your cock[if (hasBalls = true)  and behind your [balls]]");
			else Render.text("the blank expanse of flesh that is your crotch");
			Render.text(" start to tingle and squirm... mmm... that feels nice!  There's a sensation you, like, can't describe, and then your crotch feels all wet... but in a good, sticky sorta way.  Oh, wow!  <b>You've, like, just grown a new virgin pussy!</b>  Awesome!");
...
public removeBimboChampagne():void {
	...
	if(player.statusAffects.get("BimboChampagne").value2 > 0) {
		...
		Render.text("  As the trecherous brew fades, your [chest] loses some of its... bimboliciousness.  Your back feels so much lighter without the extra weight dragging down on it.");
	}
	if(player.statusAffects.get("$1").value3 > 0) {
		Render.text("  At the same time, your [vagina] slowly seals itself up, disappearing as quickly as it came.  Goodbye womanhood.");
		player.removeVagina();
	}
	if(player.statusAffects.get("BimboChampagne").value4 > 0) {
		...
		Render.text("  Of course, the added junk in your trunk fades too, leaving you back to having a [butt].");
	}
...
public bazaarNiamh():void {
	...
	if(flags[FlagEnum.NIAMH_STATUS] == 2) {
		...
		//[Bizarre Bazaar, repeat encounter]
			...
			Render.text("Surrounded by blonde-haired, horny women, herms and traps, many of them sporting stomachs swollen with Niamh's copious champagne lactation, the bimbo catgirl enthusiastically waves you over.  \"<i>[name]!  Like, what can I do you for?</i>\" she giggles.");
		}
	}
	//Corrupt:
	else {
		//(General notification:
		Render.text("Given the pile of snoozing bodies, you can probably hazard a guess as to where Niamh decided to set up shop.");
		//(Approach:
...
private bazaardNiamhDrink():void {
	clearOutput();
	player.stats.gems -= 2;
	Render.text("Approaching the fallen-from-grace (yet completely content) catgirl, you toss her a couple gems and request a glass of the house drink.");
	//Bimbo:
	if(flags[FlagEnum.NIAMH_STATUS] == 2) {
	...
	//Corrupt
		...
		Render.text("Succubus milk Niamh Not implemented yet");
		doNext(camp.returnToCampUseOneHour);
	}
}
...
private drinkFromZeTap():void {
	clearOutput();
	Render.text("Your eyes lock onto her large, ever-dribbling nipples, the froth of spilt champagne caked on the table supporting her mammoth breasts, and you know you just have to have it from the source.  You push your way forward and seat yourself on a plush cushion that, from its many tears and suspicious-smelling stains, has seen better days, then grasp the huge boob in front of you, making the ditzy catgirl yowl in anticipation of pleasure.");
...
private getBimboChampFromNiamh():void {
	clearOutput();
	Render.text("You explain your need for a bottle of her champagne; the ditzy kitty takes several moments to register your words, then nods idly.  \"<i>Works fer me!</i>\" she responds, reaching behind her and rummaging in a large burlap bag.  She produces a flask from the sack's depths, unscrewing the cap and attempting to line it up with a nipple.  Try as she might, she just can't quite reach all the way, and with an airy giggle, she passes it to you instead.");
...
public seanBimboBrewing():void {
	clearOutput();
	Render.text("The uncharacteristically gentlemanly incubus raises his eyebrow as you produce an armful of filled flasks and drop them onto his mostly-empty table.  \"<i>I assume you'd like me to do something special with these,</i>\" he quips, somewhat dryly.");
...
public getBimboozeFromSean():void {
	clearOutput();
	Render.text("Sean turns as you enter, managing a thin smile as he regards you.  \"<i>Excellent timing, I have just finished this!</i>\"");
	Render.text("He scoops a familiar-looking bottle from a tabletop, passing it off to you.  \"<i>Be somewhat careful with this,</i>\" he warns as you take it.  \"<i>The original's deleterious effect on the mind has been duplicated, I assure you.</i>\"\n\n");
	Render.text("You drop the vial into a pouch on your person.\n\n");
...
private sellYourBooze():void {
	...
	if(player.bRows() == 1) Render.text("With a chipper giggle Niamh pulls you closer to the bar.  With both hands she helps you heft your tits up and on to the counter.");
	//If player has four or six breasts.
	else Render.text("Niamh giggles as you try to move your breasts closer to the bar to be milked by the patrons, but it's obvious only the top pair of your tits can easily rest on the counter top.  \"<i>Lassie, it might be unconventional, but givin' the circumstance mayhaps it'd be best if ye laid down on the bar.</i>\"");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  You feel your excitement dripping down the inside of your thighs as your pussy wettens.");
	dynStats("lus", 10 + player.stats.sens/5, "resisted", false);
	//[If player has cocks]
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  You start to feel sympathetic sensations in [eachCock] as the hands and paws of the bar goers milk your nipples.  With their fingers sliding over your sensitive nubs you can easily imagine them sliding up and down your cock");
	...
	//If lust is high
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("You can feel your labia dripping with need.  ");
		if(player.lowerBody.cockSpot.hasCock()) {
			Render.text("pre-cum drizzles down your " + multiCockDescriptLight() + " as the need to fuck a pussy grows stronger.  A few wandering hands reveals some apparent interest amongst the folks in the bar in seeing your genitals as active as your nipples.");
		}
		//[FUCK THE BAR] [LEAVE]
...
private barBeerOrgyTits():void {
	...
	if(player.lowerBody.butt.buttRating >= 20) Render.text("  She slaps your ass to emphasize her meaning, and the action sends jiggling waves through each immense cheek.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 20) Render.text("Your giant boobs are large enough to temporarily hang off the sides of your body as you're carried.");
	Render.text("  Your [butt] hits the table first as the crowd puts you down.  Some patrons begin sucking on your nipples directly.");
	// [IF player has four or more giant tits, but not hugely pregnant]
	if((player.pregnancyIncubation == 0 || player.pregnancyIncubation > 100) && player.bRows() > 1) Render.text("  You are essentially a pile of " + boozeBoobsType() + " filled tits.");
	// [IF player is hugely pregnant with four or more tits]
	else if(player.bRows() > 1) Render.text("  You are essentially a giant pregnant belly surrounded by your [chest] on the table, helpless to the sexual advances of the bar goers and you can't get enough of it.");
	...
	if(model.time.hours < 15 && flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > 0 && player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.pregnancyIncubation == 0) Render.text("  Some folks give yelps and cheers when they see the bulge that repeatedly forms in your abdomen each time Urta thrusts herself inside you.");
	...
	//ELSE IF player has pussy and Urta is not in the bar.
		...
		if(player.vaginalCapacity() < 60) Render.text("  One is sporting a cock that is obviously too massive for your pussy, and eventually backs down so that the other one, despite still being on the large size, can take the honor of fucking your cunt silly.");
		//ELSE IF [player has a gaping pussy or bigger]
		else Render.text("  They're both sporting large canine cocks, but one is particularly massive while the other would likely be a loose fit for a pussy as naturally stretched out as yours.  The largest one wins out and positions himself to give your overly wide fuck hole a much needed stretching.");
	...
	if(player.pregnancyIncubation > 0 && player.pregnancyIncubation < 100) Render.text("  \"<i>Oi!  Careful folks, this babe could blow at any time!</i>\"  Niamh pats your belly with chiding affection.");
	...
	if(player.upperBody.chest.hasFuckableNipples()) Render.text("  Eventually some of the randier drunks start whipping out their cocks, and begin pressing the heads against the openings of your nipples.  Within moments you feel your booze bloated breasts being fucked silly by " + num2Text(player.upperBody.chest.countNipples()) + " hard cocks.  Each thrust causes " + boozeBoobsType() + " to spill out around their shafts.");
	// ELSE[Character doesn't have fuckable nipples]
	else Render.text("  Some of the hornier customers start shoving cocks of all shapes and sizes into the moist wet cleavages formed by yours and Niamh's breasts.");
	...
	if(player.upperBody.chest.hasFuckableNipples()) Render.text("  The cocks filling your nipples start pumping hot sperm into your breasts, mixing with your " + boozeBoobsType() + " and causing every nerve ending in your nipples to tingle.  Some of them ejaculate so much that it feels as if your breasts are swelling even larger, almost to the point of bursting before the cum spills out.");
	//IF [Player has cocks]
	if(player.lowerBody.cockSpot.count() > 0) {
		Render.text("  You feel as if your body is exploding everywhere.  Niamh gags momentarily as her mouth fills with your seed, and you realize your cock");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s have");
		else Render.text(" has");
		Render.text(" begun spurting ");
		if(player.lowerBody.cockSpot.count() == 1) Render.text("its ");
		else Render.text("their ");
		Render.text("jizz.  Niamh swallows it down her throat but not without some of it getting plastered across her face.");
		//IF [character is pregnant]
		if(player.pregnancyIncubation > 0 && player.pregnancyIncubation < 200) Render.text("  Globs of cum splatter across your gravid belly.");
	}
	...
	if(player.statusAffects.get("Edryn").value1 >= 5 && player.gender == 3 && model.time.hours == 14 && flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > 0) {
		...
		// IF [player has a single cock]
		if(player.lowerBody.cockSpot.count() == 1) Render.text("  Edryn moans out loud in ecstasy as your [cock] fills her up.");
		// If [player has multiple cocks]
		else if(player.lowerBody.cockSpot.count() > 1) Render.text("  The heads of your cocks press in between Edryn's labia causing her to cry out, \"<i>Oh gawd!  How many dicks are y-you f-f-fucking meeee with???</i>\"  Edryn's orgasm cuts her voice off and every muscle in her cunt contracts around your " + multiCockDescriptLight() + ".");
		...
		if(player.pregnancyIncubation > 0 && player.pregnancyIncubation < 200) Render.text("  Your pregnant belly presses up against Edryn's ass as both Urta's cock and Edryn's pussy keeps you pinned in place between them.");
		// IF[player is not pregnant]
		else Render.text("  Urta's cock and Edryn's pussy pin you in place between them, and as Urta fucks you deeper her cock head forces your belly to bulge outwards until it presses against Edryn's ass.");
		...
		if(player.bRows() == 2) Render.text("  Your second row of tits get thrust out to either side every time Urta's torso presses your own against Edryn.");
		//IF [player has six giant breasts]
		else if(player.bRows() == 3) Render.text("  Your lowest row of tits slap against all three of your thighs as the love making becomes more brutal.");
		//IF [player has large balls]
		if(player.lowerBody.balls > 0 && player.lowerBody.ballSize > 14) Render.text("  Your hanging testicles bounce around between everyone's legs.");
		//IF [player has giant ass]
		if(player.lowerBody.butt.buttRating >= 22) Render.text("  The fat of your ass cheeks jiggles and quakes even as Urta practically stuffs her hips into your crack.  \"<i>Damn, you've got a bigger ass than Edryn,</i>\" she chides as she spanks you.  \"<i>Hey!</i>\"  Edryn calls out with a false tone of hurt pride.");
	...
	// IF [Urta but not Edryn is present in the bar and sex with her is unlocked and character has pussy]
		...
		if(player.bRows() > 1) Render.text("  Your rows of breasts shift around as your posture changes, making squelching sounds as they slide around.");
		//If [player not pregnant]
		if(player.pregnancyIncubation == 0) Render.text("  Urta's small ocean of sperm streams out from your pussy like a river down your legs as you try to stand.");
	...
	//Generic ending if the first two don't trigger
		...
		// IF [player has pussy]
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  The giant knot of the dog morph finally breaks its way inside causing your pussy to become overstretched.  The pleasurable torture only gets worse as he begins unloading a river of his cum inside you.  It begins squirting out around his cock and soaks both of your thighs in the process.");
		player.cuntChange(60,true,true,false);

		Render.text("  Niamh finally has her fill and dismounts you, but no sooner has her pussy left your face than a determined cock belonging to some sort of cat morph fills your mouth.  Your eyes bug out as it thrusts down your throat.");
		// If [player has pussy]
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  You gag on it at the same time a new cock fills your cunt, ready to renew the thrusting that's been causing your tits to bounce all over the place.");

		//IF [player has single cock small-med cock]
		if(player.lowerBody.cockSpot.count() == 1 && player.cockArea(0) < 75) Render.text("\n\nA cat girl with six C-cup breasts jumps up onto the table and mounts you.  She grabs your cock and proceeds to shove it up between the folds of her tight pussy.");
		...
		if(player.lowerBody.cockSpot.count() == 1) Render.text("  The six titted girl smiles at you, patting her belly while your cum runs down the inside of her thighs.  You only now wonder about how fertile she might be.");
		// IF [player has multiple cocks]
		else if(player.lowerBody.cockSpot.count() > 1) Render.text("  Various girls around the room are patting their bellies and giving you sultry looks while your cum continues to drip from their pussies.  You passively wonder if any of them might indeed be hauling around some larger pregnant bellies in a few days, but you try to push those thoughts from your head.  At the moment they seem happy.");
		// IF [player has pussy]
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			Render.text("  You walk slightly bow legged out of the bar.  Cum is still dripping from your snatch and will likely continue to do so for a while.");
			//IF [player is not pregnant]
			if(player.pregnancyType == 0) Render.text("  You can't help but wonder how virile those dog morphs might have been as their cum and the cum of other customers sloshes around inside your uterus.");
			temp = rand(6);
			switch(temp) {
				case 0:
