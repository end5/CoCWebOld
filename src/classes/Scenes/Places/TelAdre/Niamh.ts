...
public function getANiamhBeer():void {
	...
	//Feeder
	if(player.perks.has("Feeder")) outputText("  You understand exactly what she's going through; the weight of a huge pair of breasts, the sensitivity as they engorge with fluids, the near-maddening sensation of the contents sloshing around underneath your straining skin...");
...
private function drinkNiamhsBeerInTelAdre():void {
	...
	if(player.lactationQ() >= 300 && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5 && rand(2) == 0 && flags[FlagEnum.MET_NIAMH] > 0) {
		...
		if(isHolidays() && date.date >= 31) {
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) outputText("pussy to wetten");
			if(player.gender == 3) outputText(" and ");
			if(player.lowerBody.cockSpot.hasCock()) outputText("[eachCock] to stiffen");
	...
	//16:00 ending
		...
		if(flags[FlagEnum.MET_NIAMH] == 0) outputText("the cat girl's");
...
public function bimboChampagne(player:Player,clearScreen:boolean,intro:boolean):void {
	...
	if(player.statusAffects.has("BimboChampagne")) {
		...
		if(!player.lowerBody.vaginaSpot.hasVagina()) {
			...
			player.genderCheck();
			outputText("\n\nYou can feel ");
			if(player.lowerBody.cockSpot.hasCock()) outputText("the flesh under your cock[if (hasBalls = true)  and behind your [balls]]");
...
private function sellYourBooze():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  You feel your excitement dripping down the inside of your thighs as your pussy wettens.");
	dynStats("lus", 10 + player.stats.sens/5, "resisted", false);
	//[If player has cocks]
	if(player.lowerBody.cockSpot.hasCock()) outputText("  You start to feel sympathetic sensations in [eachCock] as the hands and paws of the bar goers milk your nipples.  With their fingers sliding over your sensitive nubs you can easily imagine them sliding up and down your cock");
	...
	//If lust is high
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("You can feel your labia dripping with need.  ");
		if(player.lowerBody.cockSpot.hasCock()) {
...
private function barBeerOrgyTits():void {
	...
	if(player.lowerBody.butt.buttRating >= 20) outputText("  She slaps your ass to emphasize her meaning, and the action sends jiggling waves through each immense cheek.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 20) outputText("Your giant boobs are large enough to temporarily hang off the sides of your body as you're carried.");
	...
	if(player.upperBody.chest.hasFuckableNipples()) outputText("  Eventually some of the randier drunks start whipping out their cocks, and begin pressing the heads against the openings of your nipples.  Within moments you feel your booze bloated breasts being fucked silly by " + num2Text(player.upperBody.chest.countNipples()) + " hard cocks.  Each thrust causes " + boozeBoobsType() + " to spill out around their shafts.");
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("s have");
		else outputText(" has");
	...
	if(player.statusAffects.get("Edryn").value1 >= 5 && player.gender == 3 && model.time.hours == 14 && flags[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > 0) {
		...
		// IF [player has a single cock]
		if(player.lowerBody.cockSpot.count() == 1) outputText("  Edryn moans out loud in ecstasy as your [cock] fills her up.");
		// If [player has multiple cocks]
		else if(player.lowerBody.cockSpot.count() > 1) outputText("  The heads of your cocks press in between Edryn's labia causing her to cry out, \"<i>Oh gawd!  How many dicks are y-you f-f-fucking meeee with???</i>\"  Edryn's orgasm cuts her voice off and every muscle in her cunt contracts around your " + multiCockDescriptLight() + ".");
		...
		if(player.lowerBody.balls > 0 && player.lowerBody.ballSize > 14) outputText("  Your hanging testicles bounce around between everyone's legs.");
		//IF [player has giant ass]
		if(player.lowerBody.butt.buttRating >= 22) outputText("  The fat of your ass cheeks jiggles and quakes even as Urta practically stuffs her hips into your crack.  \"<i>Damn, you've got a bigger ass than Edryn,</i>\" she chides as she spanks you.  \"<i>Hey!</i>\"  Edryn calls out with a false tone of hurt pride.");
	...
	//Generic ending if the first two don't trigger
		...
		// If [player has pussy]
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  You gag on it at the same time a new cock fills your cunt, ready to renew the thrusting that's been causing your tits to bounce all over the place.");

		//IF [player has single cock small-med cock]
		if(player.lowerBody.cockSpot.count() == 1 && player.cockArea(0) < 75) outputText("\n\nA cat girl with six C-cup breasts jumps up onto the table and mounts you.  She grabs your cock and proceeds to shove it up between the folds of her tight pussy.");
		...
		if(player.lowerBody.cockSpot.count() == 1) outputText("  The six titted girl smiles at you, patting her belly while your cum runs down the inside of her thighs.  You only now wonder about how fertile she might be.");
		// IF [player has multiple cocks]
		else if(player.lowerBody.cockSpot.count() > 1) outputText("  Various girls around the room are patting their bellies and giving you sultry looks while your cum continues to drip from their pussies.  You passively wonder if any of them might indeed be hauling around some larger pregnant bellies in a few days, but you try to push those thoughts from your head.  At the moment they seem happy.");
		// IF [player has pussy]
		if(player.lowerBody.vaginaSpot.hasVagina()) {
