...
public visitZeMagesTower():void {
	...
	if(flags[FlagEnum.TIMES_BEEN_TO_LIBRARY] == 0 || model.time.hours <= 17)  {
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00175] > 0) addButton(2,"Mali",talkToMali);
	}
	if(flags[FlagEnum.TIMES_VISITED_MALI] > 0) addButton(2,"Mali",talkToMali);
...
private firstTowerVisit():void {
	clearOutput();
	Render.text("You make your way to the largest fixture of the city, the impressive tower in the center.  The large spire could easily hold everyone you know ten times over and still have room to spare.  It is far too large for a city with Tel'adre's population – but then, you reflect, so is Tel'adre itself.");
	...
	if (model.time.hours <= 17) { //Don't want to meet Quinn if he's not supposed to be there
		Render.text("  A single man carefully turns through the pages of one book");
...
private towerFollowUpVisits():void {
	...
	if (flags[FlagEnum.TIMES_BEEN_TO_LIBRARY] == -1) { //Return visits before you meet Quinn. Either you meet him or you continue to go to the library at night like some bibliophile vampire
		if(model.time.hours <= 17) {
			Render.text("You return to the mage's tower.  Entering the main room, you're surprised to see a man carefully turning the pages of one of the tomes");
			commonQuinnTroduction();
		}
		else {
			Render.text("As before, there's no one here.  At least there's no lack of reading material.  Looking up and down the staircases reveals two locked doors, so it's unlikely you could do anything but study here.");
			flags[FlagEnum.TIMES_BEEN_TO_LIBRARY] = -2; //This will be incremented to -1 by the visitZeMagesTower function after we return
		}
		return;
	}
	...
	if(model.time.hours <= 17) {
		Render.text("You return to the mage's tower.  Entering the main room, Quinn is carefully inspecting the pages of a book.  The room looks slightly more organized from when you last saw it, but it looks as though Quinn will be working on it for some time.");
	...
	//(follow-up visits, 18:00-20:00)
		...
		Render.text("You return to the mage's tower.  Entering the main room, Quinn is nowhere to be seen.  The room looks slightly cleaner from when you saw it last, but at the rate he's going it seems like it will be some time before it's finished.");
...
private commonQuinnTroduction():void {
	Render.text(", though he does not seem to be reading it.  Stacks of books sit next to him.  As you close the door, he glances up at you.");
...
private studyInTA():void {
	...
	//[Study, 6:00-17:00]
	if(model.time.hours <= 17) {
		Render.text("You ask Quinn if you can use the library to study and learn.");
	...
	//[Study, 18:00-20:00]
		...
		if (flags[FlagEnum.TIMES_BEEN_TO_LIBRARY] == -1) {
			Render.text("Looking around you decide to spend some time reading");
		}
		else {
			Render.text("Without Quinn to hassle you and request your absence from the presences, you have some time to read");
		}
		Render.text(" through some of the literature collected by the Covenant.");
		if(rand(3) == 0) {
			//magic)
...
private youOkayBuddy():void {
	clearOutput();
	Render.text("A bit perturbed by Quinn's countenance and apparent exhaustion you can't help but inquire as to his well-being.");
...
private talkToMali():void {
	...
	//[[Mali], player has spellblade]
		...
		Render.text("You tell Quinn you're here to see Mali.  He seems intrigued by the wrapped blade you're carrying, but doesn't ask any questions.  Unlocking the second floor as usual, he escorts you to Mali's quarters.");
...
//			player.perks.remove("WizardsFocus");
	...
	//[[Mali], player does not have spellblade]
		...
		Render.text("You tell Quinn you're here to see Mali.  He rolls his eyes but doesn't say anything, unlocking the second floor and leading you up once more.");
