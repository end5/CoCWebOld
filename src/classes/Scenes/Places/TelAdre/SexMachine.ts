...
private function useTheSexMachine():void {
	...
	if(player.gender == 1) {
		if(flags[FlagEnum.TIMES_USED_SEX_MACHINE_AS_MALE] == 0) {
			...
			if(player.lowerBody.balls > 0) outputText("your " + ballsDescriptLight() + " pulling up tightly ", false);
	...
	//[If you decide to mess with the machine, female:]
		...
		if(flags[FlagEnum.TIMES_USED_SEX_MACHINE_AS_FEMALE] == 0) {
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("  It slips a fourth cock between your " + chestDesc() + ", using two arms to press them together and fuck your tits as well, splashing a burst of mysterious cum on your face with every thrust.", false);
			outputText("\n\n", false);
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("your breasts covered in cum and ", false);
		...
		//[Female repeat scene]
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText(", and it doesn't take long for the machine to slip a cock between your breasts as well, starting to tit fuck you as it rotates you in your restraints to optimize your position", false);
	...
	//[Hermaphrodite]
		...
		if(flags[FlagEnum.TIMES_USED_SEX_MACHINE_AS_HERM] == 0) {
			...
			if(player.lowerBody.cockSpot.count() > 1) outputText("primary cock", false);
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("  Well, not every part of you, but that is rapidly fixed as a final cock rubs against your " + chestDesc() + ", starting to stimulate your chest as well.", false);
			outputText("\n\n", false);
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("  The cock on your chest pumps cum onto your " + chestDesc() + ", as y", false);
