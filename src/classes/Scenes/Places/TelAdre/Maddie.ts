...
private function followMinotaurIntoBackroom():void {
	...
	if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00241] == 0) {
		...
		if(player.perks.has("MinotaurCumAddict")) outputText("You lick your lips when you realize you're meeting the source of the 'special' éclairs.", false);
...
public function nopeAintGotNoneODemSpeculIngredimathings():void {
	...
	if(player.stats.cor > 50) outputText("starting a fight inside Tel'Adre", false);
...
public function handOverIngredientsItBeBakingTimeYo():void {
	...
	if(player.perks.has("MinotaurCumAddict")) outputText("  You lurch forward involuntarily, craving the rest of his jism, but he pushes you into the wall and grunts, \"<i>No,</i>\" in a tone that brooks no disagreement.  It actually shocks you out of your addicted haze.", false);
	outputText("\n\n", false);
	...
	if(player.perks.has("MinotaurCumAddict")) outputText("  Your mouth salivates at the thought.", false);
...
private function waitForSlutCake():void {
	...
	if(player.stats.cor < 33) outputText("antsy", false);
	else if(player.stats.cor < 66) outputText("nervous about this whole thing", false);
	else outputText("intrigued by this whole thing", false);
	outputText(".  One of the waitresses brings you a glass of milk, and ", false);
	if(player.stats.cor < 50) outputText("it smells normal enough, so you go ahead and sip on it", false);
...
private function talkToMaddie():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("is like, all out of icing mix!  So I'm going to borrow a few cups from you, 'kay?", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.stats.cor < 33) outputText("You muster as much authority as you can in such a compromising position and explain to Maddie that what comes out of there is NOT icing.", false);
		else if(player.stats.cor < 66) outputText("You offhandedly mention that you don't actually make icing.", false);
		...
		if(!player.lowerBody.cockSpot.hasSheath()) outputText("encircles the base", false);
		...
		if(player.lowerBody.balls > 0) outputText(", the cum-heavy spheres bouncing in your twitching sack as they get ready to explode", false);
		...
		if(player.lowerBody.cockSpot.count() == 1) outputText(", your " + cockDescript(x) + " emptying every drop of jizz into the baked cum-tank.", false);
		else {
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			if(player.lowerBody.cockSpot.count() == 2) outputText(" along with your other penis", false);
