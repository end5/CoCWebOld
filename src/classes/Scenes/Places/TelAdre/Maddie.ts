...
internal function procMaddieOneIntro():void {
	...
	//(REPEAT) 
		...
		MainScreen.text("You walk into the bakery and a burly, hair-covered arm grabs your shoulder.  The familiar voice of a minotaur barks, \"<i>You.  You can help.  Come.</i>\"  You turn, but he's already walking towards an 'employees only' door.  Do you follow?", false);
	}
...
public nopeAintGotNoneODemSpeculIngredimathings():void {
	MainScreen.text("", true);
	MainScreen.text("The chef sighs and slams a fist into the counter hard enough to dent the metal and throw the bowls full of dough inches into the air.  A number of empty éclairs bounce and roll everywhere.  The minotaur looks back at you and snorts, \"<i>Best you go.  Don't come without ingredients.</i>\"\n\n", false);
	
	MainScreen.text("Well, no point in ", false);
	if(player.stats.cor > 50) MainScreen.text("starting a fight inside Tel'Adre", false);
	else MainScreen.text("overstaying your welcome", false);
	MainScreen.text(" – you depart.", false);
...
public handOverIngredientsItBeBakingTimeYo():void {
	...
	if(player.perks.has("MinotaurCumAddict")) MainScreen.text("  You lurch forward involuntarily, craving the rest of his jism, but he pushes you into the wall and grunts, \"<i>No,</i>\" in a tone that brooks no disagreement.  It actually shocks you out of your addicted haze.", false);
	MainScreen.text("\n\n", false);
	
	MainScreen.text("Grabbing a whisk, the bull-man starts stirring the sex-filled dough with vigor, mixing the thickening blend hard enough to make his biceps ripple.  A moment later, he lifts the bowl one-handed and pulls out a giant, novelty cupcake mold from the counter. After filling the mold, the chef throws it onto his burly shoulder and grabs a sack of actual icing.  A terse grunt instructs, \"<i>Wait at tables.  You can try some when done.</i>\"  ", false);
	if(player.perks.has("MinotaurCumAddict")) MainScreen.text("  Your mouth salivates at the thought.", false);
	else MainScreen.text("You aren't sure you want to.", false);
	MainScreen.text("\n\n", false);
...
private waitForSlutCake():void {
	spriteSelect(39);
	MainScreen.text("", true);
	MainScreen.text("You walk back into the bakery proper, feeling more than a little ", false);
	if(player.stats.cor < 33) MainScreen.text("antsy", false);
	else if(player.stats.cor < 66) MainScreen.text("nervous about this whole thing", false);
	else MainScreen.text("intrigued by this whole thing", false);
	MainScreen.text(".  One of the waitresses brings you a glass of milk, and ", false);
	if(player.stats.cor < 50) MainScreen.text("it smells normal enough, so you go ahead and sip on it", false);
	else MainScreen.text("you sip on it while you wait", false);
	MainScreen.text(".  After what feels like an eternity, you get sick of waiting and push through the door into the bakery's backrooms to see what the hold-up is.  The minotaur isn't at his usual workstation, and doesn't look to have been there in quite some time.\n\n", false);
	
	MainScreen.text("Where could he have gone?  You backtrack through the ovens, looking down side-paths and searching through the labyrinthine storage rooms.  Just when you're about to give up, you hear an airy, light-headed giggle from the next room.  You peek around the corner and gasp in absolute shock.  The minotaur is pinned to the wall, his wrists stuck in place by what looks like hardened, white icing.   On top of him is the strangest - no, ONLY, cupcake-woman you've ever seen.\n\n", false);
	
	MainScreen.text("She's taller than the imprisoned minotaur, and wider too.  The pastry-girl's skin is slightly porous, colored light chocolate and gleaming in the dim light where it isn't covered by shining, blue-iced 'clothes'.  Her hair is white as whipped cream, and tied back with a cinnamon bun.  Her curvaceous form turns, jiggling ever so slightly as she takes you in with her green, gum-drop eyes and revealing her whipped-cream bra.  The novelty cup-cake mold is balanced atop her head, worn like a comparatively tiny fez.\n\n", false);
	
	MainScreen.text("The minotaur chef is still wearing his poofy hat, but he's pinned completely and irrevocably under this baked behemoth as she bounces and grinds on his convulsing member.  While you watch, his balls shrink smaller and smaller, emptying their pent up, steamy cargo directly into the cupcake's soft, cushiony center.  She grows larger from the sudden intake of fresh jism, giggling as she drains every drop from her creator.  \"<i>Tee-hee!  Mmm, you're like, delicious and stuff, creat- cr... dad!  So sticky and yummy, just like me!</i>\" exclaims the fluffy slut-cake.\n\n", false);
	
	MainScreen.text("Utterly shocked and drained, the chef-o-taur's eyes roll back in his sockets.  He slumps weakly under his creation as she bounces a few last times, futilely trying to squeeze more cum from the slumping minotaur-dick.  The cupcake-girl rises at last, not with yeast, but with a new-found purpose.  The reflective, alien surface of her eyes locks against your groin as she takes one lumbering step after another in your direction.  Her massive, spongy tits wobble dangerously close to you, nearly entrancing you with their beautiful, unnatural curves.\n\n", false);
	
	MainScreen.text("Running seems like a very good idea.  Who knows what she has planned for you?", false);
	//[RUN] [TRY TO TALK]
...
private talkToMaddie():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		MainScreen.text(", running a sponge-soft hand over the love-muscle.  You rock your hips, trying to squirm away.  Maddie laughs, breathily chortling while her well-rounded breasts slide to either side of you and pin you to the wall.\n\n", false);
		
		MainScreen.text("\"<i>So is this like, where the icing spouts out right?</i>\" asks the confectionery cutey, squeezing you softly.  \"<i>Awww, how sad – yours is stuck, just like Daddy's!  I'll have to squeeze and rub it until it'll let out the icing.</i>\"\n\n", false);
		...
		MainScreen.text("  When the jizz-guzzling pastry-girl pulls back at last to free your empty member, it's coated from top to bottom in gooey blue jelly, though it's tinged white in places.  The milked-out member slowly softens", false);
		if(player.lowerBody.cockSpot.count() > 1) {
			if(player.lowerBody.cockSpot.count() == 2) MainScreen.text(" along with your other penis", false);
			else MainScreen.text(" along with your other dicks", false);
		}
		MainScreen.text(".  Satisfied, your body goes limp and sags against the wall while your face leans on the cupcake-girl's departing breast.\n\n", false);
