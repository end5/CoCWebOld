...
public function approachFrosty():void {
	...
	else
	{
		...
		Render.text("You go out to Frosty’s stand; she’s there manning it with a huge smile on her face. “<i>Hello there customer, how may I help you?</i>” You greet her with your own smile and hello and take a look at her menu:\n");
		Render.text("1. Cupcake x 1 = 15gems\n");
		Render.text("2. Cupcake x 5 = 60gems\n");
		Render.text("3. Cupcakes x 10 = 110gems\n");
		Render.text("4. Cupcakes x 20 = 200gems\n");
...
public function frostyMainMenu():void
{
	...
	if(player.stats.gems >= 60) addButton(1,"x5",cupcakeBuy,5);
	if(player.stats.gems >= 110) addButton(2,"x10",cupcakeBuy,10);
	if(player.stats.gems >= 200) addButton(3,"x20",cupcakeBuy,20);
	if(flags[FlagEnum.SIGNED_FROSTYS_CONTRACT] == 0) addButton(8,"Contract",getAFuckingFuckContractFromFucks);
...
public function frostySpecialsMenu(free:boolean = false):void 
{
	...
	//2. Mouth-15. Points
	menu();
	if(!free && flags[FlagEnum.FROSTY_POINTS] < 5) Render.text("\n\nYou don't have enough points for any services.")
	else if(!free && flags[FlagEnum.FROSTY_POINTS] < 15) Render.text("\n\nYou don't have enough points for mouth service.")
	if(free || flags[FlagEnum.FROSTY_POINTS] >= 5) addButton(0,"Hand",frostysHandsAreColdHolyShitWhyDontYouWarmTHoseMitsUp);
	if(free || flags[FlagEnum.FROSTY_POINTS] >= 15) addButton(1,"Mouth",useFrostysMouth);
	if(free || flags[FlagEnum.FROSTY_POINTS] >= 1) addButton(8,"EatHerOut",cunnilingateFrosty);
...
public function frostysHandsAreColdHolyShitWhyDontYouWarmTHoseMitsUp():void
{
	...
	if(player.lowerBody.cockSpot.hasCock() && !player.lowerBody.vaginaSpot.hasVagina())
	{
		...
		Render.text("\n\n“<i>Ooooh, your ");
		if(player.biggestCockLength() < 7) Render.text("little");
		else Render.text("big");
		Render.text(" friend is just waiting for me to milk his cream out. Tee-hee.</i>”");
	...
	{
		Render.text("“<i>Okay, one stroke-and-poke comin’ right up</i>” she snickers at her own name for using her hand to pleasure a herm. She ducks under the table and pulls up a big, fancy, cushioned chair that looks like it belongs to a noble and also shouldn’t be able to fit in the tiny space under her booth.");
...
public function useFrostysMouth():void
{
	...
	{
		Render.text("“<i>One banana split sundae special coming right up.</i>” You roll your eyes at the comment as Frosty blurs away her display then vaults over the counter with ease and is suddenly behind you. You quickly turn around just as she pushes your backside against her stand and falls to her knees. She’s about to pull the lower half of your [armor] off but decides not to. You wonder what she’s going to do as she stretches her jaw and sticks out her tongue for a few minutes. After she’s done with that, she cracks her neck and looks at your pants, like it’s a challenge. Then, in an instant, she’s able to undo your [armor] with just her tongue and teeth and has you standing waist-down naked. Now you have second thoughts of having any of your naughty bits near that mouth.");
...
public function genderlessCatchAllForFrosty():void
{
	...
	{
		Render.text("“<i>Sorry customer, you need a cock or a pussy or BOTH if I’m to do that!</i>” she sings in a cheesy jingle tone that makes you go, “<i>ughhhhhh.</i>”");
	}
	//[Back to Frosty’s Special Menu.]
	frostySpecialsMenu();
}
...
function analWithACupcakeSloot():void
{
	...
	{
		Render.text("“<i>So you wanna be the pitcher or the catcher?</i>” she ask with devilish smile. What the hell is that suppose to mean?");
		//[pitcher?]  [catcher?]
	}
	else
	{
