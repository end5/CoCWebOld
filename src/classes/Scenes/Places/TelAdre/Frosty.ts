...
public approachFrosty():void {
	...
	else
	{
		...
		MainScreen.text("You go out to Frosty’s stand; she’s there manning it with a huge smile on her face. “<i>Hello there customer, how may I help you?</i>” You greet her with your own smile and hello and take a look at her menu:\n");
		MainScreen.text("1. Cupcake x 1 = 15gems\n");
		MainScreen.text("2. Cupcake x 5 = 60gems\n");
		MainScreen.text("3. Cupcakes x 10 = 110gems\n");
		MainScreen.text("4. Cupcakes x 20 = 200gems\n");
...
public frostySpecialsMenu(free:boolean = false):void 
{
	...
	//2. Mouth-15. Points
	menu();
	if(!free && Flags.list[FlagEnum.FROSTY_POINTS] < 5) MainScreen.text("\n\nYou don't have enough points for any services.")
	else if(!free && Flags.list[FlagEnum.FROSTY_POINTS] < 15) MainScreen.text("\n\nYou don't have enough points for mouth service.")
	...
	if(free || Flags.list[FlagEnum.FROSTY_POINTS] >= 15) MainScreen.addButton(1,"Mouth",useFrostysMouth);
	if(free || Flags.list[FlagEnum.FROSTY_POINTS] >= 1) MainScreen.addButton(8,"EatHerOut",cunnilingateFrosty);
...
{
	MainScreen.clearText();
	MainScreen.text("You make your purchase and swiftly devour the results. They're surprisingly delicious!");
	//15
...
{
	MainScreen.clearText();
	MainScreen.text("You follow Frosty to her booth. She goes behind the counter and spins the menu with her fingertip and stops it with the palm of her hand. She goes back to her business while you look over the menu.  The lettering and style of it has changed into something more appealing and seductive.");
...
public noIDontWantAFreebieDiseaseYouSlut():void {
	MainScreen.clearText();
	MainScreen.text("You’re too busy right now, so you deny the free service. “<i>Okeydokey, see you around, [name]!</i>” Frosty does a twirl on one foot and runs back to her stand. Such a nice girl.");
	menu();
...
public noContractForMeSloot():void {
	MainScreen.clearText();
	MainScreen.text("Something about signing a contract doesn’t seem right to you, especially one for cupcakes. You shake your head no; she looks a bit sad about being rejected but quickly perks back up.");
...
{
	MainScreen.clearText();
	MainScreen.text("Frosty zooms around the counter, appearing next to you like a pink blur “<i>OH! You changed your mind!?</i>” she asks excitedly with her hands held up to her face like she’s praying.");
	//[Yes [No]
	//[Yes] 
...
{
	MainScreen.clearText();
	MainScreen.text("Frosty lets out a discouraged “<i>Oh</i>” and slowly walks back to her side of the counter.");
...
{
	MainScreen.clearText();
	MainScreen.text("Frosty spins the menu with a fingertip and stops it with the palm of her hand. She goes back to her business while you look over the menu.  The lettering and style of it has changed into something more appealing and seductive:\n\n1. Hand – 5 points\n2. Mouth - 15 points\nSPECIAL: Eat Me Out - 1 point\n\n<b>Current points: " + frostyPoints() + "</b>");
	frostySpecialsMenu();
}

private frostyPoints(arg: number = 0): number {
	if(arg == 0) return Flags.list[FlagEnum.FROSTY_POINTS];
	//If any change, add it in.
	Flags.list[FlagEnum.FROSTY_POINTS] += arg;
	//Make sure it aint negative
	if(Flags.list[FlagEnum.FROSTY_POINTS] < 0) Flags.list[FlagEnum.FROSTY_POINTS] = 0;
	return Flags.list[FlagEnum.FROSTY_POINTS];
}
...
public frostysHandsAreColdHolyShitWhyDontYouWarmTHoseMitsUp():void
{
	...
	if(player.lowerBody.cockSpot.hasCock() && !player.lowerBody.vaginaSpot.hasVagina())
	{
		...
		MainScreen.text("\n\n“<i>Ooooh, your ");
		if(player.biggestCockLength() < 7) MainScreen.text("little");
		else MainScreen.text("big");
		MainScreen.text(" friend is just waiting for me to milk his cream out. Tee-hee.</i>”");
	...
	{
		MainScreen.text("“<i>Okay, one stroke-and-poke comin’ right up</i>” she snickers at her own name for using her hand to pleasure a herm. She ducks under the table and pulls up a big, fancy, cushioned chair that looks like it belongs to a noble and also shouldn’t be able to fit in the tiny space under her booth.");
...
public useFrostysMouth():void
{
	...
	{
		MainScreen.text("“<i>One banana split sundae special coming right up.</i>” You roll your eyes at the comment as Frosty blurs away her display then vaults over the counter with ease and is suddenly behind you. You quickly turn around just as she pushes your backside against her stand and falls to her knees. She’s about to pull the lower half of your [armor] off but decides not to. You wonder what she’s going to do as she stretches her jaw and sticks out her tongue for a few minutes. After she’s done with that, she cracks her neck and looks at your pants, like it’s a challenge. Then, in an instant, she’s able to undo your [armor] with just her tongue and teeth and has you standing waist-down naked. Now you have second thoughts of having any of your naughty bits near that mouth.");
...
public genderlessCatchAllForFrosty():void
{
	...
	{
		MainScreen.text("“<i>Sorry customer, you need a cock or a pussy or BOTH if I’m to do that!</i>” she sings in a cheesy jingle tone that makes you go, “<i>ughhhhhh.</i>”");
	}
	//[Back to Frosty’s Special Menu.]
	frostySpecialsMenu();
}
...
function analWithACupcakeSloot():void
{
	...
	{
		MainScreen.text("“<i>So you wanna be the pitcher or the catcher?</i>” she ask with devilish smile. What the hell is that suppose to mean?");
		//[pitcher?]  [catcher?]
	}
	else
	{
...
It moves only few inches on your onsides. Your mind starts racing with pleasure as Frosty’s other hand wanders over your [allBreastsDescript] and pinches on your [BreastDescriptor.describeNipple(0)], pulling and twisting the nip and taking nice hard grabs at your [ BreastDescriptor.describeBreastRow(player.upperBody.chest.get(0))] A familiar build-up forms down in your crotch, you start pushing back against Frosty’s “cock” to hurry up the climax and start pinching your other [BreastDescriptor.describeNipple(0)] minicing the same action as the torturous nymph does, abusing the sensitive bud all in the name of sexaul pleasure.   
