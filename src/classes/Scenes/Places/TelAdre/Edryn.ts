...
public function edrynBarTalk():void {
	...
	if(flags[FlagEnum.EDRYN_NEEDS_TO_TALK_ABOUT_KID] == 1) {
		...
		if(flags[FlagEnum.EDRYN_NUMBER_OF_KIDS] == 1) {
			...
			outputText("I can't believe we've had " + num2Text(flags[FlagEnum.EDRYN_NUMBER_OF_KIDS]) + " kids together!  You better come see me later on - ", false);
			if(flags[FlagEnum.EDRYN_NUMBER_OF_KIDS] < 3) outputText("I miss the way you fill me", false);
	...
	if(player.lowerBody == LOWER_BODY.CENTAUR && player.totalCocks() > 0) {
		...
		if(player.lowerBody.cockSpot.list[0].cockType == CockType.HORSE) outputText("pours out of its sheath", false);
		...
		if(player.horseCocks() > 0) {
			...
			if(player.lowerBody.cockSpot.list[0].cockLength > 16) outputText("You barely register the thump of your hardening " + Appearance.cockNoun(CockType.HORSE) + " as it smacks into the underside of the table.\n\n", false);
		...
		//Non horsedick
			...
			if(player.stats.cor > 50 || player.statusAffects.get("Edryn").value1 > 0 || flags[FlagEnum.LOW_STANDARDS_FOR_ALL]) {
				...
				if(player.lowerBody.cockSpot.list[0].cockType == CockType.TENTACLE) outputText("  She smiles coyly when your " + cockDescript(0) + " wriggles, wrapping around her arm.", false);
				outputText("\n\n", false);
				dynStats("lus", 40);
				edrynOffer();
				return;
	...
	if(player.lowerBody == LOWER_BODY.CENTAUR && player.totalCocks() > 0) {
		...
		if(player.horseCocks() > 0) {
			...
			if(player.cockArea(x) >= 24) {
				...
				if(player.lowerBody.balls > 0) outputText("balls tight and your ", false);
...
private function fuckEdrynTaur():void {
	...
	if(player.stats.cor < 75) outputText("You recoil for a second, scolding yourself for the strange thoughts, but you don't slow down the fucking in the slightest.", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 1) outputText("strokes your chest", false);
...
private function fuckEdrynNonTaur():void {
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("feeling your ring of prepuce slipping between her lips.  ", false);
	...
	if(player.statusAffects.get("Edryn").value1 <= 1) outputText("She sighs happily and looks down at you, \"<i>Ohhhh that was nice.  Most of my customers can't get me off like that.  I hope you're ok!  Don't worry about me getting pregnant either, I take some herbs to prevent that sort of thing until I'm ready for it.</i>\"\n\n", false);
	else if(player.statusAffects.get("Edryn").value1 <= 3) outputText("She sighs happily and looks down at you, \"<i>Ooh " + player.short + ", you're the best.  We should do this again soon.  Oh goddess, I can feel my juices starting just thinking about it.</i>\"\n\n", false);
	else if(player.statusAffects.get("Edryn").value1 == 4) outputText("She sighs happily and says, \"<i>Oh baby, you know what?  I'm never charging you again.  Just keep getting me off like that and we can do it anytime.</i>\"\n\n", false);
...
private function threesomeEdrynAndHel():void {
	...
	if(player.lowerBody.balls > 0) outputText(", your sack slapping wetly against the bottom of her cunny", false);
	outputText(".\n\n", false);
...
private function pregdrynOffer(cs:boolean = true):void {
	...
	if (pregnancy.incubation < 250) {
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("All of your " + multiCockDescriptLight() + " fill in seconds, growing rock hard and actually aching with their need.  ", false);
		else if(player.lowerBody.cockSpot.count() == 1) outputText("Your " + cockDescript(0) + " fills in seconds, growing rock hard and actually aching with need.  ", false);
		outputText("You're totally dazed by the massive spike in arousal, and ", false);
		if(player.stats.cor + player.stats.lib < 100 || player.lowerBody.cockSpot.count() < 1) outputText("you struggle not to reach into your " + player.armorName + " to touch yourself.", false);
		else {
			outputText("you can't stop yourself from grabbing ", false);
			if(player.lowerBody.cockSpot.count() == 1) outputText("your ", false);
		...
		if(player.lowerBody.balls > 0) outputText(ballsDescriptLight(), false);
		else if(player.lowerBody.cockSpot.hasSheath()) outputText("sheath", false);
		else if(player.lowerBody.cockSpot.count() > 0) outputText(cockDescript(0), false);
	...
	if(player.cockArea(x) >= 24 && player.cockArea(x) < 300) {
		...
		if(flags[FlagEnum.EDRYN_NUMBER_OF_KIDS] == 0) outputText("Does my scent have an even stronger effect on you now", false);
...
public function fuckPregEdryn():void {
	...
	if(player.lowerBody != LOWER_BODY.CENTAUR) {
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("chosen ", false);
	...
	//TAURZILLA
		...
		if(!player.lowerBody.cockSpot.hasSheath()) outputText("by the base", false);
...
private function jizzFromEatingPregdrynOut():void {
	...
	if(player.lowerBody == LOWER_BODY.CENTAUR) {
		outputText("  Your hooves twitch weekly on the floor, forgetten about as you focus entirely on your hands, mouth, and pulsating " + Appearance.cockNoun(CockType.HUMAN), false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING) outputText("squirting", false);
...
private function edrynFucktroduction():void {
	...
	if(!player.isTaur()) {
		...
		else if(flags[FlagEnum.EDRYN_NUMBER_OF_KIDS] == 0) outputText("Her arm finds its way back inside your " + player.armorName + ", pulling you towards a back room.  A round of chuckles chases you through the bar room until they're silenced by the heavy thud of a door closing behind you.  You're pushed against the wall hard enough to make your teeth click.  The stars gradually clear from your view, revealing a massive gash, drooling with clear feminine slime.  The scent assails you, ", false);
		...
		if(player.lowerBody.cockSpot.list[0].cockType == CockType.HORSE) outputText("making your " + cockDescript(x) + " twitch and ache.  It floods your body with animalistic desires, overpowering any sense of propriety you might have once had.", false);
...
public function eatEdrynPussyLikeABawss():void {
	...
	if(player.lowerBody.cockSpot.count() == 1) outputText("its underside");
	...
	if(flags[FlagEnum.TIMES_EATEN_EDRYN_PUSSY_RUT] == 0) outputText("smile tugging at the corners of her mouth at your bravado");
	...
	if(player.tongueType > TONUGE.HUMAN) outputText("She's not even begun to taste the fruits of your talents, and you let your long, long tongue spool out inside her, pressing hard on her walls and tenderly flicking across each sensitive fold inside her.  ");
