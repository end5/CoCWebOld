	...
	public static const MAX_MASSAGE_BONUS_DURATION: number = 24;			// Duration of the bonus
		...
		*/
		private function sexEroticMassage():void
		{
			...
			if (flags[FlagEnum.PC_FETISH] >= 1) outputText("unable to resist either the tingle of lust that runs through you at being naked in front of Uma, nor the urge to flaunt your feminine physique for her own gratification, ");
			...
			if (player.gender == 3) outputText("and ");
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("juices ");
		...
		public function sexGetFingered():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("past [eachCock], and ");
			...
			if (player.hasVirginVagina())
			{
				...
				if (player.lowerBody.cockSpot.hasCock()) outputText(", your forgotten cock spraying cum in a pearlescent arc to splatter onto the floor");
				outputText(".\n\n");
		...
		private function sexGetABJFromDisMilfyLesboSlut():void
		{
			...
			//VARIANTS NEEDED
			if(flags[FlagEnum.UMA_TIMES_SUCKED_YOU] < 2) outputText("It’s clear that she doesn’t really enjoy the notion that much, considering the way she’s looking at you.  \"<i>All right, let’s get this over with.</i>\"  ");
			else if(flags[FlagEnum.UMA_TIMES_SUCKED_YOU] < 5) outputText("She actually seems to be smiling a little as she looks at you.  \"<i>All right, time to see if I can handle this brutish thing.</i>\"  ");
			...
			if(player.lowerBody.cockSpot.count() == 1) outputText("your");
			...
			if(flags[FlagEnum.UMA_TIMES_SUCKED_YOU] >= 2) outputText(", but I guess you're changing that, aren't you?");
			else outputText(".");
			outputText("</i>\"  Then, she playfully licks along the underside of your cock, putting that lengthy tongue to work. \"<i>");
			if(flags[FlagEnum.UMA_TIMES_SUCKED_YOU] < 2) outputText("You don't taste bad, all things considered.");
			else if(flags[FlagEnum.UMA_TIMES_SUCKED_YOU] < 5) outputText("I think I'm starting to get used to your taste, [name].");
			...
			if(player.lowerBody.cockSpot.list[x].cockLength < 9) outputText(" until she hits your [sheath]");
			...
			if(flags[FlagEnum.UMA_TIMES_SUCKED_YOU] < 2) outputText("Girls taste better.");
			else if(flags[FlagEnum.UMA_TIMES_SUCKED_YOU] < 5) outputText("Not too bad.");
			...
			if(player.lowerBody.cockSpot.count() > 1)
			{
				...
				if(player.lowerBody.cockSpot.count() == 2) outputText("one of ");
				...
				if(flags[FlagEnum.UMA_TIMES_SUCKED_YOU] < 3) outputText("perhaps thinking it will end this sooner");
				else outputText("perhaps encouraged by the way you swell in her mouth");
				outputText(", she wraps ");
				if(player.lowerBody.cockSpot.count() == 2) outputText("one hand around your girthy tool and starts pumping");
			...
			if(player.stats.cor <= 33) outputText("pleasantly explain");
			else if(player.stats.cor <= 66) outputText("explain");
			else outputText("harshly explain");
			outputText(".  ");
			if(flags[FlagEnum.UMA_TIMES_SUCKED_YOU] < 2) outputText("You can feel her nod in agreement through your pole, and soon after, she's plunging rhythmically up and down.");
			...
			if(flags[FlagEnum.UMA_TIMES_SUCKED_YOU] < 5)
			{
				...
				if(player.lowerBody.cockSpot.count() > 1) outputText("s erupt");
			...
			//{More than five blowjobs!}
				...
				else if(player.cumQ() <= 100) 
				{
					...
					if(player.lowerBody.balls > 0) outputText("  It rolls down to your [sack], coating it in warm, wet cum-shine.");
				}
				else {
				...
				if(player.lowerBody.cockSpot.count() > 1) {
					...
					if(player.lowerBody.cockSpot.count() > 2) outputText("s release");
					...
					if(player.lowerBody.cockSpot.count() == 2) outputText("It's ropes");
			...
			if(flags[FlagEnum.UMA_TIMES_SUCKED_YOU] < 2) 
			{
				...
				if(player.stats.cor <= 40) outputText(" apologize, and ");
		...
		private function sexEatHerOut():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText("Blood engorges [eachCock],  ");
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText("moisture gathers on your netherlips, drops of your arousal sliding out of your [pussy] and down towards your [feet].  ");
			...
			if (player.upperBody.head.earType == 2 || player.upperBody.head.earType == 9 || player.upperBody.head.earType == 5 || player.upperBody.head.earType == 7) outputText("between your "); 
			
			if (player.upperBody.head.earType == 2) outputText("canine ");
			if (player.upperBody.head.earType == 9) outputText("vulpine ");
			if (player.upperBody.head.earType == 5) outputText("feline ");
			if (player.upperBody.head.earType == 7) outputText("loporid ");

			if (player.upperBody.head.earType == 2 || player.upperBody.head.earType == 9 || player.upperBody.head.earType == 5 || player.upperBody.head.earType == 7) outputText("ears.  ");
