...
private appearanceOfLoppe():void {
	...
	if(Flags.list[FlagEnum.LOPPE_FURRY] == 0) {
		MainScreen.text("Loppe is a 6'2\" bunny-girl with deep brown eyes set in her pretty face.  Shoulder-length black hair and a pair of snow-white rabbit ears adorn the top of her head, tilted and lying backward to conceal the insides.  Curiously, her face is that of a normal human woman.");
...
public loppeFirstMeeting():void {
	MainScreen.clearText();
	MainScreen.text("Wandering towards the bar, you notice an unusual commotion; there are a lot more people hanging around here than is usual for the time of day.  Feeling curious, you go over to investigate, and ask a bystander what all of this is about.");
...
private noLoppeInterest():void {
	MainScreen.clearText();
	MainScreen.text("Dancers aren't really your thing, and you shake your head.");
...
private mebbeLaterToLoppe():void {
	MainScreen.clearText();
	MainScreen.text("You're not really in the mood for this right now, so you leave the bar.");
...
private yesToMeetingLoppe():void {
	...
	MainScreen.text("There's no harm in staying a while...");
...
private yesToLoppeMeetingTwo():void {
	...
	MainScreen.text(images.showImage("loppe-show-at-bar"));
	MainScreen.text("Suddenly the bar goes silent and the lights dim; a figure clad in a white mantle steps onto the stage; discarding the covering with a swift movement, it reveals the comely visage of a bunny-girl - a literal bunny-girl, in that apart from the obvious bunny ears and the hints of rabbit-like legs, she actually looks otherwise human despite the prevalence of anthropomorphic animals in this city.");
	...
	//[(corr < 40)]
	if(player.stats.cor < 40) MainScreen.text(" as politely as possible. It felt nice to see a display like that here; makes it feel a little less like a wasteland of sex-mad monsters and a little more like home");
	MainScreen.text(".");
...
private notReallyInterestedInLoppe():void {
	MainScreen.clearText();
	MainScreen.text("You decline, and the girl gives you a look of disappointment.");
...
private sureBakeryWithLoppe():void {
	MainScreen.clearText();
	MainScreen.text("You consider the time of day, and the girl offering, and decide that it can't hurt.  Loppe smiles and takes your hand, leading you towards a nearby bakery.");
	...
	if(player.stats.gems >= 30) {
		...
		//[(corr > 40)
		if(player.stats.cor > 40) MainScreen.text("  A small voice inside wonders if she'd eat so extravagantly were she footing her own bill.");
		//remove 30 gems
...
private NoWayLoppe():void {
	MainScreen.clearText();
	MainScreen.text("Loppe lets out a breath she had obviously been holding in a bitter sigh, the ghost of the words, \"<i>I knew it</i>\" echoing faintly as she does so.  \"<i>Well, I understand.  I just had to give it a try all the same.  It was nice talking to you, [name].  Might see you again, someday.  Sorry for wasting your time...</i>\"");
...
private noLoppesHouse():void {
	MainScreen.clearText();
	MainScreen.text("Loppe gives a disappointed sigh, but smiles at you all the same.  \"<i>I understand... if you ever feel like talking some more, you can find me in the gym.  I need to keep myself in tip-top shape for my dancing shows, after all.</i>\"");
...
private yesLoppesHouse():void {
	MainScreen.clearText();
	//Loppe can now be found in the Gym
	MainScreen.text("The dancer smiles mischievously at you.  \"<i>Wonderful.  I'm going to show you just how great my body looks without this dress.  You wouldn't believe how much time I spend in the gym, working out.</i>\"  Loppe grabs her last cookie in one hand and your arm in the other, leading you away - presumably to her house.  Enroute, she giggles constantly, provoking a question from you.");
...
public loppeGenericMeetings():void {
	MainScreen.clearText();
	MainScreen.text("You decide to approach the bunny-girl.  Loppe smiles and wipes the sweat off her brow with the towel.  \"<i>Hey there, [name], nice seeing you around here.  So... do you want to do something?  Talk, maybe?  Or go to my place for a 'workout'?</i>\" she asks with a smirk.");
	//Appearance
	//Talk
	//Sex
...
private talkWithLoppe():void {
	MainScreen.clearText();
	MainScreen.text("That 'workout' is going to have to be postponed; you have some questions.");
	...
	if(Flags.list[FlagEnum.LOPPE_TIMES_SEXED] > 0) MainScreen.addButton(1,"Children",askLoppeAboutChildren);
	//Gossip (after sexin' once)
	if(Flags.list[FlagEnum.LOPPE_TIMES_SEXED] > 0) MainScreen.addButton(2,"Gossip",gossipWithLoppe);
	//Working (after [Loppe]
	if(Flags.list[FlagEnum.TIMES_ASKED_LOPPE_ABOUT_LOPPE] > 0) MainScreen.addButton(3,"Working",talkWithLoppeAboutWorking);
	//Her Mom (available after choosing \"<i>Loppe</i>\" at least once)
	if(Flags.list[FlagEnum.TIMES_ASKED_LOPPE_ABOUT_LOPPE] > 0) MainScreen.addButton(4,"Her Mom",chatWithLoppeAboutHerMom);
	//Her Village (after [Loppe]
	if(Flags.list[FlagEnum.TIMES_ASKED_LOPPE_ABOUT_LOPPE] > 0) MainScreen.addButton(5,"Her Village",chatWithLoppeAboutLoppesVillage);
	//The curse (unfinished; available after choosing \"<i>Loppe</i>\" at least once)
	//Fondle (after "Loppe"; see appropriate section)
	if(Flags.list[FlagEnum.TIMES_ASKED_LOPPE_ABOUT_LOPPE] > 0 && Flags.list[FlagEnum.LOPPE_TIMES_SEXED] > 0) MainScreen.addButton(6,"Tease Her",fondleAndTease);
	MainScreen.addButton(9,"Back",loppeGenericMeetings);
}

//Loppe (edited) (C)
private talkWithLoppeAboutLoppe():void {
	MainScreen.clearText();
	MainScreen.text("You tell the laquine that you're curious about her, that you'd like to know more about her: who she is, where she comes from, et cetera.");
	...
	//(if first dialogue)
	if(Flags.list[FlagEnum.TIMES_ASKED_LOPPE_ABOUT_LOPPE] == 0) {
	...
	//set LoppeChat = 1
	Flags.list[FlagEnum.TIMES_ASKED_LOPPE_ABOUT_LOPPE] = 1;
	doNext(camp.returnToCampUseOneHour);
}
...
private askLoppeAboutChildren():void {
	MainScreen.clearText();
	MainScreen.text("You decide to ask Loppe for her thoughts on children.  Has she considered having any?");
	...
	MainScreen.addButton(2,"Someday",loppeIWantKidsSomedayJeezeQuitHasslingMe);
	MainScreen.addButton(3,"Soon",loppeIWantKidsSoonOkayCanWeFuck);
	MainScreen.addButton(4,"No Opinion",noOpinionOnLoppeArt);
}

//[No Opinion] (spacebar default)
private noOpinionOnLoppeArt():void {
	MainScreen.clearText();
	MainScreen.text("You decline to answer, waving the question off.  Loppe raises an eyebrow at that.  \"<i>Haven't thought about it either, huh?</i>\"");
...
private loppeIDontReallyWantKidsYouStupidTwat():void {
	MainScreen.clearText();
	MainScreen.text("You confess you aren't really a 'kids' person; you don't think you could ever envision yourself as a parent, especially in this crazy world.");
...
private loppeIWantKidsSoonOkayCanWeFuck():void {
	MainScreen.clearText();
	MainScreen.text("With a faint smile, you declare that having children is definitely in your future.  You want to be a parent; all you need is to find somebody who feels similar to you.");
...
private gossipWithLoppe():void {
	MainScreen.clearText();
	MainScreen.text("You ask Loppe if she has any comments on the other residents of Tel'Adre.  The laquine smiles at you.  \"<i>It's not polite to pry into other people's lives, y'know?</i>\"");
	...
	if(Flags.list[FlagEnum.NUMBER_OF_TIMES_MET_SCYLLA] > 0) MainScreen.addButton(1,"Scylla",gossipWithLoppeAboutScylla);
	//Jasun
	if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00179] > 0) MainScreen.addButton(2,"Jasun",gossipWithLoppeAboutJasun);
	//Heckel
	if(Flags.list[FlagEnum.MET_HECKEL] > 0) MainScreen.addButton(3,"Heckel",gossipWithLoppeAboutHeckel);
	//Edryn
	MainScreen.addButton(4,"Edryn",gossipWithLoppeAboutEdryn);
	//Lottie
	if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00281] > 0) MainScreen.addButton(5,"Lottie",gossipWithLoppeAboutLottie);
	//Cotton
	if(Flags.list[FlagEnum.COTTON_MET_FUCKED] > 0) MainScreen.addButton(6,"Cotton",gossipWithLoppeAboutCotton);
	//Back (spacebar default)
	MainScreen.addButton(9,"Back",talkWithLoppe);
}
...
private gossipWithLoppeAboutUrta():void {
	...
	if(Flags.list[FlagEnum.TIMES_FUCKED_URTA] <= 0 || Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] == -1) {
		MainScreen.text("\"<i>Ah, so you've met Watch Captain Urta?  She's famous around here, you know; people say she's a legendary bare-knuckle brawler and one of the toughest guards on the force.  The only problem is that she's not really that easy to approach... I guess she prefers to keep to herself.  Although she acts very friendly with that pretty centauress, Edryn.</i>\"");
		...
		MainScreen.text("Loppe smirks at you.  \"<i>I heard she's been getting along nicely with a certain outsider; you wouldn't happen to know anything about that, would you, [name]?</i>\"");
		//[It's Me] [No]
		menu();
	...
	//(if LoppeUrtaKnowledge > 0)
		...
		MainScreen.text("\"<i>Urta again, huh?</i>\"  Loppe stares you, but her curious expression turns into one of smug triumph, like a cat that's caught a mouse.  \"<i>So... she has a big horse-cock.  I'm beginning to see a pattern here, sugar.   You like women with huge equine parts?  Is that why you hang out with me?  With Urta?</i>\"");
		menu();
		...
		MainScreen.addButton(2,"Deny",denyToLoppeThatYouLoveZeHorsecock);
	}
}
...
private noLoppeWhosFuckingUrta():void {
	MainScreen.clearText();
	MainScreen.text("You shake your head, claiming that you have no idea what she's talking about.  Loppe looks at you, then shrugs, clearly not caring if you don't have any gossip on the subject to share.");
	//end scene
	doNext(camp.returnToCampUseOneHour);
}
//[=It's Me=]
...
private playAlongWivLoppesesUrtaGossip():void {
	MainScreen.clearText();
	MainScreen.text("And... what would she do if, hypothetically, you <i>did</i> have a fetish for dickgirls ready to give a stallion an inferiority complex?");
...
private admitToLoppeThatYouLoveZeHorsecock():void {
	MainScreen.clearText();
	MainScreen.text("You look her in the eyes and tell her she's hit the nail on the head; you do have a fetish for chicks with dicks, and having a horsecock makes a dickgirl even sexier to you.");
...
private dontLoppesHouse4Fucks():void {
	MainScreen.clearText();
	MainScreen.text("Loppe raspberries you.  \"<i>You tell me you love herms with big horse cocks and now you won't have sex with me?  You're one big tease, [name].</i>\"");
...
private denyToLoppeThatYouLoveZeHorsecock():void {
	MainScreen.clearText();
	MainScreen.text("Not in the mood to play along with Loppe's little game, you simply state that your reasons are your own and will stay that way for now.");
...
private gossipWithLoppeAboutScylla():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>The nun who likes sucking on dicks?</i>\" Loppe asks you.  \"<i>Yes, I've run into her at the Wet Bitch.  She offered to blow me too.  I was tempted, y'know?  She has those wonderful looking lips... nice big breasts too... but ultimately I wound up refusing.  It's just... not right.</i>\"");
...
private gossipWithLoppeAboutHeckel():void {
	MainScreen.clearText();
	MainScreen.text("You ask if Loppe's ever had a run-in with a herm hyena who's usually running in the track at the gym.");
...
private gossipWithLoppeAboutEdryn():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>That pretty centauress watchwoman that hangs in the Wet Bitch when she's off duty?  Yes, it's no secret that she specializes in particular sorts of wetwork for those with... compatible endowments,</i>\" Loppe says, giving you a lecherous wink.");
...
private gossipWithLoppeAboutLottie():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>That pig-girl that started hanging around the gym in the evenings?  I heard about her... I believe she wants to get fit,</i>\" Loppe comments conversationally.");
	//(if PC's training Lottie)
	if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00299] == 1) {
	...
	else if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00281] > 0) {
...
private talkWithLoppeAboutWorking():void {
	MainScreen.clearText();
	MainScreen.text("Her curse naturally impacts her social life, but what about work?  How do she and her mother put food on the table?");
	...
	//set LoppeChat = 2
	Flags.list[FlagEnum.TIMES_ASKED_LOPPE_ABOUT_LOPPE] = 2;
	doNext(camp.returnToCampUseOneHour);
}
...
private chatWithLoppeAboutHerMom():void {
	MainScreen.clearText();
	//req LoppeChat > 0
	MainScreen.text("Loppe rubs her chin, deep in thought.  \"<i>Where do I start?  I guess by saying that my mom is a super mom.  She raised me all on her own, and she's always had time for me, despite having to keep up with her job.  I love her very much.</i>\"");
	...
	//(If PC is male:)
	if(player.gender == 1) MainScreen.text("  Well, from what Loppe's told you, Uma wouldn't be attracted to you anyway.");
	...
	if (Flags.list[FlagEnum.LOPPE_PC_MET_UMA] == 0)
	{
		...
		MainScreen.addButton(1, "Mebbe Later", camp.returnToCampUseOneHour);
	}
	else
	{
...
private chatWithLoppeAboutLoppesVillage():void {
	MainScreen.clearText();
	MainScreen.text("You ask if Loppe would be willing to tell you about her village.");
...
private loppeSexChoice(bakery:boolean = false):void {
	...
	if(Flags.list[FlagEnum.LOPPE_TIMES_SEXED] == 0) {
		...
		if(!bakery) {
			MainScreen.text("That 'workout' seems like a nice option right now.");
			...
			//(else)
				...
				//[(lib>40)]
				if(player.stats.lib >= 40) MainScreen.text(".. it almost sounds exciting.");
			}
		...
		MainScreen.text("It's not very difficult to reach Loppe's place; it happens to be located only a few blocks away from the gym, behind a small shop with the sign \"<i>Kemono's Oriental Clinic</i>\".");
		...
		//[(corr < 40)]
		if(player.stats.cor < 40) MainScreen.text("  You tell her they're quite nice... though privately you're curious what the interior looks like.");
		else MainScreen.text("  They don't seem to work very well - after all, <i>you're</i> here.");
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			if(player.smallestCockArea() <= loppeCapacity()) MainScreen.text("Pulling her onto your dick would result in some cowgirl fun, though you'd have to deal with her cock pointed right at you when she came.  ");
			else MainScreen.text("As good as it would feel to shove your [cock smallest] into her cunt, the nervous looks she's giving it tell you what her response would probably be.  ");
		}
		//[(any cock)
		if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("You could just whip it out and ask what she thinks of it, compared to hers - it might lead to some cross words and crossed swords, though.  ");
		if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("You could take her monster length vaginally, but if her libido is what she claims, you'll probably wind up quite stretched.  ");
		//[(boobs >= boobjob req)
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) MainScreen.text("Her hardening horsecock looks like it would fit between your [chest], an act likely to net you a messy demonstration of her enthusiasm.  ");
		MainScreen.text("There's always the option to receive her anally, though with her vaunted libido, you'd probably end up so flooded with her cum that it would wash from your mouth.  Or you could be a prick-tease and leave.");
		//Display sex options
		//[Cowgirl][Frot][TakeVaginal][Boobjob][TakeAnal][Bail]
	}
	//Repeat Intro (edited)
	else {
		MainScreen.text("Loppe's other 'workout' sounds pretty good right now.");
		...
		MainScreen.text("There's always the option to receive her anally, though with her vaunted libido, you'd probably end up so flooded with her cum that it would wash from your mouth.  Or you could be a prick-tease and leave.");
		//[(loppesexed)
		MainScreen.text("  Just jerking that cock and squeezing it so hard her balls swell up when she comes, though, that might be fun.  It'd be interesting to see how big they can get.");
	...
	if(player.lowerBody.cockSpot.hasCock() && player.stats.lust >= 33) {
		if(player.cockThatFits(loppeCapacity()) >= 0)
			MainScreen.addButton(0,"Cow-girl",loppeRidesCocks);
	}
	if(player.lowerBody.cockSpot.hasCock() && player.stats.lust >= 33)
		MainScreen.addButton(1,"Get BJ",loppeWorshipsDicks);
	if(player.lowerBody.vaginaSpot.hasVagina() && player.stats.lust >= 33)
		MainScreen.addButton(2,"TakeVaginal",getFuckedInYerTwatYaCunt);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 4) MainScreen.addButton(3,"Boob-job",boobjobLoppe);
	if(Flags.list[FlagEnum.LOPPE_TIMES_SEXED] > 0) MainScreen.addButton(4,"SqueezeJob",loppeSqueezedickWhateverThatIs);
	if(player.lowerBody.isTaur() && player.stats.lust >= 33) MainScreen.addButton(5,"TakeAnal",getAssFuckedByLoppeAsACentaur);
	else if(player.stats.lust >= 33) MainScreen.addButton(5,"TakeAnal",getButtFuckedNonHoarseByLoppe);
	MainScreen.addButton(9,"Leave",beATeaseAndLeaveLoppeAfterSexInvite);
}
...
private loppeRidesCocks():void {
	MainScreen.clearText();
	MainScreen.text(images.showImage("loppe-rides-your-cocks"));
	MainScreen.text("Looking over the hermaphroditic, horse-cocked bunny-girl, you contemplate your options.  You settle yourself ");
	//[(not centaur)]
	if(!player.lowerBody.isTaur()) MainScreen.text("on her bed, making yourself comfortable, and start suggestively stroking yourself");
	//(centaur)
	else MainScreen.text("cumbersomely on the floor, rolling over and spreading your hindlegs to expose yourself");
	MainScreen.text(".  Quietly, you ask Loppe how she would feel about indulging her feminine half.");
	...
	if(y >= 0 && player.lowerBody.cockSpot.count() == 2) MainScreen.text("  \"<i>In fact... I could just eat both of those up.</i>\"");
	//(3+ fit cocks)
	else if(y >= 0 && player.lowerBody.cockSpot.count() == 3) MainScreen.text("  \"<i>But I'm afraid I'll only be able to handle two of them.</i>\"");
	if(player.lowerBody.cockSpot.count() > 1 && y >= 0) MainScreen.text("\n\nBetter than the usual reaction... at least she's willing to give more than one some love.");

	MainScreen.text("\n\nLoppe pushes you on your back and gently takes hold of ");
	if(y >= 0) MainScreen.text("two of ");
	MainScreen.text("your cock");
	if(y >= 0) MainScreen.text("s");
	MainScreen.text(", rubbing gently.  \"<i>Just sit back and leave everything in my capable hands, sugar.</i>\"");
	...
	if(y >= 0) MainScreen.text("s");
	MainScreen.text(" nice and slick.  You gasp in pleasure and surprise as you feel Loppe's erect cock sidle up to yours, helping lube you with its leaking pre. Raising up your hips, you clumsily try to ");
	if(y < 0) MainScreen.text("slide your shaft against her");
	else MainScreen.text("pinion her shaft between your");
	MainScreen.text(" own, shivering from the sensation of your sensitive " + player.skin() + " against her proud horseflesh, already drooling even though you can plainly feel that it's only half-erect.");
	if(player.lowerBody.balls > 0) MainScreen.text("  Your [balls] gently brush and rub against her own swollen cum-factories, and you can't wait to empty your overfilled sac into her waiting womb.");
	...
	if(y >= 0) MainScreen.text("; its neighbor is aimed for her tight rosebud");
	MainScreen.text(".  \"<i>Itadakimasu!</i>\" Loppe says, licking her lips as she finally slides you home.");
	...
	MainScreen.text(", yet squeezing you as tightly as a velvety vise.  In fact, she's gripping you so hard that you can't seem to push yourself any further into her depths; she's squeezing you much too tightly to move in or out, an outright amazing feat, if Loppe gets half as much action as she claims to.");
	...
	if(y >= 0) MainScreen.text("s");
	MainScreen.text(" with happy squeals of pleasure and happiness, softly accompanied by the wet sounds of your coupling.  You gasp and cry, savoring every motion as she squeezes and wriggles, her inner walls rippling so deliciously around your cock");
	if(y >= 0) MainScreen.text("s");
	MainScreen.text(", stroking them in the most delicious way.  You thrust your hips, trying to slip deeper inside her, but still she's holding you at bay.");
	...
	if(y >= 0) MainScreen.text(" and watertight ass");
	MainScreen.text(".  The sensation borders on the thin line between pleasure and pain, and you can't help but moan into Loppe's kiss; you wrap your ");
	if(player.lowerBody.isDrider()) MainScreen.text("spindly legs");
	else if(player.lowerBody.isTaur()) MainScreen.text("forelegs");
	else MainScreen.text("arms");
	MainScreen.text(" around the bunny-girl's ");
	if(Flags.list[FlagEnum.LOPPE_FURRY] == 0) MainScreen.text("silky-smooth");
	else MainScreen.text("softly furred");
	MainScreen.text(" back and hold her tight against you, her long horse-cock sandwiched between your bellies, smearing both of you with drooling precum, rivers of the stuff flowing out steadily with every thrust and grind the two of you make.");
	...
	if(!player.lowerBody.isTaur()) {
		MainScreen.text("<b>You realize that if you keep holding onto Loppe, you're going to end up with a face covered in herm-cum - if you act quickly, though, you can avoid the impromptu facial.  You could even turn her hose of a cock back on her.</b>");
...
private loppeRidesYouNoFaceJizz():void {
	...
	MainScreen.text("You let go of the horse-dicked rabbit and she gratefully swivels herself fully upright, cock jutting out over your belly - instinctively, your ");
	if(player.lowerBody.isTaur()) MainScreen.text("foreleg tucks under her cock, angling it away from your body.");
...
private loppeRidesYouSpunksInYourEye():void {
	...
	MainScreen.text("You keep your ");
	if(player.lowerBody.isDrider()) MainScreen.text("legs");
	else MainScreen.text("arms");
	...
	if(y >= 0) MainScreen.text(", while her anal ring constricts your " + CockDescriptor.describeCock(player, y) + " tightly, intent on holding you in place");
	MainScreen.text(".");
...
private loppeRidesPCCockFinal():void {
	...
	MainScreen.text(images.showImage("loppe-rides-your-cock"));
	MainScreen.text("\n\nYour " + CockDescriptor.describeCock(player, x) + " gushes fluids into her hungry womb");
	if(y >= 0 || player.lowerBody.vaginaSpot.hasVagina()) {
		MainScreen.text(", while ");
		if(y>= 0) MainScreen.text("your second " + CockDescriptor.describeCock(player, y) + " just as easily pumps her perverted ass full of baby juice");
		if(y>= 0 && player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" and ");
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("your [vagina] ");
			if(player.lowerBody.vaginaSpot.get(0).vaginalWetness < 3) MainScreen.text("drizzles");
			else if(player.lowerBody.vaginaSpot.get(0).vaginalWetness < 5) MainScreen.text("spatters");
			else MainScreen.text("gushes");
			MainScreen.text(" femcum onto the bed below you, forgotten during the act but not unsympathetic to her ministrations");
		}
	}
	MainScreen.text(".  You cry out as you cum long and hard, arching your back off of the bed in your pleasure before finishing and collapsing, gasping for breath as, above you, the laquine comes to her own climactic end.");
	...
	if(player.lowerBody.isDrider() || player.lowerBody.isTaur()) MainScreen.text("forelegs");
	else MainScreen.text("arms");
	MainScreen.text(" around her, pulling her into a post-coital cuddle.  Her wet cock quietly slaps against your belly as she embraces you back, clearly savoring the closeness the same way you are.  However, moments later, you feel Loppe's hips start to twitch, slowly building into more lustful thrustings, seconds before Loppe pushes herself back into a deliberately upright position, her pace building steadily.  With a lusty smile and a mischievous twinkle in her eyes, Loppe looks you straight in the eye as she proclaims, \"<i>It's time for round two, [name]!</i>\"");
	...
	MainScreen.text(CockDescriptor.describeMultiCockShort(player) + " and slides down your body, pressing her soft, cum-slickened breasts against you.  You moan as the smooth curves of her chest press against [eachCock]; the cum-slick ");
	if(Flags.list[FlagEnum.LOPPE_FURRY] == 0) MainScreen.text("skin");
	else MainScreen.text("fur");
	MainScreen.text(" give you the impression of being wrapped in the moist folds of a woman's sex.  As Loppe slides further down you feel her teasingly licking at the tip");
	if(y >= 0) MainScreen.text("s");
	MainScreen.text(" of your " + CockDescriptor.describeMultiCockShort(player) + ".");

	MainScreen.text("\n\nThe skillful dance of her warm, wet tongue on the sensitive skin of your post-orgasmic member");
	if(y >= 0) MainScreen.text("s");
	MainScreen.text(" pull");
	if(y < 0) MainScreen.text("s");
	MainScreen.text(" an involuntary shudder from you; the erotic stimulation sends blood pulsing back into your veins, and your shaft");
	if(y >= 0) MainScreen.text("s rise");
	else MainScreen.text(" rises");
	MainScreen.text(" to full mast again, so swiftly ");
	if(y < 0) MainScreen.text("it almost smacks");
	else MainScreen.text("they almost smack");
	MainScreen.text(" the laquine on her nose, leaving you ready to be mounted by her once again.");
	...
	if(player.minLust() <= 30 && player.stats.lib <= 50) MainScreen.text("; if I have to stop to tend to you after every coupling, I'll never get these empty");
	MainScreen.text(".</i>\"  She points with a smirk to her swollen balls.  Er... didn't she just blow her load? Her balls look at least as full as, if not fuller than, when you started, and they nearly slosh with eagerness to be rid of their cargo!");
	...
	if(y >= 0) MainScreen.text("s");
	MainScreen.text(" once more.  You moan as once again you follow the lusty dickgirl into the throes of pleasure...");
	...
	//[(if high cum amount)]
	if(player.cumQ() >= 500) MainScreen.text(" despite your usually messy orgasms");
	MainScreen.text(".");
	...
	//[(Exhibitionist)
	if(Flags.list[FlagEnum.PC_FETISH] > 0) MainScreen.text("; it gives you something of a thrill, though nowhere near as much as taking them off for her in the first place did");
	MainScreen.text(".  Finished, you depart.");
	player.orgasm();
	player.stats.tou += .5;
player.stats.lib += .5;
player.stats.sens += -4;
	Flags.list[FlagEnum.LOPPE_TIMES_SEXED]++;
	//3 hours pass.
...
private loppeWorshipsDicks():void {
	MainScreen.clearText();
	MainScreen.text(images.showImage("loppe-worships-your-cock"));
	MainScreen.text("As your eyes sweep over the naked half-breed's form, they focus on her equine masculinity, already standing proud and eager at the prospect of sex.  Distinctly aware of your own male appendage");
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
	MainScreen.text(", you conversationally ask Loppe what she thinks of your [cock biggest], an idea starting to form in your mind.");

	//(if cock area <= 17.5)
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() <= 17.5) {
	...
	else if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() <= 35) {
		...
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > loppeCapacity()) MainScreen.text("even if it does look too big to fit.");
		else MainScreen.text("or maybe I should get a taste first?");
		MainScreen.text("</i>\"  She licks her lips.");
	}
	...
	if(player.lowerBody.balls > 0) MainScreen.text(" and balls to balls");
	MainScreen.text(", the delicious friction pushing you over the edge.");
...
private getFuckedInYerTwatYaCunt():void {
	MainScreen.clearText();
	MainScreen.text(images.showImage("loppe-sticks-it-in-your-vagoo"));
	MainScreen.text("As you contemplate the delicious possibilities that the herm presents, your eyes are drawn to her admirable piece of horse-meat.  With a half-grin, you settle yourself on her bed, ");
	//[(mans)
	if(!player.lowerBody.isTaur()) MainScreen.text("spreading your labia in an invitation that needs no words.");
	else MainScreen.text("exposing your rump and spreading your hind legs in your best effort to make her feel welcome.");

	MainScreen.text("\n\nLoppe smirks at you and winks.  \"<i>Don't worry; I'll treat you like a ");
	//[(femininity <= 40)
	if(player.femininity > 50) MainScreen.text("princess.  A sexy, smoking hot princess");
	else MainScreen.text("prince.  A dashing, dandy prince");
	MainScreen.text(" that I intend to fill up with my laquine love.</i>\"  She wastes no time and pounces on the bed, ready to bury her face between your [legs], licking her lips while taking deep breaths to inhale as much of your pheromones as she can.  You blink at the speed with which she moves, but smile, repositioning yourself to give her better access");
	//[(herm non-horse)
	if(player.gender == 3 && !player.lowerBody.isTaur()) {
		MainScreen.text(", though she needs to move aside " + CockDescriptor.describeMultiCockSimpleOne(player) + " ");
		if(player.lowerBody.balls > 0) MainScreen.text("and [balls] ");
		MainScreen.text("to really get at your cunt");
	}
	MainScreen.text(".");
	...
	//[(not horse and herm)
	if(!player.lowerBody.isTaur() && player.gender == 3) {
		MainScreen.text(", under your ");
		if(player.lowerBody.balls > 0) MainScreen.text("[sack]");
		else MainScreen.text(CockDescriptor.describeMultiCockShort(player));
		MainScreen.text(",");
	}
	MainScreen.text(" to lick and kiss your sensitive labia.  You arch your back and thrust your crotch at her, letting the laquine have your precious pussy to taste and suckle and ravage... she's pretty good with that tongue of hers.  Loppe lifts her head and wipes her face with her arm.  \"<i>I'm even better with this,</i>\" she says, mirroring your thoughts and holding her painfully erect equine-shaft.  You watch as it throbs, every vein bulging visibly, balls engorged as they churn up a load of cum to deposit in your waiting womb.  She ducks to take another lick, tasting you like one tastes a fine wine.");
	...
	if(player.lowerBody.vaginaSpot.get(0).virgin) {
		...
		//[(low corr)]
		if(player.stats.cor < 50) MainScreen.text("  You look coy, for her benefit, and cast your eyes down shyly.");
		else MainScreen.text("  You do your best to warn her away from the topic with your eyes, eager to get on with the fuck.");
		//(not for horsez)
		...
		MainScreen.text("\n\nYou reach out ");
		if(player.stats.cor < 50) MainScreen.text("and tap her playfully on the nose");
		else MainScreen.text("in your irritation and squeeze her face imploringly");
		MainScreen.text(".  Would you be here, with your clothes off in Loppe's bed, if you weren't okay with it?");
	...
	if(player.vaginalCapacity() >= 35) MainScreen.text(" bottoms out, completely hilted inside you");
	//[else:
	else MainScreen.text(" bumps against your cervix");
	MainScreen.text(". You moan in delight as she sinks into you");
	if(player.lowerBody.vaginaSpot.get(0).virgin) MainScreen.text(", repressing a shiver of pain as she tears through your hymen, taking your virginity");
	else if(player.vaginalCapacity() < 35) MainScreen.text(", forcing your entrance wide to accommodate herself");
	MainScreen.text(".");
	...
	//[(not horse)
	if(!player.lowerBody.isTaur()) MainScreen.text("  You wrap your [legs] around her hips as best you can, thrusting in your efforts to meet her strong rhythm, feeling your cunt rippling around her wonderfully long, thick horsecock.");
	...
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text(", precum begins to drool from your " + CockDescriptor.describeMultiCockShort(player));
	MainScreen.text(".");
	...
	if(!player.lowerBody.isTaur()) {
		...
		if(player.gender >= 3) MainScreen.text(" and smearing both your bellies with herm-pre");
		MainScreen.text(".");
	}
	//Centaur:
	else {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		MainScreen.text(", neglected cock fountaining cum ");
		if(player.cumQ() < 500) MainScreen.text("between you and painting you both");
		else MainScreen.text("gushing onto the bedsheets and smearing your belly");
		MainScreen.text(" with this aspect of your dual-sexed orgasm");
	}
	MainScreen.text(".");
	...
	if(!player.lowerBody.isTaur() && player.lowerBody.cockSpot.hasCock()) MainScreen.text(", giving you a quick peck on the cheek and licking a small gob of cum that seems to have stuck to your chin");
	MainScreen.text(".");
	...
	//[(low corr)
	if(player.stats.cor < 50) MainScreen.text("a pleading look... and if there's maybe a trace of horror dancing in the depths of your eyes, well, that's understandable, isn't it?");
	else MainScreen.text("an incendiary glare, letting her know just what you think of her posturing.");
	MainScreen.text("  Loppe bursts out laughing.  \"<i>Just kidding, sugar.  If we go another round I think my dick might fall off.</i>\"");
	...
	if(silly()) MainScreen.text("  \"<i>Da! Sandvich is kredit to team!</i>\"");
	...
	//[Libido >=50:
	if(player.stats.lib >= 50 || player.minLust() >= 30) MainScreen.text("  Not that you didn't just titillate yourself thinking of it...");
	MainScreen.text("  Your lover laughs, \"<i>Okay, you got me there... but, anyway, I have some chores to run for my mom, so I guess I'll see you later?</i>\"");
...
private getButtFuckedNonHoarseByLoppe():void {
	MainScreen.clearText();
	MainScreen.text(images.showImage("loppe-sticks-it-in-your-butt"));
	MainScreen.text("You swallow as your eyes are drawn to the equine sausage jutting out between the bunny-girl's legs, and know you just have to have it.  Slowly, you spread yourself upon Loppe's bed, scooting up toward the head and pillows and kneeling, [butt] in the air, inviting the laquine to do with you as she will.");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" pussy oozing femlube");
	MainScreen.text("... you beg her to go faster, to do it harder!");

	MainScreen.text("\n\nLoppe happily complies, thrusting herself deeper and stronger.  ");
	//[(cock)
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("One of her hands reaches around to grasp your pre-slickned [cock biggest], stroking you in time with her thrusts.  ");
	if(player.gender == 2) MainScreen.text("One of her hands reaches around to tease your dripping [vagina] and erect [clit], dipping a finger in every time she bottoms out.  ");
	MainScreen.text("Wet slapping noises fill the room, and you gasp in pleasure each time her cum-churning balls slap against ");
	if(player.lowerBody.balls > 0) MainScreen.text("yours");
	else MainScreen.text("your [butt]");
	MainScreen.text(".  Loppe lays down on top of you, panting hotly on your neck, moaning in pleasure each time your sphincter contracts.");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		MainScreen.text("  A gush of sexual fluids from your crotch ");
		if(player.lowerBody.vaginaSpot.get(0).vaginalWetness < 3) MainScreen.text("spatters");
		else if(player.lowerBody.vaginaSpot.get(0).vaginalWetness < 4) MainScreen.text("splashes");
		else if(player.lowerBody.vaginaSpot.get(0).vaginalWetness < 5) MainScreen.text("drenches");
		else MainScreen.text("completely soaks");
		MainScreen.text(" the bed-linen under you.");
	}
	...
	if(Flags.list[FlagEnum.SATYR_KIDS] > 0) MainScreen.text("You laugh softly; hopefully she'll never have to meet the creature that dispelled that same doubt in you.  ");
	MainScreen.text("\"<i>I think this is time for sleeping, not joking.</i>\"  With a trace of wounded dignity, she lays herself down and curls into your back, one arm wrapped around your gurgling belly.");
	...
	//(First Time)
	if(Flags.list[FlagEnum.LOPPE_TIMES_SEXED] == 0) {
		MainScreen.text("Loppe laughs nervously.  \"<i>Sorry about that.  It's just that I usually can't bag a real cutie like you, and I think I might have gone a bit overboard... is there anyway I can make it up to you?</i>\"");
...
private getAssFuckedByLoppeAsACentaur():void {
	MainScreen.clearText();
	MainScreen.text(images.showImage("loppe-sticks-it-in-your-butt-you-centaur"));
	MainScreen.text("You swallow as your eyes are drawn to the equine sausage jutting out between the bunny-girl's legs, and know you just have to have it.  But is she confident in her ability, even with your shape?");
	...
	//[ass wet)
	if(player.lowerBody.butt.analWetness > 0) MainScreen.text("  She doesn't even have to bother with lube; your ass is wet enough to accommodate her on its own.");
	...
	if(player.lowerBody.balls > 0) MainScreen.text("rs");
	else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("r pussy");
	MainScreen.text(".");

	MainScreen.text("\n\nYou moan as she gets into her task");
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text(", feeling your erect [cock] press against your underbelly, mashed between your centaur half and the soft carpet of Loppe's room.  You hope she doesn't mind a few stains...  ");
	//(else vag)
	else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("; your cunt is sopping, making a small pool of juices one the floor.  ");
	else MainScreen.text(".  ");
	MainScreen.text("She seems more worked up than usual, perhaps because you have a horse-like lower body?  \"<i>Are your instincts acting up?</i>\" you tease her.  \"<i>Or do you just love ass that much?</i>\"");
	...
	if(player.gender <= 1) MainScreen.text("  Even if you are the wrong sex entirely for that.");
	...
	//[PC has cock:
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("  The bumps against your prostate send waves of pleasure running through [eachCock] and spreading along your body, until they reach your throat and come out as a ragged groan.  ");
	MainScreen.text("If she keeps this up for too much longer, you will pop any moment now...");
	...
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("  [EachCock] spasms and unloads beneath you, smearing you and the carpet below and pooling thickly with fluid forced out by the prostate stimulation.");
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Your vagina contracts, trying to grip at a phantom member as you spill your fluids on the carpet beneath you.");
	...
	if(player.skinType != SkinType.SCALES) MainScreen.text(" have goosebumps all over your body");
	else MainScreen.text("r scales are practically standing on end");
	MainScreen.text(".");

	MainScreen.text("\n\n\"<i>Ohhh... you are incredible, [name].  ");
	//[PC has dick that fits)
	if(player.cockThatFits(loppeCapacity()) >= 0) MainScreen.text("Though I hope you won't mind riding me, sometime,</i>\" she laughs.  \"<i>");
	MainScreen.text("How about another round, hmm?  Big strong 'taur like you should certainly be able to hold a little more loving inside of you, right?</i>\"  She croons happily, running her hands up and down your ass as if trying to entice you.");

	//(Low Libido)
	if(player.stats.lib < 33) {
	...
	MainScreen.text("  Anyway, your clothes are back in my room; sorry to fuck and run, but I've got some errands to run before mom gets home - you'll have to see yourself out, okay?</i>\"");
...
private loppeSqueezedickWhateverThatIs():void {
	MainScreen.clearText();
	MainScreen.text(images.showImage("loppe-orgazm-denial"));
	//(if LoppeDenial == 0)
	if(Flags.list[FlagEnum.LOPPE_DENIAL_COUNTER] == 0) {
		MainScreen.text("After seeing Loppe's prodigious output, you wonder how she would handle not being able to orgasm.");
	...
	else if(Flags.list[FlagEnum.LOPPE_DENIAL_COUNTER] == 1) {
	...
	if(player.stats.cor < 50) {
		MainScreen.text("  You loosen your grip; this was supposed to be pleasurable, not painful, so it's definitely time to stop.");
		//Goto [Let Go]
		menu();
	...
	//(High Corruption, no new pg)
		...
		MainScreen.text("  You suppose you could let her go... on the other hand, it might be fun to see just how big she'll get if you keep holding her in.");
		menu();
		MainScreen.addButton(0,"Let Go",letsLoppeGoCum);
		MainScreen.addButton(1,"Hold",superLoppeOrgasmDenialGo);
	}
...
private letsLoppeGoCum():void {
	MainScreen.clearText();
	MainScreen.text(images.showImage("loppe-comes-after-orgasm-denial"));
	MainScreen.text("As your fingers slacken on the laquine's straining shaft, they're practically pushed away by the violent force of long-delayed ejaculation, which rockets out of her horse-cock with immense speed and volume as you back away.  For an instant you almost think like she's going to punch a hole through the wall.  She screams, both in pleasure and relief as she fountains cum like a perverted geyser.  Spooge flies through the air, spilling all over the room; you manage to avoid the brunt of it, but some droplets inevitably hit you.");
	...
	Flags.list[FlagEnum.LOPPE_DENIAL_COUNTER] = 3;
	dynStats("lus", 30+player.stats.lib/10, "resisted", false);
	Flags.list[FlagEnum.LOPPE_TIMES_SEXED]++;
	doNext(camp.returnToCampUseOneHour);
}
//[=Hold=]
private superLoppeOrgasmDenialGo():void {
	MainScreen.clearText();
	MainScreen.text(images.showImage("loppe-comes-after-moar-orgasm-denial"));
	MainScreen.text("For fear of not being able to hold her back anymore, you return both hands to tightly gripping her shaft.  Tears escape Loppe's eyes as she's denied yet again.  \"<i>Stop...</i>\" she says meekly.");
	...
	MainScreen.text("\n\n\"<i>Bye, sugar!");
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("  Maybe next time I'll be the one to give you a bit of a holdout!");
	MainScreen.text("</i>\"");
	//leave, set LoppeDenial flag = 7
	Flags.list[FlagEnum.LOPPE_DENIAL_COUNTER] = 5;
	//(very high corr)
	if(player.stats.cor >= 90) {
		...
		Flags.list[FlagEnum.LOPPE_DENIAL_COUNTER] = 10;
...
private boobjobLoppe():void {
	MainScreen.clearText();
	MainScreen.text(images.showImage("loppe-give-her-a-titty-fuck"));
	MainScreen.text("You contemplate your options, unthinkingly rubbing your breasts.  As you do so, you notice your lover is observing your hands very keenly.  With a grin, you grope your tits and ask if Loppe likes your display.");
	...
	if((player.stats.str < 50 && !player.lowerBody.isTaur()) || (player.stats.str < 30 && player.lowerBody.isTaur())) {
		MainScreen.text("  Struggle as you may, you're powerless to stop the drooling laquine as she sends you crashing down on her bed.  The whole piece of furniture shakes ominously with the force of your impact, and for a moment you think it will break apart.");
		...
		if(player.upperBody.head.face.tongueType > TongueType.HUMAN) MainScreen.text("looping your inhuman tongue around it and stroking for extra stimulus even while ");
		MainScreen.text("grabbing your breasts and squeezing the horsemeat between them.  The sooner you help her get over this frenzy, the sooner she'll get off of you... hopefully.  \"<i>Ah, sugar!  I knew you liked it rough!</i>\" your suddenly forceful lover shouts, ominously.");
		...
		if(silly()) MainScreen.text(".  You can't help but think of Family Matters");
		MainScreen.text(".  You simply nod and groan, hands cradling your distended gut.  Loppe snuggles up against you, rubbing your bloated midriff.  \"<i>Sorry about that... hmm, how about we just cuddle and rest?</i>\"");
		...
		MainScreen.text("  You hold the laquine's arms and force her onto the floor; she lands with an audible \"<i>oomph</i>\" and you grin as you hold the eager herm down.");
		...
		//end scene
		MainScreen.text("\n\n");
		if(player.stats.cor >= 60) MainScreen.text("You don't really feel like it, but you figure you had better humor the greedy herm if you want to keep fucking her whenever you like.  ");
		MainScreen.text("Oh, well... you reach down and heft the surprised hybrid off of the floor and onto her spattered bed, then slide yourself into it as best you can, wrapping your arms around her.  She hugs you back, then yawning widely.  \"<i>Naptime...</i>\"");
	...
	if((player.stats.str < 50 && !player.lowerBody.isTaur()) || (player.stats.str < 30 && player.lowerBody.isTaur())) MainScreen.text("- while your stomach is still heavily swollen and gurgles in protest, it's not bad enough to impair you - ");
	MainScreen.text("before going over and dressing yourself.  Loppe waves goodbye to you as you head out once again.");
...
private beATeaseAndLeaveLoppeAfterSexInvite():void {
	MainScreen.clearText();
	MainScreen.text(images.showImage("loppe-teaze-and-leave"));
	MainScreen.text("Despite the hybrid beauty standing stark naked before you, you decide that you aren't in the mood after all.");
...
private fondleAndTease(output:boolean = true):void {
	if(output) {
		MainScreen.clearText();
		MainScreen.text("You lean in over the table and tell Loppe you'd like to talk about her.  She looks vaguely confused.  \"<i>Well, I already told you about everything I can think of.  Was there something you wanted to explore in more detail?</i>\"");
...
private teaseLoppeNSuck():void {
	MainScreen.clearText();
	MainScreen.text("You lean over the table and give the laquine a kiss.  Confused, but not displeased, Loppe eagerly returns it, then watches as you break the lip lock and start to slide yourself under the table.  \"<i>Umm... sugar?  What are you doing?</i>\"");
	...
	if(player.upperBody.head.face.tongueType > TongueType.HUMAN) MainScreen.text(", inhumanly long tongue looping out to coil wetly around the length");
	MainScreen.text(".  You suckle gently, letting the salty tang of hot spunk wash over your tastebuds, and then start to gulp and swallow your way down her prick's sizable length.");
...
private teaseLoppeKissRun():void {
	...
	dynStats("lus", 5+player.stats.lib/20, "cor", .5);
	doNext(camp.returnToCampUseOneHour);
}
}
}
