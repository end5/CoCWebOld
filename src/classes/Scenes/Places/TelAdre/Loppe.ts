...
private function yesToLoppeMeetingTwo():void {
	...
	//[(corr < 40)]
	if(player.stats.cor < 40) outputText(" as politely as possible. It felt nice to see a display like that here; makes it feel a little less like a wasteland of sex-mad monsters and a little more like home");
	outputText(".");
...
private function sureBakeryWithLoppe():void {
	...
	if(player.stats.gems >= 30) {
		...
		//[(corr > 40)
		if(player.stats.cor > 40) outputText("  A small voice inside wonders if she'd eat so extravagantly were she footing her own bill.");
		//remove 30 gems
	...
	if(flags[FlagEnum.LOPPE_TIMES_SEXED] == 0) {
		...
		if(!bakery) {
			...
			//(else)
				...
				//[(lib>40)]
				if(player.stats.lib >= 40) outputText(".. it almost sounds exciting.");
			}
		...
		//[(corr < 40)]
		if(player.stats.cor < 40) outputText("  You tell her they're quite nice... though privately you're curious what the interior looks like.");
		...
		//[(any cock)
		if(player.lowerBody.cockSpot.hasCock()) outputText("You could just whip it out and ask what she thinks of it, compared to hers - it might lead to some cross words and crossed swords, though.  ");
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("You could take her monster length vaginally, but if her libido is what she claims, you'll probably wind up quite stretched.  ");
		//[(boobs >= boobjob req)
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) outputText("Her hardening horsecock looks like it would fit between your [chest], an act likely to net you a messy demonstration of her enthusiasm.  ");
...
private function loppeRidesCocks():void {
	...
	if(y >= 0 && player.lowerBody.cockSpot.count() == 2) outputText("  \"<i>In fact... I could just eat both of those up.</i>\"");
	//(3+ fit cocks)
	else if(y >= 0 && player.lowerBody.cockSpot.count() == 3) outputText("  \"<i>But I'm afraid I'll only be able to handle two of them.</i>\"");
	if(player.lowerBody.cockSpot.count() > 1 && y >= 0) outputText("\n\nBetter than the usual reaction... at least she's willing to give more than one some love.");
	...
	if(player.lowerBody.balls > 0) outputText("  Your [balls] gently brush and rub against her own swollen cum-factories, and you can't wait to empty your overfilled sac into her waiting womb.");
	...
	if(flags[FlagEnum.LOPPE_FURRY] == 0) outputText("silky-smooth");
...
private function loppeRidesPCCockFinal():void {
	...
	if(y >= 0 || player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(y>= 0 && player.lowerBody.vaginaSpot.hasVagina()) outputText(" and ");
	...
	if(flags[FlagEnum.LOPPE_FURRY] == 0) outputText("skin");
	else outputText("fur");
	...
	if(player.minLust() <= 30 && player.stats.lib <= 50) outputText("; if I have to stop to tend to you after every coupling, I'll never get these empty");
	...
	//[(Exhibitionist)
	if(flags[FlagEnum.PC_FETISH] > 0) outputText("; it gives you something of a thrill, though nowhere near as much as taking them off for her in the first place did");
...
private function loppeWorshipsDicks():void {
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() <= 17.5) {
		...
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > loppeCapacity()) outputText("even if it does look too big to fit.");
	...
	if(flags[FlagEnum.LOPPE_FURRY] > 0) outputText("fluffy ");
	...
	//[(lib > 40)
	if(player.stats.lib >= 40) outputText(" - nor would you want to; she looks just right covered in your fluids");
	outputText(".");
	...
	if(player.lowerBody.balls > 0) outputText(" and balls to balls");
...
private function getFuckedInYerTwatYaCunt():void {
	...
	if(player.vaginas[0].virgin) {
		...
		//[(low corr)]
		if(player.stats.cor < 50) outputText("  You look coy, for her benefit, and cast your eyes down shyly.");
		...
		outputText("\n\nYou reach out ");
		if(player.stats.cor < 50) outputText("and tap her playfully on the nose");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", precum begins to drool from your " + multiCockDescriptLight());
	outputText(".");
	...
	if(!player.isTaur() && player.lowerBody.cockSpot.hasCock()) outputText(", giving you a quick peck on the cheek and licking a small gob of cum that seems to have stuck to your chin");
	outputText(".");
	...
	//[(low corr)
	if(player.stats.cor < 50) outputText("a pleading look... and if there's maybe a trace of horror dancing in the depths of your eyes, well, that's understandable, isn't it?");
	...
	//[Libido >=50:
	if(player.stats.lib >= 50 || player.minLust() >= 30) outputText("  Not that you didn't just titillate yourself thinking of it...");
...
private function getButtFuckedNonHoarseByLoppe():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" pussy oozing femlube");
	outputText("... you beg her to go faster, to do it harder!");

	outputText("\n\nLoppe happily complies, thrusting herself deeper and stronger.  ");
	//[(cock)
	if(player.lowerBody.cockSpot.hasCock()) outputText("One of her hands reaches around to grasp your pre-slickned [cock biggest], stroking you in time with her thrusts.  ");
	...
	if(player.lowerBody.balls > 0) outputText("yours");
	...
	if(flags[FlagEnum.SATYR_KIDS] > 0) outputText("You laugh softly; hopefully she'll never have to meet the creature that dispelled that same doubt in you.  ");
...
private function getAssFuckedByLoppeAsACentaur():void {
	...
	if(player.lowerBody.balls > 0) outputText("rs");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("r pussy");
	outputText(".");

	outputText("\n\nYou moan as she gets into her task");
	if(player.lowerBody.cockSpot.hasCock()) outputText(", feeling your erect [cock] press against your underbelly, mashed between your centaur half and the soft carpet of Loppe's room.  You hope she doesn't mind a few stains...  ");
	//(else vag)
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("; your cunt is sopping, making a small pool of juices one the floor.  ");
	else outputText(".  ");
	...
	//[PC has cock:
	if(player.lowerBody.cockSpot.hasCock()) outputText("  The bumps against your prostate send waves of pleasure running through [eachCock] and spreading along your body, until they reach your throat and come out as a ragged groan.  ");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  [EachCock] spasms and unloads beneath you, smearing you and the carpet below and pooling thickly with fluid forced out by the prostate stimulation.");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your vagina contracts, trying to grip at a phantom member as you spill your fluids on the carpet beneath you.");
	...
	if(player.skinType != SKIN.SCALES) outputText(" have goosebumps all over your body");
...
private function superLoppeOrgasmDenialGo():void {
	...
	outputText("\n\n\"<i>Bye, sugar!");
	if(player.lowerBody.cockSpot.hasCock()) outputText("  Maybe next time I'll be the one to give you a bit of a holdout!");
	outputText("</i>\"");
...
private function boobjobLoppe():void {
	...
	if(player.tongueType >= TONUGE.DEMONIC) outputText("retract your tongue into your mouth and ");
	...
	if((player.str < 50 && !player.isTaur()) || (player.str < 30 && player.isTaur())) {
		...
		if(player.tongueType > TONUGE.HUMAN) outputText("looping your inhuman tongue around it and stroking for extra stimulus even while ");
		...
		//end scene
		outputText("\n\n");
		if(player.stats.cor >= 60) outputText("You don't really feel like it, but you figure you had better humor the greedy herm if you want to keep fucking her whenever you like.  ");
...
private function teaseLoppeNSuck():void {
	...
	if(player.tongueType > TONUGE.HUMAN) outputText(", inhumanly long tongue looping out to coil wetly around the length");
