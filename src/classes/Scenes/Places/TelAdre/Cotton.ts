...
public function cottonGreeting():void {
	...
	if(flags[FlagEnum.LIFETIME_GYM_MEMBER] == 0) outputText("The centauress sees you starting for the horse-girl and says, \"<i>Go ahead and talk, but if you want to work out with her I'll have to charge you.</i>\"\n\n", false);
	//Greeting (first time):
...
private function acceptYoga():void {
	...
	//First time
		...
		//(If PC is male:
		else if(player.lowerBody.cockSpot.hasCock()) outputText("  She cradles your " + cockDescript(0) + " and smiles at you, \"<i>Well, we might find a use for that later.</i>\"", false);
		//(If PC is female:
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  She pats your groin and says, \"<i>Y'know, I know some good exercises that really work the vaginal muscles... perhaps we'll talk about that later.</i>\"", false);
		outputText("\n\n", false);
...
private function cottonChat():void {
	...
	//(Scylla chat)
		...
		if(flags[FlagEnum.NUMBER_OF_TIMES_MET_SCYLLA] < 5  && flags[FlagEnum.NUMBER_OF_TIMES_MET_SCYLLA] > 0) outputText("While you're doing your stretches, you find yourself chatting about the folks of Tel'Adre. \"<i>Scylla?</i>\" Cotton says, \"<i>She's the oddly dressed woman at the Wet Bitch, right? Can't say I know much about her. She's so secretive.</i>\"\n\n", false);
		//(if Scylla has formed support group)
		else if(flags[FlagEnum.NUMBER_OF_TIMES_MET_SCYLLA] >= 5) outputText("While you're doing your stretches, you find yourself chatting about the folks of Tel'Adre. \"<i>Scylla?</i>\" Cotton says, \"<i>I hear she formed an addiction support group. Good on her, there's a lot of people in and around town who need help. I'm glad she's stepping up.</i>\"\n\n", false);
	}
	//(Vala chat)
	else if(choice == 4) {
		//(Only if Vala has been freed)
		if(flags[FlagEnum.FREED_VALA] != 0) outputText("While you're doing your stretches, you find yourself chatting about the folks of Tel'Adre. \"<i>Vala?</i>\" Cotton says, \"<i>That's the new waitress at the Wet Bitch, right? She's cute. A shame what she's gone through. Sometimes I wish we could wipe out every last imp.</i>\"\n\n", false);
	}
	//(Jojo chat)
	else if(choice == 5) {
	...
	*/
	...
	if(player.isTaur()) {
		...
		if(player.cockArea(x) >= 100) {
			if(player.lowerBody.cockSpot.count() == 1) outputText("You feel your " + cockDescript(x) + " stirring beneath your large body, while Cotton's remains curiously limp.  While her cock dangles, yours strains for attention.", false);
			//(If PC has multiple cocks)
			else if(player.lowerBody.cockSpot.count() > 1) outputText("You feel your " + multiCockDescriptLight() + " stirring beneath your large body, while Cotton's remains curiously limp.  While her cock dangles, your group of cocks strains for attention.", false);
		}
		else {
			if(player.lowerBody.cockSpot.count() == 1) outputText("You feel your " + cockDescript(x) + " stirring beneath your large body, while you watch Cotton's do the same. Her cock rubs against your stomach while yours strains for attention.", false);
			//(If PC has multiple cocks)
			else if(player.lowerBody.cockSpot.count() > 1) outputText("You feel your " + multiCockDescriptLight() + " stirring beneath your large body, while you watch Cotton's do the same. Her cock rubs against your stomach while your group of cocks strains for attention.", false);
		}
		//(If PC has a pussy, add the following)
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Meanwhile, your " + vaginaDescript(0) + " behind you moistens both from the steam and from arousal, and your " + clitDescript() + " aches, craving attention.", false);
		outputText("\n\n", false);
		...
		if(player.lowerBody.cockSpot.count() == 2) outputText("  With a slight grin, you begin poking your extra dick at Cotton's other hole. You hear an indignant grunt, followed by a sigh, and feel her hands on your member, guiding it all the way into her hole.", false);
		//(If the player has more cocks, add)
		else if(player.lowerBody.cockSpot.count() > 2) outputText("  The rest of your cocks jiggle and bob, rubbing up against her thighs and butt wildly.", false);
		...
		//(If PC has a pussy)
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your " + cockDescript(x) + " erupts inside Cotton's warm cunt which clenches you tightly, while your own femme sex twinges and clenches on air, drooling juices down your legs.", false);
		//(else)
	...
	//(Repeat fuck her, for nagas)
		...
		if(player.cockArea(x) >= 100) {
			//(If PC has one cock)
			if(player.lowerBody.cockSpot.count() == 1) outputText("  You feel your " + cockDescript(x) + " stirring beneath you, slowly coming to attention, though Cotton's remains curiously limp.  Your " + player.cockHead(x) + " rubs against her belly, sending ripples of pleasure up your spine.", false);
			//(If PC has multiple cocks)
			else if(player.lowerBody.cockSpot.count() > 1) outputText("  You feel your " + multiCockDescriptLight() + " stirring beneath you, slowly coming to attention, though Cotton's remains curiously limp.  The group of cocks rubs together as you make out, sending ripples of pleasure up your spine.", false);
		}
		else {
			//(If PC has one cock)
			if(player.lowerBody.cockSpot.count() == 1) outputText("You feel your " + cockDescript(x) + " stirring beneath you, slowly coming to attention alongside Cotton's equine member.  The two rub together as you make out, sending ripples of pleasure up your spine.", false);
			//(If PC has multiple cocks)
			else if(player.lowerBody.cockSpot.count() > 1) outputText("You feel your " + multiCockDescriptLight() + " stirring beneath you, slowly coming to attention alongside Cotton's equine member. The group of cocks rubs together as you make out, sending ripples of pleasure up your spine.", false);
		}
		//(If PC has a pussy, add the following)
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Your " + vaginaDescript(0) + " meanwhile moistens both from the water and from arousal, and your " + clitDescript() + " aches, craving attention.", false);
		outputText("\n\n", false);
		...
		//(If player has two cocks, add)
		if(player.lowerBody.cockSpot.count() == 2) outputText("  With a slight grin, you position your extra dick at Cotton's other hole. She quirks her eyebrow and a look of panic momentarily crosses her face, but you push forward anyway. Cotton's eyes roll up briefly and her lip quivers.", false);
		//(If the player has more cocks, add)
		else if(player.lowerBody.cockSpot.count() > 2) outputText("  The rest of your cocks strain, aching for holes to fill. With none available, they throb as they rub against Cotton's smooth skin.", false);
		//(If player is too big, add)
	...
	//(Repeat Fuck Her, for humanoid bodies)
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your " + vaginaDescript(0) + " moistens both from the water and from arousal, and your " + clitDescript() + " aches, craving attention.", false);
		outputText("\n\n", false);
		...
		//(If player has two cocks, add)
		if(player.lowerBody.cockSpot.count() == 2) outputText("  With a slight grin, you position your other dick at Cotton's other hole. She quirks her eyebrow and a look of panic momentarily crosses her face, but you push forward anyway. Cotton's eyes roll up briefly and her lip quivers.", false);
		//(If the player has more cocks, add)
		if(player.lowerBody.cockSpot.count() > 2) outputText("  The rest of your cocks strain, aching for holes to fill. With none available, they throb as they rub against Cotton's smooth skin.", false);
		//(If player is too big, add)
		if(player.cockArea(x) > 70) outputText("  Though you are far too big for her, you make sure to stuff her as much as you can. Her cunt, big as it is, squeezes tightly on your " + cockDescript(x) + ". She shivers and quakes, and leans against the shower wall, gripping the shower-head for support. You thrust your overly large tool inside her over and over, stretching her already not-insignificant cunt even wider. Her tongue actually rolls out of her snout, lolling to the side while her eyes roll up into her head, lost in the pleasure. Her body rocks with an orgasm while you piston away under the spray of water.", false);
		//(if player is too small (under 4</i>\") add)
		else if(player.lowerBody.cockSpot.list[x].cockLength < 4) {
...
private function cottonFucksYouInShowerRepeat():void {
	...
	if(player.isTaur()) {
		...
		//(If PC has a vagina)
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Spying your " + vaginaDescript(0) + ", Cotton smiles and flicks your " + clitDescript() + " teasingly before slipping two fingers inside your folds and bringing them to her mouth, licking them clean. \"<i>Mmm... I love the taste of your juices, pet...</i>\"", false);
		//(if PC is genderless)
	...
	//(Repeat get fucked for humanoids and nagas)
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			//(If PC has a large penis, bigger than Cotton's capacity, add)
			if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 70) outputText("  \"<i>Oh my, what's this?</i>\" She puts a hand under your enormous, yet embarrassingly limp cock and lifts it slightly. \"<i>My little pet has such a big dick... Just how I like it. Perhaps next time I'll get to try it out... but not today, hm? This is all about you right now.</i>\"", false);
			//(If PC has a penis under 4</i>\", add)
...
public function cottonTantricSex():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" familiar", false);
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  Meanwhile, your own " + cockDescript(0) + " hangs limply in front of you, tingling with arousal, but unable to harden. Cotton gives it a pat and says, \"<i>Worry not, pet, there's only room for one cock right now, but I'll want it some other time.</i>\"", false);
	outputText("\n\n", false);
	...
	//(If PC has a penis)
	if(player.lowerBody.cockSpot.hasCock()) outputText("  Your " + cockDescript(0) + " twitches and tingles, releasing its pent up seed in a dribbling torrent down your belly and chest, right into your face and pooling around your head.", false);
	//(If PC has a vagina)
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your " + vaginaDescript(0) + " shivers and clamps down hard on the invading cock, pulsing and milking it as it unloads directly into your womb.", false);
...
private function cottonPregnantAlertII():void {
	...
	if (flags[FlagEnum.EDRYN_NUMBER_OF_KIDS] > 0 || (telAdre.edryn.pregnancy.isPregnant && flags[FlagEnum.EDRYN_PREGNANT_AND_NOT_TOLD_PC_YET] != 0)) outputText("  Should have figured as much after you got Edryn pregnant.");
	...
	if(flags[FlagEnum.EDRYN_NEVER_SEE_AGAIN] == 0) outputText("you'll stick with me through this, right?");
...
private function beABadCottonDad():void {
	...
	if(flags[FlagEnum.EDRYN_NEVER_SEE_AGAIN] == 0) outputText("and slaps you as hard as she can");
	else outputText("and punches you as hard as she can");
	outputText(". \"<i>Fuck you 'pet'.");
	if(flags[FlagEnum.EDRYN_NEVER_SEE_AGAIN] > 0) outputText("  That was for Edryn too.");
...
private function cottonContraceptionToggle():void {
	...
	if(flags[FlagEnum.COTTON_KID_COUNT] < 3) outputText("Well that's true,</i>\" she admits.  \"<i>But I leave it up to you, pet.");
...
private function visitCottonKids():void {
	...
	if(flags[FlagEnum.COTTON_KID_COUNT] == 1) outputText("kid");
	...
	if(flags[FlagEnum.COTTON_KID_COUNT] == 1) {
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] == 1) outputText("him");
		else outputText("her");
	}
	...
	if(scene == 0) {
		outputText("\n\nYour ");
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) outputText("baby girl is");
		else outputText("baby boy is");
		outputText(" sitting up in ");
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) outputText("her");
		else outputText("his");
		...
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) outputText("she");
		else outputText("he");
		...
		if(flags[FlagEnum.COTTON_KID_COUNT] == 1) outputText("baby");
		...
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] == 1) outputText("colt");
		...
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] == 1) outputText("he");
		else outputText("she");
		...
		if(flags[FlagEnum.COTTON_KID_COUNT] > 1) {
			outputText("\n\nYour other ");
			if(flags[FlagEnum.COTTON_KID_COUNT] == 2) outputText("baby is");
		...
		outputText("\n\nEventually, though, ");
		if(flags[FlagEnum.COTTON_KID_COUNT] == 1) {
			if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) outputText("she grows tired");
			else outputText("he grows tired");
		}
		else outputText("they grow tired");
		outputText(" and, still giggling, settle");
		if(flags[FlagEnum.COTTON_KID_COUNT] == 1) outputText("s");
		...
		if(flags[FlagEnum.COTTON_KID_COUNT] == 1) {
			if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) outputText("she goes");
	...
	//Little Angel
		...
		if(flags[FlagEnum.COTTON_KID_COUNT] == 1) outputText("your baby is");
	...
	//Breastfeeding
		...
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) outputText("daughter");
		else outputText("son");
		...
		if(flags[FlagEnum.COTTON_KID_COUNT] > 1) outputText("  Fortunately, your other kids don't seem to be hungry, so it's just the one who needs feeding.");

		//(If PC lactates:
		if(player.lactationQ() >= 50) {
			outputText("  You could probably offer to nurse your ");
			if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] != 1) outputText("daughter");
			else outputText("son");
	...
	//Cuddlebug
		...
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) outputText("girl");
		else outputText("he");
...
private function feedYourCottonKids():void {
	...
	if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) outputText("filly ");
	...
	if(flags[FlagEnum.COTTON_KID_COUNT] == 1) outputText(" needs");
...
private function letCottonFeedKids():void {
	...
	if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) outputText("her down for her nap, hm pet?</i>\"");
