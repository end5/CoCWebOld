...
public function cottonGreeting():void {
	spriteSelect(12);
	Render.text("", true);
	Render.text(images.showImage("cotton-greeting"));
	if(flags[FlagEnum.LIFETIME_GYM_MEMBER] == 0) Render.text("The centauress sees you starting for the horse-girl and says, \"<i>Go ahead and talk, but if you want to work out with her I'll have to charge you.</i>\"\n\n", false);
	//Greeting (first time):
	if(flags[FlagEnum.COTTON_MET_FUCKED] == 0) {
		flags[FlagEnum.COTTON_MET_FUCKED] = 1;
		Render.text("You wander over to the equine on the mat, curious as to what she's doing.  She stretches out on her mat, spine flexed.  Her head cranes backwards to see you approach.  \"<i>Oh, hi there!</i>\" She grins an upside-down grin at you and finishes her stretch before standing up.  ", false);
		//(If PC height is greater than 6'6</i>\"
		if(player.tallness > 78) Render.text("Despite her impressive stature, you still look down on her.", false);
		//(If PC height is in between:
		else if(player.tallness >= 76) Render.text("She's roughly your height, which is rather impressive in itself.", false);
		//(If PC height is under 6'4</i>\":
		else Render.text("She is truly tall, and you have to look up to her.", false);
		Render.text("  She's garbed in a white and pink form-fitting tank top along with a pair of black skintight pants that come down to about her mid-shin.  The pants do absolutely nothing to hide the enormous bulge in her crotch, if anything they only enhance it.  Her dark brown skin seems smooth and hairless, unlike most equines you've met, and her red mane of hair falls just past her shoulders, though it's currently pulled back into an efficient ponytail.\n\n", false);

		//(If player has a Centaur or Naga body, replace last line with:
		if(player.isTaur() || player.isNaga()) {
			/*Render.text("\"<i>I'd love to teach you, but I'm afraid I don't know any good routines for your... body type. Sorry, pet.</i>\"", false);
			//Back to gym!
			doNext(13);
			return;*/
		...
		Render.text("\"<i>Hey, I'm Cotton, what can I do you for?</i>\" she says in a friendly manner.  You also give your name, and explain you were curious as to what she was doing.  \"<i>Oh, this? Just doing some yoga.</i>\"  Judging by your quizzical look, she continues, \"<i>Yoga is like an exercise routine for your body and soul.  When the body is happy and healthy, the mind and soul follow.  It's a very relaxing and... sensual exercise.  Would you like to try it?</i>\"", false);
		//[Yes] [No]
	...
	//Met before
		...
		//(If Centaur or Naga)
		if (player.isTaur() || player.isNaga()) {
			/*Render.text("You approach Cotton, who gives you a friendly smile. \"<i>Hey, there little pet. I'm afraid I don't know any good stretches for your body... Maybe some other time.</i>\"", false);
			doNext(13);
			return;*/
			centaurNagaBodyBookStuff();
		}
...
private function cottonGreetingCommonEnd():void {
	if (player.pregnancyIncubation <= 225 && player.pregnancyType == PregnancyType.COTTON)
	{
		...
		Render.text("As you approach Cotton, she smiles and looks at your round belly.  \"<i>Hey there my pet, I'm afraid in your condition, yoga is out of the question... but we can do some special stretches and lamaze, just get dressed as usual.</i>\"");

		Render.text("Do you want to engage in yoga lamaze with her?");
	}
	else {
		switch (pregnancy.event) {
			case 1: Render.text("You approach Cotton, who, to your surprise, is just polishing off what must have been the largest glazed pastry you've ever seen.  You notice muffin wrappers and other evidence of baked goods near her workout mat as well.  You ask if Cotton's been feeling a bit peckish recently... you correct yourself and ask if Cotton's been starving recently.");
			...
			case 4: Render.text("Cotton smiles and greets you as you approach.  Gone is her usual top, instead she's chosen to show off her enormous, round brown belly with an incredibly short white tank top which might be confused with a bra if you didn't know any better.  Though you'd imagine her movement would be quite limited, she actually moves with her usual grace... until her belly collides with a row of water bottles set out on a nearby table, knocking them to the ground.");
					...
					break;
			default:
					Render.text("You approach Cotton, who smiles and says, \"<i>Hey there my little pet, if you'd like a work out, just get changed and come on back.</i>\"\n\nDo you want to engage in yoga with her?");
		}
	}
	cottonMenu();
}
...
private function cottonMenu():void {
	...
	if (flags[FlagEnum.COTTON_KID_COUNT] > 0) addButton(1,"Visit Kids",visitCottonKids);
	if (pregnancy.isPregnant || flags[FlagEnum.COTTON_KID_COUNT] >= 1) addButton(2,"Herbs",cottonContraceptionToggle);
...
private function centaurNagaBodyBookStuff():void {
	...
	//First time with book
		...
		Render.text("You approach Cotton, who gives you an exuberant grin. \"<i>I've read through the book and I'm pretty confident I can coach you now. What do you say we give it a go? I've arranged for some proper yoga clothes for you. Well, really just the top, but that's what matters for you, isn't it?</i>\"\n\n", false);
		Render.text("Do you want to engage in Yoga with her?", false);
		flags[FlagEnum.COTTON_UNUSUAL_YOGA_BOOK_TRACKER]++;
		cottonMenu();
	}
	else cottonGreetingCommonEnd();
...
private function turnDownYogaWifCottonFirstTime():void {
	spriteSelect(12);
	Render.text("", true);
	Render.text("\"<i>That's all right, to each their own.  I'll be here if you ever change your mind.</i>\"  With that, Cotton returns to her mat and continues stretching in various poses.\n\n", false);
	doNext(camp.returnToCampUseOneHour);
}
...
private function acceptYoga():void {
	spriteSelect(12);
	Render.text("", true);
	Render.text(images.showImage("cotton-yoga"));
	...
	if(flags[FlagEnum.LIFETIME_GYM_MEMBER] == 0) {
		...
		statScreenRefresh();
		Render.text("The centauress collects ten gems for gym fees before the two of you can get into it.\n\n", false);
	}
	//(Yes) LAMAZE
	if (player.pregnancyIncubation <= 225 && player.pregnancyType == PregnancyType.COTTON)
	{
		Render.text("You change into your yoga clothes and approach Cotton, saying you'd love a lamaze class. Cotton smiles and sets up a mat for you, then sits down, urging you to sit in front of her.  You do so, feeling the bulge in her pants pressing against your rump, and her breasts at your back.  You spend the next fifteen minutes doing breathing exercises like this, and another fifteen minutes doing stretches on an exercise ball.  As you're working out, Cotton presses her body against yours, running her hands around your swollen belly at every opportunity.\n\n");

		//CHAT STUFF!
		cottonChat();

		Render.text("Once you're done and about to hit the showers, Cotton pulls you aside and says with a grin, \"<i>Up for some post-workout exercises?</i>\"", false);
	...
	//First time
		...
		Render.text("\"<i>Good, good, you won't regret it. First things first, pet, let's get you out of that dreadful clothing.</i>\"  She leads you to the lockers and helps you strip out of your " + player.armorName + ".", false);
		if(player.gender == 3) Render.text("  She spies your crotch and smiles, \"<i>Oh, best of both worlds, are we? Well you're in good company then.</i>\"", false);
		//(If PC is male:
		else if(player.lowerBody.cockSpot.hasCock()) Render.text("  She cradles your " + cockDescript(0) + " and smiles at you, \"<i>Well, we might find a use for that later.</i>\"", false);
		//(If PC is female:
		else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  She pats your groin and says, \"<i>Y'know, I know some good exercises that really work the vaginal muscles... perhaps we'll talk about that later.</i>\"", false);
		Render.text("\n\n", false);

		Render.text("She then helps you into an outfit identical to her own and you both return to the gym proper.\n\n", false);

		Render.text("She instructs you to lay on the mat, and you do. She proceeds to talk you through several poses, and hovers over you to correct your positioning here or there with a soft touch. Several times she bends over you, and her enormous bosom gets right in your face. At one point she stands over your head and you accidentally get a face full of her musky crotch. After about a half hour of this, the arousal and fatigue is too much and you explain you have to quit for now.\n\n", false);
		dynStats("lus", (10+player.stats.lib/10+player.stats.sens/20));

		Render.text("\"<i>Oh, that's too bad. But you've done pretty good for a beginner,</i>\" she helps you up off the mat and pats you gently on the back. \"<i>Want to hit the showers then?</i>\" Despite having done little more than stretching, you find you are sweating quite a bit... but something makes you wonder if her idea of hitting the shower is the same as yours.", false);
		//[Shower] or [Leave]
	...
	//Done yoga > 0 && met type = 1
		...
		Render.text("You change into your yoga clothes and approach Cotton, saying you'd love another yoga workout. Cotton smiles and sets up a mat for you. You spend the next half an hour trying different poses and stretches while she corrects you, sometimes pressing her body against yours to show you how it's done.\n\n", false);

		Render.text("Once you're done and about to hit the showers, Cotton pulls you aside and says, \"<i>I know you weren't comfortable with our shower before, so I won't join you this time. But if you ever change your mind, just say the word.</i>\"", false);
...
private function leaveAfterYoga():void {
	spriteSelect(12);
	Render.text("", true);
	Render.text("\"<i>Suit yourself. You can run around all stinky, meanwhile I'm going to go wash. Feel free to drop by later for some more yoga if you'd like.</i>\"  With that, Cotton heads off to the showers and you leave the gym.\n\n", false);
	doNext(camp.returnToCampUseOneHour);
}
...
private function cottonShowerFunTimes():void {
	...
	Render.text("", true);
	Render.text("\"<i>Perfect! Let's go,</i>\" she takes you by the arm and leads you back into the lockers, and then into the nearby showers, which are apparently unisex. You both strip down and turn on a shower head. After a few moments of scrubbing up, you give your shower buddy a surreptitious glance.\n\n", false);

	Render.text("Without the clothes on, you can see her body is quite well toned. Not overly muscular, but you can easily make out the muscles on her belly. She sports a belly ring, and each nipple has a barbell piercing. Looking a bit lower, you finally take in her equine cock. It's quite large, at least a foot long flaccid, as it is now. You wonder how big it gets... She catches you looking and gives you a devilish grin.\n\n", false);

	Render.text("\"<i>Well? Care for a little... post-workout stretching?</i>\"", false);
...
private function cottonFucksYou():void {
	...
	flags[FlagEnum.COTTON_MET_FUCKED] = 2;
	Render.text("", true);
	Render.text(images.showImage("cotton-fucks-you"));
	player.slimeFeed();
	if(player.gender == 1) {
	...
	//(Get fucked, as Female)
		...
		Render.text("Cotton smiles, continuing to massage the soap in until she gets to your rear. There, she gets another handful of soap and presses it into your crevasse, gently soaping you up from " + clitDescript() + " to tailbone. Then she carefully inserts one finger into your pussy, then two, three, and before long her entire hand is exploring your most personal depths. She giggles and withdraws her hand, \"<i>My my, such an eager little cunt you have, my pet.</i>\"\n\n", false);
		...
		Render.text("Before long you find yourself moaning beneath her, your cunt clenching and unclenching uncontrollably. \"<i>Ooh, my little pet likes it now, hmmm?</i>\" She whispers into your ear and nibbles on it ever-so-slightly. You can't help but give a breathless \"<i>Yes</i>\" in response. Cotton giggles and speeds up her thrusts. You find yourself pushing back into her, urging her to go deeper and deeper. Your clit twinges with pleasure after every thrust.\n\n", false);
		...
		Render.text("The two of you slip down onto the floor, letting the water rinse you off for the time being. After a minute, Cotton pulls her now flaccid (but still impressive) cock from your folds, which gapes open for several minutes afterward, and proceeds to clean the both of you up.\n\n", false);

		Render.text("Your 'post-workout stretch' and shower done, the two of you dry off, redress and leave the gym. Cotton takes you by the arm and says, \"<i>That was great, little pet. Come by the gym anytime. I'll be waiting.</i>\"  Then, she heads back home.  With a little grin on your face, you do the same.", false);
		player.orgasm();
		dynStats("sen", -1);
		cottonPregPCChance();
	}
...
private function fuckCottonInShowerRepeat():void {
	...
	spriteSelect(12);
	Render.text("", true);
	Render.text(images.showImage("cotton-shower-fuck-repeat"));
	let x:number = player.cockThatFits(60);
	if(x < 0) x = 0;

	/*OLD ORIGINAL REPEAT SHOWER SEX
	Render.text("You decide to take her up on her offer, pulling her into the showers and quickly disrobing. Seeing the look on your face, she does too, and quickly enters the showers, turning only one shower-head on. She turns around and grabs you, pulling you into an embrace as you kiss under the steaming water. Both your cocks stir, rising and rubbing together while you make out.\n\n", false);

	Render.text("Finally breaking the kiss, you reach down and hook your arm under one of her legs, lifting her leg up while you position your rock hard member at her pussy. She leans forward to kiss again as you thrust forwards, causing her to moan into your mouth. You thrust in and out while her horse-cock bounces between the two of you, only adding to the excitement.\n\n", false);

	Render.text("After several minutes, the hot shower, hot woman and hot sex are just too much for you, and give one last thrust, pushing your bodies forward to slam against the wall. It's clearly too much for Cotton as well, as she groans out in orgasmic ecstasy. Her cock, sandwiched between you, tenses and explodes just as yours does, covering Cotton in cum inside and out.\n\n", false);

	Render.text("After a moment you pull out, share a deep kiss, and wash each other up before redressing and leaving the gym.", false);
	*/
	//(Repeat Fuck Her, for centaurs)
	if(player.isTaur()) {
		Render.text("You decide to take her up on her offer and lead her into the showers, quickly disrobing and turning on an available shower-head. Cotton strips as well and you pull her under the stream of water, letting your horse body remain out of the water for now, sharing a kiss as steam begins to form around you. She runs a hand through your " + hairDescript() +" and grips the back of your neck, ");
		if (pregnancy.event > 1) Render.text("pulling you gently towards her bulging belly, which you can't help but put your hands against as you kiss her. The bulge is firm and solid, almost a drum of solid muscle, and you caress it as you and the mare-morph make out, eliciting soft murmurs of appreciation for your efforts.\n\n");
		else Render.text("pulling you closer.\n\n", false);

		//(If PC has one cock)
		if(player.cockArea(x) >= 100) {
			if(player.lowerBody.cockSpot.count() == 1) Render.text("You feel your " + cockDescript(x) + " stirring beneath your large body, while Cotton's remains curiously limp.  While her cock dangles, yours strains for attention.", false);
			//(If PC has multiple cocks)
			else if(player.lowerBody.cockSpot.count() > 1) Render.text("You feel your " + multiCockDescriptLight() + " stirring beneath your large body, while Cotton's remains curiously limp.  While her cock dangles, your group of cocks strains for attention.", false);
		}
		else {
			if(player.lowerBody.cockSpot.count() == 1) Render.text("You feel your " + cockDescript(x) + " stirring beneath your large body, while you watch Cotton's do the same. Her cock rubs against your stomach while yours strains for attention.", false);
			//(If PC has multiple cocks)
			else if(player.lowerBody.cockSpot.count() > 1) Render.text("You feel your " + multiCockDescriptLight() + " stirring beneath your large body, while you watch Cotton's do the same. Her cock rubs against your stomach while your group of cocks strains for attention.", false);
		}
		//(If PC has a pussy, add the following)
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Meanwhile, your " + vaginaDescript(0) + " behind you moistens both from the steam and from arousal, and your " + clitDescript() + " aches, craving attention.", false);
		Render.text("\n\n", false);

		//(If PC has breasts)
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) {
			Render.text("Cotton leans down, groping your " + biggestBreastSizeDescript() + ", taking one " + nippleDescript(0) + " into her mouth and sucking it greedily.", false);
			//(and if PC is lactating)
			if(player.biggestLactation() >= 1) {
				Render.text("  Her efforts are soon rewarded as milk begins seeping from your " + nippleDescript(0) + ". Cotton's eyes turn up to your face in surprise, but she doesn't remove her mouth, instead taking the time to gulp down your tasty milk. It isn't long before she draws back, wipes her mouth and practically tackles your other breast, eager to drain it of its precious cargo. You can't help but moan as a draining sensation overwhelms you. After a moment, Cotton pulls away and smacks her lips. \"<i>That is some tasty, tasty milk, pet, I might have to taste you more often.</i>\"", false);
				flags[FlagEnum.COTTON_BREAKFAST_CLUB] = 1;
			}
			//(else is PC is not lactating)
			else Render.text("  Cotton soon switches to the other breast, teasing your " + nippleDescript(0) + " with her talented tongue.", false);
			Render.text("\n\n", false);
		}

		Render.text("Suitably turned on now, you look around and drag one of the nearby benches from the changing rooms into the showers and pull it under the water. Cotton grins, understanding what you plan to do with it, and gets onto all fours on top of it. You walk over the bench, getting your body under the spray of water, and get your cock into position. Your fellow equine lover helps with the fine movement, placing your cock at her entrance while giving your underside a reassuring pat. You slowly push forward, and Cotton bites her lip, again giving a reassuring pat.", false);
		//(If player has two cocks, add)
		if(player.lowerBody.cockSpot.count() == 2) Render.text("  With a slight grin, you begin poking your extra dick at Cotton's other hole. You hear an indignant grunt, followed by a sigh, and feel her hands on your member, guiding it all the way into her hole.", false);
		//(If the player has more cocks, add)
		else if(player.lowerBody.cockSpot.count() > 2) Render.text("  The rest of your cocks jiggle and bob, rubbing up against her thighs and butt wildly.", false);
		//(If player is too big, add)
		if(player.cockArea(x) > 70) Render.text("  Though you are far too big for her, you make sure to stuff her as much as you can. Her cunt, big as it is, squeezes tightly on your " + cockDescript(x) + ". She shivers and quakes, and leans against the bench, gripping your forelegs for support. You thrust your overly large tool inside her over and over, stretching her already not-insignificant cunt even wider. Her tongue actually rolls out of her snout, lolling to the side while her eyes roll up into her head, lost in the pleasure. Her body rocks with an orgasm while you piston away under the spray of water.", false);
		//(if player is too small (under 4</i>\")
		else if(player.lowerBody.cockSpot.list[x].cockLength < 4) {
			Render.text("  Her approval wavers and you hear from below you, \"<i>Is it in yet?</i>\" Your face flushes red and you confirm it is, looking down with embarrassment. Cotton rubs one of your forelegs reassuringly and says, \"<i>");
			if (pregnancy.event > 1) Render.text("Don't worry pet, it's plenty adequate for me.  And you've got to admit,</i>\" she smirks, rubbing her round belly, \"<i>It gets the job done, doesn't it?");
			else Render.text("Don't worry pet, it's plenty adequate for me.");
			Render.text("</i>\" Letting a little smile cross your lips, you push your " + cockDescript(x) + " fully into her, not a big accomplishment for one of your size, but still enough to make you feel good.", false);
		}
		//(if none of the above, add)
		else Render.text("  You piston back and forth, and though Cotton's cunt is sizable, it's able to grip you tightly. Your equine lover moans and grasps at your chest, eventually seizing your leg for support, though she leans backwards.", false);
		Render.text("\n\n", false);

		Render.text("You keep this up for a while, fatigue not really an issue with a body of your size and shape. Cotton, meanwhile, gasps for breath audibly and has taken to laying her front half down on the bench, her ass right up in the air. She strokes your forelegs lovingly, showing she's still ready for more.  ");
		if(player.cockArea(x) >= 100) Render.text("Oblivious to you, her own limp cock leaks cum down onto the bench as an orgasm rocks her body.");
		else if (pregnancy.event > 1) Render.text("Oblivious to you, her own cock twitches and spasms, letting loose a spray of cum across her swollen, pregnant belly, breasts and the bench as an orgasm rocks her body.");
		else Render.text("Oblivious to you, her own cock twitches and spasms, letting loose a spray of cum across her belly, breasts and the bench as an orgasm rocks her body.", false);
		//(If player is too big, add)
		if(player.cockArea(x) > 70) Render.text("  Even with all the pounding you've been doing, you can't fit your entire girth into her, but she doesn't care. Her eyes seem to roll back and forth like it's hard to focus, and her tongue drips water, sweat and saliva onto her body. She's panting for breath and you are able to make out faint, \"<i>Oh gods, oh gods</i>\" over the running water. Were you able to see her toned stomach, you're sure you could see the outline of your " + cockDescript(x) + " in it. Her back arches under the shower, her red hair messily draped around her face and bench, and she bites her lip as another orgasm rocks her body.", false);
		Render.text("\n\n", false);

		Render.text("Finally you just can't take any more. You give one last thrust into your partner and audibly gasp in relief as an orgasm rolls across your body, hitting your nerves with bolts of ecstatic lightning.", false);
		//(If PC has a pussy)
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Your " + cockDescript(x) + " erupts inside Cotton's warm cunt which clenches you tightly, while your own femme sex twinges and clenches on air, drooling juices down your legs.", false);
		//(else)
		else Render.text("   Your " + cockDescript(x) + " erupts inside Cotton's warm cunt which clenches you tightly.", false);
		//(regardless or above, add)
		Render.text("  You give a couple more token thrusts and pull out. As you do, ", false);
		if(player.cumQ() < 200) Render.text("cum begins to seep from her folds", false);
		else if(player.cumQ() < 700) Render.text("a gush of cum escapes from Cotton's folds", false);
		else if(player.cumQ() < 1200) Render.text("a small torrent of cum escapes from Cotton's folds", false);
		else if(player.cumQ() < 2000)Render.text("cum surges from her abused hole", false);
		else Render.text("a massive flood of cum escapes from her cunt", false);
		Render.text(", immediately getting washed towards the drain.\n\n", false);

		Render.text("You both bask in the afterglow for a few moments before you stand up and help Cotton up. You return to the task of cleaning yourselves, sensually washing each other's private areas. Yoga, sex and cleanup done, you get dressed and leave the gym, giving Cotton's hand a final squeeze before departing.", false);
	}
	//(Repeat fuck her, for nagas)
		...
		if(player.cockArea(x) >= 100) {
			//(If PC has one cock)
			if(player.lowerBody.cockSpot.count() == 1) Render.text("  You feel your " + cockDescript(x) + " stirring beneath you, slowly coming to attention, though Cotton's remains curiously limp.  Your " + player.cockHead(x) + " rubs against her belly, sending ripples of pleasure up your spine.", false);
			//(If PC has multiple cocks)
			else if(player.lowerBody.cockSpot.count() > 1) Render.text("  You feel your " + multiCockDescriptLight() + " stirring beneath you, slowly coming to attention, though Cotton's remains curiously limp.  The group of cocks rubs together as you make out, sending ripples of pleasure up your spine.", false);
		}
		else {
			//(If PC has one cock)
			if(player.lowerBody.cockSpot.count() == 1) Render.text("You feel your " + cockDescript(x) + " stirring beneath you, slowly coming to attention alongside Cotton's equine member.  The two rub together as you make out, sending ripples of pleasure up your spine.", false);
			//(If PC has multiple cocks)
			else if(player.lowerBody.cockSpot.count() > 1) Render.text("You feel your " + multiCockDescriptLight() + " stirring beneath you, slowly coming to attention alongside Cotton's equine member. The group of cocks rubs together as you make out, sending ripples of pleasure up your spine.", false);
		}
		//(If PC has a pussy, add the following)
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("Your " + vaginaDescript(0) + " meanwhile moistens both from the water and from arousal, and your " + clitDescript() + " aches, craving attention.", false);
		Render.text("\n\n", false);
		...
		Render.text("Suitably turned on now, you coil your body around Cotton's torso, with your tail spreading her legs apart while you place your " + cockDescript(x) + " at her pussy. You slowly push in, and Cotton bites her lip, looking at you in approval.", false);
		//(If player has two cocks, add)
		if(player.lowerBody.cockSpot.count() == 2) Render.text("  With a slight grin, you position your extra dick at Cotton's other hole. She quirks her eyebrow and a look of panic momentarily crosses her face, but you push forward anyway. Cotton's eyes roll up briefly and her lip quivers.", false);
		//(If the player has more cocks, add)
		else if(player.lowerBody.cockSpot.count() > 2) Render.text("  The rest of your cocks strain, aching for holes to fill. With none available, they throb as they rub against Cotton's smooth skin.", false);
		//(If player is too big, add)
		...
		//(if player is too small (under 4</i>\") add)
			...
			Render.text("  Her approval wavers and she asks, \"<i>Is it in yet?</i>\" Your face flushes red and you confirm it is, looking down with embarrassment. Cotton lifts your chin and gives you a kiss, \"<i>");
			if (pregnancy.event > 1) Render.text("Don't worry pet, it's plenty adequate for me.  And you've got to admit,</i>\" she smirks, rubbing her round belly, \"<i>It gets the job done, doesn't it?");
			else Render.text("Don't worry pet, it's plenty adequate for me.");
			Render.text("</i>\" Giving her a little smile, you push your " + cockDescript(x) + " fully into her, not a big accomplishment for one of your size, but still enough to give you a little smile.", false);
		}
		//(otherwise add)
		else Render.text("  You piston back and forth, and though Cotton's cunt is sizeable, it's able to grip you tightly. Your equine lover moans and grasps at your chest, eventually seizing your shoulder for support, though she leans backwards.", false);
		Render.text("\n\n", false);

		Render.text("You keep this up for a while, before fatigue takes over. You carefully lay Cotton down in the spray of water, coiling your body under her so her rear end is up in the air, hooves resting on the ground, and continue your work unhindered. She kisses your lips, neck, and whatever she can get her mouth on.  ");
		if(player.cockArea(x) >= 100) Render.text("Her own cock, still limp, visibly twitches and begins leaking cum onto her belly as an orgasm rocks her body.");
		else if (pregnancy.event > 1) Render.text("Her own cock twitches and spasms, letting loose a spray of cum across her swollen, pregnant belly, breasts and face as an orgasm rocks her body.");
		else Render.text("Her own cock twitches and spasms, letting loose a spray of cum across her belly, breasts and face as an orgasm rocks her body.", false);
		//(If player is too big, add)
		if(player.cockArea(x) > 70) Render.text("  Even in this pseudo-missionary position you can't stuff your entire enormous girth into her, but she doesn't care. Her eyes seem to roll back and forth like it's hard to focus, and her tongue drips water, sweat and saliva onto her body. She's panting for breath and you are able to make out faint, \"<i>Oh gods, oh gods</i>\" over the running water. You can even make out the shape of your cock in Cotton's toned stomach, pumping back and forth, which brings a smile to your face. Her back arches under the shower, her hair messily spread out beneath her and she bites her lip as another orgasm rocks her body.", false);
		Render.text("\n\n", false);

		Render.text("Finally you just can't take anymore. You give one last thrust into your partner and audibly gasp in relief as an orgasm rolls across your body, hitting your nerves with bolts of ecstatic lightning.", false);
		//(If PC has a pussy)
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Your " + cockDescript(x) + " erupts inside Cotton's warm cunt, which clenches you tightly, while your own femme sex twinges and clenches on air, drooling juices down your legs.", false);
		//(else)
		else Render.text("  Your " + cockDescript(x) + " erupts inside Cotton's warm cunt, which clenches you tightly.", false);
	...
	//(Repeat Fuck Her, for humanoid bodies)
		...
		Render.text("You decide to take her up on her offer and lead her into the showers, quickly disrobing and turning on an available shower-head. Cotton strips as well and you pull her under the stream of water, sharing a kiss as steam begins to form around you. She runs a hand through your " + hairDescript() +" and grips the back of your neck, ");
		if (pregnancy.event > 1) Render.text("pulling you gently towards her bulging belly, which you can't help but put your hands against as you kiss her. The bulge is firm and solid, almost a drum of solid muscle, and you caress it as you and the mare-morph make out, eliciting soft murmurs of appreciation for your efforts.");
		else Render.text("pulling you closer.", false);
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Your " + vaginaDescript(0) + " moistens both from the water and from arousal, and your " + clitDescript() + " aches, craving attention.", false);
		Render.text("\n\n", false);
		...
		Render.text("Suitably turned on now, you reach down and hook your arm under one of Cotton's legs, lifting it up while you position your " + cockDescript(x) + " at her pussy. You slowly push in, and Cotton bites her lip, looking at you in approval.", false);
		//(If player has two cocks, add)
		if(player.lowerBody.cockSpot.count() == 2) Render.text("  With a slight grin, you position your other dick at Cotton's other hole. She quirks her eyebrow and a look of panic momentarily crosses her face, but you push forward anyway. Cotton's eyes roll up briefly and her lip quivers.", false);
		//(If the player has more cocks, add)
		if(player.lowerBody.cockSpot.count() > 2) Render.text("  The rest of your cocks strain, aching for holes to fill. With none available, they throb as they rub against Cotton's smooth skin.", false);
		//(If player is too big, add)
		if(player.cockArea(x) > 70) Render.text("  Though you are far too big for her, you make sure to stuff her as much as you can. Her cunt, big as it is, squeezes tightly on your " + cockDescript(x) + ". She shivers and quakes, and leans against the shower wall, gripping the shower-head for support. You thrust your overly large tool inside her over and over, stretching her already not-insignificant cunt even wider. Her tongue actually rolls out of her snout, lolling to the side while her eyes roll up into her head, lost in the pleasure. Her body rocks with an orgasm while you piston away under the spray of water.", false);
		//(if player is too small (under 4</i>\") add)
		else if(player.lowerBody.cockSpot.list[x].cockLength < 4) {
			Render.text("  Her approval wavers and she asks, \"<i>Is it in yet?</i>\" Your face flushes red and you confirm it is, looking down in embarrassment. Cotton lifts your chin and gives you a kiss, \"<i>");
			if (pregnancy.event > 1) Render.text("Don't worry pet, it's plenty adequate for me.  And you've got to admit,</i>\" she smirks, rubbing her round belly, \"<i>It gets the job done, doesn't it?");
			else Render.text("Don't worry pet, it's plenty adequate for me.");
			Render.text("</i>\" Giving her a little smile, you push your " + cockDescript(x) + " fully into her, not a big accomplishment for one of your size, but still enough to give you a little smile.", false);
		}
		...
		Render.text("You keep this up for a while before fatigue takes over. You carefully lay Cotton down in the spray of water and continue your work unhindered. She kisses your lips, neck, and whatever she can get her mouth on.  ");
		if(player.cockArea(x) >= 100) Render.text("Her own cock, still limp, visibly twitches and begins leaking cum onto her belly as an orgasm rocks her body.");
		else if (pregnancy.event > 1) Render.text("Her own cock twitches and spasms, loosing a spray of cum across her swollen, pregnant belly, breasts and face as an orgasm rocks her body.");
		else Render.text("Her own cock twitches and spasms, letting loose a spray of cum across her belly, breasts and face as an orgasm rocks her body.", false);
		//(If player is too big, add)
		if(player.cockArea(x) > 70) Render.text("  Even in the missionary position you can't stuff your entire enormous girth into her, but she doesn't care. Her eyes seem to roll back and forth like it's hard to focus, and her tongue drips water, sweat and saliva onto her body. She's panting for breath and you are able to make out faint, \"<i>Oh gods, oh gods</i>\" over the running water. You can even make out the shape of your cock in Cotton's toned stomach, pumping back and forth, which brings a smile to your face. Her back arches under the shower, her hair messily spread out beneath her and she bites her lip as another orgasm rocks her body.", false);
		Render.text("\n\n", false);
...
private function cottonFucksYouInShowerRepeat():void {
	...
	flags[FlagEnum.COTTON_MET_FUCKED] = 2;
	Render.text("", true);
	Render.text(images.showImage("cotton-shower-fucks-you-repeat"));
	/*OLD SEX SCENES HERE
	if(player.lowerBody.cockSpot.hasCock() && (player.gender != 3 || rand(2) == 0)) {
		...
		Render.text("Finally breaking the kiss, Cotton reaches down and hooks her arms under both your legs. You quickly wrap your arms around her neck as she lifts you off the ground. You carefully grip her with your legs as she uses one arm to position her dick at your waiting entrance. You give her a kiss just as she presses into you, and moan into her mouth. She gives a couple careful thrusts before her free hand returns to holding you.\n\n", false);

		Render.text("At this angle she can't quite get her entire girth into you, but that doesn't matter, as it feels absolutely exquisite.", false);
		player.buttChange(72,true,true,false);
		Render.text("  You take turns kissing each other's necks and nibbling each other's ears while she thrusts in and out. And after several minutes neither of you can take much more. She gives one last thrust, pulling you down further onto her cock as it explodes within you. Your whole body shudders with orgasmic energy and you bury your head into her neck, stifling a scream.\n\n", false);

		Render.text("After a moment, Cotton pulls you up, letting her shrinking member flop to the floor, and sets you down. Only now you notice your flaccid cock also came at some point, covering both your bodies in seed. You share a deep kiss again and wash each other up before redressing and leaving the gym.\n\n", false);
		player.orgasm();
		dynStats("sen", 1);
	}
	//(Shower Sex, Get Fucked as Female)
		...
		Render.text("At this angle she can't quite get her entire girth into you, but that doesn't matter, as it feels absolutely exquisite. You take turns kissing each other's necks and nibbling each other's ears while she thrusts in and out. And after several minutes neither of you can take much more. She gives one last thrust, pulling you down further onto her cock as it explodes within you. Your whole body shudders with orgasmic energy and you bury your head into her neck, stifling a scream.\n\n", false);

		Render.text("After a moment, Cotton pulls you up, letting her shrinking member flop to the floor, and sets you down. You share a deep kiss again and wash each other up before redressing and leaving the gym.", false);
		player.orgasm();
		dynStats("sen", -1);
	}*/
	...
	if(player.isTaur()) {
		...
		if(player.gender == 1) Render.text("  Your " + cockDescript(0) + " tingles betwixt your legs, but remains limp in the presence of Cotton's impressive member.", false);
		//(If PC has a penis and vagina)
		else if(player.gender == 3) Render.text("  Your " + cockDescript(0) + " tingles betwixt your legs, but remains limp in the presence of Cotton's impressive member, while your " + vaginaDescript(0) + " moistens almost immediately from the steam and arousal.", false);
		//(If PC has a vagina and no penis)
		else if(player.gender == 2) Render.text("  Your " + vaginaDescript(0) + " moistens almost immediately as you make out, both from the steam and your increasing arousal.", false);
		//(If PC is genderless)
		else Render.text("  A deep aching burns within you, a need your body is ill-equipped to process, but still your nipples harden and you find your " + assholeDescript() + " puckering in anticipation.", false);
		Render.text("\n\n", false);

		//(If PC has breasts)
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) {
			Render.text("Cotton's kisses lead down to your " + biggestBreastSizeDescript() + ", where she takes one " + nippleDescript(0) + " into her mouth, sucking it greedily and teasing it masterfully.", false);
			//(and if PC is lactating)
			if(player.biggestLactation() >= 1) {
		...
		Render.text("She continues kissing down your belly, reaching your centaur body. She makes you turn around so your rear end is in the spray of water with her.", false);
		//(If PC has a large penis, bigger than Cotton's capacity, add)
		if(player.lowerBody.cockSpot.hasCock()) {
			if(player.cockArea(0) > 70) Render.text("  \"<i>Oh my, what's this?</i>\" She puts a hand under your enormous, yet embarrassingly limp cock and lifts it slightly. \"<i>My little pet has such a big dick... Just how I like it. Perhaps next time I'll get to try it out... but not today, hm? This is all about you right now.</i>\"", false);
			//(If PC has a penis under 4</i>\", add)
			else if(player.longestCockLength() < 4) Render.text("  \"<i>Awww, what's this?</i>\" She puts a hand under your embarrassingly small and limp cock and lifts it slightly. \"<i>It's so cute and tiny. And it certainly knows its place. Only room for one cock right now, not that this is much of a cock.</i>\" She giggles and plants a kiss on the tip, \"<i>It is cute though. I love it.</i>\"", false);
			//(If PC has a penis neither large or small, add)
			else Render.text("Cotton puts a hand under your embarrassingly limp cock and smiles, \"<i>You know how to show a girl you like her... There's only room for one cock right now.</i>\"", false);
		}
		//(If PC has a vagina)
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Spying your " + vaginaDescript(0) + ", Cotton smiles and flicks your " + clitDescript() + " teasingly before slipping two fingers inside your folds and bringing them to her mouth, licking them clean. \"<i>Mmm... I love the taste of your juices, pet...</i>\"", false);
		//(if PC is genderless)
		if(player.gender == 0) Render.text("  She places a hand on your bare crotch and quirks an eyebrow. \"<i>Well this is new... Certainly not bad though.</i>\" She runs a hand along your bare mound, which somehow manages to send ripples of pleasure up your spine.", false);
		Render.text("\n\n", false);

		Render.text("Cotton continues the kisses down to your " + player.legs() + " and stands, dragging over a bench from the locker room before standing on it and giving your " + buttDescript() + " a good smack. You turn back and give her a coy look, which she returns and gives your flank another smack.", false);
		Render.text("  She takes a moment to get some water from the shower over your rear end before pressing her cock against your " + player.assholeOrPussy() + ", slipping it in gently, careful not to go too quick. You moan slightly and blush, whispering back at her, urging her to continue.", false);
	...
	//(Repeat get fucked for humanoids and nagas)
		...
		Render.text("She continues kissing a trail down your belly and to your crotch.", false);
		if(player.lowerBody.cockSpot.hasCock()) {
			//(If PC has a large penis, bigger than Cotton's capacity, add)
			if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 70) Render.text("  \"<i>Oh my, what's this?</i>\" She puts a hand under your enormous, yet embarrassingly limp cock and lifts it slightly. \"<i>My little pet has such a big dick... Just how I like it. Perhaps next time I'll get to try it out... but not today, hm? This is all about you right now.</i>\"", false);
			//(If PC has a penis under 4</i>\", add)
			if(player.longestCockLength() < 4) Render.text("  \"<i>Awww, what's this?</i>\" She puts a hand under your embarrassingly small and limp cock and lifts it slightly. \"<i>It's so cute and tiny. And it certainly knows its place. Only room for one cock right now, not that this is much of a cock.</i>\" She giggles and plants a kiss on the tip, \"<i>It is cute though. I love it.</i>\"", false);
			//(If PC has a penis neither large or small, add)
			else Render.text("  Cotton puts a hand under your embarrassingly limp cock and smiles, \"<i>You know how to show a girl you like her... There's only room for one cock right now.</i>\"", false);
		}
		//(If PC has a vagina)
		...
		//(If Naga body)
		if(player.isNaga()) {
			Render.text("Cotton continues the kisses down to your " + player.legs() + ", and stands, grabbing you by the waist and lifting you up off the ground.  ");
			if (pregnancy.event > 1) Render.text("Moving quickly, you wrap your arms around Cotton's neck and coil your body around her waist, just below her round, pregnant belly, and down one leg.  You can feel her swollen midriff rubbing against the scales on your serpentine half with each motion you make.");
			else Render.text("Moving quickly, you wrap your arms around Cotton's neck and coil your body around her waist and down one leg.", false);
		}
		//(If Humanoid body)
		else {
			Render.text("Cotton continues the kisses down to your " + player.legs() + " and stands, hooking an arm underneath each of your legs and lifting you up off the ground.  ");
			if (pregnancy.event > 1) Render.text("Moving quickly, you wrap your arms and legs around Cotton so you don't fall over, carefully avoiding her pregnant belly, which is no mean feat. Her belly is pressed into yours, letting you feel the solid weight of it, gentle kicks from your unborn child within.");
			else Render.text("Moving quickly, you wrap your arms and legs around Cotton so you don't fall over.", false);
		}
		Render.text("  She gives you a quick kiss and moves her cock to your " + player.assholeOrPussy() + ", slipping it in gently, careful not to go too quick. You give her a submissive, but encouraging kiss, and whisper in her ear, urging her to continue.", false);
		//(Stretch and appropriate virginity check)
		if(player.lowerBody.vaginaSpot.hasVagina()) player.cuntChange(72,true,true,false);
		else player.buttChange(72,true,true,false);
		Render.text("\n\n", false);

		Render.text("She can't quite fit her entire girth into you at this angle, but that doesn't matter as it feels absolutely exquisite. You take turns kissing each other's necks and nibbling on each other's ears while she thrusts in and out.  ");
		if (pregnancy.event > 1) {
			Render.text("Cotton leans her gravid bulge heavily into your belly, bending over as best she can to whisper into her ear.  \"<i>Mmm... how do you like this? Knowing you're being fucked like a woman by a woman?  A woman carrying your child, no less?  Because I like this a lot - this is so kinky, to me... oh, I'm gonna fuck the shit out of you, just like you fucked this baby into me.</i>\" She croons, even as she continues to thrust.  ");
		}
		Render.text("Your body shivers and quakes as a miniature orgasm rockets through you, somehow making your whole body more sensitive.  ");
		Render.text("Your powerful partner holds you close, her jet black cock pistoning in and out of your hole. You feel safe in her arms, secure in a way you rarely feel elsewhere. You wrap your arms more tightly around her, hugging Cotton close and biting your lip, stifling your moans and groans of passion.\n\n", false);

		Render.text("Unable to take any more, Cotton gives one last thrust, pulling your body down onto her cock and giving a whinny of a moan. A full orgasm rocks through your body, like lightning striking every nerve. Your back arches, pressing your chest into Cotton's prodigious bosom and even biting your lip doesn't stifle the long, deep moan from escaping your lips. Your equine lover's cock erupts in you, coating your insides with her hot, sticky seed, which even now begins to leak from your hole around the invading cock.\n\n", false);

		Render.text("Exhausted, Cotton sets you down on the shower floor under the spray of water, though cum leaks from your abused hole and your " + player.legs() + " quivering so much you nearly collapse. Your partner steadies you, however, and you regain your footing quickly. You return to the task of cleaning yourselves, sensually washing each other's most intimate areas. Yoga, sex and cleanup done, you get dressed and leave the gym, giving Cotton's hand a final squeeze before departing.", false);
	}
...
public function cottonTantricSex():void {
	spriteSelect(12);
	Render.text("", true);
	Render.text(images.showImage("cotton-tantric-sex"));
	player.slimeFeed();
	flags[FlagEnum.COTTON_MET_FUCKED] = 2;
	//OLD TANTRIC SMEX
	/*Render.text("You decide to ask instead about using yoga to blow off some steam. She grins, \"<i>I know just what you need. It's called tantric sex, and I think you're just limber and quick enough to try it.</i>\"\n\n", false);

	Render.text("She leads you to a private room in the gym and sets up a larger mat. She disrobes and you do the same. She sits on the mat and you sit opposite her. You spend a couple minutes just breathing, exploring your partner's body with your eyes only. Then Cotton slips her legs outwards, and leans backwards, letting her enormous member to waggle freely in the air. She curls a finger at you, and gives you your instructions.\n\n", false);

	Render.text("You stand and straddle her hips, facing her, carefully lowering yourself down onto her length. You take your time, matching your breathing with hers, until you're completely impaled. Then you stretch your legs out behind your partner and also lean back. You both gyrate your hips as your bodies glisten with a light sheen of sweat.", false);
	player.buttChange(72,true,true,false);
	Render.text("\n\n", false);

	Render.text("After several minutes of this you switch positions, with Cotton leaning forward and grabbing you under the waist while you curl your legs under you. Then Cotton pulls herself up into a kneeling position while still inside you, pushing your head and shoulders down onto the mat and your legs up into the ceiling. Several more positions later, your breathing is a little ragged but still in sync, and it's clear neither of you can take much more of this.\n\n", false);

	Render.text("With one last pose, you both stand, Cotton grabs your hips and you lean down to grab your ankles. Your whole body shivers with sexual energy, feeling like it's on fire with orgasmic light as your equine lover gives one final thrust into you and explodes, her own body shivering and shuddering. Unable to remain standing any longer, you flop down onto the mat, letting Cotton's seed slowly leak out of you, while Cotton lays down next to you, equally exhausted.\n\n", false);

	Render.text("\"<i>I must say, my little pet, you are the best yoga partner I've had in a long time.</i>\" You share a kiss and take a moment to rest before heading to the showers and then heading home.", false);*/
	//NU HOTNESSSSS
	//(Tantric Sex ONLY available for humanoids and naga, still requires 50+ Speed)
	Render.text("Knowing full well what she plans for the showers, you instead ask about more \"<i>advanced</i>\" forms of yoga. With a certain coy emphasis on advanced. She grins, \"<i>I know just what you mean. It's called tantric sex, and I think you're just limber and quick enough to try it.</i>\"\n\n", false);

	Render.text("She leads you to a private area in the gym, a large room where one side of the wall is covered in mirrors with a long bar across it. You'd guess this room is used for dance lessons, or other activities that require large amounts of space. She sets up a large mat on the floor, big enough for the both of you to sit on and then some, and proceeds to strip. \"<i>It's best to do this naked... we don't want our clothing to block our spiritual energies. Or, y'know, other things,</i>\" she says with a light laugh. You nod and strip down as well.\n\n", false);

	Render.text("Cotton sits on one end of the mat, crossing her hooved feet under her in the traditional meditative position. You sit on the other side and do the same. \"<i>Now we match our breathing, little pet. In, and out. Each breath brings our spirits closer to each other.</i>\" You inhale deeply, and she does the same. Then out. This continues for minutes, just breathing, until in one exhalation you feel a shiver run down your spine.\n\n", false);

	Render.text("\"<i>And our spirits are joined... for now,</i>\" she laughs and runs her hands along her upper torso, and in a weird, sort of detached way, you feel it. You touch your chest, rubbing a " + nippleDescript(0) + " experimentally, and you see Cotton visibly shiver and give you an approving nod. Whatever she did... you can feel what she feels, however faintly, and vice-versa!\n\n", false);

	Render.text("\"<i>Now comes the yoga portion of this little workout,</i>\" she says as she uncurls her legs, spreading them wide and long on the mat. She bends her knees and leans back, propping herself up on her arms. Her jet-black equine cock stands at full attention, pulsing steadily. \"<i>Come sit down on my cock, pet, and we'll get this party started right.</i>\"\n\n", false);

	Render.text("Gulping slightly, you stand and approach her. Cotton runs a hand along her member, causing you to shiver with the", false);
	if(player.lowerBody.cockSpot.hasCock()) Render.text(" familiar", false);
	else Render.text(" alien", false);
	Render.text(" sensations on an unfamiliar part of your body... well, her body. You stand over her hips and slowly lower yourself down, impaling your " + player.assholeOrPussy() + " on her meaty tool. In addition to the beautiful sensation of being filled, you also get the sensation of warm, tight constriction on your phantom cock.", false);
	//(for Naga Body, add)
	if(player.isNaga()) Render.text("  You impale yourself fully, settling down and sitting on her lap, stretching your snake-like body out behind and to one side of her, with your arms propping you up as you lean back.", false);
	//(for Humanoid Body, add)
	else Render.text("  You impale yourself fully, settling down and sitting on her lap, mimicking her pose with your legs stretched out behind her and your arms propping you up as you lean back.", false);
	Render.text("\n\n", false);

	Render.text("Though there's no real thrusting or vulgar movements of any kind, the feelings are intense, likely heightened by your tantric link. You can simultaneously feel Cotton's long equine cock pulse within you, as well as feel your own " + player.assholeOrPussy() + " clamping down on a cock that is not your own.", false);
	if(player.lowerBody.vaginaSpot.hasVagina()) player.cuntChange(72,true,true,false);
	else player.buttChange(72,true,true,false);
	//(If PC has a penis, add)
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  Meanwhile, your own " + cockDescript(0) + " hangs limply in front of you, tingling with arousal, but unable to harden. Cotton gives it a pat and says, \"<i>Worry not, pet, there's only room for one cock right now, but I'll want it some other time.</i>\"", false);
	Render.text("\n\n", false);

	Render.text("\"<i>Now let's move on to the Lotus,</i>\" Cotton says, and sits up.", false);
	//(Naga body, add)
	if(player.isNaga()) Render.text("  You do the same, curling your arms around your partner while your lower body coils around her midsection. Cotton pulls her legs in as well, and it almost looks like you're both sitting normally, if it weren't for you being on her lap with her dick up inside you.", false);
	//(Humanoid body, add)
	else Render.text("  You do the same and curl your arms and legs around your partner. Cotton pulls her legs in as well, and it almost looks like you're both sitting cross legged if it weren't for you being on her lap with her dick up inside you.", false);
	Render.text("  You take this opportunity to bury your face in Cotton's prodigious bosom, taking one black, perky nipple into your mouth, then the other. You can actually feel your own tongue on phantom nipples, flicking and teasing you mercilessly. You relinquish your hold on the nipples and pull back slightly, your saliva forming a slight bridge from nipple to lip.\n\n", false);

	//(If PC has breasts)
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) {
		Render.text("Cotton returns the favor, her short-snouted mouth latching on to one " + nippleDescript(0) + " and sucking it relentlessly. The sensations you get from this are, if anything, greater than normal. Your nipples feel like they're on fire, with every molecule aching to be touched, licked, sucked and teased.", false);
		//(If PC is lactating, add)
		if(player.biggestLactation() >= 1) {
			Render.text("  You feel a familiar sensation welling up in your breasts as milk begins pouring into Cotton's mouth. She grins as best she can without letting go of your tit, eagerly drinking down your milk. In yet another moment of trepidation, you realize you can taste your own milk slipping over Cotton's tongue. You smack your lips. It tastes sweet and creamy, and oh-so warm. Cotton moves to your next nipple, letting the first dribble milk onto the both of you. Again Cotton sucks on your " + nippleDescript(0) + " for a moment before it begins dispensing its precious cargo into her mouth. You taste every gulp, each one sweeter than the next, before Cotton pulls back and smacks her lips together. \"<i>You taste so sweet, pet. I simply must have you around for breakfast...</i>\"", false);
			flags[FlagEnum.COTTON_BREAKFAST_CLUB] = 1;
		}
		//(else if PC is not lactating)
		else Render.text("  She quickly switches to the other nipple, her tongue expertly teasing and tantalizing it before pulling away once more.", false);
		Render.text("\n\n", false);
	}

	Render.text("\"<i>One last pose, pet, the head game.</i>\"  She grips your " + hipDescript() + " and stands up in one fluid motion. You quickly latch on to her, making sure you don't fall. Cotton's cock shifts with the motion, going slightly deeper and you reflexively clench, which you feel on your phantom cock. \"<i>Now we just lay you down...</i>\" You gulp and release your hold on Cotton's neck, slowly letting yourself lean backwards. The cock shifts within you, feeling so strange and alien as you move down. Finally your head hits the floor, so you curve your neck slightly, letting your shoulders and head rest on the mat.\n\n", false);

	//(for Naga bodies)
	if(player.isNaga()) Render.text("Cotton takes a firm grip on your hips and ass, making sure you don't slip, so you carefully uncoil your body and slip it between Cotton's legs, then coiling it around her waist, chest, and between her breasts. The feeling is quite intense, though at this angle Cotton's cock doesn't fit completely inside you.", false);
	//(for Humanoid bodies)
	else Render.text("Cotton takes a firm grip on your hips and ass, making sure you don't slip, so you carefully extend your " + player.legs() + " out, then over and around Cotton's arms, only pausing to make sure she has a decent grip on your new position, and rest both of your feet on your equine lover's shoulders. The feeling is quite intense and unique, though at this angle Cotton's cock doesn't fit completely inside you.", false);
	Render.text("\n\n", false);

	Render.text("Now comes the thrusting. Or rather, gyrating. Cotton begins making small circular motions with her hips, letting her cock thrust ever-so-slightly in and out of you. Despite the subtle motions, combined with the tantric link you're in complete and utter bliss. Soon, you too are gyrating your hips the best you can.\n\n", false);

	Render.text("It doesn't take much longer for the sensations to consume you, feeling yourself both getting fucked and fucking at the same time. Lightning fires through every nerve in your body as an orgasm begins to overtake you. Cotton doesn't look far behind. As your body begins to tremble and arch, so too does hers.", false);
	//(If PC has a penis)
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  Your " + cockDescript(0) + " twitches and tingles, releasing its pent up seed in a dribbling torrent down your belly and chest, right into your face and pooling around your head.", false);
	//(If PC has a vagina)
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Your " + vaginaDescript(0) + " shivers and clamps down hard on the invading cock, pulsing and milking it as it unloads directly into your womb.", false);
	//(If PC doesn't have a vagina)
	else Render.text("  Your " + assholeDescript() + " shivers and clamps down hard on the invading cock, pulsing and milking it as it unloads directly into your intestines.", false);
	//(regardless of above, add)
	Render.text("  Lightning arcs from nerve to nerve, in both of your bodies. You can feel every sensation Cotton feels. In that moment, you feel perfectly as one. You aren't simply getting fucked by Cotton. You are simultaneously Cotton fucking yourself, as is she.", false);
	Render.text("\n\n", false);

	Render.text("Then in a flash, it's gone. The lightning subsided, you both collapse sideways onto the floor, panting. The tantric link is apparently gone, and you suddenly feel very alone. Cotton, seemingly sensing this, pulls out and curls up next to you, cuddling you until the sudden feeling of loss dissipates.\n\n", false);

	Render.text("After a few minutes, you feel your strength ebbing back into you and you get up. Cotton does the same, and you both spend the time to clean the mat of the results of your escapades, before retreating to the showers to clean each other up. Once that's over, you both leave the gym hand-in-hand, with Cotton giving your hand a final squeeze before you depart to your camp.", false);
	player.orgasm();
...
private function leaveCotton():void {
	spriteSelect(12);
	Render.text("", true);
	Render.text("You thank Cotton for her time, shower and head back to camp.", false);
	doNext(camp.returnToCampUseOneHour);
}
...
public function nomSomeTitMilkCereal():void {
	spriteSelect(12);
	Render.text(images.showImage("cotton-visits-you-at-camp-drinks-all-your-milk-the-asshole"));
	//(Add to Sleep screen under the sunrise line.)
	Render.text("\nAs you awaken you hear a rustling from the bushes around your camp.\n\n", false);

	//(Event proper)
	Render.text("You quickly arise and ready your weapon, prepared for anything to come through the brush. After a moment, a large dark shape bursts through the bushes. You're about to strike when you recognize the figure! It's Cotton from the gym in Tel'Adre! Rather than her normal yoga outfit, she's wearing a tight brown shirt and loose brown pants, along with a backpack slung across one shoulder.\n\n", false);

	Render.text("The horse girl holds up her hands in a nonthreatening manner. \"<i>Whoa there, pet. I just came by to join you for breakfast.</i>\" You sigh and put your weapon away. \"<i>Jumpy huh?</i>\" she says, looking around your camp. \"<i>You've got a pretty good setup here. I don't know why you don't just move into Tel'Adre. I've got a nice little place we could share... But no matter.</i>\"\n\n", false);

	Render.text("You invite her over to your bedroll and sit down. She follows suit and removes her backpack, digging through it for a minute, retrieving a clay bowl and a large brown bag. She opens the bag and pours out a bowlful of grains and oats. \"<i>Normally I just eat my breakfast raw... but today I want to try something a little different.</i>\"", false);

	Render.text("Cotton sets the bowl down in your lap and pulls down the front of your " + player.armorName + ", exposing your " + chestDesc() + " to the cool morning breeze. A shiver runs down your spine and your nipples immediately begin to harden, as if knowing what this equine girl has in mind. Cotton leans down and wraps her lips around one " + nippleDescript(0) + ", sucking sensually and flicking the tip with her tongue.\n\n", false);

	Render.text("You feel the familiar sensation of fluids being drawn from you, and soon Cotton is suckling from your milky tit. She relinquishes her hold on the teat, smacks her lips, and brings the bowl up to your breast. With the other hand, she grabs your " + nippleDescript(0) + " and breast firmly, pulling and squeezing it. Delicious white milk begins squirting from the ducts of your breast, dribbling and spraying into the bowl of grains. You can't help but moan, squirming in your seat as your tit is ruthlessly milked.\n\n", false);

	Render.text("Cotton gives the breast one last lick and moves on to the next. In no time, that one too is spraying a copious amount of milk into the bowl. The sensation is amazing, being milked by hand. So intimate and personal. Your body quakes and trembles as little earthquakes shake across your breasts and down your body. Your mouth forms an O and you let out little moans of pleasure as the boobgasm rolls throughout your body.\n\n", false);

	Render.text("Just like that, Cotton's hand leaves your breast, though the hot and aroused feeling doesn't leave. You open your eyes and look at her. She gives you a smile and takes a bite of grains.\n\n", false);

	Render.text("\"<i>Mmm, you are so tasty, pet,</i>\" she trills, \"<i>Here, try a bite.</i>\" She lifts a spoon full of grain, oats and milk to your mouth. You hesitantly take a bite and can't help but smile. The sweet, smooth taste of your milk perfectly complements the grains. You munch on it and swallow, and have another bite.\n\n", false);

	Render.text("You chat about inconsequential things, mostly the Tel'Adre rumor mill, while you both eat from the bowl. When it's completely gone, Cotton licks out the bowl and packs everything up.\n\n", false);

	Render.text("\"<i>Thanks, pet, that was delicious. I'll be sure to stop by another time so we can share a meal. Feel free to come by the gym too. We can get a little more... personal.</i>\" She kisses you on the cheek, and then departs back towards Tel'Adre.", false);
...
public function cottonPregnantAlert():void {
	clearOutput();
	Render.text("As you enter the gym, keen to work out (in one way or another), you spot Cotton in her usual area.  She's pacing around, and a worried look is plastered across her face. When she sees you, she smiles a little and approaches you. Whatever it is, it clearly can't wait.");
	//-Next-
	menu();
...
private function cottonPregnantAlertII():void {
	clearOutput();
	Render.text(images.showImage("cotton-you-got-her-preggo"));
	Render.text("\"<i>Hello, pet,</i>\" she says, and you notice worry in her voice.  There are slight bags under her eyes, and her crimson ponytail is a little unkempt, with hair jutting out at odd angles.  Something clearly has her frazzled.");
	...
	if (flags[FlagEnum.EDRYN_NUMBER_OF_KIDS] > 0 || (telAdre.edryn.pregnancy.isPregnant && flags[FlagEnum.EDRYN_PREGNANT_AND_NOT_TOLD_PC_YET] != 0)) Render.text("  Should have figured as much after you got Edryn pregnant.");
	Render.text("</i>\"  She wrenches her hands nervously and looks you in the eye. The cool, confident yoga instructor has clearly melted away, revealing the real her.  \"<i>I know we're not serious, but ");
	if(flags[FlagEnum.EDRYN_NEVER_SEE_AGAIN] == 0) Render.text("you'll stick with me through this, right?");
	else Render.text("you won't abandon me like you did Edryn, will you?");
	Render.text("</i>\"");
	//[Leave Her] [Stay]
	menu();
...
private function beABadCottonDad():void {
	clearOutput();
	Render.text("You shake your head.  You certainly can't deal with a kid.  You tell her point blank that you want nothing to do with the child.  Tears well up in her eyes, and her mouth opens and closes several times, without a single sound coming out.");
	...
	if(flags[FlagEnum.EDRYN_NEVER_SEE_AGAIN] == 0) Render.text("and slaps you as hard as she can");
	else Render.text("and punches you as hard as she can");
	Render.text(". \"<i>Fuck you 'pet'.");
	if(flags[FlagEnum.EDRYN_NEVER_SEE_AGAIN] > 0) Render.text("  That was for Edryn too.");
	Render.text("</i>\"  With that, she turns on her hoof and leaves.");
	//bold
	Render.text("<b>(Cotton will no longer speak with you.)</b>");
...
private function beAGoodCottonDad():void {
	clearOutput();
	Render.text("You take her by the shoulders and nod.  You confirm that of course you'll be there for her, whatever she needs.");
...
private function cottonPopsOutAKid():void {
	...
	if(flags[FlagEnum.COTTON_KID_COUNT] == 0) {
		...
		//{Silly Mode:}
		if(silly()) Render.text("As long as the cube shark isn't there.  ");
		Render.text("Let's go.</i>\"");
		//(Depending on PC height:
		if(player.tallness < 60) Render.text("She wraps an arm around your neck");
		else if(player.tallness < 70) Render.text("She places a hand on your shoulder");
		else if(player.tallness < 86) Render.text("She wraps an arm around your midsection");
		else Render.text("She wraps an arm around your waist");
		...
		if(kid == 1) Render.text("male");
		else if(kid == 2) Render.text("female");
		else Render.text("herm");
		Render.text(" foal.");

		Render.text("\n\nCotton turns and <i>awww's</i> as she catches sight of her ");
		if(kid == 1) Render.text("son");
		else Render.text("daughter");
		Render.text(" for the first time.  Though you don't want to let ");
		if(kid != 1) Render.text("her");
		else Render.text("him");
		Render.text(" go, you relinquish your grasp as Cotton's arms sweep ");
		if(kid != 1) Render.text("her");
		else Render.text("him");
		Render.text(" up.  With one hand she lifts up her top, exposing her large brown nipples, and pulls the child to one, where it latches on and suckles like a pro.  You just watch, ready to look away if Cotton asks, but she seems too preoccupied with nursing her new baby to pay much attention to you.");
		...
		if(kid == 3) Render.text("  I guess it runs in the family.");
		Render.text("</i>\"");
		...
		if(kid < 2) Render.text("guy");
		else Render.text("girl");
		Render.text(" situated.</i>\"  She takes the bundle from you, retrieves a fresh clean blanket, and within seconds has ");
		if(kid != 1) Render.text("her");
		else Render.text("him");
		Render.text(" expertly swaddled.  Cotton sets the child in the crib and stands back to admire her handiwork. You step up beside her and wrap an appreciative arm around her waist, telling her that she's done a wonderful job.");
	...
	//(Replaces the Approach scenes)
		...
		Render.text("As you head towards Cotton, you realize something about her is different; the swollen bulge of her late pregnancy is gone!  You ask her if everything is all right.");
		...
		if(kid == 1) Render.text("boy");
		else if(kid == 2) Render.text("girl");
		else Render.text("herm");
		Render.text("!</i>\" she says happily, pulling you into a hug.  \"<i>We could go see ");
		if(kid != 1) Render.text("her");
		else Render.text("him");
		Render.text(" if you like, ");
		if(kid != 1) Render.text("she");
		else Render.text("he");
		Render.text("'s at the daycare here, I wanted to keep a close eye on ");
		if(kid != 1) Render.text("her");
		else Render.text("him");
		Render.text(" today.</i>\"");
		...
		Render.text(", if Cotton thinks that won't upset ");
		if(kid != 1) Render.text("her");
		else Render.text("him");
		Render.text(".");
		...
		Render.text(" good to see ");
		if(kid != 1) Render.text("her");
		else Render.text("him");
		Render.text(" daddy,</i>\" she says, and leads you down a corridor and into a room marked \"<i>DAYCARE</i>\".  As soon as the door opens, you're assaulted by the laughs and giggles of kids playing.");

		Render.text("\n\nA young bunny-girl sees Cotton and says, \"<i>Oh!  I wasn't expecting you back so soon.  ");
		if(kid != 1) Render.text("She");
		else Render.text("He");
		Render.text("'s just down for a nap right now.  Would you like to see ");
		if(kid != 1) Render.text("her");
		else Render.text("him");
		Render.text("?</i>\"  You both nod your heads in the affirmative, and are taken to an off-room.  It's dark, but there's a crib just by the door, and you can make out a slumbering baby with its mother's horse ears twitching as it dreams whatever babies dream.");

		Render.text("\n\nNot wanting to wake ");
		if(kid != 1) Render.text("her");
		else Render.text("him");
		Render.text(", you look ");
		if(kid != 1) Render.text("her");
		else Render.text("him");
		Render.text(" over and then back out of the room, content with the knowledge that you'll get to spend more time with ");
		if(kid != 1) Render.text("her");
		else Render.text("him");
		Render.text(" later.  The two of you head back out to the main section of the gym, while Cotton slaps you on the back, \"<i>We sure do make 'em good, don't we?  Anyway, if you want to work out, just get changed and come on back.</i>\"");
		pregnancy.knockUpForce(); //Clear Pregnancy
		flags[FlagEnum.COTTON_KID_COUNT]++;
		if(flags[FlagEnum.COTTON_KID_COUNT] == 1) flags[FlagEnum.COTTON_OLDEST_KID_AGE] = 1;
...
private function cottonContraceptionToggle():void {
	...
	Render.text("You ask the ");
	if (pregnancy.isPregnant) Render.text("pregnant ");
	Render.text("she-stallion if she's still taking her contraceptive herbs.");
	...
	//(Cottonpreg:
	if (player.gender != 3 || pregnancy.isPregnant) Render.text("virile");
	else Render.text("fertile");
	Render.text("; you actually broke right through the protection they were supposed to give me from ending up a mommy or a daddy.  I may as well not be taking them at all when I'm with you, don't you think?</i>\" she concludes.");

	Render.text("\n\nYou admit that kind of makes sense to you, but it's not as if you're guaranteed to get through their protection, right?\n\n\"<i>");
	if(flags[FlagEnum.COTTON_KID_COUNT] < 3) Render.text("Well that's true,</i>\" she admits.  \"<i>But I leave it up to you, pet.");
	else Render.text("Well, considering the brood we have, they've already proven pretty useless, haven't they?   So, while I'll keep taking them if you want me to, I don't see much point in it.");
	Render.text("</i>\"");
...
private function tellCottonStopEatingHorsePills():void {
	clearOutput();
	Render.text("You tell Cotton that, if they're going to be that useless to her, and she doesn't mind the idea of potentially having more kids, she has your permission to stop taking them.");
...
private function tellCottonToKeepFiringBlanksAsshole():void {
	clearOutput();
	Render.text("You tell Cotton you'd rather not force her to have any more kids, so she should keep taking the herbs; in your opinion, imperfect protection is better than no protection.");

	Render.text("\n\nCotton nods, \"<i>All right then.  As they say, better safe than ");
	if(!silly()) Render.text("sorry");
	else Render.text("overrun by a herd of little horsefolk");
	Render.text(", right?</i>\"");
...
private function repeatContraceptionToggleCotton():void {
	...
	if(flags[FlagEnum.COTTON_HERBS_OFF] == 1) {
		Render.text("You tell Cotton that, while you know it's not a perfect solution, you want her to start taking her herbs again.");
		flags[FlagEnum.COTTON_HERBS_OFF] = 0;
		Render.text("\n\nCotton nods, \"<i>All right then.  As they say, better safe than ");
		if(!silly()) Render.text("sorry");
		else Render.text("overrun by a herd of little horsefolk");
	...
	//Play this scene on subsequent deactivations of Cotton Contraception
		...
		Render.text("You tell Cotton that you're ready for her to go off her herbs again if she wants to.");
		Render.text("\n\nShe nods and says, \"<i>Starting tomorrow I'll go ahead and stop, then.</i>\"");
		Render.text("\n\nYou thank her for understanding you and change the subject.");
...
public function goTellCottonShesAMomDad():void {
	clearOutput();
	Render.text("Convinced that Cotton is the father of the child in your womb, at daybreak, you pack your usual things and head off to Tel'Adre.  Stopping in at the gym, you ask for directions to Cotton's place, and then make your way there.");
...
public function birthingCottonsKids():void {
	...
	Render.text("You can feel movement underneath the skin, and watch as it bulges and shifts as another living being moves independently inside you.");
	...
	Render.text("!  Like ");
	if(kid == 1) Render.text("his");
	else Render.text("her");
	Render.text(" 'father', ");
	if(kid == 1) Render.text("he");
	else Render.text("she");
	Render.text("'s hairless except on ");
	if(kid == 1) Render.text("his");
	else Render.text("her");
	Render.text(" head, where there's a light sprinkling of dark hair.  ");
	if(kid == 1) Render.text("His");
	else Render.text("Her");
	Render.text(" feet end in tiny pristine hooves, and a little brush of a tail extends from ");
	if(kid == 1) Render.text("his");
	else Render.text("her");
	Render.text(" backside.");
	...
	Render.text(", bundle ");
	if(kid == 1) Render.text("him");
	else Render.text("her");
	Render.text(" up good and tight, get dressed, and head off towards Tel'Adre.");
	...
	if(flags[FlagEnum.PC_IS_A_DEADBEAT_COTTON_DAD] > 0) {
		...
		Render.text(".  She takes the child from you, holding ");
		if(kid == 1) Render.text("him");
		else Render.text("her");
		Render.text(" close with practiced grace.");
	...
	//[First Child with Cotton?]
		...
		if(kid == 1) Render.text("He'll");
		else Render.text("She'll");
		Render.text(" be so happy here, pet, I promise you.  I'm an excellent momma, or daddy, as the case may be.  I don't know if I've ever told you, but I had a couple of kids a few years back.  Twins.  Boy they were rambunctious little devils.  Couldn't sit still,</i>\" she smiles wistfully.");
	...
	else
	{
		...
		Render.text("'s so cute, pet.  We sure do make 'em good.  ");
		if(kid == 1) Render.text("He");
		else Render.text("She");
		Render.text("'ll just love it here with ");
		if(kid == 1) Render.text("his");
		else Render.text("her");
		Render.text(" family,</i>\" Cotton declares.  You nod your head, then ask if Cotton will be okay looking after one more child?");
...
private function visitCottonKids():void {
	clearOutput();
	Render.text("You tell Cotton that, if it's okay with her, you'd like to skip exercising today; you were hoping that you could visit your ");
	if(flags[FlagEnum.COTTON_KID_COUNT] == 1) Render.text("kid");
	else Render.text("kids");
	Render.text(", instead? Cotton's eyes widen in surprise before a happy grin splits her face.  \"<i>Sure, pet, all you had to do was ask!  Give me a minute to get my stuff, and then we'll go and see ");
	if(flags[FlagEnum.COTTON_KID_COUNT] == 1) {
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] == 1) Render.text("him");
		else Render.text("her");
	}
	else Render.text("them");
	Render.text(", all right?</i>\"  You nod your head and watch as the equine yoga instructor busies herself gathering her few daily belongings, then follow her as she heads back home, eager to see how your ");
	if(flags[FlagEnum.COTTON_KID_COUNT] == 1) {
		Render.text("child is");
	}
	else Render.text("children are");
	Render.text(" doing.");
	...
	if(scene == 0) {
		Render.text("\n\nYour ");
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("baby girl is");
		else Render.text("baby boy is");
		Render.text(" sitting up in ");
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
		else Render.text("his");
		Render.text(" crib when you both enter, cooing happily and babbling like babies do, clearly happy in ");
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
		else Render.text("his");

		Render.text(" own little way to see you.  Cotton smiles at you, \"<i>Watch this; ");
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("she");
		else Render.text("he");
		Render.text(" loves this game,</i>\" she tells you, and then slowly kneels before the crib.  \"<i>Hello, my little sweet, hellooo.</i>\"  Cotton coos, patting the ");
		if(flags[FlagEnum.COTTON_KID_COUNT] == 1) Render.text("baby");
		else Render.text("babies");
		Render.text(" on the nose and babbling to ");

		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] == 1) Render.text("him");
		else Render.text("her");

		Render.text(" in that way that parents of small children tend to do.  When the little ");
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] == 1) Render.text("colt");
		else Render.text("filly");
		Render.text(" foal is giggling merrily, Cotton slowly cover her eyes with her hands.  \"<i>Where's the baby? Where's the little baby?</i>\" she asks, then quickly uncovers her eyes, adopting an exaggerated expression of surprise.  \"<i>Why, there ");
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] == 1) Render.text("he");
		else Render.text("she");
		Render.text(" is!</i>\"  She gasps, making the baby laugh in delight.  She repeats this several times, the baby enjoying it thoroughly.  You ask Cotton if you can try, and she agrees; ");
		Render.text("your baby seems");
		Render.text(" a little surprised to see you, but that doesn't stop ");

		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
		else Render.text("him");
		Render.text(" from laughing just as hard at you as ");
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] == 1) Render.text("he");
		else Render.text("she");
		Render.text(" did at Cotton.  Feeling a little left out, Cotton rejoins you and you start playing peek-a-boo together, much to the baby's delight.");

		//[Additional Kids:
		if(flags[FlagEnum.COTTON_KID_COUNT] > 1) {
			Render.text("\n\nYour other ");
			if(flags[FlagEnum.COTTON_KID_COUNT] == 2) Render.text("baby is");
			else Render.text("babies are");
			Render.text(" clearly enjoying it just as much as your first-born, giggling and clapping at yours and Cotton's antics, even clumsily trying to mimic the game themselves.");
		}
		Render.text("\n\nEventually, though, ");
		if(flags[FlagEnum.COTTON_KID_COUNT] == 1) {
			if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("she grows tired");
			else Render.text("he grows tired");
		}
		else Render.text("they grow tired");
		Render.text(" and, still giggling, settle");
		if(flags[FlagEnum.COTTON_KID_COUNT] == 1) Render.text("s");
		Render.text(" down to sleep.  You and Cotton fuss over ");
		if(flags[FlagEnum.COTTON_KID_COUNT] == 1) {
			if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
			else Render.text("him");
		}
		...
		Render.text(" in and mussing ");
		if(flags[FlagEnum.COTTON_KID_COUNT] == 1) {
			if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
			else Render.text("his");
		}
		else Render.text("their");
		Render.text(" hair gently, but watch until ");
		if(flags[FlagEnum.COTTON_KID_COUNT] == 1) {
			if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("she goes");
			else Render.text("he goes");
		}
		else Render.text("they go");
		Render.text(" to sleep.  You share a smile with your equine lover, kiss your ");
		if(flags[FlagEnum.COTTON_KID_COUNT] == 1) {
			Render.text("kid on the cheek");
		}
		else Render.text("kids on the cheeks");
		Render.text(", kiss her on the lips, and then quietly leave Cotton's house to head back to camp.");
	}
	//Little Angel
	else if(scene == 1) {
		Render.text("\n\nHowever, it turns out that ");
		if(flags[FlagEnum.COTTON_KID_COUNT] == 1) Render.text("your baby is");
		else Render.text("all your children are");
		Render.text(" fast asleep.  Rather than disturb ");
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
		else Render.text("him");
		Render.text(", Cotton simply leans over the crib, gently stroking a cheek with a soft smile on her lips.  \"<i>I'd forgotten what it was like to be a parent... it's hard work, but it's definitely rewarding.</i>\"  She gives you a sidelong look.  \"<i>I can't say that this is exactly what I expected of our little relationship... but I'm not complaining.</i>\"");
		...
		Render.text(" to get some rest together.");
	}
	//Breastfeeding
	else if(scene == 2) {
		...
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("daughter");
		else Render.text("son");
		Render.text(" starts to wail and cry, the mare looking tired.  \"<i>Oh, you're hungry again my little one?  All right, all right, mommy will get your milk, just give me a moment...</i>\"");
		if(flags[FlagEnum.COTTON_KID_COUNT] > 1) Render.text("  Fortunately, your other kids don't seem to be hungry, so it's just the one who needs feeding.");

		//(If PC lactates:
		if(player.lactationQ() >= 50) {
			Render.text("  You could probably offer to nurse your ");
			if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] != 1) Render.text("daughter");
			else Render.text("son");
			Render.text(" for the poor mare-morph, rather than letting her use a bottle, or whatever she's going to do.");
			//[Feed] [Stay Quiet]
			menu();
	...
	//Cuddlebug
		...
		Render.text(" stares up at you with eyes wide and wet, sniffling audibly as ");
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("she");
		else Render.text("he");
		Render.text(" makes tiny little sobs.  \"<i>Aw, looks like somebody was lonely,</i>\" Cotton coos, then gives you a meaningful nudge.  Realising she wants you to hold your child, you step forward and gently lift ");
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
		else Render.text("he");
		Render.text(" up into your arms, commenting on what a big, heavy ");
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("girl");
		else Render.text("he");
		Render.text("'s getting to be.  The baby is scared and hesitant, and bursts into tears at first, but you croon and shush ");
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
		else Render.text("he");
		Render.text(" gently, holding the foal close to your ");
		Render.text("[chest] and rocking ");
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
		else Render.text("him");
		Render.text(" back and forwards; soon enough, the foal's cooing and snuggling eagerly against you as you cuddle ");
		if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
		else Render.text("him");
		Render.text(".");
		...
		Render.text(" little ears flick in the most adorable manner.  So caught up are you in cuddling your child that you don't notice Cotton until she has her arms wrapped around you, leaning her head against you as she embraces the pair of you.");
...
private function feedYourCottonKids():void {
	clearOutput();
	Render.text("You interrupt Cotton by pointing out that you have breasts full of milk; you'd be happy to nurse the little ");
	if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("filly ");
	else Render.text("cotton ");
	Render.text(" for her, if she wants.");
	...
	Render.text(" to you, by which time you have already exposed your [chest].");

	Render.text("\n\nYou carefully take your child from Cotton's hands, feeling ");
	if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
	else Render.text("his");
	Render.text(" weight in your arms, before bringing ");

	if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
	else Render.text("him");

	Render.text(" in close");
	Render.text(".  At first, the baby is confused and whimpers quietly in fear, but ");

	if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
	else Render.text("his");

	Render.text(" presence and weight make your boobs start releasing their sweet cargo.  As the scent of milk fills the air, the baby's instincts take over and it brings its little face in to first nuzzle your [nipple], and then finally close ");
	if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
	else Render.text("his");
	Render.text(" lips around it.");
	...
	Render.text(" back and forth, quietly starting to sing whatever dredges of lullabies you can remember overhearing from the nursing mothers back in your own village in Ingnam.  You can feel ");
	if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
	else Render.text("his");
	Render.text(" little heart beating inside ");
	if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
	else Render.text("his");
	Render.text(" breast, and you wonder if ");
	if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("she");
	else Render.text("he");
	Render.text(" can hear your heart inside of yours.");
	...
	if(flags[FlagEnum.COTTON_KID_COUNT] == 1) Render.text(" needs");
	else Render.text("ren need");
	Render.text(" all the energy they can get.</i>\" Her hands begin to massage your shoulders and back, kneading the flesh, helping you to relax.");
	...
	Render.text(" is just a sleepy little thing who needs to go to bed, you tell her, and you gently put the baby back in ");
	if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
	else Render.text("his");
	Render.text(" crib to get some sleep.  You recover your breasts and ask if Cotton will be okay looking after them now, or if you should stick around and help out a little longer?");
...
private function letCottonFeedKids():void {
	...
	if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her down for her nap, hm pet?</i>\"");
	else Render.text("him down for his nap, hm pet?</i>\"");


	Render.text("\n\nYou nod your head to her and gently place the sleepy baby back in ");
	if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
	else Render.text("his");
	Render.text(" crib.  You ruffle ");
	if(flags[FlagEnum.COTTON_OLDEST_KID_GENDER] >= 2) Render.text("her");
	else Render.text("his");
	Render.text(" hair and tell Cotton that she makes some gorgeous little foals.  She just nods and gives your shoulder a little squeeze, content to watch in silence for a moment.");
