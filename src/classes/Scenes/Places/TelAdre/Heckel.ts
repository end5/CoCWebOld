...
public greetHeckel():void {
	...
	else if(telAdre.brooke.brookeAffection() >= 70 && flags[FlagEnum.BROOKE_MEDIUM_SCENE] > 0 && flags[FlagEnum.MET_HECKEL] > 0 && model.time.hours >= 13 && model.time.hours <= 15 && (player.lowerBody.vaginaSpot.hasVagina() || player.cockThatFits(telAdre.brooke.brookeCapacity()) >= 0)) {
...
private heckelTraining():void {
	...
	if(flags[FlagEnum.TIMES_FUCKED_HECKEL_BLOWJOB] + flags[FlagEnum.TIMES_FUCKED_HECKEL_ANAL] == 0) {
		...
		if(player.str > 50 && player.lowerBody.cockSpot.hasCock()) {
...
private heckelLovesDaAnal(loss:boolean = false):void {
	...
	if(player.analCapacity() <= 20 && !loss) {
		MainScreen.text("Heckel lets out a barking laugh when you suggest using your " + assDescript() + ", ruffling your hair playfully. \"<i>Know your limits, " + player.short + ", I'd tear you apart down there. I'm sure there are plenty of others in this realm who can help you out with that.</i>\"\n\n", false);
		doNext(camp.returnToCampUseOneHour);
		return;
	}
	//success - requires ass to be 'a little loose'
	MainScreen.text("Heckel's eyes flash as her grin splits her face, and for just a moment you wonder if this might have been a bad idea. Before you can object, however, she's pushing you down onto your stomach, your arms and " + player.legs() + " draped over either side of the wooden bench.\n\n", false);

	MainScreen.text("You half expect her to just shove herself in right there, but she thankfully holds off. Instead, you feel her rough paws grab and knead either side of your " + buttDescript() + " as she lets out a low growl.", false);
	//[<IF TONE OVER 80>
	...
	MainScreen.text("Her large tongue feels like sandpaper as it licks up and down your anus, your spread cheeks exposing you to the world. You can feel her wet nose on your sensitive skin, and the contrast with her warm tongue makes you shiver. Her tongue pokes and prods at your " + assholeDescript() + " before slipping painlessly in, swirling expertly around before pulling out and licking some more.\n\n", false);

	//[<IF PC HAS PENIS AND VAGOO> 
	if(player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(SMultiCockDesc() + " is trapped between you and the wooden bench, and you know Heckel won't let you shift position to free it. Your " + vaginaDescript(0) + " is also being ignored, and having so much pleasure with so little stimulation is driving you crazy. You hump weakly up and down, trying to draw some pleasure out of the bench beneath you while simultaneously pushing her rolling and massaging tongue closer to your aching slit, but she ignores you and only licks deeper. She buries her face in your ass entirely, her cold nose and teeth rubbing up against your sensitive skin as her tongue pushes as far inside of you as she can get it.\n\n", false);
	
	//[<IF PC HAS PENIS>
	else if(player.lowerBody.cockSpot.hasCock()) MainScreen.text(SMultiCockDesc() + " is trapped between you and the wooden bench, and you know Heckel won't let you shift position to free it. You hump weakly at the bench in a desperate attempt to get some stimulation, and you can feel the vibrations of Heckel's laugh as she buries her snout in your ass. She doesn't stop you, but the polished wood barely gives any pleasure anyway.\n\n", false);
...
private heckelLovesAnal2():void {
	...
	//[<IF PC HAS PENIS AND VAGOO>
	if(player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("Although " + sMultiCockDesc() + " is trapped beneath you, Heckel's balls slap into your cunt with every one of her quickening thrusts.  Although neither your " + multiCockDescriptLight() + " or " + vaginaDescript(0) + " are getting much stimulation, your prostate is being milked better than it ever has. One thrust in particular makes you cry out, and Heckel laughs above you as she slams her furry thighs into yours again. \"<i>You moan like a whore, " + player.short + ", a perfect bitch for me to ride.</i>\"\n\n", false);

	//[<IF PC HAS PENIS AND BALLS> 
	else if(player.lowerBody.cockSpot.hasCock() && player.lowerBody.balls > 0) MainScreen.text(SMultiCockDesc() + " pulses beneath you with every thrust, her duo of fist-sized balls slapping into your " + ballsDescriptLight() + ". You gasp in surprise as one of her thrusts rubs against your prostate, and she barks out a single laugh above you. \"<i>You moan like a whore, " + player.short + ", a perfect bitch for me to ride.</i>\"\n\n", false);
	...
	//[<MALE PC>
	else if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("Heckel wastes no time before going back to her brutal fucking, though her thrusts are limited to short bursts now. She makes up for this in speed as you lie helplessly below her, the pain of taking her knot slowly turning into intense pleasure. The heat and pleasure spreads from your ass to your " + ballsDescriptLight() + " and cock, making it twitch at your impending orgasm. Heckel cries out above you and slams her hips into you as much as she can, and you immediately feel a burning heat shooting up your gut. She pumps again as she seeds your insides with another spurt, and this time you orgasm along with her.  Your " + ballsDescriptLight() + " get tight as your trapped " + Appearance.cockNoun(CockType.HUMAN) + " twitches, shooting your cum up between your stomach and chest. Your legs quiver beneath hers as the warmth spread further up your gut, leading you into a warm and hazy afterglow.\n\n", false);
	//[<FEMALE PC>
	else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("Heckel wastes no time before going back to her brutal fucking, though her thrusts are limited to short bursts now. She makes up for this in speed as you lie helplessly below her, the pain of taking her knot slowly turning into intense pleasure. The heat and pleasure spreads from your ass to your entire groin, and the knot pushes against the walls of your " + vaginaDescript() + " hard enough to make them rub against each other. Heckel cries out above you and slams her hips into you as much as she can, and you immediately feel a burning heat shooting up your gut. She pumps again as she seeds your insides with another spurt, and this time you orgasm along with her. Your " + vaginaDescript(0) + " spasms and gushes over your soaked thighs, your asshole spasming around her cock just as much. Your legs quiver beneath hers as the warmth spread further up your gut, leading you into a warm and hazy afterglow.\n\n", false);
...
public dominateHeckel():void {
	...
	if(flags[FlagEnum.TIMES_DOMMED_HECKEL] == 0) MainScreen.text("there's a new top dog just waiting to claim the town bitch");
	...
	if(flags[FlagEnum.TIMES_DOMMED_HECKEL] > 0) MainScreen.text("last time was a fluke");
	...
	if(flags[FlagEnum.TIMES_LOST_HECKEL_DOM_CHALLENGE] > 0) MainScreen.text(" again");
	MainScreen.text("?</i>\"  Her tough finger-tip slams into your solar plexus for emphasis");
	if(player.tou < 60) MainScreen.text(", pushing you back a bit");
	MainScreen.text(".");
	...
	if(player.str < 75) {
		...
		flags[FlagEnum.TIMES_LOST_HECKEL_DOM_CHALLENGE]++;
	}
	//{Win}
	else {
		...
		if(flags[FlagEnum.TIMES_DOMMED_HECKEL] > 0) MainScreen.text(" AGAIN");
		MainScreen.text("...</i>\"");
		MainScreen.text("\n\nTime to claim your prize...");
		dynStats("str", .5);
		menu();
		MainScreen.addButton(0,"Next",dominateHeckelConclusion);
		flags[FlagEnum.TIMES_DOMMED_HECKEL]++;
	}
}
...
private dominateHeckelConclusion():void {
	...
	if(flags[FlagEnum.PC_FETISH] > 0) MainScreen.text("You grin and blush hotly, imagining the whole of the gym watching you fuck this hyena, their watchful eyes exciting you further.");
	...
	MainScreen.text(".  You give her a knowing smile, " + player.mf("man","'woman'") + " to woman, and rock forward, sliding through her puffy labia and into her silky canal.  Those wet hyena-lips cling to every vein and rigid nook of your " + cockDescript(x) + ", slurping your penis down the slick tunnel with hungry undulations.");
	...
	if(silly()) {
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00281] > 0) MainScreen.text(", then give Lottie a high five");
