...
Render.text("Whitney marches up to you as soon as you approach the farm, a stoic expression plastered across her face.");
	...
	if(player.statusAffects.get("MetWhitney").value1 < 2) {
		...
		if(player.findStatusAffect(StatusAffects.MetWhitney) < 0) {
			...
			Render.text("You find a quaint farmhouse on the far shores of the lake.  Around the homestead are a range of gardens, filled with delicious fruits and vegetables.  Your belly rumbles, aching with hunger, as you approach the dwelling.  A figure in a pepper patch rises up to greet you, waving you over.\n\nYou do your best to conceal your surprise as you realize the farmer is a woman... with fur and canine-like features!  She giggles happily and beckons you over, \"<i>Welcome stranger, it sure is pleasant to see a new face 'round here.  My name's Whitney, and it's mighty fine I don't have to pitchfork you like most guests!</i>\"  She fills you in about the lake and her farm, telling you how the demons can't seem to stay close for long, and monsters always seem weaker the few times they have approached her farm.  Whitney flushes and rapidly changes subject, \"<i>I've got to get back to work, but you help yourself to the peppers, hun!</i>\"\n\n", true);
		}
		else {
			Render.text("You stumble across Whitney's farm again.  The anthropomorphic canine woman gives you a friendly wave and tosses you another Canine pepper.\n\n", true);
			player.addStatusValue(StatusAffects.MetWhitney,1,1);
			if(player.statusAffects.get("MetWhitney").value1 == 2) Render.text("<b>You've been to the farm enough to easily find it.  You can return by selecting it from the places menu (and will no longer encounter it during random lake exploration)</b>.\n\n", false);
		}
	...
	//Repeat Offender
		...
		if(flags[FlagEnum.KELT_BREAK_LEVEL] >= 4 && flags[FlagEnum.WHITNEY_FLIPPED_OUT_OVER_KELLY] == 0) {
			clearOutput();
			Render.text("As soon as you head to the farm, Whitney comes storming up to meet you.  \"<i>What in tarnation do you think you're pulling?!</i>\"");
		...
		else if(flags[FlagEnum.WHITNEY_FLIPPED_OUT_OVER_KELLY] == 1) Render.text("You aren't welcome on the farm proper, but you can see Kelly cantering about the fields, looking for you.", true);
		else Render.text("Whitney's farm is remarkably large for such a humble operation.  What do you want to do?", true);
		menu();
		...
		if(player.statusAffects.has("Kelt") && player.findStatusAffect(StatusAffects.KeltOff) < 0) {
			if(flags[FlagEnum.KELT_BREAK_LEVEL] >= 4) addButton(1,"Kelly",kelly.breakingKeltOptions);
		...
		if(player.hasKeyItem("Breast Milker - Installed At Whitney's Farm") >= 0) {
			...
			else if(flags[FlagEnum.WHITNEY_FLIPPED_OUT_OVER_KELLY] == 0) addButton(2,"Get Milked",getMilked);
			
		}
		if(player.hasKeyItem("Cock Milker - Installed At Whitney's Farm") >= 0 && player.lowerBody.cockSpot.count() > 0)
		{
			
			if(flags[FlagEnum.WHITNEY_FLIPPED_OUT_OVER_KELLY] == 0) addButton(5,"Milk Cock",cockPumping);
		}
		if(player.findStatusAffect(StatusAffects.MarbleRapeAttempted) < 0 && player.findStatusAffect(StatusAffects.NoMoreMarble) < 0 && player.statusAffects.has("Marble") && flags[FlagEnum.MARBLE_WARNING] == 0) {
			...
			if(flags[FlagEnum.WHITNEY_FLIPPED_OUT_OVER_KELLY] == 0) addButton(3,"Marble", meetMarble);
		}
		...
		if(flags[FlagEnum.WHITNEY_FLIPPED_OUT_OVER_KELLY] == 0) addButton(0,"Explore",exploreFarm);
		if(flags[FlagEnum.WHITNEY_FLIPPED_OUT_OVER_KELLY] == 0) addButton(6,"Talk",talkWhitney);
		if(flags[FlagEnum.WHITNEY_FLIPPED_OUT_OVER_KELLY] == 0) addButton(7,"Work",workFarm);
...
private talkWhitney():void {
	...
	if (flags[FlagEnum.MURBLE_FARM_TALK_LEVELS] > 0 && player.statusAffects.has("Kelt") && flags[FlagEnum.WHITNEY_TALK_MURBLE_AND_KELT] == 0)
	{
		...
		Render.text("You find Whitney in her usual spot underneath her tree, nose in book. She smiles at you distractedly as you approach.");
	...
	if (player.statusAffects.has("TelAdre") && player.statusAffects.get("TelAdre").value1 >= 1 && flags[FlagEnum.WHITNEY_TALK_TELADRE] == 0 && flags[FlagEnum.WHITNEY_TALK_MURBLE_AND_KELT] == 1)
	{
		...
		Render.text("You find the dog woman sitting on a stool, engaged in hand-milking a cow. She looks up sharply at your approach but smiles readily enough when she sees it’s you.");
	...
	if (flags[FlagEnum.DISCOVERED_HIGH_MOUNTAIN] > 0 && flags[FlagEnum.WHITNEY_TALK_HIGH_MOUNTAIN] == 0 && flags[FlagEnum.WHITNEY_TALK_TELADRE] == 1)
	{
		...
		Render.text("You find Whitney outside the milking shed, carefully writing labels at a trestle table and sticking them on large bottles of milk.");
		//[PC has used milking device:
		if (player.statusAffects.has("BreastsMilked"))
		{
			Render.text(" You are uncomfortably aware of the number of them which are labelled ‘[name]’, and a charged memory of strong suction on your [nipples] comes back to you.");
		}

		if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00331] > 0 && flags[FlagEnum.ISABELLA_MILKED_YET] != -1)
		{
			// Jojo only
			Render.text(" At the far end there is a small alabaster cluster labelled ‘Jojo’.");
		}
		else if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00331] == 0 && flags[FlagEnum.ISABELLA_MILKED_YET] == -1)
		{
			// Isabella Only
			Render.text(" At the far end there is a small alabaster cluster labelled ‘Isabella’.");
		}
		else if (flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00331] > 0 && flags[FlagEnum.ISABELLA_MILKED_YET] == -1)
		{
			// Both
			Render.text(" At the far end there is one alabaster cluster labelled ‘Jojo’, another labelled ‘Isabella’.");
		}
			
		Render.text(" You ask her who she sells it all to.");
		...
		{
			if (player.findStatusAffect(StatusAffects.BreastsMilked) < 0) Render.text("\n\n");
			
			Render.text("You notice a number of smaller bottles filled with a creamy fluid on the table, arranged in a cargo container. It takes you a moment to realize what it is. “<i>Why d’you think I pay you for it?</i> ” says Whitney with a laugh, catching your expression. “<i>I kin use some of it for my herd, but it’s just as easy to sell it to goblins ‘n harpies. Much better to buy it from me than to waste energy catching and beating it out of a satyr. 'Sides, how'd ya think I kept my hair so luxurious? Goblin hairdressers are top notch.</i>”");
		}
	...
	if (flags[FlagEnum.WHITNEY_TALK_DUNGEON_FOLLOWUP] == 0 && flags[FlagEnum.WHITNEY_TALK_DUNGEON] == 1)
	{
		...
		Render.text("You find Whitney hard at work in the pepper patch. You approach her cautiously, but when she sees you she hails you brightly.");
	...
	temp = rand(6);
	if(temp == 0) Render.text("It doesn't take long to find the independent farmer-girl.\n\n", false);
	if(temp == 1) Render.text("She's patrolling the edges of her farm with a wicked-looking scythe in hand.  She nods to you as you approach and fall in beside her.\n\n", false);
	if(temp == 2) Render.text("She's bent over in the pepper fields, pulling weeds left and right.  She stands up straight to wipe sweat from her fur and muzzle, and she gives you a friendly wave, encouraging you to come over and talk while she works.\n\n", false);
	if(temp == 3) Render.text("She's behind the barn, working a forge and repairing a few damaged farming tools.  Though her attention is focused on the metal on the anvil and the hammer in her hand, Whitney immediately turns and greets you, in between the blows of her hammer.\n\n", false);
	if(temp == 4) Render.text("She's rounding up a small herd of normal-looking cows.  Amazingly she has chosen to do so on foot, but is quick enough to keep up and corral her beasts.  Thankfully she's in the process of closing the gate to their pen when you finally catch up to her.  Whitney gives you a friendly smile as you come up to her and the two of you begin chatting immediately.\n\n", false);
	if(temp == 5) Render.text("She's leaning back against a thick tree with a wide-brimmed hat drooped low over her eyes.   You call out to her, thinking the dog-woman has fallen asleep, but her head snaps up and her alert eyes lock on to you immediately.  Maybe she wasn't dozing.  She calls out, \"<i>Come on over and 'ave a sit, I'm starved fer company!</i>\"  You settle in for a chat.\n\n", false);
	...
	if(rand(4) == 0 && player.hasKeyItem("Breast Milker - Installed At Whitney's Farm") < 0) {
		if(player.hasKeyItem("Breast Milker") >= 0) {
			Render.text("Before you can say much of anything, Whitney exclaims, \"<i>My stars! Is that one of them demon's milking machines?</i>\"\n\n", false);
			Render.text("You nod and tell her how you liberated it from the demonic factory and explain that even though it should be fully functional, it'll need to connect to some other machinery to work, and it's way more than any one person could handle.\n\n", false);
			...
			Render.text("Do you give the breast milker to Whitney for her to hook up?", false);
			doYesNo(whitneyMilkerHookup,whitneyMilkerRefusal);
			return;
		}
		else if(player.biggestLactation() >= 2) {
			Render.text("Whitney gives you a bemused look when you settle down for a chat.\n\n", false);
			Render.text("\"<i>Ya might wanna get that looked at darlin',</i>\" she says, gesturing at milky wetness dripping from the front of your " + player.armorName + ".\n\n", false);
			if(player.stats.cor < 33) Render.text("You blush with shame", false);
			else if(player.stats.cor <= 66) Render.text("You flush with a touch of exhibitionism", false);
			else Render.text("You flush hotly and arch your back, openly displaying your milk to the dog-girl", false);
			Render.text(" as her words sink in.   Runners of milk leak down your " + allBreastsDescript() + ", released by the stress of being so exposed.   In no time flat you're soaked in milk.\n\n", false);
			Render.text("Whitney starts to giggle, but immediately stifles it, apologizing, \"<i>Ah'm sorry, I didn't mean nothing by it.  I know a few folks who LOVE being a walking milk-fountain.  If ya like, I could probably get you set up with your own milking equipment.   You'd be able to keep up with the... fluid accumulations that seem to be troubling you.   I'd even be able to toss you a few gems if you produce enough.</i>\"\n\n", false);
			Render.text("It almost sounds too good to be true.   The farmer-girl nods, reading your expression quite clearly, \"<i>Yes, there is a bit of a catch.  I'll need 250 gems for the parts to get this all set up.   Equipment like this isn't cheap.   Whaddya say, hun?   I understand if you don't want to – you can always just wait for the milk to stop.</i>\"\n\n", false);
			if(player.stats.gems >= 250) {
				Render.text("Do you purchase a breast-milker from Whitney for 250 gems?", false);
				doYesNo(breastMilkerPurchase, breastMilkerNoPurchase);
			}
			else {
				Render.text("You don't have enough money for the milker.  You apologize and head back to camp, maybe you can get one later.", false);
				doNext(camp.returnToCampUseOneHour);
			}
			return;
		}
	...
	if(rand(4) == 0 && player.hasKeyItem("Cock Milker - Installed At Whitney's Farm") < 0 && player.hasKeyItem("Cock Milker") >= 0) {
		Render.text("Before you can say much of anything, Whitney exclaims, \"<i>My stars! Is that one of them demon's milking machines?</i>\"\n\n", false);
		Render.text("You nod and tell her how you got it and explain that even though it should be fully functional, it'll need to connect to some other machinery to work, and it's way more than any one person could handle.\n\n", false);
		...
		Render.text("Do you give the cock milker to Whitney for her to hook up?", false);
		doYesNo(createCallBackFunction(whitneyMilkerHookup,false),whitneyMilkerRefusal);
		return;
	}
	//[GENERIC TALK]
	Render.text("You tell her of your recent trials and tribulations ", false);
	if(player.stats.cor > 50) Render.text("or at least the parts you think she would want to hear ", false);
	Render.text("and she listens attentively, chiming in with witty quips and comfort when appropriate.  When you finish she tells you ", false);
	if(player.findStatusAffect(StatusAffects.FactoryOverload) < 0) Render.text("how well the farm has been going", false);
	else Render.text("how poorly the farm has been going since the lake became tainted.  She has to work three times as hard to keep her livestock and crops from succumbing to the taint, and the demons and monsters of the forest are many times more bold", false);
	Render.text(".  It feels good to get a chance to talk with another sane individual, but before long Whitney has to return to work, and you should check back on your camp.", false);
...
public workFarm():void {
	...
	if(rand(4) == 0) {
		spriteSelect(62);
		Render.text("You find Whitney getting a scythe out of her tool shed. \"<i>Do you know how to muck out a stable?</i>\" she asks when you offer to help. You admit that you did a lot of that while growing up in your village. After passing you a rake, shovel, and pitchfork, she leads you to the milking barn.", true);
		Render.text("  The first thing that hits you is the smell, a mingling of sweat, milk, droppings, and rotting hay. There are also probably some cows in Whitney's herd ready for breeding.\n\n", false);
		Render.text("Opening the door to one of the empty stalls, Whitney says, \"<i>I don't get to them as often as I should. Anything you can do would help.</i>\"\n\n", false);
		Render.text("You steel yourself, ignore your ", false);
		if(player.faceType == FACE.DOG) Render.text("sensitive ", false);
		Render.text("nose, and set to work.", false);
	...
	spriteSelect(62);
	Render.text("You ask Whitney if she could use help with anything and she points towards the pepper fields, \"<i>Ya mind gathering up some peppers for an hour or two?  I'm gonna need a few for supper tonight.  I'll even let you keep the best one!</i>\"\n\n", false);
	Render.text("You nod and borrow a basket, and set off towards the fields.  The next two hours are a blur of sweat and hard work as you prowl between the rows of plants, picking as many ripe red peppers as you can find.  When you finish, you drop the basket by Whitney's door, but not before taking your pepper.\n", false);
...
public exploreFarm():void {
	...
	if(explore == 0) {
		spriteSelect(62);
		Render.text("You run around the farm, keeping an eye for any monsters or oddities around Whitney's property.  Eventually the she-dog joins you, and the two of you have a good time pushing your speed to its limits.  ", true);
		//Less than 30 speed (+2 speed)
		if(player.stats.spe < 30) {
			dynStats("spe", 2);
			Render.text("Whitney easily outpaces you, leaving you so far behind that she laps around the farm twice for each pass you make.", false);
		}
		//Less than 50 speed (+1 speed)
		else if(player.stats.spe < 50) {
			dynStats("spe", 1);
			Render.text("Whitney is still faster than you, and manages to get far enough ahead of you to disappear from time to time.", false);
		}
		//Less than 70 speed (+.75 speed)
		else if(player.stats.spe < 70) {
			dynStats("spe", .75);
			Render.text("Whitney and you are evenly matched, and the two of you run together for a while, each pushing yourself harder in an effort to best the other.", false);
		}
		//Else (+.5 speed)
		else {
			dynStats("spe", .5);
			Render.text("Whitney falls behind, unable to cope with your speed as you tear around the farm.", false);
		}
	...
	//Other stuff
	if(explore == 1) {
		Render.text("After wandering around for a while, you find yourself atop a slight rise looking out over the farm and the distant lake. Despite the corruption you know is slowly consuming this land, being here now makes you feel so at peace you wish it could go on forever.", true);
		dynStats("cor", -rand(3));
		doNext(camp.returnToCampUseOneHour);
		return;
	}
	//Cows
	if(explore == 2) {
		Render.text("Your explorations take you to the cow pasture. There's no bull here, so the cows are all placidly grazing, building up milk for Whitney to gather. One turns to face you, and you get the odd feeling that it's trying to tell you something.\n\n", true);
		//[if have a horse dick and rape-high Lust] 
		if(player.horseCocks() > 0) {
			Render.text("Unbidden, the notion that a cow-slit would probably feel pretty good wrapped around your horse-prick flickers through your mind.  ", false);
			//[if high corruption]
			if(player.stats.cor > 60) Render.text("It makes you smile.", false);
			if(player.stats.cor < 30) Render.text("It disgusts you.", false);
			//[else/mid-corruption] 
			else if(player.stats.cor <= 60) Render.text("You aren't sure how you feel about that.", false);
			//[continue paragraph condition] 
			Render.text("  It for certain would get Whitney chasing you off with a pitchfork.\n\n", false);
			dynStats("lus", 10);
		}
		//[if no horse dick, a deep cow or horse vag, and in heat]
		else if(player.inHeat) {
			Render.text("Maybe it wants a bull? You do, one with long, thick dick-meat to satisfy your starving fuck-hole...\n\n", false);
			dynStats("lus", 15);
		}
		//- [if no dick, not in heat, but overfull with milk]
		else if(player.biggestLactation() >= 2) {
			Render.text("\"<i>Maybe she wants to be milked?</i>\" you think. You certainly do.\n\n", false);
			dynStats("lus", 3);
		}
		Render.text("Shaking your head, you clear your thoughts and turn away from the pasture. Cows don't have your problems.", false);
		doNext(camp.returnToCampUseOneHour);
		return;
	}
	if(explore == 3) {
	...
	//[NOTHING]
		...
		Render.text("You wander around, unable to find anything entertaining on this patch of rural bliss.", true);
		doNext(camp.returnToCampUseOneHour);
	}
}
...
public getMilked():void {
	...
	Render.text("The barn looms tall ahead of you as you step into its shadow.  ", false);
	if(player.findStatusAffect(StatusAffects.BreastsMilked) < 0) {
		if(player.stats.cor < 50) Render.text("You shiver nervously when you step inside.", false);
		else Render.text("You smile eagerly as you walk inside.", false);
		Render.text("  The barn is filled with the earthy smells of earth, wood, and grease.  It's clean for the most part, though the floor is just packed dirt and the stalls look old and well-used.  A bank of machinery along the wall thrums and pulses as if it's a living creature.  Hoses and cables run from it in a dozen places, disappearing into the walls.   There is even a set of stout wooden doorways along the west wall.  That must be where the farm's intelligent denizens stay.  You notice each of the stalls have name-plates on them, and there is even one that says " + player.short + ".  It must be for you.\n\n", false);
	}
	//Repeat
	else {
		Render.text("You walk over to the barn, eagerly anticipating the opportunity to get milked.", false);
		//If ilk withdrawl or high lactation no dicks
		if(player.statusAffects.has("LactationReduction") && player.totalCocks() == 0) Render.text("  Your " + nippleDescript(0) + "s are engorged and ready to be taken care of.", false);
		//If cocks
		else if(player.totalCocks() > 0) {
			Render.text("Your " + multiCockDescriptLight() + " erect", false);
			if(player.totalCocks() > 1) Render.text("s", false);
			Render.text(" and throb", false);
			if(player.totalCocks() == 1) Render.text("s", false);
			Render.text(" with desire.", false);
		}
		//If both
		if(player.statusAffects.has("LactationReduction") && player.lowerBody.cockSpot.count() > 0) {
			Render.text("Your " + nippleDescript(0) + "s and " + multiCockDescriptLight() + " grow", false);
			Render.text(" hard and ready of ", false);
			Render.text("their", false);
			Render.text(" own volition.", false);
		}
		Render.text("  The doors part easily, and you breeze into your stall in a rush.\n\n", false);
	}
	//Step into harness – first time only
	if(player.findStatusAffect(StatusAffects.BreastsMilked) < 0) {
		Render.text("A harness hangs limply in the stall, there to hold the occupant in place while they are milked of every last drop.  You exhale slowly and force yourself to step into it.  As you puzzle out the straps, it gets easier and easier to get the rest of the harness into place.  As you snap the last one into position, machinery whirs and pulls it tight, lifting you off the ground and suspending you, facedown.  The breast milk pumps pulse and vibrate on a tray below you, twitching slightly as you hear the machinery activate.\n\n", false);
	}
	//REPEAT
	else {
		Render.text("You easily attach the harnesses and lift up into position, hearing the machinery activate automatically. ", false);
	}
	...
	if(player.findStatusAffect(StatusAffects.BreastsMilked) < 0 || application == 0) {
		...
		Render.text("You manage to grab the suction cups in spite of your constrictive bindings and pull them to your " + nippleDescript(0) + "s.  They latch on immediately, ", false);
		if(player.upperBody.chest.BreastRatingLargest[0].nippleLength <= 1.5) Render.text("pulling each of your nipples entirely into the suction-tubes.  ", false);
		else Render.text("struggling to fit around each of your nipples as they slide into the suction-tubes.  ", false);
		Render.text("There is a mechanical lurching noise as the suction builds rapidly.  Your nipple swells out to " + int(player.upperBody.chest.BreastRatingLargest[0].nippleLength*1.5*10)/10 + " inches of length, turning purplish from the strain.   You can feel something welling up inside your " + allBreastsDescript() + ", building as it moves towards your " + nippleDescript(0) + "s.\n\n", false);
	}
	//Apply repeat alternate
	else if(application == 1) {
		Render.text("You stretch down and grab onto the suction cups, pulling them up to your eager nipples.  They latch on, slapping tight against you as the vacuum pressure seals them tightly against your body.  You can feel your " + nippleDescript(0) + "s pulling tight, nearly doubling in size from the intense pressure.  ", false);
		if(player.upperBody.chest.BreastRatingLargest[0].nippleLength >= 3) Render.text("They nearly burst the tubes designed to milk them by virtue of their sheer size.  ", false);
		Render.text("The sensitive flesh of your " + allBreastsDescript() + " fill with a burgeoning pressure that centers around the tubes connected to your nips.\n\n", false);
	}
	//Version 3
	...
	dynStats("lus", 40);
		...
		if(milksplosion == 1) {
			...
			Render.text(" with excitement.  The milking drags on for an hour, but your output only slows slightly, forcing the machinery to work at maximum capacity the entire time.  At last it ends, and the harnesses lower you to the ground.  The milk cups pop off, leaving your leaky tits to make a puddle on the floor.\n\n", false);
		}
		//High Output3
		if(milksplosion == 2) {
			Render.text("Milk floods the milker's cups as your breasts respond to the mechanized suckling.   The machinery groans as it kicks into high gear, working hard to keep up with your prodigious production rate.  Your nipples tingle with happy little bursts of pleasure as they continue to pour out ever greater quantities of milk.  Arousal wells up, flushing your body with a reddish tint that's difficult to hide.  You wriggle in the harness, sweating profusely and trying to grind against something, anything, whatever it takes to get off.  The milking drags on for an hour, but your breasts keep pouring out milk the entire time.  When it ends, you're lowered to the floor and released.  The milk-tubes pop off, leaving you lying in a milk-puddle as your leaky teats continue to drip.\n\n", false);
		}
	}
	//CRAZY OUTPUT1 (+60 lust)
		...
		milksplosion = rand(2);
		if(milksplosion == 0) {
			Render.text("Your " + nippleDescript(0) + "s twitch and pulse for but a moment, then unleash a torrent of milk, totally filling the tubes.  The machinery lurches, struggling to keep up as you flood the tubes.   An alarm starts blaring as milk begins leaking out around the edges – Whitney's machinery just can't keep up!  You can hear footsteps in the barn, and a pair of soft hands hold the cups against your chest.   The machinery is shut down, but another pair of hands begins massaging your " + allBreastsDescript() + ", pumping wave after wave of milk through the tubes, unaided by the machinery.  You practically ", false);
			if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("cream yourself", false);
			else if(player.lowerBody.cockSpot.count()) Render.text("jizz yourself", false);
			else Render.text("orgasm", false);
	...
	Render.text("The machinery displays " + liters + " liters of milk", false);
	//If already at cap
	if(flags[FlagEnum.WHITNEY_GEMS_PAID_THIS_WEEK] >= cap) {
		Render.text(" and displays a warning that <b>you're producing more than Whitney can pay for</b>", false);
		payout = 0;
	}
	if(payout > 0) {
		...
		Render.text(" and automatically dispenses " + num2Text(payout) + " gem" + (payout == 1 ? "" : "s") + ".  Whitney really went all out with this setup!", false);
		//Display a warning that you've capped out.
		if(flags[FlagEnum.WHITNEY_GEMS_PAID_THIS_WEEK] >= cap) Render.text("  <b>The machinery warns you that Whitney can't afford any more this week!</b>", false);
	...
	if(liters > 2) {
		...
		else if(player.stats.int < 15) Render.text("  You stretch and let out a contented moo, long and loud.  How silly!", false);
		else if(player.stats.int < 25) Render.text("  You quietly moo, then giggle to yourself at how strange you're acting.", false);
	}

	Render.text("\n\n", false);
	//Not very horny yet
	if(player.lust < 75) {
		Render.text("Feeling sore and VERY hungry, you make yourself decent and stagger back towards camp, ignoring the knowing smile Whitney gives you when you pass by her.", false);
	}
	//Horny
	else {
		Render.text("Overwhelmed with your desire, you don't even bother to cover up and make yourself decent, you just run out of the barn, " + allBreastsDescript() + " jiggling and wet, heading straight for camp.");
		if (farmCorruption.whitneyCorruption() < 90) Render.text(" It isn't until you get back that you remember the disapproving look Whitney gave you, but if anything, it only makes you hornier.", false);
		dynStats("lus=", 100);
	}
...
public cockPumping():void {
	...
	//Repeat
		...
		Render.text("You walk over to the barn, eagerly anticipating your session with the 'milker'.  ", false);
		Render.text("Your " + multiCockDescriptLight() + " erect", false);
		if(player.lowerBody.cockSpot.count() == 1) Render.text("s", false);
		Render.text(" and throb", false);
		if(player.lowerBody.cockSpot.count() == 1) Render.text("s", false);
		Render.text(" with building desire.  It's as if ", false);
		if(player.lowerBody.cockSpot.count() == 1) Render.text("it", false);
		else Render.text("they", false);
		Render.text(" know", false);
		if(player.lowerBody.cockSpot.count() == 1) Render.text("s", false);
		Render.text(" all too well what's to come.  The doors part easily, and you breeze into your stall in a rush.\n\n", false);
	}
	//First time using cock milker – factory install
	if(player.findStatusAffect(StatusAffects.CockPumped) < 0) {
		Render.text("<b>There is a note tacked to the door from Whitney:</b>\n\n<i>I got your machine all rigged up, and even made a few modifications to bring it up to speed with the machinery I use on the bulls.  Enjoy your 'milkings', and don't overdo it – there are certain messes even a farm-girl like me WON'T clean up.</i>\n\n", false);
		//Step into harness – first time only
		Render.text("A harness hangs limply in the stall, there to hold the occupant in place while they are drained of every ounce of 'milk'.  You exhale slowly and force yourself to step into it.  As you puzzle out the straps, it gets easier and easier to get the rest into place.  You snap the last one in place, and machinery whirs and pulls them tight, lifting you off the ground and suspending you, facedown.  A hatch in the far wall slides open, revealing a mass of tubes, wires, and machinery.  A ", false);
		if(player.lowerBody.cockSpot.count() == 1) Render.text("single hose wriggles out from the hole, squirming across the floor.  It stops and lifts up, slowly homing in your horny shaft.\n\n", false);
		else if(player.lowerBody.cockSpot.count() == 2) Render.text("pair of hoses wriggle out from the hole, squirming across the floor.  They stop and lift up, slowly homing in your horny shafts.\n\n", false);
		else Render.text("group of hoses wriggle out from the hole, squirming across the floor.  They stop and lift up, slowly homing in your horny shafts.\n\n", false);
		player.statusAffects.add(new StatusAffect("CockPumped",1,0,0,0)));
	}
	//REPEAT
	else {
		Render.text("You easily attach the harnesses and lift up into position, hearing the machinery activate automatically.  The hungry ", false);
		if(player.lowerBody.cockSpot.count() == 1) Render.text("hose wriggles out from the wall, curving up towards your crotch. ", false);
		else Render.text("hoses wriggle out from the wall, curving up towards your crotch. ", false);
	...
	//Massive Scene – else
		...
		if(player.lowerBody.cockSpot.count() == 1) {
			...
			Render.text("Hurried footsteps enter the barn, and a pair of firm hands grab onto your bouncing " + multiCockDescriptLight() + ", pulling the milkers back against your groin and splattering spunk around the seal from the added pressure.  The machine-suction totally cuts off, but you don't even care.  You just keep cumming, pumping the thick white cream down the tubes by the force of your orgasm alone.   Eventually your incredible orgasm winds down, and the hands release you, allowing the tubes to pop off.  They slowly retracts to the wall as your harness is lowered, dropping your exhausted body in a wet heap on the ground.  When you regain your senses, your mysterious helper is gone.\n\n", false);
		}
		//(+40 fatigue)
		fatigue(40);
	}
	//EPILOGUE
	Render.text("There's a ", false);
	if(player.cumQ() < 20) Render.text("barely noticeable ", false);
	else Render.text("thick ", false);
	Render.text("trail of your cum along the floor where the hose dragged itself back into the wall, though the machinery has closed back up and is now chugging noisily, clearly doing something.  A loud 'ding' chimes and a panel displays ", false);
	...
	cumQ = int(cumQ);
	if(cumQ > 100) Render.text(cumQ/1000 + " Ls.  ", false);
	else Render.text(cumQ + " mLs.  ", false);
	...
	if(payout > 0) {
		...
		if (payout == 1) Render.text(Num2Text(payout) + " gem rolls ", false);
		else Render.text(Num2Text(payout) + " gems roll", false);
	
	Render.text("out into a collection plate.  Whitney really put a lot of work into this! ", false);
	...
	if (player.countCockSocks("gilded") > 0) {
		...
		Render.text("ou see a few sparkling gems in your trail of cum on the floor. You reach down and pick up all " + gems + " of them</b>, and then you are", false);
	...
	Render.text(" on your way, whistling happily and feeling like taking a nap.", false);
	player.orgasm();
	doNext(camp.returnToCampUseOneHour);
}
...
private milkerBadEnd1():void {
	...
	Render.text(".  As before, the machine works you on and off for roughly an hour before the wall light once again turns green and the suction on your " + multiCockDescriptLight() + " intensifies.  Your hips take over as you buck against the machine and push out another ", false);
	if(cumQ < 100) Render.text("trickle", false);
	else if(cumQ < 600) Render.text("stream", false);
	else Render.text("torrent", false);
	Render.text(" of jism.", false);
	//[(big skeet)
	if(cumQ > 1000) {
		Render.text("  Unlike before, a fleshy valve flap pops open in the rear of the suction cup", false);
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s", false);
		Render.text(", and the pressure of your ejaculation forces squirts of cum out of the valve with every spasm, dribbling sticky whiteness back onto your crotch.", false);
	}
	Render.text("  The orgasm winds down but still the suction and the massage do not.\n\n", false);
	
	Render.text("Another 'ding'", false);
	//[(payment-worthy skeet)
	if(cumQ > 100) Render.text(" and another clatter of gems", false);
	Render.text(" reaches your ears as your second deposit is massed, but your attention is drawn to the new tube", false);
	...
	Render.text(" snaking out of the wall toward you.  Spent as you are, you can only manage a token resistance as ", false);
	if(!tubes) Render.text("it", false);
	else Render.text("one of them", false);
	Render.text(" inserts a rod-like end into your mouth and begins trickling salty fluids down your throat.", false);
	//[(if vag)  
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("  Another tube, likewise capped with a thick, firm end instead of a suction cup, reaches your " + vaginaDescript(0) + " and pushes inside, secreting more of the fluid.  ", false);
		player.cuntChange(15,true);
		//[(bigclit)
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) Render.text("A smaller suction tube fastens over your " + clitDescript() + " as it rouses from the vaginal stimulation.", false);
	}
	//[(lactation, no nipplecunts)
	if(player.biggestLactation() >= 1 && !player.upperBody.chest.hasFuckableNipples()) {
		Render.text("  Some tubes of the cup-type latch onto your " + nippleDescript(0) + "s, then begin suction of their own. You feel your breastmilk shift toward the nipples, then begin to dribble into the tubes.", false);
	}
	//[(nipplecunts)
	if(player.upperBody.chest.hasFuckableNipples()) {
		Render.text("  More tubes snake up to your " + biggestBreastSizeDescript() + " and work their way into your " + nippleDescript(0) + "s.", false);
	}
	Render.text("  " + SMultiCockDesc() + " becomes enormously hard again in a matter of seconds as you absorb the secretions.  You quickly deduce what it is you're tasting; the demon-designed tubing is feeding your own semen back into you, laced with some kind of chemical!\n\n", false);
	
	Render.text("Under the effects of this new drug, " + sMultiCockDesc() + " inflates, engorging with blood until it's much larger than usual", false);
	//[(balls)
	if(player.lowerBody.balls > 0) Render.text(" and your " + sackDescript() + " stretches as your " + ballsDescriptLight() + " nearly double in size", false);
	Render.text(".  Another orgasm passes, this time unhindered as the machine does not slow its efforts in the slightest.  More than twice the previous quantity of semen shoots out of your cock", false);
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s", false);
	Render.text(", thanks assuredly to the drug", false);
	//[(balls)
	if(player.lowerBody.balls > 0) Render.text(" and the new volume of your monstrous testicles", false);
	//[(big skeet)
	if(cumQ > 1000) Render.text(", spraying out of the overflow valve and coating your entire lower body", false);
	Render.text(".", false);
	//[(if vag)
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("  Your cunt spasms around its invader, milking the pseudo-cock and drawing the tainted semen it emits even deeper into your womb", false);
		//[(big clit)
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) Render.text(", and your clitoris twitches under the cup, threatening to black you out from the pleasure", false);
		Render.text(".", false);
	}
	//[(lactation) 
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) {
		Render.text("  Your milk sprays out of your " + nippleDescript(0) + "s", false);
		//[(nipplecunts)
		if(player.upperBody.chest.hasFuckableNipples()) Render.text(" and rings raise from the bases of the shafts inside your nipples, forming a seal,", false);
		Render.text(" as suction begins to whisk it away.", false);
	}
	Render.text("  A minute after your release, the flow in the other direction intensifies, pouring more fluid into you than before", false);
	//[(lactation)
	if(player.biggestLactation() > 1) Render.text(", the drug now tasting of the milk mixed in", false);
	Render.text(". Another chorus sounds from the counter and you hear some gems tinkling as they roll off the plate and onto the ground.\n\n", false);
	...
	if(player.stats.cor > 90) {
		Render.text("After literally hundreds of orgasms, the machine once again begins carefully regulating its suction, denying your release.  Repeatedly it teases your " + multiCockDescriptLight(), false);
		//[(vag)
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and " + vaginaDescript(0), false);
		Render.text(" to the edge of orgasm and then backs off.  All the pumping in the world can't seem to get you over it, and your frustration mounts as the minutes roll by.  A sharper pressure than usual forms at the base of your " + multiCockDescriptLight() + " as you fruitlessly grind into the tubes, wishing more than anything to climax; the green light on the wall steadfastedly refuses to light up, mocking your attempts.  From the new pressure an odd strength flows through the rest of your body, increasing your sexual fervor and giving you a second wind, and you finally manage to get a good grip on the tubes, nearly slamming them back and forth on your shaft", false);
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s", false);
		Render.text(".  This damn demon machine can't stop you from getting off anymore; with a violent shove you bury " + sMultiCockDesc() + " into the mouth", false);
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s", false);
		Render.text(" as your orgasm lets fly!  A truly miraculous amount of semen erupts from your " + multiCockDescriptLight() + ", blowing off the covers of the overflow valves entirely. Jizz shoots across the room as you feel the bulge of pressure slide up your shaft and out of your urethra; a large pink crystal falls out of an open valve-hole and onto the wet floor with a 'plop'.  ", false);
	}
	Render.text("It is at this point Whitney walks in.\n\n", false);

	Render.text("\"<i>What the FUCK!</i>\" the rancher says, her eyes bugging out at the spectacle of you tied to the machine with demonic tubing and the sheer volume of semen you've left in thick ropes all over her barn.", false);
	//[(corruption < 90)  
	if(player.stats.cor < 90) {
		Render.text("  You barely take notice of her as another explosive orgasm goes off, your enormously swollen penis", false);
		if(player.lowerBody.cockSpot.count() > 1) Render.text("es", false);
		Render.text(" pushing massive loads of semen down the tubes and out of the release valves in a firework pattern. Whitney shrinks back as some of the spooge squirts across the barn and comes to a stop less than a foot from her. The canine woman screams volubly enough to wake the entire farm. \"<i>Dammit, " + player.short + "; just... DAMMIT!</i>\" You spare her not even a thought as you anticipate your next injection of the tainted chemicals and work up another batch of jizz.", false);
		dynStats("lib", 20, "sen", 10, "lus", 80, "cor", 20);
	}
	//[(corruption > 90, high demonscore)
	else {
		Render.text("  Still filled with the residual effects of your sudden burst of energy, you pull off the tubes one by one and, taking a moment to snatch up the pink crystal, saunter over to her. Her eyes widen at the simultaneously horrifying and comical sight of you approaching with lust in your eyes and ", false);
		//[(onecox)
		if(player.lowerBody.cockSpot.count() == 1) Render.text("a gigantic, erect penis bobbing in front of you and throwing off your balance", false);
		//(multi)
		else Render.text("multiple gigantic, erect penises bobbing in front of you and dragging between your legs, head pinned against the floor", false);
		Render.text(".  Just in time she backs up and pulls a pitchfork down from the wall. \"<i>I expect I was wrong about you when we met, " + player.short + ".  Get on out of here now and never come back or I'll make sure you never go anywhere again.</i>\"  Scowling, you break off your advance and head toward the door.  Though you definitely want to fuck her, giving yourself a chance to adjust to your magnificent new body might not be a bad idea either.  After all, no matter what she says you can always come back... meanwhile you vow to find something or someone to rape or turn into your personal cock-milker.  With an amazing ", false);
		if(player.totalCocks() == 1) Render.text("dick", false);
		else Render.text("set of dicks", false);
		Render.text(" like yours, why worry about anything else for the moment?", false);
...
private centaurToysHoooooo():void {
	...
	Render.text("You find the dog-morph Whitney standing in the entrance to her barn, scratching her head with consternation.  You approach her and ask what's up.\n\n", false);

	Render.text("\"<i>Oh, hey there, " + player.short + ",</i>\" Whitney says, leaning heavily on her pitchfork.  \"<i>Not much, just trying to figure out... Hey, now!</i>\" she says, eying up your powerful centaur frame.  ", false);
	if(player.stats.cor < 50) Render.text("You shift awkwardly and ask her what's wrong.", false);
	else Render.text("You strut a bit, showing yourself off in a subtly lewd manner.  When you're finished, you ask the dog-girl if she likes what she saw.", false);
	Render.text("\n\n", false);

	Render.text("She clears her throat awkwardly.  \"<i>Uh, well... A pair of centaurs that lived here the last couple of years ran off, looks like.  Gone to get married or something, by the sound of things.  So, hey, wanna help me solve a little problem I'm having?</i>\"\n\n", false);

	Render.text("You shrug. Sure.\n\n", false);

	Render.text("\"<i>See, the two of them left some rather personal belongings behind,</i>\" Whitney says, swinging the barn door open.  Inside, lying in the middle of a stall are what looks like a tall, slender totem with a giant rubber horsecock sticking out of it and a fake mare standing on reinforced wooden legs with a glistening, horsey onahole between its thighs.  Oh, my.  \"<i>So, " + player.short + ", since you look to have the right build for 'em... they're yours.</i>\"\n\n", false);

	Render.text("You tell her sure, and spend the next few minutes loading them onto the back of your horse-body.  Even if you don't end up using them yourself, you've got plenty of room in camp for them, unlike Whitney.  Loaded up with centaur-friendly sex toys, you make your way back to camp.\n\n", false);
	
	Render.text("(<b>Key Items Gained: Fake Mare and Centaur Pole</b>)", false);
