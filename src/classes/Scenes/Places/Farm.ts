...
outputText("Whitney marches up to you as soon as you approach the farm, a stoic expression plastered across her face.");
	...
	if(player.statusAffects.get("MetWhitney").value1 < 2) {
		...
		if(player.findStatusAffect(StatusAffects.MetWhitney) < 0) {
			...
			if(player.statusAffects.get("MetWhitney").value1 == 2) outputText("<b>You've been to the farm enough to easily find it.  You can return by selecting it from the places menu (and will no longer encounter it during random lake exploration)</b>.\n\n", false);
		}
	...
	//Repeat Offender
		...
		else if(flags[FlagEnum.WHITNEY_FLIPPED_OUT_OVER_KELLY] == 1) outputText("You aren't welcome on the farm proper, but you can see Kelly cantering about the fields, looking for you.", true);
...
private function talkWhitney():void {
	...
	if(rand(4) == 0 && player.hasKeyItem("Breast Milker - Installed At Whitney's Farm") < 0) {
		if(player.hasKeyItem("Breast Milker") >= 0) {
			...
			if(player.stats.cor < 33) outputText("You blush with shame", false);
			else if(player.stats.cor <= 66) outputText("You flush with a touch of exhibitionism", false);
	...
	if(player.stats.cor > 50) outputText("or at least the parts you think she would want to hear ", false);
...
public function workFarm():void {
	...
	if(rand(4) == 0) {
		...
		if(player.faceType == FACE.DOG) outputText("sensitive ", false);
...
public function exploreFarm():void {
	...
	if(explore == 2) {
		...
		if(player.horseCocks() > 0) {
			...
			//[if high corruption]
			if(player.stats.cor > 60) outputText("It makes you smile.", false);
			if(player.stats.cor < 30) outputText("It disgusts you.", false);
			//[else/mid-corruption] 
			else if(player.stats.cor <= 60) outputText("You aren't sure how you feel about that.", false);
...
public function getMilked():void {
	...
	if(player.findStatusAffect(StatusAffects.BreastsMilked) < 0) {
		if(player.stats.cor < 50) outputText("You shiver nervously when you step inside.", false);
	...
	//Repeat
		...
		if(player.statusAffects.has("LactationReduction") && player.totalCocks() == 0) outputText("  Your " + nippleDescript(0) + "s are engorged and ready to be taken care of.", false);
		//If cocks
	...
	if(player.findStatusAffect(StatusAffects.BreastsMilked) < 0 || application == 0) {
		...
		if(player.upperBody.chest.BreastRatingLargest[0].nippleLength <= 1.5) outputText("pulling each of your nipples entirely into the suction-tubes.  ", false);
		else outputText("struggling to fit around each of your nipples as they slide into the suction-tubes.  ", false);
		outputText("There is a mechanical lurching noise as the suction builds rapidly.  Your nipple swells out to " + int(player.upperBody.chest.BreastRatingLargest[0].nippleLength*1.5*10)/10 + " inches of length, turning purplish from the strain.   You can feel something welling up inside your " + allBreastsDescript() + ", building as it moves towards your " + nippleDescript(0) + "s.\n\n", false);
	}
	//Apply repeat alternate
		...
		if(player.upperBody.chest.BreastRatingLargest[0].nippleLength >= 3) outputText("They nearly burst the tubes designed to milk them by virtue of their sheer size.  ", false);
	...
	//Medium (+30 lust)
		...
		if(milksplosion == 2) {
			...
			if(player.totalCocks() == 0) {
				...
				if(player.lowerBody.vaginaSpot.hasVagina()) outputText("wet and ", false);
	...
	//CRAZY OUTPUT1 (+60 lust)
		...
		if(milksplosion == 0) {
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) outputText("cream yourself", false);
			else if(player.lowerBody.cockSpot.count()) outputText("jizz yourself", false);
	...
	if(payout > 0) {
		...
		if(flags[FlagEnum.WHITNEY_GEMS_PAID_THIS_WEEK] >= cap) outputText("  <b>The machinery warns you that Whitney can't afford any more this week!</b>", false);
	...
	if(liters > 2) {
		...
		else if(player.stats.int < 15) outputText("  You stretch and let out a contented moo, long and loud.  How silly!", false);
		else if(player.stats.int < 25) outputText("  You quietly moo, then giggle to yourself at how strange you're acting.", false);
	}
...
public function cockPumping():void {
	...
	if(player.findStatusAffect(StatusAffects.CockPumped) < 0) {
		...
		if(player.lowerBody.cockSpot.count() == 1) outputText("single hose wriggles out from the hole, squirming across the floor.  It stops and lifts up, slowly homing in your horny shaft.\n\n", false);
		else if(player.lowerBody.cockSpot.count() == 2) outputText("pair of hoses wriggle out from the hole, squirming across the floor.  They stop and lift up, slowly homing in your horny shafts.\n\n", false);
	...
	//REPEAT
		...
		if(player.lowerBody.cockSpot.count() == 1) outputText("hose wriggles out from the wall, curving up towards your crotch. ", false);
	...
	//Light Scene – output less than 100 mLs
		...
		if(player.lowerBody.cockSpot.count() == 1) outputText("The orgasm rolls over you, shutting down your thoughts as your body spasms in its straps, boiling out thick spurts of spunk.  Wriggling cilia convulse around you, licking your " + player.cockHead() + " as it flares wide, filling with blood and pouring out cum.  You moan and scream with delight, babbling happily as you watch your cum wick up the clear tube and into the machinery in the wall.  All too soon the pleasure comes to an end, and your cock starts to soften inside its squishy prison.  The harness slowly loosens, lowering you to the ground and releasing you.\n\n", false);
	...
	//Medium Scene – output less than 250 mLs
		...
		if(player.lowerBody.cockSpot.count() == 1) {
			...
			if(player.lowerBody.balls > 0) outputText("bloated " + ballsDescriptLight() + " are", false);
			else outputText("sensitive " + cockDescript(0) + " is", false);
			outputText(" finally relieved of ", false);
			if(player.lowerBody.balls > 0) outputText("their ", false);
...
private function milkerBadEnd1():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		//[(bigclit)
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) outputText("A smaller suction tube fastens over your " + clitDescript() + " as it rouses from the vaginal stimulation.", false);
	}
	...
	//[(balls)
	if(player.lowerBody.balls > 0) outputText(" and your " + sackDescript() + " stretches as your " + ballsDescriptLight() + " nearly double in size", false);
	...
	//[(balls)
	if(player.lowerBody.balls > 0) outputText(" and the new volume of your monstrous testicles", false);
	//[(big skeet)
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		//[(big clit)
		if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) outputText(", and your clitoris twitches under the cup, threatening to black you out from the pleasure", false);
		outputText(".", false);
	}
	//[(lactation) 
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) {
		...
		//[(nipplecunts)
		if(player.upperBody.chest.hasFuckableNipples()) outputText(" and rings raise from the bases of the shafts inside your nipples, forming a seal,", false);
	...
	if(player.stats.cor < 90) {
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("es", false);
	...
	//[(corruption > 90, high demonscore)
		...
		//[(onecox)
		if(player.lowerBody.cockSpot.count() == 1) outputText("a gigantic, erect penis bobbing in front of you and throwing off your balance", false);
		//(multi)
...
private function centaurToysHoooooo():void {
	...
	if(player.stats.cor < 50) outputText("You shift awkwardly and ask her what's wrong.", false);
