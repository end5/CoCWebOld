package classes.Scenes.Dungeons.D3 
{
	...
	public class JeanClaudeScenes extends BaseContent
	{
		...
		public function gogoFuckTheseBasilisks():void
		{
			...
			Render.text("You breathe deep, steel yourself")
			if (player.hasKeyItem("Laybans") >= 0) Render.text(", put your shades on")
			Render.text(" and stride out onto the gantry. The plan is simple. You’ll keep your eyes up and march across as if you belong there, as if you are on official business.");
			if (player.demonScore() > 4) Render.text("  You can’t be mistaken for anything but a demon, after all.");
			Render.text("  If or when the lizards realize who you are, you’ll make a break for it, and hopefully by then you’ll be in reach of the exit. It’s straight-forward, it’s elegant, it will work, whatever your hammering pulse has to say on the matter.");

			Render.text("\n\nYou try and not look down as you proceed, back straight and as aloof as you can.");
			
			if (player.isNaga()) Render.text("  You’re probably imagining things but the passage of your sinuous form over the wire surface seems absurdly loud, an endless rubbing of sandpaper which seems to fill the hall.");
			else if (player.isGoo()) Render.text("  Godsdamn this form... you try not to think as you slide across the wire surface, intensely aware of the slimy sound you make as you go, of the steady drip of your fluids through the mesh, trying not to think of it splashing down on curious, blunt heads.");
			else Render.text("  Clack, clack, clack. You are intensely aware of the sound of your [feet] on the wire surface. You close your eyes and tell yourself you’re only imagining that it’s loud, that it echoes around the hall, that the basilisks would have to be deaf not to hear you.");
			...
			else
			{
				...
				else
				{
					...
					if (!player.canFly()) Render.text(" almost");
					else Render.text(" literally");
					Render.text(" fly towards the door. A solitary basilisk manages to clamber onto the catwalk ahead of you- and you simply bull-rush straight through him, his deathly eyes useless against your screened vision. You think you feel the passage of sickle claws scything fruitlessly through the air inches away from your back as you bundle through the exit and into the empty observation booth. In an ecstasy of terror you fumble with the door and manage to slam it shut, just as several heavy bodies thud into the other side. You breathe a huge sigh of relief as your hands find a deadbolt and slide it across, sealing the basilisks off from you. They hammer at it for a while longer, shouting and hissing angrily, but eventually, whilst you’re still gathering your breath, they get fed up with it and leave.");
		...
		public function capturedByBasilisks():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) Render.text(" [eachCock] stiffening");
			if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and");
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" your [vagina] beading with excitement");
			if (!player.lowerBody.cockSpot.hasCock() && !player.lowerBody.vaginaSpot.hasVagina()) Render.text(" your loins aflame with excitement");
			Render.text(" as you sink further into the yellow warmth, immersing yourself in it, so small and pliant and willing to be shaped by that fathomless, over-arching will, this huge masculine creature with a body of carved strength, with his feral, demanding smell surrounding you, with his big, luscious cocks....  The other basilisk’s spell is fading from your limbs, allowing you to move, but it is no respite at all- the more you stare into the eyes in front of you now, the weaker you feel, the more you want to puddle onto the floor and supplicate yourself before their power.");
		...
		{
			clearOutput();
			Render.text("You stare into the yellow, hypnotic eyes and for a moment see beyond them, see them as part of this being rather than the whole, see the arrogance that sits behind them, see, perhaps, a way out of this. With all your willpower you force your mouth to open, work your vocal chords- and make a 'pfft' noise.");
		...
		public function beatDaFuckhugeBasilisk(hpVictory:boolean):void
		{
			...
			if (player.mf("m", "f") == "f" && silly()) Render.text("  You raise your fists in glee and crow “<i>I AM THE LIZARD QUEEN!</i>” at the ceiling.");
		...
		private function goAway():void
		{
			...
			flags[FlagEnum.D3_BASILISKS_REMOVED_FROM_MAGPIE_HALL] = 1;
		...
		public function fuckhugeBasiliskFuckedYouUp(hpVictory:boolean):void
		{
			...
			if (!hpVictory)
			{
				Render.text("You look down at the [weapon] in your hand in bleary confusion. Why are you holding it? Everything feels slow and close, your body almost painful with arousal and sensitivity, and you can’t push beyond it, can’t fathom where you are, what is happening, who you are. It’s the golden glow- it surrounds you and blots everything else out. When Jean-Claude steps in and almost kindly grabs your [weapon] out of your unresisting hands and takes you by the chin, you almost groan. So big, so powerful, so in control- everything you are not. You gladly stare into his maddening yellow eyes, gladly intensifying the feeling of delicious helplessness they give you.");
		...
		{
			if (player.perks.has("BasiliskWomb")) bwombBadEnd();
			else regularBadEnd();
		}
		...
		private function regularBadEnd():void
		{
			...
			Render.text("The Rex’s yellow glow encompasses you, glaring out everything - your purpose, your memories, your sense of self, replaced only by the need to stare deeper and deeper into the pits of gold, the depthless lava of his will which rolls over you, heating you within and without. When he steps into you and puts his warm, smooth hand on your waist you almost flinch, you are that sensitive to his presence. He grins with the exhilaration of a conqueror as he draws you further into himself. The nagging undercurrents of your own body rise to the surface as you feel his warmth and smell his powerful musk;");
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" your [vagina] beads with excitement");
			if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and");
			if (player.lowerBody.cockSpot.hasCock()) Render.text(" [eachCock] hardens");
			if (player.lowerBody.cockSpot.hasCock() || player.lowerBody.vaginaSpot.hasVagina()) Render.text(" as"); 
			Render.text(" your body reacts uncontrollably to the presence of the being whose claws reach deeper and deeper into your mind, touching and triggering your basest desires.");
			if (player.findStatusAffect(StatusAffects.RemovedArmor) > 0) Render.text("  You realize vaguely you already have taken your clothes off, undoubtedly your own subconscious preparing you for this moment.");
			else Render.text("  You really are uncomfortably hot underneath the glare of those eyes. Hardly even aware you’re doing it, you begin to take off your [armor]. Piece after piece falls to the floor until you are naked before him.");
			...
			if (player.skinAdj != "smooth" || player.skinAdj != "latex" || player.skinAdj != "rubber")
			{
				...
				if (player.skinType == SKIN.FUR) Render.text("  Your fur drops away in ragged clumps as you drink."); 
				Render.text("  Once you are finished another black egg is pushed into your hands, then another... you sigh as the viscous gloop slides down your throat and your flesh begins to thrum, your pores puckering and then shrinking into themselves as your skin begins to gleam with the clean starkness of rubber. You slowly examine yourself once the transformation is complete, marvelling at your living, latex flesh. Yes. This feels right.");
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" your [vagina] dribbling excitement");
			if (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.cockSpot.hasCock()) Render.text(" and");
			if (player.lowerBody.cockSpot.hasCock()) Render.text(" your [cock] straining helplessly");
			Render.text(" to this depth of slutty surrender you have plumbed, your neck muscles seizing up and your limbs tightening as the thoughts sink into you.");
			...
			if (player.isBiped()) Render.text("  The transformation isn’t quite complete; your mouth opens as a will not your own forces your arms and legs to open, leaving you completely exposed. It takes you a few seconds to realize that your mouth opening wasn’t your doing, either. It won’t close.");
			else Render.text("  The transformation isn’t quite complete; your mouth opens as a will not your own forces your arms to open and your monstrous bottom half to take on a stance as close to complete submission as it will allow, your [hips] flared to the world. It takes you a few seconds to realize that your mouth opening wasn’t your doing, either. It won’t close.");
			...
			Render.text(" push roughly against your [asshole]");
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and [vagina]");
			Render.text(", it disappears into the depths forever. Your mind is a blank canvas of pure, submissive pleasure and it, like your limitless future, is golden.");
		...
		private function bwombBadEnd():void
		{
			...
			if (player.statusAffects.has("RemovedArmor"))
			{
				Render.text(" You already have taken your clothes off, undoubtedly your own subconscious preparing you for this moment, and you require no direction from Jean-Claude to sit yourself down and eagerly");
				if (player.isBiped()) Render.text(" spread your [hips]");
				else Render.text(" position yourself");
				Render.text(" for him, your breeder’s body aching to be taken.");
			}
			else
			{
				Render.text(" You require no direction from Jean-Claude to take off your [armor] once you are done. Piece after piece falls to the floor until you are naked before him. Anticipation thrumming through you, you sit yourself down and eagerly");
				if (player.isBiped()) Render.text(" spread your [hips]");
				else Render.text(" position yourself");
				Render.text(" for him, your breeder’s body aching to be taken.");
			}
			...
			if (!player.lowerBody.cockSpot.hasCock()) Render.text("  His other dick bats into your stomach, thrusting upwards towards your [chest] as he speaks, his eyes transfixing you, drawing you further into his savage, potent, golden dreams of lust and conquest as he speaks.");
			else Render.text("  His other dick thrusts against your own [cock], hardening it with incessant rubbing as he speaks, his eyes transfixing you, drawing you further into his savage, potent, golden dreams of lust and conquest as he speaks.");
