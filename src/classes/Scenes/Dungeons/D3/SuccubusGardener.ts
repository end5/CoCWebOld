	...
	public class SuccubusGardener extends Monster
	{
		...
		private showerDotEffect():void
		{
			...
			player.statusAffects.get("ShowerDotEffect").value1 = -1;
				...
				//Dot effect
			if (player.lust < 50) MainScreen.text("The tentacles' sex-juices are still covering you - still slowly arousing you. You've got a good handle on it for now.\n\n");
			...
			else if (player.lust < 100)
			{
				...
				{
					MainScreen.text(" soaked twat threatens to " + ((player.wetness() == 4) ? "drown" : "further soak") + " your [legs].");
				}
