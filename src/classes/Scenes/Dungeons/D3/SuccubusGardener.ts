package classes.Scenes.Dungeons.D3 
{
	...
	public class SuccubusGardener extends Monster
	{
		...
		public SuccubusGardener() 
		{
			...
			this.ass.analLooseness = ANAL_LOOSENESS.LOOSE;
		...
		override protected performCombatAction():void
		{
			...
			else if (player.findStatusAffect(StatusAffects.GardenerSapSpeed) < 0 && this.statusAffects.has("VineHealUsed"))
			{
				sapSpeed();
			}
			else
		...
		private vineHeal():void
		{
			...
			{
				Render.text("Tipping herself backward, the succubus gardener lets herself pitch into the writhing tendrils behind her, her mouth opened welcomingly. The tentacles gently catch her, and rather than ravishing her vulnerable form, they merely gather above her parted lips, dribbling thick flows of pink slime. Her throat bobs as she swallows, her injuries vanishing in seconds. The vines push her back up onto her feet. She's smiling a little dopily.");
				
				if (this.lustVuln <= 0.15)
				{
					Render.text(" <b>You aren't sure, but she seems to be leering at you a little more openly.</b>");
				}
				else if (this.lustVuln <= 0.45)
				{
					Render.text(" <b>She's definitely leering at you now.</b>");
				}
				else
				{
					Render.text(" <b>There's no disguising the lecherous look on her face while she eye-humps you.</b>");
				}
				
				if (this.lustVuln <= 0.3) Render.text(" Whatever is in that healing nectar must be weakening her self-control.");
			}
			else
			{
				Render.text("Acting on some unseen signal, the forest of tentacles lurches into action, surrounding its insensate mistress in a veil of wiggling green. A bundle of slime-dripping vines can be spotted through gaps in the verdant veil, hanging over their demonic caretaker with remarkable care, leaking gooey strands of pink into her mouth. Her throat works to swallow it, and when she emerges a second later, her injuries have vanished and her eyes");
				
				if (this.lustVuln <= 0.15)
				{
					Render.text(" are slightly glassy.");
				}
				else if (this.lustVuln <= 0.45)
				{
					Render.text(" are even glassier.");
				}
				else 
				{
					Render.text(" are dilated and a little slow.");
				}
		...
		private tentagrapple():void
		{
			...
			Render.text("A web of interwoven vines lashes out from behind the succubus, somehow leaving her untouched by the wave of advancing greenery. They're trying to grab you!");
			
			if (combatMiss() || combatEvade() || combatFlexibility() || combatMisdirect())
			{
				//Dodge
				Render.text(" You slip aside at the last moment, barely avoiding being wrapped in the squirming mass. It snaps back, perhaps at the limits of its reach, leaving you once more eye to eye with the alluring gardener.");
			}
			else
			{
				//Do not dodddddddggggeee
				Render.text(" You twist to the side, but one snags you by the wrist. Another loops around your [leg], and an avalanche of slime-oozing tentacles falls across the rest of you, wrapping you up in snug coils. Their grip is equal parts iron grip and lover's caress. You'd better struggle free before they really start to work on you!");
		...
		public grappleStruggle():void
		{
			...
			{
				Render.text("You scrabble desperately against the tentacles enveloping your body, pulling against the cast-iron grip around your limbs. You tug against them again and again, and with one final mighty heave, you slip free of their grasp!");
		...
		private squeeze():void
		{
			if (rand(2) == 0)
			{
				Render.text("The vines encircling you squeeze you tighter, twisting and sliding across your [skinFurScales] as they press more tightly around you. It gets hard to breathe, but at the same time, some of them are inside your [armor], gliding across your most sensitive places with oiled ease that's made all the more arousing by the force behind it.");
			}
			else
			{
				Render.text("You're intimately aware of the vegetative masses pressing down on you from every angle, lavishing you with attentions so forceful that they threaten to squeeze the very breathe from your body. It's impossible to ignore. You do your best to breathe and ignore the undulated affections, but even you can't deny the way that it makes your heart beat faster.");
		...
		private sicem():void
		{
			...
			if (damage >= 0)
			{
				...
				Render.text(" The sinuous plant-based tentacles lash at you like a dozen tiny whips! Preparing for stinging pain, you're somewhat taken aback when they pull back at the last moment, sensually caressing your most sensitive places! (" + sL + ")");
			}
			else
			{
				Render.text(" The sinuous plant-based tentacles strike at you like a dozen tiny whips, but, somehow, you manage to avoid every single one of their lashes!");
			}
		}
		...
		private corruptiveShower():void
		{
			...
			{
			//Dodge
				Render.text(" Somehow, you manage to twist out from under the organic raincloud without getting stained by a single drop, though your breath has quickened, and not just from the physical effort.");
			}
			else
			{
				//Fail
		...
		private showerDotEffect():void
		{
			...
			player.statusAffects.get("ShowerDotEffect").value1 = -1;
				...
				//Dot effect
			if (player.lust < 50) Render.text("The tentacles' sex-juices are still covering you - still slowly arousing you. You've got a good handle on it for now.\n\n");
			else if (player.lust < 60) 
			{
				Render.text("You try to wipe some of the fragrant seed from your palm, but all you succeed in doing is smearing it into your [hips].");
				if (player.stats.cor < 50) Render.text(" You're ashamed to admit");
				else Render.text(" You're a little irritated to admit"); 
				Render.text(" that it's starting to feel really good.\n\n");
			}
			else if (player.lust < 70)
			{
				Render.text("You groan at the warm slipperiness enveloping your [skinFurScales] as the tainted tentacles' fluids go to work on you. There's nothing you can do but try to endure it. If only it didn't feel so... hot to be drenched in. If you wind up losing, you hope she'll do this again....\n\n");
			}
			else if (player.lust < 80)
			{
				Render.text("You whimper as the insidious plant-sperm works on your vulnerable " + player.skinDesc + ", building pernicious desires in tiny, insistent increments. It's getting harder to focus... harder not to think about how good all those tentacles would feel in you and on you, caressing your most intimate places.\n\n");
			}
			else if (player.lust < 90)
			{
				Render.text("You shudder in place, stumbling dazedly as your ardor rises to a fever pitch. Soon, you're going to wind up too turned-on to resist, and when that happens, those tentacles are going to take you. The worst part? It's starting to sound really, really... really good to you. Not struggling, no tension... just giving in to what your body craves and loving it.\n\n");
			}
			else if (player.lust < 100)
			{
				Render.text("Ohhhh, you're close now. You can feel the need hammering inside of you, soaking in through your [skinFurScales] to stoke the fires between your [legs] into a blazing inferno, one you couldn't resist even if you wanted to. Then... then you'll be free to cum. You shake your head. Gotta hold it together");
				if (player.lowerBody.cockSpot.hasCock())
				{
					Render.text(", even while your rigid cock" + ((player.lowerBody.cockSpot.count() > 1) ? "s are":" is") + " drizzling ropes of pre unimpeded.");
				}
				else if (player.lowerBody.vaginaSpot.hasVagina())
				{
					Render.text(" soaked twat threatens to " + ((player.wetness() == 4) ? "drown" : "further soak") + " your [legs].");
				}
				else Render.text(".");
			}
			else
			{
				Render.text("Ohhh fuck, there's no holding it back now. You're gonna do it, and there's nothing you could do it stop it even if you wanted to. You're going to drop to your knees and take off your [armor]. You're going to give this beautiful demoness what she wants. You're going to let her fuck you and use you, just so long as she allows you to cum. You'll be fine once you cum, even if it means throwing away a chance to defeat Lethice.\n\n");
			...
			if (player.statusAffects.get("ShowerDotEffect").value1 < 0)
			{
				...
				Render.text("<b>The lust-inducing effects of the plant-spunk feel like they've dissipated...</b>\n\n");
			}
		}
		...
		private motorboat():void
		{
			...
			if (player.tallness <= this.tallness - 6) Render.text(", somehow short enough in spite of the height differences.");
			else if (player.tallness >= this.tallness + 6) Render.text(", somehow tall enough in spite of the height differences.");
			
			Render.text(" They're so soft and pillowy that you can't help but enjoy the feel of them on your skin, and you take a deep, contented breath before remembering where you are and struggling out of the creamy valley.");
