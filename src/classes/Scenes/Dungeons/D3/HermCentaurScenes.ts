 ...
 * @author Gedan
 */
export default class HermCentaurScenes {
	...
	private const CENTAUR_KILLED: number = 1;
	private const CENTAUR_RELEASED: number = 2;
	private const CENTAUR_FUCKED: number = 3;

	public beatThePony(hpVictory: boolean): void {
		MainScreen.clearText();
		MainScreen.text("With a heavy 'thud', the demonic red-head slumps onto her side in the grass.  Her tail flails fitfully")
		if (hpVictory) MainScreen.text(" as she tries to rise");
		else MainScreen.text(" as she tries to stretch for her pulsating, cum-dripping member");
		MainScreen.text(", but it's a futile gesture.  She doesn't have the power to face you on her own terms anymore, and she watches you with a");
		if (hpVictory) MainScreen.text(" resigned glare");
		else MainScreen.text(" lustful glare");
		MainScreen.text(" once she realizes she's utterly spent.");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(0, "Fuck Her", maleFuckHer);
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.addButton(1, "Get Fucked", femFuckHer);
		MainScreen.addButton(5, "Release", letHerGo);
		MainScreen.addButton(6, "Kill", killHer);
	}

	private killHer(): void {
		MainScreen.clearText();
		MainScreen.text("Sighing, you advance on the downed demon, trying to ignore the horrified look in her eyes as you end her.");
	...
	private letHerGo(): void {
		MainScreen.clearText();
		MainScreen.text("You dust off your [armor] and wave nonchalantly at the equine demoness.  She slowly staggers up, watching you warily.  Dismissively, you tell her to leave before she earns your ire - her queen will be dealt with soon enough.  She clops off in a huff towards the exit.");
	...
	private maleFuckHer(): void {
		Flags.list[FlagEnum.D3_CENTAUR_DEFEATED] = CENTAUR_FUCKED;
		...
		if (player.cumQ() >= 1000) MainScreen.text(" You quake at the sensation of dumping so much of your creamy batter into her stomach, nearly closing your eyes in bliss.  Luckily, you hold onto your wits.");
		if (player.cumQ() >= 5000) MainScreen.text(" It doesn't make much difference to the demon beneath you, as her belly slowly rounds from the obscene deposit you're plugging her with.");
		if (player.cumQ() >= 15000) MainScreen.text(" Bigger and bigger, she bloats until she looks fat or pregnant, jiggling with your thick spooge.");
		...
		if (player.cumQ() >= 15000) MainScreen.text(", practically dragging her gut on the stones as she runs");
		MainScreen.text(".  Well, she might have gotten away, but she won't be able to stop you from getting to Lethice.  Indeed, it seems she was running for the exit.");
	...
	private femFuckHer(): void {
		...
		menu();
		if (silly()) MainScreen.addButton(0, "HORZ GOG", femFuckHerII, HORZGOG);
		else MainScreen.addButton(0, "Horsecock", femFuckHerII, HORZGOG);
		MainScreen.addButton(1, "Dog Dick", femFuckHerII, DOGGECOCK);
		MainScreen.addButton(2, "Tentacles", femFuckHerII, TENTACOCK);
	}
	...
	private femFuckHerII(cockType: number): void {
		...
		if (cockType == TENTACOCK) {
			MainScreen.text("You give her throbbing maleness an affection pat and proclaim, \"<i>Make it a tentacle, one that will fit me properly.  Then we can fuck.</i>\"");
			...
			if (player.lowerBody.vaginaSpot.get(0).clitLength >= 6) MainScreen.text(" A coil of cock loops around your gigantic buzzer, effectively frotting it like another cock and chance at protest from your lips.");
			else MainScreen.text(" A bend in the cock grinds against your buzzer with particular intensity, effectively robbing any chance at protest from your lips.");
			...
			if (player.lowerBody.vaginaSpot.get(0).clitLength >= 6) MainScreen.text(" - you swear it spends as much time caressing your [clit] as squeezing it off");
			MainScreen.text(".  You buck your hips, dragging the butt-piercing wang along for the ride as you start to fuck in earnest.");
			...
			MainScreen.text("You give her throbbing maleness an affection pat and proclaim, \"<i>Make it a dog-cock, one that fit to knot properly.  Then we can fuck.</i>\"");
			...
			MainScreen.text("  The pleasure of being so fully stretched is enough to overwhelm any pain at such an act, and you switch over to smaller, faster strokes the leave most of the animal-cock inside you.  Bouncing atop the centaur's balls with a dopey grin on your [face], you sigh and moan.  This is how you're going to take down these demons, one fuck at a time!  The thought gives your " + player.skin() + " a fresh flush, and you pinch your nipple while imaging mounting a demon queen atop an orgy of her defeated lieutenants.");
			...
			MainScreen.text("You give her throbbing maleness an affection pat and proclaim, \"<i>Keep it equine, the flare looks delicious.  Just make sure that it will fit, and then we can fuck.</i>\"");
			...
			if (player.lowerBody.vaginaSpot.get(0).clitLength >= 6) MainScreen.text(" The hard, feminine pole twitches nicely at your touch, a far more sensitive rod than any cock.");
			MainScreen.text(" Your fingers sink into your gash with ease and set to work with ease.  Of course, they're soaked in a second.");
	...
	public inSovietCoCPonyRidesYou(hpVictory: boolean, pcCameWorms: boolean): void {
		...
		if (player.lowerBody.cockSpot.hasCock() && !player.lowerBody.vaginaSpot.hasVagina()) maleLoss(hpVictory);
		else if (player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock()) femLoss(hpVictory);
		else if (!player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock()) femLoss(hpVictory);
	}

	private femLoss(hpVictory: boolean): void {
		MainScreen.clearText();
		MainScreen.text("Dropping down, you");
		if (hpVictory) MainScreen.text(" try to pull yourself up with on your palms, utterly defeated in spite of the centaur's complete lack of physical attacks.");
		else {
			MainScreen.text("put your rump up in the air, fully exposing your");
			...
			MainScreen.text(" to the pervy, sexy equine.");
		}
		MainScreen.text(" She smirks as she trots up, commenting, \"<i>That's more like it, slut.  I don't know how you got this far, but your quest ends here.</i>\"  She casually reaches down and grabs you by the scruff of the neck, hauling you bodily up until your head nestles between her tits");
		if (player.tallness >= 7 * 12) MainScreen.text(", even though your [feet] still touch the ground");
		MainScreen.text(".  Brushing through your hair, a hand gently caresses your back as its owner coos, \"<i>You'll make a fine mare for my stable, but lets get you in the right mindset first...</i>\"");
		...
		if (!player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text(" The pleasant warmth builds with alarming speed, the diffused sensation pooling toward a focused point in your crotch. The heat seems to tunnel deeper into your core before returning to the surface, and with each iteration, the pulsating, almost liquid sensation feels hotter. The warmth quickly makes way to raw, unadulterated heat, which in turn rapidly transforms into a deeply rooted, scalding fire- but it takes on a subtly different tone; a different kind of heat. It’s only when the first traces of wetness begins to roll down the inside of your thigh that you put all the pieces together. <b>The centaur's milk has rooted a cunt into your previously genitalia-void crotch.</b>");
			player.createVagina();
			MainScreen.text("\n\n");
		}
		else MainScreen.text(" ");

		MainScreen.text("Your [vagina] is so puffy and inflamed with passion that it feels ready to burst out of your [armor], even as the drooling fem-cum makes it squish wetly with your every motion.");
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" [EachCock] fares little better, a turgid mess of pre-cum leaking lust that seems to grow harder and more sensitive by the moment.");
		MainScreen.text(" That wonderful warmth excites you sexually, a deepening puddle of lust in your loins that makes you so hot and wet that you swear it must be what's dripping out onto the cobble below.");
	...
	private femLossII(): void {
		...
		menu();
		MainScreen.addButton(0, "Next", femLossIII);
	}

	private femLossIII(): void {
		MainScreen.clearText();
		MainScreen.text("<b>Months later...</b>");
	...
	private maleLoss(hpVictory: boolean): void {
		...
		MainScreen.text("Dropping down, you");
		if (hpVictory) {
			MainScreen.text(" try to pull yourself up with on your palms, utterly defeated in spite of the centaur's complete lack of physical attacks.");
		}
		else {
			MainScreen.text(" try to pull [oneCock] out of your [armor] in an attempt to masturbate.  You're beaten, you just want to fuck or be fucked until you're both covered in a sheen of lady-spunk or your own thick cum.");
		}
		MainScreen.text(" She smirks as she trots up, commenting, \"<i>That's more like it, slut.  I don't know how you got so far thinking you were some kind of " + player.mf("stud", "queen bitch") + ".</i>\"  She casually reaches down and grabs you by the scruff of the neck, hauling you bodily up until your head nestles between her tits");
		if (player.tallness >= 7 * 12) MainScreen.text(", even though your [feet] still touch the ground");
		MainScreen.text(".  Brushing through your hair, a hand gently caresses your back as its owner coos, \"<i>I'm not sure you have what it takes to be one of my stallions...</i>\"");
		...
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" Your [vagina] fares no better, puffing up and drooling like its big brother while your [clit] grows taut and sensitive.");
		MainScreen.text(" That wonderful warmth excites you sexually, moulding your loins into a simmering blaze of desire just looking for a something to cool its ache.");
		...
		if (player.lowerBody.balls > 0) MainScreen.text(" [balls] feel fit to burst!");
		else MainScreen.text(" manhood feels swollen to bursting!");
		MainScreen.text("  Realizing the only way out will be with your tongue, you open wide");
		if (player.upperBody.head.face.tongueType != 0) MainScreen.text(" and uncoil your inhuman length to spear");
		else MainScreen.text(" and begin to lap");
		MainScreen.text(" at her inhuman entrance.  She whinnies in delight, grinding her clydesdale-sized bulk back and forth on your face as you start to get into her eating her out.");

		MainScreen.text("\n\nMoisture drizzles down your cheeks as her already-swollen vulva gets fatter with need.  Her fluids seem to be coming faster and faster, and you have to swallow some of them just to keep from drowning.  Surprisingly, it has a remarkably mild, palatable flavor.  If anything, she tastes slightly sweet, which isn't quite what you would have expected from her potent scent alone.  As turned on as you are, it doesn't take long for you to forget your dire straight and focus on tending to the inhuman woman's needs, licking, slurping, and sucking at her clit with gusto.  She's soon trembling and gushing across your " + HeadDescriptor.describeFace(player) + ", matting your hair with a final, explosive burst of fem-spunk.");
		...
		if (Flags.list[FlagEnum.D1_OMNIBUS_KILLED] != 1) MainScreen.text(", and a familiar one at that");
		MainScreen.text(".  She wears a black reflective bra, though you swear you can see pink protrusions under the shiny fabric, wriggling around constantly.  As expected with such dress, her nipples are hard as bullets.  A bulbous, ten-inch penis bobs between her thighs, thickening nicely as it fills to its full size, kissing your lips with a moist bead of pre as it fully erects.  The omnibus");
		if (Flags.list[FlagEnum.D1_OMNIBUS_KILLED] != 1) MainScreen.text(" overseer asks, \"<i>Remember me?  I thought it only proper that I thank you for letting me live.  Now, be a good boy and suck my dick - show all everyone just what a horny, helplessly fuckable toy you are.</i>\"");
		else MainScreen.text(" asks, \"<i>I hope you don't mind slurping down a little cock, because you're going to anyway.</i>\"");

		MainScreen.text("\n\nThe second herm grabs you by the");
		if (player.upperBody.head.hornType != 0) MainScreen.text(" horns");
		else MainScreen.text(" head");
		MainScreen.text(" and sidles forward, ramming her glans into your mouth and down the back of your throat.  You can feel her glans pulsing deep inside you, and as soon as the herm starts moaning, the centaur starts trotting forward again.  Amazingly, having her holding you up makes it easy to maintain your position and keep your dick bouncing in that hot twat. Fuck it! You're too horny to care anymore.");

		MainScreen.text("\n\nSlurping hard, you lick and suck the dick in your throat, so busy tending to the omnibus that you barely the notice the throbbing ache in your loins.  You cast lusty, inviting eyes around at every feminine form you see during journey to the stables, located in some backwater, seemingly forgotten part of the fortress.  You quiver with need and explode in passion, moaning into the Omnibus's cock as your " + CockDescriptor.describeCock(player, y) + " spends its lust into the oozing horse-pussy, wasting your champion-seed to father nothing more than a batch of shiftless imps.");
