package classes.Scenes.Dungeons.D3 
{
	...
	public class HermCentaurScenes extends BaseContent
	{
		...
		{
			clearOutput();
			Render.text("With a heavy 'thud', the demonic red-head slumps onto her side in the grass.  Her tail flails fitfully")
			if (hpVictory) Render.text(" as she tries to rise");
			else Render.text(" as she tries to stretch for her pulsating, cum-dripping member");
			Render.text(", but it's a futile gesture.  She doesn't have the power to face you on her own terms anymore, and she watches you with a");
			if (hpVictory) Render.text(" resigned glare");
			else Render.text(" lustful glare");
			Render.text(" once she realizes she's utterly spent.");
			...
			if (player.lowerBody.cockSpot.hasCock()) addButton(0, "Fuck Her", maleFuckHer);
			if (player.lowerBody.vaginaSpot.hasVagina()) addButton(1, "Get Fucked", femFuckHer);
		...
		{
			clearOutput();
			Render.text("Sighing, you advance on the downed demon, trying to ignore the horrified look in her eyes as you end her.");
		...
		{
			clearOutput();
			Render.text("You dust off your [armor] and wave nonchalantly at the equine demoness.  She slowly staggers up, watching you warily.  Dismissively, you tell her to leave before she earns your ire - her queen will be dealt with soon enough.  She clops off in a huff towards the exit.");
		...
		private function maleFuckHer():void
		{
			...
			if (player.cumQ() >= 1000) Render.text(" You quake at the sensation of dumping so much of your creamy batter into her stomach, nearly closing your eyes in bliss.  Luckily, you hold onto your wits.");
			if (player.cumQ() >= 5000) Render.text(" It doesn't make much difference to the demon beneath you, as her belly slowly rounds from the obscene deposit you're plugging her with.");
			if (player.cumQ() >= 15000) Render.text(" Bigger and bigger, she bloats until she looks fat or pregnant, jiggling with your thick spooge.");
			...
			if (player.cumQ() >= 15000) Render.text(", practically dragging her gut on the stones as she runs");
			Render.text(".  Well, she might have gotten away, but she won't be able to stop you from getting to Lethice.  Indeed, it seems she was running for the exit.");
		...
		private function femFuckHerII(cockType: number):void
		{
			clearOutput();
			//[Tentacle]
			if (cockType == TENTACOCK)
			{
				Render.text("You give her throbbing maleness an affection pat and proclaim, \"<i>Make it a tentacle, one that will fit me properly.  Then we can fuck.</i>\"");
				...
				if (player.lowerBody.vaginaSpot.list[0].clitLength >= 6) Render.text(" A coil of cock loops around your gigantic buzzer, effectively frotting it like another cock and chance at protest from your lips.");
				else Render.text(" A bend in the cock grinds against your buzzer with particular intensity, effectively robbing any chance at protest from your lips.");
				...
				if (player.lowerBody.vaginaSpot.list[0].clitLength >= 6) Render.text(" - you swear it spends as much time caressing your [clit] as squeezing it off");
				Render.text(".  You buck your hips, dragging the butt-piercing wang along for the ride as you start to fuck in earnest.");
			...
			{
				Render.text("You give her throbbing maleness an affection pat and proclaim, \"<i>Make it a dog-cock, one that fit to knot properly.  Then we can fuck.</i>\"");
				...
				Render.text("  The pleasure of being so fully stretched is enough to overwhelm any pain at such an act, and you switch over to smaller, faster strokes the leave most of the animal-cock inside you.  Bouncing atop the centaur's balls with a dopey grin on your [face], you sigh and moan.  This is how you're going to take down these demons, one fuck at a time!  The thought gives your " + player.skin() + " a fresh flush, and you pinch your nipple while imaging mounting a demon queen atop an orgy of her defeated lieutenants.");
			...
			{
				Render.text("You give her throbbing maleness an affection pat and proclaim, \"<i>Keep it equine, the flare looks delicious.  Just make sure that it will fit, and then we can fuck.</i>\"");
				...
				if (player.lowerBody.vaginaSpot.list[0].clitLength >= 6) Render.text(" The hard, feminine pole twitches nicely at your touch, a far more sensitive rod than any cock.");
				Render.text(" Your fingers sink into your gash with ease and set to work with ease.  Of course, they're soaked in a second.");
		...
		{
			clearOutput();
			Render.text("Dropping down, you");
			if (hpVictory) Render.text(" try to pull yourself up with on your palms, utterly defeated in spite of the centaur's complete lack of physical attacks.");
			else 
			{
				Render.text("put your rump up in the air, fully exposing your");
				...
				Render.text(" to the pervy, sexy equine.");
			}
			Render.text(" She smirks as she trots up, commenting, \"<i>That's more like it, slut.  I don't know how you got this far, but your quest ends here.</i>\"  She casually reaches down and grabs you by the scruff of the neck, hauling you bodily up until your head nestles between her tits");
			if (player.tallness >= 7 * 12) Render.text(", even though your [feet] still touch the ground");
			Render.text(".  Brushing through your hair, a hand gently caresses your back as its owner coos, \"<i>You'll make a fine mare for my stable, but lets get you in the right mindset first...</i>\"");
			...
			{
				Render.text(" The pleasant warmth builds with alarming speed, the diffused sensation pooling toward a focused point in your crotch. The heat seems to tunnel deeper into your core before returning to the surface, and with each iteration, the pulsating, almost liquid sensation feels hotter. The warmth quickly makes way to raw, unadulterated heat, which in turn rapidly transforms into a deeply rooted, scalding fire- but it takes on a subtly different tone; a different kind of heat. It’s only when the first traces of wetness begins to roll down the inside of your thigh that you put all the pieces together. <b>The centaur's milk has rooted a cunt into your previously genitalia-void crotch.</b>");
				player.createVagina();
				Render.text("\n\n");
			}
			else Render.text(" ");
			
			Render.text("Your [vagina] is so puffy and inflamed with passion that it feels ready to burst out of your [armor], even as the drooling fem-cum makes it squish wetly with your every motion.");
			if (player.lowerBody.cockSpot.hasCock()) Render.text(" [EachCock] fares little better, a turgid mess of pre-cum leaking lust that seems to grow harder and more sensitive by the moment.");
			Render.text(" That wonderful warmth excites you sexually, a deepening puddle of lust in your loins that makes you so hot and wet that you swear it must be what's dripping out onto the cobble below.");
		...
		private function maleLoss(hpVictory:boolean):void
		{
			...
			if (hpVictory)
			{
				Render.text(" try to pull yourself up with on your palms, utterly defeated in spite of the centaur's complete lack of physical attacks.");
			}
			else
			{
				Render.text(" try to pull [oneCock] out of your [armor] in an attempt to masturbate.  You're beaten, you just want to fuck or be fucked until you're both covered in a sheen of lady-spunk or your own thick cum.");
			}
			Render.text(" She smirks as she trots up, commenting, \"<i>That's more like it, slut.  I don't know how you got so far thinking you were some kind of " + player.mf("stud","queen bitch") + ".</i>\"  She casually reaches down and grabs you by the scruff of the neck, hauling you bodily up until your head nestles between her tits");
			if (player.tallness >= 7 * 12) Render.text(", even though your [feet] still touch the ground");
			Render.text(".  Brushing through your hair, a hand gently caresses your back as its owner coos, \"<i>I'm not sure you have what it takes to be one of my stallions...</i>\"");
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" Your [vagina] fares no better, puffing up and drooling like its big brother while your [clit] grows taut and sensitive.");
			Render.text(" That wonderful warmth excites you sexually, moulding your loins into a simmering blaze of desire just looking for a something to cool its ache.");
			...
			if (player.lowerBody.balls > 0) Render.text(" [balls] feel fit to burst!");
			else Render.text(" manhood feels swollen to bursting!");
			Render.text("  Realizing the only way out will be with your tongue, you open wide");
			if (player.tongueType != 0) Render.text(" and uncoil your inhuman length to spear");
			else Render.text(" and begin to lap");
			Render.text(" at her inhuman entrance.  She whinnies in delight, grinding her clydesdale-sized bulk back and forth on your face as you start to get into her eating her out.");
			...
			if (flags[FlagEnum.D1_OMNIBUS_KILLED] != 1) Render.text(", and a familiar one at that");
			Render.text(".  She wears a black reflective bra, though you swear you can see pink protrusions under the shiny fabric, wriggling around constantly.  As expected with such dress, her nipples are hard as bullets.  A bulbous, ten-inch penis bobs between her thighs, thickening nicely as it fills to its full size, kissing your lips with a moist bead of pre as it fully erects.  The omnibus");
			if (flags[FlagEnum.D1_OMNIBUS_KILLED] != 1) Render.text(" overseer asks, \"<i>Remember me?  I thought it only proper that I thank you for letting me live.  Now, be a good boy and suck my dick - show all everyone just what a horny, helplessly fuckable toy you are.</i>\"");
			else Render.text(" asks, \"<i>I hope you don't mind slurping down a little cock, because you're going to anyway.</i>\"");
			
			Render.text("\n\nThe second herm grabs you by the");
			if (player.hornType != 0) Render.text(" horns");
			else Render.text(" head");
			Render.text(" and sidles forward, ramming her glans into your mouth and down the back of your throat.  You can feel her glans pulsing deep inside you, and as soon as the herm starts moaning, the centaur starts trotting forward again.  Amazingly, having her holding you up makes it easy to maintain your position and keep your dick bouncing in that hot twat. Fuck it! You're too horny to care anymore.");
