package classes.Scenes.Dungeons.D3 
{
	...
	public class HermCentaurScenes extends BaseContent
	{
		...
		private function maleFuckHer():void
		{
			...
			if (player.lowerBody.balls > 0) outputText(" Slapping wetly against hers, your [balls] ring in an increasing torrent of fleshy slaps.");
		...
		private function femFuckHerII(cockType: number):void
		{
			...
			if (cockType == TENTACOCK)
			{
				...
				if (player.lowerBody.vaginaSpot.list[0].clitLength >= 6) outputText(" A coil of cock loops around your gigantic buzzer, effectively frotting it like another cock and chance at protest from your lips.");
				...
				if (player.lowerBody.vaginaSpot.list[0].clitLength >= 6) outputText(" - you swear it spends as much time caressing your [clit] as squeezing it off");
			...
			else
			{
				...
				if (player.lowerBody.vaginaSpot.list[0].clitLength >= 6) outputText(" The hard, feminine pole twitches nicely at your touch, a far more sensitive rod than any cock.");
		...
		private function femLoss(hpVictory:boolean):void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText(" [EachCock] fares little better, a turgid mess of pre-cum leaking lust that seems to grow harder and more sensitive by the moment.");
		...
		private function maleLoss(hpVictory:boolean):void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" Your [vagina] fares no better, puffing up and drooling like its big brother while your [clit] grows taut and sensitive.");
			...
			if (player.lowerBody.balls > 0) outputText(" [balls] feel fit to burst!");
			...
			if (flags[FlagEnum.D1_OMNIBUS_KILLED] != 1) outputText(", and a familiar one at that");
			...
			if (flags[FlagEnum.D1_OMNIBUS_KILLED] != 1) outputText(" overseer asks, \"<i>Remember me?  I thought it only proper that I thank you for letting me live.  Now, be a good boy and suck my dick - show all everyone just what a horny, helplessly fuckable toy you are.</i>\"");
