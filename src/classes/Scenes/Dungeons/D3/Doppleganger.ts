package classes.Scenes.Dungeons.D3 
{
	...
	public class Doppleganger extends Monster
	{
		...
		public mirrorAttack(damage:number):void
		{
			...
			Render.text("As you swing your [weapon] at the doppleganger, " + player.mf("he", "she") + " smiles mockingly, and mirrors your move exactly, lunging forward with " + player.mf("his", "her") + " duplicate " + weaponName + ".");
		...
		private addTalkShit():void
		{
			...
			switch (_roundCount)
			{
				...
				case 2:
					...
					if (player.lowerBody.cockSpot.hasCock()) Render.text(" strokes [his] [cock]");
					else if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" slides two fingers into [his] [vagina] and gently frigs [himself]");
					else Render.text(" slips a hand ");
		...
		public handleSpellResistance(spell:string):void
		{
			...
			if (spell == "fireball")
			{
				Render.text(" (" + player.takeDamage(player.level * 10 + 45 + rand(10)) + ")");
			}
			else if (spell == "whitefire")
			{
				Render.text(" (" + player.takeDamage(10 + (player.stats.int / 3 + rand(player.stats.int / 2))) + ")");
			}
		...
		public doAI():void
		{
			Render.text("Your duplicate chuckles in the face of your attacks.");
			addTalkShit();
		}
		...
		public Doppleganger() 
		{
			...
			initStrTouSpeInte(player.str, player.tou, player.stats.spe, player.stats.int);
			...
			if (player.lowerBody.cockSpot.count() > 0)
			{
				...
				{
					this.createCock((player.lowerBody.cockSpot.list[i] as Cock).cockLength, (player.lowerBody.cockSpot.list[i] as Cock).cockThickness, (player.lowerBody.cockSpot.list[i] as Cock).cockType);
				}
			}
			...
			for (i = 0; i < player.upperBody.chest.count(); i++)
			{
				...
				let sbr:BreastRowClass = player.upperBody.chest.list[i];
		...
		public get long():string
		{
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) str += "It’s difficult not to notice the way the mirror image of your " + player.breastDescript(player.biggestTitRow()) + " ebbs and heaves within it.";
