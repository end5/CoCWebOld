 ...
 * @author Gedan
 */
export default class SuccubusGardenerScenes {
	...
	private tentaFail(): void {
		...
		MainScreen.text("There's no way you're going to pass up an opportunity to put a succubus's lips to use. Pillowy");
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" cunt-kissers");
		else MainScreen.text(" cock-suckers");
		MainScreen.text(" like those are one in a million.");

		if (Flags.list[FlagEnum.NUMBER_OF_TIMES_MET_SCYLLA] > 0) MainScreen.text(" Perhaps Scylla's ruby mouth could give her a run for her money, but there's only one way to be sure.");
		else if (Game.telAdre.dominika.fellatrixSucked() > 0) MainScreen.text(" Perhaps Dominika's all-consuming mouth could give her a run for her money, but there's only one way to be sure.");
		MainScreen.text(" You grab hold of the blubbering slut by her curled horns, admiring the way the tentacle-juice on her lips shines in the light, and pull her against your crotch, pressing her against");
		...
		else MainScreen.text(" the sensitive skin of your groin");
		MainScreen.text(". She eagerly goes to work, moaning from the attentions of the tentacle down below, and licking with such fervor that you'd think this is what she wanted all along.");
		...
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" blow a wad down this slut's throat");
		else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" cream yourself on this slut's face");
		else MainScreen.text(" teach this bitch how to pleasure an asshole");
		MainScreen.text(". None-too-quiet slurps hang in the steamy air, accompanied by your own " + player.mf("grunts", "coos") + " of enjoyment. You reach down to cup one of her breasts, ignorant of the shadows creeping nearer.");
		...
		if (!player.lowerBody.cockSpot.hasCock()) MainScreen.text(" Its outline is clearly phallic, but how could such a huge thing ever fit inside you, let alone anyone?");
		else {
			MainScreen.text(" Its tip oozes lubricants, but the gaping pussy at its tip is big enough to hold six minotaurs' monster-cocks.");
			if (player.biggestCockLength() < 36) MainScreen.text(" How could you ever hope to fill it?");
			else MainScreen.text(" How long were they preparing this for you?");
		}
		...
		if (!player.lowerBody.cockSpot.hasCock()) // 100% gonna have a cooch by this point
		{
			MainScreen.text("\n\nYou can feel your");
			if (createdVag) MainScreen.text(" new");
			MainScreen.text(" entrance giving around the girthy monstrosity with each passing second. Wider and wider, your flesh opens up. It feels so big that you wonder if you're going to rip in half, but there is no discomfort, only the pleasure of accepting it's tumescent, inhuman length. You groan as your own fluids make themselves known, soaking your gaped lips in girlish cum. There's no way that you should be able to take this, let alone enjoy it, but you are. Oh gods, how you are!");
		...
		//Dicks
			...
			if (player.biggestCockLength() <= 12) MainScreen.text(" The ease with which you slide inside is no surprise, given the disparity in size.");
			else if (player.biggestCockLength() <= 30) MainScreen.text(" The ease with which you enter is no real surprise, given that even your bloated length" + ((player.lowerBody.cockSpot.count() > 1) ? "s are" : " is") + " small by comparison.");
			else if (player.biggestCockLength() <= 48) MainScreen.text(" You slide in with ease, perfectly matched to the hungry fuck-tunnel in size.");
			else MainScreen.text(" Sliding in must take quite the effort. The orange-hued twat is drooling around you, slowly working down an inch at a time. It takes all of you, even if it has to stretch beyond all reason to do so.");
			MainScreen.text(" It feels good, better");
			if (player.biggestCockLength() <= 12) MainScreen.text(" than a giant that looks that loose should. You'd swear it was tight little twat from how firmly it's squeezing you!");
			else if (player.biggestCockLength() <= 30) MainScreen.text(" than you had thought at a glance. Not only is it the perfect size, but it seems to hug and touch every part of your maleness" + ((player.lowerBody.cockSpot.count() > 1) ? "es" : "") + " just right.");
			else if (player.biggestCockLength() > 48) MainScreen.text(" than you would expect given how tightly-stretched it looks. You'd think it'd be pressing down painfully, but it feels tailor made to take you instead.");

			//Dicks need to grow
			if (player.biggestCockLength() <= 30) {
				MainScreen.text("\n\nGlowing orange goo leaks from the oversized slit in thick beads. You briefly wonder what purpose the odd coloration could serve when the feeling of your cock" + ((player.lowerBody.cockSpot.count() > 1) ? "s" : "") + " stretching answers. It's making you grow bigger, somehow! The sensation is akin to stretching long-dormant muscles... an subtle flexing of unrealized potential that makes you aware of just how much you can do, or in this case, how big you can get. Your vision swims as your body works to keep up with the sudden change, and you close your eyes to keep from sicking up.");
			...
			// Dicks big enough
				...
				MainScreen.text("\n\nYou don't go soft after you finish squirting. You don't even get to catch your breath. The orange channel might be holding still as it digests your load, but your dick" + ((player.lowerBody.cockSpot.count() > 1) ? "s are" : " is") + " still getting more sensitive. You don't even need motion for the clutching walls to arouse you any more, and once it starts moving... you're not sure you'll be able to handle it.");

				MainScreen.text("\n\nA few seconds later, you find out that you can't handle it when the tentacle strokes you. The wave of skittering, electric pleasure overloads your mind, and breakers for your consciousness shut it down while your dick" + ((player.lowerBody.cockSpot.count() > 1) ? "s throb" : "throbs") + ", loving every moment of it.");
	...
	private endHer(): void {
		MainScreen.clearText();
		MainScreen.text("You circle behind her an put and end to her evil while she is busy with her pet, then turn to walk away. When you look back over your shoulder, her body is gone. Nothing remains but an empty pathway.");
	...
	private leaveHer(): void {
		MainScreen.clearText();
		MainScreen.text("Figuring that the succubus's pets can keep her busy indefinitely, you turn away. A shriek of pleasure draws your attention back, and you turn in time to see dozens of coiling, leafy masses encircling her every limb, bodily carrying her into a wall of twisting, leaking cocks and pussies. Her orifices are stuffed with pumping lengths that froth with spit and girlcum, and her eyes, equal parts alarmed and aroused, widen before disappearing into the forest of green.");
	...
	private femGenderlessLoss(hpVictory: boolean): void {
		MainScreen.clearText();
		MainScreen.text("You drop to the ground, defeated and degraded");

		if (hpVictory) MainScreen.text(", not even able to push your bruised body up onto its [feet].");
		else MainScreen.text(", too turned on to try and stop your hands from diving into your needy crotch.");
		...
		if (player.statusAffects.has("FactoryOverload")) MainScreen.text(" destroy");
		else MainScreen.text(" shut down");
		MainScreen.text(" our factory would fall to a simple succubus and her lowly garden?\"</i> She strides over to you, hips wiggling in pleasure, and kneels alongside you, running her hand over your abdomen like a girl handling a new pet for the first time. <i>\"You're going to be so useful in my garden!\"</i>");

		MainScreen.text("\n\nHer garden?");
		if (!hpVictory) {
			MainScreen.text(" You cast a furtive glance towards the writhing vines.");
			if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" Your underthings were already swampy with need, but the sight of watching those eager vines squirm elicits fresh trickles of lube to course down your [legs].");
			else MainScreen.text(" You were already horny, but you find yourself wishing you had a pussy, just so you could take even more of them inside you.");
		}
		else MainScreen.text(" You cast a worried look towards the writhing vines. They seem so eager, yet they hold back for now, perhaps waiting for their mistress to finish. No... you can't go down like this!");
		...
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" Against your better judgment, you're starting to get wet from the promises of her words. You know you're being given to a neverending hell of sexual stimulation, but you can't stop your body's libido from responding to the idea.");
		MainScreen.text(" <i>\"Yeah, you want it, doncha?\"</i> Slim fingers deftly press at your");
		if (!player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text(" bare crotch, sinking into a crease that didn't exist a moment before. Her skilled touches and dark powers mold your body like a sculptor's clay, crafting a magnificent cunt on your crotch while simultaneously bringing it to a dripping wet state.");
			player.createVagina();
		}
		else {
			if (player.lowerBody.vaginaSpot.get(0).vaginalWetness <= 2) MainScreen.text(" moistening");
			else MainScreen.text(" soaked");
			MainScreen.text(" crease, and it's all you can do not to cry out and thrust back against her.");
		}

		MainScreen.text("\n\nThe succubus idly toys with your [vagina], sometimes rubbing her thumb");
		if (player.lowerBody.balls > 0) MainScreen.text(" across your [sack],");
		else if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" across the underside of your [cock],");
		else MainScreen.text(" against your [clit],");
		MainScreen.text(" confiding aloud, <i>\"To be honest, I'm a little jealous of you.\"</i>");
	...
	private femGenderlessLossII(): void {
		...
		MainScreen.text(" with foreign wetness. The vines' lubricant is surprisingly warm on your skin, and the feeling of rivulets of it running down the crack of your [butt] and over your [asshole] is surprisingly pleasant.");
		if (player.isBiped()) MainScreen.text(" Your [legs] are eased open, but who are you to stop them? You're as wet as anything, and if spreading wide is the next step towards scratching your itch, so be it.");
		MainScreen.text(" Your [clit] must look like a");
		if (player.lowerBody.vaginaSpot.get(0).clitLength > 6) MainScreen.text(" glistening, cum-soaked shaft");
		else if (player.lowerBody.vaginaSpot.get(0).clitLength > 3) MainScreen.text(" tiny, cum-soaked cock");
		else if (player.lowerBody.vaginaSpot.get(0).clitLength > 1) MainScreen.text(" swollen, lube-lacquered gumball");
		else MainScreen.text(" glistening, pink target");
		MainScreen.text(" by now. You roll your [hips] as much as your position will allow. Your passion inflamed pussy has thus far avoided the tentacles' affections, though it's the one part that wants them more than anything.");
		...
		if (player.lowerBody.cockSpot.hasCock()) {
			...
			MainScreen.text(" over the lurid squishing sounds coming from your cunt, and the two sensations combine into a riot of overwhelming sensory data. You're fucking and being fucked to two different, incongruent tempos and unable to make sense of it in the slightest.");
		}

		//Back to normal.
		MainScreen.text("\n\nMore cum splatters across the exterior of your womanhood a moment before an enclosure envelops your [clit].");
		if (player.lowerBody.vaginaSpot.get(0).clitLength > 6) MainScreen.text(" It stretches some around the cock-sized button");
		else if (player.lowerBody.vaginaSpot.get(0).clitLength > 3) MainScreen.text(" It has no trouble handling your plus-sized button");
		else MainScreen.text(" Your tiny button seems dwarfed by the tube enveloping it");
		MainScreen.text(", but that doesn't make it feel heavenly. It gently pulsates around your buzzer, rippling and squeezing, rhythmically whipping electric lashes of sensation into your fuck-crazed body one after another.");

		MainScreen.text("\n\nThe fleshy spear ravaging your honeypot quivers and slows, then thrusts hard enough to rock your body");
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) MainScreen.text(" and jiggle your tits");
		MainScreen.text(", pressing itself flat against your cervix before finally exploding, creaming your insides with a thick layer of molten-hot seed. You would groan or whimper if your throat wasn't crammed so full of plantcock. Instead, your eyes roll back under their fluttering lids, revealing a narrow band of white as you lose all control. Your body shakes like a seizure victim's, all except for the muscles down south. They're too busy clenching and squeezing the lengthy intrudor, wringing every drop of lust-imbued cum");
		if (player.isPregnant()) MainScreen.text(" to splatter against your occupied womb.");
		else MainScreen.text(" to fill your womb, to make it round with life so that others can feel what you're feeling.");

		MainScreen.text("\n\nYour girlhoney squirts, splattering over the ground.");
		if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < 4) MainScreen.text(" You may not normally be capable of such soaking orgasms, but that seems of little concern to you.");
		MainScreen.text(" You're a fountain of wetness now, fit only for cumming and taking cum in. The thought triggers a second orgasm on the back of the first while the spent tentacle exits.");
	...
	private maleLoss(hpVictory: boolean): void {
		...
		MainScreen.text("You drop to the ground, defeated and degraded");
		if (hpVictory) MainScreen.text(", not even able to push your bruised body up onto its [feet].");
		else MainScreen.text(", too turned on to try and stop your hands from rushing to your crotch. Why bother?");
	...
	private maleLossPet(): void {
		MainScreen.clearText();
		MainScreen.text("<i>\"I'll be your doggie,\"</i> you gasp with an accompanying needy arch of your back.");
		...
		if (player.biggestCockLength() >= 18) MainScreen.text(" <i>\"Oh, it's quite the big bone, isn't it? You're going to have an awful hard time dragging that around on all fours, [puppy]. Why, I bet the moment you see a bitch, you're going to be humping the ground, practically immobilized by it.");
		else if (player.biggestCockLength() >= 8) {
			MainScreen.text(" <i>\"Oh, that's quite the nice bone you've got there, isn't it? Why, everyone is going to see how big you are down there, even while you're walking on all fours. I suppose you'd be good for breeding the bitches");
			if (player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockType != CockType.DOG) MainScreen.text(", once you have an appropriately-shaped cock");
			MainScreen.text(".");
		}
		else {
			MainScreen.text(" Oh, that's quite the well-formed bone you've got there, isn't it? I could probably take you to shows, and the judges would comment on how well it matches your form. You'd like that, wouldn't you? Rolling over on your back and letting some official-looking demon measure your cock.");
		}
		MainScreen.text("\"</i>");
		...
		if (player.lowerBody.balls > 0) MainScreen.text(", just above your [balls],");
		MainScreen.text(" once more, cooing, <i>\"I knew you'd train well. Now, I'm going to lead you to your new home, and then you'll get to blow all that nasty, pent-up spunk all over my hand. But you have to be good until then, okay?\"</i>");
		...
		if (!player.hasMuzzle()) MainScreen.text(", heedless of the fact that you seem to be able to reach a little farther with every one you consume");
		MainScreen.text(", slobbering some in your vigor to get more. All the while, the depression fills higher. The sound of gushing fluid tapers down to pitter-patters of the last droplets impacting the impromptu bowl, accompanied by a moan from the demon. She's clearly enjoyed preparing your little treat.");

		MainScreen.text("\n\n<i>\"Eat up, doggy.\"</i> Her foot disappears.");

		MainScreen.text("\n\nYou look down at your drooling rod");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(", then at the puddle of sap-laced milk, then at your rod again, then finally back at the puddle before getting up onto all");
		if (player.lowerBody.isNaga()) MainScreen.text(" threes");
		else if (player.lowerBody.isTaur()) MainScreen.text(" your hooves");
		else MainScreen.text(" fours");
		MainScreen.text(".");
		if (!player.lowerBody.isTaur()) MainScreen.text(" You scrabble over, unusued to having to move in such a way,");
		else MainScreen.text(" You gallop over and drop down on your forelegs");
		MainScreen.text(" and plant your [face] right into the succubus' milk. It's a little thicker than milk normally is and stickier too, like honey, but it's also sweeter. The ambrosial fluid fills your mouth with scintillating, indescribable flavor that sets");
		if (player.lowerBody.tailType != 0) {
			MainScreen.text(" your tail");
			if (player.lowerBody.tailType == 13 && player.lowerBody.tailVenom > 1) MainScreen.text("s");
		}
		else MainScreen.text(" your stubby, new tail");
		MainScreen.text(" wagging.");
		if (player.lowerBody.tailType != 2) MainScreen.text(" Never mind that it gets longer and fuzzier with every gulp or that your");
		else MainScreen.text(" Never mind that your");
		MainScreen.text(" tongue is swelling up, wide, flat, and perfect for panting, licking, and lapping up a drink.");
		player.lowerBody.tailType = 2;
		...
		if (player.isPregnant()) MainScreen.text(" pregnant");
		MainScreen.text(" belly, dripping and drooling");
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(", lubricated with a sheen vaginal juices from your forgotten sex organ");
		...
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s thrum");
		else MainScreen.text(" thrums");
		MainScreen.text(" with ready excitement. A thin trail of pre-spooge rolls out of you, trailing on the ground behind you with each happy twitch of your bitch-");
		if (player.biggestCockLength() >= 12) MainScreen.text("breaker");
		else MainScreen.text("boner");
		MainScreen.text(". You hope she doesn't mind, but you have a hunch that she won't mind you being THAT kind of messy puppy.");
		...
		if (player.lowerBody.isTaur() || player.lowerBody.isDrider()) {
			...
			if (player.lowerBody.isDrider()) MainScreen.text(" clip-clop of your hooves");
			else MainScreen.text(" sharp clacks of your spidery claws");
			MainScreen.text(" fading into the near silent pats of padded paws. Your hands are changing too: the fingers are shortening, but your nails are growing. Fur grows out over them, and your spine shifts, all so that you can follow on all fours that much easier.");
		}
		else {
			//Nontaurs
			MainScreen.text("\n\nThe longer you follow, the easier it gets to stay on her heels.");
			if (player.lowerBody.isGoo()) MainScreen.text(" Your gooey undercarriage easily divides into two hind legs, slapping wetly with regularity as you plod along.");
			else if (player.lowerBody.isNaga()) MainScreen.text(" Your slithering tail twitches, stopping you for a second, but then it splits into two limbs, each firmer and more canine than the snake-like appendage you once had. They wobble unsteadily for a second before jumping into action.");
			else MainScreen.text(" Your [legs] shorten as you plod along, slowing you momentarily as they change. You lift your knees up so that you can stand on your [feet] once more, only they're soft, padded paws now, tipped with short nails.");
			MainScreen.text(" Your hands follow suit. First, pads form on your palms, making it easier to support your weight. Then, your fingers shorten. It's not entirely an unpleasant sensation, as it makes moving towards your home that much easier.");
	...
	private maleLossPetII(): void {
		MainScreen.clearText();
		MainScreen.text("Your kennel is amazing! It's the perfect size for you to crawl into, there's holes in it so that you don't get too hot while you wait to be let out. There's even one big enough for you to slip");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" a");
		else MainScreen.text(" your");
		MainScreen.text(" cock through. That hole is on the back wall. You can't see what's over there, but your nose can smell pussies - some of them bitch-pussies, and lots of spilled cum. Your old mind would've identified it as a glory-hole, but it's harder to focus on concepts like that now. ");
		...
		if (player.skinType != SkinType.FUR) MainScreen.text(" Was that there before?");
		MainScreen.text(" It feels very right, and in your excitement, it's hard not to hump her leg. It must be obvious to her, because your dick");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s are");
		else MainScreen.text(" is");
		MainScreen.text(" bobbing beneath you, and she gestures for you to roll onto your back.");
		...
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(" trembling and spurting out ribbons of pre. Whining in the back of your throat, you look at her, hoping she can read the affection in your eyes, wanting her to see just how completely contented you are with this state of affairs.");
		...
		if (player.cumQ() >= 2500) MainScreen.text(" A few thick ropes shoot past your head to cream the wall, eliciting a pleased giggle from your owner as she squeezes yet more from your length.");
		MainScreen.text(" You yip with glee as you cream for Mistress' fingers, but eventually, your climax trails off into a dribbly white mess, some of which clings to the demoness' nails like beads of ivory dew.");
	...
	private maleLossFeeder(): void {
		...
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" [oneCock].");
		...
		else MainScreen.text(" one's");
		MainScreen.text(" twitching length, the succubus giggles. <i>\"Can you feel it? Can you feel your body just welling up with cum? I suppose I should have mentioned that, huh?\"</i> She runs her finger back the other way, and your muscles clench, squeezing out a big, fat drop of pre from your cock, dripping down to hit");
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) MainScreen.text(" the underside of your breasts");
		else MainScreen.text(" your chin");
		MainScreen.text(". You whimper with need. It's impossible not to. Hell, your " + CockDescriptor.describeMultiCockShort(player));
		if (player.lowerBody.cockSpot.count() == 1) MainScreen.text(" is");
		else MainScreen.text(" are");
		MainScreen.text(" flexing, and if you had anything to brace off of, you'd be thrusting into the first orifice you could find.");
		...
		//Ball filling
			...
			if (player.lowerBody.ballSize > 15) MainScreen.text(" Thankfully, a few tentacles loop under the gigantic orbs to help support them.");
		}

		MainScreen.text("\n\nYou happily gurgle into the tentacle in your mouth and arch your back, blowing the first huge rope of your enhanced load");
		// 9999 - Check muh
		if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) MainScreen.text(" all over your body");
		else MainScreen.text(" directly onto the soil below");
	...
	private maleLossFeederII(): void {
		...
		if (player.biggestCockLength() < 6) {
			MainScreen.text(" swiftly engulfs your meager offering");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		}
		else if (player.biggestCockLength() < 12) {
			MainScreen.text(" rapidly devours your plus-sized length");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		}
		else if (player.biggestCockLength() < 24) {
			MainScreen.text(" devours your immense offering");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
			MainScreen.text(" with steady determination");
		}
		else {
			MainScreen.text(" slowly encircles your cum-spouting length");
			if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
			MainScreen.text(" with inhuman elasticity");
		}
		MainScreen.text(". The interior is covered with tiny, wiggling nubs that wiggle into action on contact with your trembling member");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(". It's like every single nerve has its own skilled masseuse giving it a rubdown, coaxing more pleasure from every square inch of skin than you thought it capable of producing. You cry out, totally overwhelmed, only to be silenced by a familiar presense in your mouth.");
		...
		if (player.lowerBody.tailType != 0) MainScreen.text(" past your tail to slither");
		MainScreen.text(" between your cheeks.");
		if (player.lowerBody.butt.analLooseness == 0) MainScreen.text(" The virginal tightness of your pucker holds the tentacle at bay at first. Still, it's no match for the combination of slippery plant fluid and constant pressure.");
		else if (player.lowerBody.butt.analLooseness <= 3) MainScreen.text(" The tentacle has little problem slipping through your tight backdoor, slick as it is with fluid and backed by constant pressure.");
		else MainScreen.text(" The tentacle has little difficulty squeezing into your well-loosened ass. Even without its constant, firm pressure and slippery fluid, it could enter with ease.");
		MainScreen.text(" You gasp at the odd, room-temperature feel of an intruder squirming through your rearmost passage. It's entirely unexpected, but once its mass slithers across your prostate, squeezing even bigger blobs of cum from your rapt, attentive body, you forgive its intrusion.");
		...
		if (player.lowerBody.balls == 0) MainScreen.text(" prostate");
		else MainScreen.text(" ball");
		MainScreen.text("-draining spurts of plant-food. The cradling vines squirm over your touch-heightened skin, grinding the whole of your body. It's like your entire being is being fucked");
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(", and once a thick, cock-shaped plant slides inside your [vagina], you truly are");
		MainScreen.text(".");
