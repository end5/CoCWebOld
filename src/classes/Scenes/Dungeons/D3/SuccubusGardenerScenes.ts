package classes.Scenes.Dungeons.D3 
{
	...
	public class SuccubusGardenerScenes extends BaseContent
	{
		...
		private tentaFail():void
		{
			...
			if (flags[FlagEnum.NUMBER_OF_TIMES_MET_SCYLLA] > 0) MainScreen.text(" Perhaps Scylla's ruby mouth could give her a run for her money, but there's only one way to be sure.");
			else if (getGame().telAdre.dominika.fellatrixSucked() > 0) MainScreen.text(" Perhaps Dominika's all-consuming mouth could give her a run for her money, but there's only one way to be sure.");
		...
		private endHer():void
		{
			...
			flags[FlagEnum.D3_GARDENER_DEFEATED] = GARDENER_KILLED;
		...
		private leaveHer():void
		{
			...
			flags[FlagEnum.D3_GARDENER_DEFEATED] = GARDENER_LEFT;
		...
		private femGenderlessLoss(hpVictory:boolean):void
		{
			...
			{
				if (player.wetness() <= 2) MainScreen.text(" moistening");
		...
		private femGenderlessLossII():void
		{
			...
			if (player.lowerBody.vaginaSpot.list[0].clitLength > 6) MainScreen.text(" glistening, cum-soaked shaft");
			else if (player.lowerBody.vaginaSpot.list[0].clitLength > 3) MainScreen.text(" tiny, cum-soaked cock");
			else if (player.lowerBody.vaginaSpot.list[0].clitLength > 1) MainScreen.text(" swollen, lube-lacquered gumball");
			...
			if (player.lowerBody.vaginaSpot.list[0].clitLength > 6) MainScreen.text(" It stretches some around the cock-sized button");
			else if (player.lowerBody.vaginaSpot.list[0].clitLength > 3) MainScreen.text(" It has no trouble handling your plus-sized button");
			...
			if (player.wetness() < 4) MainScreen.text(" You may not normally be capable of such soaking orgasms, but that seems of little concern to you.");
		...
		private maleLossPet():void
		{
			...
			if (player.isNaga()) MainScreen.text(" threes");
			else if (player.isTaur()) MainScreen.text(" your hooves");
			else MainScreen.text(" fours");
			MainScreen.text(".");
			if (!player.isTaur()) MainScreen.text(" You scrabble over, unusued to having to move in such a way,");
			...
			if (player.tailType != 0)
			{
				...
				if (player.tailType == 13 && player.lowerBody.tailVenom > 1) MainScreen.text("s");
			}
			else MainScreen.text(" your stubby, new tail");
			MainScreen.text(" wagging.");
			if (player.tailType != 2) MainScreen.text(" Never mind that it gets longer and fuzzier with every gulp or that your");
			...
			if (player.isTaur() || player.isDrider())
			{
				...
				if (player.isDrider()) MainScreen.text(" clip-clop of your hooves");
			...
			//Nontaurs
				...
				if (player.isGoo()) MainScreen.text(" Your gooey undercarriage easily divides into two hind legs, slapping wetly with regularity as you plod along.");
				else if (player.isNaga()) MainScreen.text(" Your slithering tail twitches, stopping you for a second, but then it splits into two limbs, each firmer and more canine than the snake-like appendage you once had. They wobble unsteadily for a second before jumping into action.");
		...
		private maleLossPetII():void
		{
			...
			if (player.skinType != SKIN.FUR) MainScreen.text(" Was that there before?");
		...
		private maleLossFeeder():void
		{
			...
			MainScreen.text(". You whimper with need. It's impossible not to. Hell, your " + multiCockDescriptLight());
		...
		private maleLossFeederII():void
		{
			...
			if (player.tailType != 0) MainScreen.text(" past your tail to slither");
			MainScreen.text(" between your cheeks.");
			if (player.ass.analLooseness == 0) MainScreen.text(" The virginal tightness of your pucker holds the tentacle at bay at first. Still, it's no match for the combination of slippery plant fluid and constant pressure.");
			else if (player.ass.analLooseness <= 3) MainScreen.text(" The tentacle has little problem slipping through your tight backdoor, slick as it is with fluid and backed by constant pressure.");
