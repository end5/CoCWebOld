package classes.Scenes.Dungeons.D3 
{
	...
	public class SuccubusGardenerScenes extends BaseContent
	{
		...
		private function tentaFail():void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" cunt-kissers");
			else outputText(" cock-suckers");
			outputText(" like those are one in a million.");
			
			if (flags[FlagEnum.NUMBER_OF_TIMES_MET_SCYLLA] > 0) outputText(" Perhaps Scylla's ruby mouth could give her a run for her money, but there's only one way to be sure.");
			...
			if (player.lowerBody.cockSpot.hasCock()) outputText(" blow a wad down this slut's throat");
			else if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" cream yourself on this slut's face");
			...
			if (!player.lowerBody.cockSpot.hasCock()) outputText(" Its outline is clearly phallic, but how could such a huge thing ever fit inside you, let alone anyone?");
			else
			{
			...
			else
			{
				...
				else if (player.biggestCockLength() <= 30) outputText(" The ease with which you enter is no real surprise, given that even your bloated length"+ ((player.lowerBody.cockSpot.count() > 1) ? "s are" : " is") +" small by comparison.");
				...
				else if (player.biggestCockLength() <= 30) outputText(" than you had thought at a glance. Not only is it the perfect size, but it seems to hug and touch every part of your maleness" + ((player.lowerBody.cockSpot.count() > 1) ? "es" : "") + " just right.");
		...
		private function femGenderlessLoss(hpVictory:boolean):void
		{
			...
			if (player.statusAffects.has("FactoryOverload")) outputText(" destroy");
			...
			if (!hpVictory) 
			{
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" Your underthings were already swampy with need, but the sight of watching those eager vines squirm elicits fresh trickles of lube to course down your [legs].");
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" Against your better judgment, you're starting to get wet from the promises of her words. You know you're being given to a neverending hell of sexual stimulation, but you can't stop your body's libido from responding to the idea.");
			...
			if (player.lowerBody.balls > 0) outputText(" across your [sack],");
			else if (player.lowerBody.cockSpot.hasCock()) outputText(" across the underside of your [cock],");
		...
		private function femGenderlessLossII():void
		{
			...
			if (player.lowerBody.vaginaSpot.list[0].clitLength > 6) outputText(" glistening, cum-soaked shaft");
			else if (player.lowerBody.vaginaSpot.list[0].clitLength > 3) outputText(" tiny, cum-soaked cock");
			else if (player.lowerBody.vaginaSpot.list[0].clitLength > 1) outputText(" swollen, lube-lacquered gumball");
			...
			if (player.lowerBody.vaginaSpot.list[0].clitLength > 6) outputText(" It stretches some around the cock-sized button");
			else if (player.lowerBody.vaginaSpot.list[0].clitLength > 3) outputText(" It has no trouble handling your plus-sized button");
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText(" and jiggle your tits");
		...
		private function maleLossPet():void
		{
			...
			else if (player.biggestCockLength() >= 8)
			{
				...
				if (player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockType != CockType.DOG) outputText(", once you have an appropriately-shaped cock");
				outputText(".");
			}
			else
			{
			...
			if (player.lowerBody.balls > 0) outputText(", just above your [balls],");
			...
			if (player.tailType != 0)
			{
				...
				if (player.tailType == 13 && player.lowerBody.tailVenom > 1) outputText("s");
			}
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText(", lubricated with a sheen vaginal juices from your forgotten sex organ");
			...
			if (player.lowerBody.cockSpot.count() > 1) outputText("s thrum");
		...
		private function maleLossPetII():void
		{
			...
			if (player.lowerBody.cockSpot.count() > 1) outputText("s are");
			else outputText(" is");
		...
		private function maleLossFeeder():void
		{
			...
			if (player.lowerBody.cockSpot.count() > 1) outputText(" [oneCock].");
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) outputText(" the underside of your breasts");
			...
			else
			{
				...
				if (player.lowerBody.ballSize > 15) outputText(" Thankfully, a few tentacles loop under the gigantic orbs to help support them.");
			}

			outputText("\n\nYou happily gurgle into the tentacle in your mouth and arch your back, blowing the first huge rope of your enhanced load");
			// 9999 - Check muh
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) outputText(" all over your body");
		...
		private function maleLossFeederII():void
		{
			...
			if (player.lowerBody.balls == 0) outputText(" prostate");
			...
			if (player.lowerBody.vaginaSpot.hasVagina()) outputText(", and once a thick, cock-shaped plant slides inside your [vagina], you truly are");
			outputText(".");
