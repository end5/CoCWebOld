 ...
 * @author Gedan
 */
export default class LivingStatue extends Monster {
	...
	private dirtKick(): void {
		...
		//blind
		if (rand(2) == 0 && !player.statusAffects.has("Blind")) {
	...
	private overhandSmash(): void {
		...
		let damage: number = 175 + int((str + weaponAttack) - rand(player.stats.tou) - player.armorDef);
	...
	private cycloneStrike(): void {
		...
		let damage: number = (175 + int((str + weaponAttack) - rand(player.stats.tou) - player.armorDef)) / (rand(3) + 2);
		//Avoid
	...
	override protected performCombatAction(): void {
		if (this.HPRatio() < 0.7 && this.findStatusAffect(StatusAffects.KnockedBack) < 0) {
			...
			if (!player.statusAffects.has("Blind") && !player.statusAffects.has("Stunned")) opts.push(dirtKick);
			if (!player.statusAffects.has("Blind") && !player.statusAffects.has("Stunned")) opts.push(concussiveBlow);
