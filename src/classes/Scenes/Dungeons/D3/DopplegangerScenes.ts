package classes.Scenes.Dungeons.D3 
{
	...
	public class DopplegangerScenes extends BaseContent
	{
		...
		public function getDemGlasses():void
		{
			...
			if (player.humanScore() > 4)  Render.text("  It is honestly surprising, given everything you have encountered since you stepped through that portal what seems years ago, that you are still recognizably human. You wonder if your parents would be able to recognize the battle-hardened " + player.mf("man", "woman") +" clad in " + player.mf("his", "her") +" [armor] standing in front of you to be the wide-eyed youth they bade their heartfelt goodbyes to all that time ago, though.");
			else Render.text("  The last time you looked into such a sharp reflection you were human; a pang of regret steals through you as you consider probably not even your parents would be able to recognize the strange "+ player.race() +" clad in "+ player.mf("his", "her") +" [armor] standing in front of you now.");
		...
		public function punchYourselfInTheBalls():void
		{
			...
			clearOutput();
			Render.text("The doppelganger falls to the floor, sobbing and broken. For one long moment you feel the nagging pull to exactly replicate [his] actions... and then it’s gone. The world comes back sharply into focus. You have won.");
			...
			menu();
			if (!player.isTaur() && (player.lowerBody.cockSpot.hasCock() || player.lowerBody.vaginaSpot.hasVagina())) addButton(0, "Oh Really?", fuckYourself);
		...
		public function fuckYourself():void
		{
			...
			if (player.lowerBody.cockSpot.count() >= 1 && !player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				if (player.isNaga()) Render.text("  You sigh at the smooth, pleasurable feeling of your scales rubbing over each other as you sensuously wind yourself around his tail until you are tightly bound together. Twin lithe snake boys wound around each other- all you really need is a paying audience. Or a stick and a hospital. You laugh with a similar tinge of madness that your partner did a short while ago and he groans raggedly in response.");
				else if (player.isDrider()) Render.text("  The tangle of spindly legs in front of you initially puts you off, but then your arachnid instincts take over and you find yourself eagerly clambering onto his bulging back before you know it, your hardening dick pressing against the human softness which rises above the gleaming chitin as your monstrous body presses heavily down onto its mirror counterpart, making him groan raggedly.");
				else if (player.skinType == SKIN.FUR && player.tailType != TAIL.NONE) Render.text("  You clamber on top of him, the rough mingling of your fur sparking deep urges within you, your animal half dreaming of frenzied, bestial mating in the wild. Your hardening dick presses against his [tail] and he groans raggedly in response.");
				else Render.text("  You clamber on top of him, enjoying the give and take of his "+ (player.isGoo() ? "gooey" : "smooth") +" flesh as you lock your [hips] around their mirror counterparts, making him groan raggedly in response.");
				...
				if (player.ass.analWetness > 3) Render.text("  No problem for a clone of you, though. You sigh as you push your bulging head against his rose, easily opening him and pushing deep into his oily, pliable depths.");
				else Render.text("  Well, tough. He is a demon, after all. You sigh as you push your bulging head against his rose, slowly working it loose with the repetitive, pleasurable motion until he finally relaxes enough for you to push into his warm, tight depths.");
				...
				if (player.perks.has("Buttslut") || player.perks.has("MaraesGiftButtslut")) Render.text("  You glory in your own decision to transform into an eager butt slut; the doppelganger’s back passage is tight, warm butter, accepting and clenching around your bulging prick like the perfect little boypussy you know it to be.");
				Render.text("  Now that you’re locked into your defeated clone you feel no real wish to cause him pain. Indeed, as you lose yourself to the rhythm to the slow, tight, gradual sex a desire to force him to feel pleasure grows. Where is the fun in seeing your mirror image suffer, after all?");
				...
				if (player.cumQ() >= 1000) Render.text(" You know exactly what to expect; his over-clocked physiology makes him a veritable cum fountain. As you ride your high, pumping his guts full of your warm seed he spurts geysers of jizz the length of his body onto the floor, wailing at the sheer size of the high owning your body forces him to feel. You sit astride him, admiring and exulting in it at the same time.");
				else Render.text("  You ride your high, pumping his guts full of your warm seed as you force him along with you, guttering jizz on the floor in front of you.");
			...
			else if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				if (player.isDrider()) Render.text("  For a moment you consider mounting her from behind, allowing your monstrous instincts to take over and fuck her ovipositor in a deathly arachnid craze, but the tangle of spindly legs in front of you puts you off. Better to do it face-to-face. You want to drink in your mirror image’s every reaction. Your prick pressing against the human softness which rises above her gleaming chitin as your body presses heavily down onto its mirror counterpart, making her groan raggedly.");
				if (player.skinType == SKIN.FUR) Render.text("  You clamber on top of her, the rough mingling of your fur sparking deep urges within you, your animal half dreaming of frenzied, bestial mating in the wild. Your dick presses into her digitigrade thigh and she groans raggedly in response.");
				else Render.text(" You clamber on top of her, enjoying the give and take of her "+ (player.isGoo() ? "gooey" : "smooth") + " flesh as you lock your [hips] around their mirror counterparts, making her groan raggedly in response.");
				...
				if (player.cumQ() >= 1000) Render.text("  You know exactly what to expect; her over-clocked physiology makes her a veritable cum fountain. As you ride your high, pumping her full of your warm seed she spurts streams of jizz the length of her body on the floor, wailing at the sheer size of the high owning your body forces her to feel. You sit astride her, exulting in and enjoying the sight of it at the same time.");
				else Render.text("  You ride your high, pumping her full of your warm seed as you force her along with you, guttering jizz on the floor in front of you.");
			...
			else if (player.lowerBody.vaginaSpot.hasVagina() && !player.lowerBody.cockSpot.hasCock())
			{
				...
				Render.text(" - as you knew it would be. Your doppelganger wriggles and whines but can’t resist as you push your pussy into hers, the mirrored sex meeting with a pleasingly wet sound.");
				...
				if (player.thickness > 75 || player.tone < 25) Render.text(" sending your shared jiggly flesh rippling in separate directions");
				else Render.text(" your shared, muscled flesh barely moving");
				Render.text(" as you push your clit into her wet opening, making her cry out with that same delicious cadence as she closes her eyes. There is an ominous snap behind you as more cracks thread themselves over the mirror’s surface.");
				...
				if (player.cumQ() > 1000) Render.text("  You know exactly what to expect; her over-clocked physiology makes her a veritable fem-cum fountain. Your vaginas spurt and spatter against each other, rivulets of shared delirium dribbling onto the floor and dousing your abdomens, the doppelganger wailing at the sheer size of the high owning your body forces her to feel. You sit astride her, exulting in and enjoy the sight of it at the same time.");
				else Render.text("  You ride your high, shared female delirium wetting your thighs as you force her to ride her high locked deep into you.");
		...
		public function inSovietCoCSelfFucksYou():void
		{
			...
			if (player.lowerBody.cockSpot.hasCock()) Render.text(", the way your turgid [cock] feels when you grasp it at the base.");
			else if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(", the way your [vagina] feels when you slide your fingers along its lips.");
			else 
			{
				Render.text(", the way your");
				player.createVagina();
				Render.text(" [vagina] feels when you slide your fingers against your... Wait, where did that come from? You push your fingers in more forcefully, determined to see through this illusion, but all you manage to do is arouse yourself beyond all reasonable measure. Somehow, <b>you've grown a tight, wet pussy to match your duplicate's!</b>");
			}
			if (player.lowerBody.cockSpot.hasCock() && !player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				if (player.lowerBody.cockSpot.count() > 1) Render.text("  Your other cocks deliriously tighten and spurt their own loads at the same time, an uncontrollable storm of a high.");
			}
			else if (player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina())
			{
