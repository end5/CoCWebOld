...
export default class D3 {
	...
	public discoverD3(): boolean {
		if (Flags.list[FlagEnum.D3_DISCOVERED] == 0 && player.hasKeyItem("Zetaz's Map") >= 0 && player.level >= 10 && rand(5) == 0) {
	...
	private roomofmirrorsRoomFunc(): boolean {
		...
		if (Flags.list[FlagEnum.D3_MIRRORS_SHATTERED] == 1) MainScreen.text(" now shattered,");
		...
		if (player.hasKeyItem("Laybans") >= 0) {
			...
			if (Flags.list[FlagEnum.D3_ENTERED_MAGPIEHALL] == 1) MainScreen.text("  Your spirits rise. They look like they may very well be made of the same material as the screen in the basilisk hall.");
	...
	private magpiehallsRoomFunc(): boolean {
		if (Flags.list[FlagEnum.D3_JEAN_CLAUDE_DEFEATED] == 0) {
			if (Flags.list[FlagEnum.D3_ENTERED_MAGPIEHALL] == 0) {
				...
				if (Flags.list[FlagEnum.BENOIT_AFFECTION] == 100) MainScreen.text("  This can only be the hall that " + Game.bazaar.benoit.benoitMF("Benoit", "Benoite") + " once worked in.");
	...
	private westwalkRoomFunc(): boolean {
		...
		if (Flags.list[FlagEnum.D3_STATUE_DEFEATED] == 0) MainScreen.text(" see a massive statue with an immense hammer");
	...
	private eastwalkRoomFunc(): boolean {
		...
		if (Flags.list[FlagEnum.D3_STATUE_DEFEATED] == 0) MainScreen.text(" a massive statue with an immense hammer.");
		else MainScreen.text(" a mound of rubble, the scattered remains of the animated statue that you slew.");
		return false;
	}
