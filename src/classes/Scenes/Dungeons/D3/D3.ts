package classes.Scenes.Dungeons.D3 
{
	...
	public class D3 extends BaseContent
	{
		...
		private function roomofmirrorsRoomFunc():boolean
		{
			...
			if (flags[FlagEnum.D3_MIRRORS_SHATTERED] == 1) outputText(" now shattered,");
			...
			else
			{
				...
				if (flags[FlagEnum.D3_ENTERED_MAGPIEHALL] == 1) outputText("  Your spirits rise. They look like they may very well be made of the same material as the screen in the basilisk hall.");
				if (player.stats.int >= 70 || player.stats.sens >= 70) outputText("  Disquiet edges down your spine. Something about this place doesn’t feel right. The room seems faded at the corners, as if it’s not quite there.");
		...
		private function magpiehallsRoomFunc():boolean
		{
			if (flags[FlagEnum.D3_JEAN_CLAUDE_DEFEATED] == 0)
			{
				if (flags[FlagEnum.D3_ENTERED_MAGPIEHALL] == 0)
				{
					...
					if (flags[FlagEnum.BENOIT_AFFECTION] == 100) outputText("  This can only be the hall that " + getGame().bazaar.benoit.benoitMF("Benoit", "Benoite") + " once worked in."); 
		...
		private function fallbackFromMagpieHallS():void
		{
			...
			if (player.hasKeyItem("Laybans") < 0 && player.stats.int >= 50) outputText("  Surely the demons themselves are not immune to the basilisks’ glares - the darkened screen is proof of that. How do they interact with the creatures, then? Maybe if you keep poking around, you might find an answer.");
		...
		private function westwalkRoomFunc():boolean
		{
			...
			if (flags[FlagEnum.D3_STATUE_DEFEATED] == 0) outputText(" see a massive statue with an immense hammer");
		...
		private function eastwalkRoomFunc():boolean
		{
			...
			if (flags[FlagEnum.D3_STATUE_DEFEATED] == 0) outputText(" a massive statue with an immense hammer.");
			else outputText(" a mound of rubble, the scattered remains of the animated statue that you slew.");
			return false;
		}
