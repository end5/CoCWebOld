package classes.Scenes.Dungeons.D3 
{
	...
	public class IncubusMechanicScenes extends BaseContent
	{
		...
		public meetAtElevator():void
		{
			...
			else if (flags[FlagEnum.D3_MECHANIC_LAST_GREET] == MECHANIC_SUCKED)
			{
			...
			else if (flags[FlagEnum.D3_MECHANIC_LAST_GREET] == MECHANIC_PAID)
			{
			...
			else if (flags[FlagEnum.D3_MECHANIC_LAST_GREET] == MECHANIC_FOUGHT)
			{
		...
		private payDaToll():void
		{
			...
			menu();
			MainScreen.addButton(0, "Next", getGame().d3.exitD3);
		}
		...
		public suckIncubusDick():void
		{
			...
			if (player.lowerBody.vaginaSpot.hasVagina())
			{
				...
				if (player.wetness() <= 2) MainScreen.text(" even more soaked than normal");
				else if (player.wetness() <= 4) MainScreen.text(" wet with fresh lust");
		...
		private rideMechanicsCock():void
		{
			...
			if (player.tailType != 0) MainScreen.text(", tail draped across his face");
		...
		private rideMechanicsCockII(cType: number):void
		{
			...
			if (player.wetness() <= 2) MainScreen.text(", letting your free-flowing pussyjuice wash over it, lubricating it with your glorious girlcum");
			else if (player.wetness() <= 3) MainScreen.text(", letting your oh-so-juicy vagina drip and dribble, soaking it with your copious girlcum");
			...
			if (player.wetness() < 4) MainScreen.text(", even though you aren't normally a squirter");
		...
		private maleLossToMechanic(hpVictory:boolean):void
		{
			...
			if (player.lowerBody.cockSpot.count() > 1)
			{
				...
				if (player.demonCocks() == player.lowerBody.cockSpot.count()) allDemon = true;
		...
		private errybodyelseLossToMechanic(hpVictory:boolean):void
		{
			...
			if (player.isNaga()) MainScreen.text("r coils don't get sore");
			...
			if (player.lowerBody.vaginaSpot.list[0].clitLength > 5) MainScreen.text(", even if that makes it truly monstrous indeed");
