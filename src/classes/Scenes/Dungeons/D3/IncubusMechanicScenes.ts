 ...
 * @author Gedan
 */
export default class IncubusMechanicScenes {
	...
	private useLiftPostDefeat(): void {
		MainScreen.clearText();
		MainScreen.text("You spend a minute or two fiddling with the lift controls, trying to understand how, exactly, to control the contraption. When you think you've got a handle on the system, you tug on a lever with authority and hastily step onto the platform.");
	...
	public suckIncubusDick(): void {
		...
		if (player.stats.gems < 500) MainScreen.text("You spare a glance to your depleted pouches and admit, \"<i>I guess I'll suck your dick then.</i>\"");
		else MainScreen.text("You shrug and smile. \"<i>I guess now is as good a time as any to suck some dick, huh?</i>\"");
		...
		if (player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text(" You find your crotch becoming");
			if (player.lowerBody.vaginaSpot.get(0).vaginalWetness <= 2) MainScreen.text(" even more soaked than normal");
			else if (player.lowerBody.vaginaSpot.get(0).vaginalWetness <= 4) MainScreen.text(" wet with fresh lust");
			else MainScreen.text(" sticky with lust")
			MainScreen.text(" at his masculine scent and appearance.")
		}
		else if (player.lowerBody.cockSpot.hasCock()) {
			MainScreen.text(" You feel a heady, turgid warmth growing in your own crotch, mirroring the ardor displayed by the cock you're about to service.");
		}
		MainScreen.text(" A patch of neatly trimmed pubic hairs lead up his abdomen to a set well-defined muscles, half-hidden under his loose not-so-coveralls.");
	...
	public beatDaMechanic(hpVictory: boolean): void {
		...
		MainScreen.clearText();
		MainScreen.text("The mechanic collapses, his overalls");
		if (hpVictory) MainScreen.text(" falling off his prone from, shredded.");
		else MainScreen.text(" hanging open as he reaches down for his exposed, dangling cock to wildly masturbate.");
		MainScreen.text(" He seems surprisingly human like that, just as mortal as anyone. You could let him go, play with him some, or cleanse the world of another demon.");

		menu();
		MainScreen.addButton(0, "Kill", killMechanic);
		MainScreen.addButton(1, "Let Go", letMechanicGo, hpVictory);
		if (player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(2, "Buttfuck", buttfuckTheMechanic, hpVictory);
		if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.addButton(3, "Ride Cock", rideMechanicsCock);
	}

	private killMechanic(): void {
		Flags.list[FlagEnum.D3_MECHANIC_FIGHT_RESULT] = MECHANIC_KILLED;
		MainScreen.clearText();
		MainScreen.text("He proves blessedly easy to kill, and you roll the body off the cliffs to avoid alerting any of Lethice's other ilk.");
	...
	private letMechanicGo(hpVictory: boolean): void {
		...
		MainScreen.clearText();
		MainScreen.text("Feeling merciful, you point off the side of the cliff. <i>\"Leave this place and never return. If you do, I might have to kill you. I'd rather not shed any more blood than necessary, but I will do what I have to in order to cleanse this land. Leave and never return. Maybe you can find your own peace without driving everything around you to corruption.\"</i>");

		MainScreen.text("\n\nThe incubus");
		if (hpVictory) MainScreen.text(" slowly struggles to his feet");
		else MainScreen.text(" reluctantly stops masturbating");
		MainScreen.text(", nodding. His skin cracks as two leathery wings unfurl. <i>\"If that's how it has to be.... For what it's worth, I hope you win. Lethice is a bitch.\"</i> Slowly walking towards the cliff, he beats his wings and lifts off the ground. \"<i>Come visit me sometime. Maybe I can show you a good time as thanks.</i>\"");
	...
	private buttfuckTheMechanic(hpVictory: boolean): void {
		...
		if (!hpVictory) MainScreen.text(" throbbing hard");
		else MainScreen.text(" gently bobbing");
		MainScreen.text(" dick, but you've got a better idea. You got more than one look at the curvature of his butt while you were fighting, tempting your burgeoning bulge to the point of distraction. You hastily remove your [armor] and smile. [EachCock] springs up, full and hard. You take");
		if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(" one");
		else MainScreen.text(" it");
		MainScreen.text(" in hand, pumping slowly as you eye your tainted prey. He whimpers");
		if (hpVictory) MainScreen.text(" in need, and");
		else MainScreen.text(", but");
		MainScreen.text(" his cock seems to get even bigger and harder. The poor guy doesn't realize what a reaming he's in for.");
		...
		MainScreen.text("\n\nThe incubus's dick-flopping diminishes as it begins to properly engorge once more, aiming higher with each beat of the demon's heart as he nears his orgasm. You push him down a bit so that your " + CockDescriptor.describeCock(player, x) + " will press more powerfully across his prostate, panting from your own exertion. It's enough. The demon mewls out a long, high-pitched moan and erupts, painting a line of white up his overalls all the way to his chin. The next blobs of cum splatter off his chin. Each new ejaculation seems a little larger than the one before. You're doing your best to milk him from behind, and he's doing his best to whimper like a bitch and paint his face. A smile breaks across your face. It's more than a fair trade.");

		MainScreen.text("\n\nEventually, the spent demon finishes, dribbling semen so weak that it's practically clear into a salty puddle between his knees. You don't slow or stop. In fact, your " + CockDescriptor.describeCock(player, x) + " feels bigger and harder than it was a few seconds. The velvety folds of the demon's pre-slickened asshole are caressing you wonderfully, slowly egging you towards a climax of your own. You fuck him hard enough to knock him off-balance, dropping his cheek into his own spooge-puddle. Then, you cum, flooding your toy's asshole with its reward for a job well done.");

		////Low cum exit
		if (player.cumQ() <= 250) {
			MainScreen.text("\n\nYou pull");
			if (player.hasKnot(x)) MainScreen.text(", popping your knot");
			MainScreen.text(" out after depositing your meager load into your demonic fuck-hole. One slick strand of creamy spunk is leaking out over the demon's ballsack.");
		}
		//Medium cum exit
		else if (player.cumQ() <= 1000) {
			MainScreen.text("\n\nYou pull");
			if (player.hasKnot(x)) MainScreen.text(", popping your knot");
			MainScreen.text(" out of the demon's creampied fuck-hole with a smile. His stretched-out ass looks virtually glazed with cum, and you can see small rivers of it trickling down over his balls and slowly shrinking cock.");
		}
		//High cum exit
		else if (player.cumQ() <= 2500) {
			MainScreen.text("\n\nYou pull");
			if (player.hasKnot(x)) MainScreen.text(", popping your knot");
			MainScreen.text(" out of the demon's well-fucked asshole with a smile. As soon as the bulk of your dick unplugs him, a river of white spills out, washing over his balls and cock to drip down into his puddle. You smirk at the white-glazed demon. His butt is still full of your spunk.");
		}
		//Huge cum
		else {
			MainScreen.text("\n\nYou pull");
			if (player.hasKnot(x)) MainScreen.text(", popping your knot");
			MainScreen.text(" out after depositing a massive, belly-bulging load directly into the demon's guts. A tidal wave of goo bursts out of his over-pressurized asshole to fall on the ground, and as it dies off, small rivers of ivory spunk escape the lake in his ass to run down his balls and cock. His belly is");
			if (player.cumQ() <= 5000) MainScreen.text(" pudgy");
			else if (player.cumQ() <= 10000) MainScreen.text(" swollen");
			else MainScreen.text(" absolutely gravid");
			MainScreen.text(" with the distended weight of the cum packing his body. From time to time, fresh streamers of sperm squirt out of his thoroughly cream-pied backdoor.");
		}

		//No new paragraph
		MainScreen.text("The incubus slumps down into the puddled semen, gasping in exhaustion.");

		MainScreen.text("\n\nYou wipe your " + CockDescriptor.describeCock(player, x) + " off on his practically glowing bottom before turning to re-dress. When you turn back, he's gone, though a trail of white drops indicates the direction of his flight. It looks like he fled towards the cliff, perhaps growing wings. Oh well, that's one less obstacle between you and Lethice.");
	...
	private rideMechanicsCockII(cType: number): void {
		...
		//Dogdick
			...
			MainScreen.text("Giving the incubus's sweaty tool an affectionate squeeze, you decide, \"<i>Grow a dog-cock. Let's see if I can take the knot, shall we, pet?</i>\"");
		...
		if (player.lowerBody.vaginaSpot.get(0).vaginalWetness <= 2) MainScreen.text(", letting your free-flowing pussyjuice wash over it, lubricating it with your glorious girlcum");
		else if (player.lowerBody.vaginaSpot.get(0).vaginalWetness <= 3) MainScreen.text(", letting your oh-so-juicy vagina drip and dribble, soaking it with your copious girlcum");
		else MainScreen.text(", letting your lubricant drip over it");
		MainScreen.text(". You stuff two fingers inside and stroke deep before pulling them out and using them to spread you wide open. The " + monster.cockDescriptShort() + " pulsates with every beat of its demonic owner's heart, needing you every bit as much as you now need it.");
		...
		let mCockArea: number = monster.cockArea(0);
		...
		if (tightFit) MainScreen.text(" well-stretched");
		else MainScreen.text(" stuffed");
		MainScreen.text(" twat to silence him.");
		...
		if (player.statusAffects.has("IncubusBribed")) MainScreen.text(" Nothing but hentai mags for you to drain your balls with....");
		MainScreen.text("</i>\"");
		...
		if (player.lowerBody.vaginaSpot.get(0).clitLength >= 5) MainScreen.text(" You grab hold of it in your hand and start to stroke it to the tempo your rising and falling body is setting, pumping the oversensitive organ like a dick. Every stroke sends your walls into fits of fluttering ecstasy. It feels so good that you don't think you'll be able to stop until you're cumming and gushing all over your demonic lover.");
		else MainScreen.text(" You thumb at it, rubbing in slow circles around it. Sometimes you push on its hood. Other times you oh-so-gently squeeze at it. It feels so good that the touches drive you crazy with ecstasy, and your walls flutter excitedly around your demonic lover.");
		MainScreen.text(" His only reaction is to gasp and groan, balls lurching in his sack.");
		...
		if (cType == MECHANIC_HORZ_GOG) MainScreen.text(", the head flaring to seal itself against your cervix");
		else if (cType == MECHANIC_DOG_COCK) MainScreen.text(", the knot popping in just in time to start bloating, locking you in place");
		MainScreen.text(", and jism begins to shoot out in waves.");
		if (!player.isPregnant()) {
			MainScreen.text(" Your empty womb drinks up the virile, seed-filled sludge with abandon.");
			if (cType == MECHANIC_HORZ_GOG) MainScreen.text(" After all, that bestial horse-cock doesn't really leave it anywhere else to go. The demonic animal-cunt just continues spurting straight on in to have its way with your eggs, and you're sure there isn't a contraceptive this side of Mareth that could stop you from having his imps.");
			else if (cType == MECHANIC_DOG_COCK) MainScreen.text(" After all, that immense knot doesn't really leave it anywhere else to go. Once every fold is bathed in alabaster doggie-cream, the continued ejaculations start pouring in after your eggs. His knot is so big and stretching you so wide; you don't think he would pop out even if he filled your belly to look nine months pregnant. Worse, or perhaps better, you're sure with how thick and virile his seed feels that you'll be getting pregnant with his imps from this.");
			player.knockUp(PregnancyType.IMP, PregnancyType.INCUBATION_IMP);
		}
		else {
			MainScreen.text(" Your blocked womb deflects the seed, forcing it all to stay in your channel");
			if (cType != MECHANIC_DOG_COCK) MainScreen.text(" and drip out over his balls, much to his chagrin.");
			else MainScreen.text(" and stretch you further.");
		}
		...
		if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < 4) MainScreen.text(", even though you aren't normally a squirter");
		MainScreen.text(". The pleasure is so great that you can feel electric impulses arcing between neurons, short-circuiting your throughs into involuntary ecstasy. Your eyes roll back, and your [hips] twitch and shake, vibrating around the object that brough you to this perfect");
		if (!player.isPregnant()) MainScreen.text(" insemination");
		else MainScreen.text(" tryst");
		MainScreen.text(".");
	...
	private maleLossToMechanic(hpVictory: boolean): void {
		...
		//Lust loss
		if (!hpVictory) {
			MainScreen.text("Your [legs] are quivering too wildly to support you anymore, and you collapse, splaying yourself out so that your rigid cock");
			...
			//Medium cocks
				...
				MainScreen.text("\"<i>You're so... average. I suppose you'll do, if the succubi feel like having a drab, everyday orgasm.</i>\" The incubus shrugs. \"<i>A dick will do...</i>\"");
			}
			//Big cocks
			...
			//Huge cocks
				...
				MainScreen.text("\"<i>Seriously?</i>\" the incubus asks. \"<i>How am I supposed to work with something so gargantuan?</i>\" He disdainfully pulls a vial from his pocket and upends it over your crotch. The effect is immediate. [Eachcock] shrinks from ridiculously large down to horse-sized. <i>\"There, that's better.\"</i>");
				//Shrink down to 18" by 3"
				player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockLength = 18;
				player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockThickness = 3;
			}
			...
			MainScreen.text(" length inside any hole and thrust until you're quivering in orgasmic release. That gorgeous demon-cock taunts you, swaying just out of reach.");
		}
		//HP Loss
		else {
			MainScreen.text("Dropping prone, you gasp for breath, struggling against your injured body to rise. Your muscles burn with the effort, exhausted from the fight, and ultimately, you fail to stand. Slumping down on the ground, you're forced to come to terms with your defeat. The incubus has won. You came all this way, gave your best, and got smacked down by a lower demon for all your troubles. Some champion you are.");

			MainScreen.text("\n\nLeering down at you, the incubus swiftly tears away your [armor], his fingers suddenly razor-sharp claws. He is precise in his attentions, leaving your [skin] pristine and unbroken. You get the impression he doesn't want damaged goods. He regards your " + CockDescriptor.describeMultiCockShort(player) + " with interest, judging them.");
		...
		if (player.lowerBody.cockSpot.count() > 1) {
			...
			if (player.lowerBody.cockSpot.countType(CockType.DEMON) == player.lowerBody.cockSpot.count()) allDemon = true;

			if (!allDemon) MainScreen.text(" The skin of your transforming length is darkening to a purplish hue, and in the wake of the magical cock-ring, nodules are slowly emerging, growing into an imitation of the cock you're still suckling.");
			MainScreen.text(" The incubus chuckles, easily pushing the ring the rest of the way down, smiling at the way your demonic tip pulsates and leaks pre-cum. Once it's nestled firmly against your groin, your new master withdraws his hands (and his cock). <b>Your singular, demonic cock is impossibly hard.</b>");
		}
		else {
			//Single
	...
	private maleLossToMechanicII(): void {
		MainScreen.clearText();
		MainScreen.text("An agonizingly perfect pussy is suddenly pressing down on your cock, subsuming it in slick, hot depths that cling and squeeze with undulations so skillful you'd be cumming already if it weren't for the magic ring.");
	...
	private errybodyelseLossToMechanic(hpVictory: boolean): void {
		...
		//Genderless HP Loss
		else if (!player.lowerBody.vaginaSpot.hasVagina() && hpVictory) {
			MainScreen.text("Collapsing onto the ground, you look up at the demonic aggressor with hate in your eyes, unwilling to admit defeat even when staring full-on into its face. The incubus, for his part, simply smiles down at you as he steps closer, claws clicking noisily on the smooth, tiled pavement.");
			...
			if (player.upperBody.chest.hasFuckableNipples()) MainScreen.text(" slipping inside a [nipple] to thrust just like your other hand. You work your upper and lower pussies like a woman possessed.");
			else {
				MainScreen.text(" pinching and tugging on");
				if (player.upperBody.chest.countNipples() > player.totalBreasts()) MainScreen.text(" nipple after nipple, your myriad breastly endowments providing you with oh so many buttons to tend to");
				else MainScreen.text("your [nipples] one after another");
			}
			MainScreen.text(" Alas, all good things must come to an end, and your lovely, lovely little lollipop is rudely pulled from your questing lips with a noisy pop.");
		...
		if (player.lowerBody.isNaga()) MainScreen.text("r coils don't get sore");
		else if (!player.isBiped()) MainScreen.text(" don't even need to support yourself anymore");
		else MainScreen.text(" knees don't get sore");
		MainScreen.text(". How wonderful! If you move just right, you can make the whole contraption sway slightly to get a proper dick-sucking rhythm going. His pearly pre-cream oozes onto your tongue, and you swallow as much of it as you can.");
		...
		if (player.lowerBody.vaginaSpot.get(0).clitLength > 5) MainScreen.text(", even if that makes it truly monstrous indeed");
		MainScreen.text(". It drips long, sticky strands of girlish lust as you swallow, only becoming more soaked as time passes as your body becomes even more productive. A plaintive whimper escapes around the edges of your phallic gag, but your cry falls on deaf ears. The incubus is too busy fitting pipes together beneath you to care.");
