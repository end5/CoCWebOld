	...
	public class IncubusMechanicScenes extends BaseContent
	{
		...
		public function meetAtElevator():void
		{
			if (flags[FlagEnum.D3_MECHANIC_LAST_GREET] == MECHNIC_NO_SELECTION)
			{
				...
				if (player.statusAffects.has("IncubusBribed")) outputText(", a rolled-up hentai magazine tucked neatly into the chest-pocket of his overalls");
		...
		public function suckIncubusDick():void
		{
			...
			if (player.stats.gems < 500) outputText("You spare a glance to your depleted pouches and admit, \"<i>I guess I'll suck your dick then.</i>\"");
		...
		private function rideMechanicsCockII(cType: number):void
		{
			...
			if (player.statusAffects.has("IncubusBribed")) outputText(" Nothing but hentai mags for you to drain your balls with....");
			outputText("</i>\"");
		...
		private function errybodyelseLossToMechanic(hpVictory:boolean):void
		{
			...
			else if (!player.lowerBody.vaginaSpot.hasVagina() && hpVictory)
			{
				...
				if (player.upperBody.chest.hasFuckableNipples()) outputText(" slipping inside a [nipple] to thrust just like your other hand. You work your upper and lower pussies like a woman possessed.");
				else
				{
					outputText(" pinching and tugging on");
					if (player.upperBody.chest.countNipples() > player.totalBreasts()) outputText(" nipple after nipple, your myriad breastly endowments providing you with oh so many buttons to tend to");
			...
			if (player.lowerBody.vaginaSpot.list[0].clitLength > 5) outputText(", even if that makes it truly monstrous indeed");
