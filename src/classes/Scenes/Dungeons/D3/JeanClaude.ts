export default class JeanClaude extends Monster {
	private doubleAttack(): void {
		MainScreen.text("The monstrous basilisk suddenly lunges forward, snarling wordlessly as he swings his cutlass at you twice, the blows of a savage yet expert swordsman.\n\n");
	...
	public handleTease(lustDelta: number, successful: boolean): void {
		if (!player.statusAffects.has("RemovedArmor")) {
			...
			if (!successful) {
				...
				player.statusAffects.add(new StatusAffect("RemovedArmor", 0, 0, 0, 0)));

				if (this.findStatusAffect(StatusAffects.JCLustLevel) < 0) {
					this.statusAffects.add(new StatusAffect("JCLustLevel", 1, 0, 0, 0)));
					lustVuln += 0.1;
				}
				else {
					this.statusAffects.get("JCLustLevel").value1 = 1;
				}
	...
	public JeanClaude() {
		...
		this.createCock(12, 2, CockType.LIZARD);
		this.balls = 2;
		this.ballSize = 6;
