...
export default class EncapsulationPod extends Monster {
	...
	public get long(): string {
		//[Round 1 Description]
		let _long: string;
		if (findStatusAffect(StatusAffects.Round) < 0) _long = "You're totally trapped inside a pod!  The walls are slimy and oozing moisture that makes the air sickeningly sweet.  It makes you feel a little dizzy.  Tentacles are climbing up your " + LowerBodyDescriptor.describeLegs(player) + " towards your crotch, doing their best to get under you " + player.inventory.armor.displayName + ".  There's too many to try to pull away.  Your only chance of escape is to create a way out!";
		//[Round 2 Description]
		else if (statusAffects.get("Round").value1 == 2) {
			...
			_long += "inch of your " + LowerBodyDescriptor.describeLegs(player) + ", crotch, and " + game.ButtDescriptor.describeButt(player) + ".";
		}
		//[Round 3 Description]
		else if (statusAffects.get("Round").value1 == 3) {
		...
		//[Round 4 Description]
			...
			if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) _long += "the underside of your " + game.BreastDescriptor.describeAllBreasts(player);
			else _long += "your chest";
			_long += " in moments.  You're being fucked by a bevy of tentacles while your nipples are ";
			if (!player.upperBody.chest.hasFuckableNipples()) _long += "fondled ";
			else _long += "fucked ";
