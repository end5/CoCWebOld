package classes.Scenes.Dungeons.Factory
{
	...
	public class IncubusMechanic extends Monster {
		...
		private cockTripAttack():void {
			...
			MainScreen.text("The incubus lunges forward in a clumsy attack that you start to side-step, only to feel something grip behind your " + game.buttDescript() + " and pull your " + player.legs() + " out from under you.");
			if ((player.stats.spe-30) > rand(60)) {
				MainScreen.text("  You spin as you fall, twisting your " + player.legs() + " free and springing back to your " + player.feet() + " unharmed.");
			}
			else { //Fall down go boom
				MainScreen.text("  You land hard on your ass, momentarily stunned as the demonic cock-tentacle curls around your " + player.legs() + ", smearing them with oozing demonic fluids.");
				if (player.lust >= 80 || player.stats.cor >= 80) {
					MainScreen.text("  Moaning with desire, you lick your lips as you slide your well-lubricated " + player.legs() + " free.  You gather a dollop of cum and lick it seductively, winking at the incubus and hoping to make him cave into his desire.");
					game.dynStats("lus", 13, "cor", 1);
				}
				else if (player.lust >= 50 || player.stats.cor >= 50) {
					MainScreen.text("  Blushing at the scent and feel of cum on your " + player.legs() + ", you twist and pull free.  You find yourself wondering what this demon's dick would taste like.");
					game.dynStats("lus", 8 + player.stats.cor / 20);
				}
				else {
					MainScreen.text("  Disgusted, you pull away from the purplish monstrosity, the act made easier by your well-slimed " + player.legs() + ".");
			...
			switch (rand(3)) {
				...
				case 1: //Chest
					if (player.upperBody.chest.hasFuckableNipples()) {
						MainScreen.text(allBreastsDescript() + ".  The gooey demon-seed oozes and slides over you with a mind of its own, forcing its way into your open nipples.  You can feel it moving around inside you, doing its best to prepare you for its master.");
						game.dynStats("lus", 3);
						if (player.findStatusAffect(StatusAffects.DemonSeed) < 0)
							player.statusAffects.add(new StatusAffect("DemonSeed", 5, 0, 0, 0)));
					...
					else MainScreen.text(allBreastsDescript() + ".  Thankfully it doesn't seem to have much effect.");
					break;
				default: //Crotch
					if (player.lowerBody.vaginaSpot.count() > 0) {
						MainScreen.text("crotch.  The gooey demon-seed oozes and slides over you with a mind of its own, forcing its way past your " + player.armorName + " and into your " + vaginaDescript(0) + ".  You can feel it moving around inside you, doing its best to prepare you for its master.");
						game.dynStats("lus", 3);
						if (player.findStatusAffect(StatusAffects.DemonSeed) < 0)
							player.statusAffects.add(new StatusAffect("DemonSeed", 5, 0, 0, 0)));
						else player.statusAffects.get("DemonSeed").value1 = 8;
