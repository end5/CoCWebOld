export default class OmnibusOverseer extends Monster {
	...
	private milkAttack(): void {
		...
		if ((player.stats.spe > 50 && rand(4) == 0) || (player.perks.has("Evade") && rand(3) == 0) || (player.perks.has("Misdirection") && rand(4) == 0 && player.inventory.armor.displayName == "red, high-society bodysuit")) {
		...
		//You didn't dodge
			...
			if (rand(2) == 0) {
				...
				MainScreen.text("The milk splashes into your " + player.inventory.armor.displayName + ", soaking you effectively.  ");
				if (player.lowerBody.cockSpot.count() > 0) {
					MainScreen.text("Your " + CockDescriptor.describeCock(player, 0) + " gets hard as the milk lubricates and stimulates it.  ");
			...
			if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier > 1) MainScreen.text("Milk dribbles from your " + BreastDescriptor.describeAllBreasts(player) + " in sympathy.");
		}
		game.combatRoundOver();
	}
	...
	public OmnibusOverseer() {
		...
		this.upperBody.head.hairLength = 42;
