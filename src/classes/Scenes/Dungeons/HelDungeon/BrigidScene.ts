...
package classes.Scenes.Dungeons.HelDungeon
{
	...
	public class BrigidScene extends BaseContent
	{
		...
		public pcDefeatsBrigid():void {
			...
			cleanupAfterCombat();
			flags[FlagEnum.HEL_BRIGID_DEFEATED] = 1;
		}
		...
		public pcDefeatedByBrigid():void {
			...
			if(flags[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] == 0) {
				...
				menu();
				MainScreen.addButton(0, "Next", getGame().harpyQueenBeatsUpPCBadEnd, true);
			}
			else {
		...
		private brigitPostQueenDeathBadEndII():void {
			...
			if(player.gender <= 1 || player.pregnancyIncubation > 0) {
				...
				if(player.lowerBody.cockSpot.count() > 1 || player.cumQ() > 1000) MainScreen.text("  " + SMultiCockDesc() + " holds forth for a while, until Kiri's ass, pussy, and tailfeathers are soaked with oozing, white cum.");
			...
			//(else if not visibly pregnant F or H)
				...
				if(player.lowerBody.cockSpot.hasCock()) MainScreen.text(sMultiCockDesc() + " bobbing up and down, drooling pre-cum, as ");
