...
package classes.Scenes.Dungeons.HelDungeon
{
	...
	public class BrigidScene extends BaseContent
	{
		...
		public pcDefeatsBrigid():void {
			...
			else Render.text("too badly beaten");
			Render.text(" to continue the fight.  You quickly snatch the keys from inside her shield, and hook them onto your own belt.");
		...
		public pcDefeatedByBrigid():void {
			...
			if(flags[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] == 0) {
				...
				Render.text("\"<i>That's it?!</i>\" Brigid screams, as you collapse in front of her.  \"<i>A weak little piece of trash like you took out our queen?</i>\"");
		...
		private brigitPostQueenDeathBadEndII():void {
			...
			if(player.gender <= 1 || player.pregnancyIncubation > 0) {
				...
				if(!player.lowerBody.cockSpot.hasCock()) {
					...
					if(player.pregnancyIncubation > 0) Render.text("  Once this disgusting thing you're carrying falls out of you, we'll use you more traditionally, but...</i>\"  She pauses to press uncomfortably on your pregnant belly, then resumes speaking.  \"<i>I have no intention of letting you sit idle for however long that takes.");
					Render.text("  For now, I've given you the most obvious modification - I can think of something else later.</i>\"");
				...
				if(player.lowerBody.cockSpot.count() > 1 || player.cumQ() > 1000) Render.text("  " + SMultiCockDesc() + " holds forth for a while, until Kiri's ass, pussy, and tailfeathers are soaked with oozing, white cum.");
				Render.text("  Next to you, the salamander on the other table climaxes as well, dumping a load into Hel that sizzles when it leaks onto his fiery, iron-bound tail.  Your former lover squirms and shivers as the scalding-hot load fills her cunt, and her own prick twitches weakly, depositing a small string of bubbling jism on the stone table.");
			...
			//(else if not visibly pregnant F or H)
				...
				if(player.lowerBody.cockSpot.hasCock()) Render.text(" as the phoenix's cunt slides onto your [cock smallest]");
				Render.text(".  Brigid makes her way up the stairs, leaving you and the others at the mercy of the assorted horde.  The harpy rides your mouth greedily, knocking your head into the hard stone floor, and the last thing you can see around her feathery thighs before you slip into unconsciousness again is Kiri being pulled roughly from her resting place atop Hel by a mean-looking, wide-hipped harpy matron who takes her place, sitting sideways and drawing her talons sadistically across Hel's vulnerable chest to smear Kiri's seed around...");
