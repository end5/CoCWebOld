 ...
 */
export default class BrigidScene {
	//Brigid the Jailer -- PC Victorious
	public pcDefeatsBrigid(): void {
		MainScreen.clearText();
		MainScreen.text("The harpy jailer collapses, ");
		if (monster.lust > 99) MainScreen.text("too turned on");
		else MainScreen.text("too badly beaten");
		MainScreen.text(" to continue the fight.  You quickly snatch the keys from inside her shield, and hook them onto your own belt.");
	...
	public pcDefeatedByBrigid(): void {
		...
		if (Flags.list[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] == 0) {
			MainScreen.text("\"<i>Tsk tsk tsk,</i>\" the harpy jailer croons, looming over you as you slump to the ground. \"<i>You shouldn't have messed with me, bitch!</i>\" she snaps, giving you a rough kick to the side. \"<i>Now, let's see what Mother has to say about this...</i>\"");
			//(Go to \"<i>Harpy Breeding Slut</i>\" Bad End)
			menu();
			MainScreen.addButton(0, "Next", Game.harpyQueenBeatsUpPCBadEnd, true);
		}
		else {
			MainScreen.text("\"<i>That's it?!</i>\" Brigid screams, as you collapse in front of her.  \"<i>A weak little piece of trash like you took out our queen?</i>\"");
	...
	private brigitPostQueenDeathBadEndII(): void {
		...
		if (player.gender <= 1 || player.pregnancyIncubation > 0) {
			...
			if (!player.lowerBody.cockSpot.hasCock()) {
				...
				if (player.pregnancyIncubation > 0) MainScreen.text("  Once this disgusting thing you're carrying falls out of you, we'll use you more traditionally, but...</i>\"  She pauses to press uncomfortably on your pregnant belly, then resumes speaking.  \"<i>I have no intention of letting you sit idle for however long that takes.");
				MainScreen.text("  For now, I've given you the most obvious modification - I can think of something else later.</i>\"");
			...
			if (player.lowerBody.cockSpot.count() > 1 || player.cumQ() > 1000) MainScreen.text("  " + CockDescriptor.describeMultiCockSimpleOne(player, true) + " holds forth for a while, until Kiri's ass, pussy, and tailfeathers are soaked with oozing, white cum.");
			MainScreen.text("  Next to you, the salamander on the other table climaxes as well, dumping a load into Hel that sizzles when it leaks onto his fiery, iron-bound tail.  Your former lover squirms and shivers as the scalding-hot load fills her cunt, and her own prick twitches weakly, depositing a small string of bubbling jism on the stone table.");
		...
		//(else if not visibly pregnant F or H)
			...
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text(" as the phoenix's cunt slides onto your [cock smallest]");
			MainScreen.text(".  Brigid makes her way up the stairs, leaving you and the others at the mercy of the assorted horde.  The harpy rides your mouth greedily, knocking your head into the hard stone floor, and the last thing you can see around her feathery thighs before you slip into unconsciousness again is Kiri being pulled roughly from her resting place atop Hel by a mean-looking, wide-hipped harpy matron who takes her place, sitting sideways and drawing her talons sadistically across Hel's vulnerable chest to smear Kiri's seed around...");
