	...
	public class BrigidScene extends BaseContent
	{
		...
		private function brigitPostQueenDeathBadEndII():void {
			...
			if(player.gender <= 1 || player.pregnancyIncubation > 0) {
				...
				if(player.lowerBody.cockSpot.count() > 1 || player.cumQ() > 1000) outputText("  " + SMultiCockDesc() + " holds forth for a while, until Kiri's ass, pussy, and tailfeathers are soaked with oozing, white cum.");
			...
			//(else if not visibly pregnant F or H)
				...
				if(player.lowerBody.cockSpot.hasCock()) outputText(sMultiCockDesc() + " bobbing up and down, drooling pre-cum, as ");
				...
				if(player.lowerBody.cockSpot.hasCock()) outputText(" as the phoenix's cunt slides onto your [cock smallest]");
