...
export default class CharacterAppearanceMenu implements Menu {
    ...
    private heightRace(character: Character) {
        // Determine race type:
    ...
    private face(character: Character) {
        if (character.torso.neck.head.face.type === FaceType.HUMAN || character.torso.neck.head.face.type === FaceType.SHARK_TEETH || character.torso.neck.head.face.type === FaceType.BUNNY || character.torso.neck.head.face.type === FaceType.SPIDER_FANGS || character.torso.neck.head.face.type === FaceType.FERRET_MASK) {
            ...
            else if (character.torso.neck.head.face.type === FaceType.SPIDER_FANGS)
                DisplayText("  A set of retractable, needle-like fangs sit in place of your canines and are ready to dispense their venom.");
            else if (character.torso.neck.head.face.type === FaceType.FERRET_MASK)
        ...
        else if (character.torso.neck.head.face.type === FaceType.FERRET) {
            if (character.skin.type === SkinType.PLAIN)
                ...
                DisplayText("  Your face is coated in " + character.torso.neck.head.hair.color + " fur with [skin] underneath, an adorable cross between human and ferret features.  It is complete with a wet nose and whiskers.");
        }
    ...
    private eyes(character: Character) {
        ...
        else if (character.torso.neck.head.face.eyes.type === EyeType.BLACK_EYES_SAND_TRAP)
            DisplayText("  Your eyes are solid spheres of inky, alien darkness.");
    }

    private hair(character: Character) {
        // if bald
        if (character.torso.neck.head.hair.length === 0) {
            if (character.skin.type === SkinType.FUR)
                DisplayText("  You have no hair, only a thin layer of fur atop of your head.  ");
            else DisplayText("  You are totally bald, showing only shiny " + character.skin.tone + " " + character.skin.desc + " where your hair should be.");
    ...
    private tongue(character: Character) {
        ...
        else if (character.torso.neck.head.face.tongue.type === TongueType.DEMONIC)
        ...
        else if (character.torso.neck.head.face.tongue.type === TongueType.DRACONIC)
    ...
    private horns(character: Character) {
        // Demonic horns
        if (character.torso.neck.head.horns.type === HornType.DEMON) {
            if (character.torso.neck.head.horns.amount === 2)
                DisplayText("  A small pair of pointed horns has broken through the " + character.skin.desc + " on your forehead, proclaiming some demonic taint to any who see them.");
            if (character.torso.neck.head.horns.amount === 4)
                DisplayText("  A quartet of prominent horns has broken through your " + character.skin.desc + ".  The back pair are longer, and curve back along your head.  The front pair protrude forward demonically.");
            if (character.torso.neck.head.horns.amount === 6)
                DisplayText("  Six horns have sprouted through your " + character.skin.desc + ", the back two pairs curve backwards over your head and down towards your neck, while the front two horns stand almost eight inches long upwards and a little forward.");
            if (character.torso.neck.head.horns.amount >= 8)
                DisplayText("  A large number of thick demonic horns sprout through your " + character.skin.desc + ", each pair sprouting behind the ones before.  The front jut forwards nearly ten inches while the rest curve back over your head, some of the points ending just below your ears.  You estimate you have a total of " + Utils.numToCardinalText(character.torso.neck.head.horns.amount) + " horns.");
        }
        // Minotaur horns
        if (character.torso.neck.head.horns.type === HornType.COW_MINOTAUR) {
            if (character.torso.neck.head.horns.amount < 3)
            ...
            if (character.torso.neck.head.horns.amount >= 3 && character.torso.neck.head.horns.amount < 6)
                DisplayText("  Two moderately sized horns grow from your forehead, similar in size to those on a young bovine.");
            if (character.torso.neck.head.horns.amount >= 6 && character.torso.neck.head.horns.amount < 12)
                DisplayText("  Two large horns sprout from your forehead, curving forwards like those of a bull.");
            if (character.torso.neck.head.horns.amount >= 12 && character.torso.neck.head.horns.amount < 20)
            ...
            if (character.torso.neck.head.horns.amount >= 20)
        ...
        if (character.torso.neck.head.horns.type === HornType.DRACONIC_X2) {
            DisplayText("  A pair of " + Utils.numToCardinalText(character.torso.neck.head.horns.amount) + " inch horns grow from the sides of your head, sweeping backwards and adding to your imposing visage.");
        }
        // Super lizard horns
        if (character.torso.neck.head.horns.type === HornType.DRACONIC_X4_12_INCH_LONG)
        ...
        // Antlers!
        if (character.torso.neck.head.horns.type === HornType.ANTLERS) {
            if (character.torso.neck.head.horns.amount > 0)
                DisplayText("  Two antlers, forking into " + Utils.numToCardinalText(character.torso.neck.head.horns.amount) + " points, have sprouted from the top of your head, forming a spiky, regal crown of bone.");
        }
        // BODY PG HERE
        DisplayText("You have a humanoid shape with the usual torso, arms, hands, and fingers.");
    }

    private wings(character: Character) {
        ...
        if (character.torso.wings.type === WingType.BAT_LIKE_TINY)
        ...
        if (character.torso.wings.type === WingType.FEATHERED_LARGE)
            DisplayText("  A pair of large, feathery wings sprout from your back.  Though you usually keep the " + character.torso.neck.head.hair.color + "-colored wings folded close, they can unfurl to allow you to soar as gracefully as a harpy.");
        ...
        else if (character.torso.wings.type === WingType.DRACONIC_LARGE)
        ...
        else if (character.torso.wings.type === WingType.GIANT_DRAGONFLY)
        ...
        else if (character.torso.arms.type === ArmType.SPIDER)
    ...
    private hips(character: Character) {
        if (character.torso.hips.legs.type === LegType.PONY)
        ...
        if (!character.torso.hips.legs.isTaur()) {
            if (character.thickness > 70) {
                ...
                if (character.torso.hips.rating >= 10 && character.torso.hips.rating < 15)
                    DisplayText(" that would be much more noticeable if you weren't so wide-bodied, and");
                if (character.torso.hips.rating >= 15 && character.torso.hips.rating < 20)
    ...
    private butt(character: Character) {
        if (character.torso.hips.legs.isTaur()) {
            // FATBUTT
            if (character.tone < 65) {
                ...
                if (character.torso.butt.rating >= 6 && character.torso.butt.rating < 10)
                    DisplayText(" is fairly plump and healthy.");
                if (character.torso.butt.rating >= 10 && character.torso.butt.rating < 15)
                    DisplayText(" jiggles a bit as you trot around.");
                if (character.torso.butt.rating >= 15 && character.torso.butt.rating < 20)
    ...
    private tail(character: Character) {
        ...
        if (character.torso.tails.reduce(Tail.HasType(TailType.SPIDER_ABDOMEN), false)) {
            ...
            const spiderButt = character.torso.tails.filter(Tail.FilterType(TailType.SPIDER_ABDOMEN)).sort(Tail.VenomMost)[0];
        ...
        if (character.torso.tails.reduce(Tail.HasType(TailType.BEE_ABDOMEN), false)) {
            const beeButt = character.torso.tails.filter(Tail.FilterType(TailType.BEE_ABDOMEN)).sort(Tail.VenomMost)[0];
        ...
        if (character.torso.tails.reduce(Tail.HasType(TailType.SHARK), false)) {
        ...
        if (character.torso.tails.reduce(Tail.HasType(TailType.BUNNY), false))
            ...
            else DisplayText("  " + Utils.numToCardinalCapText(character.torso.tails.count) + " swishing " + character.torso.neck.head.hair.color + " fox's tails extend from your " + ButtDescriptor.describeButt(character) + ", curling around your body - the soft fur feels lovely.");
        }
        ...
        // appearance
        else if (character.torso.tails.reduce(Tail.HasType(TailType.RACCOON), false)) {
            DisplayText("  A black-and-" + character.torso.neck.head.hair.color + "-ringed raccoon tail waves behind you.");
        }
        else if (character.torso.tails.reduce(Tail.HasType(TailType.MOUSE), false)) {
            // appearance
            DisplayText("  A naked, " + character.skin.tone + " mouse tail pokes from your butt, dragging on the ground and twitching occasionally.");
        }
    }

    private lowerBody(character: Character) {
        ...
        else if (character.torso.hips.legs.type === LegType.FERRET) DisplayText("  Two furry, digitigrade legs form below your [hips].  The fur is thinner on the feet, and your toes are tipped with claws.");
        else if (character.torso.hips.legs.type === LegType.HOOFED)
            DisplayText("  Your legs are muscled and jointed oddly, covered in fur, and end in a pair of bestial hooves.");
        else if (character.torso.hips.legs.type === LegType.DOG)
            DisplayText("  Two digitigrade legs grow downwards from your waist, ending in dog-like hind-paws.");
        else if (character.torso.hips.legs.type === LegType.NAGA)
        ...
        // Horse body is placed higher for readability purposes
        ...
        else if (character.torso.hips.legs.type === LegType.DEMONIC_CLAWS)
        ...
        else if (character.torso.hips.legs.type === LegType.BEE)
            ...
            DisplayText("  Your legs are covered with " + character.torso.neck.head.hair.color + " plumage.  Thankfully the thick, powerful thighs are perfect for launching you into the air, and your feet remain mostly human, even if they are two-toed and tipped with talons.");
        else if (character.torso.hips.legs.type === LegType.KANGAROO)
            DisplayText("  Your furry legs have short thighs and long calves, with even longer feet ending in prominently-nailed toes.");
        else if (character.torso.hips.legs.type === LegType.CHITINOUS_SPIDER_LEGS)
        ...
        else if (character.torso.hips.legs.type === LegType.DRIDER_LOWER_BODY)
    ...
    private pregnancy(character: Character) {
        ...
        if (buttWomb.isPregnantWith(PregnancyType.FROG_GIRL) || buttWomb.isPregnantWith(PregnancyType.SATYR)) {
            if (womb.isPregnantWith(PregnancyType.OVIELIXIR_EGGS)) {
                // Compute size
                const eggAmount = character.statusAffects.get(StatusAffectType.Eggs).value3 + character.statusAffects.get(StatusAffectType.Eggs).value2 * 10;
    ...
    private neck(character: Character) {
    ...
    private chest(character: Character) {
        ...
        if (chest.count === 1) {
            ...
            if (firstRow.nipples.count === 1)
                DisplayText(Utils.numToCardinalText(firstRow.nipples.count) + " " + firstRow.nipples.length + "-inch " + BreastDescriptor.describeNipple(character, firstRow) + ".");
            else
                DisplayText(Utils.numToCardinalText(firstRow.nipples.count) + " " + firstRow.nipples.length + "-inch " + BreastDescriptor.describeNipple(character, firstRow) + "s.");
            ...
            for (let rowIndex = 0; rowIndex < chest.count; rowIndex++) {
                ...
                if (breastRow.nipples.count === 1)
                    DisplayText(Utils.numToCardinalText(breastRow.nipples.count) + " " + chest.sort(BreastRow.BreastRatingLargest)[0].nipples.length + "-inch " + BreastDescriptor.describeNipple(character, breastRow) + " each.");
                else
                    DisplayText(Utils.numToCardinalText(breastRow.nipples.count) + " " + chest.sort(BreastRow.BreastRatingLargest)[0].nipples.length + "-inch " + BreastDescriptor.describeNipple(character, breastRow) + "s each.");
    ...
    private lowerBodySex(character: Character) {
    ...
    private cocks(character: Character) {
        // Cock stuff!
    ...
    private balls(character: Character) {
        if (character.torso.balls.quantity > 0) {
            ...
            DisplayText("  You estimate each of them to be about " + Utils.numToCardinalText(Math.round(character.torso.balls.size)) + " ");
    ...
    private vaginas(character: Character) {
    ...
    private noReproductiveOrgans(character: Character) {
    ...
    private butthole(character: Character) {
    ...
    private piercing(character: Character) {
    ...
    private gems(character: Character) {
