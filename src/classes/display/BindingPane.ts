package classes.display 
{
	...
	{	
		...
		private let _keyDict:Dictionary;
		
		private let _functions:Array;
		private let _newFuncs:Array;
		
		private let _content:MovieClip;
		private let _contentChildren: number;
		...
		public BindingPane(inputManager:InputManager, xPos: number, yPos: number, width: number, height: number, uiscrollwidth: number) 
		{
			...
			*  was literally on crack, I SWEAR TO GOD. */
			...
			let blank:MovieClip = new MovieClip();
		...
		private InitContentObjects():void
		{
			...
			let helpLabel:TextField = new TextField();
			...
			for (let i: number = 0; i < _functions.length; i++)
			{
				...
				let newLabel:BindDisplay = new BindDisplay(this.width);
				...
				let genPrimaryCallback:Function = function(funcName:string, inMan:InputManager):Function
				{
				...
				let genSecondaryCallback:Function = function(funcName:string, inMan:InputManager):Function
				{
					return function():void
					{
		...
		private UpdateContentObjects():void
		{
			for (let i: number = 0; i < _functions.length; i++)
			{
				let currLabel:BindDisplay = _content.getChildByName(_functions[i].Name) as BindDisplay;
		...
		private PopulateKeyboardDict():void
		{
			...
			let keyNames:XMLList = keyDescriptions..constant.@name;
