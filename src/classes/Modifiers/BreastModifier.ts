...
export default class BreastModifier {
    public static growSmallestBreastRow(character: Character, amount: number, rowsGrown: number, display: boolean) {
        const chest = character.torso.chest;
        if (chest.count === 0)
            return;

        // Chance for "big tits" perked characters to grow larger!
        if (character.perks.has(PerkType.BigTits) && Utils.chance(33) && amount < 1)
            amount = 1;
    ...
    public static growTopBreastRowDownwards(character: Character, amount: number, rowsGrown: number, display: boolean) {
        const chest = character.torso.chest;
        if (chest.count === 0)
            return;
    ...
    public static growTopBreastRow(character: Character, amount: number, rowsGrown: number, display: boolean) {
        const chest = character.torso.chest;
        if (chest.count === 0)
            return;
     ...
     * @param growthType
     */
    public static growTits(character: Character, amount: number, rowsGrown: number, growthType: number): void {
    }

    public static shrinkTits(character: Character, ignoreHyperHappy: boolean = false): void {
        if (Flags.list[FlagEnum.HYPER_HAPPY] && !ignoreHyperHappy) {
            return;
        }
        if (character.torso.chest.count === 1) {
            ...
            if (topRow.rating > 0) {
                ...
                if (topRow.rating >= 1 && Utils.rand(100 / 2) && !character.perks.has(PerkType.BigTits)) {
                    superShrink = true;
                    topRow.rating--;
                }
