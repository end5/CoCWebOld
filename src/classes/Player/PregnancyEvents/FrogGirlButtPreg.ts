...
export default class FrogGirlButtPreg implements IPregnancyEvent {
    public incubationDisplay(player: Player, incubationTime: number) {
        if (incubationTime === 8) {
            ...
            // cock:
            else if (player.torso.cocks.count > 0) DisplayText("  Splashing against the underside of your " + CockDescriptor.describeMultiCockShort(player) + ", the slime leaves a warm, oozy sensation that makes you just want to rub [eachCock] over and over and over again.");
            // genderless:
            else DisplayText("  Your asshole begins twitching, aching for something to push through it over and over again.");
