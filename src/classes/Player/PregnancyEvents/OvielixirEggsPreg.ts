...
export default class OvielixirEggsPreg implements IPregnancyEvent {
    ...
    public birthScene(player: Player) {
        if (player.torso.vaginas.count === 0) {
            ...
            DisplayText("\nYour pregnant belly suddenly begins shrinking, until it disappears.\n").bold();
        }
        DisplayText().newline();
        // Small egg scenes
        if (player.statusAffects.get(StatusAffectType.Eggs).value2 === 0) {
            ...
            if (player.statusAffects.get(StatusAffectType.Eggs).value3 < 10) {
                ...
                DisplayText(this.eggDescript(player));
                DisplayText(", driving yourself to the very edge of orgasm.");
                player.stats.lustNoResist = 100;
                // dynStats("lus=", 100, "resisted", false);
            }
            // High quantity
            else {
                DisplayText("A strange desire overwhelms your sensibilities, forcing you to shed your " + player.inventory.equipment.armor.displayName + " and drop to your hands and knees.   You manage to roll over and prop yourself up against a smooth rock, looking down over your pregnant-looking belly as green fluids leak from you, soaking into the ground.   A powerful contraction rips through you and your legs spread instinctively, opening your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " to better deposit your precious cargo.   You see the rounded surface of an egg peek through your lips, mottled with strange colors.   You push hard and it drops free with an abrupt violent motion.  The friction and slimy fluids begin to arouse you, flooding your groin with heat as you feel the second egg pushing down.  It slips free with greater ease than the first, arousing you further as you bleat out a moan from the unexpected pleasure.  Before it stops rolling on the ground, you feel the next egg sliding down your slime-slicked passage, rubbing you perfectly as it slides free.  You lose count of the eggs and begin to masturbate, ");
                if (player.torso.clit.length > 5) DisplayText("jerking on your huge clitty as if it were a cock, moaning and panting as each egg slides free of your diminishing belly.  You lubricate it with a mix of your juices and the slime until ");
                if (player.torso.clit.length > 2 && player.torso.clit.length <= 5) DisplayText("playing with your over-large clit as if it were a small cock, moaning and panting as the eggs slide free of your diminishing belly.  You spread the slime and cunt juice over it as you tease and stroke until ");
                if (player.torso.clit.length <= 2) DisplayText("pulling your folds wide and playing with your clit as another egg pops free from your diminishing belly.  You make wet 'schlick'ing sounds as you spread the slime around, vigorously frigging yourself until ");
                DisplayText("you quiver in orgasm, popping out the last of your eggs as your body twitches nervelessly on the ground.   In total you lay " + this.eggDescript(player) + ".");
                player.orgasm();
        ...
        // Large egg scene
            ...
            DisplayText("A sudden shift in the weight of your pregnant belly staggers you, dropping you to your knees.  You realize something is about to be birthed, and you shed your " + player.inventory.equipment.armor.displayName + " before it can be ruined by what's coming.  A contraction pushes violently through your midsection, ");
            if (player.torso.vaginas.get(0).looseness < VaginaLooseness.LOOSE)
                DisplayText("stretching your tight cunt painfully, the lips opening wide ");
            if (player.torso.vaginas.get(0).looseness >= VaginaLooseness.LOOSE && player.torso.vaginas.get(0).looseness <= VaginaLooseness.GAPING_WIDE)
                DisplayText("temporarily stretching your cunt-lips wide-open ");
            if (player.torso.vaginas.get(0).looseness > VaginaLooseness.GAPING_WIDE)
                DisplayText("parting your already gaping lips wide ");
            ...
            if (player.torso.clit.length > 5) DisplayText("your massive cock-like clit, jacking it off with the slimy birthing fluids as lube.   It pulses and twitches in time with your heartbeats, its sensitive surface overloading your fragile mind with pleasure.  ");
            if (player.torso.clit.length > 2 && player.torso.clit.length <= 5) DisplayText("your large clit like a tiny cock, stroking it up and down between your slime-lubed thumb and fore-finger.  It twitches and pulses with your heartbeats, the incredible sensitivity of it overloading your fragile mind with waves of pleasure.  ");
            if (player.torso.clit.length <= 2) DisplayText("your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " by pulling your folds wide and playing with your clit.  Another egg pops free from your diminishing belly, accompanied by an audible burst of relief.  You make wet 'schlick'ing sounds as you spread the slime around, vigorously frigging yourself.  ");
            DisplayText("You cum hard, the big eggs each making your cunt gape wide just before popping free.  You slump down, exhausted and barely conscious from the force of the orgasm.  ");
            if (player.statusAffects.get(StatusAffectType.Eggs).value3 >= 11) DisplayText("Your swollen belly doesn't seem to be done with you, as yet another egg pushes its way to freedom.   The stimulation so soon after orgasm pushes you into a pleasure-stupor.  If anyone or anything discovered you now, they would see you collapsed next to a pile of eggs, your fingers tracing the outline of your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " as more and more eggs pop free.  In time your wits return, leaving you with the realization that you are no longer pregnant.  ");
            DisplayText("\n\nYou gaze down at the mess, counting " + this.eggDescript(player) + ".");
            player.orgasm();
            // dynStats("resisted", false);
        }
        DisplayText("\n\nYou feel compelled to leave the eggs behind, ").bold();
