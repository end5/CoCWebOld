    ...
    public incubationDisplay(player: Player, incubationTime: number) {
        ...
        if (incubationTime === 20) {
            ...
            if (player.torso.cocks.count === 1) {
                ...
                if (player.torso.cocks.filter(Cock.Type(CockType.HORSE)).length === 1) DisplayText(", each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air, until your " + CockDescriptor.describeCock(player, firstCock) + " is twitching and dripping, the flare swollen and purple.  ");
                if (player.torso.cocks.filter(Cock.Type(CockType.DOG)).length === 1) DisplayText(", each inhalation making it thicker, harder, and firmer.  You suck in huge lungfuls of air, desperate for more, until your " + CockDescriptor.describeCock(player, firstCock) + " is twitching and dripping, its knot swollen to the max.  ");
                if (player.torso.cocks.filter(Cock.Type(CockType.HUMAN)).length === 1) DisplayText(", each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air, until your " + CockDescriptor.describeCock(player, firstCock) + " is twitching and dripping, the head swollen and purple.  ");
                // FAILSAFE FOR NEW COCKS
                if (player.torso.cocks.filter(Cock.Type(CockType.HUMAN)).length === 0 && player.torso.cocks.filter(Cock.Type(CockType.DOG)).length === 0 && player.torso.cocks.filter(Cock.Type(CockType.HORSE)).length === 0) DisplayText(", each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air until your " + CockDescriptor.describeCock(player, firstCock) + " is twitching and dripping.  ");
            }
