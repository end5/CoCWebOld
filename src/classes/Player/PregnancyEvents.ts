...
export default class PregnancyEvent {
    public updatePregnancy(player: Player): boolean {
        ...
        let pregText: string = "";
        if ((player.pregnancyIncubation <= 0 && incubationTime <= 0) ||
        ...
        if (player.pregnancyIncubation == 1) {
            ...
            if (!player.statusAffects.has("Birthed")) player.statusAffects.add(StatusAffectFactory.create(StatusAffectType.Birthed, 1, 0, 0, 0)));
		else {
                player.statusAffects.get("Birthed").value1 += 1;
        ...
        if (incubationTime == 1 && player.buttPregnancyType == PregnancyType.FROG_GIRL) {
        }
        //Give birf if its time... to ANAL EGGS
        if (incubationTime == 1 && player.buttPregnancyType == PregnancyType.DRIDER_EGGS) {
        }
        //Bive birf to dragons
        if (player.pregnancyIncubation == 1 && player.pregnancyType == PregnancyType.EMBER) {
        }
        //GIVE BIRF TO TRAPS
        if (incubationTime == 1 && player.buttPregnancyType == PregnancyType.SANDTRAP_FERTILE) {
        }
        //Give birth (if it's time) to beeeeeeez
        if (incubationTime == 1 && player.buttPregnancyType == PregnancyType.BEE_EGGS) {
        }
        ...
        //Satyr butt preg
        if (player.buttPregnancyType == PregnancyType.SATYR && incubationTime == 1) {
        }
        ...
        if (player.pregnancyIncubation == 1 && player.pregnancyType == PregnancyType.AMILY) {
            if (Flags.list[FlagEnum.AMILY_FOLLOWER] == 2 || Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00170] > 0)
                player.knockUpForce(PregnancyType.MOUSE, player.pregnancyIncubation);
        }
        //Amily failsafe - converts PC with pure babies to mouse babies if Amily is with Urta
        if (player.pregnancyIncubation == 1 && player.pregnancyType == PregnancyType.AMILY) {
            if (Flags.list[FlagEnum.AMILY_VISITING_URTA] == 1 || Flags.list[FlagEnum.AMILY_VISITING_URTA] == 2)
                player.knockUpForce(PregnancyType.MOUSE, player.pregnancyIncubation);
        }
