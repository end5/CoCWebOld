...
export class TailWhip implements CombatAction {
    ...
    public isPossible(player: Player): boolean {
        return player.torso.tails.filterType(TailType.SHARK).length > 0 || player.torso.tails.filterType(TailType.LIZARD).length > 0 || player.torso.tails.filterType(TailType.KANGAROO).length > 0 || player.torso.tails.filterType(TailType.DRACONIC).length > 0 || player.torso.tails.filterType(TailType.RACCOON).length > 0);
    }
    ...
    public use(player: Player, monster: Character) {
        ...
        if ((player.statusAffects.has(StatusAffectType.Blind) && Utils.rand(2) === 0) ||
            ...
            if (!monster.statusAffects.has(StatusAffectType.CoonWhip))
                monster.statusAffects.set(StatusAffectType.CoonWhip, StatusAffectFactory.create(StatusAffectType.CoonWhip, Math.round(monster.combat.stats.defense() * .75), player.torso.tails.filterType(TailType.RACCOON).length <= 0 ? 2 : 4, 0, 0));
        }
        DisplayText("\n\n");
    }
}
