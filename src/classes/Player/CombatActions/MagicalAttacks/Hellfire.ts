...
export default class Hellfire extends PlayerSpellAction {
    ...
    public use(player: Player, monster: Character) {
        ...
        if (monster.charType == CharacterType.LivingStatue) {
            DisplayText.text("The fire courses over the stone behemoths skin harmlessly. It does leave the surface of the statue glossier in its wake.");
            return;
        }
        ...
        if (monster.desc.short == "Isabella") {
            DisplayText.text("  Isabella shoulders her shield into the path of the crimson flames.  They burst over the wall of steel, splitting around the impenetrable obstruction and washing out harmlessly to the sides.\n\n");
            ...
            if (player.perks.has("Evade") && Utils.rand(2) == 0) {
                ...
                DisplayText.text("Your own fire smacks into your face, arousing you!");
            ...
            if (monster.stats.int < 10) {
                ...
                if (monster.stats.lustVuln > 0) {
                    DisplayText.text("  Your foe cries out in surprise and then gives a sensual moan as the flames of your passion surround them and fill their body with unnatural lust.\n");
                    monster.stats.lust += monster.stats.lustVuln * damage / 6;
                }
                else {
                    DisplayText.text("  The corrupted fire doesn't seem to have affect on " + monster.desc.a+ monster.desc.short + "!\n");
                }
            }
        }
        DisplayText.text("\n");
