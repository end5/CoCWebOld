    ...
    public castSpell(player: Player, monster: Character) {
        ...
        if (monster.charType == CharacterType.JeanClaude) {
            ...
            if (Utils.rand(player.stats.spe) >= 50 || Utils.rand(player.stats.int) >= 50) {
                ...
                monster.combat.stats.loseHP(Math.floor(10 + (player.stats.int / 3 + Utils.rand(player.stats.int / 2)) * player.combat.stats.spellMod()), player);
            }
            else {
        ...
        if (monster.charType == CharacterType.LivingStatue) {
            ...
            monster.statusAffects.add(StatusAffectFactory.create(StatusAffectType.Blind, 5 * player.combat.stats.spellMod(), 0, 0, 0));
