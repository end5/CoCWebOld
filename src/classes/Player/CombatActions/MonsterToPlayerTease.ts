    ...
    public use(character: Character, monster: Character): void {
        if (character.stats.lust < 35) {
            ...
            if (character.torso.cocks.count > 0)
                DisplayText(CockDescriptor.describeMultiCockShort(character) + " hardening ");
            else if (character.torso.vaginas.count > 0) DisplayText(VaginaDescriptor.describeVagina(character, character.torso.vaginas.get(0)) + " get wetter ");
            ...
            if (character.gender === 1) DisplayText(CockDescriptor.describeMultiCockShort(character) + " towards the nearest inviting hole.");
            if (character.gender === 2) DisplayText(VaginaDescriptor.describeVagina(character, character.torso.vaginas.get(0)) + " towards the nearest swinging cock.");
