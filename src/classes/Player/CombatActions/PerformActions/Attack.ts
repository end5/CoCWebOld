...
export default class Attack implements CombatAction {
    ...
    private miss(character: Character, monster: Character): void {
        ...
        if (character.perks.has(PerkType.DoubleAttack) && character.stats.spe >= 50 && Flags.list[FlagEnum.DOUBLE_ATTACK_STYLE] < 2) {
            if (character.combat.effects.has(CombatEffectType.FirstAttack)) character.combat.effects.remove(CombatEffectType.FirstAttack);
                ...
                // Alternate!
                else if (character.stats.str < 61 && Flags.list[FlagEnum.DOUBLE_ATTACK_STYLE] === 1)
                    character.combat.effects.set(CombatEffectType.FirstAttack, CombatEffectFactory.create(CombatEffectType.FirstAttack, 0, 0, 0, 0));
            }
        }
    ...
    private crit(character: Character): boolean {
        return Utils.rand(100) <= 4 || (character.perks.has(PerkType.Tactician) && character.stats.int >= 50 && (character.stats.int - 50) / 5 > Utils.rand(100));
    }
    ...
    private applyDamage(character: Character, monster: Character, damage: number, crit: boolean): void {
        ...
        if (damage > 0) {
            ...
            if (monster.stats.lustVuln > 0) {
                ...
                if (character.inventory.equipment.weapon.displayname === "coiled whip" && Utils.rand(2) === 0) {
            ...
            if (character.inventory.equipment.weapon.displayname === "huge warhammer" || character.inventory.equipment.weapon.displayname === "spiked gauntlet" || character.inventory.equipment.weapon.displayname === "hooked gauntlets") {
                ...
                // 50% Bleed chance
                if (character.inventory.equipment.weapon.displayname === "hooked gauntlets" && Utils.rand(2) === 0 && monster.combat.stats.defense() < 10 && !monster.combat.effects.has(CombatEffectType.IzmaBleed)) {
