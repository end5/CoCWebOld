...
export default class PlayerTease implements PlayerCombatAction {
    ...
    private determineTeaseChoice(player: Player, monster: Character, bimbo: boolean, bro: boolean, futa: boolean) {
    ...
    public use(player: Player, monster: Character): void {
        ...
        switch (choice) {
            ...
            case TeaseType.ButtShake:
                //Display
                DisplayText.text("You slap your " + ButtDescriptor.describeButt(player));
                if (player.lowerBody.butt.buttRating >= 10 && player.tone < 60) DisplayText.text(", making it jiggle delightfully.");
            ...
            case TeaseType.BreastJiggle:
                ...
                if (player.upperBody.chest.count() == 1) {
                    ...
                    DisplayText.text("Your lift your top, exposing your " + BreastDescriptor.describeBreastRow(player.upperBody.chest.get(0)) + " to " + monster.desc.a + monster.desc.short + ".  You shake them from side to side enticingly.");
                    if (player.stats.lust >= 50) DisplayText.text("  Your " + BreastDescriptor.describeNipple(player, player.upperBody.chest.get(0)) + "s seem to demand " + monster.desc.possessivePronoun + " attention.");
                }
                //Multirow
                if (player.upperBody.chest.count() > 1) {
                    //50 + 10% per breastRow + breastSize%
                    DisplayText.text("You lift your top, freeing your rows of " + BreastDescriptor.describeBreastRow(player.upperBody.chest.get(0)) + " to jiggle freely.  You shake them from side to side enticingly");
                    if (player.stats.lust >= 50) DisplayText.text(", your " + BreastDescriptor.describeNipple(player, player.upperBody.chest.get(0)) + "s painfully visible.");
                    else DisplayText.text(".");
                    chance++;
                }
                breasts = true;
            ...
            case TeaseType.BimboButtShake:
                DisplayText.text("You turn away and bounce your " + ButtDescriptor.describeButt(player) + " up and down hypnotically");
                //Big butts = extra text + higher success
                if (player.lowerBody.butt.buttRating >= 10) {
                    DisplayText.text(", making it jiggle delightfully.  " + monster.desc.capitalA + monster.desc.short + " even gets a few glimpses of the " + ButtDescriptor.describeButthole(player) + " between your cheeks.");
                    chance += 3;
                }
                //Small butts = less damage, still high success
                else {
                    DisplayText.text(", letting " + monster.desc.a + monster.desc.short + " get a good look at your " + ButtDescriptor.describeButthole(player) + " and " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".");
                    chance += 1;
                    vagina = true;
                }
                ass = true;
            ...
            case TeaseType.BimboPussyFlash:
                ...
                if (player.lowerBody.vaginaSpot.get(0).clitLength > 3) DisplayText.text("  You smile as your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " swells out from the folds and stands proudly, begging to be touched.");
                else DisplayText.text("  You smile and pull apart your lower-lips to expose your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + ", giving the perfect view.");
            ...
            case TeaseType.BimboSpecial:
                DisplayText.text("The glinting of light catches your eye and you whip around to inspect the glittering object, turning your back on " + monster.desc.a + monster.desc.short + ".  Locking your knees, you bend waaaaay over, " + BreastDescriptor.describeChest(player) + " swinging in the open air while your " + ButtDescriptor.describeButt(player) + " juts out at the " + monster.desc.a + monster.desc.short + ".  Your plump cheeks and " + LowerBodyDescriptor.describeHips(player) + " form a jiggling heart-shape as you eagerly rub your thighs together.\n\n");
                DisplayText.text("The clear, warm fluid of your happy excitement trickles down from your loins, polishing your " + SkinDescriptor.skin(player) + " to a glossy, inviting shine.  Retrieving the useless, though shiny, bauble, you hold your pose for just a moment longer, a sly little smile playing across your lips as you wiggle your cheeks one more time before straightening up and turning back around.");
                vagina = true;
                chance++;
                damage += 2;
                break;
            ...
            case TeaseType.Pregnant:
                ...
                if (player.upperBody.chest.LactationMultipierLargest[0].lactationMultiplier >= 1) {
                    DisplayText.text("  Your other hand moves to expose your " + BreastDescriptor.describeChest(player) + ", cupping and squeezing a stream of milk to leak down the front of your " + player.inventory.armorSlot.equipment.displayName + ".  \"<i>Help a mommy out.</i>\"\n\n");
                    chance += 2;
                    damage += 4;
                }
                const largestIncubation = player.pregnancy.listLargestIncubationTime[0].incubation;
            ...
            case TeaseType.DogTease:
                ...
                if (player.lowerBody.vaginaSpot.hasVagina()) DisplayText.text("parted cunt-lips");
            ...
            case TeaseType.ClothesGenderlessServant:
                DisplayText.text("You turn your back to your foe, and flip up your butt flap for a moment.   Your " + ButtDescriptor.describeButt(player) + " really is all you have to offer downstairs.");
                ass = true;
                chance++;
                damage += 2;
                break;
            ...
            case TeaseType.ClothesCrotchRevealing:
                DisplayText.text("You do a series of poses to accentuate what you've got on display with your crotch revealing clothes, while asking if your " + GenderDescriptor.mf(player, "master", "mistress") + " is looking to sample what is on display.");
                chance += 2;
                damage += 4;
                break;
            //34 Maid Costume (female only)
            case TeaseType.ClothesMaid:
                DisplayText.text("You give a rather explicit curtsey towards " + monster.desc.a + monster.desc.short + " and ask them if your " + GenderDescriptor.mf(player, "master", "mistress") + " is interested in other services today.");
                chance++;
                damage += 2;
                breasts = true;
                break;
            ...
            case TeaseType.ClothesServantBoy:
                DisplayText.text("You brush aside your crotch flap for a moment, then ask " + monster.desc.a + monster.desc.short + " if, " + GenderDescriptor.mf(player, "Master", "Mistress") + " would like you to use your " + CockDescriptor.describeMultiCockShort(player) + " on them?");
                penis = true;
                chance++;
                damage += 2;
                break;
            ...
            case TeaseType.Kitsune3:
                DisplayText.text("Leaning forward, you bow down low, raising a hand up to your lips and blowing " + monster.desc.a + monster.desc.short + " a kiss.  You stand straight, wiggling your " + LowerBodyDescriptor.describeHips(player) + " back and forth seductively while trailing your fingers down your front slowly, pouting demurely.  The tip of ");
                if (player.lowerBody.tailVenom == 1) DisplayText.text("your");
                else DisplayText.text("a");
                DisplayText.text(" bushy tail curls up around your " + LowerBodyDescriptor.describeLeg(player) + ", uncoiling with a whipping motion that makes an audible crack in the air.");
                ass = true;
                chance++;
                damage += 1;
                break;
            case TeaseType.Kitsune4:
                ...
                if (player.lowerBody.tailVenom == 1) DisplayText.text("  Your tail twists and whips about, sliding around your " + LowerBodyDescriptor.describeHips(player) + " in a slow arc and framing your rear nicely as you slowly lift your " + player.inventory.armorSlot.equipment.displayName + ".");
                else DisplayText.text("  Your tails fan out, twisting and whipping sensually, sliding up and down your " + LowerBodyDescriptor.describeLegs(player) + " and framing your rear nicely as you slowly lift your " + player.inventory.armorSlot.equipment.displayName + ".");
                DisplayText.text("  As your [butt] comes into view, you brush your tail" + ((player.lowerBody.tailVenom > 1) ? "s" : "") + " across it, partially obscuring the view in a tantalizingly teasing display.");
                ass = true;
                anus = true;
                chance++;
                damage += 2;
            ...
            case TeaseType.KitsuneGendered:
                DisplayText.text("Smirking coyly, you sway from side to side, running your tongue along your upper teeth seductively.  You hook your thumbs into your " + player.inventory.armorSlot.equipment.displayName + " and pull them away to partially reveal ");
                if (player.lowerBody.cockSpot.count() > 0) DisplayText.text(CockDescriptor.describeMultiCockSimpleOne(player));
                if (player.gender == 3) DisplayText.text(" and ");
                if (player.gender >= 2) DisplayText.text("your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)));
                DisplayText.text(".  Your bushy tail" + ((player.lowerBody.tailVenom > 1) ? "s" : "") + " cross" + ((player.lowerBody.tailVenom > 1) ? "" : "es") + " in front, wrapping around your genitals and obscuring the view teasingly.");
                vagina = true;
                penis = true;
                damage += 2;
                chance++;
            ...
            case TeaseType.Cowgirl:
                ...
                if (cows == 0) {
                    ...
                    if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 3) DisplayText.text("dripping ");
            ...
            case TeaseType.ClothesBikiniMail:
                ...
                if (maiden == 0) {
                    ...
                    if (player.lowerBody.vaginaSpot.isVirgin()) DisplayText.text("Think you can handle a virgin's infinite lust?");
        ...
        if (Utils.rand(100) <= chance + Utils.rand(bonusChance)) {
            ...
            if (Flags.list[FlagEnum.PC_FETISH] >= 1 && !urtaQuest.isUrta()) {
                ...
                player.stats.lust += 2 + Utils.rand(3);
            }
