    ...
    public use(player: Player, monster: Character): number {
        ...
        if (monster.charType == CharacterType.Basilisk) {
            ...
            //Counter attack fails: (Utils.random chance if PC int > 50 spd > 60; PC takes small physical damage but no block or spd penalty)
                ...
                MainScreen.text("Holding the basilisk in your peripheral vision, you charge forward to strike it.  Before the moment of impact, the reptile shifts its posture, dodging and flowing backward skillfully with your movements, trying to make eye contact with you. You twist unexpectedly, bringing your " + player.inventory.weaponSlot.equipment.displayname + " up at an oblique angle; the basilisk doesn't anticipate this attack!  ", false);
            }
        }
        //Worms are special
        ...
        if (monster.desc.short == "anemone") {
            ...
            if (Utils.rand(10) <= 1) {
                MainScreen.text("Seeing your " + player.inventory.weaponSlot.equipment.displayname + " raised, the anemone looks down at the water, angles her eyes up at you, and puts out a trembling lip.  ", false);
        ...
        if (damage <= 0) {
            damage = 0;
            MainScreen.text("Your attacks are deflected or blocked by " + monster.desc.a + monster.desc.short + ".", false);
        }
        else {
            MainScreen.text("You hit " + monster.desc.a + monster.desc.short + "! (" + damage + ")", false);
        ...
        if (damage > 0) {
            ...
            if (monster.stats.lustVuln > 0) {
                if (player.inventory.weaponSlot.equipment.perk == "Aphrodisiac Weapon") {
                    ...
                    MainScreen.text("\n" + monster.desc.capitalA + monster.desc.short + " shivers as your weapon's 'poison' goes to work.", false);
                }
                if (player.inventory.weaponSlot.equipment.displayname == "coiled whip" && Utils.rand(2) == 0) {
                    ...
                    if (!monster.desc.plural) MainScreen.text("\n" + monster.desc.capitalA + monster.desc.short + " shivers and gets turned on from the whipping.", false);
                    else MainScreen.text("\n" + monster.desc.capitalA + monster.desc.short + " shiver and get turned on from the whipping.", false);
                }
                if (player.inventory.weaponSlot.equipment.displayname == "succubi whip") {
                    ...
                    if (!monster.desc.plural) MainScreen.text("\n" + monster.desc.capitalA + monster.desc.short + " shivers and moans involuntarily from the whip's touches.", false);
                    else MainScreen.text("\n" + monster.desc.capitalA + monster.desc.short + " shiver and moan involuntarily from the whip's touches.", false);
                    if (Utils.rand(2) == 0) {
            ...
            if (player.inventory.weaponSlot.equipment.displayname == "huge warhammer" || player.inventory.weaponSlot.equipment.displayname == "spiked gauntlet" || player.inventory.weaponSlot.equipment.displayname == "hooked gauntlets") {
                ...
                if (Utils.rand(10) == 0 && !monster.perks.has("Resolute")) {
                    MainScreen.text("\n" + monster.desc.capitalA + monster.desc.short + " reels from the brutal blow, stunned.", false);
                ...
                if (player.inventory.weaponSlot.equipment.displayname == "hooked gauntlets" && Utils.rand(2) == 0 && monster.defense() < 10 && !monster.statusAffects.has("IzmaBleed")) {
                    if (monster.charType == CharacterType.LivingStatue) {
                        ...
                        if (monster.desc.plural) MainScreen.text("\n" + monster.desc.capitalA + monster.desc.short + " bleed profusely from the many bloody gashes your hooked gauntlets leave behind.", false);
                        else MainScreen.text("\n" + monster.desc.capitalA + monster.desc.short + " bleeds profusely from the many bloody gashes your hooked gauntlets leave behind.", false);
                    }
                }
            }
