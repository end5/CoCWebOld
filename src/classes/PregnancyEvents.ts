...
export default class PregnancyEvent {
    public updatePregnancy(): boolean {
        ...
        if (player.pregnancyIncubation > 1) {
            ...
            if (player.pregnancyType == PregnancyType.ANEMONE) {
                ...
                if (player.pregnancyIncubation == 120) {
                    ...
                    if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" and keeps your " + vaginaDescript(0) + " aroused from the constant tingling in your womb", false);
                    MainScreen.text(".</b>\n", false);
        ...
        if (player.buttPregnancyIncubation > 1) {
            if (player.buttPregnancyType == PregnancyType.FROG_GIRL) {
                if (player.buttPregnancyIncubation == 8) {
                    ...
                    //[cock:
                    else if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("  Splashing against the underside of your " + player.multiCockDescriptLight() + ", the slime leaves a warm, oozy sensation that makes you just want to rub [eachCock] over and over and over again.");
                    //genderless: 
            ...
            if (player.buttPregnancyType == PregnancyType.BEE_EGGS) {
                ...
                if (player.buttPregnancyIncubation == 20) {
                    ...
                    if (player.lowerBody.cockSpot.count() == 1) {
                        ...
                        if (player.horseCocks() == 1) MainScreen.text(", each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air, until your " + cockDescript(0) + " is twitching and dripping, the flare swollen and purple.  ", false);
                        if (player.dogCocks() == 1) MainScreen.text(", each inhalation making it thicker, harder, and firmer.  You suck in huge lungfuls of air, desperate for more, until your " + cockDescript(0) + " is twitching and dripping, its knot swollen to the max.  ", false);
                        if (player.normalCocks() == 1) MainScreen.text(", each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air, until your " + cockDescript(0) + " is twitching and dripping, the head swollen and purple.  ", false);
                        //FAILSAFE FOR NEW COCKS
                        if (player.normalCocks() == 0 && player.dogCocks() == 0 && player.horseCocks() == 0) MainScreen.text(", each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air until your " + cockDescript(0) + " is twitching and dripping.  ", false);
                    }
                    if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("groin.  Your " + player.multiCockDescriptLight() + " fill and grow with every lungful of the stuff you breathe in.  You suck in great lungfuls of the tainted air, desperate for more, your cocks twitching and dripping with need.  ", false);
        ...
        if (player.buttPregnancyIncubation == 1 && player.buttPregnancyType == PregnancyType.BEE_EGGS) {
            ...
            if (player.lowerBody.vaginaSpot.count() > 0) MainScreen.text("Your " + vaginaDescript(0) + " becomes engorged and sensitive.  ", false);
            ...
            if (player.averageLactation() >= 1 && player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) MainScreen.text("Seeking even greater sensation, your hands gather the honey and massage it into your " + breastDescript(0) + ", slowly working up to your nipples.  Milk immediately begins pouring out from the attention, flooding your chest with warmth.  ", false);
            ...
            if (player.buttChange(20, true)) MainScreen.text("\n", false);
            if (player.lowerBody.butt.buttRating < 17) {
                //Guaranteed increase up to level 10
                if (player.lowerBody.butt.buttRating < 13) {
                    player.lowerBody.butt.buttRating++;
        ...
        if (player.pregnancyType == PregnancyType.BUNNY && player.pregnancyIncubation == 1) {
            ...
            MainScreen.text("You sink into restful unconsciousness, marveling at how stretchy and sensitive your " + vaginaDescript(0) + " feels now.", false);
        ...
        if (player.pregnancyType == PregnancyType.ANEMONE && player.pregnancyIncubation == 1) {
            ...
            MainScreen.text("Your " + player.armorName + " feels damp around the groin and you reach down to check the area.  The  " + vaginaDescript(0) + " you feel is dilated and slick with unusual wetness; your water must have broken!\n\n", false);
            ...
            if (flags[FlagEnum.ANEMONE_KID] > 0) {
                MainScreen.text("As you take in the sight, small nodules around the tip begin to form and lengthen, until the little anemone is capped by a mop of wriggling blue-green tentacles.  Horrified, you grasp it at the base and give it a sharp pull.  The pain makes you lock up and nearly takes away your consciousness as its sticky surface releases its grip on your labia and " + clitDescript() + "!   It writhes and slips out of your pain-wracked hands, leaving them tingling.  As you lie there, stunned, it begins to inch back toward your " + vaginaDescript(0) + ".  Footfalls sound next to you, and a blue hand picks up the squirming, cilliated creature.  Kid A gives you a shy smile, then turns to her barrel.  A quick splash and a filled waterskin later, she heads toward the stream, toting your grub-like offspring.");
            ...
            //[(if pc has 0-9 existing cocks)
                ...
                MainScreen.text("As you take in the sight, small nodules around the tip begin to form and lengthen, until the little anemone is capped by a mop of wriggling blue-green tentacles.  Horrified, you grasp it at the base and give it a sharp pull.  The pain makes you lock up and nearly takes away your consciousness as its sticky surface releases its grip on your labia and " + clitDescript() + "!  The small anemone and you both lay there twitching, but it recovers its bearings first; through your haze of pain you watch it flexing its body, wedging the head under itself, and elevating the base.", false);
                player.cuntChange(20, true, true, false);

                MainScreen.text("\n\nBeset by a panic, you watch as the strange thing sets butt-end down on your pubic mound and adheres", false);
                //[(if cocks)
                if (player.lowerBody.cockSpot.count() > 0) MainScreen.text(" below your " + player.multiCockDescriptLight(), false);
                ...
                //[(dick slot 1 exists)
                if (player.lowerBody.cockSpot.count() > 0) MainScreen.text("  The tentacles writhe around, rubbing against your " + player.multiCockDescriptLight(), false);
                //(doesn't exist)
                ...
                MainScreen.text(" and you quickly become fully erect from the aphrodisiac they inject.  Over and over the tentacles caress " + player.sMultiCockDesc() + " sensually, leaving behind a tingling trail of vibrant pleasure", false);
                //[(if no dick1 and no balls)
                if (player.totalCocks() == 1 && player.lowerBody.balls == 0) MainScreen.text("; you feel a pressure build below the shaft, near your asshole", false);
                ...
                //[(if boobs)
                if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text(allBreastsDescript() + " and ", false);
                MainScreen.text("stomach", false);
                //[(dick1 exists)
                if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(", followed in short order by white squirts from " + player.sMultiCockDesc() + " remaining", false);
                MainScreen.text(".  Your " + vaginaDescript(0) + " quivers and pulses as well, adding ", false);
                if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLICK) MainScreen.text("a trickle", false);
                else if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLAVERING) MainScreen.text("a squirt", false);
            ...
            if (flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] < 100) {
                ...
                //it's a boy!
                {
                    ...
                    if (player.lowerBody.hipRating < 10) {
                        player.lowerBody.hipRating++;
                        MainScreen.text("After the birth your " + player.armorName + " fits a bit more snugly about your " + hipDescript() + ".", false);
                    }
                ...
                //it's a girl!
                {
                    ...
                    if (player.statusAffects.get("Marble").value4 >= 20) {
                        MainScreen.text("As the child contentedly drinks from your " + nippleDescript(0) + ", Marble tells you, \"<i>Sweetie, somehow I know that our kids won't have to worry about the addictive milk.  It will only make them healthy and strong.</i>\"  You nod at her and put the child into the nursery.  ", false);
                    }
                    else {
        ...
        if (player.pregnancyIncubation == 1 && (player.pregnancyType == PregnancyType.CENTAUR || player.pregnancyType == PregnancyType.KELT)) {
            ...
            player.knockUpForce(); //Clear Pregnancy
            if (player.averageLactation() > 0 && player.averageLactation() < 5) {
                MainScreen.text("  Your " + allBreastsDescript() + " won't seem to stop dribbling milk, lactating more heavily than before.", false);
                player.boostLactation(.5);
            }
            ...
            if (player.pregnancyIncubation == 1) {
                ...
                if (player.statusAffects.get("Eggs").value2 == 0) {
                    ...
                    if (player.statusAffects.get("$1").value3 < 10) {
                        ...
                        MainScreen.text(eggDescript(), false);
                    ...
                    //High quantity
                        ...
                        if (player.lowerBody.vaginaSpot.list[0].clitLength > 5) MainScreen.text("jerking on your huge clitty as if it were a cock, moaning and panting as each egg slides free of your diminishing belly.  You lubricate it with a mix of your juices and the slime until ", false);
                        if (player.lowerBody.vaginaSpot.list[0].clitLength > 2 && player.lowerBody.vaginaSpot.list[0].clitLength <= 5) MainScreen.text("playing with your over-large clit as if it were a small cock, moaning and panting as the eggs slide free of your diminishing belly.  You spread the slime and cunt juice over it as you tease and stroke until ", false);
                        if (player.lowerBody.vaginaSpot.list[0].clitLength <= 2) MainScreen.text("pulling your folds wide and playing with your clit as another egg pops free from your diminishing belly.  You make wet 'schlick'ing sounds as you spread the slime around, vigorously frigging yourself until ", false);
                        MainScreen.text("you quiver in orgasm, popping out the last of your eggs as your body twitches nervelessly on the ground.   In total you lay " + eggDescript() + ".", false);
                        player.orgasm();
                ...
                //Large egg scene
                    ...
                    MainScreen.text("A sudden shift in the weight of your pregnant belly staggers you, dropping you to your knees.  You realize something is about to be birthed, and you shed your " + player.armorName + " before it can be ruined by what's coming.  A contraction pushes violently through your midsection, ", false);
                    if (player.vaginas[0].vaginalLooseness < VAGINA_LOOSENESS.LOOSE) MainScreen.text("stretching your tight cunt painfully, the lips opening wide ", false);
                    if (player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.LOOSE && player.vaginas[0].vaginalLooseness <= VAGINA_LOOSENESS.GAPING_WIDE) MainScreen.text("temporarily stretching your cunt-lips wide-open ", false);
                    if (player.vaginas[0].vaginalLooseness > VAGINA_LOOSENESS.GAPING_WIDE) MainScreen.text("parting your already gaping lips wide ", false);
                    ...
                    if (player.lowerBody.vaginaSpot.list[0].clitLength > 5) MainScreen.text("your massive cock-like clit, jacking it off with the slimy birthing fluids as lube.   It pulses and twitches in time with your heartbeats, its sensitive surface overloading your fragile mind with pleasure.  ", false);
                    if (player.lowerBody.vaginaSpot.list[0].clitLength > 2 && player.lowerBody.vaginaSpot.list[0].clitLength <= 5) MainScreen.text("your large clit like a tiny cock, stroking it up and down between your slime-lubed thumb and fore-finger.  It twitches and pulses with your heartbeats, the incredible sensitivity of it overloading your fragile mind with waves of pleasure.  ", false);
                    if (player.lowerBody.vaginaSpot.list[0].clitLength <= 2) MainScreen.text("your " + vaginaDescript(0) + " by pulling your folds wide and playing with your clit.  Another egg pops free from your diminishing belly, accompanied by an audible burst of relief.  You make wet 'schlick'ing sounds as you spread the slime around, vigorously frigging yourself.  ", false);
