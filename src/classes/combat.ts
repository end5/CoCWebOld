export default class Combat {
        ...
        if (inCombat) {
            ...
            //Player lost
                ...
                if (flags[FlagEnum.COMBAT_BONUS_XP_VALUE] > 0) {
                    ...
                    MainScreen.text("  Somehow you managed to gain " + flags[FlagEnum.COMBAT_BONUS_XP_VALUE] + " XP from the situation.");
                    flags[FlagEnum.COMBAT_BONUS_XP_VALUE] = 0;
                }
                //Bonus lewts
                if (flags[FlagEnum.BONUS_ITEM_AFTER_COMBAT_ID] != "") {
                    MainScreen.text("  Somehow you came away from the encounter with " + ItemType.lookupItem(flags[FlagEnum.BONUS_ITEM_AFTER_COMBAT_ID]).longName + ".\n\n");
    ...
    public lustAttack(): void {
        if (player.lust < 35) {
            ...
            if (player.lowerBody.cockSpot.count() > 0)
                MainScreen.text(player.multiCockDescriptLight() + " hardening ");
            else if (player.lowerBody.vaginaSpot.count() > 0) MainScreen.text(vaginaDescript(0) + " get wetter ");
            ...
            if (player.gender == 1) MainScreen.text(player.multiCockDescriptLight() + " towards the nearest inviting hole.");
            if (player.gender == 2) MainScreen.text(vaginaDescript(0) + " towards the nearest swinging cock.");
    ...
    private struggle(): void {
        if (monster.statusAffects.has("MinotaurEntangled")) {
            ...
            if (rand(3) == 0 || rand(80) < player.str / 2) {
                MainScreen.text("As the creature attempts to adjust your position in its grip, you free one of your " + player.legs() + " and hit the beast in its beak, causing it to let out an inhuman cry and drop you to the ground smartly.\n\n");
    ...
    public fantasize(): void {
        ...
        if (player.armorName == "goo armor") {
            ...
            MainScreen.text("You aren't sure if it's just the fantasy, but your " + ballsDescriptLight() + " do feel fuller than before...\n", false);
            ...
            MainScreen.text("You fantasize about grabbing " + monster.a + monster.short + " and forcing " + monster.pronoun2 + " against a " + nippleDescript(0) + ", and feeling your milk let down.  The desire to forcefeed SOMETHING makes your nipples hard and moist with milk.\n", false);
            temp2 = 5 + rand(player.stats.lib / 8 + player.stats.cor / 8)
        }
        else {
...
private combatStatusesUpdate():void {
	...
	if(player.statusAffects.has("StoneLust")) {
        if (player.lowerBody.vaginaSpot.count() > 0) {
            if (player.lust < 40) MainScreen.text("You squirm as the smooth stone orb vibrates within you.\n\n", false);
            if (player.lust >= 40 && player.lust < 70) MainScreen.text("You involuntarily clench around the magical stone in your twat, in response to the constant erotic vibrations.\n\n", false);
            if (player.lust >= 70 && player.lust < 85) MainScreen.text("You stagger in surprise as a particularly pleasant burst of vibrations erupt from the smooth stone sphere in your " + vaginaDescript(0) + ".\n\n", false);
            if (player.lust >= 85) MainScreen.text("The magical orb inside of you is making it VERY difficult to keep your focus on combat, white-hot lust suffusing your body with each new motion.\n\n", false);
        }
        else {
	...
	if(player.inHeat && player.lowerBody.vaginaSpot.count() > 0 && monster.totalCocks() > 0) {
        ...
        MainScreen.text("Your " + vaginaDescript(0) + " clenches with an instinctual desire to be touched and filled.  ", false);
	...
	if(player.inRut && player.totalCocks() > 0 && monster.hasVagina()) {
        ...
        if (monster.plural) MainScreen.text("our " + player.multiCockDescriptLight() + " dribbles pre-cum as you think about plowing " + monster.a + monster.short + " right here and now, fucking " + monster.pronoun3 + " " + monster.vaginaDescript() + "s until they're totally fertilized and pregnant.\n\n", false);
        else MainScreen.text("our " + player.multiCockDescriptLight() + " dribbles pre-cum as you think about plowing " + monster.a + monster.short + " right here and now, fucking " + monster.pronoun3 + " " + monster.vaginaDescript() + " until it's totally fertilized and pregnant.\n\n", false);
    }
	if(player.statusAffects.has("NagaVenom")) {
        ...
        if (player.perks.has("Medicine") && rand(100) <= 14) {
            ...
            if (player.lowerBody.vaginaSpot.hasVagina()) {
                ...
                MainScreen.text("Your " + player.cockDescript(0) + " pulses and twitches, overwhelmed with the desire to breed.  ", false);
            }
            if (player.gender == 0) {
                MainScreen.text("You feel a tingle in your " + assholeDescript() + ", and the need to touch and fill it nearly overwhelms you.  ", false);
            }
	...
	//{Bonus Lust Descripts}
        ...
        if(monster.plural) {
            ...
            if (monster.vaginas.length > 0) {
                ...
                if (monster.lust >= 85) MainScreen.text(monster.capitalA + monster.short + "' " + monster.vaginaDescript() + "s are practically soaked with their lustful secretions.  ", false);
            }
        }
		else {
			...
			if(monster.vaginas.length > 0) {
                ...
                if (monster.lust >= 85) MainScreen.text(monster.capitalA + monster.short + "'s " + monster.vaginaDescript() + " is practically soaked with her lustful secretions.  ", false);
            }
        }
    }
}
...
switch (select) {
    ...
    case 0:
        //Display
        MainScreen.text("You slap your " + buttDescript(), false);
    ...
    case 1:
        ...
        if (player.upperBody.chest.count() == 1) {
            ...
            MainScreen.text("Your lift your top, exposing your " + breastDescript(0) + " to " + monster.a + monster.short + ".  You shake them from side to side enticingly.", false);
            if (player.lust >= 50) MainScreen.text("  Your " + nippleDescript(0) + "s seem to demand " + monster.pronoun3 + " attention.", false);
        }
        //Multirow
        if (player.upperBody.chest.count() > 1) {
            //50 + 10% per breastRow + breastSize%
            MainScreen.text("You lift your top, freeing your rows of " + breastDescript(0) + " to jiggle freely.  You shake them from side to side enticingly", false);
            if (player.lust >= 50) MainScreen.text(", your " + nippleDescript(0) + "s painfully visible.", false);
            else MainScreen.text(".", false);
            chance++;
        }
        breasts = true;
    ...
    case 2:
        if (player.isTaur()) {
            ...
            MainScreen.text("You open your " + player.armorName + ", revealing your ", false);
            if (player.lowerBody.cockSpot.count() > 0) {
                chance++;
                damage++;
                if (player.lowerBody.cockSpot.count() == 1) MainScreen.text(player.cockDescript(0), false);
                if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(player.multiCockDescriptLight(), false);
    ...
    //4 butt shake
    case 4:
        MainScreen.text("You turn away and bounce your " + buttDescript() + " up and down hypnotically", false);
        //Big butts = extra text + higher success
        if (player.lowerBody.butt.buttRating >= 10) {
            MainScreen.text(", making it jiggle delightfully.  " + monster.capitalA + monster.short + " even gets a few glimpses of the " + assholeDescript() + " between your cheeks.", false);
            chance += 3;
        }
        //Small butts = less damage, still high success
        else {
            MainScreen.text(", letting " + monster.a + monster.short + " get a good look at your " + assholeDescript() + " and " + vaginaDescript(0) + ".", false);
            chance += 1;
            vagina = true;
        }
        ass = true;
    ...
    //5 breast jiggle
    case 5:
        MainScreen.text("You lean forward, letting the well-rounded curves of your " + allBreastsDescript() + " show to " + monster.a + monster.short + ".", false);
        MainScreen.text("  You cup them in your palms and lewdly bounce them, putting on a show and giggling the entire time.  An inch at a time, your " + player.armorName + " starts to come down, dropping tantalizingly slowly until your " + nippleDescript(0) + "s pop free.", false);
        if (player.lust >= 50) {
            if (player.upperBody.chest.hasFuckableNipples()) {
                chance++;
    ...
    case 6:
        if (player.perks.has("BimboBrains") || player.perks.has("FutaFaculties")) {
            MainScreen.text("You coyly open your " + player.armorName + " and giggle, \"<i>Is this, like, what you wanted to see?</i>\"  ", false);
        }
        else {
            MainScreen.text("You coyly open your " + player.armorName + " and purr, \"<i>Does the thought of a hot, ", false);
        ...
        if (player.lowerBody.vaginaSpot.list[0].clitLength > 3) MainScreen.text("  You smile as your " + clitDescript() + " swells out from the folds and stands proudly, begging to be touched.", false);
        else MainScreen.text("  You smile and pull apart your lower-lips to expose your " + clitDescript() + ", giving the perfect view.", false);
        if (player.lowerBody.cockSpot.count() > 0) MainScreen.text("  Meanwhile, " + player.sMultiCockDesc() + " bobs back and forth with your gyrating hips, adding to the display.", false);
        //BONUSES!
        if (player.lowerBody.cockSpot.hasCock()) {
            if (player.perks.has("BulgeArmor")) damage += 5;
            penis = true;
    ...
    //10 Bulgy groin thrust
    case 10:
        MainScreen.text("You lean back and pump your hips at " + monster.a + monster.short + " in an incredibly vulgar display.  The bulging, barely-contained outline of your " + player.cockDescript(0) + " presses hard into your gear.  ", false);
    ...
    case 11:
        if (silly() && rand(2) == 0) MainScreen.text("You strike a herculean pose and flex, whispering, \"<i>Do you even lift?</i>\" to " + monster.a + monster.short + ".", false);
            ...
            MainScreen.text("You open your " + player.armorName + " just enough to let your " + player.cockDescript(0) + " and " + ballsDescriptLight() + " dangle free.  A shiny rope of pre-cum dangles from your cock, showing that your reproductive system is every bit as fit as the rest of you.  ", false);
    ...
    case 13:
        ...
        if (player.biggestLactation() >= 1) {
            MainScreen.text("  Your other hand moves to expose your " + chestDesc() + ", cupping and squeezing a stream of milk to leak down the front of your " + player.armorName + ".  \"<i>Help a mommy out.</i>\"\n\n", false);
            chance += 2;
            damage += 4;
        }
    ...
    //32 Genderless servant clothes
    case 32:
        MainScreen.text("You turn your back to your foe, and flip up your butt flap for a moment.   Your " + buttDescript() + " really is all you have to offer downstairs.", false);
        ass = true;
        chance++;
        damage += 2;
        break;
    ...
    //35 Servant Boy Clothes (male only)
    case 35:
        MainScreen.text("You brush aside your crotch flap for a moment, then ask " + monster.a + monster.short + " if, " + player.mf("Master", "Mistress") + " would like you to use your " + player.multiCockDescriptLight() + " on them?", false);
        penis = true;
        chance++;
        damage += 2;
        break;
    ...
    case 38:
        ...
        break;
    case 39:
        MainScreen.text("Leaning forward, you bow down low, raising a hand up to your lips and blowing " + monster.a + monster.short + " a kiss.  You stand straight, wiggling your " + hipDescript() + " back and forth seductively while trailing your fingers down your front slowly, pouting demurely.  The tip of ");
        if (player.lowerBody.tailVenom == 1) MainScreen.text("your");
        else MainScreen.text("a");
        MainScreen.text(" bushy tail curls up around your " + player.leg() + ", uncoiling with a whipping motion that makes an audible crack in the air.");
        ass = true;
        chance++;
        damage += 1;
        break;
    case 40:
        ...
        if (player.lowerBody.tailVenom == 1) MainScreen.text("  Your tail twists and whips about, sliding around your " + hipDescript() + " in a slow arc and framing your rear nicely as you slowly lift your " + player.armorName + ".");
        else MainScreen.text("  Your tails fan out, twisting and whipping sensually, sliding up and down your " + player.legs() + " and framing your rear nicely as you slowly lift your " + player.armorName + ".");
        ...
        break;
    case 41:
        MainScreen.text("Smirking coyly, you sway from side to side, running your tongue along your upper teeth seductively.  You hook your thumbs into your " + player.armorName + " and pull them away to partially reveal ");
        if (player.lowerBody.cockSpot.count() > 0) MainScreen.text(player.sMultiCockDesc());
        if (player.gender == 3) MainScreen.text(" and ");
        if (player.gender >= 2) MainScreen.text("your " + vaginaDescript(0));
...
if (monster.cocks.length > 0) {
    if (monster.lust >= 60 && monster.cocks.length > 0) MainScreen.text("You see " + monster.pronoun3 + " " + monster.multiCockDescriptLight() + " dribble pre-cum.  ", false);
    ...
    if (monster.lust >= 30 && monster.lust < 60 && monster.cocks.length > 1) MainScreen.text("You see " + monster.pronoun3 + " " + monster.multiCockDescriptLight() + " harden uncomfortably.  ", false);
}
if (monster.vaginas.length > 0) {
    if (monster.plural) {
        if (monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.NORMAL) MainScreen.text(monster.capitalA + monster.short + "'s " + monster.vaginaDescript() + "s dampen perceptibly.  ", false);
        if (monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.WET) MainScreen.text(monster.capitalA + monster.short + "'s crotches become sticky with girl-lust.  ", false);
        if (monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLICK) MainScreen.text(monster.capitalA + monster.short + "'s " + monster.vaginaDescript() + "s become sloppy and wet.  ", false);
        if (monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.DROOLING) MainScreen.text("Thick runners of girl-lube stream down the insides of " + monster.a + monster.short + "'s thighs.  ", false);
        if (monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING) MainScreen.text(monster.capitalA + monster.short + "'s " + monster.vaginaDescript() + "s instantly soak " + monster.pronoun2 + " groin.  ", false);
    }
    else {
        if (monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.NORMAL) MainScreen.text(monster.capitalA + monster.short + "'s " + monster.vaginaDescript() + " dampens perceptibly.  ", false);
        if (monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.WET) MainScreen.text(monster.capitalA + monster.short + "'s crotch becomes sticky with girl-lust.  ", false);
        if (monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLICK) MainScreen.text(monster.capitalA + monster.short + "'s " + monster.vaginaDescript() + " becomes sloppy and wet.  ", false);
        if (monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.DROOLING) MainScreen.text("Thick runners of girl-lube stream down the insides of " + monster.a + monster.short + "'s thighs.  ", false);
        if (monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING) MainScreen.text(monster.capitalA + monster.short + "'s " + monster.vaginaDescript() + " instantly soaks her groin.  ", false);
    }
}
MainScreen.text("\n\n", false);
doNext(playerMenu);
...
if (rand(4) == 0) {
    ...
    if (player.gender == 0) MainScreen.text(assholeDescript() + " tingles with a desire to be filled as your libido spins out of control.", false);
    if (player.gender == 1) {
        if (player.lowerBody.cockSpot.count() == 1) MainScreen.text(player.cockDescript(0) + " twitches obscenely and drips with pre-cum as your libido spins out of control.", false);
        else MainScreen.text(player.multiCockDescriptLight() + " twitch obscenely and drip with pre-cum as your libido spins out of control.", false);
    }
    if (player.gender == 2) MainScreen.text(vaginaDescript(0) + " becomes puffy, hot, and ready to be touched as the magic diverts into it.", false);
    if (player.gender == 3) MainScreen.text(vaginaDescript(0) + " and " + player.multiCockDescriptLight() + " overfill with blood, becoming puffy and incredibly sensitive as the magic focuses on them.", false);
...
if (player.findStatusAffect(StatusAffects.GooArmorSilence) < 0) MainScreen.text("You take in a deep breath and unleash a wave of corrupt red flames from deep within.", false);
...
public kick():void {
    ...
    if(player.lowerBody == LOWER_BODY.KANGAROO) {
        //(tail)
        if (player.tailType == TAIL.KANGAROO) MainScreen.text("You balance on your flexible kangaroo-tail, pulling both legs up before slamming them forward simultaneously in a brutal kick.  ", false);
        //(no tail) 
	...
	//(bunbun kick) 
	else if(player.lowerBody == LOWER_BODY.BUNNY) MainScreen.text("You leap straight into the air and lash out with both your furred feet simultaneously, slamming forward in a strong kick.  ", false);
    //(centaur kick)
    else if(player.lowerBody == LOWER_BODY.CENTAUR) MainScreen.text("You lurch up onto your backlegs, lifting your forelegs from the ground a split-second before you lash them out in a vicious kick.  ", false);
    //(bipedal hoof-kick) 
    else if(player.lowerBody == LOWER_BODY.HOOFED) MainScreen.text("You twist and lurch as you raise a leg and slam your hoof forward in a kick.  ", false);
...
public possess():void {
    ...
    if (monster.inte > 100) MainScreen.text("they were FAR more mentally prepared than anything you can handle, and you're summarily thrown out of their body before you're even able to have fun with them.  Darn, you muse.\n\n", false);
    else MainScreen.text("they have a body that's incompatible with any kind of possession.\n\n", false);
}
	//Success!
...
public runAway(callHook:boolean = true):void {
	...
	//FAIL FLEE
		...
		if(player.canFly()) {
            if (monster.plural) MainScreen.text(monster.capitalA + monster.short + " manage to grab your " + player.legs() + " and drag you back to the ground before you can fly away!", false);
            else MainScreen.text(monster.capitalA + monster.short + " manages to grab your " + player.legs() + " and drag you back to the ground before you can fly away!", false);
        }
		//fail
		else if(player.tailType == TAIL.RACCOON && player.upperBody.head.earType == EARS.RACCOON && player.perks.has("Runner")) MainScreen.text("Using your running skill, you build up a head of steam and jump, but before you can clear the ground more than a foot, your opponent latches onto you and drags you back down with a thud!");
        //Nonflyer messages
        else {
            //Huge balls messages
            if(player.lowerBody.balls > 0 && player.lowerBody.ballSize >= 24) {
                if (player.lowerBody.ballSize < 48) MainScreen.text("With your " + ballsDescriptLight() + " swinging ponderously beneath you, getting away is far harder than it should be.  ", false);
                else MainScreen.text("With your " + ballsDescriptLight() + " dragging along the ground, getting away is far harder than it should be.  ", false);
            }
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 35 || player.lowerBody.butt.buttRating >= 20 || player.lowerBody.hipRating >= 20) {
                ...
                if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 35 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 66) {
                    if (player.lowerBody.hipRating >= 20) {
                        MainScreen.text("Your " + hipDescript() + " forces your gait to lurch slightly side to side, which causes the fat of your " + player.skinTone + " ", false);
                        if (player.lowerBody.butt.buttRating >= 20) MainScreen.text(buttDescript() + " and ", false);
                    ...
                    else if (player.lowerBody.butt.buttRating >= 20) MainScreen.text("Your " + player.skinTone + buttDescript() + " and " + chestDesc() + " wobble and bounce heavily, throwing you off balance and preventing you from moving quick enough to escape.", false);
                ...
                //FOR PLAYERS WITH MASSIVE BREASTS
                    ...
                    if (player.lowerBody.hipRating >= 20) {
                        MainScreen.text("Your " + chestDesc() + " nearly drag along the ground while your " + hipDescript() + " swing side to side ", false);
                        if (player.lowerBody.butt.buttRating >= 20) MainScreen.text("causing the fat of your " + player.skinTone + buttDescript() + " to wobble heavily, ", false);
                        MainScreen.text("forcing your body off balance and preventing you from moving quick enough to get escape.", false);
                    }
                    else if (player.lowerBody.butt.buttRating >= 20) MainScreen.text("Your " + chestDesc() + " nearly drag along the ground while the fat of your " + player.skinTone + buttDescript() + " wobbles heavily from side to side, forcing your body off balance and preventing you from moving quick enough to escape.", false);
                ...
                //FOR PLAYERS WITH EITHER GIANT HIPS OR BUTT BUT NOT THE BREASTS
                    ...
                    MainScreen.text("Your " + hipDescript() + " swing heavily from side to side ", false);
                    if (player.lowerBody.butt.buttRating >= 20) MainScreen.text("causing your " + player.skinTone + buttDescript() + " to wobble obscenely ", false);
                    MainScreen.text("and forcing your body into an awkward gait that slows you down, preventing you from escaping.", false);
                }
                //JUST DA BOOTAH
                else if (player.lowerBody.butt.buttRating >= 20) MainScreen.text("Your " + player.skinTone + buttDescript() + " wobbles so heavily that you're unable to move quick enough to escape.", false);
            }
...
public anemoneSting():void {
    ...
    if(prob <= rand(101)) {
        ...
        else MainScreen.text("You rush " + monster.a + monster.short + ", whipping your hair around to catch it with your tentacles, but " + monster.pronoun1 + " easily dodges.  Oy, you hope you didn't just give yourself whiplash.", false);
    }
	//Success!
	else {
        ...
        else MainScreen.text("As the venom infiltrates " + monster.pronoun3 + " body, " + monster.pronoun1 + " twitches and begins to move more slowly, hampered half by paralysis and half by arousal.", false);
...
public berzerk():void {
	...
	if(player.statusAffects.has("Berzerking")) {
        MainScreen.text("You're already pretty goddamn mad!", true);
...
public corruptedFoxFire():void {
    ...
    MainScreen.text("You are too tired to use this ability.", true);
    doNext(magicalSpecials);
    return;
}
if (player.statusAffects.has("ThroatPunch") || player.statusAffects.has("WebSilence")) {
    MainScreen.text("You cannot focus to use this ability while you're having so much difficult breathing.", true);
    doNext(magicalSpecials);
    return;
}
//This is now automatic - newRound arg defaults to true:	menuLoc = 0;
...
else MainScreen.text("  The dark fog recedes as quickly as it rolled in as they push back your illusions, resisting your hypnotic influence.\n\n");
enemyAI();
}
...
else MainScreen.text("  Like the snapping of a rubber band, reality falls back into its rightful place as they resist your illusory conjurations.\n\n");
enemyAI();
}
...
public doCombat(eventNum:number):void
{
	...
	if(eventNum == 5000) {
			...
			if (player.statusAffects.has("KnockedBack"))
			{
				...
				MainScreen.addButton(1, "Tease", tease);
				MainScreen.addButton(2, "Spells", temp2);
				...
				MainScreen.addButton(7, waitT, wait);
	...
	if(eventNum == 5019) {
			MainScreen.text("You see " + monster.a + monster.short + " make sudden arcane gestures at you!\n\n", false);
			dynStats("lus", player.stats.lib / 10 + player.stats.cor / 10 + 10);
			if (player.lust < 30) MainScreen.text("You feel strangely warm.  ", false);
			if (player.lust >= 30 && player.lust < 60) MainScreen.text("Blood rushes to your groin as a surge of arousal hits you, making your knees weak.  ", false);
			if (player.lust >= 60) MainScreen.text("Images of yourself fellating and fucking the imp assault your mind, unnaturally arousing you.  ", false);
			if (player.lowerBody.cockSpot.count() > 0) {
				if (player.lust >= 60 && player.lowerBody.cockSpot.count() > 0) MainScreen.text("You feel your " + player.multiCockDescriptLight() + " dribble pre-cum.", false);
				if (player.lust >= 30 && player.lust < 60 && player.lowerBody.cockSpot.count() == 1) MainScreen.text("Your " + player.cockDescript(0) + " hardens, distracting you further.", false);
				if (player.lust >= 30 && player.lust < 60 && player.lowerBody.cockSpot.count() > 1) MainScreen.text("Your " + player.multiCockDescriptLight() + " harden uncomfortably.", false);
				if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  ", false);
			}
			if (player.lowerBody.vaginaSpot.count() > 0) {
				if (player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.NORMAL && player.lowerBody.vaginaSpot.count() == 1) MainScreen.text("Your " + allVaginaDescript() + " dampens perceptibly.", false);
				if (player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.NORMAL && player.lowerBody.vaginaSpot.count() > 1) MainScreen.text("Your " + allVaginaDescript() + " dampen perceptibly.", false);
				if (player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.WET && player.lowerBody.vaginaSpot.count() > 0) MainScreen.text("Your crotch becomes sticky with girl-lust.", false);
				if (player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLICK && player.lowerBody.vaginaSpot.count() == 1) MainScreen.text("Your " + allVaginaDescript() + " becomes sloppy and wet.", false);
				if (player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLICK && player.lowerBody.vaginaSpot.count() > 1) MainScreen.text("Your " + allVaginaDescript() + " become sloppy and wet.", false);
				if (player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.DROOLING && player.lowerBody.vaginaSpot.count() > 0) MainScreen.text("Thick runners of girl-lube stream down the insides of your thighs.", false);
				if (player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING && player.lowerBody.vaginaSpot.count() == 1) MainScreen.text("Your " + allVaginaDescript() + " instantly soaks your groin.", false);
				if (player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING && player.lowerBody.vaginaSpot.count() > 1) MainScreen.text("Your " + allVaginaDescript() + " instantly soak your groin.", false);
			}
			MainScreen.text("\n", false);
	...
	if(eventNum == 5020) {
			...
			if (player.statusAffects.has("StoneLust")) {
				MainScreen.text("The orb inside you grows warm, almost hot, suffusing your body with heat and arousal.  ", false);
				dynStats("lus", 8 + int(player.stats.sens) / 10);
			}
			else {
				MainScreen.text("You feel the sands shift by your " + player.feet() + ", and look down to see something slip out of the sands and into your clothes!  It feels incredibly smooth and circular as it glides upward along your " + player.leg() + ", its progress unaffected by your frantic effort to dislodge it.  ", false);
	...
	if(eventNum == 5021) {
			if (monk == 2) {
				MainScreen.text("Jojo looks lost in thought for a moment, and fails to attack.  ", false);
				monster.lust += 4;
			}
			if (monk == 3) {
				MainScreen.text("Jojo blushes as he fights you, distracted by a stray thought.  You think you see a bulge in the loose cloth of his pants.  ", false);
				monster.lust += 8;
			}
			if (monk == 4) {
				MainScreen.text("Jojo stumbles, shakes his head, and pulls one of his hands away from the stiff tent in his pants.  ", false);
				monster.lust += 10;
			}
			if (monk == 5) {
				MainScreen.text("Jojo frantically jerks his " + monster.cockDescriptShort(0) + ", stroking the " + monster.cockDescriptShort(0) + " as it leaks pre-cum at the sight of you.  ", false);
				monster.lust += 15;
			}
			if (monster.lust > 50 && monster.lust < 60) MainScreen.text("The mouse's skin remains flushed with the beginnings of arousal.  ", false);
			if (monster.lust >= 60 && monster.lust < 70) MainScreen.text("The mouse's eyes constantly dart over your most sexual parts, betraying his lust.  ", false);
			if (monster.lust >= 70 && monster.lust < 85) MainScreen.text("The mouse is having trouble moving due to the rigid protrusion from his groin.  ", false);
			if (monster.lust >= 85) MainScreen.text("The mouse is panting and softly whining, each movement seeming to make his bulge more pronounced.  You don't think he can hold out much longer.  ", false);
			if (monster.lust >= 100) doNext(endLustVictory);
			else doNext(1);
	}
			//Jojo loses via lust
	if(eventNum == 5022) {
			MainScreen.text("  You disrobe and prepare to ", false);
			if (monk == 5) MainScreen.text("fuck your violent little slut senseless.  ", false);
	...
	if(eventNum == 5024) {
		if(monk == 2 || monk == 3)
		{
				...
				MainScreen.text("Jojo grins wickedly as he senses your defeat, " + monster.cockDescriptShort(0) + " throbbing hard.  ", true);
				if (player.lust >= 100) {
					if (player.gender == 1) {
					...
					if (player.gender >= 2) {
						...
						if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("Your " + player.cockDescript(0) + " splatters the ground with cum repeatedly, until both your genders are raw and sore.  ", false);
						else MainScreen.text("Your " + vaginaDescript(0) + " cums on him many more times it until it is sore and tender, dripping with spunk.  ", false);
						MainScreen.text("You black out as Jojo cums AGAIN, forcing a river of spunk from your already over-filled uterus.", false);
	...
	if(eventNum == 5025)
	{
		...
		{
			MainScreen.text("<b>GAME OVER</b>", true);
			menu();
			MainScreen.addButton(0, "Game Over", gameOver);
			MainScreen.addButton(3, "NewGamePlus", charCreation.newGamePlus);
			if (flags[FlagEnum.EASY_MODE_ENABLE_FLAG] == 1 || debug) MainScreen.addButton(4, "CHEAT", camp.campMenu);
	...
	if(eventNum == 5035) {
		...
		if (!this.testingBlockExiting) {
			...
			MainScreen.addButton(0, "Game Over", gameOver);
			MainScreen.addButton(3, "NewGamePlus", charCreation.newGamePlus);
			if (flags[FlagEnum.EASY_MODE_ENABLE_FLAG] == 1 || debug) MainScreen.addButton(4, "Debug Cheat", camp.campMenu);
	...
	if(eventNum == 5028) {
			...
			if (player.perks.has("MarblesMilk")) {
				...
				MainScreen.text("The last thing you hear is the Sand Witch screaming \"<i>What the hell is going on?  I ALWAYS get what I want!</i>\"", false);
			}
			else {
	...
	if(eventNum == 5036) {
			//Blind dodge change
			if (monster.statusAffects.has("Blind")) {
				MainScreen.text(monster.capitalA + monster.short + " completely misses you with a blind sting!!", false);
				combatRoundOver();
				return;
			}
			//Determine if dodged!
			if (player.stats.spe - monster.spe > 0 && int(Math.random() * (((player.stats.spe - monster.spe) / 4) + 80)) > 80) {
				if (player.stats.spe - monster.spe < 8) MainScreen.text("You narrowly avoid " + monster.a + monster.short + "'s stinger!", false);
				if (player.stats.spe - monster.spe >= 8 && player.stats.spe - monster.spe < 20) MainScreen.text("You dodge " + monster.a + monster.short + "'s stinger with superior quickness!", false);
				if (player.stats.spe - monster.spe >= 20) MainScreen.text("You deftly avoid " + monster.a + monster.short + "'s slow attempts to sting you.", false);
				combatRoundOver();
				return;
			}
			//determine if avoided with armor.
			if (player.armorDef >= 10 && rand(4) > 0) {
				MainScreen.text("Despite her best efforts, " + monster.a + monster.short + "'s sting attack can't penetrate your armor.", false);
				combatRoundOver();
				return;
			}
			...
			//Lust 50% of the time
			if (rand(2) == 0) {
				MainScreen.text("Searing pain lances through you as " + monster.a + monster.short + " manages to sting you!  You stagger back a step and nearly trip, flushing hotly.  ", false);
				MainScreen.text("Oh no!  You've been injected with some kind of aphrodisiac.  You've got to keep focused, you can't think about... fucking... ", false);
				if (player.gender == 1) MainScreen.text("or dripping honey-slicked cunts beckoning you. ", false);
				if (player.gender == 2) MainScreen.text("planting your aching sex over her face while you lick her sweet honeypot. ", false);
				if (player.gender == 3) MainScreen.text("or cocks, tits, and puffy nipples. ", false);
				dynStats("lus", 25);
				if (player.lust > 60) {
					MainScreen.text(" You shake your head and struggle to stay focused,", false);
					if (player.gender == 1 || player.gender == 3) MainScreen.text(" but it's difficult with the sensitive bulge in your groin.", false);
					if (player.gender == 2) MainScreen.text(" but can't ignore the soaking wetness in your groin.", false);
					if (player.stats.sens > 50) MainScreen.text("  The sensitive nubs of your nipples rub tightly under your " + player.armorName + ".", false);
				}
				else MainScreen.text(" You shake your head and clear the thoughts from your head, focusing on the task at hand.", false);
				if (player.findStatusAffect(StatusAffects.lustvenom) < 0) player.statusAffects.add(new StatusAffect("lustvenom", 0, 0, 0, 0)));
			}
			//Paralise the other 50%!
			else {
				MainScreen.text("Searing pain lances through you as " + monster.a + monster.short + " manages to sting you!  You stagger back a step and nearly trip, finding it hard to move yourself.", false);
				...
				if (player.statusAffects.has("ParalyzeVenom")) {
					...
					MainScreen.text("  It's getting much harder to move, you're not sure how many more stings like that you can take!", false);
				}
				else {
					player.statusAffects.add(new StatusAffect("ParalyzeVenom", 2, 2, 0, 0)));
					dynStats("str", -2, "spe", -2);
					MainScreen.text("  You've fallen prey to paralyzation venom!  Better end this quick!", false);
				}
			}
			if (player.lust >= 100) doNext(endLustLoss);
			else doNext(5000);
	...
	if(eventNum == 5037) {
			...
			menuLoc = 0;
			if (player.lowerBody.tailVenom < 33) {
				MainScreen.text("You do not have enough venom to sting right now!", true);
				doNext(physicalSpecials);
				return;
			}
			//Worms are immune!
	...
	if(eventNum == 5039) {
			MainScreen.text("The creature surges forward slowly with a swing that you easily manage to avoid.  You notice traces of green liquid spurt from the creature as it does, forming a thin mist that makes your skin tingle with excitement when you inhale it.", false);
			dynStats("lus", player.stats.lib / 10 + 8);
			doNext(1);
	}
			//Green Slime Lust lossssss!
	if(eventNum == 5040) {
			MainScreen.text("The creature collapses backwards as its cohesion begins to give out, and the faint outline of eyes and a mouth form on its face.  Its chest heaves as if it were gasping, and the bolt upright erection it sports visibly quivers and pulses before relaxing slightly.", false);
			monster.lust -= 13;
			doNext(1);
	}
			//Demon pack attack
	if(eventNum == 5043) {
			...
			//Determine if dodged!
			if (player.stats.spe - monster.spe > 0 && int(Math.random() * (((player.stats.spe - monster.spe) / 4) + 80)) > 80) {
				MainScreen.text("You duck, weave, and dodge.  Despite their best efforts, the throng of demons only hit the air and each other.", false);
				combatRoundOver();
				return;
			}
			//Determine if evaded
			if (player.perks.has("Evade") && rand(100) < 10) {
				MainScreen.text("Using your skills at evading attacks, you anticipate and sidestep " + monster.a + monster.short + "' attacks.", false);
				combatRoundOver();
				return;
			}
			//("Misdirection"
			if (player.perks.has("Misdirection") && rand(100) < 15 && player.armorName == "red, high-society bodysuit") {
				MainScreen.text("Using Raphael's teachings, you anticipate and sidestep " + monster.a + monster.short + "' attacks.", false);
				combatRoundOver();
				return;
			}
			//Determine if cat'ed
			if (player.perks.has("Flexibility") && rand(100) < 6) {
				MainScreen.text("With your incredible flexibility, you squeeze out of the way of " + monster.a + monster.short + "' attacks.", false);
				combatRoundOver();
				return;
			}
			//Determine damage - str modified by enemy toughness!
			...
			if (temp <= 0) {
				temp = 0;
				if (!monster.plural) MainScreen.text("You deflect and block every " + monster.weaponVerb + " " + monster.a + monster.short + " throw at you.", false);
				else MainScreen.text("You deflect " + monster.a + monster.short + " " + monster.weaponVerb + ".");
			}
			if (temp > 0 && temp < 6) {
				MainScreen.text("You are struck a glancing blow by " + monster.a + monster.short + "! (" + temp + ")", false);
			}
			if (temp > 5 && temp < 11) {
				MainScreen.text(monster.capitalA + monster.short + " wound you! (" + temp + ")", false);
			}
			if (temp > 10 && temp < 21) {
				MainScreen.text(monster.capitalA + monster.short + " stagger you with the force of " + monster.pronoun3 + " " + monster.weaponVerb + "s! (" + temp + ")", false);
			}
			if (temp > 20) {
				MainScreen.text(monster.capitalA + monster.short + " <b>mutilates</b> you with powerful fists and " + monster.weaponVerb + "s! (" + temp + ")", false);
			}
			statScreenRefresh();
			MainScreen.text("\n", false);
			combatRoundOver();
			...
			//Demonic lust attack!
	if(eventNum == 5044) {
			if (player.lust < 35) MainScreen.text("The demons press in close against you and although they fail to hit you with an attack, the sensation of their skin rubbing against yours feels highly erotic.", false);
			if (player.lust >= 35 && player.lust < 65) {
				MainScreen.text("The push of the demons' sweaty, seductive bodies sliding over yours is deliciously arousing and you feel your ", false);
				if (player.lowerBody.cockSpot.count() > 0) MainScreen.text(player.multiCockDescriptLight() + " hardening ", false);
				else if (player.lowerBody.vaginaSpot.count() > 0) MainScreen.text(vaginaDescript(0) + " get wetter ", false);
				MainScreen.text("in response to all the friction.", false);
			}
			if (player.lust >= 65) {
				MainScreen.text("As the demons mill around you, their bodies rub constantly over yours, and it becomes harder and harder to keep your thoughts on the fight or resist reaching out to touch a well lubricated cock or pussy as it slips past.  You keep subconsciously moving your ", false);
				if (player.gender == 1) MainScreen.text(player.multiCockDescriptLight() + " towards the nearest inviting demonic hole.", false);
				if (player.gender == 2) MainScreen.text(vaginaDescript(0) + " towards the nearest swinging cock.", false);
				if (player.gender == 3) MainScreen.text("aching cock and thirsty pussy towards the nearest thing willing to fuck it.", false);
				if (player.gender == 0) MainScreen.text("groin, before remember there is nothing there to caress.", false);
			}
			dynStats("lus", 10 + player.stats.sens / 10);
			combatRoundOver();
	}
	...
	if(eventNum == 5056) {
			MainScreen.text("You turn to confront the worms and combat begins!", true);
			spriteSelect(76);
			startCombat(new WormMass());
	}
			//Do nothing
	if(eventNum == 5057) {
			MainScreen.text("You do nothing, allowing the worms to enter combat range!", true);
			startCombat(new WormMass());
	}
			//worm toggle on
	if(eventNum == 5058) {
			wormsOn();
			MainScreen.text("You actually think it's kind of a hot idea, and wonder if such creatures actually exist in this land as you make your way back to camp.", true);
			doNext(camp.returnToCampUseOneHour);
	}
			//worm toggle off
	if(eventNum == 5059) {
			MainScreen.text("You shudder in revulsion and figure the sign to be the result of someone's perverted fantasy.", true);
			wormsOff();
			doNext(camp.returnToCampUseOneHour);
	}
			//InfestAttack
	...
	if(eventNum == 5061) {
			MainScreen.text("You shrug and keep walking, not sure how you feel about the strange sign.", true);
	...
	if(eventNum == 5071) {
			...
			if (monster.statusAffects.has("PCTailTangle")) {
				...
				MainScreen.text("You writhe uselessly, trapped inside the goo girl's warm, seething body. Darkness creeps at the edge of your vision as you are lulled into surrendering by the rippling vibrations of the girl's pulsing body around yours.", true);
				temp = takeDamage(.35 * maxHP());
				MainScreen.text(" (" + temp + ")", false);
				combatRoundOver();
				return;
				...
				MainScreen.text("The naga's grip on you tightens as you relax into the stimulating pressure.", true);
				dynStats("lus", player.stats.sens / 5 + 5);
				takeDamage(5 + rand(5));
				combatRoundOver();
				return;
				...
				if (player.lowerBody.cockSpot.count() > 0) MainScreen.text("The creature continues spiraling around your cock, sending shivers up and down your body. You must escape or this creature will overwhelm you!", true);
				else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("The creature continues sucking your clit and now has latched two more suckers on your nipples, amplifying your growing lust. You must escape or you will become a mere toy to this thing!", true);
				else MainScreen.text("The creature continues probing at your asshole and has now latched " + num2Text(player.upperBody.chest.countNipples()) + " more suckers onto your nipples, amplifying your growing lust.  You must escape or you will become a mere toy to this thing!", true);
				dynStats("lus", (8 + player.stats.sens / 10));
				combatRoundOver();
				return;
			}
			...
			else MainScreen.text("You decide not to take any action this round.\n\n", true);
			enemyAI();
			return;
	}
			//Tentacle Struggle
	if(eventNum == 5077) {
			...
			if (monster.statusAffects.has("PCTailTangle")) {
				...
				//[Struggle](successful) :
				if (rand(3) == 0 || rand(80) < player.str) {
					MainScreen.text("You claw your fingers wildly within the slime and manage to brush against her heart-shaped nucleus. The girl silently gasps and loses cohesion, allowing you to pull yourself free while she attempts to solidify.", true);
					player.statusAffects.remove("GooBind");
				}
				//Failed struggle
				else {
				...
				if (rand(3) == 0 || rand(80) < player.str / 1.5) {
					MainScreen.text("You wriggle and squirm violently, tearing yourself out from within the naga's coils.", true);
					player.statusAffects.remove("NagaBind");
				}
				else {
					MainScreen.text("The naga's grip on you tightens as you struggle to break free from the stimulating pressure.", true);
					dynStats("lus", player.stats.sens / 10 + 2);
					takeDamage(7 + rand(5));
				}
				combatRoundOver();
				...
				//Fail to break free
					...
					MainScreen.text("Despite trying to escape, the creature only tightens its grip, making it difficult to breathe.\n\n", false);
					takeDamage(5);
					if (player.lowerBody.cockSpot.count() > 0) MainScreen.text("The creature continues spiraling around your cock, sending shivers up and down your body. You must escape or this creature will overwhelm you!", false);
					else if (player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("The creature continues sucking your clit and now has latched two more suckers on your nipples, amplifying your growing lust. You must escape or you will become a mere toy to this thing!", true);
					else MainScreen.text("The creature continues probing at your asshole and has now latched " + num2Text(player.upperBody.chest.countNipples()) + " more suckers onto your nipples, amplifying your growing lust.  You must escape or you will become a mere toy to this thing!", true);
					dynStats("lus", (3 + player.stats.sens / 10 + player.stats.lib / 20));
					combatRoundOver();
	...
	if(eventNum == 5079) {
			if (player.fatigue + physicalCost(25) > 100) {
				MainScreen.text("You're too fatigued to fire the bow!", true);
				doNext(5000);
				return;
			}
			if (monster.statusAffects.has("BowDisabled")) {
				MainScreen.text("You can't use your bow right now!", true);
				doNext(5000);
				return;
			}
			fatigue(25, 2);
			...
			//[Bow Response]
			if (monster.short == "Isabella") {
				if (monster.statusAffects.has("Blind")) MainScreen.text("Isabella hears the shot and turns her shield towards it, completely blocking it with her wall of steel.\n\n", false);
				else MainScreen.text("You arrow thunks into Isabella's shield, completely blocked by the wall of steel.\n\n", false);
				if (isabellaFollowerScene.isabellaAccent()) MainScreen.text("\"<i>You remind me of ze horse-people.  They cannot deal vith mein shield either!</i>\" cheers Isabella.\n\n", false);
				else MainScreen.text("\"<i>You remind me of the horse-people.  They cannot deal with my shield either!</i>\" cheers Isabella.\n\n", false);
				enemyAI();
				return;
			}
			...
			if (damage < 0) damage = 0;
			if (damage == 0) {
				if (monster.inte > 0) MainScreen.text(monster.capitalA + monster.short + " shrugs as the arrow bounces off them harmlessly.\n\n", false);
				else MainScreen.text("The arrow bounces harmlessly off " + monster.a + monster.short + ".\n\n", false);
				enemyAI();
				return;
			}
			if (monster.short == "pod") MainScreen.text("The arrow lodges deep into the pod's fleshy wall", false);
			else if (monster.plural) MainScreen.text(monster.capitalA + monster.short + " look down at the arrow that now protrudes from one of " + monster.pronoun3 + " bodies", false);
			else MainScreen.text(monster.capitalA + monster.short + " looks down at the arrow that now protrudes from " + monster.pronoun3 + " body", false);
			if (player.perks.has("HistoryFighter")) damage *= 1.1;
			damage = doDamage(damage);
			monster.lust -= 20;
			if (monster.lust < 0) monster.lust = 0;
			if (monster.HP <= 0) {
				if (monster.short == "pod") MainScreen.text(". (" + String(damage) + ")\n\n", false);
				else if (monster.plural) MainScreen.text(" and stagger, collapsing onto each other from the wounds you've inflicted on " + monster.pronoun2 + ".  (" + String(damage) + ")\n\n", false);
				else MainScreen.text(" and staggers, collapsing from the wounds you've inflicted on " + monster.pronoun2 + ".  (" + String(damage) + ")\n\n", false);
				doNext(endHpVictory);
				return;
			}
	...
	if(eventNum == 5144) {
		MainScreen.text("Which of your special fire-breath attacks would you like to use?", true);
