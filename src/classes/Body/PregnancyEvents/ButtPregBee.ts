...
export default class ButtPregBee implements IPregnancyEvent {
    public incubationDisplay(player: Player) {
        ...
        if (player.buttPregnancyIncubation == 20) {
            ...
            if (player.lowerBody.cockSpot.count() == 1) {
                ...
                if (player.lowerBody.cockSpot.countType(CockType.HORSE) == 1) MainScreen.text(", each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air, until your " + CockDescriptor.describeCock(player, 0) + " is twitching and dripping, the flare swollen and purple.  ", false);
                if (player.lowerBody.cockSpot.countType(CockType.DOG) == 1) MainScreen.text(", each inhalation making it thicker, harder, and firmer.  You suck in huge lungfuls of air, desperate for more, until your " + CockDescriptor.describeCock(player, 0) + " is twitching and dripping, its knot swollen to the max.  ", false);
                if (player.normalCocks() == 1) MainScreen.text(", each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air, until your " + CockDescriptor.describeCock(player, 0) + " is twitching and dripping, the head swollen and purple.  ", false);
                //FAILSAFE FOR NEW COCKS
                if (player.normalCocks() == 0 && player.lowerBody.cockSpot.countType(CockType.DOG) == 0 && player.lowerBody.cockSpot.countType(CockType.HORSE) == 0) MainScreen.text(", each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air until your " + CockDescriptor.describeCock(player, 0) + " is twitching and dripping.  ", false);
            }
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text("groin.  Your " + player.CockDescriptor.describeMultiCockShort(player) + " fill and grow with every lungful of the stuff you breathe in.  You suck in great lungfuls of the tainted air, desperate for more, your cocks twitching and dripping with need.  ", false);
    ...
    public birth(player: Player) {
        ...
        if (player.averageLactation() >= 1 && player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) MainScreen.text("Seeking even greater sensation, your hands gather the honey and massage it into your " + BreastDescriptor.describeBreastRow(player.upperBody.chest.get(0)) + ", slowly working up to your nipples.  Milk immediately begins pouring out from the attention, flooding your chest with warmth.  ", false);
