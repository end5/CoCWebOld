...
export default class PregAnemone implements IPregnancyEvent {
    ...
    public birth(player: Player) {
        ...
        //[(if pc has 0-9 existing cocks)
            ...
            //[(if cocks)
            if (player.lowerBody.cockSpot.count() > 0) MainScreen.text(" below your " + player.CockDescriptor.describeMultiCockShort(player), false);
            ...
            //[(dick slot 1 exists)
            if (player.lowerBody.cockSpot.count() > 0) MainScreen.text("  The tentacles writhe around, rubbing against your " + player.CockDescriptor.describeMultiCockShort(player), false);
            //(doesn't exist)
            ...
            MainScreen.text(" and you quickly become fully erect from the aphrodisiac they inject.  Over and over the tentacles caress " + player.CockDescriptor.describeMultiCockSimpleOne(player) + " sensually, leaving behind a tingling trail of vibrant pleasure", false);
            ...
            //[(dick1 exists)
            if (player.lowerBody.cockSpot.count() > 1) MainScreen.text(", followed in short order by white squirts from " + player.CockDescriptor.describeMultiCockSimpleOne(player) + " remaining", false);
