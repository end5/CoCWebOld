...
export function brownEgg(large: boolean, player: Player): void {
    Render.text("You devour the egg, momentarily sating your hunger.\n\n", true);
    if (!large) {
        Render.text("You feel a bit of additional weight on your backside as your " + ButtDescriptor.buttDescription(player) + " gains a bit more padding.", true);
        player.lowerBody.butt.buttRating++;
    }
    else {
        Render.text("Your " + ButtDescriptor.buttDescription(player) + " wobbles, nearly throwing you off balance as it grows much bigger!", true);
        player.lowerBody.butt.buttRating += 2 + Utils.rand(3);
    }
    if (Utils.chance(33)) {
        if (large)
            Render.text(player.modThickness(100, 8), false);
        else
            Render.text(player.modThickness(95, 3), false);
    }
}
...
export function purpleEgg(large: boolean, player: Player): void {
    ...
    if (!large || player.lowerBody.hipRating > 20) {
        Render.text("You stumble as you feel your " + LowerBodyDescriptor.hipDescription(player) + " widen, altering your gait slightly.", false);
        player.lowerBody.hipRating++;
    }
    else {
        Render.text("You stagger wildly as your hips spread apart, widening by inches.  When the transformation finishes you feel as if you have to learn to walk all over again.", false);
        player.lowerBody.hipRating += 2 + Utils.rand(2);
    }
    if (Utils.chance(33)) {
        if (large)
            Render.text(player.modThickness(80, 8), false);
        else
            Render.text(player.modThickness(80, 3), false);
    }
}
    ...
    if (!large) {
        ...
        if (player.lowerBody.balls > 0) {
            if (player.lowerBody.ballSize > 15) {
                ...
                Render.text("Your scrotum slowly shrinks, eventually disappearing entirely!  <b>You've lost your balls!</b>\n\n", false);
            }
        }
        //Fertility boost
        if (player.lowerBody.vaginaSpot.hasVagina() && player.fertility < 40) {
            Render.text("You feel a tingle deep inside your body, just above your " + VaginaDescriptor.vaginaDescript(player, player.lowerBody.vaginaSpot.list[0]) + ", as if you were becoming more fertile.\n\n", false);
            player.fertility += 5;
        }
    }
    //LARGE
        ...
        if (player.lowerBody.vaginaSpot.count() > 0 && player.fertility < 70) {
            Render.text("You feel a powerful tingle deep inside your body, just above your " + vaginaDescript(0) + ". Instinctively you know you have become more fertile.\n\n", false);
            player.fertility += 10;
        }
    }
    if (Utils.rand(3) == 0) {
        if (large) Render.text(player.modFem(100, 8), false);
        else Render.text(player.modFem(95, 3), false);
    }
}
...
export function blueEgg(large: boolean, player: Player): void {
    let temp2: number = 0;
    let temp3: number = 0;
    Render.text("You devour the egg, momentarily sating your hunger.", true);
    if (!large) {
        //Kill pussies!
        ...
        if (player.lowerBody.cockSpot.count() > 0) {
            ...
            if (player.lowerBody.cockSpot.count() == 1) {
                ...
                //Display the degree of thickness change.
                if (temp3 >= 1) {
                    if (player.lowerBody.cockSpot.count() == 1) Render.text("  Your " + multiCockDescriptLight() + " spreads rapidly, swelling an inch or more in girth, making it feel fat and floppy.", false);
                    else Render.text("  Your " + multiCockDescriptLight() + " spread rapidly, swelling as they grow an inch or more in girth, making them feel fat and floppy.", false);
                }
                if (temp3 <= .5) {
                    if (player.lowerBody.cockSpot.count() > 1) Render.text("  Your " + multiCockDescriptLight() + " feel swollen and heavy. With a firm, but gentle, squeeze, you confirm your suspicions. They are definitely thicker.", false);
                    else Render.text("  Your " + multiCockDescriptLight() + " feels swollen and heavy. With a firm, but gentle, squeeze, you confirm your suspicions. It is definitely thicker.", false);
                }
                if (temp3 > .5 && temp2 < 1) {
                    if (player.lowerBody.cockSpot.count() == 1) Render.text("  Your " + multiCockDescriptLight() + " seems to swell up, feeling heavier. You look down and watch it growing fatter as it thickens.", false);
                    if (player.lowerBody.cockSpot.count() > 1) Render.text("  Your " + multiCockDescriptLight() + " seem to swell up, feeling heavier. You look down and watch them growing fatter as they thicken.", false);
                }
                dynStats("lib", 1, "sen", 1, "lus", 20);
            }
    ...
    //LARGE
        ...
        if (player.bRows() > 1 || player.lowerBody.butt.buttRating > 5 || player.lowerBody.hipRating > 5 || player.lowerBody.vaginaSpot.hasVagina()) Render.text("\n\n", false);
        //Kill pussies!
        if (player.lowerBody.vaginaSpot.count() > 0) {
            Render.text("Your vagina clenches in pain, doubling you over.  You slip a hand down to check on it, only to feel the slit growing smaller and smaller until it disappears, taking your clit with it!\n\n", false);
            if (player.bRows() > 1 || player.lowerBody.butt.buttRating > 5 || player.lowerBody.hipRating > 5) Render.text("  ", false);
            player.removeVagina(0, 1);
            player.lowerBody.vaginaSpot.list[0].clitLength = .5;
            player.genderCheck();
        }
        //Kill extra boobages
        if (player.bRows() > 1) {
            Render.text("Your back relaxes as extra weight vanishes from your chest.  <b>Your lowest " + breastDescript(player.bRows() - 1) + " have vanished.</b>", false);
            if (player.lowerBody.butt.buttRating > 5 || player.lowerBody.hipRating > 5) Render.text("  ", false);
            //Remove lowest row.
            player.removeBreastRow((player.bRows() - 1), 1);
        }
        //Ass/hips shrinkage!
        if (player.lowerBody.butt.buttRating > 5) {
            Render.text("Muscles firm and tone as you feel your " + buttDescript() + " become smaller and tighter.", false);
            if (player.lowerBody.hipRating > 5) Render.text("  ", false);
            player.lowerBody.butt.buttRating -= 2;
        }
        if (player.lowerBody.hipRating > 5) {
            Render.text("Feeling the sudden burning of lactic acid in your " + hipDescript() + ", you realize they have slimmed down and firmed up some.", false);
    ...
    if (Utils.rand(3) == 0) {
        if (large) Render.text(player.modFem(0, 8), false);
        else Render.text(player.modFem(5, 3), false);
    }
}
...
export function blackRubberEgg(large: boolean, player: Player): void {
    ...
    if (!large) {
        ...
        if ((player.skinAdj != "smooth" && player.skinAdj != "latex" && player.skinAdj != "rubber") || player.skinDesc != "skin") {
            ...
            if (player.skinType == SKIN.PLAIN) Render.text(" loses its blemishes, becoming flawless smooth skin.", false);
            if (player.skinType == SKIN.FUR) Render.text(" falls out in clumps, revealing smooth skin underneath.", false);
            if (player.skinType == SKIN.SCALES) Render.text(" begins dropping to the ground in a pile around you, revealing smooth skin underneath.", false);
            if (player.skinType > SKIN.SCALES) Render.text(" shifts and changes into flawless smooth skin.", false);
    ...
    if (large) {
        ...
        if (player.skinDesc == "skin" && player.skinAdj == "smooth") {
            ...
            if (Utils.rand(2) == 0) {
                ...
                Render.text("a layer of pure latex.  ", false);
            }
            else {
                player.skinDesc = "skin";
                player.skinAdj = "rubber";
                Render.text("a layer of sensitive rubber.  ", false);
            }
            flags[FlagEnum.PC_KNOWS_ABOUT_BLACK_EGGS] = 1;
            if (player.stats.cor < 66) Render.text("You feel like some kind of freak.", false);
            else Render.text("You feel like some kind of sexy " + player.skinDesc + " love-doll.", false);
