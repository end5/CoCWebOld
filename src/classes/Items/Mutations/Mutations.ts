...
export default class Mutations {
    ...
    public vitalityTincture(player: Player): void {
        ...
        if (Utils.chance(33))
            outputText(player.modTone(95, 3), false);
    }
    ...
    public scholarsTea(player: Player): void {
        ...
        if (Utils.rand(3) == 0)
            outputText(player.modTone(15, 1), false);
    ...
    public incubiDraft(tainted: boolean, player: Player): void {
        ...
        if (rando < 50) {
            if (player.lowerBody.cockSpot.count() == 1) {
                ...
                if (player.lowerBody.cockSpot.list[0].cockType != CockType.DEMON) outputText("  With the transformation complete, your " + cockDescript(0) + " returns to its normal coloration.", false);
        ...
        if (Utils.rando >= 50 && Utils.rando < 93) {
            ...
            if (player.lowerBody.cockSpot.count() == 1) {
                ...
                //Display the degree of thickness change.
                if (temp3 >= 1) {
                    if (player.lowerBody.cockSpot.count() == 1) outputText("  Your cock spreads rapidly, swelling an inch or more in girth, making it feel fat and floppy.", false);
                ...
                if (temp3 <= .5) {
                    if (player.lowerBody.cockSpot.count() > 1) outputText("  Your cocks feel swollen and heavy. With a firm, but gentle, squeeze, you confirm your suspicions. They are definitely thicker.", false);
                ...
                if (temp3 > .5 && temp2 < 1) {
                    if (player.lowerBody.cockSpot.count() == 1) outputText("  Your cock seems to swell up, feeling heavier. You look down and watch it growing fatter as it thickens.", false);
                    if (player.lowerBody.cockSpot.count() > 1) outputText("  Your cocks seem to swell up, feeling heavier. You look down and watch them growing fatter as they thicken.", false);
                }
                if (tainted) dynStats("lib", 3, "sen", 5, "lus", 10, "cor", 3);
                else dynStats("lib", 3, "sen", 5, "lus", 10);
            }
        ...
        player.genderCheck();
        if (Utils.rand(4) == 0 && tainted) outputText(player.modFem(5, 2), false);
        if (Utils.rand(4) == 0 && tainted) outputText(player.modThickness(30, 2), false);
    }
    ...
    public minotaurCum(player: Player): void {
        ...
        if (flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] == 0) outputText("potent musk washes over you.", false);
        else outputText("heavenly scent fills your nostrils.", false);
        if (flags[FlagEnum.MINOTAUR_CUM_ADDICTION_TRACKER] < 50) outputText("  It makes you feel dizzy, ditzy, and placid.", false);
        ...
        if (player.lowerBody.vaginaSpot.hasVagina()) {
            ...
            if (player.lowerBody.vaginaSpot.list[0].clitLength < 3) outputText("parting your lips.", false);
            else outputText("bursting free of your lips and bobbing under its own weight.", false);
            if (player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.NORMAL) outputText("  Wetness builds inside you as your " + vaginaDescript(0) + " tingles and aches to be filled.", false);
            else if (player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.SLICK) outputText("  A trickle of wetness escapes your " + vaginaDescript(0) + " as your body reacts to the desire burning inside you.", false);
            else if (player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.DROOLING) outputText("  Wet fluids leak down your thighs as your body reacts to this new stimulus.", false);
    ...
    public minotaurBlood(player: Player): void {
        ...
        if (player.lowerBody != LOWER_BODY.HOOFED && player.lowerBody != LOWER_BODY.CENTAUR) {
            if (changes < changeLimit && Utils.rand(3) == 0) {
                ...
                if (player.skinType != SKIN.FUR) outputText("  A fine coat of fur grows out below your waist, itching briefly as it fills in.");
        ...
        if (!flags[FlagEnum.HYPER_HAPPY]) {
            ...
            if (changes < changeLimit && player.bRows() > 1 && Utils.rand(3) == 0) {
                ...
                outputText(". The " + nippleDescript(player.upperBody.chest.count() - 1) + "s even fade until nothing but ", false);
                if (player.skinType == SKIN.FUR) outputText(player.hairColor + " " + player.skinDesc, false);
        ...
        if (Utils.rand(4) == 0) outputText(player.modFem(5, 10), false);
        if (Utils.rand(4) == 0) outputText(player.modTone(85, 3), false);
        if (Utils.rand(4) == 0) outputText(player.modThickness(70, 4), false);
        //Default
        if (changes == 0) {
    ...
    public equinum(player: Player): void {
        ...
        if ((player.gender == 1 || player.gender == 3) && Utils.rand(1.5) == 0 && changes < changeLimit) {
            ...
            if (Utils.rand(2) == 0 && changes < changeLimit && player.horseCocks() > 0) {
                ...
                if (player.stats.cor + player.stats.lib < 50) outputText(" to have such a splendid tool.  You idly daydream about cunts and pussies, your " + Appearance.cockNoun(CockType.HORSE) + " plowing them relentlessly, stuffing them pregnant with cum", false);
                if (player.stats.cor + player.stats.lib >= 50 && player.stats.cor + player.stats.lib < 80) outputText(" to be this way... You breath the powerful animalistic scent and fantasize about fucking centaurs night and day until their bellies slosh with your cum", false);
                if (player.stats.cor + player.stats.lib >= 75 && player.stats.cor + player.stats.lib <= 125) outputText(" to be a rutting stud.  You ache to find a mare or centaur to breed with.  Longing to spend your evenings plunging a " + Appearance.cockNoun(CockType.HORSE) + " deep into their musky passages, dumping load after load of your thick animal-cum into them.  You'd be happy just fucking horsecunts morning, noon, and night.  Maybe somewhere there is a farm needing a breeder..", false);
                if (player.stats.cor + player.stats.lib > 125) outputText(" to whinny loudly like a rutting stallion.  Your " + Appearance.cockNoun(CockType.HORSE) + " is perfect for fucking centaurs and mares.  You imagine the feel of plowing an equine pussy deeply, bottoming out and unloading sticky jets of horse-jizz into its fertile womb.  Your hand strokes your horsecock of its own accord, musky pre dripping from the flared tip with each stroke.  Your mind wanders to the thought of you with a harem of pregnant centaurs.", false);
                outputText(".", false);
                if (player.stats.cor < 30) outputText("  You shudder in revulsion at the strange thoughts and vow to control yourself better.", false);
                if (player.stats.cor >= 30 && player.stats.cor < 60) outputText("  You wonder why you thought such odd things, but they have a certain appeal.", false);
                if (player.stats.cor >= 60 && player.stats.cor < 90) outputText("  You relish your twisted fantasies, hoping to dream of them again.", false);
                if (player.stats.cor >= 90) outputText("  You flush hotly and give a twisted smile, resolving to find a fitting subject to rape and relive your fantasies.", false);
                    ...
                    //multiple
                        ...
                        while (temp3 > 0) {
                            ...
                            if (player.upperBody.chest.list[temp3].breastRating > 3) {
                                ...
                                temp2++;
                                outputText("\n", false);
                                if (temp3 < player.upperBody.chest.count() - 1) outputText("...and y", false);
                                else outputText("Y", false);
                                outputText("our " + breastDescript(temp3) + " shrink, dropping to " + player.breastCup(temp3) + "s.", false);
                            }
                        }
        ...
        if (changes < changeLimit && Utils.rand(5) == 0 && player.eyeType > EYES.HUMAN) {
            if (player.eyeType == EYES.BLACK_EYES_SAND_TRAP) {
                ...
                if (player.eyeType == EYES.FOUR_SPIDER_EYES) outputText("  Your multiple, arachnid eyes are gone!</b>", false);
        ...
        if (Utils.rand(3) == 0) outputText(player.modTone(60, 1), false);
        //FAILSAFE CHANGE
        if (changes == 0) {
    ...
    public succubiMilk(tainted: boolean, player: Player): void {
        ...
        if (player.stats.cor < 35) outputText("You wonder why in the gods' names you would drink such a thing, but you have to admit, it is the best thing you have ever tasted.", true);
        if (player.stats.cor >= 35 && player.stats.cor < 70) {
            ...
            if (player.gender == 2 || player.gender == 3) {
                ...
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.DRY) outputText("tingly.", false);
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.NORMAL) outputText("wet.", false);
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.WET) outputText("sloppy and wet.", false);
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLICK) outputText("sopping and juicy.", false);
                if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) outputText("dripping wet.", false);
            }
            else if (player.lowerBody.cockSpot.hasCock()) outputText("  You feel a building arousal, but it doesn't affect your cock.", false);
        }
        if (player.stats.cor >= 70) {
            ...
            if (player.gender == 2 || player.gender == 3) {
                ...
                if (player.lowerBody.vaginaSpot.count() > 1) outputText(" quiver in orgasm, ", false);
                if (player.lowerBody.vaginaSpot.count() == 1) outputText(" quivers in orgasm, ", false);
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.DRY) outputText("becoming slightly sticky.", false);
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.NORMAL) outputText("leaving your undergarments sticky.", false);
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.WET) outputText("wet with girlcum.", false);
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLICK) outputText("staining your undergarments with cum.", false);
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.DROOLING) outputText("leaving cunt-juice trickling down your leg.", false);
                if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLAVERING) outputText("spraying your undergarments liberally with slick girl-cum.", false);
                player.orgasm();
            }
            else if (player.gender != 0) {
                if (player.lowerBody.cockSpot.count() == 1) outputText("  You feel a strange sexual pleasure, but your " + multiCockDescript() + " remains unaffected.", false);
        ...
        //Increase pussy wetness or grow one!!
            ...
            if (player.lowerBody.vaginaSpot.count() > 0) {
                ...
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING) {
                    if (player.lowerBody.vaginaSpot.count() == 1) outputText("Your " + vaginaDescript(0) + " gushes fluids down your leg as you spontaneously orgasm.", false);
                ...
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.DROOLING) {
                    if (player.lowerBody.vaginaSpot.count() == 1) outputText("Your pussy feels hot and juicy, aroused and tender.  You cannot resist as your hands dive into your " + vaginaDescript(0) + ".  You quickly orgasm, squirting fluids everywhere.  <b>You are now a squirter</b>.", false);
                    if (player.lowerBody.vaginaSpot.count() > 1) outputText("Your pussies feel hot and juicy, aroused and tender.  You cannot resist plunging your hands inside your " + vaginaDescript(0) + "s.  You quiver around your fingers, squirting copious fluids over yourself and the ground.  The fluids quickly disappear into the dirt.", false);
                    player.orgasm();
                    if (tainted) dynStats("cor", 1);
                }
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLICK) {
                    if (player.lowerBody.vaginaSpot.count() == 1) outputText("You feel a sudden trickle of fluid down your leg.  You smell it and realize it's your pussy-juice.  Your " + vaginaDescript(0) + " now drools lubricant constantly down your leg.", false);
                    if (player.lowerBody.vaginaSpot.count() > 1) outputText("You feel sudden trickles of fluids down your leg.  You smell the stuff and realize it's your pussies-juices.  They seem to drool lubricant constantly down your legs.", false);
                }
                ...
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.NORMAL) {
                    if (player.lowerBody.vaginaSpot.count() == 1) outputText("A feeling of intense arousal passes through you, causing you to masturbate furiously.  You realize afterwards that your " + vaginaDescript(0) + " felt much wetter than normal.", false);
        ...
        if (Utils.rando >= 90) {
            if (player.skinTone == "blue" || player.skinTone == "purple" || player.skinTone == "indigo" || player.skinTone == "shiny black") {
                if (player.lowerBody.vaginaSpot.count() > 0) {
                    ...
                    if (player.lowerBody.vaginaSpot.list[0].clitLength > 3 && !player.perks.has("BigClit")) outputText("  After some time it shrinks, returning to its normal aroused size.  You guess it can't get any bigger.", false);
                    if (player.lowerBody.vaginaSpot.list[0].clitLength > 5 && player.perks.has("BigClit")) outputText("  Eventually it shrinks back down to its normal (but still HUGE) size.  You guess it can't get any bigger.", false);
        ...
        if (tainted) {
            ...
            if (Utils.rand(3) == 0) outputText(player.modTone(15, 2), false);
        }
        else {
            outputText(player.modFem(90, 1), false);
            if (Utils.rand(3) == 0) outputText(player.modTone(20, 2), false);
        }
        player.genderCheck();
    }
    ...
    public caninePepper(type: number, player: Player): void {
        ...
        if (type <= 0 && crit > 1 && player.skinType == SKIN.FUR && player.faceType == FACE.DOG && player.upperBody.head.earType == EARS.DOG && player.lowerBody == LOWER_BODY.DOG && player.tailType == TAIL.DOG && Utils.rand(2) == 0 && player.statusAffects.has("DogWarning")) {
            ...
            if (temp == 0) {
                ...
                if (player.perks.has("MarblesMilk")) outputText("All you know is that there is a scent on the wind, it is time to hunt, and at the end of the day you need to come home for your milk.", false);
        ...
        if (type == 3) {
            ...
            if (player.stats.cor < 50) outputText("  You shake your head, blushing hotly.  Where did that thought come from?", false);
        }
        ...
        if (type == 4) {
            ...
            if (player.lowerBody.cockSpot.count() > 0) {
                ...
                if (player.dogCocks() > 0) {
                    ...
                    outputText("\n\n", false);
                    if (temp3 < .06) outputText("Your " + Appearance.cockNoun(CockType.DOG) + " feels unusually tight in your sheath as your knot grows.", false);
                    if (temp3 >= .06 && temp3 <= .12) outputText("Your " + Appearance.cockNoun(CockType.DOG) + " pops free of your sheath, thickening nicely into a bigger knot.", false);
                    if (temp3 > .12) outputText("Your " + Appearance.cockNoun(CockType.DOG) + " surges free of your sheath, swelling thicker with each passing second.  Your knot bulges out at the base, growing far beyond normal.", false);
        ...
        if (changes < changeLimit && player.dogScore() >= 3 && Utils.rand(4) == 0) {
            ...
            if (player.gender <= 1 || (player.gender == 3 && Utils.rand(2) == 0)) {
                ...
                //break1
                if (player.stats.cor < 33 || !player.lowerBody.cockSpot.hasCock()) outputText("\nYou shake your head to clear the unwanted fantasy from your mind, repulsed by it.", false);
                else {
                    outputText("  Heart pounding, your shaft pops free of its sheath on instinct, as you take off after the new scent.  Caught firmly in the grip of a female's heat, you ignore your master's cry as you disappear into the wild, " + Appearance.cockNoun(CockType.DOG) + " growing harder as you near your quarry.  You burst through a bush, spotting a white-furred female.  She drops, exposing her dripping fem-sex to you, the musky scent of her sex channeling straight through your nose and sliding into your " + Appearance.cockNoun(CockType.DOG) + ".", false);
                    dynStats("lus", 5 + player.stats.lib / 20);
                    //Break 2
    ...
    public impFood(player: Player): void {
        ...
        if (player.lowerBody.cockSpot.count() > 0) {
            ...
            if (Utils.rand(30) == 0 && player.skinTone != "red") {
                if (player.skinType == SKIN.FUR) outputText("\n\nUnderneath your fur, your skin ", false);
    ...
    public succubisDelight(tainted: boolean, player: Player): void {
        ...
        if (player.stats.cor < 33) outputText("  This stuff is gross, why are you drinking it?", false);
        //high corruption
        if (player.stats.cor >= 66) outputText("  You lick your lips, marvelling at how thick and sticky it is.", false);
        //Corruption increase
    ...
    public useMarbleMilk(player: Player): void {
        ...
        if (player.statusAffects.get("Marble").value2 < 30 && player.statusAffects.get("$1").value3 == 0) outputText("You gulp down the bottle's contents; Marble makes some good tasting milk.\n\n", false);
        //[before the player is addicted, Addiction < 50]
        else if (player.statusAffects.get("$1").value3 <= 0) outputText("You gulp down the bottle's contents; Marble makes some really good tasting milk.\n\n", false);
        else if (player.statusAffects.get("$1").value3 > 0) {
            //[player is completely addicted]
            if (player.perks.has("MarblesMilk")) outputText("You gulp down the bottle's contents; it's no substitute for the real thing, but it's a nice pick me up.\n\n", false);
            else {
                //[player is no longer addicted]
                if (player.perks.has("MarbleResistant")) outputText("You gulp down the bottle's contents; you're careful not to get too attached to the taste.\n\n", false);
                //[player is addicted]
    ...
    public laBova(tainted: boolean, enhanced: boolean, player: Player): void {
        ...
        if (player.lowerBody.cockSpot.count() > 0 && Utils.rand(2) == 0 && !flags[FlagEnum.HYPER_HAPPY]) {
            ...
            if (player.lowerBody.cockSpot.list[temp].cockLength < 2) {
                ...
                if (player.lowerBody.cockSpot.count() == 1 && !player.lowerBody.vaginaSpot.hasVagina()) {
                    ...
                    if (player.lowerBody.balls > 0) outputText("  At the same time, your " + ballsDescriptLight() + " fall victim to the same sensation; eagerly swallowed whole by your crotch.", false);
        ...
        if (enhanced) {
            ...
            if (player.upperBody.chest.list[0].nipplesPerBreast == 1) {
                ...
                if (player.upperBody.chest.count() >= 3 && player.upperBody.chest.list[2].nipplesPerBreast == 1) {
                    ...
                    outputText("  <b>You have a total of " + num2Text(player.upperBody.chest.countNipples()) + " nipples.</b>");
                }
            }
        ...
        if (Utils.rand(3) == 0) outputText(player.modFem(79, 3), false);
        if (Utils.rand(3) == 0) outputText(player.modThickness(70, 4), false);
        if (Utils.rand(5) == 0) outputText(player.modTone(10, 5), false);
    }
    ...
    public goblinAle(player: Player): void {
        ...
        if (player.skinTone != "green" && player.skinTone != "grayish-blue" && player.skinTone != "dark green" && player.skinTone != "pale yellow" && changes < changeLimit && Utils.rand(2) == 0) {
            ...
            if (player.skinType == SKIN.FUR) outputText("skin", false);
    ...
    public gooGasmic(player: Player): void {
        ...
        if (Utils.rand(2) == 0) outputText(player.modFem(85, 3), false);
        if (Utils.rand(2) == 0) outputText(player.modThickness(20, 3), false);
        if (Utils.rand(2) == 0) outputText(player.modTone(15, 5), false);
    }
    ...
    public sharkTooth(type: number, player: Player): void {
        ...
        if (player.faceType != FACE.SHARK_TEETH && Utils.rand(3) == 0 && changes < changeLimit) {
            outputText("\n\n", false);
            if (player.faceType > FACE.HUMAN && player.faceType < FACE.SHARK_TEETH) outputText("Your " + player.face() + " explodes with agony, reshaping into a more human-like visage.  ", false);
        ...
        if (((player.skinTone != "rough gray" && player.skinTone != "orange and black striped") || player.skinType != SKIN.PLAIN) && Utils.rand(7) == 0 && changes < changeLimit) {
            outputText("\n\n", false);
            if (player.skinType == SKIN.FUR || player.skinType == SKIN.SCALES) outputText("Your " + player.skinDesc + " falls out, collecting on the floor and exposing your supple skin underneath.  ", false);
            else if (player.skinType == SKIN.GOO) outputText("Your gooey skin solidifies, thickening up as your body starts to solidy into a more normal form. ", false);
            ...
            if (type == 0) {
                ...
                outputText("Your skin begins to tingle and itch, before rapidly shifting to a shiny orange color, marked by Utils.random black stripes. You take a quick look in a nearby pool of water, to see your skin has morphed in appearance and texture to become more like a tigershark!", false);
                player.skinType = SKIN.PLAIN;
                player.skinDesc = "skin";
        ...
        if (player.upperBody.wingType != WING.SHARK_FIN && changes < changeLimit && Utils.rand(3) == 0) {
            outputText("\n\n", false);
            if (player.upperBody.wingType > WING.NONE) outputText("Your wings fold into themselves, merging together with your back.  ", false);
    ...
    public snakeOil(player: Player): void {
        ...
        if (player.stats.spe < 70 && Utils.rand(2) == 0) {
            ...
            if (player.stats.spe < 40) outputText("  Of course, you're nowhere near as fast as that.", false);
            changes++;
        }
        //Removes wings
        ...
        if (changes == 0 && player.faceType == FACE.SNAKE_FANGS && player.lowerBody != LOWER_BODY.NAGA && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.lowerBody.balls > 0 && player.lowerBody.ballSize > 10) outputText("  You're happy not to have to drag those testicles around with you anymore.", false);
        ...
        /*
         ...
         //(if multicock)
         if(player.lowerBody.cockSpot.count() > 1) outputText("one of ", false);
    ...
    public catTransformation(player: Player): void {
        ...
        if (player.stats.lib < 80 && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            if (player.catCocks() > 0) {
                ...
                if (player.stats.cor < 33) outputText("You need to control yourself better.", false);
                else if (player.stats.cor < 66) outputText("You're not sure how you feel about the fantasy.", false);
        ...
        if (Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.goIntoHeat(false)) {
                if (intensified) {
                    ...
                    if (player.skinType == SKIN.FUR) outputText("underneath your fur ", false);
                    outputText("as images and fantasies ", false);
                    if (player.stats.cor < 50) outputText("assault ", false);
        ...
        if (player.lowerBody.cockSpot.count() > 0 && player.catCocks() < player.lowerBody.cockSpot.count() &&
            ...
            if (!player.lowerBody.cockSpot.hasSheath()) {
                ...
                if (player.lowerBody.balls > 0) outputText("  Thankfully, your balls appear untouched.", false);
            }
        ...
        if (player.catCocks() > 0 && Utils.rand(3) == 0 && changes < changeLimit && !flags[FlagEnum.HYPER_HAPPY]) {
            ...
            if (temp == 1) {
                ...
                if (temp2 == 1) outputText("  Upon further inspection, all your " + Appearance.cockNoun(CockType.CAT) + "s have shrunk!", false);
                changes++;
            }
        }
        ...
        if (player.tailType == TAIL.CAT && player.upperBody.head.earType == EARS.CAT &&
            ...
            if (player.lowerBody == LOWER_BODY.HOOFED || player.lowerBody == LOWER_BODY.CENTAUR) {
                ...
                if (player.lowerBody == LOWER_BODY.CENTAUR) outputText("  You feel woozy and collapse on your side.  When you wake, you're no longer a centaur and your body has returned to a humanoid shape.", false);
            }
            //Goo to cat
        ...
        if (changes < changeLimit) {
            if (Utils.rand(2) == 0) outputText(player.modThickness(5, 2), false);
            if (Utils.rand(2) == 0) outputText(player.modTone(76, 2), false);
            if (player.gender < 2) if (Utils.rand(2) == 0) outputText(player.modFem(65, 1), false);
    ...
    public reptilum(player: Player): void {
        ...
        if (player.stats.lib < 100 && changes < changeLimit && Utils.rand(3) == 0) {
            ...
            if (player.lowerBody.cockSpot.count() > 0 && (player.gender != 3 || Utils.rand(2) == 0)) {
                ...
                if (player.lowerBody.cockSpot.count() > 1) outputText("each of ", false);
            ...
            //(COOCH)
            else if (player.lowerBody.vaginaSpot.hasVagina()) outputText("puddling in your " + vaginaDescript(0) + ".  An instinctive desire to mate and lay eggs spreads through you, increasing your lust and boosting your sex-drive.", false);
            //(TARDS)
        ...
        if (player.lizardCocks() == 0 && player.lowerBody.cockSpot.count() > 0 && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            outputText("to tip, undulating and convulsing as its color lightens, darkens, and finally settles on a purplish hue.  Your " + Appearance.cockNoun(CockType.HUMAN) + " resolves itself into a bulbous form, with a slightly pointed tip.  The 'bulbs' throughout its shape look like they would provide an interesting ride for your sexual partners, but the perverse, alien pecker ", false);
            if (player.stats.cor < 33) outputText("horrifies you.", false);
            else if (player.stats.cor < 66) outputText("is a little strange for your tastes.", false);
            else {
                outputText("looks like it might be more fun to receive than use on others.  ", false);
                if (player.lowerBody.vaginaSpot.hasVagina()) outputText("Maybe you could find someone else with one to ride?", false);
        ...
        if (player.lowerBody.cockSpot.count() > 1 && player.lizardCocks() > 0 && player.lowerBody.cockSpot.count() > player.lizardCocks() && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.lowerBody.cockSpot.count() == 2) outputText("your other dick", false);
        ...
        if (player.lizardCocks() == player.lowerBody.cockSpot.count() && changes < changeLimit && player.statusAffects.has("Infested")) {
            ...
            if (player.lowerBody.balls > 1) outputText("  The remaining " + num2Text(player.lowerBody.balls - 1) + " slither out the pre-stretched holes with ease, though the last one hangs from your tip for a moment before dropping to the ground.", false);
        ...
        if (player.biggestLactation() >= 1 && changes < changeLimit && Utils.rand(4) == 0) {
            if (player.upperBody.chest.countNipples() == 2) outputText("\n\nBoth of your", false);
            ...
            if (player.upperBody.chest.hasFuckableNipples()) outputText("but sexual fluid ", false);
        ...
        if (player.averageNipplesPerBreast() > 1 && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            if (player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("'breast'.", false);
    ...
    public neonPinkEgg(pregnantChange: boolean, player: Player): void {
        ...
        if (pregnantChange) {
            ...
            if (player.pregnancyType == PregnancyType.BUNNY) {
                ...
                if (player.buttPregnancyType == PregnancyType.BUNNY) outputText("and ", false);
            }
            if (player.buttPregnancyType == PregnancyType.BUNNY) outputText("backdoor ", false);
            if (player.buttPregnancyType == PregnancyType.BUNNY && player.pregnancyType == PregnancyType.BUNNY) outputText("rumble", false);
        ...
        if (changes < changeLimit && Utils.rand(5) == 0) {
            ...
            if (player.stats.lib < 60) {
                ...
                if (!player.lowerBody.cockSpot.hasCock() || (player.gender == 3 && Utils.rand(2) == 0)) {
                    if (player.stats.lib < 30) {
                        ...
                        if (player.stats.cor < 25) outputText("You're repulsed by such shameful thoughts.", false);
                        else if (player.stats.cor < 60) outputText("You worry that this place is really getting to you.", false);
                        else if (player.stats.cor < 90) outputText("You pant a little and wonder where the nearest fertile male is.", false);
                ...
                if (player.lowerBody.cockSpot.hasCock()) {
                    if (player.stats.lib < 30) {
                        ...
                        if (Utils.rand(2) == 0) outputText("female hare until she's immobilized by all her eggs", false);
                        ...
                        outputText(". ", false);
                        if (player.stats.cor < 25) outputText("You're repulsed by such shameful thoughts.", false);
                        else if (player.stats.cor < 50) outputText("You worry that this place is really getting to you.", false);
                        else if (player.stats.cor < 75) outputText("You pant a little and wonder where the nearest fertile female is.", false);
            ...
            //Libido over 60? FUCK YEAH!
                ...
                if (player.skinType > SKIN.PLAIN) outputText(" through your " + player.skinDesc, false);
                outputText(".  ", false);
            ...
            //MEGALIBIDO
                ...
                if (player.stats.cor < 33) outputText("You sigh, trying not to give in completely.", false);
                else if (player.stats.cor < 66) outputText("You pant and groan, not sure how long you'll even want to resist.", false);
                else {
                    outputText("You smile and wonder if you can ", false);
                    if (player.stats.lib < 100) outputText("get your libido even higher.", false);
        ...
        if ((player.lowerBody.balls > 0 || player.lowerBody.cockSpot.hasCock()) && player.cumQ() < 3000 && Utils.rand(3) == 0 && changeLimit > 1) {
            ...
            if (player.lowerBody.balls > 0) {
                ...
                //(medium cum quantity) < 250
                    ...
                    if (player.lowerBody.cockSpot.hasCock()) outputText("  In the process, you brush " + sMultiCockDesc() + " and discover a bead of pre leaking at the tip.", false);
                }
                //(large cum quantity) < 750
                    ...
                    if (player.lowerBody.cockSpot.hasCock()) outputText(SMultiCockDesc() + " leaks and dribbles pre-cum down your " + player.legs() + " as your body's cum production kicks up even higher.", false);
                ...
                //(XL cum quantity) < 2000
                    ...
                    if (player.lowerBody.cockSpot.hasCock()) outputText("A thick trail of slime leaks from " + sMultiCockDesc() + " down your " + player.leg() + ", pooling below you.", false);
                ...
                //(XXL cum quantity)
                    ...
                    if (player.lowerBody.cockSpot.hasCock()) outputText("pre-cum explodes from " + sMultiCockDesc() + ", running down your " + player.leg() + " and splattering into puddles that would shame the orgasms of lesser " + player.mf("males", "persons") + ".  You rub yourself a few times, nearly starting to masturbate on the spot, but you control yourself and refrain for now.", false);
        ...
        if (player.lowerBody != LOWER_BODY.BUNNY && changes < changeLimit && Utils.rand(5) == 0 && player.upperBody.head.earType == EARS.BUNNY) {
            ...
            //Non-taurs
                ...
                if (player.tailType > TAIL.NONE) outputText(", nearly smashing your tail flat", false);
    ...
    public goldenSeed(type: number, player: Player): void {
        ...
        if ((player.stats.lib < 90 || Utils.rand(3) == 0) && Utils.rand(3) == 0 && changes < changeLimit) {
            ...
            if (player.stats.lib < 40) {
                ...
                if (player.lowerBody.vaginaSpot.hasVagina()) outputText(" The moistness of your " + vaginaDescript() + " seems to agree.", false);
                else if (player.lowerBody.cockSpot.hasCock()) outputText(" The hardness of " + sMultiCockDesc() + " seems to agree.", false);
                dynStats("lus", 5);
            }
            //(sub 75 lib)
            else if (player.stats.lib < 75) outputText("\n\nHeat, blessed heat, works through you from head to groin, leaving you to shudder and fantasize about the sex you could be having right now.\n\n", false);
            //(hi lib)
            else if (player.stats.lib < 90) outputText("\n\nSexual need courses through you, flushing your skin with a reddish hue while you pant and daydream of the wondrous sex you should be having right now.\n\n", false);
            //(90+)
            ...
            if (player.stats.lib >= 40) {
                ...
                if (player.lowerBody.cockSpot.hasCock() && (player.gender != 3 || Utils.rand(2) == 0)) {
                    ...
                    if (Utils.rand(2) == 0) {
                        ...
                        if (player.lowerBody.cockSpot.count() > 1) {
                            ...
                            if (player.lowerBody.cockSpot.count() > 2) outputText("es rub ", false);
                ...
                //Cunts!
                    ...
                    if (Utils.rand(2) == 0) {
                        ...
                        if (player.upperBody.chest.hasFuckableNipples()) outputText("and pussy leaking over ", false);
        ...
        if (changes < changeLimit && player.upperBody.chest.count() > 1 && Utils.rand(3) == 0 && !flags[FlagEnum.HYPER_HAPPY]) {
            changes++;
            outputText("\n\nYou stumble back when your center of balance shifts, and though you adjust before you can fall over, you're left to watch in awe as your bottom-most " + breastDescript(player.upperBody.chest.count() - 1) + " shrink down, disappearing completely into your ", false);
            if (player.upperBody.chest.count() >= 3) outputText("abdomen", false);
            else outputText("chest", false);
            outputText(". The " + nippleDescript(player.upperBody.chest.count() - 1) + "s even fade until nothing but ", false);
            if (player.skinType == SKIN.FUR) outputText(player.hairColor + " " + player.skinDesc, false);
            else outputText(player.skinTone + " " + player.skinDesc, false);
        ...
        if (changes < changeLimit && player.upperBody.chest.list[0].breastRating < 2 && Utils.rand(3) == 0) {
            ...
            if (player.upperBody.chest.list[0].breastRating < 1) outputText("  <b>You have breasts now!</b>", false);
            player.upperBody.chest.list[0].breastRating = 2;
        }
        //****************
        //General Appearance:
        ...
        if (player.femininity < 85 && changes < changeLimit && Utils.rand(3) == 0) {
            changes++;
            outputText(player.modFem(85, 3 + Utils.rand(5)), false);
        }
        ...
        if ((player.skinTone != "tan" && player.skinTone != "olive" && player.skinTone != "dark" && player.skinTone != "light") && changes < changeLimit && Utils.rand(5) == 0) {
            ...
            if (player.skinType == SKIN.FUR) outputText("the skin under your " + player.hairColor + " " + player.skinDesc, false);
        ...
        if (player.thickness > 25 && changes < changeLimit && Utils.rand(3) == 0) {
            outputText(player.modThickness(25, 3 + Utils.rand(4)), false);
            changes++;
        }
        //Remove odd eyes
        if (changes < changeLimit && Utils.rand(5) == 0 && player.eyeType > EYES.HUMAN) {
    ...
    public kangaFruit(type: number, player: Player): void {
        ...
        if (player.lowerBody.cockSpot.hasCock()) {
            ...
            if (player.kangaCocks() < player.lowerBody.cockSpot.count() && (type == 1 && Utils.rand(2) == 0) && changes < changeLimit) {
                ...
                if (player.lowerBody.cockSpot.hasSheath()) outputText("your sheath", false);
        ...
        if (player.tailType != TAIL.KANGAROO && changes < changeLimit && Utils.rand(4) == 0 && (type != 1 || player.upperBody.head.earType == EARS.KANGAROO)) {
            ...
            //gain roo tail from bee tail:
                ...
                if (player.tailType == TAIL.BEE_ABDOMEN) outputText("hollow stinger drops out of the end, taking the venom organ with it.", false);
    ...
    public sweetGossamer(type: number, player: Player): void {
        ...
        if (player.lowerBody.cockSpot.hasCock() && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            if (temp == 1) {
                ...
                if (player.lowerBody.cockSpot.count() == 1) outputText("It's", false);
        ...
        if (player.skinType != SKIN.PLAIN && (player.upperBody.head.earType == EARS.HUMAN || player.upperBody.head.earType == EARS.ELFIN) && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.skinType == SKIN.SCALES) outputText("are", false);
        ...
        if (player.armType != ARM.SPIDER && changes < changeLimit && Utils.rand(4) == 0) {
            outputText("\n\n", false);
            //(Bird pretext)
            if (player.armType == ARM.HARPY) outputText("The feathers covering your arms fall away, leaving them to return to a far more human appearance.  ", false);
    ...
    /*
            public  applyLustStick(player:Player):void
            {
                ...
                if (player.statusAffects.has("LustStickApplied")) {
                    ...
                    if (player.lowerBody.cockSpot.hasCock()) outputText("It tingles a little, but the drugs have little to no effect on you now.", false);
    ...
    */
    ...
    public broBrew(player: Player): void {
        ...
        if (player.perks.has("BimboBody")) {
            ...
            outputText("\n\n", false);
            if (player.perks.has("BimboBrains")) outputText("<b>(Lost Perks - Bimbo Brains, Bimbo Body)\n", false);
        ...
        if (player.stats.int > 50) outputText(", though you're a bit worried by how much you enjoyed the simple, brutish act", false);
        outputText(".\n\n", false);
        ...
        //(No dick)
            ...
            if (player.lowerBody.balls == 0) outputText("  Two rounded orbs drop down below, filling out a new, fleshy sac above your " + player.legs() + ".  Sweet!  You can probably cum buckets with balls like these.", false);
            outputText("\n\n", false);
            player.lowerBody.cockSpot.add(new Cock());
    ...
    public numbRocks(player: Player): void {
        ...
        if (Utils.rand(5) == 0) {
            if (player.findStatusAffect(StatusAffects.Dys) < 0) {
                ...
                if (player.lowerBody.vaginaSpot.hasVagina()) {
                    if (!player.lowerBody.cockSpot.hasCock()) outputText("your ");
                    outputText(vaginaDescript(0) + " ", false);
                }
                if (!(player.lowerBody.cockSpot.hasCock() || player.lowerBody.vaginaSpot.hasVagina())) outputText(assholeDescript() + " ", false);
        ...
        if (!player.perks.has("ThickSkin") && player.stats.sens < 30 && Utils.rand(4) == 0) {
            ...
            if (player.skinType == SKIN.PLAIN) outputText("your skin", false);
    ...
    public sensitivityDraft(player: Player): void {
        ...
        if (Utils.rand(4) == 0 && player.findStatusAffect(StatusAffects.LustyTongue) < 0) {
            ...
            if (player.lowerBody.vaginaSpot.hasVagina()) outputText("your", false);
    ...
    public foxTF(enhanced: boolean, player: Player): void {
        ...
        if (player.faceType == FACE.FOX && player.tailType == TAIL.FOX && player.upperBody.head.earType == EARS.FOX && player.lowerBody == LOWER_BODY.FOX && player.skinType == SKIN.FUR && Utils.rand(3) == 0) {
            if (flags[FlagEnum.FOX_BAD_END_WARNING] == 0) {
                ...
                if (player.lowerBody.tailVenom > 1) outputText("  Your tails thrash around violently as they begin to fuse painfully back into one, the fur bristling back out with a flourish.");
        ...
        if (changes < changeLimit && enhanced && player.bRows() < 4 && player.upperBody.chest.list[player.bRows() - 1].breastRating > 1) {
            ...
            if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 8) outputText("it's difficult to see past the wall of tits obscuring your view.");
            ...
            if (player.upperBody.chest.list[player.bRows() - 1].nipplesPerBreast > 1) outputText(", not to mention number");
            else if (player.upperBody.chest.hasFuckableNipples()) outputText(", not to mention penetrability");
        ...
        if ((enhanced || player.upperBody.head.earType == EARS.FOX) && player.lowerBody != LOWER_BODY.FOX && changes < changeLimit && Utils.rand(5) == 0) {
            ...
            //reg legs, not digitigrade
                ...
                if (player.lowerBody == LOWER_BODY.BEE) outputText(", while a hideous cracking fills the air");
    ...
    public foxJewel(mystic: boolean, player: Player): void {
        ...
        if (player.skinType == SKIN.FUR || player.skinType == SKIN.SCALES && ((mystic) || (!mystic && Utils.rand(2) == 0))) {
            ...
            if (player.skinType == SKIN.FUR) outputText("  You stare in horror as you pull your fingers away holding a handful of " + player.hairColor + " fur!  Your fur sloughs off your body in thick clumps, falling away to reveal patches of bare, " + player.skinTone + " skin.");
            else if (player.skinType == SKIN.SCALES) outputText("  You stare in horror as you pull your fingers away holding a handful of dried up scales!  Your scales continue to flake and peel off your skin in thick patches, revealing the tender " + player.skinTone + " skin underneath.");
    ...
    */
    ...
    public trapOil(player: Player): void {
        ...
        if (player.stats.cor < 30) outputText("hesitantly ");
        else if (player.stats.cor > 70) outputText("eagerly ");
        ...
        if (player.longestCockLength() >= 3.5 && player.lowerBody.cockSpot.hasCock() && Utils.rand(2) == 0 && changes < changeLimit) {
            ...
            if (player.lowerBody.cockSpot.count() == 1) outputText("it is");
            ...
            if (player.lowerBody.cockSpot.count() == 1) outputText("it seems");
        ...
        if (player.lowerBody.vaginaSpot.hasVagina() && player.vaginaType() != 5 && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            //(Corruption <50:
            if (player.stats.cor < 50) outputText("  After a few cautious touches you decide it doesn't feel any different- it does certainly look odd, though.");
    ...
    public ringtailFig(player: Player): void {
        ...
        if (player.upperBody.head.earType == EARS.RACCOON && player.lowerBody != LOWER_BODY.RACCOON && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            //from hooves or hard feet, including centaurs and bees
                ...
                if (player.lowerBody == LOWER_BODY.HOOFED || player.lowerBody == LOWER_BODY.CENTAUR || player.lowerBody == LOWER_BODY.PONY) outputText("pops right out of its hoof just in time");
        ...
        if (player.faceType != FACE.RACCOON_MASK && player.faceType != FACE.RACCOON && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            //from snout (will not overwrite full-coon snout but will overwrite others)
                ...
                if (((player.hairColor == "black" || player.hairColor == "midnight") && (player.skinType == SKIN.FUR || player.skinType == SKIN.SCALES))) outputText("your face apparently returned to normal shape, albeit still covered in " + player.skinFurScales() + ".  You look closer and discover a darker, mask-line outline on your already inky visage.  <b>You now have a barely-visible raccoon mask on your otherwise normal human face.</b>");
                else if ((player.skinTone == "ebony" || player.skinTone == "black") && (player.skinType == SKIN.PLAIN || player.skinType == SKIN.GOO)) outputText("your face apparently returned to normal shape.  You look closer and discover a darker, mask-line outline on your already inky visage.  <b>You now have a barely-visible raccoon mask on your normal human face.</b>");
    ...
    public mouseCocoa(player: Player): void {
        ...
        if (player.tallness < 60 && (player.analCapacity() < 100 || (player.vaginalCapacity() < 100 && player.lowerBody.vaginaSpot.hasVagina())) && changes < changeLimit && Utils.rand(3) == 0) {
            outputText("\n\nYour ");
            if (player.vaginalCapacity() < 100 && player.lowerBody.vaginaSpot.hasVagina()) outputText("[vagina]");
        ...
        if (player.upperBody.head.earType != EARS.MOUSE && changes < changeLimit && Utils.rand(4) == 0) {
            outputText("\n\nYour ears ");
            if (player.upperBody.head.earType == EARS.HORSE || player.upperBody.head.earType == EARS.COW || player.upperBody.head.earType == EARS.DOG || player.upperBody.head.earType == EARS.BUNNY || player.upperBody.head.earType == EARS.KANGAROO) outputText("shrink suddenly");
        ...
        if ((player.skinType != SKIN.FUR || (player.skinType == SKIN.FUR && (player.hairColor != "brown" && player.hairColor != "white"))) && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            if (player.skinType != SKIN.FUR) {
                ...
                if (player.skinType > SKIN.PLAIN) outputText("es");
                outputText(" all over");
                ...
                if (player.skinType == SKIN.SCALES) outputText(", pushing your scales out with little pinches");
    ...
    private demonChanges(player: Player): void {
        ...
        if (player.tailType != TAIL.DEMONIC && player.horns > 0) {
            if (player.tailType != TAIL.NONE) {
                outputText("\n\n", false);
                if (player.tailType == TAIL.SPIDER_ADBOMEN || player.tailType == TAIL.BEE_ABDOMEN) outputText("You feel a tingling in your insectile abdomen as it stretches, narrowing, the exoskeleton flaking off as it transforms into a flexible demon-tail, complete with a round spaded tip.  ", false);
        ...
        if ((player.faceType != FACE.HUMAN || player.skinType != SKIN.PLAIN) && Utils.rand(3) == 0) {
            ...
            //De-fur
                ...
                outputText("\n\n", false);
                if (player.skinType == SKIN.FUR) outputText("Your skin suddenly feels itchy as your fur begins falling out in clumps, <b>revealing inhumanly smooth skin</b> underneath.", false);
                if (player.skinType == SKIN.SCALES) outputText("Your scales begin to itch as they begin falling out in droves, <b>revealing your inhumanly smooth " + player.skinTone + " skin</b> underneath.", false);
                player.skinType = SKIN.PLAIN;
                player.skinDesc = "skin";
            }
        }
