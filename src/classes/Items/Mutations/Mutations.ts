...
export default class Mutations {
    ...
    public ceruleanPotion(player: Player): void {
        ...
        if (player.gender == Gender.NONE && Flags.get(FlagEnum.CERULEAN_POTION_NEUTER_ATTEMPTED) > 0) {
            Render.text("You take another sip of the Cerulean Potion.  You find it soothing and become very excited about the possibility of another visit from the succubus.", true);
        }
        else if (player.gender == Gender.HERM && Flags.get(FlagEnum.CERULEAN_POTION_HERM_USED) > 0) {
            Render.text("With anticipation, you chug down another bottle of the Cerulean Potion. A warm sensation radiates out from your stomach as you feel the potion course through your body.", true);
        }
        //All else
        else {
            Render.text("The liquid tastes rather bland and goes down easily. ", true);
            //Special repeat texts
            if (player.statusAffects.has("RepeatSuccubi"))
                Render.text("You look forwards to tonight's encounter.", false);
            //First timer huh?
            else Render.text("You do not notice any real effects.  Did the merchant con you?", false);
        }
    ...
    public vitalityTincture(player: Player): void {
        player.slimeFeed();
        Render.text("You down the contents of the bottle. The liquid is thick and tastes remarkably like cherries. Within moments, you feel much more fit and healthy.", true);
        //str change
    ...
    public incubiDraft(tainted: boolean, player: Player): void {
        ...
        if (player.perks.has("HistoryAlchemist"))
            rando += 10;
        Render.text("The draft is slick and sticky, ", true);
        if (player.stats.cor <= 33)
            Render.text("just swallowing it makes you feel unclean.", false);
        if (player.stats.cor > 33 && player.stats.cor <= 66)
            Render.text("reminding you of something you just can't place.", false);
        if (player.stats.cor > 66)
            Render.text("deliciously sinful in all the right ways.", false);
        if (player.stats.cor >= 90)
            Render.text("  You're sure it must be distilled from the cum of an incubus.", false);
        //Lowlevel changes
        if (rando < 50) {
            if (player.lowerBody.cockSpot.count() == 1) {
                if (player.lowerBody.cockSpot.list[0].cockType != CockType.DEMON) Render.text("\n\nYour " + cockDescript(0) + " becomes shockingly hard.  It turns a shiny inhuman purple and spasms, dribbling hot demon-like cum as it begins to grow.", false);
                ...
                if (temp < .5) Render.text("  It stops almost as soon as it starts, growing only a tiny bit longer.", false);
                if (temp >= .5 && temp < 1) Render.text("  It grows slowly, stopping after roughly half an inch of growth.", false);
                if (temp >= 1 && temp <= 2) Render.text("  The sensation is incredible as more than an inch of lengthened dick-flesh grows in.", false);
                if (temp > 2) Render.text("  You smile and idly stroke your lengthening " + cockDescript(0) + " as a few more inches sprout.", false);
                if (tainted) dynStats("int", 1, "lib", 2, "sen", 1, "lus", 5 + temp * 3, "cor", 1);
                else dynStats("int", 1, "lib", 2, "sen", 1, "lus", 5 + temp * 3);
                if (player.lowerBody.cockSpot.list[0].cockType != CockType.DEMON) Render.text("  With the transformation complete, your " + cockDescript(0) + " returns to its normal coloration.", false);
                else Render.text("  With the transformation complete, your " + cockDescript(0) + " throbs in an almost happy way as it goes flaccid once more.", false);
            }
            if (player.lowerBody.cockSpot.count() > 1) {
                ...
                if (temp3 < .5) Render.text("  It stops almost as soon as it starts, growing only a tiny bit longer.", false);
                if (temp3 >= .5 && temp3 < 1) Render.text("  It grows slowly, stopping after roughly half an inch of growth.", false);
                if (temp3 >= 1 && temp3 <= 2) Render.text("  The sensation is incredible as more than an inch of lengthened dick-flesh grows in.", false);
                if (temp3 > 2) Render.text("  You smile and idly stroke your lengthening " + cockDescript(temp2) + " as a few more inches sprout.", false);
                Render.text("  With the transformation complete, your " + multiCockDescriptLight() + " return to their normal coloration.", false);
            }
            //NO CAWKS?
            if (player.lowerBody.cockSpot.count() == 0) {
                ...
                Render.text("The skin seems to fold back as a fully formed demon-cock bursts forth from your loins, drizzling hot cum everywhere as it orgasms.  Eventually the orgasm ends as your " + cockDescript(0) + " fades to a more normal " + player.skinTone + " tone.", false);
        ...
        if (Utils.rando >= 50 && Utils.rando < 93) {
            ...
            if (player.lowerBody.cockSpot.count() == 1) {
                ...
                //Display the degree of thickness change.
                if (temp3 >= 1) {
                    if (player.lowerBody.cockSpot.count() == 1) Render.text("  Your cock spreads rapidly, swelling an inch or more in girth, making it feel fat and floppy.", false);
                    else Render.text("  Your cocks spread rapidly, swelling as they grow an inch or more in girth, making them feel fat and floppy.", false);
                }
                if (temp3 <= .5) {
                    if (player.lowerBody.cockSpot.count() > 1) Render.text("  Your cocks feel swollen and heavy. With a firm, but gentle, squeeze, you confirm your suspicions. They are definitely thicker.", false);
                    else Render.text("  Your cock feels swollen and heavy. With a firm, but gentle, squeeze, you confirm your suspicions. It is definitely thicker.", false);
                }
                if (temp3 > .5 && temp2 < 1) {
                    if (player.lowerBody.cockSpot.count() == 1) Render.text("  Your cock seems to swell up, feeling heavier. You look down and watch it growing fatter as it thickens.", false);
                    if (player.lowerBody.cockSpot.count() > 1) Render.text("  Your cocks seem to swell up, feeling heavier. You look down and watch them growing fatter as they thicken.", false);
                }
                if (tainted) dynStats("lib", 3, "sen", 5, "lus", 10, "cor", 3);
                else dynStats("lib", 3, "sen", 5, "lus", 10);
            }
        ...
        player.genderCheck();
        if (Utils.rand(4) == 0 && tainted) Render.text(player.modFem(5, 2), false);
        if (Utils.rand(4) == 0 && tainted) Render.text(player.modThickness(30, 2), false);
    }
    ...
    public growDemonCock(growCocks: number): void {
        ...
        Render.text("\n\nYou shudder as a pressure builds in your crotch, peaking painfully as a large bulge begins to push out from your body.  ", false);
        if (temp == 1) {
            Render.text("The skin seems to fold back as a fully formed demon-cock bursts forth from your loins, drizzling hot cum everywhere as it orgasms.  In time it fades to a more normal coloration and human-like texture.  ", false);
        }
        else {
            Render.text("The skin bulges obscenely, darkening and splitting around " + num2Text(temp) + " of your new dicks.  For an instant they turn a demonic purple and dribble in thick spasms of scalding demon-cum.  After, they return to a more humanoid coloration.  ", false);
        }
        if (temp > 4) Render.text("Your tender bundle of new cocks feels deliciously sensitive, and you cannot stop yourself from wrapping your hands around the slick demonic bundle and pleasuring them.\n\nNearly an hour later, you finally pull your slick body away from the puddle you left on the ground.  When you look back, you notice it has already been devoured by the hungry earth.", false);
        player.orgasm();
    }

    public tatteredScroll(player: Player): void {
        Render.text("Your wobbly " + player.legs() + " give out underneath you as your body's willpower seems to evaporate, your mouth reading the words on the scroll with a backwards sounding sing-song voice.\n\n", true);
        if (player.hairColor == "sandy blonde") {
            Render.text("Your mouth forms a smile of its own volition, reading, \"<i>Tresed eht retaw llahs klim ruoy.</i>\"\n\n", false);
            ...
            if (player.biggestLactation() > 0) {
                ...
                if (player.biggestLactation() < 2) Render.text("powerful jets of milk spray from your nipples, spraying thick streams over the ground.  You moan at the sensation and squeeze your tits, hosing down the tainted earth with an offering of your milk.  You blush as the milk ends, quite embarassed with your increased milk production.  ", false);
            ...
            if (player.biggestLactation() == 0) {
                Render.text("A pleasurable release suddenly erupts from your nipples!  Twin streams of milk are spraying from your breasts, soaking into the ground immediately.  It stops all too soon, though a voice in your head assures you that you can lactate quite often now.  ", false);
            ...
            Render.text("Your mouth forms a smile of its own volition, reading, \"<i>nuf erutuf rof riah ydnas, nus tresed eht sa ydnas.</i>\"\n\nYou feel a tingling in your scalp, and realize your hair has become a sandy blonde!", false);
    ...
    public minotaurCum(player: Player): void {
        ...
        Render.text("", true);
        Render.text("As soon as you crack the seal on the bottled white fluid, a ", false);
        if (flags[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] == 0) Render.text("potent musk washes over you.", false);
        else Render.text("heavenly scent fills your nostrils.", false);
        if (flags[FlagEnum.MINOTAUR_CUM_ADDICTION_TRACKER] < 50) Render.text("  It makes you feel dizzy, ditzy, and placid.", false);
        else Render.text("  It makes you feel euphoric, happy, and willing to do ANYTHING to keep feeling this way.", false);
        Render.text("  Unbidden, your hand brings the bottle to your lips, and the heady taste fills your mouth as you convulsively swallow the entire bottle.", false);
        //-Raises lust by 10.
        //-Raises sensitivity
        ...
        if (player.lowerBody.cockSpot.count() > 0) {
            ...
            Render.text("our " + multiCockDescriptLight() + " aches, flooding with blood until it's bloating and trembling.", false);
        }
        if (player.lowerBody.vaginaSpot.hasVagina()) {
            Render.text("  Your " + clitDescript() + " engorges, ", false);
            if (player.lowerBody.vaginaSpot.list[0].clitLength < 3) Render.text("parting your lips.", false);
            else Render.text("bursting free of your lips and bobbing under its own weight.", false);
            if (player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.NORMAL) Render.text("  Wetness builds inside you as your " + vaginaDescript(0) + " tingles and aches to be filled.", false);
            else if (player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.SLICK) Render.text("  A trickle of wetness escapes your " + vaginaDescript(0) + " as your body reacts to the desire burning inside you.", false);
            else if (player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.DROOLING) Render.text("  Wet fluids leak down your thighs as your body reacts to this new stimulus.", false);
            else Render.text("  Slick fluids soak your thighs as your body reacts to this new stimulus.", false);
        }
        //(Minotaur fantasy)
    ...
    public minotaurBlood(player: Player): void {
        ...
        let temp3: number = 0;
        //Set up output
        Render.text("You drink the bubbling red fluid, tasting the tangy iron after-taste.", true);
        //STATS
        //Strength h
        ...
        if (player.lowerBody != LOWER_BODY.HOOFED && player.lowerBody != LOWER_BODY.CENTAUR) {
            if (changes < changeLimit && Utils.rand(3) == 0) {
                ...
                if (player.skinType != SKIN.FUR) Render.text("  A fine coat of fur grows out below your waist, itching briefly as it fills in.");
                Render.text("<b>  You now have hooves in place of your feet!</b>", false);
        ...
        if (!flags[FlagEnum.HYPER_HAPPY]) {
            ...
            if (player.lowerBody.vaginaSpot.count() > 0) {
                if (player.vaginas[0].vaginalLooseness > VAGINA_LOOSENESS.TIGHT) {
                    ...
                    if (player.lowerBody.cockSpot.count() == 0) {
                        Render.text("  Strangely, your clit seems to have resisted the change, and is growing larger by the moment... shifting into the shape of a small ribbed minotaur-like penis!  <b>You now have a horse-cock!</b>", false);
                        player.lowerBody.cockSpot.add(new Cock());
            ...
            if (changes < changeLimit && player.bRows() > 1 && Utils.rand(3) == 0) {
                ...
                if (player.bRows() >= 3) Render.text("abdomen", false);
                else Render.text("chest", false);
                Render.text(". The " + nippleDescript(player.upperBody.chest.count() - 1) + "s even fade until nothing but ", false);
                if (player.skinType == SKIN.FUR) Render.text(player.hairColor + " " + player.skinDesc, false);
                else Render.text(player.skinTone + " " + player.skinDesc, false);
                Render.text(" remains. <b>You've lost a row of breasts!</b>", false);
                dynStats("sen", -5);
                ...
                //multiple
                    ...
                    while (temp3 < player.upperBody.chest.count()) {
                        if (player.upperBody.chest.list[temp3].breastRating >= 1) {
                            ...
                            if (temp2 > 1) Render.text("...and y", false);
                            else Render.text("Y", false);
                            Render.text("our " + breastDescript(temp3) + " shrink, dropping to " + player.breastCup(temp3) + "s.", false);
                        }
                        temp3++;
                    }
        ...
        if (player.lowerBody.cockSpot.count() > 0 && Utils.rand(2) == 0 && changes < changeLimit) {
            ...
            if (selectedCockValue != -1) {
                ...
                Render.text("<b>  You now have a");
                if (player.horseCocks() > 1) Render.text("nother")
                Render.text(" horse-penis.</b>", false);
                changes++;
            }
        }
                ...
                if (player.hornType == HORNS.COW_MINOTAUR) {
                    ...
                    if (player.lowerBody.vaginaSpot.count() > 0) {
                        if (player.horns > 4) {
                            ...
                            Render.text("Your headache clears as lust washes through you unnaturally.  You feel as if you haven't cum in months.", false);
                    ...
                    //Males horns get 'uge.
                        ...
                        if (player.horns < 3) Render.text("They are the size of tiny nubs.", false);
                        if (player.horns >= 3 && player.horns < 6) Render.text("They are similar to what you would see on a young bull.", false);
                        if (player.horns >= 6 && player.horns < 12) Render.text("They look like the horns on a grown bull, big enough and dangerous enough to do some damage.", false);
                        if (player.horns >= 12 && player.horns < 20) Render.text("They are large and wicked looking.", false);
                        if (player.horns >= 20) Render.text("They are huge, heavy, and tipped with dangerous points.", false);
        ...
        if (Utils.rand(4) == 0) Render.text(player.modFem(5, 10), false);
        if (Utils.rand(4) == 0) Render.text(player.modTone(85, 3), false);
        if (Utils.rand(4) == 0) Render.text(player.modThickness(70, 4), false);
        //Default
        if (changes == 0) {
            Render.text("\n\nMinotaur-like vitality surges through your body, invigorating and arousing you!\n", false);
            if (player.lowerBody.balls > 0) {
                Render.text("Your balls feel as if they've grown heavier with the weight of more sperm.\n", false);
                player.hoursSinceCum += 200;
            }
            HPChange(50, true);
            dynStats("lus", 50);
    ...
    public equinum(player: Player): void {
        ...
        //Set up output
        Render.text("You down the potion, grimacing at the strong taste.", true);
                ...
                if (player.statusAffects.get("HorseWarning").value1 > 0) {
                    ...
                    if (player.lowerBody.cockSpot.count() > 0) {
                        ...
                        if (player.horseCocks() > 0) {
                            ...
                            if (player.gender == 0 || player.gender == 1) Render.text("He is clearly a male, but you are somewhat confused as you can see not one but three bulges where his manhood would be.\n\n", false);
                            if (player.gender == 2) Render.text("She is clearly a female, as you can see her six breasts jiggle as she walks towards you, small stains appearing on her shirt where her nipples are.\n\n", false);
                            if (player.gender == 3) Render.text("You are somewhat confused as you can see a bulge near her thighs but also huge boobs jiggling as she walks, and you can't say if she's a male or female.\n\n", false);
                            Render.text("As soon as you lay eyes on the creature, a wave of nostalgia overtakes you. Somehow, looking at that creature makes you sad, as if you forgot something important.\n\n\"<i>How strange to see a horse here all alone,</i>\" the creature muses, \"<i>In any case, you're still the least bizarre creature I've met here.  Not to mention the only one that hasn't tried to rape me,</i>\" it says with a sigh.\n\nYou answer with an interrogative whinny.\n\n\"<i>Hey, I've got an idea. I'll take you back to the camp. I'll feed you and in return you can help me complete my quest. What do you say?</i>\"\n\nInstinctively, you utter a happy and approving whinny.\n\nYou failed in your quest, losing your focus and more importantly, losing yourself.  But, even so, you found a new meaning to your life, and have a new chance to succeed where you once failed.", false);
                            getGame().gameOver();
                            return;
                        }
                    }
        ...
        if (Utils.rand(2) == 0) {
            ...
            if (player.tou >= 75) {
                ...
                if (player.gender == 1 || player.gender == 3) Render.text("stallion's.", false);
                else Render.text("mare's.", false);
            }
            //NOT MAXXED
            else {
                dynStats("tou", 1.25);
        ...
        if ((player.gender == 1 || player.gender == 3) && Utils.rand(1.5) == 0 && changes < changeLimit) {
            ...
            if ((player.horseCocks() + player.demonCocks()) < player.lowerBody.cockSpot.count()) {
                ...
                if (player.lowerBody.cockSpot.count() == 1) {
                    ...
                    if (temp3 == 1) Render.text("  <b>Your penis has transformed into a horse's!</b>", false);
                }
                //MULTICOCK
                else {
                    dynStats("lib", 5, "sen", 4, "lus", 35);
                    ...
                    if (temp == -1) {
                        CoC_Settings.error("");
                        Render.text("FUKKKK ERROR NO COCK XFORMED", true);
                    }
                    //Already have a sheath
                    if (player.horseCocks() > 1 || player.dogCocks() > 0) Render.text("  Your sheath tingles and begins growing larger as the cock's base shifts to lie inside it.", false);
                    else Render.text("  You feel a tightness near the base where your skin seems to be bunching up.  A sheath begins forming around your " + cockDescript(temp) + "'s root, tightening and pulling your " + cockDescript(temp) + " inside its depths.", false);
                    temp2 = player.increaseCock(temp, Utils.rand(4) + 4);
                    Render.text("  The shaft suddenly explodes with movement, growing longer and developing a thick flared head leaking steady stream of animal-cum.", false);
                    Render.text("  <b>You now have a horse-cock.</b>", false);
                }
            ...
            //Players cocks are all horse-type - increase size!
                ...
                Render.text("\n\n", false);
                if (temp2 > 2) Render.text("Your " + cockDescript(temp) + " tightens painfully, inches of taut horse-flesh pouring out from your sheath as it grows longer.  Thick animal-pre forms at the flared tip, drawn out from the pleasure of the change.", false);
                if (temp2 > 1 && temp2 <= 2) Render.text("Aching pressure builds within your sheath, suddenly releasing as an inch or more of extra dick flesh spills out.  A dollop of pre beads on the head of your enlarged " + cockDescript(temp) + " from the pleasure of the growth.", false);
                if (temp2 <= 1) Render.text("A slight pressure builds and releases as your " + cockDescript(temp) + " pushes a bit further out of your sheath.", false);
                changes++;
            }
            ...
            if (Utils.rand(2) == 0 && changes < changeLimit && player.horseCocks() > 0) {
                ...
                if (player.stats.cor + player.stats.lib < 50) Render.text(" to have such a splendid tool.  You idly daydream about cunts and pussies, your " + Appearance.cockNoun(CockType.HORSE) + " plowing them relentlessly, stuffing them pregnant with cum", false);
                if (player.stats.cor + player.stats.lib >= 50 && player.stats.cor + player.stats.lib < 80) Render.text(" to be this way... You breath the powerful animalistic scent and fantasize about fucking centaurs night and day until their bellies slosh with your cum", false);
                if (player.stats.cor + player.stats.lib >= 75 && player.stats.cor + player.stats.lib <= 125) Render.text(" to be a rutting stud.  You ache to find a mare or centaur to breed with.  Longing to spend your evenings plunging a " + Appearance.cockNoun(CockType.HORSE) + " deep into their musky passages, dumping load after load of your thick animal-cum into them.  You'd be happy just fucking horsecunts morning, noon, and night.  Maybe somewhere there is a farm needing a breeder..", false);
                if (player.stats.cor + player.stats.lib > 125) Render.text(" to whinny loudly like a rutting stallion.  Your " + Appearance.cockNoun(CockType.HORSE) + " is perfect for fucking centaurs and mares.  You imagine the feel of plowing an equine pussy deeply, bottoming out and unloading sticky jets of horse-jizz into its fertile womb.  Your hand strokes your horsecock of its own accord, musky pre dripping from the flared tip with each stroke.  Your mind wanders to the thought of you with a harem of pregnant centaurs.", false);
                Render.text(".", false);
                if (player.stats.cor < 30) Render.text("  You shudder in revulsion at the strange thoughts and vow to control yourself better.", false);
                if (player.stats.cor >= 30 && player.stats.cor < 60) Render.text("  You wonder why you thought such odd things, but they have a certain appeal.", false);
                if (player.stats.cor >= 60 && player.stats.cor < 90) Render.text("  You relish your twisted fantasies, hoping to dream of them again.", false);
                if (player.stats.cor >= 90) Render.text("  You flush hotly and give a twisted smile, resolving to find a fitting subject to rape and relive your fantasies.", false);
                    ...
                    //multiple
                        ...
                        while (temp3 > 0) {
                            ...
                            if (player.upperBody.chest.list[temp3].breastRating > 3) {
                                ...
                                temp2++;
                                Render.text("\n", false);
                                if (temp3 < player.upperBody.chest.count() - 1) Render.text("...and y", false);
                                else Render.text("Y", false);
                                Render.text("our " + breastDescript(temp3) + " shrink, dropping to " + player.breastCup(temp3) + "s.", false);
                            }
                        }
        ...
        if (changes < changeLimit && Utils.rand(6) == 0 && player.lowerBody == LOWER_BODY.HOOFED) {
            ...
            if (player.gender > 0) {
                Render.text("  After taking a moment to get used to your new body, you notice that your genitals now reside between the back legs on your centaur body.", false);
            }
            dynStats("spe", 3);
        ...
        if (changes < changeLimit && Utils.rand(5) == 0 && player.eyeType > EYES.HUMAN) {
            if (player.eyeType == EYES.BLACK_EYES_SAND_TRAP) {
                ...
                if (player.eyeType == EYES.FOUR_SPIDER_EYES) Render.text("  Your multiple, arachnid eyes are gone!</b>", false);
                Render.text("  <b>You have normal, humanoid eyes again.</b>", false);
            }
        ...
        if (player.tailType != TAIL.HORSE && Utils.rand(2) == 0 && changes < changeLimit) {
            ...
            Render.text("  <b>You now have a horse-tail.</b>", false);
        ...
        if (Utils.rand(3) == 0) Render.text(player.modTone(60, 1), false);
        //FAILSAFE CHANGE
        if (changes == 0) {
    ...
    public succubiMilk(tainted: boolean, player: Player): void {
        ...
        if (player.stats.cor < 35) Render.text("You wonder why in the gods' names you would drink such a thing, but you have to admit, it is the best thing you have ever tasted.", true);
        if (player.stats.cor >= 35 && player.stats.cor < 70) {
            Render.text("You savor the incredible flavor as you greedily gulp it down.", true);
            if (player.gender == 2 || player.gender == 3) {
                Render.text("  The taste alone makes your " + vaginaDescript(0) + " feel ", false);
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.DRY) Render.text("tingly.", false);
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.NORMAL) Render.text("wet.", false);
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.WET) Render.text("sloppy and wet.", false);
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLICK) Render.text("sopping and juicy.", false);
                if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) Render.text("dripping wet.", false);
            }
            else if (player.lowerBody.cockSpot.hasCock()) Render.text("  You feel a building arousal, but it doesn't affect your cock.", false);
        }
        if (player.stats.cor >= 70) {
            Render.text("You pour the milk down your throat, chugging the stuff as fast as you can.  You want more.", true);
            if (player.gender == 2 || player.gender == 3) {
                Render.text("  Your " + vaginaDescript(0), false);
                if (player.lowerBody.vaginaSpot.count() > 1) Render.text(" quiver in orgasm, ", false);
                if (player.lowerBody.vaginaSpot.count() == 1) Render.text(" quivers in orgasm, ", false);
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.DRY) Render.text("becoming slightly sticky.", false);
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.NORMAL) Render.text("leaving your undergarments sticky.", false);
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.WET) Render.text("wet with girlcum.", false);
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLICK) Render.text("staining your undergarments with cum.", false);
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.DROOLING) Render.text("leaving cunt-juice trickling down your leg.", false);
                if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLAVERING) Render.text("spraying your undergarments liberally with slick girl-cum.", false);
                player.orgasm();
            }
            else if (player.gender != 0) {
                if (player.lowerBody.cockSpot.count() == 1) Render.text("  You feel a strange sexual pleasure, but your " + multiCockDescript() + " remains unaffected.", false);
                else Render.text("  You feel a strange sexual pleasure, but your " + multiCockDescript() + " remain unaffected.", false);
            }
        }
        ...
        if (Utils.rando <= 75) {
            ...
            if (player.upperBody.chest.count() == 0) {
                Render.text("A perfect pair of B cup breasts, complete with tiny nipples, form on your chest.", false);
        ...
        //Increase pussy wetness or grow one!!
            ...
            if (player.lowerBody.vaginaSpot.count() > 0) {
                ...
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING) {
                    if (player.lowerBody.vaginaSpot.count() == 1) Render.text("Your " + vaginaDescript(0) + " gushes fluids down your leg as you spontaneously orgasm.", false);
                    else Render.text("Your " + vaginaDescript(0) + "s gush fluids down your legs as you spontaneously orgasm, leaving a thick puddle of pussy-juice on the ground.  It is rapidly absorbed by the earth.", false);
                    player.orgasm();
                    if (tainted) dynStats("cor", 1);
                }
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.DROOLING) {
                    if (player.lowerBody.vaginaSpot.count() == 1) Render.text("Your pussy feels hot and juicy, aroused and tender.  You cannot resist as your hands dive into your " + vaginaDescript(0) + ".  You quickly orgasm, squirting fluids everywhere.  <b>You are now a squirter</b>.", false);
                    if (player.lowerBody.vaginaSpot.count() > 1) Render.text("Your pussies feel hot and juicy, aroused and tender.  You cannot resist plunging your hands inside your " + vaginaDescript(0) + "s.  You quiver around your fingers, squirting copious fluids over yourself and the ground.  The fluids quickly disappear into the dirt.", false);
                    player.orgasm();
                    if (tainted) dynStats("cor", 1);
                }
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLICK) {
                    if (player.lowerBody.vaginaSpot.count() == 1) Render.text("You feel a sudden trickle of fluid down your leg.  You smell it and realize it's your pussy-juice.  Your " + vaginaDescript(0) + " now drools lubricant constantly down your leg.", false);
                    if (player.lowerBody.vaginaSpot.count() > 1) Render.text("You feel sudden trickles of fluids down your leg.  You smell the stuff and realize it's your pussies-juices.  They seem to drool lubricant constantly down your legs.", false);
                }
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.WET) {
                    Render.text("You flush in sexual arousal as you realize how moist your cunt-lips have become.  Once you've calmed down a bit you realize they're still slick and ready to fuck, and always will be.", false);
                }
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.NORMAL) {
                    if (player.lowerBody.vaginaSpot.count() == 1) Render.text("A feeling of intense arousal passes through you, causing you to masturbate furiously.  You realize afterwards that your " + vaginaDescript(0) + " felt much wetter than normal.", false);
                    else Render.text("A feeling of intense arousal passes through you, causing you to masturbate furiously.  You realize afterwards that your " + vaginaDescript(0) + " were much wetter than normal.", false);
                }
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS.DRY) {
                    Render.text("You feel a tingling in your crotch, but cannot identify it.", false);
                }
        ...
        if (Utils.rando >= 90) {
            if (player.skinTone == "blue" || player.skinTone == "purple" || player.skinTone == "indigo" || player.skinTone == "shiny black") {
                if (player.lowerBody.vaginaSpot.count() > 0) {
                    ...
                    if (player.lowerBody.vaginaSpot.list[0].clitLength > 3 && !player.perks.has("BigClit")) Render.text("  After some time it shrinks, returning to its normal aroused size.  You guess it can't get any bigger.", false);
                    if (player.lowerBody.vaginaSpot.list[0].clitLength > 5 && player.perks.has("BigClit")) Render.text("  Eventually it shrinks back down to its normal (but still HUGE) size.  You guess it can't get any bigger.", false);
        ...
        if (Utils.rand(40) + player.stats.cor / 3 > 35 && tainted) demonChanges(player);
        if (tainted) {
            Render.text(player.modFem(100, 2), false);
            if (Utils.rand(3) == 0) Render.text(player.modTone(15, 2), false);
        }
        else {
            Render.text(player.modFem(90, 1), false);
            if (Utils.rand(3) == 0) Render.text(player.modTone(20, 2), false);
        }
        player.genderCheck();
    }
    ...
    public caninePepper(type: number, player: Player): void {
        ...
        if (type == 0) {
            if (Utils.rand(100) < 15) {
                ...
                Render.text("The pepper tastes particularly potent, searingly hot and spicy.", false);
            }
            else Render.text("The pepper is strangely spicy but very tasty.", false);
        }
        //Oversized pepper
        if (type == 1) {
            crit = int(Math.Utils.random() * 20) / 10 + 2;
            Render.text("The pepper is so large and thick that you have to eat it in several large bites.  It is not as spicy as the normal ones, but is delicious and flavorful.", false);
        }
        //Double Pepper
        if (type == 2) {
            crit = int(Math.Utils.random() * 20) / 10 + 2;
            Render.text("The double-pepper is strange, looking like it was formed when two peppers grew together near their bases.", false);
        }
        //Black Pepper
        if (type == 3) {
            crit = int(Math.Utils.random() * 20) / 10 + 2;
            Render.text("This black pepper tastes sweet, but has a bit of a tangy aftertaste.", false);
        }
        //Knotty Pepper
        if (type == 4) {
            crit = int(Math.Utils.random() * 20) / 10 + 2;
            Render.text("The pepper is a bit tough to eat due to the swollen bulge near the base, but you manage to cram it down and munch on it.  It's extra spicy!", false);
        }
        //Bulbous Pepper
        if (type == 5) {
            crit = int(Math.Utils.random() * 20) / 10 + 2;
            Render.text("You eat the pepper, even the two orb-like growths that have grown out from the base.  It's delicious!", false);
        }
        //OVERDOSE Bad End!
        if (type <= 0 && crit > 1 && player.skinType == SKIN.FUR && player.faceType == FACE.DOG && player.upperBody.head.earType == EARS.DOG && player.lowerBody == LOWER_BODY.DOG && player.tailType == TAIL.DOG && Utils.rand(2) == 0 && player.statusAffects.has("DogWarning")) {
            ...
            if (temp == 0) {
                ...
                if (player.perks.has("MarblesMilk")) Render.text("All you know is that there is a scent on the wind, it is time to hunt, and at the end of the day you need to come home for your milk.", false);
                else Render.text("All you know is that there is a scent on the wind, and it is time to hunt.", false);
            }
        ...
        if (type == 3) {
            ...
            if (player.stats.cor < 50) Render.text("  You shake your head, blushing hotly.  Where did that thought come from?", false);
        }
        if (player.str < 50 && Utils.rand(3) == 0) {
            dynStats("str", (crit));
            if (crit > 1) Render.text("\n\nYour muscles ripple and grow, bulging outwards.", false);
        ...
        if (player.stats.int > 30 && Utils.rand(3) == 0 && changes < changeLimit && type != 3) {
            dynStats("int", (-1 * crit));
            Render.text("\n\nYou feel ", false);
            if (crit > 1) Render.text("MUCH ", false);
            Render.text("dumber.", false);
            changes++;
        }
        //-Remove feather-arms (copy this for goblin ale, mino blood, equinum, canine pepps, demon items)
        if (changes < changeLimit && player.armType == ARM.HARPY && Utils.rand(4) == 0) {
        ...
        //if(type != 2 && type != 4 && type != 5) Render.text("\n", false);
        //Double Pepper!
        ...
        if (type == 4) {
            ...
            if (player.lowerBody.cockSpot.count() > 0) {
                ...
                if (player.dogCocks() > 0) {
                    ...
                    Render.text("\n\n", false);
                    if (temp3 < .06) Render.text("Your " + Appearance.cockNoun(CockType.DOG) + " feels unusually tight in your sheath as your knot grows.", false);
                    if (temp3 >= .06 && temp3 <= .12) Render.text("Your " + Appearance.cockNoun(CockType.DOG) + " pops free of your sheath, thickening nicely into a bigger knot.", false);
                    if (temp3 > .12) Render.text("Your " + Appearance.cockNoun(CockType.DOG) + " surges free of your sheath, swelling thicker with each passing second.  Your knot bulges out at the base, growing far beyond normal.", false);
        ...
        if (player.lowerBody.cockSpot.count() > 0) {
            ...
            if (player.cumMultiplier < 2 && Utils.rand(2) == 0 && changes < changeLimit) {
                ...
                if (temp < player.cumMultiplier + .05 * crit) {
                    ...
                    if (crit > 1) Render.text("  A bit of milky pre dribbles from your " + multiCockDescriptLight() + ", pushed out by the change.", false);
                }
                changes++;
            }
            //Oversized pepper
            ...
            if (player.upperBody.chest.list[0].breastRating > 0 && player.lowerBody.vaginaSpot.count() > 0) {
                ...
                if (player.upperBody.chest.count() < 3 && Utils.rand(2) == 0 && changes < changeLimit) {
                    ...
                    if (player.lowerBody.vaginaSpot.count() > 0 && player.upperBody.chest.list[0].breastRating <= player.upperBody.chest.count()) {
                        ...
                        player.upperBody.chest.list[0].breastRating += 2;
                        Render.text(player.breastCup(0) + " size.  But it doesn't stop there, you feel a tightness beginning lower on your torso...", false);
                        changes++;
                    }
                    //Had 1 row to start
                    ...
                    if (crit > 1) {
                        if (crit > 2) {
                            Render.text("  You heft your new chest experimentally, exploring the new flesh with tender touches.  Your eyes nearly roll back in your head from the intense feelings.", false);
                            dynStats("sen", 6, "lus", 15, "cor", 0)
                        }
                        else {
                            Render.text("  You touch your new nipples with a mixture of awe and desire, the experience arousing beyond measure.  You squeal in delight, nearly orgasming, but in time finding the willpower to stop yourself.", false);
                ...
                //If already has max doggie breasts!
                    ...
                    while (temp > 1 && temp2 == 0) {
                        ...
                        if (player.upperBody.chest.list[temp].breastRating + 1 < player.upperBody.chest.list[temp - 1].breastRating) {
                            ...
                            Render.text("\nYour ", false);
                            if (temp == 0) Render.text("first ", false);
                            if (temp == 1) Render.text("second ", false);
                            if (temp == 2) Render.text("third ", false);
                            if (temp == 3) Render.text("fourth ", false);
                            if (temp == 4) Render.text("fifth ", false);
                            if (temp > 4) Render.text("", false);
                            Render.text("row of " + breastDescript(temp) + " grows larger, as if jealous of the jiggling flesh above.", false);
        ...
        //Grow tits if have NO breasts/nipples AT ALL
            ...
            Render.text("  A sensitive nub grows on the summit of each tit, becoming a new nipple.", false);
            player.createBreastRow();
            player.upperBody.chest.list[0].breastRating = 2;
            player.upperBody.chest.list[0].breasts = 2;
        ...
        if (changes < changeLimit && player.dogScore() >= 3 && Utils.rand(4) == 0) {
            changes++;
            Render.text("\n\n", false);
            Render.text("Images and thoughts come unbidden to your mind, overwhelming your control as you rapidly lose yourself in them, daydreaming of... ", false);
            //cawk fantasies
            if (player.gender <= 1 || (player.gender == 3 && Utils.rand(2) == 0)) {
                Render.text("bounding through the woods, hunting with your master.  Feeling the wind in your fur and the thrill of the hunt coursing through your veins intoxicates you.  You have your nose to the ground, tracking your quarry as you run, until a heavenly scent stops you in your tracks.", false);
                dynStats("lus", 5 + player.stats.lib / 20);
                //break1
                if (player.stats.cor < 33 || !player.lowerBody.cockSpot.hasCock()) Render.text("\nYou shake your head to clear the unwanted fantasy from your mind, repulsed by it.", false);
                else {
                    Render.text("  Heart pounding, your shaft pops free of its sheath on instinct, as you take off after the new scent.  Caught firmly in the grip of a female's heat, you ignore your master's cry as you disappear into the wild, " + Appearance.cockNoun(CockType.DOG) + " growing harder as you near your quarry.  You burst through a bush, spotting a white-furred female.  She drops, exposing her dripping fem-sex to you, the musky scent of her sex channeling straight through your nose and sliding into your " + Appearance.cockNoun(CockType.DOG) + ".", false);
                    dynStats("lus", 5 + player.stats.lib / 20);
                    //Break 2
                    if (player.stats.cor < 66) Render.text("\nYou blink a few times, the fantasy fading as you master yourself.  That daydream was so strange, yet so hot.", false);
                        ...
                        Render.text("  Unable to wait any longer, you mount her, pressing your bulging knot against her vulva as she yips in pleasure. The heat of her sex is unreal, the tight passage gripping you like a vice as you jackhammer against her, biting her neck gently in spite of the violent pounding.", false);
                        dynStats("lus", 5 + player.stats.lib / 20);
                        //break3
                        if (player.stats.cor < 80) {
                            if (player.lowerBody.vaginaSpot.count() > 0) Render.text("\nYou reluctantly pry your hand from your aching " + vaginaDescript(0) + " as you drag yourself out of your fantasy.", false);
            ...
            //Pure female fantasies
                ...
                Render.text("wagging your dripping " + vaginaDescript(0) + " before a pack of horny wolves, watching their shiny red doggie-pricks practically jump out of their sheaths at your fertile scent.", false);
                dynStats("lus", 5 + player.stats.lib / 20);
                //BREAK 1
                if (player.stats.cor < 33) {
                    Render.text("\nYou shake your head to clear the unwanted fantasy from your mind, repulsed by it.", false);
                    ...
                    Render.text("  In moments they begin their advance, plunging their pointed beast-dicks into you, one after another.  You yip and howl with pleasure as each one takes his turn knotting you.", false);
                    dynStats("lus", 5 + player.stats.lib / 20);
                    //BREAK 2
                    if (player.stats.cor <= 66) {
                        Render.text("\nYou blink a few times, the fantasy fading as you master yourself.  That daydream was so strange, yet so hot.", false);
                        ...
                        Render.text("  The feeling of all that hot wolf-spooge spilling from your overfilled snatch and running down your thighs is heavenly, nearly making you orgasm on the spot.  You see the alpha of the pack is hard again, and his impressive member is throbbing with the need to breed you.", false);
                        dynStats("lus", 5 + player.stats.lib / 20);
                        //break3
                        if (player.stats.cor < 80) {
                            Render.text("\nYou reluctantly pry your hand from your aching " + vaginaDescript(0) + " as you drag yourself out of your fantasy.", false);
                ...
                Render.text("wagging your [asshole] before a pack of horny wolves, watching their shiny red doggie-pricks practically jump out of their sheaths at you after going so long without a female in the pack.");
                dynStats("lus", 5 + player.stats.lib / 20);
                //BREAK 1
                if (player.stats.cor < 33) {
                    Render.text("\nYou shake your head to clear the unwanted fantasy from your mind, repulsed by it.", false);
                    ...
                    if (player.stats.cor <= 66) {
                        ...
                        Render.text("  The feeling of all that hot wolf-spooge spilling from your overfilled ass and running down your thighs is heavenly, nearly making you orgasm on the spot.  You see the alpha of the pack is hard again, and his impressive member is throbbing with the need to spend his lust on you.", false);
                        dynStats("lus", 5 + player.stats.lib / 20);
                        //break3
                        if (player.stats.cor < 80) {
    ...
    public impFood(player: Player): void {
        Render.text("", true);
        if (player.lowerBody.cockSpot.count() > 0) {
            Render.text("The food tastes strange and corrupt - you can't really think of a better word for it, but it's unclean.", false);
            ...
            if (Utils.rand(30) == 0 && player.skinTone != "red") {
                if (player.skinType == SKIN.FUR) Render.text("\n\nUnderneath your fur, your skin ", false);
                else Render.text("\n\nYour " + player.skinDesc + " ", false);
                if (Utils.rand(2) == 0) player.skinTone = "red";
                else player.skinTone = "orange";
                Render.text("begins to lose its color, fading until you're as white as an albino.  Then, starting at the crown of your head, a reddish hue rolls down your body in a wave, turning you completely " + player.skinTone + ".", false);
            }
            return;
        }
        else {
            Render.text("The food tastes... corrupt, for lack of a better word.\n", false);
    ...
    public succubisDelight(tainted: boolean, player: Player): void {
        ...
        Render.text("You uncork the bottle and drink down the strange substance, struggling to down the thick liquid.", true);
        //low corruption thoughts
        if (player.stats.cor < 33) Render.text("  This stuff is gross, why are you drinking it?", false);
        //high corruption
        if (player.stats.cor >= 66) Render.text("  You lick your lips, marvelling at how thick and sticky it is.", false);
        //Corruption increase
        ...
        if (player.lowerBody.balls > 0 && Utils.rand(3) == 0) {
            Render.text(player.modFem(12, 3), false);
        }
    }
    ...
    public succubisDream(player: Player): void {
        ...
        if (player.lowerBody.balls > 0 && Utils.rand(3) == 0) {
            Render.text(player.modFem(12, 5), false);
        }
    }
    ...
    public hairDye(color: string, player: Player): void {
        if (player.hairColor.indexOf("rubbery") != -1 || player.hairColor.indexOf("latex-textured") != -1) {
            Render.text("You massage the dye into your " + hairDescript() + " but the dye cannot penetrate the impermeable material your hair is composed of.", true);
            return;
        }
        if (player.hairLength == 0) {
            Render.text("You rub the dye into your bald head, but it has no effect.", true);
            return;
        }
        Render.text("You rub the dye into your " + hairDescript() + ", then use a bucket of cool lakewater to rinse clean a few minutes later.  ", true);
        player.hairColor = color;
        Render.text("You now have " + hairDescript() + ".", false);
        if (player.lust > 50) {
    ...
    public purePearl(player: Player): void {
        Render.text("You cram the pearl in your mouth and swallow it like a giant pill with some difficulty.  Surprisingly there is no discomfort, only a cool calming sensation that springs up from your core.", true);
    ...
    public lactaid(player: Player): void {
        player.slimeFeed();
        let i: number = 0;
        Render.text("You gulp down the bottle of lactaid, easily swallowing the creamy liquid.", true);
        //Bump up size!
        ...
        if (player.biggestLactation() < 1) {
            Render.text("\n\n", false);
            Render.text("You feel your " + nippleDescript(0) + "s become tight and engorged.  A single droplet of milk escapes each, rolling down the curves of your breasts.  <b>You are now lactating!</b>", false);
        ...
        //Boost lactation
            ...
            Render.text("\n\n", false);
            Render.text("Milk leaks from your " + nippleDescript(0) + "s in thick streams.  You're lactating even more!", false);
            for (i = 0; i < player.upperBody.chest.count(); i++) {
        ...
        dynStats("lus", 10);
        if (Utils.rand(3) == 0) {
            Render.text(player.modFem(95, 1), false);
        }
    }
    ...
    public useMarbleMilk(player: Player): void {
        ...
        if (player.statusAffects.get("Marble").value2 < 30 && player.statusAffects.get("$1").value3 == 0) Render.text("You gulp down the bottle's contents; Marble makes some good tasting milk.\n\n", false);
        //[before the player is addicted, Addiction < 50]
        else if (player.statusAffects.get("$1").value3 <= 0) Render.text("You gulp down the bottle's contents; Marble makes some really good tasting milk.\n\n", false);
        else if (player.statusAffects.get("$1").value3 > 0) {
            //[player is completely addicted]
            if (player.perks.has("MarblesMilk")) Render.text("You gulp down the bottle's contents; it's no substitute for the real thing, but it's a nice pick me up.\n\n", false);
            else {
                //[player is no longer addicted]
                if (player.perks.has("MarbleResistant")) Render.text("You gulp down the bottle's contents; you're careful not to get too attached to the taste.\n\n", false);
                //[player is addicted]
                else Render.text("You gulp down the bottle's contents; you really needed that.\n\n", false);
            }
        }
        ...
        if (player.statusAffects.has("MarbleWithdrawl")) {
            ...
            Render.text("You no longer feel the symptoms of withdrawal.\n\n", false);
        }
    ...
    public laBova(tainted: boolean, enhanced: boolean, player: Player): void {
        ...
        //ItemUseText:
        Render.text("You drink the ", true);
        if (enhanced) Render.text("Pro Bova", false);
        else Render.text("La Bova", false);
        Render.text(".  The drink has an odd texture, but is very sweet.  It has a slight aftertaste of milk.", false);
        ...
        if (player.lowerBody.cockSpot.count() > 0 && Utils.rand(2) == 0 && !flags[FlagEnum.HYPER_HAPPY]) {
            ...
            if (player.lowerBody.cockSpot.list[temp].cockLength < 2) {
                ...
                if (player.lowerBody.cockSpot.count() == 1 && !player.lowerBody.vaginaSpot.hasVagina()) {
                    Render.text("Your " + cockDescript(0) + " suddenly starts tingling.  It's a familiar feeling, similar to an orgasm.  However, this one seems to start from the top down, instead of gushing up from your loins.  You spend a few seconds frozen to the odd sensation, when it suddenly feels as though your own body starts sucking on the base of your shaft.  Almost instantly, your cock sinks into your crotch with a wet slurp.  The tip gets stuck on the front of your body on the way down, but your glans soon loses all volume to turn into a shiny new clit.", false);
                    if (player.lowerBody.balls > 0) Render.text("  At the same time, your " + ballsDescriptLight() + " fall victim to the same sensation; eagerly swallowed whole by your crotch.", false);
                    Render.text("  Curious, you touch around down there, to find you don't have any exterior organs left.  All of it got swallowed into the gash you now have running between two fleshy folds, like sensitive lips.  It suddenly occurs to you; <b>you now have a vagina!</b>", false);
        ...
        if (enhanced) {
            ...
            if (player.upperBody.chest.list[0].nipplesPerBreast == 1) {
                ...
                if (player.upperBody.chest.count() >= 2 && player.upperBody.chest.list[1].nipplesPerBreast == 1) {
                    Render.text("A moment later your second row of " + breastDescript(1) + " does the same.  <b>You have sixteen nipples now!</b>", false);
                ...
                if (player.upperBody.chest.count() >= 3 && player.upperBody.chest.list[2].nipplesPerBreast == 1) {
                    Render.text("Finally, your ");
                    if (player.bRows() == 3) Render.text("third row of " + breastDescript(2) + " mutates along with its sisters, sprouting into a wonderland of nipples.", false);
                    else if (player.bRows() >= 4) {
                        Render.text("everything from the third row down mutates, sprouting into a wonderland of nipples.", false);
                    ...
                    Render.text("  <b>You have a total of " + num2Text(player.upperBody.chest.countNipples()) + " nipples.</b>");
                }
            }
            //QUAD DAMAGE IF WEIRD SHIT BROKE BEFORE
                ...
                if ((player.upperBody.chest.BreastRatingLargest[0].nippleLength < 1.5 && tainted) || (!tainted && player.upperBody.chest.BreastRatingLargest[0].nippleLength < 1)) {
                    Render.text("  Your " + nippleDescript(0) + "s swell up, growing larger to accommodate your increased milk flow.", false);
        ...
        if (tainted && player.lowerBody != LOWER_BODY.HOOFED && player.upperBody.head.earType == EARS.COW) {
            if (changes < changeLimit && Utils.rand(3) == 0) {
                ...
                //Catch-all
                if (player.lowerBody > LOWER_BODY.NAGA) Render.text("\n\nYou stagger as your " + player.feet() + " change, curling up into painful angry lumps of flesh.  They get tighter and tighter, harder and harder, until at last they solidify into hooves!", false);
                Render.text("  A coat of beastial fur springs up below your waist, itching as it fills in.<b>  You now have hooves in place of your feet!</b>", false);
                player.lowerBody = LOWER_BODY.HOOFED;
                dynStats("cor", 0);
                changes++;
            }
        ...
        if (Utils.rand(3) == 0) Render.text(player.modFem(79, 3), false);
        if (Utils.rand(3) == 0) Render.text(player.modThickness(70, 4), false);
        if (Utils.rand(5) == 0) Render.text(player.modTone(10, 5), false);
    }


    public blackSpellbook(player: Player): void {
        Render.text("You open the small black book, and discover it to be an instructional book on the use of black magic.  Most of it is filled with generic information about black magic - how it is drawn from emotions (typically lust), and how it has the power to affect bodies and emotions.  It also warns against using it on oneself, as it is difficult to draw on your emotions while meddling with your own body.  In no time at all you've read the whole thing, but it disappears into thin air before you can put it away.", true);
    ...
    public whiteSpellbook(player: Player): void {
        Render.text("You open the white tome, and discover it to be an instructional book on the use of white magic.  Most of it is filled with generic information about white magic - how it is drawn for mental focus, is difficult to use when tired or aroused, and can be used to create and control energy.  In no time at all you've read the whole thing, but it disappears into thin air before you can put it away.", true);
        if (player.stats.int < 30) {
            Render.text("\n\nYou feel greatly enlightened by your time spent reading.", false);
            dynStats("int", 4);
        }
    ...
    public lustDraft(fuck: boolean, player: Player): void {
        player.slimeFeed();
        Render.text("You drink the ", true);
        if (fuck) Render.text("red", false);
        else Render.text("pink", false);
        Render.text(" potion, and its unnatural warmth immediately flows to your groin.", false);
        ...
        if (player.lust >= 100 && !kGAMECLASS.inCombat) {
            ...
            if (player.lowerBody.cockSpot.count() > 0) {
                Render.text(multiCockDescriptLight() + " erupts in front of you, liberally spraying the ground around you.  ", false);
            }
            if (player.lowerBody.cockSpot.count() > 0 && player.lowerBody.vaginaSpot.count() > 0) {
                Render.text("At the same time your ", false);
            }
            if (player.lowerBody.vaginaSpot.count() > 0) {
                Render.text(vaginaDescript(0) + " soaks your thighs.  ", false);
            }
            if (player.gender == 0) Render.text("body begins to quiver with orgasmic bliss.  ", false);
            Render.text("Once you've had a chance to calm down, you notice that the explosion of pleasure you just experienced has rocked you to your core.  You are a little hornier than you were before.", false);
    ...
    public goblinAle(player: Player): void {
        ...
        if (player.perks.has("HistoryAlchemist")) changeLimit++;
        Render.text("You drink the ale, finding it to have a remarkably smooth yet potent taste.  You lick your lips and sneeze, feeling slightly tipsy.", true);
        dynStats("lus", 15);
        //Stronger
        if (player.str > 50) {
            dynStats("str", -1);
        ...
        if (player.skinTone != "green" && player.skinTone != "grayish-blue" && player.skinTone != "dark green" && player.skinTone != "pale yellow" && changes < changeLimit && Utils.rand(2) == 0) {
            ...
            if (player.skinType == SKIN.FUR) Render.text("skin", false);
            else Render.text(player.skinDesc, false);
            Render.text(" turned " + player.skinTone + ".  No way!  It's staying, it really changed color!", false);
        }
        //Face!
    ...
    public gooGasmic(player: Player): void {
        Render.text("You take the wet cloth in hand and rub it over your body, smearing the strange slime over your " + player.skinDesc + " slowly.", true);
        //Stat changes
        //libido up to 80
        ...
        if (player.upperBody.head.hairType != 3) {
            ...
            if (player.hairColor != "green" && player.hairColor != "purple" && player.hairColor != "blue" && player.hairColor != "cerulean" && player.hairColor != "emerald") {
                Render.text("  Stranger still, the hue of your semi-liquid hair changes to ");
        ...
        if (player.upperBody.head.hairType == 3 && (player.skinDesc != "skin" || player.skinAdj != "slimy")) {
            ...
            if (player.skinTone != "green" && player.skinTone != "purple" && player.skinTone != "blue" && player.skinTone != "cerulean" && player.skinTone != "emerald") {
                Render.text("  Stranger still, your skintone changes to ");
        ...
        if (player.skinAdj == "slimy" && player.skinDesc == "skin" && player.lowerBody != LOWER_BODY.GOO) {
            ...
            if (player.tallness < 36) {
                player.tallness = 36;
                Render.text("  The goo firms up and you return to your previous height.  It would truly be hard to get any shorter than you already are!", false);
            }
        ...
        if (Utils.rand(2) == 0) Render.text(player.modFem(85, 3), false);
        if (Utils.rand(2) == 0) Render.text(player.modThickness(20, 3), false);
        if (Utils.rand(2) == 0) Render.text(player.modTone(15, 5), false);
    }
    ...
    public sharkTooth(type: number, player: Player): void {
        ...
        if (Utils.rand(2) == 0) changeLimit++;
        if (player.perks.has("HistoryAlchemist")) changeLimit++;
        if (type == 0) Render.text("You have no idea why, but you decide to eat the pointed tooth. To your surprise, it's actually quite brittle, turning into a fishy-tasting dust. You figure it must just be a tablet made to look like a shark's tooth.", true);
        else if (type == 1) Render.text("You have no idea why, but you decide to eat the pointed, glowing tooth. To your surprise, it's actually quite brittle, crumbling into a fishy-tasting dust. Maybe it's just a tablet made to look like a shark's tooth.", true);
        //STATS
        ...
        if (type == 1 && (!player.lowerBody.cockSpot.hasCock()) && changes < changeLimit && Utils.rand(3) == 0) {
            ...
            if (player.lowerBody.balls == 0) {
                Render.text(" and a pair of balls", false);
                player.lowerBody.balls = 2;
        ...
        if (((player.skinTone != "rough gray" && player.skinTone != "orange and black striped") || player.skinType != SKIN.PLAIN) && Utils.rand(7) == 0 && changes < changeLimit) {
            Render.text("\n\n", false);
            if (player.skinType == SKIN.FUR || player.skinType == SKIN.SCALES) Render.text("Your " + player.skinDesc + " falls out, collecting on the floor and exposing your supple skin underneath.  ", false);
            else if (player.skinType == SKIN.GOO) Render.text("Your gooey skin solidifies, thickening up as your body starts to solidy into a more normal form. ", false);
            else if (type == 0) Render.text("Your skin itches and tingles becoming slightly rougher and turning gray.  ", false);
            if (type == 0) {
                Render.text("You abruptly stop moving and gasp sharply as a shudder goes up your entire frame. Your skin begins to shift and morph, growing slightly thicker and changing into a shiny grey color. Your skin now feels oddly rough too, comparable to that of a marine mammal. You smile and run your hands across your new shark skin.", false);
                player.skinType = SKIN.PLAIN;
                player.skinDesc = "skin";
                ...
                Render.text("Your skin begins to tingle and itch, before rapidly shifting to a shiny orange color, marked by Utils.random black stripes. You take a quick look in a nearby pool of water, to see your skin has morphed in appearance and texture to become more like a tigershark!", false);
                player.skinType = SKIN.PLAIN;
                player.skinDesc = "skin";
    ...
    public snakeOil(player: Player): void {
        ...
        Render.text("Pinching your nose, you quickly uncork the vial and bring it to your mouth, determined to see what effects it might have on your body. Pouring in as much as you can take, you painfully swallow before going for another shot, emptying the bottle.", false);
        //(if outside combat)
        if (!kGAMECLASS.inCombat) Render.text("  Minutes pass as you start wishing you had water with you, to get rid of the aftertaste.", false);
        //+ speed to 70!
        if (player.stats.spe < 70 && Utils.rand(2) == 0) {
            ...
            if (player.stats.spe < 40) Render.text("  Of course, you're nowhere near as fast as that.", false);
            changes++;
        }
        //Removes wings
        ...
        if (changes == 0 && player.tongueType == TONUGE.SNAKE && player.faceType != FACE.SNAKE_FANGS && Utils.rand(3) == 0 && changes < changeLimit) {
            ...
            if (player.faceType != FACE.HUMAN && player.faceType != FACE.SHARK_TEETH && player.faceType != FACE.BUNNY && player.faceType != FACE.SPIDER_FANGS) {
                Render.text("  As the change progresses, your " + player.face() + " reshapes.  The sensation is far more pleasant than teeth cutting into gums, and as the tingling transformation completes, <b>you've gained with a normal-looking, human visage.</b>");
            }
        ...
        if (changes == 0 && player.faceType == FACE.SNAKE_FANGS && player.lowerBody != LOWER_BODY.NAGA && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.lowerBody.balls > 0 && player.lowerBody.ballSize > 10) Render.text("  You're happy not to have to drag those testicles around with you anymore.", false);
            Render.text("  But then, scales start to form on the surface of your skin, slowly becoming visible, recoloring all of your body from the waist down in a snake-like pattern. The feeling is... not that bad actually, kind of like callous, except on your whole lower body. The transformation complete, you get up, standing on your newly formed snake tail. You can't help feeling proud of this majestic new body of yours.", false);
        ...
        /*
         ...
         Render.text("your " + multiCockDescriptLight() + " starts to throb painfully and swell to its full size.  With a horrifying ripping sensation, your cock splits down the middle, the pain causing you to black out momentarily.", false);
         Render.text("When you awaken, you quickly look down to see that where ", false);
         //(if multicock)
         if(player.lowerBody.cockSpot.count() > 1) Render.text("one of ", false);
         Render.text("your " + multiCockDescriptLight() + " was, you now have two pointed reptilian cocks, still stiff and pulsing.", false);
         }*/
        //Default change - blah
        if (changes == 0) Render.text("\n\nRemakarbly, the snake-oil has no effect.  Should you really be surprised at snake-oil NOT doing anything?", false);
    }
    ...
    /*
            public  extensionSerum(player:Player):void
            {
                ...
                Render.text("You open the bottle of hair extension serum and follow the directions carefully, massaging it into your scalp and being careful to keep it from getting on any other skin.  You wash off your hands with lakewater just to be sure.", false);
    ...
    */
    ...
    public Hummus(player: Player): void {
        Render.text("", true);
        if (debug) {
            Render.text("You're about to eat the humus when you see it has bugs in it. Not wanting to eat bugged humus or try to debug it you throw it into the portal and find something else to eat.", false);
            player.destroyItems(consumables.HUMMUS_, 1);
            return;
        }
        Render.text("You shovel the stuff into your face, not sure WHY you're eating it, but once you start, you just can't stop.  It tastes incredibly bland, and with a slight hint of cheese.", false);
        player.str = 30;
        player.stats.spe = 30;
        player.tou = 30;
        player.stats.int = 30;
    ...
    public coal(player: Player): void {
        let changes: number = 0;
        Render.text("", true);
        Render.text("You handle the coal rocks experimentally and they crumble to dust in your hands!  You cough as you breathe in the cloud, sputtering and wheezing.  After a minute of terrible coughing, you recover and realize there's no remaining trace of the rocks, not even a sooty stain on your hands!", false);
        //Try to go into intense heat
    ...
    public catTransformation(player: Player): void {
        ...
        //Text go!
        Render.text("", true);
        Render.text("You take a bite of the fruit and gulp it down. It's thick and juicy and has an almost overpowering sweetness. Nevertheless, it is delicious and you certainly could use a meal.  You devour the fruit, stopping only when the hard, nubby pit is left; which you toss aside.", false);
        ...
        if (player.stats.lib < 80 && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            if (player.catCocks() > 0) {
                ...
                if (player.stats.cor < 33) Render.text("You need to control yourself better.", false);
                else if (player.stats.cor < 66) Render.text("You're not sure how you feel about the fantasy.", false);
                else Render.text("You hope to find a willing partner to make this a reality.", false);
            }
            //Else –
            else {
                ...
                if (player.lust > 60) Render.text("even more ", false);
                Render.text("turned on.", false);
            }
        ...
        if (Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.goIntoHeat(false)) {
                if (intensified) {
                    ...
                    if (player.skinType == SKIN.FUR) Render.text("underneath your fur ", false);
                    Render.text("as images and fantasies ", false);
                    if (player.stats.cor < 50) Render.text("assault ", false);
                    else Render.text("fill ", false);
                    Render.text(" your mind.  Lithe cat-boys with their perfect, spine-covered cocks line up behind you, and you bend over to present your needy pussy to them.  You tremble with the desire to feel the exotic texture of their soft barbs rubbing your inner walls, smearing your " + vaginaDescript(0) + " with their cum as you're impregnated.  Shivering, you recover from the fantasy and pull your fingers from your aroused sex.  <b>It would seem you've gone into heat!</b>", false);
                }
                changes++;
            }
        }
                ...
                for (let k: number = 0; k < player.upperBody.chest.count(); k++) {
                    ...
                    if (player.upperBody.chest.list[k].breastRating > temp2) {
                        ...
                        if (player.upperBody.chest.list[k].breastRating > 10) {
                            ...
                            if (temp3 == 0) Render.text("\n\nThe " + breastDescript(0) + " on your chest wobble for a second, then tighten up, losing several cup-sizes in the process!", false);
                            else Render.text("  The change moves down to your " + num2Text2(k + 1) + " row of " + breastDescript(0) + ". They shrink greatly, losing a couple cup-sizes.", false);
                        }
                        //Small change
                        else {
                            ...
                            else Render.text("  Your " + num2Text2(k + 1) + " row of " + breastDescript(k) + " gives a tiny jiggle as it shrinks, losing some off its mass.", false);
                        }
        ...
        if (player.lowerBody.cockSpot.count() > 0 && player.catCocks() < player.lowerBody.cockSpot.count() &&
            ...
            if (!player.lowerBody.cockSpot.hasSheath()) {
                Render.text("Then, it begins to shrink and sucks itself inside your body.  Within a few moments, a fleshy sheath is formed.", false);
                if (player.lowerBody.balls > 0) Render.text("  Thankfully, your balls appear untouched.", false);
            }
            else Render.text("Then, it disappears back into your sheath.", false);
        ...
        if (player.catCocks() > 0 && Utils.rand(3) == 0 && changes < changeLimit && !flags[FlagEnum.HYPER_HAPPY]) {
            ...
            if (temp == 1) {
                ...
                Render.text("  Although the package is smaller, it feels even more sensitive – as if it retained all sensation of its larger size in its smaller form.", false);
                dynStats("sen", 5);
                //Make note of other dicks changing
                if (temp2 == 1) Render.text("  Upon further inspection, all your " + Appearance.cockNoun(CockType.CAT) + "s have shrunk!", false);
                changes++;
            }
        }
        ...
        if (player.tailType == TAIL.CAT && player.upperBody.head.earType == EARS.CAT &&
            ...
            if (player.lowerBody == LOWER_BODY.HOOFED || player.lowerBody == LOWER_BODY.CENTAUR) {
                ...
                if (player.lowerBody == LOWER_BODY.CENTAUR) Render.text("  You feel woozy and collapse on your side.  When you wake, you're no longer a centaur and your body has returned to a humanoid shape.", false);
            }
            //Goo to cat
        ...
        if (changes < changeLimit) {
            if (Utils.rand(2) == 0) Render.text(player.modThickness(5, 2), false);
            if (Utils.rand(2) == 0) Render.text(player.modTone(76, 2), false);
            if (player.gender < 2) if (Utils.rand(2) == 0) Render.text(player.modFem(65, 1), false);
            else Render.text(player.modFem(85, 2), false);
        }
    }
    ...
    public reptilum(player: Player): void {
        ...
        //clear screen
        Render.text("", true);
        Render.text("You uncork the vial of fluid and drink it down.  The taste is sour, like a dry wine with an aftertaste not entirely dissimilar to alcohol.  Instead of the warmth you'd expect, it leaves your throat feeling cold and a little numb.", false);
        ...
        if (player.stats.lib < 100 && changes < changeLimit && Utils.rand(3) == 0) {
            ...
            if (player.lowerBody.cockSpot.count() > 0 && (player.gender != 3 || Utils.rand(2) == 0)) {
                Render.text("filling ", false);
                if (player.lowerBody.cockSpot.count() > 1) Render.text("each of ", false);
                Render.text("your " + multiCockDescriptLight() + " with the desire to breed.  You get a bit hornier when you realize your sex-drive has gotten a boost.", false);
            }
            //(COOCH)
            else if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("puddling in your " + vaginaDescript(0) + ".  An instinctive desire to mate and lay eggs spreads through you, increasing your lust and boosting your sex-drive.", false);
            //(TARDS)
            else Render.text("puddling in your featureless crotch for a split-second before it slides into your " + assDescript() + ".  You want to be fucked, filled, and perhaps even gain a proper gender again.  Through the lust you realize your sex-drive has been permanently increased.", false);
        ...
        if (player.lizardCocks() == 0 && player.lowerBody.cockSpot.count() > 0 && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            Render.text("to tip, undulating and convulsing as its color lightens, darkens, and finally settles on a purplish hue.  Your " + Appearance.cockNoun(CockType.HUMAN) + " resolves itself into a bulbous form, with a slightly pointed tip.  The 'bulbs' throughout its shape look like they would provide an interesting ride for your sexual partners, but the perverse, alien pecker ", false);
            if (player.stats.cor < 33) Render.text("horrifies you.", false);
            else if (player.stats.cor < 66) Render.text("is a little strange for your tastes.", false);
            else {
                Render.text("looks like it might be more fun to receive than use on others.  ", false);
                if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("Maybe you could find someone else with one to ride?", false);
                else Render.text("Maybe you should test it out on someone and ask them exactly how it feels?", false);
            }
            Render.text("  <b>You now have a bulbous, lizard-like cock.</b>", false);
            //Actually xform it nau
            if (player.lowerBody.cockSpot.hasSheath()) {
                player.lowerBody.cockSpot.list[temp2].cockType = CockType.LIZARD;
                if (!player.lowerBody.cockSpot.hasSheath()) Render.text("\n\nYour sheath tightens and starts to smooth out, revealing ever greater amounts of your " + cockDescript(temp2) + "'s lower portions.  After a few moments <b>your groin is no longer so animalistic – the sheath is gone.</b>", false);
            }
        ...
        if (player.lowerBody.cockSpot.count() > 1 && player.lizardCocks() > 0 && player.lowerBody.cockSpot.count() > player.lizardCocks() && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.lowerBody.cockSpot.count() == 2) Render.text("your other dick", false);
            else Render.text("another one of your dicks", false);
            Render.text(" starts to change into the strange reptilian shape you've grown familiar with.  It warps visibly, trembling and radiating pleasurable feelings back to you as the transformation progresses.  ", false);
            if (player.cumQ() < 50) Render.text("pre-cum oozes from the tip", false);
            else if (player.cumQ() < 700) Render.text("Thick pre-cum rains from the tip", false);
            else Render.text("A wave of pre-cum splatters on the ground", false);
            Render.text(" from the pleasure of the change.  In moments <b>you have a bulbous, lizard-like cock.</b>", false);
        ...
        if (player.lizardCocks() == player.lowerBody.cockSpot.count() && changes < changeLimit && player.statusAffects.has("Infested")) {
            ...
            if (player.lowerBody.balls > 1) Render.text("  The remaining " + num2Text(player.lowerBody.balls - 1) + " slither out the pre-stretched holes with ease, though the last one hangs from your tip for a moment before dropping to the ground.", false);
            Render.text("  The white creature joins its kin on the ground and slowly slithers away.  Perhaps they prefer mammals? In any event, <b>you are no longer infected with worms</b>.", false);
        ...
        if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1 && player.gender == 1 && changes < changeLimit && Utils.rand(3) == 0) {
            ...
            //(BOTH – no new PG)
            Render.text("  With the change in weight and gravity, you find it's gotten much easier to move about.", false);
        ...
        if (player.biggestLactation() >= 1 && changes < changeLimit && Utils.rand(4) == 0) {
            if (player.upperBody.chest.countNipples() == 2) Render.text("\n\nBoth of your", false);
            else Render.text("\n\nAll of your many", false);
            Render.text(" nipples relax.  It's a strange feeling, and you pull back your top to touch one.  It feels fine, though there doesn't seem to be any milk leaking out.  You give it a squeeze and marvel when nothing ", false);
            if (player.upperBody.chest.hasFuckableNipples()) Render.text("but sexual fluid ", false);
            Render.text("escapes it.  <b>You are no longer lactating.</b>  That makes sense, only mammals lactate!  Smiling, you muse at how much time this will save you when cleaning your gear.", false);
        ...
        if (player.averageNipplesPerBreast() > 1 && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            if (player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) Render.text("'breast'.", false);
            else Render.text("breast.", false);
            changes++;
        ...
        if (player.lowerBody.vaginaSpot.hasVagina() && !player.perks.has("Oviposition") && changes < changeLimit && Utils.rand(5) == 0 && player.lizardScore() > 3) {
            ...
            Render.text("(<b>Perk Gained: Oviposition</b>)", false);
        ...
        if (flags[FlagEnum.HAIR.GROWTH_STOPPED_BECAUSE_LIZARD] == 0 && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            Render.text("(<b>Your hair has stopped growing.</b>)", false);
            changes++;
        ...
        if (player.lowerBody != LOWER_BODY.LIZARD && changes < changeLimit && Utils.rand(5) == 0) {
            ...
            Render.text("  <b>You have reptilian legs and claws!</b>", false);
        ...
        if (player.skinType != SKIN.SCALES && player.upperBody.head.earType == EARS.LIZARD && player.tailType == TAIL.LIZARD && player.lowerBody == LOWER_BODY.LIZARD && changes < changeLimit && Utils.rand(5) == 0) {
            ...
            //(no fur)
                ...
                Render.text(player.skinTone + " scales.</b>", false);
            }
    ...
    /*
            ...
            {
                Render.text("You toss a wingstick at your foe!  It flies straight and true, almost as if it has a mind of its own as it arcs towards " + monster.a + monster.short + "!\n", true);
                //1% dodge for each point of speed over 80
                if (monster.spe - 80 > Utils.rand(100) + 1) {
                    Render.text("Somehow " + monster.a + monster.short + "'", false);
                    if (!monster.plural) Render.text("s", false);
                    Render.text(" incredible speed allows " + monster.pronoun2 + " to avoid the spinning blades!  The deadly device shatters when it impacts something in the distance.", false);
                }
                //Not dodged
                else {
                    let damage:number = 40 + Utils.rand(61);
                    Render.text(monster.capitalA + monster.short + " is hit with the wingstick!  It breaks apart as it lacerates " + monster.pronoun2 + ". (" + damage + ")", false);
                    monster.HP -= damage;
    ...
    */
    ...
    public neonPinkEgg(pregnantChange: boolean, player: Player): void {
        ...
        else Render.text("", true);
        //If not pregnancy, mention eating it.
        if (!pregnantChange) Render.text("You eat the neon pink egg, and to your delight it tastes sweet, like candy.  In seconds you've gobbled down the entire thing, and you lick your fingers clean before you realize you ate the shell – and it still tasted like candy.", false);
        //If pregnancy, warning!
        if (pregnantChange) {
            Render.text("\n<b>Your egg-stuffed ", false);
            if (player.pregnancyType == PregnancyType.BUNNY) {
                Render.text("womb ", false);
                if (player.buttPregnancyType == PregnancyType.BUNNY) Render.text("and ", false);
            }
            if (player.buttPregnancyType == PregnancyType.BUNNY) Render.text("backdoor ", false);
            if (player.buttPregnancyType == PregnancyType.BUNNY && player.pregnancyType == PregnancyType.BUNNY) Render.text("rumble", false);
            else Render.text("rumbles", false);
            Render.text(" oddly, and you have a hunch that something's about to change</b>.", false);
        }
        //STATS CHANGURYUUUUU
        //Boost speed (max 80!)
        if (changes < changeLimit && Utils.rand(3) == 0 && player.stats.spe < 80) {
            if (player.stats.spe < 30) Render.text("\n\nTingles run through your muscles, and your next few movements seem unexpectedly fast.  The egg somehow made you faster!", false);
            else if (player.stats.spe < 50) Render.text("\n\nYou feel tingles running through your body, and after a moment, it's clear that you're getting faster.", false);
            else if (player.stats.spe < 65) Render.text("\n\nThe tight, ready feeling you've grown accustomed to seems to intensify, and you know in the back of your mind that you've become even faster.", false);
        ...
        if (changes < changeLimit && Utils.rand(5) == 0) {
            ...
            if (player.stats.lib < 60) {
                ...
                if (!player.lowerBody.cockSpot.hasCock() || (player.gender == 3 && Utils.rand(2) == 0)) {
                    if (player.stats.lib < 30) {
                        ...
                        if (player.stats.cor < 25) Render.text("You're repulsed by such shameful thoughts.", false);
                        else if (player.stats.cor < 60) Render.text("You worry that this place is really getting to you.", false);
                        else if (player.stats.cor < 90) Render.text("You pant a little and wonder where the nearest fertile male is.", false);
                        else Render.text("You grunt and groan with desire and disappointment.  You should get bred soon!", false);
                    }
                ...
                if (player.lowerBody.cockSpot.hasCock()) {
                    if (player.stats.lib < 30) {
                        ...
                        if (Utils.rand(2) == 0) Render.text("female hare until she's immobilized by all her eggs", false);
                        else Render.text("herm rabbit until her sack is so swollen that she's forced to masturbate over and over again just to regain mobility", false);
                        Render.text(". ", false);
                        if (player.stats.cor < 25) Render.text("You're repulsed by such shameful thoughts.", false);
                        else if (player.stats.cor < 50) Render.text("You worry that this place is really getting to you.", false);
                        else if (player.stats.cor < 75) Render.text("You pant a little and wonder where the nearest fertile female is.", false);
                        else Render.text("You grunt and groan with desire and disappointment.  Gods you need to fuck!", false);
                    }
            ...
            //Libido over 60? FUCK YEAH!
                ...
                if (player.skinType > SKIN.PLAIN) Render.text(" through your " + player.skinDesc, false);
                Render.text(".  ", false);
                if (player.gender == 1) Render.text("Compression tightens down on " + sMultiCockDesc() + " as it strains against your " + player.armorName + ".  You struggle to fight down your heightened libido, but it's hard – so very hard.", false);
                else if (player.gender == 0) Render.text("Sexual hunger seems to gnaw at your " + assholeDescript() + ", demanding it be filled, but you try to resist your heightened libido.  It's so very, very hard.", false);
                else if (player.gender == 2) Render.text("Moisture grows between your rapidly-engorging vulva, making you squish and squirm as you try to fight down your heightened libido, but it's hard – so very hard.", false);
                else Render.text("Steamy moisture and tight compression war for your awareness in your groin as " + sMultiCockDesc() + " starts to strain against your " + player.armorName + ".  Your vulva engorges with blood, growing slicker and wetter.  You try so hard to fight down your heightened libido, but it's so very, very hard.  The urge to breed lingers in your mind, threatening to rear its ugly head.", false);
            }
            //MEGALIBIDO
            else {
                ...
                if (player.stats.cor < 33) Render.text("You sigh, trying not to give in completely.", false);
                else if (player.stats.cor < 66) Render.text("You pant and groan, not sure how long you'll even want to resist.", false);
                else {
                    Render.text("You smile and wonder if you can ", false);
                    if (player.stats.lib < 100) Render.text("get your libido even higher.", false);
                    else Render.text("find someone to fuck right now.", false);
                }
            }
        }
        ...
        if (player.stats.sens < 60 && changes < changeLimit && Utils.rand(3) == 0) {
            ...
            //(low)
            if (Utils.rand(3) != 2) {
                Render.text("The feeling of small breezes blowing over your " + player.skinDesc + " gets a little bit stronger.  How strange.  You pinch yourself and nearly jump when it hurts a tad more than you'd think. You've gotten more sensitive!", false);
                dynStats("sen", 5);
            }
            //(BIG boost 1/3 chance)
            else {
                dynStats("sen", 15);
                Render.text("Every movement of your body seems to bring heightened waves of sensation that make you woozy.  Your " + player.armorName + " rubs your " + nippleDescript(0) + "s deliciously", false);
                if (player.upperBody.chest.hasFuckableNipples()) {
                    Render.text(", sticking to the ", false);
                    if (player.biggestLactation() > 2) Render.text("milk-leaking nipple-twats", false);
                    else Render.text("slippery nipple-twats", false);
                }
                else if (player.biggestLactation() > 2) Render.text(", sliding over the milk-leaking teats with ease", false);
                else Render.text(" catching on each of the hard nubs repeatedly", false);
                Render.text(".  Meanwhile, your crotch... your crotch is filled with such heavenly sensations from ", false);
                if (player.gender == 1) {
                    Render.text(sMultiCockDesc() + " and your ", false);
                    if (player.lowerBody.balls > 0) Render.text(ballsDescriptLight(), false);
                    else Render.text(assholeDescript(), false);
                }
                else if (player.gender == 2) Render.text("your " + vaginaDescript(0) + " and " + clitDescript(), false);
                else if (player.gender == 3) {
                    Render.text(sMultiCockDesc() + ", ", false);
                    if (player.lowerBody.balls > 0) Render.text(ballsDescriptLight() + ", ", false);
                    Render.text(vaginaDescript(0) + ", and " + clitDescript(), false);
                }
                //oh god genderless
                else Render.text("you " + assholeDescript(), false);
                Render.text(" that you have to stay stock-still to keep yourself from falling down and masturbating on the spot.  Thankfully the orgy of tactile bliss fades after a minute, but you still feel way more sensitive than your previous norm.  This will take some getting used to!", false);
            }
        }
        ...
        if (player.lowerBody.vaginaSpot.hasVagina() && !player.perks.has("BunnyEggs") && changes < changeLimit && Utils.rand(4) == 0 && player.bunnyScore() > 3) {
            Render.text("\n\nDeep inside yourself there is a change.  It makes you feel a little woozy, but passes quickly.  Beyond that, you aren't sure exactly what just happened, but you are sure it originated from your womb.\n\n", false);
            Render.text("(<b>Perk Gained: Bunny Eggs</b>)", false);
        ...
        if ((player.lowerBody.balls > 0 || player.lowerBody.cockSpot.hasCock()) && player.cumQ() < 3000 && Utils.rand(3) == 0 && changeLimit > 1) {
            ...
            if (player.lowerBody.balls > 0) {
                ...
                //(medium cum quantity) < 250
                    ...
                    if (player.lowerBody.cockSpot.hasCock()) Render.text("  In the process, you brush " + sMultiCockDesc() + " and discover a bead of pre leaking at the tip.", false);
                }
                //(large cum quantity) < 750
                    ...
                    if (player.lowerBody.cockSpot.hasCock()) Render.text(SMultiCockDesc() + " leaks and dribbles pre-cum down your " + player.legs() + " as your body's cum production kicks up even higher.", false);
                    else Render.text("You wince, feeling pent up and yet unable to release.  You really wish you had a cock right about now.", false);
                }
                //(XL cum quantity) < 2000
                    ...
                    if (player.lowerBody.cockSpot.hasCock()) Render.text("A thick trail of slime leaks from " + sMultiCockDesc() + " down your " + player.leg() + ", pooling below you.", false);
                    else Render.text("You grunt, feeling terribly pent-up and needing to release.  Maybe you should get a penis to go with these balls...", false);
                    Render.text("  It's quite obvious that your cum production has gone up again.", false);
                }
                //(XXL cum quantity)
                else {
                    ...
                    if (player.lowerBody.cockSpot.hasCock()) Render.text("pre-cum explodes from " + sMultiCockDesc() + ", running down your " + player.leg() + " and splattering into puddles that would shame the orgasms of lesser " + player.mf("males", "persons") + ".  You rub yourself a few times, nearly starting to masturbate on the spot, but you control yourself and refrain for now.", false);
                    else Render.text("You pant and groan but the pleasure just turns to pain.  You're so backed up – if only you had some way to vent all your seed!", false);
                }
            }
        ...
        if (player.lowerBody != LOWER_BODY.BUNNY && changes < changeLimit && Utils.rand(5) == 0 && player.upperBody.head.earType == EARS.BUNNY) {
            ...
            //Non-taurs
                ...
                if (player.tailType > TAIL.NONE) Render.text(", nearly smashing your tail flat", false);
                else Render.text(" hard enough to sting", false);
                Render.text(" while the change works its way through you.  Once it finishes, <b>you discover that you now have fuzzy bunny feet and legs</b>!", false);
            }
            changes++;
    ...
    public goldenSeed(type: number, player: Player): void {
        ...
        //Generic eating text:
        Render.text("", true);
        Render.text("You pop the nut into your mouth, chewing the delicious treat and swallowing it quickly.  No wonder harpies love these things so much!", false);
        //****************
        //Stats:
        //****************
        ...
        if ((player.stats.lib < 90 || Utils.rand(3) == 0) && Utils.rand(3) == 0 && changes < changeLimit) {
            ...
            if (player.stats.lib < 40) {
                ...
                if (player.lowerBody.vaginaSpot.hasVagina()) Render.text(" The moistness of your " + vaginaDescript() + " seems to agree.", false);
                else if (player.lowerBody.cockSpot.hasCock()) Render.text(" The hardness of " + sMultiCockDesc() + " seems to agree.", false);
                dynStats("lus", 5);
            }
            //(sub 75 lib)
            else if (player.stats.lib < 75) Render.text("\n\nHeat, blessed heat, works through you from head to groin, leaving you to shudder and fantasize about the sex you could be having right now.\n\n", false);
            //(hi lib)
            else if (player.stats.lib < 90) Render.text("\n\nSexual need courses through you, flushing your skin with a reddish hue while you pant and daydream of the wondrous sex you should be having right now.\n\n", false);
            //(90+)
        ...
        if (changes < changeLimit && player.upperBody.chest.count() > 1 && Utils.rand(3) == 0 && !flags[FlagEnum.HYPER_HAPPY]) {
            changes++;
            Render.text("\n\nYou stumble back when your center of balance shifts, and though you adjust before you can fall over, you're left to watch in awe as your bottom-most " + breastDescript(player.upperBody.chest.count() - 1) + " shrink down, disappearing completely into your ", false);
            if (player.upperBody.chest.count() >= 3) Render.text("abdomen", false);
            else Render.text("chest", false);
            Render.text(". The " + nippleDescript(player.upperBody.chest.count() - 1) + "s even fade until nothing but ", false);
            if (player.skinType == SKIN.FUR) Render.text(player.hairColor + " " + player.skinDesc, false);
            else Render.text(player.skinTone + " " + player.skinDesc, false);
        ...
        if (changes < changeLimit && player.upperBody.chest.list[0].breastRating < 2 && Utils.rand(3) == 0) {
            ...
            if (player.upperBody.chest.list[0].breastRating < 1) Render.text("  <b>You have breasts now!</b>", false);
            player.upperBody.chest.list[0].breastRating = 2;
        }
        //****************
        //General Appearance:
        ...
        if (player.femininity < 85 && changes < changeLimit && Utils.rand(3) == 0) {
            changes++;
            Render.text(player.modFem(85, 3 + Utils.rand(5)), false);
        }
        ...
        if ((player.skinTone != "tan" && player.skinTone != "olive" && player.skinTone != "dark" && player.skinTone != "light") && changes < changeLimit && Utils.rand(5) == 0) {
            ...
            if (player.skinType == SKIN.FUR) Render.text("the skin under your " + player.hairColor + " " + player.skinDesc, false);
            else Render.text("your " + player.skinDesc, false);
            Render.text(" has changed to become ", false);
            temp = Utils.rand(4);
            ...
            Render.text(player.skinTone + " colored.</b>", false);
        }
        ...
        if (player.thickness > 25 && changes < changeLimit && Utils.rand(3) == 0) {
            Render.text(player.modThickness(25, 3 + Utils.rand(4)), false);
            changes++;
        }
        //Remove odd eyes
        if (changes < changeLimit && Utils.rand(5) == 0 && player.eyeType > EYES.HUMAN) {
        ...
        if (player.lowerBody != LOWER_BODY.HARPY && changes < changeLimit && (type == 1 || player.tailType == TAIL.HARPY) && Utils.rand(4) == 0) {
            ...
            changes++;
            //(cont)
            Render.text("While humanoid in shape, they have two large, taloned toes on the front and a single claw protruding from the heel. The entire ensemble is coated in " + player.hairColor + " feathers from ankle to hip, reminding you of the bird-women of the mountains. <b>You now have harpy legs!</b>", false);
        }
        //-Feathery Tail
    ...
    public kangaFruit(type: number, player: Player): void {
        Render.text("", true);
        Render.text("You squeeze the pod around the middle, forcing the end open.  Scooping out a handful of the yeasty-smelling seeds, you shovel them in your mouth.  Blech!  Tastes like soggy burnt bread... and yet, you find yourself going for another handful...", false);
        ...
        if (player.lowerBody.cockSpot.hasCock()) {
            ...
            if (player.kangaCocks() < player.lowerBody.cockSpot.count() && (type == 1 && Utils.rand(2) == 0) && changes < changeLimit) {
                ...
                if (player.lowerBody.cockSpot.hasSheath()) Render.text("your sheath", false);
                else Render.text("a sheath that forms at the base of it", false);
                Render.text(".  <b>You now have a kangaroo-penis!</b>", false);
        ...
        if (player.tailType != TAIL.KANGAROO && changes < changeLimit && Utils.rand(4) == 0 && (type != 1 || player.upperBody.head.earType == EARS.KANGAROO)) {
            ...
            //gain roo tail from bee tail:
                ...
                if (player.tailType == TAIL.BEE_ABDOMEN) Render.text("hollow stinger drops out of the end, taking the venom organ with it.", false);
                else Render.text("spinnerets drop out of the end, taking the last of your webbing with it.", false);
                Render.text("  By the time you're back to yourself, the insectile carapace has fallen off completely, leaving you with a long, thick, fleshy tail in place of your proud, insectile abdomen.  <b>You now have a kangaroo tail!</b>  You wipe the errant spittle from your mouth as you idly bob your new tail about.", false);
            }
    ...
    public sweetGossamer(type: number, player: Player): void {
        ...
        if (player.perks.has("HistoryAlchemist")) changeLimit++;
        //Consuming Text
        if (type == 0) Render.text("You wad up the sweet, pink gossamer and eat it, finding it to be delicious and chewy, almost like gum.  Munching away, your mouth generates an enormous amount of spit until you're drooling all over yourself while you devour the sweet treat.", false);
        else if (type == 1) Render.text("You wad up the sweet, black gossamer and eat it, finding it to be delicious and chewy, almost like licorice.  Munching away, your mouth generates an enormous amount of spit until you're drooling all over yourself while you devour the sweet treat.", false);
        ...
        if (player.str > 70 && Utils.rand(3) == 0) {
            ...
            if (player.spiderScore() < 4) Render.text("Wait, you're not a spider, that doesn't make any sense!", false);
            else Render.text("Well, maybe you should put your nice, heavy abdomen to work.", false);
            dynStats("str", -1);
            changes++;
        }
        //****************
        ...
        if (player.lowerBody.cockSpot.hasCock() && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            if (temp == 1) {
                ...
                Render.text(" out, you look closely.  ", false);
                if (player.lowerBody.cockSpot.count() == 1) Render.text("It's", false);
                else Render.text("They're", false);
                Render.text(" definitely thicker.", false);
                let counter: number;
                changes++;
            }
        }
        ...
        if (player.skinType != SKIN.PLAIN && (player.upperBody.head.earType == EARS.HUMAN || player.upperBody.head.earType == EARS.ELFIN) && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.skinType == SKIN.SCALES) Render.text("are", false);
            else Render.text("is", false);
            Render.text(" falling to the ground, revealing flawless, almost pearly-white skin underneath.  <b>You now have pale white skin.</b>", false);
    ...
    /*
            public  applyLustStick(player:Player):void
            {
                ...
                if (player.statusAffects.has("LustStickApplied")) {
                    ...
                    Render.text("You carefully open the sweet-smelling tube and smear the lipstick over the coat you already have on your lips.  <b>No doubt another layer will make it last even longer!</b>  ", false);
                    ...
                    Render.text("You carefully open the sweet-smelling tube and smear the lipstick over your lips.  ", false);
                    if (player.lowerBody.cockSpot.hasCock()) Render.text("It tingles a little, but the drugs have little to no effect on you now.", false);
                    else Render.text("Honestly, it amazes you that something as little as a kiss can make a man putty in your hands.", false);
    ...
    */
    ...
    public broBrew(player: Player): void {
        ...
        if (player.perks.has("BimboBody")) {
            ...
            if (!player.lowerBody.cockSpot.hasCock()) {
                ...
                Render.text("A swelling begins behind your man-meat, and you're assailed with an incredibly peculiar sensation as two sperm-filled balls drop into a newly-formed scrotum.  Frikkin' sweet!\n\n", false);
                player.lowerBody.balls = 2;
                player.lowerBody.ballSize = 3;
            }
            Render.text("Finally, you feel the transformation skittering to a halt, leaving you to openly roam your new chiseled and sex-ready body.  So what if you can barely form coherent sentences anymore?  A body like this does all the talking you need, you figure!", false);
            ...
            Render.text("\n\n", false);
            if (player.perks.has("BimboBrains")) Render.text("<b>(Lost Perks - Bimbo Brains, Bimbo Body)\n", false);
            else Render.text("<b>(Lost Perk - Bimbo Body)\n", false);
            ...
            Render.text("(Gained Perks - Futa Form, Futa Faculties)</b>", false);
            player.genderCheck();
            return;
        }
        //HP restore for bros!
        if (player.perks.has("BroBody") || player.perks.has("FutaForm")) {
            Render.text("You crack open the can and guzzle it in a hurry.  Goddamn, this shit is the best.  As you crush the can against your forehead, you wonder if you can find a six-pack of it somewhere?\n\n", false);
            fatigue(-33);
            HPChange(100, true);
            return;
        }
        Render.text("Well, maybe this will give you the musculature that you need to accomplish your goals.  You pull on the tab at the top and hear the distinctive snap-hiss of venting, carbonating pressure.  A smoky haze wafts from the opened container, smelling of hops and alcohol.  You lift it to your lips, the cold, metallic taste of the can coming to your tongue before the first amber drops of beer roll into your waiting mouth.  It tingles, but it's very, very good.  You feel compelled to finish it as rapidly as possible, and you begin to chug it.  You finish the entire container in seconds.\n\n", false);

        Render.text("A churning, full sensation wells up in your gut, and without thinking, you open wide to release a massive burp. It rumbles through your chest, startling birds into flight in the distance.  Awesome!  You slam the can into your forehead hard enough to smash the fragile aluminum into a flat, crushed disc.  Damn, you feel stronger already", false);
        if (player.stats.int > 50) Render.text(", though you're a bit worried by how much you enjoyed the simple, brutish act", false);
        Render.text(".\n\n", false);

        //(Tits b' gone)
        if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) {
            Render.text("A tingle starts in your " + nippleDescript(0) + "s before the tight buds grow warm, hot even.  ", false);
            if (player.biggestLactation() >= 1) Render.text("Somehow, you know that the milk you had been producing is gone, reabsorbed by your body.  ", false);
            Render.text("They pinch in towards your core, shrinking along with your flattening " + allChestDesc() + ".  You shudder and flex in response.  Your chest isn't just shrinking, it's reforming, sculping itself into a massive pair of chiseled pecs.  ", false);
            if (player.upperBody.chest.count() > 1) {
                Render.text("The breasts below vanish entirely.  ", false);
            ...
            player.statusAffects.remove("Feeder");
            player.perks.remove("Feeder");
            Render.text("All too soon, your boobs are gone.  Whoa!\n\n", false);
        }

        Render.text("Starting at your hands, your muscles begin to contract and release, each time getting tighter, stronger, and more importantly - larger.  The oddness travels up your arms, thickens your biceps, and broadens your shoulders.  Soon, your neck and chest are as built as your arms.  You give a few experimental flexes as your abs ", false);
        if (player.tone >= 70) Render.text("further define themselves", false);
        else Render.text("become extraordinarily visible", false);
        Render.text(".  The strange, muscle-building changes flow down your " + player.legs() + ", making them just as fit and strong as the rest of you.  You curl your arm and kiss your massive, flexing bicep.  You're awesome!\n\n", false);

        Render.text("Whoah, you're fucking ripped and strong, not at all like the puny weakling you were before.  Yet, you feel oddly wool-headed.  Your thoughts seem to be coming slower and slower, like they're plodding through a marsh.  You grunt in frustration at the realization.  Sure, you're a muscle-bound hunk now, but what good is it if you're as dumb as a box of rocks?  Your muscles flex in the most beautiful way, so you stop and strike a pose, mesmerized by your own appearance.  Fuck thinking, that shit's for losers!\n\n", false);

        //(has dick less than 10 inches)
        if (player.lowerBody.cockSpot.hasCock()) {
            if (player.lowerBody.cockSpot.list[0].cockLength < 10) {
                Render.text("As if on cue, the familiar tingling gathers in your groin, and you dimly remember you have one muscle left to enlarge.  If only you had the intelligence left to realize that your penis is not a muscle.  In any event, your " + cockDescript(0) + " swells in size, ", false);
                if (player.lowerBody.cockSpot.list[0].cockThickness < 2.75) {
                    Render.text("thickening and ", false);
                    player.lowerBody.cockSpot.list[0].cockThickness = 2.75;
                }
                Render.text("lengthening until it's ten inches long and almost three inches wide.  Fuck, you're hung!  ", false);
                player.lowerBody.cockSpot.list[0].cockLength = 10;
            }
            //Dick already big enough! BALL CHECK!
            if (player.lowerBody.balls > 0) {
                Render.text("Churning audibly, your " + sackDescript() + " sways, but doesn't show any outward sign of change.  Oh well, it's probably just like, getting more endurance or something.", false);
            }
            else {
                Render.text("Two rounded orbs drop down below, filling out a new, fleshy sac above your " + player.legs() + ".  Sweet!  You can probably cum buckets with balls like these.", false);
                player.lowerBody.balls = 2;
                player.lowerBody.ballSize = 3;
            }
            Render.text("\n\n", false);
        ...
        //(Pussy b gone)
        if (player.lowerBody.vaginaSpot.hasVagina()) {
            Render.text("At the same time, your " + vaginaDescript(0) + " burns hot, nearly feeling on fire.  You cuss in a decidedly masculine way for a moment before the pain fades to a dull itch.  Scratching it, you discover your lady-parts are gone.  Only a sensitive patch of skin remains.\n\n", false);
            player.removeVagina(0, 1);
        }
        player.genderCheck();
        //(below max masculinity)
        if (player.femininity > 0) {
            Render.text("Lastly, the change hits your face.  You can feel your jawbones shifting and sliding around, your skin changing to accommodate your face's new shape.  Once it's finished, you feel your impeccable square jaw and give a wide, easy-going grin.  You look awesome!\n\n", false);
            player.modFem(0, 100);
        }
        Render.text("You finish admiring yourself and adjust your " + player.armorName + " to better fit your new physique.  Maybe there's some bitches around you can fuck.  Hell, as good as you look, you might have other dudes wanting you to fuck them too, no homo.\n\n", false);
        ...
        Render.text("<b>(Bro Body - Perk Gained!)\n", false);
        Render.text("(Bro Brains - Perk Gained!)</b>\n", false);//int to 20.  max int 50)
        if (player.perks.has("Feeder")) {
            Render.text("<b>(Perk Lost - Feeder!)</b>\n", false);
            player.perks.remove("Feeder");
        }
    ...
    public ectoplasm(player: Player): void {
        ...
        if (player.stats.lib < 100 && Utils.rand(3) == 0 && changes < changeLimit) {
            ...
            Render.text(" a trace amount of the ghost girl's lust is transferred into you.  How horny IS she, you have to wonder...", false);
            dynStats("lib", 1);
            if (player.stats.lib < 50) dynStats("lib", 1);
            changes++;
        }
        ...
        if (changes < changeLimit && !player.perks.has("Incorporeality") && (player.skinTone == "white" || player.skinTone == "sable") && player.upperBody.head.hairType == 2) {
            ...
            Render.text("<b>(Gained Perk:  Incorporeality</b>)", false);
            player.createPerk(PerkLib.Incorporeality, 0, 0, 0, 0);
        }
        //Effect Script 8: 100% chance of healing
        if (changes == 0) {
            Render.text("You feel strangely refreshed, as if you just gobbled down a bottle of sunshine.  A smile graces your lips as vitality fills you.  ", false);
    ...
    public isabellaMilk(player: Player): void {
        Render.text("", true);
        Render.text("You swallow down the bottle of Isabella's milk.", false);
        if (player.fatigue > 0) Render.text("  You feel much less tired! (-33 fatigue)", false);
        fatigue(-33);
    }
    ...
    public shriveledTentacle(player: Player): void {
        Render.text("", true);
        Render.text("You chew on the rubbery tentacle; its texture and taste are somewhat comparable to squid, but the half-dormant nematocysts cause your mouth to tingle sensitively.", false);
        let changes: number = 0;
        let changeLimit: number = 1;
        if (Utils.rand(2) == 0) changeLimit++;
        if (Utils.rand(3) == 0) changeLimit++;
    ...
    public numbRocks(player: Player): void {
        ...
        Render.text("You pop open the package of numb rocks and dump it into your waiting mouth.  The strange candy fizzes and pops, leaving the nerves on your tongue feeling a bit deadened as you swallow the sweet mess.", false);
        ...
        if (Utils.rand(5) == 0) {
            if (player.findStatusAffect(StatusAffects.Dys) < 0) {
                ...
                if (player.lowerBody.vaginaSpot.hasVagina()) {
                    if (!player.lowerBody.cockSpot.hasCock()) Render.text("your ");
                    Render.text(vaginaDescript(0) + " ", false);
                }
                if (!(player.lowerBody.cockSpot.hasCock() || player.lowerBody.vaginaSpot.hasVagina())) Render.text(assholeDescript() + " ", false);
                Render.text(" numbs up too.  You give yourself a gentle touch, but are quite disturbed when you realize you can barely feel it.  You can probably still fuck something to get off, but regular masturbation is out of the question...", false);
        ...
        if (!player.perks.has("ThickSkin") && player.stats.sens < 30 && Utils.rand(4) == 0) {
            Render.text("Slowly, ", false);
            if (player.skinType == SKIN.PLAIN) Render.text("your skin", false);
            else Render.text("the skin under your " + player.skinDesc, false);
            Render.text(" begins to feel duller, almost... thicker.  You pinch yourself and find that your epidermis feels more resistant to damage, almost like natural armor!\n<b>(Thick Skin - Perk Gained!)</b>", false);
    ...
    public sensitivityDraft(player: Player): void {
        Render.text("", true);
        Render.text("You pop the cork on this small vial and drink down the clear liquid.  It makes your lips and tongue tingle strangely, letting you feel each globule of spit in your mouth and each breath of air as it slides past your lips.", false);
        ...
        if (Utils.rand(4) == 0 && player.findStatusAffect(StatusAffects.LustyTongue) < 0) {
            Render.text("The constant tingling in your mouth grows and grows, particularly around your lips, until they feel as sensitive as ", false);
            if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("your", false);
            else Render.text("a woman's", false);
            Render.text(" lower lips.  You'll have to be careful not to lick them!", false);
            //(Lustytongue status)
        ...
        if (player.lust > 70 || player.stats.lib > 70) {
            Render.text("You shiver and think of how much better it'll make sex and masturbation.", false);
        }
        else Render.text("You worry it'll make it harder to resist the attentions of a demon.", false);
        dynStats("sen", 10, "lus", 5);
    }

    public foxTF(enhanced: boolean, player: Player): void {
        clearOutput();
        if (!enhanced) Render.text("You examine the berry a bit, rolling the orangish-red fruit in your hand for a moment before you decide to take the plunge and chow down.  It's tart and sweet at the same time, and the flavors seem to burst across your tongue with potent strength.  Juice runs from the corners of your lips as you finish the tasty snack.");
        else Render.text("You pop the cap on the enhanced \"Vixen's Vigor\" and decide to take a swig of it.  Perhaps it will make you as cunning as the crude fox Lumi drew on the front?");
        let changes: number = 0;
        let changeLimit: number = 1;
        if (enhanced) changeLimit += 2;
        if (Utils.rand(2) == 0) changeLimit++;
        ...
        if (player.faceType == FACE.FOX && player.tailType == TAIL.FOX && player.upperBody.head.earType == EARS.FOX && player.lowerBody == LOWER_BODY.FOX && player.skinType == SKIN.FUR && Utils.rand(3) == 0) {
            if (flags[FlagEnum.FOX_BAD_END_WARNING] == 0) {
                ...
                Render.text("\n\nYou scarf down the ");
                if (enhanced) Render.text("fluid ");
                else Render.text("berries ");
                Render.text("with an uncommonly voracious appetite, taking particular enjoyment in the succulent, tart flavor.  As you carefully suck the last drops of ochre juice from your fingers, you note that it tastes so much more vibrant than you remember.  Your train of thought is violently interrupted by the sound of bones snapping, and you cry out in pain, doubling over as a flaming heat boils through your ribs.");
                ...
                if (player.lowerBody.tailVenom > 1) Render.text("  Your tails thrash around violently as they begin to fuse painfully back into one, the fur bristling back out with a flourish.");
        ...
        if (player.cumQ() < 5000 < 2 && Utils.rand(3) == 0 && changes < changeLimit && player.lowerBody.cockSpot.hasCock()) {
            ...
            if (player.lowerBody.balls == 0) Render.text("\n\nYou feel a churning inside your gut as something inside you changes.", false);
            if (player.lowerBody.balls > 0) Render.text("\n\nYou feel a churning in your " + ballsDescriptLight() + ".  It quickly settles, leaving them feeling somewhat more dense.", false);
            Render.text("  A bit of milky pre dribbles from your " + multiCockDescriptLight() + ", pushed out by the change.", false);
            changes++;
        }
        if (changes < changeLimit && player.lowerBody.balls > 0 && player.lowerBody.ballSize > 4 && Utils.rand(3) == 0) {
            ...
            changes++;
            Render.text("  You now have a [balls].");
        }
        //Sprouting more!
        if (changes < changeLimit && enhanced && player.bRows() < 4 && player.upperBody.chest.list[player.bRows() - 1].breastRating > 1) {
            ...
            if (!enhanced) Render.text("berry ");
            else Render.text("drink ");
            Render.text("settles deeper inside you.  A second later, the unpleasant gut-gurgle passes, and you let out a tiny burp of relief.  Before you finish taking a few breaths, there's an itching below your " + allChestDesc() + ".  You idly scratch at it, but gods be damned, it hurts!  You peel off part of your " + player.armorName + " to inspect the unwholesome itch, ");
            if (player.upperBody.chest.BreastRatingLargest[0].breastRating >= 8) Render.text("it's difficult to see past the wall of tits obscuring your view.");
            else Render.text("it's hard to get a good look at.");
            Render.text("  A few gentle prods draw a pleasant gasp from your lips, and you realize that you didn't have an itch - you were growing new nipples!");
            Render.text("\n\nA closer examination reveals your new nipples to be just like the ones above in size and shape");
            if (player.upperBody.chest.list[player.bRows() - 1].nipplesPerBreast > 1) Render.text(", not to mention number");
            else if (player.upperBody.chest.hasFuckableNipples()) Render.text(", not to mention penetrability");
            Render.text(".  While you continue to explore your body's newest addition, a strange heat builds behind the new nubs. Soft, jiggly breastflesh begins to fill your cupped hands.  Radiant warmth spreads through you, eliciting a moan of pleasure from your lips as your new breasts catch up to the pair above.  They stop at " + player.breastCup(player.bRows() - 1) + "s.  <b>You have " + num2Text(player.bRows() + 1) + " rows of breasts!</b>");
            player.createBreastRow();
        ...
        while (counter > 1) {
            ...
            if (player.upperBody.chest.list[counter].breastRating <= (player.upperBody.chest.list[counter - 1].breastRating - 1) && changes < changeLimit && Utils.rand(2) == 0) {
                ...
                Render.text("  You do a quick measurement and determine that your " + num2Text2(counter + 1) + " row of breasts are now " + player.breastCup(counter) + "s.");
        ...
        if ((enhanced || player.upperBody.head.earType == EARS.FOX) && player.lowerBody != LOWER_BODY.FOX && changes < changeLimit && Utils.rand(5) == 0) {
            ...
            //reg legs, not digitigrade
                ...
                if (player.lowerBody == LOWER_BODY.BEE) Render.text(", while a hideous cracking fills the air");
                Render.text(".  When the spasms subside and you can once again stand, <b>you find that your legs have been changed to those of a fox!</b>");
            }
        ...
        if (player.skinType == SKIN.FUR && player.faceType != FACE.FOX && changes < changeLimit && Utils.rand(5) == 0) {
            ...
            if (silly()) Render.text("  And they called you crazy...");
            changes++;
    ...
    public godMead(player: Player): void {
        clearOutput();
        Render.text("You take a hearty swig of mead, savoring the honeyed taste on your tongue.  Emboldened by the first drink, you chug the remainder of the horn's contents in no time flat.  You wipe your lips, satisfied, and let off a small belch as you toss the empty horn aside.");
        ...
        if (Utils.rand(3) == 0) {
            ...
            if (silly()) Render.text("  Maybe you should run around shirtless, drink, and fight!  Saxton Hale would be proud.");
            dynStats("str", 1);
        }
        //Tough:
        else {
    ...
    public sheepMilk(player: Player): void {
        Render.text("You gulp the bottle's contents, and its sweet taste immediately invigorates you, making you feel calm and concentrated", true);
    ...
    public eatEmberEgg(player: Player): void {
        ...
        Render.text("You crack the shell easily and swallow the large yolk and the copious amounts of albumen - the yolk is blue, while the rest is crimson-tinted.  It tastes like... well, it tastes mostly of spiced mint, you think.");
    ...
    public foxJewel(mystic: boolean, player: Player): void {
        ...
        if (((mystic && Utils.rand(2) == 0) || (!mystic && Utils.rand(4) == 0)) && changes < changeLimit && player.femininity != 50) {
            Render.text(player.modFem(50, 2), false);
            changes++;
        }
        ...
        //[Vag of Holding] - rare effect, only if PC has high vaginal looseness
            ...
            if (silly()) Render.text("  <b>Your vagina has become a universe unto itself, capable of accepting colossal insertions beyond the scope of human comprehension!</b>");
            else Render.text("  <b>Your vagina is now capable of accepting even the most ludicrously sized insertions with no ill effects.</b>");
        ...
        //[Grow 9th tail and gain Corrupted Nine-tails perk]
            ...
            Render.text("Your bushy tails begin to glow with an eerie, ghostly light, and with a crackle of electrical energy, split into nine tails.  <b>You are now a nine-tails!  But something is wrong...  The cosmic power radiating from your body feels...  tainted somehow.  The corruption pouring off your body feels...  good.</b>");
        ...
        if (player.skinType == SKIN.FUR || player.skinType == SKIN.SCALES && ((mystic) || (!mystic && Utils.rand(2) == 0))) {
            ...
            if (player.skinType == SKIN.FUR) Render.text("  You stare in horror as you pull your fingers away holding a handful of " + player.hairColor + " fur!  Your fur sloughs off your body in thick clumps, falling away to reveal patches of bare, " + player.skinTone + " skin.");
            else if (player.skinType == SKIN.SCALES) Render.text("  You stare in horror as you pull your fingers away holding a handful of dried up scales!  Your scales continue to flake and peel off your skin in thick patches, revealing the tender " + player.skinTone + " skin underneath.");
            Render.text("  Your skin slowly turns raw and red under your severe scratching, the tingling sensations raising goosebumps across your whole body.  Over time, the itching fades, and your flushed skin resolves into a natural-looking ");
            player.skinType = SKIN.PLAIN;
            player.skinAdj = "";
            player.skinDesc = "skin";
            ...
            Render.text(player.skinTone + " complexion.");
            Render.text("  <b>You now have " + player.skin() + "!</b>");
            changes++;
        }
    ...
    //Kitsune's Gift
            ...
            {
                clearOutput();
                Render.text("Curiosity gets the best of you, and you decide to open the package.  After all, what's the worst that could happen?\n\n");
    ...
    //			menuLoc = MENU_LOCATION_KITSUNE_GIFT;
                ...
                //[Fox Jewel]
                    case 0: 
                    Render.text("As the paper falls away, you carefully lift the cover of the box, your hands trembling nervously.  The inside of the box is lined with purple velvet, and to your delight, sitting in the center is a small teardrop-shaped jewel!");
                ...
                //[Fox Berries]
                    case 1:
                    Render.text("As the paper falls away, you carefully lift the cover of the box, your hands trembling nervously.  The inside of the box is lined with purple velvet, and to your delight, there is a small cluster of orange-colored berries sitting in the center!");
                ...
                //[Gems]
                    case 2:
                    Render.text("As the paper falls away, you carefully lift the cover of the box, your hands trembling nervously.  The inside of the box is lined with purple velvet, and to your delight, it is filled to the brim with shining gems!");
                ...
                //[Kitsune Tea/Scholar's Tea] //Just use Scholar's Tea and drop the "trick" effect if you don't want to throw in another new item.
                    case 3:
                    Render.text("As the paper falls away, you carefully lift the cover of the box, your hands trembling nervously.  The inside of the box is lined with purple velvet, and to your delight, it contains a small bag of dried tea leaves!");
                ...
                //[Hair Dye]
                    case 4:
                    Render.text("As the paper falls away, you carefully lift the cover of the box, your hands trembling nervously.  The inside of the box is lined with purple velvet, and to your delight, it contains a small vial filled with hair dye!");
                    let itype:ItemType = [
                        consumables.RED_DYE,
                        consumables.BLOND_D,
                        consumables.BLACK_D,
                ...
                //[Dud]
                    case 10:
                    Render.text("As the paper falls away, you carefully lift the cover of the box, your hands trembling nervously.  The inside of the box is lined with purple velvet, but to your disappointment, the only other contents appear to be nothing more than twigs, leaves, and other forest refuse.");
    ...
    /* Now handled by DeBimbo.as
            public  deBimbo(player:Player):void
            {
                ...
                if (!player.perks.has("BimboBrains") && !player.perks.has("FutaFaculties")) {
                    Render.text("You can't use this right now, and it's too expensive to waste!\n\n");
                    if (debug) {}
                    else {
    ...
    */
    ...
    public fishFillet(player: Player): void {
        clearOutput();
        if (!getGame().inCombat) Render.text("You sit down and unwrap your fish fillet. It's perfectly flaky, allowing you to break it off in bite-sized chunks.  The salty meal disappears quickly, and your stomach gives an appreciative gurgle.");
        //(In combat?)
        else Render.text("You produce the fish fillet from your bag.  Rather than unwrap it and savor the taste as you normally would, you take a large bite out of it, leaf wrapping and all.  In no time your salty meal is gone, your stomach giving an appreciative gurgle.");
    ...
    public trapOil(player: Player): void {
        ...
        if (Utils.rand(3) == 0) changeLimit++;
        if (player.perks.has("HistoryAlchemist")) changeLimit++;
        Render.text("You pour some of the oil onto your hands and ");
        if (player.stats.cor < 30) Render.text("hesitantly ");
        else if (player.stats.cor > 70) Render.text("eagerly ");
        Render.text("rub it into your arms and chest.  The substance is warm, coating and ever so slightly numbing; it quickly sinks into your skin, leaving you feeling smooth and sleek.");
        ...
        if (player.longestCockLength() >= 3.5 && player.lowerBody.cockSpot.hasCock() && Utils.rand(2) == 0 && changes < changeLimit) {
            ...
            Render.text(" incredibly sensitive and retract into your body.  Anxiously you pull down your underclothes to examine your nether regions.  To your relief ");
            if (player.lowerBody.cockSpot.count() == 1) Render.text("it is");
            else Render.text("they are");
            Render.text(" still present, and as you touch ");
            if (player.lowerBody.cockSpot.count() == 1) Render.text("it");
            else Render.text("them");
            Render.text(", the sensitivity fades, however - a blush comes to your cheeks - ");
            if (player.lowerBody.cockSpot.count() == 1) Render.text("it seems");
            else Render.text("they seem");
            Render.text(" to have become smaller.");
            temp = 0;
            while (temp < player.lowerBody.cockSpot.count()) {
        ...
        if (player.lowerBody.vaginaSpot.hasVagina() && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            dynStats("sen", -2);
            //High fertility:
            if (player.fertility >= 30) Render.text("It feels like your overcharged reproductive organs have simmered down a bit.");
            //Average fertility:
            else if (player.fertility >= 5) Render.text("You feel like you have dried up a bit inside; you are left feeling oddly tranquil.");
            //[Low/No fertility:
            else {
                Render.text("Although the numbness makes you feel serene, the trap oil has no effect upon your ");
                if (player.fertility > 0) Render.text("mostly ");
                Render.text("sterile system.");
                //[Low/No fertility + Trap/Corruption  >70:
                if (player.stats.cor > 70) Render.text("  For some reason the fact that you cannot  as nature intended makes you feel helpless and submissive.  Perhaps the only way to be a useful creature now is to find a dominant, fertile being willing to plow you full of eggs? You shake the alien, yet oddly alluring thought away.");
            }
        ...
        if (player.lowerBody.vaginaSpot.hasVagina() && player.vaginaType() != 5 && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            //(Wet:
            if (player.wetness() >= 3) Render.text("  Your natural lubrication makes it gleam invitingly.");
            //(Corruption <50:
            if (player.stats.cor < 50) Render.text("  After a few cautious touches you decide it doesn't feel any different- it does certainly look odd, though.");
            else Render.text("  After a few cautious touches you decide it doesn't feel any different - the sheer bizarreness of it is a big turn on though, and you feel it beginning to shine with anticipation at the thought of using it.");
            Render.text("  <b>Your vagina is now ebony in color.</b>");
            dynStats("sen", 2, "lus", 10);
            player.vaginaType(5);
            changes++;
        }
    ...
    public purityPeach(player: Player): void {
        clearOutput();
        Render.text("You bite into the sweet, juicy peach, feeling a sensation of energy sweeping through your limbs and your mind.  You feel revitalized, refreshed, and somehow cleansed.");
        fatigue(-15);
    ...
    public purpleFruitEssrayle(player: Player): void {
        ...
        if (player.averageNipplesPerBreast() < 4) {
            Render.text("  At first you think nothing has changed, but a second look confirms that your breasts now sport the same quartet of cow-like nipples the bovine plant-girl bears.");
    ...
    public ringtailFig(player: Player): void {
        clearOutput();
        //eat it:
        Render.text("You split the fruit and scoop out the pulp, eating it greedily.  It's sweet and slightly gritty with seeds, and you quickly finish both halves.");
        ...
        if (Utils.rand(4) == 0 && player.thickness < 80 && changes < changeLimit) {
            Render.text(player.modThickness(80, 2), false);
            changes++;
        }
        //bodypart changes:
        ...
        if (player.upperBody.head.earType == EARS.RACCOON && player.lowerBody != LOWER_BODY.RACCOON && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            //from hooves or hard feet, including centaurs and bees
                ...
                if (player.lowerBody == LOWER_BODY.HOOFED || player.lowerBody == LOWER_BODY.CENTAUR || player.lowerBody == LOWER_BODY.PONY) Render.text("pops right out of its hoof just in time");
                else Render.text("comes loose inside its long boot, and you pull it free with irritation only");
            ...
            //from human, demon, paw feet
                ...
                if ((player.lowerBody == LOWER_BODY.HUMAN || player.lowerBody == LOWER_BODY.DEMONIC_HIGH_HEELS || player.lowerBody == LOWER_BODY.DEMONIC_CLAWS) && player.skinType != SKIN.FUR) Render.text("The sensation of walking around on what feels like a second pair of hands is so weird that you miss noticing the itchy fur growing in over your legs...  ");
                Render.text("<b>You now have raccoon paws!</b>");
            }
        ...
        if (player.faceType != FACE.RACCOON_MASK && player.faceType != FACE.RACCOON && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.faceType == FACE.HUMAN || player.faceType == FACE.SHARK_TEETH || player.faceType == FACE.SNAKE_FANGS || player.faceType == FACE.BUNNY) {
                ...
                if (player.faceType == FACE.SHARK_TEETH || player.faceType == FACE.SNAKE_FANGS || player.faceType == FACE.BUNNY) {
                    Render.text("Your prominent teeth chatter noisily at first, then with diminishing violence, until you can no longer feel them jutting past the rest!  ");
                }
                Render.text("Shaking your head a bit, you wait for your energy to return, then examine your appearance.  ");
                ...
                if (((player.skinTone == "ebony" || player.skinTone == "black") && (player.skinType == SKIN.PLAIN || player.skinType == SKIN.GOO)) || ((player.hairColor == "black" || player.hairColor == "midnight") && (player.skinType == SKIN.FUR || player.skinType == SKIN.SCALES))) {
                    Render.text("Nothing seems different at first.  Strange... you look closer and discover a darker, mask-line outline on your already inky visage.  <b>You now have a barely-visible raccoon mask.</b>");
                }
                else Render.text("A dark, almost black mask shades the " + player.skinFurScales() + " around your eyes and over the topmost portion of your nose, lending you a criminal air!  <b>You now have a raccoon mask!</b>");
            }
            //from snout (will not overwrite full-coon snout but will overwrite others)
                ...
                if (((player.hairColor == "black" || player.hairColor == "midnight") && (player.skinType == SKIN.FUR || player.skinType == SKIN.SCALES))) Render.text("your face apparently returned to normal shape, albeit still covered in " + player.skinFurScales() + ".  You look closer and discover a darker, mask-line outline on your already inky visage.  <b>You now have a barely-visible raccoon mask on your otherwise normal human face.</b>");
                else if ((player.skinTone == "ebony" || player.skinTone == "black") && (player.skinType == SKIN.PLAIN || player.skinType == SKIN.GOO)) Render.text("your face apparently returned to normal shape.  You look closer and discover a darker, mask-line outline on your already inky visage.  <b>You now have a barely-visible raccoon mask on your normal human face.</b>");
                else Render.text("your face returned to human dimensions, but shaded by a black mask around the eyes and over the nose!  <b>You now have a humanoid face with a raccoon mask!</b>");
            }
    ...
    public mouseCocoa(player: Player): void {
        ...
        //use:
        Render.text("You pop several of the beans in your mouth and suck; they immediately reward you by giving up an oily, chocolatey flavor with a hint of bitterness.  For several minutes you ");
        if (!player.isTaur()) Render.text("sit and ");
        Render.text("enjoy the taste.");
        ...
        if (player.tallness < 60 && (player.analCapacity() < 100 || (player.vaginalCapacity() < 100 && player.lowerBody.vaginaSpot.hasVagina())) && changes < changeLimit && Utils.rand(3) == 0) {
            Render.text("\n\nYour ");
            if (player.vaginalCapacity() < 100 && player.lowerBody.vaginaSpot.hasVagina()) Render.text("[vagina]");
            else Render.text("[asshole]");
            Render.text(" itches, and you shyly try to scratch it, looking around to see if you're watched.  ");
            if (player.isTaur()) Render.text("Backing up to a likely rock, you rub your hindquarters against it, only to be surprised when you feel your hole part smoothly against the surface, wider than you're used to!");
            else Render.text("Slipping a hand in your [armor], you rub vigorously; your hole opens more easily and your fingers poke in farther than you're used to!");
            Render.text("  It feels unusual - not bad, really, but definitely weird.  You can see how it would come in handy, now that you're smaller than most prospective partners, but... shaking your head, you ");
            if (player.isTaur()) Render.text("back away from your erstwhile sedimentary lover");
            else Render.text("pull your hand back out");
            Render.text(".");
            //adds some lust
        ...
        if (player.upperBody.head.earType != EARS.MOUSE && changes < changeLimit && Utils.rand(4) == 0) {
            Render.text("\n\nYour ears ");
            if (player.upperBody.head.earType == EARS.HORSE || player.upperBody.head.earType == EARS.COW || player.upperBody.head.earType == EARS.DOG || player.upperBody.head.earType == EARS.BUNNY || player.upperBody.head.earType == EARS.KANGAROO) Render.text("shrink suddenly");
            else Render.text("pull away from your head");
            Render.text(", like they're being pinched, and you can distinctly feel the auricles taking a rounded shape through the pain.  Reaching up to try and massage away their stings, <b>you're not terribly surprised when you find a pair of fuzzy mouse's ears poking through your " + hairDescript() + ".</b>");
        ...
        if (player.upperBody.head.earType == EARS.MOUSE && player.tailType != TAIL.MOUSE && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            if (player.tailType > TAIL.NONE) {
                ...
                if (player.tailType == TAIL.HORSE) Render.text("elongating");
                else Render.text("compressing");
                Render.text(" into a long, thin line.  With a shudder, it begins to shed until it's completely, starkly nude.  <b>Your new mouse tail looks a bit peaked.</b>");
            }
        ...
        if (player.upperBody.head.earType == EARS.MOUSE && (player.faceType == FACE.HUMAN || player.faceType == FACE.SHARK_TEETH || player.faceType == FACE.BUNNY || player.faceType == FACE.SPIDER_FANGS || player.faceType == FACE.RACCOON_MASK) && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.faceType != FACE.HUMAN) Render.text("the sharp teeth receding and ");
            Render.text("your incisors lengthening.  It's not long before they're twice as long as their neighbors and the obvious growth stops, but the pressure doesn't go away completely.  <b>Well, you now have mouse incisors and your face aches a tiny bit - wonder if they're going to keep growing?</b>");
        ...
        if ((player.skinType != SKIN.FUR || (player.skinType == SKIN.FUR && (player.hairColor != "brown" && player.hairColor != "white"))) && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            if (player.skinType != SKIN.FUR) {
                ...
                if (player.skinType > SKIN.PLAIN) Render.text("es");
                Render.text(" all over");
                if (player.tailType > TAIL.NONE) Render.text(", except on your tail");
                Render.text(".  Alarmed and suspicious, you tuck in your hands, trying to will yourself not to scratch, but it doesn't make much difference.  Tufts of ");
                temp = Utils.rand(10);
                if (temp < 8) {
                    Render.text("brown");
                ...
                Render.text(" fur begin to force through your skin");
                if (player.skinType == SKIN.SCALES) Render.text(", pushing your scales out with little pinches");
                Render.text(", resolving the problem for you.  <b>You now have fur.</b>");
            }
            //from other color fur
                ...
                Render.text(" fuzz coming in behind it that soon grows to full-fledged fur.");
            }
            player.skinAdj = "";
            player.skinDesc = "fur";
            player.skinType = SKIN.FUR;
    ...
    private demonChanges(player: Player): void {
        ...
        if (player.tailType != TAIL.DEMONIC && player.horns > 0) {
            if (player.tailType != TAIL.NONE) {
                Render.text("\n\n", false);
                if (player.tailType == TAIL.SPIDER_ADBOMEN || player.tailType == TAIL.BEE_ABDOMEN) Render.text("You feel a tingling in your insectile abdomen as it stretches, narrowing, the exoskeleton flaking off as it transforms into a flexible demon-tail, complete with a round spaded tip.  ", false);
                else Render.text("You feel a tingling in your tail.  You are amazed to discover it has shifted into a flexible demon-tail, complete with a round spaded tip.  ", false);
                Render.text("<b>Your tail is now demonic in appearance.</b>", false);
            }
        ...
        if (player.horns == 0 || (Utils.rand(player.horns + 3) == 0)) {
            if (player.horns < 12 && (player.hornType == HORNS.NONE || player.hornType == HORNS.DEMON)) {
                Render.text("\n\n", false);
                if (player.horns == 0) {
                    Render.text("A small pair of demon horns erupts from your forehead.  They actually look kind of cute.  <b>You have horns!</b>", false);
                }
                else Render.text("Another pair of demon horns, larger than the last, forms behind the first row.", false);
            ...
            //Text for shifting horns
                ...
                Render.text("\n\n", false);
                Render.text("Your horns shift, shrinking into two small demonic-looking horns.", false);
                player.horns = 2;
        ...
        if ((player.faceType != FACE.HUMAN || player.skinType != SKIN.PLAIN) && Utils.rand(3) == 0) {
            ...
            if (player.faceType != FACE.HUMAN) {
                Render.text("\n\n", false);
                Render.text("Your visage twists painfully, returning to a more normal human shape, albeit with flawless skin.  <b>Your face is human again!</b>", false);
                player.faceType = FACE.HUMAN;
            }
            //De-fur
            else if (player.skinType != SKIN.PLAIN) {
                Render.text("\n\n", false);
                if (player.skinType == SKIN.FUR) Render.text("Your skin suddenly feels itchy as your fur begins falling out in clumps, <b>revealing inhumanly smooth skin</b> underneath.", false);
                if (player.skinType == SKIN.SCALES) Render.text("Your scales begin to itch as they begin falling out in droves, <b>revealing your inhumanly smooth " + player.skinTone + " skin</b> underneath.", false);
                player.skinType = SKIN.PLAIN;
                player.skinDesc = "skin";
            }
        }
        ...
        if (player.upperBody.wingType != WING.BAT_LIKE_LARGE && Utils.rand(8) == 0 && player.stats.cor >= 50) {
            ...
            if (player.upperBody.wingType == WING.BAT_LIKE_TINY && player.stats.cor >= 75) {
                Render.text("\n\n", false);
                Render.text("Your small demonic wings stretch and grow, tingling with the pleasure of being attached to such a tainted body.  You stretch over your shoulder to stroke them as they unfurl, turning into full-sized demon-wings.  <b>Your demonic wings have grown!</b>", false);
                ...
                Render.text("\n\n", false);
                Render.text("The muscles around your shoulders bunch up uncomfortably, changing to support the new bat-like wings growing from your back.  You twist your head as far as you can for a look and realize your fin has changed into ", false);
                Render.text("small ", false);
                player.upperBody.wingType = WING.BAT_LIKE_TINY;
                player.upperBody.wingDesc = "tiny, bat-like";
                Render.text("bat-like demon-wings!", false);
            }
            else if (player.upperBody.wingType == WING.BEE_LIKE_SMALL || player.upperBody.wingType == WING.BEE_LIKE_LARGE) {
                Render.text("\n\n", false);
                Render.text("The muscles around your shoulders bunch up uncomfortably, changing to support your wings as you feel their weight increasing.  You twist your head as far as you can for a look and realize they've changed into ", false);
                if (player.upperBody.wingType == WING.BEE_LIKE_SMALL) {
                    ...
                    Render.text("large ", false);
                    player.upperBody.wingType = WING.BAT_LIKE_LARGE;
                    player.upperBody.wingDesc = "large, bat-like";
                }
                Render.text("<b>bat-like demon-wings!</b>", false);
            }
            //No wings
            else if (player.upperBody.wingType == WING.NONE) {
                Render.text("\n\n", false);
                Render.text("A knot of pain forms in your shoulders as they tense up.  With a surprising force, a pair of small demonic wings sprout from your back, ripping a pair of holes in the back of your " + player.armorName + ".  <b>You now have tiny demonic wings</b>.", false);
                player.upperBody.wingType = WING.BAT_LIKE_TINY;
                player.upperBody.wingDesc = "tiny, bat-like";
            }
    ...
    public herbalContraceptive(player: Player): void {
        ...
        // Placeholder, sue me
        Render.text("You chew on the frankly awfully bitter leaves as quickly as possible before swallowing them down.");
    ...
    public princessPucker(player: Player): void {
        ...
        Render.text("You uncork the bottle, and sniff it experimentally.  The fluid is slightly pink, full of flecks of gold, and smelling vaguely of raspberries.  Princess Gwynn said it was drinkable.\n\n");

        Render.text("You down the bottle, hiccuping a bit at the syrupy-sweet raspberry flavor.  Immediately following the sweet is a bite of sour, like sharp lime.  You pucker your lips, and feel your head clear a bit from the intensity of flavor.  You wonder what Gwynn makes this out of.\n\n");

        Render.text("Echoing the sensation in your head is an answering tingle in your body.  The sudden shock of citrusy sour has left you slightly less inclined to fuck, a little more focused on your priorities.\n\n");
        ...
        if (player.hairColor != "pink") {
            if (Utils.rand(5) == 0) {
                Render.text("A slight tingle across your scalp draws your attention to your hair.  It seems your " + player.hairColor + " is rapidly gaining a distinctly pink hue, growing in from the roots!\n\n");
    ...
    public ferretTF(player: Player): void {
        ...
        clearOutput();
        Render.text("Feeling parched, you gobble down the fruit without much hesitation. Despite the skin being fuzzy like a peach, the inside is relatively hard, and its taste reminds you of that of an apple.  It even has a core like an apple. Finished, you toss the core aside.");
            ...
            if (temp >= 0 && Utils.rand(2) == 0 && changes < changeLimit) {
                if (player.lowerBody.cockSpot.list[temp].cockLength > 6 && !flags[FlagEnum.HYPER_HAPPY]) {
                    ...
                    if (player.lowerBody.cockSpot.list[temp].cockLength / 6 >= player.lowerBody.cockSpot.list[temp].cockThickness) {
                        Render.text("  Luckily, it doen’t seem to have lost its previous thickness.");
                    }
                    else {
        ...
        if (player.skinType != SKIN.FUR && player.upperBody.head.earType == EARS.FERRET && player.tailType == TAIL.FERRET && player.lowerBody == LOWER_BODY.FERRET && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            Render.text("  <b>You now have " + player.hairColor + " fur!</b>");
            player.skinType = SKIN.FUR;
            changes++;
        }
        //Tail TFs!
