...
export default class WhiskerFruit extends Consumable {
    ...
    public use(player: Player) {
        ...
        //Text go!
        DisplayText.clear();
        DisplayText.text("You take a bite of the fruit and gulp it down. It's thick and juicy and has an almost overpowering sweetness. Nevertheless, it is delicious and you certainly could use a meal.  You devour the fruit, stopping only when the hard, nubby pit is left; which you toss aside.");
        ...
        if (player.stats.lib < 80 && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            if (player.lowerBody.cockSpot.countType(CockType.CAT) > 0) {
                ...
                if (player.stats.cor < 33) DisplayText.text("You need to control yourself better.");
                else if (player.stats.cor < 66) DisplayText.text("You're not sure how you feel about the fantasy.");
                else DisplayText.text("You hope to find a willing partner to make this a reality.");
            }
            //Else �
            else {
                ...
                if (player.stats.lust > 60) DisplayText.text("even more ");
                DisplayText.text("turned on.");
            }
            player.stats.lib += 1;
        ...
        if (Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.goIntoHeat()) {
                if (intensified) {
                    if (Utils.rand(2) == 0) DisplayText.text("\n\nThe itch inside your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " is growing stronger, and you desperately want to find a nice cock to massage the inside.");
                    ...
                    DisplayText.text("\n\nThe interior of your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " clenches tightly, squeezing with reflexive, aching need.  Your skin flushes hot ");
                    if (player.skinType == SkinType.FUR) DisplayText.text("underneath your fur ");
                    DisplayText.text("as images and fantasies ");
                    if (player.stats.cor < 50) DisplayText.text("assault ");
                    else DisplayText.text("fill ");
                ...
                for (let index: number = 0; index < player.upperBody.chest.count(); index++) {
                    ...
                    if (selectedBreastRow.breastRating > breastShrinkageThreshold) {
                        ...
                        if (selectedBreastRow.breastRating > 10) {
                            ...
                            if (!shrinkingHappened) DisplayText.text("\n\nThe " + BreastDescriptor.describeBreastRow(selectedBreastRow) + " on your chest wobble for a second, then tighten up, losing several cup-sizes in the process!");
                            else DisplayText.text("  The change moves down to your " + Utils.numToOrdinalText(index + 1) + " row of " + BreastDescriptor.describeBreastRow(selectedBreastRow) + ". They shrink greatly, losing a couple cup-sizes.");
                        }
                        //Small change
                        else {
                            ...
                            else DisplayText.text("  Your " + Utils.numToOrdinalText(index + 1) + " row of " + BreastDescriptor.describeBreastRow(selectedBreastRow) + " gives a tiny jiggle as it shrinks, losing some off its mass.");
                        }
            ...
            for (let index: number = 0; index < player.lowerBody.cockSpot.count(); index++) {
                ...
                if (selectedCock.cockType == CockType.CAT) {
                    ...
                    if (!selectedCock.hasSheath()) {
                        DisplayText.text("Then, it begins to shrink and sucks itself inside your body.  Within a few moments, a fleshy sheath is formed.");
                        if (player.lowerBody.balls > 0) DisplayText.text("  Thankfully, your balls appear untouched.");
                    }
                    else DisplayText.text("Then, it disappears back into your sheath.");
        ...
        if (player.lowerBody.cockSpot.countType(CockType.CAT) > 0 && Utils.rand(3) == 0 && changes < changeLimit && !Flags.list[FlagEnum.HYPER_HAPPY]) {
            ...
            for (let index: number = 0; index < player.lowerBody.cockSpot.count(); index++) {
                ...
                if (selectedCock.cockType == CockType.CAT && selectedCock.cockLength > 6) {
                    ...
                    if (selectedCock.cockLength > 16) {
                        if (changedCock == 0)
                            DisplayText.text("\n\nYour " + CockDescriptor.describeCock(player, selectedCock) + " tingles, making your sheath feel a little less tight.  It dwindles in size, losing a full third of its length and a bit of girth before the change finally stops.");
                        selectedCock.cockLength *= .66;
                        changedCock++;
                    }
                    else if (selectedCock.cockLength > 6) {
                        if (changedCock == 0)
                            DisplayText.text("\n\nYour " + CockDescriptor.describeCock(player, selectedCock) + " tingles and withdraws further into your sheath.  If you had to guess, you'd say you've lost about two inches of total length and perhaps some girth.");
            ...
            DisplayText.text("  Although the package is smaller, it feels even more sensitive � as if it retained all sensation of its larger size in its smaller form.");
            player.stats.sens += 5;
            //Make note of other dicks changing
            if (changedCock > 1) DisplayText.text("  Upon further inspection, all your " + CockDescriptor.nounCock(CockType.CAT) + "s have shrunk!");
            changes++;
        }
        ...
        if (player.lowerBody.tailType != TailType.CAT && player.upperBody.head.earType == EarType.CAT && Utils.rand(5) == 0 && changes < changeLimit) {
            if (player.lowerBody.tailType == TailType.NONE) {
                ...
                if (chance == 0) DisplayText.text("\n\nA pressure builds in your backside. You feel under your " + player.inventory.armorSlot.equipment.displayName + " and discover an odd bump that seems to be growing larger by the moment. In seconds it passes between your fingers, bursts out the back of your clothes and grows most of the way to the ground. A thick coat of fur springs up to cover your new tail. You instinctively keep adjusting it to improve your balance. <b>You now have a cat-tail.</b>");
                if (chance == 1) DisplayText.text("\n\nYou feel your backside shift and change, flesh molding and displacing into a long, flexible tail! <b>You now have a cat tail.</b>");
                if (chance == 2) DisplayText.text("\n\nYou feel an odd tingling in your spine and your tail bone starts to throb and then swell. Within a few moments it begins to grow, adding new bones to your spine. Before you know it, you have a tail. Just before you think it's over, the tail begins to sprout soft, glossy " + player.upperBody.head.hairColor + " fur. <b>You now have a cat tail.</b>");
            }
        ...
        if (player.lowerBody.tailType == TailType.CAT && player.upperBody.head.earType == EarType.CAT && Utils.rand(5) == 0 && changes < changeLimit && player.lowerBody.type != LowerBodyType.CAT) {
            ...
            if (player.lowerBody.type == LowerBodyType.HOOFED || player.lowerBody.type == LowerBodyType.CENTAUR) {
                ...
                if (player.lowerBody.type == LowerBodyType.CENTAUR) DisplayText.text("  You feel woozy and collapse on your side.  When you wake, you're no longer a centaur and your body has returned to a humanoid shape.");
            }
            //Goo to cat
        ...
        if (player.lowerBody.tailType == TailType.CAT &&
            ...
            else if (chance == 1) DisplayText.text("\n\nMind-numbing pain courses through you as you feel your facial bones rearranging.  You clutch at your face in agony as your skin crawls and shifts, your visage reshaping to replace your facial playeristics with those of a feline. <b>You now have an anthropomorphic cat-face.</b>");
