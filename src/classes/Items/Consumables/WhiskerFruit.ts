...
export default class WhiskerFruit extends Consumable {
    ...
    public use(character: Character) {
        ...
        if (character.stats.spe < 75 && Utils.rand(3) === 0 && changes < changeLimit) {
            // low speed
        ...
        if (character.stats.str < 40 && Utils.rand(3) === 0 && changes < changeLimit) {
        ...
        else if (character.stats.str > 60 && Utils.rand(2) === 0) {
        ...
        // Does not add to change total
        if (character.stats.tou > 50 && Utils.rand(2) === 0) {
        ...
        // Libido gain
        if (character.stats.lib < 80 && changes < changeLimit && Utils.rand(4) === 0) {
            // Cat dicked folks
        ...
        if (character.torso.chest.count > 0 && changes < changeLimit && Utils.rand(4) === 0 && !Flags.list[FlagEnum.HYPER_HAPPY]) {
        ...
        // Cat penorz shrink
        if (character.torso.cocks.filter(Cock.FilterType(CockType.CAT)).length > 0 && Utils.rand(3) === 0 && changes < changeLimit && !Flags.list[FlagEnum.HYPER_HAPPY]) {
        ...
        // DA EARZ
        if (character.torso.neck.head.ears.type !== EarType.CAT && Utils.rand(5) === 0 && changes < changeLimit) {
            // human to cat:
        ...
        if (!character.torso.tails.reduce(Tail.HasType(TailType.CAT), false) && character.torso.neck.head.ears.type === EarType.CAT && Utils.rand(5) === 0 && changes < changeLimit) {
        ...
        if (character.torso.tails.reduce(Tail.HasType(TailType.CAT), false) && character.torso.neck.head.ears.type === EarType.CAT && Utils.rand(5) === 0 && changes < changeLimit && character.torso.hips.legs.type !== LegType.CAT) {
            // hoof to cat:
        ...
        if (character.torso.tails.reduce(Tail.HasType(TailType.CAT), false) &&
            ...
            DisplayText("\n\nYour " + character.skin.desc + " begins to tingle, then itch. You reach down to scratch your arm absent-mindedly and pull your fingers away to find stUtils.Utils.rands of " + character.torso.neck.head.hair.color + " fur. Wait, fur?  What just happened?! You spend a moment examining yourself and discover that <b>you are now covered in glossy, soft fur.</b>\n\n");
            character.skin.type = SkinType.FUR;
        ...
        if (changes < changeLimit) {
            if (Utils.rand(2) === 0) DisplayText(BodyModifier.displayModThickness(character, 5, 2));
            if (Utils.rand(2) === 0) DisplayText(BodyModifier.displayModTone(character, 76, 2));
            if (character.gender < 2) if (Utils.rand(2) === 0) DisplayText(BodyModifier.displayModFem(character, 65, 1));
