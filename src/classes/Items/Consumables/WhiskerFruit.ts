    ...
    public use(player: Player) {
        ...
        if (player.stats.lib < 80 && changes < changeLimit && Utils.rand(4) === 0) {
            ...
            // Else �
                ...
                if (player.stats.lust > 60) DisplayText("even more ");
        ...
        if (Utils.rand(4) === 0 && changes < changeLimit) {
            ...
            if (player.goIntoHeat()) {
                if (intensified) {
                    if (Utils.rand(2) === 0) DisplayText("\n\nThe itch inside your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " is growing stronger, and you desperately want to find a nice cock to massage the inside.");
                    else DisplayText("\n\nThe need inside your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " grows even stronger.  You desperately need to find a mate to 'scratch your itch' and fill your womb with kittens.  It's difficult NOT to think about a cock slipping inside your moist fuck-tunnel, and at this point you'll have a hard time resisting ANY male who approaches.");
                }
                else {
                    DisplayText("\n\nThe interior of your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " clenches tightly, squeezing with reflexive, aching need.  Your skin flushes hot ");
                    if (player.skin.type === SkinType.FUR) DisplayText("underneath your fur ");
                    DisplayText("as images and fantasies ");
                    if (player.stats.cor < 50) DisplayText("assault ");
        ...
        if (player.torso.chest.count > 0 && changes < changeLimit && Utils.rand(4) === 0 && !Flags.list[FlagEnum.HYPER_HAPPY]) {
            let breastShrinkageThreshold: number = 0;
            let shrinkingHappened: boolean = false;
            // Determine if shrinkage is required
            if (player.torso.vaginas.count <= 0 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 2) breastShrinkageThreshold = 2;
            else if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 4) breastShrinkageThreshold = 4;
            // IT IS!
                ...
                for (let index: number = 0; index < player.torso.chest.count; index++) {
                    ...
                    if (selectedBreastRow.rating > breastShrinkageThreshold) {
                        ...
                        if (selectedBreastRow.rating > 10) {
                            selectedBreastRow.rating -= 2 + Utils.rand(3);
                            ...
                            else DisplayText("  The change moves down to your " + Utils.numToOrdinalText(index + 1) + " row of " + BreastDescriptor.describeBreastRow(selectedBreastRow) + ". They shrink greatly, losing a couple cup-sizes.");
                        }
                        // Small change
                        else {
                            ...
                            else DisplayText("  Your " + Utils.numToOrdinalText(index + 1) + " row of " + BreastDescriptor.describeBreastRow(selectedBreastRow) + " gives a tiny jiggle as it shrinks, losing some off its mass.");
                        }
            ...
            for (let index: number = 0; index < player.torso.cocks.count; index++) {
                ...
                if (selectedCock.type === CockType.CAT) {
                    ...
                    if (!selectedCock.hasSheath()) {
                        ...
                        if (player.torso.balls.quantity > 0) DisplayText("  Thankfully, your balls appear untouched.");
                    }
        ...
        // DA EARZ
        if (player.torso.neck.head.ears.type !== EarType.CAT && Utils.rand(5) === 0 && changes < changeLimit) {
            // human to cat:
        ...
        if (!player.torso.tails.reduce(Tail.HasType(TailType.CAT), false) && player.torso.neck.head.ears.type === EarType.CAT && Utils.rand(5) === 0 && changes < changeLimit) {
            if (player.torso.tails.count === 0) {
                ...
                if (chance === 2) DisplayText("\n\nYou feel an odd tingling in your spine and your tail bone starts to throb and then swell. Within a few moments it begins to grow, adding new bones to your spine. Before you know it, you have a tail. Just before you think it's over, the tail begins to sprout soft, glossy " + player.torso.neck.head.hair.color + " fur. <b>You now have a cat tail.</b>");
            }
        ...
        if (player.torso.tails.reduce(Tail.HasType(TailType.CAT), false) && player.torso.neck.head.ears.type === EarType.CAT && Utils.rand(5) === 0 && changes < changeLimit && player.torso.hips.legs.type !== LegType.CAT) {
            ...
            if (player.torso.hips.legs.type === LegType.HOOFED || player.torso.hips.legs.type === LegType.CENTAUR) {
                ...
                if (player.torso.hips.legs.type === LegType.CENTAUR) DisplayText("  You feel woozy and collapse on your side.  When you wake, you're no longer a centaur and your body has returned to a humanoid shape.");
            }
            // Goo to cat
        ...
        if (player.torso.tails.reduce(Tail.HasType(TailType.CAT), false) &&
            ...
            DisplayText("\n\nYour " + player.skin.desc + " begins to tingle, then itch. You reach down to scratch your arm absent-mindedly and pull your fingers away to find stUtils.Utils.rands of " + player.torso.neck.head.hair.color + " fur. Wait, fur?  What just happened?! You spend a moment examining yourself and discover that <b>you are now covered in glossy, soft fur.</b>\n\n");
            player.skin.type = SkinType.FUR;
        ...
        if (changes < changeLimit) {
            if (Utils.rand(2) === 0) DisplayText(BodyModifier.displayModThickness(player, 5, 2));
            if (Utils.rand(2) === 0) DisplayText(BodyModifier.displayModTone(player, 76, 2));
            if (player.gender < 2) if (Utils.rand(2) === 0) DisplayText(BodyModifier.displayModFem(player, 65, 1));
            else DisplayText(BodyModifier.displayModFem(player, 85, 2));
        }
    }
}
