...
export default class Reptilum extends Consumable {
    ...
    public use(character: Character) {
        ...
        if (character.stats.spe > 50 && changes < changeLimit && Utils.rand(4) === 0) {
            DisplayText("\n\nYou start to feel sluggish and cold.  Lying down to bask in the sun might make you feel better.");
            character.stats.spe += -1;
            changes++;
        }
        // -Reduces sensitivity.
        if (character.stats.sens > 20 && changes < changeLimit && Utils.rand(3) === 0) {
        ...
        if (character.stats.lib < 100 && changes < changeLimit && Utils.rand(3) === 0) {
        ...
        if (character.stats.tou < 70 && changes < changeLimit && Utils.rand(3) === 0) {
            // (+3)
        ...
        if (character.torso.cocks.filter(Cock.FilterType(CockType.LIZARD)).length <= 0 && character.torso.cocks.count > 0 && changes < changeLimit && Utils.rand(4) === 0) {
        ...
        if (character.torso.cocks.count > 1 && character.torso.cocks.filter(Cock.FilterType(CockType.LIZARD)).length > 0 && character.torso.cocks.count > character.torso.cocks.filter(Cock.FilterType(CockType.LIZARD)).length && Utils.rand(4) === 0 && changes < changeLimit) {
        ...
        if (character.torso.cocks.filter(Cock.FilterType(CockType.LIZARD)).length === 1 && character.torso.cocks.count === 1 && Utils.rand(4) === 0 && changes < changeLimit) {
        ...
        if (character.torso.cocks.filter(Cock.FilterType(CockType.LIZARD)).length === character.torso.cocks.count && changes < changeLimit && character.statusAffects.has(StatusAffectType.Infested)) {
            ...
            if (character.torso.balls.quantity > 1) DisplayText("  The remaining " + Utils.numToCardinalText(character.torso.balls.quantity - 1) + " slither out the pre-stretched holes with ease, though the last one hangs from your tip for a moment before dropping to the ground.");
        ...
        if (character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1 && character.gender === 1 && changes < changeLimit && Utils.rand(3) === 0) {
            // (HUEG)
        ...
        // -Lactation stoppage.
        if (character.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 1 && changes < changeLimit && Utils.rand(4) === 0) {
        ...
        // -VAGs
        if (character.torso.vaginas.count > 0 && !character.perks.has(PerkType.Oviposition) && changes < changeLimit && Utils.rand(5) === 0 && RaceScore.lizardScore(character) > 3) {
        ...
        if (character.torso.neck.head.horns.type !== HornType.DRACONIC_X4_12_INCH_LONG && changes < changeLimit && Utils.rand(5) === 0) {
                ...
                if (character.torso.neck.head.horns.type === HornType.DRACONIC_X2) {
                    if (character.torso.neck.head.horns.amount < 12) {
                        if (Utils.rand(2) === 0) {
                            ...
                            character.torso.neck.head.horns.amount += 2 + Utils.rand(4);
                        }
        ...
        if (character.torso.hips.legs.type !== LegType.LIZARD && changes < changeLimit && Utils.rand(5) === 0) {
            // Hooves -
        ...
        if (character.torso.tails.reduce(Tail.HasType(TailType.LIZARD), false) && character.torso.hips.legs.type === LegType.LIZARD && changes < changeLimit && Utils.rand(5) === 0) {
            // No tail
        ...
        if (character.torso.neck.head.ears.type !== EarType.LIZARD && character.torso.tails.reduce(Tail.HasType(TailType.LIZARD), false) && character.torso.hips.legs.type === LegType.LIZARD && changes < changeLimit && Utils.rand(5) === 0) {
        ...
        if (character.skin.type !== SkinType.SCALES && character.torso.neck.head.ears.type === EarType.LIZARD && character.torso.tails.reduce(Tail.HasType(TailType.LIZARD), false) && character.torso.hips.legs.type === LegType.LIZARD && changes < changeLimit && Utils.rand(5) === 0) {
            // (fur)
            if (character.skin.type === SkinType.FUR) {
                // set new skin.tone
                if (Utils.rand(10) === 0) {
                    if (Utils.rand(2) === 0) character.skin.tone = "purple";
        ...
        // -Lizard-like face.
        if (character.torso.neck.head.face.type !== FaceType.LIZARD && character.skin.type === SkinType.SCALES && character.torso.neck.head.ears.type === EarType.LIZARD && character.torso.tails.reduce(Tail.HasType(TailType.LIZARD), false) && character.torso.hips.legs.type === LegType.LIZARD && changes < changeLimit && Utils.rand(5) === 0) {
