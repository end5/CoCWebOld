    ...
    public use(character: Character) {
        ...
        if (chance <= 75) {
            ...
            if (character.torso.chest.count > 0) {
                if (character.torso.chest.get(0).rating < 2 && Utils.rand(3) === 0) breastGrowth++;
                if (character.torso.chest.get(0).rating < 5 && Utils.rand(4) === 0) breastGrowth++;
                if (character.torso.chest.get(0).rating < 6 && Utils.rand(5) === 0) breastGrowth++;
            }
            DisplayText("\n\n");
            if (character.torso.chest.count === 0) {
            ...
            if (!Flags.list[FlagEnum.HYPER_HAPPY]) {
                ...
                if (character.torso.cocks.count > 0) {
                    ...
                    lengthenAmount += CockModifier.growCock(character, longestCock, (Utils.rand(3) + 1) * -1);
                    DisplayText("\n\n");
        ...
        if (character.torso.vaginas.count === 0 && (Utils.rand(3) === 0 || (chance > 75 && chance < 90))) {
        ...
        // Increase pussy wetness or grow one!!
            ...
            // Shrink cawk
            if (character.torso.cocks.count > 0 && !Flags.list[FlagEnum.HYPER_HAPPY]) {
                DisplayText("\n\n");
                const longestCock = character.torso.cocks.sort(Cock.LongestCocks)[0];
                // Shrink said cock
                if (longestCock.length < 6 && longestCock.length >= 2.9)
                    longestCock.length -= .5;
                const lengthChange: number = CockModifier.growCock(character, longestCock, -1 * (Utils.rand(3) + 1));
            ...
            if (character.skin.tone === "blue" || character.skin.tone === "purple" || character.skin.tone === "indigo" || character.skin.tone === "shiny black") {
                if (character.torso.vaginas.count > 0) {
                    ...
                    if (((character.perks.has(PerkType.BigClit)) && character.torso.clit.length < 6)
                        ...
                        character.torso.clit.length += (Utils.rand(4) + 2) / 10;
                    }
        ...
        if (Utils.rand(40) + character.stats.cor / 3 > 35 && this.tainted) GenericTransforms.demonChanges(character);
        if (this.tainted) {
            DisplayText(BodyModifier.displayModFem(character, 100, 2));
            if (Utils.rand(3) === 0) DisplayText(BodyModifier.displayModTone(character, 15, 2));
        }
        else {
            DisplayText(BodyModifier.displayModFem(character, 90, 1));
            if (Utils.rand(3) === 0) DisplayText(BodyModifier.displayModTone(character, 20, 2));
        }
