...
export default class FoxBerry extends Consumable {
    ...
    public use(player: Player) {
        ...
        if (player.upperBody.head.hairColor != "golden-blonde" && player.upperBody.head.hairColor != "reddish-orange" && player.upperBody.head.hairColor != "silver" && player.upperBody.head.hairColor != "white" && player.upperBody.head.hairColor != "red" && player.upperBody.head.hairColor != "black" && changes < changeLimit && Utils.rand(4) == 0) {
            let hairTemp: number = Utils.rand(10);
            if (hairTemp < 5) player.upperBody.head.hairColor = "reddish-orange";
            else if (hairTemp < 7) player.upperBody.head.hairColor = "red";
        ...
        //dog cocks!
        if (changes < changeLimit && Utils.rand(3) == 0 && player.lowerBody.cockSpot.countType(CockType.DOG) < player.lowerBody.cockSpot.count()) {
            const choices: Cock[] = player.lowerBody.cockSpot.listCockType(CockType.DOG);
            if (choices.length != 0) {
                ...
                selectedCock.cockType = CockType.DOG;
