...
export default class FoxBerry extends Consumable {
    ...
    public use(character: Character) {
        ...
        if (character.torso.neck.head.face.type === FaceType.FOX &&
            ...
            character.skin.type === SkinType.FUR && Utils.rand(3) === 0
        ) {
        ...
        if (changes < changeLimit && Utils.rand(3) === 0 && (character.stats.lib < 80 || character.stats.int < 80 || character.stats.sens < 80)) {
        ...
        if (changes < changeLimit && Utils.rand(3) === 0 && character.stats.str > 40) {
        ...
        if (changes < changeLimit && Utils.rand(3) === 0 && character.stats.tou > 30) {
        ...
        if (character.torso.neck.head.hair.color !== "golden-blonde" && character.torso.neck.head.hair.color !== "reddish-orange" && character.torso.neck.head.hair.color !== "silver" && character.torso.neck.head.hair.color !== "white" && character.torso.neck.head.hair.color !== "red" && character.torso.neck.head.hair.color !== "black" && changes < changeLimit && Utils.rand(4) === 0) {
            ...
            DisplayText("\n\nYour scalp begins to tingle, and you gently grasp a stUtils.Utils.rand of hair, pulling it out to check it.  Your hair has become " + character.torso.neck.head.hair.color + "!");
        }
        // [Adjust hips toward 10 � wide/curvy/flared]
        if (changes < changeLimit && Utils.rand(3) === 0 && character.torso.hips.rating !== 10) {
            // from narrow to wide
        ...
        if (changes < changeLimit && character.torso.neck.head.hair.type === 4 && Utils.rand(3) === 0) {
        ...
        if (character.torso.neck.head.hair.type !== 4 && (character.torso.neck.head.hair.length > 26 || character.torso.neck.head.hair.length < 16) && changes < changeLimit && Utils.rand(4) === 0) {
            if (character.torso.neck.head.hair.length < 16) {
                character.torso.neck.head.hair.length += 1 + Utils.rand(4);
                DisplayText("\n\nYou experience a tingling sensation in your scalp.  Feeling a bit off-balance, you discover your hair has lengthened, becoming " + Utils.numToCardinalText(Math.round(character.torso.neck.head.hair.length)) + " inches long.");
            }
            else {
                character.torso.neck.head.hair.length -= 1 + Utils.rand(4);
                DisplayText("\n\nYou experience a tingling sensation in your scalp.  Feeling a bit off-balance, you discover your hair has shed a bit of its length, becoming " + Utils.numToCardinalText(Math.round(character.torso.neck.head.hair.length)) + " inches long.");
            }
            changes++;
        }
        ...
        // dog cocks!
        if (changes < changeLimit && Utils.rand(3) === 0 && character.torso.cocks.filter(Cock.FilterType(CockType.DOG)).length < character.torso.cocks.count) {
        ...
        if (character.cumQ() < 5000 && Utils.rand(3) === 0 && changes < changeLimit && character.torso.cocks.count > 0) {
            let cumMultiplierChange: number = 2 + Utils.rand(4);
            // Lots of cum raises cum multiplier cap to 2 instead of 1.5
            if (character.perks.has(PerkType.MessyOrgasms)) cumMultiplierChange += Utils.rand(10);
        ...
        if (changes < changeLimit && character.torso.balls.quantity > 0 && character.torso.balls.size > 4 && Utils.rand(3) === 0) {
        ...
        // HEAT!
        if (character.statusAffects.get(StatusAffectType.Heat).value2 < 30 && Utils.rand(6) === 0 && changes < changeLimit) {
        ...
        // [Grow Fur]
        // FOURTH
        if ((this.enhanced || character.torso.hips.legs.type === LegType.FOX) && character.skin.type !== SkinType.FUR && changes < changeLimit && Utils.rand(4) === 0) {
            // from scales
        ...
        // [Grow Fox Legs]
        // THIRD
        if ((this.enhanced || character.torso.neck.head.ears.type === EarType.FOX) && character.torso.hips.legs.type !== LegType.FOX && changes < changeLimit && Utils.rand(5) === 0) {
        ...
        // Grow Fox Ears]
        // SECOND
        if ((this.enhanced || character.torso.tails.reduce(Tail.HasType(TailType.FOX), false)) && character.torso.neck.head.ears.type !== EarType.FOX && changes < changeLimit && Utils.rand(4) === 0) {
        ...
        // FIRST
        if (!character.torso.tails.reduce(Tail.HasType(TailType.FOX), false) && changes < changeLimit && Utils.rand(4) === 0) {
            // from no tail
        ...
        if (character.skin.type === SkinType.FUR && character.torso.neck.head.face.type !== FaceType.FOX && changes < changeLimit && Utils.rand(5) === 0) {
        ...
        if (character.tone > 40 && changes < changeLimit && Utils.rand(2) === 0) {
