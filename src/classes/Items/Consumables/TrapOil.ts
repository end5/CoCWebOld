...
export default class TrapOil extends Consumable {
    ...
    public use(player: Player) {
        ...
        //Testicle Reduction:
        if (player.lowerBody.balls > 0 && player.lowerBody.cockSpot.hasCock() && (player.lowerBody.ballSize > 1 || !player.statusAffects.has("Uniball")) && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            if (Utils.rand(4) == 0 && changes < changeLimit) {
                if (player.femininity < 70 && player.femininity >= 60) {
                    ...
                    if (!player.perks.has("Androgyny")) {
                        player.perks.add(new Perk("Androgyny", 0, 0, 0, 0));
        ...
        if (player.lowerBody.vaginaSpot.hasVagina() && player.lowerBody.vaginaSpot.get(0).vaginaType != VaginaType.BLACK_SAND_TRAP && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            //(Wet:
            if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 3) MainScreen.text("  Your natural lubrication makes it gleam invitingly.");
            //(Corruption <50:
