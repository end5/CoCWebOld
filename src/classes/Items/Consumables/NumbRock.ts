...
export default class NumbRock extends Consumable {
    ...
    public use(character: Character) {
        ...
        if (character.stats.lust >= 33) {
            ...
            character.stats.lust -= 20 + Utils.rand(40);
        }
        if (Utils.rand(5) === 0) {
            if (!character.statusAffects.has(StatusAffectType.Dysfunction)) {
                ...
                character.statusAffects.add(StatusAffectType.Dysfunction, 50 + Utils.rand(100), 0, 0, 0);
            }
            else {
                ...
                character.statusAffects.get(StatusAffectType.Dysfunction).value1 = 50 + Utils.rand(100);
            }
        }
        else if (Utils.rand(4) === 0 && character.stats.int > 15) {
            ...
            character.stats.int -= 1 + Utils.rand(5);
        }
        if (!character.perks.has(PerkType.ThickSkin) && character.stats.sens < 30 && Utils.rand(4) === 0) {
            DisplayText("Slowly, ");
