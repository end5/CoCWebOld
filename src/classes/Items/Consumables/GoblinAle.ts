...
export default class GoblinAle extends Consumable {
    ...
    public use(player: Player) {
        ...
        DisplayText.clear();
        DisplayText.text("You drink the ale, finding it to have a remarkably smooth yet potent taste.  You lick your lips and sneeze, feeling slightly tipsy.");
        ...
        if (changes < changeLimit && Utils.rand(3) == 0 && player.lowerBody.vaginaSpot.hasVagina() && player.statusAffects.get(StatusAffectType.BonusVCapacity).value1 < 40) {
            ...
            DisplayText.text("\n\nThere is a sudden... emptiness within your " + VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + ".  Somehow you know you could accommodate even larger... insertions.");
            changes++;
        }
        //Boost fertility
        ...
        if (changes < changeLimit && player.upperBody.chest.count() > 1 && Utils.rand(3) == 0) {
            ...
            DisplayText.text(". The " + BreastDescriptor.describeNipple(player, lastRow) + "s even fade until nothing but ");
        ...
        if (player.skinTone != "green" && player.skinTone != "grayish-blue" && player.skinTone != "dark green" && player.skinTone != "pale yellow" && changes < changeLimit && Utils.rand(2) == 0) {
            ...
            if (player.skinType == SkinType.FUR) DisplayText.text("skin");
            else DisplayText.text(player.skinDesc);
            DisplayText.text(" turned " + player.skinTone + ".  No way!  It's staying, it really changed color!");
        }
        //Face!
