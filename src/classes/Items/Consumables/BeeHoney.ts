...
export default class BeeHoney extends Consumable {
    ...
    public use(character: Character): boolean {
        ...
        if (changes < changeLimit && pure) { // Special honey will also reduce corruption, but uses different text and is handled separately
            ...
            // Libido Reduction
            if (character.stats.cor > 0 && changes < changeLimit && Utils.chance(66) && character.stats.lib > 40) {
        ...
        // Intelligence Boost
        if (changes < changeLimit && Utils.rand(2) === 0 && character.stats.int < 80) {
        ...
        // Appearance Stuff
        // Hair Color
        if (changes < changeLimit && (character.torso.neck.head.hair.color !== "shiny black" && character.torso.neck.head.hair.color !== "black and yellow") && character.torso.neck.head.hair.length > 10 && Utils.chance(50)) {
            DisplayText("\n\nYou feel your scalp tingling, and you grab your hair in a panic, pulling a stUtils.rand forward.  ");
            if (Utils.rand(9) === 0) character.torso.neck.head.hair.color = "black and yellow";
        ...
        // Hair Length
        if (changes < changeLimit && character.torso.neck.head.hair.length < 25 && Utils.rand(3) === 0) {
            DisplayText("\n\nFeeling a bit off-balance, you discover your hair has lengthened, ");
            character.torso.neck.head.hair.length += Utils.rand(4) + 1;
        ...
        // Antennae
        if (changes < changeLimit && character.torso.neck.head.antennae === AntennaeType.NONE && character.torso.neck.head.horns.amount === 0 && Utils.rand(3) === 0) {
        ...
        // Horns
        if (changes < changeLimit && character.torso.neck.head.horns.amount > 0 && Utils.rand(3) === 0) {
        ...
        // Bee Legs
        if (changes < changeLimit && character.torso.hips.legs.type !== LegType.BEE && character.torso.hips.legs.type !== LegType.CENTAUR && Utils.rand(4) === 0) {
        ...
        if (chest.reduce(BreastRow.AverageNipplesPerBreast, 0) > 1 && changes < changeLimit && Utils.rand(4) === 0) {
        ...
        if (changes < changeLimit && (character.torso.tails.count === 0 || Utils.rand(1.5) === 0) && Utils.rand(4) === 0) {
        ...
        // Venom Increase
        for (const beeButt of character.torso.tails.filterType(TailType.BEE_ABDOMEN)) {
        ...
        if (changes < changeLimit && character.torso.wings.type === WingType.BEE_LIKE_SMALL && Utils.rand(4)) {
            changes++;
        ...
        if (changes < changeLimit && (character.torso.wings.type === WingType.NONE || character.torso.wings.type === WingType.SHARK_FIN) && Utils.rand(4)) {
            changes++;
        ...
        if (special) { // All the speical honey effects occur after any normal bee transformations (if the character wasn't a full bee morph)
            ...
            if (cocks.count > 0) {
                ...
                selectedCock.length = Utils.rand(3) + 8;
                ...
                DisplayText("</b>");
                selectedCock.length += Utils.rand(3) + 4; // 4 to 6 inches in length
                selectedCock.thickness += 0.1 * Utils.rand(5) + 0.5; // 0.5 to 1 inches in thickness
                character.stats.sens += 5;
            }
                ...
                selectedCock.length += 0.1 * Utils.rand(10) + 1;
                selectedCock.thickness += 0.1 * Utils.rand(2) + 0.1;
