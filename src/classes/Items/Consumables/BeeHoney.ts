...
export default class BeeHoney extends Consumable {
    ...
    public use(player: Player): boolean {
        ...
        if (changes < changeLimit && pure) { //Special honey will also reduce corruption, but uses different text and is handled separately
            outputText("\n\n");
            changes++;
            if (player.stats.cor > 80) outputText("Your head aches, as if thunder was echoing around your skull.  ");
            else if (player.stats.cor > 60) outputText("You feel a headache forming just behind your eyes.  In no time flat it reaches full strength.  ");
            else if (player.stats.cor > 40) outputText("A wave of stinging pain slices through your skull.  ");
            else if (player.stats.cor > 20) outputText("A prickling pain spreads throughout your skull.  ");
            else outputText("You feel a mildly unpleasant tingling inside your skull.  ");
            if (player.stats.cor > 0) outputText("It quickly passes, leaving you more clearheaded");
            ...
            else if (player.stats.cor > 0) outputText(".");
        }
        //bee item corollary:
        ...
        if (changes < changeLimit && (player.upperBody.head.hairColor != "shiny black" && player.upperBody.head.hairColor != "black and yellow") && player.upperBody.head.hairLength > 10 && Utils.chance(50)) {
            ...
            outputText("Your hair is now " + player.upperBody.head.hairColor + ", just like a bee-girl's!");
            changes++;
        }
        //Hair Length
        ...
        if (changes < changeLimit && player.upperBody.chest.list.length > 2 && Utils.rand(3) == 0 && !Flags.has[FlagEnum.HYPER_HAPPY]) {
            ...
            if (player.upperBody.chest.list.length >= 3) outputText("abdomen");
            else outputText("chest");
            outputText(". The " + getGame().nippleDescript(player.upperBody.chest.list[player.upperBody.chest.list.length - 1]) + "s even fade until nothing but ");
            if (player.skinType == SkinType.FUR) outputText(player.upperBody.head.hairColor + " " + player.skinDesc);
        ...
        if (player.upperBody.chest.averageNipplesPerBreast() > 1 && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            if (player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("'breast'.");
