...
import { AntennaeType, HornType } from "../../Modules/HeadModule";
import { LowerBodyType, TailType } from "../../Modules/LowerBodyModule";
import Perk from "../../Perks/Perk";
import Cock, { CockType } from "../../Modules/Cock";
import { WingType } from "../../Modules/UpperBodyModule";
...
export default class BeeHoney extends Consumable {
    ...
    public canUse(player: Player) {
        if (this.value == BeeHoney.SPECIAL_HONEY_VALUE && player.statusAffects.get("Exgartuan").value1 == 1) { //Exgartuan doesn't like the special honey
            Render.text("You uncork the bottle only to hear Exgartuan suddenly speak up.  <i>“Hey kid, this beautiful cock here doesn’t need any of that special bee shit.  Cork that bottle up right now or I’m going to make it so that you can’t drink anything but me.”</i>  You give an exasperated sigh and put the cork back in the bottle.");
            return false;
        }
        return true;
    }
    ...
    public use(player: Player): boolean {
        ...
        //Drink text
        if (special) {
            Render.text("You uncork the bottle and pour the incredibly strong smelling concentrated honey down your throat.  Its taste is also mighty intense.  All at once you feel the effects of the substance start to course through your body.");
        }
        else { //Text for normal or pure
            Render.text("Opening the crystal vial, you are greeted by a super-concentrated wave of sweet honey-scent.  It makes you feel lightheaded.  You giggle and lick the honey from your lips, having drank down the syrupy elixir without a thought.");
        }
        if ((pure || special) && player.pregnancyType == PregnancyType.FAERIE) { //Pure or special honey can reduce the corruption of a phouka baby
            ...
            Flags.sub(FlagEnum.PREGNANCY_CORRUPTION, 1);
        ...
        if (changes < changeLimit && pure) { //Special honey will also reduce corruption, but uses different text and is handled separately
            Render.text("\n\n");
            changes++;
            if (player.stats.cor > 80) Render.text("Your head aches, as if thunder was echoing around your skull.  ");
            else if (player.stats.cor > 60) Render.text("You feel a headache forming just behind your eyes.  In no time flat it reaches full strength.  ");
            else if (player.stats.cor > 40) Render.text("A wave of stinging pain slices through your skull.  ");
            else if (player.stats.cor > 20) Render.text("A prickling pain spreads throughout your skull.  ");
            else Render.text("You feel a mildly unpleasant tingling inside your skull.  ");
            if (player.stats.cor > 0) Render.text("It quickly passes, leaving you more clearheaded");
            player.stats.cor += -(1 + (player.stats.cor / 20))
            //Libido Reduction
            if (player.stats.cor > 0 && changes < changeLimit && Utils.chance(66) && player.stats.lib > 40) {
                Render.text(" and settling your overcharged sex-drive a bit.");
                player.stats.lib -= 3;
                player.stats.lust -= 20;
                changes++;
            }
            else if (player.stats.cor > 0) Render.text(".");
        }
        //bee item corollary:
        ...
        if (changes < changeLimit && (player.upperBody.head.hairColor != "shiny black" && player.upperBody.head.hairColor != "black and yellow") && player.upperBody.head.hairLength > 10 && Utils.chance(50)) {
            ...
            Render.text("Your hair is now " + player.upperBody.head.hairColor + ", just like a bee-girl's!");
            changes++;
        }
        //Hair Length
        if (changes < changeLimit && player.upperBody.head.hairLength < 25 && Utils.rand(3) == 0) {
            ...
            Render.text("becoming " + getGame().hairDescript() + ".");
            changes++;
        }
        //-Remove extra breast rows
        if (changes < changeLimit && player.upperBody.chest.list.length > 2 && Utils.rand(3) == 0 && !Flags.has[FlagEnum.HYPER_HAPPY]) {
            changes++;
            Render.text("\n\nYou stumble back when your center of balance shifts, and though you adjust before you can fall over, you're left to watch in awe as your bottom-most " + getGame().breastDescript(player.upperBody.chest.count() - 1) + " shrink down, disappearing completely into your ");
            if (player.upperBody.chest.list.length >= 3) Render.text("abdomen");
            else Render.text("chest");
            Render.text(". The " + getGame().nippleDescript(player.upperBody.chest.list[player.upperBody.chest.list.length - 1]) + "s even fade until nothing but ");
            if (player.skinType == SkinType.FUR) Render.text(player.upperBody.head.hairColor + " " + player.skinDesc);
            else Render.text(player.skinTone + " " + player.skinDesc);
            Render.text(" remains. <b>You've lost a row of breasts!</b>");
            player.stats.sens -= 5
        ...
        if (player.upperBody.chest.averageNipplesPerBreast() > 1 && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            if (player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) Render.text("'breast'.");
            else Render.text("breast.");
            changes++;
        ...
        if (special) { //All the speical honey effects occur after any normal bee transformations (if the player wasn't a full bee morph)
            if (!player.lowerBody.cockSpot.hasCock()) {
                ...
                if (player.lowerBody.cockSpot.list[0].cockArea <= 20)
                    Render.text("It now swings as low as your knees!");
                else if (player.lowerBody.cockSpot.list[0].cockArea <= 50)
                    Render.text("While erect, your massive member fills the lower half of your vision.");
                else Render.text("Your member is now simply huge, you wonder what in the world could actually take your massive size now?");
                Render.text("</b>");
            ...
            else if (player.lowerBody.cockSpot.list[0].cockType != CockType.BEE && player.race() == "bee-morph") {
