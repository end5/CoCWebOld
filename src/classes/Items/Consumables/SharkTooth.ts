...
export default class SharkTooth extends Consumable {
    ...
    public use(character: Character) {
        ...
        if (((character.stats.str < 60 && this.enhanced) || character.stats.str < 50) && Utils.rand(3) === 0) {
            character.stats.str += 1 + Utils.rand(2);
        ...
        if (((character.stats.spe < 100 && this.enhanced) || character.stats.spe < 75) && Utils.rand(3) === 0) {
            character.stats.spe += 1 + Utils.rand(3);
            changes++;
        ...
        if (character.stats.sens > 25 && Utils.rand(1.5) === 0 && changes < changeLimit) {
            character.stats.sens += -1 - Utils.rand(3);
            changes++;
        ...
        if (((character.stats.lib < 100 && this.enhanced) || character.stats.lib < 75) && Utils.rand(3) === 0 && changes < changeLimit) {
            character.stats.lib += 1 + Utils.rand(3);
            changes++;
        ...
        if (character.stats.int > 40 && Utils.rand(3) === 0 && changes < changeLimit) {
            character.stats.int += -1 - Utils.rand(3);
            changes++;
        ...
        if (this.enhanced && (character.gender === 0 || (character.torso.vaginas.count <= 0 && changes < changeLimit && Utils.rand(3) === 0))) {
            changes++;
            // (balls)
        ...
        if (this.enhanced && (character.torso.cocks.count <= 0) && changes < changeLimit && Utils.rand(3) === 0) {
            // Genderless:
        ...
        if (this.enhanced && (character.torso.balls.quantity === 0 || character.torso.balls.quantity === 2) && character.torso.cocks.count > 0 && changes < changeLimit && Utils.rand(3) === 0) {
        ...
        // Transformations:
        // Mouth TF
        if (character.torso.neck.head.face.type !== FaceType.SHARK_TEETH && Utils.rand(3) === 0 && changes < changeLimit) {
            DisplayText("\n\n");
        ...
        // Tail TF
        if (character.torso.tails.reduce(Tail.HasType(TailType.SHARK), false) && Utils.rand(3) === 0 && changes < changeLimit) {
            changes++;
        ...
        // Hair
        if (character.torso.neck.head.hair.color !== "silver" && Utils.rand(4) === 0 && changes < changeLimit) {
            changes++;
        ...
        // Skin
        if (((character.skin.tone !== "rough gray" && character.skin.tone !== "orange and black striped") || character.skin.type !== SkinType.PLAIN) && Utils.rand(7) === 0 && changes < changeLimit) {
            DisplayText("\n\n");
        ...
        // FINZ R WINGS
        if (character.torso.wings.type !== WingType.SHARK_FIN && changes < changeLimit && Utils.rand(3) === 0) {
            DisplayText("\n\n");
