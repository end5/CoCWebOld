    ...
    public use(character: Character) {
        ...
        if (character.torso.chest.reduce(BreastRow.AverageRating, 0) < 8) {
            ...
            if (character.torso.chest.count === 1)
                BreastModifier.growSmallestBreastRow(character, (1 + Utils.rand(5)), 1, true);
            else BreastModifier.growSmallestBreastRow(character, 1 + Utils.rand(2), character.torso.chest.count, true);
        }
        ...
        // Boost lactation
            ...
            for (let index = 0; index < character.torso.chest.count; index++) {
                character.torso.chest.get(index).lactationMultiplier += 1 + Utils.rand(10) / 10;
            }
        }
