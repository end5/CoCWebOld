...
export default class Reducto extends Consumable {
    ...
    private reductoBalls(player: Player): void {
        MainScreen.clearText();
        MainScreen.text("You smear the foul-smelling paste onto your " + player.sackDescript() + ".  It feels cool at first but rapidly warms to an uncomfortable level of heat.\n\n");
        player.lowerBody.ballSize -= Utils.rand(4) + 2;
        if (player.lowerBody.ballSize < 1) player.lowerBody.ballSize = 1;
        MainScreen.text("You feel your scrotum shift, shrinking down along with your " + player.ballsDescriptLight() + ".  Within a few seconds the paste has been totally absorbed and the shrinking stops.");
        player.stats.lib -= 2;
    ...
    private reductoBreasts(player: Player): void {
        MainScreen.clearText();
        MainScreen.text("You smear the foul-smelling ointment all over your " + player.allBreastsDescript() + ", covering them entirely as the paste begins to get absorbed into your " + player.skinDesc + ".\n");
    ...
    private reductoButt(player: Player): void {
        MainScreen.clearText();
        MainScreen.text("You smear the foul-smelling paste onto your " + player.buttDescript() + ".  It feels cool at first but rapidly warms to an uncomfortable level of heat.\n\n");
        if (player.lowerBody.butt.buttRating >= 15) {
            ...
            MainScreen.text("You feel much lighter as your " + player.buttDescript() + " jiggles slightly, adjusting to its smaller size.");
        }
        else {
            player.lowerBody.butt.buttRating -= Utils.rand(3) + 1;
            if (player.lowerBody.butt.buttRating < 1) player.lowerBody.butt.buttRating = 1;
            MainScreen.text("After a few seconds your " + player.buttDescript() + " has shrunk to a much smaller size!");
        }
        player.stats.lib -= 2;
        player.stats.lust -= 10;
        game.inventory.itemGoNext();
    ...
    private reductoClit(player: Player): void {
        MainScreen.clearText();
        MainScreen.text("You carefully apply the paste to your " + player.clitDescript() + ", being very careful to avoid getting it on your " + player.vaginaDescript(0) + ".  It burns with heat as it begins to make its effects known...\n\n");
        ...
        MainScreen.text("Your " + player.clitDescript() + " shrinks rapidly, dwindling down to almost half its old size before it finishes absorbing the paste.");
        player.stats.sens += 2;
        player.stats.lust += 10;
        game.inventory.itemGoNext();
    }
    ...
    private reductoCock(player: Player): void {
        ...
        if (player.lowerBody.cockSpot.list[0].cockType == CockType.BEE) {
            ...
            MainScreen.text("You smear the repulsive smelling paste over your " + player.multiCockDescriptLight() + ".  It immediately begins to grow warm, almost uncomfortably so, as your " + player.multiCockDescriptLight() + " begins to shrink.\n\n");
            if (player.lowerBody.cockSpot.count() == 1) {
                MainScreen.text("Your " + player.cockDescript(0) + " twitches as it shrinks, disappearing steadily into your " + (player.lowerBody.cockSpot.hasSheath() ? "sheath" : "crotch") + " until it has lost about a third of its old size.");
                player.lowerBody.cockSpot.list[0].cockLength *= 2 / 3;
                player.lowerBody.cockSpot.list[0].cockThickness *= 2 / 3;
            }
            else { //MULTI
                MainScreen.text("Your " + player.multiCockDescriptLight() + " twitch and shrink, each member steadily disappearing into your " + (player.lowerBody.cockSpot.hasSheath() ? "sheath" : "crotch") + " until they've lost about a third of their old size.");
    ...
    private reductoNipples(player: Player): void {
        MainScreen.clearText();
        MainScreen.text("You rub the paste evenly over your " + player.nippleDescript(0) + "s, being sure to cover them completely.\n\n");
        //Shrink
        if (player.upperBody.chest.BreastRatingLargest[0].nippleLength / 2 < 0.25) {
            ...
            MainScreen.text("Your " + player.nippleDescript(0) + "s get smaller and smaller, stopping when they are roughly half their previous size.");
