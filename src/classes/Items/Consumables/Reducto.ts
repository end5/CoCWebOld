    ...
    private reductoCock(player: Player): void {
        ...
        if (player.lowerBody.cockSpot.list[0].cockType == CockType.BEE) {
            ...
            if (player.lowerBody.cockSpot.count() == 1) {
                outputText("Your " + player.cockDescript(0) + " twitches as it shrinks, disappearing steadily into your " + (player.lowerBody.cockSpot.hasSheath() ? "sheath" : "crotch") + " until it has lost about a third of its old size.");
                player.lowerBody.cockSpot.list[0].cockLength *= 2 / 3;
                player.lowerBody.cockSpot.list[0].cockThickness *= 2 / 3;
            }
            else { //MULTI
                outputText("Your " + player.multiCockDescriptLight() + " twitch and shrink, each member steadily disappearing into your " + (player.lowerBody.cockSpot.hasSheath() ? "sheath" : "crotch") + " until they've lost about a third of their old size.");
