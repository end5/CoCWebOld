...
export default class Eggs extends Consumable {
    ...
    private brownEgg(character: Character): void {
        ...
        if (!this.large) {
            ...
            character.torso.butt.rating += 2 + Utils.rand(3);
        }
        if (Utils.chance(33)) {
            if (this.large)
    ...
    private purpleEgg(character: Character): void {
        ...
        if (!this.large || character.torso.hips.rating > 20) {
            ...
            character.torso.hips.rating += 2 + Utils.rand(2);
        }
        if (Utils.chance(33)) {
            if (this.large)
            ...
            if (character.torso.cocks.count > 0) {
                ...
                if (character.torso.cocks.count > 1) {
                    ...
                    for (const cock of character.torso.cocks) {
                        cockAmountLengthened += CockModifier.growCock(character, cock, Utils.rand(3) + 2);
                ...
                if (character.torso.cocks.count === 1) {
                    ...
                    cockAmountLengthened = CockModifier.growCock(character, character.torso.cocks.get(0), Utils.rand(3) + 2);
        ...
        // LARGE
            ...
            if (character.torso.cocks.count > 0) {
                ...
                if (character.torso.cocks.count > 1) {
                    DisplayText("\n\nYour " + CockDescriptor.describeMultiCock(character) + " fill to full-size... and begin growing obscenely.  ");
                    for (const cock of character.torso.cocks) {
                        cockAmountLengthened += CockModifier.growCock(character, cock, Utils.rand(3) + 5);
                ...
                if (character.torso.cocks.count === 1) {
                    ...
                    cockAmountLengthened = CockModifier.growCock(character, character.torso.cocks.get(0), Utils.rand(3) + 5);
                    CockModifier.displayLengthChange(character, cockAmountLengthened, 1);
                    // Display the degree of thickness change.
                    if (cockAmountThickened >= 1) {
                        if (character.torso.cocks.count === 1) DisplayText("  Your " + CockDescriptor.describeMultiCockShort(character) + " spreads rapidly, swelling an inch or more in girth, making it feel fat and floppy.");
    ...
    private whiteEgg(character: Character): void {
        ...
        // LARGE
            ...
            if (character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].nipples.length < 3 && character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 0) {
                ...
                character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].nipples.length += (Utils.rand(2) + 3) / 10;
                character.stats.lust += 15;
            }
            // NIPPLECUNTZZZ
            ...
            // chance of hair change
                ...
                if (character.torso.neck.head.hair.color.indexOf("rubbery") === -1 && character.torso.neck.head.hair.color.indexOf("latex-textured") && character.torso.neck.head.hair.length !== 0) {
                    ...
                    if (character.skin.desc === "skin" && character.skin.adj === "rubber") {
                        DisplayText("\n\nYour scalp tingles and your " + HeadDescriptor.describeHair(character) + " thickens, the stUtils.Utils.rands merging into ");
