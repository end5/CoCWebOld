...
export default class LaBova extends Consumable {
    ...
    public use(character: Character) {
        ...
        // Sex bits - Duderiffic
        if (character.torso.cocks.count > 0 && Utils.rand(2) === 0 && !Flags.list[FlagEnum.HYPER_HAPPY]) {
            ...
            cockGrowth += CockModifier.growCock(character, biggestCock, (Utils.rand(3) + 1) * -1);
        ...
        if (((this.tainted && character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating <= 11) || (!this.tainted && character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating <= 5)) && changes < changeLimit && (Utils.rand(3) === 0 || this.enhanced)) {
            ...
            BreastModifier.growTopBreastRow(character, 1 + Utils.rand(3), 1, false);
            changes++;
        ...
        if (character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 4 && character.torso.chest.get(0).lactationMultiplier < 1 && changes < changeLimit && (Utils.rand(3) === 0 || boobsGrew || this.enhanced)) {
        ...
        if (this.enhanced) {
            ...
            if (character.torso.chest.get(0).nipples.count === 1) {
                ...
                if (character.torso.chest.count >= 3 && character.torso.chest.get(2).nipples.count === 1) {
                    ...
                    DisplayText("  <b>You have a total of " + Utils.numToCardinalText(character.torso.chest.countNipples()) + " nipples.</b>");
                }
            }
        ...
        // If breasts are already lactating and the character is not lactating beyond a reasonable level, they start lactating more:
            ...
            if (this.tainted && character.torso.chest.get(0).lactationMultiplier > 1 && character.torso.chest.get(0).lactationMultiplier < 5 && changes < changeLimit && (Utils.rand(3) === 0 || this.enhanced)) {
            ...
            if (!this.tainted) {
                if (character.torso.chest.get(0).lactationMultiplier > 1 && character.torso.chest.get(0).lactationMultiplier < 3.2 && changes < changeLimit && Utils.rand(3) === 0) {
                    if (Utils.rand(2) === 0) DisplayText("\n\nA wave of pleasure passes through your chest as your " + BreastDescriptor.describeBreastRow(character.torso.chest.get(0)) + " start producing more milk.");
                    else DisplayText("\n\nSomething shifts inside your " + BreastDescriptor.describeBreastRow(character.torso.chest.get(0)) + " and they feel fuller and riper.  You know that you've started producing more milk.");
                    BreastModifier.boostLactation(character, 0.75);
                    if ((character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].nipples.length < 1.5 && this.tainted) || (!this.tainted && character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].nipples.length < 1)) {
        ...
        if (!character.statusAffects.has(StatusAffectType.Feeder) && character.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 3 && Utils.rand(2) === 0 && character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 5 && character.stats.cor >= 35) {
        ...
        if (character.torso.vaginas.count > 0) {
            if (character.torso.vaginas.get(0).looseness < VaginaLooseness.LOOSE && changes < changeLimit && Utils.rand(2) === 0) {
        ...
        if (this.tainted && !character.torso.tails.reduce(Tail.HasType(TailType.COW), false) && changes < changeLimit && Utils.rand(3) === 0) {
        ...
        if (this.tainted && character.torso.neck.head.ears.type !== EarType.COW && changes < changeLimit && Utils.rand(4) === 0 && character.torso.tails.reduce(Tail.HasType(TailType.COW), false)) {
        ...
        if (((this.enhanced && character.tallness < 96) || character.tallness < 84) && changes < changeLimit && Utils.rand(2) === 0) {
        ...
        if (!this.enhanced && character.torso.hips.legs.type === LegType.HOOFED && character.torso.neck.head.face.type !== FaceType.HUMAN && changes < changeLimit && Utils.rand(4) === 0) {
        ...
        if (this.tainted && changes < changeLimit && Utils.rand(3) === 0 && character.torso.neck.head.face.type === FaceType.HUMAN) {
            ...
            if (character.torso.neck.head.horns.type === HornType.COW_MINOTAUR || character.torso.neck.head.horns.type === HornType.NONE) {
                ...
                if (character.torso.neck.head.horns.type === HornType.COW_MINOTAUR) {
                    if (character.torso.neck.head.horns.amount < 5) {
                        ...
                        character.torso.neck.head.horns.amount += 1 + Utils.rand(2);
                        changes++;
                    }
                }
        ...
        if (Utils.rand(2) === 0 && character.torso.hips.rating < 15 && changes < changeLimit) {
            if (!this.tainted && character.torso.hips.rating < 8 || this.tainted) {
                ...
                character.torso.hips.rating += 1 + Utils.rand(4);
                changes++;
            }
        }
        ...
        if (Utils.rand(2) === 0 && character.torso.butt.rating < 13 && changes < changeLimit) {
            if (!this.tainted && character.torso.butt.rating < 8 || this.tainted) {
                ...
                character.torso.butt.rating += 1 + Utils.rand(2);
                changes++;
            }
        }
        // Nipples Turn Back:
        ...
        // Debugcunt
        if (changes < changeLimit && Utils.rand(3) === 0 && character.torso.vaginas.get(0).type === 5 && character.torso.vaginas.count > 0) {
            DisplayText("\n\nSomething invisible brushes against your sex, making you twinge.  Undoing your clothes, you take a look at your vagina and find that it has turned back to its natural flesh colour.");
            character.torso.vaginas.get(0).type = 0;
            changes++;
        }
        if (Utils.rand(3) === 0) DisplayText(BodyModifier.displayModFem(character, 79, 3));
        if (Utils.rand(3) === 0) DisplayText(BodyModifier.displayModThickness(character, 70, 4));
        if (Utils.rand(5) === 0) DisplayText(BodyModifier.displayModTone(character, 10, 5));
    }
}
