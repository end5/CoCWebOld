...
export default class SweetGossamer extends Consumable {
    ...
    public use(player: Player) {
        ...
        //Consuming Text
        if (this.sweet) DisplayText.text("You wad up the sweet, pink gossamer and eat it, finding it to be delicious and chewy, almost like gum.  Munching away, your mouth generates an enormous amount of spit until you're drooling all over yourself while you devour the sweet treat.");
        else if (!this.sweet) DisplayText.text("You wad up the sweet, black gossamer and eat it, finding it to be delicious and chewy, almost like licorice.  Munching away, your mouth generates an enormous amount of spit until you're drooling all over yourself while you devour the sweet treat.");
        ...
        if (player.stats.str > 70 && Utils.rand(3) == 0) {
            ...
            if (RaceScore.spiderScore(player) < 4) DisplayText.text("Wait, you're not a spider, that doesn't make any sense!");
            else DisplayText.text("Well, maybe you should put your nice, heavy abdomen to work.");
            player.stats.str += -1;
            changes++;
        }
        //****************
        ...
        if (player.lowerBody.cockSpot.hasCock() && changes < changeLimit && Utils.rand(4) == 0) {
            ...
            if (cockGotThickened) {
                DisplayText.text("\n\nYou can feel your " + CockDescriptor.describeMultiCockShort(player) + " filling out in your " + player.inventory.armorSlot.equipment.displayName + ". Pulling ");
                if (player.lowerBody.cockSpot.count() == 1) DisplayText.text("it");
                else DisplayText.text("them");
                DisplayText.text(" out, you look closely.  ");
                if (player.lowerBody.cockSpot.count() == 1) DisplayText.text("It's");
                else DisplayText.text("They're");
                DisplayText.text(" definitely thicker.");
                let counter: number;
                changes++;
            }
        }
        ...
        if (player.skinType != SkinType.PLAIN && (player.upperBody.head.earType == EarType.HUMAN || player.upperBody.head.earType == EarType.ELFIN) && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.skinType == SkinType.SCALES) DisplayText.text("are");
            else DisplayText.text("is");
            DisplayText.text(" falling to the ground, revealing flawless, almost pearly-white skin underneath.  <b>You now have pale white skin.</b>");
        ...
        if (player.upperBody.armType != ArmType.SPIDER && changes < changeLimit && Utils.rand(4) == 0) {
            DisplayText.text("\n\n");
            //(Bird pretext)
            if (player.upperBody.armType == ArmType.HARPY) DisplayText.text("The feathers covering your arms fall away, leaving them to return to a far more human appearance.  ");
        ...
        //Drider butt
        if (!this.sweet && !player.perks.has(PerkType.SpiderOvipositor) && player.lowerBody.isDrider() && player.lowerBody.tailType == TailType.SPIDER_ABDOMEN && changes < changeLimit && Utils.rand(3) == 0 && (player.lowerBody.vaginaSpot.hasVagina() || Utils.rand(2) == 0)) {
            ...
            //V1 - Egg Count
            //V2 - Fertilized Count
            player.perks.add(PerkFactory.create(PerkType.SpiderOvipositor, 0, 0, 0, 0));
