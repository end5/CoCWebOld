...
export default class SweetGossamer extends Consumable {
    ...
    public use(player: Player) {
        ...
        if (player.torso.butt.looseness > 1 && changes < changeLimit && Utils.rand(3) === 0) {
            DisplayText("\n\nYou let out a small cry as your " + ButtDescriptor.describeButthole(player.torso.butt) + " shrinks, becoming smaller and tighter. When it's done, you feel much hornier and eager to stretch it out again.");
            player.stats.lib += 2;
            player.stats.lust += 25;
            changes++;
        ...
        // Drider butt
        if (!this.sweet && !player.perks.has(PerkType.SpiderOvipositor) && player.torso.hips.legs.isDrider() && player.torso.tails.reduce(Tail.HasType(TailType.SPIDER_ABDOMEN), false) && changes < changeLimit && Utils.rand(3) === 0 && (player.torso.vaginas.count > 0 || Utils.rand(2) === 0)) {
        ...
        if (!player.torso.tails.reduce(Tail.HasType(TailType.SPIDER_ABDOMEN), false) && (player.torso.hips.legs.type === LegType.CHITINOUS_SPIDER_LEGS || player.torso.hips.legs.type === LegType.DRIDER_LOWER_BODY) && player.torso.arms.type === ArmType.SPIDER && Utils.rand(4) === 0) {
            DisplayText("\n\n");
            // (Pre-existing tails)
        ...
        if (!this.sweet && player.torso.hips.legs.type === LegType.CHITINOUS_SPIDER_LEGS && Utils.rand(4) === 0 && player.torso.tails.reduce(Tail.HasType(TailType.SPIDER_ABDOMEN), false)) {
