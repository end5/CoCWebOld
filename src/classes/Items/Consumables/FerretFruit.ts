...
export default class FerretFruit extends Consumable {
    ...
    public use(player: Player) {
        ...
        DisplayText.clear();
        DisplayText.text("Feeling parched, you gobble down the fruit without much hesitation. Despite the skin being fuzzy like a peach, the inside is relatively hard, and its taste reminds you of that of an apple.  It even has a core like an apple. Finished, you toss the core aside.");
        ...
        if (((!cockSpot.hasCock() && player.lowerBody.hipRating > 6) || (cockSpot.hasCock() && player.lowerBody.hipRating > 4)) && Utils.rand(3) == 0 && changes < changeLimit) {
            ...
            if (Utils.rand(2) == 0 && changes < changeLimit) {
                if (longestCock.cockLength > 6 && !Flags.list[FlagEnum.HYPER_HAPPY]) {
                    ...
                    if (longestCock.cockLength / 6 >= longestCock.cockThickness)
                        DisplayText.text("  Luckily, it doen�t seem to have lost its previous thickness.");
                    else
        ...
        if (chest.averageNipplesPerBreast() > 1 && Utils.rand(4) == 0 && changes < changeLimit) {
        ...
        if (player.upperBody.head.face.eyeType == EyeType.FOUR_SPIDER_EYES && Utils.rand(3) == 0 && changes < changeLimit) {
            ...
            player.upperBody.head.face.eyeType = 0;
            changes++;
        }
        //Go into heat
        ...
        if (player.upperBody.head.face.faceType == FaceType.FERRET_MASK && player.skinType == SkinType.FUR && player.upperBody.head.earType == EarType.FERRET && player.lowerBody.tailType == TailType.FERRET && player.lowerBody.type == LowerBodyType.FERRET && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            player.upperBody.head.face.faceType = FaceType.FERRET;
            changes++;
        }
        //If face is human:
        if (player.upperBody.head.face.faceType == 0 && Utils.rand(3) == 0 && changes < changeLimit) {
            ...
            player.upperBody.head.face.faceType = FaceType.FERRET_MASK;
            changes++;
        }
        //If face is not ferret, has ferret ears, tail, and legs:
        if (player.upperBody.head.face.faceType != FaceType.HUMAN && player.upperBody.head.face.faceType != FaceType.FERRET_MASK && player.upperBody.head.face.faceType != FaceType.FERRET && Utils.rand(3) == 0 && changes < changeLimit) {
            ...
            player.upperBody.head.face.faceType = 0;
            changes++;
        }
        //No fur, has ferret ears, tail, and legs:
        if (player.skinType != SkinType.FUR && player.upperBody.head.earType == EarType.FERRET && player.lowerBody.tailType == TailType.FERRET && player.lowerBody.type == LowerBodyType.FERRET && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.upperBody.head.hairColor != "sandy brown" && player.upperBody.head.hairColor != "silver" && player.upperBody.head.hairColor != "white" && player.upperBody.head.hairColor != "brown") {
                DisplayText.text("\n\nOdder still, all of your hair changes to ");
                if (Utils.rand(4) == 0) player.upperBody.head.hairColor = "sandy brown";
                else if (Utils.rand(3) == 0) player.upperBody.head.hairColor = "silver";
                else if (Utils.rand(2) == 0) player.upperBody.head.hairColor = "white";
                else player.upperBody.head.hairColor = "brown";
                DisplayText.text(".");
            }
            DisplayText.text("  <b>You now have " + player.upperBody.head.hairColor + " fur!</b>");
            player.skinType = SkinType.FUR;
            changes++;
        }
        //Tail TFs!
        if (player.lowerBody.tailType != TailType.FERRET && player.upperBody.head.earType == EarType.FERRET && Utils.rand(3) == 0 && changes < changeLimit) {
            ...
            player.lowerBody.tailType = TailType.FERRET;
            changes++;
        }
