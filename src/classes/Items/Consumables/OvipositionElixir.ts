...
export default class OvipositionElixir extends Consumable {
    ...
    public canUse(player: Player): boolean {
        if (player.torso.vaginas.count > 0) return true;
    ...
    public use(player: Player) {
        ...
        if (player.pregnancy.womb.isPregnantWith(PregnancyType.GOO_STUFFED)) {
        ...
        if (player.pregnancy.womb.isPregnantWith(PregnancyType.WORM_STUFFED)) {
        ...
        if (player.pregnancy.womb.isPregnantWith(PregnancyType.OVIELIXIR_EGGS)) { // If player already has eggs, chance of size increase!
        ...
        if (!changeOccurred && player.pregnancy.womb.isPregnant()) {
            ...
            const pregnancy = player.pregnancy.womb.pregnancy;
