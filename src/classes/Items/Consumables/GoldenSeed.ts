...
export default class GoldenSeed extends Consumable {
    ...
    public use(player: Player) {
        ...
        //Generic eating text:
        DisplayText.clear();
        DisplayText.text("You pop the nut into your mouth, chewing the delicious treat and swallowing it quickly.  No wonder harpies love these things so much!");
        //****************
        //Stats:
        //****************
        ...
        if ((player.stats.lib < 90 || Utils.rand(3) == 0) && Utils.rand(3) == 0 && changes < changeLimit) {
            ...
            if (player.stats.lib < 40) {
                ...
                if (player.lowerBody.vaginaSpot.hasVagina()) DisplayText.text(" The moistness of your " +  VaginaDescriptor.describeVagina(player, player.lowerBody.vaginaSpot.get(0)) + " seems to agree.");
                else if (player.lowerBody.cockSpot.hasCock()) DisplayText.text(" The hardness of " + CockDescriptor.describeMultiCockSimpleOne(player) + " seems to agree.");
                player.stats.lust += 5;
            }
            //(sub 75 lib)
            else if (player.stats.lib < 75) DisplayText.text("\n\nHeat, blessed heat, works through you from head to groin, leaving you to shudder and fantasize about the sex you could be having right now.\n\n");
            //(hi lib)
            else if (player.stats.lib < 90) DisplayText.text("\n\nSexual need courses through you, flushing your skin with a reddish hue while you pant and daydream of the wondrous sex you should be having right now.\n\n");
            //(90+)
                ...
                if (player.lowerBody.cockSpot.hasCock() && (player.gender != 3 || Utils.rand(2) == 0)) {
                    ...
                    if (Utils.rand(2) == 0) {
                        ...
                        if (player.lowerBody.cockSpot.count() > 1) {
                            DisplayText.text("The extra penis");
                            if (player.lowerBody.cockSpot.count() > 2) DisplayText.text("es rub ");
                            else DisplayText.text("rubs ");
                            DisplayText.text("the skin over her taut, empty belly, drooling your need atop her.  ");
                            DisplayText.text("You jolt from the vision unexpectedly, finding your " + CockDescriptor.describeMultiCockSimpleOne(player) + " is as hard as it was in the dream. The inside of your " + player.inventory.armorSlot.equipment.displayName + " is quite messy from all the pre-cum you've drooled. Perhaps you can find a harpy nearby to lie with.");
                        }
                    }
                    //(male 2)
                    else {
        ...
        if (changes < changeLimit && player.upperBody.chest.count() > 1 && Utils.rand(3) == 0 && !Flags.list[FlagEnum.HYPER_HAPPY]) {
            ...
            DisplayText.text(". The " + BreastDescriptor.describeNipple(player, bottomBreastRow) + "s even fade until nothing but ");
        ...
        //Cannot happen at same time as row removal
            ...
            //(H+)
                ...
                DisplayText.text("\n\nYour chest pinches tight, wobbling dangerously for a second before the huge swell of your bust begins to shrink into itself. The weighty mounds jiggle slightly as they shed cup sizes like old, discarded coats, not stopping until they're " + BreastDescriptor.breastCup(player.upperBody.chest.get(0).breastRating) + "s.");
            }
        }
        //-Grow tits to a B-cup if below.
        if (changes < changeLimit && player.upperBody.chest.get(0).breastRating < 2 && Utils.rand(3) == 0) {
            changes++;
            DisplayText.text("\n\nYour chest starts to tingle, the " + player.skinDesc + " warming under your " + player.inventory.armorSlot.equipment.displayName + ". Reaching inside to feel the tender flesh, you're quite surprised when it puffs into your fingers, growing larger and larger until it settles into a pair of B-cup breasts.");
            if (player.upperBody.chest.get(0).breastRating < 1) DisplayText.text("  <b>You have breasts now!</b>");
        ...
        if ((player.skinTone != "tan" && player.skinTone != "olive" && player.skinTone != "dark" && player.skinTone != "light") && changes < changeLimit && Utils.rand(5) == 0) {
            ...
            if (player.skinType == SkinType.FUR) DisplayText.text("the skin under your " + player.upperBody.head.hairColor + " " + player.skinDesc);
            else DisplayText.text("your " + player.skinDesc);
            DisplayText.text(" has changed to become ");
            ...
            DisplayText.text(player.skinTone + " colored.</b>");
        }
        ...
        if (player.lowerBody.butt.buttRating < 8 && changes < changeLimit && Utils.rand(3) == 0) {
            ...
            changes++;
            DisplayText.text("\n\nA slight jiggle works through your rear, but instead of stopping it starts again. You can actually feel your " + player.inventory.armorSlot.equipment.displayName + " being filled out by the growing cheeks. When it stops, you find yourself the proud owner of a " + ButtDescriptor.describeButt(player) + ".");
        }
        ...
        if (player.thickness > 25 && changes < changeLimit && Utils.rand(3) == 0) {
            DisplayText.text(BodyModifier.displayModThickness(player, 25, 3 + Utils.rand(4)));
            changes++;
        }
        //Remove odd eyes
        ...
        if (player.lowerBody.type != LowerBodyType.HARPY && changes < changeLimit && (this.enhanced || player.lowerBody.tailType == TailType.HARPY) && Utils.rand(4) == 0) {
            ...
            changes++;
            //(cont)
            DisplayText.text("While humanoid in shape, they have two large, taloned toes on the front and a single claw protruding from the heel. The entire ensemble is coated in " + player.upperBody.head.hairColor + " feathers from ankle to hip, reminding you of the bird-women of the mountains. <b>You now have harpy legs!</b>");
        }
        //-Feathery Tail
        ...
        if (player.upperBody.wingType != WingType.FEATHERED_LARGE && player.upperBody.wingType > WingType.NONE && changes < changeLimit && Utils.rand(4) == 0) {
            if (player.upperBody.wingType != WingType.SHARK_FIN) DisplayText.text("\n\nSensation fades from your " + player.upperBody.wingDesc + " wings slowly but surely, leaving them dried out husks that break off to fall on the ground. Your back closes up to conceal the loss, as smooth and unbroken as the day you entered the portal.");
        ...
        if (!player.perks.has(PerkType.HarpyWomb) && player.lowerBody.type == LowerBodyType.HARPY && player.lowerBody.tailType == TailType.HARPY && Utils.rand(4) == 0 && changes < changeLimit) {
            player.perks.add(PerkFactory.create(PerkType.HarpyWomb, 0, 0, 0, 0));
