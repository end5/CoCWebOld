...
export default class Equinum extends Consumable {
    ...
    public warning(player: Player) {
        if (player.skinType == SkinType.FUR && player.upperBody.head.face.faceType == FaceType.HORSE && player.lowerBody.tailType == TailType.HORSE && (player.lowerBody.type != LowerBodyType.HOOFED)) {
            //WARNINGS
            //Repeat warnings
            if (player.statusAffects.has(StatusAffectType.HorseWarning) && Utils.rand(3) == 0) {
                if (player.statusAffects.get(StatusAffectType.HorseWarning).value1 == 0) DisplayText.text("<b>\n\nYou feel a creeping chill down your back as your entire body shivers, as if rejecting something foreign.  Maybe you ought to cut back on the horse potions.</b>");
                if (player.statusAffects.get(StatusAffectType.HorseWarning).value1 > 0) DisplayText.text("<b>\n\nYou wonder how many more of these you can drink before you become a horse...</b>");
                player.statusAffects.get(StatusAffectType.HorseWarning).value1 = 1;
            }
            //First warning
            if (!player.statusAffects.has(StatusAffectType.HorseWarning)) {
                ...
                player.statusAffects.add(StatusAffectFactory.create(StatusAffectType.HorseWarning, 0, 0, 0, 0));
            }
            //Bad End
            if (Utils.rand(4) == 0 && player.statusAffects.has(StatusAffectType.HorseWarning)) {
                //Must have been warned first...
                if (player.statusAffects.get(StatusAffectType.HorseWarning).value1 > 0) {
                    ...
                    if (player.lowerBody.cockSpot.count() > 0) {
                        ...
                        if (player.lowerBody.cockSpot.countType(CockType.HORSE) > 0) {
                            ...
                            if (player.gender == 0 || player.gender == 3) DisplayText.text("horse ");
                            if (player.gender == 1) DisplayText.text("stallion ");
                            if (player.gender == 2) DisplayText.text("mare ");
    ...
    public use(player: Player) {
        ...
        //Set up output
        DisplayText.clear();
        DisplayText.text("You down the potion, grimacing at the strong taste.");
        ...
        if (Utils.rand(2) == 0) {
            ...
            if (player.stats.tou >= 75) {
                ...
                if (player.gender == Gender.MALE || player.gender == Gender.HERM)
                    DisplayText.text("stallion's.");
                else
                    DisplayText.text("mare's.");
            }
            //NOT MAXXED
            else {
        ...
        if ((player.gender == 1 || player.gender == 3) && Utils.rand(1.5) == 0 && changes < changeLimit) {
            ...
            if ((cockSpot.countType(CockType.HORSE) + cockSpot.countType(CockType.DEMON)) < cockSpot.count()) {
                ...
                if (cockSpot.count() == 1) {
                    ...
                    if (cockTF)
                        DisplayText.text("  <b>Your penis has transformed into a horse's!</b>");
                }
                //MULTICOCK
                else {
                    player.stats.lib += 5;
                    ...
                    DisplayText.text("\n\nOne of your penises begins to feel strange.  You pull down your clothes to take a look and see the skin of your " + CockDescriptor.describeCock(player, selectedCock) + " darkening to a mottled brown and black pattern.");

                    //Already have a sheath
                    if (cockSpot.countType(CockType.HORSE) > 1 || cockSpot.countType(CockType.DOG) > 0)
                        DisplayText.text("  Your sheath tingles and begins growing larger as the cock's base shifts to lie inside it.");
                    else
                        DisplayText.text("  You feel a tightness near the base where your skin seems to be bunching up.  A sheath begins forming around your " + CockDescriptor.describeCock(player, selectedCock) + "'s root, tightening and pulling your " + CockDescriptor.describeCock(player, selectedCock) + " inside its depths.");
                    CockModifier.growCock(player, selectedCock, Utils.rand(4) + 4);
                    DisplayText.text("  The shaft suddenly explodes with movement, growing longer and developing a thick flared head leaking steady stream of animal-cum.");
                    DisplayText.text("  <b>You now have a horse-cock.</b>");
                }
            ...
            // Players cocks are all horse-type - increase size!
                ...
                DisplayText.text("\n\n");
                if (growthAmount > 2) DisplayText.text("Your " + CockDescriptor.describeCock(player, selectedCock) + " tightens painfully, inches of taut horse-flesh pouring out from your sheath as it grows longer.  Thick animal-pre forms at the flared tip, drawn out from the pleasure of the change.");
                if (growthAmount > 1 && growthAmount <= 2) DisplayText.text("Aching pressure builds within your sheath, suddenly releasing as an inch or more of extra dick flesh spills out.  A dollop of pre beads on the head of your enlarged " + CockDescriptor.describeCock(player, selectedCock) + " from the pleasure of the growth.");
                if (growthAmount <= 1) DisplayText.text("A slight pressure builds and releases as your " + CockDescriptor.describeCock(player, selectedCock) + " pushes a bit further out of your sheath.");
                changes++;
            }
            ...
            if (Utils.rand(2) == 0 && changes < changeLimit && cockSpot.countType(CockType.HORSE) > 0) {
                ...
                if (player.stats.cor + player.stats.lib < 50)
                    DisplayText.text(" to have such a splendid tool.  You idly daydream about cunts and pussies, your " + CockDescriptor.nounCock(CockType.HORSE) + " plowing them relentlessly, stuffing them pregnant with cum");
                if (player.stats.cor + player.stats.lib >= 50 && player.stats.cor + player.stats.lib < 80)
                    DisplayText.text(" to be this way... You breath the powerful animalistic scent and fantasize about fucking centaurs night and day until their bellies slosh with your cum");
                ...
                DisplayText.text(".");
                if (player.stats.cor < 30) DisplayText.text("  You shudder in revulsion at the strange thoughts and vow to control yourself better.");
                if (player.stats.cor >= 30 && player.stats.cor < 60) DisplayText.text("  You wonder why you thought such odd things, but they have a certain appeal.");
                if (player.stats.cor >= 60 && player.stats.cor < 90) DisplayText.text("  You relish your twisted fantasies, hoping to dream of them again.");
                if (player.stats.cor >= 90) DisplayText.text("  You flush hotly and give a twisted smile, resolving to find a fitting subject to rape and relive your fantasies.");
        ...
        if (player.gender == Gender.FEMALE || player.gender == Gender.HERM) {
            ...
            if (vaginaSpot.count() == 1) {
                if (vaginaSpot.get(0).vaginalLooseness <= VaginaLooseness.GAPING && changes < changeLimit && Utils.rand(2) == 0) {
                    DisplayText.text("\n\nYou grip your gut in pain as you feel your organs shift slightly.  When the pressure passes, you realize your " + VaginaDescriptor.describeVagina(player, vaginaSpot.get(0)) + " has grown larger, in depth AND size.");
                ...
                if (vaginaSpot.get(0).vaginalWetness <= VaginaWetness.NORMAL && changes < changeLimit && Utils.rand(2) == 0) {
                    DisplayText.text("\n\nYour " + VaginaDescriptor.describeVagina(player, vaginaSpot.get(0)) + " moistens perceptably, giving off an animalistic scent.");
            ...
            //Multicooch
                ...
                if (leastWet.vaginalWetness <= VaginaWetness.NORMAL && changes < changeLimit && Utils.rand(2) == 0) {
                    DisplayText.text("\n\nOne of your " + VaginaDescriptor.describeVagina(player, leastWet) + " moistens perceptably, giving off an animalistic scent.");
                ...
                if (smallest.vaginalLooseness <= VaginaLooseness.GAPING && changes < changeLimit && Utils.rand(2) == 0) {
                    DisplayText.text("\n\nYou grip your gut in pain as you feel your organs shift slightly.  When the pressure passes, you realize one of your " + VaginaDescriptor.describeVagina(player, smallest) + " has grown larger, in depth AND size.");
                ...
                if (Utils.rand(2) == 0 && changes < changeLimit) {
                    ...
                    if (chest.count() == 1) {
                        ...
                        if (selectedBreastRow.breastRating > 3) {
                            ...
                            if (!majorShrinkage)
                                DisplayText.text("\n\nYou feel a weight lifted from you, and realize your " + BreastDescriptor.describeBreastRow(selectedBreastRow) + " have shrunk to a " + BreastDescriptor.breastCup(selectedBreastRow.breastRating) + ".");
                            else
                                DisplayText.text("\n\nYou feel significantly lighter.  Looking down, you realize your breasts are MUCH smaller, down to " + BreastDescriptor.breastCup(selectedBreastRow.breastRating) + "s.");
                            changes++;
                        }
        ...
        if (changes < changeLimit && Utils.rand(6) == 0 && player.lowerBody.type == LowerBodyType.HOOFED) {
            ...
            if (player.gender > 0) {
                DisplayText.text("  After taking a moment to get used to your new body, you notice that your genitals now reside between the back legs on your centaur body.");
            }
            player.stats.spe += 3;
        ...
        if (player.skinType != SkinType.FUR && changes < changeLimit &&
            ...
            if (player.skinType == SkinType.PLAIN) DisplayText.text("\n\nAn itchy feeling springs up over every inch of your skin.  As you scratch yourself madly, you feel fur grow out of your skin until <b>you have a fine coat of " + player.upperBody.head.hairColor + "-colored fur.</b>");
            if (player.skinType == SkinType.SCALES) {
                player.skinDesc = "fur";
                DisplayText.text("\n\nYour " + player.skinTone + " scales begin to itch insufferably.  You reflexively scratch yourself, setting off an avalanche of discarded scales.  The itching intensifies as you madly scratch and tear at yourself, revealing a coat of " + player.upperBody.head.hairColor + " " + player.skinDesc + ".  At last the itching stops as <b>you brush a few more loose scales from your new coat of fur.</b>");
            }
            changes++;
        ...
        if (player.upperBody.head.earType != EarType.HORSE && player.lowerBody.tailType == TailType.HORSE && changes < changeLimit &&
            ...
            DisplayText.text("<b>You now have horse ears.</b>");
            changes++;
        }
        //Tail - no-prereq
        if (player.lowerBody.tailType != TailType.HORSE && Utils.rand(2) == 0 && changes < changeLimit) {
            ...
            if (player.lowerBody.tailType == 0) {
                DisplayText.text("\n\nThere is a sudden tickling on your ass, and you notice you have sprouted a long shiny horsetail of the same " + player.upperBody.head.hairColor + " color as your hair.");
            }
            //if other animal tail
            if (player.lowerBody.tailType > TailType.HORSE && player.lowerBody.tailType <= TailType.COW) {
                DisplayText.text("\n\nPain lances up your " + ButtDescriptor.describeButthole(player) + " as your tail shifts and morphs disgustingly.  With one last wave of pain, it splits into hundreds of tiny filaments, transforming into a horsetail.");
            }
            //if bee/spider-butt.
            ...
            DisplayText.text("  <b>You now have a horse-tail.</b>");
        ...
        if (Utils.rand(3) == 0) DisplayText.text(BodyModifier.displayModTone(player, 60, 1));
        //FAILSAFE CHANGE
        if (changes == 0) {
