...
export default class RingtailFig extends Consumable {
    ...
    public use(character: Character) {
        ...
        if (character.stats.spe < 80 && Utils.rand(3) === 0 && changes < changeLimit) {
        ...
        // gain sensitivity
        if (character.stats.sens < 80 && Utils.rand(3) === 0 && changes < changeLimit) {
        ...
        if (Utils.rand(4) && character.stats.tou > 50 && changes < changeLimit) {
        ...
        if (character.torso.cocks.count > 0) {
            // gain ball size
            if (character.torso.balls.quantity > 0 && character.torso.balls.size < 15 && Utils.rand(4) === 0 && changes < changeLimit) {
            ...
            if (character.torso.balls.quantity === 0 && character.skin.type === SkinType.FUR && Utils.rand(3) === 0 && changes < changeLimit) {
        ...
        if (Utils.rand(4) === 0 && character.thickness < 80 && changes < changeLimit) {
            DisplayText(BodyModifier.displayModThickness(character, 80, 2));
            changes++;
        }
        // bodypart changes:
        if (character.torso.tails.reduce(Tail.HasType(TailType.RACCOON), false) && Utils.rand(4) === 0 && changes < changeLimit) {
            // grow da tail
            // from no tail:
        ...
        // gain coon ears
        if (character.torso.tails.reduce(Tail.HasType(TailType.RACCOON), false) && character.torso.neck.head.ears.type !== EarType.RACCOON && Utils.rand(4) === 0 && changes < changeLimit) {
            // from dog, kangaroo, bunny, other long ears
            if (character.torso.neck.head.ears.type === EarType.DOG || character.torso.neck.head.ears.type === EarType.BUNNY || character.torso.neck.head.ears.type === EarType.KANGAROO) DisplayText("\n\nYour ears compress, constricting your ear canal momentarily.  You shake your head to get sound back, and reach up to touch the auricles, to find a pair of stubby egg-shaped ears in their place.  <b>You now have raccoon ears!</b>");
        ...
        // gain feet-coon
        if (character.torso.neck.head.ears.type === EarType.RACCOON && character.torso.hips.legs.type !== LegType.RACCOON && changes < changeLimit && Utils.rand(4) === 0) {
            ...
            // from hooves or hard feet, including centaurs and bees
            else if (character.torso.hips.legs.type === LegType.HOOFED || character.torso.hips.legs.type === LegType.CENTAUR || character.torso.hips.legs.type === LegType.BEE || character.torso.hips.legs.type === LegType.PONY || character.torso.hips.legs.type === LegType.CHITINOUS_SPIDER_LEGS) {
                ...
                if (character.torso.hips.legs.type === LegType.HOOFED || character.torso.hips.legs.type === LegType.CENTAUR || character.torso.hips.legs.type === LegType.PONY) DisplayText("pops right out of its hoof just in time");
            ...
            // from human, demon, paw feet
                ...
                if ((character.torso.hips.legs.type === LegType.HUMAN || character.torso.hips.legs.type === LegType.DEMONIC_HIGH_HEELS || character.torso.hips.legs.type === LegType.DEMONIC_CLAWS) && character.skin.type !== SkinType.FUR) DisplayText("The sensation of walking around on what feels like a second pair of hands is so weird that you miss noticing the itchy fur growing in over your legs...  ");
                DisplayText("<b>You now have raccoon paws!</b>");
            }
            character.torso.hips.legs.type = LegType.RACCOON;
            changes++;
        }
        // gain half-coon face (prevented if already full-coon)
        if (character.torso.neck.head.face.type !== FaceType.RACCOON_MASK && character.torso.neck.head.face.type !== FaceType.RACCOON && Utils.rand(4) === 0 && changes < changeLimit) {
            // from human/naga/shark/bun face
            if (character.torso.neck.head.face.type === FaceType.HUMAN || character.torso.neck.head.face.type === FaceType.SHARK_TEETH || character.torso.neck.head.face.type === FaceType.SNAKE_FANGS || character.torso.neck.head.face.type === FaceType.BUNNY) {
                DisplayText("\n\nA sudden wave of exhaustion passes over you, and your face goes partially numb around your eyes.  ");
                // (nagasharkbunnies)
                if (character.torso.neck.head.face.type === FaceType.SHARK_TEETH || character.torso.neck.head.face.type === FaceType.SNAKE_FANGS || character.torso.neck.head.face.type === FaceType.BUNNY) {
        ...
        // from humanoid - should be the only one possible
        else if (character.torso.neck.head.face.type === FaceType.RACCOON_MASK && character.torso.hips.legs.type === LegType.RACCOON && character.skin.type === SkinType.FUR && Utils.rand(4) === 0 && changes < changeLimit) {
            ...
            changes++;
            character.torso.neck.head.face.type = FaceType.RACCOON;
        }
        // fatigue damage (only if face change was not triggered)
        else if (Utils.rand(2) === 0 && changes < changeLimit && (character.torso.neck.head.face.type !== FaceType.RACCOON_MASK && character.torso.neck.head.face.type !== FaceType.RACCOON)) {
