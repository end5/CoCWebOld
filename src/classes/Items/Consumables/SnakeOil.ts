...
export default class SnakeOil extends Consumable {
    ...
    public use(player: Player) {
        ...
        if (player.stats.spe < 70 && Utils.rand(2) === 0) {
            ...
            if (player.stats.spe < 40) DisplayText("  Of course, you're nowhere near as fast as that.");
            changes++;
        }
        // Removes wings
        if (player.torso.wings.type > WingType.NONE && Utils.rand(3) === 0 && changes < changeLimit) {
        ...
        // Removes antennae
        if (player.torso.neck.head.antennae > AntennaeType.NONE && Utils.rand(3) === 0 && changes < changeLimit) {
        ...
        // 9c) III The fangs
        if (changes === 0 && player.torso.neck.head.face.tongue.type === TongueType.SNAKE && player.torso.neck.head.face.type !== FaceType.SNAKE_FANGS && Utils.rand(3) === 0 && changes < changeLimit) {
            ...
            if (player.torso.neck.head.face.type !== FaceType.HUMAN && player.torso.neck.head.face.type !== FaceType.SHARK_TEETH && player.torso.neck.head.face.type !== FaceType.BUNNY && player.torso.neck.head.face.type !== FaceType.SPIDER_FANGS) {
                DisplayText("  As the change progresses, your " + FaceDescriptor.describeFace(player) + " reshapes.  The sensation is far more pleasant than teeth cutting into gums, and as the tingling transformation completes, <b>you've gained with a normal-looking, human visage.</b>");
            }
            player.torso.neck.head.face.type = FaceType.SNAKE_FANGS;
            changes++;
        }
        ...
        if (changes === 0 && player.torso.neck.head.face.type === FaceType.SNAKE_FANGS && player.torso.hips.legs.type !== LegType.NAGA && Utils.rand(4) === 0 && changes < changeLimit) {
            ...
            player.torso.hips.legs.type = LegType.NAGA;
            changes++;
        }
        ...
        /*
         ...
         DisplayText("your " + CockDescriptor.describeMultiCockShort(player) + " starts to throb painfully and swell to its full size.  With a horrifying ripping sensation, your cock splits down the middle, the pain causing you to black out momentarily.");
         DisplayText("When you awaken, you quickly look down to see that where ");
         //(if multicock)
         if(player.torso.cocks.count > 1) DisplayText("one of ");
         DisplayText("your " + CockDescriptor.describeMultiCockShort(player) + " was, you now have two pointed reptilian cocks, still stiff and pulsing.");
         }*/
        // Default change - blah
