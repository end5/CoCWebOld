...
export default class SnakeOil extends Consumable {
    ...
    public use(player: Player) {
        ...
        DisplayText.text("Pinching your nose, you quickly uncork the vial and bring it to your mouth, determined to see what effects it might have on your body. Pouring in as much as you can take, you painfully swallow before going for another shot, emptying the bottle.");
        //(if outside combat)
        if (!Game.inCombat) DisplayText.text("  Minutes pass as you start wishing you had water with you, to get rid of the aftertaste.");
        //+ speed to 70!
        if (player.stats.spe < 70 && Utils.rand(2) == 0) {
            ...
            if (player.stats.spe < 40) DisplayText.text("  Of course, you're nowhere near as fast as that.");
            changes++;
        }
        //Removes wings
        ...
        if (changes == 0 && player.upperBody.head.face.tongueType == TongueType.SNAKE && player.upperBody.head.face.faceType != FaceType.SNAKE_FANGS && Utils.rand(3) == 0 && changes < changeLimit) {
            ...
            if (player.upperBody.head.face.faceType != FaceType.HUMAN && player.upperBody.head.face.faceType != FaceType.SHARK_TEETH && player.upperBody.head.face.faceType != FaceType.BUNNY && player.upperBody.head.face.faceType != FaceType.SPIDER_FANGS) {
                DisplayText.text("  As the change progresses, your " + FaceDescriptor.describeFace(player) + " reshapes.  The sensation is far more pleasant than teeth cutting into gums, and as the tingling transformation completes, <b>you've gained with a normal-looking, human visage.</b>");
            }
        ...
        if (changes == 0 && player.upperBody.head.face.faceType == FaceType.SNAKE_FANGS && player.lowerBody.type != LowerBodyType.NAGA && Utils.rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.lowerBody.balls > 0 && player.lowerBody.ballSize > 10) DisplayText.text("  You're happy not to have to drag those testicles around with you anymore.");
        ...
        /*
         ...
         //(if multicock)
         if(player.lowerBody.cockSpot.count() > 1) DisplayText.text("one of ");
         DisplayText.text("your " + CockDescriptor.describeMultiCockShort(player) + " starts to throb painfully and swell to its full size.  With a horrifying ripping sensation, your cock splits down the middle, the pain causing you to black out momentarily.");
         DisplayText.text("When you awaken, you quickly look down to see that where ");
         //(if multicock)
         if(player.lowerBody.cockSpot.count() > 1) DisplayText.text("one of ");
         DisplayText.text("your " + CockDescriptor.describeMultiCockShort(player) + " was, you now have two pointed reptilian cocks, still stiff and pulsing.");
         }*/
        //Default change - blah
