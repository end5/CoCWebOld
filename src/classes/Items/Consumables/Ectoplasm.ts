...
export default class Ectoplasm extends Consumable {
    ...
    public use(player: Player) {
        ...
        if (player.stats.sens >= 20 && Utils.rand(3) === 0 && changes < changeLimit) {
            DisplayText("\n\nWoah, what the... you pinch your " + SkinDescriptor.skin(player) + " to confirm your suspicions; the ghostly snack has definitely lowered your sensitivity.");
            player.stats.sens -= 2;
        ...
        if (player.stats.lib < 100 && Utils.rand(3) === 0 && changes < changeLimit) {
            ...
            DisplayText(" a trace amount of the ghost girl's lust is transferred into you.  How horny IS she, you have to wonder...");
            player.stats.lib += 1;
        ...
        if (player.torso.cocks.count > 0 && changes < changeLimit) {
            if (Utils.rand(3) === 0 && player.torso.cocks.get(0).type !== CockType.HUMAN) {
                DisplayText("\n\nA strange tingling begins behind your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", slowly crawling up across its entire length.  While neither particularly arousing nor uncomfortable, you do shift nervously as the feeling intensifies.  You resist the urge to undo your " + player.inventory.equipment.armor.displayName + " to check, but by the feel of it, your penis is shifting form.  Eventually the transformative sensation fades, <b>leaving you with a completely human penis.</b>");
        ...
        // Appearnace Change
        // Hair
        if (Utils.rand(4) === 0 && changes < changeLimit && player.torso.neck.head.hair.type !== 2) {
        ...
        // Skin
        if (Utils.rand(4) === 0 && changes < changeLimit && (player.skin.tone !== "sable" && player.skin.tone !== "white")) {
            if (Utils.rand(2) === 0) {
        ...
        // Legs
        if (changes < changeLimit && !player.perks.has(PerkType.Incorporeality) && (player.skin.tone === "white" || player.skin.tone === "sable") && player.torso.neck.head.hair.type === 2) {
            ...
            DisplayText("\n\nAn otherworldly sensation begins in your belly, working its way to your " + HipDescriptor.describeHips(player) + ". Before you can react, your " + LegDescriptor.describeLegs(player) + " begin to tingle, and you fall on your rump as a large shudder runs through them. As you watch, your lower body shimmers, becoming ethereal, wisps rising from the newly ghost-like " + LegDescriptor.describeLegs(player) + ". You manage to rise, surprised to find your new, ghostly form to be as sturdy as its former corporeal version. Suddenly, like a dam breaking, fleeting visions and images flow into your head, never lasting long enough for you to concentrate on one. You don't even realize it, but your arms fly up to your head, grasping your temples as you groan in pain. As fast as the mental bombardment came, it disappears, leaving you with a surprising sense of spiritual superiority.  <b>You have ghost legs!</b>\n\n");
        ...
        if (changes === 0) {
            ...
            StatModifier.displayCharacterHPChange(player, player.stats.level * 5 + 10);
            changes++;
        }
