...
export default class Ectoplasm extends Consumable {
    ...
    public use(character: Character) {
        ...
        if (character.stats.int < 100 && Utils.rand(3) === 0 && changes < changeLimit) {
        ...
        if (character.stats.sens >= 20 && Utils.rand(3) === 0 && changes < changeLimit) {
        ...
        if (character.stats.lib < 100 && Utils.rand(3) === 0 && changes < changeLimit) {
            // ([if libido >49]
            if (character.stats.lib < 50) DisplayText("\n\nIdly, you drop a hand to your crotch as");
        ...
        if (character.torso.cocks.count > 0 && changes < changeLimit) {
            if (Utils.rand(3) === 0 && character.torso.cocks.get(0).type !== CockType.HUMAN) {
        ...
        // Appearnace Change
        // Hair
        if (Utils.rand(4) === 0 && changes < changeLimit && character.torso.neck.head.hair.type !== 2) {
        ...
        // Skin
        if (Utils.rand(4) === 0 && changes < changeLimit && (character.skin.tone !== "sable" && character.skin.tone !== "white")) {
            if (Utils.rand(2) === 0) {
                ...
                character.skin.tone = "white";
                character.skin.adj = "milky";
                ...
                character.skin.tone = "sable";
                character.skin.adj = "ashen";
                character.skin.desc = "skin";
                character.skin.type = SkinType.PLAIN;
            }
            changes++;
