...
export default class GroPlus extends Consumable {
    ...
    public use(player: Player) {
        ...
        DisplayText.clear();
        DisplayText.text("You ponder the needle in your hand knowing it will enlarge the injection site.  What part of your body will you use it on?  ");
    ...
    private growPlusBalls(player: Player): void {
        DisplayText.clear();
        player.slimeFeed();
        DisplayText.text("You sink the needle deep into your " + BallsDescriptor.describeSack(player) + ".  It hurts like hell, but you push down the plunger and the pain vanishes as the needles contents flow into you.\n\n");
        //1 in 4 BIG growth.
        if (Utils.rand(4) == 0) {
            DisplayText.text("You feel a trembling in your " + BallsDescriptor.describeBallsShort(player) + " as the chemicals start to go to work.  You can tell they're going to be VERY effective.\n");
            player.lowerBody.ballSize += Utils.rand(4) + 2;
            DisplayText.text("They shift, stretching your " + BallsDescriptor.describeSack(player) + " tight as they gain inches of size.  You step to steady yourself as your center of balance shifts due to your newly enlarged " + BallsDescriptor.describeBallsShort(player) + ".  ");
        }
        else {
            player.lowerBody.ballSize += Utils.rand(2) + 1;
            DisplayText.text("You feel your testicles shift, pulling the skin of your " + BallsDescriptor.describeSack(player) + " a little bit as they grow to " + BallsDescriptor.describeBallsShort(player) + ".  ");
        }
    ...
    private growPlusBreasts(player: Player): void {
        DisplayText.clear();
        player.slimeFeed();
        DisplayText.text("You sink the needle into the flesh of your " + BreastDescriptor.describeAllBreasts(player) + " injecting each with a portion of the needle.\n\n");
    ...
    private growPlusClit(player: Player): void {
        DisplayText.clear();
        player.slimeFeed();
        DisplayText.text("You sink the needle into your clit, nearly crying with how much it hurts.  You push down the plunger and the pain vanishes as your clit starts to grow.\n\n");
        player.lowerBody.vaginaSpot.get(0).clitLength++;
        DisplayText.text("Your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + " stops growing after an inch of new flesh surges free of your netherlips.  It twitches, feeling incredibly sensitive.");
    ...
    private growPlusCock(player: Player): void {
        DisplayText.clear();
        player.slimeFeed();
        DisplayText.text("You sink the needle into the base of your " + CockDescriptor.describeMultiCockShort(player) + ".  It hurts like hell, but as you depress the plunger, the pain vanishes, replaced by a tingling pleasure as the chemicals take effect.\n\n");
        if (player.lowerBody.cockSpot.count() == 1) {
            DisplayText.text("Your " + CockDescriptor.describeCock(player, player.lowerBody.cockSpot.get(0)) + " twitches and thickens, pouring more than an inch of thick new length from your ");
        ...
        //MULTI
            ...
            DisplayText.text("Your " + CockDescriptor.describeMultiCockShort(player) + " twitch and thicken, each member pouring out more than an inch of new length from your ");
    ...
    private growPlusNipples(player: Player): void {
        DisplayText.clear();
        player.slimeFeed();
        DisplayText.text("You sink the needle into each of your " + BreastDescriptor.describeNipple(player, player.upperBody.chest.get(0)) + "s in turn, dividing the fluid evenly between them.  Though each injection hurts, the pain is quickly washed away by the potent chemical cocktail.\n\n");
        //Grow nipples
        DisplayText.text("Your nipples engorge, prodding hard against the inside of your " + player.inventory.armorSlot.equipment.displayName + ".  Abruptly you realize they've grown more than an additional quarter-inch.\n\n");
