...
export default class KangaFruit extends Consumable {
    ...
    public use(player: Player) {
        DisplayText.clear();
        DisplayText.text("You squeeze the pod around the middle, forcing the end open.  Scooping out a handful of the yeasty-smelling seeds, you shovel them in your mouth.  Blech!  Tastes like soggy burnt bread... and yet, you find yourself going for another handful...");
        ...
        if (player.lowerBody.ballSize >= 4 && changes < changeLimit && Utils.rand(2) == 0) {
            ...
            DisplayText.text("\n\nYour " + BallsDescriptor.describeSack(player) + " pulls tight against your groin, vibrating slightly as it changes.  Once it finishes, you give your " +  BallsDescriptor.describeBallsShort(player) + " a gentle squeeze and discover they've shrunk.  Even with the reduced volume, they feel just as heavy.");
            changes++;
        }
        //-Shorten clits to reasonable size
        if (player.lowerBody.vaginaSpot.get(0).clitLength >= 4 && changes < changeLimit && Utils.rand(5) == 0) {
            DisplayText.text("\n\nPainful pricks work through your " + VaginaDescriptor.describeClit(player, player.lowerBody.vaginaSpot.get(0)) + ", all the way into its swollen clitoral sheath.  Gods, it feels afire with pain!  Agony runs up and down its length, and by the time the pain finally fades, the feminine organ has lost half its size.");
        ...
        if (player.lowerBody.cockSpot.hasCock()) {
            if (biggestCock.cockLength >= 16 && changes < changeLimit && Utils.rand(5) == 0) {
                DisplayText.text("\n\nA roiling inferno of heat blazes in your " + CockDescriptor.describeCock(player, biggestCock) + ", doubling you over in the dirt.  You rock back and forth while tears run unchecked down your cheeks.  Once the pain subsides and you're able to move again, you find the poor member has lost nearly half its size.");
            ...
            if (player.lowerBody.cockSpot.countType(CockType.KANGAROO) < player.lowerBody.cockSpot.count() && (this.enhanced && Utils.rand(2) == 0) && changes < changeLimit) {
                ...
                if (player.lowerBody.cockSpot.hasSheath()) DisplayText.text("your sheath");
                else DisplayText.text("a sheath that forms at the base of it");
                DisplayText.text(".  <b>You now have a kangaroo-penis!</b>");
        ...
        if (player.skinType != SkinType.FUR && (player.lowerBody.type == LowerBodyType.KANGAROO || this.enhanced) && changes < changeLimit && Utils.rand(4) == 0) {
            DisplayText.text("\n\nYour " + player.skinDesc + " itches terribly all over and you try cartoonishly to scratch everywhere at once.  As you pull your hands in, you notice " + player.upperBody.head.hairColor + " fur growing on the backs of them.  All over your body the scene is repeated, covering you in the stuff.  <b>You now have fur!</b>");
            changes++;
        ...
        if (player.lowerBody.tailType != TailType.KANGAROO && changes < changeLimit && Utils.rand(4) == 0 && (!this.enhanced || player.upperBody.head.earType == EarType.KANGAROO)) {
            ...
            //gain roo tail from bee tail:
                ...
                if (player.lowerBody.tailType == TailType.BEE_ABDOMEN) DisplayText.text("hollow stinger drops out of the end, taking the venom organ with it.");
                else DisplayText.text("spinnerets drop out of the end, taking the last of your webbing with it.");
                DisplayText.text("  By the time you're back to yourself, the insectile carapace has fallen off completely, leaving you with a long, thick, fleshy tail in place of your proud, insectile abdomen.  <b>You now have a kangaroo tail!</b>  You wipe the errant spittle from your mouth as you idly bob your new tail about.");
            }
        ...
        if (!player.perks.has(PerkType.Diapause) && RaceScore.kangaScore(player) > 4 && Utils.rand(4) == 0 && changes < changeLimit && player.lowerBody.vaginaSpot.hasVagina()) {
            //Perk name and description:
            player.perks.add(PerkFactory.create(PerkType.Diapause, 0, 0, 0, 0));
