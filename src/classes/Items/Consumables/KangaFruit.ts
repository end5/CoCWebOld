...
export default class KangaFruit extends Consumable {
    ...
    public use(player: Player) {
        ...
        // ****************
        // -Hip widening funtimes
        if (changes < changeLimit && Utils.rand(4) === 0 && player.torso.hips.rating < 40) {
        ...
        // ****************
        // -Shrink balls down to reasonable size (3?)
        if (player.torso.balls.size >= 4 && changes < changeLimit && Utils.rand(2) === 0) {
        ...
        if (player.torso.cocks.count > 0) {
            ...
            if (player.torso.cocks.filterType(CockType.KANGAROO).length < player.torso.cocks.count && (this.enhanced && Utils.rand(2) === 0) && changes < changeLimit) {
                ...
                for (let index: number = 0; index < player.torso.cocks.count; index++) {
                    if (player.torso.cocks.get(index).type !== CockType.KANGAROO) {
                        player.torso.cocks.get(index).type = CockType.KANGAROO;
                        player.torso.cocks.get(index).knotMultiplier = 1;
                        break;
                    }
                }
                changes++;
        ...
        if (player.torso.neck.head.face.type !== FaceType.KANGAROO && ((player.skin.type === SkinType.FUR && player.torso.hips.legs.type === LegType.KANGAROO) || this.enhanced) && changes < changeLimit && Utils.rand(4) === 0) {
            ...
            changes++;
            player.torso.neck.head.face.type = FaceType.KANGAROO;
        }
        // -Fur (Req: Footsies)
        if (player.skin.type !== SkinType.FUR && (player.torso.hips.legs.type === LegType.KANGAROO || this.enhanced) && changes < changeLimit && Utils.rand(4) === 0) {
            DisplayText("\n\nYour " + player.skin.desc + " itches terribly all over and you try cartoonishly to scratch everywhere at once.  As you pull your hands in, you notice " + player.torso.neck.head.hair.color + " fur growing on the backs of them.  All over your body the scene is repeated, covering you in the stuff.  <b>You now have fur!</b>");
            changes++;
            player.skin.type = SkinType.FUR;
            player.skin.desc = "fur";
        }
        // -Roo footsies (Req: Tail)
        if (player.torso.hips.legs.type !== LegType.KANGAROO && (this.enhanced || player.torso.tails.filterType(TailType.KANGAROO).length >= 1) && changes < changeLimit && Utils.rand(4) === 0) {
            ...
            player.torso.hips.legs.type = LegType.KANGAROO;
            changes++;
        }
        // -Roo tail (Req: Ears)
        if (player.torso.tails.filterType(TailType.KANGAROO).length <= 0 && changes < changeLimit && Utils.rand(4) === 0 && (!this.enhanced || player.torso.neck.head.ears.type === EarType.KANGAROO)) {
            // gain roo tail:
            ...
            // gain roo tail from bee tail:
            else if (player.torso.tails.filterType(TailType.SPIDER_ABDOMEN).length >= 1 || player.torso.tails.filterType(TailType.BEE_ABDOMEN).length >= 1) {
                ...
                if (player.torso.tails.filterType(TailType.BEE_ABDOMEN).length >= 1) DisplayText("hollow stinger drops out of the end, taking the venom organ with it.");
        ...
        if (player.torso.neck.head.ears.type !== EarType.KANGAROO && changes < changeLimit && Utils.rand(4) === 0) {
            ...
            changes++;
            player.torso.neck.head.ears.type = EarType.KANGAROO;
        }
        // UBEROOOO
