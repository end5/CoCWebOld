...
export default class MouseCocoa extends Consumable {
    ...
    public use(player: Player) {
        ...
        // gain tail
        // from no tail
        if (player.torso.neck.head.ears.type === EarType.MOUSE && player.torso.tails.filterType(TailType.MOUSE).length <= 0 && changes < changeLimit && Utils.rand(4) === 0) {
            // from other tail
            if (player.torso.tails.count > 0) {
                ...
                if (player.torso.tails.filterType(TailType.HORSE).length >= 1) DisplayText("elongating");
