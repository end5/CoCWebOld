...
export default class BimboLiqueur extends Consumable {
    ...
    public canUse(player: Player) {
        if (!player.perks.has(PerkType.FutaForm))
            return true;
        DisplayText.text("Ugh.  This stuff is so, like... last year.  Maybe you can find someone else to feed it to?\n\n");
        return false;
    }

    public use(player: Player) {
        if (player.perks.has(PerkType.BroBody)) {
            DisplayText.text("You wince as the stuff hits your stomach, already feeling the insidious effects beginning to take hold.  A lengthy belch escapes your lips as your stomach gurgles, and you giggle abashedly to yourself.");
            ...
            if (largestBreasts.breastRating < 7) {
                if (largestBreasts.breastRating < 1)
                    DisplayText.text("  Tingling, your chest begins to itch, then swell into a pair of rounded orbs.  ");
                else
                    DisplayText.text("  You feel a tingling inside your breasts.  ");
            ...
            DisplayText.text("\n\n");
            DisplayText.text("A wave of numbness rolls through your features, alerting you that another change is happening.  You reach up to your feel your jaw narrowing, becoming more... feminine?  Heavy, filling lips purse in disappointment as your face takes on a very feminine cast.  You're probably pretty hot now!\n\n");
            if (player.femininity < 80) player.femininity = 80;

            DisplayText.text("Your surging, absurdly potent libido surges through your body, reminding you that you need to fuck.  Not just bitches, but guys too.  Hard cocks, wet pussies, hell, you don't care.  They can have both or a dozen of either.  You just want to get laid and bone something, hopefully at the same time!");
            DisplayText.text("\n\n<b>(Perks Lost: Bro Body");
            if (player.perks.has(PerkType.BroBrains))
                DisplayText.text(", Bro Brains");
            DisplayText.text(")\n");
            DisplayText.text("(Perks Gained: Futa Form, Futa Faculties)\n");
            ...
            if (player.upperBody.head.hairColor != "platinum blonde") {
                DisplayText.text("The first change that you notice is to your " + HeadDescriptor.describeHair(player) + ".  It starts with a tingling in your scalp and intensifies ");
                if (player.upperBody.head.hairLength < 36) {
                    DisplayText.text("as you feel the weight of your hair growing heavier and longer.");
                    player.upperBody.head.hairLength = 36;
                }
                else DisplayText.text("as your hair grows thicker and heavier.");
                DisplayText.text("  You grab a lock of the silken stUtils.rands and watch open-mouthed while streaks so blonde they're almost white flow down the " + player.upperBody.head.hairColor + " hair.  It goes faster and faster until your hair has changed into perfectly bimbo-blonde, flowing locks.\n\n");
                player.upperBody.head.hairColor = "platinum blonde";
            }

            DisplayText.text("Moaning lewdly, you begin to sway your hips from side to side, putting on a show for anyone who might manage to see you.   You just feel so... sexy.  Too sexy to hide it.  Your body aches to show itself and feel the gaze of someone, anyone upon it.  Mmmm, it makes you so wet!  ");
            if (!player.lowerBody.vaginaSpot.hasVagina()) {
                player.lowerBody.vaginaSpot.add(new Vagina());
                player.lowerBody.vaginaSpot.get(0).clitLength = 0.25;
                player.lowerBody.vaginaSpot.get(0).vaginalWetness = VaginaWetness.SLICK;
                if (player.lowerBody.isTaur()) DisplayText.text("Wait!? Wet? You wish you could touch yourself between the " + LowerBodyDescriptor.describeLegs(player) + ", but you can tell from the fluid running down your hind-legs just how soaked your new vagina is.");
                else DisplayText.text("Wait!?  Wet?  You touch yourself between the " + LowerBodyDescriptor.describeLegs(player) + " and groan when your fingers sink into a sloppy, wet cunt.");
            }
            else {
                if (player.lowerBody.isTaur()) {
                    DisplayText.text("You wish you could sink your fingers into your sloppy, wet cunt, but as a centaur, you can't quite reach.");
                    if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < VaginaWetness.SLICK)
                        player.lowerBody.vaginaSpot.get(0).vaginalWetness = VaginaWetness.SLICK;
                }
                else {
                    DisplayText.text("You sink your fingers into your ");
                    if (player.lowerBody.vaginaSpot.get(0).vaginalWetness < VaginaWetness.SLICK) {
                        DisplayText.text("now ");
                        player.lowerBody.vaginaSpot.get(0).vaginalWetness = VaginaWetness.SLICK;
                    }
                    DisplayText.text("sloppy, wet cunt with a groan of satisfaction.");
                }
            }
            ...
            if (player.lowerBody.cockSpot.hasCock()) {
                ...
                if (player.lowerBody.cockSpot.count() == 1) DisplayText.text("a penis");
                else DisplayText.text("penises");
                DisplayText.text("!</b>");
                while (player.lowerBody.cockSpot.hasCock()) {
                    player.lowerBody.cockSpot.remove(player, player.lowerBody.cockSpot.get(0));
                }
            }
            DisplayText.text("  Somehow, you feel like you could seduce anyone right now!\n\n");
            ...
            DisplayText.text("You sigh and run one hand over your " + BreastDescriptor.describeNipple(player, player.upperBody.chest.get(0)) + "s");
            ...
            if (player.lowerBody.vaginaSpot.get(0).clitLength >= 3) DisplayText.text("jerk ");
            else DisplayText.text("caress ");
            DisplayText.text("that makes your " + LowerBodyDescriptor.describeLegs(player) + " give out as you cum, splattering female fluids as you convulse nervelessly on the ground.\n\n");
            ...
            if (player.lowerBody.hipRating < 12 || player.lowerBody.butt.buttRating < 12) {
                DisplayText.text("As you start to walk off in search of a sexual partner, you feel your center of balance shifting.");
                if (player.lowerBody.hipRating < 12 && player.lowerBody.butt.buttRating < 12) {
                    DisplayText.text("  Your ass and hips inflate suddenly, forcing you to adopt a slow, swaying gait.  You find that rolling your hips back and forth comes naturally to you.  You make sure to squeeze your butt-muscles and make your curvy tush jiggle as you go.");
                    player.lowerBody.butt.buttRating = 12;
                    player.lowerBody.hipRating = 12;
                }
                else if (player.lowerBody.hipRating < 12) {
                    DisplayText.text("  Your hips widen suddenly, forcing you to adopt a slow, swaying gait.  You find that rolling yours hips back and forth comes naturally to you, and your big, obscene ass seems to jiggle all on its own with every step you take.");
                    player.lowerBody.hipRating = 12;
                }
                else {
                    DisplayText.text("  Your [butt] swells dramatically, the puffy cheeks swelling with newfound weight that jiggles along with each step.  Clenching your glutes to make the posh cheeks jiggle a little more enticingly becomes second nature to you in a few seconds.");
                    player.lowerBody.butt.buttRating = 12;
                }
                DisplayText.text("\n\n");
            }
            ...
            if (!player.perks.has(PerkType.BimboBody)) {
                DisplayText.text("<b>(Bimbo Body - Perk Gained!)\n");
                player.perks.add(PerkFactory.create(PerkType.BimboBody, 0, 0, 0, 0));
            }
            if (!player.perks.has(PerkType.BimboBrains)) {
                DisplayText.text("(Bimbo Brains - Perk Gained!)\n");//int to 20.  max int 50)
                player.perks.add(PerkFactory.create(PerkType.BimboBrains, 0, 0, 0, 0));
