...
export default class LustyMaidensArmor extends Armor {
    ...
    public canUse(player: Player): boolean {
        ...
        if (player.lowerBody.cockSpot.hasCock() || player.lowerBody.balls > 0) {
            DisplayText.text("that it won't fit you ");
            if (player.lowerBody.cockSpot.hasCock())
                DisplayText.text("or your " + CockDescriptor.describeMultiCockShort(player));
            else
                DisplayText.text("or your [balls]");
            DisplayText.text(" at all!  <b>You put your old gear back on with a sigh</b>.");
            return false;
        }
        else if (!player.lowerBody.vaginaSpot.hasVagina()) {
            DisplayText.text("that it will dig uncomfortably into your featureless groin.  <b>You put your old gear back on with a sigh</b>.");
            return false;
        }
        DisplayText.text("your [vagina] is prominently displaying your camel-toe for all to see.");
        ...
        if (player.lowerBody.vaginaSpot.isVirgin()) {
            DisplayText.text("a tight seal over your chastity, displaying your womanly status while guarding your maidenhead at the same time.  A smug, smile tugs at the corners of your mouth - who would take your virginity when they can tit-fuck your tits or fuck your butt?");
            if (player.stats.cor < 33)
                DisplayText.text("  Wait, that isn't right...");
        }
        else {
            DisplayText.text("a tight seal over your previously-claimed cunt.  Regret fills you when you realize you could have kept your chastity intact simply by servicing the lusty studs and monsters with your ass and tits.");
        }
        if (player.lowerBody.vaginaSpot.get(0).vaginalWetness >= 3)
            DisplayText.text("  The moisture you normally drip seems to soak right into the gusset instead of running down your [legs] like normal, giving you a much more chaste appearance in spite of the lewd garments that even now seem to shape your femininity and [butt] into perfectly arousing shapes.");
    ...
    onEquip(character: Character): void {
        ...
        if (character.lowerBody.vaginaSpot.NotVirgin.length > 0) {
            character.perks.add(PerkFactory.create(PerkType.SluttySeduction, 10 + Flags.list[FlagEnum.BIKINI_ARMOR_BONUS], 0, 0, 0));
        }
        else {
            character.perks.add(PerkFactory.create(PerkType.SluttySeduction, 6 + Flags.list[FlagEnum.BIKINI_ARMOR_BONUS], 0, 0, 0));
        }
    }
    ...
    public lustyMaidenPaizuri(player: Player = null, monster: Character = null): void {
        ...
        if (!player.lowerBody.vaginaSpot.isVirgin()) DisplayText.text("a gift too fine for a selfish brute like you");
        else DisplayText.text("sacred and to be shared only with a cherished loved one");
        DisplayText.text("!  Now, I'm going to squeeze all the impure thoughts out of you through your cock, so you just lie there and focus on letting them out all over my breasts.</i>\"");
        ...
        if (monster.lowerBody.balls > 0) DisplayText.text(" and fondle " + monster.desc.possessivePronoun + " balls one-handed, squeezing the virile orbs to try and coax more of " + monster.desc.possessivePronoun + " dirty, perverted thoughts to distill into salty seed");
        else if (monster.desc.short == "anemone") DisplayText.text(" and stroke " + monster.desc.possessivePronoun + " taint, even brushing over the featureless spot where an asshole would be, if she had one, to try and coax more of " + monster.desc.possessivePronoun + " dirty, perverted thoughts to distill into salty seed");
        else DisplayText.text(" and stroke " + monster.desc.possessivePronoun + " taint, even brushing close to " + monster.desc.possessivePronoun + " asshole to try and coax more of " + monster.desc.possessivePronoun + " dirty, perverted thoughts to distill into salty seed");
        ...
        if (monster.lowerBody.cockSpot.get(0).cockArea() >= 80) DisplayText.text("  It's so big and strong that it pushes right into your throat, stretching out your neck in the shape of the intruding cock.");
        DisplayText.text("  The strong, pulsing cock feels so good inside your mouth, like it belongs there, and you can't help but think that you're doing a good deed by helping " + monster.desc.a+ monster.desc.short + " empty every last perverse desire onto your purifying breasts.");
        ...
        if (player.upperBody.chest.hasFuckableNipples() || player.lactationQ() >= 100) DisplayText.text(", dripping,");
        ...
        if (player.lowerBody.vaginaSpot.isVirgin()) DisplayText.text("maidenhead");
        else DisplayText.text("sloppy gash");
        DisplayText.text(".  Still, you're not sure how much hotter you can get before you're cumming all over your g-string, letting your own dark thoughts seep into your magical underwear.");
        ...
        if (monster.lowerBody.cockSpot.get(0).cockArea() >= 80) DisplayText.text(" with a messy cough to clear your throat");
        ...
        if (monster.lowerBody.balls > 0) DisplayText.text(", " + monster.desc.possessivePronoun + " balls pumping and bouncing in " + monster.desc.possessivePronoun + " sack");
