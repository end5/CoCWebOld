...
export default class LustyMaidensArmor extends Armor {
    ...
    public canUse(player: Player): boolean {
        if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < BreastCup.A) { // {No titties}
        ...
        if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < BreastCup.D) { // {Too small titties}
        ...
        if (player.torso.cocks.count > 0 || player.torso.balls.quantity > 0) {
            ...
            if (player.torso.cocks.count > 0)
                DisplayText("or your " + CockDescriptor.describeMultiCockShort(player));
            else
        ...
        if (player.torso.vaginas.filter(Vagina.Virgin).length > 0) {
    ...
    public lustyMaidenPaizuri(player: Player = null, monster: Character = null): void {
        ...
        if (player.torso.vaginas.filter(Vagina.Virgin).length <= 0) DisplayText("a gift too fine for a selfish brute like you");
        ...
        DisplayText(" and tease, \"<i>Oh, you're going to cum already, aren't you?  Well, go ahead then.</i>\"  You pump your [chest] faster against the twitching rod and smile when a thick bead of pre sloughs off into your squishy boobs, smearing across your " + player.skin.desc + ".  You kiss it, licking the dollop that slips out of the dilating cum-slit before commanding, \"<i>Cum for me, " + GenderDescriptor.mf(monster, "boy", "girl") + ".  Let it allll out.</i>\"");
