...
export default class LustyMaidensArmor extends Armor {
	...
	public canUse(player: Player): boolean {
		...
		if (player.lowerBody.cockSpot.hasCock() || player.lowerBody.balls > 0) {
			...
			if (player.lowerBody.cockSpot.hasCock()) MainScreen.text("or your " + player.multiCockDescriptLight());
			else MainScreen.text("or your [balls]");
		...
		if (player.vaginas[0].virgin) {
			...
			if (player.stats.cor < 33) MainScreen.text("  Wait, that isn't right...");
		}
		else {
		...
		if (player.wetness() >= 3) MainScreen.text("  The moisture you normally drip seems to soak right into the gusset instead of running down your [legs] like normal, giving you a much more chaste appearance in spite of the lewd garments that even now seem to shape your femininity and [butt] into perfectly arousing shapes.");
	...
	public lustyMaidenPaizuri(player:Player = null, monster:Monster = null):void {
		...
		if(!player.hasVirginVagina()) MainScreen.text("a gift too fine for a selfish brute like you");
		...
		if(player.hasVirginVagina()) MainScreen.text("maidenhead");
