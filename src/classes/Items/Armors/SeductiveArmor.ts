...
export default class SeductiveArmor extends Armor {
    ...
    public hasNipplePiercing(character: Character): boolean {
        for (const nipple of character.inventory.equipment.piercings.nipples)
    ...
    public useText(character: Character): void {
        if (!Game.scenes.ceraphFollowerScene.ceraphIsFollower()) {
            ...
            const hasNipplePiercing = this.hasNipplePiercing(character);
    ...
    public onEquip(character: Character): void {
        // return Game.libraries.armor.get("COMFORTABLE_UNDERCLOTHES"); //After seductive armor magic the character is left in their underclothes
