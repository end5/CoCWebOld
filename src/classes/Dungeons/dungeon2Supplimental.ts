...
//	inDungeon = false;
	...
	MainScreen.text("You leave the cave behind and take off through the deepwoods back towards camp.");
...
public imtacularMultiHitzilla():void {
	//Multiattack:
	if(monster.findStatusAffect(StatusAffects.Blind) < 0) MainScreen.text("The imps come at you in a wave, tearing at you with claws!\n", false);
	//(ALT BLINDED TEXT)
	else MainScreen.text("In spite of their blindness, most of them manage to find you, aided by the clutching claws of their brothers.\n", false);
	...
	while(hits > 0) {
		...
		if(monster.statusAffects.has("Blind") && rand(3) < 2) {
			MainScreen.text(monster.capitalA + monster.short + " completely misses you with a blind attack!\n", false);
		}
		//Determine if dodged!
		else if(player.stats.spe - monster.spe > 0 && rand(((player.stats.spe-monster.spe)/4)+90) > 80) {
			if(player.stats.spe - monster.spe < 8) MainScreen.text("You narrowly avoid " + monster.a + monster.short + "'s " + monster.weaponVerb + "!\n", false);
			else if(player.stats.spe - monster.spe >= 8 && player.stats.spe-monster.spe < 20) MainScreen.text("You dodge " + monster.a + monster.short + "'s " + monster.weaponVerb + " with superior quickness!\n", false);
			else if(player.stats.spe - monster.spe >= 20) MainScreen.text("You deftly avoid " + monster.a + monster.short + "'s slow " + monster.weaponVerb + ".\n", false);
		}
		//Determine if evaded
		else if(player.perks.has("Evade") && rand(100) < 10) {
			MainScreen.text("Using your skills at evading attacks, you anticipate and sidestep " + monster.a + monster.short + "'s attack.\n", false);
		}
		else if(player.perks.has("Misdirection") && rand(100) < 10 && player.armorName == "red, high-society bodysuit") {
			MainScreen.text("With the easy movement afforded by your bodysuit and Raphael's teachings, you easily avoid " + monster.a + monster.short + "'s attack.\n", false);
		}
		//Determine if cat'ed
		else if(player.perks.has("Flexibility") && rand(100) < 6) {
			MainScreen.text("With your incredible flexibility, you squeeze out of the way of " + monster.a + monster.short + "", false);
			if(monster.plural) MainScreen.text("' attacks.\n", false);
			else MainScreen.text("'s attack.\n", false);
		}
		//OH SHIT SON YOU GOT REAPED
		else {
			if(hits == 6) MainScreen.text("You're clawed viciously by an imp!", false);
			if(hits == 5) MainScreen.text("One bites your ankle!", false);
			if(hits == 4) MainScreen.text("An imp rakes his claws down your back.", false);
			if(hits == 3) MainScreen.text("One of the little bastards manages to scratch up your legs!", false);
			if(hits == 2) MainScreen.text("Another imp punches you in the gut, hard!", false);
			if(hits == 1) MainScreen.text("Your arm is mauled by the clawing!", false);
...
public impGangBukkake():void {
	MainScreen.text("Many of the imps are overcome by the lust you've inspired.  They hover in the air around you, pumping their many varied demonic rods as they bring themselves to orgasm.\n", false);
	...
	while(hits > 0) {
		...
		//Blind dodge change
		if(monster.statusAffects.has("Blind") && rand(3) < 2) {
			MainScreen.text(monster.capitalA + monster.short + "' misguided spooge flies everyone.  A few bursts of it don't even land anywhere close to you!\n", false);
		}
		//Determine if dodged!
		else if(player.stats.spe - monster.spe > 0 && rand(((player.stats.spe-monster.spe)/4)+90) > 80) {
			damage = rand(4);
			if(damage == 0) MainScreen.text("A wad of cum spatters into the floor as you narrowly sidestep it.\n", false);
			else if(damage == 1) MainScreen.text("One of the imps launches his seed so hard it passes clean over you, painting the wall white.\n", false);
			else if(damage == 2) MainScreen.text("You duck a glob of spooge and it passes harmlessly by.  A muffled grunt of disgust can be heard from behind you.\n", false);
			else if(damage == 3) MainScreen.text("You easily evade a blast of white fluid.\n", false);
		}
		//Determine if evaded
			...
			MainScreen.text("(Misdirection) ", false);
			if(damage == 0) MainScreen.text("A wad of cum spatters into the floor as you narrowly sidestep it.\n", false);
			else if(damage == 1) MainScreen.text("One of the imps launches his seed so hard it passes clean over you, painting the wall white.\n", false);
			else if(damage == 2) MainScreen.text("You duck a glob of spooge and it passes harmlessly by.  A muffled grunt of disgust can be heard from behind you.\n", false);
			else if(damage == 3) MainScreen.text("You easily evade a blast of white fluid.\n", false);
		...
		//(2-6 hits for +10 lust each!) (-5 lust per successful hit)
			...
			damage = rand(6);
			if(damage == 0) MainScreen.text("A squirt of hot demonic cum splatters into your face!\n", false);
			if(damage == 1) MainScreen.text("Your " + allBreastsDescript() + " are coated with thick demonic spunk!\n", false);
			if(damage == 2) MainScreen.text("Some of the fluid splatters into your midriff and drools down to your waist, making your " + player.armorName + " slimy and wet.\n", false);
			if(damage == 3) MainScreen.text("Seed lands in your " + hairDescript() + ", slicking you with demonic fluid.\n", false);
			if(damage == 4) MainScreen.text("Another blast of jizz splatters against your face, coating your lips and forcing a slight taste of it into your mouth.\n", false);
			if(damage == 5) MainScreen.text("The last eruption of cum soaks your thighs and the lower portions of your " + player.armorName + ", turning it a sticky white.\n", false);
...
public impGangUber():void {
	if(monster.findStatusAffect(StatusAffects.ImpUber) < 0) {
		MainScreen.text("Three imps on the far side of the room close their eyes and begin murmuring words of darkness and power.  Your eyes widen, recognizing the spell.  Anything but that!  They're building up a massive arousal spell!  They keep muttering and gesturing, and you realize you've got one round to stop them!\n", false);
		...
		//+50 lust!
		dynStats("lus", 50);
		MainScreen.text("The imps in the back finish their spell-casting, and point at you in unison.  A wave of pure arousal hits you with the force of a freight train.   Your equipment rubs across your suddenly violently sensitive " + nippleDescript(0), false);
		if(player.biggestLactation() > 1) MainScreen.text(" as they begin to drip milk", false);
		MainScreen.text(".  The lower portions of your coverings ", false);
		if(player.lowerBody.cockSpot.count() > 0) {
			MainScreen.text("are pulled tight by your " + player.multiCockDescript() + ", ", false);
			if(player.totalCocks() > 1) MainScreen.text("each ", false);
			MainScreen.text("beading a drop of pre-cum at the tip", false);
			if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(", and in addition, the clothes around your groin ", false);
		}
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("become stained with feminine moisture", false);
			if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) MainScreen.text(" as your clit swells up in a more sensitive imitation of a cock", false);
		}
		if(player.gender == 0) MainScreen.text("rub the sensitive skin of your thighs and featureless groin in a way that makes you wish you had a sex of some sort", false);
		MainScreen.text(".\n", false);
...
public loseToImpMob():void {
	...
	//(HP) 
	if(player.HP < 1) MainScreen.text("Unable to handle your myriad wounds, you collapse with your strength exhausted.\n\n", false);
	//(LUST)
	else MainScreen.text("Unable to handle the lust coursing through your body, you give up and collapse, hoping the mob will get you off.\n\n", false);
	
	MainScreen.text("In seconds, the squirming red bodies swarm over you, blotting the rest of the room from your vision.  You can feel their scrabbling fingers and hands tearing off your " + player.armorName + ", exposing your body to their always hungry eyes.   Their loincloths disappear as their growing demonic members make themselves known, pushing the tiny flaps of fabric out of the way or outright tearing through them.   You're groped, touched, and licked all over, drowning in a sea of long tongues and small nude bodies.\n\n", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) {
		MainScreen.text("A sudden weight drops onto your chest as one of the demons straddles your belly, allowing his thick, tainted fuck-stick to plop down between your " + allBreastsDescript() + ".  The hot fluid leaking from his nodule-ringed crown  swiftly lubricates your cleavage.  In seconds the little devil is squeezing your " + breastDescript(0) + " around himself as he starts pounding his member into your tits.  The purplish tip peeks out between your jiggling flesh mounds, dripping with tainted moisture.", false);
		if(player.biggestLactation() > 1) MainScreen.text("  Milk starts to squirt from the pressure being applied to your " + breastDescript(0) + ", which only encourages the imp to squeeze even harder.", false);
		MainScreen.text("\n\n", false);
	}
	//(NIPPLECUNTS!)
	if(player.upperBody.chest.hasFuckableNipples()) {
		MainScreen.text("A rough tweak on one of your nipples startles you, but your grunt of protest is turned into a muffled moan when one of the imp's tiny fingers plunges inside your " + nippleDescript(0) + ".  He pulls his hand out, marveling at the sticky mess, and wastes no time grabbing the top of your tit with both hands and plunging himself in.", false);
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 7) MainScreen.text("  He can only get partway in, but it doesn't seem to deter him.", false);
		else MainScreen.text("  Thanks to your massive bust, he is able to fit his entire throbbing prick inside you.", false);
		MainScreen.text("  The demon starts pounding your tit with inhuman vigor, making the entire thing wobble enticingly.  The others, seeing their brother's good time, pounce on ", false);
		if(player.upperBody.chest.countNipples() > 2) MainScreen.text("each of ", false);
		MainScreen.text("your other " + nippleDescript(0), false);
		if(player.upperBody.chest.countNipples() > 2) MainScreen.text("s", false);
		MainScreen.text(", fighting over the opening", false);
		if(player.upperBody.chest.countNipples() > 2) MainScreen.text("s", false);
		MainScreen.text(".  A victor quickly emerges, and in no time ", false);
		if(player.upperBody.chest.countNipples() == 2) MainScreen.text("both", false);
		else MainScreen.text("all the", false);
		MainScreen.text(" openings on your chest are plugged with a tumescent demon-cock.\n\n", false);
	}
	//(SINGLE PEN) 
	if(!player.lowerBody.vaginaSpot.hasVagina()) {
		MainScreen.text("Most of the crowd centers itself around your lower body, taking a good long look at your " + assholeDescript() + ".  An intrepid imp steps forwards and pushes his member into the unfilled orifice.  You're stretched wide by the massive and unexpectedly forceful intrusion.  The tiny corrupted nodules stroke every inch of your interior, eliciting uncontrollable spasms from your inner muscles.  The unintentional dick massage gives your rapist a wide smile, and he reaches down to smack your ass over and over again throughout the ordeal.", false);
	...
	if(player.totalCocks() > 0) {
		MainScreen.text("Some of the other imps, feeling left out, fish out your " + player.multiCockDescript() + ".  They pull their own members alongside yours and begin humping against you, frotting as their demonic lubricants coat the bundle of cock with slippery slime.   Tiny hands bundle the dicks together and you find yourself enjoying the stimulation in spite of the brutal fucking you're forced to take.  Pre bubbles up, mixing with the demonic seed that leaks from your captors members until your crotch is sticky with frothing pre.\n\n", false);
	}
	//(ORGAZMO)
	MainScreen.text("As one, the crowd of demons orgasm.  Hot spunk gushes into your ass, filling you with uncomfortable pressure.  ", false);
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("A thick load bastes your pussy with whiteness, and you can feel it seeping deeper inside your fertile womb.  ", false);
	MainScreen.text("Your mouth is filled with a wave of thick cream.  Plugged as you are by the demon's knot, you're forced to guzzle down the stuff, lest you choke on his tainted baby-batter.", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) {
		MainScreen.text("  More and more hits your chin as the dick sandwiched between your tits unloads, leaving the whitish juice to dribble down to your neck.", false);
		if(player.upperBody.chest.hasFuckableNipples()) {
			if(player.upperBody.chest.countNipples() == 2) MainScreen.text("  The pair", false);
			else MainScreen.text("  The group", false);
			MainScreen.text(" of cocks buried in your " + nippleDescript(0) + " pull free before they cum, dumping the spooge into the gaping holes they've left behind.  It tingles hotly, making you quiver with pleasure.", false);
		}
	}
	MainScreen.text("  Finally, your own orgasm arrives, ", false);
	if(player.lowerBody.cockSpot.count() == 0) MainScreen.text("and you clench tightly around the uncomfortable intrusion.", false);
	else {
		MainScreen.text("and " + player.sMultiCockDesc() + " unloads, splattering the many demons with a bit of your own seed.  You'd smile if your mouth wasn't so full of cock.  At least you got to make a mess of them!", false);
	}
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		MainScreen.text("  Your cunt clenches around the invading cock as orgasm takes you, massaging the demonic tool with its instinctual desire to breed.  Somehow you get him off again, and take another squirt of seed into your waiting cunt.", false);
	}
	MainScreen.text("\n\n", false);
	
	MainScreen.text("Powerless and in the throes of post-coital bliss, you don't object as you're lifted on the table", false);
	if(!player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(" and forced to start drinking bottle after bottle of succubi milk", false);
	MainScreen.text(".  You pass out just as round two is getting started, but the demons don't seem to mind....", false);
	doNext(loseToImpMobII);
}
//[IMP GANGBANG VOL 2]
public loseToImpMobII():void {
	MainScreen.text("", true);
	MainScreen.text("You wake up, sore from the previous activity and a bit groggy.  You try to move, but find yourself incapable.  Struggling futilely, you thrash around until you realize your arms and legs are strapped down with heavy iron restraints.  You gasp out loud when you look down and discover your ", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) MainScreen.text("new", false);
	else MainScreen.text("much larger", false);
	MainScreen.text(" tits, wobbling with every twist and movement you make.  You're stark naked, save for a sheer and somewhat perverse nurse's outfit.   The room around you looks to be empty, though you can see a number of blankets piled in the corners and a few cages full of spooge-covered faeries, all snoring contently.\n\n", false);
	
	
	MainScreen.text("Eventually a lone imp enters the room.  It's Zetaz!  He looks you up and down and decrees, \"<i>You're ready.</i>\"  You struggle to shout him down, but all that escapes the gag in your mouth is incomprehensible gibberish.  He chuckles and flips a switch on the wall, and suddenly the most heavenly vibration begins within your sopping twat.", false);
	if(!player.lowerBody.vaginaSpot.hasVagina()) {
		MainScreen.text("...Wait, your what?  You have a cunt now!?", false);
	}
	MainScreen.text("  Your eyes cross at the pleasure as your mind struggles to figure out why it feels so good.\n\n", false);
...
public impGangVICTORY():void {
	...
	if(monster.HP < 1) MainScreen.text("The last of the imps collapses into the pile of his defeated comrades.  You're not sure how you managed to win a lopsided fight, but it's a testament to your new-found prowess that you succeeded at all.", false);
	else MainScreen.text("The last of the imps collapses, pulling its demon-prick free from the confines of its loincloth.  Surrounded by masturbating imps, you sigh as you realize how enslaved by their libidos the foul creatures are.", false);
...
public impGangGetsRapedByMale():void {
	MainScreen.text("", true);
	MainScreen.text("You walk around and pick out three of the demons with the cutest, girliest faces.  You set them on a table and pull aside your " + player.armorName + ", revealing your " + player.multiCockDescriptLight() + ".  You say, \"<i>Lick,</i>\" in a tone that brooks no argument.  The feminine imps nod and open wide, letting their long tongues free.   Narrow and slightly forked at the tips, the slippery tongues wrap around your " + cockDescript(0) + ", slurping wetly as they pass over each other in their attempts to please you.\n\n", false);
	
	MainScreen.text("Grabbing the center one by his horns, you pull him forwards until your shaft is pressed against the back of his throat.  He gags audibly, but you pull him back before it can overwhelm him, only to slam it in deep again.  ", false);
	MainScreen.text("The girly imp to your left, seeing how occupied your " + cockDescript(0) + " is, shifts his attention down to your ", false);
	...
	MainScreen.text(", licking with care", false);
	if(player.lowerBody.balls == 0) MainScreen.text(" and plunging deep inside", false);
	MainScreen.text(".  The imp to the right wraps his tongue around the base ", false);
	if(player.lowerBody.cockSpot.hasSheath()) MainScreen.text("just above your sheath ", false);
	MainScreen.text(" and pulls it tight, acting as an organic cock-ring.\n\n", false);
	
	MainScreen.text("Fucking the little bitch of a demon is just too good, and you quickly reach orgasm.  ", false);
	if(player.lowerBody.balls > 0) MainScreen.text("Cum boils in your balls, ready to paint your foe white.  ", false);
	MainScreen.text("With a mighty heave, you yank the imp forward, ramming your cock deep into his throat.  He gurgles noisily as you unload directly into his belly.   Sloshing wet noises echo in the room as his belly bulges slightly from the load, and his nose dribbles cum.   You pull him off and push him away.  He coughs and sputters, but immediately starts stroking himself, too turned on to care.", false);
	if(player.cumQ() > 1000) MainScreen.text("  You keep cumming while the other two imps keep licking and servicing you.   By the time you finish, they're glazed in spooge and masturbating as well.", false);
	MainScreen.text("\n\n", false);
	
	MainScreen.text("Satisfied, you redress and prepare to continue with your exploration of the cave.", false);
	dynStats("cor", 1);
	player.orgasm();
	cleanupAfterCombat();
}
...
public impGangGetsRapedByFemale():void {
	MainScreen.text("", true);
	MainScreen.text("You walk around to one of the demons and push him onto his back.  Your " + player.armorName + " falls to the ground around you as you disrobe, looking over your tiny conquest.  A quick ripping motion disposes of his tiny loincloth, leaving his thick demon-tool totally unprotected. You grab and squat down towards it, rubbing the corrupted tool between your legs ", false);
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLICK) MainScreen.text("and coating it with feminine drool ", false);
	MainScreen.text("as you become more and more aroused.  It parts your lips and slowly slides in.  The ring of tainted nodules tickles you just right as you take the oddly textured member further and further into your willing depths.", false);
	player.cuntChange(15,true,true,false);
	MainScreen.text("\n\n", false);
	
	MainScreen.text("At last you feel it bottom out, bumping against your cervix with the tiniest amount of pressure.  Grinning like a cat with the cream, you swivel your hips, grinding your " + clitDescript() + " against him in triumph.  ", false);
	if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) MainScreen.text("You stroke the cock-like appendage in your hand, trembling with delight.  ", false);
	MainScreen.text("You begin riding the tiny demon, lifting up, and then dropping down, feeling each of the nodes gliding along your sex-lubed walls.   As time passes and your pleasure mounts, you pick up the pace, until you're bouncing happily atop your living demon-dildo.\n\n", false);
	
	MainScreen.text("The two of you cum together, though the demon's pleasure starts first.  A blast of his tainted seed pushes you over the edge.  You sink the whole way down, feeling him bump your cervix and twitch inside you, the bumps on his dick swelling in a pulsating wave in time with each explosion of fluid.  ", false);
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLAVERING) MainScreen.text("Cunt juices splatter him as you squirt explosively, leaving a puddle underneath him.  ", false);
	else MainScreen.text("Cunt juices drip down his shaft, oozing off his balls to puddle underneath him.  ", false);
	MainScreen.text("The two of you lie together, trembling happily as you're filled to the brim with tainted fluids.\n\n", false);
	
	MainScreen.text("Sated for now, you rise up, your body dripping gooey whiteness.  Though in retrospect it isn't nearly as much as was pumped into your womb.", false);
	if (player.pregnancyIncubation == 0) MainScreen.text("  You'll probably get pregnant.", false);
	dynStats("cor", 1);
	player.orgasm();
...
public enterZetazsRoomFromTheSouth():void {
	if (flags[FlagEnum.ZETAZ_DOOR_UNLOCKED] == 0) {
		MainScreen.clearText();
		MainScreen.text("The door won't budge.");
		doNext(playerMenu);
		return;
	}
...
public getSwordAndGetTrapped():void {
	MainScreen.text("", true);
	MainScreen.text("You start to walk over to the corpse and its discarded weapon, but halfway through your journey, the unexpected happens.   The leaf-like petals shift underfoot, snapping up with lightning-quick speed.  You ", false);
	if(player.stats.spe < 50) MainScreen.text("fall flat on your " + assDescript() + ", slipping on the slick, shifting surface.", false);
	else MainScreen.text("stumble and nearly fall, slipping on the shifting, slick surface.", false);
	//[ADD 'CONTINUE' ON]
...
public encapsulationPodAI():void {
	...
	if(monster.findStatusAffect(StatusAffects.Round) < 0) {
		MainScreen.text("You shiver from the feeling of warm wetness crawling up your " + player.legs() + ".   Tentacles brush against your ", false);
		if(player.lowerBody.balls > 0) {
			MainScreen.text(ballsDescriptLight() + " ", false);
		...
		else if(player.lowerBody.balls == 0) MainScreen.text("taint ", false);
		MainScreen.text("as they climb ever-further up your body.  In spite of yourself, you feel the touch of arousal licking at your thoughts.\n", false);
		if(player.lust < 35) {
			dynStats("lus", 1);
			player.lust = 35;
			statScreenRefresh();
	...
	//[Round 2 Action]
		...
		MainScreen.text("The tentacles under your " + player.armorName + " squirm against you, seeking out openings to penetrate and genitalia to caress.  ", false);
		if(player.lowerBody.balls > 0) MainScreen.text("One of them wraps itself around the top of your " + sackDescript() + " while its tip slithers over your " + ballsDescriptLight() + ".  Another ", false);
		else MainScreen.text("One ", false);
		if(player.lowerBody.cockSpot.count() > 0) {
			MainScreen.text("prods your " + cockDescript(0) + " for a second before it begins slithering around it, snake-like.  Once it has you encircled from " + player.cockHead() + " to ", false);
			if(!player.lowerBody.cockSpot.hasSheath()) MainScreen.text("base", false);
			else MainScreen.text("sheath", false);
			MainScreen.text(", it begins to squeeze and relax to a pleasant tempo.  ", false);
		}
		else {
			if(player.lowerBody.vaginaSpot.hasVagina()) {
				MainScreen.text("prods at your groin, circling around your " + vaginaDescript(0) + " deliberately, as if seeking other toys to play with.  ", false);
				if(player.lowerBody.vaginaSpot.list[0].clitLength > 4) MainScreen.text("It brushes your " + clitDescript() + " then curls around it, squeezing and gently caressing it with a slow, pleasing rhythm.  ", false);
			}
			else {
				MainScreen.text("prods your groin before curling around to circle your " + assholeDescript() + " playfully.  The entire tendril pulses in a pleasant, relaxing way.  ", false);
			}
		}
		if(player.lowerBody.cockSpot.count() > 1) {
			MainScreen.text("Your other ", false);
			if(player.lowerBody.cockSpot.count() == 2) MainScreen.text(cockDescript(1) + " gets the same treatment, and soon both of your " + player.multiCockDescriptLight() + " are quite happy to be here.  ", false);
			else MainScreen.text(player.multiCockDescriptLight() + " get the same treatment and soon feel quite happy to be here.  ", false);
		}
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("The violation of your " + vaginaDescript(0) + " is swift and painless.  The fungus' slippery lubricants make it quite easy for it to slip inside, and you find your " + vaginaDescript(0) + " engorging with pleasure in spite of your need to escape.  The tentacle folds up so that it can rub its stalk over your " + clitDescript() + ", ", false);
			if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) MainScreen.text("and once it discovers how large it is, it wraps around it and squeezes.  It feels good!  ", false);
			else MainScreen.text("and it has quite an easy time making your bud grow hard and sensitive.  The constant rubbing feels good!  ", false);
		}
		MainScreen.text("One 'lucky' stalk manages to find your " + assholeDescript() + ".  As soon as it touches your rear 'entrance', it lunges forward to penetrate you.  The fluids coating the tentacle make your muscles relax, allowing it to slide inside you with ease.\n\n", false);
		
		MainScreen.text("The rest of the mass continues to crawl up you.  They tickle at your ", false);
		if(player.pregnancyIncubation > 0 && player.pregnancyIncubation < 120) MainScreen.text("pregnant ", false);
		MainScreen.text("belly as they get closer and closer to ", false);
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) MainScreen.text("your chest", false);
		else MainScreen.text("the underside of your " + allBreastsDescript(), false);
		MainScreen.text(".  Gods above, this is turning you on!  Your lower body is being violated in every conceivable way and it's only arousing you more.  Between the mind-numbing smell and the sexual assault you're having a hard time focusing.\n", false);
		if(player.lust < 65) {
			dynStats("lus", 1);
			player.lust = 65;
			statScreenRefresh();
	...
	//[Round 3 Action]
		...
		MainScreen.text("The wet, warm pressure of the fungus' protrusion working their way up your body feels better than it has any right to be.  It's like a combination of a warm bath and a gentle massage, and when combined with the thought-numbing scent in the air, it's nigh-impossible to resist relaxing a little.  In seconds the mass of tentacles is underneath your " + player.armorName + " and rubbing over your chest and " + nippleDescript(0) + "s.  You swoon from the sensation and lean back against the wall while they stroke and caress you, teasing your sensitive " + nippleDescript(0) + ".", false);
		if(player.upperBody.chest.hasFuckableNipples()) MainScreen.text("  Proof of your arousal leaks from each " + nippleDescript(0) + " as their entrances part for the probing tentacles.  They happily dive inside to begin fucking your breasts, doubling your pleasure.", false);
		MainScreen.text("  Moans escape your mouth as your hips begin to rock in time with the tentacles and the pulsing luminance of your fungus-pod.  It would be easy to lose yourself here.  You groan loudly enough to startle yourself back to attention.  You've got to get out!\n\n", false);
		
		MainScreen.text("The tentacles that aren't busy with your " + allBreastsDescript() + " are already climbing higher, and the slime has reached your waist.  If anything it actually makes the constant violation more intense and relaxing.  You start to sink down into it, but catch yourself and pull yourself back up.  No! You've got to fight!\n", false);
		if(player.lust < 85) {
			dynStats("lus", 1);
			player.lust = 85;
			statScreenRefresh();
	...
	//[Round 4 Action]
		...
		MainScreen.text("What's happening to you definitely isn't rape.  Not any more.  You like it too much.  You lean back against a wall of the pod and thrust your " + hipDescript() + " pitifully against a phantom lover, moaning lewdly as you're forcibly pleasured.  You grab hold of the fleshy walls with your hands and try to hold yourself up, but your " + player.legs() + " have the consistency of jello.   They fold neatly underneath you as you slide into the ooze and begin to float inside it.  It's comforting in an odd way, and while you're gasping in between moans, your balance finally gives out.  You sink deeper into the fluid and lose all sense of direction.  Up and down become meaningless constructs that no longer matter to you.\n\n", false);
		
		MainScreen.text("The thick slime passes over your lips and nose as you sink into the rising tide of bliss, and you find yourself wondering how you'll breathe.  Instinctively, you hold your breath.  Even riddled with sexual bliss and thought-obliterating drugs, you won't let yourself open your mouth when 'underwater'.  The lack of oxygen makes your heart hammer in your chest", false);
		if(player.totalCocks() > 0) {
			MainScreen.text(", and " + player.sMultiCockDesc() + " bloats with blood, getting larger than ever", false);
		}
		MainScreen.text(".  Before you can pass out, the constant penetration forces a moan from your lips.\n\n", false);
		
		MainScreen.text("A tentacle takes the opportunity to slip into your mouth along with a wave of the slime.  You try to cough out the fluid, but there isn't any air left in your lungs to push it out.  The orally-fixated tendril widens and begins to pour more of it inside you, and with nowhere else to go, it packs your goo-filled lungs to the brim before you start to swallow.  You relax and exhale the last of your air from your nose as your body calms itself.  Somehow you can breathe the fungus-pod's fluids!\n\n", false);
		
		MainScreen.text("You're floating in pure liquid bliss.  Thoughts melt away before they can form, and every inch of your body is being caressed, squeezed, or penetrated by the warm, slime-slicked tentacles.  Nearly every muscle in your body goes completely slack as you're cradled with bliss.  Without your thoughts or stress bothering you, the pleasure swiftly builds to a crescendo.\n\n", false);
		
		MainScreen.text("The wave of need starts out inside your crotch, begging to be let out, but you can't even be bothered to move your " + hipDescript() + " anymore.  Without your help, release stays just out of reach, but the tentacles working your body seem intent on spurring it on.  The one inside your " + assholeDescript() + " begins to pump more quickly, and with the added pressure, you cum quickly.  ", false);
		if(!player.lowerBody.vaginaSpot.hasVagina()) {
			MainScreen.text("Your body twitches weakly, too relaxed to move while it gets off from anal penetration.", false);
		}
		else MainScreen.text("Your body twitches weakly, too relaxed to move while it gets off from being double-penetrated.", false);
		if(player.upperBody.chest.hasFuckableNipples()) {
			MainScreen.text("  Your " + nippleDescript(0) + "s squirt around their phallic partners, leaking sexual lubricant ", false);
			if(player.biggestLactation() > 1) MainScreen.text("and milk ", false);
			MainScreen.text("while the fucking continues.", false);
		}
		if(player.lowerBody.cockSpot.count() > 0) {
			MainScreen.text("  The tentacles around " + player.sMultiCockDesc() + " squeeze and rotate, screwing you silly through your orgasm while cum dribbles in a steady stream from your loins.  Normally it would be squirting out in thick ropes, but the muscle-relaxing drugs in your system make the spurts a steady, weak flow.", false);
			if(player.cumQ() > 800) MainScreen.text("  Of course with all the semen you produce, the flesh-pod's ooze clouds over quite quickly, blocking your vision with a purple-white haze.", false);
		}
		if(player.biggestLactation() > 1) {
			MainScreen.text("Milk leaks out too, ", false);
			if(player.biggestLactation() < 2) MainScreen.text("though the slight dribble is barely noticeable to you.", false);
			else if(player.biggestLactation() < 3) MainScreen.text("coloring things a little more white.", false);
			else MainScreen.text("thickening your fluid-filled prison with nutrients.", false);
		}
...
public loseToThisShitPartII():void {
	...
	//[OPTIONAL CUM ESCAPE]
	if(player.cumQ() > 3500) {
		MainScreen.text("Your orgasm drags on for so long that you begin to feel pressure from the cum-slime surrounding you.  It doesn't seem to matter to " + player.sMultiCockDesc() + ", which is too busy sending bliss to your brain and squirting cum for the tentacles to care.  It actually kind of hurts.  The oscillating purple ambiance flashes brighter in protest for a second, and then everything releases all at once.  The pressure is gone and you're sliding down on a wave of fungal-slime cum, feeling the tentacles being pulled from you by the sudden shift of position.  Moist cave air tickles at your " + player.skinDesc + " as you come to rest on another spongy petal and begin to cough out the sludge.\n\n", false);
		
		MainScreen.text("Over the next minute your head clears and your strength returns.  You push yourself up on something hard, then glance down and realize you washed up next to the skeleton!  The bleached bone leers up at you knowingly, and everything you can see is covered in a thick layer of your spooge.  " + player.SMultiCockDesc() + " is still dripping more spunk.  Clearly your ruined orgasm didn't pump it ALL out.  You look down at the rapier and pick it up out of your mess, examining it.  The blade shines keenly, and the sword is balanced to perfection.  Though you succumbed to the same fate as its owner, your warped body saved you from sharing his fate.  Thankfully potential pods that carpet the floor don't even twitch at you.  Perhaps your orgasm was enough to sate them all?  Or maybe they've learned their lesson.", false);
	...
	//[OPTIONAL MILK ESCAPE]
		...
		MainScreen.text("Your milk-spouting " + nippleDescript(0) + "s continuously pour your breast-milk into the soupy fluids surrounding you.  Once you let down your milk, there was no stopping it.  Pressure backs up inside the flesh-pod, pressing down on you with near painful intensity, but your " + allBreastsDescript() + " refuse to give up or slow down.  Even though each squirt jacks up the force on your body, your unholy milk production will not relent.  The oscillating purple ambience flashes bright in protest, then gives out entirely, along with the pressure.  At once you're pulled away by a wave of milk-laced fungus-slime, yanking the tentacles away from your body with the change in position.\n\n", false);
	...
	if(player.gender == 1 || (player.gender == 3 && rand(2) == 0)) {
		MainScreen.text("The orgasm squirts and drips from " + player.sMultiCockDesc() + " for what seems like forever.  It feels so right, so perfect, that you actually whine in disappointment when it finally does end.  You can't even be bothered to reach down and stroke yourself.  The softening in your loins is nothing compared to your flaccid, listless muscles.  You couldn't make your arms reach down to touch yourself even if you could work up the motivation to try.  Thankfully the slippery tentacles curl back around your ", false);
		if(!player.lowerBody.cockSpot.hasSheath()) MainScreen.text("base", false);
		else MainScreen.text("sheath", false);
		MainScreen.text(" and squeeze, forcing " + player.sMultiCockDesc() + " to inflate to readiness.  Deep inside your " + assholeDescript() + ", the tentacle starts to rub against your prostate.  It caresses the male organ on each side and pauses to squeeze the center of it, pushing a few drops of sticky cum from your trembling " + Appearance.cockNoun(CockType.HUMAN) + ".\n\n", false);
		
		MainScreen.text("The vine-like stalks currently hugging " + player.sMultiCockDesc() + " constrict the base and begin to swirl around it in a circular motion.  Warm fungi-flesh and viscous, drugged ooze work together to send hot spikes of pleasure up your spinal-cord.  Despite your recent orgasm, you aren't being given any chance to recover or refill your ", false);
		if(player.lowerBody.balls > 0) MainScreen.text("balls", false);
		else MainScreen.text("prostate", false);
		MainScreen.text(".  Things like logic and rest don't matter in this warm, soupy environment, at least not to your poor, unthinking mind and erect, sensitive dick", false);
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s", false);
		MainScreen.text(".  With such stimulation coming so closely on the heels of your last orgasm, [eachCock] is suffering painful levels of pleasure.  Your whole body shakes from the sensory overload; though with your muscles so completely shut down, it's more of a shiver.\n\n", false);
		
		MainScreen.text("Another wave of sperm begins the slow escape from your helpless, pinned form, drawn out by the fungus' constant sexual ministrations.  The fluid inside your pod gurgles noisily as the fluids are exchanged, but the sensory input doesn't register to your overloaded, drugged-out shell of a mind.  You've lost yourself to mindless pleasure, and repeated, endless orgasms.  The rest of your life is spent floating in an artificial womb, orgasming over and over to feed your fungus prison, and enjoying the pleasure that long ago eroded your ability to reason.", false);
		gameOver();
	}
	//(FEM)
	else {
		MainScreen.text("You orgasm around the tentacles in your " + vaginaDescript(0) + " and " + assholeDescript() + " for what feels like hours, though some dim, half forgotten whisper of your mind tells you it can't possibly have gone on for that long.  It feels so right and so perfect that resistance is almost a foreign concept to you at this point.  How could you have tried to fight off this heaven?  You're completely limp, totally helpless, and happier than you ever remember.  The pulsing lights of your womb-like prison continue their steady beat in time with the tentacles buried in your snatch, soothing you while your body is played like a violin heading towards its latest crescendo.\n\n", false);
	
		MainScreen.text("The steady rhythm of your penetration sends rockets of bliss-powered pleasure up your spinal cord and straight into your brain, where it explodes in orgasm.  Your body barely twitches, too relaxed to work up any muscle response, involuntary or otherwise.  A moment to rest never presents itself.  The cruel fungus never relents.  It never slows, unless it's only the briefest pause to intensify the next thrust.  Were you in the open air, away from the strange fluid you're now breathing, you'd be twisting and screaming with pleasure.  Instead you float and cum in silence.\n\n", false);
...
public encapsulationVictory():void {
	if(monster.HP <= 0) {
		flags[FlagEnum.ZETAZ_FUNGUS_ROOM_DEFEATED]++;
		MainScreen.text("", true);
		MainScreen.text("The pod's wall bursts under your onslaught.  The strength goes out of the tentacles holding you at once, giving them all the power of a limp noodle.  The spongy surface of the pod gives out, and the 'petals' split apart, falling down to the ground with a heavy 'thwack'.  You stand there, exulting in your freedom.  You've won!\n\nThe rapier you approached originally still lies there, and you claim your prize.", false);
	}
	cleanupAfterCombat();
}
...
public healVala():void {
	...
	//Pearl!
		...
		MainScreen.text("A soft, warm breeze rustles your " + hairDescript() + " and for a moment the foul stench of the dungeon vanishes, setting your mind at ease and draining the tension that has been building in your gut. In a moment of clarity, you remember the beautiful Pure Pearl that Marae granted you as a reward for shutting down the demons' factory. It seems only right to use the goddess' gift to rescue one of her wayward children. Perhaps she gave it to you for this very reason? The oblivious girl has managed to work her way to a sitting position and is grinding her dripping sex against your " + player.foot() + ". You lean down and gently lift her chin up, bringing her empty, pink eyes to stare into yours. Mistaking the gentle motion for a command, she gleefully opens wide, tongue thrashing about in anticipation. You place the pink pearl at the fairy's lips and she wraps her mouth around the pale jewel, trying obediently to swallow it. However, the little fairy's mouth is far smaller than you would've thought and it seems to get stuck just behind her teeth, like a pink ball-gag. She coos a muffled moan and begins to masturbate without breaking eye contact with you.\n\n", false);
...
public ValaGetsSexed():void {
	...
	//(Herm/Male)
	if(player.totalCocks() > 0) {
		MainScreen.text("The fairy's grinding and the sweet scent leaking out of her honey pot is getting under your skin, irritating an itch that's been building for a while now. Surely it wouldn't hurt to throw the mind-broken girl a pity fuck? It's not like she's one of the demons or anything. You push the girl back, gently and nod your head, stripping your " + player.armorName + " piece by piece, teasing the fairy with your slowness. As you disrobe, you glance back at the spot she'd been chained up and see the placard on the wall again. \"<i>Vala?</i>\" you ask. \"<i>Is that your name?</i>\" She stares blankly, unresponsive.\n\n", false);

		MainScreen.text("She points at herself. \"<i>Bitch,</i>\" she explains. You grab her hands and stare into her pink eyes, holding her in your gaze until the faintest sparkle of light returns to their empty depths.\n\n", false);

		MainScreen.text("\"<i>Your name is Vala,</i>\" you command, softly. She flinches. \"<i>Say it, or I won't fuck you.</i>\" She squirms, eyes clenched, rubbing her knees together as the heat in her groin battles the conditioned rape the imps used to crush her identity. Just as you knew it would, lust wins out.\n\n", false);
...
public loseToVala():void {
	...
	if(player.gender == 0) {
		MainScreen.text("", true);
		MainScreen.text("Vala forces a bottle into your throat before your defeated form has a chance to react, and you grunt with pleasure as a new gash opens between your " + player.legs() + "!", false);
		player.createVagina();
		player.gender = 2;
...
public loseToValaAsMale():void {
	...
	MainScreen.text("You are powerless to stop the fairy as she drags you to the south wall and up to the wooden rail secured a couple of feet off the ground. \"<i>When she was still growing, Bitch was too small and tight for the masters,</i>\" your captor tells you. \"<i>They blessed her with this ladder to make us big enough. You will feel their generosity.</i>\" Gripping you under the arms, the fairy's lust-fuelled strength lifts you off the ground and flies you directly over the bristling peg ladder.\n\n", false);
	//[Next]
...
public fightValaVictory():void {
	spriteSelect(85);
	MainScreen.text("", true);
	MainScreen.text("The fairy girl collapses, well-drilled obedience robbing her limbs of their fight. She squirms to a crouching bow, fully accepting you as her new " + player.mf("Master","Mistress") + ". The warped fae's empty eyes look up at you, her face a mask of rapture as she anxiously awaits her punishment, wagging her butt in the air as lubrication gushes down her thighs. It seems being defeated has excited the broken creature to a breeding frenzy. Her endurance must be incredible to be this frisky after your battle.", false);
...
public leftValaAlone():void {
	...
	//[Heal][Use][Wake][Leave]
	menu();
	MainScreen.addButton(0, "Fix Her", tryToHealVala);
	if (player.gender > 0) {
		MainScreen.addButton(1, "Use", useValaOHYEAHSNAPINTOASLIMJIM);
		MainScreen.addButton(2, "Wake", wakeValaUpBeforeYouGoGo);
	}
	if (player.lust >= 33 && shouldraFollower.followerShouldra()) MainScreen.addButton(3, "ShouldraVala", shouldraFollower.shouldraMeetsCorruptVala);
...
public tryToHealVala():void {
	...
	if(!(player.hasItem(consumables.PURHONY,1) || player.hasItem(consumables.P_PEARL,1))) {
		MainScreen.text("You try your best with what you've got, but nothing seems to restore the broken fairy's mind to her sex-addled  body. You're going to have to go out and gather more materials. Surely there's something that can break the damage the imps have done to Vala.", false);
		doNext(playerMenu);
	}
	//(With Pure Honey)
	else if(player.hasItem(consumables.PURHONY,1)) {
		player.consumeItem(consumables.PURHONY,1);
		MainScreen.text("You're not sure if Pure Honey will do the trick, but it seems like the most likely candidate. You set the broken girl down and step over to the alchemy table. Vala clings onto your leg as you walk, and you end up dragging her across the dungeon floor. Before things can get too out of hand with the needy girl, you pull out the vial of Pure Honey and arrange the equipment in front of you. Using the cleanest of the pipettes, you take a small portion of the honey and mix it with what you hope to be water, diluting the rich mixture to a more viscous state. Working quickly, you manage to produce a draught that the weak girl can tolerate. By now, she's managed to work her way to a sitting position and is grinding her dripping sex against your shin. You lean down and hold her nose to make her open her mouth. She gleefully opens wide, tongue thrashing about in anticipation. You pour the sweet-smelling concoction down her anxious throat and begin to re-cork the rest of your honey.\n\n", false);
...
public wakeMeUpBeforeValaGoGosPtII():void {
	...
	if(player.gender == 1) {
		MainScreen.text("", true);
		MainScreen.text("You spot an unsecured set of chained manacles on the floor and an idea strikes you. Vala still sliding along your shaft, you bend down and grab the fetters, snapping one around the girl's neck like a collar. With a shudder at the cold, you lift the fairy up and lock the other end of the shackles around the root of your " + cockDescript(0) + ", steel snapping around your base tightly. The makeshift cock-ring works perfectly as your " + hipDescript() + " quiver, your body trying to orgasm but denied release by the metal loop. The fairy, meanwhile, thrashes atop your groin, the chain of her collar swinging between her tits, buffeting them with enough force to spray small white streams as she rides you. Her purple hair glimmers pink in the dull light of the dungeon as it bounces right along with her rocking hips. Even the diminished pulses of sunlight streaming from her stained and tattooed skin seems brighter as she is filled labia to cervix by your straining fuckpole.\n\n", false);
...
public purifiedFaerieBitchBar():boolean {
	...
	if (player.gender > 0) {
		MainScreen.text("  You could speak with her if you wished to.");
		return true;
	}
	MainScreen.text("  Speaking to her might be better after you get a gender...");
	return false;
}
...
public chooseValaInBar():void {
	...
	if(flags[FlagEnum.ENCOUNTERED_VALA_AT_BAR] == 0) {
		...
		if(flags[FlagEnum.WEEKLY_FAIRY_ORGY_COUNTDOWN] == 0)
			MainScreen.addButton(1,"Faeries",faerieOrgyFuck);		
	}
	if(flags[FlagEnum.SHOULDRA_MET_VALA] > 0 && shouldraFollower.followerShouldra())
		MainScreen.addButton(3,"Big You",valaBigYou);
	MainScreen.addButton(0,"You",cleansedValaRepeatBrainFucking);
	MainScreen.addButton(2, "Cum Bath", cumBath);
...
public cleansedValaRepeatBrainFucking():void {
	spriteSelect(60);
	MainScreen.text("", true);
	MainScreen.text("She flashes a beautiful smile and tells the bartender that she'll be taking her break early. Grabbing you by the hand, she rushes to the stockroom and sheds her dress without any trace of her former shame about her tattooed back. When you're too slow taking off your " + player.armorName + ", she helps, deft hands made all the quicker by eagerness. When the two of you are naked, she pushes you onto the well-worn stool and sits in your lap, staring into your eyes with a small smile.", false);
...
public faerieOrgyFuckMaleContinue():void {
	...
	//[Balls- 
	if(player.lowerBody.balls > 0) MainScreen.text("Vala's tongue caresses your " + ballsDescriptLight() + ", slurping hungrily, eager to get every drop out of your overburdened sac.  ", false);
	MainScreen.text("You nearly forget to breathe as jizz pours from your body until the dick-shaped shell is entirely forced from your body. Deftly, Vala catches the device before it can spill its load and she pastes her own, much larger, petal on the base, sealing your spunk inside. The verdant shaft, now that it's off your cock, seems to be a dildo perfectly molded to resemble your " + cockDescript(0) + ". It's a soft pink color, like engorged genitals, and even gently pulses with your every heartbeat.", false);
	//(Extremely high cum production: 
	if(player.cumQ() > 2000) MainScreen.text("  Even after driving the condom off of you, your cock continues its gushing orgasm, spurting thick bubbles of jizz into the air, raining down in creamy torrents. The small fairies gleefully dive after the cum globs, making a game of it, trying to catch the gooey globs before they splatter on the bed or carpet.", false);
	MainScreen.text("\n\n", false);

	MainScreen.text("\"<i>You see?</i>\" Vala asks, holding the organic device aloft with a mischievous smile. \"<i>They don't last forever, but while they do, these little toys give us a very intimate connection to loved ones. This way, I can go all week with a reminder of you inside me.</i>\" She gives you a kiss on the lips and the fairies give you a tiny chorus of applause for the entertaining show. It's good that her little friends aren't around more often, you pant to yourself, or you'd be a drooling vegetable in no time.\n\n", false);
	cheatTime(1);
	player.orgasm();
	doNext(telAdre.barTelAdre);
}
...
public faerieOrgyFuckFemaleContinue():void {
	spriteSelect(60);
	MainScreen.text("", true);
	MainScreen.text("The fairy girl lands at the edge of the bed, settling on her knees and resting her weighty chest on the mattress. Leaning in, she spreads your " + player.legs() + " with warm, delicate hands, fingertips stroking the insides of your thighs. You shiver and hug your arms under your breasts, a little nervous about this 'forest magic.' One of the fairies flutters down to Vala and hands her a pale white lily petal with a tittering giggle. The larger fairy takes the soft white bloom and, whispering a silken word to it, licks the cream-colored surface. Slithering further between your legs, she reaches your " + vaginaDescript(0) + " and brushes the petal against your " + clitDescript() + " with a teasing caress. Then, much to your surprise, she places it on the tip of her forefinger and slides it into your pussy. You gasp, the petal's soft surface rubbing your inner walls with a warmth you were unprepared for. Vala presses lightly on a spot inside your vagina and you lose your breath as the tremor of a small orgasm rumbles through your gut. Without explanation, the fairy barmaid takes a rose petal and repeats the process, another orgasm seizing you and leaving you panting. \"<i>Normally we only need the one petal, but since you and I are so much larger than the average fairy, I had my sisters bring lots. All for you,</i>\" she whispers, coyly.\n\n", false);
	
	//[If the Player has unlocked Scylla's Addiction Counseling event] 
	if(flags[FlagEnum.TIMES_SCYLLA_ADDICT_GROUP_EXPLOITED] + flags[FlagEnum.SCYLLA_TIMES_SHARED_IN_ADDICT_GROUP] + flags[FlagEnum.SCYLLA_MILK_THERAPY_TIMES] + flags[FlagEnum.SCYLLA_CUM_THERAPY_TIMES] + flags[FlagEnum.SCYLLA_SEX_THERAPY_TIMES] > 0) MainScreen.text("Another fairy flies toward Vala and you blink through the building lusty haze when you recognize the little girl. Wearing a slightly slutty crystalline gown of translucent gossamer, you recognize Pastie, the fairy from Scylla's addiction counseling meeting. When you call out to her, she bashfully waves back. \"<i>Hey, uh, " + player.short + ". It's kinda weird seeing you outside of the meetings. How are you? How have things been?</i>\" She looks at your naked, reclined form on the bed and blushes. \"<i>Oh, right, right. Um. I'm just helping out Vala, you know. Sisters and all. Uh. I'm... I'm going to just give her this petal and get something to drink, okay? All this sobriety is making this really uncomfortable for me.</i>\" The other fairies laugh and Pastie hands off her blossom before zipping out of the room, probably to steal someone's drink downstairs.\n\n", false);
...
public zetazTaunt():void {
	if(monster.findStatusAffect(StatusAffects.round) < 0) {
		...
		MainScreen.text("Zetaz asks, \"<i>Do you even realize how badly you fucked up my life, ", false);
		if(player.humanScore() >= 4) MainScreen.text("human", false);
		else MainScreen.text("'human'", false);
		MainScreen.text("?  No, of course not.  That's the kind of attitude I'd expect from one of you!</i>\"", false);
	}
	else {
		monster.addStatusValue(StatusAffects.round,1,1);
		if(monster.statusAffects.get("round").value1 == 2) MainScreen.text("\"<i>I lost my post!  And when you screwed up the factory?  I barely escaped with my life!  You ruined EVERYTHING!</i>\" screams Zetaz.", false);
		else if(monster.statusAffects.get("round").value1 == 3) MainScreen.text("Zetaz snarls, \"<i>Do you know how hard it is to hide from Lethice?  DO YOU HAVE ANY IDEA!?  I've had to live in this fetid excuse for a jungle, and just when I found some friends and made it livable, you show up and DESTROY EVERYTHING!</i>\"", false);
		else if(monster.statusAffects.get("round").value1 == 4) MainScreen.text("Zetaz explains, \"<i>I won't let you go.  I'm going to break you.</i>\"", false);
		else if(monster.statusAffects.get("round").value1 == 5) MainScreen.text("\"<i>Would it have been that bad to go along with me?  You've seen the factory.  We would've kept you fed, warm, and provided you with limitless pleasure.  You would've tasted heaven and served a greater purpose.  It's not too late.  If you come willingly I can make sure they find a good machine to milk you with,</i>\" offers the imp lord.", false);
		else if(monster.statusAffects.get("round").value1 == 6) MainScreen.text("\"<i>Why won't you fall?</i>\" questions Zetaz incredulously.", false);
		else if(monster.statusAffects.get("round").value1 == 7) MainScreen.text("The imp lord suggests, \"<i>If you give up and let me fuck your ass maybe I'll let you go.</i>\"", false);
		else if(monster.statusAffects.get("round").value1 == 8) MainScreen.text("Zetaz pants, \"<i>Just give up!  I'm nothing like the weakling you met so long ago!  I've been through hell to get here and I'm not giving it up just because you've shown up to destroy my hard work!</i>\"", false);
		else MainScreen.text("He glares at you silently.", false);
	}
}
...
public zetazAI():void {
	...
	if(monster.lust > 50 && (monster.statusAffects.has("Fear") || monster.statusAffects.has("Blind"))) {
		...
		monster.lust -= 10;
		MainScreen.text("Zetaz blinks and shakes his head while stroking himself.  After a second his turgid member loses some of its rigidity, but his gaze has become clear.  He's somehow consumed some of his lust to clear away your magic!", false);
	}
	
	//STANDARD COMBAT STATUS AFFECTS HERE
	if(monster.statusAffects.has("Stunned")) {
		MainScreen.text("Your foe is too dazed from your last hit to strike back!", false);
	...
	if(monster.statusAffects.has("Constricted")) {
		...
		if(monster.statusAffects.get("Constricted").value1 <= 0) {
			MainScreen.text("  " + monster.capitalA + monster.short + " proves to be too much for your tail to handle, breaking free of your tightly bound coils.", false);
	...
	if(monster.lust > 50 && monster.HPRatio() <= .5) {
		...
		if(attackChoice == 0) {
			...
			MainScreen.text("Zetaz grabs a bottle from a drawer and hurls it in your direction!  ", false);
			if((player.perks.has("Evade") && rand(4) == 0) ||
				...
				MainScreen.text("You sidestep it a moment before it shatters on the wall, soaking the tapestries with red fluid!", false);
			}
			else {
				MainScreen.text("You try to avoid it, but the fragile glass shatters against you, coating you in sticky red liquid.  It seeps into your " + player.skinDesc + " and leaves a pleasant, residual tingle in its wake.  Oh no...", false);
			...
			MainScreen.text("The imp leaps into the air with a powerful spring, beating his wings hard to suspend himself in the center of his bedchamber.  Dust kicks up into the air from the force of his flight and turns the room into a blinding tornado!  Small objects smack off of you, ", false);
			//(causing little damage/
			if(player.tou > 60) MainScreen.text("causing little damage", false);
			else {
				let dmg:number = 1 + rand(6);
				dmg = takeDamage(dmg);
				MainScreen.text("wounding you slightly (" + dmg + ")", false);
				
			}
			MainScreen.text(" while the dust gets into your eyes, temporarily blinding you!", false);
...
public gigaArouse():void {
	MainScreen.text("You see " + monster.a + monster.short + " make familiar arcane gestures at you, but his motions seem a lot more over the top than you'd expect from an imp.\n\n", false);
	dynStats("lus", rand(player.stats.lib/10)+player.stats.cor/10+15);
	if(player.lust < 30) MainScreen.text("Your nethers pulse with pleasant warmth that brings to mind pleasant sexual memories.  ", false);
	if(player.lust >= 30 && player.lust < 60) MainScreen.text("Blood rushes to your groin in a rush as your body is hit by a tidal-wave of arousal.  ", false);
	if(player.lust >= 60) MainScreen.text("Your mouth begins to drool as you close your eyes and imagine yourself sucking off Zetaz, then riding him, letting him sate his desires in your inviting flesh.  The unnatural visions send pulses of lust through you so strongly that your body shivers.  ", false);
	if(player.lowerBody.cockSpot.count() > 0) {
		if(player.lust >= 60 && player.lowerBody.cockSpot.count() > 0) MainScreen.text("You feel " + player.sMultiCockDesc() + " dribble pre-cum, bouncing with each beat of your heart and aching to be touched.  ", false);
		if(player.lust >= 30 && player.lust < 60 && player.lowerBody.cockSpot.count() == 1) MainScreen.text(player.SMultiCockDesc() + " hardens and twitches, distracting you further.  ", false);
	}
	if(player.lowerBody.vaginaSpot.count() > 0) {
		if(player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.NORMAL && player.lowerBody.vaginaSpot.count() == 1) MainScreen.text("Your " + allVaginaDescript() + " dampens perceptibly, feeling very empty.  ", false);
		if(player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.WET && player.lowerBody.vaginaSpot.count() > 0) MainScreen.text("Your crotch becomes sticky with girl-lust, making it clear to " + monster.a + monster.short + " just how welcome your body finds the spell.  ", false);
		if(player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLICK && player.lowerBody.vaginaSpot.count() == 1) MainScreen.text("Your " + allVaginaDescript() + " becomes sloppy and wet, dribbling with desire to be mounted and fucked.  ", false);
		if(player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.DROOLING && player.lowerBody.vaginaSpot.count() > 0) MainScreen.text("Thick runners of girl-lube stream down the insides of your thighs as your crotch gives into the demonic magics.  You wonder what " + monster.a + monster.short + "'s cock would feel like inside you?  ", false);
		if(player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING && player.lowerBody.vaginaSpot.count() == 1) MainScreen.text("Your " + allVaginaDescript() + " instantly soaks your groin with the heady proof of your need.  You wonder just how slippery you could " + monster.a + monster.short + "'s dick when it's rammed inside you?  ", false);
	}
...
public defeatZetaz():void {
	...
	MainScreen.text("", true);
	//[VICTORY HP]
	if(monster.HP < 1) MainScreen.text("Zetaz sinks down on his knees, too wounded to continue.  He looks up at you with helpless rage in his eyes and asks, \"<i>Are you satisfied now?  Go ahead then, kill me.  My life hasn't been worth living since I met you anyway.</i>\"\n\n", false);
	//[VICTORY LUST] 
	else MainScreen.text("Zetaz sinks down on his knees and fishes his massive, pre-drooling member from under his loincloth.  He looks up at you, nearly crying and moans, \"<i>Why? Ruining my life wasn't enough?  You had to make me jerk off at your feet too?  Just kill me, I don't want to live anymore.</i>\"\n\n", false);
	
	//[Both] 
	MainScreen.text("He can't die yet.  You need to know where his master, this 'Lethice', is.  It sounds like she's the queen-bitch of the demons, and if you're going to break this vicious cycle", false);
	//( or take her place)
	if(player.stats.cor > 66) MainScreen.text(" or take her place", false);
	MainScreen.text(", you need to find her and bring her down.  What do you do?", false);
...
public releaseZForInfo():void {
	MainScreen.text("", true);
	MainScreen.text("You look the pathetic imp up and down and smirk.  He closes his eyes, expecting a summary execution, but you present him with an offer instead.  If he gives you more information on Lethice and where to find her, you'll let him go scot-free and avoid him if he doesn't make a nuisance of himself.\n\n", false);
	
	MainScreen.text("\"<i>Really?</i>\" questions Zetaz in a voice laced with suspicion. \"<i>For fuck's sake, I'm already a renegade.  I'll take your deal.  It's not like it costs me anything I wouldn't give away for free anyway.</i>\"\n\n", false);
	
	MainScreen.text("Invigorated by the promise of safety and freedom, Zetaz pulls himself up and ", false);
	if(monster.HP < 1) MainScreen.text("staggers", false);
	else MainScreen.text("nearly stumbles over his lust-filled cock", false);
	MainScreen.text(" towards a desk.  His dextrous fingers twist the knob on the top drawer expertly until a quiet 'click' comes from the furniture.  He reaches down to the divider between the drawers and pulls on it, revealing a tiny, hidden compartment.  In the center of it is a detailed map of the mountain and its upper reaches.  Though the secret diagram is quite crude, it depicts a winding trail that bypasses numerous harpy nests, minotaur caves, and various unrecognizable pitfalls to reach the cloud-shrouded mountain peak.  The drawing loses much of its detail once it gets to the demon fortifications at the top, but it can't be that hard to track down Lethice once you've entered the seat of her power, can it?\n\n", false);
	
	MainScreen.text("A loincloth flies across the room and deposits itself on your shoulder, startling you from your planning.  You glance back and see Zetaz tearing through his possessions, tossing his most prized items into a burlap sack with reckless abandon.  His whole body is trembling, as he ties it to a wooden pole, never once looking up at you.  Perhaps he fears you might change your mind?  ", false);
	if(player.stats.cor > 66) {
		MainScreen.text("You smirk down at him and fold your arms over your ", false);
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) MainScreen.text("chest", false);
		else MainScreen.text(breastDescript(0), false);
		MainScreen.text(", relishing his fear while you consider the possibilities", false);
	}
	else if(player.stats.cor > 33) {
		MainScreen.text("You chuckle with amusement and watch the little bastard scrabble to pack up his life, relishing the chance to pay him back for your previous encounter", false);
	}
	else {
		MainScreen.text("You sigh and rub at your temples as the little jerk scrabbles to pack his life away.  In spite of yourself, you actually feel a little bad about the situation", false);
	}
	MainScreen.text(".  Zetaz scrambles out the south door, never once looking back at the tattered remnants of his old home.", false);
...
public sexualInterrogation():void {
	MainScreen.text("", true);
	MainScreen.text("You lean down until your face hovers over Zetaz, looking him square in the eyes, and explain, \"<i>I can't have someone who knows the way to the demons' headquarters dying before they tell me how to get there, can I?</i>\"\n\n", false);
	
	MainScreen.text("\"<i>Piss off!  You won't get shit from me,</i>\" retorts the defeated demon, \"<i>You may as well finish me off – I'll NEVER help a " + player.mf("jackass","bitch") + " like you!</i>\"\n\n", false);
	
	MainScreen.text("Smirking, you grab a strip of leather from Zetaz's dresser and dangle it over his nose.\n\n", false);
	
	MainScreen.text("You whisper, \"<i>This is all I'll need.</i>\"\n\n", false);
	
	MainScreen.text("The imp looks up with his face shrouded in confusion as he asks, \"<i>I don't think a string is going to help you much, " + player.mf("dork","skank") + ".</i>\"\n\n", false);
	
	MainScreen.text("\"<i>Give me a moment, my stupid little snitch,</i>\" you taunt as you lift his loincloth, exposing the hardness concealed within.  It pulses, growing harder and an inch longer just from your brief touch and exposure to the air.  Perhaps the imp isn't as in control of his libido as he'd like you to think?  You twirl the leather strip around his base and swiftly knot it, getting a tight enough seal to make Zetaz grunt in discomfort. \"<i>Ungh! What the fuck!? Ow, goddamnit!</i>\"\n\n", false);
	
	MainScreen.text("Even with his protests and cries, you watch his cock inflate further, until it looks stuffed far beyond his normal capacity.  It twitches and drools corrupted pre-seed as you slide your finger along his urethra, watching the member bob and twitch from the slight, soft touches.  That must feel quite good.  Zetaz confirms your hunch by lifting his hips off the ground, shaking them lewdly to try to grind against your hand.  You don't deny him the friction he craves, wrapping your hand around as much of his meat as your fingers will encircle until steady dribbles of fluid escape from his urethra.  The tainted nodules of his demonic dick begin to flare and pulse, signaling that his orgasm is almost upon him.\n\n", false);
	
	MainScreen.text("He's NOT allowed to cum – not until you get the information you need!   You slide your fingers down to his base in one fluid stroke, slamming your hand against his crotch as his orgasm starts to bubble up.  Before your opponent can attain release, you squeeze hard with one hand and tighten the leather cord with the other, clamping the base until the imp's cum is bottled up in his abdomen.  Zetaz cries, \"<i>No-no-no, let me cum, please let me cum, need-ta-cum-nooowwww.</i>\"\n\n", false);
	
	MainScreen.text("No such luck.  You wait for his body to stop convulsing and return to your task, one hand sealed around his base while the other begins to stroke him with firm, steady motions, sliding over the pebbly surface of his dick's nubs.  Your victim continues his begging and crying, but you don't let up as you pause to gather his escaped pre-cum and smear it over his tip.  Zetaz pants and groans, trembling and swelling in your hand from your efficient hand-job.   Spitting on your palm, you bump up the tempo and begin to stroke him hard and fast, sliding over his cockring-swollen prick with practiced, deliberate motions.\n\n", false);
	
	MainScreen.text("\"<i>Tell me how to find the head demon and I'll let you cum.  Don't make this any 'harder' than it has to be,</i>\" you whisper.\n\n", false);
	
	MainScreen.text("The demon's voice starts to crack in spite of his efforts to remain defiant. \"<i>No!  I-uh-won't let yo-oooooh-control meeeeee!</i>\"\n\n", false);
	
	MainScreen.text("His protests trail into incoherent squeals and babbles as you bottle up his second orgasm behind the tightly tied strap.  Again, his body twists and writhes in your grip, tortured with the ever-increasing sexual tension.  Zetaz looks up at you with a pleading, cross-eyed expression as he tries to regain his wits, but you just keep pumping away.  His balls are visibly pulsing and quivering, desperately needing to release the building pressure.  You meet his gaze calmly, your hands continuing their work on the bloated imp-cock, and you break into a knowing smile as he thickens in your grip for the third time.\n\n", false);
	
	MainScreen.text("\"<i>Well Zetaz?  Is three the lucky number, or do I have to switch hands and keep backing you up until you go mad?</i>\" you ask.\n\n", false);
	
	MainScreen.text("His hands claw the rug underneath him as he gasps, \"<i>You win, you win!  The desk has a-ah ah ahh-hidden drawer with a map to Lethice's hideout.  Please justletmecomeletmecomeletmecomePLEAAAAASE!</i>\"\n\n", false);
...
private sexualTortureTightenZetaz():void {
	...
	MainScreen.text("The unfinished wood behind the divider's facade grates your fingertips as you gently pull on it, revealing a narrow, hidden compartment.  The only object inside is a detailed map of the mountain and its upper reaches.  Though the secret diagram is quite crude, it depicts a winding trail that bypasses numerous harpy nests, minotaur caves, and various unrecognizable pitfalls to reach the cloud-shrouded mountain peak.  The drawing loses much of its detail once it gets to the demon fortifications at the top, but it can't be that hard to track down Lethice once you've entered the seat of her power, can it?\n\n", false);
...
public loseToZetaz():void {
	MainScreen.text("", true);
	MainScreen.text("\"<i>Well, isn't this familiar?</i>\" asks Zetaz as he watches your ", false);
	if(player.lust > 99) MainScreen.text("masturbating", false);
	else MainScreen.text("prone", false);
	MainScreen.text(" form with an amused expression, \"<i>The first champion in ages to retain " + player.mf("his","her") + " free will for more than a few minutes, and " + player.mf("he","she") + "'s brought to " + player.mf("his","her") + " knees by the very imp " + player.mf("he","she") + " escaped!  Once you've learned your proper place, you'll guarantee my safe return to my rightful station.  Perhaps I'll even get a promotion?  After all, you've defeated so many higher ranking demons already.</i>\"\n\n", false);
	
	//'Fix' genderless folks.
	if(player.gender == 0) {
		MainScreen.text("He squints down at you with a bemused look and laughs, \"<i>How did you lose your gender anyhow?  Never mind, we've got to do something about that!</i>\"\n\n", false);
		MainScreen.text("Zetaz grabs a bottle, uncorks it, and crams it against your lips while you're still too dazed to resist.  He massages your throat to make you swallow the milk-like fluid, and in seconds the skin of your groin splits to form a new, virgin pussy.\n\n", false);
		player.createVagina();
		player.gender = 2;
	}
...
public femaleZetazOver():void {
	...
	MainScreen.text("With your resistance ", false);
	if(player.HP < 1) MainScreen.text("beaten out of you", false);
	else MainScreen.text("moistening the delta of your legs", false);
	...
	if(player.stats.cor < 33) MainScreen.text("shame", false);
	else if(player.stats.cor < 66) MainScreen.text("confuse", false);
	else MainScreen.text("arouse", false);
	MainScreen.text(" you as you're thrown atop one of the tables.   You grunt as leather straps are produced and laid over your form to restrain you.  In the span of a minute you're completely immobilized from the neck down, and your " + player.legs() + " are kept spread to allow easy access to your " + vaginaDescript(0) + ".\n\n", false);
	
	MainScreen.text("Shuffling as they remove their garments, the entire gang of imps, as well as Zetaz, are completely nude.  They've all grown full and hard from the sight of your nubile, restrained body, and in spite of yourself you get ", false);
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) MainScreen.text("even more wet ", false);
	else MainScreen.text("a little wet ", false);
	MainScreen.text("from the masculine scent the aroused penises are producing.  ", false);
	if(player.stats.cor < 33) MainScreen.text("How could you be turned on by such a repulsive situation?  You're going to be raped, brainwashed, and either kept as a pet or tossed in a milking tube for the rest of your life and your body is acting like some horny slut!", false);
	else if(player.stats.cor < 66) MainScreen.text("You marvel at just how turned on you're getting from the strange situation.  You know you'll be raped, drugged, and used as a toy or milk cow, but your loins are thrumming with warm, wet desire.", false);
	else MainScreen.text("How did you wind up in such an arousing situation?  You're going to be raped, drugged, and probably milked in a factory for the rest of your life.  Your body is so fucking turned on that you know you'll love every second of it, but your desire to triumph and dominate mourns the loss of your freedom.", false);
	MainScreen.text("  The crowd draws close, but Zetaz's voice rings out, thick with the tone of command, \"<i>Not yet, my brothers; this one will be mine first.  I'll claim each of her holes, then you may each have your fill of her.</i>\"\n\n", false);
	
	MainScreen.text("The imps draw back, clearing a path for their leader to emerge, and the new, much more imposing Zetaz climbs atop the table.   He glances at your " + vaginaDescript(0) + " with a knowing eye and smiles, walking further forward until he's standing next to your face with his tainted, corruption-filled cock dangling overhead.  You're so distracted by the purplish-black demon-cock swinging above your lips that the sharp pain takes you completely off-guard. As soon as the discomfort passes you twist your head around to try and find the source of your irritation.\n\n", false);
	...
	MainScreen.text("Your lips start to tingle, and you run your tongue over them reflexively.  A shiver of pleasure worms through your body, and you instinctively press your " + player.legs() + " against the straps in an effort to spread them further.  Worse yet, your lips feel much plumper and fuller than a few moments ago.  ", false);
	if(flags[FlagEnum.NUMBER_OF_TIMES_MET_SCYLLA] > 0) MainScreen.text("Unbidden, Scylla's face comes to mind, and you realize the drugs coursing through your veins must be doing something similar to you!  Her visage changes to your own, though the thick, cock-sucking lips remain behind, eager to be penetrated.", false);
	else MainScreen.text("Unbidden, you imagine yourself with thick, cock-sucking lips, so swollen and bloated that they're slightly pursed and ready to be penetrated.", false);
	MainScreen.text("  Warm slipperiness slides over your lips again, feeling nearly as good as it would on your lower lips, and you pull your rebellious tongue back into your mouth with a gasp of pleasure.\n\n", false);
	
	MainScreen.text("This must be what Zetaz was waiting for, and the imp carefully injects the next chemical cocktail into the other side of your neck while you're distracted by orally masturbating your new mouth.  Your " + vaginaDescript(0) + " ", false);
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) MainScreen.text("gushes fresh fluids into a puddle on the table", false);
	else if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.WET) MainScreen.text("drools a heavy flow of liquid arousal onto the hardwood table", false);
	else MainScreen.text("begins to dribble a steady flow of liquid on to the table's girl-slicked boards", false);
	MainScreen.text(".  ", false);
	if(player.inHeat) MainScreen.text("D", false);
	else MainScreen.text("Foreign d", false);
	MainScreen.text("esires wash through your doped up body, and your hungry slit practically demands to be filled with cock and injected with semen.  It wants to be filled with... with males, and with their hot, sticky cum. No, your hot little pussy doesn't want that – you do.  Gods above and below, you want to feel your belly pumped full of imp sperm until their offspring are wriggling in your womb.  And then you want them to come in you some more!\n\n", false);
...
public femaleZetazOverPtII():void {
	MainScreen.text("", true);
	hideUpDown();
	MainScreen.text("While you're gathering your thoughts, Zetaz staggers back down the table and accepts a flask from one of his lackeys.  He guzzles down the bubbling pink fluid in seconds, and the effect is immediate and greatly pleasing to your fuck-happy worldview.  The imp's cock, which had been slowly retracting, thickens at the base and rapidly fills until it's hard and twitching with sexual need.  He glances down at your exposed " + vaginaDescript(0) + " with a hungry look and drops to his knees, lining the nodule-ringed crown of his wondrous dick up with your lust-juiced slit.\n\n", false);
	
	MainScreen.text("You look down at the male and moan, \"<i>Please, hurry up... I need your cum... your babies.  Put your cock inside me!</i>\"\n\n", false);
	
	MainScreen.text("Zetaz looks surprised at your words, and you start to wonder why, but the heat and pleasure of his long, thick member spearing your love-canal interrupts your thought process.  He reaches up, and begins to ", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) MainScreen.text("tweak your " + nippleDescript(0) + "s roughly, pulling and yanking on them as", false);
	else MainScreen.text("maul at your " + allBreastsDescript() + ", slapping and squeezing them as", false);
	MainScreen.text(" he begins to repeatedly thrust against your " + vaginaDescript(0) + ", fucking you in earnest.  The wet slap of his balls on your juice-slimed body fills the chamber and sends ripples of pleasure down your " + player.legs() + ".  With your eyelids half-closed, your tongue masturbating your lips, and your pussy practically squirting lubricants at the end of each thrust, you must look like every male's wet dream.\n\n", false);
	
	MainScreen.text("Looking around, you see a large number of the imps are masturbating, and one of the larger ones has the audacity to speak while his boss is plowing your quim with savage strokes. \"<i>Since you already got to use her mouth, I'm going to put that fuck-hole to use.</i>\"\n\n", false);
...
public zetazBadEndEpilogue():void {
	MainScreen.text("", true);
	if(player.gender == 2) {
		MainScreen.text("The once-champion, " + player.short + " was raped repeatedly by every imp that survived her initial assault.  Her mind never recovered from the initial orgy, and she found herself happy to be named 'Fuck-cow'.  She quickly became a favorite of Zetaz's ever growing brood, and surprised them all with her fertility and rapidly decreasing incubation times.  Within a few months, she was popping out litters of tiny masters even faster than Vala.  Within a year, her body was so well trained and her womb so stretched that she could keep multiple litters growing within at all times.\n\n", false);
	
		MainScreen.text("It was rare for Fuck-cow's cunt or mouth to be empty, and she delighted in servicing any male she was presented with.  Her masters even captured bee-girls, so that fuck-cow's ass could be kept as pregnant as her belly.  Fuck-cow came to love her masters dearly, and with her constantly growing ability to birth imps, she was able to incubate enough troops for Zetaz to challenge Lethice's armies.  The imps never managed to overthrow the old demon lord, but the land was eventually divided in half, split between two growing demonic empires.", false);
	}
	else if(player.gender == 3) {
		//[Epilogue]
		MainScreen.text("The champion was fucked and brainwashed repeatedly for a few more days until Zetaz was sure she understood her place in the world.  Once rendered completely obedient, they released her from her bindings.  It was time she was turned over to Lethice.  ", false);
		if(player.upperBody.wingType != WING.BAT_LIKE_TINY || player.upperBody.wingType != WING.BAT_LIKE_LARGE) MainScreen.text("Zetaz gave her one of the weaker imps to penetrate and taught her to fly with her new, demonic wings.  ", false);
		else MainScreen.text("Zetaz gave her one of the weaker imps to penetrate during the journey.  ", false);
		MainScreen.text("With preparations complete, Zetaz, the champion, and a few dozen imps flew to the mountain peak.\n\n", false);
		
		MainScreen.text("The champion was presented to Lethice, and the demonic mistress was so pleased with Zetaz's gift that she gave him a pair of nubile slave-girls and promoted him over a small army of his own kind.  Once the imps departed, Lethice put the champion through her paces, using her as a fucktoy whenever the mood took her.  The rest of the time the champion was kept bound and unable to orgasm, tortured with unholy levels of arousal, but she didn't mind.  When Lethice allowed her to cum, the champion's orgasms were long and intense enough for her to love her mistress in spite of having to be so pent up.", false);
	}
	else {
		MainScreen.text("The imps never released the champion from that chamber after that.  'He' gave birth to a healthy litter of imps a few weeks later, and the hormones from the pregnancy ", false);
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) MainScreen.text("created a decent set of chest-bumps.", false);
		else MainScreen.text("swelled her already impressive rack with milk.", false);
		MainScreen.text("  After that, the imps really took a liking to her, and she was let down from her restraints.  She never got much chance to get up though; she was well and truly fucked at every opportunity.  She was already hooked.  With her incredible libido and the constant fucking, staying was the easy choice.\n\n", false);
		
		MainScreen.text("After a few months the champion started to become acclimated to her new life, and began birthing imps in larger broods with shorter gestations.  She had become the ideal broodmother, and her worldview shrank down to two powerful priorities: acquiring cum, and birthing.", false);
	}
	player.orgasm();
	player.HP += 150;
	gameOver();
...
public hermZetazOver():void {
	...
	MainScreen.text("Your willpower starts to come back, and you struggle in vain against the tight leather straps, accomplishing nothing.  Zetaz leers down at your double-sexed form and roughly manhandles both your male and female organs as he taunts, \"<i>I don't remember ", false);
	if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("both", false);
	else MainScreen.text("all", false);
	MainScreen.text(" of these being here when we met.  Did you sample some incubi draft?  Or did you guzzle some succubi milk?  Perhaps both?  In any event, I think you could do with a little more of each.</i>\"\n\n", false);
	
	MainScreen.text("Oh no.  Your eyes widen in fear at his bold declaration, but Zetaz only throws back his head and laughs, \"<i>Oh yes!</i>\"  He turns to the mob and orders something in a tongue you don't understand, then returns to fondling your " + cockDescript(0) + ".  \"<i>How perverse.  Why would you have something like this when you have such a beautiful pussy hiding below it?</i>\" asks the imp lord.  Despite his questioning words, he doesn't stop stroking you until you're full, hard and twitching.  Your poor " + vaginaDescript(0) + " is aching from being ignored with all this building sexual tension.\n\n", false);
	
	MainScreen.text("The sounds of numerous footfalls and clinking glass signal that the mob of imps has returned, bringing what sounds like hundreds of vials worth of their foul concoctions.  Zetaz releases your tumescent member and reaches over for something, then returns to your view bearing a ring gag.  Even turned on, defeated, and immobilized on a table, you try your best to fight him, but all that gets you is slapped.  The imp's palm smacks you hard enough to stun you and leave your ears ringing, and when you blink the stars from your eyes, your mouth is forced open with your tongue hanging out lewdly.\n\n", false);
	
	MainScreen.text("Another of Zetaz's brothers, or perhaps sons, hands him a tube with a funnel, and he easily threads the funnel's tube through the ring gag.  Foul remnants of whatever it was used for last leave a sour taste on your tongue, but worse yet is the knowledge that you're going to be force-fed tainted, body-altering, mind-melting drugs.  A drop of pre-cum hits your belly and your thighs grow ", false);
	if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) MainScreen.text("sticky", false);
	else MainScreen.text("soaked", false);
	MainScreen.text(" from the thoughts.  ", false);
	if(player.stats.cor < 33) MainScreen.text("Are you really being turned on by such lewd, debased thoughts?", false);
	else if(player.stats.cor < 66) MainScreen.text("Are you this much of a pervert?  Yes, it'll feel good, but you're a little ashamed of your body's immediate and lewd reaction.", false);
	else MainScreen.text("Are you really this much of a submissive?  Yeah, sucking down drinks like this is hot as hell, but you'd like to be doing it on your own terms.  At least you'll probably start cumming after a few bottles worth of the stuff.", false);
	MainScreen.text("\n\n", false);
	
	MainScreen.text("\"<i>Hey boss!  She's already starting to drip!  To think she tried to fight us.  She's showing us her true nature – that of a pervert-slut,</i>\" raves one of the horde.  You can't pick out the source of his voice in the crowd, but the words sting enough to make your whole body blush with ", false);
	if(player.stats.cor < 33) MainScreen.text("shame", false);
	else if(player.stats.cor < 66) MainScreen.text("confusion", false);
	else MainScreen.text("arousal", false);
	MainScreen.text(".  The imp lord nods in agreement and upends the first bottle over the funnel, channeling fragrant white fluid into your mouth.  It tastes fantastic!  Your throat instinctively gulps down the creamy delight before you can make a conscious decision.  The effect is immediate and strong.  Warmth builds on your chest as weight is added to your " + allBreastsDescript() + " while a gush of fluid squirts from your " + vaginaDescript(0) + ".\n\n", false);
	...
	MainScreen.text("You swallow in loud, greedy gulps as your body is slowly warped by the fluids you're consuming.  Though your " + allBreastsDescript() + " and " + player.multiCockDescriptLight() + " sometimes shrink, they grow far more often, and after a few minutes of force-feeding, you're pleading for more each time they stop to let you breath.  You're a mess of sexual fluids, your tits are squirting milk, and your pussy squirts from every touch.  Demon horns are swelling from your brow, curling back over your ears", false);
	if(player.horns > 0) MainScreen.text(" and adding to your existing pair", false);
	else MainScreen.text(" and giving you an exotic, tainted appearance", false);
	MainScreen.text(".  ", false);
	if(player.lowerBody != LOWER_BODY.DEMONIC_HIGH_HEELS) MainScreen.text("Your " + player.feet() + " have been changing throughout the ordeal, but you didn't notice your " + player.legs() + " becoming such lissom, lengthy legs, or your heels growing long, high-heel-like spikes.  ", false);
	if(player.tailType != TAIL.DEMONIC) MainScreen.text("A tail snakes around your leg and begins to caress your " + vaginaDescript(0) + ", then plunges in to fuck the squirting orifice while you drink.  ", false);
	else MainScreen.text("Your tail snakes around your leg and begins to caress your " + vaginaDescript(0) + ", then plunges in to fuck the squirting orifice while you drink.  ", false);
	MainScreen.text("The imps start hooting and cat-calling, laughing and prodding your body with their twisted demonic members as your mind starts to come apart in the seething oven of unnatural lust.\n\n", false);
	//NEXT
	dynStats("lib", 100, "sen", 100, "lus=", 1000, "cor", 50);
...
public hermZetazOverPtII():void {
	hideUpDown();
	MainScreen.text("", true);
	MainScreen.text("You awaken midway through a loud moan and nearly jump out of your " + player.skinDesc + " in surprise, but the fire of your unnaturally stoked libido immediately reasserts yourself.  You twitch your hips to and fro, thrusting against a ", false);
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("number of ", false);
	MainScreen.text("mechanical milking device", false);
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s", false);
	MainScreen.text(".  " + player.SMultiCockDesc() + " is sucked rhythmically, producing a loud, wet, slurping noise that echoes around the small room.  You're suspended from a set of shackles on the wall, next to Vala.  The sexy faerie is chained up in a similar manner, but she's locked in coitus with a well-endowed imp, and making no secret of her enjoyment.  The sexual sight stirs your well-stimulated loins and you groan, filling the milker with what feels like gallons of male cream over the next minute and a half.\n\n", false);
	
	MainScreen.text("Slumping forwards, you hang there, but the corruption and lust in your blood refuses to be sated.  " + player.SMultiCockDesc() + " is already hard again, and after sucking your cum down some tubes, the milker begins its oh-so-pleasurable work again.  Still, you estimate it will be a few minutes before it gets you off again, so you look around the room.  A platform is set up in front of you, about knee-height and poorly built.  Judging by its height, it's probably there so that the imps can use it to fuck you without having to fly.  There's also a pair of platforms built into the walls next to each of your shoulders, though their function is less clear.\n\n", false);
	
	MainScreen.text("The door to the room bangs open, and Zetaz steps in, followed by two scrawnier-than-usual imps.  He smiles when he sees you awake and flushed, and steps up onto the platform, rubbing his palms together in excitement.\n\n", false);
	
	MainScreen.text("\"<i>You took quite well to our little experiment,</i>\" he announces, \"<i>In fact, your body is a demonic fucking machine.  I won't be transforming you into an actual demon though.  But we're going to have to have a little training to get you ready to meet Lethice.  After all the trouble you've caused her, she might want to turn you herself, or maybe hook you up in a factory?  I can't say for sure.</i>\"\n\n", false);
	
	MainScreen.text("With a flourish, the imp lord discards his loincloth, tossing it over his shoulder to reveal his erect demon dick.  He taunts, \"<i>Like what you see?</i>\" and orders his lackeys, \"<i>Go on, you know what to do.</i>\"  The pair of scrawny imps flit up to their perches while Zetaz advances and strokes himself, preparing for penetration.  Dozens of unanswered questions swarm through your mind, actually distracting you from your pending orgasm enough to ask, \"<i>Wha-what are you going to do to me?</i>\"\n\n", false);
	
	MainScreen.text("\"<i>Shhhh, shhh,</i>\" responds Zetaz, \"<i>just relax my pet.</i>\"  He ", false);
	if(player.lowerBody.balls > 0) MainScreen.text("gently shifts your " + ballsDescript() + " out of the way and ", false);
	MainScreen.text("lines up with your drooling fuck-hole, and with a long smooth stroke, he's inside you.  You cum immediately and hard, barely noticing the chanting that has started up on the adjacent platforms.  Each squirt of cum is accompanied by a thrust from Zetaz, sliding over your lube-leaking walls with ease.  The orgasm lasts nearly twice as long as your last one.  It never seems to end, but when it slowly trails off, you find yourself wondering how soon you can cum again.\n\n", false);
	
	MainScreen.text("You envision yourself on all fours, being taken in both openings by a pair of imps while you suck off a shadowy figure that your mind recognizes as your lord and master.  " + player.SMultiCockDesc() + " spurts and squirts with each penetration as your twin violators get off and stuff you full of their yummy imp cum, glazing your insides with corrupted white goo.  Maybe you'll get pregnant this time?  It's been a few weeks since your last litter.  You suck harder on your master's penis and caress his balls until he shows his affection by giving you a salty treat.  He pulls out and blasts a few ropes over your face and hair, so you do your best to look slutty to encourage him.  When he finishes, you lick your lips and beam at your master, Zetaz.\n\n", false);
...
public malesZetazOver():void {
	MainScreen.text("You've been so thoroughly ", false);
	if(player.HP < 1) MainScreen.text("beaten", false);
	else MainScreen.text("teased", false);
...
public malesZetazOverPtII():void {
	MainScreen.text("", true);
	MainScreen.text("You wake to a desert-dry, sandpapery feeling in the back of your throat as yet another moan escapes your mouth.   The ring gag is still there, and easily thwarts your tongues attempts to lick at your parched lips, but the jolts of pleasure exploding up your spine make it hard to get upset about it.  Hips rocking, you keep squirting and squirting from your orgasm, feeling each hot blast burst from your manhood until the wave of lust passes and you open your eyes.  You're in a dim cave, the one they used to hold Vala, and chained up to the wall in a similar manner.\n\n", false);
	
	MainScreen.text("While you observe the room, you realize that the waves of pleasure sliding up your spinal cord haven't stopped, and that your entire body is being shaken rhythmically.  You look down with a look of incredible, still-drugged confusion and behold the last thing you expected to see.  Somehow " + player.sMultiCockDesc() + " has been shrunk to less than half of its previous size", false);
	if(player.lowerBody.balls > 0) MainScreen.text(", and your balls have completely vanished", false);
	MainScreen.text("!  Just below your pint-sized shaft, a massive imp-cock is plowing in and out of your new, wet snatch with juicy-sounding slaps.  Y-you're a hermaphrodite!?  And what's happening to your dick?\n\n", false);
	
	MainScreen.text("A nearby imp with a limp dick and a bored-but-tired look on his face steps up after your orgasm and slathers your dick in some strange, pungent cream, chuckling up at you while he does so, \"Heh heh, your ", false);
	if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("cock's", false);
	else MainScreen.text("cocks're", false);
	MainScreen.text(" gonna be so tiny ", false);
	if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("it", false);
	else MainScreen.text("they", false);
	MainScreen.text("'ll make a baby's look huge.  Boss said we need to dose you with Reducto after each orgasm, so try not to cum too much while we gangbang you, okay?  Oh yeah, I almost forgot, I have to inject something too...</i>\"\n\n", false);
	
	MainScreen.text("The little demon picks an small, glass injector stamped in black ink with the words 'GroPlus'.  Your eyes go wide at sight of the lettering.  As your maleness dwindles, the imp carelessly flicks it to the side and lines the needle's tip up with your tiny bud – they're going to shrink your dick to nothing and pump your clit full of growth chemicals!  He plunges it in, lighting your world up with pain, but the bindings around your body prevent you from escaping or injuring yourself in struggle.  Heat erupts inside your clitty, and it visibly swells up until it nearly reaches the size of your shrinking wang.  Your rapist, or 'sexual partner' with how horny you are, thrusts hard inside you and swells, stroking your walls with the nubby protrusions of a demon's cock.  It feels so good that another orgasm builds on the spot.\n\n", false);
	
	MainScreen.text("With hot, tainted jism filling your womb, your body starts to spasm and squirt, actually making your increasingly tiny dick shake around from the force of ejaculation.  It splatters off the imp's horns and forehead, but he doesn't seem to mind much as he slumps down, dragging his still-rigid member from your cock-hungry fem-sex.  You moan wantonly, still spurting as the imp 'medic' applies another layer of Reducto to " + player.sMultiCockDesc() + ", rapidly shortening ", false);
	if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("it until it's", false);
	else MainScreen.text("them until they're", false);
	MainScreen.text(" barely three inches long, even while hard.  He pulls out another plunger and rams the needle into your still-aching clit, making it swell until it's almost five inches long and trembling like your manhood used to.\n\n", false);
	
	MainScreen.text("\"<i>Now you're starting to look like a proper bitch.  ", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 2) MainScreen.text("It doesn't look right without a decent rack, but boss said no tits for the new breeding bitch.  Sure makes it hard to get excited about fucking that new twat of yours though...", false);
	else MainScreen.text("With a rack like that and a nice, wet cunt, you'll have the other guys lining up for their turn in no time...", false);
	MainScreen.text("</i>\" rambles one of the imps.  You groan and shake your hips lewdly, still turned on after all the fucking, feeling empty without the unholy heat of an imp inside you.  A hunger buzzes away in your womb, demanding you get pregnant, and you're thrilled to see Zetaz stride in with a raging, fully erect stiffy.  It throbs hungrily as he smiles up at you and climbs atop the conveniently positioned platform.\n\n", false);
...
private incubusDeal():void {
	spriteSelect(52);
	MainScreen.clearText();
	MainScreen.text("\"<i>Excellent!  Give me a few moments to gather my things and I'll be open for business!</i>\" exclaims the strange demon.  If his story is true it's no wonder he doesn't get along with the rest of his kind.");
...
private incubusNoDeal():void {
	...
	MainScreen.clearText();
	flags[FlagEnum.ZETAZ_LAIR_DEMON_VENDOR_PRESENT] = -1;
	MainScreen.text("Sean nods, grabs a pack, and takes off running before you have a chance to kill him.");
	doNext(playerMenu);
}
...
public incubusShop():void {
	...
	MainScreen.clearText();
	MainScreen.text("Sean nods at you and slicks his hair back into place, threading it carefully around the small nubs of his horns before asking, \"<i>What can I do for you?</i>\"");
	menu();
	MainScreen.addButton(0, consumables.NUMBROX.shortName, incubusBuy, consumables.NUMBROX);
	MainScreen.addButton(1, consumables.SENSDRF.shortName, incubusBuy, consumables.SENSDRF);
	MainScreen.addButton(2, consumables.REDUCTO.shortName, incubusBuy, consumables.REDUCTO);
	MainScreen.addButton(3, weapons.SUCWHIP.shortName, incubusBuy, weapons.SUCWHIP);
	if (player.hasItem(consumables.BIMBOCH) && flags[FlagEnum.NIAMH_SEAN_BREW_BIMBO_LIQUEUR_COUNTER] == 0) {
		...
		if (player.hasItem(consumables.BIMBOCH, 5)) {
			MainScreen.text("  Luckily, you do!");
			MainScreen.addButton(4, consumables.BIMBOLQ.shortName, telAdre.niamh.seanBimboBrewing);
		}
	}
...
private incubusBuy(itype:ItemType):void {
	spriteSelect(52);
	MainScreen.clearText();
	MainScreen.text("The incubus lifts " + itype.longName + " from his shelves and says, \"<i>That will be " + (itype.value * 3) + " gems.  Are you sure you want to buy it?</i>\"");
...
public valaBigYou():void {
	...
	if(flags[FlagEnum.TIMES_VALA_CONSENSUAL_BIG] == 1) {
		MainScreen.text("Tilting your head, eyes glowing with ethereal power, you answer, \"<i>I want you... big.</i>\"");
	...
	//{REPEAT}
		...
		MainScreen.text("You tell Vala just how HUGE your need for her is, which earns you a blush from the petite faerie's fair cheeks.  She calls in a favor from another waitress - looks like Vala covered for this one's tryst with Chad as well.  That Chad guy must get around!");
		...
		if(silly()) MainScreen.text("The last thing you need is a bunch of winged archers taking potshots at Vala while she clings to a tower with you in her hand...  ");
		MainScreen.text("She seems to know the way directly to your camp, and all too soon, you've reached your place of residence.  Vala cheekily pokes your [chest] and asks, \"<i>Shouldra?</i>\"");
	}
	//{MERGE}
	...
	//[Dom Me] [Lick Me]
	menu();
	MainScreen.addButton(0,"Dom Me",bigValaDomsPC);
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(1,"Lick Me",bigValaLicksOffDudes);
	if(vapula.vapulaSlave() && player.gender > 0 && (player.lowerBody.cockSpot.hasCock() || (player.lowerBody.vaginaSpot.hasVagina() && player.hasKeyItem("Demonic Strap-On") >= 0))) {
		MainScreen.addButton(2,"Dom Vapula",valaDommyVapula3Some);
		MainScreen.addButton(3,"Vapula3Some",valaLoveyVapula3Some);
	}
}
//Big Vala: Dom Me
...
public bigValaDomsPC():void {
	MainScreen.clearText();
	MainScreen.text("Vala tilts her head to the side, causing a gust of displaced air to blow your " + hairDescript() + " wildly.  \"<i>Giving the maiden a chance to lead?  You're so sweet!</i>\"  She tickles your chin with one immense fingertip.  \"<i>But you had better give it your all, you adorable rascal, you.</i>\"");
	...
	if (flags[FlagEnum.VALA_HEALED_HONEY] == 0) MainScreen.text("\"<i>If you do a good job, I'll let you taste a pure pearl of my own,</i>\" she giggles.");
	
	//{Male:}
	if(player.gender == 1) {
		MainScreen.text("\n\nShuddering from the onslaught of female pheromones, you find your body responding without meaning to.  [EachCock]");
		if(player.lust >= 70) MainScreen.text(", already hard, begins to bead with pre-cum, mixing with the slut-leakings that have found their way under your equipment.");
		else MainScreen.text(" stiffens immediately to a full, throbbing hardness.");
		MainScreen.text("  Your body is imminently ready for breeding, erect and wanting so badly to be freed from your constricting garb, but you know that with what you asked, you won't get to sate it.");
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 150) MainScreen.text("  If only you weren't so big - you can't expand to your full size, and the tightness is almost emasculating at how it constrains your size, keeping you small enough to fit inside your [armor] but still hard as a stone, only this stone is having the pre squeezed out of it.");
	}
	//{Female:}
	else if(player.gender == 2) {
		...
		if(player.wetness() >= 3) MainScreen.text("Your [vagina] seems to always be wet, but now, with this sight before you, your girl-cum is running in rivers down your [legs] to mix into the puddling lady-spunk below.  ");
		MainScreen.text("Your armor seems tight and constricting against your rock-hard [nipples].  It's something you can't escape from, and with each passing moment, the sensations coming from your chest seem more and more worrisome, stoking your fiery lust to an incredible degree.");
	}
	//{Herm:}
	else if(player.gender == 3) {
		MainScreen.text("\n\nShuddering from the onslaught of lusty pheromones, you find [eachCock] responding even before you realize what's going on.  ");
		if(player.lust >= 70) MainScreen.text("You may have already been uncomfortably hard, but you start leaking pre-cum all over the inside of your [armor].  Of course, the copious fem-jizz that's already seeped inside your gear absorbs the extra fluid with little to no complaint.");
		else MainScreen.text("You get hard with alarming rapidity.  Being so close to such a gloriously large quim seems to overpower your better sense, and you're forced to give in to your body's imperative.");
		MainScreen.text("  Inside your undergarments, everything is already turning into a swampy, slimy mess thanks to Vala's secretions, and your own feminine drippings don't help any.  Your lips and dick");
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(" are smothered with the stuff, making you moan without meaning to.");
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 150) MainScreen.text("  If only you weren't so big!  You're compressed so tightly by your enclosing [armor], and even though you're rock-hard, you're a fraction of your full size.  If only you could get out of your gear and fuck her!");
	}
	//{Genderless}
	else {
...
public bigValaLicksOffDudes():void {
	MainScreen.clearText();
	MainScreen.text("Giant Vala cocks an eyebrow at your request in surprise.  \"<i>But, what if I accidentally swallow?  I couldn't live with myself if something should happen to my big... er, little Hero.</i>\"  She pats you on the head affectionately, unsure of how to handle herself with all of her newfound size.");
	...
	if(player.upperBody.chest.hasFuckableNipples()) MainScreen.text("  It even pauses at one to slip a tiny bit inside, too big to penetrate your nipplecunt properly but teasing it all the same.");
	MainScreen.text("  Finally, it encircles the back of your neck and curls up to slide across your chin.  You greedily slurp the tip of her oral muscle into your own mouth, and the effect is immediate.");
	...
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("each");
	else MainScreen.text("your");
	MainScreen.text(" length, unholy smoothness sliding ever so slowly back and forth as she moans.  The giant faerie drags you further inside, much to your delight.  Agonizingly sensuous pressure mounts all along your length");
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
	MainScreen.text(" until ");
	if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("it slides");
	else MainScreen.text("they slide");
	MainScreen.text(" inside her");
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 200) MainScreen.text(", the immense length sucked in by a surprising burst of suction");
	MainScreen.text(".");
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() < 200) MainScreen.text("smothering your relatively tiny pecker");
	else MainScreen.text("wrapping up your immense pecker");
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 400) MainScreen.text(", even as it hangs partway down her throat");
	MainScreen.text(", you begin to moan as well, subsumed in steamy pleasure.");
	...
	if(player.lowerBody.balls > 0) MainScreen.text(", [balls] clenching against Vala's tongue");
	MainScreen.text(" as you explode into ecstatic, jerking pumps against her slippery, sensitive tongue.  Vala's voice rises with yours the moment the first spurt of seed stains her tongue, and her fem-cum sprays out to soak her thighs with enough force to carry some smaller boulders away in the tide.");
	if(player.cumQ() < 250) MainScreen.text("  She barely notices the spunk you shoot against her tongue, her drooling saliva washing it away almost immediately, but you're sure she got a nice, salty thrill from it regardless.");
	else if(player.cumQ() < 1000) MainScreen.text("  She has to noisily gulp down the liquid deposit you leave in her mouth, but with her enhanced size, it's still no more than a creamy morsel.");
	else if(player.cumQ() < 4000) MainScreen.text("  You slowly flood her mouth with spooge until her cheeks bulge.  The spooge inundates her maw until your lower body floats in it, forcing the girl to swallow hard or risk having the teeming seed spill out around you.");
	else MainScreen.text("  You immediately flood her mouth, bulging her cheeks with obscene amounts of spunk.  She noisily gulps, mouth working futilely to handle it all, but it's not enough, and jizz begins to burst from the corners of her mouth, almost pushing you out with it.");
	MainScreen.text("  Her tongue grinds back and forth underneath you, encouraging you to deposit every single drop into her.");
...
public valaVapulaThreesome():void {
	...
	//{Repeat}e
		...
		MainScreen.text("You call for Vapula, which brings an amused smile to your giant faerie's face.");
...
public valaDommyVapula3Some():void {
	...
	if(!player.lowerBody.cockSpot.hasCock()) MainScreen.text("dildo");
	else MainScreen.text("[cock smallest]");
	MainScreen.text(" slides into the demoness's waiting snatch, which slurps it down with a hungry contraction, slapping your [hips] against hers, the slippery, demonic folds wriggling around you with such insistence that you couldn't pull out if you want to.");
	...
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text("seed");
	else MainScreen.text("fake seed");
	MainScreen.text(" without meaning to, the lustiness of the situation just far beyond your ability to bear.");
	if(player.cumQ() > 500) MainScreen.text("  The demon's purple belly balloons with cum, swelling against your own");
	if(player.cumQ() >= 1500) MainScreen.text(" as her midriff turns taut and dome-like");
	if(player.cumQ() > 500) MainScreen.text(".");
	if(player.cumQ() > 1000) MainScreen.text("  Waves of spunk pour out from Vapula's lust-swollen lips thanks to your exceedingly potent orgasm.");
	if(player.lactationQ() >= 100) {
		MainScreen.text("  Even better, your [fullChest] lets down your milk, letting it run out of you in ");
		if(player.lactationQ() <= 200) MainScreen.text("thick");
		else if(player.lactationQ() <= 2000) MainScreen.text("powerful");
		else MainScreen.text("enormous");
		MainScreen.text(" streams, tinting Vala's secretions white.");
	}
...
public valaLoveyVapula3Some():void {
	...
	if(!player.lowerBody.cockSpot.hasCock()) MainScreen.text(", and you buckle on your demonic strap-on, just in case");
	MainScreen.text(".  Holding your arms out to either side, you let Vala's cunt-soaked hand come out to lift you, a finger under each armpit and one between your legs, slickly sliding against the sensitive, hard prong there.");
	MainScreen.text("\n\nYou're lifted up high, which only serves to slide ");
	if(!player.lowerBody.cockSpot.hasCock()) MainScreen.text("the dildo");
	else MainScreen.text("[oneCock]");
	MainScreen.text(" even harder into your flawless faerie-skin, you moan out loud, matching Vapula's lascivious cries as she's mercilessly teased.  Your [feet] are brought up against Vala's immense lip, quickly sliding over the oral protrusion and into the steamy, warm cavern behind it.  The demon's shoulder brushes against your own, enough to rouse her from her sexual stupor and have her turn her head to face you.  Mischief twinkles in her lust-lidded eyes as she asks, \"<i>Oh, are you going to feed me, [Master]?</i>\"");
	...
	if(!player.lowerBody.cockSpot.hasCock()) MainScreen.text("that exotic dildo");
	else MainScreen.text("your [cock smallest]");
	MainScreen.text(".  You grab hold of her ass and the tongue that's slipping through her crack and hammer your hips home, slapping against Vapula so hard that the violet woman's cries of bliss are interrupted by the breath-stealing impacts. ");
	...
	if(!player.lowerBody.cockSpot.hasCock()) MainScreen.text("spurting dildo");
	else MainScreen.text("[cock smallest]");
	MainScreen.text(", ensuring that every single drop is passed into her ravenous gash.");
...
public kinathisValaStuff():void {
	...
	MainScreen.text("With it so late at night, there are barely any patrons at the bar.  It's likely the bar will close up soon.  Lounging in one of the empty, comfortable booths off to the side, you wave over the short, busty bar-fairy.  Vala flutters over to you, a frothy mug in hand, smiling brightly at the sight of you.  The lovely fairy girl always seems happy to see you; rescuing her from the caves really did endear you to her a great deal.");
	...
	if(player.tallness > 75) MainScreen.text("  She looks up at you as you seem to tower over her, a");
	else MainScreen.text("  A");
	MainScreen.text(" faint blush on her cheeks as she smiles and hugs your arm, beckoning you to follow.");
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) MainScreen.text("Her tongue licks around the throbbing glands, the heady animal musk filling her nose and mind with sweet lust.  ");
	else if(player.lowerBody.cockSpot.list[x].cockType == CockType.DOG) MainScreen.text("Her tongue licks around the hot building knot that gently throbs at your base.  ");
	//[anemone cock:
	else if(player.lowerBody.cockSpot.list[x].cockType == CockType.ANEMONE) MainScreen.text("Her tongue plays with the squirming tentacles that line the drone of your throbbing phallus.  She shivers and groans as the tentacles sting her full of pleasurable aphrodisiac.  ");
	MainScreen.text("Playing her way down your cock, the fey-girl gently licks up and down, sucking along your cumvein before coming back to the " + player.cockHead(x) + " of your meaty prick.");
	...
	//[cockarea <17:
	if(player.cockArea(x) < 17) MainScreen.text("Despite her size, you slide in easily, her wet, slippery pussy taking you in and squeezing you in a velvety embrace.  \"<i>Yessss, it's in, it's in!  Mmmm, come to me hero, come to Vala,</i>\" she croons to you, beckoning  you to take her.");
	else if(player.cockArea(x) < 30) MainScreen.text("\"<i>Ahhh...  yesss, it's in!  Ohh...  come to me hero, come to Vala and fill her with your love!</i>\" she croons to you, beckoning  you to take her.");
	else MainScreen.text("Ohhh! Ahhh...  Y-Yessss...  Vala’s big strong hero, oh you feel so good inside me!</i>\" she moans and groans as you stretch her tight little fairy cunt with your pulsing prick.  Her stretchy flesh distends as more and more of your cock is fed into her hungry pussy.");
	...
	if(player.cockArea(x) > 17) MainScreen.text("  Her hot little body stretches around your girthy prick, distending her belly as you sheath your throbbing arousal into her.");
	...
	if(player.lowerBody.balls > 0) {
		//if ballsize <3:
		if(player.lowerBody.ballSize < 3) MainScreen.text("  With each hard quaking thrust your balls slap against Vala’s expansive ass.  Your nuts ache under their pent up load and demand you to shoot it into the sweet fey-girl soon.");
		else if(player.lowerBody.ballSize < 6) MainScreen.text("  With each hard quaking thrust, your balls smack against Vala’s cushiony ass.  Your hefty, cum-bloated sack, so full, aches for the sweet release into your tender fairy lover.");
		else MainScreen.text("  With each hard quaking thrust, your fat balls slam against Vala’s pale squeezable ass.  The huge, virile spunk-factories go into overtime, their already taut skin growing tighter as they swell and bloat with your pent up load.  Your body's natural instincts demand you do your duty as the breeder you are and pack your small lover’s womb full of your spunk until her belly is swollen and seeded with your young.");
	}
	...
	menu();
	MainScreen.addButton(0,"Next",valaPartIIWaifuLove)
}
