...
public doDungeon(eventNo:number):void {
	...
	if(eventNo == 11001) {
		if(player.hasKeyItem("Iron Key") < 0) {
			MainScreen.text("The door is locked with a key that is not in your possession.", true);
	...
	if(eventNo == 11010) {
		...
		dungeonLoc = 0;
		if(dungeonLoc == 0) {
			MainScreen.text("You slip out the door and disappear, heading back towards your camp, leaving the hellish factory behind.", true);
	...
	if(eventNo == 11013) {
		...
		MainScreen.text("She runs a stocking covered foot up your leg and thigh, almost to your groin.  Giggling, the succubus pulls it away and asks, \"<i>So are you ready and willing?</i>\"", false);
	...
	if(eventNo == 11017) {
		spriteSelect(55);
		MainScreen.text("She frowns, \"<i>I was secretly hoping you would say that... I'm going to make you beg me to hook you into the machines.  Just wait.</i>\"", true);
		doNext(11016);
		return;
	}
	//Succubi bad end 2
	if(eventNo == 11018) {
		spriteSelect(55);
		//Arousal
		MainScreen.text("In no time flat your blood begins to burn hot with the fires of unnatural lust.  ", true);
		//Tits – regular
		if(player.biggestLactation() < 1) MainScreen.text("Your " + nippleDescript(0)  + "s begin prodding painfully against your " + player.armorName + ", every touch serving to make them harder and more erect.  ", false);
		//Tits – lactating
		if(player.biggestLactation() >= 1 && player.biggestLactation() < 3) MainScreen.text("Your " + nippleDescript(0) + "s get painfully hard as you feel milk begin backing up inside your " + allBreastsDescript() + ".   The succubus glances down mischieviously as her hands begin to grope you through your " + player.armorName + ", squeezing out a few drops of milk.  ", false);
		//Tits – megalactating
		if(player.biggestLactation() >= 3) MainScreen.text("Your " + nippleDescript(0) + "s get painfully hard as milk begins drooling down your over-productive chest, making your " + player.armorName + " slide across your leaky milk-spouts in an agonizingly pleasurable way.  ", false); 
		//Cock – single
		if(player.lowerBody.cockSpot.count() == 1) {
			if(player.cockArea(0) < 30) MainScreen.text("Swooning from sudden blood loss, you struggle to maintain the kiss as your body takes your " + cockDescript(0) + " to full hardness in seconds.  ", false);
			//Cock – single big
			if(player.cockArea(0) >= 30 && player.cockArea(0) < 100) MainScreen.text("Nearly blacking out, you struggle to stay awake as your body shifts your blood to your disproportionate " + Appearance.cockNoun(CockType.HUMAN) + ".  ", false);
			//Cock -megahuge
			if(player.cockArea(0) >= 100) MainScreen.text("As you struggle not to lose consciousness, you realize your over-aroused body had pumped most of your blood to your over-sized " + Appearance.cockNoun(CockType.HUMAN) + ", which now droops to the floor, pulsing hotly.  ", false);
		}
		//DO MULTIZ
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("The feeling of light-headedness nearly robs you of consciousness as your " + player.multiCockDescript() + " fill with blood, pulsating with arousal as they reach full size.  ", false);
		//Vagooooz
		if(player.lowerBody.vaginaSpot.count() > 0) {
			//'uge clit
			if(player.lowerBody.vaginaSpot.list[0].clitLength >= 4.5) MainScreen.text("Popping from between your thighs, your " + clitDescript() + " responds to the sheer hotness of the situation by making itself known.   You squeeze your legs tightly together, hungry for additional sensation.  ", false);
			//big clit
			if(player.lowerBody.vaginaSpot.list[0].clitLength > 1 && player.lowerBody.vaginaSpot.list[0].clitLength < 4.5) MainScreen.text("A wave of pleasure erupts from between your legs as your " + clitDescript() + " pops free.    You squeeze your legs tightly together, hungry for the additional sensations.  ", false);
			//slick
			if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLICK) MainScreen.text("Squishing wetly, your bottoms become soggy with the flood of fluids leaking from your " + vaginaDescript(0) + ".   Your legs spread apart on their own, begging for any kind of intrusion.  ", false);
			//normal
			else MainScreen.text("Groaning softly, you feel yourself getting wetter and wetter with arousal.  You wish your sticky bottoms were off so you could let something into your " + vaginaDescript(0) + ".  ", false); 
		}
		MainScreen.text("\n\n", false);
		//New PG
		...
		MainScreen.text("The succubus pushes a button on the wall, and a number of strange looking suction tubes and hoses drop down from above you.   Moving with practiced efficiency, she hooks a ", false);
		if(player.upperBody.chest.count() == 1) MainScreen.text("pair of", false);
		else MainScreen.text("number of", false);
		MainScreen.text(" hoses to your breasts, ", false);
		if(player.lowerBody.cockSpot.count() == 0) {
			if(player.lowerBody.vaginaSpot.count() > 0) MainScreen.text("and a vacuum pump to your clit.  ", false);
			else {
				MainScreen.text("and another vacuum pump to your bare groin.  In seconds a wet fleshy growth erupts.  You have grown a cock!  ", false);
				player.lowerBody.cockSpot.add(new Cock());
			}
		}
		else {
			if(player.lowerBody.vaginaSpot.count() > 0) MainScreen.text("a vacuum pump to your clit, and a pump many times bigger than your " + cockDescript(0) + " to it.  ", false);
			else MainScreen.text("and a pump many times bigger than your " + cockDescript(0) + " to it.  ", false);
		}
		MainScreen.text("At first there is only a gentle suction, you assume in order to keep them in place.  Unfinished, your captor places something large and hollow against your backdoor", false);
		if(player.lowerBody.vaginaSpot.count() == 0) MainScreen.text(".", false);
		else MainScreen.text(" and an ever larger dildo against your " + vaginaDescript(0) + ".  It seems to pulse and wiggle with a life of its own, rubbing the bumps of its lumpy head against your lips.", false);
		MainScreen.text("  You swoon as you hear the solid click of a button being pushed, and all at once all devices attached to you leap to life.", false);
		doNext(11019);
		return;
	}
	...
	if(eventNo == 11019) {
		...
		//Nipples
		MainScreen.text("The suction pulls squirt after squirt of milk from your breasts as your " + allBreastsDescript() + " start to grow, swelling heavier as they enlarge to produce more milk.  You squeal with delight as your nipples turn black, tainted by corruptive chemicals that are slowly dripped into you.  ", false);
		//Dick  
		if(player.lowerBody.cockSpot.count() > 0) {
			MainScreen.text("The vacuum-pump on your cock noisily sucks down all your spoo, ", false);
			//High cum variant
			if(player.cumQ() > 300) MainScreen.text("struggling with the amount you put off.  Grinning, the succubus reaches over and flips a lever.  You feel the suction increase as the machine is turned up to accommodate your altered physique.  ", false);
			//else
			else MainScreen.text("the suction and drugs make it so easy to just keep cumming and cumming.  ", false);
			//either or:
			MainScreen.text("Dimly, you feel a needle lodged in your taint, pumping your prostate full of something.  Your " + cockDescript(0) + " begins growing mid-orgasm, the skin turning a deep purple even as small nodule-like bumps form all over it, rapidly becoming a bloated parody of its demonic counterparts.  ", false);
		}
		//Puss Orgasmz
		if(player.lowerBody.vaginaSpot.count() > 0) MainScreen.text("Clenching tightly, your " + vaginaDescript(0) + " squeezes tightly on its intruder as its repeatedly violated by the machines.  ", false);
		//End
		...
		if(player.statusAffects.get("$1").value3 == 1) {
			MainScreen.text("Later on, you are briefly pulled out of your reverie by a familiar warm fluid flowing down your throat.  You come to your senses and see Marble looking down at you with an odd expression on her face.  ", false);
			...
			MainScreen.text("You are given a brief moment of clarity as you see Marble standing in front of you.  ", false);
			MainScreen.text("She seems to be in a state of bliss. Looking down, you see that she is wearing some kind of pair of pink panties.  Marble gasps and the surface of the panties ripples; it's a living thing!\n\nYou look around and realize you aren't alone.  ", false);
		}
		else MainScreen.text("Later on, in a moment of clarity, you look around and realize you aren't alone.  ", false);		
		...
		if(player.statusAffects.get("$1").value3 == 1 || player.statusAffects.has("CampMarble")) MainScreen.text("  With Marble here too, you'll be around for a long time.", false);
		gameOver();
		return;
	}
	...
	//Take iron key
	if(eventNo == 11028) {
		MainScreen.text("You take the <b>Iron Key</b> to keep with your other important items.", true);
		doNext(camp.campMenu);
	...
	if(eventNo == 11032) {
		...
		if(player.hasKeyItem("Hentai Comic") >= 0) {
			...
			MainScreen.text("The incubus speaks to you with a calm, deep voice, \"<i>And so the insect, unaware of its path, stumbles directly into the spider's web.  Tiny insect, you have little to offer me, but everything to offer our facility.  Why don't you come along quietly?</i>\"", true);
	...
	if(eventNo == 11033) {
		spriteSelect(30);
		MainScreen.text("\"<i>It is good to see the insect accept its fate as the spider closes in,</i>\" intones the strange demonic mechanic as he takes you by the arm and leads you deeper into the facility.  ", false);
	...
	if(eventNo == 11034) {
		spriteSelect(30);
		MainScreen.text("You hand over the Hentai Comic tentatively to the male sex demon.  As soon as he has it in his grubby mitts he sits down and starts thumbing through the pages, toying with his half-hard member the entire time.  He must really like porn.", true);
	...
	if(eventNo == 11035) {
		...
		if(monster.statusAffects.has("Blind")) {
			MainScreen.text(monster.capitalA + monster.short + " suddenly grows it's dick to obscene lengths and tries to trip you with it.  Thankfully he's so blind he wasn't aiming anywhere near you!", false);
			combatRoundOver();
			return;
		}
		MainScreen.text("The incubus lunges forward in a clumsy attack that you start to side-step, only to feel something grip behind your " + buttDescript() + " and pull your " + player.legs() + " out from under you.", false);
		if((player.stats.spe-30) > rand(60)) {
			MainScreen.text("  You spin as you fall, twisting your " + player.legs() + " free and springing back to your " + player.feet() + " unharmed.", false);
		}
		//Fall down go boom
		else {
			MainScreen.text("  You land hard on your ass, momentarily stunned as the demonic cock-tentacle curls around your " + player.legs() + ", smearing them with oozing demonic fluids.", false);
			if(player.lust >= 80 || player.stats.cor >= 80) {
				MainScreen.text("  Moaning with desire, you lick your lips as you slide your well-lubricated " + player.legs() + " free.  You gather a dollop of cum and lick it seductively, winking at the incubus and hoping to make him cave into his desire.", false);
				dynStats("lus", 13, "cor", 1);
			}
			else {
				if(player.lust >= 50 || player.stats.cor >= 50) {
					MainScreen.text("  Blushing at the scent and feel of cum on your " + player.legs() + ", you twist and pull free.  You find yourself wondering what this demon's dick would taste like.", false);
					dynStats("lus", 8 + player.stats.cor/20);
					
				}
				else {
					MainScreen.text("  Disgusted, you pull away from the purplish monstrosity, the act made easier by your well-slimed " + player.legs() + ".", false);
	...
	if(eventNo == 11036) {
		//Blind dodge change
		if(monster.statusAffects.has("Blind")) {
			MainScreen.text(monster.capitalA + monster.short + " pumps and thrusts his hips lewdly before cumming with intense force in your direction!  Thankfully his aim was off due to the blindness currently affect him.", false);
			combatRoundOver();
			return;
		}
		MainScreen.text("Your demonic foe places his hands behind his head and lewdly pumps and thrusts his hips at you.  Your eyes open wide as a globule of cum erupts from the demon-prick and flies right at you.  ", false);
		MainScreen.text("You do your best to dodge, but some still lands on your ", false);
		temp = rand(3);
		//Face
		if(temp == 0) {
			MainScreen.text("face.  The gooey demon-seed oozes and slides over you with a mind of its own, forcing its way into your mouth and nose!  You can feel it moving around inside you, doing its best to prepare you for its master.", false);
			dynStats("lus", 3);
		...
		if(temp == 1) {
			if(player.upperBody.chest.hasFuckableNipples()) {
				MainScreen.text(allBreastsDescript() + ".  The gooey demon-seed oozes and slides over you with a mind of its own, forcing its way into your open nipples.  You can feel it moving around inside you, doing its best to prepare you for its master.", false);
				dynStats("lus", 3);
				if(player.findStatusAffect(StatusAffects.DemonSeed) < 0) player.statusAffects.add(new StatusAffect("DemonSeed",5,0,0,0)));
				else player.addStatusValue(StatusAffects.DemonSeed,1,8);
				player.slimeFeed();
			}
			else MainScreen.text(allBreastsDescript() + ".  Thankfully it doesn't seem to have much effect.", false);
		}
		//Crotch
		if(temp == 2) {
			if(player.lowerBody.vaginaSpot.count() > 0) {
				MainScreen.text("crotch.  The gooey demon-seed oozes and slides over you with a mind of its own, forcing its way past your " + player.armorName + " and into your " + vaginaDescript(0) + ".  You can feel it moving around inside you, doing its best to prepare you for its master.", false);
				dynStats("lus", 3);
				if(player.findStatusAffect(StatusAffects.DemonSeed) < 0) player.statusAffects.add(new StatusAffect("DemonSeed",5,0,0,0)));
				else player.addStatusValue(StatusAffects.DemonSeed,1,8);
				player.slimeFeed();
			}
			else MainScreen.text("crotch.  Thankfully, it doesn't seem to have much effect.", false);
		}
		combatRoundOver();
		monster.lust -= 10;
	...
	if(eventNo == 11040) {
		...
		if(player.findStatusAffect(StatusAffects.TensionReleased) < 0) {
			MainScreen.text("You nod and step forwards, allowing her to hook up a modified harness and inject you with the demonic concoction.  In no time heat boils through your veins, pooling on your chest and crotch.  ", false);
		...
		//Second/third times...
			...
			if(player.statusAffects.get("TensionReleased").value1 == 0) {
				MainScreen.text("You eagerly put on the modified harness and let them inject you with more of those body-altering chemicals.  As they fill you with artificial lust and desire, you cry out and beg for more.  They oblige you and give you a larger dose than the first time.  ", false);
				//Grow dick!
				if(player.lowerBody.cockSpot.count() > 0) {
					...
					if(player.averageCockLength() >= 9 && player.averageCockThickness() < 2) {
						MainScreen.text("You feel yourself gain in thickness as well, to match your new length.  ", false);
				...
				if(player.upperBody.chest.count() == 0) {
					...
					MainScreen.text("Your chest tingles, revealing a pair of pink nipples on your new mammory glands.  ", false);
				}
				player.growTits(1, (2+rand(3)), true, 1);
				MainScreen.text("  ", false);
				MainScreen.text("Your " + nippleDescript(0) + "s ", false);
				if(player.lowerBody.cockSpot.count() > 0) MainScreen.text("and " + player.multiCockDescript(), false);
				MainScreen.text(" become rock hard, leaking fluids constantly.  ", false);
				//MALE
				...
				//HERM
				if(player.lowerBody.vaginaSpot.count() > 0 && player.lowerBody.cockSpot.count() > 0) MainScreen.text("You feel your " + player.multiCockDescript() + " getting milked by many wet holes, though you are too busy sucking cocks and moaning in ecstasy to notice who they belong to.  ", false);
			...
			//Third time
				...
				MainScreen.text("Desperate for more of the demon-drugs, you slide into the now-familiar harness and let the needles sink into your skin.   Panting in lust, you beg for them to increase the dosage again.   Desire burns through your veins as the cocktail surges through them", false);
				if(player.lowerBody.cockSpot.count() > 0) {
					MainScreen.text(", filling your " + player.multiCockDescriptLight(), false);
					MainScreen.text(" with sensation", false);
					if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("s", false);
					MainScreen.text(" as ", false);
					if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("they", false);
					else MainScreen.text("it", false);
					MainScreen.text(" grow", false);
					if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("s", false);
					MainScreen.text(" massive and engorged.  ", false);
				}
				else MainScreen.text(".  ", false);
				MainScreen.text("Your " + nippleDescript(0) + "s throb, becoming hard, puffy, and starting to dribble milk.  ", false);
				if(player.lowerBody.vaginaSpot.count() > 0) MainScreen.text("Your pussy is instantaneously soaked, filling the air with the scent of sex.  ", false);
				...
				if(player.biggestLactation() >= 3) MainScreen.text("moo", false);
				else MainScreen.text("moo", false);
				MainScreen.text(" with happiness.  She grins, promising another dose to you if you are a good cow for her.", false);
				gameOver();
				return;
			}
	...
	if(eventNo == 11044) {
		temp = rand(2);
		if(temp == 0) MainScreen.text("The demoness grips her sizable breasts and squeezes, spraying milk at you.\n", false);
		...
		if((player.stats.spe > 50 && rand(4) == 0) || (player.perks.has("Evade") && rand(3) == 0) || (player.perks.has("Misdirection") && rand(4) == 0 && player.armorName == "red, high-society bodysuit")) {
			MainScreen.text("You sidestep the gushing fluids.", false);
		}
		//You didn't dodge
		else {
			if(rand(2) == 0) MainScreen.text("The milk splatters across your face and chest, soaking you with demonic cream.  Some managed to get into your mouth, and you swallow without thinking.  It makes you tingle with warmth.  ", false);
			else {
				MainScreen.text("The milk splashes into your " + player.armorName + ", soaking you effectively.  ", false);
				if(player.lowerBody.cockSpot.count() > 0) {
					MainScreen.text("Your " + cockDescript(0) + " gets hard as the milk lubricates and stimulates it.  ", false);
					dynStats("lus", 5);
				}
				if(player.lowerBody.vaginaSpot.count() > 0) {
					MainScreen.text("You rub your thighs together as the milk slides between your pussy lips, stimulating you far more than it should.  ", false);
					dynStats("lus", 5);
				}
			}
			dynStats("lus", 7 + player.stats.sens/20);
			if(player.biggestLactation() > 1) MainScreen.text("Milk dribbles from your " + allBreastsDescript() + " in sympathy.", false);
		}
		combatRoundOver();
		return;
	}
	...
	if(eventNo == 11048) {
		...
		//Grow if none
		if(player.upperBody.chest.count() == 0) {
			MainScreen.text("<b>Your chest swells out, forming rounded C-cup globes, capped with tiny erect nipples!</b>", true);
			player.createBreastRow();
		...
		//Singular row - cup size + 3.  Nipple size to 1" if smaller.
			...
			if(player.upperBody.chest.BreastRatingLargest[0].nippleLength < .75) {
				...
				MainScreen.text("Your " + nippleDescript(0) + "s grow hard and sensitive, becoming much more noticeable inside your " + player.armorName + ".  It appears your nipples have grown larger to match.", false);
			}
		}
		//Multiple Rows...
		else {
			//Top row + 3, all other rows brought up to par.
			MainScreen.text("Your top " + breastDescript(0) + " tingle pleasantly as the magic takes effect.  You watch with fascination as they begin to swell up, like sponges exposed to water.  The top of your " + player.armorName + " is pulled tight by change, until your chest seems ready to burst free.  <b>You've gained 4 cup sizes!</b>  ", false);
			player.upperBody.chest.list[0].breastRating += 4;
			MainScreen.text("The next row of " + breastDescript(1) + " jiggle and tingle with even more pleasure than the first.  They pulsate for a few seconds, shrinking and growing rapidly until they settle at a size just below your top " + breastDescript(0) + ".  ", false);
			...
			if(player.upperBody.chest.count() >= 4) {
				MainScreen.text("Your remaining " + breastDescript(3) + " feel so wonderful that you just can't resist cupping and squeezing them as they reshape to fit in perfectly with the rest of your breasts.  ", false);
			...
			if(player.upperBody.chest.BreastRatingLargest[0].nippleLength < .75) {
				player.upperBody.chest.BreastRatingLargest[0].nippleLength += .5;
				MainScreen.text("Your " + nippleDescript(0) + "s grow hard and sensitive, becoming much more noticable inside your " + player.armorName + ".  It appears your nipples are have grown larger to match.", false);
			}
		}
		MainScreen.text("\n\n", false);
		eventParser(11055);
	...
	if(eventNo == 11049) {
		...
		if(player.lowerBody.cockSpot.count() == 1) {
			if(player.demonCocks() < 1) {
				MainScreen.text("You smile, watching your " + cockDescript(0) + " grow longer.  Inches of newfound dick-flesh erupt from your crotch in response to omnibus' dark magics.  Before you can play with your transformed tool, a wave of burning desire passes through you.  ", false);
				if(player.stats.cor < 80) MainScreen.text("You watch in horror as the skin of your " + cockDescript(0) + " turns shiny and purplish-black.  ", false);
				else MainScreen.text("Curious, you watch the skin of your " + cockDescript(0) + " turn a shiny-dark purple.  ", false);
				player.lowerBody.cockSpot.list[0].cockLength += 3 + rand(5);
				player.lowerBody.cockSpot.list[0].thickenCock(2);
				player.lowerBody.cockSpot.list[0].cockType = CockType.DEMON;
				if(player.stats.cor < 50) MainScreen.text("Corrupt nodules begin to spring up over its entire length.  <b>Your penis is transforming into a " + cockDescript(0) + "!<\b>  The new nubs wriggle about as they sprout over every inch of surface, save for the head.  Unable to do anything but groan with forced pleasure and horror, you can only watch.  One last batch of nodules forms in a ring around the crown of your " + cockDescript(0) + ", seemingly completing its transformation, until you notice, almost throwing up, that your testicles are also getting covered in black veins under your powerless eyes!  ", false);
				...
				MainScreen.text("Your " + cockDescript(0) + " leaps forwards, taking to the dark magic with ease.  Inch after inch of new length erupts from your groin as your " + cockDescript(0) + " gets longer and thicker.  It pulsates, as if promising dark pleasure as it settles into its new enhanced size.", false);
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			...
			//Not yet full of demoncocks...
				...
				MainScreen.text("You smile, watching your " + player.multiCockDescriptLight() + " grow longer.  Inches of newfound dick-flesh erupt from your crotch in response to omnibus' dark magics.  Before you can play with your transformed pleasure tools, a wave of burning desire passes through you.  You watch", false);
				if(player.stats.cor < 80) MainScreen.text(" in horror", false);
				else MainScreen.text(" curiously", false);			
				MainScreen.text(" as the skin of your " + player.multiCockDescriptLight() + " turns shiny and purplish-black.  Corrupt nodules begin to spring up over the entire length of each dick.  ", false);
				temp = player.lowerBody.cockSpot.count();
				while(temp > 0) {
					temp--;
	...
	if(eventNo == 11050) {
		...
		if(player.horns > 0 || player.antennae > ANTENNAE.NONE) {
			MainScreen.text("Your forehead itches intensely.  You cannot help but stratch madly at it.  ", false);
			if(player.horns > 0) {
				MainScreen.text("Your horns fall off, landing on the floor with a heavy thud.  ", false);
				player.horns = 0;
				player.hornType = HORNS.NONE;
			}
			if(player.antennae > ANTENNAE.NONE) {
				MainScreen.text("Antennae pop free, and float lightly down towards the floor.  ", false);
				player.antennae = ANTENNAE.NONE;
			}
		}
		//EARS
		if(player.upperBody.head.earType != EARS.HUMAN) {
			MainScreen.text("Pain erupts from both sides of your head as your ears reform and move, returning to look like your old human ears!  ", false);
			player.upperBody.head.earType = EARS.HUMAN;
		}
		//Face
		if(player.faceType != FACE.HUMAN) {
			MainScreen.text("Your facial structure rearranges itself into a normal human visage, exactly like yours was before you came to this horrid place.", false);
	...
	if(eventNo == 11051) {
		...
		if(player.upperBody.chest.count() > 1) {
			...
			MainScreen.text("Your chest tingles and begins to feel lighter.  You hastily pull open your " + player.armorName + " and realize you only have " + allBreastsDescript() + " now!  ", false);
			if(player.upperBody.chest.BreastRatingLargest[0].nippleLength > 1) {
				MainScreen.text("Your nipples shrink down to a more normal size.  ", false);
		...
		if(player.upperBody.chest.list[0].breastRating > 7) {
			MainScreen.text("The weighty flesh that constantly hangs from your chest gets lighter and lighter, vanishing rapidly.  ", false);
			player.upperBody.chest.list[0].breastRating = 3 + rand(5);
			MainScreen.text("You now have " + allBreastsDescript() + ".  ", false);
			temp++;
		}
		//Single row
		else {
			...
			if(player.upperBody.chest.hasFuckableNipples()) {
				MainScreen.text("The vagina-like openings in your nipples close, sealing themselves shut.  ", false);
		...
		if(player.skinType != SKIN.PLAIN) {
			MainScreen.text("The skin on your body itches intensely as it sheds its " + player.skinDesc + ", revealing " + player.skinTone + " skin.  ", false);
		...
		//Nothing changed
		if(temp == 0) {
				MainScreen.text("You tingle briefly but feel no obvious change.  Your chest is already fairly human.", false);
		}
		eventParser(11055);
		return;
	}
	...
	if(eventNo == 11052) {
		...
		temp = 0;
		MainScreen.text("You feel a strange shivering sensation pass through you.  ", true);
		//Remove multiple.
		if(player.lowerBody.cockSpot.count() > 1) {
			MainScreen.text("Your " + player.multiCockDescriptLight() + " shiver and retract back towards your body.  When the process finishes you are left with only your " + cockDescript(0) + ".  ", false);
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			if(player.lowerBody.cockSpot.list[0].cockLength > 12) {
				MainScreen.text("A tingling sensation worms through your " + cockDescript(0) + " as it shrinks down to a more modest eleven inches.  ", false);
				player.lowerBody.cockSpot.list[0].cockLength = 11;
				temp++;
			}
			//Super thick nerf
			if(player.lowerBody.cockSpot.list[0].cockThickness > 2) {
				MainScreen.text("Your " + cockDescript(0) + "'s obscene thickness withers down to roughly two inches of girth.  ", false);
				player.lowerBody.cockSpot.list[0].cockThickness = 2;
				temp++;
			}
			//Humanitize
			if(player.lowerBody.cockSpot.list[0].cockType != CockType.HUMAN && player.lowerBody.cockSpot.list[0].cockType != CockType.DEMON) {
				MainScreen.text("The inhuman appearance of your " + cockDescript(0) + " shifts, the flesh rearranging itself into a more human configuration.  After a few seconds you have a very normal looking penis.  ", false);
				player.lowerBody.cockSpot.list[0].cockType = CockType.HUMAN;
				temp++;
			}
			//If demon cocked....
			if(player.lowerBody.cockSpot.list[0].cockType == CockType.DEMON) {
				MainScreen.text("Your " + cockDescript(0) + " tingles as the bumps begin to fade.  After a moment the flesh darkens, and every single nodule reappears.  <b>Your corrupt penis resisted the magic!</b>  ", false);
				temp++;
			}
		}
		//Balls shrink
		if(player.lowerBody.ballSize > 5) {
			temp++;
			MainScreen.text("The " + ballsDescriptLight() + " that constantly pull so heavily on your groin tingle and shrink down to a more managable size.  ", false);
		...
		if(player.lowerBody.vaginaSpot.count() > 0) {
			if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLICK) {
				MainScreen.text("The constant fluids leaking from your " + vaginaDescript(0) + " slow down, then stop.  ", false);
		...
		//Nothing changed
		if(temp == 0) {
			MainScreen.text("You tingle briefly but feel no obvious change.  Your crotch isn't really in need of becoming more human.", false);
		}
		eventParser(11055);
		return;
	}
	...
	if(eventNo == 11053) {
		spriteSelect(16);
		MainScreen.text("", true);
		if(player.lowerBody == LOWER_BODY.HUMAN) MainScreen.text("You feel as if you should slap yourself for stupidy.  Your legs are already normal!  You flush hotly as the corrupt magics wash over you, changing nothing.", false);
		else MainScreen.text("You collapse as your " + player.legs() + " are unable to support you.  The sounds of bones breaking and reshaping fills the room, but oddly you feel no pain, only mild arousal.  You blink your eyes and sigh, and when you look down again <b>you have normal human legs</b>!", false);
		player.lowerBody = LOWER_BODY.HUMAN;
		if(player.tailType > TAIL.NONE) {
			MainScreen.text("  A moment later, your feel something detach from above your backside.  <b>You no longer have a tail!</b>", false);
	...
	if(eventNo == 11056) {
		spriteSelect(16);
		MainScreen.text("You step forwards and grab her by the head.  With an abrupt twist you snap her neck, ending at least one small part of the demonic threat.", true);
...
/* Now called directly
	...
	if(eventNo == 11075) {
		...
		dungeonLoc = 0;
		if(dungeonLoc == 0) {
			MainScreen.text("You leave the cave behind and take off through the deepwoods back towards camp.", true);
			doNext(camp.returnToCampUseOneHour);
		}
		else eventParser(1);
		return;
	...
	if(eventNo == 11077) {
		if(flags[FlagEnum.ZETAZ_DOOR_UNLOCKED] == 0) {
			MainScreen.text("The door won't budge.", true);
			doNext(camp.campMenu);
			return;
		}
		else dungeonLoc = 16;
	...
	if(eventNo == 11150) {
		inDungeon = false;
		dungeonLoc = 0;
		MainScreen.text("You leave the door behind and take off through the desert back towards camp.", true);
		doNext(camp.returnToCampUseOneHour);
		return;
	}
	if(eventNo == 11151) {
