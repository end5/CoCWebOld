...
private static const DUNGEON_WITCH_ENTRANCE_GATEWAY: number		= 23;
private static const DUNGEON_WITCH_CAVERNOUS_COMMONS: number	= 24;
private static const DUNGEON_WITCH_WEST_WARRENS_MAIN: number	= 25;
private static const DUNGEON_WITCH_CHILDRENS_PLAYROOM: number	= 26;
private static const DUNGEON_WITCH_PREGNANT_LUST_ROOM: number	= 27;
private static const DUNGEON_WITCH_WEST_WARRENS_WEST: number	= 28;
private static const DUNGEON_WITCH_NURSERY: number				= 29;
private static const DUNGEON_WITCH_PHARMACY: number				= 30;
private static const DUNGEON_WITCH_EAST_WARRENS_MAIN: number	= 31;
private static const DUNGEON_WITCH_SLEEPING_CHAMBER: number		= 32;
private static const DUNGEON_WITCH_BATH_ROOM: number			= 33;
private static const DUNGEON_WITCH_EAST_WARRENS_EAST: number	= 34;
private static const DUNGEON_WITCH_CUM_WITCH_BEDROOM: number	= 35;
private static const DUNGEON_WITCH_CUM_WITCH_OFFICE: number		= 36;
private static const DUNGEON_WITCH_SACRIFICIAL_ALTAR: number	= 37;
private static const DUNGEON_WITCH_THRONE_ROOM: number			= 38;
...
//	inDungeon = false;
	...
	MainScreen.text("You leave the door behind and take off through the desert back towards camp.");
	doNext(camp.returnToCampUseOneHour);
}

public fightCumWitch():void {
	MainScreen.clearText();
	MainScreen.text("A robed witch crests one of the dunes, her sable skin glistening with moisture in the unforgiving desert sun.  She spies you, and her dusky lips curl up in a smile while a white staff materializes in her hands.  Playfully, the woman calls, \"<i>I'm going to cast a spell on you...</i>\"");
...
public openZeDoorToParadize():void {
	...
	if(flags[FlagEnum.ENTERED_SANDWITCH_DUNGEON] == 0) {
		MainScreen.text("You hesitantly touch the dark sphere, admiring its smooth, glossy finish.  Almost as soon as you come in contact with it, it recedes into the wall.  The doorway rumbles, a giant slab vanishing into the sandy depths, opening a portal to the inside.  Meticulous carvings inlaid with pearl depict large breasted witches in great quantity, and though the specific means of the glyphs are foreign to you, it's clear this place is some kind of sanctuary for sand witches.");
		flags[FlagEnum.ENTERED_SANDWITCH_DUNGEON] = 1;
	}
	//Repeat
	else {
		MainScreen.text("Just ahead is the familiar sight of the sand witches' coven.  It's hewn from a sandstone archway buried in the side of a dune.  Pearl-inlays of big-breasted, lactating witches decorate the way inside, making it clear what you can expect to find inside.");
	}
...
public getMockedForSmallDongBySammitchMob():void {
	...
	if(player.cumQ() < 250) MainScreen.text("  A little squirt of spooge dirties the witch's two fingers, creating a small web of spunk between them.  \"<i>Heh, knew it</i>\" the witch smugly says, giving you a pathetic look.");
	else if(player.cumQ() < 500) MainScreen.text("  Spurts of sweet release shoot from your [cock smallest], glopping the bitch-witch's hand.  \"<i>Nothing too impressive</i>\" she snidely remarks as she rubs your spunk off on the dirt floor.");
	else MainScreen.text("  Your [cock smallest] shoots out a barrage of cum onto the bitch's face, caking her disbelieving features into a thick layer of slimy goop as her fellow sisters laugh, pointing at her spunk-covered face.");
	...
	if(player.lowerBody.balls > 0) MainScreen.text("  Your [balls] didn't have enough time to make another full batch.");
	MainScreen.text("  It's very watery, dripping down between her toes. Your cock has a dirty smudge of where her foot was.");
	...
	if(player.isBiped()) MainScreen.text("between your legs");
	else MainScreen.text("to your side");
	MainScreen.text(".  She takes your dirty cock in her hand.  She must have been dared to do something.  Looking at her peers with pleading eyes, she says, \"<i>Are you sure about this?</i>\"  A small group of witches egg her on, telling her to \"<i>do it.</i>\"");
	...
	//If balls:
	//[If balls & pussy:]
	if(player.lowerBody.balls > 0 && player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  She even cups your [balls] but moves to touch your [vagina] when she feels its wetness dripping off the testes."); 
	else if(player.lowerBody.balls > 0) MainScreen.text("  She even cups your tender [balls] and lightly massages them.");
	else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  She even lightly caresses the lips of your pussy.");
...
public femaleGirlsLoseToSammitches():void {
	...
	if(player.hasMuzzle()) MainScreen.text("  She fucks your muzzle like a cock as it goes into one cunt while she plays with the other one.");
	else MainScreen.text("  She grinds her dampening pussies against your lips while her clits are pressed up against your nose.");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) {
		MainScreen.text("Amazingly, as the oil spreads over your crotch, the flesh splits, revealing a small, wet entrance with a little clit.");
		player.createVagina();
		player.lowerBody.vaginaSpot.list[0].clitLength = 0.25;
	}
	MainScreen.text("Their warm streams run all over your form: down your [chest], off your [hips] and teasingly around your [vagina].  At this point, you want it so badly that you squeeze your chest, pinch your nipples and stroke your [clit].");
	...
	if(!player.hasMuzzle()) MainScreen.text("grinding on your face");
	else MainScreen.text("fucking your muzzle while she plays with herself");
	MainScreen.text(" and the four others whining out their enjoyment, making your own pussy feel neglected and needing something to fill it.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text("between");
	else MainScreen.text("on");
	MainScreen.text(" your [chest].");
	
	MainScreen.text("\n\n\"<i>Hey girls, hurry up and pick a cock already!  The ");
	if(flags[FlagEnum.SAND_WITCHES_FRIENDLY] > 0) MainScreen.text("slut");
	else MainScreen.text("initiate");
	MainScreen.text(" is getting a bit randy!</i>\"");
	
	if(flags[FlagEnum.SAND_WITCHES_FRIENDLY] > 0) MainScreen.text("\n\nYou would think more on that word, but you're too focused on the cock toy on your chest. You look at it with lust in your eyes and beg its owner to fuck you with it, make you cum with it, to make you her bitch.");
	...
	//[Next]
	menu();
	if(flags[FlagEnum.SAND_WITCHES_FRIENDLY] > 0) MainScreen.addButton(0,"Next",sandWitchMobNotBadEnd);
	else MainScreen.addButton(0,"Next",sandWitchMobBadEnd);
}

//[if for fun]
public sandWitchMobNotBadEnd():void {
	MainScreen.clearText();
	MainScreen.text("You wake up hours later outside the dungeon with all your belongings, surprisingly not missing any gems.");
	dungeonLoc = 0; //Replaces	inDungeon = false;
	player.orgasm();
	dynStats("lib", 1, "sen", -1);
	cleanupAfterCombat();
...
public memeberedFolksFindTrueWuv():void {
	...
	if(player.lowerBody.balls > 0) MainScreen.text(" while your [balls] start to work, clenching and relaxing as they brew a thick load of fuckbatter thanks to the chemical jump-start.");
	else MainScreen.text(" while your body starts to work, clenching and relaxing as it rushes to brew a thick load of fuckbatter thanks to the chemical jump-start.");
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		MainScreen.text("  Similarly textured cunnies playfully snake around your other phallus");
		if(player.lowerBody.cockSpot.count() > 2) MainScreen.text("es");
		MainScreen.text(", gleefully slurping up your twitching hardness inside ");
		if(player.lowerBody.cockSpot.count() == 2) MainScreen.text("itself");
		else MainScreen.text("themselves");
		MainScreen.text(".");
	}
	MainScreen.text("Slurping noisily, the cunt");
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s piston");
	else MainScreen.text(" piston");
	MainScreen.text(" and bob, splattering ");
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("their");
	else MainScreen.text("its");
	MainScreen.text(" dripping saliva all over your exposed crotch.  Your [hips] lurch up to meet the hungry tentacles.  Feeding the creature's cum-lust is something your body seems driven to do at this point, and you gurgle in wordless delight at her ministrations.");
...
public yoYouBeatUpSomeSandWitchesYOUMONSTER():void {
	MainScreen.clearText();
	flags[FlagEnum.SANDWITCH_MOB_DEFEATED] = 1;
	MainScreen.text("The sand witches all collapse to the floor in a vast puddle of milk and pussy juice, ");
	if(monster.HP < 1) MainScreen.text("nursing their wounds");
	else MainScreen.text("frantically making out with each other, huddling into a squirming orgy on the floor");
	MainScreen.text(".  You stand alone, the sole victor, triumphant against your enemies; you can easily continue deeper into the caves, now.");
	...
	if(player.lust >= 33) {
		...
		if(player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(0,"Fuck One",dicksHaveSexWithAWitch);
		if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.addButton(1,"Forced Lick",forceCunnilingusRimjobClitAndNipple);
	}
...
public dicksHaveSexWithAWitch():void {
	MainScreen.clearText();
	MainScreen.text("Looking over the ");
	if(monster.HP < 1) MainScreen.text("painfully ");
	...
	if(monster.lust > 99) MainScreen.text("masturbating companions");
	else MainScreen.text("groaning companions");
	...
	if(monster.lust > 99) MainScreen.text("  Judging by the omnipresent flush on her cheeks and the scent of female honey inundating the air, she's more than ready to help you with your problem.");
	else MainScreen.text("  Her cheekbones slowly color at the thought, turning almost entirely beet-red.  At the same time, her nipples seem to tighten beneath her robes, signalling her body's acceptance of your intent.");
	...
	MainScreen.text("\n\n[EachCock] rises up ");
	if(player.tallness < 60) MainScreen.text("through her buttcheeks");
	else MainScreen.text("along her arched spine");
	MainScreen.text(", fully engorged and ready for action.  For now, you're content to continue to molest your chosen fuck-toy, dipping a pair of fingers into her ");
	if(monster.lust > 99) MainScreen.text("well-soaked");
	else MainScreen.text("rapidly-moistening");
	MainScreen.text(" cunts in order to make sure she's prepared.  The witch bites her lip to stifle a lewd moan, so you drag the pads of your fingers across each of her clits simultaneously.  This shatters whatever restraint she was showing into a little more than slick cunt-juice.  Even now, it's dripping down her inner thighs.  She's properly moaning with your strokes now, and you judge she's ready.");
	...
	if(player.smallestCockArea() <= monster.vaginalCapacity()) {
		...
		if(y >= 0) MainScreen.text(" and your " + cockDescript(y) + " aimed at her other one");
		MainScreen.text(".  Her knees buckle, but you're ready for it, and she slides straight onto your supporting shaft");
		if(y >= 0) MainScreen.text("s");
		MainScreen.text(", impaling herself to the hilt.");
		...
		if(y < 0) MainScreen.text("pussy clings tight to you");
		else MainScreen.text("pussies cling tightly to you");
		MainScreen.text(".  Supported entirely by your body, the sand witch's muscles go slack (aside from the ones hugging your dick");
		if(y >= 0) MainScreen.text("s");
		MainScreen.text(").  She's breathing nice and heavy as you lift her and swing your hips back, pulling most of your " + cockDescript(x));
		if(y >= 0) MainScreen.text(" and " + cockDescript(y));
		MainScreen.text(" out before reversing the motion nigh-instantly, slamming yourself in to the hilt.  The once proud enchantress is reduced to a quivering puddle of fuck by your jackhammering thrusts, so taken by the sensations in her quim");
		if(y >= 0) MainScreen.text("s");
		MainScreen.text(" that her brains are practically drooling out of her cunts.");
		...
		if(y < 0) MainScreen.text(cockDescript(x));
		else MainScreen.text("double dongs");
		MainScreen.text(".  Waves of rippling contractions sheath you in shimmering, ephemeral ecstasy.  Like something clicking into place in your brain, a biological prerogative asserts itself, unleashing a heat that boils up out of your core like lava from a volcano.  [EachCock] spasms and releases a flow of salty jism.  The witch must feel it, because as soon as your seed makes contact with her juicy cunt");
		if(y >= 0) MainScreen.text("s");
		MainScreen.text(", her clingy lips seem to tighten around your girth to seal it in.");
		if(player.cumQ() >= 400) {
			MainScreen.text("  That fails to contain the totality of your virile offshoots, and ribbons of the stuff squirt out the sides of her lips.");
			if(player.cumQ() < 2000) MainScreen.text("  You do manage to gift her with a nice little cum-paunch for her efforts, though.");
			else MainScreen.text("  You're cumming so fast that that venting doesn't help her much.  Slowly, her belly begins to bulge outward.  It stretches and wobbles as it fills with spunk, not stopping until she looks months pregnant.  The liquid way that it jiggles with every movement will reveal the contents of her middle to any who dares look at her.");
		}
		MainScreen.text("\n\nSetting her down in front of her sisters, you let her slide off your ");
		if(y >= 0) MainScreen.text("twin cocks");
		else MainScreen.text(cockDescript(x));
		MainScreen.text(" onto the floor as an example of just what you're capable of.  You can beat the best they have and fuck them into a twitching puddle in the dirt.  Though the mob seems cowed by your display of authority, they're still fucking themselves to the sight of what you've done to their sister.  You make the rest of them take turns licking your spent spunk and caked-on cunt-juice from your phallus");
		if(y >= 0) MainScreen.text("es");
		MainScreen.text(" before getting dressed and getting back to business.");
	}
	//Too Big For Fucks
	else {
		...
		if(player.cumQ() >= 500) MainScreen.text("struggling to swallow it");
		else MainScreen.text("swallowing it");
		MainScreen.text(".  She squeezes her arms together, which pushes her boobs tighter around your orgasmically pulsating length.  It feels so good that it feels like a whole new orgasm kicks off in your [balls], even though you've already started to cum!  ");
		if(player.cumQ() < 250) MainScreen.text("The cute woman swallows every single drop.  She even gives up a satisfied moan when she finishes.");
		else if(player.cumQ() < 1000) MainScreen.text("The cute woman's cheeks balloon as she tries to contain it all.  She swallows admirably in spite of your heavy jizz-flow, only losing a few white trickles out the corners of her stretched mouth.");
		else MainScreen.text("The cute woman's cheeks flood with your obscene cum-flow.  She tries to swallow it down, but no matter how fast her fevered gulps are, the stuff still floods in with such force that it squirts from the corners of her mouth.  She just gives up after a few moments and lets it run down her chin in a waterfall.  Most of the jism puddles on the floor below her, soaking into her ponytail and matting it with salty spunk.");
...
public forceCunnilingusRimjobClitAndNipple():void {
	MainScreen.clearText();
	MainScreen.text("You eye the defeated mob");
	if(player.HP < 1) MainScreen.text(" through their torn-up robes, noting many large, dripping breasts and more than a few pairs of supple cunt-lips");
	else MainScreen.text(" through their squirming, orgiastic motions, noting their many large, dripping breasts and gushing multi-cunts");
	MainScreen.text(".  They may ");
	if(player.stats.cor < 33) MainScreen.text("be misguided in fighting you");
	else if(player.stats.cor < 66) MainScreen.text("be a little over-zealous in defending their home");
	else MainScreen.text("be crappy fighters");
	MainScreen.text(", but feeling a stirring in your nethers, you can think of at least one good use for them.  You start to strip out of your [armor], groping your butt with one hand; maybe there's a second purpose they can serve...");
	...
	if(player.wetness() >= 4) MainScreen.text(", spraying your musky girl-goo across the pretty faces at your groin, soaking them all the way to their first rows of tits.  Your juices mix with the milk dripping from their nipples into a sticky alabaster treat that makes you lick your lips and shudder in delight. The seductress on your [vagina] gurgles and sighs as she tries to keep up with your copious lady-spunk, but the way you squirt, she seems doomed to failure.");
	else if(player.wetness() >= 2) MainScreen.text(", sluicing secretions down the lips of your would-be pussy-slave, forcing her to work her throat in order to try and keep up with it.  You're so goddamned wet, and the way your channel is clenching and undulating around her tongue just forces out more of your juice.");
	else MainScreen.text(", dribbling your slippery girl-cum straight into your would-be pussy-slave, forcing her to swallow your tangy flavor.  It's hard not to turn into a dripping mess with the way her tongue works your spasming pussy, but neither of you seems to mind.");
	if(player.lowerBody.cockSpot.hasCock()) {
		MainScreen.text("  Above, [eachCock] fires lances of proper, virile spunk onto the girls' exposed backs to properly mark them.");
		if(player.cumQ() >= 500) {
			MainScreen.text("  It turns their bronzed skin into a soupy white, then dangles to the ground in thick ropes");
			if(player.cumQ() >= 1500) MainScreen.text(", each of which begins pooling into a puddle");
			if(player.cumQ() >= 2500) MainScreen.text(".  Those puddles combine to form a lake of jism beneath these lusty slaves bodies, putting their combined milky tits to shame");
			MainScreen.text(".");
		}
	}
	...
	if(player.lactationQ() >= 200) MainScreen.text(" drink their fill");
	if(player.lactationQ() >= 1000) MainScreen.text(", or at least as much as they can handle of your lactic capacity,");
	else if(player.lactationQ() < 200) MainScreen.text(" slowly suckle you");
	MainScreen.text(" while you come down from your high.  Why couldn't they have just done this to start?");
...
public gangrush():void {
	MainScreen.text("The witches close ranks and advance with raised fists, intent on beating you into submission!\n");
...
public headbuttABitch():void {
	MainScreen.text("The crowd parts, and a stockier, sturdier sorceress ambles out, fists up and head cocked back.  She makes to punch at you before pulling her fist at the last second, snapping her head forward in a powerful headbutt!  You barely have time to react!");
...
public sandstonesAreCool():void {
	MainScreen.text("The sandstorm whirling around the room suddenly ceases, and all the tiny sand particles gather together into balls, growing into several smooth stones.  Then, all the sand stones fall to the ground and slither towards you.");
	...
	if(!combatMiss() && !combatEvade() && !combatFlexibility() && !combatMisdirect()) {
		...
		if(!player.isGoo()) {
			...
			//If PC has 1 cock:
			if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("  A stone crawls onto your [cock].");
			//[If PC has multi-cocks: 
			if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("  A bunch of the stones crawl onto your " + player.multiCockDescriptLight() + ".");
			if(player.lowerBody.cockSpot.hasCock()) bonus++;
			//[If PC has cunt]
			if(player.lowerBody.vaginaSpot.hasVagina()) {
				MainScreen.text("  One stone slides up your inner thigh");
				if(player.lowerBody.balls > 0) MainScreen.text(" behind your [sack]");
				MainScreen.text(" and pops itself right into your [vagina]");
				if(player.hasVirginVagina()) MainScreen.text(", robbing you of your virginity as a trickle of blood runs down your [leg].");
				bonus++;
			}
			//[If PC has balls:
			if(player.lowerBody.balls > 0) {
				MainScreen.text("  A small set of stones settle on your [balls].");
				bonus++;
			}
			MainScreen.text("  " + num2Text(player.upperBody.chest.countNipples()) + " crawl up to your chest and over your top " + nippleDescript(0) + "s");
			if(player.bRows() > 1) {
				if(player.bRows() == 2) MainScreen.text(" and");
				else MainScreen.text(",");
				MainScreen.text(" your middle " +  nippleDescript(1) + "s");
				bonus++;
			}
			if(player.bRows() > 2) {
				MainScreen.text(", and your bottom " + nippleDescript(2) + "s");
				bonus++;
			}
			MainScreen.text(".");
			MainScreen.text("  The last stone travels up the back of your [legs] and slides right into your [asshole].");
		...
		player.statusAffects.add(new StatusAffect("LustStones",bonus,0,0,0)));
		dynStats("lus", bonus * 2 + 5 + player.stats.sens/7);
	}
	//[If attack misses]
	else {
	...
	monster.statusAffects.remove("Sandstorm");
	combatRoundOver();
}
...
public drankSomeMialk():void {
	...
	dynStats("lus", 4 + player.stats.lib/10);
...
public sandWitchMobAI():void {
	...
	else if(monster.statusAffects.has("Sandstorm") && rand(2) == 0 && player.findStatusAffect(StatusAffects.LustStones) < 0) sandstonesAreCool();
...
public cumWitchAI():void {
	...
	//Dicks only
	if(player.lowerBody.cockSpot.hasCock()) choices[choices.length] = cumMagicAttack;
	...
	//VAGOOZLES
	if(player.lowerBody.vaginaSpot.hasVagina()) choices[choices.length] = cumHungerAttack;
...
public bukkakeAttack():void {
	...
	MainScreen.text("The Cum Witch moans and daintily peels her robes away from her swollen cock-flesh.  A bubble of precum pops wetly from her urethra to splatter on the floor as her balls suddenly swell.  You look back up in time to see the telltale glow of magic surrounding her staff, but then she's thrusting her hips at you, lewdly humping the air as she unleashes rope after thick rope of potent jism in your direction!\n");
...
public cumHungerAttack():void {
	MainScreen.text("Moaning luridly, the Cum Witch swivels her staff and opens her hand to spread her fingers wide.  At the same time, you feel her magic slam into your midsection, burrowing into your womb.  ");
	if(player.pregnancyIncubation > 0) {
		MainScreen.text("Yet, whatever she tries to do fails, as her otherworldly conjuration falls apart as soon as soon as it reaches you.");
		combatRoundOver();
		return;
	}
...
public genderConfusionAttack():void {
	...
	MainScreen.text("Touching her alabaster staff to her brow, just under the brim of her hat, the Cum Witch makes a brief incantation and fixes you with her gaze.  Her eyes flash blindingly white, and then you feel her inside you, rifling through your memories, digging up memories of your childhood, your past, and throwing them against you.  ");
	if(player.stats.int/5 + rand(20) + player.level/2 < 18) {
		MainScreen.text("She batters your consciousness with conflicting memories of your gender, utterly dazing you.  How can you fight when you can barely tell who you are anymore?");
		player.statusAffects.add(new StatusAffect("Confusion",0,0,0,0)));
	}
	else {
		MainScreen.text("You parse the flood of information with mental focus and expel the intruder from your mind with a clenching of your sizable intellect.");
	}
	combatRoundOver();
}
//*Attack: Shell
public shellDefense():void {
	...
	MainScreen.text("The Cum Witch holds her staff in both hands and rotates it in a circle, chanting all the while.  Her voice rises in pitch and intensity until she's screaming out unwords of power.  With one final cry, she slams her staff down into the ground hard enough to kick up a puff of sandy dust.  It quickly settles, but the Cum Witch has some kind of glittering, reflective shield around herself now!");
	monster.statusAffects.add(new StatusAffect("Shell",3,0,0,0)));
	combatRoundOver();
}
...
public cocknosisAttack():void {
	MainScreen.text("Lifting her robes enticingly, the Cum Witch reveals her ");
	if(monster.lust < 50) MainScreen.text("half-hard");
	else if(monster.lust < 70) MainScreen.text("hard");
	else if(monster.lust < 85) MainScreen.text("throbbing");
	else MainScreen.text("hard, dripping");
	MainScreen.text(" cock.  She gently begins to sway her hips, bouncing back and forth with near-mechanical precision, her softly bobbing cock catching your eyes with its metronome-like precision.  She softly begins to speak, enunciating each word to the time and tune of her movements.");
	...
	if(player.stats.int/20 + rand(20) >= 13) {
		MainScreen.text("You chuckle at her crude attempt to hypnotize you with her member.  She stomps her foot in irritation and drops her robes back into place.");
	}
	else {
		MainScreen.text("The Witch smirks, though you're too focused on her cock to see, and she continues, \"<i>Good " + player.mf("boy","girl") + ".  Watch it sway.  You're so focused on my cock now that the world is just falling away around it, sinking into nothingness, leaving only that wonderful cock behind for you to watch.  And since it's filling your view, you can feel it filling your mind as well, can't you?</i>\"");
...
public turnIntoASammitch():void {
	MainScreen.clearText();
	MainScreen.text("Helplessly, you ");
	if(player.lust > 99) {
		if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("dig your hands into your loins, frigging your hungry cunt so hard your juices splatter the Witch's robe.");
		else MainScreen.text("dig your finger into your [asshole], frigging it like your life depended on it.");
	}
	else MainScreen.text("try to rise, but all you manage to do is slip back down to the ground.");
	MainScreen.text("  The Cum Sorceress smiles and releases her staff.  Instead of clattering to the ground, it dematerializes a piece at a time, fading away in a way that would be mesmerizing were you not otherwise occupied.  She hikes her robes up and pulls off her hat with them, shaking her surprisingly light, blonde hair free.  \"<i>Now, all that got me pretty worked up, and I don't think the girls will mind if I give you your first sampling of cum witchery.  What do you think, hun?</i>\" she asks, glancing back towards the comatose witch behind her.");
	...
	if(player.pregnancyType == 0) MainScreen.text("let's go ahead and get your first daughter inside you, shall we?");
	else MainScreen.text("let's go ahead and have some fun, shall we?  We'll have to wait until you pop out whatever's in your womb before I give you your first witch child.");
	MainScreen.text("  There will be plenty of time to make you one of us while we're waiting.</i>\"");
	...
	if(monster.lust < 65) MainScreen.text(" herself to full hardness, smiling when thick strings of cum begin to drizzle from her swollen cock-tip.");
	else if(monster.lust < 85) MainScreen.text(" herself until cum begins to drizzle from her swollen cock-tip.");
	else MainScreen.text(" herself, gathering up the constantly-dripping cum and smearing it all over her shaft.");
	...
	if(player.lust > 99) MainScreen.text(", even though your hand is knuckle-deep in your twat");
	MainScreen.text(".");
	...
	if(player.bRows() < 2) MainScreen.text("get four big, milky tits");
	else if(player.lactationQ() < 200) MainScreen.text("have your tits made into perfect milkers");
	else MainScreen.text("get your four, big tits milked");
	MainScreen.text(" or to get your first proper administration of witch-cum.");
	...
	if(player.pregnancyIncubation > 0) MainScreen.text("  It seems you've somehow managed to forget that you're already pregnant, and the Cum Witch doesn't seem keen to remind you.");
	...
	//Next
	menu();
	MainScreen.addButton(0,"Next",chicksLoseToSandWitchesBadEndFinale);
}
...
public cumWitchCumPumpBadEnd():void {
	MainScreen.clearText();
	MainScreen.text("Laid low by ");
	if(player.HP < 1) MainScreen.text("your wounds");
	else MainScreen.text("by the lust coursing through your veins");
	MainScreen.text(", you slump over against a desk, leaning heavily on it for support while ");
	if(player.HP < 1) MainScreen.text("you struggle to rise");
	else MainScreen.text("you struggle to get your crotch");
	MainScreen.text(".");
	...
	if(player.cumQ() > 1000) {
		if(player.lowerBody.balls > 0) MainScreen.text(" and such virile, cum-swollen testes.  An excellent breeder for sure!");
		else MainScreen.text(" and such a virile body!  An excellent breeder for sure!");
	}
	else MainScreen.text("but with such a pathetic virility.");
	MainScreen.text("  A little magic later, and you'll be quite suited to breeding daughters with me.  Perhaps if you do well enough, I'll allow you to seed MY womb.</i>\"");
	...
	if(player.HP < 1) MainScreen.text("grunt and strain in response, still trying to fight, even though you know it's hopeless.");
	else MainScreen.text("groan and touch yourself, trying to show her just how 'in the mood' you are.");
	MainScreen.text("  She affectionately ruffles your " + hairDescript() + " and tuts, \"<i>Now, now, it's cute that you think you still have some agency here, but you really ought to just lay back and let me take worry about it!</i>\"");
	...
	//[NEXT]
	menu();
	MainScreen.addButton(0,"Next",beACumPumpPartII);
	player.orgasm();
	dynStats("lib", 100, "sen", 100);
}
...
public beACumPumpPartII():void {
	...
	MainScreen.text(".  Of course, the wicked witch only releases you at this point.  \"<i>Do I look like one of the cum-hungry wenches around here?</i>\"");
	...
	MainScreen.text(" wilting from the force of your surprise and fear.");
	...
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
	MainScreen.text(", \"<i>I did not think you would get this far.  Your will is truly mighty, [name], but the task I gave you is something I've never been able to do.  Ten is as far as I ever got.  I would not have you damage yourself just yet.</i>  She pulls open the curtain and bellows, \"<i>Next!</i>\"");
	...
	if(player.smallestCockArea() >= 100) MainScreen.text("  The penetration is eased somewhat by your shrinking dick.  You glance at the sand witch in awe, noting her glowing hands.  She gives you a knowing wink and turns, her spell complete, leaving you to enjoy yourself.");
	...
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("Her voluminous butt is thoroughly smeared by the goo from the rest of your " + player.multiCockDescriptLight() + ", shining with white from your magical virility.  ");
	MainScreen.text("She rides you with surprising skill for one who seems to be so hesitant about such a healthy jizz output, her four breasts squishing tight against [chest].  Her tight cunny squeezes and ripples around you, rapidly growing soaked with white, thoroughly spunk-lubed and hungry for more.");
	...
	//[Next]
	menu();
	MainScreen.addButton(0,"Next",cumPumpVolumeThree);
}

public cumPumpVolumeThree():void {
	MainScreen.clearText();
	MainScreen.text("With the champion brainwashed into a little more than a loyal cum-pump for the sand witches, they grow in number and strength with alarming speed.  In the space of eight years, the desert is transformed into a verdant forest.  For better or for worse, the witches finally rival the demons in power.  They spread their influence wider, eventually ");
	if(player.statusAffects.has("FactoryOverload")) MainScreen.text("destroying");
	else MainScreen.text("rescuing");
	MainScreen.text(" Marae.  Their abilities, numbers, and familiarity at dealing with demons lend them great success at battling Lethice's growing hordes, and within the span of another decade, the demons are exterminated.  The witches are hailed as saviors, and young females of all races seek to join them.  You, of course, inseminate all of them.");
	gameOver();
}
...
public repeatLoseToCumWitchForDudes():void {
	...
	if(player.HP < 1) {
		...
		if(player.lust < 40) MainScreen.text("flaccid");
		else if(player.lust < 60) MainScreen.text("semi-flaccid");
		else if(player.lust < 70) MainScreen.text("semi-hard");
		else MainScreen.text("turgid");
		MainScreen.text(" weight.  Pumping her hand with deft strokes and watching your expression intently, the curvy woman works you into a pleasant, erotic warmth, excitement coursing through your body with such fervor that your remaining wounds seem insignificant in comparison.");
	}
	//Lust:
	else {
		MainScreen.text("Dropping down on your [legs], you rip off your [armor] and flop onto your back so that you can focus on abusing your genitalia.  [EachCock] is already hard enough to be leaking pre");
		if(player.lowerBody.balls > 0) MainScreen.text(", and your [balls] are practically quaking with need");
		MainScreen.text(".  The ebony futanari laughs at you as she confidently strides forward and discards her hat.  \"<i>Did you really have any intention of fighting me, or did you ask the Sand Mother to send us out so that we could abuse your insatiable libido?  I don't know how you didn't wind up captured, but I suppose I can tend to your needs... this time.</i>\"");
	...
	if(player.cockThatFits(monster.vaginalCapacity()) < 0) {
		...
		if(player.biggestCockLength() < player.tallness/2) MainScreen.text("[chest]");
		else if(player.biggestCockLength() < player.tallness/1.6) MainScreen.text(player.face());
		else MainScreen.text(hairDescript());
		MainScreen.text(".  The black spellcaster comments, \"<i>I must admit it is fun to look at, but it doesn't look like it's ready to blow just yet.  How about a little encouragement?</i>\"");
		...
		MainScreen.text("your [nipples]");
		if(player.upperBody.chest.hasFuckableNipples()) MainScreen.text(", sinking inside your moist tit-holes almost immediately");
		MainScreen.text(".  With two vibrators savaging your [nipples] and the black-hued beauty riding your cock like her own personal rocket, you can barely contain yourself.  Your back arches, and you try to buck your hips, to fuck her, her cock, whatever, but you're still too exhausted from the fight to shift her body weight.  You can do nothing but lie there while she uses you, humping your cock, her ebony length dragging its lurid pleasure across your " + cockDescript(x) + " while you wriggle and writhe ecstatically.");
		...
		if(player.lowerBody.balls > 0) {
			MainScreen.text("[balls], rolling the ");
			if(player.lowerBody.ballSize < 2) MainScreen.text("petite ");
			else if(player.lowerBody.ballSize >= 5) MainScreen.text("weighty ");
			MainScreen.text("orbs across her soles.");
		}
		else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("[vagina], slipping a few toes through your folds while a big toe diddles your [clit].");
		else MainScreen.text("taint, softly stroking the sensitive skin between your groin and [asshole].");
		...
		//[Facial] [No Facial]
		dynStats("lus=", 100, "resisted", false);
		MainScreen.addButton(0,"Facial",tooBigCumWitchLossFacial);
		MainScreen.addButton(1,"No Facial",tooBigCumWitchLossNoFacial);
	}
	else {
		...
		MainScreen.text("loud and firm enough for your dazzled mind to cogitate her words: \"<i>Oh, my pretty bitch " + player.mf("boy","girl") + ", feel the pressure building up inside you.  Higher and higher now... It's almost too much, isn't it?  You don't need to answer, just feel it grow thicker and hotter.  You're going to cum like a geyser, because of me, and you're going to love it.  You'll always want to cum for me, won't you?</i>\"");
		...
		if(player.lowerBody.cockSpot.count() > 1) { 
			MainScreen.text(" while your extra erection");
			if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s spurt");
			else MainScreen.text(" spurt");
			MainScreen.text(" some cream up the ebony arch of your mistress's back");
		}
		else MainScreen.text(" from her bouncing, ebony erection");
		MainScreen.text(".  At the same time, her clenching, pussy climaxes around you while you fill it.  You're spraying ropes of spooge into her so fast that it's backwashing out around your " + cockDescript(x) + ", the veins pumping almost as hard as your overactive reproductive system.");
...
public tooBigCumWitchLossFacial():void {
	MainScreen.clearText();
	MainScreen.text("Ruefully, you tell her that you don't mind a little cum in exchange for having your own abilities enhanced.  The knowing grin that spreads across her face makes it seem like she knew the result was a foregone conclusion.");
	if(player.stats.cor < 33) MainScreen.text("  You shudder at the knowledge of what's going to happen to you.  Just why did you agree to this?");
	else if(player.stats.cor < 66) MainScreen.text("  You catch yourself licking your lips as the knowledge of what is about to happen to you sinks in. Despite hastily stopping yourself, the Cum Witch still saw, and she smiles.");
	else MainScreen.text("  You give her a lewd look and lick your tongue across your lips as you anticipate the big, fat load she's going to feed you, hungry for wet, decadent pleasure regardless of inhibitions.");
	
	MainScreen.text("\n\nThe hot moisture she's secreting all over your " + cockDescript(x) + " seems to be affecting you as well as the vibrations ");
	if(player.upperBody.chest.hasFuckableNipples()) MainScreen.text("in");
	else MainScreen.text("on");
	MainScreen.text(" your [nipples].  It feels like there's so much blood being forced inside your erection that the sheer over-tumescence will make you burst.  The need is overwhelming.  You HAVE to cum, and you're so wet, so stained with spunk and juice that there's no time like now.  The Cum Witch's heels press in on your ");
	if(player.lowerBody.balls > 0) MainScreen.text("[balls], compressing them slightly as a tingle of magic lances into you, fattening them under her ministrations");
	else if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("[vagina], digging into the slippery slit as a tingle of magic spreads through it and into your core, thickening some organ inside you");
	else MainScreen.text("taint, rubbing it as a tingle of electric need phases through your " + player.skinFurScales() + " to assault your swelling prostate and seminal vesicles");
	MainScreen.text(", and in that moment, you lose all ability to hold out.  You're cumming, and you're cumming now.");
	...
	if(player.biggestCockLength() >= player.tallness/1.6) MainScreen.text("  The Cum Witch smiles as she holds it, bending it to make sure the overlong shaft deposits its thick load on your " + player.face() + " where it belongs.");
	MainScreen.text("  Your captor is still humping it even as you cum, moaning as the bulges of copious cream press on her smaller boner.  Her visage is alight with ecstasy, and her heavy nipples are dragging on your [chest] as she begins to grunt, her motions going jerky in her ecstasy.");
	...
	if(player.lowerBody.ballSize < 7) player.lowerBody.ballSize++;
	player.orgasm();
	dynStats("lib", 1, "sen", 1);
	cleanupAfterCombat();
}
...
public tooBigCumWitchLossNoFacial():void {
	...
	if(player.lowerBody.balls > 0) MainScreen.text(", just like your own");
	MainScreen.text(", and she smiles cruelly as her feet press harder on you, suddenly tingling with supernatural forces.  Almost painful heat floods through your middle as she frots you harder and harder, inadvertently tit-fucking your immense erection all the way to orgasm.");
	...
	if(player.lowerBody.ballSize > 5) player.lowerBody.ballSize -= Math.round(player.lowerBody.ballSize * 0.333);
...
public TDMsLoseToCumWitchScene():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		//if (cocks = 2)
		if(player.lowerBody.cockSpot.count() == 2) MainScreen.text("your other phallus.");
		//[if (cocks > 2)
		else MainScreen.text("the rest of your " + player.multiCockDescriptLight() + ".");
		MainScreen.text("  The futa on top of you continues to smile mysteriously, and spares not another moment of her attention on your genitals.");
	}
	...
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("one of ");
	MainScreen.text("yours...");
	...
	if(player.lowerBody.balls > 0) MainScreen.text("the one part of your manhood that you can still feel, your [sack].");
	else MainScreen.text("a part of your body just behind and above the base of [eachCock], probably your prostate.");
	...
	if(silly()) MainScreen.text("in both body and in [balls].");
	else MainScreen.text("in both body, and in spirit.");
...
public defeatedByCumWitch():void {
	if(inDungeon) {
		//Dudally-diddly.
		if(player.lowerBody.cockSpot.hasCock() && (player.gender == 1 || rand(2) == 0)) cumWitchCumPumpBadEnd();
...
public declineSandWitch():void {
	MainScreen.clearText();
	MainScreen.text("Dusting yourself off, you lower your [weapon] and leave the cum witch to recover from the humiliation of losing to you.  The haunted, hungry look in her eyes leaves little doubt that she'll challenge you again or that she still wants to fuck you.  For now, she slips down into her own puddled cum, idly touching herself.");
	if (inCombat)
		cleanupAfterCombat();
...
public menFuckUpSomeCumWitch():void {
	...
	let x: number = player.cockThatFits(monster.vaginalCapacity());
	let y: number = player.cockThatFits2(monster.vaginalCapacity());
	MainScreen.text("You shuck out of your [armor] in eager anticipation, [eachCock] aleady ");
	if(player.lust < 50) MainScreen.text("half-hard");
	else MainScreen.text("rock-hard");
	MainScreen.text(" and pulsing with growing readiness.  The cum witch looks up at you with disdain, but the rigidity of her shaft and sloppy wetness of her flushed quim leave no doubt as to her state.   Her body blushes honestly as you reposition her, spreading her legs nice and wide.  You have to lift her hefty sack to expose the thick, wet lips of her pussy.  Jumping in response, her fat cock oozes a trickle of precum onto her dusky belly, oiling her dusky skin into a sensual shine.");
	...
	if(player.cockThatFits(monster.vaginalCapacity()) < 0) MainScreen.text("most of");
	else MainScreen.text("all of the way");
	MainScreen.text(" inside her, sheathed deep in her under-used twat.  Her tight passage fits around your " + cockDescript(x) + " like a custom-made glove, a slippery warm embrace that threatens to rob you of your very reason.  The witch begins pumping her ebony cock along with the motions of your hips, throwing her head back in wordless pleasure that only a true hermaphrodite can experience.");
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Two can play at that game.");
	//Herm sexback
	if(player.gender == 3) {
		...
		MainScreen.text("\n\n");
		if(player.lowerBody.balls > 0) MainScreen.text("Your sweaty nutsacks slide over each other while you fuck, compressing together into a mass of sweaty orbs twitching against each other in a competition to disgorge their frothy load before the other.");
		else MainScreen.text("Her sweaty nutsack glides over your " + player.skinFurScales() + " as you fuck, compressing and twitching against you as it begs to discharge its frothy load.");
		...
		if(player.cumQ() > 2000) MainScreen.text("  Of course, you outlast even the jizz-witch's magics, plugging her so full of semen that her belly rounds into a gravid orb while yours remains only slightly bloated.");
		else if(player.cumQ() > 1000) MainScreen.text("  Of course, you both manage to bulge each other's bellies with sexual discharge.");
		else MainScreen.text("  Of course, she cums far longer and harder than you, plugging your belly full of semen until it has a slightly fecund curve to it.");
		...
		//sand witch preg
		player.knockUp(PregnancyType.SAND_WITCH, PregnancyType.INCUBATION_SAND_WITCH, 90);
		player.orgasm();
		dynStats("lus", 5);
	}
	//DP Males
		...
		if(player.cumQ() < 250) MainScreen.text("pours out of your dual cum-slits, making a sloppy mess of both holes.");
		else if(player.cumQ() < 500) MainScreen.text("gushes out of your dual cum-slits, soaking both holes in and out with pearly white.");
		else if(player.cumQ() < 1000) MainScreen.text("gushes out with incredible force from your dual cum-slits, quickly plugging both holes so full of seed that the witch's belly slowly rises, cum-inflated.");
		else MainScreen.text("washes out of your dual cum-slits in a tidal wave, flooding the hungry holes with your virile seed.  You cum and cum, spunking up the witch's belly.  First it bulges slightly, but all too soon you have it rounded into a pregnant dome.");
		MainScreen.text("  The bouncing girl-cock above the woman's gushing pussy does a fine job of painting her belly and bust white.  It spurts rope after rope onto her chocolate-toned flesh, and soon she looks more like an icing-drizzled tart than a formidable foe.");
	...
	//Regular Dicking
		...
		//(silly:) MainScreen.text(", and I don't want it.  I think it's messy");
		MainScreen.text(".</i>\"");
		...
		if(player.stats.cor < 50) MainScreen.text("and a hint of compassion");
		else MainScreen.text("and finally deign to let her have some pleasure");
		MainScreen.text(".  You gradually increase the tempo of your lovemaking and let your hand lazily drift lower, grabbing the witch by her soft, supple thigh.  Her skin is warm and flushed (or as flushed as ebony skin can get), and the higher your fingers reach, the damper it gets.  Gently, you prod around her lips and your own maleness to find the hardness of clit.");
		...
		if(player.cumQ() < 700) MainScreen.text(", spurting with unceasing quantity until she's absolutely flooded with jizz and a little bulgy around the middle.");
		else MainScreen.text(", spooging up her cunny until it's completely flooded, her belly bloated into a rounded dome.");
		if(player.cumQ() >= 1500) MainScreen.text("  You keep going beyond that, but there's just no room, and spouts of semen spurt from the woman's poor, over-filled twat.");
		
		MainScreen.text("\n\nThe witch sighs and idly rubs her ");
		if(player.cumQ() >= 700) MainScreen.text("swollen ");
		MainScreen.text("tummy, smearing her own wasted goo into her skin as you withdraw and re-dress.");
		if(player.stats.cor < 33) MainScreen.text("  You hope she learned something from the experience, but knowing most people in this strange land, she'll go right back to her rotten ways.");
		else if(player.stats.cor < 66) MainScreen.text("  You wonder if she'll learn anything from this and shrug nonchalantly.  Who cares?");
		else MainScreen.text("  You smirk as you wonder if she'll learn anything from this.  You hope not - it's a fun lesson to teach.");
		player.orgasm();
	}
	if (inDungeon) {
		if (inCombat)
...
public ladyVictorySex():void {
	MainScreen.clearText();
	MainScreen.text("You disrobe, casting aside the garments with a feminine sigh.  As soon as you expose yourself, the witch's eyes twinkle happily.  She caresses her stiff tool and tweaks one of her nipples as she watches you.  \"<i>If you wanted to get fucked by little old me, all you had to do was bend over, honey.</i>\"");
	...
	if(player.wetness() >= 5) MainScreen.text("  It gushes out of you in a soaking river, utterly drenching the futa's stomach, hips, and butt.");
	if(player.lactationQ() >= 50) MainScreen.text("  At the same time, milk spouts from your bosom to wash over your hermaphrodite.");
...
public tentacleVictoryGangbangCumWitch():void {
	...
	if(player.tentacleCocks() >= 4) MainScreen.text(", splatting over your frotting cock-tip in wild abandon");
	...
	MainScreen.text("  You shudder and grab the ebony sorceress's tits in your hands, squeezing her bronzed orbs tightly as you release works through you.  Jets of cum well up to plug the witch's pussy and anus simultaneously.  The spit-soaked, cum-splattered vine between her tits quivers meaningfully, and then unloads, squirting a thick wave of jizz onto her face.");
	if(player.tentacleCocks() >= 4) MainScreen.text("  Of course, your fourth prick is not to be outdone, and blasts a gooey coating of cream across the witch's own pulsating prick, wreathing it in drizzles of syrupy white.");
	if(player.cumQ() >= 500) MainScreen.text("  Wave after wave of spunk washes into the tight holes and toned form of your foe, turning her into a complete mess.");
	if(player.cumQ() >= 1000) MainScreen.text("  A few more virile splurts see to her utterly inundated state, bloating her belly into a tightly-stretched dome.");
...
public savinMakesAwesomeFemdom():void {
	...
	//if Multiboob:
	if(player.bRows() > 1) {
		MainScreen.text("She caresses each of your breasts, cupping each in turn, running her thumb over each nipple");
		if(player.lactationQ() >= 200) MainScreen.text(" until milk streams down your chest, much to her delight");
		MainScreen.text(".");
	}
	else if(player.lactationQ() >= 200) MainScreen.text("Her fingers deftly work across your [nipples], caressing out a trickle of milk, which she laps up with a long, languid motion that electrifies your sun-kissed skin.");
	else MainScreen.text("She tweaks your [nipples] between her dexterous fingers, sending shivers of pleasure up your spine, but seems oddly discontent...  \"<i>No milk for me? We'll have to do something about that...</i>\"");
	MainScreen.text("  Slowly, she works her way down from your [chest] to your belly, peeling off your [armor] as she goes to reveal more and more of your " + player.skinFurScales() + ", never neglecting to run her hands all across you, stroking and kissing along every exposed inch until she comes to your groin.  Instinctively, you writhe in her grip as she tosses the last of you [armor] aside, leaving you wholly bare between her legs.");
...
public lionpaws(skipped:boolean = false):void {
	...
	if(skipped) {
		MainScreen.text("\"<i>Really, [name]?</i>\"  Sanura smirks and shakes her head.  \"<i>I probably shouldn't do this sort of thing just because you ask for it, but your fascination with my paws is too cute to ignore.  Go on, then, strip for me and I'll get to it.</i>\"  You oblige, happily tossing aside your armor.  [EachCock], already standing erect with anticipation, bounces freely before you.  The sphinx eyes ");
		if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("it");
		else MainScreen.text("them");
		MainScreen.text(" lustfully, momentarily entranced by the swaying motions of your member");
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(".");
	}
	//[player requests big fluffy paws on their dick after solving a riddle]
	//[if event occurrence == 0]
	else if(flags[FlagEnum.PAWJOBS] == 0) {
		MainScreen.text("Sanura raises her eyebrows at your demand, her expression somewhere between surprise and curiosity.  \"<i>Well, that's something of an odd request, but I suppose I'm obliged if it's your wish.</i>\"  Not bothering to give her a chance to consider your demand any further, you pull off your armor and cast it aside.  \"<i>Can't say I've even been asked for something like this before...  I apologize in advance for my inexperience, [name].</i>\"  The sphinx looks down at her paws, obviously confused as to why you would ask for this in particular.");
	}
	//[if event occurrence > 0]
	else {
		MainScreen.text("\"<i>Again?</i>\" Sanura asks, cocking her head to the side.  \"<i>They must feel better than I imagine... well then, get out of your clothes and I'll do as you ask.");
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 100) MainScreen.text("  It's not as if there's all that much else I can do for that monster of yours anyways.");
		MainScreen.text("</i>\" She raises a paw and examines it, as though trying to figure out just what it is about them that you enjoy so much.  You cough as a means of drawing her attention once you've stripped off the last bit of your armor, shaking her from her reverie.");
	}
		
	MainScreen.text("\n\nSanura asks that you kneel, and you eagerly comply.  ");
	//[if dick isn't already hard and PC knows what's coming]
	if(player.lust < 50 && flags[FlagEnum.PAWJOBS] == 0) {
		MainScreen.text("Even as you drop to your knees before Sanura, your cock");
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(" begin");
		if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("s");
		MainScreen.text(" to harden in anticipation while your mind wonders at just what this will feel like.  ");
	}
	MainScreen.text("The sphinx bends too, so that her head is level with your crotch.  With a little bounce, she throws her large paws around your hips and roughly pulls you closer.  The guardian wastes no time, licking along the shaft");
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
	MainScreen.text(" of your ");
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("various ");
	MainScreen.text("prick");
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
	MainScreen.text(" with a soft, skilled tongue.");
	//[if pc was soft before]
	if(player.lust < 50 && flags[FlagEnum.PAWJOBS] == 0) {
		MainScreen.text("  It isn't long before her oral ministrations have your cock");
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(" standing erect, throbbing with hunger for more sensation.  You wonder as to what exactly she has planned, but you think it's shaping up to be something good.");
	}
	//[continue]
	MainScreen.text("  Her long, expert strokes methodically coat your member");
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
	MainScreen.text(" in a glistening veil of saliva. After a while, she pulls her head away and examines her work.");
	
	MainScreen.text("\n\nApparently deeming ");
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("them");
	else MainScreen.text("it");
	MainScreen.text(" sufficiently lubricated, Sanura lifts a foreleg and carefully guides her paw towards your crotch.");
	//[if PC doesn't expect it]
	if(player.lust < 50 && flags[FlagEnum.PAWJOBS] == 0) MainScreen.text("  You raise your eyebrows in surprise, but you're not in much of a position to do anything about her foot's advance.");
	MainScreen.text("  She playfully taps at your cock, purring sensually as she sets it bouncing with a light bat.  After watching it for a moment, she hesitantly pushes forward so that her paw gingerly rests against ");
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("one of ");
	MainScreen.text("your shaft");
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
	...
	if(player.smallestCockArea() < 100) MainScreen.text("  As she pushes onto you just a bit more forcefully, your member slips between two of her digits.  She continues her slow, rhythmic movements with just a bit more grip, and you're made quite thankful that her claws are so retractable as she brushes against your belly.");
	...
	if(!skipped && flags[FlagEnum.PAWJOBS] == 0) MainScreen.text("It feels wonderful in a peculiar sort of way; though it's certainly not the treatment you expected to receive, you can definitely feel your arousal being pushed along and built up by her foot.  ");
	//[if PC has 2+ dicks]
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("Not content to leave your other cocks neglected, the sphinx leans in and starts to lick and suckle once again at the pricks not beneath her paw.  You shudder at the feeling of her tongue darting from shaft to shaft and teasing your heads, sharper sensations punctuating her foot's more constant pleasuring.  ");
	MainScreen.text("The fuzzy warmth clouding your mind dazes you, and you sway slightly in place, letting her rock you back and forth with her motions.");
	...
	if(player.cumQ() < 250) MainScreen.text("\n\nWith a few final pushes you find release, spraying ropes of thick cum along the length of her leg.");
		...
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
		MainScreen.text(" like a geyser, covering both you and Sanura in a flood of your semen.  The flow doesn't stop, cum churning and burning as it roils within you.  The sphinx pulls away in an effort to dodge the unexpected volume of man-milk, but to no avail: both halves of her body are painted white and your cum drips from her hair to her skin, fur, and wings. She tries to wipe her face at least clean, but manages only to smear more across her cheek.  With a sigh, she shakes what she can off her hands.");
	}
	MainScreen.text("  Exhausted from the orgasm, you let go of her, leaving the sphinx to milk out the last drops of cum with gentle pressure from her pad on her own.  Sanura stands back up and laughs.  \"<i>Look at the mess you made!</i>\"  She places her dirtied paw on your chest and pushes you back, easily making you fall on your [ass].  \"<i>Well, that was... interesting");
	if(flags[FlagEnum.PAWJOBS] > 0) MainScreen.text(" as always");
	MainScreen.text(", [name].  Now get your clothes back on, I've a temple to guard and a coat to clean.</i>\"  At that, she turns and pads away from you, lying down in front of the door and licking at her");
	//[if cum output is normal]
	if(player.cumQ() < 250) MainScreen.text(" leg.");
	else MainScreen.text(" stained-white fur in a futile attempt to get some of your cum off.");
	...
	if (skipped) {
		dungeonLoc = 0; //Replaces inDungeon = false;
		MainScreen.addButton(0, "Next", camp.returnToCampUseOneHour);
	}
	else {
		menu();
		MainScreen.addButton(0, "Enter", openZeDoorToParadize);
...
public riddleGameGo():void {
	MainScreen.clearText();
	flags[FlagEnum.RIDDLE_ONE] = 0;
	flags[FlagEnum.RIDDLE_TWO] = 0;
	flags[FlagEnum.RIDDLE_THREE] = 0;
	MainScreen.text("\"<i>Oh!  You'll play my game?  Marvelous!  Well then, let's begin...</i>\"");
	//[NEXT]
	menu();
	MainScreen.addButton(0,"Next",riddlePicker);
}

public riddlePicker():void {
	let choices:Array = [];
	if(flags[FlagEnum.RIDDLE_ONE] != riddleOne && flags[FlagEnum.RIDDLE_TWO] != riddleOne) choices[choices.length] = riddleOne;
	if(flags[FlagEnum.RIDDLE_ONE] != riddleTwo && flags[FlagEnum.RIDDLE_TWO] != riddleTwo) choices[choices.length] = riddleTwo;
	if(flags[FlagEnum.RIDDLE_ONE] != riddleThree && flags[FlagEnum.RIDDLE_TWO] != riddleThree) choices[choices.length] = riddleThree;
	if(flags[FlagEnum.RIDDLE_ONE] != riddleFour && flags[FlagEnum.RIDDLE_TWO] != riddleFour) choices[choices.length] = riddleFour;
	if(flags[FlagEnum.RIDDLE_ONE] != riddleFive && flags[FlagEnum.RIDDLE_TWO] != riddleFive) choices[choices.length] = riddleFive;
	if(flags[FlagEnum.RIDDLE_ONE] != riddleSix && flags[FlagEnum.RIDDLE_TWO] != riddleTwo) choices[choices.length] = riddleSix;
	if(flags[FlagEnum.RIDDLE_ONE] != riddleSeven && flags[FlagEnum.RIDDLE_TWO] != riddleSeven) choices[choices.length] = riddleSeven;
	if(flags[FlagEnum.RIDDLE_ONE] != riddleEight && flags[FlagEnum.RIDDLE_TWO] != riddleEight) choices[choices.length] = riddleEight;
	if(flags[FlagEnum.RIDDLE_ONE] != riddleNine && flags[FlagEnum.RIDDLE_TWO] != riddleNine) choices[choices.length] = riddleNine;
	if(flags[FlagEnum.RIDDLE_ONE] != riddleTen && flags[FlagEnum.RIDDLE_TWO] != riddleTen) choices[choices.length] = riddleTen;
	if(flags[FlagEnum.RIDDLE_ONE] != riddleEleven && flags[FlagEnum.RIDDLE_TWO] != riddleEleven) choices[choices.length] = riddleEleven;
	if(flags[FlagEnum.RIDDLE_ONE] != riddleTwelve && flags[FlagEnum.RIDDLE_TWO] != riddleTwelve) choices[choices.length] = riddleTwelve;
	if(flags[FlagEnum.RIDDLE_ONE] == 0) {
		flags[FlagEnum.RIDDLE_ONE] = choices[rand(choices.length)];
		flags[FlagEnum.RIDDLE_ONE]();
	}
	else if(flags[FlagEnum.RIDDLE_TWO] == 0) {
		flags[FlagEnum.RIDDLE_TWO] = choices[rand(choices.length)];
		flags[FlagEnum.RIDDLE_TWO]();
	}
	else {
		flags[FlagEnum.RIDDLE_THREE] = choices[rand(choices.length)];
		flags[FlagEnum.RIDDLE_THREE]();
	}
}
...
public riddleOne():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>Hmm, here's an old one, but a good one, I think.  One of my favorites, but stop me if you know the answer already: 'I walk on four legs in the morning, two in the afternoon, and three in the evening.  What am I?'</i>\"");
	menu();
	//[A Sphinx] (if PC int < 50 add this: [A Centaur]) [A Man] (if PC int < 35 add this: [Stilts]) [Fuck it, Attack]
	MainScreen.addButton(0,"A Sphinx",riddleOneSphinx);
	MainScreen.addButton(1,"A Centaur",answerWrong);
	MainScreen.addButton(2,"A Man",answerCorrect);
	if(player.stats.int < 35) MainScreen.addButton(3,"Stilts",answerWrong);
	MainScreen.addButton(4,"Uh, ATTACK!",fuckItAttack);
}

//Special Occurrence: Pick [A Sphinx]
public riddleOneSphinx():void {
	MainScreen.clearText();
	MainScreen.text("The sphinx narrows her eyes at you, crossing her arms over her chest.  \"<i>You don't say?  Come on, step it up, [name].  Sorry, but that's wrong.  Let's try again.</i>\"");
	menu();
	if(flags[FlagEnum.RIDDLE_ONE] == riddleOne) flags[FlagEnum.RIDDLE_ONE] = 0;
	else if(flags[FlagEnum.RIDDLE_TWO] == riddleOne) flags[FlagEnum.RIDDLE_TWO] = 0;
	else if(flags[FlagEnum.RIDDLE_THREE] == riddleOne) flags[FlagEnum.RIDDLE_THREE] = 0;
	MainScreen.addButton(0,"Next",riddlePicker);
}
...
public riddleTwo():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>Mmm, I've always been fond of this one.  Delightfully simple, really - don't overthink it, dear.  'I always run, yet never walk; I murmur often, yet never talk; and I've a bed, yet never sleep.  What am I?'</i>\"");
	//[A River] [A Whisper] (if PC int < 50 add this: [A Nail]) (if PC int < 35 add this: [A Bunny Girl]) [Fuck it, Attack]
	menu();
	MainScreen.addButton(0,"A River",answerCorrect);
	MainScreen.addButton(1,"A Whisper",answerWrong);
	if(player.stats.int < 50) MainScreen.addButton(2,"A Nail",answerWrong);
	if(player.stats.int < 35) MainScreen.addButton(3,"A Bunny-Girl",answerWrong);
	MainScreen.addButton(4,"Uh, ATTACK!",fuckItAttack);
}
...
public riddleThree():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>Ooh, this is a morbid one: 'Whoever makes me, sells me.  He who buys me, never uses me. And he who uses me, well, they'll never know it.  What am I?'</i>\"");
	menu();
	//[Poison] (if PC int < 50 add this: [A Condom]) (if PC int < 35 add this: [Arrows]) [A Coffin] [Fuck it, Attack]
	MainScreen.addButton(0,"Poison",answerWrong);
	if(player.stats.int < 50) MainScreen.addButton(1,"A Condom",answerWrong);
	if(player.stats.int < 35) MainScreen.addButton(2,"Arrows",answerWrong);
	MainScreen.addButton(3,"A Coffin",answerCorrect);
	MainScreen.addButton(4,"Uh, ATTACK!",fuckItAttack);
}
...
public riddleFour():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>Here's a classic for you: 'I'm always hungry, and must be fed always, lest I flicker away.  Yet I will always bite the hand that feeds me, if it touches me.  What am I?'</i>\"");
	menu();
	//(if PC int < 35 add this: [A Fairy]) [Fire] [A Dog] (if PC int < 50 add this: [Disease]) [Fuck it, Attack]
	if(player.stats.int < 35) MainScreen.addButton(0,"A Fairy",answerWrong);
	MainScreen.addButton(1,"Fire",answerCorrect);
	MainScreen.addButton(2,"A Dog",answerWrong);
	if(player.stats.int < 50) MainScreen.addButton(3,"Disease",answerWrong);
	MainScreen.addButton(4,"Uh, ATTACK!",fuckItAttack);
}
...
public riddleFive():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>A favorite of my dear Goblin friends: 'When young, I am sweet in the sun; in middle age, I make you gay; but when I'm old, I'm more valuable than gold.  What am I?'</i>\"");
	menu();
	//(if PC int < 50 add this: [Men]) (if PC int < 35 add this: [Women]) [Wine] [Cheese] [Fuck it, Attack]
	if(player.stats.int < 50) MainScreen.addButton(0,"Men",answerWrong);
	if(player.stats.int < 35) MainScreen.addButton(1,"Women",answerWrong);
	MainScreen.addButton(2,"Wine",answerCorrect);
	MainScreen.addButton(3,"Cheese",answerWrong);
	MainScreen.addButton(4,"Uh, ATTACK!",fuckItAttack);
}
//RIDDLE 6
public riddleSix():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>This riddle always makes me a bit sad, but... 'My life lasts but hours, and in service I'm devoured.  Thin, I am quick, fat I am slow, and wind is ever my foe.  What am I?'</i>\"");
	
	//(if PC int < 35 add this: [A Goblin]) [A Candle] (if PC int < 50 add this: [A Boat]) [An Arrow] [Fuck it, Attack]
	menu();
	if(player.stats.int < 35) MainScreen.addButton(0,"A Goblin",answerWrong);
	MainScreen.addButton(1,"A Candle",answerCorrect);
	if(player.stats.int < 50) MainScreen.addButton(2,"A Boat",answerWrong);
	MainScreen.addButton(3,"An Arrow",answerWrong);
	MainScreen.addButton(4,"Uh, ATTACK!",fuckItAttack);
}
...
public riddleSeven():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>In service to the Sand Witches, I can sympathize with this riddle's speaker: 'Each morning I appear to lie at your feet; all day I'll follow you no matter how fast you run, yet I'll nearly perish in the midday sun.  What am I?'</i>\"");
	
	//[A Shadow] [A Dog] (if PC int < 35 add this: [Water]) (if PC int < 50 add this: [The Breeze]) [Fuck it, Attack]
	menu();
	MainScreen.addButton(0,"A Shadow",answerCorrect);
	MainScreen.addButton(1,"A Dog",answerWrong);
	if(player.stats.int < 35) MainScreen.addButton(2,"Water",answerWrong);
	if(player.stats.int < 50) MainScreen.addButton(3,"The Breeze",answerWrong);
	MainScreen.addButton(4,"Uh, ATTACK!",fuckItAttack);
}
...
public riddleEight():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>Tsk, a sad tale this, fleeting as a melody on the breeze: 'You've heard me before, and will again, till fast I die - then you'll but summon me again.  What am I?'</i>\"");
	//(if PC int < 35 add this: [A Demon]) (if PC int < 50 add this: [Religion]) [An Idea] [An Echo] [Fuck it, Attack]
	menu();
	if(player.stats.int < 35) MainScreen.addButton(0,"A Demon",answerWrong);
	if(player.stats.int < 50) MainScreen.addButton(1,"Religion",answerWrong);
	MainScreen.addButton(2,"An Idea",answerWrong);
	MainScreen.addButton(3,"An Echo",answerCorrect);
	MainScreen.addButton(4,"Uh, ATTACK!",fuckItAttack);
}
...
public riddleNine():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>Like a cat comes this riddle of many lives: 'Three lives have I, gentle enough to sooth the skin, light enough to caress the sky, or hard enough to shatter stone.  What am I?'</i>\"");
	
	menu();
	//(if PC int < 35 add this: [A Dick]) [Water] [A Voice] (if PC int < 50 add this: [Faith]) [Fuck it, Attack]
	if(player.stats.int < 35) MainScreen.addButton(0,"A Dick",answerWrong);
	MainScreen.addButton(1,"Water",answerCorrect);
	MainScreen.addButton(2,"A Voice",answerWrong);
	if(player.stats.int < 50) MainScreen.addButton(3,"Faith",answerWrong);
	MainScreen.addButton(4,"Uh, ATTACK!",fuckItAttack);
}
...
public riddleTen():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>Devious, this: 'Whoever makes me, tells it not; he who takes me, knows it not; and he who knows me, takes me not.  What am I?'</i>\"");
	
	//[Disease] [Counterfeits] (if PC int < 35 add this: [Piss in a River]) (if PC int < 50 add this: [Tattered Scrolls]) [Fuck it, Attack]
	menu();
	MainScreen.addButton(0,"Disease",answerWrong);
	MainScreen.addButton(1,"Counterfeits",answerCorrect);
	if(player.stats.int < 35) MainScreen.addButton(2,"Piss In A River",answerWrong);
	if(player.stats.int < 50) MainScreen.addButton(3,"Tattered Scrolls",answerWrong);
	MainScreen.addButton(4,"Uh, ATTACK!",fuckItAttack);
}
...
public riddleEleven():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>The lovely Naga that roams this desert knows my speaker well: 'A box without hinges, lock or key, yet I've golden treasure within me.  What am I?'</i>\"");
	
	//(if PC int < 35 add this: [A Treasure Chest]) (if PC int < 50 add this: [Pure Honey]) [Eggs] [Booze Bottles] [Fuck it, Attack]
	menu();
	if(player.stats.int < 35) MainScreen.addButton(0,"Teasure Chest",answerWrong);
	if(player.stats.int < 50) MainScreen.addButton(1,"Pure Honey",answerWrong);
	MainScreen.addButton(2,"Eggs",answerCorrect);
	MainScreen.addButton(3,"Booze Bottles",answerWrong);
	MainScreen.addButton(4,"Uh, ATTACK!",fuckItAttack);
}
...
public riddleTwelve():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>Here we've one that speaks to the stars and the earth at once: 'One by one we fall from the heavens, down into the depths of the past; our world is ever upturned, so that yet some time will last.  What are we?'</i>\"");
	
	//(if PC int < 50 add this: [Fallen Stars]) (if PC int < 35 add this: [Angels]) [Sand] [Rain] [Fuck it, Attack]
	menu();
	if(player.stats.int < 50)addButton(0,"Fallen Stars",answerWrong);
	if(player.stats.int < 35) MainScreen.addButton(1,"Angels",answerWrong);
	MainScreen.addButton(2,"Sand",answerCorrect);
	MainScreen.addButton(3,"Rain",answerWrong);
	MainScreen.addButton(4,"Uh, ATTACK!",fuckItAttack);
}
...
public answerCorrect():void {
	MainScreen.clearText();
	if(flags[FlagEnum.RIDDLE_THREE] == 0) {
		MainScreen.text("The sphinx-girl sighs, \"<i>That's... correct.  Not bad, I suppose.  Well, we're not done yet... I've still got some tricks up my sleeves.  Er, so to speak.</i>\"");
		menu();
		MainScreen.addButton(0,"Next",riddlePicker);
	}
	else {
		...
		if(flags[FlagEnum.BEATEN_SANURA_COUNT] == 0) {
			MainScreen.text("\"<i>W-what!?</i>\"  the sphinx gasps as you correctly answer the last of her riddles.   \"<i>I-I don't... but how?  No one's ever had the wits to answer all three riddles!</i>\"");
		...
		//BEAT THE SPHINX AT HER OWN GAME (REPEAT)
			...
			MainScreen.text("\"<i>Tsk, I lose again!</i>\" Sanura pouts, crossing her arms.  \"<i>I suppose you'll just have to give me a victory ravishing, won't you?  Go easy, though... you've already damaged my pride so....</i>\"");
		}
		flags[FlagEnum.BEATEN_SANURA_COUNT]++;
		MainScreen.text("\n\nWhat do you do?");
		//(Display Options: [Fuck Her] [Door] [Leave])
		menu();
		MainScreen.addButton(0, "North Door", openZeDoorToParadize);
		MainScreen.addButton(1, "Fuck Her", fuckDatSphinx);
		MainScreen.addButton(4, "Leave", leaveBoobsDungeon);
	}
}
...
public answerWrong():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>That's... wrong, I'm afraid,</i>\" she says, a proud smile playing across her lips.  \"<i>Wrong, wrong, wrong.  Well, perhaps </i>I<i> was wrong about </i>you<i> after all...  Here I thought you'd be some fun.  Still, though, at least your body will serve me well, I think.  Yes, you'll do nicely.  Go on then, love, strip out of your [armor] for me.  A deal's a deal, after all...</i>\"");
	//(Display Options: [Submit] [Fuck it, Attack])
	//(Submit goes to appropriate loss scene)
	menu();
	MainScreen.addButton(0, "Submit", sphinxSubmissionOptions);
	MainScreen.addButton(1, "Uh, ATTACK!", fuckItAttack);
...
public getBlown(submit:boolean = false):void {
	...
	if(player.lowerBody.balls > 0) MainScreen.text("balls empty");
	else MainScreen.text("body empties");
	MainScreen.text(" out onto her eager face.  As you feel yourself winding down from the magically-induced orgasm, however, a strange blue sigil appears in the air, encircling ");
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("each of your ");
	else MainScreen.text("your ");
	MainScreen.text("dripping cock");
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
	MainScreen.text(".  \"<i>You do want to go again, yes?  I think I've got enough magic to keep that nice [cock biggest] hard for a little fun with my new hands....</i>\"");
	...
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
	MainScreen.text(" - the salty liquid flows freely onto the desert sands from Sanura's open mouth as your ");
	if(player.lowerBody.balls > 0) MainScreen.text("[balls] churn");
	else MainScreen.text("body churns");
	MainScreen.text(" out a seemingly endless supply of seed.  Your breath comes ragged as more and more spills out of you, your newly-potent manhood making you feel lightheaded as it expels an endless tide of cum.  Slowly, gently, Sanura leans in, wrapping her lips around the tip of your [cock biggest], lapping up the river of semen that pours into her mouth.");
	
	//If Multi-cock = 3
	if(player.lowerBody.cockSpot.count() >= 3) {
		MainScreen.text("\n\nAs Sanura slurps up the tip of your [cock], her ethereal hands grasp ");
		if(player.lowerBody.cockSpot.count() == 2) MainScreen.text("your ");
		else MainScreen.text("two of your ");
		MainScreen.text("other " + num2Text(player.lowerBody.cockSpot.count()-1) + " members, wrapping tightly around their meaty lengths.  You groan throatily as tingling digits slowly work their way up your shafts, making your heart flutter wildly with the alien sensation.");
		//AND If Multicock = 5
		if(player.lowerBody.cockSpot.count() >= 5) MainScreen.text("  You barely have time to adjust to the eldritch hands jerking off your extra cocks when Sanura's own come into play, wading into the jumble of wangs growing from your crotch to grab another pair of pricks.  With five cocks encircled in hands, magic, and mouth, Sanura slowly begins to synchronize her movements, gently stroking your many cocks.");
	}
	...
	if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("s");
	MainScreen.text(" geysering into the air, raining back down in a pool around you and the now cum-covered sphinx.");
...
public forceDildos():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) player.cuntChange(10,true,true,false);
...
public fuckDatLionPussah():void {
	...
	//(Large dicks)
		...
		MainScreen.text(" with the little tuft of fur.  \"<i>That bad boy will be just perfect.</i>\"");
	}	
	...
	if(player.upperBody.chest.countNipples() == 2) MainScreen.text("both");
	else MainScreen.text("each");
	MainScreen.text(" of your nipples, which stiffen in response, just like your cock.  You can't help but think there's some sort of magic at work here.  Her tail passes between your thighs, gracefully tickling your ");
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("damp cunt, which suddenly tightens up and practically gushes with slick juices.");
	else MainScreen.text("asshole, which reflexively tightens, sending a shiver up your spine.");
	...
	if(player.hasKnot(player.biggestCockIndex())) MainScreen.text(", your knot slipping into place with a moist <b>pop</b>, ensuring you'll be locked together for the foreseeable future");
	MainScreen.text(".  Jet after jet of cum erupts from your [cock], painting the sphinx's inner walls white.");
	...
	if(player.hasKnot(player.biggestCockIndex())) MainScreen.text("which proves somewhat uncomfortable, given your knot is still lodged firmly within her, ");
	MainScreen.text("panting wildly.  It takes you several minutes to recover, the sphinx even longer, ");
	if(player.hasKnot(player.biggestCockIndex())) MainScreen.text("and even longer still for your knot to deflate enough to slip out of her, ");
	MainScreen.text("but you finally gather the strength to haul yourself up onto your [feet].");
...
public sphinxSubmissionOptions():void {
	MainScreen.clearText();
	flags[FlagEnum.TIMES_SUBMITTED_TO_SANURA]++;
	MainScreen.text("Having lost the Riddle Game, you do as the sphinx-girl commands: you pull off your [armor], leaving yourself bare under the desert sun. \"<i>Now then, my dull little pet, what shall I do with you...</i>\"");
	menu();
	//SUBMISSION:
	MainScreen.addButton(0,"Get Pegged",fenPutsHisShittyFetishInYoSphinx);
	//Reqs Penis
	if(player.lowerBody.cockSpot.hasCock()) {
		MainScreen.addButton(1,"Get Blown",getBlown,true);
		MainScreen.addButton(2,"Pawjob",lionpaws,true);
	}
}
...
public fenPutsHisShittyFetishInYoSphinx():void {
	MainScreen.clearText();
	MainScreen.text("Starting to disrobe, ");
	if(player.lowerBody.cockSpot.hasCock()) {
		MainScreen.text("you flop your " + player.multiCockDescriptLight() + " free in the dry desert heat, allowing ");
		if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("it");
		else MainScreen.text("them");
		MainScreen.text(" to sway back and forth");
	}
	else if(player.lowerBody.vaginaSpot.hasVagina()) {
		MainScreen.text("you expose your [vagina] to the hot desert air, allowing it to moisten");
	}
	else MainScreen.text("you expose your [butt], allowing it to redden");
	MainScreen.text(" as the omnipresent heat and seductive sphinx have their way with your heightened libido.  Sanura snaps her palm up in front of your face as your [armor] hits the ground, landing with a heavy thump.");
	...
	if(player.lowerBody.cockSpot.count() > 0) MainScreen.text("  Pre-cum oozes from [eachCock] in steady strings that dangle onto the dirt, painting a swirled pattern beneath you as your prostate is pushed hard enough to squeeze trickles of your salty 'milk' from you.");
	...
	if(player.lowerBody.cockSpot.count() > 0) MainScreen.text("  Whenever her flare slides past your prostate, the wide, blunt head squeezes it hard enough to express a huge strand of pre.");
	MainScreen.text("  After a few such thrusts, her bouncing, fake balls rise up in their faux, fleshy sack.  Her swollen dick-tip engorges more than you thought possible inside you, wide enough for you to touch through your belly and marvel at the size.  Quivering from head to four lion-like toes, Sanura cums.");
	...
	if(player.lowerBody.cockSpot.count() > 0) MainScreen.text("  When she crosses the lump that is your well used prostate, you lose control and shudder with a nice, long cum of your own, your arms giving out and dumping you face-down on the ground while you spurt milky whiteness onto your chest, chin, and then the ground.");
	else MainScreen.text("  When she's almost out, you give in and cum, quivering around her magical breeding tool while it finishes inseminating your guts.");
	MainScreen.text("  The head is so swollen that she actually struggles to pop it back out of your [asshole], bouncing her hips back a few times with little success");
	if(player.lowerBody.cockSpot.count() > 0) MainScreen.text(" and dragging you through your fresh, milky puddle");
	MainScreen.text(".  Eventually, she gives a mighty, sphincter-stretching lurch and ejects from your [butt], painting it white on the way out.");
...
public sandWitchMotherFriendlyMenu():void {
	...
	menu();
	if(flags[FlagEnum.SAND_WITCH_LEAVE_ME_ALONE] == 0) MainScreen.addButton(0,"StopAttacking",leaveAloneSendLackeysToggle);
	else MainScreen.addButton(0,"StartAttacking",leaveAloneSendLackeysToggle);
	MainScreen.addButton(1,"SandMothers",askHowSandMothersAreChosen);
	MainScreen.addButton(2,"Cum Witches",discussCumWitches);
	MainScreen.addButton(3,"CovenStatus",currentStateOfInterwebs);
	MainScreen.addButton(4,"History",sandWitchHistory);
	if(flags[FlagEnum.SAND_WITCH_LOOT_TAKEN] < 5) {
		MainScreen.addButton(5,"Get LaBova",getLaBova);
		MainScreen.addButton(6,"Get Lactaid",getLactaidFromWitches);
	}
	if((flags[FlagEnum.ESSY_MET_IN_DUNGEON] > 0 && flags[FlagEnum.TOLD_MOTHER_TO_RELEASE_ESSY] == 0) || (flags[FlagEnum.MET_MILK_SLAVE] > 0 && flags[FlagEnum.MILK_NAME] is Number))
		MainScreen.addButton(7,"Free Slaves",slavesDiscussion);
	if(player.lust >= 33) MainScreen.addButton(8,"Sex",sexWithFriendlySandMother);
	
	MainScreen.addButton(9, "Leave", playerMenu);
}

public slavesDiscussion():void {
	menu();
	if(flags[FlagEnum.ESSY_MET_IN_DUNGEON] > 0 && flags[FlagEnum.TOLD_MOTHER_TO_RELEASE_ESSY] == 0) MainScreen.addButton(0,"Essrayle",forest.essrayle.askMotherToReleaseEssy);
	if(flags[FlagEnum.MET_MILK_SLAVE] > 0 && flags[FlagEnum.MILK_NAME] is Number) MainScreen.addButton(1,"Milk-Slave",freeSlaves);
}
...
private freeSlaves():void {
	MainScreen.clearText();
	MainScreen.text("Thinking back to the poor, mind-broken tittymonster of a bath girl you met here in the witches' coven, you tell the Sand Mother it's about time to free her and any other slaves she's got hidden away.");
	...
	//[Yes] [No] [Gimme her]
	menu();
	MainScreen.addButton(0,"Yes",yesDemandMilkRelease);
	MainScreen.addButton(1,"No",noDemandMilkRelease);
	MainScreen.addButton(2,"Gimme Her",gimmeDatDeliciousMilkWaifuINeedMoreWaifusCauseTheTwoCowslutsWerentEnoughForMyInsatiableLacticLustandDesire);
}

//Yes (No Change)
private yesDemandMilkRelease():void {
	MainScreen.clearText();
	MainScreen.text("You tell the Sand Mother that everyone deserves freedom, even if they're addle-minded, or useful.  She can't just <i>keep people</i> because she feels like they're better off in her care.");
...
private gimmeDatDeliciousMilkWaifuINeedMoreWaifusCauseTheTwoCowslutsWerentEnoughForMyInsatiableLacticLustandDesire():void {
	MainScreen.clearText();
	MainScreen.text("You have a better idea: give the girl to you.  You can care for and protect her in your camp, but also give her at least as much freedom as is safe in these dire times.");
	...
	if(player.stats.gems < 2000) {
		MainScreen.text("\n\n<b>You haven't got that much.</b>");
	}
	menu();
	//[2 Expensive][Buy Her]
	MainScreen.addButton(0,"2 Expensive",TwoExpensive4Me);
	if(player.stats.gems >= 2000) MainScreen.addButton(1,"Buy Her",BuyHer);
}

private TwoExpensive4Me():void {
	MainScreen.clearText();
	MainScreen.text("You explain that you can't afford that much.");
...
private BuyHer():void {
	MainScreen.clearText();
	player.stats.gems -= 2000;
	MainScreen.text("You hand over two thousand of your hard-earned gems.");
	...
	menu();
	MainScreen.addButton(0, "Next", milkWaifu.arriveWithLacticWaifuAtCamp);
}

public sexWithFriendlySandMother():void {
	menu();
	//friendly cunt fuck:
	if(player.lowerBody.vaginaSpot.hasVagina() && player.lust >= 33) MainScreen.addButton(0,"GetMilkFill",lesboMilkFilling);
	//dick fucking, must fit!
	if(player.lowerBody.cockSpot.hasCock() && player.lust >= 33) {
		if(player.cockThatFits(monster.vaginalCapacity()) >= 0) MainScreen.addButton(1,"Fuck Her",friendlySandMotherFuck);
	}
	MainScreen.addButton(4,"Back",sandWitchMotherFriendlyMenu);
}
...
public sandMotherPOMenu():void {
	...
	if(flags[FlagEnum.SAND_WITCH_LEAVE_ME_ALONE] == 0) MainScreen.addButton(0,"StopAttacking",unfriendlyWitchToggle);
	else MainScreen.addButton(0,"StartAttacking",unfriendlyWitchToggle);
	if(player.lust >= 33) {
		if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.addButton(1,"Scissor",scissorAndDrink);
		if(player.tentacleCocks() >= 5) MainScreen.addButton(2,"TentacleBang",tentacleGangBang);
		if(player.lowerBody.cockSpot.hasCock()) {
			if(player.cockThatFits(monster.vaginalCapacity()) >= 0) MainScreen.addButton(3,"Fuck Her",fuckTheSandMothersCunt);
		}
	}
	if(flags[FlagEnum.SAND_WITCH_LOOT_TAKEN] < 5) {
		MainScreen.addButton(5,"Get LaBova",takeLaBovaOrLactaid, false);
		MainScreen.addButton(6,"Get Lactaid",takeLaBovaOrLactaid);
	}
	if(flags[FlagEnum.ESSY_MET_IN_DUNGEON] > 0 && flags[FlagEnum.TOLD_MOTHER_TO_RELEASE_ESSY] == 0) MainScreen.addButton(7,"Essrayle",forest.essrayle.askMotherToReleaseEssy);
	MainScreen.addButton(9, "Leave", playerMenu);
}
...
public sandMotherStuffGOA():void {
	...
	if(flags[FlagEnum.SAND_WITCHES_FRIENDLY] > 0) {
		MainScreen.text("The Sand Mother slightly inclines her head in your direction as you enter her throne room.  She is every bit as imposing as you recall, or at least as imposing as a woman with four watermelon-sized jugs on her chest can be.  \"<i>You have returned to us, far traveler.  Is there something you desire from the coven of the sands?  Simply ask for what you need, and we shall aid you against the demon Queen.</i>\"  The lactation-obsessed arch-enchantress idly pushes a whitish-blonde curl out of her eyes as she awaits your reply.");
		//Display Friendly Choices
		sandWitchMotherFriendlyMenu();
		return;
	}
	//*Encounter Repeat: Cowed
	else if(flags[FlagEnum.SAND_MOTHER_DEFEATED] > 0) {
		MainScreen.text("The Sand Mother glares down at you from atop her throne, but does not dare lift a finger against you, not after her previous defeat.  She hisses, \"<i>You've already plundered my coven!  What more do you want from us?</i>\"  even as her chest heaves in a panic.  Four mountainous peaks jut from the witch's silken robe... you have a few ideas, and the Queen Witch seems to have some of her own.");
		//Resisting Options
		...
		if(flags[FlagEnum.SAND_WITCH_LOOT_TAKEN] >= 5) MainScreen.text(", though right now it sits empty.  You'll need to wait for them to restock it");
		else MainScreen.text(", and noone could stop you from taking them");
		MainScreen.text(".");
		sandMotherPOMenu();
	}
	...
	//The Sand Mother gazes at you uncertainly as you return, her defeat at your hands still fresh in her memory.  The bleach-blond matriarch looks to have recovered from the fight, but she lamely queries, \"<i>Well, you've proven yourself mightier than my magic.  All that remains is a choice.  Do you show your true colors, pawn of demons, or do you tread the path of virtue?</i>\"  Sure, she puts on a brave face, but the nervous tics at the taut corners of her mouth reveal her worry. 
		...
		//[Explain] [Fight]
		menu();
		MainScreen.addButton(0,"Explain",explainYourSelfToZeSandBitch);
		MainScreen.addButton(1,"Fight",fightTheSandWitch);
	}
}
...
public explainYourSelfToZeSandBitch():void {
	...
	if(player.stats.cor > player.stats.int || monk >= 5 || player.statusAffects.has("Exgartuan") || amilyScene.amilyCorrupt() || flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00283] > 0 || flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0 || flags[FlagEnum.NIAMH_STATUS] > 0) {
		MainScreen.text("  \"<i>There is some truth to your tale, [name], but I am a Sand Mother.  We are schooled in the art of sussing out the corrupt or unclean.  If we could not detect disguised demons and demonic agents, we would not flourish as we do now, and this great desert would not be on the cusp of resurrection.</i>\"");
	...
	//(No new PG. Corruption is less than intellect and no major asshole moves made:) 
		...
		MainScreen.text("  \"<i>Your tale has the ring of truth to it, [name].  I am the Sand Mother of this coven, and it seems we are not enemies after all.  If you like I can instruct my daughters to let you pass through the sands unhindered.</i>\"  She spreads her arms graciously and asks, \"<i>Was there anything you would like to discuss?  It has been an age since I've had a new arrival to converse with.</i>\"\n\n(+200 XP)");
		//(Award XP)
		player.XP += 200;
		...
		statScreenRefresh();
		//(Set friendly)
		flags[FlagEnum.SAND_WITCHES_FRIENDLY] = 1;
...
public fightTheSandWitch():void {
	MainScreen.clearText();
	MainScreen.text("You brandish your [weapon] and advance threateningly.  The Sand Witch Queen slowly rises, pulling a scepter from beneath her robes with a resolute look in her eyes.  \"<i>You shall regret this action for a time, until we've corrected your diseased way of thinking.  Now, taste the power of a Sand Mother!</i>\"");
	startCombat(new SandMother(),true);
}
...
public earthShield():void {
	...
	monster.statusAffects.add(new StatusAffect("Earthshield",0,0,0,0)));
	combatRoundOver();
}
//*GigaFire
public gigaFire():void {
	...
	MainScreen.text("Releasing the scepter, the Sand Mother spreads her hands, each glowing with eldritch, white flames.  Her heels slowly float up off the ground as she closes her eyes in concentration.  You can sense the power and heat rolling off her in waves, and if you don't do something to disrupt her, you'll likely be burned to a crisp.");
	if(player.stats.int > 40) MainScreen.text("  She's not even looking at you and seems steeled against lusty interruptions.  Perhaps you can hit her hard enough to shatter her concentration.");
	monster.statusAffects.add(new StatusAffect("Gigafire",0,0,0,0)));
	combatRoundOver();
}
...
public gigaFire2():void {
	...
	//Not interrupted:
	if(monster.statusAffects.get("Gigafire").value1 < 10) {
		MainScreen.text("The Sand Mother grabs her scepter in both hands, combining the flames that wreath them into an immense, blinding conflagration.  She points at you, and the fire washes out in a wave like a serpent, twisting at you as you try to avoid it, doubling back on itself whenever it misses.  It's unavoidable!  You're enveloped in the consuming fire!");
		damage *= 3;
	}
	//Interrupted:
	else {
		MainScreen.text("Thumbling back to the ground from your damaging hit, the Sand Mother grabs at her head, one flame going out.  She recovers in time to grab the staff and fling it towards you, but it's at a fraction of the strength she intended.  Still, it burns all the same.");
	}
	damage = takeDamage(damage);
	MainScreen.text(" (" + damage + ")");
	monster.statusAffects.remove("Gigafire");
	combatRoundOver();
}
...
public sandMotherTelekinesis():void {
	MainScreen.text("She narrows her eyes at you, and an immense, magical pressure reaches out, taking hold of you.  It spins you in the air before slamming you into the walls!");
	
	let sizeMod:number = 100;
	let thickMod:number = player.thickness / 100 + 0.5;
	sizeMod += player.tallness * thickMod;
	if(sizeMod < 140) MainScreen.text("  You immediately wish you weren't so small, as you're sure she couldn't have flung a heavier champion nearly as easily.");
	else if(sizeMod >= 200) MainScreen.text("  You're glad for your size, as she couldn't seem to accelerate you into the stone as fast as she'd like.  Sometimes eating plenty pays off.");
	...
	let multiplier:number = sizeMod / 170;
...
public eatALightningBolt():void {
	...
	MainScreen.text("Holding her staff back, she lifts her free hand with her fingers outstretched in a fan.  Sparks dance between her slender digits, coiling around them like snakes.  In a flash, they rush to her palm and erupt in a lightning bolt, striking you instantly and unavoidably!");
...
public sandWitchMotherAI():void {
	...
	if(monster.statusAffects.has("Gigafire")) {
		gigaFire2();
		return;
	}
	let choices:Array = [];
...
public defeatTheSandMother():void {
	MainScreen.clearText();
	flags[FlagEnum.SAND_MOTHER_DEFEATED] = 1;
	MainScreen.text("Panting ");
	if(player.HP < 1) MainScreen.text("in exhaustion");
	else MainScreen.text("with lust");
	MainScreen.text(", the Sand Mother sinks back into her throne with sweat and milk staining her silky robes.  She groans softly, the milk-stains slowly spreading as you consider your options.  This woman dared to stand against you.  What do you do?");
	//[Fuck Her Cunt] [ScissorAnDrink] [Talk Her Down]
	menu();
	MainScreen.addButton(0,"TalkHerDown",talkDownTheMother);
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.addButton(1,"Scissor",scissorAndDrink);
	if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(monster.vaginalCapacity()) >= 0) MainScreen.addButton(2,"Fuck Her",fuckTheSandMothersCunt);
	if(player.tentacleCocks() >= 5) MainScreen.addButton(3,"TentacleFuck",tentacleGangBang);
}
public loseToTheSandMother():void {
	MainScreen.clearText();
	//DUDALICIOUS
	if(player.lowerBody.cockSpot.hasCock()) loseToSandMotherBadEnd();
	//Chicks/neuters
...
public tentacleGangBang():void {
	...
	if(player.tentacleCocks() >= 6) {
		MainScreen.text("\n\nYou still have ");
		if(player.tentacleCocks() >= 7) MainScreen.text("untended serpentine shafts to please");
		else MainScreen.text("an untended serpentine shaft to please");
		MainScreen.text(", but all the holes are full!  You take the ");
		if(player.tentacleCocks() == 6) MainScreen.text("leftover one and thread it");
		else MainScreen.text("leftover ones and thread them");
		MainScreen.text(" through the dick-dominated enchantrix's fingers.  By now, her body has completely and utterly betrayed her, and she grabs hold like a cock-hungry slut, immediately stroking up and down, jacking it like a pro.  A muffled sigh of contentment shivers up your throat-fucking cock, alerting you to your depraved foe's prick-addled state.  You begin to pump your quartette of buried cocks into their respective holes with renewed vigor.  The titty-fucking vine pistons through its curvy path along with them, sending the witch's tits jiggling and squirting wildly.  The only thing");
		if(player.tentacleCocks() == 6) MainScreen.text(" held still is the one getting a hand job, and you only hold it still");
		else MainScreen.text("s held still are the two getting hand jobs, and you only hold them still");
		MainScreen.text(" so that part of you can relax and enjoy your captive Queen's enthusiastic wanking.");
	}
	//(10+)
	...
	if(player.tentacleCocks() >= 6) MainScreen.text("  Bubbling seed soaks her frantically jerking hands.");
...
public fuckTheSandMothersCunt():void {
	...
	if(silly()) MainScreen.text("  Dat booty be poppin', yo!");
	MainScreen.text("  Another slap, this time from the other side, sends waves of motion back across, and you cannot help but give the supple cheeks a tight squeeze immediately after.");
	...
	if(player.tallness >= 90) MainScreen.text("at the nape of her neck,");
	else MainScreen.text("on her stomach,");
	...
	if(y >= 0) MainScreen.text(", grabbing a second cock as well, intent on penetrating both of her sopping entrances");
	MainScreen.text(".  The moment her labia caress your ");
	if(y >= 0) MainScreen.text("cockheads");
	else MainScreen.text(player.cockHead(x));
	MainScreen.text(", she freezes as she realizes what she's doing, and she tries to pull away.");
	...
	if(y >= 0) MainScreen.text("tools");
	else MainScreen.text(cockDescript(x));
	MainScreen.text(".  She moans out loud, throwing her bleached tresses around wildly in pleasure while inch after inch of cock snakes into her passage");
	if(y >= 0) MainScreen.text("s");
	...
	if(y >= 0) MainScreen.text("s");
	MainScreen.text(".  Semen floods out of you from the force of your orgasm, creaming the juicy cunt");
	if(y >= 0) MainScreen.text("s");
	MainScreen.text(" as ");
	if(y < 0) MainScreen.text("it clamps down");
	else MainScreen.text("they clamp down");
	MainScreen.text(", squirting femlube and leftover spunk across your [feet].");
	if(player.cumQ() >= 500) {
		MainScreen.text("  Again and again, you flood the poor spellcaster's twat");
		if(y >= 0) MainScreen.text("s");
		MainScreen.text(", shooting so much that it bubbles out in thick globs to fall wetly on the ground.");
		if(player.cumQ() >= 2000) MainScreen.text("  They fall faster and faster as you cum, the witch's belly bloating, rounding out to a ridiculous degree.  Her legs are drenched with the leaking spooge.");
	}
	MainScreen.text("  Once you've pumped every ounce of your virile fluid into the waiting receptacle");
	if(y >= 0) MainScreen.text("s");
	MainScreen.text(", you pull, admiring the salty, mark you've left on the once proud woman.");
...
public scissorAndDrink():void {
	...
	if(player.tallness < 85) MainScreen.text(" larger");
	else if(player.tallness > 105) MainScreen.text(" smaller");
	MainScreen.text(" woman, moist, slick bodies entwined as you both grope and grind.");
	...
	if(player.wetness() >= 5) MainScreen.text("explodes out in a cunt-shattering spray");
	else if(player.wetness() >= 4) MainScreen.text("gushes out in a heavy spray");
	else if(player.wetness() >= 3) MainScreen.text("pours out in a thick, wet river");
	else if(player.wetness() >= 2) MainScreen.text("drools out in a steady trickles");
	else MainScreen.text("drips out during the cunt-shattering convulsions");
	MainScreen.text(".  Bulging obscenely, your cheeks barely contain the flood of milk, and you swallow it without thinking, acting entirely on instinct.");
...
public loseToSandMotherBadEnd():void {
	MainScreen.clearText();
	MainScreen.text("Stumbling back into a wall, you try to hold your ");
	if(player.HP < 1) MainScreen.text("battered body aloft, but your wounds are too great, robbing you of strength.");
	else MainScreen.text("rebellious body aloft, but your lust is too great, turning your body into a quivering wreck.");
	MainScreen.text("  You ");
	if(player.HP < 1) MainScreen.text("spitefully watch the Sand Mother approach, only able to admire her sculpted form as she towers over your prone form.");
	else MainScreen.text("hungrily watch the Sand Mother approach, only able to touch yourself as you gaze up at her sculpted form");
	MainScreen.text(".  She sniffs in disdain as she looks down at you.  \"<i>Disappointing.  Lethice sends a weakling like you after us, and the sisters do not even slow you.  Their atonement shall be severe.</i>\"  Her eyes, blue as the desert sky, consider you again, and she mutters, \"<i>First, there is the matter of you, isn't there?</i>\"");
	...
	if(player.stats.cor < 66) MainScreen.text("aren't corrupt");
	else MainScreen.text("don't serve the demons");
	MainScreen.text(", looking up at her pleadingly ");
	if(player.lowerBody.cockSpot.count() == 1) MainScreen.text("while she gleefully pumps you, smearing your leaky juices all over your shaft.");
	else MainScreen.text("while she gleefully moves to your second cock, smearing it with your first dick's pre-cum, messing your " + player.multiCockDescriptLight() + " with each penis's leaky drippings.");
	...
	if(player.cumQ() >= 500) MainScreen.text("  Wave after wave of jism washes across you, thoroughly drenching you with your spooge, the alabaster spunk soaking you from crown to waist with sticky slickness.");
	if(player.cumQ() >= 3000) MainScreen.text("  A puddle pools around you, deepening nicely as your boundless virility does its work, inching up your body until you feel as if you'll float away in your own pearly jism.");
	...
	if(flags[FlagEnum.CUM_WITCH_DEFEATED] > 0) MainScreen.text("The Cum Witch is here, and she seems to have recovered from her defeat!");
	else MainScreen.text("This must be the Cum Witch!");
	...
	if(player.lowerBody.balls > 0) MainScreen.text("testes");
	else MainScreen.text("prostate");
	MainScreen.text(" emptying orgasm, you shudder and try to pull away, but there's nowhere to go.  Gentle hands place themselves to either side of your head, immobilizing you, and the lusty hermaphrodite begins to chant, punctuating her arcane words with slow pumps of her body across your cum-slicked middle");
	if(player.lowerBody.balls > 0) {
		MainScreen.text(", two sets of slippery balls bouncing and sliding across each other");
		if(silly()) MainScreen.text(".  Great, now it's gay");
	}
	MainScreen.text(".");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("innards");
	else MainScreen.text("folds");
	MainScreen.text(" thickens slightly, and the heaving balls below jiggle with liquid weight, swelling larger against you.  They grow heavier and heavier, until they slosh with audible, barely contained need, each as large as one of your butt-cheeks and three times as soft.");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("bowels");
	else MainScreen.text("womb");
	MainScreen.text(", pumping into you with enough force to audible splash and churn about, gently thickening your middle.  A crystalline lattice-work erects from the mental syrup, climbing higher into a brand new frame of reference, filled with the thoughts and experiences of others - no, of your past.  New memories fill in the gaps while your gut rises, forming a small bump that would be easily mistaken for pregnancy.");
...
public loseToSandMother():void {
	MainScreen.clearText();
	MainScreen.text("The world grows dizzy as your ");
	if(player.HP < 1) MainScreen.text("heavily punished and aching body is filled with too much agony to continue.");
	else MainScreen.text("hot and aroused body fills with unbearable urges that finally distract you from fighting any further.");
	MainScreen.text("  Instead, you fall backwards against the wall ");
	if(player.HP < 1) MainScreen.text("and grit your teeth in anger as the Sand Mother approaches.");
	else MainScreen.text("panting deeply, and your blood races as the Sand Mother approaches.");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) {
		MainScreen.text("Remarkably, a divot appears on your groin, slowly dividing into two puffy lips with a moist crease between.  The fresh mons of your new pussy glistens with immediate arousal as the Sand Mother's magic molds your flesh with a simple spell.  ");
		player.createVagina();
		player.lowerBody.vaginaSpot.list[0].clitLength = 0.25;
	}
	MainScreen.text("Your [vagina] is eyed momentarily as it ");
	if(player.wetness() <= 2) MainScreen.text("grows moist.");
	else if(player.wetness() <= 4) MainScreen.text("drips with arousal.");
	else MainScreen.text("drools obscenely.");
	MainScreen.text("  \"<i>We can certainly improve upon this.</i>\"  You attempt to voice opposition but she quickly silences you.  \"<i>No! Understand here and now, you live by our mercy.  Your body belongs to the sand.  Your old life is over.  From now on, your failed life will serve a greater purpose.</i>\"");
	
	MainScreen.text("\n\nYou try to resist but ");
	if(player.HP < 1) MainScreen.text("the pain of your abused body");
	else MainScreen.text("lustful urges fill your senses and");
	MainScreen.text(" hinders all of your attempts to fight back.  You try to argue that you aren't a servant of the demons, that you were fighting against them for your own people just as she is, but every time you open your mouth she forces it closed again with her magic.  \"<i>I am not interested in the lies of a demonic servant.  It is time we dealt with you once and for all.</i>\"");
	...
	if(player.bRows() == 1) {
		MainScreen.text("  Plus an extra set of these to ensure you produce as much milk as the rest of your soon-to-be sisters.");
		player.createBreastRow();
	}
	//if player has four breasts
	else if(player.bRows() == 2) MainScreen.text("  Thankfully you're already endowed with enough breasts to begin immediate production of milk.");
	//if player has six breasts
	else MainScreen.text("  Hmmm. We've never had a sister blessed with more than four breasts. I wonder if perhaps we can use your unusual anatomy to our advantage.");
	MainScreen.text("  She squeezes a [nipple] and a yelp of pleasure escapes you.  \"<i>I see you like that idea. Excellent.</i>\"");
	//if breasts are smaller than a normal sand witch's
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 3) MainScreen.text("  \"<i>Of course we'll have to grow them a bit to suit our needs.</i>\"");
	//if breasts are larger than a normal sand witch's
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 14) MainScreen.text("  \"<i>Mmmm, and with such massive ones I'm sure we can expect you to produce more than most of our sisters could.</i>\"");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 5) MainScreen.text("jiggle");
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 10) MainScreen.text("bounce");
	else MainScreen.text("quake");
	MainScreen.text(" with every twitch of her fingers inside you.");
	if(player.lactationQ() >= 50) MainScreen.text("  Streams of milk are already starting to pour down the swells of your tits.");
	MainScreen.text("  Her fingers start pressing deeper, playing upon your erogenous zones like an instrument, strumming your body's sense of pleasure to new heights.  She plays your cunt until you cum again and again around palm and gush your juice down her arm.  Everything below your waist abandons your control and submits itself to her administrations.  You cum and all you can do is grope your own tits as you ride the waves of pleasure that saturate your flesh.");
	...
	if(flags[FlagEnum.CUM_WITCH_DEFEATED] == 1) MainScreen.text("  It is obviously the Cum Witch you encountered, and she has clearly regained her strength!");
	else MainScreen.text("This must be the Cum Witch the Sand Mother sent for!");
	...
	if(player.bRows() <= 2) MainScreen.text(", admiring the extra set of breasts you already possess");
	else MainScreen.text(", admiring how you've managed to grow " + num2Text(player.bRows()) + " rows of breasts instead of just two on your panting torso");
	MainScreen.text(".");
	...
	player.orgasm();
	menu();
	MainScreen.addButton(0,"Next",sandMotherBadEndsLadiesEpilogue);
}

//Epilogue:
public sandMotherBadEndsLadiesEpilogue():void {
	MainScreen.clearText();
	MainScreen.text("The Champion awoke to a body changed. " + player.mf("His","Her") + " body had been transformed in to that of a Sand Witch's.");
	//if player had six breasts
	if(player.bRows() > 2) MainScreen.text("  Although unlike the other Sand Witches, this one sported extra breasts which she used to her utmost advantage.");
	//if player had gigantic breasts
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 22) MainScreen.text("  She was unrivaled in her ability to produce milk thanks to the natural enormity of her breasts that her other sisters couldn't come close to.  They were so large that they often impeded her movement, which she admittedly didn't mind so much.");
	...
	if(player.perks.has("BroodMother")) MainScreen.text("  She'd immediately shown a unique propensity for rapid pregnancies, birthing new sisters at twice the rate of any other Sand Witch.  Her surprisingly fertile wombs helped give an extra edge to the Sand Mother's plans.");
	
	//if player had broodmother and gigantic breasts
	if(player.perks.has("BroodMother") && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 22) MainScreen.text("  As a result, the champion who could no longer remember her true past became little more than a giant pregnant belly surrounded by " + num2Text(player.bRows()) + " breasts so massive that the other sand witches often used her tits as comfortable cushions to sit upon, their thighs tightly gripping each giant nipple against their aching cunts for pleasure.");
...
public friendlySandMotherFuck():void {
	...
	let y: number = player.cockThatFits2(monster.vaginalCapacity());
	//First Time:
	if(flags[FlagEnum.TIMES_FRIENDLY_FUCKED_SAND_MOTHER] == 0) {
		MainScreen.text("Casting your gaze around to ensure privacy, you lower your brows conspiringly and inquire into the Sand Mother's love life.");
	...
	if(!player.isTaur()) {
		MainScreen.text("holding [oneCock] in ");
		if(y >= 0) MainScreen.text("one");
		else MainScreen.text("your");
		MainScreen.text(" hand");
		if(y >= 0) MainScreen.text(" and another in the other");
		MainScreen.text(", the meaty cock-flesh wobbling, veins pulsing just under the surface as you nose ");
		if(y < 0) MainScreen.text("it");
		else MainScreen.text("them");
		MainScreen.text(" up against the twin, wet pussies.");
	}
	else {
		MainScreen.text("dick");
		if(y >= 0) MainScreen.text("s");
		MainScreen.text(" bobbing under your equine belly, the meaty cock-flesh wobbling with veins pulsing just under the surface as ");
		if(y < 0) MainScreen.text("it noses");
		else MainScreen.text("they nose");
		MainScreen.text(" up against the twin, wet pussies.");
	}
	if(player.tallness >= 90) MainScreen.text("  You have to kneel down to make it work, tall as you are.  ");
	else if(player.tallness <= 48) MainScreen.text("  You can barely reach her cunt standing, short as you are.  ");
	MainScreen.text("Her lube-glossed gates spread around your " + player.cockHead(x));
	if(y >= 0) MainScreen.text(" and other cock-tip");
	MainScreen.text(", the hot moisture gobbling up your inches, clinging hot and wet to the oversensitive, pulsating dick");
	if(y >= 0) MainScreen.text("s");
	MainScreen.text(".  The large woman sighs with relief as you scratch her unspoken, hidden itch.  Your [hips] push forward with such fervor that you doubt you could stop if you wanted, pressing hard on her sensitive, innermost folds with ");
	if(y >= 0) MainScreen.text("both your cocks");
	else MainScreen.text("your " + cockDescript(x));
	MainScreen.text(".");
			   
	//HOARSES:
	if(player.isTaur()) {
		MainScreen.text("\n\nWhinnying in delight, you rest your forelegs on ");
		if(player.tallness < 78) MainScreen.text("her shoulders");
		else MainScreen.text("the top of her throne");
		MainScreen.text(" and thrust, pumping yourself hard into her with animalistic delight.  The Sand Mother shudders beneath your bestial bulk as her hands begin to tug at her nipples through her soaked, not-so-concealing robes.  She whimpers and begins to kiss at your human abdomen, tonguing your belly button as she's mounted on your bitch-breaking erection");
		if(y >= 0) MainScreen.text("s");
		MainScreen.text(".  Grabbing a lock of platinum in your fingers, you keep a firm hold on her while you let your lower half's inhuman ardor take control of your body.");
	}
	//NOTAHOARSE:
	else {
		MainScreen.text("\n\nGroaning in delight, you rest your hands on her ");
		if(player.tallness < 60) MainScreen.text("bottom tits");
		else if(player.tallness < 84) MainScreen.text("top tits");
		else MainScreen.text("shoulders");
		MainScreen.text(" and thrust, powerfully pumping your manhood through her lube-gilded channel");
		if(y >= 0) MainScreen.text("s");
		MainScreen.text(".  The Sand Mother shudders before you, hands tugging her nipples through her soaked, not-so-concealing robes.  She's whimpering needily, and ");
		if(player.tallness < 72) MainScreen.text("you kiss her to silence her, tongue pumping to the same timing that drives your hips.");
		else MainScreen.text("you suckle one of the heavy, milk-laden tits to enhance her pleasure");
		MainScreen.text(".  Her fingers roam through your " + hairDescript() + " as she allows her passion to overwhelm her.  Splattering noisily, her juices are dripping and squirting around your hard-thrusting dong");
		if(y >= 0) MainScreen.text("s");
		MainScreen.text(", replaced by fresh secretions as fast as you squeeze them out of her.");
	}
	
	//Both
	MainScreen.text("\n\nYou groan as the suckling heat around your rod");
	if(y >= 0) MainScreen.text("s");
	MainScreen.text(" lurches into convulsions, too hot, tight, and sensuous for you to endure.  For a moment, you try your damnedest to hold out, to ride the wave of pleasure for another few minutes before you release into the agonizingly sweet puss");
	if(y < 0) MainScreen.text("y");
	else MainScreen.text("ies");
	MainScreen.text(".");
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		MainScreen.text("  Your own [vagina] quivers weakly, insides grinding against insides, squeezing every drop of juice from your ");
		if(player.wetness() >= 4) MainScreen.text("squirting ");
		MainScreen.text("passage.");
	}
	if(player.lactationQ() >= 200) {
		MainScreen.text("  Escaping from your [nipples], milk ");
		if(player.lactationQ() < 200) MainScreen.text("leaks");
		else if(player.lactationQ() < 1000) MainScreen.text("drips");
		else MainScreen.text("erupts");
		MainScreen.text(", splattering the throne with a fresh, lactic offering.  If you weren't mindlessly jamming yourself into her cunt, you'd wonder if she approves of your offering, but every neuron is busy sending your body to a mind-blowing climax.");
	}
	MainScreen.text("  You grunt as the heat inside your clenching [balls] radiates through your groin and towards [eachCock].");
	//Newline
	MainScreen.text("\n\n");
	//More than 2 dongs or one doesn't fit
	if(player.lowerBody.cockSpot.count() > 2 || (player.lowerBody.cockSpot.count() == 2 && y < 0)) {
		MainScreen.text("White love-batter fires in thick ropes onto her robes from your unbound penis");
		if(player.lowerBody.cockSpot.count() > 3 || (player.lowerBody.cockSpot.count() == 3 && y < 0)) MainScreen.text("es");
		MainScreen.text(".  One after another, the syrupy strands mix in with the dripping milk to make an alabaster drapery of sex-juice.  ");
	}
	//light ejaculate
	if(player.cumQ() < 250) {
		MainScreen.text("You spend your passion in numerous spurts, firing straight into the deepest depths of the Sand Mother's pussy.  Her slick walls squeeze you repeatedly, pulling you further inside, caressing you to encourage your orgasm to gift her with greater quantities of jism. As you continue to spurt, you realize that her hungry cunt");
		if(y >= 0) MainScreen.text("s are");
		else MainScreen.text(" is");
		MainScreen.text(" successful in some capacity, because you squirt until it's almost painful, finishing with ");
		if(y < 0) MainScreen.text("a fat white drop that's squeezed from your tip");
		else MainScreen.text("fat white drops that are squeezed from your tips");
		MainScreen.text(".");
	}
	//medium ejaculate
	...
	//high ejaculate
		...
		if(y >= 0) MainScreen.text("s go");
		else MainScreen.text(" goes");
		MainScreen.text(" straight into her womb");
		if(y >= 0) MainScreen.text("s");
		MainScreen.text(", rounding her belly to pregnant proportions.  She moans as she's filled beyond capacity with cum, only getting relief when the pressure in her packed womb");
		if(y >= 0) MainScreen.text("s grow");
		else MainScreen.text(" grows");
		MainScreen.text(" so high that jets of spooge spray out around you.  That doesn't stop you - you keep forcing more thick ejaculate into her until she's a nonsensical, babbling mess and you're totally spent.");
	}
	//END
	...
	flags[FlagEnum.TIMES_FRIENDLY_FUCKED_SAND_MOTHER]++;
	player.orgasm();
	doNext(playerMenu);
}
...
public lesboMilkFilling():void {
	...
	flags[FlagEnum.TIMES_FRIENDLY_FUCKED_SAND_MOTHER]++;
	//First Time:
	if(flags[FlagEnum.TIMES_FRIENDLY_FUCKED_SAND_MOTHER] == 1) {
		MainScreen.text("Casting your gaze around to ensure privacy, you lower your brows conspiringly and inquire into the Sand Mother's love life.");
		MainScreen.text("\n\nThe Sand Mother blushes slightly, her hand idly fanning at her reddened cheek before pushing a strand of white-blonde hair out of her face.  She bites her lower lip nervously before admitting, \"<i>Not that it is any business of yours, Champion, but no, my duties are quite demanding.  I am much too busy to saddle myself with offspring, and the milkers provide all the... stimulation that I require.</i>\"  Her large nipples are slowly stiffening, lifting her silken robes away from her breasts as they erect needily.  You swear you can hear the liquid bounty sloshing around inside her immense orbs as she shifts position, crossing her legs, and then you smell the unmistakable aroma of an aroused woman.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 10) MainScreen.text(", [fullChest] swaying enticingly as you strike your seductive pose");
	MainScreen.text(".   Freeing your body from its bindings is almost as exciting as seducing the queen of the lactic sorceresses, and you ");
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) MainScreen.text("cup your mammaries");
	else MainScreen.text("run your hands down your chest");
	MainScreen.text(" as you stride closer.");
	...
	//NOT A HORSE
		...
		if(player.tallness > 72 || player.isTaur()) MainScreen.text(", though judging by the glow around her hands, she used some magic to help");
		MainScreen.text(".  Sliding enticingly across your nethers, her pebbly, soaked tip drags perfectly up against your [clit], just waiting to be jammed home and begin inseminating you with milk.");
	}
	//Both
	...
	MainScreen.text("\n\nYou ");
	if(player.isTaur()) MainScreen.text("squeeze your own [nipples]");
	else MainScreen.text("grab hold of her head");
	MainScreen.text(" as you begin to pull away and thrust back, languidly fucking yourself on her gushing spout while it floods your pouty, dripping pussy and eager womb with lactic moisture.");
	if(!player.isTaur()) MainScreen.text("  Somehow, she's able to pull her hand away from your hip while still holding you aloft.  A glowing, arm-shaped silhouette remains, even as she diverts her attention elsewhere.");
	MainScreen.text("  'Schluck-schluck-schluck.'  You can hear her pushing her fingers deep into her neglected womanhoods, and judging by the doubled, wet sounds of penetration, she must be jamming two fingers in her doubled cunts.");
	...
	if(player.pregnancyIncubation == 0) MainScreen.text("Your poor womb is flooding with the stuff.  It's getting so full that you're starting to feel like you should be labelled 'cream-filled', and a tiny bump has begun to swell on your midsection in response.");
	else MainScreen.text("Your poor pregnant womb is battered with the stuff, but since you're already full, jets of hot cream are spraying out from your [vagina] around nipple-shaped milk-injectors.");
	//CuntChange 6
	player.cuntChange(6,true,true,false);
	MainScreen.text("\n\n\"<i>Yesssss,</i>\" you find yourself hissing, ");
	if(player.isTaur()) MainScreen.text("wishing you could rub your swollen belly");
	else MainScreen.text("rubbing your swollen belly");
	...
	if(player.pregnancyIncubation == 0) MainScreen.text("You can feel your midsection going round, wobbling wildly.  POP!  Your belly-button has turned into an outie, accompanied by a wobbling, comforting weight inside you.  Four months... five, no - six... you give up on counting, but with every second that passes, you're getting bigger and bigger, visually matching any of the pregnant women you saw growing up.");
	else MainScreen.text("You can feel it fountaining out of you, backwashing out to splatter the witch with wetness, her skin going glossy under the tide of white.  If you weren't already pregnant, you'd probably be ballooning up with her lactic cargo, a rounded, milk-filled cream-balloon.");
...
public sandWitchHistory():void {
	MainScreen.clearText();
	MainScreen.text("You inquire about the history of the sand witches, and how they came to be.  The Sand Mother gives you a benign smile and offers, \"<i>With pleasure.  It is so rare that I get to recount the story to one not of our order.</i>\"  She utters a few spidery words that seem to ring in your ears, and a luminous, floating illusion appears before her, all floating shapeless colors.  As she begins to talk, the blobs shift into the actors in her tale, allowing you to view the story as it happens.");
...
public currentStateOfInterwebs():void {
	MainScreen.clearText();
	MainScreen.text("You inquire as to the current state of the sand witches, and this coven in particular. The Queen Witch's brow creases in thought as she considers your request, tapping her chin and gazing into the intricate, swirling lights above.");
	...
	if(flags[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] == 0) MainScreen.text("All that remains is to ready those still maturing and wait on our allies to finish growing their numbers.  We shall be carried into the Demon Queen's fortress on the wings of phoenixes, to fight as glorious a battle as has ever been fought!");
	else MainScreen.text("All that remains is for us to find new aerial allies or a method of easy, sustained flight.  The phoenixes were smashed apart by fiery demons, their lair raided and destroyed by an interloper.  If only things had been different... Calais was so close.");
	MainScreen.text("</i>\"");
	...
	if(flags[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] > 0) MainScreen.text("  Of course, this is all contingent on us finding an ally that can breach the mountaintop citadel's walls.");
	MainScreen.text("  Was there something else you needed to ask about?</i>\"");
	//[friendly options]
	sandWitchMotherFriendlyMenu();
}
...
public discussCumWitches():void {
	MainScreen.clearText();
	MainScreen.text("You inquire as to why they have cum witches.");
	...
	if(player.stats.cor + player.stats.lib > 100) MainScreen.text(", though potentially fun");
	MainScreen.text(".  With a little bit of a lecherous smile, you ask if she ever tried out for it.");
	...
	menu();
	if(flags[FlagEnum.MORE_CUM_WITCHES] == 1) {
		MainScreen.text("\n\nYou already convinced her to add more cum witches to her harem so that they might experience a little freedom.");
		if(flags[FlagEnum.CUM_WITCHES_FIGHTABLE] == 0) {
			MainScreen.text("  If you asked her to send them out hunting for recruits, you could potentially fight and fuck them in the deserts sands in the future.");
			MainScreen.addButton(0,"Send Out",sendOutCumWitch);
		}
		else {
			MainScreen.text("Many of them are prowling the desert sands even now, hunting for recruits.  You could wind up having to fight them if you go out.  It's possible she might keep them away from you, if you request it.");
			MainScreen.addButton(0,"KeepThemIn",keepCumWitchesIn);
		}
	}
	else {
		...
		MainScreen.addButton(0,"Bolster Them",moreCumWitchesPlease);
	}
	//[Send Them Out] [Back]
	//[Keep Them In] [Back]
...
public sendOutCumWitch():void {
	MainScreen.clearText();
	MainScreen.text("You request she put the cum witches' boundless virility and desire to work for the coven, letting some of them out to venture the sands in search of new recruits.  (<b>From now on, there's a chance you'll encounter cum witches in the desert.</b>)");
	flags[FlagEnum.CUM_WITCHES_FIGHTABLE] = 1;
	sendOutOrKeepInEnding();
}
//Keep Them In:
public keepCumWitchesIn():void {
	MainScreen.clearText();
	MainScreen.text("You request she put a hold on sending out the cum witches.  Their aggressive, sexual recruitment methods are getting in your way, and for now, it would be best if they were out of your way.");
	flags[FlagEnum.CUM_WITCHES_FIGHTABLE] = 0;
	sendOutOrKeepInEnding();
}
...
public moreCumWitchesPlease():void {
	...
	if(player.perks.has("BimboBrains") || player.perks.has("FutaFaculties")) {
		MainScreen.text("Wouldn't it be better if there were like, lots of cum witches, with yummy cocks that you could suck?");
		MainScreen.text("\n\n\"<i>No, it wouldn't,</i>\" the Sand Mother retorts, ending the conversation.");
		doNext(playerMenu);
	}
	MainScreen.text("You ask her if it wouldn't be more humane to simply create a few more cum witches, allowing them to split their duties and actually have time to serve in other ways, either as normal sisters or perhaps helping with the recruitment.");
	...
	flags[FlagEnum.MORE_CUM_WITCHES] = 1;
	sandWitchMotherFriendlyMenu();
...
public askHowSandMothersAreChosen():void {
	MainScreen.clearText();
	MainScreen.text("You ask the Sand Mother how her people choose who will be a Sand Mother of a new coven.");
	...
	if(player.race() == "human") MainScreen.text("'re a human");
	else MainScreen.text(" were a human");
	MainScreen.text(".  Our people are notorious for forming gangs and cliques, groups focused around a leader's charisma.  It's only natural that has continued in Mareth.  It would not surprise me to learn that even the demons have their own factions and political malcontents, though I imagine they deal with them far more harshly than we.</i>\"");
...
public leaveAloneSendLackeysToggle():void {
	MainScreen.clearText();
	//Leave Alone
	if(flags[FlagEnum.SAND_WITCH_LEAVE_ME_ALONE] == 0) {
		MainScreen.text("You request that the scouts in the desert stop bothering you with their barbed offers of magic.  Fighting them is not something you want to have to do.  It's a waste of effort on both sides you that feel should come to an end.");
		...
		flags[FlagEnum.SAND_WITCH_LEAVE_ME_ALONE] = 1;
	}
	//Send Lackeys
	else {
		MainScreen.text("You mention that you wouldn't mind the witches trying to recruit you as they used to.");
		...
		flags[FlagEnum.SAND_WITCH_LEAVE_ME_ALONE] = 0;
	}
	doNext(playerMenu);
}
//*Lactaid
...
public getLactaidFromWitches():void {
	MainScreen.clearText();
	MainScreen.text("You ");
	if(player.stats.cor < 33) MainScreen.text("politely request");
	else if(player.stats.cor < 66) MainScreen.text("request");
	else MainScreen.text("none-too-politely request");
	MainScreen.text(" a dose of Lactaid from her coven.");
	...
	flags[FlagEnum.SAND_WITCH_LOOT_TAKEN]++;
	//Receive one lactaid
...
public getLaBova():void {
	MainScreen.clearText();
	MainScreen.text("You ask for some La Bova");
	if(player.stats.cor < 33) MainScreen.text(" with a blush");
	else if(player.stats.cor < 66) MainScreen.text(" a little uncertainly");
	else MainScreen.text(" with a knowing smile");
	MainScreen.text(".");
	...
	MainScreen.text("\n\nYou shake your head");
	if(player.stats.cor > 66) MainScreen.text(", though the idea does seem to nestle into your imagination quite alluringly");
	MainScreen.text(".");
...
public unfriendlyWitchToggle():void {
	...
	if(flags[FlagEnum.SAND_WITCH_LEAVE_ME_ALONE]  == 0) {
		...
		if(player.stats.cor < 50) MainScreen.text("  You probably wouldn't do something like that, at least not without provocation!/  You could see how she would think that.");
		else MainScreen.text("  As if that would stop you.");
		MainScreen.text("  Slapping her on the shoulder, you congratulate her on seeing good sense.  Her eyes narrow dangerously as she twists away from your touch.");
...
public pullLever():void {
	MainScreen.clearText();
	MainScreen.text("There is a loud rumbling from the direction of the cavernous commons...");
	flags[FlagEnum.SANDWITCH_THRONE_UNLOCKED] = 1;
	doNext(playerMenu);
}
...
public takeFertilePills():void {
	MainScreen.clearText();
	if(player.findStatusAffect(StatusAffects.Contraceptives) < 0) MainScreen.text("You aren't under the effects of a contraceptive, so taking a pink pill would do nothing.");
	//{Contraceptives}
	else {
		MainScreen.text("It doesn't take you long to figure out that the pink pill should cancel the effects of your contraceptives.  You pop it into your mouth and swallow, feeling a tingle near your crotch after a moment.  You should be capable of bearing children again");
		if(!player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text(", should you ever grow a vagina");
		MainScreen.text(".");
		player.statusAffects.remove("Contraceptives");
	}
	doNext(playerMenu);
}
...
public takeBarrenPills():void {
	...
	if(player.statusAffects.has("Contraceptives")) MainScreen.text("You're already under the effects of contraceptives.  Taking one of the brown pills wouldn't do anything.");
	//{TAKE DAT SHIT YO}
	else {
		MainScreen.text("You figure one of these brown pills should render you barren, and you pop it into your mouth, not wanting to be impregnated.");
		if(player.pregnancyIncubation > 0) MainScreen.text("  Of course, you're already pregnant, and this doesn't seem to be doing anything about THAT.");
		MainScreen.text("  You do feel an emptiness in your midsection, reassuring you that the pill did its job.");
		if(!player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("  Now if you ever re-grow a vagina, you should be fine.");
		player.statusAffects.add(new StatusAffect("Contraceptives",0,0,0,0)));
	}
	doNext(playerMenu);
}
...
public friendlyCumWitchBlessing():void {
	MainScreen.clearText();
	MainScreen.text("You ask the Cum Witch if she could use her magic to gift you with some kind of blessing since she has such an affinity for sexualized magics and fertility.  ");
	if(player.stats.cor < 33) MainScreen.text("Blushing, ");
	else if(player.stats.cor < 66) MainScreen.text("Nervously, ");
	else MainScreen.text("Boldly, ");
	MainScreen.text("you remove your armor, figuring whatever blessing she's going to give is going to be at least a little bit sexual.");
	...
	menu();
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(0,"Virility",cumWitchBlessed, true);
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.addButton(1,"Fertility",cumWitchBlessed,false);
	MainScreen.addButton(4,"Nevermind",changeMindAboutBlessings);
}
//*Nevermind ✓Kirbu
public changeMindAboutBlessings():void {
	MainScreen.clearText();
	MainScreen.text("Thinking better of it, you grab your [armor] and get dressed, telling the jizz-obsessed enchantress that you don't need her gifts for now.");
...
public cumWitchBlessed(virility:boolean = true):void {
	...
	if(player.tongueType > TONUGE.HUMAN) MainScreen.text("  Your inhuman tongue slides further and further out, following the witch's bulging, pre-cum filled urethra towards her balls where it can curl around them, embracing them with slick, slippery saliva.  You can taste her unique flavor on her skin, and it makes you shudder with arousal.");
	...
	if(virility) MainScreen.text("[eachCock] drip and drool your own seed onto the floor.  The heat inside you is so hot that you can't keep your ardor penned up, and it's slowly boiling out of your [balls] in a pleasure-filled orgasm that provides everything but relief.  You're sure that touching yourself would give you a proper orgasm, but you keep your hands where they belong - on her balls.");
	else {
		MainScreen.text("your [vagina] ");
		if(player.wetness() < 3) MainScreen.text("drip");
		else if(player.wetness() < 4) MainScreen.text("drool");
		else if(player.wetness() < 5) MainScreen.text("leak");
		else MainScreen.text("spray");
		MainScreen.text(" your juicy secretions onto the ground below.  Your lust has risen to such unbearable levels that you're clenching and dripping in orgasmic delight without any release.  You're sure just a touch on your clit would have you cumming your brains out, but you keep your hands on her balls.");
	}
	...
	if(!virility) MainScreen.text("womb");
	else MainScreen.text("[balls]");
	MainScreen.text(" that reminds you that you got exactly you asked for, even if she made you like it a little bit more than you would have liked...");
	MainScreen.text("<b>");
	if(virility) {
	...
	flags[FlagEnum.BEEN_BLESSED_BY_CUM_WITCH] = 1;
	player.orgasm();
...
public fuckOneSandWitch():void {
	...
	if(player.tentacleCocks() == 0) MainScreen.text("You grab the tanned shoulders of the closest pregnant witch and flip her over on to her back, trapping her beneath the weight of her belly and breasts while you grab both of her ankles.  Her dirty blonde, sweat-soaked hair lies in waves around her head on the floor as she pants heavily in sexual desperation.");
	//{else if player has a tentacle cock}
	else MainScreen.text("Your tentacle cock lashes outwards, extending as it goes, and wraps around the ankle of the closest girl. It flips her over on to her back, trapping her own torso beneath the weight of her heavy pregnancy and her own four massive leaking breasts.");
	//{if player has two tentacle cocks}
	if(player.tentacleCocks() > 1) MainScreen.text("  Another tentacle cock reaches out for the girl's other ankle, and together your cocks spread her tanned legs wide, revealing her large double pussy bloated from her pregnancy.");
	else MainScreen.text("  You reach forward with a hand to grab her other ankle and with your cock and arm you spread her tanned legs wide to reveal her large double pussy, bloated from her pregnancy.");
	...
	if(player.tentacleCocks() >= 2 && player.lowerBody.cockSpot.count() >= 3 && player.lowerBody.cockSpot.list[x].cockType != CockType.TENTACLE) {
	...
	else if(player.tentacleCocks() >= 2 && player.lowerBody.cockSpot.list[x].cockType != CockType.TENTACLE && player.cockThatFits2(50) >= 0 && player.lowerBody.cockSpot.count() >= 4) {
	...
	MainScreen.text("  The witch's moans and grunts of pleasure arouse the other witches in the room as they watch with the utmost jealously.  Some grab their nipples while others try to finger their pussies; whining and getting off on their envious voyeurism as they watch you fuck their sister.");
	...
	if(multi) MainScreen.text("s constantly pound");
	else MainScreen.text(" constantly pounds");
	MainScreen.text(" into her nethers. Drool escapes her lips and drips down her cheek even as her eyes roll back in her head. Talk about an easy lay.");

	//{if player has large but not extremely massive breasts} 
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 4 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 15) {
		...
		if(player.pregnancyIncubation > 0 && player.pregnancyIncubation < 200) MainScreen.text("  Each time it does so your own breasts slap against your swollen midsection.");
		//{if player has four or more large breasts and is not pregnant}
		else if(player.bRows() >= 2) MainScreen.text("  Your lower pair of breasts slap against the pregnant girl's belly with each thrust.");
	}
	//else if player has extremely huge breasts
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 15) {
		...
		if(player.bRows() == 1) MainScreen.text("  Your two heavy milk sacs sway and at times you must rest them on top of the poor witch's belly to keep from prematurely exhausting yourself.");
		else if(player.bRows() >= 2) MainScreen.text("  Resting on top of each other and on top of the witch's legs, your giant globes of tit flesh quake each time you hips move.");
		MainScreen.text("  The witches around the room can't help but gawk at how much more endowed your mammaries are than even their own, and how the wrong forwards or backwards movement could result in either you or the witch you're fucking to be suddenly smothered under the weight of your ridiculously engorged tits.");
	}
	MainScreen.text("\n\nIt isn't long before the sensations of naked wet flesh sliding along your ");
	if(multi) MainScreen.text("shafts");
	else MainScreen.text("shaft");
	MainScreen.text(" forces you to reach your own orgasm.");
	if(player.lowerBody.balls > 0) MainScreen.text("  Your [balls] tense with the anticipation of releasing the fruit of your loins into her waiting womb.");
	MainScreen.text("  With both hands you reach up and grab the fat nipples of the witch's lower breasts, tugging on them like reins as you try to thrust deeper.  \"<i>Ung! Ah!</i>\" The witch bites her lower lip in pleasure and pain as you roughly handle her leaking teats.");
	//{if player has huge or massive breasts} 
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 15) MainScreen.text("  The weight of your own tits bears heavily against your arms as you try to keep the motion of your hips constant while pulling on your lover's milk knobs.");
	//{if player's tits can lactate}
	else MainScreen.text("  Each of your [nipples] begin spraying milk over the witch as your body begins its crescendo.");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.wetness() >= 4) MainScreen.text("  Gobs of pussy juice splash against yours and the witch's legs as your orgasm forces your cunt to squirt over and over again.");
		if(player.lowerBody.balls > 0) MainScreen.text("  Your [sack] is soaking wet by the time your pussy finishes cumming.");
	}
...
public fuckTwoPregWitches():void {
	MainScreen.clearText();
	let x: number = player.cockThatFits(50);
	let y: number = player.cockThatFits2(50);
	MainScreen.text("These hormonally overwrought witches are each little more than five huge fleshy orbs of tits and bellies, desperately crawling towards you on hands and knees as their stares fixate on your " + cockDescript(y) + ".  ");
	//{if player has two long tentacle cocks} 
	if(player.tentacleCocks() >= 2) MainScreen.text("Almost of their own accord your tentacle cocks lash out, wrapping around the arms of the two closest girls, lifting them to their knees only to roll them backwards and on to their sides with their backs to each other.  ");
	else MainScreen.text("It takes little effort to grab the closest two witches and pull them up only to push them down again and on to their sides with their backs pressed against each other.  ");
	MainScreen.text("The two witches lay before you with their heaving breasts and bellies jutting out to either side while their asses are mashed against each other.  You grab an ankle from each of them and raise their legs up to expose the dual pairs of pussies that they possess.");
	
	//{for non-serpent lower body}
	if(!player.isNaga()) MainScreen.text("You straddle ");
	else MainScreen.text("You entwine your serpentine tail around ");
	MainScreen.text("their two legs that remain on the floor while allowing their raised legs to ");
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 15) MainScreen.text("slide between your cleavage");
	//{for small or non-breasted characters}
	else MainScreen.text("rest against your torso and shoulders");
	MainScreen.text(".");
	if(player.pregnancyIncubation > 0 && player.pregnancyIncubation < 200) MainScreen.text("  Each of these raised legs carefully bend at the knee to accomodate your own pregnant belly that looms over their prone figures.");
	MainScreen.text("  As each witch lays on her side both of them begin feeling your stiff cocks slap against their multiple pussies.  They moan as their four pairs of labia bloom in anticipation of the fucking to come.  Each set of nether lips are dark and dripping with gobs of desperation, but on each girl there's one set that looks particularly gaping as if the pressure inside was forcing it open.  It isn't difficult to figure out that those are the ones that lead to their already occupied wombs, while the other sets are the ones eagerly awaiting a fresh deposit of baby batter.");
	...
	//{if player lactates}
	if(player.lactationQ() >= 200) MainScreen.text("  Your own breasts begin spraying down the two pregnant sluts with your own dairy fluids.");
	//{if player has large but not huge breasts} 
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 20) MainScreen.text("  Each thrust in to the eager cunts causes your breasts to bounce up and down.");
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 20) MainScreen.text("  Each thrust in to the eager cunts causes your massive breasts to sway back and forth, slapping against your own sides.");
	//{if player has six huge breasts}
	if(player.bRows() >= 3) {
		if(player.upperBody.chest.list[2].breastRating >= 20) MainScreen.text("  Your lower beach ball sized breasts are large enough to rest on the floor to either side as you fuck, and both witches eventually lower the legs they've had raised so as to hook them around your massive mammaries and hug your flesh even closer to their thighs.");
	}
	if(player.lowerBody.balls > 0) MainScreen.text("  The closer your shafts come to their climax the more tense your balls become, preparing to pump their loads through your cocks.");
...
public fuckAllThePregWitches():void {
	...
	if(player.lowerBody.cockSpot.count() == 1 || player.cockThatFits2(50) < 0) {
	...
	else if(player.cockThatFits2(50) >= 0 && player.lowerBody.cockSpot.count() < 3) {
	...
	MainScreen.text("\n\nWith your shaft");
	if(multi) MainScreen.text("s");
	MainScreen.text(" thoroughly swallowed up by the juicy confines of the witch");
	if(multi) MainScreen.text("es");
	MainScreen.text(", you begin rocking your hips to and fro, sliding yourself in and out of ");
	if(!multi) MainScreen.text("her");
	else MainScreen.text("their");
	MainScreen.text(" velvety wet depths.  ");
	if(!multi) MainScreen.text("Her");
	else MainScreen.text("Their");
	MainScreen.text(" blissful cries of pleasure fill the room, joined with a chorus of slurping sounds as you thrust your sensitive shafts ever deeper and harder.  ");
	if(!multi) MainScreen.text("Her");
	else MainScreen.text("Their");
	MainScreen.text(" quadruple breasts jiggle and bounce along the floor and ");
	if(!multi) MainScreen.text("her");
	else MainScreen.text("their");
	MainScreen.text(" pregnant ");
	if(!multi) MainScreen.text("belly sways");
	else MainScreen.text("bellies sway");
	MainScreen.text(" with each ramming motion of your cock");
	if(multi) MainScreen.text("s");
	MainScreen.text(".  The muscles in ");
	if(!multi) MainScreen.text("her");
	else MainScreen.text("their");
	MainScreen.text(" thighs and calves tense with pleasure as you push ");
	if(!multi) MainScreen.text("her body");
	else MainScreen.text("their bodies");
	MainScreen.text(" to the limit.");
	//{if player has non-huge tits}
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 3 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 10) MainScreen.text("  Your tits bounce as you enthusiastically grind away.");
	//{else if player has huge tits}
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 10 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 20) MainScreen.text("  The heavy weight of your huge tits causes them to sway with every thrust of your hips.");
	//{if player has really huge tits} 
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 20) MainScreen.text("  The immense swells of your mammaries press down upon the ass of the girl in front of you.");
	//{if player has four non-huge tits} 
	if(player.bRows() >= 2) {
		if(player.upperBody.chest.list[1].breastRating >= 3) MainScreen.text("  Your second row of breasts slap against the ass cheeks of the girl in front of you.");
	}
	//{if player has six huge but not really huge tits} 
	if(player.bRows() >= 3) {
		if(player.upperBody.chest.list[2].breastRating >= 3 && player.upperBody.chest.list[2].breastRating < 20) {
			MainScreen.text("  Your lowermost row of breasts become squashed against the witch");
			if(multi) MainScreen.text("s");
			MainScreen.text("'s muscular thighs.");
		}
		else if(player.upperBody.chest.list[2].breastRating >= 20) {
			MainScreen.text("  Your lowermost row of breasts come to rest on the floor as you lean forwards to fuck, and the witch");
			if(multi) MainScreen.text("es");
			MainScreen.text(" can almost support ");
			if(multi) MainScreen.text("their");
			else MainScreen.text("her");
			MainScreen.text(" thighs against your obscene mammaries when they lean back against the constant ramming of your cock.");
		}
	}
	//{if player lactates}
	if(player.lactationQ() >= 200) {
		MainScreen.text("  After only a short while of mindless rutting your tits begin leaking, and in short order your glistening nipples start spraying your milk over ");
		if(!multi) MainScreen.text("the witch's tanned backside");
		else MainScreen.text("the collective tanned bodies you're impaling");
		MainScreen.text(".  A few other witches in the room eagerly step forth to suckle your [nipples] as they await their turn for impregnating.");
	}
	
	MainScreen.text("\n\nThe cum inside your loins starts to move, filling the base of your shaft");
	if(multi) MainScreen.text("s");
	MainScreen.text(" as you feel your orgasm creep up inside you.  The sucking sensations of the ");
	if(!multi) MainScreen.text("girl's cunt on your cock");
	else MainScreen.text("the girls' cunts on your cocks");
	MainScreen.text(" starts to become overwhelming, and before you realize it you're on the verge of releasing your seed.  The ");
	if(!multi) MainScreen.text("pussy");
	else MainScreen.text("pussies");
	MainScreen.text(" you're pounding seem");
	if(!multi) MainScreen.text("s");
	MainScreen.text(" to sense the imminent release of the sperm ");
	if(!multi) MainScreen.text("it craves");
	else MainScreen.text("they crave");
	MainScreen.text(" and strong contractions begin sucking on your shaft");
	if(multi) MainScreen.text("s");
	MainScreen.text(" even more fervently.  You can't help but give ");
	if(!multi) MainScreen.text("it the load of baby batter it craves");
	else MainScreen.text("them the loads of baby batter they crave");
	MainScreen.text(".");
	
	MainScreen.text("\n\nThe ");
	if(!multi) MainScreen.text("underside of your shaft swells");
	else MainScreen.text("undersides of your shafts swell");
	MainScreen.text(" as your bounty of man-seed floods your full length and stretches the ");
	if(!multi) MainScreen.text("witch's cunt");
	else MainScreen.text("witches' cunts");
	MainScreen.text(" even wider.  Pressing as far forwards as possible you align the ");
	if(!multi) MainScreen.text("tip of your tool against her abused cervix");
	else MainScreen.text("tips of your tools against their weakened cervixes");
	MainScreen.text(" just as your cum begins erupting out of you.");
	
	//{if cumNormal-cumHigh}
	if(player.cumQ() < 1000) {
		MainScreen.text("\n\nYou plaster the insides of ");
		if(!multi) MainScreen.text("her pussy");
		else MainScreen.text("their pussies");
		MainScreen.text(" with no small amount squirting directly in to ");
		if(!multi) MainScreen.text("her womb");
		else MainScreen.text("their wombs");
		MainScreen.text(".  When you finally pull out ");
		if(!multi) MainScreen.text("a stream");
		else MainScreen.text("streams");
		MainScreen.text(" of cum are left behind.");
	}
	//{else if cumVeryHigh-cumExtreme]}
	else {
		MainScreen.text("\n\nThe horny ");
		if(!multi) MainScreen.text("witch isn't");
		else MainScreen.text("witches aren't");
		MainScreen.text(" prepared for the deluge that explodes from your genitals.  Liter after liter pours forth, filling up ");
		if(!multi) MainScreen.text("her empty womb like a balloon");
		else MainScreen.text("their empty wombs like a bunch of balloons");
		MainScreen.text(" and causing your eyes to roll back in your head from the sheer pleasured sensation of emptying yourself.  When ");
		if(!multi) MainScreen.text("her belly");
		else MainScreen.text("their bellies");
		MainScreen.text(" swell as large as the neighboring pregnancies and stretch marks start appearing up and down each side you finally pull out and allow your cock");
		if(multi) MainScreen.text("s");
		MainScreen.text(" to finish spraying ");
		if(!multi) MainScreen.text("its");
		else MainScreen.text("their");
		MainScreen.text(" final loads over the ");
		if(!multi) MainScreen.text("whorish witch's back");
		else MainScreen.text("collective grouping of the witches' asses");
		MainScreen.text(".");
	}
	
	//{all}
	if(!multi) MainScreen.text("\n\nShe falls");
	else MainScreen.text("\n\nThey fall");
	MainScreen.text(" over.  Quickly, the rest of the witches in the room are eagerly raising their cunts in the air, begging for their turn at being impregnated.  Happily you oblige, shoving as much cock as you can into each wet hole that the hormonally overwrought sluts offer to you.  One pussy after another finds itself impaled on you, with you thrusting repeatedly until the massaging muscles within their cunts pull forth the rivers of baby gravy that their empty wombs are starving for.  One by one the witches find themselves filled and fall to the floor still reeling from the sensations, only for another new witch to take their place with a sopping cunt even more cum thirsty than the last.");
	
	//{if player has two or more cocks} 
	if(player.cockThatFits2(50) >= 0) MainScreen.text("\n\nFor the final witch you choose to shove two cocks inside her empty pussy.  Your double dicked thrusts force a joyous yelp from her with every thrust.  \"<i>Ah! T-t-too much!</i>\" she screams but her vaginal canal refuses to let you go regardless.  When you finally finish the torrent of sperm makes her feel as if she's on the verge of bursting.  The swelling of her womb causes her belly to bloat up enough to push her quad breasts upwards and siginificantly shift her center of gravity.  The orgasm that accompanies the feeling of your sperm flooding forcefully into her uterus and tubes is too much for her to bear and she slides off of your cocks like a well used cock sleeve, oozing gobs of cum from between her dark nether lips.");
...
public milkBathsAhoy():void {
	...
	MainScreen.text("  You could probably masturbate in the tub if you wanted to, or maybe pull the dusky milk-maid in for company....  What do you do?");
	//{If can masturbate [Drink & Masturbate] [Milk Girl] [Relax]}
	//{otherwise [next] to relaxing}
	fatigue(-10);
	menu();
	MainScreen.addButton(0,"Next",finishMilkBath);
	if(player.gender > 0) MainScreen.addButton(1,"DrinkNFap",drinkNFap);
	MainScreen.addButton(2,"Milk Girl", grabTheMilkGirl);
}

//[Next] (Relax)
public finishMilkBath():void {
	MainScreen.clearText();
	MainScreen.text("You sit in the tub for a while, letting the fragrant fluids soak into your " + player.skinFurScales() + ".  Yet, you have work to do, and eventually, you tire of relaxing in the sand witches' endless white bounty.  You pull out the tub's plug and climb out, finding a towel on the wall.  Thankfully, the milk doesn't seem to leave behind any residue, and you feel clean and refreshed, if a bit horny.");
	//(+Lust, -Fatigue)
	dynStats("lus", 10);
	fatigue(-50);
	doNext(playerMenu);
...
public grabTheMilkGirl():void {
	MainScreen.clearText();
	MainScreen.text("You call out to the milk slave before she can slink away, and wade over to the side of the tub, leaving your face a few scant inches from her massive, milk-laden jugs. She cocks her head to the side, obviously unused to the attention, and quietly whispers, \"<i>" + player.mf("M-master?","M-mistress?") + "</i>\"");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 8) MainScreen.text(", pressing deep into your own huge rack, your nipples flicking across her own leaky tits");
	MainScreen.text(".");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.balls > 0) MainScreen.text(", her other hand cupping your [balls], rolling the " + num2Text(player.lowerBody.balls) + " orbs in her palm with surprising dexterity");
		MainScreen.text(".  Leaning over the two titanic teats between you, she traces a line of kisses down your back, licking up stray drops of milk between affectionate caresses.");
	}
	//{If PC has cooch:
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) MainScreen.text("  She cups your breasts, having to reach so far around both your rack and hers that she's straining her arms to rub your [nipples], but she does so valiantly, stroking them with her incredibly deft fingers.");
		if(player.lactationQ() >= 200) MainScreen.text("  A spurt of milk escapes your own full jugs, joining the pool-full of your new friend's.  She gasps with surprise and delight, quickly nuzzling herself into your back and going to work, milking you just as she would herself, letting the hefty flow of your motherly fluids pour into the pool, odd trickles smearing down your chest, staining your chest as white as her own.");
	}
	//[Fuck Her](PC must have gender; if cooch, also C+ cups) [Don't]
	menu();
	dynStats("lus", 33, "resisted", false);
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.addButton(0,"Dick Fuck",fuckMilkbabeWithPenor);
	if(player.lowerBody.vaginaSpot.hasVagina() && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) MainScreen.addButton(1,"Lady Fuck",ladyFucks);
	MainScreen.addButton(2,"Don't Fuck",dontFuckMilkBathBabe);
}

//[Don't]
public dontFuckMilkBathBabe():void {
	MainScreen.clearText();
	MainScreen.text("You allow the girl to continue for a long, long while until your entire body feels deeply refreshed, her milk having soaked into your body and making you feel fresh and revitalized. You start to thank the milk girl for the pleasurable company, but when you open your mouth, she slips into your arms and presses her lips to yours.  Chuckling to yourself, you hold the girl as tight against yourself as her udders will allow, turning her to the side to let her nuzzle her cheek into your [chest], kissing the top of her head before the two of you climb from the pool.  You have to help her out, her massive extra weight nearly dragging her back in except for your quick reflexes.  You gather your [armor] and ruffle the milk slave's hair before turning back to the task at hand.");
	//[+Lust, +HP, -Fatigue]
...
public ladyFucks():void {
	MainScreen.clearText();
	MainScreen.text("You turn around in the milky pool, pulling the cute little slave tight against you.  She gasps with surprise, but settles as soon as you press your lips to hers, your hands wandering across her huge teats and supple, milky body.  She nuzzles up against you, her head resting on your [chest] as you hold her against yourself, stroking her dark hair.  After a few moments of such a simple pleasure, the little slave girl shifts her cheek along your breast, wrapping her full, dusky lips around your [nipple].  You let out a long moan as she suckles gently");
	if(player.lactationQ() >= 200) MainScreen.text(", drawing out a trickle of milk from your motherly reserves.  She gulps deeply, smiling up at you as a trickle of your milk runs down her chin, dripping into the pool of her own");
	MainScreen.text(".  Her hand slips up your body, brushing your vulva and [clit] before cupping your other breast, delicate fingers wrapping around your [nipple].  With deft, practiced motion, she works your teats between her fingers, working your breast like she might her own");
	if(player.lactationQ() >= 200) MainScreen.text(", milking you with skill beyond anything you've ever experienced before; and why not, when her entire existence revolves around that self-same skill?");
	else MainScreen.text(".");
	...
	if(player.lactationQ() >= 200) MainScreen.text(", spraying milk all over yourself and the milkmaid who caused your explosive pleasure");
	MainScreen.text(".  You run your fingers through the girl's hair, urging her sexual skills on as your chest heaves and quavers, riding out the massive boobgasm as fem-cum spurts from your cunt and into the milky pool below.");
	MainScreen.text("\n\nYour entire body feels deeply refreshed, her milk having soaked into your body and making you feel fresh and revitalized, and every muscle seems to have relaxed thanks to your blissful coitus.  You start to thank the milk girl for the pleasurable company, but when you open your mouth, she presses her lips to yours for a long, tongue-filled kiss.  Chuckling to yourself, you hold the girl as tightly as her udders will allow, turning her to the side to let her nuzzle her cheek into your [chest], kissing the top of her head before the two of you climb from the pool.  You have to help her out, her massive extra weight nearly dragging her back in except for your quick reflexes.  You gather your [armor] and ruffle the milk slave's hair before turning back to the task at hand.");
	//[+Lust, +HP, -Fatigue]
	player.orgasm();
	fatigue(-15);
...
public drinkNFap():void {
	MainScreen.clearText();
	MainScreen.text("\"<i>Wait,</i>\" you call out to the ebony woman, letting the milk obscure your hands as you begin to masturbate, \"<i>I want a drink.</i>\"  Sheepishly, the milk slave obliging shifts to drag her tits back into place.");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) MainScreen.text("[vagina] and idly stroke your puffy, lust-engorged vulva");
	else MainScreen.text("[cock] and idly stroke the turgid mass");
	MainScreen.text(", inadvertently drawing a lewd moan from yourself.  The fat nipple stuffed in your mouth does an adequate job of muffling your pleasurable vocalizations");
	if(player.lowerBody.cockSpot.hasCock()) MainScreen.text(", but it does little to hide the swelling of [eachCock] - you have the milk for that");
	MainScreen.text(".");
	...
	//(DA HERMS) 
	if(player.gender == 3) {
		MainScreen.text("  Your [cock] throbs painfully in your hand, so hot and hard that you're sure you must have begun to leak precum, but any fluid is swiftly washed away by the ever-present milk.");
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("  You make sure to fondle each of your members equally, caressing, squeezing, and stroking to the tempo of your swelling passion.");
		MainScreen.text("  With your off-hand, you rub your cream-lubricated fingers through your sodden gash, the flesh parting easily to allow a few of your questing fingers inside.  Delicious bliss unfolds from your [clit] as it pushes free of its hood, fully engorging, faintly throbbing from aching need.  You brush the button a few times before going back to fingering your box, yet you make sure to strum your thumb across your clit every few moments to keep yourself as close to peak as possible.  Truly, being a hermaphrodite is bliss.");
	}
	//(DA SCHLICKS)
	else if(player.gender == 2) MainScreen.text("  Your pussy juices mix freely with the tub's white-colored 'waters', allowing your cream-lubed fingers to plunge into your [vagina] with ease.  You stroke your lips and caress the interior of your birth canal with the intimate familiarity of a skilled lover, playing with your body until you feel your control slipping, so wound up with lust that you feel like an over-tightened guitar string vibrating out of control.");
	//(DA DUDES)
	else {
		MainScreen.text("  Your [cock] throbs painfully in your hand, so hot and hard that you're sure you must have begun to leak precum, but any fluid is swiftly washed away by the ever-present milk.  ");
		if(player.lowerBody.cockSpot.count() > 1) MainScreen.text("You make sure to fondle each of your members equally, caressing, squeezing, and stroking to the tempo of your swelling passion.");
		else MainScreen.text("  With your offhand, you cradle your [sack], hefting your [balls] as you feel your desire churning to new levels.");
	}
	//(TOGETHER)
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		MainScreen.text("  [EachCock] releases its own gooey load, sputtering and spurting to add to the pearly deluge.");
		if(player.cumQ() >= 500) MainScreen.text("  With every torrent of seed you release, you can see it lift partway out of the tub, propelled by your incredible virility towards the nearest female specimen.");
		if(player.cumQ() >= 3000) MainScreen.text("  Soon, the tub's fluid contents break out of their confines to stain your companion's dusky flesh white, an alabaster glaze that would entice you to further feats of debauchery were it not for the pleased contentment your maleness now radiates.");
	}
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		MainScreen.text("  Meanwhile, your juiced-up cunny feels like it's doing backflips inside you, contorting and squeezing as it gushes with fluid, feminine joy.  One brush on your [clit] knocks your [legs] out from underneath you, but thankfully, you float out the rest of your orgasm.");
	}
...
public sandPregUpdate():boolean {
	...
	if(player.pregnancyIncubation == 280) {
		...
		if(player.biggestLactation() < 1) MainScreen.text("starting to lactate");
		else MainScreen.text("lactating more");
		MainScreen.text("?  Your belly is even bulging a little bit more than food can explain.\n");
		temp = player.bRows();
		while(temp > 0) {
			temp--;
	...
	if(player.pregnancyIncubation == 216) {
		...
		while(temp > 0) {
			temp--;
			if(player.upperBody.chest.list[temp].breastRating < 20) player.upperBody.chest.list[temp].breastRating++;
			else player.upperBody.chest.list[temp].breastRating += 0.5;
		}
		while(player.biggestLactation() < 1.5) {
			player.boostLactation(.5);
		}
...
public birthAWitch():void {
	...
	if(player.lowerBody.vaginaSpot.count() == 0) {
		MainScreen.text("You feel a terrible pressure in your groin... then an incredible discomfort accompanied by the rending of flesh.  You look down and behold a vagina.  ", false);
		player.createVagina();
		genderCheck();
	}
