...
public function candyCaneTrapDiscovery():void {
	...
	if(player.stats.cor < 50) outputText("The memory brings a nostalgic smile to your [face].");
...
public function leaveBeforeMeetingCandyCaneBun():void {
	...
	if(player.stats.cor > 50 || player.stats.lib > 50) outputText("  At least not right now.  You turn back, navigating your way back to your camp.");
...
public function investigateCandyCaneBun():void {
	...
	if(player.stats.cor < 50 && player.stats.lib < 50) outputText("  Your eyes are quickly drawn to the rabbit morph's stark nudity.");
	//(If corruption or libido>50) 
	...
	//(If corruption and libido<50)
	if(player.stats.cor < 50 && player.stats.lib < 50) outputText("leave, ");
	//(If corruption or libido<50)
	...
	//(If corruption<50)
	if(player.stats.cor < 50) outputText("You calmly inform him that you didn't have the intention of violating him. ");
	//(If corruption>50)
...
public function helpWithTheCandyCane():void {
	...
	//(If corruption<50) 
	if(player.stats.cor < 50) outputText("  Not wanting to disturb him, you make your way back to camp and attempt to ignore the burning fire in your loins.");
	//(If corruption>50) 
...
public function acceptChristmasManBJ():void {
	...
	//(else if cock would please Goldilocks) (area<= 30 or some shit i dunno whatever the standard is)
		...
		//(if only one cock)
		if(player.lowerBody.cockSpot.count() == 1) outputText("She grabs onto your hips, enabling herself to move all the quicker.");
		//(if two cocks)
		else if(player.lowerBody.cockSpot.count() == 2) outputText("With her other hands she starts to work the shaft of the second cock, pumping in time with the motions of her head.");
		//(if 3+ cocks)
		...
		//(if multicock)
		if(player.lowerBody.cockSpot.count() > 1) outputText("  Your cum sprays over her, coating her skin and plumage in a white to rival their own.  Strands of your semen dripping and running over her hardly even stand out on her bright body.");
...
public function giveKamiTheChristmasRooAnal():void {
	...
	//[If player has balls]
	if(player.lowerBody.balls > 0) outputText(", your balls slapping against her with every push");
...
public function takeItRooButtStyle():void {
	...
	if(player.lowerBody.balls > 0) outputText("your own");
	//[Else if have vagina]
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your pussy");
	//[Else if neither]
	...
	outputText("\n\n");
	//[If player has pussy] 
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Your cunt oozes in return, envious of the tenderized hole above it.  ");
	...
	//[If male]
	if(player.lowerBody.cockSpot.hasCock()) outputText("  Your pent-up cock ejaculates there and then, shamelessly gushing sloppy semen against the wall in front of you.");
	//[If female]
	if(player.gender == 3) outputText("  Meanwhile, y");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Y");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("our eager pussy buckles in arousal, gushing fem-juices on the floor below you.");
...
public function KamiBlowJob():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  You think you might still need to find an outlet for that itch of yours, with [eachCock] still erect and needing release.");
...
public function unwrapPolarPete():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  [EachCock] grows stiff and soaks your [armor] with precum.");
	//(if vagina) 
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your [vagina] grows moist, dripping eager juices and soaking your [armor].");
	...
	//[(if cock) 
	if(player.lowerBody.cockSpot.hasCock()) outputText("  His continued ramming of your prostate mixed with the addictive candy crammed in your mouth sends you over the edge, [eachCock] shooting out ropes of snow white all over your chest.  Pete eyes you over with a lurid gaze, your face covered in his cum and your body covered in your own.");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your [vagina] quivers, the rough thrusting into your [asshole] mixed with the addictive candy in your mouth sending you over the edge.  With a low muffled moan your mound spills your lust over your groin, dripping down to lube Pete's thrusts as well as your [legs].  Pete gives you a lurid gaze.");
...
public function nieveComesToLife():void {
	...
	if(flags[FlagEnum.NIEVE_GENDER] == 1) outputText("  It appears to be male, judging by the modestly sized cock sprouting from its loins.");
	...
	if(flags[FlagEnum.NIEVE_MOUTH] == "gems") outputText("glittering purple");
...
public function approachNieve():void {
	...
	if(flags[FlagEnum.NIEVE_MOUTH] == "coal") outputText("dusky black");
...
public function lickNieve():void {
	...
	if(!silly()) {
		...
		//(Normal/Snake Tongue: 
		if(player.tongueType != TONUGE.DEMONIC) outputText("It doesn't get far, but elicits squeals of surprise and pleasure from your elemental slave who's already grasping at the snow surrounding her for support.");
		//Demon Tongue: 
		else if(player.tongueType == TONUGE.DEMONIC) outputText("Her eyes widen as you snake your impressive, demonic tongue into her. She grunts and squeals with pleasure, hands grasping piles of snow, wishing they were bedsheets.");
	...
	//Silly Mode:
		...
		//(no Kid A: 
		if(flags[FlagEnum.ANEMONE_KID] != 1) outputText(", Nieve takes a ladle full of water and pours it onto her snatch");
		//Kid A: 
...
public function fuckNieve():void {
	...
	if(x >= 0) {
		if(player.lowerBody.cockSpot.list[x].cockLength < 6) outputText("  \"<i>It's so cute, [Master],</i>\" Nieve says while catching a breath, \"<i>Like a little toy cock.</i>\"");
		//If overly large (>130): 
		else if(player.lowerBody.cockSpot.list[x].cockLength >= 24) outputText("  She has to take a breath almost immediately, saying, \"<i>By the fat man's beard this thing is huge.  You must make the rest of this world jealous.</i>\"");
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength < 6) outputText("\"<i>It's so cute, [Master],</i>\" Nieve says while catching a breath, \"<i>Like a little toy cock.</i>\"");
		else if(player.lowerBody.vaginaSpot.list[0].clitLength >= 24) outputText("  She has to take a breath almost immediately, saying, \"<i>By the fat man's beard this thing is huge.  You must make the rest of this world jealous.</i>\"");
		else outputText("  \"<i>Mmm,</i>\" she smacks her lips and takes a quick breath, \"<i>Such a nice example of a candycane right here.  Let's go in for another taste.</i>\"");
	}
	...
	if(x >= 0) {
		//(Human) 
		if(player.lowerBody.cockSpot.list[x].cockType == CockType.HUMAN) outputText("  Your normal, unaltered cock seems to take on a slight blue tint, although you can confirm it's not going numb.  Quite the contrary, it seems to be getting more sensitive as Nieve's lips roll over it.");
		//(Equine) 
		else if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("  Nieve fawns over your horsecock like a cultist praying to a god.  She sucks and kisses, running the tip of her tongue all along the sides until she reaches the sheathe, and then right back up the other side.");
		//(Canine) 
		...
		*/	
		//(Demonic)
		else if(player.lowerBody.cockSpot.list[x].cockType == CockType.DEMON) outputText("  Your purple, demonic shaft pulses and throbs, seemingly immune to the unnatural cold of Nieve's mouth. Seemingly oblivious to its nature, Nieve slavers over each nodule, giving every ridge and bump a kiss and more than just a lick.");
		//(Tentacle) 
		else if(player.lowerBody.cockSpot.list[x].cockType == CockType.TENTACLE) outputText("  Nieve does not seem put off by your tentacle cock in the least, in fact she confesses, \"<i>It reminds me of a comic I read before coming here.</i>\"  As she fawns over your cock, you're certain you hear whispers of \"<i>Oh, you sexy tentacle beast...</i>\" coming from her mouth when she gets her breath.");
		//(Feline) 
		else if(player.lowerBody.cockSpot.list[x].cockType == CockType.CAT) outputText("  Though the barbs on your cock are soft and pliable, they reflexively harden up after Nieve's tongue washes over them.  She doesn't pay particular attention to the barbs, but she does seem rather careful not to get one hooked on her lip.");
		//(Lizard)
		else if(player.lowerBody.cockSpot.list[x].cockType == CockType.LIZARD) outputText("  Nieve fawns over your scaled and ridged reptilian cock, slurping and smooching up and down its length. It seems a bit more resistant to the biting cold of Nieve's mouth, which is lucky when she backs off, exposing your entire wet, frigid length to the wintery air surrounding you.");
		//(Anemone) 
		else if(player.lowerBody.cockSpot.list[x].cockType == CockType.ANEMONE) outputText("  Nieve giggles whenever one of your wriggling tentacles stings her, seemingly immune to its effects, and kisses it back.  Soon after she devours your whole length, delighted to feel the sting of your cock in the back of her throat.");
		//(Kangaroo) 
		else if(player.lowerBody.cockSpot.list[x].cockType == CockType.KANGAROO) outputText("  It takes Nieve a little while to catch on that your cock isn't quite human, and by the time she figures it out, she mutters, \"<i>Huh, that's strange... I thought kangaroos had balls above the cock,</i>\" before shrugging and getting back to work.");
	...
	//(Clitoris) 
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength < 5) outputText("It's just like a little cock");
		else if(player.lowerBody.vaginaSpot.list[0].clitLength < 9) outputText("It's just like a cock");
	...
	if(x >= 0) {
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength <= 4) outputText("Your meager clit isn't much, but at this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes staring up at you, urging you on faster and faster.");
		else if(player.lowerBody.vaginaSpot.list[0].clitLength <= 12) outputText("At this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes tightly shut as she whispers dirty words you can't even make out.");
...
public function nieveFucksYou():void {
	...
	outputText("\n\nTo which you nod, ");
	if(player.stats.cor < 50) outputText("telling him that you find his form... appeasing to say the least, and would like to know how his 'parts' are working properly.");
	...
	//[if (hasVagina = true) 
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your [vag] is a sodden box so soon after he started, and you start to want more, much more!");
	//[if (hasCock = true) 
	if(player.lowerBody.cockSpot.hasCock()) outputText("  Your [cock biggest] is also rock hard in a short while and you quickly desire some more stimulation from your icy love slave.");
...
public function takeNieveAnal():void {
	...
	//[if (hasCock = true) 
	if(player.lowerBody.cockSpot.hasCock()) outputText("  Nieve then bends forward while keeping his pace to give you a reach-around.  Grabbing your own [cock biggest] in one hand, he starts to pump it to the speed of his thrusts, torturing you with the extra-stimulation in such a good way.");
	//[if (hasVagina = true)
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  As Nieve continues to pound away, he places one hand on your sopping wet cunt and begins to finger you in time with his assault, making you lust-drunk with the stimulation as he finger bangs you.");
	...
	if(player.gender > 0) {
		...
		//[if (hasVagina = true)
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText(".  Your soaking vagina makes your body quake and shudder as you orgasm, splashing your femcum all over Nieve and the ground");
	}
	outputText(".");
...
public function takeNieveVaginal():void {
	...
	//[if (hasBalls = true) 
	if(player.lowerBody.balls > 0) outputText(", gently moving your [balls] out of the way in the process");
...
public function nieveIsOver():void {
	...
	//Nieve Completed
		...
		if(player.stats.cor < 50) outputText("  You even tear up yourself, sad to see such a good friend leave, and so abruptly.");
		//Corruption >50: 
...
private function nieveReturnsPartII():void {
	...
	if(flags[FlagEnum.NIEVE_MOUTH] == "coal") outputText("coal black");
