...
public function loseToSammitchMob():void {
	...
	if(flags[FlagEnum.SAND_WITCHES_FRIENDLY] == 0) outputText("into the fire like it was trash");
...
public function getMockedForSmallDongBySammitchMob():void {
	...
	if(player.lowerBody.balls > 0) outputText("  Your [balls] didn't have enough time to make another full batch.");
	...
	//If balls:
	//[If balls & pussy:]
	if(player.lowerBody.balls > 0 && player.lowerBody.vaginaSpot.hasVagina()) outputText("  She even cups your [balls] but moves to touch your [vagina] when she feels its wetness dripping off the testes."); 
	else if(player.lowerBody.balls > 0) outputText("  She even cups your tender [balls] and lightly massages them.");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  She even lightly caresses the lips of your pussy.");
...
public function memeberedFolksFindTrueWuv():void {
	...
	if(player.HP < 1) {
		...
		if(player.upperBody.chest.hasFuckableNipples()) outputText(", fondled, and fucked");
		else outputText(" and fondled");
		outputText(".");
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your [vagina] even gets in on the action, converting your undergarments into a swampy mess of female pheromones and dripping lubricant.");
	}
	//Lust
	else {
	...
	if(player.lowerBody.cockSpot.count() == 2) outputText(" while another grabs your other one");
	else if(player.lowerBody.cockSpot.count() > 2) outputText(" while others latch onto the rest");
	...
	if(player.lowerBody.balls > 0) outputText(" while your [balls] start to work, clenching and relaxing as they brew a thick load of fuckbatter thanks to the chemical jump-start.");
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() == 2) outputText("itself");
		else outputText("themselves");
		outputText(".");
	}
	outputText("Slurping noisily, the cunt");
	if(player.lowerBody.cockSpot.count() > 1) outputText("s piston");
	else outputText(" piston");
	outputText(" and bob, splattering ");
	if(player.lowerBody.cockSpot.count() > 1) outputText("their");
	else outputText("its");
...
public function memeberedFolksFindTrueWuv2():void {
	...
	if(player.lowerBody.cockSpot.count() == 1) outputText("fighting over your cock");
...
public function forceCunnilingusRimjobClitAndNipple():void {
	...
	outputText(".  They may ");
	if(player.stats.cor < 33) outputText("be misguided in fighting you");
	else if(player.stats.cor < 66) outputText("be a little over-zealous in defending their home");
...
public function sandstonesAreCool():void {
	...
	if(!combatMiss() && !combatEvade() && !combatFlexibility() && !combatMisdirect()) {
		...
		if(!player.isGoo()) {
			...
			//If PC has 1 cock:
			if(player.lowerBody.cockSpot.count() == 1) outputText("  A stone crawls onto your [cock].");
			//[If PC has multi-cocks: 
			if(player.lowerBody.cockSpot.count() > 1) outputText("  A bunch of the stones crawl onto your " + player.multiCockDescriptLight() + ".");
			if(player.lowerBody.cockSpot.hasCock()) bonus++;
			//[If PC has cunt]
			if(player.lowerBody.vaginaSpot.hasVagina()) {
				outputText("  One stone slides up your inner thigh");
				if(player.lowerBody.balls > 0) outputText(" behind your [sack]");
			...
			outputText("  " + num2Text(player.upperBody.chest.countNipples()) + " crawl up to your chest and over your top " + nippleDescript(0) + "s");
			if(player.bRows() > 1) {
...
public function turnIntoASammitch():void {
	...
	if(player.lust > 99) {
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("dig your hands into your loins, frigging your hungry cunt so hard your juices splatter the Witch's robe.");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("  [EachCock] throbs beneath you, hard as rock and crushed between your belly and the slippery-smooth wood.  It seems unimportant compared to what's going on above it.");
...
public function cumWitchCumPumpBadEnd():void {
	...
	if(player.cumQ() > 1000) {
		if(player.lowerBody.balls > 0) outputText(" and such virile, cum-swollen testes.  An excellent breeder for sure!");
...
public function beACumPumpPartII():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("es");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("Her voluminous butt is thoroughly smeared by the goo from the rest of your " + player.multiCockDescriptLight() + ", shining with white from your magical virility.  ");
...
public function cumPumpVolumeThree():void {
	...
	if(player.statusAffects.has("FactoryOverload")) outputText("destroying");
...
public function repeatLoseToCumWitchForDudes():void {
	...
	//Lust:
		...
		if(player.lowerBody.balls > 0) outputText(", and your [balls] are practically quaking with need");
	...
	if(player.cockThatFits(monster.vaginalCapacity()) < 0) {
		...
		if(player.upperBody.chest.countNipples() > 2) outputText("two of ");
		outputText("your [nipples]");
		if(player.upperBody.chest.hasFuckableNipples()) outputText(", sinking inside your moist tit-holes almost immediately");
		...
		if(player.lowerBody.balls > 0) {
			...
			if(player.lowerBody.ballSize < 2) outputText("petite ");
			else if(player.lowerBody.ballSize >= 5) outputText("weighty ");
			outputText("orbs across her soles.");
		}
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("[vagina], slipping a few toes through your folds while a big toe diddles your [clit].");
		...
		if(player.lowerBody.cockSpot.count() > 1) { 
			...
			if(player.lowerBody.cockSpot.count() > 1) outputText("s spurt");
...
public function tooBigCumWitchLossFacial():void {
	...
	if(player.stats.cor < 33) outputText("  You shudder at the knowledge of what's going to happen to you.  Just why did you agree to this?");
	else if(player.stats.cor < 66) outputText("  You catch yourself licking your lips as the knowledge of what is about to happen to you sinks in. Despite hastily stopping yourself, the Cum Witch still saw, and she smiles.");
	...
	if(player.upperBody.chest.hasFuckableNipples()) outputText("in");
	else outputText("on");
	...
	if(player.lowerBody.balls > 0) outputText("[balls], compressing them slightly as a tingle of magic lances into you, fattening them under her ministrations");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("[vagina], digging into the slippery slit as a tingle of magic spreads through it and into your core, thickening some organ inside you");
...
public function tooBigCumWitchLossNoFacial():void {
	...
	if(player.lowerBody.balls > 0) outputText(", just like your own");
...
public function TDMsLoseToCumWitchScene():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		//if (cocks = 2)
		if(player.lowerBody.cockSpot.count() == 2) outputText("your other phallus.");
		//[if (cocks > 2)
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("one of ");
	outputText("yours...");
	...
	if(player.lowerBody.balls > 0) outputText("the one part of your manhood that you can still feel, your [sack].");
	...
	//[if (sensitivity < 70)]
	if(player.stats.sens < 70) outputText("It's more sensitive than yours, for one thing, and");
...
public function menFuckUpSomeCumWitch():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Two can play at that game.");
	//Herm sexback
	if(player.gender == 3) {
		...
		outputText("\n\n");
		if(player.lowerBody.balls > 0) outputText("Your sweaty nutsacks slide over each other while you fuck, compressing together into a mass of sweaty orbs twitching against each other in a competition to disgorge their frothy load before the other.");
	...
	//Regular Dicking
		...
		if(player.stats.cor < 50) outputText("and a hint of compassion");
		...
		if(player.stats.cor < 33) outputText("  You hope she learned something from the experience, but knowing most people in this strange land, she'll go right back to her rotten ways.");
		else if(player.stats.cor < 66) outputText("  You wonder if she'll learn anything from this and shrug nonchalantly.  Who cares?");
...
public function maleTooBigVictorySex():void {
	...
	if(player.lowerBody.balls > 0) outputText(" or fondle your [balls]");
	...
	if(player.lowerBody.balls > 0) outputText(" along with your [balls] growing tight");
...
public function lionpaws(skipped:boolean = false):void {
	...
	//[if event occurrence > 0]
		...
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 100) outputText("  It's not as if there's all that much else I can do for that monster of yours anyways.");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("s");
	outputText(" of your ");
	if(player.lowerBody.cockSpot.count() > 1) outputText("various ");
	outputText("prick");
	if(player.lowerBody.cockSpot.count() > 1) outputText("s");
	...
	outputText("\n\nApparently deeming ");
	if(player.lowerBody.cockSpot.count() > 1) outputText("them");
	else outputText("it");
	...
	if(player.lust < 50 && flags[FlagEnum.PAWJOBS] == 0) outputText("  You raise your eyebrows in surprise, but you're not in much of a position to do anything about her foot's advance.");
	...
	if(!skipped && flags[FlagEnum.PAWJOBS] == 0) outputText("It feels wonderful in a peculiar sort of way; though it's certainly not the treatment you expected to receive, you can definitely feel your arousal being pushed along and built up by her foot.  ");
	//[if PC has 2+ dicks]
	if(player.lowerBody.cockSpot.count() > 1) outputText("Not content to leave your other cocks neglected, the sphinx leans in and starts to lick and suckle once again at the pricks not beneath her paw.  You shudder at the feeling of her tongue darting from shaft to shaft and teasing your heads, sharper sensations punctuating her foot's more constant pleasuring.  ");
	...
	if(flags[FlagEnum.PAWJOBS] > 0) outputText(" as always");
...
public function getBlown(submit:boolean = false):void {
	...
	if(player.lowerBody.balls > 0) outputText(", one hand slipping down to cup your [balls], rolling them in her palm until a white trickle escapes your [cockHead biggest]");
	...
	if(player.lowerBody.balls > 0) outputText("balls empty");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("each of your ");
	else outputText("your ");
	...
	if(player.lowerBody.balls > 0) outputText("[balls] churn");
	...
	if(player.lowerBody.cockSpot.count() >= 3) {
		...
		if(player.lowerBody.cockSpot.count() == 2) outputText("your ");
		else outputText("two of your ");
		outputText("other " + num2Text(player.lowerBody.cockSpot.count()-1) + " members, wrapping tightly around their meaty lengths.  You groan throatily as tingling digits slowly work their way up your shafts, making your heart flutter wildly with the alien sensation.");
		//AND If Multicock = 5
		if(player.lowerBody.cockSpot.count() >= 5) outputText("  You barely have time to adjust to the eldritch hands jerking off your extra cocks when Sanura's own come into play, wading into the jumble of wangs growing from your crotch to grab another pair of pricks.  With five cocks encircled in hands, magic, and mouth, Sanura slowly begins to synchronize her movements, gently stroking your many cocks.");
	}
...
public function fuckDatLionPussah():void {
	...
	if(player.upperBody.chest.countNipples() == 2) outputText("both");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("damp cunt, which suddenly tightens up and practically gushes with slick juices.");
...
public function fenPutsHisShittyFetishInYoSphinx():void {
	...
	if(player.lowerBody.cockSpot.count() > 0) outputText("  Pre-cum oozes from [eachCock] in steady strings that dangle onto the dirt, painting a swirled pattern beneath you as your prostate is pushed hard enough to squeeze trickles of your salty 'milk' from you.");
	...
	if(player.lowerBody.cockSpot.count() > 0) outputText("  Whenever her flare slides past your prostate, the wide, blunt head squeezes it hard enough to express a huge strand of pre.");
	...
	if(player.lowerBody.cockSpot.count() > 0) outputText(" and dragging you through your fresh, milky puddle");
...
public function sandMotherStuffGOA():void {
	...
	//*Encounter Repeat: Cowed
		...
		if(flags[FlagEnum.SAND_WITCH_LOOT_TAKEN] >= 5) outputText(", though right now it sits empty.  You'll need to wait for them to restock it");
	...
	//The Sand Mother gazes at you uncertainly as you return, her defeat at your hands still fresh in her memory.  The bleach-blond matriarch looks to have recovered from the fight, but she lamely queries, \"<i>Well, you've proven yourself mightier than my magic.  All that remains is a choice.  Do you show your true colors, pawn of demons, or do you tread the path of virtue?</i>\"  Sure, she puts on a brave face, but the nervous tics at the taut corners of her mouth reveal her worry. 
		...
		if(player.stats.cor >= 66) outputText("You stink of corruption, but I still sense a soul within you.  You are not a demon... yet.");
...
public function gigaFire():void {
	...
	if(player.stats.int > 40) outputText("  She's not even looking at you and seems steeled against lusty interruptions.  Perhaps you can hit her hard enough to shatter her concentration.");
...
public function talkDownTheMother():void {
	...
	if(player.stats.cor < 33) outputText("a champion");
	else if(player.stats.cor < 66) outputText("beholden to no-one");
...
public function tentacleGangBang():void {
	...
	if(flags[FlagEnum.TIMES_TENTACLED_SAND_MOTHER] == 0) outputText("it can't be like this!  W-we were going to free Mareth!");
	else outputText("not again!");
	outputText("</i>\"");
	
	if(flags[FlagEnum.TIMES_TENTACLED_SAND_MOTHER] == 0) outputText("\n\nThe dumb cunt still thinks you're working with the demons!");
...
public function loseToSandMotherBadEnd():void {
	...
	if(player.stats.cor < 66) outputText("aren't corrupt");
	else outputText("don't serve the demons");
	outputText(", looking up at her pleadingly ");
	if(player.lowerBody.cockSpot.count() == 1) outputText("while she gleefully pumps you, smearing your leaky juices all over your shaft.");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText(" and ignored womanhood");
	...
	if(flags[FlagEnum.CUM_WITCH_DEFEATED] > 0) outputText("The Cum Witch is here, and she seems to have recovered from her defeat!");
	...
	if(player.lowerBody.balls > 0) outputText("testes");
	...
	if(player.lowerBody.balls > 0) outputText("empty");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) outputText("innards");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) outputText("[butt]");
...
public function menLoseToQueenMotherVolI():void {
	...
	if(flags[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] == 0) outputText("Their harpy and phoenix allies allowed them to approach Lethice's fortress unhindered, and i");
	else outputText("I");
...
public function loseToSandMother():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 3) outputText("  \"<i>Of course we'll have to grow them a bit to suit our needs.</i>\"");
	//if breasts are larger than a normal sand witch's
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 14) outputText("  \"<i>Mmmm, and with such massive ones I'm sure we can expect you to produce more than most of our sisters could.</i>\"");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 5) outputText("jiggle");
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 10) outputText("bounce");
	...
	if(flags[FlagEnum.CUM_WITCH_DEFEATED] == 1) outputText("  It is obviously the Cum Witch you encountered, and she has clearly regained her strength!");
...
public function friendlySandMotherFuck():void {
	...
	if(player.lowerBody.cockSpot.count() > 2 || (player.lowerBody.cockSpot.count() == 2 && y < 0)) {
		...
		if(player.lowerBody.cockSpot.count() > 3 || (player.lowerBody.cockSpot.count() == 3 && y < 0)) outputText("es");
...
public function lesboMilkFilling():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 10) outputText(", [fullChest] swaying enticingly as you strike your seductive pose");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("cup your mammaries");
...
public function currentStateOfInterwebs():void {
	...
	if(flags[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] == 0) outputText("All that remains is to ready those still maturing and wait on our allies to finish growing their numbers.  We shall be carried into the Demon Queen's fortress on the wings of phoenixes, to fight as glorious a battle as has ever been fought!");
	...
	if(flags[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] > 0) outputText("  Of course, this is all contingent on us finding an ally that can breach the mountaintop citadel's walls.");
...
public function discussCumWitches():void {
	...
	if(player.stats.cor + player.stats.lib > 100) outputText(", though potentially fun");
...
public function getLactaidFromWitches():void {
	clearOutput();
	outputText("You ");
	if(player.stats.cor < 33) outputText("politely request");
	else if(player.stats.cor < 66) outputText("request");
...
public function getLaBova():void {
	...
	if(player.stats.cor < 33) outputText(" with a blush");
	else if(player.stats.cor < 66) outputText(" a little uncertainly");
	...
	outputText("\n\nYou shake your head");
	if(player.stats.cor > 66) outputText(", though the idea does seem to nestle into your imagination quite alluringly");
	outputText(".");
...
public function unfriendlyWitchToggle():void {
	...
	if(flags[FlagEnum.SAND_WITCH_LEAVE_ME_ALONE]  == 0) {
		...
		if(player.stats.cor < 50) outputText("  You probably wouldn't do something like that, at least not without provocation!/  You could see how she would think that.");
...
public function takeFertilePills():void {
	...
	//{Contraceptives}
		...
		if(!player.lowerBody.vaginaSpot.hasVagina()) outputText(", should you ever grow a vagina");
		outputText(".");
...
public function takeBarrenPills():void {
	...
	if(player.statusAffects.has("Contraceptives")) outputText("You're already under the effects of contraceptives.  Taking one of the brown pills wouldn't do anything.");
	//{TAKE DAT SHIT YO}
	else {
		...
		if(!player.lowerBody.vaginaSpot.hasVagina()) outputText("  Now if you ever re-grow a vagina, you should be fine.");
...
public function friendlyCumWitchBlessing():void {
	...
	else if(player.stats.cor < 66) outputText("Nervously, ");
...
public function cumWitchBlessed(virility:boolean = true):void {
	...
	if(player.tongueType > TONUGE.HUMAN) outputText("  Your inhuman tongue slides further and further out, following the witch's bulging, pre-cum filled urethra towards her balls where it can curl around them, embracing them with slick, slippery saliva.  You can taste her unique flavor on her skin, and it makes you shudder with arousal.");
...
public function fuckOneSandWitch():void {
	...
	if(player.lowerBody.balls > 0) outputText("  Your [balls] tense with the anticipation of releasing the fruit of your loins into her waiting womb.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 15) outputText("  The weight of your own tits bears heavily against your arms as you try to keep the motion of your hips constant while pulling on your lover's milk knobs.");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.lowerBody.balls > 0) outputText("  Your [sack] is soaking wet by the time your pussy finishes cumming.");
	}
...
public function fuckTwoPregWitches():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 15) outputText("slide between your cleavage");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 20) outputText("  Each thrust in to the eager cunts causes your breasts to bounce up and down.");
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 20) outputText("  Each thrust in to the eager cunts causes your massive breasts to sway back and forth, slapping against your own sides.");
	//{if player has six huge breasts}
	if(player.bRows() >= 3) {
		if(player.upperBody.chest.list[2].breastRating >= 20) outputText("  Your lower beach ball sized breasts are large enough to rest on the floor to either side as you fuck, and both witches eventually lower the legs they've had raised so as to hook them around your massive mammaries and hug your flesh even closer to their thighs.");
	}
	if(player.lowerBody.balls > 0) outputText("  The closer your shafts come to their climax the more tense your balls become, preparing to pump their loads through your cocks.");
	...
	if(player.lowerBody.cockSpot.count() >= 4) outputText("  Streams of cum drip from their abused anuses as you finish them off.");
...
public function fuckAllThePregWitches():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 3 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 10) outputText("  Your tits bounce as you enthusiastically grind away.");
	//{else if player has huge tits}
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 10 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 20) outputText("  The heavy weight of your huge tits causes them to sway with every thrust of your hips.");
	//{if player has really huge tits} 
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 20) outputText("  The immense swells of your mammaries press down upon the ass of the girl in front of you.");
	//{if player has four non-huge tits} 
	if(player.bRows() >= 2) {
		if(player.upperBody.chest.list[1].breastRating >= 3) outputText("  Your second row of breasts slap against the ass cheeks of the girl in front of you.");
	}
...
public function milkBathsAhoy():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(player.multiCockDescriptLight());
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("[vagina]");
...
public function grabTheMilkGirl():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 8) outputText(", pressing deep into your own huge rack, your nipples flicking across her own leaky tits");
	outputText(".");
	...
	if(player.stats.cor < 70) outputText("and you're quick to pull back, not wanting to force her, but to your surprise and delight, she reaches back and takes your hand in hers, moving you back to finger her");
	// if Corrupt> 70:
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.balls > 0) outputText(", her other hand cupping your [balls], rolling the " + num2Text(player.lowerBody.balls) + " orbs in her palm with surprising dexterity");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) outputText("  She cups your breasts, having to reach so far around both your rack and hers that she's straining her arms to rub your [nipples], but she does so valiantly, stroking them with her incredibly deft fingers.");
...
public function drinkNFap():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("[vagina] and idly stroke your puffy, lust-engorged vulva");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", but it does little to hide the swelling of [eachCock] - you have the milk for that");
	outputText(".");
	...
	if(player.gender == 3) {
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("  You make sure to fondle each of your members equally, caressing, squeezing, and stroking to the tempo of your swelling passion.");
