...
//	inDungeon = false;
	dungeonLoc = 0;
	clearOutput();
	Render.text("You leave the door behind and take off through the desert back towards camp.");
	doNext(camp.returnToCampUseOneHour);
}

public fightCumWitch():void {
	clearOutput();
	Render.text("A robed witch crests one of the dunes, her sable skin glistening with moisture in the unforgiving desert sun.  She spies you, and her dusky lips curl up in a smile while a white staff materializes in her hands.  Playfully, the woman calls, \"<i>I'm going to cast a spell on you...</i>\"");
...
public openZeDoorToParadize():void {
	...
	if(flags[FlagEnum.ENTERED_SANDWITCH_DUNGEON] == 0) {
		Render.text("You hesitantly touch the dark sphere, admiring its smooth, glossy finish.  Almost as soon as you come in contact with it, it recedes into the wall.  The doorway rumbles, a giant slab vanishing into the sandy depths, opening a portal to the inside.  Meticulous carvings inlaid with pearl depict large breasted witches in great quantity, and though the specific means of the glyphs are foreign to you, it's clear this place is some kind of sanctuary for sand witches.");
		flags[FlagEnum.ENTERED_SANDWITCH_DUNGEON] = 1;
	}
	//Repeat
	else {
		Render.text("Just ahead is the familiar sight of the sand witches' coven.  It's hewn from a sandstone archway buried in the side of a dune.  Pearl-inlays of big-breasted, lactating witches decorate the way inside, making it clear what you can expect to find inside.");
	}
...
public getMockedForSmallDongBySammitchMob():void {
	...
	if(player.cumQ() < 250) Render.text("  A little squirt of spooge dirties the witch's two fingers, creating a small web of spunk between them.  \"<i>Heh, knew it</i>\" the witch smugly says, giving you a pathetic look.");
	else if(player.cumQ() < 500) Render.text("  Spurts of sweet release shoot from your [cock smallest], glopping the bitch-witch's hand.  \"<i>Nothing too impressive</i>\" she snidely remarks as she rubs your spunk off on the dirt floor.");
	else Render.text("  Your [cock smallest] shoots out a barrage of cum onto the bitch's face, caking her disbelieving features into a thick layer of slimy goop as her fellow sisters laugh, pointing at her spunk-covered face.");
	...
	if(player.lowerBody.balls > 0) Render.text("  Your [balls] didn't have enough time to make another full batch.");
	Render.text("  It's very watery, dripping down between her toes. Your cock has a dirty smudge of where her foot was.");
	...
	if(player.isBiped()) Render.text("between your legs");
	else Render.text("to your side");
	Render.text(".  She takes your dirty cock in her hand.  She must have been dared to do something.  Looking at her peers with pleading eyes, she says, \"<i>Are you sure about this?</i>\"  A small group of witches egg her on, telling her to \"<i>do it.</i>\"");
	...
	//If balls:
	//[If balls & pussy:]
	if(player.lowerBody.balls > 0 && player.lowerBody.vaginaSpot.hasVagina()) Render.text("  She even cups your [balls] but moves to touch your [vagina] when she feels its wetness dripping off the testes."); 
	else if(player.lowerBody.balls > 0) Render.text("  She even cups your tender [balls] and lightly massages them.");
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  She even lightly caresses the lips of your pussy.");
...
public femaleGirlsLoseToSammitches():void {
	...
	if(player.hasMuzzle()) Render.text("  She fucks your muzzle like a cock as it goes into one cunt while she plays with the other one.");
	else Render.text("  She grinds her dampening pussies against your lips while her clits are pressed up against your nose.");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("Amazingly, as the oil spreads over your crotch, the flesh splits, revealing a small, wet entrance with a little clit.");
		player.createVagina();
		player.lowerBody.vaginaSpot.list[0].clitLength = 0.25;
	}
	Render.text("Their warm streams run all over your form: down your [chest], off your [hips] and teasingly around your [vagina].  At this point, you want it so badly that you squeeze your chest, pinch your nipples and stroke your [clit].");
	...
	if(!player.hasMuzzle()) Render.text("grinding on your face");
	else Render.text("fucking your muzzle while she plays with herself");
	Render.text(" and the four others whining out their enjoyment, making your own pussy feel neglected and needing something to fill it.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) Render.text("between");
	else Render.text("on");
	Render.text(" your [chest].");
	
	Render.text("\n\n\"<i>Hey girls, hurry up and pick a cock already!  The ");
	if(flags[FlagEnum.SAND_WITCHES_FRIENDLY] > 0) Render.text("slut");
	else Render.text("initiate");
	Render.text(" is getting a bit randy!</i>\"");
	
	if(flags[FlagEnum.SAND_WITCHES_FRIENDLY] > 0) Render.text("\n\nYou would think more on that word, but you're too focused on the cock toy on your chest. You look at it with lust in your eyes and beg its owner to fuck you with it, make you cum with it, to make you her bitch.");
	...
	//[Next]
	menu();
	if(flags[FlagEnum.SAND_WITCHES_FRIENDLY] > 0) addButton(0,"Next",sandWitchMobNotBadEnd);
...
public sandWitchMobNotBadEnd():void {
	clearOutput();
	Render.text("You wake up hours later outside the dungeon with all your belongings, surprisingly not missing any gems.");
	dungeonLoc = 0; //Replaces	inDungeon = false;
	player.orgasm();
	dynStats("lib", 1, "sen", -1);
	cleanupAfterCombat();
...
public memeberedFolksFindTrueWuv():void {
	...
	if(player.lowerBody.balls > 0) Render.text(" while your [balls] start to work, clenching and relaxing as they brew a thick load of fuckbatter thanks to the chemical jump-start.");
	else Render.text(" while your body starts to work, clenching and relaxing as it rushes to brew a thick load of fuckbatter thanks to the chemical jump-start.");
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		Render.text("  Similarly textured cunnies playfully snake around your other phallus");
		if(player.lowerBody.cockSpot.count() > 2) Render.text("es");
		Render.text(", gleefully slurping up your twitching hardness inside ");
		if(player.lowerBody.cockSpot.count() == 2) Render.text("itself");
		else Render.text("themselves");
		Render.text(".");
	}
	Render.text("Slurping noisily, the cunt");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s piston");
	else Render.text(" piston");
	Render.text(" and bob, splattering ");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("their");
	else Render.text("its");
	Render.text(" dripping saliva all over your exposed crotch.  Your [hips] lurch up to meet the hungry tentacles.  Feeding the creature's cum-lust is something your body seems driven to do at this point, and you gurgle in wordless delight at her ministrations.");
...
public yoYouBeatUpSomeSandWitchesYOUMONSTER():void {
	...
	Render.text("The sand witches all collapse to the floor in a vast puddle of milk and pussy juice, ");
	if(monster.HP < 1) Render.text("nursing their wounds");
	else Render.text("frantically making out with each other, huddling into a squirming orgy on the floor");
	Render.text(".  You stand alone, the sole victor, triumphant against your enemies; you can easily continue deeper into the caves, now.");
	...
	if(player.lust >= 33) {
		...
		if(player.lowerBody.cockSpot.hasCock()) addButton(0,"Fuck One",dicksHaveSexWithAWitch);
		if(player.lowerBody.vaginaSpot.hasVagina()) addButton(1,"Forced Lick",forceCunnilingusRimjobClitAndNipple);
	}
...
public dicksHaveSexWithAWitch():void {
	...
	if(monster.lust > 99) Render.text("masturbating companions");
	else Render.text("groaning companions");
	...
	if(monster.lust > 99) Render.text("  Judging by the omnipresent flush on her cheeks and the scent of female honey inundating the air, she's more than ready to help you with your problem.");
	else Render.text("  Her cheekbones slowly color at the thought, turning almost entirely beet-red.  At the same time, her nipples seem to tighten beneath her robes, signalling her body's acceptance of your intent.");
	...
	Render.text("\n\n[EachCock] rises up ");
	if(player.tallness < 60) Render.text("through her buttcheeks");
	else Render.text("along her arched spine");
	Render.text(", fully engorged and ready for action.  For now, you're content to continue to molest your chosen fuck-toy, dipping a pair of fingers into her ");
	if(monster.lust > 99) Render.text("well-soaked");
	else Render.text("rapidly-moistening");
	Render.text(" cunts in order to make sure she's prepared.  The witch bites her lip to stifle a lewd moan, so you drag the pads of your fingers across each of her clits simultaneously.  This shatters whatever restraint she was showing into a little more than slick cunt-juice.  Even now, it's dripping down her inner thighs.  She's properly moaning with your strokes now, and you judge she's ready.");
	...
	if(player.smallestCockArea() <= monster.vaginalCapacity()) {
		...
		if(y >= 0) Render.text(" and your " + cockDescript(y) + " aimed at her other one");
		Render.text(".  Her knees buckle, but you're ready for it, and she slides straight onto your supporting shaft");
		if(y >= 0) Render.text("s");
		Render.text(", impaling herself to the hilt.");
		...
		if(y < 0) Render.text("pussy clings tight to you");
		else Render.text("pussies cling tightly to you");
		Render.text(".  Supported entirely by your body, the sand witch's muscles go slack (aside from the ones hugging your dick");
		if(y >= 0) Render.text("s");
		Render.text(").  She's breathing nice and heavy as you lift her and swing your hips back, pulling most of your " + cockDescript(x));
		if(y >= 0) Render.text(" and " + cockDescript(y));
		Render.text(" out before reversing the motion nigh-instantly, slamming yourself in to the hilt.  The once proud enchantress is reduced to a quivering puddle of fuck by your jackhammering thrusts, so taken by the sensations in her quim");
		if(y >= 0) Render.text("s");
		Render.text(" that her brains are practically drooling out of her cunts.");
		...
		if(y < 0) Render.text(cockDescript(x));
		else Render.text("double dongs");
		Render.text(".  Waves of rippling contractions sheath you in shimmering, ephemeral ecstasy.  Like something clicking into place in your brain, a biological prerogative asserts itself, unleashing a heat that boils up out of your core like lava from a volcano.  [EachCock] spasms and releases a flow of salty jism.  The witch must feel it, because as soon as your seed makes contact with her juicy cunt");
		if(y >= 0) Render.text("s");
		Render.text(", her clingy lips seem to tighten around your girth to seal it in.");
		if(player.cumQ() >= 400) {
			Render.text("  That fails to contain the totality of your virile offshoots, and ribbons of the stuff squirt out the sides of her lips.");
			if(player.cumQ() < 2000) Render.text("  You do manage to gift her with a nice little cum-paunch for her efforts, though.");
			else Render.text("  You're cumming so fast that that venting doesn't help her much.  Slowly, her belly begins to bulge outward.  It stretches and wobbles as it fills with spunk, not stopping until she looks months pregnant.  The liquid way that it jiggles with every movement will reveal the contents of her middle to any who dares look at her.");
		}
		Render.text("\n\nSetting her down in front of her sisters, you let her slide off your ");
		if(y >= 0) Render.text("twin cocks");
		else Render.text(cockDescript(x));
		Render.text(" onto the floor as an example of just what you're capable of.  You can beat the best they have and fuck them into a twitching puddle in the dirt.  Though the mob seems cowed by your display of authority, they're still fucking themselves to the sight of what you've done to their sister.  You make the rest of them take turns licking your spent spunk and caked-on cunt-juice from your phallus");
		if(y >= 0) Render.text("es");
		Render.text(" before getting dressed and getting back to business.");
	}
	//Too Big For Fucks
	else {
		...
		if(player.cumQ() >= 500) Render.text("struggling to swallow it");
		else Render.text("swallowing it");
		Render.text(".  She squeezes her arms together, which pushes her boobs tighter around your orgasmically pulsating length.  It feels so good that it feels like a whole new orgasm kicks off in your [balls], even though you've already started to cum!  ");
		if(player.cumQ() < 250) Render.text("The cute woman swallows every single drop.  She even gives up a satisfied moan when she finishes.");
		else if(player.cumQ() < 1000) Render.text("The cute woman's cheeks balloon as she tries to contain it all.  She swallows admirably in spite of your heavy jizz-flow, only losing a few white trickles out the corners of her stretched mouth.");
		else Render.text("The cute woman's cheeks flood with your obscene cum-flow.  She tries to swallow it down, but no matter how fast her fevered gulps are, the stuff still floods in with such force that it squirts from the corners of her mouth.  She just gives up after a few moments and lets it run down her chin in a waterfall.  Most of the jism puddles on the floor below her, soaking into her ponytail and matting it with salty spunk.");
...
public forceCunnilingusRimjobClitAndNipple():void {
	clearOutput();
	Render.text("You eye the defeated mob");
	if(player.HP < 1) Render.text(" through their torn-up robes, noting many large, dripping breasts and more than a few pairs of supple cunt-lips");
	else Render.text(" through their squirming, orgiastic motions, noting their many large, dripping breasts and gushing multi-cunts");
	Render.text(".  They may ");
	if(player.stats.cor < 33) Render.text("be misguided in fighting you");
	else if(player.stats.cor < 66) Render.text("be a little over-zealous in defending their home");
	else Render.text("be crappy fighters");
	Render.text(", but feeling a stirring in your nethers, you can think of at least one good use for them.  You start to strip out of your [armor], groping your butt with one hand; maybe there's a second purpose they can serve...");
	...
	if(player.wetness() >= 4) Render.text(", spraying your musky girl-goo across the pretty faces at your groin, soaking them all the way to their first rows of tits.  Your juices mix with the milk dripping from their nipples into a sticky alabaster treat that makes you lick your lips and shudder in delight. The seductress on your [vagina] gurgles and sighs as she tries to keep up with your copious lady-spunk, but the way you squirt, she seems doomed to failure.");
	else if(player.wetness() >= 2) Render.text(", sluicing secretions down the lips of your would-be pussy-slave, forcing her to work her throat in order to try and keep up with it.  You're so goddamned wet, and the way your channel is clenching and undulating around her tongue just forces out more of your juice.");
	else Render.text(", dribbling your slippery girl-cum straight into your would-be pussy-slave, forcing her to swallow your tangy flavor.  It's hard not to turn into a dripping mess with the way her tongue works your spasming pussy, but neither of you seems to mind.");
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("  Above, [eachCock] fires lances of proper, virile spunk onto the girls' exposed backs to properly mark them.");
		if(player.cumQ() >= 500) {
			Render.text("  It turns their bronzed skin into a soupy white, then dangles to the ground in thick ropes");
			if(player.cumQ() >= 1500) Render.text(", each of which begins pooling into a puddle");
			if(player.cumQ() >= 2500) Render.text(".  Those puddles combine to form a lake of jism beneath these lusty slaves bodies, putting their combined milky tits to shame");
			Render.text(".");
		}
	}
	...
	if(player.lactationQ() >= 200) Render.text(" drink their fill");
	if(player.lactationQ() >= 1000) Render.text(", or at least as much as they can handle of your lactic capacity,");
	else if(player.lactationQ() < 200) Render.text(" slowly suckle you");
	Render.text(" while you come down from your high.  Why couldn't they have just done this to start?");
...
public gangrush():void {
	Render.text("The witches close ranks and advance with raised fists, intent on beating you into submission!\n");
...
public headbuttABitch():void {
	Render.text("The crowd parts, and a stockier, sturdier sorceress ambles out, fists up and head cocked back.  She makes to punch at you before pulling her fist at the last second, snapping her head forward in a powerful headbutt!  You barely have time to react!");
...
public sandstonesAreCool():void {
	Render.text("The sandstorm whirling around the room suddenly ceases, and all the tiny sand particles gather together into balls, growing into several smooth stones.  Then, all the sand stones fall to the ground and slither towards you.");
	...
	if(!combatMiss() && !combatEvade() && !combatFlexibility() && !combatMisdirect()) {
		...
		if(!player.isGoo()) {
			...
			//If PC has 1 cock:
			if(player.lowerBody.cockSpot.count() == 1) Render.text("  A stone crawls onto your [cock].");
			//[If PC has multi-cocks: 
			if(player.lowerBody.cockSpot.count() > 1) Render.text("  A bunch of the stones crawl onto your " + player.multiCockDescriptLight() + ".");
			if(player.lowerBody.cockSpot.hasCock()) bonus++;
			//[If PC has cunt]
			if(player.lowerBody.vaginaSpot.hasVagina()) {
				Render.text("  One stone slides up your inner thigh");
				if(player.lowerBody.balls > 0) Render.text(" behind your [sack]");
				Render.text(" and pops itself right into your [vagina]");
				if(player.hasVirginVagina()) Render.text(", robbing you of your virginity as a trickle of blood runs down your [leg].");
				bonus++;
			}
			//[If PC has balls:
			if(player.lowerBody.balls > 0) {
				Render.text("  A small set of stones settle on your [balls].");
				bonus++;
			}
			Render.text("  " + num2Text(player.upperBody.chest.countNipples()) + " crawl up to your chest and over your top " + nippleDescript(0) + "s");
			if(player.bRows() > 1) {
				if(player.bRows() == 2) Render.text(" and");
				else Render.text(",");
				Render.text(" your middle " +  nippleDescript(1) + "s");
				bonus++;
			}
			if(player.bRows() > 2) {
				Render.text(", and your bottom " + nippleDescript(2) + "s");
				bonus++;
			}
			Render.text(".");
			Render.text("  The last stone travels up the back of your [legs] and slides right into your [asshole].");
...
public bukkakeAttack():void {
	...
	Render.text("The Cum Witch moans and daintily peels her robes away from her swollen cock-flesh.  A bubble of precum pops wetly from her urethra to splatter on the floor as her balls suddenly swell.  You look back up in time to see the telltale glow of magic surrounding her staff, but then she's thrusting her hips at you, lewdly humping the air as she unleashes rope after thick rope of potent jism in your direction!\n");
...
public genderConfusionAttack():void {
	...
	Render.text("Touching her alabaster staff to her brow, just under the brim of her hat, the Cum Witch makes a brief incantation and fixes you with her gaze.  Her eyes flash blindingly white, and then you feel her inside you, rifling through your memories, digging up memories of your childhood, your past, and throwing them against you.  ");
	if(player.stats.int/5 + rand(20) + player.level/2 < 18) {
		Render.text("She batters your consciousness with conflicting memories of your gender, utterly dazing you.  How can you fight when you can barely tell who you are anymore?");
		player.statusAffects.add(new StatusAffect("Confusion",0,0,0,0)));
	}
	else {
		Render.text("You parse the flood of information with mental focus and expel the intruder from your mind with a clenching of your sizable intellect.");
	}
	combatRoundOver();
}
//*Attack: Shell
...
public cocknosisAttack():void {
	Render.text("Lifting her robes enticingly, the Cum Witch reveals her ");
	if(monster.lust < 50) Render.text("half-hard");
	else if(monster.lust < 70) Render.text("hard");
	else if(monster.lust < 85) Render.text("throbbing");
	else Render.text("hard, dripping");
	Render.text(" cock.  She gently begins to sway her hips, bouncing back and forth with near-mechanical precision, her softly bobbing cock catching your eyes with its metronome-like precision.  She softly begins to speak, enunciating each word to the time and tune of her movements.");
	...
	if(player.stats.int/20 + rand(20) >= 13) {
		Render.text("You chuckle at her crude attempt to hypnotize you with her member.  She stomps her foot in irritation and drops her robes back into place.");
	}
	else {
		Render.text("The Witch smirks, though you're too focused on her cock to see, and she continues, \"<i>Good " + player.mf("boy","girl") + ".  Watch it sway.  You're so focused on my cock now that the world is just falling away around it, sinking into nothingness, leaving only that wonderful cock behind for you to watch.  And since it's filling your view, you can feel it filling your mind as well, can't you?</i>\"");
...
public turnIntoASammitch():void {
	clearOutput();
	Render.text("Helplessly, you ");
	if(player.lust > 99) {
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("dig your hands into your loins, frigging your hungry cunt so hard your juices splatter the Witch's robe.");
		else Render.text("dig your finger into your [asshole], frigging it like your life depended on it.");
	}
	else Render.text("try to rise, but all you manage to do is slip back down to the ground.");
	Render.text("  The Cum Sorceress smiles and releases her staff.  Instead of clattering to the ground, it dematerializes a piece at a time, fading away in a way that would be mesmerizing were you not otherwise occupied.  She hikes her robes up and pulls off her hat with them, shaking her surprisingly light, blonde hair free.  \"<i>Now, all that got me pretty worked up, and I don't think the girls will mind if I give you your first sampling of cum witchery.  What do you think, hun?</i>\" she asks, glancing back towards the comatose witch behind her.");
	...
	if(player.pregnancyType == 0) Render.text("let's go ahead and get your first daughter inside you, shall we?");
	else Render.text("let's go ahead and have some fun, shall we?  We'll have to wait until you pop out whatever's in your womb before I give you your first witch child.");
	Render.text("  There will be plenty of time to make you one of us while we're waiting.</i>\"");
	...
	if(monster.lust < 65) Render.text(" herself to full hardness, smiling when thick strings of cum begin to drizzle from her swollen cock-tip.");
	else if(monster.lust < 85) Render.text(" herself until cum begins to drizzle from her swollen cock-tip.");
	else Render.text(" herself, gathering up the constantly-dripping cum and smearing it all over her shaft.");
	...
	if(player.lust > 99) Render.text(", even though your hand is knuckle-deep in your twat");
	Render.text(".");
	...
	if(player.bRows() < 2) Render.text("get four big, milky tits");
	else if(player.lactationQ() < 200) Render.text("have your tits made into perfect milkers");
	else Render.text("get your four, big tits milked");
	Render.text(" or to get your first proper administration of witch-cum.");
...
public cumWitchCumPumpBadEnd():void {
	clearOutput();
	Render.text("Laid low by ");
	if(player.HP < 1) Render.text("your wounds");
	else Render.text("by the lust coursing through your veins");
	Render.text(", you slump over against a desk, leaning heavily on it for support while ");
	if(player.HP < 1) Render.text("you struggle to rise");
	else Render.text("you struggle to get your crotch");
	Render.text(".");
	...
	if(player.cumQ() > 1000) {
		if(player.lowerBody.balls > 0) Render.text(" and such virile, cum-swollen testes.  An excellent breeder for sure!");
		else Render.text(" and such a virile body!  An excellent breeder for sure!");
	}
	else Render.text("but with such a pathetic virility.");
	Render.text("  A little magic later, and you'll be quite suited to breeding daughters with me.  Perhaps if you do well enough, I'll allow you to seed MY womb.</i>\"");
	...
	if(player.HP < 1) Render.text("grunt and strain in response, still trying to fight, even though you know it's hopeless.");
	else Render.text("groan and touch yourself, trying to show her just how 'in the mood' you are.");
	Render.text("  She affectionately ruffles your " + hairDescript() + " and tuts, \"<i>Now, now, it's cute that you think you still have some agency here, but you really ought to just lay back and let me take worry about it!</i>\"");
...
public beACumPumpPartII():void {
	...
	Render.text(".  Of course, the wicked witch only releases you at this point.  \"<i>Do I look like one of the cum-hungry wenches around here?</i>\"");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("es");
	Render.text(" wilting from the force of your surprise and fear.");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(", \"<i>I did not think you would get this far.  Your will is truly mighty, [name], but the task I gave you is something I've never been able to do.  Ten is as far as I ever got.  I would not have you damage yourself just yet.</i>  She pulls open the curtain and bellows, \"<i>Next!</i>\"");
	...
	if(player.smallestCockArea() >= 100) Render.text("  The penetration is eased somewhat by your shrinking dick.  You glance at the sand witch in awe, noting her glowing hands.  She gives you a knowing wink and turns, her spell complete, leaving you to enjoy yourself.");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("Her voluminous butt is thoroughly smeared by the goo from the rest of your " + player.multiCockDescriptLight() + ", shining with white from your magical virility.  ");
	Render.text("She rides you with surprising skill for one who seems to be so hesitant about such a healthy jizz output, her four breasts squishing tight against [chest].  Her tight cunny squeezes and ripples around you, rapidly growing soaked with white, thoroughly spunk-lubed and hungry for more.");
...
public cumPumpVolumeThree():void {
	clearOutput();
	Render.text("With the champion brainwashed into a little more than a loyal cum-pump for the sand witches, they grow in number and strength with alarming speed.  In the space of eight years, the desert is transformed into a verdant forest.  For better or for worse, the witches finally rival the demons in power.  They spread their influence wider, eventually ");
	if(player.statusAffects.has("FactoryOverload")) Render.text("destroying");
	else Render.text("rescuing");
	Render.text(" Marae.  Their abilities, numbers, and familiarity at dealing with demons lend them great success at battling Lethice's growing hordes, and within the span of another decade, the demons are exterminated.  The witches are hailed as saviors, and young females of all races seek to join them.  You, of course, inseminate all of them.");
	gameOver();
}
...
public repeatLoseToCumWitchForDudes():void {
	...
	if(player.HP < 1) {
		...
		if(player.lust < 40) Render.text("flaccid");
		else if(player.lust < 60) Render.text("semi-flaccid");
		else if(player.lust < 70) Render.text("semi-hard");
		else Render.text("turgid");
		Render.text(" weight.  Pumping her hand with deft strokes and watching your expression intently, the curvy woman works you into a pleasant, erotic warmth, excitement coursing through your body with such fervor that your remaining wounds seem insignificant in comparison.");
	}
	//Lust:
	else {
		Render.text("Dropping down on your [legs], you rip off your [armor] and flop onto your back so that you can focus on abusing your genitalia.  [EachCock] is already hard enough to be leaking pre");
		if(player.lowerBody.balls > 0) Render.text(", and your [balls] are practically quaking with need");
		Render.text(".  The ebony futanari laughs at you as she confidently strides forward and discards her hat.  \"<i>Did you really have any intention of fighting me, or did you ask the Sand Mother to send us out so that we could abuse your insatiable libido?  I don't know how you didn't wind up captured, but I suppose I can tend to your needs... this time.</i>\"");
	...
	if(player.cockThatFits(monster.vaginalCapacity()) < 0) {
		...
		if(player.biggestCockLength() < player.tallness/2) Render.text("[chest]");
		else if(player.biggestCockLength() < player.tallness/1.6) Render.text(player.face());
		else Render.text(hairDescript());
		Render.text(".  The black spellcaster comments, \"<i>I must admit it is fun to look at, but it doesn't look like it's ready to blow just yet.  How about a little encouragement?</i>\"");
		...
		Render.text("your [nipples]");
		if(player.upperBody.chest.hasFuckableNipples()) Render.text(", sinking inside your moist tit-holes almost immediately");
		Render.text(".  With two vibrators savaging your [nipples] and the black-hued beauty riding your cock like her own personal rocket, you can barely contain yourself.  Your back arches, and you try to buck your hips, to fuck her, her cock, whatever, but you're still too exhausted from the fight to shift her body weight.  You can do nothing but lie there while she uses you, humping your cock, her ebony length dragging its lurid pleasure across your " + cockDescript(x) + " while you wriggle and writhe ecstatically.");
		...
		if(player.lowerBody.balls > 0) {
			Render.text("[balls], rolling the ");
			if(player.lowerBody.ballSize < 2) Render.text("petite ");
			else if(player.lowerBody.ballSize >= 5) Render.text("weighty ");
			Render.text("orbs across her soles.");
		}
		else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("[vagina], slipping a few toes through your folds while a big toe diddles your [clit].");
		else Render.text("taint, softly stroking the sensitive skin between your groin and [asshole].");
		...
		Render.text("loud and firm enough for your dazzled mind to cogitate her words: \"<i>Oh, my pretty bitch " + player.mf("boy","girl") + ", feel the pressure building up inside you.  Higher and higher now... It's almost too much, isn't it?  You don't need to answer, just feel it grow thicker and hotter.  You're going to cum like a geyser, because of me, and you're going to love it.  You'll always want to cum for me, won't you?</i>\"");
		...
		if(player.lowerBody.cockSpot.count() > 1) { 
			Render.text(" while your extra erection");
			if(player.lowerBody.cockSpot.count() > 1) Render.text("s spurt");
			else Render.text(" spurt");
			Render.text(" some cream up the ebony arch of your mistress's back");
		}
		else Render.text(" from her bouncing, ebony erection");
		Render.text(".  At the same time, her clenching, pussy climaxes around you while you fill it.  You're spraying ropes of spooge into her so fast that it's backwashing out around your " + cockDescript(x) + ", the veins pumping almost as hard as your overactive reproductive system.");
...
public tooBigCumWitchLossFacial():void {
	clearOutput();
	Render.text("Ruefully, you tell her that you don't mind a little cum in exchange for having your own abilities enhanced.  The knowing grin that spreads across her face makes it seem like she knew the result was a foregone conclusion.");
	if(player.stats.cor < 33) Render.text("  You shudder at the knowledge of what's going to happen to you.  Just why did you agree to this?");
	else if(player.stats.cor < 66) Render.text("  You catch yourself licking your lips as the knowledge of what is about to happen to you sinks in. Despite hastily stopping yourself, the Cum Witch still saw, and she smiles.");
	else Render.text("  You give her a lewd look and lick your tongue across your lips as you anticipate the big, fat load she's going to feed you, hungry for wet, decadent pleasure regardless of inhibitions.");
	
	Render.text("\n\nThe hot moisture she's secreting all over your " + cockDescript(x) + " seems to be affecting you as well as the vibrations ");
	if(player.upperBody.chest.hasFuckableNipples()) Render.text("in");
	else Render.text("on");
	Render.text(" your [nipples].  It feels like there's so much blood being forced inside your erection that the sheer over-tumescence will make you burst.  The need is overwhelming.  You HAVE to cum, and you're so wet, so stained with spunk and juice that there's no time like now.  The Cum Witch's heels press in on your ");
	if(player.lowerBody.balls > 0) Render.text("[balls], compressing them slightly as a tingle of magic lances into you, fattening them under her ministrations");
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("[vagina], digging into the slippery slit as a tingle of magic spreads through it and into your core, thickening some organ inside you");
	else Render.text("taint, rubbing it as a tingle of electric need phases through your " + player.skinFurScales() + " to assault your swelling prostate and seminal vesicles");
	Render.text(", and in that moment, you lose all ability to hold out.  You're cumming, and you're cumming now.");
	...
	if(player.biggestCockLength() >= player.tallness/1.6) Render.text("  The Cum Witch smiles as she holds it, bending it to make sure the overlong shaft deposits its thick load on your " + player.face() + " where it belongs.");
	Render.text("  Your captor is still humping it even as you cum, moaning as the bulges of copious cream press on her smaller boner.  Her visage is alight with ecstasy, and her heavy nipples are dragging on your [chest] as she begins to grunt, her motions going jerky in her ecstasy.");
...
public tooBigCumWitchLossNoFacial():void {
	...
	if(player.lowerBody.balls > 0) Render.text(", just like your own");
	Render.text(", and she smiles cruelly as her feet press harder on you, suddenly tingling with supernatural forces.  Almost painful heat floods through your middle as she frots you harder and harder, inadvertently tit-fucking your immense erection all the way to orgasm.");
...
public TDMsLoseToCumWitchScene():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		//if (cocks = 2)
		if(player.lowerBody.cockSpot.count() == 2) Render.text("your other phallus.");
		//[if (cocks > 2)
		else Render.text("the rest of your " + player.multiCockDescriptLight() + ".");
		Render.text("  The futa on top of you continues to smile mysteriously, and spares not another moment of her attention on your genitals.");
	}
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("one of ");
	Render.text("yours...");
	...
	if(player.lowerBody.balls > 0) Render.text("the one part of your manhood that you can still feel, your [sack].");
	else Render.text("a part of your body just behind and above the base of [eachCock], probably your prostate.");
	...
	if(silly()) Render.text("in both body and in [balls].");
	else Render.text("in both body, and in spirit.");
...
public cumWitchDefeated():void {
	...
	//Female Victory Sex
	if(player.lowerBody.vaginaSpot.hasVagina()) addButton(3,"Ladysex",ladyVictorySex);
...
public declineSandWitch():void {
	clearOutput();
	Render.text("Dusting yourself off, you lower your [weapon] and leave the cum witch to recover from the humiliation of losing to you.  The haunted, hungry look in her eyes leaves little doubt that she'll challenge you again or that she still wants to fuck you.  For now, she slips down into her own puddled cum, idly touching herself.");
	if (inCombat)
		cleanupAfterCombat();
...
public menFuckUpSomeCumWitch():void {
	...
	let x: number = player.cockThatFits(monster.vaginalCapacity());
	let y: number = player.cockThatFits2(monster.vaginalCapacity());
	Render.text("You shuck out of your [armor] in eager anticipation, [eachCock] aleady ");
	if(player.lust < 50) Render.text("half-hard");
	else Render.text("rock-hard");
	Render.text(" and pulsing with growing readiness.  The cum witch looks up at you with disdain, but the rigidity of her shaft and sloppy wetness of her flushed quim leave no doubt as to her state.   Her body blushes honestly as you reposition her, spreading her legs nice and wide.  You have to lift her hefty sack to expose the thick, wet lips of her pussy.  Jumping in response, her fat cock oozes a trickle of precum onto her dusky belly, oiling her dusky skin into a sensual shine.");
	...
	if(player.cockThatFits(monster.vaginalCapacity()) < 0) Render.text("most of");
	else Render.text("all of the way");
	Render.text(" inside her, sheathed deep in her under-used twat.  Her tight passage fits around your " + cockDescript(x) + " like a custom-made glove, a slippery warm embrace that threatens to rob you of your very reason.  The witch begins pumping her ebony cock along with the motions of your hips, throwing her head back in wordless pleasure that only a true hermaphrodite can experience.");
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Two can play at that game.");
	//Herm sexback
	if(player.gender == 3) {
		...
		Render.text("\n\n");
		if(player.lowerBody.balls > 0) Render.text("Your sweaty nutsacks slide over each other while you fuck, compressing together into a mass of sweaty orbs twitching against each other in a competition to disgorge their frothy load before the other.");
		else Render.text("Her sweaty nutsack glides over your " + player.skinFurScales() + " as you fuck, compressing and twitching against you as it begs to discharge its frothy load.");
		...
		if(player.cumQ() > 2000) Render.text("  Of course, you outlast even the jizz-witch's magics, plugging her so full of semen that her belly rounds into a gravid orb while yours remains only slightly bloated.");
		else if(player.cumQ() > 1000) Render.text("  Of course, you both manage to bulge each other's bellies with sexual discharge.");
		else Render.text("  Of course, she cums far longer and harder than you, plugging your belly full of semen until it has a slightly fecund curve to it.");
	...
	//DP Males
		...
		if(player.cumQ() < 250) Render.text("pours out of your dual cum-slits, making a sloppy mess of both holes.");
		else if(player.cumQ() < 500) Render.text("gushes out of your dual cum-slits, soaking both holes in and out with pearly white.");
		else if(player.cumQ() < 1000) Render.text("gushes out with incredible force from your dual cum-slits, quickly plugging both holes so full of seed that the witch's belly slowly rises, cum-inflated.");
		else Render.text("washes out of your dual cum-slits in a tidal wave, flooding the hungry holes with your virile seed.  You cum and cum, spunking up the witch's belly.  First it bulges slightly, but all too soon you have it rounded into a pregnant dome.");
		Render.text("  The bouncing girl-cock above the woman's gushing pussy does a fine job of painting her belly and bust white.  It spurts rope after rope onto her chocolate-toned flesh, and soon she looks more like an icing-drizzled tart than a formidable foe.");
	...
	//Regular Dicking
		...
		//(silly:) Render.text(", and I don't want it.  I think it's messy");
		Render.text(".</i>\"");
		...
		if(player.stats.cor < 50) Render.text("and a hint of compassion");
		else Render.text("and finally deign to let her have some pleasure");
		Render.text(".  You gradually increase the tempo of your lovemaking and let your hand lazily drift lower, grabbing the witch by her soft, supple thigh.  Her skin is warm and flushed (or as flushed as ebony skin can get), and the higher your fingers reach, the damper it gets.  Gently, you prod around her lips and your own maleness to find the hardness of clit.");
		...
		if(player.cumQ() < 700) Render.text(", spurting with unceasing quantity until she's absolutely flooded with jizz and a little bulgy around the middle.");
		else Render.text(", spooging up her cunny until it's completely flooded, her belly bloated into a rounded dome.");
		if(player.cumQ() >= 1500) Render.text("  You keep going beyond that, but there's just no room, and spouts of semen spurt from the woman's poor, over-filled twat.");
		
		Render.text("\n\nThe witch sighs and idly rubs her ");
		if(player.cumQ() >= 700) Render.text("swollen ");
		Render.text("tummy, smearing her own wasted goo into her skin as you withdraw and re-dress.");
		if(player.stats.cor < 33) Render.text("  You hope she learned something from the experience, but knowing most people in this strange land, she'll go right back to her rotten ways.");
		else if(player.stats.cor < 66) Render.text("  You wonder if she'll learn anything from this and shrug nonchalantly.  Who cares?");
		else Render.text("  You smirk as you wonder if she'll learn anything from this.  You hope not - it's a fun lesson to teach.");
		player.orgasm();
	}
	if (inDungeon) {
		if (inCombat)
...
public ladyVictorySex():void {
	clearOutput();
	Render.text("You disrobe, casting aside the garments with a feminine sigh.  As soon as you expose yourself, the witch's eyes twinkle happily.  She caresses her stiff tool and tweaks one of her nipples as she watches you.  \"<i>If you wanted to get fucked by little old me, all you had to do was bend over, honey.</i>\"");
	...
	if(player.wetness() >= 5) Render.text("  It gushes out of you in a soaking river, utterly drenching the futa's stomach, hips, and butt.");
	if(player.lactationQ() >= 50) Render.text("  At the same time, milk spouts from your bosom to wash over your hermaphrodite.");
...
public tentacleVictoryGangbangCumWitch():void {
	...
	if(player.tentacleCocks() >= 4) Render.text(", splatting over your frotting cock-tip in wild abandon");
	...
	Render.text("  You shudder and grab the ebony sorceress's tits in your hands, squeezing her bronzed orbs tightly as you release works through you.  Jets of cum well up to plug the witch's pussy and anus simultaneously.  The spit-soaked, cum-splattered vine between her tits quivers meaningfully, and then unloads, squirting a thick wave of jizz onto her face.");
	if(player.tentacleCocks() >= 4) Render.text("  Of course, your fourth prick is not to be outdone, and blasts a gooey coating of cream across the witch's own pulsating prick, wreathing it in drizzles of syrupy white.");
	if(player.cumQ() >= 500) Render.text("  Wave after wave of spunk washes into the tight holes and toned form of your foe, turning her into a complete mess.");
	if(player.cumQ() >= 1000) Render.text("  A few more virile splurts see to her utterly inundated state, bloating her belly into a tightly-stretched dome.");
...
public savinMakesAwesomeFemdom():void {
	...
	//if Multiboob:
	if(player.bRows() > 1) {
		Render.text("She caresses each of your breasts, cupping each in turn, running her thumb over each nipple");
		if(player.lactationQ() >= 200) Render.text(" until milk streams down your chest, much to her delight");
		Render.text(".");
	}
	else if(player.lactationQ() >= 200) Render.text("Her fingers deftly work across your [nipples], caressing out a trickle of milk, which she laps up with a long, languid motion that electrifies your sun-kissed skin.");
	else Render.text("She tweaks your [nipples] between her dexterous fingers, sending shivers of pleasure up your spine, but seems oddly discontent...  \"<i>No milk for me? We'll have to do something about that...</i>\"");
	Render.text("  Slowly, she works her way down from your [chest] to your belly, peeling off your [armor] as she goes to reveal more and more of your " + player.skinFurScales() + ", never neglecting to run her hands all across you, stroking and kissing along every exposed inch until she comes to your groin.  Instinctively, you writhe in her grip as she tosses the last of you [armor] aside, leaving you wholly bare between her legs.");
...
public lionpaws(skipped:boolean = false):void {
	...
	if(skipped) {
		Render.text("\"<i>Really, [name]?</i>\"  Sanura smirks and shakes her head.  \"<i>I probably shouldn't do this sort of thing just because you ask for it, but your fascination with my paws is too cute to ignore.  Go on, then, strip for me and I'll get to it.</i>\"  You oblige, happily tossing aside your armor.  [EachCock], already standing erect with anticipation, bounces freely before you.  The sphinx eyes ");
		if(player.lowerBody.cockSpot.count() == 1) Render.text("it");
		else Render.text("them");
		Render.text(" lustfully, momentarily entranced by the swaying motions of your member");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(".");
	}
	...
	//[if event occurrence == 0]
		...
		Render.text("Sanura raises her eyebrows at your demand, her expression somewhere between surprise and curiosity.  \"<i>Well, that's something of an odd request, but I suppose I'm obliged if it's your wish.</i>\"  Not bothering to give her a chance to consider your demand any further, you pull off your armor and cast it aside.  \"<i>Can't say I've even been asked for something like this before...  I apologize in advance for my inexperience, [name].</i>\"  The sphinx looks down at her paws, obviously confused as to why you would ask for this in particular.");
	}
	//[if event occurrence > 0]
	else {
		Render.text("\"<i>Again?</i>\" Sanura asks, cocking her head to the side.  \"<i>They must feel better than I imagine... well then, get out of your clothes and I'll do as you ask.");
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() > 100) Render.text("  It's not as if there's all that much else I can do for that monster of yours anyways.");
		Render.text("</i>\" She raises a paw and examines it, as though trying to figure out just what it is about them that you enjoy so much.  You cough as a means of drawing her attention once you've stripped off the last bit of your armor, shaking her from her reverie.");
	}
	...
	if(player.lust < 50 && flags[FlagEnum.PAWJOBS] == 0) {
		Render.text("Even as you drop to your knees before Sanura, your cock");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" begin");
		if(player.lowerBody.cockSpot.count() == 1) Render.text("s");
		Render.text(" to harden in anticipation while your mind wonders at just what this will feel like.  ");
	}
	Render.text("The sphinx bends too, so that her head is level with your crotch.  With a little bounce, she throws her large paws around your hips and roughly pulls you closer.  The guardian wastes no time, licking along the shaft");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" of your ");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("various ");
	Render.text("prick");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" with a soft, skilled tongue.");
	//[if pc was soft before]
	if(player.lust < 50 && flags[FlagEnum.PAWJOBS] == 0) {
		Render.text("  It isn't long before her oral ministrations have your cock");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" standing erect, throbbing with hunger for more sensation.  You wonder as to what exactly she has planned, but you think it's shaping up to be something good.");
	}
	//[continue]
	Render.text("  Her long, expert strokes methodically coat your member");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" in a glistening veil of saliva. After a while, she pulls her head away and examines her work.");
	
	Render.text("\n\nApparently deeming ");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("them");
	else Render.text("it");
	Render.text(" sufficiently lubricated, Sanura lifts a foreleg and carefully guides her paw towards your crotch.");
	//[if PC doesn't expect it]
	if(player.lust < 50 && flags[FlagEnum.PAWJOBS] == 0) Render.text("  You raise your eyebrows in surprise, but you're not in much of a position to do anything about her foot's advance.");
	Render.text("  She playfully taps at your cock, purring sensually as she sets it bouncing with a light bat.  After watching it for a moment, she hesitantly pushes forward so that her paw gingerly rests against ");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("one of ");
	Render.text("your shaft");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(", pushing your cock up against your waist and trembling with the effort of keeping her touch gentle.  The pad of her paw, made soft and warm by the sands of the desert, presses lightly around your cock.  Slowly, hesitantly, she begins to rub up and down, her paw gliding along your slickened skin.");
	//[if cockradius < xXXXX(most people should see this)]
	if(player.smallestCockArea() < 100) Render.text("  As she pushes onto you just a bit more forcefully, your member slips between two of her digits.  She continues her slow, rhythmic movements with just a bit more grip, and you're made quite thankful that her claws are so retractable as she brushes against your belly.");
	...
	if(!skipped && flags[FlagEnum.PAWJOBS] == 0) Render.text("It feels wonderful in a peculiar sort of way; though it's certainly not the treatment you expected to receive, you can definitely feel your arousal being pushed along and built up by her foot.  ");
	//[if PC has 2+ dicks]
	if(player.lowerBody.cockSpot.count() > 1) Render.text("Not content to leave your other cocks neglected, the sphinx leans in and starts to lick and suckle once again at the pricks not beneath her paw.  You shudder at the feeling of her tongue darting from shaft to shaft and teasing your heads, sharper sensations punctuating her foot's more constant pleasuring.  ");
	Render.text("The fuzzy warmth clouding your mind dazes you, and you sway slightly in place, letting her rock you back and forth with her motions.");
	...
	if(player.cumQ() < 250) Render.text("\n\nWith a few final pushes you find release, spraying ropes of thick cum along the length of her leg.");
		...
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" like a geyser, covering both you and Sanura in a flood of your semen.  The flow doesn't stop, cum churning and burning as it roils within you.  The sphinx pulls away in an effort to dodge the unexpected volume of man-milk, but to no avail: both halves of her body are painted white and your cum drips from her hair to her skin, fur, and wings. She tries to wipe her face at least clean, but manages only to smear more across her cheek.  With a sigh, she shakes what she can off her hands.");
	}
	Render.text("  Exhausted from the orgasm, you let go of her, leaving the sphinx to milk out the last drops of cum with gentle pressure from her pad on her own.  Sanura stands back up and laughs.  \"<i>Look at the mess you made!</i>\"  She places her dirtied paw on your chest and pushes you back, easily making you fall on your [ass].  \"<i>Well, that was... interesting");
	if(flags[FlagEnum.PAWJOBS] > 0) Render.text(" as always");
	Render.text(", [name].  Now get your clothes back on, I've a temple to guard and a coat to clean.</i>\"  At that, she turns and pads away from you, lying down in front of the door and licking at her");
	//[if cum output is normal]
	if(player.cumQ() < 250) Render.text(" leg.");
	else Render.text(" stained-white fur in a futile attempt to get some of your cum off.");
...
public riddleGameGo():void {
	...
	Render.text("\"<i>Oh!  You'll play my game?  Marvelous!  Well then, let's begin...</i>\"");
	//[NEXT]
	menu();
...
public riddleOne():void {
	clearOutput();
	Render.text("\"<i>Hmm, here's an old one, but a good one, I think.  One of my favorites, but stop me if you know the answer already: 'I walk on four legs in the morning, two in the afternoon, and three in the evening.  What am I?'</i>\"");
	menu();
	...
	if(player.stats.int < 35) addButton(3,"Stilts",answerWrong);
...
public riddleOneSphinx():void {
	clearOutput();
	Render.text("The sphinx narrows her eyes at you, crossing her arms over her chest.  \"<i>You don't say?  Come on, step it up, [name].  Sorry, but that's wrong.  Let's try again.</i>\"");
	menu();
...
public riddleTwo():void {
	clearOutput();
	Render.text("\"<i>Mmm, I've always been fond of this one.  Delightfully simple, really - don't overthink it, dear.  'I always run, yet never walk; I murmur often, yet never talk; and I've a bed, yet never sleep.  What am I?'</i>\"");
	...
	if(player.stats.int < 50) addButton(2,"A Nail",answerWrong);
	if(player.stats.int < 35) addButton(3,"A Bunny-Girl",answerWrong);
	addButton(4,"Uh, ATTACK!",fuckItAttack);
}
...
public riddleThree():void {
	clearOutput();
	Render.text("\"<i>Ooh, this is a morbid one: 'Whoever makes me, sells me.  He who buys me, never uses me. And he who uses me, well, they'll never know it.  What am I?'</i>\"");
	menu();
	//[Poison] (if PC int < 50 add this: [A Condom]) (if PC int < 35 add this: [Arrows]) [A Coffin] [Fuck it, Attack]
	addButton(0,"Poison",answerWrong);
	if(player.stats.int < 50) addButton(1,"A Condom",answerWrong);
	if(player.stats.int < 35) addButton(2,"Arrows",answerWrong);
...
public riddleFour():void {
	clearOutput();
	Render.text("\"<i>Here's a classic for you: 'I'm always hungry, and must be fed always, lest I flicker away.  Yet I will always bite the hand that feeds me, if it touches me.  What am I?'</i>\"");
	menu();
	//(if PC int < 35 add this: [A Fairy]) [Fire] [A Dog] (if PC int < 50 add this: [Disease]) [Fuck it, Attack]
	if(player.stats.int < 35) addButton(0,"A Fairy",answerWrong);
	addButton(1,"Fire",answerCorrect);
	addButton(2,"A Dog",answerWrong);
	if(player.stats.int < 50) addButton(3,"Disease",answerWrong);
	addButton(4,"Uh, ATTACK!",fuckItAttack);
}
...
public riddleFive():void {
	clearOutput();
	Render.text("\"<i>A favorite of my dear Goblin friends: 'When young, I am sweet in the sun; in middle age, I make you gay; but when I'm old, I'm more valuable than gold.  What am I?'</i>\"");
	menu();
	//(if PC int < 50 add this: [Men]) (if PC int < 35 add this: [Women]) [Wine] [Cheese] [Fuck it, Attack]
	if(player.stats.int < 50) addButton(0,"Men",answerWrong);
	if(player.stats.int < 35) addButton(1,"Women",answerWrong);
...
public riddleSix():void {
	clearOutput();
	Render.text("\"<i>This riddle always makes me a bit sad, but... 'My life lasts but hours, and in service I'm devoured.  Thin, I am quick, fat I am slow, and wind is ever my foe.  What am I?'</i>\"");
	
	//(if PC int < 35 add this: [A Goblin]) [A Candle] (if PC int < 50 add this: [A Boat]) [An Arrow] [Fuck it, Attack]
	menu();
	if(player.stats.int < 35) addButton(0,"A Goblin",answerWrong);
	addButton(1,"A Candle",answerCorrect);
	if(player.stats.int < 50) addButton(2,"A Boat",answerWrong);
...
public riddleSeven():void {
	clearOutput();
	Render.text("\"<i>In service to the Sand Witches, I can sympathize with this riddle's speaker: 'Each morning I appear to lie at your feet; all day I'll follow you no matter how fast you run, yet I'll nearly perish in the midday sun.  What am I?'</i>\"");
	...
	if(player.stats.int < 35) addButton(2,"Water",answerWrong);
	if(player.stats.int < 50) addButton(3,"The Breeze",answerWrong);
	addButton(4,"Uh, ATTACK!",fuckItAttack);
}
...
public riddleEight():void {
	clearOutput();
	Render.text("\"<i>Tsk, a sad tale this, fleeting as a melody on the breeze: 'You've heard me before, and will again, till fast I die - then you'll but summon me again.  What am I?'</i>\"");
	//(if PC int < 35 add this: [A Demon]) (if PC int < 50 add this: [Religion]) [An Idea] [An Echo] [Fuck it, Attack]
	menu();
	if(player.stats.int < 35) addButton(0,"A Demon",answerWrong);
	if(player.stats.int < 50) addButton(1,"Religion",answerWrong);
...
public riddleNine():void {
	clearOutput();
	Render.text("\"<i>Like a cat comes this riddle of many lives: 'Three lives have I, gentle enough to sooth the skin, light enough to caress the sky, or hard enough to shatter stone.  What am I?'</i>\"");
	
	menu();
	//(if PC int < 35 add this: [A Dick]) [Water] [A Voice] (if PC int < 50 add this: [Faith]) [Fuck it, Attack]
	if(player.stats.int < 35) addButton(0,"A Dick",answerWrong);
	addButton(1,"Water",answerCorrect);
	addButton(2,"A Voice",answerWrong);
	if(player.stats.int < 50) addButton(3,"Faith",answerWrong);
	addButton(4,"Uh, ATTACK!",fuckItAttack);
}
...
public riddleTen():void {
	clearOutput();
	Render.text("\"<i>Devious, this: 'Whoever makes me, tells it not; he who takes me, knows it not; and he who knows me, takes me not.  What am I?'</i>\"");
	...
	if(player.stats.int < 35) addButton(2,"Piss In A River",answerWrong);
	if(player.stats.int < 50) addButton(3,"Tattered Scrolls",answerWrong);
	addButton(4,"Uh, ATTACK!",fuckItAttack);
}
...
public riddleEleven():void {
	clearOutput();
	Render.text("\"<i>The lovely Naga that roams this desert knows my speaker well: 'A box without hinges, lock or key, yet I've golden treasure within me.  What am I?'</i>\"");
	
	//(if PC int < 35 add this: [A Treasure Chest]) (if PC int < 50 add this: [Pure Honey]) [Eggs] [Booze Bottles] [Fuck it, Attack]
	menu();
	if(player.stats.int < 35) addButton(0,"Teasure Chest",answerWrong);
	if(player.stats.int < 50) addButton(1,"Pure Honey",answerWrong);
...
public riddleTwelve():void {
	clearOutput();
	Render.text("\"<i>Here we've one that speaks to the stars and the earth at once: 'One by one we fall from the heavens, down into the depths of the past; our world is ever upturned, so that yet some time will last.  What are we?'</i>\"");
	...
	if(player.stats.int < 35) addButton(1,"Angels",answerWrong);
...
public answerCorrect():void {
	...
	if(flags[FlagEnum.RIDDLE_THREE] == 0) {
		Render.text("The sphinx-girl sighs, \"<i>That's... correct.  Not bad, I suppose.  Well, we're not done yet... I've still got some tricks up my sleeves.  Er, so to speak.</i>\"");
		menu();
		addButton(0,"Next",riddlePicker);
	}
	else {
		...
		if(flags[FlagEnum.BEATEN_SANURA_COUNT] == 0) {
			Render.text("\"<i>W-what!?</i>\"  the sphinx gasps as you correctly answer the last of her riddles.   \"<i>I-I don't... but how?  No one's ever had the wits to answer all three riddles!</i>\"");
		...
		//BEAT THE SPHINX AT HER OWN GAME (REPEAT)
			...
			Render.text("\"<i>Tsk, I lose again!</i>\" Sanura pouts, crossing her arms.  \"<i>I suppose you'll just have to give me a victory ravishing, won't you?  Go easy, though... you've already damaged my pride so....</i>\"");
		}
...
public answerWrong():void {
	clearOutput();
	Render.text("\"<i>That's... wrong, I'm afraid,</i>\" she says, a proud smile playing across her lips.  \"<i>Wrong, wrong, wrong.  Well, perhaps </i>I<i> was wrong about </i>you<i> after all...  Here I thought you'd be some fun.  Still, though, at least your body will serve me well, I think.  Yes, you'll do nicely.  Go on then, love, strip out of your [armor] for me.  A deal's a deal, after all...</i>\"");
...
public getBlown(submit:boolean = false):void {
	...
	if(player.lowerBody.balls > 0) Render.text("balls empty");
	else Render.text("body empties");
	Render.text(" out onto her eager face.  As you feel yourself winding down from the magically-induced orgasm, however, a strange blue sigil appears in the air, encircling ");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("each of your ");
	else Render.text("your ");
	Render.text("dripping cock");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(".  \"<i>You do want to go again, yes?  I think I've got enough magic to keep that nice [cock biggest] hard for a little fun with my new hands....</i>\"");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" - the salty liquid flows freely onto the desert sands from Sanura's open mouth as your ");
	if(player.lowerBody.balls > 0) Render.text("[balls] churn");
	else Render.text("body churns");
	Render.text(" out a seemingly endless supply of seed.  Your breath comes ragged as more and more spills out of you, your newly-potent manhood making you feel lightheaded as it expels an endless tide of cum.  Slowly, gently, Sanura leans in, wrapping her lips around the tip of your [cock biggest], lapping up the river of semen that pours into her mouth.");
	
	//If Multi-cock = 3
	if(player.lowerBody.cockSpot.count() >= 3) {
		Render.text("\n\nAs Sanura slurps up the tip of your [cock], her ethereal hands grasp ");
		if(player.lowerBody.cockSpot.count() == 2) Render.text("your ");
		else Render.text("two of your ");
		Render.text("other " + num2Text(player.lowerBody.cockSpot.count()-1) + " members, wrapping tightly around their meaty lengths.  You groan throatily as tingling digits slowly work their way up your shafts, making your heart flutter wildly with the alien sensation.");
		//AND If Multicock = 5
		if(player.lowerBody.cockSpot.count() >= 5) Render.text("  You barely have time to adjust to the eldritch hands jerking off your extra cocks when Sanura's own come into play, wading into the jumble of wangs growing from your crotch to grab another pair of pricks.  With five cocks encircled in hands, magic, and mouth, Sanura slowly begins to synchronize her movements, gently stroking your many cocks.");
	}
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" geysering into the air, raining back down in a pool around you and the now cum-covered sphinx.");
...
public fuckDatLionPussah():void {
	...
	//(Large dicks)
		...
		Render.text(" with the little tuft of fur.  \"<i>That bad boy will be just perfect.</i>\"");
	}	
	...
	if(player.upperBody.chest.countNipples() == 2) Render.text("both");
	else Render.text("each");
	Render.text(" of your nipples, which stiffen in response, just like your cock.  You can't help but think there's some sort of magic at work here.  Her tail passes between your thighs, gracefully tickling your ");
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("damp cunt, which suddenly tightens up and practically gushes with slick juices.");
	else Render.text("asshole, which reflexively tightens, sending a shiver up your spine.");
	...
	if(player.hasKnot(player.biggestCockIndex())) Render.text(", your knot slipping into place with a moist <b>pop</b>, ensuring you'll be locked together for the foreseeable future");
	Render.text(".  Jet after jet of cum erupts from your [cock], painting the sphinx's inner walls white.");
	
	//(low-average cum)
	if(player.cumQ() < 250) Render.text("\n\nSome of your spunk dribbles out the side and down your shaft before dripping into the hot desert sand below.");
	//(average-high cum)
	else if(player.cumQ() < 750) Render.text("\n\nA spurt of cum escapes Sanura's folds, splattering out onto your legs and the hot desert sand.");
	...
	if(player.hasKnot(player.biggestCockIndex())) Render.text("which proves somewhat uncomfortable, given your knot is still lodged firmly within her, ");
	Render.text("panting wildly.  It takes you several minutes to recover, the sphinx even longer, ");
	if(player.hasKnot(player.biggestCockIndex())) Render.text("and even longer still for your knot to deflate enough to slip out of her, ");
	Render.text("but you finally gather the strength to haul yourself up onto your [feet].");
...
public sphinxSubmissionOptions():void {
	...
	Render.text("Having lost the Riddle Game, you do as the sphinx-girl commands: you pull off your [armor], leaving yourself bare under the desert sun. \"<i>Now then, my dull little pet, what shall I do with you...</i>\"");
	menu();
	//SUBMISSION:
...
public fenPutsHisShittyFetishInYoSphinx():void {
	clearOutput();
	Render.text("Starting to disrobe, ");
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("you flop your " + player.multiCockDescriptLight() + " free in the dry desert heat, allowing ");
		if(player.lowerBody.cockSpot.count() == 1) Render.text("it");
		else Render.text("them");
		Render.text(" to sway back and forth");
	}
	else if(player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("you expose your [vagina] to the hot desert air, allowing it to moisten");
	}
	else Render.text("you expose your [butt], allowing it to redden");
	Render.text(" as the omnipresent heat and seductive sphinx have their way with your heightened libido.  Sanura snaps her palm up in front of your face as your [armor] hits the ground, landing with a heavy thump.");
	...
	if(player.lowerBody.cockSpot.count() > 0) Render.text("  Pre-cum oozes from [eachCock] in steady strings that dangle onto the dirt, painting a swirled pattern beneath you as your prostate is pushed hard enough to squeeze trickles of your salty 'milk' from you.");
	...
	if(player.lowerBody.cockSpot.count() > 0) Render.text("  Whenever her flare slides past your prostate, the wide, blunt head squeezes it hard enough to express a huge strand of pre.");
	Render.text("  After a few such thrusts, her bouncing, fake balls rise up in their faux, fleshy sack.  Her swollen dick-tip engorges more than you thought possible inside you, wide enough for you to touch through your belly and marvel at the size.  Quivering from head to four lion-like toes, Sanura cums.");
	...
	if(player.lowerBody.cockSpot.count() > 0) Render.text("  When she crosses the lump that is your well used prostate, you lose control and shudder with a nice, long cum of your own, your arms giving out and dumping you face-down on the ground while you spurt milky whiteness onto your chest, chin, and then the ground.");
	else Render.text("  When she's almost out, you give in and cum, quivering around her magical breeding tool while it finishes inseminating your guts.");
	Render.text("  The head is so swollen that she actually struggles to pop it back out of your [asshole], bouncing her hips back a few times with little success");
	if(player.lowerBody.cockSpot.count() > 0) Render.text(" and dragging you through your fresh, milky puddle");
	Render.text(".  Eventually, she gives a mighty, sphincter-stretching lurch and ejects from your [butt], painting it white on the way out.");
...
public sandWitchMotherFriendlyMenu():void {
	...
	menu();
	if(flags[FlagEnum.SAND_WITCH_LEAVE_ME_ALONE] == 0) addButton(0,"StopAttacking",leaveAloneSendLackeysToggle);
...
public slavesDiscussion():void {
	menu();
	if(flags[FlagEnum.ESSY_MET_IN_DUNGEON] > 0 && flags[FlagEnum.TOLD_MOTHER_TO_RELEASE_ESSY] == 0) addButton(0,"Essrayle",forest.essrayle.askMotherToReleaseEssy);
	if(flags[FlagEnum.MET_MILK_SLAVE] > 0 && flags[FlagEnum.MILK_NAME] is Number) addButton(1,"Milk-Slave",freeSlaves);
}
...
private freeSlaves():void {
	clearOutput();
	Render.text("Thinking back to the poor, mind-broken tittymonster of a bath girl you met here in the witches' coven, you tell the Sand Mother it's about time to free her and any other slaves she's got hidden away.");
...
private yesDemandMilkRelease():void {
	clearOutput();
	Render.text("You tell the Sand Mother that everyone deserves freedom, even if they're addle-minded, or useful.  She can't just <i>keep people</i> because she feels like they're better off in her care.");
...
private gimmeDatDeliciousMilkWaifuINeedMoreWaifusCauseTheTwoCowslutsWerentEnoughForMyInsatiableLacticLustandDesire():void {
	clearOutput();
	Render.text("You have a better idea: give the girl to you.  You can care for and protect her in your camp, but also give her at least as much freedom as is safe in these dire times.");
	...
	if(player.stats.gems >= 2000) addButton(1,"Buy Her",BuyHer);
}

private TwoExpensive4Me():void {
	clearOutput();
	Render.text("You explain that you can't afford that much.");
...
private BuyHer():void {
	...
	Render.text("You hand over two thousand of your hard-earned gems.");
...
public sexWithFriendlySandMother():void {
	menu();
	//friendly cunt fuck:
	if(player.lowerBody.vaginaSpot.hasVagina() && player.lust >= 33) addButton(0,"GetMilkFill",lesboMilkFilling);
...
public sandMotherPOMenu():void {
	...
	if(flags[FlagEnum.SAND_WITCH_LEAVE_ME_ALONE] == 0) addButton(0,"StopAttacking",unfriendlyWitchToggle);
	else addButton(0,"StartAttacking",unfriendlyWitchToggle);
	if(player.lust >= 33) {
		if(player.lowerBody.vaginaSpot.hasVagina()) addButton(1,"Scissor",scissorAndDrink);
	...
	if(flags[FlagEnum.ESSY_MET_IN_DUNGEON] > 0 && flags[FlagEnum.TOLD_MOTHER_TO_RELEASE_ESSY] == 0) addButton(7,"Essrayle",forest.essrayle.askMotherToReleaseEssy);
	addButton(9, "Leave", playerMenu);
}
...
public sandMotherStuffGOA():void {
	...
	if(flags[FlagEnum.SAND_WITCHES_FRIENDLY] > 0) {
		Render.text("The Sand Mother slightly inclines her head in your direction as you enter her throne room.  She is every bit as imposing as you recall, or at least as imposing as a woman with four watermelon-sized jugs on her chest can be.  \"<i>You have returned to us, far traveler.  Is there something you desire from the coven of the sands?  Simply ask for what you need, and we shall aid you against the demon Queen.</i>\"  The lactation-obsessed arch-enchantress idly pushes a whitish-blonde curl out of her eyes as she awaits your reply.");
	...
	//*Encounter Repeat: Cowed
		...
		Render.text("The Sand Mother glares down at you from atop her throne, but does not dare lift a finger against you, not after her previous defeat.  She hisses, \"<i>You've already plundered my coven!  What more do you want from us?</i>\"  even as her chest heaves in a panic.  Four mountainous peaks jut from the witch's silken robe... you have a few ideas, and the Queen Witch seems to have some of her own.");
		//Resisting Options
		...
		if(flags[FlagEnum.SAND_WITCH_LOOT_TAKEN] >= 5) Render.text(", though right now it sits empty.  You'll need to wait for them to restock it");
		else Render.text(", and noone could stop you from taking them");
		Render.text(".");
		sandMotherPOMenu();
	}
...
public explainYourSelfToZeSandBitch():void {
	...
	if(player.stats.cor > player.stats.int || monk >= 5 || player.statusAffects.has("Exgartuan") || amilyScene.amilyCorrupt() || flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00283] > 0 || flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0 || flags[FlagEnum.NIAMH_STATUS] > 0) {
		Render.text("  \"<i>There is some truth to your tale, [name], but I am a Sand Mother.  We are schooled in the art of sussing out the corrupt or unclean.  If we could not detect disguised demons and demonic agents, we would not flourish as we do now, and this great desert would not be on the cusp of resurrection.</i>\"");
	...
	//(No new PG. Corruption is less than intellect and no major asshole moves made:) 
		...
		Render.text("  \"<i>Your tale has the ring of truth to it, [name].  I am the Sand Mother of this coven, and it seems we are not enemies after all.  If you like I can instruct my daughters to let you pass through the sands unhindered.</i>\"  She spreads her arms graciously and asks, \"<i>Was there anything you would like to discuss?  It has been an age since I've had a new arrival to converse with.</i>\"\n\n(+200 XP)");
		//(Award XP)
		player.XP += 200;
...
public fightTheSandWitch():void {
	clearOutput();
	Render.text("You brandish your [weapon] and advance threateningly.  The Sand Witch Queen slowly rises, pulling a scepter from beneath her robes with a resolute look in her eyes.  \"<i>You shall regret this action for a time, until we've corrected your diseased way of thinking.  Now, taste the power of a Sand Mother!</i>\"");
	startCombat(new SandMother(),true);
}
...
public gigaFire():void {
	...
	Render.text("Releasing the scepter, the Sand Mother spreads her hands, each glowing with eldritch, white flames.  Her heels slowly float up off the ground as she closes her eyes in concentration.  You can sense the power and heat rolling off her in waves, and if you don't do something to disrupt her, you'll likely be burned to a crisp.");
	if(player.stats.int > 40) Render.text("  She's not even looking at you and seems steeled against lusty interruptions.  Perhaps you can hit her hard enough to shatter her concentration.");
...
public gigaFire2():void {
	...
	if(monster.statusAffects.get("Gigafire").value1 < 10) {
		Render.text("The Sand Mother grabs her scepter in both hands, combining the flames that wreath them into an immense, blinding conflagration.  She points at you, and the fire washes out in a wave like a serpent, twisting at you as you try to avoid it, doubling back on itself whenever it misses.  It's unavoidable!  You're enveloped in the consuming fire!");
		damage *= 3;
	}
	//Interrupted:
	else {
		Render.text("Thumbling back to the ground from your damaging hit, the Sand Mother grabs at her head, one flame going out.  She recovers in time to grab the staff and fling it towards you, but it's at a fraction of the strength she intended.  Still, it burns all the same.");
	}
	damage = takeDamage(damage);
	Render.text(" (" + damage + ")");
...
public sandMotherTelekinesis():void {
	Render.text("She narrows her eyes at you, and an immense, magical pressure reaches out, taking hold of you.  It spins you in the air before slamming you into the walls!");
	
	let sizeMod:number = 100;
	let thickMod:number = player.thickness / 100 + 0.5;
	sizeMod += player.tallness * thickMod;
	if(sizeMod < 140) Render.text("  You immediately wish you weren't so small, as you're sure she couldn't have flung a heavier champion nearly as easily.");
	else if(sizeMod >= 200) Render.text("  You're glad for your size, as she couldn't seem to accelerate you into the stone as fast as she'd like.  Sometimes eating plenty pays off.");
...
public eatALightningBolt():void {
	...
	Render.text("Holding her staff back, she lifts her free hand with her fingers outstretched in a fan.  Sparks dance between her slender digits, coiling around them like snakes.  In a flash, they rush to her palm and erupt in a lightning bolt, striking you instantly and unavoidably!");
...
public defeatTheSandMother():void {
	...
	Render.text("Panting ");
	if(player.HP < 1) Render.text("in exhaustion");
	else Render.text("with lust");
	Render.text(", the Sand Mother sinks back into her throne with sweat and milk staining her silky robes.  She groans softly, the milk-stains slowly spreading as you consider your options.  This woman dared to stand against you.  What do you do?");
	//[Fuck Her Cunt] [ScissorAnDrink] [Talk Her Down]
	menu();
	addButton(0,"TalkHerDown",talkDownTheMother);
	if(player.lowerBody.vaginaSpot.hasVagina()) addButton(1,"Scissor",scissorAndDrink);
	if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(monster.vaginalCapacity()) >= 0) addButton(2,"Fuck Her",fuckTheSandMothersCunt);
...
public tentacleGangBang():void {
	...
	if(player.tentacleCocks() >= 6) {
		Render.text("\n\nYou still have ");
		if(player.tentacleCocks() >= 7) Render.text("untended serpentine shafts to please");
		else Render.text("an untended serpentine shaft to please");
		Render.text(", but all the holes are full!  You take the ");
		if(player.tentacleCocks() == 6) Render.text("leftover one and thread it");
		else Render.text("leftover ones and thread them");
		Render.text(" through the dick-dominated enchantrix's fingers.  By now, her body has completely and utterly betrayed her, and she grabs hold like a cock-hungry slut, immediately stroking up and down, jacking it like a pro.  A muffled sigh of contentment shivers up your throat-fucking cock, alerting you to your depraved foe's prick-addled state.  You begin to pump your quartette of buried cocks into their respective holes with renewed vigor.  The titty-fucking vine pistons through its curvy path along with them, sending the witch's tits jiggling and squirting wildly.  The only thing");
		if(player.tentacleCocks() == 6) Render.text(" held still is the one getting a hand job, and you only hold it still");
		else Render.text("s held still are the two getting hand jobs, and you only hold them still");
		Render.text(" so that part of you can relax and enjoy your captive Queen's enthusiastic wanking.");
	}
	//(10+)
	...
	if(player.tentacleCocks() >= 6) Render.text("  Bubbling seed soaks her frantically jerking hands.");
...
public fuckTheSandMothersCunt():void {
	...
	if(y >= 0) Render.text(", grabbing a second cock as well, intent on penetrating both of her sopping entrances");
	Render.text(".  The moment her labia caress your ");
	if(y >= 0) Render.text("cockheads");
	else Render.text(player.cockHead(x));
	Render.text(", she freezes as she realizes what she's doing, and she tries to pull away.");
	...
	if(y >= 0) Render.text("tools");
	else Render.text(cockDescript(x));
	Render.text(".  She moans out loud, throwing her bleached tresses around wildly in pleasure while inch after inch of cock snakes into her passage");
	if(y >= 0) Render.text("s");
	...
	if(y >= 0) Render.text("s");
	Render.text(".  Semen floods out of you from the force of your orgasm, creaming the juicy cunt");
	if(y >= 0) Render.text("s");
	Render.text(" as ");
	if(y < 0) Render.text("it clamps down");
	else Render.text("they clamp down");
	Render.text(", squirting femlube and leftover spunk across your [feet].");
	if(player.cumQ() >= 500) {
		Render.text("  Again and again, you flood the poor spellcaster's twat");
		if(y >= 0) Render.text("s");
		Render.text(", shooting so much that it bubbles out in thick globs to fall wetly on the ground.");
		if(player.cumQ() >= 2000) Render.text("  They fall faster and faster as you cum, the witch's belly bloating, rounding out to a ridiculous degree.  Her legs are drenched with the leaking spooge.");
	}
	Render.text("  Once you've pumped every ounce of your virile fluid into the waiting receptacle");
	if(y >= 0) Render.text("s");
	Render.text(", you pull, admiring the salty, mark you've left on the once proud woman.");
...
public scissorAndDrink():void {
	...
	if(player.tallness < 85) Render.text(" larger");
	else if(player.tallness > 105) Render.text(" smaller");
	Render.text(" woman, moist, slick bodies entwined as you both grope and grind.");
	...
	if(player.wetness() >= 5) Render.text("explodes out in a cunt-shattering spray");
	else if(player.wetness() >= 4) Render.text("gushes out in a heavy spray");
	else if(player.wetness() >= 3) Render.text("pours out in a thick, wet river");
	else if(player.wetness() >= 2) Render.text("drools out in a steady trickles");
	else Render.text("drips out during the cunt-shattering convulsions");
	Render.text(".  Bulging obscenely, your cheeks barely contain the flood of milk, and you swallow it without thinking, acting entirely on instinct.");
...
public loseToSandMotherBadEnd():void {
	clearOutput();
	Render.text("Stumbling back into a wall, you try to hold your ");
	if(player.HP < 1) Render.text("battered body aloft, but your wounds are too great, robbing you of strength.");
	else Render.text("rebellious body aloft, but your lust is too great, turning your body into a quivering wreck.");
	Render.text("  You ");
	if(player.HP < 1) Render.text("spitefully watch the Sand Mother approach, only able to admire her sculpted form as she towers over your prone form.");
	else Render.text("hungrily watch the Sand Mother approach, only able to touch yourself as you gaze up at her sculpted form");
	Render.text(".  She sniffs in disdain as she looks down at you.  \"<i>Disappointing.  Lethice sends a weakling like you after us, and the sisters do not even slow you.  Their atonement shall be severe.</i>\"  Her eyes, blue as the desert sky, consider you again, and she mutters, \"<i>First, there is the matter of you, isn't there?</i>\"");
	...
	if(player.stats.cor < 66) Render.text("aren't corrupt");
	else Render.text("don't serve the demons");
	Render.text(", looking up at her pleadingly ");
	if(player.lowerBody.cockSpot.count() == 1) Render.text("while she gleefully pumps you, smearing your leaky juices all over your shaft.");
	else Render.text("while she gleefully moves to your second cock, smearing it with your first dick's pre-cum, messing your " + player.multiCockDescriptLight() + " with each penis's leaky drippings.");
	...
	if(player.cumQ() >= 500) Render.text("  Wave after wave of jism washes across you, thoroughly drenching you with your spooge, the alabaster spunk soaking you from crown to waist with sticky slickness.");
	if(player.cumQ() >= 3000) Render.text("  A puddle pools around you, deepening nicely as your boundless virility does its work, inching up your body until you feel as if you'll float away in your own pearly jism.");
	...
	if(flags[FlagEnum.CUM_WITCH_DEFEATED] > 0) Render.text("The Cum Witch is here, and she seems to have recovered from her defeat!");
	else Render.text("This must be the Cum Witch!");
	...
	if(player.lowerBody.balls > 0) Render.text("testes");
	else Render.text("prostate");
	Render.text(" emptying orgasm, you shudder and try to pull away, but there's nowhere to go.  Gentle hands place themselves to either side of your head, immobilizing you, and the lusty hermaphrodite begins to chant, punctuating her arcane words with slow pumps of her body across your cum-slicked middle");
	if(player.lowerBody.balls > 0) {
		Render.text(", two sets of slippery balls bouncing and sliding across each other");
		if(silly()) Render.text(".  Great, now it's gay");
	}
	Render.text(".");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) Render.text("innards");
	else Render.text("folds");
	Render.text(" thickens slightly, and the heaving balls below jiggle with liquid weight, swelling larger against you.  They grow heavier and heavier, until they slosh with audible, barely contained need, each as large as one of your butt-cheeks and three times as soft.");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) Render.text("bowels");
	else Render.text("womb");
	Render.text(", pumping into you with enough force to audible splash and churn about, gently thickening your middle.  A crystalline lattice-work erects from the mental syrup, climbing higher into a brand new frame of reference, filled with the thoughts and experiences of others - no, of your past.  New memories fill in the gaps while your gut rises, forming a small bump that would be easily mistaken for pregnancy.");
...
public loseToSandMother():void {
	clearOutput();
	Render.text("The world grows dizzy as your ");
	if(player.HP < 1) Render.text("heavily punished and aching body is filled with too much agony to continue.");
	else Render.text("hot and aroused body fills with unbearable urges that finally distract you from fighting any further.");
	Render.text("  Instead, you fall backwards against the wall ");
	if(player.HP < 1) Render.text("and grit your teeth in anger as the Sand Mother approaches.");
	else Render.text("panting deeply, and your blood races as the Sand Mother approaches.");
	Render.text("  You can not help but admire her incredible physique as she towers over you, looking down upon your body with disdain in her fierce eyes.  \"<i>Lethice should have known better than to send such a pathetic imbecile like you to battle us.  I can hardly believe you managed to get by so many of our sisters.</i>\"  She sighs.  \"<i>Though I suppose it'd be a greater waste if we just killed you.</i>\"");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("Remarkably, a divot appears on your groin, slowly dividing into two puffy lips with a moist crease between.  The fresh mons of your new pussy glistens with immediate arousal as the Sand Mother's magic molds your flesh with a simple spell.  ");
		player.createVagina();
		player.lowerBody.vaginaSpot.list[0].clitLength = 0.25;
	}
	Render.text("Your [vagina] is eyed momentarily as it ");
	if(player.wetness() <= 2) Render.text("grows moist.");
	else if(player.wetness() <= 4) Render.text("drips with arousal.");
	else Render.text("drools obscenely.");
	Render.text("  \"<i>We can certainly improve upon this.</i>\"  You attempt to voice opposition but she quickly silences you.  \"<i>No! Understand here and now, you live by our mercy.  Your body belongs to the sand.  Your old life is over.  From now on, your failed life will serve a greater purpose.</i>\"");
	
	Render.text("\n\nYou try to resist but ");
	if(player.HP < 1) Render.text("the pain of your abused body");
	else Render.text("lustful urges fill your senses and");
	Render.text(" hinders all of your attempts to fight back.  You try to argue that you aren't a servant of the demons, that you were fighting against them for your own people just as she is, but every time you open your mouth she forces it closed again with her magic.  \"<i>I am not interested in the lies of a demonic servant.  It is time we dealt with you once and for all.</i>\"");
	...
	if(player.bRows() == 1) {
		Render.text("  Plus an extra set of these to ensure you produce as much milk as the rest of your soon-to-be sisters.");
		player.createBreastRow();
	}
	//if player has four breasts
	else if(player.bRows() == 2) Render.text("  Thankfully you're already endowed with enough breasts to begin immediate production of milk.");
	//if player has six breasts
	else Render.text("  Hmmm. We've never had a sister blessed with more than four breasts. I wonder if perhaps we can use your unusual anatomy to our advantage.");
	Render.text("  She squeezes a [nipple] and a yelp of pleasure escapes you.  \"<i>I see you like that idea. Excellent.</i>\"");
	//if breasts are smaller than a normal sand witch's
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 3) Render.text("  \"<i>Of course we'll have to grow them a bit to suit our needs.</i>\"");
	//if breasts are larger than a normal sand witch's
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 14) Render.text("  \"<i>Mmmm, and with such massive ones I'm sure we can expect you to produce more than most of our sisters could.</i>\"");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 5) Render.text("jiggle");
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 10) Render.text("bounce");
	else Render.text("quake");
	Render.text(" with every twitch of her fingers inside you.");
	if(player.lactationQ() >= 50) Render.text("  Streams of milk are already starting to pour down the swells of your tits.");
	Render.text("  Her fingers start pressing deeper, playing upon your erogenous zones like an instrument, strumming your body's sense of pleasure to new heights.  She plays your cunt until you cum again and again around palm and gush your juice down her arm.  Everything below your waist abandons your control and submits itself to her administrations.  You cum and all you can do is grope your own tits as you ride the waves of pleasure that saturate your flesh.");
	...
	if(flags[FlagEnum.CUM_WITCH_DEFEATED] == 1) Render.text("  It is obviously the Cum Witch you encountered, and she has clearly regained her strength!");
	else Render.text("This must be the Cum Witch the Sand Mother sent for!");
	...
	if(player.bRows() <= 2) Render.text(", admiring the extra set of breasts you already possess");
	else Render.text(", admiring how you've managed to grow " + num2Text(player.bRows()) + " rows of breasts instead of just two on your panting torso");
	Render.text(".");
...
public sandMotherBadEndsLadiesEpilogue():void {
	clearOutput();
	Render.text("The Champion awoke to a body changed. " + player.mf("His","Her") + " body had been transformed in to that of a Sand Witch's.");
	//if player had six breasts
	if(player.bRows() > 2) Render.text("  Although unlike the other Sand Witches, this one sported extra breasts which she used to her utmost advantage.");
	//if player had gigantic breasts
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 22) Render.text("  She was unrivaled in her ability to produce milk thanks to the natural enormity of her breasts that her other sisters couldn't come close to.  They were so large that they often impeded her movement, which she admittedly didn't mind so much.");
	...
	if(player.perks.has("BroodMother")) Render.text("  She'd immediately shown a unique propensity for rapid pregnancies, birthing new sisters at twice the rate of any other Sand Witch.  Her surprisingly fertile wombs helped give an extra edge to the Sand Mother's plans.");
	
	//if player had broodmother and gigantic breasts
	if(player.perks.has("BroodMother") && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 22) Render.text("  As a result, the champion who could no longer remember her true past became little more than a giant pregnant belly surrounded by " + num2Text(player.bRows()) + " breasts so massive that the other sand witches often used her tits as comfortable cushions to sit upon, their thighs tightly gripping each giant nipple against their aching cunts for pleasure.");
...
public friendlySandMotherFuck():void {
	...
	if(flags[FlagEnum.TIMES_FRIENDLY_FUCKED_SAND_MOTHER] == 0) {
		Render.text("Casting your gaze around to ensure privacy, you lower your brows conspiringly and inquire into the Sand Mother's love life.");
	...
	if(!player.isTaur()) {
		Render.text("holding [oneCock] in ");
		if(y >= 0) Render.text("one");
		else Render.text("your");
		Render.text(" hand");
		if(y >= 0) Render.text(" and another in the other");
		Render.text(", the meaty cock-flesh wobbling, veins pulsing just under the surface as you nose ");
		if(y < 0) Render.text("it");
		else Render.text("them");
		Render.text(" up against the twin, wet pussies.");
	}
	else {
		Render.text("dick");
		if(y >= 0) Render.text("s");
		Render.text(" bobbing under your equine belly, the meaty cock-flesh wobbling with veins pulsing just under the surface as ");
		if(y < 0) Render.text("it noses");
		else Render.text("they nose");
		Render.text(" up against the twin, wet pussies.");
	}
	if(player.tallness >= 90) Render.text("  You have to kneel down to make it work, tall as you are.  ");
	else if(player.tallness <= 48) Render.text("  You can barely reach her cunt standing, short as you are.  ");
	Render.text("Her lube-glossed gates spread around your " + player.cockHead(x));
	if(y >= 0) Render.text(" and other cock-tip");
	Render.text(", the hot moisture gobbling up your inches, clinging hot and wet to the oversensitive, pulsating dick");
	if(y >= 0) Render.text("s");
	Render.text(".  The large woman sighs with relief as you scratch her unspoken, hidden itch.  Your [hips] push forward with such fervor that you doubt you could stop if you wanted, pressing hard on her sensitive, innermost folds with ");
	if(y >= 0) Render.text("both your cocks");
	else Render.text("your " + cockDescript(x));
	Render.text(".");
			   
	//HOARSES:
	if(player.isTaur()) {
		Render.text("\n\nWhinnying in delight, you rest your forelegs on ");
		if(player.tallness < 78) Render.text("her shoulders");
		else Render.text("the top of her throne");
		Render.text(" and thrust, pumping yourself hard into her with animalistic delight.  The Sand Mother shudders beneath your bestial bulk as her hands begin to tug at her nipples through her soaked, not-so-concealing robes.  She whimpers and begins to kiss at your human abdomen, tonguing your belly button as she's mounted on your bitch-breaking erection");
		if(y >= 0) Render.text("s");
		Render.text(".  Grabbing a lock of platinum in your fingers, you keep a firm hold on her while you let your lower half's inhuman ardor take control of your body.");
	}
	//NOTAHOARSE:
	else {
		Render.text("\n\nGroaning in delight, you rest your hands on her ");
		if(player.tallness < 60) Render.text("bottom tits");
		else if(player.tallness < 84) Render.text("top tits");
		else Render.text("shoulders");
		Render.text(" and thrust, powerfully pumping your manhood through her lube-gilded channel");
		if(y >= 0) Render.text("s");
		Render.text(".  The Sand Mother shudders before you, hands tugging her nipples through her soaked, not-so-concealing robes.  She's whimpering needily, and ");
		if(player.tallness < 72) Render.text("you kiss her to silence her, tongue pumping to the same timing that drives your hips.");
		else Render.text("you suckle one of the heavy, milk-laden tits to enhance her pleasure");
		Render.text(".  Her fingers roam through your " + hairDescript() + " as she allows her passion to overwhelm her.  Splattering noisily, her juices are dripping and squirting around your hard-thrusting dong");
		if(y >= 0) Render.text("s");
		Render.text(", replaced by fresh secretions as fast as you squeeze them out of her.");
	}
	
	//Both
	Render.text("\n\nYou groan as the suckling heat around your rod");
	if(y >= 0) Render.text("s");
	Render.text(" lurches into convulsions, too hot, tight, and sensuous for you to endure.  For a moment, you try your damnedest to hold out, to ride the wave of pleasure for another few minutes before you release into the agonizingly sweet puss");
	if(y < 0) Render.text("y");
	else Render.text("ies");
	Render.text(".");
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("  Your own [vagina] quivers weakly, insides grinding against insides, squeezing every drop of juice from your ");
		if(player.wetness() >= 4) Render.text("squirting ");
		Render.text("passage.");
	}
	if(player.lactationQ() >= 200) {
		Render.text("  Escaping from your [nipples], milk ");
		if(player.lactationQ() < 200) Render.text("leaks");
		else if(player.lactationQ() < 1000) Render.text("drips");
		else Render.text("erupts");
		Render.text(", splattering the throne with a fresh, lactic offering.  If you weren't mindlessly jamming yourself into her cunt, you'd wonder if she approves of your offering, but every neuron is busy sending your body to a mind-blowing climax.");
	}
	Render.text("  You grunt as the heat inside your clenching [balls] radiates through your groin and towards [eachCock].");
	//Newline
	Render.text("\n\n");
	//More than 2 dongs or one doesn't fit
	if(player.lowerBody.cockSpot.count() > 2 || (player.lowerBody.cockSpot.count() == 2 && y < 0)) {
		Render.text("White love-batter fires in thick ropes onto her robes from your unbound penis");
		if(player.lowerBody.cockSpot.count() > 3 || (player.lowerBody.cockSpot.count() == 3 && y < 0)) Render.text("es");
		Render.text(".  One after another, the syrupy strands mix in with the dripping milk to make an alabaster drapery of sex-juice.  ");
	}
	//light ejaculate
	if(player.cumQ() < 250) {
		Render.text("You spend your passion in numerous spurts, firing straight into the deepest depths of the Sand Mother's pussy.  Her slick walls squeeze you repeatedly, pulling you further inside, caressing you to encourage your orgasm to gift her with greater quantities of jism. As you continue to spurt, you realize that her hungry cunt");
		if(y >= 0) Render.text("s are");
		else Render.text(" is");
		Render.text(" successful in some capacity, because you squirt until it's almost painful, finishing with ");
		if(y < 0) Render.text("a fat white drop that's squeezed from your tip");
		else Render.text("fat white drops that are squeezed from your tips");
		Render.text(".");
	}
	//medium ejaculate
	...
	//high ejaculate
		...
		if(y >= 0) Render.text("s go");
		else Render.text(" goes");
		Render.text(" straight into her womb");
		if(y >= 0) Render.text("s");
		Render.text(", rounding her belly to pregnant proportions.  She moans as she's filled beyond capacity with cum, only getting relief when the pressure in her packed womb");
		if(y >= 0) Render.text("s grow");
		else Render.text(" grows");
		Render.text(" so high that jets of spooge spray out around you.  That doesn't stop you - you keep forcing more thick ejaculate into her until she's a nonsensical, babbling mess and you're totally spent.");
	}
	//END
...
public lesboMilkFilling():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 10) Render.text(", [fullChest] swaying enticingly as you strike your seductive pose");
	Render.text(".   Freeing your body from its bindings is almost as exciting as seducing the queen of the lactic sorceresses, and you ");
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) Render.text("cup your mammaries");
	else Render.text("run your hands down your chest");
	Render.text(" as you stride closer.");
	...
	//NOT A HORSE
		...
		if(player.tallness > 72 || player.isTaur()) Render.text(", though judging by the glow around her hands, she used some magic to help");
		Render.text(".  Sliding enticingly across your nethers, her pebbly, soaked tip drags perfectly up against your [clit], just waiting to be jammed home and begin inseminating you with milk.");
	}
	//Both
	...
	Render.text("\n\nYou ");
	if(player.isTaur()) Render.text("squeeze your own [nipples]");
	else Render.text("grab hold of her head");
	Render.text(" as you begin to pull away and thrust back, languidly fucking yourself on her gushing spout while it floods your pouty, dripping pussy and eager womb with lactic moisture.");
	if(!player.isTaur()) Render.text("  Somehow, she's able to pull her hand away from your hip while still holding you aloft.  A glowing, arm-shaped silhouette remains, even as she diverts her attention elsewhere.");
	Render.text("  'Schluck-schluck-schluck.'  You can hear her pushing her fingers deep into her neglected womanhoods, and judging by the doubled, wet sounds of penetration, she must be jamming two fingers in her doubled cunts.");
	...
	if(player.pregnancyIncubation == 0) Render.text("You can feel your midsection going round, wobbling wildly.  POP!  Your belly-button has turned into an outie, accompanied by a wobbling, comforting weight inside you.  Four months... five, no - six... you give up on counting, but with every second that passes, you're getting bigger and bigger, visually matching any of the pregnant women you saw growing up.");
	else Render.text("You can feel it fountaining out of you, backwashing out to splatter the witch with wetness, her skin going glossy under the tide of white.  If you weren't already pregnant, you'd probably be ballooning up with her lactic cargo, a rounded, milk-filled cream-balloon.");
...
public sandWitchHistory():void {
	clearOutput();
	Render.text("You inquire about the history of the sand witches, and how they came to be.  The Sand Mother gives you a benign smile and offers, \"<i>With pleasure.  It is so rare that I get to recount the story to one not of our order.</i>\"  She utters a few spidery words that seem to ring in your ears, and a luminous, floating illusion appears before her, all floating shapeless colors.  As she begins to talk, the blobs shift into the actors in her tale, allowing you to view the story as it happens.");
...
public currentStateOfInterwebs():void {
	clearOutput();
	Render.text("You inquire as to the current state of the sand witches, and this coven in particular. The Queen Witch's brow creases in thought as she considers your request, tapping her chin and gazing into the intricate, swirling lights above.");
	...
	if(flags[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] == 0) Render.text("All that remains is to ready those still maturing and wait on our allies to finish growing their numbers.  We shall be carried into the Demon Queen's fortress on the wings of phoenixes, to fight as glorious a battle as has ever been fought!");
	else Render.text("All that remains is for us to find new aerial allies or a method of easy, sustained flight.  The phoenixes were smashed apart by fiery demons, their lair raided and destroyed by an interloper.  If only things had been different... Calais was so close.");
	Render.text("</i>\"");
	...
	if(flags[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] > 0) Render.text("  Of course, this is all contingent on us finding an ally that can breach the mountaintop citadel's walls.");
	Render.text("  Was there something else you needed to ask about?</i>\"");
	//[friendly options]
	sandWitchMotherFriendlyMenu();
}
...
public discussCumWitches():void {
	clearOutput();
	Render.text("You inquire as to why they have cum witches.");
	...
	if(player.stats.cor + player.stats.lib > 100) Render.text(", though potentially fun");
	Render.text(".  With a little bit of a lecherous smile, you ask if she ever tried out for it.");
	...
	if(flags[FlagEnum.MORE_CUM_WITCHES] == 1) {
		...
		if(flags[FlagEnum.CUM_WITCHES_FIGHTABLE] == 0) {
			Render.text("  If you asked her to send them out hunting for recruits, you could potentially fight and fuck them in the deserts sands in the future.");
			addButton(0,"Send Out",sendOutCumWitch);
		}
		else {
			Render.text("Many of them are prowling the desert sands even now, hunting for recruits.  You could wind up having to fight them if you go out.  It's possible she might keep them away from you, if you request it.");
...
public sendOutCumWitch():void {
	clearOutput();
	Render.text("You request she put the cum witches' boundless virility and desire to work for the coven, letting some of them out to venture the sands in search of new recruits.  (<b>From now on, there's a chance you'll encounter cum witches in the desert.</b>)");
...
public keepCumWitchesIn():void {
	clearOutput();
	Render.text("You request she put a hold on sending out the cum witches.  Their aggressive, sexual recruitment methods are getting in your way, and for now, it would be best if they were out of your way.");
...
public moreCumWitchesPlease():void {
	...
	if(player.perks.has("BimboBrains") || player.perks.has("FutaFaculties")) {
		Render.text("Wouldn't it be better if there were like, lots of cum witches, with yummy cocks that you could suck?");
		Render.text("\n\n\"<i>No, it wouldn't,</i>\" the Sand Mother retorts, ending the conversation.");
		doNext(playerMenu);
	}
	Render.text("You ask her if it wouldn't be more humane to simply create a few more cum witches, allowing them to split their duties and actually have time to serve in other ways, either as normal sisters or perhaps helping with the recruitment.");
...
public askHowSandMothersAreChosen():void {
	clearOutput();
	Render.text("You ask the Sand Mother how her people choose who will be a Sand Mother of a new coven.");
	...
	if(player.race() == "human") Render.text("'re a human");
	else Render.text(" were a human");
	Render.text(".  Our people are notorious for forming gangs and cliques, groups focused around a leader's charisma.  It's only natural that has continued in Mareth.  It would not surprise me to learn that even the demons have their own factions and political malcontents, though I imagine they deal with them far more harshly than we.</i>\"");
...
public leaveAloneSendLackeysToggle():void {
	...
	if(flags[FlagEnum.SAND_WITCH_LEAVE_ME_ALONE] == 0) {
		Render.text("You request that the scouts in the desert stop bothering you with their barbed offers of magic.  Fighting them is not something you want to have to do.  It's a waste of effort on both sides you that feel should come to an end.");
	...
	//Send Lackeys
		...
		Render.text("You mention that you wouldn't mind the witches trying to recruit you as they used to.");
...
public getLactaidFromWitches():void {
	clearOutput();
	Render.text("You ");
	if(player.stats.cor < 33) Render.text("politely request");
	else if(player.stats.cor < 66) Render.text("request");
	else Render.text("none-too-politely request");
	Render.text(" a dose of Lactaid from her coven.");
...
public getLaBova():void {
	clearOutput();
	Render.text("You ask for some La Bova");
	if(player.stats.cor < 33) Render.text(" with a blush");
	else if(player.stats.cor < 66) Render.text(" a little uncertainly");
	else Render.text(" with a knowing smile");
	Render.text(".");
	...
	Render.text("\n\nYou shake your head");
	if(player.stats.cor > 66) Render.text(", though the idea does seem to nestle into your imagination quite alluringly");
	Render.text(".");
...
public unfriendlyWitchToggle():void {
	...
	if(flags[FlagEnum.SAND_WITCH_LEAVE_ME_ALONE]  == 0) {
		...
		if(player.stats.cor < 50) Render.text("  You probably wouldn't do something like that, at least not without provocation!/  You could see how she would think that.");
		else Render.text("  As if that would stop you.");
		Render.text("  Slapping her on the shoulder, you congratulate her on seeing good sense.  Her eyes narrow dangerously as she twists away from your touch.");
...
public pullLever():void {
	clearOutput();
	Render.text("There is a loud rumbling from the direction of the cavernous commons...");
...
public takeFertilePills():void {
	clearOutput();
	if(player.findStatusAffect(StatusAffects.Contraceptives) < 0) Render.text("You aren't under the effects of a contraceptive, so taking a pink pill would do nothing.");
	//{Contraceptives}
	else {
		Render.text("It doesn't take you long to figure out that the pink pill should cancel the effects of your contraceptives.  You pop it into your mouth and swallow, feeling a tingle near your crotch after a moment.  You should be capable of bearing children again");
		if(!player.lowerBody.vaginaSpot.hasVagina()) Render.text(", should you ever grow a vagina");
		Render.text(".");
...
public takeBarrenPills():void {
	...
	if(player.statusAffects.has("Contraceptives")) Render.text("You're already under the effects of contraceptives.  Taking one of the brown pills wouldn't do anything.");
	//{TAKE DAT SHIT YO}
	else {
		Render.text("You figure one of these brown pills should render you barren, and you pop it into your mouth, not wanting to be impregnated.");
		if(player.pregnancyIncubation > 0) Render.text("  Of course, you're already pregnant, and this doesn't seem to be doing anything about THAT.");
		Render.text("  You do feel an emptiness in your midsection, reassuring you that the pill did its job.");
		if(!player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Now if you ever re-grow a vagina, you should be fine.");
...
public friendlyCumWitchBlessing():void {
	clearOutput();
	Render.text("You ask the Cum Witch if she could use her magic to gift you with some kind of blessing since she has such an affinity for sexualized magics and fertility.  ");
	if(player.stats.cor < 33) Render.text("Blushing, ");
	else if(player.stats.cor < 66) Render.text("Nervously, ");
	else Render.text("Boldly, ");
	Render.text("you remove your armor, figuring whatever blessing she's going to give is going to be at least a little bit sexual.");
	...
	menu();
	if(player.lowerBody.cockSpot.hasCock()) addButton(0,"Virility",cumWitchBlessed, true);
	if(player.lowerBody.vaginaSpot.hasVagina()) addButton(1,"Fertility",cumWitchBlessed,false);
	addButton(4,"Nevermind",changeMindAboutBlessings);
}
//*Nevermind ✓Kirbu
public changeMindAboutBlessings():void {
	clearOutput();
	Render.text("Thinking better of it, you grab your [armor] and get dressed, telling the jizz-obsessed enchantress that you don't need her gifts for now.");
...
public cumWitchBlessed(virility:boolean = true):void {
	...
	if(player.tongueType > TONUGE.HUMAN) Render.text("  Your inhuman tongue slides further and further out, following the witch's bulging, pre-cum filled urethra towards her balls where it can curl around them, embracing them with slick, slippery saliva.  You can taste her unique flavor on her skin, and it makes you shudder with arousal.");
	...
	if(virility) Render.text("[eachCock] drip and drool your own seed onto the floor.  The heat inside you is so hot that you can't keep your ardor penned up, and it's slowly boiling out of your [balls] in a pleasure-filled orgasm that provides everything but relief.  You're sure that touching yourself would give you a proper orgasm, but you keep your hands where they belong - on her balls.");
	else {
		Render.text("your [vagina] ");
		if(player.wetness() < 3) Render.text("drip");
		else if(player.wetness() < 4) Render.text("drool");
		else if(player.wetness() < 5) Render.text("leak");
		else Render.text("spray");
		Render.text(" your juicy secretions onto the ground below.  Your lust has risen to such unbearable levels that you're clenching and dripping in orgasmic delight without any release.  You're sure just a touch on your clit would have you cumming your brains out, but you keep your hands on her balls.");
	}
	...
	if(!virility) Render.text("womb");
	else Render.text("[balls]");
	Render.text(" that reminds you that you got exactly you asked for, even if she made you like it a little bit more than you would have liked...");
	Render.text("<b>");
	if(virility) {
...
public fuckOneSandWitch():void {
	...
	if(player.tentacleCocks() == 0) Render.text("You grab the tanned shoulders of the closest pregnant witch and flip her over on to her back, trapping her beneath the weight of her belly and breasts while you grab both of her ankles.  Her dirty blonde, sweat-soaked hair lies in waves around her head on the floor as she pants heavily in sexual desperation.");
	//{else if player has a tentacle cock}
	else Render.text("Your tentacle cock lashes outwards, extending as it goes, and wraps around the ankle of the closest girl. It flips her over on to her back, trapping her own torso beneath the weight of her heavy pregnancy and her own four massive leaking breasts.");
	//{if player has two tentacle cocks}
	if(player.tentacleCocks() > 1) Render.text("  Another tentacle cock reaches out for the girl's other ankle, and together your cocks spread her tanned legs wide, revealing her large double pussy bloated from her pregnancy.");
	else Render.text("  You reach forward with a hand to grab her other ankle and with your cock and arm you spread her tanned legs wide to reveal her large double pussy, bloated from her pregnancy.");
	...
	Render.text("  The witch's moans and grunts of pleasure arouse the other witches in the room as they watch with the utmost jealously.  Some grab their nipples while others try to finger their pussies; whining and getting off on their envious voyeurism as they watch you fuck their sister.");
	...
	if(multi) Render.text("s constantly pound");
	else Render.text(" constantly pounds");
	Render.text(" into her nethers. Drool escapes her lips and drips down her cheek even as her eyes roll back in her head. Talk about an easy lay.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 4 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 15) {
		...
		if(player.pregnancyIncubation > 0 && player.pregnancyIncubation < 200) Render.text("  Each time it does so your own breasts slap against your swollen midsection.");
		//{if player has four or more large breasts and is not pregnant}
		else if(player.bRows() >= 2) Render.text("  Your lower pair of breasts slap against the pregnant girl's belly with each thrust.");
	}
	//else if player has extremely huge breasts
		...
		if(player.bRows() == 1) Render.text("  Your two heavy milk sacs sway and at times you must rest them on top of the poor witch's belly to keep from prematurely exhausting yourself.");
		else if(player.bRows() >= 2) Render.text("  Resting on top of each other and on top of the witch's legs, your giant globes of tit flesh quake each time you hips move.");
		Render.text("  The witches around the room can't help but gawk at how much more endowed your mammaries are than even their own, and how the wrong forwards or backwards movement could result in either you or the witch you're fucking to be suddenly smothered under the weight of your ridiculously engorged tits.");
	}
	Render.text("\n\nIt isn't long before the sensations of naked wet flesh sliding along your ");
	if(multi) Render.text("shafts");
	else Render.text("shaft");
	Render.text(" forces you to reach your own orgasm.");
	if(player.lowerBody.balls > 0) Render.text("  Your [balls] tense with the anticipation of releasing the fruit of your loins into her waiting womb.");
	Render.text("  With both hands you reach up and grab the fat nipples of the witch's lower breasts, tugging on them like reins as you try to thrust deeper.  \"<i>Ung! Ah!</i>\" The witch bites her lower lip in pleasure and pain as you roughly handle her leaking teats.");
	//{if player has huge or massive breasts} 
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 15) Render.text("  The weight of your own tits bears heavily against your arms as you try to keep the motion of your hips constant while pulling on your lover's milk knobs.");
	//{if player's tits can lactate}
	else Render.text("  Each of your [nipples] begin spraying milk over the witch as your body begins its crescendo.");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.wetness() >= 4) Render.text("  Gobs of pussy juice splash against yours and the witch's legs as your orgasm forces your cunt to squirt over and over again.");
		if(player.lowerBody.balls > 0) Render.text("  Your [sack] is soaking wet by the time your pussy finishes cumming.");
	}
...
public fuckTwoPregWitches():void {
	...
	Render.text("These hormonally overwrought witches are each little more than five huge fleshy orbs of tits and bellies, desperately crawling towards you on hands and knees as their stares fixate on your " + cockDescript(y) + ".  ");
	//{if player has two long tentacle cocks} 
	if(player.tentacleCocks() >= 2) Render.text("Almost of their own accord your tentacle cocks lash out, wrapping around the arms of the two closest girls, lifting them to their knees only to roll them backwards and on to their sides with their backs to each other.  ");
	else Render.text("It takes little effort to grab the closest two witches and pull them up only to push them down again and on to their sides with their backs pressed against each other.  ");
	Render.text("The two witches lay before you with their heaving breasts and bellies jutting out to either side while their asses are mashed against each other.  You grab an ankle from each of them and raise their legs up to expose the dual pairs of pussies that they possess.");
	
	//{for non-serpent lower body}
	if(!player.isNaga()) Render.text("You straddle ");
	else Render.text("You entwine your serpentine tail around ");
	Render.text("their two legs that remain on the floor while allowing their raised legs to ");
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 15) Render.text("slide between your cleavage");
	//{for small or non-breasted characters}
	else Render.text("rest against your torso and shoulders");
	Render.text(".");
	if(player.pregnancyIncubation > 0 && player.pregnancyIncubation < 200) Render.text("  Each of these raised legs carefully bend at the knee to accomodate your own pregnant belly that looms over their prone figures.");
	Render.text("  As each witch lays on her side both of them begin feeling your stiff cocks slap against their multiple pussies.  They moan as their four pairs of labia bloom in anticipation of the fucking to come.  Each set of nether lips are dark and dripping with gobs of desperation, but on each girl there's one set that looks particularly gaping as if the pressure inside was forcing it open.  It isn't difficult to figure out that those are the ones that lead to their already occupied wombs, while the other sets are the ones eagerly awaiting a fresh deposit of baby batter.");
	...
	//{if player lactates}
	if(player.lactationQ() >= 200) Render.text("  Your own breasts begin spraying down the two pregnant sluts with your own dairy fluids.");
	//{if player has large but not huge breasts} 
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 20) Render.text("  Each thrust in to the eager cunts causes your breasts to bounce up and down.");
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 20) Render.text("  Each thrust in to the eager cunts causes your massive breasts to sway back and forth, slapping against your own sides.");
	//{if player has six huge breasts}
	if(player.bRows() >= 3) {
		if(player.upperBody.chest.list[2].breastRating >= 20) Render.text("  Your lower beach ball sized breasts are large enough to rest on the floor to either side as you fuck, and both witches eventually lower the legs they've had raised so as to hook them around your massive mammaries and hug your flesh even closer to their thighs.");
	}
	if(player.lowerBody.balls > 0) Render.text("  The closer your shafts come to their climax the more tense your balls become, preparing to pump their loads through your cocks.");
...
public fuckAllThePregWitches():void {
	...
	Render.text("\n\nWith your shaft");
	if(multi) Render.text("s");
	Render.text(" thoroughly swallowed up by the juicy confines of the witch");
	if(multi) Render.text("es");
	Render.text(", you begin rocking your hips to and fro, sliding yourself in and out of ");
	if(!multi) Render.text("her");
	else Render.text("their");
	Render.text(" velvety wet depths.  ");
	if(!multi) Render.text("Her");
	else Render.text("Their");
	Render.text(" blissful cries of pleasure fill the room, joined with a chorus of slurping sounds as you thrust your sensitive shafts ever deeper and harder.  ");
	if(!multi) Render.text("Her");
	else Render.text("Their");
	Render.text(" quadruple breasts jiggle and bounce along the floor and ");
	if(!multi) Render.text("her");
	else Render.text("their");
	Render.text(" pregnant ");
	if(!multi) Render.text("belly sways");
	else Render.text("bellies sway");
	Render.text(" with each ramming motion of your cock");
	if(multi) Render.text("s");
	Render.text(".  The muscles in ");
	if(!multi) Render.text("her");
	else Render.text("their");
	Render.text(" thighs and calves tense with pleasure as you push ");
	if(!multi) Render.text("her body");
	else Render.text("their bodies");
	Render.text(" to the limit.");
	//{if player has non-huge tits}
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 3 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 10) Render.text("  Your tits bounce as you enthusiastically grind away.");
	//{else if player has huge tits}
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 10 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 20) Render.text("  The heavy weight of your huge tits causes them to sway with every thrust of your hips.");
	//{if player has really huge tits} 
	else if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 20) Render.text("  The immense swells of your mammaries press down upon the ass of the girl in front of you.");
	//{if player has four non-huge tits} 
	if(player.bRows() >= 2) {
		if(player.upperBody.chest.list[1].breastRating >= 3) Render.text("  Your second row of breasts slap against the ass cheeks of the girl in front of you.");
	}
	...
	if(player.bRows() >= 3) {
		if(player.upperBody.chest.list[2].breastRating >= 3 && player.upperBody.chest.list[2].breastRating < 20) {
			Render.text("  Your lowermost row of breasts become squashed against the witch");
			if(multi) Render.text("s");
			Render.text("'s muscular thighs.");
		}
		else if(player.upperBody.chest.list[2].breastRating >= 20) {
			Render.text("  Your lowermost row of breasts come to rest on the floor as you lean forwards to fuck, and the witch");
			if(multi) Render.text("es");
			Render.text(" can almost support ");
			if(multi) Render.text("their");
			else Render.text("her");
			Render.text(" thighs against your obscene mammaries when they lean back against the constant ramming of your cock.");
		}
	}
	//{if player lactates}
	if(player.lactationQ() >= 200) {
		Render.text("  After only a short while of mindless rutting your tits begin leaking, and in short order your glistening nipples start spraying your milk over ");
		if(!multi) Render.text("the witch's tanned backside");
		else Render.text("the collective tanned bodies you're impaling");
		Render.text(".  A few other witches in the room eagerly step forth to suckle your [nipples] as they await their turn for impregnating.");
	}
	
	Render.text("\n\nThe cum inside your loins starts to move, filling the base of your shaft");
	if(multi) Render.text("s");
	Render.text(" as you feel your orgasm creep up inside you.  The sucking sensations of the ");
	if(!multi) Render.text("girl's cunt on your cock");
	else Render.text("the girls' cunts on your cocks");
	Render.text(" starts to become overwhelming, and before you realize it you're on the verge of releasing your seed.  The ");
	if(!multi) Render.text("pussy");
	else Render.text("pussies");
	Render.text(" you're pounding seem");
	if(!multi) Render.text("s");
	Render.text(" to sense the imminent release of the sperm ");
	if(!multi) Render.text("it craves");
	else Render.text("they crave");
	Render.text(" and strong contractions begin sucking on your shaft");
	if(multi) Render.text("s");
	Render.text(" even more fervently.  You can't help but give ");
	if(!multi) Render.text("it the load of baby batter it craves");
	else Render.text("them the loads of baby batter they crave");
	Render.text(".");
	
	Render.text("\n\nThe ");
	if(!multi) Render.text("underside of your shaft swells");
	else Render.text("undersides of your shafts swell");
	Render.text(" as your bounty of man-seed floods your full length and stretches the ");
	if(!multi) Render.text("witch's cunt");
	else Render.text("witches' cunts");
	Render.text(" even wider.  Pressing as far forwards as possible you align the ");
	if(!multi) Render.text("tip of your tool against her abused cervix");
	else Render.text("tips of your tools against their weakened cervixes");
	Render.text(" just as your cum begins erupting out of you.");
	
	//{if cumNormal-cumHigh}
	if(player.cumQ() < 1000) {
		Render.text("\n\nYou plaster the insides of ");
		if(!multi) Render.text("her pussy");
		else Render.text("their pussies");
		Render.text(" with no small amount squirting directly in to ");
		if(!multi) Render.text("her womb");
		else Render.text("their wombs");
		Render.text(".  When you finally pull out ");
		if(!multi) Render.text("a stream");
		else Render.text("streams");
		Render.text(" of cum are left behind.");
	}
	//{else if cumVeryHigh-cumExtreme]}
	else {
		Render.text("\n\nThe horny ");
		if(!multi) Render.text("witch isn't");
		else Render.text("witches aren't");
		Render.text(" prepared for the deluge that explodes from your genitals.  Liter after liter pours forth, filling up ");
		if(!multi) Render.text("her empty womb like a balloon");
		else Render.text("their empty wombs like a bunch of balloons");
		Render.text(" and causing your eyes to roll back in your head from the sheer pleasured sensation of emptying yourself.  When ");
		if(!multi) Render.text("her belly");
		else Render.text("their bellies");
		Render.text(" swell as large as the neighboring pregnancies and stretch marks start appearing up and down each side you finally pull out and allow your cock");
		if(multi) Render.text("s");
		Render.text(" to finish spraying ");
		if(!multi) Render.text("its");
		else Render.text("their");
		Render.text(" final loads over the ");
		if(!multi) Render.text("whorish witch's back");
		else Render.text("collective grouping of the witches' asses");
		Render.text(".");
	}
	
	//{all}
	if(!multi) Render.text("\n\nShe falls");
	else Render.text("\n\nThey fall");
	Render.text(" over.  Quickly, the rest of the witches in the room are eagerly raising their cunts in the air, begging for their turn at being impregnated.  Happily you oblige, shoving as much cock as you can into each wet hole that the hormonally overwrought sluts offer to you.  One pussy after another finds itself impaled on you, with you thrusting repeatedly until the massaging muscles within their cunts pull forth the rivers of baby gravy that their empty wombs are starving for.  One by one the witches find themselves filled and fall to the floor still reeling from the sensations, only for another new witch to take their place with a sopping cunt even more cum thirsty than the last.");
	
	//{if player has two or more cocks} 
	if(player.cockThatFits2(50) >= 0) Render.text("\n\nFor the final witch you choose to shove two cocks inside her empty pussy.  Your double dicked thrusts force a joyous yelp from her with every thrust.  \"<i>Ah! T-t-too much!</i>\" she screams but her vaginal canal refuses to let you go regardless.  When you finally finish the torrent of sperm makes her feel as if she's on the verge of bursting.  The swelling of her womb causes her belly to bloat up enough to push her quad breasts upwards and siginificantly shift her center of gravity.  The orgasm that accompanies the feeling of your sperm flooding forcefully into her uterus and tubes is too much for her to bear and she slides off of your cocks like a well used cock sleeve, oozing gobs of cum from between her dark nether lips.");
...
public milkBathsAhoy():void {
	...
	Render.text("  You could probably masturbate in the tub if you wanted to, or maybe pull the dusky milk-maid in for company....  What do you do?");
...
public grabTheMilkGirl():void {
	clearOutput();
	Render.text("You call out to the milk slave before she can slink away, and wade over to the side of the tub, leaving your face a few scant inches from her massive, milk-laden jugs. She cocks her head to the side, obviously unused to the attention, and quietly whispers, \"<i>" + player.mf("M-master?","M-mistress?") + "</i>\"");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 8) Render.text(", pressing deep into your own huge rack, your nipples flicking across her own leaky tits");
	Render.text(".");
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		...
		if(player.lowerBody.balls > 0) Render.text(", her other hand cupping your [balls], rolling the " + num2Text(player.lowerBody.balls) + " orbs in her palm with surprising dexterity");
		Render.text(".  Leaning over the two titanic teats between you, she traces a line of kisses down your back, licking up stray drops of milk between affectionate caresses.");
	}
	//{If PC has cooch:
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) Render.text("  She cups your breasts, having to reach so far around both your rack and hers that she's straining her arms to rub your [nipples], but she does so valiantly, stroking them with her incredibly deft fingers.");
		if(player.lactationQ() >= 200) Render.text("  A spurt of milk escapes your own full jugs, joining the pool-full of your new friend's.  She gasps with surprise and delight, quickly nuzzling herself into your back and going to work, milking you just as she would herself, letting the hefty flow of your motherly fluids pour into the pool, odd trickles smearing down your chest, staining your chest as white as her own.");
	}
	//[Fuck Her](PC must have gender; if cooch, also C+ cups) [Don't]
	menu();
	dynStats("lus", 33, "resisted", false);
	if(player.lowerBody.cockSpot.hasCock()) addButton(0,"Dick Fuck",fuckMilkbabeWithPenor);
	if(player.lowerBody.vaginaSpot.hasVagina() && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3) addButton(1,"Lady Fuck",ladyFucks);
...
public ladyFucks():void {
	...
	Render.text(".  Her hand slips up your body, brushing your vulva and [clit] before cupping your other breast, delicate fingers wrapping around your [nipple].  With deft, practiced motion, she works your teats between her fingers, working your breast like she might her own");
	if(player.lactationQ() >= 200) Render.text(", milking you with skill beyond anything you've ever experienced before; and why not, when her entire existence revolves around that self-same skill?");
	else Render.text(".");
	...
	if(player.lactationQ() >= 200) Render.text(", spraying milk all over yourself and the milkmaid who caused your explosive pleasure");
	Render.text(".  You run your fingers through the girl's hair, urging her sexual skills on as your chest heaves and quavers, riding out the massive boobgasm as fem-cum spurts from your cunt and into the milky pool below.");
	Render.text("\n\nYour entire body feels deeply refreshed, her milk having soaked into your body and making you feel fresh and revitalized, and every muscle seems to have relaxed thanks to your blissful coitus.  You start to thank the milk girl for the pleasurable company, but when you open your mouth, she presses her lips to yours for a long, tongue-filled kiss.  Chuckling to yourself, you hold the girl as tightly as her udders will allow, turning her to the side to let her nuzzle her cheek into your [chest], kissing the top of her head before the two of you climb from the pool.  You have to help her out, her massive extra weight nearly dragging her back in except for your quick reflexes.  You gather your [armor] and ruffle the milk slave's hair before turning back to the task at hand.");
	//[+Lust, +HP, -Fatigue]
	player.orgasm();
	fatigue(-15);
...
public drinkNFap():void {
	clearOutput();
	Render.text("\"<i>Wait,</i>\" you call out to the ebony woman, letting the milk obscure your hands as you begin to masturbate, \"<i>I want a drink.</i>\"  Sheepishly, the milk slave obliging shifts to drag her tits back into place.");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("[vagina] and idly stroke your puffy, lust-engorged vulva");
	else Render.text("[cock] and idly stroke the turgid mass");
	Render.text(", inadvertently drawing a lewd moan from yourself.  The fat nipple stuffed in your mouth does an adequate job of muffling your pleasurable vocalizations");
	if(player.lowerBody.cockSpot.hasCock()) Render.text(", but it does little to hide the swelling of [eachCock] - you have the milk for that");
	Render.text(".");
	...
	//(DA HERMS) 
	if(player.gender == 3) {
		Render.text("  Your [cock] throbs painfully in your hand, so hot and hard that you're sure you must have begun to leak precum, but any fluid is swiftly washed away by the ever-present milk.");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("  You make sure to fondle each of your members equally, caressing, squeezing, and stroking to the tempo of your swelling passion.");
		Render.text("  With your off-hand, you rub your cream-lubricated fingers through your sodden gash, the flesh parting easily to allow a few of your questing fingers inside.  Delicious bliss unfolds from your [clit] as it pushes free of its hood, fully engorging, faintly throbbing from aching need.  You brush the button a few times before going back to fingering your box, yet you make sure to strum your thumb across your clit every few moments to keep yourself as close to peak as possible.  Truly, being a hermaphrodite is bliss.");
	}
	//(DA SCHLICKS)
	else if(player.gender == 2) Render.text("  Your pussy juices mix freely with the tub's white-colored 'waters', allowing your cream-lubed fingers to plunge into your [vagina] with ease.  You stroke your lips and caress the interior of your birth canal with the intimate familiarity of a skilled lover, playing with your body until you feel your control slipping, so wound up with lust that you feel like an over-tightened guitar string vibrating out of control.");
	//(DA DUDES)
	else {
		Render.text("  Your [cock] throbs painfully in your hand, so hot and hard that you're sure you must have begun to leak precum, but any fluid is swiftly washed away by the ever-present milk.  ");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("You make sure to fondle each of your members equally, caressing, squeezing, and stroking to the tempo of your swelling passion.");
		else Render.text("  With your offhand, you cradle your [sack], hefting your [balls] as you feel your desire churning to new levels.");
	}
	//(TOGETHER)
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("  [EachCock] releases its own gooey load, sputtering and spurting to add to the pearly deluge.");
		if(player.cumQ() >= 500) Render.text("  With every torrent of seed you release, you can see it lift partway out of the tub, propelled by your incredible virility towards the nearest female specimen.");
		if(player.cumQ() >= 3000) Render.text("  Soon, the tub's fluid contents break out of their confines to stain your companion's dusky flesh white, an alabaster glaze that would entice you to further feats of debauchery were it not for the pleased contentment your maleness now radiates.");
	}
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("  Meanwhile, your juiced-up cunny feels like it's doing backflips inside you, contorting and squeezing as it gushes with fluid, feminine joy.  One brush on your [clit] knocks your [legs] out from underneath you, but thankfully, you float out the rest of your orgasm.");
	}
...
public sandPregUpdate():boolean {
	...
	if(player.pregnancyIncubation == 280) {
		...
		if(player.biggestLactation() < 1) Render.text("starting to lactate");
		else Render.text("lactating more");
		Render.text("?  Your belly is even bulging a little bit more than food can explain.\n");
		temp = player.bRows();
		while(temp > 0) {
			temp--;
