...
public processJackFrostEvent():void {
	...
	//No followers
		...
		Render.text("You sigh... this really reminds you of back home... you only wish you had someone to share this feeling with... Well, there is no reason you shouldn't enjoy yourself while it snows, so you set about rolling a big ball of snow to make a snowman out of...");
		//Skip to next day...
	...
	//Rathazul
		...
		Render.text("Rathazul approaches you.  \"<i>[name]?  What is going on?  It hasn't snowed in Mareth for years now.  And why only around the camp?  I wandered out to investigate, but outside there is no snow at all...</i>\" he asks, concerned.");
	...
	//Izma
		...
		Render.text("You wonder where Izma is; as a shark, maybe she doesn't like the cold?  Even as the thought crosses your mind, you note a surprisingly large pile of snow sitting not too far - with a very suspicious striped fin jutting out of it.  You rush over, calling for Izma and asking if she's all right.");
	...
	//Jojo
		...
		if(player.statusAffects.has("PureCampJojo")) {
			Render.text("Jojo is sitting on his usual rock, one hand out to catch snowflakes with a beatific smile of awe on his face.  \"<i>Look, [name], snow!  I haven't seen snow since I was a very, very small boy.</i>\"  He tells you.");
		...
		//Corrupt
			...
			Render.text("Out in the outskirts of the camp you notice your pet mouse-slut Jojo, casually fucking a small mound of snow like an animal.  You laugh to yourself, now, now... doesn't he know that there are better things to fuck than a pitiful mound of snow on the ground...");
			...
			menu();
			if(player.lowerBody.cockSpot.hasCock()) addButton(0,"Pen Ass",penetradeCorruptJojoAss);
			if(player.lowerBody.vaginaSpot.hasVagina()) addButton(1,"Ride Cock",rideCorruptJojoCock);
	...
	//less than 5!
		...
		if(!amilyScene.amilyCorrupt()) {
			Render.text("Amily is warily prowling across the icy surface, occasionally glancing all around as if trying to spot an ambush.  Other times, she suddenly leaps up in a flurry, whirling around in mid-air as something catches her over-large ears.  All in all, it's quite obvious she's scared.");
			...
			menu();
			if(player.lowerBody.cockSpot.hasCock()) addButton(0,"Penetrate",penetradePureMouseWaifu);
			addButton(1,"Ride Cock",makeOutWithPureMouseWaifu);
			return;
		}
		//Corrupt
		else {
			Render.text("Out in the outskirts where your favorite cum-slut lives, you spot her slowly rubbing the snow over her erect nipples, the cold making her shiver in pleasure.");
			...
			if(player.gender > 0) {
				...
				if(player.lowerBody.cockSpot.hasCock()) {
					...
					if(player.lowerBody.balls > 0) Render.text(", even tickling your [balls]");
					else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(", even tickling your [cunt] for added tantalization");
					Render.text(".");
	...
	//Tainted Ember
		...
		Render.text("The dragon is wandering your camp in open fascination.  \"<i>What is this weird white stuff?  It's cold and wet... but soft and fluffy, too,</i>\" " + emberScene.emberMF("he","she") + " says, gently catching a snowflake.");
	...
	//Marble (Written by TDM himself)
		...
		if(flags[FlagEnum.MARBLE_KIDS] > 0 && flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] >= 100) {
			Render.text("You find Marble simply staring into the sky, a look of wonder covering her face.  Dancing around her in excitement ");
			if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text("are ");
			else Render.text("is ");
			Render.text("your kid");
			if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text("s");
			Render.text(", playing in the snow, blowing the snow, and experimenting with the snow.  Exactly what ");
			if(flags[FlagEnum.MARBLE_KIDS] == 1) Render.text("she");
			else Render.text("they");
			Render.text(" are up to at any given moment is really hard to say, and you're not even sure that the little girl");
			if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text("s");
			Render.text(" know");
			if(flags[FlagEnum.MARBLE_KIDS] == 1) Render.text("s herself.");
			...
			Render.text(" playing together once more.  A moment later, Marble puts her hand to her expansive backend again, before suddenly exclaiming again in realization.  \"<i>Sweetie, watch our kid");
			if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text("s");
			Render.text(" for a moment, I'll be right back!</i>\" and she leaves the camp as quickly as she can, without slipping and falling on her ass again.");

			Render.text("\n\nYou certainly don't mind staying behind for now, it is certainly a lot of fun spending time with your kid");
			if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text("s");
			Render.text(" and just having fun.  Your cow-girl daughters are certainly not lacking in energy in the cold snow, their fur covered legs and built in shoes probably help a lot with this.  ");
			if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text("They");
			else Render.text("She");
			Render.text(" listen");
			if(flags[FlagEnum.MARBLE_KIDS] == 1) Render.text("s");
			Render.text(" with rapt attention when you explain what snow is, and about all the things you use to do in it back home.  Hands-on demonstrations are also in high demand.");
			...
			if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text("s");
			Render.text(" to ride on!  Happy cries of joy fill the air for some time, as you and Marble take turns in giving rides on the sled.");
			
			//PC strength check:
			//(>=85)
			if(player.str >= 85) {
				Render.text("\n\nIt would seem that your strength has won you more favor with ");
				if(flags[FlagEnum.MARBLE_KIDS] > 1) Render.text("the girls");
				else Render.text("your daughter");
				Render.text(" when it comes to giving sled rides, Marble doesn't seem to mind too much though.  Your mate actually decides to join in the riding part after a while!");
			} 
			//(60-84)
				...
				Render.text(" in deciding who gives the best rides.  In the end, a clear winner is never decided, but everyone agrees that it was a lot of fun.");
			}
			//(<=59)
			else {
	...
	//if she ain't a follower and is a lover, just have her visit.
		...
		Render.text("\"<i>Hey, champ, what's going on here?</i>\"  Helia calls as she sees you approach.  Gentle trails of steam are wafting from the salamander's body as her own internal heat melts any snow that builds up on her.");
		...
		if(player.gender > 0) {
			Render.text(", as evidenced by ");
			if(player.lowerBody.cockSpot.hasCock()) Render.text("your erect " + player.multiCockDescriptLight());
			if(player.gender == 3) Render.text(" and ");
			else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("your ");
			if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("dripping [vagina]");
		}
		Render.text(".");
		...
		//(If PC has a cock)
		if(player.lowerBody.cockSpot.hasCock()) Render.text("  A torrent of seed splashes against Helia's taut belly, as [eachCock] throbs and spews its seed while sandwiched between the two of you.");
		//(if PC has a pussy)
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Your [vagina] quivers and spill your juices with the force of a wave.  Moisture falls on the ground to join in with Helia's own discharge.");
	...
	//Kiha
		...
		menu();
		if(player.lowerBody.cockSpot.hasCock() && player.cockThatFits(67) >= 0) addButton(0,"Fuck Her",kihaXmasFuck);
		addButton(1,"Finger Her",kihaXmasFingering);
	}
	//Feast time! Just a collection of words on the pleasant day...
	else {
		Render.text("This small holiday of yours was much needed.  You feel reinvigorated and even more determined to put an end to this struggle with demons.");
		...
		//(if no sexable follower)
			...
			if(camp.companionsCount() > 1) Render.text("s");
			Render.text(".  You can tell that you were not the only one needing this break... but unfortunately all good things must come to an end, and so you bid your companion");
			if(camp.companionsCount() > 1) Render.text("s");
			Render.text(" good night and retire for the night.");
		}
		////more than one sexable follower.
		else {
			Render.text("You take care of the preparations and cooking, whipping up a delightful meal for you and your companions.  For a moment you look around - this small band you've formed feels enough like a family that you wouldn't mind having more days like this in the future.  You vow to work extra hard to make this a reality.");
...
public penetradePureMouseWaifu():void {
	...
	if(flags[FlagEnum.AMILY_WANG_LENGTH] > 0) Render.text(", not to mention her dick's pretty hard, too");
	Render.text(".");
	
	Render.text("\n\nSitting down on your bedroll, you open your arms, inviting Amily over.  The mouse promptly approaches");
	if(flags[FlagEnum.AMILY_WANG_LENGTH] > 0) Render.text(", her own " + amilyScene.amilyCock() + " jutting out in anticipation");
	Render.text(", wriggling into your arms and angling her pussy over your " + cockDescript(x) + ".  With the ease of long practice, she slides smoothly down its length, her warm, wet depths greedily inhaling your masculinity.  \"<i>Ah, nothing like a nice hard cock in your cunt to make you feel warmer,</i>\" she coos.");
	...
	if(flags[FlagEnum.AMILY_WANG_LENGTH] > 0) Render.text(" and painting your belly white with futa-spooge");
	Render.text(".");
...
public makeOutWithPureMouseWaifu():void {
	...
	if(flags[FlagEnum.AMILY_WANG_LENGTH] > 0) Render.text("Smiling sheepishly you  tell her to mind the erection... though you're not actually looking for sex, you can't help with how cute she looks...  \"<i>Flirt.</i>\"  Amily giggles, but otherwise doesn't comment on it.  ");
	Render.text("The mouse promptly walks over and sits down just in front of you, her back facing you and giving you a perfect opportunity to hug her.  Not keen on missing such an opportunity, you do so.");
...
public penetradeCorruptJojoAss():void {
	...
	if(player.cumQ() >= 700) Render.text(" all over his own cum-engorged gut and ");
	Render.text("onto the snowy earth underneath him.  He cums and cums until he's exhausted even his prodigious balls, creating a salty quagmire of semi-melted snow and spooge under him that he promptly collapses face-first into.");
...
public rideCorruptJojoCock():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text(", your own futa-cock spewing its load, which you deliberately spray in his face for amusement");
	Render.text(".  Your orgasm triggers Jojo's own; though he has cum without permission, at least he waited for you to cum first like a good slut should.  Rope after rope of mousy sperm bats your cervix, trying to get past the barrier and into the waiting womb inside, even as Jojo tries his best to fuck you over and over as he spills his load, until he finally slumps into the ground, panting, with his mouth open and his tongue lolling out, eyes glazed over in a pleasured daze.");
