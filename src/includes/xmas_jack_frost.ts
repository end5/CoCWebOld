...
public function processJackFrostEvent():void {
		...
		//Corrupt
			...
			if(player.gender > 0) {
				...
				if(player.lowerBody.cockSpot.hasCock()) {
					...
					if(player.lowerBody.balls > 0) outputText(", even tickling your [balls]");
					else if(player.lowerBody.vaginaSpot.hasVagina()) outputText(", even tickling your [cunt] for added tantalization");
					outputText(".");
	...
	//Marble (Written by TDM himself)
		...
		if(flags[FlagEnum.MARBLE_KIDS] > 0 && flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] >= 100) {
			...
			if(flags[FlagEnum.MARBLE_KIDS] > 1) outputText("are ");
			else outputText("is ");
			outputText("your kid");
			...
			if(flags[FlagEnum.MARBLE_KIDS] == 1) outputText("she");
			...
			if(flags[FlagEnum.MARBLE_KIDS] > 1) outputText("s");
			outputText(" know");
			if(flags[FlagEnum.MARBLE_KIDS] == 1) outputText("s herself.");
			...
			if(player.str >= 85) {
				...
				if(flags[FlagEnum.MARBLE_KIDS] > 1) outputText("the girls");
	...
	//if she ain't a follower and is a lover, just have her visit.
		...
		if(player.gender > 0) {
			...
			if(player.lowerBody.cockSpot.hasCock()) outputText("your erect " + player.multiCockDescriptLight());
			if(player.gender == 3) outputText(" and ");
			else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your ");
			if(player.lowerBody.vaginaSpot.hasVagina()) outputText("dripping [vagina]");
		}
		outputText(".");
		...
		//(If PC has a cock)
		if(player.lowerBody.cockSpot.hasCock()) outputText("  A torrent of seed splashes against Helia's taut belly, as [eachCock] throbs and spews its seed while sandwiched between the two of you.");
		//(if PC has a pussy)
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your [vagina] quivers and spill your juices with the force of a wave.  Moisture falls on the ground to join in with Helia's own discharge.");
...
public function penetradePureMouseWaifu():void {
	...
	if(flags[FlagEnum.AMILY_WANG_LENGTH] > 0) outputText(", not to mention her dick's pretty hard, too");
	outputText(".");
	
	outputText("\n\nSitting down on your bedroll, you open your arms, inviting Amily over.  The mouse promptly approaches");
	if(flags[FlagEnum.AMILY_WANG_LENGTH] > 0) outputText(", her own " + amilyScene.amilyCock() + " jutting out in anticipation");
	...
	if(flags[FlagEnum.AMILY_WANG_LENGTH] > 0) outputText(" and painting your belly white with futa-spooge");
	outputText(".");
...
public function makeOutWithPureMouseWaifu():void {
	...
	if(flags[FlagEnum.AMILY_WANG_LENGTH] > 0) outputText("Smiling sheepishly you  tell her to mind the erection... though you're not actually looking for sex, you can't help with how cute she looks...  \"<i>Flirt.</i>\"  Amily giggles, but otherwise doesn't comment on it.  ");
...
public function rideCorruptJojoCock():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", your own futa-cock spewing its load, which you deliberately spray in his face for amusement");
