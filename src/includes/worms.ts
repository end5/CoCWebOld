...
public function nightTimeInfestation():void {
	...
	if(player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockThickness >= 6) outputText("as tight around the girth as you can");
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() > 2) outputText("es are");
		else outputText(" is");
		outputText("already turning into ");
		if(player.lowerBody.cockSpot.count() == 2) outputText("an ");
