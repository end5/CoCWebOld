...
public wormToggle():void {
	spriteSelect(76);
	clearOutput();
	Render.text("While wandering, you come across a crudely illustrated sign.  It depicts an imp in obvious discomfort, covered in white little worms.  It looks as if one of them is even crawling into the imp's penis!\n\nHow do you feel about that?");
	simpleChoices("Aroused", wormsOn, "Grossed Out", wormsOff, "Who Cares?", wormsPartiallyOn, "", null, "", null);
}

private wormsOn():void {
	clearOutput();
	Render.text("You actually think it's kind of a hot idea, and wonder if such creatures actually exist in this land as you make your way back to camp.");
...
private wormsPartiallyOn():void {
	clearOutput();
	Render.text("You shrug and keep walking, not sure how you feel about the strange sign.");
	player.statusAffects.add(new StatusAffect("WormsOn", 0, 0, 0, 0)));
...
private wormsOff():void {
	clearOutput();
	Render.text("You shudder in revulsion and figure the sign to be the result of someone's perverted fantasy.");
...
private wormsConfront():void {
	spriteSelect(76);
	clearOutput();
	Render.text("You turn to confront the worms and combat begins!");
...
private wormsDoNothing():void {
	spriteSelect(76);
	clearOutput();
	Render.text("You do nothing, allowing the worms to enter combat range!");
	startCombat(new WormMass());
}
...
private wormsRun():void {
	...
	if(player.stats.spe > rand(35)) {
		Render.text("Your instincts overwhelm you and you immediately turn around and run like hell in the opposite direction. You look behind you as your heart feels as if it is about to burst only to discover that the creature did not follow you. You take a moment to catch your breath and consider yourself fortunate.");
		doNext(camp.returnToCampUseOneHour);
	}
	else {
		Render.text("You turn to run, but before your " + player.feet() + " can get you away, the worms are upon you!  You turn to face them, lest they launch onto your unprotected back.");
		startCombat(new WormMass());
	}
}
...
public wormAttack():void {
	...
	if(player.stats.spe - monster.spe > 0 && int(Math.random()*(((player.stats.spe-monster.spe)/4)+80)) > 80) {
		Render.text("The worm colony flails at you with its simulated arms, but its lack of coordination allows you to easily dodge its attack.\n", false);
		combatRoundOver();
		return;
	}
	//Evade
	...
	Render.text("The worm colony strikes at you with its makeshift limbs. It strikes you for ", false);
	Render.text(String(temp), false);
	Render.text(" damage and the limb splatters, dispersing the worms comprising the false arm.", false);	
	statScreenRefresh();
	Render.text("\n", false);
	combatRoundOver();
	return;
...
public wormsEntice():void {
	...
	//FAIL
	if(rand(2) == 0) {
		if(player.lust < 50) Render.text("The worm colony stands before you and begins secreting a significant amount of slime. You are perplexed as to why the worms have done this. You shrug your shoulders and remain on guard.\n", false);
		else Render.text("The worm colony shambles over to you and attempts to grapple you. Quickly sidestepping the clumsy movements of the creature, you avoid what could have been a horrible fate as the mass falls over and splatters in its failed attempt to engulf you.\n", false);
		combatRoundOver();
		return;
	}
	//SUCCESS
	if(player.lust < 50) {
		Render.text("The worm colony stands before you and begins secreting a significant amount of slime. Inexplicably, you find that your " + cockDescript(0) + " is already erect and is throbbing. The erection is quite meddlesome and you find yourself distracted by the unwanted arousal.\n", false);
		dynStats("lus", 10+player.stats.lib/20+player.stats.cor/20);
	}
	else {
		Render.text("The worm colony shambles over to you and attempts to grapple you. Attempting to dodge, you fail to get away fast enough and fall to the ground engulfed by the mass. You are completely covered in the slimy worms!!! Incapable of avoiding any of their movements, you feel their slime coat every inch of your body and you feel the struggle and strain of each individual worm as they crawl all over you. You immediately begin flailing wildly as you cannot even breathe!", false);
		//Chance of insta-loss if infested twice
		if(player.statusAffects.has("InfestAttempted")) {
			Render.text("  Struggle as you might, the creatures overwhelm your body and prevent you from any conceivable opportunity to get them off you, Your head quickly becomes visible, allowing you to breathe as you stare helplessly at the cocoon of worms trapping you.\n\n", false);
			infest1();
			return;
		}
		//Escaped!
...
//Gone	menuLoc = 0;
	if(player.fatigue + physicalCost(40) > 100) {
		Render.text("You try to summon up an orgasm, but you're too tired and waste your time trying!");
...
public nightTimeInfestation():void {
	...
	Render.text(" open and vulnerable to the piles of spunk-hungry parasites, and as you look on in shock, you see four of them slide into your worm-packed tunnel");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(", one after another!  No!  You reach for your [cock biggest] to somehow try and stop the infestation, but it's a futile gesture.  The worm pile on your [legs] is as big as the rest of you.  Worse still, you doubt you can pull out the ones that have already crawled inside.");
	
	Render.text("\n\nGrabbing hold of [oneCock] with both hands, you pinch your fingers around ");
	if(player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockThickness >= 6) Render.text("as tight around the girth as you can");
	else Render.text("tightly around your girth");
	Render.text(", hoping to contain the squiggling menace in the upper reaches of your erection.");
	if(player.lowerBody.cockSpot.count() > 1) {
		Render.text("  It's a vain effort, as your other penis");
		if(player.lowerBody.cockSpot.count() > 2) Render.text("es are");
		else Render.text(" is");
		Render.text("already turning into ");
		if(player.lowerBody.cockSpot.count() == 2) Render.text("an ");
		Render.text("infestation highway");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" to absorb the sensual nutrition.  Some of them even slop off the sides after the dripping whiteness, to get to it before it absorbs into the brittle, wasted earth.  You cum and cum, almost without end, yet each thick jet is slurped into the writhing mass immediately, leaving behind just the parasitic, clear slime.");
