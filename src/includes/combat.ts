...
//No longer used:		itemSwapping = false;
		...
		//Player lost
			...
			if(flags[FlagEnum.COMBAT_BONUS_XP_VALUE] > 0) {
				...
				outputText("  Somehow you managed to gain " + flags[FlagEnum.COMBAT_BONUS_XP_VALUE] + " XP from the situation.");
				flags[FlagEnum.COMBAT_BONUS_XP_VALUE] = 0;
			}
			//Bonus lewts
			if (flags[FlagEnum.BONUS_ITEM_AFTER_COMBAT_ID] != "") {
				outputText("  Somehow you came away from the encounter with " + ItemType.lookupItem(flags[FlagEnum.BONUS_ITEM_AFTER_COMBAT_ID]).longName + ".\n\n");
...
public function lustAttack():void {
	if (player.lust < 35) {
		...
		else if (player.lowerBody.vaginaSpot.count() > 0) outputText(vaginaDescript(0) + " get wetter ");
...
private function wait():void {
	...
	if (monster.statusAffects.has("PCTailTangle")) {
		...
		else outputText("The creature continues probing at your asshole and has now latched " + num2Text(player.upperBody.chest.countNipples()) + " more suckers onto your nipples, amplifying your growing lust.  You must escape or you will become a mere toy to this thing!");
...
public function bite():void {
	...
	if(player.statusAffects.has("Blind")) outputText("In hindsight, trying to bite someone while blind was probably a bad idea... ", false);
	let damage:number = 0;
	//Determine if dodged!
	if((player.statusAffects.has("Blind") && rand(3) != 0) || (monster.spe - player.stats.spe > 0 && int(Math.random()*(((monster.spe-player.stats.spe)/4)+80)) > 80)) {
		if(monster.spe - player.stats.spe < 8) outputText(monster.capitalA + monster.short + " narrowly avoids your attack!", false);
		if(monster.spe - player.stats.spe >= 8 && monster.spe-player.stats.spe < 20) outputText(monster.capitalA + monster.short + " dodges your attack with superior quickness!", false);
		if(monster.spe - player.stats.spe >= 20) outputText(monster.capitalA + monster.short + " deftly avoids your slow attack.", false);
		outputText("\n\n", false);
		enemyAI();
		return;
	}
...
//This is now automatic - newRound arg defaults to true:	menuLoc = 0;
	...
	if(monster.spe - player.stats.spe > 0 && int(Math.random()*(((monster.spe-player.stats.spe)/4)+80)) > 80) {
		if(monster.spe - player.stats.spe < 8) outputText(monster.capitalA + monster.short + " narrowly avoids your stinger!\n\n");
		if(monster.spe - player.stats.spe >= 8 && monster.spe-player.stats.spe < 20) outputText(monster.capitalA + monster.short + " dodges your stinger with superior quickness!\n\n");
		if(monster.spe - player.stats.spe >= 20) outputText(monster.capitalA + monster.short + " deftly avoids your slow attempts to sting " + monster.pronoun2 + ".\n\n");
		enemyAI();
		return;
	}
...
//This is now automatic - newRound arg defaults to true:	menuLoc = 0;
	...
	if(player.statusAffects.has("ThroatPunch")) {
		...
		if(player.statusAffects.get("ThroatPunch").value1 >= 0) outputText("Thanks to Isabella's wind-pipe crushing hit, you're having trouble breathing and are <b>unable to cast spells as a consequence.</b>\n\n", false);
		else {
...
//	else
	...
	if(player.statusAffects.has("Blind")) outputText("It's impossible to see anything!\n");
	else {
...
public function tease(justText:boolean = false):void {
	...
	switch(select) {
		...
		case 0:
			...
			if(player.lowerBody.butt.buttRating >= 10 && player.tone < 60) outputText(", making it jiggle delightfully.", false);
		...
		case 2:
			if(player.isTaur()) {
				...
				if(player.lowerBody.cockSpot.count() > 0) {
					chance++;
					damage++;
					if(player.lowerBody.cockSpot.count() == 1) outputText(player.cockDescript(0), false);
					if(player.lowerBody.cockSpot.count() > 1) outputText(player.multiCockDescriptLight(), false);
		...
		case 6:
			if (player.perks.has("BimboBrains") || player.perks.has("FutaFaculties")) {
				...
				else if(player.perks.has("BimboBody")) outputText("bimbo ", false);
			...
			if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) outputText("  You smile as your " + clitDescript() + " swells out from the folds and stands proudly, begging to be touched.", false);
			else outputText("  You smile and pull apart your lower-lips to expose your " + clitDescript() + ", giving the perfect view.", false);
			if(player.lowerBody.cockSpot.count() > 0) outputText("  Meanwhile, " + player.sMultiCockDesc() + " bobs back and forth with your gyrating hips, adding to the display.", false);
			//BONUSES!
			if(player.lowerBody.cockSpot.hasCock()) {
				if(player.perks.has("BulgeArmor")) damage += 5;
				penis = true;
		...
		case 8:
			...
			if(player.perks.has("BroBrains")) outputText("Damn, " + monster.a + monster.short + " has got to love this!", false);
		...
		case 9:
			...
			if(player.perks.has("BroBrains")) outputText(monster.capitalA + monster.short + " can't resist such a heroic pose!", false);
		...
		case 10:
			...
			if(player.perks.has("BroBrains")) outputText("No way could " + monster.pronoun1 + " resist your huge cock!", false);
		...
		case 11:
			if(silly() && rand(2) == 0) outputText("You strike a herculean pose and flex, whispering, \"<i>Do you even lift?</i>\" to " + monster.a + monster.short + ".", false);
				...
				if(player.perks.has("BroBrains")) outputText("Bitches love a cum-leaking cock.", false);
		...
		case 18:
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) outputText("parted cunt-lips", false);
		...
		case 39:
			...
			if(player.lowerBody.tailVenom == 1) outputText("your");
			else outputText("a");
		...
		case 40:
			...
			if(player.lowerBody.tailVenom == 1) outputText("  Your tail twists and whips about, sliding around your " + hipDescript() + " in a slow arc and framing your rear nicely as you slowly lift your " + player.armorName + ".");
			...
			outputText("  As your [butt] comes into view, you brush your tail" + ((player.lowerBody.tailVenom > 1) ? "s" : "" ) + " across it, partially obscuring the view in a tantalizingly teasing display.");
			ass = true;
			anus = true;
			chance++;
			damage += 2;
		...
		case 41:
			...
			if(player.lowerBody.cockSpot.count() > 0) outputText(player.sMultiCockDesc());
			if(player.gender == 3) outputText(" and ");
			if(player.gender >= 2) outputText("your " + vaginaDescript(0));
			outputText(".  Your bushy tail" + ((player.lowerBody.tailVenom > 1) ? "s" : "" ) + " cross" + ((player.lowerBody.tailVenom > 1) ? "": "es") + " in front, wrapping around your genitals and obscuring the view teasingly.");
			vagina = true;
			penis = true;
			damage += 2;
			chance++;
...
//This is now automatic - newRound arg defaults to true:	menuLoc = 0;
	...
	if(monster.vaginas.length > 0) {
		if(monster.plural) {
			if(monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.NORMAL) outputText(monster.capitalA + monster.short + "'s " + monster.vaginaDescript() + "s dampen perceptibly.  ", false);
			if(monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.WET) outputText(monster.capitalA + monster.short + "'s crotches become sticky with girl-lust.  ", false);
			if(monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLICK) outputText(monster.capitalA + monster.short + "'s " + monster.vaginaDescript() + "s become sloppy and wet.  ", false);
			if(monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.DROOLING) outputText("Thick runners of girl-lube stream down the insides of " + monster.a + monster.short + "'s thighs.  ", false);
			if(monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING) outputText(monster.capitalA + monster.short + "'s " + monster.vaginaDescript() + "s instantly soak " + monster.pronoun2 + " groin.  ", false);
		}
		else {
			if(monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.NORMAL) outputText(monster.capitalA + monster.short + "'s " + monster.vaginaDescript() + " dampens perceptibly.  ", false);
			if(monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.WET) outputText(monster.capitalA + monster.short + "'s crotch becomes sticky with girl-lust.  ", false);
			if(monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLICK) outputText(monster.capitalA + monster.short + "'s " + monster.vaginaDescript() + " becomes sloppy and wet.  ", false);
			if(monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.DROOLING) outputText("Thick runners of girl-lube stream down the insides of " + monster.a + monster.short + "'s thighs.  ", false);
			if(monster.lust >= 60 && monster.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING) outputText(monster.capitalA + monster.short + "'s " + monster.vaginaDescript() + " instantly soaks her groin.  ", false);
		}
	}
	outputText("\n\n", false);
	doNext(playerMenu);
...
//This is now automatic - newRound arg defaults to true:	menuLoc = 0;
	...
	if(rand(4) == 0) {
		...
		if(player.gender == 1) {
			if(player.lowerBody.cockSpot.count() == 1) outputText(player.cockDescript(0) + " twitches obscenely and drips with pre-cum as your libido spins out of control.", false);
...
public function kick():void {
	...
	if(player.lowerBody == LOWER_BODY.KANGAROO) {
		//(tail)
		if(player.tailType == TAIL.KANGAROO) outputText("You balance on your flexible kangaroo-tail, pulling both legs up before slamming them forward simultaneously in a brutal kick.  ", false);
		//(no tail) 
	...
	//(bunbun kick) 
	else if(player.lowerBody == LOWER_BODY.BUNNY) outputText("You leap straight into the air and lash out with both your furred feet simultaneously, slamming forward in a strong kick.  ", false);
	//(centaur kick)
	else if(player.lowerBody == LOWER_BODY.CENTAUR) outputText("You lurch up onto your backlegs, lifting your forelegs from the ground a split-second before you lash them out in a vicious kick.  ", false);
	//(bipedal hoof-kick) 
	else if(player.lowerBody == LOWER_BODY.HOOFED) outputText("You twist and lurch as you raise a leg and slam your hoof forward in a kick.  ", false);
...
//This is now automatic - newRound arg defaults to true:	menuLoc = 0;
	...
	if(rand(5) == 0 || player.statusAffects.has("WebSilence")) {
		if(player.statusAffects.has("WebSilence")) outputText("You reach for the terrestrial fire, but as you ready to release a torrent of flame, it backs up in your throat, blocked by the webbing across your mouth.  It causes you to cry out as the sudden, heated force explodes in your own throat.\n\n", false);
		else if(player.statusAffects.has("GooArmorSilence")) outputText("You reach for the terrestrial fire but as you ready the torrent, it erupts prematurely, causing you to cry out as the sudden heated force explodes in your own throat.  The slime covering your mouth bubbles and pops, boiling away where the escaping flame opens small rents in it.  That wasn't as effective as you'd hoped, but you can at least speak now.");
...
//		doNext(combatMenu);
	...
	if(monster.short == "Ember") {
		...
		if(player.stats.spe > rand(monster.spe + escapeMod) || (player.perks.has("Runner") && rand(100) < 50)) {
			if(player.perks.has("Runner")) outputText("Using your skill at running, y");
			else outputText("Y");
	...
	//FAIL FLEE
		...
		//fail
		else if(player.tailType == TAIL.RACCOON && player.upperBody.head.earType == EARS.RACCOON && player.perks.has("Runner")) outputText("Using your running skill, you build up a head of steam and jump, but before you can clear the ground more than a foot, your opponent latches onto you and drags you back down with a thud!");
		//Nonflyer messages
		else {
			//Huge balls messages
			if(player.lowerBody.balls > 0 && player.lowerBody.ballSize >= 24) {
				if(player.lowerBody.ballSize < 48) outputText("With your " + ballsDescriptLight() + " swinging ponderously beneath you, getting away is far harder than it should be.  ", false);
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 35 || player.lowerBody.butt.buttRating >= 20 || player.lowerBody.hipRating >= 20)
			{
				...
				if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 35 && player.upperBody.chest.BreastRatingLargest[0].breastRating < 66)
				{
					if(player.lowerBody.hipRating >= 20)
					{
						...
						if(player.lowerBody.butt.buttRating >= 20) outputText(buttDescript() + " and ", false);
					...
					else if(player.lowerBody.butt.buttRating >= 20) outputText("Your " + player.skinTone + buttDescript() + " and " + chestDesc() + " wobble and bounce heavily, throwing you off balance and preventing you from moving quick enough to escape.", false);
				...
				//FOR PLAYERS WITH MASSIVE BREASTS
					...
					if(player.lowerBody.hipRating >= 20) {
						...
						if(player.lowerBody.butt.buttRating >= 20) outputText("causing the fat of your " + player.skinTone + buttDescript() + " to wobble heavily, ", false);
						outputText("forcing your body off balance and preventing you from moving quick enough to get escape.", false);
					}
					else if(player.lowerBody.butt.buttRating >= 20) outputText("Your " +chestDesc() + " nearly drag along the ground while the fat of your " + player.skinTone + buttDescript() + " wobbles heavily from side to side, forcing your body off balance and preventing you from moving quick enough to escape.", false);
				...
				//FOR PLAYERS WITH EITHER GIANT HIPS OR BUTT BUT NOT THE BREASTS
					...
					if(player.lowerBody.butt.buttRating >= 20) outputText("causing your " + player.skinTone + buttDescript() + " to wobble obscenely ", false);
					outputText("and forcing your body into an awkward gait that slows you down, preventing you from escaping.", false);
				}
				//JUST DA BOOTAH
				else if(player.lowerBody.butt.buttRating >= 20) outputText("Your " + player.skinTone + buttDescript() + " wobbles so heavily that you're unable to move quick enough to escape.", false);
			}
...
public function doCombat(eventNum:number):void
{
	...
	if(eventNum == 5019) {
			...
			if (player.lowerBody.cockSpot.count() > 0) {
				if (player.lust >= 60 && player.lowerBody.cockSpot.count() > 0) outputText("You feel your " + player.multiCockDescriptLight() + " dribble pre-cum.", false);
				if (player.lust >= 30 && player.lust < 60 && player.lowerBody.cockSpot.count() == 1) outputText("Your " + player.cockDescript(0) + " hardens, distracting you further.", false);
				if (player.lust >= 30 && player.lust < 60 && player.lowerBody.cockSpot.count() > 1) outputText("Your " + player.multiCockDescriptLight() + " harden uncomfortably.", false);
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText("  ", false);
			}
			if (player.lowerBody.vaginaSpot.count() > 0) {
				if (player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.NORMAL && player.lowerBody.vaginaSpot.count() == 1) outputText("Your " + allVaginaDescript() + " dampens perceptibly.", false);
				if (player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.NORMAL && player.lowerBody.vaginaSpot.count() > 1) outputText("Your " + allVaginaDescript() + " dampen perceptibly.", false);
				if (player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.WET && player.lowerBody.vaginaSpot.count() > 0) outputText("Your crotch becomes sticky with girl-lust.", false);
				if (player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLICK && player.lowerBody.vaginaSpot.count() == 1) outputText("Your " + allVaginaDescript() + " becomes sloppy and wet.", false);
				if (player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLICK && player.lowerBody.vaginaSpot.count() > 1) outputText("Your " + allVaginaDescript() + " become sloppy and wet.", false);
				if (player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.DROOLING && player.lowerBody.vaginaSpot.count() > 0) outputText("Thick runners of girl-lube stream down the insides of your thighs.", false);
				if (player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING && player.lowerBody.vaginaSpot.count() == 1) outputText("Your " + allVaginaDescript() + " instantly soaks your groin.", false);
				if (player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING && player.lowerBody.vaginaSpot.count() > 1) outputText("Your " + allVaginaDescript() + " instantly soak your groin.", false);
			}
			outputText("\n", false);
		...
		if(monk == 2 || monk == 3)
		{
				...
				if (player.lust >= 100) {
					...
					if (player.gender >= 2) {
						...
						if (player.lowerBody.cockSpot.count() > 1) outputText("Your " + player.cockDescript(0) + " splatters the ground with cum repeatedly, until both your genders are raw and sore.  ", false);
	...
	if(eventNum == 5036) {
			...
			//Determine if dodged!
			if (player.stats.spe - monster.spe > 0 && int(Math.random() * (((player.stats.spe - monster.spe) / 4) + 80)) > 80) {
				if (player.stats.spe - monster.spe < 8) outputText("You narrowly avoid " + monster.a + monster.short + "'s stinger!", false);
				if (player.stats.spe - monster.spe >= 8 && player.stats.spe - monster.spe < 20) outputText("You dodge " + monster.a + monster.short + "'s stinger with superior quickness!", false);
				if (player.stats.spe - monster.spe >= 20) outputText("You deftly avoid " + monster.a + monster.short + "'s slow attempts to sting you.", false);
				combatRoundOver();
				return;
			}
			//determine if avoided with armor.
			...
			if (rand(2) == 0) {
				...
				if (player.lust > 60) {
					...
					if (player.stats.sens > 50) outputText("  The sensitive nubs of your nipples rub tightly under your " + player.armorName + ".", false);
				}
	...
	if(eventNum == 5044) {
			...
			if (player.lust >= 35 && player.lust < 65) {
				...
				if (player.lowerBody.cockSpot.count() > 0) outputText(player.multiCockDescriptLight() + " hardening ", false);
				else if (player.lowerBody.vaginaSpot.count() > 0) outputText(vaginaDescript(0) + " get wetter ", false);
	...
	if(eventNum == 5071) {
			...
			if (monster.statusAffects.has("PCTailTangle")) {
				...
				if (player.lowerBody.cockSpot.count() > 0) outputText("The creature continues spiraling around your cock, sending shivers up and down your body. You must escape or this creature will overwhelm you!", true);
				else if (player.lowerBody.vaginaSpot.hasVagina()) outputText("The creature continues sucking your clit and now has latched two more suckers on your nipples, amplifying your growing lust. You must escape or you will become a mere toy to this thing!", true);
				else outputText("The creature continues probing at your asshole and has now latched " + num2Text(player.upperBody.chest.countNipples()) + " more suckers onto your nipples, amplifying your growing lust.  You must escape or you will become a mere toy to this thing!", true);
				dynStats("lus", (8 + player.stats.sens / 10));
				combatRoundOver();
				return;
			}
	...
	if(eventNum == 5077) {
			...
			if (monster.statusAffects.has("PCTailTangle")) {
				...
				//Fail to break free
					...
					outputText("Despite trying to escape, the creature only tightens its grip, making it difficult to breathe.\n\n", false);
					takeDamage(5);
					if (player.lowerBody.cockSpot.count() > 0) outputText("The creature continues spiraling around your cock, sending shivers up and down your body. You must escape or this creature will overwhelm you!", false);
					else if (player.lowerBody.vaginaSpot.hasVagina()) outputText("The creature continues sucking your clit and now has latched two more suckers on your nipples, amplifying your growing lust. You must escape or you will become a mere toy to this thing!", true);
					else outputText("The creature continues probing at your asshole and has now latched " + num2Text(player.upperBody.chest.countNipples()) + " more suckers onto your nipples, amplifying your growing lust.  You must escape or you will become a mere toy to this thing!", true);
					dynStats("lus", (3 + player.stats.sens / 10 + player.stats.lib / 20));
					combatRoundOver();
	...
	if(eventNum == 5079) {
			...
			//[Bow Response]
			if (monster.short == "Isabella") {
				if (monster.statusAffects.has("Blind")) outputText("Isabella hears the shot and turns her shield towards it, completely blocking it with her wall of steel.\n\n", false);
				else outputText("You arrow thunks into Isabella's shield, completely blocked by the wall of steel.\n\n", false);
