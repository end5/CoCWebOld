...
public function heliaDiscovery():void {
	...
	if(flags[FlagEnum.HEL_ISABELLA_THREESOME_ENABLED] == 0) {
		...
		//[(pussy)
		if(player.stats.cor < 50) Render.text("quickly rush over and stop Hel before she loses a leg to one of your traps");
		//(dick)
		else Render.text("lazily point out your traps to the not-quite-intruder");
		Render.text(", and guide her over to the camp proper.");
	}
	//(Else)
	else {
	...
	//[If not Centaur: 
	if(!player.isTaur()) Render.text("  Her hand slides over to rest on your thigh.");
	else Render.text("  \"<i>She gives your flank a slow, affectionate stroke.</i>\"");
	Render.text("  You ask her if that's true. With a little wink, she answers, \"<i>Well, it's not entirely untrue...</i>\"");
...
public function noDungeon():void {
	clearOutput();
	Render.text("You consider for a few moments, but ultimately decide that this is a venture you'd rather not participate in.");
...
public function agreeToHelpHeliaDungeon():void {
	clearOutput();
	Render.text("You mull the salamander's proposition over and eventually agree to assist her.  Not only will you be stopping a new race of monsters from spawning into the mountains, but you'll be getting into the lovely Helia's good graces - a win-win if ever there was one.");
	...
	//[if has lap: 
	if(!player.isTaur()) Render.text(" and cuddling up in your lap");
	Render.text(".  She nuzzles your neck and whispers, \"<i>Thanks, Champ. It means a lot to know I can count on you to... watch my back.</i>\"");
...
public function morningAfterHeliaDungeonAgreements():void {
	...
	//[If PC has >C Cups, \"<i>
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) Render.text("her head nestled between your soft tits and ");
	Render.text("snoring boorishly.  The air around you smells like hot booze and sex, yet you awaken feeling as spirited and lively as you ever have.  You give Hel a little shake, waking her.");
...
public function notYet():void {
	clearOutput();
	Render.text("You tell Hel you were only checking on her, and that you've still got some things to do.  She sighs and quietly asks you to hurry.");
	doNext(playerMenu);
}
//Introduction -- Dungeon
public function goToHeliaDungeon():void {
	clearOutput();
	Render.text("You tell Helia that yeah, you're as ready as you'll ever be.  She beams and grabs you in a tight hug.  \"<i>Thanks again, [name].  You're a real goddamn champion, you know that?</i>\"  You laugh it off, but the salamander gives you a sultry wink and starts off toward the mountains.  You're quick to follow her.");
	//(NEXT)
	doNext(goToHeliaDungeon2);
}
public function goToHeliaDungeon2():void {
	clearOutput();
	Render.text("Within the hour, you and Helia are hiking up the narrow ledges and crevices of the high mountains, slowly but steadily climbing toward a snow-capped peak.  Hel certainly seems to know where she's going - she blazes a certain and steady trail, as if she knows every path and shortcut up the mountain.  By the time you near the peak, you're convinced she's been up here before - many times, even.");
...
public function takeGooArmor():void {
	clearOutput();
	Render.text("You approach the armor rack.  A suit of heavy plated armor sits upon it, overlaying a flexible chain vest.  Contrasting against the rotting room, the armor seems to be in pristine condition, even shining.  Perhaps someone uses this heavy equipment - but surely not a harpy? You suppose you could take it.");
...
public function gooArmorAttackPhysical():void {
	if(combatMiss()) {
		Render.text("The goo-armor rushes forward and swings her sword in a mighty arc, but you dodge it!");		
	}
	else if(combatEvade()) {
		Render.text("The goo-armor rushes forward and swings her sword in a mighty arc, but you evade the attack!");
	}
	else if(combatFlexibility()) {
		Render.text("The goo-armor swings a greatsword at you in a mighty arc, but your cat-like flexibility makes it easy to twist out of the way.");		
	}
	else if(combatMisdirect()) {
		Render.text("The goo-armor swings a sword at you in a mighty arc, but your training with Raphael allows you to misdirect her into a miss!");	
	}
	//HIT!
	else {
		Render.text("The goo-armor rushes forward and swings her sword in a mighty arc.  You aren't quite quick enough to dodge her blow, and the goopy sword slams into you, throwing you back and leaving a nasty welt.");
...
public function struggleAtGooBind():void {
	...
	if(rand(10) > 0 && player.str/5 + rand(20) < 23) {
		Render.text("You try and get out of the goo's grasp, but every bit of goop you pull off you seems to be replaced by twice as much!");
	...
	//If succeed: 
		...
		Render.text("You finally pull the goop off of you and dive out of her reach before the goo-girl can re-attach herself to you.  Pouting, she refills her suit of armor and reassumes her fighting stance.");
...
public function gooSilenceAttack():void {
	Render.text("The goo pulls a hand off her greatsword and shoots her left wrist out towards you.  You recoil as a bit of goop slaps onto your mouth, preventing you from speaking - looks like you're silenced until you can pull it off!");
...
public function gooArmorBeatsUpPC():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) {
			Render.text("  As she hammers your cunny, bits of her goo swirl around your [chest], squeezing and massaging your tits.  You squirm as she roughly teases your boobs, pinching at your nipples and squeezing your tender flesh roughly.");
			//[if PC is lactating: \"<i>
			if(player.lactationQ() > 0) Render.text("  To her delight, a spray of warm milk jets out of your sore nipples, milky white mixing into blue goo like oil in water. \"<i>Mmm, tasty!</i>\" she teases, massaging more and more from you.</i>\"");
		}
		...
		if(player.tallness < 70) Render.text("little");
		else Render.text("big");
		Render.text(" girl ready to cum?  Well, go on then. Feed me!</i>\"");
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1 && player.lactationQ() > 0) Render.text("and milk ");
		Render.text("swirling around inside her.  \"<i>Well, I suppose since you fed me so well, I'll let you go.  This time! See you around, tasty!</i>\"");
	...
	//(PC has Dick)
		...
		Render.text("She begins to use her goo to peel back your armor, soon revealing your defenseless, half-erect cock");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(".  She makes a show of licking her lips as tendrils of goo wrap tightly around [eachCock] like a warm, wet onahole. You writhe and struggle against your gooey bonds, but your efforts are futile.  The goo-girl inside the armor only shakes her head at you, and squeezes [eachCock] tighter.");
		...
		Render.text(" " + player.mf("boy","girl") + " ready to cum? Well, go on then. Feed me!</i>\"");
...
public function beatUpGooArmor():void {
	spriteSelect(79);
	clearOutput();
	Render.text("Succumbing to your ");
	if(monster.lust > 99) Render.text("erotic abilities");
	else Render.text("skill in battle");
	Render.text(", the armored goo slumps backwards against the wall, unable to stand.  You loom over her, grinning as you contemplate what to do with your helpless opponent.");
...
public function refuseGooArmorOffer():void {
	spriteSelect(79);
	clearOutput();
	Render.text("You tell her to fuck off -- you don't need armor that might try to kill or rape you at night.");
...
public function takeGooArmorAndWearIt():void {
	spriteSelect(79);
	clearOutput();
	Render.text("You mull the proposition over for a few moments and then agree. Why the hell not.");
	...
	Render.text(", encasing your loins in case you need a little mid-battle release, she says.");
...
public function harpyHordeClawFlurry():void {
	Render.text("The harpies lunge at you, a veritable storm of talons and claws raining down around you.  You stumble back, trying desperately to deflect some of the attacks, but there are simply too many to block them all!  Only a single harpy in the brood seems to be holding back...\n");
...
public function harpyHordeGangBangAttack():void {
	Render.text("Suddenly, a pair of harpies grabs you from behind, holding your arms to keep you from fighting back! Taking advantage of your open state, the other harpies leap at you, hammering your chest with punches and kicks - only one hangs back from the gang assault.\n\n");
...
public function harpyHordeGangBangStruggle(clearDisp:boolean = true):void {
	...
	if(rand(10) > 0 && player.str/5 + rand(20) < 23) {
		...
		damage = takeDamage(damage);
		Render.text("You struggle in the harpies' grasp, but can't quite get free.  The brood continues to hammer away at your defenseless self. (" + damage + ")");
	}
	//Success: 
	else {
		player.statusAffects.remove("HarpyBind");
		Render.text("With a mighty roar, you throw off the harpies grabbing you and return to the fight!");
	}
	combatRoundOver();
}
...
public function pcLosesToHarpyHorde():void {
	...
	if(player.HP < 1) Render.text("brutal assault");
	else Render.text("raw sexuality");
	Render.text(", you collapse, utterly at the harpies' mercy.  The group looms over you, lusty, evil grins all around, but to your surprise, one of them shouts a harsh command, making the swarm of feathery bitches back off.  A particularly slight harpy with a shock of bright-orange hair waves the brood off, astonishingly commanding for the runt of the litter.  The other harpies hiss and growl at her, but still she speaks, \"<i>Hold it!  We can't have the intruder yet.  Mother will want to talk to " + player.mf("him","her") + " first.</i>\"");
...
public function talkToKiri():void {
	clearOutput();
	Render.text("You ask Kiri if she wouldn't mind sharing a bit of information with you.");
...
public function askKirkAboutHel():void {
	clearOutput();
	Render.text("You ask the harpy girl how she knows Hel, exactly.");
...
public function askKiriAboutHarpies():void {
	clearOutput();
	Render.text("You ask Kiri about the harpies remaining in the tower and their relative strength and position - anything to give you an advantage.");
...
public function askKiriAboutSalamander():void {
	clearOutput();
	Render.text("You ask her about the salamander prisoner you're here to help free.");
...
public function askKiriAboutKiri():void {
	clearOutput();
	Render.text("You ask Kiri to tell you a little about herself.");
...
public function kiriSexIntro():void {
	clearOutput();
	Render.text("You ask Kiri if she could help you blow off some steam.");
	...
	menu();
	if (player.gender == 0) Render.text("Unfortunately, there's not much she can do for you...");
	if (player.lowerBody.cockSpot.hasCock()) addButton(0, "Anal", kiriSexAnal);
	if (player.lowerBody.vaginaSpot.hasVagina()) addButton(1, "Get Licked", kiriSexGetLicked);
...
public function kiriSexAnal():void {
	...
	let y:number = x+1;
	Render.text("You whip your [cock " + (y) + "] out of your [armor] and tell Kiri to get on all fours.  She grimaces, but does as you ask.  You hike up her shift to reveal her large, egg-laying pussy and her tight little pucker.");
	...
	if(player.cockArea(x) > 60) Render.text("as far in as you can manage");
	else Render.text("filling her completely");
	Render.text(".  Your cock squirts a thick load inside her, shooting creamy ropes of jizz deep into her bowels ");
	//[if High Cum Production: 
	if(player.cumQ() > 500) Render.text("until your cream squelches back out around your cock and onto the floor");
	Render.text(".  With a scream of delight, Kiri clamps down on your [cock " + y + "] and climaxes too, leaking a pool of fem-spunk onto the ground.  She starts to bounce on your cock, riding out her anal orgasm until she's exhausted and you're deflated inside her.");
...
public function kiriSexGetLicked():void {
	clearOutput();
	Render.text("You ask Kiri to eat you out. She grimaces but drops to her knees and undoes the bottom of your [armor], revealing your lusty [vagina]");
	if(player.lowerBody.cockSpot.hasCock()) Render.text(" and " + player.multiCockDescriptLight());
	Render.text(".  With a word of encouragement from you, she leans forward and presses her face into your groin, letting her tongue loose to explore your lower lips.");
...
public function talkToValeria():void {
	clearOutput();
	Render.text("Now that you have a few moments to catch your breath, you ask your goo-armor what she thinks about the situation.");
...
public function tortureGear():void {
	clearOutput();
	menu();
	Render.text("You walk up to the torture rack.  ");
	if (flags[FlagEnum.HEL_DUNGEON_TAKEN_WHIP] == 0 || flags[FlagEnum.HEL_DUNGEON_TAKEN_STRAPS] == 0 || flags[FlagEnum.HEL_DUNGEON_TAKEN_DAGGER] == 0) {
		Render.text("The rack contains: ");
		if (flags[FlagEnum.HEL_DUNGEON_TAKEN_WHIP] == 0) {
			Render.text("A whip");
			addButton(0, "Succ. Whip", takeWhip);
		}
		if (flags[FlagEnum.HEL_DUNGEON_TAKEN_STRAPS] == 0) {
			if(flags[FlagEnum.HEL_DUNGEON_TAKEN_WHIP] == 0) Render.text(", ");
			Render.text("some leather straps");
			addButton(1, "BondageStraps", takeStraps);
		}
		if (flags[FlagEnum.HEL_DUNGEON_TAKEN_DAGGER] == 0) {
			if(flags[FlagEnum.HEL_DUNGEON_TAKEN_STRAPS] == 0 || flags[FlagEnum.HEL_DUNGEON_TAKEN_WHIP] == 0) Render.text(", ");
			Render.text("a lust-draft coated dagger");
...
public function helDungeonPrisonerTalk():void {
	...
	if(flags[FlagEnum.HEL_PC_TALKED_WITH_HAKON] == 0) {
		Render.text("You approach the Salamander strapped to the table.  He looks at you with his one good eye, warily gauging you as you approach.");
	...
	//[Else]
		...
		Render.text("You approach Hakon the Salamander.  He strains against his bonds, yelling at you to get Hel and get out before it's too late.  You roll your eyes and carry on.");
	}
	doNext(playerMenu);
}
...
public function phoenixPlatoonRush():void {
	Render.text("You fall back under a hail of scimitar attacks.  The sheer number of phoenixes attacking is bad enough, but their attacks are perfectly coordinated, leaving virtually no room for escape or maneuver without getting hit!\n");
...
public function phoenixPlatoonMurdersPC():void {
	clearOutput();
	Render.text("You collapse, too ");
	if(player.lust > 99) Render.text("turned on");
	else Render.text("badly injured");
	Render.text(" to continue the fight.  The platoon of heavy infantry breaks their formation, circling around you with shields still raised, keeping you from making any kind of last-ditch attack.  One prods you with the flat of her blade.  \"<i>Is " + player.mf("he","she") + " down?</i>\"");
...
public function checkOutDemBirdBitches():void {
	clearOutput();
	Render.text("You loom over the defeated heavy infantry, marveling at them.  The half-breeds were probably the most organized and efficient fighting unit you've ever come across here in Mareth, and though you defeated them, you know most denizens of the region wouldn't have stood a chance.");
...
public function phoenixMissionary():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text(" and the tight ring of her pucker");
	Render.text(".  You grasp her wide flanks and, lining your [cock " + y + "]");
	if(player.lowerBody.cockSpot.count() > 1) Render.text(" and extra boner up with her holes");
	else Render.text(" up with her hole");
	Render.text(", push in, penetrating her cunt");
	if(player.lowerBody.cockSpot.count() > 1) Render.text(" and ass");
	Render.text(" and sliding into her warm, wet channel");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text("."); 
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s drive");
	else Render.text(" drives");
	Render.text(" into her; you roll your hips back and forth for short but powerful strokes into her blazing hot innards.  As the pace picks up, you pull the phoenix-girl into a long, tender kiss.  The kiss soon turns into her moaning into your mouth as you fuck her cunt ");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("and ass ");
	Render.text("hard, slamming your hips into hers.");

	Render.text("\n\nShe cums first");
	if(player.lowerBody.cockSpot.count() > 1) Render.text(", your double penetration too much for her to handle");
	Render.text(".  The phoenix grips your shoulders tight enough for her claws to cut into you as her tight pussy ");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("and tighter sphincter spasm");
	else Render.text("spasms");
	Render.text(" around your dick");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(".  With her squirming in your embrace and squeezing down so hard, you can't help but blow your load.  You press your lips hard into hers and cum, pumping thick ropes of steaming jizz into her wet box");
	if(player.lowerBody.cockSpot.count() > 1) Render.text(" and hot asshole");
	Render.text(".  As you cum into her, you feel her reptile prick shoot off, squirting a long white rope onto her chest and yours until her tits are soaked with her spunk.");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" out of her ravaged hole");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" and gather your gear.");
...
public function phoenixAginal():void {
	...
	if(player.perks.has("BroodMother")) Render.text("You assure her she will be soon");
	else Render.text("You grin at her");
	Render.text(" and strip out of your [armor]. The phoenix, a bit more dominant than you might have liked, roughly grabs your [chest], pinching your nipples as she takes over wringing her cock from you. Oh well. You decide to roll with it and slide a hand down to your [vagina], stroking your pussy as your lover warms up.");
...
public function eldritchRopes():void {
	Render.text("The Harpy Queen flicks her left wrist at you. Before you can blink, ropes of white-hot magic hurtle toward you. You manage to duck and dodge a few of them, but a pair still grab your wrists, pulling painfully at your arms.");
...
public function ropeStruggles(wait:boolean = false):void {
	...
	if(rand(10) > 0 && player.str/5 + rand(20) < 23 || wait) {
		Render.text("You give a mighty try, but cannot pull free of the magic ropes!  The Harpy Queen laughs uproariously, pulling at your arms harder.");
		let damage: number = 25 + rand(10);
		damage = takeDamage(damage);
		Render.text(" (" + damage + ")");
	}
	else {
		Render.text("With supreme effort, you pull free of the magic ropes, causing the queen to tumble to her hands and knees.");
...
public function harpyQueenBeatsUpPCBadEnd(clearS:boolean = false):void {
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("sperm bank");
	else Render.text("incubator");
	Render.text(" in my harem.  Yes, you'll do nicely...</i>\"");
	Render.text("\n\n<b>SIX MONTHS LATER...</b>\n");
	Render.text("You groan, your wrists chafing in the manacles hanging overhead.  The harpy slut riding your huge, engorged dick crosses her eyes, screams, and cums.  Another wave of seed lurches out of your cripplingly-large balls, so massive that they drag on the floor between your [legs].  You shudder slightly, but the act of ejaculation has lost all meaning");
	if(!player.lowerBody.cockSpot.hasCock()) Render.text(", even if the sudden new sensations nearly broke your mind when the harpies used their magics to grow these huge male implements on your body");
	Render.text(".  Indeed, as soon as that slut's gotten her fill, one of her sisters pushes her roughly off your shaft and mounts you, bending over and backing into your massive schlong.  A little shudder of pleasure courses up your body as the twenty-ninth slut you've serviced today starts to milk you for your magically-enhanced seed.  The tubes pumping enchanted drugs directly into the flesh of your testes goes into overtime, flooding your system with lust and the strange concoction that creates the Phoenixes.");
...
public function harpyQueenDefeatedByPC():void {
	clearOutput();
	Render.text("With a final, loud gasp, the Harpy Queen collapses into her throne, unable to oppose you further.  Seeing their broodmother defeated, the other harpies that had been swarming around the room suddenly break off their attack and retreat, edging back to the fringes of the throne room.  Behind you, Hel stumbles out of the melee, covered in little cuts and bruises, but seeming otherwise unhurt.");
...
public function HeliaThroneRoom():void {
	clearOutput();
	Render.text("You turn your attentions to the salamander, who is currently pinning down the harpy bitch to prevent her from flying off or doing something drastic.");
...
public function heliaHakonTalk():void {
	clearOutput();
	Render.text("You decide to tell Hel who, exactly, is chained up downstairs.  When you do, she simply stares at you, wide-eyed.");
	...
	if(flags[FlagEnum.HARPY_QUEEN_EXECUTED] == 0) Render.text("  \"<i>You're going to pay for what you did to my father, you bitch.  I promise you that.</i>\"");
...
public function heliaKiriTalk():void {
	clearOutput();
	Render.text("You ask Hel about the half-breed girl that helped the two of you find this place, and offered you information below.");
...
public function heliaQueenTalk():void {
	clearOutput();
	Render.text("You nod your head toward the great big-booty broodmother.  \"<i>Ah, the queen cunt herself,</i>\" Hel says ruefully, cracking her knuckles.  \"<i>We're gonna have some fun with you, little birdie...  Aren't we, [name]?</i>\"");
	...
	if(flags[FlagEnum.HEL_KNOWS_ABOUT_HAKON] == 1) Render.text("  \"<i>Maybe snap her neck afterwards.</i>\"");
	doNext(playerMenu);
}
...
public function harpyQueenAdvantage():void {
	clearOutput();
	Render.text("You loom over the defeated Harpy Queen, who squirms underneath Hel's foot on her chest.");
...
public function letHarpyQueenGo():void {
	clearOutput();
	Render.text("You tell Hel to let up. You're letting the bitch go.");
...
public function harpyQueenLetHerGoForgive():void {
	clearOutput();
	Render.text("You and Hel stare each other down, the dead harpy's body the only thing separating you.  Two dozen enraged harpies screech and caw around you, demanding justice for their fallen queen.");
	...
	if(flags[FlagEnum.HEL_KNOWS_ABOUT_HAKON] == 1) Render.text("Dad ");
	else Render.text("that poor salamander ");
	Render.text("out of the dungeon.  I'll make sure the phoenixes and harpies don't give you two trouble on the way out.</i>\"");
...
public function killHarpyQueen():void {
	clearOutput();
	Render.text("You look from Hel to the Harpy Queen.  This bitch could have bred an entire army - and might try it again. You can't allow that.");
...
public function takeQueensStaff():void {
	clearOutput();
	Render.text("You pick up the Harpy Queen's staff.  It is a tall whitewood staff, nearly six feet in length, and covered in glowing eldritch runes, with a singular shimmering sphere of crystal at its head, which seems to have a swirling mist within.");
...
public function fuckHarpyQueen():void {
	clearOutput();
	Render.text("You decide that the queen bitch deserves to be taught a lesson and you'll use her to get you off in the process.  You whisper this to Hel, who seems quite amused by the idea.");
...
public function harpyQueenSexAnal():void {
	...
	if(player.cockArea(0) < 100) Render.text("until your hips sink into her pillowy ass");
	else Render.text("until your massive " + player.cockDescript(0) + " can go no further inside her");
	Render.text(".  Sunk in as far as you'll go, you leave your cock where it is and instead give the broodmother a hard slap.  She screeches and clamps down, wringing your " + player.cockDescript(0) + " hard.  But it's more pleasurable this time, now that she's nice and stretched by your anal intruder.");
...
public function vaginalHarpyQueenSex():void {
	clearOutput();
	Render.text("You roughly toss the harpy queen to the floor and, grinning, tell her that since you're wrecking her plans to breed phoenixes, you'll just give her a brood of champion-spawn instead.  She gapes at you, wide-eyed in confusion and fear. Hel keeps her pinned down for you as you toss your [armor] aside, revealing your " + player.cockDescript(0) + ".");
	...
	if(player.cockArea(0) < 48) Render.text("  Not to be outdone, you slam your hips forward, ramming your " + player.cockDescript(0) + " into the queen's depths until you catch up with Hel, pounding the entrance to her womb.");
	...
	if(player.cumQ() < 300) {
		Render.text("you slam your [hips] into the harpy's groin and ejaculate, launching thick, sperm-filled globs right into her waiting womb.\n\nYou cum and cum, filling the queen with all your seed until your ");
		if(player.lowerBody.balls > 0) Render.text("[balls] feel");
		else Render.text("crotch feels");
		Render.text(" hollow and empty.  Shuddering, you and Hel both withdraw, your cock and her tail a spunk and juice-covered mess.");
	}
	else {
		Render.text("you slam your [hips] into the harpy's groin and ejaculate, releasing a massive torrent of spunk deep inside the queen's womb, causing the harpy to shudder at the sheer amount of sperm you let out.  You continue to coat the harpy's walls for a minute, until your ");
		if(player.lowerBody.balls > 0) Render.text("[balls] feel");
		else Render.text("crotch feels");
		Render.text(" hollow and empty.  You and Hel slowly withdraw, causing some of your semen to leak out of the harpy's massive canal, leaving your cock and Hel's tail a spunk-and-juice-covered mess.");
	}
...
public function harpyQueenInterrogate():void {
	clearOutput();
	Render.text("Leaning over the defeated Harpy Queen, you decide to get some answers.  First, you ask her why, exactly, she kidnapped a salamander in the first place.  That's what brought you here, after all.");
