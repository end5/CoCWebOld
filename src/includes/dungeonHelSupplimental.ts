...
public function heliaDiscovery():void {
	...
	if(flags[FlagEnum.HEL_ISABELLA_THREESOME_ENABLED] == 0) {
		...
		//[(pussy)
		if(player.stats.cor < 50) outputText("quickly rush over and stop Hel before she loses a leg to one of your traps");
		//(dick)
...
public function morningAfterHeliaDungeonAgreements():void {
	...
	//[If PC has >C Cups, \"<i>
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 3) outputText("her head nestled between your soft tits and ");
...
public function gooArmorBeatsUpPC():void {
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1 && player.lactationQ() > 0) outputText("and milk ");
...
public function harpyHordeLustAttack():void {
	...
	if(player.perks.has("LuststickAdapted")) outputText("doing relatively little thanks to your adaptation");
	else {
...
public function kiriSexGetLicked():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(" and " + player.multiCockDescriptLight());
...
public function tortureGear():void {
	...
	if (flags[FlagEnum.HEL_DUNGEON_TAKEN_WHIP] == 0 || flags[FlagEnum.HEL_DUNGEON_TAKEN_STRAPS] == 0 || flags[FlagEnum.HEL_DUNGEON_TAKEN_DAGGER] == 0) {
		...
		if (flags[FlagEnum.HEL_DUNGEON_TAKEN_STRAPS] == 0) {
			if(flags[FlagEnum.HEL_DUNGEON_TAKEN_WHIP] == 0) outputText(", ");
			outputText("some leather straps");
			addButton(1, "BondageStraps", takeStraps);
		}
		if (flags[FlagEnum.HEL_DUNGEON_TAKEN_DAGGER] == 0) {
			if(flags[FlagEnum.HEL_DUNGEON_TAKEN_STRAPS] == 0 || flags[FlagEnum.HEL_DUNGEON_TAKEN_WHIP] == 0) outputText(", ");
...
public function phoenixMissionary():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText(" and the tight ring of her pucker");
	outputText(".  You grasp her wide flanks and, lining your [cock " + y + "]");
	if(player.lowerBody.cockSpot.count() > 1) outputText(" and extra boner up with her holes");
	else outputText(" up with her hole");
	outputText(", push in, penetrating her cunt");
	if(player.lowerBody.cockSpot.count() > 1) outputText(" and ass");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("s drive");
	...
	outputText("\n\nShe cums first");
	if(player.lowerBody.cockSpot.count() > 1) outputText(", your double penetration too much for her to handle");
	outputText(".  The phoenix grips your shoulders tight enough for her claws to cut into you as her tight pussy ");
	if(player.lowerBody.cockSpot.count() > 1) outputText("and tighter sphincter spasm");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText(" and hot asshole");
...
public function phoenixAginal():void {
	...
	if(player.perks.has("BroodMother")) outputText("You assure her she will be soon");
...
public function harpyQueenBeatsUpPCBadEnd(clearS:boolean = false):void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("sperm bank");
	...
	if(!player.lowerBody.cockSpot.hasCock()) outputText(", even if the sudden new sensations nearly broke your mind when the harpies used their magics to grow these huge male implements on your body");
...
public function heliaHakonTalk():void {
	...
	if(flags[FlagEnum.HARPY_QUEEN_EXECUTED] == 0) outputText("  \"<i>You're going to pay for what you did to my father, you bitch.  I promise you that.</i>\"");
...
public function heliaQueenTalk():void {
	...
	if(flags[FlagEnum.HEL_KNOWS_ABOUT_HAKON] == 1) outputText("  \"<i>Maybe snap her neck afterwards.</i>\"");
	doNext(playerMenu);
}
...
public function harpyQueenLetHerGoForgive():void {
	...
	if(flags[FlagEnum.HEL_KNOWS_ABOUT_HAKON] == 1) outputText("Dad ");
...
public function vaginalHarpyQueenSex():void {
	...
	if(player.cumQ() < 300) {
		...
		if(player.lowerBody.balls > 0) outputText("[balls] feel");
