...
public function doDungeon(eventNo:number):void {
	...
	if(eventNo == 11018) {
		...
		if(player.lowerBody.cockSpot.count() == 1) {
			...
			//Cock – single big
			if(player.cockArea(0) >= 30 && player.cockArea(0) < 100) outputText("Nearly blacking out, you struggle to stay awake as your body shifts your blood to your disproportionate " + Appearance.cockNoun(CockType.HUMAN) + ".  ", false);
			//Cock -megahuge
			if(player.cockArea(0) >= 100) outputText("As you struggle not to lose consciousness, you realize your over-aroused body had pumped most of your blood to your over-sized " + Appearance.cockNoun(CockType.HUMAN) + ", which now droops to the floor, pulsing hotly.  ", false);
		}
		//DO MULTIZ
		if(player.lowerBody.cockSpot.count() > 1) outputText("The feeling of light-headedness nearly robs you of consciousness as your " + player.multiCockDescript() + " fill with blood, pulsating with arousal as they reach full size.  ", false);
		//Vagooooz
		if(player.lowerBody.vaginaSpot.count() > 0) {
			//'uge clit
			if(player.lowerBody.vaginaSpot.list[0].clitLength >= 4.5) outputText("Popping from between your thighs, your " + clitDescript() + " responds to the sheer hotness of the situation by making itself known.   You squeeze your legs tightly together, hungry for additional sensation.  ", false);
			//big clit
			if(player.lowerBody.vaginaSpot.list[0].clitLength > 1 && player.lowerBody.vaginaSpot.list[0].clitLength < 4.5) outputText("A wave of pleasure erupts from between your legs as your " + clitDescript() + " pops free.    You squeeze your legs tightly together, hungry for the additional sensations.  ", false);
			//slick
			if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLICK) outputText("Squishing wetly, your bottoms become soggy with the flood of fluids leaking from your " + vaginaDescript(0) + ".   Your legs spread apart on their own, begging for any kind of intrusion.  ", false);
			//normal
		...
		if(player.upperBody.chest.count() == 1) outputText("pair of", false);
		else outputText("number of", false);
		outputText(" hoses to your breasts, ", false);
		if(player.lowerBody.cockSpot.count() == 0) {
			if(player.lowerBody.vaginaSpot.count() > 0) outputText("and a vacuum pump to your clit.  ", false);
			else {
			...
			if(player.lowerBody.vaginaSpot.count() > 0) outputText("a vacuum pump to your clit, and a pump many times bigger than your " + cockDescript(0) + " to it.  ", false);
		...
		if(player.lowerBody.vaginaSpot.count() == 0) outputText(".", false);
	...
	if(eventNo == 11019) {
		...
		//Puss Orgasmz
		if(player.lowerBody.vaginaSpot.count() > 0) outputText("Clenching tightly, your " + vaginaDescript(0) + " squeezes tightly on its intruder as its repeatedly violated by the machines.  ", false);
		//End
		...
		if(player.statusAffects.get("$1").value3 == 1 || player.statusAffects.has("CampMarble")) outputText("  With Marble here too, you'll be around for a long time.", false);
		gameOver();
		return;
	}
	...
	if(eventNo == 11040) {
		...
		//Second/third times...
			...
			if(player.statusAffects.get("TensionReleased").value1 == 0) {
				...
				if(player.lowerBody.cockSpot.count() > 0) outputText("and " + player.multiCockDescript(), false);
				...
				//HERM
				if(player.lowerBody.vaginaSpot.count() > 0 && player.lowerBody.cockSpot.count() > 0) outputText("You feel your " + player.multiCockDescript() + " getting milked by many wet holes, though you are too busy sucking cocks and moaning in ecstasy to notice who they belong to.  ", false);
			...
			//Third time
				...
				if(player.lowerBody.cockSpot.count() > 0) {
					...
					outputText(" as ", false);
					if(player.lowerBody.cockSpot.count() > 1) outputText("they", false);
				...
				if(player.lowerBody.vaginaSpot.count() > 0) outputText("Your pussy is instantaneously soaked, filling the air with the scent of sex.  ", false);
	...
	if(eventNo == 11049) {
		...
		if(player.lowerBody.cockSpot.count() == 1) {
			if(player.demonCocks() < 1) {
				...
				if(player.stats.cor < 80) outputText("You watch in horror as the skin of your " + cockDescript(0) + " turns shiny and purplish-black.  ", false);
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			...
			//Not yet full of demoncocks...
				...
				if(player.stats.cor < 80) outputText(" in horror", false);
	...
	if(eventNo == 11053) {
		spriteSelect(16);
		outputText("", true);
		if(player.lowerBody == LOWER_BODY.HUMAN) outputText("You feel as if you should slap yourself for stupidy.  Your legs are already normal!  You flush hotly as the corrupt magics wash over you, changing nothing.", false);
