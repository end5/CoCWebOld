...
public dontExploreGatsChristmasTale():void {
	clearOutput();
	//i. No
	Render.text("In a place like the high mountains, there's no warning to be found if you happen to be confronted by danger.  Deciding not to take your chances, you continue to walk down the path you came, leaving the peaks to head straight back to camp.");
...
public encounterQuoteUnquoteAngel():void {
	...
	if(!isHolidays()) {
		...
		if(silly()) Render.text(" (you didn't.)");
		else Render.text(".");
		Render.text("  She beams a toothless grin, popping one into her mouth and blowing out a stream of candy-scented smoke.  Suddenly, you're glad you didn't take any.");
	...
	//ii. Yes
		...
		Render.text("You haven't found anything as of yet, so you figure you might as well keep going.  Following the quiet cries from above, you trace the spiralling side of the snow-capped behemoth, eventually reaching a small cliff hanging beneath the mountain's peak.  The sight you're met with manages to take your breath away - and considering the inhabitants of the land around you, that in itself is quite a difficult feat.");
		...
		if(silly()) Render.text(", without a penis");
		Render.text(" - a loving aura surrounds the feminine creature and you can't help but feel as if she's here without ill intent.  Stating your name, you question her purpose at the high mountains.");
...
public findTheWindUpKey():void {
	...
	//[if (tallness > 60) 
	if(player.tallness > 60) Render.text("  You can't even stand straight up below the low-hanging ceiling, forced to carefully tread along on your knees over the slick, hard rock.");
	Render.text("  Squinting around the small, deep green grotto, a pedestal in the center calls out to you.  As you approach it, it becomes evident to you why things are so chilly in here; the outcrop is made up of clear ice - and within, the bodies of the many who had found this place before you.  At least that mystery is cleared up.  Atop the frozen prop lies a small, crystal... crank? Upon closer inspection, it actually resembles a - kitty titties, it's a windup key!  Despite the fact that you should've seen this one coming, you surmise that this is the item you've come searching for.");
...
public giveThatBitchAKeyYo():void {
	...
	if(flags[FlagEnum.GATS_ANGEL_TIME_TO_FIND_KEY] < 150) {
		Render.text("All throughout your perilous climb, you cannot help but ogle at the brilliant little crystal key.  You have a suspicion as to how this innocent little key will help the crying colossus on the peak, but you are not completely sure this is the artifact she required.  But you have a hunch that it is.");
		...
		Render.text("All throughout your perilous climb, you cannot help but ogle at the brilliant little crystal key.  You have a suspicion as to how this innocent little key will help the crying colossus on the peak, though you aren't entirely sure this is the artifact she required.  But you have a hunch that it is.");
...
public theWorstEndingEverDotJPG():void {
	...
	//[if (corruption > 49)
	if(player.stats.cor > 49) Render.text("  It seems like the best response to you for now.  Based on the giantess's sobbing soliloquy, you figure that standing in for the love of the soldier's life will get you straight to the action.");
	else Render.text("  Wait, what?  It appears that in your desperation to contain the situation, your lingering doubt was enough to get you blurt out a bold faced lie.  You figure that you may as well roll with it for now.");
	...
	Render.text(" to try and convince the confused soldier.  You look North over, figuring the best way to work him under your spell.  It isn't a long search, as you decide that his ");
	//[if (hasCock = true) 
	if(player.lowerBody.cockSpot.hasCock()) Render.text("keyhole");
	//[if (hasCock = false) 
	else Render.text("windup key");
	Render.text(" would make a worthy candidate for your first alluring act of debauchery.");
	...
	//[if (hasCock = true) 
	if(player.lowerBody.cockSpot.hasCock()) Render.text("You slide the key playfully down your wrist, rubbing your [cock] with your free hand.  You aren't entirely sure if he'll even feel a sensation from your penetration, but perhaps it'll at least get him in the mood.  With your manhood at full mast, you gently skirt along the edges of the keyhole.");
	else Render.text("A mischievous tongue traces along the smooth, perfect surface of the windup key.  You promptly lower the makeshift toy precariously in front of your [cunt]; if there was ever a time to make a wind up pun or remark as some sort of goofy foreplay, now would be the time.");
	...
	//[if (corruption > 49) 
	if(player.stats.cor > 49) Render.text("  ...Well, that was a waste of time.");
	...
	//[if (corruption > 49) 
	if(player.stats.cor > 49) Render.text("  More importantly, you have some unsatisfied, pent up lust that you'd like to expend.");
...
public fuckTheAngelBadEnd():void {
	...
	Render.text(", you wander your way towards the giant's mouth-muscle, climbing on top of her warm, pink bed. There's gotta be some kinda magic involved - her mint saliva sends tingles through your body, while dragging your ");
	if(player.lowerBody.cockSpot.hasCock()) Render.text(player.multiCockDescriptLight());
	else Render.text(vaginaDescript());
	Render.text(" against each bump and curve is beginning to get you hard.  She winces at the taste of your crotch against her tongue, though she's obviously never given oral to something with a heartbeat before.  Well, there's a first time for everything.");
	...
	Render.text(" along the softness of her maw causing her to twitch and drool uncomfortably.  You're beginning to fuck along her slowly, chuckling at every wince and moan she makes from the taste of your genitals.");
	...
	Render.text(" on a person's tongue to fuck it thoroughly.  You wonder if she's already long forgotten the pile of bolts she was originally trying to save.  A man frozen in time, never to wake up because - whoops!  You're taking care of her needs now.  Maybe you'll invest in a giant goblin-built sex machine, who knows?  Judging by the bitch's aroused whines, she was probably waiting for the chance to strip for the next person to find her up the mountain.  It's about time she let herself go, after all.");
...
public goodEndLeaveBeforeSexings():void {
	clearOutput();
	Render.text("You get the message, feeling that letting them do their own thing might be for the best.  With a hasty pardon, you promptly turn around and walk a good twenty yards down the mountainside, idly playing in the snow while the two reunited lovebirds do their thing.");
...
public stayForPretendNTRGoodWinjobSex():void {
	clearOutput();
	Render.text("You prefer to stay here, but the frosty giant has already gone back to her mechanical man, so you hold your explanation and continue to observe.");
	...
	if(player.lust >= 100) {
		...
		if(player.wetness() >= 4 || player.cumQ() >= 1000) Render.text(" - figuratively, and literally, as well, as your fluids drip off the edge to add to the rain of sexual fluids the giantess is laying in the midst of");
		Render.text(".");
	}
...
public conclusionOfGiantessGoodEnd():void {
	clearOutput();
	Render.text("The air is crisp, and the light powdering of snow floats through the air as if playing an invisible melody.  The warm couple are together.  Lifting her head, the giantess beams at you with a refreshing sense of spirit.");
	...
	if(flags[FlagEnum.PC_ENCOUNTERED_CHRISTMAS_ELF_BEFORE] == 0) Render.text("\n\nYou have no idea what that is.");
