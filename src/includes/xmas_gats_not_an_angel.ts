...
public function theWorstEndingEverDotJPG():void {
	...
	//[if (corruption > 49)
	if(player.stats.cor > 49) outputText("  It seems like the best response to you for now.  Based on the giantess's sobbing soliloquy, you figure that standing in for the love of the soldier's life will get you straight to the action.");
	...
	//[if (hasCock = true) 
	if(player.lowerBody.cockSpot.hasCock()) outputText("keyhole");
	//[if (hasCock = false) 
	...
	//[if (corruption > 49) 
	if(player.stats.cor > 49) outputText("  ...Well, that was a waste of time.");
	...
	//[if (corruption > 49) 
	if(player.stats.cor > 49) outputText("  More importantly, you have some unsatisfied, pent up lust that you'd like to expend.");
...
public function fuckTheAngelBadEnd():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("hot, white seed");
...
public function conclusionOfGiantessGoodEnd():void {
	...
	if(flags[FlagEnum.PC_ENCOUNTERED_CHRISTMAS_ELF_BEFORE] == 0) outputText("\n\nYou have no idea what that is.");
