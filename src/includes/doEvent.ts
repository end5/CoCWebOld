	...
	else if (eventNo == 2011)
	{
		...
		if (player.lowerBody.vaginaSpot.count() == 1)
		{
			...
			if ((player.perks.has("BigClit") && player.lowerBody.vaginaSpot.list[0].clitLength > 2) || player.lowerBody.vaginaSpot.list[0].clitLength > 3)
				outputText("almost " + num2Text(Math.floor(player.lowerBody.vaginaSpot.list[0].clitLength * 1.75)) + " inches in length. ", false);
			...
			outputText("Satisfied, the creature drops you smartly and withdraws its limbs from you and lumbers away. Covered completely in cum, you see that your " + player.lowerBody.vaginaSpot.count() + " clits have swollen up to almost four inches in length. Bruised and sore, you pass into unconsciousness, ", true);
			spriteSelect(100);
		}
		//Not too corrupt
		if (player.stats.cor < 75)
		...
		if (player.lowerBody.vaginaSpot.list[0].clitLength < 7 && player.lowerBody.vaginaSpot.list[0].clitLength >= 3.5 && player.perks.has("BigClit"))
		{
			...
			outputText("Your massive clitty eventually diminishes, retaining a fair portion of its former glory.  It is now " + int(player.lowerBody.vaginaSpot.list[0].clitLength * 10) / 10 + " inches long when aroused, ", false);
...
/*
	...
	else if (eventNo == 2150)
	{
		...
		if (player.statusAffects.has("Infested"))
		{
			...
			if (player.statusAffects.has("JojoNightWatch")) outputText("(Jojo is currently watching for enemies at night.)\n\n", false);
