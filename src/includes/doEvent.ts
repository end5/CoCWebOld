...
public doEvent(eventNo:number):void
{
	...
	else if (eventNo == 2003)
	{
		...
		if (player.findStatusAffect(StatusAffects.JojoMeditationCount) < 0)
			...
			player.statusAffects.get("JojoMeditationCount").value1 = 1;
	...
	{
		spriteSelect(50);
		Render.text("A strange woman seems to appear from the dunes themselves.  She identifies herself as a sand witch, and politely asks if she can cast a spell on you.", true);
		if (player.statusAffects.get("Exgartuan").value1 == 1 && player.cockArea(0) > 100 && player.statusAffects.get("Exgartuan").value2 == 0)
		{
			...
			Render.text("The sandwitch ", false);
			if (player.stats.cor < 50)
				Render.text("and you both turn crimson", false);
			else
				Render.text("turns crimson", false);
			Render.text(" as you yank your " + player.armorName + " back into place.  You're in charge here, not some possessed appendage!   Exgartuan yells something snide, but it's muffled too much to understand.  You look up in time to sidestep an attack from the Sand Witch.  It looks like you'll have to fight her!", false);
		...
		if (player.hairColor == "sandy blonde")
		{
			...
			if (sand == 1)
			{
				...
				if (player.stats.cor > 66)
					Render.text("You wish you had said no, so you could fuck with her and her magnificent quartet of breasts some more.", false);
			}
			doNext(13);
		}
		else
	...
	{
		spriteSelect(50);
		Render.text("With an inarticulate scream of rage, the Sand Witch attacks!", true);
		startCombat(new SandWitch());
	}
...
/*
		...
		if (player.gender == 0 && rand(3) == 0 && !player.isNaga() && !player.isTaur() && !player.isGoo())
		{
			...
			Render.text("You see a massive, shambling form emerge from the underbrush. While first appearing to be a large shrub, it shifts its bulbous mass and reveals a collection of thorny tendrils and cephalopodic limbs. Sensing your presence, it lumbers at you, full speed, tentacles outstretched.\n\n", false);
			
			if (player.stats.cor > 50 && player.stats.cor <= 75)
				Render.text("You debate the merits of running from such a creature, and realize it's now too late to escape.  ", false);
			if (player.stats.cor > 75)
				Render.text("You smile and stride forward, welcoming the pleasure you expect from such a monster.  ", false);
			//HILARIOUS NEUTER EVENT HERE
			if (player.stats.cor < 75)
				Render.text("While you attempt to resist the abomination, its raw muscle mass is too much. ", false);
			Render.text("It pins you to the ground easily. You feel slimy tentacles run up and down your groin as the creature searches for whatever gonads it expected you to have. When it realizes that you have neither penis nor vagina, it smartly casts you to the ground in apparent disgust.\n\n\"<i>WHAT THE FUCK IS THIS SHIT?!!</i>\" The creature speaks in an unnervingly human voice.\n\n", false);
			Render.text("Completely confused, all you can do is sit there in shock.\n\n\"<i>Where are your naughty bits, goddammit!</i>\" the creature bellows. \"<i>Us tentacle creatures need to FEED!</i>\"\n\n", false);
			Render.text("You sheepishly state that you are gender-neutral and have no genitalia.\n\n\"<i>You gotta be shitting me!!</i>\" the monster bellows in contempt. \"<i>Of all the motherfuckers I ambush, it has to be the ONE bastard I can't feed from! What am I supposed to do now, asshole?! I gotta eat!</i>\"", false);
			Render.text("At a loss for words, you meekly offer the creature some of your food you have packed for your journey. The creature slaps it out of your hand, almost breaking your wrist.\n\n\"<i>I can't eat that shit!</i>\" roars the abomination. \"<i>Do I look like I have a fucking mouth to chew that with?! NOOOOOO! I feed off dicks and wayward women! Cum and tit milk! YOU have NEITHER!!!</i>\"  ", false);
			Render.text("The beast slaps you squarely on the ass as if to push you along. \"<i>Get the fuck out of here!</i>\" it screams. \"<i>Get lost so I can hunt me a REAL meal!!!</i>\"", false);
			Render.text("You walk away from the creature, which hides back in the brush. After you trek a bit, you wonder if what happened really DID happen...", false);
			dynStats("lus", -5);
			doNext(13);
			return;
		}
		//Combat starter
		Render.text("You see a massive, shambling form emerge from the underbrush.  While it resembles a large shrub, a collection of thorny tendrils and cephalopodic limbs sprout from its bulbous mass.  Sensing your presence, it lumbers at you, full speed, tentacles outstretched.\n\n", false);
		if (player.stats.cor > 50 && player.stats.cor <= 75)
			Render.text("You debate the merits of running from such a creature.\n\n", false);
		if (player.stats.cor > 75)
			Render.text("You smile and stride forward, welcoming the pleasure you expect from such a monster.\n\n", false);
		//Worms get nothing!
		if (player.statusAffects.has("Infested"))
		{
			Render.text("It stops itself completely in a moment and twitches, as if sniffing the air, before turning around and disappearing into the underbrush.", false);
			doNext(13);
			return;
		}
		if (player.stats.cor > 50)
		{
			Render.text("Do you joyfully submit or fight back?\n\n", false);
	...
	else if (eventNo == 2010)
	{
		...
		if (player.gender == 1)
		{
			Render.text("You next feel the wretched sensation of another tentacle pushing its way past your anus and into your rectum. You cry more out of frustration and anger than pain as the foreign body settles a few inches inside your body. With a furious, coordinated rhythm, the monstrosity begins swelling the tentacle in your ass and ", true);
			spriteSelect(100);
			if (player.lowerBody.cockSpot.count() == 1)
				Render.text("using a sucking-stroking motion on your helpless " + multiCockDescriptLight() + ". The swelling of the ass tentacle pressures your prostate in a paradoxically pleasurable and painful manner. You realize, much to your terror, that this beast is MILKING you of your semen!", false);
			else
				Render.text("using a sucking-stroking motion on your " + multiCockDescriptLight() + ".  The swelling of the ass tentacle pressures your prostate in a paradoxical pleasurable and painful manner.  You realize, much to your terror, that this beast is MILKING you of your semen!", false);
	...
	else if (eventNo == 2011)
	{
		...
		if (player.lowerBody.vaginaSpot.count() == 1)
		{
			Render.text("Satisfied, the creature drops you smartly, withdraws its limbs from you, and lumbers away. Covered completely in cum, you see that your clitoris has swollen up to ", true);
			spriteSelect(100);
			...
			if ((player.perks.has("BigClit") && player.lowerBody.vaginaSpot.list[0].clitLength > 2) || player.lowerBody.vaginaSpot.list[0].clitLength > 3)
				Render.text("almost " + num2Text(Math.floor(player.lowerBody.vaginaSpot.list[0].clitLength * 1.75)) + " inches in length. ", false);
			//normal girls get big clits
			else
				Render.text("almost four inches in length. ", false);
			Render.text("Bruised and sore, you pass into unconsciousness ", false);
		}
		else {
			Render.text("Satisfied, the creature drops you smartly and withdraws its limbs from you and lumbers away. Covered completely in cum, you see that your " + player.lowerBody.vaginaSpot.count() + " clits have swollen up to almost four inches in length. Bruised and sore, you pass into unconsciousness, ", true);
			spriteSelect(100);
		}
		//Not too corrupt
		if (player.stats.cor < 75)
			Render.text("too intoxicated with lust to fume over your violation. ", false);
		//Very corrupt
		else
			Render.text("too intoxicated with lust to continue the pleasure. ", false);
		...
		if (player.lowerBody.vaginaSpot.list[0].clitLength < 7 && player.lowerBody.vaginaSpot.list[0].clitLength >= 3.5 && player.perks.has("BigClit"))
		{
			...
			Render.text("Your massive clitty eventually diminishes, retaining a fair portion of its former glory.  It is now " + int(player.lowerBody.vaginaSpot.list[0].clitLength * 10) / 10 + " inches long when aroused, ", false);
			if (player.lowerBody.vaginaSpot.list[0].clitLength < 5)
				Render.text("like a tiny cock.", false);
			if (player.lowerBody.vaginaSpot.list[0].clitLength >= 5 && player.lowerBody.vaginaSpot.list[0].clitLength < 7)
				Render.text("like a slick throbbing cock.", false);
			if (player.lowerBody.vaginaSpot.list[0].clitLength >= 7)
				Render.text("like a big thick cock.", false);
		}
		//Grow clit if smaller than 3.5"
		else if (player.lowerBody.vaginaSpot.list[0].clitLength < 3.5)
		{
			Render.text("In time your clit returns to a more normal size, but retains a bit of extra volume.", false);
			player.lowerBody.vaginaSpot.list[0].clitLength += .2;
		}
		//Mention that clit doesn't grow if your big enough.
		else
			Render.text("In time it returns to its normal size, losing all the extra volume.", false);
	...
	else if (eventNo == 2012)
	{
		...
		if (player.gender == 1 || player.gender == 3)
		{
			...
			else
			{
				...
				Render.text("feeling sore from the exertion. At least you hope it's just from the exertion and not from whatever the plant did.\n\n", false);
				if (player.cumQ() < 25)
					Render.text("As you depart, you notice the plants looking remarkably colorful and healthy...", false);
				if (player.cumQ() >= 25 && player.cumQ() < 250)
					Render.text("As you depart, you notice the plants' stalks bulging slightly from your deposit, their flowers wet with moisture and bright red.", false);
				if (player.cumQ() >= 250 && player.cumQ() < 500)
					Render.text("As you depart, you note the plants' stalks bulging obscenely, bits of your seed dripping from the flowers' opening.", false);
				if (player.cumQ() >= 500)
					Render.text("As you depart, you note the plants' stalks bulging out obscenely, looking like overfull balloons.  They're stretched so thin as to be transparent, your cum sloshing about inside them as they attempt to digest their meals. Steady streams of your jism leak from the flowers' lips, unable to keep it all inside.", false);
			}
			//Stat changes!
			player.orgasm();
			dynStats("sen", 2);
			...
			if (player.lowerBody.balls == 0)
				booster += 3;
			else if (player.lowerBody.ballSize < 6)
				booster += 2;
			if (player.perks.has("MessyOrgasms") && player.cumMultiplier < 3)
				booster += 1;
	...
	else if (eventNo == 2013)
	{
		...
		if (player.lowerBody.vaginaSpot.count() > 0 && rand(2) == 0)
		{
			Render.text("You saunter over to a dangling group of perverted looking vines, discarding your " + player.armorName + " along the way. Running your fingertips along the bulbous-tipped tentacle-like vines, you find one that looks ", true);
			spriteSelect(92);
			//Big medium or small
			temp = rand(3);
			//Normal
			if (temp == 0)
			{
				Render.text("well suited to your ", false);
				//determine size
				if (player.vaginas[0].vaginalLooseness == VAGINA_LOOSENESS.TIGHT)
					temp2 = 3;
				if (player.vaginas[0].vaginalLooseness == VAGINA_LOOSENESS.NORMAL)
					temp2 = 6.5;
				if (player.vaginas[0].vaginalLooseness == VAGINA_LOOSENESS.LOOSE)
					temp2 = 26;
				if (player.vaginas[0].vaginalLooseness == VAGINA_LOOSENESS.GAPING)
					temp2 = 60;
				if (player.vaginas[0].vaginalLooseness == VAGINA_LOOSENESS.GAPING_WIDE)
					temp2 = 115;
				if (player.vaginas[0].vaginalLooseness == VAGINA_LOOSENESS.LEVEL_CLOWN_CAR)
					temp2 = 175;
			}
			//Small
			if (temp == 1)
			{
				Render.text("a little small for your ", false);
				//determine size
				if (player.vaginas[0].vaginalLooseness == VAGINA_LOOSENESS.TIGHT)
					temp2 = 0;
				if (player.vaginas[0].vaginalLooseness == VAGINA_LOOSENESS.NORMAL)
			...
			if (temp == 2)
			{
				Render.text("almost too big to cram in your ", false);
				//determine size
				if (player.vaginas[0].vaginalLooseness == VAGINA_LOOSENESS.TIGHT)
					temp2 = 6;
				if (player.vaginas[0].vaginalLooseness == VAGINA_LOOSENESS.NORMAL)
			...
			{
				temp++;
				Render.text("You don't get far before you realize all the hair on your body has shifted to a verdant green color.  <b>You now have green hair.</b>  ", false);
				player.hairColor = "green";
			}
			//+hip up to 10
			if (rand(4) == 0 && player.lowerBody.hipRating <= 10)
			{
				temp++;
				Render.text("A strange shifting occurs below your waist, making your " + player.armorName + " feel tight.  <b>Your hips have grown larger</b>, becoming " + hipDescript() + ".  ", false);
				player.lowerBody.hipRating += rand(3) + 1;
				player.fertility++;
			}
			doNext(13);
		}
		else
		{
			...
			if (player.lowerBody.cockSpot.count() > 0)
			{
				...
				if (player.lowerBody.vaginaSpot.count() > 0)
					Render.text(" and " + vaginaDescript(0), false);
			}
			//Vagina
			else if (player.lowerBody.vaginaSpot.count() > 0)
				Render.text(vaginaDescript(0), false);
			//nipples
			else if (player.lowerBody.vaginaSpot.count() == 0 && player.lowerBody.cockSpot.count() == 0)
				Render.text("nipples", false);
			//Finish sentance
			Render.text(" tingle.  ", false);
			//Simple stat changes - + lust.
			dynStats("lus", 25 + player.stats.lib / 10, "cor", 2);
			...
			//+butt up to 10
			if (rand(4) == 0 && player.lowerBody.butt.buttRating <= 10)
			{
				temp++;
				Render.text("A strange shifting occurs on your backside, making your " + player.armorName + " feel tight.  <b>Your butt has grown larger</b>, becoming a " + buttDescript() + ".  ", false);
				player.lowerBody.butt.buttRating += rand(3) + 1;
			}
			//Rarely change one prick to a vine-like tentacle cock. 
			if (rand(3) == 0 && player.lowerBody.cockSpot.count() > 0 && player.hairColor == "green")
			{
				if (player.tentacleCocks() < player.lowerBody.cockSpot.count())
				{
					...
					if (player.lowerBody.cockSpot.count() == 1)
					{
						Render.text("Your feel your " + cockDescript(0) + " bending and flexing of its own volition... looking down, you see it morph into a green vine-like shape.  <b>You now have a tentacle cock!</b>  ", false);
					...
					if (player.lowerBody.cockSpot.count() > 1)
					{
						Render.text("Your feel your " + multiCockDescriptLight() + " bending and flexing of their own volition... looking down, you watch them morph into flexible vine-like shapes.  <b>You now have green tentacle cocks!</b>  ", false);
						temp2 = player.lowerBody.cockSpot.count();
						//Set cock flags
						while (temp2 > 0)
						{
							temp2--;
							player.lowerBody.cockSpot.list[temp2].cockType = CockType.TENTACLE;
						}
						temp++;
					}
				}
	...
	else if (eventNo == 2015)
	{
		...
		if (giacomo > 0)
		{
			...
			if (player.findStatusAffect(StatusAffects.WormOffer) < 0 && player.statusAffects.has("Infested"))
			{
				Render.text("Upon walking up to Giacomo's wagon, he turns to look at you and cocks an eyebrow in curiosity and mild amusement.\n\n", true);
				Render.text("\"<i>Been playing with creatures best left alone, I see</i>,\" he chuckles. \"<i>Infestations of any kind are annoying, yet your plight is quite challenging given the magnitude of corrupt creatures around here. It is not the first time I have seen one infested with THOSE worms.</i>\"\n\n", false);
				Render.text("You ask how he knows of your change and the merchant giggles heartily.\n\n", false);
				Render.text("\"<i>Do not look at me as if I am a mystic,</i>\" Giacomo heckles lightly. \"<i>Your crotch is squirming.</i>\"\n\n", false);
				Render.text("Looking down, you realize how right he is and attempt to cover yourself in embarrassment.\n\n", false);
				Render.text("\"<i>Fear not!</i>\" the purveyor jingles. \"<i>I have something that will cure you of those little bastards. Of course, there is also a chance that it will purge your system in general. This potion is not cheap. I will trade it for 175 gems.</i>\"\n\n", false);
				//Broke as a joke
				if (player.stats.gems < 175)
				{
					Render.text("You realize you don't have enough gems for such a pricey potion, but perhaps there is something else in his inventory you can buy.", false);
					doNext(2015);
				}
				//Can afford
				else
				{
					Render.text("Do you purchase his cure?", false);
					//Remove/No
					doYesNo(2081, 2015);
				}
				player.statusAffects.add(new StatusAffect("WormOffer", 0, 0, 0, 0)));
			}
			else
			{
				Render.text("You spy the merchant Giacomo in the distance.  He makes a beeline for you, setting up his shop in moments.  ", true);
				Render.text("Giacomo's grin is nothing short of creepy as he offers his wares to you. What are you interested in?", false);
				//If player is infested and knows of the cure..
				if (player.statusAffects.has("WormOffer") && player.statusAffects.has("Infested"))
				{
		...
		else
		{
			...
			Render.text("\"<i>So stranger, be you interested in some drafts to aid you in your travels, some quick pamphlets to warn you of dangers on journeys or...</i>\"\n\n", false);
			Render.text("Giacomo pauses and turns his head in both directions in a mocking gesture of paranoid observation. His little bit of theatrics does make you wonder what he is about to offer.\n", false);
			Render.text("\"<i>...maybe you would be interested in some items that enhance the pleasures of the flesh? Hmmm?</i>\"\n\n", false);
			Render.text("Giacomo's grin is nothing short of creepy as he offers his wares to you. What are you interested in?", false);
			simpleChoices("Potions", 2016, "Books", 2017, "Erotica", 2018, "", 0, "Leave", 13);
			statScreenRefresh();
			giacomo++;
	...
	{
		spriteSelect(23);
		Render.text("Which potion or tincture will you examine?", true);
		if (player.gender == 2)
	...
	{
		spriteSelect(23);
		Render.text("Which book are you interested in perusing?", true);
	...
	{
		spriteSelect(23);
		Render.text("Giacomo's grin is nothing short of creepy as he offers his wares to you. What are you interested in?", true);
		
		let dualBeltNumber: number = 0;
	...
	{
		spriteSelect(23);
		Render.text("Giacomo holds up the item and says, \"<i>Ah, yes! The quintessential elixir for all travelers, this little bottle of distilled livelihood will aid you in restoring your energy on your journey and, should you be hurt or injured, will aid the body's ability to heal itself. Yes ", true);
		if (player.gender == 1)
			Render.text("sir, ", false);
		if (player.gender == 2 || player.gender == 3)
			Render.text("madam, ", false);
		Render.text("this is liquid gold for pilgrim and adventurer alike. Interested? It is <b>15 gems</b></i>.\"  ", false);
		doYesNo(2020, 2016);
	}
	...
	else if (eventNo == 2020)
	{
		...
		{
			Render.text("\n\nGiacomo sighs, indicating you need " + String(15 - player.stats.gems) + " more gems to purchase this item.", true);
			doNext(2016);
		}
		else
		{
			player.stats.gems -= 15;
	...
	else if (eventNo == 2024)
	{
		...
		{
			Render.text("\n\nGiacomo sighs, indicating you need " + String(75 - player.stats.gems) + " more gems to purchase this item.", true);
			doNext(2016);
		}
		else
		{
	...
	else if (eventNo == 2025)
	{
		...
		if (player.gender == 0)
		{
			Render.text("She quickly moves down to your crotch... only to discover no organs down there.\n\n", false);
			Render.text("*record scratch*\n\n", false);
			
			Render.text("\"<i>Wait a fucking minute,</i>\" the Succubus says, \"<i>Where's your dick?!</i>\"\n\n", false);
			
			Render.text("As you state your genderless nature, the succubus hops off and from nowhere pulls out a large folder marked \"<i>Corruption of Champions-Script</i>\" and begins thumbing through the pages. After finding the page she is looking for, she reads it and looks off into the distance in disgust.\n\n", false);
			
			Render.text("\"<i>Hey Fenoxo and Dxasmodeus!!!!!!</i>\" the Succubus crows, \"<i>The goddamn script says that I should be milking someone's DICK!!! Man, futa, herm, I don't give a shit. YOUR OWN FUCKING SCRIPT SAYS I SHOULD BE MOUNTING AND MILKING A COCK!!!! THIS IS A SEX GAME!!!!!! THAT MEANS FUCKING! WHAT THE HELL AM I SUPPOSED TO FUCK???!!!</i>\"\n\n", false);
			
			Render.text("The Succubus looks at you with utter contempt, \"<i>THIS motherfucker doesn't have a DAMN thing! What am I supposed to do?! I can't exactly order a fucking Happy Meal!!!!!</i>\"\n\n", false);
			
			Render.text("Throwing the script down in an utter rage, the tantrum continues, \"<i>Goddammit! I can't believe this shit! HEY!!!!! INTERN!!!! Bring me my robe, aspirins and cancer sticks!!!!</i>\"\n\n", false);
			
			Render.text("The Succubus walks a few paces away where a plain-dressed woman with a clipboard hands the Succubus a pack of cigarettes and a small bottle of aspirin. She takes a fistful of the painkillers and immediately lights up a smoke. The Succubus takes a couple of drags off the cig and rubs her temples.\n\n", false);
			
			Render.text("\"<i>You two are killing me!</i>\" she groans in clear frustration, \"<i>I come to work for you perverts based off the promise of MORE perverts to feed from and you do THIS to me! I can't work like this!</i>\"\n\n", false);
			
			Render.text("The plain woman hands the Succubus a robe, which she crudely puts on as she storms off into the night.\n\n", false);
			
			Render.text("\"<i>I will discuss this horseshit with my agent,</i>\" the Succubus continues bitching, \"<i>THIS was NOT in my contract.</i>\"\n\n", false);
			
			Render.text("The Succubus stops, turns and points to you in derision. \"<i>And YOU! You no-cock, no-cunt having pissant! Take your ass back to the lab before they find out you escaped!!!!!</i>\"\n\n", false);
			
			Render.text("The Succubus resumes her stormy exit. You look at the bottle of Cerulean Potion and wonder if it REALLY had some psychotropics in it. What the hell just happened?!", false);
			flags[FlagEnum.CERULEAN_POTION_NEUTER_ATTEMPTED] = 1;
			doNext(1);
			return;
		}
		if (player.gender == 1)
			Render.text("\"<i>Awwww! Did my blue skin and pointy teeth scare you?</i>\" she says in a childish voice. \"<i>Believe me stud, if I wanted to harm you, I would not have let you wake up at all. I am here because you have 'called' me.</i>\" She teases you with the empty blue bottle you bought from the merchant. \"<i>My essence is in this bottle. Any man who drinks this, I am compelled to return the pleasure by drinking his.</i>\" The demon woman reaches her skinny hand down to your crotch where you see you have become fiercely erect. The demon gently strokes your cock until you begin oozing generous amounts of your own natural lubricants. The demon takes one of her massive breasts and teases you with her fat nipples. \"<i>Open your mouth,</i>\" she demands. \"<i>Take me into your mouth as I will soon take you into mine.</i>\"\n\n", false);
		else if (player.gender == 3)
		{
			flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00111]++;
			Render.text("\nIt is obvious that you have been confronted by a succubus. As the fire illuminates your captor, her grin widens broadly.\n\n", false);
			
			Render.text("\"<i>Well, well, well!</i>\" the Succubus jingles. \"<i>What have we here?! A little girl with a big cock!</i>\"\n\n", false);
			
			Render.text("As the Succubus looks down at your " + cockDescript(0) + ", you have quickly achieved one of the healthiest erections you have ever had. The succubus quickly poises her hairy hole over your member and allows her weight to force your dick into her womb. The demoness rests her weight in her lap as she allows you to fully penetrate her. Her womb is hot and wet and her muscles have your prick in one of the strongest grips imaginable. Even if you went totally limp, withdrawal would be an impossibility. Wincing at the sudden crushing force of her vaginal muscles, the succubus giggles inhumanly.\n\n", false);
			
			Render.text("\"<i>Quit whimpering,</i>\" the Succubus orders. \"<i>I hope the rumors about you futas are true. I need a good, fiery load of cum to get me going. I haven't had one in a while and as much as I LOVE men, they can only feed me so much.</i>\"\n\n", false);
			
			Render.text("You quickly try to struggle, but find the Succubus to be utterly dominating. She wraps her arms around your back and entwines her lean legs around your hips. The Succubus playfully licks your lips and grins.\n\n", false);
			
			Render.text("\"<i>You are getting your dick milked,</i>\" the Succubus says flatly, \"<i>Accept it. Trust me, when I am done, you will want more of me, anyway.</i>\"\n\n", false);
			
			Render.text("As the Succubus finishes her ultimatum, you feel churning vaginal contractions stroking your massive cock. Heavy, powerful, coordinated undulations work your dick as surely as the best handjob. You quickly moan in shock and pleasure at such rough treatment.", false);
		}
		dynStats("lus", 35);
		doNext(2026);
	}
	...
	else if (eventNo == 2027)
	{
		...
		if (player.gender == 1)
		{
			Render.text("Rotating herself into a 69 position, she seizes your throbbing member and effortlessly begins deep throating. Her thighs wrap around your head and confront you with her surprisingly hairy pussy. Her clitoris is long and erect, begging for attention and the smell of her pheromones enslaves you. You bury your face into her furry mound, ignoring your normal revulsion to such an unshaved state and begin eating her as well as any woman you have ever pleased. The demon takes your cock out of her mouth to cry in delight. \"<i>YES, LITTLE MAN!</i>\" she screams. \"<i>LICK ME! TEASE ME! LOVE MY WOMB WITH YOUR TONGUE!</i>\" She responds by clamping her mouth around the head of your penis and sucking smartly. A sharp pain in your ass signals the entry of her bony fingers working their way to your inner manhood. Finding the root of your sex easily, she mashes down to force you to cum.\n\n", true);
			Render.text("Finding it impossible to resist such pleasure, you immediately begin cumming. Glob after glob, stream after stream of your semen shoots into the woman's mouth. Her timed sucking ensures that she swallows each drop as you launch it into her. While you have been proud of the ability to cum in a woman for over a minute, you are wracked with both pain and pleasure as your ejaculations continue for almost ten. Once you have spent your last, the demon releases your penis to bear down on your face with her thighs and unloads a massive squirting orgasm. Your face is soaked with pussy juice as you see her cunt spasm from the force of her pleasure. The sight of her rhythmic muscles is hypnotic. She then promptly removes her finger from your ass.", false);
		}
		else
		{
			Render.text("The Succubus rears up and pins your shoulders firmly to the ground. The speed of her vaginal contractions becomes impossibly fast. As you look down at the Succubus' crotch, you can clearly see her contractions working your penis despite the massive growth of fur between your legs. The pressure deep behind your cock is crushing. With one last spasmodic push, you release a thick, superhuman gout of semen deep in the Succubus' cunt. She lets out an inhuman howl of pleasure as her womb clamps down upon you like a vise.  She literally squeezes each stream and drop of cum out of you.\n\n", false);
			
			Render.text("After a couple of minutes, you feel your dick weaken and begin going limp. Satisfied, you feel the Succubus' grip on your prick release and you quickly fall out of her womb. Much to your surprise, despite the massive load you released, not a single drop of cum falls out or is left on your prick. The Succubus begins rubbing herself all over, clearly in post-orgasmic ecstasy from absorbing your fluids. After a couple of chain-orgasms, the Succubus sits down next to the empty jar.\n\n", false);
			
			Render.text("She places the mouth of the jar next to one of her fat nipples and begins milking her tit into the jar, filling it back up with her fluids. She places the jar next to you and stands up.\n\n", false);
			
			Render.text("\"<i>It has been a long time since I soaked up a load that good,</i>\" she says, \"<i>Anytime you want more of that, just drink my milk again. It's only fair. I take your milk, you take mine.</i>\"\n\n", false);
			
			Render.text("She smiles and flies off, leaving you with a fresh bottle of \"Cerulean Potion\". As pleasing as the experience was, it has left you thoroughly exhausted.", false);
	...
	else if (eventNo == 2032)
	{
		...
		{
			Render.text("The crazy merchant said you might not need this and he was right. Written at a simple level, this was obviously intended for a city-dweller who never left the confines of their walls. Littered with childish illustrations and silly phrases, the book is informative in the sense that it does tell a person what they need and what to do, but naively downplays the dangers of the forest and from bandits. Were it not so cheap, you would be pissed at the merchant. However, he is right in the fact that giving this to some idiot ignorant of the dangers of the road saves time from having to answer a bunch of stupid questions.", true);
			doNext(2017);
			player.stats.gems -= 1;
	...
	else if (eventNo == 2033)
	{
		...
		{
			Render.text("<b>You already own a Hentai Comic!</b>", true);
			doNext(2017);
			return;
		}
		Render.text("Giacomo takes out a colorfully written magazine from his bag. The cover contains well-drawn, overly-endowed women in sexual poses. \"<i>Perhaps your taste in reading is a bit more primal, my good ", true);
		if (player.gender == 1)
			Render.text("man", false);
		if (player.gender == 2 || player.gender == 3)
			Render.text("lady", false);
		if (player.gender == 0)
			Render.text("...err, whatever you are", false);
		Render.text("</i>,\" says Giacomo. \"<i>Taken from the lands far to the east, this is a tawdry tale of a group of ladies seeking out endless pleasures.  With a half a dozen pictures on every page to illustrate their peccadilloes, you will have your passions inflamed and wish to join these fantasy vixens in their adventures! Collectable and in high demand, and even if this is not to your tastes, you can easily turn a profit on it! Care to adventure into the realm of fantasy? It's only 10 gems and I am doing YOU a favor for such a price.</i>\"", false);
		doYesNo(2034, 2017);
	}
	//Hentai Comic Sale
	...
	else if (eventNo == 2035)
	{
		...
		{
			Render.text("<b>You already own a Dildo!</b>", true);
			doNext(2018);
			return;
		}
		Render.text("Giacomo takes out a slender tube roughly over half a foot in length. \"<i>Since you seek pleasure, this is as simple and effective as it gets. This dildo is a healthy seven inches long and is suitable for most women and even adventurous men. Pick a hole, stick it in and work it to your heart's content or your partner's pleasure. The single-piece construction makes it solid, sturdy and straightforward. For 20 gems, you can take matters into your own hands. How about it?</i>\"", true);
		doYesNo(2036, 2018);
	}
	//Buy Dildo
	else if (eventNo == 2036)
	{
		...
		{
			Render.text("\n\nGiacomo sighs, indicating you need " + String(20 - player.stats.gems) + " more gems to purchase this item.", true);
			doNext(2018);
		}
		else
		{
			Render.text("After making the payment, Giacomo hands you the Dildo", true);
			doNext(2018);
	...
	else if (eventNo == 2038)
	{
		...
		{
			Render.text("\n\nGiacomo sighs, indicating you need " + String(30 - player.stats.gems) + " more gems to purchase this item.", true);
			doNext(2018);
			return;
		}
		Render.text("After making the payment, Giacomo hands you the Self-Stimulation Belt", true);
	...
	else if (eventNo == 2040)
	{
		...
		{
			Render.text("\n\nGiacomo sighs, indicating you need " + String(40 - player.stats.gems) + " more gems to purchase this item.", true);
			doNext(2018);
			return;
		}
		Render.text("After making the payment, Giacomo hands you the All-Natural Self-Stimulation Belt", true);
	...
	else if (eventNo == 2042)
	{
		...
		Render.text("Giacomo loads you up onto his cart and sets off for his next sale.  You do not care.  You do not realize what has happened.  All you know is that the creature keeps cumming and it feels... sooooo GODDAMN GOOD!", false);
		eventParser(5035);
	}
	//Buy Onahole
	else if (eventNo == 2043)
	{
		...
		Render.text("After making the payment, Giacomo hands you the Plain Onahole", true);
	...
	else if (eventNo == 2045)
	{
		...
		{
			Render.text("\n\nGiacomo sighs, indicating you need " + String(50 - player.stats.gems) + " more gems to purchase this item.", true);
			doNext(2018);
			return;
		}
		Render.text("After making the payment, Giacomo hands you the Deluxe Onahole", true);
	...
	{
		Render.text("You pull the sloppy toy from your dribbling dick and smile, shoving its slippery surface down on another of your " + multiCockDescriptLight() + ".  You rapidly work it around your cocks, orgasming until ", true);
		if (player.lowerBody.balls > 0)
			Render.text("you pass out with aching, empty balls.", false);
		else
			Render.text("you pass out with " + multiCockDescriptLight() + " sore from exertion.", false);
		player.orgasm();
		dynStats("sen", -1);
		doNext(13);
	}
	...
	else if (eventNo == 2049)
	{
		...
		{
			Render.text("\n\nGiacomo sighs, indicating you need " + String(150 - player.stats.gems) + " more gems to purchase this item.", true);
			doNext(2018);
			return;
		}
		Render.text("After making the payment, Giacomo hands you the All-Natural Onahole", true);
		player.createKeyItem("All-Natural Onahole", 0, 0, 0, 0);
		doNext(2018);
		player.stats.gems -= 150;
		statScreenRefresh();
	}
	...
	else if (eventNo == 2050)
	{
		if (player.gender == 3)
		{
			...
			Render.text(String(int(((Math.random() * player.str / 12) + player.str / 6) * 10) / 10), false);
			Render.text(" feet away from you. Delirious with pleasure, you continue your 'impression' of a semen volcano, covering yourself and the area with your seed. ", false);
			Render.text(" As your orgasms fade, you find yourself a well-fucked mess, and pass out.", false);
			player.orgasm();
			dynStats("sen", -1);
		}
		doNext(1);
...
/*
	...
	{
		Render.text("You clear your head and resolve to defeat the monstrous bee-woman.", true);
		spriteSelect(6);
...
/* Moved to rathazul.as
	...
	else if (eventNo == 2071)
	{
		if (player.stats.gems < 20)
		{
			Render.text("Rathazul says, \"<i>You do not have enough gems for that service.</i>\"", true);
	...
	{
		Render.text("You reach forward to cop a feel.  The goddess' eyes go wide with fury as a massive branch swings down, catching you in the sternum.  It hits you hard enough that you land in your boat and float back a few feet into the water.  Nothing to do but leave and hope for another chance at her breasts...", true);
		doNext(13);
	...
	{
		Render.text("You turn and run for the boat, leaving the corrupt goddess behind.  High pitched laugher seems to chase you as you row away from the island.", true);
		doNext(13);
	}
	...
	else if (eventNo == 2081)
	{
		...
		Render.text("Upon seeing the fat worm, Giacomo displays a freakish celerity by jumping off his cart, grabbing an empty container and collecting the fat worm. Regaining your senses, you look at him with inquisitive shock at what he just did.\n\n", false);
		Render.text("\"<i>You have to realize that I AM a merchant, after all.</i>\", he calmly replies. \"<i>Hell for you is heaven for someone else. This bugger will easily fetch 10,000 gems to some noble looking for a quick buzz. Don't WE know better!</i>\"\n\n", false);
		Render.text("The merchant puts away his prize and comes back to help you up. \"<i>Here.</i>\", he says as he shoves a couple of bottles into your hand. \"<i>This is on the house. You probably need it after the shock of getting those things out.</i>\"\n\n", false);
	...
	else if (eventNo == 2083)
	{
		...
		Render.text("Being too tired for anything else, you join Jojo in meditation, which does much to relive you of your former woes.", false);
	...
	else if (eventNo == 2084)
	{
		...
		if (player.findStatusAffect(StatusAffects.LustyTongue) < 0)
		{
			...
			if (rand(4) == 0)
			{
				Render.text("  The feeling doesn't seem to fade, only becoming more and more intense over the coming hour.  It will be hard to keep from getting turned on any time you lick your lips or eat some food.", false);
				player.statusAffects.add(new StatusAffect("LustyTongue", 24, 0, 0, 0)));
			}
			//ELSE
			else
				Render.text("  Thankfully, the feeling goes away after a few minutes, leaving you feeling just a bit more sensitive.", false);
		}
		...
		if (rand(2) == 0)
		{
			...
			{
				Render.text("  As you redress, you feel a sense of added weight on your chest.  After a few moments you realize your already-formidable chest has grown even larger.", false);
			...
			else
			{
				...
				{
					Render.text("  As you redress, you realize you have grown a pair of luscious breasts!  Perhaps this was a good idea after all...", false);
					player.growTits(1, 2 + rand(3), false, 1);
				}
				else //Small tits growth
				{
					Render.text("  As you redress, you realize your breasts have gotten quite a bit larger!  Maybe you can come back later when you aren't so full and plump them up a bit more.", false);
					player.growTits(1, 1 + rand(3), false, 1);
				}
			}
		}
...
/*
	...
	{
		let ttemp2:Function = null;
		let ttemp3:Function = null;
		let ttemp4:Function = null;
		let ttemp5:Function = null;
		let ttemp6:Function = null;
		let ttemp7:Function = null;
		let ttemp8:Function = null;
		let ttemp9:Function = null;
		let ttemp10:Function = null;
		if ((player.perks.has("MarblesMilk") || flags[FlagEnum.MARBLE_PURIFICATION_STAGE] >= 5) && player.hasItem(consumables.LACTAID, 1))
		{
			if(flags[FlagEnum.MARBLE_PURIFICATION_STAGE] < 5) ttemp2 = marbleScene.giveMarbleLactaid;
			else ttemp2 = marblePurification.lactaidForPureMurble;
		}
		if (player.hasItem(consumables.P_DRAFT, 1) && flags[FlagEnum.MARBLE_DICK_TYPE] == 0)
		...
		if (flags[FlagEnum.MARBLE_BOVA_LEVEL] < 2 && player.hasItem(consumables.PROBOVA, 1))
		...
		if (player.hasItem(consumables.P_LBOVA,1) && flags[FlagEnum.MARBLE_PURIFICATION_STAGE] == 1)
			ttemp8 = marblePurification.giveMarblePureQuestLabova;
		if (player.hasItem(consumables.P_S_MLK,1) && flags[FlagEnum.MARBLE_PURIFICATION_STAGE] >= 5)
			ttemp9 = marblePurification.purifiedSuccubusMilkForPureMarbles;
		if (player.hasItem(consumables.REDUCTO,1) && flags[FlagEnum.MARBLE_PURIFICATION_STAGE] >= 5)
			ttemp10 = marblePurification.pureMurbleUsesReducto;
			
		Render.text("What item do you want to give Marble?", true);
	...
	else if (eventNo == 2143)
	{
		...
		{
			Render.text("<b>You already own a dual belt!</b>", true);
			doNext(2018);
			return;
		}
		if (player.stats.gems < 50)
		{
			Render.text("You do not have enough gems to purchase this item.", true);
			doNext(2018);
			return;
		}
		Render.text("You are a bit dubious at the pleasure it could offer you, but it would be better than being raped by the creatures constantly... maybe to even work out some excess lusts... hesitantly, you reach into your bag and grab 50 gems, handing it to him. He greedily snatches it from your palm and hands you with the belt with a smile. \"<i>I promise you won't be disappointed.</i>\" He counts the gems and waves goodbye.\n\n(<b>Dual Belt acquired!</b>)", true);
	...
	{
		Render.text("", true);
		Render.text("You look at the thing in front of you and sigh, your " + multiCockDescriptLight() + " and " + vaginaDescript(0) + " dripping with fluids. With a nervous sigh you step into the underwear looking device and slip it up your legs, the cold metal feeling so good on your " + player.skinDesc + ", sending a rush of feelings up your body. You slip your " + cockDescript(0) + " down and into a hole on the front of the belt, the extra length sticking out on the other side of it. Underneath the hole and right above your pussy is another metal square with what feels like a small nub on the inside rubbing against your clit. Shivering from the feeling of it, you stay there for a moment, waiting in anticipation. Finally, you reach down to the side of the belt and flick the switch to the on position.\n\n", false);
		...
		if (player.tailType > TAIL.NONE)
			Render.text("covering up your tail, ", false);
		Render.text("and then going up your body, covering your " + allBreastsDescript() + " and neck. The only part of your body unclad by the suit is your head. The blackness feels slick and smooth, almost cold, a strange type of feeling washes over you until you realize that it is a rubber suit.\n\n", false);
		...
		Render.text("Meanwhile the latex around your " + cockDescript(0) + " begins to pulse and ripple faster than ever before. You quake and quiver, " + player.legs() + " giving out as it teases and pulses around your " + allBreastsDescript() + ". Your hands go down your body helplessly and start stroking at your encased cock, rubbing up and down your length. Unfortunately, all things must come to an end as the pleasure gets to be way too much and you feel yourself cum. Your hips buck wildly as you feel cum spurt into the latex, the end swelling up and filling like a ", false);
		if (player.cumQ() > 200)
			Render.text("massive ", false);
		Render.text("balloon. ", false);
		if (player.cumQ() >= 1000)
			Render.text("It grows larger and larger until you are sure it will pop, but it doesn't.  It just sloshes around - a huge bubble, nearly waist high. ", false);
		Render.text("Your eyes close in shivered ecstasy as your cunt spasms and clutches down around the hardened section deep inside of you. ", false);
		//([If high lactation]
		if (player.biggestLactation() >= 2)
			Render.text("Milk gushes out from your " + breastDescript(0) + " as you orgasm, filling the inside of the suit with a slick layer of milk and forming milk bubbles that hang lewdly off your chest. ", false);
		...
		Render.text("When you wake, the black latex is no longer covering your body and the belt is silent around your waist. Cum drips from the tip of your cock and the top part of your " + player.legs() + " are coated with your feminine juices. ", false);
		if (player.biggestLactation() >= 2)
			Render.text("Thin streams of creamy milk flow from your " + allBreastsDescript() + ", your torso and midsection dripping wet from the stuff. ", false);
		Render.text("Completely sated, you take off the belt, finding it slides off easily, and put it away in your campsite, eagerly awaiting the time you can next use it and have the suit work you over once more.", false);
...
/*
	...
	else if (eventNo == 2149)
	{
		...
		{
			Render.text("You offer Jojo the chance to stay at your camp, but before you can finish your sentence he shakes his head 'no' and stalks off into the woods, remembering.", false);
			doNext(13);
			return;
		}
	...
	else if (eventNo == 2150)
	{
		...
		if (player.statusAffects.has("Infested"))
		{
			...
			if (player.statusAffects.has("JojoNightWatch")) Render.text("(Jojo is currently watching for enemies at night.)\n\n", false);
		...
		else
		{
			...
			{
				Render.text("You approach the boulder where Jojo usually sits, and as soon as you're close Jojo approaches you with urgency. “<i>By Marae! [name], we must do something! I feel the corruption surrounding you like a dense fog. We need to meditate or I’m going to lose you!</i>” Jojo pleads.\n\n");
			}
			else if (player.stats.cor > 10)
			{
			...
			else
			{
				...
				if (selector == 0)
				{
					Render.text("You walk toward the boulder where Jojo usually sits, and see him cross legged with his eyes closed.  At first he seems to be deep in meditation, but when you approach his mouth curls into a smile; he gets up and opens his eyes regarding you with a welcoming expression.  “<i>Greetings [name], is there anything I can assist you with?</i>”\n\n");
				}
				else if (selector == 1)
				{
					Render.text("You walk up to the boulder where Jojo usually sits and find him a few paces behind it. He is standing and practicing his form, gracefully moving from one pose to the next. As you approach him you see his ears visibly perk and he turns his head towards you without breaking his stance, saying, “<i>Greetings [name], is there anything I can assist you with?</i>”\n\n");
				}
				else if (selector == 2)
				{
					Render.text("You find Jojo sitting cross-legged on a flat rock with his staff leaning against his shoulder, thinking.  He looks to you and nods, \"<i>Greetings, " + player.short + ".  Is there something I could do to assist you?</i>\"\n\n", true);
				}
			}
			
			// New "offer of help" menu
			if (player.stats.cor > 10)
			{
			...
			else
			{
				...
				if (flags[FlagEnum.UNLOCKED_JOJO_TRAINING] == 1) addButton(2, "Train", jojoScene.apparantlyJojoDOESlift);
	...
	else if (eventNo == 2151)
	{
		...
		let doClear:boolean = jojoScene.doClear;
	...
	else if (eventNo == 2153)
	{
		...
		Render.text("You ask Jojo if he'd like to go on a hunt through the woods to clear out some of the corrupted creatures, and the mouse readily agrees.  He asks if you've been getting a bit stir-crazy from having your camp in one place as the two of you walk into the woods...", true);
...
/* Moved to rathazul.as
	...
	else if (eventNo == 2190)
	{
		...
		//[low corruption]
		if (player.stats.cor < 33)
			Render.text("in disgust.", false);
		else if (player.stats.cor < 66)
			Render.text("in confusion.", false);
		else
			Render.text("reluctantly.", false);
		...
		if (player.statusAffects.get("MinoPlusCowgirl").value1 == 0)
			Render.text("  Apparently this isn't an uncommon show, and the locals enjoy it immensely.", false);
		//Lust!
		dynStats("lus", 5 + player.stats.lib / 20 + player.minoScore() + player.cowScore());
		doNext(13);
	}
	...
	{
		Render.text("Tamani adopts a fighting pose and says, \"<i>If I have to I'll beat my children out of you!</b>\"", true);
...
/*
	...
	else if (eventNo == 2941)
	{
		...
		{
			Render.text("<b>You already own a yoga guide!</b>", true);
			doNext(2017);
			return;
		}
		if (player.stats.gems < 100)
		{
			Render.text("You cannot afford a yoga guide!", true);
			doNext(2017);
			return;
		}
		Render.text("You exchange 100 gems for the tome.  Now you can finally enjoy a workout with Cotton!", true);
	...
	{
		flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00275] = 1;
		rathazul.chooseArmorOrRobes();
	}
	else if (eventNo == 3004)
	{
		flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00275] = 2;
		rathazul.chooseArmorOrRobes();
	}
