...
public function plantyLadySeeWhatHappens():void {
	clearOutput();
	outputText("You decide to not offer any resistance as the vines begin to tighten around your body.  When they have you nearly constricted, they stop suddenly.  Looking downward confirms the vines relaxing and slowly uncoiling from around your legs." + (player.stats.cor >= 66 ? "  A sense of disappointment comes to you at the sight of the retreating vegetation, robbing you of potential fun." : "") + "  Standing there in utter confusion, you scratch the back of your head and survey the thick bush that now surrounds you, when a creature appears with a soft rustle to the left!");
...
public function plantWomanStruggle():void {
	...
	//(struggle: failure, or Intro 1.2.3) (< 40 STR)
		...
		if(player.gender > 0) {
			...
			if(player.gender == 3 || (player.lowerBody.vaginaSpot.count() + player.lowerBody.cockSpot.count() > 1)) outputText("them")
			else outputText("it");
...
public function escapeFromPlantWomanAndDeclineSex():void {
	...
	if(player.stats.cor < 50) outputText("you are taking your leave and don't wish for any trouble.");
	...
	if(9999 == 9999) {
		...
		if(player.stats.cor < 50) outputText("walking away");
	...
	//Failure to persuade/intimidate (SD 1.1f)
		...
		if(player.stats.cor < 50) outputText("calm her down");
		else outputText("remind her that attacking you will earn her a world of pain");
		outputText(", but she isn't buying any of it.  You ");
		if(player.stats.cor < 50) outputText("shrug and turn you back, beginning the long trek back towards camp");
...
public function fuckMyAssPlantLadyWhileImTrapped():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("stroke");
	else outputText("finger");
	outputText(" you.");
	
	outputText("\n\nAs she takes ");
	if(player.lowerBody.cockSpot.hasCock()) outputText("[oneCock]");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your " + clitDescript());
...
shudder from the sensation of your [asshole]'s tightness.  Once the floral creature regains her composure, she begins to pump in and out of your ass at a gradually faster pace.");
	...
	if(player.shortestCockLength() < 48) {
		outputText("\n\nYour ");
		if(player.lowerBody.balls > 0) outputText("[balls] and ");
		outputText(cockDescript(player.shortestCockIndex()) + " flap");
		if(player.lowerBody.balls > 0) outputText("s");
	...
	if(player.lowerBody.balls == 0) outputText("up");
	else outputText("down");
	outputText(" to your ");
	if(player.lowerBody.balls == 0) outputText(nippleDescript(0));
...
public function trappedFlowerSexBlowjobAhoy():void {
	...
	//(If PC has balls: 
	if(player.lowerBody.balls > 0) outputText("wrap themselves around your ([balls] and begin to squeeze");
	//(If PC has a cock(s): 
	else if(player.lowerBody.cockSpot.hasCock()) outputText("wrap themselves around your " + multiCockDescriptLight() + " and begin to squeeze");
	//(If PC has a pussy: 
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("take hold of you clitDescript() and tug hard");
	//(If PC is genderless: 
...
public function trappedPlantSexPussyGOOO():void {
	...
	if(player.lowerBody.cockSpot.list[x].cockLength < 4) {
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("s gain");
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("es are now ");
	...
	//(if one cock:
	if(player.lowerBody.cockSpot.count() == 1) outputText("  The plant creature begins to speed up her thrusting against your cock, eager to drive you over the edge with the tightness of her hole.");
	else {
		//(If multi-cocked: 
	...
	//(Multi-cocked: 
	if(player.lowerBody.cockSpot.count() > 1) outputText("  Buried deep in the makeshift vaginas, [eachCock] thrusts forward and eagerly deposits the nourishment the creature craves.");
	//Vagina: 
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Cumming hard, your " + clitDescript() + " releases a liberal stream of juices down the vine that fucks you, dissolving into the appendage as you moan in furious release.");
...
public function trappedPlantSexWantsToBlowYa():void {
	...
	if(player.lowerBody.cockSpot.count() >= 4 || player.totalCockThickness() >= 10) outputText("... or she tries to, as she fumbles with your freakish bundle of dicks.  Unable to fit them all into her hand she shrugs");
...
public function trappedPlantSexBlowMe():void {
	...
	if(player.longestCockLength() < 24) {
		...
		if(player.totalCockThickness() < 6) {
			...
			if(player.lowerBody.cockSpot.count() > 1) outputText(".  What a champ");
		}
		...
		if(player.lowerBody.cockSpot.count() > 2) {
			...
			if(player.lowerBody.cockSpot.list[player.thinnestCockIndex()].cockLength < 12) outputText(" against her face, while planting her hands against your thighs");
		...
		if(player.lowerBody.cockSpot.count() >= 2) outputText("two ");
		...
		if(player.lowerBody.cockSpot.count() >= 4) outputText("upper shafts");
		else if(player.lowerBody.cockSpot.count() >= 3) outputText("upper shaft");
		else outputText("[hips]");
		outputText(" as she better positions herself for the entirety of your ");
		outputText("cock");
		if(player.lowerBody.cockSpot.count() > 1) outputText("s as they make");
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("s");
		outputText(" of your ");
		if(player.lowerBody.cockSpot.count() > 1) outputText("two ");
		outputText("dick");
		if(player.lowerBody.cockSpot.count() > 1) outputText("s");
		...
		if(player.lowerBody.balls > 0) outputText("scrotum's");
		else {
			if(player.lowerBody.cockSpot.count() == 1) outputText("cock's");
		...
		if(player.lowerBody.cockSpot.count() == 1) outputText("prop some of your length up on her average body to prevent it from straining towards the ground.");
...
public function plantAnalGivingTheDick():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("s poke");
...
public function failStruggle69():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(multiCockDescriptLight());
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("[clit]");
...
public function nowayStruggle69Plant():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(multiCockDescriptLight);
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("[clit]");
...
public function struggle69Plant(pt: number = 1):void {
	...
	if(target == 2) {
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			...
			if(player.lowerBody.cockSpot.count() > 1) outputText("s explode");
	...
	//Cock(s): 
		...
		if(player.lowerBody.balls > 0) outputText("testicles begin");
		else {
			outputText("cock");
			if(player.lowerBody.cockSpot.count() == 1) outputText(" begins");
			else outputText("s begin");
		}
		outputText(" to churn ");
		if(player.lowerBody.balls == 0 && player.lowerBody.cockSpot.count() == 1) outputText("its");
...
public function defeatOrDomPlantAndCuntFuck():void {
	...
	//If PC has vagina: 
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Needing some self stimulation to bring your lust to a boil, you lower your hand to your [clit] and begin to stroke your love button, eager to get yourself off in more than one way");
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.cockSpot.count() == 2) outputText(" flaps");
	...
	if(player.lowerBody.balls == 0) outputText(multiCockDescriptLight());
	else outputText(ballsDescriptLight());
	outputText(" acknowledge");
	if(player.lowerBody.balls == 0 && player.lowerBody.cockSpot.count() == 1) outputText("s");
	outputText(" her desire and continue to shoot thick streams of your seed into her");
	if(player.lowerBody.cockSpot.count() > 1) outputText(" as well as on her stomach and the ground beneath you");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("; your vaginaDescript(0) quivers with the need for release, letting loose with its own torrent of lady fluids as you moan in orgasm");
	...
	if(player.lowerBody.balls > 0) outputText("[balls] really hurt");
	else {
		outputText("dick really hurts");
		if(player.lowerBody.cockSpot.count() > 1) outputText("s really hurt");
	}
...
public function victoryDefeatPlantGirlGetBJMainSCREENON():void 
{
	...
	//PC has balls: (
	if(player.lowerBody.balls > 0) outputText(" cusps your [balls] in her hand, and suck on the sides of your [sack]. She licks upwards from your testicles and caresses them before");
	...
	if(player.tentacleCocks() > 0 && (player.tentacleCocks() == player.totalCocks() || rand(2) == 0)) {
		...
		if(player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockLength < 14) outputText("takes the entirety of your [cock biggest] in her mouth and down her throat.  You lurch forward as your cock hits the back of her throat, moaning intensely at the plant girl's ability to take your whole penis in her mouth.");
...
public function plantGirlPCWinGetTentacleBJ():void {
	...
	//[If tentacle cock is longer than 48 inches]
		...
		if(player.lowerBody.balls > 0 || player.lowerBody.vaginaSpot.hasVagina()) {
			outputText(" as your ");
			if(player.lowerBody.balls > 0) outputText("[balls] slap");
...
public function beatPlantGirlDomLetsFuckAnal():void {
	...
	if (inCombat) {
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText(" grabs a of your penis and guides it to her vagina.");
		else {
			outputText(", while she ");
			//PC has testicles: (
			if(player.lowerBody.balls > 0) outputText("fondles your testicles sensually; surprisingly she doesn't injure you by squeezing too hard despite the rough fucking.");
		}
		//Vagina check:
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			if(player.lowerBody.cockSpot.count() == 1 && player.lowerBody.balls > 0) outputText("  Not content with just teasing your balls, the");
			else outputText("  The");
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText(" along with its vagina-clad brother");
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Your [vagina] obliges, and a few steams of lady spunk flow from your [clit] and intertwine with the fauna creature's fluids on the ground.");
	...
	//(Dominant Let's Fuck! (giving anal) D 1.3
		...
		if(player.lowerBody.balls > 0) outputText(" your balls [balls] and");
...
public function victorySuckPlantgirlsDick():void {
	...
	//(Dominant intro, D 1.4: 
		...
		//PC has a dick: 
		if(player.lowerBody.cockSpot.hasCock()) outputText("take [oneCock] and poke the tip of your head against her pelvis; where a penis would be located if she were a guy... or pretty much everything else around here.");
	...
	if (inCombat) {
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("[eachCock] is rigidly erect");
		else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your [vagina] is gazed over with your secretions");
		//Genderless: 
...
public function beatPlantGirlAndTakePlantCockInButt():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("[eachCock] becomes rock solid");
	if(player.gender == 3) outputText(" while ");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your pussy becomes incredibly wet");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("[eachCock] jerks upward, the last shot of blood filling and hitting them like a thunderous punch");
	if(player.gender == 3) outputText(", while ");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your pussy leaks a bit of girl cum.  It slowly drools out onto her pelvis");
	if(player.gender == 0) outputText("your body quivers from the sensation");
	outputText(".  ");
	//PC has balls: 
	if(player.lowerBody.balls > 0) outputText("Your [balls] laying on top of her pelvis, y");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("Your vaginal muscles clenching up along with your sphincter, y");
	else outputText("Y");
 	outputText("ou ");
	if(player.lowerBody.balls > 0) outputText("begin to massage your [sack] as you raise your hips upward");
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("relish in the moment and allow a hand to drop to your sex as you raise your hips, eager to please both ends during this act");
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) outputText("place both hands on her shoulders");
	...
	if(player.gender > 0) {
		...
		if(player.lowerBody.cockSpot.hasCock()) outputText("[oneCock]");
		if(player.lowerBody.vaginaSpot.hasVagina()) outputText("your vulva");
	}
	//PC is genderless:
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("strokes [oneCock] with her hands");
	if(player.gender == 3) outputText(" as she also ");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("massages your labia");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("quickens her pace of stroking your cock");
	if(player.gender == 3) outputText(" and ");
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("fingers the interior of your pussy, causing you to roll your eyes into the back of your head as she sensually teases the inside of your sex");
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("vaginal muscles clamping down and drooling pussy juice all over the plant woman's lap");
...
public function defeatPlantgirlBlowAndButtfuckPure():void {
	...
	//(pure: 
	if(player.stats.cor < 50) outputText("giggle");
	//Corrupted: 
