...
public function encounterPlantWoman():void {
	...
	if(flags[FlagEnum.TIMES_MET_PLANTGIRL] == 0) {
		Render.text("While out exploring the deep woods you come across a thick bush full of vines and thorns that is obstructing your path.  Strange...that wasn't there a few days ago.  You remember the perverted, sexual vegetation, but not this.  You scan the brush for a way to get through to no avail.  As you begin to turn, you hear an eerie, ominous coo which causes you to freeze, listening for further movement or noise.  After a few moments, you sigh and begin to head back the way you came... only to hit another patch of dense foliage, blocking your advance once again.");
...
public function plantyLadySeeWhatHappens():void {
	clearOutput();
	Render.text("You decide to not offer any resistance as the vines begin to tighten around your body.  When they have you nearly constricted, they stop suddenly.  Looking downward confirms the vines relaxing and slowly uncoiling from around your legs." + (player.stats.cor >= 66 ? "  A sense of disappointment comes to you at the sight of the retreating vegetation, robbing you of potential fun." : "") + "  Standing there in utter confusion, you scratch the back of your head and survey the thick bush that now surrounds you, when a creature appears with a soft rustle to the left!");
...
public function plantLadyLetsFuck(from:number = 1.1):void {
	clearOutput();
	Render.text("Throwing caution to the wind, you decide to engage in a little 'carnal' pleasure with this creature. How to do you want to sex the plant woman?");
...
public function plantWomanStruggle():void {
	...
	//(struggle: failure, or Intro 1.2.3) (< 40 STR)
		...
		Render.text("Despite your best efforts, you can't seem to break free from the vines as they tense around your limbs and then your waist.  You bite your lower lip and brace for whatever is coming to \"greet\" you.");
		//(leads to 1.2.3m)
		...
		if(player.gender > 0) {
			Render.text("sending blood rushing down to your ");
...
public function struggleFailDoItPlantSmex():void {
	clearOutput();
	Render.text("As she goes in for another pseudo kiss you lurch forward and connect with your lips, pressing them hard against her and fiercely returning her intimate gesture. She responds by snaking her tongue in your mouth, slapping hers against yours and twisting them around like a frustrated child with a knotted shoe lace.");
...
public function escapeFromPlantWomanAndDeclineSex():void {
	...
	Render.text("You stare at the plant woman and shake your head.  Sorry, you're not interested.  The plant woman continues to stare at you, uncertain of what you are saying.  You gesture to the monster that ");
	if(player.stats.cor < 50) Render.text("you are taking your leave and don't wish for any trouble.");
	else Render.text("following or attacking would be a bad idea.");
	//(Connects to failure to persuade)
	//(Success, SD 1.1s)
	if(9999 == 9999) {
		Render.text("\n\nThe monster, from what you can tell, deliberates your gesture of ");
		if(player.stats.cor < 50) Render.text("walking away");
		else Render.text("intimidation");
		Render.text(" and shrugs.  She promptly flees back into the woods.  You are left wondering just how messed up the local fauna and wildlife in Mareth is before promptly returning to camp.");
		doNext(camp.returnToCampUseOneHour);
	}
	//Failure to persuade/intimidate (SD 1.1f)
	else {
		Render.text("\n\nThe plant monster scowls at you when you attempt to leave.  You try to ");
		if(player.stats.cor < 50) Render.text("calm her down");
		else Render.text("remind her that attacking you will earn her a world of pain");
		Render.text(", but she isn't buying any of it.  You ");
		if(player.stats.cor < 50) Render.text("shrug and turn you back, beginning the long trek back towards camp");
		else Render.text("take a step forward and ready your weapon");
		Render.text("; she responds by attempting to whip you with one of her vine appendages, causing you to ");
		if(!player.isTaur()) Render.text("leap");
		else Render.text("gallop");
		Render.text(" out of the way.  She's misses, but it's obvious she will not take no for an answer. Looks like you have a fight on your hands...");
		//(leads to Fight 1.0]
		startCombat(monster);// TODO create PlantGirl
		doNext(1);
	}
...
public function fuckMyAssPlantLadyWhileImTrapped():void {
	...
	if(player.analCapacity() < 20) Render.text("  You moan again from the pain and clamp down, causing your captor to
shudder from the sensation of your [asshole]'s tightness.  Once the floral creature regains her composure, she begins to pump in and out of your ass at a gradually faster pace.");
	...
	if(player.shortestCockLength() < 48) {
		Render.text("\n\nYour ");
		if(player.lowerBody.balls > 0) Render.text("[balls] and ");
		Render.text(cockDescript(player.shortestCockIndex()) + " flap");
		if(player.lowerBody.balls > 0) Render.text("s");
		Render.text(" against your thighs as she fucks you harder and harder as her hand finds its way down to your prick");
		if(player.totalCock() > 1) Render.text("s");
		Render.text(".  The creature twists and jerks at the head of your cock and works diligently to stimulate your rock hard " + multiCockDescriptLight() + ", intent on making you moan out for her.");
	}
	//PC has a vagina: 
	...
	//(if long tongued: 
	if(player.tongueType > TONUGE.HUMAN) Render.text("  In a cute but spiteful way, your tongue is pulled forcefully out of your mouth and into her lips as she pulls her head back; the plant woman doesn't get too far from your face before you run out of tongue to give.  This suits her just fine as she takes to sucking on your incredibly long tongue in a manner similar to fellatio.  Pursed on the bottom of yours, she darts her head back and forth along the length of it, edging further into her mouth as she nears closer and closer to your own.  Her dominance without question, she reaches your lips and slips her tongue in, sealing it in with a firm kiss.");
	//Not long tongued: 
	else Render.text("  Your tongue is viciously attacked by the sudden intrusion of her own.  However, you are more than happy to return the favor as you counter attack, swapping each other's saliva in the process.");
	
	Render.text("The feminine creature's hand then finds its way ");
	if(player.lowerBody.balls == 0) Render.text("up");
	else Render.text("down");
	Render.text(" to your ");
	if(player.lowerBody.balls == 0) Render.text(nippleDescript(0));
	else Render.text("[balls]");
	Render.text(", massaging and caressing them gently as she moves to kissing at your neck.  Her thrusts become shorter in length as her breathing becomes frantic, signaling that her orgasmic peak is near. The plant woman ");
	//PC has hair: (grabs you by the hair)
	if(player.hairLength > 0) Render.text("grabs you by the hair");
	//PC doesn't have hair: embraces you)
	else Render.text("embraces you");
	Render.text(" as she cries out in a fierce moan, her cock convulsing as she thrusts wildly against you and planting her hot seed deep into you.  The sensual act finally over, your head droops down as you pant heavily; the plant girl slumps her head down onto your shoulder and uses you to hold herself up.  She's breathing heavily in your ear while regaining some of her energy and composure.");
...
public function trappedFlowerSexBlowjobRejection():void {
	Render.text("You push against the plant girl's thighs and shake your head, telling her that you are interested in THAT. You comment on how you wouldn't mind playing with something else though, as you begin to stroke your finger around the plant woman's labia.  She stops and ponders for a moment, and then giggles as her appendage disappears back into her body.");
...
public function trappedFlowerSexBlowjobAhoy():void {
	clearOutput();
	Render.text("\"<i>MHHMMpffffrmPH!</i>\ you mumble as her cock hits the back of your mouth, the plant creature clearly intent on jumping into the rougher sex sooner rather than later. The plant girl giggles as you struggle to breathe and not vomit from the violent intrusion. Placing both of her hands on the back of your head she begins to pump her hips, gliding her cock along your (tongue descript). You gasp for air as you try to push yourself off of the girl and get some room to breathe, but she has a pretty firm grip. She continues thrusting, but is increasingly annoyed that you aren't putting much effort into it right now.  A pair a vines ");
	//(If PC has balls: 
	if(player.lowerBody.balls > 0) Render.text("wrap themselves around your ([balls] and begin to squeeze");
	//(If PC has a cock(s): 
	else if(player.lowerBody.cockSpot.hasCock()) Render.text("wrap themselves around your " + multiCockDescriptLight() + " and begin to squeeze");
	//(If PC has a pussy: 
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("take hold of you clitDescript() and tug hard");
	//(If PC is genderless: 
	else Render.text("take hold of your [nipples], fiercely pulling on them to try and convince you to put some work into it");
	Render.text(".");
...
public function trappedPlantSexPussy():void {
	clearOutput();
	Render.text("Staring down at your " + multiCockDescriptLight() + ", the plant woman's eyes become fixated and betray a sense of longing, of curiosity, and eagerness.  Approaching your well restrained body, she takes [oneCock] into her soft hand and brushes it's sensitive head against her smooth pelvis, fairly close to the creature's moist and sweet smelling pussy.  Proceeding to wrap its arms around you, the plant woman takes [oneCock] in-between her legs and against her clit, signaling the creature's intent to plunge its hungry cunt down on your dick.");
...
public function trappedPlantSexPussyGOOO():void {
	...
	if(player.lowerBody.cockSpot.list[x].cockLength < 4) {
		...
		Render.text(" swelling up. The head");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" of your cock");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" swell");
		if(player.lowerBody.cockSpot.count() == 1) Render.text("s");
		Render.text(" to what seems to be double ");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("their ");
		else Render.text("its ");
		Render.text("original size, and your unsubstantial shaft");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s gain");
		else Render.text(" gain");
		Render.text(" some length in the process.  Is this some kind of allergic reaction?  A chemical?  Some kind of hormone? Regardless of how this is done, your once insignificant penis");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("es are now ");
		else Render.text(" is now ");
		Render.text("of sufficient length to penetrate this creature; though, you suspect the effects are temporary.");
	...
	else if(player.tallness < 72) Render.text("raises her face to yours ");
	//If height is greater than 72 inches, but less than 96 inches: 
	else if(player.tallness < 84) Render.text("head against your [chest] ");
	//(If Height is less than 96 inches: 
	else if(player.tallness < 96) Render.text("forehead against your chest, sniffing you intensely as she takes in your musk.  The creature of flora and warped flesh moans and shivers as she acquires your " + player.mf("manly","feminine") + " scent, and ");
	//(If height is greater than 96 inches: 
	else Render.text("places her head against your stomach, eyes closing as the creature takes in the rising and descent of your stomach ");
	Render.text(" as she slowly begins to work her hips, grinding the innards of her hot and moist sex against your " +  cockDescript(x) + ".");
	//(if one cock:
	if(player.lowerBody.cockSpot.count() == 1) Render.text("  The plant creature begins to speed up her thrusting against your cock, eager to drive you over the edge with the tightness of her hole.");
	else {
		//(If multi-cocked: 
	...
	//(Multi-cocked: 
	if(player.lowerBody.cockSpot.count() > 1) Render.text("  Buried deep in the makeshift vaginas, [eachCock] thrusts forward and eagerly deposits the nourishment the creature craves.");
	//Vagina: 
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Cumming hard, your " + clitDescript() + " releases a liberal stream of juices down the vine that fucks you, dissolving into the appendage as you moan in furious release.");
...
public function trappedPlantSexBlowMe():void {
	clearOutput();
	Render.text("The thought of getting a blowjob from this rather attractive... um... plant lady excites you greatly, springing your flaccid maleness from it's slumber.  Before the plant woman's very eyes, [eachCock] expands like heated metal; only in a much more dramatic and erotic fashion.");
	//(if cock(s) are 3-24 inches:)
	if(player.longestCockLength() < 24) {
		Render.text("  Now staring her straight in the eyes, your ");
		Render.text(multiCockDescriptLight() + " stare");
		if(player.lowerBody.cockSpot.count() == 1) Render.text("s");
		Render.text(" defiantly at the plant woman; erect in a such a fashion that suggests insult at the attractive creature before ");
		if(player.lowerBody.cockSpot.count() == 1) Render.text("it");
		else Render.text("them");
		Render.text(" that will be tending to them today.  Eager to rise to said challenge, the plant woman attempts to take your " + multiCockDescriptLight() + " into her mouth and ");
		
		//(if cock(s) do not exceed 6 inches in total diameter:)
		if(player.totalCockThickness() < 6) {
			Render.text(" manages to fit ");
			if(player.lowerBody.cockSpot.count() == 1) Render.text("it");
			else Render.text("them");
			Render.text(" into her mouth with little difficulty");
			if(player.lowerBody.cockSpot.count() > 1) Render.text(".  What a champ");
		}
		//if total cocks exceed 6 inches total diameter, but up to two cocks fall under 6 inches: 
			...
			Render.text(" manages to take two of your pricks into her mouth without dislocating a jaw or showing any obvious of discomfort");
		}
		else Render.text(" manages to swallow your dick into her mouth");
		Render.text(".");
		//(If cock count is more than two: 
		if(player.lowerBody.cockSpot.count() > 2) {
			Render.text("  The rest of your left out cocks ache for her touch; as she takes the two cocks she has further into her mouth, she raises her arms and attempts to take hold of your members, stroking them as best as she can as she works you with her mouth");
			//(cock(s) are 3-12 inches: 
			if(player.lowerBody.cockSpot.list[player.thinnestCockIndex()].cockLength < 12) Render.text(" against her face, while planting her hands against your thighs");
			//Cocks are 13-24 inches: 
			else Render.text(" in-between her shoulder and neck, using them both to form a tight hole for your other cocks to fuck");
			Render.text(".");
		}
		//		(jump to Continued... (“normal” dick sized oral) )
		//Continued... (“normal” dick sized sexing)
		Render.text("With your member");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" inside of her mouth, she begins the long awaited sucking and stimulation; starting out slowly at first, she slides the top of her tongue along the bottom of your prick");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(", going so gradual that you believe she is savoring the taste of you in her mouth.  To your surprise, she pulls back and lets your cock");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" pop out of her maw, smacking and licking her lips as she grunts her approval like someone who has just enjoyed a fine meal.  Shaking her now intrusive hair out of the way of her eyes, she darts her head forward and takes your ");
		if(player.lowerBody.cockSpot.count() >= 2) Render.text("two ");
		Render.text("saliva covered member");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" into her mouth; with little regard for her gag reflex, she raises her hands on your ");
		if(player.lowerBody.cockSpot.count() >= 4) Render.text("upper shafts");
		else if(player.lowerBody.cockSpot.count() >= 3) Render.text("upper shaft");
		else Render.text("[hips]");
		Render.text(" as she better positions herself for the entirety of your ");
		Render.text("cock");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s as they make");
		else Render.text(" as it makes");
		Render.text(" contact with the back of her throat.  You feel a bit of pre-cum leak out from the tip");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" of your ");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("two ");
		Render.text("dick");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" and ooze against the back of her throat, causing her to moan in delight as your sticky fluids coat her throat.");
		...
		//If Goo legs: 
			...
			if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
			Render.text(" as you assert a degree of control and influence on your pleasuring.  The sticky mess that is your body solicits a series of lustfully moans from the woman who tends to your loins with her mouth.");
		}
		Render.text("\n\nBobbing her head back and forth against you, she sucks and roughs up your cock");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" with her energetic efforts");
		if(player.lowerBody.cockSpot.count() == 1) Render.text(".");
		else {
			Render.text(", while she cheek fucks the remainder of your " + multiCockDescriptLight() + ".");
		}
		...
		if(player.lowerBody.balls > 0) {
			Render.text(" and the base of your shaft");
			if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		}
		Render.text(", she squeezes with considerable force, almost as if she is attempting to bottle up pressure in your ");
		if(player.lowerBody.balls > 0) Render.text("[sack]");
		else Render.text("loins");
		Render.text(" so that your creamy contents will explode forth into her craw.  Like a knotted up hose line, your cock");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" begin");
		if(player.lowerBody.cockSpot.count() == 1) Render.text("s");
		Render.text(" to ache due to the irritating discomfort in your shaft");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text("; you're unsure if you will be able to achieve orgasm with such a tense grip on your spear");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text("!  In a rhythmic fashion, the creature ascends your member");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" and promptly descends back downward, flooding your nerves with pleasure and a growing need to cum as she darts back and forth like a grounded raven chasing after a worm.");
		...
		Render.text(" tightens considerably;  with gusto, she starts stroking the base of your shaft like a squeeze tube, eager to aid your sperm in their journey to her stomach as they unknowingly rush to their certain doom.  Bleat red, her face compresses against your loins as she takes load after load into her throat, moaning like a wanton harlot and swallowing every once in a while to ensure that there will be room for more splooge in her mouth.  Just when you think she is going to injure you with her fierce grip, she lets you go; and not a moment too soon it seems, judging from the redness in your ");
		if(player.lowerBody.balls > 0) Render.text("scrotum's");
		else {
			if(player.lowerBody.cockSpot.count() == 1) Render.text("cock's");
			else Render.text("cocks'");
		}
		Render.text(" skin.  Content with your 'donation', the plant woman releases your cock");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" from her mouth with a loud pop; leaving trails of drool and semen on her chin as she does so.  The vines holding you retract from around your limbs and allows you to suit up and leave, whilst she cradles her sated stomach and hums to herself.");
		player.orgasm();
	}
	else {
		//(If all cocks exceed 24 inches, and two to three cocks measure 6 inches in diameter or less total: 
		Render.text("  Despite having to position herself a considerable distance away from your body, the plant woman manages to take [oneCock] and fits it into her mouth.  Due to the distance between you two, the plant woman has to ");
		if(player.lowerBody.cockSpot.count() == 1) Render.text("prop some of your length up on her average body to prevent it from straining towards the ground.");
		else Render.text("prop your other dicks up with her average body to prevent them from hitting the dirt covered ground.  Obviously she has plans for them, and doesn't want her prize catches getting dirty.");
		...
		if(player.cumQ() < 500) Render.text("covered");
		else Render.text("drenched");
		Render.text(" in your salty seed as she grins like a fool; obviously dumbfounded at her sheer dumb luck to find  a virile specimen such as yourself. The vines holding you retract, allowing your drained body to slowly fall to the ground. Panting heavily, you lie stomach first against the ground and collect your composure as the plant woman walks up to you and strokes your back and hair. She stays at your side until you muster the will and energy to pull yourself to your feet; at which point she helps you to your [armor] and helps you to put it back on. With a smile, she sees you off as you head back to camp.  “Strange that she didn't leave you and let something else have fun with me”, you wonder in your head as you leave the deep woods.");
		player.orgasm();
	}
	doNext(camp.returnToCampUseOneHour);
}
...
public function plantAnalGivingTheDick():void {
	clearOutput();
	Render.text("The sight of this nice, ample ass and the thought of fucking a supple  little pucker piques your " + multiCockDescriptLight() + ", causing ");
	if(player.lowerBody.cockSpot.count() == 1) Render.text("it");
	else Render.text("them");
	Render.text(" to become semi-erect.  As you reach her full ass, your prick");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s poke");
	else Render.text(" pokes");
	Render.text(" at her soft behind, signaling to her that now is the time guide you into her tight pucker.  Slowly, she edges your cock against the tight opening of her asshole.  Realizing that the asshole ");
	if(player.lowerBody.cockSpot.count() == 1) Render.text("it");
	else Render.text("they");
	Render.text(" will hopefully penetrate is a winner in the tightness department, your " + multiCockDescriptLight() + " surge");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" with vigor and anticipation.   The plant woman can barely contain her surprise as your dick");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" become");
	if(player.lowerBody.cockSpot.count() == 1) Render.text("s");
	Render.text(" rock hard and push");
	if(player.lowerBody.cockSpot.count() == 1) Render.text("es");
	Render.text(" against her butt hole, her hand shooting up her mouth to contain a giggle.  Looking back over her shoulder at you, she smirks as she pushes against your cock head");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(", easing ");
	if(player.lowerBody.cockSpot.count() == 1) Render.text("it");
	else Render.text("them");
	Render.text(" into her tight pucker.  With a sudden and rough slide forward your cock");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" satisfyingly penetrate");
	if(player.lowerBody.cockSpot.count() == 1) Render.text("s");
	Render.text(" her anus, leading to a slight gasp as the plant woman takes you inside of her.");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(", causing you to shudder in delicious pleasure as you moan loudly.");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" as she weakly bounces against you, squeezing down on you with just the right amount of force that it drives you past the point of no return, unleashing a ");
	//Cum output low: 
	if(player.cumQ() < 20) Render.text("small amount of semen");
	else if(player.cumQ() < 250) Render.text("generous amount of cum");
	else Render.text("flood of sticky spunk");
	Render.text(" from your cock");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" and into her tight asshole.");
...
public function failStruggle69():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text(multiCockDescriptLight());
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("[clit]");
	else Render.text("[asshole]");
	Render.text(" with her hands as she moves in for the 'kill'.");
...
public function nowayStruggle69Plant():void {
	clearOutput();
	Render.text("The dizzying sensation of being hung upside down like a hunter's catch in a tree is bad enough; the idea of this thing forcing her fuck hole against your face and licking your genitalia is more than enough for you to try and break free.  Thrashing around in the vine prison like a fly caught in a spider web, you try to show the plant woman that you are not okay with her choice in 'having fun' with you.  Your erratic and frenzied rejection dawns on the plant woman, and she pouts as she dejectedly flips you up right.  Her stare carries with it a plea for you to reconsider, but you ensure that you will not be swayed on this matter.  With a sad expression on her face, she stares down at the ground, lamenting your denial; raising an eyebrow, you call out the plant creature and assure her through your ");
	if(player.lowerBody.cockSpot.hasCock()) Render.text(multiCockDescriptLight);
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("[clit]");
	Render.text(" that you still want to fool around.  Conveying your preference, you gesture to her that she should... ");
	//(leads to trapped scene selection)
	doNext(9999);
}

//[69! pussy, service both, leads to TsI: 1.6a]
public function struggle69Plant(pt: number = 1):void {
	clearOutput();
	...
	if(pt == 2) {
		Render.text("Abandoning yourself to the plant woman's desires, you proceed to stretch your tongue and take her sweet cunt, allowing her soft sex to drag across it.  The soft tongue of your lover against your ");
		if(target == 2) Render.text("[clit]");
		else Render.text("[cock]");
		Render.text(" flushes your body with arousal as she teases you");
		
		//PC pussy chosen:
		if(target == 2) Render.text(", allowing her soft lips to surround your " + clitDescript() + ", 'trapping' it for her much anticipated assault.");
		//PC cock chosen: 
		else Render.text(".  Wasting no time with 'formalities,' she stuffs your " + cockDescript(0) + " into her mouth, firmly sealing her lips around your shaft and suckling along the sensitive flesh of your glans.");
	}
	//[69!, TsI: 1.6a, give her oral] 
	else if(pt == 1) {
		Render.text("Abandoning yourself to the plant woman's desires, you proceed to stretch your tongue and take her sweet, drooling cock along it, allowing her pre to seep along your mouth muscle and ignite your taste buds with her excitement. All too happy that you've chosen the rock hard prick between her hips, the plant woman is eager to reciprocate");
		//PC pussy chosen: 
		if(target == 2) Render.text(" as she allows her soft lips to surround your [clit], 'trapping' it for her much anticipated assault.");
		//PC cock chosen: 
		else Render.text(" as she wastes no time with 'formalities,' stuffing your " + cockDescript(0) + " into her mouth, firmly sealing her lips around your shaft and suckling along the sensitive flesh of your glans.");
	}
	//[69!, TsI: 1.6a, service both]
		...
		if(target == 2) Render.text(" as she allows her soft lips to surround your [clit], “trapping” it for her much anticipated assault.");
		//PC cock chosen: 
		else Render.text(" as she wastes no time with 'formalities,' stuffing your [cock] into her mouth, firmly sealing her lips around your shaft and suckling along the sensitive flesh of your glans.");
		Render.text("  Not to be an inconsiderate 'guest,' you decide to give a little love to her clit as well, soliciting a girlish little squeal of surprise for your lover.  Seems she lucked out this time with such an open-minded partner.");
	}
	...
	if(target == 2) {
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			...
			Render.text(" teasingly.  The pompous creature gives you a stare down at your form and grins at the “superior” position she thinks she has you in.");
		}
		...
		if(player.lowerBody.cockSpot.hasCock()) {
			Render.text("  With your feminine release comes a slightly painful build up of pressure in your loins; then, the hot and heavy eruption of cum in the air as your prick");
			if(player.lowerBody.cockSpot.count() > 1) Render.text("s explode");
			else Render.text(" explodes");
			Render.text(", coating the floral vixen in your seed.");
		}
	...
	//Cock(s): 
		...
		if(pt == 1) Render.text("cock");
		else if(pt == 2) Render.text("love button");
		else Render.text("love button and cock");
		Render.text(", trying your best to guide your tongue around her ");
		if(pt == 2) Render.text("clit and vagina");
		else if(pt == 1) Render.text("glans and shaft");
		else Render.text("clit, vagina, glans, and shaft");
		Render.text(" while you hang upside down.  No doubt loving every second of this, the plant woman starts gyrating and grinding her sex against your face, moaning and whimpering between mouthfuls of cock as you ");
		if(pt == 2) Render.text("probe her depths");
		else Render.text("do your best to service the stiff prick in your mouth");
		Render.text(".");
		
		//(PC is herm: 
		if(player.gender == 3) Render.text("  In reward for remembering your “duties,” a set of fingers find their way along your feminine sex and into your honey pot, her teasing of your [vagina] ensuring that no sexes on your body are needlessly left out.");
...
public function struggleFailPlantPussyFuckNoWay():void {
	clearOutput();
	Render.text("Whether it's because you ");
	//PC is virgin: 
	if(player.vaginas[0].virgin) Render.text("want to protect your chastity, or you ");
...
public function victoryVsPlant():void
{
	clearOutput();
	//(Victory, V 1.1L):
	Render.text("The plant woman sinks to her knees, clutching her ");
	//PC HP victory: 
	if(monster.HP < 1) Render.text("shoulder and sides.");
	else Render.text("breasts and pussy, twisting at her teats and finger herself like a woman possessed.");
...
public function defeatOrDomPlantAndCuntFuck():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		Render.text("  Your extra endowment");
		if(player.lowerBody.cockSpot.count() == 2) Render.text(" flaps");
		else Render.text("s flap");
		Render.text(" wildly against her thighs and ass, quietly acknowledging the call to arms and prepare to do their part.");
	}
	...
	Render.text(".  The sensation is too much, and you cry out as you flood her vagina with your hot semen.  The plant woman senses your orgasmic release and begins to squeeze your sides with her legs, as if pumping you for all of your seed.  Your ");
	if(player.lowerBody.balls == 0) Render.text(multiCockDescriptLight());
	else Render.text(ballsDescriptLight());
	Render.text(" acknowledge");
	if(player.lowerBody.balls == 0 && player.lowerBody.cockSpot.count() == 1) Render.text("s");
	Render.text(" her desire and continue to shoot thick streams of your seed into her");
	if(player.lowerBody.cockSpot.count() > 1) Render.text(" as well as on her stomach and the ground beneath you");
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("; your vaginaDescript(0) quivers with the need for release, letting loose with its own torrent of lady fluids as you moan in orgasm");
	Render.text(".  After a few moments, you slump down on the plant woman's chest, completely spent. A few minutes rest does you good, and with time you rise back up and look down at your partner; a dazed, blissful expression has fallen on her features, the creature lost in pure nirvana as she cradles her stomach. Satisfied with how things turned out, you give her one last kiss and struggle to get back up.  Damn, your ");
	if(player.lowerBody.balls > 0) Render.text("[balls] really hurt");
	else {
		Render.text("dick really hurts");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s really hurt");
	}
	Render.text(", but looking back at the heap of a woman behind you, you concur that it was totally worth it.");
	...
	if(player.HP < hpMax()) {
		Render.text("Upon returning to camp, you notice that some of the wounds the plant creature kissed have actually healed.  You're not sure how exactly this worked, but at least you don't have to put up with too many cut and bruises now.");
		//(HP restored +50).
		//9999 add HP
	}
	player.orgasm();
...
public function victoryDefeatPlantGirlGetBJMainSCREENON():void 
{
	...
	//PC has balls: (
	if(player.lowerBody.balls > 0) Render.text(" cusps your [balls] in her hand, and suck on the sides of your [sack]. She licks upwards from your testicles and caresses them before");
	else Render.text(" licks up from your gouch before"); 
	Render.text(" slowly proceeding towards your glans and swirling her tongue around it like a tootsie roll.  It isn't long before the plant woman suddenly stops and looks up at you with mischievous eyes.  She raises her eyebrows twice, and then ");
	...
	if(player.tentacleCocks() > 0 && (player.tentacleCocks() == player.totalCocks() || rand(2) == 0)) {
		...
		if(player.lowerBody.cockSpot.list[player.biggestCockIndex()].cockLength < 14) Render.text("takes the entirety of your [cock biggest] in her mouth and down her throat.  You lurch forward as your cock hits the back of her throat, moaning intensely at the plant girl's ability to take your whole penis in her mouth.");
		
		//[If cock size is greater than 14 inches]
		else Render.text("attempts to take as much of your member as she possibly can.  Your [cock biggest] slams against the back of her throat, and the plant woman tries to take a little more of you to no avail.  She looks up at you apologetically, but you pet her head and ensure that she knows you aren't upset with her.");
		
		Render.text("  She stares up at you with a flirtatious look and pulls your cock out of her throat, seeking to get a little more intimate with the head of your dick.  You grip on her hair tightly with every pleasurable sensation as her head bobs back and forth between your thighs.");
	...
	//______________________________________________________________________________
	//(if Non-goo: 
	if(!player.isGoo()) Render.text(".  She pulls your dick out of her mouth and closes her eyes as she gets ready to catch the rest of your load with her mouth.  Thick globs of semen erupt from your multiCockDescriptLight() and strike her in the face.  She herself orgasms as your torrent of semen washes over her head and chest, covering both liberally.");
	//(If goo: 
	else Render.text(", bucking hard against her face as you grunt furiously through gritted teeth and a fierce expression of release.  The plant woman inhales deeply just as you cum into her hot, moist mouth, producing a satisfied series of muffled groans as you deposit your seed.");
...
public function plantGirlPCWinGetTentacleBJ():void {
	...
	if(player.lowerBody.cockSpot.list[x].cockLength < 48) {
		...
		else Render.text(nippleDescript(0));
		Render.text(" and begins to squeeze tighter and tighter.  “Owww! Owww! Okay, Okay! I get it!” you cry out as you once again ram your " + cockDescript(x) + " down her throat.  The plant woman responds by massaging your ");
		if(player.lowerBody.balls > 0) Render.text("balls");
		else Render.text("nipples");
		Render.text(", showing that this is what she wants.  You laugh out loud at the ridiculousness of the situation: you're using a plant creature's throat as a makeshift cunt, and she doesn't seem to be running out of air anytime soon.");
	...
	//(V and D 1.2tc) conclusion: 
	Render.text("After a few minutes, you gather enough strength to rise to your feet and pull your flaccid cock from her mouth.  Content, she gives it one last lick before passing out into a deep sleep.  You smile and put your [armor] back on, eager to get back to camp.");
	player.orgasm();
	cleanupAfterCombat();
}
...
public function beatPlantGirlDomLetsFuckAnal():void {
	...
	if (inCombat) {
		...
		else Render.text("Your pelvis slaps");
		Render.text(" against her pussy, causing wet slapping sounds and coating your [sack] liberally with her sexual secretions.  The plant woman moans weakly");
		if(player.lowerBody.cockSpot.count() > 1) Render.text(" grabs a of your penis and guides it to her vagina.");
		else {
			Render.text(", while she ");
			//PC has testicles: (
			if(player.lowerBody.balls > 0) Render.text("fondles your testicles sensually; surprisingly she doesn't injure you by squeezing too hard despite the rough fucking.");
		}
		//Vagina check:
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			if(player.lowerBody.cockSpot.count() == 1 && player.lowerBody.balls > 0) Render.text("  Not content with just teasing your balls, the");
			else Render.text("  The");
			Render.text(" attractive creature darts her hand down to your [vagina], fingering you passionately as she takes the length of your meat into her hungry sex.");
		}
		...
		if(player.lowerBody.cockSpot.count() > 1) Render.text(" along with its vagina-clad brother");
		Render.text(".  She slumps against you, lost in pleasure and sensation as you drench her interior with hot, sticky cum.  The plant woman's nectar-flavored milk shoots forth from her breasts and coats the ground in front of the both of you, causing her to shake and moan furiously.");
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("Your [vagina] obliges, and a few steams of lady spunk flow from your [clit] and intertwine with the fauna creature's fluids on the ground.");
		Render.text("  As the plant woman comes down from her climax, you lick your hands clean of her nectar and pull out from her.  You shoot her a look of appreciation, and she fires back a look of utter contentment and bliss.  You leave her to recuperate from the act and head back to camp.");
		player.orgasm();
		cleanupAfterCombat();
	}
	//(Dominant Let's Fuck! (giving anal) D 1.3
	else {
		Render.text("Before the plant gets a chance to take two steps forward,you put your hand out towards her, signaling her to stop. Almost immediately the plant woman pauses her movement towards you, like a herbivore as it listens for the sounds of predators. Knuckles against your chin, you stare longingly at the creature's hips as she stands motionless, wondering why you wanted her to stop. Signaling with your index finger, you instruct the perplexed female to turn around. Reluctantly she spins around and exposes her ample rump to your gaze, causing a violent twitch in your pants as her full bottom comes into view. Satisfied, you proceed towards the plant woman, disrobing as you go.");
...
public function pcBeatsPlantgirlAndCorruptAssFuckPt2(tail:boolean = false):void {
	...
	if(!tail) Render.text("herself");
	else Render.text("her labia while you fuck her with your tail");
	Render.text(" and playing with her breasts as you pump her ass with all of your might.  You can no longer control the timing of your orgasm, and you howl in savage delight as you bottom out in this plant creature's sphincter.  She cries out so loud that you swear she almost ruptures your ear drums, slamming against you in a uncontainable thrashing orgasm.  You place your hands on the top of her thighs and push downward, intent on burying your cock");
	if(y >= 0) Render.text("s");
	Render.text(" deep in her ass as you fill her up with your tainted splooge.  She quivers violently; her body intent on getting some room but you will have none of it.  You keep her pinned against your lap until you have finished cumming and pull out of her, wiping yourself off with her hair as you proceed to replace your [armor].  The plant woman lies against the ground, now utterly dominated and unable to move.  As you walk away, you hear a rustling in the bush near her.  You let out a loud laugh as you picture an imp or tentacle monster emerging and proceeding to pick up where you left off.  As much as you would like to stick around and find out what fate lies in store for her, you have other things to do.  You turn and head back to camp.");
	player.orgasm();
	cleanupAfterCombat();
}
...
public function beatPlantGirlAndTakePlantCockInButt():void {
	...
	//(Dominant, D 1.5: 
	else Render.text("With careful coordination and care, you quickly and deftly sweep the plant woman off her feet and lower her to the ground on her back.  Her eyes go wide as you do so, and she tenses up; possibly because she thinks that you might be attacking her.  The soft descent to the ground changes her demeanor a bit, but her body language suggests that she is still wary.  With uncertainty, she stares up at you and awaits your next move.");
	//Scene leads to V and D 1.5)
	//(Victory and Dominant Scenes converge here, V and D 1.4): 
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("[eachCock] becomes rock solid");
	if(player.gender == 3) Render.text(" while ");
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("your pussy becomes incredibly wet");
	if(player.gender == 0) Render.text("nipples become erect");
	Render.text(" in response to your own arousal and desire.  The tip of her penis presses against your cheeks and pushes its way into you as you allow it past your anus and into the depths of your ass.  The creature opens her mouth in an 'ohh' expression, enjoying the sensation of breaking past your [asshole].  As she bottoms out, ");
	if(player.lowerBody.cockSpot.hasCock()) Render.text("[eachCock] jerks upward, the last shot of blood filling and hitting them like a thunderous punch");
	if(player.gender == 3) Render.text(", while ");
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("your pussy leaks a bit of girl cum.  It slowly drools out onto her pelvis");
	if(player.gender == 0) Render.text("your body quivers from the sensation");
	Render.text(".  ");
	//PC has balls: 
	if(player.lowerBody.balls > 0) Render.text("Your [balls] laying on top of her pelvis, y");
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("Your vaginal muscles clenching up along with your sphincter, y");
	else Render.text("Y");
 	Render.text("ou ");
	if(player.lowerBody.balls > 0) Render.text("begin to massage your [sack] as you raise your hips upward");
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("relish in the moment and allow a hand to drop to your sex as you raise your hips, eager to please both ends during this act");
	else Render.text("grab her nipples and begin to play with them");
	Render.text(".  Slowly, you lower your hips back down once again onto the plant woman's penile member, causing your partner to moan in lustful appreciation.  A sense of fullness floods you as you allow yourself to take her entire member inside of you.  You ");
	if(!player.lowerBody.vaginaSpot.hasVagina()) Render.text("place both hands on her shoulders");
	else Render.text("continue to finger yourself");
	Render.text(" as you bob your [asshole] against her lap, moaning loudly as you do.  The plant woman's hands shoot out and ");
	//PC has a pussy (and/or) a dick:
	if(player.gender > 0) {
		Render.text("grab both your [chest] and ");
		if(player.lowerBody.cockSpot.hasCock()) Render.text("[oneCock]");
		if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("your vulva");
	}
	//PC is genderless:
	else Render.text("your nipples, playfully tugging on them");
	Render.text(", intent on magnifying your own pleasure.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("strokes [oneCock] with her hands");
	if(player.gender == 3) Render.text(" as she also ");
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("massages your labia");
	if(player.gender == 0) Render.text("massages you");
	Render.text(".");
	Render.text("  The plant woman thrusts in kind,  mashing her pelvis against your own as you bounce off the creature. You manage to time your descent with her thrusts to intensify both of your pleasures.  She ");
	if(player.lowerBody.cockSpot.hasCock()) Render.text("quickens her pace of stroking your cock");
	if(player.gender == 3) Render.text(" and ");
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("fingers the interior of your pussy, causing you to roll your eyes into the back of your head as she sensually teases the inside of your sex");
	if(player.gender == 0) Render.text("raises herself to deliver a passionate kiss");
	Render.text(".  You feel her thrusts getting shorter now, and you know that her she going to blow her load soon.  With gradually closing eyes and increasingly frequent panting and moaning, she grabs your hips and holds them in place as she thrusts upward one last time.  With a lusty moan, her spunk shoots into your hot innards with significant force, allowing you to feel the impact of every shot of cum in your anus.");
	Render.text("\n\nYou cry out in fierce orgasm, ");
	if(player.lowerBody.cockSpot.hasCock()) {
		Render.text("[eachCock] erupting its own payload onto her face and chest");
	}
	if(player.gender == 3) Render.text(", ");
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("vaginal muscles clamping down and drooling pussy juice all over the plant woman's lap");
	if(player.gender == 0) Render.text("twitching futilely around the plant-girl's swollen seeder");
	Render.text(".  With a groan, you collapse against the creature's chest, desperately trying to catch your breath from the taxing ordeal.  The plant woman hums approvingly, stroking your hair as your body ascends and descends from her own because of your heavy breathing.  After some time, you are able to muster the strength to pull yourself off of the plant creature, her cock slipping out of your ass and unleashing a flood of sappy jism from your anus.  Content, she passes out into a deep sleep.  You grin to yourself, putting your [armor] back on and heading back to camp.");
	player.orgasm();
	if (inCombat)
		cleanupAfterCombat();
	doNext(camp.returnToCampUseOneHour);
...
public function defeatPlantgirlBlowAndButtfuckPure():void {
	//V 1.6 and D 1.6
	clearOutput();
	Render.text("With your hands on your hips you " + player.mf("walk","sway"));
	
	//(Victory, V 1.6:)
	if (inCombat)
		Render.text(" towards the battered creature as she struggles to get back to her feet.  Her efforts are quite for naught as her arms give out, causing her to hit the ground hard.");
	//(Scene leads to V and D 1.6)
	//(Dominant, D 1.6: 
	else Render.text(" towards the plant woman, causing her to giggle in response as she beckons you over to her.");
	//(Scene leads to V and D 1.6)
}
...
public function defeatPlantgirlBlowAndButtfuckPure():void {
	...
	if(player.isNaga()) Render.text("now exposed ");
	Render.text(clitDescript() + ".  The plant woman yelps in surprise as the vine makes contact with your labia, causing you to jump a little.  You turn to face the plant woman, who is blushing profusely and resting her hand on her pussy.  Looking at the vegetation, you deduce that she must be able to feel you along her vines and that these creepers may also function as penile appendages.  A grin spreads across your face as the realization hits you, and you bring the vine back to your sex and tease your vulva.  Soft moans escape the plant girl as the head of the perverted vegetation brushes past your clit, and you ");
	//(pure: 
	if(player.stats.cor < 50) Render.text("giggle");
	//Corrupted: 
	else Render.text("smirk");
	Render.text(" at the woman's predicament.  Deciding to stop teasing the poor girl, you take the head of the creeper and push it into your pussy, edging it upwards at a gradual pace so that your partner can feel the contours of your " + vaginaDescript(0) + ".");
...
public function vineAndCockWithPlantGirlPCWin(scene: number = 1):void {
