...
public function crazyVDayShenanigansByVenithil():void {
	...
	Render.text("Moving through the streets of Tel'Adre, you turn your steps towards the Wet Bitch, only to find that there's some odd event going on nearby.  Deciding the town is a safe enough place to let yourself be led astray by curiosity, you decide to investigate a bit closer.");
...
public function helpValentinesDay():void {
	clearOutput();
	Render.text("Deciding to try and improve the overall appreciation of romance and love in possibly the last pure city of Mareth, you smile to Scylla and tell her she can count on you.  Her happy expression makes you forget any doubts you might've had before.");
	menu();
...
public function leaveValentinesDayForever():void {
	clearOutput();
	Render.text("Ultimately, this is not worth your time.  You wish Scylla good luck, but tell her you won't help her make the holiday more popular in Tel'Adre, as you have other things to do.  Her expression turns sad and she nods in understanding as you turn away and go back into the streets of Tel'Adre.");
...
public function helpValentinesDayIII():void {
	clearOutput();
	Render.text("You spend the rest of the evening participating in holiday games and showing people around the stall, listening to Scylla giving a talk or two about love and the celebration of it.  All in all, the little popularization of the holiday turned out to be a moderate success, but a success after all thanks to the joint effort of the four of you.");
	...
	if(player.cockThatFits(28) >= 0 || player.lowerBody.vaginaSpot.hasVagina()) addButton(2,"Pastie",goVisitPastyVDay);
...
public function cuddleWithScyllaVDay():void {
	...
	//[if (hasVagina = true)]
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Your [vagina] is starting to get moist, making a small mess in your underclothes.");
	//[if (hasCock = true)]
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  You realize it'd probably be a done deal if you didn't resist the pull of your hardening cock towards one of Scylla's orifices, but you decide to spare her any temptations and endure yourself.");
	Render.text("  In time, your ignited lust slowly changes to constant arousal, and Scylla just sitting there, watching the sun with a beautiful smile on her face, clearly content to just have someone in her arms on this evening is helping you control yourself a lot better.");
...
public function makeOutWithScyllaVDay():void {
	...
	//[if (cocks = 1)
	if(player.lowerBody.cockSpot.count() == 1) Render.text("your [cock] is completely hard and stiff, forming a bulge in your clothing");
	else if(player.lowerBody.cockSpot.count() > 1) Render.text("[eachCock] is by now completely hard, eager to be touched and wanting to be caressed by the nun you're kissing");
	Render.text(".");
...
public function feedScyllaVDay():void {
	...
	Render.text("You feed Scylla what feels like a gallon of jizz!");
	player.orgasm();
...
public function goVisitAbby():void {
	clearOutput();
	Render.text("\"<i>You... you're kidding, right?  That fatty's words messing with yah head?  I always figured you were somewhat less... messed up in the head than Suck-a-Lot and Always-Drunk.  Was I wrong?</i>\"");
	...
	//[if (hasVagina = false)
	else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("\"<i>Well, that's unexpected. I have certain doubts about you showing me such a good time,</i>\" she says.  You tell her you don't actually require a cock at all to do this, and she seems surprised.");
...
public function fuckAbbyVDay():void {
	...
	Render.text("Figuring that the goblin girl's felt so out of place amid a celebration of love, you might as well show her a little affection the only way she's likely to understand.  Pulling aside your [armor] enough to let [eachCock] spring free, you step up behind the bent-over girl and rest your hands on her thick thighs.  The emerald girl's breath quickens as the heat of your manhood presses between the jiggling globes of her ass cheeks and a whimper escapes her throat before she realizes it.  Coughing, in an attempt to cover the moment of weakness, she turns her head to the side to give you a disapproving eye.  “What is this, romance?  Just jam it in, already!” she commands, gruffly.  Her face is flushed a deeper forest hue, pink irises twinkling in the dim light of her spartan home.");
...
public function pleasureAbbyVDay():void {
	...
	//([Naga tongue]
	if(player.tongueType == TONUGE.SNAKE) Render.text("  Your fingers make it into her pussy alongside your tongue, and the prehensile thing, longer and more flexible than a human's, ravages the insides of her pussy, tasting everywhere as Abby shakes in her orgasm.  Her pussy eagerly tries to milk both your fingers and your tongue, clenching and gripping at them as it drips her pussy juices all over your face.  Abby screams out in a loud, uninhibited orgasm at last, expressing her ecstasy.");
	//([Demon/Dragon Tongue]
	else if(player.tongueType == TONUGE.DEMONIC) Render.text("  Abylon shakes in excitement, pleasure, and fear as you extend your inhuman tongue to reach deep into the recesses of her pussy.  She cannot hold out a moment longer, her cunt gripping your inhuman muscle tightly, the hole and muscles inside quivering as you can taster her orgasmic juices with every single point of your tongue.  She screams, shudders and moans, tongue lolling out from the intense pleasure as you force your own tongue to roll around inside her and stretch her in multiple ways. Only you holding her stops Abylon from slumping forward, it seems...");
	//([Human tongue]
	else Render.text("  Hooking your fingers at the front and back of her cunny, you plunge your tongue in and wiggle it eagerly.  Abby clamps down on all three invaders and loudly moans out, screaming to her content at last, as your face is flooded with pussy juice.");
...
public function goVisitPastyVDay():void {
	clearOutput();
	Render.text("You lean over the table Pastie is no longer dancing, but rather laying, on, and, conspiratorially, ask her if she'd like to go and get something to drink.");
	...
	if(player.hasItem(consumables.BIMBOCH) || player.hasItem(consumables.BC_BEER) || player.hasItem(consumables.GOB_ALE) || player.lowerBody.vaginaSpot.hasVagina()) {
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) addButton(3,"Pussy",pastieValentineIntro,"vag");
	}
	else {
...
public function pastieValentineIntro(choice:string = ""):void {
	clearOutput();
	if(choice == "vag") {
		Render.text("You tell Pastie that indeed, she could get her fix, as you have something you're sure every fairy likes to drink not far away... between your legs.  \"<i>T-that's an... unusual drink proposition...  Umm, and I'm more susceptible to girls than most fairies, so... Ohhh, who am I kidding, that sounds pretty good right now.</i>\"");
	...
	//({Any other drink}. 
		...
		Render.text("You present the drink to Pastie and she flashes you a grin as she flies up and away, leading you into an alley.  \"<i>Well, lemme at it!  A drink sure as hell sounds good right now and none of this seems like it'd be really bad.</i>\"");
		menu();
...
public function valentineDrinkPastie(itype:ItemType):void {
	clearOutput();
	Render.text("One you're away from any prying eyes, Pastie turns to you and rubs her little fairy hands together, apparently barely able to hold her enthusiasm to finally get a bit drunk.  You're afraid it may not end at one drink, either, and for a moment, wonder about whether this is all right or not...");
	//Corruption 0-20] 
	if(player.stats.cor < 33) Render.text("  It probably isn't.");
	//[Corruption 21-75]
	else if(player.stats.cor < 75) Render.text("  You guess as long as she's fine afterwards, it should be okay.");
	//[Corruption 76-100]
	else Render.text("  As long as she's in the mood to drink more than just the alcohol afterwards, you don't really care.");
...
public function valentineDayGetPastieDrink(itype:ItemType):void {
	...
	if(player.lowerBody.cockSpot.hasCock()) addButton(1,"RubOnDick",rubPastieOnYourWangDawg);
	if(player.lowerBody.vaginaSpot.hasVagina()) addButton(2,"Go In Pussy",goForAPushayDivePasty);
}
...
public function fuckPastieForVDay():void {
	...
	Render.text("With a semi-sadistic grin, you watch Pastie saunter towards you, clearly allured by the scent of your genitals.  You stroke your " + cockDescript(x) + ", quickly bringing it to an erection, imagining how tight even this little thing will feel inside Pastie's small, but now unnaturaly stretchable body.");
...
public function rubPastieOnYourWangDawg():void {
	...
	if(player.lowerBody.balls > 0) Render.text("  Your [balls] lurch, " + player.skin() + " contracting as liquid passion rises within you.");
	Render.text("  Your [ass] clenches in anticipation, heralding your climax, so you slide the fairy up your shaft, pulling free from the grip of her arms and legs, holding the confused girl directly in front of your " + player.multiCockDescriptLight() + " while loosing your grasp to bear her entire, curvaceous form to the open air.  With a shuddering gasp, you erupt your intoxicating cream directly into the fey girl, your orgasm");
	//low cum production:
	if(player.cumQ() < 250) Render.text(" easily drenching the tiny fairy, in plump pearls up and down her naked flesh.");
	else if(player.cumQ() < 750) Render.text(" leaving her and the arm you're holding her in dripping with a sticky alabaster lacquer.");
	//high cum production:
	else Render.text(" almost blasting her right out of your hand in the torrent of white that spills forth from your mighty member.");
	Render.text("  The spunk-plastered pixie shudders in your palm, overwhelmed by the spoo drenching her.  She masturbates, feverishly, climaxing in seconds and then again a moment later.  You watch her for a few minutes, the batter-basted bimbo jilling herself silly from the heightened sensitivity your spoo shower has given her.");
...
public function goForAPushayDivePasty():void {
	...
	//[if (hascock = true)
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  All the while, you continue to jerk off [eachCock], preparing yourself for an orgasm on both sides.");
	Render.text("  She finally takes a full dive inside, and you moan out in pleasure, feeling her little body roll and explore in there, her tongue tasting every bit of fluid she can from what's moisturizing your inner passage.");
	...
	//[if (hascock = true)]
	if(player.lowerBody.cockSpot.hasCock()) Render.text("  At the same time, your cock deposits its own load onto the ground as your hermaphrodite genitals achieve a simultaneous orgasm.");
	//[if (isSquirter = true)
	if(player.wetness() >= 4) Render.text("  She doesn't have much to say in the matter when your female ejaculation truly shoots in, though, sending her rolling out of your [vagina] and onto the ground.");
