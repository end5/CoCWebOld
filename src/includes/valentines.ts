...
public function cuddleWithScyllaVDay():void {
	...
	//[if (hasVagina = true)]
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("  Your [vagina] is starting to get moist, making a small mess in your underclothes.");
	//[if (hasCock = true)]
	if(player.lowerBody.cockSpot.hasCock()) outputText("  You realize it'd probably be a done deal if you didn't resist the pull of your hardening cock towards one of Scylla's orifices, but you decide to spare her any temptations and endure yourself.");
...
public function makeOutWithScyllaVDay():void {
	...
	//[if (cocks = 1)
	if(player.lowerBody.cockSpot.count() == 1) outputText("your [cock] is completely hard and stiff, forming a bulge in your clothing");
	else if(player.lowerBody.cockSpot.count() > 1) outputText("[eachCock] is by now completely hard, eager to be touched and wanting to be caressed by the nun you're kissing");
	outputText(".");
...
public function goVisitAbby():void {
	...
	//[if (hasVagina = false)
	else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("\"<i>Well, that's unexpected. I have certain doubts about you showing me such a good time,</i>\" she says.  You tell her you don't actually require a cock at all to do this, and she seems surprised.");
...
public function pleasureAbbyVDay():void {
	...
	//([Naga tongue]
	if(player.tongueType == TONUGE.SNAKE) outputText("  Your fingers make it into her pussy alongside your tongue, and the prehensile thing, longer and more flexible than a human's, ravages the insides of her pussy, tasting everywhere as Abby shakes in her orgasm.  Her pussy eagerly tries to milk both your fingers and your tongue, clenching and gripping at them as it drips her pussy juices all over your face.  Abby screams out in a loud, uninhibited orgasm at last, expressing her ecstasy.");
	//([Demon/Dragon Tongue]
...
public function valentineDrinkPastie(itype:ItemType):void {
	...
	//Corruption 0-20] 
	if(player.stats.cor < 33) outputText("  It probably isn't.");
	//[Corruption 21-75]
	else if(player.stats.cor < 75) outputText("  You guess as long as she's fine afterwards, it should be okay.");
	//[Corruption 76-100]
...
public function rubPastieOnYourWangDawg():void {
	...
	if(player.lowerBody.balls > 0) outputText("  Your [balls] lurch, " + player.skin() + " contracting as liquid passion rises within you.");
...
public function goForAPushayDivePasty():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("your other hand moving downwards to tug on [oneCock], ");
	...
	//[if (hascock = true)
	if(player.lowerBody.cockSpot.hasCock()) outputText("  All the while, you continue to jerk off [eachCock], preparing yourself for an orgasm on both sides.");
	...
	//[if (hascock = true)]
	if(player.lowerBody.cockSpot.hasCock()) outputText("  At the same time, your cock deposits its own load onto the ground as your hermaphrodite genitals achieve a simultaneous orgasm.");
	//[if (isSquirter = true)
