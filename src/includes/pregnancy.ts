...
public updatePregnancy():boolean {
	...
	if(player.pregnancyIncubation == 1) {
		...
		if(player.findStatusAffect(StatusAffects.Birthed) < 0) player.statusAffects.add(new StatusAffect("Birthed",1,0,0,0)));
		else {
	...
	if (player.pregnancyIncubation > 1) {
		...
		if (player.pregnancyType == PregnancyType.COTTON) {			
			...
			if (player.pregnancyIncubation == 32 || player.pregnancyIncubation == 64 || player.pregnancyIncubation == 85 || player.pregnancyIncubation == 150) {
				displayedUpdate = true;
				//Increase lactation!
				if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3 && player.mostBreastsPerRow() > 1 && player.biggestLactation() >= 1 && player.biggestLactation() < 2) {
				...
				if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3 && player.mostBreastsPerRow() > 1 && player.biggestLactation() > 0 && player.biggestLactation() < 1) {
					Render.text("\nDrops of breastmilk escape your nipples as your body prepares for the coming birth.\n", false);
					player.boostLactation(.5);
				}				
				//Lactate if large && not lactating
				if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3 && player.mostBreastsPerRow() > 1 && player.biggestLactation() == 0) {
		...
		if (player.pregnancyType == PregnancyType.IMP) {			
			...
			if(player.pregnancyIncubation == 216) {
				...
				if(player.stats.cor < 40) Render.text("You are distressed by your unwanted pregnancy, and your inability to force this thing out of you.</b>", false);
				if(player.stats.cor >= 40 && player.stats.cor < 75) Render.text("Considering the size of the creatures you've fucked, you hope it doesn't hurt when it comes out.</b>", false);
				if(player.stats.cor >= 75) Render.text("You think dreamily about the monstrous cocks that have recently been fucking you, and hope that your offspring inherit such a pleasure tool.</b>", false);
			...
			if(player.pregnancyIncubation == 72) {
				...
				if(player.stats.cor < 40) Render.text("making it difficult to function.</b>", false);
				if(player.stats.cor >= 40 && player.stats.cor < 75) Render.text("and you wonder how much longer you have to wait.</b>", false);
				if(player.stats.cor >= 75) Render.text("and you're eager to give birth, so you can get impregnated again by corrupted or monstrous cum filling out your eager womb.</b>", false);
				Render.text("\n", false);
			...
			if(player.pregnancyIncubation == 48) {
				...
				if(player.stats.cor < 40) Render.text("Afterwards you feel somewhat disgusted with yourself.</b>\n", false);
				if(player.stats.cor >= 40 && player.stats.cor < 75) Render.text("You estimate you'll give birth in the next few days.</b>\n", false);
				if(player.stats.cor >= 75) Render.text("You find yourself daydreaming about birthing demons repeatedly, each time being re-impregnated by your hordes of lusty adolescent children.</b>\n", false);
				displayedUpdate = true;
			}
		}
		//Minotaur Pregnancy!
		if (player.pregnancyType == PregnancyType.MINOTAUR) {			
			...
			if(player.pregnancyIncubation == 72) {
				...
				if(player.stats.cor < 40) Render.text("making it difficult to function.</b>", false);
				if(player.stats.cor >= 40 && player.stats.cor < 75) Render.text("and you wonder how much longer you have to wait.</b>", false);
				if(player.stats.cor >= 75) Render.text("and you're eager to give birth, so you can get impregnated again by monstrous cocks unloading their corrupted seed directly into your eager womb.</b>", false);
				Render.text("\n", false);
				dynStats("spe", -3, "lib", 1, "sen", 1, "lus", 4);
				displayedUpdate = true;
			}
			if(player.pregnancyIncubation == 48) {
				...
				if(player.stats.cor < 40) Render.text("Afterwards you feel somewhat disgusted with yourself.</b>\n", false);
				if(player.stats.cor >= 40 && player.stats.cor < 75) Render.text("You estimate you'll give birth in the next few days.</b>\n", false);
				if(player.stats.cor >= 75) Render.text("You find yourself daydreaming about birthing some huge monstrous beast, and raising it to fuck your wet pussy over and over.</b>\n", false);
			}
			if(player.pregnancyIncubation == 32 || player.pregnancyIncubation == 64 || player.pregnancyIncubation == 85 || player.pregnancyIncubation == 150) {
				displayedUpdate = true;
				//Increase lactation!
		...
		if (player.pregnancyType == PregnancyType.BUNNY) {
			...
			if(player.pregnancyIncubation == 156) {
				Render.text("\n<b>The unmistakable bulge of pregnancy is visible in your tummy.  ", false);
				if(player.stats.cor < 40) Render.text("You are distressed by your unwanted pregnancy, and your inability to force this thing out of you.</b>", false);
				if(player.stats.cor >= 40 && player.stats.cor < 75) Render.text("You find yourself wondering what giving birth to bunny-girls is like.</b>", false);
				if(player.stats.cor >= 75) Render.text("You dreamily wonder if you could find a bunny willing to put more than two eggs inside you at once.</b>", false);
				dynStats("spe", -1, "lib", 1, "sen", 1, "lus", 2);
				Render.text("\n", false);
				displayedUpdate = true;				
			}
			...
			if(player.pregnancyIncubation == 72) {
				...
				if(player.stats.cor < 40) Render.text("making it difficult to function.</b>", false);
				if(player.stats.cor >= 40 && player.stats.cor < 75) Render.text("and you wonder how much longer you have to wait.</b>", false);
				if(player.stats.cor >= 75) Render.text("and you're eager to give birth so you'll be able to get pregnant again.</b>", false);
				Render.text("\n", false);
				dynStats("spe", -3, "lib", 1, "sen", 1, "lus", 4);
				displayedUpdate = true;
			}
			if(player.pregnancyIncubation == 48) {
				...
				if(player.stats.cor < 40) Render.text("Afterwards you feel somewhat disgusted with yourself.</b>\n", false);
				if(player.stats.cor >= 40 && player.stats.cor < 75) Render.text("You estimate you'll give birth in the next few days.</b>\n", false);
				if(player.stats.cor >= 75) Render.text("You find yourself daydreaming about birthing bunnies repeatedly, each time being re-impregnated with dozens of eggs from your lusty adolescent children.</b>\n", false);
				displayedUpdate = true;
			}
		}
		//Marblz Pregnancy!
		if (player.pregnancyType == PregnancyType.MARBLE) {			
			...
			if(player.pregnancyIncubation == 72) {
				...
				if(player.stats.cor < 40) Render.text("making it difficult to function.</b>", false);
				if(player.stats.cor >= 40 && player.stats.cor < 75) Render.text("and you wonder how much longer you have to wait.</b>", false);
				if(player.stats.cor >= 75) Render.text("and you're eager to give birth, so you can get fill your womb with a new charge.</b>", false);
				Render.text("\n", false);
				dynStats("spe", -3, "lib", 1, "sen", 1, "lus", 4);
				displayedUpdate = true;
			}
			...
			if(player.pregnancyIncubation == 85) {
				...
				if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 0) {
					Render.text("your chest has begun to feel a bit odd.  Your run your hands over it to find that your breasts have grown to around C-cups at some point when you weren't paying attention!  ", false);
					player.upperBody.chest.list[0].breastRating = 3;
				}
				else if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 1) {
					Render.text("your breasts feel oddly tight in your top.  You put a hand to them and are startled when you find that they've grown to C-cups!  ", false);
					player.upperBody.chest.list[0].breastRating = 3;
				}
				else if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 10) {
					Render.text("your breasts feel oddly full.  You grab them with your hands, and after a moment you're able to determine that they've grown about a cup in size.  ", false);
					player.upperBody.chest.list[0].breastRating++;
				}
				else {
					Render.text("your breasts feel a bit odd.  You put a hand on your chest and start touching them.  ", false);
				}
				if(player.biggestLactation() < 1) {
					Render.text("You gasp slightly in surprise and realize that you've started lactating.", false);
					player.boostLactation(player.upperBody.chest.count());
				} 
				else {
					Render.text("A few drips of milk spill out of your breasts, as expected.  Though, it occurs to you that there is more milk coming out than before.", false);
		...
		if (player.pregnancyType == PregnancyType.MOUSE || player.pregnancyType == PregnancyType.JOJO) {
			...
			if(player.pregnancyIncubation == 48) {
				...
				if(player.stats.cor < 40) Render.text("Afterwards you feel somewhat disgusted with yourself.</b>\n", false);
				if(player.stats.cor >= 40 && player.stats.cor < 75) Render.text("You estimate you'll give birth in the next few days.</b>\n", false);
				if(player.stats.cor >= 75) Render.text("You find yourself daydreaming about birthing hundreds of little babies, and lounging around while they nurse non-stop on your increasingly sensitive breasts.</b>\n", false);				
			}
			if(player.pregnancyIncubation == 32 || player.pregnancyIncubation == 64 || player.pregnancyIncubation == 85 || player.pregnancyIncubation == 150) {
				//Increase lactation!
				if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3 && player.mostBreastsPerRow() > 1 && player.biggestLactation() >= 1 && player.biggestLactation() < 2) {
		...
		if (player.pregnancyType == PregnancyType.AMILY) {
			...
			if(player.pregnancyIncubation == 216) {
				...
				if(flags[FlagEnum.AMILY_FOLLOWER] == 1) Render.text("  Amily smiles at you reassuringly. \"<i>We do have litters, dear, this is normal.</i>\"", false);
				Render.text("</b>", false);
				Render.text("\n", false);
				dynStats("spe", -1, "lib", 1, "sen", 1, "lus", 2);
				displayedUpdate = true;				
		...
		if (player.pregnancyType == PregnancyType.SPIDER || player.pregnancyType == PregnancyType.DRIDER_EGGS) {	
			...
			if(player.pregnancyIncubation == 250) {
				...
				if(player.thickness > 60 && player.tone < 40) Render.text(" even for you", false);
				Render.text(", maybe you should cut back on all the food you've been consuming lately?</b>\n", false);
				displayedUpdate = true;	
			}
			if(player.pregnancyIncubation == 216) {
			...
			if(player.pregnancyIncubation == 72) {
				...
				if(player.stats.cor < 50) Render.text("  You shudder and shake your head, wondering why you're thinking such unusual things.", false);
				Render.text("</b>\n", false);
				displayedUpdate = true;
			}
			if(player.pregnancyIncubation == 32 || player.pregnancyIncubation == 64 || player.pregnancyIncubation == 85 || player.pregnancyIncubation == 150) {
		...
		if (player.pregnancyType == PregnancyType.EMBER) {
			...
			if(flags[FlagEnum.EMBER_OVIPOSITION] > 0) {
				...
				if(player.pregnancyIncubation == 30) {
					...
					dynStats("sen", 5, "lus", (5+player.stats.sens/20));
					//If Corruption < 40
			...
			//Pregnancy Notes: Live Birth 
				...
				if(player.pregnancyIncubation == 120) {
					...
					if(flags[FlagEnum.EMBER_ROUNDFACE] == 1) pregText += ", blush at " + emberScene.emberMF("his","her") + " own over-enthusiastic reply";
				...
				if(player.pregnancyIncubation == 90) {
					...
					if(flags[FlagEnum.EMBER_GENDER] == 1 && flags[FlagEnum.EMBER_MILK] > 0) pregText += "  You can't resist smirking and patting one of your shemale dragon's bountiful breasts, noting that maybe you should let him do all the breast-feeding.";
					
					pregText += "\n\n";
					if(flags[FlagEnum.EMBER_ROUNDFACE] > 0) pregText += "Ember blushes.  ";
		...
		if (player.pregnancyType == PregnancyType.ANEMONE) {			
			...
			if(player.pregnancyIncubation == 185) {
				Render.text("\n<b>The unmistakable bulge of pregnancy is visible in your tummy.  ", false);
				if(player.stats.cor < 40) Render.text("You are distressed by your unwanted pregnancy, and your inability to force this thing out of you.</b>", false);
				if(player.stats.cor >= 40 && player.stats.cor < 75) Render.text("Considering the possible fathers, you hope it isn't that big.</b>", false);
				if(player.stats.cor >= 75) Render.text("You think dreamily about the cocks that have recently been fucking you, and hope that your offspring inherit such a divine pleasure tool.</b>", false);
				dynStats("spe", -1, "lib", 1, "sen", 1, "lus", 2);
				Render.text("\n", false);
				displayedUpdate = true;				
			}
			...
			if(player.pregnancyIncubation == 120) {
				...
				if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and keeps your " + vaginaDescript(0) + " aroused from the constant tingling in your womb", false);
				Render.text(".</b>\n", false);
			...
			if(player.pregnancyIncubation == 72) {
				...
				if(player.stats.cor < 40) Render.text("and constantly shifts and wriggles.  What manner of beast are you bringing into the world?</b>", false);
				if(player.stats.cor >= 40 && player.stats.cor < 75) Render.text("and you wonder how much longer you have to wait.</b>", false);
				if(player.stats.cor >= 75) Render.text("and you're eager to give birth, so you can get impregnated again by corrupted or monstrous cum filling out your eager womb.</b>", false);
				Render.text("\n", false);
				dynStats("spe", -3, "lib", 1, "sen", 1, "lus", (5 + player.stats.lib / 20));
				displayedUpdate = true;
			}
			if(player.pregnancyIncubation == 48) {
				Render.text("\n<b>You rub your hands over your bulging belly, lost in the sensations of motherhood.  ", false);
				if(player.stats.cor < 40) Render.text("Afterwards you feel somewhat disgusted with yourself, but horny.</b>\n", false);
				if(player.stats.cor >= 40 && player.stats.cor < 75) Render.text("You estimate you'll give birth in the next few days.  You hope the birth is as erotically charged as the pregnancy has been.</b>\n", false);
				if(player.stats.cor >= 75) Render.text("You find yourself daydreaming  about birthing cilia-covered worms, orgasming each time their thousands of stingers brush by your clit and fill it full of sensation-enhancing drugs.</b>\n", false);
				dynStats("spe", -1, "lib", 1, "sen", 1, "lus", (10+player.stats.lib/20));
				displayedUpdate = true;
			}
		}
		//Hellhound Pregnancy!
		...
		//Frog Eggs
			...
			if(player.pregnancyIncubation == 8) {
				...
				if(player.lowerBody.vaginaSpot.hasVagina()) {
					...
					//pussy:
					if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Against your [vagina], the slime feels warm and cold at the same time, coaxing delightful tremors from your [clit].");
					//[balls:
					else if(player.lowerBody.balls > 0) Render.text("  Slathered in hallucinogenic frog slime, your balls tingle, sending warm pulses of pleasure all the way up into your brain.");
					//genderless: 
					else Render.text("  Your [vagina] begins twitching, aching for something to push through it over and over again.");
					Render.text("  Seated in your own slime, you moan softly, unable to keep your hands off yourself.");
	...
	if(player.buttPregnancyIncubation > 1) {
		if (player.buttPregnancyType == PregnancyType.FROG_GIRL) {
			if(player.buttPregnancyIncubation == 8) {
				...
				//[cock:
				else if(player.lowerBody.cockSpot.hasCock()) Render.text("  Splashing against the underside of your " + player.multiCockDescriptLight() + ", the slime leaves a warm, oozy sensation that makes you just want to rub [eachCock] over and over and over again.");
				//genderless: 
				else Render.text("  Your asshole begins twitching, aching for something to push through it over and over again.");
				Render.text("  Seated in your own slime, you moan softly, unable to keep your hands off yourself.");
				dynStats("lus=", 100, "resisted", false);
				displayedUpdate = true;
			}
		...
		if (player.buttPregnancyType == PregnancyType.BEE_EGGS) {
			...
			if(player.buttPregnancyIncubation == 20) {
				...
				if(player.lowerBody.cockSpot.count() > 0) Render.text("The aroma seems to permeate your very being, slowly congregating in your ", false);
				if(player.lowerBody.cockSpot.count() == 1) {
					Render.text(cockDescript(0), false);
					if(player.horseCocks() == 1) Render.text(", each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air, until your " + cockDescript(0) + " is twitching and dripping, the flare swollen and purple.  ", false);
					if(player.dogCocks() == 1) Render.text(", each inhalation making it thicker, harder, and firmer.  You suck in huge lungfuls of air, desperate for more, until your " + cockDescript(0) + " is twitching and dripping, its knot swollen to the max.  ", false);
					if(player.normalCocks() == 1) Render.text(", each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air, until your " + cockDescript(0) + " is twitching and dripping, the head swollen and purple.  ", false);
					//FAILSAFE FOR NEW COCKS
					if(player.normalCocks() == 0 && player.dogCocks() == 0 && player.horseCocks() == 0) Render.text(", each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air until your " + cockDescript(0) + " is twitching and dripping.  ", false);
				}
				if(player.lowerBody.cockSpot.count() > 1) Render.text("groin.  Your " + player.multiCockDescriptLight() + " fill and grow with every lungful of the stuff you breathe in.  You suck in great lungfuls of the tainted air, desperate for more, your cocks twitching and dripping with need.  ", false);
				Render.text("You smile knowing you couldn't stop from masturbating if you wanted to.\n", false);
		...
		if (player.buttPregnancyType == PregnancyType.SANDTRAP || player.buttPregnancyType == PregnancyType.SANDTRAP_FERTILE) {
			if(player.buttPregnancyIncubation == 36) {
				...
				if(player.buttPregnancyType == PregnancyType.SANDTRAP_FERTILE) Render.text(" bloated and full; the sensation isn't entirely unpleasant.");
				else {
					Render.text("increasingly empty, as though some obstructions inside you were being broken down.");
	...
	if (player.buttPregnancyIncubation == 1 && player.buttPregnancyType == PregnancyType.BEE_EGGS) {
		Render.text("\n", false);
		Render.text("There is a sudden gush of honey-colored fluids from your ass.  Before panic can set in, a wonderful scent overtakes you, making everything ok.  ", false);
		if(player.lowerBody.cockSpot.count() > 0) Render.text("The muzzy feeling that fills your head seems to seep downwards, making your equipment hard and tight.  ", false);
		if(player.lowerBody.vaginaSpot.count() > 0) Render.text("Your " + vaginaDescript(0) + " becomes engorged and sensitive.  ", false);
		Render.text("Your hand darts down to the amber, scooping up a handful of the sticky stuff.  You wonder what your hand is doing as it brings it up to your mouth, which instinctively opens.  You shudder in revulsion as you swallow the sweet-tasting stuff, your mind briefly wondering why it would do that.  The stuff seems to radiate warmth, quickly pushing those nagging thoughts away as you scoop up more.\n\n", false);
		Render.text("A sudden slip from below surprises you; a white sphere escapes from your anus along with another squirt of honey.  Your drugged brain tries to understand what's happening, but it gives up, your hands idly slathering honey over your loins.  The next orb pops out moments later, forcing a startled moan from your mouth.  That felt GOOD.  You begin masturbating to the thought of laying more eggs... yes, that's what those are.  You nearly cum as egg number three squeezes out.  ", false);
		if(player.averageLactation() >= 1 && player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) Render.text("Seeking even greater sensation, your hands gather the honey and massage it into your " + breastDescript(0) + ", slowly working up to your nipples.  Milk immediately begins pouring out from the attention, flooding your chest with warmth.  ", false);
		Render.text("Each egg seems to come out closer on the heels of the one before, and each time your conscious mind loses more of its ability to do anything but masturbate and wallow in honey.\n\n", false);
		Render.text("Some time later, your mind begins to return, brought to wakefulness by an incredibly loud buzzing...  You sit up and see a pile of dozens of eggs resting in a puddle of sticky honey.  Most are empty, but a few have hundreds of honey-bees emptying from them, joining the massive swarms above you.  ", false);
		if(player.stats.cor < 35) Render.text("You are disgusted, but glad you were not stung during the ordeal.  You stagger away and find a brook to wash out your mouth with.", false);
		if(player.stats.cor >= 35 && player.stats.cor < 65) Render.text("You are amazed you could lay so many eggs, and while the act was strange there was something definitely arousing about it.", false);
		if(player.stats.cor >= 65 && player.stats.cor < 90) Render.text("You stretch languidly, noting that most of the drugged honey is gone.  Maybe you can find the Bee again and remember to bottle it next time.", false);
		if(player.stats.cor >= 90) Render.text("You lick your lips, savoring the honeyed residue on them as you admire your thousands of children.  If only every night could be like this...\n", false);
		player.buttKnockUpForce(); //Clear Butt Pregnancy
		player.orgasm();
		dynStats("int", 1, "lib", 4, "sen", 3);
		if(player.buttChange(20, true)) Render.text("\n", false);
		if(player.lowerBody.butt.buttRating < 17) {
			//Guaranteed increase up to level 10
			if(player.lowerBody.butt.buttRating < 13) {
				player.lowerBody.butt.buttRating++;
	...
	if (player.pregnancyType == PregnancyType.ANEMONE && player.pregnancyIncubation == 1) {
		...
		Render.text("Your " + player.armorName + " feels damp around the groin and you reach down to check the area.  The  " + vaginaDescript(0) + " you feel is dilated and slick with unusual wetness; your water must have broken!\n\n", false);
		
		Render.text("Hurriedly you strip off your gear and sit down with your back against a rock.  Focusing yourself, you attempt to prepare for labor; you try to remember your recent partners and worry about what kind of monstrous infant you might have to force out of your " + vaginaDescript(0) + ".  The first contraction comes and you push as hard as you can, to be rewarded with the feeling of something sliding out between your labia.  You attempt a few more pushes but nothing further seems forthcoming; curious, you look down at your crotch only to discover a blue stalk sticking proudly out of your vagina!\n\n", false);
		
		if(flags[FlagEnum.ANEMONE_KID] > 0) {
			Render.text("As you take in the sight, small nodules around the tip begin to form and lengthen, until the little anemone is capped by a mop of wriggling blue-green tentacles.  Horrified, you grasp it at the base and give it a sharp pull.  The pain makes you lock up and nearly takes away your consciousness as its sticky surface releases its grip on your labia and " + clitDescript() + "!   It writhes and slips out of your pain-wracked hands, leaving them tingling.  As you lie there, stunned, it begins to inch back toward your " + vaginaDescript(0)+ ".  Footfalls sound next to you, and a blue hand picks up the squirming, cilliated creature.  Kid A gives you a shy smile, then turns to her barrel.  A quick splash and a filled waterskin later, she heads toward the stream, toting your grub-like offspring.");
		...
		//[(if pc has 0-9 existing cocks)
			...
			Render.text("As you take in the sight, small nodules around the tip begin to form and lengthen, until the little anemone is capped by a mop of wriggling blue-green tentacles.  Horrified, you grasp it at the base and give it a sharp pull.  The pain makes you lock up and nearly takes away your consciousness as its sticky surface releases its grip on your labia and " + clitDescript() + "!  The small anemone and you both lay there twitching, but it recovers its bearings first; through your haze of pain you watch it flexing its body, wedging the head under itself, and elevating the base.", false);
			player.cuntChange(20,true,true,false);
			
			Render.text("\n\nBeset by a panic, you watch as the strange thing sets butt-end down on your pubic mound and adheres", false);
			//[(if cocks)
			if(player.lowerBody.cockSpot.count() > 0) Render.text(" below your " + player.multiCockDescriptLight(), false);
			Render.text(". A sharp pinch lances through the nerves in your groin and sends your hands to it reflexively.  This smaller pain, coupled with the adrenaline and dopamine that have finally chased the fog from your head, is enough to pull your thoughts into focus for another attempt to remove your strange, parasitic offspring.  You shift your grip and pull a few more times, but the thing doesn't budge.  The handling of it only serves to make the stalk thicken and become stiff; gradually you notice that you're feeling the sensation of your own pulling not from the skin at the point of attachment but from the stalk itself, and this realization is accompanied by the ring of tentacles opening and pulling back to reveal the crown of a penis!  <b>You have a new anemone-penis!</b>", false);
			//[(dick slot 1 exists)
			if(player.lowerBody.cockSpot.count() > 0) Render.text("  The tentacles writhe around, rubbing against your " + player.multiCockDescriptLight(), false);
			//(doesn't exist)
			else Render.text("  The tentacles curl inwards, rubbing on the head of your new blue pecker", false);
			player.createCock((4+rand(3)),1.2);
			player.lowerBody.cockSpot.list[player.lowerBody.cockSpot.count()-1].cockType = CockType.ANEMONE;
			Render.text(" and you quickly become fully erect from the aphrodisiac they inject.  Over and over the tentacles caress " + player.sMultiCockDesc() + " sensually, leaving behind a tingling trail of vibrant pleasure", false);
			//[(if no dick1 and no balls)
			if(player.totalCocks() == 1 && player.lowerBody.balls == 0) Render.text("; you feel a pressure build below the shaft, near your asshole", false);
			Render.text(".  As the venom and the rubbing work you to the edge of climax, your muscles clench and a ", false);
			if(player.cumQ() < 100) Render.text("glob", false);
			else if(player.cumQ() < 500) Render.text("squirt", false);
			else Render.text("spray", false);
			Render.text(" of semen shoots from your new penis and lands on your ", false);
			//[(if boobs)
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 1) Render.text(allBreastsDescript() + " and ", false);
			Render.text("stomach", false);
			//[(dick1 exists)
			if(player.lowerBody.cockSpot.count() > 1) Render.text(", followed in short order by white squirts from " + player.sMultiCockDesc() + " remaining", false);
			Render.text(".  Your " + vaginaDescript(0) + " quivers and pulses as well, adding ", false);
			if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLICK) Render.text("a trickle", false);
			else if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLAVERING) Render.text("a squirt", false);
			else Render.text("nearly a cupful of fluid", false);
			Render.text(" from your female orgasm to the puddle on the ground below your ass.\n\n", false);
		...
		//[(if PC has 10 existing cocks) && no kid
			...
			player.orgasm();
			dynStats("lib", 2, "sen", 5);
			if(player.findStatusAffect(StatusAffects.AnemoneArousal) < 0) player.statusAffects.add(new StatusAffect("AnemoneArousal",0,0,0,0)));
		}		
		player.knockUpForce(); //Clear Pregnancy
		Render.text("Exhausted by the 'birth' and the climax, you slip into a doze.\n", false);
	}
	...
	if (player.pregnancyIncubation == 1 && player.pregnancyType == PregnancyType.IMP) {
		...
		Render.text("A sudden gush of fluids erupts from your vagina - your water just broke.  You grunt painfully as you feel wriggling and squirming inside your belly, muscle contractions forcing it downwards.  ", false);
		if(player.stats.cor < 50) Render.text("You rue the day you encountered that hateful imp.  ", false);
		Render.text("The pain begins to subside as your delivery continues... replaced with a building sensation of pleasure.  Arousal spikes through you as the contractions intensify, and as you feel something pass you have a tiny orgasm.\n\nYet you feel more within you, and the contractions spike again, pushing you to orgasm as you pass something else.  It repeats, over and over, nearly a dozen times you birth and orgasm.  After an eternity of procreation and pleasure, you sense your ordeal is over and collapse, unconscious.", false);
		
		
		if(player.vaginas[0].vaginalLooseness == VAGINA_LOOSENESS.TIGHT) player.vaginas[0].vaginalLooseness++;
		//50% chance
		if(player.vaginas[0].vaginalLooseness < VAGINA_LOOSENESS.GAPING_WIDE && rand(2) == 0) {
		...
		if(player.averageLactation() > 0 && player.averageLactation() < 5) {
			Render.text("  Your breasts won't seem to stop dribbling milk, lactating more heavily than before.", false);
			player.boostLactation(.5);
		}
		//Lactate if large && not lactating
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 3 && player.mostBreastsPerRow() > 1 && player.averageLactation() == 0) {
			Render.text("  As you ponder the implications, <b>you realize your breasts have been slowly lactating</b>.  You wonder how much longer it will be before they stop.", false);
			player.boostLactation(1);
		}
		player.boostLactation(.01);
		//Enlarge if too small for lactation
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating == 2 && player.mostBreastsPerRow() > 1) {
			Render.text("  <b>Your breasts have grown to C-cups!</b>", false);
			player.growTits(1, 1, false, 3);
		}
		//Enlarge if really small!
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating == 1 && player.mostBreastsPerRow() > 1) {
			Render.text("  <b>Your breasts have grown to B-cups!</b>", false);
			player.growTits(1, 1, false, 3);
		}
		if(player.vaginas[0].vaginalWetness == VAGINA_WETNESS.DRY) player.vaginas[0].vaginalWetness++;
	...
	if (player.pregnancyIncubation == 1 && player.pregnancyType == PregnancyType.MARBLE) {
		...
		if(flags[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] < 100) {
			...
			Render.text("Marble seems to understand what happened, but is really disappointed with you, \"<i>Sweetie, why couldn't you wait until after I'd finished the nursery...?</i>\"", false);
			...
			//it's a boy!
			{
				...
				Render.text("The little boy is already starting to look like he is a few years old; he’s trotting around on his little hoofs.", false);
				//Increase the size of the PC’s hips, as per normal for pregnancies, increase birth counter
				if(player.lowerBody.hipRating < 10) {
					player.lowerBody.hipRating++;
					Render.text("After the birth your " + player.armorName + " fits a bit more snugly about your " + hipDescript() + ".", false);
				}
				if (flags[FlagEnum.MARBLE_BOYS] == 0)
				//has Marble had male kids before?
				{
					Render.text("You notice that Marble seems to be deep in thought, and you ask her what is wrong.  She starts after a moment and says, \"<i>Oh sweetie, no, it's nothing really.  I just never thought that I'd actually be able to father a son is all.  The thought never occurred to me.", false);
				}
				//Add to marble-kids:
				flags[FlagEnum.MARBLE_KIDS]++;
				flags[FlagEnum.MARBLE_BOYS]++; //increase the number of male kids with Marble
			}
			...
			//it's a girl!
			{
				...
				Render.text("You give an almighty heave and finally manage to push the shoulders out. The rest is downhill from there.  Once you've pushed the child completely out, Marble lays you down on the ground.  You rest there for a few moments, trying to catch your breath after the relatively difficult birthing.  When you finally have a chance to get up, you see that Marble has a small cowgirl cradled in her arms, suckling on her nipple.  You can hardly believe that you managed to push out a girl that big!  Marble seems to understand and tells you that the child is actually a fair bit bigger now than when she came out.\n\n", false);
				Render.text("She helps you stand up and gives you the little girl to suckle for yourself.  ", false);
				if(player.statusAffects.get("Marble").value4 >= 20) {
					Render.text("As the child contentedly drinks from your " + nippleDescript(0) + ", Marble tells you, \"<i>Sweetie, somehow I know that our kids won't have to worry about the addictive milk.  It will only make them healthy and strong.</i>\"  You nod at her and put the child into the nursery.  ", false);
				} 
				else {
					Render.text("You put the child to your breast and let her drink down your milk.  You sigh in contentment and Marble says, \"<i>See sweetie?  It's a really good feeling, isn't it?</i>\"  You can't help but nod in agreement.  After a minute the little girl has had enough and you put her into the nursery.\n\n", false);
				}
				Render.text("The little girl is already starting to look like she is a few years old; she's trotting around on her little hooves.", false);
				//Add to marble-kids:
				flags[FlagEnum.MARBLE_KIDS]++;
			}
			//Increase the size of the PC's hips, as per normal for pregnancies, increase birth counter
	...
	if (player.pregnancyIncubation == 1 && player.pregnancyType == PregnancyType.MINOTAUR) {
		...
		if(player.lowerBody.butt.buttRating < 12 && rand(2) == 0) {
			player.lowerBody.butt.buttRating++;
			Render.text("\n\nYou notice your " + buttDescript() + " feeling larger and plumper after the ordeal.", false);
		}
		...
		if(flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00328] == 0) flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00328] = 150;		
	}
	...
	if (player.pregnancyIncubation == 1 && player.pregnancyType == PregnancyType.AMILY) {
		...
		Render.text("\n", false);
		if(player.lowerBody.vaginaSpot.count() == 0) {
			Render.text("You feel a terrible pressure in your groin... then an incredible pain accompanied by the rending of flesh.  <b>You look down and behold a new vagina</b>.  ", false);
			player.createVagina();
			genderCheck();
		}
	...
	if (player.pregnancyType == PregnancyType.OVIELIXIR_EGGS && player.pregnancyIncubation > 0) {
		...
		if(player.pregnancyIncubation == 1) {
			...
			//Large egg scene
				...
				Render.text("A sudden shift in the weight of your pregnant belly staggers you, dropping you to your knees.  You realize something is about to be birthed, and you shed your " + player.armorName + " before it can be ruined by what's coming.  A contraction pushes violently through your midsection, ", false);
				if(player.vaginas[0].vaginalLooseness < VAGINA_LOOSENESS.LOOSE) Render.text("stretching your tight cunt painfully, the lips opening wide ", false);
				if(player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.LOOSE && player.vaginas[0].vaginalLooseness <= VAGINA_LOOSENESS.GAPING_WIDE) Render.text("temporarily stretching your cunt-lips wide-open ", false);
				if(player.vaginas[0].vaginalLooseness > VAGINA_LOOSENESS.GAPING_WIDE) Render.text("parting your already gaping lips wide ", false);
				Render.text("as something begins sliding down your passage.  A burst of green slime soaks the ground below as the birthing begins in earnest, and the rounded surface of a strangely colored egg peaks between your lips.  You push hard and the large egg pops free at last, making you sigh with relief as it drops into the pool of slime.  The experience definitely turns you on, and you feel your clit growing free of its hood as another big egg starts working its way down your birth canal, rubbing your sensitive vaginal walls pleasurably.   You pant and moan as the contractions stretch you tightly around the next, slowly forcing it out between your nether-lips.  The sound of a gasp startles you as it pops free, until you realize it was your own voice responding to the sudden pressure and pleasure.  Aroused beyond reasonable measure, you begin to masturbate ", false);
				if(player.lowerBody.vaginaSpot.list[0].clitLength > 5) Render.text("your massive cock-like clit, jacking it off with the slimy birthing fluids as lube.   It pulses and twitches in time with your heartbeats, its sensitive surface overloading your fragile mind with pleasure.  ", false);
				if(player.lowerBody.vaginaSpot.list[0].clitLength > 2 && player.lowerBody.vaginaSpot.list[0].clitLength <= 5) Render.text("your large clit like a tiny cock, stroking it up and down between your slime-lubed thumb and fore-finger.  It twitches and pulses with your heartbeats, the incredible sensitivity of it overloading your fragile mind with waves of pleasure.  ", false);
				if(player.lowerBody.vaginaSpot.list[0].clitLength <= 2) Render.text("your " + vaginaDescript(0) + " by pulling your folds wide and playing with your clit.  Another egg pops free from your diminishing belly, accompanied by an audible burst of relief.  You make wet 'schlick'ing sounds as you spread the slime around, vigorously frigging yourself.  ", false);
				Render.text("You cum hard, the big eggs each making your cunt gape wide just before popping free.  You slump down, exhausted and barely conscious from the force of the orgasm.  ", false);
				if(player.statusAffects.get("$1").value3 >= 11) Render.text("Your swollen belly doesn't seem to be done with you, as yet another egg pushes its way to freedom.   The stimulation so soon after orgasm pushes you into a pleasure-stupor.  If anyone or anything discovered you now, they would see you collapsed next to a pile of eggs, your fingers tracing the outline of your " + vaginaDescript(0) + " as more and more eggs pop free.  In time your wits return, leaving you with the realization that you are no longer pregnant.  ", false);
				Render.text("\n\nYou gaze down at the mess, counting " + eggDescript() + ".", false);
				player.orgasm();
				dynStats("resisted", false);
			}
			Render.text("\n\n<b>You feel compelled to leave the eggs behind, ", false);
			if(player.statusAffects.has("AteEgg")) Render.text("but you remember the effects of the last one you ate.\n</b>", false);
			else Render.text("but your body's intuition reminds you they shouldn't be fertile, and your belly rumbles with barely contained hunger.\n</b>", false);
