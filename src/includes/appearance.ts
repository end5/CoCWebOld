...
export default class Appearance {
    public appearance(player, e: MouseEvent = null): void 
    {
	    ...
	    {
		    if(player.skinType == Skin.PLAIN) outputText("  Your face is an adorable cross between human and ferret features, complete with a wet nose and whiskers.  The only oddity is your lack of fur, leaving only [skin] visible on your ferret-like face.",false);
        ...
        {
            outputText("  A pair of " + Utils.numToCardinalText(player.horns) + " inch horns grow from the sides of your head, sweeping backwards and adding to your imposing visage.", false);
	    }
	    //Super lizard horns
	    ...
	    {
		    if(player.horns > 0) 
                outputText("  Two antlers, forking into " + Utils.numToCardinalText(player.horns) + " points, have sprouted from the top of your head, forming a spiky, regal crown of bone.");
	    }
	    //BODY PG HERE
	    ...
	    else if(player.tailType == Tail.Fox) 
	    {
            ...
            else outputText("  " + Utils.numToCardinalCapText(player.lowerBody.tailVenom) + " swishing " + player.hairColor + " fox's tails extend from your " + assDescript() + ", curling around your body - the soft fur feels lovely.");
	    }
	    ...
	    else if(player.lowerBody == LowerBody.FERRET) outputText("  Two furry, digitigrade legs form below your [hips].  The fur is thinner on the feet, and your toes are tipped with claws.", false);
	    ...
	    {
            outputText("You have " + Utils.numToCardinalText(player.upperBody.chest.list[temp].breasts) + " " + breastDescript(temp) + ", each supporting ", false);
		    if(player.upperBody.chest.list[0].nipplesPerBreast == 1) 
                outputText(Utils.numToCardinalText(player.upperBody.chest.list[temp].nipplesPerBreast) + " " + player.upperBody.chest.BreastRatingLargest[0].nippleLength + "-inch " + nippleDescript(temp) + ".", false);
            else outputText(Utils.numToCardinalText(player.upperBody.chest.list[temp].nipplesPerBreast) + " " + player.upperBody.chest.BreastRatingLargest[0].nippleLength + "-inch " + nippleDescript(temp) + "s.", false);
	    ...
	    {
            outputText("You have " + Utils.numToCardinalText(player.upperBody.chest.count()) + " rows of breasts, the topmost pair starting at your chest.\n", false);
		    while (temp < player.upperBody.chest.count()) 
		    {
                ...
                outputText(Utils.numToCardinalText(player.upperBody.chest.list[temp].breasts) + " " + breastDescript(temp) + " with ", false);
			    if(player.upperBody.chest.list[temp].nipplesPerBreast == 1) 
                    outputText(Utils.numToCardinalText(player.upperBody.chest.list[temp].nipplesPerBreast) + " " + player.upperBody.chest.BreastRatingLargest[0].nippleLength + "-inch " + nippleDescript(temp) + " each.", false);
                else outputText(Utils.numToCardinalText(player.upperBody.chest.list[temp].nipplesPerBreast) + " " + player.upperBody.chest.BreastRatingLargest[0].nippleLength + "-inch " + nippleDescript(temp) + "s each.", false);
	    ...
	    if(player.lowerBody.cockSpot.count() == 1) 
	    {
		    ...
		    if(Math.round(10*player.lowerBody.cockSpot.list[temp].cockThickness)/10 < 2) 
		    {
			    if(Math.round(10*player.lowerBody.cockSpot.list[temp].cockThickness)/10 == 1) 
				    outputText(player.lowerBody.cockSpot.list[temp].cockThickness + " inch thick.", false);
                else
                    outputText(Math.round(10 * player.lowerBody.cockSpot.list[temp].cockThickness) / 10 + " inches thick.", false);
		    }
            else
                outputText(Utils.numToCardinalText(Math.round(10 * player.lowerBody.cockSpot.list[temp].cockThickness) / 10) + " inches wide.", false);
		    //Horsecock flavor
            ...
            if (((player.lowerBody.cockSpot.list[temp].cockType == Cock.DOG) || (player.lowerBody.cockSpot.list[temp].cockType == Cock.FOX)) || (player.lowerBody.cockSpot.list[temp].cockType == Cock.FOX)) 
		    {
			    ...
			    //List thickness
			    outputText("  The knot is " + Math.round(player.lowerBody.cockSpot.list[temp].cockThickness * player.lowerBody.cockSpot.list[temp].knotMultiplier * 10)/10 + " inches wide when at full size.", false);
		    }
		    //Demon cock flavor
	...
	
	    {
		...
		
		    {
			    ...
			    if(rando == 0) 
			    {
				    ...
				    outputText(" is ", false);
				    outputText(player.lowerBody.cockSpot.list[temp].cockLength + " inches long and ", false);
				    if(Math.floor(player.lowerBody.cockSpot.list[temp].cockThickness) >= 2) 
                        outputText(Utils.numToCardinalText(Math.round(player.lowerBody.cockSpot.list[temp].cockThickness * 10)/10) + " inches wide.", false);
				    else 
				    {
					    if(player.lowerBody.cockSpot.list[temp].cockThickness == 1) 
						    outputText("one inch wide.", false);
					    else outputText(Math.round(player.lowerBody.cockSpot.list[temp].cockThickness*10)/10 + " inches wide.", false);
				    }
			    }
			    if(rando == 1) 
			    {
				    outputText("--One of your ", false);
				    outputText(player.cockDescript(temp) + "s is " + Math.round(10*player.lowerBody.cockSpot.list[temp].cockLength)/10 + " inches long and ", false);
				    if(Math.floor(player.lowerBody.cockSpot.list[temp].cockThickness) >= 2) 
                        outputText(Utils.numToCardinalText(Math.round(player.lowerBody.cockSpot.list[temp].cockThickness * 10)/10) + " inches thick.", false);
				    else 
				    {
					    if(player.lowerBody.cockSpot.list[temp].cockThickness == 1) 
						    outputText("one inch thick.", false);
					    else outputText(Math.round(player.lowerBody.cockSpot.list[temp].cockThickness*10)/10 + " inches thick.", false);
				    }
			    }
			    if(rando == 2) 
			    {
			    ...
			    if(rando == 3) 
			    {
				    ...
				    outputText(player.cockDescript(temp) + " is " + Math.round(10*player.lowerBody.cockSpot.list[temp].cockLength)/10 + " inches long and ", false);
				    if(Math.floor(player.lowerBody.cockSpot.list[temp].cockThickness) >= 2) 
                        outputText(Utils.numToCardinalText(Math.round(player.lowerBody.cockSpot.list[temp].cockThickness * 10)/10) + " inches in diameter.", false);
				    else 
				    {
					    if(Math.round(player.lowerBody.cockSpot.list[temp].cockThickness*10)/10 == 1) 
						    outputText("one inch in diameter.", false);
					    else outputText(Math.round(player.lowerBody.cockSpot.list[temp].cockThickness*10)/10 + " inches in diameter.", false);
				    }
			    }
			    //horse cock flavor
                if (player.lowerBody.cockSpot.list[temp].cockType == Cock.HORSE) 
                ...
                if ((player.lowerBody.cockSpot.list[temp].cockType == Cock.DOG) || (player.lowerBody.cockSpot.list[temp].cockType == Cock.FOX)) 
			    {
				    ...
				    //List knot thickness
				    outputText("  The knot is " + Math.floor(player.lowerBody.cockSpot.list[temp].cockThickness * player.lowerBody.cockSpot.list[temp].knotMultiplier * 10)/10 + " inches thick when at full size.", false);
			    }
			    //Demon cock flavor
                if (player.lowerBody.cockSpot.list[temp].cockType == Cock.DEMON) 
			    {
	    ...
	    if(player.lowerBody.balls > 0) 
	    {
            ...
            outputText("  You estimate each of them to be about " + Utils.numToCardinalText(Math.round(player.lowerBody.ballSize)) + " ", false);
	    ...
	    if(player.lowerBody.vaginaSpot.count() > 0) 
	    {
		    ...
		    if(player.lowerBody.vaginaSpot.count() == 1) 
			    outputText("You have a " + vaginaDescript(0) + ", with a " + player.lowerBody.vaginaSpot.list[0].clitLength + "-inch clit", false);
		    if(player.vaginas[0].virgin) 
			    outputText(" and an intact hymen", false);
		    outputText(".  ", false);
		    if(player.lowerBody.vaginaSpot.count() > 1) 
			    outputText("You have " + player.lowerBody.vaginaSpot.count()+ " " + vaginaDescript(0) + "s, with " + player.lowerBody.vaginaSpot.list[0].clitLength + "-inch clits each.  ", false);
