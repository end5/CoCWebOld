...
private static const DUNGEON_CAVE_ENTRANCE: number			= 10;
private static const DUNGEON_CAVE_TUNNEL: number			= 11;
private static const DUNGEON_CAVE_GATHERING_HALL: number	= 12;
private static const DUNGEON_CAVE_FUNGUS_CAVERN: number		= 13;
private static const DUNGEON_CAVE_TORTURE_ROOM: number		= 14;
private static const DUNGEON_CAVE_SECRET_TUNNEL: number		= 15;
private static const DUNGEON_CAVE_ZETAZ_CHAMBER: number		= 16;
...
//	inDungeon = false;
	dungeonLoc = 0;
	clearOutput();
	Render.text("You leave the cave behind and take off through the deepwoods back towards camp.");
...
public function impGangAI():void {
	if(monster.statusAffects.has("ImpUber")) impGangUber();
...
public function imtacularMultiHitzilla():void {
	//Multiattack:
	if(monster.findStatusAffect(StatusAffects.Blind) < 0) Render.text("The imps come at you in a wave, tearing at you with claws!\n", false);
	//(ALT BLINDED TEXT)
	else Render.text("In spite of their blindness, most of them manage to find you, aided by the clutching claws of their brothers.\n", false);
	...
	while(hits > 0) {
		...
		damage = 0;
		//Blind dodge change
		if(monster.statusAffects.has("Blind") && rand(3) < 2) {
			Render.text(monster.capitalA + monster.short + " completely misses you with a blind attack!\n", false);
		}
		//Determine if dodged!
		else if(player.stats.spe - monster.spe > 0 && rand(((player.stats.spe-monster.spe)/4)+90) > 80) {
			if(player.stats.spe - monster.spe < 8) Render.text("You narrowly avoid " + monster.a + monster.short + "'s " + monster.weaponVerb + "!\n", false);
			else if(player.stats.spe - monster.spe >= 8 && player.stats.spe-monster.spe < 20) Render.text("You dodge " + monster.a + monster.short + "'s " + monster.weaponVerb + " with superior quickness!\n", false);
			else if(player.stats.spe - monster.spe >= 20) Render.text("You deftly avoid " + monster.a + monster.short + "'s slow " + monster.weaponVerb + ".\n", false);
		}
		//Determine if evaded
		else if(player.perks.has("Evade") && rand(100) < 10) {
			Render.text("Using your skills at evading attacks, you anticipate and sidestep " + monster.a + monster.short + "'s attack.\n", false);
		}
		else if(player.perks.has("Misdirection") && rand(100) < 10 && player.armorName == "red, high-society bodysuit") {
			Render.text("With the easy movement afforded by your bodysuit and Raphael's teachings, you easily avoid " + monster.a + monster.short + "'s attack.\n", false);
		}
		//Determine if cat'ed
		else if(player.perks.has("Flexibility") && rand(100) < 6) {
			Render.text("With your incredible flexibility, you squeeze out of the way of " + monster.a + monster.short + "", false);
			if(monster.plural) Render.text("' attacks.\n", false);
			else Render.text("'s attack.\n", false);
		}
		//OH SHIT SON YOU GOT REAPED
		else {
			if(hits == 6) Render.text("You're clawed viciously by an imp!", false);
			if(hits == 5) Render.text("One bites your ankle!", false);
			if(hits == 4) Render.text("An imp rakes his claws down your back.", false);
			if(hits == 3) Render.text("One of the little bastards manages to scratch up your legs!", false);
			if(hits == 2) Render.text("Another imp punches you in the gut, hard!", false);
			if(hits == 1) Render.text("Your arm is mauled by the clawing!", false);
...
public function impGangBukkake():void {
	Render.text("Many of the imps are overcome by the lust you've inspired.  They hover in the air around you, pumping their many varied demonic rods as they bring themselves to orgasm.\n", false);
	...
	while(hits > 0) {
		...
		//Blind dodge change
		if(monster.statusAffects.has("Blind") && rand(3) < 2) {
			Render.text(monster.capitalA + monster.short + "' misguided spooge flies everyone.  A few bursts of it don't even land anywhere close to you!\n", false);
		}
		//Determine if dodged!
		else if(player.stats.spe - monster.spe > 0 && rand(((player.stats.spe-monster.spe)/4)+90) > 80) {
			damage = rand(4);
			if(damage == 0) Render.text("A wad of cum spatters into the floor as you narrowly sidestep it.\n", false);
			else if(damage == 1) Render.text("One of the imps launches his seed so hard it passes clean over you, painting the wall white.\n", false);
			else if(damage == 2) Render.text("You duck a glob of spooge and it passes harmlessly by.  A muffled grunt of disgust can be heard from behind you.\n", false);
			else if(damage == 3) Render.text("You easily evade a blast of white fluid.\n", false);
		}
		//Determine if evaded
		else if(player.perks.has("Evade") && rand(100) < 30) {
			damage = rand(4);
			...
			Render.text("(Misdirection) ", false);
			if(damage == 0) Render.text("A wad of cum spatters into the floor as you narrowly sidestep it.\n", false);
			else if(damage == 1) Render.text("One of the imps launches his seed so hard it passes clean over you, painting the wall white.\n", false);
			else if(damage == 2) Render.text("You duck a glob of spooge and it passes harmlessly by.  A muffled grunt of disgust can be heard from behind you.\n", false);
			else if(damage == 3) Render.text("You easily evade a blast of white fluid.\n", false);
		...
		//Determine if cat'ed
		else if(player.perks.has("Flexibility") && rand(100) < 15) {
			damage = rand(4);
			Render.text("(Agility) ", false);
			if(damage == 0) Render.text("A wad of cum spatters into the floor as you narrowly sidestep it.\n", false);
			else if(damage == 1) Render.text("One of the imps launches his seed so hard it passes clean over you, painting the wall white.\n", false);
			else if(damage == 2) Render.text("You duck a glob of spooge and it passes harmlessly by.  A muffled grunt of disgust can be heard from behind you.\n", false);
			else if(damage == 3) Render.text("You easily evade a blast of white fluid.\n", false);
		...
		//(2-6 hits for +10 lust each!) (-5 lust per successful hit)
			...
			damage = rand(6);
			if(damage == 0) Render.text("A squirt of hot demonic cum splatters into your face!\n", false);
			if(damage == 1) Render.text("Your " + allBreastsDescript() + " are coated with thick demonic spunk!\n", false);
			if(damage == 2) Render.text("Some of the fluid splatters into your midriff and drools down to your waist, making your " + player.armorName + " slimy and wet.\n", false);
			if(damage == 3) Render.text("Seed lands in your " + hairDescript() + ", slicking you with demonic fluid.\n", false);
			if(damage == 4) Render.text("Another blast of jizz splatters against your face, coating your lips and forcing a slight taste of it into your mouth.\n", false);
			if(damage == 5) Render.text("The last eruption of cum soaks your thighs and the lower portions of your " + player.armorName + ", turning it a sticky white.\n", false);
			dynStats("lus", (7+int(player.stats.lib/40+player.stats.cor/40)));
		}
		monster.lust -= 5;
		hits--;
	}
...
public function impGangUber():void {
	if(monster.findStatusAffect(StatusAffects.ImpUber) < 0) {
		Render.text("Three imps on the far side of the room close their eyes and begin murmuring words of darkness and power.  Your eyes widen, recognizing the spell.  Anything but that!  They're building up a massive arousal spell!  They keep muttering and gesturing, and you realize you've got one round to stop them!\n", false);
		monster.statusAffects.add(new StatusAffect("ImpUber",0,0,0,0)));
	}
	else {
		//(OH SHIT IT GOES OFF) 
		//+50 lust!
		dynStats("lus", 50);
		Render.text("The imps in the back finish their spell-casting, and point at you in unison.  A wave of pure arousal hits you with the force of a freight train.   Your equipment rubs across your suddenly violently sensitive " + nippleDescript(0), false);
		if(player.biggestLactation() > 1) Render.text(" as they begin to drip milk", false);
		Render.text(".  The lower portions of your coverings ", false);
		if(player.lowerBody.cockSpot.count() > 0) {
			Render.text("are pulled tight by your " + player.multiCockDescript() + ", ", false);
			if(player.totalCocks() > 1) Render.text("each ", false);
			Render.text("beading a drop of pre-cum at the tip", false);
			if(player.lowerBody.vaginaSpot.hasVagina()) Render.text(", and in addition, the clothes around your groin ", false);
		}
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			Render.text("become stained with feminine moisture", false);
			if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) Render.text(" as your clit swells up in a more sensitive imitation of a cock", false);
		}
		if(player.gender == 0) Render.text("rub the sensitive skin of your thighs and featureless groin in a way that makes you wish you had a sex of some sort", false);
		Render.text(".\n", false);
		monster.statusAffects.remove("ImpUber");
	}
}
...
public function loseToImpMob():void {
	Render.text("", true);
	//(HP) 
	if(player.HP < 1) Render.text("Unable to handle your myriad wounds, you collapse with your strength exhausted.\n\n", false);
	//(LUST)
	else Render.text("Unable to handle the lust coursing through your body, you give up and collapse, hoping the mob will get you off.\n\n", false);
	
	Render.text("In seconds, the squirming red bodies swarm over you, blotting the rest of the room from your vision.  You can feel their scrabbling fingers and hands tearing off your " + player.armorName + ", exposing your body to their always hungry eyes.   Their loincloths disappear as their growing demonic members make themselves known, pushing the tiny flaps of fabric out of the way or outright tearing through them.   You're groped, touched, and licked all over, drowning in a sea of long tongues and small nude bodies.\n\n", false);
			   
	Render.text("You're grabbed by the chin, and your jaw is pried open to make room for a swollen dog-dick.   It's shoved in without any warmup or fan-fare, and you're forced to taste his pre in the back of your throat.  You don't dare bite down or resist in such a compromised position, and you're forced to try and suppress your gag reflex and keep your teeth back as he pushes the rest of the way in, burying his knot behind your lips.\n\n", false);
	
	//(tits)
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) {
		Render.text("A sudden weight drops onto your chest as one of the demons straddles your belly, allowing his thick, tainted fuck-stick to plop down between your " + allBreastsDescript() + ".  The hot fluid leaking from his nodule-ringed crown  swiftly lubricates your cleavage.  In seconds the little devil is squeezing your " + breastDescript(0) + " around himself as he starts pounding his member into your tits.  The purplish tip peeks out between your jiggling flesh mounds, dripping with tainted moisture.", false);
		if(player.biggestLactation() > 1) Render.text("  Milk starts to squirt from the pressure being applied to your " + breastDescript(0) + ", which only encourages the imp to squeeze even harder.", false);
		Render.text("\n\n", false);
	}
	//(NIPPLECUNTS!)
	if(player.upperBody.chest.hasFuckableNipples()) {
		Render.text("A rough tweak on one of your nipples startles you, but your grunt of protest is turned into a muffled moan when one of the imp's tiny fingers plunges inside your " + nippleDescript(0) + ".  He pulls his hand out, marveling at the sticky mess, and wastes no time grabbing the top of your tit with both hands and plunging himself in.", false);
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 7) Render.text("  He can only get partway in, but it doesn't seem to deter him.", false);
		else Render.text("  Thanks to your massive bust, he is able to fit his entire throbbing prick inside you.", false);
		Render.text("  The demon starts pounding your tit with inhuman vigor, making the entire thing wobble enticingly.  The others, seeing their brother's good time, pounce on ", false);
		if(player.upperBody.chest.countNipples() > 2) Render.text("each of ", false);
		Render.text("your other " + nippleDescript(0), false);
		if(player.upperBody.chest.countNipples() > 2) Render.text("s", false);
		Render.text(", fighting over the opening", false);
		if(player.upperBody.chest.countNipples() > 2) Render.text("s", false);
		Render.text(".  A victor quickly emerges, and in no time ", false);
		if(player.upperBody.chest.countNipples() == 2) Render.text("both", false);
		else Render.text("all the", false);
		Render.text(" openings on your chest are plugged with a tumescent demon-cock.\n\n", false);
	}
	//(SINGLE PEN) 
	if(!player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("Most of the crowd centers itself around your lower body, taking a good long look at your " + assholeDescript() + ".  An intrepid imp steps forwards and pushes his member into the unfilled orifice.  You're stretched wide by the massive and unexpectedly forceful intrusion.  The tiny corrupted nodules stroke every inch of your interior, eliciting uncontrollable spasms from your inner muscles.  The unintentional dick massage gives your rapist a wide smile, and he reaches down to smack your ass over and over again throughout the ordeal.", false);
	...
	if(player.totalCocks() > 0) {
		Render.text("Some of the other imps, feeling left out, fish out your " + player.multiCockDescript() + ".  They pull their own members alongside yours and begin humping against you, frotting as their demonic lubricants coat the bundle of cock with slippery slime.   Tiny hands bundle the dicks together and you find yourself enjoying the stimulation in spite of the brutal fucking you're forced to take.  Pre bubbles up, mixing with the demonic seed that leaks from your captors members until your crotch is sticky with frothing pre.\n\n", false);
	}
	//(ORGAZMO)
	Render.text("As one, the crowd of demons orgasm.  Hot spunk gushes into your ass, filling you with uncomfortable pressure.  ", false);
	if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("A thick load bastes your pussy with whiteness, and you can feel it seeping deeper inside your fertile womb.  ", false);
	Render.text("Your mouth is filled with a wave of thick cream.  Plugged as you are by the demon's knot, you're forced to guzzle down the stuff, lest you choke on his tainted baby-batter.", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) {
		Render.text("  More and more hits your chin as the dick sandwiched between your tits unloads, leaving the whitish juice to dribble down to your neck.", false);
		if(player.upperBody.chest.hasFuckableNipples()) {
			if(player.upperBody.chest.countNipples() == 2) Render.text("  The pair", false);
			else Render.text("  The group", false);
			Render.text(" of cocks buried in your " + nippleDescript(0) + " pull free before they cum, dumping the spooge into the gaping holes they've left behind.  It tingles hotly, making you quiver with pleasure.", false);
		}
	}
	Render.text("  Finally, your own orgasm arrives, ", false);
	if(player.lowerBody.cockSpot.count() == 0) Render.text("and you clench tightly around the uncomfortable intrusion.", false);
	else {
		Render.text("and " + player.sMultiCockDesc() + " unloads, splattering the many demons with a bit of your own seed.  You'd smile if your mouth wasn't so full of cock.  At least you got to make a mess of them!", false);
	}
	if(player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("  Your cunt clenches around the invading cock as orgasm takes you, massaging the demonic tool with its instinctual desire to breed.  Somehow you get him off again, and take another squirt of seed into your waiting cunt.", false);
	}
	Render.text("\n\n", false);
	
	Render.text("Powerless and in the throes of post-coital bliss, you don't object as you're lifted on the table", false);
	if(!player.lowerBody.vaginaSpot.hasVagina()) Render.text(" and forced to start drinking bottle after bottle of succubi milk", false);
	Render.text(".  You pass out just as round two is getting started, but the demons don't seem to mind....", false);
	doNext(loseToImpMobII);
}
//[IMP GANGBANG VOL 2]
public function loseToImpMobII():void {
	Render.text("", true);
	Render.text("You wake up, sore from the previous activity and a bit groggy.  You try to move, but find yourself incapable.  Struggling futilely, you thrash around until you realize your arms and legs are strapped down with heavy iron restraints.  You gasp out loud when you look down and discover your ", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) Render.text("new", false);
	else Render.text("much larger", false);
	Render.text(" tits, wobbling with every twist and movement you make.  You're stark naked, save for a sheer and somewhat perverse nurse's outfit.   The room around you looks to be empty, though you can see a number of blankets piled in the corners and a few cages full of spooge-covered faeries, all snoring contently.\n\n", false);
	
	
	Render.text("Eventually a lone imp enters the room.  It's Zetaz!  He looks you up and down and decrees, \"<i>You're ready.</i>\"  You struggle to shout him down, but all that escapes the gag in your mouth is incomprehensible gibberish.  He chuckles and flips a switch on the wall, and suddenly the most heavenly vibration begins within your sopping twat.", false);
	if(!player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("...Wait, your what?  You have a cunt now!?", false);
	}
	Render.text("  Your eyes cross at the pleasure as your mind struggles to figure out why it feels so good.\n\n", false);
	
	Render.text("Zetaz pours a few bottles into a larger container and connects a tube to an opening on the bottom of the bottle.  Your eyes trace the tube back to the gag in your mouth, and after feeling around with your tongue, you realize it's been threaded through the gag and down your throat.   Zetaz lifts up the bottle and hangs it from a hook on the ceiling, and you watch in horror as the fluid flows through the tube, helpless to stop it.  You shake your head desperately, furious at having fallen into the little fucker's hands at last.\n\n", false);
	
	Render.text("Zetaz walks up and paws at your ", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) Render.text("new", false);
	else Render.text("larger", false);
	Render.text(" mounds, flitting into the air to bring himself to eye-level.  He rambles, \"<i>It's so good to see you again, " + player.short + ".  Because of you, I had to flee from my honored place by Lethice's side.  I've had to hide in this fetid forest.  I'll admit, it hasn't been all bad.  We've caught a few faeries to play with, and with you here, the boys and I will have lots of fun.  We just need to reshape that troubled mind a little bit.</i>\"\n\n", false);
	
	Render.text("You barely register his monologue.  You're far too busy cumming hard on the vibrating intruder that's currently giving your stuffed snatch the workout of a lifetime.  Zetaz chuckles at your vacant stare and massages your temples gently, and you feel the touch of his dark magic INSIDE you.  It feels warm and wet, matching the feel of your body's other intrusion.   You try to fight it, and for a moment you feel like you might push the demon out of your mind.  Then your body cums, and your resistance melts away.  You violently thrash against your restraints, caving in to the pleasure as the imp rapes your body and mind as one.\n\n", false);
	
	Render.text("The desire to protect your village drips out between your legs, and thoughts of your independence are fucked away into nothing.  It feels good to cum, and your eyes cross when you see the bulge at your master's crotch, indicative of how well you're pleasing him.  It feels so good to obey!  Zetaz suddenly kisses you, and you enthusiastically respond in between orgasms.\n\n", false);
	
	Render.text("You gladly live out the rest of your life, fucking and birthing imps over and over as their live-in broodmother.", false);
	player.orgasm();
	player.HP += 100;
	gameOver();
}
...
public function impGangVICTORY():void {
	Render.text("", true);
	//Flag them defeated!
	flags[FlagEnum.ZETAZ_IMP_HORDE_DEFEATED] = 1;
	if(monster.HP < 1) Render.text("The last of the imps collapses into the pile of his defeated comrades.  You're not sure how you managed to win a lopsided fight, but it's a testament to your new-found prowess that you succeeded at all.", false);
	else Render.text("The last of the imps collapses, pulling its demon-prick free from the confines of its loincloth.  Surrounded by masturbating imps, you sigh as you realize how enslaved by their libidos the foul creatures are.", false);
...
public function impGangGetsRapedByMale():void {
	Render.text("", true);
	Render.text("You walk around and pick out three of the demons with the cutest, girliest faces.  You set them on a table and pull aside your " + player.armorName + ", revealing your " + player.multiCockDescriptLight() + ".  You say, \"<i>Lick,</i>\" in a tone that brooks no argument.  The feminine imps nod and open wide, letting their long tongues free.   Narrow and slightly forked at the tips, the slippery tongues wrap around your " + cockDescript(0) + ", slurping wetly as they pass over each other in their attempts to please you.\n\n", false);
	
	Render.text("Grabbing the center one by his horns, you pull him forwards until your shaft is pressed against the back of his throat.  He gags audibly, but you pull him back before it can overwhelm him, only to slam it in deep again.  ", false);
	Render.text("The girly imp to your left, seeing how occupied your " + cockDescript(0) + " is, shifts his attention down to your ", false);
	...
	else Render.text("ass", false);
	Render.text(", licking with care", false);
	if(player.lowerBody.balls == 0) Render.text(" and plunging deep inside", false);
	Render.text(".  The imp to the right wraps his tongue around the base ", false);
	if(player.lowerBody.cockSpot.hasSheath()) Render.text("just above your sheath ", false);
	Render.text(" and pulls it tight, acting as an organic cock-ring.\n\n", false);
	
	Render.text("Fucking the little bitch of a demon is just too good, and you quickly reach orgasm.  ", false);
	if(player.lowerBody.balls > 0) Render.text("Cum boils in your balls, ready to paint your foe white.  ", false);
	Render.text("With a mighty heave, you yank the imp forward, ramming your cock deep into his throat.  He gurgles noisily as you unload directly into his belly.   Sloshing wet noises echo in the room as his belly bulges slightly from the load, and his nose dribbles cum.   You pull him off and push him away.  He coughs and sputters, but immediately starts stroking himself, too turned on to care.", false);
	if(player.cumQ() > 1000) Render.text("  You keep cumming while the other two imps keep licking and servicing you.   By the time you finish, they're glazed in spooge and masturbating as well.", false);
	Render.text("\n\n", false);
	
	Render.text("Satisfied, you redress and prepare to continue with your exploration of the cave.", false);
	dynStats("cor", 1);
	player.orgasm();
	cleanupAfterCombat();
}
...
public function impGangGetsRapedByFemale():void {
	Render.text("", true);
	Render.text("You walk around to one of the demons and push him onto his back.  Your " + player.armorName + " falls to the ground around you as you disrobe, looking over your tiny conquest.  A quick ripping motion disposes of his tiny loincloth, leaving his thick demon-tool totally unprotected. You grab and squat down towards it, rubbing the corrupted tool between your legs ", false);
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLICK) Render.text("and coating it with feminine drool ", false);
	Render.text("as you become more and more aroused.  It parts your lips and slowly slides in.  The ring of tainted nodules tickles you just right as you take the oddly textured member further and further into your willing depths.", false);
	player.cuntChange(15,true,true,false);
	Render.text("\n\n", false);
	
	Render.text("At last you feel it bottom out, bumping against your cervix with the tiniest amount of pressure.  Grinning like a cat with the cream, you swivel your hips, grinding your " + clitDescript() + " against him in triumph.  ", false);
	if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) Render.text("You stroke the cock-like appendage in your hand, trembling with delight.  ", false);
	Render.text("You begin riding the tiny demon, lifting up, and then dropping down, feeling each of the nodes gliding along your sex-lubed walls.   As time passes and your pleasure mounts, you pick up the pace, until you're bouncing happily atop your living demon-dildo.\n\n", false);
	
	Render.text("The two of you cum together, though the demon's pleasure starts first.  A blast of his tainted seed pushes you over the edge.  You sink the whole way down, feeling him bump your cervix and twitch inside you, the bumps on his dick swelling in a pulsating wave in time with each explosion of fluid.  ", false);
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLAVERING) Render.text("Cunt juices splatter him as you squirt explosively, leaving a puddle underneath him.  ", false);
	else Render.text("Cunt juices drip down his shaft, oozing off his balls to puddle underneath him.  ", false);
	Render.text("The two of you lie together, trembling happily as you're filled to the brim with tainted fluids.\n\n", false);
	
	Render.text("Sated for now, you rise up, your body dripping gooey whiteness.  Though in retrospect it isn't nearly as much as was pumped into your womb.", false);
	if (player.pregnancyIncubation == 0) Render.text("  You'll probably get pregnant.", false);
	dynStats("cor", 1);
	player.orgasm();
	player.knockUp(PregnancyType.IMP, PregnancyType.INCUBATION_IMP - 14, 50);
	cleanupAfterCombat();
}
...
public function enterZetazsRoomFromTheSouth():void {
	if (flags[FlagEnum.ZETAZ_DOOR_UNLOCKED] == 0) {
		clearOutput();
		Render.text("The door won't budge.");
		doNext(playerMenu);
		return;
	}
...
public function getSwordAndGetTrapped():void {
	Render.text("", true);
	Render.text("You start to walk over to the corpse and its discarded weapon, but halfway through your journey, the unexpected happens.   The leaf-like petals shift underfoot, snapping up with lightning-quick speed.  You ", false);
	if(player.stats.spe < 50) Render.text("fall flat on your " + assDescript() + ", slipping on the slick, shifting surface.", false);
	else Render.text("stumble and nearly fall, slipping on the shifting, slick surface.", false);
	//[ADD 'CONTINUE' ON]
...
public function encapsulationPodAI():void {
	...
	if(monster.findStatusAffect(StatusAffects.Round) < 0) {
		Render.text("You shiver from the feeling of warm wetness crawling up your " + player.legs() + ".   Tentacles brush against your ", false);
		if(player.lowerBody.balls > 0) {
			Render.text(ballsDescriptLight() + " ", false);
		...
		else if(player.lowerBody.balls == 0) Render.text("taint ", false);
		Render.text("as they climb ever-further up your body.  In spite of yourself, you feel the touch of arousal licking at your thoughts.\n", false);
		if(player.lust < 35) {
			dynStats("lus", 1);
			player.lust = 35;
			statScreenRefresh();
	...
	//[Round 2 Action]
	else if(monster.statusAffects.get("Round").value1 == 2) {
		Render.text("The tentacles under your " + player.armorName + " squirm against you, seeking out openings to penetrate and genitalia to caress.  ", false);
		if(player.lowerBody.balls > 0) Render.text("One of them wraps itself around the top of your " + sackDescript() + " while its tip slithers over your " + ballsDescriptLight() + ".  Another ", false);
		else Render.text("One ", false);
		if(player.lowerBody.cockSpot.count() > 0) {
			Render.text("prods your " + cockDescript(0) + " for a second before it begins slithering around it, snake-like.  Once it has you encircled from " + player.cockHead() + " to ", false);
			if(!player.lowerBody.cockSpot.hasSheath()) Render.text("base", false);
			else Render.text("sheath", false);
			Render.text(", it begins to squeeze and relax to a pleasant tempo.  ", false);
		}
		else {
			if(player.lowerBody.vaginaSpot.hasVagina()) {
				Render.text("prods at your groin, circling around your " + vaginaDescript(0) + " deliberately, as if seeking other toys to play with.  ", false);
				if(player.lowerBody.vaginaSpot.list[0].clitLength > 4) Render.text("It brushes your " + clitDescript() + " then curls around it, squeezing and gently caressing it with a slow, pleasing rhythm.  ", false);
			}
			else {
				Render.text("prods your groin before curling around to circle your " + assholeDescript() + " playfully.  The entire tendril pulses in a pleasant, relaxing way.  ", false);
			}
		}
		if(player.lowerBody.cockSpot.count() > 1) {
			Render.text("Your other ", false);
			if(player.lowerBody.cockSpot.count() == 2) Render.text(cockDescript(1) + " gets the same treatment, and soon both of your " + player.multiCockDescriptLight() + " are quite happy to be here.  ", false);
			else Render.text(player.multiCockDescriptLight() + " get the same treatment and soon feel quite happy to be here.  ", false);
		}
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			Render.text("The violation of your " + vaginaDescript(0) + " is swift and painless.  The fungus' slippery lubricants make it quite easy for it to slip inside, and you find your " + vaginaDescript(0) + " engorging with pleasure in spite of your need to escape.  The tentacle folds up so that it can rub its stalk over your " + clitDescript() + ", ", false);
			if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) Render.text("and once it discovers how large it is, it wraps around it and squeezes.  It feels good!  ", false);
			else Render.text("and it has quite an easy time making your bud grow hard and sensitive.  The constant rubbing feels good!  ", false);
		}
		Render.text("One 'lucky' stalk manages to find your " + assholeDescript() + ".  As soon as it touches your rear 'entrance', it lunges forward to penetrate you.  The fluids coating the tentacle make your muscles relax, allowing it to slide inside you with ease.\n\n", false);
		
		Render.text("The rest of the mass continues to crawl up you.  They tickle at your ", false);
		if(player.pregnancyIncubation > 0 && player.pregnancyIncubation < 120) Render.text("pregnant ", false);
		Render.text("belly as they get closer and closer to ", false);
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) Render.text("your chest", false);
		else Render.text("the underside of your " + allBreastsDescript(), false);
		Render.text(".  Gods above, this is turning you on!  Your lower body is being violated in every conceivable way and it's only arousing you more.  Between the mind-numbing smell and the sexual assault you're having a hard time focusing.\n", false);
		if(player.lust < 65) {
			dynStats("lus", 1);
			player.lust = 65;
			statScreenRefresh();
	...
	//[Round 3 Action]
	else if(monster.statusAffects.get("Round").value1 == 3) {
		Render.text("The wet, warm pressure of the fungus' protrusion working their way up your body feels better than it has any right to be.  It's like a combination of a warm bath and a gentle massage, and when combined with the thought-numbing scent in the air, it's nigh-impossible to resist relaxing a little.  In seconds the mass of tentacles is underneath your " + player.armorName + " and rubbing over your chest and " + nippleDescript(0) + "s.  You swoon from the sensation and lean back against the wall while they stroke and caress you, teasing your sensitive " + nippleDescript(0) + ".", false);
		if(player.upperBody.chest.hasFuckableNipples()) Render.text("  Proof of your arousal leaks from each " + nippleDescript(0) + " as their entrances part for the probing tentacles.  They happily dive inside to begin fucking your breasts, doubling your pleasure.", false);
		Render.text("  Moans escape your mouth as your hips begin to rock in time with the tentacles and the pulsing luminance of your fungus-pod.  It would be easy to lose yourself here.  You groan loudly enough to startle yourself back to attention.  You've got to get out!\n\n", false);
		
		Render.text("The tentacles that aren't busy with your " + allBreastsDescript() + " are already climbing higher, and the slime has reached your waist.  If anything it actually makes the constant violation more intense and relaxing.  You start to sink down into it, but catch yourself and pull yourself back up.  No! You've got to fight!\n", false);
		if(player.lust < 85) {
			dynStats("lus", 1);
			player.lust = 85;
			statScreenRefresh();
	...
	//[Round 4 Action]
		...
		Render.text("What's happening to you definitely isn't rape.  Not any more.  You like it too much.  You lean back against a wall of the pod and thrust your " + hipDescript() + " pitifully against a phantom lover, moaning lewdly as you're forcibly pleasured.  You grab hold of the fleshy walls with your hands and try to hold yourself up, but your " + player.legs() + " have the consistency of jello.   They fold neatly underneath you as you slide into the ooze and begin to float inside it.  It's comforting in an odd way, and while you're gasping in between moans, your balance finally gives out.  You sink deeper into the fluid and lose all sense of direction.  Up and down become meaningless constructs that no longer matter to you.\n\n", false);
		
		Render.text("The thick slime passes over your lips and nose as you sink into the rising tide of bliss, and you find yourself wondering how you'll breathe.  Instinctively, you hold your breath.  Even riddled with sexual bliss and thought-obliterating drugs, you won't let yourself open your mouth when 'underwater'.  The lack of oxygen makes your heart hammer in your chest", false);
		if(player.totalCocks() > 0) {
			Render.text(", and " + player.sMultiCockDesc() + " bloats with blood, getting larger than ever", false);
		}
		Render.text(".  Before you can pass out, the constant penetration forces a moan from your lips.\n\n", false);
		
		Render.text("A tentacle takes the opportunity to slip into your mouth along with a wave of the slime.  You try to cough out the fluid, but there isn't any air left in your lungs to push it out.  The orally-fixated tendril widens and begins to pour more of it inside you, and with nowhere else to go, it packs your goo-filled lungs to the brim before you start to swallow.  You relax and exhale the last of your air from your nose as your body calms itself.  Somehow you can breathe the fungus-pod's fluids!\n\n", false);
		
		Render.text("You're floating in pure liquid bliss.  Thoughts melt away before they can form, and every inch of your body is being caressed, squeezed, or penetrated by the warm, slime-slicked tentacles.  Nearly every muscle in your body goes completely slack as you're cradled with bliss.  Without your thoughts or stress bothering you, the pleasure swiftly builds to a crescendo.\n\n", false);
		
		Render.text("The wave of need starts out inside your crotch, begging to be let out, but you can't even be bothered to move your " + hipDescript() + " anymore.  Without your help, release stays just out of reach, but the tentacles working your body seem intent on spurring it on.  The one inside your " + assholeDescript() + " begins to pump more quickly, and with the added pressure, you cum quickly.  ", false);
		if(!player.lowerBody.vaginaSpot.hasVagina()) {
			Render.text("Your body twitches weakly, too relaxed to move while it gets off from anal penetration.", false);
		}
		else Render.text("Your body twitches weakly, too relaxed to move while it gets off from being double-penetrated.", false);
		if(player.upperBody.chest.hasFuckableNipples()) {
			Render.text("  Your " + nippleDescript(0) + "s squirt around their phallic partners, leaking sexual lubricant ", false);
			if(player.biggestLactation() > 1) Render.text("and milk ", false);
			Render.text("while the fucking continues.", false);
		}
		if(player.lowerBody.cockSpot.count() > 0) {
			Render.text("  The tentacles around " + player.sMultiCockDesc() + " squeeze and rotate, screwing you silly through your orgasm while cum dribbles in a steady stream from your loins.  Normally it would be squirting out in thick ropes, but the muscle-relaxing drugs in your system make the spurts a steady, weak flow.", false);
			if(player.cumQ() > 800) Render.text("  Of course with all the semen you produce, the flesh-pod's ooze clouds over quite quickly, blocking your vision with a purple-white haze.", false);
		}
		if(player.biggestLactation() > 1) {
			Render.text("Milk leaks out too, ", false);
			if(player.biggestLactation() < 2) Render.text("though the slight dribble is barely noticeable to you.", false);
			else if(player.biggestLactation() < 3) Render.text("coloring things a little more white.", false);
			else Render.text("thickening your fluid-filled prison with nutrients.", false);
		}
	...
	if(monster.findStatusAffect(StatusAffects.Round) < 0) {
		monster.statusAffects.add(new StatusAffect("Round",2,0,0,0)));
	}
...
public function loseToThisShitPartII():void {
	...
	//[OPTIONAL CUM ESCAPE]
	if(player.cumQ() > 3500) {
		Render.text("Your orgasm drags on for so long that you begin to feel pressure from the cum-slime surrounding you.  It doesn't seem to matter to " + player.sMultiCockDesc() + ", which is too busy sending bliss to your brain and squirting cum for the tentacles to care.  It actually kind of hurts.  The oscillating purple ambiance flashes brighter in protest for a second, and then everything releases all at once.  The pressure is gone and you're sliding down on a wave of fungal-slime cum, feeling the tentacles being pulled from you by the sudden shift of position.  Moist cave air tickles at your " + player.skinDesc + " as you come to rest on another spongy petal and begin to cough out the sludge.\n\n", false);
		
		Render.text("Over the next minute your head clears and your strength returns.  You push yourself up on something hard, then glance down and realize you washed up next to the skeleton!  The bleached bone leers up at you knowingly, and everything you can see is covered in a thick layer of your spooge.  " + player.SMultiCockDesc() + " is still dripping more spunk.  Clearly your ruined orgasm didn't pump it ALL out.  You look down at the rapier and pick it up out of your mess, examining it.  The blade shines keenly, and the sword is balanced to perfection.  Though you succumbed to the same fate as its owner, your warped body saved you from sharing his fate.  Thankfully potential pods that carpet the floor don't even twitch at you.  Perhaps your orgasm was enough to sate them all?  Or maybe they've learned their lesson.", false);
	...
	//[OPTIONAL MILK ESCAPE]
		...
		Render.text("Your milk-spouting " + nippleDescript(0) + "s continuously pour your breast-milk into the soupy fluids surrounding you.  Once you let down your milk, there was no stopping it.  Pressure backs up inside the flesh-pod, pressing down on you with near painful intensity, but your " + allBreastsDescript() + " refuse to give up or slow down.  Even though each squirt jacks up the force on your body, your unholy milk production will not relent.  The oscillating purple ambience flashes bright in protest, then gives out entirely, along with the pressure.  At once you're pulled away by a wave of milk-laced fungus-slime, yanking the tentacles away from your body with the change in position.\n\n", false);
	...
	//Done if escaped
	if(monster.lust == 100) {
		flags[FlagEnum.ZETAZ_FUNGUS_ROOM_DEFEATED]++;
		cleanupAfterCombat();
		return;
	}
	//[BAD-END GO]
	...
	if(player.gender == 1 || (player.gender == 3 && rand(2) == 0)) {
		Render.text("The orgasm squirts and drips from " + player.sMultiCockDesc() + " for what seems like forever.  It feels so right, so perfect, that you actually whine in disappointment when it finally does end.  You can't even be bothered to reach down and stroke yourself.  The softening in your loins is nothing compared to your flaccid, listless muscles.  You couldn't make your arms reach down to touch yourself even if you could work up the motivation to try.  Thankfully the slippery tentacles curl back around your ", false);
		if(!player.lowerBody.cockSpot.hasSheath()) Render.text("base", false);
		else Render.text("sheath", false);
		Render.text(" and squeeze, forcing " + player.sMultiCockDesc() + " to inflate to readiness.  Deep inside your " + assholeDescript() + ", the tentacle starts to rub against your prostate.  It caresses the male organ on each side and pauses to squeeze the center of it, pushing a few drops of sticky cum from your trembling " + Appearance.cockNoun(CockType.HUMAN) + ".\n\n", false);
		
		Render.text("The vine-like stalks currently hugging " + player.sMultiCockDesc() + " constrict the base and begin to swirl around it in a circular motion.  Warm fungi-flesh and viscous, drugged ooze work together to send hot spikes of pleasure up your spinal-cord.  Despite your recent orgasm, you aren't being given any chance to recover or refill your ", false);
		if(player.lowerBody.balls > 0) Render.text("balls", false);
		else Render.text("prostate", false);
		Render.text(".  Things like logic and rest don't matter in this warm, soupy environment, at least not to your poor, unthinking mind and erect, sensitive dick", false);
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s", false);
		Render.text(".  With such stimulation coming so closely on the heels of your last orgasm, [eachCock] is suffering painful levels of pleasure.  Your whole body shakes from the sensory overload; though with your muscles so completely shut down, it's more of a shiver.\n\n", false);
		
		Render.text("Another wave of sperm begins the slow escape from your helpless, pinned form, drawn out by the fungus' constant sexual ministrations.  The fluid inside your pod gurgles noisily as the fluids are exchanged, but the sensory input doesn't register to your overloaded, drugged-out shell of a mind.  You've lost yourself to mindless pleasure, and repeated, endless orgasms.  The rest of your life is spent floating in an artificial womb, orgasming over and over to feed your fungus prison, and enjoying the pleasure that long ago eroded your ability to reason.", false);
		gameOver();
	}
	//(FEM)
	else {
		Render.text("You orgasm around the tentacles in your " + vaginaDescript(0) + " and " + assholeDescript() + " for what feels like hours, though some dim, half forgotten whisper of your mind tells you it can't possibly have gone on for that long.  It feels so right and so perfect that resistance is almost a foreign concept to you at this point.  How could you have tried to fight off this heaven?  You're completely limp, totally helpless, and happier than you ever remember.  The pulsing lights of your womb-like prison continue their steady beat in time with the tentacles buried in your snatch, soothing you while your body is played like a violin heading towards its latest crescendo.\n\n", false);
	
		Render.text("The steady rhythm of your penetration sends rockets of bliss-powered pleasure up your spinal cord and straight into your brain, where it explodes in orgasm.  Your body barely twitches, too relaxed to work up any muscle response, involuntary or otherwise.  A moment to rest never presents itself.  The cruel fungus never relents.  It never slows, unless it's only the briefest pause to intensify the next thrust.  Were you in the open air, away from the strange fluid you're now breathing, you'd be twisting and screaming with pleasure.  Instead you float and cum in silence.\n\n", false);
	
		Render.text("Fluids gurgle and shift inside the pod as they are exchanged.  If you were capable of noticing the sound or change, you might wonder if it's harvesting your sexual fluids, but even those thoughts are beyond you now. You've lost yourself to mindless pleasure, and repeated, endless orgasms.  The rest of your life is spent floating in an artificial womb, orgasming over and over to feed your fungus prison, and enjoying the pleasure that long ago eroded your ability to reason.", false);
		gameOver();
	}
}
...
public function encapsulationVictory():void {
	if(monster.HP <= 0) {
		flags[FlagEnum.ZETAZ_FUNGUS_ROOM_DEFEATED]++;
		Render.text("", true);
		Render.text("The pod's wall bursts under your onslaught.  The strength goes out of the tentacles holding you at once, giving them all the power of a limp noodle.  The spongy surface of the pod gives out, and the 'petals' split apart, falling down to the ground with a heavy 'thwack'.  You stand there, exulting in your freedom.  You've won!\n\nThe rapier you approached originally still lies there, and you claim your prize.", false);
	}
	cleanupAfterCombat();
}
...
public function ValaGetsSexed():void {
	...
	//(Herm/Male)
	if(player.totalCocks() > 0) {
		Render.text("The fairy's grinding and the sweet scent leaking out of her honey pot is getting under your skin, irritating an itch that's been building for a while now. Surely it wouldn't hurt to throw the mind-broken girl a pity fuck? It's not like she's one of the demons or anything. You push the girl back, gently and nod your head, stripping your " + player.armorName + " piece by piece, teasing the fairy with your slowness. As you disrobe, you glance back at the spot she'd been chained up and see the placard on the wall again. \"<i>Vala?</i>\" you ask. \"<i>Is that your name?</i>\" She stares blankly, unresponsive.\n\n", false);

		Render.text("She points at herself. \"<i>Bitch,</i>\" she explains. You grab her hands and stare into her pink eyes, holding her in your gaze until the faintest sparkle of light returns to their empty depths.\n\n", false);

		Render.text("\"<i>Your name is Vala,</i>\" you command, softly. She flinches. \"<i>Say it, or I won't fuck you.</i>\" She squirms, eyes clenched, rubbing her knees together as the heat in her groin battles the conditioned rape the imps used to crush her identity. Just as you knew it would, lust wins out.\n\n", false);
...
public function rejectFuckingVala():void {
	spriteSelect(85);
	Render.text("", true);
	if(flags[FlagEnum.TIMES_PC_DEFEATED_VALA] == 0) {
		Render.text("The fairy's weak insistence has begun to get obnoxious. What kind of prisoner dry humps her rescuer? Actually, if the heavy flow of lubricating girl cum dripping out of her pussy is any indication, it's the wettest humping you've had with your " + player.armorName + " still on. You seize the girl's shoulders and hold her up, pushing her away from your goo-stained lower body. You assure the girl that you won't be having sex with her here. It's far too dangerous, you tell her, to leave yourself vulnerable right now. You'll take her to safety when the demons are defeated. You try to impress on her the importance of speed and stealth, but you might as well be talking to a big-breasted brick wall. When she makes a grab at your crotch, you've had enough and throw her back.\n\n", false);

		Render.text("The fairy gathers herself and raises her eyes to you, mad passion whirling in their pink depths. \"<i>But Bitch is horny!</i>\" she demands, madly. Her wings gain sudden life, flapping rapidly to pull her frail body off the floor. Hovering before you, she curls her fingers into desperate claws and rakes at you. She's too far gone, you realize. You're going to have to fight the broken fairy!", false);
	}
	else {
		Render.text("What's wrong with this fairy? You've already beaten her once, but she's still dripping and grinding against you. If anything, it's even wetter than the last humping the sex-addicted faerie forced on you. You seize the girl's shoulders and hold her up, pushing her away from your goo-stained lower body once again and re-iterate that you won't be having sex with her right now – you have other tasks that need your attention.  It doesn't work, and once again she makes a move towards your crotch.", false);
		if(flags[FlagEnum.TIMES_PC_DEFEATED_VALA_AND_RAEPED] > 0) Render.text("  You smirk in wonder at the horny little slut.  You guess you'll have to put her back into her place and give her another dose of 'love'.", false);
		Render.text("\n\n", false);

		Render.text("The fairy stumbles up and fondles herself madly, already looking close to defeat. \"<i>Bitch doesn't want to leave masters!  Masters have good cum.  Let bitch show you how wonderful it tastes.</i>\" she demands, madly. Her wings gain sudden life, flapping rapidly to pull her frail body off the floor. Hovering before you, she curls her fingers into desperate claws and rakes at you. She's too far gone, you realize. You're going to have to fight the broken fairy, AGAIN!", false);
	}
	//Initiate fight
...
public function loseToVala():void {
	...
	if(player.gender == 0) {
		Render.text("", true);
		Render.text("Vala forces a bottle into your throat before your defeated form has a chance to react, and you grunt with pleasure as a new gash opens between your " + player.legs() + "!", false);
		player.createVagina();
		player.gender = 2;
...
public function loseToValaAsMale():void {
	...
	Render.text("You are powerless to stop the fairy as she drags you to the south wall and up to the wooden rail secured a couple of feet off the ground. \"<i>When she was still growing, Bitch was too small and tight for the masters,</i>\" your captor tells you. \"<i>They blessed her with this ladder to make us big enough. You will feel their generosity.</i>\" Gripping you under the arms, the fairy's lust-fuelled strength lifts you off the ground and flies you directly over the bristling peg ladder.\n\n", false);
	//[Next]
...
public function fightValaVictory():void {
	spriteSelect(85);
	Render.text("", true);
	Render.text("The fairy girl collapses, well-drilled obedience robbing her limbs of their fight. She squirms to a crouching bow, fully accepting you as her new " + player.mf("Master","Mistress") + ". The warped fae's empty eyes look up at you, her face a mask of rapture as she anxiously awaits her punishment, wagging her butt in the air as lubrication gushes down her thighs. It seems being defeated has excited the broken creature to a breeding frenzy. Her endurance must be incredible to be this frisky after your battle.", false);
	flags[FlagEnum.TIMES_PC_DEFEATED_VALA]++;
	//[Fuck] [Leave]
	if(player.gender > 0) {
...
public function valaFightVictoryFuckPtII():void {
	...
	flags[FlagEnum.TIMES_PC_DEFEATED_VALA_AND_RAEPED]++;
	cleanupAfterCombat();
}
...
public function tryToHealVala():void {
	...
	if(!(player.hasItem(consumables.PURHONY,1) || player.hasItem(consumables.P_PEARL,1))) {
		Render.text("You try your best with what you've got, but nothing seems to restore the broken fairy's mind to her sex-addled  body. You're going to have to go out and gather more materials. Surely there's something that can break the damage the imps have done to Vala.", false);
		doNext(playerMenu);
	}
	//(With Pure Honey)
	else if(player.hasItem(consumables.PURHONY,1)) {
		player.consumeItem(consumables.PURHONY,1);
		Render.text("You're not sure if Pure Honey will do the trick, but it seems like the most likely candidate. You set the broken girl down and step over to the alchemy table. Vala clings onto your leg as you walk, and you end up dragging her across the dungeon floor. Before things can get too out of hand with the needy girl, you pull out the vial of Pure Honey and arrange the equipment in front of you. Using the cleanest of the pipettes, you take a small portion of the honey and mix it with what you hope to be water, diluting the rich mixture to a more viscous state. Working quickly, you manage to produce a draught that the weak girl can tolerate. By now, she's managed to work her way to a sitting position and is grinding her dripping sex against your shin. You lean down and hold her nose to make her open her mouth. She gleefully opens wide, tongue thrashing about in anticipation. You pour the sweet-smelling concoction down her anxious throat and begin to re-cork the rest of your honey.\n\n", false);
...
public function wakeMeUpBeforeValaGoGosPtII():void {
	...
	if(player.gender == 1) {
		Render.text("", true);
		Render.text("You spot an unsecured set of chained manacles on the floor and an idea strikes you. Vala still sliding along your shaft, you bend down and grab the fetters, snapping one around the girl's neck like a collar. With a shudder at the cold, you lift the fairy up and lock the other end of the shackles around the root of your " + cockDescript(0) + ", steel snapping around your base tightly. The makeshift cock-ring works perfectly as your " + hipDescript() + " quiver, your body trying to orgasm but denied release by the metal loop. The fairy, meanwhile, thrashes atop your groin, the chain of her collar swinging between her tits, buffeting them with enough force to spray small white streams as she rides you. Her purple hair glimmers pink in the dull light of the dungeon as it bounces right along with her rocking hips. Even the diminished pulses of sunlight streaming from her stained and tattooed skin seems brighter as she is filled labia to cervix by your straining fuckpole.\n\n", false);
...
public function isValaAtBar():boolean {	return (flags[FlagEnum.FREED_VALA] != 0 && model.time.hours >= 12 && model.time.hours <= 21); }

//Purified Fairy
public function purifiedFaerieBitchBar():boolean {
	if (!isValaAtBar()) return false;
	if (flags[FlagEnum.FUCKED_VALA_AT_BAR_TODAY] > 0) {
	...
	if (player.gender > 0) {
		Render.text("  You could speak with her if you wished to.");
		return true;
	}
	Render.text("  Speaking to her might be better after you get a gender...");
	return false;
}
...
public function chooseValaInBar():void {
	...
	if(flags[FlagEnum.ENCOUNTERED_VALA_AT_BAR] == 0) {
	...
	else if(flags[FlagEnum.SHOULDRA_MET_VALA] == 1) {
		flags[FlagEnum.SHOULDRA_MET_VALA] = 2;
		Render.text("As soon as you call for Vala, she flutters over with a smile.  Before you even manage a word, she kisses you full on the mouth.  The kiss is a passionate, sloppy mess that has her tongue piercing your lips with reckless abandon, fluttering around your own as she presses her demon-distorted body against you, allowing you to feel just how gigantic her breasts are on her small frame.  It's deliciously obscene.");
	...
	else if(model.time.hours == 20 && rand(2) == 0 && player.lowerBody.cockSpot.hasCock()) {
		kinathisValaStuff();
		return;
	}
	else {
		...
		if(flags[FlagEnum.WEEKLY_FAIRY_ORGY_COUNTDOWN] == 0) {
	...
	if(flags[FlagEnum.SHOULDRA_MET_VALA] > 0 && shouldraFollower.followerShouldra())
...
public function cleansedValaFuckHerBrainsOutPtII():void {
	...
	//Part 2 - girls
		...
		if(player.stats.cor > 40) dynStats("cor", -.5);
	}
	//[End Encounter]
	cheatTime(1);
...
public function cleansedValaRepeatBrainFucking():void {
	spriteSelect(60);
	Render.text("", true);
	Render.text("She flashes a beautiful smile and tells the bartender that she'll be taking her break early. Grabbing you by the hand, she rushes to the stockroom and sheds her dress without any trace of her former shame about her tattooed back. When you're too slow taking off your " + player.armorName + ", she helps, deft hands made all the quicker by eagerness. When the two of you are naked, she pushes you onto the well-worn stool and sits in your lap, staring into your eyes with a small smile.", false);
...
public function faerieOrgyFuckFemaleContinue():void {
	spriteSelect(60);
	Render.text("", true);
	Render.text("The fairy girl lands at the edge of the bed, settling on her knees and resting her weighty chest on the mattress. Leaning in, she spreads your " + player.legs() + " with warm, delicate hands, fingertips stroking the insides of your thighs. You shiver and hug your arms under your breasts, a little nervous about this 'forest magic.' One of the fairies flutters down to Vala and hands her a pale white lily petal with a tittering giggle. The larger fairy takes the soft white bloom and, whispering a silken word to it, licks the cream-colored surface. Slithering further between your legs, she reaches your " + vaginaDescript(0) + " and brushes the petal against your " + clitDescript() + " with a teasing caress. Then, much to your surprise, she places it on the tip of her forefinger and slides it into your pussy. You gasp, the petal's soft surface rubbing your inner walls with a warmth you were unprepared for. Vala presses lightly on a spot inside your vagina and you lose your breath as the tremor of a small orgasm rumbles through your gut. Without explanation, the fairy barmaid takes a rose petal and repeats the process, another orgasm seizing you and leaving you panting. \"<i>Normally we only need the one petal, but since you and I are so much larger than the average fairy, I had my sisters bring lots. All for you,</i>\" she whispers, coyly.\n\n", false);
	
	//[If the Player has unlocked Scylla's Addiction Counseling event] 
	if(flags[FlagEnum.TIMES_SCYLLA_ADDICT_GROUP_EXPLOITED] + flags[FlagEnum.SCYLLA_TIMES_SHARED_IN_ADDICT_GROUP] + flags[FlagEnum.SCYLLA_MILK_THERAPY_TIMES] + flags[FlagEnum.SCYLLA_CUM_THERAPY_TIMES] + flags[FlagEnum.SCYLLA_SEX_THERAPY_TIMES] > 0) Render.text("Another fairy flies toward Vala and you blink through the building lusty haze when you recognize the little girl. Wearing a slightly slutty crystalline gown of translucent gossamer, you recognize Pastie, the fairy from Scylla's addiction counseling meeting. When you call out to her, she bashfully waves back. \"<i>Hey, uh, " + player.short + ". It's kinda weird seeing you outside of the meetings. How are you? How have things been?</i>\" She looks at your naked, reclined form on the bed and blushes. \"<i>Oh, right, right. Um. I'm just helping out Vala, you know. Sisters and all. Uh. I'm... I'm going to just give her this petal and get something to drink, okay? All this sobriety is making this really uncomfortable for me.</i>\" The other fairies laugh and Pastie hands off her blossom before zipping out of the room, probably to steal someone's drink downstairs.\n\n", false);
...
public function takeBondageStraps():void {
	clearOutput();
	flags[FlagEnum.ZETAZ_LAIR_TOOK_BONDAGE_STRAPS]++;
...
public function zetazTaunt():void {
	if(monster.findStatusAffect(StatusAffects.round) < 0) {
		monster.statusAffects.add(new StatusAffect("round",1,0,0,0)));
		Render.text("Zetaz asks, \"<i>Do you even realize how badly you fucked up my life, ", false);
		if(player.humanScore() >= 4) Render.text("human", false);
		else Render.text("'human'", false);
		Render.text("?  No, of course not.  That's the kind of attitude I'd expect from one of you!</i>\"", false);
	}
	else {
		monster.addStatusValue(StatusAffects.round,1,1);
		if(monster.statusAffects.get("round").value1 == 2) Render.text("\"<i>I lost my post!  And when you screwed up the factory?  I barely escaped with my life!  You ruined EVERYTHING!</i>\" screams Zetaz.", false);
		else if(monster.statusAffects.get("round").value1 == 3) Render.text("Zetaz snarls, \"<i>Do you know how hard it is to hide from Lethice?  DO YOU HAVE ANY IDEA!?  I've had to live in this fetid excuse for a jungle, and just when I found some friends and made it livable, you show up and DESTROY EVERYTHING!</i>\"", false);
		else if(monster.statusAffects.get("round").value1 == 4) Render.text("Zetaz explains, \"<i>I won't let you go.  I'm going to break you.</i>\"", false);
		else if(monster.statusAffects.get("round").value1 == 5) Render.text("\"<i>Would it have been that bad to go along with me?  You've seen the factory.  We would've kept you fed, warm, and provided you with limitless pleasure.  You would've tasted heaven and served a greater purpose.  It's not too late.  If you come willingly I can make sure they find a good machine to milk you with,</i>\" offers the imp lord.", false);
		else if(monster.statusAffects.get("round").value1 == 6) Render.text("\"<i>Why won't you fall?</i>\" questions Zetaz incredulously.", false);
		else if(monster.statusAffects.get("round").value1 == 7) Render.text("The imp lord suggests, \"<i>If you give up and let me fuck your ass maybe I'll let you go.</i>\"", false);
		else if(monster.statusAffects.get("round").value1 == 8) Render.text("Zetaz pants, \"<i>Just give up!  I'm nothing like the weakling you met so long ago!  I've been through hell to get here and I'm not giving it up just because you've shown up to destroy my hard work!</i>\"", false);
		else Render.text("He glares at you silently.", false);
	}
}
...
public function zetazAI():void {
	...
	//turn.
	if(monster.lust > 50 && (monster.statusAffects.has("Fear") || monster.statusAffects.has("Blind"))) {
		monster.statusAffects.remove("Fear");
		monster.statusAffects.remove("Blind");
		monster.lust -= 10;
		Render.text("Zetaz blinks and shakes his head while stroking himself.  After a second his turgid member loses some of its rigidity, but his gaze has become clear.  He's somehow consumed some of his lust to clear away your magic!", false);
	}
	
	//STANDARD COMBAT STATUS AFFECTS HERE
	if(monster.statusAffects.has("Stunned")) {
		Render.text("Your foe is too dazed from your last hit to strike back!", false);
		monster.statusAffects.remove("Stunned");
		combatRoundOver();
		return;
	}
	let select:number=1;
	...
	if(monster.statusAffects.has("Constricted")) {
		...
		if(monster.statusAffects.get("Constricted").value1 <= 0) {
			Render.text("  " + monster.capitalA + monster.short + " proves to be too much for your tail to handle, breaking free of your tightly bound coils.", false);
			monster.statusAffects.remove("Constricted");
		}
	...
	if(monster.lust > 50 && monster.HPRatio() <= .5) {
		Render.text("The imp lord shudders from his wounds and the pulsing member that's risen from under his tattered loincloth.  He strokes it and murmurs under his breath for a few moments.  You're so busy watching the spectacle of his masturbation that you nearly miss the sight of his bruises and wounds closing!  Zetaz releases his swollen member, and it deflates slightly.  He's used some kind of black magic to convert some of his lust into health!", false);
		monster.addHP(0.25 * monster.eMaxHP());
		monster.lust -= 20;
		dynStats("lus", 2);
	}
	else {
		let attackChoice:number = rand(3);
		if(attackChoice == 0) {
			//Chucks faux-heat draft ala goblins. - 
			Render.text("Zetaz grabs a bottle from a drawer and hurls it in your direction!  ", false);
			if((player.perks.has("Evade") && rand(4) == 0) ||
				(player.perks.has("Flexibility") && rand(6) == 0) ||
				(player.stats.spe > 65 && rand(10) == 0) ||
				(player.perks.has("Misdirection") && rand(100) < 20 && player.armorName == "red, high-society bodysuit")) {
				Render.text("You sidestep it a moment before it shatters on the wall, soaking the tapestries with red fluid!", false);
			}
			else {
				Render.text("You try to avoid it, but the fragile glass shatters against you, coating you in sticky red liquid.  It seeps into your " + player.skinDesc + " and leaves a pleasant, residual tingle in its wake.  Oh no...", false);
			...
			Render.text("The imp leaps into the air with a powerful spring, beating his wings hard to suspend himself in the center of his bedchamber.  Dust kicks up into the air from the force of his flight and turns the room into a blinding tornado!  Small objects smack off of you, ", false);
			//(causing little damage/
			if(player.tou > 60) Render.text("causing little damage", false);
			else {
				let dmg:number = 1 + rand(6);
				dmg = takeDamage(dmg);
				Render.text("wounding you slightly (" + dmg + ")", false);
				
			}
			Render.text(" while the dust gets into your eyes, temporarily blinding you!", false);
...
public function gigaArouse():void {
	Render.text("You see " + monster.a + monster.short + " make familiar arcane gestures at you, but his motions seem a lot more over the top than you'd expect from an imp.\n\n", false);
	dynStats("lus", rand(player.stats.lib/10)+player.stats.cor/10+15);
	if(player.lust < 30) Render.text("Your nethers pulse with pleasant warmth that brings to mind pleasant sexual memories.  ", false);
	if(player.lust >= 30 && player.lust < 60) Render.text("Blood rushes to your groin in a rush as your body is hit by a tidal-wave of arousal.  ", false);
	if(player.lust >= 60) Render.text("Your mouth begins to drool as you close your eyes and imagine yourself sucking off Zetaz, then riding him, letting him sate his desires in your inviting flesh.  The unnatural visions send pulses of lust through you so strongly that your body shivers.  ", false);
	if(player.lowerBody.cockSpot.count() > 0) {
		if(player.lust >= 60 && player.lowerBody.cockSpot.count() > 0) Render.text("You feel " + player.sMultiCockDesc() + " dribble pre-cum, bouncing with each beat of your heart and aching to be touched.  ", false);
		if(player.lust >= 30 && player.lust < 60 && player.lowerBody.cockSpot.count() == 1) Render.text(player.SMultiCockDesc() + " hardens and twitches, distracting you further.  ", false);
	}
	if(player.lowerBody.vaginaSpot.count() > 0) {
		if(player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.NORMAL && player.lowerBody.vaginaSpot.count() == 1) Render.text("Your " + allVaginaDescript() + " dampens perceptibly, feeling very empty.  ", false);
		if(player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.WET && player.lowerBody.vaginaSpot.count() > 0) Render.text("Your crotch becomes sticky with girl-lust, making it clear to " + monster.a + monster.short + " just how welcome your body finds the spell.  ", false);
		if(player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLICK && player.lowerBody.vaginaSpot.count() == 1) Render.text("Your " + allVaginaDescript() + " becomes sloppy and wet, dribbling with desire to be mounted and fucked.  ", false);
		if(player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.DROOLING && player.lowerBody.vaginaSpot.count() > 0) Render.text("Thick runners of girl-lube stream down the insides of your thighs as your crotch gives into the demonic magics.  You wonder what " + monster.a + monster.short + "'s cock would feel like inside you?  ", false);
		if(player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING && player.lowerBody.vaginaSpot.count() == 1) Render.text("Your " + allVaginaDescript() + " instantly soaks your groin with the heady proof of your need.  You wonder just how slippery you could " + monster.a + monster.short + "'s dick when it's rammed inside you?  ", false);
	}
...
public function defeatZetaz():void {
	flags[FlagEnum.DEFEATED_ZETAZ]++;
	Render.text("", true);
	//[VICTORY HP]
	if(monster.HP < 1) Render.text("Zetaz sinks down on his knees, too wounded to continue.  He looks up at you with helpless rage in his eyes and asks, \"<i>Are you satisfied now?  Go ahead then, kill me.  My life hasn't been worth living since I met you anyway.</i>\"\n\n", false);
	//[VICTORY LUST] 
	else Render.text("Zetaz sinks down on his knees and fishes his massive, pre-drooling member from under his loincloth.  He looks up at you, nearly crying and moans, \"<i>Why? Ruining my life wasn't enough?  You had to make me jerk off at your feet too?  Just kill me, I don't want to live anymore.</i>\"\n\n", false);
	
	//[Both] 
	Render.text("He can't die yet.  You need to know where his master, this 'Lethice', is.  It sounds like she's the queen-bitch of the demons, and if you're going to break this vicious cycle", false);
	//( or take her place)
	if(player.stats.cor > 66) Render.text(" or take her place", false);
	Render.text(", you need to find her and bring her down.  What do you do?", false);
...
public function releaseZForInfo():void {
	Render.text("", true);
	Render.text("You look the pathetic imp up and down and smirk.  He closes his eyes, expecting a summary execution, but you present him with an offer instead.  If he gives you more information on Lethice and where to find her, you'll let him go scot-free and avoid him if he doesn't make a nuisance of himself.\n\n", false);
	
	Render.text("\"<i>Really?</i>\" questions Zetaz in a voice laced with suspicion. \"<i>For fuck's sake, I'm already a renegade.  I'll take your deal.  It's not like it costs me anything I wouldn't give away for free anyway.</i>\"\n\n", false);
	
	Render.text("Invigorated by the promise of safety and freedom, Zetaz pulls himself up and ", false);
	if(monster.HP < 1) Render.text("staggers", false);
	else Render.text("nearly stumbles over his lust-filled cock", false);
	Render.text(" towards a desk.  His dextrous fingers twist the knob on the top drawer expertly until a quiet 'click' comes from the furniture.  He reaches down to the divider between the drawers and pulls on it, revealing a tiny, hidden compartment.  In the center of it is a detailed map of the mountain and its upper reaches.  Though the secret diagram is quite crude, it depicts a winding trail that bypasses numerous harpy nests, minotaur caves, and various unrecognizable pitfalls to reach the cloud-shrouded mountain peak.  The drawing loses much of its detail once it gets to the demon fortifications at the top, but it can't be that hard to track down Lethice once you've entered the seat of her power, can it?\n\n", false);
	
	Render.text("A loincloth flies across the room and deposits itself on your shoulder, startling you from your planning.  You glance back and see Zetaz tearing through his possessions, tossing his most prized items into a burlap sack with reckless abandon.  His whole body is trembling, as he ties it to a wooden pole, never once looking up at you.  Perhaps he fears you might change your mind?  ", false);
	if(player.stats.cor > 66) {
		Render.text("You smirk down at him and fold your arms over your ", false);
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) Render.text("chest", false);
		else Render.text(breastDescript(0), false);
		Render.text(", relishing his fear while you consider the possibilities", false);
	}
	else if(player.stats.cor > 33) {
		Render.text("You chuckle with amusement and watch the little bastard scrabble to pack up his life, relishing the chance to pay him back for your previous encounter", false);
	}
	else {
		Render.text("You sigh and rub at your temples as the little jerk scrabbles to pack his life away.  In spite of yourself, you actually feel a little bad about the situation", false);
	}
	Render.text(".  Zetaz scrambles out the south door, never once looking back at the tattered remnants of his old home.", false);
...
public function sexualInterrogation():void {
	Render.text("", true);
	Render.text("You lean down until your face hovers over Zetaz, looking him square in the eyes, and explain, \"<i>I can't have someone who knows the way to the demons' headquarters dying before they tell me how to get there, can I?</i>\"\n\n", false);
	
	Render.text("\"<i>Piss off!  You won't get shit from me,</i>\" retorts the defeated demon, \"<i>You may as well finish me off – I'll NEVER help a " + player.mf("jackass","bitch") + " like you!</i>\"\n\n", false);
	
	Render.text("Smirking, you grab a strip of leather from Zetaz's dresser and dangle it over his nose.\n\n", false);
	
	Render.text("You whisper, \"<i>This is all I'll need.</i>\"\n\n", false);
	
	Render.text("The imp looks up with his face shrouded in confusion as he asks, \"<i>I don't think a string is going to help you much, " + player.mf("dork","skank") + ".</i>\"\n\n", false);
	
	Render.text("\"<i>Give me a moment, my stupid little snitch,</i>\" you taunt as you lift his loincloth, exposing the hardness concealed within.  It pulses, growing harder and an inch longer just from your brief touch and exposure to the air.  Perhaps the imp isn't as in control of his libido as he'd like you to think?  You twirl the leather strip around his base and swiftly knot it, getting a tight enough seal to make Zetaz grunt in discomfort. \"<i>Ungh! What the fuck!? Ow, goddamnit!</i>\"\n\n", false);
	
	Render.text("Even with his protests and cries, you watch his cock inflate further, until it looks stuffed far beyond his normal capacity.  It twitches and drools corrupted pre-seed as you slide your finger along his urethra, watching the member bob and twitch from the slight, soft touches.  That must feel quite good.  Zetaz confirms your hunch by lifting his hips off the ground, shaking them lewdly to try to grind against your hand.  You don't deny him the friction he craves, wrapping your hand around as much of his meat as your fingers will encircle until steady dribbles of fluid escape from his urethra.  The tainted nodules of his demonic dick begin to flare and pulse, signaling that his orgasm is almost upon him.\n\n", false);
	
	Render.text("He's NOT allowed to cum – not until you get the information you need!   You slide your fingers down to his base in one fluid stroke, slamming your hand against his crotch as his orgasm starts to bubble up.  Before your opponent can attain release, you squeeze hard with one hand and tighten the leather cord with the other, clamping the base until the imp's cum is bottled up in his abdomen.  Zetaz cries, \"<i>No-no-no, let me cum, please let me cum, need-ta-cum-nooowwww.</i>\"\n\n", false);
	
	Render.text("No such luck.  You wait for his body to stop convulsing and return to your task, one hand sealed around his base while the other begins to stroke him with firm, steady motions, sliding over the pebbly surface of his dick's nubs.  Your victim continues his begging and crying, but you don't let up as you pause to gather his escaped pre-cum and smear it over his tip.  Zetaz pants and groans, trembling and swelling in your hand from your efficient hand-job.   Spitting on your palm, you bump up the tempo and begin to stroke him hard and fast, sliding over his cockring-swollen prick with practiced, deliberate motions.\n\n", false);
	
	Render.text("\"<i>Tell me how to find the head demon and I'll let you cum.  Don't make this any 'harder' than it has to be,</i>\" you whisper.\n\n", false);
	
	Render.text("The demon's voice starts to crack in spite of his efforts to remain defiant. \"<i>No!  I-uh-won't let yo-oooooh-control meeeeee!</i>\"\n\n", false);
	
	Render.text("His protests trail into incoherent squeals and babbles as you bottle up his second orgasm behind the tightly tied strap.  Again, his body twists and writhes in your grip, tortured with the ever-increasing sexual tension.  Zetaz looks up at you with a pleading, cross-eyed expression as he tries to regain his wits, but you just keep pumping away.  His balls are visibly pulsing and quivering, desperately needing to release the building pressure.  You meet his gaze calmly, your hands continuing their work on the bloated imp-cock, and you break into a knowing smile as he thickens in your grip for the third time.\n\n", false);
	
	Render.text("\"<i>Well Zetaz?  Is three the lucky number, or do I have to switch hands and keep backing you up until you go mad?</i>\" you ask.\n\n", false);
	
	Render.text("His hands claw the rug underneath him as he gasps, \"<i>You win, you win!  The desk has a-ah ah ahh-hidden drawer with a map to Lethice's hideout.  Please justletmecomeletmecomeletmecomePLEAAAAASE!</i>\"\n\n", false);
...
public function loseToZetaz():void {
	Render.text("", true);
	Render.text("\"<i>Well, isn't this familiar?</i>\" asks Zetaz as he watches your ", false);
	if(player.lust > 99) Render.text("masturbating", false);
	else Render.text("prone", false);
	Render.text(" form with an amused expression, \"<i>The first champion in ages to retain " + player.mf("his","her") + " free will for more than a few minutes, and " + player.mf("he","she") + "'s brought to " + player.mf("his","her") + " knees by the very imp " + player.mf("he","she") + " escaped!  Once you've learned your proper place, you'll guarantee my safe return to my rightful station.  Perhaps I'll even get a promotion?  After all, you've defeated so many higher ranking demons already.</i>\"\n\n", false);
	
	//'Fix' genderless folks.
	if(player.gender == 0) {
		Render.text("He squints down at you with a bemused look and laughs, \"<i>How did you lose your gender anyhow?  Never mind, we've got to do something about that!</i>\"\n\n", false);
		Render.text("Zetaz grabs a bottle, uncorks it, and crams it against your lips while you're still too dazed to resist.  He massages your throat to make you swallow the milk-like fluid, and in seconds the skin of your groin splits to form a new, virgin pussy.\n\n", false);
		player.createVagina();
		player.gender = 2;
	}
...
public function femaleZetazOver():void {
	...
	Render.text("With your resistance ", false);
	if(player.HP < 1) Render.text("beaten out of you", false);
	else Render.text("moistening the delta of your legs", false);
	...
	if(player.stats.cor < 33) Render.text("shame", false);
	else if(player.stats.cor < 66) Render.text("confuse", false);
	else Render.text("arouse", false);
	Render.text(" you as you're thrown atop one of the tables.   You grunt as leather straps are produced and laid over your form to restrain you.  In the span of a minute you're completely immobilized from the neck down, and your " + player.legs() + " are kept spread to allow easy access to your " + vaginaDescript(0) + ".\n\n", false);
	
	Render.text("Shuffling as they remove their garments, the entire gang of imps, as well as Zetaz, are completely nude.  They've all grown full and hard from the sight of your nubile, restrained body, and in spite of yourself you get ", false);
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) Render.text("even more wet ", false);
	else Render.text("a little wet ", false);
	Render.text("from the masculine scent the aroused penises are producing.  ", false);
	if(player.stats.cor < 33) Render.text("How could you be turned on by such a repulsive situation?  You're going to be raped, brainwashed, and either kept as a pet or tossed in a milking tube for the rest of your life and your body is acting like some horny slut!", false);
	else if(player.stats.cor < 66) Render.text("You marvel at just how turned on you're getting from the strange situation.  You know you'll be raped, drugged, and used as a toy or milk cow, but your loins are thrumming with warm, wet desire.", false);
	else Render.text("How did you wind up in such an arousing situation?  You're going to be raped, drugged, and probably milked in a factory for the rest of your life.  Your body is so fucking turned on that you know you'll love every second of it, but your desire to triumph and dominate mourns the loss of your freedom.", false);
	Render.text("  The crowd draws close, but Zetaz's voice rings out, thick with the tone of command, \"<i>Not yet, my brothers; this one will be mine first.  I'll claim each of her holes, then you may each have your fill of her.</i>\"\n\n", false);
	
	Render.text("The imps draw back, clearing a path for their leader to emerge, and the new, much more imposing Zetaz climbs atop the table.   He glances at your " + vaginaDescript(0) + " with a knowing eye and smiles, walking further forward until he's standing next to your face with his tainted, corruption-filled cock dangling overhead.  You're so distracted by the purplish-black demon-cock swinging above your lips that the sharp pain takes you completely off-guard. As soon as the discomfort passes you twist your head around to try and find the source of your irritation.\n\n", false);
	
	Render.text("Zetaz turns away from you, holding a spent needle in one of his clawed hands as he exchanges it with one of his kin for another injector, only this one is filled with viscous white fluid.  He glances down at you, watching you intently for some kind of reaction, but you won't give him the satisfaction!  Even so, the room is getting so bright that your eyes start tearing up, and you blink repeatedly to rid yourself of them before half-closing your eyelids to shield your poor pupils.  Maybe that's what he's looking for?  The room spins and you find yourself thankful to be strapped down; even if only seated, you would probably tumble from your chair.\n\n", false);
	
	Render.text("Your lips start to tingle, and you run your tongue over them reflexively.  A shiver of pleasure worms through your body, and you instinctively press your " + player.legs() + " against the straps in an effort to spread them further.  Worse yet, your lips feel much plumper and fuller than a few moments ago.  ", false);
	if(flags[FlagEnum.NUMBER_OF_TIMES_MET_SCYLLA] > 0) Render.text("Unbidden, Scylla's face comes to mind, and you realize the drugs coursing through your veins must be doing something similar to you!  Her visage changes to your own, though the thick, cock-sucking lips remain behind, eager to be penetrated.", false);
	else Render.text("Unbidden, you imagine yourself with thick, cock-sucking lips, so swollen and bloated that they're slightly pursed and ready to be penetrated.", false);
	Render.text("  Warm slipperiness slides over your lips again, feeling nearly as good as it would on your lower lips, and you pull your rebellious tongue back into your mouth with a gasp of pleasure.\n\n", false);
	
	Render.text("This must be what Zetaz was waiting for, and the imp carefully injects the next chemical cocktail into the other side of your neck while you're distracted by orally masturbating your new mouth.  Your " + vaginaDescript(0) + " ", false);
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) Render.text("gushes fresh fluids into a puddle on the table", false);
	else if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.WET) Render.text("drools a heavy flow of liquid arousal onto the hardwood table", false);
	else Render.text("begins to dribble a steady flow of liquid on to the table's girl-slicked boards", false);
	Render.text(".  ", false);
	if(player.inHeat) Render.text("D", false);
	else Render.text("Foreign d", false);
	Render.text("esires wash through your doped up body, and your hungry slit practically demands to be filled with cock and injected with semen.  It wants to be filled with... with males, and with their hot, sticky cum. No, your hot little pussy doesn't want that – you do.  Gods above and below, you want to feel your belly pumped full of imp sperm until their offspring are wriggling in your womb.  And then you want them to come in you some more!\n\n", false);
	
	Render.text("That sexy-... no, that bastard's dick is so hard, and he's starting to squat down now that you're feeling so randy.  The artificial needs coursing through your body make it hard to resist, but you've got to try!  You can't open your mouth and... mmm, it feels so good when those nubs touch your bee-stung lips.  Giving in isn't an option, even if you can't stop him from fucking your mouth, you aren't going to curl your tongue around his member and lick it, just like that, sliding it over his bumpy surface until corrupted pre is dripping onto your tongue. Yes, you won't let him out of your mouth until you can get his seed inside you, what are the other imps waiting for?  Your other hole is soooo hungry!\n\n", false);
	
	Render.text("The mental incongruities in your thoughts are subsumed in a wave of hot, sticky fuck that's slowly rising over your thought processes with each lick and suck of Zetaz's thick, sexy dick.  He plunges down, stuffing your greedy gullet with the full length of his elephantine member and letting you know just how much he's enjoying your oral cum-hole.  You stick your tongue out to slurp at his desire-filled balls, swooning at the feeling of so much cock-flesh and slippery tongue sliding between your sensitive-as-a-pussy lips.  They twitch and pull tightly against his groin as he grabs your " + hairDescript() + " and hilts himself, allowing your lips to seal around his base as his urethra rhythmically bulges with orgasm.  A feeling of warm fullness grows in your gut with each pulse of cum, and you work your throat muscles to squeeze his tip of every last drop while you try to get off on the feelings coming from your mouth.\n\n", false);
	
	Render.text("Once finished, the imp yanks himself up and pulls his orgasm-distended member from your lips with such force that it feels like each of his nubs is flicking your lips.  The orgy of oral pleasure sets off fireworks in your head strong enough to cross your eyes and make you babble incoherent 'thank-you's and moans.  You pant happily and lick the residue of Zetaz's love from your lips, shivering from the sensitivity and trying to come to grips with what happened.  It doesn't do much good – you're already getting horny again, and you still haven't been knocked up.  Even though you know something about the situation is deeply wrong, you're horny as hell and desperately desire to be a mother.  Maybe it's just that there's all these strong, handsome males here but none of them are fucking your horny, wet twat.  There's something wrong with that!", false);
...
public function zetazBadEndEpilogue():void {
	Render.text("", true);
	if(player.gender == 2) {
		Render.text("The once-champion, " + player.short + " was raped repeatedly by every imp that survived her initial assault.  Her mind never recovered from the initial orgy, and she found herself happy to be named 'Fuck-cow'.  She quickly became a favorite of Zetaz's ever growing brood, and surprised them all with her fertility and rapidly decreasing incubation times.  Within a few months, she was popping out litters of tiny masters even faster than Vala.  Within a year, her body was so well trained and her womb so stretched that she could keep multiple litters growing within at all times.\n\n", false);
	
		Render.text("It was rare for Fuck-cow's cunt or mouth to be empty, and she delighted in servicing any male she was presented with.  Her masters even captured bee-girls, so that fuck-cow's ass could be kept as pregnant as her belly.  Fuck-cow came to love her masters dearly, and with her constantly growing ability to birth imps, she was able to incubate enough troops for Zetaz to challenge Lethice's armies.  The imps never managed to overthrow the old demon lord, but the land was eventually divided in half, split between two growing demonic empires.", false);
	}
	else if(player.gender == 3) {
		//[Epilogue]
		Render.text("The champion was fucked and brainwashed repeatedly for a few more days until Zetaz was sure she understood her place in the world.  Once rendered completely obedient, they released her from her bindings.  It was time she was turned over to Lethice.  ", false);
		if(player.upperBody.wingType != WING.BAT_LIKE_TINY || player.upperBody.wingType != WING.BAT_LIKE_LARGE) Render.text("Zetaz gave her one of the weaker imps to penetrate and taught her to fly with her new, demonic wings.  ", false);
		else Render.text("Zetaz gave her one of the weaker imps to penetrate during the journey.  ", false);
		Render.text("With preparations complete, Zetaz, the champion, and a few dozen imps flew to the mountain peak.\n\n", false);
		
		Render.text("The champion was presented to Lethice, and the demonic mistress was so pleased with Zetaz's gift that she gave him a pair of nubile slave-girls and promoted him over a small army of his own kind.  Once the imps departed, Lethice put the champion through her paces, using her as a fucktoy whenever the mood took her.  The rest of the time the champion was kept bound and unable to orgasm, tortured with unholy levels of arousal, but she didn't mind.  When Lethice allowed her to cum, the champion's orgasms were long and intense enough for her to love her mistress in spite of having to be so pent up.", false);
	}
	else {
		Render.text("The imps never released the champion from that chamber after that.  'He' gave birth to a healthy litter of imps a few weeks later, and the hormones from the pregnancy ", false);
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) Render.text("created a decent set of chest-bumps.", false);
		else Render.text("swelled her already impressive rack with milk.", false);
		Render.text("  After that, the imps really took a liking to her, and she was let down from her restraints.  She never got much chance to get up though; she was well and truly fucked at every opportunity.  She was already hooked.  With her incredible libido and the constant fucking, staying was the easy choice.\n\n", false);
		
		Render.text("After a few months the champion started to become acclimated to her new life, and began birthing imps in larger broods with shorter gestations.  She had become the ideal broodmother, and her worldview shrank down to two powerful priorities: acquiring cum, and birthing.", false);
	}
	player.orgasm();
	player.HP += 150;
	gameOver();
...
public function hermZetazOver():void {
	...
	Render.text("Your willpower starts to come back, and you struggle in vain against the tight leather straps, accomplishing nothing.  Zetaz leers down at your double-sexed form and roughly manhandles both your male and female organs as he taunts, \"<i>I don't remember ", false);
	if(player.lowerBody.cockSpot.count() == 1) Render.text("both", false);
	else Render.text("all", false);
	Render.text(" of these being here when we met.  Did you sample some incubi draft?  Or did you guzzle some succubi milk?  Perhaps both?  In any event, I think you could do with a little more of each.</i>\"\n\n", false);
	
	Render.text("Oh no.  Your eyes widen in fear at his bold declaration, but Zetaz only throws back his head and laughs, \"<i>Oh yes!</i>\"  He turns to the mob and orders something in a tongue you don't understand, then returns to fondling your " + cockDescript(0) + ".  \"<i>How perverse.  Why would you have something like this when you have such a beautiful pussy hiding below it?</i>\" asks the imp lord.  Despite his questioning words, he doesn't stop stroking you until you're full, hard and twitching.  Your poor " + vaginaDescript(0) + " is aching from being ignored with all this building sexual tension.\n\n", false);
	
	Render.text("The sounds of numerous footfalls and clinking glass signal that the mob of imps has returned, bringing what sounds like hundreds of vials worth of their foul concoctions.  Zetaz releases your tumescent member and reaches over for something, then returns to your view bearing a ring gag.  Even turned on, defeated, and immobilized on a table, you try your best to fight him, but all that gets you is slapped.  The imp's palm smacks you hard enough to stun you and leave your ears ringing, and when you blink the stars from your eyes, your mouth is forced open with your tongue hanging out lewdly.\n\n", false);
	
	Render.text("Another of Zetaz's brothers, or perhaps sons, hands him a tube with a funnel, and he easily threads the funnel's tube through the ring gag.  Foul remnants of whatever it was used for last leave a sour taste on your tongue, but worse yet is the knowledge that you're going to be force-fed tainted, body-altering, mind-melting drugs.  A drop of pre-cum hits your belly and your thighs grow ", false);
	if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) Render.text("sticky", false);
	else Render.text("soaked", false);
	Render.text(" from the thoughts.  ", false);
	if(player.stats.cor < 33) Render.text("Are you really being turned on by such lewd, debased thoughts?", false);
	else if(player.stats.cor < 66) Render.text("Are you this much of a pervert?  Yes, it'll feel good, but you're a little ashamed of your body's immediate and lewd reaction.", false);
	else Render.text("Are you really this much of a submissive?  Yeah, sucking down drinks like this is hot as hell, but you'd like to be doing it on your own terms.  At least you'll probably start cumming after a few bottles worth of the stuff.", false);
	Render.text("\n\n", false);
	
	Render.text("\"<i>Hey boss!  She's already starting to drip!  To think she tried to fight us.  She's showing us her true nature – that of a pervert-slut,</i>\" raves one of the horde.  You can't pick out the source of his voice in the crowd, but the words sting enough to make your whole body blush with ", false);
	if(player.stats.cor < 33) Render.text("shame", false);
	else if(player.stats.cor < 66) Render.text("confusion", false);
	else Render.text("arousal", false);
	Render.text(".  The imp lord nods in agreement and upends the first bottle over the funnel, channeling fragrant white fluid into your mouth.  It tastes fantastic!  Your throat instinctively gulps down the creamy delight before you can make a conscious decision.  The effect is immediate and strong.  Warmth builds on your chest as weight is added to your " + allBreastsDescript() + " while a gush of fluid squirts from your " + vaginaDescript(0) + ".\n\n", false);
	
	Render.text("Zetaz is just getting started.  Before you have time to react to your predicament, the next bottle is empty and thicker cream is flooding your mouth.  You don't swallow for a moment, so the imp pours another bottle in, backing up more of the fluid.  Faced with a choice between corruption and drowning, you try to gulp down enough liquid to breathe.  " + player.SMultiCockDesc() + " puffs and swells, spurting thick ropes of cum as it adds a half-dozen inches to its length.  Your eyes cross from the sudden change, but you get a fresh breath before the imps begin to pour several bottles in at once.\n\n", false);
	
	Render.text("You swallow in loud, greedy gulps as your body is slowly warped by the fluids you're consuming.  Though your " + allBreastsDescript() + " and " + player.multiCockDescriptLight() + " sometimes shrink, they grow far more often, and after a few minutes of force-feeding, you're pleading for more each time they stop to let you breath.  You're a mess of sexual fluids, your tits are squirting milk, and your pussy squirts from every touch.  Demon horns are swelling from your brow, curling back over your ears", false);
	if(player.horns > 0) Render.text(" and adding to your existing pair", false);
	else Render.text(" and giving you an exotic, tainted appearance", false);
	Render.text(".  ", false);
	if(player.lowerBody != LOWER_BODY.DEMONIC_HIGH_HEELS) Render.text("Your " + player.feet() + " have been changing throughout the ordeal, but you didn't notice your " + player.legs() + " becoming such lissom, lengthy legs, or your heels growing long, high-heel-like spikes.  ", false);
	if(player.tailType != TAIL.DEMONIC) Render.text("A tail snakes around your leg and begins to caress your " + vaginaDescript(0) + ", then plunges in to fuck the squirting orifice while you drink.  ", false);
	else Render.text("Your tail snakes around your leg and begins to caress your " + vaginaDescript(0) + ", then plunges in to fuck the squirting orifice while you drink.  ", false);
	Render.text("The imps start hooting and cat-calling, laughing and prodding your body with their twisted demonic members as your mind starts to come apart in the seething oven of unnatural lust.\n\n", false);
	//NEXT
	dynStats("lib", 100, "sen", 100, "lus=", 1000, "cor", 50);
...
public function malesZetazOver():void {
	Render.text("You've been so thoroughly ", false);
	if(player.HP < 1) Render.text("beaten", false);
	else Render.text("teased", false);
...
public function malesZetazOverPtII():void {
	Render.text("", true);
	Render.text("You wake to a desert-dry, sandpapery feeling in the back of your throat as yet another moan escapes your mouth.   The ring gag is still there, and easily thwarts your tongues attempts to lick at your parched lips, but the jolts of pleasure exploding up your spine make it hard to get upset about it.  Hips rocking, you keep squirting and squirting from your orgasm, feeling each hot blast burst from your manhood until the wave of lust passes and you open your eyes.  You're in a dim cave, the one they used to hold Vala, and chained up to the wall in a similar manner.\n\n", false);
	
	Render.text("While you observe the room, you realize that the waves of pleasure sliding up your spinal cord haven't stopped, and that your entire body is being shaken rhythmically.  You look down with a look of incredible, still-drugged confusion and behold the last thing you expected to see.  Somehow " + player.sMultiCockDesc() + " has been shrunk to less than half of its previous size", false);
	if(player.lowerBody.balls > 0) Render.text(", and your balls have completely vanished", false);
	Render.text("!  Just below your pint-sized shaft, a massive imp-cock is plowing in and out of your new, wet snatch with juicy-sounding slaps.  Y-you're a hermaphrodite!?  And what's happening to your dick?\n\n", false);
	
	Render.text("A nearby imp with a limp dick and a bored-but-tired look on his face steps up after your orgasm and slathers your dick in some strange, pungent cream, chuckling up at you while he does so, \"Heh heh, your ", false);
	if(player.lowerBody.cockSpot.count() == 1) Render.text("cock's", false);
	else Render.text("cocks're", false);
	Render.text(" gonna be so tiny ", false);
	if(player.lowerBody.cockSpot.count() == 1) Render.text("it", false);
	else Render.text("they", false);
	Render.text("'ll make a baby's look huge.  Boss said we need to dose you with Reducto after each orgasm, so try not to cum too much while we gangbang you, okay?  Oh yeah, I almost forgot, I have to inject something too...</i>\"\n\n", false);
	
	Render.text("The little demon picks an small, glass injector stamped in black ink with the words 'GroPlus'.  Your eyes go wide at sight of the lettering.  As your maleness dwindles, the imp carelessly flicks it to the side and lines the needle's tip up with your tiny bud – they're going to shrink your dick to nothing and pump your clit full of growth chemicals!  He plunges it in, lighting your world up with pain, but the bindings around your body prevent you from escaping or injuring yourself in struggle.  Heat erupts inside your clitty, and it visibly swells up until it nearly reaches the size of your shrinking wang.  Your rapist, or 'sexual partner' with how horny you are, thrusts hard inside you and swells, stroking your walls with the nubby protrusions of a demon's cock.  It feels so good that another orgasm builds on the spot.\n\n", false);
	
	Render.text("With hot, tainted jism filling your womb, your body starts to spasm and squirt, actually making your increasingly tiny dick shake around from the force of ejaculation.  It splatters off the imp's horns and forehead, but he doesn't seem to mind much as he slumps down, dragging his still-rigid member from your cock-hungry fem-sex.  You moan wantonly, still spurting as the imp 'medic' applies another layer of Reducto to " + player.sMultiCockDesc() + ", rapidly shortening ", false);
	if(player.lowerBody.cockSpot.count() == 1) Render.text("it until it's", false);
	else Render.text("them until they're", false);
	Render.text(" barely three inches long, even while hard.  He pulls out another plunger and rams the needle into your still-aching clit, making it swell until it's almost five inches long and trembling like your manhood used to.\n\n", false);
	
	Render.text("\"<i>Now you're starting to look like a proper bitch.  ", false);
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 2) Render.text("It doesn't look right without a decent rack, but boss said no tits for the new breeding bitch.  Sure makes it hard to get excited about fucking that new twat of yours though...", false);
	else Render.text("With a rack like that and a nice, wet cunt, you'll have the other guys lining up for their turn in no time...", false);
	Render.text("</i>\" rambles one of the imps.  You groan and shake your hips lewdly, still turned on after all the fucking, feeling empty without the unholy heat of an imp inside you.  A hunger buzzes away in your womb, demanding you get pregnant, and you're thrilled to see Zetaz stride in with a raging, fully erect stiffy.  It throbs hungrily as he smiles up at you and climbs atop the conveniently positioned platform.\n\n", false);
...
private function incubusDeal():void {
	spriteSelect(52);
	clearOutput();
	Render.text("\"<i>Excellent!  Give me a few moments to gather my things and I'll be open for business!</i>\" exclaims the strange demon.  If his story is true it's no wonder he doesn't get along with the rest of his kind.");
	
	//[Next – to room]
	flags[FlagEnum.ZETAZ_LAIR_DEMON_VENDOR_PRESENT] = 1;
	doNext(playerMenu);
}
...
private function incubusNoDeal():void {
	...
	clearOutput();
	flags[FlagEnum.ZETAZ_LAIR_DEMON_VENDOR_PRESENT] = -1;
	Render.text("Sean nods, grabs a pack, and takes off running before you have a chance to kill him.");
	doNext(playerMenu);
}

public function incubusShop():void {
	spriteSelect(52);
	if (flags[FlagEnum.NIAMH_SEAN_BREW_BIMBO_LIQUEUR_COUNTER] == 1) {
		telAdre.niamh.getBimboozeFromSean();
		return;
	}
	clearOutput();
	Render.text("Sean nods at you and slicks his hair back into place, threading it carefully around the small nubs of his horns before asking, \"<i>What can I do for you?</i>\"");
	menu();
...
private function incubusBuy(itype:ItemType):void {
	spriteSelect(52);
	clearOutput();
	Render.text("The incubus lifts " + itype.longName + " from his shelves and says, \"<i>That will be " + (itype.value * 3) + " gems.  Are you sure you want to buy it?</i>\"");
	if(player.stats.gems < (itype.value * 3)) {
...
private function incubusTransact(itype:ItemType):void {
	spriteSelect(52);
	clearOutput();
	player.stats.gems -= itype.value * 3;
	statScreenRefresh();
...
public function valaCumBath2():void {
	...
	if(player.lowerBody.balls > 0) player.lowerBody.ballSize += .3;
	flags[FlagEnum.VALA_CUMBATH_TIMES]++;
	flags[FlagEnum.VALA_TIMES_CONSENSUAL_SEX]++;
	player.orgasm();
	dynStats("lib", -1.5);
	doNext(camp.returnToCampUseOneHour);
...
public function valaBigYou():void {
	clearOutput();
	flags[FlagEnum.TIMES_VALA_CONSENSUAL_BIG]++;
	flags[FlagEnum.VALA_TIMES_CONSENSUAL_SEX]++;
	//{FIRST TIME}
	if(flags[FlagEnum.TIMES_VALA_CONSENSUAL_BIG] == 1) {
		Render.text("Tilting your head, eyes glowing with ethereal power, you answer, \"<i>I want you... big.</i>\"");
	...
	//{REPEAT}
		...
		Render.text("You tell Vala just how HUGE your need for her is, which earns you a blush from the petite faerie's fair cheeks.  She calls in a favor from another waitress - looks like Vala covered for this one's tryst with Chad as well.  That Chad guy must get around!");
		...
		if(silly()) Render.text("The last thing you need is a bunch of winged archers taking potshots at Vala while she clings to a tower with you in her hand...  ");
		Render.text("She seems to know the way directly to your camp, and all too soon, you've reached your place of residence.  Vala cheekily pokes your [chest] and asks, \"<i>Shouldra?</i>\"");
	}
	//{MERGE}
	...
	if(player.lowerBody.cockSpot.hasCock()) addButton(1,"Lick Me",bigValaLicksOffDudes);
	if(vapula.vapulaSlave() && player.gender > 0 && (player.lowerBody.cockSpot.hasCock() || (player.lowerBody.vaginaSpot.hasVagina() && player.hasKeyItem("Demonic Strap-On") >= 0))) {
...
public function bigValaDomsPC():void {
	clearOutput();
	Render.text("Vala tilts her head to the side, causing a gust of displaced air to blow your " + hairDescript() + " wildly.  \"<i>Giving the maiden a chance to lead?  You're so sweet!</i>\"  She tickles your chin with one immense fingertip.  \"<i>But you had better give it your all, you adorable rascal, you.</i>\"");
	...
	if (flags[FlagEnum.VALA_HEALED_HONEY] == 0) Render.text("\"<i>If you do a good job, I'll let you taste a pure pearl of my own,</i>\" she giggles.");
	
	//{Male:}
	if(player.gender == 1) {
		Render.text("\n\nShuddering from the onslaught of female pheromones, you find your body responding without meaning to.  [EachCock]");
		if(player.lust >= 70) Render.text(", already hard, begins to bead with pre-cum, mixing with the slut-leakings that have found their way under your equipment.");
		else Render.text(" stiffens immediately to a full, throbbing hardness.");
		Render.text("  Your body is imminently ready for breeding, erect and wanting so badly to be freed from your constricting garb, but you know that with what you asked, you won't get to sate it.");
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 150) Render.text("  If only you weren't so big - you can't expand to your full size, and the tightness is almost emasculating at how it constrains your size, keeping you small enough to fit inside your [armor] but still hard as a stone, only this stone is having the pre squeezed out of it.");
	}
	//{Female:}
	else if(player.gender == 2) {
		...
		if(player.wetness() >= 3) Render.text("Your [vagina] seems to always be wet, but now, with this sight before you, your girl-cum is running in rivers down your [legs] to mix into the puddling lady-spunk below.  ");
		Render.text("Your armor seems tight and constricting against your rock-hard [nipples].  It's something you can't escape from, and with each passing moment, the sensations coming from your chest seem more and more worrisome, stoking your fiery lust to an incredible degree.");
	}
	//{Herm:}
	else if(player.gender == 3) {
		Render.text("\n\nShuddering from the onslaught of lusty pheromones, you find [eachCock] responding even before you realize what's going on.  ");
		if(player.lust >= 70) Render.text("You may have already been uncomfortably hard, but you start leaking pre-cum all over the inside of your [armor].  Of course, the copious fem-jizz that's already seeped inside your gear absorbs the extra fluid with little to no complaint.");
		else Render.text("You get hard with alarming rapidity.  Being so close to such a gloriously large quim seems to overpower your better sense, and you're forced to give in to your body's imperative.");
		Render.text("  Inside your undergarments, everything is already turning into a swampy, slimy mess thanks to Vala's secretions, and your own feminine drippings don't help any.  Your lips and dick");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" are smothered with the stuff, making you moan without meaning to.");
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 150) Render.text("  If only you weren't so big!  You're compressed so tightly by your enclosing [armor], and even though you're rock-hard, you're a fraction of your full size.  If only you could get out of your gear and fuck her!");
	}
	//{Genderless}
	else {
...
public function bigValaLicksOffDudes():void {
	clearOutput();
	Render.text("Giant Vala cocks an eyebrow at your request in surprise.  \"<i>But, what if I accidentally swallow?  I couldn't live with myself if something should happen to my big... er, little Hero.</i>\"  She pats you on the head affectionately, unsure of how to handle herself with all of her newfound size.");
	...
	if(player.upperBody.chest.hasFuckableNipples()) Render.text("  It even pauses at one to slip a tiny bit inside, too big to penetrate your nipplecunt properly but teasing it all the same.");
	Render.text("  Finally, it encircles the back of your neck and curls up to slide across your chin.  You greedily slurp the tip of her oral muscle into your own mouth, and the effect is immediate.");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("each");
	else Render.text("your");
	Render.text(" length, unholy smoothness sliding ever so slowly back and forth as she moans.  The giant faerie drags you further inside, much to your delight.  Agonizingly sensuous pressure mounts all along your length");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" until ");
	if(player.lowerBody.cockSpot.count() == 1) Render.text("it slides");
	else Render.text("they slide");
	Render.text(" inside her");
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 200) Render.text(", the immense length sucked in by a surprising burst of suction");
	Render.text(".");
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() < 200) Render.text("smothering your relatively tiny pecker");
	else Render.text("wrapping up your immense pecker");
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 400) Render.text(", even as it hangs partway down her throat");
	Render.text(", you begin to moan as well, subsumed in steamy pleasure.");
	...
	if(player.lowerBody.balls > 0) Render.text(", [balls] clenching against Vala's tongue");
	Render.text(" as you explode into ecstatic, jerking pumps against her slippery, sensitive tongue.  Vala's voice rises with yours the moment the first spurt of seed stains her tongue, and her fem-cum sprays out to soak her thighs with enough force to carry some smaller boulders away in the tide.");
	if(player.cumQ() < 250) Render.text("  She barely notices the spunk you shoot against her tongue, her drooling saliva washing it away almost immediately, but you're sure she got a nice, salty thrill from it regardless.");
	else if(player.cumQ() < 1000) Render.text("  She has to noisily gulp down the liquid deposit you leave in her mouth, but with her enhanced size, it's still no more than a creamy morsel.");
	else if(player.cumQ() < 4000) Render.text("  You slowly flood her mouth with spooge until her cheeks bulge.  The spooge inundates her maw until your lower body floats in it, forcing the girl to swallow hard or risk having the teeming seed spill out around you.");
	else Render.text("  You immediately flood her mouth, bulging her cheeks with obscene amounts of spunk.  She noisily gulps, mouth working futilely to handle it all, but it's not enough, and jizz begins to burst from the corners of her mouth, almost pushing you out with it.");
	Render.text("  Her tongue grinds back and forth underneath you, encouraging you to deposit every single drop into her.");
...
public function valaVapulaThreesome():void {
	...
	//{Repeat}e
		...
		Render.text("You call for Vapula, which brings an amused smile to your giant faerie's face.");
...
public function valaDommyVapula3Some():void {
	...
	if(!player.lowerBody.cockSpot.hasCock()) Render.text("dildo");
	else Render.text("[cock smallest]");
	Render.text(" slides into the demoness's waiting snatch, which slurps it down with a hungry contraction, slapping your [hips] against hers, the slippery, demonic folds wriggling around you with such insistence that you couldn't pull out if you want to.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("seed");
	else Render.text("fake seed");
	Render.text(" without meaning to, the lustiness of the situation just far beyond your ability to bear.");
	if(player.cumQ() > 500) Render.text("  The demon's purple belly balloons with cum, swelling against your own");
	if(player.cumQ() >= 1500) Render.text(" as her midriff turns taut and dome-like");
	if(player.cumQ() > 500) Render.text(".");
	if(player.cumQ() > 1000) Render.text("  Waves of spunk pour out from Vapula's lust-swollen lips thanks to your exceedingly potent orgasm.");
	if(player.lactationQ() >= 100) {
		Render.text("  Even better, your [fullChest] lets down your milk, letting it run out of you in ");
		if(player.lactationQ() <= 200) Render.text("thick");
		else if(player.lactationQ() <= 2000) Render.text("powerful");
		else Render.text("enormous");
		Render.text(" streams, tinting Vala's secretions white.");
	}
...
public function valaLoveyVapula3Some():void {
	...
	if(!player.lowerBody.cockSpot.hasCock()) Render.text(", and you buckle on your demonic strap-on, just in case");
	Render.text(".  Holding your arms out to either side, you let Vala's cunt-soaked hand come out to lift you, a finger under each armpit and one between your legs, slickly sliding against the sensitive, hard prong there.");
	Render.text("\n\nYou're lifted up high, which only serves to slide ");
	if(!player.lowerBody.cockSpot.hasCock()) Render.text("the dildo");
	else Render.text("[oneCock]");
	Render.text(" even harder into your flawless faerie-skin, you moan out loud, matching Vapula's lascivious cries as she's mercilessly teased.  Your [feet] are brought up against Vala's immense lip, quickly sliding over the oral protrusion and into the steamy, warm cavern behind it.  The demon's shoulder brushes against your own, enough to rouse her from her sexual stupor and have her turn her head to face you.  Mischief twinkles in her lust-lidded eyes as she asks, \"<i>Oh, are you going to feed me, [Master]?</i>\"");
	...
	if(!player.lowerBody.cockSpot.hasCock()) Render.text("that exotic dildo");
	else Render.text("your [cock smallest]");
	Render.text(".  You grab hold of her ass and the tongue that's slipping through her crack and hammer your hips home, slapping against Vapula so hard that the violet woman's cries of bliss are interrupted by the breath-stealing impacts. ");
	...
	if(!player.lowerBody.cockSpot.hasCock()) Render.text("spurting dildo");
	else Render.text("[cock smallest]");
	Render.text(", ensuring that every single drop is passed into her ravenous gash.");
...
public function kinathisValaStuff():void {
	...
	Render.text("With it so late at night, there are barely any patrons at the bar.  It's likely the bar will close up soon.  Lounging in one of the empty, comfortable booths off to the side, you wave over the short, busty bar-fairy.  Vala flutters over to you, a frothy mug in hand, smiling brightly at the sight of you.  The lovely fairy girl always seems happy to see you; rescuing her from the caves really did endear you to her a great deal.");
	...
	if(player.tallness > 75) Render.text("  She looks up at you as you seem to tower over her, a");
	else Render.text("  A");
	Render.text(" faint blush on her cheeks as she smiles and hugs your arm, beckoning you to follow.");
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) Render.text("Her tongue licks around the throbbing glands, the heady animal musk filling her nose and mind with sweet lust.  ");
	else if(player.lowerBody.cockSpot.list[x].cockType == CockType.DOG) Render.text("Her tongue licks around the hot building knot that gently throbs at your base.  ");
	//[anemone cock:
	else if(player.lowerBody.cockSpot.list[x].cockType == CockType.ANEMONE) Render.text("Her tongue plays with the squirming tentacles that line the drone of your throbbing phallus.  She shivers and groans as the tentacles sting her full of pleasurable aphrodisiac.  ");
	Render.text("Playing her way down your cock, the fey-girl gently licks up and down, sucking along your cumvein before coming back to the " + player.cockHead(x) + " of your meaty prick.");
	...
	//[cockarea <17:
	if(player.cockArea(x) < 17) Render.text("Despite her size, you slide in easily, her wet, slippery pussy taking you in and squeezing you in a velvety embrace.  \"<i>Yessss, it's in, it's in!  Mmmm, come to me hero, come to Vala,</i>\" she croons to you, beckoning  you to take her.");
	else if(player.cockArea(x) < 30) Render.text("\"<i>Ahhh...  yesss, it's in!  Ohh...  come to me hero, come to Vala and fill her with your love!</i>\" she croons to you, beckoning  you to take her.");
	else Render.text("Ohhh! Ahhh...  Y-Yessss...  Vala’s big strong hero, oh you feel so good inside me!</i>\" she moans and groans as you stretch her tight little fairy cunt with your pulsing prick.  Her stretchy flesh distends as more and more of your cock is fed into her hungry pussy.");
	...
	if(player.cockArea(x) > 17) Render.text("  Her hot little body stretches around your girthy prick, distending her belly as you sheath your throbbing arousal into her.");
	...
	if(player.lowerBody.balls > 0) {
		//if ballsize <3:
		if(player.lowerBody.ballSize < 3) Render.text("  With each hard quaking thrust your balls slap against Vala’s expansive ass.  Your nuts ache under their pent up load and demand you to shoot it into the sweet fey-girl soon.");
		else if(player.lowerBody.ballSize < 6) Render.text("  With each hard quaking thrust, your balls smack against Vala’s cushiony ass.  Your hefty, cum-bloated sack, so full, aches for the sweet release into your tender fairy lover.");
		else Render.text("  With each hard quaking thrust, your fat balls slam against Vala’s pale squeezable ass.  The huge, virile spunk-factories go into overtime, their already taut skin growing tighter as they swell and bloat with your pent up load.  Your body's natural instincts demand you do your duty as the breeder you are and pack your small lover’s womb full of your spunk until her belly is swollen and seeded with your young.");
	}
