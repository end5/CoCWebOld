...
public function imtacularMultiHitzilla():void {
	...
	while(hits > 0) {
		...
		//Determine if dodged!
			...
			if(player.stats.spe - monster.spe < 8) outputText("You narrowly avoid " + monster.a + monster.short + "'s " + monster.weaponVerb + "!\n", false);
			else if(player.stats.spe - monster.spe >= 8 && player.stats.spe-monster.spe < 20) outputText("You dodge " + monster.a + monster.short + "'s " + monster.weaponVerb + " with superior quickness!\n", false);
			else if(player.stats.spe - monster.spe >= 20) outputText("You deftly avoid " + monster.a + monster.short + "'s slow " + monster.weaponVerb + ".\n", false);
		}
		//Determine if evaded
...
public function impGangUber():void {
	if(monster.findStatusAffect(StatusAffects.ImpUber) < 0) {
		...
		if(player.lowerBody.cockSpot.count() > 0) {
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) outputText(", and in addition, the clothes around your groin ", false);
		}
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			outputText("become stained with feminine moisture", false);
			if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) outputText(" as your clit swells up in a more sensitive imitation of a cock", false);
		}
...
public function loseToImpMob():void {
	...
	if(player.upperBody.chest.hasFuckableNipples()) {
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 7) outputText("  He can only get partway in, but it doesn't seem to deter him.", false);
		...
		if(player.upperBody.chest.countNipples() > 2) outputText("each of ", false);
		outputText("your other " + nippleDescript(0), false);
		if(player.upperBody.chest.countNipples() > 2) outputText("s", false);
		outputText(", fighting over the opening", false);
		if(player.upperBody.chest.countNipples() > 2) outputText("s", false);
		outputText(".  A victor quickly emerges, and in no time ", false);
		if(player.upperBody.chest.countNipples() == 2) outputText("both", false);
	...
	if(player.lowerBody.vaginaSpot.hasVagina()) outputText("A thick load bastes your pussy with whiteness, and you can feel it seeping deeper inside your fertile womb.  ", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) {
		...
		if(player.upperBody.chest.hasFuckableNipples()) {
			if(player.upperBody.chest.countNipples() == 2) outputText("  The pair", false);
	...
	if(player.lowerBody.cockSpot.count() == 0) outputText("and you clench tightly around the uncomfortable intrusion.", false);
	else {
	...
	if(!player.lowerBody.vaginaSpot.hasVagina()) outputText(" and forced to start drinking bottle after bottle of succubi milk", false);
...
public function loseToImpMobII():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("new", false);
...
public function impGangGetsRapedByMale():void {
	...
	if(player.lowerBody.balls == 0) outputText(" and plunging deep inside", false);
	outputText(".  The imp to the right wraps his tongue around the base ", false);
	if(player.lowerBody.cockSpot.hasSheath()) outputText("just above your sheath ", false);
	...
	if(player.lowerBody.balls > 0) outputText("Cum boils in your balls, ready to paint your foe white.  ", false);
...
public function impGangGetsRapedByFemale():void {
	...
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLICK) outputText("and coating it with feminine drool ", false);
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) outputText("You stroke the cock-like appendage in your hand, trembling with delight.  ", false);
	...
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLAVERING) outputText("Cunt juices splatter him as you squirt explosively, leaving a puddle underneath him.  ", false);
...
public function getSwordAndGetTrapped():void {
	...
	if(player.stats.spe < 50) outputText("fall flat on your " + assDescript() + ", slipping on the slick, shifting surface.", false);
...
public function encapsulationPodAI():void {
	...
	if(monster.findStatusAffect(StatusAffects.Round) < 0) {
		...
		else if(player.lowerBody.balls == 0) outputText("taint ", false);
	...
	//[Round 2 Action]
		...
		if(player.lowerBody.balls > 0) outputText("One of them wraps itself around the top of your " + sackDescript() + " while its tip slithers over your " + ballsDescriptLight() + ".  Another ", false);
		...
		if(player.lowerBody.cockSpot.count() > 0) {
			...
			if(!player.lowerBody.cockSpot.hasSheath()) outputText("base", false);
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) {
				...
				if(player.lowerBody.vaginaSpot.list[0].clitLength > 4) outputText("It brushes your " + clitDescript() + " then curls around it, squeezing and gently caressing it with a slow, pleasing rhythm.  ", false);
			}
			else {
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			...
			if(player.lowerBody.cockSpot.count() == 2) outputText(cockDescript(1) + " gets the same treatment, and soon both of your " + player.multiCockDescriptLight() + " are quite happy to be here.  ", false);
		...
		if(player.lowerBody.vaginaSpot.hasVagina()) {
			...
			if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) outputText("and once it discovers how large it is, it wraps around it and squeezes.  It feels good!  ", false);
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("your chest", false);
	...
	//[Round 3 Action]
		...
		if(player.upperBody.chest.hasFuckableNipples()) outputText("  Proof of your arousal leaks from each " + nippleDescript(0) + " as their entrances part for the probing tentacles.  They happily dive inside to begin fucking your breasts, doubling your pleasure.", false);
...
public function loseToThisShitPartII():void {
	...
	if(player.gender == 1 || (player.gender == 3 && rand(2) == 0)) {
		...
		outputText(" and squeeze, forcing " + player.sMultiCockDesc() + " to inflate to readiness.  Deep inside your " + assholeDescript() + ", the tentacle starts to rub against your prostate.  It caresses the male organ on each side and pauses to squeeze the center of it, pushing a few drops of sticky cum from your trembling " + Appearance.cockNoun(CockType.HUMAN) + ".\n\n", false);
...
public function useValaPtII():void {
	...
	//HERPY DERPY HERMY
		...
		if(player.lowerBody == LOWER_BODY.NAGA) outputText("tail", false);
...
public function valaGetsSexedPtDuece():void {
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		...
		//(large girth dicks)
			...
			if(player.lowerBody.vaginaSpot.hasVagina()) outputText("nuzzling your joy buzzer", false);
...
public function rejectFuckingVala():void {
	...
	if(flags[FlagEnum.TIMES_PC_DEFEATED_VALA] == 0) {
		...
		if(flags[FlagEnum.TIMES_PC_DEFEATED_VALA_AND_RAEPED] > 0) outputText("  You smirk in wonder at the horny little slut.  You guess you'll have to put her back into her place and give her another dose of 'love'.", false);
		outputText("\n\n", false);
...
public function valaFightVictoryFuck():void {
	...
	if(player.gender == 2) {
		...
		//(PC Has Wings: 
		if(player.upperBody.wingType > WING.NONE) outputText("You decide to give her a hand, and flap your " + player.upperBody.wingDesc + " as hard as you can, the added thrust giving her the start she needs.  ", false);
...
public function valaFightVictoryFuckPtII():void {
	...
	//herm2
		...
		//(2 Dicks-  
		if(player.lowerBody.cockSpot.count() > 1) outputText("Your " + cockDescript(0) + " inside the fairy girl's ass erupts first, pumping your seed into her bowls, her hips distorting with the bursting flow of your cum, sphincter's clenching powerless to stop your alabaster flood.  ", false);
...
public function wakeMeUpBeforeValaGoGosPtII():void {
	...
	//h2
		...
		//(2 Dicks- 
		if(player.lowerBody.cockSpot.count() > 1) outputText("The " + cockDescript(1) + " inside the fairy girl's ass erupts first, pumping your seed into her bowels, her hips distorting with the bursting flow of your cum, her sphincters clenching powerless to stop your alabaster flood.  ", false);
...
public function faerieOrgyFuckMaleContinue():void {
	...
	//[Balls- 
	if(player.lowerBody.balls > 0) outputText("Vala's tongue caresses your " + ballsDescriptLight() + ", slurping hungrily, eager to get every drop out of your overburdened sac.  ", false);
...
public function faerieOrgyFuckFemaleContinue():void {
	...
	//[If the Player has unlocked Scylla's Addiction Counseling event] 
	if(flags[FlagEnum.TIMES_SCYLLA_ADDICT_GROUP_EXPLOITED] + flags[FlagEnum.SCYLLA_TIMES_SHARED_IN_ADDICT_GROUP] + flags[FlagEnum.SCYLLA_MILK_THERAPY_TIMES] + flags[FlagEnum.SCYLLA_CUM_THERAPY_TIMES] + flags[FlagEnum.SCYLLA_SEX_THERAPY_TIMES] > 0) outputText("Another fairy flies toward Vala and you blink through the building lusty haze when you recognize the little girl. Wearing a slightly slutty crystalline gown of translucent gossamer, you recognize Pastie, the fairy from Scylla's addiction counseling meeting. When you call out to her, she bashfully waves back. \"<i>Hey, uh, " + player.short + ". It's kinda weird seeing you outside of the meetings. How are you? How have things been?</i>\" She looks at your naked, reclined form on the bed and blushes. \"<i>Oh, right, right. Um. I'm just helping out Vala, you know. Sisters and all. Uh. I'm... I'm going to just give her this petal and get something to drink, okay? All this sobriety is making this really uncomfortable for me.</i>\" The other fairies laugh and Pastie hands off her blossom before zipping out of the room, probably to steal someone's drink downstairs.\n\n", false);
...
public function zetazTaunt():void {
	if(monster.findStatusAffect(StatusAffects.round) < 0) {
		...
		if(monster.statusAffects.get("round").value1 == 2) outputText("\"<i>I lost my post!  And when you screwed up the factory?  I barely escaped with my life!  You ruined EVERYTHING!</i>\" screams Zetaz.", false);
		else if(monster.statusAffects.get("round").value1 == 3) outputText("Zetaz snarls, \"<i>Do you know how hard it is to hide from Lethice?  DO YOU HAVE ANY IDEA!?  I've had to live in this fetid excuse for a jungle, and just when I found some friends and made it livable, you show up and DESTROY EVERYTHING!</i>\"", false);
		else if(monster.statusAffects.get("round").value1 == 4) outputText("Zetaz explains, \"<i>I won't let you go.  I'm going to break you.</i>\"", false);
		else if(monster.statusAffects.get("round").value1 == 5) outputText("\"<i>Would it have been that bad to go along with me?  You've seen the factory.  We would've kept you fed, warm, and provided you with limitless pleasure.  You would've tasted heaven and served a greater purpose.  It's not too late.  If you come willingly I can make sure they find a good machine to milk you with,</i>\" offers the imp lord.", false);
		else if(monster.statusAffects.get("round").value1 == 6) outputText("\"<i>Why won't you fall?</i>\" questions Zetaz incredulously.", false);
		else if(monster.statusAffects.get("round").value1 == 7) outputText("The imp lord suggests, \"<i>If you give up and let me fuck your ass maybe I'll let you go.</i>\"", false);
		else if(monster.statusAffects.get("round").value1 == 8) outputText("Zetaz pants, \"<i>Just give up!  I'm nothing like the weakling you met so long ago!  I've been through hell to get here and I'm not giving it up just because you've shown up to destroy my hard work!</i>\"", false);
...
public function gigaArouse():void {
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		if(player.lust >= 60 && player.lowerBody.cockSpot.count() > 0) outputText("You feel " + player.sMultiCockDesc() + " dribble pre-cum, bouncing with each beat of your heart and aching to be touched.  ", false);
		if(player.lust >= 30 && player.lust < 60 && player.lowerBody.cockSpot.count() == 1) outputText(player.SMultiCockDesc() + " hardens and twitches, distracting you further.  ", false);
	}
	if(player.lowerBody.vaginaSpot.count() > 0) {
		if(player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.NORMAL && player.lowerBody.vaginaSpot.count() == 1) outputText("Your " + allVaginaDescript() + " dampens perceptibly, feeling very empty.  ", false);
		if(player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.WET && player.lowerBody.vaginaSpot.count() > 0) outputText("Your crotch becomes sticky with girl-lust, making it clear to " + monster.a + monster.short + " just how welcome your body finds the spell.  ", false);
		if(player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLICK && player.lowerBody.vaginaSpot.count() == 1) outputText("Your " + allVaginaDescript() + " becomes sloppy and wet, dribbling with desire to be mounted and fucked.  ", false);
		if(player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.DROOLING && player.lowerBody.vaginaSpot.count() > 0) outputText("Thick runners of girl-lube stream down the insides of your thighs as your crotch gives into the demonic magics.  You wonder what " + monster.a + monster.short + "'s cock would feel like inside you?  ", false);
		if(player.lust >= 60 && player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING && player.lowerBody.vaginaSpot.count() == 1) outputText("Your " + allVaginaDescript() + " instantly soaks your groin with the heady proof of your need.  You wonder just how slippery you could " + monster.a + monster.short + "'s dick when it's rammed inside you?  ", false);
	}
...
public function defeatZetaz():void {
	...
	//( or take her place)
	if(player.stats.cor > 66) outputText(" or take her place", false);
...
private function sexualTortureReleaseZetaz():void {
	...
	if(player.lust > 60 || player.stats.lib > 60 || player.stats.cor > 60) outputText(", or perhaps perversion,", false);
...
public function femaleZetazOver():void {
	...
	if(player.stats.cor < 33) outputText("shame", false);
	else if(player.stats.cor < 66) outputText("confuse", false);
	...
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) outputText("even more wet ", false);
	else outputText("a little wet ", false);
	outputText("from the masculine scent the aroused penises are producing.  ", false);
	if(player.stats.cor < 33) outputText("How could you be turned on by such a repulsive situation?  You're going to be raped, brainwashed, and either kept as a pet or tossed in a milking tube for the rest of your life and your body is acting like some horny slut!", false);
	else if(player.stats.cor < 66) outputText("You marvel at just how turned on you're getting from the strange situation.  You know you'll be raped, drugged, and used as a toy or milk cow, but your loins are thrumming with warm, wet desire.", false);
	...
	if(flags[FlagEnum.NUMBER_OF_TIMES_MET_SCYLLA] > 0) outputText("Unbidden, Scylla's face comes to mind, and you realize the drugs coursing through your veins must be doing something similar to you!  Her visage changes to your own, though the thick, cock-sucking lips remain behind, eager to be penetrated.", false);
	...
	if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.DROOLING) outputText("gushes fresh fluids into a puddle on the table", false);
	else if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.WET) outputText("drools a heavy flow of liquid arousal onto the hardwood table", false);
...
public function femaleZetazOverPtII():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("tweak your " + nippleDescript(0) + "s roughly, pulling and yanking on them as", false);
...
public function zetazBadEndEpilogue():void {
	...
	if(player.gender == 2) {
		...
		if(player.upperBody.wingType != WING.BAT_LIKE_TINY || player.upperBody.wingType != WING.BAT_LIKE_LARGE) outputText("Zetaz gave her one of the weaker imps to penetrate and taught her to fly with her new, demonic wings.  ", false);
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 1) outputText("created a decent set of chest-bumps.", false);
...
public function hermZetazOver():void {
	...
	if(player.lowerBody.cockSpot.count() == 1) outputText("both", false);
	...
	if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.DROOLING) outputText("sticky", false);
	else outputText("soaked", false);
	outputText(" from the thoughts.  ", false);
	if(player.stats.cor < 33) outputText("Are you really being turned on by such lewd, debased thoughts?", false);
	else if(player.stats.cor < 66) outputText("Are you this much of a pervert?  Yes, it'll feel good, but you're a little ashamed of your body's immediate and lewd reaction.", false);
	...
	if(player.stats.cor < 33) outputText("shame", false);
	else if(player.stats.cor < 66) outputText("confusion", false);
	...
	outputText(".  ", false);
	if(player.lowerBody != LOWER_BODY.DEMONIC_HIGH_HEELS) outputText("Your " + player.feet() + " have been changing throughout the ordeal, but you didn't notice your " + player.legs() + " becoming such lissom, lengthy legs, or your heels growing long, high-heel-like spikes.  ", false);
	if(player.tailType != TAIL.DEMONIC) outputText("A tail snakes around your leg and begins to caress your " + vaginaDescript(0) + ", then plunges in to fuck the squirting orifice while you drink.  ", false);
...
public function hermZetazOverPtII():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("number of ", false);
	...
	if(player.lowerBody.balls > 0) outputText("gently shifts your " + ballsDescript() + " out of the way and ", false);
...
public function malesZetazOverPtII():void {
	...
	if(player.lowerBody.balls > 0) outputText(", and your balls have completely vanished", false);
	...
	if(player.lowerBody.cockSpot.count() == 1) outputText("cock's", false);
	...
	if(player.lowerBody.cockSpot.count() == 1) outputText("it until it's", false);
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 2) outputText("It doesn't look right without a decent rack, but boss said no tits for the new breeding bitch.  Sure makes it hard to get excited about fucking that new twat of yours though...", false);
...
public function valaCumBath():void {
	...
	if(flags[FlagEnum.VALA_CUMBATH_TIMES] == 0) {
		...
		if(player.cumQ() < 2000) {
			...
			//IF BALLS: 
			if(player.lowerBody.balls > 0) outputText("  Your balls react immediately, filling with a cold weight that nearly knocks you to your ass.  Your [armor] bulges obscenely as the sack hanging between your legs fills with liquid pressure that squeezes your bloating flesh.");
...
public function bigValaDomsPC():void {
	...
	if (flags[FlagEnum.VALA_HEALED_HONEY] == 0) outputText("\"<i>If you do a good job, I'll let you taste a pure pearl of my own,</i>\" she giggles.");
	...
	//{Herm:}
		...
		if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 150) outputText("  If only you weren't so big!  You're compressed so tightly by your enclosing [armor], and even though you're rock-hard, you're a fraction of your full size.  If only you could get out of your gear and fuck her!");
	}
	//{Genderless}
	else {
...
public function bigValaLicksOffDudes():void {
	...
	if(player.upperBody.chest.hasFuckableNipples()) outputText("  It even pauses at one to slip a tiny bit inside, too big to penetrate your nipplecunt properly but teasing it all the same.");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("each");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("s");
	outputText(" until ");
	if(player.lowerBody.cockSpot.count() == 1) outputText("it slides");
	else outputText("they slide");
	outputText(" inside her");
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 200) outputText(", the immense length sucked in by a surprising burst of suction");
	outputText(".");
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() < 200) outputText("smothering your relatively tiny pecker");
	else outputText("wrapping up your immense pecker");
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 400) outputText(", even as it hangs partway down her throat");
	...
	if(player.lowerBody.cockSpot.biggestCocks[0].cockArea() >= 400) outputText(", cock partway down her throat");
	...
	if(player.lowerBody.balls > 0) outputText(", [balls] clenching against Vala's tongue");
...
public function valaDommyVapula3Some():void {
	...
	if(!player.lowerBody.cockSpot.hasCock()) outputText("buckling on your demonic strap-on");
	...
	if(!player.lowerBody.cockSpot.hasCock()) outputText("dildo");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("seed");
...
public function valaLoveyVapula3Some():void {
	...
	if(!player.lowerBody.cockSpot.hasCock()) outputText(", and you buckle on your demonic strap-on, just in case");
	...
	if(!player.lowerBody.cockSpot.hasCock()) outputText("the dildo");
	...
	if(!player.lowerBody.cockSpot.hasCock()) outputText("that exotic dildo");
	...
	if(!player.lowerBody.cockSpot.hasCock()) outputText("spurting dildo");
	else outputText("[cock smallest]");
...
public function kinathisValaStuff():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("most appealing phallus");
	...
	if(player.lowerBody.cockSpot.list[x].cockType == CockType.HORSE) outputText("Her tongue licks around the throbbing glands, the heady animal musk filling her nose and mind with sweet lust.  ");
	else if(player.lowerBody.cockSpot.list[x].cockType == CockType.DOG) outputText("Her tongue licks around the hot building knot that gently throbs at your base.  ");
	//[anemone cock:
	else if(player.lowerBody.cockSpot.list[x].cockType == CockType.ANEMONE) outputText("Her tongue plays with the squirming tentacles that line the drone of your throbbing phallus.  She shivers and groans as the tentacles sting her full of pleasurable aphrodisiac.  ");
	...
	if(player.lowerBody.balls > 0) {
		//if ballsize <3:
		if(player.lowerBody.ballSize < 3) outputText("  With each hard quaking thrust your balls slap against Vala’s expansive ass.  Your nuts ache under their pent up load and demand you to shoot it into the sweet fey-girl soon.");
		else if(player.lowerBody.ballSize < 6) outputText("  With each hard quaking thrust, your balls smack against Vala’s cushiony ass.  Your hefty, cum-bloated sack, so full, aches for the sweet release into your tender fairy lover.");
