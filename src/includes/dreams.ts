...
public dreamSelect():boolean {
	...
	dreamtemp = rand(player.stats.lib/10 + player.stats.cor/20) + player.stats.cor/20;
	...
	if(player.minotaurAddicted()) {
		...
		choices[choices.length] = 6;
		if(player.perks.has("MinotaurCumAddict")) choices[choices.length] = 6;
	}
	//Akbal
	if(flags[FlagEnum.AKBAL_SUBMISSION_COUNTER] > 4) {
		if(flags[FlagEnum.AKBAL_SUBMISSION_COUNTER] > 10) choices[choices.length] = 7;
		if(flags[FlagEnum.AKBAL_SUBMISSION_COUNTER] > 13) choices[choices.length] = 7;
		if(flags[FlagEnum.AKBAL_SUBMISSION_COUNTER] > 15) choices[choices.length] = 7;
		if(flags[FlagEnum.AKBAL_SUBMISSION_COUNTER] > 18) choices[choices.length] = 7;
		choices[choices.length] = 7;
		choices[choices.length] = 7;
	}
	//Exgartuboobs
	if(player.statusAffects.get("Exgartuan").value1 == 2) {
		choices[choices.length] = 8;
		choices[choices.length] = 8;
		choices[choices.length] = 8;
	}
	//Exgartucock
	if(player.statusAffects.get("Exgartuan").value1 == 1 && player.lowerBody.cockSpot.hasCock()) {
	...
	//Dom + Scylla
	if(flags[FlagEnum.NUMBER_OF_TIMES_MET_SCYLLA] > 0 && flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00150] > 0) {
		choices[choices.length] = 12;
		choices[choices.length] = 12;
		choices[choices.length] = 12;
		if(player.lowerBody.cockSpot.hasCock() && player.stats.cor > 50) {
	...
	if (player.perks.has("Lusty")) dreamLust += (dreamtemp / 3);
	
	// Well Adjusted cuts by half
	if (player.perks.has("WellAdjusted")) dreamLust = (dreamLust / 2);
	...
	if(dreamtemp > 5) {
		...
		if(daydream == 0) {
			if(player.stats.cor <= 33) { 
				...
				if(rand(2) == 0) Render.text("tender sex with a sweetheart back home.  ", false);
				else {
					Render.text("beautiful nude ", false);
					if(player.gender <= 1 || (player.gender == 3 && rand(2) == 0)) Render.text("women ", false);
					else Render.text("men ", false);
					Render.text("massaging you, slowly moving their hands over your most intimate places.  ", false);
				}
			}
			if(player.stats.cor > 33 && player.stats.cor <= 66) {
				Render.text("being violently raped and used by demons.  ", false);
			}
			if(player.stats.cor > 66) {
				dreamtemp = rand(5);
				if(dreamtemp <= 3) Render.text("giving yourself to the demons fully, allowing yourself to be tied down and owned as your body is warped for their twisted pleasures.  ", false);
				else Render.text("being captured and taken to a rusted building with dark smokestacks that belch sweet purplish smoke.  Inside are rows of multi-dicked, huge breasted humans, approximately thirty of them.  Each is shackled with their legs spread and torso bent over, and each has tight fitting suction tubes fitting over their tits and cocks.  Every set of tubes is pulsing with suction, drawing rivers of sticky white fluids from its slave's over-endowed breasts and balls.  You shudder in horror and arousal as you realize the victims seem to be arranged by age, ending with an empty machine next to the youngest slut-cow.  The inhuman strength of your captors easily overpowers your struggles as you are forced into your shackles, the metal locks clicking with finality.  A funnel is forced into your mouth, force-feeding you slick corrupted fluids that taste like sex and make your head swim.  Your vision fades as you feel heat in your chest and groin, making you swoon from the drugged cocktail and pleasure of your new best friends – the suction tubes. All you can hear is your own desperate moans... no, wait... that was a dream... but it was so twisted and hot that you're still panting with lust.  ", false);
			}
		}
		//canine
		else if(daydream == 2) {
			...
			if(player.gender <= 1 || (player.gender == 3 && rand(2) == 0)) {
				if(rand(2) == 0) Render.text("locking a thick knotted cock inside a female, the pheromones of her heat making your maleness twitch and flex, the knot bulging obscenely as you begin to impregnate her.  ", false);
				else Render.text("stroking a knotted doggie-prick, gently stroking and squeezing it, yipping in pleasure as your pointed dog-cock leaks steady streams of fluids.  ", false);
			}
			//female
			else {
				//heat dream!
				if(player.inHeat) {
					Render.text("being pregnant, your belly bulging obscenely with the fruits of all your frequent copulations, and your breasts swollen with breast milk.  You imagine your condition enhancing your libido, driving you seek out sexual partners willing to pleasure your distorted form.  ", false);
				}
				//normal dream
				else
				{
					Render.text("being in heat, your cunt sopping wet with moisture and desire, intense pheromones pouring off you to make all the males rigid and ready.  ", false);
				}
			}
		}
		//horse
		else if(daydream == 1) {
			if(player.gender == 1 || (player.gender == 3 && rand(2)==0)) Render.text(" running the plains with a harem of beautiful centaur fillies.  Your bloated equine endowments swelling with blood at the sight of their fertile backsides and potent pheromones.   You dream of staying up half the night to service your animalistic brides insatiable desires, plugging them over and over until your baby-batter is running in steady streams from their backsides.  ", false);
			else Render.text("running your own farm, complete with a stable of docile, horse-morphed studs.   Of course each had to be captured as he came through the portal, until you had enough studs to keep your cunny happy and your womb filled with equine-spunk.  You saunter out to the barn and smile at the immediate reaction your presence has on your livestock, their leering eyes glued to every curve of your nude body.   Each of them lines up, guiding their rapidly hardening shafts through specially crafted holes in the stalls.  You giggle as your oldest acquisition struggles, trying to get the massive flare of his head through the hole before it gets any bigger.  You dream of walking down the aisle, granting each stud the release he so desires, taking the largest and most worthy to feed your tainted womb's thirst for hot spunk.  ", false);
		}
		//cow
		else if(daydream == 3) {
			if((player.gender == 1 && player.upperBody.chest.BreastRatingLargest[0].breastRating >= 5) || (player.gender == 3 && rand(2) == 0)) {
				Render.text("having full and lactating breasts, searching for the right person to give your milk to.  You come across a pretty human girl and invite her to your breast.  She eagerly jumps into your arms and starts to suckle from your ample bosom.  You then guide your erect " + player.cockDescript(0) + " into her waiting lower lips.  The two of you stand there in eternal bliss as she suckles on your chest and you thrust into her womanhood, until you wake from the dream.  ", false);
			}
		...
		//minotaur cum
			...
			Render.text("You cum.  You scream in glorious and divine pleasure, granted by your immaculate god.  The acolytes watching masturbate themselves, in awe of the raptured orgasm Akbal has granted you.  Your tongue lolls out as he pounds into you, the haze of delight briefly devouring your thoughts in all-consuming green fire.\n\n", false);

			Render.text("\"<i>Glory unto Akbal,</i>\" Buttslut whispers.\n\n", false);

			Render.text("You awake with a start, grumbling slightly before turning over and trying to get back to sleep.  Your asshole twitches.", false);
		}
		//Exgartuboobs
		else if(daydream == 8) {
			Render.text("drifting through darkness to arrive at your lover's bed.  She purrs and beckons you over, almost entirely hidden behind her massive chest.  You grunt slightly, and between your legs your cock rises, rises, rises - growing in measurements of feet rather than inches.  Excited and eager she moans as you enter her cleavage, thrusting into the ample soft flesh eagerly.\n\n", false);

			Render.text("Her hands stroke her tits, encouraging you to push in deeper, harder.  She jiggles the overabundant flesh, sending comfortable ripples down all of her obscene cleavage.  \"<i>Harder,</i>\" she pants out eagerly, \"<i>harder!</i>\"  You grunt and pound, gripping onto her huge nipples to brace yourself.  Under her chin your cockhead bursts up, grinding into her neck and making her gasp excitedly.\n\n", false);

			Render.text("Eventually you cum, splattering your spunk all across her features and the top of her tits.  It runs down the massive flesh in slow dribbles, dimples in the enormous bust causing the spooge to puddle slowly.  Licking her lips, she grinds the cum into her breasts before pausing.  Looking at you carefully, she speaks.  \"<i>So, is this your dream?</i>\" Exgartuan asks.  \"<i>Or mine?</i>\"\n\n", false);

			Render.text("You awake with a start.  Your " + breastDescript(0) + " jiggles grumpily.", false);
		}
		//Exgartucock
		else if(daydream == 9) {
			Render.text("drifting through darkness to you arrive at your lover's bed.  He smiles and strides forward confidently, gently pressing his massive cock into your stomach and easing you down.  A few spurts of pre-cum sink into your chest, causing it to swell and round.  \"<i>I thought you'd like to match,</i>\" he says charmingly, as his humongous cock is eclipsed by your ballooning tits.  You smirk coyly in return.\n\n", false);

			Render.text("Hands roaming across your growing bust, you eagerly jiggle it against his prick.  The steaming meat resting in your cleavage feels less like an invader and more like a prisoner, trapped between your massive mounds.  He obviously appreciates your efforts, groaning and grunting as he thrusts into your overwhelming titflesh.  Thick as it is, you can easily feel every vein and bulge in his shaft as it pushes past your sensitive mammaries.\n\n", false);

			Render.text("The titfuck feels fantastic, your over-inflated chest burning with sensation along every inch of their fullness.  Your lower body is completely hidden from view, and you couldn't reach out and grab your partner if you tried.  Only rarely in his eager thrusts does his cockhead emerge from between your cleavage, more usually plowing into your bosom and remaining hidden.  Eventually the pleasure from the act grows to be too much for him, and cum splatters all through your squeezed-together tits.  It erupts and flows out of your cleavage, running down your breast.\n\n", false);

			Render.text("Your partner licks his lips and sighs in slow release, before pausing.  Looking at you carefully, he speaks.  \"<i>So, is this your dream?</i>\" Exgartuan asks.  \"<i>Or mine?</i>\"\n\n", false);

			Render.text("You awake with a start.  Your cock flops over grumpily.", false);
		}
		//Rubber/Latex Skin
		else if(daydream == 10) {
			Render.text("your lips swelling with a single breath.  No, more precisely, they inflate.  They grow, and they stretch, and your mouth opens into an O.  You fall, fall, fall backwards, landing on your resilient posterior and bouncing lightly.  As you stare at the ceiling, your arms curve up at the elbow, rigid and unmoving.  You do not blink.\n\n", false);

			Render.text("Eventually, someone enters the room.  They pull you into a sitting position, and then shove their cock into your big, O-shaped lips.  Your body squeaks as they piston into your rubber hole, and eventually dump their load inside of you.  They let go, and you fall back.  Later they return, pick you up and put you on your chest.  The rubber of your asshole stretches around their dick, tight but yielding to a superior force.\n\n", false);

			Render.text("Like before, you drop to the ground after, used.\n\n", false);

			Render.text("So it goes, the cycle repeating.  Eternally making your plump O face, unmoving without prompting, and discarded.  The latex sex doll in an unknown harem.  Did they ever know you were once a mortal?\n\n", false);

			Render.text("You awake from your dream panting slightly, but not sweating.  You don't do that anymore.\n\n", false);

			Render.text("You're not certain how to feel about that.", false);
		}
		...
		//Scylla + dom + dong
			...
			if(player.gender <= 1) {
				...
				//Male Trap Dream #2: 
					...
					if(player.lowerBody.cockSpot.hasCock()) Render.text("  [EachCock] is fully erect and throbbing.");
					else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("  Your [vagina] dripping wet with need.");
					Render.text("  You shudder as you recall the erotic dream you were just having, a hand caressing your aching loins.");
					//end dream
					//lust increase
				}
				dynStats("lus", 25);
			...
			//Dream about living in a borrow and having lots and lots of kids.  Many furred, big eared bodies are running around and you feel the pregnant belly of your mate, or feel your own pregnant belly.
				...
				Render.text("finding yourself in an underground burrow, with many small furred bodies with large ears scurrying around all over.  The place is warm and cozy, while also filled with the smell of mice and sex.  You love it down here, and you love seeing all your energetic children running around and having fun.\n\n", false);
				Render.text("You have a great larder, able to feed everyone as much as they need.  It has never been a problem to keep it full.  It's a veritable paradise for a rodent such as yourself.  Then you find your way into the master bedroom and see its massive bed, the site where your great family was brought into existence.\n\n", false);
				
				if(player.gender == 1 || (player.gender ==3 && rand(2)==0)) {
					Render.text("Things blur for a moment, and you find your head resting against a pregnant belly, covered in fine fur.  The results of your virility as a father, and soon to be added to the great brood that you helped bring into this world...\n\n", false);
				} else {
					Render.text("Things blur for a moment, and you find yourself laying on the bed with your hands wrapped around your heavily pregnant belly.  The product of your fertility and the virility of your great mate, and soon to be added to the great brood you've already brought into the world...\n", false);
				}
			}
		}
		doNext(playerMenu);
...
public dayTenDreams():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) {
		if(player.stats.cor < 50) {
			Render.text("Your task completed, you return through the portal to your home.  Exhausted but happy, the sight of your village fills you with a surge of emotions.  Most prominently, and perhaps most enjoyably: pride.  You did it.\n\n", false);

			Render.text("You drink and celebrate with your people long into the night, feasting and receiving accolades and praise.  It is a party the likes of which you have never seen, and rightfully so.  Never again will your village be threatened by the demons.  Late in the evening, you arrive at the home of your childhood and greet your mother.\n\n", false);

			Render.text("\"<i>Welcome back,</i>\" she smiles.\n\n", false);

			Render.text("\"<i>Thank you, Mom,</i>\" you respond, wrapping your arms around her and kissing her deeply.  Your hands take hold of her rear, gripping-\n\n", false);

			Render.text("Shocked awake, you blink as the light of your dying fire flickers dimly across the campsite.  That was...\n\n", false);

			Render.text("This place is getting to you.", false);
		}
		else {
			Render.text("Your task completed, you return through the portal to your home.  Exhausted but happy, the sight of your village fills you with a surge of emotions.  Most prominently, and perhaps most enjoyably: pride.  You did it.\n\n", false);
			...
			Render.text("\"<i>Thank you, Mom,</i>\" you respond, wrapping your arms around her and kissing her deeply.  Your hands take hold of her rear, gripping it in great handfuls and squeezing her possessively.  Nearly throwing her in the bed, you delight in her playful squeals.  Mounting her, you tear away her dress and plunge into her cunt.\n\n", false);

			Render.text("\"<i>Oooh, god!</i>\" she exclaims,  \"<i>Such a good boy!  Such a big boy!</i>\"\n\n", false);

			Render.text("\"<i>That's right, Mom,</i>\" you grunt, taking hold of her breast and sucking harshly on it.  \"<i>All grown up.</i>\"  You continue to grope her roughly as you pound into her cunt, slapping your balls into her thighs.  Her pussy squeezes you with a mother's love, and you soon find yourself dumping heaping loads of your spunk within her.  You rise, and bring your dick to her mouth so that she can clean you.\n\n", false);

			Render.text("As your mother laps at your shaft, coated in her juices, you look confidently down at her belly.  Though you love your mother, you're looking forward to having a daughter, and training her to love you just as much as a mother does. One day, perhaps even to replace her.\n\n", false);

			Render.text("You awaken in a sweat.  Fuck.", false);
		}
	}
	else {
		if(player.stats.cor < 50) {
			...
			Render.text("\"<i>Welcome back,</i>\" he smiles.\n\n", false);

			Render.text("\"<i>Thank you, daddy,</i>\" you respond, wrapping your arms around him and kissing him deeply.  Your hands slip into his pants, lowering-\n\n", false);
			...
			Render.text("\"<i>Thank you, daddy,</i>\" you respond, wrapping your arms around him and kissing him deeply.  Your hands slip into his pants, lowering them and exposing his crotch.  Too excited to control yourself, you drag your lips down his chest before wrapping them around his cock and taking him into your mouth.  You pull him deep, teasing his balls with your lower lip.  Though it isn't necessary to gag - you could have swallowed him easily - you do anyway.  Daddy deserves a show.\n\n", false);

			Render.text("His cum splatters into your throat.\n\n", false);

			Render.text("You smile with glazed teeth as you straddle him in his bed and mount him.  Some disappointment lingers in you that you didn't just let daddy take your ass then and there, but the prospect of his cum flooding your womb and fertilizing you is too delightful to pass up.\n\n", false);

			Render.text("\"<i>Do you like it?</i>\" you ask, before moaning his name.\n\n", false);

			Render.text("\"<i>You're a good girl,</i>\" he answers.\n\n", false);

			Render.text("His balls gurgle as they fill you.  You squeal in delight and lick his chest, demonstrating your affection.  Clenching him tightly, you make sure not to spill daddy's seed, and imagine giving birth to a sexy, busty daughter, teaching her to love him.  One day, teaching her to replace you.\n\n", false);

			Render.text("You wake up in a sweat.  Fuck.", false);
		}
	}
	if(player.stats.cor < 50) dynStats("lus", 10);
...
public displacerDreamII():void {
	...
	if(silly()) Render.text("k");
	Render.text("not a drop of jism has escaped.");
...
public displacerDreamIII():void {
	...
	let x: number = player.lowerBody.cockSpot.count();
	while (x > 0) {
		x--;
		if(player.lowerBody.cockSpot.list[x].sock == "amaranthine" && player.lowerBody.cockSpot.list[x].cockType != CockType.DISPLACER) {
			if(player.lowerBody.cockSpot.list[x].cockType != CockType.DOG) player.lowerBody.cockSpot.list[x].knotMultiplier = 1.5;
			player.lowerBody.cockSpot.list[x].cockType = CockType.DISPLACER;
		}
	}
	doNext(playerMenu);
}
