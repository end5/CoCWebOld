...
public function gravyBoatDatTurkturk():void {
	...
	if(player.lowerBody.cockSpot.list[0].cockThickness >= 5) outputText(", her stomach bulging obscenely with your inhuman girth");
	outputText(".");
...
public function turkeyDesertBitches():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) outputText(", cupping a tit");
	...
	if(player.lowerBody.cockSpot.count() == 2) outputText(" with both cocks, one ramming into her abused cunny and the other right up her vulnerable little bumhole");
	else if(player.lowerBody.cockSpot.count() > 2) outputText(" with every cock you've got, half in the pink and half in the stink, spreading her as wide as you are in both barrels");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText(" and bowels");
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("s deposit the last of their");
