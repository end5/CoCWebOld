...
public helloAgain():void {
	...
	if(player.cockArea(0) <= 60) Render.text("and down to the base");
	else Render.text("and swallowing your shaft until her throat is bulging with your massive manhood");
	Render.text(".  She bobs up and down your length, hefting up her huge rack when she's at the peak to rub your sensitive underside, trying to titfuck you but just too damn big to get them into the action.  You suppose you'll have to take care of that...");
	menu();
...
public gravyBoatDatTurkturk():void {
	...
	if(player.lowerBody.cockSpot.list[0].cockThickness >= 5) Render.text(", her stomach bulging obscenely with your inhuman girth");
	Render.text(".");
...
public seasonHerDatTurkeyBitch():void {
	...
	if(player.hasKeyItem("Deluxe Dildo") >= 0) Render.text("Tamani's dildo");
	else if(player.hasItem(consumables.L_DRAFT)) Render.text("a bottle of Lust draft");
	else if(player.hasItem(consumables.F_DRAFT)) Render.text("a bottle of Fuck draft");
	else Render.text("A FUCKIN ERROR");
	Render.text(".  You pull it out and squeeze a copious load of the bubblegum pink aphrodisiac out into your hand.  The second it touches your skin, you feel a palpable warmth spreading out through your nerves, skin burning with sensitivity as the roiling, viscous substance pools in your cupped hand.  You can't help but shudder as the pink veno's effect reaches out through your body, setting your whole person to tingling - and especially your " + player.multiCockDescriptLight() + ", which rises to an almost painful hardness.");
...
public turkeyDesertBitches():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 1) Render.text(", cupping a tit");
	Render.text(" as the venom takes control, flushing your skin and forcing out all thoughts but one: fuck that turkey! There's nothing for it, now: you grab Gobbles' hips and plunge in");
	if(player.lowerBody.cockSpot.count() == 2) Render.text(" with both cocks, one ramming into her abused cunny and the other right up her vulnerable little bumhole");
	else if(player.lowerBody.cockSpot.count() > 2) Render.text(" with every cock you've got, half in the pink and half in the stink, spreading her as wide as you are in both barrels");
	Render.text(", shocking her so much that she can't hold back, screaming and cumming on the spot! You groan, thrusting forward as her spasming muscles contract around your cock");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(", milking you as hard as she can while her orgasm overtakes her.  Gravy geysers out of her swollen tits as fem-cum splashes out around your deep-buried cock");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(", drenching your [hips] and [legs] in creamy girl-spunk, tinged pink by the poison you fist-fucked up her cunt.");
	...
	{
		Render.text("her stomach is bulging obscenely with the sheer volume of cockflesh rammed up her cunt");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("and ass");
	}
	else Render.text("your hips are slapping into hers, making her butt bounce with the impact");
	Render.text(".  She cries her one word to the heavens as you pound into her hole");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(", thrusting your cock");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" deep inside.  Her avian legs break free of her hold, wrapping around your [hips] and pulling you in, locking you in her spasming hole");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" as she cums and cums, pleading, \"<i>Gobble!</i>\" at the top of her lungs.  You switch your grip to her bountiful bosom, roughly squeezing her tits as you hump your cock");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" in and out of her slick slit{and quivering behind}, pinching her nipples to give the squealing turkey just a little more pleasure.");
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text(" and ass");
	Render.text(", her legs spurring you on every time you withdraw, muscles clenching when you pause, never letting you rest.  Needy slut! But she's more than willing to do her part, thrusting her hips up to meet you, squeezing and milking your cock");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" every time you pierce her, working your shaft");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" so expertly that you know you won't be able to hold on for long - and neither will she, the way she's panting and gasping along with you!");
	
	Render.text("\n\nYou draw nearer to the breaking point, hips unrelentingly slamming your shaft");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" deep into Gobbles' eager depths, urged on by her happy squawks and gleeful cries.  Lust-venom-addled as the two of you are, orgasm approaches like a wave of relief, crashing through your bodies in beautiful unison.  \"<i>Gobbles!</i>\" she cries as her cunt ");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("and ass ");
	Render.text("squirm around your cock");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(", wringing the cum from you with powerful spastic contraction.  Her arms pull you down into the valley of her gravy-laden bosom, squeezing your face between her massive mounds as she cums, drawing the spunk right out of you.  With a potent, feral roar, you cum, " + player.multiCockDescriptLight() + " blasting its salty load deep into Gobbles' womb");
	if(player.lowerBody.cockSpot.count() > 1) Render.text(" and bowels");
	Render.text("; her back arches, teats wobbling around your head as she takes your cum, squawking and gobbling gaily as more and more of your virile spunk pours into her, mixing with her own fem-cum and the remnants of the lusty draft your fucked into her.");
	
	Render.text("\n\n\"<i>Gobble!</i>\" she sighs happily as your cock");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s deposit the last of their");
	else Render.text(" deposits the last of its");
	Render.text(" cream into her, fully stuffing your turkey-girl.  Panting, you let your head rest between her tits a moment long, shaft");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" wilting inside her until like a broken dam, her cunt");
	if(player.lowerBody.cockSpot.count() > 1) Render.text(" and ass");
	Render.text(" begin");
	if(player.lowerBody.cockSpot.count() == 1) Render.text("s");
	Render.text(" to drool spunk, pooling onto the hungry ground with the gallons of spilt gravy and fem-spooge.");
...
public pigSlutRoastingGreet():void {
	...
	dynStats("lus", 5);
	menu();
	if(player.stats.gems >= 1) addButton(0,"Throw gem",getARoastPiggueOinkOinkOinkMotherfucker);
...
public getARoastPiggueOinkOinkOinkMotherfucker():void {
	...
	Render.text("our nostrils flare, taking in great amounts of the salacious stench.");
...
public tenderizeDatPigSloot():void {
	...
	if(player.tailType > TAIL.NONE) Render.text("You even bring your tail from behind your back to join in, the unfamiliar texture causing the sausage slut to let out surprised, high-pitched squeaks. ");
	Render.text("You notice the pigslut moving backwards, her ass being pushed into the air as she attempts to feel the full power of your spanking, her pussy spraying juices onto the soaked sheets.");
...
public getDoneEatingOutPigSlut():void {
	...
	if(player.thickness < 100) {
		Render.text("  You notice that you seem a bit more... jiggly, a slight paunch in your stomach now as you grow a bit more plump than you once were.");
	...
	if(player.lowerBody.butt.buttRating < 35) {
		Render.text("  You notice your ass filling out, gaining more mass as it visibly grows before your eyes."); 
	...
	if(player.lowerBody.hipRating < 35) {
		Render.text(" Your hips grow too, widening and giving you more sexy curves.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating < 20) {
		Render.text(" Your boobs also grow, gaining size as it seems that every part of your body wants to join in on the growth.");
	...
	Render.text("\n\nYou sigh");
	if(changed) Render.text(", feeling your body expand as you waddle out back towards camp with belly full of sweet syrup");
	Render.text(". For only one gem, that was a pretty good time...");
	doNext(camp.returnToCampUseTwoHours);
}
