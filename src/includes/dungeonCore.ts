...
//			text6 = "West";
			if (player.hasKeyItem("Iron Key") < 0) {
				Render.text("  It seems your opponent dropped a small iron key as she fled.", false);
//				choice3 = takeIronKey;
//				text3 = "Iron Key";
...
//			choice5 = drinkCoffee;
			...
			if(player.demonScore() >= 4 && player.stats.cor > 75) {
				Render.text("The busty succubus turns, her barely contained breasts jiggling obscenely as she notices you, \"<i>Oh, like hi there ", false);
				if(player.gender == 1) Render.text("stud", false);
				else Render.text("sexy", false);
				Render.text("!</i>\"  She stops, sniffing the air, a curious expression on her face as she slowly circles you, her heals clicking loudly on the floor.  A knowing grin blooms across her face as understanding hits her.\n\n", false); 
				Render.text("She exclaims, \"<i>Omigawsh!  You're the champion!  Your, like, soul is still there and everything!  But, you're like, completely corrupt an' stuff!  Ya know what'd be fun?  I could fuck you 'til you cum so hard your soul melts out an' you turn into a demon.  Wouldn't that be great?</i>\"\n\n", false);
				Render.text("The secretarial demoness pulls out a file and fiddles with her nails, murmuring, \"<i>I guess if you don't wanna, we could just hook you up in the factory.  What's it gonna be?</i>\"", false);
//				text1 = "Fight";
...
//				itemMenu = null;
			...
			//Not recognized
				...
				Render.text("!  You haven't seen a confused human about calling itself a champion have you?</i>\"\n\nShe shakes her more-than-ample bosom from side to side as she licks her lips and offers, \"<i>If you do, be sure and bring them back here ok?  We've got their spot all ready for them, but that little prick Zetaz fucked up the pickup.  Tell you what – if you bring me the 'champion' I'll ", false);
				if(player.totalCocks() > 0) Render.text("give you the blowjob of a lifetime", false);
				else if(player.lowerBody.vaginaSpot.hasVagina()) Render.text("lick your honeypot 'til you soak my face", false);
				else Render.text("give you a new addition and show you how to use it", false);
...
//				itemMenu = null;
				...
				Render.text("!  What's a cute little morsel like you doing by yourself out here?</i>\"", false);
//				text1 = "Fight";
//				choice1 = succubusCombatStart;
//				text2 = "Talk";
...
//				choice2 = talkToIncubus;				
	...
	if(dungeonLoc == DUNGEON_FACTORY_REPAIR_CLOSET) {
		Render.text("<b><u>Repair Closet</u></b>\nAs you carefully slip inside the room, you note with some relief that it seems to be an empty storage closet. The room is tiny, barely 6' by 8' and almost entirely empty.  The one piece of furniture inside the closet is a simple wooden cabinet, placed against the far wall.  ", true);
		if (player.statusAffects.has("BuiltMilker"))
			Render.text("The shelves are empty.  ", false);
		else {
			Render.text("The shelves of the cabinet hold various pieces of pump machinery, probably used to repair complete machines further into the factory.  ", false);
			if (player.stats.int >= 40) {
				Render.text("You realize there are enough pieces here to put together a breast-milking pump or a cock-milker.  ", false);
...
//		text7 = "South";
//		choice7 = 11001;
		Render.text("The only exit is back to the south.", false);
	...
	if(dungeonLoc == DUNGEON_FACTORY_MAIN_CHAMBER) {
		...
		if(player.findStatusAffect(StatusAffects.DungeonShutDown) < 0) {
			Render.text("<b><u>Main Chamber</u></b>\nThis cavernous chamber is filled with a cacophony of sexual moans.  Rows of harnesses are spaced evenly throughout this room, nearly all of them filled with delirious-looking humans.  Each is over-endowed with huge breasts and a penis of elephantine proportions.  The source of their delirium hangs down from the ceiling - groups of hoses that end with needles buried deep into the poor 'girls' flesh, pumping them full of demonic chemicals.  Constant sucking and slurping noises emanate from nipple and cock pumps as they keep the victims in a state of near-constant orgasm.  ", true);
			if(player.stats.cor < 50) Render.text("You wish you could free them, but it would take the better part of a day to get them all free.  It'd be better to find the control room and shut down the infernal machinery.  ", false);
			else Render.text("You wish you had some machinery like this for yourself.  It looks so fun!  Still, you suppose you should find the control panel to shut this down and free these people.  ", false);
			Render.text("There is a doorway to the east marked with an 'exit' sign above it.  Along the southern wall is a stairwell that leads up to some kind of foreman's office.  Perhaps the controls are in there?", false);
		}
		//Dungeon shut down.
		else {
			Render.text("The chamber is significantly emptier since you've shut down this factory.  Roughly half the girls appear to have left.  The rest seem to be pre-occupied by fucking each other in a massive orgy.  A few enterprising ladies have found leather outfits and appear to be helping to manually administer the chemical cocktails to those engaged in rampant sexual exploits.  It seems some of them preferred a life of near-constant orgasm to their freedom.  There is a door to the east marked as 'EXIT', and a stairwell along the south wall that leads to an overseer's office.", true);
...
//				text3 = "Desk";
	...
	if(dungeonLoc == DUNGEON_FACTORY_PUMP_CONTROL) {
		//PUMP CONTROL ROOM
		Render.text("<b><u>Pump Control Room</u></b>\n", true);
		if (player.findStatusAffect(StatusAffects.DungeonShutDown) < 0) {
			Render.text("This room is little more than a closet in reality.  There is a simple set of mechanical controls on a finely crafted terminal against the far wall.  You spend a moment looking over them, and realize you have three options to deal with this place.\n\n", true);
			Render.text("-You could close the storage vent valves and overload the fluid storage systems.  The storage tanks along the back portion of the building would rupture, releasing thousands of gallons of tainted fluids into the surrounding area, but the facility's systems would suffer catastrophic failures and shut down forever.\n", false);
			//(Consequences - lake goddess becomes tainted!)
			Render.text("-You could perform a system shutdown and then smash the controls.  It'd let the girls go and keep the factory shut down in the short term.  However most of the equipment would be undamaged and the place could be re-opened without too much work on the demons' part.\n", false);
			//(Consequences - If Marcus is a demon he takes over running the factory forever.  If not, nothing bad happens)
			Render.text("-You could leave the equipment to continue running.  After all, the girls downstairs did seem to be enjoying themselves...\n", false);
...
//			choice5 = factoryShutdown;
			...
			Render.text("This room is little more than a closet in reality.  There is a simple set of mechanical controls on the a finely crafted terminal against the far wall.  The controls are now inoperable, due to the damage your actions have caused.", false);
		}
//		choice6 = 11006;
//		text6 = "West";
...
//		choice1 = 11006;
		...
		if (player.statusAffects.has("TakenLactaid")) {
			if (player.statusAffects.get("TakenLactaid").value1 > 0) {
				Render.text("There is a crate with " + num2Text(player.statusAffects.get("TakenLactaid").value1) + " bottles of something called 'Lactaid' inside.\n\n", false);
//				text3 = "Lactaid";
//				choice3 = storageTakeLactaid;
			...
			Render.text("There is a crate with five bottles of something called 'Lactaid' inside.\n\n", false);
//			text3 = "Lactaid";
//			choice3 = storageTakeLactaid;
			addButton(2, "Lactaid", storageTakeLactaid);
		}
		if (player.statusAffects.has("TakenGroPlus")) {
			if (player.statusAffects.get("TakenGroPlus").value1 > 0) {
				Render.text("There is a crate with " + num2Text(player.statusAffects.get("TakenGroPlus").value1) + " bottles of something called 'Gro+' inside.\n\n", false);
//				text4 = "GroPlus";
//				choice4 = storageTakeGroPlus;
			...
			Render.text("There is a crate with five bottles of something called 'Gro+' inside.\n\n", false);
//			text4 = "GroPlus";
//			choice4 = storageTakeGroPlus;
			addButton(3, "GroPlus", storageTakeGroPlus);
		}
	...
	if(dungeonLoc == DUNGEON_CAVE_ENTRANCE) {
		Render.text("<b><u>The Cave Entrance</u></b>\n", true);
		Render.text("The entrance to this cave is far bigger than the cave itself.  It looks to be a totally natural formation.  Outside, to the south, is a veritable jungle of plant-life.  There are massive trees, vines, and ferns everywhere.  The cave grows narrower the further north you go, until it's little more than a claustrophobic tunnel burrowing deep into the earth.", false);
...
//				choice3 = incubusShop;
	...
	if(dungeonLoc == DUNGEON_CAVE_TUNNEL) {
		Render.text("<b><u>Cave Tunnel</u></b>\n", true);
		Render.text("This cave tunnel slants downwards to the north, and upwards to the south.  You can see sunlight and feel a fresh breeze from the latter direction, though the walls and air around you are damp with moisture.  You realize that the floor of this cave is fairly smooth and even, as if some attempt had been made to level it out.  You can see a bricked up wall along the north end of the tunnel.  It has a crudely fashioned wooden door in the center of it.", false);
//		text7 = "South";
//		choice7 = 11066;
//		text1 = "North";
//		choice1 = 11068;
...
//				choice3 = leftValaAlone;
		...
		//Not here
		else Render.text("In the far corner, there are a set of empty manacles, originally set up to contain Vala, who you've long since freed.", false);
		//Movements
//		text1 = "North";
//		choice1 = 11071;
//		text6 = "West";
//		choice6 = 11068;
	...
	if(dungeonLoc == DUNGEON_CAVE_SECRET_TUNNEL) {
		Render.text("<b><u>Secret Tunnel</u></b>\n", true);
		Render.text("This passage is the least livable area that you've seen out of the entire cave.  The walls and floor are little more than dirt and rocks, and explosions of dust burst from the ceiling with each tentative movement you make.  For a moment, a wave of claustrophobia threatens to rob you of your nerve, but you blink the pervasive particles from your eyes and focus on why you're here.  ", false);
		//If zetaz not yet defeated
		if(flags[FlagEnum.DEFEATED_ZETAZ] == 0) Render.text("You're going to find Zetaz and pay him back for drugging you on your first day here.  ", false);
		Render.text("A crude door on the southern edge of the tunnel leads back to the imp's sleeping chambers, but the tunnel continues away, curving sharply to the west where a far more lavish door marks the far side of the subterranean passage.", false);
...
//		choice7 = 11070;
	...
	if(dungeonLoc == DUNGEON_CAVE_ZETAZ_CHAMBER) {
		Render.text("<b><u>Zetaz's Chambers</u></b>\n", true);
		Render.text("You've stepped into the most lavish room in the entire cave system, and marvel at the difference between this magnificent abode and your own crudely constructed campsite.  The stone walls are covered in stolen tapestries that each look to have been liberated from a unique source.  Judging by the variety of depictions and art styles in this one room, you've barely met a fraction of the races that once inhabited the lands of Mareth.  A pair of bright, smokeless lanterns hang from each wall, lit from within by obviously magical spheres of luminescence.  Various pieces of stolen furniture decorate the room, surrounding a four-post bed decorated with masterfully done carvings of various carnal acts.", false);
		if(flags[FlagEnum.ZETAZ_DOOR_UNLOCKED] == 0) {
			Render.text("  <b>There's a bolt holding a door to the south closed, but you give it a gentle tug and it comes unlocked.</b>", false);
			flags[FlagEnum.ZETAZ_DOOR_UNLOCKED] = 1;
		}
		Render.text("\n\n", false);
		
		if(flags[FlagEnum.DEFEATED_ZETAZ] == 0) {
			Render.text("A familiar imp is looking at you with a bewildered expression painted across his face.  You recognize his face immediately – this is Zetaz!  Oddly, he seems to have grown much larger in the time since your previous meeting.  He's over four feet tall and much more solidly built!\n\n", false);
			Render.text("Zetaz whines, \"<i>Seriously?  You show up here!?  First you make me lose my job, and now you beat up my friends and track dirt in my bedroom!?  I've had enough!</i>\"", false);
...
//			choice2 = 11071;
	...
	if(dungeonLoc == DUNGEON_HEL_GUARD_HALL) {
		//ROOM 1: Guard Hall
		Render.text("<b><u>Guard Hall</u></b>\n", true);
		//Room Description:
		Render.text("You stand in what might have been a guard room once upon a time.  Now it is a ruined, ransacked mess.  It seems not to have been used in years, and the table, chairs, and spears lined up against the wall have all rotted away to almost nothing.");
		//[If Armor has not been taken/fought with: 
		if(flags[FlagEnum.WON_GOO_ARMOR_FIGHT] + flags[FlagEnum.LOST_GOO_ARMOR_FIGHT] == 0) {
			Render.text("  However, a suit of half-plate armor has been left up against the eastern wall, hanging loosely on a rack; it seems to be in usable shape.");
//			text4 = "Armor";
//			choice4 = takeGooArmor;
			addButton(3, "Armor", takeGooArmor);
		}
		Render.text("  You see a pair of heavy iron doors leading northward, though they seem so rusty and heavy that opening them is sure to alert anyone nearby, and a small trapdoor leading down.");
...
//		choice3 = 11085;
	...
	if(dungeonLoc == DUNGEON_HEL_WINE_CELLAR) {
		Render.text("<b><u>Wine Cellar</u></b>\n", true);
		//(Accessed from the Trapdoor button)
		Render.text("You've dropped down into a small underground hidey-hole, with ");
		if(player.tallness < 60) Render.text("just enough room to stand up in");
		else Render.text("a ceiling so low you have to crouch");
		Render.text(".  To your surprise, nothing horrifying jumps out and tries to rape you.  You see a few horns of mead slung up in a wine rack - they smell a bit pungent, but alcohol improves with age they say...");
		if (flags[FlagEnum.HEL_DUNGEON_MEAD_LOOTED] < 5) {
			Render.text(" (There are " + (5 - flags[FlagEnum.HEL_DUNGEON_MEAD_LOOTED]) + "x God's Mead horns here to take.)\n\n");
//			text4 = "God'sMead";
...
//		choice3 = 11084;
	...
	if(dungeonLoc == DUNGEON_HEL_STAIR_WELL) {
		clearOutput();
		//Room 3: Stair Well
		Render.text("<b><u>Stair Well</u></b>\n", true);
		//(Upon clicking in:)
		if (flags[FlagEnum.HEL_HARPIES_DEFEATED] == 0) {
			Render.text("You open the heavy double doors and cringe as a loud \"<i>SCREECH!</i>\" echoes out and up the next room - a wide open stairwell, it seems, with minimal cover.  The perfect place for a harpy to fight... Oh, shit!");
			...
			if (flags[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] == 0) {
				Render.text("There's a pile of drugged, unconscious harpies you've already defeated on the floor, as well as Kiri, the only one that didn't attack you.  You recall that she knows Hel and is here to help the both of you.");
...
//				choice8 = 11087;
				...
				Render.text("There's a pile of drugged, unconscious harpies you've already defeated on the floor.  Kiri appears to have left.");
//				text6 = "South Door";
//				choice6 = 11084;
//				text3 = "Go Upstairs";
//				choice3 = 11088;
...
//				choice8 = 11087;
	...
	if(dungeonLoc == DUNGEON_HEL_DUNGEON) {
		...
		if(flags[FlagEnum.HEL_BRIGID_DEFEATED] == 0) {
			...
			Render.text("You're standing in a small dungeon room, nearly gagging on the smells of burnt meat and smoke.  A number of nasty torture devices hang on the walls, and a table sits in the middle of the room, ");
			if (flags[FlagEnum.HEL_PC_TALKED_WITH_HAKON] == 0) {
				Render.text("on which the salamander prisoner lies");
//				text4 = "Prisoner";
				addButton(3, "Prisoner", helDungeonPrisonerTalk);
			}
			else {
				Render.text("on which Hakon lies");
//				text4 = "Hakon";
				addButton(3, "Hakon", helDungeonPrisonerTalk);
			}
			Render.text(".");
			if (player.hasKeyItem("Harpy Key A") >= 0 && player.hasKeyItem("Harpy Key B") >= 0) Render.text("\n\n<b>You have the keys to release the prisoner, but you may want to make sure you have everything from this place that you want before you make your escape.  You doubt you'll be able to return in the future.</b>");
...
//			choice5 = tortureGear;
	...
	if(dungeonLoc == DUNGEON_HEL_MEZZANINE) {
		clearOutput();
		Render.text("<b><u>Mezzanine</u></b>\n", true);
		//(Intro; Before Battle)
		if(flags[FlagEnum.HEL_PHOENIXES_DEFEATED] == 0) {
			Render.text("You ascend the heavy stone steps, circling the tower's walls as you ascend.  You are stopped perhaps half-way to the second main floor on a small terrace level with a wide open view overlooking the vale beneath the high mountains.  As you step onto the mezzanine, you watch with a scowl as a number of tall, muscular hermaphrodites step out from the shadows.  Each is clad in heavy chainmail and wields a scimitar and a blood-red shield, but is otherwise nude, revealing their reptilian pricks and slick pussies.  The soldiers standing before you look like harpies, but they have scaled, humanoid legs, long, fiery tails and their wings are the darkest crimson.  These are phoenixes - the dread half-breed warriors you and Hel are here to stop!");
			startCombat(new PhoenixPlatoon());
			return;
		}
		else {
			Render.text("You're standing in the Mezzanine of the tower, a small terrace with a magnificent view of the High Mountains and the valleys below.  There are stairs leading up and down from here, as well as a pile of defeated phoenixes that don't look like they'll be recovering for a bit.");
...
//			choice4 = checkOutDemBirdBitches;
	...
	if(dungeonLoc == DUNGEON_HEL_THRONE_ROOM) {
		...
		if(flags[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] == 0) {
			...
			//Room Description:
			Render.text("You stand in the harpy throne room - a long, circular room dominated by a high throne surrounded by cushions and drapes.  A single long carpet flows from the heavy double doors to the throne, reminding you of a castle's great hall in days gone by.  A number of harpies cower in the shadows, afraid to oppose you further now that their mighty leader is defeated.");
			//[if PC hasn't executed the queen: 
			if (flags[FlagEnum.HARPY_QUEEN_EXECUTED] == 0) {
//				text5 = "Harpy Queen";
//				choice5 = harpyQueenAdvantage;
				Render.text("  The Harpy Queen slumps in her throne, insensate.");
				addButton(3, "Helia", HeliaThroneRoom);
				addButton(4, "Harpy Queen", harpyQueenAdvantage);
			}
			else if (flags[FlagEnum.TOOK_QUEEN_STAFF] == 0) addButton(4, "Take Staff", takeQueensStaff);
...
//			choice5 = leaveBoobsDungeon;
			...
			if (flags[FlagEnum.TIMES_SUBMITTED_TO_SANURA] + flags[FlagEnum.TIMES_WINFUCKED_SANURA] > 0) {
				Render.text("You approach Sanura the sphinx as she pads around the great stone doorframe.  A playful grin spreads across her thin lips as you approach.  \"<i>O-ho!  Back again, I see.  Mmm, it's been so dull since last you <i>came</i>.  There's no one more fun to play out here in the wastes.  So... care to try your hand at my game once more?");
				if (flags[FlagEnum.BEATEN_SANURA_COUNT] > 0) Render.text("  Or would you rather skip the formalities?  We both know who's got the sharper wit, I should think.");
				Render.text("</i>\"");
			}
			else {
				Render.text("The sphinx, Sanura, is padding around the stone doorframe.  Occasionally she beats her leonine wings or gives a mighty yawn, obviously bored by a present lack of stimulation.  Seeing you standing about, however, Sanura gives you a sultry come-hither look and a seductive wink.  You're not sure if she wants to tempt your mind or your body.");
			}
//			text3 = "Riddle Game";
//			choice3 = riddleGameGo;
			addButton(2, "Riddle Game", riddleGameGo);
...
//			choice5 = leaveBoobsDungeon;
	...
	if(dungeonLoc == DUNGEON_WITCH_CAVERNOUS_COMMONS) {
		clearOutput();
		Render.text("<b><u>Cavernous Commons</u></b>\n");
		Render.text("Dancing lights swirl around the roof of the cavern, twirling around each other in patterns too intricate to follow.  Whatever they are, they're clearly magical, and they lend this place an otherworldly ambience unmatched by anything you've seen.  This huge room reminds you of your village commons in a way - it's clearly a communal area.  There's a water-pump in the northwest corner and a blazing purple bonfire in the center of the chamber, heating the cool underground air.  The ground is dirt, rather than sand, and hard-packed as any road.  Various chairs and benches are set up for witches to relax in.  ");
		if (flags[FlagEnum.SANDWITCH_MOB_DEFEATED] == 0) {
			Render.text("Worst of all, a huge assortment of spellcasters is assembling into a mob, obviously hostile.");
			startCombat(new SandWitchMob(), true);
			return;
		}
		else Render.text("The women you defeated before have returned to their tasks, casting wary glances your way from time to time but no longer threatening.");
		Render.text("  Cave tunnels lead in to the east and west into more underground chambers.  A path leads south towards the exit.");
...
//		choice7 = 11135;
	...
	if(dungeonLoc == DUNGEON_WITCH_PREGNANT_LUST_ROOM) {
		clearOutput();
		Render.text("<b><u>West Warrens Eastern Portion South Side (Lust Room)</u></b>\n");
		Render.text("This room is surprisingly large - big enough to hold the " + num2Text(rand(6) + 5) + " heavily pregnant women inside plus perhaps a dozen more.  Like the outer tunnel, this room is lit by magic, though its contents are equally mundane, if a great deal more... interesting.  There's female sex-toys of every variety on almost every surface.  They sit in piles on the floor, they hang from the walls, and there are even some mounted on the wall, to be fucked in place.  Many such toys have multiple shafts and come in shapes from standard to canine to obscenely equine.  All of the witches are presently engaged in coitus with each other or their 'marital aids', but once you enter, they glance at you with hungry, lust-filled eyes.");
		if (silly()) Render.text("  Clearly, if you wanted to, you could put some extra meat in a sand witch.");
//		text1 = "North";
//		choice1 = 11135;
...
//		choice2 = 11135;
	...
	if(dungeonLoc == DUNGEON_WITCH_NURSERY) {
		clearOutput();
		Render.text("<b><u>West Warrens Western Portion North Side (Nursery)</u></b>\n");
		Render.text("As soon as you clear the curtain, you realize there's nothing of interest to you here.  The room is lit with rose pink globes, and the furniture in the room is filled with sleeping mothers, nursing infants, or older children taking naps.  The room is packed with bodies, and while it smells strongly of femininity, there's nothing worth looking into present here.");
//		text7 = "South";
//		choice7 = 11138;
...
//				choice3 = friendlyCumWitchBlessing;
			...
			if (flags[FlagEnum.CUM_WITCH_DEFEATED] == 0) {
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) Render.text("but then, maybe you can come to serve us as a mother.  Our tribe is not wasteful.");
				else if (player.lowerBody.cockSpot.hasCock()) Render.text("but perhaps, once you have been disabused of your notions of freedom, you could serve as my loyal cum-pump.  It does get so tiring inseminating all these girls alone.");
				else Render.text("but then, perhaps you could be made to serve in other ways.");
				Render.text("</i>\"");
...
//		choice7 = 11134;
	...
	if(dungeonLoc == DUNGEON_WITCH_THRONE_ROOM) {
		clearOutput();
		Render.text("<b><u>Sand Mother's Throne</u></b>\n");
...
//	inDungeon = false;
	dungeonLoc = 0;
	clearOutput();
	Render.text("You slip out the door and disappear, heading back towards your camp, leaving the hellish factory behind.");
...
private function relieveTension():void {
	...
	if (player.findStatusAffect(StatusAffects.TensionReleased) < 0) {
		Render.text("You nod and step forwards, allowing her to hook up a modified harness and inject you with the demonic concoction.  In no time heat boils through your veins, pooling on your chest and crotch.  ");
	...
	//Second/third times...
		...
		if (player.statusAffects.get("TensionReleased").value1 == 0) {
			Render.text("You eagerly put on the modified harness and let them inject you with more of those body-altering chemicals.  As they fill you with artificial lust and desire, you cry out and beg for more.  They oblige you and give you a larger dose than the first time.  ");
			//Grow dick!
			if (player.lowerBody.cockSpot.count() > 0) {
				...
				if (player.averageCockLength() >= 9 && player.averageCockThickness() < 2) {
					Render.text("You feel yourself gain in thickness as well, to match your new length.  ");
			...
			if (player.upperBody.chest.count() == 0) {
				...
				Render.text("Your chest tingles, revealing a pair of pink nipples on your new mammory glands.  ");
			}
			player.growTits(1, (2 + rand(3)), true, 1);
			Render.text("  ");
			Render.text("Your " + nippleDescript(0) + "s ");
			if (player.lowerBody.cockSpot.count() > 0) Render.text("and " + player.multiCockDescript());
			Render.text(" become rock hard, leaking fluids constantly.  ");
			//MALE
			...
			//HERM
			if (player.lowerBody.vaginaSpot.count() > 0 && player.lowerBody.cockSpot.count() > 0) Render.text("You feel your " + player.multiCockDescript() + " getting milked by many wet holes, though you are too busy sucking cocks and moaning in ecstasy to notice who they belong to.  ");
		...
		//Third time
			...
			Render.text("Desperate for more of the demon-drugs, you slide into the now-familiar harness and let the needles sink into your skin.   Panting in lust, you beg for them to increase the dosage again.   Desire burns through your veins as the cocktail surges through them");
			if (player.lowerBody.cockSpot.count() > 0) {
				Render.text(", filling your " + player.multiCockDescriptLight());
				Render.text(" with sensation");
				...
				if (player.lowerBody.cockSpot.count() == 1) Render.text("s");
				Render.text(" massive and engorged.  ");
			}
			else Render.text(".  ");
			Render.text("Your " + nippleDescript(0) + "s throb, becoming hard, puffy, and starting to dribble milk.  ");
			if (player.lowerBody.vaginaSpot.count() > 0) Render.text("Your pussy is instantaneously soaked, filling the air with the scent of sex.  ");
...
public function factoryFinisher():void {
	...
	if(player.lowerBody.cockSpot.count() == 0 || player.upperBody.chest.BreastRatingLargest[0].breastRating <= 1) {
		Render.text("A small inner voice pipes up to remind you that you don't have ", false);
		if(player.lowerBody.cockSpot.count() == 0) {
			Render.text("anything in your groin to suck on", false);
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 1) Render.text(" or ", false);
		}
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 1) Render.text("any adornments on your chest", false);
		Render.text(".  That voice trails off as that feeling of perfect pleasure and rightness sweeps it away with the last remnants of sleep.\n\n", false);
	}
	else Render.text("A small inner voice tries to warn you of something, only to be swept away in the feelings of perfect pleasure and rightness that wash away the last remnants of your sleep.\n\n", false);
	Render.text("You realize that the moaning voice is your own, and find that the thought just turns you on more.\n\n", false);
	Render.text("'<i>You're such a horny slut!</i>' echoes a voice in your head.  You want to nod and smile, but are prevented by something.  You realize you're strapped into some kind of chair and harness so securely that you can't even move.  Tiny soothing fingers massage your temples, rubbing away the fears that moments ago threatened to interrupt your pleasure.  You can see a ", false);
	if(player.totalBreasts() == 2) Render.text("pair of ", false);
	else Render.text("multitude of ", false);
	Render.text(" clear hoses coming away from your cow-like chest udders.  ", false);
	if(player.biggestLactation() <= 1.5) Render.text("Creamy white milk is flowing in a steady stream up the tubes and away from you.  ", false);
	else Render.text("The hoses bulge obscenely as they struggle to keep up with the torrents of creamy-white milk you're producing.  ", false);  
	Render.text("Even more wanton moans erupt from your disobedient lips now that you know what's going on.  You're not just a horny slut.  You're a horny cow-slut who's getting off on having her tits pumped.  The massage you're getting feels so good once you realize that.\n\n", false); 
	Render.text("A snap echoes through the pumping room, nearly drowned out by the moans of the other milk-sluts around you.  You look around as you realize the band to restrain your head has been unlatched.  You take advantage of your newfound freedom and look around.  Rows and rows of other girls are there, just like you.  Almost all of them have bigger tits and fuller milk-tubes.  In addition, they all have enormous members that would drag on the floor were it not for the gigantic tubes encapsulating each and every one.  ", false);
	Render.text("The girl next to you squirms and cums, wriggling inside her harness as waves of sticky goop are pumped down her cock-tube into a floor-socket.  She just keeps going and going, making you wonder how she can make so much of the stuff.  As the sight excites you, the pleasure in your own crotch redoubles.  Looking down thanks to your newfound freedom, you see your own giant encapsulated member; though not as large as your neighbor's, it still looks and feels wonderful.\n\n", false); 
	Render.text("The lining of the tube squeezes and massages your trapped prick expertly, even as those hands continue to work on your mind.  Some part of you suspects that your thoughts are being manipulated, but the carnal pleasure you are experiencing is so amazing that you have no intention of resisting. If being a cumslut for your sexy demonic masters is what it takes, so be it. Cramming a massive demon-cock in your throat, getting a few others up your holes to keep you pregnant all the time, and being their busty hermaphrodite breeding tool would be your joy and privilege.  ", false);
	if(player.statusAffects.has("CampMarble")) {
		Render.text("As if reading your thoughts, the hands stop massaging, and their owner snaps their fingers. You see Marble step in front of you, wearing an odd set of pink panties with a dick-like protrusion sticking out the front of them.  At the command of the figure behind you, she presents the panty-cock to you.  Happy to be of service, you spread your jaws and engulf as much of the great penis-like thing as you can, while the figure behind you moves around and takes Marble in the ass.  You continue to suck on the pink flesh until you feel it pour some kind of unholy load into your stomach.  Gurgling in pleasure, you start cumming yourself, all the while appeasing your demonic masters by servicing your once lover.\n\n", false);
	}
	else Render.text("As if reading your thoughts, the hands stop massaging, and their owner comes in front of you, presenting you with a meaty, throbbing cock.  Happy to be of service, you spread your jaws and engulf as much of the great penis as you can, until you feel it pouring his unholy load into your stomach.  Gurgling in pleasure, you start cumming yourself, all the while attending to one or more of your demonic masters.\n\n", false);
...
private function succubusTalkOne():void {
	...
	Render.text("She runs a stocking covered foot up your leg and thigh, almost to your groin.  Giggling, the succubus pulls it away and asks, \"<i>So are you ready and willing?</i>\"");
...
private function succubusBadEndPartTwo():void {
	...
	if (player.biggestLactation() < 1) { //Tits – regular
		Render.text("Your " + nippleDescript(0) + "s begin prodding painfully against your " + player.armorName + ", every touch serving to make them harder and more erect.  ");
	}
	else if (player.biggestLactation() < 3) { //Tits – lactating
		Render.text("Your " + nippleDescript(0) + "s get painfully hard as you feel milk begin backing up inside your " + allBreastsDescript() + ".   The succubus glances down mischieviously as her hands begin to grope you through your " + player.armorName + ", squeezing out a few drops of milk.  ");
	}
	else { //Tits – megalactating
		Render.text("Your " + nippleDescript(0) + "s get painfully hard as milk begins drooling down your over-productive chest, making your " + player.armorName + " slide across your leaky milk-spouts in an agonizingly pleasurable way.  "); 
	}
	if (player.lowerBody.cockSpot.count() == 1) { //Cock – single
		if (player.cockArea(0) < 30) {
			Render.text("Swooning from sudden blood loss, you struggle to maintain the kiss as your body takes your " + cockDescript(0) + " to full hardness in seconds.  ");
		}
		else if (player.cockArea(0) < 100) { //Cock – single big
			Render.text("Nearly blacking out, you struggle to stay awake as your body shifts your blood to your disproportionate " + Appearance.cockNoun(CockType.HUMAN) + ".  ");
		}
		else Render.text("As you struggle not to lose consciousness, you realize your over-aroused body had pumped most of your blood to your over-sized " + Appearance.cockNoun(CockType.HUMAN) + ", which now droops to the floor, pulsing hotly.  "); //Cock -megahuge
	}
	//DO MULTIZ
	if (player.lowerBody.cockSpot.count() > 1) Render.text("The feeling of light-headedness nearly robs you of consciousness as your " + player.multiCockDescript() + " fill with blood, pulsating with arousal as they reach full size.  ");
	if (player.lowerBody.vaginaSpot.count() > 0) { //Vagooooz
		if (player.lowerBody.vaginaSpot.list[0].clitLength >= 4.5) { //'uge clit
			Render.text("Popping from between your thighs, your " + clitDescript() + " responds to the sheer hotness of the situation by making itself known.   You squeeze your legs tightly together, hungry for additional sensation.  ");
		}
		else if (player.lowerBody.vaginaSpot.list[0].clitLength > 1) { //big clit
			Render.text("A wave of pleasure erupts from between your legs as your " + clitDescript() + " pops free.    You squeeze your legs tightly together, hungry for the additional sensations.  ");
		}
		else if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLICK) { //slick
			Render.text("Squishing wetly, your bottoms become soggy with the flood of fluids leaking from your " + vaginaDescript(0) + ".   Your legs spread apart on their own, begging for any kind of intrusion.  ");
		}
		else Render.text("Groaning softly, you feel yourself getting wetter and wetter with arousal.  You wish your sticky bottoms were off so you could let something into your " + vaginaDescript(0) + ".  "); //normal
	}
	Render.text("\n\n"); //New PG
	...
	Render.text("The succubus pushes a button on the wall, and a number of strange looking suction tubes and hoses drop down from above you.   Moving with practiced efficiency, she hooks a ");
	if (player.upperBody.chest.count() == 1)
		Render.text("pair of");
	else Render.text("number of");
	Render.text(" hoses to your breasts, ");
	if (player.lowerBody.cockSpot.count() == 0) {
		if (player.lowerBody.vaginaSpot.count() > 0)
			Render.text("and a vacuum pump to your clit.  ");
		else {
			Render.text("and another vacuum pump to your bare groin.  In seconds a wet fleshy growth erupts.  You have grown a cock!  ");
			player.lowerBody.cockSpot.add(new Cock());
		}
	}
	else if (player.lowerBody.vaginaSpot.count() > 0)
		Render.text("a vacuum pump to your clit, and a pump many times bigger than your " + cockDescript(0) + " to it.  ");
	else Render.text("and a pump many times bigger than your " + cockDescript(0) + " to it.  ");
	Render.text("At first there is only a gentle suction, you assume in order to keep them in place.  Unfinished, your captor places something large and hollow against your backdoor");
	if (player.lowerBody.vaginaSpot.count() == 0)
		Render.text(".");
	else Render.text(" and an ever larger dildo against your " + vaginaDescript(0) + ".  It seems to pulse and wiggle with a life of its own, rubbing the bumps of its lumpy head against your lips.");
	Render.text("  You swoon as you hear the solid click of a button being pushed, and all at once all devices attached to you leap to life.");
...
private function succubusBadEndPartThree():void {
	...
	//Nipples
	Render.text("The suction pulls squirt after squirt of milk from your breasts as your " + allBreastsDescript() + " start to grow, swelling heavier as they enlarge to produce more milk.  You squeal with delight as your nipples turn black, tainted by corruptive chemicals that are slowly dripped into you.  ");
	//Dick  
	if (player.lowerBody.cockSpot.count() > 0) {
		Render.text("The vacuum-pump on your cock noisily sucks down all your spoo, ");
		//High cum variant
		if (player.cumQ() > 300)
			Render.text("struggling with the amount you put off.  Grinning, the succubus reaches over and flips a lever.  You feel the suction increase as the machine is turned up to accommodate your altered physique.  ");
		//else
		else Render.text("the suction and drugs make it so easy to just keep cumming and cumming.  ");
		//either or:
		Render.text("Dimly, you feel a needle lodged in your taint, pumping your prostate full of something.  Your " + cockDescript(0) + " begins growing mid-orgasm, the skin turning a deep purple even as small nodule-like bumps form all over it, rapidly becoming a bloated parody of its demonic counterparts.  ");
	}
	//Puss Orgasmz
	if (player.lowerBody.vaginaSpot.count() > 0) Render.text("Clenching tightly, your " + vaginaDescript(0) + " squeezes tightly on its intruder as its repeatedly violated by the machines.  ");
	//End
	...
	if (player.statusAffects.get("$1").value3 == 1) {
		Render.text("Later on, you are briefly pulled out of your reverie by a familiar warm fluid flowing down your throat.  You come to your senses and see Marble looking down at you with an odd expression on her face.  ");
		...
		Render.text("You are given a brief moment of clarity as you see Marble standing in front of you.  ");
		Render.text("She seems to be in a state of bliss. Looking down, you see that she is wearing some kind of pair of pink panties.  Marble gasps and the surface of the panties ripples; it's a living thing!\n\nYou look around and realize you aren't alone.  ");
	}
	else Render.text("Later on, in a moment of clarity, you look around and realize you aren't alone.  ");		
	...
	if (player.statusAffects.get("$1").value3 == 1 || player.statusAffects.has("CampMarble")) Render.text("  With Marble here too, you'll be around for a long time.");
	gameOver();
}
...
public function succubusVictoryRape():void {
	...
	if(player.lowerBody.cockSpot.count() > 0 && (player.gender != 3 || rand(2))) {
		...
		if(player.cockArea(0) > monster.vaginalCapacity()) {
			...
			Render.text("Still, your possessed maleness is far from finished as it continues to slide along her belly and between her still-dripping tits, and before long you feel another orgasm building.  You hang on for dear life, hoping just to stay conscious through the ordeal...\n\n\nHours later you pull away, sated.  For now.", false);
		}
		//((IT FITS))
		else {
			Render.text("You plunge in to her velvety depths and feel her rippling cunt-muscles contract tightly around you for a perfect fit.  She gasps as each cock-distorting nodule bounces her two-inch clitty, making your mirror image moan like a bitch in heat.  The corrupted fluids dripping from her snatch squelch loudly, making your groin burn with pleasure.  ", false);
			if(player.lowerBody.balls > 0) Render.text("Looking down, you even see the veins on your sack darkening to solid black as the corruption begins tainting your sperm-factories.  ", false);
			Render.text("You pull back, letting the ring of pointed fleshy barbs spring free before plunging them back in. The pleasure makes you swoon, nearly forcing you to orgasm on the spot.  ", false);
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) Render.text("Your female double reaches down to pinch your nipple, spurring you on, \"<i>Please, could you like, cum for me?</i>\"", false);
			//New PG
			Render.text("\n\n", false);
			player.lowerBody.cockSpot.list[0].cockType = CockType.DEMON;
			Render.text("The succubus bucks her hips weakly, still clearly defeated, but egging on your orgasm as dark fluids squelch free from her quim.  \"<i>Please... can't you see how hot you're making me?  I've made your cock perfect, so please make me cum!</i>\" she begs as she quivers with delight.  ", false);  
			Render.text("You pause to think about it, not noticing that your groin is pounding away with furious energy, splattering cum and pre over her thighs as your new cock's instincts take control from your waist down.  Gasping with sudden pleasure, you feel a flash of heat pass through your tainted meatstick as an orgasm builds.", false);
			//New PG
			Render.text("Each of the new bumps and rounded spines of your " + cockDescript(0) + " flood with blood, doubling in size as orgasm overtakes you.  ", false);
			Render.text("Your hips bury your entire length inside her, acting on their own as your " + cockDescript(0) + " clenches, pouring more and more spunk inside her", false);
			if(player.lowerBody.balls > 0) Render.text(" as your balls empty their load queue", false);
			Render.text(".  ", false);
			//Big cum
			if(player.cumQ() >= 50 && player.cumQ() < 400) Render.text("You inhale as black cum spatters from her entrance, her belly distending slightly as you empty what feels like a gallon inside her.  ", false);
			//((Ginormohuge)) 
			if(player.cumQ() >= 400) Render.text("Gasping in pleasure and surprise, you marvel as her belly visibly expands with each eruption of your dark load.  At first it looks like a tiny belly, but by the time the orgasm finishes, your girlish double looks like a woman in her ninth month of pregnancy – with twins.", false);
	...
	//FEMSAUCE
		...
		if(monster.HP < 1) Render.text("Your foe staggers and falls hard on her ass, utterly defeated.  Her bruises and lacerations slowly fade and heal, regenerating with the aid of her demonic powers.  You easily tear through her clothes, leaving only the damaged stockings that gird her legs. It doesn't take much to force her down on her back and straddle her as you disrobe, ready to take your pleasure.\n\n", false);
		else Render.text("Your foe drops to her knees, stuffing three digits into her greedy snatch as arousal overcomes her desire to subdue you.  With great care, you approach your insensible enemy and tear off her clothes, leaving her wearing only the remains of her stockings as you force her down on her back.  As if possessing a will of their own, her legs lewdly spread as you disrobe.\n\n", false);
		Render.text("Her budding clit rises from between her folds, hardening like a tiny three inch dick.\n\n", false);
		if(player.biggestLactation() > 1) {
			Render.text("<b>You could scissor with her, or maybe force-feed her some of the milk you've backed up.  Which will it be?</b>", false);
...
public function dungeonSuccubusForceFeed():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 6) Render.text(", nearly smothering the succubus with the swell of tit-flesh", false);
	Render.text(".  You jiggle back and forth, lining up a " + nippleDescript(0) + " with the demon's parted lips.  You press your weight down threateningly, making it clear you could suffocate her with a boob.\n\n", false);  
	Render.text("\"<i>Drink up bitch, these tits are full!</i>\" you command.\n\n", false);
	Render.text("The succubus tentatively takes a " + nippleDescript(0) + " into her mouth, sighing at the tangy taste of your sweat-drenched skin.  Her lips gently suckle, wrapping around the perky hardening nub as it fills with arousal and milk.  You feel something let go inside your " + breastDescript(0) + " and the succubus smiles, now working to free more of your trapped cream.\n\n", false);
	if(player.biggestLactation() < 2) Render.text("Her flexible tongue easily curls around one of your " + nippleDescript(0) + "'s, letting her pull and tug on it as she increases the suction from her lips.  Your body rewards her efforts with a faster flow of milk that she sucks down as if she were starving.\n\n", false); 
	if(player.biggestLactation() >= 2 && player.biggestLactation() < 3) {
		Render.text("Her flexible tongue wraps around your milk-engorged nipple, pulling it tightly as she increases the suction of her lips.  Your body wastes no time rewarding her and she begins gulping down a steady supply of your breastmilk with a pleased expression on her face. You muse to yourself that perhaps succubi are masochists as breast-milk runs freely from your un-milked ", false);
		if(player.totalBreasts() > 2) Render.text("tits.\n\n", false);
		else Render.text("tit.\n\n", false);
	}
	if(player.biggestLactation() >= 3 && player.biggestLactation() < 4) {
		Render.text("Her flexible tongue wraps around a milk-swollen nipple, immediately squeezing out a jet of thick breast-milk.  The tongue squeezes and tugs while the succubus ramps up the suction between her thick bee-stung lips.  Your body rewards her with fountains of milk, forcing her to visibly gulp and struggle to keep up with the heavy flow.  Milk runs freely down the " + player.skinDesc + " on your chest, pooling around the succubus' groin and your own as the unattended nipple", false);
		if(player.totalBreasts() > 2) Render.text("s", false);
		Render.text(" can't help but dribble in sympathy.\n\n", false);
	}
	if(player.biggestLactation() >= 4) {
		Render.text("Her flexible tongue wraps around a milk-bloated nipple, immediately releasing a massive spray of cream that pours into her gullet, nearly choking her.  You stifle a giggle and pull her closer.  Thankfully, her determined tongue manages to stay in place and start tugging your nipple about, releasing even more of your over-large milk production.  She struggles, her throat and cheeks bulging from your explosive output of milk, until it overwhelms her and begin to pour out of her nose.  More milk pours from your unoccupied nipple", false);
		if(player.totalBreasts() > 2) Render.text("s", false);
		Render.text(" in sympathy, drenching your " + player.skinDesc + " with creamy goodness until it puddles on your captive demon.\n\n", false);
	}
	if(player.upperBody.chest.countNipples() == 2) Render.text("As your first nipple drains, you move her to your other breast, being sure to motivate her cunt by grinding it with your " + player.foot() + ".  She squeals and sucks harder, emptying the last of your milk with a cute burp.  ", false);
	if(player.upperBody.chest.countNipples() > 2 && player.upperBody.chest.countNipples() < 6) Render.text("As your first " + nippleDescript(0) + " drains, you forcibly move her to the next, unleashing a fresh batch of milk for her to feast upon.  Eventually, it too dries up, so you migrate her onwards to your next " + nippleDescript(0) + ".  After she drains each of your " + num2Text(player.upperBody.chest.countNipples()) + ", you smile happily at your now emptied breasts.  ", false);
	if(player.upperBody.chest.countNipples() >= 6) Render.text("As your first " + nippleDescript(0) + " drains, you force her over to the next, unleashing even more milk for her to feast upon.  In time, that " + nippleDescript(0) + " also empties and you rotate her on to the next.  The cycle repeats seemingly endlessly as you work her from nipple to nipple, relieving the insistent pressure of your breasts and slowly filling her with your milk.  ", false);
	if(player.averageLactation() * player.totalBreasts() < 6) Render.text("Her belly bulges slightly from all the breast-milk she's consumed.\n\n", false);
	else Render.text("The succubus looks bloated and pregnant from all the milk you've forced into her.  She sloshes and moans incoherently from the strain of it all.\n\n", false);
	Render.text("Despite the relief your " + allBreastsDescript() + " now feel, your " + vaginaDescript(0), false);
	if(player.lowerBody.cockSpot.count() > 0) Render.text(" and " + player.multiCockDescriptLight() + " feel hungrier than ever.  ", false);
	else Render.text(" feels hungrier than ever.  ", false);
	Render.text("You shove your crotch into your milk-dazed foe's white-stained visage, grinding your " + vaginaDescript(0) + " into her mouth until you cum all over her", false);
	if(player.lowerBody.cockSpot.count() == 0) {
		temp = rand(3);
		if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLAVERING) Render.text(", soaking her with girl-cum.", false);
		else {
			if(player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.WET) Render.text(", slicking her face with girlish cum.", false);
			else Render.text(", drenching her with a deluge of girlcum.", false);
		}
	}
	else Render.text(", and unloading a wave of hot spunk into her hair.", false);
	...
	if(player.lowerBody.cockSpot.count() > 0) Render.text(" and a thick layer of spunk", false);
	Render.text(".", false);
	Render.text("\n\nYou turn away with a bemused sigh.  When you glance back, she has vanished!", false);
	dynStats("lus", -50);
	cleanupAfterCombat();
...
simpleChoices("Fight", startIncubusFight, "Trade", tradeComic, "Submit", submitToIncubus, "", null, "", null);
		...
		Render.text("The incubus speaks to you with a calm, deep voice, \"<i>And so the insect, unaware of its path, stumbles directly into the spider's web.  Tiny insect, you have little to offer me, but everything to offer our facility.  Why don't you come along quietly?</i>\"");
...
public function incubusLossRape():void {
	...
	//Tentacle gangbang
		...
		if(player.lust > 99) Render.text("Molten arousal pumps through your veins, burning away your reason with an unquenchable desire to mate. You drop your top, exposing your " + allBreastsDescript() + " to your foe in a submissive display, ", false);
		else Render.text("You lower your top, exposing your nubile form to your foe in a submissive display, ", false);
		Render.text("lowering your eyes and fixating on his now-rigid demonic member.  Right before your eyes, it begins splitting and dividing into thinner prehensile penises that squirm about in the air, each one reminding you of a snake on the prowl.  ", false);
		if(player.stats.cor < 80) Render.text("In a disgusting display", false);
		else Render.text("As you grope yourself noisily with your hand into your undergarments, a salacious smile on your lips", false);
		Render.text(", you watch as his pricks pulse and thicken out until their masses are as wide as his original dick.\n\n", false);
		if(player.stats.cor >= 80) Render.text("As you realize their size and number, you open your eyes wide and smile broadly, reflexively spreading your legs wide, practically begging him to fuck you.  ", false);
		Render.text("In a flash, each fat tentacle-cock whips out and surrounds your body in slick demon-flesh.  The tentacles constrict, working in pairs to take off every piece of your " + player.armorName + ".  ", false);
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) Render.text("They make sure to rub each of your breasts, spending a few seconds smearing slick pre-cum into your " + nippleDescript(0) + ".  ", false);
		Render.text("A pair of them slides into your undergarments, pressing against your needy crotch and teasing your " + assholeDescript() + " with more slick demonic cum.  ", false);
		if(player.stats.cor < 80) Render.text("You wriggle and whine,", false);
		else Render.text("You grab the thick tentacle-cock working on your cunt with both hands, as you can barely grab it with one. Then,  while letting out moans fit for a bitch in heat, which you are, you begin", false);
		if(player.lowerBody.vaginaSpot.count() > 0) Render.text("squeezing your legs around them and grinding your " + clitDescript() + " against the oddly textured demon-cock.", false);
		else {
			if(player.lowerBody.balls > 0) Render.text("grinding down against the ribbed and textured demonic cock as it slides between your " + ballsDescriptLight() + ".", false);
			else if(player.lowerBody.cockSpot.count() > 0) Render.text("squeezing your legs around the thick demonic flesh as a steady dribble of pre-cum drips from your " + cockDescript(0) + ".", false);
			if(player.gender == 0) Render.text("grinding suggestively on your captor's rods, barely noticing as the last of your " + player.armorName + " falls away.", false);
		}
		Render.text("\n\n", false);
		//FUCKKKING
		//Female paragraph
		if(player.lowerBody.vaginaSpot.count() > 0 && (player.lowerBody.cockSpot.count() == 0 || rand(2))) {
			Render.text("The incubus at last decides to tend to your over-aroused body and pulls you off the ground with his tentacles, suspending you in mid-air.  ", false);
			if(player.stats.cor < 80) Render.text("You feel your " + player.legs() + " lifted and pulled tight as countless demonic cocks encircle your body, binding and constraining you further.  You whimper as a demonic tentacle probes your back door while a thicker one lines itself up just below your " + clitDescript() + ".  ", false);
			else Render.text("As he lifts you, you spread your legs, showing him your cunt which is dripping wet from anticipation, and grabbing your ass to give him a perfect view of your " + assholeDescript() + ".  You then beg him, between lecherous moans, to use his three biggest tentacles, as you want to have the most sensations.  With a sadistic smile, he lines up two tentacle cocks about as big as your arm over your " + vaginaDescript(0) + " and your " + assholeDescript() + ".  ", false);
			Render.text("You then giggle and try to wiggle your " + hipDescript() + " forward, begging for him to take you and quench the fire burning in your " + vaginaDescript(0) + ".  The incubus obliges, pressing forth with both drooling members and simultaneously plugging your front and back doors.  ", false);
			if(player.stats.cor < 80) Render.text("You gasp from pleasure and surprise, ", false);
			else Render.text("You open your mouth wide, as the enormous tentacle-cocks force their way in, dripping demonic pre-cum all over. Quickly, it acts as additional lubricant, and the pain largely subsides.  You find that these massive demonic cocks' size is perfect to stimulate, in an incredibly pleasurable way, all sides of your cunt at once, and to stretch your ass just a bit over your preferred size.  As the incubus starts moving his appendages in rhythm, you're lost in heavenly pleasure, eyes closed, letting out deafening moans of lust, your legs and arms dangling without any thought for dignity.  You are deeply ", false);
			Render.text("enjoying the knobbed texture of his shafts as you're double-penetrated by a single demon. The incubus smirks as another cock-tentacle wraps itself up around your neck like a shiny student collar and plugs your noisy little mouth.  You groan into his member as you're ", false); 
			if(player.stats.cor < 80) Render.text("squeezed and caressed by the writhing tentacle-pricks in and around your body, lost in the pleasure and taste of demonic pre-cum.\n\n", false);
			else Render.text("getting roughly fucked by the two tentacle-cocks at the same time.  Taking the tentacle-cock in your mouth with both hands, you eagerly swallow every bit of demonic pre-cum, then suckle on the huge cock-slit. \n\n", false);	
			
			//FemCum
			if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) Render.text("You nearly cum on the spot when the cock fucking your pussy loops its length around your " + clitDescript() + ", the cum-slickened coils driving you mad with pleasure as they coil, slide, and jerk around your clit as if it was a cock.  ", false);
			else Render.text("You nearly cum on the spot when the cock fucking your pussy curves up to rub its textured nodules against your " + clitDescript() + ".  ", false);
			player.cuntChange(player.vaginalCapacity()*.8, true);
			if(player.stats.cor >= 80) Render.text("You cum more times than you are able to count, each time causing a tightening of your fuckholes, which increases the rubbing against the demonic nodules and sends another wave of pleasure to your dazed brain.  You begin to drool freely, reveling in this most unholy mating.  ", false);
			Render.text("The prick in your mouth surges forward, sliding deep into your throat.  The coils around your neck tighten in response, choking your neck into a tight cock-sleeve as you feel bulges of cum moving along its length.  In moments you feel your belly starting to grow full, sloshing with cum as you become desperate to breathe.  The tentacles lodged in your " + assholeDescript() + " and " + vaginaDescript(0) + " react in similar fashion, stretching you wide as they begin pumping your body full of vast quantities of spunk.  A few free tentacles begin spurting gobs of the white stuff onto your " + player.skinDesc + ", soaking you in the stuff as you black out from a combination of oxygen deprivation and pleasure.", false);
			player.orgasm();
			dynStats("cor", 25);
		...
		//Male/Genderless
			...
			Render.text("The incubus at last decides to tend to your over-aroused body and pulls you off the ground with his tentacles, suspending you in mid-air.  You feel your " + player.legs() + " lifted and pulled tight as countless demonic cocks encircle your body, binding and constraining you further.  You whimper as a demonic tentacle probes your back door, slathering your " + assholeDescript() + " with a nubby cock-head as it slowly presses forward and fills you with incredible tainted warmth.  Each nub and ridge that grinds past your sphincter bumps against the organs in your backdoor and fills you with pleasure that only increases as more of the demonic pre-cum is wicked into your body.  The gasp you started fades into a soft croon of pleasure before being muffled entirely by another thick prick.  The shaft belonging to the dick in your mouth curls around your neck like the collar a pet or submissive slut would wear.", false);
			if(player.lowerBody.cockSpot.count() > 0) Render.text("  As your body's orifices fill with more and more dripping demonic dick-flesh, your " + cockDescript(0) + " becomes painfully hard.  Another tentacle wastes no time in wrapping itself tightly around the base like a cock-ring.  The rest of the demon's prehensile tool slides along your shaft, curling around to squeeze and jerk you off.", false);
			if(player.lowerBody.balls > 0) Render.text("  You groan around the dick plugging your throat as another tentacle-like appendage wraps around your " + sackDescript() + ", pulling your " + ballsDescriptLight() + " down and gently squeezing them.", false);
			Render.text("\n\n", false);
			//Genderless Orgasm
			if(player.gender == 0) Render.text("The tainted cum mixed with the sensation of fullness provide you with pleasures beyond what your genderless body could accomplish on its own.  You writhe as the demon face-fucks you with one tendril while another continues to bury itself ever-deeper into your abused " + assholeDescript() + ".  ", false);
			if(player.lowerBody.cockSpot.count() > 0) Render.text("The cock-tentacle around your " + cockDescript(0) + "  increases the pace of its stimulation as it begins to spurt hot wet cum over you, giving it lubrication as it jacks you off while staying tight around your base to prevent you from an orgasming.  ", false);
			Render.text("You feel cum pulse through the tentacles encircling you as the incubus loses control of his tentacles.  Cum pumps into your belly, suffusing you with drug-like warmth as the tentacle around your neck pulls tight enough to squeeze the cock inside your throat.  You squirm and gasp for oxygen as spooge begins unloading into and around your body to the point where you aren't sure where your body begins and the demonic-spunk ends.  You twitch in what you assume is orgasm as you fight to breathe; all the while more cum is squeezed into your stuffed belly and ruined anus. The tentacle in your ass backs out slowly, having filled every inch of your intestines with cum, until it pops free with a splatter.", false);
			Render.text("\n\n", false);
			if(player.lowerBody.cockSpot.count() > 0) {
				Render.text("Being so thoroughly used and stimulated pushes you over the edge of orgasm, and your ", false);
				if(player.lowerBody.balls > 0) Render.text("balls", false);
				else Render.text("prostate", false);
				Render.text(" unloads with enough force to squeeze past the constrictor clutching at your groin.\n\n", false);  
				//Small cum
				if(player.cumQ() < 50) Render.text("You groan and orgasm with enough force to splatter a few ropes of cum into the sea of demon-spunk that soaks you from head to toe.  ", false);
				//Big cum
				if(player.cumQ() >= 50 && player.cumQ() < 400) Render.text("Your orgasm goes off like a shotgun blast, splattering the incubus with a huge wad of cum.  It's but the first of many, and though each load of jizz is of comparable size, the force behind them diminishes until the last few blasts drip down your body and soak your " + player.legs() + " and " + player.feet() + ".  ", false);
				//Huge cum
				if(player.cumQ() >= 400) Render.text("Your orgasm goes off like a volcano, visibly distending your " + cockDescript(0) + " as a huge wave of cum erupts from your groin, painting the incubus and floor with your spoo.  You cry from the sheer pleasure as the next wave builds and erupts, nearly as large as the last.  The demon-cock controlling your prick aims this blast up, forcing you to soak your " + hairDescript() + " and face with slick goo. The orgasming drags on and on while you slowly turn blue from oxygen deprivation. Before long, both you and the incubus are buried under a wave of white.  ", false);
			}
			Render.text("The feeling is so intense that your " + hipDescript() + " twitch and move of their own volition while your eyes roll back in pleasure.\n\n", false);
			Render.text("You black out just as you feel the cock-tentacle in your throat retracting. You dully feel your body drop to the ground, your pregnant-looking belly sloshing with demon jizz.", false);
			player.buttChange(monster.cockArea(0), true);
			player.orgasm();
			dynStats("cor", 25);
			if (player.findStatusAffect(StatusAffects.DungeonShutDown) < 0)
...
public function incubusVictoryRapeSex():void {
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		Render.text("With a few deft motions, you shift your " + player.armorName + " to expose your ", false);
		if(player.gender == 3) {
			if(player.lowerBody.balls > 0) Render.text(player.multiCockDescriptLight() + ", " + ballsDescriptLight() + ", and " + vaginaDescript(0), false);
			else Render.text(player.multiCockDescriptLight() + " and " + vaginaDescript(0), false);
		}
		else {
			if(player.lowerBody.balls > 0) Render.text(player.multiCockDescriptLight() + " and " + ballsDescriptLight(), false);
			else Render.text(player.multiCockDescriptLight(), false);
		}
		Render.text(".  Having resolved to take the demon's backdoor, you approach his weakened form with brimming confidence.  He looks up, clearly hoping your plan is to squat on his throbbing member.  You dispel his misguided notion when you grab him by the horns and shove his face against the floor. He struggles weakly until you press down harder, making it clear he is to stay in position - on his knees with his head down and his ass in the air.  Circling your prey, you inspect his flawless body and carefully note that the hole at his crotch actually exposes a fair portion of his very supple and surprisingly feminine-looking backside.\n\n", false);
		Render.text("You don't waste any time, gripping your " + cockDescript(0) + " in one hand and ", false);
		if(player.lowerBody.cockSpot.list[0].cockType == CockType.HORSE) Render.text("pressing your thick flare ", false);
		if(player.lowerBody.cockSpot.list[0].cockType == CockType.DOG) Render.text("pressing your pointed tip ", false);
		if(player.lowerBody.cockSpot.list[0].cockType == CockType.HUMAN || player.lowerBody.cockSpot.list[0].cockType.Index > 2) Render.text("pressing your head ", false);
		Render.text("between the incubus' cheeks towards his inhumanly smooth rear-passage.  You gasp in delight at the tight ribbed texture of his asshole as you slide ", false);
		if(player.lowerBody.cockSpot.list[0].cockLength > 10) Render.text("deep inside ", false);
		else Render.text("inside ", false);
		Render.text(".  The demon underneath you grunts in an attempt to sound displeased, but it's plain to see the pre-cum he's dripping all over the floor.  What a slut!  You slap his ass and begin roughly butt-fucking him, panting with each plunge into the depths of his ridged passage, gradually increasing your tempo until your " + hipDescript() + " fill the room with loud slapping noises.", false);
		if(player.lowerBody.balls > 0) Render.text("  Your " + ballsDescriptLight() + " swing freely, smacking into the demon's own and making both of you squeal and dribble more pre-cum.", false);
		Render.text("\n\n", false);
		//(CUM)  
		if(player.lowerBody.balls > 0) Render.text("You feel your " + ballsDescriptLight() + " draw up tight against your body.  ", false);
		Render.text("Warm heat begins to build inside your groin, pooling under the base of your " + cockDescript(0) + ".  You realize you're about to paint this demon's gut with white, the thought only turning you on more.  ", false);
		if(player.cumQ() > 200) Render.text("You groan as you feel your urethra being stretched by the sheer volume of fluid beginning to shoot through it.  ", false);
		Render.text("You throw back your head and cum, slapping the incubus' ass with one hand while you grip and squeeze the jiggling flesh of his other cheek.  ", false);
		if(player.cumQ() < 50) Render.text("A few thick spurts later and y", false);
		if(player.cumQ() >= 50 && player.cumQ() < 400) Render.text("Thick jets of cum pump into the demon's plump backside, soon building up a wave of pressure that pushes back against you.  Y", false);
		if(player.cumQ() >= 400) Render.text("A massive cock-distending bulge of cum works through your shaft, splashing into the demon's rectum in an explosive burst of pleasure. Unfortunately for your victim, it is only the first of many such cum-blasts. In no time flat, jism is spurting from his overfilled rectum while his belly looks a few months pregnant. You feel weak from discharging so much fluid, and y", false);
		Render.text("ou fall back, the fluid of your orgasm dripping from your " + cockDescript(0) + " and the gaping asshole of your latest conquest.\n\nYou turn to gather your " + player.armorName + ", and when you look back the demon is gone, leaving only a small puddle of male fluids in his wake.", false);
		player.orgasm();
		cleanupAfterCombat();
		return;
	}
...
public function omnibusVictoryEvent():void {
	...
	if (monster.lust > 99) {
		...
		Render.text("\"<i>Ooooh no.  You can't tell the other demons I got off so easily!  I'll never live it down,</i>\" she cries, \"<i>You've beaten me, please if you let me go and promise not to tell the other demons I'll use my magic to give you a gift!  My magic is powerful, I can do nearly ANYTHING with it when the subject desires the changes.</i>\"\n\n");
	}
	else {
		Render.text("The omnibus trembles and drops to her knees, utterly defeated.\n\n");
		Render.text("\"<i>Please, if you'll let me go I could use my magics to give you nearly anything you want!  Just please don't tell the other demons what happened here, I'd never live it down,</i>\" she begs.\n\n");
	}
	Render.text("What do you do?  You could use her boon increase the size of your endowments or maybe regain some of your lost humanity!  Or you could play it safe and turn down her offer.  Although then you'd have to decide to let her go or kill her.");
	menu();
...
private function omnibusVictoryGrowBreasts():void {
	...
	//Grow if none
	if (player.upperBody.chest.count() == 0) {
		Render.text("<b>Your chest swells out, forming rounded C-cup globes, capped with tiny erect nipples!</b>");
		player.createBreastRow();
	...
	//Singular row - cup size + 3.  Nipple size to 1" if smaller.
		...
		Render.text("Your " + allBreastsDescript() + " tingle pleasantly as the magic takes effect.  You watch with fascination as they begin to swell up, like sponges exposed to water.  The top of your " + player.armorName + " is pulled tight by change, until your chest seems ready to burst free.  <b>You've gained 3 cup sizes!</b>  ");
		player.upperBody.chest.list[0].breastRating += 3;
		if (player.upperBody.chest.BreastRatingLargest[0].nippleLength < .75) {
			...
			Render.text("Your " + nippleDescript(0) + "s grow hard and sensitive, becoming much more noticeable inside your " + player.armorName + ".  It appears your nipples have grown larger to match.");
		}
	}
	//Multiple Rows...
	else {
		//Top row + 3, all other rows brought up to par.
		Render.text("Your top " + breastDescript(0) + " tingle pleasantly as the magic takes effect.  You watch with fascination as they begin to swell up, like sponges exposed to water.  The top of your " + player.armorName + " is pulled tight by change, until your chest seems ready to burst free.  <b>You've gained 4 cup sizes!</b>  ");
		player.upperBody.chest.list[0].breastRating += 4;
		Render.text("The next row of " + breastDescript(1) + " jiggle and tingle with even more pleasure than the first.  They pulsate for a few seconds, shrinking and growing rapidly until they settle at a size just below your top " + breastDescript(0) + ".  ");
		...
		if (player.upperBody.chest.count() >= 4) {
			Render.text("Your remaining " + breastDescript(3) + " feel so wonderful that you just can't resist cupping and squeezing them as they reshape to fit in perfectly with the rest of your breasts.  ");
		...
		if (player.upperBody.chest.BreastRatingLargest[0].nippleLength < .75) {
			player.upperBody.chest.BreastRatingLargest[0].nippleLength += .5;
			Render.text("Your " + nippleDescript(0) + "s grow hard and sensitive, becoming much more noticable inside your " + player.armorName + ".  It appears your nipples are have grown larger to match.");
		}
	}
	Render.text("\n\n");
...
private function omnibusVictoryGrowDick():void {
	...
	if (player.lowerBody.cockSpot.count() == 1) {
		if (player.demonCocks() < 1) {
			Render.text("You smile, watching your " + cockDescript(0) + " grow longer.  Inches of newfound dick-flesh erupt from your crotch in response to omnibus' dark magics.  Before you can play with your transformed tool, a wave of burning desire passes through you.  ");
			if (player.stats.cor < 80) Render.text("You watch in horror as the skin of your " + cockDescript(0) + " turns shiny and purplish-black.  ");
			else Render.text("Curious, you watch the skin of your " + cockDescript(0) + " turn a shiny-dark purple.  ");
			player.lowerBody.cockSpot.list[0].cockLength += 3 + rand(5);
			player.lowerBody.cockSpot.list[0].thickenCock(2);
			player.lowerBody.cockSpot.list[0].cockType = CockType.DEMON;
			if (player.stats.cor < 50)
				Render.text("Corrupt nodules begin to spring up over its entire length.  <b>Your penis is transforming into a " + cockDescript(0) + "!<\b>  The new nubs wriggle about as they sprout over every inch of surface, save for the head.  Unable to do anything but groan with forced pleasure and horror, you can only watch.  One last batch of nodules forms in a ring around the crown of your " + cockDescript(0) + ", seemingly completing its transformation, until you notice, almost throwing up, that your testicles are also getting covered in black veins under your powerless eyes!  ");
			...
			Render.text("Your " + cockDescript(0) + " leaps forwards, taking to the dark magic with ease.  Inch after inch of new length erupts from your groin as your " + cockDescript(0) + " gets longer and thicker.  It pulsates, as if promising dark pleasure as it settles into its new enhanced size.");
	...
	if (player.lowerBody.cockSpot.count() > 1) {
		...
		if (player.demonCocks() == player.lowerBody.cockSpot.count()) {
			Render.text("Your " + player.multiCockDescriptLight() + " leap forwards, taking to the dark magic with ease.  Inch after inch of new length erupts from your groin as your " + player.multiCockDescriptLight() + " get longer and thicker.  They pulsate, as if promising dark pleasure as they settle into their new enhanced size.");
			while (temp > 0) {
				temp--;
		...
		//Not yet full of demoncocks...
			...
			Render.text("You smile, watching your " + player.multiCockDescriptLight() + " grow longer.  Inches of newfound dick-flesh erupt from your crotch in response to omnibus' dark magics.  Before you can play with your transformed pleasure tools, a wave of burning desire passes through you.  You watch");
			if (player.stats.cor < 80)
				Render.text(" in horror");
			else Render.text(" curiously");
			Render.text(" as the skin of your " + player.multiCockDescriptLight() + " turns shiny and purplish-black.  Corrupt nodules begin to spring up over the entire length of each dick.  ");
			while (temp > 0) {
				temp--;
...
private function omnibusVictoryNormalFace():void {
	...
	if (player.horns > 0 || player.antennae > ANTENNAE.NONE) {
		Render.text("Your forehead itches intensely.  You cannot help but stratch madly at it.  ");
		if (player.horns > 0) {
			Render.text("Your horns fall off, landing on the floor with a heavy thud.  ");
			player.horns = 0;
			player.hornType = HORNS.NONE;
		}
		if (player.antennae > ANTENNAE.NONE) {
			Render.text("Antennae pop free, and float lightly down towards the floor.  ");
	...
	if (player.upperBody.head.earType != EARS.HUMAN) {
		Render.text("Pain erupts from both sides of your head as your ears reform and move, returning to look like your old human ears!  ");
		player.upperBody.head.earType = EARS.HUMAN;
		changed = true;
	}
	//Face
	if (player.faceType != FACE.HUMAN) {
		Render.text("Your facial structure rearranges itself into a normal human visage, exactly like yours was before you came to this horrid place.");
		player.faceType = FACE.HUMAN;
		changed = true;
	}
	//Nothing changed
	if (!changed) Render.text("You tingle briefly but feel no obvious change.  Your face was already fairly human.");
	omnibusVictoryPostBoon();
}
...
private function omnibusVictoryNormalChest():void {
	...
	if (player.upperBody.chest.count() > 1) {
		...
		Render.text("Your chest tingles and begins to feel lighter.  You hastily pull open your " + player.armorName + " and realize you only have " + allBreastsDescript() + " now!  ");
		changed = true;
	}
	//Size!
	if(player.upperBody.chest.list[0].breastRating > 7) {
		Render.text("The weighty flesh that constantly hangs from your chest gets lighter and lighter, vanishing rapidly.  ");
		player.upperBody.chest.list[0].breastRating = 3 + rand(5);
		Render.text("You now have " + allBreastsDescript() + ".  ");
		changed = true;
	}
	//Fix nips
	if (player.upperBody.chest.BreastRatingLargest[0].nippleLength > 1) {
		Render.text("Your nipples shrink down to a more normal size.  ");
		player.upperBody.chest.BreastRatingLargest[0].nippleLength = .75;
		changed = true;
	}
	if (player.upperBody.chest.hasFuckableNipples()) {
		Render.text("The vagina-like openings in your nipples close, sealing themselves shut.  ");
		player.upperBody.chest.list[0].fuckable = false;
		changed = true;
	}
	//Normal chest, normal skin
	if (player.skinType != SKIN.PLAIN) {
		Render.text("The skin on your body itches intensely as it sheds its " + player.skinDesc + ", revealing " + player.skinTone + " skin.  ");
		player.skinDesc = "skin";
		player.skinType = SKIN.PLAIN;
		changed = true;
	}
	//Nothing changed
	if (!changed) Render.text("You tingle briefly but feel no obvious change.  Your chest is already fairly human.");
	omnibusVictoryPostBoon();
}
...
private function omnibusVictoryNormalGroin():void {
	...
	let changed:boolean = false;
	clearOutput();
	Render.text("You feel a strange shivering sensation pass through you.  ");
	//Remove multiple.
	if (player.lowerBody.cockSpot.count() > 1) {
		Render.text("Your " + player.multiCockDescriptLight() + " shiver and retract back towards your body.  When the process finishes you are left with only your " + cockDescript(0) + ".  ");
	...
	if (player.lowerBody.cockSpot.hasCock()) {
		if (player.lowerBody.cockSpot.list[0].cockLength > 12) {
			Render.text("A tingling sensation worms through your " + cockDescript(0) + " as it shrinks down to a more modest eleven inches.  ");
			player.lowerBody.cockSpot.list[0].cockLength = 11;
			changed = true;
		}
		//Super thick nerf
		if (player.lowerBody.cockSpot.list[0].cockThickness > 2) {
			Render.text("Your " + cockDescript(0) + "'s obscene thickness withers down to roughly two inches of girth.  ");
			player.lowerBody.cockSpot.list[0].cockThickness = 2;
			changed = true;
		}
		//Humanitize
		...
		if (player.lowerBody.cockSpot.list[0].cockType == CockType.DEMON) {
			Render.text("Your " + cockDescript(0) + " tingles as the bumps begin to fade.  After a moment the flesh darkens, and every single nodule reappears.  <b>Your corrupt penis resisted the magic!</b>  ");
			changed = true;
		}
		else if (player.lowerBody.cockSpot.list[0].cockType != CockType.HUMAN) {
			Render.text("The inhuman appearance of your " + cockDescript(0) + " shifts, the flesh rearranging itself into a more human configuration.  After a few seconds you have a very normal looking penis.  ");
	...
	if (player.lowerBody.ballSize > 5) {
		Render.text("The " + ballsDescriptLight() + " that constantly pull so heavily on your groin tingle and shrink down to a more managable size.  ");
		player.lowerBody.ballSize = 2 + rand(3);
		changed = true;
	}
	if (changed) Render.text("\n\n");
	//Vajajay
	if (player.lowerBody.vaginaSpot.count() > 0) {
		if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLICK) {
			Render.text("The constant fluids leaking from your " + vaginaDescript(0) + " slow down, then stop.  ");
			player.vaginas[0].vaginalWetness = VAGINA_WETNESS.WET;
			changed = true;
		}		
	}
	//Nothing changed
	if (!changed) Render.text("You tingle briefly but feel no obvious change.  Your crotch isn't really in need of becoming more human.");
	omnibusVictoryPostBoon();
}
...
private function omnibusVictoryNormalLegs():void {
	...
	if (player.lowerBody == LOWER_BODY.HUMAN)
		Render.text("You feel as if you should slap yourself for stupidy.  Your legs are already normal!  You flush hotly as the corrupt magics wash over you, changing nothing.");
	else Render.text("You collapse as your " + player.legs() + " are unable to support you.  The sounds of bones breaking and reshaping fills the room, but oddly you feel no pain, only mild arousal.  You blink your eyes and sigh, and when you look down again <b>you have normal human legs</b>!");
	player.lowerBody = LOWER_BODY.HUMAN;
	if (player.tailType > TAIL.NONE) {
		Render.text("  A moment later, your feel something detach from above your backside.  <b>You no longer have a tail!</b>");
...
public function omnibusLossRape():void {
	...
	else Render.text("Forgetting about the fight, you lean on the desk and slide your free hand under your " + player.armorName + ", seeking any pleasure you can get.  ", false);
	Render.text("Sensing victory, the demoness sidles up next to you and pushes you into a chair.  Incapable of resisting, all you can do is watch as she opens your " + player.armorName+ " for easier access.  She steps back and admires her handiwork as she gives you a thorough looking over.\n\n", false);  
	Render.text("\"<i>I have just the thing for a ", false);
	if(player.gender <= 1) Render.text("man", false);
	else Render.text("woman", false);
	Render.text(" such as you.  I've been crossbreeding the parasites that developed in the deep jungle, trying to create the PERFECT slave-maker.  You get to be my first test subject,</i>\" she says.\n\n", false);
	Render.text(" She sees the look of fear creeping into your eyes and pats you comfortingly, \"<i>Awww don't worry. It'll feel REALLY good.  If anything you should feel honored to be assisting an Omnibus in her experiments.</i>\"\n\n", false);
	Render.text(" She opens one of the desk drawers, and searches briefly before her eyes light up with recognition.  \"<i>Here we are,</i>\" she says as she pulls something free...", false);
...
public function omnibusLossRape2():void {
	...
	//(Multi dicks)
	if(player.lowerBody.cockSpot.count() > 1) {
		Render.text("In her hand is a mass of shining green material.  She turns to face you, bringing it closer and letting you see the lights shift and change on its luminescent surface.\n\n", false);
		Render.text("\"<i>For someone as... different as you, we will have to try this creature.  I've bred it from a mixture of plant-tentacles, dazzle-weed, and what we've taken to calling pussy plants,</i>\" she mentions, her hands working to open the mass on the table.  The interior surface is a mass of slimy undulating protrusions that wriggle feverishly as they are exposed to the air.  She gathers up the thing in her arms while continuing to speak to you, \"<i>You see, my plant will encapsulate your members tightly, wrapping them in sticky wetness.  Its fluids are a perfect blend of aphrodisiacs, lubricants, and will-sapping narcotics.  You'll love it.</i>\"\n\n", false);
		Render.text("You make a desperate attempt to escape her chair, but your body fails to do much more than squirm in place.  She drops the creature squarely into your crotch and hops up onto her desk to watch.  Thousands of tiny wet nodules immediately begin massaging your " + player.multiCockDescriptLight(), false);
		if(player.lowerBody.balls > 0) Render.text(" and " + ballsDescript(), false);
		Render.text(".  You groan as the pleasure washes over you like a wave.  Your squirming stops as your hips begin twitching into the air, as if begging for even more stimulation.  It's not fair how good this feels... you can't help it, it's just too hard to fight.\n\n", false);
		Render.text("You watch with detached fascination as each of your " + player.multiCockDescript() + " is wrapped tightly in shiny green material.  The shape of each penis is still clearly defined under the pulsating green stuff, though you can see it shifting and rippling over your lengths as it pleasures you.  It almost looks like some kind of kinky bondage-toy.  Aware of your attentions, the green stuff squeezes you tightly and begins flashing beautiful bioluminescent color patterns across its surface that scatter your thoughts as you watch.  You blink a few times as the green mass rolls more of itself out, curling over your ", false);
		if(player.lowerBody.balls > 0) {
			Render.text("balls", false);
			if(player.lowerBody.vaginaSpot.count() > 0) Render.text(" and " + vaginaDescript(0), false);
		}
		else if(player.lowerBody.vaginaSpot.count() > 0) Render.text(vaginaDescript(0), false);
		else Render.text("taint", false);
		Render.text(", sliding up your abdomen, and oozing down over your hips.  As it spreads the colors fill more and more of your head, clearing away your thoughts of resistance.\n\n", false);
		Render.text("A soothing female voice talks to you from somewhere, \"<i>Did I mention it's specifically tuned to ensnare the conscious mind with it's pretty colors?  I must have forgot.  Well, I see you've discovered it on your own.  The colors are just so perfect for opening your mind to me, aren't they?  They just chase away your thoughts and let my words slip deep into your subconscious.  I bet it feels nice to just focus on the colors and let my pet tease your cocks, doesn't it?</i>\"\n\n", false);
		Render.text("You nod without any awareness of the act.\n\nThe voice laughs and continues while the creature reaches around your " + hipDescript() + " and slides a feeler between your cheeks, completing the tight loop around your groin, \"<i>That's good.  You want to let the creature cover as much of you as it wants.  Being sex-food for a symbiotic plant is arousing beyond measure.</i>\"  You feel the creature licking at your " + assholeDescript() + " until it relaxes, and then slides something inside.  A warm wetness spreads through your bowels as something begins caressing your prostate from inside you.\n\n", false);
		Render.text("Overloaded with pleasure, you feel your " + player.multiCockDescriptLight() + " pulse and cum, creating translucent green cum balloons the size of ", false);
		if(player.cumQ() < 50) Render.text("apples ", false);
		if(player.cumQ() >= 50 && player.cumQ() < 300) Render.text("cantaloupes ", false);
		if(player.cumQ() >= 300) Render.text("watermelons ", false);
		Render.text("at the end of each of your dicks.  The creature's flashing intensifies while your hips quake uncontrollably, pumping the last of your load feeds into the wonderful plant.  The light-show grows brighter, totally emptying any remaining stray thoughts and leaving you feeling wonderfully open.\n\n", false);
		Render.text("\"<i>Being used for your cum is great,</i>\" says the voice and you agree, it is great.\n\n", false);
		Render.text("\"<i>Your greatest fetish is allowing demonic creatures to feed on your cum,</i>\" she says, and it feels so right.  Your cum is meant for demons and plants to feast on.  Just the thought makes you want to orgasm again.\n\n", false);
		Render.text("\"<i>Since you provide food-source, that must make you livestock.  You like being livestock.  Livestock don't have to think.  Livestock follow orders.  Best of all, as livestock you can live your favorite fetish of being milked of all your cum, every hour of every day,</i>\" the voice says, filling your mind with new thoughts.  Of course it's right, you can just let a demon or tentacle plant milk you and do all the hard stuff, like thinking.  All you have to do is cum.  The thought makes you shiver as the plant-suit absorbs the encapsulated bubbles of jizz.  The dazzling lights grow even brighter as it takes in the nutrients.\n\n", false);
		Render.text("*FLASH* \"<i>You want to cum for the plant.</i>\"\n\n", false);
		Render.text("Tendrils of plant crawl up your belly, coating you in slime as they massage every inch of you.\n\n", false);
		Render.text("*FLASH* \"<i>You need to cum for the plant.</i>\"\n\n", false);
		if(player.upperBody.chest.count() == 1) Render.text("They reach the lower curve of your breasts.\n\n", false);
		if(player.upperBody.chest.count() > 1) Render.text("They slide over your lowest pair of breasts, encapsulating them in wriggling tightness.\n\n", false);
		Render.text("*FLASH* \"<i>You love cumming for anything and anyone.</i>\"\n\n", false);
		if(player.upperBody.chest.count() == 3) Render.text("Your middle breasts tingle with absolute pleasure as they too become engulfed in tightness.\n\n", false);
		if(player.upperBody.chest.count() == 2) Render.text("You groan as the plant grows up the summit of your top breasts, coating the bottom half of your aureola.\n\n", false);
		if(player.upperBody.chest.count() == 1) Render.text("Your " + nippleDescript(0) + "s become hard as steel as the wave of slick pleasure washes over them.\n\n", false);  
		Render.text("*FLASH* \"<i>You love being told to orgasm.</i>\"\n\n", false);
		if(player.upperBody.chest.count() == 1) Render.text("The wriggling mass slides up the top-most parts of your breasts, narrowing into two tiny tendrils that loop around your neck.\n\n", false);
		if(player.upperBody.chest.count() >= 2) Render.text("The wriggling mass climbs your top pair of breasts with ease, wrapping your diamond-hard nipples in slime and sensation.  It continues climbing upward, narrowing into two bands that loop around the back of your neck.\n\n", false);
		Render.text("*FLASH* \"<i>To orgasm is to obey.  You love to orgasm.  You love to obey.  You love to obey my voice more than any other.  Obeying my voice gave you these orgasms.  Since you love to obey me, you must be my pet.</i>\"\n\n", false);
		Render.text("Your mistress' OTHER pet wraps around your neck, forming a choker comprised of shifting green colors.  You smile as you realize it is done - you've become one of her pet cattle.  Your body is wrapped in an emerald sea of shifting pleasure, just like your mistress wanted.  If it weren't for the obvious bulges of your " + player.multiCockDescriptLight() + ", you'd look to be wearing an extraordinarily revealing one piece swim-suit.  The constant teasing at your crotch continues, and you stay rock-hard, even though you just came.  The idea of being milked to feed your new clothing just turns you on so much that you bet you're leaking constant streams of pre-cum for your new green master.\n\n", false);
		Render.text("The flashing subsides, and your new thoughts rush into the void.  You immediately begin masturbating your encapsulated members as you seek to obey.  To orgasm is to obey.  To obey is to orgasm.  You discover that you can feel every touch through the skin of your 'clothing'.  You increase the tempo, knowing that your orgasm will be feeding the creature that now lives on you, fulfilling your deepest darkest desires.  You cum again, just as hard as before, inflating " + num2Text(player.lowerBody.cockSpot.count()) + " shiny green balloons with the proof of your obedience.\n\n", false);
		
		if(player.statusAffects.has("CampMarble")) {
			Render.text("Suddenly, a loud scream is heard down on the factory floor. You and your mistress turn to see Marble dashing up the stairs to the foremen's office.  Your mistress looks over at her and says with some amusement, \"<i>Oh ho!  So another cow has come to join in the fun.</i>\"\n\n\"<i>Sweetie! What has she done to you?</i>\" Marble exclaims, \"<i>What has she put on you?!</i>\"\n\n\"<i>Oh, so you knew this girl?</i>\" your mistress asks you, \"<i>It's a Lacta Bovine from the looks of it, so it seems this time I'll be adding a real cow to the pens.</i>\"  Marble turns to your mistress and brandishes her hammer, but the horror from the thought of your mistress being hurt causes you to spring forward and grab Marble.  The brief distraction gives your mistress a chance to sink a syringe into Marble's shoulder, and within moments she slumps onto the ground unconscious.\"\n\n", false);
			Render.text("Your mistress turns back to you and smiles.\n\n\"<i>Well, she should make a fine replacement for you in the pens,</i>\" she says before tapping her chin thoughtfully and looking back at you, \"<i>Really is convenient that I don't have to worry about my new pet dying on me now, hun.</i>\"  Then she pushes you back into the chair and says \"<i>But first...\"\n\n", false);
			
		}
		else Render.text("Your mistress looks down with approval and speaks, \"<i>Very good.  ", false);
		Render.text("I want you to stay here and cum 'til morning.  My pet needs lots of nutrition to recharge, and I have plans for new ways to teach you to obey tomorrow.</i>\"\n\n", false);
		Render.text("Happy to have such a wonderful task, you spend the next day being bathed in drugged aphrodisiacs, cumming over and over and over.  Every morning the creature flashes you into obedience while the voice teaches you more and more about how to think.  After a week you're the perfect pet.  By the end of your first month of servitude, any memories of your past life are gone.  You spend the rest of your days feeding your mistress and her pet, and helping her refine and breed her pets in order to teach others the way.", false);
		gameOver();
		return;
	}
	//Dick version
	if(player.lowerBody.cockSpot.count() == 1) {
		Render.text("In her hand is a squirming purplish mass.  It has a smooth outer surface, spotted with dark shades of iridescent purple. The opposite side is comprised of a smooth mucusy membrane covered with wriggling pink cilia.\n\n", false);
		Render.text("She leans over you with a predatory smile, \"<i>This little guy is my favorite.  I've even given him a bit of 'field testing'.</i>\"  She gestures towards a small dripping orifice, explaining, \"<i>You see, once I put this on you, it'll open up niiice and wide.  It'll suck your nice little cock into its mouth and starting squeezing and massaging you with each of its tiny tentacles until you can't help but release all your ", false);
		if(player.stats.cor < 33) Render.text("sweet ", false);
		if(player.stats.cor >= 66) Render.text("tainted ", false);
		Render.text("sexual energies deep into its gullet.  And that's just the start!</i>\"  Her hands let go of the squirming mass, dropping it squarely into your lap.\n\n", false);
		if(player.averageCockLength() < 15) Render.text("With one swift motion, the beast engulfs your " + cockDescript(0) + " in its slimy maw.  ", false);
		else Render.text("Distending obscenely, the beast starts engulfing your " + cockDescript(0) + " in its slimy maw, progressing along its entire length until you can no longer see your pleasure tool.  ", false);		
		Render.text("The slimy tentacles waste no time, massaging you with mechanical precision.  You groan in helpless pleasure, growing to painful hardness within the squirming confines of the creature.  Three protrusions sprout from the creature's core, dripping with slime of their own, and covered on the inside with the same wriggling protrusions that now massage your trapped member.  Two curl around your " + hipDescript() + ", while the last one", false);
		if(player.lowerBody.balls > 0) Render.text(" smothers your balls, entrapping them in sticky sensation as it continues across your taint between your butt-cheeks.  ", false);
		else Render.text(" journeys over your taint before travelling between your butt-cheeks.  ", false);
		Render.text("The three tendrils join together in the back, forming a seemless tiny purple triangle.  It really rides up high, tickling your " + assholeDescript() + " with constant teasing.  You're wearing an organic purple thong!\n\n", false);
		Render.text("You try to endure, but the humiliation is too much for you to take.  The pleasure and shame push you past your limit.  You let out a squeal of mixed agony and delight as the proof of your pleasure boils out into the creature.  You pant and twitch, helpless to resist the strength of your orgasm as your jism fills the creature, distorting it visibly around your member.  Sighing, you relax as the assault winds down, the squirming tentacles relaxing noticeably as they work to digest their 'meal'.\n\n", false);
		Render.text("\"<i>Enjoy yourself?  The best part is about to start,</i>\" she says with an evil glint in her eye.  You sit bolt upright as your living thong squirms and shifts, pressing something rigid against the ring of your " + assholeDescript() + ".  You reach down, trying to pull the creature off, but its outer covering is surprisingly hard, and seals almost perfectly against your " + player.skinDesc + ".  You look up with terror in your eyes, a pleading look painted across your face.\n\n", false);
		Render.text("She cocks her head to the side with an inquisitive look and asks, \"<i>So it's found your back door I take it?</i>\"  You nod sheepishly, squealing as the rigid growth pushes through your sphincter, violating you completely.  She continues with a nonchalant tone, though her eyes seem to be drinking in the scene, \"<i>That thing you feel drilling into your ass?  It's a carefully evolved injection appendage.  Don't worry, once it settles in it won't move much.  It's just going to get nice and cozy with your prostate and a few major blood vessels.  Then it's going to reward you for cumming!</i>\"\n\n", false);
		Render.text("You feel it burrow a little deeper, and then curve up.  It presses against something inside of you in a way that makes your " + cockDescript(0) + " twitch uncontrollably.  You're sure that if it weren't for the greedy tentacle-panties around your dick you would've seen a huge dollop of pre-cum squeeze out.  Filled with angst and worry as to what is to come, you ask, \"<i>Ummm, h-h-how is it going to reward me?</i>\"\n\n", false);
		Render.text("She winks, petting the mottled surface of your purple-cock-prison as you feel a sensation of warm wetness in your backside.  At the same time you nearly jump as you feel a painful pinch in your prostate.  The demoness licks her lips and answers, \"<i>Well, it rewards you in two ways pet.  One:  It empties a specially designed cocktail of drugs directly into your bowels, where they'll be absorbed slowly into the body.</i>\"  As if on cue a gentle warmth spreads through your torso, radiating out into your limbs, and settling like calming mist in your head.  You relax utterly, enjoying the feeling in spite of your worries.\n\n", false);
		Render.text("She coos, petting your still-hard member and the creature around it.  Miraculously you can feel both the touch of her silky fingers and the constant pleasurable squirming of the panties themselves.  You twitch your " + cockDescript(0) + " against her hand, giggling happily.\n\n", false);
		Render.text("\"<i>That's right, it's a good reward isn't it?</i>\" she asks as she continues to fondle you, \"<i>those drugs are making you docile and extraordinarily suggestible.  For instance – every time I talk you can feel my hands caressing and fondling your member", false);
		if(player.lowerBody.balls > 0) Render.text(" and teasing your balls", false);
		Render.text(".  You see?  I'm not even touching you anymore and you're still twitching.  My my, what an obedient slave you're going to be.</i>\"\n\n", false);
		Render.text("You pant and groan while she talks to you, still feeling the combined efforts of the panty-creature and your master's wonderful hands, \"<i>And I haven't even told you about the second part of your reward.  If you want me to tell you, you'll need to admit out loud what we both already know – that you're my obedient slave-toy.  Say it toy.</i>\"\n\n", false);
		Render.text("\"<i>I-I-I'm your obedia—ahhh-nt s-s-lave toy,</i>\" you moan.  As soon as the words leave your mouth, you know it's true, but that settles in the back of your mind.  You're eager to know how the creature and your mistress will reward you for being such an obedient-toy.  And of course, to get her talking again so you can feel those smooth fingertips caress you once more.\n\n", false);
		Render.text("\"<i>You really are my good toy already, aren't you?</i>\" she muses, \"<i>You just love pleasing, me don't you toy?</i>\"  You nod feverishly, eliciting a happy laugh from your mistress as she lectures you, \"<i>The second part of your reward is an injection of its venom directly into your prostate.  You may not have noticed with the constant teasing your cock is enduring, but by now your prostate should have doubled in size.  If I ever separate you and your training-suit, you'll notice you're producing so much pre-cum that it's dribbling out ALL the time.  Your orgasms won't get much bigger, but you'll find yourself pouring out pre as you get more and more turned on.  After all, my baby here needs to feed.</i>\"\n\n", false);
		Render.text("Your mistress pats your obscene purple panties tenderly and whispers in your ear, \"<i>Be a good toy and cum for mistress.</i>\"  You smile broadly as your hips piston in the air, as if fucking an imaginary twat.  Cum boils out from your ", false);
		if(player.lowerBody.balls > 0) Render.text(ballsDescriptLight() + " and ", false);
		Render.text(" over-sized prostate, filling the chamber around your cock with thick blasts of seed.  You smile happily as the tentacle-chamber distorts to hold your load, bulging out into a more spherical appearance.  You slump down as your orgasm finishes and you begin to feel even more 'reward' fill your now greedy-hole.\n\n", false);
		if(player.statusAffects.has("CampMarble")) {
			Render.text("Suddenly, a loud scream is heard down on the factory floor. You and your mistress turn to see Marble dashing up the stairs to the foremen's office.  Your mistress looks over at her and says with some amusement, \"<i>Oh ho!  So another cow has come to join in the fun.</i>\"\n\n\"<i>Sweetie! What has she done to you?</i>\" Marble exclaims, \"<i>What has she put on you?!</i>\"\n\n\"<i>Oh, so you knew this girl?</i>\" your mistress asks you, \"<i>It's a Lacta Bovine from the looks of it, so it seems this time I'll be adding a real cow to the pens.</i>\"  Marble turns to your mistress and brandishes her hammer, but the horror from the thought of your mistress being hurt causes you to spring forward and grab Marble.  The brief distraction gives your mistress a chance to sink a syringe into Marble's shoulder, and within moments she slumps onto the ground unconscious.\"\n\n", false);
		...
		Render.text("Your mistress pats your head and whispers commands in your ear while the now-sated slave-making creature devours your cum, turning it into more 'reward'.  You don't pay attention to her words, what's important is serving mistress and cumming for your panty-toy as often as possible.  You don't need to worry, she will tell you what to think.  She's just so perfect and amazing, you don't know why anyone would want to harm her or her wonderful creations.  'Gods it feels good to obey' is the last thought your mind ever thinks for itself.\n\n", false);
		Render.text("In the days to come, you spend your time being teased by your new mistress until you feel as if you'll burst, then being brought to sudden explosive orgasms that fill your panty-prison to capacity.  After every session you black out, but each time you mind less and less.  You wanted to be here, having these wonderful orgasms and obeying your beautiful mistress.\n\n", false);
		Render.text("After a month she starts letting you live without your favorite panties.  You beg her to put them back on you, but she often makes you crawl around the factory, pooling pre-cum everywhere from your swollen prick as you beg her to be put back into the pleasure-panties.  Sometimes, if you're lucky, she'll fuck you, or send you out to catch another adventurer.  There is nothing you love more than cumming into your tentacle-panties while another one of your mistress' creations teaches a slut how to embrace her true nature.", false);
		gameOver();
		return;
	}
	//(Female) 
	if(player.lowerBody.vaginaSpot.count() == 1 || player.gender == 0) {
		Render.text("In her hand is a seamless pair of panties.  Their surface reflects light perfectly, as if its bright pink surface were coated in slippery oil or made from latex.  ", false);
		if(player.gender == 0) {
			Render.text("The demoness smiles with wicked intent and yanks the bottoms of your " + player.armorName + " the rest of the way off.  Your lack of genetalia does not faze her, and she responds by swiftly pulling out a needle and injecting your groin.  In seconds your crotch splits open, revealing a fresh virgin vagina.  Licking her perfect lips with anticipation, she inverts the panties and holds them up for you to see.\n\n", false);
			player.createVagina();
		}
		else Render.text("The demoness smiles with wicked intent and yanks your " + player.armorName + "'s bottom the rest of the way off.  She leans close, smiling and inhaling the scent of your sex, savoring it like a aroma of a fine wine.  Licking her perfect lips with anticipation, she inverts the panties and holds them up for you to see.\n\n", false);
		Render.text("They aren't panties at all, but instead some living creature.  The entire inside surface of the living garment is covered with fleshy pink nodules that wriggle constantly, practically dripping with a pungent lubricant that smells not unlike your own juices.  Horrifyingly, there is a large lump of flesh towards the front.  Its surface is ribbed and pulses, constantly swelling and shrinking.  It's clearly designed to enter the passage of anyone who wears it.  Worse yet is a smaller narrower protrusion on the backside.  This... creature... will certainly do its best to plug both your holes.\n\n", false);
		Render.text("Your captor pulls it back and leans closer, letting the scent of her own fragrant puss fill the air.  It smells tangy and sweet and makes you ", false);
		if(player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.WET) Render.text("wet ", false);
		else if(player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.DROOLING) Render.text("drip on the chair ", false);
		else Render.text("soak the chair ", false);
		...
		Render.text("You shake your head in desperate denial and start to cry as you realize she intends to keep you locked in some kind of hellish pleasure-prison.  The panties slide up your legs with ease, and with a swift movement, the demon lifts your ass up and slips them into position with a wet 'SQUELCH'.  You moan as it goes to work, wrapping your " + clitDescript() + " in slippery tightness.  The two 'lumps' you observed elongate, the ridged surfaces making your " + vaginaDescript(0) + " quiver and dance with pleasure.  In mere seconds you're panting hotly and ready to cum.  Your crying devolves into heated moans of pleasure and longing.\n\n", false);
		Render.text("Bright red eyes fill your vision as the beautiful visage comes closer.  She whispers hotly in your ear, \"<i>I bet it feels good doesn't it?  Do you feel wet and horny?  I bet you'd love to throw yourself on my cock and get off right now.</i>\"\n\n", false);
		Render.text("You blink away the tears and nod frantically; you're so close!  But every time you feel an orgasm start to build the creature eases up just enough to keep you away from your orgasm.\n\n", false);
		Render.text("\"<i>You see, these panties are attuned to our kind.  I've worked hard to breed a pair that could be taught to only provide release when a demon cums in or on them.  Fortunately for you, the nodules will actually open to allow a demon's dick in either passage.  And just for our succubi friends, they can grow a protrusion from the front, and transmit the sensations to you,</i>\" she says as she demonstrates by bringing her throbbing purplish prick close to your pink-enclosed groin.  The surface of the panties splits with a line down the front, reshaping to reveal your pink-covered camel-toe.\n\n", false);  
		Render.text("She asks, \"<i>I won't be a rapist my dear.  This cock will only enter you if you desire the pleasure it can bring you.  You could say no and just enjoy being on the edge until your will finally crumbles.</i>\"\n\n", false);
		if(player.statusAffects.has("CampMarble")) {
			Render.text("Suddenly, a loud scream is heard down on the factory floor. You and your mistress turn to see Marble dashing up the stairs to the foremen's office.  Your mistress looks over at her and says with some amusement, \"<i>Oh ho!  So another cow has come to join in the fun.</i>\"\n\n\"<i>Sweetie! What has she done to you?</i>\" Marble exclaims, \"<i>What has she put on you?!</i>\"\n\n\"<i>Oh, so you knew this girl?</i>\" your mistress asks you, \"<i>It's a Lacta Bovine from the looks of it, so it seems this time I'll be adding a real cow to the pens.</i>\"  Marble turns to your mistress and brandishes her hammer, but the horror from the thought of your mistress being hurt causes you to spring forward and grab Marble.  The brief distraction gives your mistress a chance to sink a syringe into Marble's shoulder, and within moments she slumps onto the ground unconscious.\"\n\n", false);
		...
		Render.text("It takes less than a second for you to moan out, \"<i>Taaaaake meeeee!</i>\"\n\n", false);
		Render.text("She smiles and lifts you up from the chair with her strong arms, and sits down on the desk.  She lowers your symbiote-covered lips onto her demon dick and coos with delight as you slide down her length, taking the entire thing in your greedy depths.  If anything, the creature inside you makes sex even better - you feel a combination of her nub-covered cock fucking you and the ridges of the panty-creature as it continues to stimulate you.  It drives you mad with pleasure, and you begin bouncing yourself up and down, spearing your " + vaginaDescript(0) + " with the demon's pole.\n\n", false);
		Render.text("She giggles and reaches down.  Too aroused to care, you just keep fucking her and moaning in delight.  Her hands come up and begin to massage and rub your " + allBreastsDescript() + " taking special care to tweak and tug on your nipples.  They become as hard as ", false);
		if(player.upperBody.chest.BreastRatingLargest[0].nippleLength < .5) Render.text("erasers ", false);
		else if(player.upperBody.chest.BreastRatingLargest[0].nippleLength < 3) Render.text("bullets ", false);
		else Render.text("tiny cocks ", false);
		Render.text("in moments", false);
		if(player.biggestLactation() > 2) Render.text(" and start to drip with milk", false);
		Render.text(".  You sigh with disappointment when her hands drop away.  You were so close to orgasm.  She reaches back up and places something wet and warm on ", false);
		if(player.upperBody.chest.count() <= 1) Render.text("your " + nippleDescript(0), false);
		if(player.upperBody.chest.count() > 1) Render.text("your top " + nippleDescript(0), false);
		Render.text(".  You look down and see two star-shaped creatures made of glistening pink (just like your panties!) attached to your " + nippleDescript(0) + "s.  They pulse and ripple as they constantly massage and suck.  ", false);
		if(player.biggestLactation() > 1) Render.text("Your milk erupts, spraying out from a tiny hole in the center of the star.  In response the creature increases the force of its sucking action, making you fountain milk constantly.  ", false);
		if(player.upperBody.chest.count() > 1) Render.text("While you continue to fuck that meat pole and watch the creatures squirming on your nipples, more are affixed to each of your remaining " + nippleDescript(0) + "s, until every single one is covered with its own tiny pleasure-creature.\n\n", false);
...
public function demonBadEnd():void {
	...
	//[genderless]
	if(player.gender == 0) {
		Render.text("entire body blushes read before the sexy seductress.  She looks at you, frowning as she murmers, \"<i>Now this just won't do.  You look more like a " + player.boyGirl() + " to me, so why don't I make the plumbing match the exterior?</i>\"\n\n", false);  
		Render.text("Her palm caresses your crotch, stoking the warmth inside you until it blazes white-hot with new sensation.  Your skin ripples and parts, ", false);
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 1) {
			Render.text("pushed apart the thick flesh of a powerful demonic member, complete with two swollen balls.", false);
			player.gender = 1;
			player.lowerBody.cockSpot.add(new Cock());
			player.lowerBody.cockSpot.list[0].cockLength = 10;
			player.lowerBody.cockSpot.list[0].cockThickness = 2;
			...
			Render.text("gushing with fluids as it shapes itself into a hungry demonic cunt.", false);  
			player.gender = 2;
			player.createVagina();
	...
	if(player.gender == 1) {
		//Multispoooooo
		if(player.lowerBody.cockSpot.count() > 1) {
			Render.text(player.multiCockDescriptLight() + " pulsate, straining for just a touch of the succubus' hand.  She paces around you, giggling and toying with you as your " + player.multiCockDescript() + " seem to follow her, twitching and thickening any time she takes a step closer.\n\n", false);
			Render.text("She reaches out, cupping the underside of a shaft, slowly stroking your most sensitive places while she stops the bimbo-like voice and teases, \"<i>Awww, so hard and ready.  It looks to me like you're already a slave to your desires.  You're twitching and dripping, just from the soft touches of your enemy's fingers.  Are you truly so in need of release as to willingly offer it to me?  No, don't answer, your " + cockDescript(0) + " already has.</i>\"\n\n", false);
			Render.text("You glance down, seeing just how hard her words have made you.  You squirm your " + hipDescript() + "s pathetically, trying to hump her hand and increase the stimulation.  The succubus immediately releases you and draws back, shedding her secretary's clothes like a snake sheds its skin.  Now clad only in a tight leather corset and thigh-high stockings with garters, the succubus tosses you onto a table, surprising you with her raw strength.  Seemingly from nowhere, she produces a whip, winding it tightly around ", false);
			if(player.lowerBody.balls > 0) Render.text("your " + ballsDescriptLight() + " and ", false);
			Render.text("the bases of your " + player.multiCockDescriptLight() + ".\n\n", false);
			Render.text("\"<i>There, that'll make sure you feel every squeeze and caress of my velvet walls, and keep you from getting off until you're ready,</i>\" says the succubus as she climbs the table and straddles your eager form.\n\n", false);
			Render.text("She lifts herself up with her shapely legs and spreads her thighs, proudly revealing her puffy pierced folds.  They drip with demonic nectar as she wiggles over you, spattering you with demon-honey.  Slowly, nearly imperceptibly, she swivels the lewd opening closer and closer, and your cocks, as if possessed, angle themselves upward towards the juicy target.  The grinning succubus looks over her shoulder and asks, \"<i>Ready are we? Ok, I won't keep you waiting.</i>\"\n\n", false);
			Render.text("Marvelous heat and wetness sweeps over you in a fluid motion, wrapping your " + cockDescript(0) + " tightly.  You sigh happily, already lost in the feeling of having a succubus' tight walls wriggling around you.  Were you not already so corrupt, you would probably be coming already, but as it is, you can lie there and enjoy it, reveling in the sensations your unholy lover is spreading through your body.  You shiver, finally approaching your climax, but as it nears you find yourself denied by the whip binding your " + player.multiCockDescriptLight() + ".  It isn't just the physical tightness either – something else about it keeps your release buried deep, inaccessible.\n\n", false);
			Render.text("\"<i>Have you hit it yet?</i>\" the succubus asks as she rocks on top of you, \"<i>I've placed a block inside you.  Don't worry, it's temporary, it'll only stop you from orgasming for a few days...</i>\"\n\n", false);
			Render.text("You moan pitifully, begging for her to remove it and allow you to cum.\n\n", false);
			Render.text("\"<i>Oh I can't remove it,</i>\" she says, \"<i>The only way you'll be rid of it with any sort of certainty is to melt through it with something stronger.  Something, like, I don't know, the focused remains of your soul and humanity.  Now you think on that while I melt away any doubts you might have.</i>\"\n\n", false);
			Render.text("She resumes fucking you, driving you insane with need, all the while fiddling with her clit and pulling up a nipple to lick.  It feels so good, but you NEED to cum.  She fucks you like that for hours, until the table collapses under the pair of you and dumps you both on the floor. More than anything you crave release, and over time you cave in further and further to the need.  In time, you can feel the block weakening, melting, and eroding.  Your life has been distilled down into this one moment, this one desire, this need for release.  The block shatters, melting away under the force of your need.\n\n", false);
			Render.text("A look of shock and pleasure spreads over the succubus's face as you release into her hot snatch, cumming with a force unlike anything you've felt before.  Her walls squeeze and caress in time with your orgasm, milking you of every drop.  Your body clenches and squeezes, shuddering as the orgasm continues for far longer than normal.  Though you don't feel like you're pushing out as much fluid as normal, somehow it feels even better, like a slow drip of pleasure and release.  When at last your " + cockDescript(0) + " empties, you feel drained and strangely energized at the same time, and your entire torso is spattered with rapidly hardening pink crystals.\n\n", false);
			Render.text("The slutty succubus stands up, her puffy vulva coated in a shining pink fluid.  Did that just come out of you?  She grunts, her eyes glowing for a moment as the pink goop disappears into her skin, vanishing entirely.\n\n", false);
			Render.text("\"<i>Ahhhhh,</i>\" she sighs, \"<i>nothing like fresh Lethicite.  Mmmm, yours was potent!</i>\"\n\n", false);
			Render.text("You stand up, dissatisfied at the sudden lack of sensation you're forced to endure.  The gloating demoness looks rather pleased with herself, and brimming with newfound power.  You resolve to ", false);
			if(player.findStatusAffect(StatusAffects.MaraesLethicite) < 0) Render.text("gather some yourself at the next opportunity...", false);
			else Render.text("devour Marae's as soon as you get a chance.", false);
			Render.text("You greedily gather up the lethicite splattered on your body and devour it, turning it into raw demonic power.  You really do need to get more of this... but first you know a certain demoness you intend to wrap around your ", false);
			if(player.demonCocks() > 0) Render.text("growing", false);
			else Render.text("new", false);
			Render.text(" demon-cock for a few more orgasms.", false);
		}
		//SINGLEZ NITE
		else {
			Render.text(player.multiCockDescriptLight() + " pulsates, straining for just a touch of the succubus' hand.  She paces around you, giggling and toying with you as your " + player.multiCockDescript() + " seems to follow her, twitching and thickening anytime she takes a step closer.\n\n", false);
			Render.text("She reaches out, cupping the underside of the shaft, slowly stroking your most sensitive places while she stops the bimbo-like voice and teases, \"<i>Awww, so hard and ready.  It looks to me like you're already a slave to your desires.  You're twitching and dripping, just from the soft touches of your enemy's fingers.  Are you truly so in need of release as to willingly offer it to me?  No, don't answer, your " + cockDescript(0) + " already has.</i>\"\n\n", false);
			Render.text("You glance down, seeing just how hard her words have made you.  You squirm your " + hipDescript() + "s pathetically, trying to hump her hand and increase the stimulation.  The succubus immediately releases you and draws back, shedding her secretary's clothes like a snake sheds its skin.  Now clad only in a tight leather corset and thigh-high stockings with garters, the succubus tosses you onto a table, surprising you with her raw strength.  Seemingly from nowhere, she produces a whip, winding it tightly around ", false);
			if(player.lowerBody.balls > 0) Render.text("your " + ballsDescriptLight() + " and ", false);
			Render.text("the base of your " + cockDescript(0) + ".\n\n", false);
			Render.text("\"<i>There, that'll make sure you feel every squeeze and caress of my velvet walls, and keep you from getting off until you're ready,</i>\" says the succubus as she climbs the table and straddles your eager form.\n\n", false);
			...
			Render.text("The slutty succubus stands up, her puffy vulva coated in a shining pink fluid.  Did that just come out of you?  She grunts, her eyes glowing for a moment as the pink goop disappears into her skin, vanishing entirely.\n\n", false);
			Render.text("\"<i>Ahhhhh,</i>\" she sighs, \"<i>nothing like fresh Lethicite.  Mmmm your's was potent!</i>\"\n\n", false);
			Render.text("You stand up, dissatisfied at the sudden lack of sensation you're forced to endure.  The gloating demoness looks rather pleased with herself, and brimming with newfound power.  You resolve to ", false);
			if(player.findStatusAffect(StatusAffects.MaraesLethicite) < 0) Render.text("gather some yourself at the next opportunity...", false);
			else Render.text("devour Marae's as soon as you get a chance.", false);
			Render.text("  But first you know a certain demoness you intend to wrap around your ", false);
			if(player.demonCocks() > 0) Render.text("growing", false);
			else Render.text("new", false);
			Render.text(" demon-cock for a few more orgasms.", false);
		}
	...
	//[female]
		...
		Render.text(vaginaDescript(0) + " grows wet and ready, practically juicing itself as the demoness' hand caresses your inner thigh.  She teases, \"<i>Oh my! You're so wet and ready and I haven't even touched your moist little cum-receptacle.  You're a slut aren't you?  Who else would be so turned on by the idea of cumming until all your humanity drips out?</i>\"\n\n", false);
		Render.text("The words make you blush hard, shaming you and stoking the growing fire between your " + player.legs() + ".  You know two things for certain: she's right and you're more turned on that ever.  You don't resist as the demoness easily lifts you up, setting you down on a table with your " + player.legs() + " spread.  \"<i>There,</i>\" she comments, \"<i>now your juicy snatch is on display, just like you've always wanted.</i>\"\n\n", false);
		Render.text("She effortlessly swings her lissomelegs onto the table as she pulls herself up, mounting you as a man might.  You can feel waves of heat rolling off her sex, bathing your own slit in her warmth.  ", false);
		if(player.lowerBody.vaginaSpot.list[0].clitLength >= 2) Render.text("Your " + clitDescript() + " pushes free, nuzzling against her hairless cunt and slipping inside, as if drawn in by its desire.  She openly moans, and begins rocking on top of you.  You gasp in delight as she rides your " + clitDescript() + ", fucking and grinding against it.  ", false);
		else Render.text("She lowers herself down, rubbing smooth hairless netherlips over you, smearing you with her fragrant demon-honey.  You feel her clit grinding on your own, drawing out gasps of delight from both of your mouths as she relentlessly scissors against you.  ", false);
		Render.text("In no time flat you feel your climax building.  Your " + vaginaDescript(0) + " quivers and grows wetter in anticipation.  Tugging on your aching " + nippleDescript(0) + "s and aching for release, you squirm under your demonic mistress, smashing your " + vaginaDescript(0) + " against her in a lewd attempt to find your orgasm.  It does not happen, and you moan in disappointment as the pleasure continues to build, oblivious to your desire for orgasm.\n\n", false);
		Render.text("\"<i>Have you hit it yet?</i>\" the succubus asks as she rocks on top of you, \"<i>I've placed a block inside you.  Don't worry, it's temporary, it'll only stop you from orgasming for a few days...</i>\"\n\n", false);
		Render.text("You moan pitifully, begging for her to remove it and allow you to cum.\n\n", false);
		Render.text("\"<i>Oh I can't remove it,</i>\" she says, \"<i>The only way you'll be rid of it with any sort of certainty is to melt through it with something stronger.  Something, like, oh I don't know, the focused remains of your soul and humanity.  Now you think on that while I grind away any remaining doubts you might have.</i>\"\n\n", false);
		Render.text("The beautiful succubus on top of you picks up the pace, grinding harder against your " + vaginaDescript(0) + " and " + clitDescript() + ", pushing you beyond anything you've ever felt before.  She leans down over you, licking her lips to reveal an inhumanly long tongue.  It snakes down, curving around you " + allBreastsDescript() + " before it arches up to flick a " + nippleDescript(0) + ".  ", false);
		if(player.upperBody.chest.hasFuckableNipples() && player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) Render.text("You gasp as the tongue slides inside each of your breasts, violating them in turn thanks to your strange anatomy.\n\n", false);
		else Render.text("You gasp as it curls around each of your " + nippleDescript(0) + "s in turn, tugging them lewdly.\n\n", false);  
	 	Render.text("She fucks you like that for hours, until the table collapses under the pair of you and dumps you both on the floor. More than anything you find yourself craving release, and over time you cave in further and further to the need.  You start to feel the block weakening, melting, and eroding.  Your life has been distilled down into this one moment, this one desire, and this need for release.  The block shatters, melting away under the force of your need as you explosively orgasm.\n\n", false); 
		Render.text("Sparkling pink fluid splatters between the two of you as you cum, squirting hard", false);
		if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLAVERING) Render.text(" for the first time", false);
		Render.text(".  The succubus throws back her head and lets loose a moan of ecstasy, her entire body shivering with your own as both of your heads fill with fireworks of pleasure.  Nervelessly, she rolls off of you, her tail contracting hard around your leg while the two of you share the moment.\n\n", false);
		Render.text("The succubus interrupts your delight by recovering far faster than you, rolling up to a standing position and watching something between your legs.  You prop yourself up on your elbows to see what the fuss is about.  Between your legs something curious is happening – a trickle of pinkish fluid is still escaping your nethers, rolling towards a rapidly expanding pool, along with every other drop of the pink goop.  Before your very eyes the pool grows until every drop of pink fluid has collected together, and it grows upwards, solidifying into a sparkling crystalline shape.\n\n", false);
		Render.text("Before you can react, she grasps the newly-formed lethicite and noisily begins eating it, her eyes glowing with newfound power.  Watching her makes you more than a little jealous and angry with yourself.  You should've taken the lethicite and gained its power!  No use fretting about it, you can still fuck this succubus for a few hours before you go out in search of your own victims...\n\n", false);
	}
	//[HERM ENDING]
	else {
		//Buh.  Zombie fen need brains.
		Render.text(player.multiCockDescript() + " and " + vaginaDescript(0) + " grow wet and ready, both starting to leak fluids as the succubus' hand traces your inner thigh.  She teases, \"<i>Oh my! You're so wet and ready and I haven't even touched your moist little cum-receptacle.  And that throbbing cock!  How obscene!  You're a slut aren't you?  Who else would be so turned on by the idea of cumming until your humanity is splattered between my legs?</i>\"\n\n", false);
		Render.text("The words make you blush hard, shaming you and stoking the growing fire between your legs.  You know two things for certain: she's right and you're more turned on that ever.  You don't resist as the demoness easily lifts you up, setting you down on a table with your legs spread.  \"<i>There,</i>\" she comments, \"<i>now all of your fun-parts are on display.  Maybe I should call in an incubus and a few imps to watch.  I bet you'd like that wouldn't you?</i>\"\n\n", false);
		Render.text("She effortlessly swings her lissomelegs onto the table as she pulls herself up, mounting you in a single swift motion.  You can feel waves of heat rolling off her sex, bathing your " + cockDescript(0) + " in her warmth.  ", false);
		if(player.lowerBody.vaginaSpot.list[0].clitLength >= 2) Render.text("Your " + clitDescript() + " pushes free, nuzzling against her tight asshole and slipping inside, as if drawn in by its desire.  She openly moans, and begins rocking on top of you.  You gasp in delight as she rides your " + clitDescript() + ", fucking her ass and grinding against it.", false);
		else Render.text("She lowers herself down, rubbing smooth hairless netherlips over your crotch and vulva, smearing you with her fragrant demon-honey.  You feel her clit grinding on your belly, drawing out gasps of delight from both of your mouths as she relentlessly works her body against your own.", false);
		...
		Render.text("A look of shock and pleasure spreads over the succubus' face as you release into her hot snatch, cumming with a force unlike anything you've felt before.  Her walls squeeze and caress in time with your orgasm, milking you of every drop.  Your body clenches and squeezes, shuddering as the orgasm continues for far longer than normal.  Though you don't feel like you're pushing out as much fluid as normal, somehow it feels even better, like a slow drip of pleasure and release.  When at last your " + cockDescript(0) + " empties, you feel drained and strangely energized at the same time.\n\n", false);
		Render.text("The slutty succubus stands up, her puffy vulva coated in a shining pink fluid.  Did that just come out of you?  She grunts, her eyes glowing for a moment as the pink goop disappears into her skin, vanishing entirely.\n\n", false);
		Render.text("\"<i>Ahhhhh,</i>\" she sighs, \"<i>nothing like fresh Lethicite.  Mmmm, your's was soooo potent!</i>\"\n\n", false);
		Render.text("You stand up, dissatisfied at the sudden lack of sensation you're forced to endure.  The gloating demoness looks rather pleased with herself, and brimming with her new-found power.  You resolve to ", false);
		if(player.findStatusAffect(StatusAffects.MaraesLethicite) < 0) Render.text("gather some yourself at the next opportunity...", false);
		else Render.text("devour Marae's as soon as you get a chance.", false);
		Render.text("  But first you know a certain demoness you intend to wrap around your ", false);
		if(player.demonCocks() > 0) Render.text("growing", false);
		...
		Render.text(" demon-cock for a few more orgasms.", false);
		Render.text("  Before you get into that, you spy a small piece of pink crystal on the floor between your legs.  You snatch it and devour it before the succubus has a chance and eat it, turning part of your soul into new-found demonic strength before you return to a long night of sex...", false);
	}
	player.orgasm();
