...
//				itemMenu = null;
			...
			//Not recognized
				...
				else if(player.lowerBody.vaginaSpot.hasVagina()) outputText("lick your honeypot 'til you soak my face", false);
...
//		choice7 = 11001;
	...
	if(dungeonLoc == DUNGEON_FACTORY_MAIN_CHAMBER) {
		...
		if(player.findStatusAffect(StatusAffects.DungeonShutDown) < 0) {
			...
			if(player.stats.cor < 50) outputText("You wish you could free them, but it would take the better part of a day to get them all free.  It'd be better to find the control room and shut down the infernal machinery.  ", false);
...
//		choice1 = 11006;
		...
		if (player.statusAffects.has("TakenLactaid")) {
			if (player.statusAffects.get("TakenLactaid").value1 > 0) {
				outputText("There is a crate with " + num2Text(player.statusAffects.get("TakenLactaid").value1) + " bottles of something called 'Lactaid' inside.\n\n", false);
//				text3 = "Lactaid";
...
//			choice3 = storageTakeLactaid;
		...
		if (player.statusAffects.has("TakenGroPlus")) {
			if (player.statusAffects.get("TakenGroPlus").value1 > 0) {
				outputText("There is a crate with " + num2Text(player.statusAffects.get("TakenGroPlus").value1) + " bottles of something called 'Gro+' inside.\n\n", false);
//				text4 = "GroPlus";
...
//		choice6 = 11068;
	...
	if(dungeonLoc == DUNGEON_CAVE_SECRET_TUNNEL) {
		...
		if(flags[FlagEnum.DEFEATED_ZETAZ] == 0) outputText("You're going to find Zetaz and pay him back for drugging you on your first day here.  ", false);
...
//		choice3 = 11085;
	...
	if(dungeonLoc == DUNGEON_HEL_WINE_CELLAR) {
		...
		if (flags[FlagEnum.HEL_DUNGEON_MEAD_LOOTED] < 5) {
			outputText(" (There are " + (5 - flags[FlagEnum.HEL_DUNGEON_MEAD_LOOTED]) + "x God's Mead horns here to take.)\n\n");
//			text4 = "God'sMead";
...
//			choice5 = leaveBoobsDungeon;
			...
			if (flags[FlagEnum.TIMES_SUBMITTED_TO_SANURA] + flags[FlagEnum.TIMES_WINFUCKED_SANURA] > 0) {
				...
				if (flags[FlagEnum.BEATEN_SANURA_COUNT] > 0) outputText("  Or would you rather skip the formalities?  We both know who's got the sharper wit, I should think.");
				outputText("</i>\"");
			}
			else {
...
//				choice3 = friendlyCumWitchBlessing;
			...
			if (flags[FlagEnum.CUM_WITCH_DEFEATED] == 0) {
				...
				if (player.lowerBody.vaginaSpot.hasVagina()) outputText("but then, maybe you can come to serve us as a mother.  Our tribe is not wasteful.");
				else if (player.lowerBody.cockSpot.hasCock()) outputText("but perhaps, once you have been disabused of your notions of freedom, you could serve as my loyal cum-pump.  It does get so tiring inseminating all these girls alone.");
...
private function relieveTension():void {
	...
	//Second/third times...
		...
		if (player.statusAffects.get("TensionReleased").value1 == 0) {
			...
			if (player.lowerBody.cockSpot.count() > 0) outputText("and " + player.multiCockDescript());
			outputText(" become rock hard, leaking fluids constantly.  ");
			//MALE
			if (player.lowerBody.cockSpot.count() > 0 && player.lowerBody.vaginaSpot.count() == 0) outputText("Glancing over into the sea of sex, you find yourself drawn to the nearest pussy, as if it was the only thing in the world to matter.  You lose track of the time as you fuck hard dozens of gaping cunts, each of them overflowing with cum from all the participants in this infernal orgy.  ");
			//FEMALE
			...
			//HERM
			if (player.lowerBody.vaginaSpot.count() > 0 && player.lowerBody.cockSpot.count() > 0) outputText("You feel your " + player.multiCockDescript() + " getting milked by many wet holes, though you are too busy sucking cocks and moaning in ecstasy to notice who they belong to.  ");
		...
		//Third time
			...
			if (player.lowerBody.vaginaSpot.count() > 0) outputText("Your pussy is instantaneously soaked, filling the air with the scent of sex.  ");
...
public function factoryFinisher():void {
	...
	if(player.lowerBody.cockSpot.count() == 0 || player.upperBody.chest.BreastRatingLargest[0].breastRating <= 1) {
		...
		if(player.lowerBody.cockSpot.count() == 0) {
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 1) outputText(" or ", false);
		}
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating <= 1) outputText("any adornments on your chest", false);
...
private function succubusBadEndPartTwo():void {
	...
	if (player.lowerBody.cockSpot.count() == 1) { //Cock – single
		if (player.cockArea(0) < 30) {
			...
			outputText("Nearly blacking out, you struggle to stay awake as your body shifts your blood to your disproportionate " + Appearance.cockNoun(CockType.HUMAN) + ".  ");
		}
		else outputText("As you struggle not to lose consciousness, you realize your over-aroused body had pumped most of your blood to your over-sized " + Appearance.cockNoun(CockType.HUMAN) + ", which now droops to the floor, pulsing hotly.  "); //Cock -megahuge
	}
	//DO MULTIZ
	if (player.lowerBody.cockSpot.count() > 1) outputText("The feeling of light-headedness nearly robs you of consciousness as your " + player.multiCockDescript() + " fill with blood, pulsating with arousal as they reach full size.  ");
...
private function succubusBadEndPartThree():void {
	...
	//Puss Orgasmz
	if (player.lowerBody.vaginaSpot.count() > 0) outputText("Clenching tightly, your " + vaginaDescript(0) + " squeezes tightly on its intruder as its repeatedly violated by the machines.  ");
	//End
	...
	if (player.statusAffects.get("$1").value3 == 1 || player.statusAffects.has("CampMarble")) outputText("  With Marble here too, you'll be around for a long time.");
	gameOver();
}
...
public function succubusLossRape():void {
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		...
		if(player.lowerBody.balls > 0) {
			//[[balls]]
			if(player.lowerBody.ballSize < 6) outputText("Your sultry captor leans low over you, her luscious tits wobbling enticingly as she reaches down and caresses your " + ballsDescriptLight() + " with soft touches.  Almost immediately you feel them clench with boiling heat, growing heavy and churning with a load big enough to satisfy a thirsty succubus.", false); 
			//[[huge balls]]
		...
		if(player.lust > 99) {
			...
			if(player.lowerBody.vaginaSpot.count() > 0) outputText(vaginaDescript(0) + " as the last pieces fall away.\n\n", false);
		...
		if(player.lowerBody.vaginaSpot.count() > 0) outputText("your " + clitDescript() + " parts your folds, growing harder.", false);
	...
	if(player.lowerBody.cockSpot.count() > 0 && (player.gender != 3 || rand(2))) {
		...
		if(player.cockArea(0) > monster.vaginalCapacity()) {
			...
			if(player.cumQ() >= 50 && player.cumQ() < 500) {
				...
				if(player.lowerBody.balls > 0) outputText("Your " + ballsDescriptLight() + " practically glow with relief as they begin contracting.  Their entire surface is covered with black veins that radiate from your demonic prick, sharing the corruption with your sperm factories.  ", false);  
			...
			if(player.cumQ() >= 500) {
				...
				if(player.lowerBody.balls > 0) outputText("Your " + ballsDescript() + " twitch and shake, the veiny surface of your sack darkening as the corruption begins to fill them.  ", false);
		...
		//((IT FITS))
			...
			if(player.lowerBody.balls > 0) outputText("Looking down, you even see the veins on your sack darkening to solid black as the corruption begins tainting your sperm-factories.  ", false);
			...
			if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 0) outputText("Your female double reaches down to pinch your nipple, spurring you on, \"<i>Please, could you like, cum for me?</i>\"", false);
			//New PG
			outputText("\n\n", false);
			player.lowerBody.cockSpot.list[0].cockType = CockType.DEMON;
			...
			if(player.lowerBody.balls > 0) outputText(" as your balls empty their load queue", false);
			outputText(".  ", false);
			//Big cum
...
public function dungeonSuccubusForceScissor():void {
	...
	if(player.lowerBody.vaginaSpot.list[0].clitLength >= 3) outputText("You groan with wanton desire as your " + clitDescript() + " continues to grow and grow until reaching full size and slipping inside the defeated slut's sloppy pleasure-hole.  ", false);
...
public function dungeonSuccubusForceFeed():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating > 6) outputText(", nearly smothering the succubus with the swell of tit-flesh", false);
	...
	if(player.lowerBody.cockSpot.count() > 0) outputText(" and " + player.multiCockDescriptLight() + " feel hungrier than ever.  ", false);
	...
	if(player.lowerBody.cockSpot.count() == 0) {
		temp = rand(3);
		if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLAVERING) outputText(", soaking her with girl-cum.", false);
		else {
			if(player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.WET) outputText(", slicking her face with girlish cum.", false);
	...
	if(player.lowerBody.cockSpot.count() > 0) outputText(" and a thick layer of spunk", false);
	outputText(".", false);
	outputText("\n\nYou turn away with a bemused sigh.  When you glance back, she has vanished!", false);
	dynStats("lus", -50);
	cleanupAfterCombat();
...
public function incubusLossRape():void {
	...
	if(player.upperBody.chest.hasFuckableNipples() && player.lust >= 100) {
		...
		if(player.upperBody.chest.countNipples() > 2) outputText("  Each of those divides again, splitting into four prehensile penises.", false);
		if(player.upperBody.chest.countNipples() > 4) outputText("  They continue dividing until his wriggling mass is sufficient to penetrate every single nipple and then some.", false); 
		outputText("\n\n", false);
	...
	//Tentacle gangbang
		...
		if(player.stats.cor < 80) outputText("In a disgusting display", false);
		...
		if(player.stats.cor >= 80) outputText("As you realize their size and number, you open your eyes wide and smile broadly, reflexively spreading your legs wide, practically begging him to fuck you.  ", false);
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("They make sure to rub each of your breasts, spending a few seconds smearing slick pre-cum into your " + nippleDescript(0) + ".  ", false);
		...
		if(player.stats.cor < 80) outputText("You wriggle and whine,", false);
		...
		if(player.lowerBody.vaginaSpot.count() > 0) outputText("squeezing your legs around them and grinding your " + clitDescript() + " against the oddly textured demon-cock.", false);
		else {
			if(player.lowerBody.balls > 0) outputText("grinding down against the ribbed and textured demonic cock as it slides between your " + ballsDescriptLight() + ".", false);
			else if(player.lowerBody.cockSpot.count() > 0) outputText("squeezing your legs around the thick demonic flesh as a steady dribble of pre-cum drips from your " + cockDescript(0) + ".", false);
		...
		if(player.lowerBody.vaginaSpot.count() > 0 && (player.lowerBody.cockSpot.count() == 0 || rand(2))) {
			...
			if(player.stats.cor < 80) outputText("You feel your " + player.legs() + " lifted and pulled tight as countless demonic cocks encircle your body, binding and constraining you further.  You whimper as a demonic tentacle probes your back door while a thicker one lines itself up just below your " + clitDescript() + ".  ", false);
			...
			if(player.stats.cor < 80) outputText("You gasp from pleasure and surprise, ", false);
			...
			if(player.stats.cor < 80) outputText("squeezed and caressed by the writhing tentacle-pricks in and around your body, lost in the pleasure and taste of demonic pre-cum.\n\n", false);
			...
			//FemCum
			if(player.lowerBody.vaginaSpot.list[0].clitLength > 3) outputText("You nearly cum on the spot when the cock fucking your pussy loops its length around your " + clitDescript() + ", the cum-slickened coils driving you mad with pleasure as they coil, slide, and jerk around your clit as if it was a cock.  ", false);
		...
		//Male/Genderless
			...
			if(player.lowerBody.cockSpot.count() > 0) outputText("The cock-tentacle around your " + cockDescript(0) + "  increases the pace of its stimulation as it begins to spurt hot wet cum over you, giving it lubrication as it jacks you off while staying tight around your base to prevent you from an orgasming.  ", false);
...
public function incubusVictoryRapeSex():void {
	...
	if(player.lowerBody.cockSpot.count() > 0) {
		...
		if(player.gender == 3) {
			if(player.lowerBody.balls > 0) outputText(player.multiCockDescriptLight() + ", " + ballsDescriptLight() + ", and " + vaginaDescript(0), false);
			else outputText(player.multiCockDescriptLight() + " and " + vaginaDescript(0), false);
		}
		else {
			if(player.lowerBody.balls > 0) outputText(player.multiCockDescriptLight() + " and " + ballsDescriptLight(), false);
		...
		if(player.lowerBody.cockSpot.list[0].cockType == CockType.HORSE) outputText("pressing your thick flare ", false);
		if(player.lowerBody.cockSpot.list[0].cockType == CockType.DOG) outputText("pressing your pointed tip ", false);
		if(player.lowerBody.cockSpot.list[0].cockType == CockType.HUMAN || player.lowerBody.cockSpot.list[0].cockType.Index > 2) outputText("pressing your head ", false);
		...
		if(player.lowerBody.cockSpot.list[0].cockLength > 10) outputText("deep inside ", false);
		...
		if(player.lowerBody.balls > 0) outputText("  Your " + ballsDescriptLight() + " swing freely, smacking into the demon's own and making both of you squeal and dribble more pre-cum.", false);
		outputText("\n\n", false);
		//(CUM)  
		if(player.lowerBody.balls > 0) outputText("You feel your " + ballsDescriptLight() + " draw up tight against your body.  ", false);
	...
	//(VAGINAL - Requires Vagina)
		...
		if(player.gender == 3) {
			if(player.lowerBody.balls > 0) outputText(player.multiCockDescriptLight() + ", " + ballsDescriptLight() + ", and " + vaginaDescript(0), false);
			else if(player.lowerBody.balls > 0) outputText(player.multiCockDescriptLight() + " and " + vaginaDescript(0), false);
		}
		else {
		...
		if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLICK) outputText("In no time flat your drooling fluids soak him in slippery wetness.  ", false);
		if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLICK && player.vaginas[0].vaginalWetness != VAGINA_WETNESS.DRY) outputText("Before long, you've lubricated a fair portion of his tool with wetness.  ", false);
		if(player.vaginas[0].vaginalWetness == VAGINA_WETNESS.DRY) outputText("Despite your usual light lubrication, you manage to moisten the top-half of his tool with wetness.  ", false);
...
private function omnibusVictoryGrowDick():void {
	...
	if (player.lowerBody.cockSpot.count() == 1) {
		if (player.demonCocks() < 1) {
			...
			if (player.stats.cor < 80) outputText("You watch in horror as the skin of your " + cockDescript(0) + " turns shiny and purplish-black.  ");
...
public function omnibusLossRape2():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		...
		if(player.lowerBody.balls > 0) {
			...
			if(player.lowerBody.vaginaSpot.count() > 0) outputText(" and " + vaginaDescript(0), false);
		}
		else if(player.lowerBody.vaginaSpot.count() > 0) outputText(vaginaDescript(0), false);
		...
		if(player.upperBody.chest.count() == 1) outputText("They reach the lower curve of your breasts.\n\n", false);
		if(player.upperBody.chest.count() > 1) outputText("They slide over your lowest pair of breasts, encapsulating them in wriggling tightness.\n\n", false);
		outputText("*FLASH* \"<i>You love cumming for anything and anyone.</i>\"\n\n", false);
		if(player.upperBody.chest.count() == 3) outputText("Your middle breasts tingle with absolute pleasure as they too become engulfed in tightness.\n\n", false);
		if(player.upperBody.chest.count() == 2) outputText("You groan as the plant grows up the summit of your top breasts, coating the bottom half of your aureola.\n\n", false);
		if(player.upperBody.chest.count() == 1) outputText("Your " + nippleDescript(0) + "s become hard as steel as the wave of slick pleasure washes over them.\n\n", false);  
		outputText("*FLASH* \"<i>You love being told to orgasm.</i>\"\n\n", false);
		if(player.upperBody.chest.count() == 1) outputText("The wriggling mass slides up the top-most parts of your breasts, narrowing into two tiny tendrils that loop around your neck.\n\n", false);
		if(player.upperBody.chest.count() >= 2) outputText("The wriggling mass climbs your top pair of breasts with ease, wrapping your diamond-hard nipples in slime and sensation.  It continues climbing upward, narrowing into two bands that loop around the back of your neck.\n\n", false);
	...
	if(player.lowerBody.cockSpot.count() == 1) {
		...
		if(player.stats.cor < 33) outputText("sweet ", false);
		if(player.stats.cor >= 66) outputText("tainted ", false);
		...
		if(player.lowerBody.balls > 0) outputText(" smothers your balls, entrapping them in sticky sensation as it continues across your taint between your butt-cheeks.  ", false);
		...
		if(player.lowerBody.balls > 0) outputText(" and teasing your balls", false);
		...
		if(player.lowerBody.balls > 0) outputText(ballsDescriptLight() + " and ", false);
	...
	if(player.lowerBody.vaginaSpot.count() == 1 || player.gender == 0) {
		...
		if(player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.WET) outputText("wet ", false);
		else if(player.vaginas[0].vaginalWetness <= VAGINA_WETNESS.DROOLING) outputText("drip on the chair ", false);
		...
		if(player.upperBody.chest.BreastRatingLargest[0].nippleLength < .5) outputText("erasers ", false);
		else if(player.upperBody.chest.BreastRatingLargest[0].nippleLength < 3) outputText("bullets ", false);
		...
		if(player.upperBody.chest.count() <= 1) outputText("your " + nippleDescript(0), false);
		if(player.upperBody.chest.count() > 1) outputText("your top " + nippleDescript(0), false);
		...
		if(player.upperBody.chest.count() > 1) outputText("While you continue to fuck that meat pole and watch the creatures squirming on your nipples, more are affixed to each of your remaining " + nippleDescript(0) + "s, until every single one is covered with its own tiny pleasure-creature.\n\n", false);
...
public function demonBadEnd():void {
	...
	if(player.gender == 1) {
		...
		if(player.lowerBody.cockSpot.count() > 1) {
			...
			if(player.lowerBody.balls > 0) outputText("your " + ballsDescriptLight() + " and ", false);
	...
	//[female]
		...
		if(player.upperBody.chest.hasFuckableNipples() && player.upperBody.chest.BreastRatingLargest[0].breastRating > 2) outputText("You gasp as the tongue slides inside each of your breasts, violating them in turn thanks to your strange anatomy.\n\n", false);
		...
		if(player.vaginas[0].vaginalWetness < VAGINA_WETNESS.SLAVERING) outputText(" for the first time", false);
	...
	//[HERM ENDING]
		...
		if(player.lowerBody.vaginaSpot.list[0].clitLength >= 2) outputText("Your " + clitDescript() + " pushes free, nuzzling against her tight asshole and slipping inside, as if drawn in by its desire.  She openly moans, and begins rocking on top of you.  You gasp in delight as she rides your " + clitDescript() + ", fucking her ass and grinding against it.", false);
