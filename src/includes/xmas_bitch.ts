...
public function xmasBitchEncounter():void
{
	...
	if (flags[FlagEnum.PC_ENCOUNTERED_CHRISTMAS_ELF_BEFORE] > 0) outputText("nother");
	...
	if (flags[FlagEnum.PC_ENCOUNTERED_CHRISTMAS_ELF_BEFORE] > 0)
	{
		...
		if (flags[FlagEnum.KELT_BREAK_LEVEL] < 4) outputText("Kelt, though he's getting coal AGAIN");
	...
	else
	{
		...
		if (flags[FlagEnum.KELT_BREAK_LEVEL] < 4) outputText("Kelt, though he's getting coal this year");
	...
	if (player.stats.cor >= 90 || monk >= 5 || player.statusAffects.has("Exgartuan") || amilyScene.amilyCorrupt() || flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00283] > 0 || flags[FlagEnum.UNKNOWN_FLAG_NUMBER_00282] > 0 || flags[FlagEnum.NIAMH_STATUS] > 0)
	{
		...
		if (flags[FlagEnum.PC_ENCOUNTERED_CHRISTMAS_ELF_BEFORE] > 0) outputText("Here we go again...\n\n");
...
public function unwrapElfyPresent():void {
	...
	if(player.stats.spe < 25) outputText("clumsily ", false);
...
public function dickXmasElfGo():void {
	...
	if(player.cockArea(0) < 80) {
		...
		if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText("making them jiggle", false);
...
public function vagFuckXmasElf():void {
	...
	if(player.vaginas[0].vaginalWetness == VAGINA_WETNESS.SLAVERING) outputText("gushing fluids", false);
	else if(player.vaginas[0].vaginalWetness >= VAGINA_WETNESS.SLICK) outputText("dripping fluids", false);
...
public function xmasFuckFollowup():void {
	...
	if(player.perks.has("ElvenBounty")) outputText(" <b>again</b>");
	outputText("?", false);
