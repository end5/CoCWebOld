...
public pumpkinFuckEncounter():void {
	clearOutput();
	Render.text("Just ahead, in the middle of the path, lies a seeming innocuous gourd - a pumpkin to be precise.  The growth is oddly out of place.  Nothing else grows on the soft animal-forged path you now tread, and the pumpkin and its vines are moist, as if wet with morning dew.  They are the only plants in the vicinity to bear such moisture, gleaming dully in the muted arboreal light.");
	if(flags[FlagEnum.PUMPKIN_FUCK_YEAR_DONE] > 0) Render.text("  An old, haunting memory surfaces, and you realizes this seems... familiar.  Did you find this last year?  Yes... it could have been this very same pumpkin...");
...
public pumpkinFuck():void {
	...
	Render.text("Well, fuck it.  You're horny, and this little pumpkin doesn't look like it could stop you from fucking that gushing gourd-pussy.  As you shed your [armor], a few of the rough, slime-sweating vines feebly rise up to embrace you, but you brush them away, having no time for corrupt foolishness.  You're going to bust a nut in this fruit's sweet, supple folds, not let some malformed tentacle-horror molest you.");
	...
	if(player.cumQ() < 200) Render.text("Your weak load disappears into the plant's sucking gullet, wicked away as fast as you fire.");
	else if(player.cumQ() < 1000) Render.text("Your thick load disappears into the plant's sucking gullet almost as fast as you're firing it, though you can see a bit of white creampie forming around the edges of your cock.");
	else {
		Render.text("Your massive load floods the pumping, a deluge of semen squirting around your [cock biggest] even as the pumpkin starts to enlarge.  Every titanic spooge-squirt pumps the gourd up another level.  Soon, the rind is rubbing your belly.  A few blasts later, it's lifting you from the ground, your still-spurting cock lodged firmly in its cunt.  The suction feels so much stronger now - you're not sure you could pull out now if you wanted to, and you don't!");
	}
	if(player.cumQ() < 1000) {
		...
		if(player.weaponName == "fists") Render.text("brandishing your fists");
		else Render.text("picking up your [weapon]");
		Render.text(" as if it would somehow protect your nude body.");
...
public pumpkinFuckPartII():void {
	...
	Render.text(" as she explains, \"<i>Almost a decade ago, my sister, Marae, bolstered by Tel'Adre's Covenant, sealed me away, bound me up in barren, lifeless soil.</i>\"  Her grip tightens, drawing a drop of blood and a wince of pain from you.  \"<i>She didn't approve of my decision to throw my lot in with Lethice's army.  The bitch always did think she knew best.</i>\"  The painful cage around your package relaxes, and she goes on, \"<i>You have no idea how hard it was to grow that pumpkin to the surface... but with your more than generous donation, you gave me the life I needed to break free.</i>\"");
	...
	if(player.lowerBody.cockSpot.count() > 1) {
		Render.text(", her free hand working ");
		if(player.lowerBody.cockSpot.count() == 2) Render.text("your other");
		else Render.text("another");
		Render.text(" dick with child-like glee");
	}
	Render.text(".  \"<i>");
	if(player.race() != "human") Render.text("I know what you were, virile suitor.  ");
	Render.text("There's never too much of a good thing.  Of course, my champion must have the proper train of thought...</i>\"");
...
public pumpkinFuckPartIII():void {
	clearOutput();
	Render.text("SLAP!  The crack of a palm on your face rouses you, a red handprint left on your cheek.  Fera smirks at you as you stir woozily in your plant-woven prison.  \"<i>Did you enjoy yourself?</i>\"  You slowly nod, feeling... liberated, free.  The situation is less worrying than before.  Fera just wanted to thank you, right?");
	Render.text("\n\nThe sultry orange-haired maiden ");
	if(player.lowerBody.balls > 0) Render.text("pats your sloshing sack");
	else Render.text("caresses a sensitive spot beneath your dick");
	Render.text(" as the vines unwind, allowing you to stand on your own [feet].  You go to wipe the cum from your nose and upper lip as soon as you're free.  It's dry.  Did any of that even happen?");
...
public mountPumpkin():void {
	...
	if(player.pregnancyIncubation > 0 || (player.wetness() < 2 && player.totalFertility() < 30)) {
		Render.text("Well, fuck it.  You're horny, and this little pumpkin doesn't look like it could stop you from fucking that gushing stem-rod.  As you shed your [armor], a few of the rough, slime-sweating vines feebly rise up to embrace you, but you brush them away, having no time for corrupt foolishness.  You're going to mount this fruit's hard, pulsing phallus, not let some malformed tentacle-horror molest you.");
	...
	//Female characters don't have an easy analogue to cum production unless we count milk production, and female gender doesn't require either breasts or lactation to play.  So I just went for high vaginal wetness, not pregnant, and likely to become pregnant if she had sex with a male creature.  I hope that works out OK.
		...
		Render.text("\n\nA faint rumbling is the only warning you get (or need).  You jump back and ready yourself for anything, ");
		if(player.weaponName != "fists") Render.text("picking up your [weapon]");
		else Render.text("raising your guard");
		Render.text(" as if it would somehow protect your nude body.");
...
public pumpkinMountingLevelII():void {
	clearOutput();
	Render.text("The pumpkin's titanic stem shivers, the mighty shaft trembling immensely before the pumpkin splits from the top causing it to topple.  The crack goes around the rind in multiple directions, causing it to split open like a flower as strings of orangey-purple goo hang wetly between the pieces for a moment.  A moment later, the sectioned fruit falls apart, at last revealing the corrupt core.  No trace of the mammoth copulation remains.");
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) Render.text(", her free hand working your [fullChest] with glee");
	Render.text(".  <i>\"There's never too much of a good thing.  Of course, my champion must have the proper train of thought...\"</i>");
...
public mountPumpkinsIII():void {
	clearOutput();
	Render.text("SLAP!  The crack of a palm on your face rouses you, a red handprint left on your cheek.  Fera smirks at you as you stir woozy in your plant-woven prison.  <i>\"Did you enjoy yourself?\"</i>  You slowly nod, feeling... liberated... free.  The situation is less worrying than before.  Fera just wanted to thank you, right?");
...
public leaveFerasWonderland():void {
	clearOutput();
	Render.text("Nah, that whole place is probably some kind of giant venus fly trap. Fuck that.");
...
public exploreFerasWonderland():void {
	...
	if(player.stats.int >= 40) {
		Render.text(", and with that accomplished, you could head back to camp.  Now, though, you'd rather stay; it smells and looks too good to leave this place.  If you focus on your better sense, you could still leave.");
		if(player.lowerBody.cockSpot.hasCock()) addButton(0,"Continue",fuckDemCorruptFlowerLips);
		else addButton(0,"Continue",stickAStamenInIt);
		addButton(4,"Leave",leaveAfterFlowerHuffing);
	}
	else {
		Render.text(", but that thought slips out of your head as it fills with more of that sinfully exciting aroma, making you feel so hot, horny, and eager.  You have to get more of it, submerse yourself deeper in its cloying clutches as you watch the pretty colors drain away your worries and concerns.");
		if(player.lowerBody.cockSpot.hasCock()) addButton(0,"Next",fuckDemCorruptFlowerLips);
		else addButton(0,"Next",stickAStamenInIt);
	}	
	if(player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) {
		Render.text("\n\nThe only ");
		if(player.stats.int >= 40) Render.text("other ");
		Render.text("question left to you is, are you more concerned with slipping [oneCock] into those subliminally-whispering lips or guiding a writhing stamen into your [vagina] and [asshole] so that you can take in as much nectar as possible?");
...
public leaveAfterFlowerHuffing():void {
	clearOutput();
	Render.text("The farther you get from that glade, the more your head clears and the more you realize how close you came to disaster.  You resolve to avoid it in the future, lest you fall prey to that entrancing pollen and the hypnotic petals once more.");
	doNext(camp.returnToCampUseOneHour);
}
//Fuck the Lips
public fuckDemCorruptFlowerLips():void {
	...
	if(player.longestCockLength() >= 20) {
		Render.text("easily ramming your huge endowment");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" straight into the erotic-looking mouth");
	}
	else if(player.longestCockLength() >= 8) {
		Render.text("slipping your large endowments");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" right into the erotic-looking mouth");
	}
	else if(player.longestCockLength() >= 4) {
		Render.text("easing the top hal");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("ves");
		else Render.text("f");
		Render.text(" of your phallus");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("es"); 
		Render.text(" into the erotic-looking mouth");
	}
	else {
		Render.text("barely managing to slip your tip");
		if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
		Render.text(" into the erotic-looking mouth");
	}
	Render.text(". The contact is everything you thought it would be: surprisingly warm, soaking-wet, and squeezing just tight enough to make your pillar");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" of lust twitch in excitement.");
	...
	if(player.lowerBody.balls > 0) Render.text("[balls] against the cushy lower lip, wreathing them in syrupy sap or nectar.  You're not really sure what the appropriate name for such a substance is, and you're well past caring.");
	else Render.text("loins against the cushy, sap-slicked lips, compressing their hot, smooth surfaces against your " + player.skin() + ".  You're not really sure if sap is the appropriate name for such a substance, but you're far beyond caring at this point.");
	Render.text("  The fast-diminishing light doesn't even register, as your eyes have closed as you subsume yourself in pleasure.");
	...
	Render.text(" buried in, so hot, wet, and soft. With each passing moment, you become more relaxed, more open and exposed, and more aroused than before. You would probably be afraid if you were capable of any kind of reason.");
...
public dickBoostingFuntimes():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" just in time to take the first thrust from your hips, burying yourself into them and immediately pulling back, setting an easy, average rhythm.  At the same time, the suction starts, causing your ");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("heads");
	else Render.text("[cockHead]");
	Render.text(" to swell up a little, and when the rest of your length is plowed through the tight, slippery ring, the tugging vacuum increases, plumping your dick");
	if(player.lowerBody.cockSpot.count() > 1) Render.text("s");
	Render.text(" bigger, harder, and fatter than ");
	if(player.lowerBody.cockSpot.count() == 1) Render.text("it");
	else Render.text("they");
	Render.text(" should be.");
	...
	Render.text(" so that you can start the cycle anew.  You love that, and this is the perfect place for that kind of behavior, isn't it? Everybody in this world wants to fuck, they just like to fight to see who gets to be on top, and you're good at winning, aren't you, pet champion?</i>\" Fera ");
	if(player.lowerBody.balls > 0) Render.text("cups your [sack] tenderly");
	else Render.text("tickles your taint");
	Render.text(" to spur you to thrust harder while she fills your head with lurid thoughts.");
	...
	Render.text(" as you do, whispering, \"<i>Good " + player.mf("boy","girl") + ",</i>\" into your ears as dark energies flow into you, refilling your fast-emptying reservoirs faster than they can empty.  \"<i>Don't stop yet, [name].  You're going to be doing this a lot, so I'm giving you lots of practice.</i>\"");
...
public stickAStamenInIt():void {
	...
	else if(player.isGoo()) Render.text("goo");
	else Render.text("knees");
	Render.text(", you drag yourself up until the 'floor' flattens out and you nearly ram into the pursed lips, stopping yourself abruptly once you realize you've reached your goal.");
	...
	if(player.lowerBody.cockSpot.hasCock()) Render.text("or ");
	Render.text("a tight, dick-milking rape-mother");
	if(!player.lowerBody.cockSpot.hasCock()) Render.text(", or a libidinous dick-girl, sowing your seed everywhere you can and saving your cunt for only the strongest of mates");
	Render.text("?</i>\"");
	...
	if(!player.lowerBody.cockSpot.hasCock()) addButton(2,"Dick",growADickForFera);
}

//Gape
public gapeMePleaseFera():void {
	clearOutput();
	Render.text("Fera smiles as you announce your choice.  \"<i>I thought you might say that, little champion.  Now, you simply relax and enjoy the pleasure that only my normal weak-willed prey are allowed to experience.  You will be the first to taste it and be allowed free.  How wonderful!</i>\"  She pets your forehead.");
	...
	if(player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.LEVEL_CLOWN_CAR) Render.text(", spreading its well-widened lips as wide as they've ever been");
	else if(player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.GAPING) Render.text(", spreading your lips well beyond the widest they've ever been and still farther.");
	else if(player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.LOOSE) Render.text(", meeting a bit of token resistance at your unprepared entryway and slithering stamen after stamen inside, the forerunners widening you until the entirety of the column can pass inside, gaping you almost inhumanly wide.");
	else Render.text(", meeting more than a little resistant at your unstretched entry and finding a way in anyhow. First one tentacle slithers in, then a second, then a fourth, and so on. One after another, they spread you wider and wider, opening you until the entire wiggling column is buried deep, pulling your lips ridiculously wide.");
	...
	if(player.lowerBody == LOWER_BODY.HUMAN) Render.text("toe");
	else if(player.tailType > TAIL.NONE) Render.text("tail");
	else Render.text("[foot]");
	Render.text(" as you climax, arching your back and making guttural, beast-like sounds of pleasure.  Your eyelids hang low, lazily obscuring some of those wonderful colors from your view as the pleasure and plant-sap work on you.  Shuddering, you gush girlcum harder than ever, spraying a thick wave of lady-spunk across the tentacles, shining them with your juices");
	if(player.wetness() < 4) Render.text(" even though you're not a squirter");
	Render.text(".  They pump on with intense, unrelenting energy, squishing wetly with every thrust, splattering juice everywhere as they ruin your poor, convulsing cunny.");
	player.cuntChange(90000,true,true,false);
...
public gapeByFeraEpilogue():void {
	clearOutput();
	Render.text("You awaken fully dressed with a familiar ache between your legs. Damn, that was a good nap.  You rise, stretching as you rub your back, sore from lying against a tree for so long.  You suppose you could hunt down a tentacle monster and see if it'd really give you a filling");
	if(player.pregnancyIncubation == 0) Render.text(", but you'd rather fuck something that'll get you good and pregnant with a well-hung fuck-monster, like a minotaur!");
	else Render.text(".");
	Render.text("  You idly reach into your [armor] and stuff four fingers into yourself before sighing and removing them.  It just isn't the same as being full of cock.");
...
public tightLikeAToigar():void {
	clearOutput();
	Render.text("Fera smiles.  \"<i>I had hoped we might do that, little champion.  Now, you simply relax and enjoy the pleasure that only my normal weak-willed prey are allowed to experience.  You will be the first to taste it and be allowed free.  How wonderful!</i>\"  She pets your forehead.");
	...
	if(player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.LEVEL_CLOWN_CAR) Render.text(", slipping past your capacitive entrance with ease");
	else if(player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.GAPING) Render.text(", sliding into your welcoming entrance fairly easily");
	else if(player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.LOOSE) Render.text(", slowly working its way into your decently unstretched entrance");
	else Render.text(", slowly burrowing past your tight, restrictive entrance");
	Render.text(" and leaving a trail of slick, black corruption in its wake.  The tendril pushes the whole way up to your cervix, gently butting up against your womb's portal before dragging back out, sputtering more inky cum all the way.  When it pops out, the head catches the tip of your [clit], causing your [vagina] to spasm and contract, squeezing down on the foreign fluid with such strength that a geyser of sable spunk erupts from your quivering quim.");
...
public growADickForFera():void {
	...
	if(player.tailType > TAIL.NONE) Render.text(" and press your tail to the ground");
	Render.text(".  You feel the wetness of the hollow tube as it presses against you along with a lance of pure desire and heat in the very center, undoubtedly caused by the tainted needle as it slips through your unprotected flesh.  The thing is compressed so tightly against you that it looks like the entire injector was able to bury itself in your body.");
