...
public function pumpkinFuckEncounter():void {
	...
	if(flags[FlagEnum.PUMPKIN_FUCK_YEAR_DONE] > 0) outputText("  An old, haunting memory surfaces, and you realizes this seems... familiar.  Did you find this last year?  Yes... it could have been this very same pumpkin...");
...
public function pumpkinFuck():void {
	...
	if(player.lowerBody.balls > 0) outputText("balls deep ");
	...
	outputText("\n\n");
	if(player.lowerBody.balls > 0) outputText("Your [balls] slap");
...
public function pumpkinFuckPartIII():void {
	...
	if(player.lowerBody.balls > 0) outputText("pats your sloshing sack");
...
public function pumpkinMountingLevelII():void {
	...
	if(player.upperBody.chest.BreastRatingLargest[0].breastRating >= 2) outputText(", her free hand working your [fullChest] with glee");
...
public function exploreFerasWonderland():void {
	...
	if(player.lowerBody.cockSpot.hasCock() && player.lowerBody.vaginaSpot.hasVagina()) {
		outputText("\n\nThe only ");
		if(player.stats.int >= 40) outputText("other ");
...
public function fuckDemCorruptFlowerLips():void {
	...
	if(player.longestCockLength() >= 20) {
		...
		if(player.lowerBody.cockSpot.count() > 1) outputText("ves");
		else outputText("f");
	...
	if(player.lowerBody.balls > 0) outputText("[balls] against the cushy lower lip, wreathing them in syrupy sap or nectar.  You're not really sure what the appropriate name for such a substance is, and you're well past caring.");
	...
	outputText("\n\nFera rubs your ");
	if(player.lowerBody.balls == 0) outputText("inner thighs");
...
public function dickBoostingFuntimes():void {
	...
	if(player.lowerBody.cockSpot.count() > 1) outputText("heads");
	...
	if(player.lowerBody.balls > 0) outputText("cups your [sack] tenderly");
...
public function stickAStamenInIt():void {
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText(", [eachCock] bobbing");
	...
	if(player.lowerBody.cockSpot.hasCock()) outputText("or ");
	outputText("a tight, dick-milking rape-mother");
	if(!player.lowerBody.cockSpot.hasCock()) outputText(", or a libidinous dick-girl, sowing your seed everywhere you can and saving your cunt for only the strongest of mates");
	outputText("?</i>\"");
...
public function gapeMePleaseFera():void {
	...
	if(player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.LEVEL_CLOWN_CAR) outputText(", spreading its well-widened lips as wide as they've ever been");
	else if(player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.GAPING) outputText(", spreading your lips well beyond the widest they've ever been and still farther.");
	else if(player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.LOOSE) outputText(", meeting a bit of token resistance at your unprepared entryway and slithering stamen after stamen inside, the forerunners widening you until the entirety of the column can pass inside, gaping you almost inhumanly wide.");
	...
	if(player.lowerBody == LOWER_BODY.HUMAN) outputText("toe");
...
public function tightLikeAToigar():void {
	...
	if(player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.LEVEL_CLOWN_CAR) outputText(", slipping past your capacitive entrance with ease");
	else if(player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.GAPING) outputText(", sliding into your welcoming entrance fairly easily");
	else if(player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS.LOOSE) outputText(", slowly working its way into your decently unstretched entrance");
