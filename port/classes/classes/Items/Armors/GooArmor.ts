...
export class GooArmor extends Armor {
    ...
    public useText(): void { //Produces any text seen when equipping the armor normally
        ...
        if (this.game.player.hasVagina()) outputText("[vagina]");
        if (this.game.player.hasVagina() && this.game.player.hasCock()) outputText(" and ");
        if (this.game.player.hasCock()) outputText(this.game.player.multiCockDescriptLight());
        if (this.game.player.gender == 0) outputText("groin");
