...
import { CockTypeGroup, CockTypesEnum } from './CockTypesEnum';
import { Creature } from './Creature';
import { StatusAffects } from './StatusAffects';
import { rand, randomChoice } from '../../Engine/Utilities/SMath';

class Appearance {
    public inverseMap(x: object): object {
        var result: object = {};
    ...
    public hairOrFur(i_creature: Creature): string {
    ...
    public hairDescription(i_creature: Creature): string {
     ...
     */
    public tongueDescription(i_character: Character): string {
    ...
    public nippleDescription(i_creature: Creature, i_rowNum: number): string {
        //DEBUG SHIT!
    ...
    public hipDescription(i_character: Character): string {
        var description: string = "";
        var options: Array<any>;
    ...
    */
    ...
    public cockDescription(cockType: CockTypesEnum, length: number, girth: number, lust: number = 50, cumQ: number = 10, isPierced: boolean = false, hasSock: boolean = false, isGooey: boolean = false): string {
        if (rand(2) == 0) {
            if (cockType == CockTypesEnum.HUMAN) return this.cockAdjective(cockType, length, girth, lust, cumQ, isPierced, hasSock, isGooey) + " " + this.cockNoun(cockType);
            else return this.cockAdjective(cockType, length, girth, lust, cumQ, isPierced, hasSock, isGooey) + ", " + this.cockNoun(cockType);
        }
        return this.cockNoun(cockType);
    }
    ...
    public cockAdjective(cockType: CockTypesEnum, length: number, girth: number, lust: number = 50, cumQ: number = 10, isPierced: boolean = false, hasSock: boolean = false, isGooey: boolean = false): string {
    ...
    private cockAdjectives(i_cockLength: number, i_cockThickness: number, i_cockType: CockTypesEnum, i_creature: Creature): string {
        var description: string = "";
        var rando: number = 0;
        ...
        //Lust stuff
            ...
            if (i_creature.cumQ() > 50 && i_creature.cumQ() < 200 && rand(2) == 0) {
                //for hroses and dogs
                if (CockTypeGroup[i_cockType] == "animal") description += "animal-pre leaking";
            ...
            if (i_creature.cumQ() >= 200 && rand(2) == 0) {
                //for horses and dogs
                if (CockTypeGroup[i_cockType] == "animal") description += "animal-spunk dripping";
    ...
    public cockMultiNoun(cockType: CockTypesEnum): string {
        var options: Array<any>;
        var description: string = "";
        if (cockType == CockTypesEnum.HUMAN) {
            options = ["cock",
     ...
     */
    public ballsDescription(i_forcedSize: boolean, i_plural: boolean, i_creature: Creature, i_withArticle: boolean = false): string {
    ...
    public sackDescript(i_creature: Creature): string {
        if (i_creature.balls == 0) return "prostate";
    ...
    /* Moved to Creature.as
            public  sheathDescription(i_character:Character): string
            {
    ...
    */
    ...
    public vaginaDescript(i_creature: Creature, i_vaginaIndex: number = 0): string {
    ...
    public clitDescription(i_creature: Creature): string {
        var description: string = "";
        var options: Array<any>;
        var haveDescription: boolean = false;
     ...
     */
    public buttDescription(i_character: Character): string {
        var description: string = "";
        var options: Array<any>;
     ...
     */
    public buttDescriptionShort(i_creature: Creature): string {
        var description: string = "";
        var options: Array<any>;
    ...
    public assholeDescript(i_creature: Creature): string {
        var description: string = "";
    ...
    public wingsDescript(i_creature: Creature): string {
    ...
    */
    ...
    public breastCup(size: number): string {
     ...
     */
    public breastCupInverse(name: string, defaultValue: number = 0): number {
        ...
        for (var i: number = 0; i < this.BREAST_CUP_NAMES.length; i++) {
            if (name == this.BREAST_CUP_NAMES[i]) return i;
            if (this.BREAST_CUP_NAMES[i].indexOf(name) == 0) return i + (big ? 1 : 0);
        }
        return defaultValue;
    }

    public createMapFromPairs(src: Array<any>): Object {
        var result: Object = {};
        for (var i: number = 0; i < src.length; i++) result[src[i][0]] = src[i][1];
        return result;
    }

    public DEFAULT_GENDER_NAMES: Object = this.createMapFromPairs(
        [
    ...
    public DEFAULT_SKIN_NAMES: Object = this.createMapFromPairs(
        [
    ...
    public DEFAULT_SKIN_DESCS: Object = this.createMapFromPairs(
        [
            [SKIN_TYPE_PLAIN, "skin"],
            [SKIN_TYPE_FUR, "fur"],
            [SKIN_TYPE_SCALES, "scales"],
    ...
    public DEFAULT_HAIR_NAMES: Object = this.createMapFromPairs(
        [
            [HAIR_NORMAL, "normal"],
    ...
    public DEFAULT_FACE_NAMES: Object = this.createMapFromPairs(
        [
            [FACE_HUMAN, "human"],
            [FACE_HORSE, "horse"],
            [FACE_DOG, "dog"],
    ...
    public DEFAULT_TONGUE_NAMES: Object = this.createMapFromPairs(
        [
            [TONUGE_HUMAN, "human"],
            [TONUGE_SNAKE, "snake"],
    ...
    public DEFAULT_EYES_NAMES: Object = this.createMapFromPairs(
        [
            [EYES_HUMAN, "human"],
    ...
    public DEFAULT_EARS_NAMES: Object = this.createMapFromPairs(
        [
            [EARS_HUMAN, "human"],
            [EARS_HORSE, "horse"],
            [EARS_DOG, "dog"],
    ...
    public DEFAULT_HORNS_NAMES: Object = this.createMapFromPairs(
        [
    ...
    public DEFAULT_ANTENNAE_NAMES: Object = this.createMapFromPairs(
        [
            [ANTENNAE_NONE, "non-existant"],
            [ANTENNAE_BEE, "bee"]
        ]
    );
    public DEFAULT_ARM_NAMES: Object = this.createMapFromPairs(
        [
    ...
    public DEFAULT_TAIL_NAMES: Object = this.createMapFromPairs(
        [
    ...
    public DEFAULT_WING_NAMES: Object = this.createMapFromPairs(
        [
    ...
    public DEFAULT_WING_DESCS: Object = this.createMapFromPairs(
        [
            [WING_TYPE_NONE, "non-existant"],
            [WING_TYPE_BEE_LIKE_SMALL, "small bee-like"],
            [WING_TYPE_BEE_LIKE_LARGE, "large bee-like"],
    ...
    public DEFAULT_LOWER_BODY_NAMES: Object = this.createMapFromPairs(
        [
    ...
    public DEFAULT_PIERCING_NAMES: Object = this.createMapFromPairs(
        [
    ...
    public DEFAULT_VAGINA_TYPE_NAMES: Object = this.createMapFromPairs(
        [
            [VAGINA_TYPE_HUMAN, "human"],
            [VAGINA_TYPE_BLACK_SAND_TRAP, "black sandtrap"]
        ]
    );
    public DEFAULT_VAGINA_WETNESS_SCALES: Array<any> = [
    ...
    public DEFAULT_VAGINA_LOOSENESS_SCALES: Array<any> = [
    ...
    public DEFAULT_ANAL_WETNESS_SCALES: Array<any> = [
    ...
    public DEFAULT_ANAL_LOOSENESS_SCALES: Array<any> = [
    ...
    public DEFAULT_HIP_RATING_SCALES: Array<any> = [
    ...
    public DEFAULT_BUTT_RATING_SCALES: Array<any> = [
     ...
     */
    public describeByScale(value: number, scale: Array<any>, lessThan: string = "less than", moreThan: string = "more than"): string {
     ...
     */
    public numberOfThings(n: number, name: string, pluralForm: string = null): string {
     ...
     * Positive only!
     */
    public feetsAndInches(n: number): string {
     ...
     * 13 -> 13" (2'1")
     */
    public inchesAndFeetsAndInches(n: number): string {
        if (n < 12) return n + "\"";
        return n + "\" (" + this.feetsAndInches(n) + ")";
    }

    public allBreastsDescript(creature: Creature): string {
        var storage: string = "";
        ...
        storage += this.biggestBreastSizeDescript(creature);
        return storage;

    }

    public tailDescript(i_creature: Creature): string {
    ...
    public oneTailDescript(i_creature: Creature): string {
        if (i_creature.tailType == TAIL_TYPE_NONE) {
            trace("WARNING: Creature has no tails to describe.");
            return "<b>!Creature has no tails to describe!</b>";
        }
    ...
    public biggestBreastSizeDescript(creature: Creature): string {
        ...
        if (rand(2) == 0) descript += this.breastSize(creature.breastRows[temp142].breastRating);
        //Nouns!
        temp14 = rand(10);
    ...
    */
    ...
    public assholeOrPussy(creature: Creature): string {
        ...
        return this.assholeDescript(creature);
    }


    public multiCockDescriptLight(creature: Creature): string {
        ...
        if (creature.cockTotal() == 1) {
            if (dogCocks == 1) return this.cockNoun(CockTypesEnum.DOG);
            if (horseCocks == 1) return this.cockNoun(CockTypesEnum.HORSE);
        ...
        if (currCock == 2) {
            ...
            if (same) {
                ...
                if (normalCocks == 2) descript += " " + this.cockNoun(CockTypesEnum.HUMAN) + "s";
                if (horseCocks == 2) descript += ", " + this.cockNoun(CockTypesEnum.HORSE) + "s";
                if (dogCocks == 2) descript += ", " + this.cockNoun(CockTypesEnum.DOG) + "s";
                //Failsafe
                if (creature.cocks[0].cockType.Index > 2) descript += ", " + this.cockNoun(creature.cocks[0].cockType) + "s";
            }
            //Nonidentical
            else {
        ...
        if (currCock == 3) {
            ...
            if (same) {
                ...
                descript += creature.cockAdjective();
                if (normalCocks == 3) descript += " " + this.cockNoun(CockTypesEnum.HUMAN) + "s";
                if (horseCocks == 3) descript += ", " + this.cockNoun(CockTypesEnum.HORSE) + "s";
                if (dogCocks == 3) descript += ", " + this.cockNoun(CockTypesEnum.DOG) + "s";
                //Tentacles
                if (creature.cocks[0].cockType.Index > 2) descript += ", " + this.cockNoun(creature.cocks[0].cockType) + "s";
            }
            else {
    ...
    public multiCockDescript(creature: Creature): string {
        if (creature.cocks.length < 1) {
            CoC_Settings.error("");
        ...
        if (currCock == 3) {
            ...
            if (same) {
                ...
                if (dogCocks == 3)
                    descript += ", " + this.cockNoun(CockTypesEnum.DOG) + "s";
                //Tentacles
                if (creature.cocks[0].cockType.Index > 2) descript += ", " + this.cockNoun(creature.cocks[0].cockType) + "s";   // Not sure what's going on here, referencing index *may* be a bug.
...
export { appearance as Appearance };