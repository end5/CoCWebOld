...
export class BreastStore extends Utils implements SaveAwareInterface {
    ...
    private static LACTATION_BOOST: Array<number> = [0, 0, 2, 3, 6, 9, 17]; //Disabled, None, Light, Moderate, Strong, Heavy, Epic
    ...
    public updateAfterLoad(game: CoC): void {
        ...
        var flagData: Array<string> = String(game.flags[this._breastFlag]).split("^");
    ...
    public lactating(): boolean { return this._lactation >= LACTATION_LIGHT; }
    ...
    public milkIsFull(): boolean { return (this._lactation <= LACTATION_NONE ? 0 : this._fullness >= 50); }

    public milkIsOverflowing(): boolean {
        return (this._lactation <= LACTATION_NONE ? 0 : this._fullness >= 60 + 5 * LACTATION_BOOST[this._lactation]); //Probably pretty desperate to be milked by this point
    }
    ...
    public milkQuantity(): number {
        if (this._lactation <= LACTATION_NONE) return 0;
