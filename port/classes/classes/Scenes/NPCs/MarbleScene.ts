...
override public marbleFollower(): boolean {
...
public helpMarble1():void {
	...
	marbleStatusChange(Math.floor(player.str/10),0);
...
public helpMarble2():void {
	...
	marbleStatusChange(Math.floor(player.spe/10),0);
	//(increase player lust)
	dynStats("lus", 10);
	if(afterMarbleHelp()) return;
	outputText("When the two of you finish and you start to leave, Marble calls out to you, \"<i>Wait, let me give you something!</i>\" You turn and look back at her as she rushes up to you.  Smiling brilliantly, the cow-girl hands you a bottle full of warm milk, \"<i>My gift to you for your help, fresh from the source,</i>\" she says, patting her sizable chest.\n\n", false);
...
private marbleAppearance():void {
	...
	if(flags[kFLAGS.MARBLE_DICK_TYPE] > 0) outputText("She has grown a " + marbleCock() + " since you brought her to camp. It is " + num2Text(Math.floor(flags[kFLAGS.MARBLE_DICK_LENGTH])) + " inches long and " + num2Text(Math.floor(flags[kFLAGS.MARBLE_DICK_THICKNESS])) + " inches thick.\n\n", false);
...
private milkMarbleCunnilingling():void
{
	...
	var pLust: number = Math.floor( 20 + ((player.lib + player.cor) / 10) );
	dynStats( "lus", pLust );
	dynStats( "sen", 1 );
...
private milkMarbleBarFollowTail():void
{
	...
	{
		var pLust: number = Math.floor( 10 + player.lib / 10 );
		dynStats( "lus", pLust );
	}
	doNext(camp.returnToCampUseTwoHours);
}
...
private milkMarbleLeaveAfterBar():void
{
	...
	var pLust: number = Math.floor( 5 + player.lib / 20 );
	dynStats( "lus", pLust );
	flags[kFLAGS.MARBLE_LUST] += 5;
	doNext(camp.returnToCampUseTwoHours);
}
