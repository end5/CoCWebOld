...
private calledShot():void {
	...
	var damage: number = Math.floor((str + weaponAttack) - rand(player.tou) - player.armorDef);
	...
	if(damage <= 0 || (rand(2) == 0 && (combatMiss() || combatEvade() || combatFlexibility() || combatMisdirect()))) outputText("\nYou avoid the hit!");
		...
		private helSpawnShieldBash():void {
			clearOutput();
			var damage: number = Math.floor((str) - rand(player.tou) - player.armorDef);
			// Stuns a bitch
		...
		private tailWhipShitYo():void {
			...
			var damage: number = Math.floor((str) - rand(player.tou));
