	export class Minerva extends Monster
	{
		...
		private minervaBite():void
		{
			...
			var damage: number = Math.floor((str + 85) - rand(player.tou) - player.armorDef);
...
//Flying kick:
		private minervaKnowsKungfu():void
		{
			...
			var damage: number = Math.floor((str + weaponAttack + 100) - rand(player.tou) - player.armorDef);
			spe -= 70;
			if (combatMiss() || combatEvade() || combatFlexibility() || combatMisdirect()) {
...
//Tail-whip
		...
		{
			var damage: number = Math.floor((str + 35) - rand(player.tou) - player.armorDef);
...
//Halberd stab:
		private minervaUsesHalberdStab():void
		{
			...
			var damage: number = Math.floor((str + weaponAttack) - rand(player.tou));
...
//Halberd CHOP:
		private minervaUsesHalberdCHOP():void
		{
			...
			var damage: number = Math.floor((str + 100) - rand(player.tou) - player.armorDef);
...
//White Fire
		private kiteFire():void
		{
			...
			var damage: number = Math.floor(10 + (inte / 3 + rand(inte / 2)) * 1.5);
			damage = player.takeDamage(damage);
			outputText(" (" + damage + ")");
			combatRoundOver();
		}
