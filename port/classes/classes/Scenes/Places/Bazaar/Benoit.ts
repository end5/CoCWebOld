...
private benoitSellMenu():void {
	...
	for (var slot: number = 0; slot < 5; slot++) {
		if (player.itemSlots[slot].quantity > 0 && Math.floor(player.itemSlots[slot].itype.value / sellMod) >= 1) {
			outputText("\n" + Math.floor(player.itemSlots[slot].itype.value / sellMod) + " gems for " + player.itemSlots[slot].itype.longName + ".");
...
private benoitTransactBuy(slot: number = 1):void {
	...
	if(player.gems < Math.floor(buyMod * itype.value)) {
	...
	player.gems -= Math.floor(buyMod * itype.value);
	statScreenRefresh();
...
private benoitSellTransact(slot: number, sellMod: number):void {
	...
	player.gems += Math.floor(player.itemSlots[slot].itype.value / sellMod);
...
private benoitSellAllTransact(totalItems: number, sellMod: number):void {
	...
	for (var slot: number = 0; slot < 5; slot++) {
		if (player.itemSlots[slot].quantity > 0 && Math.floor(player.itemSlots[slot].itype.value / sellMod) >= 1) {
			itemValue += player.itemSlots[slot].quantity * Math.floor(player.itemSlots[slot].itype.value / sellMod);
