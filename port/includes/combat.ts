...
export function cleanupAfterCombat(nextFunc: Function = null): void {
...
export function packAttack(): void {
    //Determine if dodged!
    if (player.spe - monster.spe > 0 && Math.floor(Math.random() * (((player.spe - monster.spe) / 4) + 80)) > 80) {
    ...
    //Determine if cat'ed
        ...
        temp = Math.floor((monster.str + monster.weaponAttack) - rand(player.tou) - player.armorDef); //Determine damage - str modified by enemy toughness!
        if (temp <= 0) {
            temp = 0;
            if (!monster.plural)
...
function fireBow(): void {
    ...
    //Hit!  Damage calc! 20 +
    var damage: number = 0;
    damage = Math.floor((20 + player.str / 3 + player.statusAffectv1(StatusAffects.Kelt) / 1.2) + player.spe / 3 - rand(monster.tou) - monster.armorDef);
...
export function bite(): void {
    ...
    var damage: number = 0;
    //Determine if dodged!
    if ((player.findStatusAffect(StatusAffects.Blind) >= 0 && rand(3) != 0) || (monster.spe - player.spe > 0 && Math.floor(Math.random() * (((monster.spe - player.spe) / 4) + 80)) > 80)) {
    ...
    damage = Math.floor((player.str + 45) - rand(monster.tou) - monster.armorDef);
...
export function attack(): void {
    ...
    if (monster.short == "worms") {
        ...
        if (rand(100) + player.inte / 3 >= 50) {
            temp = Math.floor(player.str / 5 - rand(5));
            if (temp == 0) temp = 1;
    ...
    var damage: number = 0;
    //Determine if dodged!
    if ((player.findStatusAffect(StatusAffects.Blind) >= 0 && rand(2) == 0) || (monster.spe - player.spe > 0 && Math.floor(Math.random() * (((monster.spe - player.spe) / 4) + 80)) > 80)) {
	...
	if(player.hasPerk("Double Attack") >= 0 && player.str <= 60) {
		if(player.weaponName == "deadly spear") damage = Math.floor((player.str + player.weaponAttack) - Math.random()*(monster.tou));
		else if(player.weaponName == "jeweled rapier") damage = Math.floor((player.str + player.weaponAttack) - Math.random()*(monster.tou));
		else if(player.weaponName == "katana") damage = Math.floor((player.str + player.weaponAttack) - Math.random()*(monster.tou + monster.armorDef - 5));
		else damage = Math.floor((player.str + player.weaponAttack) - Math.random()*(monster.tou + monster.armorDef));
	}*/
    //BASIC DAMAGE STUFF
...
export function goreAttack(): void {
    ...
    if (temp >= rand(100)) {
        ...
        if (rand(4) > 0) {
            ...
            damage = Math.floor(player.str + horns * 2 - rand(monster.tou) - monster.armorDef);
        }
        //CRIT
        else {
            //doubles horn bonus damage
            damage = Math.floor(player.str + horns * 4 - rand(monster.tou) - monster.armorDef);
...
export function playerStinger(): void {
    ...
    if (monster.spe - player.spe > 0 && Math.floor(Math.random() * (((monster.spe - player.spe) / 4) + 80)) > 80) {
...
export function combatMiss(): Boolean {
    return player.spe - monster.spe > 0 && Math.floor(Math.random() * (((player.spe - monster.spe) / 4) + 80)) > 80;
...
function combatStatusesUpdate(): void {
    ...
    if (player.findStatusAffect(StatusAffects.StoneLust) >= 0) {
        ...
        dynStats("lus", 7 + Math.floor(player.sens) / 10);
    }
    ...
    if (player.findStatusAffect(StatusAffects.DemonSeed) >= 0) {
        ...
        dynStats("lus", (player.statusAffectv1(StatusAffects.DemonSeed) + Math.floor(player.sens / 30) + Math.floor(player.lib / 30) + Math.floor(player.cor / 30)));
    }
...
export function startCombatImmediate(monster: Monster, _plotFight: Boolean): void {
...
export function spellHeal(): void {
    ...
    if (rand(4) == 0) {
        ...
        temp = Math.floor((player.inte / (2 + rand(3)) * spellMod()) * (maxHP() / 150));
...
export function spellBlind(): void {
    ...
    if (monster is JeanClaude)
    {
        ...
        if (rand(player.spe) >= 50 || rand(player.inte) >= 50) {
            ...
            monster.HP -= Math.floor(10 + (player.inte / 3 + rand(player.inte / 2)) * spellMod());
        }
        else {
...
export function spellWhitefire(): void {
    ...
    temp = Math.floor(10 + (player.inte / 3 + rand(player.inte / 2)) * spellMod());
    //High damage to goes.
...
export function spellCleansingPalm(): void {
    ...
    temp = Math.floor((player.inte / 4 + rand(player.inte / 3)) * (spellMod() * corruptionMulti));
...
export function dragonBreath(): void {
    ...
    var damage: number = Math.floor(player.level * 8 + 25 + rand(10));
...
export function fireballuuuuu(): void {
    ...
    var damage: number;
    damage = Math.floor(player.level * 10 + 45 + rand(10));
    if (player.findStatusAffect(StatusAffects.GooArmorSilence) >= 0) {
        outputText("<b>A growl rumbles from deep within as you charge the terrestrial fire, and you force it from your chest and into the slime.  The goop bubbles and steams as it evaporates, drawing a curious look from your foe, who pauses in her onslaught to lean in and watch.  While the tension around your mouth lessens and your opponent forgets herself more and more, you bide your time.  When you can finally work your jaw enough to open your mouth, you expel the lion's - or jaguar's? share of the flame, inflating an enormous bubble of fire and evaporated slime that thins and finally pops to release a superheated cloud.  The armored girl screams and recoils as she's enveloped, flailing her arms.</b> ", false);
        player.removeStatusAffect(StatusAffects.GooArmorSilence);
        damage += 25;
...
export function corruptedFoxFire(): void {
    ...
    var dmg: number = Math.floor(10 + (player.inte / 3 + rand(player.inte / 2)) * spellMod());
...
export function immolationSpell(): void {
    ...
    var temp: number = Math.floor(75 + (player.inte / 3 + rand(player.inte / 2)) * spellMod());
    temp = doDamage(temp);
    outputText(" (" + temp + ")\n\n");
...
/* Now called directly
//5000 6999
export function doCombat(eventNum: number):void
{
	var temp2: any = 0;
	var temp3: any = 0;
	var temp4: any = 0;
	...
	if(eventNum == 5020) {
			...
			if (player.findStatusAffect(StatusAffects.StoneLust) >= 0) {
				...
				dynStats("lus", 8 + Math.floor(player.sens) / 10);
			}
			else {
				...
				dynStats("lus", 4 + Math.floor(player.sens) / 10);
			}
			doNext(1);
	}
	...
	if(eventNum == 5043) {
			...
			//Determine damage - str modified by enemy toughness!
			temp = Math.floor((monster.str + monster.weaponAttack) - rand(player.tou) - player.armorDef);
