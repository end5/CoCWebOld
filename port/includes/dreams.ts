import { breastDescript, chestDesc, vaginaDescript } from './descriptors';
import {
    addButton,
    clearOutput,
    doNext,
...
export function dreamSelect(): boolean {
	...
	var choices: Array<any> = new Array();
	//dream quantity
	var dreamtemp: number = 0;
	dreamtemp = rand(kGAMECLASS.player.lib/10 + kGAMECLASS.player.cor/20) + kGAMECLASS.player.cor/20;
	
	//BUILD UP CHOICES ARRAY
	var temp = kGAMECLASS.player.humanScore();
	//Add human numbers to choices
	while(temp > 0) {
		choices[choices.length] = 0;
		temp--;
	}
	temp = kGAMECLASS.player.dogScore();
	while(temp > 0) {
		choices[choices.length] = 2;
		temp--;
	}
	temp = kGAMECLASS.player.horseScore();
	while(temp > 0) {
		choices[choices.length] = 1;
		temp--;
	}
	temp = kGAMECLASS.player.cowScore();
	while(temp > 0) {
		choices[choices.length] = 3;
		temp--;
	}
	temp = kGAMECLASS.player.catScore();
	while(temp > 0) {
		choices[choices.length] = 4;
		temp--;
	}
	temp = kGAMECLASS.player.demonScore();
	while(temp > 0) {
		choices[choices.length] = 5;
		temp--;
	}
	//Two chances if addicted, three if with perk!
	if(kGAMECLASS.player.minotaurAddicted()) {
		choices[choices.length] = 6;
		choices[choices.length] = 6;
		if(kGAMECLASS.player.findPerk(PerkLib.MinotaurCumAddict) >= 0) choices[choices.length] = 6;
	}
	//Akbal
	if(kGAMECLASS.flags[kFLAGS.AKBAL_SUBMISSION_COUNTER] > 4) {
		if(kGAMECLASS.flags[kFLAGS.AKBAL_SUBMISSION_COUNTER] > 10) choices[choices.length] = 7;
		if(kGAMECLASS.flags[kFLAGS.AKBAL_SUBMISSION_COUNTER] > 13) choices[choices.length] = 7;
		if(kGAMECLASS.flags[kFLAGS.AKBAL_SUBMISSION_COUNTER] > 15) choices[choices.length] = 7;
		if(kGAMECLASS.flags[kFLAGS.AKBAL_SUBMISSION_COUNTER] > 18) choices[choices.length] = 7;
		choices[choices.length] = 7;
		choices[choices.length] = 7;
	}
	//Exgartuboobs
	if(kGAMECLASS.player.statusAffectv1(StatusAffects.Exgartuan) == 2) {
		choices[choices.length] = 8;
		choices[choices.length] = 8;
		choices[choices.length] = 8;
	}
	//Exgartucock
	if(kGAMECLASS.player.statusAffectv1(StatusAffects.Exgartuan) == 1 && kGAMECLASS.player.hasCock()) {
		choices[choices.length] = 9;
		choices[choices.length] = 9;
		choices[choices.length] = 9;
	}
	//Latexy Skinz
	if(kGAMECLASS.player.skinAdj == "latex" || kGAMECLASS.player.skinAdj == "rubber") {
	...
	//Dom + Scylla
	if(kGAMECLASS.flags[kFLAGS.NUMBER_OF_TIMES_MET_SCYLLA] > 0 && kGAMECLASS.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00150] > 0) {
		choices[choices.length] = 12;
		choices[choices.length] = 12;
		choices[choices.length] = 12;
		if(kGAMECLASS.player.hasCock() && kGAMECLASS.player.cor > 50) {
	...
	//Sand trap
	if(kGAMECLASS.player.sandTrapScore() >= 2) {
		choices[choices.length] = 16;
		choices[choices.length] = 16;
		choices[choices.length] = 16;
		choices[choices.length] = 16;
	}
	if(kGAMECLASS.player.mouseScore() >= 3) {
	...
	if (kGAMECLASS.player.findPerk(PerkLib.Lusty) >= 0) dreamLust += (dreamtemp / 3);
	
	// Well Adjusted cuts by half
	if (kGAMECLASS.player.findPerk(PerkLib.WellAdjusted) >= 0) dreamLust = (dreamLust / 2);
	...
	if(dreamtemp > 5) {
		...
		if(daydream == 0) {
			if(kGAMECLASS.player.cor <= 33) { 
				...
				if(rand(2) == 0) outputText("tender sex with a sweetheart back home.  ", false);
					...
					if(kGAMECLASS.player.gender <= 1 || (kGAMECLASS.player.gender == 3 && rand(2) == 0)) outputText("women ", false);
					else outputText("men ", false);
					outputText("massaging you, slowly moving their hands over your most intimate places.  ", false);
				}
			}
			if(kGAMECLASS.player.cor > 33 && kGAMECLASS.player.cor <= 66) {
		...
		//cow
			...
			if((kGAMECLASS.player.gender == 1 && kGAMECLASS.player.biggestTitSize() >= 5) || (kGAMECLASS.player.gender == 3 && rand(2) == 0)) {
				outputText("having full and lactating breasts, searching for the right person to give your milk to.  You come across a pretty human girl and invite her to your breast.  She eagerly jumps into your arms and starts to suckle from your ample bosom.  You then guide your erect " + kGAMECLASS.player.cockDescript(0) + " into her waiting lower lips.  The two of you stand there in eternal bliss as she suckles on your chest and you thrust into her womanhood, until you wake from the dream.  ", false);
			}
		...
		//cat
			...
			//FEMALE
			if(kGAMECLASS.player.hasVagina() && (!kGAMECLASS.player.hasCock() || rand(2) == 0))
		...
		//Scylla + dom + dong
			...
			if(kGAMECLASS.player.gender <= 1) {
				...
				//Male Trap Dream #2: 
					...
					if(kGAMECLASS.player.hasCock()) outputText("  [EachCock] is fully erect and throbbing.");
					else if(kGAMECLASS.player.hasVagina()) outputText("  Your [vagina] dripping wet with need.");
			...
			if(kGAMECLASS.player.gender == 0 || rand(2) == 0) {
...
export function dayTenDreams():void {
	...
	if(kGAMECLASS.player.cor < 50) dynStats("lus", 10);
...
export function displacerDreamIII():void {
	...
	var x: number = kGAMECLASS.player.cockTotal();
	while (x > 0) {
		x--;
		if(kGAMECLASS.player.cocks[x].sock == "amaranthine" && kGAMECLASS.player.cocks[x].cockType != CockTypesEnum.DISPLACER) {
			if(kGAMECLASS.player.cocks[x].cockType != CockTypesEnum.DOG) kGAMECLASS.player.cocks[x].knotMultiplier = 1.5;
			kGAMECLASS.player.cocks[x].cockType = CockTypesEnum.DISPLACER;
		}
	}
	doNext(playerMenu);
}
