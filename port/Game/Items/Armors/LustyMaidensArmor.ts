...
export class LustyMaidensArmor extends Armor {
    ...
    public canUse(): boolean {
        ...
        if (this.game.player.hasCock() || this.game.player.balls > 0) {
            ...
            if (this.game.player.hasCock()) outputText("or your " + this.game.player.multiCockDescriptLight());
        ...
        if (this.game.player.vaginas[0].virgin) {
            ...
            if (this.game.player.cor < 33) outputText("  Wait, that isn't right...");
        }
        else {
        ...
        if (this.game.player.wetness() >= 3) outputText("  The moisture you normally drip seems to soak right into the gusset instead of running down your [legs] like normal, giving you a much more chaste appearance in spite of the lewd garments that even now seem to shape your femininity and [butt] into perfectly arousing shapes.");
    ...
    public lustyMaidenPaizuri(player: Player = null, monster: Monster = null): void {
        ...
        if (monster == null) monster = this.game.monster;
        clearOutput();
