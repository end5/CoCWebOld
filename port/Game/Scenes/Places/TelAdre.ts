...
private oswaldPawnMenu():void { //Moved here from Inventory.as
	...
	for (var slot: number = 0; slot < 5; slot++) {
		if (player.itemSlots[slot].quantity > 0 && player.itemSlots[slot].itype.value >= 1) {
			outputText("\n" + Math.floor(player.itemSlots[slot].itype.value / 2) + " gems for " + player.itemSlots[slot].itype.longName + ".");
...
private oswaldPawnSell(slot: number):void { //Moved here from Inventory.as
	spriteSelect(47);
	var itemValue: number = Math.floor(player.itemSlots[slot].itype.value / 2);
	clearOutput();
	if (itemValue == 0)
...
private oswaldPawnSellAll():void {
	...
	for (var slot: number = 0; slot < 5; slot++) {
		if (player.itemSlots[slot].quantity > 0 && player.itemSlots[slot].itype.value >= 1) {
			itemValue += player.itemSlots[slot].quantity * Math.floor(player.itemSlots[slot].itype.value / 2);
...
private anotherButton(button: number, nam: string, func:Function, arg: any = -9000): number {
