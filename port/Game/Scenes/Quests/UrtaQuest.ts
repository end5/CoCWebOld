...
//			doNext(getGame().combatMenu);
	...
	var damage: number;
	//Determine if dodged!
	if(monster.findStatusAffect(StatusAffects.Blind) < 0 && (rand(3) == 0 || (player.findStatusAffect(StatusAffects.Blind) >= 0 && rand(2) == 0) || (monster.spe - player.spe > 0 && Math.floor(Math.random()*(((monster.spe-player.spe)/4)+80)) > 80))) {
