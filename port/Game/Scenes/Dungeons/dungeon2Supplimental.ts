...
import { gameOver, playerMenu } from './eventParser';
import { dungeonLoc } from './OnLoadVariables';
...
import { EncapsulationPod } from '../classes/classes/Scenes/Dungeons/DeepCave/EncapsulationPod';
import { ImpHorde } from '../classes/classes/Scenes/Dungeons/DeepCave/ImpHorde';
import { Vala } from '../classes/classes/Scenes/Dungeons/DeepCave/Vala';
...
export function leaveZetazsLair(): void {
    ...
    doNext(kGAMECLASS.camp.returnToCampUseOneHour);
}
...
export function impGangAI(): void {
    if (kGAMECLASS.monster.findStatusAffect(StatusAffects.ImpUber) >= 0) impGangUber();
    else if (kGAMECLASS.monster.lust > 50 && rand(2) == 0) impGangBukkake();
    else {
    ...
    if (rand(2) == 0) {
        outputText("\nOne of the tiny demons latches onto one of your " + kGAMECLASS.player.legs() + " and starts humping it.  You shake the little bastard off and keep fighting!", false);
        dynStats("lus", 1);
    }
    combatRoundOver();
}
...
export function imtacularMultiHitzilla(): void {
    //Multiattack:
    if (kGAMECLASS.monster.findStatusAffect(StatusAffects.Blind) < 0) outputText("The imps come at you in a wave, tearing at you with claws!\n", false);
    //(ALT BLINDED TEXT)
    ...
    while (hits > 0) {
        ...
        damage = 0;
        //Blind dodge change
        if (kGAMECLASS.monster.findStatusAffect(StatusAffects.Blind) >= 0 && rand(3) < 2) {
            outputText(kGAMECLASS.monster.capitalA + kGAMECLASS.monster.short + " completely misses you with a blind attack!\n", false);
        }
        //Determine if dodged!
        else if (kGAMECLASS.player.spe - kGAMECLASS.monster.spe > 0 && rand(((kGAMECLASS.player.spe - kGAMECLASS.monster.spe) / 4) + 90) > 80) {
            if (kGAMECLASS.player.spe - kGAMECLASS.monster.spe < 8) outputText("You narrowly avoid " + kGAMECLASS.monster.a + kGAMECLASS.monster.short + "'s " + kGAMECLASS.monster.weaponVerb + "!\n", false);
            else if (kGAMECLASS.player.spe - kGAMECLASS.monster.spe >= 8 && kGAMECLASS.player.spe - kGAMECLASS.monster.spe < 20) outputText("You dodge " + kGAMECLASS.monster.a + kGAMECLASS.monster.short + "'s " + kGAMECLASS.monster.weaponVerb + " with superior quickness!\n", false);
            else if (kGAMECLASS.player.spe - kGAMECLASS.monster.spe >= 20) outputText("You deftly avoid " + kGAMECLASS.monster.a + kGAMECLASS.monster.short + "'s slow " + kGAMECLASS.monster.weaponVerb + ".\n", false);
        }
        //Determine if evaded
        else if (kGAMECLASS.player.findPerk(PerkLib.Evade) >= 0 && rand(100) < 10) {
            outputText("Using your skills at evading attacks, you anticipate and sidestep " + kGAMECLASS.monster.a + kGAMECLASS.monster.short + "'s attack.\n", false);
        }
        else if (kGAMECLASS.player.findPerk(PerkLib.Misdirection) >= 0 && rand(100) < 10 && kGAMECLASS.player.armorName == "red, high-society bodysuit") {
            outputText("With the easy movement afforded by your bodysuit and Raphael's teachings, you easily avoid " + kGAMECLASS.monster.a + kGAMECLASS.monster.short + "'s attack.\n", false);
        }
        //Determine if cat'ed
        else if (kGAMECLASS.player.findPerk(PerkLib.Flexibility) >= 0 && rand(100) < 6) {
            outputText("With your incredible flexibility, you squeeze out of the way of " + kGAMECLASS.monster.a + kGAMECLASS.monster.short + "", false);
            if (kGAMECLASS.monster.plural) outputText("' attacks.\n", false);
        ...
        //OH SHIT SON YOU GOT REAPED
            ...
            damage = 20 - rand(kGAMECLASS.player.tou / 10);
...
export function impGangBukkake(): void {
    ...
    while (hits > 0) {
        ...
        //Blind dodge change
        if (kGAMECLASS.monster.findStatusAffect(StatusAffects.Blind) >= 0 && rand(3) < 2) {
            outputText(kGAMECLASS.monster.capitalA + kGAMECLASS.monster.short + "' misguided spooge flies everyone.  A few bursts of it don't even land anywhere close to you!\n", false);
        }
        //Determine if dodged!
        else if (kGAMECLASS.player.spe - kGAMECLASS.monster.spe > 0 && rand(((kGAMECLASS.player.spe - kGAMECLASS.monster.spe) / 4) + 90) > 80) {
            damage = rand(4);
        ...
        //Determine if evaded
        else if (kGAMECLASS.player.findPerk(PerkLib.Evade) >= 0 && rand(100) < 30) {
            damage = rand(4);
        ...
        //Determine if cat'ed
        else if (kGAMECLASS.player.findPerk(PerkLib.Flexibility) >= 0 && rand(100) < 15) {
            damage = rand(4);
        ...
        //(2-6 hits for +10 lust each!) (-5 lust per successful hit)
            ...
            if (damage == 2) outputText("Some of the fluid splatters into your midriff and drools down to your waist, making your " + kGAMECLASS.player.armorName + " slimy and wet.\n", false);
            ...
            if (damage == 5) outputText("The last eruption of cum soaks your thighs and the lower portions of your " + kGAMECLASS.player.armorName + ", turning it a sticky white.\n", false);
            dynStats("lus", (7 + Math.floor(kGAMECLASS.player.lib / 40 + kGAMECLASS.player.cor / 40)));
        }
        kGAMECLASS.monster.lust -= 5;
        hits--;
    }
}
...
export function impGangUber(): void {
    if (kGAMECLASS.monster.findStatusAffect(StatusAffects.ImpUber) < 0) {
        ...
        kGAMECLASS.monster.createStatusAffect(StatusAffects.ImpUber, 0, 0, 0, 0);
    }
    else {
        //(OH SHIT IT GOES OFF) 
        //+50 lust!
        ...
        if (kGAMECLASS.player.biggestLactation() > 1) outputText(" as they begin to drip milk", false);
        outputText(".  The lower portions of your coverings ", false);
        if (kGAMECLASS.player.cockTotal() > 0) {
            outputText("are pulled tight by your " + kGAMECLASS.player.multiCockDescript() + ", ", false);
            if (kGAMECLASS.player.totalCocks() > 1) outputText("each ", false);
            outputText("beading a drop of pre-cum at the tip", false);
            if (kGAMECLASS.player.hasVagina()) outputText(", and in addition, the clothes around your groin ", false);
        }
        if (kGAMECLASS.player.hasVagina()) {
            outputText("become stained with feminine moisture", false);
            if (kGAMECLASS.player.clitLength > 3) outputText(" as your clit swells up in a more sensitive imitation of a cock", false);
        }
        if (kGAMECLASS.player.gender == 0) outputText("rub the sensitive skin of your thighs and featureless groin in a way that makes you wish you had a sex of some sort", false);
        outputText(".\n", false);
        kGAMECLASS.monster.removeStatusAffect(StatusAffects.ImpUber);
    }
}
...
export function loseToImpMob(): void {
    outputText("", true);
    //(HP) 
    if (kGAMECLASS.player.HP < 1) outputText("Unable to handle your myriad wounds, you collapse with your strength exhausted.\n\n", false);
    //(LUST)
    ...
    outputText("In seconds, the squirming red bodies swarm over you, blotting the rest of the room from your vision.  You can feel their scrabbling fingers and hands tearing off your " + kGAMECLASS.player.armorName + ", exposing your body to their always hungry eyes.   Their loincloths disappear as their growing demonic members make themselves known, pushing the tiny flaps of fabric out of the way or outright tearing through them.   You're groped, touched, and licked all over, drowning in a sea of long tongues and small nude bodies.\n\n", false);
    ...
    //(tits)
    if (kGAMECLASS.player.biggestTitSize() > 1) {
        ...
        if (kGAMECLASS.player.biggestLactation() > 1) outputText("  Milk starts to squirt from the pressure being applied to your " + breastDescript(0) + ", which only encourages the imp to squeeze even harder.", false);
        outputText("\n\n", false);
    }
    //(NIPPLECUNTS!)
    if (kGAMECLASS.player.hasFuckableNipples()) {
        ...
        if (kGAMECLASS.player.biggestTitSize() < 7) outputText("  He can only get partway in, but it doesn't seem to deter him.", false);
        ...
        if (kGAMECLASS.player.totalNipples() > 2) outputText("each of ", false);
        outputText("your other " + nippleDescript(0), false);
        if (kGAMECLASS.player.totalNipples() > 2) outputText("s", false);
        outputText(", fighting over the opening", false);
        if (kGAMECLASS.player.totalNipples() > 2) outputText("s", false);
        outputText(".  A victor quickly emerges, and in no time ", false);
        if (kGAMECLASS.player.totalNipples() == 2) outputText("both", false);
    ...
    if (!kGAMECLASS.player.hasVagina()) {
        ...
        kGAMECLASS.player.buttChange(12, true, true, false);
        outputText("\n\n", false);
    }
    //(DOUBLE PEN)
    else {
    ...
    if (kGAMECLASS.player.hasVagina()) outputText("A thick load bastes your pussy with whiteness, and you can feel it seeping deeper inside your fertile womb.  ", false);
    ...
    if (kGAMECLASS.player.biggestTitSize() > 1) {
        ...
        if (kGAMECLASS.player.hasFuckableNipples()) {
            if (kGAMECLASS.player.totalNipples() == 2) outputText("  The pair", false);
    ...
    if (kGAMECLASS.player.cockTotal() == 0) outputText("and you clench tightly around the uncomfortable intrusion.", false);
    else {
        outputText("and " + kGAMECLASS.player.sMultiCockDesc() + " unloads, splattering the many demons with a bit of your own seed.  You'd smile if your mouth wasn't so full of cock.  At least you got to make a mess of them!", false);
    }
    if (kGAMECLASS.player.hasVagina()) {
    ...
    if (!kGAMECLASS.player.hasVagina()) outputText(" and forced to start drinking bottle after bottle of succubi milk", false);
...
export function loseToImpMobII(): void {
    ...
    if (kGAMECLASS.player.biggestTitSize() < 1) outputText("new", false);
    ...
    kGAMECLASS.player.orgasm();
    kGAMECLASS.player.HP += 100;
    gameOver();
}
...
export function impGangVICTORY(): void {
    outputText("", true);
    //Flag them defeated!
    kGAMECLASS.monster[kFLAGS.ZETAZ_IMP_HORDE_DEFEATED] = 1;
    if (kGAMECLASS.monster.HP < 1) outputText("The last of the imps collapses into the pile of his defeated comrades.  You're not sure how you managed to win a lopsided fight, but it's a testament to your new-found prowess that you succeeded at all.", false);
    ...
    if (kGAMECLASS.player.lust >= 33 && kGAMECLASS.player.gender > 0) {
        outputText("\n\nFeeling a bit horny, you wonder if you should use them to sate your budding urges before moving on.  Do you rape them?", false);
        if (kGAMECLASS.player.gender == 1) simpleChoices("Rape", impGangGetsRapedByMale, "", null, "", null, "", null, "Leave", cleanupAfterCombat);
        if (kGAMECLASS.player.gender == 2) simpleChoices("Rape", impGangGetsRapedByFemale, "", null, "", null, "", null, "Leave", cleanupAfterCombat);
        if (kGAMECLASS.player.gender == 3) simpleChoices("Male Rape", impGangGetsRapedByMale, "Female Rape", impGangGetsRapedByFemale, "", null, "", null, "Leave", cleanupAfterCombat);
    }
...
export function impGangGetsRapedByMale(): void {
    ...
    if (kGAMECLASS.player.balls > 0) outputText(ballsDescriptLight(), false);
    else if (kGAMECLASS.player.hasVagina()) outputText(vaginaDescript(0), false);
    else outputText("ass", false);
    outputText(", licking with care", false);
    if (kGAMECLASS.player.balls == 0) outputText(" and plunging deep inside", false);
    outputText(".  The imp to the right wraps his tongue around the base ", false);
    if (kGAMECLASS.player.hasSheath()) outputText("just above your sheath ", false);
    ...
    if (kGAMECLASS.player.balls > 0) outputText("Cum boils in your balls, ready to paint your foe white.  ", false);
    ...
    if (kGAMECLASS.player.cumQ() > 1000) outputText("  You keep cumming while the other two imps keep licking and servicing you.   By the time you finish, they're glazed in spooge and masturbating as well.", false);
    outputText("\n\n", false);
...
export function impGangGetsRapedByFemale(): void {
    outputText("", true);
    outputText("You walk around to one of the demons and push him onto his back.  Your " + kGAMECLASS.player.armorName + " falls to the ground around you as you disrobe, looking over your tiny conquest.  A quick ripping motion disposes of his tiny loincloth, leaving his thick demon-tool totally unprotected. You grab and squat down towards it, rubbing the corrupted tool between your legs ", false);
    if (kGAMECLASS.player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_SLICK) outputText("and coating it with feminine drool ", false);
    ...
    kGAMECLASS.player.cuntChange(15, true, true, false);
    outputText("\n\n", false);
    ...
    if (kGAMECLASS.player.clitLength > 3) outputText("You stroke the cock-like appendage in your hand, trembling with delight.  ", false);
    ...
    if (kGAMECLASS.player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_SLAVERING) outputText("Cunt juices splatter him as you squirt explosively, leaving a puddle underneath him.  ", false);
    ...
    if (kGAMECLASS.player.pregnancyIncubation == 0) outputText("  You'll probably get pregnant.", false);
    dynStats("cor", 1);
    kGAMECLASS.player.orgasm();
    kGAMECLASS.player.knockUp(PregnancyStore.PREGNANCY_IMP, PregnancyStore.INCUBATION_IMP - 14, 50);
    cleanupAfterCombat();
}

export function enterZetazsRoomFromTheSouth(): void {
    if (kGAMECLASS.monster[kFLAGS.ZETAZ_DOOR_UNLOCKED] == 0) {
        clearOutput();
...
export function getSwordAndGetTrapped(): void {
    ...
    if (kGAMECLASS.player.spe < 50) outputText("fall flat on your " + assDescript() + ", slipping on the slick, shifting surface.", false);
...
export function getTrappedContinuation(): void {
    ...
    outputText("Meanwhile, a number of tentacles have sprung up from below, and are crawling up your " + kGAMECLASS.player.legs() + ".  It's becoming fairly clear how the skeleton wound up in this cave...  You've got to escape!", false);
...
export function encapsulationPodAI(): void {
    //[Round 1 Action]
    if (kGAMECLASS.monster.findStatusAffect(StatusAffects.Round) < 0) {
        outputText("You shiver from the feeling of warm wetness crawling up your " + kGAMECLASS.player.legs() + ".   Tentacles brush against your ", false);
        if (kGAMECLASS.player.balls > 0) {
            outputText(ballsDescriptLight() + " ", false);
            if (kGAMECLASS.player.hasVagina()) outputText("and ", false);
        }
        if (kGAMECLASS.player.hasVagina()) outputText(vaginaDescript(0) + " ", false);
        else if (kGAMECLASS.player.balls == 0) outputText("taint ", false);
        ...
        if (kGAMECLASS.player.lust < 35) {
            dynStats("lus", 1);
            kGAMECLASS.player.lust = 35;
            statScreenRefresh();
        }
    }
    //[Round 2 Action]
    else if (kGAMECLASS.monster.statusAffectv1(StatusAffects.Round) == 2) {
        outputText("The tentacles under your " + kGAMECLASS.player.armorName + " squirm against you, seeking out openings to penetrate and genitalia to caress.  ", false);
        if (kGAMECLASS.player.balls > 0) outputText("One of them wraps itself around the top of your " + sackDescript() + " while its tip slithers over your " + ballsDescriptLight() + ".  Another ", false);
        else outputText("One ", false);
        if (kGAMECLASS.player.cockTotal() > 0) {
            outputText("prods your " + cockDescript(0) + " for a second before it begins slithering around it, snake-like.  Once it has you encircled from " + kGAMECLASS.player.cockHead() + " to ", false);
            if (!kGAMECLASS.player.hasSheath()) outputText("base", false);
            ...
            if (kGAMECLASS.player.hasVagina()) {
                ...
                if (kGAMECLASS.player.clitLength > 4) outputText("It brushes your " + clitDescript() + " then curls around it, squeezing and gently caressing it with a slow, pleasing rhythm.  ", false);
            }
            else {
        ...
        if (kGAMECLASS.player.cockTotal() > 1) {
            outputText("Your other ", false);
            if (kGAMECLASS.player.cockTotal() == 2) outputText(cockDescript(1) + " gets the same treatment, and soon both of your " + kGAMECLASS.player.multiCockDescriptLight() + " are quite happy to be here.  ", false);
            else outputText(kGAMECLASS.player.multiCockDescriptLight() + " get the same treatment and soon feel quite happy to be here.  ", false);
        }
        if (kGAMECLASS.player.hasVagina()) {
            ...
            if (kGAMECLASS.player.clitLength > 3) outputText("and once it discovers how large it is, it wraps around it and squeezes.  It feels good!  ", false);
        ...
        if (kGAMECLASS.player.pregnancyIncubation > 0 && kGAMECLASS.player.pregnancyIncubation < 120) outputText("pregnant ", false);
        outputText("belly as they get closer and closer to ", false);
        if (kGAMECLASS.player.biggestTitSize() < 1) outputText("your chest", false);
        ...
        if (kGAMECLASS.player.lust < 65) {
            dynStats("lus", 1);
            kGAMECLASS.player.lust = 65;
            statScreenRefresh();
        }
    }
    //[Round 3 Action]
    else if (kGAMECLASS.monster.statusAffectv1(StatusAffects.Round) == 3) {
        outputText("The wet, warm pressure of the fungus' protrusion working their way up your body feels better than it has any right to be.  It's like a combination of a warm bath and a gentle massage, and when combined with the thought-numbing scent in the air, it's nigh-impossible to resist relaxing a little.  In seconds the mass of tentacles is underneath your " + kGAMECLASS.player.armorName + " and rubbing over your chest and " + nippleDescript(0) + "s.  You swoon from the sensation and lean back against the wall while they stroke and caress you, teasing your sensitive " + nippleDescript(0) + ".", false);
        if (kGAMECLASS.player.hasFuckableNipples()) outputText("  Proof of your arousal leaks from each " + nippleDescript(0) + " as their entrances part for the probing tentacles.  They happily dive inside to begin fucking your breasts, doubling your pleasure.", false);
        ...
        if (kGAMECLASS.player.lust < 85) {
            dynStats("lus", 1);
            kGAMECLASS.player.lust = 85;
            statScreenRefresh();
        }
    }
    //[Round 4 Action]
    else {
        outputText("What's happening to you definitely isn't rape.  Not any more.  You like it too much.  You lean back against a wall of the pod and thrust your " + hipDescript() + " pitifully against a phantom lover, moaning lewdly as you're forcibly pleasured.  You grab hold of the fleshy walls with your hands and try to hold yourself up, but your " + kGAMECLASS.player.legs() + " have the consistency of jello.   They fold neatly underneath you as you slide into the ooze and begin to float inside it.  It's comforting in an odd way, and while you're gasping in between moans, your balance finally gives out.  You sink deeper into the fluid and lose all sense of direction.  Up and down become meaningless constructs that no longer matter to you.\n\n", false);
        ...
        if (kGAMECLASS.player.totalCocks() > 0) {
            outputText(", and " + kGAMECLASS.player.sMultiCockDesc() + " bloats with blood, getting larger than ever", false);
        }
        ...
        if (kGAMECLASS.player.hasFuckableNipples()) {
            ...
            if (kGAMECLASS.player.biggestLactation() > 1) outputText("and milk ", false);
            outputText("while the fucking continues.", false);
        }
        if (kGAMECLASS.player.cockTotal() > 0) {
            outputText("  The tentacles around " + kGAMECLASS.player.sMultiCockDesc() + " squeeze and rotate, screwing you silly through your orgasm while cum dribbles in a steady stream from your loins.  Normally it would be squirting out in thick ropes, but the muscle-relaxing drugs in your system make the spurts a steady, weak flow.", false);
            if (kGAMECLASS.player.cumQ() > 800) outputText("  Of course with all the semen you produce, the flesh-pod's ooze clouds over quite quickly, blocking your vision with a purple-white haze.", false);
        }
        if (kGAMECLASS.player.biggestLactation() > 1) {
            outputText("Milk leaks out too, ", false);
            if (kGAMECLASS.player.biggestLactation() < 2) outputText("though the slight dribble is barely noticeable to you.", false);
            else if (kGAMECLASS.player.biggestLactation() < 3) outputText("coloring things a little more white.", false);
    ...
    //Set kGAMECLASS.monster for rounds
    if (kGAMECLASS.monster.findStatusAffect(StatusAffects.Round) < 0) {
        kGAMECLASS.monster.createStatusAffect(StatusAffects.Round, 2, 0, 0, 0);
    }
    else kGAMECLASS.monster.addStatusValue(StatusAffects.Round, 1, 1);
    combatRoundOver();
}
...
export function loseToThisShitPartII(): void {
    ...
    outputText("", true);
    //[OPTIONAL CUM ESCAPE]
    if (kGAMECLASS.player.cumQ() > 3500) {
        outputText("Your orgasm drags on for so long that you begin to feel pressure from the cum-slime surrounding you.  It doesn't seem to matter to " + kGAMECLASS.player.sMultiCockDesc() + ", which is too busy sending bliss to your brain and squirting cum for the tentacles to care.  It actually kind of hurts.  The oscillating purple ambiance flashes brighter in protest for a second, and then everything releases all at once.  The pressure is gone and you're sliding down on a wave of fungal-slime cum, feeling the tentacles being pulled from you by the sudden shift of position.  Moist cave air tickles at your " + kGAMECLASS.player.skinDesc + " as you come to rest on another spongy petal and begin to cough out the sludge.\n\n", false);

        outputText("Over the next minute your head clears and your strength returns.  You push yourself up on something hard, then glance down and realize you washed up next to the skeleton!  The bleached bone leers up at you knowingly, and everything you can see is covered in a thick layer of your spooge.  " + kGAMECLASS.player.SMultiCockDesc() + " is still dripping more spunk.  Clearly your ruined orgasm didn't pump it ALL out.  You look down at the rapier and pick it up out of your mess, examining it.  The blade shines keenly, and the sword is balanced to perfection.  Though you succumbed to the same fate as its owner, your warped body saved you from sharing his fate.  Thankfully potential pods that carpet the floor don't even twitch at you.  Perhaps your orgasm was enough to sate them all?  Or maybe they've learned their lesson.", false);
        //(switch from loss to victory, sword loot)
        kGAMECLASS.monster.lust = 100;
        kGAMECLASS.player.orgasm();
    }
    //[OPTIONAL MILK ESCAPE]
    else if (kGAMECLASS.player.lactationQ() > 3500 || (kGAMECLASS.player.lactationQ() + kGAMECLASS.player.cumQ() > 4500)) {
    ...
    //(GENDERLESS)
    else if (kGAMECLASS.player.gender == 0) {
        ...
        kGAMECLASS.monster.lust = 100;
        kGAMECLASS.monster.XP = 1;
        kGAMECLASS.player.orgasm();
    }
    //Done if escaped
    if (kGAMECLASS.monster.lust == 100) {
        kGAMECLASS.monster[kFLAGS.ZETAZ_FUNGUS_ROOM_DEFEATED]++;
        cleanupAfterCombat();
        return;
    }
    //[BAD-END GO]
    //(MALE)  
    if (kGAMECLASS.player.gender == 1 || (kGAMECLASS.player.gender == 3 && rand(2) == 0)) {
        outputText("The orgasm squirts and drips from " + kGAMECLASS.player.sMultiCockDesc() + " for what seems like forever.  It feels so right, so perfect, that you actually whine in disappointment when it finally does end.  You can't even be bothered to reach down and stroke yourself.  The softening in your loins is nothing compared to your flaccid, listless muscles.  You couldn't make your arms reach down to touch yourself even if you could work up the motivation to try.  Thankfully the slippery tentacles curl back around your ", false);
        if (!kGAMECLASS.player.hasSheath()) outputText("base", false);
        else outputText("sheath", false);
        outputText(" and squeeze, forcing " + kGAMECLASS.player.sMultiCockDesc() + " to inflate to readiness.  Deep inside your " + assholeDescript() + ", the tentacle starts to rub against your prostate.  It caresses the male organ on each side and pauses to squeeze the center of it, pushing a few drops of sticky cum from your trembling " + Appearance.cockNoun(CockTypesEnum.HUMAN) + ".\n\n", false);

        outputText("The vine-like stalks currently hugging " + kGAMECLASS.player.sMultiCockDesc() + " constrict the base and begin to swirl around it in a circular motion.  Warm fungi-flesh and viscous, drugged ooze work together to send hot spikes of pleasure up your spinal-cord.  Despite your recent orgasm, you aren't being given any chance to recover or refill your ", false);
        if (kGAMECLASS.player.balls > 0) outputText("balls", false);
        ...
        if (kGAMECLASS.player.cockTotal() > 1) outputText("s", false);
...
export function useVala(): void {
    ...
    //HERMY WORMY
    else if (kGAMECLASS.player.gender == 3) {
        outputText("You shrug. The girl is so hopelessly lost in pleasure that you doubt she could ever return to the real world anyway. There donesn't seem to be any demons around, and it'd be a good idea to relieve the tension that's been building in your gut since you stepped into this wretched place. Besides, you chuckle as you strip off your " + kGAMECLASS.player.armorName + ", you've always wondered what it'd be like to take a fairy.\n\n", false);
...
export function useValaPtII(): void {
    ...
    //HERPY DERPY HERMY
        ...
        if (kGAMECLASS.player.lowerBody == LOWER_BODY_TYPE_NAGA) outputText("tail", false);
...
export function freeValazLooseCoochie(): void {
    ...
    //[Heal (if the kGAMECLASS.player has Pure Honey)] [Sex] [Reject]
    var heal: Function = null;
    if (kGAMECLASS.player.hasItem(kGAMECLASS.consumables.PURHONY, 1)) heal = healVala;
    if (kGAMECLASS.player.hasItem(kGAMECLASS.consumables.P_PEARL, 1)) heal = healVala;
    //Choicez go here.  I can haz fucks?
    simpleChoices("Fix Her", heal, "Sex", (kGAMECLASS.player.gender > 0 ? ValaGetsSexed : null), "Reject", rejectFuckingVala, "", null, "", null);
}
...
export function healVala(): void {
    spriteSelect(85);
    outputText("", true);
    if (kGAMECLASS.player.hasItem(kGAMECLASS.consumables.PURHONY, 1)) {
        kGAMECLASS.player.consumeItem(kGAMECLASS.consumables.PURHONY, 1);
        kGAMECLASS.monster[kFLAGS.VALA_HEALED_HONEY] = 1;
    ...
    //Pearl!
        ...
        kGAMECLASS.player.consumeItem(kGAMECLASS.consumables.P_PEARL, 1);
        outputText("A soft, warm breeze rustles your " + hairDescript() + " and for a moment the foul stench of the dungeon vanishes, setting your mind at ease and draining the tension that has been building in your gut. In a moment of clarity, you remember the beautiful Pure Pearl that Marae granted you as a reward for shutting down the demons' factory. It seems only right to use the goddess' gift to rescue one of her wayward children. Perhaps she gave it to you for this very reason? The oblivious girl has managed to work her way to a sitting position and is grinding her dripping sex against your " + kGAMECLASS.player.foot() + ". You lean down and gently lift her chin up, bringing her empty, pink eyes to stare into yours. Mistaking the gentle motion for a command, she gleefully opens wide, tongue thrashing about in anticipation. You place the pink pearl at the fairy's lips and she wraps her mouth around the pale jewel, trying obediently to swallow it. However, the little fairy's mouth is far smaller than you would've thought and it seems to get stuck just behind her teeth, like a pink ball-gag. She coos a muffled moan and begins to masturbate without breaking eye contact with you.\n\n", false);
        ...
        outputText("Leaving the way you came, Vala makes her exodus from the abyssal cavern. Despite her savagely warped body, you do not doubt that her renewed vigor for life will let her achieve some measure of happiness again. You feel like you've managed to do a truly selfless thing in this den of iniquity. Defeating kGAMECLASS.monsters is satisfying, but it's the lives you save that really make you feel like a hero. You sigh contentedly and wonder where she'll end up, now that she's been given her life back.", false);

        //(Vala unlocked in The Wet Bitch)[End Encounter]
        kGAMECLASS.monster[kFLAGS.FREED_VALA] = 1;
        doNext(playerMenu);
    }
...
export function valaGetsSexedPtDuece(): void {
    ...
    if (kGAMECLASS.player.cockTotal() > 0) {
        ...
        if (x >= 0) {
            ...
            if (kGAMECLASS.player.hasVagina()) outputText("slit.\n\n", false);
        ...
        //(large girth dicks)
            ...
            if (kGAMECLASS.player.biggestTitSize() < 1) outputText("chest", false);
            ...
            if (kGAMECLASS.player.hasVagina()) outputText("nuzzling your joy buzzer", false);
...
export function rejectFuckingVala(): void {
    spriteSelect(85);
    outputText("", true);
    if (kGAMECLASS.monster[kFLAGS.TIMES_PC_DEFEATED_VALA] == 0) {
        outputText("The fairy's weak insistence has begun to get obnoxious. What kind of prisoner dry humps her rescuer? Actually, if the heavy flow of lubricating girl cum dripping out of her pussy is any indication, it's the wettest humping you've had with your " + kGAMECLASS.player.armorName + " still on. You seize the girl's shoulders and hold her up, pushing her away from your goo-stained lower body. You assure the girl that you won't be having sex with her here. It's far too dangerous, you tell her, to leave yourself vulnerable right now. You'll take her to safety when the demons are defeated. You try to impress on her the importance of speed and stealth, but you might as well be talking to a big-breasted brick wall. When she makes a grab at your crotch, you've had enough and throw her back.\n\n", false);
        ...
        if (kGAMECLASS.monster[kFLAGS.TIMES_PC_DEFEATED_VALA_AND_RAEPED] > 0) outputText("  You smirk in wonder at the horny little slut.  You guess you'll have to put her back into her place and give her another dose of 'love'.", false);
        outputText("\n\n", false);
...
export function loseToVala(): void {
    spriteSelect(85);
    if (kGAMECLASS.player.gender == 3) loseToValaAsHerm();
    if (kGAMECLASS.player.gender == 1) loseToValaAsMale();
    if (kGAMECLASS.player.gender == 2) loseToValaFemale();
    if (kGAMECLASS.player.gender == 0) {
        outputText("", true);
        outputText("Vala forces a bottle into your throat before your defeated form has a chance to react, and you grunt with pleasure as a new gash opens between your " + kGAMECLASS.player.legs() + "!", false);
        kGAMECLASS.player.createVagina();
        kGAMECLASS.player.gender = 2;
...
export function loseToValaAsMale(): void {
    ...
    //[Next]
    if (kGAMECLASS.player.ass.analLooseness < 2) doNext(loseToValaAsMaleIITight);
    else if (kGAMECLASS.player.ass.analLooseness < 3) doNext(loseToValaMaleIILoose);
    else if (kGAMECLASS.player.ass.analLooseness < 5) doNext(loseToValaMaleIIVeryLoose);
...
export function loseToValaAsMaleIITight(): void {
    ...
    outputText("", true);
    //(tight ass)
    //	if(kGAMECLASS.player.ass.analLooseness <= 1) {
    ...
    kGAMECLASS.player.ass.analLooseness = 2;
    //[Next]
...
export function loseToValaMaleIILoose(): void {
    ...
    kGAMECLASS.player.ass.analLooseness++;
    //[Next]
...
export function loseToValaMaleIIVeryLoose(): void {
    ...
    kGAMECLASS.player.ass.analLooseness = 5;
    //[Next]
...
export function fightValaVictory(): void {
    spriteSelect(85);
    outputText("", true);
    outputText("The fairy girl collapses, well-drilled obedience robbing her limbs of their fight. She squirms to a crouching bow, fully accepting you as her new " + kGAMECLASS.player.mf("Master", "Mistress") + ". The warped fae's empty eyes look up at you, her face a mask of rapture as she anxiously awaits her punishment, wagging her butt in the air as lubrication gushes down her thighs. It seems being defeated has excited the broken creature to a breeding frenzy. Her endurance must be incredible to be this frisky after your battle.", false);
    kGAMECLASS.monster[kFLAGS.TIMES_PC_DEFEATED_VALA]++;
    //[Fuck] [Leave]
    if (kGAMECLASS.player.gender > 0) {
...
export function valaFightVictoryFuck(): void {
    ...
    if (kGAMECLASS.player.gender == 2) {
        ...
        //(PC Has Wings: 
        if (kGAMECLASS.player.wingType > WING_TYPE_NONE) outputText("You decide to give her a hand, and flap your " + kGAMECLASS.player.wingDesc + " as hard as you can, the added thrust giving her the start she needs.  ", false);
    ...
    //(Male)
    else if (kGAMECLASS.player.gender == 1) {
        outputText("If the girl's got energy to spare, you'd better try to wear her down or she might alert more demons after you leave. Commanding her to bow before you, the broken fairy is all too happy to comply, falling on her face, her arms spread wide, panting with desire. You circle around her and strip your " + kGAMECLASS.player.armorName + " off, admiring her plump little ass, all the plumper for how whip-thin her waist and legs are. You bend down to grab her waist and haul her ass upward, spearing her on your stiffening " + cockDescript(0) + " without further ceremony. It's important she knows that foreplay is for obedient pets, you note to yourself with a grin. With a vicious thrust, you fill the girl's stretched pussy and bend her over at a right angle. Smacking her ass, you command the little bitch to fly and she shivers with delight.\n\n", false);
    ...
    //(Herm)
        ...
        outputText("underside of your " + kGAMECLASS.player.foot() + " against the back of her head and apply pressure, forcing her face toward the seething meal. \"<i>Eat up,</i>\" you command the fairy, and her resistance crumbles, allowing her face to be fully submerged in the spunk, bubbles rising from the slimy bowl as she begins to suck it down with mindless obedience.\n\n", false);
...
export function valaFightVictoryFuckPtII(): void {
    ...
    //herm2
        ...
        //[2 dicks – 
        if (kGAMECLASS.player.totalCocks() > 1) {
...
export function leftValaAlone(): void {
    ...
    outputText("", true);
    //first post meeting
    if (kGAMECLASS.monster[kFLAGS.INVESTIGATED_VALA_AFTER_ZETAZ_DEFEATED] == 0 || kGAMECLASS.monster[kFLAGS.TIME_SINCE_VALA_ATTEMPTED_RAPE_PC] == 0) {
        ...
        kGAMECLASS.monster[kFLAGS.INVESTIGATED_VALA_AFTER_ZETAZ_DEFEATED]++;
        kGAMECLASS.monster[kFLAGS.TIME_SINCE_VALA_ATTEMPTED_RAPE_PC] = 6;
    }
    //Anytime within 6 hours of waking/raping/meeting her
    else if (kGAMECLASS.monster[kFLAGS.TIME_SINCE_VALA_ATTEMPTED_RAPE_PC] > 0) {
    ...
    if (kGAMECLASS.player.lust >= 33 && kGAMECLASS.shouldraFollower.followerShouldra()) addButton(3, "ShouldraVala", kGAMECLASS.shouldraFollower.shouldraMeetsCorruptVala);
...
export function tryToHealVala(): void {
    ...
    outputText("", true);
    //(Without Pure Honey)
    if (!(kGAMECLASS.player.hasItem(kGAMECLASS.consumables.PURHONY, 1) || kGAMECLASS.player.hasItem(kGAMECLASS.consumables.P_PEARL, 1))) {
    ...
    //(With Pure Honey)
    else if (kGAMECLASS.player.hasItem(kGAMECLASS.consumables.PURHONY, 1)) {
        kGAMECLASS.player.consumeItem(kGAMECLASS.consumables.PURHONY, 1);
...
export function useValaOHYEAHSNAPINTOASLIMJIM(): void {
    ...
    //Herm
        ...
        outputText("Heading back to the chained fairy, you rub the head of the wooden dildo between her petal-shaped labia, turning the cock as you do so, to lubricate the whole 18\" of the monstrous thing. You stroke her juices into the glistening finish until it's difficult to keep hold of and then you place the flared head at the entrance to her rape-worn love box. With exquisite slowness, you press the dildo against her pussy and apply pressure until it begins to part Vala's lips, pushing her slit wider and wider. The fairy finally seems to come to, under your teasing penetration and she coos at the stimulation, without questioning the source. She wiggles her tight little butt and shakes her chest, to set her absurdly large breasts swinging in the air, plump flesh slapping togeather. You encounter resistance just past her lower lips and you roll the flare in circles, cold wood rubbing hot skin and soaking up the squirting girl's natural lubrication. With a wicked grin, you pull it out of her cunt and brace the kGAMECLASS.monster against her asshole instead. Then, putting your hand on the far end of the dildo, you push as hard as you can, jamming it into the fairy's rump.", false);
    }
    //[Next]
    dynStats("lus", 80);
...
export function wakeValaUpBeforeYouGoGo(): void {
    ...
    if (kGAMECLASS.player.gender == 1) {
        ...
        outputText("The fairy's grinding and the sweet scent leaking out of her honey pot is as tempting as ever. You push the girl back gently and nod your head, stripping your " + kGAMECLASS.player.armorName + " piece by piece, teasing the fairy with your slowness. She curls her lips into a wanton smile. \"<i>Now fuck Vala!</i>\" she begs. You chuckle and show your satisfaction at the small restoration of her mind by grabbing one of her supple tits and leaning down to suck out some of her chest-bloating milk. She arches her back under your touch and clenches her muscles. Her hands find your genitals, eager fingers sliding across your sensitive " + kGAMECLASS.player.skinDesc + ". Her hand grabs your " + cockDescript(0) + ", thumb and pinkie forming a tight circle at your base while her other fingers stroke up and down your shaft. The fairy's touch is surprisingly light for the rough treatment she's endured, and you're quickly brought to hardness under her caresses.\n\n", false);
...
export function wakeMeUpBeforeValaGoGosPtII(): void {
    ...
    //h2
        ...
        //(2 Dicks- 
        if (kGAMECLASS.player.cockTotal() > 1) outputText("The " + cockDescript(1) + " inside the fairy girl's ass erupts first, pumping your seed into her bowels, her hips distorting with the bursting flow of your cum, her sphincters clenching powerless to stop your alabaster flood.  ", false);
        ...
        outputText("When your orgasm subsides, you finally manage to switch off the imp pump in your " + vaginaDescript(0) + " and pull it out, tenderly. Your abdomen has been filled enough to leave a paunch of infernal cream below your belly. You press down on it, demonic ichor spilling out of your swollen vagina and splashing between your " + hipDescript() + ". You sigh and hope the seed doesn't take hold. Speaking of taking hold, you switch off the gushing kGAMECLASS.monster pump and carefully pull your " + kGAMECLASS.player.multiCockDescriptLight() + " out of the fairy, making sure the metal dildo remains fixed in place to hold the cum inside. You slide out from between her legs and rise unsteadily to look at your handiwork. The fairy has been filled, top to bottom, all three gallons you sprayed in her bowl gone down her thirsty gullet. The jizz you packed into her womb bloats her even further, the girl's belly nearly four times larger than it was when you found her. She wriggles on the ground, lost to the cummy heaven her mindless body has been plunged into. You're not sure what kind of fucked up child that little exchange would give her, but if anybody had the vagina to pass a kGAMECLASS.monster, it'd be Vala. You probably won't get to find out, though. Who knows what kind of incubation period the chimera-kid will have?", false);
    }
    //[End Encounter]
    kGAMECLASS.player.orgasm();
    dynStats("cor", 1);
...
export function isValaAtBar(): Boolean { return (kGAMECLASS.monster[kFLAGS.FREED_VALA] != 0 && kGAMECLASS.model.time.hours >= 12 && kGAMECLASS.model.time.hours <= 21); }

//Purified Fairy
export function purifiedFaerieBitchBar(): Boolean {
    if (!isValaAtBar()) return false;
    if (kGAMECLASS.monster[kFLAGS.FUCKED_VALA_AT_BAR_TODAY] > 0) {
...
export function chooseValaInBar(): void {
    ...
    if (kGAMECLASS.player.hasCock()) cumBath = valaCumBath;
    clearOutput();
    menu();
    //(First meeting)
    if (kGAMECLASS.monster[kFLAGS.ENCOUNTERED_VALA_AT_BAR] == 0) {
        kGAMECLASS.monster[kFLAGS.ENCOUNTERED_VALA_AT_BAR]++;
    ...
    else if (kGAMECLASS.monster[kFLAGS.SHOULDRA_MET_VALA] == 1) {
        kGAMECLASS.monster[kFLAGS.SHOULDRA_MET_VALA] = 2;
    ...
    else if (kGAMECLASS.model.time.hours == 20 && rand(2) == 0 && kGAMECLASS.player.hasCock()) {
        kinathisValaStuff();
        return;
    }
    else {
        ...
        if (kGAMECLASS.monster[kFLAGS.WEEKLY_FAIRY_ORGY_COUNTDOWN] == 0) {
            ...
            //[Fairies][You][Leave]
            simpleChoices("Faeries", faerieOrgyFuck, "You", cleansedValaRepeatBrainFucking, "Cum Bath", cumBath, "", null, "Leave", kGAMECLASS.telAdre.barTelAdre);
        }
        else simpleChoices("You", cleansedValaRepeatBrainFucking, "", null, "Cum Bath", cumBath, "", null, "Leave", kGAMECLASS.telAdre.barTelAdre);
        if (kGAMECLASS.monster[kFLAGS.WEEKLY_FAIRY_ORGY_COUNTDOWN] == 0)
            addButton(1, "Faeries", faerieOrgyFuck);
    }
    if (kGAMECLASS.monster[kFLAGS.SHOULDRA_MET_VALA] > 0 && kGAMECLASS.shouldraFollower.followerShouldra())
...
export function cleansedValaFuckHerBrainsOut(): void {
    ...
    if (kGAMECLASS.player.totalCocks() > 0) {
        ...
        if (kGAMECLASS.player.totalCocks() > 1) outputText("s", false);
    ...
    //[Female Growth scene]
        ...
        //[Lactating- 
        if (kGAMECLASS.player.biggestLactation() >= 1) outputText("  The pressure is enough to draw your milk to the surface, tiny jets splashing between the two of you as the fairy milks you with two incredibly agile fingers.", false);
        outputText("\n\n", false);

        //[Short characters- 3'- 4'11"] 
        if (kGAMECLASS.player.tallness <= 59) {
        ...
        //[Tall characters- 6'3"+]
            ...
            if (kGAMECLASS.player.biggestLactation() < 1) {
                ...
                kGAMECLASS.player.boostLactation(3);
            }
    ...
    kGAMECLASS.player.orgasm();
    kGAMECLASS.monster[kFLAGS.VALA_TIMES_CONSENSUAL_SEX]++;
}
...
export function cleansedValaFuckHerBrainsOutPtII(): void {
    ...
    if (kGAMECLASS.player.totalCocks() > 0) {
        ...
        if (kGAMECLASS.player.totalCocks() > 0) outputText("s", false);
        ...
        //[All characters]
        outputText("Vala's irrepressible energy is restored in short order, and she re-dresses swiftly, barely noticing the tiny milk spots staining her green dress or the thin trail of cum leaking between her legs as she flutters a foot off the ground. She gives you a kiss on the cheek and winks affectionately. \"<i>Visit me any time, okay " + kGAMECLASS.player.short + "? For a free drink or... anything else,</i>\" she winks. She bundles her hair back up into a sensible bun and flies out of the stockroom, ignoring the curious glances that follow her as she retrieves her plain apron. You sigh appreciatively and retrieve your own clothes only to find that you've grown! Whatever growth drug the imps gave to Vala must not be entirely out of her system, because her milk seems to have added an extra inch to your frame.", false);
        if (kGAMECLASS.player.tallness >= 120) {
            outputText("\n\nOr was that just your imagination?  On closer examination you're not any bigger, but you were huge to begin with!", false);
            kGAMECLASS.player.tallness--;
        }
        //[Player grows 1", lust drops to 0, corruption drops by 1]
        kGAMECLASS.player.tallness++;
        if (kGAMECLASS.player.cor > 40) dynStats("cor", -.3);
    }
    //Part 2 - girls
    else if (kGAMECLASS.player.hasVagina()) {
        //[All characters]
        outputText("Vala's irrepressible energy is restored in short order, and she redresses swiftly barely noticing the tiny milk spots staining her green dress or the thin trail of lubrication leaking between her legs as she flutters a foot off the ground. She gives you a kiss on the cheek and winks affectionately. \"<i>Visit me anytime, okay " + kGAMECLASS.player.short + "? For a free drink or... anything else,</i>\" she winks. She bundles her hair back up into a sensible bun and flies out of the stockroom, ignoring the curious glances that follow her as she retrieves her plain apron. You sigh appreciatively and retrieve your own clothes only to find that you've grown! Whatever growth drug the imps gave to Vala must not be entirely out of her system, because her milk seems to have added an extra inch to your frame.", false);
        if (kGAMECLASS.player.tallness >= 120) {
            outputText("\n\nOr was that just your imagination?  On closer examination you're not any bigger, but you were huge to begin with!", false);
            kGAMECLASS.player.tallness--;
        }
        ...
        if (kGAMECLASS.player.cor > 40) dynStats("cor", -.5);
    }
    //[End Encounter]
    cheatTime(1);
...
export function cleansedValaRepeatBrainFucking(): void {
    spriteSelect(60);
    outputText("", true);
    outputText("She flashes a beautiful smile and tells the bartender that she'll be taking her break early. Grabbing you by the hand, she rushes to the stockroom and sheds her dress without any trace of her former shame about her tattooed back. When you're too slow taking off your " + kGAMECLASS.player.armorName + ", she helps, deft hands made all the quicker by eagerness. When the two of you are naked, she pushes you onto the well-worn stool and sits in your lap, staring into your eyes with a small smile.", false);
...
export function faerieOrgyFuck(): void {
    spriteSelect(60);
    outputText("", true);
    outputText("Vala giggles and nods to the bartender to indicate she'll be taking her break. Taking you by the hand, she flutters upstairs to one of the far rooms of the tavern. When she opens the door, you're startled to find the room is virtually filled with fairies. Unlike Vala, they're the type you're more used to seeing- three to four inches tall, the girls are clothed in shiny black leather straps and wear tiny sable stockings tipped with crystalline heels. There's quite a variety of them and dozens of eyes settle on you as Vala pulls you into the room, closing the door behind you. \"<i>These are my sisters, from the deep forest,</i>\" she introduces. \"<i>In my state, a kGAMECLASS.monster or other predator of the woods would surely catch me or, worse, use me to lure my sisters to danger. But with a few complimentary bottles of whiskey, the captain of Tel'Adre's city guard was willing to give them passage to visit me in the city once a week, as long as they stay out of mischief.</i>\" The fairies lift off and buzz around you, teeny voices introducing themselves, asking your name, marveling at your huge muscles, or otherwise fawning over you. It's a bit much, truth be told.\n\n", false);

    outputText("You thank Vala for introducing you, but you've really got to be going, you claim. Demons to defeat, maidens to rescue, all that. The large fairy chuckles and gives you a squeeze. \"<i>You're so cute when you're flustered. Don't worry, we're not going to all jump you at once- you'd probably end up like I did! No, I asked my sisters here to help me with a little forest magic. Don't you want to see how fairies masturbate?</i>\" You're a little taken aback by the question, but you nod all the same. \"<i>All right girls, the petals please.</i>\" The cloud of fairies in front of you disperses, each winged vixen scattering to different corners of the room to retrieve hidden flower petals. Each blossom, you note, matches the hair color of the fairy holding it, creating a dizzying array of  hues as they form circles around the two of you. Vala guides you to the bed and gently removes your " + kGAMECLASS.player.armorName + " before instructing you to lie down.\n\n", false);
    kGAMECLASS.monster[kFLAGS.WEEKLY_FAIRY_ORGY_COUNTDOWN] = 6;
    kGAMECLASS.monster[kFLAGS.VALA_TIMES_CONSENSUAL_SEX]++;
    //[Herm]
    if (kGAMECLASS.player.gender == 3) {
    ...
    else if (kGAMECLASS.player.gender == 2) doNext(faerieOrgyFuckFemaleContinue);
    else if (kGAMECLASS.player.gender == 1) doNext(faerieOrgyFuckMaleContinue);
}
...
export function faerieOrgyFuckMaleContinue(): void {
    spriteSelect(60);
    outputText("", true);
    outputText("The fairy girl lands at the edge of the bed, settling on her knees and resting her weighty chest on the mattress. Leaning in, she spreads your " + kGAMECLASS.player.legs() + " with warm, delicate hands, fingertips stroking the insides of your thighs. You shiver and squeeze the bedspread between your fingers, a little nervous about this 'forest magic.' One of the fairies flutters down to Vala and hands her a pale white lily petal with a tittering giggle. The larger fairy takes the soft white bloom and, whispering a silken word to it, licks the cream-colored surface. Slithering further between your legs, she reaches your " + cockDescript(0) + " and brushes the petal against your throbbing glans with a teasing caress. Then, much to your surprise, she places it on the tip of her forefinger and places it on the base of your cock. You gasp, the petal's soft surface rubbing sensitive skin with a warmth you were unprepared for. After the briefest delay, you lose your breath as the tremor of a small, barely contained orgasm rumbles through your gut. Without hesitation, the fairy barmaid takes a rose petal and repeats the process, another almost-orgasm seizing you and leaving you panting. \"<i>Normally we only need the one petal, but since you and I are so much larger than the average fairy, I had my sisters bring lots. All for you,</i>\" she whispers, coyly.\n\n", false);

    //[If the Player has unlocked Scylla's Addiction Counseling event]
    if (kGAMECLASS.monster[kFLAGS.TIMES_SCYLLA_ADDICT_GROUP_EXPLOITED] + kGAMECLASS.monster[kFLAGS.SCYLLA_TIMES_SHARED_IN_ADDICT_GROUP] + kGAMECLASS.monster[kFLAGS.SCYLLA_MILK_THERAPY_TIMES] + kGAMECLASS.monster[kFLAGS.SCYLLA_CUM_THERAPY_TIMES] + kGAMECLASS.monster[kFLAGS.SCYLLA_SEX_THERAPY_TIMES] > 0) {
        outputText("Another fairy flies toward Vala and you blink through the building lusty haze when you recognize the little girl. Wearing a slightly slutty crystalline gown of translucent gossamer, it seems to be Pastie, the fairy from Scylla's addiction counseling meeting. When you call out to her, she bashfully waves back. \"<i>Hey, uh, " + kGAMECLASS.player.short + ". It's kinda weird seeing you outside of the meetings. How are you? How have things been?</i>\" She looks at your naked, reclined form on the bed and blushes. \"<i>Oh, right, right. Um. I'm just helping out Vala, you know. Sisters and all. Uh. I'm... I'm going to just give her this petal and get something to drink, okay? All this sobriety is making this really uncomfortable for me.</i>\" The other fairies laugh and Pastie hands off her blossom before zipping out of the room, probably to steal someone's drink downstairs.\n\n", false);
    }
    ...
    //[Balls- 
    if (kGAMECLASS.player.balls > 0) outputText("Vala's tongue caresses your " + ballsDescriptLight() + ", slurping hungrily, eager to get every drop out of your overburdened sac.  ", false);
    ...
    if (kGAMECLASS.player.cumQ() > 2000) outputText("  Even after driving the condom off of you, your cock continues its gushing orgasm, spurting thick bubbles of jizz into the air, raining down in creamy torrents. The small fairies gleefully dive after the cum globs, making a game of it, trying to catch the gooey globs before they splatter on the bed or carpet.", false);
    outputText("\n\n", false);
...
export function faerieOrgyFuckFemaleContinue(): void {
    spriteSelect(60);
    outputText("", true);
    outputText("The fairy girl lands at the edge of the bed, settling on her knees and resting her weighty chest on the mattress. Leaning in, she spreads your " + kGAMECLASS.player.legs() + " with warm, delicate hands, fingertips stroking the insides of your thighs. You shiver and hug your arms under your breasts, a little nervous about this 'forest magic.' One of the fairies flutters down to Vala and hands her a pale white lily petal with a tittering giggle. The larger fairy takes the soft white bloom and, whispering a silken word to it, licks the cream-colored surface. Slithering further between your legs, she reaches your " + vaginaDescript(0) + " and brushes the petal against your " + clitDescript() + " with a teasing caress. Then, much to your surprise, she places it on the tip of her forefinger and slides it into your pussy. You gasp, the petal's soft surface rubbing your inner walls with a warmth you were unprepared for. Vala presses lightly on a spot inside your vagina and you lose your breath as the tremor of a small orgasm rumbles through your gut. Without explanation, the fairy barmaid takes a rose petal and repeats the process, another orgasm seizing you and leaving you panting. \"<i>Normally we only need the one petal, but since you and I are so much larger than the average fairy, I had my sisters bring lots. All for you,</i>\" she whispers, coyly.\n\n", false);

    //[If the Player has unlocked Scylla's Addiction Counseling event] 
    if (kGAMECLASS.monster[kFLAGS.TIMES_SCYLLA_ADDICT_GROUP_EXPLOITED] + kGAMECLASS.monster[kFLAGS.SCYLLA_TIMES_SHARED_IN_ADDICT_GROUP] + kGAMECLASS.monster[kFLAGS.SCYLLA_MILK_THERAPY_TIMES] + kGAMECLASS.monster[kFLAGS.SCYLLA_CUM_THERAPY_TIMES] + kGAMECLASS.monster[kFLAGS.SCYLLA_SEX_THERAPY_TIMES] > 0) outputText("Another fairy flies toward Vala and you blink through the building lusty haze when you recognize the little girl. Wearing a slightly slutty crystalline gown of translucent gossamer, you recognize Pastie, the fairy from Scylla's addiction counseling meeting. When you call out to her, she bashfully waves back. \"<i>Hey, uh, " + kGAMECLASS.player.short + ". It's kinda weird seeing you outside of the meetings. How are you? How have things been?</i>\" She looks at your naked, reclined form on the bed and blushes. \"<i>Oh, right, right. Um. I'm just helping out Vala, you know. Sisters and all. Uh. I'm... I'm going to just give her this petal and get something to drink, okay? All this sobriety is making this really uncomfortable for me.</i>\" The other fairies laugh and Pastie hands off her blossom before zipping out of the room, probably to steal someone's drink downstairs.\n\n", false);
...
export function takeBondageStraps(): void {
    clearOutput();
    kGAMECLASS.monster[kFLAGS.ZETAZ_LAIR_TOOK_BONDAGE_STRAPS]++;
    kGAMECLASS.inventory.takeItem(kGAMECLASS.armors.BONSTRP, playerMenu);
}

//ZETAZ START
export function zetazTaunt(): void {
    if (kGAMECLASS.monster.findStatusAffect(StatusAffects.round) < 0) {
        kGAMECLASS.monster.createStatusAffect(StatusAffects.round, 1, 0, 0, 0);
        outputText("Zetaz asks, \"<i>Do you even realize how badly you fucked up my life, ", false);
        if (kGAMECLASS.player.humanScore() >= 4) outputText("human", false);
        else outputText("'human'", false);
        outputText("?  No, of course not.  That's the kind of attitude I'd expect from one of you!</i>\"", false);
    }
    else {
        kGAMECLASS.monster.addStatusValue(StatusAffects.round, 1, 1);
        if (kGAMECLASS.monster.statusAffectv1(StatusAffects.round) == 2) outputText("\"<i>I lost my post!  And when you screwed up the factory?  I barely escaped with my life!  You ruined EVERYTHING!</i>\" screams Zetaz.", false);
        else if (kGAMECLASS.monster.statusAffectv1(StatusAffects.round) == 3) outputText("Zetaz snarls, \"<i>Do you know how hard it is to hide from Lethice?  DO YOU HAVE ANY IDEA!?  I've had to live in this fetid excuse for a jungle, and just when I found some friends and made it livable, you show up and DESTROY EVERYTHING!</i>\"", false);
        else if (kGAMECLASS.monster.statusAffectv1(StatusAffects.round) == 4) outputText("Zetaz explains, \"<i>I won't let you go.  I'm going to break you.</i>\"", false);
        else if (kGAMECLASS.monster.statusAffectv1(StatusAffects.round) == 5) outputText("\"<i>Would it have been that bad to go along with me?  You've seen the factory.  We would've kept you fed, warm, and provided you with limitless pleasure.  You would've tasted heaven and served a greater purpose.  It's not too late.  If you come willingly I can make sure they find a good machine to milk you with,</i>\" offers the imp lord.", false);
        else if (kGAMECLASS.monster.statusAffectv1(StatusAffects.round) == 6) outputText("\"<i>Why won't you fall?</i>\" questions Zetaz incredulously.", false);
        else if (kGAMECLASS.monster.statusAffectv1(StatusAffects.round) == 7) outputText("The imp lord suggests, \"<i>If you give up and let me fuck your ass maybe I'll let you go.</i>\"", false);
        else if (kGAMECLASS.monster.statusAffectv1(StatusAffects.round) == 8) outputText("Zetaz pants, \"<i>Just give up!  I'm nothing like the weakling you met so long ago!  I've been through hell to get here and I'm not giving it up just because you've shown up to destroy my hard work!</i>\"", false);
...
export function zetazAI(): void {
    ...
    //turn.
    if (kGAMECLASS.monster.lust > 50 && (kGAMECLASS.monster.findStatusAffect(StatusAffects.Fear) >= 0 || kGAMECLASS.monster.findStatusAffect(StatusAffects.Blind) >= 0)) {
        kGAMECLASS.monster.removeStatusAffect(StatusAffects.Fear);
        kGAMECLASS.monster.removeStatusAffect(StatusAffects.Blind);
        kGAMECLASS.monster.lust -= 10;
    ...
    if (kGAMECLASS.monster.findStatusAffect(StatusAffects.Stunned) >= 0) {
        outputText("Your foe is too dazed from your last hit to strike back!", false);
        kGAMECLASS.monster.removeStatusAffect(StatusAffects.Stunned);
        combatRoundOver();
        return;
    }
    var select: number = 1;
    ...
    if (kGAMECLASS.player.findStatusAffect(StatusAffects.Exgartuan) >= 0 && kGAMECLASS.player.statusAffectv2(StatusAffects.Exgartuan) == 0 && rand(3) == 0) {
        kGAMECLASS.exgartuan.exgartuanCombatUpdate();
        outputText("\n\n", false);
    }
    if (kGAMECLASS.monster.findStatusAffect(StatusAffects.Constricted) >= 0) {
        //Enemy struggles - 
        outputText("Your prey pushes at your tail, twisting and writhing in an effort to escape from your tail's tight bonds.", false);
        if (kGAMECLASS.monster.statusAffectv1(StatusAffects.Constricted) <= 0) {
            outputText("  " + kGAMECLASS.monster.capitalA + kGAMECLASS.monster.short + " proves to be too much for your tail to handle, breaking free of your tightly bound coils.", false);
            kGAMECLASS.monster.removeStatusAffect(StatusAffects.Constricted);
        }
        kGAMECLASS.monster.addStatusValue(StatusAffects.Constricted, 1, -1);
        combatRoundOver();
        return;
    }
    ...
    if (kGAMECLASS.monster.lust > 50 && kGAMECLASS.monster.HPRatio() <= .5) {
        ...
        kGAMECLASS.monster.addHP(0.25 * kGAMECLASS.monster.eMaxHP());
        kGAMECLASS.monster.lust -= 20;
        dynStats("lus", 2);
    }
    else {
        ...
        if (attackChoice == 0) {
            ...
            if ((kGAMECLASS.player.findPerk(PerkLib.Evade) >= 0 && rand(4) == 0) ||
                (kGAMECLASS.player.findPerk(PerkLib.Flexibility) >= 0 && rand(6) == 0) ||
                (kGAMECLASS.player.spe > 65 && rand(10) == 0) ||
                (kGAMECLASS.player.findPerk(PerkLib.Misdirection) >= 0 && rand(100) < 20 && kGAMECLASS.player.armorName == "red, high-society bodysuit")) {
                outputText("You sidestep it a moment before it shatters on the wall, soaking the tapestries with red fluid!", false);
            }
            else {
                outputText("You try to avoid it, but the fragile glass shatters against you, coating you in sticky red liquid.  It seeps into your " + kGAMECLASS.player.skinDesc + " and leaves a pleasant, residual tingle in its wake.  Oh no...", false);
                //[Applies: "Temporary Heat" status]
                if (kGAMECLASS.player.findStatusAffect(StatusAffects.TemporaryHeat) < 0) kGAMECLASS.player.createStatusAffect(StatusAffects.TemporaryHeat, 0, 0, 0, 0);
            }
        }
            ...
            if (kGAMECLASS.player.tou > 60) outputText("causing little damage", false);
            else {
            ...
            kGAMECLASS.player.createStatusAffect(StatusAffects.Blind, 1, 0, 0, 0);
        }
...
export function gigaArouse(): void {
    outputText("You see " + kGAMECLASS.monster.a + kGAMECLASS.monster.short + " make familiar arcane gestures at you, but his motions seem a lot more over the top than you'd expect from an imp.\n\n", false);
    dynStats("lus", rand(kGAMECLASS.player.lib / 10) + kGAMECLASS.player.cor / 10 + 15);
    if (kGAMECLASS.player.lust < 30) outputText("Your nethers pulse with pleasant warmth that brings to mind pleasant sexual memories.  ", false);
    if (kGAMECLASS.player.lust >= 30 && kGAMECLASS.player.lust < 60) outputText("Blood rushes to your groin in a rush as your body is hit by a tidal-wave of arousal.  ", false);
    if (kGAMECLASS.player.lust >= 60) outputText("Your mouth begins to drool as you close your eyes and imagine yourself sucking off Zetaz, then riding him, letting him sate his desires in your inviting flesh.  The unnatural visions send pulses of lust through you so strongly that your body shivers.  ", false);
    if (kGAMECLASS.player.cocks.length > 0) {
        if (kGAMECLASS.player.lust >= 60 && kGAMECLASS.player.cocks.length > 0) outputText("You feel " + kGAMECLASS.player.sMultiCockDesc() + " dribble pre-cum, bouncing with each beat of your heart and aching to be touched.  ", false);
        if (kGAMECLASS.player.lust >= 30 && kGAMECLASS.player.lust < 60 && kGAMECLASS.player.cocks.length == 1) outputText(kGAMECLASS.player.SMultiCockDesc() + " hardens and twitches, distracting you further.  ", false);
    }
    if (kGAMECLASS.player.vaginas.length > 0) {
        if (kGAMECLASS.player.lust >= 60 && kGAMECLASS.player.vaginas[0].vaginalWetness == VAGINA_WETNESS_NORMAL && kGAMECLASS.player.vaginas.length == 1) outputText("Your " + allVaginaDescript() + " dampens perceptibly, feeling very empty.  ", false);
        if (kGAMECLASS.player.lust >= 60 && kGAMECLASS.player.vaginas[0].vaginalWetness == VAGINA_WETNESS_WET && kGAMECLASS.player.vaginas.length > 0) outputText("Your crotch becomes sticky with girl-lust, making it clear to " + kGAMECLASS.monster.a + kGAMECLASS.monster.short + " just how welcome your body finds the spell.  ", false);
        if (kGAMECLASS.player.lust >= 60 && kGAMECLASS.player.vaginas[0].vaginalWetness == VAGINA_WETNESS_SLICK && kGAMECLASS.player.vaginas.length == 1) outputText("Your " + allVaginaDescript() + " becomes sloppy and wet, dribbling with desire to be mounted and fucked.  ", false);
        if (kGAMECLASS.player.lust >= 60 && kGAMECLASS.player.vaginas[0].vaginalWetness == VAGINA_WETNESS_DROOLING && kGAMECLASS.player.vaginas.length > 0) outputText("Thick runners of girl-lube stream down the insides of your thighs as your crotch gives into the demonic magics.  You wonder what " + kGAMECLASS.monster.a + kGAMECLASS.monster.short + "'s cock would feel like inside you?  ", false);
        if (kGAMECLASS.player.lust >= 60 && kGAMECLASS.player.vaginas[0].vaginalWetness == VAGINA_WETNESS_SLAVERING && kGAMECLASS.player.vaginas.length == 1) outputText("Your " + allVaginaDescript() + " instantly soaks your groin with the heady proof of your need.  You wonder just how slippery you could " + kGAMECLASS.monster.a + kGAMECLASS.monster.short + "'s dick when it's rammed inside you?  ", false);
    }
    if (kGAMECLASS.player.lust >= 100) doNext(endLustLoss)
    else doNext(playerMenu);
}


export function defeatZetaz(): void {
    kGAMECLASS.monster[kFLAGS.DEFEATED_ZETAZ]++;
    outputText("", true);
    //[VICTORY HP]
    if (kGAMECLASS.monster.HP < 1) outputText("Zetaz sinks down on his knees, too wounded to continue.  He looks up at you with helpless rage in his eyes and asks, \"<i>Are you satisfied now?  Go ahead then, kill me.  My life hasn't been worth living since I met you anyway.</i>\"\n\n", false);
    //[VICTORY LUST] 
    ...
    //( or take her place)
    if (kGAMECLASS.player.cor > 66) outputText(" or take her place", false);
...
export function releaseZForInfo(): void {
    ...
    if (kGAMECLASS.monster.HP < 1) outputText("staggers", false);
    ...
    else if (kGAMECLASS.player.cor > 33) {
    ...
    kGAMECLASS.player.createKeyItem("Zetaz's Map", 0, 0, 0, 0);
    cleanupAfterCombat();
}
...
export function sexualInterrogation(): void {
    ...
    outputText("\"<i>Piss off!  You won't get shit from me,</i>\" retorts the defeated demon, \"<i>You may as well finish me off – I'll NEVER help a " + kGAMECLASS.player.mf("jackass", "bitch") + " like you!</i>\"\n\n", false);
    ...
    outputText("The imp looks up with his face shrouded in confusion as he asks, \"<i>I don't think a string is going to help you much, " + kGAMECLASS.player.mf("dork", "skank") + ".</i>\"\n\n", false);
...
function sexualTortureReleaseZetaz(): void {
    ...
    if (kGAMECLASS.player.lust > 60 || kGAMECLASS.player.lib > 60 || kGAMECLASS.player.cor > 60) outputText(", or perhaps perversion,", false);
...
function endZetaz(): void {
    ...
    kGAMECLASS.player.createKeyItem("Zetaz's Map", 0, 0, 0, 0);
    //(ZETAZ IS DEAD)
    kGAMECLASS.monster[kFLAGS.ZETAZ_DEFEATED_AND_KILLED]++;
    cleanupAfterCombat();
}
...
export function loseToZetaz(): void {
    ...
    if (kGAMECLASS.player.lust > 99) outputText("masturbating", false);
    else outputText("prone", false);
    outputText(" form with an amused expression, \"<i>The first champion in ages to retain " + kGAMECLASS.player.mf("his", "her") + " free will for more than a few minutes, and " + kGAMECLASS.player.mf("he", "she") + "'s brought to " + kGAMECLASS.player.mf("his", "her") + " knees by the very imp " + kGAMECLASS.player.mf("he", "she") + " escaped!  Once you've learned your proper place, you'll guarantee my safe return to my rightful station.  Perhaps I'll even get a promotion?  After all, you've defeated so many higher ranking demons already.</i>\"\n\n", false);
    ...
    if (kGAMECLASS.player.gender == 1) malesZetazOver();
    if (kGAMECLASS.player.gender == 2) femaleZetazOver();
    if (kGAMECLASS.player.gender == 3) hermZetazOver();
}
...
export function femaleZetazOver(): void {
    ...
    if (kGAMECLASS.player.HP < 1) outputText("beaten out of you", false);
    ...
    if (kGAMECLASS.player.cor < 33) outputText("shame", false);
    else if (kGAMECLASS.player.cor < 66) outputText("confuse", false);
    else outputText("arouse", false);
    outputText(" you as you're thrown atop one of the tables.   You grunt as leather straps are produced and laid over your form to restrain you.  In the span of a minute you're completely immobilized from the neck down, and your " + kGAMECLASS.player.legs() + " are kept spread to allow easy access to your " + vaginaDescript(0) + ".\n\n", false);
    ...
    if (kGAMECLASS.player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_DROOLING) outputText("even more wet ", false);
    else outputText("a little wet ", false);
    outputText("from the masculine scent the aroused penises are producing.  ", false);
    if (kGAMECLASS.player.cor < 33) outputText("How could you be turned on by such a repulsive situation?  You're going to be raped, brainwashed, and either kept as a pet or tossed in a milking tube for the rest of your life and your body is acting like some horny slut!", false);
    else if (kGAMECLASS.player.cor < 66) outputText("You marvel at just how turned on you're getting from the strange situation.  You know you'll be raped, drugged, and used as a toy or milk cow, but your loins are thrumming with warm, wet desire.", false);
    ...
    outputText("Your lips start to tingle, and you run your tongue over them reflexively.  A shiver of pleasure worms through your body, and you instinctively press your " + kGAMECLASS.player.legs() + " against the straps in an effort to spread them further.  Worse yet, your lips feel much plumper and fuller than a few moments ago.  ", false);
    if (kGAMECLASS.monster[kFLAGS.NUMBER_OF_TIMES_MET_SCYLLA] > 0) outputText("Unbidden, Scylla's face comes to mind, and you realize the drugs coursing through your veins must be doing something similar to you!  Her visage changes to your own, though the thick, cock-sucking lips remain behind, eager to be penetrated.", false);
    ...
    if (kGAMECLASS.player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_DROOLING) outputText("gushes fresh fluids into a puddle on the table", false);
    else if (kGAMECLASS.player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_WET) outputText("drools a heavy flow of liquid arousal onto the hardwood table", false);
    else outputText("begins to dribble a steady flow of liquid on to the table's girl-slicked boards", false);
    outputText(".  ", false);
    if (kGAMECLASS.player.inHeat) outputText("D", false);
...
export function femaleZetazOverPtII(): void {
    ...
    if (kGAMECLASS.player.biggestTitSize() < 1) outputText("tweak your " + nippleDescript(0) + "s roughly, pulling and yanking on them as", false);
    else outputText("maul at your " + allBreastsDescript() + ", slapping and squeezing them as", false);
    outputText(" he begins to repeatedly thrust against your " + vaginaDescript(0) + ", fucking you in earnest.  The wet slap of his balls on your juice-slimed body fills the chamber and sends ripples of pleasure down your " + kGAMECLASS.player.legs() + ".  With your eyelids half-closed, your tongue masturbating your lips, and your pussy practically squirting lubricants at the end of each thrust, you must look like every male's wet dream.\n\n", false);
...
export function zetazBadEndEpilogue(): void {
    ...
    if (kGAMECLASS.player.gender == 2) {
        ...
        if (kGAMECLASS.player.wingType != WING_TYPE_BAT_LIKE_TINY || kGAMECLASS.player.wingType != WING_TYPE_BAT_LIKE_LARGE) outputText("Zetaz gave her one of the weaker imps to penetrate and taught her to fly with her new, demonic wings.  ", false);
        ...
        if (kGAMECLASS.player.biggestTitSize() < 1) outputText("created a decent set of chest-bumps.", false);
...
export function hermZetazOver(): void {
    ...
    if (kGAMECLASS.player.cockTotal() == 1) outputText("both", false);
    ...
    if (kGAMECLASS.player.vaginas[0].vaginalWetness < VAGINA_WETNESS_DROOLING) outputText("sticky", false);
    else outputText("soaked", false);
    outputText(" from the thoughts.  ", false);
    if (kGAMECLASS.player.cor < 33) outputText("Are you really being turned on by such lewd, debased thoughts?", false);
    else if (kGAMECLASS.player.cor < 66) outputText("Are you this much of a pervert?  Yes, it'll feel good, but you're a little ashamed of your body's immediate and lewd reaction.", false);
    ...
    if (kGAMECLASS.player.cor < 33) outputText("shame", false);
    else if (kGAMECLASS.player.cor < 66) outputText("confusion", false);
    ...
    outputText("Zetaz is just getting started.  Before you have time to react to your predicament, the next bottle is empty and thicker cream is flooding your mouth.  You don't swallow for a moment, so the imp pours another bottle in, backing up more of the fluid.  Faced with a choice between corruption and drowning, you try to gulp down enough liquid to breathe.  " + kGAMECLASS.player.SMultiCockDesc() + " puffs and swells, spurting thick ropes of cum as it adds a half-dozen inches to its length.  Your eyes cross from the sudden change, but you get a fresh breath before the imps begin to pour several bottles in at once.\n\n", false);

    outputText("You swallow in loud, greedy gulps as your body is slowly warped by the fluids you're consuming.  Though your " + allBreastsDescript() + " and " + kGAMECLASS.player.multiCockDescriptLight() + " sometimes shrink, they grow far more often, and after a few minutes of force-feeding, you're pleading for more each time they stop to let you breath.  You're a mess of sexual fluids, your tits are squirting milk, and your pussy squirts from every touch.  Demon horns are swelling from your brow, curling back over your ears", false);
    if (kGAMECLASS.player.horns > 0) outputText(" and adding to your existing pair", false);
    else outputText(" and giving you an exotic, tainted appearance", false);
    outputText(".  ", false);
    if (kGAMECLASS.player.lowerBody != LOWER_BODY_TYPE_DEMONIC_HIGH_HEELS) outputText("Your " + kGAMECLASS.player.feet() + " have been changing throughout the ordeal, but you didn't notice your " + kGAMECLASS.player.legs() + " becoming such lissom, lengthy legs, or your heels growing long, high-heel-like spikes.  ", false);
    if (kGAMECLASS.player.tailType != TAIL_TYPE_DEMONIC) outputText("A tail snakes around your leg and begins to caress your " + vaginaDescript(0) + ", then plunges in to fuck the squirting orifice while you drink.  ", false);
...
export function hermZetazOverPtII(): void {
    hideUpDown();
    outputText("", true);
    outputText("You awaken midway through a loud moan and nearly jump out of your " + kGAMECLASS.player.skinDesc + " in surprise, but the fire of your unnaturally stoked libido immediately reasserts yourself.  You twitch your hips to and fro, thrusting against a ", false);
    if (kGAMECLASS.player.cockTotal() > 1) outputText("number of ", false);
    outputText("mechanical milking device", false);
    if (kGAMECLASS.player.cockTotal() > 1) outputText("s", false);
    outputText(".  " + kGAMECLASS.player.SMultiCockDesc() + " is sucked rhythmically, producing a loud, wet, slurping noise that echoes around the small room.  You're suspended from a set of shackles on the wall, next to Vala.  The sexy faerie is chained up in a similar manner, but she's locked in coitus with a well-endowed imp, and making no secret of her enjoyment.  The sexual sight stirs your well-stimulated loins and you groan, filling the milker with what feels like gallons of male cream over the next minute and a half.\n\n", false);

    outputText("Slumping forwards, you hang there, but the corruption and lust in your blood refuses to be sated.  " + kGAMECLASS.player.SMultiCockDesc() + " is already hard again, and after sucking your cum down some tubes, the milker begins its oh-so-pleasurable work again.  Still, you estimate it will be a few minutes before it gets you off again, so you look around the room.  A platform is set up in front of you, about knee-height and poorly built.  Judging by its height, it's probably there so that the imps can use it to fuck you without having to fly.  There's also a pair of platforms built into the walls next to each of your shoulders, though their function is less clear.\n\n", false);
    ...
    if (kGAMECLASS.player.balls > 0) outputText("gently shifts your " + ballsDescript() + " out of the way and ", false);
...
export function malesZetazOver(): void {
    ...
    if (kGAMECLASS.player.HP < 1) outputText("beaten", false);
    ...
    outputText(" you as you're thrown atop one of the tables and rolled onto your side.   You grunt as leather straps are produced and laid over your form to restrain you.  In the span of a minute you're completely immobilized from the neck down, and your " + kGAMECLASS.player.legs() + " are kept spread to allow easy access to " + kGAMECLASS.player.sMultiCockDesc() + " and " + assholeDescript() + ".\n\n", false);
    ...
    if (kGAMECLASS.player.ass.analLooseness < 4) outputText("worried he'll try to force the mammoth between his thighs into your backdoor", false);
    ...
    outputText("It has a bubblegum-like taste that makes your tongue tingle as it passes into your belly, but the more pressing sensation of " + kGAMECLASS.player.sMultiCockDesc() + " getting rock-hard lets you know exactly what you just drank.  Even though you just finished chugging down that foul drink, the imps uncork another pair of potions and dump them into the funnel.  The sweet fluids flood your mouth, and once again you swallow and chug rather than drown.   After you finish the last swallow, you pant, completely out of breath and getting hotter by the moment.  Your " + kGAMECLASS.player.skinDesc + " tingles and sweats, growing more and more sensitive with every passing second while " + kGAMECLASS.player.sMultiCockDesc() + " begins to drip and drool.\n", false);

    outputText("Zetaz hands the funnel to an underling with a knowing laugh and repositions himself over your " + kGAMECLASS.player.legs() + ".  Warm pressure pushes at your " + assholeDescript() + ", forcing your clenching flesh to yield around the intruder.  Normally such an instant penetration would be irritating, or perhaps painful, but the sudden pressure on your prostate only serves to release a copious squirt of pre-cum.  An unwelcome moan slips past your lips and sends a titter of laughter through the mob.  As if losing wasn't bad enough – they all know you're getting off on having your " + assDescript() + " penetrated.  The worst part is that the humiliation is just making the situation hotter and " + kGAMECLASS.player.sMultiCockDesc() + " harder.\n\n", false);

    outputText("You nearly choke as an unexpected wave of potions washes through the funnel into your mouth, but you start swallowing and gulp down what feels like a half-dozen lust potions before you can breathe again.  " + kGAMECLASS.player.SMultiCockDesc() + " starts squirting and spurting, dumping heavy loads of cum onto the table and your belly from the effects of the potions alone.  Zetaz gathers a massive dollop in his hand and smears it over himself, using it as lubricant to penetrate your poor, beleaguered asshole with savage, rough strokes that smash against your prostate at the apex of each thrust.  You moan loudly and lewdly through the tube in your mouth, wriggling against your restraints and spurting helplessly as you're penetrated over and over.\n\n", false);
...
export function malesZetazOverPtII(): void {
    ...
    outputText("While you observe the room, you realize that the waves of pleasure sliding up your spinal cord haven't stopped, and that your entire body is being shaken rhythmically.  You look down with a look of incredible, still-drugged confusion and behold the last thing you expected to see.  Somehow " + kGAMECLASS.player.sMultiCockDesc() + " has been shrunk to less than half of its previous size", false);
    if (kGAMECLASS.player.balls > 0) outputText(", and your balls have completely vanished", false);
    ...
    if (kGAMECLASS.player.cockTotal() == 1) outputText("cock's", false);
    else outputText("cocks're", false);
    outputText(" gonna be so tiny ", false);
    if (kGAMECLASS.player.cockTotal() == 1) outputText("it", false);
    ...
    if (kGAMECLASS.player.biggestTitSize() < 2) outputText("It doesn't look right without a decent rack, but boss said no tits for the new breeding bitch.  Sure makes it hard to get excited about fucking that new twat of yours though...", false);
...
function incubusDeal(): void {
    ...
    //[Next – to room]
    kGAMECLASS.monster[kFLAGS.ZETAZ_LAIR_DEMON_VENDOR_PRESENT] = 1;
    doNext(playerMenu);
}
...
export function incubusShop(): void {
    spriteSelect(52);
    if (kGAMECLASS.monster[kFLAGS.NIAMH_SEAN_BREW_BIMBO_LIQUEUR_COUNTER] == 1) {
    ...
    menu();
    addButton(0, kGAMECLASS.consumables.NUMBROX.shortName, incubusBuy, kGAMECLASS.consumables.NUMBROX);
    addButton(1, kGAMECLASS.consumables.SENSDRF.shortName, incubusBuy, kGAMECLASS.consumables.SENSDRF);
    addButton(2, kGAMECLASS.consumables.REDUCTO.shortName, incubusBuy, kGAMECLASS.consumables.REDUCTO);
    addButton(3, kGAMECLASS.weapons.SUCWHIP.shortName, incubusBuy, kGAMECLASS.weapons.SUCWHIP);
    if (kGAMECLASS.player.hasItem(kGAMECLASS.consumables.BIMBOCH) && kGAMECLASS.monster[kFLAGS.NIAMH_SEAN_BREW_BIMBO_LIQUEUR_COUNTER] == 0) {
        outputText("\n\nSean could probably do something with the Bimbo Champagne if you had enough of it...");
        if (kGAMECLASS.player.hasItem(kGAMECLASS.consumables.BIMBOCH, 5)) {
            outputText("  Luckily, you do!");
            addButton(4, kGAMECLASS.consumables.BIMBOLQ.shortName, kGAMECLASS.telAdre.niamh.seanBimboBrewing);
        }
    }
...
function incubusBuy(itype: ItemType): void {
    ...
    if (kGAMECLASS.player.gems < (itype.value * 3)) {
...
function incubusTransact(itype: ItemType): void {
    spriteSelect(52);
    clearOutput();
    kGAMECLASS.player.gems -= itype.value * 3;
    statScreenRefresh();
    kGAMECLASS.inventory.takeItem(itype, incubusShop);
}
...
export function valaCumBath(): void {
    ...
    clearOutput();
    //FIRST TIME INTRO
    if (kGAMECLASS.monster[kFLAGS.VALA_CUMBATH_TIMES] == 0) {
        ...
        //[If the kGAMECLASS.player's cum production is under X] 
        if (kGAMECLASS.player.cumQ() < 2000) {
            ...
            //IF BALLS: 
            if (kGAMECLASS.player.balls > 0) outputText("  Your balls react immediately, filling with a cold weight that nearly knocks you to your ass.  Your [armor] bulges obscenely as the sack hanging between your legs fills with liquid pressure that squeezes your bloating flesh.");
        ...
        //OR [If the kGAMECLASS.player's cum production is over X]
        else {
            outputText("[pg]In response, you cup the pale girl's chin and flash a sly smile of your own.  Arching a " + kGAMECLASS.player.hairColor + " eyebrow, you assure her that won't be a problem.  Vala's hands sink down to your crotch, fey fingers pushing aside your [armor] to encircle your swelling member.  \"<i>I don't doubt it,</i>\" she purrs, lightly tracing the crest of your [cock]'s head.");
            //[IF BALLS: 
            if (kGAMECLASS.player.balls > 0) outputText("  The fairy's hands slide a bit further and she turns up her palms to cup your [balls], her thin fingers delicately massaging the heft of your over-burdened sac.  \"<i>I don't know who's more excited, me or these guys,</i>\" she teases, drawing you into another passionate embrace.  Grasping your testicles just tightly enough to send a shiver of pleasure across the back of your neck, she wets her lips and whispers, \"<i>Who am I kidding?  No one wants this more than me.</i>\"");
        ...
        //[Low Cum Production: 
        if (kGAMECLASS.player.cumQ() < 2000) outputText("Retrieving a vial of glistening oil, she offers the concoction to you with a laugh.  \"<i>The gal who makes these is so silly.  She always giggles when I pay her a visit and blushes when I call her 'bunny girl'.  She's so cute I just want to give her a squeeze!</i>\" You chuckle and down the draft, a thrill of cold weight filling your gut.  ");
...
export function valaCumBath2(): void {
    ...
    if (kGAMECLASS.player.cumQ() < 2000) {
        kGAMECLASS.player.cumMultiplier += 6;
        if (kGAMECLASS.player.balls > 0) kGAMECLASS.player.ballSize += .5;
    }
    kGAMECLASS.player.cumMultiplier += 2;
    if (kGAMECLASS.player.balls > 0) kGAMECLASS.player.ballSize += .3;
    kGAMECLASS.monster[kFLAGS.VALA_CUMBATH_TIMES]++;
    kGAMECLASS.monster[kFLAGS.VALA_TIMES_CONSENSUAL_SEX]++;
    kGAMECLASS.player.orgasm();
    dynStats("lib", -1.5);
    doNext(kGAMECLASS.camp.returnToCampUseOneHour);
...
export function valaBigYou(): void {
    clearOutput();
    kGAMECLASS.monster[kFLAGS.TIMES_VALA_CONSENSUAL_BIG]++;
    kGAMECLASS.monster[kFLAGS.VALA_TIMES_CONSENSUAL_SEX]++;
    //{FIRST TIME}
    if (kGAMECLASS.monster[kFLAGS.TIMES_VALA_CONSENSUAL_BIG] == 1) {
    ...
    if (kGAMECLASS.player.hasCock()) addButton(1, "Lick Me", bigValaLicksOffDudes);
    if (kGAMECLASS.vapula.vapulaSlave() && kGAMECLASS.player.gender > 0 && (kGAMECLASS.player.hasCock() || (kGAMECLASS.player.hasVagina() && kGAMECLASS.player.hasKeyItem("Demonic Strap-On") >= 0))) {
...
export function bigValaDomsPC(): void {
    ...
    if (kGAMECLASS.monster[kFLAGS.VALA_HEALED_HONEY] == 0) outputText("\"<i>If you do a good job, I'll let you taste a pure pearl of my own,</i>\" she giggles.");

    //{Male:}
    if (kGAMECLASS.player.gender == 1) {
        outputText("\n\nShuddering from the onslaught of female pheromones, you find your body responding without meaning to.  [EachCock]");
        if (kGAMECLASS.player.lust >= 70) outputText(", already hard, begins to bead with pre-cum, mixing with the slut-leakings that have found their way under your equipment.");
    ...
    //{Female:}
        ...
        if (kGAMECLASS.player.wetness() >= 3) outputText("Your [vagina] seems to always be wet, but now, with this sight before you, your girl-cum is running in rivers down your [legs] to mix into the puddling lady-spunk below.  ");
    ...
    //{Herm:}
        ...
        if (kGAMECLASS.player.lust >= 70) outputText("You may have already been uncomfortably hard, but you start leaking pre-cum all over the inside of your [armor].  Of course, the copious fem-jizz that's already seeped inside your gear absorbs the extra fluid with little to no complaint.");
        ...
        if (kGAMECLASS.player.cockTotal() > 1) outputText("s");
        outputText(" are smothered with the stuff, making you moan without meaning to.");
        if (kGAMECLASS.player.biggestCockArea() >= 150) outputText("  If only you weren't so big!  You're compressed so tightly by your enclosing [armor], and even though you're rock-hard, you're a fraction of your full size.  If only you could get out of your gear and fuck her!");
    }
    //{Genderless}
    else {
...
export function bigValaLicksOffDudes(): void {
    ...
    if (kGAMECLASS.player.hasFuckableNipples()) outputText("  It even pauses at one to slip a tiny bit inside, too big to penetrate your nipplecunt properly but teasing it all the same.");
    ...
    if (kGAMECLASS.player.cockTotal() > 1) outputText("each");
    ...
    if (kGAMECLASS.player.cockTotal() > 1) outputText("s");
    outputText(" until ");
    if (kGAMECLASS.player.cockTotal() == 1) outputText("it slides");
    else outputText("they slide");
    outputText(" inside her");
    if (kGAMECLASS.player.biggestCockArea() >= 200) outputText(", the immense length sucked in by a surprising burst of suction");
    outputText(".");
    ...
    if (kGAMECLASS.player.biggestCockArea() < 200) outputText("smothering your relatively tiny pecker");
    else outputText("wrapping up your immense pecker");
    if (kGAMECLASS.player.biggestCockArea() >= 400) outputText(", even as it hangs partway down her throat");
    ...
    if (kGAMECLASS.player.biggestCockArea() >= 400) outputText(", cock partway down her throat");
    ...
    if (kGAMECLASS.player.balls > 0) outputText(", [balls] clenching against Vala's tongue");
    ...
    if (kGAMECLASS.player.cumQ() < 250) outputText("  She barely notices the spunk you shoot against her tongue, her drooling saliva washing it away almost immediately, but you're sure she got a nice, salty thrill from it regardless.");
    else if (kGAMECLASS.player.cumQ() < 1000) outputText("  She has to noisily gulp down the liquid deposit you leave in her mouth, but with her enhanced size, it's still no more than a creamy morsel.");
    else if (kGAMECLASS.player.cumQ() < 4000) outputText("  You slowly flood her mouth with spooge until her cheeks bulge.  The spooge inundates her maw until your lower body floats in it, forcing the girl to swallow hard or risk having the teeming seed spill out around you.");
...
export function valaVapulaThreesome(): void {
    //{First Meeting} 
    kGAMECLASS.monster[kFLAGS.TIMES_VAPULA_AND_GIANT_VALA]++;
    if (kGAMECLASS.monster[kFLAGS.TIMES_VAPULA_AND_GIANT_VALA] == 1) {
...
export function valaDommyVapula3Some(): void {
    ...
    if (!kGAMECLASS.player.hasCock()) outputText("buckling on your demonic strap-on");
    ...
    if (!kGAMECLASS.player.hasCock()) outputText("dildo");
    ...
    if (kGAMECLASS.player.hasCock()) outputText("seed");
    ...
    if (kGAMECLASS.player.cumQ() > 500) outputText("  The demon's purple belly balloons with cum, swelling against your own");
    if (kGAMECLASS.player.cumQ() >= 1500) outputText(" as her midriff turns taut and dome-like");
    if (kGAMECLASS.player.cumQ() > 500) outputText(".");
    if (kGAMECLASS.player.cumQ() > 1000) outputText("  Waves of spunk pour out from Vapula's lust-swollen lips thanks to your exceedingly potent orgasm.");
    if (kGAMECLASS.player.lactationQ() >= 100) {
        outputText("  Even better, your [fullChest] lets down your milk, letting it run out of you in ");
        if (kGAMECLASS.player.lactationQ() <= 200) outputText("thick");
        else if (kGAMECLASS.player.lactationQ() <= 2000) outputText("powerful");
...
export function valaLoveyVapula3Some(): void {
    ...
    if (!kGAMECLASS.player.hasCock()) outputText(", and you buckle on your demonic strap-on, just in case");
    ...
    if (!kGAMECLASS.player.hasCock()) outputText("the dildo");
    ...
    if (!kGAMECLASS.player.hasCock()) outputText("that exotic dildo");
    ...
    if (!kGAMECLASS.player.hasCock()) outputText("spurting dildo");
    else outputText("[cock smallest]");
...
export function kinathisValaStuff(): void {
    clearOutput();
    var x: number = kGAMECLASS.player.cockThatFits(50);
    if (x < 0) x = kGAMECLASS.player.smallestCockIndex();
    ...
    if (kGAMECLASS.player.tallness > 75) outputText("  She looks up at you as you seem to tower over her, a");
    else outputText("  A");
    ...
    if (kGAMECLASS.player.cockTotal() > 1) outputText("most appealing phallus");
    ...
    outputText("\n\nHer diminutive tongue sucks around your " + kGAMECLASS.player.cockHead(x) + ", teasing and dancing circles around you.  Her silky hair feels good around your fingers, making the perfect hand-hold as you keep her in place as she sucks and plays with you.  Pulling back with a slurp and a gasp, Vala licks her lips before kissing her way down your shaft.  ");
    if (kGAMECLASS.player.cocks[x].cockType == CockTypesEnum.HORSE) outputText("Her tongue licks around the throbbing glands, the heady animal musk filling her nose and mind with sweet lust.  ");
    else if (kGAMECLASS.player.cocks[x].cockType == CockTypesEnum.DOG) outputText("Her tongue licks around the hot building knot that gently throbs at your base.  ");
    //[anemone cock:
    else if (kGAMECLASS.player.cocks[x].cockType == CockTypesEnum.ANEMONE) outputText("Her tongue plays with the squirming tentacles that line the drone of your throbbing phallus.  She shivers and groans as the tentacles sting her full of pleasurable aphrodisiac.  ");
    outputText("Playing her way down your cock, the fey-girl gently licks up and down, sucking along your cumvein before coming back to the " + kGAMECLASS.player.cockHead(x) + " of your meaty prick.");
    ...
    outputText("\n\nNot wanting to wait a moment longer, you surge forward, pushing your " + cockDescript(x) + " between her  thighs, the " + kGAMECLASS.player.cockHead(x) + " gently probing the small tight sex that lay between them, her fey-honey oozing from the womanly flower like sweet nectar from the most exotic of plants.  The touch of your meaty masculinity against her puffy, sensitive nethers draws a gentle gasp from the excited woman.  Taking hold of her hips, you push forward and sink your aching, throbbing spear into her tender, needy body.  ");
    //[cockarea <17:
    if (kGAMECLASS.player.cockArea(x) < 17) outputText("Despite her size, you slide in easily, her wet, slippery pussy taking you in and squeezing you in a velvety embrace.  \"<i>Yessss, it's in, it's in!  Mmmm, come to me hero, come to Vala,</i>\" she croons to you, beckoning  you to take her.");
    else if (kGAMECLASS.player.cockArea(x) < 30) outputText("\"<i>Ahhh...  yesss, it's in!  Ohh...  come to me hero, come to Vala and fill her with your love!</i>\" she croons to you, beckoning  you to take her.");
    ...
    if (kGAMECLASS.player.cockArea(x) > 17) outputText("  Her hot little body stretches around your girthy prick, distending her belly as you sheath your throbbing arousal into her.");

    outputText("\n\nYour hands roam over her as you buck your hips into your fairy lover, your hands groping her round ass, her massive milky mammaries swaying and jiggling from your hard thrusts.  The bounce and ripple of her creamy udders is too much to bear, their hypnotic shakes drawing your hands to them, squeezing and manhandling those luscious orbs.  Spurts of rich milk splash across your " + kGAMECLASS.player.skinFurScales() + " as you pinch and squeeze her nipples.  The rough touches, the pinching, the squeezes and gropes draw long moans and groans from the panting, lusty fairy.  Your fey-lover murmurs your name as you take her, beckoning you for more, for you to keep fucking her and to make her scream.  \"<i>Haa haa, ohhhh, yessss fuck me, fuck Vala’s tight little cunt!  Give me all your burning love!</i>\" she pants and cries out as she bucks her hips up to you, her arms reaching up to hold onto you as you start to speed up your thrusts.");
    ...
    if (kGAMECLASS.player.balls > 0) {
        //if ballsize <3:
        if (kGAMECLASS.player.ballSize < 3) outputText("  With each hard quaking thrust your balls slap against Vala’s expansive ass.  Your nuts ache under their pent up load and demand you to shoot it into the sweet fey-girl soon.");
        else if (kGAMECLASS.player.ballSize < 6) outputText("  With each hard quaking thrust, your balls smack against Vala’s cushiony ass.  Your hefty, cum-bloated sack, so full, aches for the sweet release into your tender fairy lover.");
...
export function valaPartIIWaifuLove(): void {
    clearOutput();
    kGAMECLASS.model.time.hours = 6;
    kGAMECLASS.model.time.days++;
    kGAMECLASS.camp.sleepRecovery(false);
    statScreenRefresh();
