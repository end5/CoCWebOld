...
privates, only. Dxasmodeus grants exclusive rights to Fenoxo to add upon events to meet with 
...
private tentacleRapeContinuationForFemales():void {
	...
	if (player.clitLength < 7 && player.clitLength >= 3.5 && player.findPerk(PerkLib.BigClit) >= 0) {
		...
		outputText("Your massive clitty eventually diminishes, retaining a fair portion of its former glory.  It is now " + Math.floor(player.clitLength * 10) / 10 + " inches long when aroused, ");
