	export class TamanisDaughters extends Goblin
	{
		...
		override protected function performCombatAction():void
		{
			...
			switch (rand(select)) {
				case 0:
					createStatusAffect(StatusAffects.Attacks, Math.floor(flags[kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS] / 20), 0, 0, 0); //Tamani's Daughters get multiattacks!
					eAttack();
					break;
				case 1:
					special1();
		...
		public TamanisDaughters()
		{
			...
			this.bonusHP = 50 + (Math.floor(flags[kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS] / 2) * 15);
			this.lust = 30;
			this.lustVuln = .65;
