		...
		protected function whipAttack():void
		{
			if (findStatusAffect(StatusAffects.WhipReady) >= 0) {
				...
				if (temp == 0) {
					...
					player.takeDamage(4);
					game.dynStats("lus", 6 + Math.floor(player.sens / 20));
				}
				//Whip yo tits!
				if (temp == 1) {
					...
					player.takeDamage(9);
					game.dynStats("lus", 4 + Math.floor(player.sens / 15));
				}
				//Whip yo groin
				if (temp == 2) {
					if (player.gender == 0) {
					...
					if (player.gender == 1) {
						outputText("groin, dealing painful damage to your " + player.multiCockDescriptLight() + ", doubling you over in agony (" + Math.floor((player.tou * 2 + 50) / 4) + ")", false);
						game.dynStats("lus", -15);
						player.takeDamage(Math.floor((player.maxHP()) / 4));
					}
					if (player.gender == 2) {
					...
					if (player.gender == 3) {
						outputText("groin, dealing painful damage to your " + player.multiCockDescriptLight() + " and " + player.vaginaDescript(0) + ", doubling you over in agony (" + Math.floor((player.tou * 2 + 50) / 3) + ")", false);
						game.dynStats("lus", -20);
						player.takeDamage(Math.floor((player.maxHP()) / 3));
					}
				}
				//Whip yo legs
				if (temp == 3) {
