	export class Sheila extends Monster
	{
		...
		private sheilaFrogPunch():void {
			...
			//Hit:
				...
				damage = Math.floor((str + weaponAttack) - rand(player.tou) - player.armorDef);
		...
		private sheilaFlyingKick():void {
			...
			//Hit:
				...
				damage = Math.floor((str + 50 + weaponAttack) - rand(player.tou) - player.armorDef);
				if(damage < 1) damage = 2;
				damage = player.takeDamage(damage);
				outputText(" (" + damage + ")");
		...
		private suspiciousGlMath.floor():void {
