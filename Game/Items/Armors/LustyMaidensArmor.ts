...
export class LustyMaidensArmor extends Armor {
    ...
    public onEquip(character: Character): void {
        ...
        if (character.body.vaginas.find(Vagina.NotVirgin)) {
...
export function lustyMaidenPaizuri(character: Character, monster: Character): NextScreenChoices {
    DisplayText().clear();
    DisplayText("You make sure " + monster.desc.a + monster.desc.short + " is comfortably lying down, " + monster.desc.possessivePronoun + " " + describeCockShort(monster.body.cocks.get(0)) + " exposed to the air");
    if (monster.stats.lust < 50)
        ...
        DisplayText(", nicely turgid but quite ready to feel the sensuous pleasure of your girls' tight squeeze.  You lean over the defeated foe and kiss the rod just under the " + describeCockHead(monster.body.cocks.get(0)) + ", smiling when it expands under your slow kisses.  Your fingers move up to play with the sensitive, urethral bulge that runs along the underside, and in no time, " + monster.desc.a + monster.desc.short + " is hard as a rock, so ready that " + monster.desc.possessivePronoun + " member is lifting up on its own.");
    else
        DisplayText(", bouncing with each beat of " + monster.desc.possessivePronoun + " heart, thick beads of pre dribbling from " + monster.desc.possessivePronoun + " tip as you bat " + monster.desc.possessivePronoun + " hands away before " + monster.desc.subjectivePronoun + " can waste the load " + monster.desc.subjectivePronoun + "'s saved up for you.");
    ...
    if (character.body.vaginas.filter(Vagina.Virgin).length <= 0)
        DisplayText("a gift too fine for a selfish brute like you");
    else
        DisplayText("sacred and to be shared only with a cherished loved one");
    ...
    if (monster.body.balls.count > 0)
        DisplayText(" and fondle " + monster.desc.possessivePronoun + " balls one-handed, squeezing the virile orbs to try and coax more of " + monster.desc.possessivePronoun + " dirty, perverted thoughts to distill into salty seed");
    else if (monster.desc.short === "anemone")
        DisplayText(" and stroke " + monster.desc.possessivePronoun + " taint, even brushing over the featureless spot where an asshole would be, if she had one, to try and coax more of " + monster.desc.possessivePronoun + " dirty, perverted thoughts to distill into salty seed");
    else
        DisplayText(" and stroke " + monster.desc.possessivePronoun + " taint, even brushing close to " + monster.desc.possessivePronoun + " asshole to try and coax more of " + monster.desc.possessivePronoun + " dirty, perverted thoughts to distill into salty seed");
    DisplayText(".  A startled moan slips out of " + monster.desc.possessivePronoun + " lips, but you're just getting warmed up.  You dive down onto " + monster.desc.possessivePronoun + " " + describeCockShort(monster.body.cocks.get(0)) + ", taking the " + describeCockHead(monster.body.cocks.get(0)) + " straight into your mouth with a smooth gulp.");
    if (monster.body.cocks.get(0).area >= 80)
        DisplayText("  It's so big and strong that it pushes right into your throat, stretching out your neck in the shape of the intruding cock.");
    ...
    if (character.body.chest.find(BreastRow.FuckableNipples) || character.lactationQ() >= 100) DisplayText(", dripping,");
    ...
    if (character.body.vaginas.find(Vagina.Virgin)) DisplayText("maidenhead");
    ...
    if (monster.body.cocks.get(0).area >= 80)
        DisplayText(" with a messy cough to clear your throat");
    ...
    if (monster.body.balls.count > 0)
        DisplayText(", " + monster.desc.possessivePronoun + " balls pumping and bouncing in " + monster.desc.possessivePronoun + " sack");
