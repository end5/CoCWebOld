...
export class BeeHoney extends Consumable {
    ...
    public use(character: Character) {
        ...
        if (changes < changeLimit && (character.body.hair.color !== "shiny black" && character.body.hair.color !== "black and yellow") && character.body.hair.length > 10 && randInt(2) === 0) {
            ...
            DisplayText("Your hair is now " + character.body.hair.color + ", just like a bee-girl's!");
            changes++;
        }
        // Hair Length
        if (changes < changeLimit && character.body.hair.length < 25 && randInt(3) === 0) {
            ...
            DisplayText("becoming " + describeHair(character) + ".");
            changes++;
        }
        ...
        if (changes < changeLimit && chest.length > 2 && randInt(3) === 0 && !User.settings.hyperHappy) {
            ...
            if (character.body.skin.type === SkinType.FUR)
                DisplayText(character.body.hair.color + " " + character.body.skin.desc);
            else
                DisplayText(character.body.skin.tone + " " + character.body.skin.desc);
