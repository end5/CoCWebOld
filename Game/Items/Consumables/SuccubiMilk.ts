...
export class SuccubiMilk extends Consumable {
    ...
    public use(character: Character) {
        ...
        if (character.stats.cor >= 35 && character.stats.cor < 70) {
            ...
            else if (character.body.cocks.length > 0) DisplayText("  You feel a building arousal, but it doesn't affect your cock.");
        }
        if (character.stats.cor >= 70) {
            ...
            if (character.gender === Gender.FEMALE || character.gender === Gender.HERM) {
                ...
                if (character.body.vaginas.length > 1) DisplayText(" quiver in orgasm, ");
                if (character.body.vaginas.length === 1) DisplayText(" quivers in orgasm, ");
                ...
                if (character.body.cocks.length === 1) DisplayText("  You feel a strange sexual pleasure, but your " + describeCocks(character) + " remains unaffected.");
                else DisplayText("  You feel a strange sexual pleasure, but your " + describeCocks(character) + " remain unaffected.");
            }
        }
        if (this.tainted) {
        ...
        if (chance <= 75) {
            ...
            if (character.body.chest.length === 0) {
                ...
                Mod.Breast.growTopBreastRow(character, breastGrowth, character.body.chest.length, true);
        ...
        // Increase pussy wetness or grow one!!
            ...
            if (character.body.vaginas.length > 0) {
                ...
                if (character.body.vaginas.get(0).wetness === VaginaWetness.SLAVERING) {
                    if (character.body.vaginas.length === 1) DisplayText("Your " + describeVagina(character, character.body.vaginas.get(0)) + " gushes fluids down your leg as you spontaneously orgasm.");
                ...
                if (character.body.vaginas.get(0).wetness === VaginaWetness.DROOLING) {
                    if (character.body.vaginas.length === 1) DisplayText("Your pussy feels hot and juicy, aroused and tender.  You cannot resist as your hands dive into your " + describeVagina(character, character.body.vaginas.get(0)) + ".  You quickly orgasm, squirting fluids everywhere.  <b>You are now a squirter</b>.");
                    if (character.body.vaginas.length > 1) DisplayText("Your pussies feel hot and juicy, aroused and tender.  You cannot resist plunging your hands inside your " + describeVagina(character, character.body.vaginas.get(0)) + "s.  You quiver around your fingers, squirting copious fluids over yourself and the ground.  The fluids quickly disappear into the dirt.");
                    character.orgasm();
                    if (this.tainted) character.stats.cor += 1;
                }
                if (character.body.vaginas.get(0).wetness === VaginaWetness.SLICK) {
                    if (character.body.vaginas.length === 1) DisplayText("You feel a sudden trickle of fluid down your leg.  You smell it and realize it's your pussy-juice.  Your " + describeVagina(character, character.body.vaginas.get(0)) + " now drools lubricant constantly down your leg.");
                    if (character.body.vaginas.length > 1) DisplayText("You feel sudden trickles of fluids down your leg.  You smell the stuff and realize it's your pussies-juices.  They seem to drool lubricant constantly down your legs.");
                }
                ...
                if (character.body.vaginas.get(0).wetness === VaginaWetness.NORMAL) {
                    if (character.body.vaginas.length === 1) DisplayText("A feeling of intense arousal passes through you, causing you to masturbate furiously.  You realize afterwards that your " + describeVagina(character, character.body.vaginas.get(0)) + " felt much wetter than normal.");
