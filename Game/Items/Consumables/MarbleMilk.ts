...
export class MarbleMilk extends Consumable {
    ...
    public use(character: Character) {
        ...
        if (character.effects.get(StatusEffectType.Marble).value2 < 30 && character.effects.get(StatusEffectType.MarblesMilk).value3 === 0) CView.text("You gulp down the bottle's contents; Marble makes some good tasting milk.\n\n");
        // [before the character is addicted, Addiction < 50]
        else if (character.effects.get(StatusEffectType.MarblesMilk).value3 <= 0) CView.text("You gulp down the bottle's contents; Marble makes some really good tasting milk.\n\n");
