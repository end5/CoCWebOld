...
export class MarbleMilk extends Consumable {
    ...
    public use(character: Character) {
        ...
        if (character.statusAffects.get(StatusEffectType.Marble).value2 < 30 && character.statusAffects.get(StatusEffectType.MarblesMilk).value3 === 0) DisplayText("You gulp down the bottle's contents; Marble makes some good tasting milk.\n\n");
        // [before the character is addicted, Addiction < 50]
        else if (character.statusAffects.get(StatusEffectType.MarblesMilk).value3 <= 0) DisplayText("You gulp down the bottle's contents; Marble makes some really good tasting milk.\n\n");
        else if (character.statusAffects.get(StatusEffectType.MarblesMilk).value3 > 0) {
        ...
        // Purge withdrawl
        if (character.statusAffects.has(StatusEffectType.MarbleWithdrawl)) {
            character.statusAffects.remove(StatusEffectType.MarbleWithdrawl);
        ...
        if (character.statusAffects.has(StatusEffectType.BottledMilk)) {
            character.statusAffects.get(StatusEffectType.BottledMilk).value1 = (6 + randInt(6));
        }
        else character.statusAffects.add(StatusEffectType.BottledMilk, 12, 0, 0, 0);
    }
}
