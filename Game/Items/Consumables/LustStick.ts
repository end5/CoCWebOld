...
export class LustStick extends Consumable {
    ...
    public use(character: Character) {
        if (character.effects.has(StatusEffectType.LustStickApplied)) {
            ...
            character.effects.add(StatusEffectType.LustStickApplied, 24, 0, 0, 0);
            DisplayText("You carefully open the sweet-smelling tube and smear the lipstick over your lips.  ");
            if (character.body.cocks.length > 0) DisplayText("It tingles a little, but the drugs have little to no effect on you now.");
