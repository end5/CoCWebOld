...
export class LustStick extends Consumable {
    ...
    public canUse(character: Character): boolean {
        if (character.body.cocks.count > 0 && !character.perks.has(PerkType.LuststickAdapted)) {
    ...
    public use(character: Character) {
        if (character.statusAffects.has(StatusEffectType.LustStickApplied)) {
            character.statusAffects.get(StatusEffectType.LustStickApplied).value1 = randInt(12) + 12;
            ...
            character.statusAffects.add(StatusEffectType.LustStickApplied, 24, 0, 0, 0);
            DisplayText("You carefully open the sweet-smelling tube and smear the lipstick over your lips.  ");
            if (character.body.cocks.count > 0) DisplayText("It tingles a little, but the drugs have little to no effect on you now.");
