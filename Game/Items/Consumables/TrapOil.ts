...
export class TrapOil extends Consumable {
    ...
    public use(character: Character) {
        ...
        if (character.body.vaginas.length > 0 && randInt(4) === 0 && changes < changeLimit) {
            ...
            character.stats.sens += -2;
            // High fertility:
            if (character.body.fertility >= 30) DisplayText("It feels like your overcharged reproductive organs have simmered down a bit.");
            // Average fertility:
            else if (character.body.fertility >= 5) DisplayText("You feel like you have dried up a bit inside; you are left feeling oddly tranquil.");
            // [Low/No fertility:
            else {
                DisplayText("Although the numbness makes you feel serene, the trap oil has no effect upon your ");
                if (character.body.fertility > 0) DisplayText("mostly ");
