    ...
    public use(character: Character) {
        ...
        if (character.body.ears.type === EarType.RACCOON && character.body.legs.type !== LegType.RACCOON && changes < changeLimit && randInt(4) === 0) {
            ...
            // from hooves or hard feet, including centaurs and bees
                ...
                if (character.body.legs.type === LegType.HOOFED || character.body.legs.type === LegType.CENTAUR || character.body.legs.type === LegType.PONY) DisplayText("pops right out of its hoof just in time");
            ...
            // from human, demon, paw feet
                ...
                if ((character.body.legs.type === LegType.HUMAN || character.body.legs.type === LegType.DEMONIC_HIGH_HEELS || character.body.legs.type === LegType.DEMONIC_CLAWS) && character.body.skin.type !== SkinType.FUR) DisplayText("The sensation of walking around on what feels like a second pair of hands is so weird that you miss noticing the itchy fur growing in over your legs...  ");
        ...
        if (character.body.face.type !== FaceType.RACCOON_MASK && character.body.face.type !== FaceType.RACCOON && randInt(4) === 0 && changes < changeLimit) {
            ...
            if (character.body.face.type === FaceType.HUMAN || character.body.face.type === FaceType.SHARK_TEETH || character.body.face.type === FaceType.SNAKE_FANGS || character.body.face.type === FaceType.BUNNY) {
                ...
                else DisplayText("A dark, almost black mask shades the " + skinFurScales(character) + " around your eyes and over the topmost portion of your nose, lending you a criminal air!  <b>You now have a raccoon mask!</b>");
            }
            // from snout (will not overwrite full-coon snout but will overwrite others)
                ...
                if (((character.body.hair.color === "black" || character.body.hair.color === "midnight") && (character.body.skin.type === SkinType.FUR || character.body.skin.type === SkinType.SCALES))) DisplayText("your face apparently returned to normal shape, albeit still covered in " + skinFurScales(character) + ".  You look closer and discover a darker, mask-line outline on your already inky visage.  <b>You now have a barely-visible raccoon mask on your otherwise normal human face.</b>");
                else if ((character.body.skin.tone === "ebony" || character.body.skin.tone === "black") && (character.body.skin.type === SkinType.PLAIN || character.body.skin.type === SkinType.GOO)) DisplayText("your face apparently returned to normal shape.  You look closer and discover a darker, mask-line outline on your already inky visage.  <b>You now have a barely-visible raccoon mask on your normal human face.</b>");
