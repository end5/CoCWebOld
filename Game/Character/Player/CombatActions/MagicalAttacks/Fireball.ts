...
export class Fireball implements CombatAction {
    ...
    public use(character: Character, monster: Character): NextScreenChoices {
        ...
        if (randInt(5) === 0 || character.effects.has(StatusEffectType.WebSilence)) {
            if (character.effects.has(StatusEffectType.WebSilence)) DisplayText("You reach for the terrestrial fire, but as you ready to release a torrent of flame, it backs up in your throat, blocked by the webbing across your mouth.  It causes you to cry out as the sudden, heated force explodes in your own throat.\n\n");
            else if (character.effects.has(StatusEffectType.GooArmorSilence)) DisplayText("You reach for the terrestrial fire but as you ready the torrent, it erupts prematurely, causing you to cry out as the sudden heated force explodes in your own throat.  The slime covering your mouth bubbles and pops, boiling away where the escaping flame opens small rents in it.  That wasn't as effective as you'd hoped, but you can at least speak now.");
