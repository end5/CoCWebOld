    ...
    public incubationDisplay(player: Player, incubationTime: number) {
        ...
        if (incubationTime === 20) {
            ...
            if (player.body.cocks.length === 1) {
                ...
                if (player.body.cocks.filter(Cock.FilterType(CockType.DOG)).length === 1) DisplayText(", each inhalation making it thicker, harder, and firmer.  You suck in huge lungfuls of air, desperate for more, until your " + describeCock(player, firstCock) + " is twitching and dripping, its knot swollen to the max.  ");
                if (player.body.cocks.filter(Cock.FilterType(CockType.HUMAN)).length === 1) DisplayText(", each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air, until your " + describeCock(player, firstCock) + " is twitching and dripping, the head swollen and purple.  ");
