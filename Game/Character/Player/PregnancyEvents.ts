...
export function updatePregnancy(player: Player) {
    ...
    if (player.pregnancy.womb.isPregnant() && player.pregnancy.womb.pregnancy.incubation === 1) {
        ...
        if (!player.statusAffects.has(StatusEffectType.Birthed)) player.statusAffects.add(StatusEffectType.Birthed, 1, 0, 0, 0);
    }
    else {
        player.statusAffects.get(StatusEffectType.Birthed).value1 += 1;
        if (!player.perks.has(PerkType.BroodMother) && player.statusAffects.get(StatusEffectType.Birthed).value1 >= 10) {
...
export function eggDescript(player: Player, plural: boolean = true): string {
    let descript: string = "";
    if (player.statusAffects.has(StatusEffectType.Eggs)) {
        descript += numToCardinalText(player.statusAffects.get(StatusEffectType.Eggs).value3) + " ";
        // size descriptor
        if (player.statusAffects.get(StatusEffectType.Eggs).value2 === 1) descript += "large ";
        /*color descriptor
        ...
        5 - rubbery black -
        */
        if (player.statusAffects.get(StatusEffectType.Eggs).value1 === 0) descript += "brown ";
        if (player.statusAffects.get(StatusEffectType.Eggs).value1 === 1) descript += "purple ";
        if (player.statusAffects.get(StatusEffectType.Eggs).value1 === 2) descript += "blue ";
        if (player.statusAffects.get(StatusEffectType.Eggs).value1 === 3) descript += "pink ";
        if (player.statusAffects.get(StatusEffectType.Eggs).value1 === 4) descript += "white ";
        if (player.statusAffects.get(StatusEffectType.Eggs).value1 === 5) descript += "rubbery black ";
        // EGGS
