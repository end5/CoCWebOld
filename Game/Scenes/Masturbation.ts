...
export function display(player: Character): NextScreenChoices {
    ...
    if ((player.perks.has(PerkType.HistoryReligious) && player.stats.cor <= 66) || (player.perks.has(PerkType.Enlightened) && player.stats.cor < 10)) {
        if (player.statusAffects.has(StatusAffectType.Exgartuan) && player.statusAffects.get(StatusAffectType.Exgartuan).value2 === 0)
            choices.push(["Masturbate", masturbateGo]);
        else choices.push(["Meditate", meditate]);
    }
    else choices.push(["Masturbate", masturbateGo]);
    // catofellato
    if (player.torso.cocks.count > 0 && (player.perks.has(PerkType.Flexibility) || User.flags.get<PlayerFlags>(CharacterType.Player).TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY > 0)) {
        choices.push(["Lick Cock", catAutoLick]);
    }
    if (player.torso.vaginas.count > 0 && (player.perks.has(PerkType.Flexibility) || User.flags.get<PlayerFlags>(CharacterType.Player).TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY > 0)) {
        choices.push(["Lick 'Gina", lickYerGirlParts]);
    }
    if (player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE)).length > 0 && player.torso.vaginas.count > 0) {
        choices.push(["Tentapussy", tentacleSelfFuck]);
    }
    if (player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE)).length > 0) {
        choices.push(["Tentabutt", tentacleGoesUpYerPooperNewsAtEleven]);
    }
    if (player.canOvipositBee() && player.stats.lust >= 33 && player.torso.cocks.sort(Cock.LargestCockArea)[0].area > 100) {
        choices.push(["LayInCock", getHugeEggsInCawk]);
    }
    if (player.pregnancy.ovipositor.canOviposit() && player.torso.chest.find(BreastRow.FuckableNipples) && player.stats.lust >= 33 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 21) {
        choices.push(["LayInTits", layEggsInYerTits]);
    }
    if (fappingItems(player).choices.length > 0)
        persistantChoices.push(["Items", fappingItems]);
    else if (choices.length === 1) { // If you can only masturbate or meditate the normal way then do that automatically
        if ((player.perks.has(PerkType.HistoryReligious) && player.stats.cor <= 66) || (player.perks.has(PerkType.Enlightened) && player.stats.cor < 10)) {
            if (player.statusAffects.has(StatusAffectType.Exgartuan) && player.statusAffects.get(StatusAffectType.Exgartuan).value2 === 0)
                return masturbateGo(player);
            else return meditate(player);
        }
        else return masturbateGo(player);
    }

    return { choices: [["Masturbate", masturbateGo]], persistantChoices: [["Back", Menus.Player]] };
}

function fappingItems(player: Character): NextScreenChoices {
    const canReachCock: boolean = player.torso.cocks.count > 0 && (!player.torso.hips.legs.isTaur() || player.torso.cocks.sort(Cock.LongestCocks).length >= player.tallness * (5 / 6));
    const choices = [];
    if (player.inventory.keyItems.has("Deluxe Dildo") && player.torso.vaginas.count > 0 && !player.torso.hips.legs.isTaur()) {
        choices.push(["D. Dildo", deluxeDildo]);
    }
    if (player.inventory.keyItems.has("All-Natural Onahole") && canReachCock) {
        choices.push(["AN Onahole", allNaturalOnaholeUse]);
    }
    if (player.inventory.keyItems.has("Deluxe Onahole") && canReachCock) {
        choices.push(["D Onahole", deluxeOnaholeUse]);
    }
    if (player.inventory.keyItems.has("Plain Onahole") && canReachCock) {
        choices.push(["Onahole", onaholeUse]);
    }
    if (player.inventory.keyItems.has("Self-Stimulation Belt") && player.torso.vaginas.count > 0 && !player.torso.hips.legs.isTaur()) {
        choices.push(["Stim-Belt", stimBeltUse]);
    }
    if (player.inventory.keyItems.has("All-Natural Self-Stimulation Belt") && player.torso.vaginas.count > 0 && !player.torso.hips.legs.isTaur()) {
        choices.push(["AN Stim-Belt", allNaturalStimBeltUse]);
    }
    if (player.inventory.keyItems.has("Dual Belt") && player.gender === Gender.HERM && !player.torso.hips.legs.isTaur()) {
        choices.push(["Dual Belt", dualBeltMasturbation]);
    }
    if (player.inventory.keyItems.has("Fake Mare") && player.torso.cocks.count > 0 && player.torso.hips.legs.isTaur()) {
        choices.push(["Fake Mare", centaurDudesGetHorseAids]);
    }
    if (player.inventory.keyItems.has("Centaur Pole") && player.torso.vaginas.count > 0 && player.torso.hips.legs.isTaur()) {
        choices.push(["C. Pole", centaurGirlsGetHorseAids]);
    }
    if (player.inventory.keyItems.has("Dildo")) {
        choices.push(["Anal Dildo", dildoButts]);
        if (player.torso.vaginas.count > 0) choices.push(["Dildo", stickADildoInYourVagooSlut]);
    }
    return { choices, persistantChoices: [["Back", display]] };
}

// Non-shitty masturbation
export function masturbateGo(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    //     return;
    // }
    // if (player.countCockSocks("gilded") > 0 && User.flags.get<PlayerFlags>(CharacterType.Player).GILDED_JERKED < player.countCockSocks("gilded")) {
    //     gildedCockTurbate();
    //     return;
    // }
    ...
    if (player.torso.cocks.count > 0) {
        if (player.torso.cocks.count === 1) {
            DisplayText("You stroke your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)));
    ...
    if (player.torso.vaginas.count > 0) {
        if (player.torso.vaginas.count === 1) {
            ...
            if (player.stats.lib < 45)
                DisplayText("You touch and play with your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", ");
            else if (player.stats.lib < 70)
                DisplayText("You slap your pussy softly, ");
            else DisplayText("You touch your enflamed and aroused " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", ");
            switch (player.torso.vaginas.get(0).wetness) {
                ...
                case VaginaWetness.DROOLING:
                    DisplayText("slicking your fingers in the juices that constantly dribble from " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + "  "); break;
                default:
        ...
        if (player.torso.vaginas.count > 1) {
            ...
            else DisplayText("Touch your enflamed and aroused " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + "s, ");
            switch (player.torso.vaginas.get(0).wetness) {
                case VaginaWetness.DRY:
                    DisplayText("expertly arousing your female parts.  "); break;
                case VaginaWetness.NORMAL:
                ...
                case VaginaWetness.SLICK:
                    DisplayText("smiling as your fingers become coated in your slick fluids.  "); break;
                case VaginaWetness.DROOLING:
                    DisplayText("slicking your fingers in the juices that constantly dribble from " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + "s  "); break;
                default:
    ...
    if (player.torso.cocks.count === 1) {
        ...
        if (onlyCock.thickness < 1.8)
            DisplayText("You easily wrap a hand around your " + Desc.Cock.describeCock(player, onlyCock) + " and start masturbating.  ");
        else if (onlyCock.thickness < 3) {
            DisplayText("You have some difficulty fitting your hand around your " + Desc.Cock.describeCock(player, onlyCock) + ", relishing the feelings of your ");
            ...
            DisplayText("You use both hands to grip your " + Desc.Cock.describeCock(player, onlyCock) + ", feeling the throbbing of your " + (onlyCock.hasKnot() ? "knot" : "member") + " as you begin to masturbate.  ");
        }
        else DisplayText("You grasp onto your " + Desc.Cock.describeCock(player, onlyCock) + " with both hands, but fail to encircle it fully as you begin to masturbate.  ");
        // If length > 12 proud
        ...
        if (onlyCock.length < 12) {
            if (onlyCock.type === CockType.HUMAN)
                ...
                DisplayText("You stroke quickly, feeling the tiny 'barbs' of your " + Desc.Cock.describeCock(player, onlyCock) + " sliding through your fingers, even darting down to circle the sensitive skin around your sheath.  ");
            else if (onlyCock.type === CockType.LIZARD)
                DisplayText("You stroke quickly, pleasuring your sensitive " + Desc.Cock.describeCock(player, onlyCock) + ", sliding fingers over each ridge and bump that covers its knotty length.  ");
            else if (onlyCock.type === CockType.ANEMONE)
                DisplayText("You stroke quickly, gasping as your fingers are stung repeatedly with the aphrodisiac-laced tentacles around the base of your " + Desc.Cock.describeCock(player, onlyCock) + " and under its crown.  ");
            ...
            if (onlyCock.type === CockType.HUMAN)
                DisplayText("You delight in teasing the crown of your " + Desc.Cock.describeCock(player, onlyCock) + ", rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  It seems to pulse and twitch with each stroke, responding to every touch.  ");
            else if (onlyCock.type === CockType.HORSE)
                DisplayText("You delight in teasing the sensitive flared tip of your " + Desc.Cock.describeCock(player, onlyCock) + ", rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  It seems to pulse and ripple with each stroke, responding to every touch.  ");
            else if (onlyCock.type === CockType.DOG)
                DisplayText("You delight in teasing the pointed tip of your " + Desc.Cock.describeCock(player, onlyCock) + ", rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  Your knot seems to pulse and twitch with each stroke, reacting to every touch.  ");
            else if (onlyCock.type === CockType.TENTACLE)
                DisplayText("You delight in teasing the over-sized mushroom-like tip of your " + Desc.Cock.describeCock(player, onlyCock) + ", caressing it after every stroke as you squeeze out dollops of pre to smear along its slimy length.  It writhes and twists in your hands of it's own volition, lengthening and shortening with each set of strokes.  ");
            else if (onlyCock.type === CockType.DEMON)
                DisplayText("You delight in teasing the larger bumps that form a ring around the crown of your " + Desc.Cock.describeCock(player, onlyCock) + ", watching as they twitch and spasm in time with the dollops of pre you're squeezing out and smearing over the entire length.  ");
            else if (onlyCock.type === CockType.CAT)
                DisplayText("You delight in teasing the sensitive nubs all along your " + Desc.Cock.describeCock(player, onlyCock) + ", circling the cock-tip at the end of each stroke to gather pre and slather it over your entire length.  Each of the tiny 'barbs' provides bursts of pleasure with each stroke, driving you on.  ");
            else if (onlyCock.type === CockType.LIZARD)
                DisplayText("You delight in teasing the rounded bulbs that cover your " + Desc.Cock.describeCock(player, onlyCock) + ", circling them with your fingertips before sliding up to the urethra and gathering a drop of pre.  You smear it over your sensitive reptile skin and revel in the pleasure radiating through you.  ");
            else if (onlyCock.type === CockType.ANEMONE)
                DisplayText("You delight in grabbing hold of the tiny, stinging tentacles around the base and squeezing them between your " + Desc.Cock.describeCock(player, onlyCock) + " and hand.  Aphrodisiac pours into your blood as you release them and stroke along the length, gathering dollops of pre to coat yourself with and 'accidentally' bumping the other tentacles at the crown.  ");
            else if (onlyCock.type === CockType.DISPLACER)
                DisplayText("You delight in teasing the opened tip of your " + Desc.Cock.describeCock(player, onlyCock) + ", rubbing it at the end of each stroke, watching it squeeze out dollops of pre that you smear over it to tease yourself with.  Your knot seems to pulse and twitch with each stroke, reacting to every touch.  ");
            else DisplayText("You delight in teasing the crown of your " + Desc.Cock.describeCock(player, onlyCock) + ", rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  It seems to pulse and twitch with each stroke, responding to every touch.  ");
        }
        else if (onlyCock.length < 26) {
            if (onlyCock.type === CockType.HUMAN)
                DisplayText("The head of your " + Desc.Cock.describeCock(player, onlyCock) + " wobbles towards your face as you masturbate, a dollop of pre slowly growing atop it.  ");
            else if (onlyCock.type === CockType.HORSE)
                DisplayText("The flared tip of your " + Desc.Cock.describeCock(player, onlyCock) + " wobbles towards your face as you masturbate, a dollop of pre slowly growing atop it.  ");
            else if (onlyCock.type === CockType.DOG)
                DisplayText("The pointed tip of your " + Desc.Cock.describeCock(player, onlyCock) + " angles towards your face as you masturbate, a dollop of pre slowly leaking from it.  ");
            else if (onlyCock.type === CockType.TENTACLE)
                DisplayText("The overly wide head of your " + Desc.Cock.describeCock(player, onlyCock) + " bumps against your lips as you masturbate, waving back and forth like a snake as it searches for an orifice.  ");
            else if (onlyCock.type === CockType.DEMON)
                DisplayText("The purplish head of your " + Desc.Cock.describeCock(player, onlyCock) + " bumps against your lips as you masturbate, flushing darkly with every beat of your heart.  ");
            else if (onlyCock.type === CockType.CAT)
                DisplayText("The slightly pointed tip of your " + Desc.Cock.describeCock(player, onlyCock) + " bumps against your lips as you masturbate, flushing with blood as your spines grow thicker in your hand.  ");
            else if (onlyCock.type === CockType.LIZARD)
                DisplayText("The pointed, purple tip of your " + Desc.Cock.describeCock(player, onlyCock) + " bumps against your lips while its knotted surface flushes near-purple and seems to grow thicker.  ");
            else if (onlyCock.type === CockType.ANEMONE)
                DisplayText("The tentacle-ringed tip of your " + Desc.Cock.describeCock(player, onlyCock) + " brushes against your lips, making them tingle with artificial heat while you stroke it.  ");
            else if (onlyCock.type === CockType.DISPLACER)
                DisplayText("The blooming tip of your " + Desc.Cock.describeCock(player, onlyCock) + " angles towards your face as you masturbate, a dollop of pre slowly leaking out of the outstretched top.  ");
            else DisplayText("The head of your " + Desc.Cock.describeCock(player, onlyCock) + " wobbles towards your face as you masturbate, a dollop of pre slowly growing atop it.  ");
            // try to stick it in a titty!
            if (player.torso.chest.find(BreastRow.FuckableNipples) && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 3) {
                // UNFINISHED - TWEAK PENETRATION VALUES
                nippleFuck = true;
                titFuckSingle(player);
            }
            else {
                // autofellatio doesnt get to be true unless you keep it in your mouth instead of sticking it in your boob.
                DisplayText("You give in to temptation and swallow the tip, slurping greedily as you milk your " + Desc.Cock.describeCock(player, onlyCock) + " of its pre-cum.  ");
                autofellatio = true;
            }
            if (player.torso.chest.find(BreastRow.CanTitFuck) && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 3 && !nippleFuck) {
                DisplayText("Your hands migrate to your breasts of their own accord, wrapping your titflesh around your " + Desc.Cock.describeCock(player, onlyCock) + ", jacking it up and down in your pillowy tits.  ");
            ...
            if (onlyCock.type === CockType.HUMAN)
                DisplayText("The head of your " + Desc.Cock.describeCock(player, onlyCock) + " wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with your cock.  ");
            else if (onlyCock.type === CockType.HORSE)
                DisplayText("The flared tip of your " + Desc.Cock.describeCock(player, onlyCock) + " wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with horsemeat.  ");
            else if (onlyCock.type === CockType.DOG)
                DisplayText("The head of your " + Desc.Cock.describeCock(player, onlyCock) + " wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with your " + Desc.Cock.describeCock(player, onlyCock) + ".  ");
            else if (onlyCock.type === CockType.TENTACLE)
                DisplayText("The bulbous, mushroom-like head of your " + Desc.Cock.describeCock(player, onlyCock) + " pushes against your face eagerly, smearing pre-cum over your lips as it seeks entrance to the nearest orifice.  You're unable to resist opening your mouth to suck it down, filling your mouth with slick rubbery cock-tentacle.  ");
            else if (onlyCock.type === CockType.DEMON)
                DisplayText("The tainted swollen head of your " + Desc.Cock.describeCock(player, onlyCock) + " pushes against your face, smearing sweet pre-cum over your lips.  You're unable to resist opening wide and taking in the demonic member, submitting wholly to the desire to pleasure your corrupted body-parts.  ");
            else if (onlyCock.type === CockType.CAT)
                DisplayText("The pointed tip of your " + Desc.Cock.describeCock(player, onlyCock) + " pushes against your face, smearing pre-cum over your lips and tickling you with the many barbs.  You're unable to resist opening wide and taking in the " + Desc.Cock.describeCock(player, onlyCock) + ", half-humming half-purring in contentment.  ");
            else if (onlyCock.type === CockType.LIZARD)
                DisplayText("The slightly-pointed tip of your " + Desc.Cock.describeCock(player, onlyCock) + " pushes against your face, smearing pre-cum over your eager lips.   You can't resist opening wide and slipping it inside as your hands caress the reptilian bulges along your length.  ");
            else if (onlyCock.type === CockType.ANEMONE)
                DisplayText("The rounded, tentacle-ringed tip of your " + Desc.Cock.describeCock(player, onlyCock) + " slides against your face, smearing pre-cum over your lips and stinging them with aphrodisiacs that make you pant with lust.  You can't resist opening wide to greedily slurp it down, and in seconds tiny, tingling stings are erupting through your oral cavity, filling you with lust and pleasure.  ");
            else if (onlyCock.type === CockType.DISPLACER)
                DisplayText("The head of your " + Desc.Cock.describeCock(player, onlyCock) + " wobbles over, bumping your face and smearing your lips with its copious pre-cum.  Wiggling against your lips, the various protrusions of the 'star' at the tip smear you with your heady emissions.   You are unable to resist opening your mouth and sucking it down, filling your mouth with your " + Desc.Cock.describeCock(player, onlyCock) + ".  ");
            else DisplayText("The head of your " + Desc.Cock.describeCock(player, onlyCock) + " wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with your cock.  ");
            // try to stick it in a titty!
            if (player.torso.chest.find(BreastRow.FuckableNipples) && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 3) {
                // In-between text
                // UNFINISHED
                ...
                if (player.stats.cor > 60) {
                    if (onlyCock.type === CockType.HUMAN)
                        ...
                        DisplayText("The sweet scent of your " + Desc.Cock.describeCock(player, onlyCock) + " fills your nostrils as inch after inch of tentacle forces its way down your throat.  You struggle briefly with it, wrestling to keep it from pushing the whole way into your gut, swooning with the pleasure that fighting with your own cock induces.  ");
                    else if (onlyCock.type === CockType.DEMON)
                        DisplayText("The spicy demonic odor your " + Desc.Cock.describeCock(player, onlyCock) + " radiates fills your nostrils as you force inch after inch of tainted meat down your throat.  You struggle briefly, but your " + Desc.Cock.describeCock(player, onlyCock) + " quickly overwhelms your resistance and your gag reflex momentarily seems to disappear.  ");
                    else if (onlyCock.type === CockType.CAT)
                        ...
                        DisplayText("The salty, dry odor of your " + Desc.Cock.describeCock(player, onlyCock) + " fills your nostrils as you force inch after inch into your mouth, swallowing as much of the bulgy shaft as you can.  ");
                    else if (onlyCock.type === CockType.ANEMONE)
                        DisplayText("The stinging, aphrodisiac-laced " + Desc.Cock.describeCock(player, onlyCock) + " slowly works its way down your throat as you swallow it deeper and deeper, gurgling happily as your throat grows more sensitive.  ");
                    else if (onlyCock.type === CockType.DISPLACER)
                        DisplayText("The strong animal scent of your cock fills your nostrils as you force inch after inch into your mouth, deepthroating as much of yourself as you can.  ");
            ...
            if (player.torso.chest.find(BreastRow.FuckableNipples) && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 3 && !nippleFuck) {
                DisplayText("Your hands reach up to your pillowy breasts, wrapping them around the shaft of your " + Desc.Cock.describeCock(player, onlyCock) + ", causing you to let out muffled moans of excitment. ");
                if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 0) DisplayText("Jets of milk squirt out of your nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ");
            }
        }
    }
        ...
        DisplayText("You grasp one of your " + Desc.Cock.describeMultiCockShort(player) + " in each hand, ");
        if (player.torso.cocks.reduce(Cock.AverageCockThickness, 0) <= 1.8)
        ...
        // Get into it
        // Play with it if <= 10"
        if (player.torso.cocks.reduce(Cock.AverageCockLength, 0) <= 10)
        ...
        // Tittyfuck if D cup+
        else if (player.torso.cocks.reduce(Cock.AverageCockLength, 0) < 20) {
            ...
            if (player.torso.chest.find(BreastRow.FuckableNipples)) {
                // check for dogcocks
                if (player.torso.cocks.find(Cock.FilterType(CockType.DOG))) {
                    // make sure that your cocks will fit in your tits if dogcocks
                    if (player.torso.chest.find(BreastRow.FuckableNipples)) {
                        nippleFuck = true;
                        if (multiTitFuck(player)) autofellatio = true;
                    }
                        ...
                        autofellatio = true;
                        // UNFINISHED
                        if (player.torso.cocks.find(Cock.FilterType(CockType.HUMAN))) titFuckSingle(player);
            ...
            if (player.torso.chest.find(BreastRow.CanTitFuck) && !nippleFuck) {
                DisplayText("You wrap your " + Desc.Breast.breastCup(player.torso.chest.get(0).rating) + " tits around your " + Desc.Cock.describeMultiCockShort(player) + ", sandwiching them inside your pillowy mounds.  With delicious slowness you begin jacking yourself off with your tits, your shafts rubbing together between your luxurious breasts, tongue running figure eights across the tips. ");
                if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 0) DisplayText("Jets of milk squirt out of your nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ");
            }
            // Titfuck (enough breasts for both!)
            if (player.torso.chest.find(BreastRow.CanTitFuck) && player.torso.chest.sort(BreastRow.MostBreastsPerRow)[0].count > 2 && !nippleFuck) {
                DisplayText("You wrap your " + Desc.Breast.breastCup(player.torso.chest.get(0).rating) + " tits around your " + Desc.Cock.describeMultiCockShort(player) + ", squeezing them between your " + player.torso.chest.count * 2 + " breasts.  With delicious slowness you begin jacking yourself off with your tits, smooth breastflesh massaging your members as you aggressively lick and suck at the tips.  ");
            ...
            if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating <= 3 && !nippleFuck) {
                DisplayText("You slowly stroke yourself, pausing every few strokes to smear the streaks of pre that leak from your " + Desc.Cock.describeMultiCockShort(player) + " over yourself.  As you become more and more lubed and horny, you increase the tempo, until at last you are pumping yourself hard and fast, squeezing your cocks together in pleasure.  ");
            }
        }
        // Deepthroat yourself, maybe titfuck if equipped.
        else {
            // autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs
            DisplayText("Your " + Desc.Cock.describeMultiCockShort(player) + " brush against your face, throbbing with need.   You open your mouth and suck one of your pricks into your mouth, taking as much of it as possible, running your tongue along the underside.  You smile and pop it free, then take a different shaft into your maw.  ");
            // Try to stick it in a titty!
            if (player.torso.chest.find(BreastRow.FuckableNipples)) {
                // check for dogcocks
                if (player.torso.cocks.find(Cock.FilterType(CockType.DOG))) {
                    ...
                    if (player.torso.chest.find(BreastRow.FuckableNipples)) {
                        ...
                        nippleFuck = true;
                        autofellatio = true;
                        if (player.torso.cocks.find(Cock.FilterType(CockType.HUMAN)) || player.torso.cocks.find(Cock.FilterType(CockType.HORSE))) titFuckSingle(player);
                    }  // if you only have dogcocks that are too big, no nipple fuck! Otherwise, run normal multiTitFuck
                }
                else {
                    nippleFuck = true;
            ...
            // Titfuck (squeeze em both in between 2!
            if (player.torso.chest.find(BreastRow.CanTitFuck) && !nippleFuck) {
                DisplayText("You wrap your " + Desc.Breast.breastCup(player.torso.chest.get(0).rating) + " tits around your " + Desc.Cock.describeMultiCockShort(player) + ", sandwiching them inside your pillowy mounds.  With delicious slowness you begin jacking yourself off with your tits, your shafts rubbing together between your luxurious breasts, mouthfucking yourself with each of your cocks in turn. ");
                if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 0) DisplayText("Jets of milk squirt out of your nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ");
            }
            // Titfuck (enough breasts for both!)
            if (player.torso.chest.sort(BreastRow.MostBreastsPerRow)[0].count > 2 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 3 && !nippleFuck) {
        ...
        DisplayText("You grasp one of your " + Desc.Cock.describeMultiCockShort(player) + " in each hand, ");
        if (player.torso.cocks.reduce(Cock.AverageCockThickness, 0) <= 1.8)
            DisplayText("wrapping your fingers around each tool and just holding yourself, the feelings of your " + numToCardinalText(player.torso.cocks.count) + " cocks making you feel giddy.  ");
        ...
        // Tittyfuck if D cup+
            ...
            if (player.torso.chest.find(BreastRow.FuckableNipples)) {
                ...
                if (player.torso.cocks.find(Cock.FilterType(CockType.DOG))) {
                    ...
                    if (player.torso.chest.find(BreastRow.FuckableNipples)) {
                        ...
                        nippleFuck = true;
                        autofellatio = true;
                        if ((player.torso.cocks.find(Cock.FilterType(CockType.HUMAN)) || player.torso.cocks.filter(Cock.FilterType(CockType.HORSE)).length > 0) && player.torso.cocks.count - player.torso.cocks.filter(Cock.FilterType(CockType.DOG)).length === 1) titFuckSingle(player);
                        if (player.torso.cocks.count - player.torso.cocks.filter(Cock.FilterType(CockType.DOG)).length > 1) multiTitFuck(player);
                    } // if you only have dogcocks that are too big, no nipple fuck! Otherwise, run normal multiTitFuck
                }
                else {
                    nippleFuck = true;
            ...
            // Titfuck (squeeze em both in between 2!
            if (player.torso.chest.find(BreastRow.CanTitFuck) && !nippleFuck) {
                DisplayText("You wrap your " + Desc.Breast.breastCup(player.torso.chest.get(0).rating) + " tits around your " + Desc.Cock.describeMultiCockShort(player) + ", sandwiching them inside your pillowy mounds.  With delicious slowness you begin jacking yourself off with your tits, your shafts rubbing together between your luxurious breasts, your tongue running figure eights across the tips. ");
                if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 0) DisplayText("Jets of milk squirt out of your nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ");
            }
            // Titfuck (enough breasts for both!)
            if (player.torso.chest.sort(BreastRow.MostBreastsPerRow)[0].count > 2 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 3 && !nippleFuck) {
        ...
        // Deepthroat yourself, maybe titfuck if equipped.
        if (player.torso.cocks.reduce(Cock.AverageCockLength, 0) >= 20) {
            // autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs
            DisplayText("Your " + Desc.Cock.describeMultiCockShort(player) + " brush against your face, throbbing with need.   You open your mouth and suck one of your pricks into your mouth, taking as much of it in as possible, running your tongue along the underside.  You smile and pop it free, then take a different shaft into your maw.  ");
            // Try to stick it in a titty!
            if (player.torso.chest.find(BreastRow.FuckableNipples)) {
                // check for dogcocks
    ...
    if (player.torso.vaginas.count > 0) {
        ...
        if (player.torso.vaginas.count === 1) {
            ...
            switch (player.torso.vaginas.get(0).wetness) {
                ...
                case VaginaWetness.DROOLING:
                    DisplayText("Warm wetness runs down your legs in thick streams, pouring from your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".  You smile sheepishly and stroke up your now slickened legs to your pussy-lips, parting them and letting your fingers dive inside the wet channel.  "); break;
                default:
    ...
    // ORGASM GOES HERE
    orgazmo(player, autofellatio, nippleFuck);
    ...
    if (player.torso.cocks.count > 0) {
        ...
        if (player.torso.cocks.count === 1) {
            ...
            if (player.stats.lib < 30)
                ...
                DisplayText("You roll and begin to doze, your semi-erect " + Desc.Cock.describeCock(player, onlyCock) + " flopping against you. ");
            else if (player.stats.lib <= 80) {
                DisplayText("As you close your eyes and relax, your " + Desc.Cock.describeCock(player, onlyCock) + " surges back to erectness, ensuring ");
            ...
            else DisplayText("You groan and drift to sleep, your rigid " + Desc.Cock.describeCock(player, onlyCock) + " pulsing and throbbing with continual lust.");
        }
        // Multi Cock
        else {
            if (player.stats.lib < 30)
...
function genderlessMasturbate(player: Character) {
    DisplayText().clear();
    // Early prep
    if (player.stats.cor < 15)
        DisplayText("You sheepishly find some rocks to hide in, where you remove your clothes.\n\n");
    ...
    DisplayText("Your " + Desc.Butt.describeButthole(player.torso.butt) + " begins to twitch. It's practically crying out for attention.\n\n");
    DisplayText("You lay down on your side and reach gingerly behind yourself.  The palm of your hand comes to rest on your " + Desc.Butt.describeButt(player) + ".  You slide your finger into your crack and find your " + Desc.Butt.describeButthole(player.torso.butt) + ".  You run your finger slowly around the sensitive ring of your hole and feel a tingle emanating from it. A smile creeps across your lips as you begin to imagine what is about to happen.\n\n");
    ...
    if (player.torso.butt.wetness < 2) {
        ...
        DisplayText(" in a generous helping of saliva and head back for your " + Desc.Butt.describeButthole(player.torso.butt) + ".\n\n");
    }
    ...
    if (player.torso.butt.looseness === 5) {
        DisplayText("A devilish thought crosses your mind. You have taken into yourself all manner of beasts and beings. There is only one real way to achieve the pleasure you have gotten from them on your own. You slowly force your whole hand into your " + Desc.Butt.describeButthole(player.torso.butt) + " and are greeted with a fullness that you never thought you would achieve without assistance. As you move in and out you begin to slowly close your hand into a fist and open it up again over and over.\n\n");
    }
    // All scene types -
...
function titForeplay(player: Character) {
    ...
    if (player.torso.chest.count === 1 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 3) {
        if (player.stats.lib < 45)
            DisplayText("You caress your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + " gently with your fingers");
        else if (player.stats.lib < 70)
            DisplayText("You grope your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + " agressively with both hands");
        else DisplayText("You squeeze your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + " brutally with both hands");
    }
    else if (player.torso.chest.count > 1 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 3) {
        if (player.stats.lib < 45)
            DisplayText("You hesitantly run your hands across your front, caressing each of your " + numToCardinalText(player.torso.chest.count * 2) + " breasts in turn. ");
        else if (player.stats.lib < 70)
            DisplayText("You run your hands across your front, squeezing each of your " + numToCardinalText(player.torso.chest.count * 2) + " breasts in turn. ");
        else DisplayText("You aggressively run your hands across your front, groping each of your " + numToCardinalText(player.torso.chest.count * 2) + " breasts in turn. ");
        ...
        if (player.stats.cor <= 25)
            DisplayText("You blink back tears of shame as you experience the tactile proof of your abnormal endowments, unable to resist mashing your " + Desc.Breast.describeAllBreasts(player) + " together");
        else if (player.stats.cor < 75)
            DisplayText("Though a small part of you is horrified by your abnormal endowments, it doesn't stop you from mashing your " + Desc.Breast.describeAllBreasts(player) + " together");
    ...
    if (player.torso.chest.find(BreastRow.FuckableNipples) && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 3) {
        ...
        else if (player.torso.vaginas.reduce(Vagina.AverageLooseness, 0) < 4)
            DisplayText("puckered holes at the tip of each nipple. ");
        // add "many if lots of nips
        else if (player.torso.chest.reduce(BreastRow.AverageNipplesPerBreast, 0) > 1)
            DisplayText("many ");
        ...
        else if (player.torso.vaginas.reduce(Vagina.AverageWetness, 0) < 4)
            DisplayText("you begin to slowly spread the slippery secretions leaking from your engorged nipples all over your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + ". ");
        else {
            DisplayText("little spurts of ");
            // Lactating?
            if (player.torso.chest.reduce(BreastRow.AverageLactation, 0) > 0) DisplayText(" milky ");
            DisplayText("girlcum squirt out of your engorged nipples. ");
        }
        // Special extras for lactation
        if (player.torso.chest.reduce(BreastRow.AverageLactation, 0) > 0 && player.torso.chest.reduce(BreastRow.AverageLactation, 0) < 2)
            DisplayText("Your freakish mammaries drip a sticky combination of cunt-juice and milk, spattering your " + Desc.Leg.describeLegs(player) + " and crotch.");
        else if (player.torso.chest.reduce(BreastRow.AverageLactation, 0) < 3)
            DisplayText("Your freakish mammaries leak thin streams of sticky girlcum mixed with milk all over your " + Desc.Leg.describeLegs(player) + " and crotch.");
        else DisplayText("Your freakish mammaries constantly drool a steady stream of milky cunt-juice, drenching your " + Desc.Leg.describeLegs(player) + " and crotch in milky goo.");
    }
    // If nipples aren't fuckable.
        ...
        if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 1 && player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier < 2)
            DisplayText("Droplets of milk dribble from each nipple, spattering milk onto your " + Desc.Leg.describeLegs(player) + " and crotch.  ");
        else if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier < 3)
            DisplayText("Thin squirts of milk spray from each nipple, spattering milk onto your " + Desc.Leg.describeLegs(player) + " and crotch.  ");
        else if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 3) DisplayText("A constant stream of milk drizzles from each teat, soaking your " + Desc.Leg.describeLegs(player) + " and crotch.  ");
    }
}

function titFuckSingle(player: Character) {
    // different based on player.libido
    if (player.stats.lib < 45)
        DisplayText("You grip your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " and begin cautiously guiding it towards ");
    else if (player.stats.lib < 70) {
        ...
        DisplayText("tip of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " against the opening of ");
    }
    else {
        ...
        switch (player.torso.cocks.get(0).type) { // Different way of choosing cock to use, results in consistent description of cock[0]
            ...
            case CockType.TENTACLE:
                ...
                break;
            default:
                DisplayText(Desc.Cock.describeCockHead(player.torso.cocks.get(0)) + " of ");
        }
        DisplayText("your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " into ");
    }
    // More than one row?
    ...
    if (player.torso.chest.reduce(BreastRow.AverageNipplesPerBreast, 0) > 1) DisplayText("many ");
    ...
    if (player.torso.chest.reduce(BreastRow.AverageLactation, 0) === 0) {
        if (player.torso.vaginas.reduce(Vagina.AverageWetness, 0) < 2)
            DisplayText("Y");
        else if (player.torso.vaginas.reduce(Vagina.AverageWetness, 0) < 4) {
            ...
            DisplayText("your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " and y");
        }
        else DisplayText("Slick juices squirt out from around your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " and y");
    }
    else {
        if (player.torso.chest.reduce(BreastRow.AverageLactation, 0) < 2) {
            ...
            DisplayText("Milky girlcum squirts out from around your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", staining ");
            switch (player.torso.cocks.get(0).type) { // Different way of choosing cock to use, results in consistent description of cock[0]
                case CockType.ANEMONE:
    ...
    if (Math.round(player.torso.cocks.get(0).area) >= player.vaginalCapacity()) {
        ...
        if (player.stats.sens < 45)
            DisplayText("ou grunt with exertion as you attempt to stuff your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " into ");
        else if (player.stats.sens < 70)
            DisplayText("ou blink back tears as you attempt to stuff your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " into ");
        else DisplayText("ou scream with a combination of pain and pleasure as you attempt to stuff your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " into ");
        ...
        if (Math.round(player.torso.cocks.get(0).area) === player.vaginalCapacity()) {
            if (player.torso.vaginas.reduce(Vagina.AverageLooseness, 0) < 2)
                DisplayText("Your poor, tortured nipple is barely up to the task of accepting the " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", but accept it it does. ");
            else if (player.torso.vaginas.reduce(Vagina.AverageLooseness, 0) < 4)
                DisplayText("The engorged opening at the end of your swollen nipple is stretched to its limit as you shove your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " home. ");
            else {
                switch (player.torso.cocks.get(0).type) { // Different way of choosing cock to use, results in consistent description of cock[0]
                    case CockType.ANEMONE:
                        DisplayText("The swollen tips of your bloated nipple wrap around the stinging tentacles that surround your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + "'s tip, convulsing with wet squishing sounds as they become red and enflamed with artificial lust.  ");
                        break;
                    case CockType.CAT:
                        DisplayText("The swollen tips of your bloated nipples stretch around the barbed tip of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", swallowing it like an enormous mouth.  ");
                        break;
                    case CockType.DEMON:
                        DisplayText("The swollen lips of your bloated nipple stretch around the nodule-ringed tip of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", swallowing it like an enormous mouth.  ");
                        break;
                    case CockType.DISPLACER:
                        DisplayText("The swollen tips of your bloated nipple wrap around the outstretched head of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", convulsing with wet, squishing sounds as it wriggles inside you.  ");
                        break;
                    case CockType.DOG:
                    case CockType.FOX:
                        DisplayText("The swollen lips of your bloated nipple stretch around the pointed tip of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " swallowing it like an enormous mouth.  ");
                        break;
                    case CockType.HORSE:
                        DisplayText("The swollen lips of your bloated nipple stretch around the flared tip of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " swallowing it like an enormous mouth.  ");
                        break;
                    case CockType.HUMAN:
                        DisplayText("The swollen lips of your bloated nipple stretch around the tip of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " swallowing it like an enormous mouth.  ");
                        break;
                    case CockType.LIZARD:
                        DisplayText("The swollen tips of your bloated nipple wrap around the pointed tip of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", stretching oddly as it swallows the knot-covered appendage.  ");
                        break;
                    case CockType.TENTACLE:
                        DisplayText("The swollen lips of your bloated nipple stretch around the rounded tip of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", swallowing it like an enormous mouth.  ");
                        break;
                    default:
                        DisplayText("tips of your bloated nipples wrap around the " + Desc.Cock.describeCockHead(player.torso.cocks.get(0)) + " of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", swallowing it like an enormous mouth.  ");
                }
            }
            DisplayText("With each thrust, you bury your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " deeper into your greedy tit. Overwhelmed by the combined sensations, your mind is barely able to cope with the intense feeling of fullness where no such feeling should be possible.");
        }
        // if it doesnt really fit
        if (Math.round(player.torso.cocks.get(0).area) > player.vaginalCapacity()) {
            if (player.torso.vaginas.reduce(Vagina.AverageLooseness, 0) < 2)
                DisplayText("Your poor, tortured nipple is woefully insufficient compared to your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", delerious with arousal, you keep trying anyway. ");
            else if (player.torso.vaginas.reduce(Vagina.AverageLooseness, 0) < 4)
                DisplayText("The engorged opening at the end of your swollen nipple is stretched to its limits and beyond as you shove your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " home. ");
            else {
                switch (player.torso.cocks.get(0).type) { // Different way of choosing cock to use, results in consistent description of cock[0]
                    case CockType.ANEMONE:
                        DisplayText("The swollen lips of your bloated nipple gape wide, but the stinging tip of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " spreads them wider and fills them with artificial lust.  ");
                        break;
                    case CockType.CAT:
                        DisplayText("The swollen lips of your bloated nipple gape wide, but the barbed tip of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " spreads them even wider.  ");
                        break;
                    case CockType.DEMON:
                        DisplayText("The swollen lips of your bloated nipple gape wide, but the tip of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " spreads them even wider.  ");
                        break;
                    case CockType.DISPLACER:
                        DisplayText("The swollen lips of your bloated nipple gape wide, but the wide head of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " spreads them even wider.  ");
                        break;
                    case CockType.DOG:
                    case CockType.FOX:
                        DisplayText("The swollen lips of your bloated nipple gape wide, but the massive girth of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " spreads them even wider.  ");
                        break;
                    case CockType.HORSE:
                        DisplayText("The swollen lips of your bloated nipple gape wide, but the flared tip of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " spreads them even wider.  ");
                        break;
                    case CockType.HUMAN:
                        DisplayText("The swollen lips of your bloated nipple gape wide, but the tip of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " spreads them even wider.  ");
                        break;
                    case CockType.LIZARD:
                        DisplayText("The swollen lips of your bloated nipple gape wide, but the pointed tip of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " slowly spreads them even wider.  ");
                        break;
                    case CockType.TENTACLE:
                        DisplayText("The swollen lips of your bloated nipple gape wide, but the tip of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " spreads them even wider.  ");
                        break;
                    default:
                        DisplayText("The swollen lips of your bloated nipple gape wide, but the " + Desc.Cock.describeCockHead(player.torso.cocks.get(0)) + " of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " spreads them even wider.  ");
                }
            }
            DisplayText("Grunting and sweating with effort, you stuff as much of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " into your overstretched nipple as you can fit. The feeling of incredible tightness around your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " combines with the pain of your distended nipple to form a mindbending sensation that makes your head spin. ");
        }
        if (player.torso.chest.reduce(BreastRow.AverageLactation, 0) > 0)
            DisplayText("Milky ");
        else DisplayText("Slick ");
        if (player.torso.vaginas.reduce(Vagina.AverageWetness, 0) < 2) DisplayText("girl-lube quickly coats the whole length of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " in a glistening layer of fuck juice. ");
        else if (player.torso.vaginas.reduce(Vagina.AverageWetness, 0) < 4) {
            DisplayText("girl-lube drizzles down the length of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " in thick streams, ");
            switch (player.torso.cocks.get(0).type) { // Different way of choosing cock to use, results in consistent description of cock[0]
                case CockType.ANEMONE:
        ...
        else DisplayText("girl-lube spurts out of your tortured nipple with each thrust of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", spattering your arms and face with your secretions. ");
    }
    if (Math.round(player.torso.cocks.get(0).area) < player.vaginalCapacity()) {
        // Different noises based on sensitivity
        if (player.stats.sens < 45)
        ...
        DisplayText(" as your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " slides into ");
        // Different based on looseness
        if (player.torso.vaginas.reduce(Vagina.AverageLooseness, 0) < 2)
            DisplayText("the small, over-stretched opening of your swollen nipple. Your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " penetrates your swollen nipple easily, sliding halfway in on your first thrust. ");
        else if (player.torso.vaginas.reduce(Vagina.AverageLooseness, 0) < 4)
            DisplayText("the engorged and distended opening of your fat, swollen nipple. Your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " plunges deeply into your freakishly engorged nipple, penetrating it easily. ");
        else {
            ...
            DisplayText("tip of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " and begin to slide down its length, ");
            switch (player.torso.cocks.get(0).type) { // Different way of choosing cock to use, results in consistent description of cock[0]
                case CockType.ANEMONE:
        ...
        DisplayText("You revel in the sensation as you slowly stroke your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " in and out of your distended nipple. Your shaft is enveloped in the warm, wet embrace of your freakish tit, and ");
        if (player.torso.chest.reduce(BreastRow.AverageLactation, 0) > 0)
            DisplayText("milky ");
        else DisplayText("slippery ");
        if (player.torso.vaginas.reduce(Vagina.AverageWetness, 0) < 2)
            DisplayText("girl-lube quickly coats the whole length of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " in a glistening layer of fuck juice. ");
        else if (player.torso.vaginas.reduce(Vagina.AverageWetness, 0) < 4)
            DisplayText("girl-lube drizzles down the length of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " in thick streams, pooling at your crotch. ");
        else {
            if (player.torso.cocks.filter(Cock.FilterType(CockType.HUMAN)).length > 0)
                DisplayText("girl-lube pours out of your swollen nipple over your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " and hands, pooling on the ground below you. ");
            else if (player.torso.cocks.find(Cock.HasSheath))
                DisplayText("girl-lube drizzles down the length of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " in thick streams, pooling in and around your sheath. ");
                ...
                DisplayText("girl-lube pours out of your swollen nipple over your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " and hands, pooling on the ground below you. ");
            else DisplayText("girl-lube pours out of your swollen nipple over your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " and hands, pooling on the ground below you. ");
        }
    }
}

function multiTitFuck(player: Character): boolean {
    ...
    const randomCock: Cock = randomChoice(player.torso.cocks);
    multiNippleFuckPrep(player, randomCock);
    ...
    if (fittableCocks > 2 && holeTotal < fittableCocks) {
        ...
        if (player.vaginalCapacity() >= 2 * randomCock.area) {
            ...
            if (2 * holeTotal >= fittableCocks)
                DisplayText("You giggle with glee as you realize that you will be able to jam every single one of your " + Desc.Cock.describeMultiCockShort(player) + " into your abused nipples, thanks to double-penetration! ");
            else DisplayText("With a flash of irritation, you realize that even if you stick two cocks in each hole, you still won't be able to fit all " + numToCardinalText(player.torso.cocks.count) + " of your dicks into your abused tits. Deciding to make the best of it, you prepare to stuff in as many as you can. ");
        }
    ...
    // Round and compare cock thickness to vag looseness
    if (Math.round(randomCock.area) >= player.vaginalCapacity()) {
        // Different noises based on sensitivity
        if (player.stats.sens < 45)
    ...
    if (Math.round(randomCock.area) < player.vaginalCapacity()) {
        if (!doubleUp) {
            ...
            if (4 * Math.round(player.torso.cocks.reduce(Cock.AverageCockThickness, 0)) < player.torso.vaginas.reduce(Vagina.AverageLooseness, 0)) {
                if (player.torso.vaginas.reduce(Vagina.AverageLooseness, 0) < 2)
...
function multiNippleFuckPrep(player: Character, randomCock: Cock) {
    ...
    if (randomCock.type === CockType.HUMAN) {
        DisplayText("tip of ");
        // more than one?
        if (player.torso.cocks.filter(Cock.FilterType(CockType.HUMAN)).length > 1)
            DisplayText("one of your " + numToCardinalText(player.torso.cocks.filter(Cock.FilterType(CockType.HUMAN)).length) + " " + Desc.Cock.describeCock(player, randomCock) + "s ");
        else DisplayText("your " + Desc.Cock.describeCock(player, randomCock) + " ");
    }
    // Applying randomization - horse cocks
    if (randomCock.type === CockType.HORSE) {
        DisplayText("flared tip of ");
        // more than one?
        if (player.torso.cocks.filter(Cock.FilterType(CockType.HORSE)).length > 1)
            DisplayText("one of your " + numToCardinalText(player.torso.cocks.filter(Cock.FilterType(CockType.HORSE)).length) + " " + Desc.Cock.describeCock(player, randomCock) + "s ");
        else DisplayText("your " + Desc.Cock.describeCock(player, randomCock) + " ");
    }
    // Applying randomization - dog cocks
    if (randomCock.type === CockType.DOG) {
        DisplayText("pointed tip of ");
        // more than one?
        if (player.torso.cocks.filter(Cock.FilterType(CockType.DOG)).length > 1)
            DisplayText("one of your " + numToCardinalText(player.torso.cocks.filter(Cock.FilterType(CockType.DOG)).length) + " " + Desc.Cock.describeCock(player, randomCock) + "s ");
        else DisplayText("your " + Desc.Cock.describeCock(player, randomCock) + " ");
    }
    // Applying randomization - everything else
    if (randomCock.type >= CockType.DEMON) {
        DisplayText("tip of ");
        // more than one?
        if (player.torso.cocks.filter(Cock.FilterType(CockType.HUMAN)).length > 1)
            DisplayText("one of your " + numToCardinalText(player.torso.cocks.filter(Cock.FilterType(CockType.HUMAN)).length) + " " + Desc.Cock.describeCock(player, randomCock) + "s ");
...
// ORGASM COOOOAD
function orgazmo(player: Character, selfSucking: boolean, nippleFuck: boolean) {
    DisplayText("\n\n");
    if (player.torso.cocks.count > 0) {
        if (player.torso.cocks.count === 1) {
            ...
            if (player.torso.cocks.filter(Cock.FilterType(CockType.HUMAN)).length === 1)
                ...
                DisplayText("swelling, bulbous knot.  You feel it growing tighter and tighter until it's nearly twice the width of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  The pressure is an agonizing pleasure that only builds higher and higher as you come closer and closer to orgasm.  ");
            else if (player.torso.cocks.filter(Cock.FilterType(CockType.HORSE)).length === 1)
                DisplayText("swollen equine sheath, slowly beginning to work its way up your shaft.  Pre-cum begins pouring from your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", slicking your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " as you get ready to blow.  ");
            else if (player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE)).length === 1)
                DisplayText("wiggling vine-like cock, growing rapidly.  You feel the rubbery surface of your tentacle prick contracting as it nears release.  Thick bulges of fluids collect and travel along the length of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", the first of which is almost to your oversized tip.  ");
            else if (player.torso.cocks.filter(Cock.FilterType(CockType.DEMON)).length === 1)
                DisplayText("tainted man-meat, growing rapidly.  You feel the nodules close to the base swell and pulse, starting a rippling wave of pleasure that migrates upwards.  The ring of bumps around your crown double in size, flaring out as your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " prepares to unload.  ");
            else if (player.torso.cocks.filter(Cock.FilterType(CockType.CAT)).length === 1)
                DisplayText("full, cat-like sheath.  You feel it tingling and throbbing as the spines pulsate with arousal.  You feel the barbs at the bottom thickening as bulges of fluid migrate through your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " towards the tip.  ");
            else if (player.torso.cocks.filter(Cock.FilterType(CockType.LIZARD)).length === 1)
                DisplayText("swollen member.  You can feel it tingling and bulging strangly as it begins to contract and pulse.  Pre-cum leaks from your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " in a steady stream as each 'bulb' nearly doubles in size, and then one at a time, they deflate while your urethra dilates wide.  ");
            else if (player.torso.cocks.filter(Cock.FilterType(CockType.ANEMONE)).length === 1)
                DisplayText("base.  The tentacles surrounding your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " go nuts, constricting around it, inadvertently firing aphrodisiacs into the orgasmic flesh as they wring a potent, hip-jerking climax from you.  ");
            else DisplayText("twitching manhood, growing rapidly.  You stroke furiously, feeling the pressure of your cum as it nears release.  ");
            DisplayText("Pleasurable spasms overwhelm you as cum erupts from your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)));
            ...
            if (selfSucking) {
                if (player.cumQ() < 25)
                    ...
                    DisplayText("Pulse after pulse of cum erupts from your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " into your mouth.  You swallow what you can but it's too much for you.  Cum runs down your cock to pool on you as your orgasm drags on.  ");
                else DisplayText("Your orgasm never seems to end, and your world dissolves into the feelings from your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " as it erupts jet after jet of cum into your mouth.  You nearly gag, cum overflowing to spray out in a river, pooling around you.  ");
            }
            else if (nippleFuck) {
                if (player.cumQ() < 25)
                ...
                else DisplayText("Your orgasm never seems to end, and your world dissolves into the feelings from your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " as it erupts jet after jet of cum into your abused tit.  With a *pop*, your cock is forced free by the pressure. It continues to squirt semen everywhere, while a river of cum pours out of your dialated nipple, pooling around you.  ");
            }
            else {
                if (player.cumQ() < 25)
        ...
        if (player.torso.cocks.count > 1) {
            ...
            if (player.torso.cocks.reduce(Cock.MajorityType, undefined) === CockType.DOG) { // Primary Dog
                const dogCock = player.torso.cocks.find(Cock.FilterType(CockType.DOG));
                DisplayText("Your feel your knots bulging and swelling, growing tighter and tighter until they're nearly double the width of a " + Desc.Cock.describeCock(player, dogCock) + ".  The agonizing pressure builds higher and tighter with every passing second as you get closer and closer to orgasm.  ");
            }
            else if (player.torso.cocks.reduce(Cock.MajorityType, undefined) === CockType.HORSE) { // Primary Horse
                const horseCock = player.torso.cocks.find(Cock.FilterType(CockType.HORSE));
                DisplayText("You feel a pulsing in your sheath, slowly working its way up your " + Desc.Cock.describeCock(player, horseCock) + "s.  Pre-cum pours from your " + Desc.Cock.describeCock(player, horseCock) + "s, slicking the wobbly equine shafts as they get ready to blow.  ");
            }
            else { // Primary Normal
            ...
            if (nippleFuck) {
                ...
                else DisplayText("With an explosive release, you finally cum.  Your orgasm never seems to end, and your world dissolves into the feelings from your " + Desc.Cock.describeMultiCockShort(player) + " as they erupt jet after jet of cum into your abused tits.  With a series of wet *pops*, your cocks are forced free by the pressure. They continue to squirt semen everywhere, while rivers of cum pour out of your dilated nipples, pooling around you.  ");
            }

            // Doing the perverted sprinkler after dicks have popped out.
            DisplayText("Pleasure overwhelms your fragile mind as cum erupts from your " + Desc.Cock.describeMultiCockShort(player) + ".  Your hips jerk in the air in time with your eruptions, cum spraying from you as your body does its best impression of a perverted sprinkler.  ");
            if (selfSucking) {
                if (player.cumQ() < 25)
                    ...
                    DisplayText("Pulse after pulse of cum erupts from your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " into your mouth.  You swallow what you can but it's too much for you.  Cum runs down your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " to pool on you as your orgasm drags on.  Jizz rains over you the entire time from the rest of your \"equipment\".");
                else DisplayText("Your orgasm never seems to end, and your world dissolves into the feelings from your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " as it erupts jet after jet of cum into your mouth.  You nearly gag, cum overflowing to spray out in a river, pooling around you.  Your other 'equipment' rains jizz upon you the whole while, soaking you in a cum-puddle.");
            }
    ...
    if (player.torso.vaginas.count > 0) {
        ...
        if (player.torso.vaginas.count === 1) {
            ...
            // Wet orgasms
            if (player.torso.vaginas.get(0).wetness === VaginaWetness.SLAVERING) DisplayText("A veritable gush of fluid explodes from your nethers, pulsing in time with the ripples of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".  ");
        }
    ...
    if (player.torso.chest.reduce(BreastRow.AverageLactation, 0) >= 2 || player.torso.chest.find(BreastRow.FuckableNipples)) {
        // FUCK ANOTHER SHODDILY WRITTEN FUNCTION TO DEBUG/PORT
        // WHYYYYY
        titCum(player, player.cumQ());
    }
    // DONE!
    player.orgasm();
...
function titCum(player: Character, cumQuantity: number = 3) {
    ...
    if (player.torso.chest.reduce(BreastRow.AverageLactation, 0) >= 2 && !player.torso.chest.find(BreastRow.FuckableNipples)) {
        ...
        if (player.torso.chest.reduce(BreastRow.AverageLactation, 0) <= 2.6) DisplayText("thin streams of milk spray out of your ");
        if (player.torso.chest.reduce(BreastRow.AverageLactation, 0) > 2.6 && player.torso.chest.reduce(BreastRow.AverageLactation, 0) < 3) DisplayText("thick gouts of milk spurt out of your ");
        if (player.torso.chest.reduce(BreastRow.AverageLactation, 0) >= 3) DisplayText("huge torrents of milk blast out of your ");
        // different description based on size of nips
        if (player.torso.chest.reduce(BreastRow.AverageNippleLength, 0) <= 1) DisplayText("erect nipples, ");
        if (player.torso.chest.reduce(BreastRow.AverageNippleLength, 0) > 1 && player.torso.chest.reduce(BreastRow.AverageNippleLength, 0) < 4) DisplayText("fat, puckered nipples, ");
        if (player.torso.chest.reduce(BreastRow.AverageNippleLength, 0) >= 4) DisplayText("huge, swollen teats, ");
        if (player.torso.chest.reduce(BreastRow.AverageLactation, 0) <= 2.6) DisplayText("spattering milk everywhere. ");
        if (player.torso.chest.reduce(BreastRow.AverageLactation, 0) > 2.6 && player.torso.chest.reduce(BreastRow.AverageLactation, 0) < 3) DisplayText("covering everything nearby. ");
        if (player.torso.chest.reduce(BreastRow.AverageLactation, 0) >= 3) DisplayText("drenching the entire area.");
    }
    ...
    if (player.torso.chest.find(BreastRow.FuckableNipples)) {
        ...
        // Different description based on wetness
        if (player.torso.vaginas.reduce(Vagina.AverageWetness, 0) < 2) DisplayText("dribble ");
        if (player.torso.vaginas.reduce(Vagina.AverageWetness, 0) >= 2 && player.torso.vaginas.reduce(Vagina.AverageWetness, 0) < 4) DisplayText("spray ");
        if (player.torso.vaginas.reduce(Vagina.AverageWetness, 0) >= 4) DisplayText("gush ");
        // Lactating?
        if (player.torso.chest.reduce(BreastRow.AverageLactation, 0) > 0) DisplayText("milk and ");
        DisplayText("pussy juice everywhere.");
    }
}

// (D. Dildo) – a floppy pink dildo with aphrodisiac reservoir
function deluxeDildo(player: Character): NextScreenChoices {
    player.slimeFeed();
    DisplayText().clear();
    // [USE FEMALE]
    if (player.torso.vaginas.count > 0) {
        ...
        DisplayText("  A drop of pink aphrodisiac leaks from the tip, offering you a hint of the pleasure you're in for.   You make sure to catch it on your crotch, letting the fluid seep into your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".  Warmth radiates outwards, spreading to your thighs as your " + Desc.Vagina.describeClit(player));
        ...
        if (player.stats.cor < 50)
            DisplayText("before slowly working it inside you, gasping at the enhanced sensitivity of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".");
        else DisplayText("ramming it deep inside you, moaning as it rubs your now over-sensitive walls.");
        DisplayText("  You splay your " + Desc.Leg.describeLegs(player) + " and lie there with it inside you, feeling it respond to your wetness, becoming more and more turned on by the second.\n\n");

        // (Kinda dry)
        if (player.torso.vaginas.get(0).wetness < VaginaWetness.SLICK)
            DisplayText("The thickness of the toy gradually increases, filling you more and more effectively as it reacts to your bodily fluids.  You grab it two-handed and start slamming it into your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", vigorously fucking yourself with the swelling dong.  The sensations just keep getting better and better as more and more of the goblin's sex-drug leaks into you.  Even your " + Desc.Vagina.describeClit(player) + " and cunt-lips tingle with need.  You answer that need by picking up the pace, pistoning faster and faster.\n\n");
        // (Pretty wet)
        else if (player.torso.vaginas.get(0).wetness < VaginaWetness.SLAVERING) {
            DisplayText("The toy's girth seems to pulse and swell within you, spreading you wide open as it sops up your natural wetness and grows larger.  You grab it in a two-handed grip and begin working it in and out of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", gasping and twitching as every ridge and feature of the dildo rubs you just right.  Every inch of your nethers tingles with a desire to be touched, rubbed, and squeezed. ");
            if (player.torso.cocks.count > 0) {
                DisplayText("Even your " + Desc.Cock.describeMultiCock(player) + " ache");
        ...
        player.orgasm();
        DisplayText("\n");
        Mod.Vagina.displayStretchVagina(player, player.vaginalCapacity() * 0.9, true);
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
}

// onaHole use - game should already have checked if player has a cock! CHECK BEFORE CALLING
function onaholeUse(player: Character): NextScreenChoices {
    ...
    if (!player.statusAffects.has(StatusAffectType.PlainOnaholeUsed)) {
        ...
        if (player.torso.cocks.count > 1)
            DisplayText("With your free hand, you slap your " + Desc.Cock.describeMultiCockShort(player) + " to 'attention' and ease the onahole over your cocks.  ");
    ...
    // If player is already flagged, show repeated use text!
        ...
        onaholeRepeatUse(player, true);
    else onaholeRepeatUse(player, false);

    player.stats.sens += -.75;
    return onaholeContinuation(player);
}

function deluxeOnaholeUse(player: Character): NextScreenChoices {
    DisplayText().clear();
    // Flag after first use!
...
function onaholeContinuation(player: Character): NextScreenChoices {
    if (player.torso.cocks.count > 1) {
...
function onaholeMulticockContinuation(player: Character): NextScreenChoices {
    DisplayText("You pull the sloppy toy from your dribbling dick and smile, shoving its slippery surface down on another of your " + Desc.Cock.describeMultiCockShort(player) + ".  You rapidly work it around your cocks, orgasming until ");
    if (player.torso.balls.quantity > 0)
        DisplayText("you pass out with aching, empty balls.");
    else DisplayText("you pass out with " + Desc.Cock.describeMultiCockShort(player) + " sore from exertion.");
    player.stats.sens += -1;
    player.orgasm();
    return { next: Scenes.camp.returnToCampUseOneHour };
}

function onaholeFutaContinuation(player: Character): NextScreenChoices {
    ...
    DisplayText(Math.floor((((Math.random() * player.stats.str / 12) + player.stats.str / 6) * 10) / 10).toString());
...
function allNaturalOnaholeUse(player: Character): NextScreenChoices {
    DisplayText().clear();
    // First use!
...
function stimBeltUse(player: Character): NextScreenChoices {
    DisplayText().clear();
    // FIRST TIME USAGE
    if ((player.inventory.keyItems.has("Self-Stimulation Belt"))) {
        // First use! Flag after first use!
        Mod.Vagina.displayStretchVagina(player, 1, true, true, false);
        if (!player.statusAffects.has(StatusAffectType.used_self_dash_stim)) {
            ...
            DisplayText(Desc.Breast.breastCup(player.torso.chest.get(0).rating));
...
function allNaturalStimBeltUse(player: Character): NextScreenChoices {
    DisplayText().clear();
    if (player.inventory.keyItems.has("All-Natural Self-Stimulation Belt")) {
        // First time!
        if (!player.statusAffects.has(StatusAffectType.UsedNaturalSelfStim)) {
            ...
            Mod.Vagina.displayStretchVagina(player, 1, true);
            player.slimeFeed();
            return { next: allNaturalSelfStimulationBeltContinuation };
        }
        // Multiple uses
        else {
            // Low corruption variant
            ...
            // High corruption variant
                ...
                if (player.stats.cor >= 100) return { next: allNaturalSelfStimulationBeltBadEnd };
...
function allNaturalSelfStimulationBeltContinuation(player: Character): NextScreenChoices {
    DisplayText().clear();
...
function allNaturalSelfStimulationBeltBadEnd(): NextScreenChoices {
...
function lickYerGirlParts(player: Character): NextScreenChoices { // Female cat masturbation
    DisplayText().clear();
    if (!player.perks.has(PerkType.Flexibility)) {
        DisplayText("You undress from your " + player.inventory.equipment.armor.displayName + " and take a seat down on the ground. You spread your legs and look down at your sex. It's aching for something more than just your fingers, and you have a craving to taste the lustful juices leaking out. A very perverted idea of cats flashes through your brain, putting a naughty smile on your face. You lay on your side and spread your legs, giving you a perfect view of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " You lean your head down towards the pleasure-hole, only to be stopped half-way there. You stick your tongue out, trying to add a few more inches, but this doesn't do anything except increase your appetite and your lust as a drop of warm saliva falls onto your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ". You stretch and wriggle your tongue out in a fruitless effort to taste your dripping wet cunt, craving the feeling of your tongue caressing your lips and penetrating into your depths... but it is not to be. You sit back up, frustrated and even more aroused than you were before.");
        player.stats.lust += 15;
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
    // [1st time doing this]
    if (User.flags.get<PlayerFlags>(CharacterType.Player).TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY === 0) {
        User.flags.get<PlayerFlags>(CharacterType.Player).TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY++;
        DisplayText("You take off your " + player.inventory.equipment.armor.displayName + " and take a seat on the ground. You spread your legs and look down at your sex. It's aching for something more than just your fingers, and you have a craving to taste the lustful juices leaking out. A very perverted idea flashes through your brain, putting a smile on your face. You lay on your side and spread your legs again, giving you a perfect view of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ". You lean your head down with your tongue sticking out; closer and closer you come to your own cunt, feeling the heat from your puss flowing against your face as your own hot breath returns the warmth. You're only a small distance away from tasting it before you can't bend any farther.  Your cunny can almost feel your tongue wriggling its slimy warm wetness only a few centimeters away. You pull your head back and let out a frustrated sigh before you remember how the cats in your village got to those hard to reach places: they stretched one of their legs straight up. Following their example, you point one leg straight to the sky and close your eyes as you plunge your head down. You slowly open one eye to see that you're face to face with your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + "; you're amazed that you are actually able to do it. You begin lapping your tongue up and down the slutty snatch.\n\n");

        DisplayText("The feeling is amazing, as you flick your tongue across your swollen " + Desc.Vagina.describeClit(player) + ". Juices leak from your moist hole, filling your mouth with the sweet taste of girlcum. You can feel your entire sex pulsing and throbbing around your tongue as you plumb the depths of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".  The vibrations from your moans and screams of pleasure cause the intense feelings to increase, making you a slave to your own needs. Your juices flow down your chin as you try to swallow every last drop. You crane your neck, giving you deeper access to your wanting honey pot. You can feel yourself tightening around your tongue as it rams into your pussy, soaking up the juices as you slowly reach your peak.  You lick and suck hard around your " + Desc.Vagina.describeClit(player) + ", using both your hands to spread your cunt farther open. You move your head in an up and down motion just like a cat when they groom themselves. Your lapping and tongue play continues until you can feel your body tense up, ready to cum.\n\n");
...
function catAutoLick(player: Character): NextScreenChoices { // Male cat masturbation
    DisplayText().clear();
    // NOT FEXIBLE
    if (!player.perks.has(PerkType.Flexibility)) {
        // Fails [Herm has a 50/50 chance of getting either.]
        // [Male]
        DisplayText("You undress from your " + player.inventory.equipment.armor.displayName + " and take a seat down on the ground, your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " pointing straight at your face. You stroke the erect member a few times, but then remember the cats back at the village. You stare at your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + "; the more you look at the cock, the more your mouth craves to suck on it. You open your mouth as wide as you can and lean towards your cock, only to be stopped halfway to the tip. You stick your tongue out and try to lick the head. You pretend you're rolling your tongue around the head, but this only makes your cock harder in eagerness. You throw your head forward in an attempt to flick your tongue against it, but the " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " is pulled back as you go forward. You slump your back onto the ground and let out a frustrated groan. The only thing you've managed to do is make yourself more aroused than when you started.");
        player.stats.lust += 15;
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
    // 1st time
    ...
    // [Repeatable]
        ...
        if (player.inventory.equipment.cockSocks.find(EquipSlot.FilterName(CockSockName.Gilded)) && User.flags.get<PlayerFlags>(CharacterType.Player).GILDED_JERKED < player.inventory.equipment.cockSocks.filter(EquipSlot.FilterName(CockSockName.Gilded)).length) {
            ...
            player.inventory.gems += gemsCreated;
        }
        else {
            if (User.flags.get<PlayerFlags>(CharacterType.Player).TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY > 10 && player.torso.balls.quantity > 1 && randInt(5) === 0) {
                DisplayText("But it's not enough and you are forced to come up for air.");

                DisplayText("\n\nWhile gasping for air, you scowl at your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " in disapproval. That's when your " + Desc.Balls.describeSack(player) + " catches your attention. It's gleaming with your sex sheen and you watch one of your balls slowly slide off to one side.");

                DisplayText("\n\nYou think you can make it! You throw your other leg over your head and both feet come to rest on your back. You push your mouth towards your " + Desc.Balls.describeSack(player) + ", slowly walking your toes down your back. You are only an inch from your " + Desc.Balls.describeBalls(true, true, player) + " now and your own aroma fills your nostrils, spurring your on. Then, finally, you are there! And it nearly knocks you out! The sensation of your own balls in your mouth is incredible--you can feel them churning in your mouth--and your [feet] start kneeding your back. You bring your hands up to massage your " + Desc.Balls.describeBalls(true, true, player) + " as well, making them take turns in your mouth. Completely intoxicated by your own scent you loose all track of time--there is only the bliss of sucking, licking, and massaging your own balls...");

                DisplayText("\n\nAbrubtly you realize that your " + Desc.Cock.describeMultiCockSimpleOne(player) + " have soaked your torso in precum--apparently you've been on the edge for some time. In one swift motion, you pick your head up and slam your mouth down over your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " leaving your hands to continue their ball massage.");

                // add cum quantity conditional text...
                DisplayText("\n\nThe release is immediate. You feel the contractions of your climax against your [face], your [feet] involuntarly start massaging your back, and you feel your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " expand and contract in your mouth as seed pumps through it into your stomach. It's more relaxing than anything else--each contraction makes you feel like you might be melting a little, like you might remain in this position forever.");

                DisplayText("\n\nYour [feet] and hands are still masagging their respective charges when you realize you are starting to go flaccid! You make an attempt to massage every last drop of cum from your " + Desc.Balls.describeBalls(true, true, player) + ", sucking on your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " continuously as it goes down. It slowly slips from your mouth once it's completely deflated, causing you to frown slightly. Still feeling great in this position, you go down on your " + Desc.Balls.describeSack(player) + " again--alternating between licking and sucking them with your mouth and massaging them with your hands.");
...
function meditate(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    player.stats.lust -= 50;
    player.stats.cor += -.3 - 0.3 * player.inventory.equipment.cockSocks.filter(EquipSlot.FilterName(CockSockName.Alabaster)).length;
    if (player.perks.has(PerkType.Enlightened) && player.stats.cor < 10) Mod.Stat.displayCharacterHPChange(player, 50);
    return { next: Scenes.camp.returnToCampUseTwoHours };
}

function dualBeltMasturbation(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplayText("You look at the thing in front of you and sigh, your " + Desc.Cock.describeMultiCockShort(player) + " and " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " dripping with fluids. With a nervous sigh you step into the underwear looking device and slip it up your legs, the cold metal feeling so good on your " + player.skin.desc + ", sending a rush of feelings up your body. You slip your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " down and into a hole on the front of the belt, the extra length sticking out on the other side of it. Underneath the hole and right above your pussy is another metal square with what feels like a small nub on the inside rubbing against your clit. Shivering from the feeling of it, you stay there for a moment, waiting in anticipation. Finally, you reach down to the side of the belt and flick the switch to the on position.\n\n");
    DisplayText("The belt whirs to life, shaking on your waist, sending jolts of pleasure through your clit as the small inside nub hits it. \"<i>Ohh...</i>\" Suddenly, the ring around your cock vibrates and then tightens hard around your cock, the belt sinking onto your body and locking in place. Worry sets in instantly as you try to wiggle and take it off, but it is no use. You see something black bubble from the edges of the metal, worried even more that it might be some sort of acid. It begins to creep across your skin at a disturbing rate, going down your " + Desc.Leg.describeLegs(player) + " and encasing them in the blackness, wrapping your cock, ");
    if (player.torso.tails.count > 0) DisplayText("covering up your tail, ");
    DisplayText("and then going up your body, covering your " + Desc.Breast.describeAllBreasts(player) + " and neck. The only part of your body unclad by the suit is your head. The blackness feels slick and smooth, almost cold, a strange type of feeling washes over you until you realize that it is a rubber suit.\n\n");
    DisplayText("Before you can do anything else, the belt activates again and the latex covering of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " begins to tighten and pulse around the meat, warming up to feel like a virgin cunt. A moan is dragged from your lips as it begins to ripple and pulse, simulating the feeling of fucking a tight hole as the entire suit molds itself to your body. Before you can get too used to the feeling of the suit milking your cock, the nub that had been teasing your clit suddenly expands and pushes out, the slick feeling of the latex pushing into your pussy.  The hardened black latex splits your tunnel and spreads you wide as it goes in deep. Your eyes widen for a moment as both stop, and then your world explodes in a flash of pleasure. The hardened lump begins to piston in and out of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", vibrating wildly as a lump grows in on top in precisely the right spot to rub back and forth on your g-spot.\n\n");
    DisplayText("Meanwhile the latex around your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " begins to pulse and ripple faster than ever before. You quake and quiver, " + Desc.Leg.describeLegs(player) + " giving out as it teases and pulses around your " + Desc.Breast.describeAllBreasts(player) + ". Your hands go down your body helplessly and start stroking at your encased cock, rubbing up and down your length. Unfortunately, all things must come to an end as the pleasure gets to be way too much and you feel yourself cum. Your hips buck wildly as you feel cum spurt into the latex, the end swelling up and filling like a ");
    ...
    // ([If high lactation]
    if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 2) DisplayText("Milk gushes out from your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + " as you orgasm, filling the inside of the suit with a slick layer of milk and forming milk bubbles that hang lewdly off your chest. ");
    ...
    DisplayText("When you wake, the black latex is no longer covering your body and the belt is silent around your waist. Cum drips from the tip of your cock and the top part of your " + Desc.Leg.describeLegs(player) + " are coated with your feminine juices. ");
    if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 2) DisplayText("Thin streams of creamy milk flow from your " + Desc.Breast.describeAllBreasts(player) + ", your torso and midsection dripping wet from the stuff. ");
...
function centaurMasturbation(player: Character): boolean {
    DisplayText().clear();
    const canMasturbateHugeCock: boolean = player.torso.cocks.count > 0 && (player.tallness * (5 / 6) < player.torso.cocks.sort(Cock.LongestCocks)[0].length);
    if (player.torso.chest.find(BreastRow.FuckableNipples)) {
        if (canMasturbateHugeCock && randInt(2) === 0) { // 50/50 chance of either if you can do both
            centaurHugeCock(player);
            return true;
        }
        centaurNippleCunt(player);
        return true;
    }
    ...
    centaurCantMasturbate(player); // Failsafe - you suck.
    return false;
}

function centaurNippleCunt(player: Character) {
    DisplayText("You shrug out of your " + player.inventory.equipment.armor.displayName + ", ");
    if (User.flags.get<PlayerFlags>(CharacterType.Player).PC_FETISH > 0)
        DisplayText("panting lustily as you envision being caught masturbating your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s.");
    else if (player.stats.cor < 33)
        DisplayText("blushing a bit as you look down at your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s.");
    else if (player.stats.cor < 66)
        DisplayText("shivering as the air hits your exposed " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s.");
    else DisplayText("smiling to yourself as you blatantly oogle your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s.");
    ...
    if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 2)
        DisplayText("  The sudden displacement makes your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + " jiggle enticingly, and the sudden movement fires arcs of pleasure deep into your body.");
    ...
    DisplayText("  Your free fingers slowly stroke around the outer edges of your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s' lips, pausing when you feel a tiny sensitive clit-like nub just inside the top of the opening.\n\n");

    DisplayText("Shivering with pleasure, you kneel down to prevent your legs from going out from underneath you.   Your " + Desc.Face.describeFace(player) + " flushes hotter as you toy with the slippery nipple-cunts, feeling them growing hard and puffy as you turn yourself on more and more.  ");
    ...
    if (player.torso.vaginas.count > 0 || player.torso.cocks.count > 0) {
        ...
        if (player.torso.cocks.count > 0) {
            if (player.torso.vaginas.count > 0)
                DisplayText(Desc.Cock.describeMultiCockShort(player) + " and " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)));
        ...
        if (player.torso.cocks.count > 1)
            DisplayText("  You feel each of your " + Desc.Cock.describeMultiCockShort(player) + " tapping against your belly as your muscles clench, getting as much pleasure as possible from your male endowments.");
        else if (player.torso.cocks.count === 1)
            DisplayText("  You feel your " + Desc.Cock.describeMultiCockShort(player) + " tapping against your belly as your muscles clench, getting as much pleasure as possible from your male endowment.");
        if (player.torso.balls.quantity > 0) DisplayText("  Rocking slightly inside your sack, your " + Desc.Balls.describeBalls(true, true, player) + " repeatedly clench up against your body, ready to release.");
        DisplayText("\n\n");
    }
    // (ORGAZMO)
    DisplayText("A slow wave of contractions starts deep inside each of your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s.  It intensifies as it rises towards the surface of your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + ", reaching a crescendo that brings you to the peak of pleasure.  Your eyes roll back as you slump down in orgasmic bliss, fingers pumping relentlessly at you slippery nipple-holes.  Noisy wet squelches and incessant moaning rouse you from your pleasure-induced coma, eventually waking you back to reality.\n\n");
    ...
    if (player.torso.cocks.count > 0) {
        ...
        DisplayText("released from your " + Desc.Cock.describeMultiCockShort(player) + " by the intense climax.");
    ...
    // Real aftermath
    DisplayText("Judging by the sky, at least an hour has passed.  You sigh and pry your cramped fingers from your aching " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s, rubbing the sore entrances with your palm before you climb back up onto your " + Desc.Leg.describeFeet(player) + ".  As you get dressed, you're very conscious of how much better you feel from the wonderful finger-fuck.  ");
    if (player.stats.cor > 66)
...
function centaurHugeCock(player: Character) {
    // Set plurality and primary cock.
    const longestCock = player.torso.cocks.sort(Cock.LongestCocks)[0];
    const plural: boolean = (player.torso.cocks.count > 1);
    DisplayText("You shrug out of your " + player.inventory.equipment.armor.displayName + ", not that it does much to impede your " + Desc.Cock.describeMultiCockShort(player) + " as a centaur.   ");
    ...
    DisplayText("your " + Desc.Cock.describeMultiCockShort(player) + " swells out underneath you, hanging down from your equine hindquarters as ");
    if (!plural) DisplayText("it grows full and hard.");
    else DisplayText("they grow full and hard.");
    if (plural)
        DisplayText("  After a moment they peek out from under your forelegs, proudly displaying your " + Desc.Cock.describeMultiCockShort(player) + ".");
    else {
        DisplayText("  After a moment it peeks out from under your forelegs, proudly displaying your ");
        if (longestCock.type === CockType.HORSE) DisplayText("flared tip");
        else if (longestCock.type === CockType.DEMON) DisplayText("nodule-ringed head");
        else if (longestCock.type === CockType.DOG) DisplayText("pointed erection");
        else if (longestCock.type === CockType.TENTACLE) DisplayText("wriggling, mushroom-like tip");
        else if (longestCock.type === CockType.CAT) DisplayText("spiny head");
        else if (longestCock.type === CockType.LIZARD) DisplayText("pointed, purple tip");
        else if (longestCock.type === CockType.ANEMONE) DisplayText("tentacle-ringed head");
        else DisplayText("dome-like head");
        DisplayText(".");
    }
    if (player.tallness * 1.5 < longestCock.length) {
    ...
    if (player.tallness * 1.5 >= longestCock.length) {
        ...
        DisplayText(Desc.Cock.describeCock(player, longestCock) + " and lift up, pressing it tightly against your belly as you begin to stroke along the sensitive underside.  ");
        if (player.cumQ() > 500)
        ...
        if (player.torso.vaginas.count > 0) DisplayText("  Your neglected " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " aches for penetration, but there's no way you'll be able to reach it in your current state.");
        ...
        if (player.torso.cocks.count === 2)
            ...
            DisplayText("  Your other " + Desc.Cock.describeMultiCockShort(player) + " mimic their lucky brother's pleasure, even though they've been ignored in order for you to focus on your current 'toy'.");
    ...
    // Get started (long)
        ...
        DisplayText(Desc.Cock.describeCock(player, longestCock) + " and lift it up, curving it slightly as you begin to stroke the underside with long, fluid caresses.  You even manage to bend down and give it a lick, forcing you into a pleasurable shudder.");
        if (player.cumQ() > 500)
            DisplayText("  Thick streams of pre-cum start to drizzle from your tip, forced out due to your prodigious production of sexual fluid.");
        else if (player.cumQ() > 100)
            DisplayText("  A dribble of pre-cum starts to leak from the tip as you get more into the act.");
        ...
        // STROKE (long)
        DisplayText("You smear your pre-cum over the tip, rolling your palm across the sensitive tip in a way that makes it hard to stand.  Meanwhile your other hand is making great use of your incredible length, jacking you off with long fluid strokes.  In no time flat, you can feel your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " pulsing in your grip, growing harder in time with your fevered stroking.");
        if (player.torso.cocks.count === 2)
            DisplayText("  Your other dick mimics its lucky brother's pleasure, even though it's been ignored in order for you to focus on your current 'toy'.");
        else if (player.torso.cocks.count > 2)
            DisplayText("  Your other " + Desc.Cock.describeMultiCockShort(player) + " mimic their lucky brother's pleasure, even though they've been ignored in order for you to focus on your current 'toy'.");
    ...
    // Cumjaculation
    if (player.tallness * 1.5 >= longestCock.length)
        DisplayText("Squeezing tightly around your " + Desc.Cock.describeCockHead(player.torso.cocks.get(0)));
    else DisplayText("Pumping hard");
    DisplayText(", you orgasm, feeling relief flow through you with every pump of ");
    if (player.torso.balls.quantity > 0)
        DisplayText("your " + Desc.Balls.describeBalls(true, true, player));
    ...
    else if (player.stats.minLust() > 40 || player.stats.lib > 50) {
        if (plural)
    ...
    if (player.torso.vaginas.count > 0) {
        ...
        if (player.torso.chest.find(BreastRow.FuckableNipples)) {
            ...
            DisplayText(" of your " + numToCardinalText(player.torso.chest.countNipples()) + " " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s quiver, ");
...
function centaurCantMasturbate(player: Character) {
    if (User.flags.get<PlayerFlags>(CharacterType.Player).UNABLE_TO_MASTURBATE_BECAUSE_CENTAUR === 0) {
        ...
        User.flags.get<PlayerFlags>(CharacterType.Player).UNABLE_TO_MASTURBATE_BECAUSE_CENTAUR++;
    }
    else {
        DisplayText("You still can't reach around to masturbate yourself.  Being half-horse sure is inconvenient!\n\n");
    }
    // (If Milker)
    if (player.inventory.keyItems.has("Cock Milker - Installed At Whitney's Farm") && player.torso.cocks.count > 0)
...
function centaurDudesGetHorseAids(player: Character): NextScreenChoices {
    const cock = player.torso.cocks.sort(Cock.LargestCockArea)[0];
    DisplayText().clear();
    if (player.inventory.keyItems.has("Fake Mare") && player.inventory.keyItems.get("Fake Mare").value1 === 0) {
        if (player.stats.cor < 50)
    ...
    DisplayText("Seeing the toy's exposed, gaping genitals, you feel a stirring in your " + Desc.Cock.describeMultiCockShort(player) + ".  You yearn to touch yourself, but your centaur lower body prevents you, as usual.  Grunting with annoyance, you trot up to the toy and give its wide cunt an experimental fisting.  ");
    // [If small cock:
    if (cock.area < 30)
    ...
    DisplayText("Satisfied the toy is ready for you, you clop back a few paces and surge forward.  You mount the toy easily, your belly running across its smooth, warm back until your chest bumps against the mare's head.  You grip her shoulders for support and start to buck your hips, your " + Desc.Cock.describeCock(player, cock) + " poking around in search of entrance.  Finally, you feel the tip of your prick ");
    if (cock.area < 30)
        DisplayText("pressing against the tight ring of the toy's anus");
    else DisplayText("lining up with the toy's gaping cunt");
    DisplayText(".  You rear your " + Desc.Hip.describeHips(player) + " and slam yourself into the mare's waiting hole.\n\n");

    DisplayText("The toy's passage seems to shift and contract around your " + Desc.Cock.describeCock(player, cock) + ", molding itself to perfectly sheathe you.  What a marvelous little toy!  You slide on up until you hilt yourself, your crotch pressed against the mare's wide ass as your " + Desc.Breast.describeChest(player) + " squeezes against her back.  Now fully mounted, you begin to rut on the mare toy, slapping your hips");
    if (player.torso.balls.quantity > 0)
        DisplayText(" and " + Desc.Balls.describeBalls(true, true, player));
    ...
    DisplayText("Finally able to get the release you weren't able to give yourself, you feel an orgasm mounting deep within you.  You let out a loud, equine whinny as you hump the toy faster and faster, pounding her with your " + Desc.Cock.describeCock(player, cock) + " until the pleasure overwhelms you.  You cry out as you cum, squirting your centaur-cum as far into the toy as you can shoot it");
    ...
    if (player.inventory.equipment.cockSocks.find(EquipSlot.FilterName(CockSockName.Gilded)) && User.flags.get<PlayerFlags>(CharacterType.Player).GILDED_JERKED < player.inventory.equipment.cockSocks.filter(EquipSlot.FilterName(CockSockName.Gilded)).length) {
        ...
        DisplayText(".\n\nSated, you spend a few blissful minutes enjoying the warmth and tightness of the mare-like onahole until your " + Desc.Cock.describeCock(player, cock) + " is soft inside it.  You scamper off of her, dropping back to your four equine feet.  <b>You are startled to see glittering gems pouring from her ");
        if (cock.area >= 30)
            DisplayText("horsecunt");
        else DisplayText("asshole");
        ...
        DisplayText("Sated, you spend a few blissful minutes enjoying the warmth and tightness of the mare-like onahole until your " + Desc.Cock.describeCock(player, cock) + " is soft inside it.  You scamper off of her, dropping back to your four equine feet.  With a contented yawn, you disassemble the toy and haul it off back to your stash, leaking your cum the entire way.");
    }

    player.orgasm();
    player.stats.sens += -2;
    player.inventory.keyItems.get("Fake Mare").value1 = 1;
    return { next: Scenes.camp.returnToCampUseOneHour };
}

// [Masturbate] -- [CentaurPole] -- [Fem/Herm Centaurs]
function centaurGirlsGetHorseAids(player: Character): NextScreenChoices {
    DisplayText().clear();
    if (player.inventory.keyItems.has("Centaur Pole") && player.inventory.keyItems.get("Centaur Pole").value1 === 0) {
        // [If low Corruption:]
        if (player.stats.cor < 50)
...
function getHugeEggsInCawk(player: Character): NextScreenChoices {
    DisplayText().clear();
    // Bee Eggs in Huge Cock + Exgartuan: Finished (Slywyn)(edited)
    if (player.statusAffects.get(StatusAffectType.Exgartuan).value1 === 1 && player.statusAffects.get(StatusAffectType.Exgartuan).value2 === 0 && User.flags.get<PlayerFlags>(CharacterType.Player).TIMES_EGGED_IN_COCK === 0) {
        ...
        if (player.stats.cor < 33) {
            ...
            if (Scenes.camp.hasCompanions()) DisplayText(", hoping one of your companions sees");
        }
        else if (player.stats.cor < 66)
        ...
        if (Scenes.shouldraFollower.followerShouldra()) DisplayText(", with Shouldra's assistance,");
        ...
        if (player.torso.balls.quantity > 0) {
            ...
            if (player.pregnancy.ovipositor.fertilizedEggs > 0) DisplayText(", and you know beyond a doubt that they'll be fertilized before much longer and you'll be able to lay your own eggs");
            DisplayText(".");
        }
        // [If no balls:]
        else {
    ...
    if (player.pregnancy.ovipositor.fertilizedEggs > 0 && User.flags.get<PlayerFlags>(CharacterType.Player).DICK_EGG_INCUBATION === 0) {
        User.flags.get<PlayerFlags>(CharacterType.Player).DICK_EGG_INCUBATION = 48;
    }
    player.pregnancy.ovipositor.dumpEggs();
    player.orgasm();
    User.flags.get<PlayerFlags>(CharacterType.Player).TIMES_EGGED_IN_COCK++;
    return { next: Scenes.camp.returnToCampUseOneHour };
}

// Birth Bee Eggs Out Your Wang: Finished (Slywyn)(edited)
export function birthBeeEggsOutYourWang(player: Character) {
    ...
    User.flags.get<PlayerFlags>(CharacterType.Player).DICK_EGG_INCUBATION = 0;
    player.orgasm();
}
...
function layEggsInYerTits(player: Character): NextScreenChoices {
    DisplayText().clear();
    if (player.statusAffects.get(StatusAffectType.Exgartuan).value1 === 2 && player.statusAffects.get(StatusAffectType.Exgartuan).value2 === 0) {
        ...
        if (player.pregnancy.ovipositor.eggs < 20) {
        ...
        else if (player.pregnancy.ovipositor.eggs < 40) {
    ...
    if (!player.statusAffects.has(StatusAffectType.Eggchest)) {
        player.statusAffects.add(StatusAffectType.Eggchest, 3 + randInt(10), 1 + randInt(4), 0, 0);
...
function tentacleSelfFuck(player: Character): NextScreenChoices {
    const tentCock = player.torso.cocks.find(Cock.FilterType(CockType.TENTACLE));
    // Pick a second dick that isn't the first.
    const otherCock: Cock = randomChoice(player.torso.cocks.filter((a: Cock) => a !== tentCock));
    DisplayText().clear();
    ...
    DisplayText("Almost immediately, your " + Desc.Cock.describeCock(player, tentCock) + " perks up like a pet expecting to be fed, and you have to admit that you plan to give that squirming tentacle exactly what it desires - a hot, slippery slit to nestle inside of.  Already, your [vagina] has grown ");
    if (player.torso.vaginas.get(0).wetness <= 2)
        DisplayText("moist");
    else if (player.torso.vaginas.get(0).wetness <= 3)
        DisplayText("wet");
    ...
    if (otherCock) DisplayText("  It loops around your " + Desc.Cock.describeCock(player, otherCock) + " while you fight with it, strangling the other phallus in tight coils of squeezing, floral friction.");
    ...
    if (otherCock) {
        ...
        if (otherCock.type === CockType.TENTACLE) {
            ...
            Mod.Butt.displayStretchButt(player, otherCock.area, true, true, false);
        }
    }
    ...
    if (player.torso.chest.find(BreastRow.FuckableNipples)) DisplayText(", even stuffing a finger inside a sloppy, dripping nipplecunt");
    DisplayText(".");
    ...
    // No bonus cawks!
        ...
        if (otherCock && otherCock.type === CockType.TENTACLE && otherCock.length >= 20) {
    ...
    // One more:
        ...
        DisplayText("\n\nThe last green tool rises up above you.  It surveys the view before it in a crude approximation of a sentient surveyor, seeming to take in the shifting, squiggling view of your ardent masturbation.  In truth, you're just trying to think of where to stick it.  A droplet of liquid lust leaks from the tip in sympathetic pleasure, and it gives you an idea for just what to do with it.  It stretches out towards your torso but veers left at the last second.  Dripping onto your shoulder, the pulsating plant-prick slowly pushes into your armpit, lubricating your " + Desc.Skin.skinFurScales(player) + " with its amorous liquid.  You're fucking your armpit and it feels divine, so good that you quickly slick your pit and twist the spare cock around yourself so that it can double-fuck both sides.");
    }
    ...
    DisplayText(".");
    if (player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE)).length === 2) {
        DisplayText("  Your untended " + Desc.Cock.describeCock(player, otherCock) + " blows spunk over you ");
        if (player.cumQ() < 50)
    ...
    player.orgasm();
    player.stats.sens += -1 * (1 + player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE)).length);
    return { next: Scenes.camp.returnToCampUseOneHour };
}
...
function tentacleGoesUpYerPooperNewsAtEleven(player: Character): NextScreenChoices {
    const tentacle = player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE)).sort(Cock.LargestCockArea)[0];
    DisplayText().clear();
    ...
    DisplayText("  You groan and lazily remove your " + Desc.Cock.describeCock(player, tentacle) + " from your anus as you give in to your pleasure-induced drowsiness.");
    if (player.torso.cocks.count > 0) {
        // Single Cock
        if (player.torso.cocks.count === 1) {
            if (player.stats.lib < 30)
                ...
                DisplayText("  You roll and begin to doze, your semi-erect " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " flopping against you. ");
            else if (player.stats.lib <= 80) {
                DisplayText("  As you close your eyes and relax, your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " surges back to erectness, ensuring ");
                if (player.stats.cor < 50)
                    DisplayText("your dreams will be filled with sex.");
                else DisplayText("you dream in a depraved kinky fantasia.");
            }
            else DisplayText("  You groan and drift to sleep, your rigid " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " pulsing and throbbing with continual lust.");
        }
        // Multi Cock
        if (player.torso.cocks.count > 1) {
            if (player.stats.lib < 30)
...
function gildedCockTurbate(player: Character): NextScreenChoices {
    DisplayText().clear();
    const gildedCock = player.inventory.equipment.cockSocks.map((equipSlot, index, array) => {
        if (EquipSlot.FilterName(CockSockName.Gilded)(equipSlot, index, array)) {
            return player.torso.cocks.get(index);
        }
    }).sort(Cock.LargestCockArea)[0];
    ...
    DisplayText("\n\nPressure builds, and tremors of pleasure run through your body as you stroke faster and faster.  You let out a small moan, as you pump the cocksleeve up and down your " + Desc.Cock.describeCock(player, gildedCock) + ".  Your cock swells within the confines of the sleeve, throbbing against the metallic confines of the sheath.  You shudder, arching your back, as you climax, your " + Math.round(player.cumQ()) + "mLs of cum arcing up high into the air overhead.");
    ...
    DisplayText(" gems falls down upon your body instead of cum, bouncing off your " + Desc.Skin.skinFurScales(player) + ".");

    const gems: number = midasCockJackingGemsRoll(player);

    DisplayText("\n\n<b>You have just enough wherewithal to gather up the spent " + numToCardinalText(gems) + " gems before falling asleep for a quick nap.</b>");
    player.orgasm();
    player.stats.sens += -1;
    player.inventory.gems += gems;
    User.flags.get<PlayerFlags>(CharacterType.Player).GILDED_JERKED++;
...
function stickADildoInYourVagooSlut(player: Character): NextScreenChoices {
    DisplayText().clear();
    if (player.torso.vaginas.find(Vagina.Virgin)) { // LOW CORRUPTION DEFLOWER
        if (player.stats.cor <= 50) {
...
function dildoButts(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    if (User.settings.silly()) DisplayText(", the alliteration of the experience further arousing you");
    ...
    if (player.torso.cocks.count > 0 && player.torso.vaginas.count > 0)
        ...
        DisplayText("\n\nYou moan in ecstasy while cum spurts from your " + Desc.Cock.describeMultiCockShort(player) + ". ");
...
function midasCockJackingGemsRoll(player: Character): number {
    const cockSocks: number = player.inventory.equipment.cockSocks.filter(EquipSlot.FilterName(CockSockName.Gilded)).length;
...
function onaholeRepeatUse(player: Character, corrupted: boolean) {
    ...
    if (corrupted) {
        ...
        DisplayText("Much to your annoyance and embarrassment, you feel the need to relieve yourself of your tension. Sighing in reservation, you remove the used onahole from your sack. Operating with a mind of its own, your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " springs to attention, anticipating the coming stimulation and release. Your member easily pushes past the opening in the toy and you begin working your penis with some vigor. Pleasure begins to pulse through your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " as you build yourself up. The force of the building fluids pushes against your inner organs, creating the paradoxical sensation of pain and pleasure. Pure instict takes over, and your hips begin to buck reflexively. With a sudden, sharp pinch at the base of your member, the need to cum takes over your body");
        ...
        if (gemsCreated > 0) {
            ...
            DisplayText(" of gems falls down upon your body instead of cum, bouncing off your " + Desc.Skin.skinFurScales(player) + ".\n\n<b>You force yourself to pick up the " + gemsCreated + " gems before passing out.</b>");
        }
        else {
