...
export class MarbleScene extends NPCAwareContent implements TimeAwareInterface {
	...
	//trace("Marble Stats: Aff-"+player.statusAffects.get(StatusAffectType.Marble).value1+" Add-" + player.statusAffects.get(StatusAffectType.Marble).value2 + " isA-" + player.statusAffects.get("$1").value3 + ".");
	...
	*/
	...
	public timeChange(): boolean {
		...
		if (player.statusAffects.has(StatusAffectType.CampMarble)) {
			...
			if (randInt(2) === 0) Flags.list[FlagEnum.MARBLE_LUST] += 1;
			if (player.statusAffects.get(StatusAffectType.Marble).value4 > 50) Flags.list[FlagEnum.MARBLE_LUST] += .3;
			if (player.statusAffects.get(StatusAffectType.Marble).value4 > 70) Flags.list[FlagEnum.MARBLE_LUST] += .3;
			...
			else if (!player.statusAffects.has(StatusAffectType.MarbleHasItem) && marbleScene.marbleAtCamp()) {
				// Off cooldown
				if (!player.statusAffects.has(StatusAffectType.MarbleItemCooldown)) {
					if (randInt(10) === 0) {
						DisplaySprite(41);
						DisplayText("\n<b>You find a note from Marble back at camp, letting you know that she has an item for you!</b>\n");
						player.statusAffects.set(new StatusAffect("MarbleItemCooldown", (24 + randInt(24)))), 0, 0, 0);
						player.statusAffects.set(new StatusAffect("MarbleHasItem", randInt(10))), 0, 0, 0);
						needNext = true;
					}
				}
			}
		...
		if (player.statusAffects.has(StatusAffectType.MarbleItemCooldown)) {
			...
			if (player.statusAffects.get(StatusAffectType.MarbleItemCooldown).value1 < 1) player.statusAffects.remove("MarbleItemCooldown");
		}
		if (!player.statusAffects.has(StatusAffectType.Infested)) Flags.list[FlagEnum.MARBLE_GROSSED_OUT_BECAUSE_WORM_INFESTATION] = 0;
		if (player.statusAffects.has(StatusAffectType.MarblesMilk) && !player.perks.has(PerkType.MarblesMilk)) {
			...
			if (player.statusAffects.get(StatusAffectType.MarblesMilk).value1 <= 0) {
				...
				if (player.statusAffects.get("$1").value3 > 0) {
					...
					if (player.statusAffects.has(StatusAffectType.BottledMilk)) {
						...
						player.statusAffects.add(StatusAffectType.MarbleWithdrawl, 0, 0, 0, 0);
						player.stats.tou += -5;
						player.stats.int += -5;
					}
				}
		...
		if (player.statusAffects.get("$1").value3 > 0 && !player.perks.has(PerkType.MarbleResistant) && !player.perks.has(PerkType.MarblesMilk) && player.statusAffects.get(StatusAffectType.Marble).value2 > 25) {
			//If player does not have marble's milk or bottled milk, go into withdrawl
			if (!player.statusAffects.has(StatusAffectType.MarblesMilk) && !player.statusAffects.has(StatusAffectType.BottledMilk)) {
		...
		if (player.statusAffects.get("$1").value3 <= 0) {
			if (player.statusAffects.get(StatusAffectType.Marble).value2 > 60) player.changeStatusValue(StatusAffects.Marble, 2, 60);
		}
		//Withdrawl removal if you get unaddicted.
		if (player.statusAffects.has(StatusAffectType.MarbleWithdrawl)) {
			if (player.statusAffects.get(StatusAffectType.Marble).value2 <= 25) {
			...
			else if (player.statusAffects.has(StatusAffectType.BottledMilk)) {
				DisplayText("\nYou no longer feel the symptoms of withdrawal.\n");
				needNext = true;
				player.statusAffects.remove("MarbleWithdrawl");
				player.stats.tou += 5;
		...
		if (player.statusAffects.has(StatusAffectType.BottledMilk)) {
			...
			if (player.statusAffects.get(StatusAffectType.BottledMilk).value1 <= 0) player.statusAffects.remove("BottledMilk");
		}
		if (model.time.hours > 23) {
			Flags.list[FlagEnum.MARBLE_PLAYED_WITH_KIDS_TODAY] = 0;
			if (player.statusAffects.has(StatusAffectType.Marble) && player.statusAffects.get(StatusAffectType.Marble).value2 > 0) marbleStatusChange(0, -1);
		}
		return needNext;
	}
	...
	public timeChangeLarge(): boolean {
		...
		if (player.statusAffects.get("$1").value3 > 0 && player.statusAffects.get(StatusAffectType.Marble).value2 < 25 && !player.perks.has(PerkType.MarblesMilk) && !player.perks.has(PerkType.MarbleResistant) && model.time.hours === 6) {
			DisplaySprite(41);
			...
			DisplayText("You find Marble in her room.  When you come in she looks up at you and starts.  \"<i>What happened?</i>\" she asks, \"<i>Something about you is completely different from before...</i>\"  You explain to her that you've gotten over your addiction and no longer crave her milk.\n");
			//(reduce corr by 5)
			player.stats.cor += -5;
			...
			if (player.statusAffects.get("$1").value3 === 1) {
				...
				if (player.statusAffects.get(StatusAffectType.Marble).value1 < 30) {
					...
					player.statusAffects.add(StatusAffectType.NoMoreMarble, 0, 0, 0, 0);
				}
				//Affection 30-89, version 1
				if (player.statusAffects.get(StatusAffectType.Marble).value1 >= 30 && player.statusAffects.get(StatusAffectType.Marble).value1 < 90) {
				...
				if (player.statusAffects.get(StatusAffectType.Marble).value1 >= 90) {
					...
					player.statusAffects.add(StatusAffectType.CampMarble, 0, 0, 0, 0);
			...
			//Ashamed Marble
				...
				if (player.statusAffects.get(StatusAffectType.Marble).value1 < 30) {
					...
					DisplayText("The piece of paper looks like a page torn from a book.  It looks like an entry from an encyclopedia of sorts, it reads in formal script:\n");
					DisplayText("<b><u>Codex: Lacta Bovine</u></b>\n");
					...
					DisplayText("Skin and Fur: <i>The skin tone for these creatures is very close to being human, their fur more closely follows the common minotaur fur colors: brown, black or white with brown spots.</i>\n");
					DisplayText("Behavior: <i>The behavior of Lacta Minotaurs varies greatly between each individual.  The only major unifying piece of behavior is their desire to give milk to almost any living creature, and their high libido, common to all corrupted creatures.</i>\n");
				...
				if (player.statusAffects.get(StatusAffectType.Marble).value1 >= 30 && player.statusAffects.get(StatusAffectType.Marble).value1 < 70) {
				...
				if (player.statusAffects.get(StatusAffectType.Marble).value1 >= 70) {
		...
		if (player.statusAffects.get("$1").value3 > 0 && player.statusAffects.get(StatusAffectType.Marble).value2 >= 100 && !player.perks.has(PerkType.MarblesMilk) && !player.perks.has(PerkType.MarbleResistant) && model.time.hours === 6) {
			DisplaySprite(41);
			...
			DisplayText("You find Marble in her room.  When you come in she looks up at you and smiles deeply.  \"<i>What happened?</i>\" she asks, \"<i>Something about you feels so wonderful and right.</i>\"  You explain to her that you've finally become entirely dependent on her milk.\n");
			...
			if (player.statusAffects.has(StatusAffectType.MarblesMilk)) {
				player.statusAffects.remove("MarblesMilk");
			}
			...
			if (player.statusAffects.get("$1").value3 === 1) {
				...
				if (player.statusAffects.get(StatusAffectType.Marble).value1 < 49) {
					...
					return { next: marbleBadEndFollowup };
					return true;
				}
				//Affection 50-79, type 1:
				if (player.statusAffects.get(StatusAffectType.Marble).value1 >= 50 && player.statusAffects.get(StatusAffectType.Marble).value1 < 79) {
				...
				//High affection
				if (player.statusAffects.get(StatusAffectType.Marble).value1 >= 80) {
			...
			//Marble didn't like you addicted
				...
				//Affection < 80, type 2:
				else if (player.statusAffects.get(StatusAffectType.Marble).value1 < 80) {
		...
		if (checkedMarbleMilk++ === 0 && model.time.hours === 6 && player.perks.has(PerkType.MarblesMilk)) {
			//Marble is at camp
			if (player.statusAffects.has(StatusAffectType.CampMarble)) {
			...
			//Marble isn't at camp
				...
				if (!player.statusAffects.has(StatusAffectType.NoMoreMarble)) {
	...
	public marbleFollower(): boolean {
		return player.statusAffects.has(StatusAffectType.CampMarble);
	}
	...
	public marbleBreastSize(): string {
		if (Flags.list[FlagEnum.MARBLE_PURIFIED] > 0) {
			...
			if (player.statusAffects.get(StatusAffectType.Marble).value4 > 30) return "HH cup";
			else return "G cup"
		}
	}

	//Initial encounter (1 hour duration) - comes up in the barn volunteering to help milk:
	public encounterMarbleInitially() {
		DisplaySprite(41);
		player.statusAffects.add(StatusAffectType.Marble, 0, 0, 0, 40);
		DisplayText("While exploring at Whitney's farm, you run across the furry southern belle almost immediately.  She looks like she has a job for you.\n\n", true);
		DisplayText("Whitney tells you that one of her barn's residents, a cow-girl named Marble, is sore from overusing the milk machines.  She asks you to go and give the cow-girl a gentler touch from a living being.\n\n");
		...
		if (player.tallness < 60) {
			DisplayText("She looks down at you with a smile and says \"<i>Aww, you're so cute!  Did you come for my milk?  I'm always happy to give it, but since I'm kinda sore right now, you'll have to be gentle. Okay little one?</i>\"  She moves towards you and tries to pick you up.");
		...
		MainScreen.simpleChoices(["Caress", "Suckle", "Rape", "", "Leave"], [caressMarble, suckleMarble, rapeDAHMARBLEZ, null, turnOffMarbleForever]);
	}

	private turnOffMarbleForever() {
		DisplayText().clear();
		DisplaySprite(41);
		//player.statusAffects.set(new StatusAffect("No_More_Marble",0,0,0,0)));
	...
	public marbleWarningStateMeeting() {
		DisplayText().clear();
		DisplaySprite(41);
		...
		MainScreen.simpleChoices(["Apologize", "Pick Fight", "Leave4Ever", "", "Leave"], [apologizetoWalkingTitsIMEANMARBLE, pickAFight, pickAFight, null, leaveNonFriendsMarble]);
	}
	//Leave (Z)
	private leaveNonFriendsMarble() {
		DisplayText().clear();
		DisplaySprite(41);
		DisplayText("Smiling politely and just as insincerely as Marble, you beg her pardon and excuse yourself.");
		//end event, initial non-friends event can repeat in future explorations
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	//Leave forever (Z)
	private leaveNonFriendsMarble4EVERRRR() {
		DisplayText().clear();
		DisplaySprite(41);
		player.statusAffects.add(StatusAffectType.NoMoreMarble, 0, 0, 0, 0);
		Flags.list[FlagEnum.MARBLE_WARNING] = 2;
		DisplayText("Answering the cow-girl with a blank look, you shake your head and walk away, resolving to avoid Marble from now on.");
		//Marble is removed from the game
		//end event
		return { next: Scenes.camp.returnToCampUseOneHour };
	}

	private apologizetoWalkingTitsIMEANMARBLE() {
		DisplayText().clear();
		DisplaySprite(41);
		...
		//Basic scene 
		DisplayText("You give chase after the bovine woman, wandering around the many plants in search of the runaway.  Her constant giggling makes sure you know you're going in the right direction, but sometimes she likes to double back or make false trails so the game is more interesting.  ");
		//[(intelligence check; <15, 15-40, 41+) 
		if (player.stats.int < 15) DisplayText("Eventually you find Marble stopped, looking towards you with her hands in the air saying, \"<i>You caught me!  Come here.</i>\"  She beckons you towards her chest, and you don't make her wait.");
		else if (player.stats.int < 40) DisplayText("Eventually you find Marble stopped and waiting for you.  She puts her hands in the air and says, \"<i>You caught me!</i>\"  It's fairly clear she's given herself up, but when she folds her hands in front of her chest and presses her breasts together, then tells you to come over, you aren't complaining.");
		else DisplayText("It isn't too hard to figure out that Marble isn't really trying, and you easily catch her off guard on one of her double backs.  She doesn't even notice you until you peek out from between the stalks next to her, reaching out and getting a handful of her backside.  \"<i>Clever " + Desc.Gender.mf(player, "boy", "girl") + "...</i>\" she says.");
	...
	private pickAFight() {
		...
		//[Stay][Fuck That]
		MainScreen.simpleChoices(["Stay", "Fuck That", "", "", ""], [stayForFights, getOutOfDodge, null, null, null]);
	}
	//[Stay]
	private stayForFights() {
		DisplayText().clear();
		DisplaySprite(41);
		DisplayText("You fold your arms over your chest and scowl as Marble trudges back over the fields carrying a huge hammer.  Part of you feels terribly juvenile to be solving an argument with violence - but the other part is cheering at the opportunity to put the bossy cow in her place.");
		//go to battle
		startCombat(new Marble(), true);
	}

	//[Fuck That]
	private getOutOfDodge() {
		DisplayText().clear();
		DisplaySprite(41);
		DisplayText("The hell you will... the truth is the truth no matter how many talking hammers show up.  Catharsis completed, you leave the farm and its cows behind.");
	...
	internal function marbleFightWin() {
		DisplaySprite(41);
		DisplayText().clear();
		//Win by hp
		if (monster.stats.HP < 1) DisplayText("Marble falls to the ground defeated; she looks up at you helplessly, wondering what you're going to do next.  ");
		//win by lust
		else DisplayText("Marble collapses and looks at you longingly, pulling up her skirt with a look of desperation in her eyes.  ");
	...
	internal function marbleFightLose() {
		...
		DisplayText().clear();
		//lose by hp
		if (player.stats.HP < 1) DisplayText("After a few too many blows to the head, you topple over to the ground.  ");
		//lose by lust
		else DisplayText("Overcome by desire, you fall to your knees, and start masturbating furiously.  Disgusted with you, Marble hits you upside the head once more, knocking you over.  ");
		DisplayText("She leans in close to your head and whispers \"<i>Don't ever come near me again, or I will crush your head with this hammer.</i>\"  She stands up and walks away from you as you pass out from your head injuries.  ");
		return { next: Scenes.camp.returnToCampUseOneHour };
	}

	//Rape in room (Z)
	private rapeMarbleInHerRoom() {
		DisplayText().clear();
		DisplaySprite(41);
		DisplayText("You aren't going to give up on this opportunity, but you don't want to have an audience either.  So you drag Marble and her hammer back to her room, and throw Marble onto her bed, grabbing and twisting her nipples, causing her to cry out in pain and pleasure.");
		//continue onto original rape
		DisplaySprite(41);
		DisplayText("  You suddenly grab at her breasts and squeeze them roughly, at which point she screams and ");
		...
		if (player.torso.cocks.count > 0) {
			DisplayText("Chuckling to yourself, you free your " + Desc.Cock.describeMultiCockShort(player) + " from your " + player.inventory.equipment.armor.displayName + ".  You spend a moment to ask the helpless cow-girl if she is ready, her only response being a whimper, before ");
			...
			if (player.torso.cocks.get(0).thickness < 3) {
				...
				if (player.torso.cocks.get(0).length <= 8) {
					DisplayText("forcing your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " in as far as it will go.  ");
				} else {
					DisplayText("forcing your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " in to the hilt.  ");
				}
				//the raping proper
				...
				DisplayText("You taunt her one more time before feeling your body get racked by an orgasm and you blow your load inside her.  ");
				//set player's lust to 0
				player.orgasm();
			}
			//now if the player doesn't fit
			else {
				DisplayText("attempting to push your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " inside her.  Of course, the girth of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " makes this a rather difficult operation, and it becomes clear after a few moments that it just isn't going to fit.  Instead, you contend yourself by rubbing yourself between her ample ass cheeks, occasionally stroking your " + Desc.Cock.describeMultiCockShort(player) + " in pride.\n\n");
				//is the player corrupt enough to get the fantasy?
				if (player.stats.cor >= 33)
					marbleRapeCorruptFantasy();
				DisplayText("You taunt her one more time before feeling your body get racked by an orgasm and you blow your load onto her ass.  ");
				//set player's lust to 0
				player.orgasm();
			}
		}
		//dickless girls
			...
			DisplayText("Keeping Marble in place, you get your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " ready to take in the other end of the dildo before doing so with gusto.  Much to Marble's discomfort, you manipulate the dildo in ways to heighten your own pleasure but give Marble a less enjoyable experience.  You ask her if she likes it, to which she responds with a whine and an attempt to move into a more comfortable position.  You tighten your grip on her, and she freezes again.\n\n");
			//is the player corrupt enough to get the fantasy?
			if (player.stats.cor >= 33)
				marbleRapeCorruptFantasy();
			DisplayText("You taunt her one more time before feeling your body get racked by a satisfying orgasm from using Marble's own toy against her.  ");
			//set player's lust to 0
			player.orgasm();
		}
		//the genderless option
			...
			if (player.stats.cor >= 33)
				marbleRapeCorruptFantasy();
			DisplayText("With just one more thing to do, you laugh at Marble before shoving your full fist inside her.  The act results in that familiar gasp of pain and pleasure.  Playing with her is indeed quite satisfying.  ");
			//Reduce player lust by 20
			player.stats.lust += -20;
		}
		//Pass several hours
		//Just before Marble hits the player with her hammer in original rape scene
		DisplayText("Satisfied, you pull back from the cow-girl's quivering body, and collect her hammer from the floor, informing her that you'll be taking it as compensation for the trouble she's caused you.  After dressing, you exit the barn.");
	...
	//Force-feed (by Spy) (Z)
	private forceFeedMarble() {
		DisplayText().clear();
		DisplaySprite(41);
		//[If player has Feeder perk]
		if (player.perks.has(PerkType.Feeder)) DisplayText("You bet this cow-girl loves to get milked and nursed on just like you, but how often does she get to taste the creamy sweetness of milk?  Having her suck on your own leaky tits would be doing her a favor, right?  You approach the defeated cow-girl; her eyes are still locked onto you, wondering what you're going to do next.  Well, not that you can do much with this crowd watching you...\n\n");
		...
		DisplayText(", but she keeps them tightly closed, refusing to drink your sweet milk.");
	...
	private resistMarbleInitially() {
		DisplaySprite(41);
		//(player chose resist)
		DisplayText().clear();
		DisplayText("Surprised by your resistance, she pulls back and apologizes for being presumptuous.  ");
		//- continue to the next part
		DisplayText("\"<i>My name's Marble, what's yours?</i>\" she asks you.  You introduce yourself and exchange a few pleasantries before she asks how she can help you.  You tell her that you actually came to help her, explaining that Whitney said she could use a gentle touch.  \"<i>Oh that would be nice</i>\", she says \"<i>Spending the night connected to the milking machine was a mistake, and now I need something gentle.</i>\"  How will you help her?");
		//- player chooses caress, suckle, or rape
		MainScreen.simpleChoices(["Caress", "Suckle", "Rape", "", ""], [caressMarble, suckleMarble, rapeDAHMARBLEZ, null, null]);
	}

	private marblePicksYouUpInitially() {
		DisplaySprite(41);
		...
		//new paragraph
		DisplayText("Once you'd had enough, you take your mouth off her teat and lean against her chest.  Marble puts her hands around you and ");
		if (player.torso.head.earType > EarType.HUMAN) DisplayText("gently scratches behind your ears.  ");
		else DisplayText("lightly caresses your head.  ");
	...
	//(player chose suckle)
	private suckleMarble() {
		DisplaySprite(41);
		DisplayText().clear();
		DisplayText("You suggest that you could gently suckle her breasts to make her feel better.  \"<i>That sounds wonderful!</i>\" she exclaims cheerfully, putting her hands under her ample mounds.  \"<i>There is nothing I love more than giving milk to living things.</i>\"  ");
		//[if player is under 5 feet tall]
		if (player.tallness < 60) DisplayText("Realizing you might have trouble reaching her breasts, you grab one of the chairs from the table.  ");
	...
	public encounterMarbleExploring() {
		DisplaySprite(41);
		DisplayText().clear();
		DisplayText("While wandering around the farm, you meet the cow-girl Marble heading towards the barn.  ");
		//[player height < 5 feet]
		if (player.tallness < 60) DisplayText("Marble gives her customary greeting of hugging you to her breast before telling you that she is off to get milked at the barn.  ");
		//[player hight >= 5 feet]
		else DisplayText("You exchange a quick greeting before Marble tells you that she is off to get milked at the barn.  ");
		//[affection <30]
		if (player.statusAffects.get(StatusAffectType.Marble).value1 < 30) {
		...
		//[affection >=30]
			...
			if (player.statusAffects.get(StatusAffectType.Marble).value2 < 40) {
			...
			//[if addiction is 40 or over] 
				...
				return { next: drinkMarbleMilk };
			}
		}
	}

	//(player chooses yes)
	private drinkMarbleMilk() {
		DisplaySprite(41);
		DisplayText().clear();
		...
		if (player.torso.head.earType > EarType.HUMAN) DisplayText(" and occasionally scratching behind your ears");
		DisplayText(".  ");
		DisplayText("Once you've had your fill, you pull back and the two of you smile at each other.  \"<i>It's really nice for you isn't it sweetie?  Nice for me too to have someone like you that can give a good suck on my itching nipples.</i>\"\n\n");
		...
		if (player.statusAffects.get(StatusAffectType.Marble).value2 >= 50) {
		...
		if (player.statusAffects.get(StatusAffectType.Marble).value2 < 50)
			DisplayText("After drinking Marble's milk, a feeling of euphoria spreads through you as you leave the farm in high spirits.");
		applyMarblesMilk();
		return { next: Scenes.camp.returnToCampUseOneHour };
	}

	//(player chooses no)
	private playerRefusesMarbleMilk() {
		DisplaySprite(41);
	...
	public encounterMarbleExploring2() {
		DisplaySprite(41);
		DisplayText().clear();
		DisplayText("You decide to pay Marble a visit at her room.  ");
		//[player height <5]
		if (player.tallness < 60) DisplayText("As you step into her room, she eagerly rushes over and hugs you to her breast. \"<i>You're as cute as ever, sweetie!</i>\"  ");
		DisplayText("She is happy to see you and treats you to a small meal while you have a pleasant chat.  ");
		if (Flags.list[FlagEnum.MURBLE_FARM_TALK_LEVELS] < 7) {
			extendedMurbelFarmTalkz();
			return { next: Scenes.camp.returnToCampUseOneHour };
			return;
		}
		else DisplayText("While you talk mostly about unimportant things, there is some discussion about the world and the dangers within.");
		//[addiction >30]
		if (player.statusAffects.get(StatusAffectType.Marble).value2 > 30) {
		...
		//[affection <60]
		if (player.statusAffects.get(StatusAffectType.Marble).value1 < 60) {
			...
			if (!player.statusAffects.has(StatusAffectType.FuckedMarble)) {
	...
	private turnDownMarbleSexFirstTime() {
		DisplaySprite(41);
		DisplayText().clear();
		DisplayText("She stares at you for a few moments as your refusal sinks in.  \"<i>So you don't feel the same way about me...  I'm sorry, I won't ever ask you again,</i>\" she says sadly.  \"<i>Maybe I'll see you later.</i>\" She directs you out the door.  You realize that refusing her will permanently affect your relationship.");
	...
	private turnDownMarbleSexRepeat() {
		DisplaySprite(41);
		DisplayText().clear();
		DisplayText("She looks disappointed at your refusal but quickly brightens up and says, \"<i>Ok sweetie, next time then.</i>\" On that note, you bid farewell to the pretty cow-girl and return to your camp.");
	...
	//(player chose yes)
	private AcceptMarblesSexualAdvances() {
		DisplaySprite(41);
		//Standard sex (See sex section)
		standardSex(true);
		if (!player.statusAffects.has(StatusAffectType.FuckedMarble)) player.statusAffects.add(StatusAffectType.FuckedMarble, 0, 0, 0, 0);
	...
	public helpMarble1() {
		...
		//[player str <20] 
		if (player.stats.str < 20) DisplayText("Unfortunately, the crates are quite heavy and you end up having to stick with small ones to keep up with Marble's pace.  She doesn't appear to mind, just enjoying having someone to talk to while she works, even if it doesn't save her many trips.\n\n");
		//[player str >=20, <50]
		if (player.stats.str >= 20 && player.stats.str < 50) DisplayText("You try your best, but for every crate you carry, Marble caries three. She doesn't mind though, since you'll end up saving her a quarter of the trips she would have had to make.\n\n");
		//[player str >=50, <80]
		if (player.stats.str >= 50 && player.stats.str < 80) DisplayText("You put your back into the job and manage to match Marble in her efforts.  She is really impressed with your strength, and together you can cut the number of trips needed in half.\n\n");
		//[player str >=80]
		if (player.stats.str >= 80) DisplayText("Marble may be strong, but you are stronger.  She is amazed as you manage to take even more crates at a time than she can, only held back by the number you can balance.  Thanks to your efforts, the chore only takes a third of the number of trips it normally would have taken.\n\n");
		DisplayText("After a little while, you notice that Marble is walking with an almost mesmerizing sway in her hips as she carries the crates; it is rather hard to take your eyes off her.  ");
		...
		inventory.takeItem(consumables.M__MILK, Scenes.camp.returnToCampUseOneHour);
	}

	//Help out Marble, version 2 (can occur anytime before Marble knows about her milk):
	public helpMarble2() {
		DisplaySprite(41);
		DisplayText("You run into Whitney at the farm, and ask if there's something you could do.\n\n", true);
		...
		//[player spd <20] 
		if (player.stats.spe < 20) DisplayText("Even though Marble often stops to munch on a weed, she is still able to get more weeds then you do.  Despite her size, she can move surprisingly fast.  Regardless, she enjoys simply having you there while she works, and you get to enjoy the view.\n\n");
		//[player spd >=20, <50]
		if (player.stats.spe >= 20 && player.stats.spe < 50) DisplayText("You put in a good effort at cleaning out the weeds, and Marble often gives you a good look at her rear when she finds a tasty looking weed.\n\n");
		//[player spd >=50, <80]
		if (player.stats.spe >= 50 && player.stats.spe < 80) DisplayText("Moving quickly through the fields, you surprise Marble with your speed so much that she jokingly pouts that you're getting to all the tasty weeds before she has a chance to eat them.  You still end up getting a few good views of her ass.\n\n");
		//[player spd >=80] 
		if (player.stats.spe >= 80) DisplayText("Weeding the field is a breeze for you, going fast enough that you're able to bring weeds to Marble faster than she can eat them.  In the end, you do almost all the work yourself.  She does reward you with a good view for your efforts.\n\n");
		//(increase player spd)
	...
	private afterMarbleHelp(): boolean {
		...
		if (player.statusAffects.get(StatusAffectType.Marble).value2 >= 40) {
			...
			if (player.stats.int < 40 && randInt(2) === 0) {
				...
				DisplayText("Once you've had your fill, you pull back and the two of you smile at each other.  \"<i>It's really nice for you isn't it sweetie?  Nice for me too to have someone like you that can give a good suck on my sensitive nipples.</i>\"\n\n");
			...
			//[player succeeds the int check] 
				...
				DisplayText("While you're working, you are continually plagued by the thought of drinking from Marble's breasts, but you're able to keep those thoughts at bay and continue working normally.\n\n");
			}
		}
		return false;
	}
	...
	private marbleAddiction(newPage: boolean) {
		DisplaySprite(41);
		//[start a new page]
		if (newPage) DisplayText().clear();
		...
		//(bold text)
		DisplayText("<b>Marble's milk is addictive, and you are now addicted to it.</b>\n\n");
		DisplayText("You pull back from her and look up into her eyes.  \"<i>Sweetie, how are you feeling?  Do you like drinking my milk?  Do you want to always drink my milk?</i>\" she says to you with uncertainty.  How do you reply?\n\n");
		doYesNo(wantMarbleAddiction, doNotWantMarbleAddiction);
	}

	//(player chose want)
	private wantMarbleAddiction() {
		DisplaySprite(41);
		DisplayText().clear();
	...
	private doNotWantMarbleAddiction() {
		DisplaySprite(41);
		DisplayText().clear();
	...
	private withdrawlFarmVisit() {
		DisplaySprite(41);
		DisplayText("You visit Whitney's farm once again. She quickly sees the tell-tale signs of your need and lets you know where Marble is.\n\n");
	...
	*/
	...
	public addictedEncounterHappy(clearS: boolean = true) {
		DisplaySprite(41);
		if (clearS) DisplayText().clear();
		//First visit post addiction:
		if (!player.statusAffects.has(StatusAffectType.MalonVisitedPostAddiction)) {
			DisplayText("You find Marble coming out of the barn, holding one of her bottles of milk.  When she spots you, she hurries over and hands you the bottle.  \"<i>I want to find something out. Can you drink from that bottle?</i>\" she asks.  Do you drink it?");
			//- player chooses yes/no
			doYesNo(playerAgreesToDrinkMarbleMilkBottled, playerRefusesToDrinkBottledMilk);
			player.statusAffects.add(StatusAffectType.MalonVisitedPostAddiction, 0, 0, 0, 0);
		}
		//Return visits
		else {
			//Addiction event version 1:
			if (randInt(2) === 0) {
				DisplayText("You find Marble in her room, softly humming while reading a book on her bed.  You walk up to her, and without looking away from her book she says, \"<i>I can smell your need, sweetie. Are you ready for your drink?</i>\" She sets the book down and turns to you, her hands under her breasts as she leans forward.\n\n");
				//- inte check to avoid immediately drinking, if succeeded: 
				if (player.stats.int >= 40) {
					DisplayText("Will you drink her milk?");
					//- player chooses yes/no
			...
			//Addiction event version 2:
				...
				DisplayText("You find Marble in the midst of one of her chores.  She smiles at you and says that if you help her with her chores, she will give you a bottle of milk to soothe your nerves.  Do you do it for the milk, Marble, or refuse?");
				//player chooses milk / Marble / refuse
				MainScreen.simpleChoices(["Marble", "Milk", "", "", "Refuse"], [marbleChoreHelpChooseMarble, marbleChoreHelpChooseMilk, null, null, marbleChoreRefusal]);
			}
		}
	}
	//(player chose yes to drink bottled milk)
	private playerAgreesToDrinkMarbleMilkBottled() {
		DisplaySprite(41);
		DisplayText().clear();
		...
		if (player.statusAffects.has(StatusAffectType.BottledMilk)) {
			player.addStatusValue(StatusAffects.BottledMilk, 1, (3 + randInt(6)));
		}
		else player.statusAffects.add(StatusAffectType.BottledMilk, 8, 0, 0, 0);
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	//(player chose no to drinking bottled milk)
	private playerRefusesToDrinkBottledMilk() {
		DisplaySprite(41);
		DisplayText().clear();
		...
		//If the player is addicted, this item negates the withdrawal effects for a few hours (suggest 6), there will need to be a check here to make sure the withdrawal effect doesn't reactivate while the player is under the effect of 'Marble's Milk'.
		if (player.statusAffects.has(StatusAffectType.BottledMilk)) {
			player.addStatusValue(StatusAffects.BottledMilk, 1, (1 + randInt(6)));
		}
		else player.statusAffects.add(StatusAffectType.BottledMilk, 3, 0, 0, 0);
		return { next: Scenes.camp.returnToCampUseOneHour };
	}

	//(player chose yes, or failed check)
	private playerDrinksMarbleMilk() {
		DisplaySprite(41);
		DisplayText().clear();
	...
	//(player chose no)
	private playerDeclinesToDrinkMarbleMilk() {
		DisplaySprite(41);
		DisplayText().clear();
	...
	//(player chose milk)
	private marbleChoreHelpChooseMilk() {
		DisplaySprite(41);
		DisplayText().clear();
	...
	//(player chose Marble)
	private marbleChoreHelpChooseMarble() {
		DisplaySprite(41);
		DisplayText().clear();
	...
	private marbleChoreRefusal() {
		DisplaySprite(41);
		DisplayText().clear();
		DisplayText("You angrily tell her that you aren't going to work for her milk and turn away, leaving her visibly upset.  Your body seems to be upset at your refusal too, feeling painful all over.  Fortunately, you also feel a temporary reprieve from the symptoms of your withdrawal.");
	...
	public marbleEncounterAddictedNonWithdrawl() {
		DisplaySprite(41);
		DisplayText().clear();
		...
		if (player.statusAffects.get(StatusAffectType.Marble).value1 >= 80) {
			DisplayText("As you eat, she looks deeply into your eyes for a moment. You think she is going to say something, but she shakes her head and avoids your questions about it for the rest of your time together.\n\n");
		}
		DisplayText("After you finish, she thanks you for treating her to your company and asks you to come back soon.  You return to your camp, knowing you will probably be seeing her again soon for something less pleasant.");
		//(increase affection by 10)
		marbleStatusChange(5, 0);
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	public encounterMarbleAshamedAddiction(clearS: boolean = true) {
		if (clearS) DisplayText().clear();
		DisplaySprite(41);
		//First visit post addiction:
		if (!player.statusAffects.has(StatusAffectType.MalonVisitedPostAddiction)) {
			...
			MainScreen.simpleChoices(["Comfort", "Blame", "", "", ""], [AshamedAddictionComfort, AshamedAddictionBlame, null, null, null]);
			player.statusAffects.add(StatusAffectType.MalonVisitedPostAddiction, 0, 0, 0, 0);
			return;
		}
		//REPEAT
		...
		if (randInt(2) === 0) {
			...
			MainScreen.simpleChoices(["Resist", "Give In", "", "", ""], [resistAddiction, addictionGiveIn, null, null, null]);
		}
		//Addiction event version 2:
		else {
			...
			MainScreen.simpleChoices(["Dump It", "Refuse", "Beg For It", "", ""], [dumpMarblesMilk, refuseMarblesMilkAddiction, ashamedAddictionBegForIt, null, null]);
		}
	}
	//(player chose to blame her)
	private AshamedAddictionBlame() {
		DisplaySprite(41);
		DisplayText().clear();
	...
	private AshamedAddictionComfort() {
		DisplaySprite(41);
		DisplayText().clear();
		DisplayText("You walk straight up to her and wrap your arms around her.  She just stands there idly for a moment before embracing you back.  ");
		//[player height less than 5 feet]
		if (player.tallness < 60) DisplayText("She pulls you into her chest and you feel relieved to see the Marble you know is still in there.  You feel warm drops of water fall on your head and look up to find Marble crying fresh tears, but this time with a big smile on her face.\n\n");
		//[player height greater than or equal to 5 feet]
		else DisplayText("You hear her breath start to come in short breaths and look at her face to find Marble crying fresh tears, but this time with a big smile on her face.\n\n");
	...
	private withdrawalDelay() {
		if (player.statusAffects.has(StatusAffectType.BottledMilk)) {
			player.addStatusValue(StatusAffects.BottledMilk, 1, (1 + randInt(6)));
		}
		else {
			player.statusAffects.add(StatusAffectType.BottledMilk, 3, 0, 0, 0);
		}
	...
	//(player chose refuse)
	private refuseMarblesMilkAddiction() {
		DisplaySprite(41);
		DisplayText().clear();
		...
		if (player.tallness < 60) DisplayText("hugs you to her chest, ");
		else DisplayText("gives you a big hug, ");
	...
	//(player chose beg)
	private ashamedAddictionBegForIt() {
		DisplaySprite(41);
		DisplayText().clear();
	...
	//Exploration event while addicted (event triggered while addicted, but not suffering withdrawal):
	public marbleEncounterAddictedNonWithdrawlAshamed() {
		DisplaySprite(41);
		DisplayText().clear();
		DisplayText("You decide to pay Marble a visit, as it would be nice to spend some time with her while you aren't in withdrawal.  You find her in her room and she looks at you ");
		//[affection >= 30]
		if (player.statusAffects.get(StatusAffectType.Marble).value1 >= 30) DisplayText("worriedly ");
		DisplayText("for a moment before it dawns on her that you aren't shaking.\n\n");
		//[affection >= 30] 
		if (player.statusAffects.get(StatusAffectType.Marble).value1 >= 30) DisplayText("\"<i>Sweetie, w");
		else DisplayText("\"<i>W");
		DisplayText("hy are you here if you don't need my milk?</i>\"  You explain that you just want to enjoy her company like you used to.  She gives a genuine smile that probably hasn't been on her face for a while, and the two of you have a meal together in her room.\n\n");
		//[affection is 80 or more]
		if (player.statusAffects.get(StatusAffectType.Marble).value1 >= 80) {
			DisplayText("While you're eating, Marble looks into your eyes deeply for a moment and you think she is going to say something, but she shakes her head and seems more reserved for the rest of your time together.\n\n");
		}
		DisplayText("After you finish, she thanks you for thinking of her like this, even with what you are going through.  You return to your camp, knowing you will probably be seeing her again soon for something less pleasant.");
		//increase affection by 10)
		marbleStatusChange(10, 0);
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	public postAddictionFarmMornings() {
		DisplaySprite(41);
		//(if player is completely addicted, do this event at the start of every day)
		DisplayText().clear();
		DisplayText("You hurry over to the farm to get your daily dose of Marble's milk.  It takes an hour of your day, but your body is satisfied.");
	...
	public postAddictionFarmExplorings() {
		DisplaySprite(41);
		DisplayText().clear();
		DisplayText("You decide to pay Marble a visit at her room.  ");
		//[player height <5]
		...
		if (player.perks.has(PerkType.MarbleResistant)) DisplayText("uncomfortably ");
		DisplayText("aware of the smell of Marble's milk.");
		player.stats.int += 1;
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	//Murble farm talks
	private extendedMurbelFarmTalkz() {
		//--- First Conversation ---
		switch (Flags.list[FlagEnum.MURBLE_FARM_TALK_LEVELS]) {
			case 0:
				DisplayText("During your talk, Marble asks where you're from.");
				//[if PC is human] 
				if (player.race() === "human") {
					DisplayText("  \"<i>The only other human I've ever met is that wandering trader Giacomo,</i>\" she tells you \"<i>but he doesn't really talk about himself.  Maybe you could tell me about humans?  I was wondering where they live and what kind of people they are.</i>\"");
					//[if PC is shorter then 5 feet] 
					if (player.tallness < 60) DisplayText("  Her eyes light up. \"<i>Are they all as cute as you?</i>\"");
					//[if PC is taller then 6'6" feet] 
					else if (player.tallness > 78) DisplayText("  \"<i>Are most of you this tall?</i>\"");
				}
				//[if PC is cow-girl/cowboi]
				else if (player.race() === "cow-morph" || player.race() === "cow-boy") {
					DisplayText("  \"<i>It's so nice to see another of my kind,</i>\" she tells you, \"<i>I haven't seen any since I left home.  Where are you from?</i>\"");
				}
				//[if PC is a dogmorph] 
				else if (player.race() === "dog-morph") DisplayText("  \"<i>I've seen lots of dog-morphs before, are you from Barkersvile?</i>\"");
				//[if PC is a centaur]
				else if (player.torso.hips.legs.isTaur()) DisplayText("  \"<i>I've seen a few centaurs before, but they don't seem to have regular homes.  They're nomads, wandering the plains.  Are you the same?</i>\"");
				//[if PC is not human, cow-girl/cowboi, dogmorph, or centaur] 
				else DisplayText("  \"<i>It's very rare that we get a " + player.race() + " here.  Are you from around these parts?</i>\"");
				DisplayText("\n\nYou sigh and think back for a moment before answering her.");
				//[if PC is not human anymore] 
				if (player.race() != "human") DisplayText("\n\nYou start by explaining that you weren't born as what you appear to be; you were once a human.  Marble is surprised by this, but when you start to explain how you came to be what you are, she stops you.  \"<i>You don't need to tell me the power of some of the things in this world.  Mommy taught me how to find LaBova if I ever lose a part of my bovinity,</i>\" she says, winking at you.  \"<i>I don't know of anything that will give humanity though, so I can't really help you if you want to change back...</i>\"  You tell her that's fine and that you'll look on your own if you need to do so.  \"<i>Well then, where is your human home?</i>\" she asks.");
			...
			//--- Second Conversation ---
			case 1:
				DisplayText("After you've been talking about inconsequential things for a few minutes, Marble asks you, \"<i>Sweetie, do you ever miss your home?  Do you ever wish you could be back with your parents?</i>\"");
				//[PC is pure] 
				...
				else DisplayText("You chuckle and say that you hardly think of them with all the other fun things to think about.  Then you pause and say that it is in the moments that you do think about them that keep you from forgetting who you once were.");
			...
			case 3:
				//why she left
				DisplayText("Marble looks over you a moment before saying, \"<i>You've mentioned this 'mission' a few times now; I guess that's why you left home?</i>\"  You nod.  \"<i>Fulfilling a duty, huh?  My story isn't quite the same as yours, and certainly not so noble!</i>\"  She laughs at this.");
				//[if PC is under 5 feet tall]
				if (player.tallness < 60) DisplayText("  \"<i>Sorry, sorry, it's just that you're so cute!  I can't picture you as some great hero leaving behind home and family for a mission.</i>\"  She wipes a few tears from her face.");
				//[if PC is over 7 feet tall]
				else if (player.tallness > 84) DisplayText("  \"<i>I'm sure a giant like you was the perfect choice for this mission.</i>\"");
				...
				if (player.statusAffects.get(StatusAffectType.Marble).value1 >= 60) DisplayText("  \"<i>Even though the start of the trip didn't turn out all that well, I think I found what I was looking for.</i>\"");
			...
			case 5:
				...
				if (player.perks.has(PerkType.HistorySlut) || player.perks.has(PerkType.HistoryWhore)) DisplayText("Well, you certainly gave an arousing description.  I think I can share something in return.");
				else if (player.stats.cor < 33) DisplayText("Sweetie, after you told me such a nice story, of course I will.");
				else if (player.stats.cor < 66) DisplayText("Sweetie, I can tell that you care a great deal about what happened, even if you try to hide it. Of course I'll share.");
				else DisplayText("Well, you certainly gave an arousing description.  I think I can share something in return.");
				DisplayText("  After my falling out with Ansgar, I noticed that I really missed nursing him more than anything else.</i>\"  She stops for a moment, shaking her head.  \"<i>That's not quite right.  It's more that I needed to keep nursing; it just felt like everything was wrong with me if I wasn't nursing something.</i>\"");
			...
			default:
				...
				if (player.stats.str < 90) DisplayText("  In fact, I don't think there's anyone out there who is stronger than me.");
				else DisplayText("  You're probably the only other person I've ever met who is stronger than me.");
				DisplayText("</i>\"");
				...
				if (!player.statusAffects.has(StatusAffectType.CampMarble)) DisplayText(", except that mission of yours that's so important to you");
				DisplayText(".</i>\"");
				...
				if (!player.statusAffects.has(StatusAffectType.CampMarble)) {
					DisplayText("  I may not show it, but I'm actually really lonely on the inside.");
					//[if addiction quest is active, and Marble is ashamed of her milk]
					if (player.statusAffects.get("$1").value3 === 2) DisplayText("  I really just wanted someone who loved me and loved nursing from my breast.  I had no idea what that would do to you.");
					//[if addiction quest is active, and Marble is happy with her milk]
					else if (player.statusAffects.get("$1").value3 === 1) DisplayText("  But I think that I've found the one who will change that.  Right sweetie?");
					//[if addiction quest has not yet triggered]
					else DisplayText("  Though, I'm not giving up hope yet.");
					DisplayText("</i>\"");
				}
				else DisplayText("  Until I met you, I was actually really lonely on the inside.</i>\"");
	...
	public postAddictionFarmHelpings() {
		DisplaySprite(41);
		DisplayText().clear();
		DisplayText("Smiling, Whitney suggests that you go help Marble out with her chores.  You readily agree and go out to meet with her.  Afterwards, Marble offers you a bottle of her milk.  ");
		//[if player is no longer addicted]
		if (player.perks.has(PerkType.MarbleResistant)) DisplayText("She assures you that you can't get addicted to it again if you don't drink her milk directly from her breasts.");
		//(randomly raise either str or spd)
		dynStats("str", randInt(2), "spe", randInt(2));
		//(player gets a bottle of Marble's milk)
		inventory.takeItem(consumables.M__MILK, Scenes.camp.returnToCampUseOneHour);
	}
	...
	public postAddictionCampMornings(extra: boolean = true) {
		DisplaySprite(41);
		//(if player is completely addicted, do this event at the start of every day)
		...
		if (!player.inventory.items.has(consumables.M__MILK, 5)) {
			...
			if (player.perks.has(PerkType.MarbleResistant)) DisplayText("She assures you that you'll be fine as long as you don't drink directly from her breasts.");
	...
	private standardSex(newpage: boolean = true) {
		DisplaySprite(41);
		if (newpage) {
			DisplayText().clear();
		}
		...
		if (player.gender > 0) {
			...
			if (player.inventory.equipment.armor.displayName === "bondage patient clothes" ||
				player.inventory.equipment.armor.displayName === "crotch-revealing clothes" || player.inventory.equipment.armor.displayName === "cute servant's clothes" ||
				player.inventory.equipment.armor.displayName === "maid's clothes" || player.inventory.equipment.armor.displayName === "servant's clothes") {
				DisplayText("You smirk at her and demonstrate just how easy it is to get at your genitals while wearing this outfit.  But you decide to not disappoint her, making a show of removing your " + player.inventory.equipment.armor.displayName + " and pretending to have some modestly when you show off your ");
				...
				DisplayText(".  You also make sure that there is no lingering clothing on your body for her, just as she did for you.\n\n");
			}
			//(player is not wearing fetish gear)
			else {
				DisplayText("You smile in return and begin removing your " + player.inventory.equipment.armor.displayName + " from your body.  You start by slowly slipping off your top to reveal your " + Desc.Breast.describeAllBreasts(player) + ", ");
				//[player's breasts are at least B cup]
				if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) DisplayText("running one of your hands down them before continuing.");
				//[player has small or no breasts]
				else DisplayText("running one of your hands over them before continuing on.");
				DisplayText("  You slip open the lower half of your clothes, revealing your ");
				if (player.torso.cocks.count > 0) {
					DisplayText(Desc.Cock.describeMultiCockShort(player));
					//player has at least one dick and a vagina]
					if (player.torso.vaginas.count > 0) DisplayText(" and your ");
				...
				//[player has a vagina]
				if (player.torso.vaginas.count > 0) DisplayText(Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)));
				DisplayText(".  You then remove any other clothing, leaving your body bare.\n\n");
			}
			//(player is a herm)
			if (player.gender === Gender.HERM) {
				DisplayText("Marble's eyes widen as you show off your equipment.  \"<i>You have both?</i>\" she says in awe. \"<i>I'm not sure if I can pleasure both parts of you at the same time... which would you prefer to use?</i>\"\n\nWhich of your genitals do you want to have sex with?");
			...
			//male
			if (player.gender === Gender.MALE) marbleSex2Continued(1);
			//female
			if (player.gender === Gender.FEMALE) marbleSex2Continued(2);
		}
		return { next: Scenes.camp.returnToCampUseOneHour };
	}

	public marbleSex2Continued(genders: number) {
		DisplaySprite(41);
		if (genders < 0) {
			genders = -genders;
			DisplayText().clear();
		...
		//(player is male)
		if (genders === 1) {
			DisplayText("Marble leans back and invites you to come.  With a coy grin, you slowly crawl towards her, each movement bringing the two of you closer until you are on top of her body. She puts her hands around you and lies back completely.  You carefully line up your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " and thrust into her warm snatch.  ");
			//[player is less than 8 inches in length]
			if (player.torso.cocks.get(0).length < 8) DisplayText("You easily slide all the way into her depths before beginning your hungry thrusts.  ");
			//[player is more than 8 inches in length]
			else DisplayText("She is unable to take all of you, but you're hardly discouraged as you begin your hungry thrusts.  ");
			DisplayText("She gasps slightly and her arms clamp down on you, ");
			//[player is between 4 and 5 feet in height]
			if (player.tallness < 60 && player.tallness >= 48) DisplayText("keeping your head tightly locked between her breasts.  ");
			//[player is not between 4 and 5 feet in height] 
			else DisplayText("keeping your body tightly locked against her.  ");
			DisplayText("Her tight grip does nothing to slow your thrusts, only helping to bring the both of you closer and closer to sweet release.  Finally, you push into her as far as you can");
			//[player has a knot and is not more than 8 inches long]
			if (player.torso.cocks.get(0).length < 8 && player.torso.cocks.find(Cock.HasKnot)) DisplayText(", and with a pop, your knot slips inside of her");
			DisplayText(".  Deep within her, your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " explodes.  Within moments, Marble feels her orgasm too, her grip intensifying immensely.  You hear her give a sigh that sounds almost like a moo and her grip relents.  The two of you lay there panting, your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " still inside her.\n\n");
		}
		//(player is female, I hope I did this right, I'm not familiar with the exact mechanics of female/female toys)
		else {
			DisplayText("Marble grins at you and reaches into the drawers of the mini-dresser next to her bed, pulling out a long, double-ended dildo. She slips one half into her own hole and invites you over to her.  With a coy grin, you slowly crawl towards her, each movement bringing the two of you closer until you slip your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " onto the other end of the dildo.  Sighing, Marble lies down underneath you and begins to rock back and forth.  You bring your own ministrations to the mix in response. Soon, the two of you are panting and moaning against each other with the dildo sliding around between you.  Suddenly, she locks her arms around you tightly and ");
			//[player is between 4 and 5 feet in height]
			if (player.tallness < 60 && player.tallness >= 48) DisplayText("squeezes your head into her breasts.  ");
			//[player is not between 4 and 5 feet in height]
			else DisplayText("pulls you securely against her.  ");
			DisplayText("Marble shudders with pleasure and redoubles her efforts at moving the dildo, quickly pushing you over the edge as well");
			//[player has at least one dick]
			if (player.torso.cocks.count > 0) DisplayText(".  Your " + Desc.Cock.describeMultiCockShort(player) + " erupts while clamped between your bodies");
			DisplayText(".  Her grip soon relents and the two of you lay there panting, the dildo still connecting you.  You can hear Marble say under her breath \"<i>That was way better than on my own...</i>\"\n\n");
			//(remove vaginal virginity?  "Your hymen is torn...")
			Mod.Vagina.displayStretchVagina(player, 8, true);
		}
		//(first time sex)
		if (!player.statusAffects.has(StatusAffectType.FuckedMarble)) {
			DisplayText("After a few minutes pass, Marble breaks the silence. \"<i>Sweetie, that was wonderful. You're really special to me, yah know?  Please remember that.</i>\"  You know that your relationship is special too; you won't forget Marble anytime soon.");
			player.statusAffects.add(StatusAffectType.FuckedMarble, 0, 0, 0, 0);
		}
		//(repeat sex)
		else {
			DisplayText("Marble sighs and gives you a big smile. \"<i>Sweetie, you're just as wonderful to be with as ever.  I'm always looking forward to our times together,</i>\" she tells you.  You would be inclined to agree with her.");
		}
		player.orgasm();
		//– return to trigger
	}
	...
	private marbleMilkSex(newpage: boolean = true) {
		DisplaySprite(41);
		if (newpage) {
			DisplayText().clear();
			return { next: Scenes.camp.returnToCampUseOneHour };
		...
		DisplayText("Drinking her milk has filled you with an intense need, and you can see that need in Marble's eyes too.  You have no choice; you are going to have sex with her.\n\n");
		//[player is wearing fetish gear]
		if (player.inventory.equipment.armor.displayName === "bondage patient clothes" ||
			player.inventory.equipment.armor.displayName === "crotch-revealing clothes" || player.inventory.equipment.armor.displayName === "cute servant's clothes" ||
			player.inventory.equipment.armor.displayName === "maid's clothes" || player.inventory.equipment.armor.displayName === "servant's clothes") {
			DisplayText("She moves first to get a good view of your equipment.  ");
		}
		//[player is not wearing fetish gear]
		else {
			DisplayText("She moves first and pulls open your " + player.inventory.equipment.armor.displayName + " to get a view of your equipment.  ");
		}
		//(player is male, or a herm)
		if (player.torso.cocks.count > 0) {
			//[player has only human or animal dicks]
			if (player.torso.cocks.count === player.torso.cocks.filter(Cock.FilterType(CockType.HORSE) + player.torso.cocks.countType(CockType.DOG) + player.torso.cocks.countType(CockType.HUMAN))).length {
				DisplayText("Marble seems pleased at the sight of your " + Desc.Cock.describeMultiCockShort(player) + " and she pushes you back onto her bed.  She removes her own skirt and stops for a moment at her tail.  Giggling slightly, she uses her ribbon-tied tail to brush at your " + Desc.Cock.describeMultiCockShort(player) + " before climbing on top of you and slipping her legs to your sides.\n\n");
			}
			//[player has at least one of a different kind of dick]
			else {
				DisplayText("Marble gives a long hard look before pointing at your ");
				temp = player.torso.cocks.count;
				while (temp > 0) {
					temp--;
					if (player.torso.cocks.get(temp).type.Index > 2) {
						DisplayText(Desc.Cock.describeCock(player, temp));
						temp = -1;
					}
				}
			...
			if (player.torso.cocks.count > 1) DisplayText("\"<i>So, which one is your favorite?</i>\"  Marble asks you while taking measure of your " + Desc.Cock.describeMultiCockShort(player) + ".  Before you have a chance to answer, Marble grabs a hold of your central " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " and says \"<i>I'll bet it's this one.</i>\"  ");
			DisplayText("Without much hesitation, Marble lifts herself up, and impales her moist lips upon your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  The two of you gasp as you sink into her interior.  You feel each inch slowly flow inside her until ");
			//[dick(0) is less than 8 inches long]
			if (player.torso.cocks.get(0).length < 8) DisplayText("every bit of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " is deep inside.  ");
			// //[dick(0) is more than 8 inches long]
			else DisplayText("Marble bottoms out at eight inches and you can go no further.  ");
		...
		//(player is female)
			...
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 2) {
				DisplayText("Marble runs her fingers along your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " for a moment before slipping a hand onto your " + Desc.Breast.describeBreastSize(character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + ".  She smiles at you and says \"<i>I think that maybe we should play up here. You did enjoy mine, so I think I'll enjoy yours.</i>\"  She helps you slips off your garments and lies you down on her bed, quickly taking her place on top of you.  She sticks her tongue out before putting her mouth to one of your " + Desc.Breast.describeNipple(character, character.torso.chest.get(0)) + "s and giving it a series of gentle licks.  Taking a moment to lick her lips, Marble starts to rub, caress and lick each of your " + Desc.Breast.describeAllBreasts(player) + " in turn with her hands and face, always gently and always lovingly.  Marble seems to be quite familiar with these techniques; she has probably practiced on herself many times before.\n\n");
			}
			//[player has smaller then C breasts]
			else {
				DisplayText("She teases your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " with her fingers for a moment before putting her hand on one of your " + Desc.Breast.describeBreastSize(character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + ".  \"<i>I'm not sure what to do with teats this small, but I'll do my best. Though I feel kinda sorry for you, sweetie.</i>\"  You open your mouth to make an indignant response, but Marble puts a figure on your lips to shush you. She gives you an intense stare and tells you in no uncertain terms that since you enjoyed her breasts so much, it's only fair that she gets to experience your " + Desc.Breast.describeAllBreasts(player) + ".  She then sticks out her tongue at you before giving your " + Desc.Breast.describeNipple(character, character.torso.chest.get(0)) + " a lick and rub down, which she extends to the ");
				if (player.torso.chest.countNipples() > 2) DisplayText("other");
				else DisplayText("rest");
				DisplayText(".\n\n");
			}
			//[after either]
			DisplayText("After finishing up");
			//[if player has more than two breasts]
			if (character.torso.chest.count * 2 > 2) DisplayText(" with each of your " + Desc.Breast.describeAllBreasts(player));
			else DisplayText(" with each of your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)));
			DisplayText(", Marble looks up at you and sticks her tongue out again.  \"<i>Now it's time for the main course!</i>\" she whispers before lowering her head down to one of your " + Desc.Breast.describeNipple(character, character.torso.chest.get(0)) + "s and closing her mouth around it. She soon begins to suckle your nipple.\n\n");
			//[if player has nipplecunts]
			if (player.torso.chest.find(BreastRow.FuckableNipples)) {
				DisplayText("As Marble's tongue rubs against your " + Desc.Breast.describeNipple(character, character.torso.chest.get(0)) + " it slips inside, much to her surprise.  She pulls back for a moment and gives you a confused look.  You tell her it isn't a problem; it only makes you happier. Understanding, Marble quickly resumes her efforts.  Her talented tongue fucks your " + Desc.Breast.describeNipple(character, character.torso.chest.get(0)) + " in earnest.");
				//[player is lactating]
				if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 1) DisplayText("  The whole while, Marble continues to gulp down your milk.");
			}
			//[if player is lactating]
			else if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 1) DisplayText("Her suckling soon brings a stream of milk to her lips that she gulps down eagerly.  You give a contented sigh, but are soon brought out of your revelry as her sucking becomes more stimulating and intense.");
			//[player is neither lactating or has nipplecunts]
			if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier <= 1 && !player.torso.chest.find(BreastRow.FuckableNipples)) DisplayText("Of course, since you aren't lactating, nothing but your " + Desc.Breast.describeNipple(character, character.torso.chest.get(0)) + " reaches her lips.  Marble doesn't seem to be put off by this and still gives you a thoroughly enjoyable experience.");
			DisplayText("\n\n");
			DisplayText("After a while, Marble pulls back from your " + Desc.Breast.describeNipple(character, character.torso.chest.get(0)) + " and tells you, \"<i>Sweetie, I know how annoying it is to only have one of your nipples serviced, so I will be sure to finish the job.</i>\"  She dives right into your ");
			if (player.torso.chest.countNipples() > 2) DisplayText("next");
			else DisplayText("other");
			DisplayText(" " + Desc.Breast.describeNipple(character, character.torso.chest.get(0)) + " and starts the experience all over again.  By the end, her efforts have successfully rung an orgasm from you. After cleaning up a bit, Marble sends you off in high spirits.");
		}
		player.orgasm();
	}
	...
	public marbleAddictionSex(newpage: boolean = true) {
		DisplaySprite(41);
		if (newpage) {
			DisplayText().clear();
			return { next: Scenes.camp.returnToCampUseOneHour };
		...
		DisplayText("You rock against each other, your hands working to remove Marble's clothes while her hands work at yours.  As your respective clothing falls to the floor, ");
		//[player is under 5.5 feet in height]
		if (player.tallness < 66) DisplayText("Marble bends over and kisses you deeply on the lips, a kiss that you eagerly return.  She lifts you up into the air, her lips still locked on yours, and holds you tightly against her body. A full minute passes before she lowers you down so your bodies can be joined.\n\n");
		//[player is between 5.5 feet and 6.5 feet in height]
		else if (player.tallness < 78) DisplayText("Marble lowers her head and locks her lips with yours, a show of passion that you eagerly return.  The two of you stay like that for a while, time seeming to standing still.  Then Marble breaks the kiss and looks deeply into your eyes as a sly grin spreads across her face.\n\n");
		//[player is over 6.5 feet in height]
		else DisplayText("Marble grabs a hold of your head and pulls your mouth to hers for a passionate kiss.  You are only too eager to comply and join in.  Even without her vice grip on your head, it feels like you couldn't possibly leave her wonderful mouth. But you eventually push back and look into her eyes intensely.\n\n");
		//[player has at least one dick]
		if (player.torso.cocks.count > 0) {
			DisplayText("The feeling of need is almost palpable within the two of you, and when the time comes, neither of you hesitate. You and Marble push yourselves together, forcing your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " deep into the soft folds of Marble's sex.  The two of you shudder and bask for a moment in the wonderful feeling of being connected, before gently starting to rock against one another.  Little else matters to you right now other than the warm, loving body that your arms are wrapped tightly around");
			//[player height is between 4 feet and 5 feet]
			if (player.tallness >= 48 && player.tallness <= 60) DisplayText(", the breasts your head is clamped between,");
			DisplayText(" and the warm folds where your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " is embedded.  You can't help but think about how wonderful a person Marble is, and you bet that Marble feels the same way about you.  \"<i>I love you sweetie.</i>\" Marble says to you gently.  You assure her that you feel the same, and at the moment, nothing else matters.\n\n");
		}
		//[player only has a vagina]
		else if (player.torso.vaginas.count > 0) {
			DisplayText("The feeling of need is almost palpable within the two of you.  Marble giggles and reaches behind her to grab something.  She shows you a long double-ended dildo and explains it will connect the two of you.  She pushes one end of it into your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " and the other end into the folds of her own sex.  The two of you shudder and bask for a moment in the wonderful feeling of being connected, before gently starting to rock against one another.  Little else matters to you right now other than the warm, loving body that your arms are wrapped tightly around");
			//[player height is between 4 feet and 5 feet]
			if (player.tallness >= 48 && player.tallness <= 60) DisplayText(" and the breasts your head is clamped between");
			DisplayText(".  You can't help but think about how wonderful a person Marble is, and you bet that Marble feels the same way about you.  \"<i>I love you sweetie.</i>\" Marble says to you gently.  You assure her that you feel the same, and at the moment, nothing else matters.");
			//(remove vaginal virginity? "Your hymen is torn...")
			Mod.Vagina.displayStretchVagina(player, 8, true);
			DisplayText("\n\n");
		}
		//[after either]
		DisplayText("Long after the two of you finish your lovemaking, you still continue to hold on to one another.  Eventually Marble ");
		//[player height is under 5.5 feet]
		if (player.tallness < 66) DisplayText("sets you down and ");
	...
	private marbleCampFuckFUCKFUCKFUCK() {
		DisplaySprite(41);
		marbleCampSexNew();
	}
	...
	public marbleStatusChange(affection: number, addiction: number, isAddicted: number = -1) {
		if (!player.statusAffects.has(StatusAffectType.Marble)) player.statusAffects.add(StatusAffectType.Marble, 0, 0, 0, 40);
		//Values only change if not brought to conclusion
		if (!player.perks.has(PerkType.MarblesMilk) && !player.perks.has(PerkType.MarbleResistant)) {
		...
		trace("Marble Affection: " + player.statusAffects.get(StatusAffectType.Marble).value1);
		trace("Marble Addiction: " + player.statusAffects.get(StatusAffectType.Marble).value2);
	}

	private applyMarblesMilk() {
		player.slimeFeed();
		let str: number = 5;
		let tou: number = 10;
		...
		if (!player.statusAffects.has(StatusAffectType.MarblesMilk)) {
			player.statusAffects.add(StatusAffectType.MarblesMilk, 12, 0, 0, 0);
		...
		if (player.statusAffects.get(StatusAffectType.MarblesMilk).value1 > 36) player.changeStatusValue(StatusAffects.MarblesMilk, 1, 36);
	...
	*/
	...
	I will refer to it as player.statusAffects.get(StatusAffectType.Marble).value4, replace it with what you will use.
	...
	*/
	//tion camp
	public interactWithMarbleAtCamp() {
		DisplaySprite(41);
		let gatherEvent;
		let milkEvent;
		let sexEvent;
		...
		DisplayText().clear();
		if (Flags.list[FlagEnum.MARBLE_PURIFICATION_STAGE] != 2) DisplayText("How will you interact with Marble?");
		//Reminder to visit Rathazul
		else {
			DisplayText("<i>\"Hello, sweetie; have you had a chance to talk to that alchemist yet? I’m so close to being pure at last, but I need his help if I’m to get over this last hurdle.\"</i>", true);
		}
		...
		if (player.statusAffects.has(StatusAffectType.MarbleHasItem)) gatherEvent = marbleGathered;
	...
	private marbleTalkOverhaul() {
		DisplayText().clear();
		DisplayText("What do you want to discuss with Marble?");
		...
		if ((Flags.list[FlagEnum.MARBLE_PURIFICATION_STAGE] < 5 && Flags.list[FlagEnum.MARBLE_LUST] >= 20) || (Flags.list[FlagEnum.MARBLE_PURIFICATION_STAGE] >= 5 && Flags.list[FlagEnum.MARBLE_TIME_SINCE_NURSED_IN_HOURS] >= 4)) {
			DisplayText("[pg]<b>Marble is fidgeting around uncomfortably, perhaps she needs to be milked?</b>");
			MainScreen.addButton(2, "Milking", milkMarble);
		}
		if (player.stats.cor < 50 && player.statusAffects.get(StatusAffectType.Marble).value4 < 60) MainScreen.addButton(1, "Just Talk", talkWithMarbleAtCamp);
	...
	private marbleSleepToggle() {
		DisplayText().clear();
		if (Flags.list[FlagEnum.SLEEP_WITH] != "Marble") {
			DisplayText("Marble says, \"<i>That sounds lovely, Sweetie.</i>\"");
			Flags.list[FlagEnum.SLEEP_WITH] = "Marble";
		}
		else {
			DisplayText("Marble folds her arms across her bountiful bosom and sighs, \"<i>If you say so, Sweetie.</i>\"");
	...
	private sleepWith(arg: string = "") {
		Flags.list[FlagEnum.SLEEP_WITH] = arg;
	}

	//Kid playtime
	private marbleKidsPlaytime() {
		DisplayText().clear();
		let choices: Array = [];
		...
		if (select === 1) {
			if (Flags.list[FlagEnum.MARBLE_KIDS] === 1 && Flags.list[FlagEnum.MARBLE_BOYS] === 1) {
				...
				DisplayText("Your daughter immediately clops up to you and jumps into your arms.  \"<i>" + Desc.Gender.mf(player, "Daddy, daddy", "Mommy, mommy") + "!  Play with me!</i>\"  You laugh at the girl in your arms, so young and yet full of energy.  She proceeds to rope you into a grand adventure, where you play the parts of the one who searches for a hero to save their camp, and the monster that has to be stopped, and also the monster's hostage that needs to be saved, while she is the great bovine saviour that carries out this grand quest.");
				DisplayText("\n\nThe game finally ends with the bovine saviour victorious over the horde of ravenous monsters  - somewhere along the line it was decided that hostages were no fun and single combat wasn't impressive enough, and you exhausted your theatrical skills to provide a bevy of satisfyingly scary faces and growls - and the people of the land (you) rejoicing.  You're left a bit tired afterwards, but you certainly don't regret having played with your child.");
			}
		...
		if (select === 2) {
			...
			//([corruption check, <=40] 
			if (player.stats.cor < 66) DisplayText("  You do make sure to skip over the items of a more explicit nature throughout.");
			DisplayText("  Eventually your story comes to an end, and you bid farewell to your kids.");
		...
		//requires at least 2 kids
		if (select === 4) {
			DisplayText("You notice Marble is sitting on some wood logs with your children.  Curious, you decide to check what exactly is going on.");
			...
			if (Flags.list[FlagEnum.MARBLE_KIDS] === 2) DisplayText("both");
			else DisplayText("about half of your children");
			DisplayText(" have at least some knack for it.");
		...
		if (select === 5) {
			DisplayText("Marble looks up when you approach and gives you a weary smile.  \"<i>Hello, sweetie.</i>\"");
			...
			DisplayText("\n\n\"<i>" + Desc.Gender.mf(player, "Daddy", "Mommy") + "!  Is this it?  Is this it?</i>\"");
			...
			DisplayText("\n\nYour kids are just a little bit tired from all the work and it doesn't seem they'll pester Marble much more today.  One of them even grabs you, complaining, \"<i>" + Desc.Gender.mf(player, "Daddy", "Mommy") + ", I'm sleepy!</i>\"  You pat her head and smile, telling her that you'll be home soon enough.");
			...
			//[Height below 5 feet]
			if (player.tallness < 60) DisplayText("grabs you and immediately lifts you up to her chest, giving you a big hug.");
			//[Height above 5 feet]
			else DisplayText("embraces you strongly, pressing her warm body against yours.");
			DisplayText("\n\n\"<i>Thank you sweetie.  I love you.</i>\"\n\n");
			//increase fatigue by 15, decrease lust by 10, and decrease libido by 1
			fatigue(15);
			//Takes 1-2 hours?
			inventory.takeItem(consumables.W_FRUIT, Scenes.camp.returnToCampUseOneHour);
			//end event
			return;
		}
		if (select === 6) {
	...
	private breakUpWithMarble() {
		DisplaySprite(41);
		DisplayText().clear();
		DisplayText("Are you sure you want to break up with Marble?  You won't be able to get her back if you do so.");
	...
	//[Yes]
	private definitelyBreakUpWithWithMarble() {
		DisplaySprite(41);
		DisplayText().clear();
		DisplayText("You approach Marble and tell her that you need a bit of space and some time away from her.  She freezes at your words, and asks you to repeat yourself.  You do so and she nods, her face almost impassive.  \"<i>I guess I'll go back to the farm.  ");
		if (player.perks.has(PerkType.MarblesMilk))
			DisplayText("I'll see you there when you need my milk, alright?");
		else
			DisplayText("I'll see you later then.");
		DisplayText("</i>\"  Marble collects her things");
		if (Flags.list[FlagEnum.MARBLE_KIDS] === 1) DisplayText(" and her child");
		else if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) DisplayText(" and her children");
		DisplayText(", and leaves the camp behind.");
		return { next: playerMenu };
	...
	private talkWithMarbleAtCamp() {
		DisplaySprite(41);
		if (!player.statusAffects.has(StatusAffectType.MarbleSpecials)) {
			player.statusAffects.add(StatusAffectType.MarbleSpecials, 0, 0, 0, 0);
		}
		return { next: playerMenu };
		//I don't understand your code enough yet to come up with the conditions for these ifs:
		//General thoughts:
		DisplayText("You call Marble over and the two of you sit down on some rocks to chat.  After thinking a bit, you sort through everything that has happened in this strange land, trying to come up with a new story for your bovine friend.  ", true);
		...
		if (player.statusAffects.has(StatusAffectType.MetMarae) && Flags.list[FlagEnum.MARBLE_CAMPTALK_LEVEL] < 1) {
		...
		else if (player.statusAffects.has(StatusAffectType.FoundFactory) && Flags.list[FlagEnum.MARBLE_CAMPTALK_LEVEL] < 2) {
			DisplayText("You tell Marble you found a demonic factory and relate everything you know about it.  \"<i>Be careful in there,</i>\" Marble tells you, \"<i>I'm certain that place will consume you if you're unprepared.</i>\"");
			Flags.list[FlagEnum.MARBLE_CAMPTALK_LEVEL] = 2;
		}
		//The player has cleared the factory and shut it down
		else if (player.statusAffects.has(StatusAffectType.DungeonShutDown) && Flags.list[FlagEnum.MARBLE_CAMPTALK_LEVEL] < 3) {
			DisplayText("You tell Marble about what you found inside the factory.  She is horrified at what was being done to the other champions and assures you that no one should ever <i>belong</i> in a place like that. You continue and tell of the overseer and her fate. Marble reacts with surprise, ");
			if (player.perks.has(PerkType.OmnibusGift))
				DisplayText("and hopes that you've learned your lesson about accepting <i>gifts</i> from demons.  ");
			else
				DisplayText("but concludes that what you did was probably for the best.  At least you didn't fall for her trick.  ");
			DisplayText("Finally, you tell her how you shut down the factory and what happened to the captured champions.  Marble is shocked that some stayed, but says she doesn't think there is much the two of you could do to help them if they're already addicted. \"<i>You should probably just leave them be, for now.</i>\"");
		...
		else if (player.statusAffects.has(StatusAffectType.MaraeComplete) && !player.statusAffects.has(StatusAffectType.FactoryOverload) && Flags.list[FlagEnum.MARBLE_CAMPTALK_LEVEL] < 4) {
			DisplayText("Marble is very happy to hear you helped Marae.  With the factory taken care of and Marae's corruption postponed for some time, the both of you will probably sleep a little easier tonight. ");
			Flags.list[FlagEnum.MARBLE_CAMPTALK_LEVEL] = 4;

		}
		//The player has met the corrupted Marae after blowing the storage tanks
		else if (player.statusAffects.has(StatusAffectType.MaraeComplete) && player.statusAffects.has(StatusAffectType.FactoryOverload) && Flags.list[FlagEnum.MARBLE_CAMPTALK_LEVEL] < 4) {
			DisplayText("Your story about what had happened to Marae seems to have shaken up Marble a little.  Though, you notice that she seems to be getting more and more aroused as you relate your story.  ");
			if (player.perks.has(PerkType.MaraesGiftFertility))
			...
			else if (player.perks.has(PerkType.MaraesGiftStud))
			...
			if (!player.perks.has(PerkType.MaraesGiftFertility) &&
				!player.perks.has(PerkType.MaraesGiftStud)) {
				DisplayText("You finish your tale by recounting how you ran away.  She isn't really sure how to respond to your decision, but Marble does thank you for not leaving her behind and joining Marae.");
			}
			Flags.list[FlagEnum.MARBLE_CAMPTALK_LEVEL] = 4;
		}
		//(story) Talk about Zetaz's unexplored lair(Z)
			...
			DisplayText("You tell Marble about the cave you found deep in the forest.  For some reason, this seems to bother Marble, but when you ask her what is wrong, she claims it's just a feeling and tells you not to worry about it.  Maybe you should make sure you're ready before you go back there?");
			Flags.list[FlagEnum.MARBLE_CAMPTALK_LEVEL] = 5;
		}
		//story) Talk about Tel'Adre(Z)
		else if (player.statusAffects.get(StatusAffectType.TelAdre).value1 >= 1 && Flags.list[FlagEnum.MARBLE_TELADRE_STORY] === 0) {
		...
		else DisplayText("You have no new stories to share with Marble, so you chat for a bit about inconsequential things.");
		...
		DisplayText("The topic of conversation turns to your mission and you ask Marble what she thinks you should be doing next.  ");
		//If (player has not yet met Marae)
		if (!player.statusAffects.has(StatusAffectType.MetMarae)) {
			DisplayText("\"<i>Well sweetie, I guess you should start with getting to know the place a little better.  Why don't you look some more around the lake outside the farm?  I think that's the safest place to start.</i>\"");
		}
		else if (player.statusAffects.has(StatusAffectType.MetMarae) && !player.statusAffects.has(StatusAffectType.FoundFactory)) {
			//check if the player is far too weak to go to the factory
			if (player.level < 3) DisplayText("\"<i>I think we should help out Marae and shut down that factory she mentioned was in the mountains, but I don't think you're ready to go into the Mountains yet. They can be brutal - get a little more practice, and make sure you've got a good weapon.</i>\"");
			else DisplayText("\"<i> I think we should help out Marae and shut down that factory she mentioned was in the mountains. I have no idea what will happen in there, though, so make sure you're as ready as you can be before you go.</i>\"\n\n");
		}
		//Player has found factory but not shut it down.
		else if (player.statusAffects.has(StatusAffectType.FoundFactory) && !player.statusAffects.has(StatusAffectType.DungeonShutDown))
			DisplayText("\"<i>You still haven't shut down the factory yet, have you?</i>\"  You shake your head.  \"<i>Well then go do it!</i>\"");
		//(player has completed the factory but has not returned to Marae)
		else if (player.statusAffects.has(StatusAffectType.DungeonShutDown) && !player.statusAffects.has(StatusAffectType.MaraeComplete) && !player.statusAffects.has(StatusAffectType.MetCorruptMarae))
			DisplayText("\"<i>You haven't gone back to Marae yet have you?</i>\"  You shake your head.  \"<i>Well then go see her!  I'm sure she really wants to thank you.</i>\"");
		//If PC has not yet discovered Zetaz's lair or Tel'Adre (Z)
		else if (Flags.list[FlagEnum.DISCOVERED_DUNGEON_2_ZETAZ] === 0 || player.statusAffects.get(StatusAffectType.TelAdre).value1 === 0)
			DisplayText("\"<i>Well sweetie, maybe you should explore the world more?  From what I've heard from the centaurs who stop by the farm, I'm sure you could find something interesting in the deep forest, or maybe the desert,</i>\" she suggests.");
		//(suggestion) If PC has found Tel'Adre, but not Zetaz's lair(Z)
		else if (Flags.list[FlagEnum.DISCOVERED_DUNGEON_2_ZETAZ] === 0 && player.statusAffects.get(StatusAffectType.TelAdre).value1 >= 1)
			DisplayText("\"<i>I was thinking that the deep woods probably still hide secrets from you, sweetie... a few times I've been there, I've noticed large numbers of imps carrying supplies.  Think there could be something to that?</i>\" she asks.");
		...
		//(suggestion) if PC has discovered Zetaz's lair but has not cleared the cave(Z)
			...
			DisplayText("\"<i>Well, you haven't finished exploring that cave in the woods yet, have you?  Why don't you go there?</i>\" she offers.  She still seems to be a bit bothered by something, but doesn't voice it.");
		//Nothing to advise!
		else DisplayText("\"<i>Well sweetie, this is it. Your mission has been to stop the demons, Lethice is their leader. Once you're ready, you need to go to their high mountain lair and take them down. Make sure you've finished everything else you want to do before you face Lethice, you won't get a second chance.</i>\"");
		//OLD else DisplayText("\"<i>Sweetie, you've finished all that there is in the main story of the game so far.  You'll have to wait for Fenoxo to add more,</i>\" she says to you winking. \"<i>In the meantime, explore! There's a lot in here that isn't a part of the main story.  You could also try changing your form and see how that changes each encounter.  Just don't get caught or lose your head, ok sweetie?<i/>\"");

		return { next: interactWithMarbleAtCamp };
	}

	//do a sex scene, use the current camp one for now, this event advances the time by an hour.
	private marbleSexExpanded() {
		DisplaySprite(41);
	...
	private gotMilk() {
		DisplaySprite(41);
		return { next: Scenes.camp.returnToCampUseOneHour };
		DisplayText("You ask Marble for a bottle of her milk, and she happily hands you one.  ", true);
		inventory.takeItem(consumables.M__MILK, Scenes.camp.returnToCampUseOneHour);
	}

	private marbleGathered() {
		DisplaySprite(41);
		return { next: playerMenu };
		...
		DisplayText("You ask Marble about any supplies she might have found.  She smiles and hands you her latest find.\n\n", true);
		...
		if (Flags.list[FlagEnum.MARBLE_PURIFICATION_STAGE] != 1 && player.statusAffects.get(StatusAffectType.MarbleHasItem).value1 <= 4)
			inventory.takeItem(consumables.VITAL_T, Scenes.camp.returnToCampUseOneHour);
		//Tanned Leather clothes, armor, def: 5 (three days)
		else if (Flags.list[FlagEnum.MARBLE_PURIFICATION_STAGE] != 1 && player.statusAffects.get(StatusAffectType.MarbleHasItem).value1 <= 7)
			inventory.takeItem(armors.LEATHRA, Scenes.camp.returnToCampUseOneHour);
		...
		else inventory.takeItem(consumables.LABOVA_, Scenes.camp.returnToCampUseOneHour);
	...
	private marbleInfo() {
		DisplaySprite(41);
		DisplayText("Marble is a loyal friend and lover who has decided to help you with your quest.  She can be interacted with while she is at camp.  ", true);
		DisplayText("She can share some of her thoughts and give advice on your current situation, or supply you with bottles of her milk and other useful items that she has found while scavenging.  You can also get Marble to consume some of the items you find.\n\n");
		//explain morning drinking sessions if the player is an addict
		if (player.perks.has(PerkType.MarblesMilk)) {
			DisplayText("She will nurse you every morning automatically to satisfy your addiction.  ");
			//otherwise tell the player that drinking bottles of her milk is safe
		}
		else {
			if (Flags.list[FlagEnum.MARBLE_PURIFIED] === 0) DisplayText("So long as you don't drink milk from Marble's breasts again, you don't have to worry about getting addicted.  ");
			else DisplayText("Now that she's been purified, you can drink as much of her milk as you like without fear.  ");
		}
		DisplayText("Once camp improvements have been implemented, you will be able to get her to work on upgrading the camp instead of searching for supplies.");
		return { next: interactWithMarbleAtCamp };
	}
	...
	private canGiveItem(): boolean {
		...
		else if (player.perks.has(PerkType.MarblesMilk)) return true;
		return false;
	}

	public giveItem() {
		DisplayText().clear();
		DisplayText("What item do you want to give Marble?");
		...
		else if (player.perks.has(PerkType.MarblesMilk)) MainScreen.addButton(0, "Lactaid", giveMarbleLactaid);
		MainScreen.addButton(9, "Back", interactWithMarbleAtCamp);
	}

	//The player gives Marble an item, right now only Lactaid will be here, and only if the player is fully addicted
	public giveMarbleLactaid() {
		DisplaySprite(41);
		//Lactaid
		...
		DisplayText("You show Marble the bottle of Lactaid that you found and ask her if she could drink it.  She gives you a knowing smile. \"<i>So you want a special treat?  Ok sweetie, give it here.</i>\"  She takes the bottle from you and easily drinks the liquid.  Within moments, she gasps and ", true);
		if (player.tallness < 60) DisplayText("lifts you up to her chest before saying breathlessly saying \"<i>Drink now!</i>\"  You waste no time obliging her.\n\n");
		else if (player.tallness > 72) DisplayText("breathlessly says\"<i>Drink now!</i>\"  You waste no time and descend upon her breasts.\n\n");
		else DisplayText("breathlessly says, \"<i>Drink now!</i>\"  You quickly move forward to get a drink.\n\n");
	...
	*/
	private marbleCampSexNew() {
		DisplaySprite(41);
		if (player.statusAffects.has(StatusAffectType.Infested)) {
			DisplayText("  You call Marble over and ask her if she can give you some release.  She smiles at you and gently grips your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " in one of her hands before recoiling in horror.  \"<i>Uh, why don't you take care of that problem of yours in your cock first, sweetie?  Then I'll help you get release.</i>\"  It looks like Marble isn't willing to help you get release while you have worms infecting your cock.");
			return;
		}
		//Marble isn't willing to pleasure the PC if her lust is lower than 0
		if (Flags.list[FlagEnum.MARBLE_LUST] < 0) {
			DisplayText("  You call Marble over to see if she can help you get some release.  She looks at you sourly before saying, \"<i>I'm not in the mood right now, but feel free to use the toy in my bag on yourself.</i>\"  Well, that's generous... er, wait, did she just tell you to go fuck yourself?");
			//end event
			return;
		}
		...
		if (!player.torso.hips.legs.isNaga()) {
			//Feeding
			if (player.perks.has(PerkType.MarblesMilk) && player.gender > 0 && randInt(2) === 0) {
				...
				DisplayText().clear();
				DisplayText("You tell Marble you're a little bit horny and you're wondering if she'd be willing to do something special for you in private.  Hearing you whisper your idea, she giggles and strokes your [face].");
				DisplayText("\n\n\"<i>That's so cute, sweetie!  Of course I could do that for you.</i>\"");
				DisplayText("  She leads you by the hand to sit in a more comfortable place.");
				...
				if (player.tallness < 66) DisplayText("right breast.");
				//[height above 5' 7''] 
				else DisplayText("left breast.");
				//[+ if height above 8' 6'']
				if (player.tallness > 90) DisplayText("  You do need to bend over in order to access her breast as well as allow her access to your groin, but while slightly uncomfortable, your plan is worth the effort.");
				...
				//( [Male/Herm] 
				if (player.torso.cocks.count > 0) {
					DisplayText("She chuckles as her hand reaches your " + Desc.Cock.describeMultiCockShort(player) + ".  \"<i>You seem quite ready and eager for something, sweetie.</i>\"");
				}
				else {
					DisplayText("She chuckles as her hand reaches your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".  ");
					//[Vaginal wetness 0-1] 
					if (player.torso.vaginas.get(0).wetness < 2) DisplayText("\"<i>Oh, sweetie, this just won't do!  It's like you're not looking forward to this at all!</i>\"");
					//[Vaginal wetness 2-3] 
					else if (player.torso.vaginas.get(0).wetness < 4) DisplayText("\"<i>It seems you're quite in need of something down here, sweetie...</i>\"");
					//[Vaginal wetness 4]
					else if (player.torso.vaginas.get(0).wetness < 5) DisplayText("\"<i>My, my, sweetie; this is so wet... are you sure we should stimulate it more?</i>\"");
					//[Vaginal Wetness 5] 
					else DisplayText("\"<i>Oh my, sweetie; it seems like you need to be toweled off...</i>\"");
				}
				...
				//[Male/herm] 
				if (player.torso.cocks.count > 0) {
					DisplayText("\n\nHer hand continues stroking your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", very gently at first, but once you intensify your suckling, she gasps slightly and gives your member an appreciative squeeze, increasing the intensity.  You buck your hips slightly but she just strokes your " + Desc.Head.describeHair(player) + " as if to calm you down.");
					DisplayText("\n\n\"<i>It doesn't hurt, right?  I want to make you feel good, sweetie...  If you keep sucking me, I'll make you feel very, very good.</i>\"");

					DisplayText("\n\nYou gently moan into her breast in response, causing her to giggle as she plays with the tip of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  She returns to the pumping motion and increases the pace just as you decide you need something to grab onto - and the most pleasant things at hand are her breasts.  Marble gasps in surprise and moans as you rub her breasts while suckling, and starts jerking you off even faster in response.");
					//( [Female Marble]
					if (Flags.list[FlagEnum.MARBLE_DICK_LENGTH] === 0) DisplayText("  Her thighs seem to be rubbing together, and the place you're lying on feels a tiny bit wet, so you assume it isn't only you who's getting more and more excited by this.");
					else DisplayText("  Her thighs are gently rubbing beneath you, and you feel something hard poking you in the back.  You smile around the nipple you have in your mouth.");
					//[If balls] : A
					if (player.torso.balls.quantity > 0) DisplayText("  As her hand reaches the base of your cock, she stops stroking it to take a moment and gently roll your " + Desc.Balls.describeBalls(true, true, player) + " with her fingers.  \"<i>Are these ready to shoot out for me soon, sweetie?</i>\"");
					//([If pussy] : 
					if (player.torso.vaginas.count > 0) DisplayText("  From the base of your cock, her hand moves even lower, gently rubbing your " + Desc.Vagina.describeClit(player) + ".  \"<i>This seems to also require some attention...  Maybe later, sweetie, if you do a good job nursing from me.  But right now, I want to see you shoot off,</i>\" she says, as her hand moves back to your " + Desc.Cock.describeMultiCockShort(player) + ".");
					DisplayText("\n\nShe starts tugging at and stroking your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", eager to make you cum now.  You buck your hips again and she responds more agressively this time, pushing you back down with her breasts as her touch starts to become unbearable.  \"<i>You're close, aren't you, sweetie?  It's okay, I want to see you cum.</i>\"");
					...
					if (player.cumQ() < 500) DisplayText("  You cover her hand as she keeps jerking you off, squeezing out tiny little droplets once you're done ejaculating.");
					else if (player.cumQ() < 1000) DisplayText("  Your load is as large as always, and ropes of cum fly into the air, covering her arm and your abdomen... and you swear one string even lands of her right breast along the way.  \"<i>That's it, sweetie, shoot all you have.</i>\"");
					else DisplayText("  You pretty much cover yourself, Marble, and the surrounding ground with your enormous white load of cum.  She seems numbed from the sheer output you're capable of.  \"<i>Uh... sweetie...</i>\"");
					DisplayText("\n\n\"<i>Well... that was fun.</i>\"  Marble slowly pulls you away from her breast");
					if (player.cumQ() >= 1000) DisplayText(", then wipes the nipple clean on the other");
					DisplayText(".  \"<i>Now, sweetie, if you'd be so kind to suck on the other one...</i>\"");
				}
				//[Female] 
				else {
					DisplayText("\n\nHer hand moves onto your pleasure button, starting to gently rub your [clit] as her milk flows into your mouth.  When you intensify your suckling, she gasps slightly and slides a finger into your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", pumping it in and out as her palm has its way with your [clit].  You buck your hips slightly but she just strokes your " + Desc.Head.describeHair(player) + " as if to calm you down.");
					DisplayText("\n\n\"<i>That feels good, doesn't it, sweetie?  You should suck my breasts some more, I'd really appreciate that.</i>\"");
					DisplayText("\n\nYou gently moan into her nipple in response, causing her to giggle as she plays with the tip your " + Desc.Vagina.describeClit(player) + ".");
					//([Normal clit] 
					if (player.torso.clit.length < 3) DisplayText("\n\nShe rubs it with care but quite intensely, bringing you a tingling, overwhelming pleasure.");
					...
					DisplayText("\n\nYou're at the edge already, and the fingering of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " as well as the rubbing motion against your [clit] soon send you over it.  Thrashing in Marble's embrace, you release your feminine juices all over her hand.");
					if (player.torso.vaginas.get(0).wetness >= 4) DisplayText("  Messy as it is, Marble never stops, wanting to make the orgasm as intense as possible.");
					//([Squirter]
					if (player.torso.vaginas.get(0).wetness > 4) DisplayText("  Soon, you're shooting your femcum all over her arm, and she giggles at the intensity and visibility of your orgasm.");
			...
			//ORAL
			else if (randInt(2) === 0 && player.gender > 0) {
				DisplayText().clear();
				...
				DisplayText("You walk up to your big-breasted lover, a feverish look in your eyes.  \"<i>Hmm... looks like you need to release some tension, don't you sweetie?</i>\"  You nod in approbation, ");
				if (player.torso.cocks.count > 0) DisplayText("a tent growing in your " + player.inventory.equipment.armor.displayName);
				else if (player.torso.vaginas.count > 0) DisplayText("a moistness growing under your " + player.inventory.equipment.armor.displayName);
				DisplayText(" as you keep staring at her tender eyes; you instinctively know she's going to handle your junk with care.");
				...
				if (player.torso.cocks.count > 0) {
					DisplayText("[eachCock] throbs");
				}
				else {
					DisplayText("your " + Desc.Vagina.describeClit(player) + " quivers in arousal");
				}
				DisplayText(".\n\n");
				...
				if (player.torso.vaginas.count > 0 && player.torso.cocks.count <= 0) DisplayText("probing her tongue in your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", licking you in the most appropriate places");
				else DisplayText("wrapping her tongue around your rod of dickflesh, squeezing and pumping beads of pre-cum up your shaft");
				DisplayText(".  She's doing an amazing job, and you thrash wildly under her smothering tongue assaults.");
				//([vag only/has a cock]
				DisplayText("  Marble keeps ");
				if (player.torso.vaginas.count > 0 && player.torso.cocks.count <= 0) DisplayText("poking your cunt");
				else DisplayText("suckling the juices out of your urethra and literally absorbing the drops of pre-cum as they spurt out of the tip of your junk.");
				//[if multicock]
				if (player.torso.cocks.count > 1) DisplayText("  She alternatively sucks and jerks every cock you have, making sure there isn't a single meat-tower that isn't being taken care of.");
				if (player.torso.balls.quantity > 0) DisplayText("  Her hands softly rub your [balls], making them churn and swell in pure arousal.");
				...
				if (player.gender === Gender.FEMALE) DisplayText("tongue-fuck");
				else DisplayText("deepthroat");
				DisplayText(" you.  Your hips thrust back and forth, enjoying the fluid motion of her tongue working over your wet, saliva-slathered genitals.");
				//[if cock] 
				if (player.torso.cocks.count > 0) {
					DisplayText("  Her lips tightly enclose your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)));
					if (player.torso.cocks.count > 1) DisplayText(" while she strokes another");
					DisplayText(", acting as an organic cock-ring.  She looks up to you as you watch her lips undulating under the repeated pulsations of your spooge-pumping " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  These cocksuckers are teasing you in the most infuriating way, and you savagely shove more of your prick down her throat, stretching her jaw and making her moan in delight, although her cries of pleasure are muffled by the pecker stuffing her mouth.");
				}
				//[if vagina]
				else DisplayText("  Her flexible, yet firm tongue really feels like a dong and it keeps provoking you with its prolonged digging. With a mighty thrust, you shove her against your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", forcing the entirety of her oral muscle inside.");
				...
				if (player.torso.cocks.count > 0) {
					...
					if (player.perks.has(PerkType.MarblesMilk)) DisplayText("usually");
					else DisplayText("used to");
					DisplayText(" suckle her tits.");
					//[if high cum production] 
					if (player.cumQ() >= 500) DisplayText("  Your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " keeps spouting more seed inside her, efficiently distending her belly");
					//[if cum production is massive]until she looks ready to give birth.[/]
					if (player.cumQ() > 1500) DisplayText(" until she looks ready to give birth.");
					DisplayText("  Her lips cling to your cum-tube as if her life depended on it; she sucks viciously, milking your junk of every drop of its baby-batter.");
				}
				//[if vagina]
				else DisplayText("\n\nYour " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " quivers and vibrates before releasing a vigorous discharge of fem-spunk, splattering Marble's lips and mouth; she drinks all your juices with an insatiable cum-thirst.  Her tongue explores your insides as if to make sure there's no girlcum left to absorb.");
				...
				DisplayText("\"<i>You feeling antsy, sweetie?</i>\" Marble asks as you walk up to her.  \"<i>Don't worry; I'm ready and able to help.</i>\"  ", true);
				//Select one option based on the sum of the player's and Marble's Corr, similar to the one in regular masterbation
				if (player.stats.cor + player.statusAffects.get(StatusAffectType.Marble).value4 < 66) {
					DisplayText("The two of you find a secluded place in the camp and slip off your clothes.\n\n");
				}
				else if (player.stats.cor + player.statusAffects.get(StatusAffectType.Marble).value4 < 132) {
					DisplayText("The two of you strip down right there in the middle of camp.\n\n");
				}
				else {
					DisplayText("You both look around for another member to join in. No one jumps at the offer, so the two of you strip down.\n\n");
				}
				//all the foreplay here
				DisplayText("Marble smiles at you and playfully pushes her sizable breasts together a few times before ");
				...
				if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 3) {
					DisplayText("pressing them against your own " + Desc.Breast.describeBreastSize(character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + ".  She giggles slightly as she puts one of them between her mounds and rubs them up and down around it, teasing your " + Desc.Breast.describeNipple(character, character.torso.chest.get(0)) + " with her own " + marbleNips() + ", a trickle of milk leaking out onto your locked chests.");
				} else {
					DisplayText("pressing them against your chest.  She rubs them up and down against you as she moves around, making sure she has rubbed each part of your upper body at least three times over.  She is especially happy with moving your head around between her breasts.");
				}
				//If Player has at least one dick, Marble titfucks that
				if (player.torso.cocks.count > 0) {
					DisplayText("\n\nShe moves down to your crotch and slips your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " between her breasts.  She smiles at you and says, \"<i>This is what you were hoping for, right?</i>\" before squeezing them together tightly and pushing your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " in and out.  You sigh in pleasure and smile down at Marble as she pumps you between her breasts.  But it seems Marble is unsatisfied with only that; she quickens her pace and starts moving each of her breasts up and down opposite each other.");
					//Multiple dick side note
					if (player.torso.cocks.count > 1) {
						DisplayText("  Yet still unsatisfied, Marble engulfs and rub the rest of your " + Desc.Cock.describeMultiCockShort(player) + " within her ample mounds.  ");
					}
				}
				//If Player has a vagina, Marble titfucks that
				if (player.torso.vaginas.count > 0) {
					DisplayText("\n\nShe slides down your body to rub and caress your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " with her breasts, sometimes pushing one of her " + marbleNips() + " inside your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " and pulling a small gasp from you.  ");
					//If clit is too small to be titfucked...
					if (player.torso.clit.length < 2) {
						DisplayText("  Marble runs down each of your legs once with her breasts before unexpectedly plunging two of her fingers inside your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".");
					}
					else {
						DisplayText("  Marble then pulls your " + Desc.Vagina.describeClit(player) + " between her breasts and does her best to pump and stimulate it as though it were a cock.");
					}
				}
				//If Player is genderless, Marble improvises
				if (player.gender === Gender.NONE) {
					DisplayText("  She then says, \"<i>I think I've found a way to make you cum, sweetie,</i>\" with a sly expression.  She moves down from your upper body and starts rubbing and caressing your lower body too. It is extremely arousing, but it doesn't seem to be enough to push you over the edge.  Marble stands up, kisses you and rubs a single finger against the smooth skin of your groin.  You are so close to going over the edge.  Then one of her fingers pushes up against your " + Desc.Butt.describeButthole(character.torso.butt) + ", causing you to go rigid from the new stimulation.");
				}
				//Sweet release
				...
				if (player.torso.cocks.count > 0) {
					...
					if (player.cumQ() < 25) DisplayText("  In moments, a small amount of your cum is covering Marble's breasts.");
					else if (player.cumQ() < 250) DisplayText("  After a few moments, a fair amount of your cum is covering Marble's breasts and face.");
					else if (player.cumQ() < 500) DisplayText("  You keep cumming and cumming, until eventually Marble is completely drenched in your cum.");
					else DisplayText("  Your seemingly endless orgasm eventually results in the two of you becoming completely drenched in your cum, as well as a large part of the ground around you.");
				}
				...
				if (player.torso.vaginas.count > 0) {
					...
					if (player.torso.vaginas.get(0).wetness < VaginaWetness.WET) DisplayText("  As you recover, you see that a small amount of your girly fluids has leaked onto Marble's breasts.");
					else if (player.torso.vaginas.get(0).wetness <= VaginaWetness.SLICK) DisplayText("  As you recover, you see that Marble has been covered fairly liberally with your girly fluids.");
					else if (player.torso.vaginas.get(0).wetness < 5) DisplayText("  As you recover, you see that Marble is decently drenched in your girly fluids.");
					else DisplayText("  It takes you a minute to recover, but upon doing so, you blush and realize just how soaked with fluid you've become.");
				}
				//finally some philosophy on what an orgasm would feel like for someone who is genderless
				if (player.gender === Gender.NONE) DisplayText("  It's not quite the same as when you had genitals, but it is undeniably an orgasm.  While it is essentially a pointless orgasm, it's also almost pure in a way.  There is not a feeling inside you but the explosion of pleasure.");
		...
		//NAGAS
			...
			if (player.stats.lib >= 50) DisplayText("  Quietly, you reach down and hook your fingers into the hem of her clothing, allowing her to pull it half off with her next shift and expose her pussy");
			if (Flags.list[FlagEnum.MARBLE_DICK_LENGTH] > 0) DisplayText(" and " + marbleCock());
			DisplayText(".  \"<i>Why, you shameless " + Desc.Gender.mf(player, "pervert", "hussy") + "!</i>\" the cow-girl says in mock-offense, winking at you.");
			...
			if (player.gender > 0) {
				...
				if (player.torso.vaginas.count > 0) DisplayText(Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)));
				DisplayText(".  \"<i>Well, sweetie, looks like I don't have to go digging for treasure; it's already right here for me,</i>\" she whispers to you.  With one hand she ");
				if (player.torso.cocks.count > 0) DisplayText("grabs your shaft");
				else DisplayText("penetrates your pussy");
				DisplayText(", while using her long tongue to lick the ");
				if (player.torso.cocks.count > 0) DisplayText("tip");
				else DisplayText("clit");
				DisplayText(".  You cry out in pleasure, and start to wrap your long tail about Marble's shapely body.  You can't hold back for long, and soon reach orgasmic bliss, all while tightly clinging to your cow-girl lover.");
				if (player.torso.cocks.count > 0) DisplayText("  [EachCock] unleashes a load of semen right into her mouth, which she swallows with pink-faced, wanton gusto.");
				if (player.torso.vaginas.count > 0) DisplayText("  Your [pussy] runs and squirts as you moan, drooling your juices into the little valley where Marble's breasts press against your scales.");
			}
			else {
				DisplayText("\n\nWhen she reaches where your genitals would be - if you had any - you're panting with pleasure.  Marble gives you a smile and whispers, \"<i>Don't worry sweetie, I know just how to make you cum.</i>\"  She then hugs your body tightly, reaching her arms around and squeezing and slapping your [butt], while slipping a finger into your " + Desc.Butt.describeButthole(character.torso.butt) + ".  You gasp at the surprise, but are soon panting even harder.  The fact that she is still rubbing her breasts on your abdomen even as she teases you helps even more, and you can't help but coil your lower body around Marble, rubbing every part of it on her.  In moments, you cry out in bliss as your unusual anal orgasm washes over you.");
			}
	...
	private marbleNips(): string {
		if (!player.statusAffects.has(StatusAffectType.MarbleSpecials)) {
			player.statusAffects.add(StatusAffectType.MarbleSpecials, 0, 1, 0, 0);
		}
		//Set nipples to 1 if uninitialized
		if (player.statusAffects.get(StatusAffectType.MarbleSpecials).value2 === 0) player.changeStatusValue(StatusAffects.MarbleSpecials, 2, 1);
		if (player.statusAffects.get(StatusAffectType.MarbleSpecials).value2 === 1) return "nipples";
		if (player.statusAffects.get(StatusAffectType.MarbleSpecials).value2 === 4) return "quad-nipples";
	...
	private rapeDAHMARBLEZ() {
		...
		DisplayText().clear();
		DisplayText("You decide that rather than helping her, you are going to roughly manhandle her breasts and rape her.  You suddenly grab at her breasts and squeeze them roughly, at which point she screams and ");
		...
		if (raped) {
			...
			//wrapping things up
			DisplayText("You close your eyes and revel in the moment before feeling Marble roll over and grab one of your hands.  You open your eyes just in time to see a big hammer-head flying towards your face...  ");
		...
		player.statusAffects.add(StatusAffectType.MarbleRapeAttempted, 0, 0, 0, 0);
	}
	...
	private marbleRapeCorruptFantasy() {
		DisplaySprite(41);
		...
		if (player.stats.cor >= 66) {
			DisplayText("You shake your head, clearing the bizarre fantasy from your mind and returning to the more immediate enjoyment.\n\n");
		}
		else {
			DisplayText("You smile to yourself and think, \"<i>One day,</i>\" before returning to the more immediate enjoyment.\n\n");
		}
	}

	//Follow up fight
	//player spots Marble while exploring the farm
	public marbleAfterRapeBattle() {
		DisplaySprite(41);
		DisplayText("While exploring the farm, you notice the cow-girl that hit you earlier, Marble, coming out of the barn.  You could try confronting her if you want to, or you could just avoid her from now on.", true);
	...
	private marbleAfterRapeStartFight() {
		DisplaySprite(41);
		DisplayText("You drop into your own combat stance; it's time to get even with her for last time.  ", true);
		//Do battle with Marble
		startCombat(new Marble(), true);
	}

	//if the player leaves, that's it
	private marbleAfterRapeNo() {
		DisplaySprite(41);
		DisplayText("You shake your head. It's just not worth the headache to deal with this cow.  You turn around and leave; you aren't going to be seeing her anymore.", true);
		return { next: Scenes.camp.returnToCampUseOneHour };
	}


	public marbleBadEndFollowup() {
		DisplaySprite(41);
		DisplayText().clear();
		//Variables for this function:
		//morph – keeps track of player's form (human, dog-morph, centaur)
		let morph: string = "human";
		if (player.hips.legs.type === LegType.CENTAUR) morph = "centaur";
		if (RaceScore.dogScore(player) >= 4) morph = "dog-morph";
		if (player.horseScore() >= 3) {
			if (player.hips.legs.type === LegType.CENTAUR) morph = "centaur-morph";
		...
		DisplayText("At that moment, an earthly female voice calls out from inside the barn.  \"<i>Sweetie, is that someone you know?</i>\"  The older champion smiles and replies to the voice, \"<i>Yes Marble, it is.  You should come out and meet him.</i>\"  <i>\"Alright,</i>\" the voice calls back.  ");
		//could the player get Marble pregnant?
		if (player.gender === Gender.MALE || player.gender === Gender.HERM) {
			DisplayText("A tall female cow-girl then steps out of the barn entrance.  The young champion notes just how pretty she is, if a bit imposing, but he also quickly notices that she is quite clearly pregnant.  \n\n");
			DisplayText("<i>\"Marble,</i>\" the older champion says to her, \"<i>this is a friend, he's actually from my village.  This is Marble, she is the reason I left my mission.  I care about her too much, and she is carrying my child.</i>\"  ");
			//mysterious baby?
			if (player.gender === Gender.HERM && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2 && player.torso.cocks.sort(Cock.LargestCockArea)[0].area < 15) {
				DisplayText("\"<i>Wait, your child?!  How?</i>\" the younger champion stammers. The older one grins at him and says, \"<i>So I'm the first you've met?  Well then...</i>\" before dropping her pants and revealing her " + Desc.Cock.describeMultiCockShort(player) + " and her " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " to him.  The younger champion's jaw drops and he simply stares at her for a time, during which she giggles at him and says, \"<i>They're fully functional too.  Would you like to see them in action?</i>\"  The younger champion finds he is only able to stammer out an incoherent answer to this.  \"<i>I think I'll take that as a 'yes'. Don't worry, it'll be fun.</i>\" the older champion says, leading the young one along with Marble back inside the barn.  \"<i>Afterwards, maybe I can give you some advice on dealing with this world.</i>\"  ");
			}
			...
			DisplayText("A tall female cow-girl then steps out of the barn entrance.  The young champion notes just how pretty she is, if a bit imposing.\n\n");
		...
		if (player.statusAffects.get("$1").value3 === 1) {
			DisplayText("After a moment, \"<i>By the way,</i>\" Marble's voice can be heard from inside saying, \"<i>do you like milk?\"</i>");
			//dun dun dun!  That was dumb; I apologize for the lame joke.
		}
		//wrap things up
		DisplayText("\n\n");
		DisplayText("You've abandoned your quest due to your inability to refuse Marble, thanks to the effect her milk has on you.  However, you may have a chance to help the next one complete his quest, or maybe you won't.  As it stands now, this is where your story ends.");
		Game.gameOver();
	}

	//BIRTHING OVERWRITES SECKS
	//if(Flags.list[FlagEnum.MARBLE_PREGNACY_INCUBATION] === 1)
	public marblePoopsBaybees() {
		//Normal shitz
		if (pregnancy.type === PregnancyType.PLAYER) {
			...
			if (Flags.list[FlagEnum.MARBLE_PURIFIED] > 0 && randInt(2) === 0) {
				...
				if (Flags.list[FlagEnum.MARBLE_BOYS] === 0) {
					...
					DisplayText("You ask her why she hesitated like that.  \"<i>Oh?</i>\" she starts, looking up at you, \"<i>Sorry sweetie, it's just that I never thought I would give birth to a boy.  It just never occurred to me.</i>\"  Then her expression changes.");
				}
				else {
				...
				if (Flags.list[FlagEnum.MARBLE_DICK_TYPE] > 0) DisplayText(", but you notice that she does not have a cock of any kind. It seems that trait isn't passed on");
				DisplayText(".  The little girl's face is a really pretty one; you're sure that she'll grow up to be like her mom.  You hand Marble the child and she puts the crying child to her chest. The little girl stops crying at once and starts eagerly gobbling down Marble's milk.\n\n");
				//If (PC is addicted to Marble)
				if (player.perks.has(PerkType.MarblesMilk)) {
					DisplayText("\"<i>Don't worry sweetie,</i>\" Marble tells you, \"<i>somehow I know that she won't get addicted.\"</i>  ");
				}
				else {
					DisplayText("\"<i>Oh my,\"</i> Marble says to you, \"<i>It's just as wonderful as when you suckled me when my milk was addictive; I'd forgotten the feeling.\"</i>  ");
				}
				DisplayText("Marble looks at her other breast a moment before looking at you and saying \"<i>Still, I think I could use you on the other side.</i>\" You oblige her by suckling her other breast.\n\n");
				//since the PC must either be addicted, or have removed Marble's addictive trait in order to father a child with her, there is no need for a check for addiction here!
				DisplayText("Once the two of you have had your fill, Marble puts the child inside the nursery.  The little girl is already starting to look like she is a few years old, and is already trotting around on her little hoofs.  Marble turns to look at you and asks \"<i>Ok, I'll name her ");
				...
				if (Flags.list[FlagEnum.MARBLE_KIDS] === 0) {
					...
					DisplayText(", yes, that's a good name for her.</i>\"");

				}
				else {
					DisplayText("...hmm, well Sweetie, I can't think of a good name right now, I'll figure one out tomorrow.</i>\"");
				}
			}
		...
		//Eggs
			...
			DisplayText("Marble stands up and gives her legs a stretch before clopping off.  You shrug your shoulders and look at the pile of eggs before grabbing the nicest looking one.\n\n");
			//Gain appropriate egg
			//default
			let itype: ItemType;
			//Large eggs
			if (randInt(3) === 1) {
				itype = consumables.LARGE_EGGS[randInt(consumables.LARGE_EGGS.length)];
			}
			//Small eggs
			else {
				itype = consumables.SMALL_EGGS[randInt(consumables.SMALL_EGGS.length)];
			}
			if (timeQ > 0)
	...
	public marbleNightSleepFlavor(): boolean {
		...
		if (Flags.list[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] === 0 && player.pregnancyType === PregnancyType.MARBLE && (player.pregnancyIncubation <= 128)) {
			DisplayText("<b>Citing your pregnant belly, Marble informs you she'll be getting to work on building a nursery for your coming cow-child soon.</b>\n\n");
			Flags.list[FlagEnum.MARBLE_NURSERY_CONSTRUCTION]++;
		}
		//PREGGO ALERTZ
		switch (pregnancy.eventTriggered()) {
			case 2: DisplayText("<b>You notice that Marble seems to have gained some weight.</b>\n\n"); //3 days in
				break;
			case 3: DisplayText("<b>Marble's belly has gotten a fair bit bigger; she may be pregnant.</b>\n\n"); //8 days in
				break;
			case 4: DisplayText("<b>Marble's belly has gotten bigger; she is obviously pregnant."); //12 days in
				...
				if (Flags.list[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] === 0 && pregnancy.type != PregnancyType.OVIELIXIR_EGGS) {
					Flags.list[FlagEnum.MARBLE_NURSERY_CONSTRUCTION]++;
					DisplayText("  She lets you know that she'll be busy working on a nursery for her child for a while.");
				}
				DisplayText("</b>\n\n");
				break;
			case 5: DisplayText("<b>Marble's pregnancy has advanced further still, though the structure of her body keeps it from slowing her down.</b>\n\n"); //18 days in
				break;
			case 6: DisplayText("<b>Marble is probably getting close to giving birth, as her belly has gotten very large.</b>\n\n"); //24 days in
			default:
		}
		...
		else if (Flags.list[FlagEnum.MARBLE_LUST] < 50 || player.gender === Gender.NONE) {
			DisplayText("As you go to lie down for the night, you find that Marble has gotten there before you.  ");
			if (Flags.list[FlagEnum.MARBLE_LUST] < 0) DisplayText("She makes a point of turning to face away from you when you climb into your bedroll.  Looks like she's still annoyed at you.");
			else if (Flags.list[FlagEnum.MARBLE_LUST] < 20) DisplayText("She lies next to you contentedly when you climb into your bedroll.");
			else if (Flags.list[FlagEnum.MARBLE_LUST] < 30) DisplayText("Marble gives a soft sigh when you climb into your bedroll.");
			else if (Flags.list[FlagEnum.MARBLE_LUST] < 40) DisplayText("Marble fidgets around a bit when you climb into your bedroll.");
			else DisplayText("Marble seems rather worked up when you climb into your bedroll.");
			if (player.stats.lust < 33 && Flags.list[FlagEnum.MARBLE_LUST] < 20) DisplayText("  After a few moments of getting comfortable, you drift off to sleep.");
			else if (player.stats.lust < 33) {
				DisplayText("  You wonder for a moment if Marble is maybe looking for something more from you.");
				//(50% chance of this happening)
				if (randInt(2) === 0) {
					marbleCuddlin();
				}
				else DisplayText("  However, you're not aroused enough to consider sex.");
			}
			else {
				DisplayText("  Do you ask Marble if she'd like to have sex?");
				//[Ask][Don't Ask]
		...
		//Replaces laying down texts when Marble is at or over 50 lust
			...
			DisplayText("You lie down and notice that Marble hasn't gone to bed yet.  You decide not worry about it too much until you hear Marble come clopping up outside.  \"<i>Sweetie?</i>\" she calls out to you as she crawls in next to you.  \"<i>Sweetie, I'm feeling really horny ");
			//if (PC's lust is under minimum to request sex)
			if (player.stats.lust < 33) {
				DisplayText("and I know you really aren't in the mood, but could you please help me out?</i>\"  You sit up and ask her what it is.  She looks at you slightly embarrassed before asking you, \"<i>Could you please ");
				//If (Marble is a herm)
				if (Flags.list[FlagEnum.MARBLE_DICK_TYPE] > 0)
					DisplayText("suck my " + marbleCock() + "?  I really, really need it.</i>\"");
				else
					DisplayText("lick my womanhood?  I really want to feel your tongue bring me to release...</i>\"");
				...
				if (player.torso.hips.legs.isNaga() && Flags.list[FlagEnum.MARBLE_DICK_LENGTH] > 0) tailJob = giveMarbleTailjobRelease;
				MainScreen.simpleChoices(["Yes", "No", "Tailjob", "", ""], [marbleNomNoms, playerMenu, tailJob, null, null]);
			}
			else {
				DisplayText(".  I can tell you want this too, so can we please have sex?</i>\"  You sit up and consider her request.  Will you have sex with Marble?  If not, you could always offer to give her oral instead.");
				//options: yes/no/oral (yes continues to the next part of the scene as it did before.
				MainScreen.simpleChoices(["Yes", "No", "Oral", "", ""], [marbleNightSexIntro, playerMenu, marbleNomNoms, null, null]);
			}
			return true;
		}
		//END NEW NIGHT LOGIC
	...
	private marbleCuddlin() {
		DisplayText("  Before too long, Marble reaches over and ");
		...
		if (!pcPregWithMarblesKids() && !marblePregWithPCKids()) {
			...
			if (player.tallness <= 78) DisplayText("pulls your head on top of her chest.  She idly strokes your " + Desc.Head.describeHair(player) + " and says, \"<i>Sweetie, how about I be your pillow for the night?</i>\"  You happily agree, and soon you drift off to the sound of her steady breathing while you lay between her sizable breasts.");
			else DisplayText("wraps her arms around you, gently pulling you against her.  \"<i>Sweetie, do you mind if we stay like this for a while?</i>\"  There is no way you could refuse, and before long, the two of you drift off to sleep in each other's arms.");
		}
		//Preg-cuddles
		else {
			...
			//PC be preggers only
				...
				DisplayText("gently pets your swollen belly.  \"<i>Sweetie,</i>\" she whispers, \"<i>I can feel our child.</i>\"  She puts her furry ears to your growing belly, gently rubbing it all the while.  After a few moments she snuggles up against your body and the two of you drift off to sleep.");
			else
				DisplayText("pulls your hand to her belly, and puts her own onto yours.  \"<i>The miracle of two lovers carrying each other's child,</i>\" she says.  It certainly isn't something you ever expected to happen, but that doesn't make it any less wonderful.  The two of you drift off to sleep together, hands on each other.");
		}
	}
	//Ask for sex
	private atNightAskMarbleForSomeSexMaybe() {
		DisplayText().clear();
		DisplayText("You turn to face Marble, and ");
		//if (Marble's lust is negative)
		if (Flags.list[FlagEnum.MARBLE_LUST] < 0) {
			DisplayText("see the annoyed back of her head.  Oh right, she's probably not in the mood for this.  You'll have to find another way of satisfying your needs tomorrow.");
		}
		else if (Flags.list[FlagEnum.MARBLE_LUST] < 20) {
			DisplayText("gently ask her if she wants to fool around.  After a moment you hear her breathe out and she says, \"<i>Sorry sweetie, I'm not feeling up to it right now.</i>\"");
			//Marble cuddles the PC
			marbleCuddlin();
		}
		else {
			DisplayText("gently ask her if she wants to fool around.  After a moment you hear her breathe out and, with a lewd voice, she says, \"<i>I was worried you'd never ask, sweetie...</i>\"");
			//trigger Marble sex
	...
	//SEKCS
	private marbleNightSexIntro(clear: boolean = true) {
		...
		else if (player.gender === Gender.MALE && x.area > marbleCuntCapacity()) {
			DisplayText("You lie down next to her, but you can't help wishing that the two of you could have sex.  Suddenly, you feel Marble's hand grab a hold of your " + Desc.Cock.describeCock(player, x) + ".  She sighs and says, \"<i>Sweetie, I'm afraid that you won't fit inside me.  Do you think you could get a little smaller, just for me?</i>\"  After a moment of getting comfortable, the two of you go to sleep.");
			return { next: playerMenu };
		}
		//Ok lets light this fire!
		DisplayText("At that moment, you feel Marble breathe in your ear, \"<i>I know you want this, don't you sweetie?</i>\"\n\n");
		//If (PC is under 5 feet tall) {
		if (player.tallness < 60) {
			DisplayText("Marble rolls you onto your back and pulls your head up onto her ample cleavage.  \"<i>I want it too. Just relax and let me take care of this.</i>\"  Marble keeps your head locked in her cleavage as she uses her other hand to slowly free the two of you of your garments.\n\n");
			DisplayText("Your head rests comfortably between Marble's breasts the entire time, her warm breath blowing softly on your hair.  You feel almost completely relaxed, yet incredibly turned on at the same time.  For some reason, it just feels right to lay there and breathe in her scent, enjoying the wonderful love pillows she is forcing you to relax between.  She runs her hand down your " + Desc.Breast.describeBreastSize(character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + ", gently cooing to you.  Her hand slips down to your ");
			//If (PC has dick) {
			if (player.torso.cocks.count > 0) {
				DisplayText(Desc.Cock.describeMultiCockShort(player));
			...
			//If (PC has vag) 
			if (player.torso.vaginas.count > 0) DisplayText(Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)));
			DisplayText(" and gently caresses you.  She whispers into your ear, \"<i>Soon sweetie, soon we'll both get what we want.</i>\"\n\n");
			DisplayText("Finished with removing your clothes and her own, Marble releases her grip on your head and turns your body over.\n\n");
		}
		else {
			DisplayText("She reaches over and puts her hand onto your waist, gently slipping her hand inside your undergarments to touch your ");
			//If (PC has dick) {
			if (player.torso.cocks.count > 0) {
				DisplayText(Desc.Cock.describeMultiCockShort(player));
				//If (PC both dick and vag) 
			...
			//If (PC has vag) 
			if (player.torso.vaginas.count > 0) DisplayText(Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)));
			DisplayText(" while you slip your own hand into her clothes and ");
			//If (Marble is a herm) {
			if (Flags.list[FlagEnum.MARBLE_DICK_TYPE] > 0) {
				DisplayText("gently rub her " + marbleCock() + ".  After a moment, you move your hand down lower and ");
			}
			DisplayText(" run your fingers down her moist womanhood.  The two of you quiver under each other's ministrations.  Both of you work to quickly free yourselves from your clothing, growing increasingly excited at where things are going.  Free of your garments, you move overtop of Marble.\n\n");
		}
		//If (player is a herm, and their dick can fit inside Marble)
		if (player.gender === Gender.HERM) {
			if (x.area <= marbleCuntCapacity()) {
				DisplayText("\"<i>So sweetie, what part will you be using today?</i>\" she asks you.");
				//Player chooses if they want to use their dick or their vagina.
				MainScreen.simpleChoices(["Dick", "Vagina", "", "", ""], [marbleNightSexDudes, marbleNightSexChicks, null, null, null]);
			}
			else {
				DisplayText("Marble says, \"<i>Well sweetie, since that " + Desc.Cock.describeCock(player, x) + " of yours is too big to fit inside me, let's put that " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " to use.</i>\"\n\n");
				//GIRL SECKS GOES HERE
				return { next: marbleNightSexChicks };
			}
		}
		else {
			//Go to next scene based on the PC's and Marble's attributes
			//DUDEZ
			if (player.gender === Gender.MALE) return { next: marbleNightSexDudes };
			//CHICKZ
			else return { next: marbleNightSexChicks };
		}
	}
	//Sex function for if PC is male
	private marbleNightSexDudes() {
		DisplaySprite(41);
		//Try to find a dick that fits.
		let x: number = player.cockThatFits(marbleCuntCapacity());
		if (x < 0) x = 0;
		Flags.list[FlagEnum.MARBLE_LUST] = 0;
		DisplayText().clear();
		DisplayText("You gently lift yourself up and slide the tip of your " + Desc.Cock.describeCock(player, x) + " into her waiting hole.  Ever so slowly, you push yourself further and further in.  Each inch gained brings gentle moans and coos from Marble, encouraging you onward.  ");
		//Can the PC's main cock go all the way?
		//If (cock 0 is longer than 8 inches) 
		if (player.torso.cocks.get(x).length > 8) {
			DisplayText("There is only one thing that will stop your advance: when you reach the end of the line.  \"<i>Keep going sweetie, there is room left.  Make sure you fill me all the way.</i>\"  With a groan, your " + Desc.Cock.describeCock(player, x) + " hits the furthest part of Marble's confines and you can go no further.\n\n");
		}
		//otherwise, Marble doesn't mind, she'll hold all of them instead
		else {
			DisplayText("Nothing will stop your advance; your cock will be fully enveloped inside Marble's body.  \"<i>That's right sweetie... Ah, let me fully envelop you.</i>\"  With a sigh of satisfaction, your bodies come completely together.\n\n");
		}
		DisplayText("You look Marble in the eye and the two of you relish the moment; you are two lovers, connected in the most enjoyable of ways.  ");
		//Is Marble going to take the initiative here?
		if (player.stats.cor + player.stats.lib < player.statusAffects.get(StatusAffectType.Marble).value4 + 10) {
			DisplayText("You get so caught up in the moment that Marble takes the initiative. \"<i>Now, now, it's not nice to keep me waiting, sweetie,</i>\" she says before lifting up your hips and repeatedly inserting you inside her at a fair pace.  You're caught off guard and fall forward, ");
			...
			//nah, but if you have boobies, maybe they hit hers?
				...
				DisplayText("your " + Desc.Breast.describeBreastSize(character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + " pressing against her own lovely pillows.  Marble giggles slightly before giving you a kiss on the lips.  She continues moving your hips the whole time.  Of course, as you start to near your peak, you get your control back and start thrusting into her of your own accord.  You can hear Marble beginning to breathe faster as she moves her hands onto your back.  She holds you tightly and gives you another kiss.\n\n");
			}
			//Fun, the PC's breasts fall onto Marble's Face!  If they have them, anyway...
			else {
				if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 4) {
					DisplayText("landing your " + Desc.Breast.describeBreastSize(character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + " into Marble's face!  You hear a slight muffled cry underneath you as Marble takes her hands off your hips and lifts you up off of her.  ");
				}
				else {
					DisplayText("crashing down on Marble and doing a good job of flattening her breasts under your size.  \"<i>Hey!</i>\" she cries out, letting go of your hips to lift your body off of her.  ");
				}
			}
			DisplayText("You look down at her annoyed expression, before grinning at her and beginning to impale her moist and wonderful snatch yourself.  In a moment, her expression changes from a pouty face to one of ecstasy.  The two of you are nearing your peaks now.\n\n");
		}
		//nope the PC is the one
		else {
			DisplayText("It isn't enough to hold you back for more than a moment, though, and soon you're thrusting deep inside her at a comfortable rhythm for the two of you.  You smile at her and she smiles back as you start fondling and playing with Marble's lovely pillows.  She starts to run her fingers onto her love button, crying out in pleasure as you continue to thrust your " + Desc.Cock.describeCock(player, x) + " inside her.\n\n");
			//If (PC's breasts are B cups or bigger)
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) DisplayText("You pull your hands back up and begin to run them over your " + Desc.Breast.describeBreastSize(character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + ", toying and teasing your nipples as your " + Desc.Cock.describeCock(player, x) + " continues to pound away at Marble's confines.  Marble soon has her hands on the breasts you vacated, clearly enjoying teasing herself just as much as you are.  The two of you cry out in pleasure from the stimulation; you can feel your orgasm fast approaching.\n\n");
			else DisplayText("The sensations are quickly becoming too much for you, and you're unable to do anything but focus on the feeling of your " + Desc.Cock.describeCock(player, x) + " inside her wonderful confines.  Marble grins at your difficulties and teases, \"<i>Am I just too much for you, sweetie?  Ah!</i>\" She gasps; it seems that you're becoming too much for her too.\n\n");
		}
		//ORGASMO
		DisplayText("You gasp as you feel your insides churning and the walls of Marble's sex collapsing tightly down on you.  Once again, you've brought each other to the point of blissful orgasm around the same time.  ");
		//let's talk about the PC's cum production
		if (player.cumQ() < 250) {
			DisplayText("Marble cries out in joy as your " + Desc.Cock.describeCock(player, x) + " unleashes its load within her insides.  ");
		}
		else {
			DisplayText("Marble gasps in both pleasure and pain as you fill up her insides to almost bursting, and large amounts of your jizz spill out around her slit.  ");
		}
		//Now, does the PC have more than one cock?  Cum comes out of those too.
		if (player.torso.cocks.count > 1) {
			DisplayText("Of course, ");
			if (player.torso.cocks.count > 2) {
				DisplayText("the rest of your " + Desc.Cock.describeMultiCockShort(player));
			}
			else {
				DisplayText("your other tool");
			}
			DisplayText(" unleashes its own load, liberally covering both of you.  ");
		}
		//does Marble spray her cock?
		if (Flags.list[FlagEnum.MARBLE_DICK_TYPE] > 0) {
			DisplayText("Trapped between your bodies, Marble's " + marbleCock() + " spasms and sprays the two of you.");
		}
	...
	private marbleNightSexChicks() {
		DisplaySprite(41);
		Flags.list[FlagEnum.MARBLE_LUST] = 0;
		DisplayText().clear();
		if (Flags.list[FlagEnum.MARBLE_DICK_TYPE] === 0) {
			DisplayText("You gently lift yourself up as Marble produces her double dildo and slips it inside her womanhood.  Smiling at her, you slowly lower your body and impale your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " onto the artificial rod.  ");
			//Loosen PC's vagina if they are really tight, also removes virginity
			Mod.Vagina.displayStretchVagina(player, 14, true);
			//(Message about vagina becoming looser)
			DisplayText("After adjusting to the feeling of the shaft inside you, you can't deny what a wonderful feeling it is to be connected like this.  ");
			//Does Marble take the initiative?
			if (player.stats.cor + player.stats.lib < player.statusAffects.get(StatusAffectType.Marble).value4 + 10) {
				DisplayText("Marble gives you a grin and starts lifting your hips up and down, while simultaneously bouncing her own hips.  The combination results in both your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " and Marble's being pistoned by her double dildo.  The two of you start moaning in unison from the overwhelming feelings, leaving you unable to do anything but let her buckle and bounce you around like a rough wagon, and you love every second of it.\n\n");
				DisplayText("\"<i>Oh sweetie, am I going too fast for you?  Here, let me give you a chance to catch your breath.</i>\"  She offers as she slows down her rapid movement, letting your mind settle back into place.\n\n");
				//If (PC's breasts are D cup or bigger)
				if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 4) {
					DisplayText("Marble takes her hands off your hips and lets you start to rise and fall on her in your own time.  She still rises up to meet you as you descend and lowers herself down each time you rise, matching your timing, but her hands have a much more pressing concern: your " + Desc.Breast.describeBreastSize(character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + ".  Marble starts working her hands expertly to tease and play with your " + Desc.Breast.describeBreastSize(character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + ". She seems to know all the right ways to move her fingers, clearly having a lot of experience with her own, and you can't help but moan at the attention.  She giggles and pants in pleasure herself; it seems she may be enjoying this play just as much as you!\n\n");
				}
				else {
					DisplayText("Marble decides it would be a good time to start playing with your " + Desc.Breast.describeBreastSize(character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + " and starts teasing a " + Desc.Breast.describeNipple(character, character.torso.chest.get(0)) + " with one of her hands as the other continues to gently lift you up and down.  You moan a little at her ministrations as she smiles at you and says, \"<i>You know, sweetie, maybe those cute little boobies of yours could stand to get a little bigger.  I'm sure I could do some great things to you then.</i>\" With a grin still on her face, she gasps and the two of you feel your climaxes approaching.\n\n");
				}
				...
				DisplayText("\"<i>Sweetie!</i>\" Marble gasps as you start to play with her breasts.  ");
				//Does Marble help out, or does she go for the PC's breasts?
				if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 4) {
					DisplayText("She reaches her hands up and starts to play with your " + Desc.Breast.describeBreastSize(character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + ", flicking and toying with them at the same pace that you play with hers.  The two of you start a friendly little contest to see who can stimulate the other's chest better; all the while, you continue to rise and fall above her.\n\n");
				}
				else {
					DisplayText("Marble reaches her hands up and grabs onto yours, pulling them down tightly against her chest.  She gets you to play with each of her " + marbleNips() + " in turn, making sure that you flick them and tease them in just the right way to get her to cry out in ecstasy again and again.\n\n");
				}
				DisplayText("Within moments, your rapid pistoning on the dildo is pushing the two of you closer and closer to the edge.  ");
			}
			DisplayText("The two of you cry out in orgasm as your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " clamps down on one end of her dildo and her womanhood clamps down on the other end.  ");
			//Does this PC have a cock too?
			//If (PC has at least 1 cock)
			if (player.torso.cocks.count > 0) {
				DisplayText("At the same time, your " + Desc.Cock.describeMultiCockShort(player) + " sprays its own load onto Marble's chest and stomach.  ");
			}
			DisplayText("The two of you give one more shudder from the wonderful stimulation, before collapsing on top of one another.  ");
		}
		//PC is female, Marble is a herm
		else {
			player.slimeFeed();
			DisplayText("You gently lift yourself up into the air and guide your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " to the tip of her waiting " + marbleCock() + ".  You slowly lower your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " onto her quivering " + marbleCock() + ", gaining inch by inch ");
			//is she satisfied?
			...
			if (player.vaginalCapacity() > Flags.list[FlagEnum.MARBLE_DICK_LENGTH] * Flags.list[FlagEnum.MARBLE_DICK_THICKNESS]) {
				DisplayText("until she is fully within you.  The two of you shudder in pleasure, and Marble says, \"<i>Oh sweetie, you got it all in there.  Being inside another woman like this is something I'll never really get used to, but I can't deny how good it feels.</i>\"\n\n");
			}
			//Marble seems to think you can take more than maybe you should be able to take
			else {
				DisplayText("until you bottom out and can take no more of her.  \"<i>No more?</i>\" Marble asks you a little sadly, \"<i>You sure you can't take a little more?</i>\" she adds as she starts pushing you down onto her " + marbleCock() + ". You are forced to stretch to accommodate more of her and you shudder slightly from the strain.  ");
				//(Message about vagina becoming looser)
				DisplayText("Marble grins at you and says, \"<i>See?  I knew you that you could fit a little more.</i>\"\n\n");
			}
			DisplayText("After adjusting to the feeling of her inside you, you can't deny what a wonderful feeling it is to be connected like this.  ");
			//Does Marble take the initiative?
			if (player.stats.cor + player.stats.lib < player.statusAffects.get(StatusAffectType.Marble).value4 + 10) {
				DisplayText("Marble gives you a grin and starts lifting your hips up and down, while simultaneously bouncing her own hips.  The combination results in your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " being pistoned by her " + marbleCock() + ".  The two of you start moaning in unison from the overwhelming feelings, leaving you unable to do anything but let her buckle and bounce you around like a rough wagon, and you love every second of it.\n\n");
				DisplayText("\"<i>Oh sweetie, am I going too fast for you?  Here, let me give you a chance to catch your breath.</i>\"  She offers as she slows down her rapid movement, letting your mind settle back into place.  ");
				if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 4) {
					DisplayText("Marble takes her hands off your hips and lets you start to rise and fall on her in your own time.  She still rises up to meet you as you descend and lowers herself down each time you rise, matching your timing, but her hands have a much more pressing concern: your " + Desc.Breast.describeBreastSize(character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + ".  Marble starts working her hands expertly to tease and play with your " + Desc.Breast.describeBreastSize(character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + ". She seems to know all the right ways to move her fingers, clearly having a lot of experience with her own, and you can't help but moan at the attention.  She giggles and pants in pleasure herself; it seems she may be enjoying this play just as much as you!\n\n");
				}
			...
			//no, no, the PC is much hornier than she is
				...
				DisplayText("Within moments, your rapid pistoning on her " + marbleCock() + " is pushing the two of you closer and closer to the edge.  ");
			}
			DisplayText("The two of you cry out in orgasm as your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " clamps down on her " + marbleCock() + " as it fills you with its seed.  ");
			//Does this PC have a cock too?
			if (player.torso.cocks.count > 0) {
				DisplayText("At the same time, ");
				if (player.torso.cocks.count > 1) DisplayText("each of ");
				DisplayText("your " + Desc.Cock.describeMultiCockShort(player) + " sprays its own load onto Marble's chest and stomach.  ");
			}
			DisplayText("The two of you give one more shudder from the wonderful stimulation, before collapsing on top of one another.  ");
			//Pregnancy chance for PC, ¼ their fertility
			player.knockUp(PregnancyType.MARBLE, PregnancyType.INCUBATION_MARBLE, 150);
			Mod.Vagina.displayStretchVagina(player, Flags.list[FlagEnum.MARBLE_DICK_THICKNESS] * Flags.list[FlagEnum.MARBLE_DICK_LENGTH], true);
		}
		marbleSexFinish();
	}

	private marblePreggoChance(preggerMult: number) {
		//pregger odds - 20% max
		//5% base + 1% per 25mLz
		...
		if (player.perks.has(PerkType.FertilityPlus)) preggerOdds += 5;
		//If has 'stud perk' almost always get her pregnant
		if (player.perks.has(PerkType.MaraesGiftStud)) preggerOdds += 25;
		preggerOdds *= preggerMult;
		//GET HER PREGNANT
		trace("MARBLE PREGGO ODDS: " + preggerOdds);

		if (randInt(100) < preggerOdds && (player.perks.has(PerkType.MarblesMilk) || Flags.list[FlagEnum.MARBLE_PURIFICATION_STAGE] >= 5)) {
	...
	private marbleSexFinish() {
		...
		player.orgasm();
		player.stats.sens += -1;
		return { next: camp.doSleep };
	}

	//Pleasing Marble
	private marbleNomNoms() {
		DisplaySprite(41);
		Flags.list[FlagEnum.MARBLE_LUST] = 0;
		DisplayText().clear();
		//intro for non corrupt Marble
		//DisplayText("You go to lie down and notice that Marble hasn't gone to bed yet.  You decide not worry about it too much until you hear Marble come clopping up beside you.  \"<i>Sweetie?</i>\" she calls out to you as she crawls in next to you.  \"<i>Sweetie, I'm feeling really horny and I know you really aren't in the mood, but could you please help me out?</i>\"  You sit up and ask her how you can help; you could never refuse her.  She looks at you slightly embarrassed before asking you, \"<i>Could you please ");
		//If (Marble is a herm) {
		if (Flags.list[FlagEnum.MARBLE_DICK_TYPE] > 0) {
			player.slimeFeed();
			//DisplayText("suck my " + marbleCock() + "?  I really, really need it.</i>\"");
			if (player.stats.cor < 30) {
				DisplayText("You aren't really comfortable about doing this, but you can't really turn Marble down.  Especially after you were the one to give her that " + marbleCock() + ".\n\n");
			}
			else {
				DisplayText("Sucking her tasty-looking " + marbleCock() + " sounds like it would be a lot of fun and you have no problem agreeing.\n\n");
			}
			//sucking her cock scene
			DisplayText("Marble lifts up her skirt and gives you a good look at her very erect " + marbleCock() + ".  You position yourself overtop of her and take a deep breath, taking in her smell.  ");
			//is the PC overwhelmed by her animalistic smell?
			if (player.statusAffects.has(StatusAffectType.SlimeCraving) || player.stats.lib + player.stats.cor - player.stats.int > 35) {
		...
		DisplayText("\"<i>Thank you so much sweetie.  Next time if you're in the mood, maybe we can have fun together,</i>\" says Marble.");
		//Increase PC lust by (random number 0-10) + lib/10
		dynStats("lus", randInt(10) + player.stats.lib / 10);
		return { next: playerMenu };
	}

	//Next set of items that can be given to Marble
	//Pure incubus draft
	public MarbleDigsDraftsYo() {
		DisplaySprite(41);
		DisplayText().clear();
		...
		DisplayText("Marble drops the potion and grabs at her crotch.  Turning away from you, she gives an excited cry that is a mix between a moo and a moan as her body convulses slightly.  After a moment, she turns back to you and shows her new 7\" long and 2\" wide human cock.  She gives you a ");
		if (player.statusAffects.get(StatusAffectType.Marble).value4 > 50) DisplayText("big ");
		else DisplayText("nervous ");
		DisplayText("smile.  <b>Marble is now a hermaphrodite.</b>  ");
		return { next: Scenes.camp.returnToCampUseOneHour };
	...
	public MarblepinkEgg() {
		player.inventory.items.consumeItem(consumables.PINKEGG);
		MarblePEggEffects();
	}
	public MarbleLPinkEgg() {
		player.inventory.items.consumeItem(consumables.L_PNKEG);
		MarblePEggEffects();
	}
	private MarblePEggEffects() {
		DisplaySprite(41);
		DisplayText().clear();
		//removes her dick, no other effect.  Can only be given if Marble has a dick.
		DisplayText("You hand Marble the pink egg.  She looks at it thoughtfully for a moment before her eyes light up in recognition.  \"<i>This is one of those magic eggs made from those egg elixirs.  If I remember right, this one removes the male traits of those who eat them.  ");
		if (player.statusAffects.get(StatusAffectType.Marble).value4 > 50) {
			...
			DisplayText("So does this mean you don't want me to have a cock anymore, sweetie?</i>\"  You assure her that yes, you don't want her to have it anymore.  She seems relieved by this and admits, \"<i>Thank you sweetie, I don't think I really liked having it,</i>\" before eating the egg.\n\n");
		}
		DisplayText("She lifts up her skirt and the two of you watch as her " + marbleCock() + " is absorbed back into her body.  <b>Marble is now purely female.</b>  ");
	...
	//Ovi-elixir
	public marbleOvulatesLikeMadDawg() {
		DisplaySprite(41);
		DisplayText().clear();
		...
		if (Flags.list[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] < 100 && pregnancy.type === PregnancyType.PLAYER) {
			DisplayText("Marble refuses to take the elixir to speed up her pregnancy until after she has finished the nursery. ");
			return { next: Scenes.camp.returnToCampUseOneHour };
			return;
		}
		else if (pregnancy.isPregnant) {
			DisplayText("After taking the elixir, Marble's belly rumbles slightly.  You can both tell that her pregnancy has advanced.  ");
			let newIncubation: number = pregnancy.incubation - (40 + randInt(30));
			if (newIncubation <= 0) newIncubation = 1;
			pregnancy.knockUpForce(pregnancy.type, newIncubation);
		}
		else {
			DisplayText("After taking the elixir, Marble tells you that her belly feels full.  It sounds like she'll be laying eggs soon.  ");
	...
	*/
	...
	private giveMurbleProBova() {
		...
		if (Flags.list[FlagEnum.MARBLE_BOVA_LEVEL] === 0) {
			...
			DisplayText("You hand Marble another bottle of the ProBova.  She studies your face seriously for a few moments before saying, \"<i>Now, you're absolutely sure that you want me to drink this?  Remember that whatever happens will likely be permanent.</i>\"");
			//player chooses yes/no
			doYesNo(giveMarbleTheProBovas4Sho, giveItem);
		}
	}
	//if yes
	public giveMarbleTheProBovas4Sho() {
		DisplayText().clear();
		DisplaySprite(41);
	...
	//Appearance
	private marbleAppearance() {
		DisplayText().clear();
		DisplaySprite(41);
		//Gives Marble's appearance screen, some of these values change depending 
		//on her level of corruption.
		DisplayText("Marble is a ");
		if (Flags.list[FlagEnum.MARBLE_BOVA_LEVEL] === 0) DisplayText("6 foot 4 ");
		else if (Flags.list[FlagEnum.MARBLE_BOVA_LEVEL] === 1) DisplayText("6 foot 8 ");
		else DisplayText("7 foot 4 ");
		DisplayText("inch tall cow-girl, and while she isn't very fast, she has very strong muscles; though they are difficult to spot under her skin's layer of fat. <b>She is currently wearing farm-hand clothes and using a large two-handed hammer as a weapon.</b>  ");
		if (Flags.list[FlagEnum.MARBLE_BOVA_LEVEL] <= 1) DisplayText("She has a fairly normal soft human face, with light-colored skin.  ");
		else DisplayText("She has a soft face that is a mix of bovine and human features, with thick brown fur covering her body.  ");

		DisplayText("Her shoulder-length brown hair is parted by a pair of rounded cow-ears that stick out sideways from her head.  ");

		if (player.statusAffects.get(StatusAffectType.Marble).value4 <= 20) DisplayText("Two small horns grow from her forehead, similar in size and appearance to those on a young female bovine.  ");
		else if (player.statusAffects.get(StatusAffectType.Marble).value4 <= 50) DisplayText("Two medium sized horns grow from her forehead, similar in size and appearance to those on a female bovine.  ");
		else DisplayText("Two fairly large horns grow from her forehead, similar in appearance to those on a female bovine.  ");
		...
		if (Flags.list[FlagEnum.MARBLE_BOVA_LEVEL] <= 1) DisplayText(", human until about half-way down her thigh.  The lower portion of her legs is covered in thick dark brown fur and ends in a pair of bestial hooves.\n\n");
		else DisplayText(" that are oddly jointed and end in a pair of bestial hooves.\n\n");

		if (player.statusAffects.get(StatusAffectType.Marble).value4 <= 15) DisplayText("She has two pillowy breasts, each supporting a 0.4-inch lactating " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + " bra.\n\n");
		else if (player.statusAffects.get(StatusAffectType.Marble).value4 <= 25) DisplayText("She has two large breasts, each supporting a 0.6-inch lactating " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + ".\n\n");
		else if (player.statusAffects.get(StatusAffectType.Marble).value4 <= 35) DisplayText("She has two basketball-sized breasts, each supporting a 0.8-inch milk-seeping " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + " bra.\n\n");
		else DisplayText("She has two basketball-sized breasts, each supporting a 1-inch milk-seeping " + marbleNip() + ".  She could easily fill an " + marbleBreastSize() + " bra.\n\n");
		...
		switch (pregnancy.event) {
			case 2: DisplayText("It looks like she's put on some weight since coming to your camp.\n\n"); //3 to 8 days in
				break;
			case 3: DisplayText("Her belly has swollen a bit; she may be pregnant.\n\n"); //8 to 12 days in
				break;
			case 4: DisplayText("Her belly is obviously swollen; she is almost certainly pregnant.\n\n"); //12 to 18 days in
				break;
			case 5: DisplayText("Her belly is very swollen; she is very pregnant.\n\n"); //18 to 24 days in
				break;
			case 6: DisplayText("Her belly is extremely swollen and occasionally quivers when whatever she is pregnant with moves around.\n\n"); //24+ days in
			default:
		}

		if ((Flags.list[FlagEnum.MARBLE_PURIFICATION_STAGE] < 5 && Flags.list[FlagEnum.MARBLE_LUST] >= 20) || (Flags.list[FlagEnum.MARBLE_PURIFICATION_STAGE] >= 5 && Flags.list[FlagEnum.MARBLE_TIME_SINCE_NURSED_IN_HOURS] >= 4)) {
			DisplayText("<b>Marble is fidgeting around uncomfortably, perhaps she needs to be milked?</b>\n\n");
		}

		if (Flags.list[FlagEnum.MARBLE_DICK_TYPE] > 0) DisplayText("She has grown a " + marbleCock() + " since you brought her to camp. It is " + numToCardinalText(int(Flags.list[FlagEnum.MARBLE_DICK_LENGTH])) + " inches long and " + numToCardinalText(int(Flags.list[FlagEnum.MARBLE_DICK_THICKNESS])) + " inches thick.\n\n");

		if (player.statusAffects.get(StatusAffectType.Marble).value4 <= 50) DisplayText("She has a pussy, with a 0.5 inch clit.\n\n");
		else if (player.statusAffects.get(StatusAffectType.Marble).value4 <= 75) DisplayText("She has a cunt, with a 0.6 inch clit.  You can see moisture gleaming from it.\n\n");
		else DisplayText("She has a fuck-hole, with a 0.7 inch clit.  Moisture gleams in her cunt, its lips slightly parted.\n\n");
		//since I assume Marble may become ass fucked in the future, Boolean Marble.analVirgin:
		if (player.statusAffects.get(StatusAffectType.MarbleSpecials).value1 === 0) DisplayText("She has one virgin bum-hole, placed between her squeezable butt-cheeks where, you presume, it belongs.");
		else DisplayText("She has one bum-hole, placed between her squeezable butt-cheeks where, you presume, it belongs.");
		return { next: interactWithMarbleAtCamp };
	}
	...
	//- PC is a naga
	//- Marble has a cock
	private giveMarbleTailjobRelease() {
		DisplayText().clear();
		DisplayText("You tell Marble that while you're not in a mood to suck her off today, you're not just going to leave her alone with it, either, and ask for her permission to do an exotic alternative.  Slowly, she nods, apparently not so sure about what you mean or whether she should agree.");
	...
	private milkMarble() {
		...
		if (Flags.list[FlagEnum.MARBLE_MILKED_BEFORE] < 1) {
			...
			DisplayText("[pg]There is no harm in checking out her stall, at least.  You agree to go with her.");

			DisplayText("[pg]The two of you hold hands and quickly warp over to Whitney's farm and, with a wave to the proprietor, Marble escorts you to the barn.  The smells of the place are as familiar as ever, but this particular trip has an unusual feel to it since you're going to visit a room you've never been in before.");
			...
			DisplayText("[pg]You apologize for bringing it up and suggest that maybe you should get down to the milking?");

			DisplayText("[pg]She perks up instantly and pulls down ");
			if (Flags.list[FlagEnum.MARBLE_BOVA_LEVEL] < 1) DisplayText("two");
			else DisplayText("eight");
			DisplayText(" suction cups off of the milker machine and carefully attaches them to her nipples, then flips a switch, causing it to come to life.  In an instant, milk starts flowing out of her " + marbleNips() + " into the cups and then up the clear tubes into the machine.  She lets out a relaxed sigh, then turns her gaze back to you.");

			DisplayText("[pg]\"<i>Well sweetie, is there something you've got in mind you'd like to do?</i>\"");

			Flags.list[FlagEnum.MARBLE_MILKED_BEFORE] = 1;
		}
		else {
			DisplayText("Sensing that Marble looks like she is going to get milked soon, you offer to escort her to her stall personally.");

			DisplayText("[pg]As usual, Marble is pleased at the prospect and agrees instantly, and the two of you head over to her small homely space in Whitney's operation.");

			DisplayText("[pg]Once Marble has herself settled in and hooked up, she turns to you and asks if you've got something in mind.");
		}
		...
		if (player.cockThatFits(marbleCuntCapacity()) >= 0 && (player.torso.hips.legs.isBiped() || player.torso.hips.legs.isNaga() || player.torso.hips.legs.isGoo())) MainScreen.addButton(1, "Fuck Her", milkMarbleFuckDatCowPussy);
		if (player.torso.vaginas.count > 0 && Flags.list[FlagEnum.MARBLE_DICK_TYPE] > 0 && (player.torso.hips.legs.isBiped() || player.torso.hips.legs.isGoo())) MainScreen.addButton(2, "Take Her", milkMarbleTakeHerDick);
		MainScreen.addButton(3, "Her on Bar", milkMarbleOnTheBar);
		MainScreen.addButton(4, "Leave", milkMarbleNoMilking);
	}


	private milkMarbleCunnilingling() {
		DisplayText().clear();

		DisplayText("You suggest that maybe she'd like it if you licked her vagina while being milked.");

		DisplayText("[pg]A huge smile spreads across her face, \"<i>Oh sweetie, that sounds like it would be wonderful!  You're the best; you know that?</i>\"");

		DisplayText("[pg]She takes a seat on the edge of the chair in the middle of the room and spreads her legs wide.  Her skirt slips up her ");
		if (Flags.list[FlagEnum.MARBLE_BOVA_LEVEL] < 1) DisplayText("semi-human");
		else DisplayText("inhuman");
		DisplayText(" legs, giving you a clear look at her waiting womanhood.  With a coy sideways glance, she wiggles her fingers in invitation for you to go down on her.");

		DisplayText("[pg]In order to get the show on the road, you ");
		if (player.torso.hips.legs.isBiped()) DisplayText("get down on your knees, and slowly crawl forward");
		else DisplayText("lower your body close to the ground, and slowly inch forward");
		DisplayText(" like an animal who's found something they want.  When you get close to her, your lover runs a finger up the inside of her thigh as her tail sneaks its way up around her waist and drapes itself over your prize.");
		...
		DisplayText("[pg]You take hold of two strands of the bow and tell Marble she shouldn't have given you such a gift.");

		DisplayText("[pg]\"<i>Oh, not at all sweetie.  You deserve it!</i>\"");

		DisplayText("[pg]You slip the bow open, unwrapping your present, and pull her tail aside to reveal her moist and inviting opening and exclaim that it was what you always wanted!");

		DisplayText("[pg]\"<i>Oh, I'm so glad you like it.  Why don't you try it out right now?</i>\"");
		...
		DisplayText("[pg]There is a long drawn out cry of joy when you bring your [tongue] to the source of her womanly pleasure.  ");
		if (Flags.list[FlagEnum.MARBLE_DICK_TYPE] > 0) DisplayText("With the first flick across its pink flesh, Marble grabs ahold of her " + marbleCock() + ", and tightly squeezes it, slowly pushing her hand down it.");
		else DisplayText("With the first flick across its pink flesh, Marble grabs onto an arm of her chair, gripping it tightly.");
		DisplayText("  You can't help but laugh and ask her if your treatment is too much, suggesting that maybe you should save playing more for later.");

		DisplayText("[pg]\"<i>No more teasing me, [name], bite me now!</i>\"");

		DisplayText("[pg]How can you refuse such a request?");

		DisplayText("[pg]\"<i>Ahhhhooooo!</i>\"  The wonderful bovine women gives a grateful cry of pleasure as your teeth nip into her nub of engorged skin and a burst of liquid sprays out onto your chin.  ");
		if (Flags.list[FlagEnum.MARBLE_DICK_TYPE] > 0) DisplayText("Of course, there is also a spurt of white which sprays out over her lap and your head.  ");
		DisplayText("Obviously, while the milkers don't normally get her aroused, they can seriously amplify the effects of any pleasure she gets.");

		DisplayText("[pg]You ask your lover if that was good.");

		DisplayText("[pg]\"<i>Lovely. Thanks for giving me the best start to being milked sweetie.</i>\"  She notices that you're all wet, \"<i>Oh, here, let me get that for you.</i>\" and she wipes the fluid off your [face] with a grin.");

		DisplayText("[pg]You then tie her bow back up on her tail and say you'll save the rest for later.");

		DisplayText("[pg]\"<i>Oh, I'll be looking forward to it.  See you later sweetie!</i>\"  She gives you a slightly awkward peck on the cheek around the milking tubes, that sends you into a blissful state.");
	...
	private milkMarbleFuckDatCowPussy() {
		let x: number = player.cockThatFits(marbleCuntCapacity());
		DisplayText().clear();

		DisplayText("You wonder aloud if maybe she'd be interested in a little intercourse action. You'd be perfectly prepared to provide for her.");

		DisplayText("[pg]\"<i>Hmm, well, your offer is tempting, but I'm going to have to see what's available before I make a final decision.</i>\"");
		...
		DisplayText("[pg]You step forward and allow your lover to get \"acquainted\" with your goods.");
		...
		if (player.tallness < 68) {
			DisplayText("[pg]\"<i>Oh sweetie.</i>\"  She pulls you into a deep hug against her chest.  \"<i>It's too much!  Hah, hahahahaha!  No more joking around.  I need you inside me now.</i>\"  She carefully carries you over to the bench and lays you down on your back, then straddles the bench above your " + Desc.Cock.describeCock(player, x) + ".  \"<i>Are you ready for me?</i>\" she calls out in a deep husky voice, breasts heaving, tail swishing, and all the while the milker machine continues to hum above her.");
		}
		else {
			DisplayText("[pg]You pull her into a deep hug as she continues to her hysterics and gently caress her hair.  You assure her that if the games are too much, you can skip straight to the action.");

			DisplayText("[pg]After a moment she settles down and returns the embrace.  \"<i>Hmmhmm, yes.  No more joking around.  I need you inside me now.</i>\"");

			DisplayText("[pg]You break away from her and casually walk over to the bench and lay down on top of it.  Marble follows immediately behind you, and before you've even finished laying down, she's already standing above with a hungry look on her face, breasts heaving, and tail swishing.");

			DisplayText("[pg]\"<i>Are you ready for me?</i>\" she asks in a deep husky voice.");
		}

		DisplayText("[pg]With a grin, you tell her you're always ready for her.");

		DisplayText("[pg]The words have barely left your mouth before Marble drops her waist and engulfs your " + Desc.Cock.describeCock(player, x) + ", swallowing it up to the base in a single motion.  \"<i>Ohhhhhh, yes!</i>\"  Marble happily rubs her belly with one hand, while another cubs her breast.  She shakes her hips from side to side a couple times, savoring the feeling of your cock rubbing back and forth across her insides.");
		...
		DisplayText("[pg]\"<i>Oh no!  Damn it, now what?</i>\" Marble stops moving and looks around worried.");
		...
		if (player.perks.has(PerkType.MarblesMilk)) {
			...
			dynStats("lib", .3);
			if (player.stats.cor < player.statusAffects.get(StatusAffectType.Marble).value4) dynStats("cor", 1);
			dynStats("tou", .1);
			dynStats("str", .1);
			dynStats("sen", -2);
			marblePreggoChance(1);
		}
		else if (player.statusAffects.get(StatusAffectType.Marble).value4 > 30) {
			...
			DisplayText("[pg]Finally the two of you cry out together in orgasmic pleasure, and you spray out your seed into her passage, mixing with a rush of her own fluids into a slurry of sex juices sloshing around your " + Desc.Cock.describeCock(player, x) + ".  The two of you settle down, with Marble laying down on you contentedly for a moment.");

			DisplayText("[pg]Suddenly a voice with a southern drawl call out from above you, \"<i>You kids havin a lots of fun down there?</i>\"");

			DisplayText("[pg]The two of you stare at each other in surprise and tense up.");

			DisplayText("[pg]\"<i>Next time, please try to be a bit gentler on the equipment, if ya don't mind.  I'd rather not have to do fix it every time you two get frisky.</i>\"  There is a slam of metal on metal as Whitney finishes working on the broken milker and it whirls back to life.");

			DisplayText("[pg]You and Marble each look each other in the eye and burst out laughing.  You then help her reattach the cups and excuse yourself.  Just as you're leaving, Marble blows you a kiss and says, \"<i>We should definitely do this again sometime.  Let's just give Whitney a break first.</i>\"");
			...
			DisplayText("[pg]You give a gasp of pleasure of your own and try your best to properly milk her nipples in the same way that cows back home were milked, in spite of how roughly your milkee is fucking right now.  If anything, being milked has made her more aggressive, and her tail is swishing around madly.");
	...
	private milkMarbleTakeHerDick() {
		DisplayText().clear();

		DisplayText("You ask her if maybe she wants to penetrate you while she is using that milking machine.");

		DisplayText("[pg]She looks thoughtful for a moment, tapping her finger against her lips while her tail swishes back and forth.  With the tubes coming off of her nipples, it actually looks rather comical.");

		DisplayText("[pg]Spotting your look, she frowns at you, \"<i>Oh, you think this is silly do you, [name]?  I'll have you know that I'm seriously trying to figure out how to make this work.</i>\"  She indicates her rather physical attachment to the room, and the furnishings it has available to it.");

		DisplayText("[pg]You give it a little thought, then suggest that you could lean up against that railing while she fucks you from behind.");

		DisplayText("[pg]\"<i>Take you from behind, huh?  I'd really rather look into your eyes while I'm having sex, but there aren't many options here, are there?</i>\"");

		DisplayText("[pg]To silence her complaining, you quickly strip");
		if (player.tallness < 56) DisplayText(", grab a box,");
		DisplayText(" and lean against the railing, then shake your [ass] in her direction with your vagina on full display, wet with arousal.  You look back at her to see if you're having the desired effect.");

		DisplayText("[pg]Marble is licking her lips with a nervous look on her face while holding her tail in her hand and running her fingers through it's brush.  \"<i>You make a very persuasive argument, sweetie.  Plus, if you're looking at me like that, I guess I really don't have anything to complain about, do I?</i>\"");

		DisplayText("[pg]She strips off her short skirt and takes up a position behind you, hands on your hips and erect cock pressed against your entrance.  You look down between your legs and catch sight of her tail hanging down between her legs, the barest hint of movement to it.  \"<i>Well sweetie, are you ready for me?</i>\"");

		DisplayText("[pg]You retort that if you weren't, you wouldn't have suggested this in the first place.");

		DisplayText("[pg]\"<i>Heh, good point.  Alright, here I go...</i>\"  She slowly works her way inside you, inch by inch.  ");
		if (player.vaginalCapacity() < Flags.list[FlagEnum.MARBLE_DICK_LENGTH] * Flags.list[FlagEnum.MARBLE_DICK_THICKNESS]) DisplayText("With each new bit of length inserted, she waits a few moments for you to adjust to her size, letting you fill out to accommodate her.  ");
		DisplayText("With the final bit of her inserted into you, her tail suddenly shoots straight up out of your sight, and she lets out a long exaggerated sigh of pleasure as the milking machine continues to whirl above her.");
	...
	private milkMarbleOnTheBar() {
		DisplayText().clear();

		DisplayText("You suggest that you'd be interested in playing with her while she's draped over the bar on the one side of the room.");

		DisplayText("[pg]She narrows her eyes at you, considering the proposition.  \"<i>I don't know sweetie, doesn't really sound like something I'd like that much.  I'd like to be in a position where I can do more than just take it...</i>\"");

		DisplayText("[pg]You assure her that you'll make it good enough that it will be worth while to let you have complete control.");

		DisplayText("[pg]After a few more moments of thought, Marble gives you a very serious look, \"<i>You don't get complete control, I'll be giving the instructions here, oh, and you'd better do a good job.</i>\"  Suddenly she smiles, \"<i>Otherwise, I'll just have to turn things around!</i>\"");
		...
		DisplayText("[pg]\"<i>Alright sweetie, time to get me ready.  Let's see how you do.</i>\"  She turns her head back, and simply waits for you to start while the milking machine continues to hum above her.");
		...
		DisplayText("[pg]\"<i>Hmm,</i>\" comes a pleased moan from the cowgirl in front of you, and her tail lifts up ever so slightly as the first signs of moisture and arousal form around her womanhood.");

		DisplayText("[pg]Next, you run a finger across her entrance, teasing her ever so slightly.  A small jolt flies through her body, and her tail shoots straight up.");

		DisplayText("[pg]\"<i>Ah, that's a good start sweetie, now, put a finger inside me.</i>\"");
		...
		DisplayText("[pg]\"<i>Maybe you need my help after all.  Want to let my tail do the talking sweetie?  Let me show you how I like being penetrated.  Then again, your tongue might be nice to feel next too...\"</i>");

		if (player.cockThatFits(marbleCuntCapacity()) > 0) DisplayText("[pg]You idly consider grabbing and pulling on her tail, so you could have a chance to take charge, though Marble would probably try and take control in that case for sure...");
	...
	private milkMarbleBarFollowTail() {
		let x: number = player.cockThatFits(marbleCuntCapacity());
		let y: number = player.cockThatFits2(marbleCuntCapacity());
		DisplayText().clear();

		DisplayText("You ask Marble where her tail will lead you.");

		DisplayText("[pg]She gives a low chuckle, \"<i>Why don't you wait and find out?</i>\"");

		if (x >= 0) {
			DisplayText("[pg]Her ribbon adorned, bovine appendage slowly unwraps from your wrist and instead searches around your crotch for something else.  Eventually the tuff comes into contact with your " + Desc.Cock.describeCock(player, x) + ", and runs its fur across your length lovingly for a few times, then curls around your base and pulls you in towards Marble's waiting womanhood.");

			DisplayText("[pg]\"<i>Mhehehe, found it!</i>\"");

			DisplayText("[pg]It was obvious that you couldn't hide from her for long.");

			DisplayText("[pg]\"<i>Oh I know sweetie, but it's always so much fun looking.</i>\"");
			...
			if (y >= 0) DisplayText("  You find your " + Desc.Cock.describeCock(player, y) + " slides up comfortably in between her cheeks, and squeeze them together a bit to give it a nice tight valley of soft flesh to run through.");
			...
			DisplayText("[pg]Everything centers onto three things: the feel of aroused cunt surrounding your shaft, rippling and squelching with each slow plunge and retreat from its depths, the soft springy ass and hip curves under your hands that runs oh so pleasantly under your hands, and of course, Marble's tail curled around the base of your " + Desc.Cock.describeCock(player, x) + ".  The sensation of her thin appendage, soaked and sweat yet still soft with fur, constantly quivering and twitching in time with the movements of your two bodies, draws your entire focus.");
		}
		else {
			...
			DisplayText("[pg]You ask Marble if there is any chance you've found what she wanted?");

			DisplayText("[pg]\"<i>Why yes sweetie.  What gave it away?</i>\"");

			DisplayText("[pg]It was just a hunch.");
			...
			DisplayText("[pg]\"<i>What's wrong sweetie?  You haven't managed to get stuck have you?  Here, let me give you a hand....</i>\"");
			...
			DisplayText("[pg]\"<i>Alright, no more teasing.  You're more than just a plaything after all.  I've got to let you do something.</i>\"");
		...
		if (x > 0) {
			DisplayText("[pg]Finally, the grip on your root relaxes, and you're finally able to explode inside her. Her tail slowly slips off of you and falls limp next to the connection between your bodies.  Several gasps and groans come out of your lips, and you ask Marble if she was satisfied.");
		}
		else {
		...
		DisplayText("[pg]Your lover girl straightens up and turns around to face you.  She fixes you with a stern look for a few moments before pulling you into a deep french kiss.  \"<i>Well, if you're always that good at following directions, I'm not sure how I wouldn't be.</i>\"");
	...
	private milkMarbleBarCunnilingling() {
		DisplayText().clear();

		DisplayText("You ask Marble to release your wrist so you can get into a better position for taking care of her.  You're feeling an ache to harvest her other womanly fluid.");

		DisplayText("[pg]\"<i>Of course sweetie, I'm feeling very wet down there.  I definitely think I'm going to need you to take care of that.</i>\"");
		...
		DisplayText("[pg]After a moment's climb, you find your fingers curling around the root of her bottom's bestial extension.  An idea strikes you, and you comment on how you need to brush her off before you start harvesting.");

		DisplayText("[pg]\"<i>Oh?  What do you mean by that?</i>\" comes a somewhat bemused response.");

		DisplayText("[pg]You don't reply, instead you make a small ring with your thumb and finger and slide her tail through the hole as it flicks and swings around.  When you reach the thick hair at the end, you tighten your grip and direct the brush to the entrance of her womanhood.");

		DisplayText("[pg]\"<i>Oh!  Ohhhhhhh...</i>\"");
		...
		DisplayText("[pg]You nuzzle in against her soft bum once more, and bat at her tail playfully for a few moments.");

		DisplayText("[pg]\"<i>Hmm, I'd say that you've made standing like this worth the while, [name].  I think I might be willing to go like this again, if you're able to keep up this level of performance.</i>\"");
		...
		DisplayText("[pg]After a moment, her tail flicks back out of her passage and she shakes her head at you.  \"<i>Thank you for the thought, sweetie, but please don't get me worked up again now, unless you're going to take care of me again.</i>\"");

		DisplayText("[pg]Alright, you'll end the teasing, for now.");
	...
	private milkMarbleBarPullTail() {
		let x: number = player.cockThatFits(marbleCuntCapacity());
		DisplayText().clear();

		DisplayText("Probably against your better judgment, you decide to take charge and grab onto Marble's tail, roughly untwisting it off of you and yanking it upwards.");

		DisplayText("[pg]\"<i>Ow!  What the hell sweetie?!</i>\"");

		DisplayText("[pg]You ignore her protests and smack her ass once, then roughly penetrate her with your " + Desc.Cock.describeCock(player, x) + " while yelling at her to take it.");
		...
		if (Flags.list[FlagEnum.MARBLE_DICK_TYPE] > 0) DisplayText("  She grabs her erect member, and rubs it against your cheek a few times, spreading around some of her pre-cum.");

		DisplayText("[pg]\"<i>You like it when I take charge, don't you?  Hmmhmm, yes...</i>\"");

		if (Flags.list[FlagEnum.MARBLE_DICK_TYPE] === 0) {
			DisplayText("[pg]She drops her hips and roughly forces her womanhood down onto your face, cutting off your ability to breathe.  You sputter and gasp underneath her at the sudden suffocation, though it lasts only a few seconds before she releases you.");

			DisplayText("[pg]\"<i>Aww, was that too rough?  Don't worry.  I'll be a bit more gentle.</i>\"");

			DisplayText("[pg]Marble then settles herself down a bit more gently, this time giving you plenty of room to breathe while also keeping your mouth tightly locked against her slick passage.");
			...
			DisplayText("[pg]She forces your mouth open and pushes her hips forward, roughly shoving her " + numToCardinalText(Math.round(Flags.list[FlagEnum.MARBLE_DICK_LENGTH])) + "-inch cock into your hapless mouth and down your throat.  You choke and sputter in surprise at the sudden suffocation, but it only lasts a few seconds before she pulls back out.");

			DisplayText("[pg]\"<i>Aww, was that too rough?  Don't worry, I'll be a bit more gentle.</i>\"");

			DisplayText("[pg]Marble then thrusts forward much more gently, giving you plenty of time to get use to the sensation of her member inside your mouth.  She also is sure to keep herself well outside of your throat.");

			DisplayText("[pg]\"<i>Alright, let's put that rude mouth of yours to a much better job.  Suck me.</i>\"");

			DisplayText("[pg]You do as you're bid, and get an approving smile in response.  You wrap tightly around her with your lips and slide your tongue up and down the bottom of her shaft while also playing across the urethra.  She pumps herself into you a few times, and you get a taste of some of her pre-cum.");
		}
		...
		DisplayText("[pg]\"<i>Hmm, sorry sweetie, I just thought I'd try something.  It didn't feel the way I expect, but I really like the way you look with my tail in your mouth like that.  If you suck on it for a bit, I might be able to forgive you for yanking it before.</i>\"");

		DisplayText("[pg]You run your tongue around the sweaty fur and get an approving look from your bovine lover.  Then you shake your jaw back and forth a few times and slowly let her bush slide out of your mouth.  As the last of the hair slips out, you give a small kiss to the tip.");

		DisplayText("[pg]\"<i>Did you like that sweetie?");
		if (User.settings.silly()) DisplayText("  Want to brush your teeth with my tail next time?");
		DisplayText("</i>\"  She nods and considers you for a moment.  \"<i>Well, I think I'm feeling a lot better now.  I think I can forgive you for what you did.  In fact, you did such a good job that I think I'll give you a reward!</i>\"");

		DisplayText("[pg]With a smile, you ask her what she's got planned for you.");
		...
		DisplayText("[pg]After a moment, you suddenly feel something pushing up against the entrance to your [vagOrAss]!  You try to ask what Marble has planned, but the pressure forces something bushy into your hole.  She just stuck her tail into your [vagOrAss]!");

		DisplayText("[pg]\"<i>Well sweetie, what do you think?</i>\"  It quivers a few times as Marble resumes the rocking of her wide womanly hips.  \"<i>Never mind, if you don't like it, well you can just try and end this as soon as possible.  If you do like it, then I guess...</i>\"  She leans forward as she switches from a swinging movement to a bounce on your " + Desc.Cock.describeCock(player, x) + ", and her furry tufft of a backend appendage starts to push its way in deeper sliding easily thanks to your saliva on it.  \"<i>You'll just have to bear it as long as you can!</i>\"");

		DisplayText("[pg]You've never seen Marble this aggressive before, but the cause is rather obvious: the whirring milking machine that's currently attached to her breasts.  Obviously giving milk is triggering some kind of dominance instinct, and it's incredibly sexy!");
		...
		DisplayText("[pg]You snuggle up to her bosom, thinking something very similar.  Things certainly haven't been dull while you've been in Marble's milking stall, that's for certain.");
	...
	private milkMarbleLeaveAfterBar() {
		DisplayText().clear();

		DisplayText("You don't answer Marble and instead continue to finger her womanhood until she cries out in orgasm.  You tell her you'd be happy to help her out again whenever she'd like someone to accompany her to get milked.");

		DisplayText("[pg]She is a bit disappointed that you didn't go further after getting her on the bar, but doesn't have a problem with you leaving.  She does tell you that she's looking forward to having you in her milker again some time.");
	...
	private milkMarbleNoMilking() {
		DisplayText().clear();
		DisplayText("You shake your head and say that you think you'll be fine.");

		DisplayText("[pg]\"<i>Oh, alright then sweetie, I'll see you later.</i>\"  She takes a seat in the chair in the middle of the room and picks up where she left off in her book.  You excuse yourself and return back to camp.");
