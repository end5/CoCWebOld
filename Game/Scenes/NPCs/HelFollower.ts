...
import { returnToCampUseOneHour, campLoversMenu, doSleep, companionsCount } from "../Camp";
import { followerKiha } from "./KihaFollower";
import { isabellaFollower, isabellaAccent } from "./IsabellaFollowerScene";
...
import { helPregnant } from "./HelSpawnScene";
...
import { describeCock, describeCockHead } from "../../Descriptors/CockDescriptor";
...
export const HelFollowerFlags = {
    HELIA_ANAL_TRAINING: 0,
    HEL_AFFECTION_FOLLOWER: 0,
    HEL_HARPY_QUEEN_DEFEATED: 0,
    HEL_BONUS_POINTS: 0,
    HARPY_QUEEN_EXECUTED: 0,
    ...
    UNKNOWN_FLAG_NUMBER_00326: 0,
    MINO_SONS_HAVE_SOPHIE: 0,
    UNKNOWN_FLAG_NUMBER_00283: 0,
    KEEP_HELIA_AND_SOPHIE: 0,
    HELIA_ANAL_TRAINING_OFFERED: 0,
    HEL_LOVE: 0,
    HELSPAWN_AGE: 0,
    HELIA_TALK_SEVEN: 0,
    HEL_TALK_EIGHT: 0,
    HELSPAWN_NAME: 0,
    HEL_GUARDING: 0,
    FOLLOWER_HEL_TALKS: 0,
    SLEEP_WITH: 0,
    IZMA_NO_COCK: 0,
    HEL_CAN_SWIM: 0,
    VALARIA_AT_CAMP: 0,
    HELIA_BIRTHDAY_OFFERED: 0,
    HELIA_BDAY_DRINKS: 0,
    HELIA_BDAY_HAKON_AND_KIRI: 0,
    HELIA_BDAY_PHOENIXES: 0,
    HELIA_BDAY_FOX_TWINS: 0,
    URTA_MET_HEL: 0,
    FREED_VALA: 0,
};
User.flags.set(FlagType.Hel, HelFollowerFlags);
...
export function heliaAnalCapacity(): number {
    let anal: number = 85;
    if (HelFollowerFlags.HELIA_ANAL_TRAINING >= 1) anal += 100;
    if (HelFollowerFlags.HELIA_ANAL_TRAINING >= 2) anal += 300;
    return anal;
}

export function helAffection(diff: number = 0): number {
    if (HelFollowerFlags.HEL_AFFECTION_FOLLOWER > 70 && HelFollowerFlags.HEL_HARPY_QUEEN_DEFEATED === 0) HelFollowerFlags.HEL_AFFECTION_FOLLOWER = 70;
    if (HelFollowerFlags.HEL_AFFECTION_FOLLOWER < 100 || (HelFollowerFlags.HEL_BONUS_POINTS === 0 && diff < 0)) {
        HelFollowerFlags.HEL_AFFECTION_FOLLOWER += diff;
        if (HelFollowerFlags.HEL_AFFECTION_FOLLOWER >= 100) HelFollowerFlags.HEL_AFFECTION_FOLLOWER = 100;
        if (HelFollowerFlags.HEL_AFFECTION_FOLLOWER < 0) HelFollowerFlags.HEL_AFFECTION_FOLLOWER = 0;
    }
    else if (followerHel()) {
        HelFollowerFlags.HEL_AFFECTION_FOLLOWER = 100;
        HelFollowerFlags.HEL_BONUS_POINTS += diff * 3;
        if (diff > 0) if (HelFollowerFlags.HEL_BONUS_POINTS > 150) HelFollowerFlags.HEL_BONUS_POINTS = 150;
        else if (diff < 0) if (HelFollowerFlags.HEL_BONUS_POINTS < 0) HelFollowerFlags.HEL_BONUS_POINTS = 0;
    }
    return HelFollowerFlags.HEL_AFFECTION_FOLLOWER;
    // trace("HEL AFFECTION" + HelFollowerFlags.HEL_AFFECTION_FOLLOWER);
}
...
export function heliaFollowerIntro(player: Character): NextScreenChoices {
    ...
    CView.text("\n\nGroggily, you ask the salamander what she's doing at your ");
    ...
    if (HelFollowerFlags.HARPY_QUEEN_EXECUTED > 0) CView.text("kill");
    else CView.text("bring down");
    CView.text(" the Harpy Queen... and getting my sister out of that shithole.  For giving me a chance to meet my dad.  For everything.</i>\"  You start to tell her that you're happy to help, but Hel cuts you off, speaking quickly: <i>\"Kiri, Dad, and I have been trying to make it work, living out on the plains - we really have - but Dad's not in any shape to fight and Kiri's no good at it; the plains are just too dangerous for them and Dad's been having nightmares about mom, and...\"</i> she pauses to take a gasping breath, unaccustomed to talking so quickly.");
...
function iCantLetFireButtsRapeMyCampsButt(player: Character): NextScreenChoices {
    ...
    if (HelFollowerFlags.HEL_FOXY_FOURSOME_WARNED > 0 || HelFollowerFlags.HEL_EDRYN_OFFER > 0) CView.text("like usual");
    CView.text(", and maybe you and I can spend some quality time together.  Alright?\"</i>");

    CView.text("\n\nYou nod and tell your fiery lover you'll be sure to do just that.");
    HelFollowerFlags.HELIA_FOLLOWER_DISABLED = 1;
    return { next: campMenu };
}

// [Come2Camp] -Dirty
function comeToCampHeliaIWantTailInButt(player: Character): NextScreenChoices {
    CView.clear();
    ...
    // place holder
    Time.hour++;
    return { next: afterMoveInBoningAnalFireTail };
}

// Afterwards, play:
// Hel Moving into Camp -McGirt
function afterMoveInBoningAnalFireTail(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("<b>An hour later...</b>");
    ...
    HelFollowerFlags.HEL_FOLLOWER_LEVEL = 2;
    return { next: campMenu };
}

// [Just Friends] -Dirt
function justFriendsWithAnalTailWaifu(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Awkwardly, you spend the next few minutes saying that, while you appreciate the offer and her affections, you're more interested in just being friends with Hel, as you have been for some time.");
    ...
    if (player.body.tallness > 84) CView.text("up");
    else if (player.body.tallness < 60) CView.text("down");
    else CView.text("over");
    CView.text(" to give you a little peck on the cheek.  <i>\"Well, that's that, then.  I guess.  Well, see you around, [name].\"</i>");

    CView.text("\n\n<i>\"Sure will.\"</i>");

    CView.text("\n\nYou wave as Hel retreats back toward her own home.");
    HelFollowerFlags.HELIA_FOLLOWER_DISABLED = .5;
    return { next: returnToCampUseOneHour };
}

// Hel Comes to Camp -- Intro Scenes (Play in Order)
export function helFollowersIntro(player: Character): NextScreenChoices {
    CView.clear();
    // (If Kiha is at camp & has "met" Hel before)
    if (followerKiha() && HelFollowerFlags.HEL_INTROS_LEVEL < 1) {
        HelFollowerFlags.HEL_INTROS_LEVEL = 1;
        CView.text("You pace around camp, awaiting the return of your new companion.  After several minutes, you notice Kiha sitting behind you, eyeing you with an eyebrow cocked.  Suddenly self-conscious, you stop and face the dragon-girl.");
    ...
    // If Isabella
    else if (isabellaFollower() && HelFollowerFlags.HEL_INTROS_LEVEL < 2) {
        HelFollowerFlags.HEL_INTROS_LEVEL = 2;
        // is at Camp (She and Hel are cool)
        if (HelFollowerFlags.HEL_ISABELLA_THREESOME_ENABLED > 0) {
            CView.text("Showing Hel around, you eventually come to the part of your camp inhabited by the towering cowgirl Isabella.  When you arrive, you find Isabella reclining in her armchair, humming a sweet melody as she cleans her tower shield.");
        ...
        // If Isabella is at Camp (And she and Hel are NOT cool)
            ...
            if (isabellaAccent()) CView.text("<i>\"You are... velcome,\"</i> Isabella says before collecting some of the scattered belongings from the ground.  You continue your tour, now that the girls are...  not going to murder each other in the middle of the night, at least.");
            else CView.text("<i>\"You're... welcome,\"</i> Isabella says before collecting some of the scattered belongings from the ground.  You continue your tour, now that the girls are...  not going to murder each other in the middle of the night, at least.");
            HelFollowerFlags.HEL_ISABELLA_THREESOME_ENABLED = 1;
        }
    }
    // If Rath is in Camp
    else if (HelFollowerFlags.HEL_INTROS_LEVEL < 3 && player.effects.has(StatusEffectType.CampRathazul)) {
        HelFollowerFlags.HEL_INTROS_LEVEL = 3;
        CView.text("You take Hel over to the small section of camp Rathazul has cordoned off for his 'laboratory,' surrounding himself with glass tubes and beakers and other, stranger instruments.  You poke through the array of equipment to find old Rath sitting in front of some experiment or another, furiously scribbling notes.  With a light cough, you alert him to your presence.");
        ...
        CView.text("\n\n<i>\"Not all,</i>\" Hel says with a wink.  <i>\"THIS salamander, for instance, is moving in...  Oh, right about here, I'd say,\"</i> she says, looking toward what's to be her part of the ");
    ...
    // If Bimbo Sophie is at Camp:
    else if (HelFollowerFlags.HEL_INTROS_LEVEL < 4 && bimboSophie() && HelFollowerFlags.FOLLOWER_AT_FARM_SOPHIE === 0) {
        HelFollowerFlags.HEL_INTROS_LEVEL = 4;
        CView.text("As you help Hel string up her hammock between a few of the rocks inside your perimeter, you hear the tell-tale flapping of useless wings and a clattering of claws on the hard-packed dirt.  You brace for impact as your bimbo harpy prances up to you, planting a big, wet kiss on your cheek");
        ...
        if (player.stats.int >= 40 || player.stats.int / 5 + randInt(20) + 1 > 10) {
            CView.text("Or, maybe there's a way to make this work...");
            // (Display Option: [Boot Sophie] [Boot Hel] [Work it Out])

            return {
                choices: [
                    ["Boot Sophie", bimboSophieGetsBooted4Firebutt],
                    ["Boot Hel", bootHelOutForBimboSophie],
                    ["Work It Out", workItOutWithSophieAndFireTits],
                ]
            };
        }
        ...
        HelFollowerFlags.HEL_INTROS_LEVEL = 9001;
        return campMenu(player);
    }

    return { next: helFollowersIntro };
}

// [Boot Sophie]
function bimboSophieGetsBooted4Firebutt(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You sigh and pick Sophie up, slinging the harpy over your shoulders.  <i>\"Yaaaay~\"</i> she cheers.  </i>\"I'm going for a ride!\"</i>");
    ...
    if (HelFollowerFlags.UNKNOWN_FLAG_NUMBER_00326 >= 3) {
        ...
        HelFollowerFlags.MINO_SONS_HAVE_SOPHIE = 1;
    }
    ...
    HelFollowerFlags.UNKNOWN_FLAG_NUMBER_00283 = 1;
    return { next: returnToCampUseOneHour };
}

// [Boot Hel]
function bootHelOutForBimboSophie(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("<i>\"Nope, you get out,\"</i> you answer, scowling at the salamander.");
...
// [Work it Out]
function workItOutWithSophieAndFireTits(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You cross your arms and tell Hel to deal with it, remarking that you won't just dump Sophie out in the wilderness - she can't take care of herself - but you still want Hel around.");
    ...
    CView.text("\n\nYou explain your bimbofication of the slutty harpy, and remind her that you've been saying she's too stupid to leave ");
    ...
    HelFollowerFlags.KEEP_HELIA_AND_SOPHIE = 1;
    return { next: returnToCampUseOneHour };
}

// If Hel is at Camp and Isabella Arrives, neither are cool
export function angryHelAndIzzyCampHelHereFirst(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Showing Isabella around, you eventually come to the chaotic, cluttered part of camp inhabited by Hel the salamander, who's currently sitting on her hammock sharpening her scimitar.");
    CView.text("\n\nYou only have a moment to remember Hel's disdain for the cowgirl before...");
    ...
    CView.text("hat are YOU doing here?!\"</i>");
...
export function heliaFollowerMenu(player: Character, display: boolean): NextScreenChoices {
    if (display) {
        CView.clear();
        CView.sprite(SpriteName.Hel); // 68;
    }
    if (HelFollowerFlags.HEL_FOLLOWER_LEVEL === 2) {
        if (HelFollowerFlags.HELIA_ANAL_TRAINING_OFFERED === 0 && display && player.body.cocks.sort(Cock.Largest)[0].area > heliaAnalCapacity()) {
            return heliaAnalTrainingPrompt(player);
        }
        if (display) CView.text("You call your salamander lover over, and in a few moments Hel walks your way, hips and tail swaying gaily as she moves.  She wraps an arm around your shoulders, pressing her soft, warm body against yours, and grins.  <i>\"Heyya, lover mine.  You need anything?\"</i>");
        ...
        choices[0] = ["Appearance", helSpawnScene.heliasAppearanceScreen];
        choices[1] = ["Sex", heliaRoughSex];
        choices[2] = ["Threesomes", heliaThreesomes];
        choices[4] = ["Talk", heliaOptions];
        if (!helScene.pregnancy.isPregnant) choices[5] = ["Spar", sparWithHeliaFirebuttsAreHot];
        else CView.text("\n\n<b>Helia will not spar or box while pregnant.</b>");
        if (!helScene.pregnancy.isPregnant) choices[6] = ["Box", boxWithInCampHel];
        if (HelFollowerFlags.HEL_LOVE === 1 || HelFollowerFlags.HEL_LOVE === -1) {
            if (player.body.cocks.length > 0 && player.body.cocks.find(Cock.CockThatFits(heliaCapacity())) && player.stats.lust >= 33 &&
                !helPregnant() && HelFollowerFlags.HELSPAWN_AGE === 0) choices[7] = ["Have A Kid", helSpawnScene.haveAKid];
        }
        choices[9] = ["Back", campLoversMenu];

        return { choices };
    }
    else if (HelFollowerFlags.HEL_FOLLOWER_LEVEL === 1) {
        if (HelFollowerFlags.HEL_HARPY_QUEEN_DEFEATED === 1) {
            if (display) CView.text("(You've reached the culmination of Helia's current storyline.  Stay tuned for more!)");
        }
        else {
            if (display) CView.text("You approach Hel as she's pacing around   She's clad in her normal field attire: a simple scale bikini top and leather thong which supports her scimitar's scabbard.  Her cloak is loosely thrown over her shoulders, giving her a slight measure of protection from the mountain's harsh environs.");
            if (display) CView.text("\n\n\"<i>Heya, [name]! Ready to hit the road?</i>\"");
            // (Display Options: [Dungeon] [Not Yet])
            return { choices: [["Dungeon", goToHeliaDungeon], ["", undefined], ["", undefined], ["", undefined], ["Not Yet", notYet]] };
        }
    }
}

function heliaOptions(player: Character): NextScreenChoices {
    if (helScene.pregnancy.event >= 3 && HelFollowerFlags.HELIA_TALK_SEVEN === 0) {
        heliaTalkSeven();
        return;
    }
    if (HelFollowerFlags.HELSPAWN_AGE === 1 && HelFollowerFlags.HEL_TALK_EIGHT === 0) {
        heliaTalkEight();
        return;
    }

    const choices: ScreenChoice[] = [];
    choices[0] = ["Discuss", talkToHel];
    if (Time.hour >= 21) choices[1] = ["Cuddle", hugASmokeyTail];
    else choices[2] = ["Hug", hugASmokeyTail];
    if (HelFollowerFlags.HELIA_ANAL_TRAINING_OFFERED > 0 && HelFollowerFlags.HELIA_ANAL_TRAINING < 2 && player.body.cocks.sort(Cock.Largest)[0].area > heliaAnalCapacity() && player.inventory.items.reduce(Inventory.TotalQuantityOf(ConsumableName.GoblinAle), 0) >= 1) choices[3] = ["Anal Train", heliaGapeSceneChoices];
    choices[5] = ["Bathe", takeABath];
    if (HelFollowerFlags.HELSPAWN_AGE === 1) choices[7] = [HelFollowerFlags.HELSPAWN_NAME, helSpawnScene.playWithYourKid];
    if (HelFollowerFlags.HEL_GUARDING === 0) choices[8] = ["GuardCamp", helGuardToggle];
    else choices[8] = ["NoGuarding", helGuardToggle];
    choices[9] = ["Back", partial(heliaFollowerMenu, player, true)];
...
function helFollowerAppearance(player: Character): NextScreenChoices {
	...
	choices[0] = ["Next", heliaFollowerMenu];
}*/
// Hel: Spar Intro
function sparWithHeliaFirebuttsAreHot(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Giving Hel a playful punch on the shoulder, you ask the salamander-girl if she'd be up for a little battle practice.");
    ...
    const monster = new Hel();
    monster.effects.add(StatusEffectType.Sparring, 0, 0, 0, 0);
    // No gems.
    monster.stats.XP = 1;
...
function loseToSparringHeliaLikeAButtRapedChump(player: Character): NextScreenChoices {
    CView.clear();
    // If HP loss)
    if (player.stats.HP < 1) CView.text("You collapse on the ground, overwhelmed by pain and exhaustion caused by the berserker's onslaught.");
    else CView.text("Your arousal is too great, and your mind can no longer focus on anything but a desperate need for release.  Lust robs you of your will, and with buckling knees you collapse.");
    ...
    return heliaRoughSex(player, false);
}

// PC Whips Hel's Ass
function PCBeatsUpSalamanderSparring(player: Character, monster: Character): NextScreenChoices {
    CView.clear();
    // (If HP loss)
    ...
    else CView.text("Panting heavily, knees snaking, she collapses, heavily leaning upon her sword.");

    CView.text("\n\n<i>\"Come on, [name],\"</i> Hel groans, swaying unsteadily.  ");
    if (monster.stats.HP < 1) CView.text("<i>\"Kick my ass a little harder, why don't you?\"</i>");
    else CView.text("<i>\"Turn me on till I can't think straight, why don't you?\"</i>");
    CView.text("  You chuckle and offer her a hand up. The blushing Salamander takes it as you pull her into a tight embrace.  The cool wetness between her thighs a potent reminder of how much your lover seems to get off on violence.  Since she's so turned on, you could easily turn this into some rough loving.");
...
function talkToHel(player: Character): NextScreenChoices {
    CView.clear();
    // Hel Talk 1 (Play at First Time)
    if (HelFollowerFlags.FOLLOWER_HEL_TALKS === 0) {
        CView.text("You run a hand through Hel's hair and ask the recent addition to your camp if she'd like to talk.");
        ...
        // (If Toughness <50:
        if (player.stats.tou < 50) CView.text("You cough and gag, eyes misting as the pure-grain booze burns your throat worse than hot coals.  Hel laughs riotously, slapping you on the back hard enough to cause booze to snort out through your nose.");
        else CView.text("You knock back the stein easily, your super-human endurance keeping you from choking on the powerful brew.  Impressed, Hel gives you a high-five and throws her head back in a laugh.");
        CView.text("\n\n<i>\"So yeah,\"</i> Hel laughs, throwing an arm around your shoulder, <i>\"I'm settling in just fine. Good booze, great ..  even better company.\"</i>");
    ...
    // Hel Talk 2
    else if (HelFollowerFlags.FOLLOWER_HEL_TALKS === 1) {
        CView.text("Sitting Hel down, you ask if she's got any stories she'd like to share.  After all, she's been adventuring for years - surely she's got some fun tales to tell.");
    ...
    // Hel Talk 5(C)
    else if (HelFollowerFlags.FOLLOWER_HEL_TALKS === 4) {
        CView.text("<i>\"Hey, [name],\"</i> Hel says, slipping out of your grasp with a come-hither wag of her finger, <i>\"wanna have a drink with me?\"</i>");
        CView.text("\n\n<i>\"Sure,\"</i> you say, following Hel over to her still.");

        CView.text("\n\nShe pours out two tankards of ");
        if (helPregnant()) CView.text("beer ");
        else CView.text("fizzy, non-alcoholic beer ");
        CView.text("and, swinging onto your lap, clinks her glass against yours.  <i>\"To loving friends and an awesome new home,\"</i> she says, raising her tankard high before knocking it back.  Before you can finish yours, Hel belches loudly and violently shakes her head");
        if (!helPregnant()) CView.text(", her eyes crossing a little.  Looks like she was dipping in before you got here...");
        else CView.text(".");

        CView.text("\n\nHel laughs ");
        if (!helPregnant()) CView.text("drunkenly");
        else CView.text("happily");
        if (!helPregnant()) {
            CView.text(", filling her cup up again and, with a wide smile, jumping out of your lap.  <i>\"Hey, lover, check this out!\"</i> she shouts, grabbing her fiery tail in one hand and swirling a mouth-full of alcohol.  You have just enough time to take cover before Hel swings her tail around and spits a stream of pure-grain over it, resulting in a great gout of flame that streaks into the heavens.");
            // if Kiha is in camp:
            if (followerKiha()) CView.text("  From across camp, you hear a certain dusky dragoness shout <i>\"HEY! That's MY trick, firebutt!\"</i>");
            ...
            if (followerKiha()) CView.text("further pissing off Kiha, who throws a rock at Hel, and ");
            CView.text("shooting up like a beacon over the wasteland.");
        }
        else {
            CView.text(" and launches into a series of debauched jokes. You answer her, lewd jest for jest.");
        }
        CView.text("\n\n<i>\"Hehehe.  Nice one, lover,\"</i> Hel laughs, pulling you ");
        if (player.body.tallness < 60) CView.text("up ");
        else CView.text("down ");
        CView.text("into a hug, mashing your face into her warm, soft bosom.  You struggle for air, finally bursting out of her tight cleavage.  Hel chuckles, running her hand through your [hair] as her fiery tail wags gaily behind her.");
    ...
    // Hel Talk 6 (Needs Isabella and Kiha at camp; at least 1 gem)(C)
    else if (HelFollowerFlags.FOLLOWER_HEL_TALKS === 5 && player.inventory.gems >= 1 && isabellaFollower() && followerKiha()) {
        let gems: number = 0;
        CView.text("<i>\"Hey, [name],\"</i> Hel says with a sly grin.  <i>\"Me, Izzy, and spitfire were just playing a little   Wanna deal in?\"</i>");
        ...
        if (!isabellaAccent()) CView.text(", obviously only somewhat interested");
        CView.text(".");
        ...
        // {Make an INT check}
        if (randInt(20) + 1 + player.stats.int / 5 >= 15) {
            // [Successful]
        ...
        // [Fail 2]
            ...
            if (isabellaAccent()) {
                CView.text("\n\n<i>\"Ja, ja, come to mama Isabella,\"</i> your feisty cow-girl laughs, throwing down a stunning, crushing hand.  Groaning, you, Hel, and Kiha all relinquish the last of the gems you can spare for the ");
                CView.text("\n\n<i>\"Oh, I zee ein new skirt in ze future!\"</i> Isabella laughs, dumping your gems into her pouch before sauntering off.");
            }
            else {
                CView.text("\n\n<i>\"Yeah, yeah, come to mama Isabella,\"</i> your feisty cow-girl laughs, throwing down a stunning, crushing hand.  Groaning, you, Hel, and Kiha all relinquish the last of the gems you can spare for the ");
        ...
        if (player.inventory.gems + gems < 0) gems = player.inventory.gems;
        if (gems < 0) CView.text("\n\nYou lost " + gems + " gems in the game!");
        else CView.text("\n\nYou gained " + gems + " gems in the game!");
        player.inventory.gems += gems;

    }
    else {
        HelFollowerFlags.FOLLOWER_HEL_TALKS = 1;
        return talkToHel(player);
    }

    HelFollowerFlags.FOLLOWER_HEL_TALKS++;
    return { next: returnToCampUseOneHour };
}

// Shut up, slut
function shutUpHelTalks(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Quickly you interject before Helia can continue with the story; you've heard quite enough.  <i>\"Whaaaat?  We were just getting to the good part...\"</i>  Hel whines, rolling her eyes.  <i>\"Hey, you're the one who asked!\"</i>");
    ...
    if (player.body.cocks.length > 0) {
        ...
        CView.text("\n\nSuddenly, Hel flips herself into your lap, straddling your [legs].  <i>\"Don't worry though, lover... Your dick's still the best!\"</i>  She plants a quick kiss on your lips before bounding off to another part of ");
    }
    // {If PC is cockelless:
    else {
...
// Hug(C)
function hugASmokeyTail(player: Character): NextScreenChoices {
    CView.clear();
    // Cuddle with Hel (Replaces Hug @ 21:00+)
    if (Time.hour >= 21) {
        CView.text("As the sun sets over the camp, you see Helia standing over her hammock, stretching and yawning, ready to turn in for the night.  You approach her, sliding your arms around her supple waist and burying your face in her soft crimson locks, holding your lover close.  Hel giggles girlishly as you give her a long hug, nuzzling into the nape of her neck.");
        ...
        if (player.body.tails.length > 0) CView.text("s");
        CView.text(" intertwined.  <i>\"Oh, this is nice,\"</i> Hel laughs, running her long, scaled fingers through your hair.  <i>\"So, you wanna stay with me tonight, lover mine?  I'd appreciate the company....\"</i>");
...
export function sleepyNightMareHel(player: Character) {
...
// Rough Sex
// Into Text
function heliaRoughSex(player: Character, output: boolean = true): NextScreenChoices {
    if (output) {
        CView.clear();
        CView.text("<i>\"Mmm, need to blow off some steam, lover?\"</i>  Hel grins, reaching around to undo the straps of her scale bikini.  <i>\"Well, I'm all for that!\"</i>");
    }
    ...
    if (player.body.cocks.length > 0 && player.stats.lust >= 33) {
        ...
        if (player.body.cocks.find(Cock.CockThatFits(heliaCapacity())) && buttons < 9) {
            choices[buttons] = ["FuckVag", helScene.beatUpHelAndStealHerWalletFromHerVagina];
            buttons++;
        }
        // 85 ass capacity
        if (player.body.cocks.find(Cock.CockThatFits(heliaAnalCapacity())) && buttons < 9) {
            choices[buttons] = ["Anal", helScene.fuckHelsAss];
            buttons++;
        }
        if (buttons < 9) {
            choices[buttons] = ["Get Blown", helScene.helBlowsYou];
            buttons++;
        }
        if (player.body.cocks.find(Cock.CockThatFits(heliaCapacity())) && player.body.cocks.filter(Cock.CocksThatFit(heliaCapacity()))[1] && buttons < 9) {
            choices[buttons] = ["DoublePen", helScene.dpHel];
            buttons++;
        }
        if (buttons < 9) {
            choices[buttons] = ["Tail Wank", helScene.helTailWanksYourDickBecauseSheLovesYouDesuDesuHoraHora];
            buttons++;
        }
    }
    if (player.body.vaginas.length > 0 && player.stats.lust >= 33 && buttons < 9) {
        choices[buttons] = ["GetLicked", helScene.getLickedByHel];
        buttons++;
    }
    if (player.stats.lust >= 33 && buttons < 9) {
        choices[buttons] = ["TailPeg", helScene.helTailPegging];
        buttons++;
    }
    // Morph-based: [Possession] [Mount Her] [Hanging 69] [Coil Her Up] [Tentafuck])
    if (player.stats.lust >= 33 && player.body.legs.isTaur()) {
        if (player.body.cocks.length > 0) {
            if (player.body.cocks.find(Cock.CockThatFits(heliaCapacity())) && buttons < 9) {
                choices[buttons] = ["Mount Her", centaurMountsCampHel];
                buttons++;
            }
        }
        if (player.inventory.keyItems.has("Centaur Pole") && player.body.vaginas.length > 0 && buttons < 9) {
            choices[buttons] = ["CentaurToy", femtaurPlusCampHel];
            buttons++;
        }
    }
    if (player.stats.lust >= 33 && player.perks.has(PerkType.Incorporeality) && izmaFollower() && HelFollowerFlags.IZMA_NO_COCK === 0 && buttons < 9) {
        choices[buttons] = ["Possess", heliaCampPossession];
        buttons++;
    }
    if (player.stats.lust >= 33 && player.body.cocks.filter(Cock.FilterType(CockType.TENTACLE)).length > 6) {
        choices[buttons] = ["Tentacles", heliaFollowerTentafuck];
        buttons++;
    }
    if (player.stats.lust >= 33 && player.body.legs.isNaga()) {
        // "Rough" Sex -- Naga Coil (Female w/ Naga Lower Body)
        if (player.body.vaginas.length > 0 && buttons < 9) {
            choices[buttons] = ["NagaCoilF", nagaCoilForHelCampWithGirls];
            buttons++;
        }
        // "Rough" Sex (Well, it's in the menu, if not in spirit) -- Naga Coil (Male w/ Naga Lower Body)
        // Male naga shit
        if (player.body.cocks.length > 0 && buttons < 9) {
            if (player.body.cocks.find(Cock.CockThatFits(heliaCapacity()))) {
                choices[buttons] = ["NagaCoilM", inCampHelNagaLuv];
                buttons++;
            }
        }
    }
    ...
    if (inCombat)
        choices[9] = ["Leave", returnToCampUseOneHour];
    else choices[9] = ["Back", heliaFollowerMenu];
}
...
function heliaCampPossession(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("With a coy grin, you let your ghostly attributes go transparent, and ask Hel if she'd mind taking an extra passenger for a little bit.");
...
function possessIzma(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You decide to give Hel's legs a stretch and trot over to the stream near   Walking along the shore, it isn't long before you see a certain dark-red fin cresting the water.  A moment later your beta, Izma, breaches, jumping up out of the water and landing a few feet away.  Still dripping from her swim, you watch as Izma shakes a bit of water off before going to get her grass skirt - for now, though, you can see her hefty fifteen-inch rod and quad swinging in the breeze.");
...
// "Rough" Sex (Well, it's in the menu, if not in spirit) -- Naga Coil (Male w/ Naga Lower Body)
// Male naga shit
function inCampHelNagaLuv(player: Character): NextScreenChoices {
    CView.clear();
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(heliaCapacity()));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    CView.text("You slither over to the salamander and circle around her, your serpentine lower body trailing around her legs as you sweep Helia into a tight embrace.  She grins wolfishly, pressing her lips to yours; you let her tongue enter your mouth, entwining with your tongue, letting her distract herself as you coil more and more of your snake-body around the salamander.  By the time Hel realizes what you're up to, there isn't much she can do about it - you grab her arms and press them to her sides as you bind her up past the waist, wrapping your lover in your lengthy, serpentine form.");
    ...
    CView.text("\n\nUnable to move much in your serpentine embrace, Hel begins to rapidly contract her vaginal and anal muscles, squeezing and grinding against your " + describeCock(player, cockThatFits) + " and tip.  Content to close your eyes and bask in the pleasures of your lover, ready to lose yourself in pleasure.  You relax, leaning back on yourself as you buck your hips into your lover's sweet hot box, flick your tail inside her ass, and play your tongue around her mouth, tasting her warm saliva and the aftertaste of beer.");

    CView.text("\n\nYou cum easily, only a small grunt escaping your lips as Hel's delicious cunt brings you over, pulling your " + describeCock(player, cockThatFits) + " deep into her and tightening your hold around her.  The sudden squeeze is the last hurrah as you empty your ");
    if (player.body.balls.count > 0) CView.text("[balls]");
    else CView.text("load");
    CView.text(" well into her womb.  Your tail spasms as you cum, running wild inside Hel's rectum until she cums too, yelping into your open mouth as she climaxes a moment after you, girlcum squirting out of her twat until your [hips] are thoroughly soaked.");
...
// "Rough" Sex -- Naga Coil (Female w/ Naga Lower Body)
function nagaCoilForHelCampWithGirls(player: Character): NextScreenChoices {
    CView.clear();
...
function centaurMountsCampHel(player: Character): NextScreenChoices {
    let cockThtFits = player.body.cocks.find(Cock.CockThatFits(heliaCapacity()));
    if (!cockThtFits) cockThtFits = player.body.cocks.sort(Cock.Smallest)[0];
    CView.clear();
    CView.text("You trot around your lover, your powerful centaur frame brushing lightly against her.  Hel grins, reaching out to pet your flanks and running her claws across your " + skinFurScales(player) + ".  \"<i>You've got a helluva body,</i>\" she smiles, and with an invitation from you, swings up onto your back, wrapping her arms tightly around your waist as you break out into a canter around ");

    CView.text("\n\nShe nudges you toward her part of   When you arrive, Hel leaps off and grabs some ropes from her pack.  You raise an eyebrow, but allow her to tie them around your equine half, creating a makeshift harness underneath you.  Once done, Hel lowers herself to her knees and, pushing aside your [armor], takes hold of your " + describeCock(player, cockThtFits) + ".  She gives it an experimental stroke, running her warm, leathery palm and scaled fingers across your length until you let out a little whinny and stamp your hooves.");

    CView.text("\n\nHel pats your flank with her free hand, but doesn't let up, instead adding her long, slender tongue to the mix.  She flicks it along the " + describeCockHead(cockThtFits) + " of your " + describeCock(player, cockThtFits) + ", sending shivers of pleasure up your rigid shaft.  You ache to do something here, to add to your own pleasure or return it to your partner, ");
    // if no boobs:
    if (player.body.chest.sort(BreastRow.Largest)[0].rating < 3) CView.text(" but your body is too big, and you must content yourself to endure Hel's ministrations");
    else CView.text(" but all you can do is take hold of your [chest] and tease and tweak your [nipples], getting what pleasure you can from your sensitive teats");
    CView.text(".");

    CView.text("\n\nAfter a few mind-hazing minutes of salamander hand-job, Hel takes the " + describeCockHead(cockThtFits) + " of your cock into her mouth, slurping up the tip of your " + describeCock(player, cockThtFits) + ".  Your breath catches in your throat as the first inches of your cockmeat are engulfed in the hot, wet embrace of Helia's mouth.  Her lizard-tongue wastes no time in flicking across your " + describeCockHead(cockThtFits) + ", the forked tip playing across your urethra, drawing forth a thick dollop of pre that soon smears her cheeks with white.  She pumps your cock into her mouth, licking and sucking on your crown as her pistoning hands force thicker globs of sticky precum out of you, eagerly swallowing every drop - but only after savoring it, rolling each drop around in her mouth until her entire gob is a sticky mess, thick webs of seed coating her lips each time she swallows another appetizing taste.");

    CView.text("\n\nThen as quickly as it began, Hel's blowjob comes to a halt.  The salamander pops your prick out of her maw and, giving it one last kiss on the " + describeCockHead(cockThtFits) + ", lets it fall aside as she moves.  You lean around yourself awkwardly as your lover ducks beneath your horse-half, clambering into her harness.  Hel disappears from view, completely obscured by your own body; all you can feel of her is her weight pulling down on your back and her powerful scaled legs hooking around your thighs, spreading wide as she grabs your cock again.  You groan pleasurably as the tip brushes her lower lips, the heat of her inhuman pussy radiating out to set your nerves ablaze.");
    ...
    if (cockThtFits.area > heliaCapacity()) CView.text("she can cram in, the rest of your length twitching between her legs");
    else CView.text("you have to offer her");
    CView.text(".  Finally, though, you manage to settle down, your breath coming ragged as Hel's vaginal muscles clamp and release, gently massaging your shaft with expert movements, always keeping you on edge until...");

    CView.text("\n\nTHWACK!");

    CView.text("\n\nYou start forward as a rough tail-slap lands on your flank.  You reflexively step forward, your equine muscles acting on instinct...  which only serves to rock Hel off and down again on your " + describeCock(player, cockThtFits) + ", her impromptu harness moving her like a swing. You arch your back as the movement hammers Hel down on your cock like a free-swinging onahole, but the moment of pleasure lurches out of control when she gives you another hard tail-swat, and another; your insticts carry you forward until you reach a full canter. Shudders and twitches wrack your body as your own motions rock Hel on and off your [cock], slamming her back down on your shaft again and again.");
    ...
    CView.text("\n\nAs Hel lets out another high-pitched shriek of pleasure, you finally feel yourself hurtling over the edge of bliss. You rear yourself up as you cum, staggering on your hind legs as the first searing spurt of spooge squirts into the screaming salamander.  A huge spurt of cum jets out of your " + describeCock(player, cockThtFits) + ", smearing Hel's spasming walls white with your potent seed.  As your prick fires off again and again, you slowly bring yourself to a halt, your prick still twitching inside Hel's cum-soaked box.  When you've brought yourself to a complete stop, you feel a sudden lightness as Hel flops out of her harness, quickly surrounded by a small pool of jizz leaking out of her.");
...
function femtaurPlusCampHel(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You trot around your lover, your powerful centaur frame brushing lightly against her. Hel grins, reaching out to pet your flanks and running her claws across your " + skinFurScales(player) + ".  \"<i>You've got a helluva body,</i>\" she smiles, and with an invitation from you, swings up onto your back, wrapping her arms tightly around your waist as you break out into a canter around   She gives you a gentle nudge toward a certain pile of your gear, and it doesn't take but a moment for you to notice she's urging you toward the special toys Whitney gave you for your equine-like body's needs.");
...
function heliaFollowerTentafuck(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("With a flourish you pull your [armor] off, exposing your writhing mass of tentacle-cocks for Hel`s enjoyment.  \"<i>Wow, think you got enough of th--urf!</i>\" the salamander starts to say before you shut her up with one of your tentacle pricks, lashing forward and plugging her mouth.  Hel staggers back, flailing around before grabbing the cock you've shoved down her throat and giving it a rough tug.  Just as she nearly succeeds in pulling it out, your other wriggling members wrap around her thick thighs; you pull her in toward you, grabbing her shoulders as your pricks peel off her bikini bottom to reveal the smooth face of Helia's crotch.");
    ...
    if (player.body.balls.count > 0) CView.text("[balls]");
    else CView.text("wriggling tentacles");
    CView.text(" feel painfully empty and white slush pours from all her holes, painting the ground white beneath her dangling feet.");
...
function boxWithInCampHel(player: Character): NextScreenChoices {
    ...
    player.stats.fatigue += 5 + randInt(20);
    // If INT check succeeds:
    if (player.stats.int / 20 + 1 + randInt(20) >= 13) {
    ...
    else if (player.stats.str / 20 + 1 + randInt(20) >= 13) {
...
function muddyLizardFeet(player: Character): NextScreenChoices {
    ...
    if (player.body.balls.count > 0) CView.text("her heel coming to caress your taut sack");
    else if (player.body.vaginas.length > 0) CView.text("her heel brushing against the nub of your [clit]");
    else CView.text("her heel going so low as to brush your taint, making your [asshole] clench in surprise before she moves back up your length");
    CView.text(".  You groan lustily as her toes and soft sole jerk you off, gently urging the tight knot of pleasure in your gut to fruition.  Cupping one of her hefty tits, Hel licks her lips as the first thick smears of pre leak from your cock's [cockHead biggest], smearing onto her long-clawed toes.");
    ...
    if (player.body.cocks.sort(Cock.Largest)[0].area > heliaCapacity()) CView.text("you feel your [cockHead biggest] against the lips of her cervix, as fully into her as you can go");
    else CView.text("your groin meets Helia's, your [cock] wholly enveloped within her");
    CView.text(".  Before you can think to withdraw, Hel tightens her hold on you, whispering huskily, \"<i>Just leave it... just for a second, [name].  Oh, God,</i>\" she moans, her eyes closing as she savors the feeling of you buried so deep inside her, your slick prick twitching as her walls massage it, milking it of the precum that freely flows from your tip.");
...
function takeABath(player: Character): NextScreenChoices {
    ...
    if (HelFollowerFlags.HEL_CAN_SWIM === 0) {
        ...
        return {
            choices: [
                ["Drop It", heliaFollowerMenu],
                ["Push It", pushHelToSwim],
            ]
        };
    }
    // {Repeat Intro (If you taught Hel to swin; else, play 1st Time)}
        ...
        CView.clear();
        CView.text("You ruffle Hel's hair and ask if the fiery salamander's up for a dip.");
        ...
        if (player.inventory.equipment.armor.displayName === "goo armor" && player.stats.lust >= 33 && 9999 === 0) {
...
// Teach Her
function teachHelToSwim(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Putting a hand on Hel's shoulder, you offer to teach her how to swim.");
    ...
    HelFollowerFlags.HEL_CAN_SWIM = 1;
    return { next: returnToCampUseOneHour };
}

// Threesomes
export function heliaThreesomes(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You slip an arm around Hel's waist and ask if she's up for some... group activities?");
    ...
    const choices: ScreenChoice[] = [];
    if (vapulaSlave() && player.stats.lust >= 33 && player.body.cocks.length > 0) {
        if (player.body.cocks.find(Cock.CockThatFits(heliaCapacity()))) {
            choices[1] = ["Vapula", heliaAndVapula];
        }
        else CView.text("\n\nYou're too big to fuck Helia and Vapula with your cock.");
    }
    if (followerKiha() && player.stats.lust >= 33 && player.gender > 0) {
        choices[0] = ["Kiha", campHelAndKihaThreeSome];
    }
    if ((player.inventory.equipment.armor.displayName === "goo armor" || HelFollowerFlags.VALARIA_AT_CAMP === 1) && player.stats.lust >= 33 && player.body.vaginas.length > 0) {
        choices[2] = ["Valeria", helAndValeriaCampThreesomes];
    }
    if (player.stats.lust >= 33 && bimboSophie()) {
        choices[3] = ["Sophie", helAndSluttyHarpy];
    }
    if (player.stats.lust < 33) CView.text("\n\n<b>You aren't horny enough to start a threesome.</b>");
    choices[9] = ["Back", heliaFollowerMenu];
...
// Helia at Camp, Intro
function heliaAndVapula(player: Character): NextScreenChoices {
    CView.clear();
    ...
    CView.text("You gab Hel and start for the edge of camp, where a certain succubus dwells....");
    ...
    CView.text("\n\nYou feel hands wandering across your body, Hel slowly, sensuously, pulling off your [armor].  Soon, the two of you are bare, clothes and weapons discarded all around the succubus.  You push Hel over, grinding your hard " + describeCock(player, cockThatFits) + " between her legs.  As you grope the salamander girl's body, you gesture at Vapula and tell her to come closer so that you can stroke her, too.  You begin caressing each other, spending a few moments in languid pleasure until your " + describeCock(player, cockThatFits) + " looks ready to explode.");
    ...
    CView.text("\n\nThis libertine orgy cannot last indefinitely, and even your girls are trembling convulsively, ready to reach their climax. You manage to slide your " + describeCock(player, cockThatFits) + " up the reptile girl's twat a few more times before blasting her with your spooge.  You fill Hel's interior, pouring cum like a torrent into her womb");
    // [IF cum production massive:
    if (player.cumQ() >= 750) CView.text(" until her belly swells and starts spurting ropes of jism back out of her love-tunnel");
    CView.text(".  Your junk palpitates and bulges as globs push their way out of your shaft before being absorbed by her needy pussy, and her skin and scales seem to literally boil with consumed lust.  At the peak of your orgasm, you stare at your girls' faces: Hel's is nothing but complete, insensate bliss, while Vapula is literally crying in ecstasy, tears of joy streaming down her cheeks.");

    CView.text("\n\nSeeing how much pleasure you're giving them is quite touching, and you give each of them a long, ardent kiss, letting each girl know how the other tastes like.  At last, when your " + describeCock(player, cockThatFits) + " has unloaded its last rope of jism, you pull out and break the kiss.  Hel pants and sighs, utterly drained; you see her tail popping out of Vapula's ass, and the released succubus just falls on her back, cunt exposed and mouth open in blissful exhaustion.");
    ...
    if (!followerHel()) {
        CView.text("  You kiss her and ask if she'll be able to find her way back to her home.");
...
function dudeHeliaAndKihaThreeSome(player: Character): NextScreenChoices {
    ...
    CView.text("\n\nKiha gives the salamander a grave look, but soon softens, nuzzling her chin in your " + describeHair(player) + ".  Hel sighs, but leans down to give you each a quick kiss on the head; her foot never misses a beat, though, and combined with Kiha's sensual massaging, you realize you're dangerously close to the edge again.  You grit your teeth and let loose an animalistic growl, the only forewarning the distracted lizards get before the first burst issues forth from your [cockHead], smearing the sole of Hel's foot with creamy seed.  The salamander makes a happy gasp as another squirt of cum smears her clawed toes, a particularly strong arc fountaining so high that it smears her thighs.");
...
function helAndValeriaCampThreesomes(player: Character): NextScreenChoices {
    CView.clear();
    // If PC is wearing Valeria:
    if (player.inventory.equipment.armor.displayName === "goo armor") CView.text("Giving Hel a little wink, you ask your current clothing if she'd like to come out and play.  The gooey parts of your armor slither out, Valeria's face materializing out of your breastplate with a grin.  \"<i>Howdy, firebutt,</i>\" the armor-goo says, giving Hel a big hug. Your salamander lover giggles, squeezing Val so hard she pops in half, only to reform beside you.  \"<i>Thanks for the invite, partner... I was getting pretty hungry, you know...</i>\"");
    // {if PC is NOT wearing Valeria:}
    else CView.text("You look over to your stash and give a sharp whistle, calling Valeria over.  An amorphous blue blob soon pops up, forming into Valeria's shapely body as she strides over to give Helia a big hug.  Your salamander lover giggles, squeezing Val so hard she pops in half, only to reform beside you as you ask if she'd like to join in on your fun.  She beams, nodding quickly.  \"<i>Thanks for the invite, partner... I was getting pretty hungry, you know...</i>\"");

    return { next: helXValeriaFemalePC };
}

// Hel x Valeria Female Version
function helXValeriaFemalePC(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("In the blink of an eye, Valeria slithers around Hel's body, almost completely encasing her in thick blue goo, leaving only her head and her resplendent reptilian rack bare.  Hel grins at you, putting a hand to her crotch and taking a handful of goo between her fingers.  With a bit of effort, she shapes a pair of thick blue pricks out of her groin, each shimmering in the sunlight as Hel strokes Valeria's new appendages, coaxing out a drop of slimey pre from both tips.  Valeria's face reappears on Hel's flat belly a moment after she's done, looking down to her groin with glee as she further shapes the erect pillars of slime, morphing them into a thick knotty doggy-dick on top, and a huge-flared horsecock on bottom.  You lick your lips as the dog-dick's knot inflates enticingly.");
...
function helGuardToggle(player: Character): NextScreenChoices {
    CView.clear();
    // [If Hel is NOT guarding Camp]
    if (HelFollowerFlags.HEL_GUARDING === 0) {
        CView.text("You ask Hel if she wouldn't mind watching the perimeter for a few hours during the night.");
        CView.text("\n\n\"<i>Sure thing, [name]!  I've got your back!</i>\" she says, her tail reaching around to give you a pop on the [butt].");
        HelFollowerFlags.HEL_GUARDING = 1;
    }
    // [If Hel IS guarding Camp]
    else {
        CView.text("You tell Hel she doesn't need to keep watch at night anymore.");
        CView.text("\n\n\"<i>Is that a fact?  Well, I guess that gives us more time to party at night!</i>\"");
        HelFollowerFlags.HEL_GUARDING = 0;
    }

    return { next: heliaFollowerMenu };
}

// The Set Up
// Triggered randomly upon visiting Helia in  Requires you have a dick bigger than Helia’s butthole.
function heliaAnalTrainingPrompt(player: Character): NextScreenChoices {
    CView.clear();
    HelFollowerFlags.HELIA_ANAL_TRAINING_OFFERED = 1;
    CView.text("“<i>Hey there, lover mine. Got a minute?</i>” Helia asks with dangerous glint in her eyes.");
    ...
    // Helia menu
    return heliaFollowerMenu(player, false);
}
// Give Her Ale
function giveHeliaAnalTraining(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You hand Helia a bottle of goblin ale and question if she’s sure this won’t make her loose.");
    ...
    if (player.body.balls.count > 0) CView.text("She tilts forward to press her chin into your [sack] and begins to lather your [balls] with frothy, warm, salamander spit. One hand hefts a weighty nut as she works, caressing it as if it would somehow coax the cum out of it. ");
    CView.text("Helia slides her palms along either side of your urethra, letting her fingers handle the central, sensitive bit on the underside, following in the path of slick saliva she left for them to follow. She rubs her nose into the sensitive area just below your [cockHead biggest]. You grunt as pre-cum begins to pour in response, your bloated boner flexing atop Helia’s head as you feel a dangerously powerful orgasm welling up within you.");
    ...
    if (player.body.balls.count > 0) CView.text(", hard enough to lift your nuts in their [sack]");
    CView.text(", and you erupt, cumming a steamy hot load of spunk straight onto Helia’s face, head, and back.");
    if (player.cumQ() < 250) CView.text(" You run out before long, but it’s still enough to make her a white painted mess. She licks the excess from her upper lip and shudders happily.");
    else if (player.cumQ() < 2000) CView.text(" You spurt rope after rope onto her, actually drenching her hair with alabaster moisture. Her back and face look equally smothered with spooge when you finish. She’s happy to wipe the goop out of her eyes and shovel it into her mouth, swallowing it down with a satisfied smile.");
    else {
        CView.text(" You pour ivory waves of spooge across her body, soaking her from head to toe with the fruit of your loins, liberally basting her with baby-batter. She groans as the excess washes out around her knees");
        if (player.cumQ() >= 3000) CView.text(", but you keep cumming, cascading fresh waves onto her until there’s not a single part of her that isn’t stained and dripping with your salty outflow");
        CView.text(". Helia licks her hands clean, wipes her eyes and devours that as well, and then gives you a dripping-wet, sensuous smile.");
    }
    ...
    HelFollowerFlags.HELIA_ANAL_TRAINING = 1;
    if (player.body.cocks.sort(Cock.Largest)[0].area < heliaAnalCapacity()) {
        CView.text("then maybe I can handle you, my hung " + mf(player, "stud", "breeder") + ".");
    }
    else {
        CView.text("then maybe we can see about trying something bigger.");
    }
    CView.text("</i>”");

    player.inventory.items.consumeItem(ConsumableName.GoblinAle, 1);
    player.orgasm();
    player.stats.sens += -1;
...
function heliaGapeSceneChoices(player: Character): NextScreenChoices {
    if (HelFollowerFlags.HELIA_ANAL_TRAINING === 0) return giveHeliaAnalTraining(player);
    else if (HelFollowerFlags.HELIA_ANAL_TRAINING === 1) return heliaAnalTrainingPartTwo(player);
}

// Anal Training #2
function heliaAnalTrainingPartTwo(player: Character): NextScreenChoices {
    player.inventory.items.consumeItem(ConsumableName.GoblinAle, 1);
    CView.clear();
    CView.text("Pulling out another bottle of ale you dangle it before the slutty salamander, saying, “<i>A present for my favorite buttslut.</i>”");
    ...
    return {
        choices: [
            ["Anal Train", yesMakeHeliaAGapeSlut],
            ["Stop", noGapeSlutForHelia],
        ]
    };
}
// Stop the Asspocalypse
function noGapeSlutForHelia(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You step up and kiss Helia, telling her that she doesn’t have to this for you, at least not right now.");
...
function yesMakeHeliaAGapeSlut(player: Character): NextScreenChoices {
    ...
    if (player.stats.str < 50) CView.text("strain to ");
    CView.text("heft it up, pressing the [cockHead biggest] on Helia’s thirsty lips. She sandwitches the " + numToCardinalText(Math.round(player.body.cocks.sort(Cock.Largest)[0].length)) + "-thick shaft in between her tits and left arm while her right tickles the sensitive underside. Her tongue slides around your urethra, collecting every single drop of pre-cum and stimulating nerve endings you didn’t even know you had. You groan and flex powerfully, preparing to dump a thick load into the hot salamander’s mouth. She spreads her lips around your cum-slit and kisses your cock hard, sucking powerfully, drawing the bubbling tide of wide straight out of your [balls] with her hungry movements.");
    ...
    if (player.cumQ() < 1000) CView.text(", ignoring the way your sheer volume of spunk leaks from the corners of her mouth and nose, her belly gradually rounding with cum in addition to thick dildo");
    CView.text(". The slutty salamander eventually pulls it off and ");
    if (player.cumQ() < 2000) CView.text("squeezes a last few dollops onto her face, letting them stain her with your scent.");
    else CView.text("allows you to hose her down with your ongoing waves of salty jism, soaking her in your sexual scent.");
    CView.text(" Helia burps. “<i>Fuck me that was awesome! Whoah, don’t mind me, but I, uh... I think I’m gonna stay here for a little while, maybe just rock up and down till I come down off this buzz. She blushes and begins to finger her snatch again. Just let me... just let me get used to.</i>” She shoos you away and goes back to toying with herself, getting used to her new “flexibility”.");
...
export function heliasBirthday(player: Character): NextScreenChoices {
    ...
    HelFollowerFlags.HELIA_BIRTHDAY_OFFERED = 1;

    return {
        choices: [
            ["Stay Home", stayHomeFromHeliaParty],
            ["Go To Party", goWithHelia],
        ]
    };
}

// Stay Home
function stayHomeFromHeliaParty(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Aww, lame,</i>\" Hel groans, sighing.  \"<i>Fiiiiiine.  I guess I'll just go have awesome fun and party hard without you. Don't stick too hard into that mud, lover,</i>\" she chuckles, giving you a reassuringly affectionate kiss on the cheek before trucking off.");
    return { next: campMenu };
}

function helPartyMenu(player: Character): NextScreenChoices {

    const choices: ScreenChoice[] = [];
    // heliaDrinks
    if (HelFollowerFlags.HELIA_BDAY_DRINKS === 0) choices[0] = ["Drinks", heliaDrinks];
    // heliaHakonAndKiri
    if (HelFollowerFlags.HELIA_BDAY_HAKON_AND_KIRI === 0) choices[1] = ["Hakon 'n Kiri", heliaHakonAndKiri];
    // heliaPhoenixes
    if (HelFollowerFlags.HELIA_BDAY_PHOENIXES === 0) choices[2] = ["Phoenixes", heliaPhoenixes];
    // heliaAndFoxTwins
    if (HelFollowerFlags.HELIA_BDAY_FOX_TWINS === 0) choices[3] = ["Fox Twins", heliaAndFoxTwins];
    // leaveWithoutSex
    choices[4] = ["Leave", leaveWithoutSex];
    // leaveWithGirls
    if (HelFollowerFlags.HELIA_BDAY_FOX_TWINS > 0) choices[5] = ["Leave w/Girls", leaveWithGirls];

    return { choices };
}

// Go with Her
function goWithHelia(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Sounds like fun.  Let's go,</i>\" you say, offering your arm to Hel.  She grins and takes it, holding herself close as she takes the lead, steady paces quickly drawing you out into the vast deserts of Mareth beside your lover.");
    // {If Kiha, Isabella, and/or Valeria are at camp:}
    if (isabellaFollower() || followerKiha() || HelFollowerFlags.VALARIA_AT_CAMP === 1) {
        CView.text("  Looking over your shoulder as you depart, though, you notice that camp looks particularly... empty. Perhaps your other camp-mate");
        if (companionsCount() > 1) CView.text("s are");
        else CView.text(" is");
        CView.text(" out, too? You shrug. You and Hel aren't the only ones with lives, you suppose.");
    }
    ...
    // if less than 7' PC:
    if (player.body.tallness < 84) CView.text("sweeping you off your feet");
    else CView.text("hanging off you");
    CView.text(" as you approach the great gates.  The only one on gate duty today is Urta, the grey-coated fox curled up under a small awning beside the gate with a huge canteen resting beside you.  Her centauress partner is noticeably missing, but Urta still gives you a wave and a grin you can't quite read.");

    CView.text("\n\n\"<i>Hey, cutie,</i>\" she says with a wink, pushing open the door for you.");
    // If PC did UrtaQuest:
    CView.text("\n\n");
    if (HelFollowerFlags.URTA_MET_HEL === 1) CView.text("From the quick look she and the salamander exchange, you get the feeling they've met before. Weird. Still, ");
    CView.text("Urta gives you a small nod as Hel leads you into town, past the half-empty guard post and into the bustling main street.  The market way's particularly busy in the late afternoon, bustling with activity as merchants hawk their wares and the first of the street-walkers start to come out, standing in seductive poses in the shade of the alleys.  More than a few send catcalls your way, a few even offering discounts for couples.");
    ...
    if (player.body.tallness <= 72) CView.text(", her head resting on your shoulder");
    CView.text(".  \"<i>Hey, thanks for coming, [name],</i>\" she says quietly, squeezing your arm, \"<i>Kiri's done nothing but talk about you since the tower.  I think someone has a crush...</i>\"");
    ...
    const list = ["Edryn"];
    if (HelFollowerFlags.VALARIA_AT_CAMP === 1) list.push("Valeria");
    ...
    CView.text((list).join(" and ") + ". You turn to Helia and cock a grin as you see the surprise etched on her face, followed by the growing smile.  \"<i>Happy birthday, Helia!</i>\" they all shout out at once, raising mugs frothing over with beer.");
    ...
    if (HelFollowerFlags.HARPY_QUEEN_EXECUTED > 0) CView.text("who forgave us for the queen's death");
    else CView.text("who wanted to be friends still, even after the Tower");
    CView.text(" and... it sort of became a thing.  Hope you don't mind!</i>\"");
    ...
    if (HelFollowerFlags.HARPY_QUEEN_EXECUTED > 0) CView.text("finally killing that bitch of a queen");
    else CView.text("setting the queen right, but leaving her alive for when it's finally time to bring the fight to Lethice");
    CView.text(".  You nod at his thanks, and his hand is soon replaced by Helia's as she grabs you on her way toward the gigantic cake.  The towering white confectionary seems more than big enough to feed everyone in the group, a huge extravagance on the part of her family.  You hear a quiet whisper about ");
    if (HelFollowerFlags.FREED_VALA === 1) CView.text("a certain waitress fairy having... supplied the icing herself");
    else CView.text("a certain minotaur chef having supplied the icing himself");
    CView.text(" as Hel is handed an oversized knife and urged to blow out the candles atop it, arranged into a perfect 23.");
    ...
    if (HelFollowerFlags.HEL_FOXY_FOURSOME_WARNED === 1) CView.text("a pair of lovely fox-eared herms");
    else CView.text("Miko and Mai");
    CView.text(" burst out, their big breasts covered in nothing but tiny pink pasties and an all-too-small thong sheathing their red pricks.  Arm in arm with tail wagging furiously, they cry out \"<i>Happy birthday, Hel!</i>\" before hopping out and slipping into the salamander's arms.  \"<i>Ladies,</i>\" Hel says, grinning like an idiot, a moment before she's pushed into a waiting chair behind her.  One of the girl's busts is quickly shoved into her face as the rest of the group laughs and starts passing out another round of drinks.");

    // {PC can play each short scene once}
    return helPartyMenu(player);
}

function heliaDrinks(player: Character): NextScreenChoices {
    CView.clear();
    // Drinks
    CView.text("You saunter up to the bar, where the staff have laid out more than enough beers for the entire party.  Edryn and a handful of phoenixes are milling around, exchanging tips for swordsmanship: Edryn advocating charging in with a greatweapon swinging, as the phoenixes try and explain their shield-wall tactics.  You knock back a drink and listen in, watching with interest as the phoenix girls get progressively more and more flustered");
    // if PC is male:
    if (player.body.cocks.length > 0 && player.body.vaginas.length > 0) CView.text(" just as you do");
    CView.text(" in the presence of the steadily mounting pheromone cloud around the lusty centauress.  Before you've finished your drink, all the half-harpies are sporting sizable tents in their britches and are subtly sniffing around, trying to figure out what's happening to them... just before Edryn grabs the lead phoenix by the arm and leads her off into one of the back rooms, shooting the others a wink and a flicking tail over her drooling horse-sex.  The others are quick to follow.");
    HelFollowerFlags.HELIA_BDAY_DRINKS = 1;
    return helPartyMenu(player);
}

// Hakon & Kiri
function heliaHakonAndKiri(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You meander over to the veteran salamander and his half-harpy daughter, and quickly find yourself surrounded by fluttering red feathers as Kiri practically jumps on you, hugging you.  \"<i>Hey, [name], long time no see,</i>\" she says, smiling demurely as she leads you back to where she and her father were standing, removed from the real bustle of the party.");
    ...
    CView.text(".  We salamanders are a bit more open about that sort of thing, though, if you haven't noticed.  Helia's regaled us with more than a few of her 'stories,' let me tell you. Isn't that right, sweetheart?</i>\"");
    ...
    HelFollowerFlags.HELIA_BDAY_HAKON_AND_KIRI = 1;
    return helPartyMenu(player);
}

// Phoenixes
function heliaPhoenixes(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You approach the small squad of phoenixes milling around the cake, mostly talking amongst themselves.  They nod as you approach, a bit wary of the person who recently kicked their asses inside out.  Still, they nod respectfully as you approach.");
    ...
    if (HelFollowerFlags.HARPY_QUEEN_EXECUTED > 0) CView.text("dead");
    else CView.text("deposed");
    CView.text(", he thought it would be good to try and at least... know each other.  I'm not complaining.  Things like this party... never would have seen anything like this under Queen Calais.  It's nice to see new things, meet new people.</i>\"");

    CView.text("\n\nYou see a few glances going toward Edryn and the fox twins");
    if (followerKiha() && isabellaFollower()) CView.text(", then to the fiery dragoness and the ultra-busty cowgirl talking in the corner");
    CView.text(", and you figure you know what they mean.");
    HelFollowerFlags.HELIA_BDAY_PHOENIXES = 1;
    return helPartyMenu(player);
}

// Hel & Fox Twins
function heliaAndFoxTwins(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You approach the foxy strippers, still well into giving Helia a dual lap-dance, their red puppy peckers poking well out of their tiny thongs.  They grin as you approach, and Hel pulls you into her lap, and you quickly find yourself with a pair of big, soft breasts to rest your head on, courtesy of ");
    if (HelFollowerFlags.HEL_FOXY_FOURSOME_WARNED === 1) CView.text("Mai");
    else CView.text("one of the foxes");
    CView.text(".  Hel plants a deep, serpent-tongue filled kiss on your lips, nuzzling her cheek on your [chest].");
    ...
    HelFollowerFlags.HELIA_BDAY_FOX_TWINS = 1;
    return helPartyMenu(player);
}
// Leave {Or: Joke's on you, no (special) birthday sex!}
function leaveWithoutSex(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You decide you've had about enough of the party for now. You slip over to Hel and tell her you're ready to head out when she is.");
...
// Leave w/ Girls
function leaveWithGirls(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You decide you've had about enough of the party for now.  You slip over to Hel and tell her you're ready to head out when she is.");
...
function helAndSluttyHarpy(player: Character): NextScreenChoices {
    if (player.body.cocks.length <= 0 && player.body.vaginas.length <= 0) {
        return helAndSluttyHarpyGenderless(player);
    }
    else if (player.body.cocks.length > 0 && player.body.vaginas.length <= 0) {
        return helAndSluttyHarpyMale(player);
    }
    else if (player.body.cocks.length <= 0 && player.body.vaginas.length > 0) {
        return helAndSluttyHarpyFemale(player);
    }

    // Choice

    const choices: ScreenChoice[] = [];
    choices[0] = ["Cock", helAndSluttyHarpyMale];
    choices[1] = ["Pussy", helAndSluttyHarpyFemale];
    choices[9] = ["Back", heliaThreesomes];

    return { choices };
}

function helAndSluttyHarpyMale(player: Character): NextScreenChoices {
    CView.clear();

    CView.text("You take Helia’s hand and lead her through the camp in an attempt to find someone to include in your threesome. After some searching you see Sophie sitting on a rock and get a wonderful idea. You ask Hel if she would be ok with a threesome with the busty harpy. Her smile turns to a serious frown for a moment.");
    ...
    if (player.body.cocks.sort(Cock.Largest)[0].length >= 8) CView.text(" no longer be hilted inside");
    else CView.text(" slip out of");
    CView.text(" her sodden cunt, and hear Hel gasp for air. Before the birdslut can comment on the matter, you spin her around and push her down onto Helia, leaving them face to face.");
    ...
    if (player.body.cocks.length === 1) {
        ...
        CView.text(" to boil over. White jissom blasts from your [cock], coating Helia and Sophie’s stomachs, as well as the underside of the pair’s tits.");
        if (player.body.vaginas.length > 0) CView.text(" Your unused [vagina] cums with your cock in a show of sympathetic orgasm, coating your thighs with femcum.");
    }
    // 2-3 cocks
    else if (player.body.cocks.length <= 3) {
        ...
        if (player.cumQ() < 500) CView.text(" with your cum");
        else {
            CView.text(" full of cum");
            if (player.cumQ() <= 1000) {
                CView.text(" until it overflows");
                if (player.cumQ() >= 5000) CView.text(", pooling around your knees");
            }
        }
        CView.text(".");
        if (player.body.cocks.length === 3) CView.text(" Your unused cock splatters the two with hot, white jizz.");
        if (player.body.vaginas.length > 0) CView.text(" Your unused [vagina] cums with your cocks in a show of sympathetic orgasm, coating your thighs with femcum.");
    }
    // > 4 cocks
    else {
        ...
        if (HelFollowerFlags.HELIA_ANAL_TRAINING === 0) CView.text(" tight");
        else if (HelFollowerFlags.HELIA_ANAL_TRAINING === 1) CView.text(" well-trained");
        else if (HelFollowerFlags.HELIA_ANAL_TRAINING >= 2) CView.text(" gaped");
        CView.text(" asshole. She gasps at the sudden double penetration, and you reciprocate because of the furnace-like heat of both her holes. Your [cock biggest3] lines up with Sophie’s slutty pussy and drives home while your [cock smallest] does the same to her ass."); // Haha look at me being a lazy shit.
        ...
        CView.text(". The two girls seem to share your problem, their teeth clenched and clinging tightly to each other trying to resist. Seeing their clearly shared pleasure in the moment puts you over the edge, and you let loose the torrent of liquid lust from [eachCock]. Sophie cums as the tsunami of jizz floods her levies, the copious amount spilling back out of her holes and pooling around your knees.");
        if (player.body.cocks.length > 4) CView.text(" Your extra cocks spray pearly cum all over the nearby ground.");
        if (player.body.vaginas.length > 0) CView.text(" Your unused [vagina] cums with your cocks in sympathetic orgasm, coating your thighs with femcum.");
    }
    // Merge
...
function helAndSluttyHarpyFemale(player: Character): NextScreenChoices {
    ...
    CView.text("\n\n<i>“Ooooh yeahh, thats like, perfect [name], keep licking mama Sophie like that”</i> Sophie coos between moans, as if you needed to be told. Helia seems to notice that you’re getting more into it, and ups her  You moan loudly as Helia’s long tongue slips deeper into your cunt, licking your walls as it goes. Her mouth sucks on your hole and her bottom lip brushes against your [clit].");
    ...
    if (player.body.cocks.length >= 1) {
        CView.text("The blasts spatter against the underside of Sophie’s gigantic melons and slide off Hel’s sides like an overfull cream pastry.");
    }
...
function helAndSluttyHarpyGenderless(player: Character): NextScreenChoices {
    CView.clear();

    CView.text("You walk up to Helia and ask if she would be up for fucking you with that wondrous tail of hers. She smiles slyly and wraps her arms around you. You take her response as an unspoken yes, hugging her back and pulling her off to find a better place for your loving.");
