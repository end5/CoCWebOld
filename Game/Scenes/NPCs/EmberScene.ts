...
import { campFollowers, returnToCampUseOneHour, returnToCampUseTwoHours } from "../Camp";
...
import { describeOneOfYourCocks, describeCocksLight, describeCock, describeCockHead } from "../../Descriptors/CockDescriptor";
import { mf } from "../../Descriptors/GenderDescriptor";
import { nagaRaceScore, lizardRaceScore, dragonRaceScore } from "../../Body/RaceScore";
...
import { displayGoIntoRut, displayGoIntoHeat } from "../../Modifiers/BodyModifier";
import { FlagWomb } from "../../Body/Pregnancy/FlagWomb";

export const EmberFlags = {
    MINOTAUR_CUM_ADDICTION_STATE: 0,
    EMBER_CURRENTLY_FREAKING_ABOUT_MINOCUM: 0,
    EMBER_LUST_BITCHING_COUNTER: 0,
    EMBER_BITCHES_ABOUT_PREGNANT_PC: 0,
    DRANK_EMBER_BLOOD_TODAY: 0,
    EMBER_GENDER: 0,
    EMBER_AFFECTION: 0,
    EMBER_COR: 0,
    EMBER_HATCHED: 0,
    EMBER_CHILDREN_MALES: 0,
    EMBER_CHILDREN_FEMALES: 0,
    EMBER_CHILDREN_HERMS: 0,
    EMBER_ROUNDFACE: 0,
    EMBER_INTERNAL_DICK: 0,
    EMBER_OVIPOSITION: 0,
    EMBER_MILK: 0,
    TIMES_FOUND_EMBERS_EGG: 0,
    EGG_BROKEN: 0,
    TOOK_EMBER_EGG: 0,
    EMBER_EGG_FLUID_COUNT: 0,
    EMBER_JACKED_ON: 0,
    EMBER_HAIR: 0,
    EMBER_OVI_BITCHED_YET: 0,
    EMBER_TALKS_TO_PC_ABOUT_PC_MOTHERING_DRAGONS: 0,
    EMBER_PREGNANT_TALK: 0,
    TAMANI_NUMBER_OF_DAUGHTERS: 0,
    EMBER_EGGS: 0,
    TIMES_BUTTFUCKED_EMBER: 0,
    EMBER_PUSSY_FUCK_COUNT: 0,
    EMBER_INCUBATION: 0,
    TIMES_EMBER_LUSTY_FUCKED: 0,
};
User.flags.set(FlagType.Ember, EmberFlags);
/**
...
// EMBER_OVI_BITCHED_YET: number = 540; //Used to trigger return emberBitchesAboutPCBeingFullOfEggs(player, )
// EMBER_LUST_BITCHING_COUNTER: number = 541;
// EMBER_CURRENTLY_FREAKING_ABOUT_MINOCUM: number = 542; // Used to trigger return minotaurJizzFreakout(player, )
...
class EmberTimeAware implements ITimeAware {
    ...
    public timeChange(player: Character): boolean {
        ...
        // trace("\nEmber time change: Time is " + Time.hour + ", incubation: " + pregnancy.incubation + ", event: " + pregnancy.event);
        if (this.flagWomb.isPregnant()) {
            if (emberPregUpdate(player)) needNext = true;
            if (this.flagWomb.pregnancy.incubation === 0) {
                emberGivesBirth(player);
                this.flagWomb.clear(); // Clear Pregnancy
                needNext = true;
            }
        }
        // Ember fuck cooldown
        if (player.effects.get(StatusEffectType.EmberFuckCooldown).value1 > 0) {
            player.effects.get(StatusEffectType.EmberFuckCooldown).value1 += -1;
            if (player.effects.get(StatusEffectType.EmberFuckCooldown).value1 < 1) player.effects.remove(StatusEffectType.EmberFuckCooldown);
        }
        // Ember napping
        if (player.effects.has(StatusEffectType.EmberNapping)) {
            player.effects.get(StatusEffectType.EmberNapping).value1 += -1;
            if (player.effects.get(StatusEffectType.EmberNapping).value1 <= 0) player.effects.remove(StatusEffectType.EmberNapping);
        }
        if (followerEmber() && !player.effects.has(StatusEffectType.EmberNapping)) {
            // Mino cum freakout - PC partly addicted!
            if (EmberFlags.MINOTAUR_CUM_ADDICTION_STATE === 1 && !player.perks.has(PerkType.MinotaurCumAddict) && EmberFlags.EMBER_CURRENTLY_FREAKING_ABOUT_MINOCUM === 0) {
                minotaurJizzFreakout();
                needNext = true;
            }
            // Ember is freaking out about addiction, but PC no longer addicted!
            else if (EmberFlags.MINOTAUR_CUM_ADDICTION_STATE === 0 && EmberFlags.EMBER_CURRENTLY_FREAKING_ABOUT_MINOCUM === 1) {
            ...
            if (player.stats.lust >= 100 && player.gender > 0) {
                EmberFlags.EMBER_LUST_BITCHING_COUNTER++;
                if (EmberFlags.EMBER_LUST_BITCHING_COUNTER >= 10) {
                    emberBitchesAtYouAboutLustiness(player);
                    needNext = true;
                }
            }
            // Reset lust counter if not max lust'ed
            else EmberFlags.EMBER_LUST_BITCHING_COUNTER = 0;
        }
        if (Time.hour > 23) {
            if (!player.body.wombs.find(Womb.Pregnant)) EmberFlags.EMBER_BITCHES_ABOUT_PREGNANT_PC = 0;
            EmberFlags.DRANK_EMBER_BLOOD_TODAY = 0;
        }
        return needNext;
    }

    public timeChangeLarge(player: Character): NextScreenChoices {
        if (!player.effects.has(StatusEffectType.EmberNapping) && followerEmber() && !player.effects.has(StatusEffectType.EmberFuckCooldown)) {
            // Ember get's a whiff of fuckscent and knocks up PC!
            if (player.body.vaginas.length > 0 && player.effects.has(StatusEffectType.Heat) && player.body.wombs.find(Womb.NotPregnant) && randInt(10) === 0 && (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3)) {
                return emberRapesYourHeatness(player);
            }
            else if (player.body.cocks.length > 0 && player.effects.has(StatusEffectType.Rut) && !this.flagWomb.isPregnant() && randInt(10) === 0 && EmberFlags.EMBER_GENDER >= 2) {
                return emberRapesYourHeatness(player);
            }
        }
    }

    public serialize(): void | object {
        return {
            flagWomb: this.flagWomb.serialize(),
        };
    }

    public deserialize(saveObject: EmberTimeAware): void {
        this.flagWomb.deserialize(saveObject.flagWomb);
    }
}

const emberPregEvents = [330, 270, 200, 180, 100, 75, 48, 15];
// Event: 0 (= not pregnant),  1,   2,   3,   4,   5,  6,  7,  8 (< 15)

const emberTimeAware = new EmberTimeAware();

const emberWomb = emberTimeAware.flagWomb;

export function emberAffection(changes: number = 0): number {
    EmberFlags.EMBER_AFFECTION += changes;
    if (EmberFlags.EMBER_AFFECTION > 100) EmberFlags.EMBER_AFFECTION = 100;
    else if (EmberFlags.EMBER_AFFECTION < 0) EmberFlags.EMBER_AFFECTION = 0;
    return EmberFlags.EMBER_AFFECTION;
}

function emberCorruption(changes: number = 0): number {
    EmberFlags.EMBER_COR += changes;
    if (EmberFlags.EMBER_COR > 100) EmberFlags.EMBER_COR = 100;
    else if (EmberFlags.EMBER_COR < 0) EmberFlags.EMBER_COR = 0;
    return EmberFlags.EMBER_COR;
}

export function followerEmber(): boolean {
    return EmberFlags.EMBER_HATCHED > 0;

}

export function emberMF(man: string, woman: string): string {
    if (EmberFlags.EMBER_GENDER === 1) return man;
    else return woman;
}
...
function emberHasCock(): boolean {
    return (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3);
}

export function emberChildren(): number {
    return (EmberFlags.EMBER_CHILDREN_MALES + EmberFlags.EMBER_CHILDREN_FEMALES + EmberFlags.EMBER_CHILDREN_HERMS);
}

function emberInternalDick(): boolean {
    return (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK > 0);
}

// Approaching Ember (Z)
export function emberCampMenu(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("ember-visit-at-camp");
    // Low Affection:
    if (emberAffection() <= 25) CView.text("Ember sighs as you approach, and doesn't even look you in the eye before speaking.  \"<i>What do you want?</i>\"");
    // Moderate Affection:
    else if (emberAffection() <= 75) CView.text("Ember fidgets as " + emberMF("his", "her") + " tail starts to sway from side to side, " + emberMF("he", "she") + " looks at you and asks, \"<i>What is it?</i>\"");
    else CView.text("Ember's eyes light up as you close in on " + emberMF("him", "her") + ", and " + emberMF("he", "she") + " smiles nervously.  \"<i>Y-Yes?</i>\"");
    // OPTIONS HERE
    // [APPEARANCE]
    ...
    if (EmberFlags.EMBER_OVIPOSITION > 0 && EmberFlags.EMBER_GENDER >= 2 && !emberWomb.isPregnant()) egg = emberIsAnEggFactory;
    if (EmberFlags.EMBER_MILK > 0) milk = getMilkFromEmber;
    return { choices: [["Appearance", embersAppearance], ["Talk", talkToEmber], ["DrinkBlood", bloodForTheBloodGod], ["Drink Milk", milk], ["Get Egg", egg], ["Sex", emberSexMenu], ["Spar", decideToSparEmbra], ["", undefined], ["", undefined], ["Back", campFollowers]] };
}

// Approach for sex - initial output when selecting [Sex] menu (Z)
function emberSexMenu(player: Character, output: boolean = true): NextScreenChoices {
    if (output) {
        CView.clear();
        CView.text("You ogle Ember, checking out the nuances of " + emberMF("his", "her") + " body.");
        // (Low Affection)
        if (emberAffection() <= 25) CView.text("\n\n\"<i>Why are you looking at me like that?</i>\" " + emberMF("he", "she") + " says, flatly.");
        // (Medium Affection)
        else if (emberAffection() < 75) CView.text("\n\n\"<i>What is it?  Is something wrong with my body?</i>\" Ember asks, checking " + emberMF("him", "her") + "self.");
        // (High Affection)
        ...
        CView.text("  ");
        if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) {
            CView.text(emberMF("His", "Her") + " cock ");
            if (EmberFlags.EMBER_INTERNAL_DICK === 1 || EmberFlags.EMBER_ROUNDFACE === 0) CView.text("is poking out of " + emberMF("his", "her") + " slit");
            else CView.text("is starting to swell with blood");
            CView.text(".  ");
        }
        if (EmberFlags.EMBER_GENDER >= 2) CView.text("You can see tiny rivulets of moisture starting to run down Ember's inner thighs as they rub together, barely hiding her precious treasure from your hungry eyes.  ");
        CView.text("Well, " + emberMF("he", "she") + " is a sexy beast; you ask what naturally comes to mind.");
    ...
    let catchAnalFunc: ClickFunction;
    let pitchAnal: ClickFunction;
    let blowEmber: ClickFunction;
    let getBlown: ClickFunction;
    let eatOut: ClickFunction;
    let getEatenOut: ClickFunction;
    let penetrateHer: ClickFunction;
    let getPenetrated: ClickFunction;
    ...
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) {
        ...
        catchAnalFunc = catchAnal;
        if (player.stats.lust >= 33 && player.body.vaginas.length > 0) {
            getPenetrated = getPenetratedByEmberLastSexSceneWoooo;
        }
    }
    // scenes that require Ember to have a cunt
    if (EmberFlags.EMBER_GENDER >= 2) {
        eatOut = slurpDraggieCunnies;
        if (player.body.cocks.length > 0 && player.stats.lust >= 33) penetrateHer = penetrateEmbrah;
    }
    if (player.body.vaginas.length > 0 && player.stats.lust >= 33) getEatenOut = getEatenOutByEmbra;
    if (player.body.cocks.length > 0 && player.stats.lust >= 33) {
        getBlown = stickDickInKnifeDrawer;
        pitchAnal = stickItInEmbersButt;
    }
    // return { choices: [ ["Catch Anal", catchAnal], ["Pitch Anal", pitchAnal], ["Blow Ember", blowEmber], ["Get Blown", getBlown], ["Eat Her Out", eatOut], ["Get Eaten Out", getEatenOut], ["Penetrate Her", penetrateHer], ["Get Penetrated", getPenetrated], ["", 0], ["Leave", emberCampMenu],  ] };

    const choices: ScreenChoice[] = [];
    choices[0] = ["Catch Anal", catchAnalFunc];
    choices[1] = ["Pitch Anal", pitchAnal];
    choices[2] = ["Blow Ember", blowEmber];
    choices[3] = ["Get Blown", getBlown];
    choices[4] = ["Eat Her Out", eatOut];
    choices[5] = ["Get Eaten Out", getEatenOut];
    choices[6] = ["Penetrate Her", penetrateHer];
    choices[7] = ["Get Penetrated", getPenetrated];
    if (emberAffection() >= 95 && player.body.cocks.length > 0 && player.body.cocks.find(Cock.CockThatFits(emberVaginalCapacity())) && (player.inventory.items.has(ConsumableName.LustDraft) || player.stats.lib >= 50 || player.stats.minLust() >= 40))
        choices[8] = ["LustyFuck", highAffectionEmberLustFuck];
    choices[9] = ["Leave", emberCampMenu];
...
export function findEmbersEgg(player: Character): NextScreenChoices {
    CView.clear();
    if (EmberFlags.TIMES_FOUND_EMBERS_EGG === 0) {
        CView.text("You spot a cave entrance partially hidden behind mossy vegetation and decide to investigate.");
        ...
        CView.text("You spot a familiar cave partially hidden behind the mossy vegetation and decide to confirm your suspicion.");
    ...
    EmberFlags.TIMES_FOUND_EMBERS_EGG++;
    return { choices: [["Take It", takeEmbersEggHomeInADoggieBag], ["Destroy It", destroyBabyEmberYouMonster], ["", undefined], ["", undefined], ["Leave", leaveEmbersAssOutToDry]] };
}

// [=Leave=] (Z)
function leaveEmbersAssOutToDry(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You can't decide what to do right now, so you leave the egg where it is and return to your ");
...
// [=Destroy it=] (Z)
function destroyBabyEmberYouMonster(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Raising your [weapon], you rain down blow after blow upon the egg.  The shell is freakishly tough, taking a lot of punishment before it shatters apart to spill a wave of egg white onto your " + describeFeet(player) + "; a great pulpy mass of weirdly bluish-red yolk remains in the broken shell.");
    ...
    EmberFlags.EGG_BROKEN = 1;
    // [Eat][Leave]
    return { choices: [["Eat It", eatEmbersYolkLikeAnEvenBiggerDick], ["", undefined], ["", undefined], ["", undefined], ["Leave", returnToCampUseOneHour]] };
}

// [=Eat=]
function eatEmbersYolkLikeAnEvenBiggerDick(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Unsure of where the impulse comes from, but uncaring, you crouch over the ruined shell of your 'kill' and begin messily scooping handfuls of yolk into your mouth.");
    ...
    player.inventory.keyItems.add("Dragon Eggshell", 0, 0, 0, 0);
    // (+5-10 to strength, toughness, and speed.)
    // (+20 Corruption)
    // (also slimefeed!)
    player.stats.str += 5 + randInt(5);
    player.stats.tou += 5 + randInt(5);
    player.stats.int += 5 + randInt(5);
    player.stats.cor += 20;
...
// [Yes]
export function getSomeStuff(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Your mouth tightens in consternation, and you pull out the shell of the so-called 'dragon egg', passing it over and asking if she can use it.");
    ...
    player.inventory.keyItems.remove("Dragon Eggshell");
    return player.inventory.items.createAdd(player, ItemType.Weapon, WeaponName.DragonShellShield, armorShop);
}
...
// [=Take=] (Z)
function takeEmbersEggHomeInADoggieBag(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You decide to take the egg, figuring that perhaps this dragon could aid you in your quest.");
    // (If player is shorter than 7 feet)
    if (player.body.tallness < 84) CView.text("  Lifting it isn't as much of a problem as you thought; it's surprisingly light.  It is, however, very big and very awkward to carry.");
    else CView.text("  Between the egg's surprising lightness, and your own size and wide arms, you can easily carry the egg.");
    ...
    // set flags
    player.inventory.keyItems.add("Dragon Egg", 0, 0, 0, 0);
    EmberFlags.TOOK_EMBER_EGG = 1;
    EmberFlags.EMBER_COR = 50;
    return { next: returnToCampUseOneHour };
}

// Modified Camp Description (Z)
export function emberCampDesc() {
    // Iz Ember an egg?
    if (EmberFlags.EMBER_HATCHED === 0) CView.text("\nThat mysterious egg that you brought back to the camp is sitting in the crude nest you made.\n");
    // NOT AN EGG! HAHA!
    else {
        const choice: number = randInt(3);
        if (choice === 0) CView.text("Ember is lying in front of " + emberMF("his", "her") + " excavated den, watching the camp and resting.  Every once in a while " + emberMF("his", "her") + " eyes dart in your direction, before " + emberMF("he", "she") + " quickly looks away.\n\n");
        else if (choice === 1) CView.text("Ember doesn't seem to be around " + emberMF("his", "her") + " excavated den... it doesn't take long before Ember lands in front of it and then takes off again, apparently occupied with darting around in the sky for fun.\n\n");
        else CView.text("Ember appears to be fighting to stay awake; sometimes " + emberMF("he", "she") + " falls into a doze, but quickly snaps back to consciousness.\n\n");
    }
}

// Followers Descriptor (Z)
function emberFollowerDesc() {
    CView.text("The mysterious egg you found in the cave sits in the grass nest you assembled for it; it is three feet tall and nearly two feet in circumference.  The nest itself isn't very pretty, but at least it's sturdy enough to keep the egg from rolling around.\n\n");
}
...
export function emberEggInteraction(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You approach the egg you found in that illusion-concealed cave. Though the light continues to pulse with its heartbeat overtones, it still just sits there, doing nothing.");
    // (If the egg Corruption level is 0-25, aka \"<i>Pure</i>\")
    if (EmberFlags.EMBER_COR <= 25) {
        CView.text("  As you observe the egg, it glows with soft, metered pulses and you're overcome with a sense of calm and peace.  Watching them, you feel serene, as if untouched by this world's taint.");
    }
    // (else If the egg Corruption level is 26-74, aka \"<i>Tainted</i>\")
    else if (EmberFlags.EMBER_COR <= 75) {
        CView.text("  As you observe the egg, it glows with bright, gaudy pulses and you're overcome with a sense of arrogance and strength.  You feel somehow as if you could do anything, as if you were superior to everything, and no one could dare say otherwise.");
    }
    // (If the egg Corruption level is 75-100, aka \"<i>Corrupt</i>\")
    else {
        CView.text("  As you observe the egg you realize what you had taken for its pulses are actually just its normal color; the egg is actually 'glowing' with a black light!  As you stare, mesmerized, you begin to consider the pleasing contrast that would result if you covered it in your ");
        if (player.gender === 0) CView.text("cum, if you had any... ");
        else {
            if (player.body.cocks.length > 0) CView.text("white ");
            else CView.text("glistening girl-");
            CView.text("cum... ");
        }
        CView.text(" You stop yourself and shake your head.  Where did that thought come from?");
        player.stats.lust += 10 + player.stats.cor / 10;

    }
    // (If player has lust >= 33)
    if (player.stats.lust >= 33 && (EmberFlags.EMBER_EGG_FLUID_COUNT < 5 || EmberFlags.EMBER_JACKED_ON === 0)) {
        ...
        if (EmberFlags.EMBER_EGG_FLUID_COUNT === 4) {
        ...
        // [Yes][No]
        // [= Yes =]
        return { yes: masturbateOntoAnEgg, no: dontEggFap };
    ...
    let fap: ClickFunction;
    if (player.stats.lust >= 33) fap = masturbateOntoAnEgg;
    let draft: ClickFunction;
    if (player.inventory.items.has(ConsumableName.IncubusDraft)) draft = partial(useIncubusDraftOnEmber, player, false);
    let pDraft: ClickFunction;
    if (player.inventory.items.has(ConsumableName.IncubusDraftPure)) pDraft = partial(useIncubusDraftOnEmber, player, true);
    let milk: ClickFunction;
    if (player.inventory.items.has(ConsumableName.SuccubiMilk)) milk = partial(useSuccubiMilkOnEmber, player, false);
    let pMilk: ClickFunction;
    if (player.inventory.items.has(ConsumableName.SuccubiMilkPure)) pMilk = partial(useSuccubiMilkOnEmber, player, true);
    let hair: ClickFunction;
    if (player.inventory.items.has(ConsumableName.HairExtensionSerum)) hair = hairExtensionSerum;
    let ovi: ClickFunction;
    if (player.inventory.items.has(ConsumableName.OvipositionElixir)) ovi = useOviElixerOnEmber;
    let lactaid: ClickFunction;
    if (player.inventory.items.has(ConsumableName.Lactaid)) lactaid = useLactaidOnEmber;
    let hatch: ClickFunction;
    if (EmberFlags.EMBER_EGG_FLUID_COUNT >= 5 && EmberFlags.EMBER_JACKED_ON > 0 && EmberFlags.EMBER_GENDER > 0) {
        hatch = hatchZeMuzzles;
        CView.text("\n\n<b>The egg is ready to be hatched - if you're just as ready.</b>");
    }
    if (hatch !== undefined) return {
        choices: [
            ["Hatch", hatch],
            ["Blood", giveEmberBludSausages],
            ["IncubiDraft", draft],
            ["Pure Draft", pDraft],
            ["Succubi Milk", milk],
            ["Pure Milk", pMilk],
            ...
            ["Ovi Elixir", ovi],
            ["Lactaid", lactaid],
            ["Back", leaveWithoutUsingAnEmberItem]
        ]
    };
    else if (fap !== undefined) return {
        choices: [
            ["Masturbate", fap],
            ["Blood", giveEmberBludSausages],
            ["IncubiDraft", draft],
...
// [= No =]
function dontEggFap(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Shaking your head, confused and startled by these strange impulses, you step away for a moment. Once away from the egg, its pattern of pulsations returns to normal and you feel the urges disappear.");
    ...
    if (player.inventory.items.has(ConsumableName.Lactaid)) lactaid = useLactaidOnEmber;
    let hatch: ClickFunction;
    if (EmberFlags.EMBER_EGG_FLUID_COUNT >= 5 && EmberFlags.EMBER_JACKED_ON > 0 && EmberFlags.EMBER_GENDER > 0) hatch = hatchZeMuzzles;

    CView.text("  The egg's rhythm suddenly changes; as if it were excited by something - something that you have brought near it.");
    ...
    if (hatch !== undefined) CView.text("\n\n<b>The egg is ready to be hatched - if you're just as ready.</b>");

    if (hatch !== undefined) return {
        choices: [
            ["Hatch", hatch], ["Blood", giveEmberBludSausages], ["IncubiDraft", draft], ["Pure Draft", pDraft], ["Succubi Milk", milk], ["Pure Milk", pMilk], ["Hair Serum", hair], ["Ovi Elixir", ovi], ["Lactaid", lactaid], ["Back", leaveWithoutUsingAnEmberItem]
        ]
    };
    else if (fap !== undefined) return {
        choices: [
            ["Masturbate", fap], ["Blood", giveEmberBludSausages], ["IncubiDraft", draft], ["Pure Draft", pDraft], ["Succubi Milk", milk], ["Pure Milk", pMilk], ["Hair Serum", hair], ["Ovi Elixir", ovi], ["Lactaid", lactaid], ["Back", leaveWithoutUsingAnEmberItem]
        ]
    };
    else return {
        choices: [
...
function leaveWithoutUsingAnEmberItem(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You shake your head; it would probably be best not to tamper with it. Returning the items to your pockets, you leave the egg alone.  As you put them away, the egg's glow slows down dramatically, almost as if it were feeling... disappointment?");
    return { next: inventoryMenu };
}

// Incubus Draft/Purified Incubus Draft (Z)
function useIncubusDraftOnEmber(player: Character, purified: boolean = false): NextScreenChoices {
    CView.clear();
    if (purified) {
        player.inventory.items.consumeItem(ConsumableName.IncubusDraftPure);
        EmberFlags.EMBER_COR -= 10;
        if (EmberFlags.EMBER_COR < 0) EmberFlags.EMBER_COR = 0;
    }
    else {
        player.inventory.items.consumeItem(ConsumableName.IncubusDraft);
        EmberFlags.EMBER_COR += 10;
        if (EmberFlags.EMBER_COR > 100) EmberFlags.EMBER_COR = 100;
    }
    CView.text("Uncorking the vial, you drizzle the slimy off-white fluid onto the pointed cone of the egg.  It oozes slowly across the surface, then seeps through the shell, leaving not a drop of moisture.");
    if (EmberFlags.EMBER_GENDER === 3 || EmberFlags.EMBER_GENDER === 0) {
        CView.text("  The egg's shell slowly changes to a soft, pastel blue.");
        EmberFlags.EMBER_GENDER = 1;
    }
    else if (EmberFlags.EMBER_GENDER === 2) {
        CView.text("   The egg's shell slowly changes to a lavender hue.");
        EmberFlags.EMBER_GENDER = 3;
    }
    EmberFlags.EMBER_EGG_FLUID_COUNT++;
    return { next: campMenu };
}

// Succubi Milk/Purified Succubi Milk (Z)
function useSuccubiMilkOnEmber(player: Character, purified: boolean = false): NextScreenChoices {
    CView.clear();
    if (purified) {
    ...
    CView.text("Popping the cap off of the milk bottle, you pour the contents onto the egg - the porous shell soaks up the milk as fast as you dump it, spilling not a drop.");
    // (If Unsexed or Herm:
    if (EmberFlags.EMBER_GENDER === 3 || EmberFlags.EMBER_GENDER === 0) {
        CView.text("  The egg's shell slowly changes to a muted pink color.");
        EmberFlags.EMBER_GENDER = 2;
    }
    // If Male:
    else if (EmberFlags.EMBER_GENDER === 1) {
        CView.text("  The egg's shell slowly changes to a lavender hue.");
        EmberFlags.EMBER_GENDER = 3;
    }
    EmberFlags.EMBER_EGG_FLUID_COUNT++;
...
// Ovi Elixir (Z)
function useOviElixerOnEmber(player: Character): NextScreenChoices {
    CView.clear();
    player.inventory.items.consumeItem(ConsumableName.OvipositionElixir);
    // max uses 1
    CView.text("Uncorking the crystalline bottle, you pour the strange green liquid inside onto the egg, briefly wondering what on earth it could want with this stuff, before catching your fallacy.  It's an egg, right?  It can't want things...  The fluid spills all over the shell, coating it, and then seeps inside, leaving the egg's previously pale surface marked with small green splotches.");
    EmberFlags.EMBER_OVIPOSITION = 1;
    EmberFlags.EMBER_EGG_FLUID_COUNT++;
    return { next: campMenu };
}

// Lactaid (Z)
function useLactaidOnEmber(player: Character): NextScreenChoices {
    CView.clear();
    player.inventory.items.consumeItem(ConsumableName.Lactaid);
    // max uses 1
    CView.text("Feeling a little bemused, you pour the creamy fluid onto the egg.  It is absorbed through the shell, and a spiderwork of creamy yellow vein-like markings suddenly forms on the shell's surface.");
    EmberFlags.EMBER_MILK = 1;
    EmberFlags.EMBER_EGG_FLUID_COUNT++;
    return { next: campMenu };
}

// Hair Extension Serum (Z)
function hairExtensionSerum(player: Character): NextScreenChoices {
    CView.clear();
    player.inventory.items.consumeItem(ConsumableName.HairExtensionSerum);
    // max uses 2
    CView.text("Wondering at your motivations, you pour the goblin gunk onto the egg.  Most rolls harmlessly off of the shell, leaving you annoyed at the waste... until you see ");
    if (EmberFlags.EMBER_HAIR === 0) {
        CView.text("a narrow tiger-stripe pattern suddenly develop");
        EmberFlags.EMBER_HAIR = 1;
    }
    else {
        CView.text("the tiger-stripes multiply");
        EmberFlags.EMBER_HAIR = 2;
    }
    CView.text(" on the egg.");
    EmberFlags.EMBER_EGG_FLUID_COUNT++;
    return { next: campMenu };
}

// Your Blood (Z)
function giveEmberBludSausages(player: Character): NextScreenChoices {
    CView.clear();
    // max uses 2
    CView.text("Examining your hand and the egg's reaction to it, you wonder if this is what the book meant by \"<i>sharing your essence</i>\".  Could be worth trying.  Wincing in pain as you bite the skin on your thumb, you smear the bloody digit along the surface of the egg, marking its exterior in crimson.  Shortly thereafter the blood is absorbed, leaving only a stain.  You wait expectantly for something else to happen");
    // [(0 prior),
    if (EmberFlags.EMBER_ROUNDFACE === 0) {
        CView.text(" but the egg just glows its excitement, as if it wanted still more.");
        EmberFlags.EMBER_INTERNAL_DICK = 1;
    }
    else {
        CView.text(", but nothing does.  How disappointing.");
        EmberFlags.EMBER_INTERNAL_DICK = 0;
    }
    player.stats.HP--;
    EmberFlags.EMBER_ROUNDFACE = 1;
...
function masturbateOntoAnEgg(player: Character): NextScreenChoices {
    CView.clear();
    if (player.gender === 0) {
        CView.text("The light pulses decrease in speed as you disrobe and expose your bare crotch, leaving you disappointed after summoning your perversity to bring you this far.  You feel as if you've let it down somehow...  This is confusing!  You decide to go away and deal with this fickle egg another time.");
        return { next: returnToCampUseOneHour };
    }
    // Nothing changes. PC can go do something else, lose no time.
    // Male Version (Z)
    if (player.gender === 1) {
        CView.text("The light of the egg pulses rapidly, throwing strange illumination and shadow over your form as you hastily peel off your [armor], too far gone to recognize the absurdity.  Your heart is racing so fast with excitement, lust, anticipation... it actually matches the tempo of the pulses from the egg, when you care to notice.");
        CView.text("\n\nGrabbing your [cock] in your hands, you stand in front of the egg, ");
        if (player.body.cocks.length <= 2) CView.text("pumping vigorously.");
        else CView.text("wrangling all your shafts together into one awkward bouquet of male organs and furiously stroking and squeezing them as best you can manage");
        CView.text(".  The egg's pulsations lure you on, coaxing you to squeeze, pull, thrust, and massage " + describeOneOfYourCocks(player) + " as best you can.  Harder and faster you go, feeling the churning ache from deep inside you.  Finally, with a cry of release, you unleash a ");
        if (player.cumQ() < 100) CView.text("trickle");
        else if (player.cumQ() <= 500) CView.text("stream");
        else if (player.cumQ() <= 1000) CView.text("gout");
        else CView.text("wave");
        CView.text(" of cum onto the egg.");
    ...
    // Female Version (Z)
        ...
        if (player.body.vaginas.get(0).wetness <= 3) CView.text("few drops");
        else if (player.body.vaginas.get(0).wetness < 5) CView.text("squirt");
        else CView.text("torrent");
        CView.text(" of girlcum jumps from your pussy onto the egg.");
        ...
        // Herm Version (Z)
        CView.text("The light of the egg pulses rapidly, throwing strange illumination and shadow over your form as you hastily peel off your [armor], too far gone to recognize the absurdity.  Your heart is racing so fast with excitement, lust, anticipation... it actually matches the tempo of the pulses from the egg, when you care to notice.");
        CView.text("\n\nTormented by the need in both your " + describeCocksLight(player) + " and your [vagina], you awkwardly straddle the egg's upper surface, allowing you to grind your netherlips against its shell and stroke [eachCock] at the same time.  It is an awkward, herky-jerky act, struggling to avoid falling off... but the sensation so makes up for it.  Your [vagina] slides and grinds against the egg's hard, unyielding shell as your hand tugs and pulls ");
        if (player.body.cocks.length === 1) CView.text("your [cock]");
        else CView.text("as many of your cocks as you can manage to grab without falling off");
        CView.text(".  Finally, relentlessly, inexorably, you cum, spraying your semen into the air to spatter back onto the egg, mixing with the girlish juices from your netherlips to soak into the egg's surface, leaving it slick with your mixed sexual fluids.");
    ...
    if (EmberFlags.EMBER_EGG_FLUID_COUNT < 5) {
...
function hatchZeMuzzles(player: Character): NextScreenChoices {
    ...
    if (EmberFlags.EMBER_ROUNDFACE === 0) {
        ...
        if (EmberFlags.EMBER_GENDER === 1) {
            ...
            CView.text(" the monster's crotch; there, a deceptively small slit in the flesh suddenly disgorges a 16-inch penis unlike anything you've ever seen before, bearing a rounded, elongated head and a series of ridges that give it an almost segmented look.  A pair of apple-sized balls drop into place under it.  He is most definitely male");
            if (EmberFlags.EMBER_MILK > 0) CView.text(", drooling nipples notwithstanding");
            CView.text(".");
        }
        // Female Anthro (Z)
        else if (EmberFlags.EMBER_GENDER === 2) {
            ...
            CView.text(" the monster's crotch; there, you see that the fine scales actually separate to reveal a slick-looking pussy.  She's clearly a female, with no noteworthy 'additions' that you can see.");
        }
        // Herm Anthro (Z)
        else {
            CView.text("\n\nIt's huge, standing 7 feet tall at the very least.  Its build is lean and slender, with powerful arms and legs that end in reptilian claws, complete with splay-toed feet capped by menacing talons.");
            ...
            CView.text("the monster's crotch; there, you see the scales part in two places.  The lower opening is unmistakably a pussy; but from the slit just above it suddenly distends a 16-inch penis unlike anything you've ever seen before, bearing a rounded, elongated head and a series of ridges that give it an almost segmented look.  Beneath it, a pair of apple-sized balls fall into place heavily, leaving you no doubt that she is a hermaphrodite.");
        }
    }
    // Boring version
    else {
        ...
        if (EmberFlags.EMBER_GENDER === 1) {
            ...
            if (EmberFlags.EMBER_MILK > 0 || EmberFlags.EMBER_HAIR > 0) CView.text(", complete with ");
            if (EmberFlags.EMBER_MILK > 0) {
                CView.text("huge breasts, easily F-cups");
                if (EmberFlags.EMBER_HAIR > 0) CView.text(" and ");
            }
            if (EmberFlags.EMBER_HAIR > 0) CView.text("long, feminine locks of hair");
            CView.text(", but his maleness is undeniable.  Especially when you spot ");
            if (EmberFlags.EMBER_INTERNAL_DICK > 0) CView.text("the slit in his pelvis that disgorges a foot and a half-long inhuman penis");
            else CView.text("a foot and a half long human penis that sways between his legs");
            CView.text(", completed by apple-size nuts held inside a fleshy sack.");
        }
        ...
        // Herm Monstergirl (Z)
            ...
            if (EmberFlags.EMBER_INTERNAL_DICK === 0) CView.text("huge, human prick");
            else CView.text("huge inhuman cock hanging from some kind of internal sheath");
            CView.text(" and apple-size nuts held inside a fleshy sack slung under - the ensemble hangs nearly level with her knees.  She... or he? is obviously a hermaphrodite.");
        }
    }
    // Aftermath (Z)
    return { next: meetEmberAftermath };
}

// Aftermath (Z)
function meetEmberAftermath(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You can only stand there and stare at this strange creature, supposedly a dragon, for what feels like hours.");
    ...
    EmberFlags.EMBER_HATCHED = 1;
    player.inventory.keyItems.remove("Dragon Egg");
    return { next: returnToCampUseOneHour };
}

// Appearance (shows Ember's appearance, always available)
function embersAppearance(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("ember-examine-appearance");
    // Anthro Ember's Appearance (Z)
    if (EmberFlags.EMBER_ROUNDFACE === 0) {
        CView.text("Ember is a 7' 3\" tall humanoid dragon, with supple, long limbs and a build that is toned and athletic, with powerful underlying muscles.  " + emberMF("He", "She") + " looks strong and imposing, but ");
        if (EmberFlags.EMBER_GENDER === 1) CView.text(" not overly muscled.");
        else CView.text(" feminine.");
        ...
        if (EmberFlags.EMBER_GENDER === 1) {
            ...
            if (EmberFlags.EMBER_MILK > 0) CView.text(" dribbling breasts,");
            CView.text(" snout, wings, and horns, gives him the silhouette of a prince from your village's stories: dashing and handsome.");
        }
        // (Female/Herm)
        else {
        ...
        if (EmberFlags.EMBER_HAIR === 1) {
            if (EmberFlags.EMBER_GENDER === 1) CView.text("  Short ");
            else CView.text("  Shoulder-length ");
            CView.text("steel-gray hair sprouts from " + emberMF("his", "her") + " head.  You'd think that a dragon with hair would look weird, but it actually compliments Ember's looks very well.");
        }
        // (if Ember has a level 2 mane)
        else if (EmberFlags.EMBER_HAIR === 2) {
            CView.text("  Tracing " + emberMF("his", "her") + " spine, a mane of hair grows; starting at the base of " + emberMF("his", "her") + " neck and continuing down " + emberMF("his", "her") + " tail, ending on the tip of " + emberMF("his", "her") + " tail in a small tuft.  It is the same color as the hair on " + emberMF("his", "her") + " head, but shorter and denser; it grows in a thick vertical strip, maybe two inches wide.  It reminds you vaguely of a horse's mane.");
        }
        ...
        CView.text("  The ");
        if (EmberFlags.EMBER_ROUNDFACE === 0) CView.text("little ");
        CView.text("exposed flesh of Ember's body is a light shade of pink; but flushes when " + emberMF("he", "she") + "'s aroused, drawing your eyes towards " + emberMF("his", "her") + " most sexual parts.");

        switch (emberWomb.event) {
            case 2:
                CView.text("  At the moment there's a slight pudgyness to her golden belly.");
                break;
            case 3:
                CView.text("  Her golden belly is a little larger and firmer than usual.");
                break;
            case 4:
            case 5:
                CView.text("  Her golden belly has grown quite a bit.  Ember often rests her hand on it, especially when she sees you looking at it.");
                break;
            case 6:
                CView.text("  Her large pregnant belly has forced Ember to change her posture.  It looks like she has a big golden bullseye painted on her stomach.  Ember looks tired but happy.");
                break;
            case 7:
                CView.text("  Her swollen belly is as large as that of any pregnant woman you can remember from Ingnam and you " + (EmberFlags.EMBER_OVIPOSITION > 0 ? "can easily make out the egg's outline under her golden skin" : "occasionally see movement as the baby shifts in Ember's womb") + ".");
                break;
            case 8:
            case 9:
                CView.text("  Ember's golden belly is stretched taut by the large " + (EmberFlags.EMBER_OVIPOSITION > 0 ? "egg" : "baby") + " in her womb.  You're sure she'll give birth very soon, there just isn't room for the " + (EmberFlags.EMBER_OVIPOSITION > 0 ? "egg" : "baby") + " to grow any larger.");
                break;
            default:
        }
        ...
        if (EmberFlags.EMBER_MILK > 0 || EmberFlags.EMBER_GENDER >= 2) CView.text("F-cup soft, pillowy breasts");
        else CView.text("flat manly pecs");
        CView.text(" covered in fine scales excepting " + emberMF("his", "her") + " areolas; 0.5 inch nipples protrude from the center of the ");
        if (EmberFlags.EMBER_MILK > 0 || EmberFlags.EMBER_GENDER >= 2) CView.text("generous mounds");
        else CView.text("masculine pectorals");
        CView.text(".");
    ...
    // Dragon-girl Appearance (By Radar) (Z)
        ...
        CView.text("Ember is a 7' 3</i>\" tall ");
        if (EmberFlags.EMBER_GENDER === 1) CView.text("male");
        else if (EmberFlags.EMBER_GENDER === 2) CView.text("female");
        else CView.text("hermaphrodite");
        CView.text(" dragon-" + emberMF("boy", "girl") + ", with slender limbs and a thin frame; " + emberMF("he", "she") + " refuses to wear any kind of clothing and exposes " + emberMF("him", "her") + "self naturally to the world.  " + emberMF("He", "She") + " sports a rather human looking face, but with several key differences.  Where a normal set of human eyes would be located, instead a pair of orange, reptilian eyes stares back at you, filled with immeasurable pride and ferocity.");
        ...
        // Short:
        if (EmberFlags.EMBER_HAIR < 1) CView.text("is quite short, giving " + emberMF("him", "her") + " that definitive " + emberMF("masculine", "tomboy") + " look.");
        else CView.text("drops down to " + emberMF("his", "her") + " shoulders, giving " + emberMF("him", "her") + " the look of the " + emberMF("handsome", "beautiful") + " warriors from your village legends.");

        CView.text("\n\n" + emberMF("His", "Her") + " chest is also human in appearance and houses a pair of ");
        if (EmberFlags.EMBER_MILK > 0 || EmberFlags.EMBER_GENDER >= 2) CView.text("F-cup breasts that support 0.5 inch nipples and hang heavily; you idly wonder if " + emberMF("he", "she") + "'ll develop lower back problems as she spends more time in Mareth");
        else CView.text("manly pectorals with 0.5 inch nipples");
        CView.text(".  Just below " + emberMF("his", "her") + " collarbone, in the middle of " + emberMF("his", "her") + " chest, you see what looks like a small, golden, heart-shaped scale; adorning the chest like a birthmark of some sort.");
        ...
        if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) {
            if (EmberFlags.EMBER_INTERNAL_DICK === 0) CView.text("  " + emberMF("He", "She") + " sports a flaccid penis and a pair of apple-sized balls that sit dangerously exposed to the elements, let alone to naked blades or a heavy blow.  Yet " + emberMF("he", "she") + " doesn't seem concerned about that in the least, almost daring anyone to focus on them.  While " + emberMF("he", "she") + " isn't aroused right now, Ember's penis can reach a length of approximately 16 inches, and it looks to be about 2 inches thick.");

            else CView.text("  " + emberMF("He", "She") + " sports what looks like a protective slit of some sort, protecting " + emberMF("his", "her") + " dick from the elements as well as stray blows.  You can't see it right now; but you remember it to be about 16 inches long and 2 inches thick.");
        }
        // Ember has a pussy:
        if (EmberFlags.EMBER_GENDER >= 2) CView.text("  The inviting lips of a human-looking pussy purse between her legs; some moisture seems to have gathered on her labia, giving it a slick look that just begs for attention.");
    ...
    return { next: emberCampMenu };
}

// Talk
function talkToEmber(player: Character): NextScreenChoices {
    ...
    if (player.body.wombs.find(Womb.Pregnant)) { // Extra check might protect against inappropriate Ember complaints
        if (EmberFlags.EMBER_OVI_BITCHED_YET === 0 && player.body.wombs.find(Womb.PregnantWithType(PregnancyType.OVIELIXIR_EGGS))) {
            emberBitchesAboutPCBeingFullOfEggs();
            return { next: returnToCampUseOneHour };
        }
        if (player.body.wombs.find(Womb.PregnantWithType(PregnancyType.EMBER))) {
            const emberPregWomb = player.body.wombs.filter(Womb.PregnantWithType(PregnancyType.EMBER)).sort(Womb.LargestPregnancy)[0];
            if (emberPregWomb.pregnancy.incubation < 200 && EmberFlags.EMBER_BITCHES_ABOUT_PREGNANT_PC === 0) {
                manEmberBitchesAboutPCPregnancy();
                return { next: returnToCampUseOneHour };
            }
            if (emberPregWomb.pregnancy.incubation < 300 && EmberFlags.EMBER_TALKS_TO_PC_ABOUT_PC_MOTHERING_DRAGONS === 0) {
                emberTalksToPCAboutPCDragoNPregnancy(player);
                return { next: returnToCampUseOneHour };
            }
        }
    }
    if (EmberFlags.EMBER_PREGNANT_TALK === 0 && emberWomb.event > 1) {
        emberIsPregnantFirstTimeTalkScene(player);
        return { next: returnToCampUseOneHour };
    }
    CView.clear();
    CView.text("What will you talk about?");
    // Else you can pick one of three topics:
    // Talk about Dragons
    // Talk about Exploring
    // Talk about Yourself
    return { choices: [["Dragons", talkToEmberAboutDragonzzz], ["Exploring", discussExplorationWithEmber], ["Yourself", talkToEmberAboutYourself], ["", undefined], ["Back", emberCampMenu]] };
}

// Talk about Dragons (Z)
function talkToEmberAboutDragonzzz(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You ask Ember to tell you more about " + emberMF("his", "her") + " species.");
    const choice: number = randInt(5);
    if (choice === 0) {
        ...
        if (EmberFlags.EMBER_ROUNDFACE === 1) CView.text(", a blush creeping onto " + emberMF("his", "her") + " cheeks");
        CView.text(".");
...
// Exploration (Z)
function discussExplorationWithEmber(player: Character): NextScreenChoices {
    CView.clear();
    const choice: number = randInt(4);
    let subChoice: number = 0;
    CView.text("You ask Ember for news of anything interesting " + emberMF("he", "she") + " has seen during " + emberMF("his", "her") + " aerial explorations of Mareth.");
    ...
    if (choice === 0) {
        ...
        if (subChoice === 0) CView.text("imps. Only a few imps, and they seemed very nervous. I'm guessing that whatever naturally lives there is something that they really don't want to get involved with.");
        else if (subChoice === 1) CView.text("two goblins in the foothills, arguing with each other. One was saying that the other shouldn't go up into the hills, as apparently the minotaurs that live there are too much for an untrained girl like her to take without being split open. The second goblin just laughed at her, called her a 'wimp' and told her that she's going to get herself 'some juicy bull-cock and tasty mino-spooge'. Ugh, disgusting little creatures.");
        else if (subChoice === 2) CView.text("a pair of muscle-bound bull-men beating on each other with their bare fists. They spent over an hour smashing each other into a bloody pulp, and then the winner promptly started fucking the loser up the ass.  I had seen more than enough by that point, so I left.");
        else if (subChoice === 3) CView.text("this... creature... that looked kind of like a human woman, but with a big dick where her clit should be.  She was walking around stark naked, 'cept for a bunch of piercings, and leading this bull-man along like a pet by a chain attached to a ring anchored into his cockhead.");
        else if (subChoice === 4) CView.text("a couple of goblins sharpening scissors on some rocks outside of a cave with a door on it. Weird. Wonder what they could be doing in there?");
        else if (!player.effects.has(StatusEffectType.WormsOff) && randInt(2) === 0) CView.text("a horrible swarm of slimy white worms, clumped together into a mockery of a human form and squelching along.  It managed to latch onto this two-headed dog-creature and... ugh!  The worms started forcing their way into both of its cocks!  I've never seen anything so disgusting!");
        else if (subChoice === 5) CView.text("this two-headed dog-morph loping around; it spotted an imp, dropped to all fours, then gave chase. Managed to catch the ugly little demon, whereupon it ass-raped it, then ate it.");
    }
    else if (choice === 1) {
        subChoice = randInt(5);
        ...
        if (subChoice === 0) CView.text("a whole tribe of imps, just lounging around in a glade, jerking themselves off or squabbling over food.  Nasty little things, but easily dispatched.");
        else if (subChoice === 1) CView.text("a  goblin with a huge pregnant belly, laughing to herself and swilling down that ale they brew, slopping it all over herself.  Little hedonists.");
        else if (subChoice === 2) CView.text("this strange bee-woman creature... she made this, this music that started messing with my head.  I spat a tongue of flames at her and she flew away in fright, luckily.");
        // (If player has impregnated Tamani)
        else if (subChoice === 3 && EmberFlags.TAMANI_NUMBER_OF_DAUGHTERS > 0) {
            CView.text("that green-skinned baby-making whore, Tamani.  She was letting some of her daughters suckle from her and grinning ear to ear as she named the 'prize catch' she got to father them, exhorting them to hunt him down.");
            if (EmberFlags.EMBER_GENDER >= 2) CView.text("  You should have more pride than to let some brainless cunt like that have her way with you!");
        }
        // (If player has not yet impregnated Tamani)
        else if (subChoice === 3 && !tamaniWomb.isPregnant()) CView.text("one goblin being teased by a bunch of pregnant goblins for not being pregnant yet.  She just spat back that she wanted a 'better catch' to be her baby-maker than a mere imp and wandered off.");
        // (If Jojo isn't in the camp & not corrupt)
        else if (randInt(2) === 0 && PlayerFlags.monk <= 1 && !player.effects.has(StatusEffectType.PureCampJojo)) CView.text("this mouse-morph monk, sitting in a glade and meditating. A goblin tried to proposition him; he just gave her a lecture and sent her running away in tears.  When an imp tried to attack him, he crushed its skull with a staff he had.  Not bad moves for such a weedy little thing...");
        else CView.text("one glade I touched down in to catch myself a nice brace of plump coneys, when all of a sudden this... this thing made out of flailing vines and fruit attacks me.  It went up in a puff of smoke once I torched it, of course.");
    }
    else if (choice === 2) {
        subChoice = randInt(2);
        ...
        if (subChoice === 0) {
            CView.text("a pair of shark-women - well, one was a woman, the other had breasts but also had a cock");
            if (EmberFlags.EMBER_GENDER === 3) CView.text(" like me");
            CView.text(". They were on the beach, fucking each other's brains out.");
        }
        else {
            // (If Whitney's Farm is not yet in the Places menu)
            if (randInt(3) === 0 && player.effects.get(StatusEffectType.MetWhitney).value1 <= 1) CView.text(" a big farm on the shoreline.  I saw some sort of cow-woman tending to fields of peppers, and a dog-woman herding cattle. They looked pretty fat and tasty, too... What?  I meant the cattle, stupid!  And I wouldn't have taken them - it's no fun snatching up livestock.  I prefer to chase down a deer or a wild auroch or something like that.");
            // (if Fetish Cult is encounterable)
            else if (randInt(3) === 0 && player.effects.has(StatusEffectType.FetishOn)) CView.text("a pair of humans, arguing about sex.  They both wanted it, but the guy said he wanted to do the 'naughty schoolboy and female teacher' routine, while the girl wanted to do the 'noblewoman and love-servant' routine.  Weird; their clothes kept changing back and forth as they argued.");
            // (If Beautiful Sword has not been acquired)
            else if (randInt(3) === 0 && !player.effects.has(StatusEffectType.TookBlessedSword) && !player.effects.has(StatusEffectType.BSwordBroken)) CView.text("a sword jammed into a tree.  Weird; what kind of idiot would stick a weapon there like that?  And what kind of weakling wouldn't be able to take it out?");
            else if (randInt(3) === 0 && !player.effects.has(StatusEffectType.CampRathazul)) CView.text("a smelly rat-man moping around while some weird equipment bubbled and boiled.  I think maybe he was an alchemist.");
            else CView.text("a great blob of green goo, sliding along and minding its own business.  I could swear it looked up at me once, and grew a penis... that can't be right, though.");
        }
    }
    else if (choice === 3) {
        subChoice = randInt(4);
        ...
        if (subChoice === 0) CView.text("a woman with four big breasts, squeezing milk out of her tits and into the sand.  I didn't know breasts could hold that much milk!");
        else if (subChoice === 1) CView.text("a whole tribe of demons, lounging around an oasis.  Would have been too much bother to kick the crap out of them, so I left them alone - well, alright, I did buzz them to make them scatter like scared sheep for fun.");
        // (if player hasn't solved Marcus & Lucia's argument)
        else if (randInt(2) === 0 && !player.effects.has(StatusEffectType.WandererDemon) && !player.effects.has(StatusEffectType.WandererHuman)) CView.text("a human with balls so big he had to carry them in a wheelbarrow, trundling through the wasteland with a succubus.  They were arguing about whether or not he should become an incubus.");
        else {
            CView.text("this strange creature, like a woman with a snake's tail for legs, slithering through the sand.  ");
            if (player.body.legs.isNaga()) CView.text("She looked a lot like you.");
            else CView.text("I've never seen anything like her before.");
        }
    }
    CView.text("</i>\"");
    return { next: returnToCampUseOneHour };
...
function talkToEmberAboutYourself(player: Character): NextScreenChoices {
    CView.clear();
    let points: number = 0;
    CView.text("You ask Ember what " + emberMF("he", "she") + " thinks about you.");
    // (Low Affection)
    ...
    // (If PC Str < 50)
    if (player.stats.str < 50) CView.text("  \"<i>You could use more training; you look pretty weak...</i>\" Ember says, critically.");
    // (If PC Str < 90)
    else if (player.stats.str < 90) {
        CView.text("  \"<i>You're well on your way to being as strong as a dragon, but there's still room for improvement.</i>\" Ember says, pleased.");
        // (+1 point)
        points++;
    }
    // (If PC Str >= 90)
    else {
        CView.text("  \"<i>With muscles like these, you could easily lift anything you wanted... even me...</i>\" Ember trails off dreamily, almost purring.  \"<i>N-Not that I'd want you to carry me in your arms or anything...</i>\"");
        // (+2 points)
        points += 2;
    }
    CView.text("\n\nNext, Ember pokes at your chest and your ribs.  ");
    // (If PC Tou < 50)
    if (player.stats.tou < 50) CView.text("\"<i>I don't know how you could hope to survive out there.  You look like the wind could blow you away.</i>\"");
    // (If PC Tou < 90)
    else if (player.stats.tou < 90) {
        CView.text("\"<i>Not bad; some more training and you could be as tough as me!</i>\" Ember says, pleased.");
        // (+1 point)
        points++;
    }
    // (If PC Tou >= 90)
    else {
        CView.text("\"<i>Your body's as tough as rock, you should show off more frequently...  N-Not that I'd want you to go naked!  You should always wear something to protect yourself,</i>\" " + emberMF("he", "she") + " quickly adds.");
        // (+1 Affection)
        emberAffection(1);
        // (+2 points)
        points += 2;
    ...
    // (If PC Int < 50)
    if (player.stats.int < 50) CView.text("\"<i>You're still very naive... anyone or anything could trick you at any time...</i>\"  Ember frowns.");
    // (If PC Int < 90)
    else if (player.stats.int < 90) {
        CView.text("\"<i>I see wisdom in your eyes, but you could always use more lessons.</i>\"");
        // (+1 point)
        points++;
    }
    // (If PC Int >= 90)
    else {
        CView.text("Ember's eyes widen.  \"<i>I see great wisdom in your eyes...</i>\"  Ember's face grows more absent by the minute as " + emberMF("he", "she") + " looks at you in deep thought, until the trance breaks.  " + emberMF("He", "She") + " blows at your face, brushing a claw on your forehead.  \"<i>Y-You had something in your face... I wasn't staring into your steely eyes.</i>\"");
        // (+2 points)
        points += 2;
    }
    ...
    // (If PC Spd < 50)
    if (player.stats.spe < 50) CView.text("You try to block the pebble, but you're not quick enough.  It hits your belly, not hard enough to hurt.  \"<i>You need to work on your reactions; anything could surprise you out there and you'd be helpless.</i>\"");
    // (If PC Spd < 90)
    else if (player.stats.spe < 90) {
        CView.text("You successfully deflect the pebble.  \"<i>Not bad!  Next time try to catch it,</i>\"  Ember says, pleased.");
        // (+1 point)
        points++;
    }
    // (If PC Spd >= 90)
    else {
        CView.text("You easily catch the pebble and throw it back at Ember, surprising " + emberMF("him", "her") + ".  \"<i>With reflexes like these, you could even...</i>\" Ember mumbles.  \"<i>D-Don't even think about it!</i>\" " + emberMF("he", "she") + " snaps.");
        // (+2 points
        points += 2;
    }
    CView.text("\n\nSatisfied, Ember turns to take another look at you.");
    // (If PC has high dragon or lizard score)
    if (nagaRaceScore(player) >= 3 || lizardRaceScore(player) >= 3 || dragonRaceScore(player) >= 3) {
        CView.text("  Ember's eyes linger on your form.  After a moment of awkward silence, you clear your throat.  " + emberMF("he", "she") + " blinks and says hurriedly.  \"<i>Sorry... I was just admiring you-</i>\" Realizing what " + emberMF("he", "she") + " was about to say,and quickly blurts out.  \"<i>I mean the weather! Yes, nice day today isn't it?</i>\"  You're not convinced, but let it slide. Ember recomposes and clears " + emberMF("his", "her") + " throat before saying.");
        // (+1 Affection)
        points++;
    }
    CView.text("\n\n\"<i>All right, here's what I think about you.</i>\"  ");
    if (points < 2) {
        CView.text("\"<i>This is not good at all; it's a miracle you even managed to survive in this world thus far.  With the things I've seen roaming the land...</i>\" Ember trails off, concerned.  \"<i>You should stay in the camp; I'll help you train.</i>\"  You don't much like the assessment.");
    }
    // (else if points < 6)
    else if (points < 6) {
        CView.text("\"<i>You're doing fine, but make sure you don't slack off, and keep training.</i>\"  You thank Ember for sharing " + emberMF("his", "her") + " thoughts.");
    }
    // (else)
    else {
        CView.text("\"<i>You're quite a catch... if you strolled down the street you'd have dragons fawning all over you...</i>\"  Realizing what " + emberMF("he", "she") + " just said, Ember coughs furiously.  \"<i>I-I mean... lesser dragons might fawn all over you.  You don't meet my standards!</i>\"");
        emberAffection(5);
    }
    return { next: returnToCampUseOneHour };
}
...
function manEmberBitchesAboutPCPregnancy() {
    CView.clear();
    EmberFlags.EMBER_BITCHES_ABOUT_PREGNANT_PC = 1;
    // (Low Affection)
    if (emberAffection() <= 25) CView.text("The two of you talk about nothing in particular.  It's light, airy and pointless.  When you finish up, though, you realize something odd; Ember was doing " + emberMF("his", "her") + " best all throughout the conversation to avoid looking at your pregnant belly - almost as if " + emberMF("he", "she") + " were upset by it?");
    // (Moderate Affection)
    else if (emberAffection() < 75) CView.text("Ember stares coldly at your gravid midriff as icebreaker after icebreaker from you falls flat.  You ask what's wrong.  \"<i>Nothing.  Nothing is wrong,</i>\" Ember states, flatly.");
    // (High Affection)
    else CView.text("The expression Ember gives you is very cold.  \"<i>What is the meaning of this!?</i>\" " + emberMF("he", "she") + " says, motioning towards your bloated belly.  \"<i>You're supposed to help <b>me</b> breed more dragons; not slut around with trash!</i>\"  Ember blows out an indignant puff of smoke and walks away.");
}
...
// Occurs only once.
// To be implimented once preggers is set up.
function emberTalksToPCAboutPCDragoNPregnancy(player: Character) {
    CView.clear();
    EmberFlags.EMBER_TALKS_TO_PC_ABOUT_PC_MOTHERING_DRAGONS = 1;
    CView.text("You notice Ember's eyes are fixated on your swollen belly, and cautiously ask what " + emberMF("he", "she") + "'s looking at.");
...
function emberIsPregnantFirstTimeTalkScene(player: Character) {
    CView.clear();
    EmberFlags.EMBER_PREGNANT_TALK = 1;
    CView.text("You can't help but stare at Ember's swollen belly; it's still so hard to take in that you have actually fathered a child with a creature of legend.  Especially given that there are times when it's hard to be entirely certain that Ember genuinely likes you.");
    CView.text("\n\nEmber catches you staring");
    if (EmberFlags.EMBER_ROUNDFACE === 1) CView.text(" and blushes");
    CView.text(", one of her hands rubbing her belly lovingly and tenderly.  \"<i>W-what is it?  Why are you looking at me like that?  Something wrong?</i>\"");

    CView.text("\n\nYou merely give her a faint smile and tell her that she's beautiful when pregnant.");
    if (player.stats.cor >= 66) CView.text("  You almost choke on those cheesy words; still, she'll never put out for you if you don't butter her up.");
    ...
    if (player.stats.cor >= 66) CView.text("  Is she seriously buying this nonsense?  Your smile would be miles wide if you let your lip curl even a bit, so you keep a straight face with due dilligence.");
    ...
    if (player.stats.cor < 30) CView.text("  Given her attitude and nature, you know how lucky you are to have someone like her want to be your mate.");
...
function emberBitchesAboutPCBeingFullOfEggs() {
    CView.clear();
    CView.text("As you try and think of a topic to talk about, you realize Ember is staring at your egg-swollen stomach - not with anger or disdain, but with interest.  With a smirk, you place one hand on your belly and ask if " + emberMF("he", "she") + " finds you interesting to look at like this.");
    ...
    EmberFlags.EMBER_OVI_BITCHED_YET = 1;
}
...
export function emberBitchesAtYouAboutLustiness(player: Character) {
    CView.text("\nYou strive to keep your mind focused, but... your libido is screaming at you, ");
    if (player.body.cocks.length > 0) {
        CView.text("your " + numToCardinalText(player.body.cocks.length) + " stiff as iron");
        if (player.body.vaginas.length > 0) CView.text(" and ");
    }
    if (player.body.vaginas.length > 0) CView.text("your " + describeVagina(player, player.body.vaginas.get(0)) + " slick and wet with moisture, ready to fuck");
    CView.text(".  You want sex so bad it almost hurts...");
    ...
    CView.text("\n");
    EmberFlags.EMBER_LUST_BITCHING_COUNTER = 0;
}
...
export function minotaurJizzFreakout() {
    ...
    if (followerKiha()) CView.text("\n\nKiha saunters up and smirks.  \"<i>I thought I had a temper.</i>\"");
    EmberFlags.EMBER_CURRENTLY_FREAKING_ABOUT_MINOCUM = 1;
    CView.text("\n");
}
...
export function emberGetOverFreakingOutAboutMinoJizz() {
    EmberFlags.EMBER_CURRENTLY_FREAKING_ABOUT_MINOCUM = 0;
    ...
    CView.text("\n\nYou point out that this means " + emberMF("he", "she") + " no longer has to hunt down minotaurs, for your sake. Which furthermore means, you note, that she can stop leaving cow skulls and hooves and other rubbish all over the ");
    ...
    // (High Affection)
        ...
        // (Male/HermEmber:
        if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text(", especially for sucking cock,");
        CView.text(" you should come to me instead; I'll help you.</i>\"  Then as if realising what " + emberMF("he", "she") + "'s just implied, Ember looks away; and hurriedly adds, \"<i>But not the way you're thinking...</i>\"");
...
function bloodForTheBloodGod(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You ask Ember if " + emberMF("he", "she") + " would be willing to give you a taste of " + emberMF("his", "her") + " blood, desirous of the power that lies within it.");
    // (If Ember hasn't recovered from the last time " + emberMF("he","she") + " shared her blood)
    if (EmberFlags.DRANK_EMBER_BLOOD_TODAY === 1 && !debug) {
    ...
    // (Medium Affection)
        ...
        if (player.stats.cor < 50) CView.text(", expressing your gratitude; gently, you kiss " + emberMF("his", "her") + " fingers, then the cut, letting its cool, iron-tinted tang roll across your lips and tongue.  You carefully lick, trying to avoid causing pain as you drink that which " + emberMF("he", "she") + " has so generously offered you.");
        else CView.text(", quickly drawing it to your mouth and forming a seal around the cut with your lips, anxious to let not a drop escape.");
    ...
    EmberFlags.DRANK_EMBER_BLOOD_TODAY = 1;
    // Medium/high get stat boosts!
    const stat: number = randInt(4);
    ...
    // [Continue][Stop]
    return { choices: [["Continue", drinkDeeplyOfDagronBlud], ["Stop", noMoDagronBlud]] };
}

// [=Stop=]
function noMoDagronBlud(player: Character): NextScreenChoices {
    CView.clear();
    if (emberAffection() < 75) {
        CView.text("You decide to stop for now and pull away.  Ember licks " + emberMF("his", "her") + " own wound " + emberMF("him", "her") + "self and you thank " + emberMF("him", "her") + " for sharing.");
        CView.text("\n\n\"<i>D-Don't mention it...</i>\"");
    }
    else {
        CView.text("You decide to stop for now and pull away.  Ember licks " + emberMF("his", "her") + " lips, draws " + emberMF("his", "her") + " tongue back into " + emberMF("his", "her") + " mouth and purrs with delight.  When " + emberMF("he", "she") + " realizes what " + emberMF("he", "she") + "'s doing, though, " + emberMF("he", "she") + " sobers.  \"<i>D-Don't get any strange ideas...</i>\"");
...
function drinkDeeplyOfDagronBlud(player: Character): NextScreenChoices {
    CView.clear();
    if (emberAffection() < 75) {
        CView.text("You decide to continue drinking Ember's blood; intent on acquiring all the power it can bring out from within you.");
...
// TF messages (Z)
function emberTFs(player: Character) {
    let changes: number = 0;
    const changeLimit: number = 2;
    // Gain Dragon Dick
    if (changes < changeLimit && player.body.cocks.filter(Cock.FilterType(CockType.DRAGON)).length < player.body.cocks.length && randInt(3) === 0) {
        const choices = player.body.cocks.filter((cock) => cock.type !== CockType.DRAGON);
        // Randomly choose one of those locations
        const selectedCock = randomChoice(choices);
        CView.text("\n\nYour " + describeCock(player, selectedCock) + " tingles as pins and needles sweep across it.  You pull open your [armor] to watch as it changes; the tip elongates and tapers, like a spear; a series of ridges form along the shaft, giving it an almost segmented look, and a prominent knot swells at its base.  You can't resist stroking it, until it begins dripping pre; ");
        if (player.stats.sens >= 50) CView.text("however, it's not until you press on your new, sensitive knot that you manage to blow your load and enjoy the last few spasms of pleasure as it finally finishes changing.");
        else CView.text("but you sternly rein in your hands and tuck them into your armpits as the arousing changes run their course.");
        CView.text("  <b>You now have a dragon penis.</b>");
        ...
        changes++;
        // Apply the TF
        selectedCock.type = CockType.DRAGON;
        selectedCock.knotMultiplier = 1.3;
    }
    // Gain Dragon Head
    if (changes < changeLimit && randInt(3) === 0 && player.body.face.type !== FaceType.DRAGON && EmberFlags.EMBER_ROUNDFACE === 0) {
        ...
        player.body.face.type = FaceType.DRAGON;
        changes++;
    }
    // -Existing horns become draconic, max of 4, max length of 1'
    if (player.body.horns.type !== HornType.DRACONIC_X4_12_INCH_LONG && changes < changeLimit && randInt(5) === 0) {
        // No dragon horns yet.
        if (player.body.horns.type !== HornType.DRACONIC_X2) {
            // Already have horns
            if (player.body.horns.count > 0) {
                // High quantity demon horns
                if (player.body.horns.type === HornType.DEMON && player.body.horns.count > 4) {
                    ...
                    player.body.horns.count = 12;
                    player.body.horns.type = HornType.DRACONIC_X4_12_INCH_LONG;
                }
                else {
                    ...
                    player.body.horns.type = HornType.DRACONIC_X2;
                    if (player.body.horns.count > 13) {
            ...
            if (player.body.horns.type === HornType.DRACONIC_X2) {
                if (player.body.horns.count < 12) {
                    if (randInt(2) === 0) {
                        ...
                        player.body.horns.count += 2 + randInt(4);
                    }
                    if (player.body.horns.count >= 12) CView.text("  <b>Your horns settle down quickly, as if they're reached their full size.</b>");
                    changes++;
                }
                // maxxed out, new row
                else {
    ...
    // Gain Dragon Ears
    if (changes < changeLimit && randInt(3) === 0 && player.body.ears.type !== EarType.DRAGON) {
        player.body.ears.type = EarType.DRAGON;
    ...
    // Gain Dragon Tongue
    if (changes < changeLimit && randInt(3) === 0 && player.body.tongue.type !== TongueType.DRACONIC) {
        ...
        player.body.tongue.type = TongueType.DRACONIC;
        changes++;
    ...
    // Gain Dragon Scales
    if (player.body.skin.type !== SkinType.SCALES && changes < changeLimit && randInt(3) === 0) {
        ...
        player.body.skin.type = SkinType.SCALES;
        player.body.skin.adj = "";
        player.body.skin.desc = "scales";
        // def bonus of scales
    }
    // Gain Dragon Legs
    if (player.body.legs.type !== LegType.DRAGON && changes < changeLimit && randInt(3) === 0) {
        // (if drider)
        if (player.body.legs.type === LegType.DRIDER_LOWER_BODY)
        ...
        player.body.legs.type = LegType.DRAGON;
        changes++;
    }
    // Gain Dragon Tail
    if (player.body.tails.find((tail) => tail.type !== TailType.DRACONIC) && changes < changeLimit && randInt(3) === 0) {
        // (If no tail)
        if (player.body.tails.length <= 0) CView.text("\n\nA sudden dull, throbbing pain in your " + describeButt(player) + " forces your hands to it; you can feel an ominous lump over your tail bone, swelling bigger and bigger with every heartbeat.  All of a sudden, it seems to explode, jutting out and around until it hovers near your ankles, the skin under your flesh hard and scaly.  <b>You now have a dragon tail flicking at your back, flexible as a whip.</b>");
        // (If tail)
        ...
        player.body.tails.clear();
        player.body.tails.add(new Tail(TailType.DRACONIC));
        changes++;
    }
    /*
     ...
     */
    // Grow Dragon Wings
    if (player.body.wings.type !== WingType.DRACONIC_LARGE && changes < changeLimit && randInt(3) === 0) {
        if (player.body.wings.type === WingType.NONE) {
            ...
            player.body.wings.type = WingType.DRACONIC_SMALL;
            player.body.wings.desc = "small, draconic";
        }
        // (If Small Dragon Wings Present)
        else if (player.body.wings.type === WingType.DRACONIC_SMALL) {
            ...
            player.body.wings.type = WingType.DRACONIC_LARGE;
            player.body.wings.desc = "large, draconic";
        }
        else if (player.body.wings.type === WingType.SHARK_FIN) {
    ...
    if (dragonRaceScore(player) >= 4 && changes < changeLimit && !player.perks.has(PerkType.Dragonfire)) {
    ...
    if (dragonRaceScore(player) >= 4 && randInt(3) === 0 && player.gender > 0) {
        ...
        CView.text(", making you wish Ember hadn't run you off.  All you can think about now is fucking " + emberMF("his", "her") + "; ");
        if (player.body.cocks.length > 0 && EmberFlags.EMBER_GENDER >= 2) {
            CView.text("filling her womb with your seed and fertilizing her eggs");
            if (player.body.vaginas.length > 0 && EmberFlags.EMBER_GENDER === 3) CView.text(" even while ");
        }
        if (player.body.vaginas.length > 0 && (EmberFlags.EMBER_GENDER === 3 || EmberFlags.EMBER_GENDER === 1)) CView.text("taking that hard, spurting cock inside your own " + describeVagina(player, player.body.vaginas.get(0)));
        CView.text("... too late, you realize that <b>Ember's blood has sent your draconic body into ");
        if (player.body.cocks.length > 0 && EmberFlags.EMBER_GENDER >= 2) {
            CView.text("rut");

            displayGoIntoRut(player);
            changes++;
        }
        else {
            CView.text("heat");

            displayGoIntoHeat(player);
            changes++;
        }
        CView.text("</b>.");
    }
...
function emberIsAnEggFactory(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You ask Ember if she would be willing to lay an egg for you.");
    // (Low Affection)
    if (emberAffection() <= 25) {
        ...
        if (player.stats.cor >= 40) CView.text("or sell it for quick cash ");
        CView.text("anyway, but you don't tell her that.");
        ...
        if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) {
            CView.text(" and her ");
            if (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK > 0) CView.text("dragon-dick jutting from its slit,");
            else CView.text("veiny, bulbous penis fully");
            CView.text(" erect");
        }
        CView.text(".  One of her hands gently teases her clit, as another traces her outer labia; she bites her lips, trying to stifle her moans of pleasure, but it's useless... every touch brings a sigh.");
        ...
        // [(if Ember has a dick)
        if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text(" and her throbbing cock is soft enough to hide");
        CView.text(" before she presents the egg to you.");
        ...
        return player.inventory.items.createAdd(player, ItemType.Consumable, ConsumableName.DragonEgg, returnToCampUseOneHour);
    }
    // (Medium Affection)
    else if (emberAffection() < 75) {
        CView.text("\n\nEmber gasps");
        if (EmberFlags.EMBER_ROUNDFACE === 1) CView.text(" and her cheeks grow red with embarrassment");
        CView.text(".  \"<i>T-This...</i>\"  She can't quite bring herself to finish the sentence and settles for just looking at the ground.");
        ...
        // [Watch][Fob Off]
        return { choices: [["Watch", watchMediumAffectionEmberEggLay], ["Don't Watch", dontWatchEmberLayEgg]] };
    }
    // (High Affection)
    else {
        ...
        if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text(", and her cock points you accusingly");
        CView.text(".");
        ...
        if (player.body.tongue.type > TongueType.HUMAN) CView.text(" and out... and out...");
        CView.text(" and then lean forward to give her a great, wet, sloppy lick, straight up the center of her pussy");
        // (E.Herm:
        if (EmberFlags.EMBER_GENDER === 3) CView.text(" not stopping until you have slurped your way to the very tip of her cock");
        CView.text(", savoring the unmistakable taste of her intimate juices.");
        ...
        if (player.body.tongue.type > TongueType.HUMAN) CView.text("which is quite far indeed, ");
        CView.text("caressing and stroking and playing all the tricks you can possibly think of to orally pleasure your draconic lover.  From the amount of juices beginning to seep onto your lapping tongue");
        if (EmberFlags.EMBER_GENDER === 3) CView.text(" and dribbling down her painfully stiff dick");
        CView.text(", you think you're doing rather well.");
        ...
        if (player.body.tongue.type > TongueType.HUMAN) CView.text("  Right at this time, you feel something round and smooth on the tip of your tongue, gently spreading Ember's walls.  Realizing that this can only be her egg, you start trying to worm your long, sinuous tongue between it and her innermost walls, hoping to coax it out of her.");
...
// [Fob Off]
function dontWatchEmberLayEgg(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You take her hand and tell her that you wouldn't dream of intruding on her privacy, but ask her to think of you if she needs the inspiration.  She looks away shyly, and the barest hint of a smile breaks on her face.  Seems like she's already following your instructions.");
    ...
    if (EmberFlags.EMBER_GENDER === 3) CView.text(".. it even drips with some kind of off-white fluid.");
    CView.text("  \"<i>H-here's your egg.  Use it while it's fresh, okay?</i>\"  Her eyes glaze over a bit at the suggestion, and she giggles.  ");
    // git a dragon egg, no Ember affection change
    return player.inventory.items.createAdd(player, ItemType.Consumable, ConsumableName.DragonEgg, returnToCampUseOneHour);
}

// [Watch]
function watchMediumAffectionEmberEggLay(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Ember fails to hide her arousal when you accept.  \"<i>Okay, then follow me.</i>\"  The two of you move into a secluded spot.  Once she is certain nobody is around to spy, Ember turns to face you");
    // (if Ember has a cock:
    if (EmberFlags.EMBER_GENDER === 3) CView.text(", cock poking straight out");
    if (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK > 0) CView.text(" of its hiding place");
    CView.text(".  \"<i>Okay... but you're only allowed to watch!  If you do anything weird, I swear I'll smack you!</i>\"");
    CView.text("\n\nYou promise her that you won't do anything");
    if (player.stats.lib >= 50) CView.text(" she doesn't ask you to, beaming a salacious grin");
    CView.text(".");
    ...
    if (player.gender > 0) {
        CView.text("; ");
        if (player.body.cocks.length > 0) {
            CView.text("the bulge inside your [armor]");
            if (player.body.vaginas.length > 0) CView.text(" and ");
        }
        if (player.body.vaginas.length > 0) CView.text("wetness gathering in your own pussy");
        CView.text(" more than indicate");
        if (player.body.cocks.length <= 0 || player.body.vaginas.length <= 0) CView.text("s");
        CView.text(" your desire to break your promise.");
    }
    ...
    if (EmberFlags.EMBER_GENDER === 3) CView.text(" and caressing her throbbing cock");
    CView.text(".");
    CView.text("\n\nWith a deep moan, Ember shakes and orgasms.");
    if (EmberFlags.EMBER_GENDER === 3) CView.text("  Ropes of jism spew out of her cock, arching into the air to hit the ground nearby.");
    CView.text("  The egg plugs her wet pleasure hole, preventing any liquid from escaping, until finally with a wet, squelching pop,it flies out of her pussy and into your waiting hands; releasing a flood of juices.");
    ...
    if (EmberFlags.EMBER_GENDER === 3) CView.text(" and her cock hanging out");
    CView.text(", it's hard to see her as the same angrily defensive ice queen from before.");
...
// Get Milk
function getMilkFromEmber(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("ember-drink-her-milk");
    CView.text("You think for a few moments, then find your gaze drawn to Ember's round, firm");
    if (EmberFlags.EMBER_ROUNDFACE === 0) CView.text(", scaly");
    CView.text(" breasts, " + emberMF("his", "her") + " perky nipples bare as always and enticing you.  With a repressed smile, you ask if " + emberMF("he", "she") + "'ll let you suckle " + emberMF("his", "her") + " milk.");
    // Summary:
    ...
    if (emberAffection() <= 25) {
        ...
        // noEmberkids:
        if (EmberFlags.EMBER_EGGS === 0 && emberChildren() === 0) CView.text(emberMF("he", "she") + " has no offspring to feed them with");
        // (1+ EmberEggs:
        else if (EmberFlags.EMBER_EGGS > 0) CView.text(emberMF("his", "her") + " children haven't hatched yet");
        else CView.text(emberMF("his", "her") + " children don't drink nearly enough to properly empty " + emberMF("him", "her") + "), so is " + emberMF("he", "she") + " just going to suffer with " + emberMF("his", "her") + " breasts so full and aching?");
        ...
        if (EmberFlags.EMBER_ROUNDFACE === 0) CView.text("scales");
        else CView.text("skin");
        CView.text(".  " + emberMF("He", "She") + " lets out a cute gasp of pleasure, eyes darting furiously around to look at anything but you. You beam a smile and " + emberMF("he", "she") + " turns her head away in embarrassment.  \"<i>T-They're not getting any emptier like this, y'know!?</i>\"");
        ...
        if (EmberFlags.EMBER_GENDER !== 2) {
            ...
            if (!player.body.legs.isTaur()) CView.text("belly");
            else CView.text("back");
            CView.text(" and look down at Ember's engorging shaft.  Smiling around Ember's nipple, you start to gently stroke and squeeze the partially-erect draconic dick " + emberMF("he", "she") + "'s presenting.  If " + emberMF("he", "she") + "'s enjoying this so much, well, why not make it all the more fun for " + emberMF("him", "her") + "?");
        }
        // (else)
        else {
        ...
        if (player.effects.has(StatusEffectType.DragonBreathCooldown)) {
            player.effects.remove(StatusEffectType.DragonBreathCooldown);
            CView.text("  Your throat feels soothed as the scratching and soreness die down; you feel like you could shout to the mountaintops!");
        }
        // (no new PG, PC has dragon-morph status and is opposite Ember's sex:
        if (randInt(2) === 0 && dragonRaceScore(player) >= 4 && player.gender > 0 && (player.gender !== EmberFlags.EMBER_GENDER || (player.gender === 3 && EmberFlags.EMBER_GENDER === 3))) {
            CView.text("  Though, a sudden swell of lust races through your ");
            if (player.body.cocks.length > 0) {
                CView.text(describeCock(player, player.body.cocks.get(0)));
                if (player.body.vaginas.length > 0) CView.text(" and ");
            }
            ...
            if (player.body.vaginas.length > 0 && (EmberFlags.EMBER_GENDER === 3 || EmberFlags.EMBER_GENDER === 1)) CView.text("taking that hard, spurting cock inside your own " + describeVagina(player, player.body.vaginas.get(0)));
            CView.text("... too late, you realize that <b>Ember's milk has sent your draconic body into ");
            if (player.body.cocks.length > 0 && EmberFlags.EMBER_GENDER >= 2) {
                CView.text("rut");
    ...
    // (High Affection)
        ...
        if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) {
            CView.text("the faintest hint of " + emberMF("his", "her") + " cock ");
            if (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK > 0) CView.text("emerging");
            else CView.text("hardening");
        }
        else CView.text("small beads of moisture gathering on her honeypot");
        CView.text(".  \"<i>Ok... I suppose I could use some help draining myself, but no funny moves!</i>\"");
        ...
        if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) {
            CView.text(emberMF("his", "her") + " erecting dragon-prick, gently stroking its");
            if (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK > 0) CView.text(" strangely ridged, almost scaly");
            CView.text(" surface");
        }
        else CView.text("her gently-dripping cunt, sliding in between the lips to stroke the wet interior");
        CView.text(".");
        ...
        if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text("stroke " + emberMF("his", "her") + " shaft");
        else CView.text("probe gently into her slick depths");
        CView.text(".  Ember squirms and moans, humming softly, while you suckle and tease, delighting as much in the pleasure you are giving as in the delicious milk cascading down your thirsty throat.");
        ...
        CView.text("  You tell your dragon that you're finished.  You're up and turning to leave when a looping tail around your waist stops you.");
...
function decideToSparEmbra(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You feel like you could use some practice; just to be ready for whatever you stumble upon when adventuring, and ask Ember how " + emberMF("he", "she") + "'d feel about sparring with you.");
    // (Low Affection)
    if (emberAffection() <= 25) {
    ...
    return CombatManager.beginBattle(player, new Ember());
}

export function beatEmberSpar(player: Character): NextScreenChoices {
    CView.clear();
    if (emberAffection() <= 25) {
        CView.text("Ember lies on " + emberMF("his", "her") + " back, while you stand over the defeated dragon triumphantly.  You extend a helping hand, intent on pulling " + emberMF("him", "her") + " up, but " + emberMF("he", "she") + " bats it away, flinching in shame at " + emberMF("his", "her") + " defeat.");
    ...
    // Medium affection
        ...
        CView.text("Ember lies on " + emberMF("his", "her") + " back, while you stand over the defeated dragon triumphantly.  You extend a helping hand, intent on helping " + emberMF("him", "her") + " up; " + emberMF("he", "she") + " takes it.");
    ...
    // High affection
        ...
        CView.text("Ember lies on " + emberMF("his", "her") + " back, while you stand over the defeated dragon triumphantly.  You extend a helping hand, intent on helping " + emberMF("him", "her") + " up; " + emberMF("he", "she") + " gladly accepts your help.");
...
export function loseToEmberSpar(player: Character): NextScreenChoices {
    CView.clear();
    // Low affection
    if (emberAffection() <= 25) {
        CView.text("Panting, you drop your fighting stance and sit on the floor, lifting a hand and saying you've had enough.");
    ...
    // High Affection
        ...
        CView.text("\n\nEmber sighs.  \"<i>Okay then... I'll be up for a rematch whenever you want.  Now let's get you back.</i>\" Ember picks you up bodily and walks with you back to the ");
...
function catchAnal(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("ember-fucks-your-ass");
    CView.text("You contemplate Ember's somewhat dominant streak in bed and ask if " + emberMF("he", "she") + " feels in the mood to ride your ass.");
    // (Low affection)
    if (emberAffection() <= 25) {
    ...
    if (emberAffection() < 75) {
        ...
        if (EmberFlags.EMBER_INTERNAL_DICK === 1 || EmberFlags.EMBER_ROUNDFACE === 0) CView.text(" of its hiding place");
        CView.text(", growing slowly but steadily.");
    ...
    CView.text("\n\nEmber leads you a short distance away from camp, to a small clearing next to a dried river.  " + emberMF("He", "She") + " selects a relatively lush-looking patch of grass and gestures at it.  Eagerly, you disrobe, casually throwing your [armor] aside as you present Ember with an enticing view of your " + describeButt(player) + ",");
    // PC has balls:
    if (player.body.balls.count > 0) {
        CView.text(" testicles swaying lightly as you arch your backside up at " + emberMF("him", "her"));
        if (player.body.vaginas.length > 0) CView.text(" while... ");
        else CView.text(".");
    }
    if (player.body.vaginas.length > 0) CView.text(" your " + describeClit(player) + " drips with every passing moment.");
    ...
    if (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK > 0) CView.text("segmented");
    else CView.text("smooth");
    CView.text(" length of " + emberMF("his", "her") + " prick between your " + describeButt(player) + " cheeks, letting you feel the refreshing coolness of " + emberMF("his", "her") + " pre dripping from the tip.  \"<i>This... isn't going to work,</i>\" " + emberMF("he", "she") + " growls.");
    ...
    if (EmberFlags.EMBER_ROUNDFACE > 0) CView.text("mouth");
    else CView.text("maw");
    CView.text(" and sliding " + emberMF("his", "her") + " cock meaningfully against your pucker.  \"<i>You sure you want this?  Last chance to back out.</i>\"  As if you're going to turn away now, so close to filling that hunger for a nice, hot cock in your ass.  Rearing your rump up, you show " + emberMF("him", "her") + " that you're ready if " + emberMF("he", "she") + " is.");
    ...
    if (player.body.butt.looseness === 0 || player.analCapacity() < 25) CView.text(", leaving both of you wincing as the sensitive head of " + emberMF("his", "her") + " cock struggles to make room for its insertion.  \"<i>I can't believe you really want me to put it here... you're so tight it almost hurts,</i>\" " + emberMF("he", "she") + " whimpers.");
    // (PC has a loose asshole:)
    if (player.analCapacity() < 50) {
        CView.text(", causing you to gasp as " + emberMF("his", "her") + " ");
        if (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK === 1) CView.text("draconic ");
        CView.text("dick slips inside of you; stretching you and making you wince a bit in pain as " + emberMF("his", "her") + " spit rubs off along the interior of your anus.  \"<i>It feels pretty snug... good thing I prepared for this,</i>\" " + emberMF("he", "she") + " remarks with a gasp of pleasure.");
    }
    // (PC has a gaping asshole:)
    else {
        CView.text("; the looseness of your " + describeButthole(player.body.butt) + " gives " + emberMF("him", "her") + " no resistance whatsoever as " + emberMF("he", "she") + " glides in without trouble.  \"<i>Huh... that was easier than I expected,</i>\" " + emberMF("he", "she") + " remarks in surprise.  \"<i>What have you been getting yourself into?</i>\"");
    }
    displayStretchButt(player, 32, true, true, false);

    CView.text("\n\nEmber settles for a slow rhythm, pumping with slow strokes; gently guiding " + emberMF("his", "her") + " ");
    if (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK === 1) CView.text("draconic ");
    CView.text("prick in and out, reaching for your deepest recesses to rub on the right spots.");
    ...
    // (PC has prick:
    if (player.body.cocks.length > 0) CView.text("  You can feel the dragon's member rubbing against your prostate, making " + describeOneOfYourCocks(player) + " jump to painful erectness, bubble pre-cum, and drool it down the shaft to pool on the ground.");
    // (PC has cunt:
    if (player.body.vaginas.length > 0) {
        CView.text("  The stimulus is starting to make you wet as well, your [vagina] drooling feminine lubricant ");
        if (player.body.balls.count > 0) CView.text("over your [balls]");
        CView.text(" to puddle underneath you");
        if (player.body.cocks.length > 0) CView.text(", mingling with your pre to form an ever-growing pool of mixed sexual fluids.");
    }
    ...
    // (if PC has balls:
    if (player.body.balls.count > 0) CView.text("  " + emberMF("His", "Her") + " own balls swing into yours, and every time they connect a shrill wave of pleasure flows through you.");
    ...
    if (emberWomb.event > 3) {
        if (EmberFlags.EMBER_OVIPOSITION === 0) CView.text("swollen ");
        else CView.text("egg-laden ");
    }
    CView.text("belly, and start to rise and sink, pistoning your ass up and down. You have total control over the sex, and you intend to use that.");
    ...
    if (player.body.vaginas.length > 0) CView.text("your cunt gushing feminine lubricant to glaze Ember's belly");
    if (player.body.vaginas.length > 0 && player.body.cocks.length > 0) CView.text(" and ");
    if (player.body.cocks.length > 0) {
        CView.text("your cock ");
        if (player.cumQ() < 250) CView.text("spurting gouts of cum onto Ember's upper torso");
        else if (player.cumQ() < 1000) CView.text("raining spunk across the supine dragon");
        else CView.text("cascading your man-cream across your blissed-out lover until " + emberMF("he", "she") + " is completely glazed in a sperm coating");
    }
    CView.text(".");

    CView.text("\n\nEmber hugs you close, enjoying the afterglow");
    if (player.gender > 0) CView.text(" and rubbing the results of your own orgasm against you");
    CView.text(".  You sink readily into the dragon's embrace, rubbing against " + emberMF("him", "her") + ", feeling sleepy and blissful after a good, hard fuck.  Sadly, this moment of respite doesn't last long; you feel a certain discomfort as Ember's body heats up.  Looking up, you see " + emberMF("his", "her") + " eyes are wide open.  Before you can ask " + emberMF("him", "her") + " what's wrong, " + emberMF("he", "she") + " knocks you off with a start.");
...
function suckEmberCock(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("ember-give-her-a-blowjob");
    CView.text("You stare at Ember's crotch, imagining the taste of draconic cum on your tongue.  The thought makes you lick your lips in eagerness, and you ask if Ember would enjoy having you suck on " + emberMF("his", "her") + " cock.");
    ...
    if (EmberFlags.EMBER_GENDER === 1) {
        if (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK === 1) CView.text("his single genital slit");
        else CView.text("his genitals");
    }
    else {
        if (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK === 1) CView.text("her twin genital slits");
        else CView.text("her genitals");
    }
    CView.text(", " + emberMF("his", "her") + " ");
    if (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK === 1) CView.text("draconic ");
    CView.text("prick already hard and throbbing.  The dragon taps a clawed finger idly on the stump, awaiting your attention.");
    ...
    if (player.body.cocks.length > 0) CView.text("  [EachCock] hardens, spearing aimlessly into the ground in your arousal.");
    if (player.body.vaginas.length > 0) CView.text("  Your [vagina] starts to seep with feminine juices, your [clit] hardening in anticipation as your excitement dribbles down onto the thirsty ground.");
    ...
    if (EmberFlags.EMBER_ROUNDFACE === 0) CView.text("scaly");
    else CView.text("soft-skinned");
    CView.text(" lower belly");
    if (emberWomb.event > 3) CView.text(", though the gravid orb resists you");
    CView.text(", and discover a surge of pride in you that you got all sixteen inches of it down.");
    ...
    if (EmberFlags.EMBER_GENDER === 3 && (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK > 0)) {
        CView.text("  Struck by perverse inspiration, you manage to wriggle your tongue under the base of " + emberMF("his", "her") + " cock and thrust it into " + emberMF("his", "her") + " genital slit.  You guide it as deep into the strangely pussy-like orifice as you can, tickling and caressing.");
        // (DemonTongue:
        if (player.body.tongue.type > TongueType.HUMAN) CView.text("  Your inhuman length slithers deeper and deeper inside, and you realize you can feel two rounded objects; " + emberMF("his", "her") + " balls! You're actually touching the testicles that are normally locked away inside " + emberMF("his", "her") + " body, except when " + emberMF("he", "she") + " reaches " + emberMF("his", "her") + " most aroused states...");
    }
    ...
    if (player.stats.cor < 80) CView.text("amused");
    else CView.text("increasingly leery of " + emberMF("his", "her") + " batty behavior");
    CView.text(", then pick yourself up to head back to the ");
...
function stickDickInKnifeDrawer(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("ember-gives-you-a-blowjob");
    CView.text("Trying to appear confident and a little aloof, you suggest that maybe Ember would be willing to put " + emberMF("his", "her") + " kinky tongue to work on your cock.");
    ...
    // (Medium Affection)
        ...
        // Sex menu hooooo
        return emberSexMenu(player, false);
    }
    // (High Affection)
    else {
        CView.text("\n\nEmber looks at your crotch, then back at you; " + emberMF("his", "her") + " eyes glitter conspiratorially. \"<i>Ok... but only this once!</i>\"");
    }
    CView.text("\n\nRealising " + emberMF("he", "she") + " is looking at you to lead, you indicate " + emberMF("he", "she") + " should follow you and start heading for the outskirts of   Once you trust you are a safe distance away, you ");
    if (!player.body.legs.isNaga()) CView.text("seat yourself on a conveniently lichen-covered boulder.");
    else CView.text("coil up on your serpentine tail) and start peeling off the clothes on your lower half, exposing [eachCock] to the air and awaiting Ember's response.");
    ...
    // const y: number = largestCock + 1;
    if (player.body.cocks.length > 1) {
        CView.text("  " + emberMF("He", "She") + " selects the biggest cock and gives it a gentle stroke,");
    }
    else CView.text("  " + emberMF("He", "She") + " takes your " + describeCock(player, largestCock) + " in hand and begins gently stroking it,");
    CView.text(" sniffing your musk with obvious delight.");

    // (if Ember chose a dragon cock)
    if (largestCock.type === CockType.DRAGON) CView.text("\n\n\"<i>Such a magnificent, beautiful cock... it feels powerful... and familiar.</i>\"");
    ...
    if (EmberFlags.EMBER_ROUNDFACE === 0) CView.text(", the scales on " + emberMF("his", "her") + " palms producing an indescribable feeling as they slide across your sensitive flesh,");
    CView.text(" and unthinkingly blurt out your understanding.");

    CView.text("\n\nSatisfied at that, Ember extends " + emberMF("his", "her") + " tongue, slowly wrapping it around your " + describeCock(player, largestCock) + ", an act that makes you shiver as the inhumanly long, prehensile appendage twines around your member like a slick-skinned snake.  " + emberMF("He", "She") + " coils it around your shaft expertly, gripping you into a makeshift cock-sleeve.");

    CView.text("\n\nLooking down at Ember, you see the dragon wreathed in happiness; " + emberMF("he", "she") + "'s so focused on " + emberMF("his", "her") + " task that " + emberMF("he", "she") + " doesn't even notice you staring.  Using " + emberMF("his", "her") + " coiled tongue " + emberMF("he", "she") + " begins jerking you off; the sensation is familiar and yet so alien... it only takes a few strokes before your " + describeCock(player, largestCock) + " is glistening with Ember's saliva, and in a particularly pleasurable tug, you moan.  A bead of pre escapes your " + describeCockHead(largestCock) + " and slowly slides down your shaft, until it makes contact with Ember's tongue.");

    CView.text("\n\nThe moment " + emberMF("he", "she") + " tastes you, " + emberMF("his", "her") + " eyes seem to glow.  In a whip-like motion, Ember pulls " + emberMF("his", "her") + " tongue back into " + emberMF("his", "her") + " awaiting mouth, engulfing your " + describeCock(player, largestCock) + ".  You gasp as the cool, slick flesh surrounds you, heedless of the sharp teeth whose hard surfaces you can occasionally feel grazing you.  You trust your dragon too much to believe " + emberMF("he", "she") + " would hurt you.");

    CView.text("\n\nEmber sucks you hard, slurping around your shaft with " + emberMF("his", "her") + " prehensile tongue and cooing with pleasure.");
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text("  " + emberMF("His", "Her") + " shaft is already hard as rock, and sometimes you can feel it poke your " + describeFeet(player) + ", smearing small beads of Ember's barely contained pleasure on your " + skinFurScales(player) + ".");
    if (EmberFlags.EMBER_GENDER >= 2) CView.text("  Looking behind her, you see a small pool of her feminine juices has formed; pheromones fly through the air and hit you like a wave, spurring you on, making you feed Ember more of your delicious pre.");
    ...
    if (EmberFlags.EMBER_HAIR === 0 && EmberFlags.EMBER_ROUNDFACE === 0) CView.text("scaly pate");
    else CView.text("hair");
    CView.text(", telling " + emberMF("him", "her") + " that " + emberMF("he", "she") + " certainly has your attention and you're looking forward to finding out what " + emberMF("he", "she") + " has in mind.");
    ...
    else CView.text(describeCock(player, largestCock));
    CView.text(" and begins licking you from base to tip in slow, almost torturous, strokes.  \"<i>Mmm.  So tasty...</i>\"  Sparks of pleasure surge up your body from the dragon's ministrations, and you find yourself biting back words because you're not sure if you want " + emberMF("him", "her") + " to go faster or to keep at it like this.");
    ...
    CView.text("\n\nYou don't have long to wait, and with a wordless cry, you unleash yourself into Ember's thirsty mouth.  As the first jet hits Ember's tongue, " + emberMF("he", "she") + " cries out in bliss and buries your erupting " + describeCock(player, largestCock) + " as far into " + emberMF("him", "her") + "self as far as " + emberMF("he", "she") + " can.");
    ...
    if (player.cumQ() < 250) {
        ...
        if (!player.body.legs.isTaur()) CView.text("a finger");
        else CView.text("the thumb of her wing");
        CView.text(" into your ass to tickle your prostate, drawing a few last drops out of you.");
    }
    // (Medium Cum Amount)
    ...
    if (player.stats.cor < 75) CView.text(" and chuckle; you know " + emberMF("he", "she") + " loves you, really.");
    else CView.text(", folding your arms; " + emberMF("his", "her") + " dementia is getting worse...");
...
function stickItInEmbersButt(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("ember-fuck-her-in-her-buttz");
    CView.text("Your eyes are drawn to Ember's butt like iron filings to a magnet.  Haunted by the temptation to see " + emberMF("him", "her") + " bent over and offering " + emberMF("his", "her") + " ass to your hungry touch, you ask if Ember would be willing to be the catcher in a bout of anal sex.");
    ...
    // (High Affection)
        ...
        if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) {
            CView.text(emberMF("his", "her") + " stiffening cock");
            if (EmberFlags.EMBER_GENDER === 3) CView.text(" and ");
        }
        if (EmberFlags.EMBER_GENDER === 2) CView.text("her ");
        if (EmberFlags.EMBER_GENDER >= 2) CView.text("dripping pussy");
        CView.text(" make it obvious that the idea is at least partially arousing.");
    }
    // (first time and corr < 40)
    if (EmberFlags.TIMES_BUTTFUCKED_EMBER === 0) CView.text("\n\nYou ask if " + emberMF("he", "she") + "'s really so certain that it won't fit; you're eager to give it a try if " + emberMF("he", "she") + " is, but you won't push if " + emberMF("he", "she") + "'s really that scared of the idea...");
    ...
    EmberFlags.TIMES_BUTTFUCKED_EMBER++;
    ...
    if (player.body.vaginas.length > 0) CView.text("and [vagina] ");
    CView.text("to the dragon.  Once you're fully undressed, you turn to look at Ember, to see " + emberMF("him", "her") + " openly masturbating; " + emberMF("he", "she") + " pants as " + emberMF("he", "she") + " strokes " + emberMF("his", "her") + " ");
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text("cock ");
    if (EmberFlags.EMBER_GENDER === 3) CView.text("and ");
    if (EmberFlags.EMBER_GENDER >= 2) CView.text("pussy ");
    CView.text("with a hand, while " + emberMF("he", "she") + " suckles on the fingers of the other.");

    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(emberAnalCapacity()));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];

    CView.text("\n\nOnce " + emberMF("he", "she") + " sees you're fully stripped, " + emberMF("he", "she") + " walks over to you and pushes you on the grassy ground with a growl of lust.  The slick digits of one hand find themselves wrapped around your erect " + describeCock(player, cockThatFits) + ", while the others press into Ember's tight pucker, slowly stretching it to accommodate you.");

    // First Time
    if (EmberFlags.TIMES_BUTTFUCKED_EMBER === 1) {
    ...
    CView.text("\n\nYou ");
    if (player.stats.cor < 50) CView.text("promise the dragon you understand, and insist you wouldn't dream of doing otherwise; you want this to be good for " + emberMF("him", "her") + ", not to cause " + emberMF("him", "her") + " pain.");
    else CView.text("nod.");

    CView.text("\n\n\"<i>Umm... okay then.</i>\"  Stepping over you, the dragon lifts " + emberMF("his", "her") + " tail and aligns you with the puckered hole.  Slowly, Ember rubs " + emberMF("his", "her") + " ass against you, stimulating you to produce a single drop of pre to ease your way in.  " + emberMF("He", "She") + " doesn't have to wait long, and with a deep breath, " + emberMF("he", "she") + " finally lets gravity run its course and your " + describeCock(player, cockThatFits) + " plunges into " + emberMF("his", "her") + " tight depths.");
    ...
    if (EmberFlags.EMBER_ROUNDFACE === 0) CView.text("scaly ");
    CView.text("cheeks and rubbing the base of " + emberMF("his", "her") + " long, flexible tail to try and convey just how wild " + emberMF("his", "her") + " ass is driving you.");

    CView.text("\n\n\"<i>Ow!  Okay, okay!  Just stop and give me a moment to adjust at least,</i>\" Ember replies");
    if (EmberFlags.EMBER_ROUNDFACE === 0) CView.text(", panting with lust");
    else CView.text(", a blush adorning " + emberMF("his", "her") + " face");
    CView.text(".  You force yourself to remain still, giving the horny dragon a chance to recover from your " + describeCock(player, cockThatFits) + " impaling " + emberMF("his", "her") + " back passage, savoring the feeling of " + emberMF("his", "her") + " cool muscles squeezing and rippling around your shaft.");

    CView.text("\n\nAfter a few minutes, Ember finally starts to move, stroking you with " + emberMF("his", "her") + " inner muscles.  ");
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text("Droplets of pre slide along " + emberMF("his", "her") + " own shaft and down " + emberMF("his", "her") + " balls to gather at your belly.  ");
    if (EmberFlags.EMBER_GENDER >= 2) CView.text("Her slick pussy drips rhythmically, wetting your lower body with slick dragon juice.  ");
    CView.text(emberMF("His", "Her") + " lusty moans each time " + emberMF("he", "she") + " comes down on your " + describeCock(player, cockThatFits) + " send a thrill up your spine, and the deep purring emanating from " + emberMF("his", "her") + " chest is audible enough to give you an idea of how much Ember is enjoying this, despite " + emberMF("his", "her") + " initial reluctance.");
    ...
    else CView.text("the base of your spine");
    CView.text(", and you groan to Ember that you're going to cum, soon.");
    ...
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text(", jet upon jet of spunk arcing over " + emberMF("his", "her") + " head to paint the ground below");
    if (EmberFlags.EMBER_GENDER >= 2) CView.text("; a veritable gush of juices spews forth from her forgotten love-hole to splash against your lower body and over herself");
    CView.text(".  The force of " + emberMF("his", "her") + " orgasm makes the dragon clench " + emberMF("his", "her") + " ass, sending waves of renewed pleasure through you.");
    ...
    if (player.stats.str < 70) {
        CView.text("desperately try to grab any of the flying pieces, but Ember keeps you pinned with " + emberMF("his", "her") + " tail");
        CView.text(".");
    ...
    // (else str >=80)
        ...
        CView.text("grab " + emberMF("his", "her") + " tail, pulling the dragon off " + emberMF("his", "her") + " feet and throwing " + emberMF("him", "her") + " to the ground as you roll over... and causing " + emberMF("him", "her") + " to drop them everywhere as " + emberMF("he", "she") + " falls.");
        ...
        // New
        CView.text("\"<i>What do you think you're doing!?</i>\" Ember yells at you, obviously mad at your little stunt, one hand reaching behind to prevent any more cum from spilling.");
...
function slurpDraggieCunnies(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("ember-eat-out-her-vagoo");
    CView.text("You contemplate the possibilities, and then offer to get down on your knees before the dragon and pleasure her orally.  You would love to have a taste of some dragon juice.");
    ...
    if (emberAffection() <= 25) {
        ...
        if (!User.settings.silly()) CView.text("with me");
        else CView.text("on my affection meter");
        CView.text(" just because you're offering to do this,</i>\" the dragon hastily adds.");
    }
    // (Medium Affection)
    else if (emberAffection() < 75) {
        ...
        if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text(" and slowly hardening shaft");
        CView.text(".");
    }
    // (High Affection)
    else {
        ...
        if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text(" and shaft");
        CView.text(".  \"<i>Ok... I suppose I would like that,</i>\" Ember replies, trying to calm herself down, lest she seem too excited by the idea.");
    }
    ...
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text("  With Ember's cock jutting out above your head, you can't resist sliding your tongue along the underside of her shaft, just for an extra teasing motion.");
    ...
    if (EmberFlags.EMBER_ROUNDFACE > 0) CView.text("blushing all the while and ");
    CView.text("panting from the stimulation.");
    ...
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text("  You think droplets of pre are starting to slide down her girl-cock and drip onto your head... but that's part and parcel of loving a herm, so you pay it no attention.");
    ...
    if (player.body.tongue.type > TongueType.HUMAN) CView.text("inhuman ");
    ...
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text(", heedless of the steady rain of pre dribbling into your " + describeHair(player) + ".  You're confident she'll crack soon...");
    ...
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text("  Her throbbing shaft, not intending to be left out of the fun, records the event by marking the tree bark with white dragon spooge.");
    ...
    if (User.settings.silly()) CView.text("  My meter's not that full!");
    CView.text("</i>\"");
    ...
    if (player.body.cocks.length > 0) CView.text("stiffened shaft");
    if (player.gender === 3) CView.text(" and ");
    if (player.body.vaginas.length > 0) CView.text("moist cunt");
    if (player.gender === 0) CView.text("empty crotch");
    CView.text(" in sympathy, you head back to ");
...
function getEatenOutByEmbra(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("ember-eats-your-vagoo-out");
    CView.text("You think about Ember's long tongue and the many advantages it must have, when you suddenly get an idea of what you'd like to do. You ask Ember if " + emberMF("he", "she") + "'d be willing to put that tongue of " + emberMF("his", "hers") + " to use and eat you out.");
    // (Low Affection)
    if (emberAffection() <= 25) {
    ...
    if (!player.body.legs.isNaga()) CView.text(" with your [legs] spread wide");
    CView.text(", displaying your [vagina].  You tell Ember that " + emberMF("he", "she") + "'ll need to make the next move from here, smiling as you do so.");
    ...
    if (player.body.cocks.length > 0) CView.text(", as well as the musk emanating from your drooling cock");
    CView.text(".  Ember's eyes close as " + emberMF("he", "she") + " savors your scent, committing it to memory and licking " + emberMF("his", "her") + " lips in preparation for the task ahead.");
    ...
    if (player.body.cocks.length > 0) CView.text(", and pre-cum beginning to bubble out of " + describeOneOfYourCocks(player));
    CView.text(".  You wriggle in delight, praising Ember's skill with " + emberMF("his", "her") + " tongue and begging " + emberMF("him", "her") + " to keep going.");

    CView.text("\n\nEmber presses on, nudging your pleasure button with " + emberMF("his", "her") + " nose and wrapping your [vagina] with " + emberMF("his", "her") + " ");
    if (EmberFlags.EMBER_ROUNDFACE === 0) CView.text("maw");
    else CView.text("mouth");
    CView.text(", kissing your contracting box as if it were a long-lost lover, pushing " + emberMF("his", "her") + " tongue ever deeper inside you, as if intent on reaching your womb.");
    ...
    if (player.body.vaginas.get(0).wetness >= 4) CView.text("spattering");
    else CView.text("drenching");
    CView.text(" Ember's face with your feminine honey");
    if (player.body.cocks.length > 0) CView.text(" even as " + describeOneOfYourCocks(player) + " belches cum all over the preoccupied dragon");
    CView.text(".");
    ...
    if (player.body.vaginas.get(0).wetness >= 4) CView.text("  However, your squirting spatters make this an impossible task.  Ember doesn't seem to care, even as " + emberMF("his", "her") + " face is completely covered in femcum.");
    ...
    CView.text("\n\nYou smile, hating to see " + emberMF("him", "her") + " go, but so loving to watch " + emberMF("him", "her") + " leave.  Shaking off your pleasurable fantasies, you manage to pull yourself back upright, redress yourself, and return to ");
...
function penetrateEmbrah(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("ember-fuck-her-in-the-vagoo-with-your-penor");
    ...
    CView.text("Your eyes are drawn to the dragon's crotch, and you ask if she has any feminine itches she'd like scratched.");
    ...
    // (High Affection)
        ...
        if (EmberFlags.EMBER_GENDER === 3) {
            if (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK === 1) CView.text(" and her cock peek out of its protective slit");
            else CView.text(" and her cock begin hardening");
        }
        CView.text(" in anticipation.  \"<i>Yeah... maybe... why do you ask?</i>\"");
    }
    // (First Time)
    if (EmberFlags.EMBER_PUSSY_FUCK_COUNT === 0) {
        ...
        if (player.stats.cor > 75) CView.text("  As if bitches like her could qualify as maidens in any but the most literal sense.");
    ...
    CView.text("\n\nYou ");
    if (player.stats.cor < 50) CView.text("hasten to undress yourself for the impatient dragoness, quickly pulling");
    else CView.text("languidly denude yourself, making sure to exaggerate every movement as you pull");
    CView.text(" off your [armor] until you are standing naked before her, letting her see what you have to offer in terms of phallic delights.");

    // If the PC is too big
    if (player.body.cocks.find(Cock.CockThatFits(emberVaginalCapacity()))) {
        CView.text("\n\nEmber looks at your " + describeCocksLight(player) + ", then touches her pussy in thought.  Finally, she growls in exasperation.  \"<i>");
        if (player.body.cocks.length === 1) CView.text("It doesn't ");
        else CView.text("None of them ");
        CView.text("fit!</i>\"");
        ...
        if (player.body.cocks.length === 1) CView.text("that... that... monster inside me,");
        else CView.text("any of those... those... monsters inside me,");
        CView.text(" you'll tear me apart!</i>\"  Ember finishes by exhaling a puff of smoke in frustration... clearly she wants this as much as you.");

        CView.text("\"<i>Find some way to shrink ");
        if (player.body.cocks.length === 1) CView.text("that");
        else CView.text("those");
        CView.text(" and then come back!</i>\"  Ember turns on her heels and walks away, moodier than usual.");
        return { next: returnToCampUseOneHour };
    }
    ...
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(emberVaginalCapacity()));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    if (player.body.cocks.find(Cock.CockThatFits(emberVaginalCapacity())) && player.body.cocks.filter(Cock.CocksThatFit(emberVaginalCapacity()))[1]) {
        CView.text("\n\n\"<i>You have quite a selection, but I only need one... this one!</i>\" Ember says, taking your " + describeCock(player, cockThatFits) + " in her hand and stroking it into a full erection.");
    }
    // (if PC has a dragon cock)
    if (cockThatFits.type === CockType.DRAGON) {
        ...
        CView.text("\n\nEmber takes your " + describeCock(player, cockThatFits) + " in her hand and begins gently stroking you to full mast.  \"<i>I hope you know how to use this,</i>\" Ember remarks.");
        CView.text("\n\nYou assure her that you know exactly what you're doing. ");
    }
    CView.text("\n\nDeciding it's better for the dragon to psyche herself up first, you wait patiently; it doesn't look like it will be long before she makes the first move.  Ember's hands roam all over your " + describeCock(player, cockThatFits) + " and you think you can almost hear a faint purring emanating from her chest.  Slowly one of her hands reaches down to massage her moistened netherlips.  Ember gasps in pleasure as she continues masturbating both of you in a daze, until she suddenly snaps out of her trance.");
    ...
    CView.text("smile and, with a flamboyant grace, ");
    CView.text("gently lay yourself down, spreading your limbs and making it clear you're ready for whatever she has in mind.");

    CView.text("\n\nEmber can't resist licking " + emberMF("his", "her") + " lips as she straddles you, aligning your " + describeCock(player, cockThatFits) + " with her dripping pussy; then finally, with a sigh, letting gravity help your erect shaft inside her inviting depths.  You grit your teeth as her walls close around your prick, the firm muscles inside hungrily swallowing your length and starting to instinctively ripple and surge around it, beginning to milk you.  Your own instincts in turn compel you to buck and thrust, striving to bury yourself further into Ember's eager depths.");
    ...
    CView.text("\n\nYou are too caught up in your thrusting to reply for a moment, but manage to snatch back enough of your wits to tell her that if she wants to be in control, she'll need to act like the one in control.  You then resume plunging your hips up into hers, sheathing and unsheathing your " + describeCock(player, cockThatFits) + " with each powerful, anxious thrust.");

    CView.text("\n\nEmber growls and pins you under her by your shoulders, then begins moving her hips up and down");
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) {
        CView.text("; her ");
        if (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK > 0) CView.text("draconic");
        else CView.text("humanoid");
        CView.text(" prick slaps wetly against your belly with each thrust, smearing pre on you as the engorged tool bounces back and forth");
    }
    CView.text(".");
    ...
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text("; her own erect cock spurts a small jet of pre");
    CView.text(".");
    ...
    if (player.cumQ() < 250) CView.text("dribble");
    else if (player.cumQ() < 1000) CView.text("flow");
    else CView.text("gush");
    CView.text(" from her used cunt and you sit on the ground, while Ember remains limp on the ground, panting. Still suffering from the afterglow of your recent orgasm, you dimly realize that Ember herself doesn't seem to have cum yet; the only fluids dripping from her pussy are yours");
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text(", and her cock, while engorged and swollen, hasn't poured forth yet");
    CView.text(".");
    ...
    if (player.stats.cor >= 66) {
        ...
        emberAffection(-5);
        EmberFlags.EMBER_PUSSY_FUCK_COUNT++;
        return { next: returnToCampUseOneHour };
    }
    // else
    ...
    if (EmberFlags.EMBER_GENDER === 3) CView.text("; her cock jumps and unloads on the ground below, marking this as the spot of your lovemaking");
    CView.text(".  Her pussy grabs at you like a vice, constricting you with such pressure that you have trouble moving, even though her cunt is well-lubed from your mixed fluids.");
    ...
    // (High Cum Amount)
        ...
        if (player.body.balls.count === 0) CView.text("prick is ");
        else CView.text("[balls] are ");
        CView.text("finished, Ember doesn't only look more than a few months pregnant; she looks absolutely huge, like her water could break at any moment!");
    }
    ...
    return { next: penetrateEmbrahPartII };
}

// PART II!
function penetrateEmbrahPartII(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You wake up to find Ember's face hovering over you with a smile; once she realizes you're awake, she quickly averts her gaze.");
    ...
    if (player.cumQ() < 1000) {
        ...
        player.body.cumMultiplier += 1 + randInt(3);
    }
    // (else if already at Ember-given ceiling)
    else CView.text("\n\nStill, you don't think you could produce any more cum than you already do...");

    CView.text("\n\nYour decency restored, you return to ");
    EmberFlags.EMBER_PUSSY_FUCK_COUNT++;
    return { next: returnToCampUseTwoHours };
}

// Get Penetrated - also horse-proof, sorry folks! (Z)
function getPenetratedByEmberLastSexSceneWoooo(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("ember-sticks-her-penor-in-your-vagoo");
    CView.text("Your eyes are drawn to the 16 inches of cool, throbbing ");
    if (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK > 0) CView.text("dragon");
    else CView.text("human");
    CView.text(" dick between Ember's legs, and you lick your lips hungrily.  Your " + describeVagina(player, player.body.vaginas.get(0)) + " throbs eagerly as you tell Ember you have certain itches you need to scratch... is Ember enough of a " + emberMF("man", "herm") + " to scratch them for you?");
    ...
    // (High Affection)
        ...
        CView.text("Ember's cock jumps ");
        if (EmberFlags.EMBER_ROUNDFACE > 0 && EmberFlags.EMBER_INTERNAL_DICK === 0) CView.text("to attention");
        else CView.text("out of its protective slit");
        CView.text(" at the mere mention of what you expect " + emberMF("him", "her") + " to do.  \"<i>I'm more than enough to handle two... no... three of you!  And if you want proof all you have to do is ask!</i>\" Ember boasts proudly.");
    }
    ...
    if (player.body.tongue.type === TongueType.HUMAN) CView.text("yours ");
    else CView.text("your own inhumanly sinuous muscle ");
    CView.text("and slithering almost into your throat.  " + emberMF("He", "She") + " kisses you madly, even as " + emberMF("he", "she") + " sinks to " + emberMF("his", "her") + " knees and gently lays you out on the ground, clearly ready to start the sexing.");
    if (player.body.cocks.length > 0) CView.text("  As if " + emberMF("his", "her") + " hands caressing your cock weren't evidence of that.");
    ...
    if (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK > 0) CView.text("draconic ");
    else CView.text("human-like ");
    CView.text("cock begins making its way past your labia.");

    // (if Ember has a dragon cock)
    if (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK > 0) {
        ...
        displayStretchVagina(player, 32, true, false, true);
        CView.text("Finally you arrive at the base of " + emberMF("his", "her") + " cock where a bulbous knot sits; thankfully it hasn't inflated yet, but still, you can't help but groan as you are stretched even more by its entrance.");
    }
    // else
    else {
        ...
        CView.text("  It's not as if it isn't still warm... but its lower temperature feels relieving and exciting all the same.");
    }
    ...
    CView.text("\n\nEmber leans over you, panting, " + emberMF("his", "her") + " hot breath mere inches from your " + describeFaceShort(player) + ", gazing at you through half-lidded, lust-addled eyes.  \"<i>You feel so good... so hot... so sexy...</i>\" Ember says mid-pant.  You pant right back, reaching up and wrapping your arms around " + emberMF("his", "her") + " neck, staring back at " + emberMF("him", "her") + " with the same expression and telling " + emberMF("him", "her") + " you think " + emberMF("he", "she") + " feels just as good.");
    ...
    if (player.body.cocks.length > 0) {
        CView.text("  Your cock explodes, belching cum to ");
        if (player.cumQ() < 250) CView.text("spatter");
        else if (player.cumQ() < 1000) CView.text("drench");
        else if (player.cumQ() < 2000) CView.text("completely cover");
        else CView.text("utterly soak");
        CView.text(" your draconic partner.");
    }
    ...
    if (player.perks.has(PerkType.HistoryWhore) || player.perks.has(PerkType.HistorySlut)) CView.text("  Using the skills you've honed, you make the muscles in your vagina ripple and wrinkle, teasing the cock caught inside you in a way few women can.");
    CView.text("  Mmm... you can feel your own second orgasm coming in hot.  Maybe after another 4 or 5, you'll let the dragon go...");
...
function getPenetratedByEmberLastSexSceneWooooPartII(player: Character): NextScreenChoices {
    ...
    if (emberAffection() < 75) CView.text(emberMF("his", "her") + " cock was unlike anything else in this world?");
    else CView.text(emberMF("he", "she") + " could handle two or even three of you at the same time?");
    CView.text("  You expected more resistance if that was the case.");
...
export function emberRapesYourHeatness(player: Character): NextScreenChoices {
    ...
    if (player.body.vaginas.length > 0 && player.effects.has(StatusEffectType.Heat) && (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3))
        CView.text("your [vagina] aching to be filled and your womb put to its proper purpose");
    else CView.text("[eachCock] burning to be seeding ripe, ready wombs");
    CView.text(".");

    CView.text("\n\nEmber responds by ");
    if (player.body.vaginas.length > 0 && player.effects.has(StatusEffectType.Heat) && (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3))
        CView.text("humping against you, smearing your thigh with dragon pre.");
    else CView.text("tightly pressing her drooling, puffy netherlips on your thighs.");
    CView.text("  \"<i>I need you, [name].  I need you so badly... can you see how badly I need you?</i>\" Ember asks, panting in barely contained lust.  \"<i>I want to fuck you so badly... Let's make a baby now!</i>\"");
    player.stats.lust += 10 + player.stats.lib / 10;

    CView.text("\n\nWhat do you say?");
    // [Accept] [Deny]
    return { choices: [["Accept", timeToPuffTheMagicDragon], ["Deny", fuckOffEmberIWantANap]] };
}

// [=Deny=]
function fuckOffEmberIWantANap(player: Character): NextScreenChoices {
    CView.clear();
    player.stats.lust += 10 + player.stats.lib / 10;
    ...
    CView.text(" so yearn");
    if (player.gender === 1 || player.gender === 2) CView.text("s");
    CView.text(" to take Ember up on " + emberMF("his", "her") + " offer! ...But you are better than this; you are not some mindless animal, a slave to your lusts.  You fight down the arousal gnawing at you and tell Ember you don't want to fuck " + emberMF("him", "her") + ".");
    ...
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text("protruding prick");
    if (EmberFlags.EMBER_GENDER === 3) CView.text(" and ");
    if (EmberFlags.EMBER_GENDER >= 2) CView.text("creaming wet vagina");
    CView.text(".  No doubt " + emberMF("he", "she") + "'s off to take care of " + emberMF("his", "her") + " needs by " + emberMF("him", "her") + "self.  You doubt " + emberMF("he", "she") + "'s going to be happy once " + emberMF("his", "her") + " head is clear... still, " + emberMF("he", "she") + "'ll get over it.");
    emberAffection(-10);
    // Set some cooldown so this doesn't proc all the goddamn time!
    player.effects.add(StatusEffectType.EmberFuckCooldown, 12, 0, 0, 0);
    return { next: campMenu };
}

// [=Accept=]
function timeToPuffTheMagicDragon(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Dazed and befuddled, you sniff Ember right back.  Mmm... " + emberMF("He", "She") + " smells delicious too, you tell " + emberMF("him", "her") + ".");
    ...
    if (player.body.wombs.find(Womb.NotPregnant) && !emberWomb.isPregnant() && EmberFlags.EMBER_GENDER === 3 && player.gender === 3) {
        CView.text("\n\n(Who should bear the kids?)");
        // [Ember] [You]
        return { choices: [["Ember", breedEmberPregnantAsIfThereWasAnyOtherKindOfBreeding], ["You", getKnockedUpByEmbrahBroBaby]] };
    }
    // [Play appropriate breeding scene.]
    // Female Breeding Scene:
    // PC not pregnant, Ember has dick, PC is in heat.
    else if (player.body.wombs.find(Womb.NotPregnant) && (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) && player.effects.has(StatusEffectType.Heat)) {
        return getKnockedUpByEmbrahBroBaby(player);
    }
    // Male scene
    // PC has dick, ember not pregnant, PC is in rut
    else {
        return breedEmberPregnantAsIfThereWasAnyOtherKindOfBreeding(player);
    }
}
...
function getKnockedUpByEmbrahBroBaby(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Ember grabs you and rolls you around, pinning you under " + emberMF("his", "her") + " weight, whilst kissing you.  You allow the dragon to press you into the ground, rubbing your hands across ");
    if (EmberFlags.EMBER_GENDER === 1 && EmberFlags.EMBER_MILK === 0) CView.text("his flat, muscular chest");
    else CView.text(emberMF("his", "her") + " soft, squeezable boobs");
    CView.text(" and savoring the kiss.  Emboldened, you poke your tongue into " + emberMF("his", "her") + " mouth, to see how " + emberMF("he", "she") + " will react.");
    ...
    if (EmberFlags.EMBER_OVIPOSITION === 0) CView.text("little dragons");
    else CView.text("dragon eggs");
    CView.text(", my beautiful.</i>\"  Ember resumes " + emberMF("his", "her") + " kissing; " + emberMF("his", "her") + " own hands roaming all over your body, as if mapping you out through touch, looking for the best places to touch and tease.");
    ...
    if (EmberFlags.EMBER_OVIPOSITION === 0) CView.text("dragonlings");
    else CView.text("dragon eggs");
    CView.text(".  You want to breed!");
    ...
    CView.text("  You groan throatily, your need burning through you, and start trying to grind yourself against Ember's shaft.  " + emberMF("He", "She") + " pins you down, though, making it impossible for you to actually do anything more than rub " + emberMF("his", "her") + " glans against your needy netherlips.");
    ...
    // (if Ember has a vagina)
    if (EmberFlags.EMBER_GENDER === 3) CView.text("\n\nYour bedroll is growing slick underneath you as lubricant drools from the herm dragon's neglected cunt, her tail thrashing around madly before sliding into the wet orifice to help goad her on in her goal to breed you.  ");
    else CView.text("\n\n");
    CView.text("You growl throatily, feeling that same need burning inside you. You clutch and claw against the dragon, fingers grasping deeply into " + emberMF("his", "her") + " scaly limbs as you drag, scrape, and thrust yourself against " + emberMF("him", "her"));
    if (player.body.cocks.length > 0) CView.text(", your neglected [cock] drooling pre-cum that is being smeared all across your belly and " + emberMF("his", "hers") + " with every move you make");
    CView.text(".  Gods, you can feel " + emberMF("him", "her") + " filling you...");
    ...
    CView.text("Your contracting walls milk the dragon's sensitive cock, trying to coax even more seed out of your panting dragon lover.  \"<i>I-it's time,</i>\" Ember mutters, crashing into your wet bedroll and thrusting deep inside you.");
    ...
    if (EmberFlags.EMBER_ROUNDFACE === 1) CView.text("blushing ");
    CView.text("face.  \"<i>But if you must know, I do love you.</i>\"   With a final sigh Ember sets down on your bedroll for a quick nap.");
    ...
    if (EmberFlags.EMBER_OVIPOSITION === 0) CView.text("baby dragon");
    else CView.text("dragon egg");
    CView.text(".  Yawning, you curl up to the dragon for a quick nap of your own.");
    player.orgasm();
    player.stats.sens += -2;

    // Preg shit goez hurdur
    if (!player.effects.has(StatusEffectType.Contraceptives))
        player.body.wombs.find(Womb.NotPregnant).knockUp(new Pregnancy(PregnancyType.EMBER, IncubationTime.EMBER), 0, true); // Will always impregnate unless contraceptives are in use
    player.effects.add(StatusEffectType.EmberFuckCooldown, 36, 0, 0, 0);
    return { next: partial(emberBreedingAfterMathWatchOutForRadioactiveFallout, player, false) };
}

// Breeding Ember
function breedEmberPregnantAsIfThereWasAnyOtherKindOfBreeding(player: Character): NextScreenChoices {
    CView.clear();
    // Silently steal her virginity.
    EmberFlags.EMBER_PUSSY_FUCK_COUNT++;
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(emberVaginalCapacity()));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    CView.text("Ember catches you and rolls you around, pinning you to the ground under her.  She smiles at you seductively and reaches down to stroke your " + describeCock(player, cockThatFits) + ".");
    if (player.body.balls.count > 0) CView.text("  She then reaches down lower to rub at your cum filled orbs.");
    if (player.body.vaginas.length > 0) CView.text("  The tip of her tail gently teases your slick [vagina] with tiny strokes.");
    CView.text("  You shudder and instinctively thrust towards her, letting her feel your needy cock, full and aching to father young.");
    ...
    CView.text("\n\nEmber jumps off you momentarily to inhale your musk, stroking you until a bead of pre forms on your " + describeCockHead(cockThatFits) + ".  Ember's eyes grow wide, as if she had spotted a pearl; and she wastes no time in extending her tongue to lick the bead away.  \"<i>Hmm, delicious...  You're ready, and so am I!</i>\"");

    CView.text("\n\nAiming your cock upwards; Ember straddles you and lowers herself on your pulsing " + describeCock(player, cockThatFits) + ", hissing in pleasure as your hot flesh finally makes contact with her drooling fuckhole");
    if (emberHasCock()) CView.text(", her own cock springing up and throbbing with the electric contact.");
    else CView.text(".");
    ...
    if (EmberFlags.EMBER_ROUNDFACE === 0) CView.text("scale-covered ");
    CView.text("orbs gently squish ");
    if (EmberFlags.EMBER_MILK > 0) CView.text("and leak milk ");
    CView.text("under your grasp.");
    ...
    if (EmberFlags.EMBER_MILK > 0) CView.text("the slowly trickling rivulets of milk along with ");
    CView.text("Ember's pleased moans are enough to make you push away such thoughts, you're glad she has those!");

    CView.text("\n\nYou hungrily kiss her ");
    if (EmberFlags.EMBER_ROUNDFACE === 0) CView.text("draconic ");
    else CView.text("oh-so-human ");
    CView.text("face, nipping lightly at her lip, rolling your tongue over hers and savoring her inhuman tastes; Ember answers in kind with a kiss of her own, purring and moaning into your mouth.  Her vibrating breasts massage your hands as you knead, lift, and then roll them about.");

    // (if Ember has a dick)
    if (emberHasCock()) {
        CView.text("\n\nYou feel something poke your [chest] and look down to see Ember's painfully erect ");
        if (EmberFlags.EMBER_ROUNDFACE === 0 || EmberFlags.EMBER_INTERNAL_DICK === 1) CView.text("dragon ");
        CView.text("prick.  With a mischievous curl of your lips, you pull Ember deeper into your kiss and grab her cock, stroking it vigorously.");
    }
    ...
    if (emberHasCock()) CView.text("her cock spurts a few jets of cum onto your [chest]; ");
    CView.text("her eyes roll to the back of her head.  With a muffled roar she finally caves, a veritable wave of dragon juices crashing into your lower body and pooling on your bedroll, filling the whole tent with the delicious smell of sex and pheromones.");

    // (If Ember has a dick)
    if (emberHasCock()) CView.text("  Ember's cock wets the side of the tent with continuous jets of cum, only adding to the already overpowering smell of sex as she empties herself on top of you.");
    ...
    if (emberHasCock()) CView.text("her spent cock throb; ");
    CView.text("her eyes shut tight in pleasure as she reaches her second orgasm.  You muffle her groan of pleasure with one of your own.  A second wave of dragon fluids joins the first in wetting your lower body, along with your bedroll.");
    if (emberHasCock()) CView.text("  Little gobs of cum spill from her cock to slide down her shaft and gather on your belly button.");
    CView.text("  Her tail coils around you, helping her stay in position as she melts on top of you like putty.");
    ...
    if (player.body.vaginas.length > 0) CView.text("your neglected cunt splashing fluid down your [legs] and ");
    CView.text("your " + describeCock(player, cockThatFits) + " gushing into the dragon's ready womb.");
    ...
    // (Low Cum Amount)
    if (player.cumQ() < 250) CView.text("  Though you usually don't cum that much, Ember's contracting walls literally suck the cum out of you; forcing you to give up more than you usually would... something you're quite glad to do!");
    // (Medium Cum Amount)
    else if (player.cumQ() < 1000) CView.text("  Your generous helping of cum works its way towards her awaiting womb.  In fact, due to the delicious massage your shaft is receiving from Ember's contracting, almost sucking vaginal walls, you find yourself pumping out even more cum that you would usually have, quickly filling her to the brim and beyond.  It's not until Ember's belly is slightly distended that you stop.");
    // (High Cum Amount)
    else CView.text("  You dump a huge load of jism into Ember's awaiting belly, but her contracting walls seem to milk you for ever more!  Ember is filled to the brim, her belly already slightly distended, but you still continue to fill her with your seed.  Ember's tightly sealed pussy lips means the cum has nowhere to go, so instead it gathers in her womb, distending her belly until she looks at least a few months pregnant; and by the look of pleasure on Ember's panting face, you don't think she minds it.");

    CView.text("\n\nWith a gasp and a sigh of utter relief, you slump into Ember's arms, collapsing onto her ");
    if (emberHasCock()) CView.text("cum-slick ");
    CView.text("belly and laying there to regain your strength, ");
    if (emberHasCock()) CView.text("dismissive of the spent cock lying sandwiched between you as you start ");
    CView.text("feeling the heat in her gut that marks the beginning of a child.");

    CView.text("\n\nEmber purrs and pulls you into another kiss, holding you close with her tail, legs and arms; intent on keeping you right where you are.  Well, it's not like you could go anywhere else with her pussy effectively keeping your " + describeCock(player, cockThatFits) + " hostage inside its depths.");
    ...
    emberWomb.knockUp(new Pregnancy(PregnancyType.PLAYER, IncubationTime.EMBER), emberPregEvents);
    player.effects.add(StatusEffectType.EmberFuckCooldown, 36, 0, 0, 0);
    player.effects.remove(StatusEffectType.Rut);
    player.orgasm();
    player.stats.sens += -2;

    return { next: partial(emberBreedingAfterMathWatchOutForRadioactiveFallout, player, true) };
}

// Bred/Breeding Aftermath
function emberBreedingAfterMathWatchOutForRadioactiveFallout(player: Character, emberPregged: boolean = true): NextScreenChoices {
    CView.clear();
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(emberVaginalCapacity()));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    CView.text("You wake up, feeling replenished after your exhausting mating session with your draconic lover, and stretch the last few kinks out.  As you do, you realize you're in still in your tent, which is perfectly clean, with no trace of the copious sexual fluids that you and Ember were splattering everywhere before you took your impromptu nap.");
    ...
    CView.text("\n\nEmber's cheeks ");
    if (EmberFlags.EMBER_ROUNDFACE === 0) CView.text(" are probably blushing under " + emberMF("his", "her") + " scales.");
    else CView.text("blush.");
    CView.text("  \"<i>You're welcome, but this is the only time I'll clean up after you!  I'm not your personal maid!</i>\"");
    ...
    if (!emberPregged) CView.text("touch your belly");
    else CView.text("point at Ember's belly");
    CView.text(", and you ask the dragon if " + emberMF("he", "she") + " thinks it \"<i>took</i>\".");

    CView.text("\n\nEmber puffs " + emberMF("his", "her") + " chest and proudly boasts, \"<i>Of course it did!  We're both very virile!  And after all the cum you ");
    if (emberPregged) CView.text("pumped into me...");
    else CView.text("had me pump into you...");
    CView.text("</i>\" Ember trails off, and you can see " + emberMF("his", "her") + " hand move to " + emberMF("his", "her") + " crotch to caress " + emberMF("his", "her") + " ");
    if (emberPregged) CView.text("slit");
    else CView.text("cock");
    CView.text(", as if remembering the intense sex session you two just had.");
    ...
    CView.text("\n\nEmber's ");
    if (EmberFlags.EMBER_ROUNDFACE === 1) CView.text("blush");
    else CView.text("discomfort");
    CView.text(" deepens, and " + emberMF("he", "she") + " frowns at you.  \"<i>A-are you questioning my virility!?  Well, if you want, I can go again, right now!</i>\"");
    ...
    CView.text("\n\nEmber ");
    if (EmberFlags.EMBER_ROUNDFACE === 1) CView.text("blushes");
    else CView.text("frowns");
    CView.text(" once more.  \"<i>D-don't get ahead of yourself!</i>\" " + emberMF("he", "she") + " yells indignantly, then gets up and rushes away.  You watch and give a smile; " + emberMF("he", "she") + " is champing at the bit to make children with you, but can't bring " + emberMF("himself", "herself") + " to confess how " + emberMF("he", "she") + " really feels.  Still, you're quite certain your efforts took.");
    // (if PC now pregnant:
    if (!emberPregged) CView.text("  You touch your belly with a smirk.  Very certain they took indeed...");
    return { next: campMenu };
}

function emberPregUpdate(player: Character): boolean {
    // trace("EMBER PREG: " + EmberFlags.EMBER_INCUBATION + "EMBER AFF: " + emberAffection());
    switch (emberWomb.eventTriggered()) {
        case 1: //
        ...
        case 4:
            ...
            if (EmberFlags.EMBER_OVIPOSITION > 0) {
                CView.text("Ember notices you looking.  \"<i>W-what? Never seen a pregnant woman before?</i>\" she asks indignantly, although she can't hide her smile as you continue to look.\n");
            }
            else {
                CView.text("Ember catches you looking" + (EmberFlags.EMBER_ROUNDFACE === 1 ? " and blushes" : "") + ".  \"<i>W-what is it?</i>\"  You reply merely that she carries a baby bump very well; she looks good pregnant.  \"<i>Oh, uh...  Thanks I guess?</i>\" she replies, looking away and flicking her tongue out nervously.\n");
            }
            return true;
        case 5:
            if (EmberFlags.EMBER_OVIPOSITION > 0) {
...
export function emberGivesBirth(player: Character) {
    ...
    if (EmberFlags.EMBER_OVIPOSITION === 0) {
        ...
        CView.text(" as her womb works to expel its long-time occupant.  You place a hand on either side of the distended orb and begin to massage it, trying to help soothe the strained, aching muscles.  Your eyes go from her stomach to her crotch; her vagina is opened wide as the infant inside her begins making its way into the world.");
        if (emberHasCock()) {
            if (EmberFlags.EMBER_ROUNDFACE === 0 && EmberFlags.EMBER_INTERNAL_DICK === 0) CView.text("  Her human-like cock dangles heavily in front of her pussy, partially erect from the muscular spasms.");
            else CView.text("  Her draconic cock has been pushed from its internal sheathe by the pressure, but it's too painful for her to be erect.");
        }
        ...
        if (EmberFlags.EMBER_ROUNDFACE === 0) CView.text("n anthro dragon");
        else CView.text(" hybrid of human and dragon");
        CView.text(".  It's a beautiful strong, healthy little ");
        const roll: number = randInt(100);
        if (roll < 40) CView.text("boy");
        else if (roll < 80) CView.text("girl");
        else CView.text("herm");
        CView.text(".");

        CView.text("\n\nYou cradle ");
        if (roll < 40) CView.text("him");
        else CView.text("her");
        CView.text(" in your arms, uncaring of the stains as the fluid drenching its little body soaks into your [armor], gently soothing ");
        if (roll < 40) CView.text("him");
        else CView.text("her");
        CView.text(".  Then, seeing Ember is recovering from her recent exertions, you grin proudly at her and gently hold your ");
        if (roll < 40) CView.text("son");
        else CView.text("daughter");
        CView.text(" out for her to take.");
        ...
        if (emberChildren() > 1) CView.text("  Your other draconic offspring quietly peek into the lair, having vacated the place when Ember went into labor.  They smile at the sight of their new sibling, gently slipping in to curl up beside the mother and new child.");
        CView.text("  With a contented expression on your face, you leave Ember and your ");
        if (emberChildren() <= 1) CView.text("new child");
        else CView.text("newly expanded draconic brood");
        CView.text(" to get some rest, leaving the den.\n");
        if (roll < 40) EmberFlags.EMBER_CHILDREN_MALES++;
        else if (roll < 80) EmberFlags.EMBER_CHILDREN_FEMALES++;
        else EmberFlags.EMBER_CHILDREN_HERMS++;
    }
    // Ember Lays Egg
    else {
        ...
        CView.text(" as her womb works to expel its long-time occupant.  You place a hand on either side of the distended orb and begin to massage it, trying to help soothe the strained, aching muscles.  Your eyes go from her stomach to her crotch; her vagina is opened wide as the globular egg inside her begins dropping its way into the world.");
        if (emberHasCock()) {
            if (EmberFlags.EMBER_ROUNDFACE > 0 && EmberFlags.EMBER_INTERNAL_DICK === 0) CView.text("  Her human cock dangles heavily in front of her pussy, partially erect from the muscular spasms.");
            else CView.text("  Her draconic cock has been pushed from its internal sheathe by the pressure, but it's too painful for her to be erect.");
        }
        CView.text("\n\nMind racing, you bend your head in close and stick out your tongue, sliding it back in and out of your mouth to get it nice and wet before delivering a long, sloppy lick to her inner walls.  You can taste her, and the strange salty-blood taste of amniotic fluid mingles with her natural lubricants.  It's an unusual taste but not unbearable, and you begin to lick with greater fervor and purpose.  Your intention is to try and drown the pain of her contractions with pleasurable stimulus.  It's a crazy idea, but it makes sense in a place like Mareth.");
        ...
        if (EmberFlags.EMBER_EGGS > 0) {
            CView.text(", along with the ");
            if (EmberFlags.EMBER_EGGS === 1) CView.text("other");
            else CView.text(numToCardinalText(EmberFlags.EMBER_EGGS) + " others");
        }
        CView.text(", before telling her that she's beautiful too, and your daughters will certainly hatch and grow up to be just as gorgeous as she.");
        ...
        EmberFlags.EMBER_EGGS++;
    }
    player.effects.add(StatusEffectType.EmberNapping, 12, 0, 0, 0);
}

export function giveBirthToEmberKids(player: Character) {
    const roll: number = randInt(100);
    CView.text("\n");
    // PC Gives Live Birth
    if (EmberFlags.EMBER_OVIPOSITION === 0) {
        ...
        if (player.body.balls.count > 0) CView.text("slowly pushing your [balls]");
        else if (player.body.cocks.length > 0) CView.text("slowly pushing your " + describeCocksLight(player));
        if (player.body.balls.count > 0 || player.body.cocks.length > 0) CView.text(" out of " + emberMF("his", "her") + " way, ");
        CView.text("then " + emberMF("he", "she") + " blows softly on your contracting love-hole, slowly extending " + emberMF("his", "her") + " tongue to penetrate you.");
        ...
        if (player.perks.has(PerkType.Masochist)) CView.text("deliciously ");
        CView.text("on the boundary between pleasure and pain.  You aren't sure how much more you can take.");
        ...
        CView.text("\n\nEmber calmly smiles at you and points to your [chest].  You follow " + emberMF("his", "her") + " finger to see the little dragon nursing from your " + describeNipple(player, player.body.chest.get(0)) + ". \"<i>Sorry.  ");

        if (roll < 40) CView.text("He");
        else CView.text("She");
        CView.text(" was getting hungry and I didn't know what to do,</i>\" Ember explains.");
        if (EmberFlags.EMBER_MILK > 0) {
            CView.text("  \"<i>I tried to feed ");
            if (roll < 40) CView.text("him");
            else CView.text("her");
            CView.text(" myself, but ");
            if (roll < 40) CView.text("he");
            else CView.text("she");
            CView.text(" wanted yours...</i>\"");
        }

        CView.text("\n\nYou sigh softly, stroking your newborn's head even as it industriously sucks away at your " + describeNipple(player, player.body.chest.get(0)) + ".  Speaking of which, you ask Ember what you've had - a boy?  A girl?  Both?");
        ...
        CView.text(" face and starts to cry softly.  You gently help ");
        if (roll < 40) CView.text("him");
        else CView.text("her");
        CView.text(" up onto your shoulder and gently pat ");
        if (roll < 40) CView.text("him");
        else CView.text("her");
        CView.text(" on the back between ");
        if (roll < 40) CView.text("his");
        else CView.text("her");
        CView.text(" little wings, eliciting a burp that clearly leaves ");
        if (roll < 40) CView.text("him");
        else CView.text("her");
        CView.text(" feeling better.  ");
        if (roll < 40) CView.text("He");
        else CView.text("She");
        CView.text(" coos, giggles and nuzzles into your neck, clearly happy to be here in the real world at last.");
    ...
    // PC Lays Egg
        ...
        CView.text("\n\nYou're not certain how long you were sleeping for when you finally regain consciousness.  You wake, though, to the most wonderful sensations emanating from your " + describeNipple(player, player.body.chest.get(0)) + ", and the feel of soft hands caressing and squeezing your [chest].  You open your eyes and find Ember leaning over you, greedily nursing " + emberMF("himself", "herself") + " from your milk.  You can't resist asking what " + emberMF("he", "she") + "'s doing.");

        CView.text("\n\nEmber ");
        if (EmberFlags.EMBER_ROUNDFACE > 0) CView.text("blushes and ");
        CView.text("withdraws, licking " + emberMF("his", "her") + " lips of a stray droplet of milk.  \"<i>Sorry, it's just that you looked so full, and all that milk would've been wasted...  So, I thought I could help myself, not that I've been wanting to drink your milk or anything like that.</i>\"");
        ...
        if (EmberFlags.EMBER_EGGS > 0) CView.text("s hatch");
        else CView.text(" hatches");
        CView.text(", anyway.  The dragon ");
        if (EmberFlags.EMBER_ROUNDFACE > 0) CView.text("blushes and then ");
        CView.text("scurries away, even as you pull yourself upright and get ready to go about your business.\n");
        EmberFlags.EMBER_EGGS++;
    }
    player.effects.add(StatusEffectType.EmberNapping, 5, 0, 0, 0);
}
...
function highAffectionEmberLustFuck(player: Character): NextScreenChoices {
    CView.clear();
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(emberVaginalCapacity()));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    CView.text("You strip your [armor] and watch Ember as " + emberMF("he", "she") + " appraises your naked body.  You can see " + emberMF("his", "her"));
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) {
        CView.text(" dragon cock ");
        if (emberInternalDick()) CView.text("poking out of " + emberMF("his", "her") + " protective slit");
        else CView.text("growing erect");
    }
    if (EmberFlags.EMBER_GENDER === 3) CView.text(" and");
    if (EmberFlags.EMBER_GENDER >= 2) CView.text(" her pussy beginning to drip in excitement");
    CView.text(".");

    CView.text("\n\n\"<i>Mmm... now that's a view and a half; just look at you... what a gorgeous " + mf(player, "man", "girl") + ", with such ");
    if (player.body.cocks.length === 1) CView.text("a wonderful " + describeCock(player, player.body.cocks.get(0)));
    else CView.text(" wonderful cocks");
    CView.text("... and you're all mine, here and now,</i>\" " + emberMF("he", "she") + " croons appreciatively, giving you a lewd wink and flick of " + emberMF("his", "her") + " tongue.  \"<i>Still, while I'm enjoying the view, don't keep a " + emberMF("guy", "girl") + " in suspense; what do you have planned?</i>\" " + emberMF("he", "she") + " asks, tail flicking from side to side in an amused manner.");
    ...
    if (EmberFlags.TIMES_EMBER_LUSTY_FUCKED === 0) {
        ...
        CView.text("\"<i>All of them,</i>\" you state.");
        ...
        CView.text("\n\nYou nod");
        if (player.stats.lib < 50 && player.stats.minLust() < 40) {
            CView.text(", grabbing a vial of Lust Draft from your pouches");
            player.inventory.items.consumeItem(ConsumableName.LustDraft);
        }
        CView.text(".");
    }
    EmberFlags.TIMES_EMBER_LUSTY_FUCKED++;
    ...
    if (EmberFlags.EMBER_GENDER === 1) {
        ...
        CView.text("\n\nYou look into his eyes and smile, slowly pressing into his tight boy-pussy and spreading his cheeks with your girth.  Ember moans, arches his back and growls with delight, ass already clenching eagerly around your invading " + describeCock(player, cockThatFits) + ".  A few more humps and you feel yourself go as far inside his ass as possible.");
        ...
        CView.text("\n\nYour ");
        if (EmberFlags.EMBER_ROUNDFACE === 0) CView.text("she-dragon");
        else CView.text("dragon-girl");
        CView.text(" moans throatily and gives you a rapturous look.  \"<i>I love you, [name]... gods, I love you so much, and I'm so happy I can finally say that to your face,</i>\" she tells you.");
    ...
    // else if Ember has a pussy and the PC more than one cock:
        ...
        CView.text(" of those?  I-I know it's not exactly part of the pose and all, but...</i>\"");
        ...
        if (EmberFlags.EMBER_GENDER === 3) CView.text(", not to mention the stiffness of her dick");
        CView.text(", making it obvious she's ready.  \"<i>Oh, my mate... Please, fill me now!  Quit teasing me and just shove your cocks up your dragon's ready holes, jam them in as far as they can fit!</i>\" she pleads, but unable to meet your eyes in her embarrassment at her dirty talk.");

        CView.text("\n\nYou have the urge to do just that, but you'd also like to savor it.  You align your shafts with her ready holes and begin pushing in.  It's a strange, but pleasant, feeling.  Her ass tries to reject the intruding advances of your " + describeCock(player, cockThatFits) + ", while her pussy seems intent on pulling your other " + describeCock(player, cockThatFits) + " in.  It only takes a couple insistent humps before you pierce the barrier formed by her sphincter and penetrate her ass.");
        CView.text("\n\nEmber cries out, her whole body quivering in delight, both holes squeezing and clenching as they try to suck you inside");
        if (EmberFlags.EMBER_GENDER === 3) CView.text(", cock dribbling precum onto her belly");
        CView.text(".");

        CView.text("\n\nThe feeling of having ");
        if (player.body.cocks.length === 2) CView.text("both ");
        else CView.text("two of ");
        CView.text("your cocks enveloped in slick tightness and warmth is almost enough to make you fill her up with your seed right then and there, but somehow you manage to hold on.");

        CView.text("\n\nYour ");
        if (EmberFlags.EMBER_ROUNDFACE === 0) CView.text("she-dragon");
        else CView.text("dragon-girl");
        CView.text(" moans throatily and gives you a rapturous look.  \"<i>I love you, [name]... gods, I love you so much, and I'm so happy I can finally say that to your face,</i>\" she tells you.  Then she starts looking less in love and more in lust.  \"<i>But I also just love these cocks; these two wonderful breeding rods - come on, mate!  Breed your horny dragon!</i>\" she commands lustfully.");
    }
    ...
    if (EmberFlags.EMBER_GENDER >= 2 && player.body.cocks.length > 1) CView.text(" and ass");
    CView.text(" rippling and squeezing your intruding member");
    if (EmberFlags.EMBER_GENDER >= 2 && player.body.cocks.length > 1) CView.text("s");
    CView.text(".");
    ...
    if (EmberFlags.EMBER_GENDER >= 2) CView.text("vagina");
    else CView.text("ass");
    CView.text(" of " + emberMF("his", "hers") + " the pounding it deserves.");

    CView.text("\n\n\"<i>C-come on... cum into me, please!  Fill me with your seed!</i>\" Ember gasps, pleading with you.");

    CView.text("\n\nYou penetrate " + emberMF("him", "her") + " once, twice and finally hilt as much of your " + describeCock(player, cockThatFits) + " as you can inside " + emberMF("his", "her"));
    if (EmberFlags.EMBER_GENDER >= 2) {
        CView.text(" slick pussy");
        if (player.body.cocks.length > 1) CView.text(" and tight ass");
    }
    else CView.text(" tight ass");
    CView.text(" and blow your load.");

    CView.text("\n\nEmber howls exultantly as your steaming load gushes inside of " + emberMF("him", "her") + ".  " + emberMF("His", "Her") + " ");
    if (EmberFlags.EMBER_GENDER >= 2) {
        CView.text("cunt shudders, splashing femcum all over your intruding shaft");
        if (EmberFlags.EMBER_GENDER === 3) CView.text(", and her ");
        else CView.text(", ");
    }
    if (EmberFlags.EMBER_GENDER === 3 || EmberFlags.EMBER_GENDER === 1) {
        CView.text("cock erupts, spraying draconic cum up into the air to rain back down upon " + emberMF("his", "her") + " body, spattering " + emberMF("him", "her") + " in " + emberMF("his", "her") + " own juices and ");
    }
    CView.text("leaving " + emberMF("him", "her") + " gasping for breath as the climax ebbs away.");
    ...
    if (EmberFlags.EMBER_GENDER >= 2 && player.body.cocks.length > 1) CView.text("s");
    CView.text(", letting a satisfying stream of white mat the leaves below.  You smile at Ember, shaft");
    if (player.body.cocks.length > 1) CView.text("s");
    CView.text(" still flagging above " + emberMF("him", "her") + ".");
    ...
    if (EmberFlags.EMBER_GENDER === 1) CView.text("ass");
    else if (player.body.cocks.length === 1) CView.text("pussy");
    else CView.text("holes");
    CView.text(" with a squelch.  It feels so good... taking " + emberMF("him", "her") + " one time after the other.");
    ...
    if (EmberFlags.EMBER_GENDER >= 1 && player.body.cocks.length > 1) CView.text("s");
    CView.text(".  The dragon lets out an exultant cry as " + emberMF("his", "her") + " cum-slimed ");
    if (EmberFlags.EMBER_GENDER === 1) CView.text("ass squeezes");
    else if (player.body.cocks.length === 1) CView.text("pussy squeezes");
    else CView.text("holes squeeze");
    CView.text(" you, milking your ");
    if (EmberFlags.EMBER_GENDER === 1) CView.text("shaft as his cock spurts dragon-seed onto the leaves below him, filling the air with the scent of spunk and matting them into a steaming morass.");
    // {twin shafts} as {her cunt spasms wetly, drenching your lap with fresh femcum} {and/or} {[her] cock spurts dragon-seed onto the leaves below {her}, filling the air with the scent of spunk and matting them into a steaming morass}
        ...
        if (player.body.cocks.length === 1) CView.text("shaft as her cunt spasms wetly, drenching your lap with fresh femcum");
        else CView.text("twin shafts as her cunt spasms wetly, drenching your lap with fresh femcum");
        if (EmberFlags.EMBER_GENDER === 3) CView.text(" and her cock spurts dragon seed onto the leaves below her, filling the air with the scent of spunk and matting them into a steaming morass");
        CView.text(".");
    }
    CView.text("  Groaning and moaning like a whore, the dragon's wings beat, sending  cool air wafting over your overheating bodies before " + emberMF("he", "she") + " slumps onto the ground, barely able to hold " + emberMF("him", "her") + "self upright.  \"<i>...So good...</i>\"  Ember pants.  \"<i>I... uh... are you sure you want... round three?</i>\" " + emberMF("he", "she") + " asks, sounding a little tired as " + emberMF("he", "she") + " does, obviously not sure if you'll manage it.");
    ...
    // Else:
        ...
        if (EmberFlags.EMBER_GENDER >= 2) CView.text("her cunt starts to drip with moisture");
        if (EmberFlags.EMBER_GENDER === 3) CView.text(" and ");
        if (EmberFlags.EMBER_GENDER === 3 || EmberFlags.EMBER_GENDER === 1) CView.text(emberMF("his", "her") + " cock begins to grow erect again");
        CView.text(", amazement giving way to arousal.  " + emberMF("He", "She") + " licks " + emberMF("his", "her") + " lips with " + emberMF("his", "her") + " inhuman tongue, clearly eager to begin again.");
    }

    CView.text("\n\nEmber crawls over towards you, gripping the base of your shaft");
    if (player.body.cocks.length > 1 && EmberFlags.EMBER_GENDER >= 2) CView.text("s");
    CView.text(" tenderly yet firmly, stroking you slowly.  \"<i>I can't believe how hot I get when I see you sporting ");
    if (player.body.cocks.length === 1) CView.text("this");
    else CView.text("these");
    CView.text(".  It's just so... intoxicating... your scent, the way you look at me, everything really.</i>\"  You pat Ember's head");
    if (EmberFlags.EMBER_ROUNDFACE === 1 || EmberFlags.EMBER_HAIR > 0) {
        CView.text(", ruffling " + emberMF("his ", "her "));
        if (EmberFlags.EMBER_HAIR >= 2) CView.text("mane");
        else CView.text("hair");
    }
    CView.text(".  " + emberMF("He", "She") + " leans against your hand, hugging your midriff and letting your shaft");
    if (player.body.cocks.length > 1) CView.text("s");
    CView.text(" brush against " + emberMF("his", "her") + " cheek.");
    ...
    if (EmberFlags.EMBER_GENDER >= 0) CView.text("mingled ");
    ...
    if (EmberFlags.EMBER_GENDER === 1) CView.text("depths");
    else CView.text("nethers");
    if (EmberFlags.EMBER_GENDER >= 2 && player.body.cocks.length > 1) CView.text(" and depths");
    CView.text(".  \"<i>Ahn.  D-deeper...</i>\"  You hook your arm under " + emberMF("his", "her") + " knee and pull " + emberMF("his", "her") + " leg up, nearly throwing the dragon off balance.  \"<i>Ah!  D-do you like it when I let you take charge?  Ugh- oh!  Well, I think maybe I kind of like letting you be in charge, too...</i>\"  You barely pay attention to " + emberMF("his", "her") + " teasing remarks, instead focusing on exploring " + emberMF("his", "her") + " cummy ");
    if (EmberFlags.EMBER_GENDER === 1) CView.text("ass");
    else if (player.body.cocks.length === 1) CView.text("pussy");
    else CView.text("love-holes");
    CView.text(".  \"<i>Uhn... yes...  take me again.</i>\"  " + emberMF("He", "She") + " lets " + emberMF("his", "her") + " tongue loll out as " + emberMF("he", "she") + " pants in pleasure.");
    ...
    if (EmberFlags.EMBER_GENDER >= 2 && player.body.cocks.length > 1) CView.text("both of your dicks");
    else CView.text("your dick");
    if (EmberFlags.EMBER_GENDER >= 2) CView.text(", moisture drenching the cock buried in her cunt as it slobbers greedily across your burning flesh.");
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text("  " + emberMF("His", "Her") + " cock throbs and pulsates, ripples of arousal giving way to cumvein-bulging jets that shoot from " + emberMF("his", "her") + " prick to splatter wetly against the wall.");
    CView.text("  Wobbling unsteadily, the dragon becomes increasingly dependent on you to hold " + emberMF("him", "her") + " upright - a climax is coming, and it looks like it's going to be big...");
    ...
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) {
        CView.text(emberMF("his", "her") + " cock fountaining seed across the wall, practically whitewashing it in steaming hot dragon-spunk");
    }
    if (EmberFlags.EMBER_GENDER === 3) CView.text(" and ");
    if (EmberFlags.EMBER_GENDER >= 2) CView.text("her cunt gushing female ejaculate, almost soaking you with the cascade of feminine fluids and puddling wetly against your [feet]");
    CView.text(".");
    ...
    CView.text(", feeling the excess slide out around your cock");
    if (EmberFlags.EMBER_GENDER >= 2 && player.body.cocks.length > 1) CView.text("s");
    CView.text(".  \"<i>Ahh... more seed from my lover...</i>\" " + emberMF("he", "she") + " whispers, nearly passing out from the pleasure.  You feel yourself grow dizzy with pleasure and tumble onto the soft grass covering the den's floor, bringing Ember along.  Your dick");
    if (EmberFlags.EMBER_GENDER >= 2 && player.body.cocks.length > 1) CView.text("s");
    CView.text(" slide");
    if (player.body.cocks.length === 1) CView.text("s");
    CView.text(" out of " + emberMF("his", "her") + " with a wet slurp, allowing your deposit to leak its way under your prone forms.");
    ...
    return {
        choices: [
            ["Yes", stayWithEmberAfterLustFuck],
            ["No", noStayingForCuddlesPostLustFuck],
        ]
    };
}
...
function noStayingForCuddlesPostLustFuck(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You tell Ember that you can't stay, you have to get going now.  " + emberMF("He", "She") + " looks a bit disappointed, but forces " + emberMF("him", "her") + "self to smile all the same.  \"<i>I understand, you have other things to do... just know that I'll always be here for you, for better or worse.</i>\"  You ");
    // 50 or less Corruption:
    if (player.stats.cor < 50) CView.text("thank her for being so understanding");
    else CView.text("grunt an acknowledgement");
    CView.text(" and then gather your things before heading off to wash yourself down.");
    return { next: returnToCampUseOneHour };
}

// [=Yes=]
function stayWithEmberAfterLustFuck(player: Character): NextScreenChoices {
    CView.clear();
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(emberVaginalCapacity()));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    CView.text("With a smile, you tell " + emberMF("him", "her") + " that you'd be happy to.  \"<i>Great, come here...</i>\" " + emberMF("he", "she") + " croons, scooting over towards you.  You open your arms and allow the dragon to snuggle up against you, folding your arms comfortably under " + emberMF("his", "her") + " wings.");
    ...
    // else if Ember is female:
        ...
        CView.text("\n\nShe breaks the kiss and lightly strokes your shaft, smiling at you.  Then she aligns the tip of your " + describeCock(player, cockThatFits) + " with her pussy.");

        return { next: partial(penetrateWithEmber, player, true) };
    }
        ...
        if (EmberFlags.EMBER_ROUNDFACE > 0) CView.text("-girl");
        CView.text(" states, idly rubbing your cocks together.  One of your hands slips down between the two of you, sliding over the ridged surface of the herm dragon's cock before pressing itself against her well-used quim; sure enough, it's already drooling eagerly at the prospect of being filled again.");
        ...
        return {
            choices: [
                ["Frotting", frottingWithFrottingEmberHerm],
                ["Penetrate", penetrateEmberHerm],
            ]
        };
    }
}
...
// [=Frotting=]
function frottingWithFrottingEmberHerm(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You think the matter over, and then slowly rub your [cock biggest] against Ember's to answer her question.  The dragon-herm gasps, then smiles lewdly at you.");
    return frottingWithEmber(player, false);
}

// [=Penetrate=]
function penetrateEmberHerm(player: Character): NextScreenChoices {
    CView.clear();
    ...
    return penetrateWithEmber(player, false);
}

// Frotting:
// clear = true
function frottingWithEmber(player: Character, clear: boolean): NextScreenChoices {
    const largestCock = player.body.cocks.sort(Cock.Largest)[0];
    if (clear) CView.clear();
    else CView.text("\n\n");
    CView.text("Ember thrusts against your shaft; the ridges of " + emberMF("his", "her") + " dick stimulate your " + describeCock(player, largestCock) + " and you moan at the feeling.  \"<i>Come on, [name].  Are you going to make me do all the work?</i>\" " + emberMF("he", "she") + " teases you.  You slowly stroke your shaft against " + emberMF("his", "hers") + ", asking just what " + emberMF("he", "she") + " has in mind; wasn't " + emberMF("he", "she") + " planning on taking a breather?");
    ...
    if (EmberFlags.EMBER_ROUNDFACE > 0) CView.text(emberMF("-boy", "-girl"));
    CView.text("'s overly long tongue sticks out as pleasure overrides " + emberMF("his", "her") + " senses.  You groan as you slide your cock against Ember's, the thought occurring to you that this is going to be pretty messy when the inevitable comes, but you are too overwhelmed to actually say so.");
    ...
    if (EmberFlags.EMBER_GENDER >= 2) CView.text("  Her moist dragon pussy is not helping the mess in the least.  You can smell her femcum pooling along with her pre.");
    ...
    if (EmberFlags.EMBER_GENDER >= 2) CView.text("  Her other hand frigs her pussy as she comes down from her orgasm, gushing female juices and leaking some of the cum you've pumped into her earlier.");

    CView.text("\n\nThe dragon");
    if (EmberFlags.EMBER_ROUNDFACE > 0) CView.text(emberMF("-boy", "-girl"));
    CView.text(" uses what strength " + emberMF("he", "she") + " still has to crawl over to you and slide back between your arms.  \"<i>Sorry, just had to get that last bit out of my system.</i>\"  " + emberMF("He", "She") + " yawns lowdly.  \"<i>[name]?  How about a quick nap?</i>\"  " + emberMF("He", "She") + " asks not even bothering to hear your reply before exhaustion gets " + emberMF("his", "her") + " and " + emberMF("he", "she") + " falls asleep, snoring lightly.  You smile at " + emberMF("him", "her") + " and stroke " + emberMF("his", "her"));
    if (EmberFlags.EMBER_HAIR === 0 && EmberFlags.EMBER_ROUNDFACE === 0) CView.text(" head");
    else if (EmberFlags.EMBER_HAIR === 2) CView.text(" mane");
    else CView.text(" hair");
    CView.text(" before allowing yourself to also fall asleep.");
    player.orgasm();
    // return { next: 14 };

    return { next: emberJizzbangbangEnding };
}

// Penetrate:
function penetrateWithEmber(player: Character, clear: boolean = true): NextScreenChoices {
    if (clear) CView.clear();
    else CView.text("\n\n");
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(emberVaginalCapacity()));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    CView.text("\"<i>Go on.</i>\"  She moves her arms around you and into a hug.  \"<i>Enter me.</i>\"  With no further prelude needed, you slide yourself into the damp interior of her cunt, the organ eagerly accepting you back for the fourth time.");
    ...
    if (EmberFlags.EMBER_MILK > 0) CView.text(", milk seeping across your fingers,");
    CView.text(" and promise you will, sliding slowly in until you have hilted yourself yet again.");

    CView.text("\n\nThe dragon");
    if (EmberFlags.EMBER_ROUNDFACE > 0) CView.text("-girl");
    CView.text(" moans at your ministrations");
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) {
        CView.text(", her draconic prick pressing tightly against you as it leaks a steady stream of pre, matting ");
        if (player.body.chest.sort(BreastRow.Largest)[0].rating > 1) CView.text("your [chest], ");
        CView.text("your belly");
        if (player.body.chest.sort(BreastRow.Largest)[0].rating > 1) CView.text(",");
        CView.text(" and Ember's own.");
    }
    CView.text("  \"<i>Do you like playing with my body?</i>\"  Ember asks, gazing at you through half-lidded eyes.  You give her a playful smirk and run your fingers expertly over her nipples, making her moan and mewl at the stimulation and saying more than mere words could on the matter.");
    ...
    if (player.body.tails.length > 0) CView.text("the base of your tail, tugging lightly on it and stroking it for a moment, then her hands move on to ");
    CView.text("your [butt], grabbing the ");
    ...
    if (player.body.wings.type > WingType.NONE) CView.text("stopping momentarily to stroke along your " + player.body.wings.desc + " wings before continuing up and ");
    CView.text("stopping at the back of your head.");

    CView.text("\n\nSlowly she guides you towards one of her erect nipples.  \"<i>");
    if (EmberFlags.EMBER_MILK > 0) CView.text("Drink my milk, I've been saving it up specially for you,");
    else CView.text("I may not have any milk in my breasts right now, but maybe if you keep suckling I'll be able to make you some,");
    CView.text("</i>\" Ember teases you.  You smile at her and accept the nipple, rolling it between your lips and exerting gentle, teasing pressure with your teeth.");
    if (EmberFlags.EMBER_MILK > 0) CView.text("  You savor the resultant gush of sweet dragon's milk as it squirts obediently down your throat.");
    ...
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) CView.text("  A small dollop of pre escapes Ember's dragon-dick at the pleasure of the movement.");
    CView.text("  \"<i>And don't forget about my other breast either.</i>\"  She takes your hand in her own and guide it to her other mound, helping you knead it.");
    ...
    if (EmberFlags.EMBER_MILK > 0) CView.text("allowing milk to spill into your mouth and gulp down sweet load after load, ");
    CView.text("listening to her moan softly in pleasure from your actions, feeling her netherlips rippling around your " + describeCock(player, cockThatFits) + ".  Your nursing is suddenly interrupted when you're pulled off Ember's breasts and into a deep kiss.  The dragon-girl shoves her tongue inside your mouth, exploring it in detail");
    if (EmberFlags.EMBER_MILK > 0) CView.text(", not even caring that she's tasting her own milk as you finish gulping it down");
    CView.text(".  You kiss her back as eagerly as you can until lack of air forces you to break the kiss and catch your breath.");
    ...
    if (EmberFlags.EMBER_ROUNDFACE > 0) CView.text("-girl");
    CView.text(" roaring towards the ceiling, then turning to look at you as her pussy constricts and milks you for all you're worth.  Already spent from your past three orgasms, you just sit back and enjoy yourself being milked by her strong vaginal muscles one last time.");
    ...
    CView.text(".");
    if (EmberFlags.EMBER_GENDER === 3) CView.text("  Her draconic cock throbs all the way through your orgasm, shooting blanks a few times before spurting a couple ropes of pre onto her belly.");
...
function emberJizzbangbangEnding(player: Character): NextScreenChoices {
    ...
    CView.text(", letting you see the whiteness on " + emberMF("his", "her") + " tongue, before tipping " + emberMF("his", "her") + " head back and gulping it down, licking " + emberMF("his", "her") + " lips and moaning as if " + emberMF("he", "she") + " was tasting a fine wine.  \"<i>Thanks for the snack, my mate.</i>\" " + emberMF("He", "She") + " croons at you, a teasing expression on " + emberMF("his", "her") + " face.");
    ...
    if (EmberFlags.EMBER_GENDER === 1 || EmberFlags.EMBER_GENDER === 3) {
        ...
        if (EmberFlags.EMBER_ROUNDFACE === 0) CView.text("anthro");
        else CView.text("monster-" + emberMF("boy", "girl"));
        CView.text(" to come clean.  \"<i>Cut me some slack... I'm not made of stone, you know.  I was just giving my mate a blowjob, you can't seriously expect me not to react to that...</i>\"  " + emberMF("He", "She") + " crosses " + emberMF("his", "her") + " arms and blows a puff of smoke as " + emberMF("he", "she") + " looks away, obviously flustered.");
