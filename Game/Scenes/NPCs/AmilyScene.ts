...
export class AmilyScene extends NPCAwareContent implements TimeAwareInterface {
	...
	*/
	...
	public exploreVillageRuin() {
		...
		if (randInt(5) === 0 && (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00254] === 0 || Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00255] === 0)) {
			let rack: number = 0;
			//Already got weapon
			...
			DisplayText("While picking through the ruined houses and abandoned structures of this dilapidated village, you manage to find something useful!  There's an intact but empty ");
			if (rack === 0) DisplayText("armor");
			else DisplayText("weapon");
			DisplayText(" rack here.  It looks like it could hold nine different ");
			if (rack === 0) DisplayText("armors");
			else DisplayText("weapons");
			DisplayText(".  You check it over and spot an easy way to fold it up for transport.  This would be a fine addition to your camp, so you pack it up and haul it back.");
			if (rack === 1) {
		...
		//Schrödinger Amily corrupted that damn place!
			...
			amilySprite();
			DisplayText("You enter the ruined village, still laughing at your past nefarious deeds. Maybe it's just your imagination, but you feel like this entire place reeks of corruption now... You explore for an hour, then go back to your camp, knowing your tainted slave will be more than happy to satisfy your urges.");
			return { next: Scenes.camp.returnToCampUseOneHour };
			return;
		}
		//Remove worm block if player got rid of worms.
		if (Flags.list[FlagEnum.AMILY_GROSSED_OUT_BY_WORMS] === 1) {
			if (!player.statusAffects.has(StatusAffectType.Infested)) Flags.list[FlagEnum.AMILY_GROSSED_OUT_BY_WORMS] = 0;
		}
		...
		if (Flags.list[FlagEnum.AMILY_PC_GENDER] === player.gender) {
			...
			if (player.gender === Gender.MALE) {
				...
				if (Flags.list[FlagEnum.AMILY_AFFECTION] >= 50 && Flags.list[FlagEnum.AMILY_FUCK_COUNTER] === 0 && Flags.list[FlagEnum.AMILY_PC_GENDER] === 1) {
					DisplayText("Wandering into the ruined village, you set off in search of Amily.\n\n");
					/*NOPE!
					[Player meets the requirements to stalk Amily]
					if(player.stats.spe > 50 && player.stats.int > 40) {
						DisplayText("Using all of your knowledge, skill and cunning, you sneak and squirm through the ruins until you finally find yourself coming up right behind the dusty mouse girl. She's picking berries off of a small bush and hasn't noticed you yet.\n\n");
						DisplayText("How do you approach her?");
					...
					DisplayText("After wondering for a while how on earth you are going to track down Amily, you hear a whistle. Looking around, you see her waving cheekily at you from around a corner; it's pretty obvious that you have a long way to go before you'll be able to beat her at this kind of game.\n\n");

					DisplayText("\"<i>Ah... do you have the time to talk? There's something I want to get off my chest,</i>\" Amily nervously asks.\n\n");
				...
				if (Flags.list[FlagEnum.AMILY_MET] === 0) {
					...
					DisplayText("\"<i>Don't make any sudden moves!</i>\" A voice calls out, high pitched and a little squeaky, but firm and commanding. You freeze to avoid giving your assailant a reason to shoot at you again. \"<i>Stand up and turn around, slowly,</i>\" it commands again. You do as you are told.\n\n");
					...
					if (player.humanScore() > 4) DisplayText("Are you ");
					else DisplayText("Were you ");
					DisplayText("one of those... humans, I've heard sometimes wander into this world?</i>\"\n\n");

					DisplayText("You admit that, yes, you are a human, and then ask her why she remains here in this empty wasteland of a settlement.\n\n");
					...
					DisplayText("She stares at you intently, and you ask her what the matter is.\n\n");
					...
					MainScreen.simpleChoices(["AcceptEagerly", "Hesitantly", "NoFurries", "Refuse", ""], [acceptAmilysOfferEagerly, acceptAmilyOfferHesitantly, amilyNoFur, refuseAmilysOffer, null]);
					//Set flag for 'last gender met as'
					Flags.list[FlagEnum.AMILY_PC_GENDER] = player.gender;
					return;
				}
				//[Remeeting if previously refused]
				else if (player.gender === Gender.MALE && Flags.list[FlagEnum.AMILY_OFFER_ACCEPTED] === 0) {
					DisplayText("Wandering into the ruined village, you set off in search of Amily.\n\n");
					/*NOPE!
					//[Player meets the requirements to stalk Amily]
					if(player.stats.spe > 50 && player.stats.int > 40) {
			...
			//GIRL MEETINZ
				...
				if (Flags.list[FlagEnum.AMILY_MET] === 0) {
					...
					DisplayText("She shakes her head and smiles at you wistfully. \"<i>Listen to me, rambling. I'm sorry again for attacking you. But, take care out there; there's a lot of freaky monsters that will do the most unspeakable things to a woman if they can catch her.</i>\"\n\n");

					DisplayText("You thank her, and she brushes it off.\n\n");

					DisplayText("\"<i>Hey, us girls gotta stick together, right?</i>\" She winks at you then wanders off behind a partially collapsed wall, disappearing into the rubble.");
					//Set flag for 'last gender met as'
					Flags.list[FlagEnum.AMILY_PC_GENDER] = player.gender;
					return { next: Scenes.camp.returnToCampUseOneHour };
					return;
			...
			//Herm Meetinz
				...
				if (Flags.list[FlagEnum.AMILY_MET] === 0) {
					...
					DisplayText("She looks thoughtful. \"<i>You know...</i>\" She begins, but stops and ");
					//[If breasts are flat, manly breasts]
					if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < 1) DisplayText("sniffs the air intensely, her whiskers quivering. ");
					//[If breasts are A-cup or bigger]
					else DisplayText("stares at the bulge in your top, as well as the bulge in your bottom.  ");
					DisplayText("\"<i>Never mind,</i>\" she says after a moment. \"<i>You're a hermaphrodite, aren't you? Forget I mentioned it.</i>\"\n\n");

					DisplayText("She turns and walks away, vanishing into the dust and the rubble like magic.");
					//Set flag for 'last gender met as'
					Flags.list[FlagEnum.AMILY_PC_GENDER] = player.gender;
					return { next: Scenes.camp.returnToCampUseOneHour };
					return;
				...
				if (((Flags.list[FlagEnum.AMILY_AFFECTION] >= 15 && randInt(3) === 0) || Flags.list[FlagEnum.AMILY_AFFECTION] >= 20) && Flags.list[FlagEnum.AMILY_HERM_QUEST] === 0) {
					whyNotHerms();
					return;
				}
			...
			//Genderless
				...
				if (Flags.list[FlagEnum.AMILY_MET] === 0) {
					...
					if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < 1) {
						...
						DisplayText("Sheepishly, you look down at the ground and confess that as much as you might like to help, that's actually impossible.\n\n");

						DisplayText("Amily looks hurt. \"<i>Why?</i>\" she demands desperately.\n\n");

						DisplayText("Highly embarrassed but unable to think of a way to articulate it, you drop your pants and let her see the flat and featureless expanse of flesh that is your crotch.\n\n");
						...
						DisplayText("She stops, however, just before rounding a wall. \"<i>There's this stuff you'll find in bottles called Incubus Draft. If you drink that, it'll make you a boy - but I'd find an alchemist first, so he can remove the corruption from it.</i>\"\n\n");

						DisplayText("She continues walking away. After she has vanished, though, another musing drifts back to you. \"<i>There's also this stuff called Succubus Milk you can do the same thing with, if you want to be a girl.</i>\"\n\n");
					}
					//(If breasts > A-Cup)
					else {
						DisplayText("She shakes her head and smiles at you wistfully. \"<i>Listen to me, rambling. I'm sorry again for attacking you. But, take care out there; there's a lot of freaky monsters that will do the most unspeakable things to a woman if they can catch her.</i>\"\n\n");

						DisplayText("Blushing, you explain to her that you aren't actually a woman. She looks very puzzled at this.\n\n");

						DisplayText("\"<i>But you have boobs... and I don't see a crotch-bulge,</i>\" she says, sounding almost petulant. \"<i>I don't smell a vagina, either... Wait, are you telling me you don't have either pair of genitals?</i>\" she asks, clearly dumbfounded.\n\n");

						DisplayText("Embarrassed, you admit that is so.\n\n");
						...
						DisplayText("Having evidently regained her confidence, she winks and then vanishes behind a tumbled-down wall, leaving you alone.");
					}
					//Set flag for 'last gender met as'
					Flags.list[FlagEnum.AMILY_PC_GENDER] = player.gender;
					return { next: Scenes.camp.returnToCampUseOneHour };
		...
		if(player.stats.spe > 50 && player.stats.int > 40 && randInt(4) === 0) {
			DisplayText("Deciding to find Amily first instead of waiting for her to find you, you set off into the ruins. Using all of your knowledge, skill and cunning to figure out where she is likely to be, you make your way there without giving yourself away.\n\n");
			//[Amily is not pregnant]
			if(Flags.list[FlagEnum.AMILY_INCUBATION] === 0) {
				DisplayText("Finally, you find her squatting down in front of a small bush. She's industriously picking it clean of berries, gulping down almost as many as she puts into a small sack at her side.\n\n");
			}
			//[Amily is slightly pregnant]
			else if(Flags.list[FlagEnum.AMILY_INCUBATION] >= 90) {
				DisplayText("Finally, you find her rummaging through the contents of a home that has been torn open. She appears to be looking for as many old strips of cloth as she can find.\n\n");
			}
			//[Amily is heavily pregnant]
			else {
				DisplayText("Finally, you find her emerging from a turn-off, pulling up her pants and muttering to herself about her bladder.\n\n");
			}
			DisplayText("How do you approach her?");
			//Announce yourself / Scare her
		...
		if (Flags.list[FlagEnum.AMILY_PC_GENDER] != player.gender) {
			...
			//|| (player.gender === Gender.NONE && Flags.list[FlagEnum.AMILY_AFFECTION] < 15)) {
		...
		switch (pregnancy.event) {
			...
			case 5: //Amily is slightly pregnant
				...
				//[Low Affection]
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15 || player.gender === Gender.NONE) DisplayText("\"<i>Well, I guess despite whatever other faults you may have, you can get the job done.</i>\" She says, not looking directly at you.\n\n");
				//[Medium Affection]
				else if (Flags.list[FlagEnum.AMILY_AFFECTION] < 40) DisplayText("\"<i>Thank you. With your help, my people will soon live again.</i>\" She strokes her belly, grinning happily. \"<i>Is there something you want to talk about?</i>\"\n\n");
				//[High Affection]
				else DisplayText("\"<i>Thank you, thank you! I couldn't have done this without you!</i>\" She exclaims. \"<i>You've done a wonderful, noble thing, and I'm glad I found you to be their father. So, not that it isn't great to see you again, but why did you come to visit?</i>\"\n\n");
				break;
			case 6:
			case 7:
			case 8: //Amily is heavily pregnant
				...
				//[Low Affection]
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15 || player.gender === Gender.NONE) DisplayText("She seems to be paying more attention to her gravid midriff than to you, and it's several long moments before she finally speaks. \"<i>These children will be born soon. I guess I owe you my thanks for being willing to father them.</i>\"\n\n");
				//[Medium Affection]
				else if (Flags.list[FlagEnum.AMILY_AFFECTION] < 40) DisplayText("She groans softly. \"<i>This isn't an easy task, you know. But I still want to thank you. Maybe, when these ones are born, you'll be willing to help me make some more?</i>\" She asks, her tail gently waving behind her.\n\n");
				//[High Affection]
				else DisplayText("\"<i>I should have known you were coming; they always start kicking up a storm when you're here – did you know that?</i>\" She smiles beatifically. \"<i>They know their daddy already, they do. With your help, a new generation of my people will have a chance to grow up free from the taint of demons. Was there something on your mind?</i>\"\n\n");
				break;
			default: //Amily is not pregnant
				DisplayText("It doesn't take long for Amily to materialize out of the ruins. Her blowpipe and dagger are both thrust into her belt, and she's still wearing the same tattered clothes as before.\n\n");
				//[Low Affection]
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15 || player.gender === Gender.NONE) {
					if (Flags.list[FlagEnum.AMILY_MET_AS] === 2 && player.gender === Gender.FEMALE) DisplayText("She crosses her arms and smiles at you. \"<i>So you came back huh?  Did you want to chat with little old me?</i>\" she asks.\n\n");
					else DisplayText("She crosses her arms and taps her fingers on her shoulder. \"<i>So, why are you here? What do you want?</i>\" she asks.\n\n");
				}
				//[Medium Affection]
				else if (Flags.list[FlagEnum.AMILY_AFFECTION] < 40) {
					DisplayText("She smiles softly upon seeing you. \"<i>It's always good to see somebody else who hasn't given in to corruption. Did you have something on your mind?</i>\"\n\n");
				}
				//[High Affection]
				else {
					DisplayText("She grins at you with open delight. \"<i>Hey there, " + player.short + "! It's great to see you again... ");
					if (player.torso.cocks.count > 0) {
						DisplayText("Have you come to knock me up?");
						if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0 && player.pregnancyIncubation === 0) DisplayText(" Or have you come to get knocked up?");
					}
					else if (player.torso.vaginas.count > 0) {
						if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0 && player.pregnancyIncubation === 0) DisplayText("Have you come back so I could stuff another bun in your oven?");
						else DisplayText("Did you come back for a little 'quality time' with me?");
					}
					DisplayText("</i>\" she teases, but her body language ");
					if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) {
						DisplayText("and the erection tenting her pants ");
						player.stats.lust += 5;
					}
					DisplayText("suggests that it's no joking matter.\n\n");
				}
		}
		...
		if (player.inventory.items.has(consumables.P_DRAFT) && Flags.list[FlagEnum.AMILY_WANG_LENGTH] === 0 && Flags.list[FlagEnum.AMILY_HERM_QUEST] === 2 && Flags.list[FlagEnum.AMILY_AFFECTION] >= 40 && player.gender === Gender.HERM) {
			...
			DisplayText("You could probably bring up the efficiency of having two hermaphrodite mothers, particularly since you have this purified incubi draft handy.\n\n");
		}

		let sex: Function = determineAmilySexEvent();
		simpleChoices("Sex", sex, "Talk", talkToAmily, "Both", (sex === null ? null : talkThenSexWithAmily), "Efficiency", efficiency, "Leave", Scenes.camp.returnToCampUseOneHour);
		//Set flag for 'last gender met as'
		Flags.list[FlagEnum.AMILY_PC_GENDER] = player.gender;
	...
	private determineAmilySexEvent(forced: boolean = false): Function {
		...
		if (player.gender === Gender.HERM && Flags.list[FlagEnum.AMILY_HERM_QUEST] === 2) {
			//Amily is herm too!
			if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) {
	...
	private acceptAmilysOfferEagerly() {
		...
		DisplayText("The mouse-woman looks appalled. \"<i>Must you be so vulgar?</i>\" she complains.\n\n");

		DisplayText("You tell her that this is precisely what she's asking you to do, and you'll be happy to do so if that is what she wants.\n\n");
		...
		DisplayText("Your brash enthusiasm has made Amily uncomfortable, and your quick surrender to baser impulses has made you slightly more lustful.");
		//Offer accepted
	...
	private acceptAmilyOfferHesitantly() {
		...
		DisplayText("Softly, you ask if she really does want you to mate with her, to father her offspring.\n\n");

		DisplayText("\"<i>Yes. You're the best hope I have... the only hope I have.</i>\" She replies, sadly.\n\n");

		DisplayText("You bow your head and tell her that if she really does need your help, you will help her – even if it does mean doing things with her that she doesn't want.\n\n");

		DisplayText("She blinks at you, clearly surprised. \"<i>I've never met a male who actually cared if a female wanted sex or not...</i>\" She then smiles gently. \"<i>It's nice to meet somebody who can still care about people as something other than fuck toys. Please, come with me.</i>\"\n\n");

		DisplayText("She eagerly leads you down a path, her tail swishing back and forth energetically. She seems very happy by your acceptance.\n\n");

		DisplayText("It seems you've made Amily happy by asking if this is what she wants.");
		//Offer accepted
		Flags.list[FlagEnum.AMILY_OFFER_ACCEPTED] = 1;
		//{+5 Affection}
		Flags.list[FlagEnum.AMILY_AFFECTION] += 5;
	...
	private refuseAmilysOffer() {
		...
		DisplayText("Amily stares at you in disbelief. \"<i>No? What do you mean, no? I'm honestly offering to have sex with you here.</i>\"\n\n");
		...
		DisplayText("Amily is wide-eyed when you finish. \"<i>I have... I haven't heard anybody say things like that, think like that, in a long time.</i>\" She smiles, faintly, then fiercely shakes her head. \"<i>I really do need your help... but I can only respect your conviction. I do hope that we can come to terms later, though.</i>\"\n\n");

		DisplayText("She gives you a bow and then leaves, giving you the chance to turn around and leave this ruined village yourself.\n\n");

		DisplayText("You have impressed Amily considerably, and reigning in your sexual impulses has helped to calm your libido.\n\n");
		//{+10 Affection}
	...
	private amilyRemeetingContinued() {
		...
		simpleChoices("Accept", secondTimeAmilyOfferedAccepted, "RefusePolite", secondTimeAmilyRefuseAgain, "Just Talk", repeatAmilyTalk, "Get Lost", tellAmilyToGetLost, "Leave", Scenes.camp.returnToCampUseOneHour);
	}
	...
	private secondTimeAmilyRefuseAgain() {
		...
		DisplayText("\"<i>All right; it is your choice. But my offer still stands, you know,</i>\" she tells you.\n\n");

		DisplayText("You let her know you'll remember that, and then turn and leave.");
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	private repeatAmilyTalk() {
		...
		DisplayText("\"<i>Just to talk?</i>\" Amily asks, and then adds quietly, \"<i>Well... it has been a long time since I actually had somebody to talk to...</i>\" She looks distracted for a moment, but then she smiles. Clearly, Amily is pleased with the prospect. \"<i>So, is there anything in particular you want to talk about?</i>\"\n\n");
		...
		switch (pregnancy.event) {
			...
			case 5: //Amily is slightly pregnant
				...
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) {
					DisplayText("She stares at you, puzzled. \"<i>Why? I'm already pregnant,</i>\" she tells you. \"<i>...Forget it. You can have sex when I need to get pregnant again. Go find a goblin if you want to fuck some brainless baby-stuffed whore!</i>\"\n\n");
					DisplayText("Amily can still move quickly despite her pregnancy, and you are very promptly left all alone. Perhaps it would be better not to broach the subject that bluntly with her while she's in this state.\n\n");
				...
				//[Medium Affection]
					...
					DisplayText("She is clearly surprised, putting a hand to her swelling midriff. But then she shrugs and says, \"<i>Well, I guess I do owe you that much for helping me.</i>\"\n\n");
					DisplayText("Though she does set off and indicate for you to follow, you realize that she's not too happy about your reason for being here.\n\n");
					//[/ Go to [Medium Affection Sex]]
					return { next: amilySexHappens };
				}
				//[High Affection]
				else {
					DisplayText("\"<i>You still want me, even though I'm already pregnant?</i>\" she asks – not angry or disappointed, but sounding rather pleased. \"<i>Well, how can I say no to you?</i>\" She smiles broadly and begins to walk away, doing her best to give you a sexy wiggle of her hips as an invitation for you to follow her.\n\n");
			...
			case 8: //Amily is heavily pregnant
				...
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) {
					...
					DisplayText("Annoyed, she turns and waddles off. You do not give chase; you can tell that you've offended her.\n\n");
					//Reduce affection
					Flags.list[FlagEnum.AMILY_AFFECTION] -= 3;
					return { next: Scenes.camp.returnToCampUseOneHour };
				}
				//[Medium Affection]
					...
					DisplayText("She boggles as if she can't believe you. \"<i>You can't be that desperate you'd want somebody as fat and knocked up as I am!</i>\" she protests.\n\n");
					DisplayText("You insist to her that you're not joking – you really do think she's sexy enough to make love to.\n\n");
					DisplayText("\"<i>...Well, I guess I'm flattered, but... do you have the faintest idea how to make love to a woman who is pregnant? Especially one as far along as I am?</i>\"\n\n");
					DisplayText("You are forced to concede that, actually, you don't.\n\n");
					DisplayText("\"<i>It's not that I don't like you, " + player.short + ", it's just... well, I don't feel comfortable doing that,</i>\" she explains apologetically.\n\n");
					DisplayText("You apologize back for confronting her with something she's uncomfortable with, and leave for your own camp, lest you insult her seriously.");
					Flags.list[FlagEnum.AMILY_AFFECTION] -= 3;
					return { next: Scenes.camp.returnToCampUseOneHour };
				}
				//[High Affection]
				else {
					DisplayText("She looks a little puzzled by the request, but then smiles with sincere pleasure. \"<i>I'm game if you are, dear.</i>\" She winks and offers her hand to you. You take it, and let her lead you to her chosen nesting site.\n\n");
			...
			default: //Amily is not pregnant
				//[Low Affection]
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) {
					DisplayText("\"<i>Of course you did. Well, come on, I guess I can oblige you. It's the only way I'm going to get pregnant.</i>\"\n\n");
					DisplayText("She sets off, clearly leading the way as you follow her.\n\n");
					//[/ Go to [Low Affection Sex]]
					return { next: amilySexHappens };
				}
				//[Medium Affection]
				else if (Flags.list[FlagEnum.AMILY_AFFECTION] < 40) {
					DisplayText("\"<i>Well, I guess you'll do. I mean, I still need to get pregnant,</i>\" she teases you, tail waving merrily. \"<i>Follow me.</i>\"\n\n");
					DisplayText("You have to push yourself to keep up with her, but she's clearly just playing with you by moving so quickly rather than seriously trying to escape you.\n\n");
					//[/ Go to [Low Affection Sex]]
					return { next: amilySexHappens };
				}
				//[High Affection]
				else {
					DisplayText("Amily doesn't bother to say anything; she just grins like the cat that ate the canary (well, the mouse that ate the cheesecake, anyway). She grabs hold of your hand and does her best to pull you as fast as she can towards her closest bolt-hole.\n\n");
					//[/ Go to [Low Affection Sex]]
					return { next: amilySexHappens };
				}
		}
	...
	private talkToAmily() {
		...
		else DisplayText("You tell Amily that you came here because you wanted to talk with her, and you have no desire to approach her sexually on this encounter.\n\n");
		...
		if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) {
			switch (pregnancy.event) {
				...
				case 4:
				case 5: //Amily is slightly pregnant
					DisplayText("\"<i>I could use someone to talk to, I suppose,</i>\" she says plainly, but you can clearly see that she's very happy that's what you want to do.\n\n");
					break;
				case 6:
				case 7:
				case 8: //Amily is heavily pregnant
					DisplayText("\"<i>Oh, NOW you want to get to know me,</i>\" she complains, but her tone is gentle – amused even, and she clearly isn't as unhappy as her words may imply. Heavily, she sits herself down unceremoniously. \"<i>But... there are things weighing on my mind. I really could use somebody to talk to.</i>\"\n\n");
					break;
				default: //Amily is not pregnant
					if (Flags.list[FlagEnum.AMILY_MET_AS] === 2 && player.gender === Gender.FEMALE) DisplayText("\"<i>A chat would be lovely,</i>\" she says, clearly enjoying herself.  \"<i>I... I hardly ever get a chance to find someone to chat with.  Sometimes it seems like everyone in Mareth just wants to breed non-stop...</i>\" she murmers to herself.  \"<i>Well, what shall we talk about?</i>\" she asks, seemingly quite happy with your presence.\n\n");
					else DisplayText("\"<i>You want to talk? No sex?</i>\" she asks, clearly having a hard time believing it. \"<i>I... I haven't had the chance to talk to anyone in years. It's been so long...</i>\" she murmurs to herself, and you think you see the start of a tear glinting in her eye. \"<i>Well, what do you want to talk about?</i>\" she asks, seemingly quite happy that's what you're here for.\n\n");
			}
		}
		//[Medium Affection]
		else if (Flags.list[FlagEnum.AMILY_AFFECTION] < 40) {
			DisplayText("\"<i>Of course, " + player.short + ", I always enjoy our talks.  What shall we discuss this time?</i>\" she asks happily.\n\n");
		}
		//[High Affection]
		else {
			DisplayText("She smiles playfully at you. \"<i>And here I was thinking we knew each other already.  But if you want, I'm always happy to talk.</i>\"\n\n");
		}
		//[/ Go to random [Conversation]]
		return { next: talkWithCuntIMeanAmily };
	}
	...
	private talkThenSexWithAmily() {
		...
		switch (pregnancy.event) {
			...
			case 5: //Amily is slightly pregnant
				//[Low Affection]
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) {
					DisplayText("She rubs her belly thoughtfully. \"<i>I guess a bit of conversation would be nice, after all this time. Sex, though? Maybe if you're lucky.</i>\" She's already heading off, encouraging you to follow her.\n\n");
					//[/ Go to random [Conversation], then small chance of [Low Affection Sex]]
					return { next: talkWithCuntIMeanAmily };
				}
				//[Medium Affection]
				else if (Flags.list[FlagEnum.AMILY_AFFECTION] < 40) {
					DisplayText("\"<i>Talking to you is always nice... and, why the hell not? I'm not that big yet, I don't think?</i>\"\n\n");
					DisplayText("You assure her that she still looks trim and lean to you.\n\n");
					DisplayText("\"<i>Flatterer. Come on, I have something to eat back in my den.</i>\"\n\n");
					//[/ Go to random [Conversation], then to [Medium Affection Sex]]
					return { next: talkToAmilyWithSexAfter };
				}
				//[High Affection]
				else {
					DisplayText("\"<i>Well, I could maybe do without the sex part...</i>\" Amily muses, rubbing her chin. Then she grins. \"<i>We'll see how things work out, all right?</i>\"\n\n");
					DisplayText("You assure her that's fine, and the two of you find a relatively comfortable patch to sit down on so you can talk.\n\n");
			...
			case 8: //Amily is heavily pregnant
				//[Low Affection]
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) {
					DisplayText("She stares at you, then smiles faintly. \"<i>Talk? Talk is good... it's so quiet here; I spent so many years without anybody to talk to. But sex? In my condition? No, I don't think so.</i>\"\n\n");
					DisplayText("Despite her refusing the prospect of sex, she happily takes a seat on a toppled column and invites you to join her.\n\n");
					//[/ Go to random [Conversation]]
					return { next: talkWithCuntIMeanAmily };
				}
				//[Medium Affection]
					...
					DisplayText("You are forced to concede that you don't have any real ideas how sex between you would work with her in her current state.\n\n");
					DisplayText("Amily smiles and pulls up a seat on a mound of leaf litter. \"<i>That's all right; you meant well. And even if we can't have sex, we can still talk. Anything on your mind in particular?</i>\"\n\n");
					//[/ Go to random [Conversation]]
					return { next: talkWithCuntIMeanAmily };
				}
				//[High Affection]
				else {
					DisplayText("She grins at you playfully. \"<i>It's just wonderful to finally have somebody to talk to after all these years. And maybe I'll let you make love to me afterwards, if you're a good listener.</i>\"\n\n");
					DisplayText("She waddles over to something that – in the distant past – might have been a stone seat for public convenience, and seats herself upon it heavily. \"<i>So, what do you want to talk about?</i>\" she asks.\n\n");
			...
			default: //Amily is not pregnant
				//[Low Affection]
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) {
					DisplayText("\"<i>...Well, maybe you're not like everyone else in this world after all,</i>\" she finally answers. Though she walks away without a second word, she seems rather pleased by your answer.\n\n");
					DisplayText("\"<i>Hey, hurry up!</i>\" she calls back over her shoulder. You snap out of your musings and follow her.\n\n");
					//[/ Go to random [Conversation], then to [Low Affection Sex]]
					return { next: talkToAmilyWithSexAfter };
				}
				//[Medium Affection]
				else if (Flags.list[FlagEnum.AMILY_AFFECTION] < 40) {
					DisplayText("She smiles at you. \"<i>Well... I was feeling a little tired, a little lonely, and... maybe a little horny. Why not?</i>\"\n\n");
					DisplayText("She crooks a finger at you as a gesture to follow her.\n\n");
					//[/ Go to random [Conversation], then to [Medium Affection Sex]]
					return { next: talkToAmilyWithSexAfter };
				}
				//[High Affection]
	...
	private sneakyUberAmilyRemeetingsAnnounce() {
		...
		switch (pregnancy.event) {
			...
			case 5: //Amily is slightly pregnant
				//[Low Affection]
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) DisplayText("\"<i>I guess maybe I'm starting to slow down with this hanging off of me,</i>\" she grumbles. \"<i>Something you wanted?</i>\"\n\n");
				//[Medium Affection]
				else if (Flags.list[FlagEnum.AMILY_AFFECTION] < 40) DisplayText("\"<i>Be honest – did this make me easier to find?</i>\" she asks, pushing out her belly for emphasis. Then, seemingly uncaring about the answer, she changes the subject. \"<i>Is there a reason you wanted to see me?</i>\"\n\n");
				//High Affection
				else DisplayText("\"<i>I may be pregnant, but I didn't make this any easier on you now, did I?</i>\" She smirks. \"<i>It takes great skill to find me... but what is your reason?</i>\"\n\n");
				break;
			case 6:
			case 7:
			case 8: //Amily is heavily pregnant
				//[Low Affection]
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) DisplayText("\"<i>The only reason you could possibly find me is because this huge belly makes me too slow to move quickly and too big to hide. You know that, don't you?</i>\" she proclaims defensively, before changing her tone and asking curiously, \"<i>...Why are you here, anyway?</i>\"\n\n");
				//[Medium Affection]
				else if (Flags.list[FlagEnum.AMILY_AFFECTION] < 40) DisplayText("\"<i>I guess it's not as easy for me to hide as it once was. So, why are you here?</i>\"\n\n");
				//[High Affection]
				else DisplayText("\"<i>It's not easy for me to run and hide anymore, these days. I'm glad you decided to do the guardian angel act. So, how can I thank you for coming to check on me?</i>\"\n\n");
				break;
			default: //Amily is not pregnant
				//[Low Affection]
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) DisplayText("\"<i>How on earth did you get here? Nobody's ever been able to track me!</i>\" she protests. Then she shakes her head. \"<i>Ah well, I guess if it's you, it's no matter. So, whaddya want?</i>\"\n\n");
				//[Medium Affection]
				else if (Flags.list[FlagEnum.AMILY_AFFECTION] < 40) DisplayText("\"<i>Hmm... I'll have to work on that; I can't let just any bozo trail me,</i>\" she proclaims, smiling mischievously. \"<i>What's up?</i>\"\n\n");
				//[High Affection]
				else DisplayText("\"<i>You're coming along nicely, lover mine.</i>\" She smiles, proud as can be at your display of skill. \"<i>So, what brings you running to me?</i>\" she teases.\n\n");
		}
		//Sex / Talk / Talk then sex
		if (player.stats.lust >= 33) MainScreen.simpleChoices(["Sex", "Talk", "Both", "", ""], [sexWithAmily, talkToAmily, talkThenSexWithAmily, null, null]);
		else MainScreen.simpleChoices(["", "Talk", "", "", ""], [null, talkToAmily, null, null, null]);
	}
	...
	private scareAmilyRemeetingsProBaws() {
		...
		if (player.stats.spe < 50) {
			player.takeDamage(-5);
			//{Player takes minor HP damage}
			DisplayText("You scramble backwards, but it still cuts a nasty gash into your flesh. Amily looks poised to strike again, but stops when she sees that it's you.\n\n");
			//[Low Affection]
			if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) {
			...
			//[Medium Affection]
				...
				DisplayText("\"<i>That's what happens when you surprise someone with a knife,</i>\" she says, though clearly worried. \"<i>Hey, are you all right?</i>\" she asks.\n\n");

				DisplayText("You assure her that, while it hurts, it's nothing serious.\n\n");

				DisplayText("She looks relieved. \"<i>That's good... you did kind of deserve it, but killing you for being a dummy isn't how I'd like this relationship to end... So, why are you here, anyway?</i>\"\n\n");
			}
			//[High Affection]
			else {
				...
				if (pregnancy.isPregnant) DisplayText("  \"<i>Do you want our children to grow up without their father?</i>\" she asks softly, touching her bulging belly with one hand.\n\n");
				DisplayText("\n\n");

				DisplayText("You apologize for your foolishness, and promise it won't happen again.\n\n");

				DisplayText("\"<i>See that it doesn't,</i>\" she sniffs, primly. Then she grins in relief. \"<i>So, why did you want to come and see me?</i>\"\n\n");
			}
		}
		//[Player Speed is 50 or higher]
		else {
			DisplayText("You manage to leap backwards just in time to avoid a strike that could have seriously hurt you. Amily recovers quickly and readies her knife again, only to realize that it's you.\n\n");
			//[Low Affection]
			if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) {
				...
				DisplayText("Okay, maybe she hasn't entirely calmed down yet.\n\n");
			}
			//[Medium Affection]
			else if (Flags.list[FlagEnum.AMILY_AFFECTION] < 40) {
				DisplayText("An irritated expression crosses her face. \"<i>Are you insane!? Do you have any idea how stupid that was? I could have killed you!</i>\" she bellows, before slowly calming down. \"<i>Ah, well... no harm, no foul, I guess...</i>\"\n\n");
			}
			//[High Affection]
			else {
				DisplayText("Fear gives way to relief, which is then tinged with anger. \"<i>Of all the stupid half-witted stunts! Do you have any idea how dangerous that was?</i>\" She latches onto you, tears starting to glimmer in her eyes. \"<i>Do you know how I would have felt if I'd hurt you - if I'd killed you? Promise me you won't sneak up on me like that again!</i>\"\n\n");

				DisplayText("You promise you won't startle her like that in the future, and her face lights up with relief. \"<i>Good. So, that unpleasantness aside... why are you here?</i>\"\n\n");
			}
		}
		//Sex / Talk / Talk then sex
	...
	private scareAmilyOnDesperatePleaMeeting() {
		...
		if (player.stats.spe < 50) {
			player.takeDamage(5);
			DisplayText("You scramble backwards, but it still cuts a nasty gash into your flesh. Amily looks poised to strike again, but stops when she sees that it's you. She looks apologetic – well, somewhat. \"<i>Are you all right? I'm sorry, but that was honestly the stupidest thing I've ever seen someone do!</i>\" She approaches you and makes sure that you aren't seriously hurt.\n\n");
			DisplayText("\"<i>You'll live,</i>\" Amily declares quickly, though there is an honest sound of relief in her voice. \"<i>Ah... do you have the time to talk? There's something I want to get off my chest,</i>\" she asks, clearly nervous and maybe a little embarrassed.\n\n");
		}
		//[Player Speed is 50 or higher]
		else {
			DisplayText("You manage to leap backwards just in time to avoid a strike that could have seriously hurt you. Amily recovers quickly and readies her knife again, only to realize that it's you. An irritated expression crosses her face. \"<i>Are you insane!? Do you have any idea how stupid that was? I could have killed you!</i>\" she bellows, before slowly calming down. \"<i>Ah, well... no harm, no foul, I guess...</i>\"\n\n");
			DisplayText("She shifts awkwardly. \"<i>Ah... do you have the time to talk? There's something I want to get off my chest,</i>\" Amily nervously asks.\n\n");
		}
		desperateFinallyAmily();
	}
	private desperateFinallyAmily() {
		...
		//Accept her / Turn her down gently / Turn her down bluntly
		MainScreen.simpleChoices(["Accept Her", "TurnDownGently", "TurnDownBlunt", "", ""], [desperateAmilyPleaAcceptHer, desperateAmilyPleaTurnDown, desperateAmilyPleaTurnDownBlunt, null, null]);
	}
	...
	private desperateAmilyPleaAcceptHer() {
		...
		//set accepted flag
		Flags.list[FlagEnum.AMILY_OFFER_ACCEPTED] = 1;
		DisplayText("With a gentle smile, you reach out and take hold of her hand. You tell her that you do like her too; you just wanted to know her as a person before you would take something as precious to her as her virginity. If she still wants you, then you want to go with her now.\n\n");

		DisplayText("Amily stares at you, stunned. After a moment, she embraces you fiercely and begins to drag you away.\n\n");
		//[/ Go to [High Affection Sex]]
		return { next: amilySexHappens };
	}
	...
	private desperateAmilyPleaTurnDown() {
		...
		DisplayText("Amily sniffs loudly, tears blatantly running down her cheeks. \"<i>If... if that's the way it has to be, then,</i>\" she sniffles, \"<i>I... I guess that there's nothing left for me here. I'll just have to leave... Maybe I can find somewhere that will at least give me shelter.</i>\"\n\n");
		//[Player has found Tel'Adre]
		if (player.statusAffects.get(StatusAffectType.TelAdre).value1 >= 1) {
			DisplayText("You tell her that you've discovered a hidden city in the desert, free of corruption. Amily looks shocked, but clearly grateful as you assure her of its existence and provide instructions on how to get there.\n\n");
		}
		else {
			DisplayText("Looking dejected, Amily slowly begins to walk away. However, just before she makes her final turn to disappear, she turns back to you. \"<i>I'll always remember you,</i>\" she promises sincerely – and then she is gone.\n\n");
		}
		DisplayText("Feeling the weight of the empty village pressing in on you, you quickly retreat yourself. There's no point coming back here.\n\n");
		//turn off village.
		Flags.list[FlagEnum.AMILY_VILLAGE_ENCOUNTERS_DISABLED] = 1;
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	private desperateAmilyPleaTurnDownBlunt() {
		...
		DisplayText("\"<i>Why you-! I bare my soul to you, and this is how you repay me?!</i>\" Amily screams; rage, hurt and betrayal are all evident in her words.\n\n");

		DisplayText("You jeer at her that it's not your fault that she's so pathetic, falling for the first person to take pity on her and talk to her.\n\n");

		DisplayText("Amily responds by spitting a stream of the most jarringly foul language at you. In her rage, she hurls a blowpipe dart at you with her hands, which you easily swat aside. Cursing all the while, she races off in a fury.\n\n");

		DisplayText("You know she's never going to come back.");
	...
	private fuckingMouseBitchPopsShitOut() {
		...
		//Leave / Watch / Help
		MainScreen.simpleChoices(["Leave", "Watch", "Help", "", ""], [pregnancyIsScaryGoddamnMousePregnancyImNotWatchingThisShit, heyIGotTicketsToMicePoppingOut, helpThatFukkinUngratefulBitchGiveBirth, null, null]);
	}
	...
	private pregnancyIsScaryGoddamnMousePregnancyImNotWatchingThisShit() {
		...
		DisplayText("The next morning, you find a note scratched onto a slab of bark beside your sleeping roll, reading, \"<i>The babies and I are both fine. No thanks to you!</i>\"\n\n");
		//{Affection goes down}
		Flags.list[FlagEnum.AMILY_AFFECTION] -= 10;
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	private heyIGotTicketsToMicePoppingOut() {
		...
		//([horsecock]
		if (Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) DisplayText(", mousy figure – pink, hairless and ");
		else DisplayText(" figure –");
		...
		DisplayText("You watch as baby after baby slips free of its mother's womb and into the wider world. ");
		//([horsecock]
		if (Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) DisplayText(" Though hairless like regular infant mice,");
		else DisplayText(" Though remarkably similar to a regular infant - minus the ears and tail, of course -");
		...
		//([horsecock]
		if (Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) DisplayText(" and fur has already begun to cover their formerly-naked bodies.");
		else DisplayText(" maturing dramatically as you watch.");
		...
		DisplayText("Making sure that there doesn't seem to be any danger, you quietly let yourself out. It seems that she's too concerned about the children to notice you leave.");
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	private helpThatFukkinUngratefulBitchGiveBirth() {
		...
		if ((player.gender === Gender.HERM || Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) && randInt(2) === 0) amilyKid = "herm";
		if (player.gender === Gender.HERM && Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) amilyKid = "herm";
		DisplayText(amilyKid + ".\n\n");

		DisplayText("Awestruck, you tell Amily that she's beautiful and you really mean it. Even though she's a different species, she's your daughter and she's gorgeous to you.\n\n");
		...
		DisplayText("Uncertain, you do as you are told; your daughter latches onto her mother's ");
		...
		DisplayText(" is the same color as your hair.\n\n");
		DisplayText("Amily is tired, but she smiles at you happily. \"<i>I... thank you. For being here. For me – and them,</i>\" she says. You assure her that it was no problem. You sit there with her, letting your inquisitive offspring examine you and tussle with you while their mother regains her strength.\n\n");
		DisplayText("As the rambunctious little mouselets burn up their energy and curl up beside Amily to sleep, you gently excuse yourself and return to camp.");
		//{Affection goes up}
		Flags.list[FlagEnum.AMILY_AFFECTION] += 5;
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	private thisIsAReallyShittyBadEnd() {
		...
		DisplayText("Except for today. Amily walks out to confront you casually. \"<i>Ah, " + player.short + ", there you are,</i>\" she states. \"<i>Good. I was hoping to see you one last time; I thought you at least deserved a goodbye.</i>\"\n\n");

		DisplayText("\"<i>A goodbye?</i>\" you echo, confused.\n\n");
		...
		DisplayText("\"<i>The results of the litters that you fathered with me, yes.</i>\" Amily nods. \"<i>They are my children – my tribe – and we are now numerous and strong enough to leave these lands... find a new place to carve out a home for us.</i>\"\n\n");

		DisplayText("As you watch, they begin gathering at one street opening behind their mother. They're clad in rags and wield makeshift weapons, but look determined and surprisingly fierce.\n\n");

		DisplayText("\"<i>This is goodbye, " + player.short + ". You may not have been the most noble of [men]... but you did help me in my quest, and I am grateful – no matter how selfish your reasons may have been.</i>\"\n\n");
		...
		DisplayText("Amily has left the region with her children to found a new colony elsewhere.\n\n");
		return { next: Scenes.camp.returnToCampUseOneHour };
		//{Amily can no longer be encountered}
		//{Ruined Village removed from Places list}
		Flags.list[FlagEnum.AMILY_VILLAGE_ENCOUNTERS_DISABLED] = 1;
	...
	private thisFunctionProbablySucksTooOhYeahAmilyFunction() {
		...
		DisplayText("\"<i>" + player.short + "! Darling! You're here!</i>\"\n\n");

		DisplayText("You hear her delighted squeak, but she moves too quickly for you to catch, fiercely tackling you and embracing you.\n\n");

		DisplayText("\"<i>I was wondering if maybe we should go and find you first, I didn't want you to miss this chance.</i>\" She tells you, nuzzling your side emphatically.\n\n");

		DisplayText("Too confused to even hug her back, you ask what she's talking about.\n\n");

		DisplayText("Amily blinks and looks confused, then smiles. \"<i>Oh, right, I'm getting ahead of myself. Kids! Come and say hello to your father!</i>\" She calls out in delight.\n\n");
		...
		DisplayText("\"<i>These are...?</i>\" You begin hesitantly, unable to say it.\n\n");

		DisplayText("“<i>Our children, yours and mine");
		// Mix
		...
		// Only PC birth
			...
			DisplayText(", planted by me inside of you.</i>”");
		}
		// Only Amily birth
		else if (Flags.list[FlagEnum.PC_TIMES_BIRTHED_AMILYKIDS] === 0 && Flags.list[FlagEnum.AMILY_BIRTH_TOTAL] > 0) {
			DisplayText(", planted by you inside of me.</i>”");
		}
		DisplayText("  Amily trills happily.\n\n");

		DisplayText("You can only stare at the multitude in shock. From somewhere inside you, a lone thought wondering what your mother would say at the sight of so many grandchildren rears itself, only to be brushed aside when the realization of what else Amily said manages to sink in. You ask if she's leaving.\n\n");

		DisplayText("\"<i>Yes. There's enough of them here now that they can survive leaving this place to establish a new village elsewhere.</i>\" She confirms. \"<i>...Oh, don't look so gloomy!</i>\" She laughs, watching your face as you digest this news. \"<i>I'm not going with them.</i>\"\n\n");

		DisplayText("You can only start in shock and disbelief at this statement. Amily steps close, takes you hand in hand, and does her best to look you in the eyes. \"<i>");
		if (Flags.list[FlagEnum.AMILY_MET_AS] === 1) DisplayText("When we first met, you were just sperm to me, a pure male to father the children I needed. ");
		else DisplayText("When we first met, I was afraid of you, I needed your male essence, but I was prejudiced against your other gender. ");
		DisplayText("But as I got to know you, you became more to me. I love you, " + player.short + ", and I don't want to leave you. You came to this world on a quest, didn't you? Well, you helped me with my quest, so it's only fair I help you with yours. The children have been taught well, they can make their own way from here and now... but I want to come back with you, to stay with you and help however I can. Will you let me?");
		DisplayText("</i>\"\n\n");

		DisplayText("You shake off your confusion as her words sink in. Smiling broadly, you wrap her in your arms and tell her you want her to come back with you. She smiles, and hugs you back with surprising fierceness, even as your many children react with delight.\n\n");

		DisplayText("You spend over an hour getting to know your many sons and daughters, helping them pack up their meager supplies and wishing them well as they set off. Then, with Amily's hand in yours, you return to your camp, leaving behind this empty village to be reclaimed by the wilderness.\n\n");
		//Oh boy.  So excited.  wee.  Blech.
		DisplayText("\\[<b>Amily has joined you as a lover.</b>\\]\n\n");
		...
		if (player.statusAffects.has(StatusAffectType.CampMarble)) {
	...
	public talkWithCuntIMeanAmily(sexAfter: boolean = false) {
		...
		if (player.gender === Gender.FEMALE || (player.gender === Gender.HERM && Flags.list[FlagEnum.AMILY_HERM_QUEST] < 2)) convo = randInt(12);
		//Boost affection!
		Flags.list[FlagEnum.AMILY_AFFECTION] += 2 + randInt(3);
		player.stats.cor += -.34;
		//Conversation: Items
		if (convo === 0) {
			DisplayText("The two of you swap tales about your respective adventures, and from there the topic drifts to the various potions and elixirs discovered in this world.\n\n");

			DisplayText("\"<i>You know...  I don't have the equipment needed to practice alchemy, but I do know a few things about it.</i>\" Amily says.  \"<i>If you can bring me a potion or a reagent, I may be able to remember some of the things my father taught me.</i>\"\n\n");

			//(If player has no main item:)
			if (player.itemSlot1.quantity === 0) {
				DisplayText("Promising you'll keep that in mind, you take your leave of Amily.\n\n");
			...
			//(If player has an item:)
				...
				DisplayText("You remember that you have something in your pockets that might be of interest, and show it to Amily.\n\n");
			}
			if (player.itemSlot1.itype === consumables.EQUINUM) {
				DisplayText("\"<i>That's a distillation of horse essence, I think.</i>\" Amily says.  \"<i>I guess it would probably make you stronger and tougher... but horses aren't smart, and it might be too strong for a human to handle without changing them,</i>\" she warns you.\n\n");
			}
			//Canine Pepper & Variants:
			else if (player.itemSlot1.itype === consumables.CANINEP) {
				DisplayText("\"<i>Looks kind of like a dog's dick, doesn't it?  Especially this one with the big knot-like bulge or this one with the ball-like bulbs.  I suppose it would make you more dog-like... but I'm pretty sure you should avoid these jet-black ones.  I can't remember why...</i>\" she trails off, wracking her brain.\n\n");
			}
			//Canine Pepper & Variants:
			...
			//Succubus Milk/Incubus Draft:
				...
				DisplayText("She recoils with a hiss.  \"<i>That's demon fluid, it is - like drinking liquid corruption! Avoid that stuff if you can; it'll turn you into a demon, and supercharge your sex-drive.  I've heard it can even mess with your gender if you drink too much of the opposite stuff.</i>\"\n\n");
			}
			//Succubi's Delight:
			else if (player.itemSlot1.itype === consumables.SDELITE) {
				DisplayText("\"<i>Full of taint, no question of that.  Succubi give it to males who haven't become demons yet; makes them better able to produce cum, and pushes them towards demonhood.</i>\"\n\n");
			}
			//Wet Cloth:
			else if (player.itemSlot1.itype === consumables.WETCLTH) {
				DisplayText("\"<i>I... have no idea what that is.</i>\" she says, looking confused.  \"<i>I guess it's... slimey?  Concentrate of goo?  I think it's got something to do with whatever's been polluting the lake, so I wouldn't rub it into your skin.</i>\"\n\n");
			}
			//Bee Honey:
			else if (player.itemSlot1.itype === consumables.BEEHONY) {
				DisplayText("\"<i>Honey from a giant bee?</i>\" she asks eagerly, perking up.  \"<i>Oh, that stuff's delicious! I hear it's full of special essences secreted by the giant bees, though, so it could have transformative effects.</i>\"\n\n");
			}
			//Pure Honey:
			else if (player.itemSlot1.itype === consumables.PURHONY) {
				DisplayText("\"<i>You managed to get your hands on ultra-pure giant bee honey?</i>\" she asks, sounding impressed.  \"<i>I hear that stuff's so pure it can actually help purge the eater of demonic taint - but it's probably otherwise the same as regular bee honey.</i>\"\n\n");
			}
			//Green Glob:
			else if (player.itemSlot1.itype === useables.GREENGL) {
				DisplayText("\"<i>A blob of slime from a green gel?  Hmm...</i>\" she looks thoughtful.  \"<i>I think I remember my dad once telling me you could make a really strong armor out of a special distillation of green oozes.  I can't say for sure, and I wouldn't have the equipment even if I did remember.</i>\"\n\n");
			}
			//Bee Chitin:
			else if (player.itemSlot1.itype === useables.B_CHITN) {
				DisplayText("\"<i>If you had a sufficient mass of this stuff, you could make a suit of armor out of it.  It needs special alchemical reagents, though, otherwise it'll just get all brittle and smashed up.</i>\"\n\n");
			}
			else {
				DisplayText("She looks at it and shrugs, not really familiar with it.\n\n");
			}
			DisplayText("Thanking her for her help, you return the item to your pocket");
			if (sexAfter) DisplayText(".\n\n");
			else DisplayText(" and take your leave of the mouse-girl.\n\n");
		}
		//Conversation: Minotaurs
		else if (convo === 1) {
			DisplayText("The two of you swap tales about your respective adventures, and from there the topic drifts to the strange bull-men that you have seen haunting the mountains.\n\n");
			...
			DisplayText("\"<i>I fought it off and ran,</i>\" she insists, looking a little insulted, ");
			//(If player hasn't had sex with Amily:
			if (Flags.list[FlagEnum.AMILY_FUCK_COUNTER] === 0) DisplayText("\"<i>I am still a virgin, after all.</i>\"\n\n");
			//(If player has had sex with Amily:
			else DisplayText("\"<i>I was a virgin when we met, in case you forgot.</i>\"\n\n");

			DisplayText("\"<i>The big brute was stupid enough to follow me; once I got to the trees, where he couldn't get around so easily, I put a poisoned dart in each of his eyes.  When he stopped thrashing around, I walked up and slit his throat.</i>\"\n\n");

			DisplayText("She looks quite proud of herself for that.  Feeling a bit disturbed by her ruthlessness, you thank her for the warning");
			if (sexAfter) DisplayText(".\n\n");
			else DisplayText(" and excuse yourself.\n\n");
		}
		//Conversation: Sand Witches
		else if (convo === 2) {
			DisplayText("The two of you swap tales about your respective adventures, and from there the topic drifts to the mysterious female magic-users you've seen in the Desert.\n\n");

			DisplayText("\"<i>Sand Witches, eh? Funny, it seems the only pseudo-humans left in this area that aren't demons are magic-users... even though it was their usage of magic that turned the first demons into demons,</i>\" Amily notes.  \"<i>Or so I heard, anyway,</i>\" she adds.\n\n");

			DisplayText("You ask her if she knows what the Sand Witches are trying to do out there in the desert.\n\n");

			DisplayText("\"<i>Not a clue.  I think... I think that they might be some kind of druidic sect or something. Restoring life to the desert, trying to make it into grassland or something like that,</i>\" Amily suggests.\n\n");

			DisplayText("You ask her why she comes to that conclusion.\n\n");
			...
			DisplayText("You ask her how on earth she would know that all Sand Witches have four milk-filled breasts.  She blushes deeply and fidgets, clearly embarrassed.  Finally, she speaks up.\n\n");

			DisplayText("\"<i>I... kind of got lost in the desert once.  A Sand Witch approached me and asks me if I'd let her cast a spell on me.  When I refused, she attacked, trying to beat me down so she could cast it on me anyway.</i>\"\n\n");

			DisplayText("You interrupt to ask her what the spell would have done.\n\n");
			...
			DisplayText("You assure her that she doesn't need to spell it out.  She looks grateful, then continues.\n\n");
			...
			DisplayText("Thanking her for the advice, you promise you'll be more careful if you see them in the future");
			if (sexAfter) DisplayText(".\n\n");
			else DisplayText(" and take your leave.\n\n");
		}
		//Conversation: Giant Bees
		else if (convo === 3) {
			DisplayText("The two of you swap tales about your respective adventures, and from there the topic drifts to the strange bee-like women you've seen in the forest.\n\n");

			DisplayText("\"<i>Giant Bees?  They're a strange race,</i>\" Amily says.  \"<i>They're not really corrupt, but at the same time, they act kind of like demons.</i>\"\n\n");

			DisplayText("You ask her to explain how that works.\n\n");

			DisplayText("\"<i>Well... the Giant Bees you've probably seen?  The official name for them is the fertile caste, but folks tend to just call them handmaidens,</i>\" Amily explains.\n\n");

			DisplayText("At your confused look, she continues.\n\n");

			DisplayText("\"<i>Giant Bees hatch from each egg as hundreds of regular-looking little bees.  And fertiles are involved in that - for some reason, the eggs need to incubate inside a living being to hatch in the first place.</i>\"\n\n");
			...
			DisplayText("You thank her for her advice, and then, curious, ask how she knows so much about these creatures.\n\n");

			DisplayText("Amily looks flustered.  \"<i>Well, they, they have been willing to trade honey in the past if you confront them peacefully and... alright, I'll admit it, when times have been really lean, I've been willing to host some giant bee eggs in exchange for honey.</i>\"\n\n");

			DisplayText("You smile and thank her for sharing, noting that she didn't need to tell you such personal information");
			if (sexAfter) DisplayText(".\n\n");
			else DisplayText(", and politely excuse yourself.\n\n");
		}
		//Conversation: Fetish Cultists & Zealouts
		else if (convo === 4) {
			DisplayText("The two of you swap tales about your respective adventures, and from there the topic drifts to the bizarrely-dressed people you've caught glimpses of around the lake.\n\n");
			...
			DisplayText("Telling her that it's all right if she doesn't know that much, you get up");
			if (sexAfter) DisplayText(" and sigh.\n\n");
			else DisplayText(" and leave.\n\n");
		}
		//Conversation: Imps
		else if (convo === 5) {
			DisplayText("The two of you swap tales about your respective adventures, and from there the topic drifts to the small demons you've seen in your travels.\n\n");
			...
			DisplayText("Her tail is lashing furiously from side to side, and, clearly worked up, she tensely excuses herself ");
			if (sexAfter) DisplayText("and regains her composure.\n\n");
			else {
				DisplayText("and leaves.");
				if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 0) DisplayText("  As you set off back to camp,");
				else DisplayText("  As you sit back down in camp,");
				DisplayText(" you have little doubt that she's gone to find and kill an imp.\n\n");
			}
		}
		//Conversation: Shark Girls
		else if (convo === 6) {
			DisplayText("The two of you swap tales about your respective adventures, and from there the topic drifts to the humanoid female sharks you've seen while rowing your boat.\n\n");

			DisplayText("\"<i>Shark Girls? Near as I can tell, they used to be a village of humans who lived right here on the lake... then the lake got polluted, and turned them all into... well, what they are now.</i>\"\n\n");

			if (!izmaFollower()) DisplayText("She looks pensive.  \"<i>Odd... I don't think they have any males left, but on very rare occasions I've seen these weird tiger-striped Shark Girls... and they always had huge cocks and balls as well.  But, whether female or herm, they seem to only care about fighting and fucking... and from the way I've seen them going at it, I don't think they see any difference between the two any more.</i>\"\n\n");

			DisplayText("You ask her if she has any advice on fighting them.\n\n");

			DisplayText("\"<i>I'm afraid not.  They don't come to the shore too often, never mind too far out of the lake. They're probably resistant to pain and have a really wicked bite, though,</i>\" she tells you.\n\n");

			if (!sexAfter) DisplayText("Thanking her for her time, you take your leave of her.\n\n");
			else DisplayText("You thank her for her time as the conversation winds down.\n\n");
		}
		//Conversation: Goblins
		else if (convo === 7) {
			DisplayText("The two of you swap tales about your respective adventures, and from there the topic drifts to the strange green women you've seen in the forest and the mountains.\n\n");
			...
			DisplayText("You ask if they're aggressive.\n\n");
			...
			if (!sexAfter) {
				if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 0) DisplayText("Thanking her for her time, and the warning, you head back to your own camp.\n\n");
				else DisplayText("Thanking her for her time, and the warning, you sit back down in your camp.\n\n");
			}
			else DisplayText("You thank her for her time as the conversation winds down.\n\n");
		}
		//Conversation: What was life in her village like?
		else if (convo === 8) {
			DisplayText("You think for a moment, wondering what to ask her. Then you shrug your shoulders and tell her that you'd like to know a little more about her, that you want to get to know her better.\n\n");

			DisplayText("Amily looks surprised... but pleased.  \"<i>I... You're really interested in hearing about me?  Well... okay.  What do you want to know?</i>\"\n\n");

			DisplayText("You rack your brains for a few moments, then ask her to tell you what life was like in her village, before everything.\n\n");

			DisplayText("\"<i>Well, what was life like in your village, huh?</i>\" she responds immediately.  \"<i>If I gotta share something private like that, then the least you can do is reciprocate.</i>\"\n\n");
			...
			DisplayText("You see a small tear form in her eye and trickle down her cheek.  Unable to say why, you reach out and wipe it away with your finger.  She looks at you, startled");
			if (!sexAfter) DisplayText(", and, embarrassed and unable to explain why you did that, you politely take your leave.");
			else DisplayText(", before giving you a warm smile.\n\n");
		}
		//Conversation: Who was she before it was all lost?
			...
			DisplayText("You rack your brains for a few moments, then ask her to tell about herself, about who she was, before everything.\n\n");

			DisplayText("\"<i>Well, what were you like before you came to this world, huh?</i>\" she responds immediately.  \"<i>If I gotta share something private like that, then the least you can do is reciprocate.</i>\"\n\n");
			...
			DisplayText("You ask if she didn't get on with her father.\n\n");
			...
			DisplayText("Gently, you reach out and take her hand, trying to offer her some comfort, and then ask what happened to her parents.\n\n");
			...
			DisplayText("\"<i>I'll be alright,</i>\" she sobs.  \"<i>But... please... leave me alone now? Please?</i>\"\n\n");

			DisplayText("Wanting to respect her privacy, you place a hand on her shoulder and give her a squeeze, attempting to convey that you will always be ready to talk if she needs it.");
			if (!sexAfter) DisplayText("  Then you leave her, giving her the chance to mourn her lost family.\n\n");
		}
		//Conversation: How did it fall?
			...
			DisplayText("Hesitantly, you ask her what happened to cause her village to fall.  Why is she the only survivor?\n\n");
			...
			DisplayText("You realize she's starting to shiver.\n\n");

			DisplayText("\"<i>There were so many of them... so horrible.  Imps by the dozens, the hundreds, succubi, incubi, creatures I can't even name.  Twisted forms, all blurring into each other...</i>\"\n\n");

			DisplayText("You place a hand on her shoulder, trying to draw her back into the real world.  She stops shaking and continues, calmer.\n\n");
			...
			DisplayText("Tears are pouring down her face.  Unable to think of anything else to do, you wrap your arms around her, holding her as she leans against you and weeps silently.  Finally, you feel her tears stop, and she gently pushes you away.\n\n");

			DisplayText("\"<i>Thank you.  It's been so long... but it still hurts, remembering.</i>\"\n\n");

			DisplayText("You tell her that you're sorry for bringing up such painful memories");
			if (sexAfter) DisplayText(".");
			else DisplayText(", then excuse yourself once you are certain she is okay.\n\n");
		}
		//Conversation: How did she survive?
			...
			DisplayText("You ask her, hesitantly, how she managed to survive the destruction of her village, and how she's stayed alive ever since.\n\n");
			...
			DisplayText("She grins at you, mischievously.  \"<i>Any more questions?</i>\"\n\n");

			DisplayText("You shake your head \"<i>no</i>");
			if (!sexAfter) {
				if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 0) DisplayText("\", politely excuse yourself, and head back to your own camp. It sounds like she's doing better at keeping a steady supply of food and water going than you are.  But if that's the case... why does she look so thin?\n\n");
				else DisplayText("\", politely excuse yourself, and sit back down in camp.  It sounds like she's doing better at keeping a steady supply of food and water going than you are.  But if that's the case... why does she look so thin?\n\n");
			}
			else DisplayText(".\"");
		}
		//Conversation: What does she plan to do when she gives birth?
			...
			DisplayText("You point out that she's told you why she wants you to father children with her, but she hasn't told you what she will do once she gives birth.\n\n");

			DisplayText("\"<i>You actually care what happens to them afterwards?</i>\" She asks, seemingly having a hard time believing that.\n\n");

			DisplayText("You insist that, yes, you do want to know.\n\n");
			...
			DisplayText("You ask how that is supposed to work, given that she is very obviously not corrupt.\n\n");
			...
			DisplayText("Feeling a little enlightened, you then ask her just why she needs to have so many children, and so fast.  You remember that she wants her race to be reborn outside of demonic slavery, but what will she do when she judges she has enough.\n\n");
			...
			DisplayText("You thank her for the explanation.\n\n");
		}
		else if (convo === 13) {
			//[Amily Talks About Kitsune]
			DisplayText("The two of you swap tales about your respective adventures, and from there the topic drifts to the odd many-tailed fox women you've seen deep in the woods.\n\n");
			DisplayText("\"<i>Kitsune,</i>\" she says matter-of-factly, nodding gently.  \"<i>There's a race that's hard to categorize, if there ever was one.</i>\"\n\n");

			DisplayText("You ask her what she means by this.\n\n");
			...
			DisplayText("You nod, noting that you can see how that is similar to some of the demons you've come across.\n\n");
			DisplayText("\"<i>Physically, they're not the strongest, but they don't have to be – once they've got you caught in one of their illusions, you're pretty much under their power. Despite that, they don't seem to be particularly evil... at least, for the most part.</i>\"\n\n");

			DisplayText("You give her a slightly quizzical look and then press her on the details.\n\n");
			...
			DisplayText("You ask how anyone knows this much about them, if even meeting one is so dangerous.  Amily scratches her head a bit and blushes, clearly a little embarrassed to not have a definitive answer for this one.  \"<i>W-well... it's mostly just conjecture.  Like I said, I'm only repeating what I've heard.</i>\"\n\n");

			DisplayText("She pauses for a moment, and then speaks up with another interesting fact in an attempt to save face.  \"<i>They do seem to have a strange fascination with tentacle beasts, for some reason.</i>\"\n\n");

			DisplayText("From the blush on her face, you get the feeling that she may be speaking from experience.  You don't press her on the matter though, ");

			DisplayText("just nodding politely as you process the information.");
		}
		//Conversation: How will she care for her children?
			...
			if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 0) {
				DisplayText("You pause for a few moments, trying to think of a way to phrase this delicately, then ask her how she plans on caring for her children.\n\n");
				...
				DisplayText("You pause for a few moments, trying to think of a way to phrase this delicately, then ask her how she thinks your children are doing.\n\n");

				DisplayText("A hint of worry creeps into her expression, matching your own; but she remains resolute. “<i>The first of our kids grew up so fast, and they seem to be doing OK out there without their parents to dote on them. The eldest are doing a great job caring for the young ones.</i>” she answers.\n\n");
			}

			DisplayText("Feeling somewhat more reassured, but not entirely so, you get ready to leave.\n\n");

			DisplayText("\"<i>Hey, hold on a second,</i>\" Amily tells you.  \"<i>You were worried, weren't you? Worried about me...? About the children?</i>\"\n\n");

			DisplayText("You nod and admit you were worried, yes.\n\n");

			DisplayText("\"<i>That's... that's sweet of you,</i>\" Amily says, clearly shocked.  \"<i>I didn't think you would actually care...</i>\" she trails off, looking thoughtful.");
			if (!sexAfter) {
				if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 0) DisplayText("  Then, as if realising you are still here, she waves at you to go, getting up and leaving herself.  Wondering what that was about, you return to camp.\n\n");
				else DisplayText("  Then, as if realising you are still here, she waves at you to go, getting up and leaving herself.  Wondering what that was about, you sit down in camp.\n\n");
			}
		}
		if (sexAfter) {
	...
	private stickItInMouseTwatForTheFirstTimeNOTWORTHALLBULLSHIT() {
		...
		if (player.tallness < 60) DisplayText("Fortunately, being smaller than average means you have less difficulty following her than you might.");
		else if (player.tallness >= 84) DisplayText("Your considerable size makes it surprisingly tricky to get around, but you manage to stay with her.");
		...
		//[Wait for Her]
		//[Kiss Her]
		MainScreen.simpleChoices(["Take Charge", "Wait 4 Her", "Kiss Her", "", ""], [FirstTimeAmilyTakeCharge, beSomeKindofNervousDoucheAndWaitForAmily, kissAmilyInDaMoufFirstTimeIsSomehowBetterThatWay, null, null]);
	}
	...
	private FirstTimeAmilyTakeCharge() {
		...
		DisplayText("\"<i>Hey, what's the big idea?</i>\" She protests indignantly.\n\n");

		DisplayText("\"<i>You wanted sex with me, so just shut up, lie back and take it.</i>\" You snap back at her.\n\n");

		DisplayText("Amily goes quiet, her eyes hardening into gimlets. It's pretty obvious she's not happy about this in the slightest, but she doesn't protest as you roughly pull off her shirt and her pants, exposing her pink sex, and then begin to tear off your own clothes.\n\n");
		...
		if (player.torso.cocks.get(0).length >= 14) {
			...
			else if (player.torso.cocks.get(0).type === CockType.DEMON) {
				DisplayText("Horror and disgust write themselves on her features as your knobbly, diabolic penis is revealed to her. \"<i>What kind of monster are you?! You'll kill me with that unholy thing!</i>\"\n\n");
				DisplayText("\"<i>Just shut up and you'll enjoy this.</i>\" You promise her.\n\n");
			}
			//(If player has an equine penis 14 inches or more long)
			else if (player.torso.cocks.get(0).type === CockType.HORSE) {
				DisplayText("Her eyes go wide with shock and fear as she discovers your stallion-like cock. \"<i>Even in this world, being hung like a horse isn't a good thing! I can't take something like that!</i>\"\n\n");
		...
		DisplayText("Without further ado, you thrust into her, eliciting a shriek of equal parts pain and rage. Ignoring that, you focus on trying to squeeze as much as your cock as you can into her warm, tight, velvety depths. She sobs and moans as you struggle, ");

		if (player.torso.cocks.get(0).length >= 14) DisplayText("failing to fit in more than a foot of your cock's length, ");
		DisplayText("but she doesn't try very hard to fight you off.\n\n");
		...
		DisplayText("Only when you are sure that the last of your climax is over do you pull out, carelessly striding over to retrieve your clothing and start getting dressed. Amily stares at you, her eyes hard and sharp as flints. \"<i>Was it good for you?</i>\" She spits. \"<i>Let's hope we've both gotten what we want out of this.</i>\"\n\n");

		DisplayText("\"<i>I'll be happy to come back and do it again if you need.</i>\" You jeer back, finishing dressing yourself and leaving her without so much as a backwards glance.\n\n");
		player.orgasm();
		//Affection downer
		Flags.list[FlagEnum.AMILY_AFFECTION] -= 5;
		amilyPreggoChance();
	...
	private beSomeKindofNervousDoucheAndWaitForAmily() {
		...
		DisplayText("Despite this passionate display, though, she doesn't seem to really be 'feeling' it, more going through the motions to arouse you. You are too horny to care that much.\n\n");
	...
	private kissAmilyInDaMoufFirstTimeIsSomehowBetterThatWay() {
		...
		DisplayText("Enjoying the sensations, you start caressing her to increase her pleasure, too. It doesn't take long for you both to orgasm, before Amily sinks down on you, looking into your eyes with a smile on her face.\n\n");

		DisplayText("\"<i>...That was... wow. Uh, I mean.</i>\" She hastily corrects herself, blushing. \"<i>I guess you weren't so bad... I knew I had a good feeling about you.</i>\" She smiles. \"<i>You do know I'm not neccessarily pregnant, right? We're going to have to do this again.</i>\"\n\n");

		DisplayText("You smile, and tell her that you're happy to do it with her as often as it takes. She blushes so red ");
		//([horsecock]
		if (Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0)
			DisplayText("it's almost like the fur on her cheeks has turned red.");
		else
			DisplayText("she nearly resembles an imp!");
		DisplayText("  Excusing yourself, you get dressed, receiving a lazy wave goodbye and a happy smile as you head out of the door and head for the main street, from there finding the way back to your camp.\n\n");
		//Affection boost?
		Flags.list[FlagEnum.AMILY_AFFECTION] += 3;
		player.orgasm();
		return { next: Scenes.camp.returnToCampUseOneHour };
	...
	public amilySexHappens() {
		...
		//If too big
		if (x === -1 && player.torso.cocks.count > 0) {
			DisplayText("Amily looks between your legs and doubles over laughing, \"<i>There is no way that thing is fitting inside of me!  You need to find a way to shrink that thing down before we get in bed!</i>\"");
		...
		if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) {
			...
			DisplayText("It's hard for you to say if you were led by a different route this time, but soon you are in what Amily has to offer for a private bedchamber, and she begins to reach for her clothes, obviously expecting you to do the same thing.\n\n");
			MainScreen.simpleChoices(["Business", "Playtime 1st", "", "", ""], [amilySexBusiness, amilySexPlaytimeFirst, null, null, null]);
		}
		//Moderate Affection Sex:
			...
			if (pregEvent >= 1 && pregEvent <= 5) DisplayText("  However, she does sometimes touch the swell signifying the litter growing inside her, and when she does her attitude becomes uncertain and nervous.");
			DisplayText("\n\n");

			DisplayText("Once you are inside, Amily gently tries to push you onto the bedding where you will be mating. Once you are seated, she smiles at you with a teasing expression and begins to slowly strip herself off, clearly trying to make the act seem as erotic as possible.");
			if (pregEvent >= 6) DisplayText("  However, her confidence visibly slips when she has to fully bare the bulging belly that marks her pregnant state, but she musters the confidence and starts to show it off for you as well.");
			MainScreen.simpleChoices(["Step In", "Watch Show", "", "", ""], [amilyStepTheFuckIn, amilyEnjoyShow, null, null, null]);
		}
		else {
	...
	private amilySexPlaytimeFirst() {
		...
		DisplayText("\"<i>W-What are you doing?</i>\" She asks, curious and a little wary.\n\n");
		...
		DisplayText("\"<i>Just making sure that you haven't... you know, picked up something you shouldn't.</i>\" She explains softly.\n\n");
		player.stats.lust += 5;
		amilySexPtII();
	}
	...
	private amilySexPtII() {
		...
		if (player.statusAffects.has(StatusAffectType.Infested)) {
			DisplayText("\"<i>EWWWW!  You're infested!</i>\" she shrieks, \"<i>Get out!  Don't come back 'til you get rid of the worms!</i>\"\n\nYou high tail it out of there.  It looks like Amily doesn't want much to do with you until you're cured.");
			return { next: Scenes.camp.returnToCampUseOneHour };
			Flags.list[FlagEnum.AMILY_AFFECTION] -= 3;
	...
	private continueAmilySmex() {
		...
		{
			DisplayText("Determined to make this good for Amily too, you resume stroking and caressing her, doing your best to meet her thrusts with your own, and planting the odd kiss on the nape of her neck");
			//([horsecock]
			if(Flags.list[FlagEnum.AMILY_NOT_FURRY]==0)
				DisplayText(" - not actually an unpleasant experience, despite the fur");
			DisplayText(". She is surprised, and tenses warily at first, but then melts under your ministrations, squeaking softly in her appreciation of your efforts.");
		}
		*/
		DisplayText("But all good things must come to an end, and soon you both build to a mutual climax. Once you have regained your strength, you stop holding each other and begin to dress, ready to go your separate ways once more. At the door, though, Amily stops you.\n\n");

		DisplayText("\"<i>Thank you, that was... nice...,</i>\" the little mousegirl says with a blush. \"<i>Maybe... we can... do it again?</i>\"\n\n");

		DisplayText("She seems surprised that she actually enjoyed it (at least a little), but she's definitely willing to repeat the experience. You assure her that you'll come back, and then resume your travels.");
		//Knock up, PC stats, etc.
		Flags.list[FlagEnum.AMILY_FUCK_COUNTER]++;
		amilyPreggoChance();
		//Slight affection gain?
		Flags.list[FlagEnum.AMILY_AFFECTION] += 1 + randInt(2);
		player.orgasm();
		player.stats.sens += -1;
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	private amilyEnjoyShow() {
		...
		if (pregnancy.event >= 6) DisplayText("  Even though she was clearly a little nervous about her gravid state in the beginning, as she continues, she grows in confidence to the point it seems she has almost forgotten about it.");
		DisplayText("\n\n");
		AmilyMidSexLevel2();
	}
	...
	private AmilyMidSexLevel2() {
		...
		DisplayText("Do you do a striptease of your own or just strip naked and get to business?");
		MainScreen.simpleChoices(["Striptease", "Business", "", "", ""], [StripForAmilyYouSlut, getDownWithSexTiem, null, null, null]);
	}
	...
	private AmilyMiddleGradeSexOver() {
		...
		//Affection gain here?
		amilyPreggoChance();
		Flags.list[FlagEnum.AMILY_AFFECTION] += 3 + randInt(4);
		Flags.list[FlagEnum.AMILY_FUCK_COUNTER]++;
		MainScreen.simpleChoices(["Say Goodbye", "Stay A While", "", "", ""], [sayGoodByeToAmilyPostSecks, stayAfterAmilyMiddleGradeSecks, null, null, null]);
	}
	...
	private amilyHighAffectionSecks() {
		...
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] === 0) DisplayText("sucking on her sensitive clit");
		else DisplayText("licking and kissing her human-like cock");
		DisplayText(" to stimulate her further. In response, Amily moans loudly and spreads her legs further apart, an invitation to continue. You happily oblige your lover, burying two fingers into her wet cunt while you move to other parts of her body.");
		if (pregnancy.isPregnant) DisplayText("  As you move your head across her beautiful form, you stop at her growing baby bump and give it a small kiss.");
		DisplayText("  Your head hovers at her breasts");
		if (pregnancy.isPregnant) DisplayText(", which seem to have grown from the pregnancy,");
		DisplayText(" and get to work licking and suckling her nipples, rubbing the sensitive mounds.");
		if (pregnancy.isPregnant) DisplayText("  When you get a dribble of milk in your mouth it surprises you, but you certainly don't stop.  \"<i>Hah...You're going to have to teach the children how that's done,</i>\" Amily says, in between fevered breaths.");
		DisplayText("\n\n");
		...
		DisplayText("Amily moans from the pleasure and raises her hips up to meet your thrusts, desperate for more of your loving.  She whispers a few dirty things to you between shallow breaths, \"<i>");
		if (!pregnancy.isPregnant) DisplayText("Fill me up with everything you have... I want to be a mother for your children, just as much as I want to be a mother of my own people,");
		else DisplayText("No need to hold back, pump as much cum into me as you can,");
		...
		//(if Amily is herm:
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText(" and cum spurts into the air between you, splattering on you both");
		DisplayText(".\n\n");

		if (!pregnancy.isPregnant) DisplayText("\"<i>If that didn't knock me up... I don't care as much as you'd think. It was magnificent either way,");
		else DisplayText("\"<i>Hmm...My kids are pretty lucky that their father is such a virile specimen,");
		DisplayText("</i>\" Amily says as she catches her breath, reaching up to ruffle your hair. You give her a bashful smile, glad to see you've made her so happy.\n\n");

		DisplayText("The two of you lie together for some time, and it's with great regret that you tell her that you need to check in on your own camp. Amily seems disappointed, not wanting you to leave, but understands why you need to go. \"<i>");
		if (!pregnancy.isPregnant) DisplayText("Okay... Well, I'm sure you'll be back. I will need your help again if this doesn't set,");
		else DisplayText("Okay dear...But you better come back some time. You don't want your children to have abandonment issues, do you?");
		DisplayText("</i>\" Amily says while rubbing her stomach. You smile at her and nod, promising you'll come back, before setting off for your own camp.");
		/*OLD
		DisplayText("Amily really didn't waste any time getting to her hidden bedroom, sprinting as fast as she could with you in tow.");
		...
		if(Flags.list[FlagEnum.AMILY_INCUBATION] > 0) DisplayText("  As you move your head across her beautiful form, you stop at her growing baby bump and give it a small kiss.");
		DisplayText("  Your head hovers at her breasts ");
		if(Flags.list[FlagEnum.AMILY_INCUBATION] > 0) DisplayText("which seem to have grown from the pregnancy ");
		DisplayText("and get to work licking and suckling her nipples, and rubbing the sensitive mounds.");
		if(Flags.list[FlagEnum.AMILY_INCUBATION] > 0) DisplayText("  When you get a dribble of milk in your mouth it surprises you, but you certainly don't stop \"<i>Hah... You're going to have to teach the children how that's done,</i>\" Amily says in between fevered breaths.");
		DisplayText("\n\n");
		...
		DisplayText("Amily moans from the pleasure and raises her hips up to meet your thrusts, desperate for more of your loving, whispering a few dirty things to you between shallow breaths \"<i>");
		//not preggo
		if(Flags.list[FlagEnum.AMILY_INCUBATION] === 0) DisplayText("Fill me up with everything you have...I want to be a mother for your children, just as much as I want to be a mother of my own people...");
		else DisplayText("No need to hold back, pump as much cum into me as you can...");
		DisplayText("</i>\" she whispers in a sultry tone, and her words are enough to send you over the edge. You grunt loudly, feeling as if your cock is about to explode from the exertion, blasting Amily so full of your cum that it starts to ooze out. Amily gives a cute little cry, and her vaginal walls clamp down on your sensitive member with enough force to make you wince, as girlcum sprays out onto your thighs.\n\n");

		//no preggo
		if(Flags.list[FlagEnum.AMILY_INCUBATION] === 0) DisplayText("\"<i>If that didn't knock me up...I don't care as much as you'd think. It was magnificent either way,");
		else DisplayText("\"<i>Hmm...My kids are pretty lucky that their father is such a virile specimen,");
		DisplayText("</i>\" Amily says as she catches her breath, reaching up to ruffle your hair. You give her a bashful smile, glad to see you've made her so happy.\n\n");

		DisplayText("The two of you lie together for some time, and it's with great regret that you tell her that you need to check in on your own camp. Amily seems dissapointed, not wanting you to leave, but understands why you need to go. \"<i>");
		//no pregg
		if(Flags.list[FlagEnum.AMILY_INCUBATION] === 0) DisplayText("Okay...Well, I'm sure you'll be back. I will need your help again if this doesn't set,");
		else DisplayText("Okay dear... but you'd better come back some time. You don't want your children to have abandonment issues, do you?");
		DisplayText("</i>\" Amily says, rubbing her stomach. You smile at her and nod, promising you'll come back, before setting off for your own camp.");
		*/
		//boost affection
		Flags.list[FlagEnum.AMILY_AFFECTION] += 2 + randInt(4);
		Flags.list[FlagEnum.AMILY_FUCK_COUNTER]++;
		player.orgasm();
		player.stats.sens += -1;
		return { next: Scenes.camp.returnToCampUseOneHour };
	...
	private fuckAmilyPreg() {
		...
		DisplayText("Once inside the little safe spot you help Amily undress from her restrictive garments, getting a good grope at every sensual curve your hands roam over. It's only when Amily herself is fully undressed that you remove your own gear.\n\n");
		...
		DisplayText("She pulls your head in, your lips locking in a passionate display, her hands tracing a line down to your chest until she's gently fondling your cock.  ");
		//(If player has a tail)
		if (player.torso.tailType > TailType.NONE) DisplayText("You even feel Amily wrap her mousy tail around your own, making you chuckle softly into your lovers mouth.  ");
		else DisplayText("A tickling sensation hits your body, making you snort and giggle, realising that Amily's tail is fondling your thigh.  ");
		DisplayText("Your eyes catch sight of Amily's swollen breasts, seeing a few drops of milk on her stiff nipples. You smirk and fondle her breasts, breaking the kiss every few moments to get a taste of her milk. Amily moans in response, tightening her grip on you every time your lips return to her own.\n\n");
		...
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText("\"<i>Remember when you said this cock was a bad thing?</i>\" you tease, causing Amily to blush and playfully punch you in the shoulder.  \"<i>Sh-shut up!  It's... it's pretty incredible, there, I said it.</i>\"\n\n");
		...
		DisplayText("Eventually, with great sadness and regret, you leave your lover's side and head off back to camp, vowing to return.");
		//boost affection
		Flags.list[FlagEnum.AMILY_AFFECTION] += 2 + randInt(4);
		Flags.list[FlagEnum.AMILY_FUCK_COUNTER]++;
		player.orgasm();
	...
	public amilyFollowerEncounter() {
		...
		//Jojo + Amily Spar
		if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 1 && Flags.list[FlagEnum.AMILY_MET_PURE_JOJO] === 1 && Flags.list[FlagEnum.AMILY_SPAR_WITH_PURE_JOJO] === 0 && player.statusAffects.has(StatusAffectType.PureCampJojo)) {
		...
		DisplayText().clear();
		//Non corrupt!
		if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 1) DisplayText("As you approach the mouse-woman, her big, hairless ears twitch and she turns to face you with a smile. \"<i>Hi, " + player.short + "! What's up?</i>\" She grins at you.\n\n");
		//Corrupt!
		else {
		...
		switch (pregnancy.event) {
			case 2: if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 1) { //Pure
				...
				break;
			case 3:
			case 4: DisplayText("Amily's belly is starting to protrude a little. She's unquestionably pregnant.\n\n");
				break;
			case 5:
			case 6: DisplayText("Amily's belly has gotten very big. She must be carrying more than one child.\n\n");
				break;
			case 7: if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 1) { //Pure
				DisplayText("Amily's swollen stomach moves on occasion, warranting an unthinking pat from her to calm the restless children within.\n\n");
			}
			else { //Corrupt
				DisplayText("Amily's swollen stomach moves on occasion, warranting a stroke from her to urge the restless children within to come out soon.\n\n");
			}
				break;
			case 8: DisplayText("Amily's bulge frequently wriggles and squirms, though this doesn't seem to bother her. " + (Flags.list[FlagEnum.AMILY_FOLLOWER] === 1 ? "T" : "She smiles with glee, t") + "he children mustn't have too much longer until they are born.\n\n");
			default:
		}
		amilyMenu(true);
	}
	...
	private amilyCorruptSexMenu() {
		...
		if (player.gender > 0) {
			...
			if (Flags.list[FlagEnum.FOLLOWER_AT_FARM_AMILY] === 0) return { next: playerMenu };
			else return { next: kGAMECLASS.farm.farmCorruption.rootScene };
		}
	}
	...
	private amilyAppearance() {
		...
		if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 1) {
			//(Start [horsecock]
			DisplayText("Amily is a 5' 2\" tall " + what + ", with a lean and wiry build. Her pink eyes normally twinkle merrily, but they can turn hard and cold if the circumstances warrant, just as the normally friendly grin " + onHerMuzzle + "can turn cruel and harsh when she is angry. " + furDesc + " a long, hairless mouse's tail that sways and twitches constantly from her behind. She is currently wearing " + Flags.list[FlagEnum.AMILY_CLOTHING] + ". She has " + amilyHips() + " and a " + amilyButt() + ".\n\n");
			//(End [horsecock]
			DisplayText("She has a pair of " + amilyTits() + " on her chest. They have " + Flags.list[FlagEnum.AMILY_NIPPLE_LENGTH] + "-inch nipples at their tips and must be at least " + Appearance.breastCup(Flags.list[FlagEnum.AMILY_CUP_SIZE]) + "s.\n\n");
			...
			if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) {
				...
				DisplayText("She has a " + Flags.list[FlagEnum.AMILY_WANG_LENGTH] + "-inch penis growing from her crotch, replacing the clitoris that would normally jut from her vulva. It is surprisingly human looking, " + dickDesc + " - it even has a foreskin, not a sheath.\n\n");
			}
			DisplayText("Amily has a little pink pussy in between her legs; " + stopSayingNetherlipsFuck + ".");
		}
		else {
			//Start [horsecock]
			...
			// End [horsecock]
			DisplayText("She has a pair of " + amilyTits() + " on her chest. They have " + Flags.list[FlagEnum.AMILY_NIPPLE_LENGTH] + "-inch nipples at their tips and must be at least DD-cups.\n\n");

			//(If Amily has a penis:
			if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) {
				DisplayText("She has a half-erect " + Flags.list[FlagEnum.AMILY_WANG_LENGTH] + "-inch penis growing from her crotch. It is surprisingly human looking, " + ((Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) ? "naked and hairless " : "") + "and pink and throbbing");
				if (Flags.list[FlagEnum.AMILY_HAS_BALLS_AND_SIZE] === 0) DisplayText(" - it even has a foreskin, not a sheath.\n\n");
				else DisplayText(", though it does have a" + ((Flags.list[FlagEnum.AMILY_NOT_FURRY] === 1) ? " little sheath." : " fuzzy little sheath.") + "\n\n");
			}
			if (Flags.list[FlagEnum.AMILY_HAS_BALLS_AND_SIZE] > 0) {
				DisplayText("She has " + amilyBalls() + " dangling ");
				if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText("underneath her shaft");
				else DisplayText("from her groin");
				DisplayText(", churning with cum as you gaze at them.\n\n");
			}
			//Need to account for pussy wetness here.
			DisplayText("Amily has a ");
			if (Flags.list[FlagEnum.AMILY_VAGINAL_WETNESS] === 2) DisplayText("moist, ");
			if (Flags.list[FlagEnum.AMILY_VAGINAL_WETNESS] === 3) DisplayText("wet, ");
			if (Flags.list[FlagEnum.AMILY_VAGINAL_WETNESS] === 4) DisplayText("dripping, ");
			if (Flags.list[FlagEnum.AMILY_VAGINAL_WETNESS] >= 5) DisplayText("soaked, ");
			// [Horsecock]
			DisplayText("pink pussy in between her legs; " + stopSayingNetherlipsFuck + ".\n");
			...
			if (kGAMECLASS.farm.farmCorruption.hasTattoo("amily")) {
				...
				if (kGAMECLASS.farm.farmCorruption.amilyFullTribalTats()) {
					...
					if (kGAMECLASS.farm.farmCorruption.numTattoos("amily") > 1) DisplayText("She has the following tattoos emblazoned across her body:\n");
					else DisplayText("\nShe has ");

					if (Flags.list[FlagEnum.AMILY_TATTOO_COLLARBONE] != 0) DisplayText(Flags.list[FlagEnum.AMILY_TATTOO_COLLARBONE] + "\n");
					if (Flags.list[FlagEnum.AMILY_TATTOO_SHOULDERS] != 0) DisplayText(Flags.list[FlagEnum.AMILY_TATTOO_SHOULDERS] + "\n");
					if (Flags.list[FlagEnum.AMILY_TATTOO_LOWERBACK] != 0) DisplayText(Flags.list[FlagEnum.AMILY_TATTOO_LOWERBACK] + "\n");
					if (Flags.list[FlagEnum.AMILY_TATTOO_BUTT] != 0) DisplayText(Flags.list[FlagEnum.AMILY_TATTOO_BUTT] + "\n");
				}
			}
		}
	...
	private fuckTheMouseBitch() {
		...
		if (Flags.list[FlagEnum.AMILY_WAIT_FOR_PC_FIX_JOJO] > 0) {
			DisplayText("Amily pushes you away and says, \"<i>Not until we fix Jojo.</i>\"  You sigh and grumble.  No sex today!");
			return { next: amilyFollowerEncounter };
			return;
		}

		DisplayText("You give Amily a seductive smile and tell her that you want to make love to her.\n\n");

		if (randInt(2) === 0) DisplayText("She grins wickedly back, \"<i>Ooh, is that so? Well, I suppose I might be able to squeeze you in...</i>\"\n\n");
		else DisplayText("She giggles, \"<i>What kept you so long? I've been waiting way too long.</i>\"\n\n");
		DisplayText("At your expression, she laughs, \"<i>We had about seventy kids before I moved in here; I think I'm a bit too experienced to be shy about sex now.</i>\"\n\n");

		DisplayText("She strides over to take you in her arms, \"<i>So, what would you like to do, " + player.short + "?</i>\"\n\n");

		//[Take Charge]
		//[Let Amily Lead]
		//[Make Babies - Requires player has a penis, or Amily has a penis & player has a vagina]
		if (player.torso.cocks.count > 0 || (player.torso.vaginas.count > 0 && Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0)) {
			//Make babies option
			if (Flags.list[FlagEnum.AMILY_ALLOWS_FERTILITY] === 0) {
				DisplayText("Amily is currently taking herbs that will prevent pregnancy and keep her sterile.\n\n");
				bText = "MakeBabies";
				babies = makeChildren;
				//Send make babies to an appropriate override
				if (izmaFollower() && Flags.list[FlagEnum.AMILY_X_IZMA_POTION_3SOME] === 0 && player.torso.cocks.count > 0 && !izmaScene.pregnancy.isPregnant && !pregnancy.isPregnant) {
					babies = amilyXIzmaSuperPregOptions;
				}
			}
			//No Babies Option
			else {
				DisplayText("Amily is currently off her infertility herbs and may get pregnant.\n\n");
				bText = "No Babies";
				babies = makeChildren;
			}
		}
	...
	private amilyTakesChargeSex() {
		...
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0 && player.torso.vaginas.count > 0)
		...
		DisplayText("You stride up to her and take her in your arms, kissing her deeply. She melts enthusiastically into your embrace, kissing you back just as hard, her tail winding around your " + LegDescriptor.describeLeg(player) + ". You lead her back to the nest she has made for herself and firmly but gently place her on her back there. She smiles up at you. \"<i>Ooh, taking charge, are we?</i>\" She trills with pleasure, tail waving to and fro with sincere excitement.\n\nWhat will you do?");
	...
	private takeChargeAmilyFuck() {
		...
		//(if Amily is herm:
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText("  Her pants begin to tent as her male organ reacts, and you start to idly stroke it as well for emphasis.");
		DisplayText("\n\n");

		DisplayText("Amily giggles, and then starts to wriggle out of her pants, shaking her " + amilyHips() + " and giving you a chance to grab hold of her " + amilyButt() + " as she does so. Soon, she's naked from the waist down, her little pink netherlips already winking at you");
		//(if Amily is herm:
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText(" and her " + amilyCock() + " standing erect");
		...
		DisplayText("Grinning at each other with obvious satisfaction in your eyes, you slowly relax and cuddle in the afterglow for some time, before you decide that you'll definitely repeat this soon.");
		amilyPreggoChance();
		return { next: Scenes.camp.returnToCampUseOneHour };
		player.orgasm();
		player.stats.sens += -1;
	...
	private takeChargeAmilyEatOut() {
		...
		DisplayText("She's obviously expecting you to strip down, and is surprised when you instead flop down and wriggle backwards, looking a little disconcerted as she realizes just what you have in mind. You are staring at her vagina");
		//(if herm:
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText(", and her " + amilyCock() + " is sticking up where her clitoris should be.  Y");
		else DisplayText(", and y");
		DisplayText("ou reach out with your tongue to give it a probing lick. Amily squeaks loudly at the sensation and, emboldened, you start to lick more enthusiastically, running your tongue around each of her netherlips, reaching up to tickle ");
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] === 0) DisplayText("her clitoris.");
		else DisplayText("the base of her cock.");
		DisplayText("\n\n");
		...
		//(if herm:
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText(" as her penis erupts and sprays its own cum into the air to sprinkle the both of you");
		DisplayText(".\n\n");

		DisplayText("She lies there, gasping for breath, even as you pick yourself up and start to clean yourself off. \"<i>Not my favorite...</i>\" She squeaks. \"<i>But definitely can't argue with the results.</i>\" You smile, and leave her in her nest to get her strength back.\n\n");
		return { next: Scenes.camp.returnToCampUseOneHour };
		player.stats.int += .25;
		player.stats.lust += 10;
	}
	...
	private takeChargeAmilyGetSucked() {
		...
		DisplayText("You try to gasp a warning to Amily that you are about to cum, but all that emerges from your throat is a hollow groan.  Even as cum pours from your cock into her waiting mouth, ");
		...
		//(If Amily is herm:
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText("She turns halfway back to you as she goes. \"<i>I hope you'll remember this and return the favor someday,</i>\" she calls out to you. She then resumes walking off.");
		return { next: Scenes.camp.returnToCampUseOneHour };
		player.orgasm();
		player.stats.sens += -1;
	}
	...
	private takeChargeAmilyMountHer() {
		...
		DisplayText("You collapse backwards off of Amily, waiting to regain your breath and your strength, then compliment Amily on just how good she is with her extra appendage.\n\n");

		DisplayText("\"<i>Flatterer.</i>\" Is all that she says, but she's smiling happily, even as you both clean up and go your seperate ways again.");
		//PREGGO CHECK HERE
	...
	private letAmilyLead() {
		...
		if (player.torso.cocks.count > 0) {
			//Too big
			if (x === -1) DisplayText("Amily glances down at the beast between your legs and says, \"<i>I was going to ride you, but since you're SOOO big I think I'll have to get creative...</i>\"\n\n");
		...
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0 && player.torso.vaginas.count > 0) choices[choices.length] = 5;
		//========================================
		//CHOOSE PROPER SCENE
		let scene: number = choices[randInt(choices.length)];
		//========================================
		//B1: Tailjob
		if (scene === 0) {
		...
		//B2: Hand/Footjob
			...
			DisplayText("With a satisfied smile, you turn to other things.");
			return { next: Scenes.camp.returnToCampUseOneHour };
			player.orgasm();
			player.stats.sens += -1;
			return;
		...
		//B3: Eat Out
			...
			DisplayText("Once you are at the nest, Amily pushes you down onto your back. You lay there in the soft, sweet-smelling vegetation as she strips off. Then, to your surprise, she pounces onto your chest, straddling your neck and leaving you face to face with her vagina.");
			//(if Amily is herm:
			if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText("  Her cock bobs merrily in your field of vision.");
			DisplayText("\n\n");

			DisplayText("\"<i>You've got a very sweet tongue, you know that?</i>\" Amily giggles. \"<i>Let's see just how sweet it is...</i>\"\n\n");

			DisplayText("You don't need any further nudging; you open your mouth, bend forward and stick out your tongue to give her vagina a probing lick. Despite herself, Amily squeaks at the sensation and, emboldened, you start to lick more enthusiastically, running your tongue around each of her netherlips.\n\n");

			//(If Amily is herm:
			if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText("\"<i>Could you... could you give it a lick, please?</i>\" she whispers, pointing at her male addition. Wanting to make her happy, you kiss the tip of her penis, eliciting a squeak of shock, before you start to lick from the base of her " + amilyCock() + " to the tip again. You then return your attention to her vagina itself.\n\n");
			...
			//(if Amily is herm:
			if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText("  Her penis erupts and sprays its own cum into the air to sprinkle the both of you.");
			DisplayText("\n\n");

			DisplayText("She flops over onto your belly on her back, gasping for air, leaving her crotch sitting on your neck. Finally, she recovers and sits up, dragging her damp pussy ");
			//(if Amily is herm:
			if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText("and limp cock ");
		...
		//B4: Get Ridden
			...
			DisplayText("She howls in unison with you at this final climax");
			//(if high cum amount:
			if (player.cumQ() > 500) DisplayText(", your torrents of spooge making her belly distend visibly");
			DisplayText(", and then collapses onto your front. You lie there together, arms unthinkingly winding around each other. Amily is the first of you to stir. \"<i>Do you... Are you happy, that I'm here?</i>\" You assure her that you are. \"<i>That's good...</i>\" She yawns. \"<i>Because I'm so happy to be here with you.</i>\"\n\n");

			DisplayText("Exhausted, you feel a quick nap is in order yourself. When you wake up, you're alone in the nest but Amily is nearby; she hands you some food and then points you in the direction of the stream to wash up.\n\n");
			amilyPreggoChance();
			return { next: Scenes.camp.returnToCampUseOneHour };
			player.orgasm();
			player.stats.sens += -1;
		...
		//B5: Suck Off
			...
			DisplayText("You smirk up at her, because she's fooling nobody, then lean forward and engulf the head of the " + amilyCock() + ". Amily's breath explodes out of her in shock as you start to lick and suck upon her futanari member. \"<i>You... you really aren't scared of this?</i>\" She whispers, moaning softly as you grow bolder, starting to lick the shaft, doing your best to swallow all " + Flags.list[FlagEnum.AMILY_WANG_LENGTH] + " inches of it.\n\n");
			...
			DisplayText("Amily's cock pops from your mouth as she flops over onto your chest, " + amilyTits() + " heaving as she gasps for breath. \"<i>Was... Was that...</i>\" She swallows and tries again. \"<i>Is that what it's like? ");
			if (player.torso.cocks.count > 0) DisplayText("When I do that for you?</i>\"\n\n");
			else DisplayText("For a guy?</i>\"\n\n");

			DisplayText("You shrug, uncertain. Slowly, Amily sits up and gets off of you. \"<i>I... um... thank you.</i>\" She says, then quickly steals a kiss from you before running off. She's in such a hurry that she's clear over on the other side of the camp before you can tell her that she left her pants behind.");
			return { next: Scenes.camp.returnToCampUseOneHour };
			player.stats.int += .25;
			player.stats.lust += 10;
			return;
		...
		//Let Amily Lead: Amily Mounts You
			...
			DisplayText("Once you're at the nest, Amily pushes you down onto your back. You lay there in the soft, sweet-smelling vegetation as she strips off. Then she starts tugging insistently at your pants, and you wriggle to help her. Soon, you are both naked, and Amily takes a step back, stroking her " + amilyCock() + ".\n\n");
	...
	private giveAmilyAPresent() {
		...
		if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 1) {
			...
			DisplayText("\"<i>Aw, how sweet of you.</i>\" She smiles, delighted and full of love.\n\n");
		}
		else {
			DisplayText("You look at the horny cum-bucket and wonder what you should make her take this time.\n\n");
		}
		...
		else if (player.inventory.items.has(armors.S_SWMWR) && Flags.list[FlagEnum.AMILY_OWNS_BIKINI] === 0 && player.torso.cocks.count > 0 && player.cockThatFits(61) >= 0 && !amilyCorrupt()) {
			DisplayText("You could give her a bikini, then invite her for a swim in the stream to show it off.\n\n");
			MainScreen.addButton(8, "Bikini", amilySwimFuckIntro);
			haveGift = true;
		}
		else if (Flags.list[FlagEnum.GIVEN_AMILY_NURSE_OUTFIT] === 0 && !amilyCorrupt()) {
			if (player.inventory.items.has(armors.NURSECL) && player.torso.cocks.count > 0 && player.cockThatFits(61) >= 0) {
				DisplayText("You could give Amily the nurse's outfit you got, though it barely covers anything at all, and would likely be inviting some roleplay from the kinky mouse-girl.\n\n");
				MainScreen.addButton(8, "NurseClothes", amilyNurseCheckup);
				haveGift = true;
			}
			else if (player.inventory.items.has(armors.NURSECL))
				DisplayText("You have a hunch if you had a penis that wasn't too big, giving Amily a nurse's outfit might set off some kinky roleplay.\n\n");
			else {
				DisplayText("You have a hunch that if you had a nurse's outfit you could get Amily to give you a rather erotic checkup, provided ");
				if (player.torso.cocks.count <= 0)
					DisplayText("you had a dick.");
				else if (player.cockThatFits(61) < 0) DisplayText(" your dick could fit.");
			}
		}
		if (haveGift)
			...
			if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 1)
				DisplayText("You realize that you don't have any items she would be interested in, and apologize.");
			else DisplayText("You realize you don't have any items worth using on her.");
	...
	public giveAmilyPureIncubusDraft() {
		...
		if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 1) {
			...
			if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) {
				...
				if (
					...
					DisplayText("Amily sees the vial you are holding as you approach and shakes her head.  \"<i>I'm not sure I should take any more of that stuff.</i>\"\n\n");

					DisplayText("Seeing your look of disappointment, she explains, \"<i>I'm flattered you want more of me inside you, but this thing's already nearly a fifth of my size.  If it gets any bigger, I'm going to have trouble staying conscious while erect.</i>\"\n\n");

					//[if corruption high (50+?) or high vaginal depth/looseness]
					if (player.stats.cor >= 50 || player.vaginalCapacity() >= 50) {
						DisplayText("You struggle to contain your disappointment, having entertained lurid fantasies of being stretched beyond all reason by her ever-growing member, but you can't deny her logic: an unconscious mouse-girl wouldn't be nearly as much fun, even if she was exceptionally well endowed.");
						//if corruption VERY high -75+?]
						if (player.stats.cor >= 75) DisplayText("  Although the idea of raping yourself with your futa-mouse lover's helpless body is appealing, you realize this is one argument you simply won't win.");
						DisplayText("\n\n");
					}
					//otherwise]
					else DisplayText("Her logic is undeniable, even though you had looked forward to enjoying even more of her body than before. You nod and explain that you understand perfectly; you want what's best for her above anything else.\n\n");

					DisplayText("You put the vial back in its pouch, as Amily excuses herself and walks off.");
			...
			//Normal Amily
				...
				DisplayText("You offer up the vial of purified Incubus Draft - and Amily promptly leaps back with a shrill squeak that borders on a scream in terms of volume.\n\n");

				DisplayText("\"<i>That's liquid corruption!</i>\" She protests. \"<i>Have you gone insane? I'm not drinking that, and you shouldn't either!</i>\"\n\n");

				DisplayText("You hastily assure her that it is purified and so neither of you have to worry about joining the ranks of the demons. She still looks skeptical, but then nods slowly and approaches you.\n\n");

				DisplayText("\"<i>All right... but, are you sure you want to give that to me? You know it will make me grow a penis, right?</i>\"\n\n");
		...
		//CORRUPT
			...
			if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] === 0) {
				...
				DisplayText("Amily stops her moaning; apparently the transformation is over... But it's such a small dick... Only four inches long and barely one inch thick...\n\n");

				DisplayText("Do you make her grow a huge one?");
			...
			//Too big!
				...
				DisplayText("You reconsider - her small frame probably couldn't handle anything larger.");
				return { next: amilyFollowerEncounter };
			}
		}
	}
	...
	private giveAmilyPureIncubusDraft4Realz() {
		...
		DisplayText("Amily is now a hermaphrodite. Her human-like penis is four inches long and one inch thick.\n\n");

		DisplayText("Catching her breath, she stares at her new appendage with an unreadable expression, then pulls her clothes back on with a grimace. You decide to give her some time alone to adjust to the change.");
		amilyDickGrow();
		return { next: amilyFollowerEncounter };
	}
	...
	public giveAmilyPurifiedSuccubusMilk() {
		...
		if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 1) {
			DisplayText("You offer her a vial of demonic milk, assuring her as you do so that the corruptive elements have been removed from it.\n\n");
			...
			//(If Amily's breast size is DD-cup):
				...
				DisplayText("She looks thoughtful for a moment, then shakes her head reluctantly. \"<i>I'm sorry darling, I really am, but I think I've got big enough breasts as it is.</i>\" She then smirks, and playfully jiggles her abundant cleavage. \"<i>Don't you agree?</i>\" she teases.\n\n");
				DisplayText("Swallowing hard, you have to agree, which makes Amily laugh.");
				player.stats.lust += 5;
			}
		}
		//CORRUPT UNZ
			...
			else DisplayText("  Juices flood out of her pussy like water comes out of a spring, running down her body and pooling underneath her.");
			DisplayText("\n\n");

			//(if Amily's wetness < 4 (squirter))
			if (Flags.list[FlagEnum.AMILY_VAGINAL_WETNESS] < 5) {
				DisplayText("You let go of Amily's legs and she plops down in her pooled juices. You observe her as she comes down from her afterglow and realize her pussy is now much wetter; you're tempted to make her use her newly lubed up pussy, but refrain from doing so.\n\n");
				DisplayText("It'll be more fun to let her lust build; no doubt her pussy must be extra sensitive after this healthy dose of milk. You toss the vial away and casually stride back into the camp, leaving Amily panting on her juices.");
			}
			//(else)
			else {
				...
				DisplayText("Looks like giving her more milk only makes her pussy sensitive. You're tempted to make use of her new, sensitive pussy, but refrain from doing so. It'll be more fun to let her lust build. You toss the vial away and casually stride back into the camp, leaving Amily panting in her juices.");
			}
		}
		return { next: amilyFollowerEncounter };
	}
	...
	private amilyDrinksSuccubusDelight() {
		...
		if (Flags.list[FlagEnum.AMILY_HAS_BALLS_AND_SIZE] === 0) {
			...
			DisplayText("You decide she needs a little push if she's going to have balls. \"<i>Lay down bitch,</i>\" you order her; being called a bitch only makes her smile as she complies.\n\n");
		...
		//Too much
			...
			DisplayText("You realize that giving Amily any more Succubi's Delight would make her practically immobile.  No, she doesn't need any more.  Maybe once you've secured a more appropriate place to keep an over-endowed harem.");
		}
	...
	public giveAmilyAPinkEgg() {
		...
		if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 1) {
			...
			DisplayText("\"<i>Thank you, that's such a relief. So, anything else you wanted?</i>\" she squeaks merrily, quite clearly feeling very cheerful now.");
		}
		//Corrupt
		else {
	...
	public giveAmilyAWhiteEgg() {
		...
		DisplayText("You hold out a white egg, telling her that it will make her nipples grow.\n\n");
		//Pure
		if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 1) {
			...
			//(If Nipples are 4 inches:
				...
				DisplayText("She shakes her head. \"<i>I'm sorry, but I think these are big and sensitive enough already. I'm a mouse, not a cow.</i>\" At that, she insistently walks away, clearly not going to take the egg. You return it to your pocket and decide to let her be.");
			}
		}
		//Corrupt!
		else {
	...
	public giveAmilyABrownEgg() {
		...
		DisplayText("You hold out a brown egg, telling her that it will make her butt grow.\n\n");
		if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 1) {
			...
			if (Flags.list[FlagEnum.AMILY_ASS_SIZE] < maxSizePure || (Flags.list[FlagEnum.AMILY_ASS_SIZE] < maxSizeHypr && Flags.list[FlagEnum.HYPER_HAPPY])) {
				...
				if (player.inventory.items.has(consumables.BROWNEG)) {
					...
					Flags.list[FlagEnum.AMILY_ASS_SIZE] += 1 + randInt(2);
				}
				else {
					player.inventory.items.consumeItem(consumables.L_BRNEG);
					Flags.list[FlagEnum.AMILY_ASS_SIZE] += 2 + randInt(3);
				}
			...
			//(If Amily's butt is "delightfully jiggly":
				...
				DisplayText("She looks at the egg, and then shakes her head. \"<i>Sorry, but I've got more than enough junk in my trunk for my taste.</i>\" Sensing she won't back down on this, you put the egg back. \"<i>So, is there anything else you wanted?</i>\" She asks.");
			}
		}
		//IMPURE
		else {
	...
	public giveAmilyAPurpleEgg() {
		...
		DisplayText("You hold out a purple egg, telling her that it will make her hips grow.\n\n");
		if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 1) {
			...
			if (Flags.list[FlagEnum.AMILY_HipRating] < maxSizePure || (Flags.list[FlagEnum.AMILY_HipRating] < maxSizeCorr && Flags.list[FlagEnum.HYPER_HAPPY])) {
				...
				if (player.inventory.items.has(consumables.PURPLEG)) {
					...
					Flags.list[FlagEnum.AMILY_HipRating] += 2 + randInt(3);
				}
	...
	*/
	...
	public giveAmilySomePants() {
		...
		//(If Amily is wearing anything other than Tattered Rags:
			...
			DisplayText("\"<i>More new clothes? Ooh, you're spoiling me, " + player.short + "</i>!\" she teases you. Unabashed in the slightest at being naked in front of you, she strips down, doing her best to give you a little show as she does so. She then redresses herself in her new offering. \"<i>How do I look?</i>\" she giggles.\n\n");
			DisplayText("You assure her that she looks beautiful. \"<i>Flatterer.</i>\" She smirks, and then wanders off to the stream.");
		}
	...
	private makeTheMouseAMilkCowMoo():void {
		DisplayText().clear();
		DisplayText("\"<i>Lactaid? You really want to try drinking mouse milk?</i>\" Amily asks, a little uncertainly, a little playfully.  You nod and assure that you do.  \"<i>Alright, if you insist.</i>\" She proclaims, taking the pink bottle from you and drinking the creaminess to be found within.\n\n");
	...
	*/
	...
	private amilyHips(): string {
		...
		rando = randInt(2);
		if (rando === 0) {
			if (randInt(2) === 0 && Flags.list[FlagEnum.AMILY_HipRating] >= 15) desc += "flanks";
			else desc += "hips";
		}
		if (rando === 1) {
			if (randInt(2) === 0 && Flags.list[FlagEnum.AMILY_HipRating] >= 15) desc += "flanks";
	...
	private amilyBalls(): string {
		...
		//size!
		if (Flags.list[FlagEnum.AMILY_HAS_BALLS_AND_SIZE] > 1 && randInt(3) <= 1) {
	...
	private corruptAmilyOralSuckOff() {
		...
		//[(if PC has a pussy)
		if (player.torso.vaginas.count > 0) {
			DisplayText("; getting some of your juices on her " + ((Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) ? "muzzle" : "lips"));
			if (player.torso.balls.quantity > 0) DisplayText(" and ");
		}
	...
	private corruptAmilyLickPussiesLikeAPro() {
		...
		//, [(if PC has a cock)
		if (player.torso.cocks.count > 0) {
			DisplayText(", your cock springing up into attention");
			if (player.torso.balls.quantity > 0) DisplayText(" and ");
		}
		//[(if PC has balls)
		...
		if (player.vaginalCapacity() >= 45 && Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) {
			...
			// BEGIN [Horsecock]
			DisplayText("You giggle and decide to take things one step further; you ");
			if (Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0)
		...
		DisplayText("Amily's rather desperate and eager ministrations are effective and you feel yourself on the edge of orgasm.  ");
		...
		DisplayText("You chuckle and dismiss her with a wave.");
		player.orgasm();
		player.stats.sens += -1;
		player.stats.cor += 1;
		return { next: Scenes.camp.returnToCampUseOneHour };
	...
	private corruptAmilyScissorsLikeAPro() {
		...
		//[(if Amily has a cock)
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText(" Amily's " + amilyCock() + " throbs,");
		DisplayText(" and she smiles once she gets what you want to do.\n\n");

		DisplayText("You examine Amily's crotch closely, ");
		//[(if Amily has a cock)
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) {
			DisplayText("idly stroking her " + amilyCock());
			//(if Amily has balls)
			if (Flags.list[FlagEnum.AMILY_HAS_BALLS_AND_SIZE] > 0) DisplayText(" and " + amilyBalls() + ", which you shift to reveal her ");
		...
		//[(if Amily's a squirter)
		if (Flags.list[FlagEnum.AMILY_VAGINAL_WETNESS] >= 5) DisplayText("  A veritable jet of juices shoots up, splashing against your netherlips, some of it even making it inside.");
		else DisplayText("  Wet squeltches and splats resound around you, as the results of Amily's orgasm flood what little space remains between the two of you.");
		//[(if Amily has a cock)
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText(" Her " + amilyCock() + " throbs and cum splashes on her breasts and face.");
		DisplayText("\n\n");
	...
	private corruptAmilysPussyGetsMotherfuckingFucked() {
		...
		//[(If Amily is pregnant)
		if (pregnancy.isPregnant) DisplayText("  You wonder if your unborn children will appreciate their bath. Their mother certainly does.");
		//(If not squirter:
		if (Flags.list[FlagEnum.AMILY_VAGINAL_WETNESS] < 5) DisplayText("  Her fluids slop wetly over your crotch, painting between your legs with her lubricant.");
		//(If squirter:
		else DisplayText("  A cascade of fluids pours from inside her; if you had allowed her to ride atop you, you'd be sopping with her femcum.");
		//(If Amily is herm:
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText("  It doesn't take long - seconds, at most - before her pulsing, twitching cock finally explodes, spewing gush after gush of herm-spunk all over her tits and face, which Amily eagerly laps up.");
		DisplayText("\n\n");

		DisplayText("You orgasm finally ends.");
		//[(if PC has large cum amount)
		if (player.cumQ() >= 1000) {
			//(if Amily is not pregnant)
			if (!pregnancy.isPregnant) DisplayText("  Amily's womb is so full of cum that it's bloated as if she was carrying a full litter of mousy sluts ready to birth.");
			//(else)
			else DisplayText("  Amily's belly looks ready to explode; besides the litter she's carrying, you've also filled her with enough cum to feed the little mousy sluts for week.");
		}
		...
		DisplayText("</i>\" she tells you softly.\n\n");
	...
	private corruptAmilyCampBonesPCWithHerCock() {
		...
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] < 6) DisplayText("small");
		else if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] < 9) DisplayText("average");
		...
		//[(if PC is a squirter)
		if (player.torso.vaginas.get(0).wetness >= 5) DisplayText("  Your jets of femcum wash down all the seed Amily spills inside you, pushing it all out around her cock and painting Amily's lower body in juice and cum.");
		DisplayText("  Slowly Amily withdraws, her cock finally beginning to shrink as she kneels subserviently before you; head down in shame and guilt at her earlier outburst.\n\n");

		DisplayText("\"<i>Fetch me a rope.</i>\" You order her; Amily quickly gets up and runs off, returning with a rope in hand. \"<i>Now kneel,</i>\" you order her once again. She does as you say and awaits her punishment patiently.");
		...
		//[(if PC has a dick)
		if (player.torso.cocks.count > 0) DisplayText("  If she did, you might have to return the favor...");
		//(else)
		else DisplayText("  If she did, you'll just have to tie her up and get someone to return the favor...");
	...
	private corruptAmilyBuckFutter() {
		...
		else DisplayText("  Some cum begins leaking out of her ass, before she plugs herself with a hand.");
		DisplayText("\n\n");

		DisplayText("\"<i>Finish your work,</i>\" you order the tired mousette. She spins around");
		//[(if PC has large cum amount)
	...
	private amilyLesboLetHerGo() {
		...
		if(player.hasStatusAffect(StatusAffects.CampMarble) >= 0 || urtaLove()) {
			DisplayText("You put a hand on her shoulder, bringing her to a stop. She looks so hopeful at you that it's almost painful, but you tell her that, while you do care for her and you like her as a friend, you're already in a relationship with somebody.\n\n");

			DisplayText("\"<i>Are you? ...I see. Well, I'm happy that you, at least, found somebody. I... You're still welcome to come by and talk, but I'll respect your wishes.</i>\" Amily tells you. Evidently still quite embarrassed, she apologises and then melts away into the ruins again.");
			//(Amily is now locked out of a relationship with the player)
		}*/
		DisplayText("You watch her go, feeling a little guilty, but you just don't swing that way. You can only hope she'll be all right.\n\n");
	...
	private amilyPostConfessionGirlRemeeting() {
		...
		DisplayText("You agree that it is, then ask if something is the matter.\n\n");

		DisplayText("Amily scuffs the ground nervously. \"<i>It's like this... You know I love you, don't you? But that I also want - I *need* - to have children to resurrect my race?</i>\"\n\n");

		DisplayText("You nod your agreement and ask her what exactly she means.\n\n");
	...
	private amilyOnGirlSurpriseBonerAcceptance() {
		...
		DisplayText("She still starts leading you away, though.");
	...
	private amilyOnGirlSurpriseBonerREJECT() {
		...
		DisplayText("Amily stops, her new cock wilting, her expression making it quite obvious that she's heartbroken. Her head falls, tears dripping from her eyes, and she turns and runs away. You glare after her as she vanishes, sobbing, into the ruins, hoping she never comes back.");
	...
	public girlyGirlMouseSex() {
		...
		//(If first time:
		if (Flags.list[FlagEnum.AMILY_TIMES_FUCKED_FEMPC] === 0) DisplayText("Amily stops and lets go of your hand, blushing faintly and looking embarrassed. \"<i>So, ah... how do we do this? I... I've never been attracted to another woman before, how does sex even work between us?</i>\"\n\n");
		else DisplayText("\"<i>Remember how you had to take charge the first time?</i>\" She grins. \"<i>Care to see if you've still got it?</i>\"\n\n");
		...
		if (Flags.list[FlagEnum.AMILY_TIMES_FUCKED_FEMPC] === 0) DisplayText("\"<i>...I didn't know it could feel so good with another woman.... But I was never attracted to women before.</i>\" Amily murmurs to herself.\n\n");
		else DisplayText("\"<i>...Does it make me a lesbian, that I love this so much? Or am I just so lonely for company that even another woman is good?</i>\" Amily asks. Then she musters the energy to shake her head. \"<i>It doesn't matter. I love you.</i>\"\n\n");

		DisplayText("Your own strength returning to you, you sit up and smile at your mousey lover before giving her a deep kiss, tasting your juices and letting her get a taste of her own. Then you redress yourself and return to your camp.");
		player.orgasm();
	...
	public hermilyOnFemalePC() {
		...
		else DisplayText("\"<i>It gets me every time when that happens. Is this what it's like for men?</i>\" she wonders.\n\n");

		DisplayText("You smile and reach up to stroke her cheek. She smiles back and reaches down to pat you on your belly.");
		//(If player is preg
		if (player.isPregnant()) {
			if (player.pregnancyType === PregnancyType.AMILY)
				DisplayText("\"<i>Boy, this is weird.  I'm a woman and I'm going to be a dad.");
			else DisplayText("\"<i>After you give birth to this baby come and see me when you're ready for mine.  This is really weird, I'm a woman and I can't wait to be a dad.");
		}
		//not preg yet!
		else {
			DisplayText("\"<i>Let's see if you'll be a mommy from this load... If not, well, I guess we'll have to try again.");
			//PREGGO CHECK HERE
			player.knockUp(PregnancyType.AMILY, PregnancyType.INCUBATION_MOUSE);
		}
		DisplayText("</i>\"  Chuckling softly, you lay there and embrace your lover for a time and then, reluctantly, you get dressed and leave.");
		player.orgasm();
	...
	public pcBirthsAmilysKidsQuestVersion() {
		...
		if (Flags.list[FlagEnum.PC_TIMES_BIRTHED_AMILYKIDS] + Flags.list[FlagEnum.AMILY_BIRTH_TOTAL] >= 5) {
			...
			DisplayText("\"<i>Get some rest, darling. There are things you and I need to talk about,</i>\" Amily instructs you.\n\n");

			DisplayText("You are eager to comply, though your last thought as you sink into unconsciousness is to wonder what Amily wants to talk about.");
	...
	public postBirthingEndChoices() {
		...
		//(if player is female:
		if (player.gender === Gender.FEMALE) DisplayText("another woman");
		//, if player is herm:
		else DisplayText("a hermaphrodite");
		DisplayText(", but... I love you. The children, they're going to leave here now, and set up a new village somewhere else. But I... I want to stay here with you. Forever. Please, say yes.</i>\"\n\n");
		DisplayText("Do you accept her offer?");
		Flags.list[FlagEnum.PC_TIMES_BIRTHED_AMILYKIDS]++;
		MainScreen.simpleChoices(["Accept", "StayFriends", "ShootDown", "", ""], [acceptAmilyAsYourFemaleWaifu, declineButBeFriends, notInterestedInDumbshitMouseBitches, null, null]);
	}
	...
	private acceptAmilyAsYourFemaleWaifu() {
		...
		DisplayText("\"<i>Well, I better start moving in, huh?</i>\" she jokes. She then flops down on your sleeping roll beside you, \"<i>There we are, I'm moved in.</i>\" She grins at you, and you can't help but laugh.\n\n");
	...
	private declineButBeFriends() {
		...
		DisplayText("Amily doesn't look entirely happy, but you assure her that you will keep coming back to see her. And when you tease at the possibility of a few more litters in your respective futures, stroking her penis through her tattered pants, she blushes but agrees to go.\n\n");
	...
	private notInterestedInDumbshitMouseBitches() {
		...
		DisplayText("Amily reels, heartstruck, her expression making it clear that her heart has shattered, tears rolling down her face. \"<i>I...I didn't know that was the way you felt about me. F-Fine, if that's how it is...</i>\" She bursts into sobs and runs away; you know she'll never come back.\n\n");
	...
	private amilyNewGenderConfrontation() {
		...
		DisplayText("Suddenly, Amily stops and looks puzzled, her nose twitching. \"<i>Have you changed...? Yes, you have! You've been messing with some of those weird potions and things that show up here and there - you've altered your gender, haven't you?</i>\"\n\n");

		DisplayText("You nod your head and admit you have.\n\n");
		//Male tooo
		if (oldGender === 1) {
			//[Male To Female]
			if (player.gender === Gender.FEMALE) {
				//Low Affection:
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) {
					DisplayText("\"<i>Oh, great. Now what am I going to do with you? Why on earth would you stuff this up?</i>\" Amily complains. She shakes her head. \"<i>Come back later - I'm too frustrated to talk to you now.</i>\" She storms away and you decide it would be best to take her advice.\n\n");
					//(Player must now begin the Female Quest from the beginning.)
				}
				//Medium Affection:
				else if (Flags.list[FlagEnum.AMILY_AFFECTION] < 40) {
					DisplayText("\"<i>I... I... why would you do that?</i>\" Amily asks, looking hurt. \"<i>I... no, This wasn't an effort by you to hurt me, I'm sorry, I was being selfish.</i>\" She apologizes. \"<i>But... I don't know what we can do any more. I... I need time to think.</i>\" She turns and walks away, and you decide to give her what she asks for.");
			...
			//[Male To Herm]
				...
				//Low Affection:
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) {
					DisplayText("\"<i>...Are you a herm, now?</i>\" She asks, sounding appalled. When you confirm it, she grimaces in disgust. \"<i>Stay away from me! You're not coming near my bed again until you're all man again!</i>\" She orders, and then storms off.\n\n");
					//(Player must now begin the Herm Quest from the beginning.)
				}
				//Medium Affection:
				else if (Flags.list[FlagEnum.AMILY_AFFECTION] < 40) {
					DisplayText("She looks intimidated. \"<i>I... I'm sorry, but I don't think I can share my bed with you, not any more. Please, find a way to become male again, then come back to me?</i>\" She pleads, then slips away.\n\n");
					//(Amily's affection score remains unchanged, but the player must make the "What's Wrong With Herms" scene.)
				}
				//High Affection:
				else {
					DisplayText("\"<i>I...</i>\" She swallows hard. \"<i>This is a great shock, I must confess, but... But I care too much to lose you. I don't care if you've got a pussy of your own, now. I still want to be with you.</i>\" She smiles at you, feebly. \"<i>So, as I was saying, what do you want to talk about?</i>\"\n\n");
			...
			//[Any to Genderless]
				...
				//Low Affection:
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) {
					DisplayText("She looks at you in disdain. \"<i>How can you be so stupid as to completely remove all gender from yourself? Get out of my sight and don't come back until you're one gender or the other again!</i>\" She then proceeds to storm off.\n\n");
					//(Amily will repeat this scene on each remeeting until the player becomes a gender other than Genderless.)
				}
				else if (Flags.list[FlagEnum.AMILY_AFFECTION] < 40) {
					DisplayText("She shakes her head sadly. \"<i>I guess this kind of puts a kink in our relationship, doesn't it? Still, I'll always be willing to talk with you.</i>\"\n\n");
					//(The player can only Talk with Amily on each remeeting until they have become a gender other than Genderless.)
				}
				//High Affection:
				else {
					DisplayText("She looks upset and concerned - but for your sake, not hers. \"<i>I can't imagine what catastrophe robbed you like this. Please, find a way to change yourself back? Man, woman, even herm, I can't bear to see you like this... but I'll give you all the support I can.</i>\"\n\n");
					//(The player can only Talk with Amily on each remeeting until they have become a gender other than Genderless.)
				}
			}
		}
		//[Female...
			...
			if (player.gender === Gender.MALE) {
				//Low Affection:
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) {
					DisplayText("Amily looks deeply surprised. \"<i>You... you turned yourself from a woman into a man? ...For me?</i>\" She scuffs her foot at the ground in embarrassment. \"<i>I... I don't know what to say. But... will you hear me out, now that you have changed?</i>\"\n\n");
				...
				//Medium Affection:
					...
					DisplayText("She looks surprised, unsure of what to say. \"<i>I... thank you. I do feel we've grown closer, but this will make things much easier...</i>\"");
					//(Begin Male variant of Amily's quest.)
					//FEN: Increase affection!
					Flags.list[FlagEnum.AMILY_AFFECTION] += 5;
					//FEN: If PC has had any kids with her, set as good to go!
			...
			//[Female to Herm]
				...
				if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] === 0) {
					//Low Affection:
					if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) {
						DisplayText("She looks at you for a long time, then shakes her head in disbelief. \"<i>What woman in her right mind would grow a dick? Ah, well, get rid of the pussy, and then you and I may have something to talk about. But, for now, we'll just talk.</i>\"");
						//(Begin herm variant of Amily's quest.)
					}
					//Medium Affection:
					else if (Flags.list[FlagEnum.AMILY_AFFECTION] < 40) {
						DisplayText("\"<i>I... don't take this the wrong way, " + player.short + ", but... I'm not so sure we can be together any more while you have that. Be one thing or the other, not both.</i>\" Amily states. \"<i>But we can still talk, this time.</i>\"");
				...
				//Amily grew a dick for you.
					...
					DisplayText("Amily looks you up and down, blushes and says, \"<i>Did you get a little jealous of me and decide to have some fun for yourself?  I-I didn't want it to be this way, but I guess we can both repopulate my race now.  How wonderful.</i>\"");
					Flags.list[FlagEnum.AMILY_HERM_QUEST] = 2;
		...
		//Herm toooooooo
			...
			if (player.gender === Gender.MALE) {
				//Low Affection:
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) {
					DisplayText("She looks you over and smiles. \"<i>Well, now that's more like it. You and I, we need to talk...</i>\"");
				...
				//Medium Affection:
					...
					DisplayText("\"<i>And here I was starting to get used to you like that... but I'm happy you made such a change for me.</i>\" She tells you, smiling.\n\n");
					Flags.list[FlagEnum.AMILY_AFFECTION] += 2;
					//mark as agreed to preg-quest!
					Flags.list[FlagEnum.AMILY_OFFER_ACCEPTED] = 1;
					//(Use the Remeeting scene options.)
					if (player.stats.lust >= 33) sex = sexWithAmily;
					if (sex != null) MainScreen.simpleChoices(["Sex", "Talk", "Both", "", ""], [sex, talkToAmily, talkThenSexWithAmily, null, null]);
					else MainScreen.simpleChoices(["", "Talk", "", "", ""], [null, talkToAmily, null, null, null]);
					return;
				}
				//High Affection:
				else {
					DisplayText("\"<i>I was comfortable with who you were, you didn't have to change on my account...</i>\" Amily says, clearly looking guilty. When you assure her that you did this voluntarily, she brightens up. \"<i>Well, I am happy to have you all man - so, what were you wanting to speak about?</i>\"");
					//(Use the Remeeting scene options.)
					Flags.list[FlagEnum.AMILY_AFFECTION] += 2;
					//mark as agreed to preg-quest!
					Flags.list[FlagEnum.AMILY_OFFER_ACCEPTED] = 1;
			...
			//[Herm to Female]
				...
				//Low Affection:
				if (Flags.list[FlagEnum.AMILY_AFFECTION] < 15) {
					DisplayText("\"<i>Well, I guess it's nice to see another woman around... though I could have used you as all male. So, do you want to talk?</i>\" Amily asks.\n\n");
				...
				//Medium Affection:
					...
					DisplayText("\"<i>You didn't need to change yourself for my sake... but, I do like having somebody who can really understand what life in this world is like.</i>\" Amily notes.");
					//(Amily's affection remains unchanged, but the quest switches to the female variant.)
				}
				//High Affection:
				else {
					DisplayText("Amily looks kind of disappointed. \"<i>I will always love you no matter who you are, but... I was kind of used to that nice cock of yours, love.</i>\" She shakes her head. \"<i>Ah, well, if it's you, then sex is sex to me.</i>\" She smiles.");
	...
	private whyNotHerms() {
		...
		DisplayText("\"<i>You looked deep in thought; what's the matter?</i>\" She asks.\n\n");

		DisplayText("You remind her of how, when you first met, she spoke of her having to take \"<i>a role</i>\" in freeing her people, and how she was about to ask you for help, but she stopped when she realized you were a hermaphrodite.\n\n");
		...
		DisplayText("You ask if that is why she was particularly interested in you being human.\n\n");

		DisplayText("\"<i>Yes. I've heard that humans are both inherently pure and very weak-seeded; if I could find a human man before the demons caught him, he would be the perfect mate to help me with that goal, because the children he fathered on me would be pure in both senses.</i>\"\n\n");

		DisplayText("Crossing your arms, you sarcastically ask what you are; chopped liver?\n\n");

		DisplayText("Amily stands up and does her best to look you in the eyes, crossing her arms as well. \"<i>You're not a man - you're a hermaphrodite. I can't lie with you - what if the babies inherit that trait?</i>\"\n\n");

		DisplayText("You ask if that wouldn't be an advantageous trait, allowing each individual to both impregnate and be impregnated and letting their population swell all the faster.\n\n");

		DisplayText("\"<i>But it's unnatural!</i>\" She barks... well, squeaks indignantly, anyway. \"<i>Women with cocks, men with cunts - before those fucking demons, you never saw creatures like that! They're not normal! I mean, you don't seem to be a bad person, but I could never have sex with someone like that!</i>\"\n\n");

		DisplayText("At that, she turns and runs off, quickly vanishing into the rubble. You choose not to pursue; it seems she's clearly not in the mood to talk about it.\n\n");
		return { next: Scenes.camp.returnToCampUseOneHour };
	...
	private fuckNoYouWontBeAmilysHermDaddy() {
		...
		DisplayText("Amily winces, looking deeply hurt. \"<i>I... You're right, what I said was unforgivable. I... think it's best that we part ways.</i>\"\n\n");

		DisplayText("Looking deeply sad, she turns and walks away, vanishing into the urban wilderness in that way only she can. Instinctively, you realize that you will never see her again.");
		Flags.list[FlagEnum.AMILY_VILLAGE_ENCOUNTERS_DISABLED] = 1;
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	public makeAmilyAHerm() {
		...
		DisplayText("\"<i>That's true... I... I can't say I can understand what life must be like for you like that.</i>\" She admits.\n\n");

		DisplayText("You ask if she would be willing to try and see your view on things - you happen to have a vial of Incubus Draft from which the tainting elements have been removed.\n\n");

		DisplayText("She looks very nervous. \"<i>I... I mean... I don't really want to do that.</i>\"\n\n");
	...
	private playerBirthsWifAmilyMiceInCamp() {
		...
		DisplayText("Pain shoots through you as they pull open your cervix forcefully, causing you to cry out involuntarily. At once, Amily suddenly appears, racing over from across the camp. \"<i>Is it time? Are you going into labor?</i>\" She asks, worry evident in her voice.\n\n");

		DisplayText("You grunt in affirmation with a trace of irritation; ");
		//Repeat brood
		...
		DisplayText("\"<i>Because there are only trace elements of those chemicals left in our bodies, they should age normally from this point on.</i>\" Amily notes.\n\n");

		DisplayText("Worry fills you; how on earth are you going to take care of such small, defenseless creatures in a place like this? Before you can voice such concerns to Amily, though, rustling in the bushes catches your attention. As you watch, a pair of young " + ((Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) ? "mouse-morphs" : "mouse fellows") + " appear and walk confidently into your camp. They're two of your children, the ones who Amily told you had left the area.\n\n");

		DisplayText("\"<i>I hope you don't mind, " + player.short + ", but I contacted them to come and take their new siblings back with them shortly after you told me you were pregnant.</i>\" Amily says, sheepishly. \"<i>We just can't really care for them here, and they'll be safer and happier with all their siblings.</i>\"\n\n");
	...
	public amilyPopsOutKidsInCamp() {
		...
		if (Flags.list[FlagEnum.AMILY_FOLLOWER] === 1) {
			...
			DisplayText("You favor your lover with a reassuring smile, placing the baby to her breast with the practice of experience, then resume catching the next one to emerge from her.\n\n");
			...
			DisplayText("\"<i>Because there are only trace elements of those chemicals left in my bloodstream, they developed and grew a little faster than normal, but they should age normally from this point on.</i>\" Amily notes.\n\n");

			DisplayText("Worry fills you; how on earth are you going to take care of such small, defenseless creatures in a place like this? Before you can voice such concerns to Amily, though, rustling in the bushes catches your attention. As you watch, a pair of young " + ((Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) ? "mouse-morphs" : "mouse fellows") + " appear and walk confidently into your camp. They're two of your children, the ones who Amily told you had left the area.\n\n");

			DisplayText("\"<i>I hope you don't mind, " + player.short + ", but I contacted them to come and take their new siblings back with them shortly after we confirmed I was pregnant.</i>\" Amily says, sheepishly. \"<i>We just can't really care for them here, and they'll be safer and happier with all their siblings.</i>\"\n\n");
		...
		//CORRUPT!
			...
			//[If Amily has balls:
			if (Flags.list[FlagEnum.AMILY_HAS_BALLS_AND_SIZE] > 0) DisplayText("  Her testicles shake and quiver, swaying in the breeze as the pussy behind them readies itself to disgorge the life crammed into her swollen stomach above.");
			DisplayText("  Her eyes are rolled up in her head and her face is contorted into an expression of unholy orgasmic delight.\n\n");

			DisplayText("With a lewd moan, Amily pants, \"<i>Yes, it's coming! It's coming!!</i>\" Then she loses her balance and falls onto her back, her legs spreading wide; as if she wanted the whole world to see what's about to take place. And then it begins... A gush of fluids");
			//[(if Amily has a dick)
			if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText(" followed by a spurt of cum");
			DisplayText(" signals the coming of the first mouse. The head topped with wide mousy ears appears from between Amily's stretched lips, its small hands follow in suit;");
			//[(if Amily has balls)
			if (Flags.list[FlagEnum.AMILY_HAS_BALLS_AND_SIZE] > 0) DisplayText(" it grabs Amily's balls and uses them to try and pull itself out;");
			DisplayText(" its efforts to leave its mother's leaking canal are finally paid off when a jet of fluids slicken its lower half, making its exit easier. As soon as it's out, another mousy head starts showing; Amily moans and screams her pleasure as the process repeats itself over and over.\n\n");
	...
	public makeChildren() {
		...
		DisplayText().clear();
		if (Flags.list[FlagEnum.AMILY_ALLOWS_FERTILITY] === 1) {
			DisplayText("You ask Amily if she wouldn't mind going back on her herbs for now.  The mousegirl snickers and asks, \"<i>Had enough babies huh?</i>\" before she gives you a knowing wink.  Amily assures you, \"<i>Don't worry " + player.short + ", we've brought more than enough life into the world.  I don't mind a break from it.</i>\"");
	...
	private meetAmilyAsACorruptAsshat() {
		...
		DisplayText("Curious about how Amily is holding up, you head back into the ruined village. This time, you don't bother making any secret of your presence, hoping to attract Amily's attention quicker. After all, she did say that the place is basically empty of anyone except her, and you can handle a measly Imp or Goblin.\n\n");
		...
		if (!player.perks.has(PerkType.Evade) && !player.perks.has(PerkType.Flexibility)) {
			DisplayText("Amily's sudden reaction catches you off guard and the dart hits you; almost immediately you feel your body going stiff. Amily doesn't even wait to see if she hit you or not before running away, yelling back at you.\n\n");

			DisplayText("\"<i>Don't come near me again!  You're tainted, ruined!</i>\" Her voice is panicked, as she disappears into the ruins.\n\n");

			DisplayText("It takes almost two hours before you are able to move again, and your joints are still stiff and painful as you hobble back to camp, cursing that mouse. Maybe you should teach her not to mess with you.\n\n");

			//-(if PC's int >= 40)
			if (player.stats.int >= 40) DisplayText("You begin to formulate a plan, but to put this plan into motion you'll require a Lust Draft and some Goblin Ale. That damn rat is going to rue the day she denied you.\n\n");
			//-(else)
			else DisplayText("You think about some of the more interesting potions you found while exploring; perhaps you could use some of them...");
		}
		//(else)
		else {
			DisplayText("Despite Amily's fast reactions, they're no match for your trained reflexes; you bend just enough for the dart to barely graze you. Amily doesn't even wait to see if she hit you or not before running away, yelling back at you.\n\n");

			DisplayText("\"<i>Don't come near me again!  You're tainted, ruined!</i>\" Her voice is panicked, as she disappears into the ruins.\n\n");

			DisplayText("That stupid cunt; how dare she attack you after asking you to help her! So what if you are aren't so pure anymore; you're still very much capable of helping her! Maybe you should teach her not to mess with you...?\n\n");
	...
	private cookAmilyASnack() {
		...
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00169] > 0) {
			...
			if (!(player.inventory.items.has(consumables.L_DRAFT) || player.inventory.items.has(consumables.F_DRAFT)) || !player.inventory.items.has(consumables.GOB_ALE)) {
				DisplayText("You think about going into the Ruined Village, but you don't have the ingredients to create more of Amily's medicine. You return to your camp.");
				return { next: Scenes.camp.returnToCampUseOneHour };
				return;
			}
			//(if PC is genderless and has the ingredients.)
			else if (player.gender === Gender.NONE) {
				DisplayText("You think about going into the Ruined Village, but without a cock or a pussy you can't complete the mixture. You return to your camp.");
				return { next: Scenes.camp.returnToCampUseOneHour };
				return;
			}
			//(else)
		...
		//First Time
			...
			if (!(player.inventory.items.has(consumables.L_DRAFT) || player.inventory.items.has(consumables.F_DRAFT)) || !player.inventory.items.has(consumables.GOB_ALE)) {
				DisplayText("You think about going into the Ruined Village, but decide it's best to wait until you have a plan underway (maybe some lust draft and a goblin ale to get the ball rolling... you return to your camp.");
				return { next: Scenes.camp.returnToCampUseOneHour };
				return;
			}
			//(else if the PC is genderless)
			else if (player.gender === Gender.NONE) {
				DisplayText("You think about going into the Ruined Village, but decide to turn back; right now, you just don't have the proper 'parts' to get the job done, and so you return to your camp.\n\n");
				return { next: Scenes.camp.returnToCampUseOneHour };
				return;
			}
			//(else)
			else {
				DisplayText("You pick up a bowl and carefully pour the contents of the ");
	...
	private sendCorruptCuntToFarm() {
		...
		DisplayText("You tell your pet mouse that she is to head towards the lake, find a farm, present herself to the lady who works there and do as she says.");
	...
	private backToCamp() {
		...
		DisplayText("You tell her to head back to camp; there are things you need to do to her you can’t do whilst she’s here. Repeatedly. Amily bites her lip, trembling with excitement.");
	...
	private harvestMilk() {
		...
		DisplayText("You tell Amily that you want her hooked up to a milking machine whenever possible; you need her fluids.");
	...
	private stopHarvestingMilk() {
		...
		DisplayText("You tell Amily to stop producing succubus milk; you’re practically drowning in the stuff.");
	...
	private talkWithCORRUPTCUNT(sexAfter: boolean = false) {
		...
		//Girls dont get to listen to amily talk about being knocked up.
		//Herms either unless she's okay'ed them for dad-hood.
		// if(player.gender === Gender.FEMALE || (player.gender === Gender.HERM && Flags.list[FlagEnum.AMILY_HERM_QUEST] < 2)) convo = randInt(12); <-- I think this is likely a copypaste issue from the original talk menu.
		//Boost affection!
		Flags.list[FlagEnum.AMILY_AFFECTION] += 2 + randInt(3);
		player.stats.cor += .34;
		//Conversation: Items
		if (convo === 0) {
			...
			//(If player has no main item:)
			if (player.itemSlot1.quantity === 0) {
				DisplayText("You tell her that you'll call for her, if you ever need her knowledge.\n\n");
			...
			if (player.itemSlot1.itype === consumables.EQUINUM) {
				DisplayText("\"<i>That's a distillation of horse essence, I think,</i>\" Amily says. \"<i>It would probably make you stronger and tougher... Perhaps it could also give you a horse cock. Or perhaps give you a pussy as wet as mine,</i>\" she giggles. \"<i>Whatever happens, could you come test it on me?</i>\" she asks seductively.\n\n");
			}
			//Canine Pepper & Variants:
			else if (player.itemSlot1.itype === consumables.CANINEP) {
				DisplayText("\"<i>Looks kind of like a dog's dick, doesn't it? Especially this one with the big knot-like bulge or this one with the ball-like bulbs. I suppose it would make you more dog-like... Have you ever tried one of the jet-black ones?  Those look yummy...</i>\" she says, licking her lips.\n\n");
			}
			//Canine Pepper & Variants:
			else if (player.itemSlot1.itype === consumables.LARGEPP) {
				DisplayText("\"<i>Looks kind of like a dog's dick, doesn't it? Especially this one with the big knot-like bulge or this one with the ball-like bulbs. I suppose it would make you more dog-like... Have you ever tried one of the jet-black ones?  Those look yummy...</i>\" she says, licking her lips.\n\n");
			...
			//Canine Pepper & Variants:
				...
				DisplayText("\"<i>Looks kind of like a dog's dick, doesn't it? Especially this one with the big knot-like bulge or this one with the ball-like bulbs. I suppose it would make you more dog-like... Have you tried this black one?  It looks yummy...</i>\" she says, licking her lips.\n\n");
			}
			//Canine Pepper & Variants:
			else if (player.itemSlot1.itype === consumables.KNOTTYP) {
				DisplayText("\"<i>Looks kind of like a dog's dick, doesn't it? Especially this one with the big knot-like bulge or this one with the ball-like bulbs. I suppose it would make you more dog-like... Have you ever tried one of the jet-black ones?  Those look yummy...</i>\" she says, licking her lips.\n\n");
			...
			//Succubi's Delight:
				...
				DisplayText("\"<i>Full of taint, no question of that. Succubi give it to males who haven't become demons yet; makes them better able to produce cum, and pushes them towards demonhood. Why don't you try some?</i>\" she asks with a smile.\n\n");
			}
			//Wet Cloth:
			else if (player.itemSlot1.itype === consumables.WETCLTH) {
			...
			//Pure Honey:
				...
				DisplayText("\"<i>You managed to get your hands on ultra-pure giant bee honey?</i>\" she asks, sounding impressed. \"<i>I hear that stuff's so pure it can actually help purge the eater of demonic taint - but why would you want to do that?</i>\" she asks mockingly.\n\n");
			}
			//Green Glob:
			else if (player.itemSlot1.itype === useables.GREENGL) {
			...
			DisplayText("Satisfied, you wave her off. She pouts, disappointed that you decided to not fuck her this time; but obeys anyway.\n\n");
		}
		//Conversation: Minotaurs
		else if (convo === 1) {
		...
		//Conversation: Giant Bees
			...
			DisplayText("You decide to talk to Amily about the adventures she's had when she was still adventuring. From there the topic drifts to the strange bee-like women you've seen in the forest.\n\n");

			DisplayText("\"<i>Giant Bees? They're a strange, but kinky race,</i>\" Amily says. \"<i>They're not really corrupt, but sometimes they act kind of like demons.</i>\"\n\n");
			...
			DisplayText("\"<i>Giant Bees hatch from each egg as hundreds of regular-looking little bees.  The fertiles are involved in that - for some reason, the eggs need to incubate inside a living being to hatch in the first place.</i>\"\n\n");
		...
		//Conversation: Imps
			...
			DisplayText("You decide to talk to Amily about the adventures she's had when she was still adventuring. From there the topic drifts to the small demons you've seen in your travels.\n\n");
		...
		//Conversation: Goblins
			...
			DisplayText("You decide to talk to Amily about the adventures she's had when she was still adventuring.  From there the topic drifts to the strange green women you've seen in the forest and the mountains.\n\n");

			DisplayText("\"<i>Ah, Goblins,</i>\" Amily says, nodding her head in approval. \"<i>They used to just experiment with potions and build machines. But now all they do is fuck... a much needed improvement if you ask me. They actively seek to get pregnant, giving birth is like the biggest, most prolonged orgasm to them.</i>\"\n\n");
			...
			//(else)
			else DisplayText("  Can you please knock me up? I don't care how, just order me to fuck until I get pregnant.");
			DisplayText("</i>\"\n\n");

			DisplayText("You step closer and pat her head approvingly, telling her she's a good girl. Amily takes the opportunity to nuzzle your crotch affectionately. Then you dismiss her with a wave.\n\n");
			dynStats("lus", (5 + player.stats.lib / 20));
		}
		//Conversation: What was life in her village like?
		else if (convo === 8) {
		...
		//Conversation: Who was she before it was all lost?
			...
			DisplayText("\"<i>At first... I cared. He was busy all the time with his alchemical experiments, so I tried to take an interest in learning alchemy; it was a way to get closer to him, but it never really interested me that much,</i>\" she says, nonchalantly. \"<i>I was closer to my mom than my dad, I guess.</i>\"\n\n");
		...
		//Conversation: How did she survive?
			...
			//[(if Amily has a dick)
			if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText(", and cock hardening");
			DisplayText(" with excitement.\n\n");
		}
		else if (convo === 12) {
			//[Corrupted Amily talks about Kitsune]
			DisplayText("You decide to talk to Amily about the adventures she had when she was still adventuring, and from there the topic drifts to the odd many-tailed fox women you've seen deep in the woods.\n\n");
			DisplayText("\"<i>They're weird,</i>\" she says, touching her chin absently as she thinks.\n\n");
			DisplayText("You ask her what she means by this.\n\n");
			...
			DisplayText("You ask how anyone knows this much about them, if even meeting one is so dangerous.  Amily scratches her head a bit and blushes, apologizing profusely.  \"<i>Please forgive your ignorant mouse-slut, [master], I'm only repeating what I've heard.</i>\"\n\n");

			DisplayText("She pauses for a moment, and then speaks up with another interesting fact in an attempt to please you.  \"<i>They do seem to have a strange fascination with tentacle beasts, for some reason.  Oooh just thinking about it gets me so horny!  Please [master], fuck me?  Please, oh please!</i>\"\n\n");

			DisplayText("You chuckle a little and tell her you'll think about it.  As you leave, she begins to masturbate, no doubt fantasizing about being used by a tentacle beast.");
		}
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	public conquerThatMouseBitch() {
		...
		DisplayText().clear();
		//By HP:
		if (monster.stats.HP < 1) DisplayText("Amily falls on the ground, unconscious. You drag the mousette towards an empty spot in her hideout. You smile as you strip her clothes off, then you find a rope to tie her up. You tie her hands together, then tie her hands to her ankles, ensuring she won't be able to fight you back. After that, you dispose of anything that could be used as a weapon, or could aid her in her escape, then sit down on a nearby stool to wait until she's recovered her consciousness. After all, it's much more fun if she's awake to see everything you'll do to her...\n\n");
		//By Lust:
		else DisplayText("Amily lets go of her knife and begins masturbating in earnest, too horny to fight. You walk up to her, pick up one of her darts and stab her, knocking her out. You drag the mousette towards an empty spot in her hideout. You smile as you strip her clothes off, then you find a rope to tie her up. You tie her hands together, then tie her hands to her ankles, ensuring she won't be able to fight you back. After that you dispose of anything that could be used as a weapon, or could aid her in her escape, then sit down on a nearby stool to wait until she's recovered her consciousness.  After all, it's much more fun if she's awake to see everything you'll do to her...\n\n");
	...
	private chooseYourAmilyRape() {
		...
		//2nd rape scene
			...
			if (player.gender === Gender.MALE) return { next: rapeCorruptAmily2Male };
			else if (player.gender === Gender.FEMALE) return { next: rapeCorruptAmily2Female };
			else if (player.gender === Gender.HERM) MainScreen.simpleChoices(["MaleFocus", "FemaleFocus", "", "", ""], [rapeCorruptAmily2Male, rapeCorruptAmily2Female, null, null, null]);
		}
		//3nd rape scene
		else if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00170] === 2) {
			if (player.gender === Gender.MALE) return { next: rapeCorruptAmily3Male };
			else if (player.gender === Gender.FEMALE) return { next: rapeCorruptAmily3Female };
			else if (player.gender === Gender.HERM) MainScreen.simpleChoices(["MaleFocus", "FemaleFocus", "", "", ""], [rapeCorruptAmily3Male, rapeCorruptAmily3Female, null, null, null]);
		}
		//4nd rape scene
	...
	private rapeCorruptAmily1() {
		...
		DisplayText().clear();
		//[Raping Amily]
		DisplayText("You wait for a while, idly looking at the mixture you made for Amily, until a groan draws your attention towards the bound mouse.\n\n");

		DisplayText("\"<i>Huh!? What happened!? Why am I tied up!?</i>\" She quickly blurts out, panicked.\n\n");

		DisplayText("\"<i>Hello Amily,</i>\" you say.\n\n");

		DisplayText("The mousette looks at you and then at the bubbling mixture in your hand. \"<i>W-What is this!?</i>\" she asks you.\n\n");

		DisplayText("\"<i>Something I made just for you,</i>\" you answer, grining perversely.\n\n");

		DisplayText("\"<i>No! Oh goddess please no! Don't do this, I beg you! Please just let me go!</i>\" Amily pleads, her eyes getting teary.\n\n");

		DisplayText("You laugh at her, \"<i>You can't expect me to just let you go after that stunt you pulled.</i>\" Then you begin to strip.\n\n");
		...
		//[(if herm)
		if (player.gender === Gender.HERM) {
			DisplayText("Which part of you should Amily lick?");
			MainScreen.simpleChoices(["Cock", "Pussy", "", "", ""], [rapeCorruptAmily1Male, rapeCorruptAmily1Female, null, null, null]);
		}
		//Cocks!
		else if (player.gender === Gender.MALE) return { next: rapeCorruptAmily1Male };
		//Cunts!
	...
	private rapeCorruptAmily1Female() {
		...
		//[(If PC's a squirter)
		if (player.torso.vaginas.get(0).wetness >= 5) DisplayText(", a jet of fluids escape your lower lips and hit her straight in the face; but she pays no mind");
		...
		DisplayText("You laugh as she splutters. \"<i>Look at what you've done,</i>\" you say, motioning at the spilled juice all around her; she finishes coughing and takes a deep breath.\n\n");

		DisplayText("\"<i>More,</i>\" she says.\n\n");

		DisplayText("\"<i>More? After all the mess you've made? I don't think you deserve more.</i>\" You reply.\n\n");

		DisplayText("She begins tearing up, desperate to have more of your delicious juices.\n\n");

		DisplayText("\"<i>Please! I Need it! Please!</i>\" she wails.\n\n");
		...
		if (player.torso.vaginas.get(0).wetness >= 5) DisplayText("She tries to gulp down everything she can, but ultimately a lot of it escapes her mouth and ends up on her face and all around her.  ");
		DisplayText("You keep feeding her more and more cum, your orgasm lasting much longer than usual. Amily's belly even distends a bit from the quantity, her pussy leaks juice like an open tap; it seems servicing you was enough to drive her over the edge several times.\n\n");

		DisplayText("Finally done, you let go of her and get up; she proceeds to slump down and give a small burp of satisfaction, then drift off into sleep. You untie her and proceed to get dressed; you give her a light pat on the thigh and return to your camp. You'll have to do this again sometime later...");
		player.orgasm();
		player.stats.lib += -2;
		player.stats.cor += 5;
		if (Game.inCombat) return { next: Scenes.camp.returnToCampUseOneHour };
	...
	private rapeCorruptAmily2Male() {
		...
		DisplayText("You gasp in surprise; there's no way she could have done this before your last meeting! Has she been practicing?\n\n");

		DisplayText("Soon you feel that she's taken in all that ");
		//[(if PC's dick is huge)
		if (player.cockThatFits(61) < 0) DisplayText("is possible.");
		//(else)
		else DisplayText("you have to offer.");
		DisplayText("\n\n");

		DisplayText("You pat her head and say. \"<i>Good girl.</i>\"\n\n");

		DisplayText("She smiles and gives your dick a big slurp, drawing a moan from you.\n\n");
		...
		//[(if PC has balls)
		if (player.torso.balls.quantity > 0) DisplayText("  Just the thought of it is enough to make your balls churn.");
		DisplayText("  You imagine Amily growing ever more eager to blow you; begging for your cum like the slut she is. The previously prudish mouse reduced to nothing more than a cumbucket for you to unload into.\n\n");

		DisplayText("Suddenly, you grab her hair and roughly shove yourself as far in as you can; Amily doesn't scream in pain, instead she moans in pleasure, eyes wide and eager at what she's about to receive.\n\n");

		DisplayText("You unload into her, jet after delicious jet of cum bursting into her throat to hit her stomach, even distending it a bit.");
		//[(If PC has large cum amount)
		if (player.cumQ() >= 750) DisplayText("  You feel your enourmous load coming back through her throat and spilling from her mouth and nose, smearing your crotch as you pull away to let her breathe.");
		DisplayText("\n\n");
	...
	private rapeCorruptAmily2Female() {
		...
		//[(if PC is a squirter)
		if (player.torso.vaginas.get(0).wetness >= 5) DisplayText(", giving Amily a juice bath; even if the mousy cunt did get better at drinking, there is no way she'd be able to drink all of your fluids");
		DisplayText(".");
		//(else)
		if (player.torso.vaginas.get(0).wetness < 5) DisplayText("  A flood of juices escape your pussy and go directly into the mousy cunt's hungry maw, however, some of it spills; even if Amily did get better at drinking, she's still a long way from being the perfect fucktoy you want her to be.");
		DisplayText("\n\n");

		DisplayText("Suddenly you grab her ears and pull her against your dripping pussy as hard as you can. Amily doesn't even seem to notice the pain; all she does is moan in pleasure as her tongue continues lapping excitedly, eager to receive her reward.\n\n");
		...
		//[(if PC is a squirter)
		if (player.torso.vaginas.get(0).wetness >= 5) DisplayText("  Your orgasm lasts much longer than usual. Eventually you release Amily and finish off on top of her head.\n\n");
	...
	private rapeCorruptAmily2Epilogue() {
		...
		DisplayText("Amily falls on her back, panting happily and licking her mouth to taste as much of you as possible. That's when you notice her beginning to change, slowly but significantly.\n\n");

		DisplayText("Her breasts start filling up, and her butt and hips grow bigger; you can't help but smile at how you're changing the mousette.\n\n");

		DisplayText("You lower yourself and open one of her ears wide, before whispering, \"<i>Be ready for when I come back, there's a lot more where this came from,</i>\" then you get up and walk away to fetch more ingredients for Amily's 'medicine'.");
	...
	private rapeCorruptAmily3Male() {
		...
		DisplayText("Amily begins moving her ");
		if (Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) DisplayText("furry ");
		...
		if (player.stats.cor < 60) {
			DisplayText("Satisfied for the moment, you leave the smiling mouse lying in a pool of cum and return to the camp.");
			player.orgasm();
			player.stats.cor += 2;
			return { next: Scenes.camp.returnToCampUseOneHour };
			return;
		...
		//(else)
		DisplayText("That was good, but now it's time to reward Amily for her efforts; besides you could really use a proper blow job.\n\n");
		...
		//[(if PC has large cum amount)
		if (player.cumQ() >= 750) DisplayText("  You keep pumping and she keeps drinking; her distended belly doesn't stop growing until she looks heavily pregnant and you're spent.");
		DisplayText("\n\n");
	...
	private rapeCorruptAmily3Female() {
		...
		//(else)
		else DisplayText("moisture gathers and drips over Amily's sensitive nipples, causing the mousette to sigh in pleasure and smile as she looks at her wet nipples.");
		DisplayText("\n\n");

		//[(if PC's clit is big enough)
		if (player.torso.clit.length >= 4) DisplayText("Amily then crushes your sensitive clit between her breasts, working her mounds over it like it was a cock. You gasp and moan as Amily smiles up at you, happy to see your reaction to her stimulation.");
		//(else)
		...
		//(if PC is < 60 Corruption)
		if (player.stats.cor < 60) {
			DisplayText("Satisfied for the moment, you leave the smiling mouse lying in a pool of juices and return to the camp.");
			player.orgasm();
			player.stats.cor += 2;
			return { next: Scenes.camp.returnToCampUseOneHour };
			return;
		}
		DisplayText("That was good, but now it's time to reward Amily for her efforts, besides you could really use a proper licking.\n\n");
	...
	private rapeCorruptAmily3Epilogue() {
		...
		DisplayText("She gets up and kneels, looking up to you in attention. \"<i>I'm willing to take you in as my cumbucket, only if you obey all my orders and keep practicing to pleasure me even more,</i>\" you say.\n\n");

		DisplayText("Amily happily answers, \"<i>Anything you desire!</i>\"\n\n");

		DisplayText("\"<i>I will return when I think you're ready.</i>\" You say, then leave her to her own devices.");
	...
	private rapeCorruptAmily4Meeting() {
		...
		//(if PC is genderless)
		if (player.gender === Gender.NONE) {
			DisplayText("You would love to play with your mouse bitch, but you don't have the parts for that; so you return to the camp.");
			return { next: Scenes.camp.returnToCampUseOneHour };
			return;
		}
		...
		//[(if herm)
		if (player.gender === Gender.HERM) {
			DisplayText("Which part should you use to finish off the mousette?");
			//[Cock] [Pussy]
			MainScreen.simpleChoices(["Cock", "Pussy", "", "", ""], [rapeCorruptAmily4Male, rapeCorruptAmily4Female, null, null, null]);
		}
		else if (player.gender === Gender.FEMALE) return { next: rapeCorruptAmily4Female };
	...
	private rapeCorruptAmily4Male() {
		...
		else if (player.torso.vaginas.count > 0) DisplayText("your pussy");
		else DisplayText("the base of your cock");
		DisplayText(" a teasing lick. You smile back and say, \"<i>Go ahead, pleasure me.</i>\" Amily moans and begins moving her tongue and maw along your shaft.\n\n");

		DisplayText("You grip her ears for leverage, when did she get so good at this? She must've been practicing all the time... She bobs her head along you eagerly, hungrily, and almost desperately.");
		//[(if PC has balls)
		if (player.torso.balls.quantity > 0) DisplayText("  Sometimes you even feel her nipples brush your balls teasingly.");
		DisplayText("  Each time her nose bumps against your crotch, she swallows, massaging your whole length; and each time she pulls back, she blows, sending pleasurable chills along your body.");
		DisplayText("\n\n");
		...
		//[(if PC has balls)
		if (player.torso.balls.quantity > 0) DisplayText("  One of her hands even begins kneading your balls, attempting to coax even more out of you.");
		DisplayText("\n\n");

		//(if PC's corruption < 80)
		if (player.stats.cor < 80) {
			DisplayText("You push her back and withdraw, spent. Amily looks up to you adoringly, ");
			//[(if PC has huge cum amount)
			if (player.cumQ() >= 750) DisplayText("her belly distended to the point that she looks pregnant and just about ready to give birth.");
			//(else)
			else DisplayText("her belly filled with the huge load you just dumped in her.");
			DisplayText("\n\n");

			DisplayText("You try to summon more strength to continue fucking the mouse's wonderful throat, but for the moment you're spent. \"<i>Go and keep practicing, I'll come to feed you later.</i>\" Amily smiles, licks her lips and gives your cock a parting kiss before running away to one of her hideouts.\n\n");

			DisplayText("You return to the camp.");
			player.orgasm();
			player.stats.cor += 3;
			return { next: Scenes.camp.returnToCampUseOneHour };
			return;
		}
		DisplayText("You push her back and withdraw, not yet satisfied. <b>A familiar power gathers inside you, and you decide to tap into it.</b>\n\n");
		...
		DisplayText("Amily wastes no time and gets to work.  You let her suck on her favorite thing in the world for a while; then you tell her to lick you, to work her tongue all around your shaft");
		//[(if PC has balls)
		if (player.torso.balls.quantity > 0) DisplayText(" and balls");
		DisplayText(".\n\n");

		DisplayText("She obeys you at once; following your orders is like second nature to her. An image of an even more corrupted Amily comes to your mind. Demonic features adorn her as she becomes a mix of succubus and mouse. You focus into those thoughts and send them towards your cock.\n\n");

		DisplayText("\"<i>Stop and stand still. It's time to baptise you,</i>\" you tell her, panting.\n\n");
	...
	private rapeCorruptAmily4Female() {
		...
		DisplayText("She goes back to nuzzling and kissing along your netherlips; she inhales deeply, enjoying the scent of your wet pussy, then exhales right on your clit. It tickles you enough to make you ");
		//[(if PC is a squirter)
		...
		if (player.torso.clit.length >= 4) DisplayText("  One of her hands begins stroking your erect clit as if it was a cock. The pleasure of the act nearly overloads your senses.");
		DisplayText("\n\n");

		DisplayText("You feel the biggest orgasm yet build up, and you decide to reward your obedient little cumslut by making sure she gets all of it. \"<i>Clamp down on my pussy slut. I'm about to reward you for all your effort.</i>\" Amily loses no time in obeying your orders. She gives your pussy one final lap, and you feel it.");
		//[(if PC is a squirter)
		if (player.torso.vaginas.get(0).wetness >= 5) DisplayText("  Amily barely has to work to drink your juices, your pussy jets them forth with such force that all of them go straight through Amily's mouth and into her throat.");
		//(else)
		else DisplayText("  Amily drinks from you like a parched man drinks from a well.");
		DisplayText("  Every drop offered is consumed, something you've come to expect from your elected cum-bucket. One final powerful gush of fluids signals the end of your orgasm.\n\n");

		//(if PC's corruption < 80)
		if (player.stats.cor < 80) {
			DisplayText("You push her back and withdraw, spent, ");
			//[(if PC is a squirter)
			if (player.torso.vaginas.get(0).wetness >= 5) DisplayText("her belly distended from the immense amount of femcum she just drank.");
			//(else)
			else DisplayText("her belly slightly distended from the flood of juices you just dumped in her.");
			DisplayText("\n\n");

			DisplayText("You feel like you should continue, but are too weak to do so... \"<i>Go and keep practicing, I'll come to feed you later,</i>\" you tell her.  Amily smiles, licks her lips and gives your pussy a parting kiss before running away to one of her hideouts.\n\n");
		...
		DisplayText("  She falls silent and you say, \"<i>Very well. From now on I am your one and only mistress. You will address me as such, always. My word is law, and you shall do whatever I tell you to, whenever I tell you to. Rise, it's time to break you into your new role as my cum-dumpster.</i>\"\n\n");
	...
	private rapeCorruptAmily4Epilogue() {
		...
		DisplayText("Her " + ((Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) ? "fur turns to a lewd purple" : "hair turns into a lewd purple, skin fading to a light lavender") + "; her small horns grow and become more defined; small bat-like wing sprout from her shoulders; the spade-like tip of her tail grows bigger and more defined; ");
		if (Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0)
			DisplayText("her feet warp into a mix of mousy foot-paws and demonic claws, complete with ebony claws and a fourth joint");
	...
	private stalkingZeAmiliez() {
		...
		if (player.stats.int >= 50 && player.stats.spe >= 65) {
			...
			DisplayText("You wait for quite some time, but eventually you hear Amily approaching; she checks the hideout for any signs of intrusions, but you have concealed your presence well and she finds nothing. Satisfied, she sets down a bag with some fruits she gathered and sits down on a stool, letting out a sigh of relief.\n\n");
			...
			DisplayText("\"<i>N-No! Get off me!</i>\" She yells back, panicked at her predicament. You hold both her wrists with one hand above her head, then take one of her darts and stab her in the thigh. She squeaks loudly and ceases her struggling, the drugs within the dart quickly knocking her out.\n\n");
		...
		//(if PC's intelligence >= 50)
			...
			DisplayText("This is your cue to act; you quickly burst out of your hideout and swipe her blowpipe away. Amily jumps away in surprise and reaches for her knife, assuming a fighting stance. You ready your " + player.weaponName + " and prepare to teach the foolish mouse a lesson.\n\n");
			//[Proceed to battle.]
		...
		//(if PC's speed >= 65)
			...
			amilySprite();
			DisplayText("You walk about the village, looking for Amily, but after searching for a while with no luck, you catch a break when you hear some bushes moving.\n\n");

			DisplayText("You spot Amily just in time to see a dart flying towards you; but you quickly sidestep the dart and rush towards her. Realising she missed, she reaches for another dart; but you won't allow it, you take a rock lying on the ground and throw it at her blowpipe, striking it out of her hand.\n\n");

			DisplayText("Panicked, she takes her knife and prepares to fight you. You ready your " + player.weaponName + " and prepare to teach the foolish mouse a lesson.\n\n");
			//[Proceed to battle.]
			startCombat(new Amily(), true);
		}
		else {
			DisplayText("You search for Amily high and low, but can't find a single trace of her. Frustrated, you return to the camp.  Maybe if you were smarter or faster you could find her.");
	...
	private stalkingZeAmiliez2() {
		...
		//(if PC is genderless)
		if (player.gender === Gender.NONE) {
			DisplayText("You think about going into the ruined village, but playing with Amily is not going to be possible if you don't have the parts for it... You return to your camp.");
			return { next: Scenes.camp.returnToCampUseOneHour };
		}
		//(else)
		else {
			...
			DisplayText("It doesn't take long before you locate her and you immediately see how she's changed after the first dose of her special medicine.\n\n");
			...
			DisplayText("She squeaks in surprise, apparently she hadn't noticed you; she begins fidgeting and shaking.\n\n");

			DisplayText("\"<i>What's the matter? Don't you want more?</i>\" You ask.\n\n");

			//(if PC's corruption < 45)
			if (player.stats.cor < 45) {
				DisplayText("Amily shakes her head and yells, \"<i>No! I can't!</i>\" before darting off.\n\n");

				DisplayText("You laugh and put the bottle away, then return to your camp.\n\n(Not corrupt enough...)");
				return { next: Scenes.camp.returnToCampUseOneHour };
				return;
			}
			...
			DisplayText("Once she's close, she kneels before you and you uncork the bottle. \"<i>Open wide!</i>\" you say, smiling.\n\n");

			DisplayText("You shove the bottle into her mouth and she greedily gulps down everything. Once empty, you discard the bottle and watch as she moans loudy and juices her pants.\n\n");

			DisplayText("You laugh, but take a step back when she reaches for her knife; then proceeds to cut off her ragged clothing, disposing of it along with the knife. \"<i>Aaaah... Much better,</i>\" she says in relief.\n\n");

			DisplayText("Glad to see she meant you no harm, you decide it's time to reward her for her openness. \"<i>Now open wide once more, Amily. It's time for your reward.</i>\" you tell her.\n\n");

			DisplayText("\"<i>Yes! Please I need it!</i>\" she says eagerly, closing her eyes and opening her mouth.");
			player.removeKeyItem("Potent Mixture");
			//RAPE 2 GO
			chooseYourAmilyRape();
		}
	...
	private stalkingZeAmiliez3() {
		...
		DisplayText("She takes the bottle from your hand and throws it away. \"<i>No. All I need is your cum... Please let me drink more. I-I'll do anything you want, just let me have a taste. Please?</i>\" She begs.\n\n");
		player.removeKeyItem("Potent Mixture");
	...
	public farewellNote() {
		...
		DisplayText("So what if you are aren't so pure anymore; you're still very much capable of helping her!  Maybe you should teach her a lesson?\n\n");
		...
		DisplayText("  Maybe she had a point though?  You are getting pretty corrupt... these thoughts are proof enough.\n");
	...
	public amilyReturns() {
		...
		DisplayText("\"<i>Thank you, " + player.short + ". I don't know how you did it, or if it had anything to do with me, but you managed to do something that I've never heard of anyone else having the will to do. You turned back from demonhood and repurified yourself again.</i>\"\n\n");

		DisplayText("She squeezes you tightly, then gets up to start making breakfast.\n\n");
		DisplayText("<b>Amily has moved back in.</b>\n");
	...
	public amilyNoFur() {
		...
		DisplayText("There's a long pause while Amily digests your implication.  \"<i>You want me to... change?</i>\" she asks quietly.  \"<i>Would that... make you want to mate with me?</i>\"  You can't make any promises, but it would definitely change your considerations, you explain.\n\n");
	...
	private amilyDefurrify() {
		...
		Flags.list[FlagEnum.AMILY_OFFER_ACCEPTED] = 1;
		amilySprite();
		DisplayText("It's not long after arriving in the ruins, darting about to announce your presence, before Amily steps out from a dilapidated alley.  She squares up to you with a hand planted on her slender hip, looking to you expectantly.", true);
		...
		DisplayText("She stalks off into the ruins once more, humming a little tune as she goes.  You note a little more spring in her step, now that hope is restored in repopulating her race.\n\n");
		return { next: Scenes.camp.returnToCampUseOneHour }; // To camp
	}
	...
	public amilyDefurryOfferAtCamp() {
		...
		if (!amilyCanHaveTFNow()) {
			DisplayText("Unfortunately, you can't do such a thing until you have acquired the necessary magical prerequisites.  In simple terms, you need a <b>golden seed</b> for a human face, a <b>black egg</b> to get rid of the fur, and some <b>purified succubus milk</b> to round things off.", true);
			return { next: amilyFollowerEncounter };
			return;
		}
		//EAT ZE ITEMS!
	...
	public dateNightFirstTime() {
		...
		if (pregnancy.isPregnant) {
			DisplayText("\"<i>Perhaps once I'm no longer pregnant.  I wouldn't want to hurt the little ones,</i>\" Amily answers.");
			return { next: amilyFollowerEncounter };
			return;
		}
		...
		DisplayText("\"<i>Hey there, lover,</i>\" Urta says happily as you approach the gates.  However, her gaze quickly shifts to the little mouse-girl at your side.  Urta wrinkles her brow in confusion for a moment before slow-dawning comprehension spreads across her face.  \"<i>So, " + player.short + ", uh... who's your friend?</i>\"\n\n");
		...
		else if (model.time.hours < 6) DisplayText("despite the early time, bustling with centaur and canine-morphs on their way to their pre-dawn activities.");
		else if (model.time.hours < 12) DisplayText("despite the morning hours, busy with centaur and canine-morphs hurrying to their own personal tasks.");
		else DisplayText(" with centaur and canine-morphs busy with their own tasks.");
		...
		DisplayText("Urta scans the tavern's main room briefly, sees you, and strides over.  With a charming smile, Urta asks if she can have a seat.  You try to gently explain that you're on a date right now, but ");
		DisplayText("Amily interrupts, slurring about how she'd like to meet some of your friends.  Oh, this is going to end well...\n\n");
		...
		//DisplayText("\"<i>Huh? Oh yeah, ish fiiine,</i>\" Amily drawls, half-drunk. Oh, this is going to end well.)
		DisplayText("The fox-girl grabs a chair from an adjacent table and brings it over, sitting reverse and leaning heavily on the back rest.  Before she can say anything more than a simple greeting, however, Amily grabs a bottle from ");
		if (Flags.list[FlagEnum.ENCOUNTERED_VALA_AT_BAR] === 0) DisplayText("a passing waitress");
		else DisplayText("a rather alarmed Vala as she flutters by");
		DisplayText(". \"<i>Herr, ish gud,</i>\" Amily slurs, pawing off the bottle to a surprised Urta.\n\n");

		DisplayText("Well, this could be interesting.  If you get both the girls drunk, it might be easy (or inevitable) for something sexual to happen.  Or, you could take Amily home right now and make sure nothing untoward happens to either of your lovers.");
		//(Display Options: [Drink!] [Leave])
		MainScreen.simpleChoices(["Drink", "", "", "", "Leave"], [liqueurUpTheWaifus, null, null, null, amilyXUrtaRunAWAY]);
	}
	...
	private liqueurUpTheWaifus() {
		...
		DisplayText("Before long, Urta's swaying in her chair, her brief snippets of conversation becoming more and more lewd with each passing minute.  Eventually, much to your delight, an unintentional show of cleavage from Amily sets off the beast.\n\n");

		DisplayText("THUNK!\n\n");

		DisplayText("The table is nearly lifted off its base. Urta gasps, embarrassed, and Amily nearly leaps out of her seat in surprise.  \"<i>W-What the hell was that!?</i>\" she squeaks.  Before you can answer, Amily ducks her head under the table - and comes back up blushing beet-red.\n\n");
		...
		DisplayText("Unabashedly aroused by your mouse-lover getting the fox-girl off, you gently suggest that the three of you go and get a room.\n\n");

		DisplayText("\"<i>Mmm, I thought you'd never ask,</i>\" Urta purrs, giving a particularly violent shudder.\n\n");

		DisplayText("\"<i>Oh, that shounds loooovely,</i>\" Amily adds.\n\n");

		DisplayText("Grinning, you saunter over to the bar, rent a room, and lead the girls upstairs.  Half-drunk yourself, you stumble up the last few stairs and hurriedly unlock the door.  The room is sparsely decorated, sporting a single large bed.\n\n");

		DisplayText("Urta and Amily flop down on the bed, already undressing and teasing each other - Urta running her hands ");
		if (Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) DisplayText("through the mouse's fur ");
		else DisplayText("over the mouse's skin ");
		DisplayText("and Amily wrapping her lithe little tail around Urta's massive endowment.  You strip off your " + player.inventory.equipment.armor.displayName + " and, looming over the girls, decide on how you want to go about this.");
		//(Display Appropriate Options: [Use Cock] [Use Vag])
		if (player.gender === Gender.MALE) MainScreen.simpleChoices(["Use Cock", "", "", "", ""], [threesomeAmilUrtaCAWKS, null, null, null, null]);
		if (player.gender === Gender.FEMALE) MainScreen.simpleChoices(["", "Use Vagina", "", "", ""], [null, urtaXAmilyCuntPussyVagSQUICK, null, null, null]);
		if (player.gender === Gender.HERM) MainScreen.simpleChoices(["Use Cock", "Use Vagina", "", "", ""], [threesomeAmilUrtaCAWKS, urtaXAmilyCuntPussyVagSQUICK, null, null, null]);
		if (player.gender === Gender.NONE) simpleChoices("Nevermind", Scenes.camp.returnToCampUseOneHour, "", null, "", null, "", null, "", null);
	}
	...
	public threesomeAmilUrtaCAWKS() {
		...
		DisplayText("\"<i>That was wonderful,</i>\" Amily sighs, exhausted.\n\n");

		DisplayText("\"<i>Mmm,</i>\" Urta purrs, giving her ass a playful slap. \"<i>We gotta do this again sometime.</i>\"\n\n");

		DisplayText("You couldn't agree more.");
	...
	public urtaXAmilyCuntPussyVagSQUICK() {
		...
		DisplayText("\"<i>Mmm,</i>\" she purrs, giving your cheek a last stroke with her thumb.  \"<i>We gotta do this again sometime.</i>\"\n\n");
		player.orgasm();
		player.stats.sens += -2;
		DisplayText("You couldn't agree more.");
	...
	private urtaXAmilyAfterMurrrath() {
		...
		Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] = 3 + randInt(4);
	...
	public pureAmilyPutsItInYourRectumDamnNearKilledEm() {
		...
		DisplayText("Before she can misunderstand your intentions, you lean ");
		if (player.tallness >= 40) DisplayText("down ");
		else DisplayText("over ");
		DisplayText("and whisper in her ear where exactly you would like her to use her cock, which makes her cheeks grow hot with embarrassment.\n\n");

		//(1st time):
		if (Flags.list[FlagEnum.AMILY_TIMES_BUTTFUCKED_PC] === 0) {
			DisplayText("\"<i>Seriously...?</i>\" she says, looking at you skeptically. \"<i>You want that?</i>\"\n\n");

			DisplayText("You nod to her, still idly stroking her through her pants, assuring her that that is exactly what you want.\n\n");
			...
			DisplayText("You give her a slightly skeptical look, asking her what exactly is in that stuff, but she scoffs a bit and says \"<i>Relax, it's just a little burn ointment... Should do the trick though.</i>\"\n\n");
		}
		//(Else):
		else {
			DisplayText("\"<i>You want that?  Well... Okay,</i>\" she says, smiling a bit in spite of the rosy blush covering her face.\n\n");
		...
		DisplayText("It isn't long before Amily's sensual thrusting and touching has you on the precipice of your orgasm, and you voice your pleasure loudly, digging your fingertips into the ground and moaning.");
		if (player.torso.vaginas.count > 0) {
			DisplayText("  Sexual fluids ");
			if (player.torso.vaginas.get(0).wetness <= 1) DisplayText("drip ");
			else if (player.torso.vaginas.get(0).wetness <= 3) DisplayText("dribble ");
			else DisplayText("gush ");
			DisplayText("down your thighs");
			if (player.torso.cocks.count > 0) DisplayText(" and p");
		}
		if (player.torso.cocks.count > 0) {
		...
		DisplayText("With a shuddering sigh, Amily dismounts you, and you roll onto your side next to her, reaching back to caress her cheek and tell her how wonderful she did.  She wraps her arms around you, hooking a leg around your waist softly, and the two of you lie there and cuddle for some time.\n\n");

		DisplayText("\"<i>Mm... you know... maybe we should do this again sometime,</i>\" she says with a devious smirk, kissing your cheek.");
	...
	private fuckPureAmilysHeiny() {
		...
		if (x < 0) x = 0;
		DisplayText("You pause and flash her a coy smirk, then you gently place the tip of a finger on her nose, slowly running it down along her body, over her lip, between her breasts, across her stomach, finally stopping to firmly grip her " + amilyButt() + ".");
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText("  Her pants tent from arousal as her male organ reacts to your sensual touching, and you start to teasingly stroke it as well.");
		DisplayText("\n\n");

		//(First Time):
		if (Flags.list[FlagEnum.TIMES_FUCKED_AMILYBUTT] === 0) {
			DisplayText("She looks ");
			if (player.tallness >= 60) DisplayText("up ");
			else if (player.tallness <= 40) DisplayText("down ");
			DisplayText("at you with a slightly puzzled look in her eye, and you try as casually as you can to bring up the subject of anal sex.  You ask her if she's ever considered it, and tell her it could be fun.\n\n");

			DisplayText("\"<i>I... I mean, you want to put your... in there?</i>\" she says, her brow furrowed a bit.  You are worried for a moment that you may have stepped over a line, but she slowly tilts her head, giving you a bit of a shrug and a contemplative grimace.\n\n");

			DisplayText("\"<i>Well... I had never really considered it up until now, but... if you're sure that's what you want,</i>\" she says, her lips pursed a bit as she mulls it over.\n\n");

			DisplayText("You smile broadly, nodding, and assure her you'll be gentle as you begin to strip out of your " + player.inventory.equipment.armor.displayName + ".\n\n");

			DisplayText("\"<i>Woah, woah, hold on there,</i>\" she says, putting up a hand gently to halt your eager advances.  \"<i>I said I'd do it, but you're not coming in the back door without greasing the hinges a bit first.</i>\"\n\n");
		...
		//(Else):
			...
			DisplayText("She smirks back at you just as coyly, trailing her fingers up your chest and laying a small kiss on your cheek.  \"<i>Oh, you're in the mood for that sort of thing again, are you?</i>\"\n\n");
		...
		DisplayText("  Turning around, she lowers herself onto the ground carefully, her whiplike tail raising out of the way as she slides onto her knees and elbows.\n\n");
		...
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText("her own member twitch and dribble a small puddle of clear pre-cum onto the ground below her, joining with ");
		DisplayText("a noticeable trail of juices gradually sliding down her thighs.\n\n");

		DisplayText("You very carefully pull back, Amily drawing a sharp intake of breath when you are fully freed from the grips of her pucker.  Gently sliding down alongside her, you draw her into a warm embrace, cuddling with her for a long while afterward");
		if (player.cumQ() > 500) DisplayText(" and gently stroking along the curves of her belly with a smile");
		DisplayText(".");
		player.orgasm();
		player.stats.sens += -2;
	...
	private amilyIncest() {
		...
		amilySprite();
		//AKA:
		//DisplayText(Fen gets donated to write something that makes him moderately uncomfortable.)
		DisplayText("When you go looking for Amily, you find her on the edge of camp, talking with three of her offspring.  The purple-");
		if (Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) DisplayText("furred");
		else DisplayText("haired");
		...
		if (Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) DisplayText("fur");
		else DisplayText("skin");
		DisplayText(".  You suppose that with so much corruption coursing through them, incest really wouldn't bother them in the slightest.");
		DisplayText("\n\nThe herm is cuddling up next to Amily and smearing ropes of doggie cum ");
		if (Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) DisplayText("through her mother's fur");
		...
		if (player.torso.cocks.count > 0) {
			...
			else DisplayText("[pg]Sadly, you're too big to fuck any of them.");
		}
		else DisplayText("[pg]If only you had a cock, you could fuck them all.");
	...
	private fuckIncestCunts(all: boolean) {
		...
		DisplayText().clear();
		amilySprite();
		DisplayText("You step up and ");
		if (!all) DisplayText("tell Amily to send the male away - you've got pussies to fuck.  She squeaks in excitement, barely noticing as her son tramps off.");
		...
		if (all) {
			...
			if (Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) DisplayText("muzzle");
			...
			else if (player.cumQ() < 500) DisplayText("raining ");
			else if (player.cumQ() < 2000) DisplayText("pouring ");
			else DisplayText("spraying ");
			DisplayText("down to the ground.  Clearly, your boy needs some training in how to hold his cum.  You close your eyes and doze, feeling as sated as you are tired.");
		}
		else {
	...
	private amilyEggStuff() {
		...
		if (player.gender > 0) {
			//(if sexed)
			DisplayText(", your [if (hasCock = true) [cock] stiffening][if (isHerm = true)  and ][if (hasVagina = true) [vagina] starting to burn with need]");
		}
		DisplayText(" whilst your body instinctively reacts to the pleasure it's feeling, ovipositor slowly thickening as it pulses with need.");
		...
		//if dogmorph
		if (RaceScore.dogScore(player) >= 4) DisplayText(", which you know for a fact are <i>very</i> good,");
		...
		//if amily = herm
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText("and you notice a growing bulge in her trousers");
		DisplayText(".  Looks like you aren't entirely twisting her arm to get her to go along with this.  \"<i>Fine, I'll help lighten that load you've got, just...  don't expect me to help out as much around camp if I'm full of eggs.</i>\"");
	...
	public layEggsInAmily() {
		...
		else if (player.eggs() < 40) DisplayText("heaving");
		else DisplayText("unbelievably swollen");
		...
		if (player.gender === Gender.NONE) DisplayText("blank groin");
		DisplayText(", teasing you into unconsciously humping against her slender digits.");
		if (pregnancy.isPregnant) DisplayText("  \"<i>But try to remember that I'm already carrying.  I don't want it getting too cramped in there; so make sure you don't miss, alright?</i>\"");
	...
	private layEggsInAmilysCorruptedHole() {
		...
		if (player.gender === Gender.NONE) DisplayText("featureless mound");
		DisplayText(", your bulging ovipositor fully emerging at the same time.");
		...
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText(" and her erect shaft shuddering as it throbs in the air");
		DisplayText(" telling you everything.  You ask her if she's sure she isn't projecting just a little, the mousegirl blushing furiously as she sheepishly nods, a demure smile spreading across her face.");
		DisplayText("\n\nWith her reddened lips ");
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText("and stiff cock ");
		...
		if (player.gender > 0) DisplayText("yours");
		else DisplayText("your groin");
		...
		if (player.torso.cocks.count > 0) {
			...
			if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText(", her shaft oozing thick globs of pre-cum that roll down her cock to smear against your own as she moves against you");
			DisplayText(".");
		}
		//(Females + AmilyUndicked)
		else if (player.torso.vaginas.count > 0 && Flags.list[FlagEnum.AMILY_WANG_LENGTH] === 0) {
		...
		//{(Genderless)
			...
			DisplayText("She moans as your shaft rubs against her, her own thrusts against your blank groin increasing in pace every time your overly sensitive pseudo-member grazes her behind.  Her femcum splatters your lower half with each roll of her hips");
			if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText(", her shaft covering your stomach with thick strands of pre-cum");
			DisplayText(" as thrusts become faster and harder.  You pant huskily in time with your lover, hands sliding around her hips to grab her cheeks in an attempt to sink your throbbing length inside her.");
		}
		...
		//(Females + AmilyHerm)
			...
			DisplayText("  Amily jumps forwards suddenly in reaction to your uncontrolled spasm, her shaft suddenly spearing you and forcing a desperate cry from your lips.  She responds in kind, a strangled moan escaping her as she slumps against you, her hips starting to thrust wildly into your trembling [vagina].");
		}
		//(F/F or Genderless)
		else DisplayText("  You moan as Amily slumps against you, her arms sliding around your neck as her crotch presses ever more forcefully against your.  Your eyes lock with hers as she groans, her lower body undulating against yours with a desperate need, your abdomen straining as your body flushes with desire.");
		...
		//(Males/Herms)
		if (player.torso.cocks.count > 0) DisplayText("you feel her passage clamp down around your shaft, her body shaking as she whimpers softly above you.");
		//(Females + AmilyHerm)
		else if (player.torso.vaginas.count > 0) DisplayText("her thrusts falter slightly, her member twitching within you as your ovipositor fills her body up, shaking slightly with pleasure as her hot shaft brushes against your walls.");
		//(F/F or Genderless)
		else DisplayText("her body hammers into you with more urgency, each hit covering you with another splatter of her femspunk as she groans at how much you're filling her.");

		DisplayText("\n\nAnother moan bursts out of you as her passage");
		if (player.torso.cocks.count > 0) DisplayText("s quiver");
		else DisplayText(" quivers");
	...
	public amilyLaysEggsLikeABitch() {
		...
		DisplayText(" with one hand, the other catching your eye as it moves beneath her top, apparently caressing her breasts.");
		...
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] === 0) DisplayText("pussy visibly quivering");
		else DisplayText("member oozing a thick glob of pre-cum");
		DisplayText(" as her body shudders with pleasure.");

		DisplayText("\n\nYou decide to leave the girl to her ministrations, ");
		//if abdomen
		if (player.canOviposit()) DisplayText("though the sight has certainly gotten you thinking about what you'll be doing with your next clutch.\n");
		else DisplayText("though a distinct heat in your nethers leaves you wishing you had another clutch to unload right now\n");
	...
	public amilySwimFuckIntro() {
		...
		if (Flags.list[FlagEnum.AMILY_TIMES_SWIMFUCKED] === 0) {
			...
			if (player.stats.lib >= 60) DisplayText(", eagerly anticipating the opportunity to drink in her form while it's bound up in body-hugging black fabric.");
			else if (player.stats.cor < 50) DisplayText(", happy to share the fruits of your adventure with your mousy lover.");
			else DisplayText(", secretly hoping she'll get used to putting on a show for you.");
			...
			if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText("  D-don't laugh, okay?");
			DisplayText("</i>\"");
			DisplayText("\n\nAmily strides around the rocks nervously, hands on her hips.  Her soft ");
			if (Flags.list[FlagEnum.AMILY_NOT_FURRY] > 0) DisplayText("skin");
			...
			if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) {
				DisplayText("  The top portion of it is pulled out away from her body, a taut tent that her " + amilyCock() + " is threatening to tear through at any moment.  After a moment of fidgeting, Amily pulls it out the top to flop lamely.");
				if (Flags.list[FlagEnum.AMILY_HAS_BALLS_AND_SIZE] > 0) DisplayText("  It snaps back onto her " + amilyBalls() + " to snug them tightly.");
			}
			DisplayText("  Higher up, silky onyx wreaths the globes of " + amilyTits() + ", but as you admire her new appearance, the hard tips of " + amilyNipples() + " rise up, so clearly defined that you can see their pebbly texture through the inviting swimsuit.");
		...
		//Repeat Swim Intro
			...
			DisplayText("You ask Amily if she'd like to go for a swim again.  She smiles genuinely and nods.  \"<i>Let me get dressed, I'll be right back!</i>\"");
			...
			if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) {
				DisplayText("  The only thing left exposed is her " + amilyCock() + ", which bobs lightly with each step.");
				if (Flags.list[FlagEnum.AMILY_HAS_BALLS_AND_SIZE] > 0) DisplayText("  Her balls are snugly secured in her bottoms, looking like a cute little sphere of virile fun.");
			}
			DisplayText("  The hard points of her " + amilyNipples() + " prominantly display themselves on the bikini top.  Clearly she doesn't plan to spend too much swimming, just like last time.");
	...
	private amilySwimFuckPartII() {
		...
		if (player.torso.cocks.count > 0) DisplayText("package");
		else if (player.torso.vaginas.count > 0) DisplayText("puffy lips");
		else DisplayText("[butt]");
		DisplayText(".  You sigh from the sensation and mumble, \"<i>I thought we were going swimming?</i>\"");
		...
		if (Flags.list[FlagEnum.AMILY_TIMES_SWIMFUCKED] > 1) DisplayText(", and wondering why you keep falling for it");
		...
		if (Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) DisplayText("her silky coat ");
		else DisplayText("her smooth skin ");
		DisplayText("with all ten fingers.  The mousette's tail curls around your [hips] to pull you closer, grinding herself into you with all her might.  You can feel the ");
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] === 0) DisplayText("engorged bud that is her clit");
		else DisplayText("slippery member");
		DisplayText(" sliding back and forth between your drenched bellies.  Amily gasps, \"<i>I love you,</i>\" and her slippery tunnel hugs tight, just like the rest of her body.");
		...
		if (player.torso.cocks.count > 1) {
			...
			else DisplayText(" does");
			DisplayText(" a fine job of spraying ");
			if (player.torso.cocks.count === 2) DisplayText("its load ");
			else DisplayText("their loads ");
			DisplayText("over her middle.");
		}
		DisplayText("  The heat flowing inside her sets off a second orgasm");
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText(", and she climaxes all over again, spraying the underside of her " + amilyTits() + " and your [chest] with milky mouse seed");
		DisplayText(".  Her channel soon drips with your mixed sexual fluids");
		if (player.cumQ() > 250) {
			DisplayText(", but it doesn't stop you from continuing to pump away, ");
		...
		DisplayText(".\n\n");
		DisplayText("Once finished, gently set the moaning girl down and tug her bikini bottom back in place to hold in the cream filling.  Amily moans, \"<i>Pervert,</i>\" but she doesn't stop you.  She even helps you get dressed again, and the two of you walk back to camp, hand and hand, clean and dirty at the same time.");
		player.orgasm();
		player.stats.sens += -1;
		amilyPreggoChance();
		return { next: Scenes.camp.returnToCampUseOneHour };
	...
	public drinkThePotion() {
		...
		if (Flags.list[FlagEnum.AMILY_X_IZMA_POTION_3SOME] === 1) {
			DisplayText("Amily smiles up at you while her tail coils around your [leg], and she warns, \"<i>Now, I've been fiddling with this for a while, but I haven't had a chance to test it, so it could have some unexpected side-effects.</i>\"");
		...
		//Appears in Amily's Sex Menu - Izma3Some
			...
			DisplayText("You ask Amily if she's made up any more of that potion that you, her, and Izma had so much fun with before.");
			...
			if (pregnancy.isPregnant) DisplayText("  At least I can't get any more pregnant!");
			DisplayText("</i>\"");
			...
			if (Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText(", barely concealing the half-swollen bulge beneath");
			DisplayText(".  Izma says, \"<i>What the hell, I ");
			if (izmaScene.pregnancy.isPregnant) DisplayText("suppose I can do it for fun, since I'm already pregnant.");
	...
	private izmaAmilyDrugThreeWaySex() {
		...
		else DisplayText("they've");
		DisplayText(" ever been.  The continuing caresses from your horny females have your [cock biggest] aching with a less than subtle need.  There's a certain, growing tightness in your [balls]");
		if (player.torso.balls.quantity > 0) DisplayText(", one that has your [sack] swelling slightly, the skin growing smooth and glossy as it fills with the heavy, comfortable weight of your sloshing seed");
		else DisplayText(", one that has you feeling comfortably swollen with pent-up seed just waiting to erupt");
		DisplayText(".  Droplets of pre-cum slowly trickle from [eachCock] onto the busy ladies' hands and forearms, turning their fondles into strokes so wet and lubricated that you could almost mistake them for a succubus's twat.  There's something wonderfully right about being served by these docile woman");
		if (!pregnancy.isPregnant && !izmaScene.pregnancy.isPregnant) DisplayText(", with their hungry wombs just waiting to be impregnated at your leisure");
		else if (pregnancy.isPregnant && izmaScene.pregnancy.isPregnant) DisplayText(", with their ripe wombs stuffed full of your offspring already");
		...
		if (Flags.list[FlagEnum.IZMA_NO_COCK] > 0) DisplayText("former-");
		...
		if (Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("The shark-girl's balls are pushed to the side, barely avoiding being smothered in the mouse-girl's weight, and her rigid dick is drooling up between Amily's tits, where it seems quite happy to rest.  ");
		...
		if (player.torso.cocks.count === 2) DisplayText("  You even cram your second member in alongside its big brother.  The two cunt-oiled shafts rub against each other as well as the sloppy holes on either side, leaving you grateful for the extra maleness.");
		else if (player.torso.cocks.count > 2) DisplayText("  You even cram an extra member in alongside its big brother.  The two cunt-oiled shafts rub against each other as well as the sloppy holes surrounding them, leaving you grateful for your extra maleness.  Of course, that isn't your entire package, but there isn't room to stuff any more dick into that stuffed cock-sauna.");
		if (Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("  You even bump up against Izma's forgotten package, slathering it with mixed juices.  While it's rigid with arousal, Izma seems to have almost forgotten about it, instead choosing to focus exclusively on her more womanly parts.");
		...
		if (player.torso.cocks.count > 1) DisplayText("cocks");
		else DisplayText("[cock biggest]");
		DisplayText(".  The sapphic kiss is shattered by your slit-moistening hardness, and Amily looks back at you, disentangling her tail from Izma's to caress you as she asks, \"<i>You aren't going to waste it outside, are you?  Please, cum in me!</i>\"");

		DisplayText("\n\n\"<i>No, give it to me!  ");
		if (izmaScene.pregnancy.isPregnant) DisplayText("My pussy is the obvious, stronger choice.");
		else DisplayText("I'm the stronger, obvious breeding choice.  Knock me up and you'll have fitter, better offspring.");
		...
		if (!pregnancy.isPregnant) DisplayText("Give me your babies...  Mmmm, we'll make such smart babies.  ");
		DisplayText("Just, ahhh... put it inside me and cum, cum in me again and again and fuck me till I'm pregnant with your spunk!</i>\"  She's ");
		if (Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) DisplayText("actually blushing so hard that you can see it through her fur, a crimson tint that looks even better on her with the way her tongue is hanging out");
		else DisplayText("blushing so hard her whole face is almost beet-red, a fine look on her with her tongue dangling so erotically out of her gasping mouth");
		DisplayText(".");
		...
		if (!pregnancy.isPregnant || !izmaScene.pregnancy.isPregnant) DisplayText("womb");
		else DisplayText("cunt");
		DisplayText(", judging by how full your genitals are feeling, but it doesn't matter greatly one way or the other.  There's a calm, confident surety in the back of your mind that keeps reminding you that you're in charge here, and that both these women WILL be ");
		if (!pregnancy.isPregnant || !izmaScene.pregnancy.isPregnant) DisplayText("pregnant");
		else DisplayText("squirting");
		DisplayText(" for you.  Each time you slam your ");
		if (player.torso.cocks.count > 1) DisplayText("doubled dicks");
		else DisplayText("[cock biggest]");
		...
		if (player.torso.balls.quantity > 0) DisplayText("straining balls slosh ");
		else DisplayText("straining fullness tingles ");
		...
		if (!pregnancy.isPregnant) DisplayText("  Her belly begins to puff up from the thorough insemination, at first going a little pudgy, but then turning large and bulbous.  Her empty womb is somehow taking more and more of your virile seed inside.  It's almost like her cervix is sucking down the yogurt-thick load like a one-way valve, and soon Amily's middle is positively jiggly with spunk, bulging out to either side of her from her body weight.");
		else {
			DisplayText("  With her womb already occupied by your offspring, Amily's cervix stays steadfastly closed.  There's so much jizz pumping out of you that the tight canal can't even begin to hold all of it");
			if (player.hasKnot(player.torso.cocks.sort(Cock.LargestCockArea)[0])) DisplayText(", but your knot traps it inside anyway.  You cum and cum until the pressure is almost painful, but even your magnificent knot can't hold it all back");
	...
	private izmaAmilyDrugThreeWaySex2() {
		...
		DisplayText("pregnant belly and murmuring, \"<i>Babies,</i>\" while her fingers mindlessly diddle her alabaster-painted cunt.  She shudders as aftershocks of pleasure torture her lust-wracked brain, keeping her in a horny, docile state.");
		DisplayText("\n\nIzma seems to have taken advantage of your lapse in consciousness - you're on your back, and she's straddling your chest");
		if (Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText(", dick and balls flopping lamely on top of you");
		...
		if (!izmaScene.pregnancy.isPregnant) DisplayText("be inseminated");
		else DisplayText("have her pregnant-pussy packed full again");
		DisplayText(".");

		DisplayText("\n\nBehind her, [eachCock] has remained triumphantly stiff, and ");
		if (player.torso.balls.quantity === 0) DisplayText("the pressure is as omnipresent as ever");
		else DisplayText("your [balls] are stretching your [sack] as tight as before");
		...
		if (Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText(" along with a matching jet of pre-cum from her tip");
		DisplayText(".");
		if (player.torso.cocks.count > 1) {
			DisplayText("  Strong hands wrap around ");
			if (player.torso.cocks.count > 2) DisplayText("two of ");
			DisplayText("your extra penis");
			if (player.torso.cocks.count > 2) DisplayText("es");
			DisplayText(", jacking ");
			if (player.torso.cocks.count === 1) DisplayText("it");
			else DisplayText("them");
			DisplayText(" with smooth, confident strokes.  \"<i>I bet the mouse couldn't tend to your needs like this, huh?</i>\"");
		}
		...
		if (Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText(", ignoring the dribbly cock that's leaking on your chest");
		DisplayText(".  Instinctive, half-thought voices remind you that is how it should be - so virile that you need a herd of woman to take turns satisfying your insatiable desires.  Lucky for you, you've got the start of one right here.");

		DisplayText("\n\nIzma leans down");
		if (player.torso.cocks.count === 2) DisplayText(", taking a hand away from your extra dick to support herself,");
		if (player.torso.cocks.count > 2) DisplayText(", taking a hand away from one of your extra dicks to support herself,");
		DisplayText(" and kisses you passionately.  The fervent kiss is as short as it is frenzied, but as you're recovering from it, Izma starts to talk, \"<i>You like that, Alpha?  You like having my cunt suckling on your dick?</i>\"  She swivels her hips with her words, the inner tentacles all pulling on you in concerted waves, actually lifting your penis to press deeper inside her, right up against her ");
		if (izmaScene.pregnancy.isPregnant) DisplayText("closed");
		else DisplayText("dilated");
		DisplayText(" cervix.  \"<i>");
		if (izmaScene.pregnancy.isPregnant) DisplayText("I know I'm pregnant, but I want you to cum as deeply inside me as possible, okay?  I just... I have to have your cum. I NEED to feel that warmth flooding inside me.  It'll... ohh, yes... it'll make me feel so good, so loved.");
		else DisplayText("I don't know how, but I just know that you're going to make me pregnant when you cum inside me.  It's going to get me oh-ohhhhh... so heavy with your child, and it'll be perfect and blissful.  You're going to fucking flood me with jizz, aren't you, Alpha?");
		DisplayText("</i>\"  Izma's eyes twinkle with a mixture of obscene hunger and overbearing affection.  She whispers, \"<i>Will you please");
		if (!izmaScene.pregnancy.isPregnant) DisplayText(" make me your gravid, jizz-slurping beta-wife?");
		else DisplayText(" cum in your pregnant, jizz-hungry wife?");
		DisplayText("</i>\"");

		DisplayText("\n\nFuck, it's like she's telling you to do exactly what you want to do!  There's nothing hotter than ");
		if (!izmaScene.pregnancy.isPregnant) DisplayText("watching a bitch's belly bloat as it's packed to a full, fertile dome");
		else DisplayText("stuffing a cunt so full she'll never stop dripping your seed");
		...
		if (Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("  Her four balls quake atop you, and you watch her urethra bulge, ready to splatter your [chest] with her own, lesser climax.");
		DisplayText("  Lady-spunk washes across your middle as her walls begin to wring your length, squeezing it just firmly enough to make your ardor flare hotter, triggering an avalanche of feelings within you that you could not hope to repress.");
		if (Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("  The smell of her hot, inferior seed hits your nostrils, exciting you even further as it rolls off of you in thick globs.");
		...
		if (izmaScene.pregnancy.isPregnant) {
			DisplayText("  Her womb remains steadfastly closed, already seeded to capacity, so your salty deposit quickly floods the smaller canal.  The tendrils go lax as they're dipped in your baby-batter, each limp in the heady flow.");
			if (player.hasKnot(player.torso.cocks.sort(Cock.LargestCockArea)[0])) DisplayText("  Your knot balloons explosively inside her, but the seal on her womb is tighter.");
			DisplayText("  Jism rushes out around your cock in a river.  The nest was already soaked with liquid lust from your last tryst, moments ago, but now it's soaked completely white.  After a while, Izma's actually forced off you by the incredible potency of your eruptions, falling flat on her back.");
		}
		else {
			...
			if (player.hasKnot(player.torso.cocks.sort(Cock.LargestCockArea)[0])) DisplayText("  Of course, with a knot like yours there's no need for that.  You keep her sealed up so tightly that she has no choice but to take every drop.");
			DisplayText("  The tigershark babbles, \"<i>So... full</i>\" and topples over backward under her own doubled weight");
			if (player.hasKnot(player.torso.cocks.sort(Cock.LargestCockArea)[0])) DisplayText(", your dong forced out with a noisy 'SCHLURRRPOP'");
			DisplayText(".");
		}
		DisplayText("\n\nIzma pants, \"<i>Ung... mmm... so much,</i>\" and cradles her ");
		if (!pregnancy.isPregnant) DisplayText("jizz-bloated");
		else DisplayText("child-bearing");
		...
		if (player.torso.balls.quantity > 0) DisplayText("[balls] are");
	...
	private izmaAmilyDrugThreeWaySex3() {
		...
		if (!izmaScene.pregnancy.isPregnant || !pregnancy.isPregnant) {
			...
			if (!pregnancy.isPregnant) {
				...
				if (!izmaScene.pregnancy.isPregnant) DisplayText(" and ");
			}
			if (!izmaScene.pregnancy.isPregnant) {
				DisplayText("Izma");
				izmaScene.pregnancy.knockUpForce(PregnancyType.PLAYER, PregnancyType.INCUBATION_IZMA);
			}
			DisplayText(" definitely got pregnant.</b>)");
		}
		return { next: Scenes.camp.returnToCampUseFourHours };
	}
	...
	private amilyNurseCheckupV2(repeat: boolean) {
		...
		switch (pregnancy.event) {
			case 2:
			case 3: DisplayText("slightly-protruding pregnancy is exposed beneath it");
				break;
			case 4:
			case 5: DisplayText("pregnancy-swollen tummy rounds out beneath it");
				break;
			case 6:
			case 7:
			case 8: DisplayText("pregnancy-bloated belly bulges out beneath it");
				break;
			default: DisplayText("taut midriff is fully on display");
		}
		DisplayText(".  The clingy bottom is little more than a micro-skirt, short enough that it nearly exposes her vagina to your roaming eyes.");
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText("  It bulges out lewdly in a sheer outline of her " + amilyCock() + ", which is curled over to remain inside the stretchy skirt.");
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 9) DisplayText("  The effort is a failure, as her immense cock is simply too big for such a slinky covering, and the fat tip curves down to dangle below the hem, pulsing dangerously.");
		DisplayText("  To complete the outfit, a tiny, white hat with a red cross adorns her head, tilted slightly off-center.");
		...
		if (Flags.list[FlagEnum.AMILY_NOT_FURRY] === 1) DisplayText("hand");
		else DisplayText("paw");
		DisplayText(" touches your thigh as Amily sets the 'clipboard' aside.");
		...
		if (player.torso.balls.quantity === 0) DisplayText(", testing to engorgement with soft squeezes that are almost painful with how aroused you've become.");
			...
			DisplayText(", eventually coming to rest on your [sack].  She dutifully hefts your [balls], ");
			if (player.torso.balls.size <= 4) DisplayText("rolling them in her fingers with a sly smile");
			else DisplayText("letting her fingers sink into one of the swollen orbs with a sly smile");
			DisplayText(".");
		}
		DisplayText("  Watching your face the whole time, Amily smiles seductively when you begin to pant.  You miss it, too busy looking down her exposed cleavage, enhanced by the chest-compressing tightness of her naughty nurse's outfit.  A none-too-subtle squeeze on your sensitive genitals draws you back to the present");
		if (player.torso.vaginas.count > 0) DisplayText(", while a sneaky finger slides into your cleft, testing the wetness");
		DisplayText(".");

		DisplayText("\n\nAmily explains, \"<i>It's ");
		if (player.torso.balls.quantity > 0) DisplayText("Blue Balls");
		else DisplayText("Boneritus");
		...
		if (player.torso.balls.quantity > 0) DisplayText("plump balls are");
		else DisplayText("stiff shaft is");
		DisplayText(" freed from the perverted needs that afflict you.</i>\"");
		...
		if (player.torso.cocks.count > 1) {
			...
			else if (player.torso.cocks.count === 2) DisplayText("your extra cock");
			DisplayText(".  Slowly, she pumps the spare shaft, muttering something about, \"<i>Treating allll the symptoms.</i>\"");
		}
		...
		if (player.cumQ() >= 500) {
			DisplayText("  The steady rhythm of your spurts goes on unabated for well over thirty seconds, ");
			if (!pregnancy.isPregnant) DisplayText("slowly filling Amily's womb to capacity");
			else DisplayText("slowly forcing trails of semen out of her entrance");
			DisplayText(".");
			if (player.cumQ() >= 2000) {
				DisplayText("  Amily hangs on for dear life as ");
				if (!pregnancy.isPregnant) DisplayText("she's filled to capacity, her stretchy, mallaeble middle bulging lewdly with spunky jizz, sloshing with each fresh injection.");
				else DisplayText("torrents of the stuff spray out of her entrance, her canal too filled to hold even a drop more and your penis continuing to push the pressure higher.");
			}
			if (player.cumQ() >= 500) DisplayText("  It turns your crotch into a sticky mess.");
		}
		...
		if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText(" that flings sticky ropes across your [chest].  Her back arches as she continues to launch her own jism towards you");
		DisplayText(", and then she too sags down against you, hugging you as she says, \"<i>That was... intense.</i>\"  She tries to rise, but her legs fall limp halfway up.");
		if (player.hasKnot(x)) DisplayText("  She tries again.  This time, it's your knot that keeps her rooted firmly in place, and she stays plugged like that for a good twenty minutes, her cunt basting in your animal-spooge as she enjoys the fullness only a thick knot can provide.");
		DisplayText("  Finally, she makes it up and off of you.  Her pussy is nice and wet, matted with white.");
