...
import { NextScreenChoices, ScreenChoice, ClickFunction } from "../../ScreenDisplay";
import { randInt } from "../../../Engine/Utilities/SMath";
import { PregnancyType, IncubationTime, Pregnancy } from "../../Body/Pregnancy/Pregnancy";
import { CView } from "../../../Engine/Display/ContentView";
import { returnToCampUseOneHour, returnToCampUseFourHours, sleepRecovery } from "../Camp";
import { demonRaceScore } from "../../Body/RaceScore";
...
import { describeOneOfYourCocks, describeCocksLight, describeCock, describeCockHead, nounCock } from "../../Descriptors/CockDescriptor";
...
import { vapulaSlave } from "./Vapula";
import { campCorruptJojo } from "./JojoScene";

export const SheilaFlags = {
    SHEILA_CLOCK: 0,
    SHEILA_CORRUPTION: 0,
    SHEILA_PREGNANCY_INCUBATION: 0,
    SHEILA_XP: 0,
    SHEILA_DEMON: 0,
    SHEILA_JOEYS: 0,
    SHEILA_DISABLED: 0,
    SHEILA_CITE: 0,
    UNKNOWN_FLAG_NUMBER_00141: 0,
    MINOTAUR_CUM_ADDICTION_TRACKER: 0,
    IZMA_NO_COCK: 0,
    LUMI_MET: 0,
    FUCK_FLOWER_LEVEL: 0,
    FUCK_FLOWER_KILLED: 0,
    ANEMONE_WATCH: 0,
    SLEEP_WITH: 0,
...
User.flags.set(FlagType.Sheila, SheilaFlags);

export class SheilaTimeAware implements ITimeAware {
    ...
    public timeChange(): boolean {
        flagWomb.update();
        if (Time.hour > 23) {
            if (SheilaFlags.SHEILA_CLOCK < 0) SheilaFlags.SHEILA_CLOCK++;
        }
        return false;
    }
    ...
    public deserialize(saveObject: SheilaTimeAware): void {
        this.womb.deserialize(saveObject.womb);
    }
}

const sheilaTimeAware = new SheilaTimeAware();
const flagWomb = sheilaTimeAware.womb;

function sheilaPreg(player: Character, reducedChance: boolean = false) {
    // CHANCE OF PREGGERS, GAO
    if (flagWomb.isPregnant()) return;
    let chance: number = 5;
    ...
    if (randInt(100) <= chance)
        flagWomb.knockUp(new Pregnancy(PregnancyType.PLAYER, IncubationTime.SHIELA));
}

export function sheilaCorruption(arg: number = 0): number {
    /*
    if(SheilaFlags.SHEILA_CORRUPTION + arg > 100) SheilaFlags.SHEILA_CORRUPTION = 100;
    else
    */
    if (SheilaFlags.SHEILA_CORRUPTION + arg < 0) SheilaFlags.SHEILA_CORRUPTION = 0;
    else SheilaFlags.SHEILA_CORRUPTION += arg;
    return SheilaFlags.SHEILA_CORRUPTION;
}

// Sheila's cup function!
export function sheilaCup(player: Character): string {
    if (SheilaFlags.SHEILA_CORRUPTION < 10) return "tiny, almost-absent";
    else if (SheilaFlags.SHEILA_CORRUPTION < 20) return "A-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 30) return "B-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 40) return "C-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 50) return "D-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 60) return "DD-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 70) return "E-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 80) return "EE-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 90) return "front-running";
    else if (SheilaFlags.SHEILA_CORRUPTION < 100) return "FF-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 110) return "G-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 120) return "GG-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 130) return "H-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 140) return "HH-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 150) return "'Holy Honkin' Hooters!'-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 160) return "I 'want to squeeze them'-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 170) return "II-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 180) return "Jiggle-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 190) return "JJ-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 200) return "K-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 210) return "KK-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 220) return "L-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 230) return "'Ladylove'-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 240) return "M-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 250) return "'Mega Milk'-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 260) return "N-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 270) return "NN-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 280) return "O-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 290) return "'so big it makes you say' OO-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 300) return "P-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 310 && player.body.cocks.length > 0) return "'would feel great around your' PP-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 310 && player.body.cocks.length === 0) return "PP-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 320) return "Q-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 330) return "QQ-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 340) return "R-u-serious?-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 350) return "RR-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 360) return "S-rank";
    else if (SheilaFlags.SHEILA_CORRUPTION < 370) return "'Shooting Star'-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 380) return "T-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 390) return "'damn, those are some big' TT-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 400) return "'bigger than' U-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 410) return "UU-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 420) return "V-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 430) return "'Va-Va-Voom'-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 440) return "'War Head'-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 450) return "World-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 460) return "X-Buster-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 470) return "XXXX-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 480) return "'Yay'-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 490) return "YY-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 500) return "Z-Saber-cup";
    else if (SheilaFlags.SHEILA_CORRUPTION < 510) return "ZZ-cup";
    else return "fuckin' magic";
}

export function sheilaEncounterRouter(player: Character): NextScreenChoices {
    // UNFUCK FLAGS
    if (SheilaFlags.SHEILA_CLOCK > 0)
        SheilaFlags.SHEILA_CLOCK = 0;
    if (sheilaCorruption() >= 100 && SheilaFlags.SHEILA_DEMON === 0)
        SheilaFlags.SHEILA_DEMON = 1;
    if (SheilaFlags.SHEILA_DEMON === 1)
        SheilaFlags.SHEILA_XP = 4;

    // ACTUALLY PICK ENCOUNTER!
    if (SheilaFlags.SHEILA_DEMON === 0) {
        // PREG SHIT
        if (flagWomb.isPregnant() && flagWomb.pregnancy.incubation === 0 && SheilaFlags.SHEILA_XP >= -2) {
            if (SheilaFlags.SHEILA_JOEYS === 0) return normalSheilaPregNotifNumberOne(player);
            else return normalSheilaPregNotifREPEATEDED(player);
        }
        if (SheilaFlags.SHEILA_XP === 0) return sheilaFirstEncounter(player);
        else if (SheilaFlags.SHEILA_XP === -1) return sheilaApology(player);
        // XP-2 Sheila's apology (sheila xp = -2 and demon sheila = 0):
        else if (SheilaFlags.SHEILA_XP === -2) return sheilaIsSorrySheRapedYou(player);
        // XP-3 or XP-4 She-mad (Sheila XP <= -3 and demon sheila = 0):
        else if (SheilaFlags.SHEILA_XP === -3 || SheilaFlags.SHEILA_XP === -4) return sheilaReallyMad(player);
        // XP1: Reconciliation encounter (sheila xp = 1 and demon sheila = 0):
        else if (SheilaFlags.SHEILA_XP === 1) return sheilaReconcile(player);
        // XP2: Familiarizing (Sheila XP = 2; or Sheila XP = 3 AND time =/= 20:00 and demon sheila = 0):
        else if (SheilaFlags.SHEILA_XP === 2 || (SheilaFlags.SHEILA_XP === 3 && Time.hour !== 20)) return sheilaGettingFamiliar(player);
        // XP3: Sexy Time (sheila xp = 3 AND time = 20:00 and demon sheila = 0):
        else if (SheilaFlags.SHEILA_XP === 3 && Time.hour === 20) return sheilaXPThreeSexyTime(player);
        // XP4: Fancy meeting you here (sheila xp >= 4 and demon sheila = 0):
        else if (SheilaFlags.SHEILA_XP === 4) return fuckBuddySheilaMeeting(player);
        else {
            CView.text("A BUG HAPPENED.  YOUR SHEILA_XP: " + SheilaFlags.SHEILA_XP + ". <b>Value should be between -3 to 4.</b>");
    ...
    // DEMONS!
        ...
        if (flagWomb.isPregnant() && flagWomb.pregnancy.incubation === 0) return demonImpChildren(player);
        // Demon Sheila encounter (demon sheila = 1 and sheilapreg < 4)
        else return demonSheilaEncounter(player);

    }
}

function sheilaCorruptionUpdate(player: Character) {
    ...
    if (player.stats.cor > 90) {
        ...
        const corruptionDiff = Math.abs(Math.round(player.stats.cor) - Math.round(sheilaCorruption()));
        if (player.stats.cor > sheilaCorruption()) {
            // Big change, dump ten.
            if (corruptionDiff >= 10) {
                player.stats.cor += -10;
                sheilaCorruption(10);
            }
            else {
                // Small change, do as much as possible.
                player.stats.cor -= corruptionDiff;
                sheilaCorruption(corruptionDiff);
            }
        }
        else if (player.stats.cor < sheilaCorruption()) {
            // Big Change!
            if (corruptionDiff >= 10) {
                player.stats.cor += 10;
                sheilaCorruption(-10);
...
// [XP0 - Stay Hidden]
function sheila1stEncStayHidden(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You regulate your breathing and hold yourself still to avoid rustling anything softly, waiting for the creature to finish its business and leave.");
    // no change in sheila xp
    return { next: returnToCampUseOneHour };
}

// [XP0 - Look Closer]
function sheila1ndEncLookCloser(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Your head pokes up a bit to get a better look at the rest of the creature.  It stands flat on its long feet and cranes its head further as well, taking in more of the gently rolling countryside, but then catches you out of the corner of its eye!  With a yelp it pushes itself up on its hindpaws again and calls, in an unmistakably feminine voice, \"<i>Friend or foe?!</i>\"");
...
function sheila1ndEncLookCloserPtII(player: Character): NextScreenChoices {
    ...
    // [(libido>=50)
    if (player.stats.lib >= 50) CView.text("  Noting you still checking out her body, she smirks and says, \"<i>Enjoying your lookie, then?  Up here, cobber,</i>\" causing you to snap your head up sheepishly.");
    CView.text("  You nod at the strange woman and introduce yourself.");
    if (player.desc.name !== "Sheila") CView.text("  \"<i>" + player.desc.name + ", eh?  My mates just call me Sheila.  Bunch of jokers, the bastards.</i>\"");
    else CView.text("  \"<i>You too, eh?  Sheila's what my mates call me for a nickname.  Don't suppose you got yours from a bunch of adolescent-minded wankers, though.</i>\"");
    ...
    // [(high demon score)
    if (demonRaceScore(player) >= 4) CView.text("  I mean, take a squizz at yourself.  It's like you're not even trying to hide it; I've seen imps that look more like normal people.");
    CView.text("  Forget it.</i>\"  You deny the accusation, but she continues to glare at you.");
    ...
    return {
        choices: [
            ["Talk", sheila1ndEncLookCloserPtIITalk],
            ["Leave", sheila1ndEncLookCloserPtIILeave],
        ]
    };
}

// [XP0 - Look Closer - Leave]
function sheila1ndEncLookCloserPtIILeave(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Good onya.  Maybe I'll see you around.</i>\"  With that, you take your leave of the strange woman.");
    // set sheila xp = 1
    SheilaFlags.SHEILA_XP = 1;
    return { next: returnToCampUseOneHour };
}

// [XP0 - Look Closer - Talk]
// set sheila xp = -1
function sheila1ndEncLookCloserPtIITalk(player: Character): NextScreenChoices {
    CView.clear();
    SheilaFlags.SHEILA_XP = -1;
    CView.text("\"<i>Alright, my earbashing mug, have it your own way.  I get mean when I get hungry; don't say I didn't warn ya.</i>\"  With that, Sheila takes a swing at you!  You jerk backwards to avoid the punch but move too slowly, she clips the end of your [face].  What a wind!  If she'd been trying to hit you, that sucker punch would likely have laid you out.  Just as you open your mouth to speak again, she rears back on one leg and kicks you away hard with the other, knocking the air out of you.  You land painfully on your back.");
    player.stats.HP -= 2;
    ...
    choices[0] = ["Fight", sheila1ndEncLookCloserPtIITalkFight];
    choices[4] = ["Leave", sheila1ndEncLookCloserPtIITalkLeave];
    return { choices };
}

// [XP0 - Look Closer - Talk - Leave]
function sheila1ndEncLookCloserPtIITalkLeave(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Wordlessly, you get up and back away from the woman... though not without a dirty glance.  She follows you with her eyes until you judge yourself far enough away to turn your back to her.");
    // go to camp
    return { next: returnToCampUseOneHour };
}
...
// go to fight
function sheila1ndEncLookCloserPtIITalkFight(player: Character): NextScreenChoices {
    CView.clear();
    return CombatManager.beginBattle(player, new Sheila());
}

// XP-1: PC's apology (sheila xp = -1 and demon sheila = 0):
function sheilaApology(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("As you search the plains for god-knows-what, you catch your [leg] on something; a rustling in the brush below you is the only warning you get before a cleverly hidden snare closes on it!");
    ...
    return {
        choices: [
            ["Apologize", apologySheilaApology],
            ["SlapNInsult", apologySheilaSlapSult],
            ["Nothing", apologySheilaSayNothing],
            ["Avoid Her", apologyAvoidSheila],
        ]
    };
}

// [XP-1 - Avoid Her]
function apologyAvoidSheila(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You shake your head at the leery girl, and tell her you'd be just as happy as she would if you never saw each other again.");
    ...
    // set sheilapreg = -1
    SheilaFlags.SHEILA_DISABLED = 1;
    return { next: returnToCampUseOneHour };
}

// [XP-1 - Apologize]
function apologySheilaApology(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You scratch your " + skinFurScales(player) + " absently behind an ear, not sure how to broach the topic of reconciliation when you feel like she was the aggressor, but mumble something about being sorry things started off on the wrong foot.");
...
// [XP-1 - Slap'n'sult]
function apologySheilaSlapSult(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You say nothing as she finishes with the snare and stands back up.");
    ...
    SheilaFlags.SHEILA_XP = -3;
    return CombatManager.beginBattle(player, new Sheila());
}

// [XP-1 - Nothing]
function apologySheilaSayNothing(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You decide to keep your own counsel for now, folding your arms defensively and frowning.");
    ...
    CView.text("\n\nYou grunt and watch her resume her cover in the long grass, then turn about and head back to ");
...
function sheilaIsSorrySheRapedYou(player: Character): NextScreenChoices {
    ...
    const choices: ScreenChoice[] = [];
    choices[0] = ["Forgive", sheilaIsSorryAndYouForgive];
    choices[1] = ["Fight", sheilaPologyFight];
    if (player.effects.has(StatusEffectType.KnowsArouse)) choices[2] = ["CastArouse", sheilaPologyCastArouse];
    choices[3] = ["LemmeAlone", sheilaIsSorryButLeaveMeAlone];
    return { choices };
}

// [XP-2 - Leave Me Alone]
function sheilaIsSorryButLeaveMeAlone(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You shake your head and turn away, dismissing Sheila with a pointed gesture.  \"<i>H-hey!</i>\" the girl calls angrily from behind you.  \"<i>Dammit, I said I was sorry, you bastard!  Screw you!  See if I talk to you again!</i>\"");
    // set sheilapreg = -1
    SheilaFlags.SHEILA_DISABLED = 1;
    return { next: returnToCampUseOneHour };
}
// [XP-2 - Forgive]
function sheilaIsSorryAndYouForgive(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Okay.</i>\"");
    ...
    if (SheilaFlags.SHEILA_JOEYS > 0) SheilaFlags.SHEILA_XP = 4;
    else SheilaFlags.SHEILA_XP = 1;
    return { next: returnToCampUseOneHour };
}

// [XP-2 - Fight]
function sheilaPologyFight(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Saying nothing, you raise your [weapon] and take a swing!  Sheila starts a bit and dodges just in time, then with a look of resignation, raises her fists as well.  \"<i>That'll be right, I guess.  For what it's worth, I really am sorry.</i>\"");
...
function sheilaPologyCastArouse(player: Character): NextScreenChoices {
    ...
    CView.text("\n\nBetter and better.");
    player.stats.lustNoResist += (player.stats.lib / 5);

    // [Let Her][Leave]

    const choices: ScreenChoice[] = [];
    choices[0] = ["Let Her", castArouseAndLetHer];
    choices[4] = ["Leave", castArouseAndLeaveSheila];
    return { choices };
}

// [XP-2 - Cast Arouse - Leave]
function castArouseAndLeaveSheila(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You shake your head sadly.  \"<i>I'm sorry, I haven't the time right now.</i>\"");
    ...
    SheilaFlags.SHEILA_CORRUPTION = 100;
    SheilaFlags.SHEILA_DEMON = 1;
    SheilaFlags.SHEILA_CITE = -1;
    player.stats.cor += 10;
...
function castArouseAndLetHer(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("As soon as you nod, the girl bounds to her feet and jumps you, driving you slightly downward in her ardor.");
    ...
    return { next: sheilaLossRapeRouter };
}

// XP-3 or XP-4 She-mad (Sheila XP <= -3 and demon sheila = 0):
function sheilaReallyMad(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("A rustle in the bush claims your attention, and you turn your head to see Sheila, standing up suddenly.  Her eyes narrow when she recognizes you, and she takes off running!");
    ...
    if (SheilaFlags.SHEILA_CORRUPTION >= 90) CView.text("It's so hard to remember myself anymore, and the taint you've put in my body goes crazy when you touch me.  Do you want me to get on my hands and knees?</i>\"  The woman lowers herself to the ground.  \"<i>I beg you.  ");
    CView.text("Please, please leave me alone.");
    if (SheilaFlags.SHEILA_XP === -3) CView.text("  I know you... you're not as bad as the rest of the monsters out here, so... please.");
    CView.text("</i>\"");
    // [Stand][Leave]

    const choices: ScreenChoice[] = [];
    choices[4] = ["Leave", sheilaReallyMadLeave];
    choices[0] = ["Stand", sheilaReallyMadStandGround];
    return { choices };
    // spacebar default should be leave
}

// [XP-3 - Leave]
function sheilaReallyMadLeave(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Huffing, you throw the animals down and turn away.  What a display.");
    // go to camp, reincarnate as something slightly better next life
    return { next: returnToCampUseOneHour };
}

// [XP-3 - Stand]
function sheilaReallyMadStandGround(player: Character): NextScreenChoices {
    CView.clear();
    // [(normal mode)]
    if (!User.settings.silly()) {
        CView.text("You throw the animals behind you and fold your arms.");
        CView.text("\n\nSheila shivers, then speaks.  \"<i>Alright, mate.  If that's the way it has to be.</i>\"  She raises her fists half-heartedly.");
    }
    // (silly mode)
    else {
        CView.text("Throwing the bundle to the side dramatically, you call the name of your stand!  Sheila's eyes widen as your body surges with renewed energy, and she jumps back as a ghostly other self materializes next to you.");
    ...
    const monster = new Sheila();
    if (User.settings.silly()) {
        monster.stats.HP *= 1.2;
...
function sheilaReconcile(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Oy, it's you,</i>\" sings out a voice behind you.  \"<i>Back again?</i>\"");
    ...
    const choices: ScreenChoice[] = [];
    choices[0] = ["Kay", sheilaReconcileKay];
    choices[1] = ["Don't Wanna", sheilaReconcileDunWanna];
    return { choices };
}

// [XP1 - Don't Wanna]
function sheilaReconcileDunWanna(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Raising your hands in a palms-out gesture, you decline.  Sheila colors a bit, misinterpreting your reluctance.  \"<i>Hey, it's not like I'm cracking onto you or anything.  I was just looking for a reason to eat half a rabbit myself, is all.</i>\"");
...
// [XP1 - Kay]
function sheilaReconcileKay(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Some fresh meat does sound pretty tempting.  You nod at the woman and she climbs to her feet.  \"<i>Beaut!  Do me the favor and fossick about for some tinder.</i>\"");
    // --Next--

    return { next: sheilaReconcileKay2 };
}

function sheilaReconcileKay2(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Soon enough you've got a firepit going in a small clearing.  \"<i>Ahh, classic bush theater,</i>\" Sheila says, admiring it.  The woman dextrously spits two liberated and skinned rabbit quarters on a long stick and twirls them over the lapping flame, and soon the air is full of the sizzle and pop of meat juices dripping on the hot stones and the simple tune your cook hums.  You stare at ");
    if (player.stats.lib < 50) CView.text("the beckoning hunks of meat");
    else CView.text("her rising and falling breasts under the pretext of watching her work");
    CView.text(", until she happens to look up and catch your eye.  \"<i>Sorry, no parsley,</i>\" she says, breaking into an obliviously happy smile.  Seems like her temperament preternaturally suits this lifestyle.");
    ...
    // [(pc is kangaroo)
    if (describeRace(player) === "kangaroo-morph") CView.text(" - except you, I suppose");
    CView.text(".  We trade our goods in prearranged op shops and dead drops but avoid bringing outsiders into our own settlements, even when they're right corkers.</i>\"");
    ...
    // [(lib >50)]
    if (player.stats.lib >= 50) CView.text(" like yourself - I saw you perving -");
    CView.text(" make us a little hornier, and evil buggers make us a bit more evil.  Our bodies tend to change on their own to reflect how tainted we are... which is as much as saying there are a sad lot among us who flirt with demonic energy to up our sizes to suit our egos.  But versus the sexual appetite and limitless rapacity of the demons themselves...?</i>\"");
    ...
    // set sheila xp to 2
    SheilaFlags.SHEILA_XP = 2;
    return { next: returnToCampUseOneHour };
}

// XP2: Familiarizing (Sheila XP = 2; or Sheila XP = 3 AND time =/= 20:00 and demon sheila = 0):
function sheilaGettingFamiliar(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Hey, it's [name].</i>\"");
    ...
    const choices: ScreenChoice[] = [];
    choices[0] = ["Here Often?", sheilaFriendlyComeHereOften];
    choices[1] = ["Her People", sheilaFriendlyHerPeoples];
    choices[2] = ["Curfew?", sheilaFriendlyCurfew];
    choices[3] = ["No Questions", sheilaFriendlyNoQuestions];
...
// [XP2 - No Questions]
function sheilaFriendlyNoQuestions(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You decide to hold your tongue and simply mimic her pose as best you can, relaxing on the grass.  The sun is hot but its angle not insistent here, thanks to the shelter from a broad tree nearby.  Its soft rays almost reduce you to sleep over the next forty-five minutes before your companion speaks, rousing you.");
...
// [XP2 - Come Here Often?]
function sheilaFriendlyComeHereOften(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You ask her why she enjoys being on the plains so much.");
...
// [XP2 - Her People]
function sheilaFriendlyHerPeoples(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You lie down on the grass yourself, balance the parcel on your stomach, and ask about her people.");
    ...
    if (player.body.femininity < 35) CView.text("  She looks absently at your crotch.");
    ...
    if (player.stats.lib < 50) {
        ...
        CView.text("\n\n\"<i>Anyway,</i>\" she resumes, sobering up, \"<i>it's no more precarious than anyone else's situation as long as the demons control the skies and poison the world.</i>\"  She climbs to her feet and picks up the bundle of caught   \"<i>You're making me wanna check on my mob, [name].  I'll catch you later.</i>\"");
    }
    // (lib >=50)
    else {
        ...
        if (player.body.cocks.length > 0) CView.text(", and you raise the kids at home?  Or put them with a nanny");
        CView.text("?</i>\"");

        CView.text("\n\n\"<i>Yeah, I suppose that'll work");
        if (player.body.cocks.length > 0) CView.text(", and I'll turn the kids over to");
        CView.text(" - no, I mean, ");
        if (player.body.cocks.length <= 0) CView.text("if I decide");
        else CView.text("I'm not ready for");
        CView.text(" - er... fuck <b>you!</b></i>\"  This last exclamation comes in response to your pealing laughter.  \"<i>Wanker!</i>\"");
...
// [XP2 - Curfew?]
function sheilaFriendlyCurfew(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Curfew?</i>\"");
...
// XP3: Sexy Time (sheila xp = 3 AND time = 20:00 and demon sheila = 0):
export function sheilaXPThreeSexyTime(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You prowl through the twilit grasslands, lazily looking for anything interesting before the bleak, red moon brings on the night and its restrictions.  Nothing much catches your eye, except for a rounded shape cast in silhouette against the horizon.  Initially taking it for a rock, you're surprised when it shivers at the sound of your approach!  It unbends and raises a head, turning toward you and revealing the flushed face of the bush girl, Sheila.  Her eyes quickly change from terror to relief as they settle on you, and she cries out.  \"<i>[name]!  Oh thank god; please help me!</i>\"");
    CView.text("\n\n\"<i>What's wrong?</i>\"");
    ...
    return {
        choices: [
            ["Sex", sheilaXPThreeSexyTimeSex],
            ["Guard Duty", sheilaXPThreeSexyTimeGuardDuty],
            ["Ditch Her", sheilaXPThreeSexyTimeDitchHer],
        ]
    };
}

// [XP3 - Ditch Her]
function sheilaXPThreeSexyTimeDitchHer(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You shake your head, pulling away from the woman and telling her she'll be fine.  \"<i>No!</i>\" she shrieks.  \"<i>You gotta help me!</i>\"");
...
// [XP3 - Sex]
function sheilaXPThreeSexyTimeSex(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Sheila's eyes widen with your smile, and she pulls back from you.  \"<i>That's, um...</i>\"");
    ...
    return { choices: sheilaXP4SexMenu(player) };
    // da choices from after XP3 sex

}

// [XP3 - Sex - any sex scene - Let Her Be]
function sheilaXPThreeSexyTimePostSexLetHerBe(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Pulling away from you, Sheila gathers her clothing and departs, looking satisfied.  The glance she gives you over her shoulder is even a little warmer than before.");
    // i had snoo-snoo!
    SheilaFlags.SHEILA_XP = 4;
    return { next: returnToCampUseOneHour };
}
...
function shielaXPThreeSexyTimePostSexStay(player: Character): NextScreenChoices {
    CView.clear();
    SheilaFlags.SHEILA_XP = 4;
    CView.text("You grab her tail, provoking a curious, but not discouraging, look from the woman who was planning to spend the night alone.  \"<i>What is it, [name]?</i>\"");
    ...
    return { next: shielaXPThreeSexyTimePostSexStayII };
}
// fork
function shielaXPThreeSexyTimePostSexStayII(player: Character): NextScreenChoices {
    CView.clear();
    // (has cockfit 32)
    if (player.body.cocks.length > 0 && player.body.cocks.find(Cock.CockThatFits(32))) {
        CView.text("Her hand slips down to your groin and, overcoming its trepidation, grabs your [cockFit 32] and begins to pump languidly, slowly stimulating blood back into it.  \"<i>I don't know what's come over me,</i>\" she says, giggling.  \"<i>Maybe I'm just greedy, but... I want you again.</i>\"  She pushes you");
        // [(horse)]
        if (player.body.legs.isTaur()) CView.text(" and you playfully roll");
        CView.text(" onto your back; Sheila slips her leg over you");
        if (player.body.legs.isTaur()) CView.text("r own");
        CView.text(", positioning herself atop your crotch and guiding your tip to her pussy.  It pokes through the labia slightly, welcomed with warm wetness.  \"<i>Is... this ok, [name]?  Can I?</i>\"");
        ...
        // [(not horse)]
        if (!player.body.legs.isTaur()) CView.text(" just above one " + describeNipple(player, player.body.chest.get(0)));
        else CView.text(" on the thigh");
        CView.text(".  \"<i>I'm gonna have to punish you for it, you dag.</i>\"  Excusing herself thusly, she sits up and begins to slide up and down your [cockFit 32].  ");
        // [(not horse)]
        if (!player.body.legs.isTaur()) CView.text("Your hands squeeze her hips almost of their own volition, helping her slam down with force, and her head rolls back on her neck.  ");
        CView.text("Her ");
        // [(sheila corruption < 30)]
        if (SheilaFlags.SHEILA_CORRUPTION < 30) CView.text("little breasts rise and fall with increasing frequency atop her ribcage as her pace picks up and her breaths come short and shallow.");
        else if (SheilaFlags.SHEILA_CORRUPTION > 60) CView.text("huge tits jiggle magnificently as she rocks, nearly slapping her in the face as she plunges down and then falling back into place with waves of rippling flesh.");
        else CView.text("breasts quiver invitingly as she pumps your cock, but before you can think of grabbing one yourself, both of her hands are squeezing and caressing them, drawing sighs from the woman as she toys with her erect nipples.");
        ...
        // (horse)
        if (player.body.legs.isTaur()) CView.text("barrel");
        else CView.text("chest");
        CView.text(" in her soft hair - but her hips keep pumping, incredibly.  ");
        // (pc corr >= 90 or > sheila corruption)
        if (player.stats.cor >= 90 || player.stats.cor > SheilaFlags.SHEILA_CORRUPTION) {
            CView.text("  The tight, rippling muscles of her pussy clamp down on you, nearly holding your cock in place as she tries to ride you still but only manages to jerk your dick up and down and squeeze pre-cum from it.  \"<i>S-so full,</i>\" Sheila groans, \"<i>did you just get bigger in me?</i>\"  Her lungs inflate as she sucks in her breath, and her hard nipples push into your " + skinFurScales(player) + ".");
        }
        else if (player.stats.cor < SheilaFlags.SHEILA_CORRUPTION) CView.text("  Her pussy clamps down around you, the rippling muscles so tight that she wouldn't even be able to pump if it weren't for the hot river of juice you feel around your tool.  \"<i>A-ah,</i>\" Sheila groans, \"<i>you thrust too!  Rail me silly!</i>\"  She tries to push against you with her hands, to return herself to an upright position, but can barely pull her chest apart from you - her nipples just narrowly recede from the surface of your " + skinFurScales(player) + " as her efforts peak.");
        // (else if magically equal but < 90)
        else CView.text("  Her pussy squirts and sucks alternately, trying to find purchase on your cock even as her pumping hips deny it; the sensation drives you wild.  \"<i>[name], your old fella is amazing...</i>\"");
        CView.text("  Your dick twitches and swells, as if to fulfill her words, and your pelvis rocks into Sheila's as you begin to ejaculate.  \"<i>No... pull out, or you'll get me pregnant,</i>\" she cries, when ");
        // [(single cox)
        if (player.body.cocks.length === 1) CView.text("your dick twitches inside her");
        // (multicox)
        else CView.text("she spies the first bead of pearly semen exiting");
        if (player.body.cocks.length === 2) CView.text(" your lonely, unused prick");
        if (player.body.cocks.length > 2) CView.text(" each of your lonely, unused pricks");
        CView.text(".");
        ...
        // [(multi)]
        if (player.body.cocks.length > 1) CView.text(", to be joined shortly by a messy external eruption that covers her thighs and belly in blotches of gooey white");
        CView.text(".");
        // [(big skeet)
        if (player.cumQ() >= 1000) CView.text("  It rolls out of you in waves, packing Sheila's cunt until she fills to capacity and the excess is forced out, sliming your own crotch.");
        ...
        if (player.stats.cor >= 90 && sheilaCorruption() >= 90) {
            return { next: sheilaGoesDemon };
        }
    ...
    // (if no cockFit 32 and yes vag)
        ...
        // [(horse)]
        if (player.body.legs.isTaur()) CView.text("You turn around and straddle a leg, squatting to bring your [vagina] atop her crotch.  \"<i>Push up,</i>\" you order, and Sheila obeys eagerly, twisting her body to connect her excited pussy with yours.");
        // (else if naga)
        else if (player.body.legs.isNaga()) CView.text("You slide it under her and lift gently; Sheila plays along with a disquieted expression, rolling over and raising her pussy into the air, and you press your own against her butt as your tail slides between her legs.  \"<i>See, you just need to be more... flexible,</i>\" you quip.");
        // (else if not horse and not naga)
        else CView.text("You straddle her legs with your own, pressing her pussy into yours; Sheila gasps as the hot flesh connects and you tentatively rock your hips, dragging your lower lips across.");
        ...
        // (sheila corruption > 30)
        if (sheilaCorruption() < 30) CView.text(" even though they're small and humble");
        // (else sheila corruption > 60)
        else if (sheilaCorruption() > 60) CView.text("; I worry that they're getting too mountainous");
        CView.text(".</i>\"  A hand drifts up to her nipple, pinching it as her mouth hangs open.  She rolls it in her palm, then presses the breast against her ribcage, and her shiver of arousal transmits to you through");
        if (player.body.legs.isNaga()) {
            CView.text(" her taut buttcheek");
            CView.text(".");
            ...
            CView.text(" her heated, slippery vagina.");
        ...
        // [(naga)
        if (player.body.legs.isNaga()) CView.text(" tail, drawing it up the hot opening and coiling it so you can return the warm, lube-slicked length for another pass even as you hump her butt.");
        else CView.text(" cunt, grinding her like a sex toy in search of your own climax.");
        CView.text("  \"<i>Do it!</i>\" you order, anxious to feel her pussy convulsing against you, anxious to feel your own squeezing her.  \"<i>Come with your lesbian lover!</i>\"");
        ...
        // [(pc corr >= 90 or > sheila corruption)
        if (player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) {
            CView.text("  Her vagina doesn't wait for romance, spasming and opening wide to try to suck you in, pulling at your ");
            if (player.body.legs.isNaga()) CView.text("scales");
            else CView.text("pussy");
            CView.text(" until you can take no more of the heat and come as well.");
            // [(naga)]
            if (player.body.legs.isNaga()) CView.text("  Her breasts swell in your hands as she sucks in her breath sharply, feeling your pussy drool its juice onto her ass.");
        }
        // (else if PC corr < sheila corruption)
        else if (player.stats.cor < sheilaCorruption()) {
            CView.text("  Her vagina doesn't wait for symbolism, dumping a wave of orgasm that splashes almost audibly against you.  Your pussy answers with your own climax, and begins wetting the woman underneath");
            if (player.body.legs.isNaga()) CView.text("; it feels like her breasts are pulling away from you as she tries to push her ass further into your quivering quim");
            CView.text(".");
        }
        else {
            CView.text("  Her vagina drools and spasms, not interested in gestures so much as release, and coats ");
            if (player.body.legs.isNaga()) CView.text("your tail");
            else CView.text("your own");
            CView.text(" with a fresh coat of her hot fluids which trigger the expected response from you.");
        }
        // (if cock)
        if (player.body.cocks.length > 0) {
            CView.text("  Your neglected ");
            if (player.body.cocks.length === 1) CView.text("prick quakes and spurts");
            else CView.text("pricks quake and spurt");
            CView.text(" weakly, drooling semen that pools on her skin.");
        }
        // [(not horse)
        if (!player.body.legs.isTaur()) CView.text("\n\nLeaning down as your cunt works through your climax and pulling Sheila's head up to yours, you kiss her softly on the lips.  She draws you in with an eagerness mirroring her pussy's, probing your mouth with her little pink tongue, and holds you lip-locked like that until her orgasm ends and she slumps in your arms.  You lower her to the ground as your last strokes fire, then fall beside her.");
        // (else if horse)
    ...
    // (else if no cockFit 32 and no vag)
        ...
        CView.text("Her hand lowers to her crotch and slides her middle finger into her pussy.  \"<i>I don't know what's wrong with me,</i>\" Sheila says, nervously.  \"<i>I'm not even sure why I get hot when I think of you... you don't have anything I can use.</i>\"");

        CView.text("\n\nYou smile at the admission");
        if (player.body.cocks.length > 0) CView.text(", stroking your oversized tool despite her dismissal");
        CView.text(".  \"<i>But you do?</i>\"");
        ...
        if (player.body.cocks.length > 0) {
            ...
            // (mans)
            if (!player.body.legs.isTaur()) CView.text("falls into your lap");
            // (horse)
            else CView.text("drifts between your legs");
            CView.text(", then crawls its way to your [cock].  She begins stroking it, torpidly at first, but with increasing excitement as a drop of pre-cum slides down and wets her palm.  \"<i>What's wrong, goblin girl?</i>\" you ask.  \"<i>Gotta get some sperm no matter what?</i>\"");

            CView.text("Sheila winces as your probing fingers send a shudder through her.  \"<i>Shut up, you tosser,</i>\" she retorts, yanking on your dick slightly harder in her irritation.");
        }
        // (new PG)
        ...
        // [(pc corr >= 90 or > sheila corruption)]
        if (player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) CView.text("  Her vagina heats up, squeezing your fingers so hard that you nearly can't pull out to rub her again, and you have a clear view of her breasts swelling up - arguable proof of your prowess, considering what she told you earlier.  Her people must be terrible at faking orgasm.");
        // (pc corr < sheila corruption)
        else if (player.stats.cor < sheilaCorruption()) CView.text("  Her vagina gushes with clear fluid, filling the bowl made by your hand with warmth, and you think you can see her breasts tightening up slightly as her back arches, though it's hard to be sure with how she's shaking.");
        else CView.text("  Her pussy quivers around you, and you draw the fingers out and pop them into Sheila's mouth.  The woman sucks on them greedily, and you raise an eyebrow at her.  She blushes again, but it's evidently not the first time she's done the same thing!");
        ...
        if (player.body.cocks.length > 0) {
            ...
            // [(big skeet)
            if (player.cumQ() > 1000) CView.text("  Her mouth fills quickly, and she writhes in your grip, managing to turn her head.  \"<i>Oh god, no way!</i>\" she coughs, spitting up enough semen to coat her chin as your ongoing orgasm spatters against her cheek.");
            // [(mega skeet)
            if (player.cumQ() >= 2000) CView.text("  Not satisfied with that alone, you turn her head the other way for your finish, ejaculating on the rest of her face and completing her gooey, white beard as she squirms.");
            // [(multicock)
            if (player.body.cocks.length > 1) CView.text("  The other strokes not coming from this cock do their part to decorate your lover as well, turning her hair into a slimy mess.");
            CView.text("  She escapes from your hand as you spend yourself, leaning away and gagging up seed onto the dry grass.");
        }
        CView.text("\n\nThe woman falls weakly backward");
        // (cock.length > 0)
        if (player.body.cocks.length > 0) CView.text(", her face still covered in globby white smears");
        CView.text(".  \"<i>Fuck...</i>\" she groans.  \"<i>You're a jerk, [name].");
        // [(cock.length > 0)
        if (player.body.cocks.length > 0) CView.text("  I can't believe you called me a goblin, you arsehole.");
        CView.text("</i>\"");

        CView.text("\n\nYou ");
        // [(cock.length > 0)
        if (player.body.cocks.length > 0) CView.text("can't believe how much she acts like one sometimes... but you ");
        CView.text("just laugh and tug her ear; she flicks it and pulls away from you, backhanding you softly in the gut.  \"<i>Knock it off!</i>\"");

        CView.text("\n\n\"<i>Are you really so worried about it?</i>\" you ask.  No answer comes");
        // [(cock.length > 0)
        if (player.body.cocks.length > 0) CView.text(", even after Sheila finishes wiping her face clean and stares at you");
        CView.text(", so you just content yourself with lying next to the woman for a while.");
        ...
        else player.stats.lust += player.stats.lib / 4;
...
// [XP3 - Guard Duty]
function sheilaXPThreeSexyTimeGuardDuty(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You fold your arms and frown at Sheila, but consent to help.  She brightens, then looks confused when you make no further motion.  \"<i>So...</i>\" she begins, \"<i>what should I do?</i>\"");
    ...
    // [(PC level >=8)
    if (player.stats.level >= 8) CView.text(" - which, you suppose, must include imps even though they're so far beneath you they might as well be underground");
    CView.text(".  You stand there for so long that you nearly slip into a daydream, and are only roused by the noise of a leather belt being carefully unfastened.  Focused again, you can hear Sheila's ragged breathing and her thighs rubbing together, then a hand sliding against skin and fabric... seems like she's made her decision.");
    ...
    // (else lib >= 40)
        ...
        // [(cock.length > 0)
        if (player.body.cocks.length > 0) CView.text("shift your pelvis forward");
        // (cock.length == 0)
        else CView.text("stick out your tongue playfully");
        CView.text(".  The girl giggles at that, relaxing and finally letting herself get into the masturbation.");
    ...
    SheilaFlags.SHEILA_XP = 4;
    if (player.stats.lib < 40) player.stats.lust += 5;

    else player.stats.lust += 15;

    return { next: returnToCampUseOneHour };
}

// XP4: Fancy meeting you here (sheila xp >= 4 and demon sheila = 0):
function fuckBuddySheilaMeeting(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Hey...</i>\"");
    ...
    CView.text("\n\nYou note that her breasts are " + sheilaCup(player) + " right now...");

    // [Chat][Sex][Just Hang][Maybe Later]

    return {
        choices: [
            ["Chat", fuckBuddySheilaMeetingChat],
            ["Sex", sheilaXP4Sex],
            ["Just Hang", fuckBuddySheilaMeetingJustHangOut],
            ["Maybe Later", fuckBuddySheilaMeetingMaybeLater],
        ]
    };
...
function fuckBuddySheilaMeetingMaybeLater(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You tell her you haven't the time right now, but squeeze her knee as you get up again.");
...
// [XP4 - Just Hang]
function fuckBuddySheilaMeetingJustHangOut(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("In answer, you reach down to pinch her butt and then ");
    // [(not horse or drider)
    if (!player.body.legs.isTaur() && !player.body.legs.isDrider()) CView.text("scoot your own forward");
    else CView.text("fold your legs under you");
    CView.text(" to lay in the bush's shade.");
...
// [XP4 - Chat]
function fuckBuddySheilaMeetingChat(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Feeling talkative?</i>\" Sheila asks.  \"<i>Have at yer.</i>\"");
    ...
    const choices: ScreenChoice[] = [];
    choices[0] = ["Fam'N Friends", fuckBuddySheilaMeetingChatFamilyAndFriends];
    choices[1] = ["Old Loves", fuckBuddySheilaMeetingChatOldLoves];
    choices[2] = ["Why Hide?", fuckBuddySheilaMeetingChatWhyHide];
    choices[3] = ["LiveWithMe", fuckBuddySheilaMeetingChatLiveWithMe];
    if (SheilaFlags.SHEILA_JOEYS > 0) choices[4] = ["Kids", fuckBuddySheilaMeetingChatKids];

    return { choices };
}

// [XP4 - Chat - Family&Friends]
function fuckBuddySheilaMeetingChatFamilyAndFriends(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Them?  They're doing fine.</i>\"  Sheila stares at you obtusely, trying to will you into changing the subject; you stare right back.  She sighs.  \"<i>No go, huh?</i>\"");
...
// [XP4 - Chat - Old Loves]
function fuckBuddySheilaMeetingChatOldLoves(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>[name], I don't want to talk about that,</i>\" Sheila says, \"<i>it leaves a bad taste in my mouth.</i>\"");
    ...
    // (else lib >=50)
        ...
        if (player.stats.cor >= 60) CView.text(" - though it's probably meant to be a defensive gesture, all it does is push her tits together under her white shirt and frame them in light-brown skin");
        CView.text(".  \"<i>[name], you horny whacker, I'm not gonna marry you.  Having to bull my way through a horde of monsters to see you and stand watch in shifts just to sleep together at night may be romantic once, but it's no kind of thing to do every day for the rest of my life.</i>\"");
        ...
        // [(corr < 30)
        if (player.stats.cor < 30) CView.text("; you lay a hand on her shoulder reassuringly");
        CView.text(".  Eventually she pulls away again, nose slightly red.  \"<i>Sorry, mate; didn't mean to have a tantrum on you.</i>\"");
...
function fuckBuddySheilaMeetingChatWhyHide(player: Character): NextScreenChoices {
    ...
    // (if T'A)
    if (player.effects.get(StatusEffectType.TelAdre).value1 >= 1) {
        CView.text("  After all, the city in the desert seems to be bustling and bristling with weapons to resist the demons, whatever her feelings may be about it.");
...
function fuckBuddySheilaMeetingChatLiveWithMe(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You ask Sheila to move in with you at your camp, and she turns bright red.");
...
function fuckBuddySheilaMeetingChatKids(player: Character): NextScreenChoices {
    CView.clear();
    // if joeycount = 1
    if (SheilaFlags.SHEILA_JOEYS === 1) {
        CView.text("\"<i>Ah?</i>\" Sheila says, turning toward you sheepishly.");
        ...
        if (player.body.femininity > 50) {
            CView.text("I don't know how he'll react when he sees a pretty face like yours, though; he doesn't know that I'm-");
            // [(PC is herm, fem)
            if (player.gender >= 2) {
            ...
            CView.text(" you guess, teasing her as her blush deepens.");
            CView.text("\n\n\"<i>[name], stop it.</i>\"");
        }
        // else if PC masculine or andro, no new PG)
        else {
            CView.text("I'm not sure if he intends to shake your hand or punch you, though.</i>\"");
    ...
    // if 1 < joeycount < 10
    else if (SheilaFlags.SHEILA_JOEYS < 10) {
        CView.text("\"<i>So how are the kids?</i>\"");
        ...
        if (player.stats.lib > 50) CView.text(" and tight, hot cunt");
        CView.text(" so well...  \"<i>How long does it take them to grow up?</i>\"");
        ...
        if (player.gender >= 3) CView.text("nother");
        CView.text(" lady's age, after all.</i>\"");
    ...
    // if joeycount >= 10
        ...
        CView.text("\"<i>Kids doing all right?</i>\"");
        ...
        if (player.stats.lib < 50) {
            ...
            // [(silly)]
            if (User.settings.silly()) {
                CView.text(" and clutching the life-sized [name] bodypillow I made");
                CView.text(".  But it's not going to happen right now.</i>\"");
            ...
            if (player.stats.cor < 40) player.stats.cor += -1;

        }
        // (else lib >=50)
        else {
            CView.text("\n\n\"<i>I'm still turned on by you,</i>\" you persist with a whisper.  \"<i>I want your body any time, anywhere.  Here, for example.  Or... here.</i>\"  This last comment is accompanied by your hand sliding into her shirt and over a " + sheilaCup(player) + " breast.  Sheila blushes.");
            ...
            player.stats.lustNoResist += player.stats.lib / 10 + 5;

            const choices = sheilaXP4SexMenu(player);
            choices[9] = ["RestForNow", fuckBuddySheilaMeetingChatKidsRest4Now];
            return { choices };
        }
    }
    return { next: returnToCampUseOneHour };
...
function fuckBuddySheilaMeetingChatKidsRest4Now(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Always,</i>\" you answer, \"<i>but if you need to relax and sleep for a bit, I'm here for that too.</i>\"  You draw your hand back out, patting her on the cheek.");

    CView.text("\n\nSheila kisses you on the forehead and then lays her head against your ");
    if (player.body.legs.isTaur()) CView.text("back");
    else CView.text("lap");
    CView.text(".  \"<i>You're sweet... thank you.  I will take you up on both of those offers - but maybe just the nap this arvo.  Would you wake me up in about an hour?</i>\"");
...
// [XP4 - Sex]
function sheilaXP4Sex(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You slip an arm around the bushranger and lean in.  \"<i>I was thinking we could have some sex... feel up to it?</i>\"");
...
function sheilaXP4SexMenu(player: Character): ScreenChoice[] {
    ...
    const choices: ScreenChoice[] = [];
    if (player.gender > 0) {
        if (player.body.cocks.length > 0) choices[0] = ["69WithWang", partial(consensualSheila69, player, true)];
        if (player.body.vaginas.length > 0) choices[1] = ["69WithCooch", partial(consensualSheila69, player, false)];
    }
    // [Consensual - Vaginal Sex] (req. cock <= 32 area):
    if (player.body.cocks.length > 0 && player.body.cocks.find(Cock.CockThatFits(32))) choices[2] = ["Fuck Her", consentacleVagSexForKangarooSlutBitches];
    // da scene
    choices[3] = ["Masturbation", sheilaMutualMasturbation];
...
function consensualSheila69(player: Character, cock: boolean = true): NextScreenChoices {
    ...
    if (cock) {
        if (player.body.cocks.find(Cock.CockThatFits(32))) {
            CView.text("  Though, she smiles openly when you produce your [cockFit 32].");
            ...
            // (all big cox > 32)
            CView.text("\n\nIncreasingly wide eyes offset her thin, pressed lips as you reveal " + describeOneOfYourCocks(player) + ", the voluminous flesh swaying softly to the breeze.");
            ...
            CView.text(" as she rubs her small, smooth labia vigorously.  \"<i>What do you expect me to do with one of those?</i>\"");
        }
    }
    // (vag)
    else {
        CView.text("  When you finally drop your bottoms, revealing your wet " + describeVagina(player, player.body.vaginas.get(0)) + ", she smiles softly.");
        ...
        if (player.body.clit.length > 8) CView.text(" and bring your " + describeClit(player) + " to its erect state.  \"<i>... And quite a stem,</i>\" your lover concludes.  \"<i>You sure that's not a dick?</i>\"");
        else CView.text(", producing a slight sigh and a wet finger.  \"<i>And I'm the eager one, right?</i>\" your lover asks, wryly - though her finger is just as wet.");
    }
    ...
    if (cock && player.body.cocks.find(Cock.CockThatFits(32))) CView.text("  \"<i>[name], I'm telling you it won't fit!</i>\"");
    else CView.text("  \"<i>Not talking, then?  Please yourself.</i>\"");
    CView.text("  Her commentary cuts off as you ");
    if (!player.body.legs.isTaur()) CView.text("turn around and hang your ");
    else CView.text("straddle her and squat down to dangle your ");
    if (cock) CView.text("pre-seeping shaft");
    else CView.text("wet pussy");
    CView.text(" over her head, waggling it provocatively as you slip your fingers into her.");

    CView.text("\n\n\"<i>Ah... ah... you could have just told me,</i>\" Sheila gasps, ");
    // [(cox)
    if (cock) {
        CView.text("wrapping a hand around your shaft");
        // (big)
        if (player.body.cocks.find(Cock.CockThatFits(32))) CView.text(" and guiding it to bend comfortably against her chest, drooling a trail of pre-cum as she slides it into position");
        CView.text(".");
    }
    // (vag)
    else CView.text("returning the favor and plunging into your " + describeVagina(player, player.body.vaginas.get(0)) + " with her hand.");
    CView.text("  She pumps you once, eliciting a trickle of fluid, and then speaks once more.  \"<i>Bring it lower...</i>\"");

    CView.text("\n\nYou oblige, ");
    if (!player.body.legs.isTaur()) CView.text("flattening your back");
    else CView.text("squatting lower");
    CView.text(" so your ");
    if (cock) {
        CView.text("cock sinks down");
        // [(small cock)
        if (player.body.cocks.find(Cock.CockThatFits(32))) CView.text(" into her warm, waiting mouth");
        else CView.text("; she lets go of it to grab her breasts instead, trying to push them around the sensitive shaft as she runs her tongue up the underside");
        CView.text(".");
    }
    else {
        CView.text("pussy dangles just over her face; she ");
        if (player.body.clit.length > 8) CView.text("nips and sucks at the tip of your clit, rolling it with her tongue");
        else CView.text("parts your labia with her tongue, stroking them gently and tickling your little clitoris");
        CView.text(".");
    }
    CView.text("  Not to be outdone in creativity, technique, or audacity, you ");
    if (!player.body.legs.isTaur()) CView.text("lean down and bury your own tongue all the way inside Sheila, forcing her labia open with your mouth");
    else CView.text("reach as far as you can to bring both hands into play, stroking and pulling Sheila's labia apart with one as you tease her exposed clitoris or slip inside her vagina with the other");
    CView.text("; your lover stops licking and you can feel her mouth slip away from your sensitive skin as her head rolls back.");
    ...
    // [(ladyparts)
    if (!cock) {
        CView.text("slipping her tongue back into you");
        // [(bigclit)
        if (player.body.clit.length > 8) CView.text(" and stroking your " + describeClit(player) + " rapidly with one hand");
        CView.text(".");
    }
    // (smallcox)
    else if (player.body.cocks.find(Cock.CockThatFits(32))) CView.text("taking you into her mouth once again and licking away a neglected drop of pre-cum.");
    else CView.text("stroking your cock with one hand as she pushes an erect nipple against its side to trace a line.");
    ...
    if (!player.body.legs.isTaur()) {
        ...
        if (cock && player.body.cocks.find(Cock.CockThatFits(32))) CView.text("shaking, teasing nipples and heaving chest");
        else CView.text("mouth, humming in delight");
        CView.text(".");
    }
    // (horse)
    else {
        ...
        if (cock && player.body.cocks.find(Cock.CockThatFits(32))) CView.text("hard nipples");
        else CView.text("throat");
        CView.text(".");
    }
    // merge horse with rider
    // [(big cox)
    if (cock && player.body.cocks.find(Cock.CockThatFits(32))) CView.text("\n\n\"<i>[name]!  I'm coming!</i>\" Sheila cries.");
    else CView.text("\n\n\"<i>Mmmh!  Mm mmnnn!</i>\" Sheila moans, muffled by your body.  The import doesn't hit you until a second later.");
    CView.text("  Her vagina convulses, ");
    // (pc corr >= 90 or > sheila corruption
    if (player.stats.cor >= 90 || player.stats.cor > SheilaFlags.SHEILA_CORRUPTION) {
        CView.text("trying to pull your ");
        if (!player.body.legs.isTaur()) CView.text("tongue");
        else CView.text("fingers");
        CView.text(" in ravenously.  You can literally feel her internal muscles rippling and undulating, stroking you as if they could coax out an ejaculation.  Sheila sucks in her breath as she climaxes, filling her chest; you can feel her tits pushing harder and harder against you.");
    }
    // (PC corr < sheila corruption)
    else if (player.stats.cor < SheilaFlags.SHEILA_CORRUPTION) {
        CView.text("pushing out a gush of fluid.  For what seems like several minutes, her pussy deluges your ");
        if (!player.body.legs.isTaur()) CView.text("mouth, filling you with her taste.");
        else CView.text("hands, completely soaking them in her orgasm.");
        CView.text("   Her thighs tense, forcing her pelvis into the air and her cunt toward you as her chest slumps; slowly, you feel the perky nipples pressing into your stomach pull away.");
    }
    // (else if equal but below 90)
    else if (player.stats.cor === SheilaFlags.SHEILA_CORRUPTION && player.stats.cor < 90) {
        CView.text("drooling a trickle of fluid and licking at you as she screams her ecstasy into your crotch.");
    }
    CView.text("  Sheila jams her face upward, ");
    // [(big cox)
    if (cock && player.body.cocks.find(Cock.CockThatFits(32))) CView.text("pressing her lips and tongue to the base of your shaft as she masturbates it vigorously.");
    /// (small cox)
    else if (cock) CView.text("sinking your [cockHeadFit 32] into the warm recesses of her throat where her muscles caress and coax it.");
    else CView.text("penetrating you deeply with her tongue as her hands reach up to stroke your clit furiously.");
    CView.text("  Your body gives up under her frenzied assault, laying your crotch onto the woman's face as muscle control abandons you.");

    // [(if cock-focus)]
    if (cock) {
        CView.text("\n\nStroke after stroke of semen passes down " + describeOneOfYourCocks(player) + ", ");
        // [(cock fits)
        if (player.body.cocks.find(Cock.CockThatFits(32))) {
            CView.text("pouring out into your lover's waiting throat.  She gags and you can feel the gooey seed oozing back against your [cockHeadFit 32]");
            // [(small cum)
            if (player.cumQ() < 250) CView.text("; you withdraw from her, allowing her throat to force your ejaculate back up and over her lips and cheeks with a splutter.");
            // (big cum)
            else if (player.cumQ() < 2000) CView.text(", but your relentless orgasm forces it down again, dumping more jism into her throat.  You can feel her hands clawing at your [butt] as you relieve your buildup.");
            // [(mega cum)
            else CView.text(".  Soon, her visibly-rounding stomach fills and the tip of your dick is once again washed in backflow; the next spurts force the cum all the way up, and she begins drooling the slimy semen-saliva blend from her mouth and nose, hands all the more frantic in her alarm.");
        }
        // (else doesn't fit)
        else {
            CView.text(" squirting onto and between your intertwined bodies to soak your [chest] and neck, and Sheila's softly pumping crotch.");
            // (big skeet)
            if (player.cumQ() > 1000) CView.text("  A wellspring of seed, your body continues producing until the fuzz on her thighs and tail is completely matted down with the whiteness drooling between you.");
            // (multicocks)
            if (player.body.cocks.length > 1) {
                CView.text("  Your other cock");
                if (player.body.cocks.length === 2) CView.text(" spasms");
                else CView.text("s spasm");
                CView.text(" as well, adding their own issue to the unholy mess covering the poor woman.");
            }
        }
    }
    // (else if vag-focus)
        ...
        if (player.body.vaginas.get(0).wetness <= 2) CView.text("trickle out, and are quickly sucked up by her greedy mouth");
        else if (player.body.vaginas.get(0).wetness <= 4) CView.text("gush freely, leaving her face slick");
        else CView.text("overflow, soaking her hair even as she shuts her eyes tight against the cataract");
        CView.text(".  As you climax, you wrap around her head, squeezing so hard that you flatten her ears against her skull.  Her hands latch onto your butt, trying to pull you off, but having already spent herself on her throes of passion, she's no match for yours, still ongoing, and you hold tight until your body relaxes.");
    }

    CView.text("\n\nSpent, you roll off of Sheila");
    // [(smallcox)
    if (cock && player.body.cocks.find(Cock.CockThatFits(32))) CView.text(", pulling free of her throat.  She immediately turns over as well and forces herself to her hands and knees, then gags and coughs up a large amount of your jism.  When she's dumped enough to clear her passageway, she collapses forward, smearing the mess onto her chest as she spends a few minutes to catch her breath.  Once she's got it, she turns back over and sits up, strands of your slime linking her breasts to the grass.");
    // (else not smallcox)
    else CView.text(".  She sits up, wiping sweat and stray fluids away from her eyes.");
    ...
    // [(corruption threshold warning if sheila corruption >= 80 and PC corr > sheila corruption and sheila xp =/= 3)
    else if ((player.stats.cor > 90 || player.stats.cor > sheilaCorruption()) && sheilaCorruption() >= 80 && SheilaFlags.SHEILA_XP !== 3) {
        sheilaCorruptionUpdate(player);
        ...
        return {
            choices: [
                ["Listen2Her", sheilaCorruptionWarningListenToHer],
                ["Fondle Her", sheilaCorruptionWarningsChooseFondle],
            ]
        };
    ...
    if (cock && player.body.balls.count > 0) {
        CView.text("What you see manages to pull an explosive laugh from you, tired as you are.  Sheila gives you the stink-eye until you've recovered enough to explain the source of your mirth: on her forehead, making a perfect ");
        if (player.body.balls.count === 2) CView.text("infinity");
        else CView.text("double infinity");
        CView.text(" shape against the rest of her skin, is an imprint of your " + describeBalls(true, true, player) + ".  The kangaroo girl rolls her eyes at the juvenile display and resumes her harangue.  ");
    }
    CView.text("\"<i>You could give a girl a bloody warning before that sort of behavior.  I thought you were going to kill me!</i>\"");
    ...
    // [(sheila xp = 3)
    if (SheilaFlags.SHEILA_XP === 3) {
        CView.text("\"<i>[name]... thanks.  Even being covered in your mess is better than being stuck outside all night, too scared to touch myself and too horny to run.</i>\"  Sheila kisses you on the cheek.  \"<i>My sloppy hero, eh?  I'm okay now; I can make it 'til morning when the gate reopens.</i>\"");
        ...
        return {
            choices: [
                ["Let Her Be", sheilaXPThreeSexyTimePostSexLetHerBe],
                ["Stay With Her", shielaXPThreeSexyTimePostSexStay],
            ]
        };
    }
    // (else if sheila xp =/= 3)
    else {
        CView.text("\"<i>Forget it.  With the way you worked me, I forgive you.  Just be careful, you wanker.</i>\"  Sheila closes her eyes and pulls your arm around her shoulder");
        if (cock && player.body.cocks.find(Cock.CockThatFits(32))) CView.text(", smearing your dangling hand into the mess on her tits");
        CView.text(".  The two of you lie like that for a while, until she gets up.  \"<i>I should really go... need to cure my catch.  Not to mention I'll have to clean up before I can even put my damn clothes back on.</i>\"  Sheila picks up her stuff.  \"<i>Hehe... see you soon.</i>\"");
    }
    return { next: returnToCampUseOneHour };
}

// [Consensual - Vaginal Sex] (req. cock <= 32 area):
function consentacleVagSexForKangarooSlutBitches(player: Character): NextScreenChoices {
    CView.clear();
    ...
    CView.text("Sheila doffs her hat and lowers her light eyes shyly as you lean in, then looks up and puts her arms around your neck as you slip your own about her waist.  She blushes a bit, then leans in as well; you quickly capture her lips with a small kiss, then another.  The tiniest murmur escapes as you break away from this second kiss, and then she's diving in again, locking her mouth together with yours and exploring it with her tongue.  You return her passionate kiss with one of your own as your hand drifts up from her waist, under her shirt. It reaches her semi-erect nipple quickly and you begin pinching and rubbing it between your fingers; her mouth quivers on yours as you tease, then eventually breaks away as she tilts her head back and lets out a low moan.  Taking the opportunity to escape from her kisses, you move your mouth down her long neck as your fingers trace the outline of her teat.");

    CView.text("\n\n\"<i>Ahh, god...</i>\" she sighs, as you continue tickling her neck with your lips.  She breaks her embrace and hurriedly pulls her vest and shirt over her head, revealing her " + sheilaCup(player) + " breasts; both her nipples are fully erect, sticking up like little 'touch me' signs.  You oblige them and caress her ");
    if (sheilaCorruption() <= 30) CView.text("pert");
    else CView.text("jiggly");
    CView.text(" breasts as she fiddles with your own clothing.  She manages to remove the top of your [armor], exposing your [chest]; she then returns your favor by ");
    if (!player.body.chest.find(BreastRow.FuckableNipples)) CView.text("squeezing and fondling");
    else CView.text("plunging two fingers into");
    CView.text(" your own " + describeNipple(player, player.body.chest.get(0)) + "s.  You shiver as she paws at the sensitive nubs on your chest, but don't let up your assault, moving your mouth down to tickle one of hers with your teeth and tongue.  Soon she's quaking and twitching beneath your ministrations; rubbing at her crotch with one hand as she half-heartedly tries to tease you back with the other.");

    CView.text("\n\n\"<i>Please... I can't take any more,</i>\" she whines.  \"<i>Stop pashing on my nips and put it in, mate.</i>\"  The hand at her crotch begins trying to unfasten her belt while the other grabs and fondles her breast; you smirk and complete your own undressing quickly with both hands.  As " + describeOneOfYourCocks(player) + " emerges, engorged with blood, from your clothing, she switches from undressing back to rubbing, breathing heavily and stroking herself through the fabric as she stares at your ");
    // [(if wangaroo)
    if (player.body.cocks.find(Cock.CockThatFits(32)).type === CockType.KANGAROO) CView.text("tapered, fleshy penis.  \"<i>Wow... you never see those.</i>\"");
    ...
    CView.text("\n\nYou grin.  ");
    // [(no centaur)
    if (!player.body.legs.isTaur()) CView.text("Taking hold of your " + describeCock(player, cockThatFits));
    // (centard)
    else CView.text("Attempting to maneuver your " + describeCock(player, cockThatFits) + " into position");
    CView.text(", you rub the head along her dripping snatch, smearing her fluid over it.  ");
    // [(non-cent)
    if (!player.body.legs.isTaur()) CView.text("Gripping her fuzzy thighs next, you lift her bottom and line");
    // (hoers)
    else CView.text("She lifts her bottom and lines");
    CView.text(" her slippery passage up with your cock.  \"<i>Here it comes!</i>\" you cry.  With that announcement, you thrust into her, parting her lips with your swollen " + describeCockHead(cockThatFits) + ".  She moans in relief as you finally enter her, but said relief is short-lived as you begin pumping at her slick vagina.  Gasping weakly, she lies back on the ground; ");
    if (sheilaCorruption() <= 30) CView.text("her perky breasts rock back and forth");
    else CView.text("her jiggling breasts flatten into perfect domes and undulate on her chest");
    CView.text(" as you draw in and out of her warm, moist pussy.");
    ...
    // [(non-cent)]
    if (!player.body.legs.isTaur()) {
        CView.text("You pause briefly, adjusting your angle and leaning down to kiss her deeply once again and help caress her ");
        if (sheilaCorruption() <= 30) CView.text("firm");
        else CView.text("pillowy");
        CView.text(" tits, then resume thrusting from your new position; her nipples now brush against your [chest] as you pump.  ");
    }
    // (if balls/vag)
    if (player.body.balls.count > 0 || player.body.vaginas.length > 0) {
        CView.text("Her stubby tail curls inward toward you, rubbing up against your ");
        if (player.body.balls.count > 0) CView.text("[balls]");
        else CView.text(describeVagina(player, player.body.vaginas.get(0)));
        CView.text(" and tickling you as she writhes in pleasure.  ");
        // (if vag)
        if (player.body.balls.count === 0 && player.body.vaginas.length > 0) CView.text("Your cunt twitches and drools a bit of fluid on the furry intruder.  ");
    }
    CView.text("You fuck harder, stuffing Sheila with such force that her ");
    if (sheilaCorruption() < 30) CView.text("perky, erect nipples trace long arcs against your skin");
    else CView.text("jiggly tits nearly slap into her face");
    CView.text(".  \"<i>Ohhh,</i>\" she moans, \"<i>YES!</i>\"  At that moment you feel her vagina clamp around your [cockFit 32], wringing it frantically as she reaches her climax.  You continue pumping as best you're able, but you can't stand up to the siege of her spasming pussy for long; ");

    // [(pc corr >= 90 or > sheila corruption)
    if (player.stats.cor >= 90 || player.stats.cor > SheilaFlags.SHEILA_CORRUPTION) {
        CView.text("something builds to a painful intensity in your crotch.  It feels like two mind-blowing orgasms arriving together and fighting over which one gets to come out first.  You rise toward climax quickly and then are drawn back in moments of maddening frustration, over and over.  In response you intensify your thrusts even more, sawing in and out of the girl at a pace that would be blistering if the two of you weren't leaking enough pre-cum and pussy juice to put out a brushfire.  Eventually the pleasure overwhelms the strange reticence of your orgasm; you cry out loudly as you peak, and squeeze a tiny trickle of cum into her grasping vagina.");
        // [(multi)
        if (player.body.cocks.length > 1) {
            CView.text("  Your other ");
            if (player.body.cocks.length === 2) CView.text("dick erupts");
            else CView.text("dicks erupt");
            CView.text(" as well, festooning her crotch with sticky strings.");
        }
        CView.text("  The intense sensation begins to ascend your dick slowly; your body works itself into overdrive and your partner's moans increase sympathetically as it nears the tip.  Finally, it exits your body with an enormous squeeze, alongside a jet of jism.  Sheila gives a complementary gasp as this pulse enters her, and her pussy convulses in a second orgasm, causing her back to arch.  Her breasts seem to jiggle more wildly than before as she thrashes on the ground in the grip of her pleasure.");
    }
    // (PC corruption < sheila corruption)
    else if (player.stats.cor < SheilaFlags.SHEILA_CORRUPTION) {
        CView.text("your resistance is condemned to die in failure as Sheila lets out a quavering scream of pleasure and wraps her arms and legs around you, pinning you inside.  Her vagina convulses around your cock powerfully; you could swear you feel yourself stretching longer and longer as her rippling pussy pulls you deeper in.  Just when you think that your dick must be a mile long, her whole body tenses up and her fingernails dig into your back.  Wearing a madwoman's expression, Sheila stares open-mouthed at you as her muscles lock and her pussy quivers out its fitful orgasm; even her ");
        if (sheilaCorruption() < 30) CView.text("perky");
        else CView.text("pillowy");
        CView.text(" breasts seem to be held taut, compressed and motionless.  Suddenly she rams her hips into yours violently and throws her head back, and before you can issue a word of protest, her cunt wrings your " + describeCock(player, cockThatFits) + " one last time with almost supernatural heat; your mind clouds over and a wracking orgasm shoots to the base of your spine, echoing back with the first wave of your ejaculation.");
    }
    // (else if equal but below 90)
    else {
        CView.text("your orgasm quickly supersedes your control as she wrings you and rubs her nipples against your [chest].  " + describeOneOfYourCocks(player, true) + " begins to twitch as it unloads its first squirt of semen into her spasming pussy");
        if (player.body.cocks.length > 1) CView.text(" or onto her thighs and belly");
        CView.text(".");
    }
    ...
    if (!cockThatFits.hasKnot()) {
        CView.text("You oblige shakingly, pulling your " + nounCock(cockThatFits.type) + " out of her clenching pussy with a wet, sucking noise.  A trickle of semen drools from between her lips as you unburden yourself of the rest of your load, squirting it onto her groin");
        // [(big skeet)
        if (player.cumQ() > 500) CView.text(", breasts, and face");
        CView.text(".");
        // [(Huge skeet)
        if (player.cumQ() >= 1500) CView.text("  More and more globs of semen shoot from the end of " + describeOneOfYourCocks(player) + ", pasting Sheila to the dry grass as you paint her body white.");
    }
    // (dog cock)
    else {
        CView.text("You attempt to oblige her, but your swollen knot sticks firmly in her squeezing pussy, locking you into place.  After a few fruitless tugs, you give yourself up to your orgasm with an apology, slumping against her heaving chest and helplessly dumping the rest of your load into the twitching woman. ");
        // [(big skeet)
        if (player.cumQ() >= 1000) {
            CView.text("Her belly begins to round as your overproductive gonads push out your massive store of semen");
            // [(huge skeet)
            if (player.cumQ() >= 3000) CView.text(", until it's stretched as far as possible and each successive jet forces out squirts of frothy jism from the tight seal her pussy makes around the knot");
            CView.text(". ");
        }
        // [(multi)
        if (player.body.cocks.length > 2) CView.text("Your other cocks erupt as well, soaking her crotch and matting down the fur on her thighs with whiteness. ");
    }
    CView.text("Both of your orgasms finish up");
    // (dog-c)
    if (cockThatFits.hasKnot()) CView.text(" and you finally manage to pull out of her");
    CView.text("; the two of you lie there breathing raggedly for several minutes.  Sheila breaks the silence first.");
    ...
    // [(sheila xp = 3)
    if (SheilaFlags.SHEILA_XP === 3) {
        CView.text("  Sheila looks over, thankfulness written on every inch of her brow.  \"<i>[name]... I really appreciate what you did.  I'd rather be knocked up than caught by some nightmare because I was too horny to hide properly until the colony opened - er, not that I want to be knocked up by you, sweet as you are.</i>\"");
    ...
    // (else sheila xp =/= 3)
        ...
        // [(norm skeet)
        if (player.cumQ() < 500) CView.text("as she strokes the " + skinFurScales(player) + " of your chest");
        else CView.text("as she wipes off as much jizz as she can");
        CView.text(", then get up and collect your gear.  \"<i>Um... see you later, ok?</i>\" she says.");
        return { next: returnToCampUseOneHour };
    }
...
// da scene
function sheilaMutualMasturbation(player: Character): NextScreenChoices {
    CView.clear();
    let target: string = "";
    const choices = [];
    if (player.body.cocks.length > 0) {
        if ((player.body.legs.isTaur() && player.body.tallness * (5 / 6) < player.body.cocks.sort(Cock.Longest)[0].length) || player.body.cocks.length > 0) choices[choices.length] = "cock";
    }
    if (player.body.vaginas.length > 0 && !player.body.legs.isTaur()) choices[choices.length] = "vagina";
    if (player.body.chest.find(BreastRow.FuckableNipples)) choices[choices.length] = "nipplecunt";
    if (player.gender === 0 || choices.length === 0) choices[choices.length] = "ass";

    if (choices.length === 0) {
        // CoC_Settings.error("");
        CView.text("ERROR - SHEILA MASTURBATE BROKE, SON");
        return { next: returnToCampUseOneHour };
    }
    target = choices[randInt(choices.length)];

    CView.text("You snatch Sheila's hat and pack, tossing them away, then slide a hand into the girl's shirt, slipping one of her nipples between two fingers as you nibble on her neck, and use the gentle pressure to push her onto her back.  She sighs, moving a hand under her clothes to caress yours.  \"<i>Ah,</i>\" she pants, letting out a soft puff of air, \"<i>what're we gonna do on the grass, [name]");
    if (User.settings.silly()) CView.text("-chan");
    CView.text("?</i>\"");
    ...
    // [(horse)
    if (player.body.legs.isTaur()) CView.text(", folding your legs under you,");
    CView.text(" and begin ");
    // [(adequate-length cock)
    if (target === "cock") {
        CView.text("stroking your [cock], pointing it at her as you smear pre-cum onto your fingers");
    }
    // (non-horse vag)
    else if (target === "cunt") CView.text(" sliding your hand to your [vagina], parting your lips with two fingers");
    // (gross-ass nipcunts)
    else if (target === "nipplecunt") CView.text("plunging your fingers into one of your awkward, distended nipples");
    // (Unsexed non-centaur only)
    else if (target === "ass" && !player.body.legs.isTaur()) CView.text("licking a finger and teasing your asshole with shallow strokes, gently circling the ring");
    // (centaur who hasn't got shit)
    else CView.text("sucking a single digit salaciously, fingerfucking your mouth and coating it with saliva");
    CView.text(".  \"<i>Oh... are you going to touch me, or just play with that?</i>\" Sheila asks, grinning.");
    ...
    if (target === "cock") CView.text("You return your hand to your [cock], pumping vigorously to catch up to your lover, spurred onward by the smooth skin she presses into your lips.");
    // (else if nonhorse vag)
    else if (target === "cunt") CView.text("You slide your hand between your [legs] again, intent on recovering the march Sheila's stolen on you, and begin rubbing your [clit] vigorously while sighing into her breast.");
    // (else if RNG disgusting nipplecunts)
    else if (target === "nipplecunt") CView.text("Your hand returns to your chest, slipping inside the sensitive flesh of your nipple, and you groan into Sheila's as a trickle of moisture falls from your chest.");
    // (else if non-centaur asshole)
    else if (!player.body.legs.isTaur()) CView.text("Your hand reaches back to your [asshole], neglected for too long, and a finger spears your pucker, stroking the nerves inside as you hurry to bring your arousal in line with Sheila's.");
    /// (else if fucking nothing)
    else {
        CView.text("You wish you had something to play with as you watch your lover writhe under you");
        // [(silly), even just a paperclip].]
        if (User.settings.silly()) CView.text(", even just a paperclip");
        CView.text(".");
    }
    ...
    // [(if fucking nothing)
    if (target !== "cock" && target !== "cunt" && target !== "nipplecunt") CView.text(", and yours joins her, slipping into her entrance alongside");
    CView.text(".  \"<i>Just... say I'm always yours!</i>\"  She turns you toward her face again, locking you in a kiss, and your ");
    // [(if cock)
    if (target === "cock") {
        if (player.body.cocks.length === 1) CView.text("prick begins");
        else CView.text("pricks begin");
        CView.text(" to twitch, ejaculating a stroke onto Sheila's midriff and wrist.  The girl groans in your mouth at the new heat, and her body shakes in orgasm.");
    }
    // (vag)
    else if (target === "cunt") {
        CView.text("pussy convulses, ");
        if (player.body.vaginas.get(0).wetness < 4) CView.text("drooling");
        else CView.text("squirting");
        CView.text(" around your hand.  As your body shakes and your lips rattle against Sheila's, she shivers and her own orgasm begins.");
    }
    // (nipcunts)
    else if (target === "nipplecunt") {
        CView.text("nipple constricts around your fingers in climax, oozing lubrication onto your partner.  As the drops from your " + describeBreastRow(player.body.chest.get(0)) + " hit her own, Sheila's back arches again and her own orgasm begins.");
    }
    // (jack shit)
    else CView.text("fingers are pulled into her vagina by her muscles as her orgasm begins.");
    // [( pc corr >= 90 or > sheila corruption)
    if (player.stats.cor >= 90 || player.stats.cor > SheilaFlags.SHEILA_CORRUPTION) {
    ...
    // (else if equal corr but under 90)
        ...
        CView.text("She breaks the kiss, drawing her hand from her pussy, and holds your head in front of her; her eyes soften with affection as she shudders.");
        // [(not jack shit)
        if (target === "cunt" || target === "cock" || target === "nipplecunt") CView.text("  Emptying your reserves, you allow yourself to fall into her arms, resting your head on her chest.");
    }
    ...
    else player.stats.lust += 50;

    if (player.stats.cor >= 90 && sheilaCorruption() >= 90) {

        choices[0] = ["Next", sheilaGoesDemon];
        return;
    }
    // [(corruption threshold warning if sheila corruption >= 80 and PC corr > sheila corruption and sheila xp =/= 3)
    else if ((player.stats.cor > 90 || player.stats.cor > sheilaCorruption()) && sheilaCorruption() >= 80 && SheilaFlags.SHEILA_XP !== 3) {
    ...
    // (else no corruption warning triggered)
        ...
        sheilaCorruptionUpdate(player);
        CView.text("\n\nSheila strokes your " + describeHair(player) + " as you lie ");
        if (target === "cock") CView.text("there, staring at the load you left in her bellybutton");
        else CView.text("together");
        CView.text(".  \"<i>Oh, I had so much fun, mate.");
        // [(if sex =U and sillymode)
        if (player.gender === 0 && User.settings.silly()) CView.text("  I love you, my Ken doll.");
        CView.text("</i>\"");
        // [(if sheila xp = 3)
        if (SheilaFlags.SHEILA_XP === 3) {
            ...
            choices[0] = ["Let Her Be", sheilaXPThreeSexyTimePostSexLetHerBe];
            choices[1] = ["Stay With Her", shielaXPThreeSexyTimePostSexStay];
            return;
        }
        else {
...
function sheilaCorruptionWarningsChooseFondle(player: Character): NextScreenChoices {
    ...
    if (player.body.cocks.length > 0) CView.text("erect");
    else if (player.body.vaginas.length > 0) CView.text("wet");
    else CView.text("aching");
    CView.text(".  Even as she goes, you can still see the edges of her amazing, swaying rack around the sides of her torso.");

    CView.text("\n\nBitch tease.");
    // set sheila xp to -3 and sheila corruption to 90, PC corr - 10
    SheilaFlags.SHEILA_XP = -3;
    SheilaFlags.SHEILA_CORRUPTION = 90;
    player.stats.cor += -10;
...
function sheilaCorruptionWarningListenToHer(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>So...</i>\" you answer, your feel frozen in mid-cop, \"<i>what?</i>\"");
...
function sheilaGotWhomped(player: Character, monster: Character): NextScreenChoices {
    CView.clear();
    CView.text("Sheila collapses, ");
    // [(HP)
    if (monster.stats.HP < 1) CView.text("completely spread-eagled from her injuries");
    /// (lust)
    else CView.text("rubbing her thighs together and smearing around the moisture from her pussy");
    CView.text(".  As you stand over her, she forces herself to her elbows and tries to scoot away from you, fear written on her face.  \"<i>I'm sorry... please, no...");
    // [(sheila xp > -4)
    if (SheilaFlags.SHEILA_XP > -4) CView.text("  I'll forgive you for everything else, but please, not this!");
    CView.text("</i>\" she implores.");
    // [(sheila corruption >= 90)
    if (sheilaCorruption() >= 90) CView.text("  \"<i>I-if you touch me again... god, I can't even think straight anymore!  Don't do this!</i>\"");
    // (lust >= 30)
    if (player.stats.lust >= 33) CView.text("\n\nWill you take advantage of the terrified girl or let her go?");
    ...
    if (player.stats.lust >= 33) {
        if (player.body.cocks.length > 0 && player.body.cocks.find(Cock.CockThatFits(48))) choices[0] = ["Fuck Pussy", partial(rapeSheilasCooter, player, monster)];
        if (player.body.cocks.length > 0) {
            if ((!player.body.legs.isTaur() && player.body.cocks.sort(Cock.Shortest)[0].length <= player.body.tallness * .35) || (player.body.legs.isTaur() && player.body.tallness * 1.2 <= player.body.cocks.sort(Cock.Shortest)[0].length && player.body.cocks.sort(Cock.Shortest)[0].length <= player.body.tallness * 1.55)) {
                choices[1] = ["Forced BJ", partial(forcedSheilaOral, player, true)];
            }
        }
        if (player.body.vaginas.length > 0) choices[2] = ["Forced Lick", partial(forcedSheilaOral, player, false)];
    }
    choices[9] = ["Leave", letSheilaGo];

    return { choices };
}
// if choosing Let Her Go and sheila xp > -3, set sheila xp = -1, then give xp/gems and Next button to camp
function letSheilaGo(player: Character): NextScreenChoices {
    if (SheilaFlags.SHEILA_XP >= -3) SheilaFlags.SHEILA_XP = -1;
    return { next: returnToCampUseOneHour };
}

// [Victory Rapin' - Rape Pussy] - uses cockarea <= 48:
function rapeSheilasCooter(player: Character, monster: Character): NextScreenChoices {
    CView.clear();
    // set sheila xp = -4
    SheilaFlags.SHEILA_XP = -4;
    const cockThatFits = player.body.cocks.find(Cock.CockThatFits(48));

    CView.text("You grab the girl by her fuzzy leg; she tries to kick away from you, but you pull her back, pinning her");
    // [(horse)
    if (player.body.legs.isTaur()) CView.text(" by planting your forehooves just above her hips");
    CView.text(".  Reaching down, you take the neck of her shirt in your hand, then rip it open, exposing her ");
    // [(sheila corruption < 30)
    if (sheilaCorruption() < 30) CView.text("tiny ");
    // sheila corruption > 60)
    else if (sheilaCorruption() > 60) CView.text("huge, soft ");
    CView.text("breasts, which jiggle from the force.  She tries to cover herself with her arms, but you grab and pull them aside, revealing her chest again.  As you appraise her naked flesh, she looks away, shame coloring her cheek.  Her mouth tightens and her eyes shut as you squeeze her tits roughly, flicking the nipples until they're erect.");
    ...
    // [(monster HP < 1)
    if (monster.stats.HP < 1) CView.text("sliding it between her labia and producing a damp spot, then ");
    CView.text("dragging the wet fabric around her vulva.");

    CView.text("\n\nSatisfying yourself as to her readiness, you unfasten your [armor] to extract your [cockFit 48]; the " + describeCockHead(cockThatFits) + " drips with pre-cum as you pull it loose, and you lay it on her crotch, letting the goo soak the white fabric until you can see her light-brown skin through it.  She squirms under it, bringing up another droplet with her motion, and you place the tip at the entrance to her pussy, pressing eagerly against the fabric.  \"<i>Please...</i>\" she whimpers, \"<i>I'm begging you; don't.</i>\"");

    // fuck-fork
    // [(corruption < 50)
    if (player.stats.cor < 50) {
        CView.text("\n\nYou frown at the girl.  She still wants to escape, even though her body is eager for this?  You can feel her wet pussy twitching at your " + describeCockHead(cockThatFits) + ".  Dragging the oozing tip along her soaked panties provokes a shiver and another bloom of fluid from the girl, but she bites her lip and refuses to meet your eye.");
        ...
        // [(big skeet)
        if (player.cumQ() >= 500) CView.text("  Soon you've filled her as fully as the shallow penetration will allow, and the panties begin to press back against your cock, oozing onto you whatever semen the porous dam can't contain.");
        // (mega skeet)
        else if (player.cumQ() >= 1500) CView.text("  It sags and bows around the tip of your spear, pulsing with each new stroke as the hemlines leak jizz onto her legs and butt.");
        CView.text("  Idly, you imagine what it must look like inside, each squirt having its momentum arrested by the cloth, drooling lazily into her until the next one displaces it a bit further forward, and wonder if it'll be enough to get her pregnant.  You pull away from the girl, leaving her gasping and once again averting her gaze.");

        CView.text("\n\nShe attempts to stand, holding her midriff and looking at you unhappily.  ");
        // [(big skeet)
        if (player.cumQ() >= 500) CView.text("So much cum fills her pussy that it slips out, once again restoring the opacity, though not the dryness, of her panties.  ");
        CView.text("You return her gaze, turning your nose up dismissively.  Conscious of her nudity, she pulls up her shorts and buttons her vest to conceal herself as you leave her behind.");
        ...
        if (cockThatFits.area > 32) CView.text(" and the pain of taking a cock slightly too large for her");
        CView.text(", and she actually rears back to scratch at you with her fingernails.  You catch her hand and hold it, and she responds by spitting at you!  \"<i>Fuck you, arsehole!</i>\" Sheila shrieks.  \"<i>Take it out!</i>\"  Reaching down, you slap her in the face.");
        ...
        if (sheilaCorruption() > 10) CView.text(" jiggles her breasts and");
        CView.text(" clenches her jaw.  A bloom of red colors her cheeks, one brighter than the other from the blow... though she still refuses to look at you.  Doesn't matter; having sex.  You begin to pump her pussy vigorously, the wetness enough to allow quick, smooth thrusts without any warmup.  \"<i>You're a godawful liar,</i>\" you grunt");
        // [(not centaur)
        if (!player.body.legs.isTaur()) CView.text(", leaning down to kiss her long neck and collarbone");
        CView.text(".  Sheila quivers again, still looking away, as you continue.  \"<i>I can actually feel you stroking me with your vagina already.  Are you going to come soon?  Will you tell me how much you're hating this when you do, and wrap your legs around me to punish me by holding me inside?</i>\"  The girl's blush deepens, and she bites her lip in embarrassment.  Confidently, you release her hand");
        // [(nothors)
        if (!player.body.legs.isTaur()) CView.text(", transferring yours to her breast while brushing her torn shirt to the side");
        CView.text("; without struggling further, she slowly lowers it");
        // [(nothors)
        if (!player.body.legs.isTaur()) CView.text(" to rest atop your own");
        CView.text(", confirming your suspicion.  Her eyes close and her chin raises resignedly");
        // [(nothors)
        if (!player.body.legs.isTaur()) CView.text(" as you triumphantly squeeze the flesh in your hand and kiss her under the jaw");
        CView.text(".");
        ...
        // [(mans)
        if (!player.body.legs.isTaur()) CView.text("place a hand along her face, tuning it toward you, and kiss her on the lips.");
        else CView.text("bend down, lowering your torso until you can caress her cheek.");
        CView.text("  \"<i>Of course I won't,</i>\" you answer sweetly.  \"<i>What would be the point?</i>\"  Sheila winces as you hilt yourself, sinking your [cockFit 48] so far into her that your crotch meets hers.  You shudder");
        // [(nonhors)
        if (!player.body.legs.isTaur()) CView.text(" and squeeze her breast");
        CView.text(" as your climax arrives and the first of your seed pours into her grasping pussy.  While you fill her, her back arches");
        // [(nonhors)
        if (!player.body.legs.isTaur()) CView.text(" and she presses your hand uncomfortably");
        CView.text(", gasping as her own orgasm begins.  Her knees wrap around your sides, satisfying your predicition, and her vagina");
        // [(pc corr >= 90 or > sheila corruption)
        if (player.stats.cor >= 90 || sheilaCorruption() < player.stats.cor) {
            CView.text(" pulls greedily at your [cockFit 48], drinking up your semen as fast as you can squirt it.  She sucks in breath as she climaxes, finally opening her eyes as her head tilts back");
            // [(nonhors)
            if (!player.body.legs.isTaur()) CView.text("; you can almost feel her breast heating up and swelling with blood under your hand, as the nipple pokes into your palm");
            CView.text(".");
        }
        // (PC corr < sheila corruption)
        else if (player.stats.cor < sheilaCorruption()) {
            CView.text(" floods you with juices, dumping so much around your twitching cock that you feel like you're lowering it into a hot spring.");
            // [(nonhors)
            if (!player.body.legs.isTaur()) CView.text("  She squeezes your hand so hard that you swear you can feel her breast compressing under it as she screams her pleasure.");
        }
        // (else equal but < 90)
        else {
            CView.text(" drools eagerly as you mingle your juices with hers, until the fluid begins to thicken with white seed.");
        }
        // [(big skeet)
        if (player.cumQ() >= 500) CView.text("  Overabundant as always, your semen quickly fills her to capacity and begins to squirt out around your shaft, staining her thighs.");
        // [(multicock)
        if (player.body.cocks.length > 1) {
            CView.text("  Your other ");
            if (player.body.cocks.length === 2) CView.text("dick spits");
            else CView.text("dicks spit");
            CView.text(" as well, leaving trails all over her light-brown skin.");
        }
...
function forcedSheilaOral(player: Character, dick: boolean = true): NextScreenChoices {
    CView.clear();
    // set sheila xp = -4
    SheilaFlags.SHEILA_XP = -4;
    CView.text("You lean down and grab the girl by her shirt-front, raising her to her knees; her hands automatically go to yours.");
    ...
    if (sheilaCorruption() >= 50) CView.text("soft, pillowy");
    else CView.text("small, perky");
    CView.text(" breast.  \"<i>You don't want to rip your clothes, do you?  It must be difficult to get nice fabric like this out here in the middle of nowhere.</i>\"");
    ...
    if (sheilaCorruption() < 20) CView.text("little round things that trace gentle curves under her loose shirt.");
    // (else sheila corruption < 50)
    else if (sheilaCorruption() < 50) CView.text("healthy mounds that fill out her shirt.");
    else CView.text("swollen globes that bulge over her v-neck and strain at the seams of her shirt.");
    CView.text("  Smiling at the sight, you grab the garment's hem and pull it up roughly.  Sheila blinks dumbly as you expose her erect nipples to the air, then blushes and covers them with her hands.");

    CView.text("\n\nStill smiling, you unfasten your [armor] and reveal your crotch.  ");
    // [(cock.length > 0)
    if (dick) CView.text(describeOneOfYourCocks(player, true) + " flops free, half-erect and swelling steadily with blood as you appraise the woman's body.");
    // (vag present)
    else CView.text("Your [vagina] drools slightly, labia parting and [clit] poking up; you imagine what you want to do with it.");
    CView.text("  Sheila watches your face as you slip into a fantasy, half-playing with her nipples - but just when you start to get into your daydream, she turns and tries to scramble to her feet!");
    ...
    if (dick) {
        ...
        // [(cock width <= 6")
        if (player.body.cocks.sort(Cock.Shortest)[0].thickness <= 6) {
            CView.text("parting her lips to pop neatly between them.  It feels good, but one-sided, and you order Sheila to get to work.");
        }
        else {
            CView.text("jabbing at her undersized mouth with your enormous [cockHead shortest].  \"<i>It's too thick!</i>\" she complains, trying to turn away.");
        ...
        CView.text("  The reticent woman brings her lips into play, sucking gingerly, ");
        // start length fork, no new PG
        // [(dick length <= 8)
        if (player.body.cocks.sort(Cock.Shortest)[0].length <= 8) {
            CView.text("and the warm embrace of her mouth is welcome stimulation.");
            ...
            if (player.body.balls.count > 0) CView.text("your [sack]");
            else CView.text("the base of your shaft");
            CView.text(", and the other hesitantly makes its way to her breasts as she gets into the sex.  The woman tweaks her nipple, softly at first but with mounting passion as her attentions on your dick become more lavish.  Catching this from the corner of your eye, you move ");
            if (!player.body.legs.isNaga()) CView.text("a [foot]");
            else CView.text("the tip of your tail");
            CView.text(" between her knees to her crotch, rubbing her pussy through her shorts; Sheila hums into your shaft as a wet spot appears on the cloth, and begins grinding her hips against the pressure.  You answer her enthusiasm by taking her head in your hands and stroking the surface of her long, pointed ears affectionately with your thumbs.  She flushes, then sinks down on your cock, rubbing the nerves on the slit with the back of her tongue and pulling you closer to orgasm.");
            ...
            if (player.body.balls.count > 0) CView.text(" and your [balls] tighten up");
            CView.text(".  She pushes against you with her hands, trying to get free, but you just hold her against your crotch as you lean over her head.");
        }
        // (else dick length > 8)
        else {
            CView.text("but this sort of half-assed attention won't get you off any time before sundown.");
            ...
            /// (else width > 6)
                ...
                if (player.body.balls.count > 0) CView.text(" and your [balls] draw closer to your body");
                CView.text("; still intent on calling this oral sex, you squeeze Sheila's jaw, forcing her mouth open, then push your drooling slit right up to it as the first squirt of white peeks out.");
            }
        }
        // end width fork and length fork
        // begin corruption worms fork
        // (if corr >= 70 and Infest command is unlocked)
        if (player.stats.cor >= 70 && player.effects.get(StatusEffectType.Infested).value1 === 5) {
            CView.text("\n\nA wicked idea flits through your head, brought on by the squirming annoyances ");
            if (player.body.balls.count > 0) CView.text("packing your [balls] and driving up your lust in their zeal to multiply");
            else CView.text("deep inside your body, trailing sensation along the nerves in your most intimate places and ramping up your production");
            CView.text(", and you decide to indulge it.  \"<i>Don't worry about rations for the day,</i>\" you cry out, \"<i>I've got more than enough to feed you!</i>\"  Sheila's resigned expression registers this as little more than masculine braggadocio - all the better for your surprise.  Pulling deep inside yourself, you force out a thick load of wormy jism, depositing it directly into the woman's throat; her body tenses at the texture and she tries again to pull away.  As the writhing mass is pushed down her gullet by yet more arriving after it, she begins to struggle violently, hitting you with tired fists and trying to wrench her head free of your grip, even at the cost of her ears.  Luckily, she lacks the energy to do permanent damage to either you or herself after the fight and the cruel face-fucking, and you pour your infested load into her with abandon.");
            // [(big skeet)
            if (player.cumQ() >= 2500) CView.text("  The volume of semen and slimy worms you pack her with is so great that it strains her stomach; you can actually see dozens of the annelids slithering around in a tight ball under the taut skin of her swollen midriff.");
            CView.text("  Eventually you finish and pull out, after pushing up every worm you can.  Your ");
            if (player.body.balls.count > 0) CView.text("[sack]");
            else CView.text("body");
            CView.text(" actually feels lighter by a noticeable amount - though you can still feel some deep inside you, reminding you that you'll be full again soon.  On that note...");
        ...
        // (else if corr < 70 or PC cannot use Infest at will)
            ...
            // [(big skeet)
            if (player.cumQ() >= 1000) CView.text(", and soon you can feel it lapping at the end of your shaft");
            CView.text(".");
            // [(mega skeet)
            if (player.cumQ() >= 3000) CView.text("  Your body's not done with that, though, and keeps pushing out cum until it spurts around your prick and out Sheila's nose in little squirts.");
            CView.text("  Emptying yourself, you pull Sheila's mouth away from your [cock shortest] and drag the slimy member on her chest, smearing her tits with spit and semen and wiping away the last of the mess on her shirt.");

            CView.text("\n\nYou pull the soiled garment down over Sheila's breasts");
            // [(big+ skeet)
            if (player.cumQ() >= 1000) CView.text(", though it's so wet with seed now that you can see them almost as well through it");
            CView.text(".  \"<i>This really is a nice fabric.  You should take better care of it.</i>\"  With that parting shot, you leave the queasy-looking woman to digest your load and the humiliation.");
        }
        // end worm fork
        ...
        // [(sheila corruption >= 30)
        if (sheilaCorruption() >= 30) CView.text("; her breasts jiggle pleasingly as they settle on her chest, filling you with the urge to play with them");
        CView.text(".  Straddling her, you lower your [vagOrAss] to her face.  \"<i>Lick,</i>\" you order, rubbing your [butt] on Sheila's nose");
        // [(not horse)
        if (!player.body.legs.isTaur()) CView.text(" as you fondle her tits");
        CView.text(".");
        ...
        if ((player.body.vaginas.length > 0 && player.vaginalCapacity() >= 100) || (player.body.vaginas.length <= 0 && player.analCapacity() >= 100)) CView.text("  She keeps hitting the same spot again, too - she's not even bothering to adjust her angle and touch the rest of your stretched opening.");
        ...
        if (SheilaFlags.UNKNOWN_FLAG_NUMBER_00141 > 0) CView.text("  I know a nice boy in the mountains who keeps dogs, and I'm sure he'd love to have a womb like yours for their seed.  By this time tomorrow, you could have a cock in both holes, filling you with hellhound babies since you can't please a person.");
        CView.text("</i>\"");

        CView.text("\n\nYou can feel her face heating up under your [butt] as she flushes with shame; seems like you struck a nerve.  You ");
        // [(no horse)
        if (!player.body.legs.isTaur()) CView.text("pinch her nipple for emphasis, then grind your hips across her face, mauling her nose with your ass.  \"<i>I could fuck a tree and it wouldn't be as wooden as you in the sack");
        // [(PC has found corrupt glade)
        if (ExploreFlags.Forest >= 40) CView.text(" - in fact, there are quite a few I've seen who look like better lovers than you.  Maybe I should carry you to the forest and tie you to a nice pussy-shaped giant flower to give you lessons");
        CView.text(".  ");
        // [(minotaur addiction score =/= 0%)
        if (SheilaFlags.MINOTAUR_CUM_ADDICTION_TRACKER > 0 || player.perks.has(PerkType.MinotaurCumAddict)) {
            CView.text("Perhaps I'll turn you over to a minotaur; you're bound to get better at using your tongue when you're addicted to their amazing seed.  Would you like that?  The first thing you'd see in the morning would be a huge flared cock, right in front of you, dripping with heavenly precum and waiting for your mouth... or your cunt.  You could be just another minotaur-fucking slut of the plains.  ");
        }
        CView.text("Or I could take you home and give you personal training myself, whenever I feel like it, and invite travelers to do the same");
        // [(PC has Izma with cock)
        if (izmaFollower() && SheilaFlags.IZMA_NO_COCK === 0) CView.text("; I personally know fifteen inches of fat, orange dick and four heavy balls that'd love to get in at your other end while I ride your face");
        CView.text(".</i>\"");
        ...
        if (player.effects.has(StatusEffectType.HairdresserMeeting) || SheilaFlags.LUMI_MET > 0) {
            CView.text(" while imagining her turned over to the goblins for experiments, forced to drink potion after potion - most would be dedicated to growing cocks on her or increasing semen production, of course");
        }
        CView.text(".  Your hole clenches and drools on her, and the woman's rude tongue retreats from the pressure, leaving you painfully empty.");
        // [(no horse)
        if (!player.body.legs.isTaur()) CView.text("  Even squeezing her breasts viciously can't compel her to reinsert it, though you amuse yourself trying.");
...
function sheilaGetsRapedWithADildont(player: Character): NextScreenChoices {
    ...
    if (player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) {
        CView.text("Your face heats as she kisses you sloppily, and not all of the temperature is from her mouth.  It feels like she's pulling energy right out of you, and you can swear that her breasts are growing.");
    }
    // (else PC corr < sheila corruption)
    else if (player.stats.cor < sheilaCorruption()) {
        CView.text("Her lips heat up and your body tingles; you can see her breasts shrinking as she shivers and slobbers on your face - or so it seems from the angle.");
    }
    // (else if magically equal but < 90)
    else CView.text("The kiss is wet and indecent as she shudders through her anal orgasm; her lips don't stay in one place, instead tracking drool all over your face.");
    ...
    // (cock)
    if (player.body.cocks.length > 0) CView.text("  \"<i>Perhaps next time you'd like to try a real cock in there?</i>\"  Even exhausted and messed up on goblin drugs, she has the presence of mind to shake her head at that.  You laugh.");
    ...
    player.stats.lustNoResist += player.stats.lib / 3;
...
function getBeatUpBySheila(player: Character, monster: Character): NextScreenChoices {
    CView.clear();
    CView.text("You slump down before Sheila, breathing heavily; the woman ");
    if (monster.stats.lust < 75) {
        CView.text("takes the opportunity to grab her catch and take off running!  Even if you wanted to, you couldn't catch her now... you collapse, head swimming in ");
        if (player.stats.HP < 1) CView.text("colorful spots");
        else CView.text("fantasies of her, stark naked but for her hat and waiting with her legs open");
        CView.text(".");
    ...
    // (else monster lust >= 75)
        ...
        CView.text("stares at you, desire and nerve at war in her face.  Her thighs rub together involuntarily as she tries and fails to will herself away... slowly, she takes a step toward you.");
...
function sheilaLossRapeRouter(player: Character): NextScreenChoices {
    ...
    if ((player.body.cocks.length <= 0 || randInt(3) === 0)) return sheilaForcedOralGeneric(player);
    // loss scene - cowgirl (for M,H)
    // if sheila xp >= -3, set sheila xp = -2
    else {
        return sheilaCowgirlRapesYou(player);
    }
}

// loss scene - cowgirl (for M,H)
// if sheila xp >= -3, set sheila xp = -2
function sheilaCowgirlRapesYou(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Sheila pushes insistently, and you allow her to force you to the ground and then onto your back");
    if (player.body.legs.isTaur()) CView.text("; your hooves stick awkwardly out to one side");
    CView.text(".  She blushes furiously and won't meet your eyes, but it doesn't stop her from stripping off her vest and shirt in one smooth motion, revealing her " + sheilaCup(player) + " chest.  \"<i>T-touch me,</i>\" she demands, \"<i>rub me.</i>\"  Almost as an afterthought, she adds \"<i>... please.</i>\"");

    CView.text("\n\nYou move your hands to her smooth skin, cupping a ");
    if (sheilaCorruption() < 30) CView.text("petit ");
    else if (sheilaCorruption() > 70) CView.text("massive ");
    CView.text("breast in each, squeezing them softly and rubbing the hard nipples with your thumbs.  Sheila shudders as you amuse yourself with her body, placing her hands atop yours and guiding you to her favorite spots.  You watch her as she moans; your gazes meet as she rolls her head, and she quickly restrains herself and looks away again.  \"<i>Stop staring at me...</i>\"");

    CView.text("\n\nEager to see anything but your curious face, Sheila pushes your hands away from her breasts and turns to your crotch.  She ");
    if (player.body.legs.isTaur()) CView.text("pulls your legs apart");
    else CView.text("unfastens and strips off your [armor]");
    CView.text(", revealing your " + describeCocksLight(player) + "; ");
    // [(if no cock fit 32)
    if (player.body.cocks.find(Cock.CockThatFits(32))) {
        CView.text("her eyes widen as " + describeOneOfYourCocks(player) + " bobs free, half-erect monster meat that fills her with frustration.");
        ...
        // [(horse)
        if (player.body.legs.isTaur()) CView.text(", or at least as flat as you can get it");
        CView.text(", then sits on your stomach");
        if (player.body.legs.isTaur()) CView.text(", sliding her long legs between yours");
        CView.text(".  She scoots forward slowly, and gasps as her pussy comes to rest against your hot, blood-engorged shaft.");
        ...
        if (player.body.legs.isTaur()) CView.text("forelegs fold over her torso affectionately");
        else CView.text("hands search out her breasts, caressing and squeezing");
        CView.text(" as her gasps and the picture in your head both intensify.  Sheila grabs one with her own hand, pulling it up to her mouth to smother it in kisses");
        if (!player.body.legs.isTaur()) CView.text("and suck on your fingers");
        CView.text(".  \"<i>Oh... [name]...</i>\"");
        ...
        CView.text("Sheila shivers and leans back, hair dangling, as her hips buck and her thighs squeeze your shaft.  ");
        // (PC corr >=90 or > sheila corruption)
        if (player.stats.cor >= 90 || sheilaCorruption() < player.stats.cor) {
            CView.text("You can feel your prick heating up where her pussy rubs against it... it almost feels like the rest is cooling down at the same time.  The woman atop you arches her back and sucks in her breath sharply, and her tits seem to claim your field of vision, swelling up - though that could just be the angle.");
        }
        else if (player.stats.cor < sheilaCorruption()) CView.text("Her orgasm wets your prick, coating it with fluid as she comes.  Sheila wraps herself tighter in your limbs, squeezing her breasts against her chest, and you think you can feel them get a little smaller as your head fuzzes over with her excess emotion.");
        else CView.text("Her hips lock up when she comes, leaving you to try and finish yourself as she heats up around your shaft.");

        // [(sens >=50)
        if (player.stats.sens >= 50) {
            CView.text("\n\nThe warm, smooth, wetness of her thighs is enough to bring out your own climax, and " + describeOneOfYourCocks(player) + " unburdens itself, forcing globs of semen into the air to rain down on your entwined bodies.  Sheila laughs as you coat her midriff with your mess, and playfully strokes your sensitized [cock] with one hand, sending shivers through you as she helps force your cum up your shaft.");
            // [(mega skeet)
            if (player.cumQ() >= 2000) CView.text("  The giggles and the strokes stop long before your orgasm, though; eventually Sheila just lies there, dumbfounded, as you deposit layer after layer of white lattice on her skin.  \"<i>Bloody hell,</i>\" she mumbles vaguely, either stone-sober or drunk from the sight of it.");
            CView.text("  Once the rain of semen peters out, she pulls apart from you, slimy and dripping.");
    ...
    // (else if cock fit 32)
        ...
        CView.text("as you lean over to catch her expression, you can see her eyes glimmer with desire.  Sheila notices you from her peripheral vision, and her face reddens again.  She turns her back to you.");
        ...
        // [(multicock)]
        if (player.body.cocks.length > 1) CView.text("separating out and ");
        CView.text("stroking your [cockFit 32].  A drop of precum forms, and she touches it with the index finger of her free hand, sending a tingle through you; she pulls the finger away, drawing a string of slick moisture out.  Sheila passes the finger between her breasts, wiping the fluid away on her chest and shivering.  The display is too much for you, and you reach forward, navigating your hands around her waist quietly until you can reach the buckle of her belt.  The girl sighs as you begin to unfasten it, but an errant thought brings her back to reality - she grabs your hands abruptly.");
        ...
        // (else lib >= 25)
            ...
            if (player.body.legs.isTaur()) CView.text(", pushing your legs apart");
            CView.text("; looking into your face, her frown evolves into a mischievous grin.  Sheila about-faces, turning her back to you.  \"<i>Blame yourself for this; it's rude to stare,</i>\" she giggles, hovering over your [cockHeadFit 32].  The woman sinks down with an audible sigh, parting her slick, smooth labia with your tip and swallowing your " + numToCardinalText(Math.round(player.body.cocks.find(Cock.CockThatFits(32)).length)) + " inches.  \"<i>Ohh...</i>\" Sheila moans.  She slips a hand down to your crotch to check how much of your prick is left to go, and starts a bit when she realizes she's taken the entire shaft on the first stroke!");
            ...
            if (!player.body.legs.isTaur()) CView.text(", and your hands go to her waist, fairly pushing her up and slamming her back down in your eagerness");
            CView.text(".  She rolls her head as the fuck gets wetter and faster, dangling her hair over her shoulders as her back muscles shift and flex with her strokes; her ears flick whenever she descends and rubs your neck against one of the sweet spots on her vulva.");
            ...
            // (PC corr >= 90 or > sheila corruption)
            if (player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) CView.text("You can feel her cum-slick inner walls pulling at you as she begins to orgasm, and your mind clears as you blow your load inside her, wondering when she'll likewise return to sense.");
            // (else PC corr < sheila corruption)
            else if (player.stats.cor < sheilaCorruption()) CView.text("Her pussy gushes with her orgasm, pushing out your semen and soaking her inner lips even as she continues riding your cock, and your mind clouds as you think of little except sneaking as much of your cum inside her as possible.");
            // else equal but < 90)
            else CView.text("Even as she comes mindlessly something makes you hold in your own moans, wondering how long it will take her to notice her pussy being filled with your egg-hunting sperm.");
            // [(small cum)]
            if (player.cumQ() <= 250) {
                CView.text("  Sheila continues moving her hips even as she comes, lost in bliss; on one particularly lofty ascent, your cock flops out, squirting a line of jizz along ");
                if (player.body.tails.length > 0) CView.text("your tail");
                else CView.text("the grass");
                CView.text(", and her eyes go wide.");
            }
            // (big cum)
            else {
                CView.text("  With how much Sheila is still moving her hips, it's only a matter of time before she notices the mounting pressure in her cunt... on one particularly deep push, she forces out a squirt of white, and her eyes go wide as she pulls off and lets you finish on the grass.");
            }
            ...
            CView.text(" you groan.  \"<i>Can't believe you didn't notice...</i>\"");
...
// boilerplate face-in-cunt shit
// if sheila xp >= -3, set sheila xp = -2
function sheilaForcedOralGeneric(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Sheila kisses you once more, forcing her tongue into your mouth, and slides a hand into your [armor]; you stare wonderingly into her affectionate, hungry eyes at the sudden change.  It isn't until ");
    if (player.body.cocks.length > 0) CView.text("her hand slips around your [cock]");
    else if (player.body.vaginas.length > 0) CView.text("her fingers slip between your labia");
    else CView.text("she rubs your bare crotch for a few seconds");
    CView.text(" that her reverie breaks; her face colors and she looks away, averting her gaze in shame.  She draws her hand back out guiltily, then begins to doff her shorts and panties.  \"<i>S-sorry... didn't mean to get familiar.  I'll have this over with quickly.</i>\"");

    CView.text("\n\n\"<i>That's-</i>\"  Your words are cut off as she suddenly climbs atop you, bringing her pussy to eye level");
    if (player.stats.str < 61) CView.text("; her weight bears you slowly to the ground");
    CView.text(".  Sheila rubs the smooth slit on your [face], trying to part her labia on you, and shivers a bit when your lips brush hers.");
    ...
    if (player.body.hair.type === 4) CView.text(" and flushing as your stingers release their payloads into her palm");
    CView.text(".  You obediently stick out your tongue");
    if (player.stats.cor > 60) CView.text(", motivated more by whimsy than the meager, almost playful pain,");
    CView.text(" and shallowly penetrate Sheila's vagina with it.");
    ...
    if (player.body.tongue.type > TongueType.HUMAN) {
        CView.text("  A sly look crosses your face at her request; she notices it but has no time to dismount before your long tongue shoots into her, stroking the nerves around her entrance as you unspool.  \"<i>Ah-ahhh!</i>\" she moans, \"<i>What the hell... is with your mouth?</i>\"");
        CView.text("\n\nPetulantly you stare at her as you work, unable to speak with your tongue unrolled in her cunt.  She blushes under your gaze, then leans forward and wraps her arms around you, brushing the top of your head with her shirt-clad " + sheilaCup(player) + " breasts.  \"<i>You're r-right, mate... rude to talk with your mouth f-full... aaah!</i>\"  A little firm spot inside of her provokes this last reaction; you experimentally run your tongue over it again, and the woman shudders violently.");
    ...
    // [(PC corr >= 90 or > sheila corruption)]
    if (player.stats.cor >= 90 || player.stats.cor > sheilaCorruption()) {
        CView.text("strangling your tongue with muscles attempting to lock onto it, trying to pull it in.");
        // [(demon/dragon tongue)
        if (player.body.tongue.type > TongueType.HUMAN) CView.text("  As tired as you are, you simply go along with it, allowing her rippling walls to slowly draw your tip deeper inside until it bumps against her cervix; she shivers and kisses the top of your head as you groggily run your tongue over the entrance to her womb.");
        else CView.text("  However, it doesn't get very far; you lazily flop it around inside her as she works through her arousal.");
        CView.text("  Sheila groans loudly and your vision fades; through the fog, you can swear you feel her breasts growing larger against your forehead.");
    }
    // (else PC corr < sheila corruption)
    else if (player.stats.cor < sheilaCorruption()) {
        CView.text("filling your mouth with her liquid love.  Your lips and throat heat up as you're forced to drink it from the nearness of her pussy; as she bucks her hips weakly into your face, trying to draw out her climax, you increasingly wish that you could return the favor, ");
        if (player.body.cocks.length > 0) CView.text("face-fucking her and blowing a load into her mouth, then holding her jaw shut until she swallows it unhappily.");
        else if (player.body.vaginas.length > 0) CView.text("smearing your gash on her face until you come and drown her in your squirting girl-jizz.");
        else CView.text("forcing her to tongue your asshole until you come, writhing and pressing her head into the grass with your [butt].");
    }
    else CView.text("and she pets your head affectionately as her walls squeeze your tongue.  \"<i>Oh, mate...</i>\" she whispers lovingly, completely forgetting the struggle that led to your tryst.");
    ...
    // [(str < 61)
    if (player.stats.str < 61) CView.text("climbing off and kneeling next to you.");
    else CView.text("letting herself down your body until she can put her long legs on the ground once more.");
    CView.text("  She looks into your eyes contentedly, rubbing your cheek... but then starts and looks away, guilty once more.  \"<i>Shit...</i>\" she mumbles.  \"<i>I did it again.  Even after I promised myself I wouldn't...</i>\"");
    ...
    if (player.body.tongue.type > TongueType.HUMAN) player.stats.fatigue += 15;
    player.stats.lustNoResist += player.stats.lib / 3 + 30;

    if (player.stats.lust >= 100) player.stats.lib += 1;
...
// set sheilapreg = 0
function normalSheilaPregNotifNumberOne(player: Character): NextScreenChoices {
    flagWomb.birth(); // Clear Pregnancy
    CView.clear();
    CView.text("Traipsing through the grass, you can see Sheila sitting under the shade of a low tree from a long way off.  The reverse is also true, as she waves to you from her seat, beckoning you over.  You make your way to her curiously.");
    ...
    if (player.stats.cor < 50) CView.text("  A hand placed around her shoulder seems to steady her a bit.");
    ...
    return {
        choices: [
            ["Yep, Issue", normalSheilaPregNotifNumberOneYepIssue],
            ["Nah", normalSheilaPregNotifNumberOneCoolDeal],
        ]
    };
...
function normalSheilaPregNotifNumberOneYepIssue(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>I see,</i>\" Sheila says, nonplussed.  \"<i>I'm sorry you feel that way...  Truth be told, I'm not keen myself on the idea of having a kid, but what's out is out.</i>\"");
    ...
    SheilaFlags.SHEILA_DISABLED = 2;
    SheilaFlags.SHEILA_JOEYS++;
    return { next: returnToCampUseOneHour };
}

// [Normal Preg Notif #1 - Nah]
function normalSheilaPregNotifNumberOneCoolDeal(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Sheila exhales explosively when you gently shake your head, then wraps her arms around you and buries her face in your shoulder.  \"<i>Oh, thank god,</i>\" she says, muffled.  \"<i>Truth be told, I'm not keen yet on the idea of having a joey.  Motherhood feels like something that happens to MY mum.</i>\"  She snuggles closer.  \"<i>But I like holding it.</i>\"");
    ...
    if (SheilaFlags.SHEILA_XP < 4) SheilaFlags.SHEILA_XP = 4;
    SheilaFlags.SHEILA_JOEYS++;
    return { next: returnToCampUseOneHour };
...
function normalSheilaPregNotifREPEATEDED(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Sheila is here as you step into the plains, with a grass-wrapped parcel of food open on her lap and her hat sitting upturned next to her, and waves you over.");
    ...
    if (randInt(10) < 7) CView.text("healthy.  It's eating a lot... frankly, it's a little hellraiser with a strong grip, and it cries well, too.</i>\"");
    else CView.text("weak... we thought we were going to lose it for a bit.  It wouldn't drink from the nurse unless I was there, so I had to stay up with it all night myself.  Feeling a bit stuffed.</i>\"");
    CView.text("  She looks over at you, giving you the benefit of her fatigue-lined eyes.");

    CView.text("\n\nSheila slips her hand into yours");
    if (player.stats.cor < 40) CView.text(", and you give it a squeeze");
    CView.text(".  \"<i>I just wanted to tell you about your baby.  Can't stay... have to catch up to quota still.</i>\"  She drags to her feet and turns to go, tail listlessly hanging.  Even her ears are drooping; this girl is tired.");

    flagWomb.birth(); // Clear Pregnancy
    SheilaFlags.SHEILA_JOEYS++;
    if (sheilaCorruption() > 80) SheilaFlags.SHEILA_CORRUPTION = 80;

    const choices: ScreenChoice[] = [];
    // [Help(requires >80 speed, centaur >= 5', naga, or big wings and archery)][Walk With Her][Let Her Go]
    if ((player.stats.spe > 80 && player.perks.has(PerkType.Runner)) || (player.body.legs.isTaur()) || player.body.legs.isNaga() || (player.canFly && player.inventory.keyItems.has("Bow")))
        choices[0] = ["Help", normalSheilaPregNotifREPEATEDEDHelpABitchOut];
    choices[1] = ["WalkWithHer", normalSheilaPregNotifREPEATEDEDWalkWithHer];
    choices[2] = ["Let Her Go", normalSheilaPregNotifREPEATEDEDLetHerGo];

    return { choices };
}

// [Normal Preg Subsequent - Let Her Go]
function normalSheilaPregNotifREPEATEDEDLetHerGo(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Nothing you can do about that - even if you wanted to watch your baby for her, she's keeping it hidden in a hole in the ground.  You hand her the parcel back");
    if (player.stats.cor < 40) CView.text(", imploring her to eat the rest and keep up her strength");
    CView.text(".  Sheila smiles weakly and takes it, folding the grass around again.  \"<i>Thanks, [name].  I'll see you soon... ish.</i>\"");
...
function normalSheilaPregNotifREPEATEDEDWalkWithHer(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You pick up her forgotten hat and rise as well, carefully refolding the grass around Sheila's uneaten lunch and tucking it back into her pack for her.  She takes her hat back from you distantly.");

    CView.text("\n\n\"<i>Thanks, mate... forget my own head next.</i>\"  She doesn't even put it on, simply holds it in her hand as she plods on.  You ");
    if (!player.body.legs.isNaga()) CView.text("walk");
    else CView.text("slither");
    CView.text(" next to her, easily keeping pace with the tired woman, and she raises her face to you inquiringly.  \"<i>What's up, [name]?</i>\"");
...
function normalSheilaPregNotifREPEATEDEDHelpABitchOut(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You pick up her forgotten hat and set it on her head, then fold her lunch back into its grass wrapper and tuck it into her pack.");
    ...
    // (else naga)
        ...
        CView.text("\n\nThe woman takes the opportunity to secure your 'catch', and the two of you move across the grasslands, quickly meeting her quota and more by working together like this.  \"<i>Well?</i>\" you ask Sheila, once she's loaded down with ");
    ...
    else if (player.canFly() && player.inventory.keyItems.has("Bow")) {
    ...
    // (else spd >= 80 and Runner perk)
    else if (player.stats.spe >= 80 && player.perks.has(PerkType.Runner)) {
        ...
        if (player.stats.lib < 50) CView.text("you pinch her ear, provoking an annoyed flick from it.");
        else CView.text("you slap her on the ass, grabbing a handful and taking her mind off her own misery.");
        CView.text("  \"<i>Dammit, [name]... not now,</i>\" she frowns, pulling away from you.");
        ...
        if (!player.body.legs.isGoo()) CView.text("run");
        else CView.text("glide");
        CView.text(", making a long loop around to the rear of the wary animal, then begin to systematically drive it closer to your partner, who has strung a line to the base of a shrub and is gesturing at you to force the hare into it.");
    ...
    if (Time.hour + 4 < 21) return { next: returnToCampUseFourHours };
    else {
        ...
        const choices: ScreenChoice[] = [];
        choices[0] = ["StayWithHer", normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE];
        if (!(SheilaFlags.FUCK_FLOWER_LEVEL >= 4 && SheilaFlags.FUCK_FLOWER_KILLED === 0) && !vapulaSlave())
            choices[1] = ["To Camp", normalSheilaPregNotifREPEATEDEDHelpABitchOutTOCAMP];
        choices[2] = ["Welp", normalSheilaPregNotifREPEATEDEDHelpABitchOutANDWELP];
...
function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDWELP(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Welp,</i>\" you say softly.");
    // [(corr <50)
    if (player.stats.cor < 50) CView.text("  You set a hand on Sheila's shoulder to steady her a bit as the tears start to fall.");
    CView.text("  \"<i>It'll be alright.</i>\"");
...
function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("With a wan smile, you pull the woman to her feet.  She doesn't resist, but continues looking into your face disconsolately.  \"<i>I'm so stonkered I can't even think what to do first...</i>\"");
    ...
    if (player.body.legs.isNaga()) CView.text("coil");
    else CView.text("seat");
    CView.text(" yourself, then grabs your arms and pulls them around her waist.");

    // forks in order of priority, no new pg at start
    // [(full gooey body)
    if (player.body.legs.isGoo()) {
        CView.text("  She considers for a moment, rubbing away some of the slime you've deposited on her shorts, and pulls free from your slimy skin again.  \"<i>Um... do you... would you be offended if I took my kit off, so it doesn't get ruined?</i>\"");
        if (player.stats.lib >= 40) CView.text("  The leer you display is so prurient that she seems on the verge of reconsidering, but you compose yourself and");
        else CView.text("  You");
        CView.text(" give assent with little more than a broad smile.  Sheila pulls her shirt over her head, revealing her " + sheilaCup(player) + " breasts, but raises her face to you hesitantly when her hands get to her belt.");
        ...
        player.stats.lustNoResist += player.stats.lib / 5 + player.stats.sens / 3;
    ...
    // (naga)
        ...
        if (player.stats.lib >= 40) CView.text(" - but tighten the one sliding across her breasts just slightly.  Sheila's neck turns red and her ears twitch as you rub her nipples, and you plant a kiss on her nape");
        CView.text(".  \"<i>Ahh,</i>\" she sighs, shifting in your coils.  \"<i>Hold it right there...</i>\"");
        // (apply libido-based lust damage if lib >= 40)
        if (player.stats.lib >= 40) player.stats.lustNoResist += player.stats.lib / 4;

    }
    // (horse and >=5ft)
    else if (player.body.legs.isTaur() && player.body.tallness >= 60) {
        ...
        if (player.stats.sens >= 40) CView.text("... or would be, if you weren't distracted by the nipples poking into you and the thighs squeezing your flank");
        CView.text(".");
        // (apply sens-based lust damage if sens >=40)
        if (player.stats.sens >= 40) player.stats.lustNoResist += player.stats.sens / 4;

    }
    // (harpy - minimum score for birdlegs at least)
    else if (player.body.legs.type === LegType.HARPY && player.body.tails.find(Tail.FilterType(TailType.HARPY))) {
    ...
    // (bees with boots)
    else if (player.body.legs.type === LegType.BEE) {
    ...
    // (eight-legged freaks)
    else if (player.body.legs.isDrider()) {
        ...
        spiderTail.venom -= 10;
        if (spiderTail.venom < 0) spiderTail.venom = 1;
    }
    // (else)
    ...
    if (!player.body.legs.isGoo()) CView.text("blood");
    else CView.text("goo");
    CView.text(" flowing, unsure if you'll be able to get to sleep, but Sheila has no such problems.  Within minutes, the woman is out, head nodding forward.  You suppose that means the first watch is yours... it passes slowly, despite the scenery, but eventually you nudge your partner awake when you can stay up no longer.");
    ...
    if (!player.body.legs.isTaur()) CView.text("  She grudgingly repositions, allowing you to rest against her.");
    Time.day++;
    Time.hour = 2;

    // --Next--

    return { next: normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE2 };
}

// advance time to 5:00
function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE2(player: Character): NextScreenChoices {
    CView.clear();
    Time.hour = 5;

    CView.text("Your sleep is fitful, but not totally useless, and you yawn and stir a few hours later when Sheila wakes you");
    if (player.body.legs.isGoo()) CView.text("; she's already dressed and appears to have just been outside");
    CView.text(".");
    ...
    if (player.stats.lust < 30) CView.text("  You give her a hand, then help her load up.  She gives you a peck on the cheek before she departs.  \"<i>Thanks, love.</i>\"");
    if (player.stats.lust >= 30) {
        CView.text("  Part of you is actually sad to see your cozy hovel being disassembled if it means her warm body won't be pressed up against yours anymore, and your hands begin to rove over her as she works.");
        ...
        const choices = sheilaXP4SexMenu(player);
        choices[9] = ["LeaveHerBe", normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDEREBUTLEAVEHERBE];
        return { choices };
    }
...
function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDEREBUTLEAVEHERBE(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You relent, allowing the woman to free herself with a peck on your cheek.  She loads up her catch, and then turns to you.  \"<i>Thanks, [name].  Owe you one.</i>\"");
    // This is your Protoshield! I can't take this!
    return { next: returnToCampUseOneHour };
}

// [Normal Preg Subsequent - Help - 21:00 hours - To Camp]
function normalSheilaPregNotifREPEATEDEDHelpABitchOutTOCAMP(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Wordlessly, you lift the bundle of game animals off of Sheila's shoulders and sling it over your own.  She looks up at you, confused enough to forget about her tears.  \"<i>What're you doing?</i>\"");
    ...
    // [(int >=60)]
    if (player.stats.int >= 60) CView.text("\"<i>The baby will be fine,</i>\" you venture, hazarding a guess.  \"<i>It's what the nurses are there for.</i>\"");
    else CView.text("You slip an arm around her, and ask if she's okay.");
    CView.text("  She nods mutely and pulls the food out of the fire.");
    // [(if Jojo present)
    if (player.effects.has(StatusEffectType.JojoNightWatch) || player.effects.has(StatusEffectType.PureCampJojo)) CView.text("  She extends one bird to you and another to Jojo.  He raises his hand and shakes his head, but politely.");
    else if (SheilaFlags.ANEMONE_WATCH > 0) CView.text("  Sheila offers you one of the birds, then extends another to your odd tenant.  The anemone's mouth tightens and she shakes her head vigorously, blushing a deep blue.");
    CView.text("  You eat in silence until the food is gone, and Sheila clears her throat.");
    ...
    // [(if no watch)
    if (!player.effects.has(StatusEffectType.JojoNightWatch) || !player.effects.has(StatusEffectType.PureCampJojo) || SheilaFlags.ANEMONE_WATCH === 0) CView.text("\n\nShe gets to her feet as you grin, then makes her way over to your bedroll.  It doesn't take her long to fall asleep; you amuse yourself watching her toss and turn... and drool on your pillow.");
    // (if watch)
    else {
        CView.text("\n\nYou grin and get up, and Sheila tails you to your bed.  ");
        if (!SheilaFlags.SLEEP_WITH) CView.text("She looks with some distaste at the nearby sleeper, and steps past.  You pick up the bedroll and follow her, reasoning that if she keeps her promise to be gone before dawn, it shouldn't raise any questions.  \"<i>Making friends, I see,</i>\" Sheila mutters... but as you unroll the bed again, her eyes zero in on it like a drowning man's on a raft, and she says nothing else.  ");
        CView.text("She lies down first, allowing you to cozy up to her as far as you dare.  Just as you're about to drift off, you feel Sheila take your hand and pull your arm over her body.");
    }
...
function sheilaGoesDemon(player: Character): NextScreenChoices {
    ...
    if (SheilaFlags.SHEILA_XP < -2) CView.text("always so bloody eager to fuck me!");
    else CView.text("me best mate!");
    CView.text("  Help out!</i>\"");
    ...
    player.inventory.keyItems.add("Sheila's Lethicite", 0, 0, 0, 0);
    SheilaFlags.SHEILA_CITE = 1;
    SheilaFlags.SHEILA_DEMON = 1;
...
// Demon Sheila encounter (demon sheila = 1 and sheilapreg < 4)
function demonSheilaEncounter(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("A shadow drops out of a low tree behind you with a rustle.  \"<i>Hey, lover; been a while... how about spending some time with me?</i>\"");

    CView.text("\n\nAs you turn around, Sheila sashays over to you.  Apart from a little lavender crystal dangling from her long, bare ear and several finger rings, she's naked as a jaybird now, exposing her " + sheilaCup(player) + " breasts and smooth, dripping snatch for all the world to see.  Two lithe tails, one spaded and one not, rub against her labia in anticipation as she takes a horse stance in front of you to wait on your answer, and her eyes smolder with lust.");

    // [Talk][Let Her Fuck(req. lust >=30)][Resist][Run]

    const choices: ScreenChoice[] = [];
    choices[0] = ["Talk", demonSheilaTalk];
    if (player.stats.lust >= 33) choices[1] = ["LetHerFuck", sheilaLetHerFuckYou];
    choices[2] = ["Resist", demonSheilaResist];
    // run simply returns to camp and should be the default spacebar option
    choices[4] = ["Run", returnToCampUseOneHour];

    return { choices };
}

// [Demon Sheila - Resist]
function demonSheilaResist(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You back away from the demon and raise your [weapon] with a dour glower.  Sheila purses her lips at you, then says, \"<i>Not in the mood, eh mate?  I'm so horny though... I bet I could get you into it.  Let me show you how much I love you...</i>\"  Her ears twitch furiously, belying her intentions as she");
    // [(sheila corruption < 30)
    if (sheilaCorruption() < 30) CView.text(" thrusts her black spade forward and smears some of the constantly-oozing fluid from its tip down the length with her hand");
    else CView.text(" kneads her breasts seductively, pinching her nipples");
    CView.text(".  Your posture softens a bit as she reaches out to you");
    if (sheilaCorruption() < 30) CView.text(", tilting her palm to dribble a handful of strong-scented dark oils");
    CView.text(", but as she giggles sadistically and starts to approach, you tense up again.  It's pretty clear by the way she swaggers dismissively that she isn't taking the rejection well!");
    // go to fight
    return CombatManager.beginBattle(player, new Sheila());
}

// [Demon Sheila - Talk]
function demonSheilaTalk(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Questions nag at you, and you ask Sheila if she wouldn't mind answering a few of them.  She struts over and ");
    // [(not horse)
    if (!player.body.legs.isTaur()) CView.text("takes your arm in hers, then slips the other hand into your armor");
    // [(horse)
    else CView.text("between your legs");
    CView.text(" to touch your ");
    if (player.body.cocks.length > 0) CView.text("[cock]");
    else if (player.body.vaginas.length > 0) CView.text("[vagina]");
    else CView.text("smooth skin");
    // [(lib <20)
    if (player.stats.lib < 20) CView.text(", giggling as your expression petrifies");
    CView.text(".  \"<i>Anything for you; I'll just find something to occupy my attention.  What's on your mind, love?</i>\"");
    // plus lust depending on sens
    player.stats.lustNoResist += 2 + player.stats.sens / 5;

    // [The Change][Her Colony][What Now?][Fuck Off]

    return {
        choices: [
            ["The Change", askDemonSheilaAboutDahChange],
            ["Her Colony", askDemonSheilaAboutHerColony],
            ["What Now?", talkToDemonSheilaAboutWhatNow],
            ["Fuck Off", tellSheilaDemonToFuckOff],
        ]
    };
}

// [ Demon Sheila - Talk - Fuck Off]
function tellSheilaDemonToFuckOff(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Oh,</i>\" she says.  \"<i>Don't be like that.  You're my best mate, you know.  My beloved.  I'll never let you go, ever.</i>\"  Her eyes gloss over a bit, ");
    // go to fight if bitch is crazy, else plus lust and return to camp
    player.stats.lust += 10;

    // [(RNG decides if dis bitch crazy, bias toward sane)
    if (randInt(10) < 3) {
        CView.text("and she blurts, \"<i>I know I can make you understand.  You can't go until you say it back!</i>\"  You pull her hand free and back away from her, but she advances again, trying to grab you!");
        return CombatManager.beginBattle(player, new Sheila());
    }
    else {
        CView.text("but she remasters herself with effort.  \"<i>No worries.  I'm sorry to hear you don't have any time for love, my special one.  I'll be here until you do, thinking about you and touching myself.</i>\"  She releases you and steps away, then crudely shoves her spade through her thighs, grabs it, and half-moans as she thrusts her pelvis back and forth, jerking the thick black flesh in her hand.  With a wink, she abruptly releases it, then turns her back and departs.");
        return { next: returnToCampUseOneHour };
    }
}

// [Demon Sheila - Talk - The Change]
function askDemonSheilaAboutDahChange(player: Character): NextScreenChoices {
    CView.clear();
    // [(if sheilacite >= 1)]
    if (SheilaFlags.SHEILA_CITE >= 1) {
        CView.text("Sheila blushes deeply.  \"<i>Silly.  You were there.</i>\"  She looks at you, pupils dancing as she remembers the lovemaking session that led to her current form.  \"<i>I suppose that's not what you're asking, though.</i>\"");
        ...
        if (player.body.cocks.length > 0) CView.text(", and you feel a monument of your own being erected");
        CView.text(".  \"<i>Every squeeze of my pussy compressed my focus, too, until my mind was clearer than the juice you made me flow with.  I realized my purpose there, with you.</i>\"");
    ...
    player.stats.lustNoResist += 12;

    const choices: ScreenChoice[] = [];
    choices[0] = ["Another Q.", demonSheilaTalkAnotherQuestion];
    if (player.stats.lust >= 33) choices[1] = ["LetHerFuck", sheilaLetHerFuckYou];
    choices[2] = ["Resist", demonSheilaResist];
...
function askDemonSheilaAboutHerColony(player: Character): NextScreenChoices {
    ...
    CView.text(", you ask what happened to him.  Sheila smiles serenely before answering.  \"<i>Unsurprisingly, his prick was all too eager to fill me with sperm after the countless sermons he preached about it being every woman's duty to expand the clan.  He barely lasted three minutes when I climbed atop him.  Oh, that little squirt was the most satisfying one.  Watching him melt under me drove home how much power I've always had... I wish I could have known when I was just an awkward little girl, sitting through his browbeating lectures on loyalty and servitude.</i>\"");
    ...
    // (corr >=75)
    if (player.stats.cor >= 75) CView.text("harvested that day, you sigh wistfully");
    else CView.text("stolen that day, you wince");
    CView.text(".  Sheila, peeking at you with one eye from under her bangs, interprets your expression correctly.  \"<i>Wondering where the others went, love?</i>\"  She releases her grip on your arm and holds out the hand that's not exploring your [armor], displaying several rings.  \"<i>The rings are my closest.  I kept them for sentimental reasons.  The rest, well... I ate them.</i>\"");
    ...
    // [(cock)
    if (player.body.cocks.length > 0) CView.text("she ties you down and rides you until she's so full of your spunk that she can push it back out just by pressing on her stomach");
    else CView.text("she binds your hands and sits on you, teasing your helpless body with her oozing, throbbing spade until she comes all over you, spraying dark slimy fluid from it");
    CView.text(".  You ");
    if (player.stats.cor < 50) CView.text("break eye contact, blushing");
    else CView.text("stare into her shining eyes, getting hotter and hotter as the vision plays out to the very end");
    CView.text(".");
    ...
    player.stats.lustNoResist += player.stats.lib / 3 + 5;
...
function talkToDemonSheilaAboutWhatNow(player: Character): NextScreenChoices {
    CView.clear();
    // fucking daddy issues
    CView.text("\"<i>What will I do now?  What else, my silly love?</i>\"  Sheila looks out over the plains.  \"<i>I'm always hot for you.  We could have a bit of a naughty, and make each other feel good.</i>\"");
    ...
    // (lib>=30)
    if (player.stats.lib >= 30) CView.text("\n\nYour eyebrows rise.  \"<i>Got your attention, did I?  But,</i>\"");
    else CView.text("\n\nYour mouth hardens.  \"<i>You're right,</i>\"");
    CView.text(" Sheila says, \"<i>I'm going to save that one for our honeymoon, instead.</i>\"  Her eyes twinkle with infatuation.  \"<i>I could even pull the taint of this land out of your body and use it to make mine more lewd for you, my " + mf(player, "knight-champion", "great lady") + "...</i>\"");
    ...
    if (player.stats.cor < 50) CView.text("; even though you hide it, I've seen it lurking");
    CView.text(".  Feel like flying your colors for a while?</i>\"");

    // plus libido-based lust damage, output choices [Another Question][Let Her Fuck(req lust >=30)][Resist]
    player.stats.lustNoResist += player.stats.lib / 15;
...
function demonSheilaTalkAnotherQuestion(player: Character): NextScreenChoices {
    CView.clear();
    // (if lust <=99)
    if (player.stats.lust <= 99) {
        CView.text("Sheila reaches farther down, ");
        // (cock)
        if (player.body.cocks.length > 0) CView.text("slipping more of her finger and thumb around your penis");
        else if (player.body.vaginas.length > 0) CView.text("parting your lips more");
        else CView.text("rubbing your bare groin");
        CView.text(" even as she smiles encouragingly.  \"<i>Sure, mate.  Amuse yourself.  I certainly intend to.</i>\"");
...
function sheilaLetHerFuckYou(player: Character): NextScreenChoices {
    ...
    if (sheilaCorruption() >= 40) CView.text("pillowy ");
    else CView.text("pert ");
    CView.text("breasts against you.");
...
// Post-demon pregnancy notif (demon sheila = 1 and sheilapreg = 4)
// increment imp children counter?
function demonImpChildren(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>[name]!</i>\"  A swishing in the grass behind you is the only prelude you get to a nude Sheila pressing herself against you in a hug, ");
    // (sheila corruption < 40)
    if (sheilaCorruption() < 40) CView.text("her perky nipples rubbing into your arm as she snuggles up");
    else CView.text("her enormous, pendulous breasts practically parting around you to wrap you up alongside her arms");
    CView.text(".");
    ...
    // (if no cock)
    if (player.body.cocks.length <= 0) CView.text("  Or, had...</i>\" she finishes with a frown as she reaches your bare groin.  \"<i>Where'd you leave that magnificent old fella?  I was lookin' forward to another go.</i>\"  She pulls her hand out, then leans toward you with a pout.  \"<i>I'm so horny, too...</i>\"");
    // (if cock)
    else {
        CView.text("  Still, it's not the same as having you inside me...</i>\"  She squeezes the neck of your [cock] and then pulls her hand away, slipping it between her own legs.  \"<i>I was thinking you could help me make another, and maybe up the population of men worth a damn around here.</i>\"  Reclining, she spreads her thighs, pulling apart her small, smooth labia ");
        // [(sheila corruption <=80)]
        if (sheilaCorruption() <= 80) CView.text("with her fingers to make sure the invitation is all the way open");
        // (else)
        else CView.text("with her split tail while she cradles her massive tits to make sure you see the open invitation");
        CView.text(".  Impatiently, she rubs her stubby, drooling, phallic spade against the hole, accentuating the imagery.  You can feel the blood rushing to your groin as she gives herself shallow, teasing strokes.");
    }
    flagWomb.birth(); // Clear Pregnancy
    SheilaFlags.SHEILA_IMPS++;
    // plus lust if PC has cock
    if (player.body.cocks.length > 0) player.stats.lustNoResist += 10 + player.stats.lib / 10;

    // [Other Sex][Knock Up(cock only)][Leave]

    const choices: ScreenChoice[] = [];
    choices[0] = ["Other Sex", pregDemonSheilaOtherSex];
    if (player.body.cocks.length > 0) choices[1] = ["Knock Up", pregDemonSheilaKnockUp];
    // leave should be default spacebar option
    choices[4] = ["Leave", leavePregDemonSheila];

    return { choices };
}

// [Preg Demon Sheila - Leave]
function leavePregDemonSheila(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You shake your head at her, backing away.  She sulks a bit, then speaks.  \"<i>I guess I'll let you off, since you sexed me up so well before.  Next time, though, you're gonna give me some... satisfaction.</i>\"  She runs a finger teasingly down her chest, giving you half a smile.");
    // (if cock)
    if (player.body.cocks.length > 0) CView.text("  Your refusal doesn't stop her from consummating the affair herself by slipping the oozing spade past her entrance and thrusting vigorously.  \"<i>What?</i>\" she says, shaking you from your distracted stare.  \"<i>I'm always randy now, and you're not going to spend time with me, so... move along.  Maybe someone fun will find me and join in.</i>\"  She throws her head back as her slippery tail makes a particularly deep push, crying her pleasure carelessly to the glaring sun.  The wet schlicks of her self-love follow you until the magic of the realm brings you abruptly back to ");

    // plus more lust if cock, go to camp
    if (player.body.cocks.length > 0) player.stats.lust += 10;

    return { next: returnToCampUseOneHour };
}

// [Preg Demon Sheila - Other Sex]
function pregDemonSheilaOtherSex(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Her eyebrows knit as you propose an alternative.  ");
    // [(if cock)
    if (player.body.cocks.length > 0) {
        CView.text("\"<i>Well now, I think I get a say, don't I?  I was really hoping to have your seed inside me... it's practically worth bottling.</i>\"  She ");
        if (sheilaCorruption() >= 80) CView.text("heaves onto her feet, heavy breasts swaying");
        else CView.text("nimbly hops to her clawed feet");
        CView.text(", and runs her hands distractingly along the curves of her hips.  \"<i>You can do what you like with my body, love... if you can convince me.  Of course, I'll claim the same privilege!</i>\"  Sheila jumps at you, leaving no doubt what kind of 'convincing' she expects to do!");
        return CombatManager.beginBattle(player, new Sheila());
    }
    // (if no cock)
    else if (player.body.vaginas.length > 0) {
        ...
        return beatUpDemonSheila(player, new Sheila(), false);
    } else {
        return { next: returnToCampUseOneHour };
    }
}
...
function loseToSheila(player: Character, consensual: boolean = false): NextScreenChoices {
    CView.clear();
    // Loss - if PC manages to lose by HP somehow (overrides all other losses)
    if (player.stats.HP < 1 && !consensual) {
        CView.text("Your erstwhile opponent's eyes glimmer with excitement as you collapse from your injuries, and she runs over to you.  The demon strips off your [armor] eagerly, but you can't stay awake for the fun.  Consciousness slips away and you pass out.");
        // --Next--
        Time.hour += 8;
        if (Time.hour > 23) {
            Time.hour -= 24;
        ...
        return { next: loseToDemonSheila };
    }
    else {
        const choices: ClickFunction[] = [];
        // Loss - normal cocks get rode (for cockarea <= 56)
        if (player.body.cocks.length > 0 && player.body.cocks.find(Cock.CockThatFits(56)))
            choices.push(loseToNormalSheilaAndGetRidden);
        // Loss - tail-spade sounding for fuckhueg cock (for cockarea > 56)
        if (player.body.cocks.length > 0 && player.body.cocks.sort(Cock.Largest)[0].area > 56)
            choices.push(tailSpadeSoundingForFuckHugeDongsWithDemonSheila);
        // Loss - clit-swelling and clit-sounding or clit-anal
        if (player.body.vaginas.length > 0)
            choices.push(clitSwellingDemonSheilaClitSoundingAnal);
        // Loss - unsexed joke scene
        if (player.gender === 0)
            choices.push(aintGotNoGenderAndKangarooRaped);
        // RUN DAT SHIT, YO!
        return choices[randInt(choices.length)](player);
    }
}

function loseToDemonSheila(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Eight hours later, you come to, bruised and sore... and find yourself lying on the ground with your head in Sheila's lap; she's petting your [hair] absently.  \"<i>Awake, are you?</i>\" she grins, looking down at you.  \"<i>Where did you get all those wounds, anyway?  Ya know... you can ask for my help if the wildlife's giving you trouble - we'll just go back to my place and spend all our time in the nuddy.  Our imps can bring us food and drinks.  New playmates, too.  Want to come home with me?</i>\"");
    ...
    CView.text("\n\nStill shaken, you stand and let Sheila do the same.  She narrows her eyes at you before speaking again.  \"<i>Be more careful, mate.  The idea of my " + mf(player, "boy", "girl") + " being fucked by trash makes me ill.</i>\"  Saying nothing more, the woman turns and leaves you to walk back to your ");
...
// Loss - normal cocks get rode (for cockarea <= 56)
function loseToNormalSheilaAndGetRidden(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Sheila pushes you onto the ground");
    if (player.body.legs.isTaur()) CView.text(" and you're forced to put your forelegs down for support as you come to rest on your haunches");
    else CView.text(", spreading your legs.  The demon unfastens your [armor] and strips it from your bottoms easily");
    CView.text(", exposing your " + describeCocksLight(player) + ".");

    CView.text("\n\n");
    // [(multi)
    if (player.body.cocks.length > 1) CView.text("\"<i>All of these look ripper, but... I want this one today,</i>\" she says, selecting your [cockFit 56].  ");
    CView.text("She wraps her hand around it and begins pumping up and down, squeezing out a drop of precum that she smears onto her palm.  \"<i>You're so eager,</i>\" Sheila giggles.  \"<i>Are you already about to burst?  That's no good... I wanted to play a bit more.</i>\"");

    CView.text("\n\nThe demon releases your dick, watching raptly as it bobs and twitches, then scoots up to you and");
    if (player.body.legs.isTaur()) CView.text(", draping your forelegs over her shoulders,");
    CView.text(" presses her " + sheilaCup(player) + " breasts");
    if (sheilaCorruption() < 100) CView.text(" against you, dragging her nipples against yours.  ");
    else CView.text(" against you so forcefully that they part around your chest.  ");
    CView.text("\"<i>Such a " + mf(player, "handsome bloke", "pretty sheila") + ", with such a nice dong,</i>\" she whispers, \"<i>I'm going to have to take you for a ride.  Try not to come as soon as you get in!</i>\"");

    CView.text("\n\nSheila grins viciously and sinks down on your [cockFit 56], sliding it into her pussy at a painstaking pace");
    // [(multi)
    if (player.body.cocks.length > 0) CView.text(" and slipping the unused remainder of your manhood between her thighs");
    CView.text("; you can feel every new inch of heat as she lowers her body.  The woman moans, gyrating her hips, and " + describeOneOfYourCocks(player) + " twitches in painful erectness.");
    ...
    if (player.body.balls.count > 0) {
        ...
        // (not horse)
        if (!player.body.legs.isTaur()) CView.text("  Sheila takes your face in her hands and presses it into her breasts as she reassures you.");
        ...
        CView.text("\n\nSheila's pussy spasms around you and she looks into your eyes; you feel a wave of compulsion from them, and arousal flows into you from each place your " + describeSkin(player) + " touches her body.  Unable to withstand her orgasm-fueled power, your [balls] pull close to your shaft and you let go of your climax, grabbing her hips and dumping semen into her cunt");
        if (player.body.cocks.length > 1) CView.text(" and lap");
        CView.text(" just like she's dumping feelings into your head.  \"<i>Lover!</i>\" she screams.  \"<i>Pack me with your babies!</i>\"");
        ...
        if (player.body.legs.isTaur()) CView.text("forelegs");
        else CView.text("hands");
        CView.text(", displacing a glob of cum from her pussy that's soon followed by others as you ejaculate once more.  The jizz is so dense with sperm that it's thick, almost colloidal; it jiggles on her skin, barely sliding downward as your hips ram against hers.  It takes nearly ");
        // (normal skeet)
        if (player.cumQ() < 250) CView.text("ten minutes and two");
        // (big skeet)
        else if (player.cumQ() < 1000) CView.text("thirty minutes and four");
        else CView.text("fifty minutes and eight");
        CView.text(" more orgasms, but finally your balls empty of their demonically-induced load and your lover pulls away and stands up.");
        ...
        flagWomb.knockUp(new Pregnancy(PregnancyType.PLAYER, IncubationTime.SHIELA));
        player.body.balls.size++;
        if (player.body.balls.size < 10) player.body.balls.size++;
    ...
    // (else if no balls)
        ...
        if (player.body.legs.isTaur()) CView.text("You force her downward with your forelegs to");
        else CView.text("You grab her hips and");
        CView.text(" help her impale herself on your tool.");

        CView.text("\n\nThe demon shudders as your [sheath] bumps up against her vulva.  \"<i>Y-yeah, but I never paid attention to growing dongers without balls and that kinda arcane stuff, mate... didn't think I'd end up fucking a strange " + describeRace(player) + " like you until you stuck it in me.  I always wanted to marry a normal bloke like my dad.</i>\"");
        ...
        if (player.body.cocks.length > 1) CView.text(" and lap");
        // [(big skeet)
        if (player.cumQ() >= 1000) CView.text("; she quickly reaches capacity and pulls off, jerking your messy, slimy [cockFit 56] and letting you squirt the last of your strokes all over her face and chest");
        if (player.cumQ() >= 6000) CView.text(".  So much is left that she's practically underwater by the time you're finished; she giggles and bubbles form in the oozing liquid covering her lips");
        CView.text(".");
...
// Loss - tail-spade sounding for fuckhueg cock (for cockarea > 56)
function tailSpadeSoundingForFuckHugeDongsWithDemonSheila(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("The demon opens your [armor] and undresses you, inspecting your crotch.  \"<i>Going a bit overboard, aren't you, [name]?</i>\" she taunts, rubbing your [cock biggest] with her knee.  \"<i>I'm not quite magical enough to take this monster - maybe after a few months of fucking everything I met, I'd be that loose.</i>\"  Sheila giggles at you.");
    ...
    if (player.body.cocks.length > 1) CView.text("; unguided, the ");
    if (player.body.cocks.length === 2) CView.text("other twitches and sprays");
    else if (player.body.cocks.length > 2) CView.text("others twitch and spray");
    if (player.body.cocks.length > 1) CView.text(" all over, coating her back, you, the grass, and anything ");
    if (player.body.cocks.length > 2) CView.text("else they happen to point at");
    if (player.body.cocks.length === 2) CView.text("else it happens to point at");
    CView.text(".  Your hips automatically try to push your dick further into her warm hole but only succeed in unbalancing her, forcing the demon onto her hands.  As your climax winds down, your head leans back and your cock slips from her vulva, squirting the last stroke onto her ass.");
...
// Loss - clit-swelling and clit-sounding or clit-anal
function clitSwellingDemonSheilaClitSoundingAnal(player: Character): NextScreenChoices {
    CView.clear();
    // (no horse)
    if (!player.body.legs.isTaur()) CView.text("Sheila places her hand in the small of your back and leans in, drawing closer to your face even as she pulls your waist inward.");
    else CView.text("Sheila sets her hand atop your withers, pressing down gently as she looks into your eyes.");
    CView.text("  \"<i>Why don't you relax, [name]?  Lie down and let me take care of you...</i>\"  Her eyelids lower and her mouth spreads in a sly smile, and for just a moment you lose your nerve, looking into her devilish expression.  \"<i>Don't worry, love,</i>\" she says, straightening her face.  \"<i>I promise you'll enjoy this.</i>\"");

    CView.text("\n\nBefogged but still reserved, you nonetheless allow Sheila to lower you to the grass and remove your [armor].  She ");
    // [(not horse)
    if (!player.body.legs.isTaur()) CView.text("turns around, then straddles your groin and looks over her shoulder.");
    // (Equus ferus caballus)
    else CView.text("circles you until she reaches your rump, then turns around and kneels down herself, looking at you over her shoulder.");
    CView.text("  Her black, spaded tail lifts from between her thighs and snakes toward your pussy,");
    // [(if manparts)
    if (player.body.cocks.length > 0) {
        CView.text(" caressing ");
        if (player.body.balls.count > 0) CView.text("your [balls]");
        else CView.text("the underside of your [cock biggest], before");
    }
    CView.text(" forcing your lips apart and lingering on your rapidly-swelling " + describeClit(player) + ".  \"<i>Oh, my.  What's with this ");
    if (player.body.clit.length <= 6) CView.text("lewd little");
    else CView.text("grotesque");
    CView.text(" thing doing growing from a pretty girl like you?  It's just the perfect size for some fun.</i>\"");

    CView.text("\n\nYour eyes open in alarm at the same time as her mouth does in pleasure; she jams the slit of her spade onto your button");
    if (player.body.clit.length > 6) CView.text(", taking it several inches deep");
    CView.text(".  Your head swims as pleasure from your engulfed clitoris overwhelms you, then toes the line to genuine pain.  The copious demonic fluid backing up inside her tail begins to leak around the tip, coating your clit and drooling onto your labia; your muscles tighten as the heat escalates past comfortable levels, and you begin shifting violently, trying to pull your swollen chick-stick free from the demon's anatomy.  \"<i>Ah, ah, god, [name], stop!  Don't thrash so!  You're teasing me too fast!</i>\"  Sheila's words are lost on you, sunken as you are into a hell of oversensitized skin and constant stimulation.  It feels like the nerves exposed to the torment are even multiplying, as if skin you weren't even feeling before suddenly appears to be immersed into the tainted slime.");
    CView.text("\n\nYour eyes roll in temporary shock as her tail squeezes around your " + describeClit(player) + " tighter and tighter; the siege lifts slightly, allowing you to focus with supreme effort and see that the demon is similarly affected!  Her head sags between her shoulders, even as she places two hands on the ground in front of her to steady herself.  The two tails on her butt whip spastically, squeezing another wave of painful pleasure into your crotch.  Staring, you can only watch as Sheila's thrashing black spade describes a clipped arc, a long, thick rod of flesh growing out of it to join your bodies.");

    // [(<=6")
    if (player.body.clit.length <= 6) CView.text("\n\nAwareness comes to you slowly; that flesh is your clitoris, gratuitously mutated by Sheila's fluids!");
    else CView.text("Though you're by now accustomed to the sight of your clitoris, more like a cock than a dainty feminine bump, this is something else altogether!  The huge thing poking into her tail is almost twice the thickness of the slim growth you remember and bulges with blood vessels; as you watch it, transfixed, a trickle of Sheila's oily, dark secretion slides down it, then is absorbed into some flesh near your groin, adding another layer of width with a pulse!");
    CView.text("  \"<i>Fuck, it's so tight!</i>\" the demon gasps, now resting on her elbows with her chest and face pressed to the grass.  Her tail jerks again, yanking your stalk this way and that and releasing more of her backed-up fluids.  They slide all the way to the bottom of your rod before sinking into your skin.");
    ...
    // (clit<=6" initially)
    if (player.body.clit.length <= 6) {
        CView.text("\n\nShe sits up on her knees, then turns to face you.  ");
        if (!player.body.legs.isTaur()) CView.text("Taking your face into her hand, she closes your jaw and then leans in to plant a kiss on your chin.");
        else CView.text("Her eyes glitter with mirth.");
        CView.text("  \"<i>Why the look, mate?  Seems like you're saturated for now, and just in time.  This... is...</i>\"  She pulls her tail up from the end of your clitoris, then slides it back down, slowly.  \"<i>... perfect!</i>\"  As her tight hole swallows your sensitive distaff staff once again, your eyes roll and her head drops, dragging her hair");
        // [(horse)
        if (player.body.legs.isTaur()) CView.text(" through the grass");
        else if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 1) CView.text(" between your " + describeAllBreasts(player));
        else CView.text(" along your flat chest");
        CView.text(".  \"<i>Ffuck, even your girl parts are amazing.</i>\"  Sheila plants kiss after kiss on your ");
        if (!player.body.legs.isTaur()) CView.text("[chest]");
        else CView.text(describeButt(player));
        CView.text(" as she shallowly pumps your " + describeClit(player) + " in and out of her spade.  Your pelvis twitches uselessly; with her tail waving in the air instead of braced against her, controlling the pace of the fuck is beyond you.  \"<i>Naughty sheila,</i>\" she says, squeezing your wiggling hip with a hand.  You think privately that she could just as easily be talking about herself.  \"<i>Am I not going fast enough for you?  Let me speed up.</i>\"");
        ...
        // [(PC corr<50)
        if (player.stats.cor < 50) {
            CView.text("her tail quakes and you feel something hot licking at the tip of your clitoris.  The spade pulls back and then sinks down again, and the heat gains several inches; she repeats this twice more until you can feel what must be her cum all the way down to her tail-tip, but as she attempts to pull back again and force her climax past the raw, hot skin at the edge, her tail simply jerks at your clit and she falls weakly onto the ground.");

            CView.text("\n\n\"<i>N-no... no!  So close!</i>\"  The black stalk thrashes weakly as her body loses muscle control everywhere except the parts responsible for forcing her cum out of it.  She moans piteously and babbles as her tail swells around your clit and engulfs you in her liquid climax, doubling in thickness behind the spade from the mounting fluids.  It's too much for you to watch, this arrogant woman trying to use you to get off but only managing to tease herself while suffusing your clit with her hot fluid, and combined with the temperature, pushes you over the edge you'd been flirting with.  Your " + describeVagina(player, player.body.vaginas.get(0)) + " squeezes and squirts, depositing your own orgasm onto the ");
            if (player.body.vaginas.get(0).wetness >= 4) CView.text("demon's legs");
            else CView.text("grass");
            CView.text(", and your clit swells even further inside your tormented partner.  Sheila groans, \"<i>Fuck you, [name]...</i>\"");

            CView.text("\n\nIrritated, she wraps her other frantic tail around the still-straining black one and squeezes, finally managing to get some of her climax past the ring of tight skin at the end; with the fluid forming a bridge, you feel the tension release as a wave of her black, slippery tail-cum forces her slit wide open and gushes out to soak your clit and then your pussy underneath.  \"<i>Ah!  Ah!  Finally!</i>\" she cries, great gouts of agitated slime sliding down your " + describeClit(player) + " to bubble and lick at your pussy, as if trying to get inside.");
        }
        // (else PC corr>=50)
        else {
            CView.text("her tail quakes, then pushes her pussy into yours.  \"<i>God, yes!</i>\"  A spatter of hot fluid hits your " + describeVagina(player, player.body.vaginas.get(0)) + " as your demon lover climaxes, and another begins to drool around the tip of your clit, still suspended inside her writhing tail.  The sudden labial heat is too much for you, and you answer her with a climax of your own, ");
            if (player.body.vaginas.get(0).wetness >= 4) CView.text("drizzling");
            else CView.text("copiously soaking");
            CView.text(" her pussy with your own girl-jizz.  It feels like the nearness of her own cunt pulls your orgasm into obscene lengths, and you come repeatedly, drooling more fluid onto her wanton pussy as she grinds its lips against yours.  She rubs her breasts as she strokes her convulsing vagina against your own, pinching and pulling them until you swear you can see them lengthen.");
        }
    ...
    // (else if clit >6" initially)
        ...
        // [(horse)
        if (player.body.legs.isTaur()) CView.text(" and rolls onto her back");
        CView.text(", then wraps her tails around your thickened clit.  They twist and pull, sending spirals of pleasure along the numerous nerves, and begin gently but insistently tugging you toward her tan butt.  Sheila directs the tip right between her cheeks, and you can feel her using it to rub the ring around her hole.  \"<i>I think this should be about the right size for you, love,</i>\" she teases, spearing herself on your " + describeClit(player) + ".  Inch after inch slides into her slowly, maddening you with warmth, until ");
        if (player.body.clit.length <= 12) CView.text("all of it is in her bowels and her hot cunt rests against yours");
        else CView.text("she's taken as much as she can");
        CView.text(".  She half-smiles and half-winces at you.  \"<i>Don't usually take it in this hole, but I wanted to feel your pussy going crazy for me.</i>\"  ");
        if (player.body.clit.length <= 12) CView.text("Her hips shift, dragging her lips across yours and stirring your clit through her asshole even as it squeezes you; your pussy twitches at the stimulation.");
        else {
            CView.text("Her tails, one still swollen from being stretched and tormented by your growing clit, slip into you abruptly, bringing their mild burning sensation with them; if you hadn't been saturated with it already, you'd probably pass out.");
            // [(if virgin vagina)
            if (player.body.vaginas.find(Vagina.Virgin)) CView.text("  \"<i>What's this?</i>\" Sheila teases.  \"<i>Why, your virtue is still intact, little girl!  I wouldn't want anything to happen to it!</i>\"  She pulls her tails out a bit, contenting herself with simply teasing your lips and driving you mad for penetration.");
            CView.text("  The demon shifts around your clit, sending a wave through you that makes your lips wrap around the invaders.");
        }

        CView.text("\n\nSheila places her long-taloned feet against ");
        // (not naga)
        if (!player.body.legs.isNaga()) CView.text("your thighs, spreading your [legs] wide and then hooking them over top to touch ");
        CView.text("the ground next to your ass, and uses the leverage to begin pumping your clit out of and into her asshole, caressing your " + describeVagina(player, player.body.vaginas.get(0)) + " every time she slides home.  \"<i>I could get used to this, love.  What say I keep you dosed up like this and you become my dildo?  I'll ride you with a different hole every time!</i>\"  Your eyes roll back in your head, at least partially from the irregular pace at which she's stroking your clit.  Sheila grabs her breasts as she slows down drastically once again, mashing them with her hands and sinking onto you an inch at a time.  \"<i>Fuck... I love your junk, [name].  Maybe I should catch a goblin, too, and make her tease my pussy while you fill my ass.</i>\"  She raises back up and ");
        // [(clit>12")
        if (player.body.clit.length > 12) CView.text("pulls her black tail free, dripping with your juice, then ");
        CView.text("begins to work her drooling spade along your clit again, smearing her goo along your chunky chick-stick.  You can feel the edges of the stretched hole in her tip working on your skin, opening and closing ever so slightly as her copious tainted lube oozes out.  Your " + describeClit(player) + " must be completely saturated already, because the slippery goo lingers on the surface instead of wicking into it.  \"<i>Gonna g-get you nice and wet, love...  Ohh,</i>\" Sheila says, affected to distraction by the pleasure of tracing the slick end of her stubby tail along you.");

        CView.text("\n\nFinally coating you to satisfaction with the oily secretions, Sheila begins pumping again.  The heated friction from before evaporates, replaced with a dull, distributed warmth as she slides her asshole along your clit.  With less resistance, she's able to up the tempo to frantic, scraping along the ground so fast that she begins to tear up the grass.  You bear up as much as you can under the onslaught of pumping and gyrating hips, but eventually the sensation is too much, and your nerves take over as your climax begins.  Your " + describeVagina(player, player.body.vaginas.get(0)) + " spasms, drooling onto her ");
        if (player.body.clit.length <= 12) CView.text("pussy; her smooth, small labia greedily lap at the fluid when they touch, brushing against yours and drawing further twitches and squirts.");
        else CView.text("teasing tails; they shift from stroking your labia to holding them open, allowing your pussy to squirt as it pleases.");
        CView.text("  Suddenly she stops mid-stroke as her asshole quivers, sending shivers up your orgasm-sensitized clitoris.  \"<i>Fffuck!  So soon!  I'm coming too, [name]!</i>\"  Sheila sinks down, burying your " + describeClit(player) + " into her asshole.");
        // (clit>12")
        if (player.body.clit.length <= 12) {
            CView.text("  Her tails sink into you as she squirms, parting your lips and pushing their way inside.");
            // [output hymen check, but don't remove virgin flag yet]
            if (player.body.vaginas.find(Vagina.Virgin)) CView.text("  <b>You've lost your virginity!</b>");
            CView.text("  Buried in your pussy, the spade begins drooling its black fluids in greater and greater volume as Sheila is wracked by her orgasm.");
        }

        // (PC corr<50)
        if (player.stats.cor < 50) {
            CView.text("  The demon's pussy gushes and pours with unnatural levels of fluids, soaking your crotch as a miniature cataract makes its way out of her lower lips.  Sheila spills herself onto you for at least a minute, producing more hot fluid than you thought possible, until she looks utterly dehydrated - even her breasts seem to be smaller and less jiggly.");
        }
        else CView.text("  The demon's skin seems to parch as she grinds into you relentlessly, evaporating your fluids so fast that most of them don't even have time to fall.  Her hot flesh lays heavily across your netherlips, prolonging your climax and coaxing out more fem-spray than you thought was in you.  Even your mouth seems to dry as she drinks your essence up.");
        ...
        if (player.body.clit.length > 12 && player.body.vaginas.find(Vagina.Virgin)) {
            CView.text("  \"<i>Sorry about your hymen, love, but I just couldn't resist making a sweet little slut like you into my girl.  I hope you'll remember me - I only wish I had a proper donger so I could give you my baby, too.</i>\"");
        }
        CView.text("  Even as tired as you are, what you see on the demon girl makes you laugh.  She starts at this, then follows the direction of your gaze; ");
        if (!player.body.legs.isTaur()) CView.text("her tits are covered by grass stains from her frenetic pumping.");
        else CView.text("craning around shows her that her asscheeks are completely green and wet with torn grass from slamming into you so violently and repeatedly.");
        CView.text("  \"<i>... Oh, god dammit,</i>\" Sheila gripes.  She walks over and forces your face into them with a long, slow shake, then giggles as a joke comes to her.  \"<i>There, [name]; I've stained you with my colors!</i>\"  She plants a long kiss on your grass-smeared forehead, then drops you back to the ground, straightens up, and departs.  Exhausted, you slip into a doze, wondering if you'll be able to fit your swollen clit back into your clothing when you wake or if you'll have to walk back to camp with it sticking out.  It slowly deflates as you sleep, but does retain a bit of additional length.");
    }
    ...
    if (player.body.clit.length > 12 || player.stats.cor < 50) {
        displayStretchVagina(player, 12, false, false, false);
        player.slimeFeed();
    }
    player.body.clit.length++;
    if (player.perks.has(PerkType.BigClit)) player.body.clit.length += .5;
    if (player.stats.cor < 50) {
        player.stats.cor += 10;
...
// Loss - unsexed joke scene
function aintGotNoGenderAndKangarooRaped(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Sheila sashays over to you and brazenly sticks her hand in your featureless crotch.  \"<i>It's too bad you've mutilated yourself like this, love... or did you do it just to tease me?  You know, it doesn't bug me that much.</i>\"");
    ...
    player.stats.lustNoResist += 20 + player.stats.lib / 4;

    return { next: returnToCampUseOneHour };
}

// Win against demon Sheila
function beatUpDemonSheila(player: Character, monster: Character, output: boolean): NextScreenChoices {
    if (output) {
        CView.clear();
        CView.text("\"<i>Ahh, you win, mate.  I give up.</i>\"  Sheila slumps to the ground, ");
        if (monster.stats.HP < 1) CView.text("and her wounds begin to close with a soft lilac glow.  \"<i>So fuckin' strong.  I don't suppose you've got any sperm for me, do you?  I'd love to have some of your kids...</i>\"  She looks at you beseechingly, gently fingering herself.");
        // (lust)
        else CView.text("frigging herself vigorously.  \"<i>Do with me what you will, lover; those were the terms.  Won't you take my body to satisfy yourself, you brute?</i>\"  The audacious woman sticks her tongue out, as if to goad you into treating her roughly.");
    }
    // [(lust < 30)]
    if (player.stats.lust <= 33 && output) {
    ...
    if (player.body.cocks.length > 0 && player.body.cocks.find(Cock.CockThatFits(56))) choices[0] = ["Missionary", missionaryForThePurposesOfCreatingImpsWithSheila];
    ...
    if (player.body.cocks.length > 0 && player.body.cocks.find(Cock.CockThatFits(56))) choices[1] = ["AnalHateFuck", analHateFuck];
    // Win - [Big Dick+Thighs] (usable for dicks with cockarea > 56) (all 10 of them)
    // penis envy, go!
    if (player.body.cocks.length > 0 && player.body.cocks.sort(Cock.Largest)[0].area > 56) choices[2] = ["UseHerThighs", bigDickAndThighs];
    // Win - [Penetration, in Spades] - for self-loathing vaginas
    if (player.body.vaginas.length > 0) choices[3] = ["Clit-Fuck", winAgainstDemoNSheilaForVaginas];
...
// Win - [Missionary] - for the purpose of procreation
// requires a cockarea <= 56
function missionaryForThePurposesOfCreatingImpsWithSheila(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You unfasten your [armor] and strip down completely, as Sheila's eyes drink up your bare flesh.  Denuded, you caress your cock with several strokes, producing a drop of precum.");
    ...
    if (player.stats.cor < 40) CView.text("Your ardor subsides a bit at her performance, but seeing your look of contrition, ");
    else CView.text("Your scowl hardens at her shameless playacting, and ");
    CView.text("she breaks into laughter.  \"<i>I'm just having you on, love.  You know my body's yours for the using.</i>\"");
    ...
    if (!player.body.legs.isTaur()) CView.text("shoulders");
    else CView.text("flanks");
    CView.text(" and her pupils dance in excitement; for a moment you even lose yourself in her enthusiasm, forgetting her demonhood and seeing only an earnest, adoring girl under you.  \"<i>Now, push!</i>\"");
    ...
    // (sheila corruption >=40)
    if (sheilaCorruption() >= 40) CView.text("and her soft breasts jiggle ");
    CView.text("as a twinge of sensation from your thrust provokes a gasp and forces her to reposition herself, to better find it again.  \"<i>That's the spot!  Your dick is the <b>best!</b>  Pound me, you fucking cum-hose!</i>\"");
    ...
    if (!player.body.legs.isTaur()) CView.text("arms");
    else CView.text("forelegs");
    CView.text(" wobble and it takes all your focus not to collapse onto the woman and melt into a boneless puddle; there's little left to prevent your twitching hips from granting her request, and you slam your pelvis into hers repeatedly, provoking delighted cries from your shameless partner.  \"<i>Ohh, yes!  Fill me with your baby batter, asshole!</i>\"  Sheila wraps her arms and legs around your back");
    if (player.body.legs.isTaur()) {
        CView.text(", practically lifting herself off of the ground] as she impales her cunt on your [cockFit 56] to prevent you from pulling out.");
    }
    // [(silly)
    if (User.settings.silly()) CView.text("  \"<i>Make me feel like I'm fifteen years old again and playin' with the dog-morphs in the field!</i>\"  Heavens to Betsy!");
    else CView.text("  \"<i>This is what you're here for!  Don't spill a single drop!</i>\"");
    ...
    if (sheilaCorruption() < 40) CView.text("tight");
    else CView.text("soft");
    CView.text(" chest against you");
    if (player.body.legs.isTaur()) CView.text(" as you slump forward");
    CView.text(".  Your cock ");
    if (player.body.cocks.length > 1) {
        CView.text(" and its ");
        if (player.body.cocks.length === 2) CView.text("brother");
        else CView.text("brethren");
        CView.text(" swell and squirt");
    }
    else CView.text("swells and squirts");
    CView.text(", pouring your semen into Sheila's ravenous pussy");
    if (player.body.cocks.length > 1) CView.text(" and coating her crotch with a glaze of white");
    CView.text("; her eyes glow and it briefly feels as though her body is sucking the ejaculate right out of you");
    if (player.cumQ() >= 500) CView.text(", until she fills up and the rest of your orgasm is drooled out at its own lazy pace");
    CView.text(".  She kisses you");
    if (player.body.legs.isTaur()) CView.text("r forelegs");
    CView.text(" lovingly and deeply");
    if (player.body.cocks.length > 1 || player.cumQ() >= 500) CView.text(" as the spare seed dribbles from her crotch");
    CView.text(", then breaks away and looks up into your face.");

    CView.text("\n\n\"<i>[name], you ");
    if (player.cumQ() >= 500 || player.body.cocks.length > 1) CView.text("spilled some.  I can't have your baby if you're fucking the grass.  And you ");
    CView.text("came too soon; I wasn't done.</i>\"");
    ...
    // (else if RNG doesn't end scene)
        ...
        // (if vag)
        if (player.body.vaginas.length > 0) CView.text(" as her imp spawn take turns doing the same to you from behind");
        else CView.text(" as her imp spawn gather around, stroking themselves and occasionally squirting their hot, tainted cum on her body or yours as orgasm takes them");
        CView.text(".  ");
        if (player.stats.lib < 40) CView.text("Shocked");
        else CView.text("Flushing hotly");
        CView.text(", you look away and break the connection, but Sheila's smile only widens.");
        ...
        // [(PC corr <50)
        if (player.stats.cor < 50) CView.text("Her pussy convulses as she orgasms, and you feel a wave of heat overtaking your [cock biggest], starting at the tip and moving down the shaft.  As it reaches the base, a gush of her fluids breaches the seal her pussy makes around you, forcing your jizz out along with it in a mixed white and clear squirt.  The demon's vagina squeezes and drools for half a minute, pouring her tainted liquid love out against your tightly-held body until your crotch is soaked with her sticky heat and responds with its own.");
        else CView.text("Her pussy sucks you in as she climaxes, and you can feel larger and larger waves of precum being pulled from your shaft; it almost feels like she's tugging at your entire being.  It's not long before your cock twitches and begins giving her greedy sex a second helping of your semen.");
        CView.text("  She goes limp as her twitching cunt takes over and begins drinking every drop it can get from your dick to replace the goo you pushed out");
        if (player.cumQ() >= 500) CView.text("; you nearly manage to fit it all into her this time, with your production still flagging from the last session");
        CView.text(".  Muscles weakening again, you lower yourself onto her once more and allow her to stroke your ");
        if (!player.body.legs.isTaur()) CView.text("back");
        else CView.text("sides");
        CView.text(" as she cools down.");
        ...
        player.slimeFeed();
        if (player.stats.cor < 50) player.slimeFeed();
        if (player.stats.cor < 50) {
            player.stats.cor += 10;
...
// Win - [Anal Hate-fuck] - peg-knocking with optional corr >=80 break-up (for your cock or C. Jojo's)
// if PC has both a dick and corrupted Jojo, output this scene and choice; else skip this output and go right to appropriate option
function analHateFuck(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Fucking bitch, playing with herself like nothing's the matter and you didn't just show her concrete proof of your superiority.  Part of you feels like punishing the insolent woman yourself, pounding her insides and dumping your load without a whit of regard for her own pleasure... but another part is thinking it might be fun to see her completely debased, to hammer home how pathetic she is - something like reducing her to a cock-sleeve for your personal cum-slave.");

    // [Tear Her Up][Call Jojo]

    const choices: ScreenChoice[] = [];
    choices[0] = ["TearHerUp", sheilaAnalHateFuckAGoGo];
    if (campCorruptJojo()) choices[1] = ["Call Jojo", analHateFucksWithJojo];
...
function sheilaAnalHateFuckAGoGo(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Your [cockFit 56] gets hard in your [armor] as you watch her diddle her pussy, but you set your jaw to resist giving her what she wants - a better idea is taking shape in your head.  \"<i>Turn around,</i>\" you demand.");

    CView.text("\n\nSheila bites her lip and blushes, then turns her back to you, pressing her chest into the ground and waving her ass in the air, twin tails bobbing over her round cheeks and drooling, eager cunt.  \"<i>Heehee, I'm ready for you, mate... look how wet I am!</i>\"  You strip, releasing your " + describeCocksLight(player) + ", and approach.  She hums in anticipation as you rub against her smooth labia, smearing her moisture onto your [cockHeadFit 56].  \"<i>Come on; time to put it in already!  Don't tease me!</i>\"");

    CView.text("\n\nYour smile widens, unseen, as you silently agree.  ");
    if (!player.body.legs.isTaur()) CView.text("Slipping one hand around your [cockFit 32]");
    else CView.text("Straddling her");
    CView.text(", you pull your hips back and line the [cockHeadFit 32] up, then thrust - right into her asshole.  Her head raises and her teeth grit as you force the slick tip past her ring.  \"<i>[name], what the fuck?!  That hurts!  Go easy if you're going to use that hole; I'm not good at stretching my body yet!</i>\"");

    CView.text("\n\n\"<i>No.</i>\"  You ");
    // (not centaur)
    if (!player.body.legs.isTaur()) CView.text("grab her thrashing legs, using them for leverage as you ");
    CView.text("force your dick into the angry woman.  \"<i>You talk a good game about how you love being full of cock, but when I shove one in you, you get all bitchy and act like you're not ready; why don't you just shut up and get fucked, slut?</i>\"");
    ...
    if (sheilaCorruption() >= 90) CView.text("; her enormous, cushiony breasts spread under her as your prick forces her weight heavily onto them");
    CView.text(".  She pulls up tufts and pounds the earth impotently with her fists while you sink in as far as you can.  \"<i>Fuck you!  What the fuck is wrong with you, arsehole?!  Let me go, I don't want anything to do with you anymore!</i>\"");
    ...
    if (player.stats.cor >= 80 && player.effects.has(StatusEffectType.Infested)) {

        // [No][Worms Suit You], else auto-output text from [No]
        return {
            choices: [
                ["No", sheilaAnalHateFuckAGoGoNO],
                ["WormsSuitYou", sheilaAnalHateFuckAGoGoGETYOUSOMEWORMS],
            ]
        };
    }
    else {
        ...
        // see above
        return { next: sheilaAnalHateFuckAGoGoNO };
    }
}

// [Demon Victory Sex - Anal Hate-fuck - Tear Her Up - No]
function sheilaAnalHateFuckAGoGoNO(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Go to hell.</i>\"  You bury yourself in the demon's ass as your dick begins to spasm.  Her tails writhe as the first trickle sinks into her.  \"<i>What?  Say something, I dare you.</i>\"");
    ...
    if (player.cumQ() >= 500) CView.text("  Your overproduction floods her insides, and Sheila groans as it begins to leak back out, drooling over her pussy.  \"<i>Hey, you got your wish,</i>\" you tease.  The wanton woman tries to tilt her ass higher, to tip more of your overflow into her cunt, but you laugh and slap her hard on the hip, sending her to one knee again.");
    ...
    if (player.cumQ() < 250) CView.text("trickle");
    else if (player.cumQ() < 1000) CView.text("stream");
    else CView.text("waterfall");
    CView.text(" of semen made when her body pushes out some of the foreign substance.");
...
function sheilaAnalHateFuckAGoGoGETYOUSOMEWORMS(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Mmm...</i>\" you answer, noncommittally.  Your ");
    if (player.body.balls.count > 0) CView.text("[balls]");
    else CView.text("insides");
    CView.text(" squirm, giving you pause, and you pop your [cockFit 56] out.  \"<i>Actually... I don't think I suit you at all.</i>\"");
    ...
    CView.text(" your ");
    if (player.body.balls.count > 0) CView.text("[sack]");
    else CView.text("[cockFit 56]");
    CView.text(", you can actually feel your pent-up pets, eager to be let out to play.  \"<i>No,</i>\" you say, \"<i>I know what's more your speed.  Trash like you should be cast down into the dirt, and should keep company with the creatures of it.</i>\"  Sheila seems alarmed, but as you slip into her grasping pussy, she forgets her question, grinding her hips into you to milk you dry.  You focus, forcing a wave of slimy annelids down your shaft.  \"<i>Here come your new babies!</i>\"");
    ...
    player.effects.remove(StatusEffectType.Infested);
    player.orgasm();
    player.stats.cor += 10;

    player.stats.lust += player.stats.lib / 5;

    SheilaFlags.SHEILA_DISABLED = 3;
    return { next: returnToCampUseOneHour };
}

// [Demon Victory Sex - Anal Hate-fuck - Call Jojo] - corrupted mouse version:
function analHateFucksWithJojo(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Watching her sit there and play with herself like nothing's the matter and you didn't just demonstrate her weakness for all the world to see touches a nerve deep inside you.  You scowl and focus your thoughts to call your mousey ex-monk, bent on showing this demon how a proper mendicant should behave.");
    ...
    CView.text("\n\n\"<i>Haha...</i>\"  Sheila composes her expression into one of mock-gravity, and recites, \"<i>Please, [master], may I have your minion's semen in my pussy, to make you a baby?</i>\"  Her eyes are fairly lit at the prospect; it's evident she regards this as no more than a ");
    ...
    if (sheilaCorruption() < 40) CView.text("\"<i>What's with these tiny titties?  Could any man possibly be aroused by your bug bites?  I've seen bigger breasts on incubi!</i>\"");
    else CView.text("\"<i>These disgusting sacks of fat on your chest... are you under the impression that men like giant stretch marks and nipples that hang to your knees?  Why don't you get in shape, you messy bitch?</i>\"");
    ...
    if (player.stats.cor >= 80) {
        ...
        const choices: ScreenChoice[] = [];
        choices[0] = ["Next", partial(analHateFucksWithJojoNo, player, true)];
        CView.text("\n\n<b>You could really ruin them, but you'd probably never see Jojo again.</b>");
        choices[1] = ["Ruin Them", partial(jojoRuinsTheAnalHateFuck, player, true)];

        return { choices };
    }
    else {
        return analHateFucksWithJojoNo(player, false);
    }
}

// [Demon Victory Sex - Anal Hate-fuck - Call Jojo - No]
function analHateFucksWithJojoNo(player: Character, clear: boolean): NextScreenChoices {
    ...
    player.stats.lust += player.stats.lib / 3;
    player.stats.cor += 2;
...
function jojoRuinsTheAnalHateFuck(player: Character, clear: boolean = true): NextScreenChoices {
    if (clear) CView.clear();
    else CView.text("\n\n");
    CView.text("Your mouth spreads in a sickly smile as you contemplate granting her request anyway.  \"<i>Jojo...</i>\"");
    ...
    if (sheilaCorruption() >= 90) CView.text(", mashing her tits together as they thicken");
    CView.text(".  The mouse himself seems to be growing, in fact.  His feet kick and thrash, sprouting dangerous-looking nails as his legs stretch prodigiously.  As his body mass increases, his white-furred skin begins to burst and peel like old rags, revealing new, dark flesh underneath.");
    ...
    SheilaFlags.SHEILA_DISABLED = 4;
    SheilaFlags.JOJO_DEAD_OR_GONE = 1;
    return { next: returnToCampUseOneHour };
}

// Win - [Big Dick+Thighs] (usable for dicks with cockarea > 56) (all 10 of them)
// penis envy, go!
function bigDickAndThighs(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("As Sheila diddles herself, you ");
    // [(not horse)
    if (!player.body.legs.isTaur()) {
        CView.text("doff your [armor]; the demon's eyebrows climb as you haul out " + describeOneOfYourCocks(player));
        if (player.body.cocks.length > 2) CView.text(" one at a time, practically stacking them atop each other");
        CView.text(".");
    }
    // (horse)
    else {
        CView.text("slowly draw nearer to her, your " + describeCocksLight(player) + " bobbing with each step.");
    }
    ...
    if (player.stats.cor < 40) {
        ...
        CView.text(" toward her and aim underneath her cunt to rub your [cockHead biggest] against her palm, smearing a glob of precum on it.  You can see her shivering wordlessly, sliding her fingers together to play with the hot, clingy fluid, making translucent webbing between them.");
        // [(multi)
        if (player.body.cocks.length > 1) {
            if (player.body.cocks.length === 2) CView.text("  Both");
            else CView.text("  All");
            CView.text(" of them... slide them here, love,</i>\" the demon insists.  You line the remainder of your manhood up and force it between her thighs as well.");
        }
        CView.text("  She takes the tip of your [cock biggest] in her hand and pulls gently, coaxing your forward; you close the distance until " + describeOneOfYourCocks(player) + " is peeking through her legs and your crotch is pressed against hers.");

        CView.text("\n\n\"<i>I'm going to borrow a bit of your sensation...</i>\" Sheila announces quietly.  ");
        // [(not horse and not < 5"0)
        if (!player.body.legs.isTaur() && player.body.tallness >= 60) CView.text("She unbends and kisses you on the cheek, and your face feels a bit fuzzy where her lips touch.  ");
        ...
        CView.text("\n\n\"<i>We'll see who finishes first.</i>\"  Resting herself against your " + describeCocksLight(player) + " for support, she brings her other hand up and touches you; almost the full sensation is there this time, and you sign in happiness.  Sheila begins stroking with the new hand as well as the old, grabbing ");
        if (player.body.cocks.length === 1) CView.text("your dick with both");
        else CView.text("a prick in each");
        CView.text(" and angling upward so the precum drools downward and lubes her fingers.");
    }
    // (else corr >= 40)
        ...
        if (player.body.cocks.length > 1) {
            ...
            if (player.body.cocks.length === 2) CView.text(", you slide it through her legs between your first cock and her cunt, plowing through her entrance like a furrow; she quakes and you can feel it contracting against your skin.  ");
            else CView.text("s, you push them through the woman's legs one by one, smearing warm precum on her thighs as you do; her pussy squeezes against the top of your [cock biggest] as you coat her skin with your fluids.  ");
        }
        else CView.text("\n\n");
        CView.text("Sheila scoots backward, choking up on monster cock until her ass is pressed against your crotch, and then wraps her hands around and begins to masturbate you, using your drooling fluid and her own wetness for lubrication.");

        CView.text("\n\nShe seems to get off on it as much as you do; as she strokes, her tongue hangs out slightly and she begins rubbing your " + describeCocksLight(player) + " between her " + sheilaCup(player) + " breasts, smearing your secretions down her entire torso.  Her skin is hot and feels good, but not as vivid as you'd expect.  The demon's pussy contracts wildly again, despite being at rest against your shaft, and you come to a realization: she's actually siphoning the pleasure from you!");
    }
    // end corr split
    CView.text("\n\nThe woman quickly becomes lost in her own little world, searching out the nerves on your prick");
    if (player.body.cocks.length > 1) CView.text("s");
    CView.text(" and teasing each one; you thrust weakly into her hand, helping guide her hands, but your lonely tip wants for stimulation.  As the feeling becomes so acute that you prepare to say something, Sheila uncannily anticipates you and scoots her pussy along your shaft, popping your [cockHead biggest] into her mouth.  Her tongue dabs at your slit, tasting your fluid, and her hands ");
    if (player.body.cocks.length > 2) CView.text("relinquish your other pricks and ");
    if (player.body.cocks.length === 2) CView.text("relinquish your other prick and ");
    CView.text("focus on squeezing more drops out of you.  She fairly coats her lips with it, rubbing your hole against them before popping the whole [cockHead biggest] back into her mouth.  A moan escapes you just as another one returns from Sheila's throat, transmitted through your [cock biggest].");

    CView.text("\n\nAs her attentions refocus on your [cockHead biggest], a pang of loss arises from the middle of your " + describeCocksLight(player) + " - and is quickly smothered with her chest.");
    // [(sheila corruption < 50)
    if (sheilaCorruption() < 50) CView.text("  Though her modest breasts can't even begin to encompass your girth, Sheila rubs her cleavage along your skin with gusto while she works your tip with her tongue.");
    else if (sheilaCorruption() < 150) CView.text("  Her large " + sheilaCup(player) + " breasts encompass your girth easily, and she pushes the soft, flushed skin together as she slides it up and down your cock.");
    else {
        CView.text("  The demon's ridiculously enormous breasts easily cover the entire length of your cock");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(" falling between her crotch and her mouth, making her whole body feel like a huge, wet fuckhole");
        if (player.body.cocks.length > 1) CView.text(" built for " + numToCardinalText(player.body.cocks.length) + " pricks");
        CView.text(".");
    }
    ...
    if (sheilaCorruption() >= 150) {
        CView.text("Her enormous breasts dangle all the way to the ground, forming a second, softer hole to push into with your swollen prick");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(".  ");
    }
    CView.text("\"<i>Y-yes,</i>\" she mumbles, \"<i>gonna fuck her... harder...</i>\"");

    CView.text("\n\nThe apparent answer to your question passes you by as you obliviously force your tool");
    if (player.body.cocks.length > 1) CView.text("s");
    CView.text(" through the vise of her legs; losing yourself further and further to pleasure, you don't even notice when Sheila's hips begin to buck in time with your own, pressing her mons down against your ");
    if (player.body.cocks.length > 1) CView.text("topmost ");
    CView.text("dick.  With an enormous, final thrust, you mash your crotch up against hers and begin to ejaculate.  ");
    if (player.stats.cor < 50) CView.text("The sensation seems to reawaken Sheila; she angles her body up to press her pussy close and your embezzled sensation floods back into you - along with an unexpected gift.  You can feel your... no, <i>her</i> demonic pussy convulsing repeatedly, trying to suck up sperm in vain as her own orgasm comes off, and she looks at you with sly eyes.  \"<i>A little reciprocity for my love,</i>\" the woman coos, \"<i>to show you how much I care.</i>\"");
    else CView.text("\"<i>Oh god!</i>\" Sheila shrieks.  \"<i>I'm coming!  Jizz is going everywhere!</i>\"  Indeed, as your body releases streams of cum, Sheila directs them by aiming " + describeOneOfYourCocks(player) + ", covering herself and moaning as her hands press out as much as possible.");

    CView.text("\n\nFinished, you pull your sensitive, soaked shaft");
    if (player.body.cocks.length > 1) CView.text("s");
    CView.text(" from between her still-squeezing thighs with a shudder; Sheila shudders with you until you finally draw free and break skin contact, then lowers herself carefully to the ground.  The demon turns toward you, exposing a face and chest splattered with your semen.  She stares at your deflating prick");
    if (player.body.cocks.length > 1) CView.text("s");
    CView.text(", looking almost wistful to see ");
    ...
    if (player.body.vaginas.length <= 0) CView.text("to grow a nice pussy for me so that ");
    CView.text("I can put it in you any time I want.</i>\"  She sticks her tongue out at you, and the seed dripping from her lips oozes onto it.");
...
// Win - [Penetration, in Spades] - for self-loathing vaginas
function winAgainstDemoNSheilaForVaginas(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You drop onto the woman's lap without hesitation, ");
    if (!player.body.legs.isTaur()) CView.text("mauling her tits roughly with your hands, ");
    CView.text("and grind your pussy desperately against her midriff.  She slips two fingers into you obligingly, but you ");
    if (!player.body.legs.isTaur()) CView.text("grab her face, plunging one thumb into the corner of her mouth in your haste, and");
    else CView.text("turn your head and");
    CView.text(" stare right into her eyes.  Focusing as hard as you can on your fantasy, you try to transmit your desire for something more to the demon by sheer force of will.  A sly smile parts her lips as she stares back - could this actually work?");
    ...
    if (!player.body.legs.isTaur()) {
        CView.text("crawl away on your hands");
        // [(tail or naga)
        if (player.body.tails.length > 0 && player.body.legs.isNaga()) CView.text(" and raise your tail");
    }
    else CView.text("clamber to your hooves and bend your forelegs");
    CView.text(" to present your eager [vagina].  Sheila sits up and places her hands on your [hips], then slides her pelvis up, teasing your lower lips with something bulbous and warm.  They sting slightly as it smears its slime along them, a not-entirely-pleasant sensation.");
    ...
    displayStretchVagina(player, 10, true, true, false);
    CView.text("  Oh, god, it burns!  You shake violently as your pussy is assaulted by the fluid from Sheila's black tail, waves of pain twisting through it and forcing its walls to clamp down on the invader.  The demon gasps as you squeeze and leans forward onto your back");
    if (sheilaCorruption() > 0) CView.text(", squashing her tits into you");
    CView.text(".  \"<i>Calm down!</i>\" she barks, grabbing your arms and pulling them toward her.  With hands held behind you like this, you can do little to check her enthusiasm... but Sheila doesn't begin thrusting yet.  \"<i>It'll fade soon, love, and you'll start feeling it,</i>\" she promises, kissing your ");
    if (!player.body.legs.isTaur()) CView.text("neck");
    else CView.text("withers");
    CView.text(" as her strange, still shaft continues drooling into your pussy.  Your groin heats up in response to the pain, and her words ring hollow.  The burning doesn't seem to be getting any better; you look back over your shoulder at her, fear and hurt evident on your tightened features.  She smiles nervously at you, but the flow of goo doesn't stop.  It feels wetter and slicker in your pussy every second, and soon you can feel the slime begin to seep back out of your [vagina].  Trickle after trickle of fluid escapes you, falling to the ground underneath.");
    ...
    // [(lib <20)
    if (player.stats.lib < 20) CView.text("even as proud as you are, ");
    CView.text("there's something appealing about the picture, something powerful in being cherished and used in equal measure.  Sheila screams and releases your hair, digging her fingers into your hips, spraying the inside of your pussy with a thick, oozing heat that spreads out and then dulls as your own pouring juices force her cum out.  Peering between your legs, you can see it falling to the ground; viscous black sludge that bubbles and steams as if it were trying to reach back up to your sex.");
    if (player.stats.cor < 50) {
        CView.text("  She pumps and pumps again, forcing more of the muck into your [vagina] even as she calls you the most filthy things she can think of");
        if (sheilaCorruption() > 0) CView.text("; her tits wobble violently as she thrusts, and then still as she arches her back and jams her spade all the way in");
        CView.text(".  Sheila howls to the sky as she pours her demonic slime into you in one final, solid stream.  You can feel it pushing eagerly through even your copious fluids until it's at the entrance to your womb... and then inside.");
        // [fertility up if not capped; tingle deep within body, instinctively you know you have become blah blah]
        if (player.body.fertility < 50) {
            CView.text("  There is a tingle deep inside you, making you more fertile.");
            player.body.fertility++;
        }
    }
    else {
        CView.text("  She stops pumping, falling against your back and panting as the last of her goo seeps out and is ejected around her shaft by your fountainhead of girl-cum.  You are clearer-headed than before as your orgasm dwindles, though, illogically, you could swear you felt her tits growing against you.");
    }
