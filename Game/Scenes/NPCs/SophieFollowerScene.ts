...
import { returnToCampUseOneHour, campFollowers, companionsCount } from "../Camp";
...
import { sophieIsInSeason, bimboSophie, sophieCapacity, approachBimboSophieInCamp, sophieSprite } from "./SophieBimbo";
import { partial } from "../../Utilities/Partial";
import { luststickApplication } from "./SophieScene";
import { Womb } from "../../Body/Pregnancy/Womb";

export const SophieFollowerFlags = {
    UNKNOWN_FLAG_NUMBER_00283: 0,
    UNKNOWN_FLAG_NUMBER_00282: 0,
    SOPHIE_DEBIMBOED: 0,
    SOPHIE_RECRUITED_PURE: 0,
    SOPHIE_ADULT_KID_COUNT: 0,
    SOPHIES_DAUGHTERS_DEBIMBOED: 0,
    FOLLOWER_AT_FARM_SOPHIE: 0,
    SLEEP_WITH: '',
    SOPHIE_CAMP_EGG_COUNTDOWN: 0,
    SOPHIE_DAUGHTER_MATURITY_COUNTER: 0,
    FARM_CORRUPTION_STARTED: 0,
    FOLLOWER_PRODUCTION_SOPHIE: 0,
    FOLLOWER_PRODUCTION_SOPHIE_COLORCHOICE: 0,
    FARM_EGG_STORED: 0,
    FARM_EGG_COUNTDOWN: 0,
    SOPHIE_FAMILY_INCEST: 0,
    SOPHIE_TATTOO_COLLARBONE: 0,
    SOPHIE_TATTOO_SHOULDERS: 0,
    SOPHIE_TATTOO_LOWERBACK: 0,
    SOPHIE_TATTOO_BUTT: 0,
    SOPHIE_FOLLOWER_IRRITATION: 0,
    TIMES_MORNING_SOPHIE_FEMDOMMED: 0,
    NO_PURE_SOPHIE_RECRUITMENT: 0,
    DAUGHTER_ONE_BIMBO: 0,
    DAUGHTER_TWO_BIMBO: 0,
    DAUGHTER_THREE_BIMBO: 0,
    DAUGHTER_FOUR_BIMBO: 0,
};
User.flags.set(FlagType.SophieFollower, SophieFollowerFlags);

const pregnancy = sophieScene.pregnancy; // Quick way to access sophie's pregnancyStore
...
export function sophieFollower(): boolean {
    if (SophieFollowerFlags.UNKNOWN_FLAG_NUMBER_00283 > 0) return false;
    if (bimboSophie()) return false;
    if (SophieFollowerFlags.UNKNOWN_FLAG_NUMBER_00282 > 0 && SophieFollowerFlags.SOPHIE_DEBIMBOED > 0) return true;
    if (SophieFollowerFlags.SOPHIE_RECRUITED_PURE > 0) return true;
    return false;
}

// Un-Bimbo*
function unbimboSophie(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Taking the potent mixture out of your pouch, you consider giving it to the bimbo harpy fluttering about in front of you.  She'd probably be pretty mad about the whole affair, but if you want to repair the damage you've inflicted, this is probably your best bet.  Do you un-bimbo Sophie?");
    // [Yes] [No]☼
    return { yes: yesDebimboSophie, no: noDontDebimbo };
}

// No (You Monster)*
function noDontDebimbo(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You shrug and put the potion back in your pack.  Maybe later...  A mad Sophie isn't something you particularly want to deal with right now.");
    // (Return to Sophie menu.  You monster)

    return { next: partial(approachBimboSophieInCamp, player, true) };
}

// Yes (God dammit what the fuck did I just say)*
function yesDebimboSophie(player: Character): NextScreenChoices {
    CView.clear();
    player.inventory.items.consumeItem(ConsumableName.DeBimbo);

    CView.text("You grab the air-headed harpy and pull her over to you, pressing her impressive chest against you as you ready your intellectual beverage.  \"<i>Oooh! Hey, babe, what'cha got there?</i>\" Sophie asks, staring wide-eyed at the tincture.  Telling her not to worry about it, you pop the cork and bring the vial's neck up to Sophie's mouth, her big, full lips parting eagerly.  \"<i>Hey, is it wine?  I could just <b>DIE</b> for some wine!  Then maybe, like, a good hard fucking afterwards!</i>\" she declares, jumping excitedly in your grasp - almost making you spill the drink.  You grasp her chin, steadying the harpy's head just long enough to upend the liquid into her mouth, making sure to drain every last drop into her before you tickle her throat, forcing her to swallow it all in one audible gulp.");
...
function bimboSophieAgain(player: Character, inCombat: boolean): NextScreenChoices {
    ...
    SophieFollowerFlags.SOPHIE_DEBIMBOED = 0;
    SophieFollowerFlags.SOPHIES_DAUGHTERS_DEBIMBOED = 0;
    player.inventory.items.consumeItem(ConsumableName.BimboLiqueur);
    if (inCombat)
        return { next: returnToCampUseOneHour }; // (Display Sophie's normal options.You monster)
    else approachBimboSophieInCamp(player, false);
}

// Beat Her (You Monster)*
function beatSophieAroundYouMonster(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Oh no, fuck this!  You ready your [weapon] to knock some sense into the harpy slut.  Seeing your hostile movements, Sophie draws a sharp breath, preparing to defend herself.");

    CView.text("\n\n\"<i>I'm gonna make you pay for this!</i>\"");
    // (Go to normal Sophie Combat screen)
    const monster = new Sophie();
    monster.effects.add(StatusEffectType.BimboBrawl, 0, 0, 0, 0);
    monster.effects.add(StatusEffectType.GenericRunDisabled, 0, 0, 0, 0);
    return CombatManager.beginBattle(player, monster);
}

// Successful Beat the Shit Out of Sophie (You Monster)
function beatUpDebimboSophie(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Sophie stumbles back, unable to withstand your furious assault.  You grab the harpy before she collapses, asking if she's ready to fucking listen now.  Defiantly, she looks away from you, though otherwise too weakened to resist you.");
    // [Apologize] [Why I Did It] [Let Her Go] [Bimbo again!(if PC has Bimbo Liqueur)]
    // {No INT gate now!}

    const choices: ScreenChoice[] = [];
    // choices[0] = ["Beat Her", beatUpDebimboSophie] {
    choices[1] = ["Apologize", apologizeToDebimboSophie];
    choices[2] = ["WhyIDidIt", whyIDidItToDebimboSophie];
    choices[3] = ["Let Her Go", letDebimboSophieGo];
    if (player.inventory.items.has(ConsumableName.BimboLiqueur)) choices[4] = ["Bimbo Again", bimboSophieAgain];
...
// Let Her Go*
function letDebimboSophieGo(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You decide it's probably best to let Sophie go.  She remains still for a long moment, before finally looking at you with her big, blue eyes, a strange mixture of loathing and wonder in them.  Without further hesitation, she turns and rushes off, back toward the High Mountains you'd imagine.");
    ...
    if (player.stats.lib >= 50) {
        ...
        if (player.stats.cor < 40) {
            CView.text("  You blink repeatedly, trying to clear your head of these idle thoughts, but are nonetheless aroused.");
            // {+10 AROUSAL}
...
function apologizeToDebimboSophie(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You decide to apologize to Sophie, saying you didn't mean for things to get so out of hand... you didn't know she'd change quite so much!  You only thought the potion would enhance her physique, maybe make her a little more receptive to your advances.");
...
export function sophieDaughterDebimboUpdate() {
    if (SophieFollowerFlags.SOPHIE_ADULT_KID_COUNT === 1) {
    ...
    else if (SophieFollowerFlags.SOPHIE_ADULT_KID_COUNT === 2) {
...
export function followerSophieMainScreen(player: Character): NextScreenChoices {
    ...
    if (sophieIsInSeason() && player.body.cocks.length > 0 && SophieFollowerFlags.FOLLOWER_AT_FARM_SOPHIE === 0) {
        ...
        if (bimboSophie()) CView.text("restoring her intellect made her quite skilled at it again");
        else CView.text("she's quite skilled at it");
        CView.text(".  \"<i>Ohhh, come on, [name]!  Just give it to me like a good " + mf(player, "boy", "girl") + ", would you?  I could even give you a special treat.  <b>Just give. It. To. Me.</b></i>\"");
        ...
        choices[0] = ["Appearance", sophieAppearance];
        if (player.body.cocks.length > 0) {
            if (player.body.cocks.find(Cock.CockThatFits(sophieCapacity()))) {
                choices[1] = ["Vaginal", fuckFollowerSophie];
                choices[2] = ["Special", sophieSpecial];
            }
            else CView.text("\n\nYou're too big to fit inside her!");
        }
        else CView.text("\n\nYou need a penis to fuck her!");

        if (SophieFollowerFlags.SLEEP_WITH === "Sophie") {
            choices[8] = ["NoSleepWith", sleepWithSophieToggle];
            CView.text("\n\nYou're currently sharing your bed with Sophie at night, but you could kick her out, if you wanted.");
        }
        choices[9] = ["Back", campFollowers];
        return { choices };
    }
    else if (SophieFollowerFlags.FOLLOWER_AT_FARM_SOPHIE === 0) {
        if (SophieFollowerFlags.UNKNOWN_FLAG_NUMBER_00282 > 0) CView.text("Sophie leans back in her nest as you approach, her prominent, bare F-cups on display.  She's completely nude, as always, and sits with her avian legs shamelessly spread, revealing the slick slit of her womanhood framed by downy-soft platinum blonde feathers.  \"<i>Hey there, " + mf(player, "handsome", "cutey") + ".  What can Momma Sophie do for you?  Come for a cuddle in my welcoming wings, maybe; or for something more... exciting?</i>\"");
        else CView.text("Sophie leans back in her nest as you approach, her prominant, bare DD-cups on display.  She's completely nude, as always, and sits with her avian legs demurely crossed, concealing the slick slit of her womanhood from you.  \"<i>Hey there, " + mf(player, "handsome", "cutey") + ".  What can Momma Sophie do for you?</i>\"");
    }
    else {
        CView.text("When you call her name, Sophie's gold-glossed lips curve upward in a smile. She takes her time picking her way out of her nest, and then glides down to you.");
    ...
    choices[0] = ["Appearance", sophieAppearance];
    if (player.stats.lust >= 33) choices[1] = ["Sex", sexWithFollowerSophie];
    else CView.text("\n\nYou aren't aroused enough to sleep with Sophie at the moment.");
    if (SophieFollowerFlags.FOLLOWER_AT_FARM_SOPHIE === 0) choices[8] = ["Sleep With", sleepWithSophieToggle];
    if (SophieFollowerFlags.SLEEP_WITH === "Sophie" && SophieFollowerFlags.FOLLOWER_AT_FARM_SOPHIE === 0) {
        choices[8] = ["NoSleepWith", sleepWithSophieToggle];
        CView.text("\n\nYou're currently sharing your bed with Sophie at night, but you could kick her out, if you wanted.");
    }
    if (SophieFollowerFlags.SOPHIE_CAMP_EGG_COUNTDOWN > 0 && SophieFollowerFlags.FOLLOWER_AT_FARM_SOPHIE === 0) CView.text("\n\n<b>Sophie's egg is sitting nearby.</b>");
    if (SophieFollowerFlags.SOPHIE_DAUGHTER_MATURITY_COUNTER > 0) {
        choices[7] = ["Daughter", daughterCheckup];
    }
    if (SophieFollowerFlags.FOLLOWER_AT_FARM_SOPHIE === 0) choices[9] = ["Back", campFollowers];
    else choices[9] = ["Back", rootScene];

    if (SophieFollowerFlags.FOLLOWER_AT_FARM_SOPHIE === 0 && SophieFollowerFlags.FARM_CORRUPTION_STARTED === 1) choices[2] = ["Farm Work", sendToFarm];
    if (SophieFollowerFlags.FOLLOWER_AT_FARM_SOPHIE === 1) choices[2] = ["Go Camp", backToCamp];

    if (SophieFollowerFlags.FOLLOWER_AT_FARM_SOPHIE === 1 && SophieFollowerFlags.FOLLOWER_PRODUCTION_SOPHIE === 0) choices[3] = ["Harvest Eggs", harvestEggs];
    if (SophieFollowerFlags.FOLLOWER_AT_FARM_SOPHIE === 1 && SophieFollowerFlags.FOLLOWER_PRODUCTION_SOPHIE === 1) {
        choices[3] = ["Change Eggs", changeEggs];
        choices[4] = ["Stop Harvest", stopHarvest];
    }
    return { choices };
}
...
function sendToFarm(player: Character): NextScreenChoices {
    ...
    CView.text("You tell your live-in harpy that she is to head towards the lake, find a farm, present herself to the lady who works there and do as she says. Sophie isn’t exactly enthused by this idea.");
    ...
    SophieFollowerFlags.FOLLOWER_AT_FARM_SOPHIE = 1;
...
function backToCamp(player: Character): NextScreenChoices {
    ...
    CView.text("You tell her she’s welcome to head back to camp now.");
    ...
    SophieFollowerFlags.FOLLOWER_AT_FARM_SOPHIE = 0;

    return { next: rootScene };
}

function harvestEggs(player: Character): NextScreenChoices {
    CView.clear();
    sophieSprite();

    CView.text("You ask Sophie if it’s possible for her to produce eggs of a certain color whilst she’s here.");
...
function eggSelector(player: Character): NextScreenChoices {
    const choices: ScreenChoice[] = [];
    for (let i: number = 0; i < eggColors.length; i++) {
        choices[i] = [eggColors[i], partial(postEggSelector, player, eggColors[i])];
    }
    return { choices };
}

function postEggSelector(player: Character, selected: string): NextScreenChoices {
    CView.clear();
    sophieSprite();

    CView.text("“<i>I’ll make some nice " + selected.toLowerCase() + "eggs for you then, hun,</i>” she says with a small grin. “<i>I can only produce one a week, though. I’ll put it with everything else you earn out of this hole in the ground. How’s that?</i>”");

    SophieFollowerFlags.FOLLOWER_PRODUCTION_SOPHIE = 1;
    if (SophieFollowerFlags.FOLLOWER_PRODUCTION_SOPHIE_COLORCHOICE !== selected) {
        SophieFollowerFlags.FOLLOWER_PRODUCTION_SOPHIE_COLORCHOICE = selected;
        SophieFollowerFlags.FARM_EGG_STORED = 0;
        SophieFollowerFlags.FARM_EGG_COUNTDOWN = 7;
    }
...
function stopHarvest(player: Character): NextScreenChoices {
    ...
    CView.text("You tell Sophie to stop giving you eggs; you’d rather she’d put them towards making the farm money.");
    ...
    SophieFollowerFlags.FOLLOWER_PRODUCTION_SOPHIE = 0;

    return { next: rootScene };
}

function changeEggs(player: Character): NextScreenChoices {
    CView.clear();
    sophieSprite();

    CView.text("You ask Sophie if she will change the type of eggs that she is producing for you.");
...
// [Sex]
function sexWithFollowerSophie(player: Character): NextScreenChoices {
    CView.clear();

    CView.text("\"<i>Oh, really? I was kind of hoping you'd want a taste of me soon.</i>\"  Sophie somewhat perks up at the mention of having sex with you.  \"<i>So, what'd you like? The usual, or should Momma do something special for you this time around?</i>\"  She licks her lips at the suggestion.");

    // [Usual] [Nurse] [YouMove] [IMove] [Titfuck] [GetDMilked] [Extra1] [....]
    const choices: ScreenChoice[] = [];
    if (player.body.cocks.length > 0) {
        if (player.body.cocks.find(Cock.CockThatFits(sophieCapacity()))) choices[0] = ["Vaginal", fuckFollowerSophie];
        choices[1] = ["Blowjob", sophieFollowerGivesBlowjobs];
    }
    if (SophieFollowerFlags.SOPHIE_ADULT_KID_COUNT > 0 && SophieFollowerFlags.SOPHIE_FAMILY_INCEST > 0 && player.body.cocks.find(Cock.CockThatFits(sophieCapacity())))
        choices[8] = ["DaughterFuck", sophieIncestInHerCooterOrSomethingIDunno];
    choices[9] = ["Back", followerSophieMainScreen];
    return { choices };
}

// Appearance:
function sophieAppearance(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Sophie is a big, buxom harpy, no two ways about her.  She has ");
    if (SophieFollowerFlags.UNKNOWN_FLAG_NUMBER_00282 > 0) CView.text("very ");
    CView.text("long ");
    if (SophieFollowerFlags.UNKNOWN_FLAG_NUMBER_00282 > 0) CView.text("platinum-blond");
    else CView.text("pink");
    CView.text(" 'hair' that dangles off her shoulders, like a bird's plumage.  She wears a thick layer of golden lipstick over her plump");
    if (SophieFollowerFlags.UNKNOWN_FLAG_NUMBER_00282 > 0) CView.text(", super-sized");
    CView.text(" lips at all times, like most of her kind, the gloss capable of giving her voice a compelling quality or inducing arousal on contact with males or herms.  Her face lacks the youthful quality found among many of her sisters, though you don't see any of the lines that would indicate age.");

    CView.text("\n\nSophie's breasts are ");
    if (SophieFollowerFlags.UNKNOWN_FLAG_NUMBER_00282 > 0) CView.text("so immense they would drag her to the ground if she tried to fly, a sexy, lingering aftereffect of the draft you gave her.  The wobbly bimbo-tits look to be about F-cups.");
    else CView.text("unusually large.  The buxom harpy does seem slightly encumbered by them, but still capable of flight.  Her breasts look to be about DD-cups, nice and big enough for a soft squeeze or rough tit-fuck.");
    ...
    if (hasTattoo("sophie")) {
        ...
        if (sophieFullTribalTats()) {
            ...
            if (numTattoos("sophie") > 1) CView.text("She has the following tattoos emblazoned across her body:\n");
            else CView.text("She has ");

            if (SophieFollowerFlags.SOPHIE_TATTOO_COLLARBONE !== 0) CView.text(SophieFollowerFlags.SOPHIE_TATTOO_COLLARBONE + "\n");
            if (SophieFollowerFlags.SOPHIE_TATTOO_SHOULDERS !== 0) CView.text(SophieFollowerFlags.SOPHIE_TATTOO_SHOULDERS + "\n");
            if (SophieFollowerFlags.SOPHIE_TATTOO_LOWERBACK !== 0) CView.text(SophieFollowerFlags.SOPHIE_TATTOO_LOWERBACK + "\n");
            if (SophieFollowerFlags.SOPHIE_TATTOO_BUTT !== 0) CView.text(SophieFollowerFlags.SOPHIE_TATTOO_BUTT + "\n");
        }
    }

    const choices: ScreenChoice[] = [];
    choices[4] = ["Back", followerSophieMainScreen];
    return { choices };
}

// Sleep With:*
function sleepWithSophieToggle(player: Character): NextScreenChoices {
    CView.clear();
    if (SophieFollowerFlags.SLEEP_WITH !== "Sophie") {
        CView.text("You ask Sophie if she'd like to start sleeping with you at night.  Sophie cocks an eyebrow up at the suggestion.");
        // DEBIMBO'ed
        if (SophieFollowerFlags.UNKNOWN_FLAG_NUMBER_00282 > 0) {
            ...
            CView.text("\n\nYou ");
            if (player.stats.cor < 33) CView.text("sheepishly ");
            CView.text("answer that you thought it might be nice to snuggle up with her at night, not even necessarily sexually.  Just having someone to hold during the long, cold nights would be nice, and you admit that Sophie would be a rather lovely body to cuddle up against.");
            ...
            if (player.body.cocks.sort(Cock.Smallest)[0].area <= 5 && player.body.cocks.length > 0) CView.text(", but don't be surprised if I go a little rough on that tiny dick of yours in the morning");
            CView.text("!</i>\" she calls.");
            if (player.body.cocks.sort(Cock.Smallest)[0].area <= 5 && player.body.cocks.length > 0) CView.text("\n\n<b>If you don't want her to tease you about your small penis, you had better tell her to stop sleeping with you.</b>");
        }
        // NORMAL
        else {
            ...
            if (player.body.cocks.sort(Cock.Smallest)[0].area <= 5 && player.body.cocks.length > 0) CView.text("  \"<i>You won't mind me teasing your cute little dick in the mornings, will you [name]?  It's so cute...</i>\" she muses.\n\n<b>If you don't want her to tease you about your small penis, you had better not sleep with her.</b>");
        }
        SophieFollowerFlags.SLEEP_WITH = "Sophie";
    }
    else {
        // Don't Sleep WIth:*
        CView.text("You tell Sophie that you'd rather not sleep with her for now.  She sighs heavily at the proclamation and says, \"<i>");
        if (SophieFollowerFlags.UNKNOWN_FLAG_NUMBER_00282 === 0) CView.text("Well, it is your bed.  Let me know if you want me coming back for snuggles, okay?");
        else CView.text("Couldn't handle all these curves, huh, [name]?  Don't fret, Momma Sophie will stick around until you can't stop yourself from cuddling up on her.");
        CView.text("</i>\"");
        SophieFollowerFlags.SLEEP_WITH = "";
    }
...
function fuckFollowerSophie(player: Character): NextScreenChoices {
    const cocksThatFit = player.body.cocks.filter(Cock.CocksThatFit(sophieCapacity()));
    let cockThatFits = cocksThatFit[0];
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    const secondCockThatFits = cocksThatFit[1];
    CView.clear();
    // In season intro
    if (sophieIsInSeason()) {
        CView.text("Sophie pulls you into her feathery embrace, her fingertips sensually exploring every inch of your form.  Her voice coos huskily, \"<i>So you want to help make a momma out of Momma Sophie, huh?</i>\"  She grabs your hand and stuffs it between her luscious thighs, right into the dripping-wet delta of her mons.  \"<i>Don't keep me waiting... you got me all... hot and bothered...</i>\"");
    }
    // Regular Intro
    else {
        CView.text("Sophie smirks as she saunters closer, fingertips casually exploring your body.  She coos huskily, \"<i>So, you want to have a little fun with Momma Sophie, huh?</i>\"  She spreads her luscious thighs apart, just enough to show you the glittering moisture on her mound.  \"<i>Well, let's see how much of a " + mf(player, "man", "'man'") + " you are...</i>\"");
    }
    // Both
    CView.text("  You strip out of your [armor] without a second thought and present your " + describeCock(player, cockThatFits) + " to Sophie.");

    // Small
    if (cockThatFits.area <= 5) {
    ...
    CView.text("\n\nSophie gives your " + describeCock(player, cockThatFits) + " an appreciative swat before pirouetting around, bent over with her hands on her knees and her butt up in the air.  She wiggles back and forth, her lasciviously luscious backside jiggling, the cheeks clapping audibly at the apex of each sway.  Her voice purrs, \"<i>Come and get it, if you think you're " + mf(player, "man", "woman") + " enough!</i>\"");

    CView.text("\n\nYou prowl forward, but Sophie backs up, wings flapping, her rump pushing right into you.  Her bubble-butt drives you back against one of the many standing stones, pinned between a cunt and a hard place.  Squishing noisily, your " + describeCock(player, cockThatFits) + " slides right into Sophie's ");
    if (sophieIsInSeason()) CView.text("gushing, sperm-hungry");
    else CView.text("slippery");
    CView.text(" cunt.  You moan out load at the sudden penetration and grab onto Sophie's ass for dear life.  Her ass practically devours your fingertips, the expansive backside sinking exquisitely under your touch.  It's a detail you barely have time to register, as most of your attention is riveted on the tight walls gripping you, sliding along with well-practiced sexual skill.  Sophie's hips roll slowly, pinning you against the rock in the most pleasant of ways.");
    ...
    if (player.stats.int < 50) CView.text("\n\nAbruptly");
    else CView.text("\n\nEven though you know she's using her compulsion");
    CView.text(", her words still affect you.  Her pussy just feels... so... exquisite.  It's so hot and wet that you almost feel like you could cum right now.  Your [legs] wobble, but Sophie just pushes back even harder, holding you up by your " + describeCock(player, cockThatFits) + " and the friction of your [butt] on the boulder.  The harpy watches your expression as her hips continue their slow roll, feeling your fingers twitch and knead her immense backside when the pleasure begins to mount.  You try to stay composed, but it's just so gods-damned good!  Her silken slit is so warm, wet, and just plain... erotic.  Slowly, the point where you would normally orgasm approaches, but instead of the spasming clenches that signal release, your jizz stays bottled up inside you tighter than sperm in a goblin's womb.");
    ...
    if (secondCockThatFits) CView.text("fuck it");
    else CView.text("finger my asshole");
    CView.text(" while I fuck you, [name]!</i>\" she commands, fixing you with her eyes authoritatively.  ");
    if (secondCockThatFits) CView.text("You shift to bring your " + describeCock(player, secondCockThatFits) + " up in place, just in time for Sophie's wiggling rectum to spread around it.  It slides on in without so much as a whimper.");
    else CView.text("You thumb at her asshole hesitantly, right up until her wiggling rectum presses on it, easily dilating around your invading finger.");
    CView.text("  You squeeze tighter and begin to massage without knowing why, actually able to feel yourself through her thin, internal walls.");
    ...
    if (secondCockThatFits) CView.text(" and anal walls");
    CView.text(".  Your fingers dig into her plush butt, no longer kneading, but instead curling possessively, claiming absolute ownership of that supple behind and the rapidly-filling cunt below.");
    if (secondCockThatFits) CView.text("  You pack her intestines full of cum at the same time that you sperm up her clingy cunny tunnel.");
    if (sophieIsInSeason()) CView.text("  Her pussy seems to drink the spunky deposit down, her cervix opening to allow in as much in as possible.  You can feel the suction, and if you weren't already riding out a mind-bending orgasm, you're sure you'd be cumming harder than normal right now.");
    CView.text("  Your body slacks over her, the only muscles that are actually working are the ones spasming in your loins, pumping out fresh batches of sperm.  You whimper with primal pleasure, spittle frothing at the corners of your mouth.");
    if (player.cumQ() >= 500) {
        CView.text("  Sophie's belly ");
        if (player.cumQ() < 1000) CView.text("has begun to inflate");
        else CView.text("has inflated to an absurd degree");
        CView.text(" due to all the spooge you're squirting");
        if (player.cumQ() >= 4000) CView.text(" and it's still growing larger");
        CView.text(".");
    }
    if (player.cumQ() >= 5000 || (player.cumQ() >= 500 && sophieIsInSeason())) {
        CView.text("  A huge wash of white rushes out from her well-sexed fuck-hole, soaking your legs with the mixed cum.");
    }
    CView.text("  Then, you black out, with your " + describeCock(player, cockThatFits));
    if (secondCockThatFits) CView.text(" and " + describeCock(player, secondCockThatFits));
    CView.text(" still pulsing in ecstasy.  It was such a good cum...");
    player.orgasm();
    player.stats.sens += -2;

    if (sophieIsInSeason()) sophiePregChance();

    return { next: sophieVagFollowerFollowup };
}

// [Next]
function sophieVagFollowerFollowup(player: Character): NextScreenChoices {
    CView.clear();
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(sophieCapacity()));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];

    CView.text("You're roused a few moments later by a smiling Sophie, the proof of your union dripping between her thighs.  She quips, \"<i>");
    if (sophieIsInSeason()) CView.text("If that didn't take, I don't know what will!");
    else if (cockThatFits.area <= 5) CView.text("Not too bad for a tiny girl-dick!");
    else CView.text("It's always better when you have to wait for it, isn't it?");
    CView.text("</i>\"");
...
// Blowjob*
// Sucking dicks.
function sophieFollowerGivesBlowjobs(player: Character): NextScreenChoices {
    CView.clear();
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(sophieCapacity()));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];

    CView.text("Sophie cocks her head to the side and chuckles, \"<i>You wanna feel Momma Sophie's lips around that ");
    if (cockThatFits.area <= 5) CView.text("tiny clit you call a penis");
    else if (cockThatFits.area <= sophieCapacity() / 2) CView.text("needy boy-cock");
    else CView.text("over-swollen engorgement you call a cock");
    CView.text("?</i>\"  She saunters up with her hips swaying seductively, her juicy mound squelching noisily, webs of lubricant dangling between her thighs whenever they spread.  The harpy's fingertip punches into your chest");
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 1) CView.text(", right between your [chest]");
    CView.text(", and she slowly trails it up to your chin, which she gently grabs in her long-nailed fingers.  \"<i>");
    if (SophieFollowerFlags.UNKNOWN_FLAG_NUMBER_00282 > 0) CView.text("You may have given me a bimbo's body and an insatiable need for sex,");
    else CView.text("You may have seduced me into your camp with hot sex,");
    CView.text("</i>\" she coos, \"<i>but that doesn't mean I'll wrap these beautiful lips around you for nothing.  How about you let me use that cute face of yours to grind my pussy on, and I'll return the favor?</i>\"  She blows you a kiss that culminates in her running her tongue over her swollen lips in a slow, wet circle.");
    // [Force Her] [Sixtynine]
    return {
        choices: [
            ["Force Her", forceSophieBlowjob],
            ["Sixtynine", sophieBlowsSixtyNine],
        ]
    };
}

// Blow Sixtynine*
function sophieBlowsSixtyNine(player: Character): NextScreenChoices {
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(sophieCapacity()));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    CView.clear();
    CView.text("You nod eagerly, something that gets the busty harpy smiling as her whole body flushes.  She rushes at you, leaping and flapping, speeding towards you faster as old feathers kick up in a wake behind her.  You spread your arms and let her slam into you, the impact cushioned by her weighty breasts and large, soft thighs.  Both of you roll end over end through the dirt, eventually coming to rest with her straddling your [chest] and rubbing a finger through her slippery folds.  She adds another finger to spread them wide, the pink interior pulsing wetly and dripping with female moisture.");
    ...
    if (player.body.tongue.type > TongueType.HUMAN && player.body.tongue.type <= TongueType.DRACONIC) CView.text("Longer and longer, your tongue continually unspools from your mouth, plunging deeper into the slippery channel with each passing moment.  Her taste saturates your many taste buds, and the velvety folds of her interior feel wonderful as you repeatedly slither your inhuman muscle through that syrupy passage.");
    else CView.text("Your tongue slides right in, the harpy's pussy seeming almost like a second home.  Her taste saturates your taste buds as you plunge in as deeply as possible, gliding over her velvet folds as it goes.");
    CView.text("  Sophie's immense thighs squeeze tighter in response, shutting out any last vestiges of light and pushing you harder into her sopping-wet mound.  You let your lower lip graze her rigid clitty and savor the lubricating fluid that she leaks into your waiting mouth.");

    CView.text("\n\nHer tongue and yours seem to be the only things that matter anymore.  Hers circles the entirety of your " + describeCock(player, cockThatFits) + ", accompanied by golden lips that leave behind their drugged, amber residue.  The enjoyable tingling that those chemicals induce is driving you wild with desire.  Your heart is hammering like crazy in your chest, each thrumming beat reinforcing the thickness of your erection in the bird-woman's mouth.");
    if (player.body.vaginas.length > 0) {
        CView.text("  She tenderly");
        if (player.body.balls.count > 0) CView.text(" reaches under your [balls] and");
        CView.text(" explores your [vagina] with two fingers, her thumb brushing across your [clit] every few seconds.  It seems the harpy is considerate of even your feminine needs - ones that serve to make you even harder, so full it almost hurts.");
    }
    // else
    else if (player.body.balls.count > 0) {
        CView.text("  She tenderly caresses your [sack], allowing her fingers to dance over your [balls].  It seems the harpy is considerate of all your needs in a way that makes you even harder, so full it almost hurts.");
    }
    else CView.text("She tenderly strokes a long nail across the sensitive skin of your taint.  It seems she's being very considerate to your pleasure, so considerate she has you so hard and full that it almost hurts.");
    CView.text("  Her feathery hair dangles over your [legs], tickling you as she noisily slurps and sucks, while you moan directly into her bare-skinned twat.  The vibrations set her dripping even more fiercely than before.  You do your best to dutifully drink it down.");

    CView.text("\n\nSophie hums happily as you begin to drip directly onto her tongue, the pre-cum bubbling out of your spit-polished cum-slit in a steady flow.  Though you know just what is plying your " + describeCock(player, cockThatFits) + " with slices of heavenly sensation, your gloss-drugged mind begins to slip into fantasy anyway.  Just like in the real world, Sophie's cunt is slathering your [face] with her fragrant arousal, and your pole is smothered with amber aphrodisiacs.  In the fantasy though, there's a pussy squeezing down on you and a second set of thick harpy thighs clenching around your waist.  Those fertile, egg-bearing thighs are bouncing up and down your length, and the twat above you is ripe with arousal, bounded above by a belly swollen with incubating life that will someday be your daughter.  Every morning you're sucked off to completion by one of your girls and kissed until your lips and rod are slathered to an amber shine.  Then you lie back and partake in the fruits of your position as " + mf(player, "king", "breeder-queen") + " of the harpies: creampie after creampie, tangy harpy-cunt to lick, and skilled fellatio every single time your lust begins to flag.  You groan into the slit above, your hands grabbing hold of the juicy haunches and pulling them down into you.");
    ...
    if (cockThatFits.area <= 60) CView.text("to the [sheath]");
    else CView.text("as far as she can, gagging slightly");
    CView.text(".  Her thighs begin to tremble around you, hard enough that you begin to worry that she might somehow crush your head, yet it's more like being smothered with soft pads than anything truly worrisome.  You shudder in excitement of your own and begin to flick her clit, eager to make her share in the pleasure.");
    ...
    if (player.cumQ() >= 1000 || player.body.cocks.length > 1) CView.text("  Warm goo covers your [hips] and [butt] once your virile deposit becomes too much for Sophie to handle.");
    if (player.cumQ() >= 500) CView.text("  She quickly gives up on trying to swallow it after she starts showing some cum-pudge, but her hand pumps you energetically as she milks your " + describeCock(player, cockThatFits) + ", squeezing the voluminous ropes of cum all over your [chest].");
    if (player.cumQ() >= 3000) CView.text("  You make a big white puddle with all the drippings, sinking into the cummy mud as you eat out the orgasmic harpy matron.");
    ...
    SophieFollowerFlags.SOPHIE_FOLLOWER_IRRITATION = 0;
    // Slimefeed
    player.slimeFeed();
    // 8 hours lust stick
    luststickApplication(player, 8);
    // +10 lust
    player.orgasm();
    player.stats.sens += 1;
...
// Forceful Blowjob*
function forceSophieBlowjob(player: Character): NextScreenChoices {
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(sophieCapacity()));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    CView.clear();
    // Increments a displeasure counter - doing enough of these causes her to leave in the night if it gets too high.
    SophieFollowerFlags.SOPHIE_FOLLOWER_IRRITATION++;
    CView.text("You grab hold of her and push her down on her knees, a task aided by Sophie's utter, uncomprehending surprise.  She hits the ground, pouting, her thick thighs flexing to try and help her rise.  You scold her - if she didn't want to suck your dick, she shouldn't have moved into your   Instead, she should be glad that you decided to ALLOW her to stay in your camp for a simple blowjob.  Pointing out her oozing snatch, you remind her that she's clearly enjoying this as much as you, and that she ought to just get to work and tend to her own needs while she satisfies you.");

    CView.text("\n\nSophie grumbles, but one of her hands does nestle itself down between her feathery thighs, her other going to your groin to fish out your " + describeCock(player, cockThatFits) + ".  She looks up at you through angry eyebrows softened slightly by concealed lust.  \"<i>I'll suck you off... this time,</i>\" she declares, ");
    if (player.body.balls.count > 0) CView.text("fondling your [balls] with almost eager attention,");
    else CView.text("rubbing your length with almost eager attention,");
    CView.text(" \"<i>but don't make a habit of this, or you'll wake up alone.</i>\"  Sophie's tongue flicks across your " + describeCockHead(cockThatFits) + " experimentally.  \"<i>At least you taste good.</i>\"  The slippery muscle slides across your sensitive dick-skin again, longer this time, lingering on the underside as the golden lips edge closer.");

    CView.text("\n\nImpatient, you slide your fingers through her feathery locks and edge her closer.  Her shining, amber mouth bumps into your " + describeCockHead(cockThatFits) + ", followed by a surprised chirp from the unresisting harpy.  At that first touch, the electric tingle of her glossy gold lipstick hits you, causing your heart to beat faster and your " + describeCock(player, cockThatFits) + " to thicken slightly with arousal in her hand.  You involuntarily push harder without meaning to, spreading Sophie's plump cock-kissers around the thickening meat, her spit slicking the slow, steady journey into her mouth.  Like magic, she begins to push forward on her own, almost pulling out of your grip in her eagerness.  Her mouth smoothly slides down ");
    if (cockThatFits.area <= 60) CView.text("as far as it can go, even taking you part way into her throat before she runs out of room");
    else CView.text("the whole way to your base, inch after inch of dick-flesh vanishing into her pliant throat");
    CView.text(".");

    CView.text("\n\nWith her snug, gold lips clinging around your rod, Sophie looks up with lust-lidded eyes, the noisy squelching that's coming from below leaving no doubt in your mind as to her state.  The burning, lust-fueling make-up that girds her cock-pillows seems to be seeping into you in greater and greater quantities.  It makes your whole body feel flush, burning-hot and ready to fuck her sweet, feathery behind.  You prepare to pull out, until you see those plush golden lips locked in place, sliding with slippery friction along every single inch of your " + describeCock(player, cockThatFits) + ".  You throb powerfully at that sight and flex inside her oral embrace, causing the harpy to churn out a delighted giggle, interrupted by a pleasured moan as she masturbates beneath your maleness.");
    if (player.body.cocks.length === 2) CView.text("  Your extra cock flops against her cheek, catching on her ear and slowly leaking pre-cum into her down-soft hair.");
    else if (player.body.cocks.length > 2) CView.text("  Your extra cocks flop against her cheeks, catching on her ear and shoulder and slowly leaking pre-cum into her down-soft hair.");

    CView.text("\n\nSophie abruptly grabs hold of one of her big, motherly tits and gropes herself passionately.  Her lips open just enough to separate from your " + describeCock(player, cockThatFits) + " and exude a lascivious moan as she masturbates herself.  The quivering tongue tickling at your underside is utterly enjoyable, a fact enhanced by the golden gloss now smeared all over your rod.  There's a tingling, pulsating quickening inside you.");
    if (player.body.balls.count > 0) CView.text("  Your [balls] feel almost like they're filling up, taut with liquid weight that begs to pump out");
    else CView.text("  It feels tight, like you're filling up with a warm liquid weight that's just begging to be let out");
    CView.text(" all over the motherly harpy's face.  You groan out loud with sheer excitement and fixate your eyes on Sophie's bodacious rump and thick thighs, partly obscured by her trembling tailfeathers.");

    CView.text("\n\nSlobber and drool drip down");
    if (player.body.balls.count > 0) CView.text(" your [sack] and over");
    CView.text(" your legs.  Sophie's eyes roll back slightly - not all the way but enough to proudly display her penultimate pleasure as she cums.  Her syrupy girl-cum splatters off the waste-land earth below, though the sound changes to a wetter, muddy splashing after a few seconds.  Squeezing tightly, the harpy's throat convulses around you.  Her lips lock tight as she hums in ecstasy, languidly beginning to suckle your " + describeCock(player, cockThatFits) + " as her own orgasm dies down.  You force her to move faster, pushing her head to pop back and forth, too eager for release to care for her comfort.  Judging by the steady, squelching pumping that begins anew, she doesn't care that much either.");
    CView.text("\n\nSophie suddenly compresses her gold-lacquered cock-pillows around you and sucks harder.  She flicks her eyes up at you to beg for your cum with a sluttier expression than you thought the harpy matron capable, inaudibly demanding you release inside her.  Your [hips] lurch forward to pound your " + describeCock(player, cockThatFits) + " deep into her face, and your " + describeCockHead(cockThatFits) + " tingles, expanding in orgasm.");
    if (cockThatFits.hasKnot()) CView.text("  The knot inflates just behind her lips, a golden orb that entraps you within her slick mouth and makes sure all your salty cremè is going to go down her throat.");
    ...
    if (player.cumQ() >= 500) {
        CView.text("  She gurgles noisily when her midriff begins to bulge from your liquid deposit");
        if (player.cumQ() >= 2000) CView.text(", but that's just the beginning.  You bloat her tummy to an obscene, almost spherical shape");
        if (player.cumQ() >= 4000) CView.text(", and then cum some more, the flooding jizz forced to escape from her nostrils after you finish flooding every inch of her digestive system");
        CView.text(".");
    }

    CView.text("\n\nAlmost regretfully, you ");
    if (cockThatFits.type !== CockType.DOG) CView.text("slip your spasming boner out of her golden gates, dripping the last few drops onto her pouty lower-lip.");
    else CView.text("pull against Sophie's cock-locked mouth, but your knot won't budge.  You sit there waiting for it to go down for what seems like forever, and her lipstick doesn't exactly help - it seems like your cum continues to flow and dribble the entire time, your animal-dong intent on inseminating the harpy's mouth.  Still, you manage to 'pop' free eventually, even though it stretches Sophie's jaw and echoes around your ");
    ...
    if (SophieFollowerFlags.SOPHIE_FOLLOWER_IRRITATION === 1) CView.text("Ahhh... not... not as much as... I would if you'd reciprocate!");
    else if (SophieFollowerFlags.SOPHIE_FOLLOWER_IRRITATION < 4) CView.text("Ahhh... I... I don't know how many times I'll put up with this kind of treatment!");
    else if (SophieFollowerFlags.SOPHIE_FOLLOWER_IRRITATION < 8) CView.text("Ahhh... are... are you ever going to reciprocate?");
    else if (SophieFollowerFlags.SOPHIE_FOLLOWER_IRRITATION === 8) CView.text("Ahhh... I think... I've had enough...");
    else CView.text("Ahhh... actually... I think... <b>I think if you don't treat me nicer, I might leave.</b>");
    CView.text("</i>\"");
...
function sophieSixtyNine(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Sophie cocks an eyebrow and asks, \"<i>A little sapphic delight for the horny champion?</i>\"  She snorts and rolls her eyes, \"<i>No thanks.  I'm not really that into girls.</i>\"  It doesn't seem like she's up for it.");

    return { next: sleepWithSophieToggle };
}

// Sophie Teases Small Dicks In The Morning*
// No toggles.  If you have a small penis you deserve the small penis femdom.
export function sophieSmallDongTeases(player: Character): NextScreenChoices {
    const smallestCock = player.body.cocks.sort(Cock.Smallest)[0];
    ...
    CView.text(" before lavishing its slickness along the underside.  You get so stiff - so hard - in just seconds, " + describeCock(player, smallestCock) + " swelling to try and fill the slick prison, but no matter how hard you get, that compressing cavern still seems so much larger and more capable than the small penis that it has to contort itself around.");
    CView.text("\n\nAbruptly, your lusty erection is removed from the heated embrace and left cold and wanting.  You shiver in your bed, still not quite woken, at least not until a playful flick slaps the " + describeCock(player, smallestCock) + " as it pulsates, just hard enough to be noticed without pain.  You start to jolt upright, but are immediately pushed back down by a taloned footclaw pressing down on your shoulder.  You catch a view of feathers and gigantic breasts - it must be the harpy matron, Sophie, come to play with you in the morning.");
    if (player.stats.str >= 70) CView.text("  You could force her off you if you wanted, but you didn't bring her to bed for nothing.");
    CView.text("  Her fluffy behind crashes down on your [legs], just below your waist.  With a familiar coo, she places her other foot on your chest");
    if (player.body.chest.get(0).rating >= 1) CView.text(" and squeezes your " + breastCup(player.body.chest.get(0).rating) + " breast roughy.");
    else CView.text(" and lets the talons dig into your " + skinFurScales(player) + " just enough to hurt a little.");
    ...
    if (SophieFollowerFlags.TIMES_MORNING_SOPHIE_FEMDOMMED > 0) CView.text("In the back of your mind, you know that if her words weren't true you wouldn't have kept sleeping with her and letting her do this to you.");
    else CView.text("In the back of your mind, you almost have to wonder if it's true, or if her lipstick is just now getting into your system...");
    ...
    if (player.stats.sens / 5 + randInt(20) + 1 <= 20) {
        ...
        if (SophieFollowerFlags.UNKNOWN_FLAG_NUMBER_00282 > 0) CView.text("golden");
        else CView.text("pink");
        CView.text("-haired mistress looks down at you graciously as she inexorably slides closer, and you find the intense anticipation is almost a pleasure all its own.  Then, her feverishly-warm delta contacts your sweating cock-skin, the sweet sensation of her juicy snatch against you too much to endure.  Your dainty cock dilates as wide as its diminutive girth will allow, and as you meet Sophie's knowing smile, you moan and release, spraying sperm against her plump clit and ");
        const pregWomb = player.body.wombs.find(Womb.Pregnant);
        if (pregWomb && pregWomb.pregnancy.incubation > 0 && pregWomb.pregnancy.incubation < 150) CView.text("pregnant ");
        else if (player.body.tone >= 70) CView.text("toned ");
        CView.text("belly, most of it deflecting around and over her moist lips in a spray of syrupy white.");
        ...
        if (player.cumQ() >= 500) {
            CView.text("  You cum so much that both you and Sophie wind up");
            if (player.cumQ() < 1000) CView.text(" with huge wet spots over your bodies");
            else CView.text(" soaked from the waist down");
            CView.text(".  It feels so good that neither of you mind.");
        }
        if (player.cumQ() >= 2000) CView.text("  Your bed is a reflective, sticky white as well, practically a lake of seed by now.");
    }
    // {High Sensitivity}
    else {
        ...
        const pregWomb = player.body.wombs.find(Womb.Pregnant);
        if (pregWomb && pregWomb.pregnancy.incubation > 0 && pregWomb.pregnancy.incubation < 150) CView.text(", rolling off the pregnant dome you call your tummy");
        CView.text(".");
        if (player.cumQ() >= 500) CView.text("  You cum for what seems like hours, emptying your [balls] onto yourself, every drop of wasted seed a testament to your pussy-whipped status.");
        if (player.cumQ() >= 1000) CView.text("  A puddle of jizz forms around you before long, mixing with the drippings from her pussy as she rides you into your own soaked bedding.");
        CView.text("  When you're about empty, Sophie says, \"<i>Spurt all that nasty boy-cum out for me, then you can be my lady-pet.</i>\"  Your orgasm redoubles, surging to lance out several more long, thick dribbles of pleasure onto your soaked middle.");
    ...
    if (player.cumQ() >= 1000) CView.text("huge ");
    CView.text("mess you made, wondering why you enjoyed that so much.");

    SophieFollowerFlags.TIMES_MORNING_SOPHIE_FEMDOMMED++;
    player.orgasm();
    player.stats.lib += -1;
    player.stats.sens += 3;
...
// Similar to bimbo sophie
export function sophieFollowerGoesIntoSeas() {
...
export function sophieFertilityExpired() {
...
export function sophieFertilityKnockedUpExpired() {
...
// [special]
function sophieSpecial(player: Character): NextScreenChoices {
    CView.clear();
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(sophieCapacity()));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    CView.text("\"<i>Ahhh, so you do want a bit of Momma Sophie's special treatment... such a brave " + mf(player, "boy", "girl") + ", aren't you?</i>\" She leans in closer as she starts to get rid of your [armor].  \"<i>Very brave... we might have to put you back into place.  Momma Sophie will milk this " + describeCock(player, cockThatFits) + " until the only thing that even allows you to cum anytime in the future will be the fact I drugged you with my lipstick.</i>\"");

    CView.text("\n\nSlowly, she sinks to her knees, fishing out your " + describeCock(player, cockThatFits) + " from your [armor], licking her lips eagerly.  \"<i>First, we cover it with a lot of my special lipstick, so you'll stay hard no matter what.  That, and, well, so you can't resist when Momma Sophie wants you to squirt.</i>\"");

    CView.text("\n\nIndeed, her plush golden lips wrap about your " + describeCock(player, cockThatFits) + ", slathering it with both her saliva and lipstick as she bobs her head up and down on your member, causing it to become painfully hard as you thrust your [hips] instinctively, seeking more of this delicious mouth.  She grumbles, displeased at you moving and tries to hold you in place by grabbing your [ass] and trying to hold you in place; however, this proves to be rather uneffective as you begin to shake from her oral assault, her tongue licking up and down your " + describeCock(player, cockThatFits) + ", covering it completely in a mix of slutty spit and drugged lipstick... as a matter of fact, you feel light in the head");
    if (player.body.balls.count > 0) CView.text(", and your [balls] are just waiting to unload a virile, sticky package down her throat");
    CView.text(".  Sophie seems to sense it and chuckles, pulling her mouth off your " + describeCock(player, cockThatFits) + ", causing you to whine as you seek out her mouth again.  \"<i>There, there, my little cum pump.  Momma Sophie wouldn't be so mean as to leave you and your ");
    if (cockThatFits.area <= 5) CView.text("squirting clitty");
    else if (cockThatFits.area <= 30) CView.text("needy boy-cock");
    else CView.text("giant piece of a meat rod");
    CView.text(" without blowing a nice big load. It just can't go in my mouth... and you should be nice and compliant, not doing things on your own.</i>\"");

    CView.text("\n\nYou feel Sophie simultanously push at your [chest] and pull at your [legs], causing you to slowly move down on your back, as you realize your [armor] has apparently disappeared in the meantime.  The confident harpy matron smirks and giggles while stepping over your body and showing you her plump, jiggly ass as her wet pussy lowers towards your " + describeCock(player, cockThatFits) + ", eager to devour it.  With a wet squelch, you're buried inside her and shuddering beneath her big, wobbly ass.  \"<i>My, my... you really can't hold it in at all, can you? I've barely even put you inside me and you're already so eager to paint my walls white that you're simply going to squirt straight away?</i>\"");
    ...
    if (player.body.balls.count > 0) CView.text("[balls] and ");
    CView.text(describeCock(player, cockThatFits) + " inside Sophie's schlong-milking passage, causing her to moan in satisfaction as the first of your virile cum hits her inner walls.");
    if (player.cumQ() < 250) CView.text("  She milks you for all you're worth.");
    else if (player.cumQ() < 750) CView.text("  She whimpers at how much you've discharged into her.");
    else if (player.cumQ() <= 1500) CView.text("  Your cum soon gushes out into a small puddle from her pussy and onto your [hips].");
    else CView.text("  Her ass is suddenly pushed out more towards you, cum gushing out of her wet cunt as she starts inflating from the sheer amount of cum in your first load.");

    CView.text("\n\n\"<i>Good " + mf(player, "boy", "girl") + " with a nice, obedient ");
    if (cockThatFits.area <= 5) CView.text("clitty");
    else if (cockThatFits.area <= 30) CView.text("pretty cock");
    else CView.text("humongous cunt-stretcher");
    CView.text("... all for me... but you can't go soft, can you?  You just want to squirt more of your virile cum into Momma Sophie.</i>\"  Sophie rolls her hips atop of you, her jizz and girl-cum slickened ass hitting against your pelvis as she ruthlessly uses your overly sensitive " + describeCock(player, cockThatFits) + ", going from the tip to the base as she bounces atop of you.  The drugged lipstick and her constant, magic-powered demands don't allow you to go soft even for a moment as she rides you to completion, her hand working between her thighs at her own clit all the while when finally her pussy grips hard at you, causing you to thrust your hips and moan from sheer sensitivity, unable to achieve another orgasm so quickly.");

    CView.text("\n\nWith a grin, Sophie rises from your " + describeCock(player, cockThatFits) + ", ");
    if (player.cumQ() < 250) CView.text("her pussy dripping your cum down onto the ground");
    else CView.text("your own semen drowning your [hips] and [butt] in a messy cum-puddle as it drips out of her");
    CView.text(", and turns towards you, jiggling her tits right above your [face] as she bends over you, asking, \"<i>Now, wasn't that fun, [name]?  You think we should end now, shouldn't we?  Or do you want to give momma Sophie more of your thick cum so she's definitely pregnant twice over?</i>\"  Drugged and fucked to the edge of your sense of self-worth, you practically beg her to put you inside her again, your golden-tinted member standing proudly in the air, waiting for her hips and pussy to descend.  \"<i>Good answer, very good answer.</i>\"  She descends upon you again, rubbing her pussy against the tip of your " + describeCock(player, cockThatFits) + ", teasing both herself and you before drawing you inside and hilting you down to your [sheath] into her!");
    ...
    if (cockThatFits.area <= 5) CView.text("dainty little fem-cock");
    else if (cockThatFits.area <= 30) CView.text("nice, sweet cum-squirter");
    else CView.text("immense, gorgeous beast of a cock");
    CView.text(" until it's almost broken!  Come on you sweet thing, you know you want to squirt it in me like you're a broken faucet!</i>\"");
    ...
    CView.text("\n\nYou hear Sophie's shlicking vagina swallow your entire " + describeCock(player, cockThatFits) + " and release it repeatedly, her hips and soft, plump ass bouncing off of your [legs] as she continues to fuck herself on your " + describeCock(player, cockThatFits) + " harder and harder.  The way her inner folds lap and massage at your drug-infused cock is exquisite, and it takes every ounce of willpower to indeed stop yourself from cumming inside her repeatedly like you were broken down there.  Finally, Sophie slaps her hips into your hardness, moaning out loud as her cunt squeezes and tugs at your member violently, temporarily stopping the flow of cum out of your " + describeCock(player, cockThatFits) + ", but only for a split second.  It churns and boils in your [balls], raising through your " + describeCock(player, cockThatFits) + " to be fired into Sophie's wet, ravenous cunt and baby-wanting womb.  Sophie's own orgasm and yours blend into one and prolong one another as you release ");
    if (player.cumQ() < 250) CView.text("several ropes");
    else if (player.cumQ() < 750) CView.text("a large, chaotic stream");
    else if (player.cumQ() < 2000) CView.text("wave after wave");
    else CView.text("massive amounts");
    CView.text(" of cum into her, causing her to ");
    if (player.cumQ() < 500) CView.text("moan in satisfaction");
    else if (player.cumQ() < 1000) CView.text("groan as her cunny is unable to hold it all");
    else if (player.cumQ() < 2000) CView.text("stretch atop of you, a cum-filled tummy added to the jiggling tits and soft butt");
    else CView.text("erupt your cum violently around your shaft as you stretch her to pregnancy-looking visage with your cum alone");
    CView.text(".");
    ...
    CView.text(" is probably ready to blow another load very soon, isn't it?  You just can't keep up with Momma Sophie's exquisite pussy, you have to keep cumming and cumming over and over again.</i>\"");
    ...
    CView.text("\n\nShe says that, but every lift and fall of her hips is a torture on your thoroughly milked, overly sensitive " + describeCock(player, cockThatFits) + ".  Her pussy is trying to draw out every drop of fluid you have in your body, and soon, it will succeed at doing just that it seems...");
    ...
    if (player.cumQ() < 250) CView.text("  You only drop a meager few squirts inside of her");
    else if (player.cumQ() < 750) CView.text("  Even your large cum production can't keep up with that pace");
    else CView.text("  Inhuman as your cum factories are, you're only capable of delivering a few big spurts before it slows to a dribble");
    CView.text(".  You thrash and moan and shudder under her, calling her name as she strokes your [hair] and coos, pressing her nipple against your mouth again.  \"<i>Shhh... you've done well for me, little cum-pump... give it all to Momma Sophie... It'll be over in just a moment...</i>\"");
...
// Initial Intro
export function sophieFollowerIntro(player: Character): NextScreenChoices {
...
function getOutSophieRecruitment(player: Character): NextScreenChoices {
    ...
    SophieFollowerFlags.NO_PURE_SOPHIE_RECRUITMENT = 1;
    return { next: campMenu };
}

// Kiss Her
function kissSophieRecruitment(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You shut her up the best way you know how - planting your lips upon her own, knowing full-well of the effects of her lustful lipstick.  Sophie returns the gesture, locking her tongue around yours while she tastes you, grinding her other set of lips against your attentive member.");
    ...
    if (player.body.cocks.length > 1) CView.text(", your second one sliding up between her butt");
    if (player.body.cocks.length > 3) CView.text(" while the others press against her");
    CView.text(".  Sophie cries out in pleasure, almost waking up the entire neighbourhood - which would be a bad thing, considering your neighbours.  You quickly take another taste of her lips, shutting her up while you start to pound upwards into her hungry quim.");
    ...
    return {
        choices: [
            ["Sure", sophieRecruitmentFinale],
            ["No", getOutSophieRecruitment],
        ]
    };
}

// If Sure
function sophieRecruitmentFinale(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Sophie gives you a peck on the cheek, removing herself from your body to prepare for flight.");
    ...
    if (companionsCount() >= 3) CView.text(", sure to introduce her to the other denizens along the way and smooth over any ruffled feathers before things escalate.  Sophie does a good job helping with that.  She seems to have a natural ability to defuse angry situations, something she attributes to raising her many daughters");
    CView.text(".");
    CView.text("\n\n(<b>Sophie is now available in the followers tab!</b>)");
    SophieFollowerFlags.SOPHIE_RECRUITED_PURE = 1;
    return { next: campMenu };
}
...
function sophieIncestInHerCooterOrSomethingIDunno(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Approaching the harpy nest you call out for one of your daughters.  ");
    if (SophieFollowerFlags.SOPHIE_ADULT_KID_COUNT > 1) CView.text("The prospect of their " + mf(player, "father", "mother") + "'s attention causes a small scuffle to break out between your feathery daughters. Thankfully, the small fight is soon ended and one of your daughters flies down and stands before you.  ");
    else CView.text("Your daughter, hearing your call, is more than happy to come to you.  ");
    CView.text("The curvy girl smiles at you, her eyes bright and attentive, positively sparkling with the taboo affection she holds for her beloved daddy.  Smirking to her you give her an appraising look, your eyes wandering up and down her beautiful womanly body.");

    let daughterCap: number = SophieFollowerFlags.SOPHIE_ADULT_KID_COUNT;
    ...
    // second daughter:
        ...
        if (SophieFollowerFlags.DAUGHTER_TWO_BIMBO > 0) CView.text("bimbo ");
        CView.text("lips and blows you a kiss.  She slides her hands over her hips, pushing her taut butt out even more before spanking herself for your pleasure, her lips gasping at the feeling of her self-spank as she tries to work you into a breeding frenzy.");
        ...
        if (SophieFollowerFlags.DAUGHTER_TWO_BIMBO > 0 && SophieFollowerFlags.SOPHIES_DAUGHTERS_DEBIMBOED === 0) CView.text("\"<i>Mmmm... doncha want to just like, pound my sexy ass, " + mf(player, "Daddy", "Mommy") + "?  It's so big, it's just begging to be spanked and squeezed till it's totally red and sensitive</i>\"");
        else CView.text("\"<i>Come on, " + mf(player, "Daddy", "Mommy") + ", touch me, spank me like the bad girl I am, lusting after my " + mf(player, "Daddy", "Mommy") + " like this.  Give it to me hard, spank my big round ass and pound my hot tight cunt like I know you want</i>\"");
    ...
    // third daughter:
        ...
        if (SophieFollowerFlags.DAUGHTER_THREE_BIMBO > 0 && SophieFollowerFlags.SOPHIES_DAUGHTERS_DEBIMBOED === 0) CView.text("\"<i>L-like, did I do a good job, " + mf(player, "Daddy", "Mommy") + "?  I so totally did my best for you.  I know I'm not super sexy like Mom and my sisters... but, " + mf(player, "Daddy", "Mommy") + ".. I will like, still please you, like, totally the best that I can!</i>\"");
        else CView.text("\"<i>W-well, what did you think, " + mf(player, "Dad", "Mommy") + "?  Did I do a good job?  I know I'm not as hot as Mom, but I promise I'll please you as best I can!</i>\"");
        CView.text(" she says, wanting more than anything to be able to please you.");
        ...
        if (SophieFollowerFlags.DAUGHTER_FOUR_BIMBO > 0 && SophieFollowerFlags.SOPHIES_DAUGHTERS_DEBIMBOED === 0) CView.text("\"<i>Come on " + mf(player, "Daddy", "Mommy") + ", can't you see how totally hot I am?  Don't you just want to let loose and like, just fuck me till I scream.</i>\"");
        else CView.text("\"<i>Just look at me, " + mf(player, "Daddy", "Mommy") + ", so curvy, so ready, so hot and wet for you.  Just let loose and show me your love, show me how much you love me, " + mf(player, "Daddy", "Mommy") + ", fuck me hard, make me scream and claim me as yours.</i>\"");
        CView.text(" the horny birdgirl says with a lick of her plump, kissable lips, aching hunger for your incestuous seed.  She wraps her arms around you, pulling her body against yours and sliding a long downy leg around you.  Her position opens her up to you and presses her sodden sex against your crotch.  She's such a lusty, lovely woman, and she wants you so badly, her body begging for you to take her and to seed her fertile body with your potent virility.");
        ...
        // bimbo speech
        if (SophieFollowerFlags.DAUGHTER_FOUR_BIMBO > 0 && SophieFollowerFlags.SOPHIES_DAUGHTERS_DEBIMBOED === 0) CView.text("  \"<i>Ahh... ohhh... oh, " + mf(player, "Daddy", "Mommy") + "!  You like my butt, don't you; isn't it nice?  It's so good to grab and squeeze.  I bet it would be fun to, like, spank it too....</i>\"");
        // [normal speech:]
        else CView.text("  \"<i>Mmmm... Oh, " + mf(player, "Daddy", "Mommy") + ", yessss...  That's it, touch me, touch me and play with me, do whatever you want with me.</i>\"");
        CView.text(" she coos into your ear, temping you even more with the taboo of her incestuous flesh.  With such a demanding offer of pleasure, your daughter so open and ready to receive your pleasure, how could you say no to such a lovely girl?");
    }

    return { next: partial(phaseTwoOfIncest, player, daughterCap) };
}

function phaseTwoOfIncest(player: Character, daughter: number): NextScreenChoices {
    CView.clear();
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(sophieCapacity()));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    daughter++;
    CView.text("Tightening your hold on your curvaceous daughter, you shove her down onto the ground.  Growling with an almost primal edge, you lean down and inspect your prize, hands running over her ");
    if (daughter === 1) CView.text("massive pillowy tits");
    else CView.text("tender perky breasts");
    CView.text(", your fingers sinking into her soft flesh as you caress and squeeze your daughter's bountiful body.  You lean down, kissing and sucking her neck, your lips latching on and sucking hard enough to leave your mark on her skin for the world to see.  She's unwilling to do anything against your lustful advances, mind and body ready and fully accepting whatever naughty things you would do to her.  With how heavily she is breathing and how lusty her moans and calls to you are, your daughter clearly wants the sweet incestuous union between " + mf(player, "father", "mother") + " and daughter even more than you do.  What a sexy and lovely daughter you have here...  at your aggressive touch, she groans and croons for you, beckoning to you to continue your familial onslaught.  She wraps her legs around your body, grinding her hips against you.  \"<i>Yesss, yessss!  More, " + mf(player, "Daddy", "Mommy") + "!</i>\" she squawks in between her breathy moans.");
    ...
    if (daughter === 2) CView.text("massive, round, spankable ass");
    else CView.text("gropable, soft ass");
    CView.text(" and grab her hard, squeezing and pulling her pliant flesh.  You growl to her, giving her rump a hard spank before pushing her down and pinning her beneath you.  Your eyes run over her body, taking in her creamy skin and her colorful feathers, her eyes looking back at you with burning, aching desire.  Not wanting to wait a moment longer, you free yourself from her long, silky legs and strip yourself of your [armor].");
    // [if breasts:
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 1) {
        CView.text("  Your [chest], free from the confines of your gear");
        // if small bust:
        if (player.body.chest.sort(BreastRow.Largest)[0].rating < 3) CView.text(", perk up as the cool air brushes past your hard, aroused nipples.");
        else if (player.body.chest.sort(BreastRow.Largest)[0].rating < 10) CView.text(", bounce perkily, nipples hard as your handful of breast flesh touches the cool air.");
        else CView.text(", spring forth in a wave of soft, squeezable bustiness; the kind of breasts one could curl up with and use as a pillow.");
    }

    CView.text("\n\nBetween your legs, [eachCock] flops free, your " + describeCock(player, cockThatFits) + " achingly hard and ready for your fuck-hungry daughter.  The mass of masculine flesh throbs with your arousal, the insatiable organ longing to be sheathed within the welcoming depths of your daughter's soaking cunt, her tight, velvety folds ready to accept your potent, virile essence.");

    let bimbo: boolean;
    if (SophieFollowerFlags.DAUGHTER_FOUR_BIMBO > 0 && SophieFollowerFlags.SOPHIES_DAUGHTERS_DEBIMBOED === 0)
        bimbo = (
            (
                (daughter === 4 && SophieFollowerFlags.DAUGHTER_FOUR_BIMBO > 0) ||
                (daughter === 3 && SophieFollowerFlags.DAUGHTER_THREE_BIMBO > 0) ||
                (daughter === 2 && SophieFollowerFlags.DAUGHTER_TWO_BIMBO > 0) ||
                (daughter === 1 && SophieFollowerFlags.DAUGHTER_ONE_BIMBO > 0)
            ) &&
            SophieFollowerFlags.SOPHIES_DAUGHTERS_DEBIMBOED === 0);
    ...
    if (player.body.balls.count > 0 && player.body.balls.size < 8) CView.text("Your seed swollen balls clench and lurch under the power of your orgasm, sending forth your creamy load into her waiting depths.  ");
    else if (player.body.balls.count > 0) CView.text("Your huge, breeder balls slap hard against your daughter's soft, creamy skin, letting her feel how heavy and overflowing they are with your virile essence.  The very feeling of your aching orbs against her body makes her shudder in delight at the thought of having your frothy seed bloating her belly.  Clenching with mighty contractions, your fat, bloated sack discharges its gushing virility in great, geyserious bursts of thick, sperm-packed spunk.  ");
    CView.text("Your distended cumvein bulges as your sticky seed dilates your " + describeCockHead(cockThatFits) + ", streaming forth in great lances of egg-raping potency.  The very feeling of your spermy seed flooding her clenching, quivering cunt, drives your curvaceous daughter into an eye rolling orgasm, her body bucking and writhing in raw, mind-bending ecstasy as her body clenches and squeezes you tightly.");
    ...
    // [cum volume low:
    if (player.cumQ() < 250) CView.text("  Squirting over and over into your sweet, lusty girl, you inseminate your fertile daughter, spilling as much of your cream right into her needy, hungry womb as possible.");
    // cum volume medium:
    else if (player.cumQ() < 500) CView.text("  Gushing over and over into your sweet, lusty daughter, you hold yourself tighter against her, making sure you force as much of your spunk into her waiting, hungry womb as you can.");
    // [cum volume high:
    else if (player.cumQ() < 2000) CView.text("  Gush after gush of thick rich cum flows from you, basting your daughter's womb with your virile spunk.  Wedging yourself as tightly against her as you can, you make sure that every drop packs into her fertile belly.  The potent flow of your seed quickly swells her stomach into a cute cum bloated paunch.");
    else CView.text("  Your seed floods your daughter's tight, squeezing cunt like a flood, wave after wave of your thick, frothy spunk forces its way through her convulsing cunt and right into her eager, waiting womb.  Geysers of thick, frothy spunk force their way into her womb, filling it fuller and fuller until her belly swells and swells.  Her once tight, flat tummy rounds out into a healthy round cumbaby, your thick, yogurt-like spunk overflowing from her overstuffed snatch in thick, sticky streams.");
    CView.text("  In a moment of lucidity, you look down at your daughter and see the blissed-out look on her face; her creamy skin red; her eyes rolling up into her head.  Her tongue is lolling out from her plump, cocksucking lips.  The sight of your daughter consumed by such mind bending pleasure, the ecstasy of having her " + mf(player, "father", "mother") + " cumming inside her, only gives you a sense of pride and affection for your sweet daughter.");
    ...
    if (bimbo) CView.text("  \"<i>Oh, " + mf(player, "Daddy", "Mommy") + ", oh, " + mf(player, "Daddy", "Mommy") + "...  So good!  My " + mf(player, "Daddy", "Mommy") + "'s such a hot, sexy " + mf(player, "stud", "herm") + ".  I so totally thought I was going to like, lose my mind for a second.  I love you, " + mf(player, "Daddy", "Mommy") + ".</i>\"");
    else CView.text("  \"<i>Oh, " + mf(player, "Daddy", "Mommy") + ", oh, " + mf(player, "Daddy", "Mommy") + "... so good, you're such a fucking " + mf(player, "stud", "perfect breeder") + "!  I could just get addicted to you.  I love you, " + mf(player, "Daddy", "Mommy") + ",</i>\"");
    CView.text(" she says to you as she kisses you with her soft lips.  Chuckling at your affectionate daughter, you snuggle up with her, spending some sweet, comfortable pillow time with your freshly seeded girl.");
