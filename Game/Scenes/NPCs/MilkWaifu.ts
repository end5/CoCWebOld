...
import { returnToCampUseOneHour, campSlavesMenu } from "../Camp";
...
import { followerEmber, emberMF } from "./EmberScene";
import { sophieFollower } from "./SophieFollowerScene";
...
export const MilkWaifuFlags = {
    ...
    MILKY_TATTOO_COLLARBONE: 0,
    MILKY_TATTOO_SHOULDERS: 0,
    MILKY_TATTOO_LOWERBACK: 0,
    MILKY_TATTOO_BUTT: 0,
    VALARIA_AT_CAMP: 0,
    UNKNOWN_FLAG_NUMBER_00238: 0,
};
User.flags.set(FlagType.MilkWaifu, MilkWaifuFlags);

// New Variables
// " + MilkWaifuFlags.MILK_NAME + ": This slut's name. Can't call her Bath Slut forever. Unless you do.
...
function nameZeMilkBath(player: Character): NextScreenChoices {
    ...
    // Having Named the Girl (Didn't name her Bath Slut)
        ...
        CView.text("You crouch down beside the newly-named girl, brushing a few stray strands of her dark hair from her cheeks.  \"<i>No more Bath Slut.  You're " + MilkWaifuFlags.MILK_NAME + " now.  " + MilkWaifuFlags.MILK_NAME + "'s your name.</i>\"");

        CView.text("\n\n\"<i>" + MilkWaifuFlags.MILK_NAME + "</i>\" she hesitantly repeats.  Looks like she gets it!  \"<i>" + MilkWaifuFlags.MILK_NAME + "!</i>\" she says, more confidently as you encourage her.");
    }
    ...
    if (player.effects.has(StatusEffectType.CampRathazul) && player.effects.get(StatusEffectType.MetRathazul).value2 >= 4) {
        ...
        CView.text("\n\nWith that, Rath hobbles off back to his little laboratory, leaving you with " + MilkWaifuFlags.MILK_NAME + ".");

        CView.text("\n\n\"<i>Rat nice,</i>\" she mumbles, shifting her giant teats around beneath her.");
        // {Plot Item gained: "Super Reducto"}
        player.inventory.keyItems.add("Super Reducto", 0, 0, 0, 0);
...
export function ratducto(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Looking up, you see the old rat alchemist Rathazul approaching, nose buried in an ancient-looking tome.  \"<i>Good news, [name]!</i>\" he calls, just before tripping over the prone milkmaid, going sprawling across the ground.");
...
export function milkyMenu(player: Character): NextScreenChoices {
    CView.clear();
    if (MilkWaifuFlags.FOLLOWER_AT_FARM_BATH_GIRL === 0) {
        CView.text("You wander over to " + MilkWaifuFlags.MILK_NAME + "'s pool, and find the dusky girl sitting at its rim, ");
        if (MilkWaifuFlags.MILK_SIZE === 0) CView.text("lying face-down on her massive rack, her plump little ass sticking up in the air for all to see.  Seeing you approach, she brightens up and shifts her titanic bust around to face you, all bright-eyes and smiles.  \"<i>Milk time?</i>\"");
        else if (MilkWaifuFlags.MILK_SIZE === 1) CView.text("arms crossed under her hefty, milky bust to support their still-sizable weight.  She smiles as you approach, able to stand up under her own power to give you a hug, milky tits pressed tight against you.  \"<i>I-is it milk time?</i>\" she asks, cupping her tits for you.");
        else CView.text("her long, tanned legs dangling into the tub.  She jumps up when you approach, and though still a bit unsteady with such easy movements, she's quick to leap into your arms, nuzzling into your neck and pressing her firm, perky DD-cup breasts against you, little trickles of milk staining your chest through the little shirt she's wearing.  \"<i>[name],</i>\" she purrs happily.");
    }
    else {
        if (MilkWaifuFlags.MILK_SIZE === 0) {
            CView.text("[bathgirlname] shakes vaguely out of her boob daze as you pick your way over to her.");

            CView.text("\n\n“<i>Bath time?</i>”");
        }
        else {
            CView.text("[bathgirlname] smiles at you as you pick your way over to her.");

            CView.text("\n\n“<i>Hello [name]. Is there something you need?</i>”");
        }
    }

    if (MilkWaifuFlags.FOLLOWER_AT_FARM_BATH_GIRL === 0 && MilkWaifuFlags.FARM_CORRUPTION_STARTED === 1) {
        if (MilkWaifuFlags.MILK_SIZE === 0) {
    ...
    if (MilkWaifuFlags.FOLLOWER_AT_FARM_BATH_GIRL === 1 && MilkWaifuFlags.MILK_SIZE === 0) {
        CView.text("\n\n<b>(You'd have to find some way to reduce the size [bathgirlname]'s massive tits before you could send her back to )</b>");
    }
    ...
    choices[0] = ["Appearance", milkWriteFuAppearance];
    if (MilkWaifuFlags.MILK_SIZE > 0) choices[1] = ["Milk Time!", nyanCatMilkTime];
    if (MilkWaifuFlags.MILK_SIZE < 2) choices[2] = ["Milk Bath", milkBathTime];
    if (MilkWaifuFlags.MILK_SIZE < 2 && player.inventory.keyItems.has("Super Reducto")) choices[3] = ["Reducto", superReductoUsage];
    if (MilkWaifuFlags.MILK_SIZE > 0 && player.stats.lust >= 33 && player.body.cocks.length > 0) choices[4] = ["Titfuck", titFuckDatMilkSlut];

    if (MilkWaifuFlags.FOLLOWER_AT_FARM_BATH_GIRL === 0 && MilkWaifuFlags.MILK_SIZE > 0 && MilkWaifuFlags.FARM_CORRUPTION_STARTED === 1) choices[5] = ["Farm Work", sendToFarm];
    if (MilkWaifuFlags.FOLLOWER_AT_FARM_BATH_GIRL === 1 && MilkWaifuFlags.MILK_SIZE > 0) choices[5] = ["Go Camp", backToCamp];

    if (MilkWaifuFlags.FOLLOWER_AT_FARM_BATH_GIRL === 0) choices[9] = ["Back", campSlavesMenu];
    if (MilkWaifuFlags.FOLLOWER_AT_FARM_BATH_GIRL === 1) choices[9] = ["Back", rootScene];
...
function sendToFarm(player: Character): NextScreenChoices {
    ...
    CView.text("You describe to [bathgirlname] the lake, and the farm which is situated close to it. Gently you tell her you want her to go there, present herself to the dog woman who owns it, and do as she says.");

    CView.text("“<i>Ok,</i>” says [bathgirlname], cautiously testing the idea out. “<i>You’ll come and visit sometimes, right?</i>” Of course. Mollified, the former sand witch slave gets to her feet and cautiously picks her way towards the lake. She won’t be much use protection-wise but she’ll give your milk production a boost.");
    ...
    MilkWaifuFlags.FOLLOWER_AT_FARM_BATH_GIRL = 1;
...
function backToCamp(player: Character): NextScreenChoices {
    ...
    // TODO
    CView.text("“<i>I want you to head on back to camp,</i>” you tell her. “<i>You’ll be more useful to me there.</i>” [bathgirlName]’s brow crinkles but she seems to accept your instruction.");

    CView.text("“<i>As you wish.</i>” She wipes her hands before walking slowly down and out of the farm’s gate.");

    MilkWaifuFlags.FOLLOWER_AT_FARM_BATH_GIRL = 0;

    return { next: rootScene };
}

// Appearance
function milkWriteFuAppearance(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("" + MilkWaifuFlags.MILK_NAME + " is a five-foot-five human female, with darkly tanned skin and ebony hair.  ");
    if (MilkWaifuFlags.MILK_SIZE >= 2) CView.text("Over her supple body, she wears a simple cloth shirt which cuts off just above her knees, hiding her newly-gained modesty, with a pair of silk painties and a simple bra beneath it.  ");
    CView.text("She has a human face with rich, smooth skin dotted with the faintest trace of freckles.  Her hair is long and full, hanging down her back to her waist.  She's got a curvaceous body, bust aside, with supple hips and thighs, and a big butt you can just sink your fingers into.");

    // {If GIANT boobs:
    if (MilkWaifuFlags.MILK_SIZE === 0) CView.text("\n\nHer most distinguishing feature, though, is her massive bust.  Enough to drag her down onto all fours without support, " + MilkWaifuFlags.MILK_NAME + "'s rack is absolutely gigantic.  Her areola are the size of plates, constantly dribbling motherly fluids with every breath she takes.  The slightest touch is enough to get a stream flowing from " + MilkWaifuFlags.MILK_NAME + "'s titanic teats, enough to fill her entire pool with more to spare.  Those can't be too comfortable, you reckon, even if they are almost mesmerizing in their beauty and potential for sexual pleasure.");
    else if (MilkWaifuFlags.MILK_SIZE === 1) CView.text("\n\nThough you've reduced them <i>considerably</i>, " + MilkWaifuFlags.MILK_NAME + "'s most eye-catching feature is still her tremendous bosom.  Those huge tits of hers would look more normal on a cowgirl than a human girl, and a rather petite one at that.  Her areola are palm-sized and tipped with a pair of prominent nipples, always eager to release their motherly fluids.  She can easily fill a tub with those things, and she alone could provide enough to feed a small village, producing more than the all the cows of Ingnam combined every day.");
    else {
        CView.text("\n\nThanks to your intervention, " + MilkWaifuFlags.MILK_NAME + "'s breasts aren't so cripplingly large.  Reduced to a DD-cup, you've even managed to scrape together some clothes that actually fit her.  In her current state, you might even call her bust perky, her milk-laden breasts both firm and yet so delightfully soft; you can easily sink your fingers into them, or bury your face");
        if (player.body.cocks.length > 0) CView.text(" or cock");
        CView.text(" between her pillowy bosoms.  Despite the reduction in size, " + MilkWaifuFlags.MILK_NAME + " is happy to tell you that she's still got plenty of milk inside her, though perhaps not quite as much as before: she can't quite fill the pool all by herself, but now even short milkings leave her blissfully empty for hours afterwards.  She's not an entire farm unto herself now and she seems fairly pleased by this, all things considered.");
    }
    ...
    if (hasTattoo("milky")) {
        ...
        else if (numMilkyButterflyTats() === 4) {
            CView.text("She is covered from head to tail in tattooed butterflies, as if the pretty insects are attracted to her chocolate skin. When she moves she does it with an extra bounce and flick of the head, admiring how she looks as she goes.");
        }
        else {
            if (numTattoos("milky") > 1) CView.text("She has the following tattoos emblazoned across her body:\n");
            else CView.text("She has ");

            if (MilkWaifuFlags.MILKY_TATTOO_COLLARBONE !== 0) CView.text(MilkWaifuFlags.MILKY_TATTOO_COLLARBONE + "\n");
            if (MilkWaifuFlags.MILKY_TATTOO_SHOULDERS !== 0) CView.text(MilkWaifuFlags.MILKY_TATTOO_SHOULDERS + "\n");
            if (MilkWaifuFlags.MILKY_TATTOO_LOWERBACK !== 0) CView.text(MilkWaifuFlags.MILKY_TATTOO_LOWERBACK + "\n");
            if (MilkWaifuFlags.MILKY_TATTOO_BUTT !== 0) CView.text(MilkWaifuFlags.MILKY_TATTOO_BUTT + "\n");
        }
    }

    return { next: milkyMenu };
}

// Reducto
function superReductoUsage(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You tell " + MilkWaifuFlags.MILK_NAME + " that it's about time she got rid of those massive tits of hers.  She cocks her head to the side, looking at you without comprehension.  You fish the giant vial of Reducto Rath gave you from your pack and hold it up for her to examine.  She hesitantly takes it from your hand and rolls it around, sniffing at the foul-smelling stuff.  She turns up her nose at it.");

    CView.text("\n\n\"<i>Come on, you'll feel a lot better afterwards, I promise,</i>\" you say, crouching down beside her, locking onto " + MilkWaifuFlags.MILK_NAME + "'s big brown eyes.");

    CView.text("\n\nIf by nothing else than your soothing tone, " + MilkWaifuFlags.MILK_NAME + " stops fussing about the Reducto and does her best to curl up in your lap, giving you free access to her big ol' rack.  You tussle her hair and start thinking about how to go about this.");
    ...
    if (MilkWaifuFlags.MILK_SIZE === 0) {
        ...
        // {If Rath is at camp:
        CView.text("Rathazul approaches, clearly having seen the preceeding events.  He smiles warmly at the two of you, clearly happy to have helped.  Wordlessly, the old rat produces a folded set of garments: a cloth shirt with intricate embroidery, and a pair of expensive-looking women's underthings, handing them over to " + MilkWaifuFlags.MILK_NAME + ".  She looks from the gift of clothing to the alchemist, and taking them, plants an affectionate kiss on his old brow.  He chuckles, blushing.  \"<i>I'm sorry I don't have more to give you, poor dear,</i>\" Rath says, starting to shuffle away.  \"<i>Still, I think I've held onto those long enough.</i>\"");

        CView.text("\n\nYou help " + MilkWaifuFlags.MILK_NAME + " into her new clothes, probably the first she's been able to wear in years, and leave her to get used to her new body - though not before she draws you into a long kiss, holding you tight once again and whispering her heartfelt thanks.");
        // If no Rath: "You tell " + MilkWaifuFlags.MILK_NAME + " to wait a moment, and go digging through your possessions.  It takes a few minutes, but eventually you find some comfortable-looking clothing.  She takes them eagerly, saying she'll trim them down to her size as soon as she's got herself settled down: it's a lot to take in all at once, and she seems eager to experiment with her new, slender body, walking all over camp with a gay smile.  You leave her to exercise, but not before she draws you into a long kiss, holding you tight once again and whispering her heartfelt thanks."}
    }
    MilkWaifuFlags.MILK_SIZE++;
    return { next: returnToCampUseOneHour };
}

// Milk Bath (HHH or Giant boobs only)
function milkBathTime(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Tussling " + MilkWaifuFlags.MILK_NAME + "'s hair, you tell her her breasts look a bit full.  She smiles up at you eagerly as you disrobe.  Once nude, you hop down inside and say, \"<i>Bath Time.</i>\"");
    CView.text("\n\nWith trembling anticipation, " + MilkWaifuFlags.MILK_NAME + " reaches down for her ");
    if (MilkWaifuFlags.MILK_SIZE === 0) CView.text("teat-like");
    else CView.text("prominent");
    CView.text(" nipples.  Her areola bead with white perspiration in anticipation, and the woman's hands eagerly set to work on them, starting to draw out the first hints of lactation, pouring them into the pool around you.  You drop the plug into the drain and look up.  Cooing in delight, the huge-breasted girl finally manages to get her shivering fingertips around each of her aching milk-spouts.  She massages her nipple-flesh for a moment, her eyes lidded and heavy from pleasure, and she releases the first heavy torrent of white into the tub.  As she milks herself, " + MilkWaifuFlags.MILK_NAME + "'s eyes seem to go vacant, overwhelmed by pleasure, and her mouth is too busy making sighs of relief to speak.");
    CView.text("\n\nPearly fluid quickly fills the first few inches of the tub, pouring as it is in numerous forking streams from its mocha spouts.  Deft hands massage the soft female flesh with smooth, unbroken motions, squeezing each teat from base to tip before retreating back to the bottom.  The steady back-and-forth motions cause the streams to rise and fall to the tempo, but the flow stays thick and steady enough to splatter your [hips] with white.  You relax against one of the benches at the side of the pool and idly trace your hands through the \"water,\" enjoying the feeling of the milk on your " + skinFurScales(player) + " as it rises higher and higher.  " + MilkWaifuFlags.MILK_NAME + ", your only companion, continues to knead her ");
    if (MilkWaifuFlags.MILK_SIZE === 0) CView.text("gigantic");
    else CView.text("cow-like");
    CView.text(" breasts as you watch, and you have to admit, you feel a sexual thrill sliding down your spine as you watch her heafty bosoms work to fill your tub.");
    ...
    if (player.gender === 1) CView.text(describeCocksLight(player));
    else if (player.gender === 2) CView.text("[vagina]");
    else if (player.gender === 3) CView.text(describeCocksLight(player) + " and [vagina]");
    else CView.text("[butt]");
    CView.text(", you resist the urge to touch yourself in a sexual way and focus on what you wanted to do - bathe.  " + MilkWaifuFlags.MILK_NAME + " lets out a satisfied groan, her breasts finally seeming to wind down somewhat as the milk reaches your [chest], though by now she's positively quivering with pleasure, mindlessly working her nipples with eyes rolled back in her head.");
    // {If GIANT boobs:
    ...
    CView.text("\n\n" + MilkWaifuFlags.MILK_NAME + " pulls back, licking her puffy lips and smelling strongly of female arousal, though obviously satisfied.  She whimpers, \"<i>I love bath time,</i>\" before ");
    if (MilkWaifuFlags.MILK_SIZE === 0) CView.text("starting to shift her breasts' bulk toward a comfortable spot for a nap");
    else CView.text("struggling to her feet, legs shaky with the afterglow of a might boobgasm");
    CView.text(".");
    if (player.stats.lust >= 33) CView.text("  You could probably masturbate in the tub if you wanted to, or maybe pull the dusky milk-maid in for company.");
...
function pullInZeMilkGirl(player: Character): NextScreenChoices {
    ...
    else CView.text("sexless crotch");
    CView.text(", still burning hotly with your desires");
    ...
    if (player.body.cocks.length > 0) {
        ...
        if (player.body.balls.count > 0) CView.text(", her other hand cupping your [balls], rolling the " + numToCardinalText(player.body.balls.count) + " orbs in her palm with delightful dexterity");
    ...
    if (player.body.vaginas.length > 0) {
        ...
        if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 1) {
            CView.text("  She cups your breasts, having to reach so far around both your rack and hers that she's straining her arms to rub your [nipples], but she does so valiantly, stroking them with her incredibly deft fingers.");
            if (player.lactationQ() >= 100) CView.text("  A spurt of milk escapes your own full jugs, joining the pool-full of your friend's.  She gasps with delight, quickly nuzzling herself into your back and going to work.  She milks you just as she would herself, letting the hefty flow of your motherly fluids pour into the pool, odd trickles smearing down your chest, staining your chest as white as her own.");
        }
    }
    // [Fuck Her](PC must have gender; if cooch, also C+ cups) [Don't]

    const choices: ScreenChoice[] = [];
    choices[4] = ["Don't", dontFuckTheMilkWaifu];
    if (player.body.cocks.length > 0) choices[0] = ["Fuck Her", fuckTheMilkWaifu];
    if (player.body.vaginas.length > 0 && player.body.chest.sort(BreastRow.Largest)[0].rating >= 3) choices[1] = ["LesboFuck", beARugMunchingMilkDyke];

    return { choices };
}

// 	[Don't]
function dontFuckTheMilkWaifu(player: Character): NextScreenChoices {
    CView.clear();
...
function fuckTheMilkWaifu(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You turn around and pull the milk-slut against you, her massive teats pressing hard against your [chest] until they spurt.  You stroke her cheeks, bringing her lips up to yours.  Her hand finds your [cock] again, stroking you with mounting speed as your tongue finds its way into her mouth, your hands wandering down to grope her sizable ass and flared, breeder's hips.  Your lover sighs heavily, breath filled with lust as you push her up against the rim of the tub, her legs spreading wide for easy access to her milk-lubed cunt.  She locks her arms around your shoulders, moaning happily as you press into her, your [cock] slipping easily into her sodden box.");

    CView.text("\n\nSubmerged beneath a sea of creamy milk, it's so very, very easy to slide into " + MilkWaifuFlags.MILK_NAME + ", ");
    if (player.body.cocks.get(0).area < 20) CView.text("pushing your few inches into her until your hips join, her nice and loose cunt easily taking your length");
    else if (player.body.cocks.get(0).area < 50) CView.text("hilting her in one long stroke");
...
function beARugMunchingMilkDyke(player: Character): NextScreenChoices {
    ...
    if (player.lactationQ() >= 200) CView.text(", milking you with skill beyond anything you've ever experienced before; and why not, when her entire existence revolved around that self-same skill?");
    else CView.text(".");
    ...
    CView.text(".  You run your fingers through the girl's hair, urging her sexual skills on as your chest heaves and quavers, riding out the massive boobgasm as femcum spurts from your cunt and into the milky pool below.");
...
// [Drink & Masturbate]
function drinkAndFapturbate(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Wait,</i>\" you call out to the ebony woman, letting the milk obscure your hands as you begin to masturbate, \"I want a drink.</i>\"  Sheepishly, " + MilkWaifuFlags.MILK_NAME + " obligingly shifts back toward you, presenting her huge teats.");
    ...
    if (MilkWaifuFlags.MILK_SIZE === 0) CView.text("a dinner plate");
    else CView.text("an open palm");
    CView.text(" but far more exciting.  After taking a few swallows of her body's milky treat, you reach down to your ");
    if (player.body.vaginas.length > 0) CView.text("[vagina] and idly stroke your puffy, lust-engorged vulva");
    else {
        CView.text("[cock] and idly stroke the turgid mass");
    }
    ...
    if (player.body.cocks.length > 0) {
        CView.text("  [EachCock] release its own gooey load, sputtering and spurting to add to the pearly deluge.");
        if (player.cumQ() >= 1500) CView.text("  With every torrent of seed you release, you can see it lift partway out of the tub, propelled by your incredible virility towards the nearest female specimen.");
        if (player.cumQ() >= 4000) CView.text("  Soon, the tub's fluid contents break out of their confines to stain your companion's dusky flesh white, an alabaster glaze that would entice you to further feats of debauchery were it not for the pleased contentment your maleness now radiates.");
    }
    if (player.body.vaginas.length > 0) CView.text("  Meanwhile, your juiced-up cunny feels like it's doing backflips inside you, contorting and squeezing as it gushes with fluid, feminine joy.  One brush on your [clit] knocks your [legs] out from underneath you, but thankfully, you float out the rest of your orgasm.");
...
function nyanCatMilkTime(player: Character): NextScreenChoices {
    ...
    if (MilkWaifuFlags.MILK_SIZE === 1) {
        ...
        if (isabellaFollower()) {
            ...
            CView.text("</i>\" You look up from your task to see Isabella looming over the pool, hands on her wide hips.  \"<i>You are going MUCH too hard on her, poor ");
            if (isabellaAccent()) CView.text("zing");
            else CView.text("thing");
            CView.text(".  Let Isabella show you how ");
            if (isabellaAccent()) CView.text("es ist");
            else CView.text("it's");
            CView.text(" done.</i>\"  You find yourself roughly pushed aside as Isabella jumps in with you, tossing her skirt and blouse aside as she grasps " + MilkWaifuFlags.MILK_NAME + "'s teats, massaging them much more gently.  The milk girl's eyes roll back in her head as Isabella sets to work, though it gets harder and harder as the milk flows up to her thighs and chest.");
        }

        // If PC has ONLY KIha or if PC dun have Izzy, Sophie, OR Kiha:
        if (!isabellaFollower() || !sophieFollower()) CView.text("\n\nIt takes a good long while to get " + MilkWaifuFlags.MILK_NAME + "'s bloated tits under control, but nearly an hour later you've milked her as well as you can for now.  Sopping wet, you pull yourself out of the pool and grab a towel.  With her tits lightened for the moment, " + MilkWaifuFlags.MILK_NAME + " reaches up and pulls you down to her, just long enough to plant a kiss on your cheek and whisper, \"<i>Thank you, [name].  That felt good.</i>\"");

        // If PC has Sophie or Isabella:
        else CView.text("\n\nThough you didn't intend for this to turn communal, " + MilkWaifuFlags.MILK_NAME + " certainly has more than enough to share.  You relax as the last trickles of milk pour into the pool, her breasts looking positively deflated.  You decide to leave the plug in for your friends as you clamber out, figuring they'll want to stockpile a little for the day before you drain it.  With her tits lightened for the moment, " + MilkWaifuFlags.MILK_NAME + " reaches up and pulls you down to her, just long enough to plant a kiss on your cheek and whisper, \"<i>Thank you, [name].  That felt good.</i>\"");
    ...
    // Milk Time!  (DD Boobs Ver.)
        ...
        CView.text("You sit yourself down with " + MilkWaifuFlags.MILK_NAME + " and ask if she'd like a good milking.  Her eyes brighten at the suggestion, and she whispers \"<i>Yes please,</i>\" already pulling off her shirt.  You help her get her top and bra off, leaving her full, perky breasts bare in the cool sun.  You slip behind her, running your hands across her sun-kissed skin, tracing your fingers from hips to chest; " + MilkWaifuFlags.MILK_NAME + " shivers at your slightest touch, whining lustily as your hands wrap around her milk-laden mounds.  Her reddened nipples are rock-hard by the time your fingers brush up against them, perspiring clear pearlescent drops as you circle her wide areola.");
        CView.text("\n\n" + MilkWaifuFlags.MILK_NAME + " whimpers as you squeeze a tiny trickle from her teats, the white fluid squirting out in a powerful arc, staining the campground for the briefest moment before the hungry earth swallows it up, leaving no trace but a darkened patch at the girl's knees.  Gently, you start to squeeze and knead " + MilkWaifuFlags.MILK_NAME + "'s breasts, holding her back against you as you massage the heavy burden from her seemingly unending reserves.  A steady stream of milk pours from her chest, dribbling down between your fingers and her flat belly.  Soon a pool's formed around your [legs], the barren earth not nearly fast enough to consume the bounty flowing from your friend.");

        CView.text("\n\nWith each caress and squeeze, " + MilkWaifuFlags.MILK_NAME + " shivers and shakes, her chest soon heaving with her ragged, pleasure-strained breath.  You can see her lustful secretions stain her paintes as her hidden cunt quivers, desperate for attention as you milk her over-sensitive mammaries.  Her breathing becomes more irregular, her long legs quaking and eyes rolling back as you squeeze out gallon after gallon.  Her whimpers of pleasure become outright cries, piercing the quiet of camp with orgasmic moans and shrill screams as your fingers work their magic on her, sending shockwaves of pleasure from hefty teats to sex-addled mind  that make her entire body convulse and writhe at your touch.");

        CView.text("\n\n\"<i>N-no more.  Can't...  take it...  please.  Ahhhhh,</i>\" " + MilkWaifuFlags.MILK_NAME + " cries, squirming in your embrace, your hands firmly locked on her tits, milking her for every drop.  She's in for a mighty boobgasm in mere moments: you could soothe her through it with a gentle suckle or finish her off normally.");
        // If PC has a cock that fits:
        if (player.body.cocks.find(Cock.CockThatFits(50)) && player.body.cocks.length > 0) CView.text("  Then again, maybe you could hike her panties down and give her a good fucking to send her over the edge.  In her state, she certainly won't mind!");

        const choices: ScreenChoice[] = [];
        choices[0] = ["Normal", finishMilkNormall];
        choices[1] = ["Suckle", suckleDatMilk];
        if (player.body.cocks.find(Cock.CockThatFits(50)) && player.body.cocks.length > 0) choices[2] = ["Fuck", fuckDatMilkSlat];
...
// [Finish Normally]
function finishMilkNormall(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You whisper a few soothing words into " + MilkWaifuFlags.MILK_NAME + "'s ear, but are unrelenting in your task.  With squeezing and kneading fingers, you continue to coax the milk from her tits until she's practically white with flowing cream.  A few minutes later though, you can physically feel her explode over the edge of bliss: the stream of milk from her stiff nipples doubles, blasting an arc several feet long as her voice breaks, a shrill cry to the heavens; " + MilkWaifuFlags.MILK_NAME + "'s entire body shudders, her legs nearly giving out as the boobgasm rocks her slender frame.");

    CView.text("\n\nAs soon as the explosive boobgasm subsides, she collapses back against you, chest heaving with pleasured exhaustion.  The milky stream trickles down to naught, her breasts momentarily depleted.  " + MilkWaifuFlags.MILK_NAME + " looks up at you and smiles ever so slightly, caressing your cheek before cupping her obviously-sore teats and staggering to her feet, searching for a towel.");
...
function fuckDatMilkSlat(player: Character): NextScreenChoices {
    CView.clear();
    const cockThatFits = player.body.cocks.find(Cock.CockThatFits(50));
    CView.text("Sitting behind the milky girl, chest pressed to her back tight enough to feel her every breath, you can't help but feel " + MilkWaifuFlags.MILK_NAME + "'s full, round ass brushing against your " + describeCock(player, cockThatFits) + ".  Responding to her touch, your prick begins to stiffen, filling out through your [armor] and into the valley of " + MilkWaifuFlags.MILK_NAME + "'s ass.  She gasps ever so slightly when she feels your prick pushing against her silky undergarments, but you can feel her heart race, her flesh heating as she starts to move her ass more deliberately, rubbing you from stem to head.");

    CView.text("\n\nOne of your hands drifts down from " + MilkWaifuFlags.MILK_NAME + "'s teats, fingers tracing along her supple skin and ample curves to the hem of her panties.  You pull them down with one smooth motion, baring the full mounds of her dusky ass cheeks.  She pushes back immediately, humping up and down your rod as you work to free yourself from your [armor].  Finally, your " + describeCock(player, cockThatFits) + " pops free from its binds, only to be instantly buried in " + MilkWaifuFlags.MILK_NAME + "'s rear cleavage; she gives a happy, girlish giggle when your throbbing rod pushes through her valley, practically bouncing on your cock.  You wrap your arms around her waist, one hand working her still-needy teats as the other dives between her legs, easily slipping a few fingers into her sodden gash.  " + MilkWaifuFlags.MILK_NAME + " moans lustily as your digits enter her, biting her lower lip when your fingertips caress her inner walls and spasming muscles, soon soaked in her feminine fluids.  Your thumb swirls around the bud of her clit, drawing ragged gasps of pleasure from her lips until you silence her with a kiss, driving your tongue into her mouth as your fingers assault her cunt, fingering her until she's nice, wet, and ready.");

    CView.text("\n\nYou shift forward ever so slightly, dragging your " + describeCock(player, cockThatFits) + " from " + MilkWaifuFlags.MILK_NAME + "'s butt-cleavage and into the welcoming embrace of her womanhood.  You both moan with lust as your " + describeCockHead(cockThatFits) + " presses into her, parting the folds of her cunny to feel the hot touch of her walls kissing your tip.  You slide into her with measured ease, " + describeCock(player, cockThatFits) + " spreading her walls wide as your hips move to meet, your groin pushing against her bubble butt, flesh yielding as you try and slide as much cockflesh into her eager channel as you can.");
    CView.text("\n\nBy the time you've hilted " + MilkWaifuFlags.MILK_NAME + " she's a mess, cunt drooling obscenely and tongue hanging from her mouth.  Her chest heaves in your hands as your fingers wrap back around her cherry-red nipples, her milk coming in erratic spurts as she undulates on your cock, hips and ass starting to bounce atop you.  You move to match her, hammering your [hips] home to meet her, thrusting your " + describeCock(player, cockThatFits) + " into her clinging depths.  " + MilkWaifuFlags.MILK_NAME + " cups her hands to her breasts, pooling up handfuls of milk and splashing them onto your prick and her gash, nearly dousing the heat of your lust with the rush of cool cream; but another jack-hammer thrust shows her milk to be a magnificent lubricant, letting you slip and slide into her with ease.  Soon you're both moaning your lust, pleasured gasps and sighs echoing through the camp as you fuck " + MilkWaifuFlags.MILK_NAME + "'s dripping cunt.");

    CView.text("\n\nYou can feel your orgasm mounting, surging on as your " + describeCock(player, cockThatFits) + " hammers relentlessly into " + MilkWaifuFlags.MILK_NAME + ".  With an animalistic roar, you push her down onto all fours and grab her ass for leverage.  She squeals when you push her down, but she recovers in an instant, wiggling her ass tantalizingly as your fingers sink into her pliant flesh.  With a vision full of that big, soft ass swaying as your dick pounds " + MilkWaifuFlags.MILK_NAME + "'s, you can't help but cum.  You grint your teeth and give " + MilkWaifuFlags.MILK_NAME + " a hard spank right on the ass, making her shriek with pleasure and pain, her cunt squeezing your " + describeCock(player, cockThatFits) + " at just the right moment: you shoot a load of white-hot seed straight into her womb, dick buried to the hilt inside her.  Around your spasming cock, " + MilkWaifuFlags.MILK_NAME + "'s quim quivers and contracts, milking you for every drop; her teats let loose a wild stream of milk which pools around you, nearly hiding the excess spooge that pours from her battered cunny to stain the earth.");

    CView.text("\n\nWhen your dick's finally depleted its load, you pull out with a contented sigh, wiping the last drops of spunk off on " + MilkWaifuFlags.MILK_NAME + "'s thigh.  She rolls over, a lust-dazed smile on her face, idly fingering her well-stuffed cunt and caressing her ample bosoms.  A momentary come-hither look from her beckons you over, and " + MilkWaifuFlags.MILK_NAME + " pulls you into her arms with a girlish giggle, resting you on your back and nuzzling her cheek on your [chest].  You drift off into pleasant repose, both sexually contented and your lover well-milked.");
    player.orgasm();
    player.stats.sens += -1;
