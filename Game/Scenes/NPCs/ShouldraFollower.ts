export class ShouldraFollower extends NPCAwareContent {
	...
	private shouldraTongueLicksPCs() {
		...
		if (Flags.list[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] === 0) {
			...
			if (player.torso.head.face.tongue.type > TongueType.HUMAN) DisplayText("some of ");
		...
		//if demon tongue
		else if (player.torso.head.face.tongue.type === TongueType.DEMONIC) {
	...
	private shouldraReductoMenu() {
		...
		if (player.torso.cocks.count > 0 && player.torso.cocks.sort(Cock.LongestCocks)[0].length > 4) cock = shouldraMakesCocksDisappear;
