...
import { growSmallestBreastRow, shrinkTits } from "../../Modifiers/BreastModifier";
import { growCock } from "../../Modifiers/CockModifier";
import { trace } from "console";
import { isHalloween } from "../../Utilities/Dates";
...
export const ShouldraFollowerFlags = {
    SHOULDRA_FOLLOWER_STATE: 0,
    SHOULDRA_SLEEP_TIMER: 0,
    SHOULDRA_PLOT_COUNTDOWN: 0,
    SHOULDRA_MET_VALA: 0,
    SHOULDRA_GENDERLESS_FUCK_COUNT: 0,
    GENDERLESS_MASTURBATION_WITH_GHOST_COUNT: 0,
    ZETSUKO_MET: 0,
    SHOULDRA_TONGUE_LICKS_TIMES: 0,
    SHOULDRA_PLOT_LEVEL: 0,
    SHOULDRA_TALK_NEEDED: 0,
    FREED_VALA: 0,
    SHOULDRA_MAGIC_COOLDOWN: 0,
    SHOULDRA_TIMES_NIGHT_RAPED_PC: 0,
    IZMA_CHILDREN_SHARKGIRLS: 0,
    IZMA_CHILDREN_TIGERSHARKS: 0,
    SHOULDRA_WORM_SCENE_COUNTER: 0,
    SHOULDRA_EXGARTUDRAMA: 0,
    SHOULDRA_BAKERY_TIMES: 0,
    PC_FETISH: 0,
};
User.flags.set(FlagType.ShouldraFollower, ShouldraFollowerFlags);
...
export function followerShouldra(): boolean {
    return ShouldraFollowerFlags.SHOULDRA_FOLLOWER_STATE === 1;
}

function shouldraSleeping(changes: number = 0, reset: boolean = false): boolean {
    // Adjust sleep timer
    if (reset) {
        ShouldraFollowerFlags.SHOULDRA_SLEEP_TIMER = changes;
    }
    else ShouldraFollowerFlags.SHOULDRA_SLEEP_TIMER += changes;
    if (ShouldraFollowerFlags.SHOULDRA_SLEEP_TIMER < 0) ShouldraFollowerFlags.SHOULDRA_SLEEP_TIMER = 0;
    if (ShouldraFollowerFlags.SHOULDRA_SLEEP_TIMER > 48) ShouldraFollowerFlags.SHOULDRA_SLEEP_TIMER = 48;
    // Is she sleeping?
    if (ShouldraFollowerFlags.SHOULDRA_SLEEP_TIMER > 0) return true;
    return false;
}

// Initial Question
function initialShouldersRecruitment(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("The ghost girl shows up far earlier than normal, this time around; you usually have to wait for her to make an appearance, but this time, it's as if she's seeking you.  You wait for her to say something, cast a spell, punch you in the face, anything.  She just keeps hesitating and glancing at you.  \"<i>Say,</i>\" she begins nervously, \"<i>...what would you say to us... hanging out some more?  Like, I mean... you know, me going with you.  Doing stuff and things.  That might be... nice...</i>\"");
    ...
    return { yes: recruitShouldraForCampSexyTimes, no: noFollowerShoulders };
}
// NO OF COURSE NOT THAT SHIT'S 2SCARY4ME
function noFollowerShoulders(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("Somewhat bluntly, you inform her of your lack of interest regarding having a ghostly tagalong.  Though she looks a bit disappointed, she nods, scuffing her shoes against the ground and huffing a little sigh.  \"<i>At least I'll see you later, hey?</i>\"");
...
function recruitShouldraForCampSexyTimes(player: Character): NextScreenChoices {
    ...
    ShouldraFollowerFlags.SHOULDRA_FOLLOWER_STATE = .5;
    return { next: returnToCampUseOneHour };
}

export function morningShouldraAlert(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    ...
    ShouldraFollowerFlags.SHOULDRA_FOLLOWER_STATE = 1;
    ShouldraFollowerFlags.SHOULDRA_PLOT_COUNTDOWN = 7;
...
// Non-Corrupt Vala:
function shouldraAndValaGetBigger(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    if (ShouldraFollowerFlags.SHOULDRA_MET_VALA < 1) ShouldraFollowerFlags.SHOULDRA_MET_VALA = 1;
    CView.text("You decide to make your way down to the Wet Bitch to introduce Vala to her.  A few minutes into your stay, you see the fairy serving a few tables while holding a tray filled with drinks and empty glasses.  Pointing her out, Shouldra gives a delighted squeal.");
...
function pureMacroValaCont(player: Character): NextScreenChoices {
    ...
    // Female:
        ...
        CView.text("Her finger starts pressing firmly against your chest.  You try to push it away, but the size difference makes the effort futile.  The ghost girl starts grinding against your chest with her fingertip, the friction against your breasts causing a small gasp to escape your lips, and your nips to become slightly firmer.  She runs her pinky up your leg, stopping at your vagina, and you reflexively clench your legs together. Unfazed, Shouldra forces them back open with her fingers, exposing you to the open air.  A small breeze blows through, tickling your cunt and causing you to bite your lip.");
        ...
        CView.text("\n\nYou wake up back at your ..");
    }
    shouldraSleeping(10 + randInt(15), true);
    player.orgasm();
    return { next: returnToCampUseOneHour };
}
...
function pureValaAndShouldraMacroMax(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.clear();
    player.slimeFeed();
    CView.text("Before she can do anything, you shout out a rather unusual suggestion, though to you it sounds quite exciting.  Vala or Shouldra, whoever she is, smiles wickedly and nods, lifting you up towards the glittering gash between her legs.");
    ...
    if (player.gender > 0) {
        CView.text("  Inspired by this, your own body has long since responded");
        if (player.body.cocks.length > 0) CView.text(", becoming firmly erect.  You happily piston your hips to grind your " + describeCocksLight(player) + " along the pink-glossed walls, oozing your fluids in delight.");
        else CView.text(", growing wet, though in this slick fluid, your contribution is a mere drop in the ocean.");
    }
    CView.text("  The warm fleshy dream is abruptly ended as Shouldra yanks you out by your [feet].  \"<i>I'm gonna put you the whole way in.  Do me a solid and get me off a few times, will ya?</i>\"");
    ...
    CView.text(" against the slick walls.  You push your way towards the pinkish slit in the distance begin to slide your [legs] over the inner edges of her lips, pressing your arms up above you for stability.");
    ...
    if (player.body.cocks.length > 0) {
        CView.text("  [EachCock] is quickly brought to orgasm by the exotic situation, and you fire off your own load into the deluge of sex-juice.");
        if (player.cumQ() >= 1000) CView.text("  Somehow, you pour out almost as much spunk as Vala's orgasm, turning the inside of her honeypot into an off-white mess.");
    }
    else if (player.body.vaginas.length > 0) {
        CView.text("  Your [vagina] can scarcely resist the allure of the sweet, female flesh pressing against it, and with an explosion of hot bliss, you climax");
        if (player.body.vaginas.get(0).wetness >= 4) CView.text(", squirting with joy");
        CView.text(".");
    }
    ...
    if (player.gender === 0) CView.text(" somehow");
    CView.text(", but she vanishes with a high-pitched giggle.");
...
export function shouldraMeetsCorruptVala(player: Character): NextScreenChoices {
    ...
    if (player.body.cocks.length > 0) CView.text("  Your cock slips past the folds, pressed up against your stomach as the underside is massaged with the gentle tightness within.");
    CView.text("  Once you're up to your chest, Vala stops, instead grasping your shoulders between her thumb and forefingers as she begins to gently slide you in and out of her.");
...
export function sandWitchGetsGhostly(player: Character): NextScreenChoices {
    ...
    if (player.body.cocks.length > 0 && (player.body.vaginas.length <= 0 || randInt(2) === 0)) {
        ...
        CView.text("Shouldra pokes her head out from your torso, rubbing her hands together menacingly.  \"<i>So, we're going to have some fun with this little gal, aren't we, Champ?</i>\"  The sand witch is paralyzed with fright, able only to gaze at the bizarre spectacle before her.  Looking down at the shade sticking out of you, you see her hands now poised against your chest.  The ghost girl is ready to fly out at a moment's notice.  You decide not to keep her waiting and give her the go ahead; the force of her excited exit about throws you to the sandy ground.  The eager spirit quickly hammers into the witch, digging her sitting body into the sand.  There's a little struggling, but it isn't long until the ghost subdues her prey.");
        ...
        if (!player.body.chest.find(BreastRow.FuckableNipples)) CView.text("\"<i>You know, maybe I should give you a couple of those chest-mounted treats.  They're pretty fun,</i>\" Shouldra teases, tweaking your [nipples].  ");
        CView.text("With a very telling exasperated sigh, you snatch up your [armor] and march back to ");
    }
...
function shouldraFappinTimes(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    if (player.body.cocks.length > 0 && player.effects.has(StatusEffectType.Infested)) {
        return shouldraAndWormsYoureGonnaHaveABadTime(player);
    }
    else if (player.gender === 0) return genderlessShouldraMasturbation(player);
    else if (player.body.cocks.length > 0 && (player.body.vaginas.length <= 0 || randInt(2) === 0)) return maleMasturbationProper(player);
    else {
        if (randInt(2) === 0) return girlyMasturbationWithGhosts(player);
        else return shouldraTongueLicksPCs(player);
    }
}
...
function genderlessShouldraMasturbation(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("Shouldra runs your fingers along your ");
    // [if (femininity < 50)
    if (player.body.femininity < 50) CView.text("masculine ");
    else if (player.body.femininity > 50) CView.text("feminine ");
    CView.text("form while processing your request, your eyes glowing yellow as she mulls over her choices.");
    // (same paragraph follows)
    // {if SHOULDRA_GENDERLESS_FUCK_COUNT = 0 & event occurrence = 0}
    if (ShouldraFollowerFlags.SHOULDRA_GENDERLESS_FUCK_COUNT === 0 && ShouldraFollowerFlags.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT === 0) {
    ...
    // if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 & event occurrence = 0
    else if (ShouldraFollowerFlags.SHOULDRA_GENDERLESS_FUCK_COUNT > 0 && ShouldraFollowerFlags.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT === 0) {
    ...
    // if event occurrence > 0
        ...
        // [if (corruption < 50)
        if (player.stats.cor < 50) CView.text("There isn't an answer or outcome to that question that you're in favor of, honestly.");
        else CView.text("You sure as hell wouldn't have bothered asking if you weren't.");
    }
    CView.text("  The ghost girl promptly discards your [armor] before hurrying your naked body along into the wilderness away from ");
    ...
    if (ShouldraFollowerFlags.SHOULDRA_GENDERLESS_FUCK_COUNT === 0 && ShouldraFollowerFlags.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT === 0) CView.text("Within lies a rather rustic yet well-built chest.");
    else if (ShouldraFollowerFlags.SHOULDRA_GENDERLESS_FUCK_COUNT > 0 && ShouldraFollowerFlags.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT === 0) CView.text("The moment the light reveals what lies within, your [butthole] instinctively clenches shut.  \"<i>You didn't think I'd leave my most treasured possessions behind for some goblin to run into, did you?</i>\" It's the ghost girl's chest of dildos, as well-kept as the last time you laid worried eyes on it.");
    else CView.text("Inside rests the ghost's familiar treasure trove of dildos and other toys you have yet to discover.");
    CView.text("  She has you pull the container out from its hiding place, removing the clear tarp she used to protect it from the elements.  Despite the fact that you have yet to control your body once through this entire ordeal, an intrinsic urge to open the container has you bending over to pry into the contents within.");
    ...
    if (ShouldraFollowerFlags.SHOULDRA_GENDERLESS_FUCK_COUNT > 0) CView.text("  Just what else did she pack in there since last you peered at its contents back at the ruins?");
    ...
    if (player.body.butt.looseness < 3) {
        CView.text("Your nerves screech out in pain as your rear revenant stretches out her puckered little \"jaw\" to something more fitting for what she has in store.  A few seconds later and the sensations begin to only register as pleasure.");
        // [if (corruption < 50)
        if (player.stats.cor < 50) CView.text("..  This only worries you further.");
        CView.text("  It doesn't take her long to <b>prepare your anus for any insidious ploy she's thought up.</b>");
    }
    // if analLooseness >= 3
    else {
        CView.text("\"<i>Looks as though you already get plenty of action back here, chief,</i>\" she jests.  Nevertheless, the rear revenant massages her \"jaw\" even more lax than you already are.  Even stranger, each gyration provides more and more pleasurable feedback");
        if (player.stats.cor < 50) CView.text(", which only serves to confuse you");
        CView.text(".");
    }
    ...
    if (ShouldraFollowerFlags.SHOULDRA_GENDERLESS_FUCK_COUNT > 0 || ShouldraFollowerFlags.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT > 0) CView.text("You recognize it instantly as she pulls it free from her chest: the nearly foot-long floppy horse cock she impaled you on before.");
    // {if SHOULDRA_GENDERLESS_FUCK_COUNT = 0}
    else if (ShouldraFollowerFlags.SHOULDRA_GENDERLESS_FUCK_COUNT === 0) CView.text("She holds her catch up clear for you to see; to your horror, you have clutched onto an enormous horse cock replica.  All 13 inches flop restlessly in your grip, seemingly taunting you with the foreboding future that lays before you.");
    CView.text("  Shouldra smacks the dildo of doom onto a nearby boulder as she wearily rises you to your [feet].  The inferior, forgotten playthings slide out uselessly from the ghoul's gasping craw as she slowly clambers over top the wiggling horsey wang.");
...
function nongenderlessAnalShouldraMasturbation(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("Shouldra runs your fingers along your ");
    // [if (femininity < 50)
    ...
    CView.text("form while processing your request, your eyes glowing yellow as she mulls over your request.  \"<i>So, let me get this straight.  Despite[if (hasCock = true)  this dandy dick] [if (isHerm = true)  and][if (hasVagina = true)  this serviceable snatch] you want me to work with this?</i>\" the spirit asks, smacking your hand against your [butt].  A smug nod reaffirms your desire.  ");
    // (same paragraph follows)
    // if event occurrence = 0
    if (ShouldraFollowerFlags.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT === 0) {
        CView.text("Your spectral visitor cracks your knuckles in a blustering show of confidence.  \"<i>You think you've thrown down the gauntlet with your challenge, eh, Champ?  You think rules, regulations, sanctions, trade embargos... these things can keep me down?</i>\"  What is she blathering on about?  You suspect she's just stalling; call it your own sixth sense, but as she waves your hands around as she boasts, it feels like it's taking some willpower to not just listlessly stroke your goods.  At least, it seems that way until some errant thought interrupts her tirade, a knowing grin blossoming along your mug.  \"<i>No blockade can stop me.</i>\"");
    }
    else {
        CView.text("\"<i>Up for another round then, Champ?</i>\" she asks, your body striking a familiar boxing stance from bouts long since fought.  \"<i>If you think I'll be pulling any punches then you don't have a ghost of a chance.</i>\"  Only if you had control of your body could you respond with even half the appropriate groan.");
    }
    CView.text("  The ghost girl promptly discards your [armor] before hurrying your naked body along into the wilderness away from ");
    ...
    // if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 OR event occurrence > 0
    if (ShouldraFollowerFlags.SHOULDRA_GENDERLESS_FUCK_COUNT > 0 || ShouldraFollowerFlags.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT > 0) CView.text("You recognize it instantly as she pulls it free from her chest: the nearly 2-foot floppy horse cock she impaled you on before.");
    // if SHOULDRA_GENDERLESS_FUCK_COUNT = 0
    else CView.text("She holds her catch up clear for you to see; to your horror, you have clutched onto an enormous horse cock replica.  All 13 inches flop restlessly in your grip, seemingly taunting you with the foreboding future that lays before you.");
    CView.text("  Shouldra smacks the dildo of doom onto a nearby boulder as she wearily rises you to your [feet].  The inferior, forgotten playthings slide out uselessly from the ghoul's gasping craw as she slowly clambers over top the wiggling horsey wang.");
    CView.text("\n\nToo tired to object or plead, you meekly brace for impact as the wraith lines up its target.  Your immense cheeks part way.  Your rectum gapes wide.  Your anal tongue presses firmly against your taint.  \"<i>Down the hatch!</i>\"");
    CView.text("\n\nThe forest shudders with your scream.  The phantom easily downs six inches on the first go.  She wraps her tongue around the shaft, forcing more and more into you.  You swear your pelvis is bulging.  Muffled moans blended with laughter emit from your belabored behind.  You dare not guess how deep Shoudra drills the cock into you before she climaxes - hard.  Your voluptuous ass quavers with ferocity alongside her gagged yelping, throwing you from your perch atop the rock back down to the earth.  Her fun over, you begin to feel the magic subsiding.  Having no desire to learn of the consequences of leaving a massive wang lodged in your shrinking anus, you muster what little strength you can to plumb it free.  With the experience settling down, you can't help but laugh - closer to a timid chuckle, as its the best your exhausted body can offer.  You think you hear Shouldra saying something to you, but you're too worn out to listen to her boasting.");
...
function maleMasturbationProper(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("You are nearly overwhelmed by Shouldra's delight when you allow her to focus on your [cock].  She instantly takes the lead, steering you behind a good-sized boulder and flinging your [armor] piece-by-piece as you go.");
    ...
    // if cocksize <200:
    if (player.body.cocks.get(0).area < 200) CView.text("  You stagger a bit at the unaccustomed heft of your junk altering your center of gravity, but you know that will wear off soon enough.");
    CView.text("  She reaches into her tunic and tosses you a full bottle of ectoplasm, giving you a wink before incorporealizing once more and floating into you.  All's well that ends well.");
    player.orgasm();
    player.stats.sens += -2;
...
function girlyMasturbationWithGhosts(player: Character): NextScreenChoices {
    ...
    CView.text("\"<i>Tsk!  Saying those kinds of things aloud!  Still... if you want to let it out so badly, I might let you if you beg a little more...</i>\"");
...
function shouldraTongueLicksPCs(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("The ghost girl promptly discards your [armor] before hurrying your naked body along into the wilderness away from ");
    ...
    // if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 || event occurrence > 0
    if (ShouldraFollowerFlags.SHOULDRA_GENDERLESS_FUCK_COUNT > 0 || ShouldraFollowerFlags.SHOULDRA_TONGUE_LICKS_TIMES === 0) CView.text("  The consistency of your forgetfulness while traveling this path leads you to believe that your ethereal inhabitant would rather you not come back here on your own.");
    ...
    if (ShouldraFollowerFlags.SHOULDRA_GENDERLESS_FUCK_COUNT === 0 && ShouldraFollowerFlags.SHOULDRA_TONGUE_LICKS_TIMES === 0) CView.text("  Within lies a rather rustic yet well-built chest.");
    else if (ShouldraFollowerFlags.SHOULDRA_GENDERLESS_FUCK_COUNT > 0 && ShouldraFollowerFlags.SHOULDRA_TONGUE_LICKS_TIMES === 0) CView.text("  \"<i>You didn't think I'd leave my most treasured possessions behind for some goblin to run into, did you?</i>\"  It's the ghost girl's chest of dildos, as well-kept as the last time you laid worried eyes on it.");
    else CView.text("  Inside rests the ghost's familiar treasure trove of dildos and other toys you have yet to discover.");
    CView.text("  She has you pull the container out from its hiding place, removing the clear tarp she used to protect it from the elements.  Despite the fact that you have yet to control your body once through this entire ordeal, an intrinsic urge to open the container has you bending over to pry into the contents within.");
    ...
    // if event occurrence == 0
    if (ShouldraFollowerFlags.SHOULDRA_TONGUE_LICKS_TIMES === 0) {
        ...
        if (player.body.tongue.type > TongueType.HUMAN) CView.text("some of ");
        CView.text("your tongue out, examining it.  Already, you think you've got a pretty good idea of where she's going to take this.");
    ...
    if (player.perks.has(PerkType.Flexibility)) {
        ...
        if (ShouldraFollowerFlags.SHOULDRA_TONGUE_LICKS_TIMES === 0) {
            ...
            CView.text("\n\nShe sits you down and lets her dildos fall to the ground, forgotten for the moment.  \"<i>How's this one go?</i>\" she asks, leaning down in an awkward attempt to get your face in your " + describeVagina(player, player.body.vaginas.get(0)) + ".  You start to explain the proper 'technique' to what she's trying to do, but before you can even finish your thought she's stuck a leg up in the air and has your mouth to your own snatch.  You doubt you'll ever get used to having her inside your thoughts.");
        }
        ...
        if (ShouldraFollowerFlags.SHOULDRA_TONGUE_LICKS_TIMES === 0) {
            CView.text("  Almost shocked enough to wrest control away from your companion, you begin to babble dissent and try to reel your lengthening tongue back in to no avail.  Shouldra stops for a moment, quite frustrated by the interruption.  Sitting straight up, she consoles you, \"<i>Relax, Champ, it'll stop feeling weird in a minute and you know it's all temporary.</i>\"");
        ...
        CView.text(" sliding fingers along the wet appendage, humming a little tune over your tongue until more than a foot of it droops from your mouth.  \"<i>Now we're getting somewhere,</i>\" you hear her thinking as she works her magic.  There doesn't seem to be a limit to how much she can pull out of your mouth, and soon she has more than three feet of pure tongue coiled in your lap.  Furrowing your brow in concentration, she tries to move your tongue about, but can't manage to wiggle more than the tip.  \"<i>Too thin,</i>\" she mutters, \"<i>much too weak. Doesn't feel right...</i>\"");
    ...
    // if demon tongue
    else if (player.body.tongue.type === TongueType.DEMONIC) {
        ...
        if (ShouldraFollowerFlags.SHOULDRA_TONGUE_LICKS_TIMES === 0) CView.text("  Almost shocked enough to wrest control away from your companion, you begin to babble dissent and try to reel your lengthening tongue back in to no avail.  Shouldra stops for a moment, quite frustrated by the interruption.  Sitting straight up, she consoles you, \"<i>Relax, Champ, it'll stop feeling weird in a minute and you know it's all temporary.</i>\"\n\nGranted a moment to adjust, you let yourself settle down and hand her the reins once more.  She happily goes back to ");
        else CView.text("\n\n She happily continues ");
        CView.text("sliding fingers along the wet appendage, humming a little tune over your tongue until more than an additional foot of it droops from your mouth.  \"<i>Now we're getting somewhere,</i>\" you hear her thinking as she works her magic.  There doesn't seem to be a limit to how much she can pull out of your mouth, and soon she has more than three feet of pure tongue coiled in your lap.  Furrowing your brow in concentration, she tries to move your tongue about, but can't manage to wiggle more than the tip.  \"<i>Too thin,</i>\" she mutters, \"<i>much too weak. Doesn't feel right...</i>\"");
        CView.text("\n\nTaking the midsection of your tongue in your hand, she slowly starts to rub her hand along some of its length.  It feels... odd, somehow, but not entirely unpleasant.  Though you shudder at the first rub, they start to feel more natural as she pushes you on.  Actually, it feels sort of good.  Her strokes remain constant, though some part of you wishes she'd go faster.  It doesn't even register as odd in your addled mind that your tongue is swelling in your hand with every stroke.  Shouldra starts to make wider movements along your smooth stretch of muscle.  Its girth plumps up to fill your palm nicely wherever she brings your hand, and the strange sensations of your flesh on its nerves wrack your body with shudders of pleasure even through Shouldra's grip.");
    ...
    CView.text("\n\nYou wake some time later, lying naked beside a pile of your [armor].  You're right back at   The ghost girl managed to drag you back somehow - no doubt interested in keeping the location of her secret goodies safe.");
    shouldraSleeping((10 + randInt(26)), true);
    ShouldraFollowerFlags.SHOULDRA_TONGUE_LICKS_TIMES++;
    return { next: returnToCampUseOneHour };
}
...
export function shouldraDream1(player: Character): NextScreenChoices {
    CView.text("\n");
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    ...
    else if (ShouldraFollowerFlags.SHOULDRA_PLOT_LEVEL === 1) {
    ...
    else if (ShouldraFollowerFlags.SHOULDRA_PLOT_LEVEL === 2) {
        // Dream 3 - "Recent" Goblin Possession
        // triggers one week after Dream/Talk 2
        CView.text("Few things more relaxing than heading home after a day of work.  But as you approach the dilapidated hut, some strange noises from within catch you off guard.  If it's that fucking mouse again...");
    ...
    if (ShouldraFollowerFlags.SHOULDRA_PLOT_LEVEL === 3) {
        // Dream 4 - Backstory
    ...
    ShouldraFollowerFlags.SHOULDRA_PLOT_LEVEL++;
    ShouldraFollowerFlags.SHOULDRA_TALK_NEEDED = 1;
    return { next: campMenu };
}

// Talk 1 - First Possession Discussion
function shouldraYappin(player: Character): NextScreenChoices {
    CView.clear();
    // Plot dreams!
    if (ShouldraFollowerFlags.SHOULDRA_TALK_NEEDED === 1) {
        // unlocked with Dream 1
        if (ShouldraFollowerFlags.SHOULDRA_PLOT_LEVEL === 1) {
            // Clear out that talk happened and start a countdown till next dream sequence.
            ShouldraFollowerFlags.SHOULDRA_TALK_NEEDED = 0;
            CView.sprite(SpriteName.ShouldraFollower); // 67;
            if (ShouldraFollowerFlags.SHOULDRA_PLOT_LEVEL < 4) ShouldraFollowerFlags.SHOULDRA_PLOT_COUNTDOWN = 7;
            ...
            CView.text("You decide not to beat around the bush and ask about the strange dream you had the other night.  The spirit puts on a surprised reaction, your shoulder returning to normal as she casually walks outside of your body.");
            ...
            if (player.perks.has(PerkType.Incorporeality)) CView.text("  Shouldra bumps you to the side a little as she makes her next point, \"<i>Even with your hoaky quasi-possession power, you don't really know what it's like to actually possess someone.</i>\" You look at her funny, unaware that you weren't really getting the full effect.  Not that you care!");
            ...
            CView.text(".  \"<i>No matter what treasure trove of knowledge I dig up from you, there's always something I'm more interested in.</i>\"  Your hand lifts back up, and you watch as Shouldra's hand splits off from your own, meeting below the wrist.  She holds your hand with her own ghostly appendage, resting her head on your shoulder.  The ghost girl sits there for a moment, silent, leaving you to wonder if maybe, just maybe, you're starting to mean a little more to her than just a sex toy.");
        ...
        // Talk 2 - First Transformation Discussion
            ...
            if (player.gender > 0) {
                CView.text("  Your [if (hasCock = true) [cock]][if (isHerm = true)  and ][if (hasVagina = true) [vagina]] start to swell up; looking up at the ghost, you realize that she hasn't uttered a single word!");
            }
            else CView.text("  A fleshy, penis-shaped bulge begins to swell out of your loins; looking up at the ghost, you realized she hasn't uttered a single word!");
            ...
            if (player.gender === 0) CView.text("dick");
            CView.text(" [if (isHerm = false) continues its][if (isHerm = true) continue their] march of magnification while your fingers vigorously explore their new frontiers.");
            ...
            if (player.gender === 0) CView.text("burgeoning dickflesh");
            CView.text(".  It takes some gumption for you to work through the arousal and contribute to the conversation, but you manage to point out how you're pretty sure there are other people in Mareth with the same sort of power, to some degree at least.");
        ...
        // Talk 3 - "Recent" Goblin Possession Discussion
        else if (ShouldraFollowerFlags.SHOULDRA_PLOT_LEVEL === 3) {
            // Clear out that talk happened and start a countdown till next dream sequence.
            ShouldraFollowerFlags.SHOULDRA_TALK_NEEDED = 0;
            CView.sprite(SpriteName.ShouldraFollower); // 67;
            if (ShouldraFollowerFlags.SHOULDRA_PLOT_LEVEL < 4) ShouldraFollowerFlags.SHOULDRA_PLOT_COUNTDOWN = 7;
            // unlocked with Dream 3
            CView.text("You stare at Shouldra.  \"<i>Oh, <b>what</b>?</i>\"  You stare at Shouldra.  \"<i>She fucking pissed on my floor!</i>\"  You stare at Shouldra.  The ghost shakes her head, sinking it back inside your skin. The ghost girl quickly flies out of your chest.  \"<i>You don't fuck with my stuff!</i>\"");
            ...
            if (player.body.cocks.length > 0) CView.text("</i>\"  Shouldra eyes your [cock] gingerly, acting as if she can stare right through your outfit at it.  \"<i>");
            else CView.text("  ");
            CView.text("The half-millenium or so of scarcity has really shaped my interests.  I'm at the point now where I just lose it whenever I get these ghostly mitts on a slab of meat.  And the big ones?  ");
            if (player.body.cocks.sort(Cock.Largest)[0].area >= 200) CView.text("Ones like yours?  ");
            CView.text("Then I'm just in heaven.</i>\"  Hold on.  She can turn any dick into a towering monument.  What's so special about ones that come pre-packaged, you ask?  \"<i>I kinda break the one rule I had at that point, but when I find someone with a big boner, it just... feels so good to me.  There's just something about an all-natural magnum opus that just lights my fire.</i>\"");
            ...
            CView.text("\"<i>Oh,</i>\" she says, stepping out of you and sitting herself across from you.  \"<i>You saw that.</i>\"");
    ...
    // Generic Oni Discussion!
        ...
        CView.text("On a whim, you decide to have a chat with your spectral companion, who obligingly pops free of your torso to spare you the awkward sensation of conversing with one of your own body parts.");
...
export function shouldraFollowerScreen(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.clear();
    if (!shouldraSleeping()) CView.text("You pulse a desire for interaction with Shouldra, summoning her head to form from your shoulder. \"<i>What's on your mind, Champ? I could've found out on my own, but I was... preoccupied.</i>\"");
    // Follower Screen Quick Text - Shouldra asleep
    else {
        CView.text("You try and contact Shouldra, but the ghost is still sound asleep within you.  Or whatever ghosts consider sleep after an arousing round of debauchery. Either way, you garner no response.  Best to try again later.");
        return { next: campMenu };
    }
    let sex: ClickFunction;
    if (player.stats.lust >= 33) sex = shouldraSexMenu;
    return {
        choices: [
            ["Talk", shouldraTalkMenu],
            ["Sex", sex],
            ["", undefined],
            ["Go Away", kickFollowerShouldraOut],
            ["Back", campFollowers]
        ]
    };
}

function shouldraSexMenu(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("Shouldra stirs from somewhere deep inside you, excitement evident in the swiftness of her reaction.  \"<i>Is it time for some fun, Champ?</i>\" she asks happily, her increased lust affecting you just a bit.  Do you oblige her?");
    let vala: ClickFunction;
    if (ShouldraFollowerFlags.FREED_VALA !== 0 && (Time.hour >= 12 && Time.hour <= 21)) {
    ...
    return {
        choices: [
            ["GhostMasturbate", shouldraFappinTimes],
            ["Anal Masturb", anal],
            ["Visit Vala", vala],
            ["", undefined],
            ["Back", shouldraFollowerScreen]
        ]
    };
}

// Talk Options
function shouldraTalkMenu(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    // Go directly to appropriate talk if necessary
    if (ShouldraFollowerFlags.SHOULDRA_TALK_NEEDED === 1) {
        return shouldraYappin(player);
    }
    // Allow for the expansion of bodyparts via spooky magic
    let blowShitUp: ClickFunction;
    let shrinkShit: ClickFunction;
    if (ShouldraFollowerFlags.SHOULDRA_PLOT_LEVEL >= 2 && ShouldraFollowerFlags.SHOULDRA_MAGIC_COOLDOWN <= 0) {
        blowShitUp = shouldraGroPlus;
        shrinkShit = shouldraReductoMenu;
        CView.text("\n\nShouldra could permanently expand or reduce part of your body.");
    }
    else if (ShouldraFollowerFlags.SHOULDRA_PLOT_LEVEL >= 2) CView.text("\n\nShouldra needs some time before she can grow or shrink one of your body parts permanently.");
    return {
        choices: [
            ["Normal Talk", shouldraYappin],
...
function shouldraGroPlus(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("You decide to take up Shouldra's offer on natural body enhancement.  Before you tell her as much, the eager spectre quickly assumes control of your body.  \"<i>Alright, Champ, now we're in business.  So, what did you have in mind?</i>\"");
    // Balls     Breast     Clit     Cock     Nipples     Butt     Nevermind
    let balls: ClickFunction;
    const breast: ClickFunction = shouldraGrowsYoTits;
    let clit: ClickFunction;
    let cock: ClickFunction;
    const butt: ClickFunction = shouldrasButtBigginator;
    const nipples: ClickFunction = shouldraGivesYaSomeFukkinTeats;
    if (player.body.balls.count > 0) balls = groBallsBiggaGHOSTYSTYLE;
    if (player.body.cocks.length > 0) cock = shouldraCockBloating101;
    if (player.body.vaginas.length > 0) clit = shouldraGrowsYoClit;
    return {
        choices: [
            ["Balls", balls],
            ["Breasts", breast],
            ...
            ["", undefined],
            ["", undefined],
            ["Back", shouldraTalkMenu]
        ]
    };
}

// Balls
function groBallsBiggaGHOSTYSTYLE(player: Character): NextScreenChoices {
    ShouldraFollowerFlags.SHOULDRA_MAGIC_COOLDOWN = 72;
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("\"<i>Interested in a bigger ballsack, huh?  Not only will it help pronounce your package, but a big ol' swinging sack of nuts feels great when you're really pounding.</i>\" Shouldra grasps your [balls] with both your hands and begins reciting the appropriate spell.");
    // ballsize + 2 + randInt(4) 20% chance
    if (randInt(5) === 0) {
        player.body.balls.size += 2 + randInt(4);
        CView.text("\n\nYour testicles rumble greatly, inch after inch of " + player.body.skin.tone + " flesh enveloping their impressive new bulk.  Shouldra gives your enlarged [balls] a little squeeze and leaves you to reacquaint yourself with a new center of balance.");
    }
    // ballsize + 1 + randInt(2) Else
    else {
        player.body.balls.size += 1 + randInt(2);
        CView.text("\n\nA tingling sensation courses through your scrotum, your " + player.body.skin.tone + " going taut as it gains additional mass.  Shouldra rewards your new [balls] with a light massage.");
    }
    // if ballsize > 10
    if (player.body.balls.size > 10) CView.text("  You second guess your decision when your swinging cum barrels seem to impede walking.  Shouldra reassures you that the enhanced libido and baby batter supply is worth the wider gait.");
...
// Breast
function shouldraGrowsYoTits(player: Character): NextScreenChoices {
    ShouldraFollowerFlags.SHOULDRA_MAGIC_COOLDOWN = 72;
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("\"<i>I'll give you a rack that could hypnotize anyone into a faithful tit slave,</i>\" Shouldra boasts, stretching your shoulders in preparation.  Your hands grope your [fullChest] as the spirit slips into the arcane language you've come to grow so familiar with.\n\n");
    // same growtits() as Gro+
    if (player.body.chest.length === 1) growSmallestBreastRow(player, (1 + randInt(5)), 1, true);
    else growSmallestBreastRow(player, 1 + randInt(2), player.body.chest.length, true);
    player.stats.lust += 10;

    return { next: returnToCampUseOneHour };
}
// Clit
function shouldraGrowsYoClit(player: Character): NextScreenChoices {
    ShouldraFollowerFlags.SHOULDRA_MAGIC_COOLDOWN = 72;
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("\"<i>Looking to give your joy buzzer a little jumpstart?</i>\" Shouldra teases, your hands sliding down to your inviting [cunt].  Your fingers tease your [clit] while the ghost girl spouts her arcane tongue from your mouth.  ");
    // clitLength + 1
    player.body.clit.length += 1;
    CView.text("She welcomes the new inch with a quick flick.  The sensitive [clit] twitches happily in response, confirming the success of the spell.");
    player.stats.sens += 5;
...
// Nipples
function shouldraGivesYaSomeFukkinTeats(player: Character): NextScreenChoices {
    ShouldraFollowerFlags.SHOULDRA_MAGIC_COOLDOWN = 72;
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("Shouldra rubs your hands together in anticipation, \"<i>A beautiful blossoming pair of nipples is what you need to really compliment a nice set of tits.</i>\"  Your hands gleefully tweak your [nipples] as the spirit's incantation gets underway.");
    ...
    for (const breastRow of player.body.chest)
        breastRow.nipples.length += (randInt(2) + 3) / 10;
    player.stats.lust += 15;

    let changeCount: number = 0;
    // NIPPLECUNTZZZ
    // Set nipplecunts on every row.
    if (!player.body.chest.find(BreastRow.FuckableNipples) && randInt(4) === 0) {
        for (const breastRow of player.body.chest)
        ...
        if (changeCount > 0) CView.text("  While doing so, she manages to slip a few of your fingers within the engorged teats, ecstatcy flowing through you as your digits explore their inner workings.  \"<i>I may not be a doctor, Champ, but it looks to me like <b>you've managed to gain yourself a pair of fuckable nips.</b></i>\"  The ghost girl spends a few minutes running your fingers around your [nipples], inside and out, just to confirm her diagnosis.");
    }
    return { next: returnToCampUseOneHour };
}

// Butt
function shouldrasButtBigginator(player: Character): NextScreenChoices {
    ShouldraFollowerFlags.SHOULDRA_MAGIC_COOLDOWN = 72;
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("Shouldra's haunting laugh works its way out from your mouth.  \"<i>Looking for a little more jiggle in your step, Champ?  I can get BEHIND that.</i>\"  This damnable ghost won't let you roll your eyes!  Is there no end to her torment?  After a quick crack of your knuckles, Shouldra directs your hands back onto your derriere as she transitions into spellcasting.");
    // this.player.body.butt.rating = this.player.body.butt.rating + 1 + randInt(3)); or so
    player.body.butt.rating += 1 + randInt(3);
...
function shouldraReductoMenu(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("Shouldra's offer to shrink aspects of your body sounds right up your alley, and you pulse as much to your ghostly compatriot.  There's no response.  You remind her that it was HER suggestion in the first place.  \"<i>Come ooooon, Champ.  It was just a slip of the ol' spiritual tongue.</i>\"  You cross your arms and look up and off to the side, the best way you can figure to glare at something that isn't there. \"<i>Ugh, fine.  Let's just get this over with,</i>\" Shouldra concedes, removing your [armor].");
    // Balls     Breast     Clit     Cock     Nipples     Butt     Nevermind
    let balls: ClickFunction;
    let breasts: ClickFunction;
    let clit: ClickFunction;
    let cock: ClickFunction;
    let butt: ClickFunction;
    let nipples: ClickFunction;
    if (player.body.chest.get(0).nipples.length > .25) nipples = shrinkDemNipplzForYoGhost;
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 1) breasts = shouldraReductosYourTits;
    if (player.body.butt.rating >= 2) butt = shrinkDatBootyForYoGhost;
    if (player.body.balls.count > 0 && player.body.balls.size > 1) balls = shouldraReductosYourBallsUpInsideYa;
    if (player.body.cocks.length > 0 && player.body.cocks.sort(Cock.Longest)[0].length > 4) cock = shouldraMakesCocksDisappear;
    if (player.body.vaginas.length > 0 && player.body.clit.length > .25) clit = clittyVanishingActShouldra;
    return {
        choices: [
            ["Balls", balls],
            ["Breasts", breasts],
...
// Balls
function shouldraReductosYourBallsUpInsideYa(player: Character): NextScreenChoices {
    ShouldraFollowerFlags.SHOULDRA_MAGIC_COOLDOWN = 72;
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("\"<i>Why would you want to shrivel these delicious cumquats?</i>\"  Shouldra's arsenal of puns doesn't affect your request in the slightest.  The ghost girl moves into a quick spell, savoring your testicles before finishing.");
    // this.player.body.balls.size = this.player.body.balls.size - (2 + this.randInt(4));
    player.body.balls.size -= (2 + randInt(4));
    if (player.body.balls.size < 1) player.body.balls.size = 1;
...
// Breast
function shouldraReductosYourTits(player: Character): NextScreenChoices {
    ShouldraFollowerFlags.SHOULDRA_MAGIC_COOLDOWN = 72;
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("Your arms clasp themselves around your [fullChest].  \"<i>Not the gals!  What monster would want to deflate their knockers?</i>\"  This monster, evidently.  Shouldra sighs, holding your hands in a weak casting stance as she gets the heinous act over with.");
    shrinkTits(player);
...
// Clit
function clittyVanishingActShouldra(player: Character): NextScreenChoices {
    ShouldraFollowerFlags.SHOULDRA_MAGIC_COOLDOWN = 72;
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("\"<i>Aw, really, Champ?  The best part of your snatch?</i>\"  A nod of your head reasserts your desire.  The depressed phantom hastily works through the spell, the notion of shrinking your [clit] not sitting well with her.");
    player.body.clit.length /= 1.7;
    // Set clitlength down to 2 digits in length
    player.body.clit.length = Math.floor(player.body.clit.length * 100) / 100;
...
function shouldraMakesCocksDisappear(player: Character): NextScreenChoices {
    ...
    if (player.body.cocks.length === 1) {
        ...
        const choice: number = randInt(10);
        if (choice === 0) CView.text("murderer");
        else if (choice === 1) CView.text("bastard");
        else if (choice === 2) CView.text("jerk");
        else if (choice === 3) CView.text("monster");
        else if (choice === 4) CView.text("cretin");
        else if (choice === 5) CView.text("fiend");
        else if (choice === 6) CView.text("asshole");
        else if (choice === 7) CView.text("villain");
        else CView.text("dick");
        CView.text("!</i>\" it shouts.  The tiny voice fades away as your [cock] goes still, resting at two thirds of its original size.  A single tear rolls down your cheek; you suspect the melodramatic spirit is at fault here.");
        player.body.cocks.get(0).length *= 2 / 3;
        player.body.cocks.get(0).thickness *= 2 / 3;
    }
    // MULTI
    else {
...
// Nipples
function shrinkDemNipplzForYoGhost(player: Character): NextScreenChoices {
    ShouldraFollowerFlags.SHOULDRA_MAGIC_COOLDOWN = 72;
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("\"<i>Are you kidding me, Champ?  Nipples are the most important-</i>\"  You cut the ghost girl's argument there, not wanting to leave your decision up for debate.  She instead leaves you with a deep sigh of disapproval before quickly casting the appropriate spell.");
    // Shrink
    if (player.body.chest.get(0).nipples.length / 2 < .25) {
        ...
        player.body.chest.get(0).nipples.length = .25;
    }
    else {
        ...
        player.body.chest.get(0).nipples.length /= 2;
    }
    player.stats.sens += -3;
    player.stats.lust += -5;
...
// Butt
function shrinkDatBootyForYoGhost(player: Character): NextScreenChoices {
    ShouldraFollowerFlags.SHOULDRA_MAGIC_COOLDOWN = 72;
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("Your hands quickly grasp at your bare [butt].  \"<i>You... you're desecrating a national treasure here!</i>\"  The rather odd declaration gets a chuckle from you, but you remain firm in your request to dull your derriere.  The ghost girl furthers her attempts to change your mind, but without literally changing your mind, you remain steadfast.  Defeated, your hands take one last trip around your posterior before the poltergeist recites her incantation.");
    if (player.body.butt.rating >= 15) {
        player.body.butt.rating -= 3 + Math.floor(player.body.butt.rating / 3);
    ...
    else if (player.body.butt.rating >= 10) {
        player.body.butt.rating -= 3;
        ...
        player.body.butt.rating -= 1 + randInt(3);
        if (player.body.butt.rating < 1) player.body.butt.rating = 1;
...
function kickFollowerShouldraOut(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    ...
    if (ShouldraFollowerFlags.SHOULDRA_PLOT_LEVEL < 4 || ShouldraFollowerFlags.SHOULDRA_TALK_NEEDED > 0) {
        CView.text("\"<i>Ah, you're booting me out, hey?</i>\"  Shouldra asks innocently, tumbling out of your body and corporealizing. \"<i>Wouldn't be the first, and won't be the last.  I do realize I have certain 'needs' that got cloying to many of my former hosts.</i>\"");
        ...
        CView.text("\n\nWith that, she turns on her heel and wanders out of   You watch her go for a while, then turn your focus back to other things.");
        // shouldra back to town ruins, reset to 'friendly' status
        ShouldraFollowerFlags.SHOULDRA_FOLLOWER_STATE = 0;
        return { next: returnToCampUseOneHour };
    }
    // She Fucking Explodes
    else {
        CView.text("Are you sure you want to ask her to leave after she opened herself up to you?  It's just a hunch, but she might not take this very well.");
        return { yes: definitelyKickOutCrazyGhosts, no: shouldraTalkMenu };
    }
}

// if yes
function definitelyKickOutCrazyGhosts(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("\"<i>What.</i>\"");
...
export function shouldersWarnings(player: Character): boolean {
    ...
    if (ShouldraFollowerFlags.SHOULDRA_SLEEP_TIMER === -96) {
        CView.sprite(SpriteName.ShouldraFollower); // 67;
    ...
    // 7 days (168 hours) since last Shouldra scene
    else if (ShouldraFollowerFlags.SHOULDRA_SLEEP_TIMER === -168) {
        CView.sprite(SpriteName.ShouldraFollower); // 67;
        ...
        if (randInt(3) === 0) {
            ...
            if (player.gender === 0) CView.text("libido seems to be going haywire");
            CView.text(".  Shouldra's unfulfilled sexual needs are starting to blend into your own.  Nothing you do will satisfy your urging until she's seen to her own.\n\n<b>Minimum lust raised by 20.</b>\n");
        }
        else if (randInt(2) === 0) {
    ...
    if (ShouldraFollowerFlags.SHOULDRA_SLEEP_TIMER === -216) {
        CView.sprite(SpriteName.ShouldraFollower); // 67;
...
export function nightTimeShouldraRapesThePC(player: Character): NextScreenChoices {
    ...
    ShouldraFollowerFlags.SHOULDRA_SLEEP_TIMER = 0;
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    player.orgasm();
    player.stats.lib += 1;
    player.stats.sens += 4;
    ...
    // event occurance = 0
    if (ShouldraFollowerFlags.SHOULDRA_TIMES_NIGHT_RAPED_PC === 0) {
    ...
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 1) CView.text(", circling their entire lengths around your [fullChest]");
    CView.text(".  It's quite amazing how ecstatic the tour feels, each pre-smeared exploration smothering you in sensual vibes.  Just moving the stiff little devils - particularly at your joints - sends shivers up your arm.");
    ...
    CView.text(", softening the ");
    if (player.body.cocks.length > 0) CView.text("magically enlarged member");
    else CView.text("magically gaped snatch");
    CView.text(" back to a much less craggy complexion.  Shouldra slowly drags you back to your [feet] after a minute or two of blissful respite and walks you closer to the cliffside to stare off into the moonlit horizon.  It never really dawned on you just how fucking high you are; just where the fuck did she take you?");
...
function edwardShouldraDickHandsPartII(player: Character): NextScreenChoices {
    ...
    ShouldraFollowerFlags.SHOULDRA_TIMES_NIGHT_RAPED_PC++;
    return { next: campMenu };
}
...
export function shouldraWakesUpOrPokesPCsForShitsAndGigglesIdunnoHowLongCanIMakeThisFunctionNameQuestionMark(player: Character) {
    CView.text("\n");
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    const choices = [];
    let select: number;
    ...
    // 1 PC with cock - 1
    if (player.body.cocks.length > 0 && player.body.cocks.sort(Cock.Largest)[0].area < 200) {
        choices[choices.length] = 1;
        choices[choices.length] = 1;
        choices[choices.length] = 1;
        choices[choices.length] = 1;
    }
    // 2 PC with BIG COCK - if (this.player.body.cocks.sort(Cock.Largest)[0].area >= 200)
    ...
    if (player.effects.get(StatusEffectType.Exgartuan).value1 === 1 && 9999 === 9999) {
    ...
    // 13 (is Naga)
    if (player.body.legs.isNaga()) choices[choices.length] = 13;
    // 14 (Amily is a follower)
    if (amilyFollower()) choices[choices.length] = 14;
    // 15 (Pure Jojo is a follower)
    if (player.effects.has(StatusEffectType.PureCampJojo)) choices[choices.length] = 15;
    // 16 (has any of Marae's gift perks)
    if (player.perks.has(PerkType.MaraesGiftStud) || player.perks.has(PerkType.MaraesGiftFertility) || player.perks.has(PerkType.MaraesGiftProfractory) || player.perks.has(PerkType.MaraesGiftButtslut)) choices[choices.length] = 16;
    // 17 (has had children with Izma)
    if (ShouldraFollowerFlags.IZMA_CHILDREN_SHARKGIRLS + ShouldraFollowerFlags.IZMA_CHILDREN_TIGERSHARKS > 0) choices[choices.length] = 17;
    // 18 (has Incorporeality perk)
    if (player.perks.has(PerkType.Incorporeality)) choices[choices.length] = 18;
    // 19 (possessed by Boobgartuan)
    if (player.effects.get(StatusEffectType.Exgartuan).value1 === 2) choices[choices.length] = 19;
    // 20 (PC knows any spells)
    if (player.combat.hasSpells()) choices[choices.length] = 20;
    // 21 (PC has anemone dick)
    if (player.body.cocks.filter(Cock.FilterType(CockType.ANEMONE)).length > 0) choices[choices.length] = 21;
...
function shouldraAndWormsYoureGonnaHaveABadTime(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("Your eyes glow yellow, signaling Shouldra's desire to stretch around a little.  You grant her mastery over your hands as you continue about your business.  The eager phantom decides to push things a little, however, systematically removing your [armor].  A break isn't that big a deal you figure, and you relinquish full control to the giddy ghost.  She directs you out of plain view");
    if (player.body.cocks.get(0).length < 48) CView.text(" and cuts right to the chase, ballooning your [cock] into a rigid, wide, four-foot tall slab of meat");
    CView.text(".  Unable to contain her delight, she hastily hugs your enormous dick firmly against your body and squeezes vigorously, rubbing your face against your [cockHead].  A warm stream of pre oozes out and onto your possessed face.");
    ...
    if (Time.hour <= 12) CView.text("morning ");
    else if (Time.hour <= 5) CView.text("afternoon ");
    else CView.text("evening ");
    CView.text("air.  Your hands clue the eidolon in to further activity, introducing her to your urethra all the way down to your [balls], every square wriggling inch chock-full of the little buggers, exploring their extended living quarters.");
...
export function shouldraAndExgartumonFightGottaCatchEmAll(player: Character): NextScreenChoices {
    // 40% chance to occur when player has both Shouldra and Cock Exgartuan and is going to sleep
    // Unlocks Argue 2
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("Your bedroll looks mighty comfortable after a hard day's ");
    if (player.stats.cor < 50) CView.text("work");
    else CView.text("fucking");
    CView.text(".  Just when you're [armor] and about to tuck in for the night, however, your [cock] goes fully erect!  The throbbing demonic mass is flailing about, knocking into your shit and making a mess.  You clutch onto the bucking bronco in an attempt to prevent any further damage when Exgartuan begins to shout, \"<i>Don't FUCKING touch me, you goddamn cunt!</i>\"");
    ...
    ShouldraFollowerFlags.SHOULDRA_EXGARTUDRAMA = 1;
    return { next: campMenu };
}

// Argue 2 - When out exploring
export function exgartumonAndShouldraFightPartII(player: Character) {
    ...
    ShouldraFollowerFlags.SHOULDRA_EXGARTUDRAMA = 2;
}
// Argue 3 - When out exploring again
export function exgartumonAndShouldraFightPartIII(player: Character) {
    ...
    ShouldraFollowerFlags.SHOULDRA_EXGARTUDRAMA = 3;

}

// Argue 4 - At bed, morning, time to decide
export function exgartuMonAndShouldraShowdown(player: Character): NextScreenChoices {
    ...
    if (User.settings.silly()) CView.text("  It's a man's right to choose what's done with his body.  Not anyone else's!");
    CView.text("  You shout loud enough to get the two to at least shut up and notice you're there.  It's time to end this little battle once and for all, starting with your first - and hopefully last - involvement.");
    // [Keep Exgartuan]	[Keep Shouldra]	[Keep Both!]
    return {
        choices: [
            ["Keep Exgartuan", keepExgartuanInsteadOfShouldra],
            ["Keep Shouldra", keepShouldraAndKickOutExgartuan],
            ["Keep Both", keepAllTheGhosts]
        ]
    };
}
// Keep Exgartuan
function keepExgartuanInsteadOfShouldra(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("In no uncertain terms, you announce to your former paranormal partner that you're siding with the demon on this one.  [if (corruption > 75) \"He's just a better teammate to have when it comes to hot dickings.  \"]You place your hand around your [cock] as Exgartuan as he rubs up against you, approving of your decision.  Shouldra looks on in just complete disgust and confusion.");
...
// Keep Shouldra
function keepShouldraAndKickOutExgartuan(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("You decide to jump to Shouldra's side on this one, explaining to your demonic cock your desire to go your separate ways.  Exgartuan heartily laughs at you, your [cock] wiggling in the air with each mighty heave.");
    ...
    // PC has human dick
    player.body.cocks.get(0).type = CockType.HUMAN;
    // Exgartuan departs
    player.effects.remove(StatusEffectType.Exgartuan);
    // unlocks one more scene for later that night
    ShouldraFollowerFlags.SHOULDRA_EXGARTUDRAMA = -0.5;
    return { next: campMenu };
}
// Keeping Shouldra 2 - Exgartuan's back from the dead!
export function keepShouldraPartIIExgartumonsUndeatH(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    ...
    CView.text("While slipping into your bedroll to enjoy a peaceful night's sleep, a chill runs down your spine.  You look around to-");
    ...
    ShouldraFollowerFlags.SHOULDRA_EXGARTUDRAMA = 0;
    // [next page]
    return { next: keepShouldraPartIIIExgartumonsUndeatH };
}
function keepShouldraPartIIIExgartumonsUndeatH(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("\"<i>Boo!</i>\"");
...
function keepAllTheGhosts(player: Character): NextScreenChoices {
    ...
    CView.text("\n\nShouldra grabs you by the arm, dragging you to a nearby clearing just outside   Exgartuan seems less than anxious to see what she's planning.  \"<i>What are you wasting my time with now?  Are you going to pull a dick out of a hat or something?</i>\"");
    ...
    if (player.cumQ() >= 1000) CView.text(", trails of your cum pouring out from them");
    CView.text(".");
    ...
    ShouldraFollowerFlags.SHOULDRA_EXGARTUDRAMA = 4;
    return { next: campMenu };
}

// Intro
export function shouldraBakeryIntro(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("You set off towards the bakery, and it's not long before the smell of freshly-baked pastries and sugary candies wafts through the air.  Something stirs in your mind as a familiar ghostly presence makes herself known; seems you're not the only one who noticed the scent of cinnamon.");
    ...
    // first time:
    if (ShouldraFollowerFlags.SHOULDRA_BAKERY_TIMES === 0) CView.text("You smirk, wondering how exactly a ghost manages to work up a hunger.  Your skepticism is met by an intense stabbing pain in your stomach, forcing you to buckle slightly from the cramping.  It lasts only a moment before it vanishes, however.  \"<i>You feel that, smartass?</i>\" The ghost girl sneers, \"<i>That's what decades of fasting will do to you.");
    else CView.text("You point out that she's already had something at the bakery.  \"<i>Yeah, I did.  And now I'm hungry again.  Deal with it.");
    CView.text("  Now, how about a snack?  Or two?</i>\" Her enthusiasm is clear...  and slightly worrying.");
    return { choices: [["Continue", feedShouldraACake], ["Bail Out", bailOut]] };
}

// Bail out (you pussy)
function bailOut(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("You quickly turn around and head back the way you came.  There's a moan of disappointment from Shouldra, before she settles down and goes back to doing whatever she usually does.  You try not to think about it as you leave the city and return to ");
    return { next: returnToCampUseOneHour };
}

// nter
function feedShouldraACake(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    CView.text("You decide that there's no use getting cold feet now; you wanted to go the bakery anyway, no harm in letting Shouldra have some fun while you're there.  ");
    // after first time:
    if (ShouldraFollowerFlags.SHOULDRA_BAKERY_TIMES > 0) CView.text("Well, nothing <i>permanent</i>, anyway.  ");
    CView.text("You press on, soon arriving at the ramp leading down into the bakery's interior.  It's as busy as ever, with patrons milling about, chatting between bites of pastry.");
...
function shouldraAndMaddieSittingInATree(player: Character): NextScreenChoices {
    ...
    if (player.body.tone >= 60) CView.text("well-toned");
    else if (player.body.thickness < 50) CView.text("trim");
    else CView.text("curvaceous");
    CView.text(" figure...");
    // [Go along with it] [Resist]
    return { choices: [["Go Along", goAlongWIthShouldrasEatingSpree], ["Resist", resistBeingAFatass]] };
}
// Go along with it
function goAlongWIthShouldrasEatingSpree(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ShouldraFollower); // 67;
    // if repeat scene:
    if (ShouldraFollowerFlags.SHOULDRA_BAKERY_TIMES > 0) CView.text("\"Man, you're not sure you want that kind of impact on your figure again...  \"");
    CView.text("Ah, what the hell.  You were hungry anyway.  You let Shouldra guide the cake into your mouth.  Mmm, not bad; rich, sweet and delicate.  Shouldra chuckles in your mind as she releases her control slightly, letting you lick your fingers clean of any stray crumbs.  \"<i>Well, guess you're pretty hungry yourself!  Alright, Champ, I'll give you some freedom, but just remember you're not the only one with an appetite.</i>\" You wince as your hunger intensifies to the verge of pain; looks like you're eating for two.  Your eyes lock on to the feast before you and something snaps in your mind.  You need this food, all of this food, inside you.  <i>Now</i>.");
    ...
    if (ShouldraFollowerFlags.PC_FETISH > 0) CView.text(", though not completely unwelcome");
    CView.text(".  You blush furiously and back into the corner of the (now rather cold) booth");
    if (ShouldraFollowerFlags.PC_FETISH > 0) {
        CView.text(" as your heart races and blood rushes down towards your ");
        if (player.body.cocks.length > 0) {
            CView.text("drowsy member");
            if (player.body.vaginas.length > 0) CView.text(" and ");
        }
        if (player.body.vaginas.length > 0) CView.text("trembling nether-lips");
        if (player.gender === 0) CView.text("groin");
    }
    CView.text(", before you realize that nobody seems to notice your predicament.  Shouldra reads your thoughts and responds, \"<i>Thought you might appreciate some privacy");
    if (ShouldraFollowerFlags.PC_FETISH > 0) CView.text(", even with that kink of yours");
    CView.text(", Champ.  Here's something extra anyway, just in case.</i>\" One ghostly incantation later, and ");
    if (player.body.femininity <= 50) CView.text("a buttoned shirt and pair of belted jeans");
    else CView.text("a tight-fitting tank top and skirt");
    CView.text(" adorn your person.");

    ShouldraFollowerFlags.SHOULDRA_BAKERY_TIMES++;
    player.slimeFeed();
    ...
    if (player.body.cocks.length > 0) {
        ...
        if (player.body.cocks.get(0).length < 8 || player.body.cocks.get(0).length > 25) CView.text("\"<i>First though, let's change it to something more manageable.</i>\" Arcane sounds float up towards you, causing your shaft to shift and morph until your throbbing rod is around 2 feet long and dripping pre.  \"<i>Mmm, much better!</i>\"  ");
        CView.text("Her eyes close as those sumptuous icing-white lips wrap around the [cockHead] of your aching dick, the cherry tongue canvassing your cockflesh like an artist, until she slowly, oh-so-slowly gulps down inch after gradual inch of your girth.  Her enormous chocolate rump is visible on the other side of the booth, lulling back and forth in contented bliss.");
