...
import { urtaLove, urtaCapacity, urtaSprite, knockUpUrtaChance } from "./Urta";
...
User.flags.set(FlagType.UrtaHeatRut, UrtaHeatRutFlags);

function urtaQuestDone(): boolean {
    return (UrtaHeatRutFlags.URTA_QUEST_STATUS === 1);
}

// [Approach Sober Urta]
function approachSoberUrtaHeatRutProc(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("As you walk over to the table where your foxy herm-lover sits, you think you see eyes following you, your presence attracting an unusual amount of attention. ");
    if (player.effects.has(StatusEffectType.Heat) && player.effects.has(StatusEffectType.Rut) && player.body.cocks.length > 0 && player.body.vaginas.length > 0) CView.text(" Males, females and herms alike stare at you with a mixture of puzzlement, longing and hostility, as if unsure whether they want to fight with you, fuck you senseless or be fucked senseless by you.");
    else if (player.effects.has(StatusEffectType.Rut) && player.body.cocks.length > 0) CView.text(" Males stare at you with a hint of challenge in their eyes, whilst females eye you attentively. Herms don't seem sure whether they want to fight you or fuck you.");
    // (Player in heat
    else if (player.effects.has(StatusEffectType.Heat) && player.body.vaginas.length > 0) CView.text(" Males, and individuals who look like females at first, until you spot the telltale bulge of at least one cock in garments or hanging freely between their legs, stare at you with obvious desire as you go, more than a few erections starting to form in your wake.");
    CView.text("  You ignore them all, concentrating on Urta.");

    CView.text("\n\nAs you approach, ");

    if (urtaLove()) CView.text(" she opens her arms to embrace you, and smiles.");
    else CView.text(" she smiles when she sees you, but then gives the air a sniff and immediately seems to gain more color to the skin under her fur.  \"<i>Uh, I, I see.</i>\"  You sit down next to her, but even though she isn't even suggesting anything yet, you already find it hard to control yourself.");

    if (player.effects.has(StatusEffectType.Rut) && player.body.cocks.length > 0) CView.text("  Urta is right next to you, and she has already proven herself a good lay");
    else CView.text("  You keep reminding yourself what Urta is packing between her legs and how much precious semen it can give, and can't help but want to try that now");

    CView.text(".  Not to mention, with or without the monster between her legs, her body is sexy as hell.");

    CView.text("\n\n\"<i>Well, you do help me with my own releases, so I suppose that's fair....");
    if (player.effects.has(StatusEffectType.Heat) && player.body.vaginas.length > 0) CView.text("  You... smell good, so I at least hope you're not here to tease me.");
    CView.text("</i>\"  She leans closer to you, and ");
    if (player.effects.has(StatusEffectType.Rut) && player.body.cocks.length > 0) CView.text("your eyes are quickly drawn to her full breasts as she whispers something to you. You're too hypnotized to care.");
    else CView.text("you lose yourself in your imagination about having Urta take you raw and pump you full of sperm.  There's a faint voice you hear, and you realize it's Urta's as you snap back to reality.");
    CView.text("  She coughs and repeats herself.  \"<i>Are you here because of your, um, condition? You'd prefer to blow off some steam with ");
    if (urtaLove()) CView.text("your adorable, hot vixen");
    else CView.text("someone you know and trust");
    CView.text(", right?</i>\"");
    ...
    if (player.effects.has(StatusEffectType.Rut) && player.body.cocks.length > 0) CView.text(" focus is on the hips of the fox-morph");
    else CView.text(" focus on catching a glimpse of that amazing cock, and you keep imagining what she can do with it");
    CView.text(".  Your aggressive hormonal state demands that you ");
    if (player.effects.has(StatusEffectType.Rut) && player.effects.has(StatusEffectType.Heat) && player.body.cocks.length > 0 && player.body.vaginas.length > 0) CView.text(" put a baby in her, then let her put a baby in you");
    else if (player.effects.has(StatusEffectType.Rut) && player.body.cocks.length > 0) CView.text(" put a baby in her so she can push it out of those beautiful hips");
    else CView.text(" put that beautiful big cock into your needy [vagina]");
    // [If Urta's fertility quest wasnât done]
    if (!urtaQuestDone()) CView.text(", even if something keeps reminding you that's impossible.");
    else CView.text(".");

    CView.text("\n\n\"<i>You know, [name] I'm glad to see you as always.  ");
    if (urtaLove()) CView.text("  I'm really thankful you managed to control yourself and see me with this.  You know that if you need me, I'm always ready to help, love.</i>\"");
    else CView.text("  I'm really flattered you thought of me when in such a state!</i>\"");
    ...
    if (player.effects.has(StatusEffectType.Rut) && player.effects.has(StatusEffectType.Heat) && player.body.cocks.length > 0 && player.body.vaginas.length > 0) CView.text("mixed genitals");
    else if (player.effects.has(StatusEffectType.Rut) && player.body.cocks.length > 0) CView.text("[cock biggest]");
    else CView.text("[vagina]");
    CView.text(" against her backside.");

    CView.text("\n\n\"<i>Um, you know, I'm horny, too, but at least wait 'til we get to my place...</i>\"");

    // Player in heat and rut:
    if (player.effects.has(StatusEffectType.Rut) && player.effects.has(StatusEffectType.Heat) && player.body.cocks.length > 0 && player.body.vaginas.length > 0) {
        ...
        if (player.body.cocks.find(Cock.CockThatFits(urtaCapacity()))) choices[0] = ["Cock", sateRutWithSoberUrta];
        else choices[0] = ["Cock", sateRutWithSoberUrtaButHuegDicked];
        choices[1] = ["Pussy", soberUrtaSatingPCHeat];
        return { choices };
    }
    // Player in rut:
    else if (player.effects.has(StatusEffectType.Rut) && player.body.cocks.length > 0) {
        ...
        if (player.body.vaginas.length > 0) CView.text("  Your " + describeCocksLight(player) + " and [vagina] are filling your clothes with mixed juices");
        else CView.text("  Your [cock biggest] begins to seep precum into your clothes");
        CView.text(", so you decide to dispose of them as you force Urta against a wall.");
        ...
        if (player.body.vaginas.length > 0) CView.text(" and your pussy dripping and eager");
        CView.text(".");

        if (player.body.cocks.find(Cock.CockThatFits(urtaCapacity()))) return { next: sateRutWithSoberUrta };
        else return { next: sateRutWithSoberUrtaButHuegDicked };
    }
    // Player in heat:
    else {
        ...
        return { next: soberUrtaSatingPCHeat };
    }
}

// [Approach Drunken Urta]
function approachDrunkenUrta(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("As you walk over to the table where your foxy herm-lover sits, you think you see eyes following you, your presence attracting an unusual amount of attention.");
    // Player in heat and rut:
    ...
    // Player in heat:
    else CView.text("  Males, and individuals who look like females at first, until you spot the telltale bulge of at least one cock in garments or hanging freely between their legs, stare at you with obvious desire as you go, more than a few erections starting to form in your wake.");
    CView.text("  You ignore them all, concentrating on Urta.");
    ...
    return {
        choices: [
            ["Interested", interestedInUburDrunkurtaRuts],
            ["NotInterested", notInterestedInUburDrunkUrtaRuts],
        ]
    };
}

// [=Not interested=]
function notInterestedInUburDrunkUrtaRuts(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You say it's her who seems to be different today, and gently push her away, before apologizing and making your leave.  ");
    if (player.body.face.type === 2) CView.text("Your nose does seem to pick up that Urta was and still is getting unusually aroused behind you.  ");
    if (player.effects.has(StatusEffectType.Rut) && player.body.cocks.length > 0) CView.text("[EachCock] twitches in protest at you leaving a willing lay behind.  ");
    // [Medium lust increase]

    // [If the PC has a snake tongue]
    if (player.body.tongue.type === TongueType.SNAKE) CView.text("You instinctively taste the air, and Urta suddenly seems utterly delicious. ");
    // Rut:
    if (player.effects.has(StatusEffectType.Rut) && player.body.cocks.length > 0) CView.text("Your " + describeCocksLight(player) + " hardens in your pants as you still force yourself to leave.");
    // [Medium lust increase]
    else CView.text("Your condition does seem kind of protesting against leaving such a willing lay behind, but it isn't time to be indulging it.");

    player.stats.lustNoResist += 5 + player.stats.lib / 20;
...
// [=Interested=]
function interestedInUburDrunkurtaRuts(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You raise an eyebrow and ask Urta if she minds telling you the reason for such attention and what exactly it is that you smell like.");

    // Rut And Heat:
    if (player.effects.has(StatusEffectType.Rut) && player.effects.has(StatusEffectType.Heat) && player.body.cocks.length > 0 && player.body.vaginas.length > 0) CView.text("\n\n\"<i>Gods, sexy!  You smell like you walked in here to make babies - like you want to grab the girls and the herms and plough them full of cum, even as you let the guys and the herms stuff your greedy cunt full of baby juice and make you just as pregnant.  It's the best smell ever...</i>\"");
    // Rut:
    else if (player.effects.has(StatusEffectType.Rut) && player.body.cocks.length > 0) CView.text("\n\n\"<i>Well, let me put it like this: you smell like you walked in here to grab every girl here by the hips and breed them all for hours on end, until the whole lot of them's knocked up.</i>\"");

    else if (player.effects.has(StatusEffectType.Heat) && player.body.vaginas.length > 0) CView.text("\n\n\"<i>Well, let me put it like this: You smell like you walked in here to tease and flirt with every man and herm in the place... stride into the middle of the floor like a queen, then pull off your clothes and bend over to let yourself be fucked and bred for hours until you were well and truly knocked up.</i>\"");

    // If player is in both heat and rut:
    if (player.effects.has(StatusEffectType.Rut) && player.effects.has(StatusEffectType.Heat) && player.body.cocks.length > 0 && player.body.vaginas.length > 0) {
        CView.text(" Urta shudders with desire and, to your shock, she starts openly fondling her huge prick ");
        if (!urtaLove()) CView.text("under her skirt");
        else CView.text("through her quickly-soaked dress");
        CView.text(".  \"<i>Fuuuck, but you smell so goood... Goddess, I don't know which part of you I want more!  Gimme some of that, sexy... please!</i>\" she begs you desperately.");
        ...
        return {
            choices: [
                ["Cock", partial(drunkUrtaIntroPartDuex, player, 1)],
                ["Vagina", partial(drunkUrtaIntroPartDuex, player, 2)],
                ["Watch", watchDrunkRuturtaJerkoff],
            ]
        };
    }
    else if (player.effects.has(StatusEffectType.Rut) && player.body.cocks.length > 0) {
        return drunkUrtaIntroPartDuex(player, 1, false);
    }
    else {
        return drunkUrtaIntroPartDuex(player, 2, false);
    }
}
// (If in heat OR rut, or did not pick Watch)
function drunkUrtaIntroPartDuex(player: Character, chosenSex: number = 1, newl: boolean = true): NextScreenChoices {
    if (newl) CView.clear();
    else CView.text("\n\n");
    urtaSprite();

    CView.text("Suddenly, Urta's hand dashes to ");
    // [if players has balls]
    if (player.body.balls.count > 0 && chosenSex === 1) CView.text("your [balls], rolling them in her hand and squeezing a bit too roughly.");
    else if (player.body.vaginas.length > 0) {
        CView.text("your [clit], rubbing it until it's ");
        if (chosenSex === 1) CView.text("the very same state [eachCock] seems to be in so often recently, ");
        CView.text("erect and throbbing.");
    }
    else CView.text("your [hips], stroking along them towards [eachCock] in an agonizing display of teasing.");
    ...
    if (chosenSex === 1) CView.text(", and [eachCock] already gets hard in your [armor].");
    else CView.text(", and you feel the inside of your [armor] rapidly slickened by your [vagina]'s excitement.");
    ...
    // Player in both :
    if (player.effects.has(StatusEffectType.Rut) && player.effects.has(StatusEffectType.Heat) && player.body.cocks.length > 0 && player.body.vaginas.length > 0) CView.text("and she proceeds to maul her breasts, pre-cum seeping copiously from her cock and her snatch drooling over her balls and onto the floor.");
    else if (chosenSex === 1) CView.text("but she doesn't seem to mind it for the moment.  Rather, the fur between her thighs appears to be rapidly soaking.");
    else CView.text("which she seems more than intent to push into you, although you do see a bit of female lubricant sliding down her balls.");
    ...
    if (chosenSex === 1) CView.text("hard");
    else CView.text("wet");
    CView.text(" for me</i>\"  Moments later, as you feel your naked [butt] against the table, her eyes dart to your ");
    if (chosenSex === 1) CView.text(describeCocksLight(player));
    else CView.text("[vagina]");
    CView.text(".  Her smile opens wider as Urta presents you with a sexy face that seems a mix of positively delighted and predatory.");

    // [If player is in Rut, OR is in Heat AND Rut AND chose "Cock":
    if (chosenSex === 1) CView.text("\n\n\"<i>Well, I'm sure you don't mind if I take all your spunk and those few hours of fucking all for myself, instead,</i>\" she says as she brings one leg onto the table and the side of you as she sheds her dress, lubricant now drooling from the fox's female half, falling onto your [hips].");
...
function watchDrunkRuturtaJerkoff(player: Character): NextScreenChoices {
    ...
    return {
        choices: [
            ["KeepTeasing", keepTeasingDatHornyFawkes],
            ["StopTeasing", stopTeasingDatHornyFox],
        ]
    };
...
// [=Stop Teasing=]
function stopTeasingDatHornyFox(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You decide there's no point taking things too far - besides, Urta seems just about ready to pop now, especially with how she's frantically suckling at her own cockhead.");
...
// [=Keep Teasing=]
function keepTeasingDatHornyFawkes(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You smirk; things are just starting to get fun around here, why would you stop now?  You start to amp up your routine even more, flaunting your scents and your goods for all the bar's occupants, not just the drunken fox-herm feverishly jerking herself off in front of you.  You can't recall if you've ever danced before coming to Mareth, but you don't think you do too bad as you perform an amateur strip-dance for the audience around you, slowly peeling off your [armor] and sensuously discarding them, bending over to let Urta and any other cock-bearing character present in the bar get a good look at your [vagina], ");
    if (player.body.balls.count > 0) CView.text("lewdly cupping and fondling your [balls], ");
    CView.text("slowly beginning to stroke and caress your " + describeCocksLight(player) + " for the benefit of those who bear vaginas.");
    ...
    if (player.body.legs.isGoo() || player.body.legs.isNaga()) CView.text("what passes for knees on your [legs]");
    else CView.text("knees.");
    ...
    if (player.stats.cor < 50) CView.text("run");
    else CView.text("walk triumphantly");
    CView.text(" from the racous orgy that is the Wet Bitch; truly an auspicious name.");

    // PC loses 1 hour, returns to TelâAdre, gains 15 lust, heat/rut unaffected//
    player.stats.lust += 15 + player.stats.lib / 10;
...
function soberUrtaSatingPCHeat(player: Character): NextScreenChoices {
    ...
    CView.text("As she presses your back against the wall, you spread your [legs] wide, allowing Urta easy access to your awaiting womanhood");
    // [If herm]:
    if (player.body.cocks.length > 0) {
        CView.text(" by shoving your " + describeCocksLight(player));
        if (player.body.cocks.length > 0 && player.body.balls.count > 0) CView.text(" and [balls]");
        CView.text(" out of the way");
    }
    else if (player.body.balls.count > 0) CView.text(" by shoving your [balls] out of the way");
    CView.text(".  Slowly, you feel the wide flared head of her magnificent pillar of flesh press onto your opening, spreading your vaginal lips and making your [clit] harden in anticipation just as your [butt] shivers, awaiting the intrusion.  Slowly, Urta's giant member starts to enter you.");
    if (player.vaginalCapacity() >= 100) CView.text("  Your hungry gash eagerly welcomes her, Urta easily sliding herself home.");
    else if (player.vaginalCapacity() > 60) CView.text("  You feel it stretch you in the most pleasuring of ways, filling you with warm, throbbing flesh as she pushes it deeper and deeper.");
    else CView.text("  You wince in pain at being stretched so much, but you grow wetter and wetter as Urta slowly pushes herself inside your depths, your [vagina] slowly getting wetter and warmer, accepting the intruder.  Pleasure starts to override pain and you let out a gasp.");
    ...
    // [If Very Loose]
    if (player.vaginalCapacity() >= 60) CView.text("Urta's erection seems completely at home in your depths, completely embraced by the wet walls of your throbbing [vagina].");
    else CView.text("You moan again, feeling Urta's member stretch, harden and throb, blood running through it.  You feel complete, and your heat is ignited further by the sensation of something so wonderful and big filling you.");

    CView.text("\n\nUrta leans closer into your body, pressing her big breasts against your [fullChest] as she reaches down to stimulate your ");
    if (player.body.cocks.length > 0) CView.text("[cock biggest]");
    else {
        CView.text("clitoris, ");
        if (player.body.clit.length < 2.5) CView.text("rubbing the hard, stimulated sensitive spot between her fingers");
        else CView.text("running her fingers over your entire big clit before stroking it like she'd do with a cock");
    }
    CView.text(", letting you have a few moments to get used to her mammoth member and accommodate it.");
    ...
    if (player.body.cocks.length > 0) CView.text(" Cum rushes out of [eachCock] and onto the soft fur of Urta's abdomen, then starts leaking out slowly in time with her thrusts.");
    CView.text(" Your cunt clamps down tighter onto Urta as you orgasm,");
    if (player.body.vaginas.get(0).wetness >= 4) CView.text(" pussy juice shooting out around her member, coating Urta's fur and soaking her balls and the base of her cock.");
    else CView.text(" love juices flowing liberally out of you, giving the skin of Urta's cock a visible shine and spreading onto her balls.");
    ...
    if (player.body.vaginas.get(0).wetness >= 4) CView.text("shooting out all over the place");
    else CView.text("running down her member and the crack of your [butt], coating the two of you in the proof of your passion");
    CView.text(".");
    ...
    if (player.body.cocks.length > 0) CView.text("your own member responding alike to Urta's, shooting out semen, ");
    CView.text("your [vagina] ");
    if (player.body.vaginas.get(0).wetness >= 4) CView.text("once again setting off to coat Urta in your juices.");
    else CView.text("squeezing on her, trying to milk her of her seed.");
    ...
    CView.text("\n\n\"<i>What?  ");
    if (!urtaLove()) CView.text("You're still not satisfied?</i>\"");
    else CView.text("You want to keep going, darling?  I want to make you satisfied, so I'll spare all the time I can!</i>\"");
    ...
    CView.text("hardening again in response to the act.  Urta continues to slam into your backside, delivering a small smack to your [butt].");
    ...
    // [Hermaphrodite]
    if (player.body.cocks.length > 0) CView.text("[eachCock] shoots its seed onto the floor of the alley, helplessly");
    else CView.text("your [vagina] squeezes and tugs on her shooting member, leaking its own juices all over her member, the intensity of the orgasm shaking you right into your core");
    CView.text(".");
    ...
    CView.text("\n\n\"<i>That felt amazing!  ");
    if (!urtaLove()) CView.text("If only I had you by hand every time I need to blow a load.");
    else CView.text("Your pussy is absolutely amazing, sweetheart: I hope I've helped you settle down?  Is your heat sated for the moment?");
    CView.text("</i>\"");
    ...
    CView.text("\n\n\"<i>");
    // ([Regular Urta]
    if (!urtaLove()) CView.text("Sure thing, I'm here to help,</i>\" she responds with a wink.");
    else CView.text("I'm here if you need me, love.  Whatever makes you happy.  You've helped me with my own problems so much already, so I owe you at least one.  Not to mention, making love to you feels absolutely amazing!</i>\"");
    ...
    if (!urtaQuestDone()) {
        ...
        CView.text("  She grabs your hand and helps you stand up.");
    }
    // [If Urta's fertility quest is done]
        ...
        if (UrtaHeatRutFlags.URTA_FERTILE === 1) {
            player.body.womb.knockUp(new Pregnancy(PregnancyType.URTA, IncubationTime.URTA), 25);
            player.body.womb.knockUp(new Pregnancy(PregnancyType.URTA, IncubationTime.URTA), 25);
            player.body.womb.knockUp(new Pregnancy(PregnancyType.URTA, IncubationTime.URTA), 25);
        }
    }
    ...
    UrtaHeatRutFlags.URTA_COMFORTABLE_WITH_OWN_BODY++;
    urtaLove(1);
    ...
    UrtaHeatRutFlags.URTA_TIME_SINCE_LAST_CAME += 6;

    return { next: returnToCampUseOneHour };
}

// Quote:[=Sate Rut with Sober Urta: Cock Too Big=]
function sateRutWithSoberUrtaButHuegDicked(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    const largestCock = player.body.cocks.sort(Cock.Largest)[0];
    CView.text("Urta observes your approaching " + describeCock(player, largestCock) + " with a hungry, eager expression. Without taking her eyes off of it, she flips up her ");
    if (!urtaLove()) CView.text("skirt");
    else CView.text("dress");
    CView.text(", her huge horse-cock whipping up to slap between her breasts, pre already dribbling down the tip.  However, as you get closer, her expression suddenly falls and her cock begins to sink.  \"<i>I'm sorry, but... that's just too big!</i>\" she protests.  \"<i>You smell absolutely gorgeous, and I'm willing to try with something a little bigger than usual, but that's... that's just frigging huge - there's no way I could fit that monster!</i>\" she declares. \"<i>Just because I have a stallion cock doesn't mean I have a mare's cunt!</i>\"");

    CView.text("\n\nYou stare at your " + describeCock(player, largestCock) + " in an uncomprehending fashion.  How are you going to find relief now?  You look up as Urta saunters over towards you in a seductive manner, erect cock jutting before her and a mischievous grin on her face.  \"<i>While I'll have to go and attend to this myself later, I can still give you a hand...</i>\" she purrs throatily.  She kneels before you, and you have to watch yourself to avoid having her twenty inches of cock slap your [leg].  Confidently, she reaches out to take hold of your " + describeCock(player, largestCock) + ", first with one hand, and then the other.");
    ...
    if (!urtaLove()) {
        ...
        if (player.cumQ() <= 50) CView.text("spattering spooge onto Urta's face");
        else if (player.cumQ() <= 250) CView.text("splashing her face and tits with your cum");
        else CView.text("soaking the shocked fox-morph in your jism");
        CView.text("; though you still feel the urge to breed fertile females, your lust is momentarily sated.  Urta wipes ");
        if (player.cumQ() >= 500) CView.text("halfheartedly ");
        CView.text("at the cum on her faces and gives you a friendly smile.  \"<i<There, that should feel better; I know what it's like to be all pent up with no release.</i>\"  She rises to her feet, still holding your cock, and gives it a squeeze.  \"<i>In the meantime, if you want to really cut loose, I suggest you go and find something to shrink this down.</i>\"");
    ...
    // Lover Urta Path/
        ...
        CView.text("\n\nShe pulls her head free and gasps for breath, looking at you in admiration. \"<i>By Marae... how do you manage to do this for me, lover?</i>\" she asks, stunned.  But then, gamely, she returns her attention to your " + describeCock(player, largestCock) + ", licking, suckling and mouthing at the head.  As she does this, she continues her restless assault with her hands upon the rest of your lengthy shaft with her skilled hands.  She shuffles closer, allowing you to rest yourself between her DD-cups.  \"<i>Can you feel how fast my heart is beating through your cock?  You turn me on so very much,</i>\" she purrs.");

        CView.text("\n\nIt's no surprise that eventually you can't hold back anymore and release yourself into Urta's ready mouth.  ");
        if (player.cumQ() <= 50) CView.text("She swallows it all without complaint, licking her lips clean of any stray droplets once you remove it from her mouth.");
        else if (player.cumQ() <= 250) CView.text("Her cheeks bulge at the amount you pump into her, but she's able to swallow with an admirable amount of control, gasping for breath.");
        else CView.text("Rivulets of cum burst from between her lips, but she stubbornly sucks and swallows as much as she can, her stomach starting to bulge from the sudden influx of spooge.  When you stop, she is drooling cum and gasping for breath, but she meets your eyes with pride, even as she gives a wet belch.");
    ...
    UrtaHeatRutFlags.URTA_TIME_SINCE_LAST_CAME += 1;
    return { next: returnToCampUseOneHour };
    /// Player returns to Tel'Adre Menu, usual one hor passes//
}

// Quote:[=Sate Rut with Sober Urta=]
function sateRutWithSoberUrta(player: Character): NextScreenChoices {
    const largestCock = player.body.cocks.sort(Cock.Largest)[0];
    CView.clear();
    urtaSprite();
    CView.text("Urta observes your approaching package with a hungry, eager expression. Without taking her eyes off of it, she flips up her skirt, her huge horse-cock whipping up to slap between her breasts, pre already dribbling down the tip.");
    ...
    CView.text(" she mumbles around your tongue, so caught in lust she doesn't care where you are.  Indeed, it seems your little fox-herm has something of an exhibitionist streak...");

    CView.text("\n\nWell, you don't intend to keep the lady waiting any longer then you have to, and you slide it home, allowing her testicles to drop once you are safely inside.  The globular organs bounce atop your " + describeCock(player, largestCock) + " and rock back and forth against your belly, jiggling with every movement the two of you make, Urta's cock sandwiched firmly between your bodies and held between her breasts.");
    ...
    if (urtaLove()) CView.text("blows you a kiss with a flirty wink of her eye and then ");
    CView.text("scrubs off the worst of the cum plastered in her fur with the dry parts of her clothes.  She saunters playfully over to a garbage bin, throwing them inside.");
    if (urtaLove()) CView.text("  Her hips deliberately sway in a mesmerizing fashion, tail flicking from side to side to give you a tantalizing grip of her still-dripping snatch and the now-flaccid prick underneath.");
    CView.text("  You watch as she pries at a loose stone with her claws, removing it to reveal a small cubby, from which she withdraws a spare set of clothing.");

    // If fertility quest done:
    if (urtaQuestDone()) {
        CView.text("\n\nYou ask Urta what she's going to do; you didn't have the chance to use a condom.");
        if (UrtaHeatRutFlags.URTA_FERTILE === 1) CView.text("\n\n\"<i>We'll just have to hope then, won't we, [name]?</i>\"");
        ...
        if (!urtaLove()) CView.text("\"<i>And you had better learn to control yourself, " + mf(player, "mister", "missy") + ".");
        else CView.text("\"<i>But you should learn to control yourself lover-" + mf(player, "boy", "girl") + ".");
        CView.text("  I'd rather do something like this in the privacy of my home than in the middle of the street.</i>\"");
    ...
    UrtaHeatRutFlags.TIMES_RUT_FUCKED_URTAS_CUNT++;
    player.stats.lib += 1;
    player.stats.sens -= 2;
    player.stats.lust = 0;
...
function drunkenUrtaFucksPCInHeat(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You simply smile at her, inhaling deeply through your nose; you can swear you can smell her herm-musk, wafting off of her as strongly as your own pheromones must be wafting off of you.  That wonderful, earthy, primordial scent, the odor of a healthy breeder ready and willing to mate... there's nothing like it.");
    // (Fertility Quest not done:
    if (!urtaQuestDone()) CView.text("  You know that she can't give you babies... but, honestly, at this moment, you couldn't care less. Her wonderful stallion cock is there and ready and it's all for you.");
    else CView.text("  You dimly recall that Urta could be virile, but she's right there, her huge mare-cock just dripping to plunge into your ripe [vagina] and knock you up.  You don't care anymore; you want her babies!");

    CView.text("\n\nUrta's eyes glitter with emotion and she bends down to give you another kiss before roughly grabbing her ");
    if (!urtaLove()) CView.text("skirt");
    else CView.text("dress");
    CView.text(" and hiking it up to expose her pre-spewing cock.  A steady trickle of feminine lubricant dribbles out of her gash, oozing over her apple-sized balls and dripping noisily onto the floor, but Urta couldn't care less; she's too fixated on plunging her maleness into your ready pussy to notice her feminine urges.");
    ...
    if (player.body.cocks.length > 0) {
        CView.text(", impatiently nudging aside your " + describeCocksLight(player));
        if (player.body.balls.count > 0) CView.text(" and your [balls]");
        CView.text(" to get to the feminine orifice below");
    }
    CView.text(". You cry out in joy as your [vagina] is filled, her soft fur brushing against your inner walls before she opens her mouth as wide as she can and sticks out her tongue. The long, broad, vulpine appendage slides out and up your depths, caressing the walls, lapping up your juices, ");
    if (player.body.clit.length < 2.5) CView.text("playing with your [clit]");
    else CView.text("lapping around your huge pleasure-button, slurping it into her mouth where she suckles from it");
    CView.text(", and making you thrash and scream until you think you're going to cum from the pussy-eating alone.  ");
    if (player.body.vaginas.get(0).wetness >= 4) CView.text("As if to emphasize this, your squelching, slurping pussy suddenly sprays fem-cum all over Urta's face, making her pull herself free with a shocked shake of her head.");
    else CView.text("As if sensing your impending orgasm, Urta removes her head from your [vagina], giving you a cocky grin and shaking her head.");
    ...
    UrtaHeatRutFlags.URTA_TIME_SINCE_LAST_CAME += 24;
    UrtaHeatRutFlags.URTA_COMFORTABLE_WITH_OWN_BODY++;
    urtaLove(1);
...
function sateRutWithDrunkUrtaWithHugeDick(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("Urta climbs on top of you, pressing her wet slit against the head of your cock.  She does attempt to get it inside, but the pain of being stretched quickly makes her realize that even if she managed to, the fuck would be less than enjoyable and more than a little painful.  She groans and growls in frustration, pulling herself off you and pressing her apple-sized balls ");
    if (player.body.balls.count > 0) CView.text("against your own [balls]");
    else if (player.body.vaginas.length > 0) CView.text("against your [clit]");
    CView.text("between your [hips]");
    CView.text(".");
    ...
    if (player.body.cocks.sort(Cock.Largest)[0].area < 150) CView.text("compete with minotaurs or something?");
    else CView.text("set a world record or something?");
    CView.text("</i>\"\n\n");
    ...
    CView.text("\n\nRealizing there's no point in talking to her and just somewhat sated, you return to your ");
...
function drunkUrtaRidesARutPCsCock(player: Character): NextScreenChoices {
    CView.clear();
    const largestCock = player.body.cocks.sort(Cock.Largest)[0];
    urtaSprite();
    CView.text("You decide not to fight the situation; truthfully, the promises given by the sight of the hot hermaphrodite vixen climbing on top of you ignite your lusts and sing to your rut in ways you don't think you'd be able to safely ignore.");

    CView.text("\n\nUrta lifts her balls and presses your already hard cock against her black pussy lips, justifying your decision.  Slowly, you feel your tip sink into her, then the rest of your member, as she releases a soft moan.  She keeps pressing down, forcing your " + describeCock(player, largestCock) + " inside her wet gash until you feel her butt pressed against your ");
    if (player.body.balls.count > 0) CView.text("[balls]");
    else CView.text("[hips]");
    CView.text(", and her own balls resting comfortably on your abdomen.  She grits her teeth as you feel her pussy slowly tighten around and massage your " + describeCock(player, largestCock) + ", and you groan in response.  Urta starts to bounce up and down on your member, smacking her thighs and ass into your body, not minding her own giant equine member flopping around and occasionally smacking you over your chest.  She lets her tongue flick out into the air for a moment and moans in delight, before picking up the pace and starting to pump your cock in and out of her hungry passage furiously.");
    ...
    if (player.body.balls.count > 0) CView.text("feel your balls churning");
    else CView.text("recognize the tingling sensation from your " + describeCock(player, largestCock) + " to your spine");
    CView.text(", a telltale sign that your penis is ready to unload the potent spunk inside the herm-nympho bouncing on top of you.  You try to announce this, but she doesn't slow down.  Moments later, ");
    // [Normal cum production]
    if (player.cumQ() <= 50) CView.text("you unload your thick spunk into her pussy");
    else if (player.cumQ() <= 250) CView.text("you erupt inside her, filling her with wave after wave of warm, thick semen");
    else CView.text("you blow massive surges of spunk inside her continuously.  The fox's stomach expands, rivulets of semen running down onto the table as she continues her ride on top of you");
    CView.text(".  As you discharge, the fox's pussy starts squeezing and contracting, and you recognize the hermaphrodite's own orgasm.  Thicker white liquid suddenly drips from the tip of her cock quite copiously and occasionally shoots out into the air.");
    ...
    if (largestCock.hasKnot()) CView.text("You slowly feel the blood flow to your erect penis, and the knot at the base of it seems to start to expand");
    else CView.text("You don't mind staying like this for much, much longer, and the hermaphrodite on top of you seems to share that point of view");
    CView.text(".  Summoning your slightly exhausted strength, you pull her hips down deeper into your own before starting to return the hermaphrodite's shakes and bounces with thrusts of your own.  It intensifies the sensations, making your lover yelp in surprise before smiling widely and releasing low, content moans and purrs from the depths of her throat.");
    ...
    if (largestCock.hasKnot()) CView.text("knot surge with more blood and your erection twitch, ensuring that you're completely trapped inside her until you're empty.");
    else CView.text("tip expand as you're getting ready to blow another load.");
    CView.text("  Urta moans on top of you, increasing the speed of her movements and running her hand up and down her own member faster and with more intense strokes.");
    ...
    CView.text(" Urta masturbates furiously as your body and your " + describeCock(player, largestCock) + " finally gives in to your rut and the vixen's intense lust.  The two of you come in complete sync this time, her wet, black pussy milking you with vice-like tightness and intense, rhythmic squeezing as she fires a huge rope of jizz onto the floor next to your table before painting your chest and abdomen with lesser white strings.");

    // [If large cum production]
    if (player.cumQ() >= 500 && player.cumQ() <= 1500) CView.text("\n\nUrta seems positively full and stretched now, and your rut almost subsides for a moment, her stretched tummy imitating a pregnancy quite well.");
    else if (player.cumQ() > 1500) {
        CView.text("\n\nUrta is stretched and completely full, so, holding your " + describeCock(player, largestCock) + " in her hand for a moment, she lifts herself off your cock, presses on her abdomen and allows some of your spunk to drip out of her, coating the table, your thighs and your ");
        if (player.body.balls.count > 0) CView.text("[balls]");
        else if (player.body.vaginas.length > 0 && player.body.clit.length >= 3.5) CView.text("cock-like clit");
        else if (player.body.vaginas.length > 0) CView.text("pussy lips");
        else CView.text("inside of your thighs");
        CView.text(" with your own thick white semen.  Then, she slides your softening erection back into herself again.");
    }

    CView.text("\n\n\"<i>Good... so good!  Just a few more rounds, and we'll be done - don't go fucking soft on me!</i>\"");

    CView.text("\n\nYour " + describeCock(player, largestCock) + " feels sensitive and tender after the two past orgasms, but the horny nympho just forces it deep into her snatch, the sensitive head and shaft all the way down to your base again squeezed tighter as she starts to bounce up and down, trying to breathe life into your erection.  She kisses you furiously and passionately, exploring your mouth with her tongue as she keeps the stimulation on your groin, cooing, groaning and panting as if trying to breathe her own lust into you.");

    CView.text("\n\n\"<i>Come on, come on, whatever it takes!  Squeeze those boobies, smack that ass, just get fucking hard!</i>\"");

    CView.text("\n\nFollowing her plea, you give her breasts a rather rough squeeze, and you feel your " + describeCock(player, largestCock) + " miraculously harden inside her once again.");
    if (player.body.balls.count > 0) CView.text("  Your [balls] don't seem to be completely empty yet");
    else CView.text("  It doesn't appear to be the end of what your body can produce");
    CView.text(", and your vulpine lover squeals with delight as she feels you harden inside her.  She smacks your [chest] with her huge, half-hard member.");

    CView.text("\n\n\"<i>Care to give me a hand with that?  There's quite a lot of cream to squeeze out of this one, too.</i>\"");

    CView.text("\n\nYour tired and lust-hazed mind sees no reason not to, and you grab her, so Urta's massive mare member starts to be pleasured by her own hand as well as yours.  Squealing in delight, the horny vixen returns to what seems to occupy her mind completely: bouncing fervently on top of you, with your " + describeCock(player, largestCock) + " trapped in her wet snatch.");
    ...
    if (player.body.vaginas.length <= 0 && player.body.balls.count === 0) CView.text(" the inside of your thighs");
    else if (player.body.balls.count > 0) CView.text(" your [balls]");
    else if (player.body.vaginas.length > 0) CView.text(" your [clit]");
    CView.text(" when she's pressed entirely against you.  She smacks her own bottom and you run your other hand over her body as the two of you tug on and pull her massive erection towards your face and over your [chest], thick dollops of pre-cum and leftover semen dripping from her member over your body.  Urta licks her lips as she looks down on you with ");
    if (!urtaLove()) CView.text("intense, unadultered lust");
    else CView.text("possessiveness and a hungry, but somehow warm expression");
    CView.text(".");

    CView.text("\n\nI should make you do this every day!</i>\"");
    if (urtaLove()) CView.text("  She suddenly seems a bit shy and surprised, before saying, \"<i>Make sure to remind me I said that later....</i>\"");
    ...
    if (!urtaLove()) CView.text("Show me you're " + mf(player, "the man", "the breeder") + " here!");
    else CView.text("You don't want to make me feel like I'm abusing you, do you?  I know you want this, so pound me!</i>\"");

    CView.text("\n\nAfter a while, it becomes a torture to resist the stimulation and you try to bring your own orgasm faster by responding to Urta's movement, stretching your [hips] and pushing your " + describeCock(player, largestCock) + " deeper into her pussy.  Although the two of you have trouble catching a common rhythm, she appears way more vitalized and enthusiastic about this than you are.");

    CView.text("\n\nAfter a long time of slightly too rough of a ride, your " + describeCock(player, largestCock) + " twitches inside the horny vixen again, and she almost howls in her own climax before yours is set off. Another shot of cum coats your [chest] as her hungry love-hole milks your member, forcing you to ejaculate for the third time, delivering even more semen into her insides.");
    if (player.cumQ() >= 500) CView.text("  If the two of you were thinking rationally, you'd know quite well it could be quite dangerous to her innards to continue, but Urta seems intent on continuing this ordeal, or at least, that's how you feel when looking at her expression of bliss and ecstasy, eyes rolling to the back of her head and tongue dancing around in the air.");

    CView.text("\n\nYou try to announce that you're done and that's about it, but the horny nymphomaniac that seems intent on keeping your rutting " + describeCock(player, largestCock) + " all to herself feels differently.  She allows your member to slide out of her for a moment, but only to let out some of the spunk that's filling her and to stimulate you in a different way.  Urta moves down your body, stroking your [chest] and your [nipples] before lowering her face and arms to your [hips].  She licks your sensitive head for a while before delivering a couple of noisy, strong sucks to your " + describeCock(player, largestCock) + ".  Afterwards, she licks your ");
    if (player.body.balls.count > 0) CView.text("[balls]");
    else if (player.body.vaginas.length > 0) CView.text("[vagina] and [clit]");
    else CView.text("taint");
    CView.text(" before finally running her tongue over your [asshole], and all the way up again, swallowing your member once again as she squeezes your [butt].  You alternate before moaning, groaning, and whimpering as blood seems to leave your head and entire body, surging into your " + describeCock(player, largestCock) + " and making it hard to think or move, much less resist.");
    ...
    if (urtaLove()) CView.text("  You can rest later!  I'll even give you a massage, so keep giving it to me!");
    CView.text("</i>\"");
    ...
    if (player.cumQ() <= 200) CView.text("  The table already seems like it'll be stained white by Urta's efforts alone, and your own fluid doesn't help the situation.");
    else if (player.cumQ() < 1000) CView.text("  The two of you are already fucking in a small pool of your mixed semen sauce, making naughty sounds heard all over the bar, and her vagina, dripping with your fluids, only makes the pool seem bigger.");
    else if (player.cumQ() >= 2000) CView.text("  Your mixed sperm is covering the table and the floor, and woe to anyone who will have to clean this place up.  Despite the ridiculous amount of fluids around, she seems set on the idea that the two of you are barely mid-way through the act.");
    ...
    if (player.cumQ() >= 1000) CView.text("  Well, maybe you can, but let's make some twins!");
    CView.text("  Keep going!</i>\"");
    ...
    if (player.body.tallness <= 93) CView.text("making you lick, kiss and suck on her flared, wide tip");
    else CView.text("staring deep into your eyes as her erection smacks you against your [chest]");
    CView.text(".");
    if (!urtaLove()) CView.text("  She grins at your lack of resistance.");
    else CView.text("  \"<i>You're mine, right?  Right now, you're mine and mine alone.  My wonderful virile breeder, all " + mf(player, "his", "her") + " seed just for me.</i>\"");
    ...
    if (player.body.balls.count > 0) CView.text(", your [balls] frantically gathering any cum that is left in them to prepare for another surge of shots into the hermaphrodite's eager nethers");
    CView.text(".  You seem to slowly lose the sensations in most of your body.  The awareness of your surroundings slips away, as all your thoughts and feelings focus in your groin, struggling to keep up with the mad pace of breeding that Urta has kept so far.  It's hard to keep conscious as you feel another surge of ecstasy pass all over your body and focus on your " + describeCock(player, largestCock) + ", shooting off yet again inside her coaxing hole.  You hear the sound of your name being called and the milking, squeezing sensations that you barely manage to associate with her ravenous, insatiable pussy.");
    ...
    if (player.stats.cor > 35 || UrtaHeatRutFlags.PC_FETISH > 0) CView.text("You tell her it was actually a great experience to have such passionate sex publicly.  She blushes at that suggestion.");

    // [If Urta is infertile]
    if (!urtaQuestDone()) {
        CView.text("\n\n\"<i>Well, I guess it was nice, if painful afterwards, to be of use to someone who was so ready and willing to breed.");
        if (urtaLove()) CView.text("  If only I could... uh, I guess I should just enjoy what I can.");
        CView.text("</i>\"");
    }
    // [If Urta is fertile]
    else {
    ...
    CView.text("\n\nShe ");
    if (!urtaLove()) CView.text("winks at you as you get ready to leave.  I hope I'll get to play with you again soon.</i>\"");
    else {
        CView.text("gives you a quick but loving kiss before getting up and starting to prepare to leave the house.  As you get ready to move, she mumbles something, and reaches, grabbing your arm, shyly.\n\n\"<i>[name], next time, umm: find me when I'm sober.  I'll get to see how it feels to be wanted by someone the way I usually crave you, okay?</i>\"");
    ...
    // [Both end here]
    CView.text("\n\nSatisfied even if your body hurts all over and your " + describeCock(player, largestCock) + " seems empty, dry, burning, and almost ruptured.  You leave the house with a small smile on your face, and a strong stench of sexual fluids following you back to the ");
