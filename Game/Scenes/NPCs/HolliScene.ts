...
import { numToCardinalCapText, numToCardinalText, numToOrdinalText } from "../../Utilities/NumToText";
import { returnToCampUseOneHour, returnToCampUseTwoHours } from "../Camp";
import { campMenu } from "../../Menus/InGame/PlayerMenu";
import { describeCock, describeCockHead, describeCocksLight, describeCocks } from "../../Descriptors/CockDescriptor";
...
export const HolliFlags = {
    FOUGHT_HOLLI: 0,
    FUCK_FLOWER_LEVEL: 0,
    HOLLI_SUBMISSIVE: 0,
    TIMES_FUCKED_FLOWER: 0,
    TIMES_RIDDEN_FLOWER: 0,
    HOLLI_FRUIT: 0,
    HOLLI_FRUIT_EXPLAINED: 0,
    HOLLI_DEFENSE_ON: 0,
    FOLLOWER_AT_FARM_HOLLI: 0,
    FARM_CORRUPTION_STARTED: 0,
    HOLLI_FUCKED_TODAY: 0,
    CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE: 0,
    FUCK_FLOWER_GROWTH_COUNTER: 0,
...
User.flags.set(FlagType.Holli, HolliFlags);
...
function fightHolli(player: Character): NextScreenChoices {
    HolliFlags.FOUGHT_HOLLI = 1;
    return CombatManager.beginBattle(player, new Holli());
}

export function treeMenu(player: Character, output: boolean = true): NextScreenChoices {
    if (output) CView.clear();
    let ride: ClickFunction;
    let fuck: ClickFunction;
    let burnIt: ClickFunction;
    const choices: ScreenChoice[] = [];
    if (HolliFlags.FOUGHT_HOLLI > 0) {
        // If You Fight, Run, and Come Back To Her in the Menu -Z
        CView.text("The demon tree is still here, and the face peeking from it still stares daggers at you even from a distance.  It looks like forgiveness won't be forthcoming.");
        // [Fight][Call Jojo(req Small Talisman)][Retreat]
        // Call Jojo goes to above scene, others go to fight or camp directly

        choices[0] = ["Fight", fightHolli];
        if (player.inventory.keyItems.has("Jojo's Talisman")) choices[1] = ["Call Jojo", callDatJojo];
        choices[4] = ["Back", inventoryMenu];

        return { choices };
    }
    else if (HolliFlags.FUCK_FLOWER_LEVEL === 1) {
        if (output) CView.text("The sprout looks about the same as when you first noticed it.  It's a simple, leafy shoot that only goes to about knee height.  It looks healthy and strong, with a few dozen branches and shiny green leaves.  If you look closely, the veins on the undersides of the leaf are purplish and pulse slightly with corruption.  You could easily destroy it.");
        return { choices: [["Burn It", destroyDatFukkinTree], ["", undefined], ["", undefined], ["", undefined], ["Back", inventoryMenu]] };
    }
    else if (HolliFlags.FUCK_FLOWER_LEVEL === 2) {
        // [Fuck It] [Ride Stamen] [Do Nothing] [Destroy It]
        if (output) CView.text("Before you is a large, waist-high plant of questionable origin.  A huge, violet bloom sits atop a strong, central stalk, and the obscene flower looks positively female in every sense, right down to a clit-shaped bump and moist, gently parted lips.  Inside that central passage, you can see a bevy of wiggling staman surrounding a larger, flared stalk.  The scent that wafts out of that opening is sweet and floral, but undoubtedly sexual.  It looks like an organic sex toy and is clearly corrupt.  It wouldn't be hard to get rid of right now, assuming you don't let it grow larger.");
        if (player.body.cocks.length > 0 && player.body.cocks.find(Cock.CockThatFits(100)) && player.stats.lust >= 33) fuck = fuckFuckingFuckFlowerP2;

        if (player.body.vaginas.length > 0 && player.stats.lust >= 33) ride = rideDatFuckingFukkFlowerP2;
        return { choices: [["Fuck It", fuck], ["Ride Stamen", ride], ["", undefined], ["Destroy It", destroyDatFuckingPlantAtP2], ["Back", inventoryMenu]] };
    }
    else if (HolliFlags.FUCK_FLOWER_LEVEL === 3) {
        if (output) CView.text("The familiar plant has blossomed into a nicely sized tree, though you doubt it has finished growing just yet.  It sports an outstretched canopy with nice, green leaves.  Unfortunately, you can still trace the corrupted veins on their undersides from below.  The vaginal flower is still there and is in full bloom, now several feet across and practically dripping with moisture.  Just up the trunk, there's a pair of small, roughly b-cup breasts bulging out of the bark.  They're exquisitely smooth and soft, and they ooze sweet-smelling sap that your tongue would love to taste.  In the canopy above, tentacle vines idly writhe about, though they show no sizes of aggression.");
        if (player.perks.has(PerkType.Dragonfire) || player.perks.has(PerkType.FireLord) || player.perks.has(PerkType.Hellfire) || player.effects.has(StatusEffectType.KnowsWhitefire)) {
            if (output) CView.text("\n\nIt could be burned down with your supernatural fire, but it would definitely tire you.");
            burnIt = torchP3Tree;
        }
        if (player.body.cocks.length > 0 && player.stats.lust >= 33) fuck = fuckTheFlower;
        if (player.body.vaginas.length > 0 && player.stats.lust >= 33) ride = rideTheWalrusP3;
        // [Fuck flower] [Drink Sap] [Ride Tentacles] [{Torch It}] [Leave It]
        return { choices: [["Fuck Flower", fuck], ["Drink Sap", drinkThePlantGirlsSap], ["Ride Tentacle", ride], ["Torch It", burnIt], ["Leave It", inventoryMenu]] };
    }
    else {
        // Camp Menu (edited)
        if (output) {
            ...
            if (HolliFlags.HOLLI_SUBMISSIVE === 1) {
                CView.text("\n\n\"<i>Welcome back, [name].  ");
                if (HolliFlags.TIMES_FUCKED_FLOWER > 0 && randInt(3) === 0) CView.text("Did you want to fertilize me again?  My flower is ready and waiting...");
                else if (HolliFlags.TIMES_RIDDEN_FLOWER > 0 && randInt(3) === 0) CView.text("Did you want to use my tentacles again?  They've been restless without you...");
                else if (randInt(2) === 0) CView.text("Did you come for a drink?  I've built up lots of nectar for you.  Surely, a sip would be refreshing for you...");
                else CView.text("How may I serve you?  Come, give me a command so that I can fulfill it for you.");
                CView.text("</i>\"");

            }
            else {
                CView.text("\n\n\"<i>Welcome back, [name].  ");
                if (HolliFlags.TIMES_FUCKED_FLOWER > 0 && randInt(3) === 0) CView.text("Did you want to fertilize me again?  Gardening is such exhausting work, but so rewarding...");
                else if (HolliFlags.TIMES_RIDDEN_FLOWER > 0 && randInt(2) === 0) CView.text("Did you come back to play again?  My tentacles are so restless.");
                else if (randInt(2) === 0) CView.text("Did you come for a drink?  I've built up lots of nectar for you.  Surely a hungry adventurer like yourself needs a sip, right?");
                else CView.text("Do you want to play?  Come on, taste Mother Marae's finest creation.");
                CView.text("</i>\"");
            }
            // Extra line on description:
            if (HolliFlags.HOLLI_FRUIT > 0) {
                if (HolliFlags.HOLLI_FRUIT === 1) CView.text("\n\nA large, purple fruit hangs from Holli's branches, twisting softly in the breeze.  It looks succulent and ripe.");
                else CView.text("\n\n" + numToCardinalCapText(HolliFlags.HOLLI_FRUIT) + " large, purple fruits hang from Holli's branches, twisting softly in the breeze.  They look succulent and ripe.");
                // Optional Addition:
                if (HolliFlags.HOLLI_FRUIT_EXPLAINED > 0) CView.text("  A very small, strange part of you actually feels proud to have made something like that with the tree-woman.");
            }
        }
        if (HolliFlags.HOLLI_SUBMISSIVE === 0) {
            let eat: ClickFunction;
            if (HolliFlags.HOLLI_FRUIT > 0) eat = eatHolliFruit;
            if (player.body.cocks.length > 0 && player.stats.lust >= 33) fuck = fuckHolliInZeFlowerPuss;
            if (player.body.vaginas.length > 0 && player.stats.lust >= 33) ride = level4RideHollisTentacruels;

            let guardT: string = "";
            if (HolliFlags.HOLLI_DEFENSE_ON === 1) guardT = "Stop Guarding";
            else guardT = "Guard Camp";
            burnIt = askHolliToWatch;

            // [Fuck Her] [Drink] [Tentacle Ride] {Guard Camp} {Threaten} [Leave]
            return { choices: [["Fuck Holli", fuck], ["Drink Sap", haveAMapleSyrupSnack], ["Ride Tentacles", ride], [guardT, burnIt], ["Eat A Fruit", eat], ["", undefined], ["", undefined], ["", undefined], ["", undefined], ["Leave", inventoryMenu]] };
        }
        else {

            if (player.body.cocks.length > 0 && player.stats.lust >= 33) choices[0] = ["Fuck Holli", holliGetsDickDommed];
            if (player.body.cocks.filter(Cock.FilterType(CockType.TENTACLE)).length >= 10 && player.stats.lust >= 33) choices[1] = ["TentacleBone", fullOnTentacleTasticGangBangForHolli];
            if (player.body.vaginas.length > 0 && player.stats.lust >= 33) choices[2] = ["Ride Holli", vaginalDomHollisTentacruels];
            choices[3] = ["Drink Sap", haveAMapleSyrupSnack];
            if (HolliFlags.HOLLI_FRUIT > 0) choices[4] = ["Eat A Fruit", eatHolliFruit];
            choices[5] = ["Guarding", askBrokenHolliToGuard];
            choices[9] = ["Leave", inventoryMenu];

        }

        if (HolliFlags.FOLLOWER_AT_FARM_HOLLI === 0 && HolliFlags.FARM_CORRUPTION_STARTED === 1) choices[6] = ["Farm Help", helpWithFarm];
        return { choices };
    }
}

function helpWithFarm(player: Character): NextScreenChoices {
    CView.clear();

    CView.text("You ask if a plant goddess might not be able to help the farm you recently acquired.");
    ...
    if (HolliFlags.HOLLI_FUCKED_TODAY === 0) {
        ...
        return {
            choices: [
                ["Yes", doFarmFuckery],
                ["No", noPlzDontFuckWithFarm],
            ]
        };
    }
}

function doFarmFuckery(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Holli closes her eyes and hums. The sound seems to reverberate in your bones, and you feel the ground move uneasily below you. Holli’s roots shift ponderously, and you hear new growth crackling, burying eagerly through the soil, moisture being taken in. The tree woman opens her eyes to lazily grin at you as the portentous sounds below you continue.");

    CView.text("\n\n“<i>It’s happening. I hope you and your doggie enjoy what I have done!</i>”");

    HolliFlags.FOLLOWER_AT_FARM_HOLLI = 1;

    return { next: returnToCampUseOneHour };
}

function noPlzDontFuckWithFarm(player: Character): NextScreenChoices {
    CView.clear();

    CView.text("“<i>Such a fickle mortal,</i>” she sighs. “<i>But I am a kind goddess. Ask me for it anytime, assuming you’ve... performed the rites.</i>”");

    return treeMenu(player, false);
}

function fertilizeHolli(player: Character, cock: boolean = true) {
    // 20% chance per sexing.  Up to bonus 20% for jizz or fertility! Max 62%.
    let odds: number = 20;
    if (cock && player.body.cocks.length > 0) {
        odds += player.cumQ() / 300;
        if (odds > 40) odds = 40;
        if (player.perks.has(PerkType.MaraesGiftStud)) odds += 10;
        if (player.perks.has(PerkType.FerasBoonAlpha)) odds += 10;
        if (player.perks.has(PerkType.ElvenBounty)) odds += 2;
    }
    if (!cock && player.body.vaginas.length > 0) {
        odds += player.totalFertility() / 5;
        if (odds > 40) odds = 40;
        if (player.perks.has(PerkType.MaraesGiftFertility)) odds += 10;
        if (player.perks.has(PerkType.FerasBoonBreedingBitch)) odds += 10;
        if (player.perks.has(PerkType.ElvenBounty)) odds += 2;
    }
    if (odds >= randInt(101)) {
        HolliFlags.HOLLI_FRUIT += 1 + randInt(Math.floor(odds / 10));
        // trace("Holli pregged!");
    }
}

// Sprout Phase: (edited)
export function getASprout(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Around the edge of your camp, you spot some plant-life beginning to sprout.  The barren, wasteland crust is fractured and broken, giving up a leafy green shoot.  It only reaches up to your knee, but the plant looks healthy and young.  Perhaps it will grow larger?  There is a central stem that supports most of the weight, but a dozen branches fork off, supporting a bevy of shiny green leaves.");
    ...
    if (HolliFlags.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE === 2) CView.text("  She even promised to plant a tree in your ..");
    CView.text("  You look closer and see purple veins on the undersides of the leaves, throbbing with what can only be the tainted liquids that suffuse much of this planet.  Fully grown, it might spread its corruption to your \n\n<b>Do you destroy it?</b>");
    HolliFlags.FUCK_FLOWER_LEVEL = 1;
    HolliFlags.FUCK_FLOWER_GROWTH_COUNTER = 0;
    // [Yes] [No]
    return { choices: [["Yes", destroyDatFukkinTree], ["No", letZeFuckingSproutLive]] };
}
// [Yes] Destroy Tree (edited)
function destroyDatFukkinTree(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You reach down and begin to excavate meticulously around the tainted sapling.  It has one central root that seems to go down forever, but after you've uncovered half a foot of taproot, you grab hold of it and yank.  At first, your straining seems unnoticed by the plant, the leaves rustling as it shakes and strains under your grip.  Then, all at once, it gives with a violent SNAP.  You tumble end over end, clutching to the demon-flora tightly to your [chest].  Sighing and grumbling, you get up and toss the leafy demon into your fire.");
    ...
    HolliFlags.FUCK_FLOWER_KILLED = 1;
    return { next: returnToCampUseOneHour };
}
// [No] (edited)
function letZeFuckingSproutLive(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Looking down at the sapling, you stay your wrath.  It may be corrupt, but it hasn't done anything to harm you just yet.  You give it a little pat on the uppermost leaves and leave it be.  It's not like it's going anywhere.");
...
export function fuckPlantGrowsToLevel2(player: Character): NextScreenChoices {
    CView.clear();
    HolliFlags.FUCK_FLOWER_LEVEL = 2;
    HolliFlags.FUCK_FLOWER_GROWTH_COUNTER = 0;
    CView.text("The sound of creaking, snapping vines draws your attention to the corner of your camp, where the small plant had grown.  You rush in that direction without delay, raising your [weapon] in case you need to fight.  The violent noises die away before you can round the last large stone in the intervening distance, leaving you to wonder just what transpired.  Well, you don't have to wait long - you come around the obstruction at top speed, skidding to a stop at the view before you.");
    ...
    if (player.gender === 0) CView.text("  Sucks to be it!  You don't waste time with stupid shit, like genitals.");
    CView.text("  Is this Marae's idea of a gift?");
    if (player.gender > 0) CView.text("  Well, you could give it a whirl... just being around it seems to slowly turn you on.");
    CView.text("  Of course, destroying it would be the safest option.");
    player.stats.lustNoResist += 33;
...
// Fuck It (skimmed)
function fuckFuckingFuckFlowerP2(player: Character): NextScreenChoices {
    CView.clear();
    ...
    CView.text(" and form a sticky puddle.  Now moaning, you grab the stem and start to pump the sheath up and down, using the plant like the cock-toy it was meant to be.  Lewd, wet squishes ring out through the air,");
    if (player.stats.cor < 33) CView.text(" and you timidly look around, hoping you haven't attracted an audience.");
    else if (player.stats.cor < 66) CView.text(" and you worriedly glance around, hoping yet terrified that someone will walk in on it.");
    else CView.text(" and you glance around, hoping someone will walk in on the sexy show you're making.");
    CView.text("\n\nNow panting with unbearable lust and undeniable levels of pleasure, you give up all sense of propriety and resort to fucking wildly, [hips] jumping inches into the air.  The flower's interior is a slick, silky heaven for your " + describeCock(player, cockThatFits) + ", a sucking, squeezing hole of seemingly infinite pleasure.  Inside its gluttonous gullet, the numerous tentacles twist around, the longest ones circling the base of your shaft");
    if (player.body.cocks.find(Cock.HasSheath)) CView.text(" inside your violated sheath");
    CView.text(".  With so much blood trapped in your " + describeCock(player, cockThatFits) + ", it's bigger and harder than ever, twitching valiantly against its restraints with every beat of your heart.");
    CView.text("\n\nOne of the tentacles circles your " + describeCockHead(cockThatFits) + " for a moment, and without much warning or pause, it lances forward to bury itself into your vulnerable cum-slit.  Your preconceptions are turned on their heads when there's a complete lack of pain from the abrupt penetration.  If anything, it actually feels kind of good... like there's a warm, slippery finger caressing your penis from the inside out.  It slithers in and out of you, pumping you inside, burrowing pleasure into your center even while stroking you from without.  Your body, burning from the exertion of fucking the flower's pod and the constantly-rising tidal wave of lust, begins to twitch spasmodically.");
    CView.text("\n\nThe internal suction spikes as your dick bloats larger, and with a roar of bestial pleasure, you arch your back, hands pushing your " + describeCock(player, cockThatFits) + " as deep into the plant's gullet as possible.  ");
    if (player.body.balls.count > 0) CView.text("Your [balls] gurgle, a surprisingly happy sound, and cling tight to your loins.  ");
    CView.text("Inside you, you can feel the liquid heat of your spurting release exploding out, melting everything before it with liquid, orgasmic bliss.  A low moan bursts from your lips when the sounding tentacle pulls out of your sloppy cock-slot, and the rushing tide of bubbling cum follows it, bursting into the sweet, suckling nectar-pot.");
    if (player.cumQ() > 500) CView.text("  You pour forth your prodigious essence without pause or fail, and the plant immediately begins to drink it down, the stem bulging with rounded balls of sperm that it carries underground.");
    if (player.cumQ() > 1500) CView.text("  Soon, the meager stem cannot swallow it all.  Spunk froths and squirts out around the base of your tool for a moment, and then a veritable tide of the stuff gushes out to soak  you.");
...
function rideDatFuckingFukkFlowerP2(player: Character): NextScreenChoices {
    ...
    if (player.body.clit.length > 4) CView.text("  Your heavy buzzer is so big that the flexible plant-cock has plenty of room to curl around it, and if you could see it, your clit would look almost like a candy-cane (if one set of stripes was jerking off the cane beneath).");
    if (player.body.cocks.length > 0) CView.text("  [EachCock] soon has its own partner twisting around it, a pussy-slicked vine to jerk and twine about its girth.");
    CView.text("\n\nWith nothing to occupy your roving hands, you let them slide up your " + skinFurScales(player) + " to your ");
    if (player.body.chest.length > 1) CView.text("lowest row of tits, cupping them before groping to your top row of breasts");
    else CView.text("[chest]");
    CView.text(", caressing the");
    if (player.body.chest.sort(BreastRow.Largest)[0].rating > 1) CView.text("ir");
    CView.text(" swell as you search for the lust-tightened skin of your [nipples].  They seem almost as sensitive as little clits, and as soon as you find them, you immediately set to circling them slowly, occasionally tugging and twisting when a particularly powerful sensation works through your pleasure-wracked body.");
    if (player.lactationQ() > 50) {
        CView.text("  One after the other, they begin to ");
        if (player.lactationQ() < 150) CView.text("trickle");
        else if (player.lactationQ() < 500) CView.text("leak");
        else CView.text("spray torrents of");
        CView.text(" milk.");
    }
    CView.text("  You moan in unabashed delight, your mouth hanging open in a mindless, blissful 'o' in between vocalizations.  If your friends back home could see you now, writhing in the dirt, face open in simple, animal pleasure, and perhaps worst of all, fucking a plant, you'd die of ");
    if (HolliFlags.PC_FETISH === 0 && player.stats.cor + player.stats.lib < 150) CView.text("embarrassment");
    ...
    if (player.body.cocks.length > 0) CView.text("  [EachCock] explodes in release, pumping heavy flows of seed into the petals, even as y");
    else CView.text("Y");
    CView.text("our quivering pussy, already sopping wet, releases the last of your juices in a soaked ");
    if (player.cumQ() < 50) CView.text("drizzle");
    else if (player.cumQ() < 250) CView.text("stream");
    else if (player.cumQ() < 750) CView.text("spray");
    else CView.text("torrent");
    CView.text(".  The tentacles inside you curl in on themselves and slowly pull back from the well-creamed quim, scraping out as much fluid as possible from your oversensitive honey-pot.  The binding on your [clit] releases, which of course sets off a whole new wave of rippling pleasure-contractions in your cunt.");
    ...
    player.stats.fatigue += 5;
    HolliFlags.TIMES_RIDDEN_FLOWER = 1;
    if (HolliFlags.FUCK_FLOWER_GROWTH_COUNTER < 1000) HolliFlags.FUCK_FLOWER_GROWTH_COUNTER += 4;
    return { next: returnToCampUseOneHour };
}

// Phase 3 Intro: (edited)
export function flowerGrowsToP3(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Again, you hear odd noises coming from a corner of your camp - the corner where you let that tainted shrubbery grow.");
    if (HolliFlags.TIMES_FUCKED_FLOWER + HolliFlags.TIMES_RIDDEN_FLOWER > 0) CView.text("  The fucking probably only helped it to grow faster.");
    CView.text("  The distinct sound of wood bowing and creaking under new weight catches your ears, and well before you can get to the plant, you can see a leafy canopy stretching towards the sky.  <b>It's grown into a small tree, with bark, leaves, and everything!</b>  Warily, you round one of the ancient standing stones to get a better look.");
    ...
    let burnIt: ClickFunction;
    if (player.perks.has(PerkType.Dragonfire) || player.perks.has(PerkType.FireLord) || player.perks.has(PerkType.Hellfire) || player.effects.has(StatusEffectType.KnowsWhitefire)) {
        CView.text(", though you suppose you could burn it down with magical fire if you set your mind to it");
        burnIt = torchP3Tree;
    }
    HolliFlags.FUCK_FLOWER_LEVEL = 3;
    HolliFlags.FUCK_FLOWER_GROWTH_COUNTER = 0;
    CView.text(".\n\n<b>What do you do?</b>");
    let fuck: ClickFunction;
    let ride: ClickFunction;
    ...
    if (player.body.vaginas.length > 0 && player.stats.lust >= 33) ride = rideTheWalrusP3;
    // [Fuck flower] [Drink Sap] [Ride Tentacles] [{Torch It}] [Leave It]
    return { choices: [["Fuck Flower", fuck], ["Drink Sap", drinkThePlantGirlsSap], ["Ride Tentacle", ride], ["Torch It", burnIt], ["Leave It", campMenu]] };
}

// Fuck Flower (skimmed)
function fuckTheFlower(player: Character): NextScreenChoices {
    CView.clear();
    ...
    if (player.body.vaginas.length > 0) CView.text("  Wetness builds in your [vagina] as well, but you're focusing on your maleness just now.  Your neglected twat will have to wait its turn.");
    CView.text("  A feverish sweat breaks out on your brow, though whether from your own arousal, the plant's heat, or some chemical in its juices, you cannot tell.");

    CView.text("\n\nBefore you can pull back, the hiss of something cutting through the air with lightning speed hits your eardrums.  Almost at the same time, tentacles (stamen?) snap around your neck, head, and ears, going taut in an instant.  Your " + describeFaceShort(player) + " is pulled roughly into the glossy hole, the soft folds of the petals smearing your nose and mouth with their saccharine sweetness.  You cannot help but breathe in more deeply as you struggle for your freedom, and with every struggle, every struggling gasp, [eachCock] grows hotter and harder, pulsating with tactile hunger.  With each moment that passes the motivation for your struggles shifts from freedom to fucking.  You don't care about getting away so much as getting your dick in this aromatic fuck-pot.");
    ...
    CView.text("\n\nRacking your brain for ideas, you momentarily slump into a temporary acceptance of your position, sagging heavily into the strong, fibrous tentacles.  Perhaps that's what the unnatural creature wants, for as soon as you let it have control, the tendrils swing you closer, close enough for [oneCock] to kiss the warm, wet hole.  Momentum carries you away from the promise of greater pleasure, your " + describeCock(player, largestCock) + "'s tip nicely moistened by lubricants and its own heavy bead of liquid pleasure.  When you swing back, a few inches are lucky enough to nestle inside, and you find yourself moaning without meaning to.");

    CView.text("\n\nYou're swung to and fro, held aloft by a shrubbery harness seemingly intent on teasing you.  First, you get a taste of heaven, but only on your " + describeCockHead(largestCock) + ".  Then, you're left to flutter in the breeze, the only sexual sensation available the cool wind fluttering across your whetted tip.  Over and over, the plant teases you until you lose track of time.  The closest thing to a watch you have is the gentle throb in your [balls] and its gradually strengthening intensity.  Oh, how it aches!  You feel taut and stretched like a harp string being tuned, only the musician just keeps cranking away, ratcheting up the pressure until you feel about to snap.  Your one comfort is the momentary bliss of the flower's sweet kiss, an exciting pleasure that's slowly being turned into an aching hell.");
    CView.text("\n\nDuring a particularly shameless moan, you realize that you're dipping a little deeper with each pendulous stream.  Luckily, the corrupt tree's arboreal lust must be rising like your own, and after a few slow swings, your " + describeCock(player, largestCock) + " is slipping three quarters of the way in");
    if (largestCock.area >= 150) CView.text(", sliding down the trunk as easily if the entire inside was a giant pussy just waiting to get fucked");
    CView.text(".");
    if (player.body.cocks.length > 1) {
        CView.text("  Your other penis");
        if (player.body.cocks.length > 2) CView.text("es rub");
        else CView.text(" rubs");
        CView.text(" sensually across the moist outer blossom, drooling their clear pre-cum over the petals.");
    }
    CView.text("  Schliiick.  You slide out.  Slorp-pop.  You slide in, this time to the ");
    if (player.body.cocks.find(Cock.HasSheath)) CView.text("sheath");
    else CView.text("base");
    if (player.body.balls.count > 0) CView.text(" with your gurgling balls slapping against the flower's edge");
    CView.text(", accompanied by a small squirt of sweet lubricants.");
    CView.text("\n\nYou've been kept on edge for so long that even this brief taste of liquid-soft snugness has your body clenching and warm bliss mounting inside of you.  You wrap as much of your [legs] around the trunk below as possible and give into your pleasure, thrusting your " + describeCock(player, largestCock) + " against the petalled muff each time the vines draw you away.  You rut like a demon's pet, half-mad with pleasure and desire so powerful it obviates rational thought.  The ecstasy rises like an all consuming tide, tinting your vision pink as your eyes roll back and [eachCock] thickens.");

    CView.text("\n\nJizz blasts out of your " + describeCocksLight(player) + " with uncommon force, painting the flower's interior with white");
    if (player.body.cocks.length > 1) CView.text(", as your multiple endowments allow you to drizzle the purple bloom's exterior with alabaster cream");
    CView.text(".");
    if (player.cumQ() > 500) CView.text("  Pearly ropes big enough to fill a flask lurch out from your cum-slit, propelled by your quivering orgasmic muscles deep into the trunk, and you swear you can hear the pitch of your sexual pumping rising in tone, changed by the flooded vessel.");
    if (player.cumQ() > 1500) CView.text("  A wave of your thick gunk rolls out from under your pumping cock - you've filled the tree beyond even its capacity, and the thick puddle forming beneath your [feet] is all the proof you need of your virility.");
    if (player.body.vaginas.length > 0) {
        CView.text("  Your hungry cunt, long denied in this exchange, trembles as it ");
        if (player.body.vaginas.get(0).wetness < 4) CView.text("leaks");
        else CView.text("sprays");
        CView.text(" your own female fluids in sympathy.");
    }
    CView.text("  After what feels like forever, the spurting seed no longer flows from your long-abused dick, and as your member softens, the tentacles swing you away, setting you down softly.");
    ...
    player.stats.fatigue += 5;
    if (HolliFlags.FUCK_FLOWER_GROWTH_COUNTER < 1000) HolliFlags.FUCK_FLOWER_GROWTH_COUNTER += 5;
    HolliFlags.TIMES_FUCKED_FLOWER++;
    HolliFlags.HOLLI_FUCKED_TODAY = 1;
    return { next: returnToCampUseOneHour };
}

// Drink Sap (edited)
function drinkThePlantGirlsSap(player: Character): NextScreenChoices {
    CView.clear();
    ...
    if (User.settings.silly()) CView.text(", tree hugger that you are");
    CView.text(".  The thick 'milk' quickly fills your body with energy, though it runs out nearly as soon as it started.");
...
// Ride Tentacles (C)
function rideTheWalrusP3(player: Character): NextScreenChoices {
    CView.clear();
    // (Let them pull you into the tree and bang, battering you with cum repeatedly.)
    CView.text("You eye the tentacles hiding amongst the leafy branches above, writhing obscenely around each other as they snake through the foilage.  You shed your [armor] into a worthless pile and bare your body, cupping your [chest] enticingly as you whistle up at the tree.  Your calls seem to fall on deaf ears; the whirling vines stay put up in their perch.  They're ignoring your delicious, feminine flesh!  Irritated, you stomp up to the trunk's flower and plunge your fist inside it.  Fresh fluid gushes out over your armor, releasing a spray of sweet, musky lubricants.  You blindly feel around inside the pliant plant-pussy with your fingers, giggling to yourself whenever the soft cavity clenches tightly to your arm.  Whatever this is, it can feel pleasure.");
    ...
    // cuntChange:15
    displayStretchVagina(player, 15, true, true, false);
    ...
    CView.text("\n\nAt some point, you are set back down on the ground and sprayed with a fresh batch of seed.  You rest there for a while, until you can catch your breath and move without pain.  When you start trying to clean up, you realize the puddle of cum is completely gone, likely swallowed back up the tree's hungry roots, and the only mess left is your sodden " + player.body.skin.desc + ".  You get dressed and stumble away a little drunkenly, your [vagina] raw and oozing.");
    ...
    if (HolliFlags.FUCK_FLOWER_GROWTH_COUNTER < 1000) HolliFlags.FUCK_FLOWER_GROWTH_COUNTER += 5;
    HolliFlags.TIMES_RIDDEN_FLOWER++;
    HolliFlags.HOLLI_FUCKED_TODAY = 1;
    return { next: returnToCampUseOneHour };
}

// Torch It (edited)(C)
function torchP3Tree(player: Character): NextScreenChoices {
    CView.clear();
    // Requires fire-based attack and fatigue at or below 30.  Maxes fatigue and removes the tree.
    CView.text("This has gone on long enough!  This thing cannot continue to grow in your camp any longer, and you have just the means to remove it: fire.  ");
    // [(nerdfire)
    if (player.effects.has(StatusEffectType.KnowsWhitefire)) CView.text("Closing your eyes to focus, you gather your energies, and unleash your white, supernatural flames upon the thing.");
    // (bromancefire)
    else if (player.perks.has(PerkType.FireLord)) CView.text("Sucking in your breath, you inflate your chest and stir the embers of the jaguar demon's gift.  Dredging up an enormous pine-green fireball, you exhale and launch it at the tree.");
    else CView.text("Sucking in your breath, you inflate your chest and stir the embers of your fiery gift.  Dredging up an enormous fireball, you exhale and launch it at the tree.");
    CView.text("  It goes up by the torch, though the fire starts to fade sooner than you would expect.  You huff, but repeat the action, layering a new blaze atop the previous one; you put all your concentration into this new conflagration, stoking the inferno with every ounce of your ");
    if (player.effects.has(StatusEffectType.KnowsWhitefire)) CView.text("magical will");
    else CView.text("breath");
    CView.text(".");
...
export function treePhaseFourGo(player: Character): NextScreenChoices {
    ...
    if (HolliFlags.TIMES_FUCKED_FLOWER + HolliFlags.TIMES_RIDDEN_FLOWER > 0) CView.text("Did you enjoy feeding me?  You should have seen the expression on your face when you were cumming in me.  Your eyes were rolling back and your mouth was just hanging open.  Do you know what it made me want to do?");
    else CView.text("Did you enjoy watching me grow?  You should have fed me.  It was so much harder to grow without proper nutrients.  Do you know what I thought about during all the time I spent waiting?  Do you know what it made me want to do?");
    CView.text("</i>\"");
    ...
    HolliFlags.FUCK_FLOWER_LEVEL = 4;
    HolliFlags.FUCK_FLOWER_GROWTH_COUNTER = 0;
    player.stats.lust += 15;
...
export function JojoTransformAndRollOut(player: Character): NextScreenChoices {
    ...
    if (player.stats.cor < 25) CView.text("Not willing to lie to your friend, you also admit that she seems to have changed after your actions in the factory - that, though you were trying to render the tanks permanently inoperable, the spillage you created pushed the cthonic spirit over the edge.");
    else CView.text("Though, you do leave out the precise, needless details of what caused the change.");
    CView.text("  The monk's eyes widen in surprise and surmise.");

    CView.text("\n\n\"<i>Unbelievable...</i>\" he blurts.  \"<i>");
    if (HolliFlags.TIMES_FUCKED_FLOWER + HolliFlags.TIMES_RIDDEN_FLOWER === 0) CView.text("To think that I must now consider Marae herself my enemy.  Forgive me, but this place is not safe for us anymore if it has attracted the attention of such a creature.  For now, I will return to the forest.  I know that you believe your mission keeps you here, but I urge you to stay away from that thing and watch the portal from an adequate distance.");
    // (totally deflowered that flower)
    else CView.text("And even knowing that this could be the case, you still chose to fornicate with it?  [name]... I cannot say that I am not disappointed.  This place is not safe for me anymore.  For now, I will return to the forest.");
    CView.text("</i>\"");

    CView.text("Jojo looks at you sadly; any argument you try to make is met with a resigned shake of his head.  \"<i>If the creature truly is the demonic offspring of a corrupted earth spirit, doubtless it will be powerful and the wrath of its parent severe.");
    if (player.stats.level < 20) CView.text("  Nonetheless, if you find yourself in a position where you must confront this demon or perish, call upon me.  I will render what aid I can, though I prefer it not be in vain.");
    else CView.text("  Though, a mortal of your stature and resources may be able to destroy her and escape the reprisal.  I would counsel against direct confrontation until you are sure of victory, but if you must fight, then call upon me and you will have what aid I can give.");
    CView.text("</i>\"  So saying, he presses a small talisman into your hand.");

    CView.text("\"<i>Until then, you will find me in the forest if you need my assistance.</i>\"  Jojo makes a polite bow and departs.");
    CView.text("(<b>Jojo is no longer a companion!</b>)");
    ...
    player.inventory.keyItems.add("Jojo's Talisman", 0, 0, 0, 0);
...
export function amilyHatesTreeFucking(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Your mousey cohabitant tramps up to you, irritation plain on her expression.");
    ...
    // [Stay Quiet][Slap Her]
    return { choices: [["Stay Quiet", stayQuietWhileAmilyBitchesAboutFuckingArborday], ["Slap Her", slapAmilysWhoreFace]] };
}
// [Stay Quiet]
function stayQuietWhileAmilyBitchesAboutFuckingArborday(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Amily berates you for some while longer, then eyeballs you.  Just as you begin to speak, she cuts you off again.");
    ...
    HolliFlags.AMILY_FOLLOWER = 0;
    // Set - amily flipped her shit
    HolliFlags.AMILY_TREE_FLIPOUT = 1;
    HolliFlags.UNKNOWN_FLAG_NUMBER_00173 = 0;
    // Enable village encounters
    HolliFlags.AMILY_VILLAGE_ENCOUNTERS_DISABLED = 0;
    // Change to plain mouse birth!
    const pregFromAmily = player.body.wombs.find(Womb.PregnantWithType(PregnancyType.AMILY));
    if (pregFromAmily) pregFromAmily.pregnancy.type = PregnancyType.MOUSE;
    return { next: campMenu };
}

// [Slapaho]
function slapAmilysWhoreFace(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You slap Amily across the ");
    if (HolliFlags.AMILY_NOT_FURRY === 0) CView.text("nose");
    ...
    if (HolliFlags.AMILY_NOT_FURRY === 0) CView.text("mouse-");
    CView.text("girl says, eyes wet with tears, \"<i>but I don't.  Goodbye.</i>\"");

    CView.text("\n\nWithout another word, she turns, gathers her things, and leaves.");
    HolliFlags.AMILY_FOLLOWER = 0;
    HolliFlags.AMILY_CORRUPT_FLIPOUT = 1;
    HolliFlags.AMILY_VILLAGE_ENCOUNTERS_DISABLED = 0;
    HolliFlags.AMILY_VILLAGE_ACCESSIBLE = 1;
...
// (FUCK DAT TREE BITCH)
function fuckHolliInZeFlowerPuss(player: Character): NextScreenChoices {
    CView.clear();
    HolliFlags.HOLLI_FUCKED_TODAY = 1;
    const largestCock = player.body.cocks.sort(Cock.Largest)[0];
    CView.text("With a knowing smile, you toss your [armor] aside and say, \"<i>Let's do this.</i>\"  [EachCock] juts out, hard and proud from your body, pointed straight towards the entrancing woman before you.  She leans down from her perch in the tree and rolls her shoulders from side to side, jiggling her pale green assets back and forth, traces of syrup hanging from her pendulous melons.");
    CView.text("\n\n\"<i>Well, come on over then, ");
    if (player.body.tallness < 84) CView.text("little");
    else CView.text("big");
    CView.text(" " + mf(player, "boy", "girl") + ",</i>\" the arboreal tramp suggests with a husky coo, \"<i>I got you some sugar.</i>\"  She pinches her nipples and gasps, dark streams of sugary juices dripping wetly into the soil as she beckons you closer.");
    ...
    if (player.body.cocks.length > 1) {
        if (player.body.cocks.length === 2) CView.text("  Your other penis splays");
        else CView.text("The rest of your penises splay");
        CView.text(" to the side, smearing furrows of the stuff around as they slide over the butter-smooth skin.");
    }
    CView.text(" The demon dryad's hand reaches below to ");
    if (player.body.balls.count > 0) CView.text("cradle your [balls], gently massaging your sack");
    else {
        CView.text("gently caress your ");
        if (player.body.cocks.find(Cock.HasSheath)) CView.text("sheath");
        else CView.text("loins");
    }
    CView.text(" encouragingly.  You respond by edging your [hips] upward, more effectively presenting your shaft to be pleasured.");

    CView.text("\n\nSighing happily, the immobile beauty ");
    if (largestCock.area < 100) CView.text("leans down and kisses the tip, opening her glossy green puckers to slurp it down, sucking just hard enough to make your " + describeCockHead(largestCock) + " tingle with erotic warmth.");
    else CView.text("kisses up and down your huge, towering shaft, slobbering her glossy green puckers all over your urethral bulge, making it even more of a wet mess.");
    CView.text("  You grab hold of the edges of her tits and squeeze, smushing them tight around yourself and sliding them up and down.  The plant-woman's twinkling golden eyes meet your own while she pleasures your " + describeCock(player, largestCock) + ", and her hands come to rest over top of your own, assisting you with the quick tit-fuck.");
    ...
    CView.text(", slapping wetly against you.  Soon, the slick texture of her soft melons has you throbbing eagerly and dribbling hot trails of pre-cum.  You're going to blow any second now, and the hot bloom of pleasure within you is spreading further and further.  Those gold and black eyes look up imploringly while the pale green cheeks hollow hungrily, and you release ecstatically, arching your back as you pound the heavy tits down your length.");

    CView.text("\n\nA geyser of spunk rockets out into ");
    if (largestCock.area >= 100) CView.text("the air, where it hangs for a moment before splattering down atop the plant-girl's leaf-colored hair and face, forming a gossamer shroud.");
    else CView.text("her mouth, which works noisily to swallow the hot load even as you inject it into her.  After a few swallows, she leans back and lets you shoot it up into the air, so that it can fall down over her face and hair, into a gossamer shroud.");
    // highish cum:
    if (player.cumQ() >= 500) CView.text("  You keep blasting heavy ropes of seed until her body and trunk are painted with goo, and then you cum some more, smattering spunk until she looks more like a gooey waterfall than a fey creature.");
    if (player.cumQ() >= 1500) CView.text("  Before you know it, there's a lake around the two of you and a small river running downhill away from ");
    CView.text("  A mischievous grin breaks out under the goopy facial, followed by an unnaturally long tongue that pulls gobs of the stuff into the dryad's hungry mouth.  Once she can see again, the girl gives her tits a squeeze and affectionately kisses your cock, practically worshipping it with sloppy-sweet licks.");
    ...
    if (player.body.tallness < 60) CView.text("tits");
    else CView.text("arms");
    CView.text(".  [EachCock], instead of slamming into the bark painfully, slots into something warm and wet, pulsating and pliant.  A clingy, tight hold that ripples gently around your member");
    if (player.body.cocks.length > 1) CView.text("s, massaging the bundle of cocks with uncommon ferver");
    else CView.text(", massing your tool with uncommon fervor");
    CView.text(".  Your grunt of irritation vanishes as soon as your tender sex is forced into the heavenly hole, and with the soft breasts pressing into ");
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 2) CView.text("your own");
    else CView.text("your [chest]");
    CView.text(", you give up and kiss the dryad full on the lips, biting the lower one roughly in irritation.");

    CView.text("\n\nHolli deprived you of choice, but if you have to fuck, it's going to be by your rules.  You slide your fingers through the seam between her hips and the tree, finding a squeezable, pert behind to grab hold of.  Then, you pull your hips back, and still biting her lip, you slam them home, powerfully delivering your " + describeCocksLight(player) + " into her slobbering, nectar-filled gash.  You let her lip go and menacingly growl at her, plowing her fields with animalistic vigor.");
...
function haveAMapleSyrupSnack(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You approach Holli with a hungry smile plastered across your [face], licking your chops as you eye the arboreal slut with unrestrained desire.  Of course, your gaze fixes on her heavy chest, with its distended, dripping nipples.  They leak sweet sap as you stare; the demon seems eager to feel your mouth on her slick, almond-colored buds.  Looking at you ");
    if (HolliFlags.HOLLI_SUBMISSIVE === 0) CView.text("knowingly, Holli asks, \"<i>Come for a little pick-me-up?  Well, go on; drink deeply of my nectar, </i>champion.\"  The last word is inundated with malicious, mocking tones.");
    else CView.text("eagerly, Holli asks, \"<i>Care for a little pick-me-up?  They're getting very engorged, and I'm sure it would be a refreshing snack for you.</i>\"");
...
function level4RideHollisTentacruels(player: Character): NextScreenChoices {
    ...
    if (User.settings.silly()) CView.text("  She proceeds to sing about Jack Sparrow's awesome adventures for a solid ten minutes, then goes on as if nothing had happened.");
    ...
    if (player.lactationQ() === 0) CView.text("droplets of milk dripping from your nipples.  When did you start lactating?");
    else CView.text("steady streams of milk erupting from your nipples.  When did you start lactating this hard?");
    ...
    if (!player.body.legs.isBiped()) CView.text("you'd be walking bowlegged for a week, if you had legs.");
    else CView.text("you'll be walking bowlegged for a week!");
...
// Bear Fruit(C)
function eatHolliFruit(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You reach up into Holli's branches and pluck ");
    if (HolliFlags.HOLLI_FRUIT === 1) CView.text("the fruit");
    else CView.text("one of the fruits");
    CView.text(".  ");
    if (HolliFlags.HOLLI_FRUIT_EXPLAINED === 0) {
        CView.text("Holli smiles over at you and asks, \"<i>Enjoying the fruits of our union?</i>\"");
        ...
        HolliFlags.HOLLI_FRUIT_EXPLAINED++;
    }
    CView.text("Biting into it, sweet juices seem to explode from the flesh, dribbling down your chin.  It tastes like a dessert and you chow down, happily munching away.  In no time flat, you're down to just a core.  You toss it and wipe your [face] clean, then burp.  Damn, that was good!");
    // TF CHANCES
    const tentCocks = player.body.cocks.filter(Cock.FilterType(CockType.TENTACLE));
    if (randInt(2) === 0 && player.body.cocks.length > tentCocks.length) {
        const nonTentCock = randomChoice(player.body.cocks.filter((cock) => cock.type !== CockType.TENTACLE));
        CView.text("\n\nYour " + numToOrdinalText(player.body.cocks.indexOf(nonTentCock) + 1) + " penis itches, and you idly scratch at it.  As you do, it begins to grow longer and longer, all the way to the ground before you realize something is wrong.  You pull open your [armor] and look down, discovering your " + describeCock(player, nonTentCock) + " has become a tentacle!  As you watch, it shortens back up; it's colored green except for a purplish head, and evidence seems to suggest you can make it stretch out at will.  <b>You now have a");
        if (tentCocks.length > 0) CView.text("nother");
        CView.text(" tentacle-cock!</b>");
        nonTentCock.type = CockType.TENTACLE;
        nonTentCock.knotMultiplier = 1.3;
        player.stats.sens += 3;
        player.stats.lust += 10;

    }
    HolliFlags.HOLLI_FRUIT--;
    player.stats.cor += 1;
...
function askHolliToWatch(player: Character): NextScreenChoices {
    CView.clear();
    if (HolliFlags.HOLLI_DEFENSE_ON === 0) {
        CView.text("You ask Holli if she would mind using her tentacles to help guard the camp at night.  She rolls her eyes and taunts, \"<i>I don't know; maybe if you get down on your knees and beg me.  ...And I mean </i>really<i> beg, with plenty of whimpering.  Lick my roots too.  After all, I should get something out of this.</i>\"");
        ...
        if (HolliFlags.THREATENED_HOLLI > 0) CView.text("<b>  You could always just slap the cunt.</b>");
        // choosing not to beg unlocks Threaten?
        // [Beg][Threaten][Back]
        // return { choices: [ ["Beg", begHolli4Watches], ["Threaten", 0], ["Assert Self", domUpSomeHolli], ["", 0], ["Back", treeMenu],  ] };

        const choices: ScreenChoice[] = [];
        choices[0] = ["Beg", begHolli4Watches];
        if (HolliFlags.THREATENED_HOLLI === 0) choices[1] = ["Threaten", threatenHolli];
        else choices[1] = ["Slap Her", slapDatHo];
        choices[2] = ["Assert Self", domUpSomeHolli];
        choices[4] = ["Back", partial(treeMenu, player, true)];

        return { choices };
    }
    // Stop Guarding (edited)(C)
    else {
        CView.text("You tell Holli you'd like her to stop watching at night.  She laughs, \"<i>You just want a chance to come back and beg some more, don't you?  I thought you were supposed to be tough shit, not a simpering little submissive.</i>\"");
        HolliFlags.HOLLI_DEFENSE_ON = 0;
        return { next: partial(treeMenu, player, true) };
    }
}

// Actually beg: (edited)(C)
function begHolli4Watches(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You swallow your pride and drop down prostrate on the ground.  Sighing, you grudgingly say, \"<i>Please, Holli, guard my camp at night.  I don't want to be raped by monsters.</i>\"");
    ...
    CView.text("\n\n\"<i>Hah.  I guess I'll watch your </i>\"");
    HolliFlags.HOLLI_DEFENSE_ON = 1;
    return { next: partial(treeMenu, player, true) };
}
...
function domUpSomeHolli(player: Character): NextScreenChoices {
    CView.clear();
    HolliFlags.HOLLI_FUCKED_TODAY = 1;
    CView.text("You swagger up to the tree and put your hand on the bark beside Holli.  Glancing over, she quips, \"<i>Is that supposed to be intimidating?</i>\"  The dryad smirks ingratiatingly.  \"<i>I'm the offshoot of a demon goddess.  I don't think you thought this through.</i>\"");
    ...
    let domPowah: number = player.stats.level;
    domPowah += player.body.tallness / 12;
    if (player.body.horns.count > 0) domPowah += 3;
    if (player.stats.cor > 66) domPowah += 2;
    // {fail}
    if (domPowah < 20) {
    ...
    // [Next] - Holli's Menu
    HolliFlags.HOLLI_SUBMISSIVE = 1;
    return treeMenu(player, false);
}

// Guard Camp
function askBrokenHolliToGuard(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Holli bows her head when you mention her guarding   She asks, \"<i>Shall I keep watch for foes to drain or let them into camp for you to have the pleasure of dealing with, [name]?  <b>I've been ");
    if (HolliFlags.HOLLI_DEFENSE_ON === 1) CView.text("keeping watch");
    else CView.text("sleeping, instead");
    CView.text(".</i>\"");

    // [Guard] [Don't Guard] [Back]

    const choices: ScreenChoice[] = [];
    if (HolliFlags.HOLLI_DEFENSE_ON === 1) choices[1] = ["Don't Guard", toggleBrokenHolliGuard];
    else choices[0] = ["Guard", toggleBrokenHolliGuard];
    choices[4] = ["Back", partial(treeMenu, player, true)];
...
// Guard On
function toggleBrokenHolliGuard(player: Character): NextScreenChoices {
    CView.clear();
    if (HolliFlags.HOLLI_DEFENSE_ON === 0) {
        CView.text("\"<i>All right, I'll get to have some fun then,</i>\" Holli quips while rubbing her palms together in anticipation.");
        CView.text("\n\nYou suppose you can sleep better with your pet demoness taking on the lesser whelps of this land.");
        HolliFlags.HOLLI_DEFENSE_ON = 1;
    }
    // Guard Off
    else {
        CView.text("\"<i>All right, they're yours to smite or play with,</i>\" Holli says with a wink.  You give her a glare, to which she responds with a shrug.  \"<i>What?  I know how it is!  You mortals can't fight worth a damn when you're sleeping.</i>\"");
...
// Dom Her With a Dick
function holliGetsDickDommed(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Tossing your [armor] aside, you expose your " + describeCocksLight(player) + ".  Holli's eyes fixate on [oneCock], her attention rapturously contained by a single phallus.  You grip your maleness in hand and bounce it up and down into your other palm.  The solid slaps of genital impacts echo off the surrounding rocks, each sending a pleasant tingle of pleasure through the root of your manhood.  Before long, you're completely erect and throbbingly hard, bouncing unbidden with each and every thrum of your heart.");
    ...
    if (player.body.cocks.sort(Cock.Largest)[0].area > 80) CView.text(", jaw stretching obscenely");
    CView.text(".");
    ...
    if (player.body.cocks.sort(Cock.Largest)[0].area <= 80) {
        CView.text(", swallowed to the ");
        if (player.body.balls.count > 0) CView.text("[balls]");
        else CView.text("[sheath]");
        CView.text(".");
    }
    else CView.text(", swallowed as deep as Holli's distended neck can handle.");
    CView.text("  Gurgling, the demonic dryad flutters her lengthy eyelashes and looks up at your eyes, twinkling with happiness at her servitude.  You wiggle your crotch around, and it twists your [cock biggest] around her mouth and throat.  The tainted tree's tongue massages the underside of your length, embracing you with subtle moisture and frictionless pressure.");
    CView.text("\n\nBubbling sexual pleasure begins to build up inside you, making your ");
    if (player.body.balls.count > 0) CView.text("[sack] feel taut and ready to burst");
    else CView.text("body feel strung tight like an over-tuned guitar");
    CView.text(".  Holli pulls back, inciting a snarl of displeasure to vibrate from you, but as soon as she's withdrawn, her hands clasp onto the slick shaft and begin to pump it with vigorous strokes.  Her mouth settles back into place around your [cockHead biggest], circling it with her tongue while she bobs up and down on it, snaring your most sensitive place with overwhelming sensations.");
    ...
    if (player.body.cocks.sort(Cock.Largest)[0].area > 80) CView.text("  It bends slightly as the immense girth is forced inside her, aiming down, deep inside the trunk.");
    CView.text("  Holli flops into your [chest], twitching weakly and breathing heavily.  \"<i>Fffuck, so good!</i>\" she pants, nuzzling her cheek into your collarbone while her hands play across your shoulders.  You ruthlessly mount her flowery mound with fast motions.  Juices squirt out around your member at the apex of every stroke, raining down over the surrounding ground.  You grab Holli's deep green hair and pull her back.  Her face is wrecked by bliss, her mouth and eyes moving in uncoordinated twitches.");
    ...
    CView.text("\n\nThe abruptness of your orgasm startles you.  One moment there's the sanguine ecstasy of the surrounding fuck-flesh, and the next, there's an explosive volcano of lust boiling out of your " + describeCocksLight(player) + ", geysering fountains of fluid.");
    if (player.body.cocks.length > 1) {
        CView.text("  Excess jism spurts onto Holli's jiggling udders from below, spotting the viridian mounds with white.");
        if (player.cumQ() >= 500) CView.text("  The constant sprays turn them white in little time, dripping from the peaks of her nipples along with her sticky sap.");
    }
    if (player.cumQ() > 1000) CView.text("  A trickle of sperm washes out of the demoness's entrance, somehow escaping the heavy suction.");
    if (player.cumQ() > 2000) CView.text("  A moment later, that trickle turns into a river.");
    if (player.cumQ() > 3000) CView.text("  You swear that her trunk must be absolutely stuffed with spunk.");
    if (player.cumQ() > 5000) CView.text("  A spooge-spout sprays out as the pressure goes through the roof, confirming your suspicions in the messiest way possible.");
    CView.text("  Holli's tentacles cum with her, spraying her seed through her canopy in aimless ropes.  Most of it sticks to her branches and leaves, but small amounts of it trickle down to spatter the ground.  One of them lands on the back of your hand.  You frown and push it up against the tree-woman's mouth.  She licks you clean without question, still dazed from her orgasm.");
...
function fullOnTentacleTasticGangBangForHolli(player: Character): NextScreenChoices {
    CView.clear();
    HolliFlags.HOLLI_FUCKED_TODAY = 1;
    CView.text("You remove your [armor] and expose your writhing mass of wriggling tendrils to the tree-tart.  She places a hand over her mouth and exhales, \"<i>Oh my, Mother Marae sure knows how to pick them, doesn't she?</i>\"");
    ...
    if (User.settings.silly()) CView.text(", inflatable-arm, flailing tube-cocks");
    CView.text(".  Below the orgy of arboreal affection, Holli's slick petals have spread wide.  You can actually see her interior twitching voraciously.  It opens and closes with pulsing hunger, vibrantly engorged.  As you watch, the fourth of your members lances forward of its own volition, spearing straight for that welcoming hole with no guidance from you.  It hits like a missile.  Two feet of that cock is gone in a moment, swallowed deep in the cunt-flower's bottomless depths.  A gush of girl-cum spurts out around the intruding member, and you can feel the muffled moan vibrate through the mouth-bound cock as she gives in to the four-way fucking.");
    ...
    if (player.body.balls.count > 0) CView.text("  Your [balls] draw up tight to your groin and squeeze, rhythmically discharging their heavy loads.");
...
function vaginalDomHollisTentacruels(player: Character): NextScreenChoices {
    ...
    if (!player.body.chest.find(BreastRow.FuckableNipples) && player.lactationQ() < 50) CView.text(" and pay special attention to your [nipples], tugging them in time with every sluggish thrust in your slit.");
    else if (player.lactationQ() >= 50) {
        CView.text(" and sigh at the ");
        if (player.lactationQ() < 150) CView.text("squirts");
        else if (player.lactationQ() < 800) CView.text("sprays");
        else if (player.lactationQ() < 2000) CView.text("outpouring");
        else CView.text("thick fountains");
        CView.text(" of milk that escape with every tug, timed to match the sluggish thrusts at your slit.");
    }
    else CView.text(" and sink your fingers into your [nipples], fucking them to the sluggish tempo of the thrusting tentacle.");
    CView.text("  A loud groan escapes your mouth, and you rock your [hips] against the undulating tendril.  Your body's eagerness for more pleasure is unparalleled, but you control yourself as efficiently as you control the dryad.");
    ...
    if (player.body.clit.length >= 3.5) CView.text(" for a moment before wrapping around and jerking on it");
    CView.text(".");
...
function threatenHolli(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You snarl and threaten to burn the barked bitch to the ground if she won't make herself useful after you ask politely.");
    ...
    if (player.stats.level < 30) CView.text("Mighty as you are, Marae is likely still beyond your power.  Perhaps you should just ignore this demon for the time being.");
    else CView.text("Marae is likely beyond your power yet, but... you feel like she might not pressure you too hard if you can make a suitably credible threat.");
    ...
    if ((player.gender > 0 && player.effects.has(StatusEffectType.JojoNightWatch) && player.effects.has(StatusEffectType.PureCampJojo)) || HolliFlags.ANEMONE_WATCH > 0) {
...
function slapDatHo(player: Character): NextScreenChoices {
    ...
    const choices: ScreenChoice[] = [];
    choices[0] = ["CutHerDown", cutHerDown];
    if (player.inventory.keyItems.has("Jojo's Talisman")) choices[1] = ["Call Jojo", callDatJojo];
    choices[4] = ["Ignore", partial(treeMenu, player, true)];

    return { choices };
}

// [Cut Her Down]
function cutHerDown(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Are you serious?  You really want to be broken that badly, my little toy?</i>\"  The demon folds her arms over her hefty chest, her earlier, suggestive posture completely absent now.  \"<i>You know my bark is as strong as any armor, right?  That Marae herself - the land itself - bolsters me?  Take that back right now, and I'll let you return to my good graces... if you put your face in the dirt and plead for the duty to pleasure me whenever I want.</i>\"");

    // (if PC weapon = Large Axe)
    if (player.inventory.equipment.weapon.displayName === "large axe") {
...
function callDatJojo(player: Character): NextScreenChoices {
    ...
    // go to fight
    HolliFlags.FOUGHT_HOLLI = 1;
    const monster = new Holli();
    monster.effects.add(StatusEffectType.JojoIsAssisting, 0, 0, 0, 0);
    return CombatManager.beginBattle(player, monster);
}

function defeatHolli(player: Character, monster: Character): NextScreenChoices {
    CView.clear();
    HolliFlags.FUCK_FLOWER_KILLED = 1;
    // Win without Jojo -Z
    if (!monster.effects.has(StatusEffectType.JojoIsAssisting)) {
        CView.text("Weary, the succubus godling folds into her tree, unwilling to allow you the sight of her defeat.  With her energy suppressed, you're free to ");
        if (player.inventory.equipment.weapon.displayName !== "large axe") CView.text("fetch an axe and ");
        CView.text("hack at her trunk, laboriously peeling bark and making wedge-shaped cuts in it as you try to fell the abomination.  It takes nearly eight hours, but eventually the gnarled tree topples.  No sooner does it fall than it shrivels and turns to ash.");
        let temp = 7;
        while (temp > 0) {
            temp--;
            Time.hour++;
    ...
    // Win with Jojo -Z
        ...
        CView.text("The fire-blackened mouse cheers at you as the succubus groans and retreats completely into her bark.  \"<i>Well done, [name]!  Help me carry tinder and we can burn this abomination to the ground!</i>\"");
        ...
        if (!vapulaSlave()) {
            ...
            return {
                choices: [
                    ["Yes", recruitJojoToCamp],
                    ["No", dontRecruitJojoToCamp],
                ]
            };
        }
    }
    CView.text("\n\n(Key Item Gained: Holli's Ashes)");
    player.inventory.keyItems.add("Holli's Ashes", 0, 0, 0, 0);
    return { next: returnToCampUseOneHour };
}

// [yes gimme monk pls]
function recruitJojoToCamp(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Jojo nods eagerly at you when you invite him to share your   \"<i>Excellent idea.  If you'll excuse me, I wish to attempt to purify the spot where the creature grew, and then I will fetch my things.</i>\"");
    CView.text("\n\nBowing neatly, he takes his leave.");
    player.effects.add(StatusEffectType.PureCampJojo, 0, 0, 0, 0);
    CView.text("\n\n(Key Item Gained: Holli's Ashes)");
    player.inventory.keyItems.add("Holli's Ashes", 0, 0, 0, 0);
    return { next: returnToCampUseOneHour };
}
...
// [no want]
function dontRecruitJojoToCamp(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You hold your tongue, allowing Jojo to make his way back to the forest.");
    CView.text("\n\n(Key Item Gained: Holli's Ashes)");
    player.inventory.keyItems.add("Holli's Ashes", 0, 0, 0, 0);
    return { next: returnToCampUseOneHour };
}
...
function enjoyYourBadEndBIYAAAATCH(player: Character, monster: Character): NextScreenChoices {
    ...
    if (player.gender === 1 || (randInt(2) === 0 && player.body.cocks.length > 0)) {
        ...
        if (player.body.cocks.sort(Cock.Largest)[0].thickness > 6) CView.text(", stretching her jaw wide open in a way that makes you wince,");
        CView.text(" engulfs you.  Holli's tongue plays over your slit and she looks up at you for just a moment to be sure you're watching.");
        ...
        if (player.body.cocks.sort(Cock.Largest)[0].length > 24) CView.text("her neck ripples and extends grotesquely, and her head begins to migrate down all " + numToCardinalText(Math.round(player.body.cocks.sort(Cock.Largest)[0].length)) + " of your prick with alarming speed, pushing the stem inside along with it.  ");
        else CView.text("she sinks her head down to the base of your prick in one quick thrust, bringing the stem with it.  ");
        CView.text("The sensation of having your entire [cock biggest] penetrated and sucked into her hot, wet, clinging throat pushes you back toward the edge and over, and it literally becomes a race for Holli to reach bottom before your nerves fire.  Her wet lips impact your crotch with a lewd splatter, but her tongue keeps going deep into your body, stopping only once it reaches your prostate.  A small bulge fires down the thin stem, forcing through your slit and into the shaft to lodge in your gland.  Holli's mouth and tongue withdraw once it is in place, and she grabs your cock in her hand, masturbating you vigorously.");

        CView.text("\n\nIt's too much; your climax comes in a flood");
        if (player.body.balls.count > 0) CView.text(", and your [balls] draw close to your body to unload");
        CView.text(".  Your eyes roll as you flex and flex, but nothing comes out.  As you conclude your dry orgasm, you begin to feel a pressure on your prostate.");
        ...
        if (player.body.balls.count > 0) CView.text("migrating the other direction into your [balls], entangling and infiltrating them, filling them with the same warmth, then pressing out against your sack.  ");
        else CView.text("exploring the depths of your body and curling back against the surface of your " + describeSkin(player) + ".  ");
        CView.text("Your stomach turns as the sprouts push their way through, crawling from your crotch down the outside of your [legs] to root in the dry earth.  The shoots thicken into fibrous roots, forming a trunk that supports you and pushes you closer to Holli and lower to the ground, until your writhing, tentacle-tipped prick is lined up with her pussy flower and within her easy reach.");
        ...
        return { next: holliPlaysWithPenisesBadEnd };
    }
    // branch of vagina-having
        ...
        if (player.body.legs.isTaur()) CView.text(" and turning you around");
        CView.text(", the demon buries her face in your [vagina], penetrating it with her wet, flexible tongue.");
        ...
        if (player.body.cocks.length > 0) CView.text("  [OneCock] drools precum down its length and onto the flower, triggering another spasm and another gush from your prick.  Holli laughs as you continue to self-stimulate with your uncontrollable fluids.");
        ...
        if (player.body.chest.sort(BreastRow.Largest)[0].rating > 1) CView.text("wrapping around and under your breasts to keep them permanently jutting out in a lurid display");
        else CView.text("tracing a lattice of slender, criss-crossing ivy over your chest that hangs just far enough over your [nipples] that a stiff breeze will drag it into them");
        CView.text(", then continuing over your back and up your neck.  The tendrils thicken and multiply, occluding your forearms and lower [legs] in a casing of young bark and leaf buds, and Holli's own release you as yours take over the duty of holding you in place to receive whatever pleasure she forces upon you.  You're left in a salacious pose when the growth subsides, suspended several feet off the ground with your arms held over your head, your back arched in a memorial of your incredible orgasm, ");
        if (player.body.chest.sort(BreastRow.Largest)[0].rating > 1) CView.text("your breasts cupped and pressed together by vines, ");
        if (!player.body.legs.isNaga() && !player.body.legs.isGoo()) CView.text("your thighs spread wide open, ");
        CView.text("and your nectar-drooling flower-sex exposed with your [clit]");
        if (player.body.cocks.length > 0) CView.text(" and [eachCock]");
        CView.text(" bobbing in a state of permanent semi-arousal.");
        ...
        return { next: girlsGetANiceManToBadEnd };
    }
    else {
        ...
        if (player.body.butt.wetness > 0) CView.text("Betraying your depravity, the slick crack opens wide and your greedy, wet asshole practically welcomes the demon's entire hand, sucking it up and making you shiver with repressed delight.  \"<i>Oh, mother!  It seems as though this is not a new lesson to you, is it?  My little whore, will you beg and moan as I fill you?</i>\"");
        else if (player.analCapacity() < 30) CView.text("With some difficulty, she forces it into your pucker.  \"<i>I think we'll just use this as our inroads, instead.  It may be slow going, but the journey is half the fun.</i>\"");
        else CView.text("Despite your clenching, your loose hole proves no impediment to the demon's exploration.  \"<i>Ah, how roomy you are!  I think I'll move right in.</i>\"");
        ...
        // [(iff buttslut)
        if (player.body.butt.wetness > 0) CView.text("Try as you might, you can't see far enough down your back to determine what the demon has done.  The question answers itself, though, when she presses a shaft almost as thick as her leg into your [butt], nosing it into your wet, receptive asshole.  Despite yourself, you shiver in anticipation at being filled so completely.  ");
        else if (player.analCapacity() < 30) CView.text("Your jaw hits your collarbone as Holli angles the florid head of a long, narrow penis into your field of view.  Judging by how far it comes up her chest, it's almost a foot and half long, and it's drooling sticky sap all over your back.  With a twisted smile, the demon pushes it along your spine until it reaches your [butt], then shoves it toward the hole.  ");
        else CView.text("The demon presses up to your back, brushing her nipples against your skin and shoving her new growth between your thighs.  Looking down, you can see a fat, green shaft sticking from your crotch, glistening and dripping with sticky slime.  With another moan, Holli pulls it back from between your legs, then pushes apart your asscheeks and plunges it into your [asshole].  ");
        CView.text("The sap oozing from her member coats your passage, ");
        if (player.body.butt.wetness > 0) CView.text("mingling with your own lubrication and ]easing her thrusts.  Holli's tempo picks up quickly, and soon you can feel her twitch inside you as she nears her climax.");
        ...
        // [(buttslut or loose)
        if (player.body.butt.wetness > 0 || player.analCapacity() > 30) CView.text("spilling a wave of her deposited fluids down your [legs].  ");
        else CView.text("and your once-tight pucker reflexively squeezes shut as best it can, trapping all but a few trickles of Holli's deposit inside you.  ");
        CView.text("The freshly-grown shaft between Holli's legs begins to shrivel, withering and browning like a cut flower.  With an expression of mild disdain, she pulls it from her body and casts it aside, where it crumbles to pieces.");

        CView.text("\n\nThe fluids in your gut begin to mingle, sending a disquieting pinprick sensation through the skin");
        if (player.body.skin.type === SkinType.FUR || player.body.skin.type === SkinType.SCALES) CView.text(" under your " + skinFurScales(player));
        CView.text(".  Panic intensifies as the sensation quickly focuses on your crotch, and when the tendrils holding you push you away from the demon's tree, your curiosity gets the better of you.  You look over your shoulder again, trying to see what's happening.  As you strain to watch, you feel hundreds of tiny, sharp pinches and small vines begin to creep from your body, just below your ass.  The vines merge and thread into thick trunks as they get closer to the ground, pushing deep into the dirt and mooring you to the spot.  Arousal builds in you as they join the web of roots linking the demon Marae and Holli, as does a swelling at your crotch.  Another cluster of thin vines push free from your groin, growing upward in a lattice and lashing your arms into place, ");
        if (player.body.chest.sort(BreastRow.Largest)[0].rating < 1) CView.text("folded atop your chest");
        else CView.text("crooked at your sides with your hands resting atop your [chest]");
        CView.text(".  They continue past your face, reaching over your head to form a canopy similar to Holli's own.  Struggle as you may, when they're done growing you're left unable to budge and locked in a rather artful pose, head turned over your shoulder with your [ass] thrust out");
        if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 1) CView.text(", your breasts exposed from the sides,");
        CView.text(" and your [legs] concealed below your buttcheeks by a thick tree trunk.");
...
function holliPlaysWithPenisesBadEnd(player: Character): NextScreenChoices {
    ...
    if (player.body.chest.sort(BreastRow.Largest)[0].rating < 1) CView.text("masculine shape, it bears");
    else CView.text("feminine shape, it nonetheless bears");
    CView.text(describeCocks(player) + ", standing erect and hard as iron; the brown-haired girl blushes and looks away.");
    ...
    if (player.body.cocks.length > 1) CView.text("spraying the girl's stomach and tits with waste seed from " + mf(player, "his", "her") + " unused manhood and ");
    CView.text(" filling the Champion's pussy with so much semen it cascades from her stretched hole as Holli continues to pump.");
    ...
    CView.text(" drooling aftershocks onto her breasts.");
...
function girlsGetANiceManToBadEnd(player: Character): NextScreenChoices {
    ...
    // [(if tits)
    if (player.body.chest.sort(BreastRow.Largest)[0].rating > 1) CView.text("[chest] cupped by vines and held out toward the young man, ");
    else CView.text("chest crisscrossed with a mesh of fine vines, ");
    CView.text("its legs elevated and spread apart to display a ");
    if (player.body.balls.count > 0 && player.body.balls.size > 10 && player.gender === 3) CView.text(describeCocksLight(player) + " with [balls] swinging below.");
    // (if H and ballsize < 10)
    else if (player.body.balls.count > 0 && player.body.balls.size <= 10 && player.gender === 3) CView.text(describeCocksLight(player) + " and [balls] that hang slightly over a [vagina].");
    // (if H and noballs)
    else if (player.gender === 3) CView.text(describeCocksLight(player) + " and a [vagina].");
    else CView.text("a wet, drooling cunt that practically begs to be filled.");
    CView.text("  Though its eyes are covered, the figure's mouth works constantly, biting and licking its lips as if beset by otherworldly amounts of lust.  Holli beckons the youth toward this tree.");
    ...
    if (player.body.balls.size > 10 && player.body.balls.count > 0) CView.text("lifting the hefty testicles to reveal a wet, eager pussy.");
    else CView.text("spreading the labia and exposing the eager, clenching hole inside.");
    ...
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 1) {
        CView.text("reaches up and grabs two handfuls of breast, passing his thumb over a nipple and watching the ");
        if (player.gender === 2) CView.text("woman");
        else CView.text("hermaphrodite");
        CView.text("'s mouth open and close in bliss.  ");
    }
    else CView.text("leans down and kisses the [woman/hermaphrodite]'s smooth chest, causing her to arch her back in pleasure.  ");
    if (player.body.cocks.length > 0) CView.text("With a nervous glance downward, he slides one hand to her [cock smallest].  \"<i>It's not gay if it's a chick's,</i>\" he mumbles to himself, and then begins to stroke haltingly, playing his finger along the urethra and drawing a squirt of precum from his shivering partner.  ");
    else {
        CView.text("His hand slides to the [clit] above her wet fuck-hole; collecting some of the frothing lube from their wet sex, ");
        if (player.body.clit.length < 6) CView.text("he smears it over the little button and then begins to trace up and down the underside with a finger.  ");
        else CView.text("he coats the swollen organ with it and begins to jerk it off valiantly, like a cock.  ");
        CView.text("\"<i>Does that feel good?</i>\" he asks, upping the pace of his thrusts.  ");
    }
    CView.text("As if in reply, the ");
    if (player.gender === 2) CView.text("woman");
    else CView.text("hermaphrodite");
    CView.text(" in the tree arches her back and her [vagina] clamps down on his tool");
    if (player.body.cocks.length > 0) {
        CView.text("; ");
        if (player.body.balls.count > 0) CView.text("her balls draw closer to her body and ");
        CView.text("her trembling dick");
        if (player.body.cocks.length === 1) CView.text(" throbs");
        else CView.text("s throb");
        CView.text(" as she nears climax");
    }
    CView.text(".");
    ...
    if (player.body.cocks.length > 0) {
        ...
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(" toward her face; ");
        if (player.body.cocks.sort(Cock.Largest)[0].area < 20) {
            CView.text("semen spurts and dribbles down her [chest], pooling in the combined laps and mingling with the sticky lube ringing her vagina.  ");
            if (player.cumQ() >= 500) CView.text("So much so that by the time she finishes her climax, both her partner's legs and her trunk are covered in a sheen of milky white seed.  ");
        }
        else if (player.body.cocks.sort(Cock.Largest)[0].area < 80) {
            CView.text("her mouth is now full of cockhead, and she gulps as eagerly as she spurts, drooling lines of white from the corners of her lips.  ");
            // [(big skeet)]
            if (player.cumQ() >= 500) CView.text("The waves of seed don't stop coming, pouring into her and swelling her belly out until its round bulge pushes the cock away from her mouth and the remaining squirts fall on her [chest].  ");
        }
        // else cock biggest > 80)
        else {
            CView.text("her body is easily dwarfed by her male sex's massive size, and globs of sperm shoot into her canopy to splatter among her leaves and branches.  ");
            if (player.cumQ() >= 1000) CView.text("The overproductive hermaphrodite's body pushes out so much seed that the entire canopy soon turns white and drips constantly as though it were in a tropical rainforest.  ");
        }
    }
    // (vagina and no cock)
    else {
        CView.text("\n\nA ");
        if (player.body.vaginas.get(0).wetness < 3) CView.text("gush");
        else if (player.body.vaginas.get(0).wetness < 4) CView.text("rush");
        else CView.text("torrent");
        CView.text(" of female orgasm pours from her pussy, and her mouth opens wide in pleasure.  The tree branches surrounding the young man seem to close in, pushing him toward her.  Getting the message, he leans in and kisses his lover; her tongue probes his mouth with abandon as her body shakes out its fit and her [vagina] wrings his prick.  ");
    }
    CView.text("Pushed past his limit by her lewd display, the young man groans, shudders, and begins to unload his pent-up cargo.  Falling on top of his partner's slick ");
    if (player.body.cocks.length === 1) CView.text("cock");
    else if (player.body.cocks.length > 1) CView.text("cocks");
    else CView.text("chest");
    CView.text(", he goes limp and slack-jawed as his dick squirts, filling her vagina.");
    ...
    CView.text("'s stomach.  \"<i>In fact, she seems to have enjoyed it so much that she would have me bestow a blessing on you.</i>\"");
...
function holliAndGenderlessSittingInATree(player: Character): NextScreenChoices {
    ...
    if (player.body.skin.type !== SkinType.FUR && player.body.skin.type !== SkinType.SCALES) CView.text(player.body.skin.tone + "-colored");
    else CView.text("covered in " + skinFurScales(player));
    CView.text(" and wrapping its body around the tree in a way that makes determining its sex impossible.  No... it's not wrapping around the tree, but embedded in it up to the hips!");
    ...
    if (player.body.skin.type === SkinType.SCALES) CView.text("a quiver of arousal shakes its scales");
    else if (player.body.skin.type === SkinType.FUR) CView.text("its fur begins to prick up with arousal");
    else CView.text("a blush of lust colors its " + skinFurScales(player));
    CView.text("...");
...
export function amilyComesBack(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Amily arrives with her belongings over her shoulder and a smile on her face.  \"<i>I knew you'd do the right thing, [name].  I'll get my nest set back up.</i>\"\n\n(<b>Amily has moved back in!  She can be found in the lovers tab.</b>)");
    // HolliFlags.AMILY_TREE_FLIPOUT = 1;
    HolliFlags.AMILY_FOLLOWER = 1;
    // Enable village encounters
    HolliFlags.AMILY_VILLAGE_ENCOUNTERS_DISABLED = 1;
    return { next: campMenu };
}
