...
import { campSlavesMenu, returnToCampUseOneHour, companionsCount } from "../Camp";
...
import { campCorruptJojo, tentacleJojo } from "./JojoScene";
...
export const VapulaFlags = {
    ...
    VAPULA_HAREM_FUCK: 0,
    VAPULA_DAYS_SINCE_FED: 0,
    VAPULA_FOLLOWER: 0,
    DAYS_SINCE_LAST_DEMON_DEALINGS: 0,
    IZMA_NO_COCK: 0,
    JOJO_MOVE_IN_DISABLED: 0,
    AMILY_FOLLOWER: 0,
    AMILY_VILLAGE_ENCOUNTERS_DISABLED: 0,
    AMILY_CORRUPT_FLIPOUT: 0,
    UNKNOWN_FLAG_NUMBER_00173: 0,
    FOLLOWER_AT_FARM_VAPULA_GIBS_MILK: 0,
    VAPULA_EARNED_A_SPANK: 0,
    FARM_CORRUPTION_STARTED: 0,
    FOLLOWER_PRODUCTION_VAPULA: 0,
    FARM_UPGRADES_REFINERY: 0,
    VAPULA_TATTOO_COLLARBONE: 0,
    VAPULA_TATTOO_SHOULDERS: 0,
    VAPULA_TATTOO_LOWERBACK: 0,
    VAPULA_TATTOO_BUTT: 0,
    VAPULA_THREESOMES: 0,
    VAPULA_TEASE_COUNT: 0,
    AMILY_NOT_FURRY: 0,
};
    ...
    public timeChange(player: Character): boolean {
        if (Time.hour > 23) {
            if (vapulaSlave() && VapulaFlags.FOLLOWER_AT_FARM_VAPULA === 0) {
                ...
                else VapulaFlags.VAPULA_DAYS_SINCE_FED = 0;
            }
            if (VapulaFlags.VAPULA_FOLLOWER === .5 || VapulaFlags.VAPULA_FOLLOWER === 1.5) VapulaFlags.VAPULA_FOLLOWER++;
            VapulaFlags.DAYS_SINCE_LAST_DEMON_DEALINGS++;
        }
        if (vapulaSlave() && !player.inventory.keyItems.has("Demonic Strap-On") && player.gender === 2 && VapulaFlags.FOLLOWER_AT_FARM_VAPULA === 0) {
            vapulaGivesPCAPresent(player);
            return true;
        }
        return false;
    }
...
export function vapulaSlave(): boolean {
    return VapulaFlags.VAPULA_FOLLOWER === 1;

}

// Two nights after "vagina enslave", if PC still meets initial requirements (else defer until she does)(Z)
export function femaleVapulaRecruitmentPartII(player: Character): NextScreenChoices {
    ...
    // [Vapula added]
    VapulaFlags.VAPULA_FOLLOWER = 1;
    VapulaFlags.VAPULA_HAREM_FUCK = 1;
    // [Demonic dildo added]
    player.inventory.keyItems.add("Demonic Strap-On", 0, 0, 0, 0);
    CView.text("\n\n(<b>Gained Item: Demonic Strap-On</b>)");
    return { next: campMenu };
}

export function vapulaGivesPCAPresent(player: Character) {
...
// Slave Vapula
// Camp lines
export function vapulaSlaveFlavorText(player: Character) {
    let choice: number = randInt(11);
    if (choice === 0) CView.text("Vapula is currently resting on a very rough bed of leaves she's gathered herself.  She sleeps quietly, her hand absent-mindedly laid on her vagina and the other one resting on her breasts.  A thin trickle of her juice is leaking out of her crotch.");
    if (choice === 1) CView.text("Your succubus slut, Vaplua, is furiously stroking herself on her bed of leaves, her purple body shining in sweat.  She moans raggedly, often looking up to you with lust-consumed eyes.");
    if (choice === 2) {
        if (VapulaFlags.VAPULA_HAREM_FUCK === 0) choice = 3;
        else CView.text("Vapula is currently absent, probably getting screwed by some of the imps surrounding your camp in order to get her semen rations.  You're sure she would come back instantly if you ever needed to give her YOUR cum instead.");
    }
    if (choice === 3) CView.text("Vapula is busy drinking a vial of milk that looks very much like the kind you'd usually find on imps.  Her purple cheeks seem to blush darker when she sees you; she awkwardly gulps the rest of the bottle, hiccups, and giggles softly, eyeing you with a coy smile and a lust-filled gaze.  The potion seems to have made her even more aroused than usual.");
    if (choice === 4) {
        CView.text("Vapula is currently toying with ");
        if (player.inventory.keyItems.has("Deluxe Dildo")) CView.text("your deluxe dildo");
        else CView.text("a dildo");
        CView.text(", pumping as fast as she can.  In her feverish display of lust she doesn't even notice you.");
    }
    if (choice === 5) {
        if (VapulaFlags.VAPULA_HAREM_FUCK === 0 || !(bimboSophie())) choice = 6;
        // if harem fucking is on)
        else CView.text("Vapula is fingering herself while eating out Sophie, your harpy slut.  The dumb avian matron keeps moaning like she's in heat while the succubus furiously assaults her lush fuck-hole; both of their pair of wings flap in rhythm as they feel their pleasure rising in waves.");
    }
    if (choice === 6) {
        if (VapulaFlags.VAPULA_HAREM_FUCK === 0 || VapulaFlags.IZMA_NO_COCK > 0 || !izmaFollower()) choice = 7;
        // if Izma has cock and harem fucking is on)
        else CView.text("Your succubus is currently very busy with Izma, your tigershark lover.  Izma is standing and hiding Vapula from your sight and you don't notice what Vapula is doing to her until you see her frantic bobbing movements, and hear Izma's hisses and growls of pleasure.  Obviously your succubus must get the cum she needs from somewhere.");
    }
    if (choice === 7) {
        // if C. Jojo and harem fucking is on)
        if (VapulaFlags.VAPULA_HAREM_FUCK === 0 || !campCorruptJojo()) choice = 8;
        else {
            CView.text("You routinely hear screams of pleasure outside the camp, as well as some frantic male squeaks coming from Jojo, your ");
            if (tentacleJojo()) CView.text("tentacled ");
            CView.text("mouse pet.  Vapula seems to be having a lot of fun with your fuck-toy!");
        }
    }
    if (choice === 8) {
        if (VapulaFlags.VAPULA_HAREM_FUCK === 0 || !(amilyFollower() && amilyCorrupt())) choice = 9;
        // if C. Amily and harem fucking on)
        else CView.text("Vapula is busy caressing your corrupted mousette, Amily while softly kissing her breasts.  Her fingers are buried in your fuck-toy's pussy, and Amily is moaning whorishly and doesn't even bother to stop when she looks at you, her helpless eyes betraying her lack of control over her own lust.");
    }
    if (choice === 9) {
        if (!player.effects.has(StatusEffectType.CampRathazul)) choice = 10;
        else CView.text("Vapula is having a very active conversation with Rathazul, the alchemist.  They seem to be discussing the chemical properties of demonic mixtures and the various taints that could occur from their ingestion.  The succubus is holding a bottle of a purplish white fluid that appears to be her own milk, probably waiting for an analysis.");
    }
    if (choice === 10) CView.text("Vapula is resting on her stomach, a buttplug shoved into her ass.  She doesn't seem to notice you watching her butt jiggle under the effect of her unconscious anal contractions.");
}

// Vapula-Followers interaction: Puru Puru Mouse (Z)
// switch follower names depending on conditions
export function mouseWaifuFreakout(player: Character, amily: boolean = false, jojo: boolean = false): NextScreenChoices {
    CView.clear();
    if (amily) {
        CView.text("Amily ");
        if (jojo) CView.text("and ");
    }
    if (jojo) CView.text("Jojo ");
    CView.text("walk");
    if (!(amily && jojo)) CView.text("s");
    CView.text(" up to you, worried, as Vapula struts around the ");
    CView.text("\n\n\"<i>" + player.desc.name + ", what is this? Am I dreaming or did you actually bring a demon to your camp? What in the world is wrong with you?</i>\"");
    ...
    // [if libido >=50]
    if (player.stats.lib >= 50) CView.text("\"<i>Well, as long as she swallows, I don't care,</i>\" you quip.");
    CView.text("\n\n\"<i>That's it!</i>\"  The ");
    if (jojo && amily) CView.text("mice are");
    else CView.text("mouse is");
    CView.text(" raving.  \"<i>You've clearly given in to her demonic lust.</i>\"");
    if (jojo) {
        CView.text("\n\n\"<i>I'm leaving, " + player.desc.name + ",</i>\" Jojo says.  \"<i>I only hope for your sake that you come to your senses soon... I will return to my place in the forest when you require assistance in freeing your soul of taint.</i>\"");
        VapulaFlags.JOJO_MOVE_IN_DISABLED = 1;
        player.effects.remove(StatusEffectType.JojoNightWatch);
        player.effects.remove(StatusEffectType.PureCampJojo);
    }
    if (amily) {
        CView.text("\n\nAmily shakes her head.  \"<i>Goodbye, [name].  You've changed.  What you did is pure folly.</i>\"");
        // Set - amily flipped her shit
        VapulaFlags.AMILY_FOLLOWER = 0;
        ...
        VapulaFlags.UNKNOWN_FLAG_NUMBER_00173 = 0;
    }
    // [(if PC corr > 70)
    if (player.stats.cor > 70) CView.text("\n\n\"<i>Fine. Go fuck off then, I don't need you.  I have a much better and hotter slut now. Don't hesitate to come back if you want some sweet pussy, though.</i>\"");
    CView.text("\n\n");
    if (jojo) CView.text("Jojo");
    if (amily && jojo) CView.text(" and ");
    if (amily) CView.text("Amily");
    if (amily && jojo) CView.text(" have ");
    else CView.text(" has ");
    CView.text("moved out.");
...
export function callSlaveVapula(player: Character, output: boolean): NextScreenChoices {
    ...
    // Option: Threesome
    // Option: Leave
    let mFeed: ClickFunction;
    let fFeed: ClickFunction;
    if (player.body.cocks.length > 0) {
        if (player.stats.lust >= 33) mFeed = feedVapulaACupOfJizz;
        else if (output) CView.text("\n\nYou aren't suitably aroused to feed Vapula right now.");
    }
    if (player.body.vaginas.length > 0 && player.inventory.keyItems.has("Demonic Strap-On")) {
        if (player.stats.lust >= 33) fFeed = chixFeedVapulaBlehblehIVantToZuckYourSpooo;
        else if (output && player.body.cocks.length <= 0) CView.text("\n\nYou aren't suitably aroused to feed Vapula right now.");
    }
    ...
    if (VapulaFlags.VAPULA_EARNED_A_SPANK > 0 && !player.body.legs.isTaur() && !player.body.legs.isDrider()) spank = spankVapulaLikeABoss;
    else threesome = vapulaThreesomeMenu;

    const choices: ScreenChoice[] = [
        ["Appearance", fapulaFapfapfapAppearance],
        ...
        ["", undefined],
        ["", undefined],
        ["Leave", campSlavesMenu]
    ];

    if (VapulaFlags.FOLLOWER_AT_FARM_VAPULA === 0 && VapulaFlags.FARM_CORRUPTION_STARTED === 1) choices[6] = ["Farm Work", sendToFarm];
    if (VapulaFlags.FOLLOWER_AT_FARM_VAPULA === 1) choices[6] = ["Go Camp", backToCamp];

    if (VapulaFlags.FOLLOWER_AT_FARM_VAPULA === 1 && VapulaFlags.FOLLOWER_PRODUCTION_VAPULA === 0) choices[7] = ["Harvest Milk", harvestMilk];
    if (VapulaFlags.FOLLOWER_AT_FARM_VAPULA === 1 && VapulaFlags.FOLLOWER_PRODUCTION_VAPULA === 1) choices[7] = ["Stop Harvest", stopHarvest];

    if (VapulaFlags.FOLLOWER_AT_FARM_VAPULA === 1) choices[9] = ["Back", rootScene];
    return { choices };
}
...
function sendToFarm(player: Character): NextScreenChoices {
    ...
    CView.text("You tell your succubus concubine that she is to head towards the lake, find a farm, present herself to the lady who works there and do as she says. Vapula laughs long and hard at this.");
    ...
    VapulaFlags.FOLLOWER_AT_FARM_VAPULA = 1;
...
function backToCamp(player: Character): NextScreenChoices {
    ...
    CView.text("You tell her to head back to camp; there are things you need to do to her you can’t do whilst she’s here. Repeatedly. Vapula fist pumps the sky.");
    ...
    VapulaFlags.FOLLOWER_AT_FARM_VAPULA = 0;
...
function harvestMilk(player: Character): NextScreenChoices {
    ...
    CView.text("You tell Vapula that you want her hooked up to a milking machine whenever possible; you need her fluids.");
    ...
    if (VapulaFlags.FARM_UPGRADES_REFINERY === 0) CView.text("“<i>It’ll sure as hell beat whatever disgusting thing the bitch would have me doing instead. Why don’t you make her build a concentration machine? Lethice has loads; makes one dose of cream go so much further.</i>”");
    else CView.text("“<i>As you insist, [master]. It’ll sure as hell beat working.</i>”");

    VapulaFlags.FOLLOWER_PRODUCTION_VAPULA = 1;

    return { next: rootScene };
}

function stopHarvest(player: Character): NextScreenChoices {
    CView.clear();

    CView.text("You tell Vapula to stop producing succubus milk; you’re practically drowning in the stuff.");
...
function fapulaFapfapfapAppearance(player: Character): NextScreenChoices {
    ...
    if (hasTattoo("vapula")) {
        ...
        if (vapulaFullTribalTats()) {
            ...
            if (numTattoos("vapula") > 1) CView.text("She has the following tattoos emblazoned across her body:\n");
            else CView.text("She has ");

            if (VapulaFlags.VAPULA_TATTOO_COLLARBONE !== 0) CView.text(VapulaFlags.VAPULA_TATTOO_COLLARBONE + "\n");
            if (VapulaFlags.VAPULA_TATTOO_SHOULDERS !== 0) CView.text(VapulaFlags.VAPULA_TATTOO_SHOULDERS + "\n");
            if (VapulaFlags.VAPULA_TATTOO_LOWERBACK !== 0) CView.text(VapulaFlags.VAPULA_TATTOO_LOWERBACK + "\n");
            if (VapulaFlags.VAPULA_TATTOO_BUTT !== 0) CView.text(VapulaFlags.VAPULA_TATTOO_BUTT + "\n");
        }
    }

    player.stats.lust += 10;

    return callSlaveVapula(player, false);
}

// Talk (Z)
function talkToVapulaForSomeReason(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>You want to talk?  Well it's quite rare to see " + mf(player, "studs", "girls") + " looking for conversation with the likes of me, but go ahead.  What do you want to talk about?</i>\"");
    // Option: Stop Fucking Harem OR Fucking Harem OK (if toggled on off)
    // Option: Cerulean Threesome On/Off
    let threesomeT: string = "";
    let threesomeB: ClickFunction;
    let haremT: string = "";
    const haremB: ClickFunction = toggleVapulaHaremFucks;
    if (VapulaFlags.VAPULA_HAREM_FUCK === 0) {
        CView.text("\n\nVapula will not currently fuck anyone else in your camp without permission.");
        haremT = "FuckFollowers";
    }
    else {
        CView.text("\n\nVapula will fuck any willing partners you may have around ");
        haremT = "Don'tFuck";
    }
    // IF PC has cerulean succubused before
    if (player.effects.has(StatusEffectType.RepeatSuccubi)) {
        if (VapulaFlags.VAPULA_THREESOMES === 0) {
...
// Feed
function feedVapulaACupOfJizz(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You tell Vapula you're going to reward her for being such a good slut; idly stroking [oneCock] into erection, you command her to get on her knees so she can receive her meal.  The demoness eagerly complies and grabs hold of [oneCock].  She then proceeds to lick it, coating the entirety of your junk with her warm, sweet saliva. With an expert precision, she massages all your most sensitive points, pumping pre-cum up your urethra with ruthless efficiency.  As drops of your juices dribble from the tip of your " + describeCock(player, player.body.cocks.get(0)) + " she voraciously slurps them with a gourmand's smile.");
    CView.text("\n\nHer ministrations feel good, but she keeps tickling you with her soft hands and pulpy lips, and her flexible tongue gives you the most infuriating tingles of pleasure.  With a groan of impatience, you grab her head and stuff your whole " + describeCock(player, player.body.cocks.get(0)) + " into her mouth, literally stuffing her full of your junk.  She nearly chokes at first but her old reflexes acquired from a whole life devoted to lewdness and licentiousness start kicking in; Vapula gluttonously deepthroats your meat, softly moaning as she fingers herself with one hand and caresses your ");
    if (player.body.balls.count > 0) CView.text(describeBalls(true, true, player));
    else CView.text("groin");
    CView.text(" with the other.  She sucks like a cock-hungry whore, treating your shaft like a straw as she slurps down your juices; her demonic throat feels incredibly hot and her saliva seems to make your " + describeCock(player, player.body.cocks.get(0)) + " grow even harder; her lips squeeze your meat in the most exquisite manner, sending you into short spasms of pleasure every time.");
    CView.text("\n\nShe works on it for what seems like hours.  You look down at your slut; she keeps staring at you with avid yet playful eyes, never breaking eye contact as she relentlessly throatfucks herself; you feel her lips curve a little as the shape of a ravenous smile appears on her face, and her eyes glitter with an insatiable need.  The naughty bitch! She knows she's going to get what she wants, whether you like it or not.  You wish you were able to control yourself, but the covetous succubus never lets go of your " + describeCock(player, player.body.cocks.get(0)) + ", clinging to it like an animal.");
    CView.text("\n\nYour stamina is rapidly overwhelmed and you decide to reward her effort; you savagely press Vapula's head against your groin, effectively burying all " + numToCardinalText(Math.round(player.body.cocks.get(0).length)) + " inches of your " + describeCock(player, player.body.cocks.get(0)) + " in her waiting throat; her tongue keeps teasing your rod for a few seconds, and then you cum, brutally.  ");
    if (player.body.balls.count > 0) CView.text("Your " + describeBalls(true, true, player) + " churn");
    else CView.text("Your " + describeCock(player, player.body.cocks.get(0)) + " twitches");
    CView.text(" as you release a sticky spooge fountain in her stomach.  Her lips keep squeezing your junk in order to milk everything you've got, and in your shuddering orgasm you're too eager to comply.  With a surrendering sigh, you abandon yourself to your climax and keep cumming, squirting your baby-batter to the last drop as the succubus keeps gulping it.  Her eyes are closed in an expression of complete satisfaction; she enjoys her meal to its full extent, filling her belly with your spooge");
    if (player.cumQ() >= 1500) CView.text(" until she looks six months pregnant");
    CView.text(".  At last, the cum-flow spilling through your urethra starts to ebb and your " + describeCock(player, player.body.cocks.get(0)) + " stops throbbing; only then does she removes your junk from her mouth with a loud POP.  Your " + describeCock(player, player.body.cocks.get(0)) + " appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.");
    player.orgasm();
    player.stats.cor += .5;

    VapulaFlags.VAPULA_DAYS_SINCE_FED = 0;
    VapulaFlags.VAPULA_TEASE_COUNT = 0;
    return { next: returnToCampUseOneHour };
}

// Threesome
function vapulaThreesomeMenu(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Who do you invite?");
    // Option: Amily
    let amily: ClickFunction;
    // Option: Ceraph
    let ceraph: ClickFunction;
    // Option: Sophie
    let sophie: ClickFunction;
    // Option: Jojo
    let jojo: ClickFunction;
    // Option: Izma
    let izma: ClickFunction;
    if (player.body.cocks.length > 0 || (player.body.vaginas.length > 0 && player.inventory.keyItems.has("Demonic Strap-On"))) {
        if (campCorruptJojo()) jojo = vapulaJojoThreesomes;
        if (amilyFollower() && amilyCorrupt() && player.body.cocks.length > 0) amily = vapulaAndAmilyThreesome;
        if (ceraphIsFollower() && player.body.cocks.length > 0) ceraph = vapulaCeraphThreesome;
        if (bimboSophie()) sophie = vapulaSophieThreesomeSelect;
        if (izmaFollower() && VapulaFlags.IZMA_NO_COCK === 0) izma = vapulaAndIzmaThreeSome;
    }
    return {
        choices: [
            ["Amily", amily],
...
function vapulaCeraphThreesomePleaseBoth(player: Character): NextScreenChoices {
    ...
    CView.text("You order your girls to turn around; you want to give them a good dicking.  They comply while giggling like a pair of whores, eager to get some cock up their holes.");
    CView.text("\n\nYou push Vapula onto the floor, making her fall on her back, legs spread.  You then grab hold of Ceraph's shoulders and put her on all fours, her warm snatch facing Vapula's.  Already moist, the succubus and the omnibus naturally start caressing each other, kissing and groping each other's breasts.  You watch them giving in to their own depravity before joining the show; grabbing Ceraph's plump butt, you press the tip of your " + describeCock(player, cockThatFits) + " against her backdoor.  The unexpected intrusion makes the omnibus cry, her gasps of joy only enticing you to push further, even at the cost of stretching her anal ring.  A few seconds later you're all the way in, enjoying the boiling and tight recesses of her pucker.  You roughly seize Ceraph's fuck-pillows and proceed to relentlessly assault her ass, making her yell under the conjugated efforts of your throbbing junk and the hot succubus fingering her from below.  Completely dominated by the pleasure you're inflicting on her, your slut starts muttering dirty words, often interrupted by sudden screams, \"<i>Fuck... fuck... fuck my ass... " + mf(player, "Master", "Mistress") + "...feels so- AAAAAH! Oh FUCK YEAH! Harder, " + mf(player, "Master", "Mistress") + "! Fuck your bitch!</i>\"");

    CView.text("\n\nAnnoyed by the noise she's making, you slap her firm rump repeatedly, your hands imprinting marks on her red ass-cheeks.  Your buttslut doesn't seem to care, completely blissful from being used by her rightful " + mf(player, "Master", "Mistress") + ".  She groans, grinds harder and squirms in Vapula's embrace in her efforts to impale herself further on your " + describeCock(player, cockThatFits) + ".  The sight of your sluts' love-tunnels mixing their fluids together as they squirt as well as the insane motion of Ceraph's wobbling ass sending the sweetest sensations to your rod prove too much for you; despite your best efforts to hold back, you blast Ceraph's ass with your baby-batter, the globs effectively packing her anal entrance");
    if (player.cumQ() >= 1500) CView.text(" and swelling her belly until she looks ready to give birth to a full horde of imps");
    CView.text(".");
    CView.text("\n\nYou feel your ");
    if (player.body.balls.count > 0) CView.text(describeBalls(true, true, player) + " twitch and your " + describeCock(player, cockThatFits) + " pulsate as they unload their");
    else CView.text("your " + describeCock(player, cockThatFits) + " twitch and pulsate as it unloads its");
    CView.text(" essence into your omnibus pet, bringing her to orgasm as she blubbers incoherently, \"<i>Oh yessssssssss, just like that... Yeah... my ass... cum... yessssssssss...</i>\"  Her voice goes so high pitched it trails off, letting you hear Vapula's soft moans below.  You quickly pull out your " + describeCock(player, cockThatFits) + ", still orgasming.  You hear Ceraph feebly protest at the sudden lack of cock filling her hole, \"<i>What are you doing? Put it back! Put it back already!</i>\"");
    CView.text("\n\nWith another slap you tell her to shut up and learn to share; her partner hasn't received any cock yet, and you intend to remedy it.  Grabbing Vapula's thighs this time, you lunge toward her wet fuck-hole and bury your " + describeCock(player, cockThatFits) + " to the hilt, ");
    if (player.body.balls.count > 0) CView.text(describeBalls(true, true, player) + " hitting her butt-hole with a faint thud");
    else CView.text("thudding up against her cushy butt-cheeks");
    CView.text(".  The purple sex-goddess squeals, your abrupt dick-digging utterly ravaging her cunt; her unholy warmth and impossibly tight confines are almost enough to make you cum instantly as her vaginal walls wrap in the most sensitive way around your rod, but you manage to hold off the flow of semen churning in your groin.");
    // [if 2nd cock]
    ...
    // [if multicocks]
    if (player.body.cocks.length > 1) CView.text("  She expertly jerks each of your rods in turn, gently polishing them as they grow back into an erect state.");
    CView.text("  She's obviously used to this kind of practice; even though [eachCock] felt numb after cumming so hard, you feel your lust build up again as your genitals throb once more with need.  You grab Ceraph's head and impale her on your crotch, forcefully throat-fucking her.");
    ...
    CView.text("  Your thoughts are quickly drowned in waves of pleasure, and soon your only thought is to ram more of your " + describeCock(player, cockThatFits) + " down your sluts' throats, pumping their heads alternatively around your shaft");
    if (player.body.cocks.length > 1) CView.text("s");
    if (player.body.balls.count > 0) CView.text(" as the girls keep fondling your " + describeBalls(true, true, player));
    CView.text(", bringing you extra tingles of pleasure to your body.");
    ...
    if (player.body.cocks.length > 1) CView.text("s");
    CView.text(", their lips doing their best to milk ");
    ...
    CView.text("cream.  Once your junk feels completely empty, [oneCock] is released with a loud POP.");
    ...
    if (player.stats.lib > 10) player.stats.lib += -1;
    if (player.stats.lib > 50) player.stats.lib += -1;
    if (player.stats.lib > 70) player.stats.lib += -1;
...
function vapulaAndCeraphButtfuckTrainYeehaw(player: Character): NextScreenChoices {
    CView.clear();
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(100));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    CView.text("Grinning lewdly, you dispose your omnibus and your succubus in a front line such that Ceraph's cock points toward Vapula's anus.  Ceraph pants expectantly.  \"<i>What do you have in mind this time, Master?</i>\" Without a word, you place yourself behind Ceraph's back and start stroking [eachCock] against her butt-cheeks, your tip dribbling pre-cum teasingly; you shove a few fingers into Ceraph's warm snatch, stimulating her until she lets out a muffled moan as her own cock grows to full erectness.  At last, you ready yourself, your cock knocking on Ceraph's backside.  You gently explain to your girls that you're going to initiate a butt-fuck train.  Vapula giggles softly, \"<i>Ooooh, I love this!  But... wait, I'm in the front of the -</i>\"");
    CView.text("\n\nWithout any warning, you harshly push your " + describeCock(player, cockThatFits) + " into Ceraph's tight anal recesses, making her clench and tense in arousal.  She immediately responds by grasping Vapula's squeezable thighs and digs her own, broad prick inside the succubus' colon.  The totally unexpected rectal assault makes both hell-girls scream in surprise and pleasure mixed, their anal walls saturated with intense dick-friction.  You stand here comfortably, easily lodging your " + describeCock(player, cockThatFits) + " inside Ceraph's backdoor while letting her have a better grip on Vapula's buttocks.  The omnibus' pucker is insanely hot!  Your rod of flesh feels like it's going to melt under her demonic warmth, and her astonishingly narrow colon squeezes it in the most harsh and delightful way.  You decide to speed up the pace and start sliding back and forth, ");
    if (User.settings.silly()) CView.text("screaming at the top of your lungs: \"<i>BUTT-FUCK! BUTT-FUCK! BUTT-FUCK!</i>\", soon joined by your horny anal partners");
    else CView.text("groaning and panting as you ravage Ceraph's pucker with your junk");
    CView.text(".\n\n");
    CView.text("Between two mad thrusts, you order Ceraph to imitate you in every movement, treating Vapula the same way you treat her.  You start slapping her jiggly ass, sending ripples across her red shivering flesh.  Entranced by your violent ministrations, the omnibus complies to your orders, spanking Vapula as hard as she can.  Soon moans are heard all across the butt-fuck train; the more you slap and thrust, the harder Ceraph hits the purple sex-goddess, as if she wanted to prove she was a better servant, worthier of your " + describeCock(player, cockThatFits) + ".  You reach around and knead Ceraph's tits, playing with her nipples as you devastate her backdoor with ferocious dick assaults, and she hesitantly grasps Vapula's bouncy tits, unable to match your thrusts in intensity.  She jerks her head backwards and moans, almost forgetting her own prick impaling the poor succubus in front of her.  Seeing that the poor bitch has lost all control over her own body, you stretch your arms further and reach Vapula's breasts.  The succubus is literally drooling in ecstasy; forced to feel the mighty thrusts of a strong omnibus as well as yours, she is bent over submissively, her tongue hanging and her eyes rolling under the combined strength of your anal-dickings.  You pound harder, squeezing her tits at each cock-nudge and muffling her pleasure-filled shrieks when you can.");

    CView.text("\n\nAs for Ceraph, the poor thing isn't even able to think anymore.  Her rhythmical moans have been gradually replaced with some incoherent dirty babble, \"<i>Oh fuckfuckfuck... good.. fuck...  YES! Fuck... yes... fuckfuckfu-aaaAAAAAH!</i>\"  You keep impaling the omnibus' butthole with your " + describeCock(player, cockThatFits) + ", pounding harder in hope the anal tearing will shut her up, but she only seems to get even more pleasure from such a violation of her insides.  Her mind isn't able to reason properly, torn between her ferocious butt-penetration and the pleasure her own cock is getting as it moves of its own volition in and out of Vapula's tight little pucker.");
...
function vapulaSophieThreesomeSelect(player: Character): NextScreenChoices {
    if (player.body.cocks.length > 0) return vapulaSophieThreeSome(player);
    else return vapulaSophieThreesomeCunts(player);
}

function vapulaSophieThreeSome(player: Character): NextScreenChoices {
    CView.clear();
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(100));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    CView.text("You call Sophie insistently, panting in anticipation.  The blond-feathered bimbo doesn't come at first - perhaps the dumb slut forgot her name again - but as you grow more impatient you finally notice the platinum-headed woman, her enormous boobs bouncing hypnotically and her luscious thighs swaying lustfully as she walks.  She grins beatifically and coos: \"<i>Yes?  Is there anything I can, like, do for... ooooh, but who's this?  She's sooooooo beautiful!  She's got, like, big boobs and stuff!</i>\"  Vapula sighs in consternation.  \"<i>What can you do with her, " + player.desc.name + "? I'm sure she's going to forget my presence by next hour.</i>\"");
    ...
    CView.text("\n\nWithout ceremony you grab the avian slut's fuckpillows and eagerly sandwich your meat between them, pressing the tip of your " + describeCock(player, cockThatFits) + " against her tight anus.  As soon as the first inches start sliding in, the bimbo slut flaps her wings frenetically as she squawks in pleasure, her anal muscles contracting on their own as an invitation to slide in further.  Before you even think of it ");
    if (cockThatFits.area > 75) CView.text("you've filled the entirety of her colon");
    else CView.text("your cock is already buried to the hilt, its mammoth girth bulging between her broad hips");
    CView.text("; the bulge even grinds a bit against Vapula's own butt and the succubus reaches toward you, entranced with lust.  She wraps her arms around you, letting you feel her tender boobs being squeezed against your [chest] as she gives you a mad kiss filled with passion.  Your lips remain locked for a while as you explore each other's mouths with your tongues; you almost forget the burning hot and roughly tight hole.  It entraps your " + describeCock(player, cockThatFits) + " as it continues massaging with the harpy's vice-like anal muscles.  The bimbo keeps wriggling arond your rod in order to get more of it inside, pleasantly smashing her fat rear against your groin.");
    CView.text("\n\nStill not breaking the kiss, you groan silently and pull out your " + describeCock(player, cockThatFits) + ", all smeared in pre-cum.  Sophie whimpers and wails softly until you thrust all the way back in, literally jamming her with your junk.  Her pupils widen, her mouth opens and her gigantic tits keep swinging in the dirt as her whole body trembles and shudders from the brutal anal violation.  Her cry of pleasure is so high-pitched you barely hear her subsequent moan; satisfied, you start thrusting at a more convenient rhythm, enjoying the lush depths of the avian slut as the succubus keeps making out with you.  You keep butt-fucking Sophie, unable to focus on the ministrations of the expert succubus or the extremely warm recesses of the harpy's interior.  You regularly slap her plush, fat buttocks, sending ripples across her whole fleshy body. Needless to say, the bimbo's pussy is completely drenched in fem-spunk, her wide pussy drooling trickles after trickles of her juices, betraying her utter arousal.");
    // [if 2nd cock]
    ...
    if (player.body.balls.count > 0) CView.text("balls and massages your ");
    CView.text(describeButt(player) + ".");
    // [if third cock]
    if (player.body.cocks.length > 2) CView.text("  Using her extra flexibility, she bends over and start jerking your third pole, the sweet handjob adding to your pure cocktail of pleasure.");
    CView.text("  You can feel her hard nipples poke at your skin as they gently scrape across your torso.  Vapula hardly ever interrupts her embrace, only pulling back from your lips to take care of other intimate parts of your body.  You keep riding the bimbo senseless, making her squirm and squawk in ecstasy as your own cries of pleasure are muffled by Vapula's voracious lips.");
    ...
    VapulaFlags.VAPULA_DAYS_SINCE_FED = 0;
    VapulaFlags.VAPULA_TEASE_COUNT = 0;
    if (player.stats.lust < 50) player.stats.lust = 50;
    return { next: returnToCampUseOneHour };
}

// Vapula-Sophie threesome (as female)
function vapulaSophieThreesomeCunts(player: Character): NextScreenChoices {
    CView.clear();
    ...
    CView.text("\n\nVapula sighs in consternation.  \"<i>What can you do with her, " + player.desc.name + "? I'm sure she's going to forget who I am in an hour.</i>\"");
    ...
    CView.text("\n\nWithout ceremony you grab the avian slut's fuckpillows and eagerly sandwich your dildo between them, pressing the tip of it against her tight anus.  As soon as the first inches start sliding in, the bimbo slut flaps her wings frenetically as she squawks in pleasure, her anal muscles contracting on their own as an invitation to slide in further.  Before you even think of it your strap-on is already buried to its leather hilt, its mammoth erection bulging between her broad hips; the bulge even grinds a bit against Vapula's own butt and the succubus reaches toward you, entranced with lust. She wraps her arms around you, her tender boobs against your own " + describeAllBreasts(player) + " as she gives you a mad kiss filled with passion. Your lips remain locked for a while as you explore each other's mouths with your tongue; you almost forget the tight hole wrapped around your dildo.  You are brought back to it by the bimbo wriggling around the rod rammed in her rear in an attempt to squeeze more of it into her, twisting your end around in your sopping " + describeVagina(player, player.body.vaginas.get(0)) + " as she pleasantly mashes her fat rear against your groin.");
    ...
    CView.text("\n\nVapula shifts her attention to your breasts, her long tongue running lasciviously over your " + describeNipple(player, player.body.chest.get(0)) + "s, tightly circling and gently teasing their tips until they are as sensitive and erect as any clitoris.  Her soft lips edge you further and further into your fuck daze, riding the bimbo senseless, making her squirm and squawk in ecstasy as your own cries of pleasure are muffled by Vapula's voracious lips.");
...
function vapulaAndAmilyThreesome(player: Character): NextScreenChoices {
    CView.clear();
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(100));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    ...
    if (player.body.balls.count > 0) CView.text(": her drooling pussy needs to be prepared because you're going to take her next");
    CView.text(".  Vapula smirks in excitement and eagerly shoves her tumescent pussy in front of the mousette's waiting lips; Amily keeps wagging her butt impatiently, looking forward to being used as a disposable anal-toy.  You wink at Vapula; she's going to like it.");
    CView.text("\n\nGrabbing hold of your mouse's plush buttcheeks, you stretch her anal opening and in a brutal, powerful thrust, you slide all of your " + describeCock(player, cockThatFits) + " down her interior.  Amily immediately squeaks, her mouth drooling and her whole body twitching as she adapts to the erect prick inside her.  Her cries of pleasure are suddenly muffled as Vapula, too turned on to care about your mousette, brutally grabs her head and squeezes her face between her legs.  In the meantime, you start pumping at her ass, effortlessly moving your hips backwards and forwards as your " + describeCock(player, cockThatFits) + " ravages the mousegirl's insides.  The poor creature, already driven over the edge by your relentless anal pounding, is now forced to lick the succubus' luscious pussy, her tongue being forced into probing her lush depths at each thrust received from behind.  She keeps moving back and forth between Vapula and you, roughly mishandled as both ends of her body are being abused.  However, this does not seem to decrease in the least the pleasure she's getting. She joyfully slams back her ass against your groin at every thrust, her insides always hungry for more dickflesh.  Your hips are quasi-immobile as she impales herself on her own in an insatiable cock-hunger.");
...
export function vapulaAssistsCeruleanSuccubus(player: Character): NextScreenChoices {
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(100));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    ...
    CView.text("\n<b>That night...</b>\nYou wake up Vapula, telling her to get ready for your \"guest\". You expose your " + describeCock(player, cockThatFits) + ", ordering your succubus slut to work you up into a proper aroused state.  The sleepy purple girl yawns a bit, still half-conscious from her dirty dreams, and you profit from the opening to shove your junk between her lips.  Clearly getting the message, Vapula starts licking it, her expert mouth taking all your length without even thinking of it.  She deepthroats you for a while, until you hear steps from outside the  In the moonlight you can make out a tall silhouette: the night succubus has come back.");
    ...
    CView.text("\n\nYou tell the blue woman not to worry about that and gently pat Vapula's head; the purple demoness coughs as she interrupts her divine blowjob and giggles when she notices one of her own kind.  She slowly gets up and is soon joined by your demon guest, awaiting your next orders.  The blue succubus and the purple succubus stand before you, their eyes filled with voracious lust and impatient need.  Two girls are eager for your cum, and there's only one " + mf(player, "man", "herm") + " to satisfy them both... You don't know if you'll be up to the task, but it sure can't hurt to try.  Smiling to both, you remove your " + player.inventory.equipment.armor.displayName + " and lazily lie down, your erect " + describeCock(player, cockThatFits) + " standing provocatively.  \"<i>Well, come on ladies.  Please me.  Can't you see I need some satisfaction?</i>\"");
    CView.text("\n\nAt this invitation, both luscious creatures practically jump you.  In no time you're covered with blue and purple flesh, arms and lips caressing you everywhere but focusing on your most sensitive spots.  You shiver several times as the hell-girls give you tingles of pleasure that tease you almost unbearably.  You feel hands and tongues being pressed against your " + describeCock(player, cockThatFits) + " and smearing it with saliva and pre-cum; you are kissed several times, your mouth is being padded with a flexible demonic tongue as your palate is filled with a sweet unholy taste; the scent of horny succubi makes you giddy and all the more aroused.  You feel nipples being stuffed in your mouth and you instinctively suck, gulping the succubi's essences like a newborn.");
    CView.text("\n\nDrops of some fluid start falling on your face; looking up, you see nothing but a drooling, gushing pink pussy being lowered down.  Idly wondering whose it might be, you start probing your tongue into that moist vagina, eagerly sucking and licking as the whore atop you moans and bucks wildly; from her hoarse, ragged voice you can recognize Vapula.  Gods, she's such a horny succubus slut!  You keep eating her out until she screeches madly in pleasure, probably waking up every living creature around your   At the same time, you feel your " + describeCock(player, cockThatFits) + " is being handled and inserted into a nice warm snatch.  The brutal temperature change and amazing tightness drives you near the edge, and you actually feel your spooge flowing up and down your urethra as you contract your hips in a desperate effort to control yourself.  Your groin harshly hits the blue succubus', and your " + describeCock(player, cockThatFits) + " digs even deeper in her innermost depths.  She sighs in contentment and slowly whispers, \"<i>Guests first.</i>\"");
    CView.text("\n\nShe starts riding you ferociously, clamping you down and gripping your hips with claw-like hands.  Her vaginal muscles implacably contract around your " + describeCock(player, cockThatFits) + ", acting as an boiling-hot and inescapable cock-ring.  Still trying your best to explore Vapula's love-tunnel, you reach around to get some of her tits, only to find the blue succubus' hands pinching your slut's nipples, making her shriek in torturesome pleasure.  You both keep groping her voluptuous body, making her shudder under your abusive massages.  Your head is now entirely soaked with Vapula's juices - that whore seems to orgasm ridiculously often.  Every time you consider pulling back your tongue and gasping for some fresh air, her twat squirts another crazy jet of fem-spunk down your face, making your vision blurry and leaving you with no choice but to lick her interior clean.  However, your efforts only stimulate her all the more, leading you to wonder how she can generate such an absurd amount of vaginal juices.");
    // [if multicock]
    if (player.body.cocks.length > 1) CView.text("  She suddenly seems to have an idea: leaning down to reach your second cock, she starts licking it, accompanying the savage thrusting of the blue demoness as she slurps your extra junk, giving you even more pleasure.");
    else CView.text("  The blue demoness leans down and starts licking Vapula's moist pussy, intertwining her flexible tongue with yours and pleasuring the purple slut even more.");
    ...
    CView.text("\n\nYou keep pounding away, coating your succubi with a mix of newly formed pre-cum, old cum from your previous ejaculation and vaginal juices being spilled all over your " + describeCock(player, cockThatFits) + ".  The succubus' inhuman recesses start to feel too hot for you, and with another powerful shot, you cum.  The contact of your globs of spooge hitting Vapula's backdoor makes her squirt another of her insane orgasms, but you keep releasing your milk inside her.");
    // [if second cock]
    if (player.body.cocks.length > 1) {
        CView.text("  You pack her tight snatch ");
        // [if third cock]
        if (player.body.cocks.length > 2) CView.text("and your blue lover's mouth ");
        CView.text("likewise.");
    }
    CView.text("  Jism is being spilled all over Vapula's backside and spurts in sticky jets down her baby-maker and the blue slut's waiting mouth.  Vapula's holes tremble and contract in order to keep as much spooge as possible, and the blue succubus tries to feed as well, frustrated to feel her own semen being eaten by Vapula's gluttonous tongue.  Cum keeps circulating in and out of the hell-girls, with your " + describeCock(player, cockThatFits) + " as a source.  Due to their mouths being occupied gulping semen from some hole, they only utter slow, lewd moans as they release their own juices in orgasm. The echo of your trio of bodies abandoning themselves to this debauched orgy is resonating all around your   It should be wiser to end this quickly, lest you attract a horde of horny imps willing to join the party.");
    CView.text("\n\nYou abruptly pop out your " + describeCock(player, cockThatFits) + " and rudely order your sluts to finish their work.  Still recovering from the hard fuck, they lazily obey and lick all the cum from your soaked junk.  They work quickly, not willing to let any drop of cum be stolen by their concurrent.  Sometimes their tongues will claim the same deposit of spooge and they will fight over it, their tongues entwining until they lewdly kiss each other.  The display is quite erotic but you'd prefer they'd kiss your " + describeCock(player, cockThatFits) + ", so you call them to order every time it happens.  When they're finally done emptying you from all your sperm, you give your lovers a tender kiss, thanking them for your efforts.  The blue succubus, still dazed by this intense session, hands you distractedly a bottle of her own milk.  \"<i>Woah.  That was... amazing. I don't know what she does to you but you taste much, much better when you keep this girl around.  We should definitely repeat this soon.</i>\"  Giving your dick a goodbye kiss, she staggers out of your camp, probably drunk from your huge cumload.");
    CView.text("\n\nYou ask Vapula if she agrees with what the blue succubus said. As a response, your pet hugs you tightly");
    if (player.body.tallness < 60) CView.text(", enclosing your head in her cleavage");
    CView.text(".  \"<i>That was fun! I can't wait to have another party like this! You should invite her more often!</i>\"");
    CView.text("\n\nYou know you definitely will.");
    // standard night succubus stat increases
    VapulaFlags.VAPULA_DAYS_SINCE_FED = 0;
    VapulaFlags.VAPULA_TEASE_COUNT = 0;
    player.orgasm();
    player.stats.str += randInt(2);
    player.stats.tou += randInt(2);
    player.stats.spe += randInt(2);
    player.stats.int += randInt(2);
    player.stats.cor += 2.5;

    return player.inventory.items.createAdd(player, ItemType.Consumable, ConsumableName.CeruleanPotion, campMenu);
}

// Vapula/Jojo threesome
function vapulaJojoThreesomes(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You call out in the jungle, \"<i>Slut!</i>\"  Vapula snickers, \"<i>I'm here, you know.</i>\"  You slap her ass to shut her up and quickly explain that you don't call her slut because she doesn't care, being a succubus.  There is only one fucktoy you call by that name, because he's in denial.  By Lethice you intend to have your way with him, and Vapula will get to participate too.  As she eagerly listens to you, her tongue inadvertently licking her lips, you hear soft steps behind you.  You look back and see Jojo walking hesitantly, intimidated by your unholy presence and the towering ex-dominatrix.  \"<i>You were right, he's such a cute shy slut.  I bet he secretly wants some nice cock to ram his every hole.  Don't you, little pet?</i>\"  The mousy monk huddles up little by little, bowing his head uncomfortably.");
    ...
    let tease: ClickFunction;
    let train: ClickFunction;
    if (player.body.vaginas.length > 0 || player.inventory.keyItems.has("Demonic Strap-On")) tease = NTRSomeJojos;
    if (player.body.cocks.length <= 0) { } else {
        tease = NTRSomeJojos;
...
function NTRSomeJojos(player: Character): NextScreenChoices {
    CView.clear();
    if (player.body.cocks.length > 0) {
        let cockThatFits = player.body.cocks.find(Cock.CockThatFits(100));
        if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
        CView.text("\"<i>Admit it, you'd love to use that smoking hot body, wouldn't you?  Don't lie to me.  You'd give up your soul in order to fuck her brains out.  Don't you want to plunge your dick inside her?  Don't you want to fuck my beautiful succubus wife?</i>\"");
        ...
        CView.text("\n\n\"<i>These hips... these thighs...</i>\"  Grabbing hold of Vapula's waist, you shove your " + describeCock(player, cockThatFits) + " between her legs and start grinding your junk against her vaginal entrance, teasing her in the most lustful manner. You keep grinding until your purple slut utters a lubricious moan.");
        CView.text("\n\n\"<i>No, really, her whole body is perfect.</i>\"  You run your hands all over the succubus curves, pinching here and caressing here.  \"<i>And gods, that tight anus!  Oh, Jojo, if you knew how cock-hungry she is...</i>\"  With a wicked, wanton smile, you rail Vapula's ass with your " + describeCock(player, cockThatFits) + ", filling the entirety of her interior with your dickflesh as you watch the monk.  The poor mousy slut has completely given in to his lust; he has fallen on his knees and is now stroking himself feverishly, unable to break your gaze.  You start pounding Vapula's butt; with each thrust you grunt ostensibly, letting your mouse pet know how much you enjoy it.  As for Vapula, the slutty demoness is clearly enjoying her anal treatment, the sight of a poor bitch masturbating to her being all the more arousing to her.  She also stares at him with longing eyes, as if to provoke him with her nude, horny body being fucked by someone else.  Soon she is screaming in utter ecstasy and starts babbling meaningless dirty talk, \"<i>Fuck, fuck me... oh yessssssssssssss... more!  MORE!</i>\"  She's clearly acting as a naughty bitch!  You wonder if the presence of a shy creature is helping to release her inner wantonness... Shrugging it off, you decide to focus on the nice purple body impaling itself on your " + describeCock(player, cockThatFits) + " and pound her harder, your meat releasing streams of sex juices as it pounds in and out of Vapula's pucker.");
        if (player.body.cocks.length > 1) {
            ...
            // [if third cock]
            if (player.body.cocks.length > 2) CView.text("  Since the horny slut has a demonic vaginal capacity, you might as well fit more dick in it: in no time your third dick joins its colleague in this infernal vaginal assault.");
            CView.text("  ");
        }
        else CView.text("\n\n");
        CView.text("You push in and out of her, claiming both her holes and snickering at the poor mouse thing that can get neither. With a mighty push, you put Vapula on all fours and lean down in order to touch more of her supple body.  Your purple slut is now dying with pleasure, her tongue hanging and her eyes staring behind Jojo as the monk squeaks pitifully in his pathetic self-stroking.");
        ...
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(" being filled with cock.");
        ...
        if (tentacleJojo()) CView.text("his tentacles start flying toward his ass, ");
        CView.text("his-");
        CView.text("\n\n\"<i>No.</i>\"");
...
function jojoButtFuckTrain(player: Character): NextScreenChoices {
    CView.clear();
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(100));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    CView.text("You tell Jojo it's his lucky day - you're going to have a joyful butt-fuck session with him and your purple pet.");
    CView.text("\n\n\"<i>Admit it, you'd love to use that smoking hot body, wouldn't you?  Don't lie to me.  You'd give up your soul in order to fuck her brains out.  Don't you want to plunge your dick inside her?  Don't you want to fuck my beautiful succubus wife?</i>\"");
    CView.text("\n\nJojo shakes his head in denial, but he can hardly keep any shred of credibility when his rock-hard mouse cock speaks for him.");
    ...
    CView.text("\n\nYou seize your " + describeCock(player, cockThatFits) + " and press its tip against the mouse's sphincter, slowly pushing.  Startled by the sudden anal invasion, Jojo reflexively grabs Vapula's hips and inserts his rodent prick into her.  The simultaneous penetrations make your sluts squeak and moan in ecstasy, their bodies entranced as their anal walls provide them one thousand subtle sensations.  You grip his arms, using him as leverage to pull out, and slam it back home again.  Used like the fucktoy he truly is, Jojo doesn't resist; his dick moves of its own volition in and out of Vapula's interior as you relentlessly push him back and forth.  You secretly rejoice: his tight little ass is clearly adapting to your " + describeCock(player, cockThatFits) + " and it somehow feels even better than usual.  You pound hard, making him groan and beg for more; you eagerly give him what he wants and stuff him full of your dickflesh.");
    if (cockThatFits.area > 100) CView.text("  You can actually feel his internal organs being bumped against your meat as your " + describeCock(player, cockThatFits) + " stretches his colon beyond its normal size and thrusts in and out like a charging mammoth.");
    ...
    if (tentacleJojo()) {
        ...
        // [if vagina]
        if (player.body.vaginas.length > 0) CView.text("  Last but not least, a soft cock-knock at the entrance of your " + describeVagina(player, player.body.vaginas.get(0)) + " signals an incoming intrusion.  If your mouth weren't so full of cock you would sigh in relief as your aching pussy is thoroughly penetrated.");
    }
    // [if PC has extra long tentacle cock, variable is n1]
    const tentacleCocks = player.body.cocks.filter(Cock.FilterType(CockType.TENTACLE)).filter((cock) => cock !== cockThatFits);
    const tent1 = tentacleCocks[0];
    const tent2 = tentacleCocks[1];
    const tent3 = tentacleCocks[2];
    if (tent1) CView.text("\n\nA deliciously depraved idea crosses your mind; using your dick muscles, you move your " + describeCock(player, tent1) + " toward Vapula.  It squirms for a moment as you look for her wet snatch, poking her everywhere with your tip and teasing her with your accidental ministrations, but at last you find her vaginal entrance.  Using the rubbery flexibility of your plant-like prick, you brutally shove it down her love-tunnel, trying to force inside as many inches as you can.  You can feel her instinctively altering her own body in order to accommodate more dickflesh inside her, and you keep pushing until your " + describeCock(player, tent1) + " is stretched to maximum capacity.  Somehow your tentacle rod is entirely buried in Vapula's warm depths, and her unbelievable tightness is driving you mad with lust.  It twists on its on volition inside her, its vegetable-like texture being more sensitive and receptive to pleasure than a normal prick and the sensations it sends to you are abnormally exquisite.  You almost forgot the asshole you were pounding with your " + describeCock(player, cockThatFits) + " but the way Jojo wriggles against it in order to fit more of your length inside him reminds you of your job.");
    // [if PC has another tentacle cock, variable is n2]
    if (tent2) CView.text("\n\nSince your sluts still have free holes left, you decide to fill some more fuck-holes with your junk.  Guiding  your " + describeCock(player, tent2) + " toward Vapula's mouth, you think you could use a proper blowjob from a trained succubus and wait for her to let out another moan before filling your junk completely and thrusting up and down brutally, literally mouth-fucking her and treating her lips as a tight cock-ring.  Her expert tongue wraps around your girth, squeezing it and clinging to it like it's the most precious thing in her life.  She keeps jerking back and forth due to the hard butt-fucking she's receiving, and the jolts are transmitted to your " + describeCock(player, tent2) + " in the most teasing manner.");
    // [if PC has another tentacle cock, variable is n3]
    if (tent3) CView.text("\n\nSince Vapula is already sucking you off, why not Jojo?  You still have appropriate junk left after all.  You promptly send your wriggling " + describeCock(player, tent3) + " and push it between the mousey's lips.  However, the slut won't open them!  You pound him ragingly, slamming the entirety of your " + describeCock(player, cockThatFits) + " down his colon and bruising his firm butt until he lets out a deep howl of pleasure and pain mixed; you profit from the opening and stuff his mouth so full of your " + describeCock(player, tent3) + " you think he's going to choke.  However, his mouth seems to be accustomed to take monstrous lengths - probably a side-effect of his corruption - and he quickly accommodates to your size.  Although he can't provide a blowjob as sweet as Vapula, you can definitely tell he's skilled at sucking dick.  The cute little slut!  You ram his asshole faster to reward him; this increased butt-fucking pace seems to have the desired effect, and soon your " + describeCock(player, tent3) + " is throbbing in need as is bathes in the mousey's saliva.");
...
function vapulaAndIzmaThreeSome(player: Character): NextScreenChoices {
    CView.clear();
    if (player.body.cocks.length > 0) return vapulaAndIzma(player, false);
    else return vapulaAndIzma(player, true);
}

// Vapula/Izma threesome(F)
function vapulaAndIzma(player: Character, girls: boolean = true): NextScreenChoices {
    CView.clear();
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(100));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    if (!girls) {
        CView.text("You call your beta tigershark, telling her you'll have some use for her body.  Izma eagerly approaches you, although she frowns a bit when she sees the towering succubus accompanying you.  \"<i>What can I do for my alp-... wait, what are you doing with her?</i>\"  Vapula only smirks at Izma's puzzled reaction.");
        CView.text("\n\nYou gently explain that you intend to have some extra fun, that's why you brought this gorgeous creature to please you both.  Izma isn't really convinced, but still walks up to you, not willing to antagonize her alpha.  You tell to remove her cloth and turn around so you can have a clear view of her toned butt.  \"<i>What do you have in mind, " + player.desc.name + "?</i>\"\n\n\"<i>Shhh... just do as I say.</i>\"  With a discreet gesture, you order Vapula to get on her knees in front of Izma; she's going to feed from your tigershark this time.  With a conniving smile, Vapula obeys and before your shark lover can react, you're already grinding your " + describeCock(player, cockThatFits) + " against her muscular butt-cheeks while the purple succubus grabs hold of her 15 inch long pecker.  You rub the tip of your " + describeCock(player, cockThatFits) + " against her butt crack.  \"<i>W-wait! What are you guys doing? Are you going to fuck my... I don't want to-</i>\"");
        ...
        CView.text("\n\nYour taunts don't seem to affect Izma; if anything, they only turn her on all the more.  You tease Izma's butthole with your hands and " + describeCock(player, cockThatFits) + ", stretching her fuckpillows to make room for your incoming junk.  The poor tigershark doesn't even resist anymore, completely overwhelmed by your teasing ministrations and Vapula's dexterous fondling.  Her shark tail is wagging in apprehension and excitement; her loose pussy is now freely gushing juices and her tight asshole is occasionally leaking some lubricant.  She's finally getting ready!");
        CView.text("\n\nWith a roar of triumph, you grab her toned hips and force your " + describeCock(player, cockThatFits) + " inside her, tearing her asshole and stretching it further.  Izma screeches, obviously not used to anal violation.  Her shark tail twists and wriggles nervously above her ass, nearly hitting you.  With one hand, you immobilize her inconvenient appendage; with your free hand, you grasp again on her toned buttocks, gripping the hard muscles as your herm lover contracts them; her anal walls are squeezing your " + describeCock(player, cockThatFits) + " so tightly it actually hurts.  You tell Vapula to suck harder in order to relieve your tigershark's tension.  Your skilled succubus nods and starts clamping her pink lips around Izma's mammoth prick, effectively trapping it in an organic cock-ring.  At last you feel some of the intense pressure on your " + describeCock(player, cockThatFits) + " being released, allowing you to pull out and slam back, almost bruising your groin as it mashes against her hard butt-muscles.  This time her ass completely gives in to your ferocious dick-assault.  You pull back and stuff her again, increasing the pace of your repeated thrusts as the both of you gain confidence.  Soon you're butt-fucking her senseless, devastating her rectal entrance with your towering rod.");
        CView.text("\n\nNeedless to say, Izma's mind is being torn in pleasure; the sweet blowjob she's receiving from your voracious succubus contrasts with your rough anal ploughing and brings her the weirdest mix of sensation. She growls and moans, her hands grasping Vapula's head as its bobs up and down the length of her long prick.  Her pussy is dripping drop after drop of fem-juice.  You notice her asshole is getting wetter and wetter as you keep penetrating it; could it be that your tigershark lover is a buttslut in denial?  Excited at the idea of turning the prude tigershark into an eager anal beta, you intensify the pace of your butt-fucking, determined to bring her to ecstasy by the sole power of your " + describeCock(player, cockThatFits) + ". You want her to cum from her butt being devastated, certainly not from a petty blowjob, even from a succubus.  Vapula rapidly takes note of your sudden change of rhythm; she grins wickedly and starts taking Izma's whole dickflesh into her mouth, effortlessly deepthroating her.  Izma, being utterly crushed between her two lovers, squirms and thrashes savagely; it won't be long before she cums from your combined throat- and butt-fucking.");
        CView.text("\n\nAs you give your tigershark lover another raging thrust, her groin hits Vapula's face, and the monstrously thick rod bumps against the back of her throat, making her cough unintentionally.  With a triumphal cry, you seize Izma's breasts, kneading them harshly as you pinch her nipples, caring little about her comfort in your fiery passion.  The extra stimulation is enough to make the tigershark cum, and her gaping pussy releases torrents of fem-spunk, soaking the floor with her juices.  Her orgasm is sending shivers all across her body and the way her body is trembling in your arms is enough to send you over the edge: you release a milky flood in her anus, packing her interior with seed; the contact of your goo being shot across her colon triggers her masculine orgasm, and she lets out a torrent of her own spunk into the succubus' waiting stomach, unloading her load into your purple slut as you unload yours into her.  The flow of fluids flowing in and out of her body makes her thrill in ecstasy until your " + describeCock(player, cockThatFits) + " stops vibrating, running out of spooge to shoot. Vapula keeps gluttonously sucking her subsistence out of Izma's traitorously long cock, squeezing her balls in order to extract as much semen as she can.");
        CView.text("\n\nNodding in approval, you pull out your own " + describeCock(player, cockThatFits) + " out of your tigershark's ass and tell Vapula to keep going.  With your rod still in hand, you walk around Vapula and grab hold of her plush, jiggly ass; without warning, you plunge your still-dripping rod into her tight entrance.  Unsurprisingly, the anal penetration is much easier with the purple succubus.  She jolts a bit but this doesn't reduce the intensity of her ministrations in the least; her tainted saliva appears to have an aphrodisiac effect on Izma's cock, never letting her go completely soft.  You pump your " + describeCock(player, cockThatFits) + " in and out of Vapula's plump rump while she still sucks off your tigershark lover, letting her balls fill back up with cum.  The three of you go at it for what seems like hours, Vapula wobbling back and forth between your hard butt-fuck and Izma's dick.  Her asshole is somehow as tight as Izma's, but so wonderfully hot!  The sudden change of temperature is making you shiver in pleasure.  You keep pounding away, not really caring about a slutty creature used to butt-dickings.");
        CView.text("\n\nYou stare at Izma, the two of you keeping eye contact as you both penetrate your fucktoy from both ends.  Then, at last, you wink at Izma as you both cum a second time, releasing simultaneously streams of spunk in the succubus' horny body.  She happily devours all you've got, her lips and anal-lips contracting mercilessly in order to milk both cocks fresh baby-batter.  You cum and cum, your " + describeCock(player, cockThatFits) + " and Izma's pulsating in rhythm as they paint the succubus' inner depths white.  You then pull out of her ass and walk toward Izma.  The poor tigershark is completely drained from the double-blowjob, and staggers her way toward the stream to get hydrated, pulling back her dick with a loud POP as it escapes from Vapula's needy lips.  The succubus looks disappointed to see her new sperm feeder running away, but you remind her your own " + describeCock(player, cockThatFits) + " still needs to be cleaned. With a happy sigh, she works on your junk for a while, tingling you in the most pleasant way as her tongue runs across your dickflesh and slurps all the remaining goo. When you're as empty as she's full, you both rest side to side, recovering from the intense fuck.");
    }
    // Vapula/Izma threesome (as female)
    else {
        CView.text("You call your beta tigershark, telling her you'll have some use for her body.  Izma eagerly approaches you as you slip into your strap-on, only starting to frown when she sees the towering succubus accompanying you.  \"<i>What can I do for my alp-... wait, what are you doing with her?</i>\"  Vapula only smirks at Izma's puzzled reaction.");
        CView.text("\n\nYou gently explain that you intend to have some extra fun, and you've brought this gorgeous creature to please you both.  Izma isn't really convinced, but still walks up to you, not willing to antagonize her alpha. You tell to remove her cloth and turn around so you can have a clear view of her toned butt.  \"<i>What do you have in mind, " + player.desc.name + "?</i>\"\n\n\"<i>Shhh... just do as I say.</i>\"  With a discreet gesture, you order Vapula to get on her knees in front of Izma; she's going to feed from your tigershark this time.  With a conniving smile, Vapula obeys and before your shark lover can react, you're already grinding your obscene dildo against her muscular butt-cheeks while the purple succubus grabs hold of her 15 inch long pecker.  You rub the tip of your strap-on against her butt crack, sighing as the movement sends pleasurable vibrations down into your end.");
        ...
        CView.text("\n\nNodding in approval, you pull the dildo out of your tigershark's ass with a wet pop and tell Vapula to keep going.  With your rod still in hand, you walk around Vapula and grab hold of her plush, jiggly ass and without warning stuff your still-dripping strap-on in her ass; unsurprisingly, the anal penetration is much easier with the purple succubus.  She jolts a bit but this doesn't reduce the intensity of her ministrations in the least; her tainted saliva appears to have an aphrodisiac effect on Izma's cock, never letting her get completely soft.  You pump your dildo in and out of Vapula's plump rump, enjoying the pink end stroking your dripping " + describeVagina(player, player.body.vaginas.get(0)) + " while she still sucks off your tigershark lover, letting her balls fill back up with cum.");
        ...
        CView.text("\n\nShe happily devours all you've got, her lips and nether-lips contracting mercilessly in order to milk both cocks for fresh baby-batter.  The sight of the shameless cumbucket getting what she wants pushes you to a new height and you orgasm and orgasm, your end of the strap-on vibrating furiously in your streaming depths as the other end and Izma pulsate in rhythm and paint the succubus' inner depths white.  You then pull out of her ass and walk toward Izma. The poor tigershark is completely drained from the double-blowjob, and staggers her way toward the stream to get hydrated, pulling back her dick with a loud POP as it escapes from Vapula's needy lips.  The succubus looks disappointed to see her new sperm feeder running away, but you remind her you still need to be cleaned.  With a happy sigh, she works on your " + describeVagina(player, player.body.vaginas.get(0)) + " for a while, tingling you in the most pleasant way as her tongue runs slowly across your drooling sex and slurps up all the girl slime she can possibly tease out of you.  When you're as dry as she is full, you both rest side to side, recovering from the intense fuck.");
    }
    player.orgasm();
    player.stats.sens += -2;
    player.stats.cor += 2;
...
// Feed (as female)
function chixFeedVapulaBlehblehIVantToZuckYourSpooo(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You produce the demonic dildo and hold it by the hilt of its giant purple end, wagging it lazily by your side with a playful grin.  Vapula knows what that means; she is on her knees in an instant, staring at you with wolfish anticipation.");
    // Feed/Tease
    let tease: ClickFunction;
    if (VapulaFlags.VAPULA_HAREM_FUCK === 0) tease = teaseVapula;
    return { choices: [["Feed", chicksFeedVapula], ["Tease", tease]] };
}

// Tease
function teaseVapula(player: Character): NextScreenChoices {
    CView.clear();
    // Requires: Vapula not fucking harem
    if (VapulaFlags.VAPULA_HAREM_FUCK === 0 && (VapulaFlags.VAPULA_DAYS_SINCE_FED === 0)) {
        CView.text("You decide you aren't going to give her what she wants so easily . You're also genuinely curious about how badly she needs cum, and how far you can push her.  You don't say anything as you slowly buckle the strap-on, sighing as you slip the small, pink end into your " + describeVagina(player, player.body.vaginas.get(0)) + ".  You make Vapula wait as you close your eyes and make the dildo pulse inside you, slowly filling you up and then withdrawing, letting it push up against your " + describeClit(player) + ".");
        ...
        VapulaFlags.VAPULA_TEASE_COUNT++;
    }
    // Second Tease
    // Requires: Vapula not fucking harem, teased the day before
    else if (VapulaFlags.VAPULA_HAREM_FUCK === 0 && VapulaFlags.VAPULA_DAYS_SINCE_FED === 1) {
        CView.text("You smile as once again Vapula gets into position on her knees, and you buckle on the strap-on and stand over her.  This time you make the pink end grow long, closing your eyes as it inches up your tunnel, before slowly thrusting your " + describeHips(player) + " backwards and forwards in front of your succubus, letting it shrink and grow so it climbs up and down your wet walls.");
    ...
    // Third Tease
        ...
        CView.text("Once again you buckle up, sighing with pleasure as you sink the pink dildo into your moist hole, and once again you wait until Vapula gets onto her knees before you.  She looks at you silently and sullenly as you droop the monstrous purple end over her face.");
        ...
        // Naga:
        if (player.body.legs.isNaga()) CView.text("you slide your tail under and over her trunk, wrapping your coils around her and drawing her face into your stuffed crotch.");
        // Other:
        else if (player.body.legs.isTaur() || player.body.legs.isGoo() || player.body.legs.isDrider()) CView.text("you sit the monstrous half of your body down comfortably and draw her face into your stuffed cotch.");
        else CView.text("you open your [legs] and wrap them around her head, drawing her face into your stuffed crotch.");

        CView.text("\n\nShe works on it for what seems like hours.  You look down at your slut; she keeps staring at you with avid yet playful eyes, never breaking eye contact as she relentlessly throatfucks herself; you feel her lips curve a little as the shape of a ravenous smile appears on her face, and her eyes are glittering with an insatiable need.  The naughty bitch!  She knows she's going to get what she wants, whether you like it or not.  You wish you were able to control yourself, but through the sheer force of her obscene hunger the succubus is somehow able to control your end of the strap-on, making the dildo shrink, pulsate and push into your " + describeVagina(player, player.body.vaginas.get(0)) + " involuntary, which soon has you panting, groaning, and thrusting back into her to sate your own growing need.");
        CView.text("\n\nYour stamina is rapidly overwhelmed and you decide to reward her effort; you savagely press Vapula's head against your groin, burying all eight inches of your false cock into her waiting throat; in symphony the two of you will your throbbing dildo to expand to pack you mercilessly tight, making you cum brutally.  Your " + describeClit(player) + " pulses as your vagina contracts ecstatically around the bulging sex toy; vaguely you feel release at the other end, and you hear Vapula loudly and shamelessly enjoy what the dildo pumps out.  You abandon yourself to the climax and keep thrusting your " + describeHips(player) + " into her face, girlcum spurting around your harness.");
...
// Feed (as female)
function chicksFeedVapula(player: Character): NextScreenChoices {
    CView.clear();
    // [Teased once or twice:
    if (VapulaFlags.VAPULA_TEASE_COUNT > 1) {
        CView.text("You push the pink end of the dildo into you with a sigh, then strap yourself into the harness so that the monstrous purple end bobs out in front of you.  You stand over a tense Vapula and tease her with it first, lightly brushing her face with the tip for a while.  She swallows thickly and licks her lips as you caress her with your cum fountain, but makes no movements towards it.  Eventually you giggle at her furious concentration and tell her you're going to reward her for being such a good slut, before pushing the end against her lips.  A flood of ecstatic thank-you-mistresses are swiftly muffled out as the dildo disappears into her mouth.");
    }
    else {
        CView.text("You tell Vapula you're going to reward her for being such a good slut; pushing the pink end of the dildo into you with a sigh, you strap yourself into the harness so that the monstrous purple end bobs out in front of you.  You giggle as Vapula grabs hold of it and proceeds to lick it, coating the entirety of your artificial junk with her warm, sweet saliva.");
    }
...
// Forcefeed
export function vapulaForceFeeds(player: Character): NextScreenChoices {
    if (player.body.vaginas.length > 0 && player.inventory.keyItems.has("Demonic Strap-On") && player.body.cocks.length <= 0) {
        CView.text("\n<b>In the night...</b>\nArousal blooms in your dreams, and you are relentlessly pushed into increasingly juicier and filthier fantasies; feeling hopelessly wet, you half awaken and hazily slide a hand down towards your crotch, to try and find release and rest.  Your fingers touch leather and latex where your vagina should be; bewildered, you rise to see Vapula working around your crotch, tightening the final harnesses of your strap-on, locking you into the pink dildo throbbing in your " + describeVagina(player, player.body.vaginas.get(0)) + " whilst she licks hungrily at the tip of the opposite end.  You try to sit up but the ferocious succubus pins you down ruthlessly, her strength increased by her state of hunger.  She smirks at you wickedly and whispers, \"<i>So you don't want me to use any dick but the one I gave you, yet you won't feed me?  That's very bad of you, " + player.desc.name + "... really, really bad...  Do you want to starve me?  Do you want me to beg you for cum?  You naughty slut, you better be ready because I'm going to get what is rightfully mine.  I'm HUNGRY!</i>\"");
        ...
        VapulaFlags.VAPULA_DAYS_SINCE_FED = 0;
        VapulaFlags.VAPULA_TEASE_COUNT = 0;
        VapulaFlags.VAPULA_EARNED_A_SPANK = 1;
    }
    // [if no fuck harem is on and PC hasn't fed/fucked Vapula for 5 days that scene will trigger at night]
        ...
        let cockThatFits = player.body.cocks.find(Cock.CockThatFits(100));
        if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
        CView.text("\n\nYou are suddenly awakened by a gentle tingle at the base of your crotch.  As you open your eyes, you slowly make out in the surrounding darkness a purple shape bobbing up and down leisurely.  You realize that Vapula has profited from your unconsciousness to take your genitals by force!  You try to sit up but the ferocious succubus pins you down ruthlessly, her strength increased by her state of hunger.  She releases your junk with a wet POP and starts licking her lips, slurping back any drop of pre-cum that might have escaped her voracious mouth.  She smirks at you wickedly and whispers, \"<i>So you don't want me to use any dick but yours, yet you won't feed me?  That's very bad of you, " + player.desc.name + "... really, really bad... Do you want to starve me?  Do you want me to beg you for your cum?  You naughty stud, you better be ready because I'm going to get what is rightfully mine.  I'm HUNGRY!</i>\"  With a wolfish groan, she starts sucking you off again, still pinning you down with one hand while the other tickles your crotch.");
        CView.text("\n\nShe works on it for what seems like hours. You look down at your slut; she keeps staring at you with avid yet playful eyes, never breaking eye contact as she relentlessly throatfucks herself; you feel her lips curve a little as the shape of a ravenous smile appears on her face, and her eyes are glittering with an insatiable need. The naughty bitch!  She knows she's going to get what she wants, whether you like it or not.  You wish you were able to control yourself, but the covetous succubus never lets go of your " + describeCock(player, cockThatFits) + ", clinging to it like an animal.");
        CView.text("\n\nYour stamina is rapidly overwhelmed and you decide to reward her effort; you savagely press Vapula's head against your groin, effectively burying all of your " + describeCock(player, cockThatFits) + " in her waiting throat; her tongue keeps teasing your rod for a few seconds, and then you cum, brutally.");
        if (player.body.balls.count > 0) CView.text("  Your " + describeBalls(true, true, player) + " churn");
        else CView.text("  Your " + describeCock(player, cockThatFits) + " twitches");
        CView.text(" as you release a sticky spooge fountain in her stomach.");
        ...
        if (player.cumQ() >= 1500) CView.text(" until she looks 6 months pregnant.  At last, the cum-flow spilling through your urethra starts to ebb and your " + describeCock(player, cockThatFits) + " stops throbbing; only then does she removes your junk from her mouth with a loud POP.  Your tool appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.");
        player.orgasm();
        player.stats.cor += 1;
...
// Requires bipedal-ness
function spankVapulaLikeABoss(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Without any warning, you grab the lusty succubus by the wrists and pull her into you, easily manhandling her surprised form until she's bent over your [leg], vulnerable and squealing like the gutter-slut she is.  Her wings batter at your face and chest, but after a little wrangling, you pin them flat against her back.  The best she can manage now is a few weak twitches at her wing-tips.  Her large, well-formed breasts jiggle and shake as she struggles, but all the trembling purple mountains accomplish is bringing a ready flush to your " + describeSkin(player));
    if (player.body.cocks.length > 0) CView.text(" and a healthy surge of blood to [eachCock]");
    else if (player.body.vaginas.length > 0) CView.text(" and a teltale moistness to your nethers");
    CView.text(".");
    ...
    if (companionsCount() > 2) CView.text(", drawing the eyes of your other companions.");
    else if (companionsCount() === 2) CView.text(", drawing the attention of your other companion.");
    else CView.text(", echoing off the sentinel-like rocks that seem to ring your encampment.");
    CView.text("  Her tail goes wild at the impact, whipping around in a frenzy and actually managing to slap your cheek!  That bitch!");
    ...
    if (player.perks.has(PerkType.Sadist)) CView.text("  Dishing out all this suffering is getting you a little hot under the collar, and you squirm a bit in place in anticipation of the pain to come.");
    ...
    // {+20ish lust}
    player.stats.lust += (10 + player.stats.lib / 7);

    // {Sadist: + 20 lust}
    if (player.perks.has(PerkType.Sadist)) player.stats.lust += (10 + player.stats.lib / 7);

    VapulaFlags.VAPULA_EARNED_A_SPANK = 0;
    return { next: returnToCampUseOneHour };
}
