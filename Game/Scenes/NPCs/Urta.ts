...
import { PregnancyType, Pregnancy, IncubationTime } from "../../Body/Pregnancy/Pregnancy";
...
import { returnToCampUseOneHour, returnToCampUseFourHours } from "../Camp";
import { RaphaelLikes } from "./Raphael";
import { describeBalls, describeSack } from "../../Descriptors/BallsDescriptor";
import { BreastRow } from "../../Body/BreastRow";
import { describeBiggestBreastRow, describeNipple, describeAllBreasts, describeBreastRow } from "../../Descriptors/BreastDescriptor";
import { describeLegs, describeFeet, describeLeg } from "../../Descriptors/LegDescriptor";
import { assholeOrPussy } from "../../Descriptors/BodyDescriptor";
import { describeCocks, describeCocksLight, describeCock, describeCockHead, nounCock, describeOneOfYourCocks } from "../../Descriptors/CockDescriptor";
...
export const UrtaFlags = {
    TIMES_FUCKED_URTA: 0,
    URTA_COMFORTABLE_WITH_OWN_BODY: 0,
    URTA_TIME_SINCE_LAST_CAME: 0,
    URTA_PC_LOVE_COUNTER: 0,
    URTA_PC_AFFECTION_COUNTER: 0,
    NEED_URTA_LETTER: 0,
    URTA_EGGS: 0,
    URTA_FERTILE_EGGS: 0,
    URTA_EGG_FORCE_EVENT: 0,
    URTA_ANGRY_AT_PC_COUNTDOWN: 0,
    URTA_CUM_NO_CUM_DAYS: 0,
    TIMES_RUT_FUCKED_URTAS_CUNT: 0,
    URTA_TENTACLE_GAPED: 0,
    AMILY_VISITING_URTA: 0,
    AMILY_FOLLOWER: 0,
    AMILY_NEED_TO_FREAK_ABOUT_URTA: 0,
    KATHERINE_UNLOCKED: 0,
    UNKNOWN_FLAG_NUMBER_00146: 0,
    UNKNOWN_FLAG_NUMBER_00145: 0,
    URTA_KNOWS_PC_HAS_MARBLE_FOLLOWER: 0,
    URTA_OPEN_ABOUT_EDRYN: 0,
    URTA_FERTILE: 0,
    URTA_PREGNANT_DELIVERY_SCENE: 0,
    RAPHEAL_COUNTDOWN_TIMER: 0,
    UNKNOWN_FLAG_NUMBER_00148: 0,
    UNKNOWN_FLAG_NUMBER_00350: 0,
    UNKNOWN_FLAG_NUMBER_00143: 0,
    URTA_TIMES_EGG_PREGGED: 0,
    URTA_FLATBELLY_NOTICE: 0,
    URTA_X_RAPHAEL_HAPPENED: 0,
    PC_FETISH: 0,
    URTA_PETPLAY_DONE: 0,
    PC_SEEN_URTA_SEX_TOYS: 0,
    MARBLE_NURSERY_CONSTRUCTION: 0,
    MARBLE_KIDS: 0,
    UNKNOWN_FLAG_NUMBER_00147: 0,
    URTA_SCYLLA_BIG_DICK_TIMES_DONE: 0,
    FIRST_TIME_AT_URTA_HOUSE: 0,
    URTA_QUEST_STATUS: 0,
    URTA_PREG_EVERYBODY: 0,
    EDRYN_NUMBER_OF_KIDS: 0,
    DISCUSSED_URTA_ALCOHOLISM: 0,
    URTA_FAMILY_TALK_ATTEMPTS: 0,
    PC_DISLIKES_URTA_AND_EDRYN_TOGETHER: 0,
    KATHERINE_URTA_AFFECTION: 0,
    KATHERINE_URTA_DATE: 0,
    MET_OLIVIA: 0,
};
User.flags.set(FlagType.Urta, UrtaFlags);

export class UrtaTimeAware implements ITimeAware {

    // Urta variables
    // UrtaFlags.TIMES_FUCKED_URTA = times fucked
    // UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY = urtas horsecock comfort level
    // -1 = PC pissed her off
    // 0 = first timern
    // 1-10 = slightly comfortable
    // 11+ = flaunt it
    // UrtaFlags.URTA_TIME_SINCE_LAST_CAME = hours turned off
    // 0 = ready to rock!

    // Urtas horsey bits =
    // LxW = 20x3
    // Luv Lock - UrtaFlags.URTA_PC_LOVE_COUNTER:
    // 0 = normal, 1 = love, -1 = never love, UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY -1 = hate
    /*Luv rating! - UrtaFlags.URTA_PC_AFFECTION_COUNTER
    ...
    */
    ...
    public flagWomb: FlagWomb = new FlagWomb();
    public drainedByKath: boolean;

    public constructor() {
        this.flagWomb = new PregnancyStore(UrtaFlags.URTA_PREGNANCY_TYPE, UrtaFlags.URTA_INCUBATION, 0, 0);
        this.flagWomb.addPregnancyEventSet(PregnancyType.PLAYER, 330, 334, 288, 240, 192, 144, 96, 48);
    ...
    public timeChange(): boolean {
        ...
        this.flagWomb.update();
        // trace("\nUrta time change: Time is " + Time.hour + ", incubation: " + pregnancy.incubation + ", event: " + pregnancy.event);
        if (UrtaFlags.NEED_URTA_LETTER === 1 && Time.hour === 6) urtaPregs.getUrtaLetter(); // Urta Letters
        if (this.flagWomb.isPregnant() && this.flagWomb.pregnancy.incubation === 0 && (this.flagWomb.type === PregnancyType.BEE_EGGS || PregnancyType.DRIDER_EGGS)) {
            this.flagWomb.birth(); // Silently clear Urta's egg pregnancy
            UrtaFlags.URTA_EGGS = 0;
            UrtaFlags.URTA_FERTILE_EGGS = 0;
        }
        if (UrtaFlags.URTA_TIME_SINCE_LAST_CAME > 0) UrtaFlags.URTA_TIME_SINCE_LAST_CAME--; // Count down timer for urta's lust
        if (UrtaFlags.URTA_EGG_FORCE_EVENT > 0) UrtaFlags.URTA_EGG_FORCE_EVENT--; // Countdown to urta freakout
        // Urta egg freak out
        if (UrtaFlags.URTA_EGG_FORCE_EVENT > 0 && Time.hour > 6 && Time.hour < 18 && UrtaFlags.URTA_EGG_FORCE_EVENT < 12) {
            ...
            return urtaChewsOutPC(player, false);
            needNext = true;
        }
        if (UrtaFlags.URTA_ANGRY_AT_PC_COUNTDOWN > 1) {
            UrtaFlags.URTA_ANGRY_AT_PC_COUNTDOWN--;
            if (UrtaFlags.URTA_ANGRY_AT_PC_COUNTDOWN < 1) UrtaFlags.URTA_ANGRY_AT_PC_COUNTDOWN = 1;
        }
        if (Time.hour > 23) {
            if (UrtaFlags.URTA_TIME_SINCE_LAST_CAME === 0) UrtaFlags.URTA_CUM_NO_CUM_DAYS++;
            else UrtaFlags.URTA_CUM_NO_CUM_DAYS = 0;
            if (UrtaFlags.URTA_PC_AFFECTION_COUNTER > 0) {
                UrtaFlags.URTA_PC_AFFECTION_COUNTER -= .5;
                if (UrtaFlags.URTA_PC_AFFECTION_COUNTER < 0) UrtaFlags.URTA_PC_AFFECTION_COUNTER = 0;
            }
        }
        return needNext;
    }
...
const urtaTimeAware = new UrtaTimeAware();
const urtaWomb = urtaTimeAware.flagWomb;

export function urtaSprite() {
    if (urtaDrunk()) CView.sprite(SpriteName.Urta); // 84;
    else CView.sprite(SpriteName.Urta); // 1;
}

export function urtaCapacity(): number {
    let bonus: number = UrtaFlags.TIMES_RUT_FUCKED_URTAS_CUNT * 5;
    if (bonus > 40) bonus = 40;
    if (UrtaFlags.URTA_TENTACLE_GAPED > 0) return 500;
    return 60 + bonus;
}

export function urtaLove(love: number = 0): boolean {
    if (UrtaFlags.URTA_PC_LOVE_COUNTER === -1) return false;
    if (love === 0) {
        return UrtaFlags.URTA_PC_LOVE_COUNTER === 1;
    }
    if (love > 0) {
        UrtaFlags.URTA_PC_AFFECTION_COUNTER += love;
    }
    if (love < 0) {
        UrtaFlags.URTA_PC_AFFECTION_COUNTER -= love;
    }
    if (UrtaFlags.URTA_PC_LOVE_COUNTER === 1) {
        // Queue up Amily madness
        // Only happens if freakout hasn't happened yet.
        if (love > 0 && UrtaFlags.AMILY_VISITING_URTA === 0 && amilyFollower() && UrtaFlags.AMILY_FOLLOWER === 1) UrtaFlags.AMILY_NEED_TO_FREAK_ABOUT_URTA = 1;
        return true;
    }
    else return false;
}

export function urtaAvailableForSex(): boolean { return urtaFuckbuddy() && telAdre.scylla.action !== Scylla.SCYLLA_ACTION_FUCKING_URTA && UrtaFlags.URTA_ANGRY_AT_PC_COUNTDOWN === 0; }

export function urtaFuckbuddy(): boolean { // Returns true if Urta is either the player's fuckbuddy or lover
    if (urtaBusy() || UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY === -1) return false;
    if (urtaJustFriends()) return false;
    return (UrtaFlags.TIMES_FUCKED_URTA > 0);
}

export function urtaJustFriends(): boolean { return UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY === 0 && UrtaFlags.URTA_PC_LOVE_COUNTER === -1; }

export function urtaAtBar(): boolean { // Is Urta physically at the Wet Bitch?
    return (!urtaBusy() && UrtaFlags.AMILY_VISITING_URTA !== 1 && Time.hour < 15 && UrtaFlags.KATHERINE_UNLOCKED !== 3);
}

export function urtaDrunk(): boolean {
    // Preg = no drinking!
    if (urtaWomb.isPregnant()) return false;
    if (UrtaFlags.UNKNOWN_FLAG_NUMBER_00146 === -1) return false;
    if (Time.hour > 12 && UrtaFlags.UNKNOWN_FLAG_NUMBER_00146 < 1) return true;
    if (Time.hour > 8 && UrtaFlags.UNKNOWN_FLAG_NUMBER_00146 >= 1) return true;
    else return false;
}

function urtaOpenAboutEdryn(): boolean {
    // Did it come out from the scylla threesome?
    if (UrtaFlags.UNKNOWN_FLAG_NUMBER_00145 > 0) return true;
    // Did it come out from marble?
    if (UrtaFlags.URTA_KNOWS_PC_HAS_MARBLE_FOLLOWER > 0) return true;
    // Did it come out from discussions?
    if (UrtaFlags.URTA_OPEN_ABOUT_EDRYN > 0) return true;
    return false;
}

export function knockUpUrtaChance(player: Character) { // Moved here from UrtaPregs since it needs to be accessed from outside Urta
    // GTFO IF PREGGO
    if (urtaWomb.isPregnant()) return;
    // GTFO IF NOT FERTILE
    if (UrtaFlags.URTA_FERTILE !== 1) return;
    // 10% + up to 40% @ 1000mLs of cum, with bonus virility!
    let chance: number = 10;
    let temp = player.cumQ() / 25;
    if (temp > 40) temp = 40;
    chance += temp;
    // Bonus virility time!
    ...
    if (chance > randInt(100)) {
        urtaWomb.knockUp(PregnancyType.PLAYER, 384);
        UrtaFlags.URTA_PREGNANT_DELIVERY_SCENE = 0;
    }
}
...
export function urtaBarDescript(): boolean {
    ...
    // Urta PISSED
    if (UrtaFlags.URTA_ANGRY_AT_PC_COUNTDOWN > 1) {
        CView.text("Urta is sitting at a table, swishing a bottle around and looking forlorn.  She glances up and sees you, then immediately turns away.");
        return false;
    }
    // 	[PISSED URTA TALK]
    ...
    // [Post Pissed Talk]
    if (UrtaFlags.URTA_ANGRY_AT_PC_COUNTDOWN === 1) {
        CView.text("Urta is sitting at a table, swishing a bottle around and looking forlorn.  She looks up at you and sighs, \"<i>Look, I still don't think we'll ever be the same, but to be frank, we're great in the sack and I've been lonely.  For now, I'll put the past behind me.</i>\"");
        UrtaFlags.URTA_ANGRY_AT_PC_COUNTDOWN = 0;
        return true;
    }
    // Raphael betrayed reward
    if (UrtaFlags.RAPHEAL_COUNTDOWN_TIMER === -1 && UrtaFlags.UNKNOWN_FLAG_NUMBER_00148 === 0) {
        CView.text("Urta has an ecstatic grin plastered across her muzzle, and it only gets wider when she sees you.  Perhaps you should see what all the fuss is about?");
        return true;
    }
    if (UrtaFlags.UNKNOWN_FLAG_NUMBER_00350 === 1) {
        CView.text("Urta is sitting at her usual table, holding her head in her hands.  She doesn't even seem to notice you.");
        return true;
    }
    // [URTA ASHAMED]
    if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY === -1) {
        CView.text("You see Urta on a bench in the corner with her legs crossed nervously.  Her tail is curling tightly around her leg, hiding the massive surprise between her legs.  In her hand is a mostly empty bottle of whiskey.  She's wobbling back and forth in her seat, and keeping her head down, refusing to meet your gaze any time you spare a glance her way.  It looks like she wants nothing to do with you.");
        return true;
    }
    // Post Scylla Appearance
    if (UrtaFlags.UNKNOWN_FLAG_NUMBER_00145 === 0 && UrtaFlags.UNKNOWN_FLAG_NUMBER_00143 > 0) {
        CView.text("Urta's at her normal table, nursing a glass of water with nary a bottle of alcohol around.  She's fidgety and constantly glancing your way, but she never meets your eyes or acknowledges that she saw you.  Something is eating away at her.");
        return true;
    }
    // PREGNANT URTA
    if (urtaWomb.isPregnant()) {
        if (urtaWomb.pregnancy.type === PregnancyType.PLAYER) {
            urtaPregAppearance();
        }
            ...
            // Egg Level 1:
            if (UrtaFlags.URTA_EGGS < 20) CView.text("Urta is sitting quietly at her usual seat, sipping a mug of some non-alcoholic beer.  Though her clothes are still in place, you can just make out the bulging midriff stretching them taut, a consequence of all the eggs in her womb.");
            else if (UrtaFlags.URTA_EGGS < 40) CView.text("Urta is sitting rather uncomfortably at her usual seat.  Her swollen stomach is so large it keeps her from wearing her usual outfit, instead forcing her into a mini-shirt and a skirt bottom, allowing her midriff to be bare.  She occasionally runs a hand over the expanse, as if she can't believe it's actually there.");
            // Egg Level 3:
            else CView.text("Urta's bloated belly forces her to sit side-saddle at her usual table, the lumpy, egg-stuffed mass ballooning out in front of her.  Forced into a midriff-baring ensemble, she frequently touches it, occasionally seeming to trace an egg through the taut skin.");
        }
        return true;
    }
    // Eggs Laid:
    ...
    if (UrtaFlags.URTA_TIMES_EGG_PREGGED > 0 && !urtaWomb.isPregnant() && UrtaFlags.URTA_FLATBELLY_NOTICE === 0) {
        CView.text("Urta is seated at her usual spot in her usual dress, eagerly chugging down mug after mug of booze; her belly is washboard-flat again, and she's clearly making up for lost time after having given up alcohol for her 'pregnancy'.");
        UrtaFlags.URTA_FLATBELLY_NOTICE = 1;
        return true;
    }
    // [Love Urta Bar Appearance]
    if (UrtaFlags.URTA_PC_LOVE_COUNTER === 1) {
        if (UrtaFlags.UNKNOWN_FLAG_NUMBER_00146 === -1 || !urtaDrunk()) CView.text("Urta is sitting at her usual table, sipping a glass of wine and wearing a form-fitting evening gown of shimmering black.  She looks up at you, happiness filling her eyes when she notices you entering the bar.");
        // (DRUNK)
        else CView.text("Urta is sitting at her usual table wearing a shimmering black dress.  She's pretty clearly sloshed judging by her bleary gaze as she looks up at you.  The front of her dress visibly tents, and she waves you over with a lecherous grin.");
        return true;
    }
    if (UrtaFlags.TIMES_FUCKED_URTA === 0 && UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY > -1) {
        CView.text("You see Urta on a bench in the corner with her feet propped up on a stool.  Her tail is coiled tightly around her left leg, just like every other time you've seen her.  The pretty gray fox gives you a slight nod before turning her attention back to the large square bottle of whiskey in front of her.");
        return true;
    }
    // [URTA FUCKBUDDIED]
    if (UrtaFlags.TIMES_FUCKED_URTA > 0) {
        CView.text("You see Urta on a bench in the corner with her feet propped up on a stool.  She has a small mug on a table, quite different from the swill she chugged when you first met.  Her clothing is... far more risqué.  The top she's chosen for the moment is sheer to the extreme, making her proud black nipples visible to anyone who glances her way, and the short skirt she's wearing is nearly revealing her cunny.  ");
        if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY < 11) CView.text("Thankfully she still has her other addition trapped under her tail, for now.");
        else CView.text("Her half-hard horse-cock is plainly visible to anyone who spares her a glance, barely contained by a pair of tightly woven fishnet stockings.  She's looking at you and wiggling a finger your way...");
        return true;
    }
    // CoC_Settings.error("");
...
export function urtaBarApproach(player: Character): NextScreenChoices {
    urtaSprite();
    // Images are handled by ImageManager now. This was the old way of doing things.
    // showImage(0);
    let temp: ClickFunction;
    let spank: ClickFunction;
    if ((player.body.cocks.length > 0 && player.body.cocks.find(Cock.CockThatFits(urtaCapacity()))) || player.inventory.keyItems.has("Deluxe Dildo")) spank = spankTheShitOutOfUrtaAndMakeHerCreamHerselfFromProstateStimulationAloneLikeTheHornyDrunkenSlutSheReallyIs;
    CView.clear();
    // Raphael Reward
    if (UrtaFlags.RAPHEAL_COUNTDOWN_TIMER === -1 && UrtaFlags.UNKNOWN_FLAG_NUMBER_00148 === 0) {
        UrtaFlags.UNKNOWN_FLAG_NUMBER_00148 = 1;
        player.inventory.gems += 1000;

        CView.image("urta-bar");
        CView.text("Urta claps as you walk up to her with an exuberant smile spread across her vulpine face.  She proudly announces, \"<i>You really turned in the Russet Rogue?!  I almost can't believe it.  Do you know how many women that rascal has fleeced out of their possessions?  The last count I made was over three dozen, and he even had the audacity to try it with me.  I'm ashamed to admit it nearly worked, but once he saw ALL of me, well he couldn't keep up the facade.  I nearly caught him that night.</i>\"\n\n");

        CView.text("You smirk at her tale.  Knowing Raphael, seeing a horny Urta would've made him white as a ghost.  She goes on to list some of his crimes, \"<i>He's guilty of petty theft, disturbing the peace, robbery, lewd public behavior, lying under oath, defacing a public structure, destroying a public structure... I'd probably have to get the paperwork to remember the rest.  In truth his shenanigans aren't usually a big deal individually, but they've added up and garnered him a fairly sizable bounty.  He'll serve his punishment, and I hope he learns his lesson for a change.</i>\"\n\n");

        CView.text("Urta fishes out a rather sizable pouch and tosses it onto the table, the numerous gems inside tinkling loudly as it plops down in front of you.  You untie the silken cord that holds the leather bag closed and see hundreds of sparkling gems, perhaps even a thousand.  Wow!  Raphael must have pissed off the wrong people!\n\n");

        CView.text("The gray fox knocks back a swig from her bottle and sighs as if you've just removed one of the many weights hanging from her shoulders.  She looks back at you with a twinkle in her eyes, and you realize you've definitely improved her opinion of you.  Sadly there isn't time for more chit-chat, and you head back to camp, your gem-pouch heavy with new weight.");
        // (+love score!)
        urtaLove(3);
    ...
    // PREGNANT URTA
    if (urtaWomb.isPregnant() && urtaWomb.pregnancy.type === PregnancyType.PLAYER) {
        urtaPreggoApproached();
        return;
    }
    // HERE WE GOEZ!
    if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY >= 5 && ((player.effects.has(StatusEffectType.Rut) && player.body.cocks.length > 0) || (player.effects.has(StatusEffectType.Heat) && player.body.vaginas.length > 0))) {
    ...
    if (UrtaFlags.URTA_TIME_SINCE_LAST_CAME > 0 && !urtaDrunk()) {
        CView.image("urta-bar");
        // Cockblock marble convo
        if (marbleAtCamp() && UrtaFlags.URTA_KNOWS_PC_HAS_MARBLE_FOLLOWER === 0) {
            UrtaFlags.URTA_KNOWS_PC_HAS_MARBLE_FOLLOWER = 1;
            return marbleCockuBlockuUrta(player);
        }
        // Twu wuv talk
        if (UrtaFlags.URTA_PC_AFFECTION_COUNTER >= 30 && UrtaFlags.URTA_PC_LOVE_COUNTER === 0) {
            return UrtaTwuWuvOffer(player);
        }
        // CView.text("You approach Urta, who gives you a friendly smile and begins talking with you.  Thanks to her unusual endowments, it's easy to tell she's not quite in the mood.  The conversation is still pleasant though, and the two of you knock back a few ales while Urta recounts some of the wilder scenarios she's encountered as the captain of Tel'Adre's guard.");

        return QBsTalkExpack(player);
        // return { next: barTelAdre };
    }
    // [Horny Urta Talk – Lovey]
    if (UrtaFlags.URTA_PC_LOVE_COUNTER === 1) {
        // Sworn off drinking, up till noon, or up to 8 if 'drink more'
        if (UrtaFlags.UNKNOWN_FLAG_NUMBER_00146 === -1 || !urtaDrunk()) {
            CView.image("urta-bar-drunk");
            CView.text("You walk up to Urta and you see her pivot herself to the side.  Her dress visibly tents at the sight of you, and you smirk as you realize she shifted in position to avoid slamming her hardening cock into the bottom of the table.  You cuddle close and wrap an arm around your lover, giving her a quick kiss on the lips and fondling her through her dress.  The tent immediately darkens as a rush of pre soaks into the material, and Urta half-pants, half-asks, \"<i>Oooh, lover, what do you have in mind today?</i>\"\n\n");
            CView.text("You could go back to her place, suck her off under the table, or eat her out under the table.");
            if (player.body.ovipositor.canOviposit()) {
                if (UrtaFlags.URTA_TIMES_EGG_PREGGED === 0) CView.text("\n\nYou contemplate asking Urta to help you take a load off your abdomen, but decide against it; Urta's probably too shy and pent up to let you fill her with your eggs. Maybe when she's gotten herself drunk and is more pliable, though...");
            ...
            // return { choices: [ ["Her Place", goBackToUrtasForLuvinz], ["Suck Off", blowUrtaUnderTheTableLuv], ["Eat Out", eatUrtaOutNomNomPussy], ["", 0], ["", 0],  ] };

            const choices: ScreenChoice[] = [];
            choices[0] = ["Her Place", goBackToUrtasForLuvinz];
            if (UrtaFlags.URTA_CUM_NO_CUM_DAYS >= 5) choices[1] = ["Suck Off", slurpFawkesCocksForFunAndInflation];
            else choices[1] = ["Suck Off", blowUrtaUnderTheTableLuv];
            choices[2] = ["Eat Out", eatUrtaOutNomNomPussy];
        ...
        // [Horny Urta Talk – Drunk]
            ...
            // return { choices: [ ["Jerkoff", getAPublicFacialFromUrta], ["Anal Ride", takeUrtaInTheButtPublically], ["Lay Eggs", temp], ["Spank Her", spank], ["Leave", barTelAdre],  ] };

            const choices: ScreenChoice[] = [];
            choices[0] = ["Jerkoff", getAPublicFacialFromUrta];
            choices[1] = ["Anal Ride", takeUrtaInTheButtPublically];
            choices[2] = ["Lay Eggs", temp];
            choices[3] = ["Spank Her", spank];
            if (UrtaFlags.RAPHEAL_COUNTDOWN_TIMER === -2 && RaphaelLikes(player) && UrtaFlags.URTA_X_RAPHAEL_HAPPENED === 0) {
                ...
                choices[8] = ["3SomeSurprise", urtaAndRaphaelSurprise];
            }
            choices[9] = ["Leave", barTelAdre];

            return { choices };
        }
    }
    // [Approach sloshed Urta 1st time]:
    if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY === 0 && UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY > -1) {
        CView.image("urta-bar-drunk");
        CView.text("Urta smiles broadly and noisily slurps a few dribbles of whiskey from her shiny black lips, watching you approach.  She gives you a smoking-hot 'come-hither' look and slurs, \"<i>I didn't think I'd get a chance to bump into you here, cutie.  Wanna dance?</i>\"\n\n");
        CView.text("Before you can answer, something thumps the table hard enough to shake it.  You look at Urta quizzically, and she shrugs through a blush so bright it's visible through her fur.  The lithe fox squirms in her seat, panting and gasping, \"<i>Ummm, maybe later, I just realized I have to- OH MY GOD WHAT IS THAT?</i>\"\n\n");
        CView.text("You spend a moment looking over your shoulder, but can't for the life of you figure out what she was freaking out about.  She must be really drunk!  You turn back to scold her, but Urta has vanished!  She must have left in a hurry – she left her bottle of \"<i>John Doe</i>\" behind, and still half-full.  The bar's back-door swings closed, maybe she went that way.  What do you do?");

        // (LEAVE HER TABLE) (DRINK HER BOOZE) (BACKDOOR)
        return { choices: [["Backdoor", urtaFollowedOutBack], ["Drink Booze", drinkUrtasBooze], ["", undefined], ["", undefined], ["Leave", barTelAdre]] };
    }
    ...
    if (UrtaFlags.UNKNOWN_FLAG_NUMBER_00146 !== -1 && urtaDrunk()) {
        CView.image("urta-bar-drunk");
        CView.text("The drunken fox roughly gropes you and whispers in your ear, \"<i>I NEED a good fuck right now, and it feels like someone wants a piece of Urta.  I can guess just which piece you're wanting too.  The only question is do I ");
        // CUT FOR NOW CView.text("throw you over the table and fuck you,");
        CView.text("let the beast loose and bounce you on my lap while we drink, ");
        CView.text("jerk off onto your face in front of everyone, or have to remember your ass for later when you bolt for the door.  What'll it be, hun?</i>\"");
        temp = undefined;
        if (player.body.ovipositor.canOviposit()) {
            if (UrtaFlags.URTA_TIMES_EGG_PREGGED === 0) CView.text("\n\nHmm... sounds like she wants to fuck almost as badly as you want to get these eggs out of you.  Besides, didn't Urta say to you once that she's basically barren and sterile?  You'd be doing her a favor by letting her carry your eggs, wouldn't you?  Let's see if she's drunk enough to let you fill her full...");
            temp = giveTheFoxSomeEggs;
    ...
    if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY >= 11 && randInt(2) === 0) {
        CView.image("urta-bar");
        CView.text("You approach Urta, who gives you a naughty wink and asks, \"<i>Would you like to play with my little pony?</i>\"  One of her hands is trailing along her leg, caressing her member as it hardens under her tight fishnet stockings.\n\n(You can suck her off under the table or go back to her place to fuck.)");
    }
    else {
        CView.image("urta-bar");
        CView.text("You approach Urta, who gives you a lascivious grin ");
        if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY < 11) CView.text("moments before you hear her horse-cock thump into the bottom of the table.  ");
        else CView.text("and pets her already-growing horse-cock through her fishnet stockings.  ");
        if (randInt(2) === 0) {
            CView.text("Urta asks, \"<i>I'm really horny, do you think you could help me get off?  I guess we could go fuck at my place, or you could always just help me out under the table.</i>\"  A blush colors her face through the gray fur as she suggests the latter option.");
        }
        else CView.text("She asks, \"<i>Would you like to go back to my place and help me play with my 'little' friend again?  Or maybe you could climb under the table and give me some relief?</i>\"");
    }
    if (player.body.ovipositor.canOviposit()) {
        if (UrtaFlags.URTA_TIMES_EGG_PREGGED === 0) CView.text("\n\nYou contemplate asking Urta to help you take a load off your abdomen, but decide against it; Urta's probably too shy and pent up to let you fill her with your eggs. Maybe when she's gotten herself drunk and is more pliable, though...");
        else CView.text("\n\nYou don't think it's likely Urta would agree to host your eggs in her present state; wait until she's turned down her inhibitions before asking.");
    ...
    // return { choices: [ ["Hidden BJ", blowUrtaUnderTable], ["Urta's Place", goBackToUrtasForLuvinz], ["", 0], ["", 0], ["Leave", barTelAdre],  ] };

    const choices: ScreenChoice[] = [];
    if (UrtaFlags.URTA_CUM_NO_CUM_DAYS >= 5) choices[0] = ["Hidden BJ", slurpFawkesCocksForFunAndInflation];
    else choices[0] = ["Hidden BJ", blowUrtaUnderTable];
    choices[1] = ["Urta's Place", goBackToUrtasForLuvinz];
    choices[4] = ["Leave", barTelAdre];

    return { choices };
}

function drinkUrtasBooze(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("You grab the bottle and take an experimental swig.  WOW, this stuff is STRONG.  It goes down smooth, but leaves a blacksmith's forge burning in your belly.  Kicking back, you take a few more sips, relaxing and enjoying watching the crowds of strangers mingle, flirt, and drink.  You set the bottle down and nearly miss the table, coming dangerously close to falling over.  You eye the bottle and realize it's nearly empty.  Damn that shit was good, but you've got to piss like a racehorse now.  Standing up in a hurry, you wobble around, looking for a bathroom and failing.  A waitress recognizes your expression, and with a knowing smile points towards the back door.\n\n");
    CView.text("You rush out to the alley...");
    player.stats.int += -2;
    player.stats.lust += 30;

    // to part 2!
    return { next: drinkUrtasBoozePtTwo };
}
// [NEXT]
function drinkUrtasBoozePtTwo(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("With a happy sigh you ");
    if (player.body.vaginas.length > 0 && player.body.cocks.length === 0) CView.text("squat down and ");
    CView.text("open up your " + player.inventory.equipment.armor.displayName + ", releasing the pressure onto a patch of grass.  It takes forever to stop, but when it does you feel like a million bucks.  You turn and start to close up your kit when you realize you aren't alone.  Urta is watching you, her lips parted hungrily.  She's still peeing, but what's truly shocking is the mammoth package she's got hanging between her legs.  It's a massive horse-shaft, patterned black and pink, slung down from between a pair of apple-sized balls.  Before your eyes the equine beast begins hardening, forcing the distracted fox to empty the last of her urine against a building's wall.  She blushes furiously, but the drunken fox doesn't seem to care about her shame...\n\n");
    CView.text("She shakes her massive horse-meat more than a few times, shuddering lightly with pleasure as her tip flares a bit in response.  Urta stumbles over moaning, \"<i>This ish all your fault!  You damned... sexy... mmm... letsh fuck!</i>\"\n\n");
    CView.text("The stumbling hermaphrodite is openly rubbing herself as she closes in on you.  What do you do?");
    // [RUN!] [LET HER]
    return { choices: [["Run!", drinkUrtasBoozeRun], ["Let Her", drinkUrtasBoozeLetHer]] };
}

function drinkUrtasBoozeRun(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("You turn about and run, nearly smacking into the door frame in your haste to get away.  You hear a half-strangled sob behind you as the door swings closed.");
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY = -1;
    return { next: returnToCampUseOneHour };
}
function drinkUrtasBoozeLetHer(player: Character): NextScreenChoices {
    urtaSprite();
    player.slimeFeed();
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY++;
    UrtaFlags.TIMES_FUCKED_URTA++;
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME = 5;
    ...
    CView.image("urta-behindbar-fuck");
    CView.text("You nod at Urta, feeling a bit relaxed from all the whiskey and more than a little intrigued by the beast between the slender fox's legs.  She's on you before you know it, her softly furred hand squeezing one of your ");
    if (player.body.balls.count > 0) CView.text(describeBalls(true, true, player));
    else if (player.body.chest.sort(BreastRow.Largest)[0].rating > 1) CView.text(describeBiggestBreastRow(player));
    else CView.text(describeNipple(player, player.body.chest.get(0)) + "s");
    CView.text(", as her tongue spears between your lips.  She practically mouth-rapes you as her trembling member grinds between your " + describeLegs(player) + ", smearing them with a generous helping of foxy horse-pre.  The horny drunk slurs into your ear, \"<i>Gawdsh above, are you this much of a schlut for everyone with one of these?</i>\"\n\n");

    CView.text("With surprising strength, the inebriated fox pushes you against an old bench, flat onto your ");
    if (player.body.chest.sort(BreastRow.Largest)[0].rating > 1) CView.text("tits");
    else CView.text("belly");
    CView.text(".  She pulls at your " + player.inventory.equipment.armor.displayName + " until your crotch is completely exposed and vulnerable, revealing your " + assholeOrPussy(player));
    if (player.body.cocks.length > 0) CView.text(" and " + describeCocks(player));
    CView.text(".  Urta snuggles close, brushing her nose along your thighs before plunging her tongue deep into your " + assholeOrPussy(player) + " in one powerful, drunken lick.  She slurps and licks, humming away until you're ");
    if (player.body.vaginas.length > 0) CView.text("wet");
    else if (player.body.cocks.length > 0) CView.text("suitably lubricated and leaking pre");
    else CView.text("suitably lubricated");
    CView.text(".\n\n");

    CView.text("With a quick shift in position, she forces herself inside, her arms supporting her jiggling chest as she leans over you, dragging her nipples up your back.  Her fat shaft forces your body to spread wide.  The fat head and ring of prepuce strokes your inner walls, squeezing grunts of pleasure from you as the drunken fox forcefully begins fucking you, rutting like a beast.  You can feel her nipples harden, digging into your back and eliciting moans from their owner as they're repeatedly smashed against you.  Urta starts licking your neck where it joins your shoulder, gently nipping at your tender flesh as her tip begins to flare wider inside you.");
    displayStretchVagina(player, 60, true, true, false);
    CView.text("\n\n");

    CView.text("\"<i>Mmmm I shhoulda ushed a condom, you're gonna be a messssss!</i>\" she cries as she climbs atop her sexual peak, mounting it just like she's mounting your " + assholeOrPussy(player) + ".  You're spread wide, the equine member rammed tightly ");
    if (player.body.vaginas.length > 0) CView.text("against your cervix as the tip presses firmly against it");
    else CView.text("into your rectum");
    CView.text(", unloading a thick batch of cream directly into your ");
    if (player.body.vaginas.length > 0) CView.text("waiting womb");
    else CView.text("abused asshole");
    CView.text(".");
    if (player.body.wombs.find(Womb.Pregnant) && player.body.wombs.find(Womb.Pregnant).pregnancy.incubation > 0 && player.body.wombs.find(Womb.Pregnant).pregnancy.incubation < 250) CView.text("  Whatever's growing inside you is probably bathing in the stuff now.");
    else if (player.body.vaginas.length > 0) CView.text("  You briefly wonder if she's capable of knocking you up, and what the baby would look like.");
    else CView.text("  Your backside fills with warmth as her spunk seeps deep inside you.");
    CView.text("  Another blast fills you to capacity, and you're mortified by the sensation of your body being shifted by your growing belly.\n\n");

    CView.text("Urta sucks on the edge of your ear, still pumping away as she pours a bit more spooge into your now equine-sized cavity.  Your alcohol-dulled mind reels as your body caves in to the sticky fluid assault, cumming hard and wriggling around her shaft, moaning repeatedly as you clamp down tightly.  ");
    if (player.body.cocks.length > 0) {
        if (player.body.cocks.length > 1) CView.text("Each of y");
        else CView.text("Y");
        CView.text("our " + describeCocksLight(player) + " squirts hard, unloading a batch of sticky jism under your ");
        if (player.body.wombs.find(Womb.Pregnant) && player.body.wombs.find(Womb.Pregnant).pregnancy.incubation > 0 && player.body.wombs.find(Womb.Pregnant).pregnancy.incubation < 100) CView.text("pregnancy swollen ");
        CView.text("belly, making you squish with each brutal thrust of your oversized lover's rod.");
    }
    else {
        if (player.body.vaginas.length > 0) CView.text("Your pussy dribbles and clenches, leaking Urta's whiteness as she claims it for her own.");
        else CView.text("Your " + describeButthole(player.body.butt) + " clamps down tightly, spasming in pleasure around the invader.");
    }
    if (player.body.cocks.length > 0) CView.text("  As you cum, more and more whiteness dribbles out until a puddle forms underneath you, showing Urta just how much you enjoyed the rough, drunken fuck.\n\n");
    else CView.text("\n\n");

    CView.text("She pulls out, dripping everywhere.  The fox leans down and proceeds to give your still quivering hole a tongue-bath, thoroughly cleaning her fluids from the entrance and teasing you until you clamp down, locking the rest inside.  With a gentle slap on your " + describeButt(player) + " Urta says, \"<i>Thankss for the quick fuck.  Maybe shometime you can ");
    if (player.body.cocks.length > 0) CView.text("stick it in me");
    else CView.text("come back for an even bigger load");
    CView.text(".</i>\"\n\n");
    CView.text("The fox helps you to your feet and the two of you get dressed, stumbling a bit over each other and giggling happily.  Urta gives you a quick peck on the lips and whispers, \"<i>Thanksh... for everything,</i>\" before the two of you make your way back into the bar and go your separate ways.\n\n");
    if (player.body.vaginas.length > 0) displayStretchVagina(player, 60, true);
    else displayStretchButt(player, 60, true);
    return { next: returnToCampUseOneHour };
}

// [BACKDOOR]
function urtaFollowedOutBack(player: Character): NextScreenChoices {
    urtaSprite();

    CView.image("urta-behindbar-masti");
    CView.text("The backdoor opens out into a narrow alley.  Surprisingly, the alleyway is covered in dirt and grass.  Awnings from the buildings above cover the alley in shadow, making it difficult to see anything out here.  You can hear quiet gasps and a wet, repetitive sound.  Quietly approaching the source of the noise, you eventually spot the dark-colored fox.  Her tail is no longer wrapped around her leg.  Instead, it's splayed out behind her and her skirt is pressed up over her legs.  Urta's totally exposed, and it's plain to see that between the fox-girl's legs there's more than a little horse.  Both her hands are wrapped tightly around the quivering column of horse-flesh, stroking the pre-cum dribbling fuck-stick relentlessly as her apple-sized balls bounce under her, occasionally revealing a feminine gash drooling with clear nectar.\n\n");
    CView.text("Urta moans and cries, \"<i>Ooh, just go down! Please cum and go down!  Why won't you go awayyyyyy...</i>\" eventually breaking off into blubbering sobbing, interrupted by the occasional involuntary gasp of pleasure.\n\n");
    // [Sneak away] [Watch] [Masturbate With Her] [Fuck Her]
    CView.text("(You could fuck her, masturbate with her, watch, or leave her like that.)");
    player.stats.lust += 5 + player.stats.lib / 20;

    return {
        choices: [
            ["Fuck Her", urtaFuckHer],
            ["MutualMasturb", dualUrtaMasturbation],
            ["Watch", watchUrtaJerkIt],
            ["", undefined],
            ["Leave", urtaSneakAwayFromMasturbate]
        ]
    };
}

// [Sneak Away]
function urtaSneakAwayFromMasturbate(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("You sneak away from the depressing sight, and decide to head back to ");
...
// [MASTURBATE WITH HER]
function dualUrtaMasturbation(player: Character): NextScreenChoices {
    urtaSprite();
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY++;
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME = 5;

    CView.image("urta-behindbar-masti-dual");
    CView.text("You come out of the shadows, shedding your " + player.inventory.equipment.armor.displayName + ".  ");
    if (player.stats.cor < 60) CView.text("You blush furiously at exposing yourself in such a way");
    else CView.text("Smiling seductively, you revel in exposing yourself so suddenly");
    CView.text(".  Urta looks up at you in shock, blinking tears from her eyes and stammering, \"<i>B-b-b-ut I have th-thish hideoush thing... Aren't you scared of it?</i>\"\n\n");

    CView.text("Gently wiping a tear from her muzzle, you comfort the distraught fox and sit down across from her, giving her a perfect view of your ");
    if (player.body.cocks.length > 0) {
    ...
    else CView.text("bare groin and " + describeButthole(player.body.butt));
    CView.text(".  ");
    if (player.body.cocks.length > 0) {
        CView.text("The situation gives you all the spark you need to rise to your full size, ");
        if (player.body.cocks.get(0).area < 40) CView.text("even if Urta's member dwarfs you.");
        else if (player.body.cocks.get(0).area < 70) CView.text("showing Urta you're just as big.");
        else CView.text("proving to Urta just who has the bigger burden.");
        CView.text("  ");
    }
    if (player.body.vaginas.length > 0) {
        if (player.body.vaginas.get(0).wetness > VaginaWetness.SLICK) CView.text("A trickle of wetness escapes your nether-lips, starting to puddle underneath you.");
        else CView.text("Quickly engorging, your lower lips begin to part around your " + describeClit(player) + ".");
        CView.text("  ");
    }
    CView.text("The heat coursing through you quickly shows itself on your ");
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 1) CView.text(describeAllBreasts(player));
    else CView.text("chest");
    CView.text(" as your " + describeNipple(player, player.body.chest.get(0)) + "s become full and sensitive.\n\n");

    CView.text("Urta relaxes, leaning against a wall as she returns to pleasuring her mismatched member.  Her eyes lock onto your groin as you join her, and she lets out a sigh that's equal parts relieved and inebriated.  Together, the two of you pleasure your bodies, each watching the other's squirming form and blissful expression.  ");
    if (player.body.cocks.length === 1) CView.text("Your hands caress your " + describeCock(player, player.body.cocks.get(0)) + " feverishly, tweaking every ounce of pleasure from the bloated rod.  ");
    if (player.body.cocks.length > 1) CView.text("Your hands caress your " + describeCocks(player) + " in turn, dancing back and forth from rod to rod as you attempt to play with every available pleasure outlet.  ");
    if (player.body.vaginas.length > 0) CView.text("Delving deeply between your legs, you press forward into the source of your feminine pleasures, touching and rubbing, caressing your swollen button as you get closer and closer to orgasm.\n\n");
    if (player.gender === 0) CView.text("Delving down between your legs, rubbing along your taint before you slowly penetrate your " + describeButthole(player.body.butt) + ", pleasuring yourself in the only way possible.\n\n");

    CView.text("The cute fox-girl reaches her peak, crying out in a loud moan that echoes around the alley.  Her equine shaft trembles, the medial ring becoming more pronounced while the flat head flares wide mid-orgasm.  A blast of cum launches into the air, fans out overhead, and splatters down over you.  Urta looks horrified, and yanks herself back, succeeding only in depositing the next blast of spoogey goodness into her own face.  The situation would be comical if the foxy horse-goo wasn't such a great lubricant for your hands, and with that thought in your mind, you cum too.\n\n");

    if (player.body.cocks.length > 0) {
        if (player.cumQ() < 250) {
            CView.text("Jism spurts out onto the alley floor, forming a nice little puddle as ");
            if (player.body.cocks.length > 1) CView.text("each of ");
            CView.text("your " + describeCocksLight(player) + " empties itself.  ");
        }
        else if (player.cumQ() < 500) CView.text("Jism pours out onto the alley floor as your body releases the copious amount of cum it can produce.  It puddles before you, running off the stone into the grass.  ");
        else if (player.cumQ() < 1000) {
            CView.text("Jism erupts from ");
            if (player.body.cocks.length > 1) CView.text("each of ");
            CView.text("your " + describeCocksLight(player) + ", blasting into the air before splattering onto the ground between you and Urta.  It forms a massive puddle, spreading as you pump gob after gob of cum into it until the sticky spooge is soaking your " + describeLegs(player) + " and Urta too.");
        }
        else {
            CView.text("Jism fountains from ");
            if (player.body.cocks.length > 1) CView.text("each of ");
            CView.text("your " + describeCocksLight(player) + ", flooding the small alleyway with an unholy amount of cum.  In no time you've painted Urta and yourself from head to toe, and the alleyway sloshes with the stuff as your production overwhelms the ground's ability to absorb liquid.  ");
        }
    }
    if (player.body.vaginas.length > 0) {
        CView.text("Your sex squishes wetly around a finger, clenching down tightly");
        if (player.body.vaginas.get(0).wetness === VaginaWetness.SLAVERING) CView.text(" and gushing everywhere");
        CView.text(".  ");
    }
    else CView.text("Your sphincter clenches down tightly around your invading digit as you get off, and the rapt attention of your audience certainly helps.  ");
    CView.text("Sighing happily, you give your nipple a tweak as Urta finally finishes cumming, ");
    if (player.cumQ() >= 1000) CView.text("adding to the massive pool on the ground after ");
    CView.text("drenching herself thoroughly.\n\n");

    CView.text("Urta lets out a VERY relieved-sounding sigh and murmurs, \"<i>Oh godsh, thank you.  You don't know how hard it ish to hide thish beasht from EVERYONE.  You're a good friend for helping me with thish... let'sh do thish again sometime, ok?</i>\"\n\n");
    CView.text("She staggers up to her feet and rapidly discards her sodden clothes.  With a resigned sigh, Urta pulls a loose stone out of the side of a building and removes a sheer skirt and diaphanous top from the cubby.  They slip on easily, though the new clothes look a bit more 'slutty' than her previous pair, easily displaying her hard black nipples.  Urta replaces the stone, wobbling a bit unsteadily before she ");
    if (player.cumQ() >= 1000) CView.text("tip-toes through the spunk");
    else CView.text("gets her balance and heads for the bar's backdoor");
    CView.text(".  Her tail curls up around the softening horse-cock, hiding it from view as she blows you a sloppy kiss and disappears inside.\n\n");

    CView.text("You waste no time cleaning up and stagger back inside after her, sated and happy with your new friend and her 'benefits'.");
    player.orgasm();
    return { next: returnToCampUseOneHour };
}

// [FUCK HER]
export function urtaFuckHer(player: Character, afterBefriending: boolean = false): NextScreenChoices {
    urtaSprite();
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY++;
    UrtaFlags.TIMES_FUCKED_URTA++;
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME = 5;
    const cocks: number = player.body.cocks.length;
    if (!afterBefriending) CView.clear();
    // (FUCK HER FEMALE/GENDERLESS)
    if (player.body.cocks.length === 0) {
        CView.image("urta-behindbar-female-fuckurta");
        player.slimeFeed();
        CView.text("You come out of the shadows, shedding your " + player.inventory.equipment.armor.displayName + ".  ");
        if (player.stats.cor < 50) CView.text("You blush furiously at exposing yourself in such a way");
        else CView.text("Smiling seductively; you revel in exposing yourself so suddenly");
        CView.text(".  Urta looks up at you in shock, blinking tears from her eyes and stammering, \"<i>B-b-b-ut I have th-thish hideoush thing... Aren't you scared of it?</i>\"\n\n");

        CView.text("Gently wiping a tear from her muzzle, you answer, \"<i>Not at all, but I think you'd have more luck with a nice warm hole to slide that monster into.</i>\"\n\n");

        CView.text("Urta nods, numbed by the unexpected and forward nature of your offer.  She looks up hopefully and pries her hands away from the twitching behemoth in her loins, waiting to see how you intend to deal with it.  You approach her and turn, straddling your " + describeLegs(player) + " over her, watching her expression as you lower yourself down, ");
        if (player.body.tail.type === TailType.HORSE || player.body.tail.type === TailType.DOG) CView.text("swishing your tail across her taut nipples");
        else CView.text("making her wait");
        CView.text(".  Eventually the blunted tip of Urta's horse-cock presses against your " + assholeOrPussy(player) + ".  Agonizingly, you force yourself to relax and lower yourself down on the mottled pink and black shaft.\n\n");

        CView.text("The horse-cock spreads you wide, filling you past the point of fullness as you sink further and further along its length.  Urta groans happily, her hips twitching involuntarily and forcing more of the monster-sized cock inside you.");
        if (player.body.vaginas.length > 0) {
            if (player.vaginalCapacity() >= 60) {
                CView.text("  You sigh and sink down, taking it until the folds of her sheath are rubbing your entrance.");
            }
            else {
                CView.text("  You nearly cry out as you sink down, taking it until the folds of her sheath are rubbing your entrance, even though it hurts to accommodate it.");
            }
            displayStretchVagina(player, 60, true, true, false);
        }
        else {
            ...
            displayStretchButt(player, 60, true, true, false);
        }
        CView.text("  Urta babbles, \"<i>Oh godsh yesh thank you so much-ohmigod it's SO HOT inside you.</i>\"\n\n");

        CView.text("You smirk over your shoulder and start bouncing up and down on her, slapping her balls with your " + describeButt(player) + " each time you bottom out on the thick shaft.  You can feel her ring of prepuce rubbing your inner walls, pressing on nerves normally ignored.  Urta grunts underneath you and you feel her flare growing wider inside you, stretching you further and further with every bounce atop the equine-endowed slut.  Suddenly warmth is flooding into your ");
        if (player.body.vaginas.length > 0) CView.text("womb");
        else CView.text("rectum");
        CView.text(", and Urta's tongue is lolling out from her muzzle, drooling down the side of her face as she loses herself in orgasm.\n\n");

        CView.text("The substantial flare forms a perfect plug inside you and your belly distends slightly as you're packed full of foxy-horse-cum.  Your own lusts keep your hips moving, grinding and squeezing the shaft inside you as it packs you full of Urta's seed.  Your orgasm explodes through your hips and your " + describeLegs(player) + " give out underneath you, dropping you down onto the fox-girl as the pair of you cum together.  Whiteness begins squirting out around her shaft as her prick begins softening inside you.  You sigh happily, content but too nerveless to stand for the moment.  Peeking over your shoulder and seeing Urta's satisfied expression, you guess she feels about the same.\n\n");

        CView.text("Once the horse-dick inside you has sufficiently softened, you stand up, gasping as a rush of spooge escapes from your " + assholeOrPussy(player) + " now that the 'plug' is removed.  Urta gives you a cocky grin as she's soaked with her fluids, moments before she starts staggering back to her feet.  She wipes up with her clothes and tosses them into the garbage, then wobbles up to a loose brick in the wall and pulls it free, revealing a hidden compartment filled with sets of replacement clothes.  She removes a tight skirt and sheer top, both a bit more revealing than what she wore in the bar earlier, and hurriedly dresses herself.\n\n");

        CView.text("Urta blows you a kiss as you put your own gear back together and flounces back to the bar, calling out, \"<i>We should do thish again shoon!</i>\"\n\n");

        CView.text("\"<i>Yes indeed,</i>\" you muse - the sooner the better.");
    }
    else {
        let cockThatFits = player.body.cocks.find(Cock.CockThatFits(urtaCapacity()));
        const secondCockThatFits = player.body.cocks.filter(Cock.CocksThatFit(urtaCapacity()))[1];
        if (!cockThatFits) cockThatFits = player.body.cocks.get(0);
        CView.image("urta-behindbar-male-fuckurta");
        CView.text("You come out of the shadows, shedding your " + player.inventory.equipment.armor.displayName + ".  ");
        if (player.stats.cor < 60) CView.text("You blush furiously at exposing yourself in such a way");
        else CView.text("Smiling seductively; you revel in exposing yourself so suddenly");
        CView.text(".  Urta looks up at you in shock, blinking tears from her eyes and stammering, \"<i>B-b-b-ut I have th-thish hideoush thing... Aren't you scared of it?</i>\"\n\n");

        CView.text("Gently wiping a tear from her muzzle, you answer, \"<i>Not at all, but I think you'd have more luck with something stuffed in your feminine half.</i>\"\n\n");

        CView.text("Urta gives you a shocked look, but nods and turns around, bending over an old bench and exposing her well-toned ass and glistening, black-lipped sex.  The invitation is all you need.  You wrap your arms around her, squeezing the downy-soft fur on her breasts and leveraging ");
        if (player.body.cocks.length > 0) {
            if (player.body.cocks.length > 2) CView.text("a pair of your " + describeCocksLight(player) + " against her pussy and asshole.  ");
            else if (player.body.cocks.length === 2) CView.text("your " + describeCocksLight(player) + " against her pussy and asshole.  ");
            else if (player.body.cocks.length === 1) CView.text("your " + describeCocksLight(player) + " against her pussy.  ");
        }
        // DOES IT FITZ
        if (cockThatFits.area < urtaCapacity()) {
            CView.text("The wet entrance yields before you, accepting you like a velvet glove.  It grips you tightly, though the grip is meaningless with the flood of lubricant slipping over your shaft.  ");
            if (player.body.cocks.length >= 2) CView.text("Her backdoor was rougher going – the tightly muscled entrance held you back for a moment until Urta relaxed completely, but now the tighter hole is nearly as easy to penetrate as her pussy.  ");
            CView.text("The fox is openly moaning and stroking her disproportional cock, dripping pre-cum everywhere as you do your level best to violate her ");
            if (player.body.cocks.length === 1) CView.text("hole");
            else CView.text("holes");
            CView.text(".\n\n");

            CView.text("She begins grinding against you, begging, \"<i>Oh godsh yeeeessss don't shtop fucking me!  Give me more please!  PLEASE!</i>\"\n\n");

            CView.text("Nodding, you release one of her bouncing breasts in order to smack her back-side.  It barely jiggles, but the surprised moan that slips from the fox-herm's lips is all the encouragement you need.  You release the other breast and grab her hips and ass with both hands, slamming her back against you, only pausing to deliver another smack to one of her cheeks.  Urta wiggles happily, squealing after every slap, trying her best to steady her body while you brutally ");
            if (cocks === 1) CView.text("fuck");
            else CView.text("double-penetrate");
            CView.text(" her.\n\n");

            CView.text("\"<i>Yesh oh yessssssSSSAHHHCUMMING,</i>\" cries Urta as her body goes nerveless underneath you, held up entirely by your hands");

            if (cocks > 1 && secondCockThatFits) CView.text(", " + describeCock(player, cockThatFits) + ", and " + describeCock(player, secondCockThatFits) + ".");
            else CView.text(" and " + describeCock(player, cockThatFits) + ".");
            CView.text("  Both her hands are locked tightly around her behemoth as it unloads onto the alley floor and walls.  The juicy tunnel clamps down on top of you, squeezing your " + describeCock(player, cockThatFits) + " as if Urta's life depended on it.  It's as if every ounce of her being was focusing on clamping down on the invader, massaging, squeezing, and milking it, while the rest of her body goes entirely limp.");
            if (cocks > 1 && secondCockThatFits) CView.text("  Your " + describeCock(player, secondCockThatFits) + " feels like it's about to explode inside her asshole when her sphincter clamps down tightly, acting like the world's tightest cock-ring.");
            CView.text("\n\n");

            CView.text("You fuck her savagely, abusing her spasming hole");
            if (cocks > 1 && secondCockThatFits) CView.text("s");
            CView.text(" with every thrust until your own climax bursts over you like a wave.  Urta's drooling cunt ");
            if (cocks > 1 && secondCockThatFits) CView.text("and tightly clenched ass seem");
            else CView.text("seems");
            CView.text(" to be trying to milk your body of every ounce of cum.  ");

            if (player.cumQ() < 250) {
                CView.text("After a few passionate thrusts, you're spent, and you pull free, admiring the glaze of white that remains on her cunt-lips");
                if (cocks > 1 && secondCockThatFits) CView.text(" and slowly-closing pucker");
                CView.text(".");
            }
            else if (player.cumQ() < 500) {
                CView.text("After a few passionate thrusts, you can feel your cum packing Urta's womb ");
                if (cocks > 1 && secondCockThatFits) CView.text("and gut ");
                CView.text("full of spooge, but all great things come to an end eventually.  You pull free, admiring the glaze of white that remains on her cunt-lips");
                if (cocks > 1 && secondCockThatFits) CView.text(" and slowly-closing pucker");
                CView.text(".");
            }
            else {
                CView.text("After a few passionate thrusts, Urta's womb ");
                if (cocks > 1 && secondCockThatFits) CView.text("and belly are ");
                else CView.text("is ");
                CView.text("so full of spooge that she looks positively pregnant, and you're expelled from her body by the sheer force of your orgasm.  You pause to admire your handiwork, watching whiteness drip from ");
                if (cocks > 1 && secondCockThatFits) CView.text("both her holes.");
                else CView.text("her abused hole.");
            }
            ...
            CView.text("You waste no time cleaning up and you stagger back inside after her, sated and happy with your new friend and her 'benefits'.");
            knockUpUrtaChance(player);
        }
        // (TOO BIG)
        else {
            if (cocks === 1) {
                CView.text("Urta's hole is just way too small to handle what you're packing, so you're forced to back off, much to her lament.  She moans, \"<i>Noooo! Fuck me!</i>\"\n\n");
                CView.text("You pull her down into the grass and throw her on her back.  She looks confused and a little afraid, but that melts away when you spread your " + describeLegs(player) + " across her and begin grinding your " + describeCocksLight(player) + " against her sensitive dick.  Happy, the drunken fox begins pistoning against you, gathering her copious pre and smearing it over your " + describeCocks(player) + ".  The two of you grind together, groin to groin, coating each other in a slippery mess.\n\n");
                CView.text("Urta pants out, \"<i>Ah ahhh ahhhh,</i>\" and cums, her flare ballooning out until it's twice as wide as her shaft, a wave of white jism escaping from her horse-cock.  You gather some in your hand and slather it over your own " + describeCocks(player) + ", and after a few moments of fevered masturbation, you start cumming with her.  Urta happily squirms against you, painting herself with jism and soaking her clothes with the stuff.  ");

                if (player.cumQ() < 250) CView.text("You join in, though your own contribution seems meager by comparison.");
                else if (player.cumQ() < 500) CView.text("You join in, and manage to match her spurt for spurt, until she's lying in a thick puddle of the stuff.");
                else CView.text("You join in, and make her orgasm look like a trickle compared to the flood of spunk you unleash onto her.  By the time the pair of you finish, she's soaked from head to toe, and lying in a nicely sized puddle of the stuff.");
                ...
                CView.text("After a few moments she picks herself up and wipes herself up as much as she can with her sodden clothes.  Urta quickly gives up and removes a loose brick from the wall, revealing a hidden compartment with clothes and a few towels.  It seems she's quite prepared to make a mess in this alley.  She towels the worst of it off and gets dressed in a sheer tank-top and tight skirt before staggering back towards the bar.  Urta pauses and looks over her shoulder to blow you a sloppy kiss before she goes inside.\n\n");

                CView.text("You waste no time cleaning up and mosey back inside after her, sated and happy with your new friend and her 'benefits'.");
            }
            if (cocks > 1) {
                CView.text("Urta's holes are just way too small to handle what you're packing, so you're forced to back off, much to her lament.  She moans, \"<i>Noooo! Fuck me!</i>\"\n\n");

                CView.text("You pull her down into the grass and throw her on her back.  She looks confused and a little afraid, but that melts away when you spread your " + describeLegs(player) + " across her and begin grinding each of your " + describeCocksLight(player) + " against her sensitive dick.  Happy, the drunken fox begins pistoning against you, gathering her copious pre and smearing it over your " + describeCocksLight(player) + ".  The two of you grind together, groin to groin, coating each other in a slippery mess.\n\n");

                CView.text("Urta pants out, \"<i>Ah ahhh ahhhh,</i>\" and cums, her flare ballooning out until it's twice as wide as her shaft, a wave of white jism escaping from her horse-cock.  You gather some in your hand and slather it over your own " + describeCocksLight(player) + ", and after a few moments of fevered masturbation, you start cumming with her.  Urta happily squirms against you, painting herself with jism and soaking her clothes with the stuff.  ");
                if (player.cumQ() < 250) CView.text("You join in, though your own contribution seems meager by comparison.");
                else if (player.cumQ() < 500) CView.text("You join in, and manage to match her spurt for spurt, until she's lying in a thick puddle of the stuff.");
                else CView.text("You join in, and make her orgasm look like a trickle compared to the flood of spunk you unleash onto her.  By the time the pair of you finish, she's soaked from head to toe, and lying in a nicely sized puddle of the stuff.");

                CView.text("The drunken fox groans and continues spurting weakly, \"<i>Oh godsh, letsh do this again sooon.  You don't know how much I needed thish!</i>\"\n\n");
                CView.text("After a few moments she picks herself up and wipes herself up as much as she can with her sodden clothes.  Urta quickly gives up and removes a loose brick from the wall, revealing a hidden compartment with clothes and a few towels.  It seems she's quite prepared to make a mess in this alley.  She towels the worst of it off and gets dressed in a sheer tank-top and tight skirt before staggering back towards the bar.  Urta pauses and looks over her shoulder to blow you a sloppy kiss before she goes inside.\n\n");
    ...
    player.orgasm();
    return { next: afterBefriending ? returnToCampUseFourHours : returnToCampUseOneHour };
}

// [Under Table BJ]
function blowUrtaUnderTable(player: Character): NextScreenChoices {
    urtaSprite();
    player.slimeFeed();
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY++;
    urtaLove(1);
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME = 2 + randInt(4);

    CView.image("urta-bar-bj");
    CView.text("Urta's eyes widen in shock when you suggest taking her under the table.  She stammers, \"<i>Uh, okay...</i>\" and fills a glass with straight whiskey, fueling up on liquid courage while you crawl down.  You slip down under the table, pushing her legs apart");
    if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY >= 11) CView.text(" and pulling down her fishnet to release the monster trapped inside it.  It snaps up immediately, bobbing before your face as it fills with Urta's booze-fueled lust.");
    else CView.text(" and pushing up her tight leather skirt, smiling at the sight of her trembling balls and juicy pussy.  Her tail unfurls around the equine member and it snaps up immediately, hanging before your face.");
    CView.text("  It actually wiggles in time with the beating of her heart, bouncing up as it gets harder and harder, until it smacks your nose.\n\n");

    CView.text("You hear the sound of her shot-glass slamming into the table with authority as she finishes her drink.  A bead of pre-cum bubbles out from Urta's urethra, signaling her readiness.  The horse-like penis gives off a salty scent, unique and not unpleasant.  Before you have a chance to change your mind, you lunge forward, opening your jaw as far as it will go and just barely taking her first few inches into your mouth.  The fox gasps out loud at the pleasure before she remembers to relax.  You can hear her pouring another drink, and in an effort to embarrass her, you begin sucking and licking around the edges of her flare, swallowing the surge of sweet-tasting pre-cum as it begins to fill your mouth.\n\n");

    CView.text("Urta writhes in her seat, noisily spilling alcohol over the table.  You smile around the intruder in your mouth, gripping it with both hands and stroking it with tender caresses.  The gray fox slams down the bottle; though you can't tell if it's in irritation or due to difficulty controlling her muscles, and begins chugging down what she managed to get into her glass.  It seems Urta intends to drink until she doesn't care about any embarrassment you can force upon her.\n\n");

    CView.text("Deciding you've got to get her off before she can drink herself into a stupor, you ");
    if (player.stats.cor < 70) CView.text("do your best to suppress your gag reflex and shove forwards, ramming her cock as deeply into your throat as you can.");
    else CView.text("shove forwards, ramming her massive cock so far down your throat you can feel it dribbling pre-cum into your belly.");

    CView.text("  Urta grunts, doing her best to muffle the noise as it escapes her lips, but you can feel her twitch and thicken inside you, beginning to drip with proof of her complete and total submission to your mouth.\n\n");

    CView.text("You can hear chuckles from the bar's other occupants as they realize what's going on, and you blush, wishing there was a tablecloth to hide your activities from prying eyes.  The distraction does little to divert you from your chosen task, and you close your eyes, tasting your lover's throbbing shaft as it fills your mouth.  With both hands stroking, you bob up and down, completely centering your world around the pulsating horse-prick as it fills your mouth.\n\n");

    CView.text("Without warning, Urta's legs wrap around your back, squeezing with her soft fur as she vices them closed.  You're forced to take her even more deeply, until the folds of her sheath are bumping your nose and chin and filling your nostrils with her heady musk.  You can't even breathe with such a massive intruder blocking your throat, but you do your best to get her off, knowing it's the quickest way to end it.  You suck hard and squeeze her balls, gurgling noisily around the sweaty shaft as you feel Urta give in.\n\n");

    CView.text("A wave-like motion passes through your throat obstruction, stretching you wider than ever before.  It passes, leaving behind a feeling of warm fullness.  You feel another working its way into your mouth, and realize Urta is cumming, filling your belly with direct injections of horsey fox-seed. You desperately need to breathe, but you're trapped on your lover's maleness, and with nothing else to do, you give her twitching balls a squeeze, feeling her body twitch in response as she dumps a particularly heavy batch of spunk into you.  Happy that you could at least control how hard she came, you begin blacking out, barely noticing as Urta's orgasmic contractions die off.\n\n");

    CView.text("Thumping down on your back, you're blissfully aware of the sweet air filling your lungs.  In a moment of reflection, you realize you can taste the salty flavor of Urta's seed on your lips and mouth, and the retracting horse-cock is ");
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 2) CView.text("dragging between your " + describeAllBreasts(player));
    else CView.text("dragging across your chest");
    CView.text(" as it begins to soften.  You wipe as much of her leavings from your " + describeFaceShort(player) + " as you can, licking the slippery glaze from your lips.  Before you get up, you make sure to give Urta's member a firm squeeze, punishment for forcing you to take her so deep.\n\n");

    CView.text("Once you've climbed out from under the table, you're VERY aware of the eyes of some of the nearby bar patrons on your back.  It seems your undercover act managed to draw more than a little attention.  Urta's cheeks burn bright-red under her gray fur, even though her eyes are a bit glassy and unfocused from the amount of alcohol she's ingested.  Sure that she'll remember the embarrassment, you give her a deep kiss, making her taste her residue on your lips.");
    player.stats.sens += 1;
    player.stats.lust += randInt(10) + 5 + player.stats.lib / 10;
...
// [Public Buttfucking]
function takeUrtaInTheButtPublically(player: Character): NextScreenChoices {
    urtaSprite();

    let tooBig: boolean = false;
    if (player.stats.cor < 30 && UrtaFlags.PC_FETISH === 0) {
        CView.text("No way!  You're not going to do that in front of EVERYONE.");
        return { next: barTelAdre };
    }
    player.slimeFeed();
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY++;
    ...
    urtaLove(2);
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME = 5;
    CView.image("urta-public-fuck");
    if (player.stats.cor < 60 && UrtaFlags.PC_FETISH === 0) CView.text("You can't believe you're doing it, but you undo the lower half of your " + player.inventory.equipment.armor.displayName + ".  ");
    else CView.text("You happily remove the lower half of your " + player.inventory.equipment.armor.displayName + ", more than a little excited at the prospect of putting on a show for the crowd.  ");
    CView.text("Urta casually flips up her skirt and ");
    if (UrtaFlags.TIMES_FUCKED_URTA < 11) CView.text("unwinds her tail from around the beast");
    else CView.text("fishes the beast out of her fishnets");
    CView.text(".  Watching in horror, your eyes never leave the equine pillar as it grows to its full size.  Amazingly, none of the bar's patrons seem to notice the trembling horse-cock, even though it's beginning to leak with pre-cum.\n\n");

    CView.text("You carefully shift your position, climbing in front of Urta.  With care, you grab the sensitive member just below the flare and begin rubbing it over your " + describeButt(player) + " and ready ass-hole, lubing yourself up and trying to relax.  The impatient fox grabs you by your " + describeHips(player) + " and pulls you down, forcing herself against your pucker.  ");
    // [GO TOO SMALL OR FITS, NO NEW PG]
    // [FITS]
    if (player.analCapacity() >= 60) {
        CView.text("It's a tight fit, even with a backdoor as gaping as yours.  You're thankful for your looseness as you slide down into her lap, and Urta signals her appreciation of your capacity with a throaty sigh, \"<i>Mmmm, you're good at this.</i>\"\n\n");
    }
    // [DOESN'T FIT]
    else {
        tooBig = true;
        CView.text("It's far too big for you, but that doesn't stop Urta from pushing it inside you.  You can feel yourself stretched around the invader, and it hurts enough that you almost scream out in pain, but inexorably, you're drawn down until your " + describeButt(player) + " is resting on Urta's thighs.  Her thick prick keeps you stretched uncomfortably wide, and you can feel every twitch it makes.");
        // (STRETCH TEXT HERE)
        if (!displayStretchButt(player, 60, true)) CView.text("  ");
        CView.text("Surprisingly, the over-endowed fox notices your discomfort and hands you a narrow bottle from the table, \"<i>Drink up, it'll help with the pain until you get used to me.</i>\"\n\n");

        CView.text("You look at the bottle suspiciously, reading the faded yellow label, \"<i>Barkardi 151</i>\".  Wincing from the pain, you suppose it couldn't hurt and you tip back the bottle, taking a hearty swig.  It burns like hellfire, and you nearly retch up the massive swallow before you get it down.  Amazingly, the pain in your throat and the bonfire in your belly nearly drowns out what's going on in your " + describeButthole(player.body.butt) + ".  You actually start to relax, and Urta nibbles on your ear in between sips of ale.  She holds her mug up to your lips and invites you to share her drink and you do, grateful for any additional pain-killers.\n\n");
    }
    // [BOTH]
    if (player.body.cocks.length > 0) {
        CView.text("You start to grow hard from the pressure inside you, and there's nothing you can do to stop ");
        if (player.body.cocks.length > 1) CView.text("each of ");
        CView.text("your " + describeCocksLight(player) + " from swelling to full erectness.  A bead of pre forms at ");
        if (player.body.cocks.length > 1) CView.text("each ");
        else CView.text("the ");
        CView.text("tip, and begins leaking down the shaft");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(" of your " + describeCocksLight(player) + ", squeezed out from your prostate.  ");
    }
    if (player.body.vaginas.length > 0) {
        CView.text("Heat spreads through your loins as your " + describeVagina(player, player.body.vaginas.get(0)) + " engorges with blood, allowing your " + describeClit(player) + " to ");
        if (player.body.clit.length < 2) CView.text("peep out");
        else CView.text("grow to its full size");
        CView.text(".  ");
        if (player.body.vaginas.get(0).wetness < VaginaWetness.SLICK) CView.text("Moisture beads on your lips, signaling just how turned on this total violation is making you.");
        else if (player.body.vaginas.get(0).wetness < VaginaWetness.SLAVERING) CView.text("Moisture begins dripping onto Urta's thighs, signaling just how turned on this total violation is making you.");
        else CView.text("Moisture puddles on the seat under Urta as your juices flood her thighs, revealing just how turned on this violation is making you.");
        CView.text("  ");
    }
    // (NIPPLES:
    CView.text("Your " + describeNipple(player, player.body.chest.get(0)) + "s tent up under your " + player.inventory.equipment.armor.displayName);
    if (player.body.chest.find(BreastRow.FuckableNipples) && player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier >= 1) CView.text(", leaking milky lubricants and aching to be penetrated.");
    else if (player.body.chest.find(BreastRow.FuckableNipples)) CView.text(", aching to be penetrated.");
    else if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier >= 1.5 && player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier < 2.5) CView.text(", leaking thin streams of milk that rapidly become visible to everyone around you.");
    else if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier >= 2.5) CView.text(", spurting thick streams of milk that soak your " + player.inventory.equipment.armor.displayName + " and are visible to everyone in the bar.");
    else CView.text(".");
    CView.text("\n\n");

    CView.text("Soft-furred hands grip your " + describeHips(player) + " firmly and begin to lift you up, making you gasp as her flare is dragged through your hind-end, eliciting what Urta must think are cute gasps.  You hear a 'murrrr' of contentment in your ear and Urta releases you, twitching inside you as your weight slowly carries you back down into her lap.  Once your " + describeButt(player) + " smacks into her thighs she grabs you again, dragging you back up her massive pole until you're looking down on the tables around you.  She stops and withdraws her support, guiding you back down her shaft and moaning when you take her ring of prepuce half-way down.\n\n");

    CView.text("The eyes of everyone in the place are locked onto you, and ");

    if (tooBig) CView.text("you find yourself giggling drunkenly, barely feeling any pain and arching your back like a slut for the crowd.");
    else if (player.stats.cor < 50 && UrtaFlags.PC_FETISH === 0) CView.text("you find yourself whimpering, regretting the decision to climb on top of her, but aroused in spite of it.");
    else CView.text("you find yourself sliding your hands under your " + player.inventory.equipment.armor.displayName + " and pinching your " + describeNipple(player, player.body.chest.get(0)) + "s, putting on a show for the crowd.");

    CView.text("  Urta starts breathing hard into your ear, and you start flexing your " + describeLegs(player) + " to help her lift you up.  Grateful for the effort, the fox-girl increases the pace by helping to pull you back down each time.  By now it's starting to feel pretty good, and her pre-cum is making your " + describeButthole(player.body.butt) + " so wet and slippery that you can slide quite quickly along the column of horse-meat.\n\n");

    CView.text("Wet squelches echo through the bar as you're slammed into Urta's lap harder and harder.");
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 1) {
        CView.text("  Your " + describeAllBreasts(player) + " are jiggling and bouncing wildly, ");
        if (player.body.chest.find(BreastRow.FuckableNipples)) CView.text("dripping lubricants");
        else if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier > 1) CView.text("spurting milk");
        else CView.text("rubbing your tight nipples against your " + player.inventory.equipment.armor.displayName);
        CView.text(".");
    }
    if (player.body.cocks.length > 0) {
        if (player.body.cocks.length > 1) {
            CView.text("  Steady drizzles of cum begin dripping from each of your " + describeCocksLight(player) + " as she pounds away, rubbing hard against your prostate.");
        }
        else {
            CView.text("  Steady drizzles of cum begin dripping from your " + describeCocksLight(player) + " as she pounds away, rubbing hard against your prostate.");
        }
    }
    CView.text("  You can't hold back, and start moaning quietly, trying your best to be silent and failing horribly.  For her part, your foxy lover seems equally incapable of suppressing her orgasm, and you feel her thick organ flexing inside you as it prepares to cum.\n\n");

    CView.text("Her paws pull you down hard, crushing your " + describeButt(player) + " against her as her sheath presses into your " + describeButthole(player.body.butt) + ".  The invading flesh ripples from the base towards the tip, culminating in a flood of warmth that makes your belly gurgle, and at that moment you cum.  Your sphincter clamps down tightly around Urta's marvelous member, unintentionally providing the fox with an even better orgasm.  The next pulse of cum feels even larger, and you look down in shock when you realize you can see the outline of her shaft on the " + player.body.skin.desc + " of your belly.  As she cums you can see it getting less distinct as your body becomes more and more rounded.  A few blasts later you look positively pudgy, but you can feel her softening inside you.\n\n");

    CView.text("Urta sighs and takes a sip of her drink, giving you a quick peck on the side of your neck.  You start to rise, but the drunken hermaphrodite grabs you and holds you down, \"<i>Let's just stay like this a little longer...</i>\"\n\n");

    CView.text("You shrug and relax, cuddling with her while her massive load is plugged inside you.  She keeps you there for the better part of an hour, kissing you, nipping at your neck, and sharing her drinks with you.  When you finally do get a chance to leave, you're stumbling slightly from the alcohol and your asshole gapes and drips whiteness behind you.  You don't notice the trail of cum until you get back to your camp, and it brings a blush to your cheeks.");
    player.orgasm();
    player.stats.sens += -2;
...
// [PUBLIC FACIAL]
function getAPublicFacialFromUrta(player: Character): NextScreenChoices {
    urtaSprite();
    player.slimeFeed();
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY++;
    urtaLove(2);
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME = 3 + randInt(3);

    CView.image("urta-public-facial");
    CView.text("Given the choices, it seems like just accepting the facial would be the best option.  Urta nods, grinning lasciviously as she pulls the table to the side, and gestures for you to get on the floor.  You kneel before her as she pulls back her ");
    if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY < 11) CView.text("skirt ");
    else CView.text("stockings ");
    CView.text("and allows her horse-dick to flop free in front of you.  It pulses, growing bigger with each beat of the fox's heart as she watches you submit yourself to her arousal.  You feel the eyes of everyone around you as they watch the spectacle ");
    if (player.stats.cor < 33) CView.text("and it makes you blush crimson");
    else if (player.stats.cor < 66) CView.text("and it makes you uncomfortable but horny");
    else CView.text("and it makes you randy as hell");
    CView.text(".\n\n");

    CView.text("Two dexterous fur-covered hands wrap around her mottled shaft and begin pumping it.  Almost immediately the flattened-tip begins flaring wide and produces a dollop of fragrant equine pre-cum.  Urta's balls quiver happily as she plays with herself, and as you look up at her and open wide, ");
    if (player.stats.cor < 75) CView.text("doing your best to play your part in the scene.");
    else CView.text("hoping to encourage her to absolutely soak you with cum.");
    CView.text("\n\nUrta looks down with a surprisingly shy expression and asks, \"<i>Could you... lick the tip please?  For me?</i>\"\n\n");

    if (player.stats.cor < 50) CView.text("Hesitantly, you extend your tongue and lick the underside of her flat-tip.  ");
    else CView.text("Eagerly, you lick around the outside edge of her flared tip, sticking your tongue out as far as you can to lick the underside once you finish.  ");
    CView.text("Your efforts are rewarded when Urta groans and spurts a dollop of pre-cum onto your tongue, and you can see her urethra twitching and pulsating as more of the clear fluid begins to leak out.  It drips down onto your tongue as the fox picks up her pace, feverishly masturbating her bloated shaft.  She says, \"<i>Swallow that pre and get ready slut, you'll need an empty mouth to hold all this.</i>\"\n\n");

    CView.text("You swallow, tasting the sweet pre-cum as it slides down your throat.  Risking a glance to the side, you realize there's a crowd gathering around you, and more than a few dog-dicks and horse-cocks are being stroked.  ");
    if (player.stats.cor > 50) CView.text("You hope the show will be arousing enough to get them off on you as well, and with renewed determination you turn back to Urta and give her balls a squeeze.");
    else CView.text("You shudder in shame, and hope that the crowd will disperse. It's humiliating enough waiting to have Urta soak you with cum.");
    CView.text("  She trembles and her tip flares wide, and you know she's starting to cum.\n\n");

    CView.text("Her urethra dilates before your eyes and time seems to slow down as the first blob of horse-semen is launched towards you.  Blinking at the last second, you feel it splatter over your " + describeFaceShort(player) + ".  You open your mouth wider, keeping your eyes closed but knowing you're expected to swallow some of it.  Urta's aim improves with the next blast, and it does land in your mouth.  You nearly gag from the sudden salty intrusion, and due to the volume of the seed a fair portion of it leaks out the sides of your mouth as you keep it open, expecting more.  You feel more cum land in your hair, then on your cheek, and finally on your neck and ");
    if (player.body.chest.sort(BreastRow.Largest)[0].rating < 1) CView.text("chest.  ");
    else CView.text("cleavage.  ");
    CView.text("Urta paints you with the stuff, until you're practically oozing cum.  It does eventually stop, and you wipe the jism from your eyes");
    if (player.stats.cor > 60) CView.text(", being sure to lick your fingers clean afterwards");
    CView.text(".\n\n");

    if (player.stats.cor > 50) {
        CView.text("You could probably get the crowd to cover the rest of you.  Do you?");
        return { yes: optionalUrtaBukkake, no: declineUrtaBukkake };
    }
    else {
        CView.text("You blush and try to clean yourself up.  An embarrassed barmaid forces her way through the crowd to give you a towel, and starts shooing away the assembled masturbators.  You thank her and towel off Urta's thick seed, being more than a little aroused by the scene you caused.  The fox-girl is leaning back looking VERY satisfied as she ");
        if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY >= 11) CView.text("stuffs herself back into her fishnet");
        else CView.text("curls her tail back around her member, concealing it from view");
        CView.text(".");
        player.stats.lust += 5 + randInt(5) + player.stats.lib / 10;

    }
    return { next: returnToCampUseOneHour };
}

function optionalUrtaBukkake(player: Character): NextScreenChoices {
    urtaSprite();
    player.slimeFeed();
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME -= 2;
    urtaLove(-1);

    CView.image("urta-bukkake");
    CView.text("You pause to tear off your " + player.inventory.equipment.armor.displayName + " and expose the rest of your body, and you reach out into the crowd, grabbing the nearest cock and caressing it as you beg, \"<i>Come on and cum, she missed most of me!</i>\"\n\n");
    CView.text("Urta leans back with a smile, toying with her softening member as a ring of horny studs and herms encircles you, jerking themselves off furiously.  You take turns touching and caressing them, murmuring enticing words as they start cumming on you, one or two at a time.  As those that finish step away, they're replaced by newcomers, turned on by the cum-hungry slut in front of them.  You do your best to greet them with a wink, a stroke, and a slurp, and then you present your ");
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 1) CView.text(describeAllBreasts(player));
    else CView.text(describeNipple(player, player.body.chest.get(0)) + "s");
    CView.text(" to a horny wolf just in time to take his hot load on your chest.\n\n");
    player.stats.lust += 15 + randInt(5) + player.stats.lib / 10;

    CView.text("An hour later you're totally drenched in cum.  It's dripping off you from your hair down to your " + describeFeet(player) + ", and you love it.  You lick up the mixed spooge until you start to feel full, and a blushing barmaid comes forward to hand you a towel.  You give her a salty kiss on the lips, sending her running away before you towel off and dress yourself.");
    return { next: returnToCampUseOneHour };
}
function declineUrtaBukkake(player: Character): NextScreenChoices {
    urtaSprite();
...
function goBackToUrtasForLuvinz(player: Character): NextScreenChoices {
    urtaSprite();

    CView.image("urta-home-image");
    if (UrtaFlags.URTA_PC_LOVE_COUNTER === 1) {
        CView.text("Urta gives you a warm smile and grabs you by the hand, pulling you out the Wet Bitch's backdoor and into Tel'Adre's back-alleys.  Her dress does little to conceal the bulging mass underneath or the growing wet spot that forms at the tip of her impressive tent.  The long, body-hugging garment makes it difficult for Urta to walk in her current state, and you don't make it any easier for her, constantly teasing the fox as you navigate the back-alleys of Tel'Adre.\n\n");

        CView.text("Every chance you get, you brush a hand or a hip against her sensitive nether-bulge or lust-tightened nipples.  For someone with such a bad-ass reputation, Urta's quite easy to tease, and she makes adorable gasps and shuddering shakes every time you brush against her flare or surprise her with a tender caress.  She lets out cute whimpers as she shuffles onward, but she doesn't protest as her 'male' half drenches her dress with more and more slimy pre-cum.\n\n");
        CView.text("The door to her apartment looms before you and you push her inside.  You do your best to stifle an involuntary snicker as you close the door and see a trail of Urta's moist arousal on the street right up to the door.  The apartment is impeccably clean, though you see a few large sex-toys half-hidden about the place.  For someone as ashamed of her desires as Urta was, she sure is terrible at hiding her kinks.");
        if (UrtaFlags.URTA_PETPLAY_DONE === 0 && urtaLove()) CView.text("  <b>A new, shiny leather collar lies half-concealed under a cushion, as though its owner haphazardly tried to bury it in a rush.</b>");
        CView.text("  The busty fox has already retreated from the front-room, the half-soaked dress on the floor the only indication she was there with you a moment ago.\n\n");
        CView.text("You follow a trail of pre-cum on the floor and the scent of copious female arousal deeper into the apartment, around a corner, and into Urta's relatively small bedroom.  Your vulpine lover is reclined on her bed, wearing nothing but lacy stockings and a latex cock-sheath.  ");
        if (player.body.cocks.length > 0) CView.text("In her hand is another sheath, ready to gird your own equipment in a fashion similar to her own.  ");
        return { next: urtaHomeLuvLuvinsMenu };
    }
    CView.text("Urta smiles broadly and gives you a grateful peck on the cheek.  She makes like she's going to rise, then stirs uncomfortably, her eyes flashing down towards her crotch.  The fox settles back down and asks, \"<i>Would you mind paying off my tab while I slip out of here?  You've gotten me a bit too exposed to stand around trying to pay,</i>\" as she fishes out a coin-purse.  Tossing a few gems and odd coins your way, your over-endowed lover gives you a longer kiss before pushing you away from the table.\n\n");
    ...
    CView.text("You assure her that you aren't offended");
    if (player.stats.lib > 50 || player.stats.cor > 50) CView.text(" and are actually turned on by the thought of walking through the city with such a 'big' lover hanging off your arm");
    CView.text(".  Urta smiles weakly and hooks her arm through yours, leading you towards her apartment, making sure to stick to the more abandoned back-roads.  Her caution doesn't stop the pair of you from giving a few free shows, and if anything, each brief encounter with a passerby seems to be making her bigger and harder, revealing a half-buried exhibitionist streak.\n\n");
    return goBackToUrtasForLuvinzII(player);
}

function goBackToUrtasForLuvinzII(player: Character): NextScreenChoices {
    CView.text("The journey is blessedly brief, and within minutes you arrive at Urta's house.  She fiddles with the lock anxiously for a moment before finally getting a -CLICK- and unlocking it.  You sweep inside behind your host and look around, mildly shocked at the spartan surroundings.  Urta's apartment is tiny, cramped, and nearly devoid of possessions beyond weapons, armor, and sex-toys.");
    if (UrtaFlags.PC_SEEN_URTA_SEX_TOYS === 0) {
        CView.text("  The last group is the most numerous of those three.");
        if (PlayerFlags.giacomo > 0) CView.text("  You recognize some of the toys as Giacomo's wares, though there are a fair number that seem to be custom made, and others that are clearly living creatures.");
        CView.text("  Looking over the things she had to use before she met you, it dawns on you just how lonely the poor fox has been.");
    }
    else CView.text("  While the last group used to be the most numerous, it looks like Urta has cleaned house and only has a few of the toys left.  It seems that since meeting you she hasn't had to turn to her massive device collection like she used to.  It dawns on you just what a difference you've made in Urta's life, and it brings a loving smile to your face.");
    CView.text("\n\n");

    CView.text("\"<i>Sexy, are you going to come get some?</i>\" Urta calls out from the bedroom in the back.  You slap your forehead, angry for getting so distracted while a hot piece of ass is on the line.  Heading deeper into the apartment after her, you turn the corner to her bedroom and are floored by the sight.  Urta is totally nude save for a pair of black fishnet stockings and some kind of shining latex cock-sheath.  She's on all fours on top of a fur-lined bed, and wiggling her hind-end at you with her tail lifted in the air.  The scent of need pours off from her sex, and you find yourself as ");
    if (player.body.vaginas.length > 0 && player.body.cocks.length > 0) CView.text("hard and wet as you've ever been");
    else if (player.body.cocks.length > 0) CView.text("hard as you've ever been");
    else if (player.body.vaginas.length > 0) CView.text("wet as you've ever been");
    else CView.text("ready as you've ever been to take such a monster");
    CView.text(".\n\n");

    CView.text("Urta is clearly looking to take a more passive role, and she begins stroking herself as she asks, \"<i>How do you want to fuck, lover?</i>\"");
    // [M: VAGINAL] [F: RIDE] [GENDERLESS/M: RIDE ANAL]
    let vaginal: ClickFunction;
    let femaleRides: ClickFunction;
    if (player.body.cocks.length > 0) vaginal = dudeFuckingUrtasCooch;
    if (player.body.vaginas.length > 0) femaleRides = rideUrtaTenderFemale;
    // return { choices: [ ["Fuck Vagina", vaginal], ["Ride (Vaginal)", femaleRides], ["Ride (Anal)", tenderTakeItUpTheAssFromUrta], ["No Condoms", condomlessUrtaInHouseSmex], ["", 0],  ] };

    const choices: ScreenChoice[] = [];
    choices[0] = ["Fuck Vagina", vaginal];
    choices[1] = ["Ride(Vaginal)", femaleRides];
    choices[2] = ["Ride(Anal)", tenderTakeItUpTheAssFromUrta];
    choices[3] = ["No Condoms", condomlessUrtaInHouseSmex];
    if (player.body.legs.isGoo() && player.body.skin.type === SkinType.GOO) choices[4] = ["Goo (Weird)", urtaGooTesticleVoreRuinedOrgasms];

    return { choices };
}

function rideUrtaTenderFemale(player: Character): NextScreenChoices {
    urtaSprite();
    UrtaFlags.PC_SEEN_URTA_SEX_TOYS++;
    urtaLove(1.5);
    UrtaFlags.TIMES_FUCKED_URTA++;
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY++;
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME = 3 + randInt(2);

    CView.image("urta-home-female-ride");
    CView.text("You climb into bed and playfully roll Urta over.  Her tits and horse-cock flop and jiggle from the change in position, and you watch mesmerized for a moment until Urta giggles girlishly, drawing you out of your reverie.\n\n");

    CView.text("\"<i>Do you like them that much?</i>\" she asks.\n\n");

    CView.text("You nod ");
    if (player.stats.cor < 50 || player.stats.lib < 50) CView.text("sheepishly ");
    else CView.text("eagerly ");
    CView.text("and grab ahold of her latex-wrapped member, giving it gentle strokes along the underside.  Urta moans loudly and twitches her hips, already eager for more, and the ");
    if (player.body.vaginas.get(0).wetness < VaginaWetness.WET) CView.text("growing moistness in your loins");
    else if (player.body.vaginas.get(0).wetness < VaginaWetness.DROOLING) CView.text("sopping wetness in your loins");
    else CView.text("flood of moisture leaking from your crotch");
    CView.text(" signals your readiness to take her.");
    if (player.body.cocks.length > 0) {
        if (player.body.cocks.length === 1) CView.text("  Your " + describeCocksLight(player) + " is raging hard and trembling, but ignored for now.");
        else CView.text("  Each of your " + describeCocksLight(player) + " is raging hard and trembling, but ignored for now.");
    }
    CView.text("  Climbing atop the bed with her, you swivel your " + describeHips(player) + " about, positioning her at your entrance and straddling your " + describeLegs(player) + " around her.\n\n");

    CView.text("You can see a bubble already forming at the tip of her hardness, signaling the unbridled lust your form has brought to your lover and you lower yourself with a sigh, feeling it spread your lips apart with delicious slowness as her flared tip begins penetrating you.  The horse-cock is so big and thick that it ");
    if (player.vaginalCapacity() < 60) {
        CView.text("hurts quite a bit as it pushes further and further inside.  ");
        displayStretchVagina(player, 60, true);
    }
    else {
        CView.text("stretches you nice and wide, rubbing you tightly even though you're quite stretched already.  ");
        displayStretchVagina(player, 60, true);
    }
    CView.text("You gasp and sigh happily, sinking downwards slowly.\n\n");

    CView.text("The ring of prepuce passes past your entrance ");
    if (player.vaginalCapacity() < 60) CView.text("and you bottom out there, unable to take in any more of her equine shaft.");
    else CView.text("and you keep going, sliding down along the marvelous member until your cunt-lips are rubbing her sheath against your [clit].");
    CView.text("\n\n");

    CView.text("Urta growls and grabs you by the back of the neck, pulling your " + describeFaceShort(player) + " down for a sloppy kiss.  Her smooth canine-like tongue forces its way into your mouth and curls around yours as the two of you swap spit.  Your hips, spurred on by the passion, begin to rise and fall to the tempo of the kiss, and in seconds you're both gasping and moaning against each other's lips, lost in the throes of passion.  Deep inside you, you can feel the condom filling with Urta's incredibly copious pre-cum, but every time your hips drop down the pressure squeezes some of it down along her length until it escapes around the condom's edge, dripping into her sheath.\n\n");

    // (Big Tits:
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 5) {
        CView.text("Your " + describeBreastRow(player.body.chest.get(0)) + " mash into the fox's own, your " + describeNipple(player, player.body.chest.get(0)) + "s ");
        if (!player.body.chest.find(BreastRow.FuckableNipples)) CView.text("rubbing over her black-skinned buds.");
        else CView.text("parting as her prominent black nipples slip inside your own.");
        CView.text("  You squeal at the sudden pleasure, arching your back and furthering the sensation from your over-burdened chest.");
        if (player.body.cocks.length > 0) {
            if (player.body.cocks.get(0).length > 15) {
                if (player.body.cocks.length === 1) CView.text("  Your " + describeCocksLight(player) + " pushes up between the jiggling breasts, slicking them with your pre-cum as it begins to get tit-fucked by both of your trembling forms.");
                else CView.text("  Each of your " + describeCocksLight(player) + " pushes up between the jiggling breasts, slicking them with your pre-cum as they begin to get tit-fucked by both of your trembling forms.");
            }
        }
        CView.text("\n\n");
    }

    CView.text("You both cry out in orgiastic pleasure as your shaking " + describeHips(player) + " force your tightly-stretched cunt over Urta's rubber-wrapped horse-prick again and again.  Your internal muscles spasm, clamping down tightly in sudden orgasm.  Urta's voice trembles in a low moan of satisfaction while her lower body twitches upwards with hard pumping motions.  Her dick trembles inside you, and you feel the latex around her girthy prick inflating with her seed and pressing against your cervix.\n\n");
    if (player.body.cocks.length > 0) {
        if (player.body.cocks.length === 1) {
            CView.text("Your own " + describeCocksLight(player) + " cums as well, blasting seed over both your ");
            if (player.body.cocks.get(0).length > 15 && player.body.chest.sort(BreastRow.Largest)[0].rating >= 5) CView.text("jiggling tits");
            else CView.text("bellies");
            CView.text(", glazing them in slippery goo.");
        }
        else {
            CView.text("Each of your own " + describeCocksLight(player) + " cums as well, blasting seed over both your ");
            if (player.body.cocks.get(0).length > 15 && player.body.chest.sort(BreastRow.Largest)[0].rating >= 5) CView.text("jiggling tits");
            else CView.text("bellies");
            CView.text(", glazing them in slippery goo.");
        }
    ...
    CView.text("Lying down on top of her, the two of you cuddle while joined at the hips, snuggling happily");
    if (player.body.cocks.length > 0) {
        CView.text(" and ignoring the hot, wet mess your " + describeCocksLight(player) + " ");
        if (player.body.cocks.length === 1) CView.text("has");
        else CView.text("have");
        CView.text(" left between your nude bodies");
    }
    CView.text(".  The fox-girl gives you a long, happy kiss and holds you tightly in her arms before she lets you go and begins to pull out.  The loud -SCHLLLIIIICK- of her fat member being dragged through your nethers makes you blush, and the feeling of it robs you of any strength below the waist.  The latex stretches, the cum-bubble still trapped inside you and stretching the condom obscenely for a moment, until it jerks out with a sudden 'pop'.  The relief is palpable, but your " + describeVagina(player, player.body.vaginas.get(0)) + " hangs open, with lips parted loosely after today's incredible violation.\n\n");

    CView.text("Urta sighs and languidly pulls off her condom, nearly getting cum everywhere before she ties it off.  It looks like it'll pop at any moment – there's probably over a liter of cum stored in it.  She hefts it and mocks like she's going to throw it at you, but stops and giggles before setting it down beside the bed.  Your foxy lover says, ");
    if (UrtaFlags.TIMES_FUCKED_URTA <= 10 || UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY < 11)
        CView.text("\"<i>Lovemaking is so much of a hassle to clean up if I don't use one of these!  At least I don't have to use them for the same reason as most of Tel'Adre.  It turns out I'm sterile or damn close to it, so all that fun-juice is just for show.</i>\"");
    else CView.text("</i>\"<i>You're so good to me " + player.desc.name + ".  I wish my little pony here was fertile enough for me to put a bun in your oven, if you wanted of course.</i>\"");
    ...
    CView.text("You nod before you leave, feeling satisfied and a warm glow from something other than lust.");
    // if(UrtaFlags.URTA_FERTILE == 1) player.body.womb.knockUp(new Pregnancy(21, 515), 80);
    player.orgasm();
    player.stats.sens += -2;
...
// [FUCK HER VAGINA]
function dudeFuckingUrtasCooch(player: Character): NextScreenChoices {
    urtaSprite();
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(urtaCapacity()));
    if (!cockThatFits) cockThatFits = player.body.cocks.get(0);
    const secondCockThatFits = player.body.cocks.filter(Cock.CocksThatFit(urtaCapacity()))[1];
    if (cockThatFits.area > urtaCapacity()) {
        return fuckUrtasVagButTooBig(player);
    }

    CView.image("urta-home-male-fuck");
    const cocks: number = player.body.cocks.length;
    UrtaFlags.PC_SEEN_URTA_SEX_TOYS++;
    UrtaFlags.TIMES_FUCKED_URTA++;
    urtaLove(1.5);
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY--;
    if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY < 1) UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY = 1;
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME = 2;

    CView.text("You let her know that you want her just the way she is and climb into bed behind her.  She watches you apprehensively over her shoulder as you pull out your " + describeCocksLight(player) + " and start lining yourself up to fuck her doggie-style.  ");
    if (cocks > 1 && secondCockThatFits) CView.text("Your secondary " + describeCock(player, secondCockThatFits) + " wobbles directly behind her back-door and Urta's eyes widen a bit when she realizes you're going to double-penetrate her.  ");
    if (cocks > 2) CView.text("The remainder of your " + describeCocksLight(player) + " fan out, ready to rub themselves against her butt-cheeks once you push inside.  ");
    CView.text("You can see your lover's condom starting to bloat at the tip as her horse-cock begins to flare within its confinement.  She trembles with anticipation before taking a deep breath and saying, \"<i>I'm ready... take me!</i>\"\n\n");

    CView.text("Obligingly, you ease your hips forward, reeling at the heat of her cunt-lips as they welcome your " + describeCock(player, player.body.cocks.get(0)) + "'s " + describeCockHead(player.body.cocks.get(0)) + " with slippery moisture.  ");
    if (cocks > 1 && secondCockThatFits) CView.text("Your second " + nounCock(secondCockThatFits.type) + " soon bumps against her pucker and strains against it, pushing ever-so-slowly into her tight hole as she gradually relaxes before you.  ");
    CView.text("As you carefully breach the opening");
    if (cocks > 1 && secondCockThatFits) CView.text("s");
    CView.text(", the going gets easier and easier for you.  You gently rock your [hips] forwards, sliding deep inside her until she's taken you to the hilt.  Once you're fully embedded within her, Urta wriggles happily beneath you, and you reach around to caress her drooping member, relishing in her pleasured yelps and moans.\n\n");

    CView.text("You rock back and forth, picking up the tempo gradually.  At first the gray fox is only gasping happily, but as you go faster and faster, her gasps turn into quiet moans, then transform into ecstatic squeals.  ");
    if (player.body.balls.count > 0) CView.text("Your balls loudly smack into her squelching-wet cunt and the soaked backside of HER balls");
    else CView.text("Your crotch slaps loudly against her squelching-wet cunt and backside");
    CView.text(" as you fuck her hard and fast.  Urta's cock twitches in your hand, and the pre-filled bubble hanging from her cock-tip is getting close to apple-sized.  Both of you are rutting like animals and riding on the edge of an orgasm.  Then, Urta throws her head up, arches her back, and starts howling like a well ridden bitch in heat.  Now that she's moving with you, her magnificent horse-cock fully engorges inside your grasping hand, and you slide it between your grasping fingers, masturbating her even while fucking her.\n\n");

    CView.text("Urta cums first.  Her cries of pleasure are muffled as she shoves her face into a pillow, screaming in ecstasy.  The warmth around your " + describeCock(player, cockThatFits) + " seems to grow that much hotter as it squeezes you, wringing you tightly from root to tip.  ");
    if (cocks > 1 && secondCockThatFits) CView.text("The intensity and pressure in her back-door is even worse, and your " + describeCock(player, secondCockThatFits) + " is nearly in pain from her asshole's tight, pulsating grip.  ");
    CView.text("Her equine member trembles in your grasp as it releases its sticky payload into the thick, latex condom.  You pump away at her shaft, feeling the spunk inside distort the tube-like appendage until your orgasm shocks you into releasing it.\n\n");

    CView.text("You spurt into Urta's sloppy box, creaming it until her juices begin to run in an off-white color.  ");
    if (cocks > 1 && secondCockThatFits) CView.text("The " + describeCock(player, secondCockThatFits) + " you've crammed in her ass does not disappoint either, matching its brother squirt for squirt as it spends your seed with reckless abandon.  ");
    if (cocks > 2) CView.text("Her butt and tail are covered with the stuff as each of your remaining " + describeCocksLight(player) + " splatters her with spooge, covering her fur in white sludge.  ");
    CView.text("The fox's internal muscles happily continue to squeeze and milk you, prolonging your orgasm beyond its normal duration.");
    if (player.cumQ() >= 750) CView.text("  Her body is quickly bloated thanks to your body's incredible fertility, but she seems able to take it just fine.");
    CView.text("\n\n");

    CView.text("The two of you collapse together in a shuddering pile of post-orgasmic bliss.  The shift in position frees your " + describeCocksLight(player) + " from Urta with a noisy -SHHHLLUUUCK-, but you're too wiped out to care.  The fox-girl sighs happily and rolls over, planting a kiss on your nose before she works at removing her over-filled condom.  It nearly bursts when she ties it off, but she manages to gently set it down to the floor without incident. She looks at you happily and gives you a big hug, crushing her hard nipples against ");
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 1) CView.text("your own " + describeBreastRow(player.body.chest.get(0)));
    else CView.text("your chest");
    CView.text(" as she says, \"<i>Thank you so much, lover.  I forget how good it can be in my... vagina.  Maybe we could do this again?</i>\"\n\n");

    CView.text("You give her a happy nod, and the two of you cuddle a bit longer before you leave to return to ");
    player.orgasm();
    player.stats.sens += -2;
...
function fuckUrtasVagButTooBig(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("You let her know that you want her just the way she is and climb into bed behind her.  She watches you apprehensively over her shoulder and objects, \"<i>I'm sorry lover, but that's WAYYYY too big to fit in any hole I've got.  Maybe I should go ahead and take you?  Or would you rather help me off under the table back at the bar?</i>\"");
    // [GET BUTT FUCKED] [GET CUNT-FUCKED] [BAR BJ] [Fuck it I'm leaving!]
    let vaginal: ClickFunction;
    if (player.body.vaginas.length > 0) vaginal = rideUrtaTenderFemale;
    return {
        choices: [
            ["Ride Vaginal", vaginal],
            ["Ride Anal", tenderTakeItUpTheAssFromUrta],
            ["Bar BJ", blowUrtaUnderTable],
            ["No Condom", condomlessUrtaInHouseSmex],
            ["Leave", fuckItAndLeave]
        ]
    };
}

function fuckItAndLeave(player: Character): NextScreenChoices {
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY--;
    if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY < 1) UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY = 1;
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY--;
    if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY < 1) UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY = 1;
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME = 36;
    urtaLove(-5);

    CView.text("You let Urta know that you've changed your mind and will be leaving.  You go before she can object and the sound of quiet sobbing chases you out into the streets.");
    return { next: returnToCampUseOneHour };
}

// [RIDE HER COWBOY/NEUTER]
function tenderTakeItUpTheAssFromUrta(player: Character): NextScreenChoices {
    urtaSprite();
    ...
    UrtaFlags.TIMES_FUCKED_URTA++;
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY++;
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME = 2 + randInt(3);
    const cocks: number = player.body.cocks.length;
    CView.image("urta-home-anal");
    CView.text("You give Urta's backside a playful slap and ask her to roll over.  She does so with a hesitant look on her face, wondering what you're about.  When you spread her legs and work your way forward to straddle her, a beaming smile breaks across her face and she says, \"<i>You really want me inside you, don't you " + player.desc.name + "?</i>\"\n\n");

    CView.text("Her cheeks color when you give her a nod and begin lowering your " + describeButt(player) + " towards her pillar of horse-flesh. She gasps quietly when the tip begins to push against your " + describeButthole(player.body.butt) + ".  ");
    if (player.analCapacity() >= 60) CView.text("You sigh and begin sliding down it, your previously-stretched backdoor easily accepting the gigantic intruder as it fills you.");
    else {
        CView.text("You grunt and begin sliding down it, feeling painfully stretched by the gigantic intruder but pressing on anyway, knowing full-well you'll be a bit looser by the time you take her whole length.");
        if (player.body.butt.looseness < 5) {
            if (player.body.butt.looseness === 0) CView.text("<b>  Urta has taken your anal virginity.</b>");
            else CView.text("<b>  Your ass is stretched.</b>");
            player.body.butt.looseness++;
        }
    }
    // Butt Stretched used to determine how long since last enlargement
    if (!player.effects.has(StatusEffectType.ButtStretched)) player.effects.add(StatusEffectType.ButtStretched, 0, 0, 0, 0);
    // Reset the timer on it to 0 when restretched.
    else player.effects.get(StatusEffectType.ButtStretched).value1 = 0;
    if (player.body.cocks.length > 0) {
        CView.text("  The bumps and veins keep rubbing against your prostate, and ");
        if (cocks > 1) CView.text("each of ");
        CView.text("your " + describeCocksLight(player) + " hardens near instantly from the forced stimulation.");
    }
    CView.text("  Slowly but surely, it slips further and further inside you, until you've bottomed out and have miraculously taken her entire shaft.\n\n");

    // (IF DICKS)
    if (cocks > 0) {
        CView.text("For a few moments you sit on her deeply penetrating thickness, and ");
        if (cocks > 1) CView.text("each of ");
        CView.text("your " + describeCocksLight(player) + " droops onto her belly, half-hard from the pressure on your prostate, but going a little soft from the lack of attention.");
        if (player.body.cocks.get(0).length >= 15) {
            CView.text("  Urta winks and guides ");
            if (cocks > 1) CView.text("one of ");
            CView.text("your " + describeCocksLight(player) + " between her tits");
            if (player.body.cocks.get(0).length >= 20 && player.body.cocks.get(0).length <= 28) CView.text(", even taking time to lick and suck on the head");
            CView.text(".");
            CView.text("  She begins giving you a slow tit-fuck, trying to encourage you to pick up the pace and start riding her.  It does the trick as your " + describeHips(player) + " start rocking on their own in reaction to her efforts.  She licks harder, and you reward her by lifting yourself up and sliding back down.  Her massive girth squeezes a squirt of pre-cum from your prostate with every few strokes, splattering it on her muzzle.");
        }
        CView.text("\n\n");
    }
    else {
        CView.text("For a few moments you sit like that, trying to adjust to the pressure in your backside.  After a few moments you can feel your muscles relaxing, allowing Urta to rest more comfortably inside you.  Flexing your " + describeLegs(player) + ", you pull yourself up off of her until the flared tip is just barely inside you, then you plunge back down.  Urta's mouth hangs open and her tongue lolls out, and you can feel the warmth of her leaking fluid inside of you, letting you know just how good your " + describeButthole(player.body.butt) + " feels to her.\n\n");
    }
    CView.text("You bounce on top of your fox-lover's equine rod, spearing your " + describeButthole(player.body.butt) + " on it over and over.  ");
    if (cocks === 0) CView.text("Strange pleasure spreads from your sensitive opening, and you know that somehow you're going to cum from this.  ");
    else CView.text("Powerful pleasure spreads through you as it bumps and rubs your prostate. You know you'll cum from this, and soon.  ");
    CView.text("Urta cries out and grabs you by the hips, yanking you down against her sheath.  Warmth blossoms inside you as her seed begins pumping into your bowels.  You can feel yourself filling up with her seed and you cum, clamping down around her intruder as it injects more and more foxy-horse-spunk into you.\n\n");

    // (DICKS)
    if (cocks > 0) {
        if (player.body.cocks.get(0).length < 15) CView.text("Your orgasm erupts out onto her belly, splattering her fur with your sticky cum.  ");
        else if (player.body.cocks.get(0).length < 20) CView.text("Your orgasm erupts all over her tits, slicking them with your sticky white cum.  ");
        else CView.text("Your orgasm erupts into her muzzle, filling her mouth with your sticky, white cum.  ");
        CView.text("  Urta groans underneath you and begins caressing and squeezing you, milking you of every drop.");
        if (player.cumQ() >= 500) CView.text("  Urta gulps down as much as she can but it begins leaking around her muzzle, and she eventually lets it pop free, giving up on swallowing it entirely.");
        if (player.cumQ() >= 1000 && player.cumQ() < 1500) CView.text("  Your output easily soaks her, and she gathers it up and slops it over her nipples, pinching and pulling on them.");
        if (player.cumQ() >= 1500) CView.text("  Your output soaks her and the bed with seed, leaving the two of you in a warm gooey puddle in the very center of the bed.");
        CView.text("\n\n");
    }
    CView.text("Urta pulls you down onto her breasts and gives you a long hug and a happy lick.\n\n");

    CView.text("\"<i>Thank you soooo much lover, I needed that,</i>\" she says, \"<i>Now you better go get back to your business, I've got to get cleaned up!");
    if (player.cumQ() > 750) CView.text("  You really are a messy, " + mf(player, "boy", "girl") + " aren't you?");
    CView.text("</i>\"\n\n");
    CView.text("She untangles herself from you, the filled end of her condom loudly popping free from your backside as she disengages herself from you.  The two of you do your best to clean up and get dressed, and you give her a quick but passionate kiss before ducking out the door to check up on your ");
    player.orgasm();
    player.stats.sens += -2;
...
function marbleCockuBlockuUrta(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("Urta gives you a serious look when you come up to her, \"<i>I just got a report in from some of our scouts last night.  It looks like they found your   There was a cow-girl there");
    // if nursery
    if (UrtaFlags.MARBLE_NURSERY_CONSTRUCTION === 100) CView.text(" and a nursery");
    CView.text(".");
    // (Not wuv'ed yet)
    if (UrtaFlags.URTA_PC_LOVE_COUNTER === 0) {
        CView.text("  I know we aren't a couple or anything, but, is she something to you?  Are you lovers?</i>\"\n\n");
        CView.text("Urta's cheeks color sheepishly and she says, \"<i>You don't have to answer, but whatever the truth is, you can tell me.</i>\"\n\n");
    }
    // (WUVED)
    else {
        CView.text("  I know our paths keep us apart most of the time... and multiple lovers are pretty far from uncommon, but I need to know – do you two have something going on?</i>\"\n\n");
        CView.text("Urta's looking at you nervously, clearly dreading your answer.  How do you respond?\n\n");
    }
    CView.text("Do you answer honestly, refuse to answer, or lie?");
    // [Honestly] [Refuse] [Lie]
    return { choices: [["Honestly", TellUrtaDAHTRUUUUF], ["Refuse", trufftrufftrufftruff], ["Lie", lietoUrtaAboutMarble]] };
}

// [Honestly]
function TellUrtaDAHTRUUUUF(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("You explain to Urta about how you became addicted to Marble's milk at the farm, and how despite the harshness of the untamed lands around the both of you, you formed a loving relationship.  Hearing the truth and honesty backing your words, Urta's eyes water, but the saddened fox-girl refuses to look away.  Once you've finished, she dabs away a tear and takes a swig of what you hope is ale.  She wipes the foam from her muzzle with the tear-stained rag and speaks with a husky, grief-laced voice, \"<i>I... appreciate your honesty.  Some part of me was afraid you would try to lie.  Yes, I'm a little disappointed I'm not the only one to warm your bed, but telling me the truth was the right thing to do, and ");
    if (UrtaFlags.URTA_PC_LOVE_COUNTER === 1) CView.text("I love you for it");
    else CView.text("I respect you for it");
    CView.text(".");
    if (!urtaOpenAboutEdryn()) {
        CView.text("  I suppose while we're clearing out skeletons, I should mention I've had relations with Edryn a few times since we've ");
        if (UrtaFlags.URTA_PC_LOVE_COUNTER === 1) CView.text("been together");
        else CView.text("gotten intimate");
        CView.text(".  I'm ashamed to admit I can't really control myself that well either.</i>\"\n\n");
        CView.text("Do you comfort her, tell her to toughen up and remind her she's great in the sack, or walk out in a huff?");
        ...
        return { choices: [["Comfort", comfortUrtaAfterTellinTruuf], ["Toughen Up", toughenUpUrtaAfterTellingTruuf], ["Walk Out", walkoutOnUrtaAfterTellingTruff]] };
    }
    else {
        CView.text("</i>\"");
...
// [Comfort]
function comfortUrtaAfterTellinTruuf(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("You let Urta know that you understand.  After all, in a world like this monogamy is next to impossible.  There's no shame in what she did with Edryn or your relationship with Marble.  The two of you share a heartfelt hug and resolve to continue your relationship.");
    // (If PC has Edryn (<3) & fits her req's)
    if (((player.body.legs.type === LegType.CENTAUR && player.body.cocks.length > 0) ||
        player.body.cocks.filter(Cock.FilterType(CockType.HORSE)).length > 0 ||
        (player.body.cocks.length > 0 && player.stats.cor > 50)) &&
        player.effects.get(StatusEffectType.Edryn).value1 >= 4)
        CView.text("  Urta even suggests that the two of you take turns at Edryn sometime.");
    // Slight love loss
    urtaLove(-4);
    return { next: returnToCampUseOneHour };
}
...
// [Toughen Up]
function toughenUpUrtaAfterTellingTruuf(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("You look at Urta and explain that she shouldn't take it too hard.  Both of you got into this scenario without expecting anything more from it.  You explain that though she is a good friend and GREAT in the sack, you don't see your relationship going to that level.  Urta seems ");
    if (UrtaFlags.URTA_PC_LOVE_COUNTER === 1) CView.text("heartbroken");
    else CView.text("saddened");
    CView.text(" by the news");
    if (UrtaFlags.URTA_PC_LOVE_COUNTER === 1) {
        CView.text(", and she has a defeated look in her eyes.");
        // (Luv set to 0 and blocked if in luv,
        UrtaFlags.URTA_PC_AFFECTION_COUNTER = 0;
        UrtaFlags.URTA_PC_LOVE_COUNTER = -1;
    }
    else {
        CView.text(", but you spy a determined look in her eyes.");
...
// [Walk Out]
function walkoutOnUrtaAfterTellingTruff(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("You push away from the table and turn your back on Urta, enraged at her for cheating on you with Edryn, even though you cheated on her as well.  You ignore the hypocrisy in your actions and walk out, shattering Urta's heart in the process.  As you close the door of 'The Wet Bitch' behind you, you see Urta, sobbing into a hanky while Edryn goes over to comfort her.  It figures.");
...
// [Refuse]
function trufftrufftrufftruff(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("You refuse to answer the question, telling Urta that what happens outside the walls has no impact on what the two of you have here.  She disagrees as her fairly meek attitude slips away, replaced with a sterner expression, \"<i>No.  It matters to me.  I want to know about your life and if you won't even answer... then you aren't as kind as I thought you were.</i>\"\n\n");
    CView.text("\n\nIt looks like you've have to tell her the truth or lie...");
    // [Truth] [Lie]
    return { choices: [["Truth", TellUrtaDAHTRUUUUF], ["Lie", lietoUrtaAboutMarble]] };
}

// [Lie]
function lietoUrtaAboutMarble(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("You let Urta know that the poor cow-girl needed a place to stay from the monsters that roam the wilds, and that your camp is the safest place.  In great detail you talk about sheltering her, but never taking advantage of her, and trying your best to do what's right.  Through the whole conversation Urta dabs at tears, and you assume them to be tears of relief.  The story suitably complete and air-tight, you finish and look at Urta.\n\n");
    CView.text("\"<i>I misjudged you, " + player.desc.name + ".  I didn't think you'd lie to me, and you did.  The scouts talked to her");
    if (UrtaFlags.MARBLE_KIDS === 1) CView.text(" and saw the kid");
    if (UrtaFlags.MARBLE_NURSERY_CONSTRUCTION > 1) CView.text(" and saw the kids");
    CView.text("!  I know you two are an item, and I can't believe you didn't own up to it!</i>\" she cries, \"<i>You better believe I'm fucking pissed at you.  I could've accepted sharing you, but lying to me like this...I won't ever trust you again.  Just...just go – I might be able to overlook this but it hurts too much right now.</i>\"\n\n");
    CView.text("There's nothing to do but go.  You're not sure if Urta will ever let you see her again.");
    // (Urta pissed for 48 hours, then make-up scene.  Love forever blocked)
    UrtaFlags.URTA_PC_LOVE_COUNTER = -1;
    UrtaFlags.URTA_PC_AFFECTION_COUNTER = 0;
    UrtaFlags.URTA_ANGRY_AT_PC_COUNTDOWN = 48;
    return { next: returnToCampUseOneHour };
}

// [TWU WUV TALK]
function UrtaTwuWuvOffer(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("Urta looks nervously at you, fidgeting and shifting her sizable addition under the table.  She starts talking meekly, but her voice rises in confidence and intensity as she goes, \"<i>I've been thinking " + player.desc.name + ", we've known each other for a while now.  When we're together, I don't have to worry about someone seeing something they shouldn't, or being made fun of, because it doesn't bother you and you don't care.  And what do I have to care about if you don't care about it?</i>\"\n\n");
    CView.text("She leans over and puts her hands on your shoulders, looking you in the eye as she continues, \"<i>I've never felt this comfortable around ANYONE before, and well, I think I love you.  Please, do you feel the same way about me?</i>\"\n\n");
    CView.text("Given how much time you've spent with her, and the amazing sex, her declaration doesn't really surprise you.  Do you love her back?");
    // [Yes] [No]
    return { yes: TwuWuvIsBeautifulUrta, no: noUDontLoveUrta };
}
// [No]
function noUDontLoveUrta(player: Character): NextScreenChoices {
    urtaSprite();

    // (LUV BLOCKED)
    UrtaFlags.URTA_PC_AFFECTION_COUNTER = 0;
    UrtaFlags.URTA_PC_LOVE_COUNTER = -1;
    CView.text("You gently remove her hands from your shoulders and let Urta know that you enjoy being around her, but that you aren't in love with her.  Her eyes choke up with tears, but she nods as you explain, wiping her eyes with a napkin.  Urta cries softly and murmurs, \"<i>...I didn't want to hear that, but I understand.  Do you want to keep on as we have been though?</i>\"\n\n");
    // [Yes] [No]
    return { yes: stayFuckbuddiesAfterShootingDown, no: turnDownFuckbuddiesANDLove };
}
// [yes]
function stayFuckbuddiesAfterShootingDown(player: Character): NextScreenChoices {
    urtaSprite();
    CView.text("Urta gives you a warm smile and a kiss on the cheek, \"<i>Good.  Come see me soon, ok?</i>\"\n\n");
    // (READY TO GO NEXT TIEM)
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME = 0;
    return { next: returnToCampUseOneHour };
}
// [no]
function turnDownFuckbuddiesANDLove(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("Urta bursts into tears and pushes back from the table.  She runs out of the bar, sobbing wildly.  It looks like you've totally burned that bridge.  You may as well head back to ..");
    // (BURN BRIDGES MOFO)
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY = -1;
    return { next: returnToCampUseOneHour };
}
// [Yes]
function TwuWuvIsBeautifulUrta(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("You nod and break into a wild smile.  You lean forward and the two of you embrace in a warm hug as you admit to loving the shy fox-herm.  She pulls back and kisses you passionately on the lips, and the pair of you spend an hour cuddling contentedly in a dark corner, happy to have taken things to the next level.  It passes in a flash, but you know you need to get back to check on camp, and you leave her with another quick kiss on the lips.  You " + mf(player, "chuckle", "giggle") + " when you hear her cock harden and thunk against the table again behind you.  Oh Urta...");
    // set wuv fwags
    UrtaFlags.URTA_PC_LOVE_COUNTER = 1;
    return { next: returnToCampUseOneHour };
}
...
function drinkUrtasCumGlass(player: Character): NextScreenChoices {
    ...
    CView.text("You give her a good look at the full glass and tip it back, gulping down her thick goop, letting it run out of the corners of your mouth.  Urta looks on, dumbfounded while you guzzle her remaining seed from the cup.  You set it down and burp, leaning over to give her a wet kiss, letting her taste both her sexes on your tongue.  She breaks the kiss and says, \"<i>Oh by Marae, I love you " + player.desc.name + ".  Thank you so much.</i>\"\n\n");
    CView.text("You smile knowingly and leave, intent on cleaning up a little back at ");
    player.stats.sens += 1;
...
export function blowUrtaUnderTheTableLuv(player: Character): NextScreenChoices {
    ...
    CView.text("Deciding you've got to get her off and make her spill her drink, you ");
    if (player.stats.cor < 50) CView.text("do your best to suppress your gag reflex and shove forwards, ramming her cock as deeply into your throat as you can.");
    else CView.text("shove forwards, ramming her massive cock so far down your throat that you can feel it dribbling pre-cum into your belly.");
    CView.text("  Urta grunts, doing her best to muffle the noise as it escapes her lips, but you can feel her twitch and thicken inside you, beginning to drip with proof of her complete and total submission to your mouth.\n\n");

    CView.text("You can hear chuckles from the bar's other occupants as they realize what's going on, and you blush, ");
    if (player.stats.cor < 50 && UrtaFlags.PC_FETISH <= 0) CView.text("wishing there was a tablecloth to hide your activities from prying eyes");
    else CView.text("wishing you had even more eyes watching your show");
    CView.text(".  The distraction does little to divert you from your chosen task, and you close your eyes, tasting your lover's throbbing shaft as it fills your mouth.  With both hands stroking, you bob up and down, completely centering your world around the pulsating horse-prick as it fills your mouth.\n\n");
    ...
    CView.text("Thumping down on your back, you're blissfully aware of the sweet air filling your lungs.  In a moment of reflection, you realize you can taste the salty flavor of Urta's seed on your lips and mouth, and the retracting horse-cock is ");
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 1) CView.text("dragging between your " + describeAllBreasts(player));
    else CView.text("dragging across your chest");
    CView.text(" as it begins to soften.  You wipe as much of her leavings from your " + describeFaceShort(player) + " as you can, licking the slippery glaze from your lips.  Before you get up, you make sure to give Urta's member a firm squeeze, scolding it for its forceful treatment of your mouth.\n\n");
    CView.text("Once you've climbed out from under the table, you're VERY aware of the eyes of some of the nearby bar patron's on your back.  It seems your undercover act managed to draw more than a little attention.  Urta's cheeks burn bright-red under her gray fur, though she has a loving smile on her face.  The pair of you share a tender kiss to hoots and catcalls from the audience.  Urta smiles and gushes, \"<i>You give the BEST blowjobs... I mean, WOW, but maybe next time we should go back to my place and do something a little more... fulfilling for both of us.</i>\"");
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY++;
...
// [RIDE VAGINAL]
function rideUrtasCoochLoveyDovey(player: Character): NextScreenChoices {
    urtaSprite();
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME = 2 + randInt(2);
    urtaLove(1);

    // (TOO SMALL)
    if (player.vaginalCapacity() < 40) {
        CView.text("Urta smiles at first, but as you strip down and get ready her expression turns to one of care and worry.  She apologizes, \"<i>I'm sorry sweetheart, but I don't think you could take me like that without some serious pain.  Maybe after you've loosened up that cute little snatch with some sex-toys I could give it a go... by Marae I want this, but you aren't ready for my... my thing.</i>\"\n\n");
        return { next: urtaHomeLuvLuvinsMenu };
    }
    // (SHIT FITS)
    CView.image("urta-home-female-fuck");
    CView.text("Urta bursts into a wide smile at your suggestion and scooches over, making room for you on the bed.  ");
    if (player.body.cocks.length > 0) {
        CView.text("She tosses you ");
        if (player.body.cocks.length === 1) CView.text("a latex sheath");
        else CView.text("some latex sheaths");
        CView.text(" to cover your " + describeCocksLight(player) + " with, explaining that she doesn't want to have to mop up afterward, and you grudgingly agree.  ");
    }
    CView.text("You climb in next to her, snuggling under the blankets and stroking her shaft, feeling yourself grow more and more ready as you tease your vulpine lover.  Urta doesn't move beyond rolling her eyes back and squirming her hips into your hand, trying to enhance the sensation.  She whines, \"<i>I thought you were going to fuck meee-</i>\"\n\n");

    CView.text("Her cries are muted as you launch yourself over her, brutalizing her eager lips with a ferocious kiss.  You roughly tongue-fuck her as you adjust yourself, balancing yourself with one hand while your other guides her trembling shaft closer and closer to the entrance of your " + describeVagina(player, player.body.vaginas.get(0)) + ".  You sigh happily and rub the pre-filled bulb against your opening, ");
    if (player.body.vaginas.get(0).wetness < VaginaWetness.WET) CView.text("growing moist");
    else if (player.body.vaginas.get(0).wetness < VaginaWetness.DROOLING) CView.text("growing wetter and wetter as you lubricate the tip of her condom");
    else CView.text("soaking her condom-covered cock with your copious cunt-secretions");
    CView.text(".  Urta's body goes limp under your sexual assault, totally caving in to the feelings you're stoking inside her.  The only thing she manages to keep up is the twisting of her hips as she tries to enhance the sensations radiating through her massive, flared prick.\n\n");

    CView.text("You end the kiss by pulling back, sucking her lower lip into your mouth, and then releasing it with a pop.  Nearly crying with need, your horse-endowed lover begs, \"<i>Ah, please don't stop!</i>\"\n\n");

    CView.text("Never one to disappoint, you press onwards and downwards, sliding deeper into the bed as you work the wide, flared tip against yourself.  It slowly stretches you wide and bows under your weight, though Urta doesn't voice a word of complaint or displeasure.  You swivel slightly to the side and it works its way inside you, one half of the tip sliding inside at a time.  The fox's mouth opens into an 'O' of pleasure, letting her tongue escape as she begins panting from the pleasure.\n\n");

    CView.text("Sinking down slowly and relishing the feeling of your passage being spread, you allow a low moan to escape your mouth.  Urta's hands lock onto her breasts and begin alternatively squeezing her soft mounds and fondling the black skin of her nipples.  The sensation of slow penetration is exquisite, growing within you while ever thicker horse-cock spears your womb.  You can feel the beat of Urta's heart racing through the shaft, swelling it slightly in time with each heartbeat.\n\n");

    CView.text("You bottom-out, but immediately begin rocking your hips, working your lover's twitching rod deep inside your " + describeVagina(player, player.body.vaginas.get(0)) + ".  ");
    // (LOOSENESS MESSAGES GO HERE AS APPROPRIATE)
    displayStretchVagina(player, 60, true);
    CView.text("Urta's hands grab you firmly by the " + describeButt(player) + ", reminding you just how strong the lithe fox is.  She begins working with you to lift your body up and slam it back down against her, alternating the feelings of emptiness and fullness within you.  Each time the little balloon that's formed on her flare gets larger, squishing each time she bottoms out.  As the pair of you rut faster and faster, the limited space inside you squeezes the excess pre-cum from the sheath, causing it to drip over her balls into the sheets.\n\n");

    CView.text("Bending over, you plant another kiss full on her lips, and your vulpine lover just can't restrain herself anymore.  She moans into your mouth, her body shivering, her balls tightening up until you feel them press against your ass.  Her hands grip you hard and pull you down with incredible force, locking you against her as you feel her member swelling inside you, submitting to your " + describeVagina(player, player.body.vaginas.get(0)) + "'s swelteringly hot embrace.  A burst of tightness fills you, growing as Urta squirms, unloading her massive horse-pole's seed into the latex balloon she's stuffed inside you.\n\n");

    CView.text("The filling seed balloon stretches tight inside of you, ");
    if (player.vaginalCapacity() < 80) CView.text("until there is no more room for cum and it's squirting out around the base of her cock, soaking Urta and the bed with fragrant horse-cum.");
    else CView.text("filling your body with a large bubble of hot horse-cum.");
    CView.text("  Watching your lover submit to your body's pleasure, the sight pushes you over the deep edge with her, and your body begins milking the thick shaft inside you, clenching around it as you spasm happily.  ");
    if (player.body.cocks.length > 0) {
        if (player.body.cocks.length === 1) CView.text("Y");
        else CView.text("Each of y");
        CView.text("our " + describeCocksLight(player) + " spurts, unloading waves of cream into the ");
        if (player.body.cocks.length === 1) {
            CView.text("condom Urta asked you to wear around it.");
            if (player.cumQ() >= 250) {
                if (player.cumQ() < 500) CView.text("  The balloon inflates slightly, drooping down as your maleness empties its reserves.  ");
                else if (player.cumQ() < 1000) CView.text("  The balloon swells larger and larger as you pump it full, eventually wobbling around on Urta's belly as you finish filling it.");
                else {
                    CView.text("  The balloon swells larger and larger as you pump it full, wobbling obscenely on Urta's belly and slowly drooping down onto the sheets as the sheath reaches its maximum capacity.");
                    if (player.cumQ() > 1500) CView.text("  Your potent seed pops the condom, splattering Urta with your seed as it explosively fails.");
                }
            }
        }
        else {
            CView.text("condoms Urta asked you to wear around them.");
            if (player.cumQ() >= 250) {
                if (player.cumQ() < 500) CView.text("  The balloons inflate slightly, drooping down as your malenesses empty their reserves.  ");
                else if (player.cumQ() < 1000) CView.text("  The balloons swell larger and larger as you pump them full, eventually wobbling around on Urta's belly as you finish filling them.");
                else {
                    CView.text("  The balloons swell larger and larger as you pump them full, wobbling obscenely on Urta's belly and slowly drooping down onto the sheets as the sheaths reach their maximum capacity.");
                    if (player.cumQ() > 1500) CView.text("  Your potent seed pops the condoms, splattering Urta with your seed as they explosively fail.");
                }
            }
        }
    }
    ...
    CView.text("Urta's eyes roll back in her head as her body goes nerveless, devoting all its energy to cumming as much as possible inside you.  You grab onto her breasts and do your best to hang on for dear life, riding out your orgasm and trying not to drool onto the insensate fox.  She pumps away, weakly finishing her earth-shattering orgasm, her eyes half-rolled up and heavily lidded.  You slump down on top of her and slowly recover, trembling as muscle spasms work their way through your body.\n\n");

    CView.text("Once you've recovered, you pull back with a long, wet sluuuuurrrrpPOP, and Urta's massive cum balloon slips out from between your nether-lips, pulling the rest of her shaft down onto her and bursting like a thrown water-balloon.  The splash of cum over her body rouses Urta from her orgasm-induced stupor, and the sexy fox moans with equal parts contentment and annoyance as she comes back to reality.\n\n");

    CView.text("\"<i>Oooh, I'm going to need to do laundry AGAIN!</i>\" gripes Urta, lifting herself up to her elbows.  She continues, \"<i>But I'll be damned if cumming inside you isn't worth washing all the sheets in the world.</i>\"\n\n");

    CView.text("Smiling, the two of you embrace, though you're careful not to get any of the cum on you.  Urta waves you out so she can get to cleaning, and by the time you leave you've got a spring in your step and a satisfied grin on your face.");
    // if(UrtaFlags.URTA_FERTILE == 1) player.body.womb.knockUp(new Pregnancy(21, 515), 80);
    player.orgasm();
    if (player.stats.sens > 30) player.stats.sens += -1;

    return { next: returnToCampUseOneHour };
}

// [RIDE ANAL]
function rideUrtaInButtAtHomeLove(player: Character): NextScreenChoices {
    urtaSprite();
    urtaLove(1);
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME = 2 + randInt(2);

    CView.image("urta-home-anal-urta");
    CView.text("Urta raises an eyebrow at the suggestion and blushes, though her horse-like member thickens and stiffens noticeably.  You give it a gentle slap and tease, \"<i>Don't act like this isn't what you wanted!</i>\"\n\n");

    CView.text("She squirms and smiles nervously, \"<i>It feels too good, even though it's dirty!  Besides, your sweet ass is irresistible!</i>\"\n\n");

    if (player.body.cocks.length > 0) {
        CView.text("Even though you're going to be riding her, she insists you wear ");
        if (player.body.cocks.length === 1) CView.text("a condom");
        else CView.text("some condoms");
        CView.text(" to help control the mess.  You grudgingly accept, girding your hardening shaft");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(" in tight latex.\n\n");
    }

    CView.text("You climb on to the bed, but fur-covered hands grab hold of your " + describeHips(player) + " and yank you down until something round and wide is knocking on your backdoor.  You can feel the latex sheath bubbling up, starting to give Urta's dick a more rounded tip and she begins pressing it into your backdoor.  The unexpected attempts at insertion surprised you and have kept you too tightly-clenched for the fox to batter her way inside.  She begs, \"<i>Please let me in!  I can't wait anymore!</i>\"\n\n");

    CView.text("While giving her a knowing smile, you make a conscious attempt to relax your body's surprised rear-entrance.  Almost immediately you feel her slip inside, and then you're sliding down, sinking slowly towards your partner an inch at a time.  The expression on Urta's face is priceless – flitting between ecstasy and confusion as she struggles to deal with being engulfed by your ");
    if (player.analCapacity() < 60) CView.text("tight");
    else CView.text("welcoming");
    CView.text(", warm hole.  A groan escapes your mouth as you sink down, feeling fuller and fuller until her real sheath is resting just between your ass-cheeks.");
    displayStretchButt(player, 60, true, true, false);
    if (player.body.cocks.length > 0) {
        CView.text("  You can feel tiny dribbles force their way out of ");
        if (player.body.cocks.length > 1) CView.text("each of ");
        CView.text("your urethra");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(" as your prostate is compressed.");
    }
    CView.text("\n\n");

    CView.text("Urta recovers enough to meet your gaze and licks her lips hungrily.  Her hands release your " + describeHips(player) + " and rump, sliding up your sides and in towards your " + describeAllBreasts(player) + ".  You shiver as she circles your " + describeNipple(player, player.body.chest.get(0)) + ", teasing the flesh around them while managing to avoid providing them with any stimulation.  She looks up at you expectantly with a patient look, as if waiting on something from you.\n\n");

    CView.text("You shift your hips and are rewarded by a caress over your left " + describeNipple(player, player.body.chest.get(0)) + ", but it stops almost immediately.  Catching on, you flex your " + describeLegs(player) + " and lift yourself up until her flare is the only thing connecting you before sliding back down the pulsating shaft, squeezing your muscles around it.  Urta's hands reward you by pinching and tugging on your " + describeNipple(player, player.body.chest.get(0)) + "s, ");
    if (player.body.chest.find(BreastRow.FuckableNipples)) CView.text("even slipping a finger inside their slippery depths, ");
    CView.text("spreading pleasure through the sensitive pleasure-buds.  Eager for more, you begin bouncing atop your vulpine lover, riding her as deeply as possible in return for a treasure-trove of sensation and lust.\n\n");

    CView.text("Locked together by desire, love, and one swollen cock, you rut with her for what seems like forever.  You lose count of how many times your lover moans and screams in pleasure, and your own gasps of surprised enjoyment are too numerous to tabulate.  Urta's fur slowly becomes matted with sweat, and the salty scent of lovemaking fills the air.");
    if (player.body.cocks.length > 0) {
        if (player.body.cocks.length > 1) CView.text("  Each of y");
        else CView.text("  Y");
        CView.text("our " + describeCocksLight(player) + " has filled its sheath with a golfball sized globule of pre-cum, and you can feel your body clenching and squeezing, your ");
        if (player.body.balls.count > 0) CView.text("balls");
        else CView.text("prostate");
        CView.text(" aching to unload.");
    }
    if (player.body.vaginas.length > 0) {
        CView.text("  Neglected and unused, your " + describeVagina(player, player.body.vaginas.get(0)) + " ");
        if (player.body.vaginas.get(0).wetness < VaginaWetness.DROOLING) CView.text("dampens");
        else CView.text("floods");
        CView.text(" the fox underneath you with moisture, puffy and aching for the slightest touch.");
    }
    CView.text("\n\n");

    CView.text("The lovemaking increases in tempo, slowly working its way to a feverish crescendo.  You're bouncing atop the fox wildly, and your lover cums hard.  As full as you felt with her inside you, it's nothing compared to being stuffed with her cock AND cum.  Each cum-filled contraction swells her member from base to flare, allowing you to feel it stretch your insides while it travels up your " + describeButthole(player.body.butt) + ".  It dumps the load into the bubble of cum inside you, filling you with a creamy jizz balloon.\n\n");

    // (GENDERLESS ORGAZM)
    if (player.gender === 0) {
        CView.text("You cry out with pleasure, feeling the closest thing you can to an orgasm.  Your muscles clench around the invading rod, milking it while the rest of your body trembles.  The strength drains from your " + describeLegs(player) + " and arms and you twitch there, completely impaled on your lover's thick cock, happily and completely penetrated.\n\n");
    }
    // (EVERYBODY ELSE)
    else {
        CView.text("You cry out with pleasure, feeling your body cum from the invading warmth in your " + describeButthole(player.body.butt) + ".  Shivering as the strength drains from your arms and " + describeLegs(player) + ", you let the intruding and still cumming cock support your body.");
        if (player.body.vaginas.length > 0) {
            CView.text("  Your " + describeVagina(player, player.body.vaginas.get(0)) + " clenches desperately, peaking without a single touch, but hungry for something to milk.");
            if (player.body.vaginas.get(0).wetness > VaginaWetness.WET && player.body.vaginas.get(0).wetness < VaginaWetness.SLAVERING) CView.text("  Fluids soak Urta's crotch with lust with feminine drool, though the fox barely seems to notice.");
            else if (player.body.vaginas.get(0).wetness === VaginaWetness.SLAVERING) CView.text("  An explosion of feminine fluids spatters Urta's crotch and bed, completely soaking her groin in your need, but the distracted fox barely notices.");
        }
        if (player.body.cocks.length > 0) {
            CView.text("  The warmth spreads deep inside you, firing off a series of contractions just inside your groin.  You can feel yourself about to cum, and look down just in time to see yourself squirting against your condom, deforming it from the pressure of the orgasm and slowly inflating it further.");
            if (player.cumQ() >= 750 && player.cumQ() < 1250) CView.text("  By the time you're done the balloon is the size of a basketball and looking dangerously close to bursting.");
            else if (player.cumQ() >= 1250) {
                CView.text("  By the time you're done, the balloon is the size of a watermelon and wobbling dangerously, looking like it could fail at any time!");
                if (player.cumQ() >= 1700) CView.text("  You fill it fuller and fuller, until it's bigger than you thought possible.  All at once the material gives out, soaking Urta from the tits down in cum, while your " + describeCock(player, player.body.cocks.get(0)) + " continues to pump even more onto her.  So much for not making a mess!");
            }
        }
        CView.text("\n\n");
    }
    ...
    CView.text("Exhausted, you drag yourself up, slowly feeling Urta and her sloshing deposit sliding out of you.  Her cock and its spooge and latex shrouded tip flops out with a loud 'schhhlurp', plopping down onto her.  The fox is completely insensate, and at a glance even her balls have shrunk from unloading so much thick cream.  You lean down and give Urta a quick kiss on her lips and a gentle squeeze on her nipple before you rise.\n\n");

    CView.text("She comes out of her stupor and murmurs, \"<i>Mmmm, that was nice,</i>\" as she stretches.  With practiced motions she slides off her condom, ties it off, and tosses it into a waste-can half filled with the things.  ");
    if (player.cumQ() >= 1700) CView.text("Urta sighs at the mess and laughs, \"<i>I guess we got a mess anyways, huh?</i>\"");
    else CView.text("Urta sighs contentedly and laughs, \"<i>I guess I don't have much of a mess to clean for once!</i>\"");
...
// [69]
function oralFiestyUberExplosionUrta(player: Character): NextScreenChoices {
    urtaSprite();
    urtaLove(1.5);
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME = 2 + randInt(2);
    ...
    if (!cockThatFits) cockThatFits = player.body.cocks.get(0);
    // (TOO BIG)
    if (cockThatFits.area >= 100) {
        CView.text("Urta takes one look at your " + describeCock(player, cockThatFits) + " and says, \"<i>I'm sorry babe, but there's no way I could take that monster in my mouth.  How about we do something else?</i>\"\n\n");
        return { next: urtaHomeLuvLuvinsMenu };
    }
    CView.image("urta-home-69");
    if (player.gender === 2 || (player.gender === 3 && randInt(2) === 0)) {
        CView.text("You smile and let Urta know you want to sixty-nine her.  She raises an eyebrow and teases, \"<i>I hope you don't mind having a full belly and a sore throat!</i>\" as you disrobe and climb atop her.  You turn about, straddling your legs across her face and exposing the hungry lips of your sex.  Smirking over your shoulder, you snatch her discarded dress from the floor and tie the shimmering fabric into an impromptu cock-ring.  The gray fox shudders with excitement and you watch, enraptured, as her mottled horse-cock grows larger and larger, even flaring wide at the tip.\n\n");

        CView.text("The musky scent of Urta's forearm-sized horse-prick wafts into your nostrils as it wobbles just below your " + describeFaceShort(player) + ", visibly twitching and throbbing from all the extra blood.  She whines plaintively, nuzzling her moist nose against your ");
        if (player.body.vaginas.get(0).wetness < VaginaWetness.DROOLING) CView.text("equally wet");
        else CView.text("sopping wet");
        CView.text(" cunt.  Shivering from the air on your bare skin and the unexpected touches on your nethers, you open wide, extend your tongue, and give Urta's flare a long, slow lick.  You pause at her urethra, slip ever-so-slightly inside, and then pull out as you slather the flare in spit.\n\n");

        CView.text("Urta eagerly gets to work, stabbing her tongue through your folds and into your " + describeVagina(player, player.body.vaginas.get(0)) + ".  Her efforts bring her muzzle flush against you as she pauses to slurp your pussy-juices over your " + describeClit(player) + ".  Her lips feel like liquid heat as they wrap around the incredibly sensitive bud, suckling it gently in between excursions into your passage.  Urta's slender fingers grab hold of your " + describeButt(player) + " to support her as she tongue-fucks your " + describeVagina(player, player.body.vaginas.get(0)) + " with abandon.\n\n");
        ...
        if (player.stats.cor > 50) {
            CView.text("push forwards, feeling her shaft arch as it slides into your throat.  The medial ring slips past your lips and you can feel your throat bulging as you mouth-fuck the swollen horse-cock deep into your gut.  Warmth drips into your belly as the hermaphroditic fox's member closes on its sexual peak.\n\n");
        }
        else CView.text("grab hold of her cock and begin jerking her off into your mouth.  An immediate and copious gush of pre drips onto your tongue, showing you just how much the hermaphroditic fox loves the feeling.\n\n");

        CView.text("Not far from cumming yourself, you grind your " + describeHips(player) + " on Urta's face, smearing her fur with your fragrant pussy-juices.  Her own moans of pleasure turn into noisy wet blubbering that vibrates through your " + describeVagina(player, player.body.vaginas.get(0)) + ", getting you off on the spot.  You squeal around the cock in your ");
        if (player.stats.cor <= 50) CView.text("mouth");
        else CView.text("throat");
        CView.text(", clamping your " + describeLegs(player) + " onto Urta's head as you orgasm.  ");
        if (player.body.vaginas.get(0).wetness > VaginaWetness.DROOLING) CView.text("Fluids rush from your folds, splattering her face with sex fluid.  The fox opens wide, gulping it down like someone in a desert who has just found an oasis.");
        else CView.text("Wetness slowly coats her muzzle as your canal squeezes and wriggles with involuntary pleasure-spasms.  Urta never lets up, sucking and licking your " + describeClit(player) + " in a rhythm that matches your own orgasmic contractions.");
        if (player.body.cocks.length > 0) {
            CView.text("  ");
            if (player.body.cocks.length > 1) CView.text("Each of y");
            else CView.text("Y");
            CView.text("our " + describeCocksLight(player) + " drools lamely, ");
            if (player.cumQ() < 50) CView.text("sputtering spunk onto Urta's neck.");
            else if (player.cumQ() < 250) CView.text("soaking Urta's neck and chest with your spooge.");
            else if (player.cumQ() < 700) CView.text("showering Urta's neck and body with spooge until her breasts are painted white and the bed has a small puddle soaking into it.");
            else if (player.cumQ() < 1500) CView.text("soaking Urta with a torrent of seed that drenches her from the neck down and totally soaks the bed.");
            else {
                CView.text("soaking Urta with a torrent of seed that drenches her body and floods the bed, drips onto the floor, and slowly spreads into a puddle.");
                if (player.cumQ() > 10000) CView.text("  The puddle thickens as you cum until it's nearly a foot deep.");
            }
        }
        CView.text("\n\n");

        CView.text("Too drunk with pleasure to register the telltale thickening of Urta's cock, you keep licking and sucking while you cum, drinking a steady stream of pre-cum.  Suddenly, your improvised cock-ring snaps and the straps fail from the massive pressure.  A torrent of jism ");
        if (player.stats.cor > 50) CView.text("pours down your throat, bulging it into an obscene imprint of the horse-cock that's lodged inside.  Your eyes cross from pleasure and you slump down a bit, shoving your nose into Urta's sheath and allowing her to dump her sperm directly into your belly.  Gurgling noisily, your belly slowly expands outwards by about two inches, making you feel bloated and full.");
        else CView.text("erupts into your mouth, instantly filling it with thick fox-cream.  You convulsively swallow in an attempt to keep up with it, but are quickly overwhelmed by the backed up semen-flood.  It drips from your mouth and forces open your throat, flowing directly into your belly.  Some even begins dripping out from your nose as you sway with a mixture of discomfort and pleasure atop your partner.  A few trembling squirts later you feel and look a little bloated, and your " + describeFaceShort(player) + " is thoroughly covered in foxy-horse-jizz.");
        CView.text("\n\n");

        CView.text("Both of you sputter for breath as you roll off of Urta.  She gasps and looks over at you with a smile, licking her sperm from your face and nuzzling her pussy-soaked muzzle against your cheek.  She pants, \"<i>I suppose I should be mad at you for ruining the dress, but you got me off so hard that I think I gave you a few meals worth of my... um... cum.</i>\"\n\n");

        CView.text("The fox giggles and returns to cuddling and licking you.  You rest with a happy smile plastered on your " + describeFaceShort(player) + " from the experience.  After all the snuggling, you both rise and get dressed.  Urta finds a new dress in the closet and with a long goodbye kiss, you go your separate ways.\n\n");
        player.orgasm();
        player.stats.sens += -1;
    ...
    // (DUDEZILLAZ)
    CView.text("Urta sits up and practically screams, \"<i>Really,</i>\" and squeals excitedly.  She claps and lies back as you climb into the bed, stopping only to yank off her cock-sheath and toss it into a nearby waste-bin.  You swing your " + describeLegs(player) + " over her and get into position, aligning ");
    if (player.body.balls.count > 0) {
        CView.text("your " + describeBalls(true, true, player) + " and ");
    }
    if (player.body.cocks.length > 1) CView.text("one of your ");
    else CView.text("your ");
    CView.text(describeCocksLight(player) + " with Urta's muzzle.  Her rather sizable 'addition' wobbles underneath you, blocking your vision as it wobbles back and forth under you.\n\n");

    CView.text("Your foxy lover opens wide and slips her smooth, wide tongue all over your ");
    if (player.body.balls.count > 0) CView.text(describeBalls(false, true, player, true) + ".  Slithering over and around them, she bathes your sack in her spit before moving up to your " + describeCock(player, cockThatFits) + ".");
    else CView.text(describeCock(player, cockThatFits) + ".");
    CView.text("  Her tongue curves and slurps at your shaft, swelteringly warm, wet, and tantalizingly slippery.  She tastes you from ");
    if (player.body.cocks.find(Cock.HasSheath)) CView.text("sheath");
    else CView.text("base");
    CView.text(" to " + describeCockHead(cockThatFits) + ", stopping to suckle the sensitive end briefly before breaking away for another long lick along the underside.  Her hands stroke along the sensitive " + player.body.skin.desc + " of your inner thighs, making your hips shiver and nearly pulling your " + describeCock(player, cockThatFits) + " away from your lover's hungry mouth.\n\n");

    CView.text("Whilst the fox is busy with your genitals, you begin to perform your duty, reciprocating the pleasure in turn.  You stretch your mouth wide, opening as far as you can to accommodate the mammoth beast before you.  It's difficult, but you manage to stretch wide enough to take her flare in your mouth.  It tastes vaguely salty like sweat, and in Urta's excited state it's easy to feel her flare widen and retract with each beat of her heart.  A dollop of pre-cum deposits itself on your tongue as the horny fox's nymphomaniac body reacts to you.  It tastes sweet, and you swallow as you work her deeper into your oral cavity.\n\n");

    CView.text("Muffled, excited moans reach your ears as you work on your lover's pole.  The fox is clearly enjoying herself, and her sounds of pleasure cut off as she rams your " + describeCock(player, cockThatFits) + " deep into her throat.  She gurgles noisily while her tongue slides as far down your shaft as possible, licking at your ");
    if (player.body.balls.count > 0) CView.text(describeBalls(true, true, player));
    else if (player.body.vaginas.length > 0) CView.text(describeVagina(player, player.body.vaginas.get(0)));
    else CView.text("taint");
    CView.text(" while her spittle froths around her lips.  She manages to start humming deep within her throat, vibrating your " + describeCock(player, cockThatFits) + ".  The obstruction in your mouth stifles your moan, but Urta hears the startled gurgle and slides away, then plunges back down, humming the entire time in between gasps for air.\n\n");

    CView.text("Not to be outdone, you pull back for a quick gasp of air before ramming her back inside your mouth.  You push forwards, taking her to the back of your throat");
    if (player.stats.cor < 60) CView.text(" until your gag reflex forces you back.  Not being able to deep-throat her, you shift your weight onto your right arm and grab her just below her medial ring with your left.  You start vigorously jacking her off, running your tongue around her flare and over her sensitive tip, even darting it into her urethra to get at her sweet-tasting pre-cum.");
    else CView.text(".  You easily suppress your gag reflex and push forwards, feeling her cock arch slightly in the back of your mouth as it curves into your throat.  You extend your tongue as far past your lips as you can, licking down the shaft as you cram her further and further into your gullet.  Though you can't taste it, you can feel her urethra sending slow pulses of pre-cum inside of you, depositing it directly into your belly as you stuff your nose into her balls and tongue the skin around her sheath.");
    CView.text("\n\n");

    CView.text("If someone walked in on the both of you now they'd see a breathless fox and you, each with the other's cock rammed into their face.  They'd see each of you blushing bright and trembling, on the verge of orgasm and barely holding on as the other does their best to bring about an orgasm.  Wet slurps and panicked gasps fill the air while bodies twist and squirm together on the fur-covered bed.\n\n");

    CView.text("You cum first, a victim of Urta's talented tongue and humming technique.  You'd cry out with pleasure were you not so thoroughly stuffed, and the fox slams her face into your groin after the first blast, pushing you into her throat and noisily beginning to gulp down your spooge.  ");
    if (player.cumQ() >= 500) {
        CView.text("She gurgles as she struggles to keep up with the flow, actually letting a bubbly white froth start to leak down the sides of her mouth.  ");
        if (player.cumQ() >= 1000) CView.text("It begins dripping from her nostrils as your explosive load overwhelms her ability to swallow spunk.  ");
    }
    if (player.cumQ() >= 750 && player.cumQ() < 1500) CView.text("Her belly rounds out a little from all the cum as your orgasm begins to wind down.  ");
    if (player.cumQ() >= 1500) CView.text("Her belly rounds out a little before she pulls back and you spurt all over her, soaking her face and head with more spooge than she could ever handle.  ");
    CView.text("Sated, you sigh, though Urta pushes back onto you and intently licks every ounce of jism from your body.\n\n");

    CView.text("With only a few moments to recover from your own orgasm, you feel the fox stiffening, her apple-sized balls pulling up tightly against her body as she starts to orgasm.  A cock-bulging load of thick fox-cream forces your mouth wider before exploding ");
    if (player.stats.cor < 60) CView.text("in your mouth, forcing you to compulsively swallow if you don't want to drown in it.");
    else CView.text("down your throat, directly injecting it into your belly.");
    CView.text("  More and more of the salty cream is forced into you, leaving you feeling full and bloated, with a salty aftertaste in your mouth.  The fox-girl's horse-cock does eventually stop cumming, though rather than stopping all at once it slows to a trickle over the course of a minute or two, and you slurp it down until it softens in your mouth.\n\n");
    ...
    CView.text("Urta blows you another kiss and says, \"<i>Gotta go!  Be good, and don't do anyone I wouldn't, OK?  Love you!</i>\"");
    player.orgasm();
    player.stats.sens += -1;
...
function urtaHomeLuvLuvinsMenu(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("She caresses a nipple and visibly shivers as she says, \"<i>You teased me the whole way here, and I'm COMPLETELY ready for you, any way you could want.</i>\"\n\n");
    CView.text("She blushes and continues, \"<i>So, what'll it be, lover?  A little injection of fox-cream or something else?</i>\"");
    // [Ride Vaginal] [Ride Anal] [69]
    CView.text("\n\nHow do you want to fuck with the vixen?");

    const choices: ScreenChoice[] = [];
    if (player.body.vaginas.length > 0) {
        CView.text("  She could ride your vagina.");
        choices[0] = ["Ride Vag", rideUrtasCoochLoveyDovey];
    }
    CView.text("  She could fuck your ass.");
    choices[1] = ["Ride Ass", rideUrtaInButtAtHomeLove];
    if (player.body.cocks.length > 0) {
        CView.text("  Urta could 69 with you, let you fuck her pussy, or you could even fuck her cunt full of jizz and then lick her to a second cum.");
        choices[2] = ["69", oralFiestyUberExplosionUrta];
        choices[3] = ["Vag Fuck", urtasCoochiNeedsFuckedLove];
        choices[5] = ["FuckAndLick", lickOutUrtaAtHome];
    }
    if (player.gender > 0) {
        CView.text("  There's always the option to ask her to go condomless for a bit of extra fun.");
        choices[4] = ["No Condoms", condomlessUrtaInHouseSmex];
    }
    if (player.body.legs.isGoo() && player.body.skin.type === SkinType.GOO) choices[6] = ["Goo (Weird)", urtaGooTesticleVoreRuinedOrgasms];
    if (UrtaFlags.URTA_PETPLAY_DONE >= 0 && player.gender > 0) choices[7] = ["Collar", urtaPetPlayDeletedForeverBecauseThirdProovedMeWrongAboutDice];

    return { choices };
}

function urtasCoochiNeedsFuckedLove(player: Character): NextScreenChoices {
    urtaSprite();
    urtaLove(1.5);
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME = 2 + randInt(2);
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(urtaCapacity()));
    ...
    CView.image("urta-fills-her-condom");
    CView.text("Urta gives you a knowing look as she spreads her legs and lifts her bulging sack to expose the glistening black lips of her sex.  She asks with a sultry expression, \"<i>Is that what you need, lover, a slick, wet cock-sleeve to sheath yourself in?</i>\"\n\n");

    CView.text("You nod eagerly and advance, dropping your " + player.inventory.equipment.armor.displayName + " by the wayside as you jump into bed.  She reaches around her male endowments, stroking along her lips to her puffy black clit.  Urta shivers in pleasure, spreading her legs as far as she possibly can, watching you with a hungry look as she licks her lips.  A few long strands of cunny-juice leak from her waiting hole onto the bed, practically begging you to slide inside and feel her nether-lips' velvet embrace.  You sidle up to your lover, pressing your ");
    if (player.body.cocks.length > 1) CView.text("main ");
    CView.text(describeCocksLight(player) + " at the moist opening, shivering at the warmth and desire as it wets your tip");
    if (player.body.cocks.length > 1) CView.text("s");
    CView.text(".\n\n");

    if (cockThatFits.area > urtaCapacity()) {
        CView.text("Urta whines a little as you press forward and scooches backwards, \"<i>I'm sorry " + mf(player, "stud", "baby") + ", you're just too big for my poor little pussy to take.  Maybe we could do something else?</i>\"\n\n");
        return { next: urtaHomeLuvLuvinsMenu };
    }
    CView.image("urta-home-male-fuck");
    CView.text("You push forwards, slowly sliding into the moist tunnel, feeling its warmth covering every inch of your " + describeCock(player, cockThatFits) + ". It caresses and squeezes you, surrounding you in perfect pleasure.  Urta stretches out languidly, her engorged horse-cock flopping down between her sizable breasts as it reaches a twitching, trembling hardness.  You bottom out, feeling the ");
    if (player.body.cocks.find(Cock.HasSheath)) CView.text("sensitive skin of your sheath rubbing her exterior pussy-lips");
    else CView.text("wetness of her pussy rubbing against your groin");
    CView.text(". Urta moans, \"<i>By Marae, you make my cunt feel good, lover.  I'm getting so wet... fuuuck, I'm going to need to change my furs and sheets again.</i>\"\n\n");

    CView.text("Unthinking, your " + describeHips(player) + " begin to rock back and forth, sawing in and out of the fox's exotic pussy.  She wriggles and moans happily, losing her grip on her weighty balls and letting them slide over the top-side of your pussy-slicked " + nounCock(CockType.HUMAN) + " with each movement.  You ignore it, increasing the tempo and watching as your lover's jiggling breasts shift and bounce around her flared penis, eliciting uncontrolled gasps of pleasure and causing pre to form a bubble at the tip of her latex cock-sheath.  Every thrust is met by a visible bulging of Urta's urethra and an increase in the size of the balloon hanging from her dick's flare.\n\n");

    CView.text("Wet sounding 'schliiiicks' and noisy slaps fill the air as the sex takes on a violent, furious pace.  You slam yourself into the fox's hole, marveling at how nicely it accommodates and squeezes you.  The walls clench down on you from time to time, usually accompanied by a lewd moan as Urta's muscles involuntarily contract with pleasure.  She grabs her tits with both hands and squeezes them around her shaft, tit-fucking herself in perfect rhythm with your brutal thrusts.  Urta's condom starts filling faster – it's nearly the size of an apple now – and she's openly moaning and panting, on the verge of orgasm.\n\n");

    CView.text("Your " + describeCock(player, player.body.cocks.get(0)) + " is on the verge as well, ready to blow after fucking the dirty fox-girl so hard.  She cums first, shrieking in pleasure and throwing back her head, her hands clamping her jiggling breasts tightly around the rippling flesh of her massive horse-member as it begins unloading.  The surface of Urta's condom distorts as she fires a massive jet of her stuff into it, nearly doubling the size of the balloon.  Her hips wriggle and her pussy contracts around you, rippling along your length as the lusty fox's girl-cum squirts over your ");
    if (player.body.cocks.find(Cock.HasSheath)) CView.text("sheath");
    else if (player.body.balls.count > 0) CView.text("balls");
    else CView.text("crotch");
    CView.text(".  You grab her ass and slam in a final time, reaching your own peak.\n\n");

    // [DAWG]
    if (cockThatFits.hasKnot()) {
        CView.text("Your knot flares wide, locking itself inside her convulsing cunt as your seed begins pumping into her womb.  Urta cries, \"<i>oooooooOOOOHHhhh YESSSS... You've plugged my cunt with your doggie dick!  It's squirting and pumping into my wooooomb...</i>\"\n\n");

        CView.text("You rock back and forth, unable to pull out yet stimulating the sensitive bulb of your knot.  Your " + describeCock(player, cockThatFits) + " dumps its gooey cargo directly into your lover's stopped-up sex with barely audible squishing noises.  ");
        if (player.cumQ() < 250) { }
        else if (player.cumQ() < 700) CView.text("Her belly puffs up slightly by the time you finish, thanks to your body's copious cum production.  ");
        else if (player.cumQ() < 1000) CView.text("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your overproductive " + nounCock(CockType.HUMAN) + " stops spewing inside her.  ");
        else {
            CView.text("Her belly swells larger and larger, bloating as you blast thick torrents of seed inside her, filling her completely thanks to the obstruction of your knot.  ");
            if (player.cumQ() < 3000) CView.text("By the time you finish, she looks VERY pregnant and is moaning with a mixture of pleasure and discomfort.  ");
            else {
                CView.text("Your seemingly never-ending orgasm pumps her up until she looks on the verge of giving birth, and you just keep going.  The pressure builds and Urta moans in discomfort and pleasure.  Eventually the pressure is too much, and your cum begins spraying out around your knot, soaking the pair of you as you both continue to cum hard.  ");
                if (player.cumQ() > 5000) CView.text("Not content with such an easy release, your " + describeCock(player, cockThatFits) + " unloads the last of its cream until the bed and floor are both covered under inches of your canine cum.  ");
            }
        }
        CView.text("You finally feel your " + nounCock(CockType.HUMAN) + "'s knot shrinking and pull free with a loud and messy pop.  ");
        if (player.cumQ() >= 2000) CView.text("Spunk rushes out in a torrent, splattering everywhere.  ");
        CView.text("Urta's glazed pussy-lips clench reflexively at their sudden emptiness and you hear a very satisfied sigh from the far end of the bed.\n\n");
    }
    // [HORSE]
    else if (cockThatFits.type === CockType.HORSE) {
        CView.text("You feel your flare filling with blood as it ");
        if (cockThatFits.length >= 10) CView.text("prods against Urta's cervix.");
        else CView.text("seats itself deep inside Urta's passage.");
        CView.text("  Your crotch muscles clench in an unconscious rhythm as orgasm wracks your body.  You begin spurting hard inside her, splattering it into her cervix, even allowing it to flow into her womb.  Urta cries, \"<i>ooooOOOHHHH YESSS... is this what you feel when I cum inside you?  YesYESYES- Ohhh I can feel the flare stroking and flexing inside me...</i>\"\n\n");

        CView.text("You rock back forth, unable to resist dragging the sensitive, flared tip and textured ring of your cock all along her pussy-walls, squirting seed with reckless abandon into her hot, cum-slicked cunt.  Wet squishing noises fill the apartment as the two of you rock the bed in twinned orgasms.  ");
        if (player.cumQ() < 250) { }
        else if (player.cumQ() < 700) CView.text("Her belly slowly rises as more and more cum slides into her womb, filling it.  Runners of the stuff leak down her thighs as it escapes around your bestial cock.  ");
        else if (player.cumQ() < 1000) CView.text("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your over-productive " + nounCock(CockType.HUMAN) + " has finished spewing your seed inside her.  ");
        else {
            CView.text("Her belly swells larger and larger, bloating out as you blast thick torrents of seed inside her, stuffing her completely.  Every squirt of the stuff is met with a leaking back-blast that soaks her thighs and bed in your jism.  ");
            if (player.cumQ() < 3000) CView.text("By the time you've finished, she looks heavily pregnant and cum soaks the bed.  ");
            else CView.text("Your seemingly endless orgasm pumps more and more into her until she looks ready to give birth, and  you just keep going.  She moans in pleasure and discomfort as each blast of your horse-cum forces thick runners to squirt from between her thighs.  ");
            if (player.cumQ() > 5000) CView.text("Not content with such an easy release, your " + describeCock(player, cockThatFits) + " unloads the last of its cream until the bed and floor are covered in several inches of thick equine cum.  ");
        }
        CView.text("You finally feel your flare returning to normal and pull out with a prolonged 'sluuuuurp'.  ");
        if (player.cumQ() >= 2000) CView.text("Spunk rushes out in a torrent, splattering everywhere.  ");
        CView.text("Urta's glazed pussy-lips clench reflexively at their sudden emptiness and you hear a very satisfied sigh from the far end of the bed.\n\n");
    }
    // [Everybody Else]
    else {
        CView.text("Your " + describeCock(player, player.body.cocks.get(0)) + " clenches, filling her convulsing cunt as your seed begins pumping into her womb.  Urta cries, \"<i>oooooooOOOOHHhhh YESSSS... It's so warm and wet.  Yes, lover!  Keep goo-oh-OH-innnngg!!</i>\"\n\n");

        CView.text("You rock back and forth, sliding your shaft through her spasming muscles, reveling and panting in orgiastic bliss.  Your " + describeCock(player, player.body.cocks.get(0)) + " dumps its gooey cargo deep into your lover's canal with barely audible squishing noises.  ");
        if (player.cumQ() < 250) { }
        else if (player.cumQ() < 700) CView.text("Her belly puffs up slightly by the time you finish, thanks to your body's copious cum production.  ");
        else if (player.cumQ() < 1000) CView.text("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your overproductive " + nounCock(CockType.HUMAN) + " stops spewing inside her.  ");
        else {
            CView.text("Her belly swells larger and larger, bloating as you blast thick torrents of seed inside her, filling her completely while leftover jism squirts out around your " + describeCock(player, player.body.cocks.get(0)) + ".  ");
            if (player.cumQ() < 3000) CView.text("By the time you finish, she looks VERY pregnant and is moaning with a mixture of pleasure and discomfort.  ");
            else CView.text("Your seemingly never-ending orgasm pumps her up until she looks on the verge of giving birth, and you just keep going.  Jism sprays out from her dripping cunt as you release a torrent of semen, splattering you, her, and the bed.  ");
            if (player.cumQ() > 5000) CView.text("Not content with such an easy release, your " + describeCock(player, player.body.cocks.get(0)) + " unloads the last of its cream until the bed and floor are both covered under inches of your gooey cum.  ");
        }
        CView.text("You finally feel your " + nounCock(CockType.HUMAN) + " deflating, and you pull free with a messy slurp.  ");
        if (player.cumQ() >= 2000) CView.text("Spunk rushes out in a torrent, splattering everywhere.  ");
        CView.text("Urta's glazed pussy-lips clench reflexively at their sudden emptiness and you hear a very satisfied sigh from the far end of the bed.\n\n");
    }
    // [RESUME]
    CView.text("Urta calms down at last, though her member is still throbbing weakly as she peels her condom off.  The bloated cum-bubble at the tip is the size of a basketball, and she struggles to get it tied off and disposed of.  The fox tosses it into a waste-basket already full of the things and gives you a toothy smile, \"<i>I think somebody's " + nounCock(CockType.HUMAN) + " loves my pussy almost as much as I love you!</i>\"\n\n");

    CView.text("She pulls you into her arms and gives you a long, wet kiss before breaking away to clean up.  Urta blows you a kiss and says, \"<i>Thanks again, love!</i>\"\n\n");

    CView.text("You and Urta go your separate ways after that, both stinking of sex and with a sense of deep satisfaction.");
    knockUpUrtaChance(player);
    player.orgasm();
    player.stats.sens += -1;
...
// [Back Room]
export function scyllaAndUrtaSittingInATree(player: Character): NextScreenChoices {
    urtaSprite();

    // Increment 'times caught with Scylla'
    UrtaFlags.UNKNOWN_FLAG_NUMBER_00143++;

    if (!urtaDrunk()) {
        return urtaAndScyllaBoningLikeBitchesSober(player);
    }
    const choices: ScreenChoice[] = [];
    if (UrtaFlags.UNKNOWN_FLAG_NUMBER_00147 === 0 && UrtaFlags.UNKNOWN_FLAG_NUMBER_00145 > 0) {
        CView.text("You meander towards the back-rooms and a canine waitress springs forward with a key clutched in her paw.  She blushes furiously and explains that she was tipped generously to hand it to you if you arrived.  You accept the proffered tool and smile.  It must be cheaper to pay someone to wait with a key than to fix the door each time the three of you meet up!  You snicker to yourself as you plunge the key into the lock's waiting receptacle, turn it, and push your way inside.  The scene in front of you is as arousing as it is familiar.\n\n");

        CView.text("Urta is sitting down on a crate with her legs splayed and her torso leaning drunkenly in your direction.  Her tongue is hanging out and her eyes are lidded from alcohol as she notices you and gives another high-pitched whine, not of pain, but pleasure.  The source of her 'distress' is a black-robed figure with curly black locks of hair that hang around her head like a halo and thick, glossy red lips.  Those same lips are currently wrapped firmly around Urta's twitching, lust-swollen horse-cock just above her sheath.  You can clearly see the slight bulge of Urta's medial ring highlighted through the corrupted nun's neck as she bobs up and down, so absorbed in feeding her addiction that she failed to notice your entrance.\n\n");

        CView.text("The fox is giving you a lewd look as she cradles Scylla's head in her hands and pushes down.  She slurs, \"<i>Oooooh YEAH!  Mmmmm, you made it " + player.desc.name + "!  It seems like each time Scylla does... does... oooh... this I cum bigger and harder.  MMmmmm so good... wanna watch?</i>\"\n\n");

        CView.text("Throughout the conversation she's weakly twitching her pelvis, rocking her hips against Scylla's face while her balls start to swell and churn in the nun's hands.  You close the door behind you carefully and drop the key on a crate.  It's time for some fun.\n\n");

        // Butans
        if (player.body.cocks.find(Cock.CockThatFits(urtaCapacity())) && player.body.legs.type !== LegType.CENTAUR) {
            choices[0] = ["Lap", makeUrtaSitOnYourLapWithScylla];
            CView.text("(You could make Urta sit on YOUR lap for a change, or you could jerk off and watch.");
        }
        else {
            CView.text("(You could jerk off and watch.");
        }

        choices[1] = ["Jerk", watchTwoHotBitchesAndJerkIt];
        choices[2] = ["LippleBond", lippleBondUrtaScylla];

        CView.text("  Bonding with the use of Scylla's lipples is also an option.)");

    }
    else {
        CView.text("You meander by the Wet Bitch's back rooms, trying not to look like a creeper as you listen for ");
        if (!urtaLove()) CView.text("Urta");
        else CView.text("your love");
        CView.text("'s voice.  The first few doors are silent and the next one sounds to have a spirited game of strip-poker going on inside it.  Judging by the grunts and occasional mewling moan of pleasure, some poor girl has already lost.  One of the barmaids gives you a warning glance and you keep moving, sure to avoid the lewd game of chance as you hunt for the stray, well-endowed fox.\n\n");

        CView.text("At first, the following entryway is silent, though you can see light underneath.  You're just about to move on from the portal when you hear Urta's voice give out a strained grunt.  Did some ");
        if (player.stats.cor < 40) CView.text("corrupted ruffians");
        else CView.text("demon agents");
        CView.text(" somehow get the drop on her?  She whines, high-pitched and plaintive, almost as if she was wounded, and you cannot wait a moment later.  You lunge to the door, twist the handle, and find it locked.  Without a second thought, you throw your weight into the flimsy wood paneling and crash through to the last thing you expected to see!\n\n");

        CView.text("Urta's sitting down on a crate with her legs akimbo and her torso leaning drunkenly in your direction.  Her tongue is hanging out and her eyes are lidded from alcohol as she notices you and gives another high-pitched whine, not of pain, but pleasure.  The source of her 'distress' is a black-robed figure with curly black locks of hair that hang around her head like a halo and thick, glossy red lips.  Those same lips are currently wrapped firmly around Urta's twitching, lust-swollen horse-cock just above her sheath.  You can clearly see the slight bulge of Urta's medial ring highlighted through the corrupted nun's neck as she bobs up and down, so absorbed in feeding her addiction that she failed to notice you bursting through the door.\n\n");

        CView.text("The fox is ");
        if (urtaLove()) CView.text("weakly protesting");
        else if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY >= 11) CView.text("giving you a lewd look");
        else CView.text("blushing hard");
        CView.text(" as she cradles Scylla's head in her hands and pushes down.  She slurs, \"<i>");
        if (urtaLove()) CView.text("I'm... ahhhh... s-sorry love, but we both know she's been suuuu - oh yeah, keep licking - sucking you off before, and the hungry slut just couldn't take her eyes off my cock.  Why didn't you tell me ho - OH YES!  RIGHT THERE! - h-how good she was?");
        else if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY >= 11) CView.text("Ohhh, you showed up a little late... mmmhmm yes, now lick... Oh, why didn't you tell me ho - OH YES!  RIGHT THERE! - h-how good of a cock-sucker this lewd-bodied bitch was?");
        else CView.text("H-hi " + player.desc.name + "... I didn't think I'd get to see you to- Oh yeah, like that!  Mmmm... OH!  I guess since we've been intimate before you can watch...");
        CView.text("</i>\"\n\n");

        CView.text("Throughout the conversation she's weakly twitching her pelvis, rocking her hips against Scylla's face while her balls start to swell and churn in the nun's hands.  Thankfully the doorway is at such an angle that the bulk of the bar can't see inside, and it helps that no one seems inclined to bother the three of you for now.  What do you do?\n\n");
        ...
        CView.text("  You could throw them the finger and leave.  Scylla also has two pairs of lips unoccupied - playing with them could lead to something interesting.  You could tell Urta you never want to see her again.)");

        choices[3] = ["Heartbreak", heartBreakHotelInTelAdre];
        choices[4] = ["Leave", flipUrtaTheBird];
    }
    return { choices };
}

// Sober repeat Urta x Scylla
function urtaAndScyllaBoningLikeBitchesSober(player: Character): NextScreenChoices {
    CView.clear();
    ...
    if (User.settings.silly()) CView.text("For some reason, it makes you think of mythical balloonicorns.  ");
    CView.text("Drool bubbles out the corner of her inhumanly-plump, practically-inflated lips as she tries to vocalize her happiness at seeing you here; long strands of it sway with her unceasing motions.  The creamy, alabaster skin on the nun's cheeks colors the bright red of embarrassment as you admire her dedication to dick-sucking.  Hollowing, that same skin forms into deep divots as the nun starts to suck, eyes crossing to admire the trembling column of horse-cock before her when she pulls back.  Urta's strong fingers involuntarily seek out the nun's horns, and she pulls the suckling blow-job slut's crimson cock-hole back into place, flush against her fur once more.");
    ...
    if (!urtaLove()) CView.text("fuck-buddy");
    else CView.text("lover");
    CView.text("'s lusty slip-up and skittish responses, pinching her booty for good measure while you decide just how to get involved in the potential <i>ménage à trois</i>.\n\nWhat do you do?");
    player.stats.lustNoResist += 10 + player.stats.lib / 5;

    // Get worshipped requires gigantic dong
    let worship: ClickFunction;
    let fuck: ClickFunction;
    if (player.body.cocks.find(Cock.CockThatFits(urtaCapacity()))) {
        fuck = fuckUrtaWhileScyllaSucksSober;
        CView.text("  You could <b>fuck the fox</b> while she gets her oral pleasure.");
    }
    if (player.body.cocks.sort(Cock.Largest)[0].area > urtaCapacity()) {
        worship = dockWithUrtaUnderScyllasSoberSupervision;
        CView.text("  You could whip out your gigantic cock and let the blowjob queen <b>worship</b> a REAL dick.");
    }
    CView.text("  Whatever you do, with Scylla involved, it's sure to get out of hand.");
    // [Fuck Nun Nipple] [Fuck Urta] [Get Worshipped]
    return { choices: [["Fuck Fox", fuck], ["Worshipped", worship], ["", undefined], ["", undefined], ["Back", barTelAdre]] };
}
// Fuck Urta while scylla sucks sober
function fuckUrtaWhileScyllaSucksSober(player: Character): NextScreenChoices {
    CView.clear();
    const cockThatFits = player.body.cocks.find(Cock.CockThatFits(urtaCapacity()));
    CView.text("You peel out of your straining equipment to free your " + describeCocksLight(player));
    if (player.body.balls.count > 0) CView.text(" and [balls]");
    CView.text(", hauling out your sweaty, engorged meat.  Scylla, absorbed as she is with cum-thirst, doesn't seem to notice, though her nose twitches as the scent of your musky aroma mingles with the sex-stained air.  You gently squeeze behind the fox, folding her bushy, soft tail to the side and giving you something firm to support yourself when you let her flop back into you.  Her smooth fur feels wonderful on your [chest] and she squirms and gyrates against Scylla's sucking mouth.  Fairly dripping with moisture, the fox's black-lipped sex grinds on you, resting atop [oneCock], and your length is soon soaked with her heady, animalistic fem-spunk.");
    CView.text("\n\nUrta pants, \"<i>Go on... nnn... j-just put it in already!</i>\"  You readily oblige the over-endowed beast-woman by guiding your length into the clutching vice of her nethers, shivering at the onslaught of electric bliss that rockets up your " + describeCock(player, cockThatFits) + ".  You hilt inside her");
    if (cockThatFits.area >= 40) CView.text(", clearly bulging her stomach with your sizeable cock");
    CView.text(", sliding up into the slick, welcoming passage as if you always belonged.  Rivulets of liquid desire trickle from the stretched lips to run down ");
    if (player.body.balls.count > 0) CView.text("the front of your [sack]");
    else CView.text("your [legs]");
    CView.text(".  The gray-furred femme reaches back in an awkward hug, trying to hold you against her, speared together in the most intimate of embraces.");
    ...
    if (User.settings.silly()) CView.text("a cock and a sharp place.");
    else CView.text("your lusty bodies.");
    CView.text("  The velvet glove clasping your " + describeCock(player, cockThatFits) + " tightens without warning, accompanied by a full-body quake.  Urta's eyes roll back as some invisible force works through her, culminating in a muscular clench that you feel will pop your " + describeCock(player, cockThatFits) + " clean off your body.  You grunt, grabbing hold of the fox's tits for stability and wince from the almost painful tightness of her snatch.  The moment seems to drag on forever, but then, the vixen slumps back into your arms, allowing you to support her sagging weight.");
    CView.text("\n\nBeyond your sight, the fox-girl's soft-furred sack begins to round out, growing larger in spite of the sharp tip prodding hard at her balls. The delicate, plush nuts swell bigger, big enough to pull the skin around them tight, big enough to pull Urta harder onto your " + describeCock(player, cockThatFits) + ", big enough to be painfully poked, on the verge of being pierced.  Urta's vocalizations take on a keening, banshee-like quality, but before any harm can be done, a rosy glow envelops the fox-herm's poor, brutalized sack.  Stranger still, the nun's ram-like protrusions shorten in response, actually withdrawing back into her skull, pushed back by your lover's impressive, seemingly magical testes.");
    ...
    CView.text("\n\nMoaning, Urta whimpers, \"<i>S-so full... I - ungh - I... I can't hold it!  C-cuuuummmiiiiinnng!</i>\"  Her hips jerk forward hard and fast enough that it nearly pulls the gushing pussy off your " + describeCock(player, cockThatFits) + ", but Scylla isn't so lucky.  Her nose is squished almost flat into the fox's sweaty middle-fur, chin deep in ballsack.  She doesn't seem to mind; in fact, when drizzles of frothing fluid appear at the crinkled corners of the nun's ruby lips, her eyes roll back, exposing the whites.  Meanwhile, Urta's less-than-colt-sized dong swells, but not as a return to its former glory.  No, if anything, her musky dick is still shrinking.  The sheer size of the cum-globs pumping through her urethra are visibly stretching her horse-cock, giving the illusion of increased size.");
    ...
    if (player.body.balls.count > 0) CView.text("r [balls]");
    CView.text(" give the warm snatch a dose of virile medicine.  Urta, perhaps stirred from her own orgasm by the jets of hot fluid flooding her nethers, reaches back to squeeze your butt and whimpers, \"<i>Give it to me.</i>\"");
    ...
    if (player.body.cocks.length > 1) CView.text("  The unbound portion of your package bobs in the air before coming to rest in Urta's ass-crack.  A second later, the vulpine butt is coated with sticky white icing.");
    if (player.lactationQ() > 50) CView.text("  Your sensitive nipples let down, releasing their own heady flows to compliment your climactic pleasure, spraying into the gray fur relentlessly.");
    CView.text("  Throughout it all, your " + describeCock(player, cockThatFits) + " continues to do its level best to fill the fox.  Fireworks of uncontrollable pleasure go off one after the other, alternatively numbing and locking your muscles as you release all of your pent-up lusts, desires, and needs into the welcoming cunt.");
    if (player.cumQ() >= 500) {
        CView.text("  Urta's belly bloats from the thick deposit");
        if (player.cumQ() >= 2000) CView.text(", rounding more and more, a taut cum-tank that jiggles with your thick seed.");
        else if (player.cumQ() >= 1200) CView.text(", rounding out into a swollen cum-bank that jiggles under the weight of your seed.");
        else if (player.cumQ() >= 750) CView.text(", rounding out into a gravid bump that jiggles with liquid weight.");
        else CView.text(", rounding out into a nicely-sized bump.");
        CView.text("  You run your hands across it as you give a last few pumps.");
    }
    ...
    if (player.body.tallness >= 78) CView.text("head");
    else if (player.body.tallness >= 48) CView.text("narrow middle");
    else CView.text("curvy hips");
    CView.text(" you watch the nun's throat bulge almost double wide with each swallow of seed.  Tremendous volleys of semen are pumping down her esophagus rapidfire, and yet the suckling woman barely shows any sign of being full.  A clear puddle has sprung up underfoot, centered below the cock-sucker's crotch, and you marvel at the evidence of Scylla's pleasure.  She must be cumming non-stop!");
    ...
    if (urtaLove()) CView.text("  You give your lover a wet kiss and a fondle before you go, snickering when it makes her even longer.");
    else CView.text("  You give her a quick hug and depart, smiling when you realize the brief touch made her even longer.");
    knockUpUrtaChance(player);
    // -2 sens, -100 lust
    player.orgasm();
    player.stats.sens += -2;
...
function dockWithUrtaUnderScyllasSoberSupervision(player: Character): NextScreenChoices {
    CView.clear();
    const largestCock = player.body.cocks.sort(Cock.Largest)[0];
    CView.text("Unable to contain yourself, you sigh heavily and release your [armor].  Your immense package grows bigger with each beat of your heart, the blood-swollen veins pulsing hotly as they pump your cock to its full, obscene proportions.  It slowly undulates in the air as it fills to capacity with your lusty lifeblood, and the unmistakable aroma of your masculine ");
    if (player.body.vaginas.length > 0) CView.text("half's ");
    CView.text("musk fills the air.");
    if (player.body.balls.count > 0) CView.text("  Your [sack] swings free, bouncing off the back of [oneCock] when it thickens precipitously.");
    CView.text("  Sighing with relief, you gently stroke your length as you eye the two beauties meaningfully.");
    ...
    CView.text("\n\nScylla gestures at your " + describeCock(player, largestCock) + " and commands, \"<i>Go on, fuck it.  You know you want to.</i>\"  The black restraints poof into phantasmal smoke.  At the same time, your lusty hermaphrodite ");
    if (!urtaLove()) CView.text("friend");
    else CView.text("lover");
    CView.text(" lunges forward, scattering the smokey haze in the split second it takes her to slam face-first into your dick.  A flat-tipped spear plunges through the goop plugging your urethra and into your cock, scattering streamings of stringy sexual juice in a fan around it.  To you, the penetration isn't painful in the slightest.  Your " + describeCock(player, largestCock) + " is so big that the fox's own thick prick is a nice fit, and it feels like the slippery shaft is stroking you from the inside out, massaging nerve endings you didn't even know you had.");
    CView.text("\n\nUrta grabs around the edges of your " + describeCockHead(largestCock) + " and pulls herself down, her fuzzy balls slapping wetly against the underside of your boner before pulling away again.  She starts kissing the top of your " + describeCock(player, largestCock) + " with wild, unrestrained enjoyment.  Moving with the subtlety of a jackhammer, she pistons your inner hole wildly, grunting and moaning as she throws herself completely into debauchery.  The reluctant captain is fucking your immense member with abandon and shows no signs of stopping.  Scylla watches with detached amusement and idly climbs atop the column of jiggling dickmeat.  Her heavy udders press down on either side of your cock, but all she does is prop her face up on her hand and watch Urta fuck you hard enough to splatter pre-cum on everything in a five foot radius.  Thankfully, the movements cause your cock to flex and shift enough that the nun's inertia-bound breasts feel as if they're bouncing along the sides of your boner, unintentionally giving you a tit-fuck while your cock-pussy is inelegantly plowed.");
    CView.text("\n\nUrta's apple-sized nuts pull tight against her body as she works your urethra, and somehow, she remembers that she ought to warn you.  \"<i>Can't stop!  G-g-gonna cuuuUUUUUM!!!</i>\" she screams, slamming her dick down with enough force to distort your " + describeCockHead(largestCock) + ".  A moment later, you feel her swollen sack drag against your underside, and a warm explosion blooms inside your member.  The rushing white river runs through your length and into your body, giving you the oddest sensation of pleasant fullness and complete violation.");
    if (player.body.balls.count > 0) CView.text("  Your [balls] quiver a little when the rushing seed reaches them, and you swear you hear them slosh with the horse-spunk pouring into their own reservoirs.");
    else CView.text("  Your innards gurgle slightly as the rushing seed travels through them, and you swear you can feel your prostate stretching to accommodate being suddenly stuffed with horse-spunk.");
    ...
    if (player.body.balls.count > 0) CView.text(" while your enormous balls gurgle and begin to empty");
    CView.text(".  The titanic, room-filling cock lifts up off the ground by a few inches, supported by the thick bulge of cum that rushes from ");
    if (player.body.cocks.find(Cock.HasSheath)) CView.text("sheath");
    else CView.text("base");
    CView.text(" to tip in an instant. It passes through the tight crimson seal, distending the nuns neck for a moment before it settles in her belly.  She looks almost pregnant on a mix of horse and champion-spunk for a moment, but then her belly narrows and her tits swell - just in time to take the next gush of your demonically-blessed orgasm.  Scylla appears to be in heaven, and as she swallows, the darkness leaves her eyes.  What remains is wide-eyed irresistible pleasure, and the vibrations of orgasmic screams vibrate through your swollen " + describeCockHead(largestCock) + " as you dump wave after wave of spooge through the clinging orifice.  A shiny puddle of feminine fluid widens beneath the nun's orgasm-wracked frame, and just once, you wish you could stick your head under her robes and watch it happen.");
...
function dockWithUrtaUnderScyllasSoberSupervision2(player: Character): NextScreenChoices {
    ...
    if (!urtaLove()) CView.text("waves ");
    else CView.text("kisses you ");
    CView.text("goodbye, she says, \"<i>Thank you.</i>\"");
...
// [Bird and Leave]
function flipUrtaTheBird(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("You curl your hand into a fist and extend your middle finger at the alcoholic and cum-slut.  You've got better things to do than waste your time watching those two indulge their addictions.\n\n");
    // (lose urta love points.)
    urtaLove(-1);
    // (lose urta dick comfort points.)
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY -= 2;
    if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY < 0) UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY = 0;
...
// [Jerk Off And Watch]
function watchTwoHotBitchesAndJerkIt(player: Character): NextScreenChoices {
    urtaSprite();

    CView.image("urta-scylla-masti");
    CView.text("You open your " + player.inventory.equipment.armor.displayName + " and sit down on a crate no more than a few feet away from the orally fixated couple, fishing out " + describeOneOfYourCocks(player));
    if (player.body.vaginas.length > 0) CView.text(" and exposing your " + describeVagina(player, player.body.vaginas.get(0)));
    CView.text(".  The nun's trademark demonic stubs commence sprouting, parting her hair until they rise up at least six inches from her forehead.  Meanwhile the poor, sexually frustrated vixen's balls are swelling up like balloons in Scylla's hands, growing so large that the fox's sack sags around the larger woman's hands.  The curly-haired oral slut tries to fellate and support Urta's growing size at the same time, and definitely appears to be struggling with it.\n\n");

    CView.text("You stroke " + describeOneOfYourCocks(player));
    if (player.body.vaginas.length > 0) CView.text(" and touch your " + describeVagina(player, player.body.vaginas.get(0)));
    CView.text(", watching the puffed-up, fire engine-red cock-suckers bob on the trembling horse-shaft.  They slide up and down, up and down, then you see the hint of a tongue sliding around the cock and darting through the sheath.  Urta moans appreciatively at the act, and she turns her head to watch you as you masturbate to the exceptional scene before you.  Her eyes meet yours while her hands curl around Scylla's horns and pull the nun deeper into the sheath.  The fox loses her focus, her tongue rolls out, and her balls tremble and rise tight against her body.  She's cumming!\n\n");

    CView.text("Scylla's neck is pulled tight as heavy bulges of cum work their way up the vixen's mismatched meat-pole, through the puffy, cock-sucking lips, and down the nun's throat into her hungry gullet.  Though only a few seconds pass, you watch Scylla's remarkably elastic midsection begin to bloat with what looks like a 7-month pregnancy.  She keeps her eyes closed the entire time, just silently suctioning down each plus-sized cream deposit as it's spurted out.  The only sounds in the room are Urta's half-whined moans of pleasure and the rapid slaps of your self-stimulation.\n\n");

    CView.text("The gray-furred hermaphrodite's balls are shrinking with each blast, contracting and pausing as her body launches each volley, but they've only lost about half their enhanced size and Scylla's getting rounder than she has any right to be.  The nun seems ready for this though, and hums in concentration.  Her belly stops growing immediately, but each of her already prodigious milkers begins to enlarge, sloshing noisily like a barrel being filled.  You nearly push yourself past the edge at the sight of the growing melons, but hold back – you don't want to blow until you see just how far this goes.\n\n");

    CView.text("Urta's hands let go of the corrupted woman's horns and she slumps back against the wall, but the nun just wraps her arms around the blissed-out vixen and suckles harder.  You keep watching and stroking, entranced by the sight of the spooge bubbles vanishing into Scylla's neck and plumping up her tits.  Urta's eyes are rolled back and she's drooling a runnel of spittle down her tit, around one of her black nipples, and down her belly, but she keeps cumming, even though her balls are nearly back to normal.  The poor fox is utterly unable to cope with the sensations running through her drunken form, but she's clearly enjoying it.\n\n");

    CView.text("Just as Scylla's tits start to drag against the floorboards, she pulls back, letting Urta blow a few long ropes of thick horse-cum over her face and hair.  The nun sighs happily and begins to gather it in her fingers, licking it up while the exhausted fox's cock slowly deflates, dripping a trail along the floor.  Urta recovers surprisingly quickly, blinking her eyes and pulling her tongue back into her mouth with a sigh of utter, sated contentment.  You held back for so long, but watching the nun pick pearly strands of animal cum from her hair and swallow it down like a whore is just too much.\n\n");

    CView.text("You pump hard on " + describeOneOfYourCocks(player));
    if (player.body.cocks.length > 1) CView.text(" in turn");
    CView.text(", shooting hot ropes of semen that drape over the satisfied girls.  After so much edging, and such a hot scene, your orgasm is far more intense than any normal masturbation session.  Your whole body trembles while it spurts and squirts, making sure both of the addicts get at least a little bit of your white goo on their bodies.  ");
    if (player.cumQ() >= 250) CView.text("A particularly coherent blast of seed splatters over Urta's tits, glazing her gray fur and black nipples with a thick layer of dripping spunk.  ");
    if (player.cumQ() >= 450) CView.text("Another huge gout takes Scylla straight in the face, completely soaking her with yet more salty mess that drools onto the curve of her pendulous, black-clad tits.  ");
    if (player.cumQ() >= 700) CView.text("A few more big blasts of the stuff fire off, spotting the pair with fat, sticky blotches.  ");
    if (player.cumQ() >= 1000) {
        CView.text("Your body is so productive that it utterly drenches both participants in the stuff.  Runnels of it drip over their curves while fat droplets pool into a");
        if (player.cumQ() >= 2000) CView.text("n inches-deep");
        CView.text(" puddle on the ground.  ");
    }
    CView.text("Finally spent, you slump back and smile.\n\n");

    CView.text("Scylla looks shocked, as if she's seeing you for the first time, but her nose twitches and she starts to shovel your seed into her mouth with greedy abandon.  Even so, her body rapidly 'digests' the fluid intake, and the nun starts to slim up before your eyes.  Urta wobbles up onto shaky legs and stumbles into you, giving you a wet kiss that tastes faintly of your salty leavings and then passionately hugs you.  She breaks the embrace and slurs, ");
    if (UrtaFlags.UNKNOWN_FLAG_NUMBER_00143 === 1) CView.text("\"<i>We need to do thish again!</i>\"");
    else CView.text("\"<i>You're the besht " + mf(player, "man", "woman") + " a girl could ashk for.</i>\"");
    CView.text("  Before you can answer, she gives your ass a slap and finds a water barrel to wash up in.  Scylla finishes her 'meal' and looks up at you, her alabaster skin turning completely crimson before she too runs off to clean up.\n\n");
    CView.text("You smile happily and pick up your " + player.inventory.equipment.armor.displayName + ".  It's past time you checked up on your ");
    if (urtaLove()) CView.text("  You give your lover's back a wistful grin and muse over how good her taste in women is as you leave.");
    player.orgasm();
    // +luv
    urtaLove(2);
    // +comfort
...
function makeUrtaSitOnYourLapWithScylla(player: Character): NextScreenChoices {
    urtaSprite();

    const cockThatFits = player.body.cocks.find(Cock.CockThatFits(urtaCapacity()));
    const secondCockThatFits = player.body.cocks.filter(Cock.CocksThatFit(urtaCapacity()))[1];

    CView.image("urta-scylla-lapfuck");
    CView.text("You smirk at the drunken fox and inform her in no uncertain words that you'll do more than watch.  She blinks at you, her inebriated mind unable to process new thoughts as you stalk forwards and sit behind her on the crate");
    if (player.body.legs.type === LegType.NAGA) CView.text(", snaking your tail around her legs");
    else if (player.body.legs.type === LegType.GOO) CView.text(", enveloping her legs with your goo");
    else CView.text(", straddling your " + describeLegs(player) + " around her hips");
    CView.text(".  You grab her narrow waist and pull her up, dragging Scylla's cock-locked head up with her and aligning your " + describeCock(player, cockThatFits) + " with her backdoor before you start to lower her back down.  ");
    if (secondCockThatFits) CView.text("Your " + describeCock(player, secondCockThatFits) + " even manages to get in line with her drooling, black-lipped pussy.  ");
    if ((player.body.cocks.length > 1 && !secondCockThatFits) || player.body.cocks.length > 2) CView.text("You allow your remaining, unused dickflesh to flop against her back, leaking pre-cum into her fur.  ");
    CView.text("You aren't sure if it's the alcohol in her or her own anal talents, but she slides down without a complaint, pulling Scylla's vacuum-like mouth down with her");
    if (secondCockThatFits) CView.text(" and squeezing you tightly with her sopping cunt");
    CView.text(".\n\n");

    CView.text("She grunts, \"<i>");
    if (cockThatFits && secondCockThatFits) CView.text("Oooh, and in both holes?  You naughty " + mf(player, "boy", "girl") + ", you!");
    else if (urtaLove()) CView.text("Oooh yeah, fill me love!");
    else if (randInt(2) === 0) CView.text("Oooh, you know how to treat a lady!");
    else CView.text("MMmm, that's nice!");
    CView.text("</i>\"  She's actually trying to watch you over her shoulder now, completely ignoring the cock-addled nun as the latter bobs on her horse-meat.  You make the most of it and ");
    if (urtaLove() || player.stats.cor <= 40) CView.text("kiss her full on the mouth");
    else CView.text("bite her neck");
    CView.text(" while you grab her soft, furred asscheeks and lift her up, feeling the exquisite texture of her pucker ");
    if (secondCockThatFits) CView.text("and twat ");
    CView.text("massaging your manhood the whole way.  Before you go too high, you drop her, and let her weight impale her back on top of you.  Her rapidly expanding balls slap into your own and smear them with her pussy juice.\n\n");

    CView.text("Urta's tail curls snugly around your waist, and the drunken fox takes over, lifting herself up even as Scylla's magic swells her balls past the size of grapefruits.  She openly moans when she reaches the apex, and smoothly lowers herself back down, squeezing and stroking you with her internal muscles while trying to endure the nun's incessant suckling.  The measured ");
    if (secondCockThatFits) CView.text("double penetration");
    else CView.text("anal fuck");
    CView.text(" drives you wild, but being on the bottom as you are, there isn't much you can do aside from grabbing fistfuls of vixen-ass and slapping it for encouragement.  Urta gets the message and begins to bounce vigorously, mewling and moaning as her balls get large enough to reach the floor.  You lend a hand and play with her sheath, feeling the flesh underneath quake and tremble as the fox's orgasm hits.\n\n");

    CView.text("Scylla's neck is pulled tight as Urta's urethra is stretched wide by huge bulges of cum.  You can actually feel them sliding past the fingers you've dug into her sheath.  No sooner than one squeezes past is another on its way.  You wonder how Scylla can handle such incredible volumes and the only answer you can think of is magic.  Whatever the case, her swallowing gets noisier and Urta starts bouncing even faster atop your " + describeCock(player, cockThatFits));
    if (secondCockThatFits) CView.text(" and " + describeCock(player, secondCockThatFits));
    CView.text(".  You pinch one of her shiny black nipples between your fingers as encouragement, and she actually howls with delight.  The herm vixen's cum-pumping cock starts spasming in your grip, the order of her previous cum blasts dissolving to turn her cock into a frenzied, constantly squirting spooge-hose.  Every spurt of jism that Scylla swallows is one less in the fox's magically-enhanced ballsack, and the unnatural, mighty orbs begin to shrink back towards their normal size with alarming rapidity.\n\n");

    CView.text("Scylla's growing belly and tits smush around your " + describeLegs(player) + ", swelling further and further as she tries to devour all the creamy horse-spunk the fox is hosing into her.  Yet the touch of long, gentle fingers caressing your " + describeSack(player) + " and the gradual inflation of your " + describeBalls(true, true, player) + " lets you know that the slutty nun's discovered another source of seed.  You groan as warmth and stimulation make your orbs churn and slosh, growing not just in size but also in raw, animal NEED.  Driven to near madness by the drunken fox's delicious backside and the building, backed-up cum in your sack, you grab hold of Urta with both hands and start jackhammering her asshole");
    if (secondCockThatFits) CView.text(" and pussy");
    CView.text(", pounding the lithe fox as if your life depended on your orgasm.\n\n");
    ...
    CView.text("You cum, hard enough to make you cross your eyes and slam Urta's hips into you with painful force, even through the padding of her supple buttcheeks.  The first ass-filling wave of jism completely packs her backdoor, surrounding you in a sea of your own sticky moisture.");
    if (secondCockThatFits) CView.text("  Meanwhile, your other " + nounCock(secondCockThatFits.type) + " utterly fills her womb, stuffing it so completely that the fox's belly rounds out.");
    if ((player.body.cocks.length > 1 && !secondCockThatFits) || player.body.cocks.length > 2) CView.text("  A wave of the stuff splatters her back, glazing her from the shoulder-blades to the crack of her ass and nearly catching you in the chin. Maybe there is such a thing as too many cocks?");
    CView.text("  Scylla's hands rub the fox's belly tenderly as she humps away at the lewd nun, using whatever magic she has to ensure you won't damage the drunken vixen as you continue to push larger and larger amounts of semen into her.  After three more blasts, she runs out of room inside her, and it starts squirting back out with each fresh stroke, soaking your waist, " + describeLegs(player) + ", and rapidly shrinking balls.  You lose track of time, knowing only the pleasure of orgasm and the feeling of your own spooge pumping into your lover until the unholy cumsplosion finally ends.\n\n");

    CView.text("You regain your wits to the sight of a very pregnant-looking Urta and Scylla leaning on each other and trying to make their way to a water barrel to clean up.  The vixen is so full she looks like a mother in her ninth month of pregnancy with triplets, but as she starts to clean up Scylla gently presses on the fox's belly and devours the cum she squeezes out.  Urta shakes, but doesn't emit a sound beyond a few strangled half-moans of pleasure.  If the fresh horse-cum soaking the nun's habit is any indicator, Urta must have gotten off again during your orgasm.  Wild!\n\n");

    CView.text("You grab a rag and wipe up before donning your " + player.inventory.equipment.armor.displayName + ".  Urta saunters over, looking like she's sobered up quite a bit.  She's got ");

    if (urtaLove()) CView.text("a contended smile on her face, and gives you a long, slow kiss that practically radiates her love for you.");
    else if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY < 11) CView.text("a nervous smile on her face, but gives you a quick peck all the same.");
    else CView.text("a satisfied smile on her face, and crushes her lips against yours in a passionate kiss.");
    CView.text("  She says, \"<i>I think I'll need to help Scylla to her room... somehow whatever she did to me must have pulled all the alcohol out of me and into my cum.  Don't worry, I'll ");
    if (UrtaFlags.UNKNOWN_FLAG_NUMBER_00143 <= 1) CView.text("pay for the door and ");
    CView.text("make sure she's ok.</i>\"\n\n");

    CView.text("Did her cock just twitch?  No, you scold yourself; it couldn't have, not after two orgasms like that.  Urta hooks her arm around the giggling nun's midsection and pulls the stumbling sister towards the doorway.  Scylla looks back at you and half-giggles, half-slurs, \"<i>Tee-hee, thish ish so much fun!  I love you guysh!</i>\"\n\n");

    CView.text("You make your way out with your head low, but you do see Scylla and Urta walking up the stairs towards a truly private room.  They each look a little unsteady and neither of Scylla's hands can be seen, but you're sure they'll be ok.");
    player.orgasm();
    player.stats.lib += 1;
    player.stats.sens += 1;
    player.stats.cor += .5;
    ...
    urtaLove(3);
    // +comfort
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY += 3;
    player.body.cumMultiplier += 2;
    player.body.balls.size += 1;
    if (secondCockThatFits) knockUpUrtaChance(player);
    return { next: returnToCampUseOneHour };
}

// [No Scylla]
function tellUrtaNoMoreScylla(player: Character): NextScreenChoices {
    urtaSprite();

    UrtaFlags.UNKNOWN_FLAG_NUMBER_00147 = 1;
    CView.text("You let Urta know that you're willing to overlook this, but that Scylla is a slippery slope that's best avoided.  She nods, a sober expression on her vulpine face.  Once you finish, she replies, \"<i>I understand, but you need to know – when 'it' gets too much for me to handle... if you aren't around to help I'm going to see Edryn.  S-she helps... if it wasn't for her I probably would've given up and done something stupid a long time ago.  What we have is more important, but unless we can get together every couple days I'll NEED to visit her.  ");
    if (urtaLove()) CView.text("Just please, don't doubt my love for you.  ");
    CView.text("Do I need to stop drinking too?");
    // [Drink More] [Drink Less] [No Change] [Leave Her]
    return {
        choices: [
            ["Drink More", tellUrtaToBeADrunkenHussy],
            ["Drink Less", tellUrtaToStopBeingALush],
            ["No Change", tellUrtaToStayTheSame],
            ["Leave Her", leaveUrtaAfterScyllaConfrontation]
        ]
    };
}
// [Scylla Okay]
function tellUrtaMoreScyllaIsFine(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("You give Urta a raunchy, lewd smile and let her know that you're okay with her 'helping' Scylla, and vice-versa, so long as they don't mind you popping in.  She smiles and gives you a wink, but her expression darkens after a moment of thought.  Urta interrupts to mention, \"<i>There's something else.  If I can't find you or Scylla and I NEED to cum, I'll have to visit Edryn.  She's probably the only thing that's kept me sane, and if you aren't around to help with my needs I'll visit her.  ");
    if (urtaLove()) CView.text("Just remember that YOU'RE the one I love, not her.  ");
    else CView.text("Me and her, we've been partners in the guard for a long time, but the sex is just mechanical, to sate us.  You don't have to worry about her stealing me away.  ");
    CView.text("But what about my drinking or how I get when I'm drunk?  It doesn't bother you, does it?");
    // [Drink More] [Drink Less] [No Change] [Leave Her]
    return {
        choices: [
            ["Drink More", tellUrtaToBeADrunkenHussy],
...
// [Drink More]
function tellUrtaToBeADrunkenHussy(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("You ");
    if (player.stats.cor < 33) CView.text("blush a little bit");
    else if (player.stats.cor < 66) CView.text("smile nervously");
    else CView.text("grin widely");
    CView.text(" and explain that when she gets drunk, lets her guard down, and gets a little more aggressive... you like it.  A lot.  Her ears perk up at your words, though her expression is a little uncertain while you explain it.  By the time you finish, something warm brushes by your " + describeLeg(player) + " and gently 'thunks' the table.  Clearly she's as into the idea as you.\n\n");

    CView.text("Urta smiles, lewdly at first, though it carries a bit of a predatory glint as she waves down a waitress and orders a full bottle of Barkardi 151.  You give her a rueful smile, a stroke under the table, and a kiss just bursting with tongue before you conclude the conversation.  Urta's going to be a lot of fun from now on...");
    UrtaFlags.UNKNOWN_FLAG_NUMBER_00146 = 1;
    player.stats.lust += 5;

    return { next: returnToCampUseOneHour };
}
// [Drink Less]
function tellUrtaToStopBeingALush(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("You sigh and explain that her alcoholism isn't helping anyone – not her and certainly not her relationships with others.  She nods with a knowing, sober look on her face as you recount how much harder her drinking has made your relationship.  She promises you that she won't ever get that drunk again, though she warns that it will be hard to swear off alcohol entirely.  A look of resolve enters her eyes, and she leans over the table to kiss you on the lips.  The two of you wrap up the conversation knowing that you've probably seen the last of drunken Urta.\n\n");
    UrtaFlags.UNKNOWN_FLAG_NUMBER_00146 = -1;
    return { next: returnToCampUseOneHour };
}
// [Don't Change]
function tellUrtaToStayTheSame(player: Character): NextScreenChoices {
    urtaSprite();

    CView.text("You smile and ask her why you would ever want her to change?  You ");
    if (urtaLove()) CView.text("love");
    else CView.text("like");
    CView.text(" everything about her.  She blushes hard when you tell her that, clearly enjoying the flattery and thrilled to hear that you're okay with how she chooses to blow off steam when she's not on the job.  The two of you joke around for a little longer, but before long, it's time you were on your way.  Urta's STILL blushing as she gives you a good-bye kiss – you must have earned some points with her today!");
    // (+love score)
    urtaLove(5);
    return { next: returnToCampUseOneHour };
}

function amilyXUrtaUrtaFallout(player: Character): NextScreenChoices {
    UrtaFlags.UNKNOWN_FLAG_NUMBER_00350 = 2;
    urtaSprite();

    CView.text("You ");
    if (!player.body.legs.isTaur()) CView.text("pull up a chair");
    else CView.text("sit down");
    CView.text(" at the table and stir Urta from whatever introspective thoughts she was lost in.  \"<i>I fucked her,</i>\" your well-endowed lover states out loud, looking up at you with bloodshot eyes.  \"<i>Your mousey girlfriend came up here all fire and brimstone about you cheating on her, and somehow I wound up fucking her senseless,</i>\" continues the disturbed vixen.  She pauses to take a dreg of some vile drink and mumbles, \"<i>At first, we were pissed off at each other and at you.  So we sat down and bitched.  I was having a beer, and I guess Amily wanted to be sociable, so she got one too.  We bitched.  We bitched a lot.  But as the beers piled up, I realized I liked her... I liked her a lot.  I get why you'd want to help her with her people.  Yet... as the night wore on, we both got pretty plastered.  I started getting myself stronger drinks so she wouldn't be the only one getting hammered, and well... I'm... I'm sorry.</i>\"\n\n");

    CView.text("Urta sobs, \"<i>I'm a monster.</i>\"  Her emerald eyes fix on yours for a moment before returning to her drink, awaiting your reply.\n\n");

    CView.text("(You can tell her that it's okay, and you're actually kind of glad that your girls are getting along in such a fun way.  Or, you could crush the bitch's heart for daring to fuck YOUR woman.)");
    // [It's Okay] [Heartbreak]
    return { choices: [["It's Okay", itsOkayUrtaAmilyIsHot], ["Heartbreak", crushUrtasHeart]] };
}

// [It's Okay]
function itsOkayUrtaAmilyIsHot(player: Character): NextScreenChoices {

    urtaSprite();
    CView.text("You reach out, grab Urta's shoulders, and look at her until she raises her face to look back at you.  Before she can look away, you tell her that it's fine - you had hoped your lovers would come to love each other as you do.  Urta wipes away a tear and gives you a nervous smile when you suggest that perhaps the three of you could get together sometime.\n\n");
    CView.text("Once you've finished, Urta leans forwards and plants a salty, tear-stained kiss on your lips.  The two of you spend some more time bonding, before you realize you should probably check up on your camp and go.");
    return { next: returnToCampUseOneHour };
}
// [Heart asplosion]
function crushUrtasHeart(player: Character): NextScreenChoices {

    urtaSprite();
    CView.text("You reach across the table and slap her arrogant muzzle so hard that you see her tears land on a nearby customer's sleeve.  Urta recoils, clutching a hand to her reddening cheek as she watches you with a shocked, sad expression.  You tear into her verbally, calling her a lush, a slut, and a traitorous bitch.  Each damning declaration causes the fox to flinch, as if struck by a switch, and you keep at it, determined to drive home how irrevocably she has fucked up.  You accuse her of trying to steal away your mouse-wife and proclaim her to be worse than the demons infesting the wastes around your \n\n");

    CView.text("Urta's lower lip quivers while tears stream down her face in an unimpeded river of sadness.  She drops her glass, the shattering explosion of sound coinciding with the ultimate breaking of her heart.  The fox stands up on wobbling knees and runs from the room sobbing.  You don't think she'll be a problem anymore.");
    urtaLove(-100);
    UrtaFlags.URTA_PC_LOVE_COUNTER = -1;
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY = -1;
    return { next: returnToCampUseOneHour };
...
export function chastityBeltFun(player: Character, cameFromSexMenu: boolean = true): NextScreenChoices {
    ...
    CView.clear();
    if (cameFromSexMenu) {
        CView.text("Eyeing your foxy lover, you tell her to lose the condom, you have something else planned.\n\n");
        CView.text("“<i>Lose it?  What do you have in mind?</i>” Urta asks.  She looks past you towards her cupboard.  “<i>Oh, no, not that belt!  ...Do you really enjoy seeing me make such a mess of myself when I finally get it off?</i>” she protests.\n\n");
...
// Approaching Urta:
// (Regular Urta)
function QBsTalkExpack(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    // (Lover Urta)
    if (urtaLove()) CView.text("When you approach Urta, she smiles to see you.  However, there is no rapid swivelling in her seat to avoid banging her horse-cock against the table; evidently she's not in the mood for some sex right now.  You sit down beside her, and she happily loops her arm around you, planting a tender kiss on your lips, before almost apologetically stating, \"<i>Hey there, lover.  Want to talk for a change?</i>\" with a twinkle of good-natured mirth in her eyes.");
    else CView.text("When you approach Urta, you are almost surprised at the lack of an audible 'thump'.  Seeing your confusion, the herm fox quips, \"<i>What's with the look?</i>\" while raising an eyebrow.  When you indicate that you had expected to hear her demonstrating her excitement to see you, she blushes and looks embarrassed.  \"<i>Well... you do a pretty good job of letting my little pony blow off some steam, ya know, [name]?</i>\"");
    // [Urta] [Edryn] [The Watch] [Back]
    return urtaDialogueMenu(player);
}

function friendsTalkExpack(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You approack Urta's table and she motions for you to take a seat.  She finishes writing something on one of the pieces of paperwork scattered over the table, takes a sip of her drink and leans back in her chair.  “<i>Nice to see you again " + player.desc.name + ".  What can I do for you?</i>”");
    return urtaDialogueMenu(player);
}

export function urtaDialogueMenu(player: Character): NextScreenChoices {
    const choices: ScreenChoice[] = [];
    choices[0] = ["Urta", urtaDiscussesSelf];
    choices[1] = ["Edryn", urtaDiscussesEdryn];
    choices[2] = ["The Watch", urtaDiscussesTheWatch];
    if (!urtaJustFriends()) choices[3] = ["Alcoholism", urtaDiscussesAlcholism];
    if (UrtaFlags.KATHERINE_UNLOCKED >= 4) choices[5] = ["Katherine", urtaDiscussessKatherine];
    if (urtaKids() > 0 && !player.inventory.keyItems.has("Spare Key to Urta's House"))
        choices[4] = ["Visit Kids", visitKidsFirstTime];
    else
        if (UrtaFlags.FIRST_TIME_AT_URTA_HOUSE > 0) choices[4] = ["Her House", talkToUrtaAboutHerHouse];
...
// [=Urta=]
function urtaDiscussesSelf(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You tell Urta you'd like to talk about her a little.");
    if (!urtaLove())
        CView.text("“<i>You wanna talk about me?</i>” she asks" + (urtaJustFriends() ? "" : ", blushing softly") + ".  “<i>Well, there isn't THAT much to tell, but what do you want to know?</i>”   She seems surprisingly pleased to have the chance to talk.");
    else CView.text("“<i>Well, I don't think there's that much to tell,</i>” she says with a smile.  She idly blows a bang out of her face and exhales, “<i>Well, lover, I'm an open book for you, what do you want to know?</i>”");
    ...
    else if (UrtaFlags.URTA_QUEST_STATUS === .5)
        return {
            choices: [
    ...
    else if (UrtaFlags.URTA_QUEST_STATUS === 1)
        return {
            choices: [
                ["Fertility", urtaFertilityDiscussion],
                ["Romance&Sex", urtaDiscussesSexAndRomance],
                ["Employment", urtaDiscussesEmployment],
                ["Prejudice", urtaDiscussesPrejudice],
                ["Back", urtaDialogueMenu],
...
export function flirtWithUrta(player: Character): NextScreenChoices {
    CView.clear();
    if (UrtaFlags.URTA_PC_LOVE_COUNTER === -1) { // In friends mode
        CView.text("You ask Urta if, living in Tel'Adre, it's easier to resist the powerful feelings that come from living in this world.\n\n");
        CView.text("“<i>I've lived here my whole life, so I don't have anything else to compare with.  What do you mean by 'feelings'?</i>”\n\n");
        CView.text("You tell her that in your world you never felt a deep urge to strip off all your clothes and " + (player.stats.cor < 75 ? "make sweet love to" : "slake your lust on") + " the nearest person.\n\n");
    }
    else {
        CView.text("You tell her that you don’t have any problem with herms, in fact you find some of them quite attractive.\n\n");
        CView.text("Urta gulps down a bit more of her booze and you decide to have a little fun.  ");
    }
    CView.text("You quietly slide your " + (player.body.legs.isNaga() ? "tail" : "foot") + " across under the table and rub it against the inside of her leg.  Urta practically jumps out of her seat, then tries to pretend nothing happened.\n\n");
    CView.text("You’re pretty sure most of the bar noticed, but you don’t care.  You keep rubbing and feel a thump as something large smacks against the underside of the table.  Urta drains the last of her drink and whispers, “<i>" + (UrtaFlags.URTA_PC_LOVE_COUNTER === -1 ? "What about Kath?  I... I wouldn't..." : "Please - it’s so hard...") + "</i>”\n\n");
    CView.text("You never find out exactly what she was going to say.  When you start to rub your " + (player.body.legs.isNaga() ? "tail" : "foot") + " in little circles against the root of her huge cock Urta loses her train of thought and just leans back in her chair, trying to keep from spraying her pent up cum across the floor.\n\n");
    CView.text("You ease up so she has a chance of understanding what you’re saying.  “<i>" + (UrtaFlags.URTA_PC_LOVE_COUNTER === -1 ? "I'll explain everything to Kath.  You want this as much as I do, right?" : "Yes, some chicks, with their big dicks, just do it for me.") + "</i>”  You slide " + (player.body.legs.isNaga() ? "the tip of your tail" : "your toes") + " a little lower and start moving it in little circles against her balls.\n\n");
    CView.text("Before Urta pops you get up from the table and tell her to meet you out back, right where things went off the rails that first time.  Urta nods eagerly and the edge of the table rises slightly.\n\n");
    CView.text("You wait outside, hiding in a shadowy corner.  You got Urta so excited that it’s a good fifteen minutes before she throws open the back door of the bar.  She looks hungry, yet unsure.  You’re guessing she’s had such terrible experiences with people in the past that despite your obvious interest she still can’t believe you want her.\n\n");
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY = 0; // urtaFuckHer will raise it to 1
    UrtaFlags.URTA_PC_LOVE_COUNTER = 0; // Reset both flags to put Urta back to her 'never met you before' state, ready for her first fuck
    return urtaFuckHer(player, true); // Call return urtaFuckHer(player, ) to put you on track for much Urta loving
}

function urtaFertilityDiscussion(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Urta traces a finger around the corner of her drink, \"<i>So, what do you think, [name]?");
    if (UrtaFlags.URTA_FERTILE === 1) CView.text("  Should I stay fertile?");
    else CView.text("  How about I cut loose and go fertile?  Can you imagine me with a nice, round pregnant belly and a cock virile enough to get you pregnant just from looking at it?");
    if (UrtaFlags.URTA_PREG_EVERYBODY === 1) CView.text("  Or did you come to ask me not to be fertile and virile around anyone else?");
    else {
        CView.text("  Would you mind me being able to put a bun in Edryn's oven?  Imagine all the lil foxtaurs running around!");
        if (UrtaFlags.EDRYN_NUMBER_OF_KIDS > 0) CView.text("  After all, you already had a turn!  Fair's fair!");
    }
    CView.text("</i>\"  She winks at you before blowing you a playful raspberry.\n");

    const choices: ScreenChoice[] = [];
    if (UrtaFlags.URTA_FERTILE === 1) {
        CView.text("\n<b>Urta is currently fertile.</b>");
        choices[0] = ["No Fertile", urtaFertilityToggle];
    }
    else {
        CView.text("\n<b>Urta can not currently get pregnant (or inseminate anyone, let alone you).</b>");
        choices[0] = ["Go Fertile", urtaFertilityToggle];
    }
    if (UrtaFlags.URTA_PREG_EVERYBODY === 1) {
        CView.text("\n<b>Urta thinks you're okay with her 'sharing the love (and kids)' with any other sexual partners she might have.</b>");
        choices[1] = ["No Preg", urtaPregOthersToggle];
    }
    else {
        CView.text("\n<b>Urta will not knock up or be knocked up by anyone else unless you tell her otherwise.</b>");
        choices[2] = ["Share Preg", urtaPregOthersToggle];
    }
    CView.text("\n\n<b>(Note: There is no content for Urta to impregnate or be impregnated by other NPCs, yet.)</b>");
    choices[4] = ["Back", urtaDialogueMenu];

    return { choices };
}

function urtaFertilityToggle(player: Character): NextScreenChoices {
    CView.clear();
    if (UrtaFlags.URTA_FERTILE === 1) {
        CView.text("\"<i>Well, I guess that's okay.  But you better make it worth my while later, okay?</i>\" Urta says with a half-hidden sigh.");
        UrtaFlags.URTA_FERTILE = 0;
    }
    else {
        CView.text("Urta throws her hands up and cheers, \"<i>Yesss!</i>\"  Realizing that everyone is looking over at the two of you now, she gives a sheepish smirk and glances back at you.  \"<i>I can't wait!</i>\"");
        UrtaFlags.URTA_FERTILE = 1;
    }

    return {
        choices: [
            ["Back", urtaFertilityDiscussion]
        ]
    };
}

function urtaPregOthersToggle(player: Character): NextScreenChoices {
    CView.clear();
    if (UrtaFlags.URTA_PREG_EVERYBODY === 1) {
        CView.text("\"<i>Awww, are you sure?  Wouldn't you like ");
        if (player.body.cocks.length > 0)
            CView.text("to help me make Edryn's belly bloat with our seed, and then play 'Guess Who Is The Father'?");
        else
            if (player.body.vaginas.length > 0)
                CView.text("to have me get the both of you pregnant at the same time, so we can all enjoy it together?");
            else
                CView.text("see me put this thing to work in the way that it's meant to?");
        CView.text("</i>\" Urta says, giving you a playful pinch.");
        UrtaFlags.URTA_PREG_EVERYBODY = 0;
    }
    else {
        CView.text("Urta smiles salaciously, like a cat that got the cream.  Then, without warning, she kisses you square on the lips, passionately frenching you.  After nearly a full minute of frenzied kissing, she says, \"<i>This is gonna be fun!</i>\"");
        UrtaFlags.URTA_PREG_EVERYBODY = 1;
    }
...
// [=Edryn=]
function urtaDiscussesEdryn(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You think for a moment, then tell her that you have questions about Edryn.");
    if (!urtaLove())
    ...
    const discussRomance: ClickFunction = (urtaJustFriends() ? undefined : urtaDiscussesEdrynRomance);
    return {
        choices: [
            ["History", urtaDiscussesEdrynHistory],
            ["Working", urtaDiscussesWorkingWithEdryn],
...
// [=The Watch=]
function urtaDiscussesTheWatch(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You tell Urta that you have a few questions about the Tel'Adre city watch.");
    if (!urtaLove()) CView.text("\n\nUrta perks up at that, her spine stiffening with obvious pride as she takes a hearty swig of her drink.  \"<i>What do you want to know?</i>\" she asks happily.");
    ...
    return {
        choices: [
            ["Membership", urtaDiscussesWatchMembership],
            ["Role", urtaDiscussesWatchRole],
            ["Crime", urtaDiscussesCrimeAndPunishment],
            ["Reach", urtaDiscussesTheWatchsReach],
            ["", undefined],
            ["", undefined],
            ["", undefined],
            ["", undefined],
...
function urtaDiscussesAlcholism(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    if (UrtaFlags.DISCUSSED_URTA_ALCOHOLISM === 0 && UrtaFlags.UNKNOWN_FLAG_NUMBER_00145 === 0) {
        CView.text("You quietly inform Urta that you and she need to talk about her drinking habit.  She swallows nervously and insists, \"<i>I - I only drink to try and keep my cock under control.</i>\"");
        ...
        if (player.stats.cor < 33)
            CView.text("and you feel a little bad for putting her through this");
        else
            if (player.stats.cor < 66)
                CView.text("and you would feel bad if this talk weren't necessary");
            else
                CView.text("but you don't let up for a moment");
        CView.text(".  You insist that you aren't upset with her, that you're not blaming her for anything, but you need to talk to her about this matter.");
        CView.text("[pg]Urta refuses to meet your eye for several long minutes.  Nervously, she begins tapping one clawed finger on the table.  \"<i>I... okay.  The truth is that I did start drinking originally in the hopes it would make my erections subside and make me harder to arouse.  As you've seen, it doesn't work like that - truth be told, I'm kind of a horny drunk.</i>\"");
        CView.text("[pg]You ask why, then, she does it - why does she keep drinking?");
        CView.text("[pg]\"<i>Because it's an excuse for me to cut loose, alright?</i>\" she barks indignantly.  Now she meets your eyes, staring into them with defiance.  \"<i>I work day in and day out to keep this city from falling apart, and the whole time, I have to deal with this... this thing between my legs, undermining everything I say.  ");
        if (urtaLove())
            CView.text("Lover");
        else
            CView.text("Cutey");
        CView.text(", you don't know what I've been through.  In here, when I'm off the clock, I can just have a bottle, hang out with my friends, and kind of... just... bask in everyone else's happiness.  It's not much, but it's better than sitting at home, bored and lonely.</i>\"");
        CView.text("[pg]She finally ceases her rambling dialogue and sighs loudly, shaking her head.  \"<i>Maybe it was a bad idea, but it was my idea, you know?</i>\"  Then, she looks up at you, gently.  \"<i>Besides, I got to meet you, didn't I?  Even with all the bad, I gained you for a ");
        if (!urtaLove())
            CView.text("friend.</i>\"");
        else
            CView.text("lover.  I don't think I've ever had anyone accept me like you have - even Edryn kind of holds back from me.  You... you're different.  Maybe it's because you're an off-worlder, I don't know, but meeting you has been the best thing to happen to me since I joined the guard.  Honestly... I feel like I could probably stop - I don't really need it anymore, though I do seem to wind up having a lot of fun when I do.</i>\"");
        CView.text("[pg]Urta falls silent, waiting for you to speak.  Will you tell her to start drinking less?  Will you tell her that you are fine with her drinking habits, now that she's explained her startling change in behavior and her reasons for doing so?  Will you break up with her if she's that much of an alcoholic? Or do you want her to drink even more - perhaps you find her so much more fun to be with when she's blink stinking drunk?");
        // [Drink More] [Drink Less] [No Change] [Break Up]
        return { choices: [
            ["Drink More", urtaDiscussAlcoholDrinkMore],
            ["Drink Less", urtaDiscussAlcoholDrinkLess],
            ["Be Yourself", urtaDiscussAlcoholNoChange],
            ["Break Up", breakUpWithTheBitchOverAlcoholism]
        ] };
    }
    else {
        CView.text("You tell Urta that you want to discuss her newfound drinking habits.  The grey-furred fox-morph meets your gaze calmly.  \"<i>Really?  What more do you have in mind?</i>\" she asks.");
        if (UrtaFlags.UNKNOWN_FLAG_NUMBER_00146 === 1)
            CView.text("  \"<i>I can't drink any more than I already do - I'm kind of pushing the laws to drink as much as I do already.</i>\"  She burps loudly, then starts on another bottle.");
        else
            if (UrtaFlags.UNKNOWN_FLAG_NUMBER_00146 === 0)
                CView.text("  \"<i>You said you didn't mind me drinking as much as I do... are you going to ask me to cut down?</i>\" she asks, calmly and clearly assuming that's what you intend.");
            else
                CView.text("  \"<i>I said it before, I'll say it again, I'm not going to stop drinking entirely - there's nothing wrong with a few cold ones to take the edge off.  I don't drink myself stupid any more; that should be enough for you,</i>\" she replies, defensively.");
        return { choices: [
            ["Drink A Lot", urtaDiscussAlcoholDrinkMore],
            ["Drink A Little", urtaDiscussAlcoholDrinkLess],
            ["Be Yourself", urtaDiscussAlcoholNoChange],
            ["Break Up", undefined]
        ] };
    }
...
// [=Drink More=]
function urtaDiscussAlcoholDrinkMore(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You blush a little bit and ask why she thinks you want her to drink LESS.  She stares at you quizzically and your blush deepens as you explain that when she gets drunk, lets her guard down, and gets so aggressive, you... well, you like it.  A lot.  Her ears perk up at your words, though her expression is a little uncertain while you explain it.  By the time you finish, something warm brushes by your leg and gently 'thunks' the table.  Clearly, she's as into the idea of encouraging her unrestrained, carefree self as you are.");
    if (urtaWomb.isPregnant() && urtaWomb.pregnancy.type === PregnancyType.PLAYER) {
    ...
    UrtaFlags.UNKNOWN_FLAG_NUMBER_00146 = 1;
    UrtaFlags.DISCUSSED_URTA_ALCOHOLISM = 1;
    return { next: returnToCampUseOneHour };
}
// [=Drink Less=]
function urtaDiscussAlcoholDrinkLess(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    ...
    if (urtaWomb.isPregnant() && urtaWomb.pregnancy.type === PregnancyType.PLAYER) {
        CView.text(", once she's not pregnant anymore, of course");
    }
    CView.text(".  Her eyes glitter with steely resolve, and she leans over the table to kiss you on the lips.  The two of you wrap up the conversation, with you hoping that you've seen the last of drunken Urta.");
...
// [=Family=]
function urtaDiscussesFamily(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    if (!urtaLove()) {
        CView.text("You tell Urta that you'd like to learn about her family.  What are her parents like?  Does she have any siblings?  Is she the only hermaphrodite in the family?");
        // (Regular
        if (!urtaLove()) {
            if (UrtaFlags.URTA_FAMILY_TALK_ATTEMPTS === 0)
...
// [=Sex/Romance=]
function urtaDiscussesSexAndRomance(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You ask Urta if she'll tell you her romantic & sexual history.");
    ...
    if (player.stats.int < 20) player.stats.int += .5;

    if (player.stats.int < 40) player.stats.int += .5;

    if (player.stats.int < 60) player.stats.int += .5;

    urtaLove(.5);
    // [Leave] [Tease]
    return { choices: [["Tease Her", urtaDiscussionTeaseAfterRomance], ["", undefined], ["", undefined], ["", undefined], ["Leave", urtaDiscussionLeaveAfterRomance]] };
}

// [==Leave==]
function urtaDiscussionLeaveAfterRomance(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You thank her for sharing such personal information with you.\n\n\"<i>No problem, [name].  It's good to be able to talk about it for a change,</i>\" the herm fox-morph replies.  As you get up to leave, she picks up her glass, drains it, and starts to pour herself another one.");
    return { next: returnToCampUseOneHour };
}
// [==Tease==]
function urtaDiscussionTeaseAfterRomance(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You thank her for telling you about her past like this but follow it up by asking her what she likes in the bedroom, doing your best to hide a mischievous grin.");
    ...
    if (!urtaLove())
        return { choices: [["Hidden BJ", blowUrtaUnderTable], ["Urta's Place", goBackToUrtasForLuvinz]] };
    else {
        if (urtaWomb.isPregnant() && urtaWomb.pregnancy.type === PregnancyType.PLAYER)
            return { choices: [["Her Place", goBackToUrtasForLuvinz], ["", undefined], ["Eat Out", eatUrtaOutNomNomPussy]] };
        else return { choices: [["Her Place", goBackToUrtasForLuvinz], ["Suck Off", blowUrtaUnderTheTableLuv], ["Eat Out", eatUrtaOutNomNomPussy]] };
    }
}
// [=Employment=]
function urtaDiscussesEmployment(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You ask how and when it was that Urta came to be part of the Tel'Adre watch.");
...
// [=Prejudice=]
function urtaDiscussesPrejudice(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("Hesitantly, you ask Urta if she has ever faced prejudice for her hermaphroditic nature.");
...
function urtaDiscussesEdrynHistory(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    // [=History Together=]
    CView.text("You ask Urta what her past with Edryn is.");
...
// [=Working Together=]
function urtaDiscussesWorkingWithEdryn(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You tell Urta that you're curious about her working relationship with Edryn.  Do they work together often?");
...
// [=Romance? =]
function urtaDiscussesEdrynRomance(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    // Regular
    ...
    if (!urtaLove()) {
        if (!urtaOpenAboutEdryn()) {
            UrtaFlags.URTA_OPEN_ABOUT_EDRYN = 1;
            CView.text("You tell Urta that you've noticed she and Edryn seem to hang out together a lot.  Furthermore, as a centauress, you figure Edryn's probably capable of handling what Urta has hanging between her legs.  You're curious; are they a couple?");
            ...
            // [Accept] [Reject]
            return { choices: [["Accept", urtaDiscussesEdrynNonLoveAccept], ["Reject", urtaDiscussesEdrynNonLoveREJECT]] };
        }
        // Subsequent Regular Variant:
        else {
            CView.text("You give Urta a thoughtful look, and then ask how things are with Edryn these days.");
            CView.text("\"<i>Good,</i>\" the fox-morph replies freely.  \"<i>She'll never compare to you, but she is a truly wonderful fuck... I swear, there is nothing quite like feeling a mare-cunt juice itself around you...</i>\"  She smirks at you, obviously enjoying the chance to make you squirm, but ");
            if (player.stats.cor < 66) CView.text("her manner is playful");
            else CView.text("her expression sobers when you make no response");
            CView.text(".");
            // (If player can now sleep with Edryn for free:
            if (player.effects.get(StatusEffectType.Edryn).value1 >= 5) {
                CView.text("  \"<i>Of course, you know that firsthand, don't you?</i>\" she grins.  \"<i>Did hearing that I was having sex with her make you curious about what it was like?");
                // (If player has fathered at least 1 child on Edryn:
                if (UrtaFlags.EDRYN_NUMBER_OF_KIDS > 0) CView.text("  Still, I can't believe you actually knocked her up - I had to pick my jaw up off the floor when she told me that the foal kicking inside her ballooning gut was yours.  I mean, those herbs are supposed to be foolproof...</i>\" She shakes her head, half in disbelief of your sheer virility, half in awe of it.");
                else CView.text("</i>\"");
                if (UrtaFlags.URTA_FERTILE === 0) CView.text("  \"<i>Still, I hope you don't have any idle thoughts in your head about knocking me up; there's a difference between overwhelming contraceptives and sowing seed where the field's barren,</i>\" she states, patting her flat belly for emphasis.  Despite how flippant she tries to sound when she says this, you can tell that it's something of a sore subject for her.");
            }
    ...
    // Lover
        ...
        if (!urtaOpenAboutEdryn()) {
            ...
            return { choices: [["Happy", urtaDiscussesEdrynLoveHappy], ["Tolerate", urtaDiscussesEdrynLoveTolerate], ["Break Up", urtaDiscussesEdrynLoveBreakup]] };
        }
        else {
            CView.text("With a smirk, you ask Urta how things have been between her and Edryn lately.");
            ...
            CView.text("\"<i>Hmph!  So you should be!</i>\" Urta says, sticking her nose up in the air in an indignant huff... but she can't keep a straight face for more than a few moments.  \"<i>I still can't believe you took it so well when I told you - I really am a lucky girl to have someone like you, aren't I?</i>\" she says.");
            // (If Edryn is doing the player for free:
            if (player.effects.get(StatusEffectType.Edryn).value1 >= 5) CView.text("  \"<i>Though I have to admit I was still a little jealous when Edryn told me you were having sex with her too.  Still, I guess that's just another thing we have in common, yeah?  A mutual taste in sexy ladies.</i>\"  She laughs, and mockingly shapes her hands through the air as if caressing Edryn's sizable boobs.");
            // (If Edryn has had at least one baby:
            if (UrtaFlags.EDRYN_NUMBER_OF_KIDS > 0) {
                if (UrtaFlags.URTA_QUEST_STATUS === 1) {
                    if (urtaKids() === 0) CView.text("  \"<i>...I still can't believe that you actually get her pregnant, though.  It's just not fair that Edryn's got little hooves running around, and after everything I went through, I don't have kits of my own... yet.</i>\" She gives you a slightly predatory smile along with the last word.");
                    else CView.text("  \"<i>...I still can't believe that you actually got her pregnant, though.  I guess I'll have to keep you busy in the sack if we're going to keep up, huh?</i>\" She gives you a slightly predatory smile along with the last word.");
                }
                else CView.text("  \"<i>...I still can't believe that you actually got her pregnant, though. It's just not fair that Edryn's got little hooves running around, and you and I can't have any.</i>\"  Urta sighs.");
            }
...
// [=Accept=]
function urtaDiscussesEdrynNonLoveAccept(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You think about it long and hard, and you finally decide that you can't hold Urta's dalliances with Edryn against her.  From the sound of it, Urta honestly has more reason to be interested in the centauress than in you - she's known Edryn longer, she lost her virginity to her, they're partners and friends.  Yet, despite this, she insists she feels more of a connection to you, and she was honest about what she's been doing and why.  Plus it's not as if you can't understand how she feels about the temptation.  Most creatures you've met in this weird world have either been very sexually tempting, or outright tried to make you have sex with them.");
...
// [=Reject=]
function urtaDiscussesEdrynNonLoveREJECT(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("No matter how you look at it, you can't help but feel betrayed - Urta's been sleeping around behind your back.  If she didn't want this relationship to be monogamous, she should have told you before, and you tell her as much.");
...
function urtaDiscussesEdrynLoveTolerate(player: Character): NextScreenChoices {
    ...
    UrtaFlags.PC_DISLIKES_URTA_AND_EDRYN_TOGETHER = 1;
    return { next: returnToCampUseOneHour };
}
// [=Breakup=]
function urtaDiscussesEdrynLoveBreakup(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("No matter how you look at it, you can't help but feel betrayed - Urta's been sleeping around behind your back.  If she didn't want this relationship to be monogamous, she should have told you before, and you tell her so.");
...
// [=Membership=]
function urtaDiscussesWatchMembership(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You tell Urta that you're curious about the Watch's membership.");
...
// [=Role In The City=]
function urtaDiscussesWatchRole(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You tell her that you're curious about what it is that the Watch actually does, on a day to day basis.");
...
// [=Reach=]
function urtaDiscussesTheWatchsReach(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You ask Urta just how far the Watch's authority spans from the city.");
...
// [=Crime & Punishment=]
function urtaDiscussesCrimeAndPunishment(player: Character): NextScreenChoices {
    CView.clear();
    urtaSprite();
    CView.text("You explain to Urta that you're curious about the law in Tel'Adre - what is a crime here, and what are the punishments that the Watch is likely to deal to a criminal?");
...
function spankTheShitOutOfUrtaAndMakeHerCreamHerselfFromProstateStimulationAloneLikeTheHornyDrunkenSlutSheReallyIs(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("urta-bar-male-spank");
    CView.text("You consider Urta's offers and, tempting as they are, decide they just aren't for you right now.");
    ...
    CView.text("with each swat.  \"<i>You're a rude bitch when you're drunk, Urta,</i>\" you add, giving her another swat on the ass.  She tries to stand, but you roughly push her back against the table.  You give her a fourth spank for her insolence, and can only grin when you hear her cock slam up against the table again.");
    ...
    if (player.body.cocks.find(Cock.CockThatFits(urtaCapacity()))) {
        const secondCockThatFits = player.body.cocks.filter(Cock.CockThatFits(urtaCapacity()))[1];
        CView.text("\n\nYou stride a little closer behind the drunken fox and free your own beast, dropping your " + describeCock(player, secondCockThatFits) + " between her cheeks.  She lets out a happy sigh, spreading her legs and gripping the table, seemingly without shame as half the bar starts rubbing itself to the show.  Her pussy's slick and oh-so-inviting, but you've got other plans!");
        ...
        if (player.cumQ() < 250) CView.text("you've spent your load");
        else if (player.cumQ() < 1000) CView.text("filled her utterly");
        else CView.text("streams of cum leak out of her ass, pooling at your [feet]");
        CView.text(".  Urta whines, desperate to cum, but you don't let her.  Not yet.");
...
function urtaDiscussessKatherine(player: Character): NextScreenChoices {
    ...
    if (affection === 0) { // They’re just co-workers
        ...
        CView.text("“<i>Heh, you really have a thing for " + (telAdre.katherine.hasCock() ? "herms" : "girls in need") + ", don’t ya?</i>” she says with a smile.  “<i>Not that I’m complaining.  Anyway, in terms of her work she still needs to brush up on the laws of the land and a few other things, but she’s doing fine.  Plus it’s nice to have a cat in the watch.  I had thought about saving her for undercover work, but I figure it’s better for everyone to see a cat who’s a good upright citizen.  There really aren’t that many and maybe some of the ones in gangs will rethink things when they see her on patrol.</i>”");
    }
    else if (affection < 11) { // They’ve had drunken sex with you (can't get to this point in friend mode)
        CView.text("“<i>Yeah Kath.  Well I can see why you like her.  Hot girl" + (telAdre.katherine.hasCock() ? " with a decent sized cock" : "") + ", gets a bit horny when she’s drunk.  What’s not to like?</i>”\n\n");
        CView.text("She takes a swig of her drink and gives you a hug.  “<i>So, I just want to say I’m not unhappy having sex with you.  I mean that’s always great.  But, um, I gotta admit it’s really fun when you get the three of us together.  If you decide to do that again ... I’m in.</i>”");
    }
    else if (affection < 32) { // Willing to have sober sex
        CView.text("“<i>Mmmm yeah Kath,</i>” she says dreamily, then shakes her head and laughs.  “<i>Guess that about sums things up, huh?  Yeah I like Kath, I like her quite a bit.  She’s a good watch officer and she’s a good lover in a threesome.</i>”  Urta looks a little nervous, but presses on.  “<i>So... what do you think of the two of us, when you’re not around, you know?</i>”\n\n");
        CView.text("You could tell Urta not to have sex with Katherine except when you’re around, tell her it’s alright for them to have sex whenever they want or encourage them to fuck each other’s brains out.");
        return { choices: [["Don't Fuck", urtaKathSexDont], ["Whenever", urtaKathSexWhenever], ["Encourage", urtaKathSexEncourage]] };
    }
    else { // Lovers
        CView.text("“<i>" + player.desc.name + ", I can honestly say I never expected to have this much sex.  I love you, I love Katherine.  It’s a bit weird, but I’m not complaining and neither is she and I hope neither are you.  I’m not the same girl you found sitting alone in the bar.  I’m so happy now.</i>”");
    }
    return { next: returnToCampUseOneHour };
}

function urtaKathSexDont(player: Character): NextScreenChoices {
    CView.text("You give Urta a smile tell her that while you’re happy for her you’d really prefer if she waited for you to be around before banging Katherine.  You feel a little left out.\n\n");
    CView.text("Urta gives you a weak smile and says, “<i>Alright " + player.desc.name + ", I understand.  I’ll keep it in my pants.  Course I hope you’re up for a lot of three ways.  Girl’s got to get her fix.</i>”");
    UrtaFlags.KATHERINE_URTA_DATE = Katherine.KDATE_LITTLE;
    return { next: urtaDialogueMenu };
}

function urtaKathSexWhenever(player: Character): NextScreenChoices {
    CView.text("You put your arm around Urta and tell her that you don’t mind if your favorite girls need to blow off a little steam together.  As long as they don’t wear each other out that is.\n\n");
    CView.text("Urta lets out a relieved laugh and says, “<i>That’s good.  That’s good.  I’ll make sure not to wear our kitten out.</i>”");
    UrtaFlags.KATHERINE_URTA_DATE = Katherine.KDATE_WHENEVER;
    return { next: urtaDialogueMenu };
}

function urtaKathSexEncourage(player: Character): NextScreenChoices {
    CView.text("You ask Urta what she would say if you told her you flat out expect them to have sex when you’re not around.\n\n");
    CView.text("“<i>Are you sure " + player.desc.name + "?  I mean I love the idea, I’m sure Kath will love it too, but you know what my appetite’s like.</i>”\n\n");
    CView.text("You give her a kiss and tell her you know very well.  Since you’re not in town all the time you expect Urta to see to Kath’s needs and Kath to hers.\n\n");
    CView.text("“<i>Carte blanche?  Ok " + player.desc.name + ", but I’m warning you - your sex kitten is going to be very well fucked whenever you see her.</i>”");
    UrtaFlags.KATHERINE_URTA_DATE = Katherine.KDATE_LOTS;
    return { next: urtaDialogueMenu };
}
...
export function giveTheFoxSomeEggs(player: Character): NextScreenChoices {
    ...
    if (UrtaFlags.PC_FETISH > 0) CView.text("as much fun as it might be, ");
    ...
    if (player.body.legs.isTaur()) CView.text("positioning your centaur, ");
    else if (player.body.legs.isDrider()) CView.text("drider body over the bulk of her back, ");
    CView.text("and release your ovipositor from its usual hiding slit, already dripping with ");
    if (player.perks.has(PerkType.SpiderOvipositor)) CView.text("green slime");
    else CView.text("honey");
    CView.text(" in anticipation of laying.  The appendage droops down, curling around to slide its wet length against Urta's balls and the base of her cock.  \"<i>[name], wha...?</i>\" she slurs at you, but you just gently shush her and tell her to relax.  With a little effort, you bring it slithering back up to pry at the dampness of her netherlips...");
    ...
    if (player.gender > 0) {
        ...
        if (player.gender === 3) CView.text(" and ");
        if (player.body.vaginas.length > 0) CView.text("[vagina]");
        if (player.gender === 3) CView.text(" are ");
        else CView.text(" is ");
        CView.text("starting to grow wet with arousal, matting her back with your sexual fluids, but you ignore them, too caught up in your need of release to bother with conventional pleasures.");
    }
    else CView.text("  The entirety of your sexual world begins and ends with your ovipositor, with the sensation of the mock-cock burrowing inexorably towards Urta's womb overwhelming you with pleasure.  Having no other sexual organs to distract you allows you to fully embrace it; you thrust harder and harder, desperate to ensure you have reached the womb before you start to lay.");
    ...
    if (player.perks.has(PerkType.BeeOvipositor)) CView.text("honey");
    else CView.text("spider-goo");
    CView.text(" to keep Urta moist and slick and pliable.");
    ...
    if (player.body.vaginas.length > 0) wetness += player.body.vaginas.get(0).wetness * 100;
    if (player.body.cocks.length > 0) wetness += player.cumQ();
    if (wetness < 300) CView.text("splattering");
    else if (wetness < 500) CView.text("matting");
    else if (wetness < 1000) CView.text("soaking");
    else CView.text("flooding");
    CView.text(" the fox with your gushing ");
    if (player.gender === 1) CView.text("cum");
    else if (player.gender === 2) CView.text("femcum");
    else CView.text("mixed sexual fluids");
    CView.text(".");
    ...
    if (player.gender > 0) CView.text(" just as her back is from your own release");
    CView.text(".");

    // [Egg Level 1:
    if (player.body.ovipositor.eggs < 20) CView.text("  Urta's stomach starts to swell as your clutch of eggs fills her once-empty womb, stretching into a noticeable bulge, though not one that she couldn't hide if she wanted.");

    // [Egg Level 2:
    else if (player.body.ovipositor.eggs < 40) CView.text("  As the eggs keep on coming, Urta's womb swells and bulges, crammed full of goo and eggs, growing larger and larger until Urta could easily pass for an expectant mother, ready to give birth any day now.");

    // [Egg Level 3:
    else CView.text("  You stuff Urta with eggs until you're wondering how many either of you can hold. Her belly just keeps getting rounder and rounder even as your eggs keep coming; soon, not only does she look like a woman pregnant with multiples, her skin is visibly stretched over the eggs, giving it a distinctively lumpy look rather than the round, smooth bulge she might have if your load was smaller.");
    ...
    UrtaFlags.URTA_EGGS = player.body.ovipositor.eggs;
    UrtaFlags.URTA_FERTILE_EGGS = player.body.ovipositor.fertilizedEggs;
    UrtaFlags.URTA_TIMES_EGG_PREGGED++;
    if (player.perks.has(PerkType.BeeOvipositor))
        urtaWomb.knockUp(PregnancyType.BEE_EGGS, 72);
    else
        urtaWomb.knockUp(PregnancyType.DRIDER_EGGS, 72);
    UrtaFlags.URTA_FLATBELLY_NOTICE = 0;
    // First time, tag for triggering freakout!
    if (UrtaFlags.URTA_EGG_FORCE_EVENT === 0) UrtaFlags.URTA_EGG_FORCE_EVENT = 48;
...
export function urtaChewsOutPC(player: Character, newScreen: boolean = true): NextScreenChoices {
    if (newScreen) CView.clear();
    else CView.text("\n");
    CView.text("Urta looks up at you and snorts fiercely; you almost expect steam to come flaring out of her nostrils, and her livid eyes are doing their best to burn holes straight through you.  \"<i>There you are, [name]!  You have some nerve!</i>\"");
    ...
    if (player.stats.cor > 50) CView.text(", especially given this may be the closest she'll ever get to experiencing pregnancy");
    CView.text("?");
    ...
    // [Unfertilised Eggs:
    if (UrtaFlags.URTA_FERTILE_EGGS === 0) {
    ...
    if (player.perks.has(PerkType.SpiderOvipositor)) CView.text("spider");
    else CView.text("little bee");
    CView.text("...</i>\"\n");
    // (Technically finished, now, but waiting Fen's okay before adding repeatable version)
    UrtaFlags.URTA_EGG_FORCE_EVENT = -1;
    if (newScreen) return { next: returnToCampUseOneHour };
}

// Repeatable Eggs Option:
function repeatUrtaEgging(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("urta-egg-repeat");
    CView.text("With a smile, you ask if Urta would be willing to let you lay a few eggs in her womb again?");
    ...
    if (!urtaLove()) CView.text("blatantly hanging out of her stockings and drooling pre-cum everywhere");
    else CView.text("vulgarly bulging her dress and rendering it sopping wet from her arousal");
    CView.text(", before sashaying in a drunkenly exaggerated fashion out towards the alleyway where you and she first began your relationship- and where you first 'impreggnated' her.");
    ...
    CView.text("\n\nBut it's a far different need that is driving you now, and you can't be bothered to engage in much foreplay.  You straddle the sloshed fox, ");
    if (player.body.legs.isTaur() || player.body.legs.isDrider()) {
        if (player.body.legs.isTaur()) CView.text("positioning your centaur");
        else CView.text("drider body over the bulk of her back");
        CView.text(", ");
    }
    CView.text("and release your ovipositor from its usual hiding slit, already dripping with ");
    if (player.canOvipositBee()) CView.text("honey");
    else CView.text("green slime");
    ...
    if (player.gender > 0) {
        ...
        if (player.gender === 3) CView.text(" and ");
        if (player.body.vaginas.length > 0) CView.text("[vagina]");
        CView.text(" are starting to grow wet with arousal, matting her back with your sexual fluids, but you ignore them, too caught up in your need of release to bother with conventional pleasures.");
    }
    ...
    CView.text("\n\nEgg after egg rolls down your ovipositor, each eliciting a gurgling splurt of cum from Urta's immense dick as it gushes into her, ensuring  the ground under her is soaked with sexual fluids");
    if (wetness > 0) CView.text(" just as her back is from your own release");
    CView.text(".");
    ...
    CView.text("\n\nAwkwardly hauling her distended form to its feet, she gives you a sloppy kiss, grabs her clothes and starts waddling back home, clearly intending to sleep it off.  You watch her go, then pick yourself up and head back to ");
    UrtaFlags.URTA_EGGS = player.body.ovipositor.eggs;
    UrtaFlags.URTA_FERTILE_EGGS = player.body.ovipositor.fertilizedEggs;
    UrtaFlags.URTA_TIMES_EGG_PREGGED++;
    if (player.perks.has(PerkType.BeeOvipositor))
...
// [Condomless]
function condomlessUrtaInHouseSmex(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You grab hold of the pre-filled bubble atop the fox's tip and yank, earning a moan as the latex sheath slides free, stretching it to an oblong mass before freeing her flare with an audible pop.  At the same time, you draw the second, unused condom from her unresisting grip and say, \"<i>Why don't we make a little mess?</i>\"");
    // non lover:
    ...
    const choices: ScreenChoice[] = [];
    if (player.body.cocks.length > 0) {
        if (player.body.cocks.find(Cock.CockThatFits(urtaCapacity()))) choices[0] = ["Peg Her Ass", pegUrtasAssMessy];
        else CView.text("\n\n<b>You're too big to peg her ass and make her bust a nut all over herself.</b>");
    }
    if (player.body.vaginas.length > 0) choices[1] = ["Fill Me Up", fillMeUpPleaseUrta];
    if (player.gender > 0 && player.body.legs.isNaga()) choices[2] = ["TailFun", nagaOnUrtaMess];
    choices[4] = ["Back", goBackToUrtasForLuvinz];

    return { choices };
}
// [Peg Her Ass]
function pegUrtasAssMessy(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("urta-home-pegurta");
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(urtaCapacity()));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    CView.text("Fondling the fox's apple-sized balls, you give her a hard but affectionate squeeze, rolling the heavy orbs around in your palm as you lift them high.  With her stuffed sack out of the way, Urta's oozing black-lipped gash is on full display, with engorged labia soaked in lubricants that had nowhere to go thanks to her smotheringly large scrotum.  Just below, you can see her anal star tightening and relaxing with each of the vixen's heavy breaths.  To you, it looks almost hungry, panting like an animal.");
    ...
    CView.text(describeCock(player, cockThatFits) + " down onto the sheets, a scant few inches from Urta's puckered asshole.  The fox ");
    if (urtaDrunk()) CView.text("giggles drunkenly, \"<i>You're sho dirty!</i>\" while spreading those fishnet-clad legs further.");
    else if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY < 11) CView.text("softly begs, \"<i>Be gentle, please,</i>\" while spreading those fishnet-clad legs a little further apart.");
    else CView.text("eagerly coos, \"<i>Feeling a little dirty, eh, lover?  Go on baby, you can fuck my ass anytime.</i>\"  She spreads her fishnet-clad legs so wide they almost form an unbroken, horizontal line from padded toe to padded toe, tying her soft, poofy tail around a leg to keep it out of the way.");
    CView.text("  Scooting forward, you edge your " + describeCockHead(cockThatFits) + " up to her anus.  Her body heat washes over you as you enjoy the feel of her pucker's folded flesh pressing down on your most sensitive point, lubricated by the copious female fluid that drains from the sodden gash above.");
    CView.text("\n\nYou begin to gently rock your [hips] forward, giving little, gentle pushes against the resistant muscle.  At first, you make no progress, simply forcing her anal ring to dilate, little by little.  Then, after a particularly vigorous stroke, your " + describeCockHead(cockThatFits) + " slides through the clinging ring and into the tight embrace of her slick anus.  You stop to allow her to adjust, admiring her ");
    if (urtaDrunk()) CView.text("drunken ");
    CView.text("whimpers of pleasure.  However, you don't wait long, slowly pushing forward and exploring your vixen's every inner crevice with your eager rod.");
    CView.text("\n\n\"<i>It's ");
    if (urtaDrunk()) CView.text("sho");
    else CView.text("so");
    CView.text(" different than with a condom over a toy,</i>\" the vulpine lass wonders aloud in delight.  She licks her lips and ");
    if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY > 15) CView.text("casually");
    else if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY > 11) CView.text("confidently");
    else if (UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY > 4) CView.text("a little nervously");
    else CView.text("with a shaky hand");
    CView.text(" strokes a padded fingertip down the bulging length of her urethra, shivering when it bumps across the sensitive medial ring.  A few inches in, you feel a rounded bulge bump into you, and as soon as you press against it, Urta's cock lifts up and thickens, flooded with blood.  The fox trembles for a second as you glide by the obstruction, and without any warning at all, her shaft belches a thick spurt of clear pre-cum with enough force to glaze the underside of her tits.");
    CView.text("\n\nSmiling at the lascivious display, you gather a handful of the balmy animal cream and circle it around one of her perky nipples, admiring how the black nub perks up into a puckered, sensitive tower.  Then, while still pushing into her, you repeat the action on the other side, with identical result.  Your " + describeCock(player, cockThatFits) + " milks another heavy flow from the fox's prostate, this one tinged with just a little bit of off-white.  It puddles on her belly, already forgotten.  You're pushing forward without much worry for her comfort now, and blessedly, the fox doesn't seem to mind.  Indeed, with every lurch forward, her twat's drippings come faster and faster, matched by even larger discharges from the turgid member above.  ");
    if (cockThatFits.area > urtaCapacity()) CView.text("The aching pressure around your " + describeCock(player, cockThatFits) + " doubles, and you realize she simply can't take any more.  Her anus is stretched wide, dilated to the extreme to accommodate this much of you.  There's little to do but plow her with what you can until she's fountaining white.");
    else if (cockThatFits.hasKnot()) CView.text("The pleasure seems to double when your knot butts up against her furry rump, but her asshole isn't ready for that just yet.  This bitch needs a little more loving before you knot her.");
    else {
        CView.text("The aching pleasure around your " + describeCock(player, cockThatFits) + " redoubles as you hilt her, smashing your ");
        if (!player.body.cocks.find(Cock.HasSheath)) CView.text("groin ");
        else CView.text("sheath ");
        CView.text("against her taut asshole with a heavy lurch.");
    }
    CView.text("\n\nGroping the vixen's tits and thumbing her nipples, you have your way with her chest, torturously teasing her while your " + describeCock(player, cockThatFits) + " rests immobile in her ass.  Her pussy-slicked sack rests against you, lewdly bouncing and pulsating with need, fairly ready to explode; her cock is equally aroused.  The rigid length is so hard that it hovers over her smooth abdominals without touching them, bobbing up and down while it exudes a steady drizzle of lust.  The pre-spunk is so thick on Urta's belly that her fur is matted and glossy from her hips to the underside of her tits, and ribbons of the stuff run down her sides to lubricate the sheets below.  Her hands push yours into her chest harder, smashing her tits almost flat against her.  At the same time, her hips squirm and writhe, grinding in lewd little circles against you, swirling your dick up against every part of her sensitive tail-hole.");
    ...
    if (cockThatFits.area > urtaCapacity()) CView.text("most of you ");
    else CView.text("your entire length ");
    CView.text("before she can react, clinging to every inch of cock-flesh as it's forced deeper into her interior.  A wave of ecstasy washes through the fox, slackening her muscles in a ripple that travels from her waist to her lips in a second.  Her annoyance morphs into slack-jawed pleasure, and her protests deteriorate into babbling vocalizations of enjoyment.  Before she can recover, you pull back and thrust again, battering her sphincter repeatedly as you begin to truly pound the fox's ass.");
    CView.text("\n\nUrta moans, \"<i>Fuck me!  Fuck my ass, [name]!  Gods");
    if (urtaDrunk()) CView.text("h");
    CView.text(", why does it feel s");
    if (urtaDrunk()) CView.text("h");
    CView.text("o goooo-ooooooh yes!</i>\"  Her ass clutches and relaxes to your tempo, hugging your " + describeCock(player, cockThatFits) + " tight");
    if (player.body.balls.count > 0) CView.text(" while your [balls] slap against her fuzzy buttcheeks.");
    else CView.text(" while you slap your groin against her fuzzy buttcheeks.");
    ...
    if (cockThatFits.hasKnot() && cockThatFits.area < urtaCapacity()) CView.text("  You barely notice when your knot pops through the ebony ring, locking you inside her.  All you know is that while your strokes have gotten shorter, the pleasure has gotten even greater.");
    CView.text("\n\nYou pinch the nipple you've been playing with and slam your " + describeCock(player, cockThatFits) + " home with rump-shaking force.  It slides the fox backward in her puddle of pre, slicking the underside of her legs with her sexual moisture as she's dragged through it.  Barely suppressing a moan of your own, you shimmy forward again and voraciously tackle her asshole, pegging Urta with short, brutal strokes.  Eager to see her blow, you shift your angle and press up, slamming your " + describeCockHead(cockThatFits) + " into her prostate so hard you swear you must be flattening it.  Urta screams, her balls contracting powerfully.  Her dick, already iron-hard, seems to swell thicker, looking like a flat-tipped, over-inflated sausage.  Bulging obscenely, the underside of her cock swells with lusty intent, the distention moving up with muscular contractions until the vulpine girl's cum-slit is wide and filled with white.");
    CView.text("\n\nThe onyx asshole wringes you with every ounce of force its owner can apply, convulsing into a tight cock-ring that");
    if (cockThatFits.hasKnot() && cockThatFits.area < urtaCapacity()) CView.text(" only makes your knot bigger and harder.");
    else {
        CView.text(" works you so hard an irrational part of your mind worries your manhood will be pinched off.");
        if (player.body.cocks.length > 1) CView.text("  Good thing you have more.");
    }
    CView.text("  Then, the fox stops being a fox and turns into a fountain, spewing a wave of white up over her head and into the wall.  It hits with a wet 'splat', flinging alabaster globs in every direction that burst like little bombs.  Spunk rains down over Urta, staining her bed and her in equal measure.  A lock of gunked-up hair flops across her face, temporarily blinding your orgasm-inundated lover.  Just when the dripping jizz slows and you think the worst is over, Urta goes off again, though this fountain of white has a little less force than the preceeding cumsplosion.  A wave of semen as thick as your wrist spurts out to take the fox in the face, plastering the rest of her lustrous hair to her forehead and glazing her visage with ivory lust.  Ringlets of her ");
    if (UrtaFlags.URTA_QUEST_STATUS < 0) CView.text("not-so-fertile ");
    CView.text("fluid dangle precariously from her jawline as she tries to breathe, coughing and making bubbles from her white mask.");
    ...
    if (urtaDrunk()) CView.text(" and of course, alcohol");
    CView.text(", kissing your " + describeCockHead(cockThatFits) + " every time it peeks past her tits, relishing the taste of her own lust all over it.");
    ...
    if (player.cumQ() < 250) CView.text(", swallowing with a smile and a wink.");
    else {
        CView.text(", but she's forced to close it when her cheeks are puffed beyond reason.");
        if (player.cumQ() > 750) CView.text("  You go on to wash her face with your own white sauce.");
        if (player.cumQ() >= 1200) CView.text("  Your orgasm goes on and on, coating just about everything that Urta came on with yet another layer.");
        if (player.cumQ() > 3000) {
            CView.text("  She gasps in surprise when you flood the ");
            if (player.cumQ() > 10000) CView.text("first few feet of the ");
            CView.text("room, looking up at you in admiration through cum-coated eyelashes.");
        }
    }
    CView.text("\n\nAs you finish emptying your [balls], you sag down into the fox's arms.  She plants a salty kiss on your lips ");
    if (urtaLove()) CView.text("and nuzzles against your cheek, just holding you for a few moments, lover to lover.");
    else CView.text("and sheepishly pulls back.");
    CView.text("  Climbing out of the alabaster-painted bed is no easy task, but the two of you manage, laughing and leaning on each other's arms.  Urta leads you to a cabinet in another room, and bends over to open it.  You barely notice her pulling out mops, buckets, and cleansers.  Instead, your vision is riveted to her asshole, still gaped lewdly in the shape of your " + describeCock(player, cockThatFits) + ".  When she comes up with an armload of cleaning supplies, you're ");
    if (player.stats.cor < 50) CView.text("blushing");
    else CView.text("smiling devilishly");
    CView.text(".");
    ...
    return { next: UrtaGetsPeggedCleanUp };
}
function UrtaGetsPeggedCleanUp(player: Character): NextScreenChoices {
    CView.clear();
    if (player.stats.cor >= 50) CView.text("You start to duck out, but the disapproving glare the fox gives you makes it clear that you won't be getting any more messy fun unless you stick around to clean it up this time.  Damnit.  ");
    CView.text("With Urta's help, you slowly clean up the mess you two made, nearly clogging the plumbing a few times in the process.  New sheets, pillows, and furs are brought in, and you help her fix her bed.  The floor is mopped, the walls are scrubbed, and the shutters are opened to let in some fresh air.  Thankfully, just about everything is watertight and sealed, likely something she had done to ease situations such as this.  All told, between clean-up and sex, you spend a few hours with her.");
    CView.text("\n\nTenting her apron (the only thing she's bothered to put on), Urta gives you a ");
    if (!urtaLove()) CView.text("wave");
    else CView.text("tender hug");
    CView.text(" and sends you on your way.");
    return { next: returnToCampUseFourHours };
}

// [Get Filled Up At The Cum Pump Or Something This Was Supposed To Be A Joke]
function fillMeUpPleaseUrta(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("urta-home-female-fucked");
    CView.text("Lazily discarding the latex, you caress Urta's stocking-clad leg and coyly climb into bed.  She starts to reciprocate your affections, but you push her arm away, instructing, \"<i>Let me... let me have this.</i>\"");
    ...
    if (player.body.vaginas.get(0).wetness > 3) CView.text("  You're so utterly soaked thanks to this place that a steady stream of vaginal lubricant drizzles over the meaty tool below.");
    if (player.body.cocks.length > 0) CView.text("  Hard and erect, [eachCock] stands tall and proud, but ignored.  You have other plans.");
    CView.text("  She's ready.  You're ready.  It's time.");
    ...
    if (player.body.vaginas.get(0).wetness > 3) CView.text(", though it's a drop in the bucket next to your copious moisture.  Hell, even now, rivers of your girl-goo are sliding down that spasming length, wreathing it in lady-spunk");
    else CView.text(", deliciously staining your slit");
    CView.text(".");
    // {Virgin PCs}
    if (player.body.vaginas.get(0).virgin) {
        ...
        if (player.body.tallness > 80) CView.text("[clit]");
        else if (player.body.tallness > 60) CView.text("belly");
        else CView.text("[chest]");
        CView.text(" as she cranes her neck to meet you, eye to eye.  \"<i>Really?</i>\"  You nod, biting your lower lip as you continue your frenzied undulations upon her rod, working yourself up to taking her.");
        ...
        if (urtaDrunk()) CView.text("drunkenly ");
        CView.text("asks, \"<i>Are you s");
        if (urtaDrunk()) CView.text("h");
        CView.text("ure?  You don't have to...</i>\"");
        ...
        if (player.body.cocks.length > 0) CView.text("  Drool begins to leak from [eachCock], slicking the underside with pre-cum as the pleasure affects your maleness.");
        CView.text("  Before long, you've taken Urta halfway, with her ring jutting up against your entrance.");
        // {cuntChange 30}
    ...
    if (UrtaFlags.PC_FETISH > 0) CView.text("  An exhibitionist thrill runs through you at that, making you even wetter.");
    ...
    if (player.body.cocks.length > 0) CView.text("  You don't even notice [eachCock] dripping on Urta's belly.");
    // HERMS:
    if (player.body.cocks.length > 0) {
        ...
        if (player.body.cocks.length > 1) CView.text(", followed in short order by the untouched portion of your " + describeCocksLight(player));
        CView.text(".  The next is your [vagina] going wild, convulsing from your entrance towards your womb with muscular contractions that milk Urta's cock relentlessly, causing it to grow bigger and harder inside you.  Urta whimpers slightly as you jizz over her belly");
        if (player.cumQ() > 400) {
            CView.text(", soaking her");
            if (player.cumQ() > 1000) {
                if (player.cumQ() < 2000) CView.text(" and her bed");
                else CView.text(", her bed, and damn near everything else");
            }
            CView.text(" with your ejaculate");
        }
        CView.text(".  There's a bloom of warmth from below as the vixen's orgasm rushes towards you, and all you can do is cum your brains out in anticipation of being filled.");
    }
    // NON-HERMS:
    else {
    ...
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 4) CView.text(" and your [chest] rest heavily upon your cum-stuffed form");
    CView.text(".");
    if (player.body.wombs.find(Womb.Pregnant) && player.body.wombs.find(Womb.Pregnant).pregnancy.incubation > 0 && player.body.wombs.find(Womb.Pregnant).pregnancy.incubation < 150) CView.text("  You may have been pregnant before, but you're beyond pregnant now... with cum AND child.");
    // {Goo:
    if (player.body.legs.isGoo() && player.vaginalCapacity() > 9000) CView.text("  Successive eruptions make you bigger, but your flexible cunt soon grows so bloated that the onrushing injections mostly squirt back out, painting Urta, the bed, and your [legs] white with cum.");
    else CView.text("  Successive eruptions do little to bloat you further - your body is stretched to capacity, so the onrushing injections only squirt back out through your entrance, painting Urta, the bed, and your [legs] white with cum.");
    ...
    if (player.body.wombs.find(Womb.Pregnant) && player.body.wombs.find(Womb.Pregnant).pregnancy.incubation > 0 && player.body.wombs.find(Womb.Pregnant).pregnancy.incubation < 150) CView.text("even more ");
    CView.text("pregnant for a while, at least until you can pass all that filling!  She plants a salty kiss on your lips ");
    if (urtaLove()) CView.text("and nuzzles against your cheek, just holding you for a few moments, lover to lover.");
    else CView.text("and sheepishly pulls back.");
    ...
    else if (player.stats.cor < 66) CView.text("grinning");
    else CView.text("openly leering");
    CView.text(".");
    ...
    UrtaFlags.TIMES_FUCKED_URTA++;
    player.slimeFeed();
    if (UrtaFlags.URTA_FERTILE === 1) player.body.wombs.find(Womb.NotPregnant).knockUp(new Pregnancy(PregnancyType.URTA, IncubationTime.URTA), 25);
    player.orgasm();
    player.stats.lib += .2;
    player.stats.sens += -3;

    return { next: cleanUpAfterUrtaCreamInjectsYou };
}

function cleanUpAfterUrtaCreamInjectsYou(player: Character): NextScreenChoices {
    CView.clear();
    if (player.stats.cor >= 50) CView.text("You start to duck out, but the disapproving glare the fox gives you makes it clear that you won't be getting any more messy fun unless you stick around to clean it up this time.  Damnit.  ");
...
function lippleBondUrtaScylla(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("urta-syclla-3some");
    CView.text("With a grin, you approach the busty - bustier duo and lower yourself level with the woman doing most of the work.  Urta watches you with evident anticipation, probably thinking about whether you intend to join in on the oral assault on her - maybe give some attention to her overfilled balls, swelling bigger and bigger under Scylla's tender caresses, or the wet slit of a prize behind them, but you have a different thing in mind for them.");
    ...
    // [if (cocks > 1)
    if (player.body.cocks.length > 1) CView.text("  Without much warning, you suddenly feel Urta's hand move away from your own palm, the sensation of your [cock smallest] being wrapped by her fingers also assaulting you as she squeezes and tugs at your second cock affectionately.");
    // [if (cocks > 2)
    if (player.body.cocks.length > 2) CView.text("  The barrage of sensations is intensified when Scylla also gets a bit greedy, grabbing another tool and starting to jerk it off as well.");
    ...
    // [if (cocks > 1)
    if (player.body.cocks.length > 1) CView.text(" either");
    CView.text(" the holy woman's ravenous lipples and inner, erotic tit-flesh in a mutually ecstatic, sensations-filled manner");
    if (player.body.cocks.length > 1) CView.text(" or more delicately and less hungrily with a feminine hand");
    CView.text(".");
    ...
    // if (sensitivity < 29)
    if (player.stats.sens < 29) CView.text("  You can only tell so from her expression and from Scylla moaning and gasping sharply, but it appears Urta's cock is really just about ready to shoot off cum.");
    // if (sensitivity > 29)
    else CView.text("  Scylla's titflesh slightly shifts from what is apparently the expansion of Urta's cock flare and the twitching of her massive shaft.  You're sensitive enough to notice.");
    // [if (sensitivity > 44)
    if (player.stats.sens >= 44) CView.text("  In fact, you can clearly feel the pressure inside Scylla's chest increasing as Urta's huge cock expands and twitches inside her next to your own, each such twitch sending a shiver of pleasure through both you and the nun.");
    ...
    // [If Cock type = Canine]
    if (player.body.cocks.sort(Cock.Largest)[0].hasKnot()) CView.text("  You're not making the problem any easier, in fact, your knot is halfway stuck inside her breast, surged and filled to full size right after your orgasm.");
    ...
    if (player.body.cocks.sort(Cock.Largest)[0].hasKnot()) CView.text(", your knot sufficiently deflated now");
    CView.text(", now sucking desperately at the air, looking for the tasty dick.");
    ...
    if (player.cumQ() < 500) player.body.cumMultiplier += 2;
    if (player.cumQ() < 1500) player.body.cumMultiplier += 2;
    if (player.cumQ() < 5000) player.body.cumMultiplier += 2;
    if (player.cumQ() < 20000) player.body.cumMultiplier += 2;
    return { next: returnToCampUseOneHour };
}
...
function nagaOnUrtaMess(player: Character): NextScreenChoices {
    ...
    // [if (cocks = 0)
    if (player.body.cocks.length === 0) CView.text("  Urta runs her fingers over and gently pushes into your now revealed female slit. \"<i>My lover's sweet, sweet pussy. The best in the whole world.</i>\"");
    // if (cocks = 1)
    else if (player.body.cocks.length === 1) CView.text("  \"<i>And here is my favorite cock in the whole world... Probably including mine,</i>\" she giggles and winks at you.");
    // if (cocks = 2)
    else if (player.body.cocks.length === 2) CView.text("  \"<i>Two delicious cocks, rather than just one...  You're trying to tell me you're the man here, aren't you, lover?</i>\" she giggles and winks at you.");
    else if (player.body.cocks.length === 3) CView.text("  \"<i>You want to have a spare one just in case or something?</i>\" she giggles and winks at you.");
    // [if (cocks >= 4)]
    else CView.text("  \"<i>Interesting.  It looks like a bouquet.  A beautiful bouquet of cocks just for me,</i>\" she giggles and winks at you.");

    CView.text("  Without further ado, Urta leans towards your sex to return the favor, her mouth pleasuring you just as you wrap more and more of your slightly looser coils around her member, having released her legs to allow her to move... and to allow your tail's tip to reach her needy vulpine pussy more easily from that position.");
    ...
    // CView.text("  However, it requires no justification that your herm lover is further on her way to an orgasm then you are.");
    ...
    if (player.body.vaginas.length > 0) {
        ...
        // [if (isSquirter = true)]
        if (player.body.vaginas.get(0).wetness >= 4) CView.text("  After a few convulsions, your femcum starts to shoot out of your pussy quite violently, adding to the mess your lover is making.");
    }
    // if (cocks > 0)
    if (player.body.cocks.length > 0) {
        ...
        // [if (cumHigh = true)]
        if (player.cumQ() >= 1000 && player.cumQ() < 2500) CView.text("  Her eagerness does end up giving her a swollen tummy, but she's loving and hungry for you until the end, swallowing as much as she can, as quickly as she can.");
        // (if [cumExtreme = True]
        else if (player.cumQ() >= 2500) CView.text("  Of course, your excessive load proves too much for the poor vixen to handle, and soon she is not only inflated, but painted in your cum as well, the mess the two of you are making definitely nothing to be scoffed at.");
    }
    ...
    CView.text("\n\nAfter a much more tender, if still affectionate couple activity, Urta lets you leave, but not before kissing you and sending you home with a blush.  \"<i>I guess I should let you do more unusual stuff to me more often...</i>\"  She trails off as you march back to your ..");
...
function lickOutUrtaAtHome(player: Character): NextScreenChoices {
    ...
    CView.text("\n\nYou climb overtop of Urta and allow your " + describeCock(player, cockThatFits) + " to press on into her pussy.  ");
    if (cockThatFits.area >= urtaCapacity() / 2) CView.text("It fits you like a lubricated, stretchy little glove, ensuring every movement will caress her folds.");
    else CView.text("It's nice and roomy, allowing you full control over how you want to press on her inner folds.");
    CView.text("  You push the rest of the way into her cunt and admire the velvety texture of her walls as they close in around you, inner muscles flexing and massaging your " + describeCock(player, cockThatFits) + " as she adjusts to your intrusion.");
    if (cockThatFits.area >= 200) CView.text("   Stretching ludicrously, her cunt dilates with surprising ease.  The elastic lips of her pussy cling oh-so-tightly to your girth as they're spread beyond reason.  Her hips even spread with only a modest bit of pressure and resistance as her passage is distorted, the shape of your " + describeCock(player, cockThatFits) + " visible through her stretched belly as it pushes towards her tits.  You can even make out some of the larger veins.");

    CView.text("\n\nThe hermaphrodite lifts one of her ebony nipples up to her midnight-hued lips and suckles it eagerly, writhing in the delight.  You meet her hungry eyes as you gently begin to slide back and forth, reveling in the liquid-hot heat of the vulpine woman's snatch with a lusty moan, pumping your " + describeCock(player, cockThatFits) + " in and out over and over.  Her secretions are running unchecked onto the bedsheets now, and neither of you care.  You grab hold of Urta's neck to pull her away from her nipple so that you can kiss her on the lips, and you flick your tongue into her mouth.  She demurely accepts the passionate tonguing before allowing her own to slide into yours, slithering to the tempo your [hips] set.");
    ...
    if (player.stats.cor < 66) CView.text("You're wonderful");
    else CView.text("You're the most amazing fuck");
    CView.text("</i>\"");
    ...
    if (player.cumQ() >= 500) CView.text("  Spermy juices run out of her pussy in alabaster streams thick enough to obscure the black-lips they're travelling over.");
    if (player.cumQ() >= 1500) CView.text("  The fox gives a surprised grin when her belly rounds slightly, though she seems concerned at how it makes her cum-filled prize wobble.");
    if (player.cumQ() >= 3000) CView.text("  Your virile pecker just won't stop, and by the time you finish ejaculating, Urta is pregnant with sperm, her belly round with its belly-button protruding.");

    CView.text("\n\nSharing an intimate moment with your ");
    if (urtaDrunk()) CView.text("drunken ");
    CView.text("lover, you kiss her again.  She returns it demurely, but midway through, you feel something slick and hard brushing up against you.  Urta whispers, \"<i>Sorry,</i>\" when you pull away to examine the sudden change.  Her pole is hardening again, surging up to rub the full prophylactic against your [chest].  You glance back at her, and all Urta has to say for herself is, \"<i>Sorry, lover... feeling you cum inside me is wonderful, and well, I had time to recover.</i>\"  She slowly pulls the rubber off her cock, careful not to jostle it too hard.  A second later, it's tied off and set aside.  With you still ");
    if (cockThatFits.hasKnot()) CView.text("knotted");
    else CView.text("resting");
    CView.text(" inside her juicy cunt, she asks if she needs another one, batting her eyelashes playfully.");

    CView.text("\n\nShaking your head from side to side, you pull back");
    if (cockThatFits.hasKnot()) CView.text(" in an attempt to withdraw.  Her entrance dilates slowly as your full, dog-like knot is pressed against her from inside, the pulsating, reddish flesh glistening with the mixed sexual lubricants.  You tug and tug, rocking your [hips] slowly to loosen her up, and eventually, you manage to pop out, drawing a moan of mixed anguish and pleasure from Urta's maw.");
    else CView.text(" and withdraw.");
    if (player.cumQ() >= 1500) CView.text("   Cum runs out of her packed womb in a wave, spreading all over the sheets down to her knees.  She shakes slightly as the cum runs out of her, rubbing her full middle protectively.");
    else CView.text("  Cum drools out of her still-gaping slit in steady trickles, puddling on the sheets below.  She groans happily.  Clearly the fox is proud to have such a productive lover.");
    CView.text("  You take hold of the puffy, dick-stretched pussy lips and spread them around two fingers, collecting some of the warm sex-juice on your fingertips, then, you pull the dripping, creamy treat out and smear it over Urta's mouth.  She giggles as you apply the lip gloss to her and slowly runs her tongue across the shining coat of sex.  You respond by repeating the motion, this time laying it on thicker.");

    CView.text("\n\n\"<i>This is all the lipstick you'll need today, babe,</i>\" you explain as you admire the way her cum-stained lips shine.  She's marked with your ejaculate and sure to have cock on her breath for hours.  Turning back to the glittering box below, you raise her balls and lean down under them, letting them fall into place on your " + describeHair(player) + " once you're in position.  You blow on the dripping cunny, and Urta moans lustily in response.  Then, you promise, \"<i>You're going to be satisfied after this.  Your dick and cunt are gonna cum so hard you won't be getting it up for days.</i>\"");
    ...
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME += 12;
...
function slurpFawkesCocksForFunAndInflation(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("urta-home-bj");
    CView.text("\"<i>Thanks, Lover! I need this. I've been so busy, I haven't had any relief in days!</i>\" Urta informs you as she lightly strokes herself to full erectness. It's a short trip, and in a few pumps, she's ready to go. You glance around the bar, making sure all possible prying eyes are occupied. Feeling satisfied with your findings, you slyly shuffle your way under the table.");
    ...
    if (player.body.wombs.find(Womb.Pregnant).pregnancy.incubation < 100 && player.body.wombs.find(Womb.Pregnant).pregnancy.incubation > 0) CView.text(" than it already was");
    CView.text(".  You slide your way back into your seat with some difficulty, fighting the wriggling, bouncing mass that is your stomach over the now shrinking room between the seat and table.  By now, some of the patrons have realized what has happened, pointing, whispering, rubbing their stomachs, and pulling their hands away from each other to symbolize your new... asset.");
    ...
    player.slimeFeed();
    UrtaFlags.URTA_CUM_NO_CUM_DAYS = 0;
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY += 3;
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME = 5;
    urtaLove(2);
    if (randInt(2) === 0) CView.text(displayModThickness(player, 100, 2));
    if (randInt(2) === 0) CView.text(displayModTone(player, 0, 2));
    return { next: returnToCampUseOneHour };
}
...
function urtaAndRaphaelSurprise(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Letting your lips split into a mischievious grin, you lean across the table and whisper into Urta's fluffy ear.  \"<i>Why don't you meet me in one of the back rooms with the lights off in about thirty minutes?  I'll see if I can get a friend, we can make a party of it.</i>\"");
    ...
    if (player.body.cocks.length > 0) CView.text(" and [eachCock] experience a twinge of excitement");
    CView.text(".  Grabbing a napkin and a pen from an accommodating waitress, you jot out a hasty missive:");
    ...
    return { next: raphaelAndUrtaPartTwo };
}

function raphaelAndUrtaPartTwo(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("urta-raphael-3some");
    CView.text("Narrow, familiar paws suddenly encircle your waist, and a puff of hot breath exhales past your ear.  \"<i>Greetings, my supple swan.</i>\"");
    ...
    if (player.body.tails.length > 0) CView.text("lift your tails and ");
    CView.text("try to relax with her thick slab of penis so close to your backdoor, hoping she'll be gentle.  A drunken twinge of arousal knifes through your [vagina] as you imagine her forcefully raping your hind-end, and Raphael can certainly feel the convulsive fluttering of your love tunnel as you sink into your imaginings.  The gray vixen nips at your ear as she presses against your [asshole]");
    if (player.analCapacity() < 30) CView.text(", slowly stretching your tight sphincter with constant pressure.  You're not really loose enough back there for this to be entirely comfortable, but the incredible fullness her length provides has a way of muting the pain of stretching.");
    else if (player.analCapacity() < 60) CView.text(", slowly sliding into your sphincter.  You groan into your other lover's mouth as your anus is claimed by the thick horse-pole, and you do your best to relax around the butthole-stretching shaft.");
    else CView.text(", easily slipping inside your well-practiced anus.  You groan into your other lover's mouth as the thick cock claims your ready asshole, and you're glad your butthole was so well prepared to handle having a hot meat pole injected into it.");
    displayStretchButt(player, 60, true, true, false);
    ...
    UrtaFlags.URTA_X_RAPHAEL_HAPPENED++;
    return { next: returnToCampUseOneHour };
}
...
function urtaGooTesticleVoreRuinedOrgasms(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("urta-goo-fuck");
    CView.text("You tell your vixen lover that you've got something a bit... special in mind this time.  You surge up onto the bed and gently flip Urta onto her back.  Her horsecock bobs enticingly in front of you, already as hard as ");
    if (!User.settings.silly()) CView.text("a steel rod");
    else CView.text("Marethian algebra");
    CView.text(".");
    ...
    return { next: urtasRuinedOrgasmsFromGooPartII };
}

function urtasRuinedOrgasmsFromGooPartII(player: Character): NextScreenChoices {
    CView.clear();
    // This is just getting weird as fuck now. There will be no images for this... how could there be?
    CView.text("You awaken the next morning to a panicked shout.  You bolt upright...  Or at least you try to.  Something is holding you tight, binding you in place.  You try to open your eyes, but nothing but blackness surrounds you.  Struggling, you try and wrench yourself free of your bindings.");
    ...
    if (edrynAvailable) CView.text("set of hoofsteps");
    else CView.text("pair of footsteps");
    CView.text(" enter whatever room you're in. \"<i>Hey, Urta, you haven't shown up for your shift today so...  Great googily moogily!  What happened to you?!</i>\" a voice you ");
    if (edrynAvailable) CView.text("recognize as Edryn's");
    else {
        if (UrtaFlags.MET_OLIVIA === 0) CView.text("recognize as Olivia's");
        else CView.text("don't recognize");
    }
    CView.text(" shouts.");
    ...
    if (edrynAvailable) CView.text("Edryn");
    else CView.text("Olivia");
    CView.text(", I'm going to need your help with this particular problem.</i>\"");

    CView.text("\n\nThere's a moment of silence, and then, \"<i>");
    if (edrynAvailable) CView.text("All right, but I'm still charging you for this one");
    else CView.text("Uhm, yes Captain, whatever you need");
    CView.text(".</i>\"  You hear armor falling to the ground while Urta rises from wherever she was sitting; you've been all over the house so many times it's hard to keep track of where you are.");

    CView.text("\n\nUrta wastes no time sinking her huge horsecock right into ");
    if (edrynAvailable) CView.text("Edryn");
    else CView.text("Olivia");
    CView.text("'s cunt, if that wet squelching noise is any indication.  She thrusts all the way in inside in one go, making ");
    if (edrynAvailable) CView.text("Edryn moan in pleasure");
    else CView.text("Olivia shout with surprise");
    CView.text(".  Clearly, Urta's not messing around anymore.  She pulls out and thrusts back in, and with that thrust you sway, slapping into ");
    if (edrynAvailable) CView.text("Edryn");
    else CView.text("Olivia");
    CView.text("'s underside.  The contact is electric.  The rough slap would have sent shivers down your spine, if you'd had one.  It makes your body quake and convulse with gusto.");
    ...
    if (player.body.cocks.length > 0) CView.text("cock");
    if (player.gender === 3) CView.text(" or ");
    if (player.body.vaginas.length > 0) CView.text("pussy");
    if (player.gender === 0) CView.text("asshole");
    CView.text(".  Your entire, gooey body feels like one giant clitoris, so sensitive, so vulnerable to pleasure.  With every slap, your body begins to crave release even more.  The pleasure builds with every thrust and accompanying slap.  It grows to the brink of orgasm, and surges forward, with no end in sight.");
    ...
    CView.text(".  Your eyes search around, seeing nothing but darkness once more.  You extend your gooey self, trying to tell where you are.  Wherever it is, it's tight... and there seems to be some sort of plug holding you in.");
    ...
    CView.text(" voices her displeasure, \"<i>What the... I look like I'm pregnant!  With septuplets!</i>\"");

    CView.text("\n\nUrta lets out a nervous chuckle.  \"<i>Sorry about that.  But this should be easier to deal with.  ");
    if (edrynAvailable) CView.text("All you need to do is push, right?");
    else CView.text("Let's just sit you back, and then all you need to do is push.");
    CView.text("</i>\"  Your new host grunts in discomfort");
    if (edrynAvailable) CView.text(", but obliges, sitting back on something, perhaps a bed.  You feel yourself sloshing around oddly as she lies on her back");
    CView.text(".  \"<i>Now push,</i>\" Urta says, encouragingly.");
    ...
    CView.text(".  The first thing out, thankfully is your head.  Bright light greets your eyes, almost stinging them.  You look around, seeing ");
    if (edrynAvailable) CView.text("Edryn clutching Urta's hand");
    else CView.text("Olivia lying back on the bed, clutching Urta's hand");
    CView.text(", panting heavily.  You surge outwards, first your shoulders, followed by one arm, and then the next.  The rest of your body comes out like a flood with one last push from your former host, sending you splattering onto the ground beneath you.  An exasperated, but relieved, sigh comes from ");
    if (edrynAvailable) CView.text("Edryn");
    else CView.text("Olivia");
    CView.text(", one that you share.");
    ...
    if (player.body.skin.tone !== "milky white") {
        ...
        player.body.skin.tone = "milky white";
        player.body.cumMultiplier += 10;
    }
    CView.text("\n\nUrta and ");
    if (edrynAvailable) CView.text("Edryn");
    else CView.text("a newly dressed Olivia");
    CView.text(" collapse onto the couch, exhausted.  \"<i>Gimme ten minutes, then we'll go on patrol,</i>\"  Urta says waving a hand tiredly.  You grin and take your leave, slipping out the front door and heading back to   The sun blazes high in the sky, around noon, so you've lost about a day stuck inside one person or another.");
    player.orgasm();
    player.stats.sens += 2;
...
function urtaPetPlayDeletedForeverBecauseThirdProovedMeWrongAboutDice(player: Character): NextScreenChoices {
    CView.clear();
    // First Time
    if (UrtaFlags.URTA_PETPLAY_DONE > 0) {
        return wearZeCollar(player);
    }
    CView.text("You hold up the leather collar you found in the previous room, asking when she picked it up.");
...
function wearZeCollar(player: Character): NextScreenChoices {
    ...
    if (UrtaFlags.URTA_PETPLAY_DONE === 0) {
        ...
        if (player.body.cocks.length > 0) CView.text("harder");
        if (player.gender === 3) CView.text(" and ");
        if (player.body.vaginas.length > 0) CView.text("wetter");
        CView.text(" at the touch, but you can't help but wonder about her words.  What else is there to being a good dog?");
    ...
    // Second Time
    else if (UrtaFlags.URTA_PETPLAY_DONE === 1) {
        ...
        if (player.body.tails.length > 0) CView.text(", tail wagging");
        CView.text(".  Urta beams at you and pats your head lovingly.");
    }
    // (Jump down to Scene Proper)
    // Repeats
    else {
        CView.text("You spy the collar sitting on a nearby dresser and pick it up.  You run your fingers over the engraved words on the collar's tag that reads '<i>Property of Urta</i>'.  Slipping it on, you also strip down until you're as nude as the day you were born, and kneel in front of your Owner");
        if (player.body.tails.length > 0) CView.text(", tail wagging");
        CView.text(".");
    }
    // Scenes Join Up Here
    ...
    if (player.body.tails.length <= 0) {
        ...
        if (player.body.tails.length <= 0 && player.body.butt.wetness > 0) CView.text("  Despite your embarrassment, or perhaps because of it, you find your asshole getting wet with desire, practically salivating in anticipation of that thick plug.");
    ...
    UrtaFlags.URTA_PETPLAY_DONE++;
    // [Doggystyle] ["Reward"] [Walkies! (Not available first time)]

    return {
        choices: [
            ["Doggystyle", doggyStyle],
            ["\"Reward\"", getAPetReward],
            ["Walkies", urtaTakesPCOnWalkies],
        ]
    };
}

// Doggystyle
function doggyStyle(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("urta-home-petplay-doggy");
    CView.text("\"<i>Hmm,</i>\" Urta wonders aloud, bringing a finger up to her mouth in mock-thoughtfulness.  \"<i>What should I do with my little pet now that " + mf(player, "he", "she") + "'s all collared up?</i>\"  Your Owner walks around you slowly, assessing your body.");
    ...
    else CView.text("tailplug");
    CView.text(" wags of its own accord, happily swishing back and forth.  Urta's hand dips between your buttocks, over your ");
    if (player.body.tails.length > 0) CView.text("asshole");
    else CView.text("plugged asshole");
    CView.text(", and between your thighs, ");
    if (player.body.cocks.length > 0) CView.text("grasping your [cock biggest]");
    else CView.text("her fingers slipping ever-so-slightly into the folds of your pussy");
    CView.text(".");

    CView.text("\n\nYou moan appreciatively as she runs her fingers along it, her fingers encircling your ");
    if (player.body.cocks.length > 0) CView.text("[cockHead biggest]");
    else CView.text("[clit]");
    CView.text(" while her other hand keeps your upper body firmly down.  With every stroke your body burns just a bit hotter, your breath coming a bit shorter.  ");
    if (player.body.cocks.length > 0) CView.text("Her thumb rubs against the sensitive underside of your cock");
    else CView.text("One finger slips into you, all the way down to the knuckle, and swivels around to caress your g-spot with expert accuracy");
    CView.text(", making you bite your lip in ecstasy.  You want to cry out right now, but you know you can't break character right now.  Not this early in.  The strokes come faster, and you can feel the orgasmic flames coming on; your limbs tingle and your legs are already wobbling.");
    ...
    if (player.body.cocks.length > 0) CView.text("[cock biggest] does the same");
    else CView.text("[vagina] grows slicker");
    CView.text(".  She really is going to tease you as long as she can, you think to yourself.");

    CView.text("\n\nThe first hand drifts from your belly back towards your ");
    if (player.body.cocks.length > 0) CView.text("cock");
    else CView.text("pussy");
    CView.text(", taking over where it left off just moments ago.  ");
    if (player.body.cocks.length > 0) CView.text("She wraps her fingers around your shaft, slowly pumping up and down, taking a moment to tease your [cockHead biggest] every few strokes.");
    else CView.text("She buries two fingers deep inside your cunt, followed by a third, each one twisting around to rub at your g-spot softly.");
    CView.text("  The hand on your chest switches to ");
    if (player.body.chest.reduce(BreastRow.TotalNipples, 0) === 2) CView.text("the ");
    else CView.text("an ");
    CView.text("other nipple, tweaking and teasing that one while she simultaneously ");
    if (player.body.cocks.length > 0) CView.text("jerks you off");
    else CView.text("rubs you from the inside out");
    CView.text(".  Again comes the flame of orgasmic bliss, lapping at your tingling extremities.  Your body feels suddenly electric, so overcharged with sweet, sweet pleasure that you find it hard to think.  Low moans escape your throat, entirely unbidden, while ");
    if (player.body.cocks.length > 0) CView.text("pre-cum dribbles from your dick like a leaky faucet");
    else CView.text("your folds grow even more damp, constantly drizzling fluids that run down your ass crack, pooling on the floor like a lewd lake");
    if (player.body.cocks.length > 0) CView.text(", and to top it all off, you find your [leg] inexplicably kicking in mid-air");
    CView.text(".");
    ...
    if (player.body.tails.length <= 0) CView.text(", while she grasps the plug in your ass firmly and pulls. It comes out easily with a wet <b>pop</b>, making your ass feel suddenly empty.  You guess it won't be empty for long, however");
    CView.text(".  Finally she rubs it against your [asshole], teasing you with every brush of skin against skin.");
    ...
    if (player.body.chest.sort(BreastRow.Largest)[0].rating <= 5) CView.text("resting on your folded arms");
    else CView.text("resting on your enormous boobs");
    CView.text(", while your ass remains high in the air, all the better to be fucked by your hermaphroditic Owner.  Urta pulls out a few inches just to push in even deeper, and the feeling is exquisite as, with each thrust, you get the sensation of being filled just a little bit more.  By the time her large balls slap into your ");
    if (player.body.balls.count > 0) CView.text("own");
    else if (player.body.cocks.length > 0) CView.text("painfully erect member");
    else CView.text("slit");
    CView.text(", you are overwhelmed by the full, content feeling.  Your eyes roll back in their sockets, and it's hard to focus on anything but the twenty inch dick buried in your sensitive ");
    if (!User.settings.silly()) CView.text("ass");
    else CView.text("boypussy");
    CView.text(".");
    displayStretchButt(player, 60, true, true, false);

    CView.text("\n\nSoon, Urta develops a good rhythm, thrusting in and out, her balls slapping your ");
    if (player.body.cocks.length > 0) CView.text("dick");
    else CView.text("cunt");
    CView.text(" in such a way that it tingles powerfully, fueling your impending orgasm even more.  Locked in this cadence, she pounds your ass for what seems like hours.  The air grows damp and sweat drips from the two of you, only adding to your desire.  It's not long after until your nostrils are assaulted by the salty scent of sex.  Your ");
    if (player.body.cocks.length > 0) CView.text("[cock] slaps against your belly roughly with every passing second, smearing thick globules of precum which drip down your belly, tickling your flesh like little balls of distilled pleasure");
    if (player.gender === 0) CView.text(", while your");
    if (player.body.vaginas.length > 0) CView.text(" unused pussy aches to be filled or touched, its lips puffy and red, sensitive to every hard slap of your foxy lover's balls");
    CView.text(".");
    ...
    if (player.body.cocks.length > 0) {
        CView.text("groin, sending a wave of tremors down your cock, which suddenly erupts, covering your belly and [chest] in warm gooey goodness.");
        if (player.cumQ() >= 500) CView.text("  It continues to jerk on its own, unloading its massive load until you're laying in a puddle of your own semen.");
        if (player.body.vaginas.length > 0) CView.text("  It then travels along to your ");
    }
    if (player.body.vaginas.length > 0) CView.text("[vagina], which, despite being devoid of cock, clamps down anyway, gushing with femcum, drenching your Owner's balls and legs in sticky, clear fluids.");
...
// "Reward"
function getAPetReward(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("urta-home-petplay-reward");
    CView.text("\"<i>Well, you've been a good little pet so far,</i>\" Urta says, patting you on the head. \"<i>I think a little... reward is in order.</i>\"  You quirk an eyebrow, wondering about that pause, but wag your ");
    if (player.body.tails.length > 0) CView.text("tail");
    else CView.text("tailplug");
    CView.text(" enthusiastically.");
    ...
    if (player.body.cocks.length > 0) CView.text("hard");
    if (player.gender === 3) CView.text(" and ");
    if (player.body.vaginas.length > 0) CView.text("wet");
    CView.text(", oddly enough, thinking of what your 'reward' could be.  Urta returns several minutes later, with one hand holding some kind of metal dish... a food bowl for a dog!");
    if (User.settings.silly()) CView.text("  In her other hand is what looks to be a jar of peanut butter.  What use could that have?");
    ...
    if (User.settings.silly()) CView.text(", though you wonder if perhaps that's what the peanut butter is for");
    CView.text(".  Your Owner stretches a foot out towards you, running her stocking-clad paw along your side.  The little tufts of fur protruding from the fishnet tickle your skin in a sensual fashion, and you can't stop your spine from stiffening at the contact.");
    ...
    if (player.body.chest.sort(BreastRow.Largest)[0].rating < 1) CView.text("onto your chest");
    else CView.text("between your breasts");
    CView.text(".  She takes a moment to encircle one of your nipples, which stiffens to attention along with ");
    if (player.body.chest.reduce(BreastRow.TotalNipples, 0) === 2) CView.text("its twin");
    else CView.text("the others");
    CView.text(", before sending the paw down further where it makes contact with your ");
    if (player.body.cocks.length > 0) CView.text(describeCocksLight(player));
    else CView.text("[vagina]");
    CView.text(".  There's nothing you can do to stifle the moan of pleasure that escapes your lips as your Owner's paw travels the length of your ");
    if (player.body.cocks.length > 0) CView.text("rapidly hardening shaft.");
    else CView.text("moistening slit.");

    CView.text("\n\nUrta keeps this up for a minute, lovingly stroking your ");
    if (player.body.cocks.length > 0) CView.text("dick");
    else if (player.body.vaginas.length > 0) CView.text("cunt lips");
    CView.text(" with a paw, until you bite your lip and whine at her.  She gives you a little smirk and the foot retreats.  \"<i>All right then, pet, come and get your bone,</i>\" Urta gestures down at the half-erect cock sprouting from between her thighs");
    // silly:
    if (User.settings.silly()) CView.text(", while she opens the jar of peanut butter and smears a dollop across the flat head.  Is... is that supposed to be enticement?  She really is treating you like a real dog");
    CView.text(".  You crawl towards her nervously, and start to reach up to grasp her cock, but again she mutters, \"<i>Ah-ah, good doggies don't use their hands.</i>\"");

    CView.text("\n\nNodding, you extend your tongue, running it along the length of her hardening shaft");
    if (User.settings.silly()) CView.text(", licking all of the peanut butter off the tip");
    CView.text(".  Urta shivers and lets out a \"<i>Mm,</i>\" while her pillar of horsecock reaches its full size, clocking in at what has to be a good twenty inches.  ");
    if (User.settings.silly()) CView.text("  Your owner uses her fingers to smear another large dollop of peanut butter along her own shaft, giving you an almost sadistic smirk.");
    CView.text("  You smooch and lick your way down her shaft, lapping at every little vein while Urta leans back and enjoys your ministrations.  Little gasps of pleasure escape from her lips, while moans leave yours as you smooch and suckle your way down to the base of that towering horsecock.");

    CView.text("\n\nYour foxy Owner's prick glistens by the time you finally take it into your mouth");
    if (User.settings.silly()) CView.text(", the peanut butter now long forgotten");
    CView.text(", stretching your jaw wide.  ");
    if (player.body.cocks.length > 0) CView.text("Your own cock remains almost painfully rock hard, bobbing awkwardly beneath you, dripping precum onto the floor and bowl.");
    if (player.body.vaginas.length > 0) {
        if (player.gender === 3) CView.text("  Not to mention");
        else if (player.body.vaginas.length > 0) CView.text("  Meanwhile");
        CView.text(", your pussy is as wet as it's ever been, constantly leaking juices that run down your legs, pooling on the floor.");
    }
    CView.text("  You swirl your tongue around Urta's flare, paying homage to the horsecocked goddess.  Lapping at the slit for a moment, you slip your ");
    if (player.body.tongue.type === TongueType.SNAKE) CView.text("serpentine ");
    else if (player.body.tongue.type === TongueType.DEMONIC) CView.text("demonic ");
    else if (player.body.tongue.type === TongueType.DRACONIC) CView.text("draconic ");
    CView.text("tongue along the vixen's thick cumvein, which pulses lightly.  Urta groans and runs a hand through your " + describeHair(player) + ", gripping it solidly.  You can't help as you're forced further onto the warm rod.");
    ...
    CView.text("tongue, lapping at the top layer and swallowing it down.  It's still warm, and quite salty with a bit of a sweet tang to it.  You quickly dive back into your 'reward', using your tongue as a spoon to consume every last drop in the bowl.  While you're busy doing that, Urta slides off the bed and takes up a position behind your body, ass high into the air, showing off all her goods.  The fox raises a paw, smearing some of her cum along your ");
    if (player.body.cocks.length > 0) CView.text("shaft ");
    if (player.gender === 3) CView.text(" and ");
    if (player.body.vaginas.length > 0) CView.text("slit ");
    CView.text("tantalizingly.");

    CView.text("\n\nWith every dollop of cum you consume, she gives your ");
    if (player.body.cocks.length > 0) CView.text("[cockHead biggest]");
    else CView.text("[clit]");
    CView.text(" a little twirl with a soft, padded toe.  You can't help but moan into the bowl as you go, your legs shivering and stomach twisting with desire.  You continue to lap at the bowl eagerly downing mouthful after mouthful and being rewarded with more pleasure.  Even when you've licked the bowl clean, you find yourself lowering your head further and slurping away the remnants that splattered onto the floor.  Urta seems to find this satisfactory, as she picks up the pace, ");
    if (player.body.cocks.length > 0) CView.text("jerking off your cock between two toes");
    else CView.text("rubbing your little love button like mad with one soft toe");
    CView.text(", until you just can't handle any more.");
    ...
    if (player.body.cocks.length > 0) CView.text("The cock between your legs, meanwhile, stiffens, briefly becoming harder than you've ever felt before, before blasting the floor with jet after jet of silky white cum.");
    else CView.text("Meanwhile, your vagina clamps on nothing, achingly, painfully empty.  Juices spray out onto Urta's foot and down your thighs, soaking the floor beneath you in yet more slippery femcum.");
    CView.text("  Your Owner doesn't just let it end there, however, she continues her paw-assault on your crotch, stroking and rubbing until you become little more than a gibbering mess of orgasm-infused flesh laying in a puddle of your own sexy goo.");
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME += 2;
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY += 2;
    CView.text("...");
    player.orgasm();

    return { next: feedTheBeastPartII };
}

function feedTheBeastPartII(player: Character): NextScreenChoices {
    CView.clear();
    Time.hour += 2;
...
// Walkies!
function urtaTakesPCOnWalkies(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("urta-home-petplay-walkies");
    CView.text("\"<i>Ah, I know just what we should do!</i>\" She claps her hands together and rummages through her dresser.  Urta throws on a simple little yellow sundress and makes her way to the door.  \"<i>Come on, pet</i>\" she says with a mischevious smirk, patting her leg and holding up a leather leash. \"<i>It's time to go for a walk!</i>\"");
    ...
    if (player.body.cocks.length > 0) CView.text("[eachCock] grows hard beneath you");
    if (player.gender === 3) CView.text(" and ");
    if (player.body.vaginas.length > 0) CView.text("your [vagina] moistens, ready to be on display");
    CView.text(".");
    ...
    CView.text(" a nervous little shake.  Urta smiles and opens the door wide, making you squint in the bright daylight.  You feel a tugging at your neck, and realize Urta's already set off.  You scamper off after her on all fours, not wanting to disappoint your Owner.  The two of you travel through some back alleys, thankfully free of strangers, until you come out into the main thoroughfare of the city.");
    ...
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 5) CView.text("the breasts hanging from your chest and ");
    CView.text("your openly displayed rear end.  ");
    if (player.body.cocks.length > 0) CView.text("Pre-cum drizzles from the end of your painfully erect cock");
    else CView.text("Sweet juices flow from your cunt, drizzling down onto the ground");
    CView.text(", creating a trail of horniness alongside Urta's.");
    ...
    if (player.gender === 3) CView.text(" and ");
    if (player.body.vaginas.length > 0) CView.text("wetter");
    CView.text(".  You feel like you really are a pet, out for a walk with your Owner.  It's an odd, and slightly freeing revelation, and once you come to grips with it, your entire experience changes. You find yourself trotting ahead of Urta, happily displaying yourself to all who wish to see.");
    ...
    UrtaFlags.URTA_TIME_SINCE_LAST_CAME = 0;
    UrtaFlags.URTA_COMFORTABLE_WITH_OWN_BODY += 5;
    if (UrtaFlags.PC_FETISH > 0) {
        CView.text("  <b>You're so fucking turned on after exhibiting yourself in such a way that you're dripping EVERYWHERE.  Such a good doggie.</b>");
        player.stats.lustNoResist += 100;
