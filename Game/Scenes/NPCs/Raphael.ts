...
export function RaphaelLikes(player: Character): boolean {
    ...
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 12) return false;
    // ({If player has below C cup breasts}
    if (player.body.chest.sort(BreastRow.Largest)[0].rating < 3) return false;
...
function RaphaelFirstMeetingSWOON(player: Character): NextScreenChoices {
    ...
    CView.text("\"<i>Normally I rob opulent merchants and criminals. I have no idea what drew me into your  Perhaps destiny willed this fateful meeting?</i>\" He looks up and burrs with a suave accent. \"<i>Then again, I am attracted to extraordinary splendor.</i>\" He gazes at you with an emerald shimmer in his rich green eyes.\n\n");
...
function RaphaelDressPtII(player: Character): NextScreenChoices {
    ...
    return player.inventory.items.createAdd(player, ItemType.Armor, ArmorName.RedBodysuit, campMenu);
}
...
function RaphaelEncounterIIDressFollowup(player: Character): NextScreenChoices {
    ...
    // ({If player does not meet the first encounter requirements:}
        ...
        // ({If player has above E cup breasts}
        if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 7) CView.text("\"<i>Your female curves... replaced with such... udders!</i>\" He looks at your bosom. \"<i>No woman could be elegant with such monstrosities up front!</i>\"\n\n");
        // ({If player has below C cup breasts}
        if (player.body.chest.sort(BreastRow.Largest)[0].rating < 3) CView.text("\"<i>Your female curves... gone!</i>\" He looks at your bosom. \"<i>It's hard to tell you apart from a little girl!</i>\"\n\n");
        // ({If player has grown less than girly hips}
...
function RaphaelPicnicII(player: Character): NextScreenChoices {
    ...
    // [Discuss] [Skill] [Flirt]
    return { choices: [["Fencing", RaphaelPicnicSkill], ["Thieving", RaphaelPicnicChooseThieving], ["Flirt", undefined]] };
}
function RaphaelPicnicEnd(player: Character): NextScreenChoices {
    ...
    if (RaphaelFlags.FUCK_OFF_THEIVING_RAPHAEL === 0) {
        ...
        CView.text("You catch the object: he has returned the priceless ruby pendant. You recollect yourself and head back to ");
    }
    else {
        ...
        CView.text("You recollect yourself and head back to ");
    }
    return { next: campMenu };
}
...
function fenceOfferChangeToDiscuss(player: Character): NextScreenChoices {
    CView.clear().text("You blush, intimidated, and change the subject.\n\n");
...
function postRaphaelCoitus(player: Character): NextScreenChoices {
    ...
    if (RaphaelFlags.RAPHAEL_RAPIER_TRANING === 4) {
        ...
        return player.inventory.items.createAdd(player, ItemType.Weapon, WeaponName.RaphaelsRapier, campMenu);
    }
    ...
    if (RaphaelFlags.RAPHAEL_INTELLIGENCE_TRAINING === 4) {
        ...
        player.perks.add(PerkType.Misdirection, 0, 0, 0, 0);
    }
    ...
    // [Removes Raph from the  7 days later, the Quicksilver scene plays out.]
    RaphaelFlags.RAPHEAL_COUNTDOWN_TIMER = 7;
    // Next button if not taking Rapier
    if (RaphaelFlags.RAPHAEL_RAPIER_TRANING !== 4) return { next: campMenu };
}
...
function declinePuttingOutForRogues(player: Character): NextScreenChoices {
    ...
    CView.text("When he gets off your body, you've won.  You've caught Raphael in his own   You've led him around.  He'd probably do anything for it, but cannot obtain it.\n\n");
...
function RaphaelPicnicChooseThieving(player: Character, newl: boolean = true): NextScreenChoices {
    ...
    // {Picnic Thieving at 40-49 Int. Must play out at least once.}
        ...
        CView.text("He draws a weary smile, not completely disheartened.  <i>\"It is my natural state of being... though I must confess; it takes effort to stay on top of my \"</i>\n\n");

        CView.text("When you ask him what he does for relaxation, the rogue raises his shoulders.  <i>\"Being less than on the very top of my   An audience will never know the difference however.  Working the crowds is vital.\"</i>  When you ask him if that includes you, he merely grins.\n\n");
...
function RaphaelThieverySmexPtII(player: Character): NextScreenChoices {
    ...
    // [Removes Raphael from  In 7 days, the quicksilver scene plays out]
    RaphaelFlags.RAPHEAL_COUNTDOWN_TIMER = 7;
...
function quiksilverFawkesEndGame(player: Character): NextScreenChoices {
    ...
    CView.text("It is an eerily misty morning outside, with banks of fog covering your campsite.  It only makes the second howl more haunting as the danger appears to draw closer to   You stand still and prepare yourself to face this menace, peering into the fog.  When a shadow creeps closer, you prime for combat.  However, its posture suggests it is weary and tired, slumping against a tree. When it struggles forward, it almost comes falling out of the curtaining mist.  It is only then that you notice the redness of its fur and the fineness of its clothes.  It is the fox and russet rogue, Raphael!\n\n");
    ...
    CView.text("You don't have the chance to respond before the sounds of snapping branches and rustling leaves near   Raphael jumps up and dives into the barrel you keep in camp for the storage of water, hiding himself and leaving you to face the oncoming threats by yourself.\n\n");
...
function QuiksilverFawkesPtII(player: Character): NextScreenChoices {
    ...
    CView.text("<i>\"Tel'Adre law enforcement, ma'am.  Special Division.  Outer wall Task Force,\"</i> he announces while the other wolf circles   <i>\"I am Frank 'Skoll' Holmes and this is my partner, Joe 'Hati' Watson.  We're the 'Quicksilver' brothers, also known as the 'Boys from Baskerville' by the locals.\"</i>  Skoll achieves this introduction with a soft voice that barks and growls itself through the words. You nod, still a bit taken aback.\n\n");
...
export function orphanageIntro(player: Character): NextScreenChoices {
    ...
    if (RaphaelLikes(player)) {
        if (RaphaelFlags.TIMES_ORPHANAGED_WITH_RAPHAEL === 0) {
            ...
            return { next: raphaelIntroPageTwo };
        }
...
function raphaelIntroPageTwo(player: Character): NextScreenChoices {
    CView.clear();
...
function evenMoreRaphaelIntro(player: Character): NextScreenChoices {
    CView.clear();
...
// After Sex1
function followupToFirstTimeOrphanageRaphSex(player: Character): NextScreenChoices {
    CView.clear();
...
function talkWithRedFoxLooksOutHesASpy(player: Character): NextScreenChoices {
    CView.clear();
    ...
    const menu = raphaelOrphanageSexMenu();
    menu.choices[4] = ["Leave", returnToCampUseOneHour];
    return menu;
}
