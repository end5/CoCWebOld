...
import { returnToCampUseOneHour, campLoversMenu, companionsCount, followersCount, sleepWrapper, sleepRecovery } from "../Camp";
...
export const ArianFlags = {
    ...
    ARIAN_ASS_CHAT: 0,
    ARIAN_HAS_BLOWN: 0,
    ARIAN_DOUBLE_PENETRATION_CHAT: 0,
    HYPER_HAPPY: 0,
    ARIAN_FIRST_REPTILUM: 0,
    ARIAN_MORNING: 0,
    ARIAN_EGG_CHAT: 0,
    ARIAN_EGG_COLOR: "",
    TIMES_ARIAN_DILDOED: 0,
};
User.flags.set(FlagType.Arian, ArianFlags);
...
*/
export class ArianScene implements ITimeAware {
    public timeChange(): boolean {
        if (ArianFlags.ARIAN_EGG_COUNTER > 0) ArianFlags.ARIAN_EGG_COUNTER++;
        if (Time.hour > 23) {
            if (arianFollower() && ArianFlags.ARIAN_VAGINA > 0) ArianFlags.ARIAN_EGG_EVENT++;
            ArianFlags.ARIAN_LESSONS = 0;
            ArianFlags.ARIAN_TREATMENT = 0;
        }
        return false;
    }
...
function arianCockSize(): number {
    if (ArianFlags.ARIAN_COCK_SIZE < 0 || ArianFlags.ARIAN_COCK_SIZE > 3) return 0;
    else if (ArianFlags.ARIAN_COCK_SIZE === 1) return 9;
    else if (ArianFlags.ARIAN_COCK_SIZE === 2) return 16;
    else return 36;
}

export function arianFollower(): boolean {
    return ArianFlags.ARIAN_FOLLOWER > 0;
}
export function arianMF(boy: string, girl: string): string {
    if (ArianFlags.ARIAN_COCK_SIZE > 0) {
        if (ArianFlags.ARIAN_VAGINA > 0) return girl;
        else return boy;
    }
    return girl;
}
export function arianHealth(arg: number = 0): number {
    if (arg !== 0) {
        ArianFlags.ARIAN_HEALTH += arg;
        if (ArianFlags.ARIAN_HEALTH > 100) ArianFlags.ARIAN_HEALTH = 100;
        else if (ArianFlags.ARIAN_HEALTH < 0) ArianFlags.ARIAN_HEALTH = 0;
    }
    return ArianFlags.ARIAN_HEALTH;
}
export function arianChestAdjective(): string {
    let buffer: string = "";
    const temp: number = randInt(10);
    if (ArianFlags.ARIAN_BREASTS === 0) return "";
    else if (ArianFlags.ARIAN_BREASTS === 1) {
        if (temp <= 4) buffer += "small";
        else if (temp <= 6) buffer += "petite";
        else if (temp <= 8) buffer += "perky";
        else buffer += "palm-filling";
    }
    else if (ArianFlags.ARIAN_BREASTS === 2) {
...
export function arianChest(): string {
    ...
    if (ArianFlags.ARIAN_BREASTS === 0) return "chest";

    // Tits ahoy!
    if (randInt(2) === 0) buffer += arianChestAdjective() + " ";
...
function dontHelpArianWhenYouMeetHim(player: Character, never: boolean): NextScreenChoices {
    ...
    if (never) {
        ArianFlags.ARIAN_PARK = -1;
    }
    else {
        ArianFlags.NOT_HELPED_ARIAN_TODAY = 1;
    }
...
// [=Help=]
function helpArianWhenYouMeetHim(player: Character): NextScreenChoices {
    CView.clear();

    ArianFlags.ARIAN_PARK = 1;
    ArianFlags.ARIAN_COCK_SIZE = 1;
    ArianFlags.ARIAN_CAPACITY = 50;

    CView.text("You approach the hooded figure with caution, asking if they're all right; it feels a little silly to say that, but you can't think of much else to say.");
    ...
    CView.text("\n\nYou decide to leave him for the moment, and head back to the ");
...
export function visitThePark(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("As you enter the ragged remnants of the park, you spot the sickly lizan, Arian, sitting at his usual bench, and greet him.  \"<i>Oh, hello there [name].  Good to see you.</i>\"  He waves lazily.");

    // Visit 1
    if (ArianFlags.ARIAN_PARK === 1) {
        CView.text("\n\nFeeling ");
        if (player.stats.cor < 50) CView.text("curious");
        else CView.text("bored");
        CView.text(", you decide to ask him what his story is.");
        ...
        if (player.combat.hasSpells()) {
            CView.text("  You thought spellcasting merely took fatigue and the proper mindset, not life force, and you express that sentiment to the lizan.");
        }
        ...
        CView.text("\n\nYou excuse yourself and head back to ");
    }
    // Visit 2
    else if (ArianFlags.ARIAN_PARK === 2) {
    ...
    // Visit 3
    else if (ArianFlags.ARIAN_PARK === 3) {
        ...
        if (player.stats.cor > 66) CView.text(", whilst privately thinking to yourself how useless that looks - no wonder they can't fight the demons if this is the best they're capable of");
        CView.text(".");
        ...
        if (player.stats.cor >= 66) CView.text(", keeping your real reasons for bothering with him to yourself");
        CView.text(".");
    ...
    arianHealth(1);
    ArianFlags.ARIAN_PARK++;
    return { next: returnToCampUseOneHour };
}

// First Visit
export function visitAriansHouse(player: Character): NextScreenChoices {
    CView.clear();
    if (ArianFlags.ARIAN_HEALTH < 29 || ArianFlags.ARIAN_VIRGIN === 1) arianHealth(1);
    if (arianFollower()) {
        CView.text("You approach the enchanted tent and slip easily inside the doors to the luxurious interior.  ");
        const temp: number = randInt(10);
        if (temp === 0) {
            CView.text("However, Arian isn't here right now, so you instead make yourself comfortable on the couch.  After a few minutes, Arian [Arian emself] walks in through the entrance.  \"<i>Oh, [name].  I wasn't aware you were here... have you been waiting for long?</i>\" [Arian ey] asks.  You tell [Arian em] not very long.  \"<i>That's good to hear.  So, what can I do for you?</i>\" [Arian ey] asks, with a smile.");
        }
        else if (temp === 1) {
            CView.text("Inside, the lizan is sitting at a table, fastidiously drinking from a cup of something hot while pouring over an arcane-looking text.  You politely cough to draw [Arian eir] attention and [Arian ey] looks at you, smiling.  \"<i>Hello, [name].  I was just catching up on my studies.  Can I offer you a cup of tea, or maybe something else?</i>\" [Arian ey] asks.");
        }
        else if (temp <= 2) {
            CView.text("The lizan is currently busy tinkering with some occult-looking paraphernalia when you find [Arian em].  You politely cough to attract [Arian eir] attention, then do so again when [Arian ey] fails to heed that.  It's only on the third attempt that [Arian ey] looks up apologetically from [Arian eir] work.  \"<i>Ah, [name]; I'm sorry, but I was preoccupied with something.</i>\" [Arian ey] states in an apologetic tone, indicating the mess on [Arian eir] desk.  \"<i>Was there something you wanted?</i>\" [Arian ey] asks.");
        }
        else if (temp <= 4) {
            CView.text("The smell of fresh cooking fills the air and you can see Arian happily sitting down at his couch with a plate of something just cooked.  \"<i>Oh, [name]; I was just about to eat, can I offer you a dish?  Or if you'd rather do something else, this can wait,</i>\" the lizan tells you with a smile.");
        }
        else if (temp <= 6) {
            ...
            CView.text("A strange smell hits your nose as you enter the tent; it takes you a few moments, but then you place it, your ears pricking as you hear Arian letting out some very familiar groans of release.  With a smirk, you sneak up and lean over the couch, looking right into the eyes of Arian, [Arian eir] fingers still glistening with ");
            if (ArianFlags.ARIAN_COCK_SIZE > 0) {
                if (ArianFlags.ARIAN_VAGINA > 0) CView.text("mixed sexual fluids");
                else CView.text("jizz");
            }
            else CView.text("femjizz");
            CView.text(" and, indeed, still hovering over [Arian eir] ");
            if (ArianFlags.ARIAN_COCK_SIZE > 0) {
                if (ArianFlags.ARIAN_VAGINA > 0) CView.text("twin sexual slits");
                else {
                    CView.text("cock");
                    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
                }
            }
            else CView.text("pussy");
            CView.text(".  For a long moment, [Arian ey] just stares back at you; if lizans could blush, you're certain [Arian ey]'d be red as a beet.  \"<i>[name]! I was- I was just....  Oh, this is embarrassing,</i>\" [Arian ey] mutters, looking at [Arian eir] feet.");
        ...
        if (ArianFlags.ARIAN_PARK === 4) {
            ...
            if (player.stats.cor < 33) CView.text("curiously");
            else if (player.stats.cor < 66) CView.text("casually");
            else CView.text("indifferently");
            CView.text(" ask why he's thanking you.");
            ...
            else CView.text("seat your tauric body on the floor");
            CView.text(".  You rack your brains for polite conversation, and finally ask how he's been since you saw him last.");
            ...
            CView.text("\n\nYou promise to keep an eye out.  You then head back out to check up on your ");
            ...
            if (ArianFlags.ARIAN_HERM_CHAT === 1) {
                ...
                // ArianGirlHermChat++;
                ArianFlags.ARIAN_HERM_CHAT++;
            }
            // 0-9 health:
            else if (ArianFlags.ARIAN_HEALTH < 10) {
            ...
            // 10-19 health:
            else if (ArianFlags.ARIAN_HEALTH < 20) {
            ...
            // 20-29 health:
            else if (ArianFlags.ARIAN_HEALTH < 30) {
                // Repeat this until the PC decides to sex Arian up somehow.
                if (ArianFlags.ARIAN_VIRGIN === 0 && ArianFlags.ARIAN_S_DIALOGUE === 2) {
                    ...
                    return {
                        choices: [
                            ["Eavesdrop", eavesDropOnArian],
                            ["Peep", peepOnArian],
                            ["Barge In", bargeInOnArian],
                            ["Leave", leaveFappingArian],
                        ]
                    };
                }
                // (else)
            ...
            // 30-49 health:
            else if (ArianFlags.ARIAN_HEALTH < 50) {
            ...
            // 50-74 health
            else if (ArianFlags.ARIAN_HEALTH < 75) {
                ...
                if (!player.body.legs.isTaur()) CView.text("take the offered chair");
                else CView.text("the offered chair");
                CView.text(".");
            ...
            // 75-100 health:
                ...
                if (!player.body.legs.isTaur()) CView.text(" and offers you the chair nearby");
                CView.text(".  Taking another for [Arian em]self.  \"<i>I love when you come visit, [name].  So, what are we going to do today?</i>\" [Arian ey] asks, expectantly.");
            }
            // (Display Options)
            return arianHomeMenu(player);
        }
...
function arianHomeMenu(player: Character): NextScreenChoices {

    const choices: ScreenChoice[] = [];
    if (ArianFlags.ARIAN_S_DIALOGUE === 0 && arianHealth() >= 10) choices[0] = ["Next", arianStoryDialogue1];
    else if (ArianFlags.ARIAN_S_DIALOGUE === 1 && arianHealth() >= 20) choices[0] = ["Next", arianStoryDialogue2];
    else if (ArianFlags.ARIAN_S_DIALOGUE === 2 && arianHealth() >= 30) choices[0] = ["Next", arianDialogue3];
    else if (ArianFlags.ARIAN_S_DIALOGUE === 3 && arianHealth() >= 50) choices[0] = ["Next", arianImbue];
    else if (ArianFlags.ARIAN_S_DIALOGUE === 4 && arianHealth() >= 75) choices[0] = ["Next", arianPlot4];
    else if (ArianFlags.ARIAN_S_DIALOGUE === 5 && arianHealth() >= 100) choices[0] = ["Next", arianPlot5];
    // If no story dialogue
        ...
        if (ArianFlags.ARIAN_S_DIALOGUE >= 5) choices[4] = ["Treat Corr.", treatCorruption];
        if (Time.hour >= 17 && arianFollower()) choices[8] = ["Sleep With", partial(sleepWithArian, player, true)];
        if (ArianFlags.SLEEP_WITH === "Arian") choices[8] = ["NoSleepWith", dontSleepWithArian];
        if (!arianFollower()) choices[9] = ["Back", telAdreMenu];
        else choices[9] = ["Back", campLoversMenu];
    }

    return { choices };
}

function dontSleepWithArian(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You decide not to sleep with Arian at night, for now.");
    ArianFlags.SLEEP_WITH = "";
    return arianHomeMenu(player);
}

// [=Eavesdrop=]
function eavesDropOnArian(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You sidle up to the door, pressing your ear against the wood and start to listen intently.");
    ...
    const choices: ScreenChoice[] = [];
    choices[0] = ["Barge In", bargeInOnArian];
    choices[4] = ["Leave", leaveFappingArian];

    return { choices };
}

// [=Peep=]
function peepOnArian(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Curious, you decide to take a little peek through the lock; you press yourself against it as best you can, looking through into the bedroom beyond.  True to what your ears heard, the sickly albino's health has improved enough for him to focus on more... carnal matters.  Naked from the waist down, he sits on the edge of his bed, groinal slit disgorging a single, average-sized phallus.  Maybe 6 inches long, it's a bright purple-red color, covered in strange lumps");
    if (player.body.cocks.filter(Cock.FilterType(CockType.LIZARD)).length > 0) CView.text(" just like yours");
    CView.text(", though this isn't stopping him from enthusiastically stroking himself off.");
...
// [=Leave=]
function leaveFappingArian(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You decide to let Arian have some privacy and leave for the moment... after all, what the lizan mage does in his free time is not really your business....");
    ...
    CView.text("\n\nYou promise to return later and bid him farewell.  You step outside and make your way back to your ");
    // Return to camp
    return { next: returnToCampUseOneHour };
}

// [=Barge in=]
function bargeInOnArian(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("With a wry smirk you turn the knob and find that Arian's door is unlocked; without missing a beat, you open the door and step in right in time to see a sticky rope of pre paint Arian's slender belly as he scrambles to cover himself up.");
    ...
    if (player.body.cocks.length > 0 && player.body.vaginas.length <= 0) {
        ...
        return {
            choices: [
                ["Don't Mind", youDontMindBeingGayForArian],
                ["Like Girls", youLikeGirlsNotSickLizardDudes],
            ]
        };
    }
    // (else if PC is genderless)
        ...
        // (Display Sex Menu)
        return arianSexMenu(player, false);
    }
    // (else if PC is female)
        ...
        return { next: getPenetratedByArianAndHisHitlerMustache };
    }
    // (else) //if PC is a herm
        ...
        CView.text("\n\nArian hides his face once more inside his covers and says in a whisper, \"<i>Yes....</i>\"");
        CView.text("\n\nYou pull the covers off his face and say, \"<i>Well... we'll have to fix that then.</i>\"  You slip off his bed and begin stripping off your [armor].  Arian shyly does the same, stripping off his robes until he is laying in his bed, completely naked.");
        CView.text("\n\nOnce you toss your [armor] on the floor, however, Arian's eyes widen as he realizes you're not entirely female; he eyes your " + describeCocksLight(player) + " and the moistening pussy between your legs with equal parts wonder and arousal.");
        ...
        return {
            choices: [
                ["Like Male", hermsLikeMaleArian],
                ["Like Female", hermsLikeFemaleArian],
            ]
        };
    }
...
// [=Like Male=]
function hermsLikeMaleArian(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You tell him that's not the case for you; you don't have a problem with him being a guy.  In fact, you think he looks very cute, earning you a nervous smile.  Arian relaxes, letting you look over his body and decide what you want to do....");
    // (Should you penetrate him or mount him?)
    // Penetrate - (Proceed to appropriate scene)
    // Get Penetrated - (Proceed to appropriate scene)

    const choices: ScreenChoice[] = [];
    if (player.body.cocks.length > 0 && player.body.cocks.find(Cock.CockThatFits(50))) choices[0] = ["Penetrate", giveArianAnal];
    choices[1] = ["Get Penetrated", getPenetratedByArianAndHisHitlerMustache];

    return { choices };
}
// [=Prefer Female=]
function hermsLikeFemaleArian(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You tell him that while you do like to play with guys once in a while, you prefer girls.");
    ...
    ArianFlags.ARIAN_HERM_CHAT = 1;
    ArianFlags.ARIAN_VAGINA = 1;
    ArianFlags.ARIAN_COCK_SIZE = 0;
    ArianFlags.ARIAN_BREASTS = 1;

    return { next: penetrateArian };
}

// [=Don't mind=]
function youDontMindBeingGayForArian(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You tell him that you don't have a problem with males, as long as they're cute.  You smile at him.  \"<i>You... do you really think I'm cute?</i>\"");
    ...
    if (player.body.cocks.find(Cock.CockThatFits(50))) {
        return { next: getBlownByArian };
    }
    else {
        return { next: giveArianAnal };
    }

}
// [=Like Girls=]
function youLikeGirlsNotSickLizardDudes(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You tell him that you prefer females.... Arian looks at you expectantly.  \"<i>So... if I was a girl... then you wouldn't mind?</i>\"");
...
function arianStoryDialogue1(player: Character): NextScreenChoices {
    arianHealth(1);
    CView.clear();
    CView.text("You feel like you'd like to know a bit more about Arian, so you ask if he would mind sharing some of [Arian eir] history with you.  After all, as a survivor from at least the early days of the demon war, and a wizard to boot, he's got to have some stories up [Arian eir] voluminous sleeves.");
    ...
    // PC has met Marae:
    if (player.effects.has(StatusEffectType.MetMarae)) CView.text("Marae herself told you they showed up about, what, 20-30 years ago?");
    else CView.text("you'd guess a long while ago given the general mess they seem to have made of the world.");
    ...
    CView.text("\n\nYou smile, and tell him that it's all right and he needs [Arian eir] sleep, so he had best get into bed. Arian nods and tucks himself in.  \"<i>Thank you, [name].</i>\"  You bid him goodnight and gently close [Arian eir] door behind you.  On your way out you let Boon and Laika know Arian is sleeping, then you make your way back to ");
    // ArianSDialogue++;
    ArianFlags.ARIAN_S_DIALOGUE++;
    // player returns to
    return { next: returnToCampUseOneHour };
}
...
// Can sex Arian.
function arianStoryDialogue2(player: Character): NextScreenChoices {
    CView.clear();
    arianHealth(1);
    CView.text("You look Arian over, remarking that he seems to be getting better after all.");
...
// =Drop It=
function arianStory2DropIt(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Though you do feel a little curious, you decide to stop making him uncomfortable, and tell him that it's okay, you'll let him get some sleep now.");
...
// =Pry=
function arianStoryPry(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Oh, no, you're not letting him wriggle out of this that easily.  You playfully tap [Arian eir] nose and tell him he should come clean and confess");
    if (player.stats.cor < 40) CView.text("; he'll sleep better with the burden off [Arian eir] conscience");
    CView.text(".");
    CView.text("\n\nArian closes [Arian eir] eyes and admits.  \"<i>It's someone close!</i>\"  He blurts out, hiding himself under the covers.");
    CView.text("\n\nWell, now, that's intriguing... still, no matter how you try, he won't say anything more and he won't come out.  It's quite clear what's going on so all you can do is sigh, do your best to pat [Arian eir] head through the covers, and tell him you'll come back another day and that you're sorry for being so nosey.  You then turn and walk out the door, heading down the stairs and back to ");
    return { next: returnToCampUseOneHour };
}

// ((if ArianHealth >= 30) && (ArianSDialogue == 2))
// Will Teach Magic
function arianDialogue3(player: Character): NextScreenChoices {
    CView.clear();
    arianHealth(1);
    CView.text("Before you can say anything, Arian asks you, \"<i>[name], I've been wondering....  Do you have any interest in magic?  You've done so much for me; I believe I should return the favor somehow.</i>\"");
    // ArianSDialogue++;
    ArianFlags.ARIAN_S_DIALOGUE++;
    // [Yes] [No]
...
function yesArianShouldMagicTeach(player: Character): NextScreenChoices {
    ...
    return { next: arianMagicLessons };
}

// =No=
function noArianShouldntMagicTeach(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You think it over for a moment, and then tell Arian that while you are flattered by the offer and willing to consider it, you can't say that you want to study magic right this moment.  You'd like to discuss it at some other time, please.");
...
function arianImbue(player: Character): NextScreenChoices {
    CView.clear();
    arianHealth(1);
    CView.text("Before you can say anything, Arian gasps, \"<i>Oh, [name].  I have a surprise for you.</i>\"  Arian says with a smile.");
    ...
    player.inventory.keyItems.add("Arian's Talisman", 0, 0, 0, 0);
    // ArianSDialogue++;
    ArianFlags.ARIAN_S_DIALOGUE++;
    // [Yes] [No]
    ...
    return {
        choices: [
            ["Yes", yesPlotSexArian],
            ["No", noPlotSexNauArian],
        ]
    };
}

// =Yes=
function yesPlotSexArian(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You approach the awkwardly amorous lizan and place your arms around [Arian eir] neck.  Leaning in close, you whisper into [Arian eir] ear that [Arian ey] only had to ask.");
    // (Display Sex Menu)
    return arianSexMenu(player, false);
}

// =No=
function noPlotSexNauArian(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You apologize to the lizan, telling [Arian em] that you aren't in the mood right now....");
...
function arianPlot4(player: Character): NextScreenChoices {
    CView.clear();
    arianHealth(1);
    CView.text("Before you can say anything, Arian says, \"<i>Oh, I have good news, [name]!</i>\"");
...
function arianPlot5(player: Character): NextScreenChoices {
    CView.clear();
    arianHealth(1);
    CView.text("Before you can say anything, Arian stops you.  \"<i>I've been meaning to ask you something, [name].  I've been feeling a lot better lately; in fact, I may be even better than I was before.</i>\"  Arian blushes.");
    ...
    if (companionsCount() > 1) CView.text("  There are other people living with you already, so what's one more?  Right?");
    CView.text("</i>\"");
    ...
    return {
        choices: [
            ["Accept", acceptArianMovingIntoCamp],
            ["Deny", denyAriansMoveIn],
        ]
    };
}

// [=Accept=]
function acceptArianMovingIntoCamp(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You tell Arian you'd be delighted to have [Arian em] move in with you.  Arian's face lights up like a kid's who's been given a bucket of candy.  \"<i>Really!?  Great!  I'll pack my stuff and we can go right away!</i>\"");

    // (Skip to ‘Invite to Camp')

    return { next: inviteArianToCamp };
}

// [=Deny=]
function denyAriansMoveIn(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You tell Arian you'd like some time to think about it.  Arian looks disappointed at first, but smiles at you all the same.  \"<i>I understand... no pressure....  So, what are we going to do today?</i>\"");
...
// Talk
function talkToArianChoices(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You tell Arian you'd like to talk to [Arian em].  Arian smiles at the prospect of chatting with you.  \"<i>I love talking with you; so what do you want to talk about?</i>\"");

    const choices: ScreenChoice[] = [];
    if (ArianFlags.ARIAN_VIRGIN > 0) choices[0] = ["Sexy Talk", arianSexingTalk];
    if (ArianFlags.ARIAN_S_DIALOGUE >= 3) choices[1] = ["Teach Magic", arianMagicLessons];
    if (!arianFollower() && ArianFlags.ARIAN_S_DIALOGUE >= 6) choices[4] = ["Invite2Camp", inviteArianToCamp];
    if (ArianFlags.ARIAN_VIRGIN === 0 && ArianFlags.ARIAN_S_DIALOGUE < 3) CView.text("\n\n<b>Arian doesn't have much to talk about right now.  Maybe you ought to just visit him from time to time or find him an item that would help combat [Arian eir] sickness.</b>");
    choices[9] = ["Back", arianHomeMenu];
...
function arianMagicLessons(player: Character): NextScreenChoices {
    CView.clear();
    arianHealth(1);
    CView.text("You ask Arian if [Arian ey] wouldn't mind giving you some magic lessons.");

    // (if ArianMLesson >= 4)
    if (ArianFlags.ARIAN_LESSONS >= 4) {
    ...
    if (player.stats.int < 25) {
        ...
        if (!arianFollower()) CView.text("make your way back to your ");
        else CView.text("return to your duties.");
    }
    // (else if PC int < 50 //Teach Charge Weapon if PC doesn't know.
        ...
        if (!arianFollower()) CView.text(", making your way back to ");
        else CView.text(", exiting [Arian eir] tent and going about your business.");

        // (if PC doesn't know Charge Weapon)
        if (!player.effects.has(StatusEffectType.KnowsCharge)) {
            CView.text("\n\nAs you ");
            if (!player.body.legs.isNaga()) CView.text("walk");
            else CView.text("slither");
            CView.text(" away, you start pondering Arian's lesson of the day.  You wonder if you could perhaps apply the channeling principle to strengthen your [weapon] in combat.");
            CView.text("\n\nThat would be very helpful; you'll have to try it sometime.");
            player.effects.add(StatusEffectType.KnowsCharge, 0, 0, 0, 0);
        }
    }
    else if (player.stats.int < 75) {
        CView.text("<b>Game Note: This isn't canon and will be re-written soonish.</b>");
        ...
        if (!player.effects.has(StatusEffectType.KnowsBlind)) {
            ...
            player.effects.add(StatusEffectType.KnowsBlind, 0, 0, 0, 0);
            // Return to
        }
        else {
        ...
        if (!arianFollower()) CView.text("begin the trek home.");
        else CView.text("exit [Arian eir] tent.");
        // (if PC doesn't know Whitefire)
        if (!player.effects.has(StatusEffectType.KnowsWhitefire)) {
            ...
            player.effects.add(StatusEffectType.KnowsWhitefire, 0, 0, 0, 0);
        }
    }
    ArianFlags.ARIAN_LESSONS++;
    player.stats.int += 1;

    if (player.stats.int < 75) player.stats.int += 1;
...
function arianSexingTalk(player: Character): NextScreenChoices {
    CView.clear();
    arianHealth(1);
    CView.text("You smirk knowingly at [Arian em] and ask how [Arian ey] feels about sex now that [Arian ey]'s had [Arian eir] first time?");
    ...
    if (ArianFlags.ARIAN_COCK_SIZE !== 0) {
        ...
        if (ArianFlags.ARIAN_DOUBLE_COCK === 0) {
        ...
        // Regular Cock.
        if (ArianFlags.ARIAN_COCK_SIZE === 1) {
        ...
        // Big Cock.
        else if (ArianFlags.ARIAN_COCK_SIZE === 2) {
        ...
        // Huge Cock.
        else if (ArianFlags.ARIAN_COCK_SIZE === 3) {
    ...
    if (ArianFlags.ARIAN_ANAL_XP < 33) {
        // No preference... yet.
    ...
    else if (ArianFlags.ARIAN_ANAL_XP < 66) {
    ...
    // (else if (ArianAnalXP <= 100))
    else if (ArianFlags.ARIAN_ANAL_XP <= 100) {
        ...
        if (player.body.cocks.length <= 0) {
            ...
            return {
                choices: [
                    ["Yes", yesYouButtslutIllFuckYou],
                    ["No", goddamnitNoYouButtSlut],
                ]
            };
        }
    }
    return sexTalkFinish(player, false);
}

// [=Yes=]
function yesYouButtslutIllFuckYou(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("How could you refuse such a request?  You tell [Arian em] to strip and get ready.");
...
// [=No=]
function goddamnitNoYouButtSlut(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You apologize, but you really can't do that right now.  Arian looks a bit disappointed, but smiles at you all the same.  \"<i>Oh... okay.  Next time then?</i>\"");
    CView.text("\n\nYou nod.");
    return sexTalkFinish(player, false);
}

function sexTalkFinish(player: Character, newl: boolean): NextScreenChoices {
    if (newl) CView.clear();
    else CView.text("\n\n");
    CView.text("Satisfied with your little chat, you pat the lizan's head and excuse yourself, heading back to ");
    return { next: returnToCampUseOneHour };
}

// Invite to Camp:
// Only available if ArianHealth == 100.
function inviteArianToCamp(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You ask the lizan if [Arian ey] still really wants to leave [Arian eir] comfortable home in the city and come out to live with you in your little camp in the wastelands?");
    ...
    if (ArianFlags.ARIAN_COCK_SIZE > 0) {
        if (ArianFlags.ARIAN_VAGINA === 0) CView.text("man");
        else CView.text("herm");
    }
    else CView.text("woman");
    CView.text(".");
    ...
    return { next: takeYerLizardHomePartII };
}

function takeYerLizardHomePartII(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Upon arriving at the camp, the first thing Arian notices is the shimmering portal.  \"<i>Is this... where you came from?</i>\" Arian asks.");
    ...
    if (followersCount() > 1) CView.text(" and your other companions");
    CView.text(".");
    ...
    ArianFlags.ARIAN_FOLLOWER = 1;
    return { next: returnToCampUseOneHour };
}

// Sex
// ArianHealth must be at least 20 before you can even pick Sex as an option.
function arianSexMenu(player: Character, output: boolean): NextScreenChoices {
    if (output) {
        CView.clear();
        CView.text("You ask Arian if [Arian ey] feels strong enough to do a little lovemaking.");
        // (if ArianHealth < 30)
        if (ArianFlags.ARIAN_HEALTH < 30) {
        ...
        // (if ArianHealth < 100)
        else if (ArianFlags.ARIAN_HEALTH < 100) {
    ...
    if (player.body.cocks.length > 0) {
        ...
        if (player.body.cocks.find(Cock.CockThatFits(50))) choices[0] = ["Anal - Pitch", giveArianAnal];
        // Get Blown:
        // PC must have a cock.
        choices[2] = ["Get Blown", getBlownByArian];
        // Penetrate:
        // Arian must be herm/female.
        // PC must have a cock that fits (cock area 50 or less)
        if (player.body.cocks.find(Cock.CockThatFits(ArianFlags.ARIAN_CAPACITY)) && ArianFlags.ARIAN_VAGINA > 0)
            choices[4] = ["Fuck Vagina", penetrateArian];
        // Double Pen Arian:
        ...
        if (player.body.cocks.find(Cock.CockThatFits(50)) && player.body.cocks.filter(Cock.CocksThatFit(50))[1] && ArianFlags.ARIAN_VAGINA > 0) choices[8] = ["Double Pen", doublePenetrateArian];
        // Docking
        // ArianCockSize needs to be below 3. (ArianDblCock does not affect this decision.)
        // PC cock area must be <= 30.
        if (player.body.cocks.sort(Cock.Smallest)[0].area <= 30 && ArianFlags.ARIAN_COCK_SIZE < 3 && ArianFlags.ARIAN_COCK_SIZE > 0) choices[7] = ["Docking", arianDocking];
    }
    // Get Anal:
    // Arian must have a cock.
    if (ArianFlags.ARIAN_COCK_SIZE > 0) choices[1] = ["Anal - Catch", getButtWreckedByArian];
    // Blow:
    // Arian must have a cock.
    if (ArianFlags.ARIAN_COCK_SIZE > 0) choices[3] = ["Blow " + arianMF("Him", "Her"), suckAriansDick];

    // PC must have a vagina.
    if (player.body.vaginas.length > 0) {
        // Get Penetrated:
        // Arian must have a cock.
        if (ArianFlags.ARIAN_COCK_SIZE > 0) choices[5] = ["Get Fucked", getPenetratedByArianAndHisHitlerMustache];
        if (ArianFlags.ARIAN_VAGINA > 0 && (player.inventory.keyItems.has("Dildo") || player.inventory.keyItems.has("Deluxe Dildo"))) {
            choices[6] = ["Dildo Fun", arianDildoFun];
        }
    }
    choices[9] = ["Back", arianHomeMenu];
...
// Modified by AnalXP.
// PC must have a cock that fits (cock area 50 or less)
function giveArianAnal(player: Character): NextScreenChoices {
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(50));
    CView.clear();
    arianHealth(3);
    ArianFlags.ARIAN_ANAL_XP += 10;
    if (ArianFlags.ARIAN_ANAL_XP >= 100) ArianFlags.ARIAN_ANAL_XP = 100;
    ArianFlags.ARIAN_VIRGIN += 1;
    if (ArianFlags.ARIAN_VAGINA > 0)
        CView.image("arianfemale-home-giveArianAnal");
    else
        CView.image("arianmale-home-giveArianAnal");
    ...
    if (!cockThatFits) {
        cockThatFits = player.body.cocks.sort(Cock.Smallest)[0];
    }

    CView.text("You tell Arian that, if [Arian ey]'s willing, you'd like to take [Arian em] from behind.");
    // AnalXP < 33
    ...
    if (ArianFlags.ARIAN_ANAL_XP < 33) {
        ...
        if (ArianFlags.ARIAN_VAGINA < 1) {
            ...
            if (ArianFlags.ARIAN_COCK_SIZE === 0) CView.text("girl");
            else CView.text("herm");
            CView.text(" isn't going to make it any smaller either.</i>\"  Arian fidgets and shudders as you stroke [Arian eir] behind.");
        }
        ...
        if (player.body.balls.count === 0) CView.text("the base of your cock");
        else CView.text("the bottom of your balls");
        CView.text(" and you warn Arian that you're going to cum inside [Arian em] if you keep going.  Arian drools in pained pleasure, for a moment you wonder if [Arian ey] even heard you.  \"<i>Cum.  Cum inside me.  I want it... all of it! Ah!</i>\"");
        ...
        if (ArianFlags.ARIAN_COCK_SIZE > 0) CView.text("spewing forth large gobs of cum of [Arian eir] own");
        if (ArianFlags.ARIAN_COCK_SIZE > 0 && ArianFlags.ARIAN_VAGINA > 0) CView.text(" and ");
        if (ArianFlags.ARIAN_VAGINA > 0) CView.text("wetting both your and her legs with a healthy serving of lizan pussy juice");
        CView.text(".  You shudder and gasp until, utterly spent, you can't resist sinking down atop Arian and bearing [Arian em] into the comforting embrace of the bed.  You lay there, recovering your strength from your most delightful exertions, wriggling to give the lizan a proper snuggling while you lay there.");
    ...
    // Loose, sensitive enough to make Arian scream in pleasure. (It's like a tight vagina, feels like one too, minor pain)
        ...
        if (ArianFlags.ARIAN_VAGINA < 1) {
            ...
            if (ArianFlags.ARIAN_BREASTS === 1) CView.text(" perky");
            else if (ArianFlags.ARIAN_BREASTS === 2) CView.text(" rounded");
        ...
        if (ArianFlags.ARIAN_COCK_SIZE > 0) {
            CView.text(" a");
            if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text(" pair of dripping cocks");
            else CView.text(" dripping cock");
            if (ArianFlags.ARIAN_VAGINA > 0) CView.text(" and");
        }
        if (ArianFlags.ARIAN_VAGINA > 0) CView.text(" a sopping wet pussy");
        CView.text(" greet");
        if ((ArianFlags.ARIAN_VAGINA === 0 && ArianFlags.ARIAN_DOUBLE_COCK === 0) || (ArianFlags.ARIAN_VAGINA === 1 && ArianFlags.ARIAN_COCK_SIZE === 0)) CView.text("s");
        CView.text(" your touch.  What a little slut Arian is turning into; [Arian ey]'s already raring to go, and even knowing it's going to be [Arian eir] ass that's getting fucked, too!");
        ...
        if (ArianFlags.ARIAN_VAGINA < 1) CView.text("boypussy");
        else CView.text("nether-pussy");
        CView.text(".  \"<i>Yesssss.... Oh, sometimes it hurts a bit, but the feeling, the pleasure, the pain.... It's heavenly.</i>\"");
        ...
        CView.text("\n\nArian's ass tightens around your " + describeCock(player, cockThatFits) + " as [Arian eir] ");
        if (ArianFlags.ARIAN_COCK_SIZE > 0) {
            if (ArianFlags.ARIAN_DOUBLE_COCK === 0) CView.text("cock spews its load");
            else CView.text("twin cocks spew their loads");
            CView.text(" on the bedsheets");
            if (ArianFlags.ARIAN_VAGINA > 0) CView.text(" and [Arian eir] ");
        }
        if (ArianFlags.ARIAN_VAGINA > 0) CView.text(" contracting pussy paints your lower body with lizan femcum");
        CView.text(".  Arian is only capable of moaning and shuddering as [Arian eir] powerful orgasm rocks the poor lizan to [Arian eir] core.  The extra tightness of [Arian eir] contracting butthole increases the friction on your " + describeCock(player, cockThatFits) + ", pushing you ever closer to the climax.");
        ...
        if (player.body.balls.count === 0) CView.text("base of your spine");
        else CView.text("depths of your balls");
        CView.text(".  Arian, completely blissed out, lays limply on [Arian eir] bed, [Arian eir] butt held up by your gripping hands.  With a final deep thrust you finally go over the edge.");
        ...
        // (Medium Cum Amount)
        else if (player.cumQ() < 1000) CView.text("\n\nArian's butt feels so good that you can't help but paint it in white.  Gob after gob of searing hot cum flows from your " + describeCockHead(cockThatFits) + " and into the lizan's willing bowels.  You can't help but continue thrusting inside [Arian eir] deliciously slick insides, even as Arian's distended sphincter tries its best to hold you in place.  By the time you're done, Arian looks like [Arian ey]'s three months pregnant; after a few final thrusts, you're confident you've given [Arian em] all that you can muster for the moment and pull out of the lizan's hungry ass.");
        // (High Cum Amount)
        else {
            CView.text("\n\nYou bury yourself as deep as you can into the lizan's behind and brace yourself, holding onto [Arian eir] hips as the first of many jets of cum finally escapes your throbbing " + describeCock(player, cockThatFits) + ".  You can feel the groaning lizan shudder with each blast that you pump into [Arian eir] inviting interior; each of your sticky ropes of cum filling [Arian em] up until [Arian eir] belly looks as big as a beach ball.  Arian's contracted sphincter tries its best to hold your prodigious load in, but it can't hope to contain it all; soon white jism explodes from around the seal of your cock.  Trails of your pleasure run down the lizan's legs to soak the bedsheets along with the lizan's own fluids.  One final jet pushes Arian off your shaft, and you gaze at the messy results of your recent activities.");
        }
        ...
        if (!arianFollower()) CView.text("room");
        else CView.text("tent");
        CView.text(".");
    }
    // AnalXP <= 100
    // Very Loose, sensitive enough to make Arian cum just from insertion. (Feels better than anything else. Yep, Arian really becomes a buttslut at this point)
        ...
        if (ArianFlags.ARIAN_VAGINA > 0) CView.text("wet gash is positively leaking lizan juices");
        if (ArianFlags.ARIAN_VAGINA > 0 && ArianFlags.ARIAN_COCK_SIZE > 0) CView.text(" and [Arian eir] ");
        if (ArianFlags.ARIAN_COCK_SIZE > 0) CView.text("erect cock is already leaking pre like a sieve");
        CView.text(".  You can hardly believe how far Arian has come; you ask [Arian em] if it really feels that good?  \"<i>More than you can... ah... imagine, now fill me up!</i>\" [Arian ey] demands with a sense of urgency.");
        ...
        if (ArianFlags.ARIAN_VAGINA > 0) CView.text("her pussy clenches, spilling a flood of juices; ");
        if (ArianFlags.ARIAN_VAGINA > 0 && ArianFlags.ARIAN_COCK_SIZE > 0) CView.text(" and ");
        if (ArianFlags.ARIAN_COCK_SIZE > 0) CView.text("his cock throbs and shoots rope after rope of cum onto the bedsheets");
        CView.text(". Overwhelmed by your sudden intrusion, Arian collapses forward, burying [Arian eir] face on [Arian eir] pillow and dragging you on top of [Arian em].  \"<i>Ah... [name], you feel so good.  It's amazing.  I never thought buttsex could ever feel this good.</i>\"");

        CView.text("\n\nYou almost blink in surprise; you were just throwing the term buttslut around as a joke, you didn't think Arian was really like that.  Still, there are far more important matters - like digging your way into the depths of Arian's greedy little ass with your " + describeCock(player, cockThatFits) + "!  You squeeze the lizan's scaly butt and begin to rut [Arian em] like an animal, thrusting your way in and out of [Arian eir] back passage with all the eagerness you can muster.");
        ...
        if (ArianFlags.ARIAN_COCK_SIZE > 0) {
            if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("cocks are still rock hard, and surprisingly, still leaking ropes of cum");
            else CView.text("cock is still rock hard, and surprisingly, still leaking ropes of cum");
            if (ArianFlags.ARIAN_VAGINA > 0) CView.text(", and her");
        }
        if (ArianFlags.ARIAN_VAGINA > 0) CView.text(" pussy squirting juices against your [legs] with each shuddering impact of your [hips]");
        CView.text(".  \"<i>Ah!  Yes!  More, give me more!  Harder!  Faster!</i>\" Arian pleads, before biting on [Arian eir] pillow in pleasure.");
        ...
        if (ArianFlags.ARIAN_VAGINA > 0) {
            CView.text("  Her cunt grips the air and blasts one last jet of wetness at your crotch.");
        }
        if (ArianFlags.ARIAN_COCK_SIZE > 0) CView.text("  [Arian Eir] throbbing cock flexes and shoots one last rope of lizan-cum against your belly.");
        CView.text("  [Arian Eir] ass grips you forcefully, forming a vacuum and finally loosens.  \"<i>That... that wash da best,</i>\"  Arian slurs, before finally collapsing on [Arian eir] bed.");
        ...
        // (ArianAssChat == 0)
        if (ArianFlags.ARIAN_ASS_CHAT === 0) {
            ...
            if (9999 === 9999) CView.text(" as hypocritical as that may be");
            CView.text("... besides, you think [Arian ey]'s kind of sexy when [Arian ey] gets like that.  At that Arian perks up.  \"<i>Really?</i>\"  Yes, really, you reply.  Arian smiles happily at you.  \"<i>So... do you want to go again?</i>\"");
            ...
            if (!arianFollower()) CView.text("back to Tel'Adre's streets, and from there back to ");
            else CView.text("out of [Arian eir] tent.");
            ArianFlags.ARIAN_ASS_CHAT++;
        }
        else {
...
// Get Blown:
// PC must have a cock.
function getBlownByArian(player: Character): NextScreenChoices {
    CView.clear();
    ArianFlags.ARIAN_VIRGIN += 1;
    arianHealth(3);
    if (ArianFlags.ARIAN_VAGINA > 0)
        CView.image("arianfemale-home-getbj");
    else
        CView.image("arianmale-home-getbj");
    CView.text("You trail your hand down your belly, pondering what to do.  Arian doesn't seem to notice, instead staring with anticipation at your erection.  You idly swing your hips from side to side, and notice with amusement that the lizan seems to follow it.  Building on that train of thought, you ask if [Arian ey] would be willing to suck you off.");

    // (if ArianHasBlown == 0)
    if (ArianFlags.ARIAN_HAS_BLOWN === 0) {
        CView.text("\n\nThe lizan averts [Arian eir] eyes, snapping out of [Arian eir] reverie.");
        if (ArianFlags.ARIAN_COCK_SIZE === 3) CView.text("  You notice Arian's exposed shaft slowly hardening at your invitation.");
        CView.text("  \"<i>I... Can I really?</i>\"");
    ...
    if (player.cumQ() < 250) CView.text("small");
    else if (player.cumQ() < 1500) CView.text("big");
    else CView.text("huge");
    CView.text(" pot belly.  [Arian Ey] lets go of your dick with a sigh of pleasure and lays back on the bed.  \"<i>Ah... I'm full.</i>\"");

    CView.text("\n\nYou tell [Arian em] that [Arian ey] looks full");
    if (player.cumQ() >= 1500) CView.text(", in fact, you're amazed [Arian ey] didn't pop with how much [Arian ey] drank");
    CView.text("; is [Arian ey] sure [Arian ey]'s all right?");
    ...
    if (player.cumQ() >= 1000) CView.text(" which, considering the fact [Arian ey] looks like a mother ready to birth a toddler from the size of [Arian eir] belly, isn't as easy as you'd think");
    CView.text(".  Letting the lizan use your shoulder, you pull [Arian eir] sheets up and help [Arian em] back in the bed, where [Arian ey] flops down with a protest of springs");
    if (player.cumQ() >= 250) CView.text(", [Arian eir] gut audibly sloshing as the cum inside is churned by the impact");
    CView.text(".");
    ...
    if (!arianFollower()) CView.text(" back to camp, stopping only to notify Laika and Boon that Arian is sleeping.");
    else CView.text(" out of Arian's tent.");
    player.orgasm();
    ArianFlags.ARIAN_HAS_BLOWN++;
    return { next: returnToCampUseOneHour };
}
...
// Arian must be herm/female.
// PC must have a cock that fits (cock area 50 or less)
function penetrateArian(player: Character): NextScreenChoices {
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(50));
    ...
    CView.image("arianfemale-home-penetrate");
    CView.text("You admire the transgendered lizan's body, from her feminized features, down past her [arian chest], all the way to her shapely thighs.  You tell Arian that the change looks very good on her; few boys would really be so naturally pretty when turned into a ");
    if (ArianFlags.ARIAN_COCK_SIZE > 0) CView.text("dick");
    CView.text("girl.");
    ...
    CView.text(".  \"<i>Err... readier?</i>\"  She asks, smiling nervously.");
    ...
    if (ArianFlags.ARIAN_COCK_SIZE === 3) {
        CView.text(", her exposed cock");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
        CView.text(" hardening at what she's about to do");
    }
    else if (ArianFlags.ARIAN_COCK_SIZE > 0) {
        CView.text(", her bulbous cock");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s peeking out of their");
        else CView.text(" peeking out of its");
        CView.text(" hiding place, already at full mast in anticipation");
    }
    CView.text(".  Arian swallows audibly.  \"<i>What now?</i>\" she asks, already panting in lust.");
    ...
    if (ArianFlags.ARIAN_COCK_SIZE === 0) CView.text("female");
    else CView.text("herm");
    CView.text(" lizan, until you have managed to hilt yourself inside of her.");
    ...
    if (ArianFlags.ARIAN_COCK_SIZE > 0) {
        CView.text("  Her ");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("twin shafts");
        else CView.text("shaft");
        CView.text(" bobbing just out of your field of vision, leaking pre all over your belly.");
    }
    ...
    if (ArianFlags.ARIAN_COCK_SIZE > 0) {
        CView.text("  Her cock");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s twitch as they spurt ");
        else CView.text(" twitches as it spurts ");
        CView.text("a small rope of pre over her belly, anticipating what's to come as she braces herself for the inevitable pounding you're about to give her.");
    }
    ...
    if (ArianFlags.ARIAN_COCK_SIZE > 0) {
        CView.text("Arian's ");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("twin cocks throb");
        else CView.text("cock throbs");
        CView.text(" as she splashes her [arian chest] and face with her own futa-lizan seed.  ");
    }
    CView.text("Her walls grip you tightly, almost painfully, as a flood of juices hit the " + describeCockHead(cockThatFits) + " of your " + describeCock(player, cockThatFits) + " with the force of a tidal wave, only to spill around her spread nethers and run down your lower body.  Her sopping wet pussy works overtime, trying its best to pull you in as deep as possible, intent on sucking all the cum out of ");
    if (player.body.balls.count === 0) CView.text("you");
    else CView.text("your [balls]");
    CView.text(".");

    CView.text("\n\nFinally having kept your promise, you give yourself over to the inevitable. ");
    if (player.body.vaginas.length > 0) CView.text(" A wash of fluids spills down your body from your cunt as it spasms in release and y");
    else CView.text("Y");
    CView.text("ou unleash your essence right into the lizan's burning womb.");
    ...
    if (player.cumQ() < 250) CView.text("lean");
    else if (player.cumQ() < 1000) CView.text("full");
    else CView.text("overfilled");
    CView.text(" belly.  \"<i>Stay with me, like this, a little longer?</i>\" Arian asks.");
...
// Get Anal:
// Arian must have a cock.
function getButtWreckedByArian(player: Character): NextScreenChoices {
    CView.clear();
    ArianFlags.ARIAN_VIRGIN += 1;
    arianHealth(3);
    if (ArianFlags.ARIAN_VAGINA > 0)
        CView.image("arianfemale-home-getArianAnal");
    else
        CView.image("arianmale-home-getArianAnal");
    CView.text("With a lick of your lips with your tongue, you ask how Arian would like to be on the pitching end of anal?");
    // (if AnalXP < 33)
    if (ArianFlags.ARIAN_ANAL_XP < 33) CView.text("\n\n\"<i>I don't know... won't it hurt, are you sure?</i>\"");
    // (if AnalXP < 66)
    else if (ArianFlags.ARIAN_ANAL_XP < 66) CView.text("\n\n\"<i>I like it when you use my ass, but if you want me to use yours, I would gladly comply.  That is, if you're sure you want me to...?</i>\"");
    // (if AnalXP <= 100)
    ...
    if (ArianFlags.ARIAN_COCK_SIZE >= 3) {
        CView.text("exposed cock");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s harden");
        else CView.text(" hardens");
        CView.text(" to full mast.");
    }
    else {
        CView.text(arianMF("his", "her") + " cock");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s peek out of their");
        else CView.text(" peeks out of its");
        CView.text(" hiding place, rising to full mast.");
    }

    CView.text("\n\nYou sashay yourself towards the prone lizan, straddling [Arian eir] legs and reaching out to grasp [Arian eir] ");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("primary ");
    CView.text("cock.  Slowly and gently you begin to stroke its strange, knobbly surface, your fingers eliciting moans and gasps from Arian as [Arian ey] shudders under your touch.  Precum begins to ooze from the reptilian prick's head, and you purposefully rub it into [Arian eir] shaft as a kind of makeshift lubricant.");

    CView.text("\n\n\"<i>Ohm... T-this feels great, [name], b-but if you keep this up, I won't be able to hold back!</i>\" Arian pants, [Arian eir] shaft");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
    CView.text(" throbbing against your hands.");
    ...
    displayStretchButt(player, arianCockSize(), true, true, false);
    ...
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) {
        CView.text("  [Arian Eir] other shaft twiches and sprays your ");
        if (player.body.tails.length <= 0) CView.text("back");
        else CView.text("tail");
        CView.text(".");
    }
    ...
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("stroking [Arian eir] other cock and sliding it between your ass cheeks, ");
    CView.text("caressing the base of [Arian eir] tail.  With a devious grin, you begin to gently but insistently slide your finger into [Arian eir] ");
    if (ArianFlags.ARIAN_ANAL_XP > 66) CView.text("eager ");
    CView.text("ass, probing for [Arian eir] prostrate...");

    CView.text("\n\nThe reaction is almost instantaneous; Arian moans into your kiss and you feel [Arian eir] ");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("pair of lizan-cocks practically jump");
    else CView.text("lizan-cock practically jump");
    CView.text(" back to full mast, even surprising you with Arian's eagerness.");
    ...
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("sliding [Arian eir] neglected prick through your ass-cheeks, ");
    CView.text("clenching your inner walls to grip and feel the excitingly unusual lumpy, bulbous texture of [Arian eir] shaft inside you.  You moan and gasp, telling [Arian em] that [Arian ey] feels so unique, and yet [Arian ey] stimulates you so wonderfully.");
    ...
    if (player.body.vaginas.length > 0) CView.text("cunt dripping with arousal");
    if (player.body.cocks.length > 0 && player.body.vaginas.length > 0) CView.text(" and ");
    if (player.body.cocks.length > 0) {
        CView.text("cock");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(" drooling precum onto [Arian eir] belly");
    }
    CView.text(".  You're so close now, you can feel it.  You tell Arian you're going to cum, and beg [Arian em] to cum with you.");

    CView.text("\n\nAs if on cue, Arian gasps and [Arian eir] shaft");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s twitch");
    else CView.text(" twitches");
    CView.text(", signaling [Arian eir] oncoming orgasm.  [Arian Ey] grabs the sheets on [Arian eir] bed and curls [Arian eir] toes.  \"<i>...[name]!</i>\"  Jet after jet of lizan spunk paints your bowels white");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text(", as well as your butt");
    CView.text(", as Arian climaxes");
    if (ArianFlags.ARIAN_VAGINA > 0) CView.text(", her own pussy clenching and drizzling juices on the matted sheets beneath the two of you.");
    else CView.text(".");
    ...
    if (player.body.cocks.length > 0) CView.text("  [EachCock] spasms and shoots rope after rope of cum, painting Arian's [arian chest] white, as well as the groaning lizan's face.");
    if (player.body.vaginas.length > 0) CView.text("  Your [pussy], though unattended, joins in the perverted display, leaking copious amount of fluids on top of the prone lizan.");
    ...
    // (if AnalXP <= 100)
        ...
        if (player.body.cocks.length > 0) {
            ...
            const choices: ScreenChoice[] = [];
            if (player.body.cocks.find(Cock.CockThatFits(50)) && player.body.cocks.length > 0) choices[0] = ["Yes", giveArianAnal];
            else CView.text(".  You're too big to fit inside Arian's ass, though.");
            choices[1] = ["No", returnToCampUseOneHour];
...
// Blow:
// Arian must have a cock.
function suckAriansDick(player: Character): NextScreenChoices {
    CView.clear();
    ArianFlags.ARIAN_VIRGIN += 1;
    arianHealth(3);
    if (ArianFlags.ARIAN_VAGINA > 0)
        CView.image("arianfemale-home-suckariandick");
    else
        CView.image("arianmale-home-suckariandick");
    CView.text("You make a show of lewdly licking your lips and ask Arian if [Arian ey]'d be willing to let you have a little taste of lizan essence...?");
    ...
    if (ArianFlags.ARIAN_COCK_SIZE > 3) {
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("exposed cocks hardening to full mast");
        else CView.text("exposed cock hardening to full mast");
    }
    else {
        CView.text("cock");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s emerging from their");
        else CView.text(" emerging from its");
        CView.text(" hiding place, already fully erect");
        CView.text(".");
    }
    ...
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("uppermost ");
    CView.text("cock, marveling at the strange textures of its bumpy, knobbly, bulbous surface.  It feels so strange to you, and you continue to stroke it eagerly.");
    ...
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("twin ");
    CView.text("shaft");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s tremble");
    else CView.text(" trembles");
    CView.text(" and throb");
    if (ArianFlags.ARIAN_DOUBLE_COCK === 0) CView.text("s");
    CView.text(" against your hand; pre quickly forming on the tip");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
    CView.text(" only to slowly slide down the bulbous surface of Arian's lizan prick");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
    CView.text(".  You continue to slide your hand along the increasingly slick surface, playfully asking if your hand really feels that good to [Arian em]?");
    ...
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
    CView.text(" a long, loving, slurpy lick.  The taste is surprising; a sort of sugary-tinted spice, not the usual salt of another creature.  It's actually quite pleasant, and you find yourself running your tongue up and down [Arian eir] prick as if it were some kind of candied stick, eagerly sucking and slobbering as you coax more of [Arian eir] yummy goo from its strangely-shaped tip.");
    ...
    CView.text(" one more time and erupts into your waiting mouth");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text(" and chin");
    CView.text(".  [Arian Eir] legs close around your back, effectively keeping you from moving too far away as [Arian ey] finishes giving you all of [Arian eir] load.");
    ...
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text(", oblivious to the second cock spewing cum all over your [chest] in your eagerness");
    CView.text(".  After [Arian ey] finally finishes, you continue to lick and suckle for a few minutes longer, making sure you've lapped up every last drop.  Then and then alone do you lift your head, flirtatiously licking your lips to tease the now-spent lizan.");
    ...
    CView.text(" rolling your neck in pleasure as [Arian ey] rubs it with [Arian eir] reptilian tail.");
    ...
    if (ArianFlags.ARIAN_VAGINA > 0) CView.text(" and tickle her cunt with a finger");
    CView.text(" for emphasis, kiss [Arian em] gently on the tip of [Arian eir] nose, and then get your clothes together, planning on getting dressed and heading back out again.");

    CView.text("\n\n\"<i>Uuuh... see you soon?</i>\"");
    CView.text("\n\nYou throw [Arian em] a smirk over your shoulder, shake your [ass] for [Arian eir] benefit, and head on out.");

    player.stats.lust += 10 + player.stats.lib / 5;
...
// PC must have a vagina.
// Arian must have a cock.
function getPenetratedByArianAndHisHitlerMustache(player: Character): NextScreenChoices {
    CView.clear();
    ArianFlags.ARIAN_VIRGIN += 1;
    arianHealth(3);
    if (ArianFlags.ARIAN_VAGINA > 0)
        CView.image("arianfemale-home-getpenetrated");
    else
        CView.image("arianmale-home-getpenetrated");
    CView.text("You look at Arian's ");
    if (ArianFlags.ARIAN_COCK_SIZE < 3) CView.text("slit, and think of what it's hiding in there");
    else {
        CView.text("exposed cock");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
    }
    CView.text("; then you turn to look at Arian and ask [Arian em] how would [Arian ey] feel about giving [Arian eir] ");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("twin cocks");
    else CView.text("cock");
    CView.text(" a bit of a workout?");
    ...
    if (player.body.legs.isBiped()) CView.text("spread your [legs] and ");
    CView.text("expose your [vagina] to [Arian eir] viewing pleasure.  The reaction is almost instantaneous; Arian's ");
    if (ArianFlags.ARIAN_COCK_SIZE >= 3) CView.text("exposed");
    else CView.text("hidden");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text(" twin shafts immediately fill");
    else CView.text(" shaft immediately fills");
    CView.text(" with blood, growing fully erect in ");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("their");
    else CView.text("its");
    CView.text(" bulbous, throbbing glory.");
    ...
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
    CView.text(" and get ready to thrust in; otherwise neither of you are going to be feeling good any time soon.");

    CView.text("\n\nSnapping to [Arian eir] senses, Arian quickly ");
    CView.text("kneels between your [legs]");
    CView.text(", aligning the tip of ");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("each of ");
    CView.text(arianMF("his", "her") + " ");
    CView.text("shaft");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
    CView.text(" with your [vagina]");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text(" and [asshole]");
    CView.text(".  [Arian Ey] looks at you, waiting for you to confirm that [Arian ey] should indeed get going.  You smile and nod");
    if (player.hasLongTail()) CView.text(", looping your tail around [Arian eir] waist");
    CView.text(".");
    ...
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) {
        ...
        displayStretchVagina(player, arianCockSize(), true, true, false);
        displayStretchButt(player, arianCockSize(), true, true, false);
        ...
        if (player.hasLongTail()) CView.text(", further encouraging [Arian em] to do so, by looping your tail around [Arian eir] waist");
        CView.text(".  The lizan doesn't need much encouragement to comply, easily snuggling against you, breast to breast, and sighing softly.  \"<i>I'm sorry.  Sometimes I wonder why you bother doing something like this, with someone like me,</i>\" [Arian ey] says, a hint of sadness in [Arian eir] voice.");
    ...
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("twin bulbous shafts feel");
    else CView.text("bulbous shaft feels");
    CView.text(" exquisite, and ");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("their");
    else CView.text("its");
    CView.text(" bumps massage your entrance");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
    CView.text(" unlike anything else.  You praise the lizan on being a quick study, letting go of [Arian eir] tail and grabbing onto [Arian eir] scaly shoulders instead, giving [Arian em] a few more directions so [Arian ey] can catch your most sensitive spots.");
    ...
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("and [ass] ");
    CView.text("with [Arian eir] sticky wet seed.");
    ...
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
    CView.text(" swelling inside you, the lizan grabbing your shoulders and starting to thrust [Arian em]self back into you once more.");
    ...
    if (ArianFlags.ARIAN_VAGINA === 0) CView.text("[Arian ey] himself ");
    else CView.text("she ");
    CView.text("is being overwhelmed by, spraying your ");
    if (ArianFlags.ARIAN_DOUBLE_COCK === 0) CView.text("cavity");
    else CView.text("cavities");
    CView.text(" with a second helping of lizan spunk, dredging up every last drop of jizz left in [Arian eir] internal balls before, with a groan, [Arian ey] sinks down atop you.  Arian's orgasm pushes you over the edge, and you find your pussy ");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("and ass ");
    CView.text("contracting, trying ");
    if (ArianFlags.ARIAN_DOUBLE_COCK === 0) CView.text("its");
    else CView.text("their");
    CView.text(" best to milk the poor lizan of all [Arian ey] is worth, until finally with one last spasm, you slump down and release the lizan shaft");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
    CView.text(" deeply embedded inside you, some of your mixed juices running down to mat Arian's bed.");
...
// PC must have at least 2 cocks that fit. That means two cocks with a cock area of <= 50.
// This isn't meant to give AnalXP, but given the fact that Arian's ass will get pen'd it would also be justified. Up to you Fen!
function doublePenetrateArian(player: Character): NextScreenChoices {
    CView.clear();
    ArianFlags.ARIAN_VIRGIN += 1;
    arianHealth(3);
    CView.image("arianfemale-home-doublepenetrate");
    const cockThatFits = player.body.cocks.find(Cock.CockThatFits(ArianFlags.ARIAN_CAPACITY));
    const secondCockThatFits = player.body.cocks.filter(Cock.CocksThatFit(ArianFlags.ARIAN_CAPACITY))[1];
    CView.text("You look over your feminine lizard lover, and feel your " + describeCocksLight(player) + " starting to stir in your [armor].  Since you have enough tools for the job, and Arian has enough holes, you ask if Arian would be willing to let you fuck her in both her ass and pussy at the same time?");
    ...
    if (ArianFlags.ARIAN_ANAL_XP <= 33) CView.text("tight");
    else if (ArianFlags.ARIAN_ANAL_XP <= 66) CView.text("loose");
    else CView.text("throbbing");
    CView.text(" ass and dripping wet vagina; a pair of clawed hands reach behind to spread her butt open for you.  \"<i>Okay, I'm ready.</i>\"  Arian says, looking behind at you.");
    ...
    CView.text("\n\n\"<i>What do you think I'm trying to do!?  Now get in here and paint my womb white!</i>\" Arian snaps, bracing herself on her bed and allowing her ass and pussy both to suck you in with surprising force.  As soon as you're hilted within both holes, her ass clamps shut on your " + describeCock(player, secondCockThatFits) + ", while her pussy's contractions begin truly milking you for all you're worth.  Stuck as you are, you have no option but to sit back and enjoy her contractions as you feel yourself nearing the edge of an inevitable orgasm.");
    ...
    // (Low Cum Amount)
    if (player.cumQ() <= 250) {
        CView.text("\n\nYour " + describeCocksLight(player) + " explode inside Arian's eager holes, giving them the liquid warmth they so crave.  The massage that your two cocks are receiving only enhance the intense feeling, and you find yourself cumming more than usual.  They don't stop massaging you for more, even as you stop unloading.");

        CView.text("\n\n\"<i>More, I need more for my eggs!</i>\" Arian demands, yet you are truly spent...");
    }
    // (Medium Cum Amount)
    else if (player.cumQ() <= 1000) {
        CView.text("\n\nLiquid lust floods Arian's insides, as your " + describeCocksLight(player) + " do their best to relieve ");
        if (player.body.balls.count === 0) CView.text("themselves");
        else CView.text("your [balls]");
        CView.text(" of their load; a load Arian is not only pleased to accept, but also eager to relieve you of every single stray drop off.  The tightness of her ass, pressing down on your " + describeCock(player, secondCockThatFits) + ", her pussy milking on your " + describeCock(player, cockThatFits) + ".  How could anyone refuse such an invitation?  You let yourself go, stuffing the eager lizan with more cum than you thought yourself capable of producing.");

        CView.text("\n\n\"<i>M-more,</i>\" Arian pleads, even as her belly starts to distend.");
    }
    // (High Cum Amount)
    else {
        CView.text("\n\nYou cum with such force, that if Arian's ass wasn't clamping down on your " + describeCock(player, secondCockThatFits) + " so tightly, you'd be sure she was going to get pushed off.  \"<i>So much cum!  Yesssss!</i>\" Arian moans, as you quickly give her usually lithe belly a very blatant bump.  Even though your prodigious amount of cum is enough to completely fill the eager lizan-girl, her ass and pussy work overtime to ensure you're completely spent; and you have no desire to resist.");

        CView.text("\n\nBy the time you're finished, Arian's belly is positively bulging.  \"<i>Ahhh... eggs... cum... yes...</i>\" she states in a stupor.");
    }
    CView.text("\n\nArian's ass goes slack around your " + describeCock(player, secondCockThatFits) + ", and Arian slowly slides off your shaft to plop on her bed; eyes closed in bliss, as she takes a short nap.");

    // (if ArianDblPenChat == 0)
    if (ArianFlags.ARIAN_DOUBLE_PENETRATION_CHAT === 0) {
        ...
        if (!arianFollower()) CView.text("bedchambers");
        else CView.text("tent");
        CView.text(".");
    }
    else {
...
// ArianCockSize needs to be below 3. (ArianDblCock does not affect this decision.)
// PC cock area must be <= 30.
function arianDocking(player: Character): NextScreenChoices {
    CView.clear();
    ArianFlags.ARIAN_VIRGIN += 1;
    arianHealth(3);
    if (ArianFlags.ARIAN_VAGINA > 0)
        CView.image("arianfemale-home-docking");
    else
        CView.image("arianmale-home-docking");
    CView.text("You set your eyes on Arian's genital slit, and then smile at [Arian em].  You ask how [Arian ey]'d feel about ‘hiding' your cock, rather than [Arian eirs], inside [Arian eir] slit?");
    ...
    // (if ArianDblCock == 0)
    if (ArianFlags.ARIAN_DOUBLE_COCK === 0) {
        CView.text("didn't [Arian ey] always want to have two dicks?  Well, now it's [Arian eir] chance to find out what it'd be like.");
    }
    else {
        CView.text("[Arian ey]'s already hiding two in there... an extra cock won't make such a huge difference, would it?");
    }
    ...
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("pair of ");
    CView.text("pecker");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
    CView.text(" from your prying eyes.");
    ...
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
    CView.text(" of Arian's dick");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
    CView.text(".  You brush your fingers against the tip");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
    CView.text(", seeing what kind of reaction that will elicit from your scaly lover.");
    ...
    if (ArianFlags.ARIAN_DOUBLE_COCK === 0) CView.text(" brushing up against [Arian eir] own lizan pecker");
    else CView.text("sandwiched between [Arian eir] two lizard dicks");
    CView.text(", the friction is absolutely incredible, sending sparks of pleasure cascading along your shaft.  You moan in delicious lust and tell Arian that this is absolutely incredible.");
    ...
    CView.text("slides across your intruding shaft as you go, and [Arian eir] sheathe-walls grip and squeeze as best they can, leaving you moaning and panting like a bitch in heat.  Arian's moans of pleasure join yours, [Arian eir] twitching bulbous shaft");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
    CView.text(" massaging your [cock smallest] almost like a vagina would.  Shaky hands grab a hold of your shoulders, helping you down and up on Arian's male slit.  \"<i>Oh, Marae!  It feels like I'm getting an internal handjob... it feEels so... so good!  H - harder... almost cumming!</i>\" [Arian ey] pleads, as [Arian ey] breathes hotly on your face; panting like a bitch in heat.");
    ...
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("pair of cummy masts emerge");
    else CView.text("cummy mast emerges");
    CView.text(" to feebly shoot a couple more ropes of jism, before returning limply to ");
    if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("their");
    else CView.text("its");
    CView.text(" hiding place.  \"<i>Ahhh,</i>\" is all Arian says, before slumping down.");
...
// Give Item
function giveArianAnItem(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Thinking about the many items in your possession, you ask if Arian would be willing to take something for you?");

    // (if ArianHealth < 10) //May not give anything.
    if (ArianFlags.ARIAN_S_DIALOGUE < 1) {
    ...
    else if (ArianFlags.ARIAN_HEALTH < 20) { // May only give Vitality T.
        ...
        if (!player.inventory.items.has(ConsumableName.VitalityTincture)) {
    ...
    const choices: ScreenChoice[] = [];
    if (ArianFlags.ARIAN_S_DIALOGUE === 0 && arianHealth() >= 10) return arianStoryDialogue1(player);
    else if (ArianFlags.ARIAN_S_DIALOGUE === 1 && arianHealth() >= 20) return arianStoryDialogue2(player);
    else if (ArianFlags.ARIAN_S_DIALOGUE === 2 && arianHealth() >= 30) return arianDialogue3(player);
    else if (ArianFlags.ARIAN_S_DIALOGUE === 3 && arianHealth() >= 50) return arianImbue(player);
    else if (ArianFlags.ARIAN_S_DIALOGUE === 4 && arianHealth() >= 75) return arianPlot4(player);
    else if (ArianFlags.ARIAN_S_DIALOGUE === 5 && arianHealth() >= 100) return arianPlot5(player);
    else {
        if (player.inventory.items.has(ConsumableName.VitalityTincture)) choices[0] = ["Vital Tinct", arianVitalityTincture];
        if (ArianFlags.ARIAN_HEALTH >= 20) {
            if (player.inventory.items.has(ConsumableName.IncubusDraftPure)) choices[1] = ["P. Incubi D", giveIncubusDraftToArian];
            if (player.inventory.items.has(ConsumableName.SuccubiMilkPure)) choices[2] = ["P. Suc.Milk", succubiMilkForArian];
            if (player.inventory.items.has(ConsumableName.Lactaid)) choices[3] = ["Lactaid", giveArianLactaid];
            if (player.inventory.items.has(ConsumableName.Reducto)) choices[4] = ["Reducto", giveArianReducto];
            if (player.inventory.items.has(ConsumableName.Reptilum)) choices[5] = ["Reptilum", giveArianReptilum];
        }
        choices[9] = ["Back", arianHomeMenu];
    }
...
function arianVitalityTincture(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Fishing around amongst your pockets, you withdraw a vial of that strange potion Giacomo peddles and offer it to the sickly lizan, explaining it will bolster [Arian eir] constitution and fill [Arian em] with permanent vitality.");
    ...
    player.inventory.items.consumeItem(ConsumableName.VitalityTincture);
    arianHealth(10);

    return { next: giveArianAnItem };
}
...
function giveIncubusDraftToArian(player: Character): NextScreenChoices {
    CView.clear();
    player.inventory.items.consumeItem(ConsumableName.IncubusDraftPure);
    CView.text("Fishing around in your pockets, your hand closes on the vial of purified incubus draft.  You offer this to Arian, asking ");
    if (ArianFlags.ARIAN_COCK_SIZE > 0) CView.text("if [Arian ey] would like to make [Arian eir] cock bigger");
    else CView.text("if she'd like to have a touch of her old malehood back");
    CView.text(".");
    ...
    if (ArianFlags.ARIAN_COCK_SIZE === 0) {
        ...
        if (player.body.vaginas.length > 0) CView.text("[vagina] and ");
        CView.text("[asshole].");
        ...
        CView.text("\n\nA part of you wonders if maybe she's hoping you'll join her in bed, but then you take a closer look and concede that she is genuinely tired.  You stroke her affectionately on her scaly cheek and politely excuse yourself, heading back to ");
        ...
        if (ArianFlags.ARIAN_DOUBLE_COCK === 0) CView.text("shaft emerges from its hiding place.  It throbs and grows, settling in its");
        else CView.text("pair of shafts emerge from their hiding place.  They throb and grow, settling in their");
        CView.text(" new size as Arian moans and cums all over [Arian em]self.");
        ...
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s back into their");
        else CView.text(" back into its");
        CView.text(" slit.");
        if (ArianFlags.ARIAN_COCK_SIZE === 2) {
            ArianFlags.ARIAN_COCK_SIZE++;
            player.stats.lust += 10 + player.stats.lib / 20;
            ...
            if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("them");
            else CView.text("it");
            CView.text(" until two thirds are inside, then it doesn't looks like [Arian ey] can take in anymore.");

            if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("\n\n\"<i>W-Wow, they're so big I can't even manage to hide them,</i>\" Arian pants as the tip of [Arian eir] cocks hang limply, exposed for all to see.");
            ...
            if (ArianFlags.ARIAN_VAGINA > 0) {
                ...
                if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
                CView.text(" starts hardening, no longer able to hide Arian's excitement inside her protective slit.");
            }
            else {
            ...
            CView.text("\n\nYou apologize and tell the lizan you've teased [Arian em] enough; you'll give [Arian em] a chance to calm down, and politely excuse yourself to head back to ");
        }
    }
    // (else if ArianBreasts > 1) //Boob shrinkage.
    if (ArianFlags.ARIAN_BREASTS > 1 && (!ArianFlags.HYPER_HAPPY)) {
        ArianFlags.ARIAN_BREASTS--;
    ...
    else if (ArianFlags.ARIAN_BREASTS === 1 && (!ArianFlags.HYPER_HAPPY)) { // Now you are male again.
        ...
        ArianFlags.ARIAN_BREASTS = 0;
        ArianFlags.ARIAN_VAGINA = 0;
...
function succubiMilkForArian(player: Character): NextScreenChoices {
    CView.clear();
    player.inventory.items.consumeItem(ConsumableName.SuccubiMilkPure);
    CView.text("Fishing out the bottle of purified demon's milk, you ask if Arian is willing to get ");
    if (ArianFlags.ARIAN_VAGINA > 0) CView.text("further ");
    CView.text("in touch with [Arian eir] feminine side.  For you?");
    ...
    if (ArianFlags.ARIAN_VAGINA === 0) {
        ArianFlags.ARIAN_VAGINA++;
        if (ArianFlags.ARIAN_BREASTS === 0) ArianFlags.ARIAN_BREASTS++;
        ...
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
        CView.text(" having already softened and retracted");
        // if ArianCockSize >= 3:
        if (ArianFlags.ARIAN_COCK_SIZE === 3) CView.text(" as far as it'll go");
        CView.text(" into its protective slit.");
        ...
        CView.text("\n\nYou step forward and gently run your hands over her newfound breasts. Well, if that's what she's in the mood for, you're ");
    }
    else if (ArianFlags.ARIAN_BREASTS < 3) {
        ArianFlags.ARIAN_BREASTS++;
        ...
        if (ArianFlags.ARIAN_BREASTS === 2) {
            ...
            if (ArianFlags.ARIAN_COCK_SIZE >= 3) CView.text(" and her exposed shaft begins to grow hard");
            CView.text(".");
        }
        else {
            ...
            if (ArianFlags.ARIAN_COCK_SIZE === 3) CView.text(" and you see her exposed shaft slowly rising to point at you");
            CView.text(".");
    ...
    // (else If ArianCockSize > 1) //Dick shrinkage.
    else if (ArianFlags.ARIAN_COCK_SIZE > 1 && (!ArianFlags.HYPER_HAPPY)) {
        ...
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
        CView.text(".  \"<i>Oh... that's good.  Right?</i>\" Arian looks at you nervously.  You think it over, and confess that while you are more attracted to women than men, you won't force Arian to become fully female if she doesn't want to.");
    ...
    else if (ArianFlags.ARIAN_COCK_SIZE === 1 && ArianFlags.ARIAN_DOUBLE_COCK >= 1 && (!ArianFlags.HYPER_HAPPY)) {
        ...
        ArianFlags.ARIAN_DOUBLE_COCK = 0;
    }
    else if (ArianFlags.ARIAN_COCK_SIZE === 1 && (!ArianFlags.HYPER_HAPPY)) {
        ArianFlags.ARIAN_COCK_SIZE = 0;
...
function giveArianLactaid(player: Character): NextScreenChoices {
    CView.clear();
    player.inventory.items.consumeItem(ConsumableName.Lactaid);
    CView.text("Your hand closes around the vial of lactation-inducing potion that is Lactaid.  You almost reject it automatically, but then you stop and think.  There's odder things in this world, after all.  You remove the vial and ask Arian if [Arian ey] would be willing to let you see what lizan milk tastes like.");
    ...
    if (ArianFlags.ARIAN_BREASTS === 0) {
        ...
        if (ArianFlags.ARIAN_COCK_SIZE === 3) CView.text(", her exposed cock growing hard as she flushes with arousal");
        CView.text(".  Her hands grip the sheets and you watch her ");
        if (ArianFlags.ARIAN_BREASTS === 1) CView.text("perky");
        else if (ArianFlags.ARIAN_BREASTS === 2) CView.text("luscious");
        else CView.text("pillowy");
        CView.text(" breasts grow before your eyes.  \"<i>My breasts... d-do something, [name]!</i>\" Arian pleads.  Unsure of what you should be doing, you grab her breasts and begin kneading them as they grow in your hands; her erect nipples poking your palms.  \"<i>Ah... that feels good!  Don't stop!</i>\"");
...
function giveArianReducto(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Eyeing Arian up and down, you fish your pouches for a tube of Reducto.  Once you've found it, you hand it over to Arian and tell [Arian em] you'd like [Arian em] to reduce something for you.");
    CView.text("\n\n\"<i>Umm... sure, which part?</i>\"");
    // (Display Options)

    const choices: ScreenChoice[] = [];
    if (ArianFlags.ARIAN_BREASTS > 0) choices[0] = ["Breasts", useReductoOnAriansBreasts];
    if (ArianFlags.ARIAN_COCK_SIZE > 0) choices[1] = ["Cock", useReductoOnArianCocks];
    choices[2] = ["Asshole", useReductoOnAriansAsshole];
...
// Breasts:
// Cannot go flat
function useReductoOnAriansBreasts(player: Character): NextScreenChoices {
    CView.clear();
    player.inventory.items.consumeItem(ConsumableName.Reducto);
    CView.text("You point at Arian's ");
    if (ArianFlags.ARIAN_BREASTS === 1) CView.text("small");
    else if (ArianFlags.ARIAN_BREASTS === 2) CView.text("generous");
    else CView.text("pillowy");
    CView.text(" breasts.  \"<i>Okay... I suppose it would be good to lose a bit of weight up here and spare my back.</i>\"  Arian smiles, opening the tube of Reducto and her robes; then squeezing the contents of the paste her hands.");
    ...
    if (ArianFlags.ARIAN_BREASTS > 1) {
        ...
        CView.text("The two of you watch Arian's breasts, but nothing seems to be changing.  You reach forward to feel them, and indeed they haven't shrunk down at all.");
...
// Cannot remove cocks.
function useReductoOnArianCocks(player: Character): NextScreenChoices {
    CView.clear();
    player.inventory.items.consumeItem(ConsumableName.Reducto);
    CView.text("You point at [Arian eir] crotch, mentioning that you'd like [Arian em] to be smaller.");
    ...
    if (ArianFlags.ARIAN_COCK_SIZE === 3) {
        ...
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("pair of exposed lizard cocks");
        else CView.text("exposed lizard cock");
        CView.text(" into full erection.  Finally [Arian ey] begins applying the paste.");

        CView.text("\n\nThe changes are almost immediate; Arian groans and watches as [Arian eir] oversized dick");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
        CView.text(" slowly shrink");
        if (ArianFlags.ARIAN_DOUBLE_COCK === 0) CView.text("s");
        CView.text(" to a more manageable size.  Once the transformation is complete Arian tries to fully retract [Arian eir] cock");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s back into their");
        else CView.text(" back into its");
        CView.text(" hiding place; it's a tight fit, but it fits.");
        ...
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s from their tight hiding place");
        else CView.text(" from its tight hiding place");
        CView.text(", and into full erection.  Finally [Arian ey] begins applying the Reducto.");

        CView.text("\n\nIt takes mere moments for the changes to occur; Arian groans in slight discomfort as [Arian eir] cock");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s return to their");
        else CView.text(" returns to its");
        CView.text(" original size.  Once the transformation is complete, [Arian ey] retracts ");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("them");
        else CView.text("it");
        CView.text(" back into [Arian eir] genital slit.");
    ...
    else if (ArianFlags.ARIAN_DOUBLE_COCK > 0) {
        CView.text("Then, [Arian ey] coaxes [Arian eir] twin reptilian-peckers out of their hiding place.  Finally [Arian ey] applies the paste to both shafts in turn.");
...
function useReductoOnAriansAsshole(player: Character): NextScreenChoices {
    CView.clear();
    player.inventory.items.consumeItem(ConsumableName.Reducto);
    CView.text("You ask Arian to hand the tube of reducto back over to you, telling [Arian em] that you want to make [Arian em] a little tighter when you do [Arian em] from behind.  The lizard-");
    if (ArianFlags.ARIAN_COCK_SIZE > 0) {
        if (ArianFlags.ARIAN_VAGINA === 0) CView.text("man");
        else CView.text("herm");
    }
    else CView.text("woman");
    CView.text(" does as you ask, allowing you to smear your fingers generously with the shrinking cream, even as [Arian ey] anticipates your next request and removes [Arian eir] clothes before lying down, bum up in the air and tail slowly swishing to and fro, [Arian eir] anus exposed and waiting for your ministrations.");
    // (if AnalXP == 1)
    if (ArianFlags.ARIAN_ANAL_XP <= 1) {
    ...
    // (else if AnalXP < 33)
    else if (ArianFlags.ARIAN_ANAL_XP < 33) {
        ...
        ArianFlags.ARIAN_ANAL_XP = 1;
    }
    else if (ArianFlags.ARIAN_ANAL_XP < 66) {
        ArianFlags.ARIAN_ANAL_XP -= 33;
...
function giveArianReptilum(player: Character): NextScreenChoices {
    CView.clear();
    player.inventory.items.consumeItem(ConsumableName.Reptilum);
    CView.text("Fingering the vial of reptilium, you smirk to yourself.  Quickly wiping it off your face, you instruct Arian to close [Arian eir] eyes and open [Arian eir] mouth, as you have a special surprise for [Arian em].");
    ...
    // (if ArianFirstRept == 1)
    if (ArianFlags.ARIAN_FIRST_REPTILUM === 0) {
    ...
    // (if ArianFirstRept == 1)
    if (ArianFlags.ARIAN_FIRST_REPTILUM === 1) {
    ...
    CView.text("\n\nYou shake your head and tell [Arian em] you'd rather [Arian ey] didn't lick your butt.  Why don't you go and get [Arian em] some candy instead?  \"<i>Candy!?  I love candy!  You can smear chocolate on yourself and I could lick it clean! Then we'd get chocolate flavored cloud ice-cream with " + describeRace(player) + " musk!  What a great idea!  Get your undies off so I can get started!</i>\"  [Arian Ey] pounces on you, effectively removing your underpants and exposing your ");
    if (player.body.cocks.length > 0) {
    ...
    if (player.body.vaginas.length <= 0 && player.body.cocks.length <= 0) CView.text("genderless crotch");
    CView.text(".  \"<i>Hmm, that looks much tastier than candy actually.</i>\"  [Arian Ey] licks your ");
    if (player.body.cocks.length > 0) CView.text("cock from base to tip");
    else if (player.body.vaginas.length > 0) CView.text("pussy all the way from the back to the tip of your [clit]");
    else CView.text("your sweaty crotch");
    CView.text(".  \"<i>Yummy!  I could use seconds, but roleplay time is over; let's... masturbate each other!</i>\"  [Arian Ey] begins stroking [Arian eir] ");

    if (ArianFlags.ARIAN_VAGINA > 0)
        CView.image("arianfemale-home-mutualmasturbation");
    else
        CView.image("arianmale-home-mutualmasturbation");

    if (ArianFlags.ARIAN_COCK_SIZE === 1) {
        CView.text("quickly erecting lizan cock");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
        if (ArianFlags.ARIAN_VAGINA > 0) CView.text(" and her ");
    }
    if (ArianFlags.ARIAN_VAGINA > 0) CView.text("slavering vagina");
    CView.text(".");
    ...
    if (ArianFlags.ARIAN_COCK_SIZE > 0) {
        CView.text("shooting thick ropes of cum up into the air to land on [Arian eir] ");
        if (ArianFlags.ARIAN_BREASTS === 0) CView.text("chest");
        else if (ArianFlags.ARIAN_BREASTS === 1) CView.text("perky little breasts");
        else if (ArianFlags.ARIAN_BREASTS === 2) CView.text("soft breasts");
        else CView.text("pillowy breasts");
        if (ArianFlags.ARIAN_VAGINA > 0) CView.text(" while ");
    }
    if (ArianFlags.ARIAN_VAGINA > 0) CView.text("her vagina doing its best to soak her bed with lizan femcum");
    CView.text(".");
    ...
    if (randInt(2) === 0 && ArianFlags.ARIAN_DOUBLE_COCK === 0 && ArianFlags.ARIAN_COCK_SIZE > 0) {
    ...
    if (tfed) {
        ...
        ArianFlags.ARIAN_DOUBLE_COCK = 1;
    }
    else {
...
function treatCorruption(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You ask Arian if [Arian ey] thinks [Arian ey] can help you reduce some of the taint that has infected your soul.");

    if (ArianFlags.ARIAN_TREATMENT === 1) {
        ...
        if (player.stats.cor === 0) CView.text(" - in fact, you're quite sure that Arian has purified you entirely");
        CView.text(".  You thank the lizan for [Arian eir] magical treatment.");
...
function imbueTalisman(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You tell Arian that, if it's not too much trouble, you'd like [Arian em] to ");
    if (player.inventory.keyItems.has("Arian's Talisman")) CView.text("place a spell in the enchanted talisman [Arian ey] created for you");
    else CView.text("change the spell in the talisman [Arian ey] created for you");
	...
	*/
    ...
    const choices: ScreenChoice[] = [];
    if (player.inventory.items.reduce(Inventory.TotalQuantityOf(MaterialName.BlackChitin), 0) >= 2 && player.inventory.items.has(MaterialName.ToughSpiderSilk)) choices[0] = ["Shielding", partial(arianSpellPlace, player, "Shielding Spell")];
    if (player.inventory.items.reduce(Inventory.TotalQuantityOf(ConsumableName.GoblinAle), 0) >= 2 && player.inventory.items.has(ConsumableName.SweetGossamer)) choices[1] = ["Immolation", partial(arianSpellPlace, player, "Immolation Spell")];
    choices[9] = ["Back", arianHomeMenu];

    return { choices };
}

function arianSpellPlace(player: Character, spell: string): NextScreenChoices {
    CView.clear();
    CView.text("You tell Arian that you want [Arian em] to place the " + spell + " spell in your talisman for you.");
    ...
    if (spell === "Shielding Spell") {
        player.effects.add(StatusEffectType.ShieldingSpell, 0, 0, 0, 0);
        // Shielding Spell: 2x Black Chitin and 1x Tough Silk. - Increases defense for the duration of the battle.
        player.inventory.items.consumeItem(MaterialName.BlackChitin, 2);
        player.inventory.items.consumeItem(MaterialName.ToughSpiderSilk);
    }
    if (spell === "Immolation Spell") {
        player.effects.add(StatusEffectType.ImmolationSpell, 0, 0, 0, 0);
        // Immolation Spell: 2x Goblin Ale and 1x Sweet Gossamer. - Deals damage over time.
        player.inventory.items.consumeItem(ConsumableName.GoblinAle, 2);
        player.inventory.items.consumeItem(ConsumableName.SweetGossamer);
    }
    // If charged, stay chargggggeeed
    if (player.inventory.keyItems.has("Arian's Talisman")) {
        player.inventory.keyItems.remove("Arian's Talisman");
        player.inventory.keyItems.add("Arian's Charged Talisman", 0, 0, 0, 0);
    }
    return { next: returnToCampUseOneHour };
}
function clearCharges(player: Character) {
    if (player.effects.has(StatusEffectType.ShieldingSpell)) player.effects.remove(StatusEffectType.ShieldingSpell);
    if (player.effects.has(StatusEffectType.ImmolationSpell)) player.effects.remove(StatusEffectType.ImmolationSpell);
}
export function clearTalisman(player: Character) {
    player.inventory.keyItems.remove("Arian's Charged Talisman");
    player.inventory.keyItems.add("Arian's Talisman", 0, 0, 0, 0);
}
...
// Camp Modifier
/*9999
The small tent that Arian dwells in is in its usual place in a quiet corner of the  You're not sure if he's inside, taking advantage of its uncannily larger interior, or out on one of his usual strolls outside of the
*/
// Approach Arian

// Sleep With Arian
export function sleepWithArian(player: Character, newl: boolean): NextScreenChoices {
    if (newl) CView.clear();
    ArianFlags.SLEEP_WITH = "Arian";
    CView.text("Tired after a whole day of adventuring, you decide to retire and catch some shut-eye.  While going through the day's events, you recall Arian had offered to let you stay in [Arian eir] tent and sleep with [Arian em] in [Arian eir] bed.  Your tired body could surely use a soft bed today, and maybe a certain lizan to keep you company too.  With that in mind, you head to [Arian eir] tent.");
    ...
    if (ArianFlags.ARIAN_COCK_SIZE > 0) {
        CView.text("You can clearly see [Arian eir] exposed cock");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
        CView.text(" stiffening at the sight.  ");
    }
    if (ArianFlags.ARIAN_VAGINA > 0) CView.text("You can tell that [Arian ey] is aroused by the little show, despite [Arian eir] body doing a good job of hiding it.  ");
    CView.text("You just let [Arian em] squirm, pretending you don't see [Arian eir] reactions, and then, indicating the bed, you ask which of you should get in first.");
    ...
    if (ArianFlags.ARIAN_ANAL_XP < 33) {
        ...
        choices[0] = ["Listen", listenToLowAnalXPArian];
        choices[1] = ["Sleep", dontListenToLowAnalXPArian];
    }
    // (else if AnalXP <66)
    else if (ArianFlags.ARIAN_ANAL_XP < 66) {
        ...
        choices[0] = ["Listen", listenToMediumAnalXPArian];
        choices[1] = ["Sleep", dontListenToMediumAnalXPArian];
    }
    else { // AnalXP <= 100
        ...
        choices[0] = ["Tease", TeaseHighAnalXPArian];
        choices[1] = ["Sleep", dontTeaseHighAnalXPArian];
    }
...
// [=Sleep=]
function dontListenToLowAnalXPArian(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You gently stroke the lizan's side and plant a soft, gentle kiss on [Arian eir] neck.  At your ministrations, Arian sighs and slowly falls still again.  You wait, but it seems that whatever dreams [Arian ey] was having have faded and [Arian ey]'s truly asleep now, allowing you to go back to sleep yourself.");

    return { next: sleepWrapper };
}

// [=Listen=]
function listenToLowAnalXPArian(player: Character): NextScreenChoices {
    CView.clear();
    ArianFlags.ARIAN_MORNING = 1;
    CView.text("You turn you head to try and catch what Arian might be dreaming about.");
...
// [=Sleep=]
function dontListenToMediumAnalXPArian(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You gently stroke the lizan's side and plant a soft, gentle kiss on [Arian eir] neck.  At your ministrations, Arian sighs and slowly falls still again.  You wait, but it seems that whatever dreams [Arian ey] was having have faded and [Arian ey]'s truly asleep now, allowing you to go back to sleep yourself.");
...
// [=Listen=]
function listenToMediumAnalXPArian(player: Character): NextScreenChoices {
    CView.clear();
    ArianFlags.ARIAN_MORNING = 1;
    CView.text("You turn you head to try and catch what Arian might be dreaming about.");
    ...
    if (player.body.cocks.length > 0) {
        ...
        else CView.text("finger");
        CView.text(" still worming its way into [Arian eir] fuckable ass, but it really does look like [Arian ey]'s fast asleep.  Weird.  \"<i>So many muffins... all [name] flavored...</i>\"  [Arian ey] calms down and begins snoring lightly.");
        ...
        if (player.stats.cor >= 66) CView.text(", despite the silly lizan being deserving of it");
        CView.text(".  But as payback, [Arian ey]'s going to sleep with you inside, not that it seems like it'll be much of a problem to [Arian em]... Too bad you can't hold an erection all the time while asleep, so you know eventually you'll slip out...");
    }
    else {
...
// [=Sleep=]
function dontTeaseHighAnalXPArian(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You gently stroke the lizan's side and plant a soft, gentle kiss on [Arian eir] neck.  At your ministrations, Arian sighs and slowly falls still again.  You wait, but it seems that whatever dreams [Arian ey] was having have faded and [Arian ey]'s truly asleep now, allowing you to go back to sleep yourself.");
...
// [=Tease=]
function TeaseHighAnalXPArian(player: Character): NextScreenChoices {
    CView.clear();

    if (ArianFlags.ARIAN_VAGINA > 0)
        CView.image("arianfemale-camp-dreamingArian");
    else
        CView.image("arianmale-camp-dreamingArian");
    ArianFlags.ARIAN_MORNING = 1;
...
export function wakeUpAfterArianSleep(player: Character): NextScreenChoices {
    CView.clear();
    if (player.body.cocks.length > 0) {
        if (ArianFlags.ARIAN_VAGINA > 0)
            CView.image("arianfemale-camp-dreamingArian");
        ...
        if (ArianFlags.ARIAN_ANAL_XP < 33) {
            ...
            CView.text("Your eyes open, and you suddenly realize that whatever dreams you were having... are now gone.  You make note of the still sleeping lizan right next to you and recall last night's events.  You wonder if Arian will remember any of that...");
            ...
            if (ArianFlags.ARIAN_VAGINA > 0) {
                CView.text(arianMF("his", "her") + " pussy is squirting her juices all over your lower body");
                if (ArianFlags.ARIAN_COCK_SIZE > 0) {
                    CView.text(" and ");
                }
                else CView.text("; ");
                ...
                CView.text(arianMF("his", "her") + " ");
                if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("twin shafts are");
                else CView.text("shaft is");
                CView.text(" squirting white ropes of lizan cum all over the bedsheets;");
            }
            CView.text(" what a naughty little lizan.");

            CView.text("\n\nYou can't resist playfully teasing Arian that [Arian ey] is such a buttslut.  ");
            if (player.body.vaginas.length <= 0) CView.text("Did [Arian ey] always know [Arian ey] wanted a man to top [Arian em], or is this development new to [Arian em]?");
            else CView.text("Does [Arian ey] really get off on having a girl with a dick all her own fuck [Arian em] like a girl, hmm?");
        ...
        if (ArianFlags.ARIAN_ANAL_XP < 33) {
            ...
            if (ArianFlags.ARIAN_COCK_SIZE > 0) {
                CView.text(arianMF("his", "her") + " ");
                if (ArianFlags.ARIAN_COCK_SIZE >= 3) CView.text("exposed ");
                CView.text("bulbous shaft");
                if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s begin");
                else CView.text(" begins");
                CView.text(" sporting an erection");
                if (ArianFlags.ARIAN_COCK_SIZE < 3) {
                    CView.text(", poking ");
                    if (ArianFlags.ARIAN_DOUBLE_COCK === 1) CView.text("its tip");
                    else CView.text("their tips");
                    CView.text(" out of Arian's genital slit");
                }
                CView.text(".");
            }
            else if (ArianFlags.ARIAN_VAGINA > 0) CView.text("and her moist pussy, begins forming a stream of juices that gently run its course through the inside of Arian's thighs, evaporating before hitting the floor due to the lizan's currently elevated body heat.");
            CView.text("  [Arian Ey] pants, eyes glazed.  \"<i>Uhh... [name]...</i>\"");
...
export function arianEggingEvent(player: Character): NextScreenChoices {
    CView.clear();
    ArianFlags.ARIAN_EGG_EVENT = 1;
    if (ArianFlags.ARIAN_EGG_CHAT === 0) {
        ArianFlags.ARIAN_EGG_CHAT++;
        CView.text("As you are about to enter Arian's tent, you hear a moan emanate from within.  Those aren't moans of pleasure though.  They are moans of discomfort.  Wondering if the lizan is all right, you decide to enter her tent.");
        ...
        CView.text("As you are about to enter Arian's tent, you hear a moan emanate from within.  Seems like it's that time of the month again...");
    ...
    return {
        choices: [
            ["Brown", partial(pickAnEggArian, player, "brown")],
            ["Purple", partial(pickAnEggArian, player, "purple")],
            ["Blue", partial(pickAnEggArian, player, "blue")],
            ["Pink", partial(pickAnEggArian, player, "pink")],
            ["White", partial(pickAnEggArian, player, "white")],
            ["Rubber", partial(pickAnEggArian, player, "rubbery black")],
            ["No Thanks", leaveEggs],
        ]
    };
}
...
// Pick a color
function pickAnEggArian(player: Character, color: string/* = "pink"*/): NextScreenChoices {
    CView.clear();
    ArianFlags.ARIAN_EGG_COLOR = color;
    CView.text("You tell Arian you'd like her to make you a " + color + " egg.");
    ...
    ArianFlags.ARIAN_EGG_COUNTER = 1;
    return { next: returnToCampUseOneHour };
}
// Leave
function leaveEggs(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You tell her that you don't want any colored eggs from her this month.  The lizan nods, deciding it's not necessary for her to spell out that this means she'll just produce ordinary eggs and eat them for breakfast after she's laid them.  \"<i>So, do you want something?</i>\" she asks.");
    return { next: returnToCampUseOneHour };
}
...
export function arianLaysEggs(player: Character): NextScreenChoices {
    CView.clear();
    const color: string = ArianFlags.ARIAN_EGG_COLOR;
    ArianFlags.ARIAN_EGG_COUNTER = 0;
    CView.text("As you approach the tent, you wonder if Arian's ready to lay those eggs...  Your contemplations are promptly interrupted as you hear a pained moan coming from inside.  Seems like Arian is in labor!  Without delay, you rush inside, to be greeted by the sight of naked lizan-");
    if (ArianFlags.ARIAN_COCK_SIZE > 0) CView.text("herm");
    else CView.text("girl");
    CView.text(" squatting over a wooden basin, one hand between her legs, massaging her dripping treasure, while the other is rubbing her belly to try and coax the eggs out of her.");
    ...
    let name: ConsumableName;
    if (ArianFlags.ARIAN_EGG_COLOR === "brown") name = ConsumableName.LargeEggBrown;
    else if (ArianFlags.ARIAN_EGG_COLOR === "purple") name = ConsumableName.LargeEggPurple;
    else if (ArianFlags.ARIAN_EGG_COLOR === "blue") name = ConsumableName.LargeEggBlue;
    else if (ArianFlags.ARIAN_EGG_COLOR === "pink") name = ConsumableName.LargeEggPink;
    else if (ArianFlags.ARIAN_EGG_COLOR === "white") name = ConsumableName.LargeEggWhite;
    else if (ArianFlags.ARIAN_EGG_COLOR === "rubbery black") name = ConsumableName.LargeEggBlack;
    return player.inventory.items.createAdd(player, ItemType.Consumable, name, returnToCampUseOneHour);
}
// DildoFun
function arianDildoFun(player: Character): NextScreenChoices {
    ...
    CView.clear();
    // 1st time:
    if (ArianFlags.TIMES_ARIAN_DILDOED === 0) {
        CView.text("Looking over your reptilian girlfriend, your eyes go down her body");
        if (ArianFlags.ARIAN_COCK_SIZE > 0) {
            CView.text(", past her cock");
            if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
        }
        else CView.text(",");
        CView.text(" to her dripping wet pussy, clearly yearning for some action.  Reflecting that your own love-lips feel the same way, you decide to try something fun and present to Arian the dildo that you ");
        if (player.inventory.keyItems.has("Dildo")) CView.text("bought from Giacomo");
        else CView.text("got from Tamani");
        CView.text(", asking if she's willing to give this a try.");
        ...
        CView.text("You present your dildo to Arian again, telling her that this time, you want to play with your toys - after all, she enjoyed it so much the first time.");

        CView.text("\n\nArian fidgets in embarrassment.  \"<i>Okay.</i>\"");
    }
    ArianFlags.TIMES_ARIAN_DILDOED++;
    ...
    if (ArianFlags.ARIAN_COCK_SIZE > 0) {
        CView.text("Ignoring her cock");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
        CView.text(", y");
    }
    else CView.text("Y");
    CView.text("ou slide first one finger into her slick pussy and then another, gently caressing her inner labia to see if you can find her sweet spots.  Arian's eyes open in surprise as the first pang of pleasure hits her, she moans into your kiss.  Her tongue darts to dance with your own in excitement.");
    ...
    if (ArianFlags.ARIAN_COCK_SIZE > 0) {
        CView.text("brushing it tantalizingly across the sensitive skin of her pre-oozing cock");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s");
        CView.text(" before");
    }
    CView.text(" brushing it against her outer lips, stroking it up and down each labia, rubbing it against her clitoris, gently pushing the very tip of it into her cunt only to then withdraw it before it can really start to stretch her cunt.");
    ...
    if (ArianFlags.ARIAN_COCK_SIZE > 0) {
        CView.text("Her cock");
        if (ArianFlags.ARIAN_DOUBLE_COCK > 0) CView.text("s throb");
        else CView.text("throbs");
        CView.text(" throb as a jet of whiteness escape them.  ");
    }
    CView.text("You can feel it... the suddenness of your penetration brought the poor lizan mage to an instant orgasm.");
