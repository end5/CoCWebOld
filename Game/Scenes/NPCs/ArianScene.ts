...
export class ArianScene implements UpdateInterface {
	...
	private helpArianWhenYouMeetHim() {
		...
		DisplayText("You approach the hooded figure with caution, asking if they're all right; it feels a little silly to say that, but you can't think of much else to say.");
	...
	public visitThePark() {
		...
		if (Flags.list[FlagEnum.ARIAN_PARK] === 1) {
			DisplayText("\n\nFeeling ");
			if (player.stats.cor < 50) DisplayText("curious");
			else DisplayText("bored");
			DisplayText(", you decide to ask him what his story is.");
			...
			if (player.hasSpells()) {
				DisplayText("  You thought spellcasting merely took fatigue and the proper mindset, not life force, and you express that sentiment to the lizan.");
			}
		...
		//Visit 3
			...
			if (player.stats.cor > 66) DisplayText(", whilst privately thinking to yourself how useless that looks - no wonder they can't fight the demons if this is the best they're capable of");
			DisplayText(".");
			...
			if (player.stats.cor >= 66) DisplayText(", keeping your real reasons for bothering with him to yourself");
			DisplayText(".");
	...
	public visitAriansHouse() {
		...
		if (arianFollower()) {
			DisplayText("You approach the enchanted tent and slip easily inside the doors to the luxurious interior.  ");
			let temp: number = randInt(10);
			if (temp === 0) {
				...
				DisplayText("The smell of fresh cooking fills the air and you can see Arian happily sitting down at his couch with a plate of something just cooked.  \"<i>Oh, [name]; I was just about to eat, can I offer you a dish?  Or if you'd rather do something else, this can wait,</i>\" the lizan tells you with a smile.");
			}
			else if (temp <= 6) {
				...
				if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
					if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText("mixed sexual fluids");
					else DisplayText("jizz");
				}
				else DisplayText("femjizz");
				DisplayText(" and, indeed, still hovering over [Arian eir] ");
				if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
					if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText("twin sexual slits");
					else {
						DisplayText("cock");
						if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
					}
				}
				else DisplayText("pussy");
				DisplayText(".  For a long moment, [Arian ey] just stares back at you; if lizans could blush, you're certain [Arian ey]'d be red as a beet.  \"<i>[name]! I was- I was just....  Oh, this is embarrassing,</i>\" [Arian ey] mutters, looking at [Arian eir] feet.");
			...
			if (Flags.list[FlagEnum.ARIAN_PARK] === 4) {
				...
				if (player.stats.cor < 33) DisplayText("curiously");
				else if (player.stats.cor < 66) DisplayText("casually");
				else DisplayText("indifferently");
				DisplayText(" ask why he's thanking you.");
				...
				else DisplayText("seat your tauric body on the floor");
				DisplayText(".  You rack your brains for polite conversation, and finally ask how he's been since you saw him last.");
				...
				//50-74 health
					...
					if (!player.torso.hips.legs.isTaur()) DisplayText("take the offered chair");
					else DisplayText("the offered chair");
					DisplayText(".");
				...
				//75-100 health:
					...
					if (!player.torso.hips.legs.isTaur()) DisplayText(" and offers you the chair nearby");
					DisplayText(".  Taking another for [Arian em]self.  \"<i>I love when you come visit, [name].  So, what are we going to do today?</i>\" [Arian ey] asks, expectantly.");
				}
				//(Display Options)
				arianHomeMenu();
			}
	...
	private peepOnArian() {
		...
		DisplayText(", though this isn't stopping him from enthusiastically stroking himself off.");
	...
	private arianStoryDialogue1() {
		...
		//PC has met Marae: 
		if (player.statusAffects.has(StatusAffectType.MetMarae)) DisplayText("Marae herself told you they showed up about, what, 20-30 years ago?");
		else DisplayText("you'd guess a long while ago given the general mess they seem to have made of the world.");
	...
	private arianPlot5() {
		...
		if (camp.companionsCount() > 1) DisplayText("  There are other people living with you already, so what's one more?  Right?");
		DisplayText("</i>\"");
	...
	private arianMagicLessons() {
		...
		if (player.stats.int < 25) {
			...
			if (!arianFollower()) DisplayText("make your way back to your camp.");
			else DisplayText("return to your duties.");
		}
		//(else if PC int < 50 //Teach Charge Weapon if PC doesn't know.
			...
			if (!arianFollower()) DisplayText(", making your way back to camp.");
			else DisplayText(", exiting [Arian eir] tent and going about your business.");
			...
			if (!player.statusAffects.has(StatusAffectType.KnowsCharge)) {
				...
				DisplayText(" away, you start pondering Arian's lesson of the day.  You wonder if you could perhaps apply the channeling principle to strengthen your [weapon] in combat.");
			...
			DisplayText("<b>Game Note: This isn't canon and will be re-written soonish.</b>");
			...
			if (!arianFollower()) DisplayText("begin the trek home.");
			else DisplayText("exit [Arian eir] tent.");
			//(if PC doesn't know Whitefire)
			if (!player.statusAffects.has(StatusAffectType.KnowsWhitefire)) {
	...
	private inviteArianToCamp() {
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			if (Flags.list[FlagEnum.ARIAN_VAGINA] === 0) DisplayText("man");
	...
	private takeYerLizardHomePartII() {
		...
		if (camp.followersCount() > 1) DisplayText(" and your other companions");
		DisplayText(".");
	...
	private giveArianAnal() {
		...
		if (x === -1) {
			x = player.torso.cocks.sort(Cock.SmallestCockArea)[0];
		}

		DisplayText("You tell Arian that, if [Arian ey]'s willing, you'd like to take [Arian em] from behind.");
		//AnalXP < 33
		...
		if (Flags.list[FlagEnum.ARIAN_ANAL_XP] < 33) {
			...
			if (Flags.list[FlagEnum.ARIAN_VAGINA] < 1) {
				...
				if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] === 0) DisplayText("girl");
				else DisplayText("herm");
				DisplayText(" isn't going to make it any smaller either.</i>\"  Arian fidgets and shudders as you stroke [Arian eir] behind.");
			}
			...
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) {
				DisplayText("the puffy lips of [Arian eir] drooling vagina");
				if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) DisplayText(" and ");
			}
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
				DisplayText(arianMF("his", "her") + " erecting ");
				if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("pair of reptilian pricks");
				else DisplayText("reptilian prick");
			}
			...
			if (player.torso.balls.quantity === 0) DisplayText("the base of your cock");
			else DisplayText("the bottom of your balls");
			DisplayText(" and you warn Arian that you're going to cum inside [Arian em] if you keep going.  Arian drools in pained pleasure, for a moment you wonder if [Arian ey] even heard you.  \"<i>Cum.  Cum inside me.  I want it... all of it! Ah!</i>\"");
			...
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0 && Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText(" and ");
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText("wetting both your and her legs with a healthy serving of lizan pussy juice");
			DisplayText(".  You shudder and gasp until, utterly spent, you can't resist sinking down atop Arian and bearing [Arian em] into the comforting embrace of the bed.  You lay there, recovering your strength from your most delightful exertions, wriggling to give the lizan a proper snuggling while you lay there.");
		...
		//Loose, sensitive enough to make Arian scream in pleasure. (It's like a tight vagina, feels like one too, minor pain)
			...
			if (Flags.list[FlagEnum.ARIAN_VAGINA] < 1) {
				...
				if (Flags.list[FlagEnum.ARIAN_BREASTS] === 1) DisplayText(" perky");
				else if (Flags.list[FlagEnum.ARIAN_BREASTS] === 2) DisplayText(" rounded");
			...
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
				DisplayText(" a");
				if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText(" pair of dripping cocks");
				else DisplayText(" dripping cock");
				if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText(" and");
			}
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText(" a sopping wet pussy");
			DisplayText(" greet");
			if ((Flags.list[FlagEnum.ARIAN_VAGINA] === 0 && Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] === 0) || (Flags.list[FlagEnum.ARIAN_VAGINA] === 1 && Flags.list[FlagEnum.ARIAN_COCK_SIZE] === 0)) DisplayText("s");
			DisplayText(" your touch.  What a little slut Arian is turning into; [Arian ey]'s already raring to go, and even knowing it's going to be [Arian eir] ass that's getting fucked, too!");
			...
			if (Flags.list[FlagEnum.ARIAN_VAGINA] < 1) DisplayText("boypussy");
			else DisplayText("nether-pussy");
			...
			else DisplayText("depths of your balls");
			DisplayText(".  Arian, completely blissed out, lays limply on [Arian eir] bed, [Arian eir] butt held up by your gripping hands.  With a final deep thrust you finally go over the edge.");
			...
			if (!arianFollower()) DisplayText("room");
			else DisplayText("tent");
			DisplayText(".");
		}
		//AnalXP <= 100
		//Very Loose, sensitive enough to make Arian cum just from insertion. (Feels better than anything else. Yep, Arian really becomes a buttslut at this point)
			...
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText("wet gash is positively leaking lizan juices");
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0 && Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) DisplayText(" and [Arian eir] ");
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) DisplayText("erect cock is already leaking pre like a sieve");
			DisplayText(".  You can hardly believe how far Arian has come; you ask [Arian em] if it really feels that good?  \"<i>More than you can... ah... imagine, now fill me up!</i>\" [Arian ey] demands with a sense of urgency.");
			...
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText("her pussy clenches, spilling a flood of juices; ");
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0 && Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) DisplayText(" and ");
			...
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
				if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("cocks are still rock hard, and surprisingly, still leaking ropes of cum");
				else DisplayText("cock is still rock hard, and surprisingly, still leaking ropes of cum");
				if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText(", and her");
			}
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText(" pussy squirting juices against your [legs] with each shuddering impact of your [hips]");
			DisplayText(".  \"<i>Ah!  Yes!  More, give me more!  Harder!  Faster!</i>\" Arian pleads, before biting on [Arian eir] pillow in pleasure.");
			...
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) {
				DisplayText("  Her cunt grips the air and blasts one last jet of wetness at your crotch.");
			}
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) DisplayText("  [Arian Eir] throbbing cock flexes and shoots one last rope of lizan-cum against your belly.");
			DisplayText("  [Arian Eir] ass grips you forcefully, forming a vacuum and finally loosens.  \"<i>That... that wash da best,</i>\"  Arian slurs, before finally collapsing on [Arian eir] bed.");
			...
			if (Flags.list[FlagEnum.ARIAN_ASS_CHAT] === 0) {
				...
				if (9999 === 9999) DisplayText(" as hypocritical as that may be");
				DisplayText("... besides, you think [Arian ey]'s kind of sexy when [Arian ey] gets like that.  At that Arian perks up.  \"<i>Really?</i>\"  Yes, really, you reply.  Arian smiles happily at you.  \"<i>So... do you want to go again?</i>\"");
				...
				if (!arianFollower()) DisplayText("back to Tel'Adre's streets, and from there back to camp.");
				else DisplayText("out of [Arian eir] tent.");
	...
	private getBlownByArian() {
		...
		if (Flags.list[FlagEnum.ARIAN_HAS_BLOWN] === 0) {
			...
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] === 3) DisplayText("  You notice Arian's exposed shaft slowly hardening at your invitation.");
			DisplayText("  \"<i>I... Can I really?</i>\"");
		...
		if (player.cumQ() < 250) DisplayText("small");
		else if (player.cumQ() < 1500) DisplayText("big");
		else DisplayText("huge");
		DisplayText(" pot belly.  [Arian Ey] lets go of your dick with a sigh of pleasure and lays back on the bed.  \"<i>Ah... I'm full.</i>\"");

		DisplayText("\n\nYou tell [Arian em] that [Arian ey] looks full");
		if (player.cumQ() >= 1500) DisplayText(", in fact, you're amazed [Arian ey] didn't pop with how much [Arian ey] drank");
		DisplayText("; is [Arian ey] sure [Arian ey]'s all right?");
		...
		if (player.cumQ() >= 1000) DisplayText(" which, considering the fact [Arian ey] looks like a mother ready to birth a toddler from the size of [Arian eir] belly, isn't as easy as you'd think");
		DisplayText(".  Letting the lizan use your shoulder, you pull [Arian eir] sheets up and help [Arian em] back in the bed, where [Arian ey] flops down with a protest of springs");
		if (player.cumQ() >= 250) DisplayText(", [Arian eir] gut audibly sloshing as the cum inside is churned by the impact");
		DisplayText(".");
		...
		if (!arianFollower()) DisplayText(" back to camp, stopping only to notify Laika and Boon that Arian is sleeping.");
		else DisplayText(" out of Arian's tent.");
		player.orgasm();
	...
	private penetrateArian() {
		...
		DisplayText(".  \"<i>Err... readier?</i>\"  She asks, smiling nervously.");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] === 3) {
			DisplayText(", her exposed cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
			DisplayText(" hardening at what she's about to do");
		}
		else if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			DisplayText(", her bulbous cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s peeking out of their");
			else DisplayText(" peeking out of its");
			DisplayText(" hiding place, already at full mast in anticipation");
		}
		DisplayText(".  Arian swallows audibly.  \"<i>What now?</i>\" she asks, already panting in lust.");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] === 0) DisplayText("female");
		else DisplayText("herm");
		DisplayText(" lizan, until you have managed to hilt yourself inside of her.");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			DisplayText("  Her ");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("twin shafts");
			else DisplayText("shaft");
			DisplayText(" bobbing just out of your field of vision, leaking pre all over your belly.");
		}
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			DisplayText("  Her cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s twitch as they spurt ");
			else DisplayText(" twitches as it spurts ");
			DisplayText("a small rope of pre over her belly, anticipating what's to come as she braces herself for the inevitable pounding you're about to give her.");
		}
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			DisplayText("Arian's ");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("twin cocks throb");
		...
		if (player.torso.vaginas.count > 0) DisplayText(" A wash of fluids spills down your body from your cunt as it spasms in release and y");
		else DisplayText("Y");
		DisplayText("ou unleash your essence right into the lizan's burning womb.");
		...
		if (player.cumQ() < 250) DisplayText("lean");
		else if (player.cumQ() < 1000) DisplayText("full");
		else DisplayText("overfilled");
		DisplayText(" belly.  \"<i>Stay with me, like this, a little longer?</i>\" Arian asks.");
	...
	private getButtWreckedByArian() {
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] >= 3) {
			DisplayText("exposed cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s harden");
			else DisplayText(" hardens");
			DisplayText(" to full mast.");
		}
		else {
			DisplayText(arianMF("his", "her") + " cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s peek out of their");
			else DisplayText(" peeks out of its");
			DisplayText(" hiding place, rising to full mast.");
		}

		DisplayText("\n\nYou sashay yourself towards the prone lizan, straddling [Arian eir] legs and reaching out to grasp [Arian eir] ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("primary ");
		DisplayText("cock.  Slowly and gently you begin to stroke its strange, knobbly surface, your fingers eliciting moans and gasps from Arian as [Arian ey] shudders under your touch.  Precum begins to ooze from the reptilian prick's head, and you purposefully rub it into [Arian eir] shaft as a kind of makeshift lubricant.");

		DisplayText("\n\n\"<i>Ohm... T-this feels great, [name], b-but if you keep this up, I won't be able to hold back!</i>\" Arian pants, [Arian eir] shaft");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
		DisplayText(" throbbing against your hands.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) {
			DisplayText("  [Arian Eir] other shaft twiches and sprays your ");
			if (player.torso.tailType === TailType.NONE) DisplayText("back");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("stroking [Arian eir] other cock and sliding it between your ass cheeks, ");
		DisplayText("caressing the base of [Arian eir] tail.  With a devious grin, you begin to gently but insistently slide your finger into [Arian eir] ");
		if (Flags.list[FlagEnum.ARIAN_ANAL_XP] > 66) DisplayText("eager ");
		DisplayText("ass, probing for [Arian eir] prostrate...");

		DisplayText("\n\nThe reaction is almost instantaneous; Arian moans into your kiss and you feel [Arian eir] ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("pair of lizan-cocks practically jump");
		else DisplayText("lizan-cock practically jump");
		DisplayText(" back to full mast, even surprising you with Arian's eagerness.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("sliding [Arian eir] neglected prick through your ass-cheeks, ");
		DisplayText("clenching your inner walls to grip and feel the excitingly unusual lumpy, bulbous texture of [Arian eir] shaft inside you.  You moan and gasp, telling [Arian em] that [Arian ey] feels so unique, and yet [Arian ey] stimulates you so wonderfully.");
		...
		if (player.torso.vaginas.count > 0) DisplayText("cunt dripping with arousal");
		if (player.torso.cocks.count > 0 && player.torso.vaginas.count > 0) DisplayText(" and ");
		if (player.torso.cocks.count > 0) {
			DisplayText("cock");
			if (player.torso.cocks.count > 1) DisplayText("s");
			DisplayText(" drooling precum onto [Arian eir] belly");
		}
		DisplayText(".  You're so close now, you can feel it.  You tell Arian you're going to cum, and beg [Arian em] to cum with you.");

		DisplayText("\n\nAs if on cue, Arian gasps and [Arian eir] shaft");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s twitch");
		else DisplayText(" twitches");
		DisplayText(", signaling [Arian eir] oncoming orgasm.  [Arian Ey] grabs the sheets on [Arian eir] bed and curls [Arian eir] toes.  \"<i>...[name]!</i>\"  Jet after jet of lizan spunk paints your bowels white");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText(", as well as your butt");
		DisplayText(", as Arian climaxes");
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText(", her own pussy clenching and drizzling juices on the matted sheets beneath the two of you.");
		else DisplayText(".");
		...
		if (player.torso.cocks.count > 0) DisplayText("  [EachCock] spasms and shoots rope after rope of cum, painting Arian's [arian chest] white, as well as the groaning lizan's face.");
		if (player.torso.vaginas.count > 0) DisplayText("  Your [pussy], though unattended, joins in the perverted display, leaking copious amount of fluids on top of the prone lizan.");
	...
	private suckAriansDick() {
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 3) {
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("exposed cocks hardening to full mast");
			else DisplayText("exposed cock hardening to full mast");
		}
		else {
			DisplayText("cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s emerging from their");
			else DisplayText(" emerging from its");
			DisplayText(" hiding place, already fully erect");
			DisplayText(".");
		}
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("uppermost ");
		DisplayText("cock, marveling at the strange textures of its bumpy, knobbly, bulbous surface.  It feels so strange to you, and you continue to stroke it eagerly.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("twin ");
		DisplayText("shaft");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s tremble");
		else DisplayText(" trembles");
		DisplayText(" and throb");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] === 0) DisplayText("s");
		DisplayText(" against your hand; pre quickly forming on the tip");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
		DisplayText(" only to slowly slide down the bulbous surface of Arian's lizan prick");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
		DisplayText(".  You continue to slide your hand along the increasingly slick surface, playfully asking if your hand really feels that good to [Arian em]?");
		...
		DisplayText(" one more time and erupts into your waiting mouth");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText(" and chin");
		DisplayText(".  [Arian Eir] legs close around your back, effectively keeping you from moving too far away as [Arian ey] finishes giving you all of [Arian eir] load.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText(", oblivious to the second cock spewing cum all over your [chest] in your eagerness");
		DisplayText(".  After [Arian ey] finally finishes, you continue to lick and suckle for a few minutes longer, making sure you've lapped up every last drop.  Then and then alone do you lift your head, flirtatiously licking your lips to tease the now-spent lizan.");
		...
		DisplayText(" rolling your neck in pleasure as [Arian ey] rubs it with [Arian eir] reptilian tail.");
		...
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText(" and tickle her cunt with a finger");
		DisplayText(" for emphasis, kiss [Arian em] gently on the tip of [Arian eir] nose, and then get your clothes together, planning on getting dressed and heading back out again.");
	...
	private getPenetratedByArianAndHisHitlerMustache() {
		...
		DisplayText("; then you turn to look at Arian and ask [Arian em] how would [Arian ey] feel about giving [Arian eir] ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("twin cocks");
		else DisplayText("cock");
		DisplayText(" a bit of a workout?");
		...
		if (player.torso.hips.legs.isBiped()) DisplayText("spread your [legs] and ");
		DisplayText("expose your [vagina] to [Arian eir] viewing pleasure.  The reaction is almost instantaneous; Arian's ");
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] >= 3) DisplayText("exposed");
		else DisplayText("hidden");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText(" twin shafts immediately fill");
		else DisplayText(" shaft immediately fills");
		DisplayText(" with blood, growing fully erect in ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("their");
		else DisplayText("its");
		DisplayText(" bulbous, throbbing glory.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
		DisplayText(" and get ready to thrust in; otherwise neither of you are going to be feeling good any time soon.");

		DisplayText("\n\nSnapping to [Arian eir] senses, Arian quickly ");
		DisplayText("kneels between your [legs]");
		DisplayText(", aligning the tip of ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("each of ");
		DisplayText(arianMF("his", "her") + " ");
		DisplayText("shaft");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
		DisplayText(" with your [vagina]");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText(" and [asshole]");
		DisplayText(".  [Arian Ey] looks at you, waiting for you to confirm that [Arian ey] should indeed get going.  You smile and nod");
		if (player.hasLongTail()) DisplayText(", looping your tail around [Arian eir] waist");
		DisplayText(".");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) {
			...
			if (player.hasLongTail()) DisplayText(", further encouraging [Arian em] to do so, by looping your tail around [Arian eir] waist");
			DisplayText(".  The lizan doesn't need much encouragement to comply, easily snuggling against you, breast to breast, and sighing softly.  \"<i>I'm sorry.  Sometimes I wonder why you bother doing something like this, with someone like me,</i>\" [Arian ey] says, a hint of sadness in [Arian eir] voice.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("twin bulbous shafts feel");
		else DisplayText("bulbous shaft feels");
		DisplayText(" exquisite, and ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("their");
		else DisplayText("its");
		DisplayText(" bumps massage your entrance");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
		DisplayText(" unlike anything else.  You praise the lizan on being a quick study, letting go of [Arian eir] tail and grabbing onto [Arian eir] scaly shoulders instead, giving [Arian em] a few more directions so [Arian ey] can catch your most sensitive spots.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("and [ass] ");
		DisplayText("with [Arian eir] sticky wet seed.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
		DisplayText(" swelling inside you, the lizan grabbing your shoulders and starting to thrust [Arian em]self back into you once more.");
		...
		if (Flags.list[FlagEnum.ARIAN_VAGINA] === 0) DisplayText("[Arian ey] himself ");
		else DisplayText("she ");
		DisplayText("is being overwhelmed by, spraying your ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] === 0) DisplayText("cavity");
		else DisplayText("cavities");
		DisplayText(" with a second helping of lizan spunk, dredging up every last drop of jizz left in [Arian eir] internal balls before, with a groan, [Arian ey] sinks down atop you.  Arian's orgasm pushes you over the edge, and you find your pussy ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("and ass ");
		DisplayText("contracting, trying ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] === 0) DisplayText("its");
		else DisplayText("their");
		DisplayText(" best to milk the poor lizan of all [Arian ey] is worth, until finally with one last spasm, you slump down and release the lizan shaft");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
		DisplayText(" deeply embedded inside you, some of your mixed juices running down to mat Arian's bed.");
	...
	private doublePenetrateArian() {
		...
		if (Flags.list[FlagEnum.ARIAN_ANAL_XP] <= 33) DisplayText("tight");
		else if (Flags.list[FlagEnum.ARIAN_ANAL_XP] <= 66) DisplayText("loose");
		else DisplayText("throbbing");
		DisplayText(" ass and dripping wet vagina; a pair of clawed hands reach behind to spread her butt open for you.  \"<i>Okay, I'm ready.</i>\"  Arian says, looking behind at you.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_PENETRATION_CHAT] === 0) {
			...
			if (!arianFollower()) DisplayText("bedchambers");
			else DisplayText("tent");
			DisplayText(".");
		}
		else {
	...
	private arianDocking() {
		...
		//(if ArianDblCock === 0)
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] === 0) {
			DisplayText("didn't [Arian ey] always want to have two dicks?  Well, now it's [Arian eir] chance to find out what it'd be like.");
		}
		else {
			DisplayText("[Arian ey]'s already hiding two in there... an extra cock won't make such a huge difference, would it?");
		}
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("pair of ");
		DisplayText("pecker");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
		DisplayText(" from your prying eyes.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
		DisplayText(" of Arian's dick");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
		DisplayText(".  You brush your fingers against the tip");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
		DisplayText(", seeing what kind of reaction that will elicit from your scaly lover.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] === 0) DisplayText(" brushing up against [Arian eir] own lizan pecker");
		else DisplayText("sandwiched between [Arian eir] two lizard dicks");
		DisplayText(", the friction is absolutely incredible, sending sparks of pleasure cascading along your shaft.  You moan in delicious lust and tell Arian that this is absolutely incredible.");
		...
		DisplayText("slides across your intruding shaft as you go, and [Arian eir] sheathe-walls grip and squeeze as best they can, leaving you moaning and panting like a bitch in heat.  Arian's moans of pleasure join yours, [Arian eir] twitching bulbous shaft");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
		DisplayText(" out before cumming.");
		...
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("pair of cummy masts emerge");
		else DisplayText("cummy mast emerges");
		DisplayText(" to feebly shoot a couple more ropes of jism, before returning limply to ");
		if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("their");
		else DisplayText("its");
		DisplayText(" hiding place.  \"<i>Ahhh,</i>\" is all Arian says, before slumping down.");
	...
	private giveIncubusDraftToArian() {
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) DisplayText("if [Arian ey] would like to make [Arian eir] cock bigger");
		else DisplayText("if she'd like to have a touch of her old malehood back");
		DisplayText(".");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] === 0) {
			...
			if (player.torso.vaginas.count > 0) DisplayText("[vagina] and ");
			DisplayText("[asshole].");
			...
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] === 0) DisplayText("shaft emerges from its hiding place.  It throbs and grows, settling in its");
			else DisplayText("pair of shafts emerge from their hiding place.  They throb and grow, settling in their");
			DisplayText(" new size as Arian moans and cums all over [Arian em]self.");
			...
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s back into their");
			else DisplayText(" back into its");
			DisplayText(" slit.");
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] === 2) {
				...
				if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("them");
				else DisplayText("it");
				DisplayText(" until two thirds are inside, then it doesn't looks like [Arian ey] can take in anymore.");

				if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("\n\n\"<i>W-Wow, they're so big I can't even manage to hide them,</i>\" Arian pants as the tip of [Arian eir] cocks hang limply, exposed for all to see.");
				...
				if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) {
					...
					if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
					DisplayText(" starts hardening, no longer able to hide Arian's excitement inside her protective slit.");
				}
				else {
	...
	private succubiMilkForArian() {
		...
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText("further ");
		DisplayText("in touch with [Arian eir] feminine side.  For you?");
		...
		if (Flags.list[FlagEnum.ARIAN_VAGINA] === 0) {
			...
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
			DisplayText(" having already softened and retracted");
			//if ArianCockSize >= 3:
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] === 3) DisplayText(" as far as it'll go");
			DisplayText(" into its protective slit.");
			...
			if (Flags.list[FlagEnum.ARIAN_BREASTS] === 2) {
				...
				if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] >= 3) DisplayText(" and her exposed shaft begins to grow hard");
				DisplayText(".");
			}
			else {
				...
				if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] === 3) DisplayText(" and you see her exposed shaft slowly rising to point at you");
				DisplayText(".");
		...
		//(else If ArianCockSize > 1) //Dick shrinkage.
			...
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
			DisplayText(".  \"<i>Oh... that's good.  Right?</i>\" Arian looks at you nervously.  You think it over, and confess that while you are more attracted to women than men, you won't force Arian to become fully female if she doesn't want to.");
	...
	private giveArianLactaid() {
		...
		if (Flags.list[FlagEnum.ARIAN_BREASTS] === 0) {
			...
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] === 3) DisplayText(", her exposed cock growing hard as she flushes with arousal");
			DisplayText(".  Her hands grip the sheets and you watch her ");
			if (Flags.list[FlagEnum.ARIAN_BREASTS] === 1) DisplayText("perky");
			else if (Flags.list[FlagEnum.ARIAN_BREASTS] === 2) DisplayText("luscious");
			else DisplayText("pillowy");
			DisplayText(" breasts grow before your eyes.  \"<i>My breasts... d-do something, [name]!</i>\" Arian pleads.  Unsure of what you should be doing, you grab her breasts and begin kneading them as they grow in your hands; her erect nipples poking your palms.  \"<i>Ah... that feels good!  Don't stop!</i>\"");
	...
	private useReductoOnAriansBreasts() {
		...
		if (Flags.list[FlagEnum.ARIAN_BREASTS] === 1) DisplayText("small");
		else if (Flags.list[FlagEnum.ARIAN_BREASTS] === 2) DisplayText("generous");
		else DisplayText("pillowy");
		DisplayText(" breasts.  \"<i>Okay... I suppose it would be good to lose a bit of weight up here and spare my back.</i>\"  Arian smiles, opening the tube of Reducto and her robes; then squeezing the contents of the paste her hands.");
		...
		if (Flags.list[FlagEnum.ARIAN_BREASTS] > 1) {
			...
			DisplayText("The two of you watch Arian's breasts, but nothing seems to be changing.  You reach forward to feel them, and indeed they haven't shrunk down at all.");
	...
	private useReductoOnArianCocks() {
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] === 3) {
			...
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("pair of exposed lizard cocks");
			else DisplayText("exposed lizard cock");
			DisplayText(" into full erection.  Finally [Arian ey] begins applying the paste.");

			DisplayText("\n\nThe changes are almost immediate; Arian groans and watches as [Arian eir] oversized dick");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
			DisplayText(" slowly shrink");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] === 0) DisplayText("s");
			DisplayText(" to a more manageable size.  Once the transformation is complete Arian tries to fully retract [Arian eir] cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s back into their");
			else DisplayText(" back into its");
			DisplayText(" hiding place; it's a tight fit, but it fits.");
			...
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s from their tight hiding place");
			else DisplayText(" from its tight hiding place");
			DisplayText(", and into full erection.  Finally [Arian ey] begins applying the Reducto.");

			DisplayText("\n\nIt takes mere moments for the changes to occur; Arian groans in slight discomfort as [Arian eir] cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s return to their");
			else DisplayText(" returns to its");
			DisplayText(" original size.  Once the transformation is complete, [Arian ey] retracts ");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("them");
			else DisplayText("it");
			DisplayText(" back into [Arian eir] genital slit.");
			...
			DisplayText("Then, [Arian ey] coaxes [Arian eir] twin reptilian-peckers out of their hiding place.  Finally [Arian ey] applies the paste to both shafts in turn.");
	...
	private useReductoOnAriansAsshole() {
		...
		DisplayText(" does as you ask, allowing you to smear your fingers generously with the shrinking cream, even as [Arian ey] anticipates your next request and removes [Arian eir] clothes before lying down, bum up in the air and tail slowly swishing to and fro, [Arian eir] anus exposed and waiting for your ministrations.");
		//(if AnalXP === 1)
		...
		//(else if AnalXP < 33)
			...
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
				DisplayText(", [Arian eir] cock");
				if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s suddenly jolting from their sheathe");
				else DisplayText(" suddenly jolting from its sheathe");
				if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText(" and ");
			}
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) {
				if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] === 0) DisplayText(", ");
				DisplayText("her pussy growing wet from the stimulation");
			}
	...
	private giveArianReptilum() {
		...
		if (player.torso.vaginas.count <= 0 && player.torso.cocks.count <= 0) DisplayText("genderless crotch");
		DisplayText(".  \"<i>Hmm, that looks much tastier than candy actually.</i>\"  [Arian Ey] licks your ");
		if (player.torso.cocks.count > 0) DisplayText("cock from base to tip");
		else if (player.torso.vaginas.count > 0) DisplayText("pussy all the way from the back to the tip of your [clit]");
		else DisplayText("your sweaty crotch");
		...
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText("slavering vagina");
		DisplayText(".");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			DisplayText("shooting thick ropes of cum up into the air to land on [Arian eir] ");
			if (Flags.list[FlagEnum.ARIAN_BREASTS] === 0) DisplayText("chest");
			else if (Flags.list[FlagEnum.ARIAN_BREASTS] === 1) DisplayText("perky little breasts");
			else if (Flags.list[FlagEnum.ARIAN_BREASTS] === 2) DisplayText("soft breasts");
			else DisplayText("pillowy breasts");
			if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText(" while ");
		}
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText("her vagina doing its best to soak her bed with lizan femcum");
		DisplayText(".");
		...
		if (randInt(2) === 0 && Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] === 0 && Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
	...
	private treatCorruption() {
		...
		if (Flags.list[FlagEnum.ARIAN_TREATMENT] === 1) {
			...
			player.stats.cor += -1;
			fatigue(20);
			if (player.stats.cor === 0) DisplayText(" - in fact, you're quite sure that Arian has purified you entirely");
			DisplayText(".  You thank the lizan for [Arian eir] magical treatment.");
	...
	private imbueTalisman() {
		...
		else DisplayText("change the spell in the talisman [Arian ey] created for you");
	...
	private arianSpellPlace(spell: string) {
		...
		if (spell === "Shielding Spell") {
			player.statusAffects.add(StatusAffectType.ShieldingSpell, 0, 0, 0, 0);
			//Shielding Spell: 2x Black Chitin and 1x Tough Silk. - Increases defense for the duration of the battle.
		...
		if (spell === "Immolation Spell") {
			player.statusAffects.add(StatusAffectType.ImmolationSpell, 0, 0, 0, 0);
			//Immolation Spell: 2x Goblin Ale and 1x Sweet Gossamer. - Deals damage over time.
	...
	*/
	...
	public sleepWithArian(newl: boolean = false) {
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			DisplayText("You can clearly see [Arian eir] exposed cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
			DisplayText(" stiffening at the sight.  ");
		}
		if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText("You can tell that [Arian ey] is aroused by the little show, despite [Arian eir] body doing a good job of hiding it.  ");
		DisplayText("You just let [Arian em] squirm, pretending you don't see [Arian eir] reactions, and then, indicating the bed, you ask which of you should get in first.");
	...
	private listenToMediumAnalXPArian() {
		...
		if (player.torso.cocks.count > 0) {
			...
			DisplayText(" still worming its way into [Arian eir] fuckable ass, but it really does look like [Arian ey]'s fast asleep.  Weird.  \"<i>So many muffins... all [name] flavored...</i>\"  [Arian ey] calms down and begins snoring lightly.");
			...
			if (player.stats.cor >= 66) DisplayText(", despite the silly lizan being deserving of it");
			DisplayText(".  But as payback, [Arian ey]'s going to sleep with you inside, not that it seems like it'll be much of a problem to [Arian em]... Too bad you can't hold an erection all the time while asleep, so you know eventually you'll slip out...");
		}
		else {
	...
	public wakeUpAfterArianSleep() {
		...
		if (player.torso.cocks.count > 0) {
			...
			if (Flags.list[FlagEnum.ARIAN_ANAL_XP] < 33) {
				...
				DisplayText("Your eyes open, and you suddenly realize that whatever dreams you were having... are now gone.  You make note of the still sleeping lizan right next to you and recall last night's events.  You wonder if Arian will remember any of that...");
				...
				if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) {
					DisplayText(arianMF("his", "her") + " pussy is squirting her juices all over your lower body");
					if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
						DisplayText(" and ");
					}
					else DisplayText("; ");
					...
					DisplayText(arianMF("his", "her") + " ");
					if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("twin shafts are");
					else DisplayText("shaft is");
					DisplayText(" squirting white ropes of lizan cum all over the bedsheets;");
				}
				DisplayText(" what a naughty little lizan.");

				DisplayText("\n\nYou can't resist playfully teasing Arian that [Arian ey] is such a buttslut.  ");
				if (player.torso.vaginas.count <= 0) DisplayText("Did [Arian ey] always know [Arian ey] wanted a man to top [Arian em], or is this development new to [Arian em]?");
				else DisplayText("Does [Arian ey] really get off on having a girl with a dick all her own fuck [Arian em] like a girl, hmm?");
			...
			if (Flags.list[FlagEnum.ARIAN_ANAL_XP] < 33) {
				...
				if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
					DisplayText(arianMF("his", "her") + " ");
					if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] >= 3) DisplayText("exposed ");
					DisplayText("bulbous shaft");
					if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s begin");
					else DisplayText(" begins");
					DisplayText(" sporting an erection");
					if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] < 3) {
						DisplayText(", poking ");
						if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] === 1) DisplayText("its tip");
						else DisplayText("their tips");
						DisplayText(" out of Arian's genital slit");
					}
					DisplayText(".");
				}
				else if (Flags.list[FlagEnum.ARIAN_VAGINA] > 0) DisplayText("and her moist pussy, begins forming a stream of juices that gently run its course through the inside of Arian's thighs, evaporating before hitting the floor due to the lizan's currently elevated body heat.");
				DisplayText("  [Arian Ey] pants, eyes glazed.  \"<i>Uhh... [name]...</i>\"");
	...
	public arianEggingEvent() {
		...
		if (Flags.list[FlagEnum.ARIAN_EGG_CHAT] === 0) {
			...
			DisplayText("As you are about to enter Arian's tent, you hear a moan emanate from within.  Those aren't moans of pleasure though.  They are moans of discomfort.  Wondering if the lizan is all right, you decide to enter her tent.");
			...
			DisplayText("As you are about to enter Arian's tent, you hear a moan emanate from within.  Seems like it's that time of the month again...");
	...
	public arianLaysEggs() {
		...
		DisplayText("As you approach the tent, you wonder if Arian's ready to lay those eggs...  Your contemplations are promptly interrupted as you hear a pained moan coming from inside.  Seems like Arian is in labor!  Without delay, you rush inside, to be greeted by the sight of naked lizan-");
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) DisplayText("herm");
		else DisplayText("girl");
		DisplayText(" squatting over a wooden basin, one hand between her legs, massaging her dripping treasure, while the other is rubbing her belly to try and coax the eggs out of her.");
	...
	private arianDildoFun() {
		...
		if (Flags.list[FlagEnum.TIMES_ARIAN_DILDOED] === 0) {
			DisplayText("Looking over your reptilian girlfriend, your eyes go down her body");
			if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
				DisplayText(", past her cock");
				if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
			}
			else DisplayText(",");
			DisplayText(" to her dripping wet pussy, clearly yearning for some action.  Reflecting that your own love-lips feel the same way, you decide to try something fun and present to Arian the dildo that you ");
			if (player.hasKeyItem("Dildo") >= 0) DisplayText("bought from Giacomo");
			else DisplayText("got from Tamani");
			DisplayText(", asking if she's willing to give this a try.");
			...
			DisplayText("You present your dildo to Arian again, telling her that this time, you want to play with your toys - after all, she enjoyed it so much the first time.");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			DisplayText("Ignoring her cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
			DisplayText(", y");
		}
		else DisplayText("Y");
		DisplayText("ou slide first one finger into her slick pussy and then another, gently caressing her inner labia to see if you can find her sweet spots.  Arian's eyes open in surprise as the first pang of pleasure hits her, she moans into your kiss.  Her tongue darts to dance with your own in excitement.");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			DisplayText("brushing it tantalizingly across the sensitive skin of her pre-oozing cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s");
			DisplayText(" before");
		}
		DisplayText(" brushing it against her outer lips, stroking it up and down each labia, rubbing it against her clitoris, gently pushing the very tip of it into her cunt only to then withdraw it before it can really start to stretch her cunt.");
		...
		if (Flags.list[FlagEnum.ARIAN_COCK_SIZE] > 0) {
			DisplayText("Her cock");
			if (Flags.list[FlagEnum.ARIAN_DOUBLE_COCK] > 0) DisplayText("s throb");
			else DisplayText("throbs");
			DisplayText(" throb as a jet of whiteness escape them.  ");
		}
		DisplayText("You can feel it... the suddenness of your penetration brought the poor lizan mage to an instant orgasm.");
