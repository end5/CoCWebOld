export const MarblePurificationFlags = {
MARBLE_PURIFICATION_STAGE: 0,
MARBLE_BOVA_LEVEL: 0,
UNKNOWN_FLAG_NUMBER_00238: 0,
TIMES_GIVEN_MARBLE_PURE_LABOVA: 0,
MARBLE_RATHAZUL_COUNTER_1: 0,
MARBLE_RATHAZUL_COUNTER_2: 0,
SLEEP_WITH: 0,
MURBLE_TEA_DRINKER_COUNT: 0,
MARBLE_DICK_TYPE: 0,
MARBLE_KIDS: 0,
CLARA_PURIFIED: 0,
CLARA_IMPRISONED: 0,
MARBLE_PURIFIED: 0,
MARBLE_BREAST_SIZE: 0,
MARBLE_LUST: 0,
MARBLE_TIME_SINCE_NURSED_IN_HOURS: 0,
MARBLE_WARNED_ABOUT_CORRUPTION: 0,
MARBLE_LEFT_OVER_CORRUPTION: 0,
FOLLOWER_AT_FARM_MARBLE: 0,
};
User.flags.set(FlagType.MarblePurification, MarblePurificationFlags);
...
/*
MarblePurificationFlags.MARBLE_PURIFICATION_STAGE : 0 = unstarted
MarblePurificationFlags.MARBLE_PURIFICATION_STAGE : 1 = Started, gathering labova
MarblePurificationFlags.MARBLE_PURIFICATION_STAGE : 2 = Potions gathered, need to talk to Rathazul
MarblePurificationFlags.MARBLE_PURIFICATION_STAGE : 3 = Started wiv Ratazul
MarblePurificationFlags.MARBLE_PURIFICATION_STAGE : 4 = Marble is out getting her sister
MarblePurificationFlags.MARBLE_PURIFICATION_STAGE : 5 = QUEST COMPLETE
*/
	...
	//The PC must have the Marble Resistant perk, must not have the Marble’s Milk perk to trigger this scene, they must have below 50 corruption, and they cannot have any corrupt followers in   If at any time the PC's corruption goes over 50, or they gain a corrupt follower, the quest is cancelled.  The quest can be continued if they once again drop under 50 corruption and lose all corrupt followers.
	export function BLUHBLUH(player: Character): NextScreenChoices {
		CView.clear();
	 	CView.text("When you return to camp, you find Marble seated at the campfire, staring gloomily into the ashes. Curious, you approach your bovine lover and ask her what the matter is. She looks up at you, her ");
		if(MarblePurificationFlags.MARBLE_BOVA_LEVEL <= 1) CView.text("human");
		else CView.text("bovine");
	 	CView.text(" head still in her hands, and sighs loudly. <i>\"I’m sorry, sweetie, I just... I’ve tried to ignore it, but I just don’t feel comfortable here.\"</i>");
	 	CView.text("\n\nYou ask her what’s wrong; doesn’t she like being with you?");
		if(amilyScene.amilyFollower() && !amilyScene.amilyCorrupt() || followerKiha() || MarblePurificationFlags.UNKNOWN_FLAG_NUMBER_00238 == 1)
		{
		 	CView.text("  ...Has she been fighting with ");
			if(amilyScene.amilyFollower() && !amilyScene.amilyCorrupt()) CView.text("Amily");
			else if(followerKiha()) CView.text("Kiha");
			else CView.text("Izma");
		 	CView.text("?");
		}
		...
		if(player.inventory.items.has(ConsumableName.BeeHoneyPure)) CView.text("  You then recall that bottle of honey the bee-girl gave you, and ask if maybe it might work?");
		if(player.effects.has(StatusEffectType.PureCampJojo)) CView.text("  You remember how Jojo helped you become pure, and suggest maybe his meditations could assist Marble?");
		...
		MarblePurificationFlags.MARBLE_PURIFICATION_STAGE = 1;
	...
	export function giveMarblePureQuestLabova(player: Character): NextScreenChoices
	{
		CView.clear();
		player.inventory.items.consumeItem(ConsumableName.LaBovaPure);
	 	CView.text("You tell Marble that you managed to get some LaBova alchemically purified, and you want her to drink some.");
		//First time
		if(MarblePurificationFlags.TIMES_GIVEN_MARBLE_PURE_LABOVA == 0)
		{
			//Marble’s corruption is reduced by 4
			player.effects.get(StatusEffectType.Marble).value4 += -4 ;
		...
		else if(player.effects.get(StatusEffectType.MarbleSpecials).value4 <= 30)
		{
		 	...
		 	MarblePurificationFlags.MARBLE_PURIFICATION_STAGE = 2;
		}
		//Every other time
		else
		{
			...
			if(MarblePurificationFlags.MARBLE_BOVA_LEVEL > 1) CView.text(", fur covered");
		 	CView.text(" breasts to dangle freely. Stoically she bears the pain as her stomach loudly rumbles and groans, holding her breasts up proudly as her ");
			if(player.effects.get(StatusEffectType.MarbleSpecials).value2 == 4) CView.text("eight");
			else CView.text("two");
		 	CView.text(" nipples begin spray milk everywhere - you have to take a step back to avoid being squirted yourself. Finally, Marble’s breasts stop their spraying, leaving the two of you standing in a milky puddle of mud. The cowgirl pulls her shirt back on over her body. \"<i>That stuff still tastes vile, but I guess it’s worth it. I just hope it finishes working soon.</i>\" she tells you. You assure her that you believe in her ability to get through this, and then head back to the main part of your ");
		}
		MarblePurificationFlags.TIMES_GIVEN_MARBLE_PURE_LABOVA++;
		return { next: returnToCampUseOneHour };
	}
	...
	export function visitRathazulToPurifyMarbleAfterLaBovaStopsWorkin(player: Character): NextScreenChoices
	{
		CView.clear();
		//if (Rathazul is not in camp)
		if(!player.effects.has(StatusEffectType.CampRathazul))
		{
		 	CView.text("You spot the elderly alchemist's camp on the lake once more.\n\n");
		}
	 	CView.text("As you approach the rodentine alchemist, you remember Marble’s problems and resolve to ask him if he can possibly help you. Once he finishes greeting you, you politely return the greeting and ask if he would be willing to lend his opinion on a problem you have.");
		...
		//if (Rathazul is not in camp)
		//the quest is written under the assumption that Rath is in your   I recently found out it is possible to lock him out of being a follower, so I added in this section to ensure that he is.
		if(!player.effects.has(StatusEffectType.CampRathazul))
		{
		 	CView.text("\n\n\"<i>Whoa, hold on there, " + mf(player, "boy", "girl") + ",</i>\" Rathazul stops you, \"<i>Hearing about your quest for purification makes my heart feel warm again for the first time in years.  In exchange for helping you out in this endeavor, please allow me stay at your   I'm sure you'd appreciate saving yourself the trouble of going back and forth for each little thing.</i>\"");
		 	CView.text("\n\nConsidering just how important his help has been in purifying Marble, you doubt that you'd be able to get any further without his help, and agree to let him stay in your ");
		 	CView.text("\n\n<i>\"Then let me gather my things, and you can show me your \"</i>  This takes very little time, and in very short order the two of you have arrived in your   \"<i>I'll set my stuff up while you go talk to your mate.</i>\"");
			//Rathazul is set as a follower
			player.effects.add(StatusEffectType.CampRathazul, 0, 0, 0, 0);
		}
		//{Next}
		
		choices[0] = ["Next", pageTwoOfVisitingTheRatForMurblesPurification];
	}
	export function pageTwoOfVisitingTheRatForMurblesPurification(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("Fortunately, Marble is at your camp rather than off at Whitney’s, and your bovine lover looks up at you hopefully as you approach. \"<i>Did he say he could help?</i>\" she asks, excitedly.");
	 	...
	 	CView.text("\n\nYou thank him for his help, tell him that Marble will also appreciate it, and then calmly head back to  Behind you, Rathazul begins busying himself with the strange equipment he does his work with.");
	 	MarblePurificationFlags.MARBLE_PURIFICATION_STAGE = 3;
	 	MarblePurificationFlags.MARBLE_RATHAZUL_COUNTER_1 = 24;
	...
	export function rathazulsMurbelReport(player: Character): NextScreenChoices
	{
		CView.clear();
		MarblePurificationFlags.MARBLE_RATHAZUL_COUNTER_1 = 0;
	 	CView.text("When you wake and join Marble ");
		if(companionsCount() > 2) CView.text("and your other followers ");
	 	CView.text("for breakfast this morning, you find a bleary-eyed Rathazul waiting at the campfire. Marble clucks her tongue and gives him a mug of something strong, which he accepts gratefully, noisily slurping down a mouthful. He takes a seat, staring into his mug, and then sighs softly.");
	 	...
	 	CView.text("\n\n\"<i>Then that’s the solution!</i>\" Marble declares, full of cheer. But then her face falls. \"<i>But it’ll take me a few days to get from here to home, and then more to get back. " + player.desc.name  + ", I don’t want to leave you here alone if you need my help,</i>\" she insists.");
	 	CView.text("\n\nDo you tell her it’s okay, or tell her you’re not ready to send her away?");
		//[Go] [Stay]
		
		choices[0] = ["Go", murbleShouldGoRunAlongAndGetHerCuntySisterSoTheyCanBeCuntsTogether];
		choices[1] = ["Stay", stayHereAndDontGoGetYerSisterShesABitch];
	}

	//[=Stay=]
	export function stayHereAndDontGoGetYerSisterShesABitch(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("You shake your head and tell Marble that you still need her at the camp; you’ll tell her when you can spare her.");
	...
	//[=Go=]
	export function murbleShouldGoRunAlongAndGetHerCuntySisterSoTheyCanBeCuntsTogether(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("You tell Marble that if she needs some time off to go and see her family for this matter, it’s okay with you.");
		...
		MarblePurificationFlags.MARBLE_PURIFICATION_STAGE = 4;
		MarblePurificationFlags.MARBLE_RATHAZUL_COUNTER_2 = 240;
		MarblePurificationFlags.SLEEP_WITH = "";
		return { next: returnToCampUseOneHour };
	}
	...
	export function claraShowsUpInCampBECAUSESHESACUNT(player: Character): NextScreenChoices
	{
		CView.clear();
		MarblePurificationFlags.MARBLE_RATHAZUL_COUNTER_2 = 0;
	 	CView.text("As you get up, you hear the voices of two happy women chatting pleasantly to each other; you recognize one as Marble, but the other is a stranger to you. Whoever they are, they’re clearly getting along very well, and both sound quite excited.  <i>\"Morning sweetie!</i>\"  Marble excitedly exclaims at the sight of you coming out of your tent.  <i>\"Let me introduce you to my sister, Clara.</i>\"");
		...
		if(MarblePurificationFlags.MARBLE_BOVA_LEVEL <= 1) CView.text("your mate has");
		else CView.text("what your mate use to have");
	 	CView.text("; cow legs, ears, horns, and large breasts.  However, she doesn’t have Marble’s brown fur, instead having black and white blotched fur across her legs and ears and a mane of white hair atop her head.  She is wearing a one piece thick cloth dress, with a great deal of fancy embroidery decorating it.  You also guess that she’s a bit smaller than Marble, in both the chest and height departments.");
		...
		if(player.stats.int >= 60) CView.text(" with a odd smile on her face");
		...
		if(player.stats.int >= 60) CView.text(" somewhat unconvincingly");
	 	CView.text(".");
	 	...
	 	CView.text("\n\nAt this point Rathazul suddenly interjects. \"<i>If " + player.desc.name  + " gets the milk by suckling on your breasts, " + mf(player, "he", "she") + " would probably contaminate it with " + mf(player, "his", "her") + " saliva and make it useless. I need it to be as pure as possible; hand-milking would be preferable, if you can do that, but if you must use the milkers at Whitney’s farm, that will suffice.</i>\"");

	 	CView.text("\n\nClara glares at the alchemist at this.  Then Marble shakes her head, before informing you all that, \"<i>Sorry, we Lacta Bovines can’t get milked by hand all that well, the milkers would work best.  Don’t worry, it shouldn’t take too long.</i>\"  She pulls on her younger sister’s shoulder again, and Clara gives one more stomp of her hoof before following her older sister out of the ");
		...
		choices[0] = ["Next", partTwoOfClaraShowingUpAndBeingACunt];
	}
	
	export function partTwoOfClaraShowingUpAndBeingACunt(player: Character): NextScreenChoices 
	{
		CView.clear();
	 	CView.text("You turn back to the camp and continue going about your morning routine.  By the time you’re ready to head out, the girls have come back from the farm, bearing a bottle of Clara’s milk.  The spotted bovine’s foul mood seems to have improved as well.");
		...
		if(companionsCount() >= 3)
		{
			...
			if(amilyScene.amilyFollower() && !amilyScene.amilyCorrupt())
			{
				//Amily’s response to being asked to have tea
			 	CView.text("\n\nWhen you approach Amily about having tea with Clara, she smiles brightly. \"<i>I haven’t enjoyed something like that in ages... it kind of reminds me of the old days.</i>\" She trails off, looking a little sad, but then shakes it off. \"<i>Sure, I’m </i>\"");
			...
			if (isabellaFollower())
			{
				...
				if (isabellaAccent())
				{
					CView.text("  Isabella jumps from her chair, “<i>Tea?  Ooh, ja!  I hear tea und milk iz sehr gut combination, ja?</i>”  Well, she seems rather enthusiastic if nothing else.");
				}
				else
				{
					CView.text("  Isabella jumps from her chair, “<i>Tea? Ooh, yes!  I hear tea and milk is a really good combination, yes? I've never had the opportunity to try it though...</i>”  Well, she seems rather enthusiastic if nothing else.");
				}
			...
			if(MarblePurificationFlags.UNKNOWN_FLAG_NUMBER_00238 == 1)
			{
			...
			if(player.effects.has(StatusEffectType.PureCampJojo))
			{
		...
		MarblePurificationFlags.MURBLE_TEA_DRINKER_COUNT = teaDrinkers;
		...
		choices[0] = ["Next", dumbshitCowCuntsTryToDrugYouBecauseTheyreCunts, teaDrinkers];
	}
	...
	export function dumbshitCowCuntsTryToDrugYouBecauseTheyreCunts(player: Character, teaDrinkers: number): NextScreenChoices
	{
		CView.clear();
	 	CView.text("Upon returning to the center of activity, you find a rather annoyed looking Marble sitting off to the side while Clara busies herself with a large number of cups from her pack.  Curious over what happened, you sit down next to Marble while watching Clara check the temperature of the pot.  After a moment, you ask your bovine lover why she is just watching.  \"<i>After I got the fire started, Clara insisted that I’d get in the way and ruin the tea if I tried to help,</i>\" she grumbles in annoyance.");
		...
		//new page
		if(player.stats.int >= 60) choices[0] = ["Next", highIntelligenceOutcomesVsCowCuntDrugs];
		else if(player.stats.int >= 30) choices[0] = ["Next", mediumIntelligenceReactsToDrugging];
		else choices[0] = ["Next", lowIntelligenceGetsDruggedByCowSluts];
	}
	...
	export function lowIntelligenceGetsDruggedByCowSluts(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("Once everyone has a cup, Clara smiles at all of you and holds her own cup up to her mouth and says, \"<i>To Marble’s purification and meeting new friends.</i>\"");
		...
		choices[0] = ["Next", partTwoOfLowIntelligencePCsFallingForTheIncrediblyObviousCowCuntsTrap];
		
	}
	export function partTwoOfLowIntelligencePCsFallingForTheIncrediblyObviousCowCuntsTrap(player: Character): NextScreenChoices
	{
		...
		if(MarblePurificationFlags.MARBLE_BOVA_LEVEL <= 1) CView.text("face");
		else CView.text("muzzle");
	 	CView.text(", you try to figure out what the heck is going on.  You remember meeting Marble’s sister, then getting tea, and then...  What did Clara say to Marble?  You groan slightly, your body feeling an odd old craving.");
		...
		if(player.inventory.equipment.weapon.displayName != "fists") CView.text("your [weapon] is nearby, and ");
	 	CView.text(" you’re still wearing your [armor].");
		//PC chooses fight or submit.
		
		choices[0] = ["Fight", fightCowCuntAfterBeingTotesMcGoatsDrugged];
		choices[1] = ["Submit", voluntarilyChooseCowCuntBadEndYouBad];
	}
	
	//Chose fight
	export function fightCowCuntAfterBeingTotesMcGoatsDrugged(player: Character): NextScreenChoices {
		CView.clear();
	 	CView.text("You steel yourself, ");
		if(player.inventory.equipment.weapon.displayName == "fists") CView.text("raise your fists");
		else CView.text("grab your " + player.inventory.equipment.weapon.displayName);
	 	CView.text(" and get ready to fight to free yourself from this mad milkmaid’s mammaries.  Realizing your intentions, Clara’s face turns from a crazed grin to an angry glare.  With barely contained fury, she screams at you, \"<i>Oh don’t you worry darling, I will <b>make</b> you love drinking my milk!</i>\" before standing up, hefting a nasty looking heavy mace, and charging at you.");
		//go to Clara’s fight
		return startCombatWithMarbleSister(player, false);


	}
	export function startCombatWithMarbleSister(player: Character, inCamp: boolean = true): NextScreenChoices
	{
		return CombatManager.beginBattle(player, new Clara());
		player.effects.add(StatusEffectType.NoFlee,0,0,0,0);
		if(inCamp && !player.effects.has(StatusEffectType.ClaraFoughtInCamp)) player.effects.add(StatusEffectType.ClaraFoughtInCamp,0,0,0,0);
	}
	//Chose submit
	export function voluntarilyChooseCowCuntBadEndYouBad(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("Giving into the desires that your addiction has granted you, you return to Clara’s breast, much to the young cow-girl’s moans and sighs of pleasure.  \"<i>Ahh, yes, that's a goood " + mf(player, "boy", "girl") + ",</i>\" is whispered in your ear.");
		//go to Clara’s bad end
		
		choices[0] = ["Next", loseToClara, false];
	}
	...
	export function mediumIntelligenceReactsToDrugging(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("While Clara is handing out the cups, you can’t help but think how strange everything about this feels.  You can’t put your finger on what exactly is wrong, but neither can you put the unsettling feeling out of your mind.");
		...
		choices[0] = ["Drink Tea", lowIntelligenceGetsDruggedByCowSluts];
		choices[1] = ["Don't", mediumIntelligenceDoesntDrinkCowCuntsTea];
	...
	export function mediumIntelligenceDoesntDrinkCowCuntsTea(player: Character): NextScreenChoices
	{
		...
		choices[0] = ["Fight", mediumIntelligencesFightCowCunts];
		choices[1] = ["Submit", mediumIntelligencesSubmitCowCunts];
	}

	//Fight
	export function mediumIntelligencesFightCowCunts(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("You ready yourself for battle as Clara pulls an angry looking heavy mace out and says, \"<i>Fine, we’ll doo this the hard way.</i>\"");
		//start battle with Clara
		return startCombatWithMarbleSister(player, true);
	}
	//Submit
	export function mediumIntelligencesSubmitCowCunts(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("You sigh and accept the cup from the cow-girl’s hands and down its contents.  In moments, your vision clouds over, and you fall into her soft arms.  Just before everything goes black, you hear a voice say, \"<i>Come on now, lets run away toogether.  I’ll make sure you’re addicted before you next wake up.</i>\"");
		//go to bad end
	...
	export function highIntelligenceOutcomesVsCowCuntDrugs(player: Character, teaDrinkers: number = 0): NextScreenChoices
	{
		...
		choices[0] = ["Drink Tea", lowIntelligenceGetsDruggedByCowSluts];
		choices[1] = ["Don't Drink", mediumIntelligenceDoesntDrinkCowCuntsTea];
		choices[2] = ["CallHerOut", highIntelligenceCallClaraOut];
	}
	//Call Clara Out
	...
	export function highIntelligenceCallClaraOut(player: Character): NextScreenChoices
	{
		...
		//proceed to one of the following depending on whether or not a combat ready follower is in   Sorry for the trouble this is to code...
		...
		if(companionsCount() >= 3)
		{
			...
			if(followerKiha())
			{
		 	...
		 	CView.text("\n\nYou stand up as well and ready your " + player.inventory.equipment.weapon.displayName + ", since even with such opposition it looks like the mad cow-girl still plans on fighting you.  A few minutes and bruises later, her beaten body proves this to have been a bit of a mistake on her part.");

			//Proceed to after combat in 
			
			choices[0] = ["Next", defeatClaraCuntInAFight, true];
		}
	...
	export function loseToClara(player: Character, combat: boolean = true): NextScreenChoices
	{
		CView.clear();
		//PC loses via health	
		if(combat)
		{
			if(player.stats.HP < 1) CView.text("Your body gives out from the strain, and you fall to the ground.  The last thing you hear before you pass out, is Clara’s triumphant laughter.");
			//proceed to loss scene
			//PC loses via lust
			else CView.text("You just can’t fight it anymore, and desperately start to masturbate in front of Clara.  Laughing triumphantly, Clara walks up to you and forces a pill down your throat.  A moment later, you pass out.");
			CView.text("\n\n");
		}
		//proceed to loss scene
		...
		//PC wakes up laying down on a bed roll.  They feel very groggy and weak.  After a moment, they realize that they’ve got a strong craving for milk and that they need to drink it if they’re going to survive.  Clara then shows up and proceeds to demonstrate her control over the PC, taking great pleasure in it.
	 	...
	 	CView.text("You dream of odd things, distorted images that you called out for help to.  Then you were dragged away.  You kept seeing more shapes that felt familiar and important somehow, but you couldn’t focus on them.  You’d call again and again, only to be dragged away by something else again.  Eventually your cries for help become weaker and halfhearted; you know what’s going to happen next, so why bother?  Suddenly you realize that it is a cow-girl that is dragging you away; her face looks like Marble’s at first, then it splits into a huge, wicked smile, and you panic.");
		...
		if(player.body.tallness < 70) CView.text("much too big for you.");
		else if(player.body.tallness <= 82) CView.text("just your size.");
		else CView.text("way too small for your stature.");
	 	CView.text("  You also find that you aren’t wearing your " + player.inventory.equipment.armor.displayName + ", and your " + player.inventory.equipment.weapon.displayName + " is nowhere to be seen.  ");
		...
		if((player.body.cocks.length > 0 && player.body.cocks.sort(Cock.Smallest)[0].area <= 40) && player.body.chest.sort(BreastRow.Largest)[0].rating <= 25 && !player.body.legs.isTaur() && !player.body.legs.isNaga() && !player.body.legs.isGoo() && !player.body.legs.isDrider())
		 	CView.text("Instead a plain but rather slim form fitting two piece garb adorns your body.  Whoever dressed you in that definitely wanted your bodily assets to be fully visible to anyone looking at you.");
		else CView.text("You’re completely naked.");
		
		//new page
		
		choices[0] = ["Next", losingToCowCuntsPageII];
	}
	export function losingToCowCuntsPageII(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("It is then that you realize with a shock that a familiar urge is welling up inside you.  No, it is much stronger than you remember it being before.  You want breastmilk.  You <i>need</i> breastmilk, and if you don’t get it, you’ll die.  To make matters worse, you know whose breastmilk it is too.  Clara, Marble’s younger sister, has fatally addicted you to her milk.");
		...
		if(player.effects.get(StatusEffectType.Marble).value3 == 1)
		{
		 	CView.text("\"<i>Oh?  Wondering how I know about that?  Marble told me when she was explaining about you breaking free of the addiction.  Unlike her, I’m going to make sure you behave yourself.</i>\"  ");
		} 
		else
		{
		 	CView.text("\"<i>Confused?  That was one of the things Marble found out when she was researching the nature of her addiction.  She said she didn’t want to trouble you with that knowledge, but I don’t see why.</i>\"  ");
		}
	 	CView.text("This woman is nothing like her sister, she loves the power she has over you, and loves to show you just how much she has.");
		...
		if(loversCount() == 1) CView.text("Marble");
		else CView.text("the others");
	 	CView.text(".  Then your eyes drop down to Clara’s chest, and you think of your need.");
		...
		choices[0] = ["Next", cowCuntBadEndPartIII];
	}
	//Nursing from Clara
	...
	export function cowCuntBadEndPartIII(player: Character): NextScreenChoices
	{
		...
		if(player.body.hair.length > 0) CView.text("through your " + describeHair(player));
		else CView.text("across your scalp");
	 	CView.text(", then they suddenly grip down tight.  \"<i>Let’s see if you can pick up the pace.  Drink faster, my love slave!</i>\"  You do as the voice asks, and loud moans of pleasure follow.");
		...
		choices[0] = ["Next", cowCuntBadEndPartIV];
	}

	export function cowCuntBadEndPartIV(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("Now you get a chance to see a softer side to Clara.  The cow-girl gently takes your head and guides you to the other nipple.  She continues to hold you with one arm as you gently start drinking.  With her other hand, she caresses you.  She starts with your head, slowly cleaning up your " + describeHair(player) + ".  Your feeder then delivers a small pinch to your cheek, and you hear her say, \"<i>You really are a damn sexy piece of woork.</i>\"");
		...
		{
		 	CView.text("At this point your " + describeCocksLight(player) + " is rock hard, and Clara gives it a few strokes.  She does seem to lose interest surprisingly quick though.");
		}
		else if (player.body.vaginas.length > 0 && player.body.cocks.length <= 0)
		{
		 	CView.text("You’ve become quite wet at this point, and Clara gives your " + describeVagina(player, player.body.vaginas.get(0)) + " a few rubs, before tugging on your " + describeClit(player) + " once.");
		}
		else if (player.body.cocks.length > 0 && player.body.vaginas.length > 0)
		{
		 	CView.text("Your arousal at this point is quite apparent on the whole of your genitals.  Clara gives you a few touches and caresses, but doesn’t seem all that interested.");
		}
		else
		{
		 	CView.text("Or rather, your lack of thereof.  Clara doesn’t seem to mind too much; Marble must have told her about this part of you too.");
		}
		//Clara fingers the PC’s asshole
	 	CView.text("\n\nIt then becomes rather apparent exactly what Clara really wants to play with now, and you feel her fingers slip down and around your body.  She gets herself a handful of your " + describeButt(player) + ", and then slips her fingers into your crevice.  That finger doesn't stop until it's pushed itself inside your " + describeButthole(player.body.butt) + " and triggered a response that makes Clara laugh with delight.  Feeling emboldened, the bovine girl thrusts another finger into your back entrance and starts wiggling them around.");
		...
		if(player.body.cocks.length <= 0) choices[0] = ["Next", cocklessPCsGetAbusedByCowCunts];
		else if(player.body.cocks.find(Cock.CockThatFits(20))) choices[0] = ["Next", cockTooBigForCowCuntageBadEndage];
		else choices[0] = ["Next", badEndDickThatFitsClara];
	}
	...
	export function badEndDickThatFitsClara(player: Character): NextScreenChoices
	{
		CView.clear();
		var x: number = player.body.cocks.find(Cock.CockThatFits(20));
		...
		if((player.body.cocks.length > 0 && player.body.cocks.sort(Cock.Smallest)[0].area <= 40) && player.body.chest.sort(BreastRow.Largest)[0].rating <= 25 && !player.body.legs.isTaur() && !player.body.legs.isNaga() && !player.body.legs.isGoo() && !player.body.legs.isDrider())
		{
		 	CView.text("In seconds she’s freed you of the form fitting garment that you woke up in.  ");
		}
	 	CView.text("You’re still erect from the arousal that nursing from Clara gave you.  The young cow-girl straddles your calves and gives your " + describeCock(player, xundefined) + " a slight flick with her finger and continues to smile broadly at you.  \"<i>One thing that I was really worried about was that you woouldn’t have a penis that I could actually fit inside me.  Thankfully, that isn’t the case, and that pretty pecker is just my size!</i>\"  She pulls apart the lips of her womanhood before continuing, \"<i>This will be my first time ever putting anything in here, take note just how special of a slave you are to me!</i>\"");

	 	CView.text("\n\nWell, she certainly isn’t wasting any time, as the very next instant she has lowered her opened labia onto the tip of your " + describeCock(player, xundefined) + ".  The passage is already wet and slick, thanks to her previous orgasm.  Immediately, an expression of both pain and pleasure crosses over the bovine woman’s face and she starts to pant.  You lift up your hands, bClara issues a sharp command that you aren’t to do anything at all while she has the sex she wants.  You can’t do much else but just lie back and take it.");
		...
		choices[0] = ["Next", finalBadEndWithCowCunt];
	}
	...
	export function cockTooBigForCowCuntageBadEndage(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("\"<i>Now, we do have one problem.  ");
		if(player.body.cocks.length > 1) 
		{
		 	CView.text("All of your penises are");
		}
		else CView.text("Your penis is");
	 	CView.text(" too big for a virgin like me to fit!  How the hell am I suppose to get that inside me?</i>\"  Clara stands up and starts stomping around the room.  ");
		//if (PC has at least one dick that will fit inside Marble’s current capacity)
		if(player.body.cocks.find(Cock.CockThatFits(marbleScene.marbleCuntCapacity())))
		{
		 	CView.text("\"<i>‘It’s not a flaw, it’s a feature,’ she said, ‘This is why we train ourselves to take big endowments,’ she said!  Fuck you Marble!  I’m not sticking that stupid big thing inside me.</i>\"  ");
		}
		else
		{
	 	...
	 	CView.text("She continues ranting for several minutes before turning to you.  \"<i>YOOU!  Didn’t you ever stop to consider the needs of dainty girls like me before you started abusing transformatives?</i>\"  She sighs, apparently running out of steam.  \"<i>Who knew this plan would be so hard?");
		...
		choices[0] = ["Next", cocklessPCsGetAbusedByCowCunts];
	}
	//PC is cockless
	...
	export function cocklessPCsGetAbusedByCowCunts(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("Clara straddles your head with her legs and looks down at you.  \"<i>Now you’re going to lick me until I’m satisfied.  Also, don’t you dare doo anything unless I tell you to, got it?</i>\"  She doesn’t wait for a response.  \"<i>Goood, now let's get started.</i>\"  The cow-girl promptly drops her snatch onto your face.");
		//if (PC has a long tongue)
		if(player.body.tongue.type > 0)
		{
			...
			//Spend some time with the PC licking Clara while she gives orders and directions while the PC hears the results of their work.  Clara then orgasms.
		 	...
		 	CView.text("\n\n\"<i>Mhh, huh, ah, higher!</i>\"  You obey, and move up to Clara’s clit, and the hood around it.  She has already become heavily engorged, and the instant your " + describeTongue(player) + " touches the bovine woman’s erect love button, she lets out a low shriek of pleasure!  \"<i>Woah!  Wah, mah, mooo!</i>\" she cries out as a torrent of fluid sprays from her womanhood and drenches your face.  Clara is unquestionably an inexperienced virgin, given her rapid and unexpected orgasm.  She spends several moments trying to catch her breath.");
			//Now feeling bolder, Clara has the PC use the full extent of their tongue to penetrate her.  Clara orgasms again fairly shortly thereafter.
		 	...
		 	CView.text("\n\nAbruptly, a large torrent of fluid comes rushing down the hole you’ve been pleasing.  Much to your dismay, the flood of fluid actually forces your " + describeTongue(player) + " out of her passage before nearly drowning you in girl cum.  You cough and sputter for several seconds until air finally returns to your lungs.  What energy you had before now is now gone.");
	...
	export function finalBadEndWithCowCunt(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("Years later...");
	...
	export function defeatClaraCuntInAFight(player: Character, cheated: boolean = false): NextScreenChoices
	{
		...
		else
		{
			...
			else if(!player.effects.has(StatusEffectType.ClaraFoughtInCamp))
			{
			 	CView.text("Worried about the others, you head out of the cellar and drag Clara and her things with you.  Thanks to the strange way that travel works in this world, it doesn't take you long to find your way back to   You catch your followers there just before they head out to go looking for you and dump your would be kidnaper before them.  Marble steps up next to you and looks down at her sister.");
		...
		//Raping or punishing Clara in camp with your followers
	 	CView.text("\"<i>Clara, what the hell were you trying to pull there?</i>\" Marble demands of her sister, her hands shaking in rage.");
	 	...
	 	CView.text("\n\n\"<i>No sis, you had your chance.  " + player.desc.name  + " isn't addicted to you, that means that " + mf(player, "he", "she") + "'s up for grabs.  That's how things work for our race.</i>\"");
	 	...
	 	CView.text("\n\nMarble looks at her hands in horror, just barely above a whisper you manage to hear her say, \"<i>His hands were shaking, just like " + player.desc.name  + "'s...</i>\"");
	 	...
	 	CView.text("\n\nYour cow-girl companion continues to shake in anger, but also fear and frustration.  \"<i>I, I, YES!  I want to addict " + player.desc.name  + " to my milk!  I want to so much!  I'm so afraid that " + mf(player, "he", "she") + " won't stay by my side, but I also feel so bottled up.  "); //no new paragraph

		//if (PC has another lover in camp)
		if(loversCount() > 1)
		{
		 	CView.text("I'm not even " + player.desc.name  + "'s only mate.  " + mf(player, "He", "She") + " can satisfy their needs, but can't satisfy mine?  "); //no new paragraph
		}
	 	CView.text("The frustration of not being able to nurse, someone, anyone, but especially my mate is tearing me apart!</i>\"  Marble stops screaming and her head drops.  All that's coming out of her now is heavy breathing.");

	 	CView.text("\n\n\"<i>There, you see?  Now I'm sure you understand why me and " + player.desc.name  + " here belong toogether.  I won't deny the proper order of things, and I'll make sure that " + mf(player, "he", "she") + " leads a fulfilling life as my milk slave.  " + mf(player, "He", "She") + "'ll even by my first husband!</i>\"");
	 	...
	 	CView.text("\n\n\"<i>Then if " + player.desc.name  + " loves you so much, why can't " + mf(player, "he", "she") + " change for your sake?  Why can't they become addicted to show just how much they love you?</i>\"");
	 	CView.text("\n\nMarble swallows before saying, \"<i>Do you have any idea of what " + player.desc.name  + " is trying to do?  " + mf(player, "He", "She") + " is trying to defeat the demons!  To stop the corruption for good!</i>\"");
	 	CView.text("\n\nAt this Clara bursts out laughing, \"<i>Oh, and save the world I take it?  This " + describeRace(player) + "?  Don't be ridiculous.</i>\"");
	 	CView.text("\n\nMarble starts quivering in rage once more.");
	 	CView.text("\n\n\"<i>Seriously, there is no damn way someone as pathetic as " + player.desc.name  + " could accomplish that,</i>\" she continues while struggling to her feet.  \"<i>You need to be like a twenty foot giant, champion fighter, god master of magic to pull a stunt like wiping out the demons.  " + player.desc.name  + " is none of those things.  Do you really think someone who could would actually care about a lowly Lacta Bovine like you?</i>\"  Clara pokes her finger into Marble's chest.  \"<i>I guess the saying among our kind is right, 'the first daughter has the sourest milk.'</i>\"");
		...
		if((player.body.cocks.length > 0 && player.body.cocks.find(Cock.CockThatFits(20))) || player.body.vaginas.length > 0) CView.text("use Marble's unstable state of mind to get her into a forceful threesome with Clara, ");
		//(if PC either has a cock that will fit Marble, or has a vagina)
		if((player.body.cocks.length > 0 && player.body.cocks.find(Cock.CockThatFits(marbleScene.marbleCuntCapacity()))) || player.body.vaginas.length > 0) CView.text("have sex in front of Clara so she can see what your love really is, ");
		//(if Marble is a futa)
		if(MarblePurificationFlags.MARBLE_DICK_TYPE > 0) CView.text("egg Marble into violating Clara with her cock, ");
	 	CView.text(" or try to calm Marble down so that she doesn't do anything to her sister.");
		...
		choices[0] = ["Spank", murbleSpanksCowCunt];
		if((player.body.cocks.length > 0 && player.body.cocks.find(Cock.CockThatFits(20))) || player.body.vaginas.length > 0) choices[1] = ["Threesome", pcAndMurbleDoubleTeamCowButt];
		if((player.body.cocks.length > 0 && player.body.cocks.find(Cock.CockThatFits(marbleScene.marbleCuntCapacity()))) || player.body.vaginas.length > 0) choices[2] = ["NTR", NTRIsClearlyTheWorstFetishWhyWouldYouWriteThisOMG];
		if(MarblePurificationFlags.MARBLE_DICK_TYPE > 0) choices[3] = ["Marble Cock", futaMarbleIsAHugeCowToCowCuntAndStuffsCowCuntsCuntFullOfCowCock];
		choices[4] = ["Calm Down", calmMurblesFatAssDown];
	}
	//Marble spanks Clara
	//Just let Marble punish Clara like the bad girl she is.
	export function murbleSpanksCowCunt(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("Marble seems to have a good idea on how to punish Clara already, so you just sit back and watch.");
	 	...
	 	CView.text("\n\nWith the white haired bovine's pale bare bottom high in the air, Marble gruffly commands, \"<i>Now, say you're sorry for trying to kidnap " + player.desc.name  + ".</i>\"");
		...
		choices[0] = ["Next", calmMurblesFatAssDown];
	}
	//PC and Marble rape Clara
	//Double team Clara, will be rough forceful sex.
	export function pcAndMurbleDoubleTeamCowButt(player: Character): NextScreenChoices
	//does not work with centaur, naga, drider, or goo bodies.  However, I don't particularly want to just say you can't do this scene if you have them.
	{
		CView.clear();
		var x: number = player.body.cocks.find(Cock.CockThatFits(20));
	 	CView.text("You step forward and suggest to Marble that the two of you should work together to punish Clara.  She nods and asks what you have in mind.  Well, you were thinking that a sexual punishment from the two of you would be appropriate.");
	 	CView.text("\n\n\"<i>What?  I don't want to have a threesome with my sister!  Only " + player.desc.name  + " should be able to touch me!</i>\"");
		...
		if((player.body.cocks.length > 0 && x >= 0) && MarblePurificationFlags.MARBLE_DICK_TYPE > 0)
		{
			//n is the index of the PC's biggest cock under 20 area.
		 	CView.text("Eventually you pull her up and sandwich her between your bodies.  Marble expresses an aversion to anal, so you take the backside while she takes the front.  \"<i>Hey, you can't do this to me!  I don't want to lose my virginity to my sister!</i>\" Clara cries out indignantly.");
		 	...
		 	CView.text("\n\nMarble looks to the side of her sister's head, and gives you a solemn nod.  You take that as an invitation to begin the violation and grab a good grip of Clara's expansive rear.  Then you prod your " + describeCock(player, xundefined) + " at the entrance to her anus, prompting a gasp and plea from the woman you're about to penetrate to stop.  Her sister doesn't heed her begging, and neither do you.");
		 	...
		 	CView.text("\n\n\"<i>Damn it all, why couldn't " + player.desc.name  + " have taken the front?  Now I'll never be able to experience it for the first time with my perfect mate!  You're evil, sister.</i>\"  Marble looks like she is about to blow up once more at that comment....");
		} //else if (PC has a cock, Marble does not)
		else if((player.body.cocks.length > 0 && x >= 0) && MarblePurificationFlags.MARBLE_DICK_TYPE == 0)
		{
			//n is the index of the PC's biggest cock under 20 area.
		 	...
		 	CView.text("\n\n\"<i>You will if you don't want me to twist your ears,</i>\" comes the response.  You prod your " + describeCock(player, xundefined) + " up against her tight asshole and suggest that you might be taking a different entrance if she doesn't cooperate for her punishment.  \"<i>Of course, if you're sorry for what you tried to do to " + player.desc.name  + "...</i>\" Marble suggests.");
		 	...
		 	CView.text("\n\n\"<i>Gah, sis, does your milk taste as bad as your pussy juices?  No wonder " + player.desc.name  + " didn't want yoou!</i>\"  Another shock of anger shoots through Marble's features.  Is she going to try and inflict more punishment on her sister?");
		}
		//else if (PC does not have a cock, Marble does)
		else if(MarblePurificationFlags.MARBLE_DICK_TYPE > 0)
		{
		 	...
		 	CView.text("\n\nYou grab ahold of her ears and give them a slight tug.  She'd better go along with her punishment like a good girl, or you'll be adding some extra pain onto her head.  \"<i>Of course, if you're sorry for what you tried to do to " + player.desc.name  + "...</i>\" Marble suggests.");
		 	CView.text("\n\n\"<i>What?  Why the hell would I be sorry for that?  Moop!</i>\"  There is a low slurp as your mate pushes her newly acquired seven inch shaft deep inside her sister's womanhood.  \"<i>Hey!  Yo-mmph,</i>\"  You silence her with your " + describeVagina(player, player.body.vaginas.get(0)) + ", roughly forcing it into her face.  You give a slight twist to the young bovine's ears to remind her what she should be doing, and right away her tongue comes out and starts giving you experimental licks.");
		 	...
		 	CView.text("\n\nYou express your disappointment to Marble at her sister's unfinished performance, so the fucking continues.  Thankfully, it seems that the bad bovine did get the message and has started to tease and suck on your " + describeClit(player) + " as well as continuing with her teases of your folds and depths.  You'd almost feel bad for doing what you two are doing to her if she wasn't evidently enjoying it as much as she is.  Muffled but loud gasps and moos are constantly being uttered into your " + describeVagina(player, player.body.vaginas.get(0)) + " by the bound girl between you and your lover.  Her breath upon your womanhood only serves to make you feel even better.");
		...
		{
		 	CView.text("\n\nEventually you roll her onto her back.  Then Marble sits down on her sister's face while you put your mouth to Clara's womanhood and raise your " + describeButt(player) + " up into the air so that your mate can reach your own " + describeVagina(player, player.body.vaginas.get(0)) + ".");
		 	...
		 	CView.text("\n\nNow you get to experience the pleasure of Marble's tongue starting to tease your " + describeVagina(player, player.body.vaginas.get(0)) + " with her bovine tongue.  It's easily twice the length of a human's and quite adept at pleasuring a woman's passages.  Your mate is certainly quite willing to put it to that very use, using it to plunge deep inside your depths and also expertly explore and pleasure your " + describeClit(player) + ".");
		 	...
		 	CView.text("\n\nYour hot and sweaty bodies separate from one another, each face bearing the fluids of one another's orgasms.  Clara is coughing and sputtering, \"<i>Gah, sis, does your milk taste as bad as your pussy juices?  No woonder " + player.desc.name  + " didn't want you!</i>\"  Another shock of anger shoots through Marble's features.  Is she going to try and inflict more punishment on her sister?");
		}
		player.orgasm();
	...
	export function NTRIsClearlyTheWorstFetishWhyWouldYouWriteThisOMG(player: Character): NextScreenChoices {
		CView.clear();
	 	CView.text("You ");
	 	CView.text("step forward and tell Marble that you think that since Clara thinks your love for one another isn't real, how about the two of you show her just how real it is?  \"<i>Good idea, " + player.desc.name  + ",</i>\" your lover tersely replies through her still seething visage and steps forwards.  In an instant you're forcibly pulled into a strong passionate kiss upon humanlike lips while a decidedly inhumanly long tongue invades your mouth.  “<i>Wait, are you two just gooing to fuck right in front of me?</i>\" comes an incredulous voice from somewhere behind the woman you're kissing.  Neither of you pay it any mind and continue to taste, devour, and lavish affection on each other's mouths.  An angry grunt comes once more from the woman you're showing what loves means to, and sounds of a struggle accompany it.");
	 	CView.text("\n\nProper lovemaking does not stop at kissing.  You have to be sure to show a would be thief the whole process.  Your partner is already working on that matter through efforts to free your body from the " + player.inventory.equipment.armor.displayName + " that contains it.  Not one to be outdone, you begin to remove the familiar top that oh-so-often holds bovine breasts at bay.  With Marble's melons free, she immediately pulls your head down into the great valley of flesh her breasts create.");
		...
		var x: number = player.body.cocks.find(Cock.CockThatFits(marbleScene.marbleCuntCapacity()));
		if(player.body.cocks.length > 0 && x >= 0)
		{
		 	CView.text("she drops herself onto your " + describeCock(player, xundefined) + ".  Since your bovine mate isn't in the habit of wearing any form of undergarments, her skirt doesn't really do much to get in the way of penetration.  A shriek of dismay accompanies this act.");
		...
		else if(player.body.vaginas.length > 0 && MarblePurificationFlags.MARBLE_DICK_TYPE > 0)
		{
		 	CView.text("she drops herself onto the ground next to you.  Abruptly, she lifts you up and drops your womanhood onto the member you gave her.  Since your bovine mate isn't in the habit of wearing any form of undergarments, her skirt doesn't really do much to get in the way of penetration.  A shriek of dismay accompanies this act.");
		 	CView.text("\n\nThere is no hesitation after you've been invaded, instantly you're being roughly lifted and dropped by a beast that has been awakened in Marble, and there is nothing that can stop it.  A loud slapping sound fills the air with each drop of your hips, only for them to be lifted up and dropped once more.  Your " + describeHips(player) + " have been essentially turned into a jackhammer right now, and its operator shows no signs allowing you to stray from your place on her lap.");
		}
		//else if (PC has a vagina and Marble does not have a cock)
		else if(player.body.vaginas.length > 0)
		{
		 	CView.text("she drops herself onto your legs, and grabs one of them.  Instantly, your two womanhoods are being grinded against one another with great force and urgency.  Since your bovine mate isn't in the habit of wearing any form of undergarments, her skirt doesn't really do much to get in the way of tribadism.  A shriek of dismay accompanies this act.");
	...
	export function futaMarbleIsAHugeCowToCowCuntAndStuffsCowCuntsCuntFullOfCowCock(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("You call out to Marble and suggest that she should use her new \"appendage\" to punish Clara.  She hesitates for a moment, then nods.  You doubt that you could have ever gotten her to do something like that if she wasn't as worked up as she is.");
	 	...
	 	CView.text("\n\nThe older sister hesitates for a moment, \"<i>Are you sorry for trying to steal " + player.desc.name  + " from me?</i>\"");
	...
	export function calmMurblesFatAssDown(player: Character): NextScreenChoices
	{
		CView.clear();
		...
		if((player.body.tallness >= 80 && MarblePurificationFlags.MARBLE_BOVA_LEVEL < 2) || (player.body.tallness >= 86 && MarblePurificationFlags.MARBLE_BOVA_LEVEL >= 2))
		{
		 	CView.text("tightly hugs herself against your body.  Gently stroking her hair, you're happy to be a shoulder for her to cry on, given both your affection for the gentle girl and the awareness that you're able to help her feel better.");
		} 
		//else if (PC taller than 5 feet)
		else if(player.body.tallness >= 60)
		{
		 	CView.text("returns the hug.  Rocking back and forth against one another, you're happy to be here for her, given both your affection for the lovely lady and the warm feeling of being so close to one you care so much for.");
		}
		else
		{
		 	CView.text("scoops you up, hugging you tightly into her chest.  Sandwiched between her massive mammaries, you're happy to serve as her temporary comfort doll, given both your affection for the big bovine and the pleasant accommodation of her bountiful breasts.");
		}
	 	...
	 	CView.text("\n\nYou suggest that now would probably be a good time to check on Rathazul, before deciding what to do with Clara.  \"<i>Good idea, it will give me some time to clear my head.</i>\"  The two of you leave the tied up bovine where she is and head over to the elderly alchemist's side of the ");
		//Proceed to Rathazul finishing Marble's Potion.
		
		choices[0] = ["Next", rathazulFinishingMarblesPotion];
	}
	//Rathazul Finishing Marble’s Potion
	export function rathazulFinishingMarblesPotion(player: Character): NextScreenChoices
	{
		//Initial check
		CView.clear();
	 	CView.text("The rather colorful events of the morning seem to have gone largely unnoticed by the elderly rat.  He is uncharacteristically engrossed in the whitish bubbling formula in front of him, so much so that he doesn't actually notice when you and Marble come up to him.  The two of you are surprised that he doesn't address your presence at all.  It's several moments before you clear your throat and announce your presence.  Starting, the camp's alchemist turns to you blinking in confusion for another moment, then addresses you, \"<i>Ah, " + player.desc.name  + ", Marble.  Good news, you two!  This will likely be very potent and capable of purifying cow-girls in very small doses.  Even those who have not yet been fed doses of purified LaBova.  However, it may not be effective on those unrelated to you.</i>\"");
		...
		if(MarblePurificationFlags.MARBLE_KIDS > 0)
		{
		 	CView.text("Then we'll be ready to purify you and your child");
			if(MarblePurificationFlags.MARBLE_KIDS > 0) CView.text("ren");
		 	CView.text(".  ");
		}
	 	CView.text("So come back a little later.</i>\"  The rodent returns to his reactions.");

	 	CView.text("\n\nYou turn to your cow-girl lover and ask her what she'd like to do in the meantime.  She hesitates, visibly bothered by something.  \"<i>Actually, I think I need some time on my own to think about what my sister said.  Come and get me when the formula is finished, okay?</i>\"  You nod and she heads off to the edge of the ");
	 	CView.text("\n\nYou've got some time to kill now, you guess.  Is there something you want to spend it doing?");
	
		
		choices[0] = ["Next", theFormulaIsDone];
	}
	//Let Marble know the formula is done
	export function theFormulaIsDone(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("Rathazul tells you that the formula is ready, so you head over to Marble to let her know.");
		//Scene adapted from Rayfire's scene
	 	...
	 	CView.text("\n\nMarble's forced smile fades and her features resume the crestfallen look she had earlier, thinking about the right words to say to you.  Seeing as there is no other way around the conversation she says, \"<i>To be honest sweetie, you're right, I'm not all right.</i>\"  There is a moment's hesitation before she continues, \"<i>Oh " + player.desc.name  + ", so much has happened in such a short time and it's all that's on my mind. After all the things that Clara has said I just don't know what to do.</i>\"");
		...
		else
		{
	 	...
	 	CView.text("\n\nMarble bursts into tears and pulls you into a nearly back-breaking hug, calling out your name and declaring her love for you as she continues to embrace you.  She gives you a deep kiss and you return it wholeheartedly.  When she finally lets you go, she jumps down from the wall, giving you a delightful, beautiful smile.  \"<i>You really are too good for me, " + player.desc.name  + ".  I feel a lot better now.</i>\"");
		...
		choices[0] = ["Next", purifyMarbleComplete];
	}

	//Marble is purified
	export function purifyMarbleComplete(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("The two of you arrive at Rathazul's workspace, and find the owner awaiting you, holding a bottle of whitish fluid with gold streaks running throughout.  \"<i>So is that the formula?</i>\"  Marble asks looking at the bottle.");
		...
		if(MarblePurificationFlags.MARBLE_KIDS > 0)
		{
			...
			if(MarblePurificationFlags.MARBLE_KIDS > 1) CView.text("ren");
		 	CView.text(" right away.  Give ");
			if(MarblePurificationFlags.MARBLE_KIDS == 1) CView.text("her");
			else CView.text("each");
		 	CView.text(" a two drop dose,</i>\" the alchemist instructs you and hands over the bottle of his formula.");

		 	CView.text("\n\nYou and Marble waste no time in heading over to the nursery and feed ");
			if(MarblePurificationFlags.MARBLE_KIDS > 1) CView.text("each of ");
		 	CView.text("your child");
			if(MarblePurificationFlags.MARBLE_KIDS > 1) CView.text("ren");
		 	CView.text(" two drops of the formula.  ");
			if(MarblePurificationFlags.MARBLE_KIDS > 1) CView.text("They ");
			else CView.text("She ");
		 	CView.text("take");
			if(MarblePurificationFlags.MARBLE_KIDS == 1) CView.text("s");
		 	CView.text(" it easily enough, and while there are no outward effects, ");
			if(MarblePurificationFlags.MARBLE_KIDS == 1) CView.text("your ");
			else CView.text("each ");
		 	CView.text("child does report feeling like something was lifted from ");
			if(MarblePurificationFlags.MARBLE_KIDS > 1) CView.text("their");
			else CView.text("her");
		 	CView.text(" mind.");
		...
		choices[0] = ["Next", dealingWithCowCuntPostPurification];
	}
	...
	export function dealingWithCowCuntPostPurification(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("Now there is only one more loose end in this whole effort to purify Marble: her sister.");
	 	...
	 	CView.text("\n\nMarble sighs, then speaks once more, \"<i>I figured you should know at least that, sister, before we decided what to do with you.</i>\"  She turns to you, \"<i>Well, " + player.desc.name  + ", you were the one she tried to kidnap and enslave.  What do you think we should do with her?</i>\"");
		...
		choices[0] = ["Purify", purifyMurblesSister];
		choices[1] = ["Don't", dontPurifyClara];
	}

	//Purify
	export function purifyMurblesSister(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("You ask Marble for the bottle of Rathazul's formula.  She nods, hands it to you and says, \"<i>Yeah, I think that's the best choice too.</i>\"");
	 	...
	 	MarblePurificationFlags.CLARA_PURIFIED = 1;
		//PC makes their choice: Tel'Adre, camp cage, let go."
		
		choices[0] = ["Cage", cageDatCowCunt];
		choices[1] = ["Let Go", letGoOfMarblesCowCuntSister];
	}

	//Camp Cage
	export function cageDatCowCunt(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("You say that you're going to be keeping Clara a prisoner in the   She deserves to be imprisoned so she'll no longer be a threat to anyone anymore.  Marble looks uncertain at you and asks if you're sure she needs to be locked up now that she's been purified.  You say you're certain, and your mate nods to you and says that she trusts your judgment on this matter.");
	 	...
	 	MarblePurificationFlags.CLARA_IMPRISONED = 1;
		//Clara is marked as having been purified and caged in 
		//continue to purification quest final
		
		choices[0] = ["Next", purificationQuestFinal];
	}
	//Let go
	export function letGoOfMarblesCowCuntSister(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("You say that now that Clara is no danger to anyone any more, she is free to go, so long as she promises to never bother you or your followers again.  Marble nods at this and says she agrees with your decision.");
	 	CView.text("\n\nYou and Marble untie Clara from her bonds, give her her things, and escort her out of the   She doesn't even look at either of you until she is at the edge and you tell her to leave and never come back.  She gives you one look, a look of pure rage and anger, of one who is swearing vengeance, then as soon as it started, it is over.  She steps out of the camp, and warps away into the world.");
	...
	//Don't purify
	export function dontPurifyClara(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("No, you'll offer her the mercy of letting her keep her precious corruption.  Marble tells you she will respect your decision on that matter.  Now the question is what to do with her.");
	 	CView.text("\n\nYou guess you could also build a cage in the camp if you really want to keep an eye on her yourself.  That will make her another mouth to feed, and you'll have to hear her ranting every day.  Still, the saying does go, \"keep your enemies closer.\"");
	 	CView.text("\n\nIf you're feeling especially generous, you could just let her go, with not more than a promise to never come back or try to kidnap you again.");
		//PC makes their choice: Tel'Adre, camp cage, let go."
		
		choices[0] = ["Cage", stickUnpurifiedClaraInACage];
		choices[1] = ["Let Go", letCorruptCowCuntGo];
	}

	//Camp Cage
	export function stickUnpurifiedClaraInACage(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("You say that you're going to be keeping Clara a prisoner in the   She deserves to be imprisoned so she'll be no threat to anyone anymore.  Marble nods to you and says that she trusts your judgment on this matter.");
	...
	export function purificationQuestFinal(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("\"<i>It has been quite the morning, hasn't it sweetie?</i>\"  It certainly has been, but at least now, your lover is finally free of her corruption once and for all.");
	 	CView.text("\n\nYou've gained 500 exp for helping Marble attain purity.");
		//increase exp by x
		player.stats.XP += 500;
		//Marble corruption is set to 10
		MarblePurificationFlags.MARBLE_PURIFICATION_STAGE = 5;
		MarblePurificationFlags.MARBLE_PURIFIED = 1;
		player.effects.get(StatusEffectType.Marble).value4 = 10 ;
		inCombat = false;
	...
	export function purifiedSuccubusMilkForPureMarbles(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("Do you want to ask Marble to grow her breasts bigger?");
		//PC chooses yes or no
		
		choices[0] = ["Yes", actuallyGrowPureMarblesTittiesForFunzies];
		choices[1] = ["No", marbleScene.giveItem];
	}
	//if yes
	export function actuallyGrowPureMarblesTittiesForFunzies(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("You offer Marble a bottle of purified succubus milk and tell her that you'd like her to make her bust bigger.");
		//if (Marble's breast size < 3)
		if(MarblePurificationFlags.MARBLE_BREAST_SIZE < 3)
		{
			...
			MarblePurificationFlags.MARBLE_BREAST_SIZE++;
			CView.text(marbleScene.marbleBreastSize() + " melons!");
			//if (Marble's breast size == 1)
			if(MarblePurificationFlags.MARBLE_BREAST_SIZE == 1)
			{
			...
			else if(MarblePurificationFlags.MARBLE_BREAST_SIZE == 2)
			{
			...
			MarblePurificationFlags.MARBLE_LUST += 5;
			if(MarblePurificationFlags.MARBLE_LUST > 100) MarblePurificationFlags.MARBLE_LUST = 100;
		}
		else
		{
		...
		return { next: marbleScene.giveItem };
	}

	//Reducto
	//Used to shrink Marble's breasts
	export function pureMurbleUsesReducto(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("Do you want to ask Marble to shrink her breasts down?");
		
		choices[0] = ["Yes", pureMurblePCChoosesYesToShrinkeyTits];
		choices[1] = ["No", marbleScene.giveItem];
	}

	//if yes
	export function pureMurblePCChoosesYesToShrinkeyTits(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("You offer Marble a jar of reducto and tell her you'd like her to reduce her bust's size a bit.");
		if (MarblePurificationFlags.MARBLE_BREAST_SIZE > 0)
		{
			...
			MarblePurificationFlags.MARBLE_BREAST_SIZE--;
			CView.text(marbleScene.marbleBreastSize() + " sized breasts.");
			if(MarblePurificationFlags.MARBLE_BREAST_SIZE == 0)
			{
			...
			else if(MarblePurificationFlags.MARBLE_BREAST_SIZE == 1)
			{
			...
			MarblePurificationFlags.MARBLE_LUST -= 5;
			if(MarblePurificationFlags.MARBLE_LUST < 0) MarblePurificationFlags.MARBLE_LUST = 0;
		}
		else 
		{
	...
	export function nursingFromPureMarble(player: Character): NextScreenChoices
	{
		CView.clear();
		//if(time since last nursing is < 4 hours)
		if(MarblePurificationFlags.MARBLE_TIME_SINCE_NURSED_IN_HOURS < 4)
		{
		 	CView.text("Feeling a bit thirsty, you ask Marble if there’s any chance she has some milk for you.  She gives you a sad look and informs you that no, she hasn't yet built up enough milk for you to have more.  Purifying her has made it take a bit longer for her to produce milk.");
			return { next: marbleScene.interactWithMarbleAtCamp };
		}
		else
		{
		 	CView.text("Feeling a bit thirsty, you ask Marble if theres any chance she has some milk for you.  She smiles at you and tells you she was worried you'd never ask, then leads you to a familiar, secluded part of the rocks around the ");
			...
			{
			 	CView.text("The tall cow-lady takes a seat on the rocks and you jump onto her lap, ");
			} 
			else if (player.body.tallness < 84)
			{
			 	CView.text("You lean forward and ");
			}
			else
			{
			 	CView.text("You lift up the cow-lady to get access, ");
			}
		 	CView.text("in seconds your lips are wrapped around her teats, and her wonderful milk is flowing forth.");
			...
			player.stats.HP += Math.round(player.stats.maxHP*.2) ;
			//Restore 30 fatigue
			player.stats.fatigue += -30 ;
			//increase lust by 15
			stats(0,0,0,0,0,0,15,0);
			//increase Marble lust by 10
			MarblePurificationFlags.MARBLE_LUST += 10;
			MarblePurificationFlags.MARBLE_TIME_SINCE_NURSED_IN_HOURS = 0;
			if(MarblePurificationFlags.MARBLE_LUST > 0) MarblePurificationFlags.MARBLE_LUST = 100;
			return { next: returnToCampUseOneHour };
		}
	}
	...
	export function marbleWarnsPCAboutCorruption(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("Marble comes up to you with a worried look on her face.  \"<i>Uh, sweetie?  I'm getting worried about you.  You're trying to put so much effort into freeing me of corruption, but you seem to be slipping away yourself...</i>\"  She looks at the ground, rubs her hands a few times desperately trying to spit the words out.  \"<i>I don't think I'll be able to stay by your side if you keep falling like that.  Please find a way to clear yourself of the corruption that's gripping you!</i>\"");
	 	...
	 	MarblePurificationFlags.MARBLE_WARNED_ABOUT_CORRUPTION = 1;
		//end event
		return { next: campMenu };
	}
	//Leaving from corruption
	//PC's corruption is now too high.
	export function marbleLeavesThePCOverCorruption(player: Character): NextScreenChoices
	{
		CView.clear();
	 	CView.text("Marble approaches you with a somber expression on her face, her things strapped to a pack on her back");
		//if (PC has more than one child with Marble)
		if(MarblePurificationFlags.MARBLE_KIDS > 1) CView.text(" , and with her children in tow");
		//else if (PC has one child with Marble)
		else if(MarblePurificationFlags.MARBLE_KIDS == 1) CView.text(", and holding her child's hand");
	 	CView.text(".  \"<i>I'm sorry sweetie.  I tried to warn you, but-</i>\" she stops for a moment and takes a deep breath to steady herself.  ");
		//if (PC has at least one child with Marble)
		if(MarblePurificationFlags.MARBLE_KIDS > 0)
		{
		 	CView.text("\"<i>Mommy, what's wrong?</i>\"  The young one holding Marble's hand asks nervously.\"<i>Hush now, little one.  We have to go away for awhile,</i>\" the bovine mother responds and turns back to you.  ");
		}
	 	CView.text("\"<i>You've gotten too corrupted, and I can't stay here with you anymore.  Please find a way to free yourself from the taint within you so that I can come back.  Goodbye.</i>\"");
		...
		//Marble and her kids (if any) are removed from the farm and camp, she will only return once the PC's corruption is below 40, and Vapula, Holi, Corrupted Jojo, and Corrupted Amily are not in 
		MarblePurificationFlags.MARBLE_LEFT_OVER_CORRUPTION = 1;
		player.effects.remove(StatusEffectType.CampMarble);
		return { next: returnToCampUseOneHour };
	}
	//Marble comes back after your corruption drops below 40
	export function pureMarbleDecidesToBeLessOfABitch(player: Character): NextScreenChoices
	{
		//happens after Marble has left the PC due to their corruption getting too high, and there are no corrupt followers in   She returns during the day once the PC's corruption has gone below 40.
		CView.clear();
	 	CView.text("While checking your traps to ensure that they're still working, you hear a voice call out from outside the camp, \"<i>Sweetie!</i>\"  You look up and see the smiling face of Marble");
		//if (PC has at least one child with Marble)
		if(MarblePurificationFlags.MARBLE_KIDS > 0) CView.text(" and that of your child");
		//if (PC has two or more children with Marble) 
		if(MarblePurificationFlags.MARBLE_KIDS > 1) CView.text("ren");
	 	CView.text(" just outside the   She must have somehow found out about your purification.");

	 	CView.text("\n\nShe excitedly runs up to you and gives you a big hug, holding you very tightly to her chest.  After several moment of embracing you, Marble gently whispers in your ear, \"<i>I'm so glad you managed to free yourself from the worst of the taint.</i>\"  She pulls back from you and asks if you mind if she moves back into the   How could you say no to her?");
		//re-add Marble and her kids to the camp and farm
		player.effects.add(StatusEffectType.CampMarble,0,0,0,0);
		MarblePurificationFlags.MARBLE_LEFT_OVER_CORRUPTION = 0;
		MarblePurificationFlags.MARBLE_WARNED_ABOUT_CORRUPTION = 0;
		MarblePurificationFlags.FOLLOWER_AT_FARM_MARBLE = 0;
		return { next: returnToCampUseOneHour };
	}
	//Clara prison camp descriptions
	//displayed in the camp description if Clara was imprisoned in 
	export function claraCampAddition(player: Character): NextScreenChoices
	{
		CView.text("  Nearby the portal sits a large wooden cage for keeping Marble's sister Clara prisoner.  It is tall enough for her to stand in and long enough for her to lie down completely.  ");
		//if (Clara was purified)
		if(MarblePurificationFlags.CLARA_PURIFIED > 0)
		{
			if(randInt(10)==0) CView.text("You catch Clara staring at you for a moment, then her gaze once again returns to some distant mountain top.  She has not taken her purification well.");
			else CView.text("As always, Clara sits within, staring off into the distance.  She has not taken her purification well.");
		}
		else
		{
			//if (time of day <= 7 OR >= 20)
			if(time.hours <= 7 || time.hours >= 20) CView.text("Clara is laying sprawled out on the ground of her cage, snoring loudly.");
			else
			{
				temp = randInt(4);
				switch (temp)
				{
					case 0:
						CView.text("Clara is currently fidgeting with her clothing, it looks like she's doing some repairs or embroidery.  It's one of the few grateful moments of quiet you get from her.");
						break;
					case 1:
						CView.text("Clara is heckling and throwing insults at your followers right now.  It's very annoying, but everyone ignores her.");
						break;
					case 2:
						CView.text("Clara is currently engaged in trying to tempt you into letting her out of her cage and becoming addicted to her.  It's much the same as all of her previous attempts....");
						break;
					case 3:
						CView.text("Clara is currently loudly whining about the conditions of her confinement to anyone who is willing to listen, which is no one.");
						break;
				}
			}
		}
