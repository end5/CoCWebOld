export class KihaScene extends NPCAwareContent {
	...
	public encounterKiha() {
		...
		//Tribute Offer (1st Time) - Req's Kiha not be mad and PC suggested it at some point. 
		else if (Flags.list[FlagEnum.KIHA_TOLL] === 1 && randInt(2) === 0) {
			...
			if (player.inventory.gems < 200) {
				DisplayText("You can't afford to pay her!");
				temp = null;
			}
			//[Pay] [This was my idea] [Leave] [Fight] - Leave uses standard leave text
			MainScreen.simpleChoices(["Fight", "Pay", "My Idea", "", "Leave"], [meetKihaAndFight, temp, tellKihaTributeWasYourIdea, null, leaveWhenMeetingAgressiveKiha]);
		}
		//*Repeat Encounter - Tribute Wore off 
		else if (Flags.list[FlagEnum.KIHA_TOLL] > 1 &&
			(Flags.list[FlagEnum.KIHA_TOLL_DURATION] === 1 ||
				(Flags.list[FlagEnum.KIHA_TOLL_DURATION] === 0 && randInt(2) === 0))) {
		...
		//Generic Repeat Encounter 
			...
			DisplayText("A nearby tree suddenly explodes in a shower of splintering wood and burning embers.  Kiha steps through the wreckage with her characteristic double-sided axe.\n\n");

			DisplayText("\"<i>Don't even start,</i>\" she commands, advancing on you as she continues, \"<i>I told you to avoid my domain and yet you ignored my imperative and returned.  You may be a fucking masochist or a retard, but trust me, you won't enjoy this.</i>\"\n\n");

			if (Flags.list[FlagEnum.KIHA_TOLL] === 0) {
				DisplayText("If you hurry, you might get a word in edge-wise.  What do you do?");
				//[Fight] [Ask Why][Buy Passage][Leave]
				MainScreen.simpleChoices(["Fight", "Ask Why", "Buy Passage", "", "Leave"], [meetKihaAndFight, askWhy, offerToBuyPassageFromKiha, null, leaveWhenMeetingAgressiveKiha]);
			}
			else {
				DisplayText("It's a fight!");
				startCombat(new Kiha());
			}
		}
	}
	//[Buy Passage] 
	private offerToBuyPassageFromKiha() {
		DisplayText().clear();
		DisplaySprite(72);
		DisplayText("You jingle your gem pouch and ask if you could pay her to allow you passage.  Kiha shoulders her axe and scratches at one of her horns, mulling the idea over.  She stops and abruptly shakes her head.  \"<i>Not today.  Now scram, before I change my mind!</i>\"\n\n");
		//(Unlocks toll option next encounter)
		Flags.list[FlagEnum.KIHA_TOLL] = 1;
		//[Fight] [Leave] - Same results as main fight/leave.
		MainScreen.simpleChoices(["Fight", "", "", "", "Leave"], [meetKihaAndFight, null, null, null, leaveWhenMeetingAgressiveKiha]);
	}
	//[Leave] 
	private leaveWhenMeetingAgressiveKiha() {
		DisplayText().clear();
		DisplaySprite(72);
		DisplayText("You nod and step back, retreating back towards camp.  You've no desire to fight such a fiery opponent.");
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	//[Fight]
	internal function meetKihaAndFight() {
		DisplayText().clear();
		DisplaySprite(72);
		DisplayText("You step closer and proclaim that you go where you please.  Kiha snorts and says, \"<i>Cute.  Sadly, misplaced confidence will only make this hurt that much more.</i>\"");
		//(START COMBAT)
		startCombat(new Kiha());
	}
	//[Ask Why]
	private askWhy() {
		...
		DisplayText("Thinking you may have found a way to win into the powerful warrior's good graces, you start to explain that your whole purpose here is to defeat the demon queen.  She cuts you off by exhaling a whirling tornado of fire into the air.  The scorching inferno seems to suck the very breath from your lungs, robbing you of your words.  While the after-image still hangs in your vision, she declares, \"<i>I don't give two licks about why you're here.  I'm gonna beat your ass just for wasting my time!</i>\"");

		//(START COMBAT!)
		startCombat(new Kiha());
	}
	//[Pay]
	private payKihaTribute() {
		DisplayText().clear();
		DisplaySprite(72);
		DisplayText("You sigh and pay her 200 gems.  She doesn't even mention that it was originally your idea, but still, you're getting what you want - the ability to explore her territory unhindered.  Of course, you have no idea how long this 200 gems will last.\n\n");
		//(gems -= 200;)
		player.inventory.gems -= 200;
		Flags.list[FlagEnum.KIHA_TOLL_DURATION] = 3 + randInt(4);
		//Count times paid
		...
		return { next: kihaExplore };
	}
	public kihaExplore(clearScreen: boolean = true) {
		if (clearScreen) DisplayText().clear();
		//DisplaySprite(72);
		Flags.list[FlagEnum.KIHA_TOLL_DURATION]--;
		let event: number = randInt(10);
		let itype: ItemType;
		...
		if (event < 5) {
			DisplayText("You wander around through the swamp for a while, but you don't find anything.");
			return { next: Scenes.camp.returnToCampUseOneHour };
			return;
		}
		//Reducto
		...
		DisplayText("While exploring, you find an item on the ground!  ");
		inventory.takeItem(itype, Scenes.camp.returnToCampUseOneHour);
	}

	//[This was my idea!]
	private tellKihaTributeWasYourIdea() {
		DisplayText().clear();
		DisplaySprite(72);
		...
		DisplayText("The dragon-girl yanks her axe from her back and twists her hands, making the edge glow orange in the murky swamp air.  \"<i>You should've kept your mouth shut, worm.  The offer is off the table now!  The only thing I'll trade you is pain!</i>\"");
		startCombat(new Kiha());
	}

	//*Generic PC Victory Introduction: 
	internal function kihaVictoryIntroduction() {
		Flags.list[FlagEnum.PC_WIN_LAST_KIHA_FIGHT] = 1;
		DisplayText().clear();
		DisplaySprite(72);
		DisplayText("Kiha sways back and forth for a moment, then drops her axe with numb hands.  As soon as she does, the hot glow of the weapon's cutting edge fades to silver, and the weapon lands with a heavy 'thunk' ");
		if (User.settings.silly()) DisplayText("(like Urta's cock!) ");
		DisplayText("in the dirt.  The dragoness drops to her knees and slumps back against a tree, her limbs trembling ");
		if (monster.lust > 99) DisplayText("with lust");
		else DisplayText("weakly");
		DisplayText(" as she tries to rise.\n\n");

		//(Lust) 
		if (monster.lust > 99) DisplayText("\"<i>You... can't make me like it!,</i>\" she shouts, struggling with all her might to keep her hands away from her genitals.  Somehow, she seems to be managing.");
		//(HP) 
		else DisplayText("\"<i>You... you... haven't... beaten me,</i>\" she mutters, even though it's quite clear that you have.");

		//(no new PG) 
		DisplayText("  What do you do with the wannabe dragon now that you've taken her down a peg?");
		//[OPTIONS]
		//All
		...
		if (player.torso.cocks.count > 0) {
			if (player.cockThatFits(monster.vaginalCapacity()) != -1 && player.stats.lust >= 33) {
				...
				if (player.cockThatFits2(monster.vaginalCapacity()) != -1 && player.torso.cocks.count > 1)
					DisplayText(" and ass");
				DisplayText(".");
	...
	internal function kihaLossIntro() {
		DisplayText().clear();
		DisplaySprite(72);
		//(Lust)
		if (player.stats.lust > 99) DisplayText("You give up on fighting, too horny to keep fighting.  Kiha strikes the ground with her axe and snorts out a puff of smoke.  \"<i>What a shameless slut!  You're lusting after me even in the heat of battle, like a common imp!  You aren't worthy to lick between my foot-claws!</i>\"\n\n");
		//(HP)
		else DisplayText("You collapse, too wounded to keep fighting.  Kiha strikes the ground with her axe and snorts out a puff of smoke.  \"<i>What a wimp!  I've barely started fighting and you're already beaten!</i>\"\n\n");

		//33% of tit-milk humiliation if applicable.
		if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 2) kihaMilkTitHumiliation();
		//Rape texts if horny
		else if (monster.lust >= 50) {
			DisplayText("The dragoness sighs and glances around.  \"<i>Since you weren't able to satisfy me in battle, let's see if you have anything else to offer...</i>\"\n\n");

			DisplayText("Leaving her axe to rest a few feet away, stuck on the ground, Kiha strips off your " + player.inventory.equipment.armor.displayName + " revealing your ");
			...
			if (player.gender === Gender.NONE) kihaGenderlessBeating();
			//*Male - Adj
			else if (player.gender === Gender.MALE) kihaRapesMen();
			//*Herm - Adj
			else if (player.gender === Gender.HERM) kihaRapesHerms();
			else if (player.gender === Gender.FEMALE) kihaRapesLittleGirlsISawItOnTheNews();
		}
		//If not horny, get a beating!
		else {
			DisplayText("Kiha pulls her axe back in a two handed grip, and you're sure the moment of your death is upon you.  A moment later, the flat of the blade slams into your head, knocking you unconscious.");
	...
	private kihaMilkTitHumiliation() {
		//(Does not use the defeat intro - clear screen)
		DisplayText().clear();
		DisplaySprite(72);
		if (player.stats.lust > 99) DisplayText("You slide to the ground, too turned on to fight and openly touching yourself.");
		else DisplayText("You slide to the ground, too defeated to continue to fight.");
		DisplayText("  Kiha snorts, expelling a tiny burst of flame and smoke from a nostril as she towers over your prone body.\n\n");
		...
		DisplayText("Kiha recoils at the drops of white beading atop your " + Desc.Breast.describeNipple(character, character.torso.chest.get(0)) + " and asks, \"<i>Do you really think that's hot?  It's disgusting.  A perversion of the body only a sick... MAMMAL would enjoy.</i>\"\n\n");

		DisplayText("The scaly woman leans down to tug on your " + Desc.Breast.describeNipple(character, character.torso.chest.get(0)) + "s in turn, laughing every time a spurt of milk erupts from one of your tits.  Each rough squeeze sends shivers of erotic sensation through your body, and you quickly grow aroused from the rough treatment.  Kiha keeps going, ");
		if (player.torso.cocks.count > 0) DisplayText("not noticing the stiffening in your loins");
		else if (player.torso.vaginas.count > 0) DisplayText("not noticing the scent of your growing need");
		else DisplayText("not noticing the flush of your skin");
		DisplayText(" as she callowly milks your breasts.  She isn't even blushing; if anything, her expression reminds you of a child playing with a new toy.  Kiha isn't aroused by this at all!\n\n");
		...
		DisplayText("Her hand cracks across your face, leaving a red print where it hits you.  \"<i>I said you should moo, cow.</i>\"\n\n");

		DisplayText("You blink away the tears that well up and nod, making a quiet, barely audible 'moo'.\n\n");
		...
		DisplayText("You trudge back to camp, replaying the erotic torture over and over in your over-sexed mind.  You just can't shake it, and once you get safe, you wind up spending hours masturbating, mooing over and over while you play with your body and further abuse your milky tits.  ");
		if (player.stats.cor < 33) DisplayText("How far you've fallen.");
		else if (player.stats.cor < 66) DisplayText("How confusing this all is.  Why can't things be simple like they were back home?");
		else DisplayText("Damn, that was hot.");
		player.orgasm();
		player.stats.tou += -1;
		player.stats.sens += 5;
		return { next: Scenes.camp.returnToCampUseOneHour };
	...
	//*Male - Adj
	private kihaRapesMen() {
		DisplayText().clear();
		DisplaySprite(72);
		let x: number = player.cockThatFits(monster.vaginalCapacity());
		if (x < 0) x = 0;
		DisplayText("You try to rise, but the dragon girl whips around, her long, thick tail delivering a subduing crack against the side of your head, dropping you back onto your " + Desc.Butt.describeButt(player) + ".  Triumphantly standing over you, the scaled woman eyes you with a panting breathlessness that betrays the coldness in her gaze.  She squints, arcing an imperious eyebrow as she looks you up and down, drawing her hands back to shoulder level and barely brushing against her breasts.  Her tail lashes back and forth between her legs, wrapping around her calves as she lifts a clawed foot and rests it lightly upon your stiffening groin.  Apparently, your fight left her wanting more than just your gems.  Her flexible toes squeeze your " + Desc.Cock.describeMultiCockShort(player) + " through your " + player.inventory.equipment.armor.displayName + ", and she snorts in derision.  \"<i>This is it, huh?  Pathetic.  I'm going to have to do all the work if I have any hope of you satisfying me.</i>\"\n\n");

		DisplayText("With a swift kick to your collarbone, Kiha knocks you on your back and stands over you, her vulva swollen with excitement despite her frosty insults.  She opens your " + player.inventory.equipment.armor.displayName + " with her long, clenching talons just enough to expose " + Desc.Cock.describeMultiCockSimpleOne(player) + ".  The dragoness curls her tail between her legs, the warm, scaled surface of the dexterous limb rubbing your tender flesh impatiently, tip flicking " + Desc.Cock.describeMultiCockSimpleEach(character) + " before coiling around the crest of your cockhead.  The narrow point of her backside pokes at your urethra, parting the narrow slit and tickling the sensitive, silken interior to draw a weak gasp from you.  \"<i>Yeah, I bet you'd love to get penetrated,</i>\" she scoffs, wrapping her hands around the thick girth of tail poking between her thighs like a massive, scale-textured cock.  She strokes the fat plates and thumbs the ridge-like nobs lining its underside with a hip-swaying motion that sends her chest wobbling.  \"<i>But,</i>\" she continues, her tail lifting off of your " + Desc.Cock.describeMultiCockShort(player) + " and curling upwards, \"<i>this isn't about what you want.</i>\"\n\n");
		...
		if (player.torso.hips.legs.isNaga() || player.torso.tailType === TailType.LIZARD) DisplayText("\"<i>Heh, if you had half as much dexterity as you do impudence, I'd let you do this for me.  Fat chance of that,</i>\" she remarks off-handedly.  ");
		DisplayText("Before long, her cunny practically drools, but you're not sure what she's getting off on more, the tail teasing her clit or your dick stiffening uncontrollably at her feet.  She grins, more to show her teeth than in pleasure, and begins to feed her tail into her snatch, the girl's folds parting eagerly to the rough limb, her honeypot polishing her scales to a liquid gleam almost immediately.  As she pumps long, slow thrusts inside herself, Kiha shifts to one leg, bringing her adept foot to your " + Desc.Cock.describeCock(player, x) + " once more");
		if (player.torso.balls.quantity > 0) DisplayText(", resting the heel of her foot against the swell of your " + Desc.Balls.describeBalls(true, true, player) + " and applying just enough pressure to feel the cum churning within");
		...
		DisplayText("The dragon girl's little show has already brought you to full mast and the pad-like scales of her foot rubbing up and down your length has you rock hard, helplessly twitching under her heel.  She hooks her long toes around your pulsing tip and pulls your " + Desc.Cock.describeCock(player, x) + " into the air, straining shaft rising in obedience to her demands.  Reluctantly withdrawing the tail from her slavering slit, Kiha crouches down until your flesh spear is parallel with her belly. She inches forward, your crown just a hair's breadth from her pussy and she makes a \"<i>tut</i>\" sound in the back of her throat.  \"<i>Considering the shitty battle you gave me, you certainly don't deserve this hole, nor any chance at fathering children.</i>\"  Despite her contempt, her lower lips splatter your " + Desc.Cock.describeCock(player, x) + " with the warm, liquid bounty of the dragon girl's vigorous masturbation.  You shudder at the heat showering your groin, unsure how much more teasing you can take.  Sliding forward a bit more, the victorious woman guides her narrow pucker over your throbbing meat.  \"<i>This should be more your speed, little fairy,</i>\" she taunts, before plunging herself onto your manhood.\n\n");

		DisplayText("The gushing honey pooled and slathered along her tail and asscheeks while she was frigging herself gives you more than enough lubrication to part the dragon's silken passage, muscled sphincter clenching rhythmically, more to pull you deeper inside than in response to the sudden intrusion.  You cry out at the intense tightness sucking at your " + Desc.Cock.describeCock(player, x) + " and reach up to grab the girl's breasts, but she smacks your hands away with a sharp blow.  \"<i>If you dirty my scales with your filthy mitts, I'll crush you between my thighs!</i>\" she gasps, running her own hands through her hair as her head rolls back in delight.  She rides you hard, bouncing off of your lap without regard for the bruising her athletic ass is giving your " + Desc.Hip.describeHips(player) + ", but you can hardly feel anything beyond the vice-like tightness of her rectum.  You open your mouth to beg her to ease up, but she uses the opportunity to thrust her tail between your parted lips, the warm appendage still dripping with girl-cum.  You nearly gag at the insertion, but the shock is quickly subordinated to arousal as your mouth is flooded with the second-hand taste of her sweaty nectar. You reflexively begin sucking on the scaled tip, wrapping your tongue around the scaly invader and even rolling your jaw as you lap Kiha's cream from her posterior.\n\n");

		DisplayText("With a rumbling growl, the dragoness impales herself on your " + Desc.Cock.describeCock(player, x) + " hard enough to knock the wind from your lungs. She clamps down and begins rolling her ass back and forth over your groin, stroking you off with only the rippling contractions from the toned walls of her rectum.  Your orgasm moments away, you helplessly clench your fingers in the earth");
		//[Lizard/naga tail: 
		if (player.torso.hips.legs.isNaga() || player.torso.tailType === TailType.LIZARD) DisplayText(" while your tail curls around to find your own asshole.  The grip of the dragon's nethers so intense, you barely feel the pressure of your own tail, pushing in easily to soothingly stroke your overtaxed prostate");
		...
		if (player.torso.balls.quantity > 0) DisplayText(", your " + Desc.Balls.describeBalls(true, true, player) + " practically leaping in over-burdened joy as their heavy load rushes through your flesh");
		DisplayText(".  A geyser of hot spunk sprays from your " + Desc.Cock.describeCock(player, x) + ", flooding her innards with the excruciatingly hefty torrent of seed all her teasing and abuse built within you.");
		if (player.torso.cocks.count > 1) {
			if (player.torso.cocks.count > 2) DisplayText("  Your unoccupied shafts spray ropey strands of cum all around you, jerking and straining under the weight of the dragoness.");
			else DisplayText("  Your unoccupied shaft sprays ropey strands of cum all around you, jerking and straining under the weight of the dragoness.");
		}
		DisplayText("  She pulls her sphincter tight to keep the fountaining jizz inside her and jams her tail half way down your throat until the thick flesh can't fit past your jaw anymore.  She wriggles the limb within you and pants sadistically as the bulge in your neck slides another inch deeper.  You begin to choke.\n\n");
	...
	private tsundereMasturbationChristmasCarol() {
		DisplayText().clear();
		DisplaySprite(72);
		DisplayText("You walk towards the draconic woman, removing your " + player.inventory.equipment.armor.displayName + " as you observe her firm, ");
		if (player.stats.lust > 99) DisplayText("wanton");
		else DisplayText("but dispirited");
		DisplayText(" figure slumping against the weathered bark behind her.  She eyes you carefully, trying to comprehend the situation before catching note of your ");
		if (player.gender > 0) DisplayText("aroused genitals");
		else DisplayText("distinct lack of genitals");
		DisplayText(".\n\n");

		//If Multi/Herm
		if (player.torso.cocks.count > 1 || player.gender === Gender.HERM) DisplayText("\"<i>W-what are you going to do, you freak of nature?  If you make me play with those, I swear... I swear I'll hit you!</i>\"");
		//If Cock
		else if (player.torso.cocks.count > 0) DisplayText("\"<i>G-get that away from me!  As if I'd want anything to do with your hard, juicy cock!  Idiot!</i>\"");
		//If Vagina
		else if (player.torso.vaginas.count > 0) DisplayText("\"<i>What do you think you're going to do with that pathetic, dripping pussy, huh?  I swear, if you make me l-lick that thing...</i>\"");
		//If Genderless
		else DisplayText("\"<i>H-huh?  Why the hell are you coming closer?  How can you possibly do anything when you're as smooth as a board down there, you loser!</i>\"");

		DisplayText("  Today, however, you don't have any plans to fuck her directly");
		if (player.gender === Gender.NONE) DisplayText(", and seeing as you don't have anything to fuck her with, it's not like you could");
		...
		if (player.tallness < 72) DisplayText("- though you're hidden behind her tallness");
		else DisplayText("- though you're much taller than her");
		...
		if (player.torso.cocks.count > 0) {
			DisplayText(Desc.Cock.describeMultiCockShort(player) + " against her soft flesh, ");
			if (player.torso.cocks.count === 1) DisplayText("a small bead of pre-cum forming from the tip and slathering her back with your juices.");
			else DisplayText("small beads of pre-cum forming from the tips and slathering her back with your juices.");
		}
		else DisplayText(Desc.Breast.describeNipple(character, character.torso.chest.get(0)) + "s against the hard ridges of her back, your tongue softly licking her smooth skin.");
		...
		DisplayText("Her moist juices flush from beneath her, drenching your fingers in her essence.  She slowly removes her lips from yours, moving closer towards you");
		if (player.torso.cocks.count > 0) {
			DisplayText(" and accidentally brushing against your trembling genitals, pushing you hard enough to cause you to ejaculate against her backside");
			player.orgasm();
		}
		DisplayText(".  The girl sighs in relief, tuckered out more from the sensual experience than the battle itself.  You slowly remove yourself, leaving the tired girl underneath the tree to rest.\n\n");

		DisplayText("\"<i>I... hate you... </i>\" she whispers, before turning in for a deep slumber.  You watch her soft face as she snores, graceful as a pig, before walking back to camp.");
		if (player.torso.cocks.count <= 0) player.stats.lust += 120;
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	//*Victory Tail-dildo, for girls - Fencrafted for maximum pomf (Zed)
	private kihaVictoryPomfTail() {
		DisplayText().clear();
		DisplaySprite(72);
		DisplayText("You approach the ");
		if (monster.lust > 99) DisplayText("lusty ");
		else DisplayText("wounded ");
		DisplayText("dragoness full of wicked ideas.  For her part, she looks up at you with a defiant gaze, as if it would somehow dissuade you from the sexual thoughts now guiding your body.  You circle to the side and reach down, grabbing her wriggling tail with both hands.  Kiha tries to pull it out of your hands, but in her weakened state, all it does is weakly flop like a caught fish.  Laughing, you smack her in the face with her own scaly hind-bits before you drop it.  You know just how you'll put that rudder to use, but first you've got to shuck your " + player.inventory.equipment.armor.displayName + ".\n\n");

		DisplayText("The gear lands behind you in a bed of moss");
		if (User.settings.silly()) DisplayText(" with a pomf =3");
		else DisplayText(".");
		DisplayText("  Now nude, you sway your " + Desc.Hip.describeHips(player) + " back and forth sensually, forcing Kiha to watch your body before you use her.  ");
		if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating <= 4) DisplayText("She looks away and blushes as she says, \"<i>It's not like I'd find such tiny breasts attractive!</i>\"  Her mannerisms seem to indicate that she does.");
		else if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating <= 12) DisplayText("She sighs and says, \"<i>As if I'd be impressed by udders like those!</i>\"");
		else DisplayText("She laughs and taunts, \"<i>How do you even walk with ridiculous hooters like those?  Go on, moo, cow!</i>\"");
		...
		DisplayText(" and sweat-soaked underwear.  The black, crimson-embedded slits of her eyes narrow in barely suppressed rage, but as she is now, she's no threat to you.\n\n");
		...
		if (player.torso.vaginas.get(0).wetness <= 3) DisplayText("grows wet");
		else if (player.torso.vaginas.get(0).wetness <= 4) DisplayText("quickly grows slimy");
		else DisplayText("immediately becomes soaked from your constant drippings");
		DisplayText(".\n\n");

		DisplayText("Satisfied that it is sufficiently lubricated, you squeeze the now-slippery tail and point its nubby tip at your entrance.  It bumps your " + Desc.Vagina.describeClit(player) + " as you wrangle with the struggling hind-limb, only turning you on all the more.  Feeling the tip circling your tunnel's opening, you push it up and seat the tentacle-like tail deep inside yourself.  ");
		Mod.Vagina.displayStretchVagina(player, 1, true, false, true);
		DisplayText("Gods she's so... warm, and the ridges, they're stroking every inch of your interior, even your g-spot!  Your " + Desc.Leg.describeLegs(player) + " wobble unsteadily, but spurred by lust, you keep an iron grip on your new-found toy.  Kiha doesn't stop resisting either.\n\n");

		DisplayText("Leaning back against the tree the dragon previously occupied, you slide down, letting your " + Desc.Leg.describeLegs(player) + " go completely nerveless.  Kiha's struggles unintentionally add to your pleasure, turning her tail into the world's warmest, most powerful vibrator.  Your hands begin to push more of her girthy thickness inside, stretching your poor puss further and further.  ");
		Mod.Vagina.displayStretchVagina(player, 35, true, false, true);
		...
		DisplayText("The draconic woman snaps when you pull your soiled undies from her maw.  \"<i>Bitch!  I'll get you for t-this!</i>\"\n\n");
	...
	private victoryDickKiha() {
		...
		if (monster.lust > 99) DisplayText("Just because you've aroused my body doesn't mean I'll enjoy this!");
		else DisplayText("Don't think I'll enjoy this!");
		DisplayText("</i>\"\n\n");

		DisplayText("Kiha certainly has a mouth on her, but then again, you aren't interested in that particular orifice at the moment.  You lift her leg, but she curls her tail up to obscure her pussy from your view");
		if (monster.lust > 99) DisplayText(", rubbing it back and forth against her folds without thinking about it");
		DisplayText(".  Annoyed, you twist the rebellious appendage out of your way but struggle to hold it back.  The dragon isn't making it easy for you for in the slightest.  Still, you get a nice long look at her soaked sex, and the sight ");
		if (player.stats.lust > 60) DisplayText("only makes " + Desc.Cock.describeMultiCockSimpleOne(player) + " harder.");
		else DisplayText("quickly makes " + Desc.Cock.describeMultiCockSimpleOne(player) + " hard");
		DisplayText(".  Her rear-most extremity yanks out of your grip to cover her femininity once more and you let it, for now.\n\n");

		DisplayText("Pausing for a moment, you set to removing your " + player.inventory.equipment.armor.displayName + " and unleashing " + Desc.Cock.describeMultiCockSimpleOne(player) + " ");
		...
		DisplayText("You advance on her helpless form and repeat your actions, raising her leg and shoving her tail aside.  She resists, but not as fiercely as before.  Her tail is easily kept at bay while you lower yourself to align your " + Desc.Hip.describeHips(player) + " with the Kiha's moist cunt.  The heat rolling out of her loins is unreal.  Her pussy seems to fog the air with its combination of warmth and wetness, and as you press your " + Desc.Cock.describeCock(player, x) + " closer, it feels almost like a woman is about to give you head, with her hot breath washing over your genitals.\n\n");

		//(DOUBLE DICKING VARIANT)
		if (y != -1 && player.torso.cocks.count > 1) {
			DisplayText("That perfect, dripping entrance beckons for your " + Desc.Cock.describeCock(player, x) + ", but your other penis");
			if (player.torso.cocks.count === 2) DisplayText(" is");
			else DisplayText("es are");
			DisplayText(" aching so much that you may as well try to double your pleasure and fun.  Without hesitation, you slide two cocks inside your unsuspecting partner.  The top one smoothly sheaths itself inside Kiha's velvety tunnel.  Her tightly-clenched pucker proves more resistant to your " + Desc.Cock.describeCock(player, y) + "'s sudden intrusion, pushing back with near painful force as inch after inch penetrates her delicious rear passage.  One last, glorious push completely buries both your bones in their draconic sockets, much to the delight of your captive, if her moans are to be believed.\n\n");
			...
			DisplayText("You sigh and slap her face, informing her that she should shut her trap and consider herself lucky you're spending the time to pleasure her body while you sate yourself.  Kiha barks, \"<i>Yeah right!  As if I would enjoy something like th-</i>\"\n\n");
			...
			while (temp > 0) {
				temp--;
				if (player.torso.cocks.get(temp).type === CockType.TENTACLE && temp != x && temp != y) {
					z = temp;
					break;
				}
			}
			if (player.torso.hips.legs.isNaga() || player.torso.tailType === TailType.LIZARD || z >= 0) {
				DisplayText("Spitefully, your ");
				if (z === -1) DisplayText("tail");
				else DisplayText("idle tentacle");
				...
				if (z >= 0) DisplayText(", plunging the head of your cock in and out of her throat.");
				DisplayText("\n\n");
			}

			DisplayText("Dully, you realize " + Desc.Cock.describeMultiCockSimpleOne(player) + " are twitching, her powerful clenching holes combining with your frenzied strokes too much for you to endure.  Heat boils out from your " + Desc.Balls.describeBalls(true, true, player) + " as your orgasm approaches, threatening to burst from your loins even as blackness eats at the edges of your vision.  Screw it - if she's choking you out, you're at least going to get to cum first.  You grab both her nipples as you hilt yourself and twist, HARD.  Jizm spurts from your cocks, glazing Kiha's hot, juicy puss,");
			if (player.torso.hips.legs.isNaga() || player.torso.tailType === TailType.LIZARD || z >= 0) DisplayText(" gasping mouth,");
			DisplayText(" and tight anus with waves of white.");
			if (player.cumQ() >= 500) DisplayText("  Her belly distends before long, stuffed by the heavy loads of cream you push out.");
			if (player.torso.cocks.count > 2) {
				DisplayText("  You even coat her belly and tits with seed thanks to your extra penis");
				if (player.torso.cocks.count > 3) DisplayText("es");
				DisplayText(".");
			}
			DisplayText("\n\n");
		...
		//(SINGLE DICKING VARIANT)
			...
			DisplayText("That perfect, dripping entrance beckons for your " + Desc.Cock.describeCock(player, x) + ", and without hesitation, you slide inside.  Kiha growls, \"<i>That's it?  How artless.</i>\"  In spite of her disappointed tone, her slippery passage squeezes down on your member with miraculous tightness, embracing your " + Desc.Cock.describeCock(player, x) + " in its firm, velvet grip.  You sigh happily and enjoy the dragon-pussy's pressure for a few moments, tuning out Kiha's griping to focus on the heavenly feelings her body is giving you.\n\n");

			DisplayText("Eventually, you become accustomed to the feel of the reptilian tunnel's simmering juices, and eager for more stimulation, you begin to saw your " + Desc.Cock.describeCock(player, x) + " in and out of sodden box.  The draconic woman beneath you lets out whimpers and moans of pleasure mixed with discontent, unable to resist you and unable to allow herself to enjoy it.  She squirms and wriggles beneath your forced affections, growing ");
			if (monster.lust > 99) DisplayText("more aroused despite her feigned indifference");
			else DisplayText("very aroused in spite of her desire to fight you");
			DisplayText(".  For your part, you pick up the pace, pushing yourself faster and faster towards an eventual climax.  Kiha places her scaled fingers on her chest to stop it from jiggling in response, but you push them away, wanting her breasts to bounce freely for your amusement.\n\n");

			DisplayText("All too soon you find your " + Desc.Cock.describeCock(player, x) + " giving in to the pleasure of Kiha's sizzling snatch, warmth building beneath your penis as it nears climax.  You let go of her tail as you increase your tempo and push your body towards release.  The dragon's scaled appendage immediately sets to work on your back, battering you with heavy slaps that could seriously damage you if her full strength were behind them.  As it is, they may leave a few bruises.  You tweak one of Kiha's nipples and smile as she moans in sudden pleasure, the lurid vocalization being just what you need to hear to cum inside her.\n\n");

			DisplayText("You push yourself in all the way to the hilt and unload, firing jet after jet of thick cream into the dragon's dribbling pussy, only making it leak even more.  ");
			if (player.cumQ() >= 500) DisplayText("A chubby bulge appears on her taut, defined belly, the only visible sign of your copious cum production.  ");
			if (player.cumQ() >= 1000) DisplayText("Even after that, thick gouts of the stuff spray out through her cock-stuffed lips, forming a puddle beneath your foe.  ");
			DisplayText("You pull out");
			if (player.stats.cor < 33) DisplayText(", clean up, and get dressed, leaving Kiha behind to consider the effects of her arrogance.");
			else if (player.stats.cor < 66) DisplayText(", wipe your cock off on her face, and get dressed, leaving Kiha to consider being a little more humble.");
			else DisplayText(", smear your dribbling cock all over her face, make her lick it clean, and then, get dressed.  Maybe the bitch will consider a little humility when addressing the real owner of this territory - you.");
			DisplayText("  Somehow, you doubt she'll learn anything from the encounter.");
		}
		player.orgasm();
		return { next: Scenes.camp.returnToCampUseOneHour };
	}

	private kihaRapesLittleGirlsISawItOnTheNews() {
		DisplayText().clear();
		DisplaySprite(72);
		...
		DisplayText("Despite the pleasure of your crumbling restraint, the pleading shudders of your cumming body makes her even more aggressive than before.  \"<i>Yes!  Beg for mercy!  You don't deserve a minute of my time! How could someone like me ever be interested in a pig like you?</i>\"  To punctuate the game she's playing with your honeypot, Kiha pinches your " + Desc.Vagina.describeClit(player) + " between her fingers, the narrow nails of her claws squeezing your sensitive nub so tightly your jaw drops in a soundless scream of bliss.\n\n");

		DisplayText("\"<i>Here's how this is going to go,</i>\" she murmurs when you've finally stopped gasping for breath.  \"<i>You will lick me until I tell you to stop, and every time you displease me I'm going to take it out on your sensitive little fuck hole.</i>\"  She swings around and sits on your face, smashing her wet cunt on your " + Desc.Face.describeFace(player) + ".  \"<i>Be grateful that I'm letting you do this,</i>\" she gurgles, pinching your clit again for emphasis.  Doing as you're told, you lick deeply into the warm velvet of the dragon's folds.");
		if (player.torso.head.face.tongue.type === TongueType.SNAKE) DisplayText("  You are grateful for your forked tongue, which allows you to cover more of her pussy at once.");
		else if (player.torso.head.face.tongue.type === TongueType.DEMONIC) DisplayText("  Your inhumanly long, prehensile tongue is a blessing here; you can reach into her deepest recesses, slurp your way into every nook and cranny, and generally provide a tongue-fuck that no human ever could.");
		DisplayText("\n\n");

		DisplayText("Despite your ethusiastic efforts, no amount of lapping, sucking, and nibbling seems able to please her and, after a few minutes, you feel another painful pinch on your " + Desc.Vagina.describeClit(player) + ", your pink pearl throbbing between her fingers. \"<i>That's not how you do it! Since you're so incapable, I'll have to show you how to do it! Take notes.</i>\" She bends over, the heat of her face breathing a scintillating curtain of warmth over your quivering quim. Shoving her nimble tongue into your pussy with a lightning-quick motion, you tremble at the delicious wetness of her tingling saliva licking the depths of your loins by dragging the her muscled tip across your flesh, before blowing a hot gasp inside your vulnerable depths, pleasure rippling through your walls.\n\n");

		DisplayText("You shudder in ecstasy tinged with fear and redouble your efforts, slurping down the now steadily flowing juices of your reptilian tormentor like a woman dying of thirst who is suddently confronted with a fresh spring of water. With another moan, Kiha looses a gasp of heated air that nearly feels like agonizingly wonderful flames into your aching snatch and you nearly go over the edge, your juices evaporating into a lewd puff of lusty perfume that wafts into the dragoness' nostrils. \"<i>Yes, cum for me,</i>\" she coos, as she grinds her pussy against your " + Desc.Face.describeFace(player) + ", flooding your mouth and nose with her own smoldering fragrance.\n\n");

		DisplayText("Unable and unwilling to do anything but obey, you climax with a shuddering tensing of your muscles that vibrates the dragon lying atop you.");
		if (player.torso.vaginas.get(0).wetness >= 5) DisplayText("  Your juices spray into her face like cum from a cock, your quantity of lubricants unable to resist cascading from the pressure of your orgasm.");
		DisplayText("  She responds in kind, gushing her unspoken attraction across your clenched features. Drowning in dragon juices and shuddering in pleasure, you finally pass out.\n\n");

		DisplayText("By the time you come to, Kiha is long gone.");
		player.orgasm();
		player.stats.sens += 2;
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	private rapeKihaWithWORDS() {
		DisplayText().clear();
		DisplaySprite(72);
		if (Flags.list[FlagEnum.KIHA_TALK_STAGE] === 0) {
			DisplayText("Kneeling down, you meet the defeated dragoness' gaze and ask her why she's so violent and territorial.  Kiha looks up at you in confusion and asks, \"<i>Wait... you defeated me, and you want to... talk?</i>\"\n\n");
		...
		//Conversation Level 2 (40- Corruption! PUREBABIES ONLY!) (Zed)
			...
			DisplayText("Once again, you crouch down and begin talking with your foe.  Kiha immediately sighs and says, \"<i>This again? I thought I told you - you're wrong.</i>\"\n\n");

			DisplayText("You ask her if she really believes that.  After all, you've beaten her twice now.  Kiha shudders and yells, \"<i>I just said so!  Gods, don't be so... so fucking dense!  If you were right, Lethice wouldn't be on her throne and they never would've been able to make monsters like... ahh, n-never mind.  Dumbass!</i>\"\n\n");
			...
			DisplayText("Once you finish she comes out of it and mutters, \"<i>So you had it hard?  It doesn't matter.  I had it harder.</i>\"  She launches into the air and flies away before you can answer.");
			if (silly() && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00281] > 0 && telAdre.lottie.lottieMorale() <= 33) DisplayText("  Is that what it feels like to be Lottie?");
		}
		//Conversation Level 3 (40- Corruption! PUREBABIES ONLY!) (Zed)
		else if (Flags.list[FlagEnum.KIHA_TALK_STAGE] === 2) {
			DisplayText("Even though you've taken the dragoness down again, you settle down for another chat instead of anything more carnal.  Kiha smiles at that, relaxing a little as she asks, \"<i>Still sticking to your morals, huh?</i>\"\n\n");
			...
			DisplayText("You nod again to which she replies, \"<i>Not even if pigs could fly!  And before you suggest it, pig-nosed imps don't count.</i>\"  You fold your arms across your chest expectantly and remind her that you're the victor here.\n\n");

			DisplayText("Kiha raves, \"<i>You won because... I LET you win.  I just wanted to see how much information you'd give up.</i>\"\n\n");
			...
			DisplayText("The draconic woman looks very much like a young, nervous girl in that moment.  Her cheeks blush and she leaps up, flying away before you can ask her any more.");
		}
		//Conversation Level 4: Requires Low Corruption, Repeating for now (Zed)
		else {
			DisplayText("You sit down next to Kiha and start to talk again.  While she initially gives the barest hint of a smile, she soon begins to sulk.  No matter what you say or do you can't seem to bring her out of it.  Perhaps for now, there's simply no more to discuss with her.");
		}
	...
	*/
	...
	private analRapuzulaKiha() {
		...
		DisplayText("You watch the dragoness slump against the tree, her tail barely covering her soaked snatch.  You lick your lips hungrily at the prospect of ravishing the haughty dragon-girl's cunny, but decide she deserves something a bit more special.  You want that tight ass of hers, and you want it now.\n\n");

		DisplayText("You tell her to turn around but, still defiant, she refuses.  Glowering, you use your most commanding voice and bellow the command at her.  She still refuses.\n\n");
		if (x.area <= 20) DisplayText("You quickly shed your " + player.inventory.equipment.armor.displayName + ", letting your " + Desc.Cock.describeMultiCockShort(player) + " out to breathe in the warm, swampy air.  Her red eyes widen slightly, but she quickly regains her composure, suddenly all confidence despite her obvious loss.  \"<i>Oh? What do you think you're gonna do with THAT, huh?  Just ravish me like some kind of sex-crazed demon? Is that your genius plan?</i>\"\n\n");
		else DisplayText("You quickly shed your " + player.inventory.equipment.armor.displayName + ", letting your " + Desc.Cock.describeMultiCockShort(player) + " out to breathe in the warm, swampy air. Her red eyes widen slightly, but she quickly regains her composure, suddenly all confidence despite her obvious loss.  \"<i>Hah, look at that pathetic thing.  You might have gotten me a little hot, but I really doubt you could please anyone with that puny piece of equipment.</i>\"\n\n");

		DisplayText("Her words sting, and you resolve to make sure she enjoys what you're about to do.  You approach the defensive dragon and take her arms in your hands and turn her around.  Surprisingly, she resists your movements very little, only struggling when you aggressively push her face into the tree.\n\n");
		...
		DisplayText("As soon as your fingers touch her skin, her tail whips around your " + Desc.Cock.describeMultiCockShort(player) + ", wrapping up your groin");
		if (player.torso.balls.quantity > 0) DisplayText(" and threatening to crush your " + Desc.Balls.describeBalls(true, true, player));
		DisplayText(".  \"<i>Ooh, you're a feisty one, aren't you,</i>\" you goad. \"<i>You can dish out the snappy insults just fine, but taking them... Your temper is horrible.</i>\"  You wince in pain, as the scaly tail wrapped around your genitals tightens in response.\n\n");
		...
		if (player.stats.cor < 33) DisplayText("You move your hands from her two mounds and down her sides, resting just above her hips. Hoping that this will work, you begin to tickle the stubborn girl.  ");
		else if (player.stats.cor < 66) DisplayText("Since she seems to be concentrating on your hands, you frantically try to think of a solution that doesn't involve them.  An idea forms and, deciding it's now or never, you lean forward and nibble on the tip of her ear.  ");
		else DisplayText("Since she seems to be concentrating on your hands at the moment, your mind races trying to find a solution that doesn't involve them.  An idea forms and, deciding it's now or never, you lean forward and slowly slide your tongue across her cheek.  ");

		DisplayText("She shrieks in shock from your sudden treatment. Having the desired effect, your distraction gives you the split second you need to slip your genitals out of her tail's grasp. You quickly place your " + Desc.Cock.describeCock(player, x) + " at her tight pucker, and forcefully slide forward.  Your head breaks through the tight ring and pushes its way inside.  Your surprise teasing seems to have shocked her enough to stop clenching her ass, and the additional lube helps in letting you sheath half your length inside.  The dragoness moans in what you assume is pleasure, finally breaking the cold facade that she's been so desperately trying to maintain.\n\n");

		DisplayText("The heat that now encircles your " + Desc.Cock.nounCock(player.torso.cocks.get(x).type) + " is tremendous; her tightness, incredible.  You're nearly overwhelmed by the urge to slam your whole length home and just cum your brains out.  But you want her to enjoy this, too.  You slowly slide back out of her rectum, stopping when just the tip of the head is barely inside. You lean forward, resting your chest against the warm skin of her back, and thrust your " + Desc.Cock.describeCock(player, x) + " back into the crushing heat of her asshole.\n\n");
		...
		if (y >= 0) {
			...
			DisplayText("You pull out one final time, placing them at her gaping ring.  Knowing what you're about to do, the dragon visibly relaxes in an attempt to make it as painless as possible.  Getting a good grip on her hips, you thrust your dongs forward and pull her back onto your cocks simultaneously.  Your " + Desc.Cock.describeCock(player, y) + " slips from the exposed hole and slides itself between her buttcheeks, while your remaining " + Desc.Cock.describeCock(player, x) + " hits home.\n\n");

			DisplayText("Kiha shudders in her own climax, soaking her thighs as you finally release your seed into her ravaged ass.  Your other " + Desc.Cock.nounCock(player.torso.cocks.get(y).type) + " follows its brother, painting her leather-colored skin with alabaster cream");
			if (player.torso.cocks.count >= 3) {
				if (player.torso.cocks.count === 3) DisplayText("; your remaining cock tarnishes the ground with your cum");
				else DisplayText("; your remaining cocks tarnish the ground with your cum");
			}
			else if (player.torso.vaginas.count > 0) DisplayText("; your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " pulses with pleasure, soaking your thighs with girl-cum as your cocks unload");
			DisplayText(".\n\n");

			DisplayText("The dragoness slumps and falls to the ground, drawing your spent shaft from her ass's depths.  It appears that she has fainted from overwhelming pain, pleasure, and exhaustion. Deciding that you're done here, you retrieve your " + player.inventory.equipment.armor.displayName + " and leave the girl in a mixed puddle of your cum and hers.  Perhaps now she'll treat you with a little more respect.  You return to camp.");
		}
		else {
			if (x.area <= 20) DisplayText("You move your hands to her breasts, feeling the lightly scaled flesh and begin tweaking her nipples in time to your continued thrusting.  She breaks her self-imposed silence with a long, pleasured sigh, but cuts it short when you laugh at her body's betrayal of her emotions.  \"<i>H-how can anyone even l-like this anyway?</i>\" she pants with barely contained pleasure.  \"<i>All you're doing is sticking your pathetic dick inside something that it isn-isn't designed for.  I-I can barely feel your ti-tiny little cock... uhh... stretching my ass, filling it with s-such force.  Its head tickling my walls with your... ohhh... pathetic attempts to get yourself o-off...</i>\"\n\n");
			else DisplayText("You move your hands to her breasts, feeling the lightly scaled flesh and begin tweaking her nipples in time to your continued thrusting.  She breaks her self-imposed silence with a long, pleasured sigh, but cuts it short when you laugh at her body's betrayal of her emotions.  \"<i>H-how can anyone even l-like this anyway?</i>\"  She pants with barely contained pleasure.  \"<i>Dicks don't even belong there, you... you freak!  It... it doesn't even feel good... stretching my ass, filling it with s-such force.  Its head tickling my walls with your... ohhh... pathetic attempts to get yourself o-off...</i>\"\n\n");

			DisplayText("Her moans become louder as you continue the butt-fuck, her hole becoming as wet as her pussy as your intruding member slides in and out of her dilated sphincter.  Her involuntary signs of enjoyment spur you on and pre flows freely from your member, allowing you to ignore her crushing tightness and enjoy the hot pleasure that envelops your " + Desc.Cock.describeCock(player, x) + ".  You doubt that even her pussy's depths would give you this kind of feeling.\n\n");

			DisplayText("You give her nipples one more rub, not realizing that she was fighting off her own orgasm.  Her spasming walls clench hard against your cock, eagerly milking you while her thick girl-cum soaks her thighs.\n\n");

			DisplayText("Grunting, you thrust one last time and spill your seed into her, filling her rectum with sticky cum.  ");
			if (player.torso.vaginas.count > 0) DisplayText("Your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " pulses with pleasure, but it's an empty orgasm, leaving you wishing you had something to fill that aching void.  Still, you cannot help but marvel at the superior feeling of cumming with a multitude of sexual organs.  ");
			DisplayText("With a contented sigh, you pull out of the dragon and thank her for the good time.  She sits back against the tree and glares at you angrily, though you can easily tell she's been utterly sated.\n\n");

			DisplayText("Deciding that you are done here, you retrieve your " + player.inventory.equipment.armor.displayName + " and leave the girl. Perhaps now she'll treat you with a little more respect. You return to camp.");
		}
		player.orgasm();
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
